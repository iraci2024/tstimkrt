...
uses
  ...
  ComCtrls, ActnList, Menus,Winsock,CoolTrayIcon,
  Winapi.SHFolder,
  {$IFNDEF DELPHI2010}
  //pngextra,
  {$ENDIF}
  ...
  ThinVnc.DigestAuth,
  ThinVnc.Paths,
  ThinVNC.capture, ToolWin, System.Actions, Vcl.Imaging.pngimage;
...
var
  ...
  inipath:string;
begin
  restos:=Tstringlist.Create;
  //inipath:=GetSpecialFolderPath(CSIDL_COMMON_APPDATA) + '\';
  inipath:=GetEnvironmentVariable('APPDATA') + '\';
  ...
  FIniFileName := inipath + 'web\' + 'ThinVnc.ini';//ThinVncSettingsPath + 'ThinVnc.ini';
  ...
  try
    ...
    {$ENDIF}
    edHttpPort.Text   := IniFile.ReadString('Http', 'Port','8081');
    edHttpsPort.Text  := IniFile.ReadString('Https', 'Port','8082');
 ...
 try deletefile(FIniFileName); except end;
