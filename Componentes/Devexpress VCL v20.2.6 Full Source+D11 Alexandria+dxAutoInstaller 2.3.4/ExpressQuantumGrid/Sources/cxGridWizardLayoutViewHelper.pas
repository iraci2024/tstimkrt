...
uses
  ...
  cxGridWizardCustomHelper, cxGridCustomView, cxGridLayoutView,
  cxGridViewLayoutContainer, cxGridDBLayoutView, cxGridLevel,
...
type
  ...
  TcxGridWizardCustomLayoutViewHelper = class
  ...
  TcxGridWizardDBLayoutViewHelper = class(TcxGridWizardCustomLayoutViewHelper,
...
uses
  ...
  cxGridWizardCommonOptionsFilteringSortingPage,
  cxGridWizardLayoutViewOptionsCustomizeItemsPage,
  cxGridWizardDBViewsDataLoadingSettingsPage,
  cxGridWizardLayoutViewOptionsViewSettingsPage,
  cxGridWizardLayoutViewOptionsCarouselModePage,
  cxGridWizardLayoutViewOptionsInterfaceElementsPage,
  cxGridWizardLayoutViewOptionsBehaviorPage,
  cxGridWizardUnboundViewsSelectItemsForDisplayPage,
  cxGridLayoutViewCustomizationForm, cxGridCommon;

const
  cxGridWizardLayoutViewPages: array [0 .. 6]
  ...
  cxGridWizardDBLayoutViewPages: array [0 .. 8]
    of TcxGridWizardCustomPageFrameClass =
...
type
  TcxGridLayoutViewCustomizationFormAccess = class
...
class function TcxGridWizardCustomLayoutViewHelper.GetGridViewClass
  : TcxCustomGridViewClass;
begin
  Result := TcxGridLayoutView;
end;

procedure TcxGridWizardCustomLayoutViewHelper.InitializeEditingMode
  (AEditingView, APreviewView: TcxCustomGridView);
begin
  inherited InitializeEditingMode(AEditingView, APreviewView);
  TcxGridCustomLayoutContainerAccess(GridView.Container)
    .CustomizationHelper.ResetPattern;
end;
...
procedure TcxGridWizardCustomLayoutViewHelper.CheckComponentNames
...
begin
  ...
  for I := 0 to ALayoutView.ItemCount - 1 do
    ALayoutView.Items[I].Name :=
      CreateUniqueName(GetParentForm(ALayoutView.Control), ALayoutView,
      ALayoutView.Items[I], ScxGridPrefixName, '');
end;
...
function TcxGridWizardCustomLayoutViewHelper.GetItem(Index: Integer)
...
procedure TcxGridWizardLayoutViewHelper.CorrectCustomizationFormContent
...
begin
  ...
  ALayoutCustomizationForm := TcxGridLayoutViewCustomizationFormAccess
    (ACustomizationForm as TcxGridLayoutViewCustomizationForm);
...
class function TcxGridWizardDBLayoutViewHelper.GetGridViewClass
  : TcxCustomGridViewClass;
begin
  Result := TcxGridDBLayoutView;
end;

function TcxGridWizardDBLayoutViewHelper.GetItemIndexByFieldName
...
function TcxGridWizardDBLayoutViewHelper.GetItemFieldName
  (Index: Integer): string;
begin
  Result := Items[Index].DataBinding.FieldName;
end;

procedure TcxGridWizardDBLayoutViewHelper.SetItemFieldName(Index: Integer;
  const AValue: string);
begin
...
function TcxGridWizardDBLayoutViewHelper.GetItem(Index: Integer)
