...
uses
  Variants, Windows, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  ExtCtrls, StdCtrls, ComCtrls, Menus,
  cxClasses, cxCustomData, cxDBData, cxNavigator, cxGridCustomView,
  cxGridCustomTableView, cxGridStructureNavigator,
  cxViewEditor, cxLookAndFeelPainters, cxButtons, cxPC, cxControls, cxGraphics,
  cxLookAndFeels, dxLayoutLookAndFeels,
  dxLayoutContainer, dxLayoutControl, dxLayoutControlAdapters, cxContainer,
  cxEdit, cxListBox, dxCoreGraphics,
  cxCustomListBox;
...
type
  TcxCustomTableViewEditor = class(TcxViewEditor)
    ...
    procedure LBColumnsDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
  ...
  protected
    ...
    procedure CollectionItemsDelete(AListBox: TListBox;
      ACollection: TCollection);
    procedure DoCreateViewItems(AMissingItemsOnly: Boolean);
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas;
      AIndex: Integer; R: TRect); overload; virtual;
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas;
      R: TRect; const APrefix: string; const ASuffix: string = '';
      ASuffixPos: Integer = 0); overload; virtual;
...
begin
  ...
  else if APersistent is TcxDataSummaryItems then
    ADataController := (APersistent as TcxDataSummaryItems).DataController
  else if APersistent is TcxDataSummaryGroups then
    ADataController := (APersistent as TcxDataSummaryGroups)
      .Summary.DataController
  else if APersistent is TcxDataSummaryGroup then
    ADataController := (APersistent as TcxDataSummaryGroup)
      .SummaryGroups.Summary.DataController
  else if APersistent is TcxDataSummaryGroupItemLinks then
    ADataController := (APersistent as TcxDataSummaryGroupItemLinks)
      .SummaryGroup.SummaryGroups.Summary.DataController;
  if (ADataController <> nil) and Supports(ADataController,
    IcxCustomGridDataController) then
  begin
    Supports(TObject(ADataController), IcxCustomGridDataController,
      AIDataController);
...
begin
  (AView.DataController as IcxCustomGridDataController)
    .CreateAllItems(AMissingItemsOnly);
end;

{ TcxCustomGridTableViewStorage }

function TcxCustomGridTableViewStorage.GetNavigatorButtons
...
class function TcxCustomGridTableViewStorage.GetViewClass
...
class function TcxCustomTableViewEditor.GetViewByObject
...
procedure TcxCustomTableViewEditor.CalculateListBoxItemHeight
  (AListBox: TListBox);
begin
  AListBox.ItemHeight := ScaleFactor.Apply(2) + TdxTextMeasurer.TextLineHeight
    (AListBox.Font);
end;
...
procedure TcxCustomTableViewEditor.DoCreateViewItems(AMissingItemsOnly
  : Boolean);
...
procedure TcxCustomTableViewEditor.DrawListBoxItem(AListBox: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; R: TRect);
begin
  DrawListBoxItem(AListBox, ACanvas, R, AListBox.Items[AIndex]);
end;

procedure TcxCustomTableViewEditor.DrawListBoxItem(AListBox: TcxListBox;
  ACanvas: TcxCanvas; R: TRect; const APrefix: string;
  const ASuffix: string = ''; ASuffixPos: Integer = 0);
begin
  ACanvas.FillRect(R);
  if APrefix <> '' then
...
function TcxCustomTableViewEditor.GetCurrentGroup: TcxDataSummaryGroup;
  ...
  function GetGroupByItems(ASummaryItems: TcxDataSummaryItems)
    : TcxDataSummaryGroup;
  var
    I: Integer;
  begin
  ...
  function GetGroupByItemLinks(ASummaryGroupItemLinks
    : TcxDataSummaryGroupItemLinks): TcxDataSummaryGroup;
  var
    I: Integer;
  begin
    Result := nil;
  ...
  begin
    ...
    else if APersistent is TcxDataSummaryItem then
      Result := GetGroupByItems(TcxDataSummaryItem(APersistent).SummaryItems)
    else if APersistent is TcxDataSummaryItems then
      Result := GetGroupByItems(TcxDataSummaryItems(APersistent))
    else if APersistent is TcxDataSummaryGroupItemLink then
      Result := GetGroupByItemLinks(TcxDataSummaryGroupItemLink(APersistent)
        .SummaryGroupItemLinks)
    else if APersistent is TcxDataSummaryGroupItemLinks then
  ...
  try
    ...
    begin
      ...
      if (ASummaryGroup <> nil) and (GetViewByObject(ASummaryGroup) = View) then
        ...
        else if Result <> ASummaryGroup then
        begin
          Result := nil;
          Break;
        end;
...
begin
  ...
  BDeleteFooterSummaryItem.Enabled := CanDeleteComponent(nil) and
    (LBFooterSummary.SelCount > 0);

  BDefaultGroupSummaryAdd.Enabled := CanAddComponent;
  BDefaultGroupSummaryDelete.Enabled := CanDeleteComponent(nil) and
    (LBDefaultGroupsSummary.SelCount > 0);

  BSummaryGroupAdd.Enabled := CanAddComponent;
  BSummaryGroupDelete.Enabled := CanDeleteComponent(nil) and
    (LBSummaryGroups.SelCount > 0);
  BGroupSummaryItemAdd.Enabled := CanAddComponent and (GetCurrentGroup <> nil);
  BGroupSummaryItemDelete.Enabled := CanDeleteComponent(nil) and
    (LBGroupSummaryItems.SelCount > 0);
  BColumnLink.Enabled := CanAddComponent and (LBUnlinkedColumns.SelCount > 0);
  BColumnUnlink.Enabled := CanDeleteComponent(nil) and
    (LBLinkedColumns.SelCount > 0);

  MIFooterSummaryAdd.Enabled := BAddFooterSummaryItem.Enabled;
  MIFooterSummaryDelete.Enabled := BDeleteFooterSummaryItem.Enabled;
  MIFooterSummarySelectAll.Enabled := LBFooterSummary.SelCount <
    LBFooterSummary.Items.Count;
  ...
  MIDefaultGroupSummarySelectAll.Enabled := LBDefaultGroupsSummary.SelCount <
    LBDefaultGroupsSummary.Items.Count;

  MISummaryGroupAdd.Enabled := BSummaryGroupAdd.Enabled;
  MISummaryGroupDelete.Enabled := BSummaryGroupDelete.Enabled;
  MISummaryGroupSelectAll.Enabled := LBSummaryGroups.SelCount <
    LBSummaryGroups.Items.Count;
  ...
  MIGroupSummaryItemsSelectAll.Enabled := LBGroupSummaryItems.SelCount <
    LBGroupSummaryItems.Items.Count;
end;
...
begin
  ...
  try
    ListBoxSaveSelection(LBColumns.InnerListBox, ASelection, AItemIndex,
      ATopIndex);
    try
      ...
      ListBoxRestoreSelection(LBColumns.InnerListBox, ASelection, AItemIndex,
        ATopIndex);
    end;
  finally
...
begin
  liColumnAddAll.Visible := (View.DataController as IcxCustomGridDataController)
    .SupportsCreateAllItems;
...
begin
  ...
  ListBoxLoadCollection(LBDefaultGroupsSummary.InnerListBox,
...
function TcxCustomTableViewEditor.GetDefaultGroupSummaryItems
  : TcxDataGroupSummaryItems;
begin
  Result := View.DataController.Summary.DefaultGroupSummaryItems;
end;

function TcxCustomTableViewEditor.GetFooterSummaryItems
...
procedure TcxCustomTableViewEditor.ReindexColumnsProc(AList: TList;
  ANewIndex: Integer);
var
  I: Integer;
begin
...
begin
  ...
  ListBoxSaveSelection(LBUnlinkedColumns.InnerListBox, ASelection1, AItemIndex1,
    ATopIndex1);
  ListBoxSaveSelection(LBLinkedColumns.InnerListBox, ASelection2, AItemIndex2,
    ATopIndex2);
  try
    ...
    begin
      ...
      begin
        ...
        if AItemLink is TComponent then
          LBLinkedColumns.Items.AddObject((AItemLink as TComponent).Name,
            AItemLink);
      end;
      ...
      begin
        ...
        if LBLinkedColumns.Items.IndexOfObject(AItemLink) = -1 then
          LBUnlinkedColumns.Items.AddObject((AItemLink as TComponent).Name,
            AItemLink);
      end;
    end;
  finally
    ListBoxRestoreSelection(LBUnlinkedColumns.InnerListBox, ASelection1,
      AItemIndex1, ATopIndex1);
    ListBoxRestoreSelection(LBLinkedColumns.InnerListBox, ASelection2,
      AItemIndex2, ATopIndex2);
  end;
end;
...
begin
  ACurrentGroup := GetCurrentGroup;
  if ACurrentGroup <> nil then
    ListBoxLoadCollection(LBGroupSummaryItems.InnerListBox,
      ACurrentGroup.SummaryItems)
  else
...
begin
  ...
  ListBoxMoveUpItems(LBColumns.InnerListBox, FColumnsPrevDragIndex,
    ReindexColumnsProc);
end;
...
begin
  FColumnsPrevDragIndex := -1;
  ListBoxMoveDownItems(LBColumns.InnerListBox, FColumnsPrevDragIndex,
    ReindexColumnsProc);
end;
...
begin
  if View.ItemCount > 0 then
    case Application.MessageBox(PChar(GetDeleteItemsCaption), 'Confirm',
      MB_YESNOCANCEL) of
      IDYES:
        View.ClearItems;
      IDCANCEL:
...
begin
  FormEditor.ListBoxApplySelection(LBFooterSummary.InnerListBox,
    FooterSummaryItems);
end;
...
begin
  FormEditor.ListBoxApplySelection(LBDefaultGroupsSummary.InnerListBox,
    DefaultGroupSummaryItems);
end;

procedure TcxCustomTableViewEditor.BDefaultGroupSummaryAddClick
  (Sender: TObject);
begin
  CollectionItemsAdd(LBDefaultGroupsSummary.InnerListBox,
    DefaultGroupSummaryItems);
end;
...
procedure TcxCustomTableViewEditor.BDefaultGroupSummaryDeleteClick
  (Sender: TObject);
begin
  CollectionItemsDelete(LBDefaultGroupsSummary.InnerListBox,
    DefaultGroupSummaryItems);
end;
...
procedure TcxCustomTableViewEditor.LBGroupSummaryItemsClick(Sender: TObject);
begin
  if GetCurrentGroup <> nil then
    FormEditor.ListBoxApplySelection(LBGroupSummaryItems.InnerListBox,
      GetCurrentGroup.SummaryItems);
end;

procedure TcxCustomTableViewEditor.BGroupSummaryItemAddClick(Sender: TObject);
begin
  if GetCurrentGroup <> nil then
    CollectionItemsAdd(LBGroupSummaryItems.InnerListBox,
      GetCurrentGroup.SummaryItems);
end;
...
begin
  if GetCurrentGroup <> nil then
    CollectionItemsDelete(LBGroupSummaryItems.InnerListBox,
      GetCurrentGroup.SummaryItems);
end;
...
begin
  ...
  try
    ...
    for I := LBLinkedColumns.Items.Count - 1 downto 0 do
      ...
      begin
        AGroupItemLink := ACurrentGroup.Links.ItemOfItemLink
          (LBLinkedColumns.Items.Objects[I]);
        AGroupItemLink.Free;
      end;
    UpdateSummaryGroupLinks;
    UpdateDesigner;
...
procedure TcxCustomTableViewEditor.LBColumnsDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  ListBoxDragOver(LBColumns.InnerListBox, Sender, Source, X, Y, State, Accept,
    FColumnsPrevDragIndex);
end;

procedure TcxCustomTableViewEditor.LBColumnsDrawItem(AControl: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect);
end;

procedure TcxCustomTableViewEditor.LBColumnsEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
  ListBoxEndDrag(LBColumns.InnerListBox, Sender, Target, X, Y,
    FColumnsPrevDragIndex);
end;

procedure TcxCustomTableViewEditor.LBColumnsDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
  ListBoxDragDrop(LBColumns.InnerListBox, Sender, Source, X, Y,
    FColumnsPrevDragIndex, ReindexColumnsProc);
end;
...
procedure TcxCustomGridTableViewMenuProvider.CreateAllItems
...
procedure TcxCustomGridTableViewMenuProvider.CreateItem
  (Sender: TcxGridViewMenuItem);
begin
  ObjectCreated(CreateViewItem(GridView));
end;

procedure TcxCustomGridTableViewMenuProvider.CreateMissingItems
  (Sender: TcxGridViewMenuItem);
begin
...
procedure TcxCustomGridTableViewMenuProvider.DeleteAllItems
  (Sender: TcxGridViewMenuItem);
begin
...
begin
  if (GridView.DataController as IcxCustomGridDataController).SupportsCreateAllItems
  then
    if (GridView.DataController as IcxCustomGridDataController).IsDataLinked
    then
    begin
      Items.AddItem(Format('Create All %ss', [ViewItemName]), CreateAllItems,
        CanAddComponent(GridView.Owner));
      Items.AddItem(Format('Create Missing %ss', [ViewItemName]),
        CreateMissingItems, CanAddComponent(GridView.Owner) and
        not(GridView.DataController as IcxCustomGridDataController)
        .HasAllItems);
    end
    else
      Items.AddItem('(no data linked)', nil, False);
  Items.AddItem(Format('Create %s', [ViewItemName]), CreateItem,
    CanAddComponent(GridView.Owner));
end;
...
begin
  ...
  if (GridView.DataController as IcxCustomGridDataController).SupportsCreateAllItems
  then
    Items.AddItem(Format('Delete All %ss', [ViewItemName]), DeleteAllItems,
      CanDeleteItems);
  inherited;
end;
...
RegisterViewMenuProviderClass(TcxCustomGridTableView,
  TcxCustomGridTableViewMenuProvider);

finalization

UnregisterViewMenuProviderClass(TcxCustomGridTableView,
  TcxCustomGridTableViewMenuProvider);
