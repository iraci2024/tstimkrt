...
uses
  ...
  dxCore, dxCoreClasses, cxClasses, cxControls, cxGraphics, cxGeometry,
...
type
  ...
  published
    property DetailFrameColor: TColor read FDetailFrameColor
      write SetDetailFrameColor default clDefault;
    property DetailFrameWidth: Integer read FDetailFrameWidth
      write SetDetailFrameWidth default cxGridLevelDefaultDetailFrameWidth;
    property DetailTabsPosition: TcxGridDetailTabsPosition
      read FDetailTabsPosition write SetDetailTabsPosition
      default cxGridLevelDefaultDetailTabsPosition;
    property TabsForEmptyDetails: Boolean read FTabsForEmptyDetails
      write SetTabsForEmptyDetails default True;
  end;
...
{$IFDEF BCBCOMPATIBLE}var {$ELSE}out {$ENDIF} AStyle: TcxStyle) of object;
  ...
  protected
    ...
    procedure DoGetOwnTabStyle(AMasterRecord: TcxCustomGridRecord;
      var AStyle: TcxStyle); virtual;
  ...
  public
    ...
    procedure GetTabParams(ARecord: TcxCustomGridRecord;
      ATabLevel: TcxGridLevel; out AParams: TcxViewParams); virtual;
  published
    property Tab: TcxStyle index lsTab read GetValue write SetValue;
    property TabsBackground: TcxStyle index lsTabsBackground read GetValue
      write SetValue;
    property OnGetOwnTabStyle: TcxGridLevelGetOwnTabStyleEvent
      read FOnGetOwnTabStyle write FOnGetOwnTabStyle;
    property OnGetTabStyle: TcxGridLevelGetTabStyleEvent read FOnGetTabStyle
      write FOnGetTabStyle;
  end;
  ...
  TcxGridLevelChangeKind = (lcName, lcCaption, lcGridView, lcStructure,
    lcVisibility);
  ...
  published
    ...
    property ImageIndex: TcxImageIndex read FImageIndex write SetImageIndex
      default -1;
    property MaxDetailHeight: Integer read FMaxDetailHeight
      write SetMaxDetailHeight default 0;
    ...
    property OnGetGridView: TcxGridLevelGetGridViewEvent read FOnGetGridView
      write FOnGetGridView;
  end;
  ...
  public
    ...
    class function CreateLinkObject(AInstance: TcxGridLevel;
      ARelation: TcxCustomDataRelation; ARecordIndex: Integer)
      : TcxCustomGridView;
...
procedure TcxGridLevelOptions.SetDetailTabsPosition
  (Value: TcxGridDetailTabsPosition);
begin
...
begin
  ...
  if Result = clDefault then
    Result := TcxCustomGrid(FLevel.Control)
      .LookAndFeelPainter.DefaultFixedSeparatorColor;
end;
...
procedure TcxGridLevelStyles.DoGetOwnTabStyle(AMasterRecord
  : TcxCustomGridRecord; var AStyle: TcxStyle);
begin
  if Assigned(FOnGetOwnTabStyle) then
    FOnGetOwnTabStyle(Level, AMasterRecord, AStyle);
end;

procedure TcxGridLevelStyles.GetDefaultViewParams(Index: Integer;
...
begin
  AStyle := nil;
  if Assigned(FOnGetTabStyle) then
    FOnGetTabStyle(Level, ATabLevel, AStyle);
...
begin
  ...
  if not(AGrid.IsLoading or AGrid.IsDestroying) and IsDesigning and
    (AGrid.StructureNavigator <> nil) then
...
var
  ...
  begin
    if FGridView <> nil then
      if (APrevGridView = nil) and (TcxCustomGrid(FControl).FocusedView = nil)
        or (APrevGridView <> nil) and APrevGridView.Focused then
...
begin
  ...
  else if Value is TcxGridLevel then
...
var
  ...
  begin
    AGridView := (ARelation.DataController as IcxCustomGridDataController)
...
begin
  AGridView := FGridView;
  if Assigned(FOnGetGridView) then
    FOnGetGridView(Self, GetMasterRecord, AGridView);
  if (AGridView = nil) or not Visible then
    Result := nil
  else
  begin
    Result := TcxCustomGridViewClass(AGridView.ClassType)
      .CreateCloned(FControl);
    Result.BeginUpdate;
    try
      TcxCustomGridViewAccess(Result).AssignPattern(AGridView);
      Result.DataController.Relations.BeginUpdate;
      try
        Result.DataController.Relations.Assign
          (GetRelationsGridView.DataController.Relations);
