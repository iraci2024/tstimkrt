...
type
  ...
  TcxGridDBTableSummaryItem = class(TcxDBDataSummaryItem, IUnknown,
  ...
  public
    ...
    property Columns[Index: Integer]: TcxGridDBColumn read GetColumn
      write SetColumn;
  published
...
function TcxGridDBTableSummaryItem.QueryInterface(const IID: TGUID;
  out Obj): HResult;
const
...
function TcxGridDBTableSummaryItem.GetProperties(AProperties: TStrings)
  : Boolean;
begin
  Result := False;
end;

procedure TcxGridDBTableSummaryItem.GetPropertyValue(const AName: string;
  var AValue: Variant);
begin
...
procedure TcxGridDBTableSummaryItem.SetPropertyValue(const AName: string;
  const AValue: Variant);
begin
  if AName = 'Column' then
    Column := TcxGridColumn(TcxCustomGridTableViewAccess.FindItemByObjectName
      (GridView, AValue));
end;
...
function TcxGridDBTableView.GetDataControllerClass
...
function TcxGridDBTableView.GetColumnByFieldName(const AFieldName: string)
  : TcxGridDBColumn;
begin
