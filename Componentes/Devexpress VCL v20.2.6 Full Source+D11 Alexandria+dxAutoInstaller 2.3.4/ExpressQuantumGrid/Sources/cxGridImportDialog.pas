...
uses
  DesignIntf, Variants, Windows, Messages, SysUtils, Classes, Graphics,
  Controls,
  Forms, Dialogs, ExtCtrls, StdCtrls, cxConverterFactory, cxGridConverter,
  cxGrid,
  TypInfo, cxGridStructureNavigator, cxGridLevel, cxGridCustomView, ComCtrls,
  cxStyles,
  Menus, cxLookAndFeelPainters, cxButtons, cxClasses, cxControls, cxPC,
  cxContainer,
  cxEdit, cxCheckBox, cxRadioGroup, cxGraphics, cxMaskEdit, cxDropDownEdit,
  cxTextEdit, cxDesignWindows, cxLookAndFeels, dxLayoutContainer,
  dxLayoutControlAdapters, dxLayoutcxEditAdapters,
  dxLayoutLookAndFeels, dxLayoutControl, cxListBox, dxForms, cxCustomListBox,
  dxCoreGraphics;
...
type
  TImportDialog = class(TdxForm)
    ...
    procedure lbComponentsForImportMeasureItem(Control: TWinControl;
...
begin
  if lbComponentsForImport.ItemIndex = -1 then
    ...
    Result := lbComponentsForImport.Items.Objects
      [lbComponentsForImport.ItemIndex] as TComponent;
end;
...
function TImportDialog.GetConverterClass(ASource: TObject)
  : TcxCustomGridConverterClass;
var
  AConverterClass: TcxCustomConverterWithStylesClass;
begin
  AConverterClass := ConverterFactory(cxGridGroupConverterName)
    .FindConverter(ASource);
  if (AConverterClass <> nil) and AConverterClass.InheritsFrom
    (TcxCustomGridConverter) then
  ...
  try
    ...
    if cbImportStyles.Checked then
      if rbCreateNewStyleRepository.Checked then
        ...
        AConverter.StyleRepository :=
          cbStyleRepositories.ItemObject as TcxStyleRepository;
...
begin
  ...
  rbUseExistingStyleRepository.Enabled :=
    cbStyleRepositories.Properties.Items.Count <> 0;
...
var
  AIndex: Integer;
begin
  edNewStyleRepository.Text := FDesigner.UniqueName
    (TcxStyleRepository.ClassName);
...
procedure TImportDialog.lbComponentsForImportMeasureItem(Control: TWinControl;
  Index: Integer; var Height: Integer);
begin
  Height := TdxTextMeasurer.TextLineHeight(lbComponentsForImport.Canvas.Handle)
    + 2 * ScaleFactor.Apply(2);
end;
