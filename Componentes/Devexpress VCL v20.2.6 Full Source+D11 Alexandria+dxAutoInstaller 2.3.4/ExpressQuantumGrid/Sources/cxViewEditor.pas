...
type
  ...
  TcxGridViewMenuItemActionEvent = procedure(Sender: TcxGridViewMenuItem)
    of object;
  ...
  public
    ...
    constructor Create(const ACaption: string; AEnabled: Boolean;
      AChecked: Boolean; AData: TObject;
      AOnAction: TcxGridViewMenuItemActionEvent);
    destructor Destroy; override;
    procedure DoAction;
...
{$ENDIF}
{$IFNDEF NONDB}
    property DBViewDataSource: TDataSource read GetDBViewDataSource
      write SetDBViewDataSource;
    property DBGridView: TcxCustomGridView read GetDBGridView;
    property ServerModeViewDataSource: TdxServerModeCustomDataSource
      read GetServerModeViewDataSource write SetServerModeViewDataSource;
...
function GetViewEditorClass(AViewClass: TcxCustomGridViewClass)
  : TcxViewEditorClass;
procedure RegisterViewEditorClass(AViewClass: TcxCustomGridViewClass;
  AViewEditorClass: TcxViewEditorClass);
procedure UnregisterViewEditorClass(AViewClass: TcxCustomGridViewClass;
  AViewEditorClass: TcxViewEditorClass);
function GetLinkedView(APersistent: TPersistent): TcxCustomGridView;
function IsViewLinkedToComponent(AView: TcxCustomGridView;
  AComponent: TComponent): Boolean;

function GetViewTemplateRegKey: string;
procedure RegisterDefaultViewStorage(AStorageClass
  : TcxCustomGridViewStorageClass);
procedure UnregisterDefaultViewStorage(AStorageClass
  : TcxCustomGridViewStorageClass);
function GetDefaultViewStorageByViewClass(AGridView: TcxCustomGridView)
  : TcxCustomGridViewStorage;

procedure RestoreViewFromTemplate(const ARegPath: string;
  AView: TcxCustomGridView);
procedure SaveViewToTemplate(const ARegPath: string; AView: TcxCustomGridView);

function CreateViewMenuProvider(AView: TcxCustomGridView)
  : TcxCustomGridViewMenuProvider;
function GetViewMenuProviderClass(AView: TcxCustomGridView)
...
begin
  ...
  begin
    AViewEditorClass := TcxViewEditorClass
      (ViewEditorClassList[I].RegisteredClass);
...
begin
  ...
  if not Result and (AComponent is TcxCustomGrid) then
  // check Levels (from Repository)
  ...
  begin
    ...
    begin
      ...
      if AGridView.InheritsFrom(ACurrent.GetViewClass) then
        if (AStorageClass = nil) or not AStorageClass.GetViewClass.InheritsFrom
          (ACurrent.GetViewClass) then
...
begin
  ...
  if AViewStorage = nil then
    Exit;
  try
    AStorage := TcxStorage.Create
      (ARegPath { + '\' + AViewStorage.GetObjectName } );
    try
...
function GetViewMenuProviderClass(AView: TcxCustomGridView)
  : TcxCustomGridViewMenuProviderClass;
begin
  Result := TcxCustomGridViewMenuProviderClass
    (ViewMenuProviderClassList.FindClass(AView.ClassType));
end;
...
procedure TcxCustomGridViewStorage.GetPropertyValue(const AName: string;
  var AValue: Variant);
begin
end;

procedure TcxCustomGridViewStorage.SetPropertyValue(const AName: string;
...
class function TcxViewEditor.GetViewByObject(APersistent: TPersistent)
  : TcxCustomGridView;
begin
  Result := nil;
end;

procedure TcxViewEditor.SetView(Value: TcxCustomGridView;
...
begin
  Result := CreateUniqueName(OwnerForm, View, AComponent,
    ATruncateClassName, '');
end;
...
begin
  ...
  begin
    AItem := Items[I];
    AChildMenuItem := AMenuItem.AddItem('', 0, False, True,
      AItem.MenuItemClick);
    AItem.Prepare(AChildMenuItem);
  end;
end;
...
constructor TcxGridViewMenuItem.Create(const ACaption: string;
  AEnabled, AChecked: Boolean; AData: TObject;
  AOnAction: TcxGridViewMenuItemActionEvent);
begin
  inherited Create;
  Caption := ACaption;
...
procedure TcxCustomGridViewMenuProvider.GetServerModeDataSourceName
  (const S: string);
begin
  FServerModeDataSourceNames.Add(S);
end;

function TcxCustomGridViewMenuProvider.GetServerModeViewDataSource
  : TdxServerModeCustomDataSource;
begin
  Result := TdxServerModeDataController(ServerModeGridView.DataController)
    .DataSource;
end;
...
begin
  TdxServerModeDataController(ServerModeGridView.DataController)
    .DataSource := Value;
end;
...
var
  ...
  AConditionalFormattingProviderIntf
...
begin
  if Supports(GridView, IcxDataControllerConditionalFormattingProviderOwner,
    AConditionalFormattingProviderIntf) and
    (AConditionalFormattingProviderIntf.GetConditionalFormattingProvider <> nil)
    and AConditionalFormattingProviderIntf.GetConditionalFormattingProvider.
    ConditionalFormatting.CanShowRulesManagerDialog then
  ...
  if (FGridWizard <> nil) and FGridWizard.IsGridViewSupported(GridView.ClassType)
  then
    Items.AddItem('Edit via Wizard...', EditByWizard,
      CanChangeComponentList(GridView.Owner));
...
var
  ...
  begin
    Result := Designer.GetComponent(ADataSourceNames[AIndex])
      = DBViewDataSource;
  end;
...
begin
  ...
  try
    if ADataSourceNames.Count = 0 then
      Exit;
...
begin
  ...
  try
    if ADataSourceNames.Count = 0 then
      Exit;
    if ADataSourceNames.Count = 1 then
      Items.AddItem('Link to ' + ADataSourceNames[0],
        LinkToServerModeDataSource, True, IsDataSourceLinked(0), TObject(0))
    else
    begin
      AItem := Items.AddItem('Link to DataSource', nil, True, False);
      for I := 0 to ADataSourceNames.Count - 1 do
...
begin
  Items.AddItem('Delete View', DeleteView, CanDeleteComponent(GridView.Owner,
    GridView));
end;

procedure TcxCustomGridViewMenuProvider.CopySettings
...
procedure TcxCustomGridViewMenuProvider.EditConditionalFormatting
  (Sender: TcxGridViewMenuItem);
var
  AIntf: IcxDataControllerConditionalFormattingProviderOwner;
begin
  if Supports(GridView, IcxDataControllerConditionalFormattingProviderOwner,
    AIntf) and (AIntf.GetConditionalFormattingProvider <> nil) then
    AIntf.GetConditionalFormattingProvider.ConditionalFormatting.
      ShowRulesManagerDialog;
end;

procedure TcxCustomGridViewMenuProvider.EditLayoutAndData
  (Sender: TcxGridViewMenuItem);
begin
...
procedure TcxCustomGridViewMenuProvider.EditByWizard
  (Sender: TcxGridViewMenuItem);
begin
  FGridWizard.Execute(GridView.Control as TcxCustomGrid, GridView);
end;

procedure TcxCustomGridViewMenuProvider.SetAsDefault
  (Sender: TcxGridViewMenuItem);
begin
...
begin
  ...
  Designer.GetComponentNames(GetTypeData(TypeInfo(TDataSource)),
    GetDBDataSourceName);
  FDBDataSourceNames := nil;
  Result.Sort;
end;

function TcxCustomGridViewMenuProvider.GetServerModeDataSourceNames
  : TStringList;
begin
  Result := TStringList.Create;
  FServerModeDataSourceNames := Result;
  Designer.GetComponentNames(GetTypeData(TypeInfo(TdxServerModeCustomDataSource)
    ), GetServerModeDataSourceName);
  FServerModeDataSourceNames := nil;
  Result.Sort;
end;

procedure TcxCustomGridViewMenuProvider.LinkToDBDataSource
  (Sender: TcxGridViewMenuItem);
var
  ADataSourceNames: TStringList;
...
begin
  ...
  try
    ADataSource := TDataSource
      (Designer.GetComponent(ADataSourceNames[Integer(Sender.Data)]));
    if DBViewDataSource <> ADataSource then
    begin
      ACanAddComponent := CanAddComponent(DBGridView.Owner);
      if ACanAddComponent then
        (DBGridView.DataController as IcxCustomGridDataController)
          .DeleteAllItems;
...
procedure TcxCustomGridViewMenuProvider.LinkToServerModeDataSource
  (Sender: TcxGridViewMenuItem);
var
  ADataSourceNames: TStringList;
...
begin
  ...
  try
    ...
    begin
      (ServerModeGridView.DataController as IcxCustomGridDataController)
        .DeleteAllItems;
...
begin
  ViewNames.Clear;
  Designer.GetComponentNames(GetTypeData(PTypeInfo(TcxCustomGridView.ClassInfo)
    ), AddViewName);
end;
...
UnregisterViewMenuProviderClass(TcxCustomGridView,
  TcxCustomGridViewMenuProvider);
