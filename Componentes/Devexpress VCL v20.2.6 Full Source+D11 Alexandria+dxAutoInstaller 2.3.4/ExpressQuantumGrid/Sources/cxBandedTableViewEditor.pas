...
uses
  ...
  ExtCtrls, StdCtrls, ComCtrls, cxEdit,
  cxGridCustomView, cxGridBandedTableView, cxGridCustomTableView, cxViewEditor,
  cxCustomTableViewEditor, cxListBox,
  cxLookAndFeelPainters, cxButtons, cxPC, cxControls, cxGraphics,
  cxLookAndFeels, dxLayoutContainer, cxTableViewEditor,
  dxLayoutControlAdapters, dxLayoutLookAndFeels, cxClasses, cxContainer,
  dxLayoutControl;
...
type
  TcxBandedTableViewEditor = class(TcxTableViewEditor)
    ...
    procedure LBBandsDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
  ...
  protected
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas;
      ...
      var ASuffixOffset: Integer); reintroduce;
...
function TcxGridBandedTableViewStorage.GetOptionsCustomize
  : TcxGridBandedTableOptionsCustomize;
begin
  Result := BandedTableView.OptionsCustomize;
end;

function TcxGridBandedTableViewStorage.GetOptionsView
...
class function TcxGridBandedTableViewStorage.GetViewClass
...
begin
  ...
  else if APersistent is TcxGridBands then
    Result := (APersistent as TcxGridBands).GridView
  else
    Result := inherited GetViewByObject(APersistent);
end;

procedure TcxBandedTableViewEditor.DrawListBoxItem(AListBox: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; R: TRect; AParentBand: TcxGridBand;
  var ASuffixOffset: Integer);
...
begin
  ...
  begin
    ...
    inherited DrawListBoxItem(AListBox, ACanvas, R, AListBox.Items[AIndex],
      GetParentBandInfoText, ASuffixOffset);
  end
  else
...
procedure TcxBandedTableViewEditor.ReindexBandsProc(AList: TList;
  ANewIndex: Integer);
var
  I: Integer;
begin
...
begin
  ListBoxMoveUpItems(LBBands.InnerListBox, FBandsPrevDragIndex,
    ReindexBandsProc);
end;

procedure TcxBandedTableViewEditor.BBandMoveDownClick(Sender: TObject);
begin
  ListBoxMoveDownItems(LBBands.InnerListBox, FBandsPrevDragIndex,
    ReindexBandsProc);
end;
...
procedure TcxBandedTableViewEditor.LBColumnsDrawItem(AControl: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect,
    TcxGridBandedColumn(View.Items[AIndex]).Position.Band,
    FColumnsListSuffixOffset);
end;

procedure TcxBandedTableViewEditor.LBBandsDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  ListBoxDragOver(LBBands.InnerListBox, Sender, Source, X, Y, State, Accept,
    FBandsPrevDragIndex);
end;

procedure TcxBandedTableViewEditor.LBBandsDrawItem(AControl: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect, Bands[AIndex].ParentBand,
    FBandsListSuffixOffset);
end;

procedure TcxBandedTableViewEditor.LBBandsEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
  ListBoxEndDrag(LBBands.InnerListBox, Sender, Target, X, Y,
    FBandsPrevDragIndex);
end;
...
procedure TcxBandedTableViewEditor.LBBandsDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
  ListBoxDragDrop(LBBands.InnerListBox, Sender, Source, X, Y,
    FBandsPrevDragIndex, ReindexBandsProc);
end;
...
procedure TcxGridBandedTableViewMenuProvider.CreateBand
...
RegisterViewMenuProviderClass(TcxGridBandedTableView,
  TcxGridBandedTableViewMenuProvider);

finalization

UnregisterViewMenuProviderClass(TcxGridBandedTableView,
  TcxGridBandedTableViewMenuProvider);
