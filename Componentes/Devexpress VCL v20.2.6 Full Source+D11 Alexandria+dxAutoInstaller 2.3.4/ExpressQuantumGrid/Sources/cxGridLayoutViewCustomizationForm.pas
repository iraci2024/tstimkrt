...
uses
  ...
  cxStyles, cxClasses, cxGridLevel, cxGrid, cxGridViewLayoutCustomizationForm,
...
type
  ...
  TcxGridLayoutViewCustomizationFormGroupPainter = class
  ...
  TcxGridViewLayoutContainer = class
  ...
  TcxGridViewLayoutControl = class
  ...
  private
    ...
    function GetGroupScrollMode(AUseScrolling: Boolean)
  ...
  protected
    ...
    function GetGridViewLayoutControlClass
      : TcxGridViewLayoutCustomizationFormLayoutControlClass; override;
    function GetGridViewLayoutLookAndFeel: TdxLayoutCxLookAndFeel; override;
    function GetLayoutLookAndFeelClass
      : TcxGridViewLayoutCustomizationFormLayoutLookAndFeelClass; override;
  ...
  public
    ...
    property DataControllerSupport: IcxCustomGridDataController
      read GetDataControllerSupport;
    property GridView: TcxGridLayoutView read GetGridView;
    property GridViewLayoutControl: TcxGridViewLayoutControl
      read GetGridViewLayoutControl;
  end;
...
uses
  cxGeometry, cxGridStrs, dxComCtrlsUtils,
...
function TcxGridLayoutViewCustomizationFormLayoutLookAndFeel.
...
function TcxGridViewLayoutContainer.GetCloneItemClass
...
function TcxGridViewLayoutControl.GetContainerClass
...
function TcxGridViewLayoutControl.GetLayoutLookAndFeel
  : TcxGridLayoutLookAndFeel;
begin
  Result := TcxGridLayoutLookAndFeel(inherited LayoutLookAndFeel);
end;

procedure TcxGridViewLayoutControl.SetLayoutLookAndFeel
  (Value: TcxGridLayoutLookAndFeel);
begin
...
procedure TcxGridLayoutViewCustomizationForm.acUseScrollingExecute
  (Sender: TObject);
var
  I: Integer;
...
begin
  ...
  try
    ...
    try
      ...
      (PreviewView.DataController as IcxCustomGridDataController)
        .AssignData(GridView.DataController);
    finally
  ...
  GridViewLayoutControl.Container.CustomizationHelper.CopyStructure
...
procedure TcxGridLayoutViewCustomizationForm.
  CalculateTreeViewPopupActionVisibilities;
begin
  inherited CalculateTreeViewPopupActionVisibilities;
  miGroupScrolling.Visible := GridView.IsDesigning and HasGroupsOnly and
    not HasRootInSelection;
end;

function TcxGridLayoutViewCustomizationForm.CheckControlOKVisible: Boolean;
begin
  Result := inherited CheckControlOKVisible or
    DataControllerSupport.IsDataChangeable;
end;
...
begin
  ...
  liConditionalFormatting.Visible := liSaveLayout.Visible and
    Supports(GridView, IcxDataControllerConditionalFormattingProviderOwner,
    AIntf) and (AIntf.GetConditionalFormattingProvider <> nil) and
    AIntf.GetConditionalFormattingProvider.ConditionalFormatting.
...
begin
  FPreviewView := gMain.CreateView(TcxCustomGridViewClass(GridView.ClassType))
    as TcxGridLayoutView;
...
function TcxGridLayoutViewCustomizationForm.GetGridViewContainerInstance
  : TdxLayoutContainer;
begin
  Result := GridView.Container;
end;

function TcxGridLayoutViewCustomizationForm.GetGridViewLayoutControlClass
  : TcxGridViewLayoutCustomizationFormLayoutControlClass;
begin
  Result := TcxGridViewLayoutControl;
end;

function TcxGridLayoutViewCustomizationForm.GetGridViewLayoutLookAndFeel
  : TdxLayoutCxLookAndFeel;
begin
  Result := TcxGridLayoutViewAccess(GridView).LayoutLookAndFeel;
end;

function TcxGridLayoutViewCustomizationForm.GetLayoutLookAndFeelClass
...
begin
  inherited Localize;
  lcMainTemplateCardGroup.Caption :=
    cxGetResourceString(@scxGridLayoutViewCustomizeFormTemplateCard);
  lcMainViewLayoutGroup.Caption :=
    cxGetResourceString(@scxGridLayoutViewCustomizeFormViewLayout);
end;
...
begin
  ...
  if (tvVisibleItems.Selected <> nil) and
    (TObject(tvVisibleItems.Selected.Data) is TdxCustomLayoutGroup) then
  begin
    AGroup := TdxCustomLayoutGroup(tvVisibleItems.Selected.Data);
    acUseHorizontalScrolling.Checked :=
      GetUseScrolling(TdxLayoutGroupAccess(AGroup).ScrollOptions.Horizontal);
    acUseVerticalScrolling.Checked :=
      GetUseScrolling(TdxLayoutGroupAccess(AGroup).ScrollOptions.Vertical);
  end;
end;

function TcxGridLayoutViewCustomizationForm.GetController
  : TcxGridLayoutViewController;
begin
  Result := TcxGridLayoutViewController(inherited Controller);
end;

function TcxGridLayoutViewCustomizationForm.GetDataControllerSupport
...
function TcxGridLayoutViewCustomizationForm.GetGridViewLayoutControl
  : TcxGridViewLayoutControl;
begin
  Result := TcxGridViewLayoutControl(inherited GridViewLayoutControl);
end;

function TcxGridLayoutViewCustomizationForm.GetGroupScrollMode
  (AUseScrolling: Boolean): TdxLayoutGroupScrollMode;
begin
...
function TcxGridLayoutViewCustomizationForm.GetUseScrolling
  (AScrollMode: TdxLayoutGroupScrollMode): Boolean;
begin
