...
uses
  ...
  cxClasses, cxLookAndFeels, cxLookAndFeelPainters, ComCtrls, dxLayoutContainer,
  dxLayoutControlAdapters, StdCtrls,
  cxRadioGroup, cxContainer, cxEdit, cxListView, dxLayoutControl, cxGrid,
  cxGridCustomView, dxWizardControl, ImgList,
  cxGridWizardCustomPage, cxGridWizardStrs, dxLayoutLookAndFeels,
  dxGalleryControl;
...
type
  ...
  TcxGridWizardCommonSelectViewPageFrame = class(TcxGridWizardCustomPageFrame)
    ...
    procedure lvGridViewsItemClick(Sender: TObject;
...
begin
  ...
  if Helper <> nil then
  begin
    AItem := lvGridViews.Gallery.FindItemByTag
      (TdxNativeInt(Helper.GetGridViewClass));
...
begin
  ...
  try
    ...
    for AIndex := Low(TcxGridWizardGridViewType)
      to High(TcxGridWizardGridViewType) do
    begin
    ...
    begin
      ...
      if not IsMultiLevelStructure or IsDetailViewCustomizing or
        AHelperInfo.HelperClass.CanBeMasterView then
      begin
        AItem := AGroups[cxGridWizardGetGridViewGroup(AHelperInfo.GridViewClass)
          ].Items.Add;
        AItem.Caption := cxGridRegisteredViews.GetDescriptionByClass
          (AHelperInfo.GridViewClass);
        AItem.Tag := TdxNativeInt(AHelperInfo.GridViewClass);
        AItem.ImageIndex := ilGridViewImages.Add(AHelperInfo.Glyph, nil);
      end;
    end;

    if IsDetailViewCustomizing and (CustomizedGrid.ActiveLevel <> nil) and
      (CustomizedGrid.ActiveLevel.GridView <> nil) then
      AMasterViewType := cxGridWizardGetGridViewGroup
        (TcxCustomGridViewClass(CustomizedGrid.ActiveLevel.GridView.ClassType))
    else
...
procedure TcxGridWizardCommonSelectViewPageFrame.lvGridViewsDblClick
  (Sender: TObject);
begin
...
procedure TcxGridWizardCommonSelectViewPageFrame.lvGridViewsItemClick
