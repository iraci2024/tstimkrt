...
uses
  ...
  Windows, Classes, Forms, SysUtils, DB, dxCoreClasses, cxClasses, cxGraphics,
  cxGrid, cxGridDBDataDefinitions,
  cxGridLevel, cxGridCustomView, dxCustomWizardControl, dxGDIPlusClasses,
  cxControls, Graphics, cxDBData,
...
type
  ...
  protected
    ...
    function GetItemProperties(Index: Integer)
    ...
    procedure SetItemCaption(Index: Integer; const AValue: string);
      virtual; abstract;
    procedure SetItemFieldName(Index: Integer; const AValue: string); virtual;
    procedure SetItemProperties(Index: Integer;
      AValue: TcxCustomEditPropertiesClass); virtual;
    procedure SetItemVisible(Index: Integer; const AValue: Boolean);
      virtual; abstract;
  public
    constructor Create(AGrid: TcxCustomGrid); virtual;
    destructor Destroy; override;

    procedure AddItem(const AFieldName: string;
      AProperties: TcxCustomEditPropertiesClass = nil); overload;
    procedure AddItem(const AFieldName: string; const ACaption: string;
      AProperties: TcxCustomEditPropertiesClass = nil); overload;
    procedure Assign(ASourceView: TcxCustomGridView); virtual;
    procedure ChangeItemIndex(const AOldIndex, ANewIndex: Integer);
      virtual; abstract;
    procedure DeleteItem(const AFieldName: string); overload; virtual;
    procedure DeleteItem(const AItemIndex: Integer); overload; virtual;
      abstract;
    function GetDefaultItemCaption: string; virtual; abstract;
    function GetItemIndexByFieldName(const AFieldName: string)
      : Integer; virtual;
    procedure InitializeEditingMode(AEditingView, APreviewView
      : TcxCustomGridView); virtual;
    procedure PreparePreview(APreviewView: TcxCustomGridView); virtual;

    procedure CorrectCustomizationFormContent(ACustomizationForm
      : TForm); virtual;
    procedure PrepareForCustomization; virtual;
    procedure RestoreAfterCustomization; virtual;

    function CreateGridView(AGrid: TcxCustomGrid; ALevel: TcxGridLevel)
      : TcxCustomGridView; virtual;
    ...
    procedure SaveGridView(ASelectedView: TcxCustomGridView;
      AGrid: TcxCustomGrid); virtual;
    procedure SaveGridViewAndDeleteDetailed(ASelectedView: TcxCustomGridView;
      AGrid: TcxCustomGrid); virtual;
    procedure SaveGridViewAsDetail(ASelectedView,
      AMasterView: TcxCustomGridView; AGrid: TcxCustomGrid); virtual;

    procedure PopulateComponents(AClassType: TComponentClass;
      AFilterProc: TcxComponentFilterProc; AList: TStrings);
    ...
    property ItemFieldName[Index: Integer]: string read GetItemFieldName
      write SetItemFieldName;
    property ItemProperties[Index: Integer]: TcxCustomEditPropertiesClass
      read GetItemProperties write SetItemProperties;
    property ItemVisible[Index: Integer]: Boolean read GetItemVisible
  ...
  protected
    ...
    procedure SetItemProperties(Index: Integer;
      AValue: TcxCustomEditPropertiesClass); override;
  ...
  public
    constructor Create(AHelperClass: TcxGridWizardCustomHelperClass;
      AInstance: HINST);
    destructor Destroy; override;

    property Glyph: TdxSmartImage read FGlyph write FGlyph;
    property GridViewClass: TcxCustomGridViewClass read GetGridViewClass;
    property HelperClass: TcxGridWizardCustomHelperClass read FHelperClass
      write FHelperClass;
  end;
  ...
  public
    procedure Add(AHelperClass: TcxGridWizardCustomHelperClass;
      AInstance: HINST);
    function GetItemIndexBy(AGridViewClass: TClass): Integer;
    function GetHelperClassBy(AGridViewClass: TClass)
      : TcxGridWizardCustomHelperClass;

    property Items[Index: Integer]: TcxGridWizardHelperInfo
      read GetItem; default;
  end;

function cxGridWizardGetGridViewGroup(AGridViewClass: TcxCustomGridViewClass)
  : TcxGridWizardGridViewType;
...
begin
  if Pos('DB', AGridViewClass.ClassName) > 0 then
    Result := gvtDB
  else if Pos('ServerMode', AGridViewClass.ClassName) > 0 then
    Result := gvtServerMode
  else if Pos('EMF', AGridViewClass.ClassName) > 0 then
    Result := gvtEMF
  else
    Result := gvtUnbound;
end;
...
procedure TcxGridWizardCustomHelper.AddItem(const AFieldName: string;
  AProperties: TcxCustomEditPropertiesClass);
var
  AIndex: Integer;
begin
  AIndex := AddNewItem;
...
procedure TcxGridWizardCustomHelper.AddItem(const AFieldName, ACaption: string;
  AProperties: TcxCustomEditPropertiesClass);
var
  AIndex: Integer;
begin
...
function TcxGridWizardCustomHelper.GetItemIndexByFieldName(const AFieldName
  : string): Integer;
begin
  Result := -1;
end;

procedure TcxGridWizardCustomHelper.InitializeEditingMode(AEditingView,
  APreviewView: TcxCustomGridView);
begin
  Assign(AEditingView);
...
procedure TcxGridWizardCustomHelper.PreparePreview(APreviewView
  : TcxCustomGridView);
begin
  APreviewView.Assign(GridView);
end;

function TcxGridWizardCustomHelper.GetItemProperties(Index: Integer)
  : TcxCustomEditPropertiesClass;
begin
  Result := nil;
end;

procedure TcxGridWizardCustomHelper.CorrectCustomizationFormContent
...
function TcxGridWizardCustomHelper.CreateGridView(AGrid: TcxCustomGrid;
  ALevel: TcxGridLevel): TcxCustomGridView;
begin
...
class function TcxGridWizardCustomHelper.GetGridViewClass
  : TcxCustomGridViewClass;
begin
  Result := nil; // CBuilder
end;

class function TcxGridWizardCustomHelper.GetGridViewType
  : TcxGridWizardGridViewType;
begin
  Result := cxGridWizardGetGridViewGroup(GetGridViewClass);
end;

procedure TcxGridWizardCustomHelper.SaveGridView(ASelectedView
  : TcxCustomGridView; AGrid: TcxCustomGrid);
var
...
procedure TcxGridWizardCustomHelper.SaveGridViewAndDeleteDetailed
  (ASelectedView: TcxCustomGridView; AGrid: TcxCustomGrid);
var
...
procedure TcxGridWizardCustomHelper.SaveGridViewAsDetail(ASelectedView,
  AMasterView: TcxCustomGridView; AGrid: TcxCustomGrid);
var
  AActualView: TcxCustomGridView;
...
begin
  ...
  begin
    AActualView := ASelectedView;
    AActualView.Assign(GridView);
    if (AActualView.Level = nil) or
      (TcxGridLevel(AActualView.Level).Parent <> AMasterLevel) then
    begin
...
procedure TcxGridWizardCustomHelper.PopulateComponents
  (AClassType: TComponentClass; AFilterProc: TcxComponentFilterProc;
  AList: TStrings);
  ...
  begin
    ...
    begin
      ...
      begin
        if AFilterProc(AComponent) and (AList.IndexOfObject(AComponent) < 0)
        then
          AList.AddObject(IfThen(AComponent.Owner.Name <> '',
            AComponent.Owner.Name + '.') + AComponent.Name, AComponent);
      end;
    end;
  end;
...
procedure TcxGridWizardCustomHelper.CheckGridLevelName(AGrid: TcxCustomGrid;
  ALevel: TcxGridLevel);
begin
  if ALevel.Name = '' then
  begin
...
procedure TcxGridWizardCustomHelper.SetItemFieldName(Index: Integer;
  const AValue: string);
begin
  // do nothing
end;

procedure TcxGridWizardCustomHelper.SetItemProperties(Index: Integer;
  AValue: TcxCustomEditPropertiesClass);
begin
  // do nothing
end;

procedure TcxGridWizardCustomHelper.AssignData(ASourceView,
  ATargetView: TcxCustomGridView);
var
  ADataController: IcxCustomGridDataController;
begin
  if Supports(ATargetView.DataController, IcxCustomGridDataController,
    ADataController) then
...
constructor TcxGridWizardHelperInfo.Create(AHelperClass
  : TcxGridWizardCustomHelperClass; AInstance: HINST);
begin
  inherited Create;
  FGlyph := TdxSmartImage.Create;
  HelperClass := AHelperClass;
  try
    Glyph.LoadFromResource(AInstance, UpperCase(GridViewClass.ClassName),
      RT_BITMAP);
  except
    on EResNotFound do
      try
...
procedure TcxGridWizardHelperInfoList.Add(AHelperClass
  : TcxGridWizardCustomHelperClass; AInstance: HINST);
begin
  inherited Add(TcxGridWizardHelperInfo.Create(AHelperClass, AInstance));
end;

function TcxGridWizardHelperInfoList.GetItemIndexBy(AGridViewClass
  : TClass): Integer;
var
  I: Integer;
begin
...
function TcxGridWizardHelperInfoList.GetHelperClassBy(AGridViewClass: TClass)
  : TcxGridWizardCustomHelperClass;
var
  I: Integer;
begin
...
function TcxGridWizardHelperInfoList.GetItem(Index: Integer)
  : TcxGridWizardHelperInfo;
begin
...
procedure TcxGridWizardCustomTableLikeViewHelper.ChangeItemIndex
  (const AOldIndex, ANewIndex: Integer);
begin
  Items[AOldIndex].Index := ANewIndex;
end;

procedure TcxGridWizardCustomTableLikeViewHelper.DeleteItem(const AItemIndex
  : Integer);
begin
...
function TcxGridWizardCustomTableLikeViewHelper.GetItemCaption
  (Index: Integer): string;
begin
  Result := Items[Index].Caption;
end;

function TcxGridWizardCustomTableLikeViewHelper.GetItemProperties
  (Index: Integer): TcxCustomEditPropertiesClass;
begin
...
function TcxGridWizardCustomTableLikeViewHelper.GetItemVisible
  (Index: Integer): Boolean;
begin
  Result := Items[Index].Visible;
end;

procedure TcxGridWizardCustomTableLikeViewHelper.SetItemCaption(Index: Integer;
  const AValue: string);
begin
  Items[Index].Caption := AValue;
end;

procedure TcxGridWizardCustomTableLikeViewHelper.SetItemProperties
  (Index: Integer; AValue: TcxCustomEditPropertiesClass);
begin
  Items[Index].PropertiesClass := AValue;
end;

procedure TcxGridWizardCustomTableLikeViewHelper.SetItemVisible(Index: Integer;
  const AValue: Boolean);
begin
  Items[Index].Visible := AValue;
end;

function TcxGridWizardCustomTableLikeViewHelper.GetGridView
  : TcxCustomGridTableView;
begin
  Result := TcxCustomGridTableView(inherited GridView);
end;

function TcxGridWizardCustomTableLikeViewHelper.GetItem(Index: Integer)
