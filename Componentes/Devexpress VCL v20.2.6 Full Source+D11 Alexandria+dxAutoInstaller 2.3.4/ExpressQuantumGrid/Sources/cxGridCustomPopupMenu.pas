...
type
  ...
  TcxGridViewHitType = (gvhtGridNone, gvhtGridTab, gvhtNone, gvhtTab, gvhtCell,
    ...
    gvhtIndicatorHeader, gvhtIndicatorBandHeader, gvhtRowIndicator,
    gvhtRowLevelIndent, gvhtBand, gvhtBandHeader, gvhtRowCaption, gvhtSeparator,
    gvhtGroupSummary, gvhtFindPanel);
  ...
  TcxGridPopupMenuItemClickProc = procedure(ASender: TObject;
    AHitTest: TcxCustomGridHitTest; var AHandler: TNotifyEvent; AParams: TList;
  ...
  protected
    ...
    function CanPopup(AHitTest: TcxCustomGridHitTest): Boolean;
    procedure DoChanged(const APopupMenu: TComponent;
      const AView: TcxCustomGridView;
      AViewClass: TcxCustomGridViewClass); virtual;
    property GridViewClass: TcxCustomGridViewClass read FViewClass
      write SetViewClass;
  public
  ...
  public
    ...
    property Items[AIndex: Integer]: TcxPopupMenuInfo read GetItem
  ...
  protected
    procedure AddMenuInfo(const AMenu: TComponent;
      AHitTypes: TcxGridViewHitTypes; ADoPopupProc: TcxGridOnPopupProc;
      AViewClass: TcxCustomGridViewClass = nil;
    ...
    function GetPopupMenuInfo(const AMenu: TComponent;
      const AView: TcxCustomGridView; ASearchDefault: Boolean = True)
      : TcxPopupMenuInfo; overload; virtual;
    procedure Notification(AComponent: TComponent;
      Operation: TOperation); override;
    property PopupMenuInfos: TcxPopupMenuInfos read FPopupMenuInfos
      write SetPopupMenuInfos;
    property MenuInfos[AIndex: Integer]: TcxPopupMenuInfo
      read GetMenuInfo; default;
  public
    ...
    function FindPopupMenuInfo(const AView: TcxCustomGridView;
      AHitType: TcxGridViewHitType; AHitTest: TcxCustomGridHitTest)
      : TcxPopupMenuInfo; virtual;
    function GetPopupMenuInfo(const AMenu: TComponent;
      AViewClass: TcxCustomGridViewClass; ASearchDefault: Boolean = True)
      : TcxPopupMenuInfo; overload; virtual;
  ...
  protected
    function GetPopupMenuInfo(const AMenu: TComponent;
      const AView: TcxCustomGridView; ASearchDefault: Boolean = True)
      : TcxPopupMenuInfo; overload; override;
  ...
  public
    function GetPopupMenuInfo(const AMenu: TComponent;
      AViewClass: TcxCustomGridViewClass; ASearchDefault: Boolean = True)
      : TcxPopupMenuInfo; overload; override;
    procedure RegisterPopupMenu(const AMenu: TComponent;
      AHitTypes: TcxGridViewHitTypes; ADoPopupProc: TcxGridOnPopupProc;
      AViewClass: TcxCustomGridViewClass); virtual;
    procedure UnRegisterPopupMenu(const AMenu: TComponent;
  ...
  public
    ...
    constructor Create(AMenuClass: TcxPopupMenuClass;
  ...
  protected
    ...
    property Items[AIndex: Integer]: TcxPopupMenuClassInfoRec
...
function GetHitTypeByHitCode(AHitCode: Integer): TcxGridViewHitType;
const
  cxGridViewHitCodes: array [TcxGridViewHitType] of Integer =
    (htNone + cxhtGridBase, htTab + cxhtGridBase, htNone, htTab, htCell,
    htExpandButton, htRecord, htNavigator, htPreview, htColumnHeader,
    htColumnHeaderFilterButton, htFilter, htFooter, htFooterCell, htGroupFooter,
    htGroupFooterCell, htGroupByBox, htIndicator, htIndicatorHeader,
    htIndicatorBandHeader, htRowIndicator, htRowLevelIndent, htBand,
    htBandHeader, htRowCaption, htSeparator, htGroupSummary, htFindPanel);
begin
...
begin
  if (PopupMenu <> nil) and Supports(PopupMenu, IcxGridPopupMenu, APopupMenuIntf)
  then
...
begin
  ...
  if Assigned(AView) then
    AMenuInfo := TcxCustomGridPopupMenu(MenuInfos.GetOwner)
      .GetPopupMenuInfo(APopupMenu, AView)
  else
  begin
    if Assigned(AViewClass) and (MenuInfos.GetOwner is TcxGridDefaultPopupMenu)
    then
      AMenuInfo := TcxGridDefaultPopupMenu(MenuInfos.GetOwner)
        .GetPopupMenuInfo(APopupMenu, AViewClass);
  end;
  if (AMenuInfo <> nil) and (MenuInfos.GetOwner <> nil) and
    ...
    Raise EcxGridPopupMenu.Create
      (Format('This PopupMenuInfo is already registered. Index: %d',
      [AMenuInfo.Index]));
end;

function TcxPopupMenuInfo.FindCorrectComponent(const AComponent: TComponent)
  : TComponent;
var
  AOwner: TComponent;
begin
...
begin
  ...
  begin
    if (TcxPopupMenuInfos(Collection).GetSameMenuCount(FPopupMenu) = 1) and
      (FPopupMenu <> nil) and not(csDestroying in FPopupMenu.ComponentState)
    then
      FPopupMenu.RemoveFreeNotification
        (TComponent(TcxPopupMenuInfos(Collection).GetOwner));
    ...
    if FPopupMenu <> nil then
      FPopupMenu.FreeNotification(TComponent(TcxPopupMenuInfos(Collection)
        .GetOwner));
  end;
end;
...
begin
  ...
  begin
    if Assigned(FView) and not(csDestroying in FView.ComponentState) then
      FView.RemoveFreeNotification(TComponent(TcxPopupMenuInfos(Collection)
        .GetOwner));
    DoChanged(FPopupMenu, AValue, FViewClass);
    FView := AValue;
    if Assigned(FView) then
      FView.FreeNotification(TComponent(TcxPopupMenuInfos(Collection)
        .GetOwner));
  end;
end;

procedure TcxPopupMenuInfo.SetViewClass(const Value: TcxCustomGridViewClass);
begin
...
function TcxCustomGridPopupMenu.CheckShortCuts(var Message: TWMKeyDown)
  : Boolean;
begin
...
function TcxCustomGridPopupMenu.FindPopupMenuInfo
  (const AView: TcxCustomGridView; AHitType: TcxGridViewHitType;
  AHitTest: TcxCustomGridHitTest): TcxPopupMenuInfo;
var
  I: Integer;
begin
  Result := nil;
  for I := 0 to Count - 1 do
    if AHitType in MenuInfos[I].HitTypes then
      if ((AHitType in [gvhtGridNone, gvhtGridTab]) or
        (MenuInfos[I].GridView = AView)) and MenuInfos[I].CanPopup(AHitTest)
      then
      begin
        Result := MenuInfos[I];
        Break;
      end
      else if AView.InheritsFrom(MenuInfos[I].GridViewClass) and
        ((Result = nil) or MenuInfos[I].GridViewClass.InheritsFrom
        (Result.GridViewClass)) and MenuInfos[I].CanPopup(AHitTest) then
        Result := MenuInfos[I];
end;
...
begin
  ...
  begin
    ...
    else if Assigned(FPopupMenuInfos) then
    begin
...
procedure TcxCustomGridPopupMenu.SetPopupMenuInfos(const AValue
  : TcxPopupMenuInfos);
begin
...
function TcxGridDefaultPopupMenu.GetPopupMenuInfo(const AMenu: TComponent;
  AViewClass: TcxCustomGridViewClass; ASearchDefault: Boolean)
  : TcxPopupMenuInfo;
var
  I: Integer;
...
function TcxBuiltInPopupMenus.GetItem(AIndex: Integer)
  : TcxPopupMenuClassInfoRec;
begin
...
procedure TcxBuiltInPopupMenus.RegisterItemTo(ADefaultPopupMenu
  : TcxGridDefaultPopupMenu; AItemIndex: Integer);
var
  ADoPopupProc: TcxGridOnPopupProc;
  AIDoPopup: IDoPopup;
  AMenu: TComponent;
...
procedure TcxBuiltInPopupMenus.RegisterItemsTo(ADefaultPopupMenu
  : TcxGridDefaultPopupMenu);
var
  I: Integer;
begin
