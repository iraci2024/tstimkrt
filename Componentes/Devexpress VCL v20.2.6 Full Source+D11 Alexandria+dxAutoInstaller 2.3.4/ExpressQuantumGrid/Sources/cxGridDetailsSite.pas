...
type
  ...
  public
    constructor Create(ACanvas: TcxCanvas;
      AViewInfo: TcxCustomGridDetailsSiteViewInfo); virtual;
  ...
  protected
    ...
    property FocusActiveViewOnChange: Boolean read FFocusActiveViewOnChange
      write FFocusActiveViewOnChange;
  public
    ...
    property SiteViewInfo: TcxCustomGridDetailsSiteViewInfo
      read GetSiteViewInfo;
    property UnderlineHotKeys: Boolean read FUnderlineHotKeys
      write SetUnderlineHotKeys;
  end;
  ...
  TcxCustomGridDetailsSiteTabsViewInfoClass = class of
    TcxCustomGridDetailsSiteTabsViewInfo;

  TcxCustomGridDetailsSiteTabsViewInfo = class(TcxCustomGridCellViewInfo,
    IcxTabControl, IcxControlComponentState)
  private
  ...
  protected
    ...
    property FirstTabVisibleIndex: Integer read GetFirstTabVisibleIndex
      write SetFirstTabVisibleIndex;
  ...
  public
    constructor Create(ASiteViewInfo
      : TcxCustomGridDetailsSiteViewInfo); virtual;
    destructor Destroy; override;
  ...
  TcxCustomGridDetailsSiteViewInfoCachedInfoClass = class of
  ...
  protected
    ...
    function GetDesignController: TcxCustomGridDesignController;
      virtual; abstract;
    ...
    function GetTabsViewInfoClass
      : TcxCustomGridDetailsSiteTabsViewInfoClass; virtual;
    ...
    procedure GetLevelTabParams(ALevel: TcxGridLevel;
      var AParams: TcxViewParams); virtual;
    procedure InitTabHitTest(AHitTest: TcxGridDetailsSiteTabHitTest);
      virtual; abstract;

    procedure GetCachedInfo(var AInfo
      : TcxCustomGridDetailsSiteViewInfoCachedInfo); virtual;
    function GetCachedInfoClass
      : TcxCustomGridDetailsSiteViewInfoCachedInfoClass; virtual;
    procedure SetCachedInfo(var AInfo
      : TcxCustomGridDetailsSiteViewInfoCachedInfo); virtual;
    ...
    property DesignController: TcxCustomGridDesignController
    ...
    property IsActiveGridViewDestroying: Boolean
      read GetIsActiveGridViewDestroying;
  ...
  public
    ...
    procedure ChangeActiveTab(ALevel: TcxGridLevel;
      AFocusView: Boolean = False); virtual; abstract;
    ...
    property TabsViewInfo: TcxCustomGridDetailsSiteTabsViewInfo
      read FTabsViewInfo;
...
uses
  Types, SysUtils, Math, Forms, Menus, cxPCPainters, cxGrid,
  cxGridCustomTableView, cxGeometry;
...
function TcxGridDetailsSiteTabsPainter.GetViewInfo
  : TcxCustomGridDetailsSiteTabsViewInfo;
begin
...
begin
  Canvas.FrameRect(ViewInfo.FrameBounds, ViewInfo.FrameColor,
    ViewInfo.FrameWidth);
end;
...
function TcxGridLevelTabsProperties.GetLevelDisplayText
  (ALevel: TcxGridLevel): string;
...
function TcxGridLevelTabsProperties.GetSiteViewInfo
...
begin
  Properties.FocusActiveViewOnChange :=
    (Properties.SiteViewInfo.ActiveGridView <> nil) and
...
function TcxGridLevelTabsController.GetClientToScreen
...
function TcxGridLevelTabsController.GetScreenToClient
  (const APoint: TPoint): TPoint;
begin
  Result := inherited GetScreenToClient(APoint);
  Result := cxPointOffset(Result,
    Properties.SiteViewInfo.Bounds.TopLeft, False);
end;
...
procedure TcxGridLevelTabsViewInfo.AfterPaintTab(ACanvas: TcxCanvas;
...
begin
  inherited;
  if IControl.IsDesigning and SiteViewInfo.DesignController.IsObjectSelected
    (Properties.Levels[ATab.Index]) then
...
procedure TcxGridLevelTabsViewInfo.PrepareTabCanvasFont
...
begin
  ...
  if Properties.Levels[ATabViewInfo.Index] = nil then
    Exit;
  SiteViewInfo.GetLevelTabParams(Properties.Levels[ATabViewInfo.Index],
    AParams);
...
function TcxGridLevelTabsViewInfo.GetSiteViewInfo
  : TcxCustomGridDetailsSiteViewInfo;
begin
...
constructor TcxCustomGridDetailsSiteTabsViewInfo.Create(ASiteViewInfo
  : TcxCustomGridDetailsSiteViewInfo);
const
  cxDefaultTabsBounds: TRect = (Left: 0; Top: 0; Right: 289; Bottom: 193);
  // from TcxCustomTabControl
begin
...
procedure TcxCustomGridDetailsSiteTabsViewInfo.TabsPropertiesChangeHandler
  (Sender: TObject);
begin
...
function TcxCustomGridDetailsSiteTabsViewInfo.GetHitTestClass
  : TcxCustomGridHitTestClass;
begin
  Result := TcxGridDetailsSiteTabHitTest;
end;

procedure TcxCustomGridDetailsSiteTabsViewInfo.InitHitTest
...
begin
  ...
  if ATabIndex <> -1 then
    TcxGridDetailsSiteTabHitTest(AHitTest).Level := TabsProperties.Levels
      [ATabIndex]
  else
...
function TcxCustomGridDetailsSiteTabsViewInfo.PtInCaller
  (const P: TPoint): Boolean;

  function GetControlClientPoint: TPoint;
  begin
    Result := dxMapWindowPoint(SiteViewInfo.Control.Handle,
      SiteViewInfo.Container.Handle, P);
  end;
  ...
  begin
    ...
    if Control.IsDesigning then
      TcxCustomGrid(Control).StructureNavigator.SelectObject
        (TabsProperties.ActiveLevel, True);

    ALink := cxAddObjectLink(Self);
    try
      SiteViewInfo.ChangeActiveTab(TabsProperties.ActiveLevel,
        TabsProperties.FocusActiveViewOnChange);
    finally
...
procedure TcxCustomGridDetailsSiteTabsViewInfo.SetFirstTabVisibleIndex
  (Value: Integer);
begin
...
function TcxCustomGridDetailsSiteTabsViewInfo.GetPainterClass
...
const
  ResultMap: array [TcxGridDetailTabsPosition] of TcxTabPosition = (tpTop,
    tpLeft, tpTop);
begin
...
function TcxCustomGridDetailsSiteTabsViewInfo.GetController
...
function TcxCustomGridDetailsSiteTabsViewInfo.GetProperties
  : TcxCustomTabControlProperties;
begin
  Result := TabsProperties;
end;

function TcxCustomGridDetailsSiteTabsViewInfo.GetViewInfo
...
function TcxCustomGridDetailsSiteTabsViewInfo.GetDragAndDropObject
  : TcxDragAndDropObject;
begin
  Result := nil;
end;

function TcxCustomGridDetailsSiteTabsViewInfo.GetDragAndDropState
  : TcxDragAndDropState;
begin
  Result := ddsNone;
end;
...
procedure TcxCustomGridDetailsSiteTabsViewInfo.InvalidateRect(const R: TRect;
  AEraseBackground: Boolean);
begin
  SiteViewInfo.Container.InvalidateRect(cxRectOffset(R, Bounds.TopLeft),
    AEraseBackground);
end;
...
procedure TcxCustomGridDetailsSiteTabsViewInfo.GetClientBounds
  (var AClientBounds: TRect);
begin
...
function TcxCustomGridDetailsSiteTabsViewInfo.ProcessDialogChar
...
function TcxCustomGridDetailsSiteTabsViewInfo.MouseDown
...
function TcxCustomGridDetailsSiteTabsViewInfo.MouseMove
...
function TcxCustomGridDetailsSiteTabsViewInfo.MouseUp
  (AHitTest: TcxCustomGridHitTest; AButton: TMouseButton;
  AShift: TShiftState): Boolean;
var
  P: TPoint;
...
function TcxCustomGridDetailsSiteViewInfo.GetIsActiveGridViewDestroying
  : Boolean;
begin
...
function TcxCustomGridDetailsSiteViewInfo.GetLookAndFeelPainter
...
function TcxCustomGridDetailsSiteViewInfo.GetTabsPosition
...
begin
  ...
  if FNormalHeight > MaxNormalHeight then
    FNormalHeight := MaxNormalHeight;
...
function TcxCustomGridDetailsSiteViewInfo.GetPainterClass
  : TcxGridDetailsSitePainterClass;
begin
  Result := TcxGridDetailsSitePainter;
end;

function TcxCustomGridDetailsSiteViewInfo.GetTabsViewInfoClass
...
function TcxCustomGridDetailsSiteViewInfo.GetActiveGridViewValue
  : TcxCustomGridView;
begin
...
procedure TcxCustomGridDetailsSiteViewInfo.GetLevelTabDefaultParams
  (var AParams: TcxViewParams);
begin
  Level.Styles.GetTabDefaultParams(AParams);
end;

procedure TcxCustomGridDetailsSiteViewInfo.GetLevelTabParams
  (ALevel: TcxGridLevel; var AParams: TcxViewParams);
begin
  Level.Styles.GetTabParams(TcxCustomGridRecord(MasterRecord), ALevel, AParams);
end;

procedure TcxCustomGridDetailsSiteViewInfo.GetCachedInfo
  (var AInfo: TcxCustomGridDetailsSiteViewInfoCachedInfo);
begin
...
function TcxCustomGridDetailsSiteViewInfo.GetCachedInfoClass
  : TcxCustomGridDetailsSiteViewInfoCachedInfoClass;
begin
  Result := TcxCustomGridDetailsSiteViewInfoCachedInfo;
end;

procedure TcxCustomGridDetailsSiteViewInfo.SetCachedInfo
  (var AInfo: TcxCustomGridDetailsSiteViewInfoCachedInfo);
begin
...
procedure TcxCustomGridDetailsSiteViewInfo.Calculate(ALeftBound,
  ATopBound: Integer);
begin
...
procedure TcxCustomGridDetailsSiteViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
begin
...
function TcxCustomGridDetailsSiteViewInfo.GetHitTest(const P: TPoint)
...
function TcxCustomGridDetailsSiteViewInfo.IsTabVisible
...
function TcxCustomGridDetailsSiteViewInfo.ProcessDialogChar
  (ACharCode: Word): Boolean;
begin
  Result := (FTabsViewInfo <> nil) and FTabsViewInfo.ProcessDialogChar
    (ACharCode);
end;

procedure TcxCustomGridDetailsSiteViewInfo.RightToLeftConversion
  (const ABounds: TRect);
begin
