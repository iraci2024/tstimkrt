...
uses
  ...
  cxLookAndFeels, cxClasses, cxLookAndFeelPainters, cxContainer, cxEdit,
  dxLayoutcxEditAdapters,
  dxLayoutControlAdapters, dxLayoutContainer, ExtCtrls, dxLayoutControl,
  cxGridWizardCustomPage, cxGridWizardStrs,
  cxGridTableView, dxLayoutLookAndFeels, cxCheckBox, cxLabel;

type
  { TcxGridWizardTableViewOptionsBehaviorPageFrame }

  TcxGridWizardTableViewOptionsBehaviorPageFrame = class
  ...
  protected
    function GetGridLines(AHorizontalVisible, AVerticalVisible: Boolean)
      : TcxGridLines;
...
begin
  ...
  chbFocusCellOnCycle.Caption := cxGetResourceString
    (@scxgwBehaviorPageFocusCellOnCycle);
  chbFocusCellOnCycle.Hint := cxGetResourceString
  ...
  chbCellMultiSelect.Caption := cxGetResourceString
    (@scxgwBehaviorPageCellMultiSelect);
  chbCellMultiSelect.Hint := cxGetResourceString
    (@scxgwBehaviorPageCellMultiSelectHint);

  chbRecordMultiSelect.Caption := cxGetResourceString
    (@scxgwBehaviorPageRowMultiSelect);
  chbRecordMultiSelect.Hint := cxGetResourceString
  ...
  chbHorizontal.Caption := cxGetResourceString
    (@scxgwBehaviorPageGridLinesHorizontal);
  chbHorizontal.Hint := cxGetResourceString
    (@scxgwBehaviorPageGridLinesHorizontalHint);

  chbVertical.Caption := cxGetResourceString
    (@scxgwBehaviorPageGridLinesVertical);
  chbVertical.Hint := cxGetResourceString
...
begin
  ...
  AView.OptionsView.GridLines := GetGridLines(chbHorizontal.Checked,
    chbVertical.Checked);
  UpdateControlsState;
end;
...
begin
  ...
  chbHorizontal.Checked := (AView.OptionsView.GridLines = glHorizontal) or
    (AView.OptionsView.GridLines = glBoth);
  chbVertical.Checked := (AView.OptionsView.GridLines = glVertical) or
    (AView.OptionsView.GridLines = glBoth);
  UpdateControlsState;
end;
...
function TcxGridWizardTableViewOptionsBehaviorPageFrame.GetGridLines
  (AHorizontalVisible, AVerticalVisible: Boolean): TcxGridLines;
begin
  if AHorizontalVisible and AVerticalVisible then
    Result := glBoth
  else if AHorizontalVisible and not AVerticalVisible then
    Result := glHorizontal
  else if AVerticalVisible and not AHorizontalVisible then
    Result := glVertical
  else
    Result := glNone;
end;

function TcxGridWizardTableViewOptionsBehaviorPageFrame.
...
procedure TcxGridWizardTableViewOptionsBehaviorPageFrame.RefreshPreviewGrid
  (Sender: TObject);
begin
