...
type
  ...
  TcxCustomGridStructureNavigatorClass = class of
  ...
  protected
    ...
    function GetDetailsSiteViewInfoClass
      : TcxGridTopDetailsSiteViewInfoClass; virtual;
  ...
  public
    ...
    property DetailsSiteViewInfo: TcxGridTopDetailsSiteViewInfo
  ...
  published
    property CaptionAlignment: TAlignment read FCaptionAlignment
      write SetCaptionAlignment default cxGridLevelTabsDefaultCaptionAlignment;
    property ImageBorder: Integer read FImageBorder write SetImageBorder
      default cxGridLevelTabsDefaultImageBorder;
  ...
  TcxGridActiveTabChangedEvent = procedure(Sender: TcxCustomGrid;
    ALevel: TcxGridLevel) of object;
  TcxGridActiveTabChangedExEvent = procedure(Sender: TcxCustomGrid;
    ALevel: TcxGridLevel; ARecordIndex: Integer; var ADone: Boolean) of object;
  TcxGridFocusedViewChangedEvent = procedure(Sender: TcxCustomGrid;
    APrevFocusedView, AFocusedView: TcxCustomGridView) of object;
  TcxGridLayoutChangedEvent = procedure(Sender: TcxCustomGrid;
    AGridView: TcxCustomGridView) of object;
  TcxGridPrepareLockedStateImageEvent = procedure(Sender: TcxCustomGrid;
    AImage: TcxBitmap32; var ADone: Boolean) of object;

  TcxCustomGrid = class(TcxControl, IcxNavigator, IcxNavigatorRecordPosition,
    IdxSkinSupport, IcxLockedStatePaint, IcxLockedStateFontChanged)
  private
    FActiveLevel: TcxGridLevel;
    FChanges: TdxFastObjectList;
  ...
  protected
    ...
    procedure RemoveChildComponent(AComponent
      : TcxControlChildComponent); override;
    ...
    function GetLockedStatePaintHelperClass
      : TcxGridLockedStatePaintHelperClass; virtual;
    function GetLockedStateImageOptionsClass
      : TcxGridLockedStateImageOptionsClass; virtual;
    property LockedStatePaintHelper: TcxGridLockedStatePaintHelper
    ...
    procedure DoActiveTabChangedEx(ALevel: TcxGridLevel;
    ...
    procedure FocusedViewChanged(APrevFocusedView, AFocusedView
    ...
    procedure LevelChanged(ALevel: TcxGridLevel;
      AChangeKind: TcxGridLevelChangeKind);
    procedure RefreshNavigators;
    procedure StructureNavigatorChanged;
    function UpdateOnRootViewDataChange: Boolean; virtual;
    procedure ViewChanged(AView: TcxCustomGridView;
  ...
  public
    ...
    function SendNotifications(AKind: TcxGridNotificationKind;
    ...
    property DragOpening: Boolean read FDragOpening write FDragOpening
      default True;
    property DragOpeningWaitTime: Integer read FDragOpeningWaitTime
      write SetDragOpeningWaitTime default cxGridDefaultDragOpeningWaitTime;
    property FocusedView: TcxCustomGridView read FFocusedView
      write SetFocusedView;
    property Font;
    property Images: TCustomImageList read FImages write SetImages;
    ...
    property LockedStateImageOptions: TcxGridLockedStateImageOptions
      read FLockedStateImageOptions write SetLockedStateImageOptions;
    property LookAndFeel;
    property LookAndFeelPainter;
    property Painter: TcxGridPainter read FPainter;
    property RootLevelOptions: TcxGridLevelOptions read GetRootLevelOptions
      write SetRootLevelOptions;
    property RootLevelStyles: TcxGridLevelStyles read GetRootLevelStyles
      write SetRootLevelStyles;
    property StructureNavigator: TcxCustomGridStructureNavigator
      read GetStructureNavigator;
    ...
    property OnActiveTabChangedEx: TcxGridActiveTabChangedExEvent
      read FOnActiveTabChangedEx write FOnActiveTabChangedEx;
    property OnFocusedViewChanged: TcxGridFocusedViewChangedEvent
      read FOnFocusedViewChanged write FOnFocusedViewChanged;
    property OnLayoutChanged: TcxGridLayoutChangedEvent read FOnLayoutChanged
      write FOnLayoutChanged;
  published
    property RootLevelStylesEvents: TNotifyEvent read FSubClassEvents
...
uses
  RTLConsts, SysUtils, Math, cxGridCustomTableView, dxOffice11, cxGeometry,
  cxGridStrs;
...
begin
  ...
  else if (Control <> nil) and (TcxCustomGrid(Control).ViewInfo <> nil) then
  begin
...
function TcxGridLayoutChange.IsCompatibleWith
  (AChange: TcxCustomGridChange): Boolean;
begin
  Result := inherited IsCompatibleWith(AChange) or
    (AChange is TcxGridLayoutChange) and
    ((TcxGridLayoutChange(AChange).GridView = nil) or (GridView <> nil) and
    GridView.HasAsMaster(TcxGridLayoutChange(AChange).GridView)) or
    (AChange is TcxGridSizeChange) and
    ((TcxGridSizeChange(AChange).GridView = nil) or (GridView <> nil) and
    ...
    with GridView do
      ...
      begin
        ...
        if FUpdateGridViewOnly and FKeepMaster then
          LayoutChanged
        else if MasterGridView = nil then
          if { (Control = nil) or } IsPattern then
            { // } LayoutChanged
          else
            TcxCustomGrid(Control).LayoutChanged
        else if not IsPattern and not MasterGridView.SizeChanged(False,
          FKeepMaster) then
          LayoutChanged;
      end;
end;

function TcxGridSizeChange.IsCompatibleWith
  (AChange: TcxCustomGridChange): Boolean;
begin
  Result := inherited IsCompatibleWith(AChange) or
    (AChange is TcxGridSizeChange) and
    (TcxGridSizeChange(AChange).GridView = GridView) and
    (TcxGridSizeChange(AChange).KeepMaster = FKeepMaster) and
    FUpdateGridViewOnly or (AChange is TcxGridDataChange) and
    (TcxGridDataChange(AChange).GridView = GridView);
end;
...
begin
  ...
  else if FUpdateRegion <> nil then
    GridView.Painter.Invalidate(FUpdateRegion)
  else
    GridView.Painter.Invalidate;
end;

function TcxGridViewChange.IsCompatibleWith
  (AChange: TcxCustomGridChange): Boolean;
begin
  Result := inherited IsCompatibleWith(AChange) or
    (AChange is TcxGridLayoutChange) and
    (TcxGridLayoutChange(AChange).GridView = GridView) or
    (AChange is TcxGridSizeChange) and
    (TcxGridSizeChange(AChange).GridView = GridView);
end;

function TcxGridViewChange.IsEqualCore(AChange: TcxCustomGridChange): Boolean;
begin
  Result := inherited IsEqualCore(AChange) and
    ((FUpdateRegion = nil) and (TcxGridViewChange(AChange).UpdateRegion = nil)
    and EqualRect(FUpdateBounds, TcxGridViewChange(AChange).UpdateBounds) or
    (FUpdateRegion <> nil) and (TcxGridViewChange(AChange).UpdateRegion <> nil)
    and FUpdateRegion.IsEqual(TcxGridViewChange(AChange).UpdateRegion));
end;
...
function TcxCustomGridHandler.GetLookAndFeelPainter
...
function TcxGridDragOpenInfoTab.Equals
...
function TcxGridController.GetDesignControllerClass
  : TcxGridDesignControllerClass;
begin
...
procedure TcxGridController.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
...
procedure TcxGridController.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  AHitTest: TcxCustomGridHitTest;
begin
...
procedure TcxGridController.DragOver(Source: TObject; X, Y: Integer;
...
function TcxGridController.GetDragOpenInfo(AHitTest: TcxCustomGridHitTest)
...
procedure TcxGridController.StartDragOpen(ADragOpenInfo
  : TcxCustomGridDragOpenInfo);
begin
...
function TcxGridTopDetailsSiteViewInfo.GetDesignController
...
procedure TcxGridTopDetailsSiteViewInfo.InitTabHitTest
...
function TcxGridTopDetailsSiteViewInfo.DetailHasData
  (ALevel: TcxGridLevel): Boolean;
begin
...
begin
  if not Control.IsDestroying then
    inherited;
end;
...
begin
  FDetailsSiteViewInfo := TcxGridTopDetailsSiteViewInfoClass
    (GetDetailsSiteViewInfoClass).Create(Control.Levels);
...
function TcxGridViewInfo.GetDetailsSiteViewInfoClass
  : TcxGridTopDetailsSiteViewInfoClass;
begin
...
begin
  Result := TcxGridTopDetailsSiteViewInfo(FDetailsSiteViewInfo)
    .GetHitTest(Point(X, Y));
  if Result = nil then
...
begin
  Result := inherited CanCreateLockedImage and not Grid.IsUpdating and
    not Grid.UpdateLocked;
end;
...
begin
  ...
  FViews := TList.Create;
  FChanges := TdxFastObjectList.Create(False);
  FTabStop := True;
  ...
  begin
    for I := 0 to Levels.VisibleCount - 1 do
      with Levels.VisibleItems[I] do
        if GridView <> nil then
          GridView.ViewInfo.DoVisibilityChanged(False);
  end;
...
begin
  ...
  begin
    ...
    else if not Value.Visible then
      Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Result <> nil) and not Result.IsDestroying then
          Exit;
      end;
      Result := nil;
    end;
...
begin
  if (Value <> nil) and not TcxCustomGridViewAccess.CanFocus(Value) then
    Exit;
  ...
  begin
    ...
    if not IsDestroying and not IsDesigning and
      ((Value = nil) and (ActiveLevel <> nil) and (ActiveLevel.GridView <> nil)
      or (Value <> nil) and Value.IsDestroying) then
    ...
    else if IsFocused and CanFocusEx then
      SetFocus;
...
begin
  if (FStructureNavigator = nil) and IsDesigning and
    (cxGridStructureNavigatorClass <> nil) and (FController <> nil) and not FCreatingStructureNavigator
  then
  begin
...
begin
  if TcxGridTopDetailsSiteViewInfo(ViewInfo.DetailsSiteViewInfo)
    .ProcessDialogChar(Message.CharCode) then
    Message.Result := 1
  else
    inherited;
end;
...
function TcxCustomGrid.NavigatorIsActionSupported(AButtonIndex
  : Integer): Boolean;
begin
  Result := True;
end;
...
begin
  Result := 0;
  if (FocusedViewNavigator <> nil) and
    Supports(FocusedView, IcxNavigatorRecordPosition, AIntf) then
...
procedure TcxCustomGrid.RemoveChildComponent(AComponent
...
begin
  inherited;
  if not Handled then
    Handled := SendNotifications(gnkContextMenu,
...
function TcxCustomGrid.GetDesignHitTest(X, Y: Integer;
...
begin
  ...
  begin
    ...
    Result := (AForm <> nil) and (AForm.ActiveControl <> Self) and
      ...
      (not(AForm.ActiveControl is TcxControl) or TcxControl(AForm.ActiveControl)
      .IsFocused);
  end;
end;
...
procedure TcxCustomGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
...
procedure TcxCustomGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
  if not UpdateLocked then
...
procedure TcxCustomGrid.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
procedure TcxCustomGrid.Notification(AComponent: TComponent;
...
begin
  if StructureNavigator <> nil then
    StructureNavigator.BeforeGridLoading;
  inherited;
end;
...
begin
  ...
  begin
    ...
    try
      RetrieveLevels;
      FTag := ALevels;
      RenameComponents(Self, Owner, Name, AOldName, ALevels.Count,
        @cxGrid.GetLevel);
    finally
      ALevels.Free;
    end;
...
procedure TcxCustomGrid.DragOver(Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
begin
  inherited;
...
procedure TcxCustomGrid.BeginLockedStatePaint
...
function TcxCustomGrid.GetLockedStatePaintHelperClass
  : TcxGridLockedStatePaintHelperClass;
begin
  Result := TcxGridLockedStatePaintHelper;
end;

function TcxCustomGrid.GetLockedStateImageOptionsClass
  : TcxGridLockedStateImageOptionsClass;
begin
...
begin
  if StructureNavigator <> nil then
    StructureNavigator.BringToFront;
  if Assigned(FOnActiveTabChanged) then
    FOnActiveTabChanged(Self, ALevel);
end;

procedure TcxCustomGrid.DoActiveTabChangedEx(ALevel: TcxGridLevel;
...
begin
  try
    if { IsLoading or - because of details } IsDestroying then
      Exit;
    with AGridChange do
    begin
      Control := Self;
...
begin
  if Assigned(FOnLayoutChanged) then
    FOnLayoutChanged(Self, AGridView);
end;
...
var
  ...
  begin
    ...
    begin
      ...
      if AChange.IsCumulative then
        for J := 0 to I - 1 do
          if TcxCustomGridChange(AList.List[J]).IsEqual(AChange) then
          begin
            DeleteChange(I);
            Dec(I);
            Break;
  ...
  begin
    ...
    try
      ...
      begin
        AChange := TcxCustomGridChange(AFinalChanges[0]);
...
procedure TcxCustomGrid.FocusedViewChanged(APrevFocusedView,
...
begin
  if IsFocused then
    PostMessage(GetParentForm(Self).Handle, WM_NEXTDLGCTL,
      WParam(not AGoForward), LPARAM(False));
end;
...
begin
  if (FFocusedView <> nil) and not TcxCustomGridViewAccess.CanFocus(FFocusedView)
  then
...
procedure TcxCustomGrid.LevelChanged(ALevel: TcxGridLevel;
  AChangeKind: TcxGridLevelChangeKind);
begin
  StructureNavigatorChanged;
end;
...
procedure TcxCustomGrid.UpdateFocusing(AChildFocused: Boolean);
  ...
  begin
    Result := AChildFocused and not FLastFocused or not AChildFocused and
      not IsFocused;
  end;
...
function TcxCustomGrid.CreateView(AViewClass: TcxCustomGridViewClass)
  : TcxCustomGridView;
begin
...
procedure TcxCustomGrid.RegisterNotification(ANotification
  : TcxCustomGridNotification);
begin
  if FNotifications.IndexOf(ANotification) = -1 then
    FNotifications.Add(ANotification);
end;

procedure TcxCustomGrid.UnregisterNotification(ANotification
  : TcxCustomGridNotification);
begin
...
begin
  Result := False;
  if (AKind = gnkContextMenu) and IsDesigning then
    Exit;
...
procedure TcxGridViewRepository.GetChildren(Proc: TGetChildProc;
  Root: TComponent);
var
  I: Integer;
begin
...
function TcxGridViewRepository.CreateItem(AItemClass: TcxCustomGridViewClass)
  : TcxCustomGridView;
begin
