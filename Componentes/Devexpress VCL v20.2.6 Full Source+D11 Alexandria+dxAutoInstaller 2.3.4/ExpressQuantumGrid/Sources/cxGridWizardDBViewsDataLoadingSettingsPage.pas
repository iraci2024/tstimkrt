...
uses
  ...
  Dialogs, cxGraphics, cxControls, cxClasses,
  cxLookAndFeels, cxLookAndFeelPainters, cxContainer, cxEdit,
  dxLayoutcxEditAdapters, dxLayoutControlAdapters,
  dxLayoutContainer, ExtCtrls, cxTextEdit, cxMaskEdit, cxSpinEdit, cxCheckBox,
  dxLayoutControl, cxDBData,
  cxGridWizardCustomPage, cxGridWizardStrs, cxGroupBox, cxRadioGroup, cxLabel,
  dxCore, cxDropDownEdit,
  dxLayoutLookAndFeels;

type
  { TcxGridWizardDBViewsDataLoadingSettingsPageFrame }

  TcxGridWizardDBViewsDataLoadingSettingsPageFrame = class
...
begin
  ...
  lciGridModeBufferCount.Caption :=
    cxGetResourceString(@scxgwDataSettingsPageGridModeBufferCount);
  ...
  chbSynchronization.Caption := cxGetResourceString
    (@scxgwDataSettingsPageSynchronization);
  chbSynchronization.Hint := cxGetResourceString
    (@scxgwDataSettingsPageSynchronizationHint);

  chbSmartRefresh.Caption := cxGetResourceString
    (@scxgwDataSettingsPageSmartRefresh);
  chbSmartRefresh.Hint := cxGetResourceString
    (@scxgwDataSettingsPageSmartRefreshHint);

  lciMultiThreadedOptionsFiltering.Caption :=
    cxGetResourceString(@scxgwDataSettingsPageMultiThreadedOptionsFiltering);
  lciMultiThreadedOptionsSorting.Caption :=
    cxGetResourceString(@scxgwDataSettingsPageMultiThreadedOptionsSorting);
  for I := 0 to Length(scxGridWizardDefaultBooleanCaptions) - 1 do
  begin
    cbMultiThreadedOptionsFiltering.Properties.Items.Strings[I] :=
      scxGridWizardDefaultBooleanCaptions[I];
    cbMultiThreadedOptionsSorting.Properties.Items.Strings[I] :=
      scxGridWizardDefaultBooleanCaptions[I];
  end;
end;
...
const
  MultiThreadedOptionsValuesMap: array [0 .. 2] of TdxDefaultBoolean = (bFalse,
    bTrue, bDefault);
var
  ADataController: TcxDBDataController;
begin
  ADataController := (Helper as IcxGridWizardHelperDBDataControllerSupport)
    .GetDataController;

  ADataController.DataModeController.SyncMode := chbSyncMode.Checked;
  ADataController.MultiThreadedOptions.Filtering :=
    MultiThreadedOptionsValuesMap[cbMultiThreadedOptionsFiltering.ItemIndex];
  ADataController.MultiThreadedOptions.Sorting := MultiThreadedOptionsValuesMap
    [cbMultiThreadedOptionsSorting.ItemIndex];
...
const
  MultiThreadedOptionsValuesMap: array [TdxDefaultBoolean] of Integer =
    (0, 1, 2);
var
  ADataController: TcxDBDataController;
begin
  ...
  cbMultiThreadedOptionsFiltering.ItemIndex := MultiThreadedOptionsValuesMap
    [ADataController.MultiThreadedOptions.Filtering];
  cbMultiThreadedOptionsSorting.ItemIndex := MultiThreadedOptionsValuesMap
    [ADataController.MultiThreadedOptions.Sorting];
  seGridModeBufferCount.Enabled := chbGridMode.Checked;
end;

function TcxGridWizardDBViewsDataLoadingSettingsPageFrame.
...
procedure TcxGridWizardDBViewsDataLoadingSettingsPageFrame.RefreshPreviewGrid
  (Sender: TObject);
begin
  RefreshPreviewGridContent;
end;

procedure TcxGridWizardDBViewsDataLoadingSettingsPageFrame.
  chbGridModePropertiesChange(Sender: TObject);
begin
  RefreshPreviewGridContent;
  seGridModeBufferCount.Enabled := chbGridMode.Checked;
end;
