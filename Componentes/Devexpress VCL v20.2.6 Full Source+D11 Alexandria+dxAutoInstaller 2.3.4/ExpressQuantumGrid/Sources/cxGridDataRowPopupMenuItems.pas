...
begin
  Result := TcxGridDataRow(TcxGridRecordHitTest(GridPopupMenu.HitTest)
    .GridRecord);
end;
...
begin
  Result := HitGridView.OptionsCustomize.DataRowFixing and
    not HitRow.IsNewItemRow and not HitRow.IsFilterRow and
    (HitRow.FixedState <> GetOperation);
end;
...
function TcxGridDataRowFixToTopOperation.GetOperation
...
function TcxGridDataRowFixToBottomOperation.GetOperation
  : TcxDataControllerRowFixedState;
begin
...
function TcxGridDataRowUnfixOperation.GetOperation
  : TcxDataControllerRowFixedState;
begin
  Result := rfsNotFixed;
end;
...
function TcxGridDataRowPopupMenuOperations.CanProcess
  (AHitTest: TcxCustomGridHitTest): Boolean;
begin
  Result := inherited CanProcess(AHitTest) and
    (AHitTest is TcxGridRecordHitTest) and TcxGridRecordHitTest(AHitTest)
    .GridRecord.IsData;
end;
