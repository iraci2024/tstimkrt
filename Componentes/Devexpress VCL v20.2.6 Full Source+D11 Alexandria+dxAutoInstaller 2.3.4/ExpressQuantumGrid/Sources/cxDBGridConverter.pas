...
uses
  ...
  cxDropDownEdit, cxTextEdit, cxDBLookupComboBox, cxStyles, Graphics,
  cxCustomConverter;
...
function TcxDBGridConverter.FindcxColumn(const AFieldName: string)
  : TcxGridDBColumn;
var
  I: Integer;
begin
...
begin
  Result := GetWideStringProperty(AColumn, 'FieldName',
    'Field' + IntToStr(AColumn.Index));
end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              ALookupComboBoxProperties.DropDownRows :=
                GetIntegerProperty(Items[I], 'DropDownRows',
                ALookupComboBoxProperties.DropDownRows);
              ALookupComboBoxProperties.ReadOnly :=
                GetBooleanProperty(Items[I], 'ReadOnly',
                ALookupComboBoxProperties.ReadOnly);
            end
            else
            begin
              ...
              begin
                ...
                begin
                  ...
                  AComboBoxProperties :=
                    TcxComboBoxProperties(AcxColumn.Properties);
                  for J := 0 to (AObject as TStrings).Count - 1 do
                    AComboBoxProperties.Items.Add((AObject as TStrings)[J]);
                  AComboBoxProperties.ReadOnly :=
                    GetBooleanProperty(TCollection(AColumns).Items[I],
                    'ReadOnly', AComboBoxProperties.ReadOnly);
                  AComboBoxProperties.Alignment.Horz :=
                    ConvertAlignment
                    (GetEnumProperty(TCollection(AColumns).Items[I],
                    'Alignment'));
                  AComboBoxProperties.DropDownRows :=
                    GetIntegerProperty(TCollection(AColumns).Items[I],
                    'DropDownRows', AComboBoxProperties.DropDownRows);
                end
                else
                begin
                  ...
                  begin
                    ...
                    TcxTextEditProperties(AcxColumn.Properties).ReadOnly :=
                      AReadOnly;
                    TcxTextEditProperties(AcxColumn.Properties).Alignment.Horz
                      := AAlignment;
                  end;
                end;
              end;
          ...
          begin
            AReadOnly := GetBooleanProperty(Items[I], 'ReadOnly', False);
            AAlignment := ConvertAlignment(GetEnumProperty(Items[I],
              'Alignment'));
            if AReadOnly or (AAlignment <> taLeftJustify) then
            begin
              AcxColumn.PropertiesClass := TcxTextEditProperties;
          ...
          if ATitle <> nil then
          begin
            AcxColumn.Caption := GetStringProperty(ATitle, 'Caption',
              AcxColumn.Caption);
            AcxColumn.HeaderAlignmentHorz :=
              ConvertAlignment(GetEnumProperty(ATitle, 'Alignment'));
          end;
          AcxColumn.Visible := GetBooleanProperty(Items[I], 'Visible',
            AcxColumn.Visible);
          AcxColumn.Width := GetIntegerProperty(Items[I], 'Width',
            AcxColumn.Width);
        end;
      end;
  end;
          ...
          begin
            ...
            begin
              ...
              if ((AColor <> clBtnFace) and (AColor <> FFixedColor)) or
                (not DefaultFont(AFont) and not CompareFonts(AFont, FTitleFont))
              then
              begin
                AStyle := CreateStyleItem;
                (AStyle as TcxStyle).Color := AColor;
                (AStyle as TcxStyle).Font.Assign(AFont);
                (AStyle as TcxStyle).TextColor := (AStyle as TcxStyle)
                  .Font.Color;
...
ConverterFactory(cxGridGroupConverterName).RegisterConverter
  ('DB Grid Converter', TcxDBGridConverter);
