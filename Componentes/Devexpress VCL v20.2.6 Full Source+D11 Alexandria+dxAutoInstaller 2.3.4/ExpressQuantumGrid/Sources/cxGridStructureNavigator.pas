...
type
  ...
  public
    property Items[Index: Integer]: TcxGridStructureControlRowViewInfo
  ...
  private
    ...
    procedure GetLevelInfo(AIndex: Integer; var ALevelText, AViewText: string;
      var ALevelIndex: Integer; var AIsParent, AIsLast: Boolean);
  ...
  public
    ...
    function GetRowBorderColor(AColorIndex: TcxGridColorIndex;
      ASelected, AHighlighted: Boolean): TColor;
    function GetRowColor(AColorIndex: TcxGridColorIndex;
      ASelected, AHighlighted: Boolean): TColor;
    function GetRowTextColor(AColorIndex: TcxGridColorIndex;
      ASelected, AHighlighted: Boolean): TColor;
  ...
  protected
    ...
    function GetComponentByHitInfo(AHitInfo: TcxGridStructureControlHitInfo)
      : TComponent;
  ...
  public
    ...
    property OnKeyPress;
    property OnDrawBackground: TNotifyEvent read FOnDrawBackground
      write FOnDrawBackground;
    property OnSelectComponent: TcxGridStructureControlSelectComponentEvent
      read FOnSelectComponent write FOnSelectComponent;
  ...
  protected
    ...
    procedure ExecuteLevelViewsMenuItem(ALevel: TcxGridLevel;
      AMenuItem: TMenuItem);
    procedure FillLevelViewsMenu(AMenu: TMenuItem; ALevel: TcxGridLevel;
      AOnClick: TNotifyEvent);
    procedure StructureControlMouseDown(Sender: TObject; Button: TMouseButton;
  ...
  public
    ...
    destructor Destroy; override;
    property StructureControl: TcxGridStructureControl read FStructureControl;
    property OnUpdateDesigner: TNotifyEvent read FOnUpdateDesigner
      write FOnUpdateDesigner;
  end;
  ...
  private
    ...
    procedure CMDeferUpdate(var Message: TMessage);
  ...
  protected
    ...
    property MakeSelectionVisible: Boolean read FMakeSelectionVisible
      write FMakeSelectionVisible;
  public
...
function GenViewName(AOwnerComponent: TComponent;
  AView: TcxCustomGridView): string;
...
function GenViewName(AOwnerComponent: TComponent;
  AView: TcxCustomGridView): string;
begin
  Result := CreateUniqueName(AOwnerComponent.Owner, AOwnerComponent, AView,
    ScxGridPrefixName, '');
end;

function FillUnboundRegisteredViewsMenu(AMenu: TMenuItem;
  AOnClick: TNotifyEvent; ACheckAddSeparator: Boolean): Boolean;
var
  I: Integer;
begin
  Result := False;
  for I := 0 to cxGridRegisteredViews.Count - 1 do
    if (Pos(SGridDBViewPrefix, cxGridRegisteredViews.Descriptions[I]) <> 1) and
      (Pos(SGridServerModeViewPrefix, cxGridRegisteredViews.Descriptions[I]) <>
      1) and (Pos(SGridEMFViewPrefix, cxGridRegisteredViews.Descriptions[I])
      <> 1) then
    begin
      if not Result and ACheckAddSeparator then
        AMenu.Add(CreateMenuItem(AMenu.Owner, '-'));
      AMenu.Add(CreateMenuItem(AMenu.Owner, cxGridRegisteredViews.Descriptions
        [I], AOnClick, True, I));
      Result := True;
    end;
end;
...
function FillServerModeRegisteredViewsMenu(AMenu: TMenuItem;
  AOnClick: TNotifyEvent; ACheckAddSeparator: Boolean): Boolean;
var
  I: Integer;
begin
  Result := False;
  for I := 0 to cxGridRegisteredViews.Count - 1 do
    if Pos(SGridServerModeViewPrefix, cxGridRegisteredViews.Descriptions[I]) = 1
    then
    begin
      if not Result and ACheckAddSeparator then
        AMenu.Add(CreateMenuItem(AMenu.Owner, '-'));
...
begin
  ...
  ACheckAddSeparator := FillDBRegisteredViewsMenu(AMenu, AOnClick,
    ACheckAddSeparator);
  ACheckAddSeparator := FillServerModeRegisteredViewsMenu(AMenu, AOnClick,
    ACheckAddSeparator);
  ...
  begin
    ...
    try
      cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat)
        .DrawScaledButtonCross(ACanvas, R, clBtnText, cxbsNormal,
        dxSystemScaleFactor);
    finally
      ACanvas.Free;
    end;
...
procedure TDesignNotificationHandler.SelectionChanged(const ADesigner
  : IDesigner; const ASelection: IDesignerSelections);
var
...
begin
  ...
  try
    ...
    for I := 0 to FGrids.Count - 1 do
      TcxCustomGrid(FGrids[I]).StructureNavigator.SelectionChanged
        (ASelectionList);
  finally
    ASelectionList.Free;
  end;
...
function TcxGridStructureControlRowsViewInfo.GetItem(Index: Integer)
...
constructor TcxGridStructureControlViewInfo.Create(AStructureControl
  : TcxGridStructureControl);
begin
  inherited Create;
...
begin
  ...
  try
    ...
    FRowHeight := 2 * RowOffset + 2 * RowBorderWidth + 2 * TextOffset +
      FTextHeight;
    // Rows
    FRows.Clear;
    ...
    begin
      ...
      // Level
      ARow.LevelTextArea := Rect(0, ATop, Canvas.TextWidth(ARow.LevelText),
        ATop + FTextHeight);
      ...
      begin
        W := 2 * RowBorderWidth + 2 * TextOffset +
          Canvas.TextWidth(ARow.ViewText);
        ARow.ViewBounds := Rect(ARow.LevelBounds.Right - RowBorderWidth,
          ARow.LevelBounds.Top, ARow.LevelBounds.Right - RowBorderWidth + W,
          ARow.LevelBounds.Bottom);
...
function TcxGridStructureControlViewInfo.GetGridColor
...
function TcxGridStructureControlViewInfo.GetLevelColor
  (AHighlighted: Boolean): TColor;
begin
  if AHighlighted then
...
function TcxGridStructureControlViewInfo.GetRowBorderColor
  (AColorIndex: TcxGridColorIndex; ASelected, AHighlighted: Boolean): TColor;
begin
  if ASelected then
    if AHighlighted then
...
function TcxGridStructureControlViewInfo.GetRowColor
  (AColorIndex: TcxGridColorIndex; ASelected, AHighlighted: Boolean): TColor;
begin
  if ASelected then
...
function TcxGridStructureControlViewInfo.GetRowTextColor
  (AColorIndex: TcxGridColorIndex; ASelected, AHighlighted: Boolean): TColor;
begin
  if ASelected then
    Result := GetSelectedFontColor
  else if AHighlighted then
    Result := GetHighlightedFontColor
  else
    Result := GetContentFontColor;
end;

function TcxGridStructureControlViewInfo.GetSelectedColor
  (AHighlighted: Boolean): TColor;
begin
  if AHighlighted then
...
function TcxGridStructureControlViewInfo.GetViewColor
  (AHighlighted: Boolean): TColor;
begin
  if AHighlighted then
...
procedure TcxGridStructureControlViewInfo.Paint;
  ...
  procedure SetFontAndBrush(ASelected, AHighlighted: Boolean;
    AColorIndex: TcxGridColorIndex);
  begin
    Canvas.Brush.Color := dxGetNearestColor(GetRowColor(AColorIndex, ASelected,
      AHighlighted));
    Canvas.Font.Color := dxGetNearestColor(GetRowTextColor(AColorIndex,
  ...
  begin
    ...
    Canvas.DrawText(AText, ATextRect, cxAlignLeft or cxAlignVCenter or
      cxSingleLine);
  ...
  begin
    ...
    begin
      ...
      Canvas.FrameRect(ARow.LevelBounds,
        dxGetNearestColor(GetRowBorderColor(AColorIndex, ARow.LevelSelected,
        ARow.LevelHighlighted)), RowBorderWidth);
      ...
      begin
        Canvas.FrameRect(ARow.ViewBounds,
          dxGetNearestColor(GetRowBorderColor(ciView, ARow.ViewSelected,
          ARow.ViewHighlighted)), RowBorderWidth);
...
function TcxGridStructureControlViewInfo.GetLevelByIndex(AIndex: Integer)
  : TcxGridLevel;
var
  ACurrent: Integer;
...
function TcxGridStructureControl.GetHitInfo(P: TPoint)
...
begin
  ...
  for I := 0 to ViewInfo.Rows.Count - 1 do
    ...
    begin
      ...
      else if PtInRect(ViewInfo.Rows[I].ViewBounds, P) then
      begin
...
function TcxGridStructureControl.GetLevelByRowIndex(ARowIndex: Integer)
  : TcxGridLevel;
begin
...
procedure TcxGridStructureControl.SyncSelection(ASelectionList: TList);
  ...
  procedure CheckLevel(ARow: TcxGridStructureControlRowViewInfo;
  ...
  procedure CheckView(ARow: TcxGridStructureControlRowViewInfo;
    AComponent: TComponent);
  var
    ASelected: Boolean;
  begin
...
procedure TcxGridStructureControl.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  AComponent: TComponent;
begin
  ...
  begin
    ...
    if AComponent <> nil then
      SelectComponent(AComponent, (Button = mbRight) or not MultiSelect or
...
procedure TcxGridStructureControl.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  if Button in [mbLeft, mbRight] then
...
procedure TcxGridStructureControl.AddToSelection(AObject: TPersistent;
...
function TcxGridStructureControl.GetComponentByHitInfo
  (AHitInfo: TcxGridStructureControlHitInfo): TComponent;
var
  ALevel: TcxGridLevel;
begin
  Result := nil;
  if AHitInfo.RowIndex = -1 then
    Exit;
  ...
  else if AHitInfo.HitTest = htView then
  begin
    if ALevel.GridView <> nil then
...
function TcxGridStructureHelper.CanDeleteComponent
...
procedure TcxGridStructureHelper.ExecuteLevelViewsMenuItem(ALevel: TcxGridLevel;
  AMenuItem: TMenuItem);
var
...
begin
  ...
  try
    for I := 0 to ALevelViewList.ViewNames.Count - 1 do
      AMenu.Add(CreateMenuItem(AMenu.Owner, ALevelViewList.ViewNames[I],
        AOnClick));
    if ALevel.GridView <> nil then
  ...
  AMenu.Add(CreateMenuItem(AMenu.Owner, '-'));
  AMenu.Add(CreateMenuItem(AMenu.Owner, 'None', AOnClick, True, -1,
    ALevel.GridView = nil));
end;
...
procedure TcxGridStructureHelper.StructureControlMouseDown(Sender: TObject;
  ...
  begin
    FPopupMenu.Items.Add(CreateMenuItem(FPopupMenu, 'Add Level',
      CreateLevelClick, CanAddComponent));
  end;
  ...
  var
    AMenuItem: TMenuItem;
  begin
    AMenuItem := CreateMenuItem(FPopupMenu, 'Create View', nil,
      CanAddComponent);
    ...
    FPopupMenu.Items.Add(CreateMenuItem(FPopupMenu, '-'));
    FPopupMenu.Items.Add(CreateMenuItem(FPopupMenu, 'Move Level Up',
      MoveLevelClick, FPopupMenuLevel.Index > 0, -1));
    FPopupMenu.Items.Add(CreateMenuItem(FPopupMenu, 'Move Level Down',
      MoveLevelClick, FPopupMenuLevel.Index <
      FPopupMenuLevel.Parent.Count - 1, 1));
  end;

  function PrepareViewMenu(AView: TcxCustomGridView)
  ...
  begin
    ...
    begin
      if FPopupMenuLevel.IsRoot then
        PrepareGridMenu
      else if AHitInfo.HitTest = htLevel then
        PrepareLevelMenu
      else
      begin
        FViewMenuProvider.Free;
...
begin
  ...
  try
    StructureControl.GetSelection(ASelection);
    FPopupMenuLevel.Index := FPopupMenuLevel.Index +
      Integer(TMenuItem(Sender).Tag);
    UpdateDesigner;
    StructureControl.SyncSelection(ASelection);
  finally
    ASelection.Free;
...
begin
  ...
  ACanvas := FStructureControl.ViewInfo.Canvas;
  ACanvas.Font := FCustomizeButton.Font;
  ACloseButtonSize.cx := 2 * TextOffset + ACanvas.TextWidth(SCustomizeText) + 2
    * ButtonBorderSize;
  ACloseButtonSize.cy := 2 * TextOffset + ACanvas.TextHeight(SCustomizeText) + 2
    * ButtonBorderSize;
  ACustomizeButtonArea := Rect(R.Left, R.Bottom + ASeparatorHeight,
    R.Left + 2 * ButtonOffset + ACloseButtonSize.cx, R.Bottom + ASeparatorHeight
    + 2 * ButtonOffset + ACloseButtonSize.cy);
  FCustomizeButton.SetBounds(ACustomizeButtonArea.Left + ButtonOffset,
    ACustomizeButtonArea.Top + ButtonOffset, ACloseButtonSize.cx,
    ACloseButtonSize.cy);
  ASeparatorArea := Rect(ACustomizeButtonArea.Left, ACustomizeButtonArea.Top -
    ASeparatorHeight, ACustomizeButtonArea.Right, ACustomizeButtonArea.Top);
  if R.Right < ACustomizeButtonArea.Right then
    R.Right := ACustomizeButtonArea.Right;
  FSeparator.BoundsRect := Rect(ASeparatorArea.Left + ButtonOffset,
    ASeparatorArea.Top + ButtonOffset, R.Right - ButtonOffset,
    ASeparatorArea.Bottom);
  // Calc Close Button
  ACloseButtonArea := Rect(R.Right, R.Top, R.Right + 2 * ButtonOffset +
    CloseButtonWidth + 2 * ButtonBorderSize, R.Top + 2 * ButtonOffset +
    CloseButtonHeight + 2 * ButtonBorderSize);
  FCloseButton.SetBounds(ACloseButtonArea.Left + ButtonOffset,
    ACloseButtonArea.Top + ButtonOffset, CloseButtonWidth + 2 *
    ButtonBorderSize, CloseButtonHeight + 2 * ButtonBorderSize);
  if R.Bottom < ACloseButtonArea.Bottom then
    R.Bottom := ACloseButtonArea.Bottom;
  // Total Area
  R.Right := ACloseButtonArea.Right;
...
begin
  Office11FillTubeGradientRect(ACanvas.Handle, ClientBounds, clWhite,
    { 16248293 } 16115935 { 15785423 } , False);
end;

procedure TcxGridStructureNavigator.DrawStructureControlBackground
  (Sender: TObject);
begin
  MoveWindowOrg(FStructureControl.Canvas.Handle, -FStructureControl.Left,
    -FStructureControl.Top);
...
function TcxGridStructureNavigator.IsObjectSelected
...
begin
  if not HandleAllocated then
    Exit;
  if not PeekMessage(Msg, Handle, DXM_REFRESHCUSTOMIZATION,
    DXM_REFRESHCUSTOMIZATION, PM_NOREMOVE) then
