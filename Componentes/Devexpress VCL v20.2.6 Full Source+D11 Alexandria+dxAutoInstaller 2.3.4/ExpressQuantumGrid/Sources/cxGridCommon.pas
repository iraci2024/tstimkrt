...
type
  ...
  private
    FControl: TcxControl;
  protected
    function IsEqualCore(AChange: TcxCustomGridChange): Boolean;
      virtual; abstract;
  public
  ...
  public
    function Equals(AInfo: TcxCustomGridDragOpenInfo): Boolean;
      reintroduce; virtual;
  ...
  TcxGridOptionsTreeViewGetTypeItemCaptionFunction = function
    (ATypeItem: Integer): string;
  TcxGridOptionsTreeViewItemKind = (otikCategory, otikCheckBox,
    otikRadioButton);
  ...
  protected
    ...
    function GetItemImageIndex(AKind: TcxGridOptionsTreeViewItemKind;
  ...
  public
    ...
    function AddCategory(AParentID, AID: Integer;
      const ACaption: string): Integer;
    function AddCheckBox(AParentID, AID: Integer;
      const ACaption: string): Integer;
    function AddRadioButton(AParentID, AID: Integer;
      const ACaption: string): Integer;
...
procedure GetCellTextAreaSize(var ATextSize: Integer;
  AScaleFactor: TdxScaleFactor; AHorizontal: Boolean = False);
function GridCellStateToButtonState(ACellState: TcxGridCellState)
  : TcxButtonState;
...
function TcxCustomGridChange.IsCompatibleWith
...
function TcxCustomGridDragOpenInfo.Equals
...
constructor TOptionTreeNodeData.Create(AID: Integer;
  ...
  begin
    ...
    begin
      ...
      Sender.Canvas.Font.Color :=
        LookAndFeelPainter.DefaultGridOptionsTreeViewCategoryTextColor
        (cdsSelected in State);
      Sender.Canvas.Brush.Color :=
        LookAndFeelPainter.DefaultGridOptionsTreeViewCategoryColor
        (cdsSelected in State);
...
procedure TcxGridOptionsTreeView.GetNodeImageIndex(Sender: TObject;
  Node: TTreeNode);
begin
  if GetNodeKind(Node) = otikCategory then
...
procedure TcxGridOptionsTreeView.GetNodeSelectedImageIndex(Sender: TObject;
  Node: TTreeNode);
begin
  if GetNodeKind(Node) = otikCategory then
...
begin
  ...
  Result := AID;
  Items.AddChildObject(AParent, ACaption,
    TOptionTreeNodeData.Create(AID, AKind));
end;
...
begin
  ...
  begin
    Result := Items[I];
    if GetNodeID(Result) = AID then
      Exit;
  end;
  Result := nil;
end;

function TcxGridOptionsTreeView.GetItemImageIndex
  (AKind: TcxGridOptionsTreeViewItemKind; AChecked: Boolean): Integer;
begin
...
function TcxGridOptionsTreeView.GetNodeKind(ANode: TTreeNode)
  : TcxGridOptionsTreeViewItemKind;
begin
...
begin
  ...
  try
    for AItemKind := Low(AItemKind) to High(AItemKind) do
      ...
      begin
        AImage.cxCanvas.FillRect(AImage.ClientRect, ViewInfo.BackgroundColor);
...
procedure TcxGridOptionsTreeView.DrawImage(ACanvas: TcxCanvas;
  const ARect: TRect; AItemKind: TcxGridOptionsTreeViewItemKind;
  AChecked: Boolean);
var
  R: TRect;
  B: TBitmap;
  ...
  case AItemKind of
    ...
    otikCheckBox:
      begin
        ...
        LookAndFeelPainter.DrawScaledCheckButton(ACanvas, R, cxbsNormal,
          AChecked, ScaleFactor);
      end;
    otikRadioButton:
      begin
        R := cxRectCenter(R, GetRadioButtonSize);
        LookAndFeelPainter.DrawScaledRadioButton(ACanvas, R.Left, R.Top,
          cxbsNormal, AChecked, False, ACanvas.Brush.Color, ScaleFactor);
      end;
  end;
end;
...
function GridCellStateToButtonState(ACellState: TcxGridCellState)
  : TcxButtonState;
const
  ButtonStates: array [TcxGridCellState] of TcxButtonState = (cxbsNormal,
    cxbsHot, cxbsPressed);
begin
