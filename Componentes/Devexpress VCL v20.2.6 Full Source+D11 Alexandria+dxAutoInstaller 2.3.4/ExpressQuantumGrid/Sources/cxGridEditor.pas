...
uses
  ...
  cxGridViewLayoutEditor, cxLookAndFeelPainters, cxButtons, cxPC,
  cxLookAndFeels,
  cxGraphics;
...
type
  ...
  private
    ...
    procedure ShowViewFrame(AView: TcxCustomGridView; AMultiView: Boolean;
  ...
  protected
    ...
    function GetViewEditorClass(AView: TcxCustomGridView)
      : TcxViewEditorClass; virtual;
...
procedure ShowViewRepositoryEditor(ADesigner: IDesigner;
  AViewRepository: TcxGridViewRepository);
...
procedure TcxGridEditor.SelectionsChanged(const ASelection
...
function TcxGridEditor.GetSelectionInfo: TSelectionInfo;
  ...
  procedure FindView(ASelection: TDesignerSelectionList;
  ...
  begin
    for I := 0 to ASelection.Count - 1 do
      if (ASelection[I] is TcxCustomGridView) and
        IsViewLinkedToComponent(TcxCustomGridView(ASelection[I]), Component)
      then
  ...
  procedure FindLevel(ASelection: TDesignerSelectionList;
    var AInfo: TSelectionInfo);
  var
    I: Integer;
    ALevel: TcxGridLevel;
  ...
  procedure FindViewItem(ASelection: TDesignerSelectionList;
    var AInfo: TSelectionInfo);
  var
    I: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      if (Result.Level = nil) and (Result.View = nil) and not Result.MultiSelect
      then
...
function TcxGridEditor.GetViewEditorClass(AView: TcxCustomGridView)
  : TcxViewEditorClass;
begin
  Result := cxViewEditor.GetViewEditorClass
...
begin
  if (Grid = nil) or (Grid.ActiveLevel = nil) { or Grid.ActiveLevel.IsMaster }
  then
    Exit;
  ASelectionInfo := GetSelectionInfo;
  if (ASelectionInfo.Level = nil) and (ASelectionInfo.View = nil) and
    not ASelectionInfo.MultiSelect then
...
begin
  ...
  else if Component is TcxCustomGridView then
...
procedure TcxGridEditor.GridStructureControlKeyPress(Sender: TObject;
  var Key: Char);
begin
  case Key of
    #13, #33 .. #126:
...
procedure TcxGridEditor.ShowViewFrame(AView: TcxCustomGridView;
  AMultiView: Boolean; ARefreshNeeded: Boolean);
...
begin
  if (FViewEditor <> nil) and (AView <> nil) and
    (FViewEditor.ClassType = GetViewEditorClass(AView)) and
  ...
  begin
    ...
    try
      ...
      cxRedrawWindow(PViewFrame.Handle, RDW_INVALIDATE or RDW_ERASE or
        RDW_ALLCHILDREN);
    end;
  end;
...
begin
  ...
  BDeleteLevel.Enabled := CanDeleteComponent(nil) and
    (ASelectionLevelCount > 0);
  ...
  miCustomizeEditFormLayout.Visible := (LBViews.SelCount = 1) and
    ...
    (TcxGridTableView(GetSelectedViewObject).OptionsBehavior.EditMode <>
    emInplace);
end;
...
begin
  ASelectionInfo := GetSelectionInfo;
  ShowViewFrame(ASelectionInfo.View, ASelectionInfo.MultiSelect,
    ARefreshNeeded);
end;
...
function LevelsCompare(AItem1, AItem2: Pointer): Integer;
var
...
procedure TcxGridEditor.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  SaveSettings;
  inherited;
end;
