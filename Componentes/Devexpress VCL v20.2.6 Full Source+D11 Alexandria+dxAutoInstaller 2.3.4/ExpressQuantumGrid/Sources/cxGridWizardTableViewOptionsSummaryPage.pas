...
uses
  ...
  dxLayoutContainer, cxCheckBox, ExtCtrls,
...
type
  ...
  TcxGridWizardTableViewOptionsSummaryPageFrame = class
...
begin
  ...
  chbSelectedRecords.Caption := cxGetResourceString
    (@scxgwSummaryPageSelectedRecords);
  chbSelectedRecords.Hint := cxGetResourceString
    (@scxgwSummaryPageSelectedRecordsHint);

  chbMultipleSelectedRecords.Caption :=
    cxGetResourceString(@scxgwSummaryPageMultipleSelectedRecords);
  chbMultipleSelectedRecords.Hint :=
    cxGetResourceString(@scxgwSummaryPageMultipleSelectedRecordsHint);
end;
...
begin
  ...
  AView.DataController.Summary.Assign
    (PreviewGrid.ActiveView.DataController.Summary);
  AView.DataController.Summary.Options := [];
  if chbNullIgnore.Checked then
    AView.DataController.Summary.Options := AView.DataController.Summary.Options
      + [soNullIgnore];
...
begin
  ...
  AView := TcxGridTableView(Helper.GridView);
  PreviewGrid.ActiveView.DataController.Summary.Assign
    (AView.DataController.Summary);
  chbNullIgnore.Checked := soNullIgnore in AView.DataController.Summary.Options;
  chbSelectedRecords.Checked := soSelectedRecords
    in AView.DataController.Summary.Options;
  chbMultipleSelectedRecords.Checked := soMultipleSelectedRecords
    in AView.DataController.Summary.Options;
  rbAllRecords.Checked := not(chbSelectedRecords.Checked or
    chbMultipleSelectedRecords.Checked);
  ActivatePopupMenu;
end;
...
begin
  ...
  pupmSummaryCustomization.PopupMenus.Items[0].GridView :=
    PreviewGrid.ActiveView;
end;

function TcxGridWizardTableViewOptionsSummaryPageFrame.
...
procedure TcxGridWizardTableViewOptionsSummaryPageFrame.RefreshPreviewGrid
  (Sender: TObject);
begin
