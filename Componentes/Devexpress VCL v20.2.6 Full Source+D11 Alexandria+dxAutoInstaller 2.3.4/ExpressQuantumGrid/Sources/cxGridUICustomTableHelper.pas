...
type
  TcxGridCustomTableViewOperationHelper = class
    (TcxCustomGridViewOperationHelper)
  protected
    procedure RegisterOperations; override;
    procedure DoCopyToClipboard(const AParameter
      : TcxCustomGridOperationHelperParameters);
    procedure DoShowEditButtons(const AParameter
      : TcxCustomGridOperationHelperParameters);
  public
...
procedure TcxGridCustomTableViewOperationHelper.DoShowEditButtons
  (const AParameter: TcxCustomGridOperationHelperParameters);
const
  ShowEditButtonsToBool: Array [TcxGridShowEditButtons] of Boolean = (False,
    False, True);
  BoolToShowEditButtons: Array [Boolean] of TcxGridShowEditButtons = (gsebNever,
    gsebAlways);
begin
  if AParameter.IsPerform then
    ...
    SetShowProperty(AParameter, ShowEditButtonsToBool
      [TcxCustomGridTableView(AParameter.View).OptionsView.ShowEditButtons]);
end;

class function TcxGridCustomTableViewOperationHelper.GetViewClass
