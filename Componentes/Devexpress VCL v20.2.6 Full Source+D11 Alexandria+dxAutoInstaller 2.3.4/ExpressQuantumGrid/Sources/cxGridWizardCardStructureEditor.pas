...
uses
  ...
  cxButtons, cxContainer, cxEdit, cxTreeView, dxLayoutControl,
  dxLayoutLookAndFeels, cxClasses,
  cxGridCustomView, cxGridCustomTableView, cxGridCardView, cxGrid,
  cxGridWizardStrs;
...
type
  ...
  protected
    ...
    procedure PreviewGridLayoutChangedHandler(Sender: TcxCustomGrid;
      AGridView: TcxCustomGridView);
    procedure PreviewViewFocusedItemChangedHandler
...
procedure TcxGridWizardCardStructureEditorFrame.LoadSettings
  (AGrid: TcxCustomGrid);
begin
  FPreviewGrid := AGrid;
...
procedure TcxGridWizardCardStructureEditorFrame.AddRowToTreeView
...
begin
  btnAddCaptionRow.Caption := cxGetResourceString
    (@scxgwCardViewTreeViewFrameAddCaptionRow);
  btnAddCaptionRow.Hint := cxGetResourceString
    (@scxgwCardViewTreeViewFrameAddCaptionRowHint);
  miAddCaptionRow.Caption := btnAddCaptionRow.Caption;

  btnAddCategoryRow.Caption := cxGetResourceString
    (@scxgwCardViewTreeViewFrameAddCategoryRow);
  btnAddCategoryRow.Hint := cxGetResourceString
    (@scxgwCardViewTreeViewFrameAddCategoryRowHint);
  ...
  btnEditRow.Hint := cxGetResourceString
    (@scxgwCardViewTreeViewFrameEditRowHint);
  miEditRow.Caption := btnEditRow.Caption;
end;

function TcxGridWizardCardStructureEditorFrame.
  GetDefaultCaptionRowCaption: string;
begin
...
function TcxGridWizardCardStructureEditorFrame.
  GetDefaultCategoryRowCaption: string;
begin
  Result := cxGetResourceString
    (@scxgwCardViewTreeViewFrameDefaultCategoryRowCaption);
end;

function TcxGridWizardCardStructureEditorFrame.GetNodeByVisibleIndex
  (const AVisibleIndex: Integer): TTreeNode;
var
  I, AIndex: Integer;
begin
  AIndex := -1;
...
function TcxGridWizardCardStructureEditorFrame.GetVisibleIndexByNode
...
procedure TcxGridWizardCardStructureEditorFrame.PreviewGridLayoutChangedHandler
  (Sender: TcxCustomGrid; AGridView: TcxCustomGridView);
begin
  RefreshContent;
end;

procedure TcxGridWizardCardStructureEditorFrame.
  PreviewViewFocusedItemChangedHandler(Sender: TcxCustomGridTableView;
  APrevFocusedItem, AFocusedItem: TcxCustomGridTableItem);
begin
  ...
  try
    for I := 0 to PreviewView.VisibleRowCount - 1 do
      if PreviewView.VisibleRows[I].Kind = rkCategory then
        GetNodeByVisibleIndex(I).Expanded := PreviewView.VisibleRows[I]
          .Expanded;
  finally
...
procedure TcxGridWizardCardStructureEditorFrame.btnAddCaptionRowClick
  (Sender: TObject);
var
  ACaption: string;
  ARow: TcxGridCardViewRow;
begin
  ...
  if cxgwExecuteEditItemDialog(Self,
    cxGetResourceString
    (@scxgwCardViewTreeViewFrameInputQueryCaptionAddCaptionRow), ACaption) then
  begin
...
procedure TcxGridWizardCardStructureEditorFrame.btnAddCategoryRowClick
  (Sender: TObject);
var
  ACaption: string;
  ARow: TcxGridCardViewRow;
begin
  ...
  if cxgwExecuteEditItemDialog(Self,
    cxGetResourceString
    (@scxgwCardViewTreeViewFrameInputQueryCaptionAddCategoryRow), ACaption) then
  begin
    ARow := PreviewView.CreateRow;
    ARow.Caption := ACaption;
...
procedure TcxGridWizardCardStructureEditorFrame.btnEditRowClick
  (Sender: TObject);
const
  KindMap: array [0 .. 2] of TcxGridCardViewRowKind = (rkData, rkCaption,
    rkCategory);
...
begin
  if tvRows.Selected.Index >= 0 then
    PreviewView.VisibleRows[GetVisibleIndexByNode(tvRows.Selected)
      ].Focused := True;
  UpdateButtonsState;
end;

procedure TcxGridWizardCardStructureEditorFrame.tvRowsEdited(Sender: TObject;
...
procedure TcxGridWizardCardStructureEditorFrame.tvRowsExpanded(Sender: TObject;
  Node: TTreeNode);
begin
  if not FIsUpdatingExpandedNodes then
    PreviewView.VisibleRows[GetVisibleIndexByNode(Node)].Expanded :=
      Node.Expanded;
end;
