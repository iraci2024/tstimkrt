...
uses
  ...
  Dialogs, ExtCtrls, cxViewEditor, cxGridCustomView, cxGridChartView, ComCtrls,
  StdCtrls,
  Menus, cxControls, cxPC, cxLookAndFeelPainters, cxButtons, cxGraphics,
  cxLookAndFeels, dxLayoutContainer,
...
type
  ...
  protected
    ...
    function GetItemClass(AItemKind: TcxGridChartItemKind)
      : TcxGridChartItemClass; virtual;
...
function CreateChartItem(AView: TcxGridChartView;
  AItemKind: TcxGridChartItemKind): TcxGridChartItem;
begin
  AView.BeginUpdate;
  try
    case AItemKind of
    ...
    Result.Name := GetViewItemUniqueName(AView, Result,
      GetChartItemName(AItemKind));
  finally
    AView.EndUpdate;
  end;
end;
...
function TcxChartViewEditor.GetItemClass(AItemKind: TcxGridChartItemKind)
  : TcxGridChartItemClass;
begin
  case AItemKind of
    ciSeries:
...
begin
  ...
  try
    ...
    begin
      ...
      if (AObject is TcxGridChartItem) and
        (TcxGridChartItem(AObject).GridView = View) then
...
begin
  ...
  try
    ...
    ListBoxRestoreSelection(lbItems.InnerListBox, ASelection, AItemIndex,
      ATopIndex);
  end;
end;
...
begin
  ...
  try
    ListBoxRestorePos(lbItems.InnerListBox, FListBoxItemIndex[ItemKind],
      FListBoxTopIndex[ItemKind]);
  finally
    lbItems.Items.EndUpdate;
    cxRedrawWindow(lbItems.Handle, RDW_FRAME or RDW_INVALIDATE or
      RDW_UPDATENOW); // to fix the bug with scrollbar updating
  end;
end;

procedure TcxChartViewEditor.SaveListBoxParams;
begin
  ListBoxSavePos(lbItems.InnerListBox, FListBoxItemIndex[ItemKind],
    FListBoxTopIndex[ItemKind]);
end;

class function TcxChartViewEditor.GetViewByObject(APersistent: TPersistent)
  : TcxCustomGridView;
begin
  if APersistent is TcxGridChartItem then
    Result := TcxGridChartItem(APersistent).GridView
  else if APersistent is TcxGridChartDiagram then
...
procedure TcxChartViewEditor.btnItemAddExClick(Sender: TObject);
  ...
  procedure InitPopupMenu(APopupMenu: TPopupMenu;
    const AProvider: IcxGridChartViewItemsProvider);
  var
    I: Integer;
  ...
  begin
    ...
    try
      ...
      for I := 0 to ACaptions.Count - 1 do
        APopupMenu.Items.Add(CreateMenuItem(pmItems, ACaptions[I],
          miItemAddExClick, True, I, AProvider.GetItem(ItemClass, I) <> nil));
    finally
      ACaptions.Free;
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      if AIndex <> -1 then
      begin
        (DataController as IcxGridChartViewItemsProvider)
          .InitItem(AItem, AIndex);
        UpdateDesigner;
      end;
    finally
      View.EndUpdate;
...
begin
  FItemPrevDragIndex := -1;
  ListBoxMoveItems(lbItems.InnerListBox, Min(lbItems.Items.Count,
    ListBoxGetLastSelectedIndex(lbItems.InnerListBox) + 2), FItemPrevDragIndex,
    ReindexItemsProc);
end;
...
procedure TcxChartViewEditor.lbItemsDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  ListBoxDragOver(lbItems.InnerListBox, Sender, Source, X, Y, State, Accept,
    FItemPrevDragIndex);
end;
...
begin
  ...
  ACanvas.TextOut(ARect.Left + ScaleFactor.Apply(3),
    ARect.Top + ScaleFactor.Apply(3), AControl.Items[AIndex]);
end;

procedure TcxChartViewEditor.lbItemsDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
  ListBoxDragDrop(lbItems.InnerListBox, Sender, Source, X, Y,
    FItemPrevDragIndex, ReindexItemsProc);
end;

procedure TcxChartViewEditor.lbItemsEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
  ListBoxEndDrag(lbItems.InnerListBox, Sender, Target, X, Y,
    FItemPrevDragIndex);
end;
...
begin
  inherited;
  with lbItems do
    ItemHeight := 2 * Self.ScaleFactor.Apply(3) +
      TdxTextMeasurer.TextLineHeight(Font);
end;

procedure TcxChartViewEditor.tcMainChanging(Sender: TObject;
...
function TcxGridChartViewStorage.GetOptionsBehavior
  : TcxGridChartOptionsBehavior;
begin
  Result := View.OptionsBehavior;
end;

function TcxGridChartViewStorage.GetOptionsCustomize
...
procedure TcxGridChartViewMenuProvider.ActivateDiagram
...
procedure TcxGridChartViewMenuProvider.CreateDataGroup
  (Sender: TcxGridViewMenuItem);
begin
  ObjectCreated(CreateChartItem(GridView, ciDataGroup));
end;

procedure TcxGridChartViewMenuProvider.CreateSeries
  (Sender: TcxGridViewMenuItem);
begin
...
begin
  ...
  Items.AddItem('Create DataGroup', CreateDataGroup,
