...
uses
  Windows, Messages, SysUtils, Classes, Controls, Forms, DB, cxGraphics,
  cxControls, cxStyles, cxLookAndFeels,
  cxLookAndFeelPainters, dxCustomWizardControl, dxWizardControl, cxClasses,
  dxCore, cxGrid, cxGridLevel,
  cxGridCustomView, cxGridWizardStrs, cxGridWizardCustomPage,
  cxGridWizardCustomHelper, cxGridWizardTableViewHelper,
  cxGridWizardLayoutViewHelper, cxGridWizardBandedTableViewHelper,
  cxGridWizardChartViewHelper,
  cxGridWizardCardViewHelper, dxWizardControlForm, Dialogs,
  cxGridWizardServerModeViewHelper;
...
type
  ...
  private
    ...
    procedure CreatePageContent(APage: TdxWizardControlCustomPage;
      AFrameClass: TcxGridWizardCustomPageFrameClass);
    procedure DestroyHelpersPages;
    procedure ExecuteGridWizardForNextLevelCreation;
    function GetPageFrame(APage: TdxWizardControlCustomPage)
...
procedure ExecuteGridWizard(AGrid: TcxCustomGrid;
...
uses
  ...
  cxGridWizardCommonFinishPage, cxDWMApi, cxGeometry, Math;
...
begin
  ...
  if FIsNextLevelNeeded then
    FIsNextLevelNeeded :=
      MessageDlg(cxGetResourceString(@scxgwCommonCreateDetailLevelQuery),
      mtConfirmation, [mbYes, mbNo], 0) = mrYes;
end;

procedure TcxGridWizardForm.CreatePageContent(APage: TdxWizardControlCustomPage;
  AFrameClass: TcxGridWizardCustomPageFrameClass);
var
...
begin
  ...
  try
    ...
    AForm.FMultiLevelStructureMasterViewName :=
      FMultiLevelStructureMasterViewName;
...
function TcxGridWizardForm.GetPageFrame(APage: TdxWizardControlCustomPage)
  : TcxGridWizardCustomPageFrame;
begin
  if APage <> nil then
...
begin
  for I := 0 to FHelper.PageClassCount - 1 do
    CreatePageContent(wcMain.AddPage,
      TcxGridWizardCustomPageFrameClass(FHelper.PageClasses[I]));
...
begin
  ...
  FHelper := cxGridWizardHelperInfoList.GetHelperClassBy(AGridViewClass)
    .Create(FGrid);
  grPreviewGrid.ActiveLevel.GridView := grPreviewGrid.CreateView
    (FHelper.GetGridViewClass);
  UpdateCaption;
end;
...
begin
  if FSelectedGridView <> nil then
    Caption := Format(cxGetResourceString(@scxgwCommonWizardCaptionEditing),
      [FSelectedGridView.Name])
  else
...
procedure TcxGridWizardForm.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
  if ModalResult = mrCancel then
    CanClose := MessageDlg(cxGetResourceString(@scxgwCommonCloseQuery),
...
procedure TcxGridWizardForm.wcMainButtonClick(Sender: TObject;
  AKind: TdxWizardControlButtonKind; var AHandled: Boolean);
var
  I: Integer;
begin
...
procedure TcxGridWizardForm.wcMainPageChanging(Sender: TObject;
  ANewPage: TdxWizardControlCustomPage; var AAllow: Boolean);
begin
  if GetPageFrame(wcMain.ActivePage) <> nil then
