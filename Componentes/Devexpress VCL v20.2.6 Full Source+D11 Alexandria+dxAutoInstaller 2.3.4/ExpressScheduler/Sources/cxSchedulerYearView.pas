...
{$ENDIF}
  ...
  dxCore, dxCoreClasses, cxClasses, cxControls, cxGraphics, cxStyles,
...
type
  ...
  TcxSchedulerYearViewController = class
  ...
  TcxSchedulerYearViewEventCellModernViewInfo = class
  ...
  private
    ...
    function GetDaysCell(AIndex: Integer)
  ...
  protected
    ...
    function AddMonthHeader(AYear, AMonth: Integer; const ARect: TRect;
      AFirstMonth: Integer = 0; AMonthCount: Integer = 12)
      : TcxSchedulerMonthHeaderCellViewInfo;
    function AddWeekDayHeader(AWeekDay: Integer; ARect: TRect)
      : TcxSchedulerWeekDayHeaderCellViewInfo;
    function AddYearViewContentCell(ARect: TRect; ADate: TDate;
      ASelected: Boolean; AParams: TcxViewParams; AColIndex: Integer)
      : TcxSchedulerYearViewContentCellViewInfo;
    ...
    property AdapterFirstMonth: Integer read GetAdapterFirstMonth
      write FAdapterFirstMonth;
    property AdapterMonthCount: Integer read GetAdapterMonthCount
      write FAdapterMonthCount;
  ...
  public
    ...
    property DaysCells[AIndex: Integer]: TcxSchedulerYearViewContentCellViewInfo
      read GetDaysCell;
    property MonthBounds[AIndex: Integer]: TRect read GetMonthBounds;
    property MonthHeaderCells: TcxSchedulerViewInfoCellList
      read FMonthHeaderCells;
  ...
  protected
    function ContentCell(AIndex: Integer)
      : TcxSchedulerYearViewContentCellViewInfo;
  ...
  protected
    ...
    procedure InitHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest); override;
    procedure Initialize(AYear, AMonth, AFirstMonth, AMonthCount: Integer;
      ARotateHeader: Boolean);
  public
  ...
  TcxSchedulerMonthHeaderCellModernViewInfo = class
    (TcxSchedulerMonthHeaderCellViewInfo)
  protected
  ...
  TcxSchedulerYearViewContentCellModernViewInfo = class
    (TcxSchedulerYearViewContentCellViewInfo)
  protected
  ...
  TcxSchedulerYearViewGetUnusedContentStyleEvent = procedure
    (Sender: TcxSchedulerYearView; AYear, AMonth, ADayOfWeek: Integer;
    var AStyle: TcxStyle) of object;
  TcxSchedulerYearViewGetMonthHeaderStyleEvent = procedure
    (Sender: TcxSchedulerYearView; AYear, AMonth: Integer; var AStyle: TcxStyle)
    of object;
  ...
  public
    ...
    function GetUnusedContentParams(AYear, AMonth, ADayOfWeek: Integer)
      : TcxViewParams;

    property Scheduler: TcxCustomScheduler read FScheduler;
    property YearView: TcxSchedulerYearView read FYearView;
  published
    property MonthHeader: TcxStyle index cxcsMonthHeader read GetValue
      write SetValue;
    property UnusedContent: TcxStyle index cxcsUnusedContent read GetValue
      write SetValue;
    property OnGetMonthHeaderStyle: TcxSchedulerYearViewGetMonthHeaderStyleEvent
      read FOnGetMonthHeaderStyle write FOnGetMonthHeaderStyle;
    property OnGetUnusedContentStyle
      : TcxSchedulerYearViewGetUnusedContentStyleEvent
      read FOnGetUnusedContentStyle write FOnGetUnusedContentStyle;
  end;
  ...
  protected
    ...
    function CreateMonthHeaderPopupMenu
      : TcxSchedulerMonthHeaderPopupMenu; virtual;
  ...
  published
    property Active;
    property AllDayEventsOnly: Boolean read FAllDayEventsOnly
      write SetAllDayEventsOnly default False;
    property MaxSelectedDaysCount: Integer read FMaxSelectedDaysCount
      write SetMaxSelectedDaysCount default 0;
    property MonthHeaderPopupMenu: TcxSchedulerMonthHeaderPopupMenu
      read FMonthHeaderPopupMenu write SetMonthHeaderPopupMenu;
    // for internal use
  ...
  protected
    ...
    property PredefinedCurrentAnchor: TDateTime read FPredefinedCurrentAnchor
      write FPredefinedCurrentAnchor;
  end;
  ...
  TcxSchedulerMonthHeaderPopupMenuItems = set of
    TcxSchedulerMonthHeaderPopupMenuItem;
  ...
  TcxSchedulerMonthHeaderPopupMenuClickEvent = procedure
    ...
    AItem: TcxSchedulerMonthHeaderPopupMenuItem; var AHandled: Boolean)
    of object;
  ...
  public
    ...
    function GetMenuItem(AItem: TcxSchedulerMonthHeaderPopupMenuItem)
      : TMenuItem;
  published
    property Items: TcxSchedulerMonthHeaderPopupMenuItems read FItems
      write FItems default [mhpmiNewEvent, mhpmiNewAllDayEvent,
      mhpmiNewReccuringEvent, mhpmiFullYear, mhpmiHalfYear, mhpmiQuarter];
    property PopupMenu;
    property UseBuiltInPopupMenu;

    property OnClick: TcxSchedulerMonthHeaderPopupMenuClickEvent read FOnClick
      write FOnClick;
    property OnPopup: TcxSchedulerMonthHeaderPopupMenuPopupEvent read FOnPopup
      write FOnPopup;
  end;
...
function TcxSchedulerYearViewController.CreateDragEventHelper
  : TcxDragEventHelper;
begin
  Result := TcxYearViewDragEventHelper.Create(Scheduler);
end;

function TcxSchedulerYearViewController.CreateNavigation
  : TcxSchedulerViewNavigation;
begin
  Result := TcxSchedulerYearViewNavigation.Create(View);
end;

function TcxSchedulerYearViewController.CreateResizeEventHelper
  : TcxEventSizingHelper;
begin
  Result := TcxYearViewEventSizing.Create(Scheduler);
end;

procedure TcxSchedulerYearViewController.KeyDown(var Key: Word;
  Shift: TShiftState);
begin
...
function TcxSchedulerYearViewEventCellViewInfo.
...
function TcxSchedulerYearViewEventCellModernViewInfo.
  CanAutoHideStandardImages: Boolean;
begin
  Result := True;
end;
...
constructor TcxSchedulerYearViewViewInfo.Create(AOwner: TcxSchedulerSubControl);
begin
...
begin
  ...
  begin
    ...
    if (ACell.TimeStart <> NullDate) and
      ACell.UpdateSelection(not HideSelection and (ACell.TimeStart >= SelStart)
      and (ACell.TimeStart <= SelFinish)) then
...
begin
  for I := APlace.ColStart to APlace.ColFinish do
    ...
    begin
      AddButton(I, DaysCells[I].Bounds, DaysCells[I].TimeStart, True,
        APlace.Event);
...
begin
  ...
  begin
    ...
    while (AFinishIndex < (DaysCellCount - 1)) and
      AEvent.IsDayEvent(Trunc(DaysCells[AFinishIndex + 1].TimeStart)) do
      Inc(AFinishIndex);
  end
  else
  begin
    I := 0;
    while (I < DaysCellCount) and not AEvent.IsDayEvent
      (Trunc(DaysCells[I].TimeStart)) do
      Inc(I);
    if I = DaysCellCount then
      Exit;
    AFinishIndex := I;
    while (AFinishIndex < DaysCellCount) and
      AEvent.IsDayEvent(Trunc(DaysCells[AFinishIndex].TimeStart)) do
      Inc(AFinishIndex);
    Dec(AFinishIndex);
  end;
  ...
  begin
    ...
    while (I <= AFinishIndex) and
      (AStartDay.Bounds.Top = DaysCells[I].Bounds.Top) do
      Inc(I);
    ABuilder.AddEventPlace(AEvent, AStartIndex, I - 1, 1,
      TObject(MonthOf(AStartDay.DateTime)));
  end;
end;
...
begin
  ...
  ABounds.Top := ABounds.Top + APlace.LineStart *
    (ContentLineHeight + cxTextOffset);
...
function TcxSchedulerYearViewViewInfo.AddMonthHeader(AYear, AMonth: Integer;
  const ARect: TRect; AFirstMonth, AMonthCount: Integer)
  : TcxSchedulerMonthHeaderCellViewInfo;
var
  AParams: TcxViewParams;
begin
  AParams := View.Styles.GetMonthHeaderParams(AYear, AMonth);
  CreateCellInstance(MonthHeaderClass, ARect, AParams,
...
begin
  ...
  if AWeekDay >= 7 then
    Dec(AWeekDay, 7);
  CreateCellInstance(DayHeaderClass, ARect,
...
function TcxSchedulerYearViewViewInfo.AddYearViewContentCell(ARect: TRect;
  ADate: TDate; ASelected: Boolean; AParams: TcxViewParams; AColIndex: Integer)
  : TcxSchedulerYearViewContentCellViewInfo;
var
  AColor: TColor;
begin
  CreateCellInstance(ContentCellClass, ARect, AParams,
    UseRightToLeftAlignment, Result);
  ...
  begin
    ...
    Result.SetContentState(True, ASelected, FDayTextHeight - cxTextOffset,
      FSelectionParams);
...
begin
  ...
  TcxSchedulerMonthHeaderCellViewInfo(FMonthHeaderCells[AMonth + 1]).FColStart
    := AStartMonthColumn;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      R := Rect(DayBounds.Left, MonthBounds.Top, DayBounds.Right,
        MonthBounds.Bottom);
      ADate := GetMonthColumnDate(AStartOfMonth, ADaysPerMonth,
        AStartMonthColumn, J + FStartDayIndex);
      ASelected := not HideSelection and (ADate >= SelStart) and
        (ADate <= SelFinish);
      if ADate = NullDate then
      ...
      FMaxEventsCount := Max(FMaxEventsCount,
        (R.Bottom - R.Top) div FContentLineHeight + 1);
...
begin
  ...
  begin
    R.Left := Bounds.Left + FMonthHeaderWidth +
      MulDiv(W, I, FMaxYearViewWeekDays);
    R.Right := Bounds.Left + FMonthHeaderWidth +
      MulDiv(W, I + 1, FMaxYearViewWeekDays);
...
begin
  ...
  ABuilder := TcxSchedulerEventLayoutBuilder.Create
    (TcxSchedulerViewStyle.svsClassic);
  try
...
begin
  ...
  for I := 1 to 12 do
    FMonthHeaderWidth := Max(FMonthHeaderWidth,
      cxTextWidth(AFont, dxFormatSettings.LongMonthNames[I]) + 4 *
      cxTextOffset);
end;
...
begin
  ...
  R := cxRectSetHeight(cxRectSetWidth(Bounds, FMonthHeaderWidth),
    FDayHeaderHeight);
  AddMonthHeader(FYear, 0, R, AdapterFirstMonth, AdapterMonthCount)
    .RotateHeader := False;
  for I := 0 to MonthCountPerPage - 1 do
  begin
...
begin
  if (ADate < Trunc(TcxSchedulerCustomViewInfoItemAccess(FDayCells[0])
    .FDateTime)) or
    (ADate > Trunc(TcxSchedulerCustomViewInfoItemAccess(FDayCells
    [FDayCells.Count - 1]).FDateTime)) then
  begin
...
function TcxSchedulerYearViewViewInfo.ContentCellClass
...
procedure TcxSchedulerYearViewViewInfo.CustomProcessPreviouslyCalculatedEvent
...
function TcxSchedulerYearViewViewInfo.DayHeaderClass
...
function TcxSchedulerYearViewViewInfo.EventCellClass
...
function TcxSchedulerYearViewViewInfo.MonthHeaderClass
  : TcxSchedulerDayHeaderCellViewInfoClass;
const
  AClass: array [Boolean] of TcxSchedulerDayHeaderCellViewInfoClass =
    (TcxSchedulerMonthHeaderCellViewInfo,
...
begin
  ...
  Supports(TObject(Adapter), IcxSchedulerYearViewAdapter,
    FYearViewExtraAdapter);
  try
    ...
    FRangeFinish :=
      Trunc(EndOfTheMonth(EncodeDate(Year, FirstMonth + GetMonthCountPerPage
      - 1, 1)));
...
function TcxSchedulerYearViewViewInfo.GetMonthColumnDate(AStartOfMonth
  : TDateTime; ADaysPerMonth, AStartMonthColumn, AColumn: Integer): TDateTime;
begin
  Result := NullDate;
...
function TcxSchedulerYearViewViewInfo.GetSmallFont(const AParams
  : TcxViewParams): TFont;
begin
...
function TcxSchedulerYearViewViewInfo.GetDaysCell(AIndex: Integer)
  : TcxSchedulerYearViewContentCellViewInfo;
begin
...
begin
  ...
  if ViewInfo.FYearViewExtraAdapter <> nil then
    TcxSchedulerYearViewAdapter(ViewInfo.FYearViewExtraAdapter)
      .PredefinedCurrentAnchor := FCurrentAnchor;
  ViewInfo.CheckLayout(FCurrentAnchor);
  SetSelAnchor(FCurrentAnchor, AShift);
end;

procedure TcxSchedulerYearViewNavigation.ValidateSelection(var ASelStart,
...
begin
  ...
  ASelFinish := Min(Max(ASelFinish, View.FirstVisibleDate),
    View.LastVisibleDate);
  if (View.MaxSelectedDaysCount > 0) then
    ASelFinish := Max(Min(ASelFinish, ASelStart + View.MaxSelectedDaysCount -
      1), ASelStart - View.MaxSelectedDaysCount + 1);
  FSaveSelFinish := ASelFinish;
  FSaveSelStart := ASelStart;
  FCanChangeSelection := False;
end;

function TcxSchedulerYearViewNavigation.ContentCell(AIndex: Integer)
  : TcxSchedulerYearViewContentCellViewInfo;
begin
  Result := TcxSchedulerYearViewContentCellViewInfo
    (ViewInfo.ContentCells[AIndex]);
end;
...
procedure TcxSchedulerYearViewNavigation.DoKeyDown(var AKey: Word;
  AShift: TShiftState);
var
...
begin
  ...
  begin
    AIndex := ViewInfo.FContentCells.IndexOf
      (ViewInfo.DaysCells[DayOfTheRange(FCurrentAnchor) - 1]);
...
function TcxSchedulerYearViewNavigation.GetYearViewInfo
  : TcxSchedulerYearViewViewInfo;
begin
  Result := TcxSchedulerYearViewViewInfo(inherited ViewInfo);
end;
...
begin
  if FMonth = 0 then
    TcxSchedulerYearViewHitTest(AHitTest).SetHitTime(htcYear,
      EncodeDate(Year, 1, 1))
  else
    TcxSchedulerYearViewHitTest(AHitTest).SetHitTime(htcMonth,
      EncodeDate(Year, FMonth, 1));
end;

procedure TcxSchedulerMonthHeaderCellViewInfo.Initialize(AYear, AMonth,
  AFirstMonth, AMonthCount: Integer; ARotateHeader: Boolean);
begin
  FMonth := AMonth;
  FYear := AYear;
...
function TcxSchedulerMonthHeaderCellModernViewInfo.DrawBackground
...
const
  ABorders: array [Boolean] of TcxBorders = ([bRight, bBottom],
    [bLeft, bBottom]);
var
  AStyle: TFontStyles;
begin
  ...
  if not IsDrawBySkin then
    Canvas.FrameRect(Bounds,
      ExternalPainter.Painter.DefaultSchedulerDayHeaderBorderColor, 1,
...
function TcxSchedulerYearViewContentCellModernViewInfo.
...
function TcxSchedulerYearViewContentCellModernViewInfo.SaveFontStyle
...
function TcxSchedulerYearViewStyles.GetMonthHeaderParams(AYear, AMonth: Integer)
  : TcxViewParams;
var
  AStyle: TcxStyle;
begin
...
function TcxSchedulerYearViewStyles.GetUnusedContentParams(AYear, AMonth,
  ADayOfWeek: Integer): TcxViewParams;
var
  AStyle: TcxStyle;
begin
  AStyle := nil;
  ...
  case Index of
    cxcsUnusedContent:
      begin
        ...
        AParams.Color := Painter.DefaultSchedulerYearViewUnusedContentColor
          (AIsWorkTime);
        if AParams.Color = clDefault then
          AParams.Color := FYearView.PainterHelper.GetContentColor(clBtnFace,
            AIsWorkTime, Scheduler.OptionsView.Style);
      end;
    cxcsMonthHeader:
      begin
...
function TcxSchedulerYearView.CreateController
...
function TcxSchedulerYearView.CreateMonthHeaderPopupMenu
  : TcxSchedulerMonthHeaderPopupMenu;
begin
...
begin
  ...
  if AMonthCount <> 0 then
    Result := Max(cxYearViewScrollMinPos,
      ...
      ViewInfo.AdapterFirstMonth div AMonthCount - ViewInfo.GetPartOfTheYear
      (Now) + MulDiv(cxYearViewScrollCurrentYearPos, 12, AMonthCount)))
  else
...
begin
  ...
  SetScrollBarInfo(sbVertical, cxYearViewScrollMinPos,
    MulDiv(cxYearViewScrollMaxPos, 12, ViewInfo.AdapterMonthCount), 1, 1, APos,
    True, True);
end;

function TcxSchedulerYearView.InternalMouseWheel(Shift: TShiftState;
...
procedure TcxSchedulerYearView.MakeEventVisible
...
begin
  ...
  if AMonthCount = 0 then
    ...
    AYear := MulDiv(CurrentYear - cxYearViewScrollCurrentYearPos, 12,
      AMonthCount) + ViewInfo.GetPartOfTheYear(Now) + AScrollPos;
...
procedure TcxSchedulerYearView.SetGroupingKind
...
procedure TcxSchedulerYearView.GetRangeControlScales
...
procedure TcxSchedulerYearView.GetRangeControlTotalRange(out AMin,
  AMax: TDateTime);
var
  AStart: TDateTime;
begin
...
procedure TcxSchedulerYearView.SetMaxSelectedDaysCount(AValue: Integer);
begin
  AValue := Max(0, AValue);
...
function TcxSchedulerYearViewAdapter.QueryInterface(const IID: TGUID; out Obj)
  : HResult; stdcall;
begin
...
begin
  if PredefinedCurrentAnchor <> 0 then
    ...
    ACurrentAnchor := TcxSchedulerCustomViewViewInfoAccess
      (TcxSchedulerYearView(View).ViewInfo).FSelectedDays[0];
...
constructor TcxSchedulerMonthHeaderPopupMenu.Create
...
procedure TcxSchedulerMonthHeaderPopupMenu.Execute
  (AItem: TcxSchedulerMonthHeaderPopupMenuItem);
begin
  ExecuteCommand(Ord(AItem));
end;

function TcxSchedulerMonthHeaderPopupMenu.GetMenuItem
  (AItem: TcxSchedulerMonthHeaderPopupMenuItem): TMenuItem;
begin
...
begin
  CreateNewEventItems(mhpmiNewEvent in FItems, mhpmiNewAllDayEvent in FItems,
    mhpmiNewReccuringEvent in FItems, Ord(mhpmiNewEvent),
    Ord(mhpmiNewAllDayEvent), Ord(mhpmiNewReccuringEvent));
...
var
  AIndex: Integer;
begin
  if ACommand in [Ord(mhpmiNewEvent), Ord(mhpmiNewAllDayEvent),
    Ord(mhpmiNewReccuringEvent)] then
...
function TcxSchedulerMonthHeaderPopupMenu.IsValidCommand
  (ACommand: Integer): Boolean;
begin
  Result := (ACommand >= Ord(mhpmiNewEvent)) and
    (ACommand <= Ord(mhpmiQuarter));
end;
