...
uses
  ...
  cxSchedulerStrs, cxSchedulerDialogs, cxScheduler, cxStyles,
...
uses
  RTLConsts, Windows, ShellApi, dxCoreClasses, cxSchedulerCustomControls,
  cxSchedulerDayView,
  cxSchedulerGanttView, cxClasses, dxCoreReg, cxLibraryReg, cxGraphics,
...
const
  ...
  ViewNames: array [0 .. 6] of string = ('Day', 'Week', 'Weeks', 'Year',
    'TimeGrid', 'Gantt', 'Agenda');
  ViewClasses: array [0 .. 6] of TClass = (TcxSchedulerDayView,
    TcxSchedulerWeekView, TcxSchedulerWeeksView, TcxSchedulerYearView,
    TcxSchedulerTimeGridView, TcxSchedulerGanttView, TcxSchedulerAgendaView);
...
function cxShowSchedulerHolidaysComponentEditor
  (AHolidays: TcxSchedulerHolidays): Boolean;
begin
...
begin
  ...
  if Result = '' then
    Result := Format('Default (%s)',
      [cxSchedulerEditorManager.CurrentEditorFormStyle]);
end;
...
begin
  inherited;
  if (Index <> 0) or not CanSelectActiveView then
    Exit;
...
function TcxSchedulerComponentsEditor.GetViewByIndex(AIndex: Integer)
...
procedure TcxSchedulerHolidayComponentsEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  if (AIndex = 0) and cxShowSchedulerHolidaysComponentEditor
    ((GetComponent as TcxSchedulerHolidays)) then
    Designer.Modified
  else
    inherited InternalExecuteVerb(AIndex);
end;

function TcxSchedulerHolidayComponentsEditor.InternalGetVerb
...
function TcxSchedulerDBStorageResourceFieldNameProperty.GetDataSource
  : TDataSource;
begin
...
begin
  ...
  Result := Result - [paReadOnly, paSubProperties] +
...
begin
  ...
  begin
    ...
    while AIndex >= 0 do
    begin
      if AnsiCompareText(DateTimeHelper.TimeZoneInfo(AIndex).Display, Value) = 0
      then
        Break
      else
        Dec(AIndex);
...
begin
  if Value = scxFullYear then
    SetOrdValue(12)
  else if Value = scxHalfYear then
    SetOrdValue(6)
  else
    SetOrdValue(3);
end;
...
function TcxSchedulerStorageResourceImageIndexProperty.GetImages
  : TCustomImageList;
begin
...
begin
  if cxShowSchedulerHolidaysComponentEditor
    ((GetComponent(0) as TcxSchedulerHolidays)) then
    Designer.Modified;
end;
...
procedure Register;
begin
  RegisterComponents(dxCoreLibraryProductPage, [TcxSchedulerAggregateStorage,
    TcxSchedulerStorage, TcxSchedulerDBStorage, TcxScheduler, TcxDateNavigator,
  ...
  RegisterPropertyEditor(TypeInfo(TcxFieldName),
    TcxSchedulerDBStorageFieldNames, '',
    TcxSchedulerDBStorageFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(TcxFieldName), TcxSchedulerDBStorageResources,
    '', TcxSchedulerDBStorageResourceFieldNameProperty);

  RegisterPropertyEditor(TypeInfo(TcxSchedulerHolidaysLocations),
    TcxSchedulerHolidays, 'Locations', TcxSchedulerHolidayPropertyEditor);
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxScheduler, 'ViewDayEvents',
    TcxSchedulerViewDayEventsProperty);

  RegisterPropertyEditor(TypeInfo(TcxImageIndex),
    TcxSchedulerStorageResourceItem, 'ImageIndex',
    TcxSchedulerStorageResourceImageIndexProperty);
  ...
  RegisterPropertyEditor(TypeInfo(Integer), TcxSchedulerReminders, 'TimeZone',
    TcxSchedulerTimeZoneProperty);
  ...
  RegisterPropertyEditor(TypeInfo(Integer), TcxSchedulerDayView, 'TimeScale',
    TcxSchedulerTimeScaleProperty);
  ...
  // year view
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxScheduler, 'ViewYearEvents',
    TcxSchedulerViewYearEventsProperty);
  RegisterPropertyEditor(TypeInfo(Integer), TcxSchedulerYearView, 'Scale',
    TcxSchedulerViewYearScaleProperty);
