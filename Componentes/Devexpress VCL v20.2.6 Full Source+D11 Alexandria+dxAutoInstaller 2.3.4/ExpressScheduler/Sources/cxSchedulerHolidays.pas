...
type
  ...
  TcxGetHolidayRecurrenceDescriptionStringProc = function(ARecurrenceInfo
    : TcxSchedulerHolidayRecurrenceInfo): string;

  TcxSchedulerHolidaysLocationHoliday = class(TcxInterfacedCollectionItem,
    IcxStoredObject)
  private
    FDate: TDateTime;
    FName: string;
  ...
  protected
    ...
    property RecurrenceData: AnsiString read FRecurrenceData
      write SetRecurrenceData;
  public
    ...
    property RecurrenceInfo: TcxSchedulerHolidayRecurrenceInfo
      read FRecurrenceInfo;
  published
  ...
  TcxSchedulerHolidaysLocationHolidayClass = class of
  ...
  TcxSchedulerHolidaysLocation = class(TcxInterfacedCollectionItem,
    IcxStoredParent, IcxStoredObject)
  private
    ...
    procedure SetHoliday(AIndex: Integer;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    function AddHoliday(const AName: string; const ADate: TDateTime)
      : TcxSchedulerHolidaysLocationHoliday; overload; virtual;
    procedure Clear;
    procedure Delete(AIndex: Integer);
    procedure PopulateHolidayDates(AList: TcxSchedulerDateList;
      const AStart, AFinish: TDate; AOnlyVisible: Boolean = True);
    function Find(const AName: string; const ADate: TDateTime)
      : TcxSchedulerHolidaysLocationHoliday; overload; virtual;
    procedure Sort(ASortOrder: TcxDataSortOrder; ASortByDate: Boolean);

    property Count: Integer read GetCount;
    property Holidays[AIndex: Integer]: TcxSchedulerHolidaysLocationHoliday
      read GetHoliday write SetHoliday; default;
  ...
  public
    ...
    function GetLocationByName(const AName: string)
      : TcxSchedulerHolidaysLocation;
    procedure Sort(ASortOrder: TcxDataSortOrder);

    property Items[AIndex: Integer]: TcxSchedulerHolidaysLocation read GetItem
  ...
  TcxSchedulerHolidaysImportExportHolidayEvent = procedure
    (ASender: TcxSchedulerHolidays;
    AHoliday: TcxSchedulerHolidaysLocationHoliday; var Accept: Boolean)
    of object;
  TcxSchedulerHolidaysImportUnknownDateEvent = procedure
    (ASender: TcxSchedulerHolidays; var AYear, AMonth, ADay: Word;
    const ATypeCalendar: Word; var Accept: Boolean) of object;

  TcxSchedulerHolidays = class(TcxCustomComponent, IcxStoredObject,
    IcxStoredParent)
  private
    ...
    procedure GetHolidaysIndex(AIndex: Integer;
      out ALocationIndex, AHolidayIndex: Integer);
    function GetStringPart(const S: string; var APos: Integer;
      ACheckDateSeparator: Boolean = True): string;
    procedure SetLocations(const AValue: TcxSchedulerHolidaysLocations);
  protected
    procedure Changed; virtual;
    function DoExportHoliday(AHoliday: TcxSchedulerHolidaysLocationHoliday)
      : Boolean; virtual;
    function DoImportHoliday(AHoliday: TcxSchedulerHolidaysLocationHoliday)
      : Boolean; virtual;
    ...
    function TryStringToHoliday(const S: string;
      ALocation: TcxSchedulerHolidaysLocation): Boolean;
    function TryStringToLocation(const S: string;
      ALocations: TcxSchedulerHolidaysLocations): Boolean;
  ...
  public
    ...
    function AddHoliday(const ALocationName, AHolidayName: string;
      const ADate: TDateTime): TcxSchedulerHolidaysLocationHoliday;
    procedure AddListener(AListener: IcxSchedulerHolidaysListener);
    function AddLocation(const AName: string): TcxSchedulerHolidaysLocation;
      overload; virtual;
    procedure Assign(Source: TPersistent); override;
    procedure BeginUpdate;
    procedure Clear; virtual;
    procedure EndUpdate;
    function GetHolidayNamesByDate(const ADate: TDate; var ANames: string;
    ...
    property Count: Integer read GetCount;
    property Items[AIndex: Integer]: TcxSchedulerHolidaysLocationHoliday
      read GetItem; default;
  published
    property Locations: TcxSchedulerHolidaysLocations read FLocations
      write SetLocations;
...
function cxGetHolidayRecurrenceDescriptionString(ARecurrenceInfo
  : TcxSchedulerHolidayRecurrenceInfo): string;
function cxHolidayRecurrenceInfoDataToString
  (AHoliday: TcxSchedulerHolidaysLocationHoliday): AnsiString;
function cxHolidayStringToRecurrenceInfoData(const S: AnsiString): AnsiString;

const
  cxGetHolidayRecurrenceDescriptionStringProc
    : TcxGetHolidayRecurrenceDescriptionStringProc =
...
const
  ...
  cxHolidaysFormat: TFormatSettings = (DateSeparator: cxDateSeparator;
...
begin
  Result := '';
  if not Assigned(ARecurrenceInfo) or not ARecurrenceInfo.Holiday.IsRecurring
  then
    Exit;
...
begin
  ...
  begin
    ...
    HexToBin(PAnsiChar(AHoliday.RecurrenceData), PAnsiChar(Result),
      Length(Result));
  end;
end;
...
function cxCompareHolidays(AItem1,
  AItem2: TcxSchedulerHolidaysLocationHoliday): Integer;
begin
  Result := AnsiCompareText(AItem1.Name, AItem2.Name)
end;

function cxCompareHolidaysByDate(AItem1,
  AItem2: TcxSchedulerHolidaysLocationHoliday): Integer;
begin
  Result := 0;
...
function cxCompareLocations(AItem1,
  AItem2: TcxSchedulerHolidaysLocation): Integer;
begin
  Result := AnsiCompareText(AItem1.Name, AItem2.Name);
end;

procedure SortCollection(ACollection: TCollection; ACompare: TListSortCompare;
...
procedure TcxSchedulerHolidaysLocationHoliday.PopulateHolidayDates
  (AList: TcxSchedulerDateList; const AStart, AFinish: TDate;
  AOnlyVisible: Boolean = True);
var
...
begin
  ...
  if IsRecurring then
  begin
    AHolidayCalculator := TcxSchedulerHolidayCalculator.Create(Self,
      AStart, AFinish);
    try
  ...
  else if IsHolidayInRange(AStart, AFinish) then
    AList.Add(Date);
end;
...
function TcxSchedulerHolidaysLocationHoliday.CreateRecurrenceInfo
...
begin
  ...
  Filer.DefineBinaryProperty('RecurrenceData', ReadRecurrenceData,
    WriteRecurrenceData, IsRecurring);
end;
...
function TcxSchedulerHolidaysLocationHoliday.IsDateHoliday(const ADate: TDate;
  AOnlyVisible: Boolean): Boolean;
begin
  Result := (not AOnlyVisible or IsVisible) and
    ((ADate = Date) or IsRecurrenceDateHoliday(ADate)) and (Name <> '');
end;
...
function TcxSchedulerHolidaysLocationHoliday.IsHolidayInRange(const AStart,
  AFinish: TDate): Boolean;
begin
  Result := (Date >= AStart) and (Date <= AFinish);
end;

function TcxSchedulerHolidaysLocationHoliday.IsRecurrenceDateHoliday
  (ADate: TDate): Boolean;
var
  AHolidayCalculator: TcxSchedulerHolidayCalculator;
begin
...
function TcxSchedulerHolidaysLocationHoliday.GetStoredProperties
...
procedure TcxSchedulerHolidaysLocationHoliday.GetStoredPropertyValue
...
begin
  if AName = 'Date' then
    ...
    AValue := Visible
  else if AName = 'RecurrenceData' then
    AValue := RecurrenceData;
end;

procedure TcxSchedulerHolidaysLocationHoliday.SetStoredPropertyValue
...
procedure TcxSchedulerHolidaysLocationHoliday.ReadRecurrenceData
  (AReader: TStream);
var
...
procedure TcxSchedulerHolidaysLocationHoliday.WriteRecurrenceData
  (AWriter: TStream);
var
  AData: TcxSchedulerCustomRecurrenceInfoData;
begin
...
function TcxSchedulerHolidaysLocationHoliday.GetLocation
  : TcxSchedulerHolidaysLocation;
begin
...
procedure TcxSchedulerHolidaysLocationHoliday.SetRecurrenceData
...
begin
  inherited Create(Collection);
  FHolidays := TcxSchedulerHolidaysCollection.Create
    (TcxSchedulerHolidaysLocationHoliday);
...
procedure TcxSchedulerHolidaysLocation.PopulateHolidayDates
  (AList: TcxSchedulerDateList; const AStart, AFinish: TDate;
  AOnlyVisible: Boolean = True);
var
  I: Integer;
...
function TcxSchedulerHolidaysLocation.Find(const AName: string;
  const ADate: TDateTime): TcxSchedulerHolidaysLocationHoliday;
var
  I: Integer;
begin
...
procedure TcxSchedulerHolidaysLocation.Sort(ASortOrder: TcxDataSortOrder;
  ASortByDate: Boolean);
begin
  ...
  begin
    if not ASortByDate then
      ...
      SortCollection(HolidaysList, @cxCompareHolidaysByDate,
...
function TcxSchedulerHolidaysLocation.CreateChild(const AObjectName,
  AClassName: string): TObject;
begin
  Result := HolidaysList.Add;
end;

procedure TcxSchedulerHolidaysLocation.DeleteChild(const AObjectName: string;
  AObject: TObject);
begin
  AObject.Free;
end;

procedure TcxSchedulerHolidaysLocation.GetStoredChildren
...
function TcxSchedulerHolidaysLocation.GetStoredProperties
  (AProperties: TStrings): Boolean;
begin
  Result := True;
  AProperties.Add('Name');
...
procedure TcxSchedulerHolidaysLocation.GetStoredPropertyValue
  (const AName: string; var AValue: Variant);
begin
  if AName = 'Name' then
    AValue := Name
...
procedure TcxSchedulerHolidaysLocation.SetStoredPropertyValue
  (const AName: string; const AValue: Variant);
begin
  if AName = 'Name' then
    Name := AValue
...
function TcxSchedulerHolidaysLocation.GetHoliday(AIndex: Integer)
  : TcxSchedulerHolidaysLocationHoliday;
begin
  Result := TcxSchedulerHolidaysLocationHoliday(FHolidays.Items[AIndex]);
end;

function TcxSchedulerHolidaysLocation.GetLocations
  : TcxSchedulerHolidaysLocations;
begin
  ...
  begin
    ...
    try
      ...
      for I := 0 to TcxSchedulerHolidaysLocations(Source).Count - 1 do
        Add(TcxSchedulerHolidaysLocations(Source).Items[I].Name)
          .Assign(TcxSchedulerHolidaysLocations(Source).Items[I]);
    finally
      EndUpdate;
    end;
  end
...
function TcxSchedulerHolidaysLocations.Add(const AName: string)
  : TcxSchedulerHolidaysLocation;
begin
...
function TcxSchedulerHolidaysLocations.GetLocationByName(const AName: string)
  : TcxSchedulerHolidaysLocation;
var
  I: Integer;
begin
...
function TcxSchedulerHolidaysLocations.GetItem(AIndex: Integer)
  : TcxSchedulerHolidaysLocation;
begin
...
function TcxSchedulerHolidays.AddHoliday(const ALocationName,
  AHolidayName: string; const ADate: TDateTime)
  : TcxSchedulerHolidaysLocationHoliday;
var
...
procedure TcxSchedulerHolidays.AddListener
  (AListener: IcxSchedulerHolidaysListener);
begin
  if FListeners.IndexOf(AListener) = -1 then
    FListeners.Add(AListener);
end;

function TcxSchedulerHolidays.AddLocation(const AName: string)
  : TcxSchedulerHolidaysLocation;
begin
...
begin
  ...
  begin
    ...
    if AOnlyVisible and not Locations[I].Visible then
      Continue;
...
procedure TcxSchedulerHolidays.RemoveListener
  (AListener: IcxSchedulerHolidaysListener);
begin
...
procedure TcxSchedulerHolidays.RestoreFromIniFile(const AStorageName: string;
  const ARestoreName: string = '');
begin
  RestoreFrom(AStorageName, nil, TcxIniFileReader, ARestoreName);
end;

procedure TcxSchedulerHolidays.RestoreFromRegistry(const AStorageName: string;
  const ARestoreName: string = '');
begin
  RestoreFrom(AStorageName, nil, TcxRegistryReader, ARestoreName);
end;

procedure TcxSchedulerHolidays.RestoreFromStream(AStream: TStream;
  const ARestoreName: string = '');
begin
  RestoreFrom('', AStream, TcxStreamReader, ARestoreName);
end;

procedure TcxSchedulerHolidays.RestoreFromStorage(const AStorageName: string;
  AReaderClass: TcxCustomReaderClass; const ARestoreName: string = '');
begin
  RestoreFrom(AStorageName, nil, AReaderClass, ARestoreName);
end;

procedure TcxSchedulerHolidays.StoreToIniFile(const AStorageName: string;
  const ASaveName: string = '');
begin
  StoreTo(AStorageName, nil, TcxIniFileWriter, ASaveName);
end;

procedure TcxSchedulerHolidays.StoreToRegistry(const AStorageName: string;
  const ASaveName: string = '');
begin
  StoreTo(AStorageName, nil, TcxRegistryWriter, ASaveName);
end;

procedure TcxSchedulerHolidays.StoreToStream(AStream: TStream;
  const ASaveName: string = '');
begin
  StoreTo('', AStream, TcxStreamWriter, ASaveName);
end;

procedure TcxSchedulerHolidays.StoreToStorage(const AStorageName: string;
  AWriterClass: TcxCustomWriterClass; const ASaveName: string = '');
begin
...
begin
  for I := Listeners.Count - 1 downto 0 do
    ...
    begin
      ...
      else if not IsUpdatingMode then
  ...
  try
    ...
    try
      ...
      for I := 0 to AHolidaysList.Count - 1 do
        if TryStringToLocation(AHolidaysList[I], ALocations) or
          ((ALocations.Count > 0) and TryStringToHoliday(AHolidaysList[I],
          ALocations[ALocations.Count - 1])) then
        else
          Result := False;
    finally
      AHolidaysList.Free;
...
function TcxSchedulerHolidays.TryStringToHoliday(const S: string;
  ALocation: TcxSchedulerHolidaysLocation): Boolean;
var
  ADate: TDate;
  APart, AName: string;
...
function TcxSchedulerHolidays.TryStringToLocation(const S: string;
  ALocations: TcxSchedulerHolidaysLocations): Boolean;
var
...
begin
  ...
  if Result then
    Result := ALocations.Add(Copy(S, AStartPos + 1, AEndPos - AStartPos -
      1)) <> nil;
end;

function TcxSchedulerHolidays.TryStrToIntW(const S: string;
...
procedure TcxSchedulerHolidays.RestoreFrom(const AStorageName: string;
...
procedure TcxSchedulerHolidays.StoreTo(const AStorageName: string;
  AStream: TStream; AWriterClass: TcxCustomWriterClass;
  const ASaveName: string);
var
  AStorage: TcxStorage;
begin
...
function TcxSchedulerHolidays.CreateChild(const AObjectName,
  AClassName: string): TObject;
begin
  Result := TCollection(Locations).Add;
end;

procedure TcxSchedulerHolidays.DeleteChild(const AObjectName: string;
  AObject: TObject);
begin
  AObject.Free;
end;
...
function TcxSchedulerHolidays.GetStoredProperties
  (AProperties: TStrings): Boolean;
begin
  Result := True;
end;

procedure TcxSchedulerHolidays.GetStoredPropertyValue(const AName: string;
  var AValue: Variant);
begin
  // do nothing
end;

procedure TcxSchedulerHolidays.SetStoredPropertyValue(const AName: string;
...
function TcxSchedulerHolidays.GetItem(AIndex: Integer)
  : TcxSchedulerHolidaysLocationHoliday;
var
  I, J: Integer;
begin
...
function TcxSchedulerHolidays.GetStringPart(const S: string; var APos: Integer;
  ACheckDateSeparator: Boolean = True): string;
var
  L: Integer;
...
begin
  ...
  if (APos > Length(S)) or (APos <= 0) then
    Exit;
  while (APos > 0) and dxCharInSet(S[APos], ASeparators) or (S[APos] = ' ') do
    Dec(APos);
  L := APos;
  while (APos > 0) and not dxCharInSet(S[APos], ASeparators) do
    Dec(APos);
  if APos < L then
  begin
...
procedure TcxSchedulerHolidays.SetLocations(const AValue
  : TcxSchedulerHolidaysLocations);
begin
  BeginUpdate;
  try
...
function TcxSchedulerHolidayCalculator.GetRecurrenceInfo
...
function TcxSchedulerHolidayCalculator.GetHoliday
  : TcxSchedulerHolidaysLocationHoliday;
begin
...
function TcxSchedulerHolidayRecurrenceInfo.GetValidStatus
...
begin
  ...
  with TcxSchedulerHolidayCalculator.Create(Holiday,
    ...
    try
      ...
      begin
        if Holiday.RecurrenceInfo.Count = 1 then
          Exit;
...
procedure TcxSchedulerHolidayRecurrenceInfo.SetFinish(AValue: TDateTime);
begin
  AValue := DateTimeHelper.RoundTime(AValue);
  SetDataItem(@DefInfoData.Finish, SizeOf(AValue), AValue);
end;

procedure TcxSchedulerHolidayRecurrenceInfo.SetStart(const AValue: TDateTime);
begin
  Holiday.Date := AValue;
end;

function TcxSchedulerHolidayRecurrenceInfo.GetCalculatorClass
  : TcxSchedulerRecurrenceCalculatorClass;
begin
  Result := TcxSchedulerHolidayCalculator;
end;

function TcxSchedulerHolidayRecurrenceInfo.GetData
...
procedure TcxSchedulerHolidayRecurrenceInfo.SetDataItem(AOffset: Pointer;
...
procedure TcxSchedulerHolidayRecurrenceInfo.SetValue(const AValue: AnsiString);
begin
  Holiday.RecurrenceData := cxHolidayStringToRecurrenceInfoData(AValue);
end;

function TcxSchedulerHolidayRecurrenceInfo.GetHoliday
  : TcxSchedulerHolidaysLocationHoliday;
begin
  Result := TcxSchedulerHolidaysLocationHoliday(Owner);
end;
