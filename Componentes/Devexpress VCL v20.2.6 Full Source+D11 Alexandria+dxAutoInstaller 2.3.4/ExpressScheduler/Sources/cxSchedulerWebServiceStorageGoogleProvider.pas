...
type
  ...
  public
    class function GetReminders(AOverrides: TdxJSONValue)
      : TArray<Integer>; static;
  strict private
    FRecurrence: string;
    class function DateTimeToJSON(AValue: TDateTime): TdxJSONObject; static;
    class function JSONToDateTime(AJSONValue: TdxJSONValue;
      out AIsDatePart, AIsUTC: Boolean): TDateTime; static;
  ...
  protected
    ...
    property Provider: TcxSchedulerWebServiceStorageGoogleProvider
      read GetProvider;
  public
  ...
  TcxSchedulerWebServiceStorageGoogleProviderSyncEventsTask = class
    (TcxSchedulerWebServiceStorageProviderSyncEventsCustomTask)
  strict private
  const
{$REGION 'strict private const'}
    FGetEventListEndPoint =
      'https://www.googleapis.com/calendar/v3/calendars/%s/events?showDeleted=false';
    FGetEventListByPagesEndPoint =
      'https://www.googleapis.com/calendar/v3/calendars/%s/events?showDeleted=false&pageToken=%s';
    FGetEventListBetweenDatesEndPoint =
      'https://www.googleapis.com/calendar/v3/calendars/%s/events?showDeleted=false&timeMax=%s&timeMin=%s';
    FGetEventListBetweenDatesByPagesEndPoint =
      'https://www.googleapis.com/calendar/v3/calendars/%s/events?showDeleted=false&timeMax=%s&timeMin=%s&pageToken=%s';
{$ENDREGION}
  strict private
  ...
  TcxSchedulerWebServiceStorageGoogleProviderModifyEventTask = class
  ...
  TcxSchedulerWebServiceStorageGoogleProviderDismissReminderTask = class
    (TcxSchedulerWebServiceStorageProviderModifyEventCustomTask)
  protected
    function Process: TdxJSONObject; override;
  end;
  ...
  TcxSchedulerWebServiceStorageGoogleProviderDeleteEventTask = class
  ...
  TcxSchedulerWebServiceStorageGoogleProviderUpdateEventTask = class
  ...
  TcxSchedulerWebServiceStorageGoogleProviderPostEventTask = class
    (TcxSchedulerWebServiceStorageProviderPostEventCustomTask)
  strict private
  const
{$REGION 'strict private const'}
    FPostEventObjectName = '/calendar/v3/calendars/%s/events';
    FGetOccurrencesEndPoint =
      'https://www.googleapis.com/calendar/v3/calendars/%s/events/%s/instances?originalStart=%s';
{$ENDREGION}
  strict private
...
{$ENDREGION}
  ...
  protected
    ...
    procedure PopulateCalendarList(const AList
      : TdxWebServiceCalendarList); override;
...
type
  ...
  TcxSchedulerICalendarRecurrenceProperty = class
  ...
  public
    ...
    class procedure ApplyRRULE(AEvent: TcxSchedulerWebServiceEvent;
      const RRULE: string); static;
    class function GetRRULE(AEvent: TcxSchedulerWebServiceEvent)
      : string; static;
  end;

  { TcxSchedulerICalendarRecurrenceProperty }

constructor TcxSchedulerICalendarRecurrenceProperty.Create
...
class procedure TcxSchedulerICalendarRecurrenceProperty.ApplyRRULE
  (AEvent: TcxSchedulerWebServiceEvent; const RRULE: string);
var
...
class function TcxSchedulerICalendarRecurrenceProperty.GetRRULE
  (AEvent: TcxSchedulerWebServiceEvent): string;
begin
  with TcxSchedulerICalendarRecurrenceProperty.Create(AEvent) do
    try
      Result := GetValue;
...
begin
  ...
  begin
    ...
    if RecurringId <> '' then
    begin
      FOriginalStartTime := JSONToDateTime(AObject.Get('originalStartTime'),
        AIsDatePart, AIsUTC);
      if IsCancelled then
        FType := etException
      else
...
begin
  ...
  begin
    if &Type = etPattern then
      Result.AddPair('recurrence',
        TdxJSONArray.Create(TdxJSONString.Create(Format('RRULE:%s',
        [FRecurrence]))));
  end;
  ...
  begin
    ...
    begin
      AReminder := TdxJSONObject.Create(TdxJSONPair.Create('minutes',
        IntToStr(Reminders[0])));
...
procedure TdxGoogleCalendarEvent.DoAssignFrom
  (AEvent: TcxSchedulerWebServiceEvent);
begin
...
procedure TdxGoogleCalendarEvent.DoAssignTo
  (AEvent: TcxSchedulerWebServiceEvent);
begin
...
class function TdxGoogleCalendarEvent.DateTimeToJSON(AValue: TDateTime)
  : TdxJSONObject;
var
...
class function TdxGoogleCalendarEvent.JSONToDateTime(AJSONValue: TdxJSONValue;
  out AIsDatePart, AIsUTC: Boolean): TDateTime;
var
  AValue: string;
begin
...
class function TdxGoogleCalendarEvent.GetReminders(AOverrides: TdxJSONValue)
  : TArray<Integer>;
var
  I: Integer;
  AValue: Integer;
...
function TdxGoogleCalendarEvent.GetProvider
  : TcxSchedulerWebServiceStorageGoogleProvider;
begin
...
function TcxSchedulerWebServiceStorageGoogleProviderSyncEventsTask.
  GetJSONItemList: TdxJSONObject;
begin
  if FNextPageToken = '' then
    Result := TdxHttpHelper.GetRequest(UserAgent, Format(FGetEventListEndPoint,
      [TdxURI.EscapeDataString(CalendarId)]), Header)
  else
    Result := TdxHttpHelper.GetRequest(UserAgent,
      Format(FGetEventListByPagesEndPoint, [TdxURI.EscapeDataString(CalendarId),
      NextPageToken]), Header);
end;
...
procedure TcxSchedulerWebServiceStorageGoogleProviderSyncEventsTask.
...
function TcxSchedulerWebServiceStorageGoogleProviderSyncEventsTask.GetProvider
  : TcxSchedulerWebServiceStorageGoogleProvider;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProvider(inherited Provider);
end;
...
function TcxSchedulerWebServiceStorageGoogleProviderModifyEventTask.Process
  : TdxJSONObject;
var
...
begin
  ...
  try
    AObjectName :=
      Format(TcxSchedulerWebServiceStorageGoogleProvider.UpdateEventObjectName,
      [CalendarId, WebServiceEvent.GetId]);
    Result := TdxHttpHelper.PutRequest(UserAgent,
      TcxSchedulerWebServiceStorageGoogleProvider.ServerName, AObjectName,
      Header, AObject);
  finally
    AObject.Free;
  end;
...
function TcxSchedulerWebServiceStorageGoogleProviderDismissReminderTask.Process
  : TdxJSONObject;
var
  AObject: TdxJSONObject;
  AObjectName: string;
...
function TcxSchedulerWebServiceStorageGoogleProviderDeleteEventTask.Process
  : TdxJSONObject;
var
  AObjectName: string;
begin
  if WebServiceEvent.GetId = '' then
    Exit(nil);
  AObjectName := Format(TcxSchedulerWebServiceStorageGoogleProvider.
    UpdateEventObjectName, [CalendarId, WebServiceEvent.GetId]);
...
procedure TcxSchedulerWebServiceStorageGoogleProviderPostEventTask.
  DoApplyChanges;
begin
...
function TcxSchedulerWebServiceStorageGoogleProviderPostEventTask.Process
  : TdxJSONObject;
var
  AObject: TdxJSONObject;
  AObjectName: string;
...
function TcxSchedulerWebServiceStorageGoogleProviderPostEventTask.
...
begin
  Result := '';
  AGetOccurrencesEndPoint := Format(FGetOccurrencesEndPoint,
    [CalendarId, AEvent.GetPatternId,
    TdxISO8601Helper.DateTimeToString(AEvent.GetOriginalOccurrenceStartTime)]);
  AObject := TdxHttpHelper.GetRequest(UserAgent,
    AGetOccurrencesEndPoint, Header);
...
function TcxSchedulerWebServiceStorageGoogleProviderUpdateEventTask.Process
  : TdxJSONObject;
var
  AObjectName: string;
begin
  if WebServiceEvent.Id = '' then
    Exit(nil);
  AObjectName := Format(TcxSchedulerWebServiceStorageGoogleProvider.
    UpdateEventObjectName, [CalendarId, WebServiceEvent.Id]);
  Result := TdxHttpHelper.GetRequest(UserAgent,
    TcxSchedulerWebServiceStorageGoogleProvider.ServerName, AObjectName,
    Header, nil);
end;
...
constructor TcxSchedulerWebServiceStorageGoogleProvider.Create
...
class function TcxSchedulerWebServiceStorageGoogleProvider.
...
function TcxSchedulerWebServiceStorageGoogleProvider.GetWebServiceEventClass
  : TdxWebServiceEventClass;
begin
  Result := TdxGoogleCalendarEvent;
end;

function TcxSchedulerWebServiceStorageGoogleProvider.CreateDeleteEventTask
  (AEvent: TcxSchedulerWebServiceEvent;
  const AWebServiceEvent: IdxWebServiceEvent)
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProviderDeleteEventTask.Create
    (AEvent, AWebServiceEvent);
end;

function TcxSchedulerWebServiceStorageGoogleProvider.CreateDismissEventTask
  (AEvent: TcxSchedulerWebServiceEvent)
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProviderDismissReminderTask.
    Create(AEvent);
end;
...
function TcxSchedulerWebServiceStorageGoogleProvider.CreateModifyEventTask
  ...
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProviderModifyEventTask.Create
    (AEvent, AWebServiceEvent);
end;
...
function TcxSchedulerWebServiceStorageGoogleProvider.CreatePostEventTask
  ...
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProviderPostEventTask.
    Create(AEvent);
end;

function TcxSchedulerWebServiceStorageGoogleProvider.CreateSyncEventsTask
  : TcxSchedulerWebServiceStorageProviderSyncEventsCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProviderSyncEventsTask.
    Create(Self);
end;

function TcxSchedulerWebServiceStorageGoogleProvider.CreateUpdateEventTask
  (AEvent: TcxSchedulerWebServiceEvent)
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageGoogleProviderUpdateEventTask.
    Create(AEvent);
end;

function TcxSchedulerWebServiceStorageGoogleProvider.GetScopes: TStringList;
const
  AResultMap: array [Boolean] of string =
...
class function TcxSchedulerWebServiceStorageGoogleProvider.IsItemValid
  (AItem: TdxJSONValue): Boolean;
begin
  Result := AItem.GetParamValue('kind') = 'calendar#event';
end;

procedure TcxSchedulerWebServiceStorageGoogleProvider.DismissEvent
...
procedure TcxSchedulerWebServiceStorageGoogleProvider.PopulateCalendarList
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              ACalendar.EventColor := TdxAlphaColors.ToColor
                (TdxAlphaColors.FromHtml
                (AItem.GetParamValue('backgroundColor')));
              ACalendar.DefaultReminders := TdxGoogleCalendarEvent.GetReminders
                (AItem.Get('defaultReminders'));
              ACalendar.ReadOnly := AItem.GetParamValue('accessRole')
                = 'reader';
              AList.Add(ACalendar);
            end;
          end;
...
procedure TcxSchedulerWebServiceStorageGoogleProvider.UpdateOccurrences
  (AEvent: TcxSchedulerWebServiceEvent);
var
  ALink: TcxSchedulerWebServiceEvent;
begin
  ALink := TcxSchedulerWebServiceEvent
    (TcxSchedulerWebServiceEventAccess(AEvent).Link);
  while ALink <> nil do
  begin
    AddTask(CreateUpdateEventTask(ALink));
    ALink := TcxSchedulerWebServiceEvent
      (TcxSchedulerWebServiceEventAccess(ALink).Link);
  end;
end;
...
procedure TcxSchedulerWebServiceStorageGoogleProvider.PopulateColors;
  ...
  begin
    ...
    else if not TryStrToInt(Value, Result) then
      Result := -1;
  end;
...
{$ELSE}
            AItem := AArray.Items[I];
{$ENDIF}
            AColor := JSONStringToColor
              (AItem.JsonValue.GetParamValue('background'));
            if AColor <> -1 then
...
function TcxSchedulerWebServiceStorageGoogleProvider.GetAuthorizationAgent
  : TdxGoogleAPIOAuth2AuthorizationAgent;
begin
  Result := TdxGoogleAPIOAuth2AuthorizationAgent(inherited AuthorizationAgent);
end;

procedure TcxSchedulerWebServiceStorageGoogleProvider.SetAuthorizationAgent
