...
uses
  ...
  cxCalendar, cxGraphics, ComCtrls, dxCore, cxDateUtils, dxLayoutcxEditAdapters,
...
type
  ...
  public
    ...
    property LocationHoliday: TcxSchedulerHolidaysLocationHoliday
      read FLocationHoliday write SetLocationHoliday;
  end;
...
begin
  btnOk.Enabled := (Length(teName.Text) > 0) and
    (TcxDateEditAccess(deDate).DisplayText <> '') and (deDate.Date <> NullDate)
    and (LocationHoliday.Location.Find(teName.Text, deDate.Date) = nil);
end;
