...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Variants, Windows, Forms, Messages, Classes, Math, StdCtrls, Graphics,
  Controls, ExtCtrls,
  Contnrs, SysUtils, Types, RTLConsts, cxInplaceContainer, dxCore, cxControls,
  cxGraphics,
  cxEdit, cxTextEdit, cxMemo, cxCalendar, cxCheckbox, cxSpinEdit,
  cxDrawTextUtils,
  cxSchedulerStrs, cxSchedulerCustomControls, cxSchedulerUtils,
  cxSchedulerStorage, cxClasses,
  cxScrollBar, cxSchedulerCustomResourceView, cxSchedulerTimeGridView, cxTL,
  cxTLData,
  cxCustomData, cxVariants, dxMessages, dxCoreClasses;
...
type
  ...
  TcxSchedulerTreeListBrowser = class(TcxVirtualTreeList,
  ...
  protected
    function ChangeExpanding(ANode: TcxTreeListNode;
      ARecursive, AExpanded: Boolean): Boolean;
    procedure CreateAllColumns; virtual;
    procedure DragOverEventHandler(Sender, Source: TObject; X, Y: Integer;
  ...
  public
    ...
    property Style: TcxShedulerTreeBrowserStyle read FStyle write SetStyle
      default tbsTree;
  ...
  protected
    function GetColumnDataProvider(AItemHandle: TcxDataItemHandle)
      : TcxSchedulerTreeListBrowserColumnDataProvider;
    function GetChildCount(AParentHandle: TcxDataRecordHandle)
      : Integer; override;
    function GetChildRecordHandle(AParentHandle: TcxDataRecordHandle;
      AChildIndex: Integer): TcxDataRecordHandle; override;
    function GetEventByGroupID(const AGroupID: Variant; AIndex: Integer)
      : TcxSchedulerEvent;
    function GetEventByTaskLink(ALink: TcxSchedulerEventItemLink)
      : TcxSchedulerEvent;
  ...
  protected
    ...
    procedure SetValue(const AEvent: TcxSchedulerEvent;
      const AValue: Variant); virtual;
    procedure TranslationChanged;
  public
    constructor Create(ABrowser: TcxSchedulerTreeListBrowser;
      AVisible: Boolean);
  ...
  TcxSchedulerTreeListBrowserColumnCaptionDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
  ...
  TcxSchedulerTreeListBrowserColumnLocationDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
    function GetCaption: string; override;
    function GetValue(const AEvent: TcxSchedulerEvent): Variant; override;
  ...
  TcxSchedulerTreeListBrowserColumnDescriptionDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
    function GetCaption: string; override;
  ...
  TcxSchedulerTreeListBrowserColumnDateDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
    function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
  ...
  TcxSchedulerTreeListBrowserColumnStartDataProvider = class
  ...
  TcxSchedulerTreeListBrowserColumnFinishDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDateDataProvider)
  protected
    function GetCaption: string; override;
    function GetValue(const AEvent: TcxSchedulerEvent): Variant; override;
  ...
  TcxSchedulerTreeListBrowserColumnReminderDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
    function GetCaption: string; override;
    function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
  ...
  TcxSchedulerTreeListBrowserColumnProgressDataProvider = class
    (TcxSchedulerTreeListBrowserColumnDataProvider)
  protected
    function GetCaption: string; override;
    function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
  ...
  protected
    ...
    property View_ViewInfo: TcxSchedulerTimeGridViewViewInfo
      read GetView_ViewInfo;
  end;
...
function dxVarToDateTime(const AValue: Variant; const ADefaultValue: TDateTime)
  : TDateTime;
begin
  try
...
function TcxSchedulerTreeBrowserDataSource.GetColumnDataProvider
  ...
  : TcxSchedulerTreeListBrowserColumnDataProvider;
begin
  Result := TcxSchedulerTreeListBrowserColumnDataProvider
    (TcxTreeListColumn(AItemHandle).Data);
end;

function TcxSchedulerTreeBrowserDataSource.GetChildCount(AParentHandle
...
begin
  ...
  else if (AParentHandle = nil) or TcxSchedulerEvent(AParentHandle).IsGroup then
  begin
    for I := 0 to Events.Count - 1 do
      if VarEquals(TcxSchedulerEvent(Events[I]).GroupID,
        GetGroupID(AParentHandle)) then
        Inc(Result);
  end;
end;

function TcxSchedulerTreeBrowserDataSource.GetChildRecordHandle
  (AParentHandle: TcxDataRecordHandle; AChildIndex: Integer)
  : TcxDataRecordHandle;
begin
  Result := nil;
  if Browser.Style = tbsList then
...
function TcxSchedulerTreeBrowserDataSource.GetEventByGroupID(const AGroupID
  : Variant; AIndex: Integer): TcxSchedulerEvent;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    if (AEvent.Source = ALink.Link) and
      (AEvent.RecurrenceIndex = ALink.LinkRecurrenceIndex) then
    begin
      Result := AEvent;
      Break;
    end;
...
function TcxSchedulerTreeBrowserDataSource.GetGroupID(AParentHandle
...
function TcxSchedulerTreeBrowserDataSource.GetItemHandle(AItemIndex: Integer)
  : TcxDataItemHandle;
begin
  Result := Browser.Columns[AItemIndex];
end;

function TcxSchedulerTreeBrowserDataSource.GetParentRecordHandle
...
begin
  ...
  begin
    for I := 0 to Events.Count - 1 do
      if VarEquals(TcxSchedulerEvent(ARecordHandle).GroupID,
        TcxSchedulerEvent(Events[I]).ID) then
      begin
        Result := Events[I];
        Exit;
      end;
...
function TcxSchedulerTreeBrowserDataSource.GetRecordHandle
  (ARecordIndex: Integer): TcxDataRecordHandle;
begin
  Result := Events[ARecordIndex];
end;

function TcxSchedulerTreeBrowserDataSource.GetRootRecordHandle
  : TcxDataRecordHandle;
begin
  Result := nil;
end;

function TcxSchedulerTreeBrowserDataSource.GetValue(ARecordHandle
  : TcxDataRecordHandle; AItemHandle: TcxDataItemHandle): Variant;
begin
  if ARecordHandle = nil then
    Exit;
  Result := GetColumnDataProvider(AItemHandle)
    .GetValue(TcxSchedulerEvent(ARecordHandle));
end;

function cxCompareBrowserEvents(AEvent1,
  AEvent2: TcxSchedulerControlEventAccess): Integer;
begin
  if (AEvent1.VisibleIndex < 0) and (AEvent2.VisibleIndex >= 0) then
    Result := 1
  else if (AEvent1.VisibleIndex >= 0) and (AEvent2.VisibleIndex < 0) then
    Result := -1
  else
...
begin
  ...
  for I := 0 to FEvents.Count - 1 do
    if TcxSchedulerControlEventAccess(FEvents[I]).VisibleIndex < 0 then
      Inc(C);
  ...
  begin
    ...
    if AEvent.EventType <> etNone then
      while (I < FEvents.Count) and
        (TcxSchedulerControlEventAccess(FEvents[I]).Pattern = AEvent.Pattern) do
        FEvents.Delete(I);
  end;
end;

procedure TcxSchedulerTreeBrowserDataSource.SetValue(ARecordHandle
  : TcxDataRecordHandle; AItemHandle: TcxDataItemHandle; const AValue: Variant);
begin
  if ARecordHandle = nil then
    Exit;
  GetColumnDataProvider(AItemHandle)
    .SetValue(TcxSchedulerControlEvent(ARecordHandle).Source, AValue);
end;

function TcxSchedulerTreeBrowserDataSource.GetViewInfo
...
function TcxSchedulerTreeListBrowserColumnDataProvider.GetDefaultColumnWidth
  : Integer;
begin
  Result := 50;
end;

function TcxSchedulerTreeListBrowserColumnDataProvider.GetPropertiesClass
...
function TcxSchedulerTreeListBrowserColumnCaptionDataProvider.
  GetCaption: string;
begin
...
function TcxSchedulerTreeListBrowserColumnLocationDataProvider.
  GetCaption: string;
begin
...
function TcxSchedulerTreeListBrowserColumnDescriptionDataProvider.
  GetCaption: string;
begin
...
function TcxSchedulerTreeListBrowserColumnDateDataProvider.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxDateEditProperties;
end;

procedure TcxSchedulerTreeListBrowserColumnDateDataProvider.
  InitializeProperties;
begin
...
function TcxSchedulerTreeListBrowserColumnReminderDataProvider.
  GetCaption: string;
begin
...
function TcxSchedulerTreeListBrowserColumnProgressDataProvider.
  GetCaption: string;
begin
...
procedure TcxSchedulerTreeListBrowserColumnProgressDataProvider.
  InitializeProperties;
begin
  inherited InitializeProperties;
...
begin
  for I := 0 to ColumnCount - 1 do
    TcxSchedulerTreeListBrowserColumnDataProvider(Columns[I].Data)
      .TranslationChanged;
end;

function TcxSchedulerTreeListBrowser.ChangeExpanding(ANode: TcxTreeListNode;
  ARecursive, AExpanded: Boolean): Boolean;
  ...
  begin
    ...
    if not AGroup.HasChildren or not ARecursive then
      Exit;
    ALink := AGroup.First;
    while ALink <> nil do
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      while ANode <> nil do
      begin
        ChangeGroupExpanding
          (TcxSchedulerControlEvent(TcxVirtualTreeListNode(ANode).RecordHandle)
          .Source.GroupItems);
...
begin
  ...
  try
    ...
    TcxSchedulerTreeListBrowserColumnDescriptionDataProvider.Create
      (Self, False);
...
function TcxSchedulerTreeListBrowser.GetDesignHitTest(X, Y: Integer;
  Shift: TShiftState): Boolean;
begin
  Result := False;
end;

function TcxSchedulerTreeListBrowser.GetMouseWheelScrollingKind
...
function TcxSchedulerTreeListBrowser.GetTreeListColumnClass
  : TcxTreeListColumnClass;
begin
  Result := TcxTreeListBrowserColumn;
end;

function TcxSchedulerTreeListBrowser.GetViewInfoClass
  : TcxCustomControlViewInfoClass;
begin
  Result := TcxSchedulerTreeListBrowserViewInfo;
end;

procedure TcxSchedulerTreeListBrowser.DragOverEventHandler(Sender,
...
begin
  ...
  begin
    ...
    if Accept then
    begin
      FDropGroup := TcxSchedulerControlEvent
        (TcxVirtualTreeListNode(HitTest.HitNode).RecordHandle).Source;
      Accept := FDropGroup.IsGroup;
      if not Accept then
        FDropGroup := nil
      else
        Accept := not VarEquals(FDropGroup.ID, FocusedEvent.GroupID) and
          not VarEquals(FDropGroup.ID, FocusedEvent.ID);
    end;
  end;
  DragCursor := ADragCursors[Accept];
end;

procedure TcxSchedulerTreeListBrowser.DrawIndicator(Sender: TcxCustomTreeList;
  ACanvas: TcxCanvas; AViewInfo: TcxTreeListIndicatorCellViewInfo;
  var ADone: Boolean);
var
  R: TRect;
begin
  ADone := (AViewInfo.Node <> nil) and
    not(AViewInfo.Node.Focused or AViewInfo.Node.Selected);
  if ADone then
  begin
    AViewInfo.Painter.DrawScaledIndicatorItem(ACanvas, AViewInfo.BoundsRect,
      AViewInfo.Kind, AViewInfo.ViewParams.Color, ScaleFactor,
      TcxTreeListIndicatorAccess(AViewInfo).DrawBackgroundHandler);
    R := TcxTreeListIndicatorAccess(AViewInfo).TextBounds;
    cxTextOut(ACanvas.Canvas, IntToStr(AViewInfo.Node.AbsoluteIndex), R,
      cxMakeFormat(taCenterX, taCenterY), 0, 0, AViewInfo.ViewParams.Font,
      clNone, clNone);
    ACanvas.ExcludeClipRect(TcxTreeListIndicatorAccess(AViewInfo).ClipRect);
  end;
end;

procedure TcxSchedulerTreeListBrowser.KeyDown(var Key: Word;
  Shift: TShiftState);
const
...
begin
  ...
  if ((Key = VK_LEFT) or (Key = VK_RIGHT)) and (Shift * ssAltShift = ssAltShift)
  then
  begin
    if HandleAllocated then
      PostMessage(Handle, WM_GROUPUNGROUP,
        wParam(Key = ANextCellKey[UseRightToLeftAlignment]), 0);
    Key := 0;
  end;
  inherited;
...
procedure TcxSchedulerTreeListBrowser.RefreshData;
  ...
  begin
    ...
    begin
      AEvent := TcxSchedulerControlEvent(TcxVirtualTreeListNode(ANode)
        .RecordHandle);
...
function TcxSchedulerTreeListBrowser.InternalCollapseNode
  (ANode: TcxTreeListNode; ARecursive: Boolean): Boolean;
begin
  Result := ChangeExpanding(ANode, ARecursive, False);
end;

function TcxSchedulerTreeListBrowser.InternalExpandNode(ANode: TcxTreeListNode;
  ARecursive: Boolean): Boolean;
begin
  Result := ChangeExpanding(ANode, ARecursive, True);
end;

function TcxSchedulerTreeListBrowser.IsActionSupported
  (AButtonIndex: Integer): Boolean;
begin
  Result := (Storage <> nil) and Storage.IsActive and
    inherited IsActionSupported(AButtonIndex);
end;

function TcxSchedulerTreeListBrowser.IsMouseWheelHandleNeeded
  (Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := (MouseWheelScrollingKind = mwskVertical) and
    IsVerticalScrollBarVisible or (MouseWheelScrollingKind = mwskHorizontal) and
    IsScrollBarActive(sbHorizontal);
end;

function TcxSchedulerTreeListBrowser.IsVerticalScrollBarVisible: Boolean;
begin
  Result := (TcxSchedulerViewAccess(View).ViewInfo.ResourceViewShiftList.Count >
    0) or TcxSchedulerAccess(TcxSchedulerViewAccess(View).Scheduler)
    .VScrollBarVisible;
end;

procedure TcxSchedulerTreeListBrowser.Scroll(AScrollBarKind: TScrollBarKind;
...
begin
  ViewInfo.Calculate;
  DefaultRowHeight := AHeight - Byte(OptionsView.GridLines in [tlglBoth,
    tlglHorz]);
end;
...
procedure TcxSchedulerTreeListBrowser.SetMode
  (AMode: TcxSchedulerTreeBrowserDisplayMode);
begin
end;

function TcxSchedulerTreeListBrowser.GetFocusedEvent: TcxSchedulerEvent;
begin
  Result := TcxSchedulerControlEvent(TcxVirtualTreeListNode(FocusedNode)
    .RecordHandle).Source;
end;
...
begin
  TcxSchedulerEvent(AMessage.wParam).ParentGroup :=
    TcxSchedulerEvent(AMessage.lParam);
end;
...
begin
  ...
  if (APrevNode = nil) or (not AMakeGroup and VarIsNull(AEvent.GroupID)) then
    Exit;
  AGroupEvent := TcxSchedulerControlEvent(TcxVirtualTreeListNode(APrevNode)
    .RecordHandle).Source;
  Storage.BeginUpdate;
  try
    if AMakeGroup then
...
procedure TcxSchedulerTreeListBrowser.SetView(AView: TcxSchedulerTimeGridView);
begin
  FView := AView;
  if View = nil then
    CustomDataSource := nil
...
begin
  ...
  if Result and (TreeList.FocusedNode <> nil) then
    Result := not TcxSchedulerEvent(TcxVirtualTreeListNode(TreeList.FocusedNode)
      .RecordHandle).IsGroup;
end;
...
function TcxSchedulerTreeListBrowserViewInfo.GetBrowser
  : TcxSchedulerTreeListBrowser;
begin
...
function TcxSchedulerTreeListBrowserViewInfo.GetView_ViewInfo
  : TcxSchedulerTimeGridViewViewInfo;
begin
