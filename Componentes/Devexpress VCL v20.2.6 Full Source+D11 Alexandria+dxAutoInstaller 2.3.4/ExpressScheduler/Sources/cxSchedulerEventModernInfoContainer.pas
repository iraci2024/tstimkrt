...
{$ENDIF}
  ...
  Forms, Dialogs, dxForms,
  dxCore, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
  cxContainer, cxEdit, StdCtrls, cxGroupBox,
  cxGeometry, cxSchedulerCustomControls, cxSchedulerCustomResourceView,
  cxSchedulerStorage, cxSchedulerStrs,
  ExtCtrls, cxImage, cxClasses, cxSchedulerUtils, dxLayoutContainer,
...
type
  ...
  protected
    ...
    function GetCaptionFullHeight(const ACaptionRect: TRect;
  ...
  public
    ...
    procedure Initialize(AEventCell: TcxSchedulerEventCellViewInfo;
  ...
  TcxSchedulerEventModernInfoContainerClass = class of
    TcxSchedulerEventModernInfoContainer;

var
  cxSchedulerEventModernInfoContainerClass
    : TcxSchedulerEventModernInfoContainerClass =
    TcxSchedulerEventModernInfoContainer;
...
constructor TcxSchedulerEventModernInfoContainer.Create
...
procedure TcxSchedulerEventModernInfoContainer.Initialize
  (AEventCell: TcxSchedulerEventCellViewInfo;
  AEventInfo: TcxSchedulerEventModernStyleHintInfo);
begin
  dxLayoutSkinLookAndFeel1.LookAndFeel := TcxCustomSchedulerAccess(FScheduler)
    .LookAndFeel;
  dxLayoutSkinLookAndFeel1.LookAndFeel.ScrollbarMode := sbmClassic;
  dxLayoutCxLookAndFeel1.LookAndFeel := TcxCustomSchedulerAccess(FScheduler)
    .LookAndFeel;
...
begin
  liStartCaption.CaptionOptions.Text :=
    cxGetResourceString(@scxModernStyleHintStart);
  liEndCaption.CaptionOptions.Text :=
    cxGetResourceString(@scxModernStyleHintEnd);
  liLocationCaption.CaptionOptions.Text :=
    cxGetResourceString(@scxModernStyleHintLocation);
  liReminderCaption.CaptionOptions.Text :=
    cxGetResourceString(@scxModernStyleHintReminder);
  liTaskComplete.CaptionOptions.Text :=
    cxGetResourceString(@scxModernStyleHintComplete);
end;
...
procedure TcxSchedulerEventModernInfoContainer.CreateEventBitmap(AWidth,
  AHeight: Integer);
begin
  FEventBitmap := TcxAlphaBitmap.CreateSize(AWidth, AHeight);
end;

function TcxSchedulerEventModernInfoContainer.GetCaptionOutFlags
  (ACenterVert: Boolean): Integer;
const
...
begin
  Result := AAlignmentHorz[UseRightToLeftAlignment] or
    AAlignmentVert[ACenterVert] or CXTO_EXCELSTYLE or CXTO_END_ELLIPSIS or
    CXTO_EDITCONTROL or CXTO_WORDBREAK;
end;

function TcxSchedulerEventModernInfoContainer.GetCaptionFullHeight
  (const ACaptionRect: TRect; const ACaption: string; AFont: TFont): Integer;
var
  DC: HDC;
  AFontHandle: HFONT;
...
begin
  ...
  try
    ...
    ATextParams := cxCalcTextParams(DC, GetCaptionOutFlags(False) or
      CXTO_CALCROWCOUNT);
    cxMakeTextRows(DC, PChar(ACaption), Length(ACaption), R, ATextParams,
...
begin
  ...
  ATimeLineBorderColor := TcxSchedulerEventCellViewInfoAccess(EventCell)
    .GetTimeLineBorderColor;

  CreateEventBitmap(pbEvent.ClientWidth, pbEvent.ClientHeight);
  if ALabelColor = clDefault then
  begin
    ALabelColor := TcxSchedulerEventCellViewInfoAccess(EventCell)
      .Painter.DefaultSchedulerEventColor(False);
    EventBitmap.cxCanvas.FillRect(EventBitmap.ClientRect, ALabelColor);
    EventBitmap.cxCanvas.FrameRect(EventBitmap.ClientRect,
      TcxSchedulerEventCellViewInfoAccess(EventCell).GetLabelBasedBorderColor);
  end
  else
    EventBitmap.cxCanvas.FillRect(EventBitmap.ClientRect, ALabelColor);

  ATimeLineWidth := ScaleFactor.Apply(cxTimeLineWidth);
  ACaptionRect := cxRectInflate(EventBitmap.ClientRect,
    -ScaleFactor.Apply(cxTextOffset * 2));
  AStateRect := cxRect(0, 0, ATimeLineWidth, EventBitmap.Height);
  if UseRightToLeftAlignment then
  begin
    AStateRect := TdxRightToLeftLayoutConverter.ConvertRect(AStateRect,
      EventBitmap.ClientRect);
    Dec(ACaptionRect.Right, ATimeLineWidth);
  end
  else
    Inc(ACaptionRect.Left, ATimeLineWidth);
  TcxSchedulerEventCellViewInfoAccess(EventCell).PainterHelper.DrawState
    (EventBitmap.cxCanvas, AStateRect, EventCell.Event.State, [],
    ATimeLineBorderColor, svsModern);
  ...
  cxTextOut(EventBitmap.cxCanvas.Canvas, ACaption, ACaptionRect,
    GetCaptionOutFlags(ANeedCaptionHeight <= cxRectHeight(ACaptionRect)), 0, 0,
    AFont, clNone, clNone, 0, 0, 0, AFont.Color);
end;

procedure TcxSchedulerEventModernInfoContainer.pbEventPaint(Sender: TObject);
begin
  cxBitBlt(pbEvent.Canvas.Handle, EventBitmap.Canvas.Handle, pbEvent.ClientRect,
    Point(0, 0), SRCCOPY);
end;
