...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, SysUtils, Windows, Classes, Graphics, Generics.Defaults,
...
type
  ...
  private type
{$REGION 'private type'}
    TRecurrencePatternType = (AbsoluteYearly, RelativeYearly, AbsoluteMonthly,
      RelativeMonthly, Weekly, Daily);
...
{$ENDREGION}
  ...
  class var
    ...
    FRecurrencePatternTypeDictionary
      : TDictionary<string, TRecurrencePatternType>;
  ...
  strict private
    ...
    class function GetTimeZoneBias(ADate: TDateTime; const AName: string)
      : TDateTime; static;
    class function DateTimeToJSON(AValue: TDateTime): TdxJSONObject; static;
    class function JSONToDateTime(AJSONValue: TdxJSONValue; out AIsUTC: Boolean)
      : TDateTime; static;
...
{$ENDREGION}
  ...
  TcxSchedulerWebServiceStorageOfficeProviderDismissReminderTask = class
...
{$ENDREGION}
  ...
  public
    constructor Create(AEvent: TcxSchedulerWebServiceEvent; ADate: TDateTime);
      reintroduce;
  end;
...
{$ENDREGION}
  ...
  protected
    ...
    function CreateSyncOccurrencesTask(AEvent: TcxSchedulerWebServiceEvent;
  ...
  public
    constructor Create(AProvider: TcxSchedulerWebServiceStorageCustomProvider;
      const ASeriesMasterID: string); reintroduce; overload;
    constructor Create(AProvider: TcxSchedulerWebServiceStorageCustomProvider;
      const ASeriesMasterID: string; AStart, AFinish: TDateTime);
  ...
  const
{$REGION 'strict private const'}
    FGetEventCustomOccurrenceListEndPoint =
      'https://graph.microsoft.com/v1.0/me/events/%s/instances?$top=1000&$orderby=Start%%2FDateTime&startDateTime=%s&endDateTime=%s';
{$ENDREGION}
  strict private
    FLink: TcxObjectLink;
    ...
    function HasCustomEvent(ADate: TDateTime;
      out AEvent: TcxSchedulerWebServiceEvent): Boolean;
  ...
  public
    constructor Create(AProvider: TcxSchedulerWebServiceStorageCustomProvider;
      ...
      AStartDate, AFinishDate: TDateTime); reintroduce;
    destructor Destroy; override;
  end;

  { TcxSchedulerWebServiceStorageOfficeProviderModifyEventTask }

  TcxSchedulerWebServiceStorageOfficeProviderModifyEventTask = class
  ...
  TcxSchedulerWebServiceStorageOfficeProviderDeleteEventTask = class
  ...
  TcxSchedulerWebServiceStorageOfficeProviderPostEventTask = class
...
{$REGION 'strict private const'}
    ...
    FGetEventCustomOccurrenceListEndPoint =
      'https://graph.microsoft.com/v1.0/me/events/%s/instances?startDateTime=%s&endDateTime=%s';
{$ENDREGION}
  strict private
    function GetOccurrenceCloudID(const AEvent: IdxWebServiceEvent)
      : string; overload;
  protected
    class function GetOccurrenceCloudID(const AUserAgent, AHeader,
      ASeriesMasterID: string; AOccurrenceDate: TDateTime): string;
      overload; static;
  protected
    procedure DoApplyChanges; override; final;
    function ExpectResponse: Boolean; override; final;
...
{$ENDREGION}
  strict private
    ...
    function GetAuthorizationAgent
      : TdxMicrosoftGraphAPIOAuth2AuthorizationAgent; inline;
  ...
  protected
    ...
    procedure OccurrencesCheckedTimeBoundsChanged(AStart,
      AFinish: TDateTime); override;
    procedure ResetOccurrencesCheckedTimeBounds;

{$REGION 'Tasks'}
    function CreateDeleteEventTask(AEvent: TcxSchedulerWebServiceEvent;
      const AWebServiceEvent: IdxWebServiceEvent)
      : TcxSchedulerWebServiceStorageProviderCustomTask; override;
    function CreateDismissEventTask(AEvent: TcxSchedulerWebServiceEvent;
      ADate: TDateTime): TcxSchedulerWebServiceStorageProviderCustomTask;
    function CreateModifyEventTask(AEvent: TcxSchedulerWebServiceEvent;
      const AWebServiceEvent: IdxWebServiceEvent)
      : TcxSchedulerWebServiceStorageProviderCustomTask; overload; override;
    function CreatePostEventTask(AEvent: TcxSchedulerWebServiceEvent)
      : TcxSchedulerWebServiceStorageProviderCustomTask; override;
    function CreateSyncEventsTask
      : TcxSchedulerWebServiceStorageProviderSyncEventsCustomTask; override;
    function CreateSyncSeriesMasterTask(const ASeriesMasterID: string)
      : TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask;
      overload;
    function CreateSyncSeriesMasterTask(const ASeriesMasterID: string;
      AStart, AFinish: TDateTime)
      : TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask;
      overload;
    function CreateUpdateEventTask(AEvent: TcxSchedulerWebServiceEvent)
      : TcxSchedulerWebServiceStorageProviderCustomTask; override; final;
{$ENDREGION}
    procedure PopulateCalendarList(const AList: TdxWebServiceCalendarList);
      override; final;
    procedure UpdateOccurrences(AEvent: TcxSchedulerWebServiceEvent);
      override; final;

    class function IsItemListValid(AList: TdxJSONObject): Boolean;
      override; final;
    ...
    procedure AddException(const APattern: TcxSchedulerWebServiceEvent;
      ACalculator: TcxSchedulerRecurrenceCalculator);

    procedure DismissEvent(AEvent: TcxSchedulerWebServiceEvent;
      ADate: TDateTime); override;
...
{$ENDREGION}
  ...
  published
    property AuthorizationAgent: TdxMicrosoftGraphAPIOAuth2AuthorizationAgent
...
type
  ...
  public
    class function FindName<T>(ADictionary: TDictionary<string, T>;
      const Value: T): string; static;
  end;

  { TdxNamedOrdinalDictionaryHelper }

class function TdxNamedOrdinalDictionaryHelper.FindName<T>
  (ADictionary: TDictionary<string, T>; const Value: T): string;
var
  AResult: string;
begin
...
procedure TdxMicrosoftOfficeCalendarEvent.AssignFrom(const AObject
  : TdxJSONValue);
  ...
  begin
    ...
    if not FRecurrencePatternTypeDictionary.TryGetValue
      (ARecurrence.GetChildParamValue('pattern', 'type'), Result.Pattern.&Type)
    then
      Result.Pattern.&Type := TRecurrencePatternType.Daily;
    if not TryStrToInt(ARecurrence.GetChildParamValue('pattern', 'interval'),
      Result.Pattern.Interval) then
      Result.Pattern.Interval := -1;
    if not TryStrToInt(ARecurrence.GetChildParamValue('pattern', 'month'),
      Result.Pattern.Month) then
      Result.Pattern.Month := -1;
    if not TryStrToInt(ARecurrence.GetChildParamValue('pattern', 'dayOfMonth'),
      Result.Pattern.DayOfMonth) then
      Result.Pattern.DayOfMonth := -1;
    Result.Pattern.DaysOfWeek := [];
    if ARecurrence.GetChild('pattern', 'daysOfWeek').IsArray then
    begin
      ADaysOfWeek := TdxJSONArray(ARecurrence.GetChild('pattern',
        'daysOfWeek'));
    ...
    if not FDayDictionary.TryGetValue(ARecurrence.GetChildParamValue('pattern',
      'firstDayOfWeek'), Result.Pattern.FirstDayOfWeek) then
      Result.Pattern.FirstDayOfWeek := dxGetStartOfWeek;
    if not FRecurrencePatternIndexDictionary.TryGetValue
      (ARecurrence.GetChildParamValue('pattern', 'index'), Result.Pattern.Index)
    then
      Result.Pattern.Index := 0;

    if not FRecurrenceRangeTypeDictionary.TryGetValue
      (ARecurrence.GetChildParamValue('range', 'type'), Result.Range.&Type) then
    ...
    Result.Range.RecurrenceTimeZone := ARecurrence.GetChildParamValue('range',
      'recurrenceTimeZone');
    if not TryStrToInt(ARecurrence.GetChildParamValue('range',
      'numberOfOccurrences'), Result.Range.NumberOfOccurrences) then
...
begin
  if not FEventTypeDictionary.TryGetValue(AObject.GetParamValue('type'), FType)
  then
    FType := etNone;
  ...
  if TdxStringHelper.EndsWith(FDescription, #$D#$A) then
    FDescription := TdxStringHelper.Remove(FDescription,
      Length(FDescription) - 2);
  FIsRecurring := (AObject.GetParamValue('type') = 'seriesMaster') and
    AObject.HasParam('recurrence');
  ...
  begin
    ...
    if TryStrToInt(AObject.GetParamValue('reminderMinutesBeforeStart'),
      AReminderMinutesBeforeStart) then
  ...
  else if FType in [etOccurrence, etCustom, etException] then
  begin
...
class function TdxMicrosoftOfficeCalendarEvent.GetOccurrenceID
  (const AEvent: IdxWebServiceEvent): string;
begin
  if AEvent.&Type in [etOccurrence, etException] then
    Result := Format('%s-%s', [AEvent.GetPatternID,
      TdxISO8601Helper.DateTimeToString(dxDateOf(AEvent.StartDate))])
  else
    Result := AEvent.ID;
end;
...
function TdxMicrosoftOfficeCalendarEvent.CreateJSONObject: TdxJSONObject;
  ...
  begin
    Result := TdxNamedOrdinalDictionaryHelper.FindName<TDay>
      (FDayDictionary, ADay);
  end;
  ...
  begin
    ...
    Result.AddPair('type',
      TdxNamedOrdinalDictionaryHelper.FindName<TRecurrencePatternType>
      (FRecurrencePatternTypeDictionary, FPatternedRecurrence.Pattern.&Type));
    Result.AddPair('firstDayOfWeek',
      DayToString(FPatternedRecurrence.Pattern.FirstDayOfWeek));
    Result.AddPair('interval', IntToStr(FPatternedRecurrence.Pattern.Interval));
    Result.AddPair('index', TdxNamedOrdinalDictionaryHelper.FindName<Integer>
      (FRecurrencePatternIndexDictionary, FPatternedRecurrence.Pattern.Index));
    case FPatternedRecurrence.Pattern.&Type of
      TRecurrencePatternType.AbsoluteYearly:
        begin
          ...
          Result.AddPair('dayOfMonth',
            IntToStr(FPatternedRecurrence.Pattern.DayOfMonth));
        end;
  ...
  begin
    Result := TdxJSONObject.Create;
    Result.AddPair('type',
      TdxNamedOrdinalDictionaryHelper.FindName<TRecurrenceRangeType>
      (FRecurrenceRangeTypeDictionary, FPatternedRecurrence.Range.&Type));
    Result.AddPair('startDate', TdxISO8601Helper.DateTimeToString
      (FPatternedRecurrence.Range.StartDate));
    ...
    case FPatternedRecurrence.Range.&Type of
      TRecurrenceRangeType.EndDate:
        Result.AddPair('endDate', TdxISO8601Helper.DateTimeToString
          (FPatternedRecurrence.Range.EndDate));
      TRecurrenceRangeType.Numbered:
        Result.AddPair('numberOfOccurrences',
          IntToStr(FPatternedRecurrence.Range.NumberOfOccurrences));
    end;
  end;
...
begin
  ...
  Result.AddPair('location',
    TdxJSONObject.Create(TdxJSONPair.Create('displayName', Location)));
  Result.AddPair('type', TdxNamedOrdinalDictionaryHelper.FindName<TcxEventType>
    (FEventTypeDictionary, &Type));
  ...
  Result.AddPair('IsReminderOn', TdxJSONValue.CreateBooleanValue
    (FIsReminderOn));
  if Length(FReminders) >= 1 then
    Result.AddPair('reminderMinutesBeforeStart', IntToStr(FReminders[0]));
end;

procedure TdxMicrosoftOfficeCalendarEvent.DoAssignFrom
  ...
  begin
    case AEvent.RecurrenceInfo.DayType of
      cxdtEveryDay:
        Result := [dMonday, dTuesday, dWednesday, dThursday, dFriday, dSunday,
          dSaturday];
      cxdtWeekDay:
        Result := [dMonday, dTuesday, dWednesday, dThursday, dFriday];
      cxdtWeekEndDay:
        Result := [dSunday, dSaturday];
      cxdtSunday:
        Result := [dSunday];
      cxdtMonday:
        Result := [dMonday];
...
begin
  ...
  if &Type = etPattern then
  begin
    FPatternedRecurrence.Pattern.FirstDayOfWeek :=
    ...
    begin
        ...
        cxreMonthly:
          begin
            if DayType = cxdtDay then
              FPatternedRecurrence.Pattern.&Type :=
                TRecurrencePatternType.AbsoluteMonthly
            else
              FPatternedRecurrence.Pattern.&Type :=
                TRecurrencePatternType.RelativeMonthly;
      ...
      else if FPatternedRecurrence.Range.EndDate > FPatternedRecurrence.Range.StartDate
      then
...
procedure TdxMicrosoftOfficeCalendarEvent.DoAssignTo
  (AEvent: TcxSchedulerWebServiceEvent);
const
  ARecurrenceMap: array [TRecurrencePatternType] of TcxRecurrence = (cxreYearly,
    cxreYearly, cxreMonthly, cxreMonthly, cxreWeekly, cxreDaily);

  procedure SetRelativeRecurrence;
  begin
    with AEvent.RecurrenceInfo do
    begin
      if FPatternedRecurrence.Pattern.&Type
        in [TRecurrencePatternType.AbsoluteMonthly,
        TRecurrencePatternType.AbsoluteYearly] then
        DayType := cxdtDay
      else
      begin
        DayNumber := FPatternedRecurrence.Pattern.Index + 1;
        if FPatternedRecurrence.Pattern.DaysOfWeek = [dMonday, dTuesday,
          dWednesday, dThursday, dFriday, dSunday, dSaturday] then
          DayType := cxdtEveryDay
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dSunday, dSaturday]
        then
          DayType := cxdtWeekEndDay
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dMonday, dTuesday,
          dWednesday, dThursday, dFriday] then
          DayType := cxdtWeekDay
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dMonday] then
          DayType := cxdtMonday
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dTuesday] then
          DayType := cxdtTuesday
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dWednesday] then
          DayType := cxdtWednesday
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dThursday] then
          DayType := cxdtThursday
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dFriday] then
          DayType := cxdtFriday
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dSaturday] then
          DayType := cxdtSaturday
        else if FPatternedRecurrence.Pattern.DaysOfWeek = [dSunday] then
          DayType := cxdtSunday;
      end;
      OccurDays := [];
    end;
...
begin
  ...
  begin
    with AEvent.RecurrenceInfo do
    begin
      Start := AEvent.Start + dxDateOf(FPatternedRecurrence.Range.StartDate) -
        dxDateOf(AEvent.Start);
      ...
      DisplayTimeBias := GetTimeZoneBias(OriginalStart,
        FPatternedRecurrence.Range.RecurrenceTimeZone);
...
function TdxMicrosoftOfficeCalendarEvent.GetProvider
  : TcxSchedulerWebServiceStorageOfficeProvider;
begin
...
begin
  ...
  for I := 0 to TdxTimeZoneHelper.TimeZoneCount - 1 do
    if CompareText(TdxTimeZoneHelper.TimeZoneInfo(I).StandardName, AName) = 0
    then
    begin
...
class function TdxMicrosoftOfficeCalendarEvent.DateTimeToJSON(AValue: TDateTime)
  : TdxJSONObject;
var
...
class function TdxMicrosoftOfficeCalendarEvent.JSONToDateTime
  (AJSONValue: TdxJSONValue; out AIsUTC: Boolean): TDateTime;
var
  AValue: string;
  ATimeZone: string;
...
begin
  ...
  if not AIsUTC and (dxDateOf(Result) <> Result) and
    AJSONValue.HasParam('timeZone') then
  begin
...
begin
  ...
  FRecurrencePatternTypeDictionary :=
    TDictionary<string, TRecurrencePatternType>.Create;
  FRecurrencePatternTypeDictionary.Add('absoluteYearly',
    TRecurrencePatternType.AbsoluteYearly);
  FRecurrencePatternTypeDictionary.Add('relativeYearly',
    TRecurrencePatternType.RelativeYearly);
  FRecurrencePatternTypeDictionary.Add('absoluteMonthly',
    TRecurrencePatternType.AbsoluteMonthly);
  FRecurrencePatternTypeDictionary.Add('relativeMonthly',
    TRecurrencePatternType.RelativeMonthly);
  ...
  FRecurrenceRangeTypeDictionary :=
    TDictionary<string, TRecurrenceRangeType>.Create;
...
function TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.CanAddEvent
  (const AEvent: IdxWebServiceEvent): Boolean;
begin
  Result := (TdxMicrosoftOfficeCalendarEvent(AEvent).&Type <> etOccurrence) and
    inherited CanAddEvent(AEvent);
end;

procedure TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.
  DoApplyChanges;
var
...
function TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.
  GetJSONItemList: TdxJSONObject;
begin
  if FNextLink = '' then
...
const
  AContentTypeMap: array
    [TcxSchedulerWebServiceStorageOfficeProvider.TContentType]
    of string = ('Prefer: outlook.body-content-type="text"',
    'Prefer: outlook.body-content-type="text"',
    'Prefer: outlook.body-content-type="HTML"');
begin
  inherited Initialize;
  Header := Format('%s'#13#10'%s',
    [Header, AContentTypeMap[Provider.ContentType]]);
end;

procedure TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.
  ProcessItemListValues(AList: TdxJSONObject);
begin
...
function TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.
  GetJSONItemListUri: string;
begin
  Result := Format(FGetEventListEndPoint, [CalendarID])
end;

function TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.GetProvider
  : TcxSchedulerWebServiceStorageOfficeProvider;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProvider(inherited Provider);
end;
...
constructor TcxSchedulerWebServiceStorageOfficeProviderDismissReminderTask.
  Create(AEvent: TcxSchedulerWebServiceEvent; ADate: TDateTime);
begin
  inherited Create(AEvent);
  FDate := ADate;
end;

procedure TcxSchedulerWebServiceStorageOfficeProviderDismissReminderTask.
  DoApplyChanges;
begin
  // do nothing
end;

function TcxSchedulerWebServiceStorageOfficeProviderDismissReminderTask.
  ExpectResponse: Boolean;
begin
  Result := False;
end;

function TcxSchedulerWebServiceStorageOfficeProviderDismissReminderTask.Process
  : TdxJSONObject;
var
  AEventID: string;
begin
  if Event.EventType = etPattern then
    AEventID := TcxSchedulerWebServiceStorageOfficeProviderPostEventTask.
      GetOccurrenceCloudID(UserAgent, Header, Event.CloudID, FDate)
  else
  ...
  Result := TdxHttpHelper.PostRequest(UserAgent,
    ...
    Format(FDismissEventObjectName, [AEventID]), Header, nil);
end;

{ TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask }

constructor TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
  Create(AProvider: TcxSchedulerWebServiceStorageCustomProvider;
  const ASeriesMasterID: string);
begin
...
procedure TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
  DoApplyChanges;
begin
  // do nothing
end;

function TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
  DoExecute: TdxTaskCompletedStatus;
const
  ATimeRangeMap: array [TcxRecurrence] of Integer = (365, 365 * 4,
    365 * 5, 365 * 5);

  procedure RunTask(AEvent: TcxSchedulerWebServiceEvent;
    AStart, AFinish: TDateTime);
  begin
    MainThreadSynchronize(procedure()
      begin
        dxTasksDispatcher.Run(CreateSyncOccurrencesTask(AEvent, AStart,
          AFinish));
      end);
  end;
...
begin
  ...
  if not CanProcessing then
    Exit(TdxTaskCompletedStatus.Fail);
  ACalculator := TcxSchedulerEventRecurrenceInfoAccess(AEvent.RecurrenceInfo)
    .GetCalculatorClass.Create(AEvent, AMinDate, AMaxDate)
    as TcxSchedulerOccurrenceCalculator;
  try
    AStart := AMinDate;
    ...
    begin
      if dxDateOf(ACalculator.OccurrenceFinish) > AMaxDate then
        Break;
      if dxDateOf(ACalculator.OccurrenceFinish) > dxDateOf(AStart + ATimeRange)
      then
      begin
    ...
    RunTask(AEvent, AStart, Max(dxDateOf(ACalculator.OccurrenceFinish),
      AMaxDate) + 1);
  finally
    ACalculator.Free;
  end;
...
function TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
  CreateSyncOccurrencesTask(AEvent: TcxSchedulerWebServiceEvent;
AStart, AFinish: TDateTime)
  : TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.
    Create(Provider, AEvent, AStart, AFinish);
end;

function TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
  GetProvider: TcxSchedulerWebServiceStorageOfficeProvider;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProvider(inherited Provider);
end;
...
constructor TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.
  Create(AProvider: TcxSchedulerWebServiceStorageCustomProvider;
const ASeriesMaster: TcxSchedulerWebServiceEvent;
...
destructor
  TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.Destroy;
begin
  cxRemoveObjectLink(FLink);
  inherited Destroy;
end;

function TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.
  CanAddEvent(const AEvent: IdxWebServiceEvent): Boolean;
begin
...
procedure TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.
  DoApplyChanges;
var
  AList: TdxWebServiceEventList;
...
begin
  ...
  Result := Format(FGetEventCustomOccurrenceListEndPoint,
    [FSeriesMaster.CloudID, AStart, AFinish]);
end;

procedure TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.
  InternalDoApplyChanges(AList: TdxWebServiceEventList);
...
begin
  ...
  try
    ACalculator := TcxSchedulerEventRecurrenceInfoAccess
      (SeriesMaster.RecurrenceInfo).GetCalculatorClass.Create(SeriesMaster,
      StartDate, FinishDate) as TcxSchedulerOccurrenceCalculator;
    try
      ...
      begin
        ...
        begin
          ...
          begin
            if dxDateOf(AEvent.StartDate) = dxDateOf(ACalculator.OccurrenceStart)
            then
              AList.Delete(0)
            else
            begin
...
function TcxSchedulerWebServiceStorageOfficeProviderSyncOccurrencesTask.
  HasCustomEvent(ADate: TDateTime;
out AEvent: TcxSchedulerWebServiceEvent): Boolean;
var
...
function TcxSchedulerWebServiceStorageOfficeProviderModifyEventTask.
  ExpectResponse: Boolean;
begin
  Result := WebServiceEvent.&Type <> etException;
end;

function TcxSchedulerWebServiceStorageOfficeProviderModifyEventTask.Process
  : TdxJSONObject;
var
...
begin
  ...
  try
    Result := TdxHttpHelper.PatchRequest(UserAgent,
      TcxSchedulerWebServiceStorageOfficeProvider.EndPointServerName,
      Format(TcxSchedulerWebServiceStorageOfficeProvider.UpdateEventObjectName,
      [WebServiceEvent.ID]), Header, AObject);
  finally
    AObject.Free;
  end;
end;
...
procedure TcxSchedulerWebServiceStorageOfficeProviderDeleteEventTask.
  DoApplyChanges;
begin
...
function TcxSchedulerWebServiceStorageOfficeProviderDeleteEventTask.Process
  : TdxJSONObject;
begin
  if WebServiceEvent.GetID = '' then
    Exit(nil);
  Result := TdxHttpHelper.DeleteRequest(UserAgent,
    TcxSchedulerWebServiceStorageOfficeProvider.EndPointServerName,
    Format(TcxSchedulerWebServiceStorageOfficeProvider.UpdateEventObjectName,
    [WebServiceEvent.ID]), Header, nil);
end;

{ TcxSchedulerWebServiceStorageOfficeProviderPostEventTask }

function TcxSchedulerWebServiceStorageOfficeProviderPostEventTask.
  GetOccurrenceCloudID(const AEvent: IdxWebServiceEvent): string;
begin
  Result := GetOccurrenceCloudID(UserAgent, Header, AEvent.GetPatternID,
    AEvent.GetOriginalOccurrenceStartTime);
end;

class function TcxSchedulerWebServiceStorageOfficeProviderPostEventTask.
  GetOccurrenceCloudID(const AUserAgent, AHeader, ASeriesMasterID: string;
AOccurrenceDate: TDateTime): string;
var
...
begin
  ...
  AGetOccurrencesEndPoint := Format(FGetEventCustomOccurrenceListEndPoint,
    [ASeriesMasterID, TdxISO8601Helper.DateTimeToString(ADate),
    TdxISO8601Helper.DateTimeToString(ADate + 1)]);
  AObject := TdxHttpHelper.GetRequest(AUserAgent,
    AGetOccurrencesEndPoint, AHeader);
  if Assigned(AObject) then
    try
      if AObject.HasParam('value') and AObject.GetPair('value').JsonValue.IsArray
      then
      begin
...
procedure TcxSchedulerWebServiceStorageOfficeProviderPostEventTask.
  DoApplyChanges;
begin
...
function TcxSchedulerWebServiceStorageOfficeProviderPostEventTask.Process
  : TdxJSONObject;
var
  AObject: TdxJSONObject;
...
begin
  ...
  begin
    ...
    try
      Result := TdxHttpHelper.PostRequest(UserAgent,
        TcxSchedulerWebServiceStorageOfficeProvider.EndPointServerName,
        Format(FPostEventObjectName, [CalendarID]), Header, AObject);
    finally
      AObject.Free;
    end;
  end;
...
procedure TcxSchedulerWebServiceStorageOfficeProvider.Assign
  (Source: TPersistent);
begin
  if Source is TcxSchedulerWebServiceStorageOfficeProvider then
    FContentType := TcxSchedulerWebServiceStorageOfficeProvider(Source)
      .ContentType;
  inherited Assign(Source);
end;

constructor TcxSchedulerWebServiceStorageOfficeProvider.Create
...
function TcxSchedulerWebServiceStorageOfficeProvider.CreateDeleteEventTask
  (AEvent: TcxSchedulerWebServiceEvent;
const AWebServiceEvent: IdxWebServiceEvent)
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderDeleteEventTask.Create
    (AEvent, AWebServiceEvent);
end;

function TcxSchedulerWebServiceStorageOfficeProvider.CreateDismissEventTask
  (AEvent: TcxSchedulerWebServiceEvent; ADate: TDateTime)
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderDismissReminderTask.
    Create(AEvent, ADate);
end;
...
begin
  if AEvent.EventType = etException then
    ...
    Result := TcxSchedulerWebServiceStorageOfficeProviderModifyEventTask.Create
      (AEvent, AWebServiceEvent);
end;
...
function TcxSchedulerWebServiceStorageOfficeProvider.CreatePostEventTask
  ...
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderPostEventTask.
    Create(AEvent);
end;

function TcxSchedulerWebServiceStorageOfficeProvider.CreateSyncEventsTask
  : TcxSchedulerWebServiceStorageProviderSyncEventsCustomTask;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderSyncEventsTask.
    Create(Self);
end;

function TcxSchedulerWebServiceStorageOfficeProvider.CreateSyncSeriesMasterTask
  (const ASeriesMasterID: string)
  : TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask;
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
    Create(Self, ASeriesMasterID);
end;
...
begin
  Result := TcxSchedulerWebServiceStorageOfficeProviderSyncSeriesMasterTask.
    Create(Self, ASeriesMasterID, AStart, AFinish);
end;

function TcxSchedulerWebServiceStorageOfficeProvider.CreateUpdateEventTask
  (AEvent: TcxSchedulerWebServiceEvent)
  : TcxSchedulerWebServiceStorageProviderCustomTask;
begin
  Result := nil;
...
procedure TcxSchedulerWebServiceStorageOfficeProvider.PopulateCalendarList
...
procedure TcxSchedulerWebServiceStorageOfficeProvider.UpdateOccurrences
  (AEvent: TcxSchedulerWebServiceEvent);
var
...
class function TcxSchedulerWebServiceStorageOfficeProvider.IsItemListValid
  (AList: TdxJSONObject): Boolean;
begin
  Result := AList.HasParam('@odata.context');
end;

class function TcxSchedulerWebServiceStorageOfficeProvider.IsItemValid
  (AItem: TdxJSONValue): Boolean;
begin
  Result := AItem.HasParam('@odata.etag') and AItem.HasParam('id');
end;

class function TcxSchedulerWebServiceStorageOfficeProvider.
  GetJSONItemArrayParamName: string;
begin
  Result := 'value';
end;

procedure TcxSchedulerWebServiceStorageOfficeProvider.AddException
  (const APattern: TcxSchedulerWebServiceEvent;
ACalculator: TcxSchedulerRecurrenceCalculator);
var
  AEvent: IdxWebServiceEvent;
begin
...
procedure TcxSchedulerWebServiceStorageOfficeProvider.DismissEvent
  (AEvent: TcxSchedulerWebServiceEvent; ADate: TDateTime);
begin
...
function TcxSchedulerWebServiceStorageOfficeProvider.GetAuthorizationAgent
  : TdxMicrosoftGraphAPIOAuth2AuthorizationAgent;
begin
  Result := TdxMicrosoftGraphAPIOAuth2AuthorizationAgent
    (inherited AuthorizationAgent);
end;

function TcxSchedulerWebServiceStorageOfficeProvider.GetWebServiceEventClass
  : TdxWebServiceEventClass;
begin
  Result := TdxMicrosoftOfficeCalendarEvent;
end;

procedure TcxSchedulerWebServiceStorageOfficeProvider.
  OccurrencesCheckedTimeBoundsChanged(AStart, AFinish: TDateTime);
  ...
  begin
    for I := 0 to Owner.EventCount - 1 do
      if (Owner.Events[I].EventType = etPattern) and
        (Owner.Events[I].ResourceID = CalendarID) then
        AddTask(CreateSyncSeriesMasterTask(TcxSchedulerWebServiceEvent
          (Owner.Events[I]).CloudID, AStart, AFinish));
  end;
