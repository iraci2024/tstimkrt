...
uses
  ...
  dxCore, dxCoreClasses, cxScrollBar, cxLookAndFeelPainters,
...
type
  ...
  TcxSchedulerGanttViewEventLayoutBuilder = class
  ...
  private
    ...
    function GetArrowDirection(const AArrowStart, AArrowFinish: TPoint)
      : TcxArrowDirection;
    function GetIsHot: Boolean;
    function GetRelation: TcxSchedulerEventRelation;
    procedure InitializeFinishParams(var AStartRect, AFinishRect: TRect;
      out P1, P2: TPoint);
  ...
  protected
    ...
    procedure CalculateArrowRect(const AArrowStart, AArrowFinish
      : TPoint); virtual;
    ...
    property FinishEventViewInfo: TcxSchedulerEventCellViewInfo
      read FFinishEventViewInfo;
    property StartEventViewInfo: TcxSchedulerEventCellViewInfo
      read FStartEventViewInfo;
  public
  ...
  TcxSchedulerGanttViewExpandButtonViewInfo = class
  ...
  private
    ...
    function GetVisibleEventEnumeration(AIndex: Integer)
  ...
  protected
    ...
    function CreateEventViewData(AEvent: TcxSchedulerControlEvent;
      ...
      : TcxSchedulerEventViewData; override;
    function CreateLayoutBuilder: TcxSchedulerEventLayoutBuilder; override;
    function CreateLinkItemViewInfo(AEventViewInfo
    ...
    function FindEventViewInfoBySource(AEvent: TcxSchedulerEvent;
      ...
      var AViewInfo: TcxSchedulerEventCellViewInfo): Boolean;
    function GetEventBounds(AEvent: TcxSchedulerControlEvent;
      AResource: TcxSchedulerStorageResourceItem;
      out AEventRect: TRect): Boolean;
  ...
  public
    ...
    property HotLink: TcxSchedulerGanttViewLinkViewInfo read FHotLink
      write SetHotLink;
    property Links: TcxSchedulerViewInfoCellList read FLinks;
    property LinkViewParams: TcxViewParams read GetLinkViewParams;
    property TotalProgressViewInfo: TcxSchedulerGanttViewTotalProgressViewInfo
      read FTotalProgressViewInfo;
    property View: TcxSchedulerGanttView read GetGanttView;
    property VisibleEventEnumerationList[AIndex: Integer]
      : TcxSchedulerResourceViewInfo read GetVisibleEventEnumeration;
  end;
  ...
  private
    ...
    procedure DoDrawExpandButton(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean);
    procedure DoDrawLinkItem(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean);
  public
  ...
  protected
    ...
    function CanResizeEvent(AEvent: TcxSchedulerControlEvent): Boolean;
      override;

    property View: TcxSchedulerGanttView read GetView;
    property HitExpandButton: TcxSchedulerGanttViewExpandButtonViewInfo
      read GetHitExpandButton;
  ...
  public
    ...
    property DragGroupedTaskLinks: Boolean read FDragGroupedTaskLinks
      write FDragGroupedTaskLinks;
  end;
  ...
  protected
    ...
    procedure InternalApplyTaskLink(AEvent: TcxSchedulerControlEvent;
      ALinkedEvent: TcxSchedulerEvent); virtual;
  ...
  protected
    ...
    property EventDragMode: TcxSchedulerEventDragMode read FEventDragMode
      write SetEventDragMode;
    property HitTest: TcxSchedulerGanttViewHitTest read GetHitTest;
    property View: TcxSchedulerGanttView read GetView;
    property ViewInfo: TcxSchedulerGanttViewViewInfo read GetViewInfo;
  end;
  ...
  TcxSchedulerShowTaskDependencyEditorEvent = procedure
    (Sender: TcxCustomScheduler; ALink: TcxSchedulerEventItemLink;
  ...
  protected
    ...
    function DoShowTaskDependencyEditor(ALink: TcxSchedulerEventItemLink)
      : Boolean; virtual;
    function DoStartLinkEvent(AEvent: TcxSchedulerControlEvent)
      : Boolean; virtual;
    ...
    procedure ShowTaskDependencyEditor
      (ALink: TcxSchedulerEventItemLink); virtual;
    procedure ValidateContentPopupMenuItems(var AItems
      : TcxSchedulerContentPopupMenuItems); override;
  ...
  published
    ...
    property EventsStyle: TcxSchedulerGanttViewEventStyle read FEventsStyle
      write SetEventsStyle default esDefault;
    property LinkLineColor: TColor read FLinkLineColor write SetLinkLineColor
      default clNavy;
    property ShowExpandButtons: Boolean read FShowExpandButtons
      write SetShowExpandButtons default False;
    property ShowTotalProgressLine: Boolean read FShowTotalProgressLine
      write SetShowTotalProgressLine default False;
    property TotalProgressLineColor: TColor read FTotalProgressLineColor
      write SetTotalProgressLineColor default clRed;
    property OnEndLinkEvent: TcxSchedulerEndLinkEventEvent read FOnEndLinkEvent
      write FOnEndLinkEvent;
    property OnShowTaskDependencyEditor
      : TcxSchedulerShowTaskDependencyEditorEvent
...
type
  ...
  TcxSchedulerTimeGridResourceScrollAccess = class
  ...
  TcxSchedulerGanttViewEventPlaceComparer = class(TInterfacedObject,
...
function cxGanttViewCompareEventPlaces(AEventPlace1: TcxSchedulerEventPlace;
  ...
  function CheckParentGroup(AParentEvent, AChildrenEvent
    : TcxSchedulerEvent): Boolean;
  begin
    Result := TcxSchedulerEventAccess(AParentEvent).IsParentGroupForEvent
      (AChildrenEvent);
  end;

  function CompareByParentGroups(AEvent1,
    AEvent2: TcxSchedulerControlEvent): Integer;
  begin
    if CheckParentGroup(AEvent1, AEvent2) then
      Result := -1
    else if CheckParentGroup(AEvent2, AEvent1) then
      Result := 1
    else
      Result := 0;
  end;
  ...
  begin
    if AEvent1.IsClone and (AEvent1.Source = AEvent2) then
      ASign := 1
    else if AEvent2.IsClone and (AEvent2.Source = AEvent1) then
      ASign := -1
    else
      ASign := 0;
    Result := ASign <> 0;
...
begin
  ...
  begin
    ...
    else if not CheckClone(AEvent1, AEvent2, Result) and (ATaskIndex1 = -1) then
...
function TcxSchedulerGanttViewEventPlaceComparer.Compare(const Left,
...
function TcxSchedulerGanttView.CanEventEdit(AEvent: TcxSchedulerControlEvent;
  AInplace: Boolean): Boolean;
begin
  Result := inherited CanEventEdit(AEvent, AInplace) and
    (not AInplace or not(ViewInfo.IsMilestone(AEvent) or
    ViewInfo.IsGroup(AEvent)));
end;
...
function TcxSchedulerGanttView.CreateController
...
function TcxSchedulerGanttView.DoEndLinkEvent
  (ALink: TcxSchedulerControlEvent): Boolean;
begin
  Result := True;
...
function TcxSchedulerGanttView.DoStartLinkEvent
...
begin
  Result := Format(cxGetResourceString(@scxGanttEventHint),
    [AEvent.Caption, AEvent.TaskComplete, cxDateToStr(AEvent.Start),
    cxDateToStr(AEvent.Finish)]);
end;
...
procedure TcxSchedulerGanttView.SetEventsStyle
  (AValue: TcxSchedulerGanttViewEventStyle);
begin
...
procedure TcxSchedulerGanttViewLinkViewInfo.BeforeCustomDraw
  (ACanvas: TcxCanvas);
begin
  // do nothing
end;

procedure TcxSchedulerGanttViewLinkViewInfo.Calculate(AStartRect,
  AFinishRect: TRect);
  ...
  procedure InitializePoints(var AStartPoint, AFinishPoint: TPoint;
    var ALineIndex: Integer);
  var
  ...
  case ArrowDirection of
    adUp:
      FArrowRect := Rect(AArrowStart.X - ScaleFactor.Apply(LinkLineOffset - 1),
        AArrowFinish.Y, AArrowStart.X + ScaleFactor.Apply(LinkLineOffset),
        AArrowStart.Y - ScaleFactor.Apply(LinkLineOffset));
    adDown:
      FArrowRect := Rect(AArrowStart.X - ScaleFactor.Apply(LinkLineOffset - 1),
        AArrowStart.Y + ScaleFactor.Apply(LinkLineOffset),
        AArrowStart.X + ScaleFactor.Apply(LinkLineOffset), AArrowFinish.Y);
    adLeft:
      FArrowRect := Rect(AArrowFinish.X,
        AArrowStart.Y - ScaleFactor.Apply(LinkLineOffset),
        AArrowStart.X - ScaleFactor.Apply(LinkLineOffset),
        AArrowFinish.Y + ScaleFactor.Apply(LinkLineOffset));
    adRight:
      FArrowRect := Rect(AArrowStart.X + ScaleFactor.Apply(LinkLineOffset),
        AArrowFinish.Y - ScaleFactor.Apply(LinkLineOffset), AArrowFinish.X,
        AArrowFinish.Y + ScaleFactor.Apply(LinkLineOffset));
  end;
end;
...
procedure TcxSchedulerGanttViewLinkViewInfo.DoRightToLeftConversion
...
begin
  ...
  FArrowRect := TdxRightToLeftLayoutConverter.ConvertRect(FArrowRect,
    AClientBounds);
  FArrowDirection := TdxRightToLeftLayoutConverter.ConvertArrowDirection
    (FArrowDirection);
  for I := 0 to High(FLines) do
    FLines[I] := TdxRightToLeftLayoutConverter.ConvertRect(FLines[I],
      AClientBounds);
end;

function TcxSchedulerGanttViewLinkViewInfo.GetArrowDirection(const AArrowStart,
  AArrowFinish: TPoint): TcxArrowDirection;
begin
  if AArrowStart.X = AArrowFinish.X then
    if AArrowStart.Y > AArrowFinish.Y then
      ...
      Result := adDown
  else if AArrowStart.X > AArrowFinish.X then
    Result := adLeft
  else
    Result := adRight;
end;
...
procedure TcxSchedulerGanttViewLinkViewInfo.InitializeFinishParams
  ...
  procedure GetFinishToStartRelationParams(var R: TRect; out P1, P2: TPoint;
    ATargetX: Integer);
  begin
    ATargetX := Max(ATargetX, AStartRect.Right +
      ScaleFactor.Apply(LinkLineOffset));
    if ATargetX >= R.Right then
      GetToStartRelationParams(R, P1, P2)
    else if AStartRect.Top >= R.Top then
    begin
  ...
  case Relation of
    ...
    trFinishToStart:
      if ViewInfo.IsMilestone(FinishEventViewInfo.Event) then
        GetFinishToStartRelationParams(AFinishRect, P1, P2,
          cxRectCenter(AFinishRect).X)
      else
...
begin
  ...
  InflateRect(AStartRect, ScaleFactor.Apply(LinkLineOffset),
    ScaleFactor.Apply(LinkLineOffset));
  P2 := GetStartPoint(AStartRect);
end;

function TcxSchedulerGanttViewLinkViewInfo.GetRelation
...
begin
  ...
  try
    ...
    cxLookAndFeelPaintersManager.GetPainter(lfsStandard)
      .CalculateArrowPoints(CheckRect(ArrowRect), AArrowPoints, ArrowDirection,
      True, ScaleFactor.Apply(LinkLineOffset) + Byte(IsHot) * 2);
...
function TcxSchedulerGanttViewPainter.GetViewInfo
  : TcxSchedulerGanttViewViewInfo;
begin
  Result := TcxSchedulerGanttViewViewInfo(inherited ViewInfo);
end;
...
function TcxSchedulerGanttViewHitTest.CanResizeEvent
  (AEvent: TcxSchedulerControlEvent): Boolean;
begin
...
function TcxSchedulerGanttViewHitTest.GetHitAtEventDraggingArea: Boolean;
  ...
  begin
    ...
    if (View.Controller.DragEventHelper = nil) or (ADragEvent = nil) or
      (ADragEvent.Source = nil) or (View = nil) or (View.ViewInfo = nil) then
      Exit;
...
function TcxSchedulerGanttViewHitTest.GetHitExpandButton
  : TcxSchedulerGanttViewExpandButtonViewInfo;
begin
...
function TcxSchedulerGanttViewHitTest.GetHitLink
  : TcxSchedulerGanttViewLinkViewInfo;
begin
...
function TcxSchedulerGanttDragObject.GetDragCursor(Accepted: Boolean;
  X, Y: Integer): TCursor;
const
...
function TcxSchedulerGanttDragObject.GetController
  : TcxSchedulerGanttViewController;
begin
...
function CreateCustom(APattern: TcxSchedulerEvent; ADate: TDateTime)
  : TcxSchedulerEvent;
begin
  ...
  try
    ...
    begin
      ...
      for I := 0 to Clones.Count - 1 do
        InternalApplyTaskLink
          (TcxSchedulerControlEvent(TcxSchedulerControlEvent(Clones[I]).Source),
          ALinkedEvent);
    end;
  finally
    Events.CancelClones;
...
procedure TcxGanttDragEventHelper.DragOver(const P: TPoint; State: TDragState;
...
begin
  ...
  begin
    ...
    case Destination of
      ...
      dodDateNavigator:
        ADeltaTime := Controller.StartDragHitTime -
          TcxSchedulerCustomDateNavigatorAccess(DateNavigator).HitTest.Time;
    else
      ADeltaTime := MinDateTime;
    end;
    Accepted := (Accepted or HitTest.HitAtExpandButton or HitTest.HitAtLink) and
      CanDragTo(ADeltaTime);
  end
  else
    Accepted := Controller.DragEvent.IsResourceEvent
      (Controller.HitTest.Resource, True) and
      ((Controller.HitTest.Event = nil) or
      (not Controller.HitTest.Event.IsClone and
      Controller.DragEvent.Source.CanLink(Controller.HitTest.Event.Source) and
...
procedure TcxGanttDragEventHelper.InternalApplyTaskLink
  (AEvent: TcxSchedulerControlEvent; ALinkedEvent: TcxSchedulerEvent);
var
...
begin
  if AEvent.Source.CanLink(ALinkedEvent) and Controller.View.DoEndLinkEvent
    (FHitEvent) then
  begin
    ADragEvent := AEvent;
  ...
  begin
    ...
    begin
      if (ALinksHeaderEvent.TaskLinks[I].Link = nil) or
        (ALinksHeaderEvent.TaskLinks[I].Link.TaskLinkOwners.Count > 1) then
        Continue;
...
var
  I: Integer;
begin
  if not DragObject.DragGroupedTaskLinks then
    Exit;
...
begin
  ...
  if Result then
  begin
    TcxSchedulerControlEvent(Event).GetValidTaskTimeRange(AStartTime,
      AFinishTime);
    Result := (HitTest.Time >= AStartTime) and (HitTest.Time < AFinishTime);
  end;
end;

procedure TcxGanttEventSizing.DragOver(const P: TPoint; State: TDragState;
  var Accepted: Boolean);
begin
  inherited DragOver(P, State, Accepted);
...
procedure TcxSchedulerGanttViewController.CheckTaskLinkDrawOnScroll
...
function TcxSchedulerGanttViewController.CreateDragEventHelper
  : TcxDragEventHelper;
begin
  Result := TcxGanttDragEventHelper.Create(Scheduler);
end;

function TcxSchedulerGanttViewController.CreateResizeEventHelper
  : TcxEventSizingHelper;
begin
  Result := TcxGanttEventSizing.Create(Scheduler);
end;

procedure TcxSchedulerGanttViewController.DragOver(Source: TObject;
...
procedure TcxSchedulerGanttViewController.EndDrag(Target: TObject;
  X, Y: Integer);
begin
...
function TcxSchedulerGanttViewController.GetLinkHintData
  (var AHint: string): Boolean;
begin
...
procedure TcxSchedulerGanttViewController.InitTimer(AllowStart: Boolean;
  AScrollCode: TScrollCode);
begin
...
procedure TcxSchedulerGanttViewController.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  CalcDragEventClipRect;
  FStartDragPoint := cxPointOffset(HitTest.HitPoint,
    GetDragEventClipRect.TopLeft, False);
  if (Button = mbLeft) and HitTest.HitAtExpandButton then
    HitTest.HitExpandButton.DoClick
  else if (Button = mbLeft) and HitTest.HitAtLink then
...
procedure TcxSchedulerGanttViewController.Scrolling(AScrollCode: TScrollCode;
...
begin
  ...
  begin
    ...
    if ViewInfo.ResourceCount = 0 then
      ...
      AResourceIndex := DragEvent.GetResourceItem.
        Index - Scheduler.FirstVisibleResourceIndex;
    if AResourceIndex >= 0 then
    begin
      ALastScrollPos := AScrollPos;
      AScrollPos := -TcxSchedulerTimeGridResourceScrollAccess
        (ViewInfo.ResourceViewShift[AResourceIndex]).FShift + ADelta;
      if (AScrollPos >= 0) and
        (AScrollPos <= ViewInfo.GetResourceScrollBarMax(AResourceIndex)) then
      begin
        ALastScrollPos := ViewInfo.ResourceViewShift[AResourceIndex]
          .ScrollBar.Position;
        ViewInfo.DoResourceVerticalScroll
          (ViewInfo.ResourceViewShift[AResourceIndex].ScrollBar as TObject,
          AScrollCode, AScrollPos);
        if ALastScrollPos <> ViewInfo.ResourceViewShift[AResourceIndex]
          .ScrollBar.Position then
          CheckTaskLinkDrawOnScroll(FTimerScrollBarKind, AScrollCode);
      end
      else
...
var
  AHintText: string;
begin
  Result := GetLinkHintData(AHintText) or
    ((ViewInfo.ViewStyle = svsClassic) and GetEventHintData(AHintText));
  if Result then
  begin
...
procedure TcxSchedulerGanttViewController.StartDrag(var DragObject
  : TDragObject);
begin
...
begin
  if (DragEvent <> nil) and ViewInfo.FindEventViewInfoBySource(DragEvent.Source,
    dxDateOf(DragEvent.Start), HitTest.Resource, AViewInfo) then
    FDragEventClipRect := ViewInfo.GetEventClipRect(AViewInfo)
  else
    FDragEventClipRect := cxEmptyRect;
end;

function TcxSchedulerGanttViewController.GetDragEventHelper
  : TcxGanttDragEventHelper;
begin
  Result := TcxGanttDragEventHelper(inherited DragEventHelper);
end;

function TcxSchedulerGanttViewController.GetEventHintData
  (var AHint: string): Boolean;
var
  AEvent: TcxSchedulerControlEvent;
begin
...
function TcxSchedulerGanttViewController.GetHitTest
  : TcxSchedulerGanttViewHitTest;
begin
  Result := View.HitTest;
end;
...
function TcxSchedulerGanttViewController.GetViewInfo
  : TcxSchedulerGanttViewViewInfo;
begin
  Result := View.ViewInfo;
end;

function TcxSchedulerGanttViewController.InternalPaintDraggingLink(const AStart,
  AFinish: TPoint): Boolean;
begin
...
procedure TcxSchedulerGanttViewController.SetEventDragMode
  (AValue: TcxSchedulerEventDragMode);
var
  I: Integer;
begin
  if AValue = FEventDragMode then
    Exit;
  if AValue = sedmTaskLinkTo then
  begin
    if not View.DoStartLinkEvent(DragEvent) then
      Exit;
...
procedure TcxSchedulerGanttViewEventLayoutBuilder.Calculate;
  ...
  begin
    if not FindStartOfRecurrence(AEventPlace, ACheckEventPlaceCount, APlaceIndex)
    then
...
function TcxSchedulerGanttViewEventLayoutBuilder.CreateComparer
  : IComparer<TcxSchedulerEventPlace>;
begin
  ...
  begin
    ...
    begin
      ...
      if GetUncompletedEventPoint(Rect(ALine.Left, ATop, ALine.Right,
        ATop + ViewInfo.ContentLineHeight), APoint, AResource) then
      begin
        AddPoint(ALine.Left, ATop);
        AddPoint(APoint.X, ATop + ViewInfo.ContentLineHeight div 2);
        FProgressNodes[ANodeIndex] :=
          Point(APoint.X, ATop + ViewInfo.ContentLineHeight div 2);
        Inc(ANodeIndex);
      end;
...
begin
  APointIndex := 0;
  ANodeIndex := 0;
  AVisibleLineCount := (1 + cxRectHeight(ViewInfo.Bounds)
    div ViewInfo.FEventRowHeight);
  ...
  begin
    ...
    for I := 0 to ViewInfo.ResourceCount - 1 do
      AddPointsByResource(TimeLineRect, ViewInfo.ResourceHeaderCells.Items
        [I + 1].Bounds, ViewInfo.Resources[I].ResourceItem);
...
begin
  ...
  begin
    ...
    InflateRect(R, ScaleFactor.Apply(ProgressNodeRadius),
      ScaleFactor.Apply(ProgressNodeRadius));
    ACanvas.Canvas.Ellipse(R);
  end;
end;

function TcxSchedulerGanttViewTotalProgressViewInfo.GetScaleFactor
  : TdxScaleFactor;
begin
  Result := FViewInfo.ScaleFactor;
end;

function TcxSchedulerGanttViewTotalProgressViewInfo.
  FindUncompletedEventBeforeLine(const ALine: TRect;
  AResource: TcxSchedulerStorageResourceItem;
  var AViewInfo: TcxSchedulerEventCellViewInfo): Boolean;
var
...
begin
  ...
  begin
    AEventViewInfo := TcxSchedulerEventCellViewInfo
      (ViewInfo.EventCells.Items[I]);
    R := AEventViewInfo.Bounds;
    if (AResource = nil) or (AResource = AEventViewInfo.ResourceItem) then
      if (R.Left <= ALine.Left) and (R.Top >= ALine.Top) and
        (R.Bottom <= ALine.Bottom) then
        if (AEventViewInfo.Event.TaskComplete < 100) and not AEventViewInfo.Hidden
        then
        begin
...
function TcxSchedulerGanttViewTotalProgressViewInfo.
  GetTimeLineApproximateRect: TRect;
var
...
begin
  ...
  begin
    ...
    Inc(Result.Left, Trunc((Now - AVisibleStart) * (Result.Right - Result.Left)
      / (ViewInfo.FLastVisibleTime - AVisibleStart)));
...
begin
  ...
  begin
    ...
    P := Point(Min(ALine.Left, R.Left + Trunc((R.Right - R.Left) *
      AEventViewInfo.Event.TaskComplete / 100)),
      (ALine.Top + ALine.Bottom) div 2);
  end;
end;

{ TcxSchedulerGanttViewViewInfo }

constructor TcxSchedulerGanttViewViewInfo.Create
...
begin
  if ((AEvent.Source = nil) or IsEventExpanded(AEvent.Source)) and
    IsGanttTaskVisible(AEvent) then
    inherited AddEventForCalculation(ABuilder, AEvent, AResource,
      AResourceIndex);
end;
...
begin
  ...
  if View.ShowExpandButtons then
    ...
    begin
      ...
      if AEventCell.Event.TaskLinks = nil then
        Continue;
      if not AEventCell.Hidden and (AEventCell.Event.NonExceptionLinkCount > 0)
      then
      begin
        CreateCellInstance(TcxSchedulerGanttViewExpandButtonViewInfo,
          CalculateButtonBounds(AEventCell.Bounds), LinkViewParams,
          UseRightToLeftAlignment, AButtonViewInfo);
        AButtonViewInfo.FExpanded :=
          IsTaskLinksExpanded(AEventCell.Event.TaskLinks);
...
begin
  if (View.Controller.DragEventHelper <> nil) or
    ((View.Controller.DragAndDropObject <> nil) and
    (View.Controller.DragAndDropObject.SizingHelper <> nil)) or
    not(ExpandButtons.CalculateHitTest(AHitTest) or
    Links.CalculateHitTest(AHitTest)) then
...
function TcxSchedulerGanttViewViewInfo.IsEventExpanded
...
begin
  ...
  if not Result then
  begin
    Result := (AEvent.TaskLinkOwners = nil) or
      (AEvent.TaskLinkOwners.Count = 0);
    if not Result then
    begin
      for I := 0 to AEvent.TaskLinkOwners.Count - 1 do
      begin
        Result := IsTaskLinksExpanded(AEvent.TaskLinkOwners[I].TaskLinks) and
          IsEventExpanded(AEvent.TaskLinkOwners[I]);
        if Result then
          Break;
      end;
    end;
    Result := Result and ((AEvent.ParentGroup = nil) or
      IsEventExpanded(AEvent.ParentGroup));
  end;
end;
...
procedure TcxSchedulerGanttViewViewInfo.CheckVisibleEventCounter
  (AEvent: TcxSchedulerControlEvent);
  ...
  begin
    ...
    for AResourceIndex := 0 to ResourceCount - 1 do
      if AEvent.IsSharedWithResource(VisibleEventEnumerationList[AResourceIndex]
        .ResourceID) then
      begin
...
begin
  if (AEvent.Start < FLastVisibleTime) and (AEvent.Finish > View.VisibleStart)
  then
...
function TcxSchedulerGanttViewViewInfo.CreateEventViewData
  ...
  const AStart: TDateTime; const AFinish: TDateTime;
  AResource: TcxSchedulerResourceViewInfo): TcxSchedulerEventViewData;

begin
  Result := inherited CreateEventViewData(AEvent, ABounds, AStart, AFinish,
    AResource);
...
function TcxSchedulerGanttViewViewInfo.CreateLayoutBuilder
  : TcxSchedulerEventLayoutBuilder;
begin
...
begin
  ...
  if Result then
  begin
    Result := FindEventViewInfoBySource(AEventItemLink.Link,
      AEventItemLink.Link.Start, AEventViewInfo.ResourceItem, AFinishViewInfo);
    if Result then
    begin
      CreateCellInstance(TcxSchedulerGanttViewLinkViewInfo,
        AEventViewInfo.Bounds, Bounds, LinkViewParams, UseRightToLeftAlignment,
        ALinkItem);
      ALinkItem.Initialize(Self, AEventViewInfo, AFinishViewInfo,
        AEventItemLink);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if (ATaskLinks = nil) or not IsTaskLinksExpanded(ATaskLinks) then
      Continue;
    for ALinkIndex := 0 to ATaskLinks.Count - 1 do
      if CreateLinkItemViewInfo(AEventViewInfo,
        ATaskLinks.ItemLinks[ALinkIndex], ALinkItem) then
        FLinks.Add(ALinkItem);
  end;
end;

procedure TcxSchedulerGanttViewViewInfo.CustomProcessPreviouslyCalculatedEvent
  (AEventCell: TcxSchedulerEventCellViewInfo);
begin
      ...
      begin
        ...
        with TcxSchedulerEventCellViewInfoAccess(ATempViewInfo) do
          if (ADate = NullDate) or ((dxDateOf(ContentStart) = ADate) or
            ...
            (ADate < dxDateOf(ContentFinish)))) then
            Break;
      end;
  end;
...
function TcxSchedulerGanttViewViewInfo.GetEventBounds
...
begin
  ...
  begin
    ATempViewInfo := TcxSchedulerEventCellViewInfo(EventCells.Items[I]);
    Result := (ATempViewInfo.Event = AEvent) and
      (ATempViewInfo.ResourceItem = AResource);
    if Result then
    begin
...
function TcxSchedulerGanttViewViewInfo.HasEventForResource(AResourceIndex
  : Integer): Boolean;
var
  I: Integer;
  AResourceID: Variant;
...
begin
  Result := View.Scheduler.OptionsBehavior.HotTrack and
    (View.Controller.EventDragMode = sedmNone);
end;
...
begin
  AEventCount := 0;
  for AResourceIndex := 0 to ResourceCount - 1 do
    Inc(AEventCount, VisibleEventEnumerationList[AResourceIndex]
      .VisibleEventCount);
...
begin
  ...
  if not Result then
  begin
    while (AEvent.ParentGroup <> nil) and
      AEvent.ParentGroup.GroupItems.Expanded and
      (AEvent <> AEvent.ParentGroup) do
      AEvent := AEvent.ParentGroup;
    Result := AEvent.ParentGroup = nil;
  end;
end;

function TcxSchedulerGanttViewViewInfo.IsGroup
  (AEvent: TcxSchedulerControlEvent): Boolean;
begin
  Result := AEvent.IsGroup;
end;

function TcxSchedulerGanttViewViewInfo.IsMilestone
  (AEvent: TcxSchedulerControlEvent): Boolean;
begin
...
begin
  ...
  if not TcxCustomSchedulerAccess(Scheduler).IsPopupScrollBars and
    (AKind = nbkNext) and ResourceViewShift[AResourceIndex].ScrollBar.Visible
  then
...
begin
  ...
  begin
    for I := 0 to Links.Count - 1 do
      TcxSchedulerGanttViewLinkViewInfo(Links[I])
...
procedure TcxSchedulerGanttViewViewInfo.SetHotLink
  ...
  function GetRefreshRect(AOldLink, ANewLink
    : TcxSchedulerGanttViewLinkViewInfo): TRect;
  begin
