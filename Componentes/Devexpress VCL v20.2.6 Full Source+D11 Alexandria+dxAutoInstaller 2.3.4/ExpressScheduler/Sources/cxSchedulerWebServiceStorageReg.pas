...
type
  TcxSchedulerWebServiceStorageCustomProviderAccess = class
...
function TcxSchedulerWebServiceStorageProviderProperty.GetAttributes
  : TPropertyAttributes;
var
...
procedure TcxSchedulerWebServiceStorageProviderProperty.GetValues
  (Proc: TGetStrProc);
var
  I: Integer;
begin
  inherited GetValues(Proc);
  for I := 0 to TcxSchedulerWebServiceStorage.RegisteredCalendarProviders.
    Count - 1 do
    Proc(TcxSchedulerWebServiceStorage.RegisteredCalendarProviders.
      Descriptions[I]);
end;

procedure TcxSchedulerWebServiceStorageProviderProperty.SetValue
  (const Value: string);
var
  AItem: TcxSchedulerWebServiceStorageResourceItem;
  AClass: TClass;
begin
  ...
  begin
    if Value <> '' then
      AClass := TcxSchedulerWebServiceStorage.RegisteredCalendarProviders.
    ...
    if AClass = nil then
      ...
      AItem.ProviderClass :=
        TcxSchedulerWebServiceStorageCustomProviderClass(AClass);
    Modified;
  end;
end;

function TcxSchedulerWebServiceStorageProviderProperty.GetItem
...
function TcxSchedulerCloudStorageProviderCalendarIDProperty.GetAttributes
  : TPropertyAttributes;
begin
...
procedure TcxSchedulerCloudStorageProviderCalendarIDProperty.SetValue
  (const Value: string);
var
  AProvider: TcxSchedulerWebServiceStorageCustomProvider;
  AList: TdxWebServiceCalendarList;
...
function TcxSchedulerCloudStorageProviderCalendarIDProperty.GetList
...
function TcxSchedulerCloudStorageProviderCalendarIDProperty.GetProvider
  : TcxSchedulerWebServiceStorageCustomProvider;
var
  I: Integer;
begin
...
begin
  ...
  RegisterSelectionEditor(TcxSchedulerWebServiceStorage,
    TcxSchedulerWebServiceStorageSelectionEditor);
  RegisterPropertyEditor(TypeInfo(TcxSchedulerWebServiceStorageCustomProvider),
    TcxSchedulerWebServiceStorageResourceItem, 'Provider',
    TcxSchedulerWebServiceStorageProviderProperty);
  RegisterPropertyEditor(TypeInfo(string),
    TcxSchedulerWebServiceStorageCustomProvider, 'CalendarID',
    TcxSchedulerCloudStorageProviderCalendarIDProperty);
  HideClassProperties(TcxSchedulerWebServiceStorage, ['StoreUsingGlobalTime']);
  HideClassProperties(TcxSchedulerWebServiceStorageResourceItem,
    ['ReadOnly', 'ProviderClassName', 'Parent', 'ResourceID', 'WorkDays',
    'WorkFinish', 'WorkStart']);
end;
