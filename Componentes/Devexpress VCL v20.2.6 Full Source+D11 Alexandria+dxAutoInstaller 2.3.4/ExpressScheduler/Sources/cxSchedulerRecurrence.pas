...
uses
  Types, Variants, DateUtils, Classes, SysUtils, cxDateUtils, cxSchedulerUtils,
  cxFormats;

type
  TcxDayType = (cxdtDay, cxdtEveryDay, cxdtWeekDay, cxdtWeekEndDay, cxdtSunday,
    cxdtMonday, cxdtTuesday, cxdtWednesday, cxdtThursday, cxdtFriday,
    cxdtSaturday);

  TcxRecurrence = (cxreDaily, cxreWeekly, cxreMonthly, cxreYearly);

  TcxRecurrenceValidStatus = (rvsValid, rvsReplaceOccurrenceDate,
    rvsInvalidPattern, rvsInvalidDuration);
  ...
  protected
    ...
    function GetCertainDay(const ADate, ANumber: Integer;
      ADayType: TcxDayType): Integer;
    function GetDayForMonth: Integer; virtual;
    function GetDuration: TDateTime; virtual; abstract;
    function GetDayFromOccurDays(const APrevDate, APeriodicity
      : Integer): Integer;
    function GetRecurrenceInfo: TcxSchedulerCustomRecurrenceInfo;
      virtual; abstract;
    function GetSomeDay(const ADate, ANumber: Integer;
      AWeekEnd: Boolean): Integer;
    ...
    property RecurrenceInfo: TcxSchedulerCustomRecurrenceInfo
  ...
  TcxSchedulerRecurrenceCalculatorClass = class of
    TcxSchedulerRecurrenceCalculator;
  ...
  public
    ...
    property DisplayTimeBias: Double read FDisplayTimeBias
      write FDisplayTimeBias;
    ...
    property OriginalStart: TDateTime read GetOriginalStart
      write SetOriginalStart;
    ...
    property YearPeriodicity: Integer read GetYearPeriodicity
      write SetYearPeriodicity;
  end;

const
  DefInfoData: TcxSchedulerCustomRecurrenceInfoData = (Count: 10; DayNumber: 1;
    DayType: cxdtDay; Finish: - 1; OccurDays: []; Periodicity: 1;
    Recurrence: cxreDaily; Start: 0; YearPeriodicity: 1; Version: 1;
    DismissDate: 0);

function cxGetCustomRecurrenceDescriptionString(ARecurrenceInfo
  : TcxSchedulerCustomRecurrenceInfo): string;
function cxRecurrenceInfoDataToString(const AData
  : TcxSchedulerCustomRecurrenceInfoData): AnsiString;
function cxStringToRecurrenceInfoData(const S: AnsiString)
...
function cxGetCustomRecurrenceDescriptionString(ARecurrenceInfo
  : TcxSchedulerCustomRecurrenceInfo): string;
const
  Weeks: array [1 .. 5] of string = ('first', 'second', 'third',
    'fourth', 'last');
  Days: array [cxdtEveryDay .. cxdtWeekEndDay] of string = ('day', 'weekday',
    'weekend day');
  EveryDays: array [Boolean] of string = ('every %d days', 'every day');
  EveryMonths1: array [Boolean] of string = ('day %d of every %d months',
    'day %d of every month');
  EveryMonths2: array [Boolean] of string = ('the %s %s of every %d months',
    'the %s %s of every month');

  procedure GetDateParts(out ADayStr, AWeekStr: string);
  begin
    with ARecurrenceInfo do
    begin
      if DayNumber in [1 .. 5] then
      ...
      if DayType in [cxdtEveryDay .. cxdtWeekEndDay] then
        ...
        ADayStr := dxFormatSettings.LongDayNames
          [Ord(DayType) - Ord(cxdtSunday) + 1];
    end;
  end;
    ...
    for ADay := Low(TDay) to High(TDay) do
      if ADay in ARecurrenceInfo.OccurDays then
        ...
        begin
          Dec(ACount);
          if ACount > 1 then
            Result := Result + dxFormatSettings.LongDayNames
              [Ord(ADay) + 1] + ', '
          else if ACount = 1 then
            Result := Result + dxFormatSettings.LongDayNames
              [Ord(ADay) + 1] + ' '
          else
            Result := Result + 'and ' + dxFormatSettings.LongDayNames
              [Ord(ADay) + 1];
        end;
  end;
  ...
  case ARecurrenceInfo.Recurrence of
    cxreDaily:
      if ARecurrenceInfo.DayType = cxdtWeekDay then
        ...
        Result := Format(EveryDays[ARecurrenceInfo.Periodicity = 1],
          [ARecurrenceInfo.Periodicity]);
    cxreWeekly:
      if ARecurrenceInfo.Periodicity = 1 then
        Result := 'every ' + OccurDaysToString
      else
        Result := Format('every %d weeks on %s', [ARecurrenceInfo.Periodicity,
          OccurDaysToString]);
    cxreMonthly:
      if ARecurrenceInfo.DayType = cxdtDay then
        Result := Format(EveryMonths1[ARecurrenceInfo.Periodicity = 1],
          [ARecurrenceInfo.DayNumber, ARecurrenceInfo.Periodicity])
      else
      begin
        GetDateParts(ADayStr, AWeekStr);
        Result := Format(EveryMonths2[ARecurrenceInfo.Periodicity = 1],
          [AWeekStr, ADayStr, ARecurrenceInfo.Periodicity]);
      end;
    cxreYearly:
      begin
        if ARecurrenceInfo.Periodicity in [1 .. 12] then
          AMonthStr := dxFormatSettings.LongMonthNames
            [ARecurrenceInfo.Periodicity]
        else
          AMonthStr := '';
        if ARecurrenceInfo.DayType = cxdtDay then
          Result := Format('every %s %d',
            [AMonthStr, ARecurrenceInfo.DayNumber])
        else
        begin
          GetDateParts(ADayStr, AWeekStr);
...
begin
  ...
  repeat
    ...
    if FDate = InvalidDate then
      Break;
    FOccurrenceFinish := FDate +
      (FOccurrenceFinish - dxDateOf(FOccurrenceStart));
    FOccurrenceStart := FDate + dxTimeOf(FOccurrenceStart);
    Inc(FIndex);
  until (not CalcForReminders and (dxDateOf(FOccurrenceStart) >=
    dxDateOf(FVisibleStart))) or
    (CalcForReminders and (dxDateOf(FOccurrenceStart) >=
    dxDateOf(FActualStart)));
...
function TcxSchedulerRecurrenceCalculator.GetCertainDay(const ADate,
  ANumber: Integer; ADayType: TcxDayType): Integer;
var
  AYear, AMonth, ADay: Word;
  AOffset: Integer;
begin
  DecodeDate(ADate, AYear, AMonth, ADay);
  AOffset := Ord(ADayType) - Ord(cxdtSunday) -
    (DayOfWeek(ADate - ADay + 1) - 1);
  if AOffset < 0 then
    Inc(AOffset, 7);
  Inc(AOffset, (ANumber - 1) * 7);
  if AOffset > DaysInAMonth(AYear, AMonth) - 1 then
    Dec(AOffset, 7);
...
function TcxSchedulerRecurrenceCalculator.GetDayFromOccurDays(const APrevDate,
  APeriodicity: Integer): Integer;
var
  ADelta: Integer;
  ADay: TDay;
begin
...
function TcxSchedulerRecurrenceCalculator.GetSomeDay(const ADate,
  ANumber: Integer; AWeekEnd: Boolean): Integer;
var
  AYear, AMonth, ADay: Word;
  I: Integer;
begin
...
function TcxSchedulerCustomRecurrenceInfo.GetCalculatorClass
...
function TcxSchedulerCustomRecurrenceInfo.GetDailyPatternStatus
...
function TcxSchedulerCustomRecurrenceInfo.GetMonthlyPatternStatus
  : TcxRecurrenceValidStatus;
begin
  if DayType = cxdtDay then
  begin
    Result := PatternValidStatus[(Periodicity > 0) and
      (DayNumber in [1 .. 31])];
...
function TcxSchedulerCustomRecurrenceInfo.GetWeeklyPatternStatus
  : TcxRecurrenceValidStatus;
begin
  Result := PatternValidStatus[(Periodicity > 0) and (OccurDays <> [])]
end;

function TcxSchedulerCustomRecurrenceInfo.GetYearlyPatternStatus
  : TcxRecurrenceValidStatus;
begin
  if DayType = cxdtDay then
    Result := PatternValidStatus[(Periodicity in [1 .. 12]) and (DayNumber >= 1)
      and (DayNumber <= DaysPerMonth(2000, Periodicity))]
  else
...
procedure TcxSchedulerCustomRecurrenceInfo.SetCount(const AValue: Integer);
begin
  SetDataItem(@DefInfoData.Count, SizeOf(Integer), AValue);
end;

procedure TcxSchedulerCustomRecurrenceInfo.SetDayNumber(const AValue: Integer);
begin
  SetDataItem(@DefInfoData.DayNumber, SizeOf(Integer), AValue);
end;

procedure TcxSchedulerCustomRecurrenceInfo.SetDayType(const AValue: TcxDayType);
begin
...
procedure TcxSchedulerCustomRecurrenceInfo.SetOccurDays(const AValue: TDays);
begin
  SetDataItem(@DefInfoData.OccurDays, SizeOf(TDays), AValue);
end;

procedure TcxSchedulerCustomRecurrenceInfo.SetOriginalStart
  (const AValue: TDateTime);
var
  AVersion: Byte;
begin
