...
function cxShowRecurrenceHolidayEditor
  (AHoliday: TcxSchedulerHolidaysLocationHoliday; ALookAndFeel: TcxLookAndFeel;
  out AModified: Boolean; AReadOnly: Boolean = False;
  AIsInternalActivation: Boolean = False; AControl: TControl = nil): Boolean;
...
procedure cxComboBoxSetNearestTimeInterval(ACombo: TcxComboBox;
  AMinutes: Integer);
...
function cxShowHolidaysLocationHolidayEditor(var ALocationHoliday
  : TcxSchedulerHolidaysLocationHoliday; ALookAndFeel: TcxLookAndFeel;
  AControl: TControl = nil): Boolean;
function cxShowHolidaysLocationEditor(var ALocation
  : TcxSchedulerHolidaysLocation; ALookAndFeel: TcxLookAndFeel): Boolean;
...
var
  ...
  cxRecurrenceEventEditorClass: TcxSchedulerRecurrenceEventEditorClass =
    TcxSchedulerRecurrenceEventEditor;
  cxRecurrenceHolidayEditorClass: TcxSchedulerRecurrenceHolidayEditorClass =
    TcxSchedulerRecurrenceHolidayEditor;
  cxReminderFormClass: TcxSchedulerReminderFormClass = TcxSchedulerReminderForm;
  cxHolidayEditorClass: TcxSchedulerHolidaysEditorClass =
    TcxSchedulerHolidaysEditor;
  cxTaskDependencyEditorClass: TcxSchedulerTaskDependencyEditorClass =
    TcxSchedulerTaskDependencyEditor;
  ...
  begin
    ...
    try
      for I := Low(ComboBoxTimeIntervals) to High(ComboBoxTimeIntervals) do
        AddObject(cxGetResourceString(ComboBoxTimeIntervals[I].P),
          TObject(ComboBoxTimeIntervals[I].M));
    finally
      EndUpdate;
    end;
  end;
...
begin
  ...
  Result := AEvent.Storage.EditingEventInfoList.ShowEditor(AEventInfo,
    AModified);
end;
...
begin
  Result := False;
  if not IsEventValid(AEvent) or (cxRecurrenceEventEditorClass = nil) then
    Exit;
...
begin
  Result := False;
  if not IsHolidayValid(AHoliday) or (cxRecurrenceHolidayEditorClass = nil) then
    Exit;
...
begin
  if not IsStorageValid(AStorage) or (AStorage.ResourceCount = 0) then
    Exit;
