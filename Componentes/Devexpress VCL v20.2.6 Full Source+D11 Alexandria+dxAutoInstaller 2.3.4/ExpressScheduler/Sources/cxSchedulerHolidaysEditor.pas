...
uses
  ...
  cxSchedulerHolidays, cxCheckBox, cxLookAndFeels, cxShellBrowserDialog,
  cxGraphics,
  cxDateUtils, ActnList, ImgList, cxSplitter, ExtCtrls, dxLayoutControlAdapters,
...
uses
  Types, cxSchedulerStrs, cxSchedulerDialogs, Math, cxSchedulerUtils,
  cxCustomData;
...
begin
  if (clbLocations.Count = 0) or (not ANew and (clbHolidays.Count = 0)) then
    Exit;
  if not ANew then
  begin
    CheckHolidayItemIndex;
    ALocationHoliday := FHolidays.Locations[LocationIndex].Holidays
      [HolidayIndex];
  end
  else
    ALocationHoliday := FHolidays.AddHoliday(FHolidays.Locations[LocationIndex]
      .Name, '', Date);
  if cxShowHolidaysLocationHolidayEditor(ALocationHoliday,
    clbLocations.LookAndFeel, Self) then
  begin
...
begin
  if not ANew and ((clbLocations.Count = 0) or (LocationIndex < 0) or
...
var
  ...
  AIndex: Integer;
begin
  if (LocationIndex < 0) or (LocationIndex >= clbLocations.Count) then
    Exit;
...
begin
  if (ALocationIndex < 0) or (ALocationIndex >= clbLocations.Count) then
    Exit;
  ...
  try
    ...
    begin
      ...
      begin
        if Assigned(cxGetHolidayRecurrenceDescriptionStringProc) then
          ARecurrenceDescription := cxGetHolidayRecurrenceDescriptionStringProc
            (AHoliday.RecurrenceInfo);
...
procedure TfmHolidaysEditor.clbHolidaysKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_RETURN:
