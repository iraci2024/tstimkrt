  ...
  begin
    ...
    begin
      ...
      if ATabViewInfo.IsVisibleForGoDialog and ATabViewInfo.ActuallyEnabled then
        AList.AddObject(RemoveAccelChars(ATabViewInfo.Caption, False),
          ATabViewInfo);
    end;
    AList.Sorted := pcoSort in TcxCustomTabControlPropertiesAccess
      (TabControl.Properties).Options;
  end;
...
begin
  ...
  try
    ...
    Adapter.SetImages(TcxCustomTabControlPropertiesAccess
      (TabControl.Properties).Images);
    for I := 0 to AItemCaptionList.Count - 1 do
    begin
      ATabViewInfo := TcxTabViewInfo(AItemCaptionList.Objects[I]);
      ATab := ATabViewInfo.Tab;
      AItem := Adapter.Add(ATab.Caption, MenuItemClick,
        ATabViewInfo.VisibleIndex, ATab.ImageIndex);
