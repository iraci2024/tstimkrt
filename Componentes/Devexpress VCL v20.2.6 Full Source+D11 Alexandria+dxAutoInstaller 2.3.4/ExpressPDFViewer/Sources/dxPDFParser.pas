...
uses
  SysUtils, Classes, Generics.Defaults, Generics.Collections, Math, dxHashUtils,
  dxPDFStreamFilter, dxPDFBase, dxPDFTypes,
  dxPDFUtils;
...
type
  ...
  strict private
    ...
    function CreateBooleanObject(AToken: TdxPDFTokenDescription;
      AValue: Boolean): TdxPDFBoolean;
    function GetName: string;
    function DoReadArray: TdxPDFArray; inline;
    function DoReadBooleanObject(AToken: TdxPDFTokenDescription;
      ADefaultValue: Boolean; AIsHexStrSeparatedByWhiteSpaces: Boolean)
      : TdxPDFBase; inline;
    function DoReadComment: TdxPDFComment;
    function DoReadName: TdxPDFName; inline;
    function DoReadNames: TdxPDFArray; inline;
    function DoReadNumericObject(AIsHexStrSeparatedByWhiteSpaces: Boolean)
      : TdxPDFBase; inline;
  ...
  protected
    ...
    function CreateStream(ADictionary: TdxPDFDictionary; const AData: TBytes)
      : TdxPDFStream;
    ...
    function ReadCompositeObject(AIsHexStrSeparatedByWhiteSpaces: Boolean)
      : TdxPDFBase; virtual;
    ...
    function DoReadObject(AIsHexStrSeparatedByWhiteSpaces: Boolean = False)
      : TdxPDFBase;
    ...
    function ReadReal(AInitialValue: Integer; AIsNegative: Boolean)
      : TdxPDFNumericObject;
  ...
  public
    ...
    function ReadObject(const AData: TBytes; APosition: Integer = 0)
      : TdxPDFBase;
  ...
  strict private
    ...
    function DoReadStream(ADictionary: TdxPDFDictionary): TdxPDFStream;
      overload;
  ...
  protected
    ...
    function ReadHexadecimalString(AIsHexStrSeparatedByWhiteSpaces
      : Boolean = False): TdxPDFString; virtual;

    function DoReadData(AToken: TdxPDFTokenDescription;
      AIgnoreWhiteSpace: Boolean = False;
      AEndOfDataToken: TdxPDFTokenDescription = nil;
      AHasWhiteSpaceBeforeToken: Boolean = True): TBytes; overload;
    function DoReadData(ALength: Integer; AToken: TdxPDFTokenDescription)
      : TBytes; overload;
    function DoReadDictionaryOrStream: TdxPDFBase;
    function ReadStream(const AData: TBytes; ALength: Integer)
      : TdxPDFStream; overload;
    function ReadStream(ADictionary: TdxPDFDictionary): TdxPDFStream; overload;

    property Repository: TdxPDFCustomRepository read FRepository
      write SetRepository;
  public
  ...
  public
    constructor Create(ARepository: TdxPDFCustomRepository; ANumber: Integer);
      reintroduce;
  end;
  ...
  protected
    ...
    procedure FinalizeTokens; override;
  public
    constructor Create(ARepository: TdxPDFCustomRepository; AStream: TStream);
      reintroduce;
    ...
    function ReadObjectData(AOffset: Int64;
      out ANumber, AGeneration: Integer): TBytes;
    function ReadString: string;
    function TryReadObjectHeader(AOffset: Int64;
      out ANumber, AGeneration: Integer): Boolean;
  ...
  strict private
    ...
    function ReadValue(const AData: TBytes; AWeight: Integer;
      var APosition: Int64): Integer;
    procedure Decode(const AData: TBytes;
    ...
    procedure PopulateIndexes(ATrailer: TdxPDFDictionary;
      AIndexes: TList<TdxPDFCrossReferenceIndexDescription>);
  protected
    procedure InitializeTokens; override;
    procedure FinalizeTokens; override;
  public
    ...
    function TryFindObjectOffset(ANumber, AGeneration: Integer;
    ...
    procedure ReadTrailer(AOffset: Int64;
      out ATrailer: TdxPDFDictionary); overload;
...
uses
  Variants, Character, StrUtils, dxCore, dxStringHelper, dxPDFCore;
...
function TdxPDFBaseParser.ReadObject(const AData: TBytes;
  APosition: Integer = 0): TdxPDFBase;
begin
  Data := AData;
...
begin
        ...
        if AIgnoreComment and (Current = TdxPDFDefinedSymbols.Comment) then
          repeat
            if not ReadNext then
              Exit(False) until Current in [TdxPDFDefinedSymbols.CarriageReturn,
                TdxPDFDefinedSymbols.LineFeed]
            else
              Exit(True);
          end
        else if not ReadNext then
          Exit(False);
      end;
  ...
  function TdxPDFBaseParser.CreateStream(ADictionary: TdxPDFDictionary;
    const AData: TBytes): TdxPDFStream;
  begin
  ...
  function TdxPDFBaseParser.ReadCompositeObject(AIsHexStrSeparatedByWhiteSpaces
    : Boolean): TdxPDFBase;
  begin
  ...
  begin
    FBooleanTrueToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('true'));
    FBooleanFalseToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('false'));
    FNullToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('null'));
  end;
  ...
  begin
    Result := not IsWhiteSpace and
      not(Current in [TdxPDFDefinedSymbols.Comment,
      TdxPDFDefinedSymbols.NameIdentifier, TdxPDFDefinedSymbols.StartString,
      TdxPDFDefinedSymbols.EndString, TdxPDFDefinedSymbols.StartArray,
      TdxPDFDefinedSymbols.EndArray]);
  end;
  ...
  begin
    Result := nil;
    SkipSpaces;
    if (Current = TdxPDFDefinedSymbols.StartObject) and ReadNext and
      (Current = TdxPDFDefinedSymbols.StartObject) and ReadNext then
    begin
  ...
  begin
    case Current of
      TdxPDFDefinedSymbols.Minus:
        begin
          AIsNegative := True;
          if not ReadNext or (not IsDigit and not IsPeriod and
            ((Current <> TdxPDFDefinedSymbols.Minus) or not ReadNext)) then
            TdxPDFUtils.Abort;
        end;
  ...
  function TdxPDFBaseParser.DoReadObject(AIsHexStrSeparatedByWhiteSpaces
    : Boolean = False): TdxPDFBase;
  var
  ...
  begin
    ...
    begin
      ...
      try
        while ReadNext and not(Current in [TdxPDFDefinedSymbols.CarriageReturn,
          TdxPDFDefinedSymbols.LineFeed]) do
      ...
      begin
        ...
        if IsDigit then
          ...
          case Current of
            ...
            TdxPDFDefinedSymbols.Plus, TdxPDFDefinedSymbols.Minus,
              TdxPDFDefinedSymbols.Period:
              Result := DoReadNumericObject(AIsHexStrSeparatedByWhiteSpaces);
            ...
            TdxPDFDefinedSymbols.StartString:
              Result := DoReadString;
            Byte('t'):
              Result := DoReadBooleanObject(FBooleanTrueToken, True,
                AIsHexStrSeparatedByWhiteSpaces);
            Byte('f'):
              Result := DoReadBooleanObject(FBooleanFalseToken, False,
                AIsHexStrSeparatedByWhiteSpaces);
            Byte('n'):
              begin
  ...
  function TdxPDFBaseParser.ReadNumber(const AData: TBytes)
    : TdxPDFNumericObject;
  begin
    Data := AData;
  ...
  function TdxPDFBaseParser.ReadReal(AInitialValue: Integer;
    AIsNegative: Boolean): TdxPDFNumericObject;
  ...
  begin
    ...
    begin
      ...
      begin
        while ReadNext and
          (IsDigit or IsPeriod or (Current = TdxPDFDefinedSymbols.Minus) or
          (Current = TdxPDFDefinedSymbols.Plus)) do;
    ...
    begin
      ...
      begin
        ...
        if (AExponent = nil) or (AExponent <> nil) and
          (AExponent.ObjectType <> otInteger) then
          TdxPDFUtils.Abort
        else
          try
            if TdxPDFInteger(AExponent).Value > 38 then
              ...
              AValue := CheckResultValue(SignMap[AIsNegative] * AValue *
                Power(10, TdxPDFInteger(AExponent).Value));
    ...
    Result := TdxPDFDouble.Create
      (CheckResultValue(SignMap[AIsNegative] * AValue));
  end;
  ...
  function TdxPDFBaseParser.CreateBooleanObject(AToken: TdxPDFTokenDescription;
    AValue: Boolean): TdxPDFBoolean;
  begin
    Result := nil;
  ...
  begin
    ...
    ABuilder := TdxStringBuilderManager.Get;
    try
      while not AFinished and CanContinueReading and
        (Current <> TdxPDFDefinedSymbols.EndObject) and
  ...
  function TdxPDFBaseParser.DoReadBooleanObject(AToken: TdxPDFTokenDescription;
    ADefaultValue: Boolean; AIsHexStrSeparatedByWhiteSpaces: Boolean)
    : TdxPDFBase;
  begin
  ...
  function TdxPDFBaseParser.DoReadNumericObject(AIsHexStrSeparatedByWhiteSpaces
    : Boolean): TdxPDFBase;
  begin
  ...
  procedure TdxPDFBaseParser.InitializeData(const AData: TBytes;
    APosition: Int64);
  begin
    SetLength(FData, 0);
    FData := AData;
  ...
  begin
    Result := TdxPDFReaderDictionary.Create
      (Repository as TdxPDFDocumentRepository);
  end;

  function TdxPDFStructureParser.GetEncryptionInfo: IdxPDFEncryptionInfo;
  begin
    if not Supports((Repository as TdxPDFDocumentRepository).EncryptionInfo,
      IdxPDFEncryptionInfo, Result) then
      Result := inherited GetEncryptionInfo;
  end;

  function TdxPDFStructureParser.ReadCompositeObject
  ...
  function TdxPDFStructureParser.ReadNumericObject: TdxPDFBase;
    ...
    begin
      Result := (ACurrentObject.ObjectType = otInteger) and SkipSpaces and
        (Current = Byte('R')) and not(ReadNext and CanContinueReading and
        not(Current in [TdxPDFDefinedSymbols.StartObject,
        TdxPDFDefinedSymbols.EndObject]));
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Result := TdxPDFReference.Create(ANumber,
            TdxPDFInteger(ATempNumeric).Value);
        ATempNumeric.Free;
      end
      else
        Result := ATempNumeric;
  ...
  function TdxPDFStructureParser.ReadStream(const AData: TBytes;
    ALength: Integer): TdxPDFStream;
  begin
    Data := AData;
    Result := DoReadStream(ALength);
  end;

  function TdxPDFStructureParser.ReadStream(ADictionary: TdxPDFDictionary)
    : TdxPDFStream;
  begin
  ...
  begin
    ...
    FStreamToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('stream'));
    FEndStreamToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('endstream'));
  end;
  ...
  function TdxPDFStructureParser.ReadHexadecimalString
    (AIsHexStrSeparatedByWhiteSpaces: Boolean): TdxPDFString;
  var
    AResult: TBytes;
    ASymbol: Byte;
  begin
  ...
  function TdxPDFStructureParser.DoReadData(AToken: TdxPDFTokenDescription;
    AIgnoreWhiteSpace: Boolean = False;
    AEndOfDataToken: TdxPDFTokenDescription = nil;
    AHasWhiteSpaceBeforeToken: Boolean = True): TBytes;
    ...
    function FindTokenOffset(const AToken, ABuffer: TBytes;
      AOffset, ALimit: Integer): Integer;
    var
    ...
    begin
      ...
      begin
        ...
        else if ACurrent = AToken[0] then
          AState := 1;
      end;
      Result := -1;
    end;
  ...
  var
    I, AActualSize, AEndMarkerPosition, ATokenLength, ALength, AOffset,
      ALimit: Integer;
    AData: TBytes;
  begin
    ...
    begin
      ...
      if (AEndMarkerPosition < ATokenLength) and
        (Current = AToken.Sequence[AEndMarkerPosition]) then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              AOffset := FindTokenOffset(AEndOfDataToken.Sequence, Result,
                AOffset, ALimit);
              if AOffset <> -1 then
              begin
                if AOffset < Length(Result) then
                  TdxPDFUtils.DeleteData(AOffset,
                    Length(Result) - AOffset, Result);
                Exit;
              end;
            end;
    ...
    ALength := Length(Result) - ATokenLength - 1 -
      Ord(AHasWhiteSpaceBeforeToken);
    SetLength(Result, Max(ALength, 0));
  end;

  function TdxPDFStructureParser.DoReadData(ALength: Integer;
    AToken: TdxPDFTokenDescription): TBytes;
  begin
  ...
  procedure TdxPDFStructureParser.SetRepository(const AValue
  ...
  function TdxPDFStructureParser.DoReadStream(ADictionary: TdxPDFDictionary)
    : TdxPDFStream;
  var
    ALength: Integer;
    ...
    begin
      ...
      if ALengthObject <> nil then
        case ALengthObject.ObjectType of
          ...
          otIndirectReference:
            begin
              ALengthObject := Repository.GetObject(ALengthObject.Number)
                as TdxPDFBase;
              ALength := TdxPDFInteger(ALengthObject).Value;
            end;
        else
  ...
  begin
    while Current = TdxPDFDefinedSymbols.Space do
      if not ReadNext then
        TdxPDFUtils.Abort;
    if not(Current in [TdxPDFDefinedSymbols.CarriageReturn,
      TdxPDFDefinedSymbols.LineFeed]) then
      TdxPDFUtils.Abort
    else if Current = TdxPDFDefinedSymbols.CarriageReturn then
    begin
      if not ReadNext then
        TdxPDFUtils.Abort;
      if Current <> TdxPDFDefinedSymbols.LineFeed then
  ...
  constructor TdxPDFEncryptedStructureParser.Create
    (ARepository: TdxPDFCustomRepository; ANumber: Integer);
  begin
    inherited Create(ARepository);
    FNumber := ANumber;
  end;

  function TdxPDFEncryptedStructureParser.DecryptString
    (const AData: TBytes): string;
  begin
    Result := inherited DecryptString((Repository as TdxPDFDocumentRepository)
      .EncryptionInfo.Decrypt(AData, FNumber));
  end;
  ...
  function TdxPDFObjectParser.IsValidReference
    (AReference: TdxPDFReference): Boolean;
  var
    ANumber, AGeneration: Integer;
  begin
    Result := CanReadNumber(AReference.Offset) and
      TryReadObjectHeader(AReference.Offset, ANumber, AGeneration) and
  ...
  function TdxPDFDocumentParser.ReadCrossReferences(AOffset: Integer): Boolean;
    ...
    begin
      ACrossReferencesToken := TdxPDFTokenDescription.Create
        (TdxPDFUtils.StrToByteArray('xref'));
      try
        ADelta := 0;
        if AOffset - StartXRefOffset > 0 then
          ...
          begin
            ...
            if not Result then
              ...
              ADelta := AOffset - FStream.Position +
                Length(ACrossReferencesToken.Sequence);
          end;
  ...
  function TdxPDFObjectParser.ReadIndirectObject(AOffset: Int64)
  ...
  function TdxPDFObjectParser.TryReadObjectHeader(AOffset: Int64;
    out ANumber, AGeneration: Integer): Boolean;
  var
    AObjToken: TdxPDFTokenDescription;
  begin
    SaveCurrentPosition;
    AObjToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('obj'));
    try
      FStream.Position := AOffset;
      try
        ANumber := ReadNumber;
  ...
  function TdxPDFDocumentParser.FindToken(const ATokenDescription
    : string): Boolean;
  var
  ...
  function TdxPDFDocumentParser.FindToken
    (AToken: TdxPDFTokenDescription): Boolean;
  begin
    Result := False;
    while not IsEOF do
    begin
  ...
  function TdxPDFDocumentParser.TryFindObjectOffset(ANumber,
    AGeneration: Integer; out AOffset: Int64): Boolean;
  const
    BufferSize = 1024 * 1024;
  var
    ABuffer: AnsiString;
    ASearchText: string;
    AActualReadCount, ABufferOffset, ASearchTextLength: Integer;
  begin
    Result := False;
    if FStream = nil then
      Exit;
    ASearchText := IntToStr(ANumber) + ' ' + IntToStr(AGeneration) + ' obj';
    ASearchTextLength := Length(ASearchText);
    SaveCurrentPosition;
    try
      ABufferOffset := 0;
      ...
      begin
        ...
        AOffset := PosEx(ASearchText, dxAnsiStringToString(ABuffer)) - 1;
        Result := AOffset > 0;
        if not Result then
        begin
          Inc(ABufferOffset, AActualReadCount - ASearchTextLength);
          if IsEOF then
            Break;
          FStream.Position := FStream.Position - ASearchTextLength;
        end
        else
        begin
          AOffset := ABufferOffset + AOffset;
          Break;
        end;
      end;
    finally
  ...
  function TdxPDFDocumentParser.ParseVersion(const AHeader: string)
    : TdxPDFVersion;
  ...
  begin
    ...
    try
      for APair in AFormatVersionDictionary do
        if StartsStr(APair.Key, RightStr(AHeader,
          Length(AHeader) - Length(TdxPDFKeywords.VersionSignature))) then
        begin
          Result := APair.Value;
          Break;
        end;
  ...
  begin
    ...
    try
      begin
        ACrossReferencesStartToken := TdxPDFTokenDescription.Create
          (TdxPDFKeywords.StartXRef);
        try
          while not IsEOF do
          begin
            ASymbol := ReadByte;
            ...
            begin
              ...
              begin
                ...
                SetLength(Result, Length(Result) -
                  Length(ACrossReferencesStartToken.Sequence));
  ...
  var
    ...
    AEOFToken, AEndObjToken, AObjToken, ACrossReferencesToken
  ...
  begin
    ...
    AObjToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('obj'));
    AEndObjToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('endobj'));
    AEOFToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('%%EOF'));
  ...
  begin
    FindObjects(procedure(APosition: Integer)
      var
        ASlot: TdxPDFBase;
      begin
  ...
  begin
    ...
    while not StartsStr(TdxPDFKeywords.VersionSignature, AHeader) do
      if not((Length(AHeader) = 0) or
        (Byte(AHeader[1]) <> TdxPDFDefinedSymbols.Comment)) then
        AHeader := ReadString
      else
        TdxPDFUtils.Abort;
  ...
  procedure TdxPDFDocumentParser.ReadTrailer(AOffset: Int64;
  out ATrailer: TdxPDFDictionary);
  var
  ...
  begin
    ...
    begin
      ...
      ReadTrailerStream(AActualOffset, ATrailer);
  end;
  ...
  begin
    inherited InitializeTokens;
    FCheckObjToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray(' obj'));
  end;
  ...
  begin
    inherited InitializeTokens;
    FStartXRefObjectToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray(TdxPDFKeywords.StartXRef));
  end;
  ...
  constructor TdxPDFObjectParser.Create(ARepository: TdxPDFCustomRepository;
  AStream: TStream);
  begin
    inherited Create(ARepository);
    FStream := AStream;
  ...
  function TdxPDFObjectParser.ReadObjectData(AOffset: Int64;
  out ANumber, AGeneration: Integer): TBytes;
  var
    ANext, ATokenLength: Integer;
    ABeginToken, AEndToken, AEndObjToken: TdxPDFTokenDescription;
    ANested: Boolean;
    ASymbol: Byte;
    AObjToken: TdxPDFTokenDescription;
  begin
    ...
    try
      ...
      begin
        SetLength(Result, 0);
        ABeginToken := TdxPDFTokenDescription.BeginCompare(FCheckObjToken);
        AEndToken := TdxPDFTokenDescription.BeginCompare(AEndObjToken);
        try
          ANested := False;
          while not IsEOF do
          begin
            ...
            if AEndToken.Compare(ASymbol) then
              ...
              begin
                ...
                TdxPDFUtils.DeleteData(Length(Result) - ATokenLength,
                  ATokenLength, Result);
                Break;
              end;
            if ABeginToken.Compare(ASymbol) then
            begin
              if TdxPDFUtils.IsDigit
                (Result[Length(Result) - Length(FCheckObjToken.Sequence) - 1])
              then
                ANested := True;
              ABeginToken.Free;
              ABeginToken := TdxPDFTokenDescription.BeginCompare
                (FCheckObjToken);
            end;
          end;
        finally
  ...
  function TdxPDFObjectParser.ReadToken(AToken: TdxPDFTokenDescription)
    : Boolean;
  var
    I: Integer;
    ASymbol: Byte;
  ...
  procedure TdxPDFDocumentParser.ReadCrossReferenceRecords(AStartNumber,
    ACount: Integer);
  ...
  begin
    ...
    begin
      AOffset := ReadObjectParameter(dxcXReferenceObjectOffsetDimension) -
        FPositionOffset;
      AGeneration := ReadObjectParameter
        (dxcXReferenceObjectGenerationDimension);
      ANext := ReadByte;
      if (AStartNumber = 1) and (I = 0) and (AOffset = 0) and
        (AGeneration = 65535) and (ANext = Byte('f')) then
        Dec(ANumber);
      case ANext of
        Byte('n'):
          Repository.Add(ANumber, TdxPDFReference.Create(ANumber, AGeneration,
            AOffset, False, True), False);
        Byte('f'):
          Repository.Add(ANumber, TdxPDFReference.Create(ANumber, AGeneration,
            AOffset, True), False);
      end;
      ...
      case ANext of
        TdxPDFDefinedSymbols.Space, TdxPDFDefinedSymbols.CarriageReturn:
          begin
            ANext := ReadByte;
            if (ANext <> TdxPDFDefinedSymbols.LineFeed) and
              (ANext <> TdxPDFDefinedSymbols.CarriageReturn) then
              TdxPDFUtils.Abort;
          end;
      end;
      Inc(ANumber);
  ...
  begin
    Result := dxPDFInvalidValue;
    AToken := TdxPDFTokenDescription.BeginCompare(FStartXRefObjectToken);
    try
      repeat
  ...
  begin
    ...
    AEOFToken := TdxPDFTokenDescription.Create
      (TdxPDFUtils.StrToByteArray('%%EOF'));
    try
      FStream.Position := IfThen(FStream.Size > StartXRefOffset,
        FStream.Size - StartXRefOffset, FStream.Size);
      AOffset := -1;
          ...
          begin
            ...
            begin
              Result := FStream.Position -
                Length(FStartXRefObjectToken.Sequence);
              if not ReadToken(AEOFToken) then
                Break
              else
                Continue;
  ...
  begin
    ...
    begin
      ...
      AIsObjectEndFound := FindToken('endstream') and
        IsSingleObject(AStartPosition, FStream.Position);
    end;
    if not(AIsObjectFound and AIsObjectEndFound) then
      TdxPDFUtils.Abort;
    Result := TdxPDFReference.Create(ANumber, AGeneration, AOffset,
      False, True);
  end;

  function TdxPDFDocumentParser.ReadStartXRefObject
  ...
  procedure TdxPDFDocumentParser.ReadTrailerStream(AOffset: Int64;
  out ATrailer: TdxPDFDictionary);

    function GetDecodeParameters(ATrailer: TdxPDFDictionary;
    out ATypeSize, AOffsetSize, AGenerationSize: Integer): Boolean;
    var
      ASizes: TdxPDFArray;
    begin
      ASizes := ATrailer.GetArray(TdxPDFKeywords.ShortWidths);
      Result := (ASizes <> nil) and (ASizes.Count = 3) and
        (ATrailer.GetString(TdxPDFKeywords.TypeKey) = 'XRef');
      if Result then
      begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        ATrailer.Reference;
        if GetDecodeParameters(ATrailer, ATypeSize, AOffsetSize, AGenerationSize)
        then
        begin
          AIndexes := TList<TdxPDFCrossReferenceIndexDescription>.Create;
          try
            PopulateIndexes(ATrailer, AIndexes);
            Decode(AStream.UncompressedData, AIndexes, ATypeSize, AOffsetSize,
              AGenerationSize);
  ...
  function TdxPDFDocumentParser.ReadValue(const AData: TBytes; AWeight: Integer;
  var APosition: Int64): Integer;
  var
    I: Integer;
  begin
  ...
  procedure TdxPDFDocumentParser.Decode(const AData: TBytes;
  AIndexes: TList<TdxPDFCrossReferenceIndexDescription>;
  ATypeSize, AOffsetSize, AGenerationSize: Integer);
  var
    APosition: Int64;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        case AObjectType of
          0:
            Repository.Add(ANumber, TdxPDFReference.Create(ANumber, V2, V1,
              True), False);
          1:
            Repository.Add(ANumber, TdxPDFReference.Create(ANumber, V2, V1,
              False, True), False);
          2:
            (Repository as TdxPDFDocumentRepository).AddStreamElement(ANumber,
              TdxPDFStreamElement.Create(V1, V2));
        else
  ...
  AIndexes: TList<TdxPDFCrossReferenceIndexDescription>);
    ...
    function GetIndexDescription(ANumber, ACount: Integer)
      : TdxPDFCrossReferenceIndexDescription;
    begin
  ...
  begin
    ...
    begin
      ...
      if ASymbol = TdxPDFDefinedSymbols.Comment then
        ReadString
      else if not TdxPDFUtils.IsWhiteSpace(ASymbol) then
        Break;
    end;
  end;
