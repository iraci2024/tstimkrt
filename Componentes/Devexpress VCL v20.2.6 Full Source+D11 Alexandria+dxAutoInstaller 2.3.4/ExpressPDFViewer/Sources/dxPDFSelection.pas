...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  SysUtils, Windows, Classes, Messages, Forms, Controls, StdCtrls, Graphics,
  Generics.Defaults, Generics.Collections,
  dxCore, dxCoreClasses, cxClasses, cxGeometry, cxGraphics, dxCoreGraphics,
  dxPDFCore, dxPDFTypes, dxPDFText, dxPDFDocument,
...
type
  TdxPDFMovementDirection = (mdLeft, mdDown, mdRight, mdUp, mdNextWord,
    mdPreviousWord, mdLineStart, mdLineEnd, mdDocumentStart, mdDocumentEnd);
  ...
  strict private
    ...
    procedure CalculateRendererParameters(APage: TdxPDFPage; AScale: Single;
  ...
  public
    constructor Create(ABackColor, AFrameColor: TdxAlphaColor;
      APageIndex: Integer; AImage: TdxPDFImage; const AClipRect: TdxRectF);
      reintroduce;
  ...
  strict private
    ...
    procedure AddLineRects(APageRects: TList<TdxRectF>;
      APageLine: TdxPDFTextLine; const AStartPosition,
      AEndPosition: TdxPDFPageTextPosition; var AContainsEndOfLine: Boolean);
  ...
  public
    constructor Create(ABackColor, AFrameColor: TdxAlphaColor;
      APages: TdxPDFPages; const ARanges: TdxPDFPageTextRanges); reintroduce;
    destructor Destroy; override;
  end;
  ...
  public
    class function Create(const APosition: TdxPDFTextPosition;
      const AViewData: TdxPDFDocumentCaretViewData;
  ...
  TdxPDFDocumentCaretViewDataHelper = record helper for
    TdxPDFDocumentCaretViewData
  public
    class function Create(const ATopLeft: TdxPointF; AHeight, AAngle: Single)
      : TdxPDFDocumentCaretViewData; static;
...
uses
  Types, Math, dxTypeHelpers, cxFormats, cxControls, dxGDIPlusClasses,
  dxPDFBase, dxPDFUtils, dxPDFDocumentViewer,
...
constructor TdxPDFCustomSelection.Create(ABackColor,
...
function TdxPDFCustomSelection.Contains(const APosition
...
constructor TdxPDFImageSelection.Create(ABackColor, AFrameColor: TdxAlphaColor;
  APageIndex: Integer; AImage: TdxPDFImage; const AClipRect: TdxRectF);
begin
...
function TdxPDFImageSelection.Contains(const APosition: TdxPDFPosition)
  : Boolean;
var
  R: TdxRectF;
begin
  ...
  Result := (PageIndex = APosition.PageIndex) and
    TdxPDFUtils.PtInRect(R, APosition.Point);
end;
...
begin
  ...
  begin
    ...
    Result := (AValue <> nil) and (Image = ASelection.Image) and
      (PageIndex = ASelection.PageIndex) and TdxPDFUtils.RectIsEqual(Bounds,
      ASelection.Bounds, 0.0001);
  end;
end;
...
begin
  ...
  AParameters := TdxPDFRenderParameters.Create
    (TdxPDFDocumentAccess(ADocument).State);
  try
    ...
    Result.Canvas.CopyRect(Result.ClientRect, AOriginalPageBitmap.Canvas,
      cxRect(AClipRect));
  finally
    AParameters.Free;
...
begin
  if not TdxPDFUtils.Intersects(Result, TdxPDFImageAccess(FImage).Bounds,
    FClipRect) and (Result.Width > 0) and (Result.Height > 0) then
    Result := dxRectF(Result.Left, Result.Top, Result.Left + 1,
      Result.Bottom + 1);
end;
...
function TdxPDFImageSelection.ConvertToRotationAngle(AAngle: Integer)
  : TcxRotationAngle;
begin
  case AAngle of
    270:
...
procedure TdxPDFImageSelection.CalculateRendererParameters(APage: TdxPDFPage;
  AScale: Single; out APageSize: TPoint; out AClipRect: TdxRectF);
var
  R: TdxRectF;
...
begin
  ...
  R := cxRectUnion(AClipRect, TdxRectF.CreateSize(0, 0, APageSize.X,
    APageSize.Y));
...
constructor TdxPDFTextHighlight.Create(ABackColor, AFrameColor: TdxAlphaColor;
  APages: TdxPDFPages; const ARanges: TdxPDFPageTextRanges);
begin
  inherited Create(ABackColor, AFrameColor);
  FRects := TObjectDictionary < Integer, TList < TdxRectF >>
    .Create([doOwnsValues]);
...
function TdxPDFTextHighlight.Exclude(const ARange: TdxPDFPageTextRange)
  : Boolean;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    Result := inherited Same(ATextSelection) and
      (Length(Ranges) = Length(ATextSelection.Ranges));
    if Result then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (ARange.StartPosition.WordIndex = 0) and
            (ARange.EndPosition.WordIndex = 0) or ARange.WholePage then
            AddPageRects(ARects, APage.RecognizedContent.TextLines)
          else
            for ACurrentLine in APage.RecognizedContent.TextLines do
            begin
              AddLineRects(ARects, ACurrentLine, ARange.StartPosition,
                ARange.EndPosition, AContentsEndOfLine);
...
procedure TdxPDFTextHighlight.AddLineRects(APageRects: TList<TdxRectF>;
  APageLine: TdxPDFTextLine; const AStartPosition, AEndPosition
  : TdxPDFPageTextPosition; var AContainsEndOfLine: Boolean);
var
...
begin
  ...
  begin
    AHighlights := nil;
    AContainsEndOfLine := ALine.PositionInLine(AEndPosition.WordIndex,
      AEndPosition.Offset);
    AFirstPartIndex := ALine.WordPartList[0].WordIndex;
    AEnd.WordIndex := IfThen(AContainsEndOfLine, AEndPosition.WordIndex -
      AFirstPartIndex, 0);
    AEnd.Offset := IfThen(AContainsEndOfLine, AEndPosition.Offset -
      TdxPDFTextWordPartAccess(ALine.WordPartList[AEnd.WordIndex])
      .WrapOffset, 0);
    if ALine.PositionInLine(AStartPosition.WordIndex, AStartPosition.Offset)
    then
    begin
      AStart.WordIndex := AStartPosition.WordIndex - AFirstPartIndex;
      AStart.Offset := AStartPosition.Offset - TdxPDFTextWordPartAccess
        (ALine.WordPartList[AStart.WordIndex]).WrapOffset;
      if AContainsEndOfLine then
        AHighlights := ALine.GetHighlights(AStart.WordIndex, AStart.Offset,
          AEnd.WordIndex, AEnd.Offset, True)
      else
        AHighlights := ALine.GetHighlights(AStart.WordIndex,
          AStart.Offset, True);
    end
    else if AContainsEndOfLine then
      AHighlights := ALine.GetHighlights(0, 0, AEnd.WordIndex,
        AEnd.Offset, True)
    else
    begin
      ALastPart := ALine.WordPartList.Last;
      if ((AStartPosition.WordIndex = 0) or
        (AStartPosition.WordIndex <= AFirstPartIndex)) and
        ((AEndPosition.WordIndex = 0) or
        (ALastPart.WordIndex - 1 <= AEndPosition.WordIndex)) then
        AHighlights := ALine.GetHighlights(0, 0, ALastPart.WordIndex -
          AFirstPartIndex, TdxPDFTextWordPartAccess(ALastPart)
          .Characters.Count, True);
    end;
...
procedure TdxPDFTextHighlight.AddPageRects(ARects: TList<TdxRectF>;
...
procedure TdxPDFTextHighlight.AddRects(APageIndex: Integer;
  ARects: TList<TdxRectF>);
var
...
class function TdxPDFDocumentCaretHelper.Create(const APosition
  : TdxPDFTextPosition; const AViewData: TdxPDFDocumentCaretViewData;
  const AStartCoordinates: TdxPointF): TdxPDFDocumentCaret;
begin
...
function TdxPDFDocumentCaretHelper.Same(const AValue
  : TdxPDFDocumentCaret): Boolean;
begin
  Result := Position.Same(AValue.Position) and cxPointIsEqual(StartCoordinates,
    AValue.StartCoordinates) and ViewData.Same(AValue.ViewData);
end;

{ TdxPDFDocumentCaretViewDataHelper }

class function TdxPDFDocumentCaretViewDataHelper.Create(const ATopLeft
  : TdxPointF; AHeight, AAngle: Single): TdxPDFDocumentCaretViewData;
begin
  Result.Angle := AAngle;
...
class function TdxPDFDocumentCaretViewDataHelper.Invalid
  : TdxPDFDocumentCaretViewData;
begin
  Result.Angle := -1;
  Result.Height := -1;
...
function TdxPDFDocumentCaretViewDataHelper.Same(const AValue
  : TdxPDFDocumentCaretViewData): Boolean;
begin
  Result := (Angle = AValue.Angle) and cxPointIsEqual(TopLeft, AValue.TopLeft)
    and (Height = AValue.Height);
end;
...
procedure TdxPDFViewerClickController.Click(AButton: TMouseButton;
  X, Y: Integer);
begin
  FTimer.Enabled := True;
  FClickCount := IfThen(AButton <> FLastButton, 1, FClickCount + 1);
  if (Abs(FLastClickPoint.X - X) > 0) or (Abs(FLastClickPoint.Y - Y) > 0) or
    (FClickCount = 5) then
    FClickCount := 1;
  FLastButton := AButton;
...
procedure TdxPDFImageRenderer.InitializeClipBounds(const ATopLeft,
...
procedure TdxPDFImageRenderer.SetTextMatrix(const AMatrix
