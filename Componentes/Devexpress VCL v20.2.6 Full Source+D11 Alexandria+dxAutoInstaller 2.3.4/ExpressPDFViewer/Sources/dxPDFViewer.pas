...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, SysUtils, Classes, Buttons, Messages, Forms, Controls,
  ComCtrls, StdCtrls, Graphics, Generics.Defaults,
  Generics.Collections, dxCore, dxCoreClasses, cxClasses, dxCoreGraphics,
  cxGraphics, cxGeometry, cxListView,
  dxGDIPlusClasses, dxThreading, dxProtectionUtils, dxAnimation, dxFading,
  cxLookAndFeelPainters, cxLookAndFeels,
  cxControls, cxTreeView, dxImCtrl, dxCustomHint, dxCustomPreview, cxTextEdit,
  cxTrackBar, dxPDFCore, dxPDFDocument,
  dxPDFText, dxPDFCommandInterpreter, dxPDFFontUtils, dxPDFTypes,
  dxPDFSelection, dxPDFDocumentViewer,
...
type
  ...
  TdxPDFViewerGetFindPanelVisibilityEvent = function(Sender: TdxPDFCustomViewer)
    : Boolean of object;
  TdxPDFViewerOnAttachmentActionEvent = procedure(Sender: TdxPDFCustomViewer;
    AAttachment: TdxPDFFileAttachment; var AHandled: Boolean) of object;
  TdxPDFViewerOnHyperlinkClickEvent = procedure(Sender: TdxPDFCustomViewer;
    const AURI: string; var AHandled: Boolean) of object;
  TdxPDFViewerPrepareLockedStateImageEvent = procedure
    (Sender: TdxPDFCustomViewer; AImage: TcxBitmap32; var ADone: Boolean)
    of object;

  TdxPDFViewerNavigationPaneActivePage = (apNone, apThumbnails, apBookmarks,
    apAttachments);
  ...
  TdxPDFViewerFindPanelAlignment = (fpalTopClient, fpalTopRight, fpalTopCenter,
    fpalTopLeft, fpalBottomClient, fpalBottomRight, fpalBottomCenter,
    fpalBottomLeft);
  TdxPDFViewerPageLayout = (plDefault, plSinglePageContinuous);
  TdxPDFViewerViewStateChangeType = (vsctNone, vsctRotation, vsctZooming,
    vsctSelecting, vsctScrolling);
  ...
  protected
    ...
    property RenderContentInBackground: Boolean
      read GetRenderContentInBackground write SetRenderContentInBackground
      default False;
  published
    constructor Create(AOwner: TPersistent); override;
    property RenderContentDelay: Integer read GetRenderContentDelay
      write SetRenderContentDelay
  ...
  published
    constructor Create(AOwner: TPersistent); override;
    property HighlightCurrentPage: Boolean read GetHighlightCurrentPage
      write SetHighlightCurrentPage default True;
    property PageLayout: TdxPDFViewerPageLayout read FPageLayout
      write SetPageLayout default plDefault;
  end;
  ...
  published
    ...
    property Annotations: Boolean read FAnnotations write SetAnnotations
      default True;
  ...
  protected
    procedure DoAssign(ASource: TPersistent); override;
  published
    property MaxZoomFactor: Integer read GetMaxZoomFactor write SetMaxZoomFactor
      default dxPDFViewerMaxZoomFactor;
    property MinZoomFactor: Integer read GetMinZoomFactor write SetMinZoomFactor
      default dxPDFViewerMinZoomFactor;
    property ZoomFactor: Integer read GetZoomFactor write SetZoomFactor
      default dxPDFViewerDefaultZoomFactor;
    property ZoomMode: TdxPreviewZoomMode read GetZoomMode write SetZoomMode
      default pzmNone;
    property ZoomStep: Integer read GetZoomStep write SetZoomStep
      default dxPDFViewerZoomStep;
  end;
  ...
  protected
    ...
    property Direction: TdxPDFDocumentTextSearchDirection read GetDirection
      write SetDirection;
    property ShowCloseButton: Boolean read FShowCloseButton
      write SetShowCloseButton;
    property ShowNextButton: Boolean read FShowNextButton
      write SetShowNextButton;
    property ShowOptionsButton: Boolean read FShowOptionsButton
      write SetShowOptionsButton;
    property ShowPreviousButton: Boolean read FShowPreviousButton
      write SetShowPreviousButton;
  public
    property CaseSensitive: Boolean read GetCaseSensitive
      write SetCaseSensitive;
    property HighlightSearchResults: Boolean read FHighlightSearchResults
      write SetHighlightSearchResults;
  ...
  published
    property Alignment: TdxPDFViewerFindPanelAlignment read FAlignment
      write SetAlignment default fpalTopClient;
    property Animation: TdxPDFViewerFindPanelAnimation read FAnimation
      write SetAnimation default fpaSlide;
    property AnimationTime: Integer read FAnimationTime write SetAnimationTime
      default dxPDFViewerFindPanelDefaultAnimationTime;
    property ClearSearchStringOnClose: Boolean read FClearSearchStringOnClose
      write SetClearSearchStringOnClose default False;
    property DisplayMode: TdxPDFViewerFindPanelDisplayMode read FDisplayMode
      write SetDisplayMode default fpdmManual;
  end;
  ...
  strict private
    ...
    procedure EditKeyDownHandler(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  protected
    ...
    property AnimationController: TdxPDFViewerFindPanelAnimationController
      read FAnimationController;
  ...
  strict private
    function CreatePath(ASelection: TdxPDFTextHighlight;
      AExcludeTextSelection: Boolean = False): TdxGPPath;
    ...
    procedure DrawPath(ACanvas: TcxCanvas; APath: TdxGPPath;
      AColor, AFrameColor: TdxAlphaColor);
    procedure DrawSelection(ACanvas: TcxCanvas);
    procedure DrawTextSelection(ACanvas: TcxCanvas;
      ASelection: TdxPDFTextSelection);
  protected
  ...
  protected
    ...
    procedure DrawButton(ACanvas: TcxCanvas; const ARect: TRect;
      const ACaption: string; AState: TcxButtonState); virtual;
    procedure DrawButtonBackground(ACanvas: TcxCanvas; const ARect: TRect;
      AState: TcxButtonState); virtual;
    procedure DrawButtonGlyph(ACanvas: TcxCanvas; AImage: TdxSmartGlyph;
      const ARect: TRect; AState: TcxButtonState); overload;
    procedure DrawButtonGlyph(ACanvas: TcxCanvas; AImage: TdxSmartGlyph;
      const ARect: TRect; AState: TcxButtonState; AColorize: Boolean); overload;
    procedure DrawDropDownButton(ACanvas: TcxCanvas; const ARect: TRect;
      AState: TcxButtonState);
    procedure DrawDropDownButtonGlyph(ACanvas: TcxCanvas; AImage: TdxSmartGlyph;
    ...
    procedure DrawFindPanelBackground(ACanvas: TcxCanvas;
      const ARect: TRect); virtual;
    procedure DrawFindPanelCloseButton(ACanvas: TcxCanvas; const ARect: TRect;
      AState: TcxButtonState); virtual;
    procedure DrawFindPanelOptionsButtonGlyph(ACanvas: TcxCanvas;
      AImage: TdxSmartGlyph; const ARect: TRect;
      AState: TcxButtonState); virtual;
    procedure DrawFindPanelOptionsDropDownButton(ACanvas: TcxCanvas;
      const ARect: TRect; AState: TcxButtonState); virtual;
    procedure DrawNavigationPaneBackground(ACanvas: TcxCanvas;
      const ARect: TRect);
    procedure DrawNavigationPaneButton(ACanvas: TcxCanvas; const ARect: TRect;
      AState: TcxButtonState; AMinimized, ASelected, AIsFirst: Boolean);
    procedure DrawNavigationPaneButtonGlyph(ACanvas: TcxCanvas;
      AImage: TdxSmartGlyph; const ARect: TRect; AState: TcxButtonState);
    procedure DrawNavigationPanePageBackground(ACanvas: TcxCanvas;
      const ARect: TRect);
    procedure DrawNavigationPanePageButton(ACanvas: TcxCanvas;
      const ARect: TRect; AState: TcxButtonState);
    procedure DrawNavigationPanePageCaptionBackground(ACanvas: TcxCanvas;
      const ARect: TRect);
    procedure DrawNavigationPanePageToolbarBackground(ACanvas: TcxCanvas;
      const ARect: TRect);
    procedure DrawPageBackground(ACanvas: TcxCanvas;
      const ABorderRect, AContentRect: TRect; ASelected: Boolean); virtual;
  ...
  protected
    ...
    procedure CalculateVisiblePageRange(out AStartIndex,
      AEndIndex: Integer); override;
    procedure CalculateZoomFactorForPageWidthPreviewZoomMode(AFirstPageIndex,
      ALastPageIndex: Integer); override;
    procedure CalculateZoomFactorForPagesPreviewZoomMode(AFirstPageIndex,
      ALastPageIndex: Integer); override;
  ...
  TdxPDFCustomViewer = class(TdxPDFDocumentCustomViewer, IdxSkinSupport,
    IcxLockedStatePaint)
  strict private
  type
  ...
  strict private
    ...
    function GetOnCustomDrawPreRenderPageThumbnail
      : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent;
    function GetPainter: TdxPDFViewerPainter;
    ...
    procedure SetOnCustomDrawPreRenderPageThumbnail(const AValue
      : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent);
    ...
    procedure SetOptionsLockedStateImage
      (AValue: TdxPDFViewerLockedStateImageOptions);
    procedure SetOptionsNavigationPane(const AValue
    ...
    function ControllerFromPoint(const P: TPoint;
      var AController: TdxPDFViewerCustomController): Boolean; overload;
    ...
    function OnGetPasswordHandler(Sender: TObject; {$IFDEF BCBCOMPATIBLE}var
      {$ELSE}out {$ENDIF} APassword: string): Boolean;
    procedure OnDocumentLoadedHandler(Sender: TdxPDFDocument;
      const AInfo: TdxPDFDocumentLoadInfo);
  ...
  protected
    ...
    procedure CalculatePageNumberHintText(AStartPage, AEndPage: Integer;
    ...
    procedure DrawPageBackground(ACanvas: TcxCanvas; APage: TdxPreviewPage;
    ...
    procedure MakePageVisible(APageIndex: Integer; AAnimated: Boolean);
      override;
    ...
    procedure ProcessLeftClickByPage(Shift: TShiftState;
      X, Y: Integer); override;
    ...
    function CanOpenAttachment(AAttachment: TdxPDFFileAttachment)
      : Boolean; virtual;
    function CanOpenUri(const AURI: string): Boolean; virtual;
    function CanSaveAttachment(AAttachment: TdxPDFFileAttachment)
      : Boolean; virtual;
    ...
    function CreateLockedStatePaintHelper
      : TdxPDFViewerLockedStatePaintHelper; virtual;
    function CreateOptionsLockedStateImage
      : TdxPDFViewerLockedStateImageOptions; virtual;
    ...
    procedure FindNext;
    procedure SetScrollPosition(const APosition: TPoint;
      AAnimated: Boolean = False);
    ...
    property ActiveController: TdxPDFViewerCustomController
    ...
    property FocusedCellAsAttachment: TdxPDFFileAttachment
      read GetFocusedCellAsAttachment;
    property LayoutCalculator: TdxPDFViewerCustomLayoutCalculator
      read GetLayoutCalculator;
    property LockedStatePaintHelper: TdxPDFViewerLockedStatePaintHelper
      read FLockedStatePaintHelper;
    property NavigationPane: TdxPDFViewerNavigationPane read FNavigationPane;
    property OptionsBehavior: TdxPDFViewerOptionsBehavior read FOptionsBehavior
      write SetOptionsBehavior;
    property OptionsFindPanel: TdxPDFViewerOptionsFindPanel
      read GetOptionsFindPanel write SetOptionsFindPanel;
    property OptionsLockedStateImage: TdxPDFViewerLockedStateImageOptions
      read FOptionsLockedStateImage write SetOptionsLockedStateImage;
    property OptionsNavigationPane: TdxPDFViewerOptionsNavigationPane
      read GetOptionsNavigationPane write SetOptionsNavigationPane;
    property OptionsSelection: TdxPDFViewerOptionsSelection
      read FOptionsSelection write SetOptionsSelection;
    property OptionsView: TdxPDFViewerOptionsView read FOptionsView
      write SetOptionsView;
    property OptionsZoom: TdxPDFViewerOptionsZoom read FOptionsZoom
      write SetOptionsZoom;
    property Painter: TdxPDFViewerPainter read GetPainter;
    property SelectionController: TdxPDFViewerSelectionController
      read GetSelectionController;
    ...
    property OnAttachmentOpen: TdxPDFViewerOnAttachmentActionEvent
      read FOnAttachmentOpen write FOnAttachmentOpen;
    property OnAttachmentSave: TdxPDFViewerOnAttachmentActionEvent
      read FOnAttachmentSave write FOnAttachmentSave;
    property OnDocumentLoaded: TdxPDFDocumentLoadedEvent read FOnDocumentLoaded
      write FOnDocumentLoaded;
    property OnDocumentUnloaded: TNotifyEvent read FOnDocumentUnloaded
      write FOnDocumentUnloaded;
    property OnGetFindPanelVisibility: TdxPDFViewerGetFindPanelVisibilityEvent
      read FOnGetFindPanelVisibility write FOnGetFindPanelVisibility;
    property OnGetPassword: TdxGetPasswordEvent read FOnGetPassword
      write FOnGetPassword;
    property OnHideFindPanel: TNotifyEvent read FOnHideFindPanel
      write FOnHideFindPanel;
    property OnHyperlinkClick: TdxPDFViewerOnHyperlinkClickEvent
      read FOnHyperlinkClick write FOnHyperlinkClick;
    property OnPrepareLockedStateImage: TdxPDFViewerPrepareLockedStateImageEvent
      read FOnPrepareLockedStateImage write FOnPrepareLockedStateImage;
    property OnCustomDrawPreRenderPageThumbnail
      : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent
      read GetOnCustomDrawPreRenderPageThumbnail
      write SetOnCustomDrawPreRenderPageThumbnail;
    ...
    property OnShowFindPanel: TNotifyEvent read FOnShowFindPanel
      write FOnShowFindPanel;
  public
  ...
  protected
    ...
    property Selection: TdxPDFCustomSelection read GetSelection
      write SetSelection;
  public
    ...
    procedure Select(const ARect: TRect;
      AMakeVisible: Boolean = False); overload;
    procedure SelectText(const ARange: TdxPDFPageTextRange;
      AMakeVisible: Boolean = True); overload;
    procedure SelectAll;
  end;
  ...
  strict private
    ...
    property DocumentTextSearch: TdxPDFDocumentSequentialTextSearch
      read GetDocumentTextSearch;
  ...
  protected
    ...
    procedure EndUpdate;
  public
    function Find(const AText: string): TdxPDFDocumentTextSearchResult;
      overload;
    function Find(const AText: string;
      const AOptions: TdxPDFDocumentTextSearchOptions)
      : TdxPDFDocumentTextSearchResult; overload;
  ...
  public
    ...
    procedure Add(const ARange: TdxPDFPageTextRange;
      ABackColor, AFrameColor: TdxAlphaColor); overload;
    procedure Add(const ARanges: TdxPDFPageTextRanges;
      ABackColor, AFrameColor: TdxAlphaColor); overload;
  ...
  protected
    ...
    property HitObject: TdxPDFViewerCellViewInfo read FHitObject
      write FHitObject;
  end;
  ...
  strict private
    ...
    function TryGetDocumentHitObjectAsInteractiveObject
      (out AIntf: IdxPDFInteractiveObject): Boolean;
    function TryGetDocumentHitObjectAsHintableObject
      (out AIntf: IdxPDFHintableObject): Boolean;
  ...
  protected
    ...
    property DocumentHitObject: TdxPDFRecognizedObject read FDocumentHitObject
      write SetDocumentHitObject;
    ...
    property HitAtNavigationPaneSplitter: Boolean index hcNavigationPaneSplitter
      read GetHitCode;
  ...
  protected
    function DoGetCursor: TCursor; override;
    procedure DoCalculate(const AHitPoint: TPoint); override;
    property HitAtSplitter: Boolean index hcNavigationPaneSplitter
      read GetHitCode;
  end;
  ...
  protected
    ...
    procedure CalculateMouseButtonPressed(Shift: TShiftState;
      X, Y: Integer); virtual;
  ...
  public
    ...
    class function CalculatePageSize(const ASize: TdxPointF;
      ARotationAngle: TcxRotationAngle): TdxPointF; overload;
  end;
  ...
  strict private
    ...
    function CreateView(AChangeType: TdxPDFViewerViewStateChangeType)
      : TdxPDFViewerViewState;
    function GetCurrentViewState: TdxPDFViewerViewState;
  ...
  protected
    ...
    procedure GoToNextView;
    procedure GoToPrevView;
    procedure StoreCurrentViewState(AChangeType
      : TdxPDFViewerViewStateChangeType);
  ...
  strict private
    ...
    function CreateImageSelection(const ABounds: TdxRectF)
  ...
  strict private
    ...
    function CreateRangeList(const AStart, AEnd: TdxPDFTextPosition)
      : TdxPDFPageTextRanges; overload;
    function CreateRangeList(const AStart: TdxPDFTextPosition;
      const AEnd: TdxPDFPosition): TdxPDFPageTextRanges; overload;
    ...
    function IsPositionInLine(APageIndex, ALineIndex, AWordIndex,
      AOffset: Integer): Boolean;
    function FindLine(const APosition: TdxPDFTextPosition;
      out ALine: TdxPDFTextLine): Boolean;
    function FindNearestLineByDistance(const APosition: TdxPDFPosition)
      : TdxPDFTextLine;
    function FindNearestPosition(const APosition: TdxPDFPosition;
    ...
    function MoveRight(AWordParts: TdxPDFTextWordPartList;
      APageIndex, ALineIndex, AWordIndex, AOffset: Integer;
      AProcessLastWordPart: Boolean): Boolean; overload;
    function NormalizeDirection(ADirection: TdxPDFMovementDirection)
      : TdxPDFMovementDirection;
    function SameNextWordPartIndex(APageIndex, ALineIndex,
      AWordIndex: Integer): Boolean;
  ...
  protected
    ...
    function CreateTextSelection(const ARange: TdxPDFPageTextRange)
      : TdxPDFTextSelection; overload;
    function CreateTextSelection(const ARanges: TdxPDFPageTextRanges)
      : TdxPDFTextSelection; overload;
    function GetCaretViewData(const APosition: TdxPDFTextPosition)
      : TdxPDFDocumentCaretViewData;
    ...
    property TextLines[APageIndex: Integer]: TdxPDFTextLineList
      read GetTextLines;
  ...
  strict private
  type
    TMakeRectVisibleEvent = procedure(APageIndex: Integer;
      const ARect: TdxRectF) of object;
  strict private
    ...
    function IsEmptySelection: Boolean;
    function GetImageSelection(const AArea: TdxPDFDocumentArea)
      : TdxPDFImageSelection;
    function GetTextRanges(const AArea: TdxPDFDocumentArea)
      : TdxPDFPageTextRanges;
    function GetTextSelection(const AArea: TdxPDFDocumentArea)
      : TdxPDFTextSelection;
  ...
  protected
    ...
    property Page[AIndex: Integer]: TdxPDFPage read GetPage;
    property Selection: TdxPDFCustomSelection read FSelection
      write SetSelection;
    property ScaleFactor: TdxPointF read GetScaleFactor;
    property TextLines[APageIndex: Integer]: TdxPDFTextLineList
      read GetTextLines;

    property OnMakeRectVisible: TMakeRectVisibleEvent read FOnMakeRectVisible
      write FOnMakeRectVisible;
    property OnSelectionChanged: TNotifyEvent read FOnSelectionChanged
      write FOnSelectionChanged;
  public
  ...
  protected
    ...
    function ProcessChildKey(var AKey: Word; AShiftState: TShiftState)
      : Boolean; virtual;
    procedure Clear; virtual;
    ...
    function FindNextFocusableCell(ACell: TdxPDFViewerCellViewInfo;
    ...
    property FocusedCell: TdxPDFViewerCellViewInfo read FFocusedCell
    ...
    property PressedCell: TdxPDFViewerCellViewInfo read FPressedCell
...
{$ENDREGION}
  strict private
    ...
    procedure DoSaveAttachment(const APath: string;
    ...
    procedure OnMakeRectVisibleHandler(APageIndex: Integer;
  ...
  protected
    ...
    function GetPageScrollPosition(APage: TdxPDFViewerPage;
      AScrollPositionX, AScrollPositionY: Single): TdxPointF;
    function GetPageTopLeft(APage: TdxPDFViewerPage): TdxPointF;

    function AllowSelection: Boolean;
    function GetRecognitionObjects: TdxPDFRecognitionObjects;
    ...
    procedure ExecuteOperation(const AOperation: TdxPDFInteractiveOperation);
    procedure MakeRectVisible(const ARect: TdxRectF;
      AType: TdxVisibilityType = vtCentered; AIsTopAlignment: Boolean = False);
    procedure MakeSelectionRectVisible(APageIndex: Integer;
      const ARect: TdxRectF);
    procedure OffsetContent(const AOffset: TPoint);
    procedure Rotate(AAngleDelta: Integer);
    procedure SetActiveAndMakeVisiblePage(APageIndex: Integer;
      AAnimated: Boolean);
    procedure SetScrollPosition(const APosition: TPoint;
      AAnimated: Boolean = False);
    ...
    property DocumentHitTest: TdxPDFViewerDocumentHitTest
      read GetDocumentHitTest;
    property FocusedField: TdxPDFAcroFormField read FFocusedField
      write SetFocusedField;
    property SelectionController: TdxPDFViewerSelectionController
      read FSelectionController;
    property ViewStateHistoryController: TdxPDFViewerViewStateHistoryController
      read FViewStateHistoryController;
  end;
  ...
  protected
    ...
    function CalculateHitTest(AHitTest: TdxPDFViewerCellHitTest)
      : Boolean; virtual;
  ...
  protected
    ...
    function AddCell(ACellClass: TdxPDFViewerCellViewInfoClass)
      : TdxPDFViewerCellViewInfo; overload;
    function AddCell(ACellClass: TdxPDFViewerCellViewInfoClass;
      AController: TdxPDFViewerContainerController)
      : TdxPDFViewerCellViewInfo; overload;
    function AlignToTopClientSide(AViewInfo: TdxPDFViewerCellViewInfo;
      var R: TRect): Boolean;
    function AlignToLeftSide(AViewInfo: TdxPDFViewerCellViewInfo;
      var R: TRect): Boolean;
    function AlignToRightSide(AViewInfo: TdxPDFViewerCellViewInfo;
      var R: TRect): Boolean;
  ...
  protected
    ...
    property NavigationPane: TdxPDFViewerNavigationPaneViewInfo
      read GetNavigationPane;
  end;
  ...
  TdxPDFViewerButtonViewInfo = class(TdxPDFViewerCellViewInfo,
    IcxHintableObject)
  strict private
  ...
  protected
    ...
    procedure GetFadingImages(out AFadeOutImage, AFadeInImage
      : TcxBitmap); override;
  ...
  strict private
    ...
    procedure DrawFade(AGraphics: TdxGPGraphics;
      ALeft, ATop, AWidth, AHeight: Integer; AProgress: Byte);
    procedure DrawScrollDown(AGraphics: TdxGPGraphics;
      ALeft, ATop, AWidth, AHeight, AOffset: Integer);
    procedure DrawScrollDownFade(AGraphics: TdxGPGraphics;
      ALeft, ATop, AWidth, AHeight, AOffset: Integer);
    procedure DrawScrollUp(AGraphics: TdxGPGraphics;
      ALeft, ATop, AWidth, AHeight, AOffset: Integer);
    procedure DrawScrollUpFade(AGraphics: TdxGPGraphics;
      ALeft, ATop, AWidth, AHeight, AOffset: Integer);
    procedure PrepareImage(AImage: TGraphic);
  protected
    procedure Draw(ACanvas: TCanvas; const ADestRect: TRect); overload;
  public
    constructor Create(AImage: TGraphic; ATime: Cardinal;
      AMode: TdxDrawAnimationMode; AIsHiding: Boolean); reintroduce; virtual;
    destructor Destroy; override;
  end;
  ...
  private
    ...
    procedure RunAnimation(AImage: TBitmap; ATime: Cardinal;
...
{$REGION 'private types'}
    ...
    public
      constructor Create(AController: TdxPDFViewerContainerController);
        override;
    end;
...
{$ENDREGION}
  ...
  protected
    ...
    property ActualNextButtonViewInfo: TButtonViewInfo
      read GetActualNextButtonViewInfo;
  ...
  TdxPDFViewerInteractivityController = class(TdxPDFViewerCustomObject,
    IUnknown, IdxPDFInteractivityController)
  strict private
    // IUnknown
    ...
    procedure ExecuteActions(AAction: TdxPDFCustomAction;
      AExecutedActions: TList<TdxPDFCustomAction>);
  protected
    class procedure Execute(AViewer: TdxPDFCustomViewer;
      const AOperation: TdxPDFInteractiveOperation); overload; static;
    procedure ExecuteOperation(const AOperation
      : TdxPDFInteractiveOperation); overload;
    procedure ShowDocumentPosition(const ATarget: TdxPDFTarget);
  public
    class procedure Execute(AViewer: TdxPDFCustomViewer;
      AOutline: TdxPDFOutlineTreeItem); overload; static;
    class procedure Execute(AViewer: TdxPDFCustomViewer;
      AHyperlink: TdxPDFHyperlink); overload; static;
  end;

  { TdxPDFViewerBookmarkTreeView }
  TdxPDFViewerNavigationPaneInternalControlClass = class of
  ...
  TdxPDFViewerBookmarkTreeView = class
    (TdxPDFViewerNavigationPaneInternalControl)
  strict private
  type
{$REGION 'private types'}
...
{$ENDREGION}
  strict private
    ...
    procedure OnCustomDrawItemHandler(Sender: TCustomTreeView; Node: TTreeNode;
      State: TCustomDrawState; var DefaultDraw: Boolean);
    procedure OnMouseUpHandler(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure OnKeyDownHandler(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  TdxPDFViewerAttachmentFileList = class
    (TdxPDFViewerNavigationPaneInternalControl)
  strict private
    ...
    procedure OnContextPopupHandler(Sender: TObject; MousePos: TPoint;
      var Handled: Boolean);
  ...
  strict protected
    ...
    procedure UpdateInternalControlTextSize; override;
  protected
    property DocumentAttachments: TdxPDFFileAttachmentList
      read GetDocumentAttachments;
  ...
  TdxPDFViewerNavigationPanePageCaptionButton = class
    (TdxPDFViewerButtonViewInfo)
  protected
    function CanFocus: Boolean; override;
  ...
  TdxPDFViewerNavigationPanePageButtonViewInfo = class
    (TdxPDFViewerButtonViewInfo)
  strict private
  ...
  public
    constructor Create(AController: TdxPDFViewerContainerController;
      APage: TdxPDFViewerNavigationPanePageViewInfo); reintroduce;
  end;

  { TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo }

  TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo = class
    (TdxPDFViewerCustomDropDownButtonViewInfo)
  protected
  ...
  TdxPDFViewerNavigationPanePageToolBarViewInfo = class
    (TdxPDFViewerContainerViewInfo)
  strict private
  ...
  TdxPDFViewerNavigationPanePageViewInfoClass = class of
    TdxPDFViewerNavigationPanePageViewInfo;
...
{$REGION 'private types'}
    ...
    strict private
      ...
      function AlignToClient(AViewInfo: TdxPDFViewerCellViewInfo;
        var R: TRect): Boolean;
    protected
      function GetContentMargins: TRect; override;
      function MeasureHeight: Integer; override;
      ...
      property OnGetText: THeaderOnGetTextEvent read GetOnGetText
        write SetOnGetText;
    end;
{$ENDREGION}
  strict private
  ...
  public
    constructor Create(AController: TdxPDFViewerContainerController;
      APage: TdxPDFViewerNavigationPanePage); reintroduce;
  end;

  { TdxPDFViewerBookmarksPageViewInfo }

  TdxPDFViewerBookmarksPageViewInfo = class
    (TdxPDFViewerNavigationPanePageViewInfo)
  strict private
  type
{$REGION 'private types'}
...
{$ENDREGION}
  ...
  TdxPDFViewerThumbnailsPageViewInfo = class
    (TdxPDFViewerNavigationPanePageViewInfo)
  strict private
  type
{$REGION 'private types'}
...
{$ENDREGION}
  ...
  TdxPDFViewerAttachmentsPageViewInfo = class
    (TdxPDFViewerNavigationPanePageViewInfo)
  strict private
  type
{$REGION 'private types'}
...
{$ENDREGION}
  ...
  strict private
    ...
    function GetOnCustomDrawPreRenderPage
      : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent;
  ...
  protected
    ...
    property SelectedPageIndex: Integer read GetSelectedPageIndex
      write SetSelectedPageIndex;
    property Size: Integer read GetSize write SetSize;

    property OnCustomDrawPreRenderPage
      : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent
      read GetOnCustomDrawPreRenderPage write SetOnCustomDrawPreRenderPage;
    property OnSizeChanged: TNotifyEvent read GetOnSizeChanged
      write SetOnSizeChanged;
  end;
  ...
  protected
    ...
    function GetPageIndex(AButton
      : TdxPDFViewerNavigationPanePageButtonViewInfo): Integer;

    property ActivePage: TdxPDFViewerNavigationPanePageViewInfo
      read GetActivePage;
    property ButtonsBounds: TRect read FButtonsBounds;
    property FirstPageButton: TdxPDFViewerNavigationPanePageButtonViewInfo
      read GetFirstPageButton;
    property SplitterBounds: TRect read FSplitterBounds;
  public
    constructor Create(AController: TdxPDFViewerContainerController;
      ANavigationPane: TdxPDFViewerNavigationPane); reintroduce;
  end;
  ...
  published
    property Glyph;
    property HideAfterUse: Boolean read FHideAfterUse write SetHideAfterUse
      default False;
    property TextSize: TdxPDFViewerBookmarksTextSize read FTextSize
      write SetTextSize default btsSmall;
  ...
  protected
    ...
    function CreateViewInfo: TdxPDFViewerNavigationPanePageViewInfo;
      virtual; abstract;
    function GetCaption: string; virtual; abstract;
    function GetControlClass: TdxPDFViewerNavigationPaneInternalControlClass;
      virtual; abstract;
    ...
    property Empty: Boolean read GetEmpty;
    property Glyph: TdxSmartGlyph read GetGlyph write SetGlyph;
    property Options: TdxPDFViewerOptionsNavigationPage read FOptions
      write SetOptions;
  ...
  protected
    ...
    function GetControlClass
      : TdxPDFViewerNavigationPaneInternalControlClass; override;
    procedure CreateSubClasses; override;
    procedure DestroySubClasses; override;
    ...
    property ThumbnailPreview: TdxPDFViewerPageThumbnailPreview
      read GetThumbnailPreview;
  public
  ...
  protected
    procedure DoAssign(ASource: TPersistent); override;
  public
    property ActivePage: TdxPDFViewerNavigationPaneActivePage read GetActivePage
      write SetActivePage default apNone;
    property ActivePageState: TWindowState read GetActivePageState
      write SetActivePageState default wsMinimized;
  published
    property Attachments: TdxPDFViewerOptionsAttachments read GetAttachments
      write SetAttachments;
    property Bookmarks: TdxPDFViewerOptionsBookmarks read GetBookmarks
      write SetBookmarks;
    property Thumbnails: TdxPDFViewerOptionsThumbnails read GetThumbnails
      write SetThumbnails;
  ...
  protected
    ...
    property SelectedAttachment: TdxPDFFileAttachment
      read GetSelectedAttachment;
  ...
  strict private
    ...
    procedure SetActivePageType(const AValue
  ...
  protected
    ...
    property ActivePage: TdxPDFViewerNavigationPanePage read FActivePage
      write SetActivePage;
    property ActivePageState: TWindowState read FActivePageState
      write SetActivePageState;
    property ActivePageType: TdxPDFViewerNavigationPaneActivePage
      read GetActivePageType write SetActivePageType;
    ...
    property Options: TdxPDFViewerOptionsNavigationPane read FOptions
      write SetOptions;
    ...
    property VisiblePages: TList<TdxPDFViewerNavigationPanePage>
      read GetVisiblePages;
  public
    constructor Create(AViewer: TdxPDFCustomViewer);
    destructor Destroy; override;
  end;
...
uses
  Math, Clipbrd, Variants, Contnrs, Dialogs, ShellApi, IOUtils, dxTypeHelpers,
  cxLibraryConsts, dxPrinting,
  dxGDIPlusAPI, cxEdit, cxContainer, cxSplitter, dxPDFBase, dxPDFUtils,
  dxPDFViewerPopupMenu, dxPDFViewerPasswordDialog,
...
const
  dxPDFViewerFindPanelContentMargins: TRect = (Left: 9; Top: 9; Right: 9;
    Bottom: 9);
  ...
  dxPDFViewerNavigationPaneContentMargins: TRect = (Left: 2; Top: 2; Right: 2;
    Bottom: 2);
  dxPDFViewerNavigationPageToolbarButtonHeight = 26;
  dxPDFViewerBookmarksPageContentMargins: TRect = (Left: 7; Top: 7; Right: 7;
    Bottom: 7);
...
type
  ...
  TdxPDFDocumentSequentialTextSearchAccess = class
    (TdxPDFDocumentSequentialTextSearch);
  TdxPDFDocumentViewerCustomRendererAccess = class
  ...
  TdxPDFFileAttachmentAnnotationFieldAccess = class
  ...
  TdxPDFViewerAutomaticLayoutCalculator = class
    (TdxPDFViewerCustomLayoutCalculator)
  public
  ...
  TdxPDFViewerColumnContinuousCalculator = class
    (TdxPDFViewerCustomLayoutCalculator)
  strict private
    FColumnCount: Integer;
  strict protected
    function CanAddRow(AColumnIndex: Integer; const ABounds: TRect;
      const APageSize: TPoint): Boolean; override;
    procedure AlignPagesToHorizontalCenter(const ABounds: TRect); override;
  public
    constructor Create(APreview: TdxCustomPreview; AColumnCount: Integer);
      reintroduce;
  ...
  TdxPDFViewerResizeNavigationPaneDragAndDropObject = class
...
function TdxPDFViewerCustomLayoutCalculator.GetPageSiteBounds
  (APageIndex: Integer): TRect;
begin
  Result := Pages[APageIndex].Bounds;
end;

procedure TdxPDFViewerCustomLayoutCalculator.CalculateVisiblePageRange
...
begin
  ...
  begin
    AStartIndex := 0;
    while (AStartIndex < PageCount) and not Pages.Items[AStartIndex]
      .PartVisible do
      Inc(AStartIndex);
...
procedure TdxPDFViewerCustomLayoutCalculator.
  CalculateZoomFactorForPageWidthPreviewZoomMode(AFirstPageIndex,
  ...
  begin
    ...
    for I := AFirstIndex to ALastIndex do
      Result := Max(Result,
        ScaleFactor.Apply(Pages[I].PageSize.ActualSizeInPixels.X));
  end;

begin
  ZoomFactor := MulDiv(100, PagesArea.Width, GetMaxPageSize(0, PageCount - 1));
end;

procedure TdxPDFViewerCustomLayoutCalculator.
  CalculateZoomFactorForPagesPreviewZoomMode(AFirstPageIndex,
  ALastPageIndex: Integer);
begin
  ZoomFactor := MulDiv(100, PagesArea.Height,
    ScaleFactor.Apply(Viewer.ActivePage.PageSize.ActualSizeInPixels.Y));
end;
...
procedure TdxPDFViewerCustomLayoutCalculator.ScrollContent
...
begin
  if Viewer.ViewerController <> nil then
    Viewer.ViewerController.ViewStateHistoryController.StoreCurrentViewState
      (vsctZooming);
end;
...
function TdxPDFViewerCustomLayoutCalculator.GetPageIndexByRect
...
function TdxPDFViewerCustomLayoutCalculator.GetVisibleSelectedPageIndex
  : Integer;
begin
...
procedure TdxPDFViewerCustomLayoutCalculator.ActivateVisiblePage
  (ACheckZoomLocking: Boolean = True);
begin
...
procedure TdxPDFViewerCustomLayoutCalculator.SetCurrentPageByRect
  (const ARect: TRect);
begin
...
procedure TdxPDFViewerCustomLayoutCalculator.CalculateScrollBarPosition
  (var ALeftTop: TPoint);
var
  AScale: Single;
...
begin
  ...
  if not Viewer.HitTest.HitAtDocumentViewer then
    AAnchorPoint := cxPoint(Viewer.GetPagesArea.Width div 2,
      Viewer.GetPagesArea.Height div 2);
...
procedure TdxPDFViewerAutomaticLayoutCalculator.ScrollContent
  (const AOffset: TPoint);
begin
...
constructor TdxPDFViewerColumnContinuousCalculator.Create
  (APreview: TdxCustomPreview; AColumnCount: Integer);
begin
  inherited Create(APreview);
  FColumnCount := AColumnCount;
end;

procedure TdxPDFViewerColumnContinuousCalculator.ScrollContent
  (const AOffset: TPoint);
begin
  inherited ScrollContent(AOffset);
  ActivateVisiblePage;
...
function TdxPDFViewerColumnContinuousCalculator.CanAddRow(AColumnIndex: Integer;
  const ABounds: TRect; const APageSize: TPoint): Boolean;
begin
  Result := AColumnIndex = FColumnCount;
end;

procedure TdxPDFViewerColumnContinuousCalculator.AlignPagesToHorizontalCenter
  (const ABounds: TRect);
var
...
begin
  ...
  for I := 0 to PageCount - 1 do
  begin
    TdxPreviewPageListAccess(Pages).CenterPagesInRow(PagesRows[I],
      ABoundsWidth);
...
begin
  ...
  TdxSplitterDragImageAccess(FDragImage).Canvas.Brush.Bitmap :=
    AllocPatternBitmap(clBlack, clWhite);
  Viewer.NavigationPane.RestorePage;
  FDragImage.BoundsRect := GetDragImageBounds;
  FDragImage.Show;
end;

procedure TdxPDFViewerResizeNavigationPaneDragAndDropObject.DragAndDrop
...
procedure TdxPDFViewerResizeNavigationPaneDragAndDropObject.EndDragAndDrop
  (Accepted: Boolean);
const
...
begin
  if Accepted then
  begin
    R := cxRectOffset(GetDragImageBounds,
      Control.ClientToScreen(cxNullPoint), False);
    if R.Right = Viewer.ClientBounds.Right then
      Viewer.NavigationPane.MaximizePage
    else if R.Left = Viewer.ViewInfo.NavigationPane.ButtonsBounds.Right then
      Viewer.NavigationPane.MinimizePage
    else
      Viewer.NavigationPane.PageSize := Viewer.NavigationPane.PageSize +
        (R.Left - Viewer.ViewInfo.NavigationPane.SplitterBounds.Left) *
        Sign[True];
  end;
  inherited EndDragAndDrop(Accepted);
end;

function TdxPDFViewerResizeNavigationPaneDragAndDropObject.
  GetDragDropArea: TRect;
var
  APadding: Integer;
begin
  APadding := GetPadding;
...
function TdxPDFViewerResizeNavigationPaneDragAndDropObject.GetDragAndDropCursor
...
function TdxPDFViewerResizeNavigationPaneDragAndDropObject.
  GetDragImageBounds: TRect;
var
  R: TRect;
  AOffset: TPoint;
begin
  AOffset := cxPointOffset(GetMouseCursorPos, FStartMousePos, False);
  Result := cxRectOffset(Viewer.ViewInfo.NavigationPane.SplitterBounds,
    Control.ClientToScreen(cxNullPoint));
  ...
  else if Result.Right > R.Right then
...
function TdxPDFViewerResizeNavigationPaneDragAndDropObject.GetViewer
  : TdxPDFCustomViewer;
begin
...
procedure TdxPDFViewerOptionsBehavior.SetRenderContentDelay
  (const AValue: Integer);
begin
  Viewer.RenderContentDelay := AValue;
end;

procedure TdxPDFViewerOptionsBehavior.SetRenderContentInBackground
...
procedure TdxPDFViewerOptionsView.SetHighlightCurrentPage
  (const AValue: Boolean);
var
...
procedure TdxPDFViewerOptionsView.SetPageLayout(const AValue
...
constructor TdxPDFViewerFindPanelTextEdit.Create
  (AFindPanel: TdxPDFViewerFindPanel);
begin
...
function TdxPDFViewerFindPanelTextEdit.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  if PtInRect(FFindPanel.ViewInfo.FindPanel.Edit.Bounds, GetMouseCursorClientPos)
  then
    Result := inherited DoMouseWheel(Shift, WheelDelta, MousePos)
  else
    Result := TdxPDFViewerAccess(FFindPanel.Viewer).ProcessMouseWheelMessage
      (WheelDelta);
end;
...
begin
  ...
  begin
    if Focused then
      FFindPanel.Viewer.ViewerController.FocusedCell :=
        FFindPanel.Viewer.ViewInfo.FindPanel.Edit
    else
      FFindPanel.Viewer.ViewerController.FocusedCell :=
        FFindPanel.Viewer.ViewInfo;
...
procedure TdxPDFViewerFindPanelInnerTextEdit.WMGetDlgCode
...
function TdxPDFViewerOptionsFindPanel.GetDirection
...
procedure TdxPDFViewerOptionsFindPanel.SetDirection(const AValue
  : TdxPDFDocumentTextSearchDirection);
begin
...
procedure TdxPDFViewerOptionsFindPanel.SetAlignment(const AValue
...
procedure TdxPDFViewerOptionsFindPanel.SetAnimation(const AValue
...
procedure TdxPDFViewerOptionsFindPanel.SetClearSearchStringOnClose
  (const AValue: Boolean);
begin
...
procedure TdxPDFViewerOptionsFindPanel.SetDisplayMode
  (const AValue: TdxPDFViewerFindPanelDisplayMode);
const
  ...
  begin
    ...
    begin
      if DisplayMode in [fpdmNever, fpdmAlways] then
        Viewer.FindPanel.Visible := VisibilityMap[DisplayMode] and
          Viewer.IsDocumentLoaded
    end
    else
      Viewer.FindPanel.Visible := (DisplayMode = fpdmAlways) and
        Viewer.IsDocumentLoaded;
  end;
end;

procedure TdxPDFViewerOptionsFindPanel.SetHighlightSearchResults
  (const AValue: Boolean);
begin
...
procedure TdxPDFViewerOptionsFindPanel.SetShowCloseButton
  (const AValue: Boolean);
begin
...
procedure TdxPDFViewerOptionsFindPanel.SetShowOptionsButton
  (const AValue: Boolean);
begin
...
procedure TdxPDFViewerOptionsFindPanel.SetShowPreviousButton
  (const AValue: Boolean);
begin
...
begin
  ...
  FOptionsButtonGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERFINDPANELOPTIONSBUTTON', 'PNG');
...
procedure TdxPDFViewerFindPanel.SetEdit(const AValue
  : TdxPDFViewerFindPanelTextEdit);
begin
  InternalSetEdit(AValue);
end;

procedure TdxPDFViewerFindPanel.SetOptions(const AValue
  : TdxPDFViewerOptionsFindPanel);
begin
...
procedure TdxPDFViewerFindPanel.InternalSetEdit(const AValue
  : TdxPDFViewerFindPanelTextEdit);
begin
...
procedure TdxPDFViewerFindPanel.EditKeyPressHandler(Sender: TObject;
  var Key: Char);
begin
  HideBeep(Key);
end;

procedure TdxPDFViewerFindPanel.EditKeyDownHandler(Sender: TObject;
...
begin
  Result := inherited GetVisible or cxRectIntersect(Bounds,
    TdxPDFViewerAccess(Preview).GetPagesArea);
end;

procedure TdxPDFViewerPage.CalculatePageSize;
begin
  PageSize.Size := GetPageSize(TdxPDFViewerAccess(Preview)
    .DocumentToViewerFactor);
  PageSize.MinUsefulSize := PageSize.Size;
end;

function TdxPDFViewerPage.CreatePath(ASelection: TdxPDFTextHighlight;
  AExcludeTextSelection: Boolean = False): TdxGPPath;

  function IsTextSelection: Boolean;
  begin
    Result := not Viewer.Selection.IsEmpty and
      (Viewer.Selection.Selection.HitCode = hcTextSelection);
  end;

  function InflateSelectionRect(const ARect: TdxRectF): TdxRectF;
  begin
    Result := cxRectInflate(ARect, DocumentState.ScaleFactor.X,
      DocumentState.ScaleFactor.X);
  end;
...
begin
  ...
  begin
    if IsTextSelection then
      ATextSelectionRects := TdxPDFTextHighlightAccess
        (Viewer.Selection.Selection).PageRects[Index]
    else
      ATextSelectionRects := nil;
    Result.FigureStart;
    try
      for R in AHighlights do
        if not AExcludeTextSelection or
          ((ATextSelectionRects <> nil) and not ATextSelectionRects.Contains(R)
          or (ATextSelectionRects = nil)) then
...
function TdxPDFViewerPage.GetSelectionBackColor(AColor: TdxAlphaColor)
  : TdxAlphaColor;
begin
...
function TdxPDFViewerPage.GetSelectionFrameColor(AColor: TdxAlphaColor)
  : TdxAlphaColor;
begin
  if AColor = dxacDefault then
...
begin
  Painter.DrawPageBackground(ACanvas, SiteBounds, Bounds,
    Viewer.CanHighlightCurrentPage(Selected));
end;

procedure TdxPDFViewerPage.DrawContent(ACanvas: TcxCanvas);
begin
  TdxPDFDocumentViewerCustomRendererAccess(Viewer.Renderer)
    .DrawPage(ACanvas, Index, Bounds);
end;
...
begin
  if (Viewer.ViewerController.FocusedField <> nil) and
    Supports(Viewer.ViewerController.FocusedField, IdxPDFInteractiveObject,
    AIntf) then
  begin
...
procedure TdxPDFViewerPage.DrawHighlights(ACanvas: TcxCanvas);
  ...
  begin
    ...
    try
      DrawPath(ACanvas, APath,
        GetBackColor(TdxPDFTextHighlightAccess(AHighlight).BackColor),
  ...
  begin
    Result := (Viewer.Highlights.Visible = bTrue) or
      (Viewer.Highlights.Visible = bDefault) and Viewer.IsFindPanelVisible;
  end;
...
procedure TdxPDFViewerPage.DrawImageSelection(ACanvas: TcxCanvas;
  ASelection: TdxPDFImageSelection);
var
  APath: TdxGPPath;
begin
  ...
  begin
    ...
    try
      ...
      APath.FigureFinish;
      DrawPath(ACanvas, APath, GetSelectionBackColor(ASelection.BackColor),
        GetSelectionFrameColor(ASelection.FrameColor));
    finally
      APath.Free;
    end;
  end;
...
procedure TdxPDFViewerPage.DrawPath(ACanvas: TcxCanvas; APath: TdxGPPath;
  AColor, AFrameColor: TdxAlphaColor);
var
...
procedure TdxPDFViewerPage.DrawTextSelection(ACanvas: TcxCanvas;
  ASelection: TdxPDFTextSelection);
var
  APath: TdxGPPath;
begin
...
begin
  Result := LookAndFeelPainter.GetScaledDropDownButtonRightPartSize
    (ScaleFactor);
end;
...
begin
  Result := LookAndFeelPainter.PDFViewerNavigationPaneButtonContentOffsets
    (ScaleFactor);
end;

function TdxPDFViewerPainter.NavigationPaneButtonOverlay: TPoint;
begin
  Result := LookAndFeelPainter.PDFViewerNavigationPaneButtonOverlay
    (ScaleFactor);
end;
...
begin
  Result := LookAndFeelPainter.PDFViewerNavigationPanePageCaptionContentOffsets
    (ScaleFactor);
end;
...
begin
  Result := LookAndFeelPainter.PDFViewerNavigationPanePageToolbarContentOffsets
    (ScaleFactor);
end;

function TdxPDFViewerPainter.NavigationPaneContentOffsets: TRect;
begin
  Result := LookAndFeelPainter.PDFViewerNavigationPaneContentOffsets
    (ScaleFactor);
end;

function TdxPDFViewerPainter.NavigationPanePageContentOffsets: TRect;
begin
  Result := LookAndFeelPainter.PDFViewerNavigationPanePageContentOffsets
    (ScaleFactor);
end;
...
begin
  Result := dxColorToAlphaColor
    (LookAndFeelPainter.PDFViewerSelectionColor, 127);
end;
...
procedure TdxPDFViewerPainter.DrawButton(ACanvas: TcxCanvas; const ARect: TRect;
  const ACaption: string; AState: TcxButtonState);
begin
  LookAndFeelPainter.DrawScaledButton(ACanvas, ARect, ACaption, AState,
    ScaleFactor);
end;

procedure TdxPDFViewerPainter.DrawButtonBackground(ACanvas: TcxCanvas;
  const ARect: TRect; AState: TcxButtonState);
begin
  DrawButton(ACanvas, ARect, '', AState);
end;

procedure TdxPDFViewerPainter.DrawButtonGlyph(ACanvas: TcxCanvas;
  AImage: TdxSmartGlyph; const ARect: TRect; AState: TcxButtonState);
begin
  cxDrawImage(ACanvas, ARect, AImage, nil, -1, AState <> cxbsDisabled, nil,
    ScaleFactor);
end;
...
procedure TdxPDFViewerPainter.DrawDropDownButton(ACanvas: TcxCanvas;
  const ARect: TRect; AState: TcxButtonState);
begin
  LookAndFeelPainter.DrawScaledScrollBarArrow(ACanvas, ARect, AState, adDown,
    ScaleFactor);
end;

procedure TdxPDFViewerPainter.DrawDropDownButtonGlyph(ACanvas: TcxCanvas;
  AImage: TdxSmartGlyph; const ARect: TRect; AState: TcxButtonState;
  AColorize: Boolean = True);
begin
  DrawButtonGlyph(ACanvas, AImage, ARect, AState, AColorize);
end;

procedure TdxPDFViewerPainter.DrawFocusRect(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
  ACanvas.DrawFocusRect(LookAndFeelPainter.ScaledButtonFocusRect(ACanvas, ARect,
    ScaleFactor));
end;

procedure TdxPDFViewerPainter.DrawFindPanelBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
const
...
procedure TdxPDFViewerPainter.DrawFindPanelCloseButton(ACanvas: TcxCanvas;
  const ARect: TRect; AState: TcxButtonState);
begin
  LookAndFeelPainter.DrawScaledFilterCloseButton(ACanvas, ARect, AState,
    ScaleFactor);
end;

procedure TdxPDFViewerPainter.DrawFindPanelOptionsButtonGlyph
  (ACanvas: TcxCanvas; AImage: TdxSmartGlyph; const ARect: TRect;
  AState: TcxButtonState);
begin
  AImage.ChangeColor(ButtonSymbolColor(AState));
...
procedure TdxPDFViewerPainter.DrawFindPanelOptionsDropDownButton
...
procedure TdxPDFViewerPainter.DrawNavigationPaneBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
  LookAndFeelPainter.PDFViewerDrawNavigationPaneBackground(ACanvas, ARect,
    ScaleFactor);
end;

procedure TdxPDFViewerPainter.DrawNavigationPaneButton(ACanvas: TcxCanvas;
  const ARect: TRect; AState: TcxButtonState;
  AMinimized, ASelected, AIsFirst: Boolean);
begin
  LookAndFeelPainter.PDFViewerDrawNavigationPaneButton(ACanvas, ARect, AState,
    ScaleFactor, AMinimized, ASelected, AIsFirst);
end;

procedure TdxPDFViewerPainter.DrawNavigationPaneButtonGlyph(ACanvas: TcxCanvas;
  AImage: TdxSmartGlyph; const ARect: TRect; AState: TcxButtonState);
begin
  cxDrawImage(ACanvas, ARect, AImage, nil, -1, True,
    LookAndFeelPainter.PDFViewerNavigationPaneButtonColorPalette(AState),
    ScaleFactor);
end;

procedure TdxPDFViewerPainter.DrawNavigationPanePageBackground
  (ACanvas: TcxCanvas; const ARect: TRect);
begin
  LookAndFeelPainter.PDFViewerDrawNavigationPanePageBackground(ACanvas, ARect);
end;

procedure TdxPDFViewerPainter.DrawNavigationPanePageButton(ACanvas: TcxCanvas;
  const ARect: TRect; AState: TcxButtonState);
begin
  if AState in [cxbsHot, cxbsPressed, cxbsDisabled] then
    LookAndFeelPainter.PDFViewerDrawNavigationPanePageButton(ACanvas, ARect,
      AState, ScaleFactor);
end;

procedure TdxPDFViewerPainter.DrawNavigationPanePageCaptionBackground
  (ACanvas: TcxCanvas; const ARect: TRect);
begin
  LookAndFeelPainter.PDFViewerDrawNavigationPanePageCaptionBackground
    (ACanvas, ARect);
end;

procedure TdxPDFViewerPainter.DrawNavigationPanePageToolbarBackground
  (ACanvas: TcxCanvas; const ARect: TRect);
begin
  LookAndFeelPainter.PDFViewerDrawNavigationPanePageToolbarBackground
    (ACanvas, ARect);
end;

procedure TdxPDFViewerPainter.DrawPageBackground(ACanvas: TcxCanvas;
  const ABorderRect, AContentRect: TRect; ASelected: Boolean);
begin
  LookAndFeelPainter.DrawPrintPreviewPageBackground(ACanvas, ABorderRect,
    AContentRect, ASelected, True);
end;
...
function TdxPDFViewerLockedStatePaintHelper.GetOptions
...
begin
  Result := not FIsDocumentClearing and (FDocument <> nil) and
    TdxPDFDocumentAccess(FDocument).IsLoaded;
end;
...
begin
  ...
  begin
    ViewerController.FocusedField := nil;
...
function TdxPDFCustomViewer.CanProcessScrollEvents
  (var Message: TMessage): Boolean;
begin
  Result := inherited CanProcessScrollEvents(Message) and not IsDesigning;
end;

function TdxPDFCustomViewer.CreateLayoutCalculator
...
begin
  if (FDocument <> nil) and (TdxPDFDocumentAccess(FDocument).Pages <> nil) and
    (FDocument.PageCount > 0) then
  begin
...
function TdxPDFCustomViewer.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
var
...
function TdxPDFCustomViewer.GetDragAndDropObjectClass
...
function TdxPDFCustomViewer.GetPageSizeOptionsClass
  : TdxPreviewPageSizeOptionsClass;
begin
...
procedure TdxPDFCustomViewer.CalculatePageNumberHintText(AStartPage,
  AEndPage: Integer; var AText: string);
begin
...
procedure TdxPDFCustomViewer.DoContextPopup(MousePos: TPoint;
  var Handled: Boolean);
...
begin
  ...
  begin
    ...
    if CanShowContextPopup then
      Handled := ActiveController.ContextPopup
        (ClientToScreen(GetMouseCursorClientPos));
...
begin
  if not OptionsLockedStateImage.Enabled or OptionsLockedStateImage.Enabled and
    not FIsDocumentLoading then
    inherited DrawNoPages(ACanvas);
end;

procedure TdxPDFCustomViewer.DrawPageBackground(ACanvas: TcxCanvas;
  APage: TdxPreviewPage; ASelected: Boolean);
begin
  inherited DrawPageBackground(ACanvas, APage,
    CanHighlightCurrentPage(ASelected));
end;
...
begin
  if IsDocumentLoaded and not TextSearch.IsLocked and
    not FController.KeyDown(Key, Shift) then
...
begin
  if IsDocumentLoaded and not TextSearch.IsLocked then
    ...
    else if ViewInfo.IsFocused then
...
begin
  ...
  try
    ...
    EndScrolling;
    ViewerController.ViewStateHistoryController.StoreCurrentViewState
      (vsctScrolling);
  end;
end;

procedure TdxPDFCustomViewer.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  AController: TdxPDFViewerCustomController;
begin
...
begin
  ...
  if not ViewInfo.FindPanel.Edit.IsFocused and (DragAndDropState <> ddsInProcess)
  then
  begin
    UpdateActiveController(GetMouseCursorClientPos);
...
procedure TdxPDFCustomViewer.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  AController: TdxPDFViewerCustomController;
begin
...
procedure TdxPDFCustomViewer.ProcessLeftClickByPage(Shift: TShiftState;
  X, Y: Integer);
begin
  // do nothing
end;
...
procedure TdxPDFCustomViewer.Scroll(AScrollBarKind: TScrollBarKind;
...
begin
  ...
  begin
    ...
    try
      ...
      if (APreviousPageIndex = GetTargetSelectedPageIndex) and
        (APreviousPageIndex = PageCount - 1) then
        ScrollToDocumentFooter
      else
        MakeVisible(CurrentPageIndex);
    finally
...
begin
  if IsScrollAnimationEnabled then
  begin
    if (GetTargetSelectedPageIndex = PageCount - 1) and not PageList.Last.HeaderPartVisible
    then
...
function TdxPDFCustomViewer.CanOpenAttachment(AAttachment
  : TdxPDFFileAttachment): Boolean;
begin
...
function TdxPDFCustomViewer.CanSaveAttachment(AAttachment
  : TdxPDFFileAttachment): Boolean;
begin
...
function TdxPDFCustomViewer.CreateLockedStatePaintHelper
  : TdxPDFViewerLockedStatePaintHelper;
begin
  Result := TdxPDFViewerLockedStatePaintHelper.Create(Self);
end;

function TdxPDFCustomViewer.CreateOptionsLockedStateImage
  : TdxPDFViewerLockedStateImageOptions;
begin
...
begin
  Result := (OptionsFindPanel.DisplayMode = fpdmManual) or
    Assigned(OnShowFindPanel) and Assigned(OnHideFindPanel) and
    Assigned(OnGetFindPanelVisibility);
end;

function TdxPDFCustomViewer.CanHighlightCurrentPage(AIsPageActive
  : Boolean): Boolean;
begin
...
begin
  Result := IsDocumentLoaded and (OptionsFindPanel.DisplayMode <> fpdmNever) and
    not TextSearch.IsLocked;
end;

function TdxPDFCustomViewer.CanHideFindPanel: Boolean;
begin
  Result := not TextSearch.IsLocked and
    (not IsDestroying and not IsDocumentLoaded and IsFindPanelVisible or
...
begin
  Result := IsDocumentAvailable and not FindPanel.AnimationController.Active and
    SelectionController.Caret.IsValid and
    (not IsFindPanelVisible or IsFindPanelVisible and
    not FindPanel.Edit.Focused);
end;

function TdxPDFCustomViewer.CanPrint: Boolean;
begin
  Result := dxPrintingRepository.CanBuildReport(Self) and IsDocumentLoaded and
    Document.AllowPrinting;
end;

function TdxPDFCustomViewer.CanUseAnimation: Boolean;
begin
  Result := not IsDesigning and Visible and HandleAllocated and
    IsWindowVisible(Handle) and (OptionsFindPanel.AnimationTime > 0);
end;
...
begin
  Result := nil;
  if (ViewerController.FocusedField <> nil) and
    (ViewerController.FocusedField.HitCode = hcAttachment) then
    Result := TdxPDFFileAttachmentAnnotationFieldAccess
      (ViewerController.FocusedField).Attachment
end;

function TdxPDFCustomViewer.GetLayoutCalculator
  : TdxPDFViewerCustomLayoutCalculator;
begin
...
begin
  ...
  begin
    ACaretPage := Pages[SelectionController.Caret.Position.PageIndex]
      as TdxPDFViewerPage;
    ...
    AHeight := SelectionController.Caret.ViewData.Height * DocumentScaleFactor.X
      * DocumentToViewerFactor.X;
    ASize.X := 1;
...
begin
  ...
  if cxRectIntersect(ACaretRect, ViewInfo.FindPanel.Bounds) and IsFindPanelVisible
  then
    case RotationAngle of
      raMinus90, raPlus90:
        begin
          ...
          begin
            ARealSize.X := ARealDestPoint.X + ARealSize.X -
              ViewInfo.FindPanel.Bounds.Right;
  ...
  DestroyCaret;
  if (ARealSize.X > 0) and (ARealSize.Y > 0) and
    PtInRect(ClientBounds, ARealDestPoint) then
  begin
    FreeAndNil(FCaretBitmap);
    FCaretBitmap := TcxBitmap32.Create;
    FCaretBitmap.SetSize(ARealSize.X, ARealSize.Y);
    dxGPPaintCanvas.BeginPaint(FCaretBitmap.Canvas.Handle,
      FCaretBitmap.ClientRect);
    try
      dxGPPaintCanvas.Line(ACaretRect.Left, ACaretRect.Top, ACaretRect.Right,
        ACaretRect.Bottom, Color);
    finally
...
procedure TdxPDFCustomViewer.SetScrollPosition(const APosition: TPoint;
  AAnimated: Boolean = False);
begin
...
begin
  ...
  try
    ...
    ViewerController.SetActiveAndMakeVisiblePage
      (ViewerController.ViewStateHistoryController.CurrentViewState.
      CurrentPageIndex, IsScrollAnimationEnabled);
  finally
...
procedure TdxPDFCustomViewer.UpdateActiveController(const P: TPoint);
  ...
  procedure CalculateHitTest(AController: TdxPDFViewerContainerController;
    const P: TPoint);
  begin
...
procedure TdxPDFCustomViewer.InternalSetCurrentPage(AValue: Integer;
  AAnimated: Boolean);
var
  APageIndex: Integer;
begin
  ...
  begin
    ...
    try
      ...
      EndUpdate;
      FController.ViewStateHistoryController.StoreCurrentViewState
        (vsctScrolling);
    end;
  end;
end;
...
function TdxPDFCustomViewer.GetOptionsNavigationPane
  : TdxPDFViewerOptionsNavigationPane;
begin
  Result := NavigationPane.Options;
end;

function TdxPDFCustomViewer.GetOnCustomDrawPreRenderPageThumbnail
  : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent;
begin
...
function TdxPDFCustomViewer.GetSelectionController
  : TdxPDFViewerSelectionController;
begin
...
procedure TdxPDFCustomViewer.SetDialogsLookAndFeel(const AValue
  : TcxLookAndFeel);
begin
...
procedure TdxPDFCustomViewer.SetOnSearchProgress(const AValue
  : TdxPDFDocumentTextSearchProgressEvent);
begin
...
procedure TdxPDFCustomViewer.SetOnCustomDrawPreRenderPageThumbnail
...
procedure TdxPDFCustomViewer.SetOptionsBehavior(const AValue
  : TdxPDFViewerOptionsBehavior);
begin
  FOptionsBehavior.Assign(AValue);
end;

procedure TdxPDFCustomViewer.SetOptionsFindPanel(const AValue
  : TdxPDFViewerOptionsFindPanel);
begin
  FindPanel.Options := AValue;
end;

procedure TdxPDFCustomViewer.SetOptionsLockedStateImage
  (AValue: TdxPDFViewerLockedStateImageOptions);
begin
  FOptionsLockedStateImage.Assign(AValue);
end;

procedure TdxPDFCustomViewer.SetOptionsNavigationPane
  (const AValue: TdxPDFViewerOptionsNavigationPane);
begin
  NavigationPane.Options := AValue;
end;

procedure TdxPDFCustomViewer.SetOptionsSelection(const AValue
  : TdxPDFViewerOptionsSelection);
begin
  FOptionsSelection.Assign(AValue);
end;

procedure TdxPDFCustomViewer.SetOptionsView(const AValue
  : TdxPDFViewerOptionsView);
begin
  FOptionsView.Assign(AValue);
end;

procedure TdxPDFCustomViewer.SetOptionsZoom(const AValue
  : TdxPDFViewerOptionsZoom);
begin
...
function TdxPDFCustomViewer.ControllerFromPoint(const P: TPoint;
  var AController: TdxPDFViewerCustomController): Boolean;
begin
  Result := IsDocumentLoaded and not TextSearch.IsLocked and
    not(DragAndDropState = ddsInProcess);
  if Result then
  begin
    AController := ActiveController;
    if AController = nil then
    begin
      if PtInRect(ViewInfo.DocumentViewerBounds, P) or ViewerController.MouseButtonPressed
      then
        AController := ViewerController
      else if NavigationPane.Controller.InPaneRect(P) then
...
function TdxPDFCustomViewer.ControllerFromPoint(X, Y: Integer;
  var AController: TdxPDFViewerCustomController): Boolean;
begin
...
procedure TdxPDFCustomViewer.DoSetSelPageIndex(AValue: Integer;
  AIsEventFired: Boolean);
begin
...
procedure TdxPDFCustomViewer.OnDocumentLoadedHandler(Sender: TdxPDFDocument;
  const AInfo: TdxPDFDocumentLoadInfo);
begin
  BeginUpdate;
  try
...
procedure TdxPDFCustomViewer.WMSetCursor(var Message: TWMSetCursor);
  ...
  begin
    Result := not ViewerController.InFindPanelRect(P) or HandTool or
      IsScrollBarsArea(P) or NavigationPane.Controller.InPaneRect(P);
  end;
...
var
  ...
  P: TPoint;
begin
  if not(IsDesigning or IsDestroying) then
  begin
    P := GetMouseCursorClientPos;
    if NeedUpdateCursorUnderScrollBars(P) and ControllerFromPoint(P, AController)
    then
    begin
      (AController as TdxPDFViewerContainerController)
        .CalculateHitTests(GetMouseCursorClientPos);
...
procedure TdxPDFViewerSelection.Select(const ARect: TRect;
  AMakeVisible: Boolean = False);
begin
  FController.Select(ARect);
  MakeVisible(AMakeVisible);
end;

procedure TdxPDFViewerSelection.SelectText(const ARange: TdxPDFPageTextRange;
  AMakeVisible: Boolean = True);
begin
...
procedure TdxPDFViewerSelection.SetSelection(const AValue
...
procedure TdxPDFViewerTextSearch.Find(const AText: string;
  const AOptions: TdxPDFDocumentTextSearchOptions;
...
begin
  ...
  TdxPDFDocumentSequentialTextSearchAccess(FAdvancedTextSearch)
    .Find(Viewer.Document, AText, AOptions, 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      Application.MessageBox(PChar(AMessage), PChar(Application.Title),
...
begin
  BeginUpdate;
  try
    Result := TdxPDFDocumentSequentialTextSearchAccess(DocumentTextSearch)
      .Find(Viewer.Document, FSearchString, FOptions, Viewer.CurrentPageIndex);
  finally
    EndUpdate;
  end;
end;
...
function TdxPDFViewerTextSearch.GetDocumentTextSearch
  : TdxPDFDocumentSequentialTextSearch;
begin
...
begin
  Result := cxGetResourceString
    (@sdxPDFViewerTextSearchingNoMatchesFoundMessage);
end;
...
procedure TdxPDFViewerHighlights.Add(const ARange: TdxPDFPageTextRange;
  ABackColor, AFrameColor: TdxAlphaColor);
begin
  InternalAdd(FController.CreateHighlight(ARange, ABackColor, AFrameColor));
end;

procedure TdxPDFViewerHighlights.Add(const ARanges: TdxPDFPageTextRanges;
  ABackColor, AFrameColor: TdxAlphaColor);
begin
...
begin
  ...
  if HitAtNavigationPaneSplitter then
    Result := crHSplit
  else if TryGetDocumentHitObjectAsInteractiveObject(AIntf) then
    Result := AIntf.GetCursor
  else if HitAtSelection then
    Result := crdxPDFViewerContext
  else if HitAtImage or Viewer.IsDocumentSelecting and Viewer.Selection.IsImageSelection
  then
    Result := crdxPDFViewerCross
  else if HitAtText then
    Result := crIBeam;
end;
...
begin
  ...
  else if HitAtText and HitAtSelection then
    Result := TdxPDFViewerTextPopupMenu
  else if HitAtAttachment then
...
begin
  ...
  HitCodes[hcNavigationPaneSplitter] :=
    Viewer.NavigationPane.Controller.InPaneSplitterRect(HitPoint);
  HitCodes[hcFindPanel] := Viewer.IsFindPanelVisible and
    Viewer.ViewerController.InFindPanelRect(HitPoint);
  APage := GetPage;
...
function TdxPDFViewerDocumentHitTest.GetDocumentHitObject(APage: TdxPDFPage)
...
begin
  AHyperlink := nil;
  Result := AHyperlink;
  ARecognitionObjects := Viewer.ViewerController.GetRecognitionObjects;
  if CanCalculate then
  begin
    if APage.FindHyperlink(Position.Point, Viewer.DocumentScaleFactor,
      AHyperlink, ARecognitionObjects) then
      Result := AHyperlink
    else
...
begin
  Result := not(Viewer.NavigationPane.Controller.InPaneRect(HitPoint) or
    HitAtNavigationPaneSplitter or (Viewer.DragAndDropState = ddsInProcess));
end;
...
begin
  ...
  begin
    ...
    Result := Viewer.Pages[IfThen(APageIndex = -1, 0, APageIndex)
      ] as TdxPDFViewerPage;
  end;
end;

function TdxPDFViewerDocumentHitTest.TryGetDocumentHitObjectAsInteractiveObject
  (out AIntf: IdxPDFInteractiveObject): Boolean;
begin
  Result := Supports(DocumentHitObject, IdxPDFInteractiveObject, AIntf);
end;

function TdxPDFViewerDocumentHitTest.TryGetDocumentHitObjectAsHintableObject
  (out AIntf: IdxPDFHintableObject): Boolean;
begin
  Result := Supports(DocumentHitObject, IdxPDFHintableObject, AIntf);
end;

procedure TdxPDFViewerDocumentHitTest.SetDocumentHitObject
  (const AValue: TdxPDFRecognizedObject);
begin
...
begin
  Result := inherited DoGetCursor;
  if Viewer.NavigationPane.Controller.InPaneSplitterRect(HitPoint) and HitAtSplitter
  then
    Result := crHSplit;
end;

procedure TdxPDFViewerNavigationPaneHitTest.DoCalculate(const AHitPoint
  : TPoint);
begin
  inherited DoCalculate(AHitPoint);
...
function TdxPDFViewerCustomController.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := False;
end;

procedure TdxPDFViewerCustomController.CalculateMouseButtonPressed
  (Shift: TShiftState; X, Y: Integer);
begin
  FMouseButtonPressed := ssLeft in Shift;
end;

procedure TdxPDFViewerCustomController.DoMouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxPDFViewerCustomController.DoMouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
  // do nothing
end;

procedure TdxPDFViewerCustomController.DoMouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
begin
  ...
  if (APopupMenuClass <> nil) and
    ((FPopupMenu = nil) or (FPopupMenu.ClassType <> APopupMenuClass)) then
  begin
  ...
  Result := (FPopupMenu <> nil) and
    (FPopupMenu as TdxPDFViewerCustomPopupMenu).Popup(P);
end;

function TdxPDFViewerCustomController.MouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := DoMouseWheel(Shift, WheelDelta, MousePos);
end;

procedure TdxPDFViewerCustomController.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxPDFViewerCustomController.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TdxPDFViewerCustomController.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
constructor TdxPDFViewerViewState.Create(AChangeType
  : TdxPDFViewerViewStateChangeType);
begin
  inherited Create;
...
class function TdxPDFViewerViewState.CalculatePageSize(const ASize: TdxPointF;
  ARotationAngle: TcxRotationAngle): TdxPointF;
begin
...
begin
  Result := (ChangeType = AView.ChangeType) and
    (ChangeType in [vsctZooming, vsctScrolling, vsctSelecting]) and
    (AView.TimeStamp.Date = TimeStamp.Date) and
    (AView.TimeStamp.Time - TimeStamp.Time <= dxPDFViewerViewCreationTimeOut);
end;
...
procedure TdxPDFViewerViewStateHistory.StoreViewState
  (AView: TdxPDFViewerViewState);
var
...
function TdxPDFViewerViewStateHistory.GetCurrentViewState
  : TdxPDFViewerViewState;
begin
...
function TdxPDFViewerViewStateHistory.SameView(AView,
  ACurrentView: TdxPDFViewerViewState): Boolean;
begin
  Result := (ACurrentView.ChangeType = AView.ChangeType) and
    (ACurrentView.RotationAngle = AView.RotationAngle) and
    (ACurrentView.ZoomFactor = AView.ZoomFactor) and
    (ACurrentView.ZoomMode = AView.ZoomMode) and
...
procedure TdxPDFViewerViewStateHistoryController.StoreCurrentViewState
  (AChangeType: TdxPDFViewerViewStateChangeType);
begin
  if not Viewer.IsUpdateLocked and Viewer.IsDocumentLoaded then
  begin
    if not Viewer.IsZoomingLocked or Viewer.IsZoomingLocked and
      (AChangeType = vsctZooming) then
...
begin
  ...
  try
    ...
    try
      ...
      Viewer.InternalSetCurrentPage
        (FHistory.CurrentViewState.CurrentPageIndex, False);
...
function TdxPDFViewerViewStateHistoryController.CreateView
  (AChangeType: TdxPDFViewerViewStateChangeType): TdxPDFViewerViewState;
begin
...
function TdxPDFViewerViewStateHistoryController.GetCurrentViewState
  : TdxPDFViewerViewState;
begin
...
procedure TdxPDFViewerContentSelector.SetSelection(const AValue
  : TdxPDFCustomSelection);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      AStartPositionPage := Viewer.PageList[FStartPosition.PageIndex]
        as TdxPDFDocumentCustomViewerPage;
      ASelectionRect.TopLeft := Viewer.DocumentState.ToViewerPoint
        (AStartPositionPage, FStartPosition.Point);
      ASelectionRect.BottomRight := HitTest.HitPoint;
      ASelectionRect.Normalize;
      AImageBounds := Viewer.DocumentState.ToViewerRect(AStartPositionPage,
        AImageBounds);
      ASelectionRect.Intersect(AImageBounds);
      ASelectionRect := Viewer.DocumentState.ToDocumentRect(AStartPositionPage,
        ASelectionRect);
      ASelectionRect.Normalize;
...
procedure TdxPDFViewerImageSelector.StartSelection(var AInOutsideContent
  : Boolean);
begin
...
function TdxPDFViewerImageSelector.CreateImageSelection(const ABounds: TdxRectF)
  : TdxPDFImageSelection;
begin
  Result := TdxPDFImageSelection.Create(SelectionBackColor, SelectionFrameColor,
    FSelectedImagePageIndex, FSelectedImage, ABounds);
end;

{ TdxPDFViewerTextSelector }

function TdxPDFViewerTextSelector.CreateTextHighlights(const ARanges
  : TdxPDFPageTextRanges; ABackColor, AFrameColor: TdxAlphaColor)
  : TdxPDFTextHighlight;
begin
  if ValidateRanges(ARanges) then
    Result := TdxPDFTextHighlight.Create(ABackColor, AFrameColor,
      Viewer.DocumentPages, ARanges)
  else
    Result := nil;
end;

function TdxPDFViewerTextSelector.CreateTextSelection
  (const ARange: TdxPDFPageTextRange): TdxPDFTextSelection;
var
...
function TdxPDFViewerTextSelector.CreateTextSelection(const ARanges
  : TdxPDFPageTextRanges): TdxPDFTextSelection;
begin
  if Viewer.OptionsSelection.Text and ValidateRanges(ARanges) then
    Result := TdxPDFTextSelection.Create(SelectionBackColor,
      SelectionFrameColor, Viewer.DocumentPages, ARanges)
  else
    Result := nil;
end;

function TdxPDFViewerTextSelector.GetCaretViewData(const APosition
  : TdxPDFTextPosition): TdxPDFDocumentCaretViewData;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if AWordPart.Same(APosition.Position.WordIndex, APosition.Position.Offset)
      then
      begin
        ...
        else if (AWordPart.Bounds.Angle = 3 * PI / 2) or
          (AWordPart.Bounds.Angle = PI / 2) then
        begin
        ...
        begin
          ...
          X := (AWordPart.Bounds.Left * AK0 - AWordPart.Bounds.Top - ALocation.X
            * K + ALocation.Y) / (AK0 - K);
          Y := AK0 * (X - AWordPart.Bounds.Left) + AWordPart.Bounds.Top;
        end;
        Exit(TdxPDFDocumentCaretViewData.Create(dxPointF(X, Y),
          AWordPart.Bounds.Height, ALine.Bounds.Angle));
      end;
    end;
    Result := TdxPDFDocumentCaretViewData.Create(dxPointF(cxNullPoint), 0,
      TdxPDFTextObjectAccess(ALine).Bounds.Angle);
  end;
end;

function TdxPDFViewerTextSelector.StartSelection(const APosition
  : TdxPDFPosition): Boolean;
var
  ATextPosition: TdxPDFTextPosition;
begin
  ATextPosition := Page[APosition.PageIndex].FindStartTextPosition(APosition,
    ScaleFactor, Viewer.ViewerController.GetRecognitionObjects);
...
procedure TdxPDFViewerTextSelector.MoveCaret(const APosition
  : TdxPDFTextPosition);
var
...
procedure TdxPDFViewerTextSelector.MoveCaret(ADirection
  : TdxPDFMovementDirection);
var
...
begin
  if InProgress and ((APosition.PageIndex <> FStartPageIndex) or
    not cxPointIsEqual(APosition.Point, FStartPoint)) then
  begin
...
procedure TdxPDFViewerTextSelector.SelectByKeyboard
  (ADirection: TdxPDFMovementDirection);
var
  ACaretDirection: TdxPDFMovementDirection;
  ARanges: TdxPDFPageTextRanges;
begin
    ...
    begin
      ...
      begin
        ...
        if AWordPartList.Count = 0 then
          ...
          Selection := CreateTextSelection(TdxPDFPageTextRange.Create
            (ADocumentPosition.PageIndex, AWordPartList[0].WordIndex, 0,
            AWordPartList.Last.WordIndex, AWordPartList.Last.Characters.Count));
      end;
    end);
end;
...
begin
  ...
  Select(APage, procedure
    begin
      if (ADocumentPosition.PageIndex >= 0) and
        APage.FindStartTextPosition(ADocumentPosition, ScaleFactor).IsValid then
        Selection := CreateTextSelection(TdxPDFPageTextRange.Create
          (ADocumentPosition.PageIndex));
    end);
end;
...
begin
  ...
  Select(APage, procedure
    var
      ...
      ARange: TdxPDFPageTextRange;
    begin
      AFoundPosition := APage.FindStartTextPosition(ADocumentPosition,
        ScaleFactor);
      ...
      begin
        ARange := TdxPDFPageTextRange.Create(AFoundPosition.PageIndex,
          ...
          TdxPDFPageTextPosition.Create(AFoundPosition.Position.WordIndex,
          FindWordEndPosition(AFoundPosition)));
...
function TdxPDFViewerTextSelector.GetTextLines(APageIndex: Integer)
  : TdxPDFTextLineList;
var
...
function TdxPDFViewerTextSelector.CreateRangeList(const AStart,
  AEnd: TdxPDFTextPosition): TdxPDFPageTextRanges;
var
  I: Integer;
  ...
  begin
    ...
    begin
      if (AStart.Position.WordIndex > AEnd.Position.WordIndex) or
        (AStart.Position.WordIndex = AEnd.Position.WordIndex) and
        (AStart.Position.Offset > AEnd.Position.Offset) then
        TdxPDFTextUtils.AddRange(TdxPDFPageTextRange.Create(AEnd.PageIndex,
          AEnd.Position, AStart.Position), Result)
      else
        TdxPDFTextUtils.AddRange(TdxPDFPageTextRange.Create(AStart.PageIndex,
          AStart.Position, AEnd.Position), Result);
    end
    else
    begin
      ...
      TdxPDFTextUtils.AddRange
        (TdxPDFPageTextRange.Create(AStartPosition.PageIndex,
        AStartPosition.Position, TdxPDFPageTextPosition.Create(0, -1)), Result);
      ...
      TdxPDFTextUtils.AddRange
        (TdxPDFPageTextRange.Create(AEndPosition.PageIndex,
        TdxPDFPageTextPosition.Create(0, 0), AEndPosition.Position), Result);
    end;
  end;
end;

function TdxPDFViewerTextSelector.CreateRangeList(const AStart
  : TdxPDFTextPosition; const AEnd: TdxPDFPosition): TdxPDFPageTextRanges;
begin
...
function TdxPDFViewerTextSelector.IsArrowKeys
  (ADirection: TdxPDFMovementDirection): Boolean;
begin
...
function TdxPDFViewerTextSelector.IsPositionInLine(APageIndex, ALineIndex,
  AWordIndex, AOffset: Integer): Boolean;
var
  ALine: TdxPDFTextLineAccess;
...
begin
  ...
  if ALineIndex < ALines.Count - 1 then
    Exit(not TdxPDFTextLineAccess(ALines[ALineIndex + 1])
      .PositionInLine(AWordIndex, AOffset));
  ...
  Result := not ALine.PositionInLine(AWordIndex, AOffset) or
    (ALine.WordPartList[0].WordIndex <> AWordIndex);
end;

function TdxPDFViewerTextSelector.FindLine(const APosition: TdxPDFTextPosition;
out ALine: TdxPDFTextLine): Boolean;
var
  I: Integer;
begin
  ALine := nil;
  Result := False;
  for I := 0 to Page[APosition.PageIndex].RecognizedContent.TextLines.
    Count - 1 do
    if IsPositionInLine(APosition.PageIndex, I, APosition.Position.WordIndex,
      APosition.Position.Offset) then
    begin
...
function TdxPDFViewerTextSelector.FindNearestLineByDistance(const APosition
  : TdxPDFPosition): TdxPDFTextLine;
var
  I: Integer;
  ...
  begin
      ...
      if ALine.Bounds.PtInRect(APosition.Point) then
        for APart in TdxPDFTextLineAccess(ALine).WordPartList do
          if TdxPDFTextObjectAccess(APart).Bounds.PtInRect(APosition.Point) then
            Exit(ALine);
      ADistance := TdxPDFUtils.DistanceToRect(APosition.Point,
        ALine.Bounds.RotatedRect);
    ...
    if Result = nil then
      ...
      begin
        ...
        if (ALineRect.Bottom <= APosition.Point.Y) and
          (ALineRect.Right >= APosition.Point.X) then
          Result := ALine;
      end;
  end;
end;

function TdxPDFViewerTextSelector.FindNearestPosition(const APosition
  : TdxPDFPosition; const ATextPosition: TdxPDFTextPosition)
  : TdxPDFTextPosition;
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ((ALastLineIndex >= 0) and (AWordIndex >= ALine.WordList[0].Index))
            and (ALine.WordList[ALastLineIndex].Index >= AWordIndex) then
          begin
            ANearestLine := ALine;
            Break;
          end;
...
function TdxPDFViewerTextSelector.FindWordEndPosition
  (APosition: TdxPDFTextPosition): Integer;
var
  ...
  begin
    ...
    begin
      ...
      if (ALastWordPart.WordIndex = APosition.Position.WordIndex) and
        not SameNextWordPartIndex(APosition.PageIndex, ALineIndex,
        APosition.Position.WordIndex) then
...
begin
  ...
  begin
    ...
    UpdateSelection(TdxPDFTextPosition.Create(ARange.PageIndex,
      ARange.StartPosition));
  end;
end;
...
APageIndex, ALineIndex, AWordIndex, AOffset: Integer;
AProcessLastWordPart: Boolean): Boolean;
var
...
begin
  ...
  begin
    ...
    begin
      if (TdxPDFTextWordPartAccess(AWordPart).NextWrapOffset > AOffset) or
        (((AWordIndex = ALastWordIndex) and SameNextWordPartIndex(APageIndex,
        ALineIndex, AWordIndex))) then
        MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(APageIndex,
          TdxPDFPageTextPosition.Create(AWordIndex, AOffset + 1)))
      else if TdxPDFTextWordPartAccess(AWordPart).WordEnded then
        MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(APageIndex,
          TdxPDFPageTextPosition.Create(AWordIndex + 1, 0)))
      else
        MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(APageIndex,
          TdxPDFPageTextPosition.Create(AWordIndex + 1, 1)));
      Exit(True);
    end;
  end;
  Result := False;
...
function TdxPDFViewerTextSelector.NormalizeDirection
  (ADirection: TdxPDFMovementDirection): TdxPDFMovementDirection;
const
  AngleMap: array [TdxPDFMovementDirection] of Integer = (0, 90, 180, 270, 270,
    270, 270, 270, 270, 270);
...
begin
  ...
  AAngle := Trunc(TdxPDFUtils.NormalizeRotate(AAngle + DocumentAngleMap
    [Viewer.RotationAngle]));
  case AAngle of
    0:
      Result := mdLeft;
    90:
...
function TdxPDFViewerTextSelector.SameNextWordPartIndex(APageIndex, ALineIndex,
  AWordIndex: Integer): Boolean;
var
  I: Integer;
  ALines: TdxPDFTextLineList;
begin
  ...
  if ALineIndex < ALines.Count then
    Exit(TdxPDFTextLineAccess(ALines[ALineIndex]).WordPartList[0]
      .WordIndex = AWordIndex);
  for I := APageIndex + 1 to Viewer.PageCount - 1 do
    if Page[I].RecognizedContent.TextLines.Count > 0 then
      Exit(TdxPDFTextLineAccess(Page[I].RecognizedContent.TextLines[I])
        .WordPartList[0].WordIndex = AWordIndex);
  Result := False;
end;

function TdxPDFViewerTextSelector.ValidateRanges(const ARanges
  : TdxPDFPageTextRanges): Boolean;
var
  I: Integer;
begin
...
begin
  ...
  begin
    ...
    Viewer.ViewerController.MakeSelectionRectVisible
      (Caret.Position.PageIndex, R);
  end;
end;

procedure TdxPDFViewerTextSelector.MoveAndMakeCaretVisible(const APosition
  : TdxPDFTextPosition);
begin
  MoveCaret(APosition);
  MakeCaretVisible;
...
begin
  ...
  begin
    ...
    for I := 0 to ALastLineIndex - 1 do
      ...
      begin
        SetCaretPosition(TdxPDFTextLineAccess(ALines[I + 1])
          .GetPosition(Caret.Position.PageIndex, Caret.StartCoordinates));
        Exit;
      end;
    for ANextPageIndex := Caret.Position.PageIndex +
      1 to Viewer.PageCount - 1 do
    begin
      ALines := Page[ANextPageIndex].RecognizedContent.TextLines;
      if ALines.Count > 0 then
      begin
        SetCaretPosition(TdxPDFTextLineAccess(ALines[0])
          .GetPosition(ANextPageIndex, Caret.StartCoordinates));
        Exit;
      end;
    end;
  end;
...
begin
  ...
  if AOffset > 0 then
    MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(APageIndex,
      TdxPDFPageTextPosition.Create(AWordIndex, AOffset - 1)))
  else if AWordIndex > 1 then
    MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(APageIndex,
      TdxPDFPageTextPosition.Create(AWordIndex - 1,
      FindWordEndPosition(TdxPDFTextPosition.Create(APageIndex,
      AWordIndex - 1, 0)))))
  else
  begin
    ...
    begin
      ...
      begin
        ...
        MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(APreviousPageIndex,
          TdxPDFPageTextPosition.Create(AWordPart.WordIndex,
          TdxPDFTextWordPartAccess(AWordPart).NextWrapOffset)));
        Exit;
      end;
    end;
...
var
  APageIndex, ALastLineIndex, AWordIndex, AOffset, I, ALastWordPartIndex,
  ...
  begin
    ...
    begin
      ...
      if ALine.PositionInLine(AWordIndex, AOffset) and
        MoveRight(ALine.WordPartList, APageIndex, I, AWordIndex, AOffset, True)
      then
        Exit;
    end;
    ...
    begin
      ...
      if MoveRight(AWordParts, APageIndex, ALastLineIndex, AWordIndex, AOffset,
        False) then
        Exit;
      ALastWordPartIndex := AWordParts.Count - 1;
      ...
      begin
        if (ALastWordPart.NextWrapOffset > AOffset) or
          (((AWordIndex = AWordParts[ALastWordPartIndex].WordIndex) and
        ...
        for ANextPageIndex := APageIndex + 1 to Viewer.PageCount - 1 do
          ...
          begin
            MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(ANextPageIndex,
              TdxPDFPageTextPosition.Create(1, 0)));
            Exit;
          end;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(I,
          TdxPDFPageTextPosition.Create(ALastWord.WordIndex,
          ALastWord.Characters.Count)));
        Exit;
      end;
    end;
  end;
...
begin
  ...
  begin
    ALine := TdxPDFTextLineAccess(ALines[I]);
    if ALine.PositionInLine(Caret.Position.Position.WordIndex,
      Caret.Position.Position.Offset) then
      MoveAndMakeCaretVisible
        (TdxPDFTextPosition.Create(Caret.Position.PageIndex,
...
begin
  ...
  begin
    ...
    begin
      ...
      MoveAndMakeCaretVisible
        (TdxPDFTextPosition.Create(Caret.Position.PageIndex,
        TdxPDFPageTextPosition.Create(ALast.WordIndex,
        ALast.Characters.Count)));
    end;
  end;
end;
...
begin
  ...
  AHasNextWord := False;
  if (ALastLine.WordList.Count <> 0) and
    (ALastLine.WordList[ALastLine.WordList.Count - 1].Index = ACaretWordIndex)
  then
  begin
    ...
    AHasNextWord := True;
  if AHasNextWord then
    MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(ACaretPageIndex,
      ACaretWordIndex + 1, 0))
  else
  begin
      ...
      if ALine.PositionInLine(ACaretWordIndex, ACaretOffset) then
        for AWord in ALine.WordPartList do
          if AWord.WordIndex = ACaretWordIndex then
            MoveAndMakeCaretVisible
              (TdxPDFTextPosition.Create(Caret.Position.PageIndex,
              ACaretWordIndex, AWord.Characters.Count));
    end;
  end;
end;
...
begin
  ...
  if ACaretOffset <> 0 then
    MoveAndMakeCaretVisible(TdxPDFTextPosition.Create(ACaretPageIndex,
      ACaretWordIndex, 0))
  else if ACaretWordIndex = 1 then
  begin
...
begin
  ...
  begin
    ...
    begin
      ANewPosition := TdxPDFTextLineAccess(ALines[I - 1])
        .GetPosition(APageIndex, Caret.StartCoordinates);
      AWordIndex := ANewPosition.Position.WordIndex;
      AOffset := ANewPosition.Position.Offset;
      if ALine.PositionInLine(AWordIndex, AOffset) then
        SetCaretPosition(TdxPDFTextPosition.Create(APageIndex,
          TdxPDFPageTextPosition.Create(AWordIndex, AOffset - 1)))
      else
        SetCaretPosition(TdxPDFTextPosition.Create(APageIndex,
          TdxPDFPageTextPosition.Create(AWordIndex, AOffset)));
      Exit;
    end;
  end;
  ...
  begin
    ...
    if ALines.Count > 0 then
    begin
      SetCaretPosition(TdxPDFTextLineAccess(ALines[ALines.Count - 1])
        .GetPosition(APrevPageIndex, Caret.StartCoordinates));
      Exit;
    end;
  end;
  SetCaretPosition(Caret.Position);
...
procedure TdxPDFViewerTextSelector.SetCaretPosition(const APosition
  : TdxPDFTextPosition);
begin
  if not HasCaret then
    Caret := TdxPDFDocumentCaret.Create(APosition, GetCaretViewData(APosition),
      Caret.StartCoordinates)
  else
    Caret := TdxPDFDocumentCaret.Create(APosition, GetCaretViewData(APosition),
      dxNullPointF);
  MakeCaretVisible;
...
procedure TdxPDFViewerTextSelector.UpdateSelection(const APosition
  : TdxPDFTextPosition);
begin
  MoveCaret(APosition);
...
function TdxPDFViewerSelectionController.CanExtractSelectedContent
  (AHitCode: Int64): Boolean;
begin
  Result := Viewer.CanExtractContent and (Selection <> nil) and
    (Selection.HitCode = AHitCode);
end;
...
procedure TdxPDFViewerSelectionController.MouseDown(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
  if Viewer.IsDocumentLoaded then
  begin
    FClickController.Click(Button, X, Y);
    if (ssLeft in Shift) and HitTest.HitAtPage and not HitTest.HitAtInteractiveObject
    then
    begin
      if not Viewer.HandTool then
        StartSelection(Shift)
    end
    else if not HitTest.HitAtSelection then
      LockedClear;
  end;
end;

procedure TdxPDFViewerSelectionController.MouseMove(Shift: TShiftState;
X, Y: Integer);

  function NeedSelect(AShift: TShiftState): Boolean;
  begin
    Result := (ssLeft in AShift) and ((FClickController.ClickCount <= 1) or
      HitTest.HitAtImage or FImageSelector.InProgress);
  end;
...
procedure TdxPDFViewerSelectionController.MouseUp(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
  if Viewer.IsDocumentLoaded then
    EndSelection(Shift);
...
function TdxPDFViewerSelectionController.GetImageSelection
  (const AArea: TdxPDFDocumentArea): TdxPDFImageSelection;
var
  AImage: TdxPDFImage;
begin
  Result := nil;
  for AImage in Images[AArea.PageIndex] do
    if TdxPDFUtils.Intersects(AArea.Rect, TdxPDFImageAccess(AImage).Bounds) then
      Result := TdxPDFImageSelection.Create(Viewer.Painter.SelectionBackColor,
        Viewer.Painter.SelectionFrameColor, AArea.PageIndex, AImage,
        TdxPDFImageAccess(AImage).Bounds);
end;

function TdxPDFViewerSelectionController.GetTextRanges
  (const AArea: TdxPDFDocumentArea): TdxPDFPageTextRanges;
var
...
begin
  ...
  if ATextSelection <> nil then
    try
      ...
      cxCopyData(@ATextSelection.Ranges[0], @Result[0], 0, 0,
        Length(ATextSelection.Ranges) * SizeOf(TdxPDFPageTextRange));
    finally
      ATextSelection.Free;
    end;
end;

function TdxPDFViewerSelectionController.GetTextSelection
  (const AArea: TdxPDFDocumentArea): TdxPDFTextSelection;
var
  ALine: TdxPDFTextLine;
  APageIndex: Integer;
  ARange: TdxPDFPageTextRange;
...
begin
  ...
  for ALine in TextLines[APageIndex] do
    if TdxPDFUtils.Intersects(AArea.Rect, TdxPDFTextLineAccess(ALine)
      .Bounds.RotatedRect) then
    begin
...
begin
  if FTextSelector.InProgress then
    Result := False
  else if FImageSelector.InProgress then
    Result := False
  else
    Result := not IsEmptySelection and TdxPDFCustomSelectionAccess(Selection)
      .Contains(HitTest.Position)
end;
...
begin
  ...
  begin
    ...
    else if Selection is TdxPDFTextSelection then
    begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if TdxPDFUtils.Intersects(AIntersection, dxRectF(AViewerPage.Bounds),
          ADocumentRect) then
        begin
...
procedure TdxPDFViewerSelectionController.SelectText
  (ARange: TdxPDFPageTextRange);
begin
  if Viewer.IsDocumentLoaded then
  begin
...
begin
  if not Viewer.HandTool and not Viewer.HitTest.HitAtBackground and not Viewer.IsDestroying
  then
    dxCallNotify(OnSelectionChanged, Self);
end;

procedure TdxPDFViewerSelectionController.SelectByKeyboard
  (ADirection: TdxPDFMovementDirection);
begin
...
procedure TdxPDFViewerSelectionController.SelectImage
  (const AArea: TdxPDFDocumentArea);
begin
  Selection := GetImageSelection(AArea);
  SelectionChanged;
end;

procedure TdxPDFViewerSelectionController.SelectText(const ARanges
  : TdxPDFPageTextRanges);
begin
...
function TdxPDFViewerSelectionController.CreateHighlight
  (ARange: TdxPDFPageTextRange; ABackColor, AFrameColor: TdxAlphaColor)
  : TdxPDFTextHighlight;
var
  ARanges: TdxPDFPageTextRanges;
begin
...
function TdxPDFViewerSelectionController.CreateHighlight(const ARanges
  : TdxPDFPageTextRanges; ABackColor, AFrameColor: TdxAlphaColor)
  : TdxPDFTextHighlight;
begin
  Result := FTextSelector.CreateTextHighlights(ARanges, ABackColor,
    AFrameColor);
end;

function TdxPDFViewerSelectionController.KeyDown(var AKey: Word;
...
function TdxPDFViewerSelectionController.GetSelectionAsBitmap: TcxBitmap;
  ...
  begin
    APage := TdxPDFPageAccess(TdxPDFViewerPage(Viewer.PageList[APageIndex])
      .DocumentPage);
...
procedure TdxPDFViewerSelectionController.SetCaret(const AValue
  : TdxPDFDocumentCaret);
begin
...
procedure TdxPDFViewerSelectionController.SetSelection
  (const AValue: TdxPDFCustomSelection);
begin
  if IsEmptySelection or not TdxPDFCustomSelectionAccess(Selection).Same(AValue)
  then
  begin
    FreeAndNil(FSelection);
    FSelection := AValue;
...
function TdxPDFViewerSelectionController.GetImages(APageIndex: Integer)
  : TdxPDFImageList;
begin
...
function TdxPDFViewerSelectionController.GetTextLines(APageIndex: Integer)
  : TdxPDFTextLineList;
begin
  Result := Page[APageIndex].RecognizedContent.TextLines;
end;

function TdxPDFViewerSelectionController.GetHitTest
  : TdxPDFViewerDocumentHitTest;
begin
  Result := Viewer.HitTest;
end;
...
procedure TdxPDFViewerContainerController.DoMouseDown(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxPDFViewerContainerController.DoMouseMove(Shift: TShiftState;
X, Y: Integer);
begin
...
procedure TdxPDFViewerContainerController.DoMouseUp(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
...
function TdxPDFViewerContainerController.ProcessChildKey(var AKey: Word;
...
begin
  ...
  if Result then
    if AKey = VK_TAB then
      FocusNextCell(True)
    else if FocusedCell is TdxPDFViewerButtonViewInfo then
...
function TdxPDFViewerContainerController.FindNextFocusableCell
  (ACell: TdxPDFViewerCellViewInfo; AGoForward: Boolean)
  : TdxPDFViewerCellViewInfo;
const
...
begin
  if Viewer.IsDocumentAvailable and not Viewer.IsScrollAnimationActive then
    CalculateHitTests(cxPoint(X, Y));
end;

procedure TdxPDFViewerContainerController.CellRemoving
...
procedure TdxPDFViewerContainerController.ProcessAccel
  (ACell: TdxPDFViewerCellViewInfo);
begin
  FocusedCell := ACell;
  ProcessClick(ACell);
...
procedure TdxPDFViewerContainerController.ProcessClick
  (ACell: TdxPDFViewerCellViewInfo);
begin
...
procedure TdxPDFViewerContainerController.SetHotCell(const AValue
  : TdxPDFViewerCellViewInfo);
begin
...
procedure TdxPDFViewerContainerController.SetPressedCell
  (const AValue: TdxPDFViewerCellViewInfo);
begin
...
procedure TdxPDFViewerContainerController.SetFocusedCell
  (const AValue: TdxPDFViewerCellViewInfo);
begin
  if (FFocusedCell <> AValue) and ((AValue = nil) or AValue.CanFocus) then
  begin
    FFocusedCell := AValue;
    if Viewer.HandleAllocated and Viewer.CanFocusEx and Viewer.IsDocumentLoaded
    then
    begin
...
function TdxPDFViewerNavigationPaneController.GetHitTest
  : TdxPDFViewerCellHitTest;
begin
  Result := NavigationPane.HitTest;
end;

function TdxPDFViewerNavigationPaneController.DoMouseWheel(Shift: TShiftState;
WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := True;
end;
...
procedure TdxPDFViewerNavigationPaneController.DoMouseDown(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
  CalculateHitTests(cxPoint(X, Y));
...
procedure TdxPDFViewerNavigationPaneController.DoMouseMove(Shift: TShiftState;
X, Y: Integer);
begin
  inherited DoMouseMove(Shift, X, Y);
  CheckHint;
...
procedure TdxPDFViewerNavigationPaneController.DoMouseUp(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
  CalculateHitTests(cxPoint(X, Y));
...
function TdxPDFViewerNavigationPaneController.InPaneRect
  (const P: TPoint): Boolean;
begin
  Result := PtInRect(ViewInfo.NavigationPane.Bounds, P);
end;

function TdxPDFViewerNavigationPaneController.InPaneSplitterRect
  (const P: TPoint): Boolean;
begin
  Result := PtInRect(ViewInfo.NavigationPane.SplitterBounds, P);
end;

procedure TdxPDFViewerNavigationPaneController.ExecuteOperation(const AOperation
...
function TdxPDFViewerNavigationPaneController.GetNavigationPane
  : TdxPDFViewerNavigationPane;
begin
  Result := Viewer.NavigationPane;
end;

function TdxPDFViewerNavigationPaneController.NeedShowHint
  (const AHint: string): Boolean;
begin
  Result := Viewer.OptionsBehavior.ShowHints and (FHintCell <> nil) and
    (AHint <> '') and Supports(FHintCell, IcxHintableObject) and
    cxCanShowHint(Viewer);
end;
...
procedure TdxPDFViewerNavigationPaneController.ShowHintTimerExpired
  (Sender: TObject);
var
  AHint: string;
...
begin
  ...
  if NeedShowHint(AHint) then
    FHintHelper.ShowHint(AHintAreaBounds, AHintAreaBounds, AHint, False,
      HotCell, Viewer.Font);
end;

{ TdxPDFViewerNavigationPaneController.THintHelper }

constructor TdxPDFViewerNavigationPaneController.THintHelper.Create
...
procedure TdxPDFViewerNavigationPaneController.THintHelper.CorrectHintWindowRect
  (var ARect: TRect);
begin
  inherited;
...
function TdxPDFViewerNavigationPaneController.THintHelper.GetOwnerControl
  : TcxControl;
begin
  Result := FViewer;
end;

function TdxPDFViewerNavigationPaneController.THintHelper.
...
function TdxPDFViewerController.DoMouseWheel(Shift: TShiftState;
WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := InFindPanelRect(Viewer.ScreenToClient(MousePos));
end;

function TdxPDFViewerController.ProcessChildKey(var AKey: Word;
AShiftState: TShiftState): Boolean;
begin
...
procedure TdxPDFViewerController.DoMouseDown(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxPDFViewerController.DoMouseUp(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    else if FCanClick and Supports(DocumentHitTest.DocumentHitObject,
      IdxPDFInteractiveObject, AIntf) then
    begin
...
begin
  if not Viewer.IsUpdateLocked then
    if HitAtFindPanelEdit then
      Exit
    else if DocumentHitTest.HitAtFindPanel and
      not(IsDocumentPanning or IsDocumentSelecting) then
      DoUpdate(crDefault)
    else if not HitAtFindPanelEdit then
      DoUpdate(Cursor);
end;
...
function TdxPDFViewerController.GetPageScrollPosition(APage: TdxPDFViewerPage;
AScrollPositionX, AScrollPositionY: Single): TdxPointF;
var
  ACropBox: TdxRectF;
  APosition: TdxPointF;
...
function TdxPDFViewerController.GetPageTopLeft(APage: TdxPDFViewerPage)
  : TdxPointF;
begin
...
function TdxPDFViewerController.GetRecognitionObjects: TdxPDFRecognitionObjects;
begin
...
begin
  Result := Viewer.IsDocumentLoaded and
    (Viewer.ViewInfo.FindPanel.Edit.InternalEdit.EditingValue <> '') and
    not Viewer.TextSearch.IsLocked;
end;

function TdxPDFViewerController.KeyDown(var Key: Word;
...
procedure TdxPDFViewerController.MakeRectVisible(const ARect: TdxRectF;
AType: TdxVisibilityType = vtCentered; AIsTopAlignment: Boolean = False);

  procedure DoMakeRectVisible(const R: TRect);

    function GetOffset(AItemMin, AItemMax, AClientMin,
      AClientMax: Integer): Integer;
    begin
      Result := 0;
      if AItemMin < AClientMax then
        Result := AItemMin - AClientMin
      else if AItemMax > AClientMax then
      begin
  ...
  begin
    ...
    P.X := Viewer.LeftPos + GetOffset(R.Left, R.Right, AClientRect.Left,
      AClientRect.Right);
    P.Y := Viewer.TopPos + GetOffset(R.Top, R.Bottom, AClientRect.Top,
      AClientRect.Bottom);
...
procedure TdxPDFViewerController.MakeSelectionRectVisible(APageIndex: Integer;
const ARect: TdxRectF);

  function GetRect(ALeft, ATop, ARight, ABottom: Double; APageIndex: Integer)
    : TdxRectF;
  begin
    Result.TopLeft := FSelectionController.Caret.ViewData.TopLeft;
    Result.Right := Result.Left + 1;
    Result.Bottom := Result.Top - Viewer.SelectionController.Caret.
      ViewData.Height;
    Result := (Viewer.Pages[APageIndex] as TdxPDFViewerPage)
      .ToViewerRect(cxRectAdjustF(Result));
    Result := cxRectOffset(Result, dxPointF(Viewer.ClientRect.TopLeft), False);
  end;

begin
  MakeRectVisible(GetRect(ARect.Left, ARect.Top, ARect.Right, ARect.Bottom,
    APageIndex), vtFully);
end;

procedure TdxPDFViewerController.OffsetContent(const AOffset: TPoint);
begin
  Viewer.UpdateLeftTopPosition(cxPoint(Viewer.LeftPos - AOffset.X,
    Viewer.TopPos - AOffset.Y));
end;
...
procedure TdxPDFViewerController.ShowDocumentPosition(const ATarget
  : TdxPDFTarget);
begin
  FInteractivityController.ShowDocumentPosition(ATarget);
end;

procedure TdxPDFViewerController.OpenAttachment(AAttachment
...
begin
  ...
  begin
    ...
    AFileName := TPath.Combine(TdxPDFDocumentAccess(Viewer.Document)
      .Repository.FolderName, AFileName);
    DoSaveAttachment(AFileName, AAttachment);
    dxShellExecute(AFileName, SW_SHOWMAXIMIZED);
  end;
end;

procedure TdxPDFViewerController.SaveAttachment(AAttachment
  : TdxPDFFileAttachment);
var
  APath: string;
begin
...
begin
  Result := Viewer.IsDocumentLoaded and Viewer.DocumentState.HandTool and
    MouseButtonPressed;
end;

function TdxPDFViewerController.IsDocumentSelecting: Boolean;
begin
  Result := Viewer.IsDocumentLoaded and not IsDocumentPanning and
    MouseButtonPressed;
end;
...
begin
  case TdxPDFUtils.NormalizeRotate
    (TdxPDFUtils.ConvertToIntEx(Viewer.RotationAngle) + AAngleDelta) of
    - 90, 270:
...
procedure TdxPDFViewerController.SetActiveAndMakeVisiblePage
  (APageIndex: Integer; AAnimated: Boolean);
begin
  if AAnimated then
  begin
  ...
  begin
    Viewer.SelPageIndex := APageIndex;
    Viewer.MakeVisible(Viewer.CurrentPageIndex);
  end;
end;

procedure TdxPDFViewerController.SetScrollPosition(const APosition: TPoint;
AAnimated: Boolean = False);
begin
...
procedure TdxPDFViewerController.ExecuteOperation(const AOperation
  : TdxPDFInteractiveOperation);
begin
...
begin
  if not DocumentHitTest.HitAtInteractiveObject and Viewer.DocumentState.HandTool
  then
...
procedure TdxPDFViewerController.CalculateMouseButtonPressed(Shift: TShiftState;
X, Y: Integer);
begin
  inherited CalculateMouseButtonPressed(Shift, X, Y);
  FMouseButtonPressed := FMouseButtonPressed and
    not InFindPanelRect(cxPoint(X, Y));
end;
...
begin
  ...
  if MouseButtonPressed then
    Result := crcxHandDrag
  else if not Viewer.IsScrollBarsArea(HitTest.HitPoint) and
    not cxPointIsInvalid(DocumentHitTest.HitPoint) then
    Result := crcxHand;
end;
...
procedure TdxPDFViewerController.SetFocusedField(const AValue
  : TdxPDFAcroFormField);
begin
...
function TdxPDFViewerController.GetAttachmentSavingPath
  (AAttachment: TdxPDFFileAttachment): string;
var
...
procedure TdxPDFViewerController.DoSaveAttachment(const APath: string;
AAttachment: TdxPDFFileAttachment);
var
...
function TdxPDFViewerController.IsCtrlShiftPressed(AShift: TShiftState)
  : Boolean;
begin
...
function TdxPDFViewerController.ProcessViewerKeyDown(var AKey: Word;
AShift: TShiftState): Boolean;
var
  AAltPressed: Boolean;
begin
  ...
  case AKey of
    ...
    VK_LEFT:
      if AAltPressed then
        Viewer.GoToPrevView
      else if NeedHorizontalScroll then
...
procedure TdxPDFViewerController.OnMakeRectVisibleHandler(APageIndex: Integer;
const ARect: TdxRectF);
  ...
  var
    R: TdxRectF;
  begin
    Result := TdxPDFUtils.Intersects(R, ARect, dxRectF(Viewer.ClientRect)) and
      TdxPDFUtils.RectIsEqual(R, ARect, 0.001);
  end;

var
  R: TdxRectF;
begin
  R := (Viewer.Pages[Max(APageIndex, 0)] as TdxPDFViewerPage)
    .ToViewerRect(cxRectAdjustF(ARect));
  if not IsRectVisible(R) then
    MakeRectVisible(cxRectOffset(R,
      dxPointF(Viewer.ClientRect.TopLeft), False));
end;
...
var
  AIntf: IdxPDFInteractiveObject;
begin
  if Supports(Viewer.HitTest.DocumentHitObject, IdxPDFInteractiveObject, AIntf)
  then
  begin
    case AButton of
      mbLeft:
...
constructor TdxPDFViewerController.TInteractiveObjectHinHelper.Create
  (AViewer: TdxPDFCustomViewer);
begin
  inherited Create;
  FViewer := AViewer;
...
function TdxPDFViewerController.TInteractiveObjectHinHelper.CanShowHint
  : Boolean;
begin
  Result := FViewer.OptionsBehavior.ShowHints and FViewer.HitTest.CanShowHint;
end;

function TdxPDFViewerController.TInteractiveObjectHinHelper.GetOwnerControl
  : TcxControl;
begin
  Result := FViewer;
end;

procedure TdxPDFViewerController.TInteractiveObjectHinHelper.
  CorrectHintWindowRect(var ARect: TRect);
begin
...
procedure TdxPDFViewerController.TInteractiveObjectHinHelper.ShowingTimerHandler
  (Sender: TObject);
var
  ABounds: TRect;
begin
  ...
  begin
    ShowHint(ABounds, ABounds, FViewer.HitTest.GetHintText, False, Self,
      FViewer.Font);
    FShowingTimer.Enabled := False;
  end;
end;
...
constructor TdxPDFViewerCellViewInfo.Create(AController
  : TdxPDFViewerContainerController);
begin
  inherited Create;
...
function TdxPDFViewerCellViewInfo.CalculateHitTest
  (AHitTest: TdxPDFViewerCellHitTest): Boolean;
begin
...
procedure TdxPDFViewerCellViewInfo.Draw(ACanvas: TcxCanvas;
...
function TdxPDFViewerViewInfoList.CalculateHitTest
  (AHitTest: TdxPDFViewerCellHitTest): Boolean;
var
...
function TdxPDFViewerContainerViewInfo.CalculateHitTest
  (AHitTest: TdxPDFViewerCellHitTest): Boolean;
begin
  Result := inherited CalculateHitTest(AHitTest) and
...
procedure TdxPDFViewerContainerViewInfo.SetEmptyBounds
  (AViewInfo: TdxPDFViewerCellViewInfo);
begin
...
function TdxPDFViewerContainerViewInfo.AddCell
  (ACellClass: TdxPDFViewerCellViewInfoClass): TdxPDFViewerCellViewInfo;
begin
  Result := AddCell(ACellClass, Controller);
end;

function TdxPDFViewerContainerViewInfo.AddCell
  (ACellClass: TdxPDFViewerCellViewInfoClass;
...
function TdxPDFViewerContainerViewInfo.AlignToTopClientSide
  (AViewInfo: TdxPDFViewerCellViewInfo; var R: TRect): Boolean;
var
  ARect: TRect;
begin
...
function TdxPDFViewerContainerViewInfo.AlignToLeftSide
  (AViewInfo: TdxPDFViewerCellViewInfo; var R: TRect): Boolean;
var
  ARect: TRect;
begin
...
function TdxPDFViewerContainerViewInfo.AlignToRightSide
  (AViewInfo: TdxPDFViewerCellViewInfo; var R: TRect): Boolean;
var
  ARect: TRect;
begin
...
begin
  FFindPanel := AddCell(TdxPDFViewerFindPanelViewInfo)
    as TdxPDFViewerFindPanelViewInfo;
end;

procedure TdxPDFViewerViewInfo.Draw(ACanvas: TcxCanvas;
AForce: Boolean = False);
begin
...
procedure TdxPDFViewerViewInfo.PopulateTabOrders
  (AList: TdxPDFViewerViewInfoList);
...
function TdxPDFViewerViewInfo.GetNavigationPane
  : TdxPDFViewerNavigationPaneViewInfo;
begin
...
begin
  Result := cxTextWidth(Font, Text) + Painter.ScaleFactor.Apply
    (cxTextOffset * 2);
end;
...
constructor TdxPDFViewerButtonViewInfo.Create(AController
  : TdxPDFViewerContainerController);
begin
...
begin
  if not IsEnabled then
    Result := cxbsDisabled
  else if IsPressed and (not Viewer.MouseCapture or IsHot) then
    Result := cxbsPressed
  else if IsHot then
    Result := cxbsHot
...
begin
  Result := (Controller.FocusedCell = nil) and
    (Controller.ViewInfo.FindPanel.ActualNextButtonViewInfo = Self);
end;
...
procedure TdxPDFViewerButtonViewInfo.CalculateGlyphRect;
  ...
  procedure PlaceGlyphRect(const AContentBounds: TRect;
  ...
  procedure CheckGlyphRectSize(const AContentBounds: TRect;
  ...
  begin
    ...
    PlaceGlyphRect(AContentBounds, cxRectWidth(GlyphRect),
      cxRectHeight(GlyphRect));
  end;
...
procedure TdxPDFViewerButtonViewInfo.DrawButtonBackground(ACanvas: TcxCanvas;
const ARect: TRect; AState: TcxButtonState);
begin
...
begin
  ...
  begin
    cxDrawText(ACanvas, FCaption, cxRectOffset(CaptionRect,
      GetButtonOffset(State)), DT_VCENTER or DT_SINGLELINE or DT_CENTER);
  end;
end;
...
function TdxPDFViewerButtonViewInfo.GetButtonOffset(AButtonState
  : TcxButtonState): TPoint;
var
  AShift: Integer;
begin
...
begin
  Result := cxMarginsWidth(Margins) + GlyphSize.cx +
    Painter.DropDownButtonWidth;
end;
...
begin
  ...
  if APopupMenu <> nil then
    try
      TdxPDFViewerPopupMenuAccess(APopupMenu)
        .Popup(Viewer.ClientToScreen(cxPoint(Bounds.Left, Bounds.Bottom)));
    finally
      APopupMenu.Free;
    end;
end;

procedure TdxPDFViewerCustomDropDownButtonViewInfo.DrawButtonContent
  (ACanvas: TcxCanvas);
begin
  Painter.DrawDropDownButton(ACanvas, FDropDownButtonArrowRect, State);
  Painter.DrawDropDownButtonGlyph(ACanvas, GetGlyph, GlyphRect, State,
    GetColorizeGlyph);
end;

procedure TdxPDFViewerCustomDropDownButtonViewInfo.
  CalculateDropDownButtonArrowRect;
begin
  FDropDownButtonArrowRect := FGlyphRect;
  FDropDownButtonArrowRect.Left := FGlyphRect.Right;
  FDropDownButtonArrowRect.Right := FDropDownButtonArrowRect.Left +
    Painter.DropDownButtonWidth;
...
constructor TdxPDFViewerButtonFadingHelper.Create
  (AViewInfo: TdxPDFViewerButtonViewInfo);
begin
  inherited Create;
...
begin
  Result := ButtonViewInfo.IsFadingAvailable and
    not ButtonViewInfo.Viewer.IsDesigning;
end;
...
procedure TdxPDFViewerButtonFadingHelper.GetFadingImages(out AFadeOutImage,
  ...
  begin
    ...
    ButtonViewInfo.DrawButtonBackground(Result.cxCanvas,
      Result.ClientRect, AState);
  end;

const
  StateMap: array [Boolean] of TcxButtonState = (cxbsNormal, cxbsDefault);
begin
  AFadeOutImage := PrepareImage(StateMap[ButtonViewInfo.IsDefault or
    ButtonViewInfo.IsFocused]);
  AFadeInImage := PrepareImage(cxbsHot);
end;

{ TdxPDFViewerImageAnimationTransition }

constructor TdxPDFViewerImageAnimationTransition.Create(AImage: TGraphic;
ATime: Cardinal; AMode: TdxDrawAnimationMode; AIsHiding: Boolean);
begin
  FMode := AMode;
  FIsHiding := AIsHiding;
  PrepareImage(AImage);
...
procedure TdxPDFViewerImageAnimationTransition.Draw(ACanvas: TCanvas;
const ADestRect: TRect);
begin
  FDestination.Clear;
  cxBitBlt(FDestination.Canvas.Handle, ACanvas.Handle,
    cxRect(cxRectSize(ADestRect)), ADestRect.TopLeft, SRCCOPY);
  ...
  cxBitBlt(ACanvas.Handle, FDestination.Canvas.Handle, ADestRect,
    cxNullPoint, SRCCOPY);
end;

function TdxPDFViewerImageAnimationTransition.TransitionLength
  (AImageHeight: Integer): Integer;
begin
  Result := IfThen(FMode = amFade, 100, AImageHeight);
end;

procedure TdxPDFViewerImageAnimationTransition.Draw(AGraphics: TdxGPGraphics;
const ADestRect: TRect);
var
...
begin
  ...
  case FMode of
    amScrollUp:
      DrawScrollUp(AGraphics, ADestRect.Left, ADestRect.Top, AWidth, AHeight,
        APosition);
    amScrollUpFade:
      DrawScrollUpFade(AGraphics, ADestRect.Left, ADestRect.Bottom, AWidth,
        AHeight, APosition);
    amScrollDown:
      DrawScrollDown(AGraphics, ADestRect.Left, ADestRect.Top, AWidth, AHeight,
        APosition);
    amScrollDownFade:
      DrawScrollDownFade(AGraphics, ADestRect.Left, ADestRect.Top, AWidth,
        AHeight, APosition);
    amFade:
      begin
        APosition := IfThen(not FIsHiding, Position,
          Integer(Length) - Position);
        DrawFade(AGraphics, ADestRect.Left, ADestRect.Top, AWidth, AHeight,
          APosition);
      end;
  end;
end;

procedure TdxPDFViewerImageAnimationTransition.DrawFade
  (AGraphics: TdxGPGraphics; ALeft, ATop, AWidth, AHeight: Integer;
AProgress: Byte);
begin
  AGraphics.Draw(FImage, cxRectBounds(ALeft, ATop, AWidth, AHeight),
    Rect(0, 0, AWidth, AHeight), MulDiv(255, AProgress, 100));
end;

procedure TdxPDFViewerImageAnimationTransition.DrawScrollDown
  (AGraphics: TdxGPGraphics; ALeft, ATop, AWidth, AHeight, AOffset: Integer);
begin
  AGraphics.Draw(FImage, cxRectBounds(ALeft, ATop - AHeight + AOffset, AWidth,
    AHeight));
end;

procedure TdxPDFViewerImageAnimationTransition.DrawScrollDownFade
  (AGraphics: TdxGPGraphics; ALeft, ATop, AWidth, AHeight, AOffset: Integer);
begin
  AGraphics.Draw(FImage, cxRectBounds(ALeft, ATop + AOffset, AWidth, AHeight));
end;

procedure TdxPDFViewerImageAnimationTransition.DrawScrollUp
  (AGraphics: TdxGPGraphics; ALeft, ATop, AWidth, AHeight, AOffset: Integer);
begin
  AGraphics.Draw(FImage, cxRectBounds(ALeft, ATop - AOffset, AWidth, AHeight));
end;

procedure TdxPDFViewerImageAnimationTransition.DrawScrollUpFade
  (AGraphics: TdxGPGraphics; ALeft, ATop, AWidth, AHeight, AOffset: Integer);
begin
  AGraphics.Draw(FImage, cxRectBounds(ALeft, ATop - AOffset, AWidth, AHeight));
end;
...
function TdxPDFViewerFindPanelAnimationController.CreateFindPanelBitmap
  : TcxBitmap;
var
  R: TRect;
begin
  ...
  try
    ...
    SendMessage(ViewInfo.FindPanel.Edit.InternalEdit.Handle,
      WM_SETREDRAW, 1, 0);
    try
...
function TdxPDFViewerFindPanelAnimationController.GetAnimation
  : TdxPDFViewerFindPanelAnimation;
begin
...
begin
  ...
  try
    RunAnimation(AFindPanelBitmap, Viewer.OptionsFindPanel.AnimationTime,
      GetMode, not AShowing);
  finally
    AFindPanelBitmap.Free;
  end;
end;

procedure TdxPDFViewerFindPanelAnimationController.AnimationHandler
...
procedure TdxPDFViewerFindPanelAnimationController.RedrawArea
  (const ARect: TRect);
begin
...
procedure TdxPDFViewerFindPanelAnimationController.RunAnimation(AImage: TBitmap;
ATime: Cardinal; AMode: TdxDrawAnimationMode; AIsHiding: Boolean);
begin
  FAnimation := TdxPDFViewerImageAnimationTransition.Create(AImage, ATime,
    AMode, AIsHiding);
  try
    Viewer.DeleteCaret;
...
function TdxPDFViewerFindPanelViewInfo.MeasureWidth(const ABounds
  : TRect): Integer;
...
begin
  inherited CreateCells;
  FCaption := AddCell(TdxPDFViewerCaptionViewInfo)
    as TdxPDFViewerCaptionViewInfo;
  if Viewer.OptionsFindPanel.ShowCloseButton and
    (Viewer.OptionsFindPanel.DisplayMode = fpdmManual) then
  ...
  if Viewer.OptionsFindPanel.ShowPreviousButton then
    FPreviousButton := AddCell(TPreviousButtonViewInfo)
      as TPreviousButtonViewInfo;
...
function TdxPDFViewerFindPanelViewInfo.GetActualNextButtonViewInfo
  : TButtonViewInfo;
begin
  Result := CloseButton;
end;
...
constructor TdxPDFViewerFindPanelViewInfo.TEditViewInfo.Create
  (AController: TdxPDFViewerContainerController);
begin
  inherited Create(AController);
...
function TdxPDFViewerFindPanelViewInfo.TEditViewInfo.GetEdit
  : TdxPDFViewerFindPanelTextEdit;
begin
...
function TdxPDFViewerFindPanelViewInfo.TCloseButtonViewInfo.
...
procedure TdxPDFViewerFindPanelViewInfo.TCloseButtonViewInfo.
  DrawButtonBackground(ACanvas: TcxCanvas; const ARect: TRect;
AState: TcxButtonState);
begin
  //
end;

procedure TdxPDFViewerFindPanelViewInfo.TCloseButtonViewInfo.DrawButtonContent
  (ACanvas: TcxCanvas);
begin
...
function TdxPDFViewerFindPanelViewInfo.TSearchButtonViewInfo.GetSearchDirection
  : TdxPDFDocumentTextSearchDirection;
begin
  Result := tsdForward;
end;
...
function TdxPDFViewerFindPanelViewInfo.TNextButtonViewInfo.
...
function TdxPDFViewerFindPanelViewInfo.TOptionsButtonViewInfo.IsEnabled
  : Boolean;
begin
  Result := not Viewer.TextSearch.IsLocked;
end;

function TdxPDFViewerFindPanelViewInfo.TOptionsButtonViewInfo.GetGlyph
  : TdxSmartGlyph;
begin
  Result := Viewer.FindPanel.OptionsButtonGlyph;
end;

function TdxPDFViewerFindPanelViewInfo.TOptionsButtonViewInfo.GetPopupMenuClass
  : TComponentClass;
begin
  Result := TdxPDFViewerFindPanelOptionsPopupMenu;
end;

function TdxPDFViewerFindPanelViewInfo.TOptionsButtonViewInfo.
  GetVisible: Boolean;
begin
...
function TdxPDFViewerFindPanelViewInfo.TPreviousButtonViewInfo.
  GetDefaultCaption: string;
begin
...
function TdxPDFViewerFindPanelViewInfo.TPreviousButtonViewInfo.
  GetSearchDirection: TdxPDFDocumentTextSearchDirection;
begin
  Result := tsdBackward;
end;
...
class procedure TdxPDFViewerInteractivityController.Execute
  (AViewer: TdxPDFCustomViewer; AOutline: TdxPDFOutlineTreeItem);
begin
  if AOutline <> nil then
    Execute(AViewer, TdxPDFOutlineTreeItemAccess(AOutline)
      .InteractiveOperation);
end;

class procedure TdxPDFViewerInteractivityController.Execute
  (AViewer: TdxPDFCustomViewer; AHyperlink: TdxPDFHyperlink);
begin
...
class procedure TdxPDFViewerInteractivityController.Execute
  (AViewer: TdxPDFCustomViewer; const AOperation: TdxPDFInteractiveOperation);
var
...
procedure TdxPDFViewerInteractivityController.ExecuteOperation(const AOperation
  : TdxPDFInteractiveOperation);
var
  AExecutedActions: TList<TdxPDFCustomAction>;
begin
...
procedure TdxPDFViewerInteractivityController.ShowDocumentPosition
  (const ATarget: TdxPDFTarget);
var
  APage: TdxPDFViewerPage;
  P: TdxPointF;
  ARect: TdxRectF;
begin
  if ATarget.IsValid and InRange(ATarget.PageIndex, 0, Viewer.PageCount - 1)
  then
  begin
    ...
    try
      ...
      if (ATarget.Mode = tmXYZ) and TdxPDFUtils.IsDoubleValid(ATarget.X) and
        TdxPDFUtils.IsDoubleValid(ATarget.Y) then
      begin
        P := dxPointF(ATarget.X - TdxPDFPageAccess(APage.DocumentPage)
          .CropBox.Left, ATarget.Y - TdxPDFPageAccess(APage.DocumentPage)
          .CropBox.Bottom);
        ARect := APage.ToViewerRect(dxRectF(P.X, P.Y, P.X + 1, P.Y - 1));
      end
      else
        ARect := APage.ToViewerRect
          (TdxPDFPageAccess(APage.DocumentPage).CropBox);
...
function TdxPDFViewerInteractivityController.QueryInterface(const IID: TGUID;
out Obj): HResult;
begin
...
procedure TdxPDFViewerInteractivityController.ExecuteOperation
  (AField: TdxPDFAcroFormActionField);
begin
...
procedure TdxPDFViewerInteractivityController.ExecuteActions
  (AAction: TdxPDFCustomAction; AExecutedActions: TList<TdxPDFCustomAction>);
var
...
procedure TdxPDFViewerNavigationPaneInternalControl.SetBounds
  (const AValue: TRect);
begin
...
begin
  Result := IsBookmarkSelected and (TreeView.Selected.Count > 0) and
    not TreeView.Selected.Expanded;
end;
...
begin
  ...
  if AOutline <> nil then
    Viewer.NavigationPane.Controller.ExecuteOperation
      (TdxPDFOutlineTreeItemAccess(AOutline).InteractiveOperation);
end;

procedure TdxPDFViewerBookmarkTreeView.PrintSelectedPages(APrintSections
  : Boolean);
var
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          ANode := TreeView.Items.AddChildObject(AParent, AOutline.Title,
            AOutline);
...
function TdxPDFViewerBookmarkTreeView.GetPrintPageNumbers(APrintSections
  : Boolean): TIntegerDynArray;
...
begin
  ...
  try
    Result := TdxPDFOutlineTreeAccess(OutlineTree).GetPrintPageNumbers
      (ASelectedOutlines, APrintSections);
  finally
    ASelectedOutlines.Free;
  end;
end;

procedure TdxPDFViewerBookmarkTreeView.OnChangeHandler(Sender: TObject;
Node: TTreeNode);
begin
  UpdateState;
end;

procedure TdxPDFViewerBookmarkTreeView.OnCustomDrawItemHandler
  ...
  begin
    ...
    begin
      if AOutline.Color <> dxPDFOutlineTreeItemDefaultColor then
        ...
        Canvas.Font.Color := TreeView.LookAndFeelPainter.
...
procedure TdxPDFViewerBookmarkTreeView.OnMouseUpHandler(Sender: TObject;
...
procedure TdxPDFViewerBookmarkTreeView.OnKeyDownHandler(Sender: TObject;
var Key: Word; Shift: TShiftState);
begin
...
procedure TdxPDFViewerAttachmentFileList.PopulateInternalControl;
  ...
  begin
    AIcon := nil;
    Result := SHGetFileInfo(PChar(AExtension), FILE_ATTRIBUTE_NORMAL, AFileInfo,
      SizeOf(AFileInfo), Flags) <> 0;
    if Result then
    begin
      AIcon := TIcon.Create;
...
function TdxPDFViewerAttachmentFileList.GetDocumentAttachments
...
procedure TdxPDFViewerAttachmentFileList.OnContextPopupHandler(Sender: TObject;
  ...
  begin
    ...
    begin
      APopupMenu := TdxPDFViewerNavigationPaneAttachmentPopupMenu.
        Create(Viewer);
      try
        Handled := TdxPDFViewerPopupMenuAccess(APopupMenu)
          .Popup(View.ClientToScreen(MousePos));
      finally
...
procedure TdxPDFViewerAttachmentFileList.OnKeyDownHandler(Sender: TObject;
var Key: Word; Shift: TShiftState);
begin
...
procedure TdxPDFViewerAttachmentFileList.OnMouseMoveHandler(Sender: TObject;
Shift: TShiftState; X, Y: Integer);
  ...
  begin
    ...
    if AAttachment.FileName <> '' then
      Result := cxGetResourceString
        ...
        AAttachment.FileName + dxCRLF;
    if AAttachment.Description <> '' then
      Result := Result + cxGetResourceString
    ...
    if S <> '' then
      Result := Result + cxGetResourceString
        (@sdxPDFViewerNavigationPageAttachmentModifiedCaption) + S + dxCRLF;
    S := AAttachment.GetSizeAsString;
    if S <> '' then
      Result := Result + cxGetResourceString
        (@sdxPDFViewerNavigationPageAttachmentFileSizeCaption) + S + dxCRLF;
...
function TdxPDFViewerBookmarkTreeView.TTree.DoMouseWheel(Shift: TShiftState;
WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  if not Viewer.HitTest.HitAtDocumentViewer then
    Result := inherited DoMouseWheel(Shift, WheelDelta, MousePos)
...
procedure TdxPDFViewerBookmarkTreeView.TTree.DoContextPopup(MousePos: TPoint;
var Handled: Boolean);
var
  P: TPoint;
...
procedure TdxPDFViewerBookmarkTreeView.TTree.KeyDown(var Key: Word;
Shift: TShiftState);
begin
...
procedure TdxPDFViewerBookmarkTreeView.TTree.MouseDown(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
var
  AItem: TTreeNode;
begin
...
procedure TdxPDFViewerBookmarkTreeView.TTree.SetTextSize
  (ATextSize: TdxPDFViewerBookmarksTextSize);
const
...
procedure TdxPDFViewerBookmarkTreeView.TTree.Changed(Sender: TObject;
AFont: TFont);
begin
...
function TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo.
  CanFocus: Boolean;
begin
  Result := False;
end;
...
function TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo.
  GetColorizeGlyph: Boolean;
begin
  Result := False;
end;

function TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo.GetGlyph
  : TdxSmartGlyph;
begin
...
function TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo.
  GetPopupMenuClass: TComponentClass;
begin
...
procedure TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo.
  DrawButtonBackground(ACanvas: TcxCanvas; const ARect: TRect;
AState: TcxButtonState);
begin
...
function TdxPDFViewerNavigationPanePageToolBarViewInfo.HasOptionsButton
  : Boolean;
begin
  Result := True;
end;
...
begin
  ...
  begin
    FOptionsButton :=
      AddCell(TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo)
      as TdxPDFViewerNavigationPanePageToolBarOptionsButtonViewInfo;
    FOptionsButton.OnGetPopupMenuClass := GetPopupMenuClass;
  end;
end;

procedure TdxPDFViewerNavigationPanePageToolBarViewInfo.DrawBackground
  (ACanvas: TcxCanvas);
begin
  Painter.DrawNavigationPanePageToolbarBackground(ACanvas, Bounds);
end;

function TdxPDFViewerNavigationPanePageToolBarViewInfo.GetPopupMenuClass
  : TComponentClass;
begin
  Result := nil;
end;
...
constructor TdxPDFViewerNavigationPanePageViewInfo.Create
  (AController: TdxPDFViewerContainerController;
APage: TdxPDFViewerNavigationPanePage);
begin
  inherited Create(AController);
...
function TdxPDFViewerNavigationPanePageViewInfo.CalculateHitTest
  (AHitTest: TdxPDFViewerCellHitTest): Boolean;
begin
  Result := inherited CalculateHitTest(AHitTest);
  if not FActualContentBounds.IsEmpty then
    AHitTest.HitCodes[hcNavigationPaneSplitter] :=
      not cxRectPtIn(FActualContentBounds, AHitTest.HitPoint);
end;
...
begin
  ...
  FToolBar := AddCell(GetToolbarClass)
    as TdxPDFViewerNavigationPanePageToolBarViewInfo;
  FButton := TdxPDFViewerNavigationPanePageButtonViewInfo.Create
    (Controller, Self);
end;
...
begin
  ...
  AContentBounds.Top := Painter.LookAndFeelPainter.
    PDFViewerNavigationPaneButtonRect
    (Viewer.ViewInfo.NavigationPane.FirstPageButton.Bounds, cxbsPressed, True,
    ScaleFactor).Top;
  AlignToTopClientSide(FCaption, AContentBounds);
  FCaption.Bounds := cxRect(FCaption.Bounds.Left, Bounds.Top,
    FCaption.Bounds.Right, FCaption.Bounds.Bottom);

  FActualContentBounds := AContentBounds;

  FActualContentBounds := cxRectContent(FActualContentBounds,
    cxRect(Painter.NavigationPanePageContentOffsets.Left, 0,
    Painter.NavigationPanePageContentOffsets.Right,
    Painter.NavigationPanePageContentOffsets.Bottom));
  AlignToTopClientSide(FToolBar, FActualContentBounds);
end;

procedure TdxPDFViewerNavigationPanePageViewInfo.DrawBackground
  (ACanvas: TcxCanvas);
begin
  Painter.DrawNavigationPanePageBackground(ACanvas, Bounds);
end;

function TdxPDFViewerNavigationPanePageViewInfo.GetToolbarClass
  : TdxPDFViewerCellViewInfoClass;
begin
...
function TdxPDFViewerNavigationPanePageViewInfo.TCaptionText.
  GetPainterTextColor: TColor;
begin
  Result := Painter.TitleTextColor;
end;
...
function TdxPDFViewerNavigationPanePageViewInfo.TCaptionText.GetTextAlignment
  : TAlignment;
begin
  Result := taLeftJustify;
end;

procedure TdxPDFViewerNavigationPanePageViewInfo.TCaptionText.PrepareCanvas
  (ACanvas: TcxCanvas);
begin
...
function TdxPDFViewerNavigationPanePageCaptionButton.GetGlyphAlignmentHorz
  : TAlignment;
begin
  Result := taCenter;
end;
...
procedure TdxPDFViewerNavigationPanePageCaptionButton.DrawButtonContent
  (ACanvas: TcxCanvas);
begin
  Painter.DrawButtonGlyph(ACanvas, GetGlyph, Bounds, State, GetColorizeGlyph);
end;

procedure TdxPDFViewerNavigationPanePageCaptionButton.DrawButtonBackground
  (ACanvas: TcxCanvas; const ARect: TRect; AState: TcxButtonState);
begin
...
constructor TdxPDFViewerNavigationPanePageButtonViewInfo.Create
  (AController: TdxPDFViewerContainerController;
...
function TdxPDFViewerNavigationPanePageButtonViewInfo.CalculateState
  : TcxButtonState;
begin
...
function TdxPDFViewerNavigationPanePageButtonViewInfo.GetGlyphAlignmentHorz
  : TAlignment;
begin
  Result := taCenter;
end;
...
begin
  Result := Max(GlyphSize.cy + cxMarginsHeight
    (Painter.NavigationPaneButtonContentOffsets),
    Min(Painter.NavigationPaneButtonSize.cy,
    Painter.NavigationPaneButtonSize.cy));
end;

function TdxPDFViewerNavigationPanePageButtonViewInfo.MeasureWidth: Integer;
begin
  Result := Max(GlyphSize.cx + cxMarginsWidth
    (Painter.NavigationPaneButtonContentOffsets),
    Min(Painter.NavigationPaneButtonSize.cx,
    Painter.NavigationPaneButtonSize.cy));
end;
...
procedure TdxPDFViewerNavigationPanePageButtonViewInfo.DrawButtonBackground
  (ACanvas: TcxCanvas; const ARect: TRect; AState: TcxButtonState);
begin
  // do nothing
end;

procedure TdxPDFViewerNavigationPanePageButtonViewInfo.DrawButtonContent
  (ACanvas: TcxCanvas);
begin
  Painter.DrawNavigationPaneButton(ACanvas, Bounds, State,
    Viewer.OptionsNavigationPane.ActivePageState = wsMinimized,
    IsActive, IsFirst);
...
begin
  Result := (Viewer.ViewInfo <> nil) and
    (Viewer.ViewInfo.NavigationPane <> nil);
  if Result then
  begin
    AActivePage := Viewer.ViewInfo.NavigationPane.ActivePage;
    Result := (AActivePage <> nil) and
      (Viewer.OptionsNavigationPane.ActivePageState <> wsMinimized) and
...
function TdxPDFViewerNavigationPanePageViewInfo.TMinimizeButton.GetGlyph
  : TdxSmartGlyph;
begin
...
function TdxPDFViewerNavigationPanePageViewInfo.TMaximizeButton.GetGlyph
  : TdxSmartGlyph;
begin
...
function TdxPDFViewerNavigationPanePageViewInfo.THeader.
  GetContentMargins: TRect;
begin
  Result := Painter.NavigationPanePageCaptionContentOffsets;
end;

function TdxPDFViewerNavigationPanePageViewInfo.THeader.GetOnGetText
  : THeaderOnGetTextEvent;
begin
  Result := FCaption.OnGetText;
end;

procedure TdxPDFViewerNavigationPanePageViewInfo.THeader.SetOnGetText
  (const AValue: THeaderOnGetTextEvent);
begin
  FCaption.OnGetText := AValue;
end;

function TdxPDFViewerNavigationPanePageViewInfo.THeader.AlignToClient
  (AViewInfo: TdxPDFViewerCellViewInfo; var R: TRect): Boolean;
var
  ARect: TRect;
begin
...
begin
  Result := Viewer.ViewInfo.NavigationPane.FirstPageButton.Bounds.Height +
    Painter.NavigationPaneButtonOverlay.Y;
end;
...
procedure TdxPDFViewerNavigationPanePageViewInfo.THeader.DrawBackground
  (ACanvas: TcxCanvas);
begin
...
function TdxPDFViewerBookmarksPageViewInfo.GetOutlineTreeView
  : TdxPDFViewerBookmarkTreeView;
begin
  Result := Viewer.Bookmarks.Tree;
end;

function TdxPDFViewerBookmarksPageViewInfo.GetToolbarClass
  : TdxPDFViewerCellViewInfoClass;
begin
...
function TdxPDFViewerThumbnailsPageViewInfo.GetPreview
  : TdxPDFViewerPageThumbnailPreview;
begin
...
function TdxPDFViewerThumbnailsPageViewInfo.GetToolbarClass
  : TdxPDFViewerCellViewInfoClass;
begin
...
procedure TdxPDFViewerThumbnailsPageViewInfo.Calculate;
  ...
  begin
    AContentBounds := cxRectSetRight(FToolBar.ActualContentBounds,
      FToolBar.ActualContentBounds.Right, 196);
    ...
    if FToolBar.ActualContentBounds.Width < ABounds.Width then
      ABounds := cxNullRect;
    if not cxRectIsEqual(Viewer.Thumbnails.SizeTrackBar.BoundsRect, ABounds)
    then
    begin
      Viewer.Thumbnails.SizeTrackBar.BoundsRect := ABounds;
      Viewer.Thumbnails.SizeTrackBar.Visible :=
        not cxRectIsEmpty(Viewer.Thumbnails.SizeTrackBar.BoundsRect);
    end;
  end;
...
function TdxPDFViewerThumbnailsPageViewInfo.TThumbnailsToolBar.GetPopupMenuClass
  : TComponentClass;
begin
...
function TdxPDFViewerAttachmentsPageViewInfo.GetFileList
  : TdxPDFViewerAttachmentFileList;
begin
  Result := Viewer.Attachments.FileList;
end;

function TdxPDFViewerAttachmentsPageViewInfo.GetToolbarClass
  : TdxPDFViewerCellViewInfoClass;
begin
...
function TdxPDFViewerAttachmentsPageViewInfo.TAttachmentsToolBar.
  GetPopupMenuClass: TComponentClass;
begin
  Result := nil;
end;
...
function TdxPDFViewerAttachmentsPageViewInfo.TOpenButton.
  GetColorizeGlyph: Boolean;
begin
  Result := False;
end;

function TdxPDFViewerAttachmentsPageViewInfo.TOpenButton.GetGlyph
  : TdxSmartGlyph;
begin
...
begin
  Result := cxGetResourceString
    (@sdxPDFViewerNavigationPageOpenAttachmentButtonHint);
end;
...
procedure TdxPDFViewerAttachmentsPageViewInfo.TOpenButton.DrawButtonBackground
  (ACanvas: TcxCanvas; const ARect: TRect; AState: TcxButtonState);
begin
  if AState in [cxbsHot, cxbsPressed] then
    Painter.DrawButtonBackground(ACanvas, ARect, AState);
...
function TdxPDFViewerAttachmentsPageViewInfo.TSaveButton.
  GetColorizeGlyph: Boolean;
begin
  Result := False;
end;

function TdxPDFViewerAttachmentsPageViewInfo.TSaveButton.GetGlyph
  : TdxSmartGlyph;
begin
...
begin
  Result := cxGetResourceString
    (@sdxPDFViewerNavigationPageSaveAttachmentButtonHint);
end;
...
procedure TdxPDFViewerAttachmentsPageViewInfo.TSaveButton.DrawButtonBackground
  (ACanvas: TcxCanvas; const ARect: TRect; AState: TcxButtonState);
begin
  if AState in [cxbsHot, cxbsPressed] then
    Painter.DrawButtonBackground(ACanvas, ARect, AState);
...
function TdxPDFViewerPageThumbnailPreview.GetOnCustomDrawPreRenderPage
  : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent;
begin
...
begin
  Result := TThumbnailPreviewAccess
    (FInternalControl as TdxPDFDocumentPageThumbnailViewer);
end;
...
procedure TdxPDFViewerPageThumbnailPreview.SetOnSizeChanged
  (const AValue: TNotifyEvent);
begin
  Preview.OnThumbnailSizeChanged := AValue;
end;

procedure TdxPDFViewerPageThumbnailPreview.SetSelectedPageIndex
  (const AValue: Integer);
begin
  if not Locked then
  begin
...
procedure TdxPDFViewerPageThumbnailPreview.OnSelectedPageChangedHandler
...
function TdxPDFViewerBookmarksPageViewInfo.TExpandBookmarkButton.
  GetColorizeGlyph: Boolean;
begin
  Result := False;
end;

function TdxPDFViewerBookmarksPageViewInfo.TExpandBookmarkButton.GetGlyph
  : TdxSmartGlyph;
begin
...
begin
  Result := cxGetResourceString
    (@sdxPDFViewerNavigationPageExpandBookmarkButtonHint);
end;
...
procedure TdxPDFViewerBookmarksPageViewInfo.TExpandBookmarkButton.
  DrawButtonBackground(ACanvas: TcxCanvas; const ARect: TRect;
AState: TcxButtonState);
begin
  if AState in [cxbsHot, cxbsPressed] then
...
function TdxPDFViewerBookmarksPageViewInfo.TBookmarksToolBar.GetPopupMenuClass
  : TComponentClass;
begin
...
constructor TdxPDFViewerNavigationPaneViewInfo.Create
  (AController: TdxPDFViewerContainerController;
...
function TdxPDFViewerNavigationPaneViewInfo.GetPageIndex
  (AButton: TdxPDFViewerNavigationPanePageButtonViewInfo): Integer;
begin
  Result := FPageButtons.IndexOf(AButton);
end;

function TdxPDFViewerNavigationPaneViewInfo.GetActivePage
  : TdxPDFViewerNavigationPanePageViewInfo;
begin
...
function TdxPDFViewerNavigationPaneViewInfo.GetFirstPageButton
  : TdxPDFViewerNavigationPanePageButtonViewInfo;
begin
...
procedure TdxPDFViewerNavigationPaneViewInfo.AddPage
  (APage: TdxPDFViewerNavigationPanePage);
var
...
const
  SplitterHotZone = 4;
begin
  if not cxRectIsEmpty(FPageBounds) and
    (Viewer.NavigationPane.ActivePageState = wsNormal) then
  begin
    FSplitterBounds := FPageBounds;
    FSplitterBounds.Left := FPageBounds.Right -
      ApplyScaleFactor(SplitterHotZone);
    FSplitterBounds.Right := FPageBounds.Right +
      ApplyScaleFactor(SplitterHotZone);
  end
  else
...
constructor TdxPDFViewerOptionsNavigationPage.Create(AOwner: TPersistent;
APage: TdxPDFViewerNavigationPanePage);
begin
  inherited Create(AOwner);
  FPage := APage;
...
procedure TdxPDFViewerOptionsNavigationPage.SetGlyph(const AValue
  : TdxSmartGlyph);
begin
  FPage.Glyph.Assign(AValue);
end;

procedure TdxPDFViewerOptionsNavigationPage.SetVisible
...
procedure TdxPDFViewerOptionsBookmarks.SetTextSize(const AValue
...
function TdxPDFViewerNavigationPanePage.CreateOptions
  : TdxPDFViewerOptionsNavigationPage;
begin
...
procedure TdxPDFViewerNavigationPanePage.SetOptions(const AValue
  : TdxPDFViewerOptionsNavigationPage);
begin
  FOptions.Assign(AValue);
end;

procedure TdxPDFViewerNavigationPanePage.SetVisible(const AValue
  : TdxDefaultBoolean);
begin
  if FVisible <> AValue then
  begin
...
begin
  FSizeTrackBar.Position := FSizeTrackBar.Position +
    dxPDFDocumentPageThumbnailViewerSizeStep;
end;
...
function TdxPDFViewerThumbnails.CreateOptions
  : TdxPDFViewerOptionsNavigationPage;
begin
  Result := TdxPDFViewerOptionsThumbnails.Create(Viewer, Self);
end;

function TdxPDFViewerThumbnails.CreateViewInfo
  : TdxPDFViewerNavigationPanePageViewInfo;
begin
  FViewInfo := TdxPDFViewerThumbnailsPageViewInfo.Create
    (Viewer.NavigationPane.Controller, Self);
  Result := FViewInfo;
end;
...
function TdxPDFViewerThumbnails.GetControlClass
  : TdxPDFViewerNavigationPaneInternalControlClass;
begin
...
function TdxPDFViewerThumbnails.GetThumbnailPreview
  : TdxPDFViewerPageThumbnailPreview;
begin
...
begin
  ...
  FSizeTrackBar.Hint := cxGetResourceString
    (@sdxPDFViewerNavigationPageThumbnailsSizeTrackBarHint);
  ...
  try
    ...
    FSizeTrackBar.Properties.LineSize :=
      dxPDFDocumentPageThumbnailViewerSizeStep;
  finally
...
procedure TdxPDFViewerOptionsNavigationPane.SetActivePageState
  (const AValue: TWindowState);
begin
  Viewer.NavigationPane.ActivePageState := AValue;
end;

procedure TdxPDFViewerOptionsNavigationPane.SetAttachments
  (const AValue: TdxPDFViewerOptionsAttachments);
begin
  Viewer.NavigationPane.Attachments.Options := AValue;
end;

procedure TdxPDFViewerOptionsNavigationPane.SetBookmarks
  (const AValue: TdxPDFViewerOptionsBookmarks);
begin
  Viewer.NavigationPane.Bookmarks.Options := AValue;
end;

procedure TdxPDFViewerOptionsNavigationPane.SetThumbnails
  (const AValue: TdxPDFViewerOptionsThumbnails);
begin
  Viewer.NavigationPane.Thumbnails.Options := AValue;
end;

function TdxPDFViewerOptionsNavigationPane.GetActivePage
  : TdxPDFViewerNavigationPaneActivePage;
begin
...
function TdxPDFViewerOptionsNavigationPane.GetAttachments
  : TdxPDFViewerOptionsAttachments;
begin
  Result := Viewer.NavigationPane.Attachments.Options as
    TdxPDFViewerOptionsAttachments;
end;

function TdxPDFViewerOptionsNavigationPane.GetBookmarks
  : TdxPDFViewerOptionsBookmarks;
begin
  Result := Viewer.NavigationPane.Bookmarks.Options as
    TdxPDFViewerOptionsBookmarks;
end;

function TdxPDFViewerOptionsNavigationPane.GetThumbnails
  : TdxPDFViewerOptionsThumbnails;
begin
  Result := Viewer.NavigationPane.Thumbnails.Options as
    TdxPDFViewerOptionsThumbnails;
end;

procedure TdxPDFViewerOptionsNavigationPane.SetActivePage
  (const AValue: TdxPDFViewerNavigationPaneActivePage);
begin
...
function TdxPDFViewerBookmarks.CreateViewInfo
  : TdxPDFViewerNavigationPanePageViewInfo;
begin
  FViewInfo := TdxPDFViewerBookmarksPageViewInfo.Create
    (Viewer.NavigationPane.Controller, Self);
  Result := FViewInfo;
end;
...
function TdxPDFViewerBookmarks.GetControlClass
  : TdxPDFViewerNavigationPaneInternalControlClass;
begin
...
begin
  ...
  FExpandBookmarkGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWEREXPANDBOOKMARKBUTTON', 'PNG');
end;
...
function TdxPDFViewerAttachments.CreateOptions
  : TdxPDFViewerOptionsNavigationPage;
begin
  Result := TdxPDFViewerOptionsAttachments.Create(Viewer, Self);
end;

function TdxPDFViewerAttachments.CreateViewInfo
  : TdxPDFViewerNavigationPanePageViewInfo;
begin
  FViewInfo := TdxPDFViewerAttachmentsPageViewInfo.Create
    (Viewer.NavigationPane.Controller, Self);
  Result := FViewInfo;
end;
...
function TdxPDFViewerAttachments.GetControlClass
  : TdxPDFViewerNavigationPaneInternalControlClass;
begin
...
begin
  FDefaultGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERATTACHMENTSBUTTON', 'SVG');
  FOpenAttachmentGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERATTACHMENTOPENBUTTON', 'PNG');
  FSaveAttachmentGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERATTACHMENTSAVEBUTTON', 'PNG');
end;
...
function TdxPDFViewerNavigationPane.CalculateParentClientBounds
  (const AClientRect: TRect): TRect;
begin
  if IsMaximized then
    Result := cxNullRect
  else
...
function TdxPDFViewerNavigationPane.IsFirst
  (APage: TdxPDFViewerNavigationPanePage): Boolean;
begin
  Result := FVisiblePages.IndexOf(APage) = 0;
end;

function TdxPDFViewerNavigationPane.IsVisible
  (APage: TdxPDFViewerNavigationPanePage): Boolean;
begin
...
procedure TdxPDFViewerNavigationPane.VisibilityChanged
  (APage: TdxPDFViewerNavigationPanePage);
begin
  UpdateVisiblePages;
...
function TdxPDFViewerNavigationPane.GetActivePageType
  : TdxPDFViewerNavigationPaneActivePage;
begin
  if ActivePage = nil then
    Result := apNone
  else if ActivePage = Bookmarks then
    Result := apBookmarks
  else if ActivePage = Attachments then
...
function TdxPDFViewerNavigationPane.GetVisiblePages
  : TList<TdxPDFViewerNavigationPanePage>;
begin
  UpdateVisiblePages;
  Result := FVisiblePages;
end;

procedure TdxPDFViewerNavigationPane.SetActivePage(const AValue
  : TdxPDFViewerNavigationPanePage);
begin
...
procedure TdxPDFViewerNavigationPane.SetActivePageType
  (const AValue: TdxPDFViewerNavigationPaneActivePage);
var
  APage: TdxPDFViewerNavigationPanePage;
begin
...
procedure TdxPDFViewerNavigationPane.SetActivePageState
  (const AValue: TWindowState);
begin
...
procedure TdxPDFViewerNavigationPane.SetOptions(const AValue
  : TdxPDFViewerOptionsNavigationPane);
begin
  FOptions.Assign(AValue);
end;
...
begin
  FMaximizeButtonGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERMAXIMIZEBUTTON', 'PNG');
  FMenuButtonGlyph := TdxPDFUtils.LoadGlyph('DX_PDFVIEWERMENUBUTTON', 'PNG');
  FMinimizeButtonGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERMINIMIZEBUTTON', 'PNG');
  FRestoreButtonGlyph := TdxPDFUtils.LoadGlyph
    ('DX_PDFVIEWERRESTOREBUTTON', 'PNG');
end;
