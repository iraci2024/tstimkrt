...
begin
  Result := (inherited HandlesTarget(Target) or (Target is TdxPDFCustomViewer))
    and (not NeedControlFocus or TdxPDFCustomViewer(Target).Focused);
end;
...
begin
  Result := inherited IsEnabled and
    (Control.CurrentPageIndex < Control.PageCount - 1);
end;
...
begin
  Result := inherited IsEnabled and TdxPDFViewerAccess(Control)
    .CanChangeVisibility;
end;
