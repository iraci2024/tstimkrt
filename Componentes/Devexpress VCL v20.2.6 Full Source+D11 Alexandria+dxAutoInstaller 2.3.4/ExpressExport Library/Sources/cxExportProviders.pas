...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, SysUtils, Classes, Graphics, Variants, Generics.Defaults,
  Generics.Collections, ImgList,
{$IFNDEF NONDB}
  FMTBcd, SqlTimSt,
{$ENDIF}
  cxClasses, dxCore, cxExport, cxGraphics, dxSpreadSheet, dxSpreadSheetCore,
  dxSpreadSheetClasses, dxSpreadSheetTypes,
  dxSpreadSheetGraphics, dxSpreadSheetUtils, dxSpreadSheetFormatHTML,
  dxSpreadSheetFormatXLS, dxSpreadSheetFormatTXT,
  dxSpreadSheetFormatXML, dxSpreadSheetFormatCSV, cxFormats, dxHashUtils,
  cxGeometry, dxSpreadSheetCoreStyles,
  dxSpreadSheetConditionalFormatting, dxCoreClasses, dxSpreadSheetContainers,
  cxEdit, cxImage, cxImageCombobox;
...
type
  ...
  TcxExportProvider = class(TcxCustomExportProvider, IcxExportProvider,
    IcxExportProvider2, IcxNameExportProvider)
  strict private
    ...
    function CreateStyle(const AStyle: TcxCacheCellStyle)
      : TdxSpreadSheetCellStyleHandle;
    function GetBorderStyleByWidth(AWidth: Integer)
    ...
    procedure SetupPictureContainer(AContainer: TdxSpreadSheetPictureContainer;
  ...
  protected
    ...
    // IcxExportProvider
    procedure AddSharedImageHandle(AImageList: TObject; AImageIndex: Integer;
      AHandle: TObject);
    function AddPictureContainer(const AArea: TRect; AFitMode: TcxImageFitMode;
      const ASize: TSize): TdxSpreadSheetPictureContainer; inline;
    procedure AddConditionalFormattingRule
    ...
    function SetCellGraphic(const AArea: TRect; AGraphic: TGraphic;
      AFitMode: TcxImageFitMode = ifmStretch): TObject; overload;
    function SetCellGraphic(const AArea: TRect; AStyleIndex: Integer;
      AGraphic: TGraphic; AFitMode: TcxImageFitMode = ifmStretch)
      : TObject; overload;
    procedure SetCellGraphicAsSharedHandle(const AArea: TRect;
      AStyleIndex: Integer; AHandle: TObject;
      AFitMode: TcxImageFitMode = ifmStretch);
    procedure SetCellPNGImage(const ACol, ARow: Integer; APNGImage: TGraphic;
      AFitMode: TcxImageFitMode = ifmStretch);
    ...
    // IcxExportProvider2
    function AddContainer(AItem: TcxExportDataItem; AFitMode: TcxImageFitMode)
      : TdxSpreadSheetPictureContainer; inline;
    procedure AddGraphic(AItem: TcxExportDataItem; AGraphic: TGraphic;
      AFitMode: TcxImageFitMode);
    procedure AddImageListItem(AItem: TcxExportDataItem;
      AImages: TCustomImageList; AImageIndex: Integer; ABkColor: TColor);
    procedure AddText(AItem: TcxExportDataItem; const AText: string);
    procedure AddValue(AItem: TcxExportDataItem; const AValue: Variant;
      AProperties: TcxCustomEditProperties;
      const AValueDisplayFormat: string = '';
      AValueDisplayFormatType: TcxValueDisplayFormatType = vdftAuto);
    procedure SetData(AItem: TcxExportDataItem);
    //
    function TryGetSharedImageHandle(AImageList: TObject; AImageIndex: Integer;
      var AHandle: TObject): Boolean; inline;
  ...
  TcxExportToCSVProvider = class(TcxExportProvider, IcxExportWithSeparators,
  ...
  TcxExportToTXTProvider = class(TcxExportProvider, IcxExportProviderEncoding,
    IcxExportWithSeparators)
  strict private
  ...
  protected
    function AddCellValue(ARow, AColumn: Integer; const AValue: Variant)
      : TdxSpreadSheetCell;
    procedure AddFrame(const AArea: TRect); overload; inline;
    procedure AddFrame(ARow, AColumn: Integer); overload; inline;
    procedure AddGroupBy(AItems: TdxSpreadSheetTableItems;
      AStartIndex, AFinishIndex: Integer; AExpanded: Boolean);
    procedure AddGroupByColumns(AStartIndex, AFinishIndex: Integer;
      AExpanded: Boolean = True);
    procedure AddGroupByRows(AStartIndex, AFinishIndex: Integer;
      AExpanded: Boolean = True);
    function PrepareDisplayFormat(ACell: TdxSpreadSheetCell;
      const ADisplayFormat: string): string; inline;
    procedure RemoveInnerBorders(const AArea: TRect); inline;
    procedure SetBorders(const AArea: TRect;
      AInnerStyle, AOuterStyle: TdxSpreadSheetCellBorderStyle);
    procedure SetDisplayFormat(ACell: TdxSpreadSheetCell;
      const ADisplayFormat: string); overload;
    procedure SetDisplayFormat(ARow, AColumn: Integer;
      const ADisplayFormat: string); overload;
    //
...
function dxPrepareBcdForExport(const AValue: Variant;
  AIsCurrency: Boolean): Variant;

implementation

uses
  Math, StrUtils, DateUtils, cxExportStrs, dxSpreadSheetFormatXLSX,
  dxGDIPlusClasses,
...
type
  ...
  public
    class procedure SetDataFormat(ACell: TdxSpreadSheetCell;
      const AFormatCode: string); overload;
    class procedure SetDataFormat(ACell: TdxSpreadSheetCell;
      const AFormatCodeID: Integer); overload;
  end;
...
begin
  ...
  begin
    ...
    if AError = 0 then
      Result := AInt64Value
    else if TryStrToFloat(ABcdAsSting, AFloatValue) then
      Result := AFloatValue
  end;
end;
...
function TdxSpreadSheetBasedExporter.DoCreateSheet
  (var ASheet: TdxSpreadSheetCustomView; const ACaption: string;
  AViewClass: TdxSpreadSheetCustomViewClass): Boolean;
begin
...
procedure TdxSpreadSheetBasedExporterCell.Changed
...
constructor TdxSpreadSheetBasedExporterTableRow.Create
  (AOwner: TdxDynamicItemList; AIndex: Integer);
begin
  inherited Create(AOwner, AIndex);
  MarkBestFitDirty;
end;

function TdxSpreadSheetBasedExporterTableRow.CreateRowCells
  : TdxSpreadSheetTableRowCells;
begin
  Result := TdxSpreadSheetBasedExporterTableRowCells.Create(Self);
end;

procedure TdxSpreadSheetBasedExporterTableRow.DoCellCreated
...
function TdxSpreadSheetBasedExporterTableRowCells.GetItemClass
...
function TdxSpreadSheetBasedExporterTableRows.GetItemClass
  : TdxDynamicListItemClass;
begin
...
function TdxSpreadSheetBasedExporterTableView.CreateRows
  : TdxSpreadSheetTableRows;
begin
  Result := TdxSpreadSheetBasedExporterTableRows.Create(Self);
end;

function TdxSpreadSheetBasedExporterTableView.CreateViewInfo
...
begin
  ...
  APrevDPI := dxSpreadSheetPrepareCanvas(ACanvas,
    TdxSpreadSheetAccess(TableView.SpreadSheet).Font, dxDefaultDPI);
  try
    ...
    begin
      ...
      begin
        ACell := TdxSpreadSheetBasedExporterCell
          (TdxSpreadSheetTableRowAccess(ARow).RowCells.First);
        while ACell <> nil do
        begin
          dxSpreadSheetTextService.MeasureSize(ACanvas, ACell, @AWidth,
            @AHeight);
...
function TcxExportProvider.AddPictureContainer(const AArea: TRect;
  AFitMode: TcxImageFitMode; const ASize: TSize)
...
procedure TcxExportProvider.AddSharedImageHandle(AImageList: TObject;
  AImageIndex: Integer; AHandle: TObject);
var
...
procedure TcxExportProvider.AddConditionalFormattingRule
  (ARule: TdxSpreadSheetCustomConditionalFormattingRule; AAreas: TdxRectList);
var
  ATargetRule: TdxSpreadSheetCustomConditionalFormattingRule;
begin
  TableView.ConditionalFormatting.Add
    (TdxSpreadSheetConditionalFormattingCustomRuleClass(ARule.ClassType),
    ATargetRule);
...
function TcxExportProvider.AddContainer(AItem: TcxExportDataItem;
  AFitMode: TcxImageFitMode): TdxSpreadSheetPictureContainer;
begin
...
procedure TcxExportProvider.AddGraphic(AItem: TcxExportDataItem;
  AGraphic: TGraphic; AFitMode: TcxImageFitMode);
var
...
begin
  ...
  begin
    ...
    try
      ...
      AddSharedImageHandle(AImages, AImageIndex,
        TdxSpreadSheetPictureAccess(AContainer.Picture).ImageHandle);
    finally
      ABitmap.Free;
    end;
  end
    ...
    TdxSpreadSheetPictureAccess(AContainer.Picture).ImageHandle :=
      TdxSpreadSheetSharedImageHandle(AHandle);
end;

procedure TcxExportProvider.AddText(AItem: TcxExportDataItem;
  const AText: string);
begin
  AItem.DataType := 0;
  if AText = '' then
...
procedure TcxExportProvider.AddValue(AItem: TcxExportDataItem;
  const AValue: Variant; AProperties: TcxCustomEditProperties;
  const AValueDisplayFormat: string = '';
  AValueDisplayFormatType: TcxValueDisplayFormatType = vdftAuto);
var
  R: TRect;
...
begin
  ...
  begin
    TcxImageComboBoxProperties(AProperties).GetImageComboBoxDisplayValue(AValue,
      AText, AImageIndex);
    if TcxImageComboBoxProperties(AProperties).ShowDescriptions or
      (TcxImageComboBoxProperties(AProperties).Images = nil) or
      (AImageIndex < 0) or
      (AImageIndex >= TcxImageComboBoxProperties(AProperties).Images.Count) then
      AddText(AItem, AText)
    else
      AddImageListItem(AItem, TcxImageComboBoxProperties(AProperties).Images,
        AImageIndex, GetStyle(AItem.Style).BrushBkColor);
    Exit;
  end
  else if AProperties is TcxImageProperties then
  begin
    ...
    try
      ...
      begin
        AContainer := AddContainer(AItem,
          TcxImageProperties(AProperties).FitMode);
  ...
  if FProxyCell.Style.Handle.DataFormat <> SpreadSheet.DefaultCellStyle.Handle.DataFormat
  then
  begin
...
procedure TcxExportProvider.Commit(AProgressHelper
...
function TcxExportProvider.RegisterStyle(const AStyle
...
function TcxExportProvider.SetCellGraphic(const AArea: TRect;
...
begin
  try
    AContainer := AddPictureContainer(AArea, AFitMode,
      cxSize(AGraphic.Width, AGraphic.Height));
...
procedure TcxExportProvider.SetCellGraphicAsSharedHandle(const AArea: TRect;
  AStyleIndex: Integer; AHandle: TObject; AFitMode: TcxImageFitMode);
var
  AContainer: TdxSpreadSheetPictureContainer;
begin
  SetCellStyle(AArea, AStyleIndex);
  AContainer := AddPictureContainer(AArea, AFitMode,
    TdxSpreadSheetSharedImageHandle(AHandle).Image.Size);
  TdxSpreadSheetPictureAccess(AContainer.Picture).ImageHandle :=
    TdxSpreadSheetSharedImageHandle(AHandle);
end;

procedure TcxExportProvider.SetCellPNGImage(const ACol, ARow: Integer;
  APNGImage: TGraphic; AFitMode: TcxImageFitMode);
begin
  SetCellGraphic(cxRectBounds(ACol, ARow, 1, 1), APNGImage, AFitMode);
end;

procedure TcxExportProvider.SetCellStyle(const AArea: TRect;
  AStyleIndex: Integer);
var
  C, R: Integer;
begin
...
procedure TcxExportProvider.SetCellStyle(const ACol, ARow,
  AStyleIndex: Integer);
var
  ACell: TdxSpreadSheetCell;
...
procedure TcxExportProvider.SetCellUnion(const ACol, ARow: Integer;
  H, W: Integer);
begin
  if (H > 1) or (W > 1) then
    TableView.MergedCells.Add(cxRectBounds(ACol, ARow, Max(0, W - 1),
      Max(0, H - 1)));
end;

procedure TcxExportProvider.SetCellValue(const ACol, ARow: Integer;
...
begin
  if AValueDisplayFormatType = vdftRTF then
    if dxSpreadSheetTextService.ForceSetAsRTF(TableView.CreateCell(ARow, ACol),
      dxVariantToString(AValue)) then
      Exit;
...
{$ENDIF}
  ...
  begin
    if AValueDisplayFormatType = vdftDateTime then
      ACellFormatCode := TdxSpreadSheetDisplayFormatConverter.
        ConvertDateTimeDisplayFormat(AValueDisplayFormat, ACell.AsDateTime)
    else
      ACellFormatCode := TdxSpreadSheetDisplayFormatConverter.
        ConvertFloatValueDisplayFormat(AValueDisplayFormat);
  end
  else if AValueDisplayFormatType = vdftDateTime then
    ACellFormatCode := TdxSpreadSheetDateTimeFormatHelper.GetDefaultFormat
      (ACell.AsDateTime)
  else
    ACellFormatCode := '';
...
begin
  AFormula := StringReplace(AValue, AListSeparator,
    SpreadSheet.FormulaController.FormatSettings.Data.ListSeparator,
...
procedure TcxExportProvider.SetCellValueAsString(const ACol, ARow: Integer;
  const AText: string);
begin
...
begin
  ...
  ACell := TdxSpreadSheetCellAccess(SpreadSheet.ActiveSheetAsTable.CreateCell
    (AItem.Bounds.Top, AItem.Bounds.Left));
  ...
  begin
    ...
    begin
      ...
      SetupPictureContainer(AContainer, AItem.Bounds,
        TcxImageFitMode($FF - AItem.DataType),
...
procedure TcxExportProvider.SetRange(const AColCount, ARowCount: Integer;
  IsVisible: Boolean);
begin
  // do nothing
end;
...
function TcxExportProvider.TryGetSharedImageHandle(AImageList: TObject;
  AImageIndex: Integer; var AHandle: TObject): Boolean;
var
  ASharedInfo: TcxExportSharedImagesInfo;
begin
...
procedure TcxExportProvider.SetRangeName(const AName: string;
  const ARange: TRect);
begin
  SpreadSheet.DefinedNames.Add(AName, dxReferenceToString(ARange, False),
    TableView);
end;

function TcxExportProvider.CreateStyle(const AStyle: TcxCacheCellStyle)
  : TdxSpreadSheetCellStyleHandle;
const
  AlignTextMap: array [TcxAlignText] of TdxSpreadSheetDataAlignHorz = (ssahLeft,
    ssahCenter, ssahRight);
  AlignTextVertMap: array [TcxAlignTextVert] of TdxSpreadSheetDataAlignVert =
    (ssavTop, ssavTop, ssavCenter, ssavBottom);
...
begin
  ...
  begin
    ...
    begin
      ABordersHandle.BorderColor[ABorderSide] :=
        AStyle.Borders[Ord(ABorderSide)].Color;
      ABordersHandle.BorderStyle[ABorderSide] :=
        GetBorderStyleByWidth(AStyle.Borders[Ord(ABorderSide)].Width);
    end;
  end;
  ...
  Result := SpreadSheet.CellStyles.CreateStyle(AFontHandle, nil, ABrushHandle,
    ABordersHandle);
...
function TcxExportProvider.GetBorderStyleByWidth(AWidth: Integer)
  : TdxSpreadSheetCellBorderStyle;
begin
  case AWidth of
    0:
...
function TcxExportProvider.GetStyleIndex
  (AHandle: TdxSpreadSheetCellStyleHandle): Integer;
var
  I: Integer;
begin
  for I := 0 to FStyles.Count - 1 do
...
procedure TcxExportProvider.SetupPictureContainer
  (AContainer: TdxSpreadSheetPictureContainer; const AArea: TRect;
  AFitMode: TcxImageFitMode; const ASize: TSize);
var
  R, ABounds: TRect;
begin
  ...
  begin
    ABounds := Rect(TableView.Columns.GetPosition(AArea.Left),
      TableView.Rows.GetPosition(AArea.Top),
      TableView.Columns.GetPosition(AArea.Right),
      TableView.Rows.GetPosition(AArea.Bottom));
    R := cxRectCenter(ABounds, ASize.cx, ASize.cy);
    AContainer.AnchorPoint1.Offset :=
      cxPointOffset(cxPointOffset(ABounds.TopLeft, R.TopLeft, False), 1, 1);
    AContainer.AnchorPoint2.Offset := cxPoint(ASize);
    AContainer.AnchorType := catOneCell;
  end
  else
  begin
    AContainer.AnchorPoint2.Cell := TableView.CreateCell(AArea.Bottom,
      AArea.Right);
...
procedure TcxExportToCSVProvider.Commit(AProgressHelper
  : TcxCustomProgressCalculationHelper; AHandler: TObject);
var
...
procedure TcxExportToHTMLProvider.Commit(AProgressHelper
  : TcxCustomProgressCalculationHelper; AHandler: TObject);
var
...
procedure TcxExportToTXTProvider.Commit(AProgressHelper
  : TcxCustomProgressCalculationHelper; AHandler: TObject);
var
...
begin
  case FSeparatorIndex of
    0:
      FFormatSettings.Separator := ASeparator;
    1:
      FFormatSettings.BeginString := ASeparator;
    2:
      FFormatSettings.EndString := ASeparator;
  end;
  Inc(FSeparatorIndex);
end;
...
constructor TdxCustomDataExport.Create(AControl: TObject;
...
begin
  ...
  try
    BeforeExport;
    try
      FProgressHelper := TcxProgressCalculationHelper.Create(GetStageCount + 1,
        Control, ProgressHandler);
      try
...
function TdxCustomDataExport.AddCellValue(ARow, AColumn: Integer;
  const AValue: Variant): TdxSpreadSheetCell;
begin
...
procedure TdxCustomDataExport.AddGroupByColumns(AStartIndex,
  AFinishIndex: Integer; AExpanded: Boolean = True);
begin
  AddGroupBy(View.Columns, AStartIndex, AFinishIndex, AExpanded);
end;

procedure TdxCustomDataExport.AddGroupByRows(AStartIndex, AFinishIndex: Integer;
  AExpanded: Boolean = True);
begin
  AddGroupBy(View.Rows, AStartIndex, AFinishIndex, AExpanded);
end;

function TdxCustomDataExport.PrepareDisplayFormat(ACell: TdxSpreadSheetCell;
...
begin
  ...
  begin
    ...
    Result := TdxSpreadSheetDisplayFormatConverter.ConvertDateTimeDisplayFormat
      (Result, ACell.AsDateTime)
  end
  else
    Result := TdxSpreadSheetDisplayFormatConverter.
      ConvertFloatValueDisplayFormat(Result);
end;
...
procedure TdxCustomDataExport.SetBorders(const AArea: TRect;
  AInnerStyle, AOuterStyle: TdxSpreadSheetCellBorderStyle);
var
  ...
  procedure CheckBorderStyle(ACondition: Boolean; ABorder: TcxBorder;
    AStyle: TdxSpreadSheetCellBorderStyle);
  begin
    if not ACondition or (AStyle = sscbsDefault) or (AStyles[ABorder] = AStyle)
    then
      Exit;
    AStyleChanged := True;
...
begin
  for ARow := AArea.Top to AArea.Bottom do
    ...
    begin
      ...
      AStyleHandle.Borders := SpreadSheet.CellStyles.Borders.AddBorders
        (ABordersHandle);
      ACell.StyleHandle := SpreadSheet.CellStyles.AddStyle(AStyleHandle);
    end;
end;

procedure TdxCustomDataExport.SetDisplayFormat(ACell: TdxSpreadSheetCell;
...
procedure TdxCustomDataExport.SetDisplayFormat(ARow, AColumn: Integer;
  const ADisplayFormat: string);
begin
...
procedure TdxCustomDataExport.ProgressHandler(ASender: TObject;
  AProgressValue: Integer);
var
...
class procedure TdxSpreadSheetCellStyleHelper.SetDataFormat
  (ACell: TdxSpreadSheetCell; const AFormatCode: string);
var
  AHandle: TdxSpreadSheetCellStyleHandle;
  AStyles: TdxSpreadSheetCellStyles;
begin
  if (AFormatCode <> '') and (ACell.StyleHandle.DataFormat.FormatCode <>
    AFormatCode) then
  begin

    AStyles := ACell.SpreadSheet.CellStyles;
    AHandle := ACell.StyleHandle.Clone;
    AHandle.DataFormat := AStyles.Formats.AddFormat(AFormatCode,
      AStyles.Formats.PredefinedFormats.GetIDByFormatCode(AFormatCode));
    ACell.StyleHandle := AStyles.AddStyle(AHandle);
  end;
end;

class procedure TdxSpreadSheetCellStyleHelper.SetDataFormat
  (ACell: TdxSpreadSheetCell; const AFormatCodeID: Integer);
begin
  SetDataFormat(ACell, ACell.SpreadSheet.CellStyles.Formats.PredefinedFormats.
    GetFormatHandleByID(AFormatCodeID).FormatCode);
end;
