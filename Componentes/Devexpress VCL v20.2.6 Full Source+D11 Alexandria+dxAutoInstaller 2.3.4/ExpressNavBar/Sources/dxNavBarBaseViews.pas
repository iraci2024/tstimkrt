...
uses
  ...
  dxNavBarCustomPainters, cxGeometry;
...
begin
  if not NavBar.IsDestroying then
    ...
    begin
      ...
      if (AControl <> nil) and (AControl.Parent = NavBar) and AControl.HandleAllocated
      then
...
procedure TdxNavBarBaseViewInfo.CheckControlWindowRegion
  (AGroup: TdxNavBarGroupViewInfo);
begin
  SetGroupControlWindowRegion(cxRectContent(ActiveGroupViewInfo.ItemsRect,
    GetGroupBorderOffsets), AGroup);
end;
...
begin
  if (GroupCount = 0) or (ActiveGroupViewInfo = nil) then
    Exit;
  ...
  if (ActiveGroupViewInfo.ItemsRect.Bottom + AOffsetY <
    ActiveGroupViewInfo.ItemsRect.Top + AMinHeight) then
    AOffsetY := ActiveGroupViewInfo.ItemsRect.Top + AMinHeight -
      ActiveGroupViewInfo.ItemsRect.Bottom;
...
begin
  if (ActiveGroupViewInfo <> nil) and
    (ActiveGroupViewInfo.Group.TopVisibleLinkIndex > 0) then
  begin
    AItemsRectHeight := ActiveGroupViewInfo.ItemsRect.Height + AOffsetY -
      GetGroupCaptionSeparatorWidth;
    ATopLinkIndex := Min(ActiveGroupViewInfo.Group.TopVisibleLinkIndex,
      ActiveGroupViewInfo.ItemCount - 1);
    for I := ATopLinkIndex - 1 downto 0 do
      if (ActiveGroupViewInfo.Items[ActiveGroupViewInfo.ItemCount - 1]
        .Rect.Bottom - ActiveGroupViewInfo.Items[I].Rect.Top) <= AItemsRectHeight
      then
        ATopLinkIndex := I
      else
        Break;
    TdxNavBarGroupAccess(ActiveGroupViewInfo.Group).CorrectTopVisibleIndex
      (ATopLinkIndex);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Result := 2 * GetScrollButtonVerticalSize + 3 *
        GetScrollButtonVerticalEdge;
      if AViewInfo.ItemCount > AViewInfo.Group.TopVisibleLinkIndex then
      begin
        ATopVisibleLinkViewInfo := AViewInfo.Items
          [ActiveGroupViewInfo.Group.TopVisibleLinkIndex];
        Result := Max(Result, cxRectHeight(ATopVisibleLinkViewInfo.Rect) +
          GetGroupCaptionSeparatorWidth);
      end;
    end;
  end
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ADisplacement := 0;
        if ATop or (ALinkViewInfo.Rect.Top < ActiveGroupViewInfo.ItemsRect.Top)
        then
          ADisplacement := ALinkViewInfo.Rect.Top -
            ActiveGroupViewInfo.ItemsRect.Top
        else if ALinkViewInfo.Rect.Bottom > ActiveGroupViewInfo.ItemsRect.Bottom
        then
          ADisplacement := ALinkViewInfo.Rect.Bottom -
            ActiveGroupViewInfo.ItemsRect.Bottom;
        TdxNavBarAccess(NavBar).ActiveGroupScrollBar.UpdatePosition
          (TdxNavBarAccess(NavBar).ActiveGroupScrollBar.Position +
          ADisplacement, TdxNavBarAccess(NavBar).IsScrollAnimationEnabled);
      end
      else if ATop or (ALinkViewInfo.Rect.Top <= AGroupViewInfo.ItemsRect.Top)
        or (cxRectHeight(ALinkViewInfo.Rect) >
        cxRectHeight(AGroupViewInfo.ItemsRect)) then
        ALink.Group.TopVisibleLinkIndex := AGroupViewInfo.IndexOfLinkViewInfo
          (ALinkViewInfo)
      else
        while (ALinkViewInfo.Rect.Bottom > AGroupViewInfo.ItemsRect.Bottom) do
        begin
          ALink.Group.TopVisibleLinkIndex :=
            ALink.Group.TopVisibleLinkIndex + 1;
          CalculateBounds;
        end;
...
begin
  ...
  begin
    ...
    begin
      ADisplacement := 0;
      if ATop or (AGroupViewInfo.CaptionRect.Top <
        ActiveGroupViewInfo.ItemsRect.Top) then
        ADisplacement := AGroupViewInfo.CaptionRect.Top -
          ActiveGroupViewInfo.ItemsRect.Top
      else if AGroupViewInfo.CaptionRect.Bottom > ActiveGroupViewInfo.ItemsRect.Bottom
      then
        ADisplacement := AGroupViewInfo.CaptionRect.Bottom -
          ActiveGroupViewInfo.ItemsRect.Bottom;
      TdxNavBarAccess(NavBar).ActiveGroupScrollBar.UpdatePosition
        (TdxNavBarAccess(NavBar).ActiveGroupScrollBar.Position + ADisplacement,
        TdxNavBarAccess(NavBar).IsScrollAnimationEnabled);
    end;
  end;
end;

function TdxNavBarBaseViewInfo.GetRootGroup(AGroup: TdxNavBarGroup)
  : TdxNavBarGroup;
begin
  Result := AGroup;
...
class function TdxNavBarBasePainter.ButtonPainterClass
  : TdxNavBarCustomButtonPainterClass;
begin
  Result := TdxNavBarBaseButtonPainter;
end;

class function TdxNavBarBasePainter.ScrollButtonsPainterClass
  : TdxNavBarCustomScrollButtonsPainterClass;
begin
  Result := TdxNavBarBaseScrollButtonsPainter;
end;

class function TdxNavBarBasePainter.SelectionPainterClass
...
class procedure TdxNavBarBaseSelectionPainter.GetColors
...
class procedure TdxNavBarBaseButtonPainter.InternalDrawButton(ACanvas: TCanvas;
...
begin
  ...
  if sPressed in AState then
    dxNavBarDrawSelectedFrame(ACanvas, ARect, DarkDarkBorderColor(AColor1),
      DarkDarkBorderColor(AColor1), DarkBorderColor(AColor1),
      DarkBorderColor(AColor1))
  else
    dxNavBarDrawSelectedFrame(ACanvas, ARect, LightLightBorderColor(AColor1),
      DarkDarkBorderColor(AColor1), LightBorderColor(AColor1),
      DarkBorderColor(AColor1));
  if sActive in AState then
    FillRectByColor(ACanvas.Handle, cxRectSetBottom(ARect, ARect.Bottom,
      1), clBlack);
end;

{ TdxNavBarBaseScrollButtonsPainter }

class procedure TdxNavBarBaseScrollButtonsPainter.InternalDrawBottomButton
...
class procedure TdxNavBarBaseScrollButtonsPainter.InternalDrawTopButton
  (ACanvas: TCanvas; ARect: TRect;
  AButtonPainterClass: TdxNavBarCustomButtonPainterClass;
  AColor1, AColor2: TColor; AAlphaBlend1, AAlphaBlend2: Byte;
  AGradientMode: TdxBarStyleGradientMode; ABorderColor: TColor;
...
class function TdxNavBarFlatPainter.ButtonPainterClass
  : TdxNavBarCustomButtonPainterClass;
begin
...
class procedure TdxNavBarFlatButtonPainter.InternalDrawButton(ACanvas: TCanvas;
  ARect: TRect; APicture: TPicture; AColor1, AColor2: TColor;
  AAlphaBlend1, AAlphaBlend2: Byte; AGradientMode: TdxBarStyleGradientMode;
  ABorderColor: TColor; AState: TdxNavBarObjectStates;
  AScaleFactor: TdxScaleFactor);
begin
  ...
  if sPressed in AState then
    dxNavBarDrawSelectedFrame(ACanvas, ARect, DarkDarkBorderColor(AColor1),
      LightLightBorderColor(AColor1), DarkBorderColor(AColor1), clNone)
  else if sHotTracked in AState then
    dxNavBarDrawSelectedFrame(ACanvas, ARect, LightLightBorderColor(AColor1),
      DarkDarkBorderColor(AColor1), clNone, DarkBorderColor(AColor1))
  else
    dxNavBarDrawSelectedFrame(ACanvas, ARect, LightLightBorderColor(AColor1),
      DarkBorderColor(AColor1), clNone, clNone);
  if sActive in AState then
    FillRectByColor(ACanvas.Handle, cxRectSetBottom(ARect, ARect.Bottom,
      1), clBlack);
end;
