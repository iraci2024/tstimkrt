...
uses
  ...
  dxCore, dxNavBar, dxNavBarBase, dxNavBarCollns, dxNavBarStyles,
...
type
  ...
  TdxNavBarSkinNavPaneLinkViewInfo = class
  ...
  TdxNavBarSkinNavPaneGroupViewInfo = class
    (TdxNavBarSkinBasedNavPaneGroupViewInfo);

  { TdxNavBarSkinOverflowPanelViewInfo }

  TdxNavBarSkinOverflowPanelViewInfo = class
  ...
  TdxNavBarSkinPopupControlViewInfo = class
  ...
  public
    ...
    property OverflowPanelViewInfo: TdxNavBarSkinOverflowPanelViewInfo
...
function TdxNavBarSkinPainterHelper.
...
function TdxNavBarSkinPainterHelper.GetSkinPainterData
...
procedure TdxNavBarSkinPainterHelper.LookAndFeelChanged(Sender: TcxLookAndFeel;
  AChangedValues: TcxLookAndFeelValues);
begin
  DoChanged;
end;
...
begin
  ...
  if not TdxCustomNavBarAccess(NavBar).IsInternal then
    Result := cxRectContent(Result,
      GetSkinElementOffsets(GetSkinHelper.NavPaneGroupClient, ScaleFactor));
end;
...
begin
  ...
  if not TdxCustomNavBarAccess(NavBar).IsInternal then
    Result.Left := Result.Left + GetSkinElementOffsets
      (GetSkinHelper.NavPaneGroupClient, ScaleFactor).Left;
end;

function TdxNavBarSkinNavPaneViewInfo.GetOverflowPanelViewInfoClass
...
class function TdxNavBarSkinNavPanePainter.GetViewInfoClass
  : TdxNavBarViewInfoClass;
begin
  Result := TdxNavBarSkinNavPaneViewInfo;
end;

class function TdxNavBarSkinNavPanePainter.GetGroupViewInfoClass
  : TdxNavBarGroupViewInfoClass;
begin
  Result := TdxNavBarSkinNavPaneGroupViewInfo;
end;

class function TdxNavBarSkinNavPanePainter.GetLinkViewInfoClass
  : TdxNavBarLinkViewInfoClass;
begin
  Result := TdxNavBarSkinNavPaneLinkViewInfo;
end;

class function TdxNavBarSkinNavPanePainter.GetSkinPainterHelperClass
  : TdxNavBarSkinBasedPainterHelperClass;
begin
  Result := TdxNavBarSkinPainterHelper;
end;

class function TdxNavBarSkinNavPanePainter.GetPopupControlViewInfoClass
...
procedure TdxNavBarSkinNavPanePainter.UpdateScrollBarLookAndFeel
...
function TdxNavBarSkinNavPanePainter.GetItemColorPalette(ALinkViewInfo
  : TdxNavBarLinkViewInfo): IdxColorPalette;
var
  AElement: TdxSkinElement;
begin
  AElement := FSkinBasedPainterHelper.NavPaneItem([sSelected, sPressed] *
    ALinkViewInfo.State <> []);
  if AElement <> nil then
...
function TdxNavBarSkinNavPanePainter.GetLookAndFeelPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := LookAndFeel.Painter;
end;

function TdxNavBarSkinNavPanePainter.GetOverflowPanelViewInfo
  : TdxNavBarSkinOverflowPanelViewInfo;
begin
...
procedure TdxNavBarSkinNavPanePainter.DrawBorder;
  ...
  begin
    DrawElementPart(FSkinBasedPainterHelper.NavPaneHeader, cxCanvas,
      cxRectSetBottom(ARect, ARect.Top + 1), cxRectSetHeight(ARect, 1));
  end;

  procedure DrawBottomBorder(const ARect: TRect);
  begin
    DrawElementPart(FSkinBasedPainterHelper.NavPaneSplitter, cxCanvas,
      cxRectSetTop(ARect, ARect.Bottom - 1), cxRectSetBottom(ARect,
      ARect.Bottom, 1));
  end;
...
begin
  ...
  else if not TdxCustomNavBarAccess(NavBar).IsInternal then
  begin
    if TdxNavBarSkinNavPaneViewInfo(ViewInfo).IsTopBorderNeeded then
      DrawTopBorder(ViewInfo.ClientRect);
    if TdxNavBarSkinNavPaneViewInfo(ViewInfo).IsBottomBorderNeeded and
      not NavBar.IsNavigationClient then
      DrawBottomBorder(ViewInfo.ClientRect);
  end
end;

procedure TdxNavBarSkinNavPanePainter.DrawGroupBackground(AGroupViewInfo
  : TdxNavBarGroupViewInfo);
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavPaneGroupClient, Canvas,
    AGroupViewInfo.Rect, ScaleFactor, 0, esNormal, TdxCustomNavBarAccess(NavBar)
    .IsInternal) then
    inherited DrawGroupBackground(AGroupViewInfo);
end;

procedure TdxNavBarSkinNavPanePainter.DrawGroupControl(ACanvas: TCanvas;
  ARect: TRect; AGroupViewInfo: TdxNavBarGroupViewInfo);
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavPaneGroupClient, ACanvas,
    ARect, ScaleFactor, 0, esNormal, True) then
    inherited DrawGroupControl(ACanvas, ARect, AGroupViewInfo);
end;

procedure TdxNavBarSkinNavPanePainter.DrawSeparator(ALinkViewInfo
...
begin
  ...
  begin
    ...
    DrawElementPart(FSkinBasedPainterHelper.NavPaneHeader, cxCanvas, ADrawRect,
      AClipRect);
  end
  else
    inherited;
...
begin
  Result := TdxNavBarSkinPainterHelper(TdxNavBarSkinExplorerBarPainter(Painter)
...
begin
  if IsEmbedded then
    Result := cxNullRect
  else if TdxNavBarSkinExplorerBarPainter(Painter).IsSkinAvailable then
...
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavBarBackground, Canvas,
    ViewInfo.ClientRect, ScaleFactor) then
    inherited;
end;
...
procedure TdxNavBarSkinExplorerBarPainter.DrawGroupBorder(AGroupViewInfo
  : TdxNavBarGroupViewInfo);
begin
...
procedure TdxNavBarSkinExplorerBarPainter.DrawGroupTopBorder(AGroupViewInfo
  : TdxNavBarGroupViewInfo);
var
  AElement: TdxSkinElement;
  R: TRect;
...
procedure TdxNavBarSkinExplorerBarPainter.DrawGroupControl(ACanvas: TCanvas;
  ARect: TRect; AGroupViewInfo: TdxNavBarGroupViewInfo);
begin
  if not DrawSkinElement(FSkinBasedPainterHelper.NavBarGroupClient, ACanvas,
    ARect, ScaleFactor, 0, esNormal, True) then
    inherited;
end;

procedure TdxNavBarSkinExplorerBarPainter.DrawGroupControlSplitter
  (AGroupViewInfo: TdxNavBarExplorerBarGroupViewInfo);
begin
  // do nothing
end;

procedure TdxNavBarSkinExplorerBarPainter.DrawSeparator(ALinkViewInfo
  : TdxNavBarLinkViewInfo);
var
  AClipRect, ADrawRect: TRect;
begin
...
class function TdxNavBarSkinExplorerBarPainter.GetViewInfoClass
  : TdxNavBarViewInfoClass;
begin
  Result := TdxNavBarSkinExplorerBarViewInfo;
end;

class function TdxNavBarSkinExplorerBarPainter.GetSkinPainterHelperClass
  : TdxNavBarSkinBasedPainterHelperClass;
begin
  Result := TdxNavBarSkinPainterHelper;
end;

class function TdxNavBarSkinExplorerBarPainter.SignPainterClass
...
procedure TdxNavBarSkinExplorerBarPainter.UpdateScrollBarLookAndFeel
  (ALookAndFeel: TcxLookAndFeel);
begin
  ALookAndFeel.AssignedValues := [];
  ALookAndFeel.MasterLookAndFeel := LookAndFeel;
...
procedure TdxNavBarSkinExplorerBarPainter.SetLookAndFeel
  (AValue: TcxLookAndFeel);
begin
  LookAndFeel.Assign(AValue);
end;

procedure TdxNavBarSkinExplorerBarPainter.SetSkinName
...
function TdxNavBarSkinExplorerBarPainter.GetItemColorPalette
  (ALinkViewInfo: TdxNavBarLinkViewInfo): IdxColorPalette;
begin
  if FSkinBasedPainterHelper.NavBarItem <> nil then
    Result := FSkinBasedPainterHelper.NavBarItem.GetGlyphColorPalette
      (NavBarObjectStateToSkinState(ALinkViewInfo.State))
  else
    Result := nil;
end;

function TdxNavBarSkinExplorerBarPainter.GetLookAndFeelPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := LookAndFeel.Painter;
end;
