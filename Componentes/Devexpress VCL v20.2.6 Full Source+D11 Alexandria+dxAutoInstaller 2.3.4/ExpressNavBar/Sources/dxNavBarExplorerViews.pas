...
{$ENDIF}
  ...
  dxCoreGraphics, dxNavBar, dxNavBarBase, dxNavBarCollns, dxNavBarStyles,
...
type
  ...
  public
    ...
    procedure MakeLinkVisible(ALink: TdxNavBarItemLink;
      ATop: Boolean = True); override;
    procedure MakeGroupVisible(AGroup: TdxNavBarGroup;
      AExpandGroup: Boolean = True; ATop: Boolean = True); override;

    // Hit tests
    function GetGroupViewInfoIndexAtSplitterPos(const pt: TPoint)
      : Integer; virtual;
    function GetGroupViewInfoAtSplitterPos(const pt: TPoint)
  ...
  protected
    ...
    class function GroupBorderPainterClass
      : TdxNavBarCustomGroupBorderPainterClass; override;
  ...
  public
    ...
    procedure DrawGroupControlSplitter(AGroupViewInfo
      : TdxNavBarExplorerBarGroupViewInfo); virtual;
  ...
  protected
    class procedure InternalDrawBorder(ACanvas: TCanvas; ARect: TRect;
      AColor: TColor; AWithCaption: Boolean); override;
  end;
  ...
  protected
    class function CloneBitmap(ASource: TBitmap;
      const ATargetSize: TSize): TBitmap;
  ...
  TdxNavBarAdvExplorerBarGroupViewInfo = class
  ...
  protected
    ...
    class function PrepareBitmap(ACanvas: TCanvas; ARect: TRect;
      ABitmap: TBitmap; AForeColor: TColor;
...
uses
  Types, CommCtrl, ImgList, Math, cxGraphics, dxOffice11, dxNavBarViewsFact,
...
begin
  ...
  else if AState * [sHotTracked, sPressed, sSelected] <> [] then
    Result := AHotColor
  else
    Result := ANormalColor;
end;
...
begin
  ...
  begin
    ...
    AShortage := ASpace - TdxNavBarExplorerBarViewInfo(ViewInfo)
    ...
    if Align = vaTop then
      FSplitterRect := cxRect(FRect.Left, FRect.Bottom, FRect.Right,
        FRect.Bottom + ASpace)
    else
      FSplitterRect := cxRect(FRect.Left, FRect.Top - ASpace, FRect.Right,
        FRect.Top);
  end
  else
...
procedure TdxNavBarExplorerBarGroupViewInfo.AdjustBoundsByAlignment
  (ARestSpaceValue: Integer);
begin
...
begin
  ...
  begin
    ARestPlace := ClientHeight - Groups[GroupCount - 1].Rect.Bottom -
      GetGroupEdges.Y;
    if ARestPlace > 0 then
      for I := 0 to GroupCount - 1 do
        TdxNavBarExplorerBarGroupViewInfo(Groups[I]).AdjustBoundsByAlignment
          (ARestPlace);
  end;
end;
...
procedure TdxNavBarExplorerBarViewInfo.DoCreateGroupsInfo;
  ...
  procedure InternalDoCreateGroupInfo(AIndex: Integer; AAlign: TcxTopBottom;
    AUseRestSpace: Boolean = False);
  var
  ...
  begin
    ...
    TdxNavBarExplorerBarGroupViewInfo(Groups[GroupCount - 1]).FUseRestSpace :=
      AUseRestSpace;
  end;

  procedure InternalDoCreateGroupInfos(AGroupAlign: TcxTopBottom;
  ...
  begin
    ...
    for I := 0 to NavBar.RootGroupCount - 1 do
      if NavBar.RootGroups[I].Visible and
        (NavBar.RootGroups[I].Align = AGroupAlign) then
...
begin
  ...
  for I := 0 to NavBar.RootGroupCount - 1 do
    if NavBar.RootGroups[I].Visible and NavBar.RootGroups[I].UseRestSpace and
      NavBar.RootGroups[I].Expanded then
    begin
...
function TdxNavBarExplorerBarViewInfo.GetGroupItemsVisible
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        R := cxRectBounds(ALeft, GroupsArea.Top, NavBar.ScrollBar.Width,
          AClientHeight);
        CalculateScrollBarBoundsBySizeGrip(R);
        NavBar.ScrollBar.Bounds := R;
      end;
      NavBar.ScrollBar.SetScrollParams(AMin, AMax, NavBar.ScrollBar.Position,
        APageSize);
...
begin
  Result := Groups[GroupCount - 1].Rect.Bottom + GetGroupEdges.Y -
    HeaderRect.Bottom;
end;
...
begin
  with GetAbsoluteLinksImageEdges do
    Result := Rect(ScaleFactor.Apply(Left), Top,
      ScaleFactor.Apply(Right), Bottom);
end;
...
function TdxNavBarExplorerBarViewInfo.CanHasVisibleItemsInGroup
  (AGroup: TdxNavBarGroup): Boolean;
begin
...
procedure TdxNavBarExplorerBarViewInfo.DoGroupDeactivate
  (AGroup: TdxNavBarGroup);
begin
...
function TdxNavBarExplorerBarViewInfo.IsGroupActive
  (AGroup: TdxNavBarGroup): Boolean;
begin
  Result := AGroup.Expanded;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if ALinkViewInfo.Rect.Bottom > GroupsArea.Bottom then
        ADisplacement := ALinkViewInfo.Rect.Bottom - GroupsArea.Bottom;
      NavBar.ScrollBar.UpdatePosition(NavBar.ScrollBar.Position + ADisplacement,
        TdxNavBarAccess(NavBar).IsScrollAnimationEnabled);
    end;
  end;
end;
...
function TdxNavBarExplorerBarViewInfo.GetGroupViewInfoIndexAtSplitterPos
  (const pt: TPoint): Integer;
var
  I: Integer;
begin
  Result := -1;
  for I := 0 to GroupCount - 1 do
    if ptInRect(TdxNavBarExplorerBarGroupViewInfo(Groups[I]).FSplitterRect, pt)
    then
    begin
      Result := I;
      Break;
...
function TdxNavBarExplorerBarViewInfo.GetGroupViewInfoAtSplitterPos
  (const pt: TPoint): TdxNavBarGroupViewInfo;
var
  AIndex: Integer;
begin
...
begin
  ...
  begin
    ...
    if AGroupViewInfo <> nil then
      if (cxRectHeight(ClientRect) - cxRectHeight(HeaderRect) <
        cxRectHeight(AGroupViewInfo.Rect)) or
        (AGroupViewInfo.Rect.Top - GroupsArea.Top - NavBar.ScrollBar.Position <
        GetSpaceBetweenGroups div 2) then
      begin
        ADisplacement := AGroupViewInfo.Rect.Top - GroupsArea.Top -
          GetSpaceBetweenGroups div 2;
        NavBar.ScrollBar.Position := ADisplacement;
      end
      else
      begin
        AMakeVisibleBottomEdge := AGroupViewInfo.Rect.Bottom - GroupsArea.Top +
          GetSpaceBetweenGroups div 2;
        if AMakeVisibleBottomEdge - NavBar.ScrollBar.Position > ClientRect.Bottom
        then
        begin
...
function TdxNavBarExplorerBarController.GetPartAtPos(const APoint: TPoint)
  : TdxNavBarPart;
begin
  Result.MinorPartIndex := TdxNavBarExplorerBarViewInfo(ViewInfo)
    .GetGroupViewInfoIndexAtSplitterPos(APoint);
...
procedure TdxNavBarExplorerBarController.DoMouseDown(AButton: TMouseButton;
...
procedure TdxNavBarExplorerBarController.DoMouseMove(AShift: TShiftState;
  const APoint: TPoint);
begin
...
procedure TdxNavBarExplorerBarController.DoMouseUp(AButton: TMouseButton;
  AShift: TShiftState; const APoint: TPoint);
begin
  HotPart := GetPartAtPos(APoint);
  case PressedPart.MajorPartIndex of
    ebSplitter:
      DoSplitterEndDrag(APoint);
  else
    inherited;
  end;
...
begin
  if GetPartAtPos(NavBar.ScreenToClient(GetMouseCursorPos)).MajorPartIndex = ebSplitter
  then
...
procedure TdxNavBarExplorerBarController.DoSplitterStartDrag
  (const APoint: TPoint);
begin
...
begin
  ...
  begin
    AdY := -AdY;
    AdPosition := GetSizedGroup.Control.OriginalHeight -
      (FStartDragHeight + AdY);
...
procedure TdxNavBarExplorerBarController.DoSplitterEndDrag
  (const APoint: TPoint);
begin
  DoSplitterCancelDrag;
end;
...
procedure TdxNavBarExplorerBarPainter.DrawGroup(AGroupViewInfo
  : TdxNavBarGroupViewInfo);
begin
  inherited;
  if not IsRectEmpty(TdxNavBarExplorerBarGroupViewInfo(AGroupViewInfo)
    .FSplitterRect) then
    DrawGroupControlSplitter(TdxNavBarExplorerBarGroupViewInfo(AGroupViewInfo));
end;

procedure TdxNavBarExplorerBarPainter.DrawGroupControlSplitter
  (AGroupViewInfo: TdxNavBarExplorerBarGroupViewInfo);
begin
  // do nothing
end;

class function TdxNavBarExplorerBarPainter.GetCategories
  : TdxNavBarViewCategories;
begin
  Result := [nbvcExplorerBar];
end;

class function TdxNavBarExplorerBarPainter.GetViewInfoClass
  : TdxNavBarViewInfoClass;
begin
  Result := TdxNavBarExplorerBarViewInfo;
end;

class function TdxNavBarExplorerBarPainter.GetGroupViewInfoClass
  : TdxNavBarGroupViewInfoClass;
begin
  Result := TdxNavBarExplorerBarGroupViewInfo;
end;

class function TdxNavBarExplorerBarPainter.GetLinkViewInfoClass
  : TdxNavBarLinkViewInfoClass;
begin
  Result := TdxNavBarExplorerBarLinkViewInfo;
end;

function TdxNavBarExplorerBarPainter.GetControllerClass
  : TdxNavBarControllerClass;
begin
  Result := TdxNavBarExplorerBarController;
end;

class function TdxNavBarExplorerBarPainter.GroupBorderPainterClass
  : TdxNavBarCustomGroupBorderPainterClass;
begin
  Result := TdxNavBarExplorerBarBorderPainter;
end;

class function TdxNavBarExplorerBarPainter.SignPainterClass
...
class function TdxNavBarUltraFlatExplorerBarPainter.SelectionPainterClass
...
function TdxNavBarAdvExplorerBarGroupViewInfo.BgGradientMode
...
begin
  ...
  else if sSpecial in State then
...
function TdxNavBarAdvExplorerBarGroupViewInfo.CaptionGradientMode
  : TdxBarStyleGradientMode;
begin
  if IsDefaultCaptionColor then
    Result := gmHorizontal
...
function TdxNavBarAdvExplorerBarViewInfo.BgGradientMode
  : TdxBarStyleGradientMode;
begin
  if IsDefaultBgColor then
...
begin
  NavBar.DefaultStyles.ItemDisabled.Assign(NavBar.DefaultStyles.Item);
  NavBar.DefaultStyles.ItemDisabled.Font.Color :=
    LightLightColor(dxAdvExplorerBarFontColor);
end;
...
class function TdxNavBarAdvExplorerBarPainter.GetViewInfoClass
  : TdxNavBarViewInfoClass;
begin
  Result := TdxNavBarAdvExplorerBarViewInfo;
end;

class function TdxNavBarAdvExplorerBarPainter.GetGroupViewInfoClass
  : TdxNavBarGroupViewInfoClass;
begin
  Result := TdxNavBarAdvExplorerBarGroupViewInfo;
end;

class function TdxNavBarAdvExplorerBarPainter.GetLinkViewInfoClass
  : TdxNavBarLinkViewInfoClass;
begin
  Result := TdxNavBarAdvExplorerBarLinkViewInfo;
end;

class function TdxNavBarAdvExplorerBarPainter.ButtonPainterClass
  : TdxNavBarCustomButtonPainterClass;
begin
  Result := TdxNavBarAdvExplorerButtonPainter;
end;

class function TdxNavBarAdvExplorerBarPainter.SignPainterClass
  : TdxNavBarCustomSignPainterClass;
begin
...
class procedure TdxNavBarExplorerBarBorderPainter.InternalDrawBorder
...
class procedure TdxNavBarAdvExplorerButtonPainter.InternalDrawButton
...
begin
  ...
  try
    ExcludeClipRect(ACanvas.Handle, ARect.Left, ARect.Top, ARect.Left + 2,
      ARect.Top + 1);
    ExcludeClipRect(ACanvas.Handle, ARect.Left, ARect.Top, ARect.Left + 1,
      ARect.Top + 2);
    ExcludeClipRect(ACanvas.Handle, ARect.Right - 2, ARect.Top, ARect.Right,
      ARect.Top + 1);
    ExcludeClipRect(ACanvas.Handle, ARect.Right - 1, ARect.Top, ARect.Right,
      ARect.Top + 2);
    inherited;
  finally
...
class function TdxNavBarExplorerBarSignPainter.CloneBitmap(ASource: TBitmap;
  const ATargetSize: TSize): TBitmap;
begin
...
class procedure TdxNavBarExplorerBarSignPainter.DrawSignSelection
...
class procedure TdxNavBarExplorerBarSignPainter.InternalDrawSign
  ...
  procedure BoldPolyline(const APoint1, APoint2, APoint3: TPoint;
    ADirection: Integer);
  begin
    ACanvas.Polyline([APoint2, APoint1, APoint3]);
    ACanvas.Polyline([cxPointOffset(APoint2, 1, 0), cxPointOffset(APoint1, 0,
      ADirection), cxPointOffset(APoint3, -1, 0)])
  end;
...
var
  ...
  ASignSize: Integer;
begin
  DrawSignSelection(ACanvas, ARect, AForeColor, ABackColor1,
    ABackColor2, AState);
  ...
  APoint2 := Point(APoint1.X - (2 * ASignSize - 1) div 2,
    APoint1.Y + ADirection * (ASignSize - 1));
...
class function TdxNavBarAdvExplorerBarSignPainter.PrepareBitmap
  (ACanvas: TCanvas; ARect: TRect; ABitmap: TBitmap; AForeColor: TColor;
  AState: TdxNavBarObjectStates): TBitmap;
var
  ABgColor: TColor;
  AMarksColor: TColor;
    ...
    begin
      ...
      if APixelColor <> ATransparentColor then
        if APixelColor = AMarksColor then
          AResultColors[J * Result.Width + I] :=
            dxColorToRGBQuad(ColorToRGB(AForeColor))
        else
        begin
          ABgColor := ACanvas.Pixels[ARect.Left + I, ARect.Top + J];
          APixelColor := dxGetMiddleRGB(ABgColor, APixelColor,
...
RegisterView(dxNavBarExplorerBarView, 'ExplorerBarView',
  TdxNavBarExplorerBarPainter);
RegisterView(dxNavBarUltraFlatExplorerView, 'UltraFlatExplorerBarView',
  TdxNavBarUltraFlatExplorerBarPainter);
RegisterView(dxNavBarAdvExplorerBarView, 'AdvExplorerBarView',
  TdxNavBarAdvExplorerBarPainter);
