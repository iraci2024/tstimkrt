...
uses
  ...
  cxCurrencyEdit,
  cxDropDownEdit, cxHyperLinkEdit, cxImage, cxImageComboBox, cxListBox,
  cxMaskEdit,
...
type
  ...
  TcxCustomDBEditDefaultValuesProvider = class
  ...
  protected
    ...
    property DefaultValuesProvider: TcxCustomDBEditDefaultValuesProvider
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TcxCurrencyEditProperties
...
begin
  if TcxCustomEditAccess(AEdit).DataBinding is TcxDBEditDataBinding then
    Result := TcxDBEditDataBinding(TcxCustomEditAccess(AEdit)
      .DataBinding).DataLink
  else
    Result := nil;
end;
...
begin
  Result := (DataSource <> nil) and
    (DataSource.AutoEdit or (DataSource.State in [dsEdit, dsInsert]));
end;
...
begin
  ...
  Result := Result and
    (Field.CanModify or (Field.Lookup and CanModifyLookupField(Field)));
end;
...
begin
  if IsDefaultDataAvailable and (Field.DataType in [ftString, ftWideString])
  then
    Result := Field.Size
  else
...
function TcxCustomDBEditDefaultValuesProvider.IsDisplayFormatDefined
...
procedure TcxCustomDBEditDefaultValuesProvider.FieldFreeNotification
  (Sender: TComponent);
begin
...
procedure TcxCustomDBEditDefaultValuesProvider.SetDataSource
  (Value: TDataSource);
begin
...
begin
  ...
  begin
    ...
    begin
      AIsRightAligned := (GetWindowLong(AVisualControl.Handle, GWL_EXSTYLE) and
        WS_EX_RIGHT) = WS_EX_RIGHT;
      AUseRightToLeftAlignment := DBUseRightToLeftAlignment
        (AVisualControl, Field);
...
begin
  Result := not ReadOnly and (DataField <> nil) and
    (DataField.CanModify or (DataField.Lookup and
    CanModifyLookupField(DataField)));
end;

procedure TcxEditFieldDataLink.InternalSetField(ADataField,
  APropertiesField: TField);
begin
...
procedure TcxEditFieldDataLink.UpdateField;
  ...
  begin
    if (Active or not AConsiderActive) and (DataSource <> nil) and
      (DataSource.DataSet <> nil) and
      not(csDestroying in DataSource.DataSet.ComponentState) and
      (FFieldName <> '') then
...
begin
  if DataField = nil then
    FMasterField := nil
  else if not FDataBinding.IsLookupControl or (DataField.FieldKind <> fkLookup)
    or (Pos(';', DataField.KeyFields) > 0) then
...
var
  ...
  AUseRightToLeftAlignment: Boolean;
begin
  if Assigned(FControl) and (FControl is TWinControl) and TWinControl(FControl).IsRightToLeft
  then
  begin
    AIsRightAligned := (GetWindowLong(TWinControl(FControl).Handle, GWL_EXSTYLE)
      and WS_EX_RIGHT) = WS_EX_RIGHT;
    AUseRightToLeftAlignment := DBUseRightToLeftAlignment(TControl(FControl),
      PropertiesField);
    if (AIsRightAligned and (not AUseRightToLeftAlignment)) or
      ((not AIsRightAligned) and AUseRightToLeftAlignment) then
...
class function TcxDBEditDataBinding.GetDefaultValuesProviderClass
...
procedure TcxDBEditDataBinding.DataEvent(AEvent: TDataEvent;
  AInfo: TdxNativeInt);
begin
  ...
  case AEvent of
    ...
    deRecordChange:
      if (FReceivedEvents = [deCheckBrowseMode, deUpdateState, deRecordChange])
      then
      begin
...
begin
  Result := inherited GetIsDataAvailable and cxDB.IsDataAvailable(Field) and
    cxDB.IsDataAvailable(PropertiesField);
end;
...
begin
  AEditValueSource := Edit.ActiveProperties.GetEditValueSource
    (Edit.InternalFocused);
  if not IsDataAvailable or
    (IsNull and ((AEditValueSource <> evsText) or not Assigned(Field.OnGetText)))
  then
  ...
  begin
    case AEditValueSource of
      ...
      evsText:
        ...
        else if not Edit.ActiveProperties.IsValueFormattedByProperties then
...
begin
  if Field is TAggregateField then
    Result := VarIsNull(TcxDataSetAccess(DataLink.DataSet)
      .GetAggregateValue(Field))
  else
    Result := Field.IsNull;
end;
...
{$IFNDEF DELPHIXE2}
      if (Field is TFMTBCDField) and VarIsType(AValue, varDouble) then
        AField.Value := FloatToStrF(AValue, ffFixed, 15, 18)
        // bug in VarToBcd when the type AValue is double: VarToStr(0.000012) returns 1.2E-05 which is incorrect for StrToBcd
      else
{$ENDIF}
        AField.Value := AValue;
  end;
...
begin
  ...
  begin
    if not TcxCustomEditAccess(Edit).Focused and Edit.ActiveProperties.IsValueFormattedByProvider
    then
    begin
...
function TcxDBEditDataBinding.GetDefaultValuesProvider
  : TcxCustomDBEditDefaultValuesProvider;
begin
  Result := TcxCustomDBEditDefaultValuesProvider
...
class function TcxDBCurrencyEdit.GetPropertiesClass
...
class function TcxDBHyperLinkEdit.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
class function TcxDBImageComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
