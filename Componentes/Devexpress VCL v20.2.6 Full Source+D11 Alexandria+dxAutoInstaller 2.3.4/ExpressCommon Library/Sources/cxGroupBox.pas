...
{$ENDIF}
  ...
  dxCore, dxMessages, cxControls, cxGraphics, cxClasses, cxLookAndFeels,
  dxFluentDesignFormInterfaces, cxContainer,
...
type
  TcxCaptionAlignment = (alTopLeft, alTopCenter, alTopRight, alBottomLeft,
    alBottomCenter, alBottomRight, alLeftTop, alLeftCenter, alLeftBottom,
    alRightTop, alRightCenter, alRightBottom, alCenterCenter);

  TcxPanelOffice11BackgroundKind = (pobkGradient, pobkOffice11Color,
    pobkStyleColor);
  ...
  public
    ...
    TextColor: TColor;
    Column, Row: Integer;
    function GetGlyphRect(const AGlyphSize: TSize; AAlignment: TLeftRight;
      AIsPaintCopy: Boolean): TRect; virtual;
  end;
  ...
  private
    ...
    procedure DrawNativeBackground(ACanvas: TcxCanvas;
      const ACaptionRect: TRect);
    procedure DrawNativeGroupBoxBackground(ACanvas: TcxCanvas);
    procedure DrawNativePanelBackground(ACanvas: TcxCanvas;
      const ACaptionRect: TRect);
  ...
  protected
    ...
    procedure CalculateRectsForStandardPainter(ACanvas: TcxCanvas;
      AShadowWidth: Integer);
    procedure CalculateRectsForSkinPainter(ACanvas: TcxCanvas;
      AHasNonClientArea: Boolean);
    ...
    property CaptionPosition: TcxGroupBoxCaptionPosition
  ...
  public
    ...
    property CheckBoxCheckState: TcxCheckBoxState read FCheckBoxCheckState
      write FCheckBoxCheckState;
    property Edit: TcxCustomGroupBox read GetEdit;
    property MouseFocusingRect: TRect read FMouseFocusingRect
      write FMouseFocusingRect;
  end;
  ...
  private
    ...
    procedure CalculateRects(ACanvas: TcxCanvas;
  ...
  protected
    ...
    function IsButtonGlyphTransparent(AButtonViewInfo
      : TcxGroupBoxButtonViewInfo): Boolean; virtual; abstract;
  public
    CaptionExtent: TRect;
    GlyphSize: TSize;
  end;
  ...
  protected
    ...
    procedure CalculateButtonViewInfos(AViewInfo
    ...
    procedure GetEditMetrics(AAutoHeight: Boolean; ACanvas: TcxCanvas;
      out AMetrics: TcxEditMetrics); virtual;
  ...
  public
    ...
    function GetEditConstantPartSize(ACanvas: TcxCanvas;
      ...
      : TSize; override;
    class function IsButtonNativeStyle(ALookAndFeel: TcxLookAndFeel)
      : Boolean; virtual;
  ...
  protected
    procedure DoChanged(ACollection: TCollection;
  ...
  public
    ...
    property Items[Index: Integer]: TcxButtonGroupItem read GetItem
  ...
  published
    ...
    property CaptionIndent: Integer read FCaptionIndent write SetCaptionIndent
      default 2;
    property OfficeBackgroundKind: TcxPanelOffice11BackgroundKind
      read FOfficeBackgroundKind write SetOfficeBackgroundKind
      default pobkOffice11Color;
    property WordWrap: Boolean read FWordWrap write SetWordWrap default False;
  end;

  { TcxCustomGroupBox }

  TcxGroupBoxCustomDrawEvent = procedure(Sender: TcxCustomGroupBox;
  ...
  TcxGroupBoxCustomDrawElementEvent = procedure(Sender: TcxCustomGroupBox;
    ...
    const APainter: TcxCustomLookAndFeelPainter; var ADone: Boolean) of object;
  ...
  private
    ...
    procedure CMUpdateNonClientArea(var Message: TMessage);
      message DXM_UPDATENONCLIENTAREA;
  ...
  protected
    ...
    function DoCustomDrawCaption(ACanvas: TcxCanvas; const ABounds: TRect;
      APainter: TcxCustomLookAndFeelPainter): Boolean; virtual;
    function DoCustomDrawContentBackground(ACanvas: TcxCanvas;
      const ABounds: TRect; APainter: TcxCustomLookAndFeelPainter)
      : Boolean; virtual;
    procedure DoMeasureCaptionHeight(APainter: TcxCustomLookAndFeelPainter;
    ...
    property CheckBoxCheckState: TcxCheckBoxState read GetCheckBoxCheckState
      write SetCheckBoxCheckState;
    property CheckBoxViewState: TcxEditCheckState read GetCheckBoxViewState
      write SetCheckBoxViewState;
    property Ctl3D;
    ...
    property OnCustomDraw: TcxGroupBoxCustomDrawEvent read FOnCustomDraw
      write FOnCustomDraw;
    property OnCustomDrawCaption: TcxGroupBoxCustomDrawElementEvent
      read FOnCustomDrawCaption write FOnCustomDrawCaption;
    property OnCustomDrawContentBackground: TcxGroupBoxCustomDrawElementEvent
      read FOnCustomDrawContentBackground write FOnCustomDrawContentBackground;
    property OnMeasureCaptionHeight: TcxGroupBoxMeasureCaptionHeightEvent
      read FOnMeasureCaptionHeight write FOnMeasureCaptionHeight;
  public
  ...
  published
    property CaptionBkColor: TColor read GetCaptionBkColor
      write SetCaptionBkColor stored False; // deprecated hidden
  ...
  private
    ...
    procedure DoButtonKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure DoButtonMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure DoButtonMouseWheel(Sender: TObject; Shift: TShiftState;
...
uses
  ...
  dxUxTheme, cxDrawTextUtils, cxGeometry, dxCustomFluentDesignForm;
...
function cxGroupBoxAlignmentToGroupBoxCaptionPosition
  (AAlignment: TcxCaptionAlignment): TcxGroupBoxCaptionPosition;
begin
  case AAlignment of
...
function TcxGroupBoxButtonViewInfo.GetGlyphRect(const AGlyphSize: TSize;
  AAlignment: TLeftRight; AIsPaintCopy: Boolean): TRect;
begin
...
function TcxGroupBoxViewInfo.CalculateBorderColor(AStyle: TcxContainerStyle;
...
begin
  ...
  begin
    if AEnabled then
      Result := GetEditBorderHighlightColor(TcxCustomEditStyle(AStyle)
...
procedure TcxGroupBoxViewInfo.CalculateRectsForPanel(ACanvas: TcxCanvas);
  ...
  begin
    ABorderWidth := GetContainerBorderWidth
  ...
  begin
    ...
    case Edit.Alignment of
      alTopCenter, alBottomCenter, alCenterCenter:
        OffsetRect(TextRect, (R.Right - R.Left - ATextWidth - TextRect.Left)
          div 2, 0);
      alTopRight, alRightTop, alRightCenter, alRightBottom, alBottomRight:
    ...
    case Edit.Alignment of
      alLeftCenter, alRightCenter, alCenterCenter:
        OffsetRect(TextRect, 0,
          (R.Bottom - R.Top - ATextHeight - TextRect.Top) div 2);
      alLeftBottom, alBottomLeft, alBottomCenter, alBottomRight, alRightBottom:
...
procedure TcxGroupBoxViewInfo.CalculateRectsForStandardPainter
  (ACanvas: TcxCanvas; AShadowWidth: Integer);

  procedure AdjustHorizontalCaptionRect(var R: TRect);
  begin
    case Edit.Alignment of
      alTopCenter, alBottomCenter, alCenterCenter:
        OffsetRect(R, -R.Left + (Edit.Width - AShadowWidth - (R.Right - R.Left))
          div 2, 0);
      alTopRight, alBottomRight:
        OffsetRect(R, Edit.Width - R.Right - R.Left - AShadowWidth -
          GetCaptionRectLeftBound, 0);
    ...
    case Edit.Alignment of
      ...
      alBottomLeft, alBottomCenter, alBottomRight:
        R := Rect(R.Left, Edit.Height - R.Top - (R.Bottom - R.Top), R.Right,
          Edit.Height - R.Top);
    end
  end;
  ...
  begin
    ...
    case Edit.Alignment of
      ...
      alLeftCenter:
        begin
          R := Rect(R.Top, 0, R.Bottom, Edit.Height - AShadowWidth -
            (Edit.Height - AShadowWidth - ATextWidth) div 2);
          R.Top := R.Bottom - ATextWidth - 1;
        end;
      alLeftBottom:
        begin
          R := Rect(R.Top, 0, R.Bottom, Edit.Height - AShadowWidth - R.Left);
          R.Top := R.Bottom - ATextWidth - 1;
        end;
      alRightTop:
        R := Rect(Edit.Width - R.Bottom, R.Left, Edit.Width - R.Top,
          R.Left + ATextWidth);
      alRightCenter:
        begin
          R := Rect(Edit.Width - R.Bottom, (Edit.Height - ATextWidth) div 2,
            Edit.Width - R.Top, 0);
          R.Bottom := R.Top + ATextWidth;
        end;
      alRightBottom:
        R := Rect(Edit.Width - R.Bottom, Edit.Height - R.Left - ATextWidth,
          Edit.Width - R.Top, Edit.Height - R.Left);
    end;
    case Edit.Alignment of
      alLeftTop, alRightTop:
        OffsetRect(R, 0, GetCaptionRectLeftBound);
      alLeftBottom, alRightBottom:
        OffsetRect(R, 0, -GetCaptionRectLeftBound);
    end;
  end;
...
procedure TcxGroupBoxViewInfo.CalculateRectsForSkinPainter(ACanvas: TcxCanvas;
  AHasNonClientArea: Boolean);
  ...
  procedure AdjustCaptionRect(var ACaptionRect: TRect;
    const AIndent, ACombinedRect: TRect; out ANeedCorrection: Boolean);
  var
  ...
  begin
    ...
    case CaptionPosition of
      ...
      cxgpCenter:
        ACaptionRect := cxRectCenter(ACaptionRect,
          cxSize(cxRectWidth(ACombinedRect), AHeight));
      cxgpLeft:
        begin
  ...
  procedure AdjustCombinedCaptionRect(var ACombinedRect: TRect;
    const ACaptionRect, ABorderSize: TRect);
  var
  ...
  begin
    ...
    case Edit.Alignment of
      ...
      alTopRight, alBottomRight:
        ACombinedRect.Left := ACombinedRect.Right - ACombinedRectWidth;
      alTopCenter, alBottomCenter:
        begin
          ACombinedRect.Left := (ACombinedRect.Left + ACombinedRect.Right -
            ACombinedRectWidth) div 2;
          ACombinedRect.Right := ACombinedRect.Left + ACombinedRectWidth;
        end;
      alLeftTop, alRightTop:
        ACombinedRect.Bottom := ACombinedRect.Top + ACombinedRectWidth;
      alLeftBottom, alRightBottom:
        ACombinedRect.Top := ACombinedRect.Bottom - ACombinedRectWidth;
      alLeftCenter, alRightCenter:
        begin
          ACombinedRect.Top := (ACombinedRect.Bottom + ACombinedRect.Top -
            ACombinedRectWidth) div 2;
          ACombinedRect.Bottom := ACombinedRect.Top + ACombinedRectWidth;
        end;
    end;
  ...
  begin
    case Edit.Alignment of
      alTopLeft, alTopRight, alTopCenter, alBottomLeft, alBottomRight,
        alBottomCenter, alCenterCenter:
        OffsetRect(FCheckBoxRect, 0, CaptionRect.Top +
          (cxRectHeight(CaptionRect) - cxRectHeight(FCheckBoxRect)) div 2 -
          FCheckBoxRect.Top);
    else
      OffsetRect(FCheckBoxRect, CaptionRect.Left + (cxRectWidth(CaptionRect) -
        cxRectWidth(FCheckBoxRect)) div 2 - FCheckBoxRect.Left, 0);
    end;
  end;
...
begin
  ...
  AdjustCaptionRect(CaptionRect, ACaptionRectIndent, ACombinedRect,
    ANeedCorrection);
  AdjustCombinedCaptionRect(ACombinedRect, CaptionRect,
    Painter.GroupBoxBorderSize(True, CaptionPosition));
  CalculateCheckBoxAndTextRects(ACombinedRect);
...
procedure TcxGroupBoxViewInfo.DrawCaption(ACanvas: TcxCanvas);
  ...
  begin
    ...
    Painter.DrawGroupBoxCaption(ACanvas, ACaptionRect, GetCaptionContentRect,
      ACaptionPosition);
  end;
...
begin
  ...
  try
    ...
    if (Edit.Caption = '') or Edit.DoCustomDrawCaption(ACanvas, CaptionRect,
      Painter) then
      Exit;
...
function TcxGroupBoxViewInfo.GetBiDiModeCaptionPosition
  : TcxGroupBoxCaptionPosition;
begin
  Result := cxGroupBoxAlignmentToGroupBoxCaptionPosition
    (Edit.GetBiDiModeDependentAlignment);
end;
...
begin
  ...
  begin
    ...
    else if IsCustomBackground then
  ...
  if Edit.UseRightToLeftAlignment then
    AFrameBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (AFrameBounds, Bounds);
...
procedure TcxGroupBoxViewInfo.CalculateCheckBoxAndTextRects
  (ACombinedRect: TRect);
begin
  ...
  if CheckBoxVisible then
  begin
    if Edit.Alignment in [alTopLeft, alTopCenter, alTopRight, alCenterCenter,
      alBottomLeft, alBottomCenter, alBottomRight] then
    begin
      FCheckBoxRect := cxRectSetLeft(FCheckBoxRect, ACombinedRect.Left);
      FCheckBoxRect := cxRectSetTop(FCheckBoxRect,
        ACombinedRect.Top + (cxRectHeight(ACombinedRect) -
        cxRectHeight(FCheckBoxRect)) div 2);
      TextRect.Left := FCheckBoxRect.Right + GetCheckBoxCaptionIndent;
      TextRect := cxRectSetTop(TextRect, ACombinedRect.Top);
    end
    else
    begin
      FCheckBoxRect := cxRectSetLeft(FCheckBoxRect,
        ACombinedRect.Left + (cxRectWidth(ACombinedRect) -
        cxRectWidth(FCheckBoxRect)) div 2);
      ...
      begin
        ...
        TextRect := cxRectSetTop(TextRect, FCheckBoxRect.Bottom +
          GetCheckBoxCaptionIndent);
      end
      else
      begin
        FCheckBoxRect := cxRectSetBottom(FCheckBoxRect, ACombinedRect.Bottom);
        TextRect := cxRectSetBottom(TextRect, FCheckBoxRect.Top -
          GetCheckBoxCaptionIndent);
      end;
    end;
  end;
...
function TcxGroupBoxViewInfo.GetCaptionRect(ACanvas: TcxCanvas;
...
begin
  Result := cxNullRect;
  if Assigned(Edit) and Assigned(Edit.Style.LookAndFeel.SkinPainter) and not IsPanelStyle
  then
  begin
    ACaptionPosition := CaptionPosition;
    R1 := Edit.Style.LookAndFeel.SkinPainter.GroupBoxBorderSize(True,
      ACaptionPosition);
    case ACaptionPosition of
      cxgpTop:
...
begin
  ...
  if CheckBoxVisible then
  begin
    ACheckBoxSize := GetScaledEditCheckSize(ACanvas, NativeStyle, nil, 0,
      Painter, ScaleFactor);
...
begin
  if CheckBoxVisible then
    DrawScaledEditCheck(ACanvas, FCheckBoxRect, FCheckBoxCheckState,
      FCheckBoxViewState, nil, 0, Edit.Style.BorderStyle, NativeStyle,
      BorderColor, BackgroundColor, DrawBackground(ACanvas), IsDesigning,
      Focused, True, GetPainter, ScaleFactor);
...
begin
  cxDrawText(ACanvas.Handle, Edit.Caption, TextRect,
    Edit.GetCaptionDrawingFlags);
end;

procedure TcxGroupBoxViewInfo.DrawVerticalTextCaption(ACanvas: TcxCanvas);
const
  ARotationAngleMap: array [Boolean] of TcxRotationAngle = (raMinus90,
    raPlus90);
var
  AFlags: Cardinal;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        cxLookAndFeelPaintersManager.GetPainter(lfsNative)
          .DrawGroupBoxFrame(ACanvas, R, Enabled, cxgpCenter);
      end;
    end;
  end
  else
  begin
    ...
    begin
      case BorderStyle of
        ...
        ebsFlat:
          cxLookAndFeelPaintersManager.GetPainter(lfsFlat)
            .DrawGroupBoxFrame(ACanvas, R, Enabled, cxgpCenter,
            Edit.ActiveStyle.Edges);
        ebs3D:
          if Edit.Ctl3D then
          ...
          begin
            ACanvas.FrameRect(R, clWindowFrame, 1,
              Edit.ActiveStyle.Edges, True);
            InflateRect(R, -1, -1);
            ACanvas.FrameRect(R, BackgroundColor, 1,
              Edit.ActiveStyle.Edges, True);
          end;
      end;
    end;
...
function TcxGroupBoxViewInfo.GetThemeBackgroundRect(ACanvas: TcxCanvas): TRect;
begin
  Result := ControlRect;
  if not IsPanelStyle then
    case CaptionPosition of
      ...
      cxgpBottom:
        Dec(Result.Bottom, TdxTextMeasurer.TextLineHeight
          (ACanvas.Handle) div 2);
      cxgpLeft:
...
procedure TcxGroupBoxViewInfo.DrawParentBackground(ACanvas: TcxCanvas;
  R: TRect);
var
  P: TPoint;
begin
...
begin
  ...
  try
    ...
    else if Edit.IsNativeBackground and IsThemeBackgroundPartiallyTransparent
      (OpenTheme(totButton), BP_GROUPBOX, cxNativeState[Enabled]) then
    begin
...
procedure TcxGroupBoxViewInfo.DrawNativePanelBackground(ACanvas: TcxCanvas;
  const ACaptionRect: TRect);
var
...
begin
  ...
  if BorderStyle <> ebsNone then
    InflateRect(ABackgroundRect, -Edit.LookAndFeelPainter.BorderSize,
      -Edit.LookAndFeelPainter.BorderSize);
  ...
  begin
    ...
    try
      ...
      Edit.LookAndFeelPainter.DrawPanelBackground(ACanvas, Edit,
        GetBoundsForPanel);
    finally
      ACanvas.RestoreClipRegion;
    end;
  end
  else if Edit.LookAndFeel.NativeStyle then
    if Edit.IsNativeBackground then
      cxDrawThemeParentBackground(Edit, ACanvas, ABackgroundRect)
    else
      Edit.LookAndFeelPainter.DrawPanelBackground(ACanvas, Edit,
        ABackgroundRect, BackgroundColor)
  else if Edit.LookAndFeel.Kind = lfOffice11 then
    DrawOffice11PanelBackground(ACanvas, ABackgroundRect);
end;

procedure TcxGroupBoxViewInfo.DrawOffice11PanelBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
  with Edit.LookAndFeelPainter do
    case Edit.PanelStyle.OfficeBackgroundKind of
      pobkGradient:
        DrawPanelBackground(ACanvas, Edit, R, dxOffice11ToolbarsColor1,
          dxOffice11ToolbarsColor2);
      pobkOffice11Color:
        DrawPanelBackground(ACanvas, Edit, R,
          dxGetMiddleRGB(dxOffice11ToolbarsColor1,
...
procedure TcxGroupBoxViewInfo.InternalDrawBackgroundByPainter
  (ACanvas: TcxCanvas);
var
...
begin
  ...
  else if BorderStyle = ebsNone then
    AContentRect := Bounds
  else
    AContentRect := BorderRect;

  if Edit.DoCustomDrawContentBackground(ACanvas, AContentRect, Painter) then
    Exit;
  ...
  begin
    ...
    if BorderStyle <> ebsNone then
      Painter.DrawPanelBorders(ACanvas, AContentRect);
  end
  else if BorderStyle = ebsNone then
  ...
  begin
    ...
    if Painter.IsGroupBoxTransparent(False, ACaptionPos) or
      Painter.IsGroupBoxTransparent(True, ACaptionPos) then
      DrawParentBackground(ACanvas, ControlRect);
...
procedure TcxGroupBoxViewData.CalculateButtonsViewInfo(ACanvas: TcxCanvas;
  const ABounds: TRect; const P: TPoint; Button: TcxMouseButton;
  Shift: TShiftState; AViewInfo: TcxCustomEditViewInfo; AIsMouseEvent: Boolean);
begin
...
begin
  ...
  if not IsInplace and (Edit.Alignment <> alCenterCenter) and not IsPanelStyle
  then
  begin
    AHeaderSideBorderOffset := TdxTextMeasurer.TextLineHeight(Edit.FCaptionFont)
      div 2 - 1 + cxEditMaxBorderWidth;
    case CaptionPosition of
      cxgpTop:
...
begin
  ...
  begin
    AContentOffsets := GetContentOffsetsByPainter
      (TcxGroupBoxViewInfo(AViewInfo));
...
function TcxGroupBoxViewData.GetContentOffsetsByPainter
  (AViewInfo: TcxGroupBoxViewInfo): TRect;
begin
  if HasBorders then
  begin
    if IsPanelStyle then
...
class function TcxGroupBoxViewData.IsNativeStyle(ALookAndFeel
  : TcxLookAndFeel): Boolean;
begin
  Result := AreVisualStylesMustBeUsed(ALookAndFeel.NativeStyle or
    (ALookAndFeel.Kind = lfOffice11), totEdit) and
...
procedure TcxGroupBoxViewData.CalculateRects(ACanvas: TcxCanvas;
  AEditViewInfo: TcxGroupBoxViewInfo);
begin
  Edit.AdjustCanvasFontSettings(ACanvas);
  if IsPanelStyle then
    AEditViewInfo.CalculateRectsForPanel(ACanvas)
  else if Style.LookAndFeel.SkinPainter = nil then
...
procedure TcxGroupBoxViewData.DoRightToLeftConversion
...
begin
  ...
  AGroupBoxViewInfo.CaptionRect := TdxRightToLeftLayoutConverter.ConvertRect
    (AGroupBoxViewInfo.CaptionRect, R);
  AGroupBoxViewInfo.TextRect := TdxRightToLeftLayoutConverter.ConvertRect
    (AGroupBoxViewInfo.TextRect, R);
  AGroupBoxViewInfo.FCheckBoxRect := TdxRightToLeftLayoutConverter.ConvertRect
    (AGroupBoxViewInfo.FCheckBoxRect, R);
  AGroupBoxViewInfo.MouseFocusingRect :=
    TdxRightToLeftLayoutConverter.ConvertRect
    (AGroupBoxViewInfo.MouseFocusingRect, R);
  AGroupBoxViewInfo.FocusRect := TdxRightToLeftLayoutConverter.ConvertRect
    (AGroupBoxViewInfo.FocusRect, R);
end;

{ TcxButtonGroupButtonViewInfo }

function TcxButtonGroupButtonViewInfo.CreateFadingHelper
...
begin
  Result := ViewInfo.GetGlyphRect(EditViewInfo.GetGlyphSize,
    EditViewInfo.ItemAlignment, False);
end;

function TcxButtonGroupButtonFadingHelper.GetViewInfo
  : TcxButtonGroupButtonViewInfo;
begin
  Result := TcxButtonGroupButtonViewInfo(inherited ViewInfo);
end;

function TcxButtonGroupButtonFadingHelper.InternalGetEditViewInfo
  : TcxButtonGroupViewInfo;
begin
  Result := inherited EditViewInfo as TcxButtonGroupViewInfo;
end;

function TcxButtonGroupButtonFadingHelper.PrepareFadingImage
  (AState: TcxEditButtonState): TcxBitmap32;
begin
  Result := inherited PrepareFadingImage(AState);
  if EditViewInfo.IsInplace and not(EditViewInfo.NativeStyle or
    EditViewInfo.IsBackgroundTransparent) then
    Result.MakeOpaque;
end;
...
begin
  ...
  AGlyphRect := AButtonViewInfo.GetGlyphRect(GetGlyphSize, ItemAlignment,
    IsDBEditPaintCopyDrawing);
...
function TcxButtonGroupViewInfo.GetButtonViewInfoClass
...
procedure TcxButtonGroupViewData.CalculateButtonsViewInfo(ACanvas: TcxCanvas;
  ...
  var
    ...
    AButtonViewInfo: TcxGroupBoxButtonViewInfo;
    ACapturePressing, AHoldPressing, AIsButtonPressed,
  ...
  begin
    ...
    begin
      ...
      AButtonViewInfo.Data.NativeStyle :=
        IsButtonNativeStyle(Style.LookAndFeel);
      AButtonViewInfo.Data.Transparent :=
        (Self.Style.ButtonTransparency = ebtAlways) or
      ...
      begin
        if not Enabled then
          ...
          (not IsDesigning and PtInRect(AButtonViewInfo.Bounds, P)) then
        begin
          ...
          begin
            ...
            CanPressButton(AViewInfo, I) or cxShiftStateLeftOnly(Shift, True)
            and ((Button = cxmbLeft) and CanPressButton(AViewInfo, I) or
...
begin
  ...
  ADefaultButtonHeight := Max(ACanvas.TextHeight('Zg'), GetButtonSize.cy) +
    ScaleFactor.Apply(ASizeCorrection.cy);
  ...
  begin
    ...
    begin
      ...
      AColumnWidth := AColumnWidth div AColumnsCount -
        AEditMetrics.ButtonSize.cx -
  ...
  begin
    ...
    begin
      ...
      Result.cx := (AMaxColumnWidth + AEditMetrics.ColumnWidthCorrection +
        AEditMetrics.ButtonSize.cx) * AColumnsCount + AEditMetrics.ColumnOffset
        * (AColumnsCount - 1) + AEditMetrics.WidthCorrection;
...
begin
  ...
  if (TcxGroupBoxViewInfo(AViewInfo).ItemAlignment = taRightJustify) xor UseRightToLeftAlignment
  then
  begin
    AEditMetrics.ColumnOffset := Max(-AEditMetrics.ClientWidthCorrection,
      AEditMetrics.ColumnOffset);
    AButtonWidth := (cxRectWidth(AViewInfo.Bounds) - AClientExtent.Left -
      AClientExtent.Right + AEditMetrics.ClientWidthCorrection * 2 -
  ...
  AClientHeight := cxRectHeight(AViewInfo.Bounds) - AClientExtent.Top -
  ...
  begin
    ...
    AButtonViewInfo.Bounds.Top := ATopOffset + AButtonViewInfo.Row *
      AButtonHeight;
    ...
    if UseRightToLeftAlignment then
      AButtonViewInfo.Bounds := TdxRightToLeftLayoutConverter.ConvertRect
        (AButtonViewInfo.Bounds, AViewInfo.Bounds);
    AButtonViewInfo.VisibleBounds := AButtonViewInfo.Bounds;
  end;
end;

procedure TcxButtonGroupViewData.CalculateButtonViewInfos
  (AViewInfo: TcxCustomEditViewInfo);

  function GetButtonStyle: TcxEditButtonStyle;
  const
    ButtonInplaceStyleMap: array [TcxLookAndFeelKind] of TcxEditButtonStyle =
      (btsFlat, bts3D, btsUltraFlat, btsOffice11);
    ButtonStyleMap: array [TcxEditBorderStyle] of TcxEditButtonStyle = (bts3D,
      btsFlat, btsFlat, btsFlat, bts3D, btsUltraFlat, btsOffice11);
  begin
    if IsInplace then
...
begin
  ...
  begin
    ...
    if IsInplace and not AButtonViewInfo.Enabled then
      AButtonViewInfo.TextColor := TcxContainerStyleAccess(Style)
...
procedure TcxButtonGroupViewData.GetEditMetrics(AAutoHeight: Boolean;
  ACanvas: TcxCanvas; out AMetrics: TcxEditMetrics);
begin
  AMetrics.ClientLeftBoundCorrection :=
    ScaleFactor.Apply(6 - Integer(IsInplace) * 5);
...
function TcxButtonGroupViewData.GetBiDiModeDependentAlignment
...
class function TcxCustomGroupBoxProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxGroupBoxViewInfo;
end;

class function TcxCustomGroupBoxProperties.GetViewDataClass
...
begin
  if Assigned(ACollection) then
    if AIndex = -1 then
      TcxButtonGroupItems(ACollection).InternalNotify(Self, AIndex,
        ACollectionOperation)
    else
      TcxButtonGroupItems(ACollection).InternalNotify(nil, AIndex,
        ACollectionOperation);
end;

function TcxButtonGroupItem.GetIsCollectionDestroying: Boolean;
begin
  Result := (Collection <> nil) and TcxButtonGroupItems(Collection)
    .IsDestroying;
end;
...
procedure TcxButtonGroupItems.SetItem(Index: Integer;
  Value: TcxButtonGroupItem);
begin
...
function TcxCustomButtonGroupProperties.CreatePreviewProperties
...
begin
  ...
  for I := 0 to High(AItemCaptions) do
    TcxButtonGroupItem(TcxCustomButtonGroupProperties(Result).Items.Add).Caption
      := AItemCaptions[I];
  TcxCustomButtonGroupProperties(Result).Columns := 3;
end;

class function TcxCustomButtonGroupProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxCustomButtonGroup;
end;

function TcxCustomButtonGroupProperties.GetSpecialFeatures
  : TcxEditSpecialFeatures;
begin
  Result := inherited GetSpecialFeatures + [esfMinSize];
end;

function TcxCustomButtonGroupProperties.GetSupportedOperations
...
class function TcxCustomButtonGroupProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxButtonGroupViewInfo;
end;

procedure TcxCustomButtonGroupProperties.DoAssign
...
class function TcxCustomButtonGroupProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
...
procedure TcxPanelStyle.ScaleFactorChangeHandler(Sender: TObject; M, D: Integer;
  IsLoading: Boolean);
begin
...
class function TcxCustomGroupBox.GetPropertiesClass
...
begin
  ...
  begin
    cxRedrawWindow(Handle, RDW_ERASE or RDW_INVALIDATE or RDW_FRAME or
      RDW_ALLCHILDREN);
    Realign;
  end;
end;
...
begin
  ...
  try
    ...
    Rect := cxRectContent(GetControlRect(Self),
      AViewData.GetClientExtent(Canvas, ViewInfo));
    if IsPanelStyle then
...
function TcxCustomGroupBox.InternalGetNotPublishedStyleValues
  : TcxEditStyleValues;
begin
...
begin
  ...
  begin
    if ViewInfo.CheckBoxVisible and (CheckBoxViewState in [ecsNormal, ecsHot])
    then
    begin
...
procedure TcxCustomGroupBox.UpdateCheckBoxState(Shift: TShiftState;
  X, Y: Integer);
begin
  if not Enabled then
    CheckBoxViewState := ecsDisabled
  else if not IsDesigning and ViewInfo.PointInCheckBox(X, Y) and
    (ViewInfo.FCheckPressed or not(ssLeft in Shift)) then
  begin
...
procedure TcxCustomGroupBox.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if GetCheckBoxVisible then
  begin
    if (Button = mbLeft) and not IsDesigning and ViewInfo.PointInCheckBox(X, Y)
    then
...
procedure TcxCustomGroupBox.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if GetCheckBoxVisible then
  begin
    UpdateCheckBoxState(Shift, X, Y);
    if (Button = mbLeft) and ViewInfo.PointInCheckBox(X, Y) and ViewInfo.FCheckPressed
    then
      Toggle;
...
begin
  ...
  begin
    if not IsPanelStyle then
      Result := Alignment in [alLeftTop, alLeftCenter, alLeftBottom, alTopLeft,
        alTopCenter, alTopRight];
    ...
    begin
      ...
      begin
        if IsPanelStyle then
          ...
          ATextColor := APainter.GroupBoxTextColor(Enabled,
            ViewInfo.CaptionPosition);
      end;
    end;
  ...
  begin
    ...
    begin
      if ViewInfo.Painter <> nil then
        ViewInfo.Painter.GroupBoxAdjustCaptionFont(AFont,
          ViewInfo.CaptionPosition);
    end;
  end;
...
begin
  ...
  if Assigned(FOnCustomDrawContentBackground) then
    FOnCustomDrawContentBackground(Self, ACanvas.Canvas, ABounds,
      APainter, Result);
end;
...
begin
  if IsPanelStyle then
    Result := DrawTextFlagsMap[PanelStyle.WordWrap] or
      GetPanelStyleCaptionDrawingFlags
  else
...
begin
  Result := RedrawOnResize and (IsNativeStyle or IsSkinAvailable or
...
procedure TcxCustomButtonGroup.ActivateByMouse(Shift: TShiftState;
...
begin
  ...
  begin
    ...
    begin
      ...
      SendMouseEvent(TWinControl(FButtons[AButtonIndex]), WM_LBUTTONDOWN,
        Shift, P);
    end
    else
      SendMouseEvent(TWinControl(FButtons[AButtonIndex]), WM_LBUTTONUP,
        Shift, P);
  end;
end;
...
class function TcxCustomButtonGroup.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
begin
  Result := TcxButtonGroupViewDataClass(Properties.GetViewDataClass)
    .IsButtonNativeStyle(Style.LookAndFeel);
end;
...
begin
  ...
  if AFocusedButtonIndex - ARow + AButtonsInColumn - 1 >=
    ActiveProperties.Items.Count then
    AButtonsInColumn := ActiveProperties.Items.Count -
      (AFocusedButtonIndex - ARow);
  case Key of
    VK_DOWN:
      if (ARow < AButtonsInColumn - 1) and
        SetButtonFocus(AFocusedButtonIndex + 1) then
        Key := 0;
    VK_LEFT:
      if (AColumn > 0) and SetButtonFocus(AFocusedButtonIndex -
        AButtonsPerColumn) then
        Key := 0;
    VK_RIGHT:
      if (AFocusedButtonIndex + AButtonsPerColumn < FButtons.Count) and
        SetButtonFocus(AFocusedButtonIndex + AButtonsPerColumn) then
        Key := 0;
    VK_UP:
...
begin
  with ActiveProperties.Items do
    ...
    begin
      ...
      else if ChangedItemOperation = copDelete then
...
procedure TcxCustomButtonGroup.DoButtonDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
...
procedure TcxCustomButtonGroup.DoButtonDragOver(Sender, Source: TObject;
...
procedure TcxCustomButtonGroup.DoButtonKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  KeyDown(Key, Shift);
end;
...
procedure TcxCustomButtonGroup.DoButtonKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  KeyUp(Key, Shift);
end;

procedure TcxCustomButtonGroup.DoButtonMouseDown(Sender: TObject;
...
procedure TcxCustomButtonGroup.DoButtonMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  with TWinControl(Sender) do
...
procedure TcxCustomButtonGroup.DoButtonMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  with TWinControl(Sender) do
...
procedure TcxCustomButtonGroup.DoButtonMouseWheel(Sender: TObject;
...
function TcxCustomButtonGroup.GetActiveProperties
  : TcxCustomButtonGroupProperties;
begin
  Result := TcxCustomButtonGroupProperties(InternalGetActiveProperties);
end;

procedure TcxCustomButtonGroup.SetProperties
  (Value: TcxCustomButtonGroupProperties);
begin
