...
function dxDropDownNCHeight: Integer;
// copy in TcxCustomLookAndFeelPainter.CalculatePopupClientRect
begin
...
// copy in TcxCustomLookAndFeelPainter.CalculatePopupClientRect
const
  Corners: array [Boolean, Boolean] of TdxCorner = ((coTopLeft, coBottomLeft),
    (coTopRight, coBottomRight));
begin
  Result := Corners[EditRect.Left < DropDownRect.Right,
    EditRect.Bottom < DropDownRect.Bottom];
end;

function GetHitTestByCorner(ACorner: TdxCorner): Longint;
const
  HitTests: array [TdxCorner] of Longint = (HTTOPLEFT, HTTOPRIGHT, HTBOTTOMLEFT,
    HTBOTTOMRIGHT);
begin
...
begin
  ...
  if SX <> 22 then
    Inc(ARect.Top, Delta);
  DrawFrameControl(DC, ARect, DFC_CAPTION, DFCS_CAPTIONCLOSE or DFCS_FLAT or
    States[Pressed]);
  if SX <> 22 then
    Dec(ARect.Top, Delta);
...
begin
  ...
  with R do
    StretchBlt(ADC, X, Y, Right, Bottom, AMDC, Byte(AMirrorX) * (Right - 1),
      Byte(AMirrorY) * (Bottom - 1), (2 * Byte(not AMirrorX) - 1) * Right,
