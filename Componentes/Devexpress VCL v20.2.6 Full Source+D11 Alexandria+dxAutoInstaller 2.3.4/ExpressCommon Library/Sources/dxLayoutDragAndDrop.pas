...
type
  ...
  protected
    ...
    function GetCheckedButton(AGroupIndex: Integer)
      : TdxLayoutQuickCustomizeButton;
    ...
    property Buttons[Index: Integer]: TdxLayoutQuickCustomizeButton
      read GetButtons;
    property DestinationItem: TdxCustomLayoutItem read FDestinationItem
      write SetDestinationItem;
  public
  ...
  TdxLayoutCustomSizingDragAndDropObject = class
    (TdxLayoutCustomDragAndDropObject)
  protected
  ...
  TdxLayoutSizingDragAndDropObject = class
    (TdxLayoutCustomSizingDragAndDropObject)
  private
  ...
  TdxLayoutSplitterDragAndDropObject = class
    (TdxLayoutCustomSizingDragAndDropObject)
  private
  ...
  protected
    ...
    function GetWorkAlign(AViewInfo: TdxCustomLayoutItemViewInfo)
      : TdxLayoutAlignHorz; virtual; abstract;
    function GetItemMaxSize(AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
      virtual; abstract;
    function GetItemMinSize(AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
      virtual; abstract;
    function GetItemSize(AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
      virtual; abstract;
    ...
    procedure SetItemSize(AViewInfo: TdxCustomLayoutItemViewInfo;
  ...
  TdxLayoutHSplitterDragAndDropObject = class
    (TdxLayoutSplitterDragAndDropObject)
  protected
    function GetWorkAlign(AViewInfo: TdxCustomLayoutItemViewInfo)
      : TdxLayoutAlignHorz; override;
    function GetItemMaxSize(AViewInfo: TdxCustomLayoutItemViewInfo)
      : Integer; override;
    function GetItemMinSize(AViewInfo: TdxCustomLayoutItemViewInfo)
      : Integer; override;
    function GetItemSize(AViewInfo: TdxCustomLayoutItemViewInfo)
      : Integer; override;
  ...
  TdxLayoutVSplitterDragAndDropObject = class
    (TdxLayoutSplitterDragAndDropObject)
  protected
    function GetWorkAlign(AViewInfo: TdxCustomLayoutItemViewInfo)
      : TdxLayoutAlignHorz; override;
  ...
  protected
    ...
    property DragAndDropObject: TdxLayoutDragAndDropObject
  ...
  TdxLayoutItemInsertionKind = (ikNone, ikLeft, ikTop, ikRight, ikBottom,
    ikInside);
  ...
  protected
    ...
    procedure CorrectDropAreaPart(ADestinationItem: TdxCustomLayoutItem;
    ...
    // Drawing
    function GetDragImageBackgroundColor(AViewInfo: TdxCustomLayoutItemViewInfo)
      : TColor; virtual;
    function GetDragImageFrameColor(AViewInfo: TdxCustomLayoutItemViewInfo)
      : TColor; virtual;
    procedure PaintDestinationArea(ABitmap: TcxAlphaBitmap);
    procedure PaintDraggedItem(ABitmap: TcxAlphaBitmap;
      AViewInfo: TdxCustomLayoutItemViewInfo); virtual;
    procedure PrepareDragImage;

    property DragScreenPoint: TPoint read FDragDropInfo.DragScreenPoint
      write FDragDropInfo.DragScreenPoint;
    property DropClientPoint: TPoint read FDragDropInfo.DropClientPoint
      write FDragDropInfo.DropClientPoint;
    property DropAreaPart: TdxLayoutDropAreaPart read FDragDropInfo.DropAreaPart
      write SetDropAreaPart;
    property DestinationContainer: TdxLayoutContainer
      read FDragDropInfo.DestinationContainer write SetDestinationContainer;
    property DestinationGroup: TdxCustomLayoutGroup
      read FDragDropInfo.DestinationGroup write SetDestinationGroup;
    property DestinationItem: TdxCustomLayoutItem
      read FDragDropInfo.DestinationItem write SetDestinationItem;
    property HitItem: TdxCustomLayoutItem read FDragDropInfo.HitItem
      write SetHitItem;
    property HitTest: TdxCustomLayoutHitTest read FDragDropInfo.HitTest
      write FDragDropInfo.HitTest;
  ...
  TdxLayoutSingleDragAndDropController = class
    (TdxLayoutCustomDragAndDropController);
...
uses
  Types, SysUtils, Math,
  dxHooks, dxGDIPlusAPI, dxGDIPlusClasses, cxLibraryConsts, dxLayoutSelection,
  dxDPIAwareUtils;
...
const
  ...
  dxCustomizeFormHitTestCodes = [htCustomizeForm, htAvailableItems,
    htTreeViewItems];
...
function dxLayoutGetItemViewInfo(AItem: TdxCustomLayoutItem)
  : TdxCustomLayoutItemViewInfoAccess;
begin
  Result := TdxCustomLayoutItemViewInfoAccess(AItem.ViewInfo);
end;

procedure dxLayoutCustomizeMouseHook(ACode: Integer; wParam: wParam;
...
begin
  ...
  begin
    ...
    if IsMouseDownMessage(wParam) and
      not PtInRect(FQuickCustomizationToolbar.BoundsRect, AMHS.pt) then
...
procedure dxLayoutCustomizeWndProcHook(ACode: Integer; AWParam: wParam;
  ALParam: lParam; var AHookResult: LRESULT);
var
  AMsg: PCWPStruct;
begin
  if FQuickCustomizationToolbar <> nil then
...
begin
  ...
  begin
    ...
    P := GetMouseCursorPos;
    ABounds := FQuickCustomizationToolbar.ScaleFactor.Apply
      (Rect(0, 0, 130, 80));
    ABounds := cxRectOffset(ABounds,
      Point(P.X, P.Y - ABounds.Bottom - FQuickCustomizationToolbar.ScaleFactor.
      Apply(11)));
...
begin
  ...
  FContainer := TdxLayoutContainerAccess((AControl as IdxLayoutContainerOwner)
    .GetContainer).RealContainer;
end;

procedure TdxLayoutCustomDragAndDropObject.Init(ASourceItem
  : TdxCustomLayoutItem; const P: TPoint);
begin
...
function TdxLayoutDragAndDropObject.CreateController
  : TdxLayoutCustomDragAndDropController;
begin
  case Container.DragDropMode of
    ddmDefault:
      Result := TdxLayoutSingleDragAndDropController.Create(Self);
...
function TdxLayoutDragAndDropObject.GetDragAndDropCursor
...
begin
  inherited;
  if not TdxLayoutContainerAccess((Control as TdxCustomLayoutControl).Container)
    .IsStandardCustomization then
  begin
...
procedure TdxLayoutDragAndDropObject.DragAndDrop(const P: TPoint;
...
procedure TdxLayoutDragAndDropObject.Init(ASourceItem: TdxCustomLayoutItem;
  const P: TPoint);
begin
  inherited;
  Container.Update;
...
procedure TdxLayoutCustomSizingDragAndDropObject.EndDragAndDrop
  (Accepted: Boolean);
begin
  if Accepted then
    Modified
...
procedure TdxLayoutSizingDragAndDropObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
var
  XC, YC: Integer;
begin
  ...
  if MarkerIndex in [0, 6, 7] then
    SourceItem.Width := cxRectWidth(FOriginalBounds) + XC *
      (FStartDragPoint.X - P.X);
  if MarkerIndex in [2, 3, 4] then
    SourceItem.Width := cxRectWidth(FOriginalBounds) + XC *
      (P.X - FStartDragPoint.X);
  if MarkerIndex in [0, 1, 2] then
    SourceItem.Height := cxRectHeight(FOriginalBounds) + YC *
      (FStartDragPoint.Y - P.Y);
  if MarkerIndex in [4, 5, 6] then
    SourceItem.Height := cxRectHeight(FOriginalBounds) + YC *
      (P.Y - FStartDragPoint.Y);
end;

procedure TdxLayoutSizingDragAndDropObject.Init(ASourceItem
  : TdxCustomLayoutItem; const P: TPoint);
begin
  inherited;
...
procedure TdxLayoutSplitterDragAndDropObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
  ...
  begin
    ...
    begin
      ...
      else if GetNewLeftItemSize(ADelta) > GetItemMaxSize(FLeftItem.ViewInfo)
      then
    ...
    begin
      ...
      else if GetNewRightItemSize(ADelta) > GetItemMaxSize(FRightItem.ViewInfo)
      then
        ADelta := FRightItemSize - GetItemMaxSize(FRightItem.ViewInfo);
    end;
  end;

  function DoCanResize(var ANewSize: Integer;
    AItem: TdxCustomLayoutItem): Boolean;
  begin
    Result := (AItem = nil) or TdxLayoutSplitterItemAccess(SourceItem)
      .DoCanResize(AItem, ANewSize);
  end;

  function CanResize(ANewSize1, ANewSize2: Integer): Boolean;
  begin
    Result := True;
    if FLeftItem <> nil then
      Result := Result and (ANewSize1 >= GetItemMinSize(FLeftItem.ViewInfo)) and
        (ANewSize1 <= GetItemMaxSize(FLeftItem.ViewInfo));
    if Result and (FRightItem <> nil) then
      Result := Result and (ANewSize2 >= GetItemMinSize(FRightItem.ViewInfo))
        and (ANewSize2 <= GetItemMaxSize(FRightItem.ViewInfo));
  end;

var
  ADelta: Integer;
  ASizingStrategy: TdxLayoutSizingStrategy;
  ANewSize, ANewSize2: Integer;
begin
  ...
  begin
    ...
    if (FParentItem.LayoutDirection = ldHorizontal) and
      TdxLayoutContainerAccess(Container).UseRightToLeftAlignment then
      ADelta := -ADelta;
    ...
    case ASizingStrategy of
      lssLeft:
        begin
          ...
          if DoCanResize(ANewSize, FLeftItem) then
            SetItemSize(FLeftItem.ViewInfo, ANewSize);
        end;
      lssRight:
        begin
          ANewSize := GetNewRightItemSize(ADelta);
          if DoCanResize(ANewSize, FRightItem) then
      ...
      lssClient:
        begin
          CorrectDelta(ADelta);
          ANewSize := GetNewLeftItemSize(ADelta);
          ANewSize2 := GetNewRightItemSize(ADelta);
          if DoCanResize(ANewSize, FLeftItem) and
            DoCanResize(ANewSize2, FRightItem) and CanResize(ANewSize, ANewSize2)
          then
          begin
            Container.BeginUpdate;
            try
              SetItemSize(FLeftItem.ViewInfo, ANewSize);
              SetItemSize(FRightItem.ViewInfo, ANewSize2);
            finally
...
procedure TdxLayoutSplitterDragAndDropObject.Init
...
begin
  ...
  for I := 0 to FParentItem.VisibleCount - 1 do
    FOriginalSizes[I] := Point(FParentItem.VisibleItems[I].Width,
      FParentItem.VisibleItems[I].Height);
...
procedure TdxLayoutSplitterDragAndDropObject.AssignSizingParameters;
  ...
  begin
    ...
    for I := SourceItem.VisibleIndex - 1 downto 0 do
      if GetWorkAlign(FParentItem.VisibleItems[I].ViewInfo)
        in [GetWorkAlign(FSourceItem.ViewInfo), ahClient] then
      begin
  ...
  function GetSimpleStrategy(AAlign: TdxLayoutAlignHorz)
    : TdxLayoutSizingStrategy;
  begin
    case AAlign of
      ahLeft:
        if FLeftItem <> nil then
    ...
    if (Result in [lssRight, lssLeft]) and (FLeftItem <> nil) and
      (FRightItem <> nil) and (GetWorkAlign(FLeftItem.ViewInfo) = ahClient) and
      (GetWorkAlign(FRightItem.ViewInfo) = ahClient) then
      Result := lssClient;
  end;
...
begin
  ...
  case GetWorkAlign(FSourceItem.ViewInfo) of
    ahLeft:
      if (FLeftItem <> nil) and (GetWorkAlign(FLeftItem.ViewInfo) = ahClient)
      then
        FSizingStrategy := GetSimpleStrategy(ahRight)
      else
        FSizingStrategy := GetSimpleStrategy(ahLeft);
    ahRight:
      if (FRightItem <> nil) and (GetWorkAlign(FRightItem.ViewInfo) = ahClient)
      then
        FSizingStrategy := GetSimpleStrategy(ahLeft)
      else
        FSizingStrategy := GetSimpleStrategy(ahRight);
    ...
    try
      for I := 0 to FParentItem.VisibleCount - 1 do
        if GetWorkAlign(FParentItem.VisibleItems[I].ViewInfo) = ahClient then
          SetItemSize(FParentItem.VisibleItems[I].ViewInfo,
            GetItemSize(FParentItem.VisibleItems[I].ViewInfo));
    finally
      Container.CancelUpdate;
    end;
  end;
...
begin
  ...
  try
    for I := 0 to FParentItem.VisibleCount - 1 do
      SetItemSize(FParentItem.VisibleItems[I].ViewInfo,
        GetSignificantValue(FOriginalSizes[I]));
...
function TdxLayoutHSplitterDragAndDropObject.GetWorkAlign
  (AViewInfo: TdxCustomLayoutItemViewInfo): TdxLayoutAlignHorz;
begin
  Result := AViewInfo.AlignHorz;
end;

function TdxLayoutHSplitterDragAndDropObject.GetItemMaxSize
  (AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
begin
  Result := AViewInfo.MaxWidth;
end;

function TdxLayoutHSplitterDragAndDropObject.GetItemMinSize
  (AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
begin
  Result := AViewInfo.MinWidth;
end;

function TdxLayoutHSplitterDragAndDropObject.GetItemSize
  (AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
begin
  Result := cxRectWidth(AViewInfo.Bounds);
end;

function TdxLayoutHSplitterDragAndDropObject.GetSignificantValue
...
procedure TdxLayoutHSplitterDragAndDropObject.SetItemSize
  (AViewInfo: TdxCustomLayoutItemViewInfo; ASize: Integer);
begin
  ASize := Max(ASize, 1);
  TdxCustomLayoutItemViewInfoAccess(AViewInfo).SetItemWidth(ASize,
    TdxLayoutSplitterItemAccess(SourceItem).DirectAccess);
end;

{ TdxLayoutHSplitterDragAndDropObject }

function TdxLayoutVSplitterDragAndDropObject.GetWorkAlign
  (AViewInfo: TdxCustomLayoutItemViewInfo): TdxLayoutAlignHorz;
begin
  Result := TdxLayoutAlignHorz(AViewInfo.AlignVert);
end;

function TdxLayoutVSplitterDragAndDropObject.GetItemMaxSize
  (AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
begin
  Result := AViewInfo.MaxHeight;
end;

function TdxLayoutVSplitterDragAndDropObject.GetItemMinSize
  (AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
begin
  Result := AViewInfo.MinHeight;
end;

function TdxLayoutVSplitterDragAndDropObject.GetItemSize
  (AViewInfo: TdxCustomLayoutItemViewInfo): Integer;
begin
  Result := cxRectHeight(AViewInfo.Bounds);
end;

function TdxLayoutVSplitterDragAndDropObject.GetSignificantValue
  (const P: TPoint): Integer;
begin
  Result := P.Y;
end;
...
procedure TdxLayoutVSplitterDragAndDropObject.SetItemSize
  (AViewInfo: TdxCustomLayoutItemViewInfo; ASize: Integer);
begin
  ASize := Max(ASize, 1);
  TdxCustomLayoutItemViewInfoAccess(AViewInfo).SetItemHeight(ASize,
    TdxLayoutSplitterItemAccess(SourceItem).DirectAccess);
end;
...
procedure TdxLayoutDragAndDropHelper.InitializeDragItem
  (AItem: TdxCustomLayoutItem; X, Y: Integer);
begin
...
begin
  Result := Container.CanDragAndDrop and (Control.DragAndDropState = ddsNone)
    and not(cxPointIsEqual(FMouseDownPos, cxInvalidPoint) or
    IsPointInDragDetectArea(FMouseDownPos, X, Y)) and (DragItem <> nil) and
    TdxCustomLayoutItemAccess(DragItem).CanDragAndDrop(Point(X, Y));
end;
...
function TdxLayoutDragAndDropHelper.GetDragAndDropObject
  : TdxLayoutDragAndDropObject;
begin
...
constructor TdxLayoutCustomDragAndDropController.Create
  (AOwner: TdxLayoutDragAndDropObject);
begin
  inherited Create;
  FOwner := AOwner;
...
begin
  ...
  if not cxPointIsEqual(Owner.FStartDragPoint, cxInvalidPoint) then
    FDragImageOffset := cxPointOffset(SourceItem.ViewInfo.Bounds.TopLeft,
      Owner.FStartDragPoint, False)
  else
  ...
  if SourceItem.Container.CustomizeForm <> nil then
    TdxLayoutContainerAccess(SourceItem.Container).CustomizeFormPostUpdate
      ([cfutAvailableItems, cfutVisibleItems]);
...
procedure TdxLayoutCustomDragAndDropController.EndDragAndDrop
  (Accepted: Boolean);

  function CanProcessDrop: Boolean;
  begin
    Result := Accepted and (DestinationContainer <> nil) and
      ((CanDrop and (DestinationItem <> nil)) or CanRemove or CanFloat);
  end;

  function GetDropActionType(AAreaPart: TdxLayoutDropAreaPart)
    : TdxLayoutActionType;
  begin
    Result := TdxLayoutGroupViewInfoAccess(DestinationGroup.ViewInfo)
      .GetDropActionType(AAreaPart);
  end;
...
procedure TdxLayoutCustomDragAndDropController.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
begin
  UpdateStates;
  ShowDragImage;
...
procedure TdxLayoutCustomDragAndDropController.PaintDraggedItem
  (ABitmap: TcxAlphaBitmap; AViewInfo: TdxCustomLayoutItemViewInfo);
begin
  TdxCustomLayoutItemViewInfoAccess(AViewInfo).PaintSelection(ABitmap);
end;

function TdxLayoutCustomDragAndDropController.FindDestinationContainer
  (const P: TPoint): TdxLayoutContainer;

  function HasRelation(AContainer1, AContainer2: TdxLayoutContainer): Boolean;
  begin
    Result := (TdxLayoutContainerAccess(AContainer1)
      .MasterContainer = AContainer2) or
      (TdxLayoutContainerAccess(AContainer2).MasterContainer = AContainer1) or
      (TdxLayoutContainerAccess(AContainer1)
      .MasterContainer = TdxLayoutContainerAccess(AContainer2).MasterContainer);
  end;
    ...
    begin
      ...
      begin
        ...
        if AParentForm is TdxLayoutControlCustomCustomizeForm then
          ALayoutContainer := TdxLayoutControlCustomCustomizeForm(AParentForm)
            .Container
        else
...
begin
  ...
  if (Result = nil) or not(Result.IsDesigning or HasRelation(Result, Container))
  then
    Result := Container;
end;

function TdxLayoutCustomDragAndDropController.GetDestinationGroup
  : TdxCustomLayoutGroup;
begin
  if HitItem = nil then
    Result := nil
  else if HitItem.IsRoot or (DropAreaPart in [apLastChild, apNewLayout]) and
    (HitItem is TdxCustomLayoutGroup) then
...
function TdxLayoutCustomDragAndDropController.GetFittedRect
  (const ARect: TRect): TRect;
begin
  cxRectIntersect(Result, ARect, DestinationContainer.ClientRect);
end;

procedure TdxLayoutCustomDragAndDropController.DropItem(ALayoutAction
  : TdxLayoutActionType);

  function GetOrthogonalDirection: TdxLayoutDirection;
  begin
    Result := TdxLayoutGroupAccess(DestinationGroup)
      .GetHelperClass.GetOrthogonalDirection;
  end;
  ...
  function GetDropIndex(AParentGroup: TdxCustomLayoutGroup;
    const ADropAreaInfo: TdxLayoutDragDropInfo): Integer;
  var
  ...
  begin
    ...
    if TdxLayoutContainerAccess(Container).UseRightToLeftAlignment then
      if ADropPart = apLeft then
        ADropPart := apRight
      else if ADropPart = apRight then
        ADropPart := apLeft;
    case ADropPart of
      ...
      apBetween:
        if (AParentGroup.LayoutDirection = ldHorizontal) and
          (ADropAreaInfo.DropClientPoint.X >
          ADropAreaInfo.DestinationItem.ViewInfo.Bounds.Right) or
          (AParentGroup.LayoutDirection = ldVertical) and
          (ADropAreaInfo.DropClientPoint.Y >
          ADropAreaInfo.DestinationItem.ViewInfo.Bounds.Bottom) then
          Result := ADropAreaInfo.DestinationItem.Index + 1
        else
          Result := ADropAreaInfo.DestinationItem.Index;
      apNewLayout:
        if (AParentGroup.LayoutDirection = ldHorizontal) and
          (ADropAreaInfo.DestinationItem.AlignHorz in [ahParentManaged, ahLeft])
          or (AParentGroup.LayoutDirection = ldVertical) and
          (ADropAreaInfo.DestinationItem.AlignVert in [avParentManaged, avTop])
        then
          Result := 1
        else
          Result := 0;
      apLastChild:
        begin
          ...
          if (AParentGroup.LayoutDirection = ldHorizontal) and
            (AAlign.Horz = ahRight) or
            (AParentGroup.LayoutDirection = ldVertical) and
            (AAlign.Vert = avBottom) then
            Result := 0
          else
            Result := AParentGroup.Count;
        end
  ...
  const
    LayoutDirections: array [Boolean] of TdxLayoutDirection = (ldVertical,
      ldHorizontal);
  var
  ...
  begin
    ...
    begin
      ...
      DestinationGroup.LayoutDirection := LayoutDirections
        [IsHorizontalDropAreaPart];
      AParentGroup := DestinationGroup;
    end
    else
      AParentGroup := DestinationItem.PutIntoHiddenGroup
...
begin
  dxTestCheck(ALayoutAction <> atNone,
    'TdxLayoutCustomDragAndDropController.DropItem fails');
  ...
  if TdxLayoutContainerAccess(Container).IsQuickCustomization and
    TdxLayoutContainerAccess(Container).ShowQuickCustomizationToolbar then
...
begin
  TdxCustomLayoutItemAccess(SourceItem)
    .MakeFloat(cxPointOffset(DragScreenPoint, FDragImageOffset), True);
end;

function TdxLayoutCustomDragAndDropController.GetCursor
  (Accepted: Boolean): TCursor;
begin
  if CanDrop then
...
begin
  ...
  TcxDragImageAccess(FDragImage).AlphaBlendValue :=
    TcxDragImageAccess(FDragImage).AlphaBlendValue div 3;
end;
...
procedure TdxLayoutCustomDragAndDropController.CorrectDropAreaPart
  (ADestinationItem: TdxCustomLayoutItem; var AAreaPart: TdxLayoutDropAreaPart);
begin
  if TdxLayoutContainerAccess(Container).IsQuickCustomization then
    if (ADestinationItem.Parent <> nil) and not TdxCustomLayoutItemAccess
      (ADestinationItem.Parent).CanQuickCustomized or
      ADestinationItem.IsRoot and not TdxCustomLayoutItemAccess
      (ADestinationItem).CanQuickCustomized then
      AAreaPart := apNone;
end;
...
begin
  ...
  if (HitItem <> nil) and (DestinationGroup = HitItem) and
    TdxCustomLayoutItemAccess(DestinationGroup).IsViewInfoValid then
    DestinationItem := TdxLayoutGroupViewInfoAccess(DestinationGroup.ViewInfo)
      .GetDestinationItem(FDragDropInfo)
  else
...
begin
  ...
  if not TdxCustomLayoutItemAccess(DestinationGroup).IsViewInfoValid or
    (HitTest.HitTestCode in dxCustomizeFormHitTestCodes) then
    AExpectedAlign := dxLayoutAlign(ahParentManaged, avParentManaged)
  else
    AExpectedAlign := TdxLayoutGroupViewInfoAccess(DestinationGroup.ViewInfo)
      .GetDropExpectedAlign(FDragDropInfo);
...
function TdxLayoutCustomDragAndDropController.GetSourceItem
  : TdxCustomLayoutItem;
begin
...
begin
  ...
  if TdxLayoutGroupAccess(ADragDropInfo.DestinationGroup).IsSuperfluous(False)
  then
    ADragDropInfo.DestinationGroup := ADragDropInfo.DestinationGroup.Parent;
  Result := TdxLayoutGroupViewInfoAccess
    (ADragDropInfo.DestinationGroup.ViewInfo).GetDropExpectedBounds
    (ADragDropInfo);
end;

function TdxLayoutCustomDragAndDropController.GetDragImageBackgroundColor
  (AViewInfo: TdxCustomLayoutItemViewInfo): TColor;
begin
  Result := dxLayoutDefaultDragImageBackgroundColor;
end;

function TdxLayoutCustomDragAndDropController.GetDragImageFrameColor
  (AViewInfo: TdxCustomLayoutItemViewInfo): TColor;
begin
  Result := dxLayoutSelectionBorderColor;
end;

procedure TdxLayoutCustomDragAndDropController.PaintDestinationArea
...
begin
  if (DestinationItem = nil) or (DestinationGroup = nil) or
    (DropAreaPart = apNone) or
  ...
  case TdxLayoutGroupViewInfoAccess(FDragDropInfo.DestinationGroup.ViewInfo)
    .GetDropHorzAlignForLocale(FDragDropInfo.ExpectedAlign.Horz) of
    ahLeft:
      AArrowPositions := AArrowPositions - [coTopRight, coBottomRight];
    ahRight:
      AArrowPositions := AArrowPositions - [coTopLeft, coBottomLeft];
  end;
  case FDragDropInfo.ExpectedAlign.Vert of
    avTop:
      AArrowPositions := AArrowPositions - [coBottomLeft, coBottomRight];
    avBottom:
      AArrowPositions := AArrowPositions - [coTopLeft, coTopRight];
  end;
  dxGPPaintCanvas.RoundRect(ATargetRect,
    dxLayoutDefaultDragDestinationBorderColor, 0, 5, 5, 1, 255, 0);
  ...
  begin
    if AArrowPosition in AArrowPositions then
      case AArrowPosition of
        coTopLeft:
          DrawArrowLT(ATargetRect.TopLeft);
        coBottomLeft:
          DrawArrowLB(Point(ATargetRect.Left, ATargetRect.Bottom));
        coTopRight:
          DrawArrowRT(Point(ATargetRect.Right, ATargetRect.Top));
        coBottomRight:
          DrawArrowRB(ATargetRect.BottomRight);
      end;
  end;
...
procedure TdxLayoutCustomDragAndDropController.PrepareDragImage;
  ...
  begin
    if AItem.FParentBeforeDrag <> nil then
      AScaleFactor := TdxCustomLayoutItemAccess(AItem.FParentBeforeDrag)
        .ScaleFactor
    else
  ...
  begin
    ...
    try
      ...
      try
        ...
        AViewInfo.PaintContent(TcxDragImageAccess(FDragImage).Image,
          True, False);
      finally
  ...
  procedure CreateDockedDragImage(AItem: TdxCustomLayoutItemAccess);
    ...
    function GetViewInfoBounds(AViewInfo: TdxCustomLayoutItemViewInfoAccess;
      AHasBorderChanged: Boolean): TRect;
    begin
      if AItem.IsAvailable or not AViewInfo.ActuallyVisible then
        Result := Rect(-AViewInfo.CalculateWidth,
          -AViewInfo.CalculateHeight, 0, 0)
      else
      begin
        Result := AViewInfo.OriginalBounds;
        if AHasBorderChanged then
          Result := TdxLayoutGroupViewInfoAccess(AViewInfo)
            .GetItemsAreaBounds(Result);
      end;
    end;
  ...
  begin
    ...
    begin
      AViewInfo := TdxCustomLayoutItemViewInfoAccess
        (TdxLayoutContainerViewInfoAccess(Container.ViewInfo)
        .CreateItemViewInfo(nil, AItem));
    ...
    try
      ...
      try
        AViewInfo.Calculate(GetViewInfoBounds(AViewInfo,
          AViewInfo.HasBorder <> APrevHasBorder));
        FDragDropInfo.SourceItemSize := GetItemDragSize(AViewInfo);
        FDragImage.SetBounds(0, 0, FDragDropInfo.SourceItemSize.cx,
          FDragDropInfo.SourceItemSize.cy);
...
procedure TdxLayoutCustomDragAndDropController.SetDropAreaPart
  (Value: TdxLayoutDropAreaPart);
begin
...
procedure TdxLayoutCustomDragAndDropController.SetDestinationContainer
  (Value: TdxLayoutContainer);
begin
...
begin
  if (Value = SourceItem) or ((Value is TdxCustomLayoutGroup) and
    not SourceItem.CanMoveTo(Value)) then
    Value := nil;
  ...
  begin
    ...
    begin
      if HitTest.HitTestCode in dxCustomizeFormHitTestCodes then
        DestinationItem.Container.CustomizeForm.ToggleHotTrackState
          (DestinationItem);
    end;
...
procedure TdxLayoutCustomDragAndDropController.SetHitItem
  (Value: TdxCustomLayoutItem);
begin
...
procedure TdxLayoutCustomDragAndDropController.UpdateStates;
  ...
  begin
    HitTest := FDragDropInfo.HitContainer.GetHitTest
      (FDragDropInfo.HitContainer.ScreenToClient(DragScreenPoint));
  end;
...
procedure TdxLayoutQuickCustomizeButton.Draw(ACanvas: TcxCanvas;
  AScaleFactor: TdxScaleFactor);
begin
...
procedure TdxLayoutQuickCustomizeButton.DrawIcon(ACanvas: TcxCanvas;
  AScaleFactor: TdxScaleFactor);
  ...
  case FTag of
    0:
      begin
        ...
        if FGroupIndex = 0 then
          ACanvas.FillRect(Rect(R.Left - Scale(2), R.Top - Scale(3), R.Left,
            R.Bottom + Scale(3)), FrameColor)
        else
          ACanvas.FillRect(Rect(R.Left - Scale(3), R.Top - Scale(2),
            R.Right + Scale(3), R.Top), FrameColor);
      end;
    ...
    1:
      begin
        ...
        if FGroupIndex = 0 then
          ACanvas.FillRect(Rect(R.Left + Scale(5), R.Top - Scale(3),
            R.Right - Scale(5), R.Bottom + Scale(3)), FrameColor)
        else
          ACanvas.FillRect(Rect(R.Left - Scale(3), R.Top + Scale(5),
            R.Right + Scale(3), R.Bottom - Scale(5)), FrameColor);
      end;
    ...
    2:
      begin
        ...
        R := cxRectContent(FBounds, AOffset);
        if FGroupIndex = 0 then
          ACanvas.FillRect(Rect(R.Right, R.Top - Scale(3), R.Right + Scale(2),
            R.Bottom + Scale(3)), FrameColor)
        else
          ACanvas.FillRect(Rect(R.Left - Scale(3), R.Bottom, R.Right + Scale(3),
            R.Bottom + Scale(2)), FrameColor);
      end;
...
procedure TdxLayoutQuickCustomizationToolbar.CreateParams
...
procedure TdxLayoutQuickCustomizationToolbar.CreateWindowHandle
  (const Params: TCreateParams);
begin
  inherited;
  cxSetLayeredWindowAttributes(Handle, 153);
  SetWindowRgn(Handle, CreateRoundRectRgn(0, 0, Width + 1, Height + 1, 3,
    3), False);
...
procedure TdxLayoutQuickCustomizationToolbar.Notification
...
procedure TdxLayoutQuickCustomizationToolbar.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
...
procedure TdxLayoutQuickCustomizationToolbar.MouseDown(Button: TMouseButton;
...
procedure TdxLayoutQuickCustomizationToolbar.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TdxLayoutQuickCustomizationToolbar.ClickButton
  (AButton: TdxLayoutQuickCustomizeButton);
begin
...
procedure TdxLayoutQuickCustomizationToolbar.InitButtons;
  ...
  function CreateQuickButton(const R: TRect; AGroupIndex, ATag: TcxTag)
    : TdxLayoutQuickCustomizeButton;
  begin
...
begin
  ...
  Buttons[Integer(TdxCustomLayoutItemAccess(FDestinationItem).RealAlign.Horz)
    ].FChecked := True;
  Buttons[4 + Integer(TdxCustomLayoutItemAccess(FDestinationItem)
    .RealAlign.Vert)].FChecked := True;
end;

function TdxLayoutQuickCustomizationToolbar.GetCheckedButton
  (AGroupIndex: Integer): TdxLayoutQuickCustomizeButton;
var
  I: Integer;
begin
  Result := nil;
...
begin
  ...
  AMouseDistance.X := Max(ABoundsRect.Left - AMousePos.X,
    AMousePos.X - ABoundsRect.Right);
  AMouseDistance.Y := Max(ABoundsRect.Top - AMousePos.Y,
    AMousePos.Y - ABoundsRect.Bottom);
  ...
  begin
    ...
    begin
      ...
      if AMinMouseDistance > ALimitVisibility then
        ...
        Result := MulDiv(255, ALimitVisibility - AMinMouseDistance,
          ALimitVisibility);
    end;
  end;
end;

function TdxLayoutQuickCustomizationToolbar.GetButton(const P: TPoint)
  : TdxLayoutQuickCustomizeButton;
var
  I: Integer;
begin
...
function TdxLayoutQuickCustomizationToolbar.GetActiveButton
  : TdxLayoutQuickCustomizeButton;
var
  I: Integer;
begin
...
procedure TdxLayoutQuickCustomizationToolbar.CalculateButtonState
  (const P: TPoint; AShiftState: TShiftState);
var
  APrevHotButton, ANewHotButton: TdxLayoutQuickCustomizeButton;
begin
  APrevHotButton := GetActiveButton;
  ANewHotButton := GetButton(P);
  if (APrevHotButton <> ANewHotButton) or (ANewHotButton <> nil) and
...
procedure TdxLayoutQuickCustomizationToolbar.CMMouseLeave
...
procedure TdxLayoutQuickCustomizationToolbar.WMMouseActivate
...
procedure TdxLayoutQuickCustomizationToolbar.WMNCActivate
...
function TdxLayoutQuickCustomizationToolbar.GetButtons(AIndex: Integer)
  : TdxLayoutQuickCustomizeButton;
begin
  Result := FButtons[AIndex] as TdxLayoutQuickCustomizeButton;
end;

procedure TdxLayoutQuickCustomizationToolbar.SetDestinationItem
  (AValue: TdxCustomLayoutItem);
begin
...
begin
  if DestinationItem <> nil then
    Result := Forms.GetParentForm(TdxCustomLayoutItemAccess(FDestinationItem)
      .RealContainer.ItemsParent)
  else
    Result := nil;
end;
