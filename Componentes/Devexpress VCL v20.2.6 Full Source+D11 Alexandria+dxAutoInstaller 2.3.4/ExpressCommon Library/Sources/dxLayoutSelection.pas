...
type
  ...
  TdxLayoutRunTimeSelectionHelper = class(TcxOwnedPersistent,
    IdxLayoutDesignerHelper)
  private
    FListeners: TList;
    FRefCount: Integer;
    ...
    procedure SelectionListNotifyHandler(Sender: TObject;
      AComponent: TComponent; AAction: TListNotification);
    procedure SelectionListChangedHandler(Sender: TObject;
  ...
  protected
    ...
    procedure SelectionListNotify(AComponent: TComponent;
  ...
  TdxLayoutRunTimeSelectionHelperClass = class of
    TdxLayoutRunTimeSelectionHelper;
  ...
  public
    constructor Create(ASelectionLayer: TdxSelectionLayer);
      reintroduce; virtual;
    procedure Hide;
  ...
  TdxSelectionLayerHitTestEvent = procedure(ASender: TObject;
    var AIsTransparent: Boolean) of object;
  TdxSelectionLayerEndRenameEvent = procedure(ASender: TObject;
    const AText: string; AAccept: Boolean) of object;
  ...
  public
    ...
    constructor Create(AParentControl: TcxControl; AParentWindow: HWND);
    ...
    procedure BeginRename(const ABounds: TRect; const AText: string;
      const AFont: TFont);
    ...
    property ParentControl: TcxControl read FParentControl
      write SetParentControl;
    property OnEndRename: TdxSelectionLayerEndRenameEvent read FOnEndRename
      write FOnEndRename;
    property OnHide: TNotifyEvent read FOnHide write FOnHide;
    property OnHitTest: TdxSelectionLayerHitTestEvent read FOnHitTest
      write FOnHitTest;
...
procedure TdxLayoutRunTimeSelectionHelper.AddSelectionChangedListener
...
procedure TdxLayoutRunTimeSelectionHelper.RemoveSelectionChangedListener
  (AListener: TPersistent);
begin
...
procedure TdxLayoutRunTimeSelectionHelper.DeleteComponent
  (AComponent: TComponent);
begin
  AComponent.Free;
end;

procedure TdxLayoutRunTimeSelectionHelper.DeleteComponents
  ...
  begin
    for I := AList.Count - 1 downto 0 do
      if not Supports(AList[I], IdxLayoutSelectableItem, AIntf) or not AIntf.CanDelete
      then
...
function TdxLayoutRunTimeSelectionHelper.IsComponentSelected
...
function TdxLayoutRunTimeSelectionHelper.UniqueName(const BaseName
  : string): string;
begin
  Result := '';
end;

function TdxLayoutRunTimeSelectionHelper.GetComponentClass
  (AComponent: TComponent): TComponentClass;
begin
  Result := TComponentClass(AComponent.ClassType);
end;

procedure TdxLayoutRunTimeSelectionHelper.NotifyListeners(AList: TList;
...
procedure TdxLayoutRunTimeSelectionHelper.SelectionListNotify
  (AComponent: TComponent; AAction: TListNotification);
var
  ASelectableItem: IdxLayoutSelectableItem;
begin
...
function TdxLayoutRunTimeSelectionHelper.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
...
procedure TdxLayoutRunTimeSelectionHelper.SelectionListNotifyHandler
  (Sender: TObject; AComponent: TComponent; AAction: TListNotification);
begin
  SelectionListNotify(AComponent, AAction);
end;

procedure TdxLayoutRunTimeSelectionHelper.SelectionListChangedHandler
  (Sender: TObject; AComponent: TComponent;
  AAction: TcxComponentCollectionNotification);
const
  Action: array [TcxComponentCollectionNotification] of TdxSelectionAction =
    (saAdded, saChanged, saExtracted, saExtracted, saExtracted);
var
  AList: TList;
begin
...
constructor TdxSelectionLayerInplaceEdit.Create(ASelectionLayer
  : TdxSelectionLayer);
begin
...
procedure TdxSelectionLayerInplaceEdit.Show(const ABounds: TRect;
  const AText: string);
begin
  Text := AText;
  BoundsRect := ABounds;
...
procedure TdxSelectionLayerInplaceEdit.KeyDown(var Key: Word;
  Shift: TShiftState);
    ...
    WM_SETFOCUS:
      begin
        if (GetParentForm(Self) = nil) or GetParentForm(Self)
          .SetFocusedControl(SelectionLayer) then
          Dispatch(Message);
        Exit;
      end;
    WM_KILLFOCUS:
...
constructor TdxSelectionLayer.Create(AParentControl: TcxControl;
  AParentWindow: HWND);
begin
...
procedure TdxSelectionLayer.Paint;
  ...
  begin
    ...
    try
      ...
      cxPaintTo(AParent, ABitmap.cxCanvas, cxPointInvert(AOffset),
        BoundsRect, AList);
      AList.Clear;
      (Owner as IdxLayoutDesignerHelper).GetSelection(AList);
      if AList.Count > 1 then
        for I := 0 to AList.Count - 1 do
          if not Supports(TObject(AList[I]), IdxLayoutSelectableItem) and
            (TObject(AList[I]) is TWinControl) and
            (IsParent(AParent, TWinControl(AList[I])) or (AList[I] = AParent))
          then
          begin
            ARect := cxGetWindowBounds(TWinControl(AList[I]));
            if AList[I] <> AParent then
              ARect := cxRectOffset(ARect, TWinControl(AList[I])
                .ClientToParent(Point(0, 0), AParent));
            ABitmap.cxCanvas.FillRect(cxRectBounds(ARect.Left, ARect.Top, 4,
              4), clBlack);
            ABitmap.cxCanvas.FillRect(cxRectBounds(ARect.Right - 4, ARect.Top,
              4, 4), clBlack);
            ABitmap.cxCanvas.FillRect(cxRectBounds(ARect.Left, ARect.Bottom - 4,
              4, 4), clBlack);
            ABitmap.cxCanvas.FillRect(cxRectBounds(ARect.Right - 4,
              ARect.Bottom - 4, 4, 4), clBlack);
          end;
    finally
      AList.Free;
    end;
...
begin
  ...
  try
    ...
    cxAlphaBlend(ABitmap.cxCanvas.Handle, SelectionImage.cxCanvas.Handle,
      ClientRect, ClientRect);
    cxBitBlt(cxCanvas.Handle, ABitmap.cxCanvas.Handle, ClientRect,
      cxNullPoint, SRCCOPY);
  finally
    ABitmap.Free;
  end;
...
procedure TdxSelectionLayer.BeginRename(const ABounds: TRect;
  const AText: string; const AFont: TFont);
begin
  FEditorMode := True;
  ...
  case Message.Msg of
    WM_MOUSEFIRST .. WM_MOUSELAST:
      begin
        ...
        if AParent <> nil then
        begin
          TWMMouse(Message).Pos :=
            PointToSmallPoint
            (cxPointOffset(SmallPointToPoint(TWMMouse(Message).Pos),
            BoundsRect.TopLeft));
          Message.Result := AParent.Perform(Message.Msg, Message.WPARAM,
            Message.LParam);
        end
        else
          inherited;
...
begin
  ...
  MoveWindow(FInplaceEdit.Handle, R.Left, R.Top, R.Right - R.Left,
