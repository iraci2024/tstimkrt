...
{$ENDIF}
  ...
  cxLookAndFeelPainters, cxGeometry, cxGraphics, cxContainer, cxControls,
  cxEdit, cxTextEdit;

const
  dxIncrementalFilteringNavigationKeys = [VK_DOWN, VK_UP, VK_PRIOR, VK_NEXT,
    VK_HOME, VK_END];
...
type
  ...
  protected
    ...
    procedure DrawSearchEditButton(Sender: TcxEditButtonViewInfo;
      ACanvas: TcxCanvas; var AHandled: Boolean); virtual;
    ...
    procedure SearchEditKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState); virtual; abstract;
  ...
  TdxCustomIncrementalFilteringContainerHelper = class
    (TdxCustomIncrementalFilteringHelper)
  protected
  ...
  TdxIncrementalFilteringFilterPopupHelper = class
    (TdxCustomIncrementalFilteringContainerHelper)
  protected
  ...
  public
    ...
    property ShowSearchInfoPanel: Boolean read FShowSearchInfoPanel
      write SetShowSearchInfoPanel;
    property ViewInfo: TdxCustomIncrementalFilteringContainerViewInfo
      read GetViewInfo; // for internal use
  end;
...
begin
  ...
  begin
    if not(Sender.Data.UseSkins or Sender.Data.NativeStyle) then
      FillRectByColor(ACanvas.Handle, Sender.Bounds,
        Sender.Data.BackgroundColor);
    GetLookAndFeelPainter.DrawScaledSearchEditButtonGlyph(ACanvas,
      Sender.Bounds, cxbsNormal, Sender.ScaleFactor);
  end;
end;
...
function TdxCustomIncrementalFilteringContainerHelper.IsActionKey
  (AKey: Word): Boolean;
begin
  Result := AKey in [VK_SPACE, VK_RETURN];
end;

procedure TdxCustomIncrementalFilteringContainerHelper.CheckSearchControl
  (AParent: TWinControl);
begin
  CreateSearchEdit;
  InitSearchEdit;
...
procedure TdxCustomIncrementalFilteringContainerHelper.SearchEditKeyDown
...
procedure TdxCustomIncrementalFilteringContainerHelper.SearchEditKeyPress
...
function TdxIncrementalFilteringFilterPopupHelper.GetMouseWheelHandler
...
procedure TdxCustomIncrementalFilteringContainerViewInfo.DrawSearchInfoPanel
  (ACanvas: TcxCanvas);
begin
  ...
  cxTextOut(ACanvas.Canvas, GetSearchInfoPanelText, SearchInfoPanelBounds,
    GetSearchInfoPanelTextDrawFlags, Font, 0, GetSearchInfoPanelTextOffset, 0,
    Painter.GetWindowContentTextColor);
end;

function TdxCustomIncrementalFilteringContainerViewInfo.
  GetSearchInfoPanelDefaultHeight: Integer;
begin
  Result := TdxTextMeasurer.TextLineHeight(Font) + 8;
end;

function TdxCustomIncrementalFilteringContainerViewInfo.
  GetSearchInfoPanelHeight: Integer;
begin
...
function TdxCustomIncrementalFilteringContainerViewInfo.
  GetSearchInfoPanelText: string;
begin
  Result := cxGetResourceString(@cxSNoMatchesFound);
end;

function TdxCustomIncrementalFilteringContainerViewInfo.
  GetSearchInfoPanelTextDrawFlags: Cardinal;
begin
...
function TdxCustomIncrementalFilteringContainerViewInfo.
  GetSearchInfoPanelTextOffset: Integer;
begin
  Result := 4;
end;

function TdxCustomIncrementalFilteringContainerViewInfo.
  GetSearchInfoPanelWidth: Integer;
begin
  Result := TdxTextMeasurer.TextWidthTO(Font, GetSearchInfoPanelText) + 2 *
    GetSearchInfoPanelTextOffset;
end;

function TdxCustomIncrementalFilteringContainerViewInfo.
  IsSearchInfoPanelVisible: Boolean;
begin
  Result := False;
end;

procedure TdxCustomIncrementalFilteringContainerViewInfo.Paint
  (ACanvas: TcxCanvas);
begin
...
constructor TdxCustomIncrementalFilteringPopupWindow.Create
...
function TdxCustomIncrementalFilteringPopupWindow.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TdxCustomIncrementalFilteringContainerViewInfo;
end;

function TdxCustomIncrementalFilteringPopupWindow.GetViewInfo
  : TdxCustomIncrementalFilteringContainerViewInfo;
begin
  Result := inherited ViewInfo as
    TdxCustomIncrementalFilteringContainerViewInfo;
end;

procedure TdxCustomIncrementalFilteringPopupWindow.SetShowSearchInfoPanel
  (const Value: Boolean);
begin
