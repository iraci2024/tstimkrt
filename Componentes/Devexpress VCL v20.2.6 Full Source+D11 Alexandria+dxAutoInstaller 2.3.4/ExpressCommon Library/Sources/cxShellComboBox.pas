...
uses
  ...
  cxClasses, cxContainer, cxDataUtils, cxDropDownEdit, cxEdit, cxGraphics,
  cxTextEdit,
...
type
  ...
  protected
    ...
    procedure CheckRoot;
    function EditValueToPIDL(const AEditValue: TcxEditValue)
      : PItemIDList; virtual;
  ...
  public
    ...
    property DropDownSizeable: Boolean read GetDropDownSizeable
      write SetDropDownSizeable default True;
    property DropDownWidth: Integer read GetDropDownWidth write SetDropDownWidth
      default 250;
    property FastSearch: Boolean read FFastSearch write SetFastSearch
      default False;
    ...
    property StoreRelativePath: Boolean read FStoreRelativePath
      write SetStoreRelativePath default True;
  ...
  TcxCustomShellComboBox = class(TcxCustomPopupEdit, IcxShellDependedControls,
    IcxShellRoot)
  private
    FAbsolutePIDL: PItemIDList;
    FDependedControls: TcxShellDependedControls;
    FIsDependentShellControlsNotification,
      FNotificationFromDependentShellControls: Boolean;
  ...
  protected
    ...
    procedure SelectedNodeChangeHandler(Sender: TObject;
      ANode: TTreeNode); virtual;
...
class function TcxFilterShellComboBoxHelper.GetFilterEditClass
  : TcxCustomEditClass;
begin
...
begin
  ...
  else if ASHFileInfo.dwAttributes and SFGAO_SHARE <> 0 then
...
begin
  ...
  begin
    ...
    ImageList_Draw(cxShellComboBoxImageList.Handle, ShellSysIconIndex,
      ACanvas.Handle, R.Left, R.Top, ILD_TRANSPARENT or
      IndexToOverlayMask(ShellSysIconOverlayIndex + 1));
    if not Transparent then
...
procedure TcxShellComboBoxViewData.EditValueToDrawValue(const AEditValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
begin
  with TcxShellComboBoxViewInfo(AViewInfo) do
  begin
    Properties.GetShellDisplayValue(AEditValue, False, Text, ShellSysIconIndex,
      ShellSysIconOverlayIndex);
...
function TcxShellComboBoxViewData.InternalGetEditConstantPartSize
...
begin
  Properties.GetShellDisplayValue(AEditValue, Focused, Result,
    AShellSysIconIndex, AShellSysIconOverlayIndex);
end;

function TcxShellComboBoxViewData.GetProperties
  : TcxCustomShellComboBoxProperties;
begin
...
begin
  ...
  Result := InternalCompareString(AText1, AText2, True) and
    (FastSearch or (AShellSysIconIndex1 = AShellSysIconIndex2) and
    (AShellSysIconOverlayIndex1 = AShellSysIconOverlayIndex2));
end;

class function TcxCustomShellComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxShellComboBox;
end;

function TcxCustomShellComboBoxProperties.GetDisplayText(const AEditValue
...
function TcxCustomShellComboBoxProperties.GetEditValueSource
  (AEditFocused: Boolean): TcxDataEditValueSource;
begin
  Result := evsValue;
end;

class function TcxCustomShellComboBoxProperties.GetLookupDataClass
  : TcxInterfacedPersistentClass;
begin
  Result := TcxShellComboBoxLookupData;
end;

procedure TcxCustomShellComboBoxProperties.GetShellDisplayValue(const AEditValue
  : TcxEditValue; AFocused: Boolean; out AText: string;
  out AShellSysIconIndex, AShellSysIconOverlayIndex: Integer);
var
  APath: string;
        ...
        case CheckShellObjectPath(APath, GetPidlName(Root.Pidl), False) of
          sptVirtual:
            begin
              ...
              if (APIDL <> nil) and not CheckViewOptions(GetShellViewOptions,
                AAttributes) then
                APIDL := nil;
              if APIDL = nil then
                AText := APath
...
class function TcxCustomShellComboBoxProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxShellComboBoxViewInfo;
end;

function TcxCustomShellComboBoxProperties.IsEditValueValid
  (var EditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := True;
end;

class function TcxCustomShellComboBoxProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
  Result := TcxShellComboBoxPopupWindow;
end;

class function TcxCustomShellComboBoxProperties.GetViewDataClass
...
function TcxCustomShellComboBoxProperties.EditValueToPIDL(const AEditValue
  : TcxEditValue): PItemIDList;
begin
  CheckRoot;
  if VarIsNull(AEditValue) or VarIsEmpty(AEditValue) then
    Result := nil
  else
    Result := PathToAbsolutePIDL(VarToStr(AEditValue), Root,
      GetShellViewOptions);
end;

function TcxCustomShellComboBoxProperties.GetShellViewOptions
  : TcxShellViewOptions;
begin
...
procedure TcxCustomShellComboBoxProperties.SetRoot(Value: TcxShellComboBoxRoot);
begin
  FRoot.Assign(Value);
end;
...
procedure TcxCustomShellComboBoxProperties.SetTreeViewOptions
  (Value: TcxPopupShellTreeViewOptions);
begin
...
procedure TcxCustomShellComboBoxProperties.SetViewOptions
  (Value: TcxShellComboBoxViewOptions);
begin
...
procedure TcxPopupShellTreeView.MouseUp(Button: TMouseButton;
...
begin
  ...
  if ((Button = mbLeft) and not RightClickSelect) or
    ((Button = mbRight) and RightClickSelect) then
    if ([htOnIcon, htOnItem, htOnLabel] * AHitTests <> []) and
      (AHitTests <> [htOnIcon, htOnItem]) then
      DoSelectNode(crEnter);
end;

procedure TcxPopupShellTreeView.DoSelectNode(AClosePopupReason
...
class function TcxCustomShellComboBox.GetPropertiesClass
...
begin
  ...
  APIDL := FPopupShellTreeView.GetNodeAbsolutePIDL
    (TraverseAllVisibleNodes(ANodeIndex));
...
begin
  ...
  TcxPopupEditPopupWindowAccess(PopupWindow).SysPanelStyle :=
    ActiveProperties.PopupSizeable;
  PopupShellTreeView.Color := ViewInfo.BackgroundColor;
  PopupWindow.Font := VisibleFont;
end;

procedure TcxCustomShellComboBox.Notification(AComponent: TComponent;
...
begin
  ...
  if Operation = opRemove then
    if AComponent = ShellListView then
      ShellListView := nil
    else if AComponent = ShellTreeView then
      ShellTreeView := nil;
end;
...
begin
  ...
  if FPopupShellTreeView <> nil then
    ...
    begin
      ...
      Options.ContextMenus := tvoContextMenus
        in ActiveProperties.TreeViewOptions;
      ...
      Options.ShowZipFilesWithFolders := scvoShowZipFilesWithFolders
        in ActiveProperties.ViewOptions;
      RightClickSelect := tvoRightClickSelect
        in ActiveProperties.TreeViewOptions;
...
var
  ...
  S: String;
begin
  if DataBinding is TcxDBTextEditDataBinding then
  // TODO Remove cxDBEdit, IsDataSourceLive
  ...
  begin
    S := GetPIDLDisplayName(APIDL, (ActiveProperties.ShowFullPath = sfpAlways)
      or (ActiveProperties.ShowFullPath = sfpNotFocused) and not IsFocused);
    ...
    repeat
      case APathType of
        ...
        sptInternalAbsoluteVirtual, sptInternalRelativeVirtual:
          APIDL := ShellObjectInternalVirtualPathToPIDL(APath,
            ActiveProperties.Root, AShellViewOptions);
        sptRelativePhysical:
          begin
            if IsDesigning or (FPopupShellTreeView.InnerTreeView.Selected = nil)
            then
            begin
            ...
            begin
              ...
              if APath = AnsiUpperCase
                (FPopupShellTreeView.InnerTreeView.Selected.Text) then
              begin
...
function TcxCustomShellComboBox.FindNodeAmongExpandedNodes(ANodeText: string)
  : TTreeNode;
...
function TcxCustomShellComboBox.GetPopupShellTreeViewClass
  : TcxPopupShellTreeViewClass;
begin
...
procedure TcxCustomShellComboBox.InternalSynchronizeEditValue
  (APIDL: PItemIDList; const ADisplayText: string = '');
var
  APIDLName: string;
  ...
  begin
    ...
    begin
      ...
      try
        GetShellSysIconIndex(APIDL, AShellSysIconIndex1,
          AShellSysIconOverlayIndex1);
        GetShellSysIconIndex(ATempPIDL, AShellSysIconIndex2,
          AShellSysIconOverlayIndex2);
...
begin
  ...
  begin
    if ActiveProperties.StoreRelativePath and
      EqualPIDLs(ActiveProperties.Root.Pidl, APIDL) then
    begin
      ANewEditValue := '';
      S1 := GetPIDLDisplayName(APIDL,
        (ActiveProperties.ShowFullPath = sfpAlways) or
        (ActiveProperties.ShowFullPath = sfpNotFocused) and not IsFocused);
    end
    else if UseInternalVirtualPathFormat then
    begin
      ATempPIDL := APIDL;
      try
        if ActiveProperties.StoreRelativePath then
        begin
          TdxNativeInt(APIDL) := TdxNativeInt(APIDL) +
            GetPidlSize(ActiveProperties.Root.Pidl);
...
begin
  with ViewInfo.ShellIconPosition do
    InvalidateRect(Rect(X, Y, X + cxShellComboBoxImageList.Width,
      Y + cxShellComboBoxImageList.Height), False);
end;
...
begin
  ...
  if FIsSelectedNodeChangeEventLocked or
    (not FPopupWindowShowing and not DataBinding.Modified and not DoEditing)
  then
    Exit;

  APIDL := FPopupShellTreeView.GetNodeAbsolutePIDL(ANode);
  try
    DataBinding.DisplayValue := GetPIDLDisplayName(APIDL,
...
var
  ATempPIDL: PItemIDList;
begin
  if FIsDependentShellControlsNotification or FNotificationFromDependentShellControls
  then
    Exit;
...
begin
  ...
  if not IsDesigning and (FPopupShellTreeView <> nil) and
    (HasPopupWindow (* or not IsInplace *) ) then
    with FPopupShellTreeView do
      if (Parent <> nil) and Parent.HandleAllocated and (InnerTreeView <> nil)
      then
      begin
        InnerTreeView.HandleNeeded;
        SendMessage(InnerTreeView.Handle, DSM_DONAVIGATE,
          WParam(FAbsolutePIDL), 0);
      end;
end;
...
function TcxCustomShellComboBox.GetActiveProperties
  : TcxCustomShellComboBoxProperties;
begin
...
procedure TcxCustomShellComboBox.SetProperties
  (Value: TcxCustomShellComboBoxProperties);
begin
...
procedure TcxCustomShellComboBox.SetShellListView
  (Value: TcxCustomShellListView);
begin
  cxSetShellControl(Self, Value, TWinControl(FShellListView));
end;

procedure TcxCustomShellComboBox.SetShellTreeView
  (Value: TcxCustomShellTreeView);
begin
...
class function TcxShellComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
GetRegisteredEditProperties.Register(TcxShellComboBoxProperties,
  scxSEditRepositoryShellComboBoxItem);
FilterEditsController.Register(TcxShellComboBoxProperties,
  TcxFilterShellComboBoxHelper);

finalization

FreeAndNil(FcxShellComboBoxImageList);
FilterEditsController.Unregister(TcxShellComboBoxProperties,
  TcxFilterShellComboBoxHelper);
