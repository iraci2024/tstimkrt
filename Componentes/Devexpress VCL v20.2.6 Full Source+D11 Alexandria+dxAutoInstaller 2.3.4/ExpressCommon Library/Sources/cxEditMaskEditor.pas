...
const
  ...
  DefaultRegExprSamples: array [0 .. DefaultRegExprSamplesCount - 1]
    of TcxSample = ((ShortDescription: 'Time of day';
    Description
    : 'The 24 hour day time:'#13#10#13#10'15:25'#13#10'2:05'#13#10'03:57';
    Mask: '(0?\d | 1\d | 2[0-3]) : [0-5]\d';
    ), (ShortDescription: 'Time of day with seconds';
    Description
    : 'The 24 hour day time with seconds:'#13#10#13#10'12:45:10'#13#10'3:00:01';
    ...
    Description
    : 'The 12 hour day time with seconds:'#13#10#13#10'10:03:10 AM'#13#10'03:00:01 PM';
    Mask: '(0?[1-9] | 1[012]) : [0-5]\d : [0-5]\d '' '' (AM | PM)';
    ), (ShortDescription: 'Date';
    Description
    : 'The mm/dd/yy or mm/dd/yyyy date with year from 1000 to 3999:'#13#10#13#10
    ...
    ), (ShortDescription: 'Extension'; Description: 'For example 15450';
    Mask: '\d\d\d\d\d';), (ShortDescription: 'Social security';
    Description: 'For example 555-55-5555'; Mask: '\d\d\d - \d\d - \d\d\d\d';
    ), (ShortDescription: 'Short zip code'; Description: 'For example 11200';
    Mask: '\d\d\d\d\d';), (ShortDescription: 'Long zip code';
    Description: 'For example 11200-0000'; Mask: '\d\d\d\d\d - \d\d\d\d';
    ), (ShortDescription: 'Decimal number'; Description: 'Any decimal number';
    Mask: '\d+';), (ShortDescription: 'Hexadecimal number';
    Description: 'Any Hexadecimal number'; Mask: '(\d | [A-F] | [a-f])+';
    ), (ShortDescription: 'Octal number'; Description: 'Any Octal number';
    Mask: '[0-7]+';), (ShortDescription: 'Binary number';
    Description: 'Any Binary number'; Mask: '[01]+';
    ), (ShortDescription: 'Yes/No';
    Description: 'Yes, No, yes, no, Y, N, y or n';
    Mask: '(Y | y)(es)? | (N | n)o?';), (ShortDescription: 'True/False';
    Description: 'True, False, true or false'; Mask: '(T | t)rue | (F | f)alse';
    ), (ShortDescription: 'Any symbols'; Description: 'Any symbols'; Mask: '.+';
    ), (ShortDescription: 'Letters only';
    Description: 'The any letters of the latin alphabet'; Mask: '[a-zA-Z]+';
    ), (ShortDescription: 'Uppercase letters';
    Description: 'Any uppercase letters of the latin alphabet'; Mask: '[A-Z]+';
    ), (ShortDescription: 'Lowercase letters';
    Description: 'Any lowercase letters of the latin alphabet';
    Mask: '[a-z]+';));

  DefaultStandardSamplesCount = 8;
  DefaultStandardSamples: array [0 .. DefaultStandardSamplesCount - 1]
    of TcxSample = ((ShortDescription: 'Phone'; Description: '(213)144-1756';
    Mask: '!\(999\)000-0000;1;_';), (ShortDescription: 'Extension';
    Description: '15023'; Mask: '!99999;1;_';
    ), (ShortDescription: 'Social security'; Description: '555-55-5555';
    Mask: '000\-00\-0000;1;_';), (ShortDescription: 'Short zip code';
    Description: '90628'; Mask: '00000;1;_';
    ), (ShortDescription: 'Long zip code'; Description: '90628-0000';
    Mask: '00000\-9999;1;_';), (ShortDescription: 'Date';
    Description: '03/24/99'; Mask: '!99/99/00;1;_';
    ), (ShortDescription: 'Long Time'; Description: '04:15:34 PM';
    Mask: '!90:00:00> LL;1;_';), (ShortDescription: 'Short Time';
    Description: '21:45'; Mask: '!90:00;1;_';));
...
type
  ...
  private
    ...
    procedure FormatStandardMask(var AMask: string;
      ANewMaskBlank: Char); overload;
    procedure FormatStandardMask(var AMask: string;
      ANewSaveLiteralChars: Boolean); overload;
...
function TcxEditMaskEditorDlg.GetSaveLiteralChars(const AEditMask
  : string): Boolean;
var
  ALen: Integer;
begin
...
function TcxEditMaskEditorDlg.GetSaveLiteralChartSymbol
...
begin
  ...
  try
      ...
      begin
        ...
        begin
          if not((AStringNumber = 0) and (AShortDescription = '')) then
            raise EcxLoadMaskException.Create
              (cxGetResourceString(@scxMaskEditIllegalFileFormat));
        end;
      end
      else if AToken = '|' then
      begin
      ...
      begin
        ...
        begin
          case AStringNumber of
            0:
              AShortDescription := AShortDescription + AToken;
            1:
              ADescription := ADescription + AToken;
            2:
              AMask := AMask + AToken;
          end;
        end;
    ...
    begin
      ...
      raise EcxLoadMaskException.Create
        (cxGetResourceString(@scxMaskEditEmptyMaskCollectionFile));
  finally
    AMemoryStream.Free;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        AShortDescription := AFile.ReadString(ASections[I],
          'ShortDescription', '');
        ADescription := AFile.ReadString(ASections[I], 'Description', '');
        AMask := AFile.ReadString(ASections[I], 'Mask', '');
        if (AShortDescription = '') and (ADescription = '') and (AMask = '')
        then
          Continue;
...
begin
  ...
  ADialog.Options := [];
  ADialog.Filter := cxGetResourceString(@scxMaskEditMaskCollectionFiles) +
    ' (*.dem, *.dxm )|*.dem;*.dxm';
  try
...
procedure TcxEditMaskEditorDlg.FormatStandardMask(var AMask: string;
  ANewMaskBlank: Char);
var
  AOldMaskBlank: Char;
...
begin
  ...
  begin
    ...
    begin
      if (AMaskLength = 2) and (AMask[1] = ';') and
        ((AMask[2] = '0') or (AMask[2] = '1')) then
        AMask := AMask + ';' + ANewMaskBlank
      else
        AMask := AMask + ';' + GetSaveLiteralChartSymbol(ASaveLiteralChars) +
          ';' + ANewMaskBlank;
    end
    else
    begin
      ...
      else if (AMask[AMaskLength - 1] = ';') and
        ((AMask[AMaskLength] = '0') or (AMask[AMaskLength] = '1')) then
        AMask := AMask + ';' + ANewMaskBlank
      else
        AMask := AMask + ';' + GetSaveLiteralChartSymbol(ASaveLiteralChars) +
          ';' + ANewMaskBlank;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Insert(GetSaveLiteralChartSymbol(ANewSaveLiteralChars), AMask,
          AMaskLength - 2);
      end
      else if (AMask[AMaskLength - 1] = ';') and
        ((AMask[AMaskLength] = '0') or (AMask[AMaskLength] = '1')) then
...
procedure TcxEditMaskEditorDlg.cxTextEdit2PropertiesChange(Sender: TObject);
var
  AMask: string;
begin
  ConvertMaskBlank;
...
procedure TcxEditMaskEditorDlg.cxCheckBox1PropertiesChange(Sender: TObject);
var
  AMask: string;
begin
  AMask := cxTextEdit1.Text;
