...
type
  ...
  protected
    ...
    property SubclassControl: TWinControl read FSubclassControl
      write SetSubclassControl;
    property TabStop default False;
  public
    destructor Destroy; override;
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    function Increment(AButton: TcxSpinEditButton): Boolean; override;
    property ActiveProperties: TcxCustomSpinButtonProperties
...
procedure TcxSpinButtonViewData.Calculate(ACanvas: TcxCanvas;
...
class function TcxCustomSpinButtonProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxSpinButton;
end;

class function TcxCustomSpinButtonProperties.GetViewDataClass
...
class function TcxCustomSpinButton.GetPropertiesClass
...
const
  AVarTypeMap: array [TcxSpinEditValueType] of TVarType = (varInteger,
    varDouble);
begin
  PrepareEditValue(Text, Result, InternalFocused);
  if not VarIsNumeric(Result) then
    Result := VarAsType(ActiveProperties.MinValue,
      AVarTypeMap[ActiveProperties.ValueType]);
end;

procedure TcxCustomSpinButton.InternalSetDisplayValue
...
procedure TcxCustomSpinButton.InternalSetEditValue(const Value: TcxEditValue;
...
begin
  ...
  with ViewInfo do
  begin
    AWidth := (BorderRect.Right - BorderRect.Left) -
      (TextRect.Right - TextRect.Left);
    if UseSkins then
      Dec(AWidth)
    else if Length(ButtonsInfo) > 0 then
...
begin
  if Assigned(FSubclassControl) and
    ...
    FPrevAssociateControlWndProcObject := cxWindowProcController.Add
      (FSubclassControl, AssociateControlWndProc);
end;
...
var
  ...
  begin
    ...
    begin
      if ClassType.ClassNameIs(Name) then
        Exit;
...
begin
  if AValue <> nil then
    for I := 0 to Parent.ControlCount - 1 do { Is control already associated ? }
      if (Parent.Controls[I] is TcxCustomSpinButton) and
        (Parent.Controls[I] <> Self) then
        if TcxCustomSpinButton(Parent.Controls[I]).Associate = AValue then
          raise EcxEditError.Create
            (AValue.Name + cxGetResourceString(@scxUDAssociated) +
            Parent.Controls[I].Name);
  ...
  if (AValue <> nil) and (AValue.Parent = Self.Parent) and
    not(AValue is TcxCustomSpinButton) and not IsClass(AValue.ClassType,
    'TcxDBTextEdit') and not IsClass(AValue.ClassType, 'TcxDBMemo') and
...
procedure TcxCustomSpinButton.SetProperties
  (Value: TcxCustomSpinButtonProperties);
begin
