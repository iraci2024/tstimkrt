...
type
  TdxServerModeDataSourceEditor = class(TcxDesignFormEditor)
    ...
    procedure lbMainStartDrag(Sender: TObject; var DragObject: TDragObject);
...
procedure dxShowServerModeDataSourceEditor(ADesigner: IDesigner;
...
procedure TdxServerModeDataSourceEditor.SelectionsChanged(const ASelection
  ...
  try
    ...
    try
      ...
      for I := 0 to DataSource.Fields.Count - 1 do
        if TdxServerModeCustomDataSourceAccess(DataSource)
          .IsPersistentField(DataSource.Fields[I]) then
          lbMain.Items.AddObject(DataSource.Fields[I].FieldName,
            DataSource.Fields[I]);
    finally
    ...
    for I := 0 to DataSet.FieldDefList.Count - 1 do
      with DataSet.FieldDefList[I] do
        ...
        begin
          ...
          if (AField = nil) or not TdxServerModeCustomDataSourceAccess
            (DataSource).IsPersistentField(AField) then
...
begin
  for I := 0 to lbMain.Items.Count - 1 do
    if lbMain.Selected[I] and (csAncestor in TField(lbMain.Items.Objects[I])
      .ComponentState) then
...
function TdxServerModeDataSourceEditor.CreateFields
  (AFieldsList: TListBox): TField;
var
  I: Integer;
  AItemIndex, ATopIndex: Integer;
...
begin
  ...
  try
    ...
    try
      TdxServerModeCustomDataSourceAccess(DataSource)
...
function TdxServerModeDataSourceEditor.DoCreateField(const AFieldName
  : string): TField;
var
  AFieldDef: TFieldDef;
begin
  AFieldDef := DataSet.FieldDefList.FieldByName(AFieldName);
  Result := TdxServerModeCustomDataSourceAccess(DataSource)
    .CreateField(AFieldDef, DataSource.Owner);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Result.Name := TdxServerModeCustomDataSourceAccess(DataSource)
        .GetFieldUniqueName(Result);
      Designer.Modified;
    end;
  finally
...
procedure TdxServerModeDataSourceEditor.Reindex(AList: TList;
  ANewIndex: Integer);
var
  I: Integer;
begin
...
function TdxServerModeDataSourceEditor.GetDataSource
...
procedure TdxServerModeDataSourceEditor.miSelectAllClick(Sender: TObject);
begin
  lbMain.SelectAll;
end;
...
procedure TdxServerModeDataSourceEditor.lbMainDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  ListBoxDragOver(lbMain, Sender, Source, X, Y, State, Accept, FPrevDragIndex);
end;

procedure TdxServerModeDataSourceEditor.lbMainDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
  ListBoxDragDrop(lbMain, Sender, Source, X, Y, FPrevDragIndex, Reindex);
end;

procedure TdxServerModeDataSourceEditor.lbMainEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
