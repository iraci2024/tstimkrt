...
type
  ...
  public
    constructor Create(AEdit: TcxCustomMaskEdit;
      AProperties: TcxCustomMaskEditProperties); virtual;
    ...
    function GetFormattedText(const AText: string;
    ...
    function GetUpdatedText(const AText: string;
      AMatchForBlanksAndLiterals: Boolean = True): string; virtual; abstract;
    property ClipboardTextLength: Integer read FClipboardTextLength
      write FClipboardTextLength;
  end;
  ...
  public
    constructor Create(AEdit: TcxCustomMaskEdit;
      AProperties: TcxCustomMaskEditProperties); override;
  ...
  protected
    ...
    function GetModeClass(AMaskKind: TcxEditMaskKind)
      : TcxMaskEditCustomModeClass; virtual;
  public
  ...
  public
    ...
    property ActiveProperties: TcxCustomMaskEditProperties
...
begin
  ...
  begin
    if FEdit.ModifiedAfterEnter then
      ...
      FEdit.ActiveProperties.PrepareDisplayValue(FEdit.EditValue, ADisplayValue,
        FEdit.Focused);
...
begin
  if (FEdit.SelStart < Length(FEdit.Text)) and
    (FMask.Items[FEdit.SelStart] is TcxStandardMaskLiteralItem) then
  begin
...
function TcxMaskEditStandardMode.GetFormattedText(const AText: string;
  ...
  begin
    ...
    begin
      ...
      if J = FMask.Count then
        Break;
      while (FMask.Items[J] is TcxStandardMaskLiteralItem) and
        not FMask.Items[J].Check(ASymbol) do
      begin
        Inc(J);
        Inc(ANewSelStart);
      end;
...
function TcxMaskEditRegExprMode.GetFormattedText(const AText: string;
  AMatchForBlanksAndLiterals: Boolean = True): string;
var
  I: Integer;
  ATextCopy: string;
...
begin
  ...
  begin
    if FEdit.SelLength > 0 then
    begin
      if (FEdit.CursorPos = FEdit.SelStart + FEdit.SelLength) and not FEdit.FShiftOn
      then
      begin
...
begin
  ...
  begin
    ...
    FTail := Copy(AText, FEdit.SelStart + ClipboardTextLength + 1,
      Length(AText));
...
begin
  Result := FMouseAction or (FEdit.Text <> '') and (FHead = '') and
...
function TcxMaskEditRegExprExMode.GetFormattedText(const AText: string;
  AMatchForBlanksAndLiterals: Boolean = True): string;
begin
  if not FRegExpr.IsCompiled then
  begin
...
begin
  ...
  if FRegExpr.IsStart then
    ...
    else if FEdit.SelStart = FDeleteNumber then
      Dec(FDeleteNumber);
...
begin
  ...
  begin
    ...
    else if (FEdit.CursorPos = FEdit.SelStart + FEdit.SelLength) and not FEdit.FShiftOn
    then
      Exit;
  end;
...
procedure TcxMaskEditRegExprExMode.CursorCorrection;
  ...
  begin
    ...
    begin
      ...
      FTail := Copy(FHead, Length(FHead) - FDeleteNumber,
        FDeleteNumber + 1) + FTail;
...
begin
  ...
  if ASelStart > Length(FHead) then
    ...
    begin
      ...
      else if ASelStart = Length(FHead) then
        Break;
    end
  else if ASelStart < Length(FHead) then
    while True do
...
function TcxCustomMaskEditProperties.GetSupportedOperations
...
function TcxCustomMaskEditProperties.IsEditValueValid(var EditValue
...
begin
  Result := not EmptyMask(FEditMask) and
    ((IDefaultValuesProvider = nil) or
    (IDefaultValuesProvider.DefaultDisplayFormat = '') and
    not IDefaultValuesProvider.IsOnGetTextAssigned and
    ((FMaskKind <> emkStandard) or
    (FEditMask <> IDefaultValuesProvider.DefaultEditMask)));
end;

procedure TcxCustomMaskEditProperties.DoPrepareDisplayValue(const AEditValue
...
procedure TcxCustomMaskEditProperties.ValidateDisplayValue(var ADisplayValue
...
begin
  ...
  begin
    ...
    begin
      AError := not TcxCustomMaskEdit(AEdit).Mode.IsFullValidText
        (VarToStr(ADisplayValue));
      if AError then
...
procedure TcxCustomMaskEditProperties.DoAssign(AProperties
...
procedure TcxCustomMaskEditProperties.SetAlwaysShowBlanksAndLiterals
  (AValue: Boolean);
begin
...
function TcxCustomMaskEditProperties.GetDisplayFormatOptions
...
function TcxCustomMaskEditProperties.GetModeClass(AMaskKind: TcxEditMaskKind)
...
function TcxCustomMaskEditProperties.SupportsMultiThreading(AUseDisplayValue
  : Boolean): Boolean;
begin
  Result := inherited SupportsMultiThreading(AUseDisplayValue) and
    not(AUseDisplayValue and IsMasked and (MaskKind in [emkRegExpr,
    emkRegExprEx]));
end;
...
class function TcxCustomMaskEdit.GetPropertiesClass
...
begin
  ...
  begin
    ...
    else if not Mode.PressDelete then
      Key := 0;
  end
...
begin
  if not ActiveProperties.IsMasked then
    inherited
  else if (Key = dxVKTab) or (Key = dxVKEscape) or not ValidateKeyPress(Key)
  then
    Key := #0
  else
  begin
    ...
    begin
      ...
      begin
        if dxCharInSet(Key, [dxVKCopy, dxVKPaste, dxVKCut{$IFDEF DELPHI16},
          dxVKSelectAll{$ENDIF}]) then
          inherited
        else if (Key = dxVKBack) and not Mode.PressBackSpace or
          (Key <> dxVKBack) and not Mode.PressSymbol(Key) then
          Key := #0;
      end;
    end;
  end;
...
begin
  ...
  FMode := ActiveProperties.GetModeClass(emkStandard)
    .Create(Self, ActiveProperties);
  FShiftOn := False;
...
procedure TcxCustomMaskEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if ActiveProperties.IsMasked then
...
procedure TcxCustomMaskEdit.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
var
  ...
  begin
    ...
    begin
      ...
      FMode := ActiveProperties.GetModeClass(emkRegExpr)
        .Create(Self, ActiveProperties)
    end
    ...
    begin
      ...
      FMode := ActiveProperties.GetModeClass(emkRegExprEx)
        .Create(Self, ActiveProperties)
    end
...
begin
  ...
  AEditMaskChanged := (FMode.EditMask <> ActiveProperties.EditMask) or
    ActiveProperties.FormatChanging;
  ...
  if not(not PropertiesChangeLocked and VarIsNull(EditValue)) and
    ActiveProperties.IsMasked then
...
begin
  if ActiveProperties.IsMasked then
    if not AEditFocused and (Mode is TcxMaskEditStandardMode) and
      (TcxMaskEditStandardMode(Mode).FMask.SaveLiteralCharacters or
      not ActiveProperties.AlwaysShowBlanksAndLiterals) then
...
function TcxCustomMaskEdit.CanChangeSelText(const Value: string;
  out ANewText: string; out ANewSelStart: Integer): Boolean;
begin
  Result := (ActiveProperties.IsMasked and Mode.ProcessSelText(Value, ANewText,
    ANewSelStart)) or inherited CanChangeSelText(Value, ANewText, ANewSelStart);
end;
...
GetRegisteredEditProperties.Register(TcxMaskEditProperties,
  scxSEditRepositoryMaskItem);
FilterEditsController.Register(TcxCustomMaskEditProperties,
  TcxFilterMaskEditHelper);

finalization

FilterEditsController.Unregister(TcxCustomMaskEditProperties,
  TcxFilterMaskEditHelper);
