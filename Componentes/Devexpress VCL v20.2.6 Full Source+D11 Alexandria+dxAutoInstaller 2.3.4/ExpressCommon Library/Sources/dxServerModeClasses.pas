...
type
  ...
  public
    ...
    destructor Destroy; override;
    function Add(AKey: Integer; const AValue: Variant)
      : TdxServerModeIntDictionaryItem;
    procedure Clear;
  ...
  public
    constructor Create(ATableSize: Integer = dxDefaultDictionaryHashTableSize);
    destructor Destroy; override;
    procedure Add(const AKey: Variant;
      const AValue: TdxServerModeIntDictionaryItem);
    procedure Clear;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure Add(AFirstRecord, ACount: Integer;
      AIsFromBottom: Boolean); overload;
    procedure Add(AStart, AFinish: Integer); overload;
    procedure Clear;

    procedure GetClosestPopulatedIndexes(AIndex: Integer;
      out AStartIndex, AFinishIndex: Integer);
  ...
  public
    ...
    function TryGetKeyByValue(const AValue: Variant; out AKey: Integer)
      : Boolean;

    property LoadedRecordsHelper: TdxLoadedRecordsHelper
      read FLoadedRecordsHelper;
  end;
  ...
  public
    ...
    function TryGetValue(const AIndex: TdxServerModeRow;
  ...
  public
    constructor Create(AParent: TdxServerModeGroupInfo;
      const AGroupValue: Variant; ALevel, ATopRecordIndex: Integer);
    destructor Destroy; override;
  ...
  public
    ...
    property Items[Index: Integer]: TdxServerModeGroupInfo
  ...
  public
    constructor Create(const AGroupValue: Variant; AChildDataRowCount: Integer;
      const ASummary: TdxServerModeRow); overload;
    constructor Create(const AGroupValue: Variant;
      const ASummary: TdxServerModeRow); overload;
  ...
  public
    property Items[Index: Integer]: TdxServerModeGroupInfoData
      read GetItem; default;
  end;
...
procedure TdxServerModeRowList.Insert(Index: Integer;
  const Value: TdxServerModeRow);
begin
  Count := Count + 1;
  if Index < Count - 1 then
    System.Move(FList[Index], FList[Index + 1], (Count - 1 - Index) *
      SizeOf(Variant));
  FList[Index] := Value;
end;
...
constructor TdxServerModeIntDictionaryItem.Create(AKey: Integer;
  const AValue: TdxServerModeRow);
begin
  inherited Create;
  FKey := AKey;
  FValue := AValue;
...
constructor TdxCustomServerModeIntDictionary.Create
  (ATableSize: Integer = dxDefaultDictionaryHashTableSize);
begin
  inherited Create;
  FCount := 0;
...
function TdxCustomServerModeIntDictionary.Add(AKey: Integer;
  const AValue: Variant): TdxServerModeIntDictionaryItem;
var
  AIndex: Integer;
...
function TdxCustomServerModeIntDictionary.TryGetValue(AKey: Integer;
...
function TdxCustomServerModeIntDictionary.Find(AKey: Integer)
  : TdxServerModeIntDictionaryItem;
var
  AIndex: Integer;
  AItem: TdxServerModeIntDictionaryItem;
...
constructor TdxServerModeLinkedDictionaryItem.Create(AKey: Cardinal;
  AValue: TdxServerModeIntDictionaryItem);
begin
  inherited Create;
  FKey := AKey;
  FValue := AValue;
...
constructor TdxServerModeLinkedDictionary.Create
  (ATableSize: Integer = dxDefaultDictionaryHashTableSize);
begin
  inherited Create;
  FCount := 0;
  FTableSize := ATableSize;
  SetLength(FTable, FTableSize);
  FillChar(FTable[0], FTableSize *
    SizeOf(TdxServerModeLinkedDictionaryItem), 0);
end;
...
procedure TdxServerModeLinkedDictionary.Add(const AKey: Variant;
  const AValue: TdxServerModeIntDictionaryItem);
var
  AIndex: Integer;
...
begin
  ...
  if AItem = nil then
    ...
    repeat
      if AItem.Next = nil then
      begin
        AItem.FNext := TdxServerModeLinkedDictionaryItem.Create
          (AHashKey, AValue);
        Break;
      end
      else
...
function TdxServerModeLinkedDictionary.ContainsKey(const AKey: Variant)
  : Boolean;
begin
  Result := Find(AKey) <> nil;
end;

function TdxServerModeLinkedDictionary.TryGetValue(const AKey: Variant;
  out AValue: Integer): Boolean;
var
  AItem: TdxServerModeLinkedDictionaryItem;
begin
...
function TdxServerModeLinkedDictionary.Find(const AKey: Variant)
  : TdxServerModeLinkedDictionaryItem;
var
  AIndex: Integer;
  AHashKey: Cardinal;
...
constructor TdxServerModeCustomDictionary.Create
  (ATableSize: Integer = dxDefaultDictionaryHashTableSize);
begin
  inherited Create;
...
procedure TdxServerModeCustomDictionary.Add(AKey: Integer;
...
function TdxServerModeCustomDictionary.GetKeyByValue(const AValue: Variant;
  out AKey: Integer): Boolean;
begin
  Result := FLinkedDictionary.TryGetValue(AValue, AKey);
end;

function TdxServerModeCustomDictionary.TryGetValue(AKey: Integer;
  out AValue: Variant): Boolean;
begin
...
function TdxLoadedRecordsBlock.GetStatus(AStart, AFinish: Integer)
  : TdxBlockRelation;
begin
  if (Start > AFinish + 1) or (Finish < AStart - 1) then
    Result := brNone
  else if (AFinish = Finish) and (AStart = Start) then
    Result := brEquals
  else if (Finish >= AFinish) and (Start <= AStart) then
    Result := brGreater
  else if (Finish <= AFinish) and (Start >= AStart) then
    Result := brLess
  else if Start = AFinish + 1 then
    Result := brPrev
  else if Finish = AStart - 1 then
    Result := brNext
  else if (AFinish + 1 > Start) and (AStart - 1 < Start) then
...
procedure TdxLoadedRecordsHelper.Add(AFirstRecord, ACount: Integer;
...
procedure TdxLoadedRecordsHelper.GetClosestPopulatedIndexes(AIndex: Integer;
  out AStartIndex, AFinishIndex: Integer);
var
  I: Integer;
  AItem: TdxLoadedRecordsBlock;
...
function TdxLoadedRecordsHelper.FindPosition(AStart: Integer;
...
begin
  ...
  while Count > I do
  begin
    AStatus := Items[I].GetStatus(Items[AItemIndex].Start,
      Items[AItemIndex].Finish);
    if AStatus = brNone then
      Break;
    if (AStatus in [brPrev, brNext, brIntersectStart, brIntersectFinish,
      brGreater]) and (Items[AItemIndex].Finish < Items[I].Finish) then
...
constructor TdxServerModeKeysDictionary.Create
  (ATableSize: Integer = dxDefaultDictionaryHashTableSize);
begin
...
function TdxServerModeKeysDictionary.TryGetKeyByValue(const AValue: Variant;
  out AKey: Integer): Boolean;
begin
...
procedure TdxServerModeLoadedKeysChunkDictionary.Add(const AIndex
  : TdxServerModeRow; AValue: Integer);
begin
  inherited Add(AValue, AIndex);
end;

function TdxServerModeLoadedKeysChunkDictionary.ContainsKey
  (const AKey: TdxServerModeRow): Boolean;
var
  AIndex: Integer;
begin
...
function TdxServerModeLoadedKeysChunkDictionary.TryGetValue
  (const AIndex: TdxServerModeRow; out AValue: Integer): Boolean;
begin
...
constructor TdxServerModeGroupInfo.Create(AParent: TdxServerModeGroupInfo;
  const AGroupValue: Variant; ALevel, ATopRecordIndex: Integer);
begin
...
function TdxServerModeGroupInfo.IsEqual(const AValue: Variant;
  AHash: Cardinal): Boolean;

  function CanUseHash(VType1, VType2: TVarType): Boolean; inline;
  begin
    Result := (VType1 = VType2) or
      (((VType1 = varOleStr) or (VType1 = varUString)) and
...
function TdxServerModeGroupInfoList.GetItem(Index: Integer)
  : TdxServerModeGroupInfo;
begin
...
constructor TdxServerModeGroupInfoData.Create(const AGroupValue: Variant;
  AChildDataRowCount: Integer; const ASummary: TdxServerModeRow);
begin
  inherited Create;
  FGroupValue := AGroupValue;
...
constructor TdxServerModeGroupInfoData.Create(const AGroupValue: Variant;
  const ASummary: TdxServerModeRow);
begin
...
function TdxServerModeGroupInfoDataList.GetItem(Index: Integer)
  : TdxServerModeGroupInfoData;
begin
