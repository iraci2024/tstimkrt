...
type
  ...
  protected
    ...
    property PictureClipboardFormat: Word read FPictureClipboardFormat
  ...
  public
    ...
    property PictureGraphicClass: TGraphicClass read FPictureGraphicClass
      write SetPictureGraphicClass;
    // !!!
    // Common
    property AlwaysSaveData: Boolean read FAlwaysSaveData write FAlwaysSaveData
      default True;
    property BlobEditKind: TcxBlobEditKind read FBlobEditKind
      write SetBlobEditKind default bekAuto;
    property BlobPaintStyle: TcxBlobPaintStyle read FBlobPaintStyle
      write SetBlobPaintStyle default bpsIcon;
    property ImmediateDropDownWhenActivated default True;
    // Memo
    property MemoAutoReplace: Boolean read FMemoAutoReplace
      write FMemoAutoReplace default False;
    property MemoCharCase: TEditCharCase read FMemoCharCase write FMemoCharCase
      default ecNormal;
    property MemoMaxLength: Integer read FMemoMaxLength write FMemoMaxLength
      default 0;
    property MemoOEMConvert: Boolean read FMemoOEMConvert write FMemoOEMConvert
      default False;
    property MemoScrollBars: TcxScrollStyle read FMemoScrollBars
      write FMemoScrollBars default ssNone;
    property MemoWantReturns: Boolean read FMemoWantReturns
      write FMemoWantReturns default True;
    property MemoWantTabs: Boolean read FMemoWantTabs write FMemoWantTabs
      default True;
    property MemoWordWrap: Boolean read FMemoWordWrap write FMemoWordWrap
      default True;
    // Picture
    property PictureAutoSize: Boolean read FPictureAutoSize
      write FPictureAutoSize default True;
    property PictureFilter: string read FPictureFilter write FPictureFilter;
    property PictureGraphicClassName: string read GetPictureGraphicClassName
      write SetPictureGraphicClassName stored IsPictureGraphicClassNameStored;
    property PictureTransparency: TcxImageTransparency read FPictureTransparency
      write FPictureTransparency default gtDefault;
    property ShowExPopupItems: Boolean read FShowExPopupItems
      write FShowExPopupItems default True;
    property ShowPicturePopup: Boolean read FShowPicturePopup
      write FShowPicturePopup default True;
  ...
  private
    ...
    procedure PictureClosePopup(Sender: TcxControl;
  ...
  protected
    ...
    function GetPictureGraphicClass(APastingFromClipboard: Boolean = False)
      : TGraphicClass;
    procedure StorageEditingHandler(Sender: TObject;
...
uses
  Variants, Math, dxTypeHelpers, cxDrawTextUtils, cxEditUtils, cxGeometry,
  cxVariants, dxCore, dxThemeConsts,
  cxLookAndFeelPainters, dxDPIAwareUtils, dxCoreGraphics;
...
begin
  ...
  if AFullText and (AProperties.BlobEditKind = bekMemo) and VarIsStr(Value) then
    Result := Value
  else if AProperties.BlobPaintStyle = bpsDefault then
  begin
  ...
  else if (AProperties.BlobPaintStyle = bpsText) and
...
begin
  inherited Calculate(ACanvas, ABounds, P, Button, Shift, AViewInfo,
    AIsMouseEvent);
  TcxBlobEditViewInfo(AViewInfo).IconRect := cxRectCenter(AViewInfo.ClientRect,
    dxGetImageSize(GetBlobImages, ScaleFactor));
  if Edit <> nil then
    EditValueToDrawValue(Edit.EditValue, AViewInfo);
  PrepareDrawTextFlags(ACanvas, AViewInfo);
end;

procedure TcxBlobEditViewData.EditValueToDrawValue(const AEditValue
...
function TcxBlobEditViewData.InternalGetEditConstantPartSize(ACanvas: TcxCanvas;
  AIsInplace: Boolean; AEditSizeProperties: TcxEditSizeProperties;
...
begin
  Result := GetBlobText(AEditValue,
    TcxCustomBlobEditProperties(Properties), False);
end;
...
begin
  ...
  if not IsInplace or (epoShowEndEllipsis in PaintOptions) then
    AEditViewInfo.DrawTextFlags := AEditViewInfo.DrawTextFlags or
      CXTO_END_ELLIPSIS;
end;
...
begin
  ...
  if (AProperties.BlobEditKind = bekMemo) and
    (AProperties.BlobPaintStyle = bpsText) then
    Result := inherited NeedShowHint(ACanvas, P, AVisibleBounds, AText,
      AIsMultiLine, ATextRect, AMaxLineCount)
  else
    Result := False;
end;
...
function TcxBlobEditViewInfo.GetBackgroundPaintingStyle
  ...
  begin
    ...
    begin
      ...
      try
        ...
        cxDrawImage(ACanvas.Handle, IconRect, IconRect, nil, GetBlobImages,
          ImageIndex, EnabledImageDrawModeMap[Enabled], True, 0, clDefault,
          False, nil);
      finally
...
procedure TcxCustomBlobEditProperties.ReadIsPictureGraphicClassNameEmpty
  (Reader: TReader);
begin
  Reader.ReadBoolean;
...
procedure TcxCustomBlobEditProperties.SetPictureGraphicClass
  (Value: TGraphicClass);
begin
...
procedure TcxCustomBlobEditProperties.SetPictureGraphicClassName
  (const Value: string);
var
...
procedure TcxCustomBlobEditProperties.WriteIsPictureGraphicClassNameEmpty
  (Writer: TWriter);
begin
...
begin
  ...
  Filer.DefineProperty('IsPictureGraphicClassNameEmpty',
    ReadIsPictureGraphicClassNameEmpty, WriteIsPictureGraphicClassNameEmpty,
    PictureGraphicClassName = '');
end;
...
function TcxCustomBlobEditProperties.GetDisplayFormatOptions
  : TcxEditDisplayFormatOptions;
begin
  Result := [];
end;

class function TcxCustomBlobEditProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
  Result := TcxBlobEditPopupWindow;
end;

class function TcxCustomBlobEditProperties.GetViewDataClass
...
const
  ABlobEditKindCorrectionA: array [TcxBlobKind] of TcxBlobEditKind = (bekBlob,
    bekBlob, bekPict, bekMemo, bekOle);
begin
  if (BlobEditKind = bekAuto) and not((IDefaultValuesProvider <> nil) and
    not IDefaultValuesProvider.IsDataAvailable) then
  begin
    LockUpdate(True);
    try
      if IDefaultValuesProvider <> nil then
        BlobEditKind := ABlobEditKindCorrectionA
          [IDefaultValuesProvider.DefaultBlobKind]
      else
...
function TcxCustomBlobEditProperties.GetDefaultPictureGraphicClass
  : TGraphicClass;
begin
...
function TcxCustomBlobEditProperties.CompareDisplayValues(const AEditValue1,
...
function TcxCustomBlobEditProperties.GetDisplayText(const AEditValue
...
function TcxCustomBlobEditProperties.GetEditValueSource(AEditFocused: Boolean)
  : TcxDataEditValueSource;
begin
  CorrectBlobEditKind;
  if BlobEditKind <> bekMemo then
    Result := evsValue
  else if (IDefaultValuesProvider <> nil) and IDefaultValuesProvider.IsBlob then
    Result := evsValue
  else
    Result := evsText;
end;
...
function TcxCustomBlobEditProperties.GetSupportedOperations
...
class function TcxCustomBlobEditProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxBlobEditViewInfo;
end;

function TcxCustomBlobEditProperties.IsEditValueValid(var AEditValue
...
begin
  ...
  else if (TranslateKey(Key) = VK_RETURN) and
    (AReturnClosed or (ssCtrl in Shift)) then
    Result := mrOk;
...
begin
  ...
  if Key = 0 then
    dxMessagesController.KillMessages(InnerControl.Handle, WM_CHAR)
    // to block Ctrl+Enter
  else
...
procedure TcxCustomBlobEdit.PictureClosePopup(Sender: TcxControl;
  AReason: TcxEditCloseUpReason);
begin
...
procedure TcxCustomBlobEdit.SaveStorage;
  ...
  begin
    Result := FStorage.EditModified and
      ((PopupWindow.ModalResult = mrOk) or
      ((PopupWindow.ModalResult <> mrCancel) and
      ActiveProperties.AlwaysSaveData)) and DoEditing;
  end;
...
procedure TcxCustomBlobEdit.CreatePopupControls;
  ...
  procedure InitButton(var AButton: TcxButton; const ACaption: string;
  ...
  begin
    ...
    FButtonWidth := Max(cxTextWidth(AFont, ACancelCaption + '00'),
      cxTextWidth(AFont, AOkCaption + '00'));
    ...
    ActiveProperties.PopupMinWidth := ActiveProperties.ScaleFactor.Apply
      (ATempWidth + dxGetSystemMetrics(SM_CXVSCROLL, ScaleFactor) +
      AButtonHeight div 2, ScaleFactor);
  end;
  ...
  case ActiveProperties.BlobEditKind of
    bekPict:
      begin
        ...
        AImageProperties.GraphicTransparency :=
          ActiveProperties.PictureTransparency;
        ...
        else if not ActiveProperties.ShowExPopupItems then
          AImageProperties.PopupMenuLayout.MenuItems :=
        ...
        AImageProperties.OnGetGraphicClass :=
          DoPopupImagePropertiesGetGraphicClass;
        LoadPicture(StorageImage.Picture, GetPictureGraphicClass, EditValue);
        StorageImage.EditModified := False;
        StorageImage.Style.LookAndFeel.MasterLookAndFeel :=
          PopupControlsLookAndFeel;
    ...
    bekMemo:
      begin
        ...
        StorageMemo.Style.LookAndFeel.MasterLookAndFeel :=
          PopupControlsLookAndFeel;
...
begin
  if HasPopupWindow and (ActiveProperties.BlobEditKind in [bekMemo, bekPict])
  then
    if StorageMemo <> nil then
      Result := StorageMemo.Text
    else
...
procedure TcxCustomBlobEdit.InternalValidateDisplayValue(const ADisplayValue
  : TcxEditValue);
begin
  if StorageMemo <> nil then
...
function TcxCustomBlobEdit.GetPictureGraphicClass(APastingFromClipboard
  : Boolean = False): TGraphicClass;
begin
  if IsInplace then
...
begin
  ...
  if HasPopupWindow and (StorageImage <> nil) then
    StorageImage.ActiveProperties.GraphicClass :=
...
begin
  ...
  if (PopupWindow.ViewInfo.SizeGripCorner in [coTopRight, coBottomRight]) xor UseRightToLeftAlignment
  then
  ...
  begin
    ACancelButtonBounds.Top := R.Top + (PopupWindow.ViewInfo.SysPanelHeight -
      AHeight) div 2 - 2;
    AAnchors := [akTop];
  end
  else
  begin
    ACancelButtonBounds.Top := R.Bottom -
      (AHeight + (PopupWindow.ViewInfo.SysPanelHeight - AHeight) div 2 - 2);
    AAnchors := [akBottom];
  end;
  ACancelButtonBounds.Width := FButtonWidth;
  ACancelButtonBounds.Height := AHeight;

  AOKButtonBounds.InitSize(ACancelButtonBounds.Left -
    (FButtonWidth + (AHeight div 3)), ACancelButtonBounds.Top,
    FButtonWidth, AHeight);

  if UseRightToLeftAlignment then
  begin
    AOKButtonBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (AOKButtonBounds, PopupWindow.ClientRect);
    ACancelButtonBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (ACancelButtonBounds, PopupWindow.ClientRect);
...
class function TcxCustomBlobEdit.GetPropertiesClass
...
begin
  ...
  if (AProperties is TcxBlobEditProperties) and
    ...
    Result := Result + [fcoLike, fcoNotLike, fcoContains, fcoNotContains,
      fcoBeginsWith, fcoEndsWith];
end;
...
const
  ImageNames: array [0 .. 7] of string = (cxbmBlobNull, cxbmBlob, cxbmMemoNull,
    cxbmMemo, cxbmPictNull, cxbmPict, cxbmOleNull, cxbmOle);
var
  ABitmap: TBitmap;
  I: Integer;
begin
...
GetRegisteredEditProperties.Register(TcxBlobEditProperties,
