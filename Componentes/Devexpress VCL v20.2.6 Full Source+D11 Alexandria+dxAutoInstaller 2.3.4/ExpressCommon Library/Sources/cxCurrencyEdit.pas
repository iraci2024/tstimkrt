...
type
  ...
  public
    ...
    property ActiveProperties: TcxCustomCurrencyEditProperties
...
function TcxCustomCurrencyEditProperties.GetAssignedValues
  : TcxCurrencyEditPropertiesValues;
begin
...
procedure TcxCustomCurrencyEditProperties.DoAssign
...
class function TcxCustomCurrencyEditProperties.GetAssignedValuesClass
...
function TcxCustomCurrencyEditProperties.GetDisplayFormatOptions
...
class function TcxCustomCurrencyEditProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxCurrencyEdit;
end;

function TcxCustomCurrencyEditProperties.GetEditValueSource
...
function TcxCustomCurrencyEditProperties.GetSupportedOperations
...
function TcxCustomCurrencyEditProperties.IsDisplayValueValid(var ADisplayValue
...
var
  ...
  S: string;
begin
  Result := not AEditFocused or (InternalGetEditFormat(C, AIsOnGetTextAssigned)
    <> '') or AIsOnGetTextAssigned;
  if not Result then
  begin
...
procedure TcxCustomCurrencyEditProperties.DoPrepareDisplayValue(const AEditValue
...
procedure TcxCustomCurrencyEditProperties.ValidateDisplayValue(var ADisplayValue
...
function TcxCustomCurrencyEdit.GetActiveProperties
  : TcxCustomCurrencyEditProperties;
begin
...
begin
  ...
  else if VarIsNull(EditValue) or
    (VarIsStr(EditValue) and (StrToCurrDef(EditValue, 0) = 0)) then
    Result := 0.00
  else
    Result := EditValue;
end;
...
begin
  with ActiveProperties do
    if IsValueBoundDefined(evbMin) and (Value < MinValue) then
      Value := MinValue
    else if IsValueBoundDefined(evbMax) and (Value > MaxValue) then
      Value := MaxValue;
...
begin
  ...
  if not IsNumericChar(Key, ntExponent) and
    not(ActiveProperties.UseThousandSeparator and
    (Key = dxFormatSettings.ThousandSeparator)) then
    Exit;
  if ((ActiveProperties.InternalGetEditFormat(AIsCurrency, AIsOnGetTextAssigned,
    Self) <> '') or AIsOnGetTextAssigned) and not IsValidNumber(Text) then
  begin
    Result := True;
    Exit;
  end;
...
begin
  if not(ActiveProperties.UseThousandSeparator and
    (Key = dxFormatSettings.ThousandSeparator)) and dxCharInSet(Key, ['.', ','])
  then
...
class function TcxCustomCurrencyEdit.GetPropertiesClass
...
begin
  ...
  begin
    ...
    begin
      AEditFormat := '0.' + DupeString('0',
        ActiveProperties.DecimalPlaces) + ';';
      EditingText := FormatFloat(AEditFormat, EditingValue);
    end;
  end;
end;

procedure TcxCustomCurrencyEdit.PrepareEditValue(const ADisplayValue
...
class function TcxFilterCurrencyEditHelper.GetFilterEditClass
  : TcxCustomEditClass;
begin
  Result := TcxCurrencyEdit;
end;
...
GetRegisteredEditProperties.Register(TcxCurrencyEditProperties,
  scxSEditRepositoryCurrencyItem);
FilterEditsController.Register(TcxCurrencyEditProperties,
  TcxFilterCurrencyEditHelper);

finalization

FilterEditsController.Unregister(TcxCurrencyEditProperties,
  TcxFilterCurrencyEditHelper);
