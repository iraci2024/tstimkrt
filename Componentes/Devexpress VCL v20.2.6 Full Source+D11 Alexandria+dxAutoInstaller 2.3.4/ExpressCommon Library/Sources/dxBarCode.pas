...
{$ENDIF}
  ...
  cxDataUtils, cxVariants, dxTouch, dxSkinsCore, dxCoreGraphics, cxEdit,
  cxEditConsts, dxBarCodeUtils;
...
type
  ...
  strict private
    function GetTransformMatrix: XFORM;
  protected
  protected
  ...
  public
    ...
    property BarCodeSymbologyClass: TdxCustomBarCodeSymbologyClass
      read FBarCodeSymbologyClass write SetBarCodeSymbologyClass;
    property BarCodeSymbologyClassName: string read GetBarCodeSymbologyClassName
      write SetBarCodeSymbologyClassName;
  ...
  protected
    ...
    procedure InternalSetEditValue(const Value: Variant;
...
begin
  ...
  try
    ...
    begin
      ADifferenceSize :=
        (cxRectWidth(ClientRect) - cxRectHeight(ClientRect)) div 2;
    ...
    begin
      ...
      BarCodeErrorType := TdxCustomBarCodeSymbologyAccess(Symbology)
        .Paint(ACanvas, ARect, VarToStr(Value), Font);
    end;
  finally
    ACanvas.RestoreDC;
  end;
  ...
  begin
    ...
    cxTextOut(ACanvas.Handle, AText, ClientRect, CXTO_CENTER_HORIZONTALLY or
      CXTO_CENTER_VERTICALLY or CXTO_WORDBREAK, Font);
  end;
end;
...
procedure TdxBarCodeViewData.Calculate(ACanvas: TcxCanvas; const ABounds: TRect;
  const P: TPoint; Button: TcxMouseButton; Shift: TShiftState;
...
begin
  ...
  ABarCodeViewInfo.ModuleWidth := ScaleFactor.Apply(AProperties.ModuleWidth,
...
procedure TdxBarCodeViewData.EditValueToDrawValue(const AEditValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
begin
  (AViewInfo as TdxBarCodeViewInfo).Value := AEditValue;
end;

function TdxBarCodeViewData.GetEditContentSize(ACanvas: TcxCanvas;
  const AEditValue: TcxEditValue;
  const AEditSizeProperties: TcxEditSizeProperties;
  ...
  begin
    TdxCustomBarCodeSymbologyAccess(AProperties.Symbology).FitMode :=
      AProperties.FitMode;
    TdxCustomBarCodeSymbologyAccess(AProperties.Symbology).ModuleWidth :=
      ScaleFactor.Apply(AProperties.ModuleWidth, AProperties.ScaleFactor);
    TdxCustomBarCodeSymbologyAccess(AProperties.Symbology).ShowText :=
      AProperties.ShowText;
    ABarCodeErrorType := TdxCustomBarCodeSymbologyAccess(AProperties.Symbology)
      .CalculateSize(VarToStr(AEditValue), Style.Font, ABarCodeSize.cx,
      ABarCodeSize.cy);
    if ABarCodeErrorType <> bceNone then
    begin
      ARect := Rect(0, 0, ScaleFactor.Apply(100),
        ScaleFactor.Apply(dxBarCodeDefaultInplaceHeight));
      case ABarCodeErrorType of
        bceInvalidCharacters:
          cxGetTextRect(ARect,
            cxGetResourceString(@sdxBarCodeInvalidCharactersError),
            ACanvas.Font, DT_CENTER or DT_WORDBREAK);
        bceInvalidTextFormat:
          cxGetTextRect(ARect,
            cxGetResourceString(@sdxBarCodeInvalidTextFormatError),
            ACanvas.Font, DT_CENTER or DT_WORDBREAK);
      end;
...
begin
  ...
  begin
    ...
    else if not VarIsNull(AEditValue) then
...
class function TdxCustomBarCodeProperties.GetViewInfoClass
...
function TdxCustomBarCodeProperties.GetSupportedOperations
...
procedure TdxCustomBarCodeProperties.DoAssign(AProperties
...
begin
  ...
  begin
    ...
    ModuleWidth := ScaleFactor.Apply(ABarCodeProperties.ModuleWidth,
      ABarCodeProperties.ScaleFactor);
...
class function TdxCustomBarCodeProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TdxBarCodeViewData;
end;

procedure TdxCustomBarCodeProperties.BarCodeSymbologyChangedHandler
  (Sender: TObject);
begin
  Changed;
end;
...
procedure TdxCustomBarCodeProperties.SetBarCodeSymbologyClass
  (AValue: TdxCustomBarCodeSymbologyClass);
begin
  ...
  begin
    ...
    TdxCustomBarCodeSymbologyAccess(FSymbology).OnChanged :=
      BarCodeSymbologyChangedHandler;
    Changed;
  end;
end;

procedure TdxCustomBarCodeProperties.SetBarCodeSymbologyClassName
  (AValue: string);
begin
  BarCodeSymbologyClass := TdxCustomBarCodeSymbologyClass
    (dxGetRegisteredBarCodeSymbologies.FindByClassName(AValue));
end;
...
procedure TdxCustomBarCodeProperties.SetSymbology
  (AValue: TdxCustomBarCodeSymbology);
begin
...
class function TdxCustomBarCode.GetPropertiesClass
...
begin
  ...
  begin
    ...
    begin
      TBitmap(Dest).SetSize(cxRectWidth(ViewInfo.ClientRect),
...
procedure TdxCustomBarCode.InternalSetEditValue(const Value: Variant;
...
GetRegisteredEditProperties.Register(TdxBarCodeProperties,
