...
uses
  ...
  dxCore, dxCoreClasses, cxClasses, cxContainer, cxControls, cxGraphics,
  cxLookAndFeels, cxGeometry,
  cxLookAndFeelPainters, cxDataUtils, cxVariants, dxTouch, dxSkinsCore,
  dxCoreGraphics, dxTypeHelpers,
  cxEdit, cxEditConsts, ImgList, dxZoomTrackBar, cxTrackBar, dxGDIPlusApi,
  dxGDIPlusClasses, dxScrollAnimation;
...
type
  ...
  TcxPopupMenuItem = (pmiCut, pmiCopy, pmiPaste, pmiDelete, pmiLoad, pmiWebCam,
    pmiSave, pmiCustom);
  TcxPopupMenuItemClick = procedure(Sender: TObject; MenuItem: TcxPopupMenuItem)
    of object;
  ...
  published
    property MenuItems: TcxPopupMenuItems read FMenuItems write FMenuItems
      default [pmiCut, pmiCopy, pmiPaste, pmiDelete, pmiLoad,
      pmiWebCam, pmiSave];
    property CustomMenuItemCaption: string read FCustomMenuItemCaption
      write FCustomMenuItemCaption;
    property CustomMenuItemGlyph: TBitmap read GetCustomMenuItemGlyph
      write SetCustomMenuItemGlyph;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    //
    property ZoomTrackBarViewInfo: TdxCustomZoomTrackBarViewInfo
      read FZoomTrackBarViewInfo; // for internal use
  end;
  ...
  protected
    procedure CalculateZoomTrackBar(ACanvas: TcxCanvas; const ABounds: TRect;
  ...
  private
    ...
    function CalculateFitMode(AProportional, AStretch: Boolean)
      : TcxImageFitMode;
    ...
    procedure SetZoomTrackBarProperties
      (AValue: TdxCustomZoomTrackBarProperties);
  protected
    ...
    property ZoomTrackBarProperties: TdxCustomZoomTrackBarProperties
      read FZoomTrackBarProperties write SetZoomTrackBarProperties;
  public
    ...
    function GetGraphicClass(AItem: TObject; ARecordIndex: Integer;
    ...
    property GraphicTransparency: TcxImageTransparency read FGraphicTransparency
      write SetGraphicTransparency default gtDefault;
    property PopupMenuLayout: TcxPopupMenuLayout read FPopupMenuLayout
      write SetPopupMenuLayout;
    property ShowFocusRect: Boolean read FShowFocusRect write SetShowFocusRect
      default True;
    property OnAssignPicture: TcxImageAssignPictureEvent read FOnAssignPicture
      write FOnAssignPicture;
  ...
  published
    ...
    property ShowZoomTrackBar: Boolean read FShowZoomTrackBar
      write SetShowZoomTrackBar default False;
  end;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property Animation: TdxDefaultBoolean read FAnimation write SetAnimation
      default bDefault;
    property AnimationLoop: TdxDefaultBoolean read FAnimationLoop
      write SetAnimationLoop default bDefault;
  end;
  ...
  private
    ...
    function CanZoomPercentByMouseWheel(AShift: TShiftState;
  ...
  protected
    ...
    function GetGraphicClass(APastingFromClipboard: Boolean = False)
      : TGraphicClass; virtual;
    ...
    // properties
    property AnimationOptions: TcxImageAnimationOptions read FAnimationOptions
    ...
    property ZoomingOptions: TcxImageZoomingOptions read FZoomingOptions
      write FZoomingOptions;
  ...
  public
    ...
    property ActiveProperties: TcxCustomImageProperties
...
begin
  ...
  for I := 0 to GetRegisteredGraphicClasses.Count - 1 do
    if InternalCompareString(AClassName, TClass(GetRegisteredGraphicClasses[I])
      .ClassName, False) then
    begin
...
begin
  inherited Create;
  FImage := AImage;
  FMenuItems := [pmiCut, pmiCopy, pmiPaste, pmiDelete, pmiLoad,
    pmiWebCam, pmiSave];
end;
...
procedure TcxCustomImageProperties.SetGraphicClass(const Value: TGraphicClass);
begin
...
procedure TcxCustomImageProperties.SetGraphicClassName(const Value: string);
var
...
procedure TcxCustomImageProperties.SetZoomTrackBarProperties
  (AValue: TdxCustomZoomTrackBarProperties);
begin
...
function TcxCustomImageProperties.CalculateFitMode(AProportional,
  AStretch: Boolean): TcxImageFitMode;
begin
...
procedure TcxCustomImageProperties.WriteIsGraphicClassNameEmpty
  (Writer: TWriter);
begin
...
function TcxCustomImageProperties.CreateZoomBarProperties
...
procedure TcxCustomImageProperties.DoAssign(AProperties
...
function TcxCustomImageProperties.GetRealStretch(const APictureSize,
  ABoundsSize: TSize): Boolean;
begin
  Result := Stretch or (Proportional and ((APictureSize.cy > ABoundsSize.cy) or
    (APictureSize.cx > ABoundsSize.cx)));
end;

class function TcxCustomImageProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TcxImageViewData;
end;

function TcxCustomImageProperties.IsScrollbarsNeeded
  (APicture: TPicture): Boolean;
begin
  Result := not Center and (FitMode = ifmNormal) and
    IsPictureAssigned(APicture);
end;
...
function TcxCustomImageProperties.GetEditValueSource(AEditFocused: Boolean)
...
function TcxCustomImageProperties.GetSupportedOperations
  : TcxEditSupportedOperations;
begin
  Result := inherited GetSupportedOperations + [esoAutoHeight, esoEditing];
end;

class function TcxCustomImageProperties.GetViewInfoClass
...
procedure TcxCustomImageProperties.ValidateDisplayValue(var DisplayValue
...
procedure TcxImageCustomScrollAnimationHelper.UpdateOwnerPosition
  (AValue: Integer);
begin
...
begin
  AValue := Max(Min(AValue, Owner.GetContentSize.cy -
    cxSize(Owner.ClientBounds).cy), 0);
end;
...
begin
  AValue := Max(Min(AValue, Owner.GetContentSize.cx -
    cxSize(Owner.ClientBounds).cx), 0);
end;

procedure TcxImageHorzScrollAnimationHelper.DoUpdateOwnerPosition
  (AValue: Integer);
begin
...
function TcxCustomImage.CanZoomPercentByMouseWheel(AShift: TShiftState;
  const AMousePos: TPoint): Boolean;
begin
...
begin
  ...
  if ZoomPercent <= 100 then
    AZoomPercent := ZoomPercent - ActiveProperties.ZoomTrackBarProperties.
      FirstRange.LineSize
  else
    AZoomPercent := ZoomPercent - ActiveProperties.ZoomTrackBarProperties.
      SecondRange.LineSize;
...
begin
  ...
  try
    ...
    // Otherwise the Icon returns the incorrect sizes
      ...
      try
        ...
        if IsPictureAssigned(Picture) then
          Picture.Graphic.Transparent :=
            ActiveProperties.GraphicTransparency = gtTransparent;
      finally
...
procedure TcxCustomImage.PreparePopup;
  ...
  begin
    ...
    begin
      ...
      Items[6].Caption := cxGetResourceString
        (@cxSMenuItemCaptionAssignFromWebCam);
      Items[7].Caption := cxGetResourceString(@cxSMenuItemCaptionSave);
    end;
  end;

  function NewItem(const ACaption: string; ABitmap: TBitmap; ATag: TcxTag)
    : TMenuItem;
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ActiveProperties.PopupMenuLayout.CustomMenuItemGlyph.
          Transparent := True;
        Add(NewItem(ActiveProperties.PopupMenuLayout.CustomMenuItemCaption,
          ActiveProperties.PopupMenuLayout.CustomMenuItemGlyph,
          TdxNativeInt(AMenuItem)));
      end
      else
...
begin
  ...
  begin
    ...
    begin
      Acx := (GetLeftPos + GetClientSize.cx / 2) * FZoomPercent /
        APrevZoomPercent;
      Acy := (GetTopPos + GetClientSize.cy / 2) * FZoomPercent /
        APrevZoomPercent;
    end
    else
    begin
    ...
    TdxScrollHelper.SetPos(Self, Round(Acx - GetClientSize.cx / 2),
      Round(Acy - GetClientSize.cy / 2));
  end
  else
    ZoomPercent := 100;
end;
...
begin
  inherited;
  if (PopupMenu = nil) and (ActiveProperties.PopupMenuLayout.MenuItems <> [])
...
function TcxCustomImage.InternalMouseWheel(Shift: TShiftState;
...
begin
  Result := (AnimationOptions.FGraphic = Picture.Graphic) and
    (AnimationOptions.FGraphic <> nil) and
...
function TcxCustomImage.IsMouseWheelHandleNeeded(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
...
begin
  ...
  begin
    TdxScrollHelper.SetPos(Self, (GetContentSize.cx - cxSize(ClientBounds).cx)
      div 2, (GetContentSize.cy - cxSize(ClientBounds).cy) div 2);
  end;
end;
...
begin
  ...
  if ViewInfo.ShowZoomTrackBar and
    (ViewInfo.ZoomTrackBarViewInfo.MouseStates <> []) then
...
procedure TcxCustomImage.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  AZoomTrackBarBounds: TRect;
begin
  AZoomTrackBarBounds := ViewInfo.ZoomTrackBarViewInfo.Bounds;
  if (Button = mbLeft) and not IsScrollBarsArea(Point(X, Y)) and ViewInfo.ShowZoomTrackBar
  then
  begin
    if cxRectPtIn(AZoomTrackBarBounds, Point(X, Y)) then
    begin
      FZoomTrackBarController.MouseDown(Button, Shift,
        X - AZoomTrackBarBounds.Left, Y - AZoomTrackBarBounds.Top);
...
begin
  ...
  ViewInfo.ShowZoomTrackBar := ZoomingOptions.ShowZoomTrackBar and
    AllowScaling and cxRectPtIn(Bounds, X, Y);
  AZoomTrackBarBounds := ViewInfo.ZoomTrackBarViewInfo.Bounds;
  if ViewInfo.ShowZoomTrackBar and (cxRectPtIn(AZoomTrackBarBounds, Point(X, Y))
    or FZoomTrackBarSliding) then
  begin
    if ViewInfo.ZoomTrackBarViewInfo.MouseStates = [] then
      FZoomTrackBarController.MouseEnter
    else
      FZoomTrackBarController.MouseMove(Shift, X - AZoomTrackBarBounds.Left,
        Y - AZoomTrackBarBounds.Top);

    InvalidateRect(AZoomTrackBarBounds, True);
  end
  else if (ViewInfo.FZoomTrackBarViewInfo.MouseStates <> []) or
    (APrevShowZoomTrackBar <> ViewInfo.ShowZoomTrackBar) then
  begin
    FZoomTrackBarController.MouseLeave;
    InvalidateRect(AZoomTrackBarBounds, True);
  end;
  ...
  if (ssLeft in Shift) and not cxRectPtIn(AZoomTrackBarBounds, Point(X, Y)) and
    not cxPointIsInvalid(FLastDragPos) and ActiveProperties.IsScrollbarsNeeded
    (Picture) and not FZoomTrackBarSliding then
  begin
    dx := FLastDragPos.X - X;
    dy := FLastDragPos.Y - Y;
...
procedure TcxCustomImage.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  AZoomTrackBarBounds: TRect;
begin
  ...
  if (Button = mbLeft) and ViewInfo.ShowZoomTrackBar and
    not cxPointIsInvalid(FLastDragPos) and AllowScaling then
    FZoomTrackBarController.MouseUp(Button, Shift, X - AZoomTrackBarBounds.Left,
      Y - AZoomTrackBarBounds.Top);
...
begin
  Result := (ActiveProperties <> nil) and
    ActiveProperties.IsScrollbarsNeeded(Picture);
end;
...
procedure TcxCustomImage.InternalSetEditValue(const Value: TcxEditValue;
  ...
  begin
    ...
    begin
      ...
      begin
        Result := (VarArrayLowBound(V1, 1) = VarArrayLowBound(V2, 1)) and
          (VarArrayLowBound(V1, 1) = 0) and
...
begin
  ...
  AImageRect := cxGetImageRect(ClientBounds, AImageSize,
    ActiveProperties.FitMode, ActiveProperties.Center, ZoomPercent);
...
function TcxCustomImage.GetZoomTrackBarActiveProperties
...
begin
  Result := cxPointOffset(MouseDownPos,
    ViewInfo.FZoomTrackBarViewInfo.Bounds.TopLeft, False);
end;
...
function TcxCustomImage.GetGraphicClass(APastingFromClipboard: Boolean = False)
  : TGraphicClass;
begin
  if IsInplace then
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (AGraphicClass <> nil) and AGraphicClass.InheritsFrom(TdxSmartImage)
        then
        begin
...
function TcxImageViewInfo.CreateZoomTrackBarViewInfo
  : TdxCustomZoomTrackBarViewInfo;
begin
  Result := TdxCustomZoomTrackBarViewInfo.Create;
end;

procedure TcxImageViewInfo.DrawTransparentBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
...
begin
  ...
  begin
    R := cxGetImageRect(CR, dxGetImageSize(Picture, ScaleFactor), FitMode,
      Center, ZoomPercent);
...
begin
  Result := TcxCustomImageProperties(EditProperties)
    .GetRealStretch(Size(Picture.Width, Picture.Height),
    Size(cxRectWidth(ClientRect), cxRectHeight(ClientRect)));
end;

{ TcxImageViewData }

procedure TcxImageViewData.Calculate(ACanvas: TcxCanvas; const ABounds: TRect;
  const P: TPoint; Button: TcxMouseButton; Shift: TShiftState;
  AViewInfo: TcxCustomEditViewInfo; AIsMouseEvent: Boolean);
var
...
begin
  ...
  AImageViewInfo.ShowFocusRect := AProperties.ShowFocusRect and Focused and
    not IsInplace;
  AImageViewInfo.FitMode := AProperties.FitMode;
  AImageViewInfo.Scalable := (AImageViewInfo.Edit <> nil) and
    TcxCustomImage(AImageViewInfo.Edit).AllowScaling;
  AImageViewInfo.FUseRightToLeftScrollBar := UseRightToLeftScrollBar;

  if AImageViewInfo.Scalable then
  begin
    if not IsDesigning then
      CalculateZoomTrackBar(ACanvas, AViewInfo.ClientRect, P, Button, Shift,
        AImageViewInfo.FZoomTrackBarViewInfo, AIsMouseEvent,
        AImageViewInfo.ZoomPercent);

    AImageViewInfo.ZoomPercent := TcxCustomImage(AImageViewInfo.Edit)
      .ZoomPercent;
  end
  else
...
procedure TcxImageViewData.EditValueToDrawValue(const AEditValue: TcxEditValue;
...
begin
  with TcxImageViewInfo(AViewInfo) do
    ...
    begin
      ...
      AGraphicClass := TcxCustomImageProperties(Properties)
        .GetGraphicClass(InplaceEditParams.Position.Item,
        InplaceEditParams.Position.RecordIndex);
      LoadPicture(Picture, AGraphicClass, AEditValue);
      if TcxCustomImageProperties(Properties).GraphicTransparency <> gtDefault
      then
        Picture.Graphic.Transparent := TcxCustomImageProperties(Properties)
          .GraphicTransparency = gtTransparent;
    end
    else if Assigned(Picture) then
      Picture.Assign(nil);
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if GetRealStretch(Result, Size(AEditSizeProperties.Width,
            AEditSizeProperties.Height)) then
          begin
            if (AEditSizeProperties.Width > 0) and (Result.cx > 0) and
              ((AEditSizeProperties.Height > 0) or
              (AEditSizeProperties.Width < Result.cx)) then
              Result := Size(AEditSizeProperties.Width,
                Round(Result.cy * AEditSizeProperties.Width / Result.cx))
            else if (AEditSizeProperties.Height > 0) and (Result.cy > 0) and
              ((AEditSizeProperties.Width > 0) or
              (AEditSizeProperties.Height < Result.cy)) then
              Result := Size(Round(Result.cx * AEditSizeProperties.Height /
                Result.cy), AEditSizeProperties.Height);
          end;
        end
        else if Length(Caption) <> 0 then
        begin
...
procedure TcxImageViewData.CalculateZoomTrackBar(ACanvas: TcxCanvas;
...
begin
  ARect := cxRectCenterHorizontally(ABounds, Min(ScaleFactor.Apply(200),
    (cxRectWidth(ABounds))));
  ARect := cxRectSetHeight(ARect, ScaleFactor.Apply(30));
  ARect := cxRectSetTop(ARect,
    Min(cxRectHeight(ABounds) - ScaleFactor.Apply(40),
    Round(7 / 8 * cxRectHeight(ABounds))));
  TdxCustomZoomTrackBarViewInfo(AViewInfo).Position := AZoomPercent;

  AZoomTrackBarViewData := TdxCustomZoomTrackBarViewData.Create
    (TcxCustomImageProperties(Properties).ZoomTrackBarProperties,
    Edit.Style, True);
  try
    ...
    AZoomTrackBarViewData.Calculate(ACanvas, cxRectSetNullOrigin(ARect), P,
      Button, Shift, AViewInfo, AIsMouseEvent);
...
GetRegisteredEditProperties.Register(TcxImageProperties,
