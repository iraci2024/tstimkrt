...
uses
  ...
  Themes,
  Menus, Messages, Types, AppEvnts, ShellApi, ShlObj, SysUtils, dxCore,
  cxControls,
...
type
  ...
  TcxNavigationEvent = procedure(Sender: TcxCustomInnerShellListView;
    APIDL: PItemIDList; ADisplayName: WideString) of object;
  TcxShellExecuteItemEvent = procedure(Sender: TObject; APIDL: PItemIDList;
  ...
  protected
    ...
    procedure DoSynchronizeRoot(AControl: TWinControl;
      ARoot: TcxCustomShellRoot);
  public
  ...
  published
    ...
    property AutoNavigate: Boolean read FAutoNavigate write FAutoNavigate
      default True;
    property CurrentFolderContextMenu: Boolean read FCurrentFolderContextMenu
      write FCurrentFolderContextMenu default False;
    property FileMask;
  end;
  ...
  TcxCustomInnerShellListView = class(TcxBaseInnerListView, IcxDropTarget,
    IcxShellDependedControls, IcxShellRoot)
  private
  ...
  protected
    ...
    function OwnerDataFetch(Item: TListItem; Request: TItemRequest)
      : Boolean; override;
    ...
    procedure DsmShellChangeNotify(var Message: TMessage);
      message DSM_SHELLCHANGENOTIFY;
    ...
    property CurrentDropTarget: IcxDropTarget read FCurrentDropTarget
      write FCurrentDropTarget;
    property DraggedObject: IcxDataObject read FDraggedObject
      write FDraggedObject;
    property DropTargetItemIndex: Integer read FDropTargetItemIndex
      write SetDropTargetItemIndex;
  ...
  public
    ...
    property DependedControls: TcxShellDependedControls read FDependedControls
      write FDependedControls; // for internal use
    property DragDropSettings: TcxDragDropSettings read FDragDropSettings
      write FDragDropSettings;
    ...
    property ListViewStyle: TcxListViewStyle read FListViewStyle
    ...
    property AfterNavigation: TcxNavigationEvent read FAfterNavigation
      write FAfterNavigation;
    property BeforeNavigation: TcxNavigationEvent read FBeforeNavigation
  ...
  protected
    function CreateFakeProducer: TdxCustomShellTreeItemProducer;
      virtual; abstract;
  ...
  TcxCustomInnerShellTreeView = class(TcxBaseInnerTreeView, IcxDropTarget,
    IcxShellDependedControls, IcxShellRoot)
  private
    FAppEvents: TApplicationEvents;
    ...
    procedure DSMShellTreeChangeNotify(var Message: TMessage);
      message DSM_SHELLTREECHANGENOTIFY;
    ...
    property PrevTargetNode: TTreeNode read FPrevTargetNode
      write SetPrevTargetNode;
  protected
    procedure CreateWnd; override;
    procedure DestroyWnd; override;
  ...
  public
    ...
    property Folders[AIndex: Integer]: TcxShellFolder read GetFolder;
    property ListView: TcxCustomInnerShellListView read FListView
      write SetListView; // for internal use
...
function cxSetShellControl(AControl, AValue: TWinControl;
  var AFieldValue: TWinControl): Boolean;

function cxShellGetImageList(AFlags: Integer): THandle;
function cxShellIsZipFile(AFolder: TcxShellFolder): Boolean;
function cxShellGetContextMenuCommandStrByShortCut(Key: Word;
  Shift: TShiftState): AnsiString; // for internal use only
procedure cxShellGetNotifyParams(Message: TMessage; out AEventID: Integer;
  out APIDL1, APIDL2: PItemIDList);
procedure cxShellInvokeContextMenuCommand(AShellFolder: IShellFolder;
  AItemPIDLList: TList; const ACommandStr: AnsiString); // for internal use only
function cxShellIsClipboardCommandContextMenuShortCut(Key: Word;
  Shift: TShiftState): Boolean; // for internal use only
procedure cxShellRegisterChangeNotifier(ANotifierPIDL: PItemIDList; AWnd: HWND;
  ANotificationMessage: Cardinal; AWatchSubtree: Boolean;
  var ANotifierData: TcxShellChangeNotifierData);
procedure cxShellUnregisterChangeNotifier(var ANotifierData
  : TcxShellChangeNotifierData);
...
procedure cxShellInvokeContextMenuCommand(AShellFolder: IShellFolder;
  AItemPIDLList: TList; const ACommandStr: AnsiString);
var
...
begin
  ...
  begin
    ...
    try
      if AShellFolder.GetUIObjectOf(0, AItemPIDLList.Count, APIDLs[0],
        IID_IContextMenu, nil, AIContextMenu) = S_OK then
      begin
...
begin
  Result := SHGetFileInfo('C:\', 0, AFileInfo, SizeOf(AFileInfo),
    SHGFI_SYSICONINDEX or AFlags);
end;

function cxShellIsZipFile(AFolder: TcxShellFolder): Boolean;
begin
  Result := AFolder.StorageCapabilities * [sfscStream, sfscFolder] =
    [sfscStream, sfscFolder];
end;

procedure SetComCtlStyle(ACtl: TWinControl; AValue: Integer;
  AUseStyle: Boolean);
const
...
procedure CheckDefaultDropEffect(grfKeyState: Integer;
  ADragDropSettings: TcxDragDropSettings; var dwEffect: Integer);
begin
    ...
    begin
      ...
      try
        ...
        ANotifierData.Handle := SHChangeNotifyRegister(AWnd,
          SHCNF_ACCEPT_INTERRUPTS or SHCNF_ACCEPT_NON_INTERRUPTS or
          SHCNF_NO_PROXY, SHCNE_RENAMEITEM or SHCNE_CREATE or SHCNE_DELETE or
          SHCNE_MKDIR or SHCNE_RMDIR or SHCNE_ATTRIBUTES or SHCNE_UPDATEDIR or
          SHCNE_UPDATEITEM or SHCNE_UPDATEIMAGE or SHCNE_RENAMEFOLDER,
          ANotificationMessage, 1, AItems);
      finally
        Dispose(AItems);
      end;
    end;
...
function cxSetShellControl(AControl, AValue: TWinControl;
  ...
  procedure UpdateDependencies(AOwner, AControl: TWinControl;
...
begin
  ...
  begin
    ...
    CurrentFolderContextMenu := TcxShellListViewOptions(Source)
      .CurrentFolderContextMenu;
  end;
...
procedure TcxShellDependedControls.DoNavigate(AControl: TWinControl;
  AItem: PItemIDList);
begin
...
procedure TcxShellDependedControls.DoSynchronizeRoot(AControl: TWinControl;
  ARoot: TcxCustomShellRoot);
begin
...
begin
  ...
  Result := ItemProducer.ItemInfo[Item.Index].CanRename and
...
procedure TcxCustomInnerShellListView.CNNotify(var Message: TWMNotify);
  ...
  begin
    ...
    AItemData.CheckUpdate(ItemProducer.ShellFolder,
  ...
  case Message.NMHdr^.code of
    ...
    LVN_GETINFOTIP:
      ItemProducer.DoGetInfoTip(Handle, PNMLVGetInfoTip(Message.NMHdr)^.iItem,
        PNMLVGetInfoTip(Message.NMHdr)^.pszText, PNMLVGetInfoTip(Message.NMHdr)
        ^.cchTextMax);
    LVN_GETDISPINFO:
      begin
        inherited;
    ...
    LVN_ODFINDITEM:
      begin
        if (PNMLVFindItem(Message.NMHdr)^.lvfi.flags and $4) <> 0
        then { LVFI_SUBSTRING }
          PNMLVFindItem(Message.NMHdr)^.lvfi.flags :=
            PNMLVFindItem(Message.NMHdr)^.lvfi.flags or LVFI_PARTIAL;
        inherited;
      end;
  else
    inherited;
...
function TcxCustomInnerShellListView.IDropTargetDragOver(grfKeyState: Integer;
...
begin
  ...
  begin
    ...
    CheckDefaultDropEffect(grfKeyState, DragDropSettings, dwEffect);
    if New then
      Result := CurrentDropTarget.DragEnter(DraggedObject, grfKeyState,
        pt, dwEffect)
    else
      Result := S_OK;
...
begin
  ...
  begin
    if FInternalSmallImages <> 0 then
      SendMessage(Handle, LVM_SETIMAGELIST, LVSIL_SMALL,
        LParam(FInternalSmallImages));
...
begin
  ...
  begin
    ...
    try
      ...
      else if Options.AutoExecute then
...
procedure TcxCustomInnerShellListView.DoContextPopup(MousePos: TPoint;
  var Handled: Boolean);
begin
  if Options.ContextMenus and (SelCount > 0) or (SelCount = 0) and Options.CurrentFolderContextMenu
  then
  begin
    Handled := True;
    ItemProducer.LockRead;
...
begin
  ...
  if Assigned(OnEdited) then
    OnEdited(Self, Items.Item[Item.iItem], S);
  ATempItem := ItemProducer.ItemInfo[Item.iItem];
  ANewName := S;
  ItemProducer.ShellFolder.SetNameOf(Handle, ATempItem.PIDL,
    PWideChar(ANewName), SHGDN_INFOLDER or SHGDN_FORPARSING, ANewPidl);
  try
    ATempItem.SetNewPidl(ItemProducer.ShellFolder, ItemProducer.FolderPidl,
      ANewPidl);
  finally
...
procedure TcxCustomInnerShellListView.AdvancedDrawItem(Sender: TCustomListView;
  ...
  begin
    ...
    if (Item.Selected or (cdsHot in State)) and not IsWinXP and
      IsXPManifestEnabled and AreVisualStylesAvailable([totMenu]) then
      dxDrawThemeBackground(ACanvas.Handle, totMenu, dxGetScaleFactor(Parent),
        ABounds, MENU_POPUPITEM, GetItemState)
  end;
  ...
  procedure DrawThumbnail(ACanvas: TcxCanvas; const AIconRect: TRect;
  ...
  begin
    ...
    if IsWinVistaOrLater and not cxIsVCLThemesEnabled then
      cxDrawImage(ACanvas.Handle, ARect, AIconRect, nil,
        ItemProducer.FThumbnails, AShellItem.ThumbnailIndex, idmNormal)
    else
      ItemProducer.FThumbnails.Draw(ACanvas.Canvas, ARect.Left, ARect.Top,
        AShellItem.ThumbnailIndex);
  end;

  procedure DrawIcon(ACanvas: TcxCanvas; const AIconRect: TRect;
    AShellItem: TcxShellItemInfo);
  var
    ARect: TRect;
  begin
    ARect := cxRectCenter(AIconRect, GetLargeIconSize);
    FLargeImages.DrawOverlay(ACanvas.Canvas, ARect.Left, ARect.Top,
      AShellItem.IconIndex, AShellItem.OverlayIndex, dsTransparent, itImage);
  end;
  ...
  procedure InternalDrawItem(ACanvas: TcxCanvas;
    const ABounds, AIconRect, ATextRect: TRect; ADefaultDrawText: Boolean;
    AShellItem: TcxShellItemInfo);
  begin
  ...
  begin
    ...
    try
      ...
      if UseRightToLeftAlignment then
        SetLayout(ABitmap.Canvas.Handle, LAYOUT_RTL or
          LAYOUT_BITMAPORIENTATIONPRESERVED);
      InternalDrawItem(ABitmap.cxCanvas, ABounds, AIconRect, ATextRect,
        ADefaultDrawText, AShellItem);
...
function TcxCustomInnerShellListView.DoAddFolder
  (AFolder: TcxShellFolder): Boolean;
begin
  Result := AFolder.IsFolder and
    (Options.ShowNonFolders or Options.ShowZipFilesWithFolders or
    not cxShellIsZipFile(AFolder)) or not AFolder.IsFolder and
...
begin
  if Failed(SHGetDesktopFolder(Desktop)) then
    Exit;
  if Succeeded(Desktop.GetDisplayNameOf(fqPidl, SHGDN_NORMAL or
    SHGDN_FORPARSING, StrName)) then
...
begin
  ...
  try
    ...
    try
      if Failed(ItemProducer.ShellFolder.GetUIObjectOf(Handle, SelCount,
        APidlList[0], IDataObject, nil, Pointer(pDataObject))) then
        Exit;
...
procedure TcxCustomInnerShellListView.DoOnIdle(Sender: TObject;
  var Done: Boolean);
var
  AList: TList;
  AItem: TcxShellItemInfo;
...
function TcxCustomInnerShellListView.CheckFileMask
  (AFolder: TcxShellFolder): Boolean;
begin
...
begin
  ...
  begin // There are no items selected, so drop target is current opened folder
    ...
    TryReleaseDropTarget;
    New := True;
    if Failed(ItemProducer.ShellFolder.CreateViewObject(Handle, IDropTarget,
      tempDropTarget)) then
      Exit;
  ...
  begin // Use one of Items as Drop Target
    ...
    try
      if Failed(ItemProducer.ShellFolder.GetUIObjectOf(Handle, 1, tempPidl,
        IDropTarget, nil, tempDropTarget)) then
        Exit;
    finally
...
function TcxCustomInnerShellListView.GetPidlByItemIndex(AIndex: Integer)
  : PItemIDList;
begin
  Result := ItemProducer.ItemInfo[AIndex].PIDL;
end;

function TcxCustomInnerShellListView.GetSortOrder(AColumnIndex: Integer)
  : TdxSortOrder;
begin
...
function TcxCustomInnerShellListView.OwnerDataFind(Find: TItemFind;
...
procedure TcxCustomInnerShellListView.SetRootField
  (const Value: TcxShellListRoot);
begin
  FRoot.Assign(Value);
end;
...
procedure TcxCustomInnerShellListView.ShellChangeNotify(AEventID: Longint;
...
procedure TcxCustomInnerShellListView.DestroySelectedPidlsList
  (ASelectedPidls: TList);
var
  I: Integer;
begin
...
function TcxCustomInnerShellListView.GetDependedControls
  : TcxShellDependedControls;
begin
...
procedure TcxCustomInnerShellListView.RootFolderChanged(Sender: TObject;
...
procedure TcxCustomInnerShellListView.SetLargeIconSize
  (const Value: TcxShellIconSize);
begin
...
procedure TcxCustomInnerShellListView.ProcessTreeViewNavigate
  ...
  begin
    ...
    cxShellGetThreadSafeFileInfo(Pointer(APIDL), 0, ASHFileInfo,
      SizeOf(ASHFileInfo), SHGFI_PIDL or SHGFI_ATTR_SPECIFIED or
      SHGFI_ATTRIBUTES);
    Result := ASHFileInfo.dwAttributes and SFGAO_FOLDER <> 0;
  end;

  function GetNearestFolderPidl(APIDL: PItemIDList; out ANewlyCreated: Boolean)
    : PItemIDList;
  var
  ...
  begin
    ...
    if not IsFolder(Result) or
      not Succeeded(GetDesktopIShellFolder.BindToObject(APIDL, nil,
      IID_IShellFolder, AFolder)) then
    begin
...
procedure TcxCustomInnerShellListView.Sort(AColumnIndex: Integer;
...
procedure TcxShellListViewCurrentFolderContextMenu.ExecuteMenuItemCommand
  (ACommandId: Cardinal);
begin
  case ACommandId of
    cmdExtraLargeId:
      ...
      lvsIcon:
        begin
          if not ListView.ThumbnailOptions.ShowThumbnails then
            Result := 2
          else if ListView.ThumbnailOptions.Width = 128 then
            Result := 0
          else if ListView.ThumbnailOptions.Width = 64 then
            Result := 1;
        end;
      lvsSmallIcon:
        Result := 3;
...
const
  ...
  AIconSizeMenuItemCaptions: array [0 .. AIconSizeMenuItemsCount - 1]
    of string = ('Extra large icons', 'Large icons', 'Medium icons',
...
begin
  ...
  AddRadioGroup(AIconSizeMenuItemCaptions, cmdExtraLargeId, 0,
...
procedure TcxShellListViewCurrentFolderContextMenu.AddCheckItem
...
procedure TcxShellListViewCurrentFolderContextMenu.AddItem(const ACaption
  : string; AId: Cardinal; AMenu: HMenu);
begin
  AddCheckItem(ACaption, AId, False, AMenu);
end;

procedure TcxShellListViewCurrentFolderContextMenu.AddRadioGroup
...
procedure TcxShellListViewCurrentFolderContextMenu.AddSeparator
  (AMenu: HMenu = 0);
begin
  if AMenu = 0 then
    AMenu := Menu;
...
function TcxShellListViewCurrentFolderContextMenu.AddSubitem(const ACaption
...
procedure TcxShellListViewProducer.ProcessItems(AIFolder: IShellFolder;
...
function TcxShellListViewProducer.CanAddFolder(AFolder: TcxShellFolder)
  : Boolean;
begin
...
procedure TcxShellListViewProducer.DoSlowInitialization
  (AItem: TcxShellItemInfo);
begin
...
function TcxShellListViewProducer.GetItemsInfoGatherer
  : TcxShellItemsInfoGatherer;
begin
  Result := ListView.ItemsInfoGatherer;
end;

function TcxShellListViewProducer.GetThumbnailIndex
...
function TcxShellListViewProducer.SlowInitializationDone
...
begin
  ...
  FThumbnails.Handle := ImageList_Create(ASize.cx, ASize.cy, ILC_COLOR32 or
    ILC_MASK, FThumbnails.AllocBy, FThumbnails.AllocBy);
end;
...
begin
  if HandleAllocated then
  begin
    AStyle := GetWindowLong(Handle, GWL_STYLE) and not(TVS_INFOTIP) or
      TVS_NOTOOLTIPS;
...
function TcxCustomInnerShellTreeView.CheckFileMask
  (AFolder: TcxShellFolder): Boolean;
begin
  Result := Options.IsFileNameValid(ExtractFileName(AFolder.PathName));
end;
    ...
    try
      ...
      if Result then
        try
          ...
          GetItemProducer(Node).ProcessItems(AProcessingFolder, AProcessingPidl,
            Node, APreloadItemCount);
        finally
...
begin
  ...
  begin
    if FInternalSmallImages <> 0 then
      SendMessage(Handle, TVM_SETIMAGELIST, TVSIL_NORMAL,
        LParam(FInternalSmallImages));
...
begin
  ...
  try
    ...
    begin
      ...
      try
        if Succeeded(ASourceItemProducer.ShellFolder.GetUIObjectOf(Handle, 1,
          ATempPidl, IDataObject, nil, ADataObject)) then
        begin
...
procedure TcxCustomInnerShellTreeView.KeyDown(var Key: Word;
  Shift: TShiftState);
...
begin
  ...
  case Key of
    ...
    if not IsEditing and Options.ContextMenus and
      (cxShellIsClipboardCommandContextMenuShortCut(Key, Shift) or
      (Key = VK_DELETE) and (Selected <> nil)) then
      InvokeContextMenuCommand(cxShellGetContextMenuCommandStrByShortCut
        (Key, Shift));
...
function TcxCustomInnerShellTreeView.IDropTargetDragOver(grfKeyState: Integer;
  pt: TPoint; var dwEffect: Integer): HResult;
var
  ANew: Boolean;
begin
...
begin
  ...
  RemoveChangeNotification;
  if AItemProducer.ShellFolder.SetNameOf(Handle, AItemInfo.PIDL,
    PWideChar(WideString(ANode.Text)), SHGDN_INFOLDER or SHGDN_FORPARSING,
    APIDL) = S_OK then
    try
      AItemInfo.SetNewPidl(AItemProducer.ShellFolder,
        AItemProducer.FolderPidl, APIDL);
...
function TcxCustomInnerShellTreeView.GetNodeFromItem(const Item: TTVItem)
  : TTreeNode;
begin
  Result := nil;
  if Items <> nil then
...
function TcxCustomInnerShellTreeView.GetItemProducer(ANode: TTreeNode)
  : TcxShellTreeItemProducer;
begin
...
function TcxCustomInnerShellTreeView.GetShellItemInfo(ANode: TTreeNode)
  : TcxShellItemInfo;
begin
  Result := GetItemProducer(ANode).ShellItemInfo;
end;

function TcxCustomInnerShellTreeView.GetDependedControls
  : TcxShellDependedControls;
begin
  Result := DependedControls;
end;
...
procedure TcxCustomInnerShellTreeView.RestoreTreeState;
  ...
  begin
    if (FStateData.TopItemIndex >= 0) and (FStateData.TopItemIndex < Items.Count)
    then
...
function TcxCustomInnerShellTreeView.DoAddFolder
  (AFolder: TcxShellFolder): Boolean;
...
begin
  Result := AFolder.IsFolder and
    (Options.ShowNonFolders or Options.ShowZipFilesWithFolders or
    not IsZipFile(AFolder)) or not AFolder.IsFolder and CheckFileMask(AFolder);
  if Assigned(FOnAddFolder) then
    FOnAddFolder(Self, AFolder, Result);
end;
...
procedure TcxCustomInnerShellTreeView.SetRootField
  (const Value: TcxShellTreeRoot);
begin
  FRoot.Assign(Value);
end;
...
procedure TcxCustomInnerShellTreeView.RootFolderChanged(Sender: TObject;
  Root: TcxCustomShellRoot);
begin
  Items.Clear;
  UpdateNode(nil, False);
...
procedure TcxCustomInnerShellTreeView.ShellChangeNotify(AEventID: Longint;
  APIDL1, APIDL2: PItemIDList);
...
procedure TcxCustomInnerShellTreeView.DSMShellTreeChangeNotify
  (var Message: TMessage);
begin
  ShellChangeNotify(Message.LParam, PPItemIDList(Message.WParam)^,
    PPItemIDList(Message.WParam + SizeOf(Pointer))^);
...
procedure TcxCustomInnerShellTreeView.DSMShellTreeRestoreCurrentPath
  (var Message: TMessage);
var
...
procedure TcxCustomInnerShellTreeView.DsmSystemShellChangeNotify
  (var Message: TMessage);
var
  AEventID: Integer;
  APIDL1, APIDL2: PItemIDList;
...
procedure TcxCustomInnerShellTreeView.DoOnIdle(Sender: TObject;
  var Done: Boolean);
var
  AList: TList;
  AItem: TcxShellItemInfo;
...
procedure TcxCustomInnerShellTreeView.GetDropTarget(out ANew: Boolean;
  APoint: TPoint);

  function InternalGetDropTarget(ANode: TTreeNode;
  ...
  begin
    ...
    begin // Root object selected
      ...
      Result := (ATempShellFolder <> nil) and
        Succeeded(ATempShellFolder.CreateViewObject(Handle, IDropTarget,
        ADropTarget));
    end
    else
...
begin
  ...
  if ANode = nil then
    TryReleaseDropTarget;
  ANew := (ANode <> nil) and ((ANode <> PrevTargetNode) or
    (CurrentDropTarget = nil));
  if ANew then
  begin
    TryReleaseDropTarget;
...
function TcxCustomInnerShellTreeView.GetNodeByPIDL(APIDL: PItemIDList)
  : TTreeNode;
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if SmallInt(ShellFolder.CompareIDs(0, AItem.PIDL, ANewItem.PIDL)) = 0
        then
        begin
...
procedure TdxCustomShellTreeItemProducer.CheckForSubitems
  (AItem: TcxShellItemInfo);
begin
...
procedure TdxCustomShellTreeItemProducer.DoSlowInitialization
  (AItem: TcxShellItemInfo);
begin
  InitializeItem(AItem);
end;

procedure TdxCustomShellTreeItemProducer.InitializeItem
  (AItem: TcxShellItemInfo);
begin
...
function TdxCustomShellTreeItemProducer.SlowInitializationDone
  (AItem: TcxShellItemInfo): Boolean;
begin
...
function TcxShellTreeItemProducer.CanAddFolder(AFolder: TcxShellFolder)
  : Boolean;
begin
  Result := TreeView.DoAddFolder(AFolder);
end;

function TcxShellTreeItemProducer.CreateFakeProducer
  : TdxCustomShellTreeItemProducer;
begin
