...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Contnrs, Controls, Forms,
  Dialogs,
  ComCtrls, ActnList, Menus, ToolWin, StdCtrls, ImgList,
  cxClasses, cxContainer, cxControls, cxGraphics, dxComCtrlsUtils,
  cxLookAndFeels, cxLookAndFeelPainters,
  cxButtons, cxEdit, cxGroupBox, cxTreeView, cxCheckBox,
  dxLayoutLookAndFeels, dxLayoutControl, dxLayoutDragAndDrop,
...
type
  TdxLayoutControlCustomizeForm = class(TdxLayoutControlCustomCustomizeForm)
    ...
    procedure tvVisibleItemsAdvancedCustomDrawItem(Sender: TCustomTreeView;
    ...
    procedure tvVisibleItemsMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    ...
    procedure tvAvailableItemsContextPopup(Sender: TObject; MousePos: TPoint;
      var Handled: Boolean);
  ...
  private
    ...
    procedure AddItemNode(ANodes: TTreeNodes; AParentNode: TTreeNode;
      AItem: TdxCustomLayoutItem; AAddChildren: Boolean = True);
    procedure AvailableItemsWndProc(var Message: TMessage);
    function CreateItem(AClass: TdxCustomLayoutItemClass;
      const ACaption: string): TdxCustomLayoutItem;
    function DoCreateItem(AClass: TdxCustomLayoutItemClass;
      const ACaption: string): TdxCustomLayoutItem;
    ...
    function TreeViewWndProcHandler(ATreeView: TTreeView;
      var Message: TMessage): Boolean;

    procedure RestoreCollapsedNodes(ATreeView: TcxTreeView;
      AList: TcxComponentList);
    procedure StoreCollapsedNodes(ATreeView: TcxTreeView;
      AList: TcxComponentList);
    procedure RestoreFirstNode(ATreeView: TcxTreeView;
      AItem: TdxCustomLayoutItem);
    procedure StoreFirstNode(ATreeView: TcxTreeView;
      out AItem: TdxCustomLayoutItem);
  protected
    ...
    function CanFloatItems(ATreeView: TcxTreeView;
      out ANeedCheck: Boolean): Boolean;
    ...
    procedure SetCustomizationCaption(AItem: TdxCustomLayoutItem;
      const ACaption: string); override;

    function FindNodeByItem(AItem: TdxCustomLayoutItem;
      out ANode: TTreeNode): Boolean;
    procedure Localize; virtual;

    procedure RefreshItemsTreeView(ATreeView: TcxTreeView; AList: TList;
      AViewKind: TdxLayoutAvailableItemsViewKind;
      ANeedSort, AShowRoot: Boolean);
    ...
    procedure StoreTreeViewWndProc(ATreeView: TTreeView;
      out AWindowProcObject: TcxWindowProcLinkedObject;
      ANewWndMethod: TWndMethod);
    procedure RestoreTreeViewWndProc(var AWindowProcObject
...
function GetCaptionOptions(AItem: TdxCustomLayoutItem)
  : TdxCustomLayoutItemCaptionOptionsAccess;
begin
...
begin
  Result := TdxCustomLayoutItemAccess(Item1).GetItemClassKind -
    TdxCustomLayoutItemAccess(Item2).GetItemClassKind;
end;

function CompareItemsByName(Item1, Item2: Pointer): Integer;

  procedure SplitNameByTrailingDigits(const AName: string;
    out ATrailingDigits: Integer; out ANameWithoutTrailingDigitals: string);
  var
  ...
  begin
    ...
    begin
      ANameWithoutTrailingDigitals := Copy(ANameWithoutTrailingDigitals, 1,
        ACurrentLength - 1);
      ACurrentLength := Length(ANameWithoutTrailingDigitals);
    end;
    if (ACurrentLength = Length(AName)) or
      not TryStrToInt(Copy(AName, ACurrentLength + 1,
      Length(AName) - ACurrentLength), ATrailingDigits) then
      ATrailingDigits := -1;
  end;
...
begin
  ...
  if (Result = 0) and (TObject(Item1) is TdxCustomLayoutGroup) then
    Result := Integer(TdxCustomLayoutGroup(Item1).Count > 0) -
      Integer(TdxCustomLayoutGroup(Item2).Count > 0);
  if Result = 0 then
...
begin
  ...
  StoreTreeViewWndProc(tvVisibleItems.InnerTreeView,
    FItemsWindowProcLinkedObject, ItemsWndProc);
  StoreTreeViewWndProc(tvAvailableItems.InnerTreeView,
    FAvailableItemsWindowProcLinkedObject, AvailableItemsWndProc);
end;
...
begin
  ...
  Constraints.MinHeight := Height - ClientHeight +
    lcMainGroup_Root.ViewInfo.MinHeight;
  Constraints.MinWidth := Width - ClientWidth +
    lcMainGroup_Root.ViewInfo.MinWidth;
end;

function TdxLayoutControlCustomizeForm.GetHitTest(const P: TPoint)
  ...
  function GetHitTestArea(ANode: TTreeNode; AItem: TdxCustomLayoutItem;
    P: TPoint): Integer;
  var
    ANodeRect: TRect;
    H: Integer;
    ...
    begin
      ...
      begin
        ...
        if P.Y < ANodeRect.Top + H then
          Result := -1
        else if P.Y > ANodeRect.Bottom - H - 2 then
          Result := 1
        else
          Result := 0;
      end;
...
begin
  ...
  else if tvAvailableItems.Visible and PtInRect(tvAvailableItems.BoundsRect, P)
  then
  begin
  ...
  begin
    ...
    TdxLayoutCustomizeFormHitTest(Result).DropAreaPart :=
      GetDropAreaPart(AHitTestArea);
  end
  else
  begin
    TdxLayoutCustomizeFormHitTest(Result).Item := nil;
    AHitTest := lcMain.GetHitTest(P);
    if (AHitTest <> nil) and ((AHitTest.GetDestinationItem = lcgTreeView) or
      (AHitTest.GetDestinationItem = lcgAvailableItems)) then
...
procedure TdxLayoutControlCustomizeForm.ToggleHotTrackState
  (AItem: TdxCustomLayoutItem);
var
  ANode: TTreeNode;
  R: TRect;
...
begin
  if Container.IsDesigning then
    Caption :=
      Format(cxGetResourceString(@sdxLayoutControlDesignerCaptionFormat),
      [cxGetFullComponentName(TdxLayoutContainerAccess(Container)
...
procedure TdxLayoutControlCustomizeForm.Notification(AComponent: TComponent;
...
begin
  ...
  if not(csDestroying in ComponentState) and (Operation = opRemove) and
    ...
    FindNodeByItem(TdxCustomLayoutItem(AComponent), ANode) then
    ANode.Delete;
end;

function TdxLayoutControlCustomizeForm.CanDeleteItems
...
procedure TdxLayoutControlCustomizeForm.DeleteItems(AList: TComponentList;
  ATreeView: TTreeView);
var
  AIntf: IdxLayoutDesignerHelper;
begin
...
function TdxLayoutControlCustomizeForm.CanFloatItems(ATreeView: TcxTreeView;
  out ANeedCheck: Boolean): Boolean;
var
  AList: TList;
  I: Integer;
...
function TdxLayoutControlCustomizeForm.FindNodeByItem
  (AItem: TdxCustomLayoutItem; out ANode: TTreeNode): Boolean;
begin
  Result := (Container <> nil) and
    (TdxCustomLayoutItemAccess(AItem).RealContainer = Container);
  if Result then
    Result := cxTreeViewFindNodeByData(tvAvailableItems.InnerTreeView, AItem,
      ANode) or cxTreeViewFindNodeByData(tvVisibleItems.InnerTreeView,
      AItem, ANode);
end;
...
begin
  Result := inherited CanModify and
    (Container.IsDesigning or not HasLockedItemInSelection);
end;
...
begin
  acAddGroup.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddGroup);
  acAddGroup.Hint := StripHotKey(acAddGroup.Caption);

  acAddItem.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddItem);
  acAddItem.Hint := StripHotKey(acAddItem.Caption);

  acAddCustomItem.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddAuxiliaryItem);
  acAddCustomItem.Hint := StripHotKey(acAddCustomItem.Caption);

  acAddEmptySpaceItem.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddEmptySpaceItem);
  acAddEmptySpaceItem.Hint := StripHotKey(acAddEmptySpaceItem.Caption);

  acAddSeparator.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddSeparatorItem);
  acAddSeparator.Hint := StripHotKey(acAddSeparator.Caption);

  acAddSplitter.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddSplitterItem);
  acAddSplitter.Hint := StripHotKey(acAddSplitter.Caption);

  acAddLabeledItem.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddLabeledItem);
  acAddLabeledItem.Hint := StripHotKey(acAddLabeledItem.Caption);

  acAddImage.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddImageItem);
  acAddImage.Hint := StripHotKey(acAddImage.Caption);

  acAddCheckBoxItem.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddCheckBoxItem);
  acAddCheckBoxItem.Hint := StripHotKey(acAddCheckBoxItem.Caption);

  acAddRadioButtonItem.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAddRadioButtonItem);
  acAddRadioButtonItem.Hint := StripHotKey(acAddRadioButtonItem.Caption);

  acAvailableItemsDelete.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormDelete);
  acAvailableItemsDelete.Hint := cxGetResourceString
    (@sdxLayoutControlCustomizeFormDeleteHint);

  acTreeViewItemsDelete.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormDelete);
  acTreeViewItemsDelete.Hint := cxGetResourceString
    (@sdxLayoutControlCustomizeFormDeleteHint);

  acAlignBy.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAlignBy);
  ...
  acTreeViewExpandAll.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormExpandAll);
  ...
  acTreeViewCollapseAll.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCollapseAll);
  ...
  acAlignLeftSide.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAlignLeftSide);
  acAlignRightSide.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAlignRightSide);
  acAlignTopSide.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAlignTopSide);
  acAlignBottomSide.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAlignBottomSide);
  acAlignNone.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormAlignNone);

  miAlignHorz.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormHAlign);
  acHAlignLeft.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormHAlignLeft);
  acHAlignCenter.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormHAlignCenter);
  acHAlignRight.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormHAlignRight);
  acHAlignClient.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormHAlignClient);
  acHAlignParent.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormHAlignParent);

  miAlignVert.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormVAlign);
  acVAlignTop.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormVAlignTop);
  acVAlignCenter.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormVAlignCenter);
  acVAlignBottom.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormVAlignBottom);
  acVAlignClient.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormVAlignClient);
  acVAlignParent.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormVAlignParent);

  miDirection.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormDirection);
  acDirectionHorizontal.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormDirectionHorizontal);
  acDirectionVertical.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormDirectionVertical);
  acDirectionTabbed.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormDirectionTabbed);

  miTextPosition.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTextPosition);
  acTextPositionLeft.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTextPositionLeft);
  acTextPositionTop.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTextPositionTop);
  acTextPositionRight.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTextPositionRight);
  acTextPositionBottom.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTextPositionBottom);

  miCaptionAlignHorz.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormCaptionAlignHorz);
  acCaptionAlignHorzLeft.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCaptionAlignHorzLeft);
  acCaptionAlignHorzCenter.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCaptionAlignHorzCenter);
  acCaptionAlignHorzRight.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCaptionAlignHorzRight);

  miCaptionAlignVert.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormCaptionAlignVert);
  acCaptionAlignVertTop.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCaptionAlignVertTop);
  acCaptionAlignVertCenter.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCaptionAlignVertCenter);
  acCaptionAlignVertBottom.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormCaptionAlignVertBottom);

  acCaption.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormItemCaption);

  acBorder.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormGroupBorder);
  acExpandButton.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormGroupExpandButton);

  lcgTreeView.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTreeViewGroup);
  lcgAvailableItems.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormListViewGroup);

  acTabbedView.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormTabbedView);
  acTabbedView.Hint := StripHotKey(acTabbedView.Caption);

  acAvailableItemsViewAsList.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormTreeView);
  acAvailableItemsViewAsList.Hint :=
    StripHotKey(acAvailableItemsViewAsList.Caption);

  acStore.Caption := cxGetResourceString(@sdxLayoutControlCustomizeFormStore);
  acStore.Hint := StripHotKey(acStore.Caption);

  acRestore.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormRestore);
  acRestore.Hint := StripHotKey(acRestore.Caption);

  acTreeViewItemRename.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormRename);
  acTreeViewItemRename.Hint := acTreeViewItemRename.Caption;

  acAvailableItemRename.Caption :=
    cxGetResourceString(@sdxLayoutControlCustomizeFormRename);
  ...
  acUngroup.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormUngroup);

  miCollapsible.Caption := cxGetResourceString
    (@sdxLayoutControlCustomizeFormSplitterCollapsible);
end;

procedure TdxLayoutControlCustomizeForm.RefreshItemsTreeView
  (ATreeView: TcxTreeView; AList: TList;
  AViewKind: TdxLayoutAvailableItemsViewKind; ANeedSort, AShowRoot: Boolean);
var
  I: Integer;
...
begin
  ...
  try
    ...
    for I := 0 to AList.Count - 1 do
      AddItemNode(ATreeView.Items, nil, TdxCustomLayoutItem(AList[I]),
        AViewKind = aivkTree);
    ...
    ATreeView.Items.EndUpdate;
    RestoreFirstNode(ATreeView, ATopItem); // TreeView bug
    CancelUpdate;
  end;
...
begin
  ...
  try
    ...
    RefreshItemsTreeView(tvAvailableItems, AList,
      Container.CustomizeAvailableItemsViewKind, True,
      Container.CustomizeAvailableItemsViewKind = aivkTree);
  finally
    AList.Free;
  end;
end;
...
procedure TdxLayoutControlCustomizeForm.RefreshVisibleItems;
  ...
  begin
    ...
    for I := 0 to TdxLayoutContainerAccess(Container)
      .FloatContainers.Count - 1 do
      AList.Add(TdxLayoutContainer(TdxLayoutContainerAccess(Container)
        .FloatContainers[I]).Root.Items[0]);
  end;
...
begin
  ...
  begin
    ...
    acAvailableItemsExpandAll.Enabled :=
      Container.CustomizeAvailableItemsViewKind = aivkTree;
    acAvailableItemsCollapseAll.Enabled :=
      Container.CustomizeAvailableItemsViewKind = aivkTree;
    acStore.Enabled := AStoringSupports;
    acRestore.Enabled := AStoringSupports and ACanRestore;
    acTreeViewItemRename.Enabled := ACanModify and
      (tvVisibleItems.Selected <> nil) and (tvVisibleItems.SelectionCount = 1)
      and not(TObject(tvVisibleItems.Selected.Data) as TdxCustomLayoutItem)
      .IsRoot and not TdxCustomLayoutItemAccess(tvVisibleItems.Selected.Data)
      .IsParentLocked and TdxLayoutContainerAccess(Container).AllowRename;
    acAvailableItemRename.Enabled := ACanModify and
      (tvAvailableItems.Selected <> nil) and
      (tvAvailableItems.SelectionCount = 1) and not TdxCustomLayoutItemAccess
      (tvAvailableItems.Selected.Data).IsParentLocked and
    ...
    acTreeViewItemsDelete.Enabled := CanDeleteItems(tvVisibleItems) and
      ACanAddItem;
    acAvailableItemsDelete.Enabled := CanDeleteItems(tvAvailableItems) and
      ACanAddItem;
    acVisibleItemsMakeFloat.Enabled := CanFloatItems(tvVisibleItems,
      ANeedCheckFloatButton);
    if acVisibleItemsMakeFloat.Enabled then
      acVisibleItemsMakeFloat.Checked := ANeedCheckFloatButton;
    acAvailableItemsMakeFloat.Enabled := CanFloatItems(tvAvailableItems,
      ANeedCheckFloatButton);
    ...
    acAddCustomItem.Enabled := acAddSeparator.Enabled or
      acAddSplitter.Enabled or acAddLabeledItem.Enabled or
      acAddEmptySpaceItem.Enabled;
    acAddCheckBoxItem.Enabled := ACanModify and ACanAddItem;
    acAddRadioButtonItem.Enabled := ACanModify and ACanAddItem;

    acAlignBy.Enabled := (tvVisibleItems.SelectionCount > 1) and ACanModify and
      ACanAddItem;
...
procedure TdxLayoutControlCustomizeForm.StoreTreeViewWndProc
  (ATreeView: TTreeView; out AWindowProcObject: TcxWindowProcLinkedObject;
  ANewWndMethod: TWndMethod);
begin
  AWindowProcObject := cxWindowProcController.Add(ATreeView, ANewWndMethod);
end;

procedure TdxLayoutControlCustomizeForm.RestoreTreeViewWndProc
  (var AWindowProcObject: TcxWindowProcLinkedObject);
begin
  cxWindowProcController.Remove(AWindowProcObject);
end;

procedure TdxLayoutControlCustomizeForm.AddItemNode(ANodes: TTreeNodes;
  AParentNode: TTreeNode; AItem: TdxCustomLayoutItem;
  AAddChildren: Boolean = True);
var
  I: Integer;
  AThisNode: TTreeNode;
...
procedure TdxLayoutControlCustomizeForm.AvailableItemsWndProc
...
function TdxLayoutControlCustomizeForm.DoCreateItem
  (AClass: TdxCustomLayoutItemClass; const ACaption: string)
  : TdxCustomLayoutItem;
begin
...
function TdxLayoutControlCustomizeForm.CreateItem
  (AClass: TdxCustomLayoutItemClass; const ACaption: string)
  : TdxCustomLayoutItem;
begin
  SaveToUndo;
...
procedure TdxLayoutControlCustomizeForm.DoAfterInsertionItem
  (AItem: TdxCustomLayoutItem);
var
  AIntf: IdxLayoutDesignerHelper;
begin
...
function TdxLayoutControlCustomizeForm.GetImageIndex
  ...
  case TdxCustomLayoutItemAccess(AItem).GetItemClassKind of
    ickGroup, ickAutoCreatedGroup:
      begin
        ...
        Result := AGroupIndexMap[TdxCustomLayoutItemAccess(AGroup).FIsFloat,
          AGroup.Locked, AGroup.LayoutDirection];
    ...
    ickLabeled:
      Result := 1;
    ickSeparator:
      Result := ASeparatorIndexMap[TdxLayoutSeparatorItemAccess(AItem)
        .IsVertical];
    ickSplitter:
...
function TdxLayoutControlCustomizeForm.GetMenuItems
...
function TdxLayoutControlCustomizeForm.HasClassInSelection
...
function TdxLayoutControlCustomizeForm.IsHiddenGroup
  (AItem: TdxCustomLayoutItem): Boolean;
begin
  Result := (AItem is TdxCustomLayoutGroup) and
    TdxCustomLayoutGroup(AItem).Hidden;
end;
...
function TdxLayoutControlCustomizeForm.GetCustomizationCaption
...
procedure TdxLayoutControlCustomizeForm.SetCustomizationCaption
...
procedure TdxLayoutControlCustomizeForm.RefreshLists(ARefreshSelection
  : Boolean = False);
begin
  if IsLocked then
    Exit;
...
begin
  acAvailableItemsViewAsList.Checked :=
    Container.CustomizeAvailableItemsViewKind = aivkTree;
  ...
  begin
    ...
    begin
      ...
      begin
        if Result then
          Break;
        Result := Result or TdxCustomLayoutItemAccess
          (TdxLayoutContainerAccess(Container).ManagedItems[I]).AllowFloating;
      end;
    end
    else
      Result := False;
...
const
  MainGroupDirectionMap: array [Boolean] of TdxLayoutDirection = (ldHorizontal,
    ldTabbed);
begin
  lcMainGroup1.LayoutDirection := MainGroupDirectionMap
    [Container.CustomizeFormTabbedView];
  ...
  lsSeparator4.Visible := (liUndo.Visible or liRedo.Visible) and
    (liShowDesignSelectors.Visible or liHighlightRoot.Visible or
    liStore.Visible or liRestore.Visible);
...
procedure TdxLayoutControlCustomizeForm.CalculateTreeViewPopupActionEnables;
  ...
  begin
    ...
    if not Result and (tvVisibleItems.SelectionCount > 1) then
      ...
      begin
        Result := TdxCustomLayoutItem(tvVisibleItems.Selections[I - 1].Data)
          .Parent = TdxCustomLayoutItem(tvVisibleItems.Selections[I]
          .Data).Parent;
        if not Result then
          Break;
      end;
...
procedure TdxLayoutControlCustomizeForm.
...
begin
  ...
  miBorder.Visible := AHasGroupsOnly and not AHasSpecialGroup and
    (cfmiBorder in AMenuItems);
  acExpandButton.Visible := AHasGroupsOnly and not AHasSpecialGroup and
    (cfmiExpandButton in AMenuItems);
  miTextPosition.Visible := (AHasGroup or AHasLabeledItem) and
    not AHasDirectionalItem and not AHasSpecialGroup and
    (cfmiCaptionLayout in AMenuItems);
  miCaptionAlignHorz.Visible := (AHasGroup or AHasLabeledItem) and
    not AHasDirectionalItem and not AHasSpecialGroup and
    (cfmiCaptionAlignHorz in AMenuItems);
  miCaptionAlignVert.Visible := (AHasGroup or AHasLabeledItem) and
    not AHasDirectionalItem and not AHasSpecialGroup and
    (cfmiCaptionAlignVert in AMenuItems);
  acCaption.Visible := (AHasGroup or AHasLabeledItem) and
    not AHasDirectionalItem and not AHasSpecialGroup and
    (cfmiCaption in AMenuItems);
  ...
  acUngroup.Visible := AHasGroupsOnly and not AHasSpecialGroup and
    (tvVisibleItems.SelectionCount = 1) and
    not TdxCustomLayoutItem(tvVisibleItems.Selected.Data).IsRoot and
...
begin
  Result := HasGroupInSelection and not HasLabeledItemInSelection and
    not HasDirectionalItemInSelection;
end;
...
begin
  ...
  for I := 0 to tvVisibleItems.SelectionCount - 1 do
  begin
    Result := IsHiddenGroup(TdxCustomLayoutItem(tvVisibleItems.Selections
      [I].Data));
    if Result then
      Break;
  end;
...
begin
  ...
  if not Container.IsDesigning then
    for I := 0 to tvVisibleItems.SelectionCount - 1 do
      ...
      begin
        Result := TdxCustomLayoutGroup(tvVisibleItems.Selections[I]
          .Data).Locked;
        if Result then
          Break;
      end;
...
begin
  ...
  for I := 0 to tvVisibleItems.SelectionCount - 1 do
  begin
    Result := TdxCustomLayoutItemAccess(tvVisibleItems.Selections[I].Data)
      .IsParentLocked;
    if Result then
      Break;
  end;
...
begin
  Result := HasClassInSelection(TdxLayoutSplitterItem) and
    not HasGroupInSelection and not HasLabeledItemInSelection;
end;
...
begin
  ...
  begin
    ...
    TAction(miTextPosition.Items[Integer(ACaptionOptions.Layout)].Action)
      .Checked := True;
    TAction(miCaptionAlignHorz.Items[CaptionAlignHorzMap
      [ACaptionOptions.AlignHorz]].Action).Checked := True;
    TAction(miCaptionAlignVert.Items[Integer(ACaptionOptions.AlignVert)].Action)
      .Checked := True;
    ...
    if AItem is TdxCustomLayoutGroup then
    begin
      TAction(miDirection.Items[Integer(TdxCustomLayoutGroup(AItem)
        .LayoutDirection)].Action).Checked := True;
      acBorder.Checked := TdxCustomLayoutGroup(AItem).ShowBorder;
      acExpandButton.Checked := TdxCustomLayoutGroup(AItem)
        .ButtonOptions.ShowExpandButton;
    end;
  end;
end;

function TdxLayoutControlCustomizeForm.CanSelectItem
  (AItem: TdxCustomLayoutItem): Boolean;
begin
  Result := (AItem <> nil) and not TdxCustomLayoutItemAccess(AItem)
    .IsDestroying and Supports(Container, IdxLayoutDesignerHelper);
end;
...
procedure TdxLayoutControlCustomizeForm.SetLayoutItemsSelections
  (ATreeView: TTreeView);
var
  AList: TList;
  AIntf: IdxLayoutDesignerHelper;
...
function TdxLayoutControlCustomizeForm.TreeViewWndProcHandler
  ...
  begin
    ...
    if (htOnItem in AHitTest) then
      ATreeView.Perform(WM_CONTEXTMENU, ATreeView.Handle,
        dxPointToLParam(GetMouseCursorPos));
  end;
...
procedure TdxLayoutControlCustomizeForm.RestoreCollapsedNodes
  (ATreeView: TcxTreeView; AList: TcxComponentList);
var
  I: Integer;
  ANode: TTreeNode;
begin
...
procedure TdxLayoutControlCustomizeForm.RestoreFirstNode(ATreeView: TcxTreeView;
  AItem: TdxCustomLayoutItem);
var
  ANode: TTreeNode;
begin
...
procedure TdxLayoutControlCustomizeForm.StoreFirstNode(ATreeView: TcxTreeView;
  out AItem: TdxCustomLayoutItem);
begin
...
begin
  ...
  try
    ...
    try
      cxTreeViewGetSelection(tvVisibleItems.InnerTreeView, AList);
      for I := 0 to AList.Count - 1 do
        TdxLayoutSplitterItem(AList[I]).AllowCloseOnClick :=
          acCollapsible.Checked;
      Container.Modified;
    finally
      AList.Free;
    end;
...
procedure TdxLayoutControlCustomizeForm.acAddCheckBoxItemExecute
  (Sender: TObject);
begin
  CreateItem(TdxLayoutCheckBoxItem,
    cxGetResourceString(@sdxLayoutControlNewCheckBoxItemCaption));
end;
...
procedure TdxLayoutControlCustomizeForm.acAddEmptySpaceItemExecute
  (Sender: TObject);
var
  AItem: TdxCustomLayoutItem;
begin
  AItem := CreateItem(TdxLayoutEmptySpaceItem,
    cxGetResourceString(@sdxLayoutControlNewEmptySpaceItemCaption));
  AItem.Width := 10;
  AItem.Height := 10;
end;
...
begin
  CreateItem(TdxLayoutSeparatorItem,
    cxGetResourceString(@sdxLayoutControlNewSeparatorItemCaption));
end;
...
procedure TdxLayoutControlCustomizeForm.acAddLabeledItemExecute
  (Sender: TObject);
begin
  CreateItem(TdxLayoutContainerAccess(Container).GetDefaultLabelClass,
    cxGetResourceString(@sdxLayoutControlNewLabeledItemCaption));
end;

procedure TdxLayoutControlCustomizeForm.acAddRadioButtonItemExecute
  (Sender: TObject);
begin
  CreateItem(TdxLayoutRadioButtonItem,
    cxGetResourceString(@sdxLayoutControlNewRadioButtonItemCaption));
end;
...
procedure TdxLayoutControlCustomizeForm.tvVisibleItemsAdvancedCustomDrawItem
  (Sender: TCustomTreeView; Node: TTreeNode; State: TCustomDrawState;
  Stage: TCustomDrawStage; var PaintImages, DefaultDraw: Boolean);
begin
end;

procedure TdxLayoutControlCustomizeForm.tvVisibleItemsEdited(Sender: TObject;
...
begin
  ...
  begin
    ...
    if ANode = nil then
      Exit;
    if (Button = mbLeft) and ANode.Selected and (ANode = ATreeView.Selected)
    then
      DragHelper.InitializeDragItem(ATreeView.Selected.Data, X, Y);
  end;
end;

procedure TdxLayoutControlCustomizeForm.tvVisibleItemsMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxLayoutControlCustomizeForm.tvVisibleItemsMouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxLayoutControlCustomizeForm.tvVisibleItemsChange(Sender: TObject;
  Node: TTreeNode);
begin
  if not IsLocked then
...
procedure TdxLayoutControlCustomizeForm.tvVisibleItemsChanging(Sender: TObject;
...
procedure TdxLayoutControlCustomizeForm.acTreeViewItemsDeleteExecute
  (Sender: TObject);
var
...
procedure TdxLayoutControlCustomizeForm.tvVisibleItemsCustomDrawItem
  ...
  begin
    Result := (dxLayoutDragAndDropObject <> nil) and
      (TdxLayoutDragAndDropObjectAccess(dxLayoutDragAndDropObject)
      .DestinationItem = AItem);
  end;

begin
  if Node.Deleting then
    Exit;
  if (TObject(Node.Data) is TdxCustomLayoutGroup) and
    TdxCustomLayoutGroupAccess(Node.Data).AutoCreated and
...
procedure TdxLayoutControlCustomizeForm.acAvailableItemsViewAsListExecute
  (Sender: TObject);
const
  AViewKindMap: array [Boolean] of TdxLayoutAvailableItemsViewKind = (aivkList,
    aivkTree);
begin
  Container.CustomizeAvailableItemsViewKind :=
    AViewKindMap[acAvailableItemsViewAsList.Checked];
end;
...
var
  ...
  AEditingText: string;
begin
  AllowEdit := not(TdxCustomLayoutItem(Node.Data).IsRoot or
    (DragHelper.DragItem <> nil) or IsKeyDown([ssCtrl, ssShift]) or
    TdxCustomLayoutItemAccess(Node.Data).IsParentLocked) and
    TdxLayoutContainerAccess(Container).AllowRename;
    ...
    VK_ESCAPE:
      begin
        ...
        if Handled then
          if tvVisibleItems.Selected <> nil then
            tvVisibleItems.Selected.EndEdit(True)
          else if tvAvailableItems.Selected <> nil then
...
begin
  ...
  try
    cxTreeViewGetSelection(tvVisibleItems.InnerTreeView, AList);
    for I := 0 to AList.Count - 1 do
      TdxCustomLayoutItem(AList[I]).AlignHorz :=
        TdxLayoutAlignHorz((Sender as TAction).Tag);
    Container.Modified;
  finally
    AList.Free;
  end;
...
procedure TdxLayoutControlCustomizeForm.pmAvailableItemsActionsPopup
  (Sender: TObject);
begin
...
begin
  ...
  try
    cxTreeViewGetSelection(tvVisibleItems.InnerTreeView, AList);
    for I := 0 to AList.Count - 1 do
      TdxCustomLayoutItem(AList[I]).AlignVert :=
        TdxLayoutAlignVert((Sender as TAction).Tag);
    Container.Modified;
  finally
    AList.Free;
  end;
...
begin
  ...
  try
    ...
    begin
      if TObject(AList[I]) is TdxCustomLayoutGroup then
        TdxCustomLayoutGroup(AList[I]).ShowBorder :=
...
procedure TdxLayoutControlCustomizeForm.tvVisibleItemsDeletion(Sender: TObject;
  Node: TTreeNode);
var
  ANode: TTreeNode;
begin
  if (csDestroying in ComponentState) or not Container.IsDesigning and IsLocked
  then
    Exit;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        if TObject(AList[I]) is TdxCustomLayoutGroup then
          TdxCustomLayoutGroup(AList[I]).ButtonOptions.ShowExpandButton :=
            acExpandButton.Checked;
      end;
      Container.Modified;
    finally
      AList.Free;
...
begin
  ...
  try
    ...
    try
      cxTreeViewGetSelection(tvVisibleItems.InnerTreeView, AList);
      for I := 0 to AList.Count - 1 do
        GetCaptionOptions(TdxCustomLayoutItem(AList[I])).Layout :=
          TdxCaptionLayout((Sender as TAction).Tag);
      Container.Modified;
    finally
      AList.Free;
    end;
...
begin
  ...
  try
    ...
    try
      cxTreeViewGetSelection(tvVisibleItems.InnerTreeView, AList);
      for I := 0 to AList.Count - 1 do
        GetCaptionOptions(TdxCustomLayoutItem(AList[I])).Visible :=
          acCaption.Checked;
      Container.Modified;
    finally
      AList.Free;
...
begin
  ...
  try
    ...
    try
      cxTreeViewGetSelection(tvVisibleItems.InnerTreeView, AList);
      for I := 0 to AList.Count - 1 do
        TdxCustomLayoutItemCaptionOptionsAccess(TdxCustomLayoutItem(AList[I])
          .CaptionOptions).AlignVert :=
          TdxAlignmentVert((Sender as TAction).Tag);
      Container.Modified;
    finally
      AList.Free;
    end;
...
const
  LayoutDirectionMap: array [TdxLayoutDirection] of TdxLayoutDirection =
    (ldHorizontal, ldVertical, ldVertical);

  function GetIndexByItem(AParent: TdxCustomLayoutGroup;
    AItem: TdxCustomLayoutItem): Integer;
  begin
  ...
  begin
    ...
    for I := 0 to AList.Count - 1 do
      Result := Min(Result, GetIndexByItem(AParent,
        TdxCustomLayoutItem(AList[I])));
  end;
...
begin
  ...
  try
    ...
    try
      ...
      AGroup := DoCreateItem(TdxLayoutContainerAccess(Container)
        .GetDefaultGroupClass,
        cxGetResourceString(@sdxLayoutControlNewGroupCaption))
...
begin
  CreateItem(TdxLayoutImageItem,
    cxGetResourceString(@sdxLayoutControlNewImageItemCaption));
end;
