...
uses
  ...
  dxCore, cxCheckBox, cxCheckComboBox, cxCheckGroup,
  ...
  cxExtEditConsts, cxExtEditRepositoryItems, cxFontNameComboBox, cxHeader,
  cxHint,
  cxHintEditor, cxLabel, cxListView, cxLookAndFeels, cxMCListBox, cxProgressBar,
  cxPropEditors, cxRichEdit, cxScrollBar, cxSpinButton, cxSplitter,
  cxSplitterEditor,
  cxTrackBar, dxZoomTrackBar, dxDBZoomTrackBar, cxTreeView, dxScreenTip,
  dxCustomHint, dxTaskbarProgress,
  dxColorEdit, dxDBColorEdit, cxGraphics, cxLibraryReg, dxRatingControl,
  dxDBRatingControl, dxGDIPlusClasses,
  dxRangeTrackBar, dxDBRangeTrackBar, dxRangeControl, dxUIAdorners,
  dxUIAdornerTargetElementPathEditor,
  dxMeasurementUnitEdit, dxTreeView, dxListView, dxListViewEditor, dxCoreReg;

const
  cxEditorsRestoreStyleCaption = 'Restore style';
  cxEditorsRestoreLookAndFeelCaption = 'Restore LookAndFeel';
  cxEditComponentEditorVerbS: array [0 .. 0] of string =
    (cxEditorsRestoreStyleCaption);
  cxEditComponentEditorVerbL: array [0 .. 0] of string =
...
type
  ...
  protected
    ...
    procedure SelectObject(AObject: TPersistent;
      AClearSelection: Boolean); override;
  ...
  protected
    ...
    procedure SynchronizeControlCheckStates
      (ADialog: TcxCheckGroupStatesEditorDlg); virtual;
  public
    procedure Edit; override;
    function GetAttributes: TPropertyAttributes; override;
  ...
  protected
    ...
    procedure SynchronizeControlCheckStates
      (ADialog: TcxCheckGroupStatesEditorDlg); override;
  end;
  ...
  TcxCheckComboBoxStatesItemsProperty = class
    (TcxChecksControlStatesItemsProperty)
  protected
    procedure InitializeDlg(ADialog: TcxCheckGroupStatesEditorDlg); override;
    procedure SynchronizeControlCheckStates
  ...
  { TcxListGroupImageIndexProperty }

  TcxListGroupImageIndexProperty = class(TImageIndexProperty)
  public
    function GetImages: TCustomImageList; override;
  end;

  { TcxListColumnImageIndexProperty }

  TcxListColumnImageIndexProperty = class(TImageIndexProperty)
  public
    function GetImages: TCustomImageList; override;
  end;
...
procedure TdxUIAdornerManagerComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  case AIndex of
    0:
      ShowAdornersEditor(Manager.Badges);
    1:
      ShowAdornersEditor(Manager.Guides);
  end;
end;

function TdxUIAdornerManagerComponentEditor.InternalGetVerb
...
procedure TdxUIAdornerManagerComponentEditor.ShowAdornersEditor
  (AAdorners: TdxCustomAdorners);
begin
  ShowFormEditorClass(Designer, Manager, AAdorners, TdxAdornersAccess(AAdorners)
    .GetName, TfrmComponentCollectionEditor);
end;
...
function TdxUIAdornerManagerDesignHelper.IsObjectSelected
  (AObject: TPersistent): Boolean;
begin
  Result := DesignHelper.IsObjectSelected(AObject);
end;

procedure TdxUIAdornerManagerDesignHelper.SelectObject(AObject: TPersistent;
...
begin
  ...
  Result := Result + [paValueEditable, paValueList, paSortList, paRevertable,
...
begin
  ATargetElementClass := TdxAdornerCustomTargetElementClass
    (dxRegisteredAdornerTargetElementTypes.FindByClassName(Value));
  if ATargetElementClass = nil then
    ATargetElementClass := TdxAdornerCustomTargetElementClass
      (dxRegisteredAdornerTargetElementTypes.FindByDescription(Value));
  for I := 0 to PropCount - 1 do
    if GetComponent(I) is TdxCustomAdorner then
      TdxCustomAdorner(GetComponent(I)).TargetElementClass :=
        ATargetElementClass;
  Modified;
end;
...
function TdxAdornerTargetElementPathProperty.GetTargetElement
...
begin
  ShowFormEditorClass(Designer, Adorners.Manager, Adorners,
    TdxAdornersAccess(Adorners).GetName, TfrmComponentCollectionEditor);
end;
...
begin
  ...
  if FHotZoneStyleClass = nil then
    FHotZoneStyleClass := TcxHotZoneStyleClass
      (GetRegisteredHotZoneStyles.FindByDescription(Value));
...
begin
  ...
  if GetComponent(0) is TcxHeaderSection then
    Result := TCustomImageList(TcxHeaderSection(GetComponent(0))
      .HeaderControl.Images);
end;
...
begin
  if GetController.HintStyle is TcxHintStyle then
    ...
    ShowCollectionEditor(Designer, Component,
      (GetController.HintStyle as TdxScreenTipStyle).ScreenTipLinks,
      'ScreenTipLinks');
end;
...
procedure TcxChecksControlStatesItemsProperty.InitializeDlg
  (ADialog: TcxCheckGroupStatesEditorDlg);
begin
end;

procedure TcxChecksControlStatesItemsProperty.SynchronizeControlCheckStates
  (ADialog: TcxCheckGroupStatesEditorDlg);
begin
end;
...
procedure TcxCheckGroupStatesItemsProperty.SynchronizeControlCheckStates
  (ADialog: TcxCheckGroupStatesEditorDlg);
var
  I: Integer;
begin
...
begin
  ShowCollectionEditor(Designer, Component,
    TcxCheckComboBoxProperties(TcxCheckComboBox(Component).Properties)
    .Items, 'Items');
end;

{ TcxCheckComboBoxStatesItemsProperty }

procedure TcxCheckComboBoxStatesItemsProperty.InitializeDlg
  (ADialog: TcxCheckGroupStatesEditorDlg);
var
...
procedure TcxCheckComboBoxStatesItemsProperty.SynchronizeControlCheckStates
  (ADialog: TcxCheckGroupStatesEditorDlg);
var
  I: Integer;
begin
...
begin
  for I := 0 to PropCount - 1 do
  begin
    Result := TcxHintStyleControllerAccess(GetComponent(I)).HintStyle <> nil;
    // to do
    if not Result then
      Exit;
  end;
...
begin
  if HasSubProperties then
    Result := cxRegisteredHintStyles.GetDescriptionByClass
      (TcxCustomHintStyle(GetOrdValue).ClassType)
  else
    Result := '';
end;
...
begin
  FHintStyleClass := TcxHintStyleClass
    (cxRegisteredHintStyles.FindByClassName(Value));
  if FHintStyleClass = nil then
    FHintStyleClass := TcxHintStyleClass
      (cxRegisteredHintStyles.FindByDescription(Value));

  ObjectInspectorCollapseProperty;
  for I := 0 to PropCount - 1 do
    TcxHintStyleControllerAccess(GetComponent(I)).HintStyleClass :=
      FHintStyleClass;
{$IFNDEF DELPHI15}
...
function TdxRangeControlClientPropertiesProperty.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes;
  if not HasSubProperties then
...
begin
  if HasSubProperties then
    Result := dxRangeControlClients.GetDescriptionByClass
      (TdxRangeControlCustomClientProperties(GetOrdValue).ClassType)
  else
    Result := '';
end;
...
begin
  AClientPropertiesClass := TdxRangeControlClientPropertiesClass
    (dxRangeControlClients.FindByClassName(Value));
  if AClientPropertiesClass = nil then
    AClientPropertiesClass := TdxRangeControlClientPropertiesClass
      (dxRangeControlClients.FindByDescription(Value));

  ObjectInspectorCollapseProperty;
  for I := 0 to PropCount - 1 do
    TdxRangeControl(GetComponent(I)).ClientPropertiesClass :=
      AClientPropertiesClass;
{$IFNDEF DELPHI15}
  UpdateObjectInspector(Designer);
{$ENDIF}
  Modified;
...
const
  AScaleNames: array [rcduMinute .. rcduYear] of string = ('Minute', 'Hour',
...
begin
  Result := [paValueList, paMultiSelect, paRevertable, paValueEditable,
    paReadOnly];
end;
...
begin
  AScaleUnit := rcduDay;
  for ACurrentScaleUnit := rcduMinute to High
    (TdxRangeControlDateTimeScaleUnit) do
...
function TdxRangeControlNumericClientRangeValueTypeProperty.GetAttributes
  : TPropertyAttributes;
begin
...
begin
  for I := varSmallint to varInt64 do
    if TdxRangeControlNumericClientProperties(GetComponent(0))
      .IsRangeValueTypeSupported(I) then
      Proc(VarTypeNames[I]);
end;

procedure TdxRangeControlNumericClientRangeValueTypeProperty.SetValue
  (const Value: string);
...
procedure TdxTaskbarProgressLinkedComponentProperty.GetValues
  (Proc: TGetStrProc);
begin
  FProc := Proc;
  inherited GetValues(CheckComponent);
...
procedure TdxTaskbarProgressLinkedComponentProperty.CheckComponent
  (const Value: string);
var
  AComponent: TComponent;
begin
...
const
  AMethodParams: array [0 .. 0] of TMethodParam = ((Flags: [pfAddress];
    Name: 'Sender'; TypeName: 'TObject'));
begin
...
begin
  ShowCollectionEditor(Designer, Component,
    TcxCheckGroupProperties(TcxCheckGroup(GetComponent).Properties)
    .Items, 'Items');
end;

{ TcxCheckBoxBasedControlSelectionEditor }

procedure TcxCheckBoxBasedControlSelectionEditor.RequiresUnits
  (Proc: TGetStrProc);
begin
  Proc('cxCheckBox');
end;
...
begin
  ...
  try
    Designer.GetComponentNames(GetTypeData(PTypeInfo(TdxRangeControl.ClassInfo)
      ), AddComponent);
...
begin
  Result := TcxCustomInnerCheckListBox
    (GetPersistentOwner(TcxCheckListBoxItem(GetComponent(0)).Collection))
    .Container.Images;
end;
...
begin
  Result := 'ExpressListView Control';
end;
...
begin
  case AIndex of
    0:
      dxEditListViewItems((Component as TdxListView).Items);
  else
...
function TcxListGroupImageIndexProperty.GetImages: TCustomImageList;
begin
  if GetComponent(0) is TdxListGroup then
    Result := TdxListGroup(GetComponent(0)).ListView.Images.GroupHeaderImages
  else
    Result := nil;
end;
...
function TcxListColumnImageIndexProperty.GetImages: TCustomImageList;
begin
  if GetComponent(0) is TdxListColumn then
    Result := TdxListColumn(GetComponent(0)).ListView.Images.ColumnHeaderImages
  else
    Result := nil;
end;
...
begin
  RegisterEditRepositoryItem(TcxEditRepositoryLabel,
    scxSEditRepositoryLabelItem);
  RegisterEditRepositoryItem(TcxEditRepositoryFontNameComboBox,
    scxSEditRepositoryFontNameComboBoxItem);
  RegisterEditRepositoryItem(TcxEditRepositoryColorComboBox,
    scxSEditRepositoryColorComboBoxItem);
  RegisterEditRepositoryItem(TcxEditRepositoryColorEdit,
    scxSEditRepositoryColorEditItem);
  RegisterEditRepositoryItem(TcxEditRepositoryProgressBar,
    scxSEditRepositoryProgressBarItem);
  RegisterEditRepositoryItem(TcxEditRepositoryTrackBar,
    scxSEditRepositoryTrackBarItem);
  RegisterEditRepositoryItem(TcxEditRepositoryRangeTrackBar,
    scxSEditRepositoryRangeTrackBarItem);
  RegisterEditRepositoryItem(TcxEditRepositoryCheckComboBox,
    scxSEditRepositoryCheckComboBox);
  RegisterEditRepositoryItem(TcxEditRepositoryCheckGroupItem,
    scxSEditRepositoryCheckGroupItem);
  RegisterEditRepositoryItem(TcxEditRepositoryRichItem,
    scxSEditRepositoryRichEditItem);
  RegisterEditRepositoryItem(TcxEditRepositoryRatingControl,
    scxSEditRepositoryRatingControlItem);
end;
...
begin
  ...
  RegisterComponents(cxEditorsLibraryProductPage, [TcxLabel, TcxProgressBar,
    TcxTrackBar, TdxZoomTrackBar, TcxCheckListBox, TcxColorComboBox,
    TcxFontNameComboBox, TcxCheckComboBox, TcxCheckGroup, TcxRichEdit,
    TdxColorEdit, TdxRatingControl, TdxRangeTrackBar, TdxRangeControl]);
  RegisterNoIcon([TdxMeasurementUnitEdit, TdxInternalTreeView, TdxListView]);
  RegisterNoIcon([TdxListGroup, TdxListColumn]);
  RegisterComponents(cxEditorsDBLibraryProductPage,
    [TcxDBLabel, TcxDBProgressBar, TcxDBTrackBar, TdxDBZoomTrackBar,
    TcxDBCheckListBox, TcxDBColorComboBox, TcxDBFontNameComboBox,
    TcxDBCheckComboBox, TcxDBCheckGroup, TcxDBRichEdit, TdxDBColorEdit,
    TdxDBRatingControl, TdxDBRangeTrackBar]);
  RegisterComponents(cxEditorsUtilitiesProductPage,
    [TcxHintStyleController, TcxSpinButton, TcxMCListBox, TcxListView,
    TcxTreeView, TcxHeader, TcxSplitter, TdxTaskbarProgress,
    TdxUIAdornerManager]);
  RegisterClasses([TdxBadge, TdxGuide]);
  RegisterNoIcon([TdxBadge, TdxGuide]);

  RegisterPropertyEditor(TypeInfo(string), TdxAdornerTargetElementPath, 'Path',
    TdxAdornerTargetElementPathProperty);
  RegisterPropertyEditor(TypeInfo(string), TdxCustomAdorner,
    'TargetElementClassName', nil);
  RegisterPropertyEditor(TypeInfo(TdxAdornerCustomTargetElement),
    TdxCustomAdorner, 'TargetElement', TdxAdornerTargetElementProperty);
  RegisterPropertyEditor(TypeInfo(TShortCut), TdxGuides, 'DeactivateKey',
    TShortCutProperty);
  RegisterPropertyEditor(TypeInfo(TdxCustomAdorners), TdxUIAdornerManager, '',
    TdxAdornersProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxCustomSplitter,
    'HotZoneClassName', nil);
  RegisterPropertyEditor(TypeInfo(Boolean), TcxCustomSplitter,
    'ResizeIgnoreSnap', nil);
  RegisterPropertyEditor(TypeInfo(TcxHotZoneStyle), TcxCustomSplitter,
    'HotZone', TcxHotZoneStyleProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomSplitter,
    'HotZoneEvents', TcxHotZoneStyleEventsProperty);
  RegisterPropertyEditor(TypeInfo(TcxEditValue), TcxCustomCheckGroup,
    'EditValue', nil);
  RegisterPropertyEditor(TypeInfo(Boolean), TcxCustomCheckGroup, 'StatesItems',
    TcxCheckGroupStatesItemsProperty);
  RegisterPropertyEditor(TypeInfo(TdxSmartGlyph), TcxCustomCheckGroupProperties,
    'Glyph', TdxMultiPartGlyphGraphicProperty);
  RegisterPropertyEditor(TypeInfo(Boolean), TcxCustomCheckComboBox,
    'StatesItems', TcxCheckComboBoxStatesItemsProperty);
  RegisterPropertyEditor(TypeInfo(TcxEditValue), TcxCustomCheckListBox,
    'EditValue', nil);
  RegisterPropertyEditor(TypeInfo(TMeasureItemEvent), TcxCustomCheckListBox,
    'OnMeasureItem', nil);
  RegisterPropertyEditor(TypeInfo(TdxSmartGlyph), TcxCustomCheckListBox,
    'Glyph', TdxMultiPartGlyphGraphicProperty);
  RegisterPropertyEditor(TypeInfo(TcxEditValue), TcxCustomCheckComboBox,
    'EditValue', nil);
  RegisterPropertyEditor(TypeInfo(Variant), TcxCustomCheckComboBox,
    'Value', nil);
  RegisterPropertyEditor(TypeInfo(Boolean), TcxCustomSpinButton,
    'AutoSize', nil);
  ...
  RegisterPropertyEditor(TypeInfo(Boolean), TcxCustomCheckComboBoxProperties,
    'AllowGrayed', nil);
  RegisterPropertyEditor(TypeInfo(TdxSmartGlyph),
    TcxCustomCheckComboBoxProperties, 'Glyph',
    TdxMultiPartGlyphGraphicProperty);
  RegisterPropertyEditor(TypeInfo(TcxProgressBarPropertiesValues),
    TcxCustomProgressBarProperties, 'AssignedValues', nil);
  RegisterPropertyEditor(TypeInfo(TComponent), TdxTaskbarCustomProgress,
    'LinkedComponent', TdxTaskbarProgressLinkedComponentProperty);
  RegisterPropertyEditor(TypeInfo(Integer), TcxCustomListView,
    'ItemIndex', nil);
  RegisterPropertyEditor(TypeInfo(TcxCustomHintStyle),
    TcxCustomHintStyleController, 'HintStyle', TcxHintStyleProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxCustomHintStyleController,
    'HintStyleClassName', nil);
  RegisterPropertyEditor(TypeInfo(TdxRangeControlCustomClientProperties),
    TdxRangeControl, 'ClientProperties',
    TdxRangeControlClientPropertiesProperty);
  RegisterPropertyEditor(TypeInfo(TdxRangeControlDateTimeScale),
    TdxRangeControlDateTimeClientProperties, 'Scale',
    TdxRangeControlPrimaryScaleProperty);
  RegisterPropertyEditor(TypeInfo(TdxRangeControlDateTimeScale),
    TdxRangeControlCustomDateTimeHeaderClientProperties, 'PrimaryScale',
    TdxRangeControlPrimaryScaleProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxRangeEditValue), TdxCustomRangeControl, '',
    TdxRangeControlRangeValueProperty);
  RegisterPropertyEditor(TypeInfo(TdxRangeEditValue),
    TdxRangeControlCustomClientProperties, '',
    TdxRangeControlRangeValueProperty);
  RegisterPropertyEditor(TypeInfo(TComponent), TdxRangeControl, 'Client',
    TdxRangeControlClientProperty);
  ...
  RegisterSelectionEditor(TcxHintStyleController,
    TcxHintStyleControllerSelectionEditor);
  RegisterSelectionEditor(TcxCheckComboBox,
    TcxCheckBoxBasedControlSelectionEditor);
  RegisterSelectionEditor(TcxCheckListBox,
    TcxCheckBoxBasedControlSelectionEditor);
  RegisterSelectionEditor(TcxEditRepositoryCheckComboBox,
    TcxCheckBoxBasedControlSelectionEditor);
  RegisterSelectionEditor(TdxRangeControl, TdxRangeControlSelectionEditor);

  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxCheckListBoxItem,
    'ImageIndex', TcxCheckListBoxItemImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxHeaderSection,
    'ImageIndex', TcxHeaderSectionImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxBadgeBackground,
    'ImageIndex', TdxBadgesImageIndexProperty);
  ...
  RegisterComponentEditor(TcxCustomHeader,
  ...
  RegisterComponentEditor(TdxTaskbarProgress,
    TcxCustomEditorsLibraryComponentEditor);
  RegisterComponentEditor(TdxRangeControl,
    TcxCustomEditorsLibraryComponentEditor);
  RegisterComponentEditor(TdxUIAdornerManager,
    TdxUIAdornerManagerComponentEditor);

  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TdxRangeControl,
    'ClientPropertiesEvents', TdxRangeControlClientEventsProperty);
  RegisterComponentEditor(TdxCustomListView, TdxListViewComponentEditor);
  RegisterPropertyEditor(TypeInfo(TImageIndex), TdxListGroup, 'TitleImage',
    TcxListGroupImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TImageIndex), TdxListColumn, 'ImageIndex',
    TcxListColumnImageIndexProperty);
end;
