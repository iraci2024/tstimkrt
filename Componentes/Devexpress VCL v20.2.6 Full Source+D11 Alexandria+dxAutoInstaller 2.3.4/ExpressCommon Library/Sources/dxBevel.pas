...
type
  ...
  TdxCustomBevel = class(TGraphicControl, IcxLookAndFeelContainer,
    IdxScaleFactor, IdxSkinSupport)
  strict private
    FcxCanvas: TcxCanvas;
...
{$ENDIF}
  ...
  public
    ...
    property Style: TdxBevelStyle read FStyle write SetStyle
      default dxbsLowered;
  end;
...
begin
  Result := IsHorzLineShape(AShape) and
    (Align in [alTop, alBottom, alCustom, alNone]);
end;

function TdxCustomBevel.CanAutoWidth(AShape: TdxBevelShape): Boolean;
begin
  Result := IsVertLineShape(AShape) and
    (Align in [alLeft, alRight, alCustom, alNone]);
end;
...
begin
  ...
  else if CanAutoHeight(Shape) then
...
procedure TdxCustomBevel.LookAndFeelChangeHandler(Sender: TcxLookAndFeel;
  ...
  begin
    ...
    begin
      if CanAutoHeight(APrevShape) and CanAutoWidth(Shape) or
        CanAutoWidth(APrevShape) and CanAutoHeight(Shape) then
