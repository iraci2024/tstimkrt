...
type
  ...
  TcxFilterItem = class(TcxInterfacedCollectionItem,
  ...
  public
    ...
    function GetProperties(AClass: TcxCustomEditPropertiesClass)
  ...
  public
    ...
    property Items[Index: Integer]: TcxFilterItem read GetItems
  ...
  public
    ...
    procedure ProcessFilterOperators(var SupportedOperations
      : TcxFilterControlOperators);
  published
    property DateTimeFormat: string read GetDateTimeFormat
      write SetDateTimeFormat;
    property PercentWildcard: Char read GetPercentWildcard
      write SetPercentWildcard default '%';
    property SoftNull: Boolean read GetSoftNull write SetSoftNull default False;
    property SupportedBetween: Boolean read FSupportedBetween
      write FSupportedBetween default True;
    property SupportedIn: Boolean read FSupportedIn write FSupportedIn
      default True;
    property SupportedLike: Boolean read FSupportedLike write FSupportedLike
      default True;
    property TranslateBetween: Boolean read GetTranslateBetween
      write SetTranslateBetween default False;
    property TranslateIn: Boolean read GetTranslateIn write SetTranslateIn
      default False;
    property TranslateLike: Boolean read GetTranslateLike write SetTranslateLike
      default False;
    property UnderscoreWildcard: Char read GetUnderscoreWildcard
  ...
  protected
    ...
    procedure CorrectOperatorClass(var AOperatorClass
      : TcxFilterOperatorClass); override;
    ...
    procedure ValidateConditions(var SupportedOperations
      : TcxFilterControlOperators); override;
  ...
  public
    ...
    procedure Clear; override;
    function GetFilterTextEx(const AFieldNamePrefix, AFieldNamePostfix
      : string): string;
    function GetPropertiesClassFromFieldType(AFieldType: TFieldType)
  ...
  published
    ...
    property DragMode;
    property Enabled;
    property FilterOptions: TcxDBFilterOptions read GetFilterOptions
...
begin
  if FCaption <> '' then
    ...
    Result := FieldName
  else if GetProperties <> nil then
...
procedure TcxFilterItem.SetPropertiesClass(const Value
  : TcxCustomEditPropertiesClass);
begin
  if (FPropertiesClass <> Value) and ((Value = nil) or IsSupportFiltering(Value))
  then
  begin
...
procedure TcxFilterItem.SetRepositoryItem(const Value: TcxEditRepositoryItem);
begin
  if FRepositoryItem <> Value then
  begin
    if (Value <> nil) and
      not(esoFiltering in Value.Properties.GetSupportedOperations) then
      Exit;
    if FRepositoryItem <> nil then
...
procedure TcxDBFilterOptions.ProcessFilterOperators(var SupportedOperations
  : TcxFilterControlOperators);
begin
...
function TcxDBFilterControl.GetFilterTextEx(const AFieldNamePrefix,
  AFieldNamePostfix: string): string;
begin
...
begin
  if (FItems <> nil) and (FItems.Count > 0) then
    Result := FItems.Count
  else if (FDataSet <> nil) and not(IsDefaultFields(FDataSet) and
    (FDataSource.State = dsInactive)) then
  begin
    Result := 0;
    with FDataSet do
...
function TcxDBFilterControl.GetProperties(Index: Integer)
  : TcxCustomEditProperties;
begin
  if (Index > -1) and (Index < GetCount) then
    if FItems.Count > 0 then
...
function TcxDBFilterControl.GetValueType(Index: Integer): TcxValueTypeClass;
  ...
  begin
    ...
    while (Length(Result) > 0) and
      ...
      Delete(Result, 1, 1);
    while (Length(Result) > 0) and (AnsiChar(Result[Length(Result)])
      in [#0 .. ' ', '"', '''', ']']) do
...
begin
  if ALink = nil then
    DataSet := nil
  else if ALink is TDataSet then
    DataSet := TDataSet(ALink);
end;
...
procedure TcxDBFilterControl.CorrectOperatorClass(var AOperatorClass
...
procedure TcxDBFilterControl.ValidateConditions(var SupportedOperations
  : TcxFilterControlOperators);
begin
