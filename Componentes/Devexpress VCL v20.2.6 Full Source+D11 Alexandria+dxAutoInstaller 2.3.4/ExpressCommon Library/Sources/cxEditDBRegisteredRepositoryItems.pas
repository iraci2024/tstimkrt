...
uses
  Variants,
  SysUtils, Classes, DB, Generics.Collections, cxClasses, cxGraphics,
...
type
  ...
  public
    ...
    function GetItem(AFieldType: TFieldType): TcxEditRepositoryItem;
      reintroduce;
    function GetItemByField(AField: TField): TcxEditRepositoryItem;
    function GetItemByDataBinding(AField: TField; ADataBinding: TObject)
      : TcxEditRepositoryItem;
    function GetLookupItem: TcxEditRepositoryItem;
    procedure RegisterItem(AFieldType: TFieldType; AVersion: Integer;
      ARepositoryItem: TcxEditRepositoryItem); reintroduce;
    procedure RegisterCurrencyItem(AVersion: Integer;
      ARepositoryItem: TcxEditRepositoryItem);
    procedure RegisterLookupItem(AVersion: Integer;
      ARepositoryItem: TcxEditRepositoryItem);
    procedure UnregisterItem(AFieldType: TFieldType; AVersion: Integer);
      reintroduce;
...
begin
  ...
  GetDefaultEditDBRepositoryItems.RegisterLookupItem
    (cxEditRegisteredItemsStandardVersion, ALookupComboBoxItem);
  // Misc
  for I := Low(TFieldType) to High(TFieldType) do
    if DBRepositoryItemClasses[I] <> nil then
      GetDefaultEditDBRepositoryItems.RegisterItem(I,
        cxEditRegisteredItemsStandardVersion,
...
var
  I: TFieldType;
begin
  GetDefaultEditDBRepositoryItems.UnregisterDefaultItem
    (cxEditRegisteredItemsStandardVersion);
  // GetDefaultEditDBRepositoryItems.UnregisterLookupItem(cxEditRegisteredItemsStandardVersion);
  for I := Low(TFieldType) to High(TFieldType) do
    if DBRepositoryItemClasses[I] <> nil then
      GetDefaultEditDBRepositoryItems.UnregisterItem(I,
        cxEditRegisteredItemsStandardVersion);
end;
...
function TcxEditDBRegisteredRepositoryItems.GetCurrencyItem
  : TcxEditRepositoryItem;
begin
...
function TcxEditDBRegisteredRepositoryItems.GetItem(AFieldType: TFieldType)
  : TcxEditRepositoryItem;
begin
  Result := inherited GetItem(Integer(AFieldType));
end;

function TcxEditDBRegisteredRepositoryItems.GetItemByField(AField: TField)
  : TcxEditRepositoryItem;
...
function TcxEditDBRegisteredRepositoryItems.GetItemByDataBinding(AField: TField;
  ADataBinding: TObject): TcxEditRepositoryItem;
var
...
begin
  ...
  begin
    if FUnshareableItems = nil then
      FUnshareableItems := TObjectDictionary<TObject, TcxEditRepositoryItem>.
        Create([doOwnsValues]);
    APrevResult := Result;
    if not FUnshareableItems.TryGetValue(ADataBinding, Result) then
    begin
      Result := GetDefaultEditRepository.CreateItem
        (TcxEditRepositoryItemClass(APrevResult.ClassType));
...
function TcxEditDBRegisteredRepositoryItems.GetLookupItem
  : TcxEditRepositoryItem;
begin
...
var
  AIndex: Integer;
begin
  if FindIndexForInsertion(FCurrencyItems, Integer(ftUnknown), AVersion, AIndex)
  then
...
var
  AIndex: Integer;
begin
  if FindIndexForInsertion(FLookupItems, Integer(ftUnknown), AVersion, AIndex)
  then
...
function TcxEditDBRegisteredRepositoryItems.Add
  (var AItems: TcxEditRegisteredRepositoryItemsDataArray;
  AFieldType: TFieldType; AVersion: Integer;
  ARepositoryItem: TcxEditRepositoryItem): Integer;
begin
  Result := inherited Add(AItems, Integer(AFieldType), AVersion,
    ARepositoryItem);
end;
...
dxUnitsLoader.AddUnit(@CreateDefaultRepositoryItems,
