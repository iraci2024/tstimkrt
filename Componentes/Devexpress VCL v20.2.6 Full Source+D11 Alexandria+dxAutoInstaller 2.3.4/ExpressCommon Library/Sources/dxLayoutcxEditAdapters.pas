...
uses
  Classes, Windows, cxControls, dxLayoutContainer, cxEdit,
  cxLookAndFeelPainters, cxGraphics, dxColorGallery;
...
begin
  ...
  if TdxCustomLayoutItemAccess(Item).IsDesigning then
    TcxCustomEditStyleAccess(ControlStyle).TransparentBorder := False;
end;

procedure TdxLayoutcxEditAdapter.InternalSetInitialSettings;
const
  BorderStyles: array [TdxLayoutBorderStyle] of TcxEditBorderStyle = (ebsNone,
    ebsSingle, ebsFlat, ebs3D);
...
begin
  ...
  then
    AControlStyle.AssignedValues := AControlStyle.AssignedValues -
      [svBorderColor, svBorderStyle, svButtonStyle, svPopupBorderStyle]
  else
  begin
    AItemBorderStyle := LayoutLookAndFeel.ItemOptions.ControlBorderStyle;
    if AControlStyle.DefaultBorderStyle <> cbsNone then
    begin
      AControlStyle.BorderColor :=
        LayoutLookAndFeel.ItemOptions.GetControlBorderColor;
