...
type
  TdxTaskbarProgressState = (tbpsNoProgress, tbpsIndeterminate, tbpsNormal,
    tbpsError, tbpsPaused);
  ...
  public
    ...
    property LinkedComponent: TComponent read FLinkedComponent
      write SetLinkedComponent;
    property Position: Int64 read FPosition write SetPosition;
    property State: TdxTaskbarProgressState read FState write SetState
      default tbpsNormal;
...
const
  TaskbarProgressState: array [TdxTaskbarProgressState] of Integer =
...
begin
  ...
  if not dxChangeWindowMessageFilterEx(AWnd, dxTaskButtonCreatedMessageID,
    MSGFLT_ALLOW, nil) then;
...
begin
  ...
  if (Application.MainForm <> nil) and (Application.MainForm.HandleAllocated)
    and (GetMainWindow <> FMainWindow) then
  begin
...
begin
  for I := 0 to FItems.Count - 1 do
    if TdxTaskbarCustomProgress(FItems[I]).Active and (FItems[I] <> AItem) then
...
procedure TdxTaskbarProgressManager.DoAppMessage(var Msg: tagMSG;
  var Handled: Boolean);
begin
  if (Msg.Message = dxTaskButtonCreatedMessageID) and (Msg.hwnd = GetMainWindow)
  then
  begin
...
begin
  if FIsLinkedComponentChangeLocked then
    Exit;
  ...
  if FState <> tbpsNoProgress then
    ...
    else if FState = tbpsIndeterminate then
      FState := tbpsNormal;
...
function TdxTaskbarCustomProgress.GetProgressBarProperties
...
begin
  ...
  else if (AValue <> nil) and not(AValue is TcxCustomProgressBar) then
    AValue := nil;
...
procedure TdxTaskbarCustomProgress.SetState(AValue: TdxTaskbarProgressState);
begin
...
begin
  ...
  begin
    dxTaskButtonCreatedMessageID := RegisterWindowMessage
      (dxTaskButtonCreatedMessage);
    TaskbarProgressManager.Initialize;
  end;
  User32lib := GetModuleHandle(user32);
  if User32lib <> 0 then
    dxChangeWindowMessageFilterEx := GetProcAddress(User32lib,
