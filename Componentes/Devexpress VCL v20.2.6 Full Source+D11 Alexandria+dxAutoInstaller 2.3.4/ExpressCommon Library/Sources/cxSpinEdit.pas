...
uses
  Types, Windows, Variants, Classes, Controls, Forms, Graphics, SysUtils,
  cxClasses,
  cxContainer, cxControls, cxDataStorage, cxDataUtils, cxEdit, cxGraphics,
  cxMaskEdit,
...
type
  ...
  TcxSpinEditButton = (sebBackward, sebForward, sebFastBackward,
    sebFastForward);
  ...
  protected
    procedure DrawHotFlatButtonBorder(ACanvas: TcxCanvas;
      ...
      out AContentRect: TRect; var APenColor, ABrushColor: TColor); override;
    ...
    procedure DrawUltraFlatButtonBorder(ACanvas: TcxCanvas;
      ...
      out APenColor, ABrushColor: TColor); override;
  ...
  public
    ...
    procedure DrawButtonContent(ACanvas: TcxCanvas;
      ...
      APenColor, ABrushColor: TColor; ANeedOffsetContent: Boolean); override;
  end;
  ...
  protected
    ...
    procedure CalculateButtonsBounds(ACanvas: TcxCanvas;
      AViewInfo: TcxCustomEditViewInfo; var ButtonsRect: TRect); virtual;
  ...
  public
    ...
    procedure CalculateButtonViewInfo(ACanvas: TcxCanvas;
      ...
      var ButtonsRect: TRect); override;
  ...
  private
    ...
    function InternalTryTextToValue(S: string;
  ...
  protected
    ...
    function GetMaxMinValueForCurrentValueType(AMinValue: Boolean = True)
      : TcxEditValue;
  ...
  public
    ...
    function IsEditValueValid(var AEditValue: TcxEditValue;
      AEditFocused: Boolean): Boolean; override;
    ...
    property SpinButtons: TcxSpinEditButtons read FSpinButtons
      write SetSpinButtons;
    property UseCtrlIncrement: Boolean read FUseCtrlIncrement
      write FUseCtrlIncrement default False;
    property ValueType: TcxSpinEditValueType read GetValueType
      write SetValueType stored IsValueTypeStored;
  ...
  protected
    ...
    function DoMouseWheelUp(Shift: TShiftState; MousePos: TPoint)
      : Boolean; override;
    function GetNewValue(AButton: TcxSpinEditButton; out AValue: Variant)
      : Boolean; virtual;
    ...
    function InternalIncrement(var AValue: Variant; AIncrement: Variant)
      : Boolean; virtual;
    ...
    function InternalPrepareEditValue(const ADisplayValue: TcxEditValue;
      ARaiseException: Boolean; out AEditValue: TcxEditValue;
    ...
    property PressedState: TcxSpinEditPressedState read FPressedState
      write SetPressedState;
  ...
  public
    ...
    procedure PrepareEditValue(const ADisplayValue: TcxEditValue;
      out AEditValue: TcxEditValue; AEditFocused: Boolean); override;
    property ActiveProperties: TcxCustomSpinEditProperties
...
type
  TcxSpinEditUltraFlatButtonPosition = (sufbpLeftMost, sufbpMiddle,
    sufbpRightMost, sufbpMiddleTop, sufbpMiddleBottom, sufbpRightTop,
    sufbpRightBottom, sufbpLeftBottom, sufbpLeftTop);
  TcxSpinEditFlatButtonPosition = (sfbpLeftSide, sfbpLeftSideRightMost,
    ...
    sfbpRightTop, sfbpRightBottom, sfbpLeftTop, sfbpLeftBottom,
...
begin
  ...
  with AViewInfo do
    if (Length(ButtonsInfo) > 0) and (ButtonsInfo[0].Data.NativePart <> TC_NONE)
      and (ButtonsPosition <> sbpHorzLeftRight) and not IsCustomDrawButton then
    begin
      APrevClipRgn := ACanvas.GetClipRegion;
      try
        ACanvas.IntersectClipRect(AViewInfo.BorderRect);
        InternalPolyLine(ACanvas, [DelimiterLine[0], DelimiterLine[1]],
...
procedure DrawSpinEditUltraFlatButtonBorder(ACanvas: TcxCanvas; R: TRect;
  ...
  out ABackgroundRect, AContentRect: TRect);
var
...
begin
  ...
  begin
    ...
    else if not AIsEditBackgroundTransparent then
  ...
  begin
    ...
    case AButtonPosition of
      ...
      sufbpMiddleTop:
        begin
          DrawButtonBorder(ACanvas, R, [bLeft, bBottom, bRight],
            AHighlightColor);
...
procedure TcxSpinEditViewInfo.DrawButtonContent(ACanvas: TcxCanvas;
  ...
  const
    ArrowDirectionMap: array [Boolean, 0 .. 3] of TcxArrowDirection =
      ((adLeft, adRight, adLeft, adRight), (adDown, adUp, adLeft, adRight));
  begin
    DrawArrow(ACanvas, ArrowSize, R,
      ArrowDirectionMap[ButtonsPosition = sbpVert,
      AViewInfo.ButtonVisibleIndex], AViewInfo.ButtonVisibleIndex > 1,
      ANeedOffsetContent, AColor);
  end;
...
begin
  ...
  else if AViewInfo.Data.State <> ebsDisabled then
  begin
...
procedure TcxSpinEditViewInfo.DrawHotFlatButtonBorder(ACanvas: TcxCanvas;
  AButtonViewInfo: TcxEditButtonViewInfo; var ARect: TRect;
  out AContentRect: TRect; var APenColor, ABrushColor: TColor);

  function GetButtonPosition: TcxSpinEditFlatButtonPosition;
  const
    AButtonPositionA: array [Boolean, Boolean, TcxSpinEditButtonsPosition,
      sebBackward .. sebForward] of TcxSpinEditFlatButtonPosition =
      ((((sfbpLeftSideRightMost, sfbpRightSideLeftMost), (sfbpRightSideLeftMost,
      sfbpRightSide), (sfbpRightBottom, sfbpRightTop)),
      ((sfbpLeftSideRightMost, sfbpRightSideLeftMost), (sfbpRightSide,
      sfbpRightSide), (sfbpMiddleBottom, sfbpMiddleTop))),
      (((sfbpLeftSideRightMost, sfbpRightSideLeftMost), (sfbpLeftSide,
      sfbpLeftSideRightMost), (sfbpLeftBottom, sfbpLeftTop)),
      ((sfbpLeftSideRightMost, sfbpRightSideLeftMost), (sfbpLeftSide,
      sfbpLeftSide), (sfbpLeftMiddleBottom, sfbpLeftMiddleTop))));
    AFastButtonPositionA: array [Boolean, TcxSpinEditButtonsPosition,
      sebFastBackward .. sebFastForward] of TcxSpinEditFlatButtonPosition =
      (((sfbpLeftSide, sfbpRightSide), (sfbpRightSideLeftMost, sfbpRightSide),
      (sfbpRightSideLeftMost, sfbpRightSide)),

      ((sfbpLeftSide, sfbpRightSide), (sfbpLeftSide, sfbpLeftSideRightMost),
      (sfbpLeftSide, sfbpLeftSideRightMost)));
  begin
    if AButtonViewInfo.ButtonIndex > 1 then
      Result := AFastButtonPositionA[UseRightToLeftAlignment, ButtonsPosition,
        TcxSpinEditButton(AButtonViewInfo.ButtonIndex)]
    else
      Result := AButtonPositionA[UseRightToLeftAlignment,
        Length(ButtonsInfo) > 2, ButtonsPosition,
        TcxSpinEditButton(AButtonViewInfo.ButtonIndex)];
  end;
...
begin
  ...
  begin
    ...
    if Data.State in [ebsPressed, ebsSelected] then
      ...
      APenColor := clBtnText;
    DrawSpinEditFlatButtonBorder(ACanvas, GetButtonPosition, ARect, ABrushColor,
      clBtnShadow);
...
procedure TcxSpinEditViewInfo.DrawNativeButtonBorder(ACanvas: TcxCanvas;
  AButtonViewInfo: TcxEditButtonViewInfo; var ARect: TRect;
  out AContentRect: TRect; var APenColor, ABrushColor: TColor);
begin
...
procedure TcxSpinEditViewInfo.DrawUltraFlatButtonBorder(ACanvas: TcxCanvas;
  AButtonViewInfo: TcxEditButtonViewInfo; AIsOffice11Style: Boolean;
  var ARect: TRect; var AContentRect: TRect;
  ...
  begin
    ...
    case AButtonViewInfo.ButtonIndex of
      ...
      cxSpinFastForwardButtonIndex:
        if UseRightToLeftAlignment and (ButtonsPosition <> sbpHorzLeftRight)
        then
          AButtonPosition := sufbpMiddle
        else
      ...
      cxSpinBackwardButtonIndex:
        ...
        else if (Length(ButtonsInfo) = 2) and
          ((ButtonsPosition = sbpHorzLeftRight) or UseRightToLeftAlignment) then
          AButtonPosition := sufbpLeftMost
        else
          AButtonPosition := sufbpMiddle;
      ...
      else if (Length(ButtonsInfo) > 2) or UseRightToLeftAlignment and
        (ButtonsPosition = sbpHorzRight) then
        AButtonPosition := sufbpMiddle
      else
        AButtonPosition := sufbpRightMost;
    end;
    DrawSpinEditUltraFlatButtonBorder(ACanvas, ARect, AButtonPosition,
      ABrushColor, AButtonViewInfo, BorderRect, Transparent,
      epoHasExternalBorder in PaintOptions, Self.BorderStyle, ARect,
      AContentRect);
  end;
end;

procedure TcxSpinEditViewInfo.DrawNativeButtonBackground(ACanvas: TcxCanvas;
...
procedure TcxSpinEditViewData.Calculate(ACanvas: TcxCanvas;
...
procedure TcxSpinEditViewData.CalculateButtonNativePartInfo(ATheme: TdxTheme;
  AButtonViewInfo: TcxEditButtonViewInfo);
const
  ButtonNativeStateMap: array [TcxEditButtonState] of Integer = (UPS_DISABLED,
    UPS_NORMAL, UPS_PRESSED, UPS_HOT);
begin
  if ATheme <> 0 then
    AButtonViewInfo.Data.NativeState := ButtonNativeStateMap
...
var
  ...
  begin
    Result := CalculateEditDefaultButtonWidth(ACanvas,
  ...
  begin
    ...
    TcxSpinEditViewInfo(AViewInfo).ArrowSize :=
      GetArrowSize(Size(AButtonWidth + 2, (ButtonsRect.Bottom - ButtonsRect.Top)
      div 2 + 2), adUp).cy;
    if ANativeStyle then
    begin
      ADefaultButtonWidth := ACanvas.TextHeight('Zg') +
        Self.GetEditContentSizeCorrection.cy +
        (AClientExtent.Top + AClientExtent.Bottom) -
        (AEditButtonsExtent.Top + AEditButtonsExtent.Bottom) - 1;
      ADefaultButtonWidth := ADefaultButtonWidth div 2 +
        Integer(Odd(ADefaultButtonWidth));
    end
    else if Style.LookAndFeel.SkinPainter <> nil then
    begin
      with Style.LookAndFeel.SkinPainter.EditButtonSize do
      begin
        ADefaultButtonWidth := cy div 2 + TcxSpinEditViewInfo(AViewInfo)
          .ArrowSize;
      ...
      ADefaultButtonWidth := (TcxSpinEditViewInfo(AViewInfo).ArrowSize * 2 - 1)
        * 2 + 1 - 2;
    dxAdjustToTouchableSize(ADefaultButtonWidth, ScaleFactor);
    AFastButtonWidth := MulDiv(ADefaultButtonWidth, 3, 2) +
      Integer(Odd(ADefaultButtonWidth));
  ...
  procedure UpdateButtonsNativePart;
    ...
    begin
      ...
      begin
        AViewInfo.ButtonsInfo[cxSpinBackwardButtonIndex].Data.NativePart :=
          SPNP_DOWN;
        AViewInfo.ButtonsInfo[cxSpinForwardButtonIndex].Data.NativePart
          := SPNP_UP;
      end
      else
      begin
      ...
      begin
        AViewInfo.ButtonsInfo[cxSpinFastBackwardButtonIndex].Data.NativePart :=
          SPNP_DOWNHORZ;
        AViewInfo.ButtonsInfo[cxSpinFastForwardButtonIndex].Data.NativePart :=
          SPNP_UPHORZ;
      end;
    end;
  ...
  begin
    if ANativeStyle and IsWinVistaOrLater and not AViewInfo.IsCustomDrawButton
    then
      Result := 1
    else
      Result := 0;
...
begin
  ...
  TcxSpinEditViewInfo(AViewInfo).ButtonsPosition :=
    Properties.SpinButtons.Position;
  ...
  begin
    ...
    begin
      Bounds := Rect(ButtonsRect.Right - AButtonWidth, ButtonsRect.Top,
        ButtonsRect.Right, ButtonsRect.Top + (cxRectHeight(ButtonsRect) -
        GetDelimiterSize) div 2);
      Y := Bounds.Bottom + GetDelimiterSize;
    end;
    with AViewInfo.ButtonsInfo[cxSpinBackwardButtonIndex] do
    begin
      Bounds := Rect(ButtonsRect.Right - AButtonWidth, Y, ButtonsRect.Right,
        ButtonsRect.Bottom);
      ButtonsRect.Right := Bounds.Left;
    end;
  end
  ...
  begin
    ...
    begin
      Bounds := Rect(ButtonsRect.Left, ButtonsRect.Top,
        ButtonsRect.Left + ADefaultButtonWidth, ButtonsRect.Bottom -
        GetBottomCorrection);
      ButtonsRect.Left := Bounds.Right;
    end;
  end
  ...
  if UseRightToLeftAlignment and
    (Properties.SpinButtons.Position <> sbpHorzLeftRight) then
  begin
    ButtonsRect := TdxRightToLeftLayoutConverter.ConvertRect(ButtonsRect,
      APrevButtonsRect);
    for I := 0 to High(AViewInfo.ButtonsInfo) do
      AViewInfo.ButtonsInfo[I].Bounds :=
        TdxRightToLeftLayoutConverter.ConvertRect
        (AViewInfo.ButtonsInfo[I].Bounds, APrevButtonsRect);
    R := APrevButtonsRect;
    if Properties.SpinButtons.ShowFastButtons then
      R.Right := AViewInfo.ButtonsInfo[cxSpinFastBackwardButtonIndex]
        .Bounds.Right
    else
    ...
    for I := 0 to High(AViewInfo.ButtonsInfo) do
      AViewInfo.ButtonsInfo[I].Bounds :=
        TdxRightToLeftLayoutConverter.ConvertRect
        (AViewInfo.ButtonsInfo[I].Bounds, R);
  end;
...
function TcxSpinEditViewData.IgnoreButtonWhileStretching(AButtonVisibleIndex
  : Integer): Boolean;
begin
...
function TcxSpinEditViewData.IsButtonLeftAligned
  (AViewInfo: TcxCustomEditViewInfo; AButtonVisibleIndex: Integer): Boolean;
begin
  if UseRightToLeftAlignment and
    (Properties.SpinButtons.Position = sbpHorzLeftRight) and
    ((AButtonVisibleIndex = cxSpinForwardButtonIndex) or
    (AButtonVisibleIndex = cxSpinFastForwardButtonIndex)) then
    Result := False
  else
    Result := (Properties.SpinButtons.Position = sbpHorzLeftRight) and
      ((AButtonVisibleIndex = cxSpinBackwardButtonIndex) or
      (AButtonVisibleIndex = cxSpinFastBackwardButtonIndex)) or
...
function TcxSpinEditViewData.GetButtonNativeTheme(AButtonViewInfo
...
class function TcxCustomSpinEditProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxSpinEditViewInfo;
end;

function TcxCustomSpinEditProperties.IsDisplayValueValid(var ADisplayValue
...
function TcxCustomSpinEditProperties.IsEditValueValid(var AEditValue
  : TcxEditValue; AEditFocused: Boolean): Boolean;
var
  AValue: TcxEditValue;
begin
...
procedure TcxCustomSpinEditProperties.ValidateDisplayValue(var ADisplayValue
...
begin
  if TcxCustomSpinEdit(AEdit).FIsCustomText then
    AError := not TcxCustomSpinEdit(AEdit).InternalPrepareEditValue
      (ADisplayValue, False, AEditValue, AErrorText)
  else
...
procedure TcxCustomSpinEditProperties.DefineProperties(Filer: TFiler);
  ...
  begin
    Result := (Increment = 0) and
      ((Filer.Ancestor = nil) or (TcxCustomSpinEditProperties(Filer.Ancestor)
      .Increment <> Increment));
  end;

  function HasZeroLargeIncrement: Boolean;
  begin
    Result := (LargeIncrement = 0) and
      ((Filer.Ancestor = nil) or (TcxCustomSpinEditProperties(Filer.Ancestor)
...
procedure TcxCustomSpinEditProperties.DoAssign(AProperties
...
class function TcxCustomSpinEditProperties.GetAssignedValuesClass
  : TcxCustomEditPropertiesValuesClass;
begin
  Result := TcxSpinEditPropertiesValues;
end;

function TcxCustomSpinEditProperties.GetEditValueSource(AEditFocused: Boolean)
...
class function TcxCustomSpinEditProperties.GetViewDataClass
...
function TcxCustomSpinEditProperties.CheckValueBounds
...
begin
  ...
  if VarIsNumericEx(Result) then
    if IsValueBoundDefined(evbMin) and (Result < MinValue) then
      Result := MinValue
    else if IsValueBoundDefined(evbMax) and (Result > MaxValue) then
      Result := MaxValue;
end;
...
function TcxCustomSpinEditProperties.GetBoundsCheckingKind
  : TcxSpinBoundsCheckingKind;
begin
  if FCircular then
    Result := bckCircular
  else if ExtendValueUpToBound then
    Result := bckExtendToBound
  else
    Result := bckDoNotExceed;
end;

function TcxCustomSpinEditProperties.GetIncrement
  (AButton: TcxSpinEditButton): Double;
begin
  case AButton of
    sebBackward:
      Result := -Increment;
...
function TcxCustomSpinEditProperties.IsValueBoundsValid
...
function TcxCustomSpinEditProperties.PrepareValue(const AValue
...
function TcxCustomSpinEditProperties.SetVariantType(const AValue: TcxEditValue)
  : TcxEditValue;
begin
...
function TcxCustomSpinEditProperties.GetAssignedValues
  : TcxSpinEditPropertiesValues;
begin
...
const
  ASpinEditValueTypeMap: array [Boolean] of TcxSpinEditValueType =
    (vtInt, vtFloat);
begin
  if AssignedValues.ValueType then
    Result := FValueType
...
function TcxCustomSpinEditProperties.InternalTryTextToValue(S: string;
  out AValue: TcxEditValue): Boolean;
var
...
procedure TcxCustomSpinEditProperties.SetAssignedValues
  (AValue: TcxSpinEditPropertiesValues);
begin
...
procedure TcxCustomSpinEditProperties.SetSpinButtons
  (AValue: TcxSpinEditButtons);
begin
  FSpinButtons.Assign(AValue);
end;

function TcxCustomSpinEditProperties.TryExtractDisplayFormat
...
begin
  ...
  begin
    ...
    else if (Pos(UpCase(Result[I]), AExtraChars) <> 0) or
      dxCharIsNumeric(Result[I]) then
      Inc(I)
    else if Pos(UpCase(Result[I]), DisplayFormat) <> 0 then
      Delete(Result, I, 1)
    else
      Inc(I);
  end;
...
procedure TcxCustomSpinEditProperties.SetValueType
  (AValue: TcxSpinEditValueType);
begin
...
function TcxCustomSpinEditProperties.VarToCurrentValueType(AValue: TcxEditValue)
  : TcxEditValue;
begin
...
class function TcxCustomSpinEdit.GetPropertiesClass
...
begin
  ...
  try
    with ActiveProperties do
      ...
      else if Value > InternalMaxValue then
...
begin
  FIsCustomTextAction := False;
  if ((Key = VK_UP) or (Key = VK_DOWN) or (Key = VK_NEXT) or (Key = VK_PRIOR))
    and not(ActiveProperties.UseCtrlIncrement and not(ssCtrl in Shift)) then
  begin
    BeginUserAction;
    try
...
function TcxCustomSpinEdit.GetNewValue(AButton: TcxSpinEditButton;
  ...
  begin
    ...
    if AValue < InternalMinValue then
      if AActualButton in [sebForward, sebFastForward] then
        ...
        Exit
    else if AValue > InternalMaxValue then
...
function TcxCustomSpinEdit.InternalGetNotPublishedStyleValues
  : TcxEditStyleValues;
begin
...
function TcxCustomSpinEdit.InternalIncrement(var AValue: Variant;
  AIncrement: Variant): Boolean;
...
begin
  ...
  Result := Result or IsControlChar(AChar) or
    (AnsiChar(AChar) in ActiveProperties.ValidChars);
  if not ActiveProperties.CanEdit and Result and
    (not IsControlChar(AChar) or dxCharInSet(AChar, [dxVKBack, dxVKDelete]))
  then
    Result := False;
end;

procedure TcxCustomSpinEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
function TcxCustomSpinEdit.DoRefreshContainer(const P: TPoint;
...
begin
  ...
  with ViewInfo do
    if (Length(ButtonsInfo) > 0) and (PressedButton = -1) and
      (CaptureButtonVisibleIndex = -1) then
      StopTracking;
end;
...
begin
  ...
  begin
    ...
    begin
      if EditValue = '' then
        Result := ActiveProperties.CheckValueBounds(0)
      else if not ActiveProperties.TryTextToValue(EditValue, Result) then
        Result := ActiveProperties.CheckValueBounds(0);
    end
    else
      Result := ActiveProperties.CheckValueBounds(0);
...
function TcxCustomSpinEdit.IncrementValueToStr(const AValue
  : TcxEditValue): string;
var
  ADisplayValue: TcxEditValue;
begin
...
function TcxCustomSpinEdit.InternalPrepareEditValue(const ADisplayValue
  : TcxEditValue; ARaiseException: Boolean; out AEditValue: TcxEditValue;
  out AErrorText: TCaption): Boolean;
var
  AError: Boolean;
  AValue: Variant;
begin
  Result := True;
  if ADisplayValue = '' then
    if (ActiveProperties.IDefaultValuesProvider <> nil) and
      ActiveProperties.IDefaultValuesProvider.DefaultRequired then
      AEditValue := ActiveProperties.CheckValueBounds(0)
    else
      AEditValue := Null
  else
  begin
    AError := not ActiveProperties.TryTextToValue
      (VarToStr(ADisplayValue), AValue);
    if AError then
    begin
      AErrorText := '';
...
class function TcxFilterSpinEditHelper.GetFilterDataType(AValueTypeClass
...
GetRegisteredEditProperties.Register(TcxSpinEditProperties,
  scxSEditRepositorySpinItem);
FilterEditsController.Register(TcxSpinEditProperties, TcxFilterSpinEditHelper);

finalization

FilterEditsController.Unregister(TcxSpinEditProperties,
  TcxFilterSpinEditHelper);
