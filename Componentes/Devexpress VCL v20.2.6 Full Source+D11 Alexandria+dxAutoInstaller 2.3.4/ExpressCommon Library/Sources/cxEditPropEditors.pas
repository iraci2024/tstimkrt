...
{$ENDIF}
  ...
  Types,
  WideStrings,
  Windows, Forms, Classes, Controls, DB, Graphics, ImgList, TypInfo,
  cxContainer, ColnEdit,
  cxDateUtils, cxDataStorage, cxEdit, cxEditRepositoryItems, cxLookAndFeels,
  cxPropEditors,
  cxDesignWindows, dxCoreReg, cxLibraryReg, ActnList, dxAlertWindow,
  dxBreadcrumbEdit, dxCoreGraphics,
  dxBarCode, dxBarCodeUtils, dxFormattedText;
...
{$ENDIF}
  ...
  TcxEditorsLibraryComponentEditorEx = class
    (TcxCustomEditorsLibraryComponentEditor)
  protected
  ...
  TcxEditorsLibraryComponentWithStylesEditor = class
    (TcxEditorsLibraryComponentEditor)
  protected
  ...
  TcxEditorsLibraryComponentWithoutStylesEditor = class
    (TcxEditorsLibraryComponentEditor)
  protected
    function GetEditItemCaption: string; override;
    procedure ExecuteEditAction; override;
  ...
  TcxEditorsLibraryCXControlComponentEditor = class
    (TcxEditorsLibraryComponentWithoutStylesEditor)
  protected
  ...
  TcxBreadcrumbEditComponentEditor = class
    (TcxEditorsLibraryCXControlComponentEditor)
  protected
    function InternalGetVerb(AIndex: Integer): string; override;
    function InternalGetVerbCount: Integer; override;
  ...
  TcxEditStyleControllerEditor = class
    (TcxEditorsLibraryComponentWithStylesEditor)
  protected
  ...
  TcxDefaultEditStyleControllerEditor = class
    (TcxEditorsLibraryComponentWithStylesEditor)
  protected
    procedure RestoreStyles; override;
  public
  ...
  TcxCustomButtonComponentEditor = class
    (TcxEditorsLibraryComponentWithoutStylesEditor)
  protected
    function GetLookAndFeel: TcxLookAndFeel; override;
  end;
  ...
  TdxAlertWindowComponentEditor = class
    (TcxEditorsLibraryComponentWithoutStylesEditor)
  protected
    function GetLookAndFeel: TcxLookAndFeel; override;
    function InternalGetVerb(AIndex: Integer): string; override;
  ...
  TcxRadioButtonComponentEditor = class
    (TcxEditorsLibraryComponentWithoutStylesEditor)
  protected
    function GetLookAndFeel: TcxLookAndFeel; override;
  end;
  ...
  TcxContainerComponentEditor = class
    (TcxEditorsLibraryComponentWithStylesEditor)
  protected
    procedure RestoreStyles; override;
  end;
  ...
  TcxCustomNavigatorComponentEditor = class
    (TcxEditorsLibraryCXControlComponentEditor)
  protected
    function InternalGetVerb(AIndex: Integer): string; override;
    function InternalGetVerbCount: Integer; override;
  ...
  TdxSliderImageComponentEditor = class
    (TcxEditorsLibraryCXControlComponentEditor)
  public
    procedure Edit; override;
  end;
  ...
  TcxDefaultEditStyleControllerStyleProperty = class
  ...
  TdxDataControllerMultithreadedSortingPropertyEditor = class
  ...
  TdxDataControllerMultithreadedFilteringPropertyEditor = class
    (TdxDefaultBooleanPropertyEditor)
  protected
    function GetDefaultValue: Boolean; override;
  end;
  ...
  TdxAlphaColorPropertyEditor = class(TOrdinalProperty,
    ICustomPropertyDrawing80, ICustomPropertyDrawing,
    ICustomPropertyListDrawing)
  private
    FSavedProc: TGetStrProc;
    procedure CalculatePreviewRect(var R: TRect; out APreviewRect: TRect);
    procedure DrawPreview(ACanvas: TCanvas; R: TRect; AColor: TdxAlphaColor;
      ASelected: Boolean);
  ...
  TdxColorDialogComponentEditor = class
    (TcxEditorsLibraryComponentWithoutStylesEditor)
  protected
    function GetLookAndFeel: TcxLookAndFeel; override;
    procedure InternalExecuteVerb(AIndex: Integer); override;
...
uses
  ...
  cxCalendar, dxBreadcrumbEditEditor, dxGalleryDesigner, dxGalleryControl,
  dxBevel,
  cxCustomData, cxGraphics, cxGeometry, dxGDIPlusClasses, Math, dxColorDialog,
  dxAlertWindowDesignTester,
  dxActivityIndicator, dxWheelPicker, dxWheelPickerItemIndexesEditor,
  dxSparkline, dxTokenEdit, dxFormattedLabel,
  dxFormattedTextConverterRTF, dxFormattedTextConverterBBCode,
  dxFormattedLabelCaptionEditor;

const
  cxEditRepositoryEditorVerb = 'Edit...';

  cxEditComponentEditorVerbA: array [0 .. 3] of string =
    (cxEditRepositoryEditorVerb, '-', 'Restore properties', 'Restore styles');

  cxCustomEditControlEditorVerbA: array [0 .. 1] of string =
    ('Restore LookAndFeel', 'Restore Styles');

  cxCustomNavigatorEditorVerb = 'Restore Buttons';

  ADefaultMethodParams: array [0 .. 0] of TMethodParam = ((Flags: [pfAddress];
...
begin
  DataSource := GetObjectProp(GetComponent(0), GetDataSourcePropName)
    as TDataSource;
...
function TcxValueTypeProperty.IsValueTypeClassValid(AValueTypeClass
...
begin
  Result := GetObjectProp(GetComponent(0), GetDataSourcePropName)
    as TDataSource;
end;
...
begin
  ...
  ShowEventMethod(Designer, AInstance, AEventName, AMethodName,
...
begin
  ...
  begin
    Result := TcxImageComboBoxProperties
      (TcxImageComboBoxItems(TcxImageComboBoxItem(GetComponent(0)).Collection)
      .Owner).LargeImages;
    if Result = nil then
      Result := TcxImageComboBoxProperties
        (TcxImageComboBoxItems(TcxImageComboBoxItem(GetComponent(0)).Collection)
...
begin
  ...
  if GetComponent(0) is TdxGalleryControlItem then
    Result := TdxCustomGalleryControlAccess
      (TdxGalleryControlItemAccess(GetComponent(0)).GalleryControl).Images;
end;
...
function TcxEditorsLibraryComponentEditorEx.InternalGetVerb
...
procedure TcxEditorsLibraryComponentEditorEx.InternalExecuteVerb
  (AIndex: Integer);
begin
  ExecuteEditAction;
end;
...
begin
  Result := (PropCount = 1) and TcxCustomMaskEdit(GetComponent(0))
    .ActiveProperties.IsMasked;
end;
...
function TcxEditorsLibraryCXControlComponentEditor.GetLookAndFeel
  : TcxLookAndFeel;
begin
...
function TcxBreadcrumbEditComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  if AIndex = 0 then
    Result := 'Edit...'
...
function TdxBreadcrumbEditSelectedPathPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes;
  if PropCount = 1 then
...
begin
  ...
  ShowEventMethod(Designer, AButtons, 'OnButtonClick',
    Component.Name + 'ButtonsButtonClick',
    cxNavigatorButtonsOnButtonClickEventParams);
end;

function TcxCustomNavigatorComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  if AIndex = 1 then
...
procedure TcxCustomNavigatorComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  if AIndex = 1 then
  begin
...
begin
  ShowEventMethod(Designer, Component, 'OnChange', Component.Name + 'Change',
    ADefaultMethodParams);
end;
...
begin
  Result := '';
  AAssignedValues := TcxCustomEditPropertiesAccess(GetComponent(0))
    .AssignedValues;
...
begin
  Result := (TcxNavigatorCustomButton(GetComponent(0))
    .Collection.Owner as TcxCustomNavigatorButtons).Images;
end;
...
begin
  Result := TcxCustomEditProperties(TcxEditButtons(TcxEditButton(GetComponent(0)
    ).Collection).Owner).Images;
end;
...
begin
  ...
  begin
    ...
    if (AButton.Action <> nil) and (AButton.Action is TCustomAction) and
      (TCustomAction(AButton.Action).ActionList <> nil) then
...
function TdxBreadcrumbEditRecentPathImageIndexProperty.GetImages
  : TCustomImageList;
begin
  Result := TdxCustomBreadcrumbEditPropertiesAccess
    (TdxBreadcrumbEditPathEditorPropertiesAccess(GetPathEditorProperties)
    .Owner).Images;
end;

function TdxBreadcrumbEditRecentPathImageIndexProperty.GetPathEditorProperties
  : TdxBreadcrumbEditPathEditorProperties;
begin
...
begin
  Result := TdxAlertWindowOptionsButtons
    (TdxAlertWindowButtons(TdxAlertWindowButton(GetComponent(0)).Collection)
    .Owner).Images;
end;
...
function TcxEditGuidelines.GetDesignerGuideType(Index: Integer)
  : TDesignerGuideType;
begin
  if Edit.HasTextBaseLine and (Index = GetCount - 1) then
    Result := gtBaseline
...
function TdxDataControllerMultithreadedSortingPropertyEditor.
...
function TdxDataControllerMultithreadedFilteringPropertyEditor.
  GetDefaultValue: Boolean;
begin
...
begin
  ...
  if not(ColorToIdent(LongInt(AColor), Result) and
    ((AAlpha = MaxByte) or not cxColorIsValid(AColor))) then
...
procedure TdxAlphaColorPropertyEditor.PropDrawName(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
begin
  DefaultPropertyDrawName(Self, ACanvas, ARect);
end;

procedure TdxAlphaColorPropertyEditor.PropDrawValue(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
begin
  if GetVisualValue <> '' then
    DrawPreview(ACanvas, cxRectCenterHorizontally(ARect, cxRectHeight(ARect)),
      GetOrdValue, ASelected)
  else
    DefaultPropertyDrawValue(Self, ACanvas, ARect);
end;

procedure TdxAlphaColorPropertyEditor.ListDrawValue(const Value: string;
...
procedure TdxAlphaColorPropertyEditor.ListMeasureHeight(const Value: string;
  ACanvas: TCanvas; var AHeight: Integer);
begin
  // do nothing
end;

procedure TdxAlphaColorPropertyEditor.ListMeasureWidth(const Value: string;
  ACanvas: TCanvas; var AWidth: Integer);
begin
  Inc(AWidth, ACanvas.TextHeight('M'));
end;

function TdxAlphaColorPropertyEditor.PropDrawNameRect
  (const ARect: TRect): TRect;
begin
  Result := ARect;
end;

function TdxAlphaColorPropertyEditor.PropDrawValueRect
  (const ARect: TRect): TRect;
begin
  Result := cxRectSetWidth(ARect, cxRectHeight(ARect));
end;

procedure TdxAlphaColorPropertyEditor.CalculatePreviewRect(var R: TRect;
  out APreviewRect: TRect);
begin
  APreviewRect := cxRectSetWidth(R, cxRectHeight(R));
  R.Left := APreviewRect.Right;
end;

procedure TdxAlphaColorPropertyEditor.DrawPreview(ACanvas: TCanvas; R: TRect;
  AColor: TdxAlphaColor; ASelected: Boolean);
...
function TdxAlphaColorPropertyEditor.ValueToAlphaColor(const AValue: string)
  : TdxAlphaColor;
var
  AColor: Integer;
begin
...
function TdxActivityIndicatorPropertiesEditor.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes;
...
begin
  APropertiesClass := TdxActivityIndicatorPropertiesClass
    (GetRegisteredActivityIndicatorProperties.FindByClassName(Value));
  if APropertiesClass = nil then
    APropertiesClass := TdxActivityIndicatorPropertiesClass
      (GetRegisteredActivityIndicatorProperties.FindByDescription(Value));
  for I := 0 to PropCount - 1 do
    TdxCustomActivityIndicatorAccess(GetComponent(I)).PropertiesClass :=
      APropertiesClass;
  Modified;
end;
...
begin
  EditGallery((GetComponent(0) as TdxWheelPickerProperties)
    .Owner as TdxWheelPicker);
end;
...
begin
  Result := TcxCustomEditProperties
    (TdxWheelPickerWheelsAccess(TdxWheelPickerWheelAccess
    (TdxWheelPickerItemsAccess(TdxWheelPickerItemAccess(GetComponent(0))
    .Collection).Owner).Collection).Owner).Images;
end;

{ TdxWheelPickerItemCollectionProperty }

function TdxWheelPickerItemCollectionProperty.GetAttributes
  : TPropertyAttributes;
begin
...
begin
  ShowCollectionEditor(Designer, Component,
    TdxSparklineProperties((GetEdit as TdxCustomSparklineEdit).Properties)
    .Series, 'Series');
end;
...
begin
  ABarCodeSymbologyClass := TdxCustomBarCodeSymbologyClass
    (dxGetRegisteredBarCodeSymbologies.FindByClassName(Value));
  if ABarCodeSymbologyClass = nil then
    ABarCodeSymbologyClass := TdxCustomBarCodeSymbologyClass
      (dxGetRegisteredBarCodeSymbologies.FindByDescription(Value));

  ObjectInspectorCollapseProperty;
  for I := 0 to PropCount - 1 do
    TdxCustomBarCodeProperties(GetComponent(I)).BarCodeSymbologyClass :=
      ABarCodeSymbologyClass;
...
begin
  Result := TdxTokenEditProperties(TdxTokenEditToken(GetComponent(0))
    .Collection.Owner).Images;
end;
...
begin
  ShowCollectionEditor(Designer, Component, (GetEdit as TdxCustomTokenEdit)
...
begin
  if dxShowFormattedLabelCaptionEditor(GetComponent(0) as TdxFormattedLabel)
  then
    Designer.Modified;
end;

function TdxFormattedLabelFormattedTextPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
  Result := [paDialog];
end;
...
function TdxFormattedTextPropertyEditor.UseFormattedTextEditor
  (var AIntf: IdxFormattedTextOwner): Boolean;
begin
  Result := Supports(GetComponent(0), IdxFormattedTextOwner, AIntf) and
    AIntf.GetUseFormattedText;
end;
