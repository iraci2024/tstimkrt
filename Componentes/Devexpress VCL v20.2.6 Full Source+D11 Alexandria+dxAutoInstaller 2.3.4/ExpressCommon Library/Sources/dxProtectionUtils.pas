...
type
  TdxGetPasswordEvent = function(Sender: TObject; {$IFDEF BCBCOMPATIBLE}var
    {$ELSE}out {$ENDIF} Password: string): Boolean of object;

  TdxHashAlgorithmType = (None, MD2, MD4, MD5, SHA1, MAC, Ripemd, Ripemd160,
    Reserved1, HMAC, Reserved2, Reserved3, SHA256, SHA384, SHA512);
...
{$REGION 'strict private const'}
    InitialValues: array [0 .. 14] of Integer = ($E1F0, $1D0F, $CC9C, $84C0,
      $110C, $0E10, $F1CE, $313E, $1872, $E139, $D40F, $84F9, $280C,
      $A96A, $4EC3);
...
{$ENDREGION}
  protected
    class function CalculateLegacyPasswordHashInt(const APassword
      : string): Integer;
    class function ProcessLowWordByte(AKey: Integer; B: Byte): Integer;
    class function ProcessHighWordByte(AKey: Integer; B: Byte;
  ...
  public
    ...
    class function CalculateOpenOfficePasswordHash(const APassword
      : string): TBytes;
    class function CalculatePasswordBytes(const APassword: string): TBytes;

    class function CalculatePasswordHash(const APassword: string;
      const ASalt: TBytes; ASpinCount: Integer; AHashType: TdxHashAlgorithmType;
      AIteratorFirst: Boolean = False): TBytes; overload;
    class procedure CalculatePasswordHash(const APassword: string;
      const ASalt: TBytes; ASpinCount: Integer; AHash: TdxHashAlgorithm;
      AIteratorFirst: Boolean = False); overload;
    class procedure CalculatePasswordHash(const APassword: TBytes;
      const ASalt: TBytes; ASpinCount: Integer; AHash: TdxHashAlgorithm;
      AIteratorFirst: Boolean = False); overload;

    class function CalculateRichEditPasswordHash(const APassword: string;
      const ASalt: TBytes; ASpinCount: Integer;
      AHashAlgorithmType: TdxHashAlgorithmType): TBytes; overload;
    class function CalculateRichEditPasswordHash(const APassword: string;
      const ASalt: TBytes; ASpinCount: Integer;
      AHashAlgorithm: TdxHashAlgorithmClass): TBytes; overload;

    class function GetHashAlgorithm(AHashAlgorithmType: TdxHashAlgorithmType)
      : TdxHashAlgorithmClass; virtual;
  end;

function dxCallGetPasswordEvent(AEvent: TdxGetPasswordEvent; ASender: TObject;
...
class function TdxPasswordHashCodeCalculator.CalculateKeyHighWord
...
class function TdxPasswordHashCodeCalculator.CalculateKeyLowWord
  (const ABytes: TBytes): Integer;
var
  ACount, I: Integer;
begin
...
class function TdxPasswordHashCodeCalculator.CalculateLegacyPasswordHash
...
class function TdxPasswordHashCodeCalculator.CalculateOpenOfficePasswordHash
  (const APassword: string): TBytes;
begin
  Result := TdxSHA1HashAlgorithm.Calculate
    (TEncoding.Unicode.GetBytes(APassword));
end;

class function TdxPasswordHashCodeCalculator.CalculatePasswordBytes
  (const APassword: string): TBytes;
var
...
class function TdxPasswordHashCodeCalculator.CalculatePasswordHash
  (const APassword: string; const ASalt: TBytes; ASpinCount: Integer;
  AHashType: TdxHashAlgorithmType; AIteratorFirst: Boolean = False): TBytes;
var
...
class procedure TdxPasswordHashCodeCalculator.CalculatePasswordHash
  (const APassword: string; const ASalt: TBytes; ASpinCount: Integer;
  AHash: TdxHashAlgorithm; AIteratorFirst: Boolean = False);
begin
  CalculatePasswordHash(TEncoding.Unicode.GetBytes(APassword), ASalt,
    ASpinCount, AHash, AIteratorFirst);
end;

class procedure TdxPasswordHashCodeCalculator.CalculatePasswordHash
  (const APassword, ASalt: TBytes; ASpinCount: Integer; AHash: TdxHashAlgorithm;
  AIteratorFirst: Boolean = False);

  function CalculateHash(AData1, AData2: Pointer;
    AData1Size, AData2Size: Integer): TBytes; overload;
  begin
    AHash.Reset;
  ...
  begin
    Result := CalculateHash(@AData1[0], @AData2[0], Length(AData1),
      Length(AData2));
  end;
...
class function TdxPasswordHashCodeCalculator.CalculateRichEditPasswordHash
  (const APassword: string; const ASalt: TBytes; ASpinCount: Integer;
  AHashAlgorithmType: TdxHashAlgorithmType): TBytes;
var
  AHashAlgorithm: TdxHashAlgorithmClass;
begin
  AHashAlgorithm := GetHashAlgorithm(AHashAlgorithmType);
  if AHashAlgorithm <> nil then
    Result := CalculateRichEditPasswordHash(APassword, ASalt, ASpinCount,
      AHashAlgorithm)
  else
    Result := TdxByteArray.Concatenate(nil,
      CalculateLegacyPasswordHashInt(APassword));
end;

class function TdxPasswordHashCodeCalculator.CalculateRichEditPasswordHash
  (const APassword: string; const ASalt: TBytes; ASpinCount: Integer;
  AHashAlgorithm: TdxHashAlgorithmClass): TBytes;
var
  AHash: TdxHashAlgorithm;
begin
  AHash := AHashAlgorithm.Create;
  try
    CalculatePasswordHash(TEncoding.Unicode.GetBytes
      (IntToHex(CalculateLegacyPasswordHashInt(APassword), 8)), ASalt,
      ASpinCount, AHash);
    Result := AHash.GetHash;
  finally
    AHash.Free;
...
class function TdxPasswordHashCodeCalculator.GetHashAlgorithm(AHashAlgorithmType
  : TdxHashAlgorithmType): TdxHashAlgorithmClass;
begin
  case AHashAlgorithmType of
    ...
    raise Exception.CreateFmt(sdxUnsupportedHashAlgorithm,
      [Ord(AHashAlgorithmType)]);
  end;
end;

class function TdxPasswordHashCodeCalculator.CalculateLegacyPasswordHashInt
  (const APassword: string): Integer;
var
  ABytes: TBytes;
  AHigh, ALow: Cardinal;
begin
  ...
  Result := Integer(((ALow shl 24) and $FF000000) or
    ((ALow shl 8) and $00FF0000) or ((AHigh shl 8) and $0000FF00) or
    ((AHigh shr 8) and $000000FF));
end;

class function TdxPasswordHashCodeCalculator.ProcessLowWordByte(AKey: Integer;
  B: Byte): Integer;
begin
  Result := (((AKey shr 14) and $0001) or ((AKey shl 1) and $7FFF)) xor B;
end;

class function TdxPasswordHashCodeCalculator.ProcessHighWordByte(AKey: Integer;
  B: Byte; ARowIndex: Integer): Integer;
var
  AMask, I: Integer;
begin
  AMask := 1;
