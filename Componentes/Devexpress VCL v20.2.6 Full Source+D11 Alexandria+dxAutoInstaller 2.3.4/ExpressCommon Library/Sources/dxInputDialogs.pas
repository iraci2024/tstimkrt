...
type
  TdxInputQueryValidationProc = reference to procedure(ValueIndex: Integer;
    const Value: string; var IsValid: Boolean);

function dxInputBox(const ACaption, APrompt, ADefaultValue: string)
  : string; overload;
function dxInputQuery(const ACaption, APrompt: string; var AValue: string)
  : Boolean; overload;
function dxInputQuery(const ACaption, APrompt: string; var AValue: string;
  AValidationProc: TdxInputQueryValidationProc): Boolean; overload;
function dxInputQuery(const ACaption: string; const APrompts: array of string;
  var AValues: array of string): Boolean; overload;
function dxInputQuery(const ACaption: string; const APrompts: array of string;
  var AValues: array of string; AValidationProc: TdxInputQueryValidationProc)
  : Boolean; overload;
function dxSelectQuery(const ACaption: string; const APrompt: string;
  AValues: TStrings; var AValue: string; AAllowCustomValues: Boolean = False;
  AValidationProc: TdxInputQueryValidationProc = nil): Boolean;

implementation

uses
  SysUtils, Generics.Collections, Generics.Defaults, dxForms,
  cxTextEdit, cxButtons, cxLabel, cxGeometry, dxCoreClasses, cxClasses, dxCore,
  cxGraphics, cxDropDownEdit, Math;
...
type
  ...
  protected
    ...
    procedure DoShow; override;
  public
    procedure Initialize(AValues: TStrings; const AValue: string;
...
function dxInputQuery(const ACaption, APrompt: string; var AValue: string;
  AValidationProc: TdxInputQueryValidationProc): Boolean;
var
...
begin
  ...
  FCancelButton.BoundsRect := cxRectSetRight(R, R.Right,
    ScaleFactor.Apply(ButtonWidth));
  R := FCancelButton.BoundsRect;
  FOkButton.BoundsRect := cxRectSetRight(R, R.Left - ScaleFactor.Apply(6),
    ScaleFactor.Apply(ButtonWidth));
...
procedure TdxInputDialog.InitializeEditor(AIndex: Integer;
  const ACaption, AValue: string);
begin
...
procedure TdxSelectDialog.Initialize(AValues: TStrings; const AValue: string;
  AAllowCustomValues: Boolean);
const
...
begin
  ...
  Editor.ItemIndex := Max(AValues.IndexOf(AValue),
    IfThen(AAllowCustomValues, -1));
