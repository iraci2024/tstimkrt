...
uses
  ...
  ClipBrd, ComCtrls, Controls, Dialogs,
  Forms, Graphics, Menus, RichEdit, StdCtrls, SysUtils,
  cxDataUtils, dxCore, dxCoreClasses, dxMessages, cxClasses, cxContainer,
  cxControls, cxEdit, cxDrawTextUtils,
  cxGraphics, cxLookAndFeels, cxMemo, cxScrollbar, cxTextEdit, dxGDIPlusApi,
  cxRichEditUtils, cxExtEditConsts,
  dxDrawRichTextUtils, cxGeometry, dxDPIAwareUtils;
...
type
  TcxRichEditStreamMode = (resmSelection, resmPlainRtf, resmRtfNoObjs,
    resmUnicode, resmTextIzed);
  ...
  TcxRichEditURLClickEvent = procedure(Sender: TcxCustomRichEdit;
    const URLText: string; Button: TMouseButton) of object;
  TcxRichEditURLMoveEvent = procedure(Sender: TcxCustomRichEdit;
    const URLText: string) of object;
  TcxRichEditQueryInsertObjectEvent = procedure(Sender: TcxCustomRichEdit;
    var AAllowInsertObject: Boolean; const ACLSID: TCLSID) of object;
  ...
  public
    ...
    property AdvancedTypography: Boolean read FAdvancedTypography
      write SetAdvancedTypography;
    ...
    property PlainText: Boolean read FPlainText write SetPlainText
      default False;
    property RichEditClass: TcxRichEditClass read FRichEditClass
      write SetRichEditClass default cxMaxVersionRichEditClass;
    // !!!
    ...
    property OnQueryInsertObject: TcxRichEditQueryInsertObjectEvent
      read FOnQueryInsertObject write SetOnQueryInsertObject;
  ...
  public
    ...
    // IOleUILinkInfo
    function GetLastUpdate(dwLink: longint; var LastUpdate: TFileTime)
      : HRESULT; stdcall;

    // IOleUILinkContainer
    function GetNextLink(dwLink: longint): longint; stdcall;
    function SetLinkUpdateOptions(dwLink: longint; dwUpdateOpt: longint)
      : HRESULT; stdcall;
    function GetLinkUpdateOptions(dwLink: longint; var dwUpdateOpt: longint)
      : HRESULT; stdcall;
    function SetLinkSource(dwLink: longint; pszDisplayName: PChar;
      lenFileName: longint; var chEaten: longint; fValidateSource: BOOL)
      : HRESULT; stdcall;
  ...
  public
    ...
    // IOleUIObjInfo
    function GetObjectInfo(dwObject: longint; var dwObjSize: longint;
      var lpszLabel: PChar; var lpszType: PChar; var lpszShortType: PChar;
    ...
    function ConvertObject(dwObject: longint; const clsidNew: TCLSID)
      : HRESULT; stdcall;
    ...
    function SetViewInfo(dwObject: longint; hMetaPict: HGLOBAL;
      dvaspect: longint; nCurrentScale: Integer; bRelativeToOrig: BOOL)
      : HRESULT; stdcall;
  end;
  ...
  public
    ...
    procedure InsertTable(AColumnCount, ARowCount: Integer;
    ...
    property DefAttributes: TTextAttributes read GetDefAttributes
      write SetDefAttributes;
    property DefAttributes2: TcxTextAttributes2 read FDefAttributes2
      write SetDefAttributes2;
    property DefaultConverter: TConversionClass read GetDefaultConverter
      write SetDefaultConverter;
    ...
    property SelAttributes2: TcxTextAttributes2 read FSelAttributes2
      write SetSelAttributes2;
  end;
  ...
  TcxRichEditStreamOperationInfo = record
    ...
    TextType: longint;
    Encoding: TEncoding end;

    TcxRichEditStrings = class(TStrings)private FConverter: TConversion;
    FIsModification: Boolean;
    FRichEdit: TcxRichInnerEdit;
    FTextType: longint;
    function CalcStreamTextType(AStreamOperation: TcxRichEditStreamOperation;
      ACustom: Boolean = False; ACustomStreamModes: TcxRichEditStreamModes = []
      ): longint;
    function GetAllowStreamModesByStreamOperation(AStreamOperation
      : TcxRichEditStreamOperation): TcxRichEditStreamModes;
    function GetStreamModes: TcxRichEditStreamModes;
  protected
  ...
  TcxRichInnerEdit = class(TRichEdit, IcxContainerInnerControl,
    IcxInnerEditHelper, IcxPaintControlsHelper)
  private
    FAdvancedTypography: Boolean;
    FAllowObjects: Boolean;
    FAutoSelect: Boolean;
    ...
    procedure WMAfterDpiChange(var Message: TMessage);
      message WM_DPICHANGED_AFTERPARENT;
    procedure WMBeforeDpiChange(var Message: TMessage);
      message WM_DPICHANGED_BEFOREPARENT;
    ...
    procedure EMExLineFromChar(var Message: TMessage);
  ...
  protected
    procedure BeforeInsertObject(var AAllowInsertObject: Boolean;
      const ACLSID: TCLSID); dynamic;
...
{$ENDIF}
    ...
    property AdvancedTypography: Boolean read GetAdvancedTypography
      write SetAdvancedTypography default False;
    property AllowObjects: Boolean read FAllowObjects write SetAllowObjects
      default True;
    ...
    property MemoMode: Boolean read FMemoMode write SetMemoMode default False;
    property RichEditClass: TcxRichEditClass read FRichEditClass
      write SetRichEditClass;
    property RichEditOle: IcxRichEditOle read GetRichEditOle;
    property RichEditOleCallBack: TcxRichEditOleCallback
      read GetRichEditOleCallBack;
  ...
  public
    ...
    function FindTexT(const ASearchStr: string; AStartPos, ALength: longint;
      AOptions: TSearchTypes; AForward: Boolean = True): Integer;
    function InsertObject: Boolean;
    function ShowObjectProperties: Boolean;
    function PasteSpecial: Boolean;
...
function AdjustRichLineBreaks(ADest, ASource: PChar;
  AShortBreak: Boolean = False): Integer;
function AdjustRichLineBreaksA(ADest, ASource: PAnsiChar;
  AShortBreak: Boolean = False): Integer;
function AdjustRichLineBreaksW(ADest, ASource: PWideChar;
  AShortBreak: Boolean = False): Integer;
function GetAdjustedRichEditText(const ARichEdit: TcxCustomRichEdit)
  : string; overload;
function GetAdjustedRichEditText(const AText: string;
  AUseShortLineBreak: Boolean): string; overload;
...
procedure cxDrawRichEdit(ADC: HDC; const ARect: TRect; ARichHandle: HWND;
  AMinCharIndex, AMaxCharIndex: Integer; ACalculateHeight: Boolean;
  out AHeight: Integer); overload;
procedure cxDrawRichEdit(ACanvas: TCanvas; const ARect: TRect;
  ARichEdit: TcxRichEdit; AMinCharIndex, AMaxCharIndex: Integer;
  ACalculateHeight: Boolean; out AHeight: Integer); overload;
...
{$IFDEF DELPHI18}
  AnsiStrings,
{$ENDIF}
  dxSpellCheckerCore, cxEditPaintUtils, cxEditUtils, cxVariants, cxDWMAPI,
  dxUxTheme, dxThemeConsts, dxThemeManager,
...
const
  RTFConversionFormat: TcxConversionFormat = (ConversionClass: TConversion;
    Extension: 'rtf'; Next: nil);
  TextConversionFormat: TcxConversionFormat = (ConversionClass: TConversion;
...
const
  ...
  cxRichEditClassNames: array [TcxRichEditClass] of string = ('RICHEDIT',
    'RICHEDIT20W', 'RICHEDIT50W');
...
begin
  ...
  OleCheck(StgCreateDocfileOnILockBytes(ALockBytes, STGM_READWRITE or
    STGM_SHARE_EXCLUSIVE or STGM_CREATE, 0, AStorage));
...
begin
  ...
  SetWindowPos(Wnd, 0, (GetSystemMetrics(SM_CXSCREEN) - cxRectWidth(ARect))
    div 2, (GetSystemMetrics(SM_CYSCREEN) - cxRectHeight(ARect)) div 3, 0, 0,
...
begin
  ...
  begin
    ...
    begin
      if AMoniker.Enum(True, AEnumMoniker) <> 0 then
        Exit;
...
begin
  ...
  begin
    ...
    if (AMkFirst <> nil) and (AMkFirst.IsSystemMoniker(AMksys) = 0) and
      (AMksys = MKSYS_FILEMONIKER) and (CreateBindCtx(0, ABindCtx) = 0) and
...
function cxGetIconMetaPict(AOleObject: IOleObject;
  ADrawAspect: longint): HGLOBAL;
var
...
begin
  Result := SendMessage(ARichEdit.Handle, EM_GETOLEINTERFACE, 0,
    LParam(@AOleInterface)) <> 0;
end;

function cxRichEditSetOleCallback(ARichEdit: TcxRichInnerEdit;
  AOleInterface: IcxRichEditOleCallback): Boolean;
begin
  Result := SendMessage(ARichEdit.Handle, EM_SETOLECALLBACK, 0,
    LParam(AOleInterface)) <> 0;
end;
...
begin
  Result := GetAdjustedRichEditText(ARichEdit.Text,
    ARichEdit.RichVersion >= 200);
end;

function GetAdjustedRichEditText(const AText: string;
  AUseShortLineBreak: Boolean): string;
var
  ALen: Integer;
begin
  ALen := Length(AText);
  ...
  if ALen > 0 then
  begin
    ALen := AdjustRichLineBreaksW(PWideChar(Result), PWideChar(AText),
      AUseShortLineBreak);
    SetLength(Result, ALen);
  end;
end;

function cxRichEditStreamLoad(dwCookie: DWORD_PTR; pbBuff: PByte; cb: longint;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          P := TEncoding.Convert(TEncoding.Default, TEncoding.Unicode,
            P, 0, pcb);
          pcb := Length(P);
        end
        else
        begin
          ...
          begin
            P := TEncoding.Convert(AStreamInfo.Encoding, TEncoding.Unicode,
              P, 0, pcb);
            pcb := Length(P);
          end;
        end;
...
begin
  ...
  try
    ...
    begin
      ...
      pcb := AStreamInfo.Converter.ConvertWriteStream(AStreamInfo.Stream, P,
        Length(P));
...
procedure SetRichDefAttributes(AEdit: TRichEdit; AFont: TFont;
  ATextColor: TColor);
begin
...
begin
  ...
  RichRenderer.RichEditClass := TcxCustomRichEditProperties(AProperties)
    .RichEditClass;
  ...
  if RichRenderer.Alignment <> TcxCustomRichEditProperties(AProperties).Alignment
  then
  begin
    RichRenderer.Alignment := TcxCustomRichEditProperties(AProperties)
      .Alignment;
    RichRenderer.Paragraph.Alignment := RichRenderer.Alignment;
  end;
  RichRenderer.AutoURLDetect := TcxCustomRichEditProperties(AProperties)
    .AutoURLDetect;
  RichRenderer.AllowObjects := TcxCustomRichEditProperties(AProperties)
    .AllowObjects;
  RichRenderer.AdvancedTypography := TcxCustomRichEditProperties(AProperties)
    .AdvancedTypography;
  RichRenderer.ZoomFactor := TcxCustomRichEditProperties(AProperties)
    .ZoomFactor;
...
procedure cxDrawRichEdit(ADC: HDC; const ARect: TRect; ARichHandle: HWND;
  ...
  out AHeight: Integer);
begin
  dxDrawRichEdit(ADC, cxRectSetNullOrigin(ARect), ARichHandle, AMinCharIndex,
...
begin
  ...
  try
    ...
    InitRichRenderer(AProperties, ANotScaledFont, AColor, ATextColor, AText,
      AScaleFactor);
    dxDrawRichEdit(ADC, cxRectSetNullOrigin(R), RichRenderer.Handle,
      AScaleFactor, 0, -1, ACalculateHeight, AHeight);
  finally
    ANotScaledFont.Free;
  end;
end;
...
procedure cxDrawRichEdit(ACanvas: TCanvas; const ARect: TRect;
  ...
  ACalculateHeight: Boolean; out AHeight: Integer); overload;
begin
  cxDrawRichEdit(ACanvas.Handle, ARect, ARichEdit.InnerControl.Handle,
...
begin
  ...
  GetDIB(ABitmap.Handle, ABitmap.Palette, PAnsiChar(ABitmapInfo)^,
    PAnsiChar(ABits)^);
  Result := '{\rtf1 {\pict\dibitmap0 ' +
    dxBinToHex(ABitmapInfo + ABits) + ' }}';
end;
...
begin
  ...
  if CanAllocateHandle(Edit) and not Edit.HandleAllocated and
    not Edit.Container.IsLoading then
    Edit.HandleNeeded;
...
function TcxRichEditStrings.CalcStreamTextType(AStreamOperation
  : TcxRichEditStreamOperation; ACustom: Boolean;
...
begin
  ...
  begin
    ...
    if (resmTextIzed in AStreamModes) and (resmTextIzed in AAllowStreamModes)
    then
      Result := SF_TEXTIZED;
  end
  else
  begin
    ...
    then
      Result := SF_RTFNOOBJS;
    if (resmPlainRtf in AStreamModes) and (resmPlainRtf in AAllowStreamModes)
    then
      Result := Result or SFF_PLAINRTF;
  end;
  if (resmSelection in AStreamModes) and (resmSelection in AAllowStreamModes)
    and not RichEdit.IsEditValueLocked then
    Result := Result or SFF_SELECTION;
end;

function TcxRichEditStrings.GetAllowStreamModesByStreamOperation
  (AStreamOperation: TcxRichEditStreamOperation): TcxRichEditStreamModes;
const
  AResultMap: array [Boolean] of TcxRichEditStreamModes =
    ([resmSelection, resmPlainRtf, resmRtfNoObjs, resmUnicode, resmTextIzed],
  ...
  try
    ...
    begin
      ...
      if ASelection.cpMin < 0 then
        ...
        ASelection.cpMin := ASelection.cpMin + FRichEdit.GetLineLength
          (Index - 1);
    ...
    AdjustRichLineBreaks(AStr, PChar(Format(AFormat, [S])),
      Length(GetLineBreakString) = 1);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          cxSendStructMessage(RichEdit.Handle, EM_STREAMIN, TextType,
            EditStream);
        end;
        if EditStream.dwError <> 0 then
          raise EOutOfResources.Create
            (cxGetResourceString(@cxSEditRichEditLoadFail));
        FTextType := TextType;
      end;
    finally
      if IsWin10OrLater then
...
begin
  try
    ...
    begin
      ...
      if EditStream.dwError <> 0 then
        raise EOutOfResources.Create
          (cxGetResourceString(@cxSEditRichEditSaveFail));
    end;
  finally
    if FConverter = nil then
      FreeAndNil(AStreamOperationInfo.StreamInfo.Converter);
...
begin
  ...
  begin
    Result := Text;
    while (Length(Result) > 0) and dxCharInSet(Result[Length(Result)],
...
{$ENDIF}
  ...
  while AConversionFormat <> nil do
    with AConversionFormat^ do
      if Extension <> AExtension then
        AConversionFormat := Next
      else
        Break;
...
procedure TcxRichEditStrings.InitStreamOperation(AStream: TStream;
  ...
  begin
    ...
    else if ContainsPreamble(AStream, TEncoding.BigEndianUnicode.GetPreamble)
    then
      Result := TEncoding.BigEndianUnicode
    else if ContainsPreamble(AStream, TEncoding.UTF8.GetPreamble) then
...
begin
  ...
  begin
    ...
    TextType := CalcStreamTextType(AStreamOperation, ACustom,
      ACustomStreamModes);
  end;
end;
...
begin
  ...
  InitStreamOperation(Stream, AStreamOperationInfo, esoSaveTo, True,
    ACustomStreamModes);
  try
    with AStreamOperationInfo do
    begin
      ...
      if EditStream.dwError <> 0 then
        raise EOutOfResources.Create
          (cxGetResourceString(@cxSEditRichEditSelectionSaveFail));
    end;
  finally
    if FConverter = nil then
      FreeAndNil(AStreamOperationInfo.StreamInfo.Converter);
...
begin
  if (Container = nil) or not Container.InnerControlDefaultHandler
...
function TcxRichInnerEdit.FindTexT(const ASearchStr: string;
  AStartPos, ALength: longint; AOptions: TSearchTypes;
  AForward: Boolean = True): Integer;

  function GetFindTextFlags: Integer;
  begin
    Result := IfThen(AForward, FR_Down) or IfThen(stWholeWord in AOptions,
      FR_WHOLEWORD) or IfThen(stMatchCase in AOptions, FR_MATCHCASE);
  end;
...
begin
  ...
  Result := cxSendStructMessage(Handle, EM_FINDTEXT, GetFindTextFlags,
    AFindText);
end;
...
begin
  ...
  try
    ...
    if OleUIInsertObjectA(AData) = OLEUI_OK then
      try
        AIsNewObject := AData.dwFlags and
        ...
        begin
          ...
          OleCheck(cxSetDrawAspect(AOleObject, AData.dwFlags and
            IOF_CHECKDISPLAYASICON <> 0, AData.hMetaPict, dvaspect));
        end;
        if HandleAllocated then
        begin
        ...
        begin
          ...
          RichEditOle.SetDvaspect(longint(REO_IOB_SELECTION),
            AReObject.dvaspect);
          if AIsNewObject then
            OleCheck(AReObject.oleobj.DoVerb(OLEIVERB_SHOW, nil, AOleClientSite,
              0, Handle, ClientRect));
          Result := True;
        end;
      finally
...
begin
  Result := False;
  if not Assigned(FRichEditOle) or (RichEditOle.GetObjectCount <= 0) then
    Exit;
  if HandleAllocated and not(SendMessage(Handle, EM_SELECTIONTYPE, 0, 0)
    in [SEL_OBJECT, SEL_MULTIOBJECT]) then
    Exit;
  ...
  OleCheck(RichEditOle.GetObject(longint(REO_IOB_SELECTION), AReObject,
    REO_GETOBJ_POLEOBJ or REO_GETOBJ_POLESITE or REO_GETOBJ_PSTG));
  ...
  begin
    ...
    ADialogCaption := Format(SPropDlgCaption,
      [cxGetOleObjectFullName(AReObject.oleobj)]);
...
function TcxRichInnerEdit.PasteSpecial: Boolean;
  ...
  procedure SetPasteFormats(var APasteFormat: TOleUIPasteEntry;
    AFormat: TClipFormat; ATymed: DWORD; const AFormatName, AResultText: string;
    AFlags: DWORD);
  begin
    with APasteFormat do
    begin
...
begin
  ...
  SetPasteFormats(APasteFormats[0], CFEmbeddedObject, TYMED_ISTORAGE, '%s',
    '%s', OLEUIPASTE_PASTE or OLEUIPASTE_ENABLEICON);
  SetPasteFormats(APasteFormats[1], CFLinkSource, TYMED_ISTREAM, '%s', '%s',
    OLEUIPASTE_LINKTYPE1 or OLEUIPASTE_ENABLEICON);
  SetPasteFormats(APasteFormats[2], CF_BITMAP, TYMED_GDI, 'Windows bitmap',
    'bitmap image', OLEUIPASTE_PASTE);
  SetPasteFormats(APasteFormats[3], CFRtf, TYMED_ISTORAGE, CF_RTF, CF_RTF,
    OLEUIPASTE_PASTE);
  SetPasteFormats(APasteFormats[4], CF_TEXT, TYMED_HGLOBAL, 'Unformatted text',
    'text without any formatting', OLEUIPASTE_PASTE);
  SetPasteFormats(APasteFormats[5], CFRETextObj, TYMED_ISTORAGE, CF_RETEXTOBJ,
    CF_RETEXTOBJ, OLEUIPASTE_PASTE);
  try
        ...
        try
          ...
          try
            ...
            begin
              ...
              dvaspect := DVASPECT_CONTENT;
              dwFlags := REO_RESIZABLE;
              OleCheck(cxSetDrawAspect(oleobj, AData.dwFlags and
                PSF_CHECKDISPLAYASICON <> 0, AData.hMetaPict, dvaspect));
            end;
            SendMessage(Handle, EM_EXGETSEL, 0, LParam(@ASelection));
            ASelection.cpMax := ASelection.cpMin + 1;
            if Succeeded(RichEditOle.InsertObject(AReObject)) then
            begin
              SendMessage(Handle, EM_EXSETSEL, 0, LParam(@ASelection));
              OleCheck(RichEditOle.SetDvaspect(longint(REO_IOB_SELECTION),
                AReObject.dvaspect));
            end;
          finally
            ZeroMemory(@AReObject, SizeOf(AReObject));
  ...
  begin
    ...
    begin
      if not cxRichEditGetOleInterface(Self, IcxRichEditOle(FRichEditOle)) then
        raise EOutOfResources.Create
          (cxGetResourceString(@cxSEditRichEditOleInterfaceFail));
    end;
    if not cxRichEditSetOleCallback(Self, RichEditOleCallBack) then
      raise EOutOfResources.Create
        (cxGetResourceString(@cxSEditRichEditCallBackFail));
    SendMessage(Handle, EM_AUTOURLDETECT,
      WParam(FAutoURLDetect and not MemoMode and ((Container = nil) or
      not Container.IsDesigning)), 0);
    SendMessage(Handle, EM_SETTYPOGRAPHYOPTIONS,
      WParam(AdvancedTypographyMap[FAdvancedTypography]),
...
begin
  ...
  begin
    if Container <> nil then
    begin
      PlainText := Container.ActiveProperties.PlainText or
        Container.ActiveProperties.MemoMode;
      Container.AdjustInnerEditFont;
      RestoreEditValue;
    end;
    SendMessage(Handle, EM_SETEVENTMASK, 0, ENM_CHANGE or ENM_SELCHANGE or
      ENM_IMECHANGE or ENM_REQUESTRESIZE or ENM_PROTECTED or ENM_KEYEVENTS or
      ENM_LINK or ENM_LANGCHANGE or ENM_OBJECTPOSITIONS or ENM_SCROLL);
    if MaxLength = 0 then
...
procedure TcxRichInnerEdit.DoGetGestureOptions(var Gestures
...
begin
  ...
  begin
    ...
    if (igPan in Gestures) and
      ((ScrollBars = ssNone) or (ScrollBars = ssHorizontal)) then
...
begin
  _TcxContainerAccess.DragOver(Container, Source, Left + X, Top + Y,
    State, Accept);
end;
...
procedure TcxRichInnerEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TcxRichInnerEdit.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
begin
  ...
  if FRichEditLibrary = 0 then
    raise EcxEditError.Create
      (cxGetResourceString(@cxSEditRichEditLibraryError));
...
procedure TcxRichInnerEdit.URLClick(const AURLText: string;
...
begin
  Result := HandleAllocated and (SendMessage(Handle, EM_CANPASTE, 0, 0) <> 0);
end;
...
begin
  Result := not dxIsProcessPerMonitorV2Aware or (RichEditClass <> recRichEdit41)
end;
...
begin
  ...
  begin
    AZoomFactor := AZoomFactor * ScaleFactor.TargetDPI /
      dxSystemScaleFactor.TargetDPI;
    AZoomFactor := cxRichEdit.CheckZoomFactor(AZoomFactor);
  end;
  SendMessage(Handle, EM_SETZOOM,
    Trunc(AZoomFactor * cxRichEditZoomDenominator), cxRichEditZoomDenominator);
end;
...
begin
  ...
  try
    if not VarIsNull(FSavedEditValue) and (Container <> nil) and HandleAllocated
    then
    begin
...
begin
  ...
  if MemoMode and (ATextMode and TM_PLAINTEXT <> 0) or not MemoMode and
    (ATextMode and TM_RICHTEXT <> 0) and not AForcedReload then
    Exit;
  AText := Text;
...
begin
  try
    ...
    if AForm <> nil then
      ...
      begin
        if (AForm.ActiveOleControl <> nil) and (AForm.ActiveOleControl <> Self)
        then
          AForm.ActiveOleControl.Perform(CM_UIDEACTIVATE, 0, 0);
        AForm.ActiveOleControl := Self;
        if AllowObjects and CanFocus then
          SetFocus;
      end
      else
      begin
...
begin
  ...
  if (AKey <> 0) and not Container.CanKeyDownModifyEdit(AKey, AShiftState) and
    not CheckTextEditState(Helper, APrevState) and not Container.IsNavigationKey
    (AKey, AShiftState) then
...
procedure TcxRichInnerEdit.WMWindowPosChanging(var Message
...
begin
  ...
  if (Container <> nil) and not(csDestroying in ComponentState) and
    Container.NeedsScrollBars and Container.HScrollBar.Visible and
        ...
        EN_LINK:
          ...
          begin
            case Msg of
              ...
              WM_RBUTTONUP:
                begin
                  if (FURLClickBtn = mbRight) and
                    (FURLClickRange.cpMin = chrg.cpMin) and
...
begin
  if Assigned(FRichEditOleCallback) then
    with TcxRichEditOleCallback(FRichEditOleCallback) do
      if Assigned(DocParentForm) and cxIsFormMDIChild(DocParentForm.Form) then
      begin
...
begin
  if Message.NotifyCode = EN_CHANGE then
    if (Container <> nil) and not Container.IsDestroying and
      (Container.ComponentState * [csLoading, csReading] = []) and Focused and
      not Container.ChangeEventsCatcher.IsLocked then
      Container.DoEditing;
...
begin
  ...
  dxDrawRichEdit(Message.WParam, cxRectSetNullOrigin(ClientRect), Handle,
    ScaleFactor, ACharIndex, -1, False, AHeight);
end;

{ TcxOleUILinkInfo }

constructor TcxOleUILinkInfo.Create(AOwner: TcxRichInnerEdit;
  AReObject: TReObject);
begin
  inherited Create;
  FRichEdit := AOwner;
...
function TcxOleUILinkInfo.GetLinkSource(dwLink: longint;
...
begin
  OleCheck(FReObject.oleobj.DoVerb(OLEIVERB_SHOW, nil, FReObject.olesite, 0,
    FRichEdit.Handle, FRichEdit.ClientRect));
  Result := S_OK;
end;
...
constructor TcxOleUIObjInfo.Create(AOwner: TcxRichInnerEdit;
  AReObject: TReObject);
begin
  inherited Create;
  FRichEdit := AOwner;
...
function TcxOleUIObjInfo.GetObjectInfo(dwObject: longint;
  var dwObjSize: longint; var lpszLabel: PChar; var lpszType: PChar;
  var lpszShortType: PChar; var lpszLocation: PChar): HRESULT;
begin
...
function TcxOleUIObjInfo.ConvertObject(dwObject: longint;
...
function TcxOleUIObjInfo.SetViewInfo(dwObject: longint; hMetaPict: HGLOBAL;
  dvaspect: longint; nCurrentScale: Integer; bRelativeToOrig: BOOL): HRESULT;
var
  AShowAsIcon: Boolean;
begin
...
begin
  ...
  else if Style = ActiveStyle then
    AdjustInnerEditFont;
  if InnerRich.HandleAllocated then
    SendMessage(InnerRich.Handle, EM_SETBKGNDCOLOR, 0,
      ColorToRGB(ViewInfo.BackgroundColor))
  else
...
function TcxCustomRichEdit.CanKeyDownModifyEdit(Key: Word;
...
function TcxCustomRichEdit.DoShowPopupMenu(AMenu: TComponent;
  X, Y: Integer): Boolean;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      if AWidth = -1 then
        AWidth := cxRichEditCalculateBestFit(ADC, S, Properties, Style.Font,
          ScaleFactor);
      Result.cx := AWidth;
      if IsInplace and (AEditSizeProperties.Width <> -1) then
        Dec(AWidth, 2);
      cxDrawRichEdit(ADC, Rect(0, 0, AWidth, 0), S, Properties, Style.Font,
        clWhite, clBlack, True, AHeight, ScaleFactor);
      if AHeight > 0 then
...
function TcxRichEditOleCallback.GetClipboardData(const chrg: TCharRange;
  reco: DWORD; out dataobj: IDataObject): HRESULT;
begin
  Result := E_NOTIMPL;
end;

function TcxRichEditOleCallback.GetContextMenu(seltype: Word;
...
function TcxRichEditOleCallback.GetDragDropEffect(fDrag: BOOL;
...
begin
  ...
  if not fDrag then
  begin
    if ((grfKeyState and (MK_CONTROL or MK_SHIFT)) = (MK_CONTROL or MK_SHIFT))
    then
...
function TcxRichEditOleCallback.QueryAcceptData(dataobj: IDataObject;
...
begin
  if (GetParentForm(FEdit) <> nil) and not Assigned(FParentFrame) and FEdit.AllowObjects
  then
  begin
...
function TcxCustomRichEditProperties.GetDisplayText(const AEditValue
...
function TcxCustomRichEditProperties.GetEditValueSource(AEditFocused: Boolean)
...
function TcxCustomRichEditProperties.GetSupportedOperations
...
procedure TcxCustomRichEditProperties.DoAssign(AProperties
...
class function TcxCustomRichEditProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TcxCustomRichEditViewData;
end;

function TcxCustomRichEditProperties.SupportsMultiThreading(AUseDisplayValue
  : Boolean): Boolean;
begin
  Result := False;
end;

class function TcxCustomRichEditProperties.GetViewInfoClass
...
procedure TcxCustomRichEditProperties.SetAdvancedTypography
  (const Value: Boolean);
begin
  if FAdvancedTypography <> Value then
  begin
...
procedure TcxCustomRichEditProperties.SetRichEditClass
  (AValue: TcxRichEditClass);
begin
  if (AValue <> FRichEditClass) and (AValue >= cxMinVersionRichEditClass) and
    (AValue <= cxMaxVersionRichEditClass) then
  begin
    FRichEditClass := AValue;
    Changed;
  end;
...
procedure TcxCustomRichEditProperties.SetStreamModes
  (const Value: TcxRichEditStreamModes);
begin
...
begin
  if IsInplace or (BorderStyle = ebsNone) or not IsWinVistaOrLater then
    inherited DrawNativeStyleEditBackground(ACanvas, ADrawBackground,
      ABackgroundBrush)
  else
    dxDrawThemeBackground(ACanvas.Handle, totEdit, ScaleFactor, Bounds,
...
procedure TcxCustomRichEditViewInfo.DrawText(ACanvas: TcxCanvas);
  ...
  begin
    ...
    begin
      if (DrawBitmap = 0) or (PrevDrawBitmapSize.cx <> cxRectWidth(TextRect)) or
        (PrevDrawBitmapSize.cy <> cxRectHeight(TextRect)) then
      begin
        if DrawBitmap <> 0 then
          DeleteObject(DrawBitmap);
        DrawBitmap := CreateCompatibleBitmap(ACanvas.Handle,
          cxRectWidth(TextRect), cxRectHeight(TextRect));
      end;
      ADC := CreateCompatibleDC(ACanvas.Handle);
      APrevBitmap := 0;
      try
        APrevBitmap := SelectObject(ADC, DrawBitmap);
        cxDrawRichEdit(ADC, TextRect, Text,
          TcxCustomRichEditProperties(EditProperties), Font, BackgroundColor,
          TextColor, False, ATempVar, ScaleFactor);
      finally
...
begin
  ...
  try
    APrevBitmap := SelectObject(ADC, DrawBitmap);
    with TextRect do
      BitBlt(ACanvas.Handle, Left, Top, Right - Left, Bottom - Top, ADC, 0,
        0, SRCCOPY);
  finally
    if APrevBitmap <> 0 then
      SelectObject(ADC, APrevBitmap);
...
function TcxCustomRichEditViewInfo.GetBackgroundPaintingStyle
  : TcxEditBackgroundPaintingStyle;
begin
  Result := bpsComboListEdit;
end;

function TcxCustomRichEditViewInfo.GetUpdateRegion
...
procedure TcxCustomRichEdit.InternalValidateDisplayValue(const ADisplayValue
  : TcxEditValue);
begin
end;
...
begin
  ...
  if InnerControl.HandleAllocated and
    Assigned(TdxSpellCheckerInstance.ISpellChecker) then
  begin
...
begin
  InnerRich.HideScrollBars := ActiveProperties.HideScrollBars and
    not IsPopupScrollBars;
...
begin
  inherited;
  AVisibleFont.Height := dxSystemScaleFactor.Apply(AVisibleFont.Height,
    ScaleFactor);
end;
...
function TcxCustomRichEdit.IsNavigationKey(Key: Word;
  Shift: TShiftState): Boolean;
begin
  Result := (((Key = VK_UP) or (Key = VK_DOWN) or (Key = VK_LEFT) or
    (Key = VK_RIGHT)) and (Shift = [])) or (Key = VK_NEXT) or (Key = VK_PRIOR)
...
procedure TcxCustomRichEdit.UpdateEditPopupMenuItems(APopupMenu: TComponent);
  ...
  begin
    APopupMenu.Items[0].Enabled := InnerRich.CanUndo and
      ((ActiveProperties.RichEditClass >= recRichEdit20) or
      not InnerRich.CanRedo);
...
function TcxCustomRichEdit.FindTexT(const ASearchStr: string;
  AStartPos, ALength: Integer; AOptions: TSearchTypes;
  AForward: Boolean = True): Integer;
begin
...
class function TcxCustomRichEdit.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxCustomRichEditProperties;
end;

procedure TcxCustomRichEdit.InsertTable(AColumnCount, ARowCount: Integer;
  AParams: TcxRichEditTableParams);
begin
...
begin
  ...
  else if (Parent = nil) or not Parent.HandleAllocated then
...
class procedure TcxCustomRichEdit.RegisterConversionFormat(const AExtension
