...
uses
  Windows, SysUtils, Classes, Controls, Dialogs, ExtCtrls, Forms, Graphics,
  cxGraphics,
...
begin
  if HasSelection and not IsTreeRoot(tvStruct.Selected) then
  begin
    ANode := tvStruct.Items.Add(tvStruct.Selected,
...
begin
  ...
  begin
    ANode := tvStruct.Items.AddChild(tvStruct.Selected,
      GenerateSubItemName(tvStruct.Selected));
    ANode.Data := TdxBreadcrumbEditEditorAdditionalData.Create('');
    tvStruct.Selected := ANode;
  end;
end;
...
function TdxBreadcrumbEditEditorForm.GenerateSubItemName
...
function TdxBreadcrumbEditEditorForm.GetAdditionalData(ANode: TTreeNode)
  : TdxBreadcrumbEditEditorAdditionalData;
begin
...
function TdxBreadcrumbEditEditorForm.GetSelectedPath(APathDelimiter
  : Char): string;
...
procedure TdxBreadcrumbEditEditorForm.Load(ABreadcrumbEdit: TdxBreadcrumbEdit);
  ...
  procedure LoadNode(AParentTreeNode: TTreeNode;
    ANode, ASelectedNode: TdxBreadcrumbEditNode);
  var
    ATreeNode: TTreeNode;
    I: Integer;
  begin
    ATreeNode := tvStruct.Items.AddChild(AParentTreeNode, ANode.Name);
    ATreeNode.Data := TdxBreadcrumbEditEditorAdditionalData.Create
      (ANode.DisplayName);
...
procedure TdxBreadcrumbEditEditorForm.Save(ABreadcrumbEdit: TdxBreadcrumbEdit);
  ...
  procedure SaveNode(ATreeNode, ASelectedTreeNode: TTreeNode;
...
begin
  ...
  try
    ...
    if cbSaveSelection.Checked then
      ABreadcrumbEdit.SelectedPath :=
        GetSelectedPath(ABreadcrumbEdit.Properties.PathEditor.PathDelimiter);
  finally
  ...
  begin
    ...
    if teItemDisplayName.EditingValue <> Null then
      GetAdditionalData(tvStruct.Selected).Text :=
        teItemDisplayName.EditingValue
    else
      GetAdditionalData(tvStruct.Selected).Text := '';
...
procedure TdxBreadcrumbEditEditorForm.tvStructChange(Sender: TObject;
  Node: TTreeNode);
begin
  UpdateControlsState;
...
procedure TdxBreadcrumbEditEditorForm.tvStructDragDrop(Sender, Source: TObject;
  X, Y: Integer);
var
...
procedure TdxBreadcrumbEditEditorForm.tvStructDragOver(Sender, Source: TObject;
...
begin
  ...
  Caption := Format(sdxBreadcrumbEditEditorCaption,
    [AOwnerCaption, AComponent.Name]);
end;
...
procedure TdxBreadcrumbEditEditorForm.tvStructDeletion(Sender: TObject;
  Node: TTreeNode);
begin
