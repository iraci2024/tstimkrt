...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Windows, Messages, Variants, Classes, Controls, Graphics, Forms, SysUtils,
  StdCtrls, MultiMon,
  ...
  cxDataStorage,
  cxTextEdit, cxVariants, cxFilterControlUtils, dxIncrementalFiltering,
  cxGeometry;
...
type
  ...
  TcxCustomDrawBorderEvent = procedure(AViewInfo: TcxContainerViewInfo;
    ACanvas: TcxCanvas; const R: TRect; var AHandled: Boolean;
    out ABorderWidth: Integer) of object;
  TcxEditDrawItemEvent = procedure(AControl: TcxCustomComboBox;
  ...
  TcxCustomEditPopupWindowViewInfo = class
    (TdxCustomIncrementalFilteringContainerViewInfo)
  strict private
    FOnCustomDrawBorder: TcxCustomDrawBorderEvent;
  protected
    function DrawCustomBorder(ACanvas: TcxCanvas; const R: TRect;
      out ABorderWidth: Integer): Boolean;
  ...
  public
    ...
    procedure DrawBorder(ACanvas: TcxCanvas; var R: TRect);
      reintroduce; virtual;
    function GetBorderExtent: TRect; virtual;
    function GetClientExtent: TRect; virtual;
    function GetSysPanelHeight: Integer; virtual;
    //
    property OnCustomDrawBorder: TcxCustomDrawBorderEvent
      read FOnCustomDrawBorder write FOnCustomDrawBorder;
  end;
  ...
  strict private
    ...
    procedure UpdateSysPanelState(const P: TPoint;
    ...
    procedure CMPopupControlKey(var Message: TMessage);
      message DXM_POPUPCONTROLKEY;
  protected
  ...
  public
    ...
    property BorderStyle: TcxEditPopupBorderStyle read FBorderStyle
    ...
    property MinSysPanelHeight: Integer read GetMinSysPanelHeight
      write SetMinSysPanelHeight;
  ...
  protected
    ...
    property GlyphButtonIndex: Integer read FGlyphButtonIndex
      write SetGlyphButtonIndex;
  public
    ...
    property ImmediateDropDownWhenActivated: Boolean
      read FImmediateDropDownWhenActivated write FImmediateDropDownWhenActivated
      default False;
    property ImmediateDropDownWhenKeyPressed: Boolean
      read FImmediateDropDownWhenKeyPressed
      write FImmediateDropDownWhenKeyPressed default True;
    ...
    property PopupAlignment: TAlignment read GetPopupAlignment
      write SetPopupAlignment default taLeftJustify;
    property PopupAutoSize: Boolean read FPopupAutoSize write FPopupAutoSize
      default True;
    property PopupClientEdge: Boolean read FPopupClientEdge
      write SetPopupClientEdge default False;
    property PopupDirection: TcxPopupDirection read FPopupDirection
      write FPopupDirection;
    property PopupHeight: Integer read FPopupHeight write SetPopupHeight
      default 200;
    property PopupHorzAlignment: TcxPopupAlignHorz read FPopupHorzAlignment
      write FPopupHorzAlignment;
    property PopupMinHeight: Integer read FPopupMinHeight
      write SetPopupMinHeight default 100;
    property PopupMinWidth: Integer read FPopupMinWidth write SetPopupMinWidth
      default 100;
    property PopupSizeable: Boolean read FPopupSizeable write SetPopupSizeable
      default False;
    property PopupSysPanelStyle: Boolean read FPopupSysPanelStyle
      write SetPopupSysPanelStyle default False;
    property PopupVertAlignment: TcxPopupAlignVert read FPopupVertAlignment
      write FPopupVertAlignment;
    property PopupWidth: Integer read FPopupWidth write SetPopupWidth
      default 250;
    ...
    property OnFinalizePopup: TNotifyEvent read FOnFinalizePopup
      write FOnFinalizePopup;
    property OnInitPopup: TNotifyEvent read FOnInitPopup write FOnInitPopup;
    property OnPopup: TNotifyEvent read FOnPopup write FOnPopup;
  published
    property ImmediateDropDown: Boolean read FImmediateDropDownWhenKeyPressed
      write FImmediateDropDownWhenKeyPressed stored False; // deprecated
    property ImmediatePopup: Boolean read FImmediateDropDownWhenActivated
      write FImmediateDropDownWhenActivated stored False; // deprecated
  end;

  TcxCustomDropDownEditPropertiesClass = class of
    TcxCustomDropDownEditProperties;
  ...
  private
    ...
    procedure CMDropDownByPaste(var Message: TMessage);
      message DXM_DROPDOWNBYPASTE;
    procedure CMHintShow(var Message: TCMHintShow); message CM_HINTSHOW;
  ...
  protected
    ...
    procedure PopupWindowCloseQuery(Sender: TObject;
      var CanClose: Boolean); dynamic;
    ...
    property PopupMouseMoveLocked: Boolean read FPopupMouseMoveLocked
      write FPopupMouseMoveLocked;
    property PopupSizeChanged: Boolean read FPopupSizeChanged;
    property SendChildrenStyle: Boolean read FSendChildrenStyle
      write FSendChildrenStyle;
    property PopupWindow: TcxCustomEditPopupWindow read FPopupWindow;
  public
    destructor Destroy; override;

    procedure Activate(var AEditData: TcxCustomEditData;
      ANeedSetFocus: Boolean = True); override;
  ...
  TcxComboBoxIncrementalFilteringHelper = class
  ...
  private
    ...
    function GetEdit: TcxCustomComboBox;
  protected
    function CreateIncrementalFilteringHelper
      : TcxComboBoxIncrementalFilteringHelper; virtual;
    ...
    property Edit: TcxCustomComboBox read GetEdit;
    property ActiveProperties: TcxCustomComboBoxProperties
      read GetActiveProperties;
  public
    constructor Create(AOwner: TPersistent); override;
    destructor Destroy; override;
    function CanResizeVisualArea(var NewSize: TSize; AMaxHeight: Integer = 0;
      AMaxWidth: Integer = 0): Boolean; override;
  ...
  public
    ...
    property Revertable: Boolean read FRevertable write FRevertable
      default False;
    property Sorted: Boolean read GetSorted write SetSorted default False;
    property OnDrawItem: TcxEditDrawItemEvent read FOnDrawItem
      write FOnDrawItem;
    property OnMeasureItem: TMeasureItemEvent read FOnMeasureItem
  ...
  public
    procedure Activate(var AEditData: TcxCustomEditData;
      ANeedSetFocus: Boolean); override;
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TcxCustomComboBoxProperties
      read GetActiveProperties;
    property ItemIndex;
  ...
  public
    destructor Destroy; override;
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TcxCustomPopupEditProperties
      read GetActiveProperties;
...
uses
  Math, Types, dxThemeConsts, dxThemeManager, dxUxTheme, cxEditConsts,
  cxEditPaintUtils,
...
procedure SetEditPopupWindowShadowRegion(APopupWindow
  : TcxCustomEditPopupWindow);
var
  R: TRect;
  AExcludeRect: TRect;
begin
  ...
  begin
    if not NativeStyle then
      ...
      begin
        ...
        begin
          ...
          AExcludeRect.TopLeft := APopupWindow.ScreenToClient
            (AExcludeRect.TopLeft);
          AExcludeRect.BottomRight := APopupWindow.ScreenToClient
            (AExcludeRect.BottomRight);
    ...
    SetWindowShadowRegion(APopupWindow.Handle, R, cxEmptyRect, NativeStyle,
      Shadow, AExcludeRect);
  end;
end;
...
procedure TcxCustomEditPopupWindowViewInfo.DrawBorder(ACanvas: TcxCanvas;
  var R: TRect);
var
...
function TcxCustomEditPopupWindowViewInfo.DrawCustomBorder(ACanvas: TcxCanvas;
  const R: TRect; out ABorderWidth: Integer): Boolean;
begin
  Result := False;
  ABorderWidth := 0;
...
procedure TcxCustomEditPopupWindowViewInfo.InternalPaint(ACanvas: TcxCanvas);
  ...
  begin
    DrawContainerShadow(ACanvas, cxRectContent(cxRectSetNullOrigin(Bounds),
      GetShadowOffsets));
  end;
...
begin
  ...
  if SysPanelStyle then
    Painter.DrawScaledPopupPanelBand(ACanvas, SysPanelBounds, SizeGripRect,
      CloseButtonRect, SizeGripCorner, CloseButtonState, GetSysPanelBorders,
      Painter.GetContainerBorderColor(False), ScaleFactor, True,
      ShowCloseButton);
...
function TcxCustomEditPopupWindowViewInfo.IsSearchInfoPanelInClientRect
  : Boolean;
begin
  Result := False;
end;
...
begin
  ...
  ALeftFlag := (AlignHorz = pahRight) and
    (Result.X + ASize.cx = AOwnerRect.Right) and
    (Result.X > ADesktopWorkArea.Left) or { pahRight + pdVertical }
    (Result.X + ASize.cx = AOwnerRect.Left) or { pahLeft + pdHorizontal }
    (Result.X + ASize.cx = ADesktopWorkArea.Right) and
    (Result.X < AOwnerRect.Right);
  ATopFlag := (AlignVert = pavBottom) and
    (Result.Y + Height = AOwnerRect.Bottom) and
    (Result.Y > ADesktopWorkArea.Top) or { pavBottom + pdHorizontal }
    (Result.Y + ASize.cy = AOwnerRect.Top) or { pavTop + pdVertical }
    (Result.Y + ASize.cy = ADesktopWorkArea.Bottom) and
    (Result.Y < AOwnerRect.Bottom);
...
begin
  ...
  if PopupAutoSize then
    APreferredWidth := AClientExtent.Left + AClientExtent.Right +
      APopupWindowVisualAreaSize.cx
  else if PopupWidth = 0 then
  ...
  if PopupAutoSize then
    APreferredHeight := AClientExtent.Top + AClientExtent.Bottom +
      APopupWindowVisualAreaSize.cy
  else if PopupHeight > 0 then
    APreferredHeight := PopupHeight
  else if APreferredHeight < MinHeight then
...
function TcxCustomEditPopupWindow.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  if Parent <> nil then
    Result := inherited DoMouseWheel(Shift, WheelDelta, MousePos)
  else
    Result := (GetOwnerControl <> nil) and TControlAccess(GetOwnerControl)
...
procedure TcxCustomEditPopupWindow.MouseDown(Button: TMouseButton;
...
begin
  ...
  if (Button = mbLeft) and (AHitTest in [htSizeGripNWSE .. htSizeNCWE]) and
    (GetCaptureControl <> nil) then
...
procedure TcxCustomEditPopupWindow.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if (Button = mbLeft) then
    if FSizingCapture then
      EndResize(True)
    else if (ViewInfo.CloseButtonState = cxbsPressed) and CloseQuery then
...
begin
  ...
  begin
    ...
    if SysPanelStyle then
    begin
      Painter.CalculateScaledPopupPanelClientRect(ClientRect, SysPanelBounds,
        SizeGripRect, CloseButtonRect, SizeGripCorner, GetBorders,
        GetSysPanelBorders, ScaleFactor, SysPanelHeight, ShowCloseButton,
        Sizeable);
  ...
  begin
    ...
    ViewInfo.SearchInfoPanelBounds.Top := ViewInfo.SearchInfoPanelBounds.Bottom
      - ViewInfo.GetSearchInfoPanelHeight;
...
function TcxCustomEditPopupWindow.CanResize(var NewWidth,
...
begin
  Result := True;
  if not Sizeable or (NewWidth = Width) and (NewHeight = Height) or CheckSizeLocked
  then
    Exit;
  AMinSize := MinSize;
  ...
  if ((NewHeight <> Height) or (NewWidth <> Width)) and
    Edit.ActiveProperties.UseLookupData and Edit.ActiveProperties.IsLookupDataVisual
  then
  begin
    AClientExtent := ViewInfo.GetClientExtent;
    AClientExtentSize := cxSize(cxMarginsWidth(AClientExtent),
      cxMarginsHeight(AClientExtent));
    ANewVisualAreaSize := cxSize(NewWidth - AClientExtentSize.cx,
      NewHeight - AClientExtentSize.cy);
    AMaxVisualAreaSize := GetMaxVisualAreaSize;
    if not Edit.ILookupData.CanResizeVisualArea(ANewVisualAreaSize,
      AMaxVisualAreaSize.cy, AMaxVisualAreaSize.cx) then
    begin
      NewWidth := Width;
      NewHeight := Height;
    end
    ...
    begin
      NewWidth := Max(ANewVisualAreaSize.cx + AClientExtentSize.cx,
        AMinSize.cx);
      NewHeight := Max(ANewVisualAreaSize.cy + AClientExtentSize.cy,
        AMinSize.cy);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    AEditBounds.BottomRight := Edit.Parent.ClientToScreen
      (AEditBounds.BottomRight);
  end;
  ...
  begin
    Result.cx := Max(ADesktopArea.Right - AEditBounds.Right,
      AEditBounds.Left - ADesktopArea.Left + AShadowWidth) + 1;
    Result.cy := cxRectHeight(ADesktopArea);
  end
  else
  begin
    Result.cx := cxRectWidth(ADesktopArea);
    Result.cy := Max(ADesktopArea.Bottom - AEditBounds.Bottom,
      AEditBounds.Top - ADesktopArea.Top + AShadowWidth);
  end;
...
procedure TcxCustomEditPopupWindow.ResizePopupWindow(ALeft, ATop, AWidth,
  AHeight: Integer);
begin
...
procedure TcxCustomEditPopupWindow.SetBorderStyle
...
function TcxCustomEditPopupWindow.GetHitTest(const P: TPoint): Integer;
  ...
  begin
    case ViewInfo.SizeGripCorner of
      coTopLeft:
        Result := Rect(ViewInfo.Bounds.Left, ViewInfo.Bounds.Top,
          ViewInfo.Bounds.Left + ASize, ViewInfo.Bounds.Top + ASize);
      coTopRight:
        Result := Rect(ViewInfo.Bounds.Right - ASize, ViewInfo.Bounds.Top,
          ViewInfo.Bounds.Right, ViewInfo.Bounds.Top + ASize);
      coBottomLeft:
        Result := Rect(ViewInfo.Bounds.Left, ViewInfo.Bounds.Bottom - ASize,
          ViewInfo.Bounds.Left + ASize, ViewInfo.Bounds.Bottom);
      coBottomRight:
        Result := Rect(ViewInfo.Bounds.Right - ASize, ViewInfo.Bounds.Bottom -
          ASize, ViewInfo.Bounds.Right, ViewInfo.Bounds.Bottom);
    end;
  end;
...
begin
  ...
  else if ViewInfo.Sizeable then
  begin
    if cxRectPtIn(ViewInfo.SizeGripSizingRect, P) then
      Result := GetSizeCornerHitTest
    else if not SysPanelStyle and cxRectPtIn(ViewInfo.Bounds, P) and
      not cxRectPtIn(ViewInfo.ClientRect, P) then
    begin
      if cxRectPtIn(GetEmbeddedSizingRect, P) then
        Result := GetSizeCornerHitTest
      else if (P.Y < ViewInfo.ClientRect.Top) and
        (ViewInfo.SizeGripCorner in [coTopLeft, coTopRight]) or
        (P.Y >= ViewInfo.ClientRect.Bottom) and
        (ViewInfo.SizeGripCorner in [coBottomLeft, coBottomRight]) then
        Result := htSizeNCNS
      else if (P.X < ViewInfo.ClientRect.Left) and
        (ViewInfo.SizeGripCorner in [coTopLeft, coBottomLeft]) or
        (P.X >= ViewInfo.ClientRect.Right) and
        (ViewInfo.SizeGripCorner in [coBottomRight, coTopRight]) then
        Result := htSizeNCWE
    end;
  end;
end;
...
procedure TcxCustomEditPopupWindow.UpdateSysPanelState(const P: TPoint;
  AShift: TShiftState);
const
  ASizeCursorMap: array [htNone .. htSizeNCWE] of TCursor = (crDefault,
    crDefault, crSizeNWSE, crSizeNESW, crSizeNS, crSizeWE);
var
  AHitTest: Integer;
begin
  AHitTest := GetHitTest(P);
...
begin
  ...
  if FSizingHitTest in [htSizeNCWE, htSizeGripNESW, htSizeGripNWSE] then
    ANewWidth := cxRectWidth(BoundsRect) - DX * ASignMap
      [ViewInfo.SizeGripCorner in [coTopLeft, coBottomLeft]]
  else
    ANewWidth := Width;
  if FSizingHitTest in [htSizeNCNS, htSizeGripNESW, htSizeGripNWSE] then
    ANewHeight := cxRectHeight(BoundsRect) - DY * ASignMap
      [ViewInfo.SizeGripCorner in [coTopLeft, coTopRight]]
  else
    ANewHeight := Height;
    ...
    begin
      ...
      begin
        if ShowContentWhileResize then
          ResizePopupWindow(ABoundsRect.Left, ABoundsRect.Top, ANewWidth,
            ANewHeight)
        else
...
begin
  ...
  if not ShowContentWhileResize and AApply then
    ResizePopupWindow(FSizeFrameBounds.Left, FSizeFrameBounds.Top,
      cxRectWidth(FSizeFrameBounds), cxRectHeight(FSizeFrameBounds));
end;
...
procedure TcxCustomDropDownEditViewData.Calculate(ACanvas: TcxCanvas;
...
begin
  KeepArrowButtonPressedWhenDroppedDown :=
    Properties.KeepArrowButtonPressedWhenDroppedDown or NativeStyle and
    IsWinVistaOrLater;
  inherited Calculate(ACanvas, ABounds, P, Button, Shift, AViewInfo,
    AIsMouseEvent);
  with TcxCustomTextEditViewInfo(AViewInfo) do
  begin
    DrawSelectionBar := not IsInplace and
      ((Edit = nil) or not((Properties.EditingStyle = esFixedList) and
      Properties.FixedListSelection and TcxCustomDropDownEdit(Edit)
      .FindSelection));
    if DrawSelectionBar then
      DrawSelectionBar := (Properties.EditingStyle = esFixedList) and
        Focused and not Transparent and not HasPopupWindow and
        not(NativeStyle and IsWinVistaOrLater);
  end;
end;

function TcxCustomDropDownEditViewData.CanPressButton
  (AViewInfo: TcxCustomEditViewInfo; AButtonVisibleIndex: Integer): Boolean;
begin
  Result := not(IsHotAndPopup and
    (AButtonVisibleIndex = Properties.DropDownButtonVisibleIndex));
end;

function TcxCustomDropDownEditViewData.GetEditNativeState
  (AViewInfo: TcxCustomEditViewInfo): Integer;
begin
  Result := inherited GetEditNativeState(AViewInfo);
  if IsWinVistaOrLater and (Properties.EditingStyle = esFixedList)
  then { totButton }
  begin
    if not Enabled then
      Result := PBS_DISABLED
...
function TcxCustomDropDownEditViewData.GetProperties
  : TcxCustomDropDownEditProperties;
begin
...
class function TcxCustomDropDownEditProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxCustomDropDownEdit;
end;

function TcxCustomDropDownEditProperties.GetSupportedOperations
...
class function TcxCustomDropDownEditProperties.GetViewDataClass
...
procedure TcxCustomDropDownEditProperties.DoAssign
...
begin
  ...
  if AProperties is TcxCustomDropDownEditProperties then
    ...
    begin
      ...
      Self.KeepArrowButtonPressedWhenDroppedDown :=
        KeepArrowButtonPressedWhenDroppedDown;
      ...
      Self.PopupMinHeight := Self.ScaleFactor.Apply(PopupMinHeight,
        ScaleFactor);
...
class function TcxCustomDropDownEditProperties.GetPopupWindowClass
...
const
  AAlignHorzMap: array [TcxPopupAlignHorz] of TAlignment = (taLeftJustify,
...
procedure TcxCustomDropDownEditProperties.
  SetKeepArrowButtonPressedWhenDroppedDown(Value: Boolean);
begin
...
const
  AAlignHorzMap: array [TAlignment] of TcxPopupAlignHorz = (pahLeft, pahRight,
    pahCenter);
begin
...
begin
  if Value < 0 then
    Value := 0
  else if (Value > 0) and (Value < FPopupMinWidth) then
...
function TcxEditPopupControlLookAndFeel.InternalGetScrollbarMode
  : TdxScrollbarMode;
begin
...
begin
  inherited;
  if (GetKeyState(VK_ESCAPE) < 0) and not Container.HasPopupWindow and
    (Container.Properties.EditingStyle = esFixedList) then
...
procedure TcxCustomDropDownEdit.ActivateByKey(Key: Char;
...
begin
  ...
  if IsTextChar(Key) and ActiveProperties.ImmediateDropDownWhenKeyPressed and
    not ActiveProperties.ImmediateDropDownWhenActivated and
    not SendActivationKey(Key) then
    DroppedDown := True;
  if not SendActivationKey(Key) and CanDropDown and
    (GetPopupFocusedControl <> nil) then
    PostMessage(PopupWindow.Handle, DXM_POPUPCONTROLKEY, WPARAM(Key), 0);
end;

procedure TcxCustomDropDownEdit.ActivateByMouse(Shift: TShiftState;
  X, Y: Integer; var AEditData: TcxCustomEditData);
begin
...
class function TcxCustomDropDownEdit.GetPropertiesClass
...
procedure TcxCustomDropDownEdit.DoEditKeyDown(var Key: Word;
  Shift: TShiftState);
var
  AKey: Word;
begin
  ...
  else if ((AKey = VK_RETURN) or (AKey = VK_ESCAPE)) and not(ssAlt in Shift) and HasPopupWindow
  then
  begin
...
function TcxCustomDropDownEdit.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := inherited DoMouseWheel(Shift, WheelDelta, MousePos);
  if not Result then
  begin
    ...
    begin
      ...
      if ILookupData.ActiveControl <> nil then
        TControlAccess(ILookupData.ActiveControl).DoMouseWheel(Shift,
          WheelDelta, MousePos);
    end;
  end;
end;
...
begin
  ...
  ANeedUpdatePopup := not ActiveProperties.IsLookupEdit and
    ActiveProperties.IncrementalFiltering and
    (Result or (AFindText = '') or (ActiveProperties.EditingStyle = esEdit) and
    ActiveProperties.IncrementalSearch);
...
function TcxCustomDropDownEdit.GetScrollLookupDataList
  (AScrollCause: TcxEditScrollCause): Boolean;
begin
  Result := not PropertiesChangeLocked and
    (not IsInplace or (AScrollCause = escMouseWheel) or
    not InplaceParams.MultiRowParent or HasPopupWindow);
end;
...
begin
  ...
  TcxCustomDropDownEditData(EditData).Width := AProperties.ScaleFactor.Revert
    (AProperties.PopupWidth);
  TcxCustomDropDownEditData(EditData).Height := AProperties.ScaleFactor.Revert
    (AProperties.PopupHeight);
end;

function TcxCustomDropDownEdit.InternalGetNotPublishedStyleValues
  : TcxEditStyleValues;
begin
...
function TcxCustomDropDownEdit.IsEditorKey(Key: Word;
  Shift: TShiftState): Boolean;
begin
  Result := inherited IsEditorKey(Key, Shift) or IsInplace and
    HasPopupWindow and ((Shift * [ssAlt, ssCtrl] <> []) or IsSysKey(Key));
end;
...
begin
  Result := (PopupWindow <> nil) and PopupWindow.HandleAllocated and
    PeekMessage(AMsg, PopupWindow.Handle, DXM_SHOWPOPUPWINDOW,
    DXM_SHOWPOPUPWINDOW, PM_NOREMOVE);
end;
  ...
  begin
    ...
    try
      ...
      if not InternalCompareString(APrevText, InnerTextEdit.EditValue, True) and
        ActiveProperties.ImmediateDropDownWhenKeyPressed and not HasPopupWindow
      then
      begin
        DroppedDown := True;
...
procedure TcxCustomDropDownEdit.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited MouseDown(Button, Shift, X, Y);
  if not FIsActivatingByMouse and ActiveProperties.DropDownOnClick and
    (Button = mbLeft) and PtInRect(ViewInfo.ClientRect, Point(X, Y)) and
    not HasPopupWindow and not FIsHotAndPopup and IsFocused then
    DroppedDown := True;
end;

function TcxCustomDropDownEdit.DoRefreshContainer(const P: TPoint;
  ...
  if HasPopupWindow then
    with ViewInfo do
      if IsButtonReallyPressed and (PressedButton = -1) then
        CloseUp(crUnknown);
  if AIsMouseEvent and not((Button = cxmbLeft) and
    (cxButtonToShift(Button) * Shift <> [])) then
    SetIsHotAndPopup;
...
begin
  Result := not(ActiveProperties.ImmediateDropDownWhenKeyPressed and
    IsPopupWindowCaptureFocus and (TranslateKey(Word(Key)) <> VK_RETURN));
end;

function TcxCustomDropDownEdit.TabsNeeded: Boolean;
begin
  Result := inherited TabsNeeded or HasPopupWindow and
    ActiveProperties.PostPopupValueOnTab;
end;
...
begin
  Result := (FLookupData <> nil) and not ActiveProperties.IsDesigning and
    ActiveProperties.IsLookupDataVisual;
end;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          LockClick(False);
          if ModifiedAfterEnter and not VarEqualsExact(APrevLookupKey,
            ILookupData.CurrentKey) then
            DoClick;
        end;
      end;
      if not ActiveProperties.AlwaysPostEditValue and
        ActiveProperties.ImmediatePost and CanPostEditValue then
...
begin
  if (PopupWindow <> nil) and PopupWindow.HandleAllocated and
    dxMessagesController.KillMessages(PopupWindow.Handle, DXM_SHOWPOPUPWINDOW)
  then
...
begin
  ...
  begin
    ...
    begin
      ...
      PopupWindow.AlphaBlendValue := TCustomFormAccess(AParentForm)
        .AlphaBlendValue;
      PopupWindow.TransparentColor := TCustomFormAccess(AParentForm)
        .TransparentColor;
      PopupWindow.TransparentColorValue := TCustomFormAccess(AParentForm)
  ...
  if AProperties.PopupSizeable then
    ...
    begin
      ...
      begin
        ...
        TcxCustomDropDownEditData(EditData).Height :=
          PopupWindow.ScaleFactor.Revert(PopupWindow.Height);
        TcxCustomDropDownEditData(EditData).Width :=
          PopupWindow.ScaleFactor.Revert(PopupWindow.Width);
      end
    end
    else
    begin
      AProperties.PopupWidth := AProperties.ScaleFactor.Apply(PopupWindow.Width,
        PopupWindow.ScaleFactor);
      AProperties.PopupHeight := AProperties.ScaleFactor.Apply
        (PopupWindow.Height, PopupWindow.ScaleFactor);
    end;
end;
...
begin
  if IsInplace and ActiveProperties.PopupSizeable then
    ...
    Result := ScaleFactor.Apply(ActiveProperties.PopupHeight,
      ActiveProperties.ScaleFactor);
end;
...
begin
  ...
  if (ILookupData <> nil) and ActiveProperties.IsLookupDataVisual and IsPopupWindowCaptureFocus
  then
  begin
...
begin
  if IsInplace and ActiveProperties.PopupSizeable then
    ...
    Result := ScaleFactor.Apply(ActiveProperties.PopupWidth,
      ActiveProperties.ScaleFactor);
end;
...
begin
  ...
  if UseRightToLeftAlignment then
    PopupWindow.AlignHorz := TdxRightToLeftLayoutConverter.ConvertPopupAlignHorz
      (AActiveProperties.PopupHorzAlignment)
  else
  ...
  PopupWindow.MinHeight := ScaleFactor.Apply(AActiveProperties.PopupMinHeight,
    AActiveProperties.ScaleFactor);
  PopupWindow.MinWidth := ScaleFactor.Apply(AActiveProperties.PopupMinWidth,
    AActiveProperties.ScaleFactor);
  ...
  PopupWindow.CloseButton := TcxCustomEditStyleAccess(ActiveStyle)
    .PopupCloseButton;
...
begin
  Result := not ActiveProperties.IsLookupEdit and
    ActiveProperties.IncrementalFiltering and ActiveProperties.UseLookupData and
    ILookupData.IsEmpty;
end;
...
begin
  ...
  LockChangeEvents(True);
  try
    if ActiveProperties.AlwaysPostEditValue and
      ActiveProperties.ImmediatePost and CanPostEditValue and InternalValidateEdit
    then
  ...
  ShortRefreshContainer(False);
  if not HandleAllocated or (ViewInfo.IsButtonReallyPressed and
    (GetMouseKeys = 0)) then
    SetIsHotAndPopup;
  // perform if VK_ESC was pressed and WM_MOUSEMOVE didn't occur
end;

procedure TcxCustomDropDownEdit.PopupWindowCloseQuery(Sender: TObject;
...
begin
  ...
  UpdateDrawValue;
  with ActiveProperties do
    if UseLookupData and FindSelection and ImmediateDropDownWhenKeyPressed and PopupWindowCapturesFocus
    then
      ILookupData.DroppedDown(Copy(DisplayText, 1, SelStart));
end;

procedure TcxCustomDropDownEdit.PositionPopupWindowChildren
...
function TcxCustomDropDownEdit.GetActiveProperties
  : TcxCustomDropDownEditProperties;
begin
...
begin
  if DroppedDown <> Value then
    if not Value then
      CloseUp(crUnknown)
    else if not IsPopupWindowShowing then
      DropDown;
end;

procedure TcxCustomDropDownEdit.SetProperties
  (Value: TcxCustomDropDownEditProperties);
begin
...
begin
  ...
  if Sizeable and ASizeChanged or not PopupAutoSize and (PopupWidth > 0) then
    APreferredWidth := PopupWidth
  else if PopupAutoSize then
  begin
    APreferredWidth := APopupWindowVisualAreaSize.cx +
      cxMarginsWidth(AClientExtent);
    APreferredWidth := Max(APreferredWidth, cxRectWidth(OwnerBounds));
  end
  else
    APreferredWidth := cxRectWidth(OwnerBounds);

  if Sizeable and ASizeChanged or (Sizeable or not PopupAutoSize) and
    (PopupHeight > 0) then
    APreferredHeight := PopupHeight
  else
    APreferredHeight := APopupWindowVisualAreaSize.cy +
      cxMarginsHeight(AClientExtent);
...
function TcxCustomComboBoxListBox.GetHeight(ARowCount: Integer;
...
begin
  if Edit.ActiveProperties.UseSearchControl and
    (Edit.ILookupData.ActiveControl <> nil) and
...
function TcxCustomComboBoxListBox.DoDrawItem(AIndex: Integer;
  const ARect: TRect; AState: TOwnerDrawState): Boolean;
begin
  Result := Edit.IsOnDrawItemEventAssigned;
  if Result then
    Edit.DoOnDrawItem(Canvas,
      Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(AIndex),
      ARect, AState);
end;
...
procedure TcxCustomComboBoxListBox.MeasureItem(Index: Integer;
  var Height: Integer);
begin
...
begin
  if Edit.ActiveProperties.ItemHeight > 0 then
    Result := ScaleFactor.Apply(Edit.ActiveProperties.ItemHeight,
      Edit.ActiveProperties.ScaleFactor)
  else
...
procedure TcxComboBoxIncrementalFilteringHelper.CheckSearchControl
  (AParent: TWinControl);
begin
...
function TcxComboBoxIncrementalFilteringHelper.GetLookAndFeelPainter
...
begin
  if FLookupData.Edit.IsLookupNavigationKey(Key, Shift) and
    not(Key in [VK_HOME, VK_END]) or (Key = VK_RETURN) or (Key = VK_ESCAPE) and
    (GetSearchText = '') then
...
procedure TcxComboBoxIncrementalFilteringHelper.SearchEditMouseWheel
  ...
  var Handled: Boolean);
begin
  Handled := TControlAccess(FLookupData.List).DoMouseWheel(Shift, WheelDelta,
    MousePos);
end;
...
begin
  if not FLookupData.IsInitializing and
    (FLookupData.Edit.HasPopupWindow or FLookupData.Edit.IsPopupWindowShowing)
  then
  begin
...
begin
  Result := True;
  if (NewSize.cx = List.Width) and
    (NewSize.cy = (List.Height + GetSearchEditHeight)) then
    Exit;
  ...
  AScrollBarSize := cxSize(TcxContainerAccess(ListContainer)
    .GetVScrollBarDefaultAreaWidth, TcxContainerAccess(ListContainer)
  ...
  begin
    ...
    if (AListClientSize.cx < List.ScrollWidth) or
      (AListClientSize.cx > AMaxWidth) then
    ...
    if (AListSize.cy > NewSize.cy) or (AListSize.cy > AMaxHeight) then
      ...
      else if NewSize.cy < List.Height then
        Break;
  end;
...
begin
  ...
  if (GetFilteredItemCount > AProperties.DropDownRows) or (AMaxHeight > 0) and
    (Result.cy > AMaxHeight) then
    Result.cx := Result.cx + TcxContainerAccess(ListContainer)
      .GetVScrollBarDefaultAreaWidth;
  if (AWidth > 0) and (Result.cx > AWidth) then
    Result.cy := Result.cy + TcxContainerAccess(ListContainer)
      .GetHScrollBarDefaultAreaHeight;
...
begin
  ...
  begin
    if SearchText = '' then
      FIncrementalFilteringHelper.SearchEdit.Top := R.Top -
        FIncrementalFilteringHelper.SearchEdit.Height - 2
    else
    begin
      ...
      R := AClientRect;
      R.Top := FIncrementalFilteringHelper.SearchEdit.BoundsRect.Bottom +
        ASearchEditOffsets.Bottom;
    end;
  end;
...
begin
  ANeedCheckUpdatePopup := IsIncrementalFiltering and not IsInitializing and
    (ActiveProperties.EditingStyle = esEdit) and
    Edit.CanSynchronizeLookupData and
    (Edit.HasPopupWindow or Edit.IsPopupWindowShowing);
...
function TcxComboBoxLookupData.CreateIncrementalFilteringHelper
  : TcxComboBoxIncrementalFilteringHelper;
begin
  Result := TcxComboBoxIncrementalFilteringHelper.Create(Self);
end;

procedure TcxComboBoxLookupData.DoInitialize(AVisualControlsParent
  : TWinControl);
begin
  inherited;
  FIncrementalFilteringHelper.CheckSearchControl(AVisualControlsParent);
  ListContainer.Styles[csNormal].LookAndFeel.MasterLookAndFeel :=
...
begin
  if (FIncrementalFilteringHelper.SearchEdit <> nil) and (SearchText <> '') then
    Result := FIncrementalFilteringHelper.SearchEdit.Height +
      cxMarginsHeight(FIncrementalFilteringHelper.GetSearchEditOffsets)
  else
    Result := 0;
end;
...
procedure TcxCustomComboBoxViewData.EditValueToDrawValue(const AEditValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
begin
  inherited EditValueToDrawValue(AEditValue, AViewInfo);
  with TcxCustomComboBoxViewInfo(AViewInfo) do
  begin
    if (Properties.DropDownListStyle = lsFixedList) and
      Assigned(Properties.OnDrawItem) then
...
begin
  ...
  else if Properties.EditingStyle = esNoEdit then
  begin
    Inc(Result.Top);
    Dec(Result.Bottom);
...
procedure TcxCustomComboBoxViewInfo.DoCustomDraw(ACanvas: TcxCanvas;
  ARect: TRect);
var
...
class function TcxCustomComboBoxProperties.GetViewInfoClass
...
procedure TcxCustomComboBoxProperties.DoAssign(AProperties
...
class function TcxCustomComboBoxProperties.GetLookupDataClass
  : TcxInterfacedPersistentClass;
begin
  Result := TcxComboBoxLookupData;
end;

class function TcxCustomComboBoxProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
  Result := TcxComboBoxPopupWindow;
end;

class function TcxCustomComboBoxProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
...
procedure TcxCustomComboBoxProperties.SetDropDownListStyle
...
procedure TcxCustomComboBox.Activate(var AEditData: TcxCustomEditData;
  ANeedSetFocus: Boolean);
begin
  inherited;
  SynchronizeItemIndex;
end;

class function TcxCustomComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
begin
  ...
  try
    ...
    LockChangeEvents(False);
    if (* ModifiedAfterEnter and *) not VarEqualsExact(APrevLookupKey,
      ILookupData.CurrentKey) and ActiveProperties.IncrementalSearch then
      DoClick;
  end;
end;
    ...
    if not HasPopupWindow and ActiveProperties.Revertable then
      ...
      begin
        ...
        if not VarEqualsExact(APrevCurrentKey, ILookupData.CurrentKey) and
          CanPostEditValue and ActiveProperties.ImmediatePost and InternalValidateEdit
        then
          InternalPostEditValue;
      end;
  finally
...
begin
  ...
  Result := ILookupData.GetVisualAreaPreferredSize
    (AMaxVisualAreaSize.cy, AWidth);
end;
  ...
  if (GetCaptureControl = Self) and (AActiveControl is TWinControl) then
    ...
    begin
      ...
      if HasPopupWindow and PtInRect(PopupWindow.ViewInfo.ClientRect,
        ...
        SendMouseEvent(AActiveControl as TWinControl, WM_LBUTTONDOWN, [],
          ScreenToClient(P));
    end;
end;
...
begin
  Result := not FLookupDataTextChangedLocked and not EditModeSetting and
    (HasPopupWindow or not ActiveProperties.CanIncrementalSearch and
    IsPopupWindowShowing or ((ActiveProperties.EditingStyle in [esEditList,
    esFixedList, esNoEdit]) or Focused and not InternalCompareString(Text,
    ILookupData.GetDisplayText(ILookupData.CurrentKey), False)));
end;
...
class function TcxFilterComboBoxHelper.GetFilterDataType(AValueTypeClass
...
procedure TcxPopupEditPopupWindowViewInfo.DrawBorder(ACanvas: TcxCanvas;
  var R: TRect);
var
...
class function TcxCustomPopupEditProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxPopupEdit;
end;

procedure TcxCustomPopupEditProperties.DoAssign(AProperties
  : TcxCustomEditProperties);
begin
  inherited;
...
class function TcxCustomPopupEditProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
...
class function TcxCustomPopupEdit.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
begin
  if ControlHasAsParent(PopupControl) then
    raise EcxEditError.Create
      (cxGetResourceString(@cxSEditPopupCircularReferencingError));
...
procedure TcxCustomPopupEdit.PositionPopupWindowChildren
  (const AClientRect: TRect);
begin
...
procedure TcxCustomPopupEdit.HidePopup(Sender: TcxControl;
  AReason: TcxEditCloseUpReason);
begin
  FCloseUpReason := AReason;
...
begin
  ...
  begin
    while (FPrevPopupControlData.ActiveControl <> nil) and
      (FPrevPopupControlData.ActiveControl <> PopupWindow) do
    begin
      FPrevPopupControlData.ActiveControl.Perform(CM_EXIT, 0, 0);
      FPrevPopupControlData.ActiveControl :=
        FPrevPopupControlData.ActiveControl.Parent;
    end;
    ...
    PopupControl.BoundsRect := dxGetScaleFactor(PopupControl)
      .Apply(FPrevPopupControlData.Bounds);
...
begin
  ...
  FPrevPopupControlData.Bounds := dxGetScaleFactor(PopupControl)
    .Revert(PopupControl.BoundsRect);
...
GetRegisteredEditProperties.Register(TcxComboBoxProperties,
  scxSEditRepositoryComboBoxItem);
GetRegisteredEditProperties.Register(TcxPopupEditProperties,
  scxSEditRepositoryPopupItem);
FilterEditsController.Register(TcxComboBoxProperties, TcxFilterComboBoxHelper);

finalization

FilterEditsController.Unregister(TcxComboBoxProperties,
  TcxFilterComboBoxHelper);
