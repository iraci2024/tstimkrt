...
type
  ...
  TcxRichEditColor = (recBlack, recBlue, recCyan, recGreen, recMagenta, recRed,
    recYellow, recWhite, recDarkBlue, recDarkCyan, recDarkGreen, recDarkMagenta,
    recDarkRed, recDarkYellow, recDarkGray, recLightGray);

  { TcxParaAttributes2 }

  TcxParaFormat2Alignment = (pfaLeft, pfaRight, pfaCenter, pfaJustify);
  TcxParaFormat2NumberingStyle = (pfnsRightParenthesis, pfnsParenthesis = $100,
    pfnsPeriod = $200, pfnsNumberOnly = $300, pfnsContinuesList = $400,
    pfnsStartNew = $8000);
  TcxParaFormat2NumberingType = (pfnNone, pfnSymbols, pfnNumber, pfnLCLetter,
    pfnUCLetter, pfnLCRoman, pfnUCRoman, pfnSequence);
  TcxParaFormat2LineSpacingRule = (pfsrSingle, pfsrOneAndHalf, pfsrDouble,
    pfsrRule3, pfsrRule4, pfsrRule5);
  TcxParaFormat2ShadingStyle = (pfsNone, pfsDarkHorizontal, pfsDarkVertical,
    pfsDarkDownDiagonal, pfsDarkUpDiagonal, pfsDarkGrid, pfsDarkTrellis,
    pfsLightHorizontal, pfsLightVertical, pfsLightDownDiagonal,
    pfsLightUpDiagonal, pfsLightGrid, pfsLightTrellis);
  TcxParaFormat2BorderLocation = (pfblLeft, pfblRight, pfblTop, pfblBottom,
    pfblInside, pfblOutside, pfblAutocolor);
  TcxParaFormat2BorderLocations = set of TcxParaFormat2BorderLocation;
  TcxParaFormat2BorderStyle = (pfbsNone, pfbs3Per4, pfbs11Per2, pfbs21Per4,
    pfbs3, pfbs41Per2, pfbs6, pfbs3Per4Double, pfbs11Per2Double,
    pfbs21Per4Double, pfbs3Per4Gray, pfbs3Per4GrayDashed);
  TcxParaFormat2TableStyle = (pftsNone, pftsCell, pftsRowDelimiter);
  TcxParaFormat2TabAlignment = (pftaLeft, pftaCenter, pftaRight, pftaDecimal,
    pftaWordBar);
  TcxParaFormat2TabLeader = (pftlNone, pftlDotted, pftlDashed, pftlUnderlined,
    pftlThick, pftlDouble);
  ...
  private
    ...
    procedure SetTabAlignment(Index: Integer;
      const Value: TcxParaFormat2TabAlignment);
    procedure SetTabCount(Value: Integer);
    procedure SetTabLeader(Index: Integer;
      const Value: TcxParaFormat2TabLeader);
    procedure SetTabValues(Index, AWidth: Integer;
      AAlignment: TcxParaFormat2TabAlignment; ALeader: TcxParaFormat2TabLeader);
  ...
  public
    ...
    property Alignment: TcxParaFormat2Alignment read GetAlignment
      write SetAlignment;

    property NumberingStart: Word read GetNumberingStart
      write SetNumberingStart;
    property NumberingStyle: TcxParaFormat2NumberingStyle read GetNumberingStyle
      write SetNumberingStyle;
    property NumberingTab: Word read GetNumberingTab write SetNumberingTab;
    property NumberingType: TcxParaFormat2NumberingType read GetNumberingType
    ...
    property TabAlignment[Index: Integer]: TcxParaFormat2TabAlignment
      read GetTabAlignment write SetTabAlignment;
    property TabCount: Integer read GetTabCount write SetTabCount;
    property TabLeader[Index: Integer]: TcxParaFormat2TabLeader
      read GetTabLeader write SetTabLeader;

    property LineSpacing: Longint index 2 read GetYSpace write SetYSpace;
    property LineSpacingRule: TcxParaFormat2LineSpacingRule
  ...
  TcxCharFormat2UnderlineType = (cfutNone, cfutSolid, cfutDotted, cfutDouble,
    cfutWordOnly);
  ...
  TcxCharFormat2FontStyleEx = (cffsAllCaps, cffsDisabled, cffsEmboss,
    cffsHidden, cffsImprint, cffsOutline, cffsRevised, cffsShadow,
    cffsSmallCaps, cffsSubscript, cffsSuperscript);
  ...
  public
    constructor Create(ARichEdit: TcxCustomEdit; AType: TAttributeType);
      reintroduce; virtual;
    procedure Assign(Source: TPersistent); override;
    ...
    property StyleEx: TcxCharFormat2FontStylesEx read GetStyleEx
      write SetStyleEx;
    property UnderlineType: TcxCharFormat2UnderlineType read GetUnderlineType
  ...
  TcxRichEditTableRowParams = record
    ...
    nParams: DWORD;
    // 0 - 2 bits - Row alignment (like PARAFORMAT::bAlignment, 1/2/3
    // \trql, trqr, \trqc)
  ...
  public
    ...
    function AddColumnParams(const AParams: TcxRichEditTableColumnParams)
      : Integer; overload;
    procedure ClearColumnParams;

    procedure InsertTable(ARichEdit: TcxCustomEdit;
...
function cxIntegerToParaFormat2BorderLocations(AValue: Integer)
  : TcxParaFormat2BorderLocations;
function cxParaFormat2BorderLocationsToInteger
  (AValue: TcxParaFormat2BorderLocations): Integer;
function cxRichEditColorToColor(AValue: TcxRichEditColor): TColor;
function cxRichEditGetCharPosition(ARichHandle: THandle; ARichVersion: Integer;
...
const
  cxAutoColorMaskMap: array [0 .. 1] of DWORD = (CFE_AUTOBACKCOLOR,
    CFE_AUTOCOLOR);
...
begin
  Result := [];
  for ALocation := Low(TcxParaFormat2BorderLocation)
    to High(TcxParaFormat2BorderLocation) do
  begin
...
const
  ResultMap: array [TcxRichEditColor] of TColor = (clBlack, clBlue, clAqua,
    clLime, clFuchsia, clRed, clYellow, clWhite, clNavy, clTeal, clGreen,
    clPurple, clMaroon, clOlive, clDkGray, clLtGray);
begin
...
begin
  ...
  begin
    if InnerRich.UseRightToLeftAlignment then
      ...
      else if AParagraph.wAlignment = PFA_RIGHT then
...
begin
  ...
  case Index of
    1:
      Result := TwipsToPixels(AParagraph.dxRightIndent);
    2:
      Result := TwipsToPixels(AParagraph.dxOffset + AParagraph.dxStartIndent);
  else
...
function TcxParaAttributes2.GetTabAlignment(Index: Integer)
  : TcxParaFormat2TabAlignment;
var
  AParagraph: PARAFORMAT2;
begin
  GetAttributes(AParagraph);
  Result := TcxParaFormat2TabAlignment
    ((AParagraph.rgxTabs[Index] shr 24) and $0F);
end;
...
function TcxParaAttributes2.GetTabLeader(Index: Integer)
...
begin
  ...
  case Index of
    0:
      Result := TwipsToPixels(AParagraph.dySpaceBefore);
    1:
      Result := TwipsToPixels(AParagraph.dySpaceAfter);
  else
...
procedure TcxParaAttributes2.SetIndent(const Index: Integer;
  const Value: Longint);
  ...
  begin
    case Index of
      0:
        Result := PFM_STARTINDENT or PFM_OFFSET;
      1:
        Result := PFM_RIGHTINDENT;
      2:
        Result := PFM_OFFSET;
    else
...
begin
  ...
  begin
    ...
    case Index of
      ...
      1:
        dxRightIndent := ANewValue;
      2:
        dxOffset := ANewValue - dxStartIndent;
    else
...
procedure TcxParaAttributes2.SetLineSpacingRule
  (Value: TcxParaFormat2LineSpacingRule);
var
  AParagraph: PARAFORMAT2;
begin
  GetAttributes(AParagraph);
...
procedure TcxParaAttributes2.SetNumberingStyle
  (Value: TcxParaFormat2NumberingStyle);
var
  AParagraph: PARAFORMAT2;
begin
  GetAttributes(AParagraph);
...
procedure TcxParaAttributes2.SetNumberingType
  (Value: TcxParaFormat2NumberingType);
var
  AParagraph: PARAFORMAT2;
begin
  InitParagraph(AParagraph);
...
procedure TcxParaAttributes2.SetTabAlignment(Index: Integer;
  const Value: TcxParaFormat2TabAlignment);
begin
...
procedure TcxParaAttributes2.SetTabLeader(Index: Integer;
  const Value: TcxParaFormat2TabLeader);
begin
  SetTabValues(Index, Tab[Index], TabAlignment[Index], Value);
end;

procedure TcxParaAttributes2.SetTabValues(Index, AWidth: Integer;
  AAlignment: TcxParaFormat2TabAlignment; ALeader: TcxParaFormat2TabLeader);
var
  AParagraph: PARAFORMAT2;
begin
  ...
  with AParagraph do
  begin
    rgxTabs[Index] := dxPixelsToTwips(AWidth) or Ord(AAlignment) shl 24 or
      Ord(ALeader) shr 28;
    dwMask := PFM_TABSTOPS;
...
procedure TcxParaAttributes2.SetYSpace(const Index: Integer;
  ...
  begin
    case Index of
      0:
        Result := PFM_SPACEBEFORE;
      1:
        Result := PFM_SPACEAFTER;
    else
...
begin
  ...
  begin
    ...
    case Index of
      0:
        dySpaceBefore := dxPixelsToTwips(Value);
      1:
        dySpaceAfter := dxPixelsToTwips(Value);
    else
...
constructor TcxTextAttributes2.Create(ARichEdit: TcxCustomEdit;
...
begin
  ...
  begin
    ...
    Protected
      ...
      begin InitFormat(AFormat);
      if InnerRich.HandleAllocated then SendMessage(InnerRich.Handle,
        EM_GETCHARFORMAT, GetFlag, LPARAM(@AFormat));
      end;

      procedure TcxTextAttributes2.InitFormat(var AFormat: CHARFORMAT2);
      begin FillChar(AFormat, SizeOf(CHARFORMAT2), 0);
      AFormat.cbSize := SizeOf(CHARFORMAT2);
      end;

      procedure TcxTextAttributes2.SetAttributes(var AFormat: CHARFORMAT2);
      begin InnerRich.HandleNeeded;
      if InnerRich.HandleAllocated then cxSendStructMessage(InnerRich.Handle,
        EM_SETCHARFORMAT, GetFlag, AFormat);
      end;
    ...
    begin
      ...
      begin
        AMask := GetMask;
        if (AMask and CFM_BOLD) <> 0 then
          Include(Result, caBold);
        if (AMask and CFM_COLOR) <> 0 then
          Include(Result, caColor);
        if (AMask and CFM_FACE) <> 0 then
          Include(Result, caFace);
        if (AMask and CFM_ITALIC) <> 0 then
          Include(Result, caItalic);
        if (AMask and CFM_SIZE) <> 0 then
          Include(Result, caSize);
        if (AMask and CFM_STRIKEOUT) <> 0 then
          Include(Result, caStrikeOut);
        if (AMask and CFM_UNDERLINE) <> 0 then
          Include(Result, caUnderline);
        if (AMask and CFM_PROTECTED) <> 0 then
    ...
    begin
      Result := [];
      AEffects := GetEffects;
      if (AEffects and CFE_BOLD) <> 0 then
        Include(Result, fsBold);
      if (AEffects and CFE_ITALIC) <> 0 then
        Include(Result, fsItalic);
      if (AEffects and CFE_UNDERLINE) <> 0 then
        Include(Result, fsUnderline);
      if (AEffects and CFE_STRIKEOUT) <> 0 then
    ...
    begin
      Result := [];
      AEffects := GetEffects;
      if (AEffects and CFE_ALLCAPS) <> 0 then
        Include(Result, cffsAllCaps);
      if (AEffects and CFE_DISABLED) <> 0 then
        Include(Result, cffsDisabled);
      if (AEffects and CFE_EMBOSS) <> 0 then
        Include(Result, cffsEmboss);
      if (AEffects and CFE_HIDDEN) <> 0 then
        Include(Result, cffsHidden);
      if (AEffects and CFE_IMPRINT) <> 0 then
        Include(Result, cffsImprint);
      if (AEffects and CFE_OUTLINE) <> 0 then
        Include(Result, cffsOutline);
      if (AEffects and CFE_REVISED) <> 0 then
        Include(Result, cffsRevised);
      if (AEffects and CFE_SHADOW) <> 0 then
        Include(Result, cffsShadow);
      if (AEffects and CFE_SMALLCAPS) <> 0 then
        Include(Result, cffsSmallCaps);
      if (AEffects and CFE_SUBSCRIPT) <> 0 then
        Include(Result, cffsSubscript);
      if (AEffects and CFE_SUPERSCRIPT) <> 0 then
        Include(Result, cffsSuperscript);
    end;
    ...
    const
      APitchMap: array [TFontPitch] of Byte = (DEFAULT_PITCH, VARIABLE_PITCH,
        FIXED_PITCH);
    var
      AFormat: CHARFORMAT2;
    begin
    ...
    procedure TcxTextAttributes2.SetStyle(const Value: TFontStyles);
      ...
      begin
        ...
        if fsBold in Value then
          Result := Result or CFE_BOLD;
        if fsItalic in Value then
          Result := Result or CFE_ITALIC;
        if fsUnderline in Value then
          Result := Result or CFE_UNDERLINE;
        if fsStrikeOut in Value then
          Result := Result or CFE_STRIKEOUT;
      end;
    ...
    procedure TcxTextAttributes2.SetStyleEx(const Value
      : TcxCharFormat2FontStylesEx);

      function InternalGetEffects: DWORD;
      begin
        Result := 0;
        if cffsAllCaps in Value then
          Result := Result or CFE_ALLCAPS;
        if cffsDisabled in Value then
          Result := Result or CFE_DISABLED;
        if cffsEmboss in Value then
          Result := Result or CFE_EMBOSS;
        if cffsHidden in Value then
          Result := Result or CFE_HIDDEN;
        if cffsImprint in Value then
          Result := Result or CFE_IMPRINT;
        if cffsOutline in Value then
          Result := Result or CFE_OUTLINE;
        if cffsRevised in Value then
          Result := Result or CFE_REVISED;
        if cffsShadow in Value then
          Result := Result or CFE_SHADOW;
        if cffsSmallCaps in Value then
          Result := Result or CFE_SMALLCAPS;
        if cffsSubscript in Value then
          Result := Result or CFE_SUBSCRIPT;
        if cffsSuperscript in Value then
          Result := Result or CFE_SUPERSCRIPT;
      end;

    var
      AMask: DWORD;
    begin
      AMask := CFM_ALLCAPS or CFM_DISABLED or CFM_EMBOSS or CFM_HIDDEN or
        CFM_IMPRINT or CFM_OUTLINE or CFM_REVISED or CFM_SHADOW or
        CFM_SMALLCAPS or CFM_SUBSCRIPT or CFM_SUPERSCRIPT;
      SetEffects(AMask, InternalGetEffects);
    end;

    procedure TcxTextAttributes2.SetUnderlineType
      (const Value: TcxCharFormat2UnderlineType);
    const
      AUnderlineTypeMap: array [TcxCharFormat2UnderlineType]
        of Byte = (CFU_UNDERLINENONE, CFU_UNDERLINE, CFU_UNDERLINEDOTTED,
        CFU_UNDERLINEDOUBLE, CFU_UNDERLINEWORD);
    var
      AFormat: CHARFORMAT2;
    begin
      InitFormat(AFormat);
    ...
    const
      AWeightMap: array [TcxCharFormat2Weight] of Word = (FW_DONTCARE, FW_THIN,
        FW_ULTRALIGHT, FW_LIGHT, FW_NORMAL, FW_MEDIUM, FW_SEMIBOLD, FW_BOLD,
    ...
    function TcxRichEditTableParams.AddColumnParams(const AParams
      : TcxRichEditTableColumnParams): Integer;
    begin
    ...
    procedure TcxRichEditTableParams.InsertTable(ARichEdit: TcxCustomEdit;
      AColumnCount, ARowCount: Integer);

      procedure InitializeCellParams(var ACell: TcxRichEditTableCellParams;
        AParams: TcxRichEditTableColumnParams);
      const
      ...
      begin
        ...
        ACell.crBrdrBottom := cxRichEditColorToColor
          (AParams.BorderColor.Bottom);
    ...
    begin
      ...
      try
        ...
        PcxRichEditTableRowParams(ARows).cbCellSize :=
          SizeOf(TcxRichEditTableCellParams);
        PcxRichEditTableRowParams(ARows).cCell := AColumnCount;
        PcxRichEditTableRowParams(ARows).cRow := ARowCount;
        PcxRichEditTableRowParams(ARows).dxCellMargin :=
          dxPixelsToTwips(CellMargins);
        ...
        if AFirstColumnParamsForAllCells then
          PcxRichEditTableRowParams(ARows).nParams :=
            PcxRichEditTableRowParams(ARows).nParams or $80;
        ...
        try
          ...
          SendMessage(ARichEdit.InnerControl.Handle, EM_INSERTTABLE,
            WParam(ARows), LPARAM(@ACells[0]));
        finally
          SetLength(ACells, 0);
        end;
      finally
    ...
    function TcxRichEditTableParams.GetDefaultColumnParams
      : TcxRichEditTableColumnParams;
    begin
