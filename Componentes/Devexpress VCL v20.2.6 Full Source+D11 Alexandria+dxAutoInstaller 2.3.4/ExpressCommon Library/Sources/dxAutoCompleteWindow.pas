...
type
  ...
  public
    ...
    //
    property CircularKeyboardNavigation: Boolean
      read FCircularKeyboardNavigation write FCircularKeyboardNavigation;
    property HighlightSearchText: Boolean read FHighlightSearchText
      write FHighlightSearchText;
    //
  ...
  public
    ...
    //
    property DisplayRowsCount: Integer read FDisplayRowsCount
      write FDisplayRowsCount;
    property HighlightSearchText: Boolean read GetHighlightSearchText
      write SetHighlightSearchText;
...
begin
  ...
  try
    LookAndFeel.Painter.DrawSizeGrip(cxPaintCanvas, ClientRect, clDefault,
      ACorner[UseRightToLeftScrollBar]);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;

procedure TdxAutoCompleteInnerListBoxSizeGrip.WMNCHitTest
...
procedure TdxCustomAutoCompleteInnerListBox.DrawItemText(const R: TRect;
...
begin
  ...
  begin
    ...
    cxTextOut(Canvas.Canvas, AItem.Caption, ABounds, GetTextFlags, ASelStart,
...
begin
  SetScrollBarInfo(sbVertical, 0, Items.Count - 1, 1, PageSize, TopIndex,
    True, False);
...
procedure TdxCustomAutoCompleteInnerListBox.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TdxCustomAutoCompleteInnerListBox.MouseUp(Button: TMouseButton;
...
function TdxCustomAutoCompleteInnerListBox.PointInSizeGrip
  (const P: TPoint): Boolean;
begin
  Result := ControlAtPos(P, False, True) = SizeGrip;
end;

function TdxCustomAutoCompleteInnerListBox.ProcessNavigationKey(var Key: Word;
...
procedure TdxCustomAutoCompleteInnerListBox.WMNCHitTest
  (var Message: TWMNCHitTest);
begin
...
begin
  InflateRect(Rect, -dxAutoCompleteWindowBorderSize,
    -dxAutoCompleteWindowBorderSize);
end;

procedure TdxCustomAutoCompleteWindow.CalculateConstraints;
begin
  Constraints.MinWidth := 2 * dxAutoCompleteWindowBorderSize +
...
begin
  ...
  begin
    ...
    Result.cy := InnerListBox.CalculateContentSize(DisplayRowsCount).cy + 2 *
      dxAutoCompleteWindowBorderSize;
  end
  else
    Result := cxNullSize;
...
function TdxCustomAutoCompleteWindow.CreateInnerListBox
  : TdxCustomAutoCompleteInnerListBox;
begin
...
procedure TdxCustomAutoCompleteWindow.LookAndFeelChanged(Sender: TcxLookAndFeel;
  AChangedValues: TcxLookAndFeelValues);
begin
  Invalidate;
end;
...
begin
  Canvas.FrameRect(ClientRect,
    LookAndFeel.Painter.GetContainerBorderColor(False));
end;
...
procedure TdxCustomAutoCompleteWindow.ProcessNavigationKey(var Key: Word;
  Shift: TShiftState);
begin
...
begin
  if InnerListBox.PointInSizeGrip
    (InnerListBox.ScreenToClient(SmallPointToPoint(Message.Pos))) then
...
procedure TdxCustomAutoCompleteWindow.SetHighlightSearchText
  (const Value: Boolean);
begin
