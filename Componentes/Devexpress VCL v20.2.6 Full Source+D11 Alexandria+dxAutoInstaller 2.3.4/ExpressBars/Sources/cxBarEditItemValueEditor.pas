...
uses
  Windows, Classes, Controls, ExtCtrls, Forms, Math, StdCtrls, SysUtils,
  TypInfo,
...
var
  ...
  begin
    ...
    begin
      AFilterEditHelper := FilterEditsController.FindHelper
    ...
    TdxBarManagerAccess(AItem.BarManager).GetRealLookAndFeel
      (AEdit.Style.LookAndFeel);
  end;
  ...
  begin
    ...
    AButtonOK := CreateButton(AForm,
      cxGetResourceString(@dxSBAR_DIALOGOK), mrOk);
    AButtonOK.Default := True;

    AButtonCancel := CreateButton(AForm,
      cxGetResourceString(@dxSBAR_DIALOGCANCEL), mrCancel);
...
begin
  ...
  try
    ...
    AButtonCancel.SetBounds(AButtonOK.BoundsRect.Right + D * 2,
      AButtonOK.Top, W, H);
    ...
    begin
      ...
      if UseFilterHelper then
      begin
        FilterEditsController.FindHelper(AProperties.ClassType)
          .GetFilterValue(AEdit, AProperties, V, S);
        AItem.EditValue := V;
      end
      else
      begin
...
begin
  Result := InternalShowValueEditor(TcxCustomBarEditItem(AItemLink.Item),
    AItemLink);
end;
