...
type
  ...
  protected
    class function CheckStatusBarRect(AFormStatusBar: TdxSkinElement;
      const R: TRect): TRect;
    class procedure DrawContainerControl(APanelStyle
      : TdxStatusBarContainerPanelStyle); override;
    class function GetSkinInfo(AStatusBar: TdxCustomStatusBar;
      out ASkinInfo: TdxSkinInfo): Boolean;
  ...
  public
    class procedure AdjustTextColor(AStatusBar: TdxCustomStatusBar;
      var AColor: TColor; Active: Boolean); override;
    ...
    class procedure DrawClippedElement(AElement: TdxSkinElement;
      ACanvas: TcxCanvas; const ARect, AClipRegion: TRect);
    ...
    class function PanelContentExtents(AStatusBar: TdxCustomStatusBar;
      AHasBorders: Boolean): TRect; override;
...
begin
  ...
  if AColor = clWindowText then
    ...
    begin
      ...
      else if ASkinInfo.BarDisabledTextColor <> nil then
        AColor := ASkinInfo.BarDisabledTextColor.Value;
    end;
end;

class function TdxStatusBarSkinPainter.ContentExtents
...
begin
  ...
  begin
    ...
    Result := cxRectOffset(TdxCustomStatusBarAccess(AStatusBar)
      .ScaleFactor.Apply(Result), TdxCustomStatusBarAccess(AStatusBar)
      .BorderWidth, TdxCustomStatusBarAccess(AStatusBar).BorderWidth);
  end
  else
    inherited ContentExtents(AStatusBar);
end;

class function TdxStatusBarSkinPainter.IsSizeGripInPanelArea
...
class function TdxStatusBarSkinPainter.SizeGripMargins
  (AStatusBar: TdxCustomStatusBar): TRect;
var
  ASkinInfo: TdxSkinInfo;
begin
  if GetSkinInfo(AStatusBar, ASkinInfo) and (ASkinInfo.FormStatusBar <> nil)
  then
    Result := TdxCustomStatusBarAccess(AStatusBar)
      .ScaleFactor.Apply(ASkinInfo.FormStatusBar.ContentOffset.Rect)
  else
    Result := cxNullRect;
end;
...
class procedure TdxStatusBarSkinPainter.DrawClippedElement
  (AElement: TdxSkinElement; ACanvas: TcxCanvas; const ARect: TRect;
  const AClipRegion: TRect);
begin
...
class procedure TdxStatusBarSkinPainter.DrawEmptyPanel
  (AStatusBar: TdxCustomStatusBar; ACanvas: TcxCanvas; R: TRect);
var
  ASkinInfo: TdxSkinInfo;
begin
  if not GetSkinInfo(AStatusBar, ASkinInfo) or (ASkinInfo.FormStatusBar = nil)
  then
...
class procedure TdxStatusBarSkinPainter.DrawPanelBorder
...
class procedure TdxStatusBarSkinPainter.DrawPanelSeparator
...
class procedure TdxStatusBarSkinPainter.DrawSizeGrip
  (AStatusBar: TdxCustomStatusBar; ACanvas: TcxCanvas; R: TRect);
var
  ASkinInfo: TdxSkinInfo;
begin
  ...
  if GetSkinInfo(AStatusBar, ASkinInfo) and (ASkinInfo.SizeGrip <> nil) then
    ASkinInfo.SizeGrip.RightToLeftDependentDraw(ACanvas.Handle, R,
      dxGetScaleFactor(AStatusBar), ACanvas.UseRightToLeftAlignment)
  else
...
begin
  if not AHasBorders then
    Result := cxNullRect
  else if GetSkinInfo(AStatusBar, AInfo) and (AInfo.LinkBorderPainter <> nil)
  then
    Result := TdxCustomStatusBarAccess(AStatusBar)
      .ScaleFactor.Apply(AInfo.LinkBorderPainter.ContentOffset.Rect)
  else
    Result := inherited PanelContentExtents(AStatusBar, AHasBorders);
end;

class function TdxStatusBarSkinPainter.SeparatorSize
  (AStatusBar: TdxCustomStatusBar): Integer;
begin
  Result := 1;
end;

class function TdxStatusBarSkinPainter.CheckStatusBarRect(AFormStatusBar
  : TdxSkinElement; const R: TRect): TRect;
begin
  Result := R;
...
class procedure TdxStatusBarSkinPainter.DrawContainerControl
...
begin
  ...
  begin
    ...
    AParentRect := cxRectOffset(AStatusBar.ClientBounds, -AControl.Left,
      -AControl.Top);
    DrawClippedElement(ASkinInfo.FormStatusBar, AControl.Canvas,
      CheckStatusBarRect(ASkinInfo.FormStatusBar, AParentRect),
      AControl.ClientRect);
  end
  else
...
begin
  Result := TdxCustomStatusBarAccess(AStatusBar)
    .LookAndFeel.SkinPainter.GetPainterData(ASkinInfo);
end;
