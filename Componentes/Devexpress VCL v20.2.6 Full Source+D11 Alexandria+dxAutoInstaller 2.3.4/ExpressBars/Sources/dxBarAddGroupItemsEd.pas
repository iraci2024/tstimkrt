...
{$ENDIF}
  ...
  dxCommon, dxBar, cxClasses, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, dxLayoutContainer, dxForms,
  dxLayoutControlAdapters, dxLayoutControl, dxLayoutLookAndFeels, cxContainer,
  cxEdit, cxListBox, Menus, cxButtons,
  cxCustomListBox;
...
type
  ...
  TfrmAddGroupItems = class(TdxForm)
    ...
    procedure lbGroupItemsDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
      AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
    procedure lbGroupItemsMeasureItem(AControl: TcxListBox; AIndex: Integer;
      var Height: Integer);
  strict private
    FGroup: TdxBarGroup;
...
procedure TfrmAddGroupItems.lbGroupItemsDrawItem(AControl: TcxListBox;
  ...
  procedure DrawLine(const S: string; const R: TRect;
    ABackgroundColor, ATextColor: TColor);
  begin
    ACanvas.Font.Color := ATextColor;
    ACanvas.FillRect(R, ABackgroundColor);
    cxExtTextOut(ACanvas.Handle, S, Point(R.Left + 2, R.Top + 1), R,
      ETO_OPAQUE);
  end;

  procedure CalculateLineColors(APainter: TcxCustomLookAndFeelPainter;
    AItem: TdxBarComponent; out ABackgroundColor, ATextColor: TColor);
  begin
    if Group.IndexOf(AItem) >= 0 then
    begin
      ATextColor := cxGetActualColor(APainter.DefaultEditorTextColor(True),
        clGrayText);
      ABackgroundColor := cxGetActualColor
        (APainter.DefaultEditorBackgroundColor(True), clWindow);
    end
    else if odSelected in AState then
    begin
      ATextColor := cxGetActualColor(APainter.DefaultSelectionTextColor,
        clHighlightText);
      ABackgroundColor := cxGetActualColor(APainter.DefaultSelectionColor,
        clHighlight);
    end
    else
    begin
      ATextColor := cxGetActualColor(APainter.DefaultEditorTextColor(False),
        clWindowText);
      ABackgroundColor := cxGetActualColor
        (APainter.DefaultEditorBackgroundColor(False), clWindow);
    end;
  end;
...
begin
  ...
  begin
    DrawLine(Copy(AValue, 2, MaxInt), ARect,
      APainter.DefaultHeaderBackgroundColor,
      APainter.DefaultHeaderBackgroundTextColor);
    Exit;
  end;
...
procedure TfrmAddGroupItems.lbGroupItemsMeasureItem(AControl: TcxListBox;
