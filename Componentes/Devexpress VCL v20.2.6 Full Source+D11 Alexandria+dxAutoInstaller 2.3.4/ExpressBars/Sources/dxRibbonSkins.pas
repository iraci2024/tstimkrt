...
{$ENDIF}
  ...
  cxGeometry,
  StdCtrls, ImgList, ExtCtrls, dxCore, cxClasses, dxCoreClasses, cxGraphics,
  dxBarSkin, dxBar,
  dxBarSkinConsts, dxGDIPlusAPI, dxGDIPlusClasses, cxLookAndFeelPainters,
  dxScreenTip, dxDPIAwareUtils, dxCoreGraphics,
...
const
  ...
  rfspBorderIconAutoHideShowUIGlyph = rfspBorderIconDisplayOptionsGlyph + 4;
  // 4 states;
  ...
  rspCollapsedToolbarGlyphBackgroundNormal = rspCollapsedToolbarNormal +
    DXBAR_STATESCOUNT;

  rspEditButtonNormal = rspCollapsedToolbarGlyphBackgroundNormal +
    DXBAR_STATESCOUNT;

  rspSmallButtonNormal = rspEditButtonNormal + DXBAR_STATESCOUNT;
  rspSmallButtonGlyphBackgroundNormal = rspSmallButtonNormal +
    DXBAR_STATESCOUNT;
  rspSmallButtonDropButtonNormal = rspSmallButtonGlyphBackgroundNormal +
    DXBAR_STATESCOUNT;

  rspLargeButtonNormal = rspSmallButtonDropButtonNormal + DXBAR_STATESCOUNT;
  rspLargeButtonGlyphBackgroundNormal = rspLargeButtonNormal +
    DXBAR_STATESCOUNT;
  rspLargeButtonDropButtonNormal = rspLargeButtonGlyphBackgroundNormal +
    DXBAR_STATESCOUNT;
  ...
  rspButtonGroupSplitButtonSeparatorNormal = rspButtonGroupBorderMiddleNormal +
    DXBAR_STATESCOUNT;

  rspToolbarNormal = rspButtonGroupSplitButtonSeparatorNormal +
    DXBAR_STATESCOUNT;
  ...
  rspLaunchButtonDefaultGlyphNormal = rspLaunchButtonBackgroundNormal +
    DXBAR_STATESCOUNT;

  rspTabScrollLeftButtonNormal = rspLaunchButtonDefaultGlyphNormal +
    DXBAR_STATESCOUNT;
  rspTabScrollRightButtonNormal = rspTabScrollLeftButtonNormal +
    DXBAR_STATESCOUNT;
  rspGroupScrollLeftButtonNormal = rspTabScrollRightButtonNormal +
    DXBAR_STATESCOUNT;
  rspGroupScrollRightButtonNormal = rspGroupScrollLeftButtonNormal +
    DXBAR_STATESCOUNT;

  rspInRibbonGalleryScrollBarLineUpButtonNormal =
    rspGroupScrollRightButtonNormal + DXBAR_STATESCOUNT;
  rspInRibbonGalleryScrollBarLineDownButtonNormal =
    rspInRibbonGalleryScrollBarLineUpButtonNormal + DXBAR_STATESCOUNT;
  rspInRibbonGalleryScrollBarDropDownButtonNormal =
    rspInRibbonGalleryScrollBarLineDownButtonNormal + DXBAR_STATESCOUNT;

  // For Ribbon 2010
  rfspCloseButtonHot = rspInRibbonGalleryScrollBarDropDownButtonNormal +
    DXBAR_STATESCOUNT;
  ...
  rbvpBackstageViewMenuItem = rbvpBackstageViewMenuHeader + 1;
  // 2 states * 5 color schemes
  rbvpBackstageViewMenuTabButton = rbvpBackstageViewMenuItem + 10;
  // 4 states * 5 color schemes
...
type
  TdxInRibbonGalleryScrollBarButtonKind = (gsbkLineUp, gsbkLineDown,
    gsbkDropDown);

  TdxRibbonApplicationButtonState = (rabsNormal, rabsHot, rabsPressed);
  TdxRibbonApplicationMenuState = (ramsHidden, ramsShownAsMenu,
    ramsShownAsFrame, ramsShownAsFullScreenFrame);
  TdxRibbonColorSchemeAccent = (rcsaYellow, rcsaBlue, rcsaGreen, rcsaOrange,
    rcsaPurple);
  ...
  TdxRibbonBorderIcon = (rbiSystemMenu, rbiMinimize, rbiMaximize, rbiHelp,
    rbiDisplayOptions, rbiAutoHideModeShowUI);
  TdxRibbonBorderIcons = set of TdxRibbonBorderIcon;
  TdxRibbonBorderIconState = (rbisNormal, rbisHot, rbisPressed, rbisInactive,
    rbisHotInactive);
  ...
  IdxRibbonSkinColorPaletteSet = interface
    ...
    function Add(AState: Integer; APalette: IdxColorPalette)
      : IdxRibbonSkinColorPaletteSet;
  ...
  TdxRibbonSkinColorPaletteSet = class(TInterfacedObject,
    IdxRibbonSkinColorPaletteSet)
  strict private
  ...
  protected const
    BorderIconStateToPartIndex: array [TdxRibbonBorderIconState] of Integer =
      (-1, 0, 1, -1, 2);
  ...
  strict private
    ...
    function GetColorPalette(ASet: IdxRibbonSkinColorPaletteSet;
      AState: Integer): IdxColorPalette;
  ...
  protected
    ...
    procedure AddTwoStateElement(ABitmap: GpBitmap; var AParts;
      const R, F: TRect; ID: Integer;
      AInterpolationMode: Integer = InterpolationModeNearestNeighbor);
    procedure InternalDrawFormBorders(DC: HDC; const R: TRect;
      const AIsActive, AIsRectangular: Boolean);
    procedure InternalDrawGlyph(DC: HDC; const R: TRect; APart: Integer;
      AColor: TColor = clDefault; ANeedCorrection: Boolean = True;
    ...
    procedure DrawArrow(DC: HDC; const R: TRect;
      AArrowDirection: TcxArrowDirection; AColor: TColor);
    ...
    procedure DrawPart(const AParts: TStatesArray; DC: HDC; const R: TRect;
      AState: Integer); overload;
    procedure GetApplicationMenuContentColors(var AInnerBorderColor,
      AOuterBorderColor, ASideColor: TColor); virtual;
    function GetArrowSize: Integer;

    function DoGetPartColor(APart: Integer; AState: Integer = 0)
      : TColor; virtual;
    function DoGetPartColorCore(APart: Integer; AState: Integer = 0): TColor;
    function DoGetPartLowColor(APart: Integer; AState: Integer = 0)
      : TColor; virtual;

    property ColorSchemeAccent: TdxRibbonColorSchemeAccent
      read FColorSchemeAccent;
    ...
    property ScaleFactorForTextures: TdxScaleFactor
      read FScaleFactorForTextures;
  public
    ...
    procedure InitializePaintData(const APaintData
      : IdxRibbonPaintData); virtual;

    // Load
    procedure LoadBitmapFromStream(const AResName: string; AImage: TdxGPImage;
      AInstance: THandle = 0); virtual;
    procedure LoadElementParts(ABitmap: GpBitmap; var AParts; const R: TRect;
      AID: Integer; const AFixedSize: TRect; const AImageIndexes: array of Byte;
      const APossibleStates: TdxByteSet; AIsTopDown: Boolean = True;
      AInterpolationMode: Integer = InterpolationModeDefault);
    procedure LoadElementPartsFromFile(const AFileName: string; var AParts;
      AID: Integer; const AFixedSize: TRect; const AImageIndexes: array of Byte;
      const APossibleStates: TdxByteSet);
    procedure LoadFourStateArray(ABitmap: GpBitmap; R: TRect;
      const Fixed: TRect; var AStateArray: TFourStateArray; AStartID: Integer;
      AIsVerticalLayout: Boolean = True;
      AInterpolationMode: Integer = InterpolationModeDefault);
    procedure LoadThreeStateArray(ABitmap: GpBitmap; R: TRect;
    ...
    procedure DrawApplicationButton(DC: HDC; const R: TRect;
      AState: TdxRibbonApplicationButtonState); virtual;
    procedure DrawApplicationMenuBackground(DC: HDC;
      const R, AContentRect: TRect); virtual;
    ...
    procedure DrawApplicationMenuExtraPanePinButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawApplicationMenuExtraPanePinButtonGlyph(DC: HDC;
      const R: TRect; AState: Integer; AChecked: Boolean); virtual;
    function GetApplicationMenuContentOffset(const ATabsBounds: TRect)
      : TRect; virtual;
    function GetApplicationMenuGlyphSize: TSize; virtual;
    function GetApplicationMenuTextColor
      (AState: TdxRibbonApplicationButtonState): TColor; virtual;
    ...
    procedure DrawButtonGroupSplitButtonSeparator(DC: HDC; const R: TRect;
      AState: Integer); virtual;

    // CollapsedToolbar
    procedure DrawCollapsedToolbarBackground(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawCollapsedToolbarGlyphBackground(DC: HDC; const R: TRect;
      AState: Integer); virtual;

    // DropDownGallery
    procedure DrawDropDownGalleryBackground(DC: HDC; const R: TRect); virtual;
    procedure DrawDropDownGalleryBottomSizeGrip(DC: HDC;
      const R: TRect); virtual;
    procedure DrawDropDownGalleryBottomSizingBand(DC: HDC;
      const R: TRect); virtual;
    procedure DrawDropDownGalleryBottomVerticalSizeGrip(DC: HDC;
      const R: TRect); virtual;
    procedure DrawDropDownGalleryTopSizeGrip(DC: HDC; const R: TRect); virtual;
    procedure DrawDropDownGalleryTopSizingBand(DC: HDC;
      const R: TRect); virtual;
    procedure DrawDropDownGalleryTopVerticalSizeGrip(DC: HDC;
      const R: TRect); virtual;

    // Edit
    procedure DrawEditArrowButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawEditButton(DC: HDC; const R: TRect; AState: Integer); virtual;
    procedure DrawEditEllipsisButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawEditSpinDownButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawEditSpinUpButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;

    // Form
    function AdjustCaptionFontSize(ASize: Integer; AUseAeroGlass: Boolean)
      : Integer; virtual;
    ...
    procedure DrawFormStatusBarPart(DC: HDC; const R: TRect;
      AIsLeft, AIsActive, AIsRaised, AIsRectangular: Boolean); virtual;
    ...
    // InRibbonGallery
    procedure DrawInRibbonGalleryBackground(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawInRibbonGalleryScrollBarBackground(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawInRibbonGalleryScrollBarButton(DC: HDC; const R: TRect;
      AButtonKind: TdxInRibbonGalleryScrollBarButtonKind;
      AState: Integer); virtual;
    ...
    function GetInRibbonGalleryScrollBarButtonGlyphColor
      ...
      : TColor; virtual;
    procedure DrawInRibbonGalleryScrollBarDropDownTouchButton(DC: HDC;
      const R: TRect; AState: Integer); virtual;
    ...
    procedure DrawMenuDetachCaption(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawMenuExtraSeparator(DC: HDC; const R: TRect;
    ...
    procedure DrawMenuScrollArea(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    ...
    // Minimize Button
    procedure DrawMinimizeRibbonButton(DC: HDC; const R: TRect;
      AState: TcxButtonState; AMinimized: Boolean); virtual;
    ...
    procedure DrawSmallButtonDropButtonMainPart(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    function GetSmallButtonColorPalette(AState: Integer)
    ...
    procedure DrawBackstageViewMenuSeparator(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    function GetBackstageViewMenuButtonColorPalette(AState: Integer)
      : IdxColorPalette; virtual;

    // BackstageViewGalleryControl
    procedure DrawBackstageViewGalleryBackground(DC: HDC;
      const R: TRect); virtual;
    procedure DrawBackstageViewGalleryGroupHeader(DC: HDC;
      const R: TRect); virtual;
    ...
    procedure DrawBackstageViewGalleryItemPinButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawBackstageViewGalleryItemPinButtonGlyph(DC: HDC;
      const R: TRect; AState: Integer); virtual;
    procedure DrawBackstageViewGalleryItemPinTag(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawBackstageViewGallerySeparator(DC: HDC;
      const R: TRect); virtual;

    // Marks
    procedure DrawMarkArrow(DC: HDC; const R: TRect; AState: Integer); virtual;
    procedure DrawMarkTruncated(DC: HDC; const R: TRect;
      AState: Integer); virtual;

    // Tab
    procedure AdjustTabFont(AFont: TFont; AState: Integer); virtual;
    procedure DrawTab(DC: HDC; const R: TRect;
      AState: TdxRibbonTabState); virtual;
    procedure DrawTabAreaBackground(DC: HDC; const R: TRect;
      AActive, AUseAeroGlass: Boolean;
      AApplicationMenuState: TdxRibbonApplicationMenuState); virtual;
    procedure DrawTabBase(DC: HDC; const R: TRect); virtual;
    procedure DrawTabGroupBackground(DC: HDC; const R: TRect; AState: Integer;
      AIsInPopup: Boolean); virtual;
    procedure DrawTabGroupHeaderBackground(DC: HDC; const R: TRect;
      AState: Integer; AIsInPopup: Boolean); virtual;
    procedure DrawTabGroupsArea(DC: HDC; const R: TRect;
      AIsQATAtBottom, AIsInPopup: Boolean); virtual;
    procedure DrawTabScrollButton(DC: HDC; const R: TRect; ALeft: Boolean;
      AState: Integer); virtual;
    procedure DrawTabScrollButtonGlyph(DC: HDC; const R: TRect;
      ALeft: Boolean); virtual;
    procedure DrawTabSeparator(DC: HDC; const R: TRect; Alpha: Byte); virtual;

    // TAT SmallButton
    procedure DrawTabAreaArrowDown(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawTabAreaButton(DC: HDC; const R: TRect;
      AState: TcxButtonState); virtual;
    procedure DrawTabAreaButtonDropButtonArrowPart(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawTabAreaButtonDropButtonMainPart(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawTabAreaMarkArrow(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawTabAreaMarkTruncated(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    function GetTabAreaButtonColorPalette(AState: Integer)
      : IdxColorPalette; virtual;

    // Context
    procedure AdjustContextFont(AFont: TFont; AUseGlass: Boolean;
      AContextColor: TColor); virtual;
    procedure AdjustContextTabFont(AFont: TFont; AState: Integer;
      AContextColor: TColor); virtual;
    procedure DrawContextBackground(DC: HDC; const R: TRect;
      AContextColor: TColor); virtual;
    procedure DrawContextBackgroundGlass(DC: HDC; const R: TRect;
      AContextColor: TColor); virtual;
    procedure DrawContextTabBackground(DC: HDC; const R: TRect;
      AState: TdxRibbonTabState; AContextColor: TColor); virtual;
    procedure DrawContextTabGroupsArea(DC: HDC; const R: TRect;
      AContextColor: TColor; AIsQATAtBottom, AIsInPopup: Boolean); virtual;
    procedure DrawContextTabSeparator(DC: HDC; const R: TRect;
      ABeginGroup: Boolean); virtual;

    // QAT
    procedure AdjustQuickAccessToolbarVertical(var ABounds: TRect;
      ANonClientDraw, ADontUseAero: Boolean); virtual;
    procedure DrawQuickAccessToolbar(DC: HDC; const R: TRect;
      ABellow, ANonClientDraw, AHasApplicationButton, AIsActive,
      ADontUseAero: Boolean); virtual;
    procedure DrawQuickAccessToolbarArrowDown(DC: HDC; const R: TRect;
      AState: Integer; ABelow: Boolean); virtual;
    procedure DrawQuickAccessToolbarDefaultGlyph(DC: HDC;
      const R: TRect); virtual;
    ...
    procedure DrawQuickAccessToolbarMarkArrow(DC: HDC; const R: TRect;
      AState: Integer; ABelow: Boolean); virtual;
    procedure DrawQuickAccessToolbarMarkTruncated(DC: HDC; const R: TRect;
      AState: Integer; ABelow: Boolean); virtual;
    procedure DrawQuickAccessToolbarPopup(DC: HDC; const R: TRect); virtual;
    procedure DrawQuickAccessToolbarSmallButton(DC: HDC; const R: TRect;
      AState: Integer); virtual;
    procedure DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC: HDC;
      const R: TRect; AState: Integer); virtual;
    procedure DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC: HDC;
      const R: TRect; AState: Integer); virtual;
    function GetQuickAccessToolbarColorPalette(AState: Integer; ABelow: Boolean)
      : IdxColorPalette; virtual;
    function GetQuickAccessToolbarLeftIndent(AHasApplicationButton,
      AUseAeroGlass: Boolean): Integer; virtual;
    function GetQuickAccessToolbarMarkButtonOffset(AHasApplicationButton,
      ABelow: Boolean): Integer; virtual;
    function GetQuickAccessToolbarOverrideWidth(AHasApplicationButton,
      AUseAeroGlass: Boolean): Integer; virtual;
    function GetQuickAccessToolbarRightIndent(AHasApplicationButton: Boolean)
      : Integer; virtual;
    ...
    function GetStatusBarPanelColorPalette(AState: Integer)
      : IdxColorPalette; virtual;
    ...
    procedure DrawScrollBarBackground(DC: HDC; const R: TRect;
      AHorizontal: Boolean); virtual;
    procedure DrawScrollBarPart(DC: HDC; const R: TRect;
      APart: TcxScrollBarPart; AState: Integer; AHorizontal: Boolean); virtual;
    ...
    procedure DrawGroupScrollButton(DC: HDC; const R: TRect; ALeft: Boolean;
      AState: Integer); virtual;
    procedure DrawGroupScrollButtonGlyph(DC: HDC; const R: TRect;
      ALeft: Boolean); virtual;

    // MiniToolbar
    procedure DrawMiniToolbarBackground(DC: HDC; const R: TRect); virtual;
    function GetMiniToolbarColorPalette(AState: Integer)
      : IdxColorPalette; virtual;
    ...
    procedure DrawGalleryGroupHeaderBackground(DC: HDC;
      const R: TRect); virtual;
    procedure DrawGalleryGroupItemSelectionFrame(DC: HDC; const ARect: TRect;
      AState: Integer); virtual;
    procedure DrawItemSeparator(DC: HDC; const R: TRect;
      AHorizontal: Boolean); virtual;
    ...
    procedure DrawRibbonFormBackground(DC: HDC; const R: TRect;
      ARibbonHeight: Integer); virtual;
    procedure DrawRibbonTopFrameArea(DC: HDC; const R: TRect;
      AUseAeroGlass: Boolean); virtual;
    ...
    procedure DrawScrollArrow(DC: HDC; const R: TRect;
      AState: Integer = 0); virtual;
    ...
    property UseExternalBorders: Boolean read FUseExternalBorders
      write FUseExternalBorders;
    property UseRightToLeftAlignment: Boolean read FUseRightToLeftAlignment
      write FUseRightToLeftAlignment;
  end;
  ...
  public
    ...
    function GetMostSuitable(const AName: string; AStyle: TdxRibbonStyle;
      ATargetDPI: Integer): TdxCustomRibbonSkin;
...
const
  dxRibbonColorSchemeAccentNames: array [TdxRibbonColorSchemeAccent]
    of string = ('Yellow', 'Blue', 'Green', 'Orange', 'Purple');
...
function IsRectangularFormBottom(AFormData: IdxRibbonFormPaintData)
  : Boolean; inline;
...
begin
  Result := (AFormData.GetFormBorderStyle in [bsDialog, bsSingle, bsToolWindow])
    or (AFormData.GetStyle = fsMDIChild) or (AFormData.GetState = wsMinimized)
    or not AFormData.UseRoundedWindowCorners;
end;
...
constructor TdxRibbonSkinColorPalette.Create(ARed, AGreen, AWhite, ABlack,
  AYellow, ABlue: TColor);
begin
  inherited Create;
...
function TdxRibbonSkinColorPaletteSet.Add(AState: Integer;
  APalette: IdxColorPalette): IdxRibbonSkinColorPaletteSet;
begin
  Result := Self;
...
procedure TdxCustomRibbonSkin.InitializePaintData(const APaintData
  : IdxRibbonPaintData);
begin
...
procedure TdxCustomRibbonSkin.LoadBitmapFromStream(const AResName: string;
  AImage: TdxGPImage; AInstance: THandle = 0);
var
  AResStream: TStream;
begin
  if AInstance = 0 then
...
procedure TdxCustomRibbonSkin.LoadElementParts(ABitmap: GpBitmap; var AParts;
  const R: TRect; AID: Integer; const AFixedSize: TRect;
  const AImageIndexes: array of Byte; const APossibleStates: TdxByteSet;
  AIsTopDown: Boolean = True;
  AInterpolationMode: Integer = InterpolationModeDefault);
var
  ...
  begin
    ...
    begin
      if Length(AImageIndexes) = 0 then
        AImageIndex := J
      else if J < Length(AImageIndexes) then
        AImageIndex := AImageIndexes[J]
      else
        AImageIndex := 0;
      ALoadRect := cxRectOffset(R, AOffsetSize.cx * AImageIndex,
        AOffsetSize.cy * AImageIndex);
      Inc(J);
      if cxRectIsEqual(cxEmptyRect, AFixedSize) then
        TStatesArray(AParts)[I] := AddPart1x1(ABitmap, ALoadRect, AID, '',
          AInterpolationMode)
      else
        TStatesArray(AParts)[I] := AddPart3x3(ABitmap, ALoadRect, AFixedSize,
          AID, '', AInterpolationMode);
    end;
    Inc(AID);
  end;
end;
...
procedure TdxCustomRibbonSkin.LoadFourStateArray(ABitmap: GpBitmap; R: TRect;
  const Fixed: TRect; var AStateArray: TFourStateArray; AStartID: Integer;
  AIsVerticalLayout: Boolean; AInterpolationMode: Integer);
var
  I: Integer;
begin
  for I := 0 to 3 do
  begin
    AStateArray[I] := AddPart3x3(ABitmap, R, Fixed, AStartID, '',
      AInterpolationMode);
...
procedure TdxCustomRibbonSkin.AdjustApplicationButtonFont(AFont: TFont;
  AState: TdxRibbonApplicationButtonState);
const
  StatesMap: array [TdxRibbonApplicationButtonState] of Integer = (DXBAR_NORMAL,
...
const
  ...
  PenColorMap: array [TdxRibbonApplicationButtonState] of TColor =
    (clBtnHighlight, clWhite, clBtnShadow);
  TabStateMap: array [TdxRibbonApplicationButtonState] of TdxRibbonTabState =
    (rtsNormal, rtsHot, rtsActive);
var
  ARect: TRect;
begin
...
procedure TdxCustomRibbonSkin.DrawApplicationMenuBackground(DC: HDC;
  const R, AContentRect: TRect);
var
...
begin
  ...
  AFooterRect := cxRectSetTop(R1, AOuterBorderRect.Bottom,
    R1.Bottom - AOuterBorderRect.Bottom);
  ...
  begin
    Parts[FApplicationMenuContentHeader].Draw(DC, AHeaderRect, 255, clDefault,
      UseRightToLeftAlignment);
    Parts[FApplicationMenuContentFooter].Draw(DC, AFooterRect, 255, clDefault,
      UseRightToLeftAlignment);
    Parts[FApplicationMenuBorder].Draw(DC, R, 255, clDefault,
      UseRightToLeftAlignment);
  end;

  GetApplicationMenuContentColors(AInnerBorderColor, AOuterBorderColor,
    ASideColor);
  DrawFrame(DC, AInnerBorderRect, clNone, AInnerBorderColor);
  DrawFrame(DC, AOuterBorderRect, clNone, AOuterBorderColor);
  FillRectByColor(DC, Rect(AOuterBorderRect.Right, AHeaderRect.Bottom, R1.Right,
    AFooterRect.Top), ASideColor);
  FillRectByColor(DC, Rect(R1.Left, AHeaderRect.Bottom, AOuterBorderRect.Left,
    AFooterRect.Top), ASideColor);
end;
...
procedure TdxCustomRibbonSkin.DrawApplicationMenuExtraPaneButton(DC: HDC;
...
procedure TdxCustomRibbonSkin.DrawApplicationMenuExtraPanePinButton(DC: HDC;
  const R: TRect; AState: Integer);
begin
  DrawApplicationMenuExtraPaneButton(DC, R, AState);
  DrawApplicationMenuExtraPanePinButtonGlyph(DC, R, AState,
    AState in [DXBAR_CHECKED, DXBAR_HOTCHECK]);
end;
...
begin
  if LowColors then
    AColor := GetPartColor
      (DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEMPINBUTTONGLYPH, AState)
  else
    AColor := clDefault;
...
function TdxCustomRibbonSkin.GetApplicationMenuContentOffset(const ATabsBounds
  : TRect): TRect;
begin
...
function TdxCustomRibbonSkin.GetApplicationMenuTextColor
  (AState: TdxRibbonApplicationButtonState): TColor;
begin
  Result := clMenuText;
end;
...
procedure TdxCustomRibbonSkin.DrawButtonGroupBorderLeft(DC: HDC;
  const R: TRect);
begin
end;

procedure TdxCustomRibbonSkin.DrawButtonGroupBorderMiddle(DC: HDC;
  const R: TRect; AState: Integer);
begin
end;

procedure TdxCustomRibbonSkin.DrawButtonGroupBorderRight(DC: HDC;
  const R: TRect);
begin
end;

procedure TdxCustomRibbonSkin.DrawButtonGroupSplitButtonSeparator(DC: HDC;
  const R: TRect; AState: Integer);
begin
end;
...
procedure TdxCustomRibbonSkin.DrawDropDownGalleryBackground(DC: HDC;
  const R: TRect);
begin
...
begin
  ...
  if LowColors then
    Parts[FDropDownGalleryBottomSizeGrip].DrawColored(DC, ARect, clWindowText,
      255, UseRightToLeftAlignment)
  else
    Parts[FDropDownGalleryBottomSizeGrip].Draw(DC, ARect, 255, clDefault,
      UseRightToLeftAlignment);
end;
...
procedure TdxCustomRibbonSkin.DrawDropDownGalleryBottomVerticalSizeGrip(DC: HDC;
  const R: TRect);
begin
  DrawDropDownGalleryVerticalSizeGrip(DC, Rect(R.Left, R.Top + 1, R.Right,
    R.Bottom));
end;
...
begin
  ...
  if UseRightToLeftAlignment then
    ARect := TdxRightToLeftLayoutConverter.ConvertRect(ARect, R);
  if LowColors then
    Parts[FDropDownGalleryTopSizeGrip].DrawColored(DC, ARect, clWindowText, 255,
      UseRightToLeftAlignment)
  else
    Parts[FDropDownGalleryTopSizeGrip].Draw(DC, ARect, 255, clDefault,
      UseRightToLeftAlignment);
end;
...
procedure TdxCustomRibbonSkin.DrawDropDownGalleryTopVerticalSizeGrip(DC: HDC;
  const R: TRect);
begin
  DrawDropDownGalleryVerticalSizeGrip(DC, Rect(R.Left, R.Top, R.Right,
    R.Bottom - 1));
end;

procedure TdxCustomRibbonSkin.DrawEditArrowButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
  // do nothing
end;
...
procedure TdxCustomRibbonSkin.DrawEditEllipsisButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
  // do nothing
end;

procedure TdxCustomRibbonSkin.DrawEditSpinDownButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
  // do nothing
end;

procedure TdxCustomRibbonSkin.DrawEditSpinUpButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
  // do nothing
end;

function TdxCustomRibbonSkin.AdjustCaptionFontSize(ASize: Integer;
  AUseAeroGlass: Boolean): Integer;
begin
  Result := ASize;
end;
...
const
  ...
  Buttons: array [TdxRibbonBorderDrawIcon] of Integer = (DFCS_CAPTIONMIN,
    DFCS_CAPTIONMAX, DFCS_CAPTIONRESTORE, DFCS_CAPTIONCLOSE,
    DFCS_CAPTIONHELP, 0, 0);
begin
...
begin
  ...
  try
    ...
    InternalDrawFormBorders(DC, R, FormPaintData.GetIsActive,
      IsRectangularFormBottom(FormPaintData));
  finally
...
begin
  ...
  try
    ...
    begin
      InternalDrawFormBorders(DC, ARect, FormPaintData.GetIsActive,
        IsRectangularFormBottom(FormPaintData));
...
const
  BorderColorMap: array [Boolean] of TColor = (clInactiveBorder,
    clActiveBorder);
var
  ARect: TRect;
...
function TdxCustomRibbonSkin.GetWindowBordersWidth(AHasStatusBar
  : Boolean): TRect;
begin
...
procedure TdxCustomRibbonSkin.DrawInRibbonGalleryScrollBarBackground(DC: HDC;
  const R: TRect; AState: Integer);
begin
  // nothing
end;
...
procedure TdxCustomRibbonSkin.DrawInRibbonGalleryScrollBarButtonGlyph(DC: HDC;
  R: TRect; AButtonKind: TdxInRibbonGalleryScrollBarButtonKind;
  AState: Integer);
var
  AArrowRect: TRect;
  AArrowSize: Integer;
...
begin
  ...
  try
    ...
    AArrowRect := cxRectOffset(cxRectCenter(R, cxSize(AArrowSize)),
      IfThen(not UseRightToLeftAlignment, 1, 2), 1);
    if Odd(AArrowSize) <> Odd(cxRectWidth(R)) then
      AArrowRect := cxRectOffset(AArrowRect,
        IfThen(not UseRightToLeftAlignment, 1), 0);
    ...
    case AButtonKind of
      ...
      gsbkDropDown:
        begin
          ...
          DrawArrow(DC, cxRectOffset(AArrowRect, 0, ScaleFactor.Apply(2)),
            adDown, AColor);
          AArrowRect := cxRectSetBottom(AArrowRect, AArrowRect.Top,
            ScaleFactor.Apply(1));
...
procedure TdxCustomRibbonSkin.DrawInRibbonGalleryScrollBarDropDownTouchButton
...
procedure TdxCustomRibbonSkin.DrawMDIButton(DC: HDC; const R: TRect;
  AButton: TdxBarMDIButton; AState: TcxButtonState);
begin
  // do nothing
end;
...
procedure TdxCustomRibbonSkin.DrawMDIButtonGlyph(DC: HDC; const R: TRect;
  AButton: TdxBarMDIButton; AState: TcxButtonState);
const
  Buttons: array [TdxBarMDIButton] of Integer = (DFCS_CAPTIONMIN,
...
procedure TdxCustomRibbonSkin.DrawMenuDetachCaption(DC: HDC; const R: TRect;
  AState: Integer);
begin
  DrawPart(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawMenuExtraSeparator(DC: HDC; const R: TRect;
  AHorizontal: Boolean);
begin
  if LowColors then
...
procedure TdxCustomRibbonSkin.DrawMenuScrollArea(DC: HDC; const R: TRect;
  AState: Integer);
begin
  DrawPart(DC, R, AState);
end;
...
begin
  if LowColors then
  begin
    Parts[FMenuSeparatorVert].DrawColored(DC, R, clMenu, 255,
      UseRightToLeftAlignment);
    Parts[FMenuSeparatorVert].DrawColored(DC, cxRectSetWidth(R, 1), clBtnShadow,
      255, UseRightToLeftAlignment);
  end
  else
    Parts[FMenuSeparatorVert].Draw(DC, R, 255, clDefault,
      UseRightToLeftAlignment);
end;

function TdxCustomRibbonSkin.GetMenuColorPalette(AState: Integer)
  : IdxColorPalette;
begin
...
function TdxCustomRibbonSkin.GetSmallButtonColorPalette(AState: Integer)
  : IdxColorPalette;
begin
...
procedure TdxCustomRibbonSkin.DrawBackstageViewBackground(DC: HDC;
  const R: TRect);
begin
  FillRectByColor(DC, R, GetPartColor(DXBAR_BACKSTAGEVIEW));
end;

procedure TdxCustomRibbonSkin.DrawBackstageViewMenuBackground(DC: HDC;
  const R: TRect);
begin
  if LowColors then
  begin
    ...
    if UseRightToLeftAlignment then
      ...
      FillRectByColor(DC, cxRectSetRight(R, R.Right, 1),
        dxInvertColor(clWindow));
  end
  else
  begin
...
procedure TdxCustomRibbonSkin.DrawBackstageViewTabButton(DC: HDC;
  const R: TRect; AState: Integer);
begin
  DrawSmallButton(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawBackstageViewMenuHeader(DC: HDC;
  const R: TRect);
begin
  if LowColors then
  begin
...
procedure TdxCustomRibbonSkin.DrawBackstageViewMenuSeparator(DC: HDC;
  const R: TRect; AState: Integer);
var
  R1: TRect;
begin
...
function TdxCustomRibbonSkin.GetBackstageViewMenuButtonColorPalette
  (AState: Integer): IdxColorPalette;
begin
  Result := GetColorPalette(FColorPaletteBackstageViewMenu, AState);
end;

procedure TdxCustomRibbonSkin.DrawBackstageViewGalleryBackground(DC: HDC;
  const R: TRect);
begin
  DrawDropDownGalleryBackground(DC, R);
end;

procedure TdxCustomRibbonSkin.DrawBackstageViewGalleryGroupHeader(DC: HDC;
  const R: TRect);
begin
...
procedure TdxCustomRibbonSkin.DrawBackstageViewGalleryItemPinButton(DC: HDC;
  const R: TRect; AState: Integer);
begin
  case AState of
    DXBAR_CHECKED:
...
procedure TdxCustomRibbonSkin.DrawBackstageViewGalleryItemPinTag(DC: HDC;
  const R: TRect; AState: Integer);
begin
  DrawBackstageViewGalleryItemPinButtonGlyph(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawBackstageViewGallerySeparator(DC: HDC;
  const R: TRect);
begin
  DrawMenuSeparatorHorz(DC, R);
end;

procedure TdxCustomRibbonSkin.DrawMarkArrow(DC: HDC; const R: TRect;
  AState: Integer);
var
  H: Integer;
begin
  H := (R.Bottom - R.Top) div 7;
  DrawPart(DC, cxRect(R.Left + 3, R.Top + H * 3, R.Right - 3,
    R.Bottom - H * 2), AState);
end;

procedure TdxCustomRibbonSkin.DrawMarkTruncated(DC: HDC; const R: TRect;
  AState: Integer);
var
  H: Integer;
begin
  ...
  begin
    H := (R.Bottom - R.Top) div 7;
    DrawPart(DC, cxRect(R.Left + H + 1, R.Top + H * 3, R.Right - H + 1,
      R.Bottom - H * 2), AState);
  end
  else
...
begin
  ...
  case AState of
    ...
    DrawFrame(DC, R1, clHighlight, dxInvertColor(clBtnFace),
      [bLeft, bTop, bRight]);
  end;
end;

procedure TdxCustomRibbonSkin.DrawTabAreaArrowDown(DC: HDC; const R: TRect;
  AState: Integer);
begin
...
procedure TdxCustomRibbonSkin.DrawTabGroupBackground(DC: HDC; const R: TRect;
  AState: Integer; AIsInPopup: Boolean);
begin
  DrawFrame(DC, R, clBtnFace, clBtnShadow, [bTop, bLeft, bRight])
end;

procedure TdxCustomRibbonSkin.DrawTabGroupHeaderBackground(DC: HDC;
  const R: TRect; AState: Integer; AIsInPopup: Boolean);
var
  R1: TRect;
begin
...
procedure TdxCustomRibbonSkin.DrawTabGroupsArea(DC: HDC; const R: TRect;
  AIsQATAtBottom, AIsInPopup: Boolean);
begin
...
procedure TdxCustomRibbonSkin.DrawTabScrollButtonGlyph(DC: HDC; const R: TRect;
  ALeft: Boolean);
const
...
begin
  ...
  try
    cxLookAndFeelPaintersManager.GetPainter(lfsStandard)
      .DrawArrow(cxPaintCanvas, cxRectInflate(R, -2, 0, -2, -4),
      ArrowDirection[ALeft], GetPartColor(rspGroupScrollArrow));
  finally
    cxPaintCanvas.EndPaint;
  end;
end;

procedure TdxCustomRibbonSkin.DrawTabSeparator(DC: HDC; const R: TRect;
  Alpha: Byte);
begin
  Parts[FTabSeparator].Draw(DC, R, Alpha);
end;

procedure TdxCustomRibbonSkin.AdjustContextFont(AFont: TFont;
  AUseGlass: Boolean; AContextColor: TColor);
const
  PartsMap: array [Boolean] of Integer = (rspContextText,
    rspContextTextOnGlass);
begin
  AFont.Color := GetPartColor(PartsMap[AUseGlass]);
end;

procedure TdxCustomRibbonSkin.AdjustContextTabFont(AFont: TFont;
  AState: Integer; AContextColor: TColor);
begin
  AFont.Color := GetPartColor(rspTabHeaderText, AState);
end;

procedure TdxCustomRibbonSkin.DrawContextBackground(DC: HDC; const R: TRect;
...
procedure TdxCustomRibbonSkin.DrawContextBackgroundGlass(DC: HDC;
  const R: TRect; AContextColor: TColor);
begin
  // do nothing
end;
...
procedure TdxCustomRibbonSkin.DrawContextTabGroupsArea(DC: HDC; const R: TRect;
  AContextColor: TColor; AIsQATAtBottom, AIsInPopup: Boolean);
begin
  DrawFrame(DC, R, clBtnFace, clBtnShadow)
end;
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbar(DC: HDC; const R: TRect;
  ...
  procedure InternalDrawQuickAccessToolbarInLowColors(ARect: TRect;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        APath.AddLine(ALeft, ARect.Bottom, ALeft,
          ARect.Bottom - (cxRectHeight(ARect) div 2));
        APath.AddArc(ALeft - 19, ARect.Top, 20, cxRectHeight(ARect) * 2,
          -45, -43);
      ...
      begin
        ...
        APath.AddArc(ARect.Right - ALeft * 2, ARect.Top, ALeft * 2,
          cxRectHeight(ARect), -90, 180);
        APath.AddLine(ARect.Right - ALeft, ARect.Bottom, ARect.Left,
          ARect.Bottom);
      end;
      if UseRightToLeftAlignment then
      begin
        AMatrix := TdxGPMatrix.CreateFlip(True, False,
          cxRectCenter(ARect).X, 0);
        try
      ...
      dxGPPaintCanvas.Path(APath, dxInvertColor(clWindow), clWindow, 1, psSolid,
        255, 255);
    finally
      APath.Free;
...
begin
  ...
  begin
    ...
    begin
      ...
      if UseRightToLeftAlignment then
      begin
        ALeftPartRect := TdxRightToLeftLayoutConverter.ConvertRect
          (ALeftPartRect, R);
        ARightPartRect := TdxRightToLeftLayoutConverter.ConvertRect
          (ARightPartRect, R);
      end;
      if LowColors then
      begin
      ...
      begin
        Parts[ALeftPart].Draw(DC, ALeftPartRect, 255, clDefault,
          UseRightToLeftAlignment);
        Parts[ARightPart].Draw(DC, ARightPartRect, 255, clDefault,
          UseRightToLeftAlignment);
      end;
    end;
  end;
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarArrowDown(DC: HDC;
  const R: TRect; AState: Integer; ABelow: Boolean);
begin
  DrawArrowDown(DC, R, AState);
end;
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarMarkArrow(DC: HDC;
  const R: TRect; AState: Integer; ABelow: Boolean);
begin
  DrawMarkArrow(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarMarkTruncated(DC: HDC;
  const R: TRect; AState: Integer; ABelow: Boolean);
begin
  DrawMarkTruncated(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarPopup(DC: HDC;
  const R: TRect);
begin
  FillRectByColor(DC, R, clBtnFace);
end;
...
function TdxCustomRibbonSkin.GetQuickAccessToolbarColorPalette(AState: Integer;
  ABelow: Boolean): IdxColorPalette;
begin
  if ABelow then
...
procedure TdxCustomRibbonSkin.DrawQuickAccessToolbarSmallButton(DC: HDC;
  const R: TRect; AState: Integer);
begin
  DrawSmallButton(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.
  DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC: HDC; const R: TRect;
  AState: Integer);
begin
  DrawSmallButtonDropButtonArrowPart(DC, R, AState);
end;
...
procedure TdxCustomRibbonSkin.
  DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC: HDC; const R: TRect;
  AState: Integer);
begin
  DrawSmallButtonDropButtonMainPart(DC, R, AState);
end;
...
procedure TdxCustomRibbonSkin.DrawStatusBarPanel(DC: HDC;
  const Bounds, R: TRect; AIsLowered: Boolean);
begin
  FillRectByColor(DC, R, clBtnFace);
end;
...
begin
  FillRectByColor(DC, R, clBtnFace);
  FillRectByColor(DC, cxRect(R.Left, R.Top + 1, R.Left + 1, R.Bottom - 1),
    clBtnShadow);
end;
...
begin
  ARect := R;
  cxRightToLeftDependentDraw(DC, ARect, UseRightToLeftAlignment, procedure
    begin
      if LowColors then
        Office11DrawSizeGrip(DC, ARect, clBtnText, clBtnShadow)
      else
        Office11DrawSizeGrip(DC, ARect,
          GetPartColor(rspStatusBarSizeGripColor1),
          GetPartColor(rspStatusBarSizeGripColor2));
    end, False);
end;

procedure TdxCustomRibbonSkin.DrawStatusBarToolbarSeparator(DC: HDC;
const R: TRect);
begin
  FillRectByColor(DC, R, clBtnFace);
  FillRectByColor(DC, cxRect(R.Left, R.Top, R.Left + 1, R.Bottom - 1),
    clBtnShadow);
end;

function TdxCustomRibbonSkin.GetStatusBarPanelColorPalette(AState: Integer)
  : IdxColorPalette;
begin
...
procedure TdxCustomRibbonSkin.DrawScrollBarBackground(DC: HDC; const R: TRect;
AHorizontal: Boolean);
begin
  cxPaintCanvas.BeginPaint(DC);
  try
    GetScrollBarPainter.DrawScaledScrollBarBackground(cxPaintCanvas, R,
      AHorizontal, ScaleFactor);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;
...
APart: TcxScrollBarPart; AState: Integer; AHorizontal: Boolean);
begin
  cxPaintCanvas.BeginPaint(DC);
  try
    GetScrollBarPainter.DrawScaledScrollBarPart(cxPaintCanvas, AHorizontal, R,
      APart, RibbonStateToButtonState(AState), ScaleFactor);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;

procedure TdxCustomRibbonSkin.DrawScrollBoxSizeGripArea(DC: HDC;
const R: TRect);
begin
...
procedure TdxCustomRibbonSkin.DrawGroupScrollButtonGlyph(DC: HDC;
const R: TRect; ALeft: Boolean);
const
  ArrowDirection: array [Boolean] of TcxArrowDirection = (adRight, adLeft);
begin
  cxPaintCanvas.BeginPaint(DC);
  try
    cxLookAndFeelPaintersManager.GetPainter(lfsStandard)
      .DrawArrow(cxPaintCanvas, cxRectInflate(R, -2, 0, -2, -2),
      ArrowDirection[ALeft], GetPartColor(rspGroupScrollArrow));
  finally
    cxPaintCanvas.EndPaint;
  end;
...
procedure TdxCustomRibbonSkin.DrawMiniToolbarBackground(DC: HDC;
const R: TRect);
begin
  FillRectByColor(DC, R, GetPartColor(DXBAR_MINITOOLBAR_BACKGROUND));
end;

function TdxCustomRibbonSkin.GetMiniToolbarColorPalette(AState: Integer)
  : IdxColorPalette;
begin
...
procedure TdxCustomRibbonSkin.DrawArrowDown(DC: HDC; const R: TRect;
AState: Integer);
begin
...
procedure TdxCustomRibbonSkin.DrawGalleryGroupHeaderBackground(DC: HDC;
const R: TRect);
begin
  FillRectByColor(DC, R, clBtnShadow);
end;

procedure TdxCustomRibbonSkin.DrawGalleryGroupItemSelectionFrame(DC: HDC;
...
procedure TdxCustomRibbonSkin.DrawItemSeparator(DC: HDC; const R: TRect;
AHorizontal: Boolean);
begin
  if LowColors then
...
procedure TdxCustomRibbonSkin.DrawRibbonFormBackground(DC: HDC; const R: TRect;
ARibbonHeight: Integer);
begin
  FillRectByColor(DC, R, GetPartColor(rfspRibbonForm));
end;

procedure TdxCustomRibbonSkin.DrawRibbonTopFrameArea(DC: HDC; const R: TRect;
AUseAeroGlass: Boolean);
begin
  // do nothing
end;

procedure TdxCustomRibbonSkin.DrawRibbonTopFrameAreaSeparator(DC: HDC;
const R: TRect);
begin
  // do nothing
end;
...
begin
  if LowColors then
    Parts[FScrollArrow].DrawColored(DC, R,
...
function TdxCustomRibbonSkin.GetPartColor(APart: Integer;
...
begin
  case APart of
    ...
    DXBAR_LAUNCHBUTTONDEFAULTGLYPH:
      Result := ScaleFactorForTextures.Apply(Parts[FLaunchButtonDefaultGlyphs[0]
        ].Size.cx);
    DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEMPINBUTTON,
      DXBAR_BACKSTAGEVIEW_GALLERYCONTROL_ITEMPINTAG,
      DXBAR_MENUEXTRAPANE_PINBUTTON:
      Result := ScaleFactorForTextures.Apply(Parts[FPinButtonGlyphs[False]]
        .Size.cx) + cxMarginsWidth(GetPartContentOffsets(APart));
  else
    Result := 0;
  end;
end;
...
begin
  DC := GetDC(0);
  FLowColors := (GetDeviceCaps(DC, BITSPIXEL) <= 8) or
    dxRibbonUseLowColorsInHighContrastThemes and IsHighContrast;
  ReleaseDC(0, DC);
end;

procedure TdxCustomRibbonSkin.AddTwoStateElement(ABitmap: GpBitmap; var AParts;
const R, F: TRect; ID: Integer;
AInterpolationMode: Integer = InterpolationModeNearestNeighbor);
begin
...
procedure TdxCustomRibbonSkin.InternalDrawFormBorders(DC: HDC; const R: TRect;
const AIsActive, AIsRectangular: Boolean);
  ...
  begin
    ...
    if AIsRectangular then
      Result.Combine(TcxRegion.Create(R.Left + 1, R.Top + CornerRadius,
        R.Right - 1, R.Bottom - 1), roAdd);
  end;
...
procedure TdxCustomRibbonSkin.InternalDrawGlyph(DC: HDC; const R: TRect;
APart: Integer; AColor: TColor = clDefault; ANeedCorrection: Boolean = True;
AAlignment: TAlignment = taCenter);
var
...
procedure TdxCustomRibbonSkin.InternalDrawSeparator(DC: HDC; const R: TRect;
AHorizontal: Boolean; AColor1, AColor2: TColor);
var
  R1: TRect;
begin
...
procedure TdxCustomRibbonSkin.LoadCommonElements(ABitmap: GpBitmap);
  ...
  begin
    Result := AddPart3x3(ABitmap, R, F, ID, '',
      InterpolationModeHighQualityBicubic);
    Parts[Result].Scale(GetTargetDPI, dxDefaultDPI);
  end;

begin
  FQATGlassAtTopLeft[True] := LoadScaledElement(cxRectBounds(0, 353, 16, 26),
    cxRect(0, 2, 2, 2), rspQATNonClientLeft1Vista);
  FQATGlassAtTopLeft[False] := LoadScaledElement(cxRectBounds(34, 353, 4, 26),
    cxRect(3, 2, 0, 1), rspQATNonClientLeft2Vista);
  FQATGlassAtTopRight := LoadScaledElement(cxRectBounds(16, 353, 18, 26),
    cxRect(0, 7, 15, 7), rspQATNonClientRightVista);
...
begin
  FMenuGlyph := AddPart3x3(ABitmap, cxRectBounds(14, 331, 3, 4),
    Rect(1, 1, 0, 1), rspMenuGlyph);
  FMenuContent := AddPart3x3(ABitmap, cxRectBounds(18, 331, 3, 4),
    Rect(0, 1, 1, 1), rspMenuContent);
  FMenuSeparatorHorz := AddPart1x1(ABitmap, cxRectBounds(17, 337, 4, 2),
    rspMenuSeparatorHorz);
  FMenuSeparatorVert := AddPart1x1(ABitmap, cxRectBounds(14, 336, 2, 2),
    rspMenuSeparatorVert);
  FDropDownBorder := AddPart3x3(ABitmap, cxRectBounds(28, 331, 8, 8),
    Rect(3, 3, 3, 3), rspDropDownBorder);
end;
...
begin
  ...
  LoadElementParts(ABitmap, FPinButtonGlyphs, R, rspPinButtonGlyph, cxNullRect,
    [0, 1], [0, 1]);
end;

procedure TdxCustomRibbonSkin.LoadRibbonButtons(ABitmap: GpBitmap);
begin
  FApplicationMenuButton := AddPart3x3(ABitmap, cxRectBounds(0, 250, 6, 22),
    DefaultFixedSize, rspApplicationMenuButton);
  LoadElementParts(ABitmap, FEditButtons, cxRectBounds(0, 116, 12, 20),
    rspEditButtonNormal, DefaultFixedSize, [0, 1, 2, 3, 4, 5, 1],
    [DXBAR_NORMAL .. DXBAR_DROPPEDDOWN, DXBAR_ACTIVEDISABLED]);
  LoadElementParts(ABitmap, FButtonGroup, cxRectBounds(73, 0, 3, 22),
...
begin
  FScrollArrow := AddPart1x1(ABitmap, cxRectBounds(14, 245, 5, 3),
    rspScrollArrow);
  FScreenTip := AddPart3x3(ABitmap, cxRectBounds(66, 0, 6, 165),
    DefaultFixedSize, rspScreenTip);
...
procedure TdxCustomRibbonSkin.LoadRibbonGallery(ABitmap: GpBitmap);
  ...
  begin
    FDropDownGalleryBottomSizeGrip :=
      AddPart3x3(ABitmap, cxRectBounds(X, Y, WC, H), cxEmptyRect,
      rspDropDownGalleryBottomSizeGrip, '', InterpolationModeNearestNeighbor);
    FDropDownGalleryTopSizeGrip :=
      AddPart3x3(ABitmap, cxRectBounds(X + WC + 1, Y, WC, H), cxEmptyRect,
      rspDropDownGalleryTopSizeGrip, '', InterpolationModeNearestNeighbor);
    FDropDownGalleryVerticalSizeGrip :=
      AddPart3x3(ABitmap, cxRectBounds(X + 2 * (WC + 1), Y, WV, H), cxEmptyRect,
      rspDropDownGalleryVerticalSizeGrip);
  end;
...
procedure TdxCustomRibbonSkin.LoadRibbonLaunchButton(ABitmap: GpBitmap);
  ...
  begin
    LoadElementParts(ABitmap, FLaunchButtonDefaultGlyphs,
      cxRectBounds(X, Y, S, S), rspLaunchButtonDefaultGlyphNormal, cxNullRect,
      [0, 1, 0, 0, 0], [DXBAR_NORMAL, DXBAR_DISABLED, DXBAR_HOT, DXBAR_ACTIVE,
      DXBAR_PRESSED], True, InterpolationModeNearestNeighbor);
  end;
...
begin
  FApplicationMenuBorder := AddPart3x3(ABitmap, cxRectBounds(48, 321, 8, 8),
    Rect(3, 3, 3, 3), rspApplicationMenuBorder);
  FApplicationMenuContentHeader :=
    AddPart1x1(ABitmap, cxRectBounds(57, 325, 2, 14),
    rspApplicationMenuContentHeader);
  FApplicationMenuContentFooter :=
    AddPart1x1(ABitmap, cxRectBounds(62, 323, 2, 25),
    rspApplicationMenuContentFooter);
...
procedure TdxCustomRibbonSkin.LoadRibbonMenuMarks(ABitmap: GpBitmap);
  ...
  begin
    LoadElementParts(ABitmap, FMarkTruncated, cxRectBounds(X, Y, W, H),
      rspMarkTruncatedNormal, cxEmptyRect, [0, 0, 1],
      [DXBAR_NORMAL, DXBAR_HOT, DXBAR_PRESSED], True);
  end;
...
begin
  LoadElementParts(ABitmap, FQATGroupButtonActive, cxRectBounds(0, 350, 22, 22),
    ...
    [DXBAR_NORMAL, DXBAR_DISABLED, DXBAR_HOT, DXBAR_ACTIVE, DXBAR_PRESSED,
    DXBAR_DROPPEDDOWN, DXBAR_ACTIVEDISABLED]);
  LoadElementParts(ABitmap, FQATGroupButtonInactive,
    cxRectBounds(23, 350, 22, 22), rspQATGroupButtonInactive, DefaultFixedSize,
    [0, 3, 1, 1, 2, 2, 1], [DXBAR_NORMAL, DXBAR_DISABLED, DXBAR_HOT,
    DXBAR_ACTIVE, DXBAR_PRESSED, DXBAR_DROPPEDDOWN, DXBAR_ACTIVEDISABLED]);
...
procedure TdxCustomRibbonSkin.LoadRibbonQATBorders(ABitmap: GpBitmap);
  ...
  procedure LoadScaledElement(var AParts: TTwoStateArray; const R, F: TRect;
  ID: Integer);
  var
    I: Boolean;
  begin
...
begin
  ...
  LoadScaledElement(FQATAtTopLeft[True], cxRectBounds(93, 113, 15, 26),
    cxRect(13, 5, 0, 5), rspQATNonClientLeft1Active);
  LoadScaledElement(FQATAtTopLeft[False], cxRectBounds(125, 113, 7, 26),
    cxRect(2, 5, 0, 5), rspQATNonClientLeft2Active);
  LoadScaledElement(FQATAtTopRight, cxRectBounds(106, 113, 18, 26),
    cxRect(0, 5, 13, 5), rspQATNonClientRightActive);
end;

procedure TdxCustomRibbonSkin.LoadRibbonTab(ABitmap: GpBitmap);
begin
  FTabSeparator := AddPart1x1(ABitmap, cxRectBounds(42, 86, 1, 22),
    rspTabSeparator);
end;
...
procedure TdxCustomRibbonSkin.CalculateArrowPoints(R: TRect;
...
begin
  cxLookAndFeelPaintersManager.GetPainter(lfsStandard).CalculateArrowPoints(R,
    P, AArrowDirection, False, GetArrowSize);
...
procedure TdxCustomRibbonSkin.DrawArrow(DC: HDC; const R: TRect;
AArrowDirection: TcxArrowDirection; AColor: TColor);
var
  APoints: TcxArrowPoints;
  ARegion: TcxRegion;
...
procedure TdxCustomRibbonSkin.DrawPart(DC: HDC; const R: TRect;
AState: Integer);
begin
  case AState of
    DXBAR_HOT, DXBAR_CHECKED, DXBAR_HOTCHECK, DXBAR_ACTIVE:
...
procedure TdxCustomRibbonSkin.DrawPart(const AParts: TStatesArray; DC: HDC;
const R: TRect; AState: Integer);
begin
  ...
  begin
    if LowColors then
      ...
      Parts[AParts[AState]].Draw(DC, R, 255, clDefault,
        UseRightToLeftAlignment);
  end;
end;

procedure TdxCustomRibbonSkin.DrawTabAreaButton(DC: HDC; const R: TRect;
AState: TcxButtonState);
var
  APart: Integer;
begin
...
procedure TdxCustomRibbonSkin.DrawTabAreaButtonDropButtonArrowPart(DC: HDC;
const R: TRect; AState: Integer);
begin
  DrawTabAreaButton(DC, R, RibbonStateToButtonState(AState));
end;

procedure TdxCustomRibbonSkin.DrawTabAreaButtonDropButtonMainPart(DC: HDC;
const R: TRect; AState: Integer);
begin
  DrawTabAreaButton(DC, R, RibbonStateToButtonState(AState));
end;

procedure TdxCustomRibbonSkin.DrawTabAreaMarkArrow(DC: HDC; const R: TRect;
AState: Integer);
begin
  DrawMarkArrow(DC, R, AState);
end;

procedure TdxCustomRibbonSkin.DrawTabAreaMarkTruncated(DC: HDC; const R: TRect;
AState: Integer);
begin
  DrawMarkTruncated(DC, R, AState);
end;

function TdxCustomRibbonSkin.GetTabAreaButtonColorPalette(AState: Integer)
  : IdxColorPalette;
begin
...
function TdxCustomRibbonSkin.DoGetPartColor(APart: Integer;
AState: Integer = 0): TColor;
begin
  Result := DoGetPartColorCore(APart, AState);
end;

function TdxCustomRibbonSkin.DoGetPartColorCore(APart: Integer;
AState: Integer = 0): TColor;
begin
  case APart of
    ...
    DXBAR_SEPARATOR_TEXTCOLOR, DXBAR_MENUEXTRAPANE_BUTTON_TEXTCOLOR,
    ...
    rspContextTextOnGlass, rspTabHeaderText:
      case AState of
        DXBAR_NORMAL:
          Result := clBtnText;
        DXBAR_DISABLED:
...
function TdxCustomRibbonSkin.DoGetPartLowColor(APart: Integer;
AState: Integer = 0): TColor;
begin
  case APart of
    rspRibbonBottomEdge:
...
function TdxCustomRibbonSkin.GetColorPalette(ASet: IdxRibbonSkinColorPaletteSet;
AState: Integer): IdxColorPalette;
begin
  if ASet <> nil then
...
function TdxRibbonSkinsManager.Contains(const AName: string;
AStyle: TdxRibbonStyle): Boolean;
var
...
function TdxRibbonSkinsManager.GetMostSuitable(const AName: string;
AStyle: TdxRibbonStyle; ATargetDPI: Integer): TdxCustomRibbonSkin;
  ...
  begin
    ...
    begin
      ASkin := Skins[I];
      if (ASkin.Style = AStyle) and ((AName = '') or SameText(ASkin.Name, AName))
      then
      begin
        if (Result = nil) or (Abs(ASkin.TargetDPI - ATargetDPI) <
          Abs(Result.TargetDPI - ATargetDPI)) then
          Result := ASkin;
      end;
    end;
  end;
...
begin
  ...
  if (Result = nil) and Assigned(GetSkinPainterProc) and
    (GetSkinPainterProc(AName) <> nil) then
    Result := FindCore(AName);
  if Result = nil then
    Result := FindCore('');
  if Result = nil then
    Result := Skins[0]
  else if Result.TargetDPI <> ATargetDPI then
...
function TdxRibbonSkinsManager.Remove(ASkinClass
  : TdxCustomRibbonSkinClass): Boolean;
var
  I: Integer;
begin
  Result := False;
