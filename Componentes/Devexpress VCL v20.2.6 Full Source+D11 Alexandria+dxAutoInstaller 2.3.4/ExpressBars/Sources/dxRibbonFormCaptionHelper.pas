...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Messages, SysUtils, Classes, Graphics, Controls, ExtCtrls,
  Forms,
  cxGeometry, dxCore, dxMessages, cxClasses, cxGraphics, cxControls, dxBar,
  dxRibbonSkins, dxCoreClasses,
  dxGDIPlusClasses;
...
type
  ...
  TdxRibbonFormDisplayOptionsMenuItem = (domiAutoHideRibbon, domiShowTabs,
  ...
  strict private
    ...
    function GetBorderIconState(AIcon: TdxRibbonBorderIcon)
      : TdxRibbonBorderIconState;
    function GetClientCaptionBounds: TRect;
    function GetClientCaptionRegion: HRGN;
    function GetClientRect: TRect;
    function GetDrawIconFromBorderIcon(AIcon: TdxRibbonBorderIcon)
      : TdxRibbonBorderDrawIcon;
    function GetExtendedCaptionAreaRegion: HRGN;
    function GetForm: TCustomForm;
    function GetFormCaptionDrawBounds: TRect;
    function GetFormCaptionRegionsForDC(DC: HDC;
  ...
  protected
    ...
    procedure BufferedDrawCaption(ADestCanvas: TcxCanvas;
      const ACaption: TCaption);
    function CalculateBorderIconSize(ABorderIcon: TdxRibbonBorderIcon;
      ACaptionHeight: Integer): TSize;
    procedure CalculateBorderIcons; virtual;
    procedure CalculateBorderIconsForCustomSkinning
      (AStyle: TdxRibbonStyle); virtual;
    ...
    function DoHint(var ANeedDeactivate: Boolean;
      out AHintText, AShortCut: string): Boolean;
    ...
    property HotBorderIcon: TdxRibbonTrackedBorderIcon read FHotBorderIcon
      write SetHotBorderIcon;
    property PressedBorderIcon: TdxRibbonTrackedBorderIcon
      read FPressedBorderIcon write SetPressedBorderIcon;
  ...
  public
    ...
    function MouseDown(const P: TPoint; AButton: TMouseButton)
      : Boolean; virtual;
    ...
    property DisplayOptionsMenu: TdxRibbonFormDisplayOptionsMenu
      read FDisplayOptionsMenu; // for internal use
  ...
  private
    ...
    function GetIsItemEnabled
      (AItem: TdxRibbonFormDisplayOptionsMenuItem): Boolean;
    function GetIsItemSelected
      (AItem: TdxRibbonFormDisplayOptionsMenuItem): Boolean;
    function GetItemCaption(AItem: TdxRibbonFormDisplayOptionsMenuItem): string;
    function GetItemDescription
      (AItem: TdxRibbonFormDisplayOptionsMenuItem): string;
    function GetItemGlyph(AItem: TdxRibbonFormDisplayOptionsMenuItem)
      : TdxSmartGlyph;
  ...
  protected
    ...
    property ShowTabGroups: Boolean read GetShowTabGroups
      write SetShowTabGroups;
  public
...
function GetDefaultWindowBordersWidth(AWndHandle: THandle;
  AScaleFactor: TdxScaleFactor): TRect; overload;
function GetDefaultWindowNCSize(AWndHandle: THandle;
...
procedure dxCombineRectRegion(var ARegion: HRGN; const R: TRect;
  ACombineMode: Integer); overload;
procedure dxCombineRectRegion(var ARegion: HRGN;
  X0, Y0, X1, Y1, ACombineMode: Integer); overload;
...
const
  BorderIconsMap: array [TdxRibbonTrackedBorderIcon] of TdxRibbonBorderIcon =
    (rbiSystemMenu, rbiSystemMenu, rbiMinimize, rbiMaximize, rbiHelp,
...
begin
  ...
  begin
    dxAdjustWindowRectEx(Result, GetWindowLong(AWndHandle, GWL_STYLE),
      GetWindowLong(AWndHandle, GWL_EXSTYLE), False, AScaleFactor);
...
procedure TdxRibbonFormCaptionHelper.CalculateWindowCaptionHitTest
...
function TdxRibbonFormCaptionHelper.CanProcessFormCaptionHitTest
...
procedure TdxRibbonFormCaptionHelper.DrawWindowBorderIcon(ACanvas: TcxCanvas;
  const ABounds: TRect; AIcon: TdxRibbonBorderIcon;
  AState: TdxRibbonBorderIconState);
begin
  if not cxRectIsEmpty(ABounds) then
    RibbonHelper.DrawRibbonFormBorderIcon(ACanvas, ABounds,
      GetDrawIconFromBorderIcon(AIcon), AState);
end;

function TdxRibbonFormCaptionHelper.GetApplicationMenuState
  : TdxRibbonApplicationMenuState;
begin
  Result := RibbonHelper.GetApplicationMenuState;
end;

procedure TdxRibbonFormCaptionHelper.GetApplicationMenuTabOrderList
  (AList: TList);
begin
...
begin
  Result := 0;
  AForm := Form;
  if (AForm <> nil) and AForm.HandleAllocated and
    GetWindowRect(AForm.Handle, AWindowRect) then
  begin
    AWidth := cxRectWidth(AWindowRect);
    AHeight := cxRectHeight(AWindowRect);
    if UseRoundedWindowCorners then
    begin
      Result := CreateRoundRectRgn(0, 0, AWidth + 1, AHeight + 1,
        Radius, Radius);
...
begin
  ...
  begin
    Result := cxRect(0, R.Top, dxGetSystemMetrics(SM_CYSIZE, ScaleFactor) + 2,
      GetWindowCaptionHeight - 2);
...
begin
  ...
  begin
    if FormState = wsMinimized then
      ...
      Result := cxPtInRegion(FFormCaptionRegions[rfrNCHitTest],
        Control.ScreenToClient(P));
  end;
end;

function TdxRibbonFormCaptionHelper.IsRibbonHelpButtonPlacedOnCaption: Boolean;
begin
  Result := RibbonHelper.HasHelpButton and
    (RibbonHelper.GetRibbonStyle >= rs2013) and (Form <> nil) and
    not(biHelp in TCustomFormAccess(Form).BorderIcons) and
    not UseAeroNCPaint(FormPaintData);
end;
...
begin
  ...
  try
    FBitmap.Clear;
    FBitmap.cxCanvas.UseRightToLeftAlignment :=
      ADestCanvas.UseRightToLeftAlignment;
    RibbonHelper.DrawRibbonFormCaption(FBitmap.cxCanvas, FFormCaptionDrawBounds,
      ACaption);
...
begin
  ...
  begin
    ...
    begin
      ...
      RibbonHelper.DrawRibbonFormCaption(Control.Canvas, FFormCaptionDrawBounds,
        ACaption);
      DrawBorderIcons(Control.Canvas);
      RestoreDC(Control.Canvas.Handle, ASaveIndex);
      ExcludeCaptionRgn(Control.Canvas.Handle);
    end
    else if FormState = wsMinimized then
      BufferedDrawCaption(ACanvas, ACaption)
    else
    begin
      BufferedDrawCaption(Control.Canvas, ACaption);
      ExcludeCaptionRgn(Control.Canvas.Handle);
    end;
  end;
end;

procedure TdxRibbonFormCaptionHelper.GetDesignInfo(out ALoadedHeight,
  ACurrentHeight: Integer);
begin
...
begin
  ...
  begin
    ...
    begin
      DwmGetWindowAttribute(FormHandle, DWMWA_CAPTION_BUTTON_BOUNDS,
        @FBorderIconsArea, SizeOf(FBorderIconsArea));
      OffsetRect(FBorderIconsArea, -GetDefaultWindowBordersWidth(FormHandle,
        ScaleFactor).Right, 0);
    end;
  end
  else
...
procedure TdxRibbonFormCaptionHelper.CalculateBorderIconsForCustomSkinning
  ...
  begin
    ...
    begin
      R := cxRectSetRight(R, R.Right, CalculateBorderIconSize(AIcon,
        ACaptionHeight).cx);
  ...
  procedure PlaceBorderIcons(Icons: array of TdxRibbonBorderIcon; var R: TRect;
    ACaptionHeight: Integer);
  var
    I: Integer;
  begin
...
begin
  ...
  begin
    ...
    if HasSysMenu then
      FBorderIconsArea.Left := Max(BorderIconsArea.Left,
        SysMenuIconBounds.Right + 2);
  end;
  ...
  begin
    ...
    begin
      FBorderIconsArea.Right := Max(FBorderIconsArea.Right,
        GetClientRect.Right);
...
begin
  AIsToolWindow := (FormHandle = 0) or
    (FormPaintData.GetFormBorderStyle in [bsToolWindow, bsSizeToolWin]);
  if AIsToolWindow then
    Result := cxSize(dxGetSystemMetrics(SM_CXSMSIZE, ScaleFactor),
      dxGetSystemMetrics(SM_CYSMSIZE, ScaleFactor))
  else
    Result := cxSize(dxGetSystemMetrics(SM_CXSIZE, ScaleFactor),
      dxGetSystemMetrics(SM_CYSIZE, ScaleFactor));

  RibbonHelper.AdjustRibbonFormBorderIconSize
    (GetDrawIconFromBorderIcon(ABorderIcon), AIsToolWindow,
    ACaptionHeight, Result);
end;
...
begin
  ...
  begin
    ...
    AIconOffsetX := Max(2, dxGetSystemMetrics(SM_CXSIZEFRAME, ScaleFactor) -
      GetWindowBordersWidth.Left);
    AQATBounds := RibbonHelper.GetRibbonQATNonClientAreaBounds;
    if cxRectIsEmpty(AQATBounds) then
      AIconOffsetY := GetDefaultWindowBordersWidth(FormHandle, ScaleFactor).Top
        + (cxRectHeight(FSysMenuBounds) - AIconSize) div 2 - 2
    else
      AIconOffsetY := (AQATBounds.Top + AQATBounds.Bottom - AIconSize) div 2;

    FSysMenuIconBounds := cxRectBounds(AIconOffsetX, AIconOffsetY, AIconSize,
      AIconSize);
    if FormState = wsMinimized then
...
begin
  Result := (FormHandle <> 0) and
    (not IsIconic(FormHandle) or IsShowMinimizedOnDesktop(FormHandle));
end;
  ...
  begin
    ...
    try
      ...
      for I := Low(TdxRibbonBorderIcon) to High(TdxRibbonBorderIcon) do
        DrawWindowBorderIcon(ACanvas, FBorderIconBounds[I], I,
          GetBorderIconState(I));
    finally
...
procedure TdxRibbonFormCaptionHelper.DrawRibbonFormBackground(DC: HDC;
  const ARect: TRect);
begin
...
function TdxRibbonFormCaptionHelper.GetBorderIconState
  (AIcon: TdxRibbonBorderIcon): TdxRibbonBorderIconState;
const
  InactiveMap: array [Boolean] of TdxRibbonBorderIconState = (rbisInactive,
    rbisHotInactive);
  PressedMap: array [Boolean] of TdxRibbonBorderIconState = (rbisHot,
    rbisPressed);
begin
  ...
  begin
    ...
    else if AIcon = BorderIconsMap[HotBorderIcon] then
...
function TdxRibbonFormCaptionHelper.GetBorderIconAtPoint(const P: TPoint)
  : TdxRibbonTrackedBorderIcon;
const
  Map: array [TdxRibbonBorderIcon] of TdxRibbonTrackedBorderIcon =
    (tbiSystemMenu, tbiMinimize, tbiMaximize, tbiHelp, tbiDisplayOptions,
    tbiAutoHideModeShowUI);
var
  I: TdxRibbonBorderIcon;
begin
  Result := tbiNone;
...
function TdxRibbonFormCaptionHelper.GetDrawIconFromBorderIcon
  (AIcon: TdxRibbonBorderIcon): TdxRibbonBorderDrawIcon;
begin
  case AIcon of
    rbiSystemMenu:
      Result := rbdiClose;
...
begin
  Result := 0;
  if FFormCaptionRegions[ARegionKind] = 0 then
    Exit;
  ...
  OffsetRgn(Result, AViewportOrg.X - AWindowOrg.X,
    AViewportOrg.Y - AWindowOrg.Y);
end;
...
begin
  ...
  Result := not(ACheckComposition and UseAeroNCPaint(FormPaintData)) and
...
function TdxRibbonFormCaptionHelper.
  GetWindowCaptionHeightForHiddenRibbon: Integer;
begin
...
begin
  ...
  if (FormHandle <> 0) and ((FormState <> wsMaximized) or
    (Form as TdxRibbonForm).UseNavigationControlCompatibleBorders) and
    GetWindowRect(FormHandle, RW) then
  begin
    OffsetRect(RW, -RW.Left, -RW.Top);
    B := GetWindowBordersWidth;
    dxCombineRectRegion(Result, 0, 0, B.Left, GetWindowCaptionHeight, RGN_DIFF);
    // exclude left border
    dxCombineRectRegion(Result, RW.Right - B.Right, 0, RW.Right,
      GetWindowCaptionHeight, RGN_DIFF); // exclude right border
...
function TdxRibbonFormCaptionHelper.GetNCHitTestRegion: HRGN;
  ...
  procedure DoCombineRegion(AResultRegion, ASourceRegion: HRGN;
    AOperation: Integer);
  begin
...
var
  ...
  DC: HDC;
begin
  if not Valid or UseAeroNCPaint(FormPaintData) or DisplayOptionsMenu.IsPopupShowing
  then
    Exit;
  if FormState = wsMinimized then
  begin
    DC := GetDCEx(FormHandle, 0, DCX_CACHE or DCX_CLIPSIBLINGS or DCX_WINDOW or
      DCX_VALIDATE);
...
var
  ALocalPoint: TPoint;
begin
  Result := Valid and (AButton = mbLeft) and
    IsBorderIconMouseEvent(P, ALocalPoint);
  if Result then
  begin
...
begin
  ...
  case AButton of
    mbLeft:
      begin
        if IsBorderIconMouseEvent(P, ALocalPoint) and
          (PressedBorderIcon <> tbiNone) then
        begin
          Result := True;
          if GetBorderIconAtPoint(ALocalPoint) = PressedBorderIcon then
            SendMessage(FormHandle, DXM_RIBBONFORM_SYSCOMMAND,
              Ord(BorderIconsMap[PressedBorderIcon]), 0);
          PressedBorderIcon := tbiNone;
        end;
...
procedure TdxRibbonFormCaptionHelper.RightToLeftConversion
  (const ABounds: TRect);
var
  I: TdxRibbonBorderIcon;
begin
  ...
  begin
    CalculateFormCaption;
    FBorderIconsArea := TdxRightToLeftLayoutConverter.ConvertRect
      (FBorderIconsArea, ABounds);
    for I := Low(TdxRibbonBorderIcon) to High(TdxRibbonBorderIcon) do
    begin
      FBorderIconBounds[I] := TdxRightToLeftLayoutConverter.ConvertRect
        (FBorderIconBounds[I], ABounds);
      FBorderIconHitTestBounds[I] := TdxRightToLeftLayoutConverter.ConvertRect
        (FBorderIconHitTestBounds[I], ABounds);
    end;
    FSysMenuBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FSysMenuBounds, ABounds);
    FSysMenuIconBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FSysMenuIconBounds, ABounds);
...
begin
  if CanProcessFormCaptionHitTest(dxMapWindowPoint(0, Handle,
...
procedure TdxRibbonFormCaptionHelper.UpdateCaptionArea
...
procedure TdxRibbonFormCaptionHelper.UpdateWindowBorderIcons
  (const AIcons: TdxRibbonBorderIcons);
begin
...
function TdxRibbonFormCaptionHelper.CreateHintViewInfo(const AHintText,
  AShortCut: string): TdxBarCustomHintViewInfo;
begin
  Result := dxBarCreateScreenTipViewInfo(BarManager, AHintText, AShortCut, nil,
    RibbonHelper.GetBarPainter);
end;

function TdxRibbonFormCaptionHelper.DoHint(var ANeedDeactivate: Boolean;
  out AHintText, AShortCut: string): Boolean;
begin
  AHintText := '';
  AShortCut := '';
...
function TdxRibbonFormCaptionHelper.GetHintPosition(const ACursorPos: TPoint;
  AHeight: Integer): TPoint;
begin
  Result := cxPoint(ACursorPos.X, ACursorPos.Y +
    ScaleFactor.Apply(dxRibbonHintOffset));
end;

procedure TdxRibbonFormCaptionHelper.SetHotBorderIcon
  (AValue: TdxRibbonTrackedBorderIcon);
begin
...
procedure TdxRibbonFormCaptionHelper.SetPressedBorderIcon
  (AValue: TdxRibbonTrackedBorderIcon);
begin
...
constructor TdxRibbonFormDisplayOptionsMenu.Create
...
begin
  ...
  Menu.PopupEx(APopupPoint.X, APopupPoint.Y, 0, cxRectHeight(ABorderIconBounds),
    True, @ABorderIconBounds);
end;
...
begin
  ...
  FItemGlyphAutoHideRibbon.LoadFromResource(HInstance,
    'DXRIBBON_DISPLAYOPTIONS_AUTOHIDERIBBONGLYPH', 'PNG');
  FItemGlyphShowTabs := TdxSmartGlyph.Create;
  FItemGlyphShowTabs.LoadFromResource(HInstance,
    'DXRIBBON_DISPLAYOPTIONS_SHOWTABSGLYPH', 'PNG');
  FItemGlyphShowTabsAndCommands := TdxSmartGlyph.Create;
  FItemGlyphShowTabsAndCommands.LoadFromResource(HInstance,
    'DXRIBBON_DISPLAYOPTIONS_SHOWTABSANDCOMMANDSGLYPH', 'PNG');
end;

procedure TdxRibbonFormDisplayOptionsMenu.PopulateItemContent
  (AItem: TdxRibbonFormDisplayOptionsMenuItem);
var
  AButton: TdxBarButton;
begin
  AButton := BarDesignController.AddInternalItem(Menu.ItemLinks, TdxBarButton,
    GetItemCaption(AItem), SelectItem, NativeInt(AItem)).Item as TdxBarButton;
...
begin
  ...
  try
    Menu.ItemLinks.BeginUpdate;
    try
      for I := Low(TdxRibbonFormDisplayOptionsMenuItem)
        to High(TdxRibbonFormDisplayOptionsMenuItem) do
...
function TdxRibbonFormDisplayOptionsMenu.GetIsItemEnabled
  (AItem: TdxRibbonFormDisplayOptionsMenuItem): Boolean;
begin
  Result := (AItem <> domiAutoHideRibbon) or (Helper.Form as TdxRibbonForm)
    .RibbonAutoHideMode.IsAvailable;
end;

function TdxRibbonFormDisplayOptionsMenu.GetIsItemSelected
  (AItem: TdxRibbonFormDisplayOptionsMenuItem): Boolean;
begin
  Result := False;
  case AItem of
...
function TdxRibbonFormDisplayOptionsMenu.GetItemCaption
  (AItem: TdxRibbonFormDisplayOptionsMenuItem): string;
begin
  case AItem of
    domiAutoHideRibbon:
      Result := cxGetResourceString
        (@dxSBAR_RIBBONDISPLAYOPTIONS_AUTOHIDERIBBON_CAPTION);
    domiShowTabs:
      Result := cxGetResourceString
        (@dxSBAR_RIBBONDISPLAYOPTIONS_SHOWTABS_CAPTION);
    domiShowTabsAndCommands:
      Result := cxGetResourceString
        (@dxSBAR_RIBBONDISPLAYOPTIONS_SHOWTABSANDCOMMANDS_CAPTION);
  end;
end;

function TdxRibbonFormDisplayOptionsMenu.GetItemDescription
  (AItem: TdxRibbonFormDisplayOptionsMenuItem): string;
begin
  case AItem of
    domiAutoHideRibbon:
      Result := cxGetResourceString
        (@dxSBAR_RIBBONDISPLAYOPTIONS_AUTOHIDERIBBON_DESCRIPTION);
    domiShowTabs:
      Result := cxGetResourceString
        (@dxSBAR_RIBBONDISPLAYOPTIONS_SHOWTABS_DESCRIPTION);
    domiShowTabsAndCommands:
      Result := cxGetResourceString
        (@dxSBAR_RIBBONDISPLAYOPTIONS_SHOWTABSANDCOMMANDS_DESCRIPTION);
  end;
end;

function TdxRibbonFormDisplayOptionsMenu.GetItemGlyph
  (AItem: TdxRibbonFormDisplayOptionsMenuItem): TdxSmartGlyph;
begin
  Result := nil;
  case AItem of
    domiAutoHideRibbon:
