...
uses
  ...
  ExtCtrls, CheckLst, Graphics,
  dxStatusBar, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters, Menus,
  cxButtons, cxControls, cxContainer, cxEdit,
  cxCheckListBox, cxTextEdit, cxMaskEdit, cxDropDownEdit, dxLayoutContainer,
  dxLayoutcxEditAdapters,
  dxLayoutControlAdapters, cxClasses, dxLayoutControl, dxLayoutLookAndFeels,
  cxCheckBox, cxCustomListBox;
...
type
  ...
  private
    ...
    function StrToIndicatorType(const AStr: string)
...
const
  StateIndicatorTypeNames: array [TdxStatusBarStateIndicatorType]
    of string = ('sitOff', 'sitYellow', 'sitBlue', 'sitGreen', 'sitRed',
    'sitTeal', 'sitPurple');
...
function TdxStatusBarIndicatorEditor.StrToIndicatorType(const AStr: string)
  : TdxStatusBarStateIndicatorType;
begin
  for Result := Low(TdxStatusBarStateIndicatorType)
    to High(TdxStatusBarStateIndicatorType) do
  begin
...
begin
  ...
  BtnDelete.Enabled := (chlbIndicators.Items.Count > 0) and
    (chlbIndicators.ItemIndex <> -1);
  cbItemTypes.Enabled := (chlbIndicators.Items.Count > 0) and
    (chlbIndicators.ItemIndex <> -1);
  liExample.Visible := (chlbIndicators.Items.Count > 0) and
    (chlbIndicators.ItemIndex <> -1);
  SetItemType;
end;
...
begin
  if (chlbIndicators.Items.Count > 0) and (chlbIndicators.ItemIndex <> -1) then
  begin
    cbItemTypes.ItemIndex := cbItemTypes.Properties.Items.IndexOf
      (chlbIndicators.Items[chlbIndicators.ItemIndex].Text);
    LoadIndicatorBitmap(imgExample.Picture.Bitmap,
      StrToIndicatorType(cbItemTypes.Text));
  end
  else
    cbItemTypes.Text := '';
end;
...
procedure TdxStatusBarIndicatorEditor.chlbIndicatorsKeyDown(Sender: TObject;
...
begin
  if (chlbIndicators.Items.Count > 0) and (chlbIndicators.ItemIndex <> -1) then
    chlbIndicators.Items[chlbIndicators.ItemIndex].Text :=
      cbItemTypes.Properties.Items[cbItemTypes.ItemIndex]
  else
