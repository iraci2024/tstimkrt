...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Windows, Forms, Messages, Classes, SysUtils, Graphics, Controls, ExtCtrls,
  ImgList, IniFiles, Contnrs, Math, Types,
  dxCore, dxCoreClasses, dxCoreGraphics, cxClasses, cxGraphics, cxControls,
  cxContainer, cxLookAndFeels, dxGDIPlusApi,
  dxGDIPlusClasses, cxGeometry, dxBar, dxBarSkinConsts, dxRibbon, dxAnimation,
  dxBarStrs, cxAccessibility,
  dxBarAccessibility;
...
type
  ...
  private
    ...
    procedure SetHotItem(ASectorIndex: Integer;
      AInEdge: Boolean = False); overload;
    ...
    procedure SetCurrentItemLinks(AValue: TdxBarItemLinks;
      AStartIndex: Integer);
    ...
    procedure ShowAnimationStep(Sender: TdxAnimationTransition;
      var APosition: Integer; var AFinished: Boolean);
    procedure HideAnimationStep(Sender: TdxAnimationTransition;
      var APosition: Integer; var AFinished: Boolean);
  ...
  protected
    ...
    procedure DoBarMouseMove(Shift: TShiftState; const APoint: TPoint;
      AItemControl: TdxBarItemControl); override;

    procedure CalculateRects;

    // drawing
    procedure InitializeColors(ASkin: IdxSkin);
    procedure DrawRadialItemBackGround(ACanvas: TcxCanvas;
      AGPGraphics: TdxGPGraphics; AItem: TdxBarItem; ASector: TdxSector);
    procedure DrawRadialItemContent(ACanvas: TcxCanvas;
      AGPGraphics: TdxGPGraphics; AItem: TdxBarItem; ARect: TRect);
    ...
    property ExpandState: TdxRadialMenuState read FExpandState
      write SetExpandState;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  ...
  TdxRibbonRadialMenuAccessibilityHelper = class
  ...
  protected
    ...
    function GetChildSectorIndex(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper): Integer; overload;
    ...
    procedure DoClickItem(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper); override;
    procedure DoDropDownItem(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper); override;
...
function GetVisibleItemCount(AItemLinks: TdxBarItemLinks;
  ASectorCount: Integer): Integer;
begin
...
function TdxRadialMenuDropDownList.GetLinkedObjectClass
...
function ExpandRectFromCenter(ARectCenter: TdxPointF;
  ARectSize: TdxSizeF): TRect;
var
  AHalfSize: TdxSizeF;
begin
  AHalfSize := dxSizeF(ARectSize.cx / 2, ARectSize.cy / 2);
  Result := dxRectI(ARectCenter.X - AHalfSize.cx, ARectCenter.Y - AHalfSize.cy,
    ARectCenter.X + ARectSize.cx - AHalfSize.cx, ARectCenter.Y + ARectSize.cy -
    AHalfSize.cy);
end;

function dxCreateRoundRectRgn(const R: TRect; const ARoundSize: TSize): HRGN;
begin
  Result := CreateRoundRectRgn(R.Left, R.Top, R.Right + 1, R.Bottom + 1,
    ARoundSize.cx, ARoundSize.cy);
end;
...
begin
  if ItemLinks.BarControl <> nil then
    (ItemLinks.BarControl as TdxRibbonRadialMenuControl).ExpandState :=
      rmsClosed;
end;
...
begin
  ...
  if (csDesigning in ComponentState) and (Ribbon <> nil) then
    TCustomdxBarControlAccess(Result).FPainter := TdxCustomRibbonAccess(Ribbon)
      .GroupsPainter;
end;
...
procedure TdxRibbonCustomRadialMenu.InternalShowRadialMenu(X, Y: Integer;
  AMode: TdxRadialMenuMode);
  ...
  begin
    ...
    begin
      Result := Max(Result,
        cxRectWidth(cxGetTextRect(ItemLinks.BarControl.Canvas.Handle,
        AItemLinks.VisibleItems[I].Caption, 2)));
  ...
  begin
    Result := Round(GetMaximumCaptionSize(ItemLinks) /
      Sin(pi / FSectorCount)) * 2;
  ...
  begin
    ...
    if Ribbon <> nil then
      TdxCustomRibbonAccess(Ribbon).AddAffiliatedBarControlForAccessibility
        (AControl.ItemLinks.BarControl);
    ...
    ABounds := ExpandRectFromCenter(dxPointF(P), dxSizeF(AControl.FMenuSize,
      AControl.FMenuSize));
...
begin
  ...
  ACoreItem := BarDesignController.AddInternalItem
    (FInternalItemLinksOwner.ItemLinks, TdxBarButton, Owner, FInternalItems)
...
procedure TdxRibbonRadialMenuControl.DoBarMouseDown(Button: TMouseButton;
...
begin
  ...
  FSmallRingRect := cxRectInflate(FCoreRingRect,
    -ScaleFactor.Apply(SmallRingOffset));
  FFullRingRect := ClientRect;

  ASize := ScaleFactor.Apply(ThickRingSize);
  FItemContentRingRect := cxRectContent(FFullRingRect,
    cxRect(ASize, ASize, ASize, ASize));
...
function TdxRibbonRadialMenuControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonRadialMenuAccessibilityHelper;
end;

function TdxRibbonRadialMenuControl.GetViewInfoClass
...
procedure TdxRibbonRadialMenuControl.ViewStateChanged
...
procedure TdxRibbonRadialMenuControl.NavigationHandler(var ACharCode: Word;
  AShiftState: TShiftState);
begin
  // do nothing
end;

function TdxRibbonRadialMenuControl.NeedsKey(AKey: Word;
...
procedure TdxRibbonRadialMenuControl.DrawRadialItemBackGround
  (ACanvas: TcxCanvas; AGPGraphics: TdxGPGraphics; AItem: TdxBarItem;
  ASector: TdxSector);
  ...
  begin
    ...
    cxPointsOffset(APoints, dxPointF(FItemContentRingRect.Right +
      ScaleFactor.Apply(ThickRingSize) / 3, FMenuCenter.Y));
...
begin
  ...
  begin
    if (FHitTest.HitTestCode = htDropDown) and (FHitTest.Item = AItem) and
      (AItem <> nil) and AItem.Enabled then
      ARingBrushColor := FDropDownItemRingHotColor
    else
      ARingBrushColor := FDropDownItemRingNormalColor;

    ARingPenColor := FItemRingDefaultColor;
    AGPGraphics.Pie(FFullRingRect, ASector.StartAngle + 0.5,
      ASector.SweepAngle - 0.5, ARingPenColor, ARingBrushColor, 2, psSolid,
      255, 255);
    DrawDropDownArrow;

    AGPGraphics.Pie(FItemContentRingRect, ASector.StartAngle - 1,
      ASector.SweepAngle + 2, ABackgroundColor, ABackgroundColor, 2, psSolid,
      255, 255);
  end;
  ...
  begin
    ...
    AGPGraphics.Arc(cxRectInflate(FItemContentRingRect, -2, -2),
      ASector.StartAngle + 2, ASector.SweepAngle - 3, AArcColor, 2,
      psSolid, 125);
  end;

  if (FHitTest.Item = AItem) and (AItem <> nil) and AItem.Enabled and
    ((FHitTest.HitTestCode = htItem) or (FHitTest.HitTestCode = htDropDown) and
...
procedure TdxRibbonRadialMenuControl.DrawRadialItemContent(ACanvas: TcxCanvas;
  AGPGraphics: TdxGPGraphics; AItem: TdxBarItem; ARect: TRect);

  function GetCurrentImage(AItem: TdxBarItem; out ACurrentGlyph: TdxSmartGlyph;
    out ACurrentImages: TCustomImageList;
  ...
  begin
    ...
    begin
      ...
      Result := IsImageAssigned(ACurrentGlyph, ACurrentImages,
        ACurrentImageIndex);
    end;
  end;
  ...
  var
    ...
    ATextHeight: Integer;
  begin
    Result := cxRectBounds(ARect.Left, ARect.Top, cxRectWidth(ARect),
      ScaleFactor.Apply(ItemImageSize));
    Result := cxRectCenter(Result, ScaleFactor.Apply(ItemImageSize),
      ScaleFactor.Apply(ItemImageSize));

    ATextHeight := TdxTextMeasurer.TextLineHeight(ACanvas.Font);
    if FAnimationStep > AnimationTurn1 then
      AAnimationImageOffset := ATextHeight - MulDiv(ATextHeight,
        FAnimationStep - AnimationTurn1, AnimationTurn3 - AnimationTurn1)
    else
  ...
  var
    ...
    AEnabled: Boolean;
  begin
    if GetCurrentImage(AItem, ACurrentGlyph, ACurrentImages, ACurrentImageIndex)
      or GetCurrentImage(GetClickableItem(AItem), ACurrentGlyph, ACurrentImages,
      ACurrentImageIndex) then
    begin
      ATempBitmap := TcxAlphaBitmap.CreateSize(GetImageRect, True);
      try
        AEnabled := AItem.Enabled and
          ((GetClickableItem(AItem) = nil) or
          (GetClickableItem(AItem).Enabled));
        cxDrawImage(ATempBitmap.cxCanvas, ATempBitmap.ClientRect, ACurrentGlyph,
          ACurrentImages, ACurrentImageIndex, AEnabled, FColorPalette,
          ScaleFactor);
  ...
  procedure DrawText(var ARect: TRect; ATextColor: TColor; AAlpha: Integer);
    ...
    begin
      ...
      try
        ...
        try
          dxGPDrawText(AGPBitmapCanvas, AText, ABitmap.ClientRect, ACanvas.Font,
            dxColorToAlphaColor(ATextColor, AAlpha), taCenter, taAlignTop, True,
            TextRenderingHintAntiAliasGridFit, StringTrimmingEllipsisCharacter,
...
begin
  ...
  if FAnimationStep > AnimationTurn2 then
    DrawText(ARect, FItemTextColor, MulDiv(ATextAlphaMap[AItem.Enabled],
      FAnimationStep - AnimationTurn2, AnimationTurn3 - AnimationTurn2));
end;

procedure TdxRibbonRadialMenuControl.DrawRadialItems(ACanvas: TcxCanvas;
...
begin
  for I := 0 to FSectorCount - 1 do
    DrawRadialItemBackGround(ACanvas, AGPGraphics, GetVisibleItem(I),
      GetItemSector(I));
  for I := 0 to FSectorCount - 1 do
    DrawRadialItemContent(ACanvas, AGPGraphics, GetVisibleItem(I),
      GetItemRect(I));
end;
...
begin
  ...
  try
    ...
    // Background
    AGPGraphics.Ellipse(FFullRingRect, FBaseBackgroundColor,
      FBaseBackgroundColor, 1, psSolid, 255, 255);

    // Items
    AGPGraphics.Ellipse(FThickRingRect, FItemRingDefaultColor, 0,
      ScaleFactor.Apply(ThickRingSize), psSolid, 255, 0);
...
begin
  ...
  try
    ...
    // Background
    AGPGraphics.Ellipse(FSmallRingRect, FBaseBackgroundColor,
      FBaseBackgroundColor, 1, psSolid, 255, 255);

    // Adornments
    // Ring1
    AGPGraphics.Ellipse(FSmallRingRect, FBaseAccentColor, 0, 2,
      psSolid, 255, 0);
    // Ring2
    if FHitTest.HitTestCode = htCore then
      AGPGraphics.Ellipse(FCoreRingRect, FBaseAccentColor, 0, 2,
        psSolid, 200, 0);

    // Core
    if IsImageAssigned(GetCurrentCoreImage) then
    begin
      AGlyphRect := cxRectCenter(ClientRect, dxGetImageSize(GetCurrentCoreImage,
        ScaleFactor));
      ACanvas.SetClipRegion
        (TcxRegion.Create(dxCreateRoundRgn(cxRectInflate(FSmallRingRect, -2, -2)
        )), roSet);
      cxDrawImage(ACanvas, AGlyphRect, GetCurrentCoreImage, ifmNormal,
        FColorPalette, ScaleFactor);
    end;
  finally
    AGPGraphics.free;
  end;
...
procedure TdxRibbonRadialMenuControl.UpdateLayer;
  ...
  begin
    ...
    AScaleMatrix.eM21 := 0;
    AScaleMatrix.eM22 := A;
    AScaleMatrix.eDx := cxRectWidth(ABitmap.ClientRect) / 2 -
      cxRectWidth(ABitmap.ClientRect) / 2 * A;
    AScaleMatrix.eDy := cxRectWidth(ABitmap.ClientRect) / 2 -
      cxRectWidth(ABitmap.ClientRect) / 2 * A;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        cxAlphaBlend(ABasicBitmap, ARotateBitmap, ABasicBitmap.ClientRect,
          ARotateBitmap.ClientRect, False, MulDiv(255, FAnimationStep,
          AnimationTurn1));
      end
      else
...
function TdxRibbonRadialMenuControl.GetVisibleItem(ASectorIndex: Integer)
  : TdxBarItem;
var
  AIndex: Integer;
begin
  ...
  if (AIndex < FInternalVisibleLinks.Count) and
    (FInternalVisibleLinks[AIndex] <> nil) then
    Result := TdxBarItemLink(FInternalVisibleLinks[AIndex]).Item
  else
    Result := nil;
end;

procedure TdxRibbonRadialMenuControl.SetCurrentItemLinks
  (AValue: TdxBarItemLinks; AStartIndex: Integer);
var
...
procedure TdxRibbonRadialMenuControl.SetHotItem(const P: TPoint);
  ...
  begin
    ...
    begin
      ...
      if (AAngle >= ASector.StartAngle) and
        (AAngle < ASector.StartAngle + ASector.SweepAngle) or
        (AAngle >= ASector.StartAngle + 360) and
        (AAngle < ASector.StartAngle + 360 + ASector.SweepAngle) then
      begin
        Result := I;
        Break;
      end;
...
begin
  ...
  begin
    ...
    else if dxRingPtIn(FFullRingRect, P) then
...
procedure TdxRibbonRadialMenuControl.SetHotItem(ASectorIndex: Integer;
  AInEdge: Boolean = False);
var
...
begin
  ...
  if (AHitTest.HitTestCode <> FHitTest.HitTestCode) or
    (AHitTest.Item <> FHitTest.Item) or
    (AHitTest.SectorIndex <> FHitTest.SectorIndex) then
  begin
    FHitTest := AHitTest;
    UpdateLayer;
  end;
...
function TdxRibbonRadialMenuControl.GetClickableItem(AItem: TdxBarItem)
  : TdxBarItem;
var
...
function TdxRibbonRadialMenuControl.GetVisibleItemCount
  ...
  begin
    ...
    begin
      ...
      try
        ...
        try
          AGPGraphics.SmoothingMode := smAntiAlias;
          AGPGraphics.Line(Scale(2), Scale(6), Scale(18), Scale(6),
            FBackArrowColor, Scale(3), psSolid, MaxByte);
          AGPGraphics.Line(Scale(10), Scale(-2), Scale(1), Scale(7),
            FBackArrowColor, Scale(3), psSolid, MaxByte);
          AGPGraphics.Line(Scale(10), Scale(14), Scale(1), Scale(5),
            FBackArrowColor, Scale(3), psSolid, MaxByte);
        finally
          AGPGraphics.free;
        end;
...
function TdxRibbonRadialMenuControl.GetItemSector(ASectorIndex: Integer)
  : TdxSector;
var
    ...
    case FExpandState of
      rmsExpanded:
        begin
          FreeAndNil(FHideAnimation);
          FAnimationStep := 0;
          FShowAnimation := TdxAnimationTransition.Create(AnimationShowTime,
            ateLinear, AnimationShowCycle);
      ...
      rmsCollapsed:
        ...
        begin
          ...
          FHideAnimation := TdxAnimationTransition.Create(AnimationHideTime,
            ateLinear, AnimationHideCycle);
...
begin
  ...
  if Result then
  begin
    if not(AItem is TdxBarButton) or TdxBarButton(AItem).CloseSubMenuOnClick
    then
      Hide;
    AItem.Click;
  end;
...
begin
  ...
  else if InternalCloseLastLevel then
    UpdateLayer
  else
    ExpandState := rmsCollapsed;
end;
...
function TdxRibbonRadialMenuControl.InternalPopup(ASectorIndex
  : Integer): Boolean;

  function GetStartIndex(AItemLinks: TdxBarItemLinks): Integer;
  begin
    Result := ASectorIndex + GetCurrentItemLinksStartIndex -
      GetVisibleItemCount(AItemLinks) div 2;
    if Result < 0 then
...
function TdxRibbonRadialMenuControl.HandleNavigationKey(var AKey: Word)
  : Boolean;
  ...
  begin
    ...
    try
      ...
      begin
        AItem := GetVisibleItem(ASectorIndex);
        if (AItem <> nil) and (AItem.Enabled or (GetOwnedLinks(AItem) <> nil))
        then
          ANavigationList.Add(Pointer(ASectorIndex));
      end;

      ANavigationIndex := ANavigationList.IndexOf(Pointer(FHitTest.SectorIndex))
        + ADirection;
      if ANavigationIndex = -2 then
        ANavigationIndex := ANavigationList.Count - 1
      else if ANavigationIndex = ANavigationList.Count then
        ANavigationIndex := -1;
...
begin
  ...
  begin
    ...
    if (FInternalVisibleLinks.Count > 8) or (FInternalVisibleLinks.Count = 8)
      and (I < GetCurrentItemLinks.VisibleItemCount - 1) then
    begin
      AMoreButtonsLink := BarDesignController.AddInternalItem
        (FInternalItemLinksOwner.ItemLinks, TdxBarSubItem, Owner,
        FInternalItems);
      ...
      for J := I to GetCurrentItemLinks.VisibleItemCount - 1 do
        AMoreButtonsItem.ItemLinks.Add.Assign
          (GetCurrentItemLinks.VisibleItems[J]);
...
procedure TdxRibbonRadialMenuControl.ShowAnimationStep
...
procedure TdxRibbonRadialMenuControl.HideAnimationStep
  (Sender: TdxAnimationTransition; var APosition: Integer;
  var AFinished: Boolean);
begin
...
begin
  ...
  begin
    ...
    (IAccessibilityHelper.GetBarHelper as
      TdxRibbonRadialMenuAccessibilityHelper)
      .ActivateKeyTips(IAccessibilityHelper);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      (IAccessibilityHelper.GetBarHelper as
        TdxRibbonRadialMenuAccessibilityHelper)
        .ActivateKeyTips(FRadialMenu.Ribbon.IAccessibilityHelper);
    end;
  end;
end;
...
constructor TdxRibbonRadialMenuAccessibilityHelper.Create
...
function TdxRibbonRadialMenuAccessibilityHelper.AreKeyTipsSupported
...
begin
  ...
  if Result and (FKeyTipWindowsManager = nil) then
    FKeyTipWindowsManager := TdxRibbonRadialMenuKeyTipWindows.Create
      (BarControl.FRadialMenu.Ribbon);
  AKeyTipWindowsManager := FKeyTipWindowsManager;
end;

function TdxRibbonRadialMenuAccessibilityHelper.GetDefaultAccessibleObject
  : IdxBarAccessibilityHelper;
begin
  Result := Self;
end;
...
procedure TdxRibbonRadialMenuAccessibilityHelper.InitializeItemKeyTipPosition
  ...
  var AKeyTipInfo: TdxBarKeyTipInfo);
var
  ASectorIndex: Integer;
  ASector: TdxSector;
  FP: TdxPointF;
begin
  ...
  begin
    ...
    FP := dxRingPoint(dxPointF(BarControl.FMenuCenter),
      BarControl.FMenuSize / 2,
      DegToRad(ASector.StartAngle + ASector.SweepAngle / 2));
...
function TdxRibbonRadialMenuAccessibilityHelper.GetChildSectorIndex
  (AItemLinkHelper: TdxBarItemLinkAccessibilityHelper): Integer;
begin
...
function TdxRibbonRadialMenuAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
var
  ASectorIndex: Integer;
begin
  if AIndex = 0 then
    Result := TdxBarItemLinkAccess(BarControl.CoreItemLink)
      .IAccessibilityHelper.GetHelper
  else
  begin
    ASectorIndex := GetChildSectorIndex(AIndex - 1);
    Result := TdxBarItemLinkAccess(BarControl.FInternalVisibleLinks
      [ASectorIndex]).IAccessibilityHelper.GetHelper;
  end;
end;
...
function TdxRibbonRadialMenuAccessibilityHelper.GetChildIndex
  (AChild: TcxAccessibilityHelper): Integer;
begin
  if AChild = TdxBarItemLinkAccess(BarControl.CoreItemLink)
    .IAccessibilityHelper.GetHelper then
    Result := 0
  else
    Result := BarControl.FInternalVisibleLinks.IndexOf
      ((AChild as TdxBarItemLinkAccessibilityHelper).ItemLink) + 1;
end;

procedure TdxRibbonRadialMenuAccessibilityHelper.DoClickItem(AItemLinkHelper
  : TdxBarItemLinkAccessibilityHelper);
begin
  if AItemLinkHelper.ItemLink = BarControl.CoreItemLink then
  begin
    ...
    begin
      if (ActiveBarControl = BarControl) and not BarControl.IsAnimationInProcess
      then
...
procedure TdxRibbonRadialMenuAccessibilityHelper.DoDropDownItem(AItemLinkHelper
  : TdxBarItemLinkAccessibilityHelper);
var
  ASectorIndex: Integer;
begin
...
function TdxRibbonRadialMenuAccessibilityHelper.HandleNavigationKey
  (var AKey: Word): Boolean;
begin
  Result := BarControl.HandleNavigationKey(AKey);
end;

function TdxRibbonRadialMenuAccessibilityHelper.GetBarControl
  : TdxRibbonRadialMenuControl;
begin
  Result := inherited GetBarControl as TdxRibbonRadialMenuControl;
end;

procedure TdxRibbonRadialMenuAccessibilityHelper.ActivateKeyTips
  (AHelper: IdxBarAccessibilityHelper);
begin
  BarNavigationController.SetKeyTipsShowingState(nil, '');
