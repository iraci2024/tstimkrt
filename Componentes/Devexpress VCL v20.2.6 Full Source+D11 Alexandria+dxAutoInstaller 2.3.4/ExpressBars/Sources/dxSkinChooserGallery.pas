...
uses
  ...
  dxCore, dxGallery, dxRibbonGallery, cxLookAndFeelPainters, dxSkinsCore,
...
type
  ...
  public
    ...
    property LookAndFeelStyle: TcxLookAndFeelStyle read FLookAndFeelStyle
      write FLookAndFeelStyle;
    property SkinName: string read FSkinName write FSkinName;
    property SkinResInstance: HINST read FSkinResInstance
      write FSkinResInstance;
  ...
  TdxSkinChooserGalleryItem = class(TdxCustomRibbonGalleryItem,
  ...
  public
    ...
    function AddSkin(ASkinDetails: TdxSkinDetails)
      : TdxSkinChooserGalleryGroupItem; overload;
    function AddSkin(const ASkinName, AGroupName: string)
      : TdxSkinChooserGalleryGroupItem; overload;
    procedure AddSkinsFromFile(const AFileName: string);
    procedure AddSkinsFromResource(AInstance: HINST;
      const AResourceName: string);
    procedure AddSkinsFromResources(AInstance: HINST);
    procedure AddSkinsFromStream(AStream: TStream; const ASkinResName: string;
      ASkinResInstance: HINST);
    function FindSkin(const ASkinName: string;
      out AItem: TdxSkinChooserGalleryGroupItem): Boolean;
    procedure PopulateGallery;
    //
    property SelectedGroupItem: TdxSkinChooserGalleryGroupItem
      read GetSelectedGroupItem write SetSelectedGroupItem;
    property SelectedSkinName: string read GetSelectedSkinName
      write SetSelectedSkinName;
  published
    // TCustomdxBarSubItem
    property Glyph;
    property ImageIndex;
    ...
    property GalleryOptions: TdxSkinChooserGalleryOptions read GetGalleryOptions
      write SetGalleryOptions;
    property GalleryInRibbonOptions;
    property GalleryInMenuOptions;
    property ItemLinks;
    property ItemOptions;
    property SkinIconSize: TdxSkinIconSize read FSkinIconSize
      write SetSkinIconSize stored IsSkinIconSizeStored;
    property SkinIconSizeInDropDown: TdxSkinIconSize
      read FSkinIconSizeInDropDown write SetSkinIconSizeInDropDown
      stored IsSkinIconSizeInDropDownStored;
    property VisibleLookAndFeelStyles: TcxLookAndFeelStyles
      read FVisibleLookAndFeelStyles write SetVisibleLookAndFeelStyles
      default [lfsSkin];
    ...
    property OnPopup;
    property OnSkinChanged: TdxSkinChooserGallerySkinChangedEvent
      read FOnSkinChanged write FOnSkinChanged;
  end;
...
function EnumResNameProc(hModule: hModule; lpszType: LPCTSTR; lpszName: LPTSTR;
  AData: TdxSkinChooserGalleryItem): Boolean; stdcall;
begin
...
constructor TdxSkinChooserGalleryOptions.Create
...
procedure TdxSkinChooserGalleryGroupItem.ApplyToLookAndFeel
...
function TdxSkinChooserGalleryItem.AddSkin(ASkinDetails: TdxSkinDetails)
  : TdxSkinChooserGalleryGroupItem;
begin
...
function TdxSkinChooserGalleryItem.AddSkin(const ASkinName, AGroupName: string)
  : TdxSkinChooserGalleryGroupItem;
begin
  Result := GalleryCategories[GetGroupIndex(AGroupName)
    ].Items.Add as TdxSkinChooserGalleryGroupItem;
...
procedure TdxSkinChooserGalleryItem.AddSkinsFromResource(AInstance: HINST;
  const AResourceName: string);
var
  AStream: TStream;
begin
...
begin
  ...
  begin
    ...
    try
      Windows.EnumResourceNames(AInstance, PChar(sdxResourceType),
        @EnumResNameProc, LPARAM(Self));
    finally
...
procedure TdxSkinChooserGalleryItem.AddSkinsFromStream(AStream: TStream;
  const ASkinResName: string; ASkinResInstance: HINST);
var
...
function TdxSkinChooserGalleryItem.FindSkin(const ASkinName: string;
  out AItem: TdxSkinChooserGalleryGroupItem): Boolean;

  function FindSkinItemInGroup(AGroup: TdxRibbonGalleryGroup;
    out AItem: TdxSkinChooserGalleryGroupItem): Boolean;
  var
    I: Integer;
  begin
    ...
    begin
      Result := SameText(ASkinName,
        TdxSkinChooserGalleryGroupItem(AGroup.Items[I]).SkinName);
      if Result then
      begin
      ...
      try
        ...
        if Assigned(OnPopulate) then
          ...
          begin
            ...
            if (APainter.LookAndFeelStyle in VisibleLookAndFeelStyles) and
              not APainter.IsInternalPainter then
            begin
              if APainter.GetPainterDetails(ASkinDetails) then
                AddSkin(ASkinDetails).LookAndFeelStyle :=
                  APainter.LookAndFeelStyle;
            end;
          end;
      finally
        GalleryCategories.EndUpdate;
...
procedure TdxSkinChooserGalleryItem.DoGroupItemClick
...
function TdxSkinChooserGalleryItem.GetGalleryOptionsClass
...
procedure TdxSkinChooserGalleryItem.PainterChanged
  (APainter: TcxCustomLookAndFeelPainter);
begin
  PopulateGallery;
end;

function TdxSkinChooserGalleryItem.GetGalleryOptions
  : TdxSkinChooserGalleryOptions;
begin
...
begin
  Result := 0;
  while (Result < GalleryCategories.Count) and
    (GalleryCategories[Result].Header.Caption <> AName) do
    Inc(Result);
...
function TdxSkinChooserGalleryItem.GetIsInternalSkin(ASkinDetails
  : TdxSkinDetails): Boolean;
begin
  Result := ASkinDetails.GroupName = '';
end;

function TdxSkinChooserGalleryItem.GetSelectedGroupItem
  : TdxSkinChooserGalleryGroupItem;
begin
...
procedure TdxSkinChooserGalleryItem.SetGalleryOptions
  (AValue: TdxSkinChooserGalleryOptions);
begin
  GalleryOptions.Assign(AValue);
end;

procedure TdxSkinChooserGalleryItem.SetSelectedGroupItem
  (AValue: TdxSkinChooserGalleryGroupItem);
begin
...
procedure TdxSkinChooserGalleryItem.SetSkinIconSizeInDropDown
  (AValue: TdxSkinIconSize);
begin
...
procedure TdxSkinChooserGalleryItem.SetVisibleLookAndFeelStyles
  (AValue: TcxLookAndFeelStyles);
begin
