...
begin
  ...
  RegisterComponents(dxBarProductPage, [TdxRibbon, TdxRibbonPopupMenu,
    TdxBarApplicationMenu, TdxRibbonStatusBar, TdxRibbonMiniToolbar,
    TdxRibbonRadialMenu]);
  RegisterNoIcon([TdxRibbonGroupsDockControl, TdxRibbonQuickAccessGroupButton,
    TdxRibbonTab]);
  ...
  RegisterPropertyEditor(TypeInfo(string), TdxCustomRibbon, 'ColorSchemeName',
    TdxRibbonColorSchemeNameProperty);
  RegisterPropertyEditor(TypeInfo(TdxBar), TdxRibbonQuickAccessGroupButton,
    'Toolbar', TdxRibbonQuickAccessGroupButtonToolbarProperty);
  RegisterPropertyEditor(TypeInfo(TComponent), TdxRibbonApplicationButton,
    'Menu', TdxRibbonApplicationMenuProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxRibbonApplicationButton, 'Glyph',
    TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxRibbonHelpButton, 'Glyph',
    TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(TdxBar), nil, '', TdxBarProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TdxBarApplicationMenu,
    'ExtraPaneEvents', TdxExtraPaneEventEditor);

  RegisterPropertyEditor(TypeInfo(TdxRibbonContext), TdxRibbonTab, 'Context',
    TdxRibbonContextProperty);

  // ExtraPane
  HideClassProperties(TdxBarApplicationMenu, ['ExtraPaneWidthRatio',
    'ExtraPaneSize', 'ExtraPaneItems', 'ExtraPaneHeader',
    'OnExtraPaneItemClick']);

  HideClassProperties(TdxRibbonQuickAccessGroupButton,
    ['Action', 'Align', 'Caption', 'Category', 'Description', 'Hint',
    'MergeKind', 'MergeOrder', 'Style']);
...
function TdxRibbonColorSchemeNameProperty.CanAddSkin
  (ASkin: TdxCustomRibbonSkin): Boolean;
begin
  Result := (ASkin.TargetDPI = dxDefaultDPI) and
    (ASkin.Style = TdxRibbon(GetComponent(0)).Style);
...
begin
  if AIndex = 0 then
    ShowFormEditorClass(Designer, Component, Ribbon.Tabs, 'Tabs',
...
function TdxRibbonQuickAccessGroupButtonToolbarProperty.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes - [paMultiSelect];
end;

procedure TdxRibbonQuickAccessGroupButtonToolbarProperty.GetValues
  (Proc: TGetStrProc);
var
  ...
  try
    for I := 0 to ARibbon.TabCount - 1 do
      ...
      begin
        ...
        if (AToolbar <> nil) and (AList.IndexOf(AToolbar.Name) = -1) and
          AGroupButton.IsToolbarAcceptable(AToolbar) then
          AList.Add(AToolbar.Name);
      end;
    if (AGroupButton.Toolbar <> nil) and
      (AList.IndexOf(AGroupButton.Toolbar.Name) = -1) then
...
function TdxRibbonQuickAccessGroupButtonToolbarProperty.GetRibbon
  : TdxCustomRibbon;
var
  AGroupButton: TdxRibbonQuickAccessGroupButton;
begin
  ...
  if AGroupButton.LinkCount = 0 then
    ...
    Result := TdxRibbonQuickAccessToolbarBarControl
      (AGroupButton.Links[0].BarControl).Ribbon;
end;
...
begin
  ShowCollectionEditor(Designer, Component, (Component as TdxRibbonStatusBar)
    .Panels, 'Panels');
end;
...
begin
  ...
  begin
    for I := 0 to PropCount - 1 do
      if Pos(' ' + Prop.GetName + ' ', PropertiesToHide) > 0 then
        Exit;
  end;
  FProc(Prop);
end;
