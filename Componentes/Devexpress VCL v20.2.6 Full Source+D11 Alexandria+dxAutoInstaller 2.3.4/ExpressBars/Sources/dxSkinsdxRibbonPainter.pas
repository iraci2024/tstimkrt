...
{$ENDIF}
  ...
  cxClasses, dxBarSkinConsts, cxGeometry,
  dxRibbon, dxBar, cxDWMApi, dxSkinInfo, dxGDIPlusAPI, dxGDIPlusClasses,
  dxOffice11, dxCore, dxCoreClasses,
...
type
  ...
  strict private
    ...
    function GetElementTextColor(AElement: TdxSkinElement; AState: Integer;
      const APropertyPrefix: string = '';
      const ADefaultTextColor: TColor = clDefault): TColor;
  ...
  protected
    function CorrectTabGroupHeaderRect(const R: TRect;
      AIsInPopup: Boolean): TRect;
    ...
    function GetBorderBounds(ASide: TcxBorder;
      const ABorders, ABounds: TRect): TRect;
    function GetBorderIconElement(AIcon: TdxRibbonBorderDrawIcon;
      AIsToolWindow: Boolean): TdxSkinElement;
    function GetBorderSkinElement(ASide: TcxBorder; AIsRectangular: Boolean)
      : TdxSkinElement;
    function GetBordersWidth(AHasStatusBar: Boolean): TRect;
    function GetColorPalette(AElement: TdxSkinElement; AState: Integer)
      : IdxColorPalette;
    ...
    function GetElementContentIndents(AElement: TdxSkinElement;
      ...
      : Boolean;
    function GetElementMinSize(AElement: TdxSkinElement;
      AScaleFactor: TdxScaleFactor): TSize;
    function GetName: string; override;
    function GetPropertyColor(AColor: TdxSkinColor): TColor;
    function GetQATBackgroundElement(ABellow, AHasApplicationButton: Boolean)
      : TdxSkinElement;
    function GetQATLeftOffset(AHasApplicationButton: Boolean): Integer;

    function DoDrawButtonGroupElement(DC: HDC; const R: TRect; AState: Integer;
      AElement: TdxSkinElement): Boolean;
    function DoDrawEditButtonGlyph(DC: HDC; const R: TRect; AState: Integer;
      AButtonKind: TcxEditBtnKind): Boolean;
    function DoDrawStatusBarBackground(DC: HDC; const R, AVisibleArea: TRect;
      AIsRaised: Boolean): Boolean;
    function DoDrawStatusBarPart(DC: HDC; const R: TRect;
      AIsRaised, AActive, AIsLeft: Boolean): Boolean;
    function DoDrawStatusBarRectangularPart(DC: HDC; const R: TRect;
      AIsRaised, AActive, AIsLeft: Boolean): Boolean;

    function DrawElement(DC: HDC; const R: TRect; AElement: TdxSkinElement)
      : Boolean; overload;
    function DrawElement(DC: HDC; const R: TRect; AState: TdxSkinElementState;
      AElement: TdxSkinElement; AImageIndex: Integer = 0): Boolean; overload;
    function DrawElement(DC: HDC; const R: TRect; AState: Integer;
      AElement: TdxSkinElement; AImageIndex: Integer = 0): Boolean; overload;
    ...
    procedure DrawColoredElement(DC: HDC; const R: TRect; AColor: TColor;
      AElement: TdxSkinElement; AState: TdxSkinElementState);
    procedure DrawFormBorder(DC: HDC; ASide: TcxBorder;
      const ABorderWidths, R: TRect; AElement: TdxSkinElement;
      AActive: Boolean);
    procedure DrawStatusBarFormBorder(DC: HDC; const ABorders: TRect);

    procedure LoadRibbonTexturesSet(AImage: TdxGPImage); override;

    property ApplicationButtonElement: TdxSkinElement
      read GetApplicationButtonElement;
  ...
  public
    ...
    procedure DrawApplicationMenuExtraPanePinButtonGlyph(DC: HDC;
      const R: TRect; AState: Integer; AChecked: Boolean); override;
    ...
    procedure DrawDropDownGalleryTopVerticalSizeGrip(DC: HDC;
      const R: TRect); override;
    procedure DrawGalleryFilterBandBackground(DC: HDC; const R: TRect);
      override;
    ...
    procedure DrawRibbonFormBackground(DC: HDC; const R: TRect;
      ARibbonHeight: Integer); override;
    ...
    procedure DrawRibbonTopFrameArea(DC: HDC; const R: TRect;
      AUseAeroGlass: Boolean); override;
    procedure DrawRibbonTopFrameAreaSeparator(DC: HDC; const R: TRect);
      override;
    ...
    procedure DrawTab(DC: HDC; const R: TRect;
      AState: TdxRibbonTabState); override;
    ...
    procedure DrawTabGroupBackground(DC: HDC; const R: TRect; AState: Integer;
      AIsInPopup: Boolean); override;
    procedure DrawTabGroupHeaderBackground(DC: HDC; const R: TRect;
      AState: Integer; AIsInPopup: Boolean); override;
    ...
    function AreBordersThin: Boolean; override;
  ...
  protected
    procedure DrawBackstageViewBorders(DC: HDC; const R, ABordersWidth: TRect;
  ...
  public
    ...
    procedure Check(AElement: TdxSkinElement; AContextColor: TColor;
...
uses
  dxSkinsStrs, cxControls, dxShadowWindow;
...
function GetSkinElementOffsets(AElement: TdxSkinElement;
...
function DrawSkinElement(AElement: TdxSkinElement; ADC: HDC; const ARect: TRect;
  AScaleFactor: TdxScaleFactor; AImageIndex: Integer = 0;
  AState: TdxSkinElementState = esNormal; ADrawContentOnly: Boolean = False;
  ARightToLeft: Boolean = False): Boolean;
var
  ADrawRect: TRect;
begin
  Result := AElement <> nil;
  ...
  begin
    ...
    try
      ...
      if ADrawContentOnly then
        ADrawRect := cxRectContent(ARect,
          cxRectInvert(GetSkinElementOffsets(AElement, AScaleFactor)))
      else
        ADrawRect := ARect;

      AElement.RightToLeftDependentDraw(cxPaintCanvas.Handle, ADrawRect,
        AScaleFactor, ARightToLeft, AImageIndex, AState);
    finally
...
type
  ...
  TdxSkinsRibbonPainterManager = class(TcxIUnknownObject,
...
function SkinElementCheckState(AElement: TdxSkinElement; AState: Integer)
  : TdxSkinElementState;
begin
  Result := dxSkinElementCheckState(AElement,
    RibbonStateToSkinElementState(AState));
end;

{ TdxSkinRibbonPainter }

constructor TdxSkinRibbonPainter.Create(APainter: TcxCustomLookAndFeelPainter;
  ATargetDPI: Integer = dxDefaultDPI);
begin
  FPainter := APainter;
  inherited Create(ATargetDPI);
  FContextTabGroupsAreaCache :=
    TdxSkinRibbonPainterContextTabGroupsAreaCacheInfo.Create;
end;
...
function TdxSkinRibbonPainter.AdjustCaptionFontSize(ASize: Integer;
...
begin
  ...
  if AElement <> nil then
  begin
    ASize := TdxSkinElementHelper.CalculateCaptionButtonSize(ASize.cy,
      AElement);
...
function TdxSkinRibbonPainter.CorrectTabGroupRect(const R: TRect;
  AIsInPopup: Boolean): TRect;
var
...
begin
  ...
  begin
    ...
    begin
      Dec(Result.Left, AContentOffset.Left - Min(AContentOffset.Left,
        AContentOffset.Right));
      Inc(Result.Right, AContentOffset.Right - Min(AContentOffset.Left,
        AContentOffset.Right));
    end;
  end;
end;

function TdxSkinRibbonPainter.CorrectTabGroupHeaderRect(const R: TRect;
  AIsInPopup: Boolean): TRect;
begin
...
function TdxSkinRibbonPainter.CorrectTabPanelRect(const R: TRect;
  AIsInPopup: Boolean): TRect;
begin
  Result := R;
...
function TdxSkinRibbonPainter.GetApplicationButtonIndent
  (const AName: string): Integer;
begin
  if UseSkins then
    Result := ScaleFactor.Apply(SkinInfo.GetIntegerPropertyValue
      (ApplicationButtonElement, AName))
  else
    Result := 0
end;

function TdxSkinRibbonPainter.GetApplicationMenuContentOffset(const ATabsBounds
  : TRect): TRect;
begin
  if not UseSkins or (SkinInfo.RibbonApplicationBackground = nil) or
    (SkinInfo.RibbonApplicationHeaderBackground = nil) or
    (SkinInfo.RibbonApplicationFooterBackground = nil) then
    Exit(inherited GetApplicationMenuContentOffset(ATabsBounds));

  Result := cxRect(ScaleFactor.Apply(SkinInfo.RibbonApplicationBackground.
    ContentOffset.Left),
    GetElementMinSize(SkinInfo.RibbonApplicationHeaderBackground, ScaleFactor)
    .cy, ScaleFactor.Apply(SkinInfo.RibbonApplicationBackground.ContentOffset.
    Right), Max(ScaleFactor.Apply(30),
    GetElementMinSize(SkinInfo.RibbonApplicationFooterBackground,
    ScaleFactor).cy));
end;
...
function TdxSkinRibbonPainter.GetBorderBounds(ASide: TcxBorder;
  const ABorders, ABounds: TRect): TRect;
begin
  Result := ABounds;
  case ASide of
...
const
  RibbonIconsToSkinFormIcons: array [TdxRibbonBorderDrawIcon]
    of TdxSkinFormIcon = (sfiMinimize, sfiMaximize, sfiRestore, sfiClose,
    sfiHelp, sfiHelp, sfiHelp);
begin
  Result := nil;
  if UseSkins then
    case AIcon of
...
function TdxSkinRibbonPainter.GetBorderSkinElement(ASide: TcxBorder;
  AIsRectangular: Boolean): TdxSkinElement;
begin
  case ASide of
    bLeft:
  ...
  begin
    ...
    begin
      ...
      if SkinInfo.RibbonStatusBarBackground <> nil then
        Result := Max(Result,
          SkinInfo.RibbonStatusBarBackground.ContentOffset.Bottom);
    end
    else
...
function TdxSkinRibbonPainter.GetColorPalette(AElement: TdxSkinElement;
  AState: Integer): IdxColorPalette;
begin
  if AElement <> nil then
    Result := AElement.GetGlyphColorPalette
      (SkinElementCheckState(AElement, AState))
  else
    Result := nil;
end;

function TdxSkinRibbonPainter.GetCustomizeButtonOutsizeQAT
  (AHasAppButton: Boolean): Boolean;
var
...
function TdxSkinRibbonPainter.GetElementMinSize(AElement: TdxSkinElement;
  AScaleFactor: TdxScaleFactor): TSize;
begin
  Result.cx := Max(cxMarginsWidth(AElement.Image.Margins.Margin),
    Max(AElement.MinSize.Width, AElement.Size.cx));
  Result.cy := Max(cxMarginsHeight(AElement.Image.Margins.Margin),
    Max(AElement.MinSize.Height, AElement.Size.cy));
  Result := AScaleFactor.Apply(Result);
end;

function TdxSkinRibbonPainter.GetElementTextColor(AElement: TdxSkinElement;
  AState: Integer; const APropertyPrefix: string = '';
  const ADefaultTextColor: TColor = clDefault): TColor;
var
...
begin
  ...
  if AElement <> nil then
  begin
    APropertyStateName := APropertyPrefix + dxSkinElementTextColorPropertyNames
      [RibbonStateToButtonState(AState)];
    if AElement.GetPropertyByName(APropertyStateName +
      dxRibbonColorSchemeAccentNames[ColorSchemeAccent], AProperty) or
      AElement.GetPropertyByName(APropertyStateName, AProperty) then
...
function TdxSkinRibbonPainter.GetQATBackgroundElement(ABellow,
  AHasApplicationButton: Boolean): TdxSkinElement;
begin
  if not UseSkins then
    Result := nil
  else if ABellow then
    Result := SkinInfo.RibbonQuickToolbarBelow
  else
    Result := SkinInfo.RibbonQuickToolbar[AHasApplicationButton];
end;

function TdxSkinRibbonPainter.GetQATLeftOffset(AHasApplicationButton
  : Boolean): Integer;
const
  QATOffsetDelta = 10;
begin
  if UseSkins then
  begin
    Result := ScaleFactor.Apply(QATOffsetDelta) + GetApplicationButtonIndent
      (sdxRibbonAppButtonRightIndent) + ScaleFactor.Apply
      (SkinInfo.GetIntegerPropertyValue(GetQATBackgroundElement(False,
      AHasApplicationButton), sdxRibbonQuickAccessToolbarOffset));
  end
  else
    Result := 0;
end;
...
const
  dxApplicationButtonStateToElementState
    : array [TdxRibbonApplicationButtonState] of TdxSkinElementState =
    (esNormal, esHot, esPressed);
begin
  if ApplicationButtonElement <> nil then
    DrawElement(DC, R, dxApplicationButtonStateToElementState[AState],
      ApplicationButtonElement, GetActualColorSchemeAccentImageIndex)
  else
    inherited DrawApplicationButton(DC, R, AState)
end;

procedure TdxSkinRibbonPainter.DrawApplicationMenuButton(DC: HDC;
...
begin
  if not UseSkins or (SkinInfo.ButtonElements = nil) then
    ...
    SkinInfo.ButtonElements.Draw(DC, R, ScaleFactor, 0,
      ButtonState[AState = DXBAR_HOT]);
end;

procedure TdxSkinRibbonPainter.DrawApplicationMenuExtraPaneButton(DC: HDC;
  const R: TRect; AState: Integer);
begin
  if not UseSkins or (SkinInfo.RibbonExtraPaneButton = nil) then
    inherited DrawApplicationMenuExtraPaneButton(DC, R, AState)
  else
    SkinInfo.RibbonExtraPaneButton.Draw(DC, R, ScaleFactor, 0,
      RibbonStateToSkinElementState(AState));
end;
...
begin
  if not UseSkins or (SkinInfo.RibbonExtraPanePinButtonGlyph = nil) then
    inherited DrawApplicationMenuExtraPanePinButtonGlyph(DC, R,
      AState, AChecked)
  else
    SkinInfo.RibbonExtraPanePinButtonGlyph.RightToLeftDependentDraw(DC, R,
      ScaleFactor, UseRightToLeftAlignment, Ord(AChecked));
end;

procedure TdxSkinRibbonPainter.DrawApplicationMenuBackground(DC: HDC;
...
begin
  if not UseSkins or (SkinInfo.RibbonApplicationBackground = nil) or
    ...
    (SkinInfo.RibbonApplicationHeaderBackground = nil) then
  ...
  begin
    ...
    R1.Right := R.Right - SkinInfo.RibbonApplicationBackground.
      ContentOffset.Right;
...
begin
  ...
  begin
    SkinInfo.RibbonBackstageViewBackButton.RightToLeftDependentDraw(DC,
      cxRectCenter(R, ScaleFactor.Apply(SkinInfo.RibbonBackstageViewBackButton.
      MinSize.Size)), ScaleFactor, UseRightToLeftAlignment, 0,
      RibbonStateToSkinElementState(AState));
  end
  else
...
procedure TdxSkinRibbonPainter.DrawBackstageViewBackground(DC: HDC;
  const R: TRect);
var
  ALogoRect: TRect;
  AOffsets: TRect;
begin
  ...
  begin
    ...
    begin
      ALogoRect := R;
      ALogoRect.Left := ALogoRect.Right - ScaleFactor.Apply
        (SkinInfo.RibbonBackstageViewImage.Size.cx);
      ALogoRect.Top := ALogoRect.Bottom - ScaleFactor.Apply
        (SkinInfo.RibbonBackstageViewImage.Size.cy);
    ...
    begin
      ...
      try
        ...
        SkinInfo.RibbonBackstageView.Draw(DC, R, ScaleFactor,
          GetActualColorSchemeAccentImageIndex);
      finally
        cxPaintCanvas.EndPaint;
      end;
    end;
...
procedure TdxSkinRibbonPainter.DrawBackstageViewMenuBackground(DC: HDC;
  const R: TRect);
begin
...
procedure TdxSkinRibbonPainter.DrawBackstageViewMenuButton(DC: HDC;
  const R: TRect; AState: Integer);
begin
  if not DrawElement(DC, R, AState, SkinInfo.RibbonBackstageViewMenuButton,
    GetActualColorSchemeAccentImageIndex) then
    inherited DrawBackstageViewMenuButton(DC, R, AState);
end;

procedure TdxSkinRibbonPainter.DrawBackstageViewMenuHeader(DC: HDC;
  const R: TRect);
begin
...
procedure TdxSkinRibbonPainter.DrawBackstageViewMenuSeparator(DC: HDC;
  const R: TRect; AState: Integer);
begin
  if not DrawElement(DC, R, AState, SkinInfo.RibbonBackstageViewMenuSeparator)
  then
...
var
  ...
  AArrowSize: TSize;
begin
  if DrawElement(DC, R, AState, SkinInfo.RibbonBackstageViewTab,
    GetActualColorSchemeAccentImageIndex) then
  begin
    case AState of
      DXBAR_CHECKED, DXBAR_HOTCHECK, DXBAR_PRESSED:
        ...
        begin
          ...
          if UseRightToLeftAlignment then
            AArrowRect := cxRectSetLeft(AArrowRect, AArrowRect.Left,
              AArrowSize.cx)
          else
            AArrowRect := cxRectSetRight(AArrowRect, AArrowRect.Right,
              AArrowSize.cx);

          SkinInfo.RibbonBackstageViewTabArrow.RightToLeftDependentDraw(DC,
            AArrowRect, dxDefaultScaleFactor, UseRightToLeftAlignment);
        end;
    end;
  end
  else
...
function TdxSkinRibbonPainter.GetBackstageViewMenuButtonColorPalette
  (AState: Integer): IdxColorPalette;
begin
  if UseSkins then
...
procedure TdxSkinRibbonPainter.DrawBackstageViewGalleryBackground(DC: HDC;
  const R: TRect);
begin
  if UseSkins and (SkinInfo.RibbonBackstageView <> nil) then
    FillRectByColor(DC, R, SkinInfo.RibbonBackstageView.Color)
...
procedure TdxSkinRibbonPainter.DrawBackstageViewGalleryItem(DC: HDC;
  const R: TRect; AState: Integer);
begin
  if UseSkins and (SkinInfo.GalleryItem <> nil) then
    SkinInfo.GalleryItem.Draw(DC, R, ScaleFactor, 0,
      RibbonStateToSkinElementState(AState))
  else
    inherited DrawBackstageViewGalleryItem(DC, R, AState);
end;

procedure TdxSkinRibbonPainter.DrawBackstageViewGallerySeparator(DC: HDC;
  const R: TRect);
begin
  if UseSkins then
  begin
...
procedure TdxSkinRibbonPainter.DrawButtonGroup(DC: HDC; const R: TRect;
  AState: Integer);
begin
  if not DoDrawButtonGroupElement(DC, R, AState,
    SkinInfo.RibbonButtonGroupButton) then
...
begin
  if not DoDrawButtonGroupElement(DC, R, AState,
    SkinInfo.RibbonButtonGroupSplitButtonRight) then
...
begin
  if not DoDrawButtonGroupElement(DC, R, AState,
    SkinInfo.RibbonButtonGroupSplitButtonLeft) then
    inherited DrawButtonGroupDropButtonMainPart(DC, R, AState);
end;

procedure TdxSkinRibbonPainter.DrawButtonGroupBackground(DC: HDC;
  const R: TRect);
begin
  if SkinInfo.RibbonButtonGroup <> nil then
  begin
    with SkinInfo.RibbonButtonGroup.Image.Margins do
      DrawClippedElement(DC, R, cxRectInflate(R, Left, 0, Right, 0),
        SkinInfo.RibbonButtonGroup, esNormal);
  end;
end;

procedure TdxSkinRibbonPainter.DrawButtonGroupSplitButtonSeparator(DC: HDC;
  const R: TRect; AState: Integer);
begin
  if UseSkins then
...
procedure TdxSkinRibbonPainter.DrawButtonGroupBorderLeft(DC: HDC;
  const R: TRect);
var
  ARect: TRect;
begin
...
procedure TdxSkinRibbonPainter.DrawButtonGroupBorderRight(DC: HDC;
  const R: TRect);
var
  ARect: TRect;
begin
  ...
  begin
    ...
    begin
      ...
      DrawClippedElement(DC, R, ARect, SkinInfo.RibbonButtonGroup,
        esNormal, roSet);
    end;
  end
  else
...
procedure TdxSkinRibbonPainter.DrawButtonGroupBorderMiddle(DC: HDC;
  const R: TRect; AState: Integer);
begin
  if UseSkins and (SkinInfo.RibbonButtonGroupSeparator <> nil) then
    SkinInfo.RibbonButtonGroupSeparator.Draw(DC, R, dxDefaultScaleFactor, 0,
      RibbonStateToSkinElementState(AState))
  else
...
begin
  if UseSkins and (SkinInfo.RibbonCollapsedToolBarBackground <> nil) then
    SkinInfo.RibbonCollapsedToolBarBackground.Draw(DC, R, ScaleFactor, 0,
      RibbonStateToSkinElementState(AState))
  else
...
begin
  ...
  begin
    if SkinInfo.RibbonContextualTabHeader.ImageCount > 1 then
      DrawColoredElement(DC, R, AContextColor,
        SkinInfo.RibbonContextualTabHeader, RibbonTabStatesMap[AState])
    else
    begin
      ARect := CorrectTabHeaderRect(R);
      dxGpFillRect(DC, ARect, AContextColor);
      SkinInfo.RibbonContextualTabHeader.Draw(DC, ARect, ScaleFactor, 0,
        RibbonTabStatesMap[AState]);
    end;
  end
  else
...
begin
  ...
  begin
    FContextTabGroupsAreaCache.Check(SkinInfo.RibbonContextualTabPanel,
      AContextColor, R, ScaleFactor);
    cxBitBlt(DC, FContextTabGroupsAreaCache.Canvas.Handle, R,
      cxNullPoint, SRCCOPY);
  end
  else
    inherited DrawContextTabGroupsArea(DC, R, AContextColor, AIsQATAtBottom,
      AIsInPopup)
end;

procedure TdxSkinRibbonPainter.DrawContextBackground(DC: HDC; const R: TRect;
...
begin
  ...
  begin
    ...
    try
      ARect := R;
      Inc(ARect.Bottom);
      IntersectClipRect(DC, ARect.Left, ARect.Top + 1, ARect.Right,
        ARect.Bottom);
      if SkinInfo.RibbonContextualTabLabel.ImageCount > 1 then
        DrawColoredElement(DC, ARect, AContextColor,
          SkinInfo.RibbonContextualTabLabel, esNormal)
      else
      begin
...
procedure TdxSkinRibbonPainter.DrawContextBackgroundGlass(DC: HDC;
...
begin
  ...
  begin
    if SkinInfo.RibbonContextualTabLabelOnGlass.ImageCount > 1 then
      DrawColoredElement(DC, R, AContextColor,
        SkinInfo.RibbonContextualTabLabelOnGlass, esNormal)
    else
    begin
      dxGpFillRectByGradient(DC, R, 0, AContextColor,
        LinearGradientModeVertical, 0, 220);
...
procedure TdxSkinRibbonPainter.DrawEditArrowButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
...
begin
  ...
  begin
    cxPaintCanvas.BeginPaint(DC);
    Painter.DrawScaledEditorButton(cxPaintCanvas, R, cxbkEditorBtn,
      RibbonStateToButtonState(AState), ScaleFactor);
    cxPaintCanvas.EndPaint;
  end
  else
...
procedure TdxSkinRibbonPainter.DrawEditEllipsisButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
...
procedure TdxSkinRibbonPainter.DrawEditSpinDownButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
...
procedure TdxSkinRibbonPainter.DrawEditSpinUpButton(DC: HDC; const R: TRect;
  AState: Integer);
begin
...
function TdxSkinRibbonPainter.DrawElement(DC: HDC; const R: TRect;
  AState: TdxSkinElementState; AElement: TdxSkinElement;
  AImageIndex: Integer = 0): Boolean;
begin
  Result := UseSkins and (AElement <> nil);
  if Result then
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor,
      UseRightToLeftAlignment, AImageIndex, AState);
end;
...
begin
  ...
  if Result then
    AElement.RightToLeftDependentDraw(DC, R, ScaleFactor,
      UseRightToLeftAlignment, AImageIndex,
      SkinElementCheckState(AElement, AState));
end;

procedure TdxSkinRibbonPainter.DrawProgressDiscreteBand(DC: HDC;
  const R: TRect);
var
  AElement: TdxSkinElement;
  ARect: TRect;
...
begin
  ...
  begin
    ...
    begin
      ...
      ARect := cxRectContent(ARect, SkinInfo.ProgressBarElements[False,
        False].ContentOffset.Rect);
    end;
    DrawClippedElement(DC, R, ARect, AElement, esNormal);
  end;
end;
...
begin
  ...
  begin
    ...
    SkinInfo.RibbonGallerySizeGrips.RightToLeftDependentDraw(DC, ARect,
      ScaleFactor, UseRightToLeftAlignment, 1);
  end
  else
...
procedure TdxSkinRibbonPainter.DrawDropDownGalleryBottomVerticalSizeGrip
...
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment then
      ARect := TdxRightToLeftLayoutConverter.ConvertRect(ARect, R);
    SkinInfo.RibbonGallerySizeGrips.RightToLeftDependentDraw(DC, ARect,
      ScaleFactor, UseRightToLeftAlignment, 2);
  end
  else
...
procedure TdxSkinRibbonPainter.DrawGalleryGroupItemSelectionFrame(DC: HDC;
...
begin
  ...
  begin
    ...
    DrawClippedElement(DC, R, R1, SkinInfo.RibbonGalleryPane,
      RibbonStateToSkinElementState(AState));
  end
  else
...
procedure TdxSkinRibbonPainter.DrawInRibbonGalleryScrollBarDropDownTouchButton
...
procedure TdxSkinRibbonPainter.DrawInRibbonGalleryScrollBarBackground(DC: HDC;
  const R: TRect; AState: Integer);
var
  R1: TRect;
  AElement: TdxSkinElement;
...
begin
  ...
  begin
    ...
    try
      ...
      AElement.Draw(cxPaintCanvas.Handle, ASource, ScaleFactor,
        AImageIndex, AState);
    finally
      cxPaintCanvas.EndPaint;
    end;
...
begin
  ...
  try
    AElement.Draw(ABitmap.Canvas.Handle, ABitmap.ClientRect, ScaleFactor,
      0, AState);
...
function TdxSkinRibbonPainter.DoDrawButtonGroupElement(DC: HDC; const R: TRect;
  AState: Integer; AElement: TdxSkinElement): Boolean;
begin
  Result := UseSkins and (AElement <> nil);
  if Result then
  begin
...
function TdxSkinRibbonPainter.DoDrawEditButtonGlyph(DC: HDC; const R: TRect;
  AState: Integer; AButtonKind: TcxEditBtnKind): Boolean;
const
...
begin
  ...
  begin
    cxPaintCanvas.BeginPaint(DC);
    try
      Painter.DrawScaledEditorButtonGlyph(cxPaintCanvas, R, AButtonKind,
        ButtonState[AState], ScaleFactor);
    finally
      cxPaintCanvas.EndPaint;
    end;
  end;
...
function TdxSkinRibbonPainter.DoDrawStatusBarBackground(DC: HDC;
  const R, AVisibleArea: TRect; AIsRaised: Boolean): Boolean;
var
  AElement: TdxSkinElement;
  AIndents: TRect;
begin
  ...
  begin
    ...
    if AIsRaised and AElement.IsAlphaUsed then
      DrawClippedElement(DC, AVisibleArea, cxRectInflate(R, AIndents), AElement,
        esNormal, roIntersect, 0);
    DrawClippedElement(DC, AVisibleArea, cxRectInflate(R, AIndents), AElement,
      esNormal, roIntersect, Integer(AIsRaised));
  end;
end;

function TdxSkinRibbonPainter.DoDrawStatusBarPart(DC: HDC; const R: TRect;
  AIsRaised, AActive, AIsLeft: Boolean): Boolean;
  ...
  begin
    ...
    if AIsLeft xor UseRightToLeftAlignment then
      Result.Right := Max(Result.Left + AMinSize.cx,
        Result.Right + AElement.Image.Margins.Right)
    else
      Result.Left := Min(Result.Right - AMinSize.cx,
        Result.Left - AElement.Image.Margins.Left);
  end;

const
  StateMap: array [Boolean] of TdxSkinElementState = (esActiveDisabled,
    esActive);
begin
  ...
  begin
    ...
    try
      ...
      SkinInfo.RibbonStatusBarBackground.RightToLeftDependentDraw
        (cxPaintCanvas.Handle,
        CalculateDestRect(SkinInfo.RibbonStatusBarBackground), ScaleFactor,
        UseRightToLeftAlignment, Integer(AIsRaised and not AIsLeft),
        StateMap[AActive])
    finally
      cxPaintCanvas.EndPaint;
    end;
...
function TdxSkinRibbonPainter.DoDrawStatusBarRectangularPart(DC: HDC;
  const R: TRect; AIsRaised, AActive, AIsLeft: Boolean): Boolean;
const
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        cxPaintCanvas.IntersectClipRect(R);
        DrawFormBorder(DC, SideMap[AIsLeft], cxRect(cxRectWidth(R), 0,
          cxRectWidth(R), 0), cxRect(R.Left, R.Top - AElement.Image.Margins.Top,
          R.Right, R.Bottom), AElement, AActive);
      finally
        cxPaintCanvas.EndPaint;
      end;
    end;
...
procedure TdxSkinRibbonPainter.DrawFormBorders(DC: HDC;
  ...
  var
    ...
    AElement: TdxSkinElement;
  begin
    AElement := GetBorderSkinElement(ASide,
    ...
    begin
      ...
      try
        ...
        AElement.Draw(ABitmap.Canvas.Handle, R, ScaleFactor,
          Integer(not FormPaintData.GetIsActive));
  ...
  begin
    if (ASide = bBottom) and PaintData.HasStatusBar then
      ...
      DrawFormBorder(DC, ASide, ABordersWidth, FormPaintData.GetBounds,
        GetBorderSkinElement(ASide, IsRectangularFormBottom(FormPaintData)),
        FormPaintData.GetIsActive);
  end;
...
begin
  ...
  begin
    cxPaintCanvas.BeginPaint(DC);
    try
      cxPaintCanvas.ExcludeClipRect(cxRectContent(FormPaintData.GetBounds,
        ABordersWidth));
...
begin
  ...
  begin
    ...
    try
      ...
      if UseRightToLeftAlignment then
        ABorderRect := TdxRightToLeftLayoutConverter.ConvertRect
          (ABorderRect, R);
      IntersectClipRect(DC, ABorderRect.Left, ABorderRect.Top,
        ABorderRect.Right, ABorderRect.Bottom);
      DrawElement(DC, ABorderRect, RibbonFormBorderStates[AActive], AElement,
        Integer(not AActive));
    finally
      RestoreDC(DC, ASaveIndex);
    end;
...
procedure TdxSkinRibbonPainter.DrawStatusBarFormBorder(DC: HDC;
  const ABorders: TRect);
var
  AOffset: Integer;
  R1, R2: TRect;
begin
  ...
  begin
    ...
    if SkinInfo.RibbonStatusBarBackground <> nil then
    begin
      AOffset := Ord(SkinInfo.RibbonStatusBarBackground.Borders.Bottom.
        ContentMargin = 0);
      if ABorders.Bottom > AOffset then
      begin
        R1 := cxRectSetTop(FormPaintData.GetBounds,
          FormPaintData.GetBounds.Bottom - ABorders.Bottom - AOffset,
          ABorders.Bottom);
        R2 := R1;
        Dec(R2.Top, SkinInfo.RibbonStatusBarBackground.Size.cy -
          ABorders.Bottom);
        DrawClippedElement(DC, R1, R2, SkinInfo.RibbonStatusBarBackground,
          esNormal);
      end;
    end;
    DrawFormBorder(DC, bBottom, Rect(ABorders.Left, 0, ABorders.Right, AOffset),
      FormPaintData.GetBounds, GetBorderSkinElement(bBottom,
      IsRectangularFormBottom(FormPaintData)), FormPaintData.GetIsActive);
  end;
end;

procedure TdxSkinRibbonPainter.DrawStatusBarGripBackground(DC: HDC;
  const R: TRect);
var
  AElement: TdxSkinElement;
...
begin
  ...
  else if not AElement.IsAlphaUsed then
  begin
    GetElementContentIndents(AElement, False, ALeftIndent, ARightIndent);
    R1 := GetBordersWidth(True);
    R1.Top := 0;
    R1.Left := ALeftIndent;
    DrawClippedElement(DC, R, cxRectInflate(R, R1), AElement, esNormal,
      roIntersect, 1);
  end;
end;
...
const
  RibbonIconStateToSkinElementState: array [TdxRibbonBorderIconState]
    of TdxSkinElementState = (esNormal, esHot, esPressed, esActive, esHot);
var
  AElement: TdxSkinElement;
begin
  ...
  if AElement <> nil then
    AElement.Draw(DC, cxRectInflate(R, -ScaleFactor.Apply(1)), ScaleFactor,
      Ord((PaintData.GetApplicationMenuState = ramsShownAsFullScreenFrame) and
      UseSkins), RibbonIconStateToSkinElementState[AState])
  else
...
begin
  ...
  begin
    AElement.UseCache := True;
    if UseExternalBorders then
      DrawSkinElement(AElement, DC, R, ScaleFactor,
        Byte(not FormPaintData.GetIsActive),
        StateMap[FormPaintData.GetIsActive], True, UseRightToLeftAlignment)
    else
      AElement.Draw(DC, R, ScaleFactor, Byte(not FormPaintData.GetIsActive),
        StateMap[FormPaintData.GetIsActive]);
  end
  else
...
begin
  if AIsRectangular and UseRoundedWindowCorners then
    AHandled := DoDrawStatusBarRectangularPart(DC, R, AIsRaised,
      AIsActive, AIsLeft)
  else
    AHandled := DoDrawStatusBarPart(DC, R, AIsRaised, AIsActive, AIsLeft);

  if not AHandled then
    inherited DrawFormStatusBarPart(DC, R, AIsLeft, AIsActive, AIsRaised,
      AIsRectangular);
end;
...
begin
  ...
  begin
    ...
    SkinInfo.RibbonTabPanelGroupButton.RightToLeftDependentDraw(DC, ARect,
      ScaleFactor, UseRightToLeftAlignment, 0, StateMap[AState]);
  end
  else
...
procedure TdxSkinRibbonPainter.DrawGroupScrollButtonGlyph(DC: HDC;
  const R: TRect; ALeft: Boolean);
begin
  // do nothing
end;

procedure TdxSkinRibbonPainter.DrawItemSeparator(DC: HDC; const R: TRect;
  AHorizontal: Boolean);
begin
  if UseSkins and (SkinInfo.RibbonTabGroupItemsSeparator <> nil) then
  begin
    SkinInfo.RibbonTabGroupItemsSeparator.Draw(DC, cxRectCenterHorizontally(R,
      SkinInfo.RibbonTabGroupItemsSeparator.MinSize.Width),
...
begin
  ...
  if AElement = nil then
    ...
    AElement.Draw(DC, cxRectInflate(R, -1), ScaleFactor, 0,
      ButtonStateToSkinElementState[AState]);
end;
...
procedure TdxSkinRibbonPainter.DrawMenuExtraSeparator(DC: HDC; const R: TRect;
  AHorizontal: Boolean);
var
  AColor: TdxSkinColor;
  R1: TRect;
...
function TdxSkinRibbonPainter.GetMenuColorPalette(AState: Integer)
  : IdxColorPalette;
begin
  if UseSkins then
...
begin
  if UseSkins and (SkinInfo.RibbonFormCaption <> nil) then
    DrawSkinElement(SkinInfo.RibbonFormCaption, DC, R, ScaleFactor, 0, esNormal,
      True, UseRightToLeftAlignment)
  else
    inherited DrawRibbonClientTopArea(DC, R);
end;

procedure TdxSkinRibbonPainter.DrawRibbonFormBackground(DC: HDC; const R: TRect;
...
procedure TdxSkinRibbonPainter.DrawRibbonTopFrameArea(DC: HDC; const R: TRect;
  AUseAeroGlass: Boolean);
begin
  if AUseAeroGlass and not DrawElement(DC, R,
    SkinInfo.RibbonHeaderBackgroundOnGlass) then
    inherited DrawRibbonTopFrameArea(DC, R, AUseAeroGlass)
end;

procedure TdxSkinRibbonPainter.DrawRibbonTopFrameAreaSeparator(DC: HDC;
  const R: TRect);
begin
  if not UseSkins then
...
begin
  ...
  begin
    AElement.UseCache := True;
    AElement.Draw(DC, R, ScaleFactor, 0, dxSkinElementCheckState(AElement,
      RibbonStateToSkinElementState(AState)));
  end;
end;
...
procedure TdxSkinRibbonPainter.DrawSmallButtonDropButtonMainPart(DC: HDC;
  const R: TRect; AState: Integer);
begin
...
function TdxSkinRibbonPainter.GetSmallButtonColorPalette(AState: Integer)
  : IdxColorPalette;
begin
  if UseSkins then
...
begin
  ...
  begin
    R1 := cxRectSetBottom(R, R.Bottom,
      ScaleFactor.Apply(SkinInfo.SizeGrip.MinSize.Height));
    if UseRightToLeftAlignment then
      R1 := cxRectSetLeft(R1, R1.Left + ScaleFactor.Apply(2),
        ScaleFactor.Apply(SkinInfo.SizeGrip.MinSize.Width))
    else
      R1 := cxRectSetRight(R1, R1.Right - ScaleFactor.Apply(2),
        ScaleFactor.Apply(SkinInfo.SizeGrip.MinSize.Width));

    SkinInfo.SizeGrip.RightToLeftDependentDraw(DC, R1, ScaleFactor,
      UseRightToLeftAlignment);
  end
  else
...
procedure TdxSkinRibbonPainter.DrawStatusBarPanelSeparator(DC: HDC;
  const R: TRect);
begin
...
procedure TdxSkinRibbonPainter.DrawStatusBarToolbarSeparator(DC: HDC;
  const R: TRect);
var
  R1: TRect;
begin
...
function TdxSkinRibbonPainter.GetStatusBarPanelColorPalette(AState: Integer)
  : IdxColorPalette;
begin
  if UseSkins then
...
begin
  if UseSkins and (SkinInfo.RibbonTab <> nil) then
    SkinInfo.RibbonTab.Draw(DC, CorrectTabHeaderRect(R), ScaleFactor, 0,
      RibbonTabStatesMap[AState])
  else
...
begin
  ...
  if AElement = nil then
    inherited DrawTabAreaBackground(DC, R, AActive, AUseAeroGlass,
      AApplicationMenuState)
  else if not(AUseAeroGlass and ExtendCaptionAreaOnTabs) then
  begin
...
procedure TdxSkinRibbonPainter.DrawTabGroupHeaderBackground(DC: HDC;
...
begin
  ...
  begin
    ...
    SkinInfo.RibbonTabPanel.Draw(DC, CorrectTabPanelRect(R, AIsInPopup),
      ScaleFactor);
  end
  else
...
begin
  if UseSkins and (SkinInfo.RibbonSmallButton <> nil) then
    SkinInfo.RibbonSmallButton.Draw(DC, R, ScaleFactor, 0,
      RibbonStateToSkinElementState(AState))
  else
...
begin
  ...
  begin
    ABitmap := TcxBitmap32.CreateSize(R, True);
    try
      SkinInfo.RibbonTabSeparatorLine.Draw(ABitmap.Canvas.Handle,
        ABitmap.ClientRect, dxDefaultScaleFactor);
...
function TdxSkinRibbonPainter.GetTabAreaButtonColorPalette(AState: Integer)
  : IdxColorPalette;
begin
  if UseSkins then
...
procedure TdxSkinRibbonPainter.DrawMiniToolbarBackground(DC: HDC;
  const R: TRect);
var
  AElement: TdxSkinElement;
  ASaveIndex: Integer;
begin
  ...
  begin
    ...
    try
      ...
      AElement.Draw(DC, cxRectInflate(R, AElement.Image.Margins.Margin),
        ScaleFactor);
    finally
      RestoreDC(DC, ASaveIndex)
    end;
...
function TdxSkinRibbonPainter.GetMiniToolbarColorPalette(AState: Integer)
  : IdxColorPalette;
begin
...
procedure TdxSkinRibbonPainter.DrawMarkTruncated(DC: HDC; const R: TRect;
  AState: Integer);
var
  AElement: TdxSkinElement;
begin
...
procedure TdxSkinRibbonPainter.DrawQuickAccessToolbar(DC: HDC; const R: TRect;
  ...
  begin
    Result := ANonClientDraw and IsCompositionEnabled and
      not(ADontUseAero or ABellow);
  end;

  function NeedDrawCustomQuickAccessToolBar(AElement: TdxSkinElement): Boolean;
  begin
    Result := (AElement = nil) or IsAeroBackgroundUsed and
      not AElement.Image.Empty;
  end;
  ...
  begin
    ...
    begin
      if AParent = nil then
        ...
        begin
          ARightIndent := GetQuickAccessToolbarRightIndent
            (AHasApplicationButton);
      ...
      begin
        ...
        Dec(ARightIndent, GetQuickAccessToolbarMarkButtonOffset
          (AHasApplicationButton, ABellow));
      end;
...
begin
  ...
  else if not IsAeroBackgroundUsed then
  begin
...
begin
  ...
  begin
    ...
    AElement.Draw(DC, cxRectContent(R,
      ScaleFactor.Apply(ABackground.ContentOffset.Rect)), ScaleFactor, 0,
      RibbonStateToSkinElementState(AState));
  end;
end;

procedure TdxSkinRibbonPainter.DrawQuickAccessToolbarPopup(DC: HDC;
  const R: TRect);
begin
...
function TdxSkinRibbonPainter.GetQuickAccessToolbarColorPalette(AState: Integer;
...
begin
  if UseSkins and (SkinInfo.PopupMenuSeparator <> nil) then
    Result := Max(SkinInfo.PopupMenuSeparator.Size.cy,
      SkinInfo.PopupMenuSeparator.MinSize.Height)
  else
    Result := inherited GetMenuSeparatorSize;
end;

function TdxSkinRibbonPainter.DoGetPartColor(APart: Integer;
    ...
    case APart of
      ...
      DXBAR_MENUARROWSEPARATOR:
        Result := cxGetActualColor
          (SkinInfo.PopupMenuSplitButton2.TextColor, clNone);
      ...
      DXBAR_BACKSTAGEVIEW_TEXTCOLOR:
        ...
        begin
          ...
          Result := GetElementTextColor(SkinInfo.RibbonBackstageView, AState,
            '', Result);
        end;
      ...
      rspContextTabHeaderText:
        begin
          if AState = DXBAR_HOT then
            ...
            Result := SkinInfo.RibbonContextualTabHeaderText
              [AState = DXBAR_ACTIVE];
      ...
      rspContextText:
        Result := GetElementTextColor
          (SkinInfo.RibbonContextualTabLabel, AState);
      rspContextTextOnGlass:
        Result := GetElementTextColor
          (SkinInfo.RibbonContextualTabLabelOnGlass, AState);
      rspContextTextShadow:
        Result := GetPropertyColor
          (SkinInfo.RibbonContextualTabLabelShadowColor);
      rspContextTextOnGlassShadow:
        begin
          Result := GetPropertyColor
            (SkinInfo.RibbonContextualTabLabelOnGlassShadowColor);
          if Result = clDefault then
      ...
      rspApplicationButton:
        Result := GetElementTextColor
          (SkinInfo.RibbonApplicationButton2010, AState);
      rspStatusBarText:
        case AState of
          DXBAR_NORMAL, DXBAR_HOT, DXBAR_HOTCHECK, DXBAR_DISABLED:
            Result := GetElementTextColor
              (SkinInfo.RibbonStatusBarButton, AState);
          DXBAR_CHECKED:
            begin
      ...
      DXBAR_GALLERYGROUPHEADERTEXT, DXBAR_GALLERYFILTERBANDTEXT:
        Result := GetElementTextColor
          (SkinInfo.RibbonGalleryGroupCaption, AState);
...
function TdxSkinRibbonPainter.GetPartContentOffsets(APart: Integer): TRect;
  ...
  function GetElementForPart(APart: Integer;
  ...
  begin
    ...
    begin
      if APart = DXBAR_APPLICATIONMENUBUTTON then
        Result.Right := Max(Result.Top,
          ScaleFactor.Apply(AElement.ContentOffset.Right)) -
          ScaleFactor.Apply(AElement.ContentOffset.Right)
      else
...
begin
  ...
  begin
    case APart of
      ...
      DXBAR_BACKSTAGEVIEW_MENUBAR_TAB:
        Result := Max(Result,
          GetElementSize(SkinInfo.RibbonBackstageViewTab).cy);
      DXBAR_BACKSTAGEVIEW_MENUBAR_ITEM:
        Result := Max(Result,
          GetElementSize(SkinInfo.RibbonBackstageViewMenuButton).cy);
      ...
      DXBAR_TOOLBAR, DXBAR_TOOLBARINPOPUP:
        Result := ScaleFactor.Apply
          (GetValue(SkinInfo.RibbonSpaceBetweenTabGroups));
      ...
      rspTabGroupBottomOffset:
        if SkinInfo.RibbonTabGroupHeader <> nil then
          Result := ScaleFactor.Apply
            (SkinInfo.RibbonTabGroupHeader.ContentOffset.Bottom);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if SkinInfo.RibbonQuickToolbar[AHasApplicationButton] <> nil then
      Inc(Result, ScaleFactor.Apply(SkinInfo.RibbonQuickToolbar
        [AHasApplicationButton].ContentOffset.Left));
...
begin
  if UseSkins then
    AOffsetProperty := SkinInfo.RibbonQATIndentBeforeCustomizeButton
      [AHasApplicationButton]
  else
    AOffsetProperty := nil;

  if ABelow or (AOffsetProperty = nil) then
    Result := inherited GetQuickAccessToolbarMarkButtonOffset
      (AHasApplicationButton, ABelow)
  else
...
begin
  if not AUseAeroGlass and UseSkins then
  begin
    Result := GetQuickAccessToolbarLeftIndent(AHasApplicationButton,
      AUseAeroGlass) + QATLeftDefaultOffset;
    Dec(Result, GetQATLeftOffset(AHasApplicationButton));
  end
  else
    Result := inherited GetQuickAccessToolbarOverrideWidth
      (AHasApplicationButton, AUseAeroGlass);
end;

function TdxSkinRibbonPainter.GetQuickAccessToolbarRightIndent
  (AHasApplicationButton: Boolean): Integer;
begin
  if UseSkins and (SkinInfo.RibbonQuickToolbar[AHasApplicationButton] <> nil)
  then
    Result := ScaleFactor.Apply(SkinInfo.RibbonQuickToolbar
      [AHasApplicationButton].ContentOffset.Right)
  else
...
function TdxSkinRibbonPainter.GetWindowBordersWidth(AHasStatusBar
  : Boolean): TRect;
begin
  if UseSkins then
...
function TdxSkinRibbonPainter.AreBordersThin: Boolean;
begin
  Result := cxMarginsWidth(GetBordersWidth(True)) <= 2 *
    dxShadowWindowThinBorderSize;
end;
...
procedure TdxSkinRibbon2010Painter.AdjustBackstageViewTabButtonFont
  (AFont: TFont);
begin
...
function TdxSkinRibbon2010Painter.GetApplicationMenuContentOffset
  (const ATabsBounds: TRect): TRect;
begin
...
begin
  if UseSkins and (APart = DXBAR_APPLICATIONBUTTON) then
    Result := cxRect(GetApplicationButtonIndent(sdxRibbonAppButtonLeftIndent) +
      1, 2, GetApplicationButtonIndent(sdxRibbonAppButtonRightIndent), 0)
  else
    Result := inherited GetPartContentOffsets(APart);
end;
...
procedure TdxSkinRibbon2013Painter.DrawBackstageViewMenuBackground(DC: HDC;
  const R: TRect);
var
  R1: TRect;
begin
...
procedure TdxSkinRibbon2013Painter.DrawBackstageViewMenuHeader(DC: HDC;
  const R: TRect);
var
  R1: TRect;
begin
...
procedure TdxSkinRibbon2013Painter.AdjustBackstageViewTabButtonFont
  (AFont: TFont);
begin
  // do nothing
end;

procedure TdxSkinRibbon2013Painter.DrawBackstageViewBorders(DC: HDC;
  const R, ABordersWidth: TRect; AIsActive: Boolean);
var
  ASaveIndex: Integer;
  AThinBorders: TRect;
  R1: TRect;
...
procedure TdxSkinRibbon2013Painter.DrawFormBorders(DC: HDC;
  const BordersWidth: TRect);
var
  ABordersWidth, R1: TRect;
  ASaveIndex: Integer;
begin
  if (PaintData.GetApplicationMenuState = ramsShownAsFullScreenFrame) and UseSkins
  then
  begin
    ASaveIndex := SaveDC(DC);
    try
      ...
      DrawBackstageViewBorders(DC, FormPaintData.GetBounds, ABordersWidth,
        FormPaintData.GetIsActive);
    finally
      RestoreDC(DC, ASaveIndex);
    end;
...
begin
  if (PaintData.GetApplicationMenuState = ramsShownAsFullScreenFrame) and UseSkins
  then
    DrawBackstageViewBorders(DC, R, cxRect(0, cxRectHeight(R), 0, 0),
      FormPaintData.GetIsActive)
  else
    inherited DrawFormCaption(DC, R);
end;
...
function TdxSkinRibbon2013Painter.GetBackstageViewBorderColor
  (AIsActive: Boolean): TColor;
  ...
  begin
    ...
    if AElement.CanUseAlternateImageSet(0, RibbonFormBorderStates[AIsActive],
      False, AAttributes) then
    begin
    ...
    else if not AElement.Image.Empty then
    begin
      ...
      try
        ...
        ABackgroundLuminance := dxGetColorRelativeLuminance
          (SkinInfo.RibbonBackstageView.Color);
        if Abs(dxGetColorRelativeLuminance(AColor1) - ABackgroundLuminance) >
          Abs(dxGetColorRelativeLuminance(AColor2) - ABackgroundLuminance) then
          Result := AColor1
        else
          Result := AColor2;
      finally
...
procedure TdxSkinRibbon2016TabletPainter.DrawCollapsedToolbarGlyphBackground
  (DC: HDC; const R: TRect; AState: Integer);
begin
  if LowColors then
    inherited DrawCollapsedToolbarGlyphBackground(DC, R, AState);
...
procedure TdxSkinRibbon2019Painter.DrawBackstageViewBackButton(DC: HDC;
  const R: TRect; AState: Integer);
var
  AGlyphRect: TRect;
  AGlyphSize: TSize;
begin
  ...
  begin
    ...
    AGlyphSize := ScaleFactor.Apply
      (SkinInfo.RibbonBackstageViewBackButton.MinSize.Size);
    ...
    AGlyphRect := cxRectOffset(AGlyphRect,
      GetPartContentOffsets(DXBAR_BACKSTAGEVIEW_MENUBAR_TAB).Left, 0);
    if UseRightToLeftAlignment then
      AGlyphRect := TdxRightToLeftLayoutConverter.ConvertRect(AGlyphRect, R);

    SkinInfo.RibbonBackstageViewBackButton.RightToLeftDependentDraw(DC,
      AGlyphRect, ScaleFactor, UseRightToLeftAlignment, 0,
      RibbonStateToSkinElementState(AState));
  end
  else
    inherited;
...
procedure TdxSkinsRibbonPainterManager.AddSkin
...
procedure TdxSkinsRibbonPainterManager.PainterAdded
  (APainter: TcxCustomLookAndFeelPainter);
begin
  AddSkin(APainter);
end;

procedure TdxSkinsRibbonPainterManager.PainterRemoved
  (APainter: TcxCustomLookAndFeelPainter);
var
  ASkin: TdxCustomRibbonSkin;
  I: Integer;
begin
  ...
  begin
    ASkin := dxRibbonSkinsManager.Skins[I];
    if (ASkin is TdxSkinRibbonPainter) and
      (TdxSkinRibbonPainter(ASkin).Painter = APainter) then
      dxRibbonSkinsManager.Remove(ASkin);
  end;
end;
...
procedure TdxSkinRibbonPainterContextTabGroupsAreaCacheInfo.Check
  (AElement: TdxSkinElement; AContextColor: TColor; const R: TRect;
  AScaleFactor: TdxScaleFactor);
var
  ABitmap: TcxBitmap32;
begin
  if (AElement <> Element) or (ContextColor <> AContextColor) or
    (ImageCount <> AElement.ImageCount) or (cxRectWidth(R) <> Width) or
    (cxRectHeight(R) <> Height) then
  begin
    SetSize(R);
    Element := AElement;
    ...
    begin
      ...
      try
        Element.Draw(ABitmap.Canvas.Handle, ABitmap.ClientRect,
          AScaleFactor, 1);
...
dxUnitsLoader.AddUnit(@TdxSkinsRibbonPainterManager.Register,
