...
uses
  ...
  SysUtils, Classes, Graphics, ImgList, dxBar, dxCoreReg, cxLibraryReg,
  dxBarReg,
...
function TdxRibbonGalleryFilterGroupsProperty.GetAttributes
  : TPropertyAttributes;
begin
  Result := [paDialog, paReadOnly, paValueEditable];
end;

function TdxRibbonGalleryFilterGroupsProperty.FilterCategory
...
procedure TdxRibbonGalleryGroupAssignedValuesProperty.GetProperties
  (Proc: TGetPropProc);
begin
  FProc := Proc;
...
begin
  for I := 0 to PropCount - 1 do
    if SameText(Prop.GetName, 'avSpaceBetweenItems') or
      SameText(Prop.GetName, 'avItemPullHighLighting') then
      Exit;
  FProc(Prop);
end;
...
begin
  FDesigner := ShowFormEditorClass(Designer,
    GetComponent(0) as TdxRibbonGalleryItem, TfrmGalleryDesigner);
end;

function TdxRibbonGalleryDesignerPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TdxRibbonGalleryGroups), TdxRibbonGalleryItem,
    'GalleryCategories', TdxRibbonGalleryDesignerPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TBitmap), TdxRibbonGalleryGroupItem, 'Glyph',
    TcxBitmapProperty);
  ...
  HideClassProperties(TdxRibbonGalleryOptions, ['CanCollapse', 'Collapsed',
    'ItemSizeInRibbon', 'MinColumnCount', 'ItemPullHighlighting', 'ItemSize',
    'ItemTextKind', 'EqualItemSizeInAllGroups', 'RowCount', 'ShowScrollBar',
    'SubMenuResizing', 'ShowItemHint']);
