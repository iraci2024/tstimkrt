...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Windows, Forms, Messages, SysUtils, Graphics, Controls, ExtCtrls, ImgList,
  IniFiles, Contnrs,
  Generics.Collections, Generics.Defaults, Classes,
  dxCore, dxCoreClasses, dxMessages, cxClasses, cxGraphics, dxTouch, cxControls,
  dxBar, cxContainer, cxLookAndFeels,
  dxScreenTip, dxBarSkin, dxFading, cxAccessibility, dxBarAccessibility,
  cxLookAndFeelPainters, dxRibbonSkins, dxForms,
  dxRibbonForm, dxRibbonFormCaptionHelper, dxBarApplicationMenu, dxShadowWindow,
  cxScrollBar, dxGdiPlusClasses,
  cxGeometry, dxCoreGraphics, dxAnimation, dxFluentDesignFormInterfaces;
...
type
  TdxRibbonMergeOption = (rmoCanCreateNewTab, rmoCanCreateNewGroup,
    rmoCanCreateQATToolbar, rmoCanCreateTabAreaToolbar,
    rmoCanCreateTabAreaSearchToolbar);
  TdxRibbonMergeOptions = set of TdxRibbonMergeOption;

  TdxRibbonPopupMenuItem = (rpmiItems, rpmiMoreCommands, rpmiQATPosition,
    rpmiQATAddRemoveItem, rpmiMinimizeRibbon, rpmiCustomizeRibbon,
    rpmiCustomizeQAT);
  TdxRibbonPopupMenuItems = set of TdxRibbonPopupMenuItem;

const
  dxRibbonDefaultMergeOptions = [rmoCanCreateNewTab, rmoCanCreateNewGroup,
...
type
  ...
  TdxRibbonGroupsDockControlViewInfoClass = class of
  ...
  TdxRibbonApplicationMenuDisplayMode = (amdmPopup, amdmFrame,
    amdmFrameFullScreen);
  ...
  TdxRibbonCustomizationFormMode = (rcfmCustomizeRibbon,
    rcfmCustomizeQuickAccessToolbar);
  TdxRibbonShowCustomizationFormFunc = function(ARibbon: TdxCustomRibbon;
    const AMode: TdxRibbonCustomizationFormMode;
    ACustomizationFormClass: TFormClass { TdxRibbonCustomizationForm } =
    nil): Boolean;

  TdxRibbonAddAccessibilityHelperProc = reference to procedure
    (AAccessibilityHelper: IdxBarAccessibilityHelper);
  ...
  IdxRibbonApplicationMenu = interface
    ...
    function Popup(ARibbon: TdxCustomRibbon;
      var AClosedByEscape: Boolean): Boolean;
  ...
  private
    ...
    procedure ListChangeHandler(Sender: TObject; AComponent: TComponent;
  ...
  TdxCustomDesignSelectionHelper = class(TInterfacedObject,
  ...
  protected
    ...
    procedure DeleteSelection(var AReference: IdxBarSelectableItem;
      ADestruction: Boolean);
    procedure ExecuteCustomizationAction(ABasicAction
    ...
    procedure SelectComponent(ASelectionOperation
  ...
  public
    ...
    destructor Destroy; override;
    procedure AdjustApplicationButtonFont(AFont: TFont;
      AState: TdxRibbonApplicationButtonState);
    procedure AdjustContextFont(AFont: TFont; AContextColor: TColor);
    procedure AdjustTabHeaderFont(AFont: TFont; AState: Integer;
      AContext: TdxRibbonContext = nil);
    ...
    procedure DrawRibbonFormBorderIcon(ACanvas: TcxCanvas; const ABounds: TRect;
      ...
      AState: TdxRibbonBorderIconState); virtual;
    procedure DrawRibbonFormBorders(ACanvas: TcxCanvas;
      const ABordersWidth: TRect); virtual;
    ...
    // client routines
    procedure DrawApplicationButton(ACanvas: TcxCanvas; const ABounds: TRect;
      AState: TdxRibbonApplicationButtonState); virtual;
    procedure DrawApplicationButtonGlyph(ACanvas: TcxCanvas;
      const ABounds: TRect; AGlyph: TdxSmartGlyph); virtual;
    procedure DrawBackground(ACanvas: TcxCanvas; const ABounds: TRect); virtual;
    procedure DrawBackgroundImage(ACanvas: TcxCanvas;
      const ABounds: TRect); virtual;
    procedure DrawContextGroupsArea(ACanvas: TcxCanvas; const ABounds: TRect;
      AIsQuickAccessToolbarAtBottom, AIsInPopup: Boolean); virtual;
    procedure DrawGlyph(ACanvas: TcxCanvas; AGlyph: TdxSmartGlyph;
      const ABounds: TRect); virtual;
    procedure DrawGlowingText(DC: HDC; const AText: string; AFont: TFont;
      const ABounds: TRect; AColor: TColor; AFlags: DWORD;
    ...
    procedure DrawGroupsArea(ACanvas: TcxCanvas; const ABounds: TRect;
      ...
      AIsInPopup: Boolean = False); virtual;
    ...
    procedure DrawRibbonFormCaptionText(ACanvas: TcxCanvas;
      const ABounds: TRect; const ADocumentName, ACaption: string); virtual;
    procedure DrawRibbonGlassFormCaptionText(ACanvas: TcxCanvas;
    ...
    procedure DrawTabAreaBackground(ACanvas: TcxCanvas;
      const ABounds: TRect); virtual;
    ...
    procedure DrawTabScrollButton(ACanvas: TcxCanvas; const ABounds: TRect;
      ALeft: Boolean; AState: TcxButtonState); virtual;
    procedure DrawTabScrollButtonGlyph(ACanvas: TcxCanvas; const ABounds: TRect;
      ALeft: Boolean); virtual;
    procedure DrawHelpButton(ACanvas: TcxCanvas; const ABounds: TRect;
      AState: TcxButtonState); virtual;
    procedure DrawHelpButtonGlyph(ACanvas: TcxCanvas; AGlyph: TdxSmartGlyph;
      const ABounds: TRect; AState: TcxButtonState); virtual;
    ...
    procedure DrawMDIButtonGlyph(ACanvas: TcxCanvas; const ABounds: TRect;
      AButton: TdxBarMDIButton; AState: TcxButtonState); virtual;
    procedure DrawMinimizeButton(ACanvas: TcxCanvas; const ABounds: TRect;
      AState: TcxButtonState; AMinimized: Boolean);
    ...
    function GetGroupCaptionBottomOffset(AInPopup: Boolean = False)
      : Integer; virtual;
    function GetTabContentOffset: TRect;

    property ColorScheme: TdxCustomRibbonSkin read GetColorScheme;
    // for internal use
    property ColorSchemeAccent: TdxRibbonColorSchemeAccent
      read GetColorSchemeAccent;
    property IsFormZoomed: Boolean read GetIsFormZoomed;
    property QuickAccessToolbarViewInfo: TdxRibbonQuickAccessToolbarViewInfo
      read GetQuickAccessToolbarViewInfo; // for internal use
  ...
  TdxRibbonHitTest = (rhtNone, rhtTab, rhtApplicationMenu, rhtContext,
    rhtTabScrollLeft, rhtTabScrollRight, rhtGroupScrollLeft,
    rhtGroupScrollRight, rhtHelpButton, rhtCustomButton, rhtFormButton);
  // keep order
  ...
  public
    ...
    property HitObjectAsButton: TdxRibbonCustomButtonViewInfo
      read GetHitObjectAsButton; // for internal use
  ...
  public
    ...
    procedure PopulateHorizontalNavigationList
      (AProc: TdxRibbonAddAccessibilityHelperProc); virtual;
  ...
  protected
    ...
    function GetBarManager: TdxBarManager; override;
    function GetNextAccessibleObject(ADirection
      : TcxAccessibilityNavigationDirection)
      : IdxBarAccessibilityHelper; override;
    ...
    function GetState(AChildID: TcxAccessibleSimpleChildElementID)
      : Integer; override;
    ...
    property Ribbon: TdxCustomRibbon read GetRibbon;
  public
    constructor CreateEx(AButtonViewInfo
      : TdxRibbonCustomButtonViewInfo); virtual;
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID)
      : TRect; override;
  end;
  ...
  TdxRibbonCustomButtonViewInfo = class(TcxIUnknownObject, IdxFadingObject,
  ...
  protected
    function CreateAccessibilityHelper
      : TdxRibbonCustomButtonAccessibilityHelper; virtual;
    function GetHint: string; virtual;
    function GetIsEnabled: Boolean; virtual;
    procedure CalculateState; virtual;
    procedure DrawButton(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState); virtual;
    procedure DrawButtonGlyph(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState); virtual;
    // IdxFadingObject
  ...
  public
    ...
    function GetHitInfo(const AHitInfo: TdxRibbonHitInfo): Boolean; virtual;
    //
    property AccessibilityHelper: IdxBarAccessibilityHelper
      read GetAccessibilityHelper;
  ...
  protected
    ...
    procedure CalculateWidths(ACanvas: TcxCanvas;
      AViewInfo: TdxRibbonViewInfo); virtual;
    procedure DoRightToLeftConversion(const ABounds: TRect); virtual;
  ...
  public
    constructor Create(ATab: TdxRibbonTab;
      AOwner: TdxRibbonTabsViewInfo); virtual;
  ...
  private
    ...
    procedure PopulateMenuItemContent(AItemLinks: TdxBarItemLinks;
      ASource: TdxRibbonTab);
  ...
  protected
    ...
    function CreateTabViewInfo(ATab: TdxRibbonTab)
      : TdxRibbonTabViewInfo; virtual;
  ...
  public
    ...
    procedure PopulateHorizontalNavigationList
      (AProc: TdxRibbonAddAccessibilityHelperProc); override;
    ...
    property Items[Index: Integer]: TdxRibbonTabViewInfo read GetTabViewInfo;
  ...
  public
    ...
    property DropDownButton: TdxRibbonTabsDropDownButtonViewInfo
  ...
  strict private
    ...
    procedure HandlerAnimate(Sender: TdxAnimationTransition;
  ...
  protected
    ...
    function CreateTabBaseAnimation(const AOldRect, ANewRect: TRect)
      : TdxRectAnimationTransition; virtual;
    procedure DoRightToLeftConversion(const ABounds: TRect); override;
  ...
  public
    ...
    procedure PopulateHorizontalNavigationList
      (AProc: TdxRibbonAddAccessibilityHelperProc); override;
    //
    property SelectedButton: TdxRibbonCustomButtonViewInfo
      read GetSelectedButton; // for internal use
    property Items[Index: Integer]: TdxRibbonCustomButtonViewInfo read GetItem;
    default; // for internal use
  end;
  ...
  strict private
    ...
    procedure InternalDrawText(ACanvas: TcxCanvas; const R: TRect;
      ATextColor: TColor; AHasGlowing: Boolean);
  protected
    FBounds: TRect;
    FNeedShowHint: Boolean;
    FTextBounds: TRect;
    ...
    function CalculateTextBounds(ACanvas: TcxCanvas; AIndent: Integer)
      : TRect; virtual;
    procedure DoRightToLeftConversion(const ABounds: TRect); virtual;
  ...
  public
    constructor Create(AOwner: TdxRibbonContextsViewInfo;
      AContext: TdxRibbonContext); virtual;
  ...
  protected
    function CreateContextViewInfo(AContext: TdxRibbonContext)
      : TdxRibbonContextViewInfo; virtual;
    procedure DoRightToLeftConversion(const ABounds: TRect); override;
  public
    procedure Calculate(const ABounds: TRect); override;
    function CalculateCaptionWidth(ACanvas: TcxCanvas;
    ...
    //
    property Items[Index: Integer]: TdxRibbonContextViewInfo read GetItem;
    default; // for internal use
  end;
  ...
  protected
    function CreateContextViewInfo(AContext: TdxRibbonContext)
      : TdxRibbonContextViewInfo; override;
  end;
  ...
  TdxRibbonApplicationButtonViewInfo = class(TdxRibbonCustomViewInfo,
    IdxFadingObject, IdxFadingObjectFadingOptions)
  strict private
    FIsHot: Boolean;
    FIsPressed: Boolean;
  ...
  public
    ...
    procedure PopulateHorizontalNavigationList
      (AProc: TdxRibbonAddAccessibilityHelperProc); override;
    //
    property ApplicationButton: TdxRibbonApplicationButton
      read GetApplicationButton;
    property Bounds: TRect read FBounds;
  ...
  TdxRibbonApplicationButtonViewInfo2010OrNewer = class
    (TdxRibbonApplicationButtonViewInfo)
  strict private
  ...
  protected
    ...
    function CalculateToolbarSize(ABar: TdxBar;
  ...
  public
    ...
    //
    property ButtonsViewInfo: TdxRibbonTabsAreaButtonsViewInfo
  ...
  strict private
    function GetApplicationButtonViewInfo
      : TdxRibbonApplicationButtonViewInfo2010OrNewer;
  protected
    procedure DoRightToLeftConversion(const ABounds: TRect); override;
    function MeasureHeight(ACanvas: TcxCanvas): Integer; override;
  public
    procedure Calculate(const ABounds: TRect); override;
    //
    property ApplicationButtonViewInfo
      : TdxRibbonApplicationButtonViewInfo2010OrNewer
      read GetApplicationButtonViewInfo;
  end;
  ...
  TdxRibbonTabsAreaViewInfo2016Tablet = class
    (TdxRibbonTabsAreaViewInfo2010OrNewer)
  strict private
    function GetQuickAccessToolbarViewInfo: TdxRibbonQuickAccessToolbarViewInfo;
  protected
  ...
  public
    property QuickAccessToolbarViewInfo: TdxRibbonQuickAccessToolbarViewInfo
      read GetQuickAccessToolbarViewInfo;
  end;
  ...
  public
    ...
    property DockControl: TdxRibbonQuickAccessToolbarDockControl
      read GetDockControl; // for internal use
  ...
  TdxRibbonQuickAccessToolbarViewInfo2016Tablet = class
    (TdxRibbonQuickAccessToolbarViewInfo)
  protected
  ...
  protected
    ...
    function CreateApplicationButtonViewInfo
      : TdxRibbonApplicationButtonViewInfo; virtual;
    function CreateContextsViewInfo: TdxRibbonContextsViewInfo; virtual;
    function CreateQuickAccessToolbarViewInfo
      : TdxRibbonQuickAccessToolbarViewInfo; virtual;
    ...
    function GetRibbonFormCaptionTextBoundsWithContext(const ABounds,
      ACenterRect: TRect; ATextWidth: Integer): TRect; virtual;
    ...
    function GetTabGroupsHeight(AIgnoreHidden: Boolean = False)
      : Integer; virtual;

    // MDI support
    function HasMDIButtons: Boolean;
    function IsMDIButtonEnabled(AButton: TdxBarMDIButton;
    ...
    procedure UpdateToolbarDockControl(ADockControl: TdxCustomRibbonDockControl;
      AIsVisible: Boolean; const ABounds: TRect);
  ...
  public
    ...
    property ApplicationButtonViewInfo: TdxRibbonApplicationButtonViewInfo
      read FApplicationButtonViewInfo; // for internal use
    ...
    // for internal use
    property GroupsDockControlSiteBounds: TRect
      read FGroupsDockControlSiteBounds;
    property GroupsDockControlSiteViewInfo
      : TdxRibbonGroupsDockControlSiteViewInfo
      read FGroupsDockControlSiteViewInfo; // for internal use
    property QuickAccessToolbarViewInfo: TdxRibbonQuickAccessToolbarViewInfo
      read FQuickAccessToolbarViewInfo; // for internal use
  ...
  protected
    function CreateApplicationButtonViewInfo
      : TdxRibbonApplicationButtonViewInfo; override;
    function CreateContextsViewInfo: TdxRibbonContextsViewInfo; override;
  ...
  protected
    ...
    function CreateQuickAccessToolbarViewInfo
      : TdxRibbonQuickAccessToolbarViewInfo; override;
    function CreateTabsAreaViewInfo: TdxRibbonTabsAreaViewInfo; override;
  ...
  TdxRibbonBarControlAccessibilityHelper = class
  ...
  TdxRibbonBarButtonLikeControlDrawParams = class
  ...
  strict private
    ...
    function GetCollapsedGroupGlyphBackgroundSize
  ...
  protected
    ...
    function CalculateRowHeight(AIconSize, ATextHeight: Integer;
      AScaleFactor: TdxScaleFactor): Integer; virtual;
    procedure DrawCollapsedToolbarBackgroundPart(ABarControl
      : TdxRibbonGroupBarControl; ACanvas: TcxCanvas; AGroupState: Integer);
    procedure DrawCollapsedToolbarContentPart(ABarControl
      : TdxRibbonGroupBarControl; ACanvas: TcxCanvas; AGroupState: Integer);
    procedure DrawToolbarContentPart(ABarControl: TdxBarControl;
      ACanvas: TcxCanvas); override;
    procedure DrawToolbarNonContentPart(ABarControl: TdxBarControl;
      DC: HDC); override;
    function GetButtonPartState(const ADrawParams
      : TdxBarButtonLikeControlDrawParams; AControlPart: Integer)
      : Integer; override;
    function GetCollapsedGroupCaptionRect(const AGroupRect: TRect)
      : TRect; virtual;
    function GetCollapsedGroupWidth(ABarControl: TdxRibbonGroupBarControl)
      : Integer; virtual;
    function GetGroupCaptionHeight: Integer; overload;
    function GetGroupCaptionHeight(ATextHeight: Integer): Integer;
      overload; virtual;
    function GetGroupMinWidth(ABarControl: TdxRibbonGroupBarControl)
      : Integer; virtual;
    function GetSmallIconSize: Integer; overload;
    function SubMenuControlUseSingleMenuWindowMode: Boolean; override;
    //
    property DrawParams: TdxRibbonBarButtonLikeControlDrawParams
      read FDrawParams;
  public
    ...
    function DropDownGalleryGetScrollBarWidth(AControlHeight: Integer;
      const AControlMargins: TRect): Integer; override;
    function DropDownGalleryGetSizingBandHeight(AIconSize, ATextSize: Integer;
    ...
    function GetToolbarContentOffsets(ABar: TdxBar;
      ADockingStyle: TdxBarDockingStyle; AScaleFactor: TdxScaleFactor;
      AHasSizeGrip: Boolean): TRect; override;
  ...
  protected
    ...
    procedure FillBackground(DC: HDC; const ADestR, ASourceR: TRect;
      ABrush: HBRUSH; AColor: TColor); override;
  ...
  IdxRibbonGroupLayoutCalculator = interface
    ...
    function CollapseMultiColumnItemControls(AGroupViewInfo
      : IdxRibbonGroupViewInfo): Boolean;
    function DecreaseMultiColumnItemControlsColumnCount(AGroupViewInfo
      : IdxRibbonGroupViewInfo): Boolean;
  ...
  TdxRibbonGroupsDockControlContentChangeAnimation = class
  ...
  public
    constructor Create(ADockControl: TdxCustomRibbonDockControl;
      ATime: Cardinal;
      ATransitionEffect: TdxAnimationTransitionEffect = ateLinear); reintroduce;
    destructor Destroy; override;
  ...
  strict private
    ...
    //
    procedure WMGestureNotify(var Message: TWMGestureNotify);
  ...
  protected
    ...
    procedure CalcRowToolbarPositions(ARowIndex: Integer;
    ...
    function GetAccessibilityHelperClass
      : TdxBarAccessibilityHelperClass; override;
    ...
    property Ribbon: TdxCustomRibbon read GetRibbon;
    property SiteViewInfo: TdxRibbonGroupsDockControlSiteViewInfo
      read GetSiteViewInfo;
  ...
  private
    ...
    function GetGroupViewInfo(AIndex: Integer)
  ...
  protected
    ...
    procedure DoRightToLeftConversion(const ABounds: TRect); virtual;
    procedure InternalScrollGroups(ADelta: Integer;
      AMaxContentWidth: Integer); virtual;
  public
    ...
    procedure RightToLeftConversion(const ABounds: TRect);
    procedure ScrollGroups(AScrollLeft: Boolean;
      AMaxContentWidth: Integer); virtual;
    ...
    property GroupViewInfos[AIndex: Integer]: TdxRibbonGroupBarControlViewInfo
      read GetGroupViewInfo;
    property ScrollButtons: TdxRibbonScrollButtons read FScrollButtons;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    //
    property AllowShowHideAnimation: Boolean read FAllowShowHideAnimation
      write FAllowShowHideAnimation;
  ...
  protected
    ...
    function NotHandleMouseMove(ACheckLastMousePos: Boolean = True)
      : Boolean; override;
    ...
    //
    property QuickAccessToolbar: TdxRibbonQuickAccessToolbar
      read GetQuickAccessToolbar;
  public
    property Ribbon: TdxCustomRibbon read GetRibbon;
  end;
  ...
  protected
    ...
    procedure DoPaintItem(AControl: TdxBarItemControl; ACanvas: TcxCanvas;
      const AItemRect: TRect); override;
    procedure InitializeForDock(ABar: TdxBar); override;
    function GetClientOffset: TRect; virtual;
    function GetItemControlDefaultViewLevel(AItemControl: TdxBarItemControl)
      : TdxBarItemViewLevel; override;
    ...
    function GetSizeForWidth(AStyle: TdxBarDockingStyle; AWidth: Integer)
      : TSize; override;
  ...
  protected
    ...
    procedure DrawBarMarkState(ABarControl: TdxBarControl; DC: HDC;
      const R: TRect; AState: TdxBarMarkState); override;
    procedure DrawButtonBackground(const ADrawParams
      : TdxBarButtonLikeControlDrawParams); override;
    procedure DrawToolbarContentPart(ABarControl: TdxBarControl;
      ACanvas: TcxCanvas); override;
    // Skin Consts
  ...
  public
    function BarMarkRect(ABarControl: TdxBarControl): TRect; override;
    function BarMarkItemRect(ABarControl: TdxBarControl): TRect; override;
    procedure ComboControlDrawArrowButton(const ADrawParams
      : TdxBarEditLikeControlDrawParams; ARect: TRect;
      AInClientArea: Boolean); override;
    function GetButtonBorderHeight(AScaleFactor: TdxScaleFactor)
      : Integer; override;
    function GetButtonBorderWidth(AScaleFactor: TdxScaleFactor)
      : Integer; override;
    function GetToolbarContentOffsets(ABar: TdxBar;
      ADockingStyle: TdxBarDockingStyle; AScaleFactor: TdxScaleFactor;
      AHasSizeGrip: Boolean): TRect; override;
  ...
  TdxRibbonQuickAccessToolbarBarControl = class
    (TdxRibbonCustomToolbarBarControl)
  strict private
  ...
  protected
    ...
    function GetMarkAccessibilityHelperClass
      : TdxBarAccessibilityHelperClass; override;
    ...
    function AllowQuickCustomizing: Boolean; override;
    procedure InitQuickCustomizeItemLinks(AQuickControl
      : TdxBarPopupControl); override;
    ...
    property ViewInfo: TdxRibbonQuickAccessToolbarBarControlViewInfo
      read GetViewInfo;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  ...
  protected
    function IsLastVisibleItemControl(AItemControl: TdxBarItemControl)
      : Boolean; override;
  end;
  ...
  strict private
    ...
    function GetQuickAccessToolbarViewInfo: TdxRibbonQuickAccessToolbarViewInfo;
  protected
    procedure DrawGroupButtonControl(ADrawParams
      : TdxBarButtonLikeControlDrawParams; const ARect: TRect); virtual;
    procedure DrawToolbarContentPart(ABarControl: TdxBarControl;
      ACanvas: TcxCanvas); override;
  ...
  TdxRibbonQuickAccessToolbarBarControlDesignHelper = class
  ...
  TdxRibbonQuickAccessPopupBarControl = class
    (TdxRibbonQuickAccessToolbarBarControl)
  strict private
  ...
  protected
    ...
    property BarControl: TdxRibbonQuickAccessToolbarBarControl
      read GetBarControl;
  public
    destructor Destroy; override;
  ...
  TdxRibbonTabAreaToolbarBarControlAccessibilityHelper = class
    (TdxRibbonBarControlAccessibilityHelper)
  protected
    function GetNextAccessibleObject(AItemLinkHelper
  ...
  protected
    procedure DoDrawText(ADC: HDC; const AText: string; var ARect: TRect;
      AFormat: Cardinal); override;
    procedure DrawToolbarContentPart(ABarControl: TdxBarControl;
      ACanvas: TcxCanvas); override;
  ...
  public
    procedure BarDrawBackground(ABarControl: TdxBarControl; ADC: HDC;
      const ADestRect, ASourceRect: TRect; ABrush: HBRUSH;
      AColor: TColor); override;
    procedure EditControlDrawBorder(const ADrawParams
      : TdxBarEditLikeControlDrawParams; var ARect: TRect); override;
    function EditControlGetBackgroundColor(const ADrawParams
      : TdxBarItemControlDrawParams): TColor; override;
    function EditControlGetTextColor(const ADrawParams
      : TdxBarItemControlDrawParams): TColor; override;
    function GetGlyphColorPalette(ABarItemControl: TdxBarItemControl;
      APaintType: TdxBarPaintType; AState: Integer): IdxColorPalette; override;
  end;
  ...
  TdxRibbonTabAreaToolbarPopupBarControl = class
    (TdxRibbonTabAreaToolbarBarControl)
  strict private
    FPainter: TdxBarPainter;
  ...
  protected
    ...
    // methods
    procedure AdjustHintWindowPosition(var APos: TPoint;
      const ABoundsRect: TRect; AHeight: Integer); override;
    procedure CalcLayout; override;
    ...
    procedure DoBarMouseDown(Button: TMouseButton; Shift: TShiftState;
      ...
      APointInClientRect: Boolean); override;
    ...
    procedure MakeItemControlFullyVisible(AItemControl
      : TdxBarItemControl); override;
    function MarkExists: Boolean; override;
    procedure ViewStateChanged(APrevValue: TdxBarViewState); override;
    procedure UpdateCaptionButton(ACaptionButton: TdxBarCaptionButton);
      override;
    ...
    property IsDesignObjectsOnClientArea: Boolean
      read GetIsDesignObjectsOnClientArea;
  ...
  protected
    ...
    property LayoutCalculator: IdxRibbonGroupLayoutCalculator
  ...
  public
    ...
    function Reduce(AStage: TdxRibbonGroupsReduceStage;
      AUpToViewLevel: TdxBarItemRealViewLevel): Boolean;
    ...
    property BottomKeyTipsBaseLinePosition: Integer
      read GetBottomKeyTipsBaseLinePosition;
    property Ribbon: TdxCustomRibbon read GetRibbon;
  ...
  protected
    ...
    procedure DoBarMouseRightButtonDown(Shift: TShiftState;
      const APoint: TPoint; AItemControl, APrevSelectedControl
      : TdxBarItemControl; APointInClientRect: Boolean); override;
    function GetCaption: TCaption; override;
    function GetPainter: TdxBarPainter; override;
    function GetPopupSize: TSize; override;
    function GetSizeForWidth(AStyle: TdxBarDockingStyle; AWidth: Integer)
    ...
    procedure InitializeForPopup(AParentBarControl: TdxBarControl;
  ...
  TdxRibbonTabGroup = class(TcxInterfacedCollectionItem, IdxBarSelectableItem,
    IdxRibbonToolbarContainer, IdxAdornerTargetElement,
  ...
  protected
    ...
    procedure LoadFromIni(ASource: TCustomIniFile;
      const ABaseSection, ADelimiter: string);
    procedure SaveToIni(ADestination: TCustomIniFile;
      const ABaseSection, ADelimiter: string);

    // IdxAdornerTargetElement
    function IdxAdornerTargetElement.GetControl =
      GetAdornerTargetElementControl;
    ...
    function IdxAdornerTargetElement.GetVisible =
      GetAdornerTargetElementVisible;
    function GetAdornerTargetElementVisible: Boolean; virtual;

    // IdxAdornerTargetElementCollection
    procedure IdxAdornerTargetElementCollection.GetElements =
      GetAdornerTargetElements;
  ...
  public
    ...
    procedure Merge(AGroup: TdxRibbonTabGroup;
      AMergeOptions: TdxRibbonMergeOptions =
      dxRibbonDefaultMergeOptions); virtual;
  ...
  published
    property AssignedValues: TdxRibbonTabGroupAssignedValues
      read FAssignedValues write SetAssignedValues stored False;
    property Caption: string read GetCaption write SetCaption
      stored GetIsCaptionAssigned;
    property MergeKind: TdxRibbonMergeKind read FMergeKind write FMergeKind
      default rmkMerge;
    property MergeOrder: Integer read FMergeOrder write SetMergeOrder default 0;
    property Restriction: TdxRibbonTabGroupRestriction read FRestriction
      write SetRestriction default rtgrNone;
    property Toolbar: TdxBar read GetToolbar write SetToolbar stored False;

    property CanCollapse: Boolean read GetCanCollapse write SetCanCollapse
      default True; // Obsolete
  end;
  ...
  protected
    function GetIniSection(const AGroupIndex: Integer;
      ASource: TCustomIniFile): string;
  ...
  public
    ...
    function Find(const ACaption: string;
      out AGroup: TdxRibbonTabGroup): Boolean;
    ...
    //
    property Items[Index: Integer]: TdxRibbonTabGroup read GetItem
  ...
  TdxRibbonCustomToolbar = class(TInterfacedPersistent,
    IdxRibbonToolbarContainer)
  private
  ...
  protected
    ...
    property IAccessibilityHelper: IdxBarAccessibilityHelper
  ...
  public
    ...
    procedure Merge(ATabAreaSearchToolbar: TdxRibbonTabAreaSearchToolbar;
      AMergeOptions: TdxRibbonMergeOptions =
      dxRibbonDefaultMergeOptions); virtual;
    procedure Unmerge(ATabAreaSearchToolbar
      : TdxRibbonTabAreaSearchToolbar = nil); virtual;
  end;

  { TdxRibbonTabAreaSearchToolbarDockControl }

  TdxRibbonTabAreaSearchToolbarDockControl = class
    (TdxRibbonTabAreaToolbarDockControl);
  ...
  public
    ...
    procedure Merge(AQuickAccessToolbar: TdxRibbonQuickAccessToolbar;
      AMergeOptions: TdxRibbonMergeOptions =
      dxRibbonDefaultMergeOptions); virtual;
    procedure Unmerge(AQuickAccessToolbar
      : TdxRibbonQuickAccessToolbar = nil); virtual;
  published
    property Position: TdxQuickAccessToolbarPosition read FPosition
      write SetPosition default qtpAboveRibbon;
  end;
  ...
  published
    ...
    property StretchGlyph: Boolean read FStretchGlyph write SetStretchGlyph
      default True;
  end;
  ...
  protected
    ...
    function ClosePopup: Boolean;
    function GetAccessibilityHelperClass
      : TdxBarAccessibilityHelperClass; virtual;
  ...
  public
    constructor Create(ATime: Cardinal; AContext: TdxRibbonContext);
      reintroduce;
  end;
  ...
  public
    ...
    property Items[Index: Integer]: TdxRibbonContext read GetItem
      write SetItem; default;
    property Ribbon: TdxCustomRibbon read FRibbon;
  end;
  ...
  TdxRibbonTab = class(TcxComponentCollectionItem, IdxBarSelectableItem,
    IdxFadingObject, IdxFadingObjectFadingOptions, IdxAdornerTargetElement)
  private
    FCaption: string;
    FContext: TdxRibbonContext;
  ...
  public
    ...
    function IsMergeAllowed: Boolean; virtual;
    function IsMerged: Boolean; virtual;
    procedure Merge(ATab: TdxRibbonTab;
      AMergeOptions: TdxRibbonMergeOptions =
      dxRibbonDefaultMergeOptions); virtual;
  ...
  published
    ...
    property Caption: string read FCaption write SetCaption;
    property Context: TdxRibbonContext read FContext write SetContext
      stored False;
  ...
  protected
    function GetAccessibilityHelperClass
      : TdxBarAccessibilityHelperClass; virtual;
    function GetIniSection(const ABaseSection, ADelimiter: string;
      const ATabIndex: Integer): string; overload;
    function GetIniSection(const ATabName: string; ASource: TCustomIniFile)
      : string; overload;
  ...
  public
    ...
    //
    property Items[Index: Integer]: TdxRibbonTab read GetItem
      write SetItem; default;
  end;
  ...
  public
    ...
    function GetApplicationButtonFont(AState: TdxRibbonApplicationButtonState)
      : TFont; virtual;
    ...
    function GetTabHeaderFont(AState: Integer;
      AContext: TdxRibbonContext): TFont;

    property Ribbon: TdxCustomRibbon read FRibbon;
  published
    property ApplicationButton: TFont index Ord(afApplicationButton)read GetFont
      write SetFont stored IsFontStored;
    property AssignedFonts: TdxRibbonAssignedFonts read FAssignedFonts
      write SetAssignedFonts default [];
    property DocumentNameColor: TColor read FDocumentNameColor
      write SetDocumentNameColor default clDefault;
    property Group: TFont index Ord(afGroup)read GetFont write SetFont
      stored IsFontStored;
    property GroupHeader: TFont index Ord(afGroupHeader)read GetFont
      write SetFont stored IsFontStored;
    property TabHeader: TFont index Ord(afTabHeader)read GetFont write SetFont
      stored IsFontStored;
  end;
  ...
  protected
    ...
    function GetControlClass: TCustomdxBarControlClass; override;
    // IdxRibbonApplicationMenu
    function ApplicationMenuPopup(ARibbon: TdxCustomRibbon;
      var AClosedByEscape: Boolean): Boolean;
    function CanShowPopup(ARibbon: TdxCustomRibbon): Boolean;
    function ClosePopup: Boolean;
    function GetDisplayMode: TdxRibbonApplicationMenuDisplayMode;
  ...
  protected
    ...
    // IdxBarHintKeeper
    function DoHint(var ANeedDeactivate: Boolean; out AHintText: string;
      out AShortCut: string): Boolean;
    function CreateHintViewInfo(const AHintText, AShortCut: string)
      : TdxBarCustomHintViewInfo;
    function GetEnabled: Boolean;
    function GetHintPosition(const ACursorPos: TPoint;
    ...
    function ProcessApplicationButtonMouseDown(Button: TMouseButton;
      AShift: TShiftState): Boolean;
    procedure ProcessContextsOnMouseDown(AIsDoubleClick: Boolean);
    procedure ProcessTabClick(ATab: TdxRibbonTab; Button: TMouseButton;
      Shift: TShiftState);
    ...
    property PressedObject: TdxRibbonHitTest read FPressedObject
  ...
  public
    ...
    property ApplicationButtonViewInfo: TdxRibbonApplicationButtonViewInfo
      read GetApplicationButtonViewInfo;
    property ApplicationMenuState: TdxRibbonApplicationMenuState
      read GetApplicationMenuState;
    property HotButton: TdxRibbonCustomButtonViewInfo read FHotButton
      write SetHotButton;
    property PressedButton: TdxRibbonCustomButtonViewInfo read FPressedButton
      write SetPressedButton;
    property Ribbon: TdxCustomRibbon read FRibbon;
  ...
  TdxRibbonGroupsDockControlScrollButtonViewInfo = class
    (TdxRibbonCustomScrollButtonViewInfo)
  protected
    procedure DrawButton(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState); override;
  ...
  public
    ...
    // for internal use
    property TabGroupsScrollButtons: TdxRibbonScrollButtons
      read FTabGroupsScrollButtons;
    property TabGroupsScrollButtonLeft
      : TdxRibbonGroupsDockControlScrollButtonViewInfo
      read FTabGroupsScrollButtonLeft; // for internal use
    property TabGroupsScrollButtonRight
      : TdxRibbonGroupsDockControlScrollButtonViewInfo
      read FTabGroupsScrollButtonRight; // for internal use
  end;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property ApplicationButton: TdxFadingOptions read FApplicationButton
      write SetApplicationButton;
  ...
  public
    ...
    procedure AdjustFormBorderIconSize(AIcon: TdxRibbonBorderDrawIcon;
      AIsToolWindow: Boolean; var ASize: TSize); virtual;
    procedure AdjustFormCaptionHeight(var AValue: Integer); virtual;
    procedure AdjustMargins(var AMargins: TRect;
      AMaxTargetValue: Integer); virtual;
  ...
  TdxRibbonStates = (rsTabsLoaded, rsCancelHintLocked, rsModifiedLocked,
    rsUpdatingColorScheme, rsSizing);
  ...
  TdxCustomRibbon = class(TcxControl, IdxSkin, IdxRibbonFormClient,
    IdxRibbonFormNonClientPart, IdxRibbonFormNonClientHelper,
    IdxRibbonFormNonClientDraw, IdxRibbonMouseWheelReceiver,
    IdxFormKeyPreviewListener, IdxBarManagerMergeOperationHandler,
  ...
  private
    ...
    function GetIniSection(const ADelimiter: string;
    ...
    procedure InternalCloseTabGroupsPopupWindow(AAllowAnimation
      : Boolean = True);
    ...
    procedure SetTabAreaSearchToolbar(const Value
      : TdxRibbonTabAreaSearchToolbar);
    ...
    procedure DXMRecalculate(var Message: TMessage);
      message DXM_RIBBON_RECALCULATE;
    procedure DXMShowApplicationMenu(var Message: TMessage);
      message DXM_RIBBON_SHOWAPPLICATIONMENU;
    procedure WMPaint(var Message: TWMPaint); message WM_PAINT;
  ...
  protected
    ...
    function CalculateFormCaptionHeight(AIsQuickAccessToolbarAtNonClientArea
      : Boolean): Integer; virtual;
    // IdxGestureClient
    function AllowPan(AScrollKind: TScrollBarKind): Boolean; override;
    procedure GestureScroll(ADeltaX, ADeltaY: Integer); override;
    function IsPanArea(const APoint: TPoint): Boolean; override;
    ...
    procedure AdjustRibbonFormBorderIconSize(AIcon: TdxRibbonBorderDrawIcon;
      AIsToolWindow: Boolean; ACaptionHeight: Integer;
      var ASize: TSize); virtual;
    function AreBordersThin: Boolean;
    ...
    function UseExternalFormBorders: Boolean;
    ...
    procedure DrawTabGroupBackground(DC: HDC; const ARect: TRect;
      AState: Integer; AIsInPopup: Boolean);
    ...
    //
    procedure SkinDrawBackground(DC: HDC; const ARect: TRect;
    ...
    procedure SkinDrawCaption(DC: HDC; const ACaption: string;
    ...
    function SkinGetPartColorPalette(APart: Integer; AState: Integer = 0)
      : IdxColorPalette;
    ...
    procedure IdxRibbonFormNonClientDraw.Changed =
      RibbonFormNonClientDrawChanged;
    ...
    // Merging
    function CreateContextAsByMerging(ASourceContext: TdxRibbonContext)
      : TdxRibbonContext; virtual;
    function CreateTabAsByMerging(ASourceTab: TdxRibbonTab)
      : TdxRibbonTab; virtual;
    ...
    function DoHideMinimizedByClick(AWnd: HWND; AShift: TShiftState;
      const APos: TPoint): Boolean; virtual;
    ...
    procedure DoTabGroupCollapsed(ATab: TdxRibbonTab;
      AGroup: TdxRibbonTabGroup); virtual;
    procedure DoTabGroupExpanded(ATab: TdxRibbonTab;
      AGroup: TdxRibbonTabGroup); virtual;

    function GetBar(ACustomizingBarControl: TCustomdxBarControl): TdxBar;
    function GetNextHorizontalAccessibleObject(ACurrentObject
      : IdxBarAccessibilityHelper;
      ADirection: TcxAccessibilityNavigationDirection)
      : IdxBarAccessibilityHelper;
    ...
    // Accessibility
    procedure AddAffiliatedBarControlForAccessibility
    ...
    property ApplicationButtonViewInfo: TdxRibbonApplicationButtonViewInfo
      read GetApplicationButtonViewInfo;
    property FormCaptionHelper: TdxRibbonFormCaptionHelper
      read FFormCaptionHelper;
    property GroupsPainter: TdxRibbonBarPainter read FGroupsPainter;
    property HighlightedTab: TdxRibbonTab read FHighlightedTab
      write SetHighlightedTab;
    property HorizontalNavigationList: TInterfaceList
      read FHorizontalNavigationList;
    property PreviousActiveTab: TdxRibbonTab read FPreviousActiveTab;
    property RibbonFormNonClientParts: TObjectList
      read FRibbonFormNonClientParts;
    property TabGroupsPopupWindow: TdxRibbonTabGroupsPopupWindow
      read FTabGroupsPopupWindow;
  ...
  public
    ...
    function ShowCustomizationForm
      (AMode: TdxRibbonCustomizationFormMode): Boolean;
    ...
    function IsMerged: Boolean; virtual;
    procedure Merge(ARibbon: TdxCustomRibbon;
      AMergeOptions: TdxRibbonMergeOptions =
      dxRibbonDefaultMergeOptions); virtual;
    ...
    property ApplicationButton: TdxRibbonApplicationButton
      read FApplicationButton write SetApplicationButton;
    property BackgroundImage: TdxSmartGlyph read FBackgroundImage
      write SetBackgroundImage;
    property BarManager: TdxBarManager read FBarManager write SetBarManager;
    property CapitalizeTabCaptions: TdxDefaultBoolean
      read FCapitalizeTabCaptions write SetCapitalizeTabCaptions default bFalse;
    property ColorScheme: TdxCustomRibbonSkin read FColorScheme
      write SetColorScheme;
    property ColorSchemeAccent: TdxRibbonColorSchemeAccent
      read FColorSchemeAccent write SetColorSchemeAccent default rcsaYellow;
    property ColorSchemeHandlers: TcxEventHandlerCollection
      read FColorSchemeHandlers;
    property ColorSchemeName: string read GetColorSchemeName
      write SetColorSchemeName stored True;
    ...
    property EnableTabAero: Boolean read FEnableTabAero write SetEnableTabAero
      default True;
    property Fonts: TdxRibbonFonts read FFonts write SetFonts;
    property HelpButton: TdxRibbonHelpButton read FHelpButton
      write SetHelpButton;
    ...
    property MinimizeOnTabDblClick: Boolean read FMinimizeOnTabDblClick
      write FMinimizeOnTabDblClick default True;
    property OptionsFading: TdxRibbonOptionsFading read FOptionsFading
      write SetOptionsFading;
    property PopupMenuItems: TdxRibbonPopupMenuItems read FPopupMenuItems
      write SetPopupMenuItems default dxRibbonDefaultPopupMenuItems;
    property QuickAccessToolbar: TdxRibbonQuickAccessToolbar
      read FQuickAccessToolbar write SetQuickAccessToolbar;
    property ShowMinimizeButton: Boolean read FShowMinimizeButton
      write SetShowMinimizeButton default True;
    property ShowTabGroups: Boolean read FShowTabGroups write SetShowTabGroups
      default True;
    property ShowTabHeaders: Boolean read FShowTabHeaders
      write SetShowTabHeaders default True;
    property Style: TdxRibbonStyle read GetStyle write SetStyle default rs2007;
    property SupportNonClientDrawing: Boolean read FSupportNonClientDrawing
      write SetSupportNonClientDrawing default False;
    property TabAreaSearchToolbar: TdxRibbonTabAreaSearchToolbar
      read FTabAreaSearchToolbar write SetTabAreaSearchToolbar;
    property TabAreaToolbar: TdxRibbonTabAreaToolbar read FTabAreaToolbar
      write SetTabAreaToolbar;
    property TabCount: Integer read GetTabCount;
    ...
    property HelpButtonScreenTip: TdxScreenTip read GetHelpButtonScreenTip
      write SetHelpButtonScreenTip;
    ...
    property OnHelpButtonClick: TdxRibbonEvent read FOnHelpButtonClick
    ...
    property OnMinimizedChanged: TdxRibbonEvent read FOnMinimizedChanged
      write FOnMinimizedChanged;
    property OnMoreCommandsExecute: TdxRibbonEvent read FOnMoreCommandsExecute
      write FOnMoreCommandsExecute;
    property OnTabChanged: TdxRibbonEvent read FOnTabChanged
      write FOnTabChanged;
    property OnTabChanging: TdxRibbonTabChangingEvent read FOnTabChanging
  ...
  public
    class function HasGroupButtonForToolbar(AQATLinks: TdxBarItemLinks;
      AToolbar: TdxBar): Boolean;
    class function IsToolbarDockedInRibbon(ARibbon: TdxCustomRibbon;
      AToolbar: TdxBar): Boolean;
  end;
  ...
  protected
    ...
    function GetAccessibilityHelperClass
      : TdxBarAccessibilityHelperClass; override;
    function GetCurrentImage(AViewSize: TdxBarItemControlViewSize;
      ASelected: Boolean; out AGlyph: TdxSmartGlyph;
      out AImages: TCustomImageList; out AImageIndex: Integer;
      out AIsImageEnabled: TdxDefaultBoolean): Boolean; override;
    function GetHint: string; override;
  ...
  TdxRibbonQuickAccessGroupButtonPopupBarControl = class
    (TdxRibbonCollapsedGroupPopupBarControl)
  private
  ...
  public
    constructor CreateForPopup(AGroupButtonControl
      : TdxRibbonQuickAccessGroupButtonControl); reintroduce; virtual;
    procedure CloseUp; override;
  ...
  protected
    class function GetAddedItemClass(const AAddedItemName: string)
      : TdxBarItemClass; override;
  ...
  protected
    // IdxBarAccessibilityHelper
    function AreKeyTipsSupported(out AKeyTipWindowsManager
      : IdxBarKeyTipWindowsManager): Boolean; override;
    function GetBarManager: TdxBarManager; override;
    ...
    function LogicalNavigationGetChild(AIndex: Integer)
      : TdxBarAccessibilityHelper; override;
    function LogicalNavigationGetChildIndex(AChild: TdxBarAccessibilityHelper)
      : Integer; override;

    property Ribbon: TdxCustomRibbon read GetRibbon;
    property TabsAreaButtonsViewInfo: TdxRibbonButtonsContainerViewInfo
      read GetTabsAreaButtonsViewInfo;
  public
  ...
  protected
    ...
    function HandleNavigationKey(var AKey: Word): Boolean; override;
    function IsNavigationKey(AKey: Word): Boolean; override;
    function LogicalNavigationGetNextAccessibleObject(AShift: TShiftState)
      : IdxBarAccessibilityHelper; override;
    procedure Select(ASetFocus: Boolean); override;
    procedure Unselect(ANextSelectedObject: IdxBarAccessibilityHelper);
      override;
  ...
  TdxRibbonApplicationButtonAccessibilityHelper = class
  ...
  TdxRibbonGroupsDockControlAccessibilityHelper = class
  ...
  TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper = class
    (TdxRibbonBarControlAccessibilityHelper)
  private
    function GetBarControl: TdxRibbonQuickAccessToolbarBarControl;
  protected
    ...
    procedure InitializeItemKeyTipPosition(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper;
  ...
  TdxRibbonQuickAccessToolbarBarControlMarkAccessibilityHelper = class
  ...
  TdxRibbonGroupBarControlAccessibilityHelper = class
    (TdxRibbonBarControlAccessibilityHelper)
  private
    function GetBarControl: TdxRibbonGroupBarControl;
  ...
  protected
    ...
    procedure GetCaptionButtonKeyTipPosition(ACaptionButton
      : TdxBarCaptionButton; out ABasePointY: Integer;
  ...
  TdxRibbonQuickAccessGroupButtonControlAccessibilityHelper = class
  ...
  public
    constructor Create(AColorScheme: TdxCustomRibbonSkin); reintroduce;
      overload; virtual;
    constructor Create(ARibbon: TdxCustomRibbon); reintroduce;
      overload; virtual;
  ...
  TdxRibbonCustomKeyTipWindows = class(TInterfacedObject,
  ...
  protected
    ...
    procedure DrawScrollBarBackground(ACanvas: TcxCanvas;
...
uses
  Math, Types, dxOffice11, CommCtrl, MultiMon, dxThreading,
  dxCustomFluentDesignForm,
  // Colors Schemes
  dxRibbonSkins2007,
  dxRibbonSkins2010,
  dxRibbonSkins2013,
  ...
  //
  dxBarStrs, dxBarSkinConsts, cxDrawTextUtils, dxHooks, cxDWMApi, dxUxTheme,
  dxThemeConsts, dxGDIPlusAPI,
  dxDPIAwareUtils, dxRibbonGroupLayoutCalculator, cxStyles;
...
const
  ...
  dxRibbonCollapsedGroupGlyphBackgroundOffsets: TRect = (Left: 3; Top: 3;
    Right: 3; Bottom: 4);
  ...
  dxRibbonBarBehaviorOptions: TdxBarBehaviorOptions = [bboAllowShowHints,
    bboClickItemsBySpaceKey, bboMouseCantUnselectNavigationItem, bboUnmoved,
...
type
  ...
  TdxBarCaptionButtonAccessibilityHelperAccess = class
  ...
  TdxBarItemControlAccessibilityHelperAccess = class
...
const
  sdxErrorToolbarUsedAsQAT =
    'This toolbar is already used as the QuickAccessToolbar';
  sdxErrorToolbarUsedInAnotherGroupInThisTab =
    'At least one group in this tab already contains this toolbar';
...
type
  ...
  public
    ...
    class procedure UnmergeToolbarContainers(ATargetContainer, ASourceContainer
      : TObject);
  end;
...
procedure dxRibbonAccessibilityEnumChildren(AIntf: IdxBarAccessibilityHelper;
  AProc: TdxRibbonAddAccessibilityHelperProc);
var
...
begin
  if not CheckGdiPlus then
    raise EdxException.CreateFmt(cxGetResourceString(@dxSBAR_GDIPLUSNEEDED),
      [AClass.ClassName]);
  if (AOwner = nil) and (Application.MainForm <> nil) then
    AOwner := Application.MainForm;
  if not(AOwner is TCustomForm) then
    raise EdxException.CreateFmt(cxGetResourceString(@dxSBAR_RIBBONBADOWNER),
      [AClass.ClassName]);
  if HasComponentOnForm(TCustomForm(AOwner), AClass) then
    raise EdxException.CreateFmt(cxGetResourceString(@dxSBAR_RIBBONMORETHANONE),
      [AClass.ClassName]);
end;

function FindMouseWheelReceiver(AWnd: HWND;
  out AReceiver: IdxRibbonMouseWheelReceiver): Boolean;
var
...
procedure dxRibbonMouseHook(ACode: Integer; wParam: wParam; lParam: lParam;
    ...
    mwrActiveBarControl:
      begin
        SendMessage(ActiveBarControl.Handle, WM_MOUSEWHEEL,
          MakeWParam(ShiftStateToKeys(KeyboardStateToShiftState),
          HiWord(AMHS.mouseData)), MakeLParam(AMHS.MouseHookStruct.pt.X,
          AMHS.MouseHookStruct.pt.Y));
        AHookResult := 1;
      end;

    mwrWindow:
      if FindMouseWheelReceiver(WindowFromPoint(AMHS.MouseHookStruct.pt),
        AReceiver) and AReceiver.CanProcessMouseWheel then
      begin
        if AReceiver.DoMouseWheel(KeyboardStateToShiftState,
          SmallInt(HiWord(AMHS.mouseData)), cxInvalidPoint) then
          AHookResult := 1;
      end
  end;
end;
...
begin
  ...
  if AMonitor <> 0 then
    ...
    Result := cxRectBounds(Screen.DesktopLeft, Screen.DesktopTop,
      Screen.DesktopWidth, Screen.DesktopHeight);
end;

procedure SelectFirstSelectableAccessibleObject(AParentObject
  : TdxBarAccessibilityHelper);
begin
...
function GetRootRibbonAccessibilityHelper(AParentWnd: HWND)
  : IdxBarAccessibilityHelper;
var
  ARibbon: TdxCustomRibbon;
begin
...
procedure RaiseMergingError(AResString: Pointer;
  ATargetComponent, ASourceComponent: TComponent); overload;
begin
  raise EdxException.Create(Format(cxGetResourceString(AResString),
    [cxGetFullComponentName(ATargetComponent),
    cxGetFullComponentName(ASourceComponent)]));
end;

{ TdxRibbonMergeHelper }

class function TdxRibbonMergeHelper.IsMergedWith(ATestToolbar,
  ATargetToolbar: TdxBar): Boolean;
begin
...
class procedure TdxRibbonMergeHelper.MergeToolbarContainers(ATargetContainer,
  ASourceContainer: TObject; ACanCreateNewToolbar: Boolean);
var
  ASourceToolbar: IdxRibbonToolbarContainer;
  ATargetToolbar: IdxRibbonToolbarContainer;
begin
  if (ATargetContainer = ASourceContainer) or not Supports(ATargetContainer,
    IdxRibbonToolbarContainer, ATargetToolbar) or not Supports(ASourceContainer,
    IdxRibbonToolbarContainer, ASourceToolbar) then
    RaiseMergingError(@dxSBAR_CANTMERGETOOLBAR);

  if not(ASourceToolbar.Ribbon.IsBarManagerValid and
    ATargetToolbar.Ribbon.IsBarManagerValid) then
...
class procedure TdxRibbonMergeHelper.UnmergeToolbarContainers(ATargetContainer,
  ASourceContainer: TObject);
...
function TdxRibbonGroupBarControlViewInfoHelper.GetOffsetsInfo
  : TdxRibbonGroupOffsetsInfo;
begin
  Result.ButtonGroupOffset := ViewInfo.BarControl.Ribbon.SkinGetPartSize
    (DXBAR_BUTTONGROUP);
...
procedure TdxRibbonCustomMergeData.ListChangeHandler(Sender: TObject;
...
constructor TdxCustomDesignSelectionHelper.Create(AComponent: TComponent;
  AOwner, AParent: TPersistent);
begin
  inherited Create;
...
// IdxBarSelectableItem
function TdxCustomDesignSelectionHelper.CanDelete(ADestruction
  : Boolean): Boolean;
begin
...
procedure TdxCustomDesignSelectionHelper.DeleteSelection(var AReference
  : IdxBarSelectableItem; ADestruction: Boolean);
begin
...
procedure TdxCustomDesignSelectionHelper.ExecuteCustomizationAction
...
function TdxCustomDesignSelectionHelper.GetNextSelectableItem
  : IdxBarSelectableItem;
begin
  Result := nil;
end;
...
function TdxCustomDesignSelectionHelper.GetSelectionStatus
  : TdxBarSelectionStatus;
begin
...
function TdxCustomDesignSelectionHelper.GetSupportedActions
  : TdxBarCustomizationActions;
begin
  Result := [];
end;
...
begin
  Result := (BarManager <> nil) and (BarManager as IdxBarDesigner)
    .IsComponentSelected(Owner);
end;

procedure TdxCustomDesignSelectionHelper.SelectComponent(ASelectionOperation
...
procedure TdxRibbonPainter.AdjustApplicationButtonFont(AFont: TFont;
  AState: TdxRibbonApplicationButtonState);
begin
...
procedure TdxRibbonPainter.AdjustContextFont(AFont: TFont;
...
procedure TdxRibbonPainter.AdjustTabHeaderFont(AFont: TFont; AState: Integer;
  AContext: TdxRibbonContext = nil);
begin
  ColorScheme.InitializePaintData(Self);
...
procedure TdxRibbonPainter.DrawApplicationButton(ACanvas: TcxCanvas;
  const ABounds: TRect; AState: TdxRibbonApplicationButtonState);
begin
...
procedure TdxRibbonPainter.DrawApplicationButtonGlyph(ACanvas: TcxCanvas;
  const ABounds: TRect; AGlyph: TdxSmartGlyph);
begin
    ...
    begin
      ...
      try
        ...
        TdxImageListPaintCache.PrepareImage(Result, ASize,
          TdxDrawImageCacheID.Create(nil, AImageList, 0, idmNormal, True, False,
          clNone, nil), nil);
      finally
        AImageList.Free;
      end;
...
procedure TdxRibbonPainter.DrawBackground(ACanvas: TcxCanvas;
  const ABounds: TRect);
var
  R: TRect;
begin
  R := ABounds;
  if QuickAccessToolbarViewInfo.Visible and
    not(QuickAccessToolbarViewInfo.AtBottom or ViewInfo.SupportNonClientDrawing)
  then
  begin
...
procedure TdxRibbonPainter.DrawBackgroundImage(ACanvas: TcxCanvas;
  const ABounds: TRect);
var
  R: TRect;
...
begin
  ...
  begin
    ...
    cxRightToLeftDependentDraw(ACanvas.Handle, R, AIsRightToLeft, procedure
      begin
...
procedure TdxRibbonPainter.DrawContextGroupsArea(ACanvas: TcxCanvas;
const ABounds: TRect; AIsQuickAccessToolbarAtBottom, AIsInPopup: Boolean);
var
...
begin
  ...
  try
    ...
    ColorScheme.DrawContextTabGroupsArea(B.cxCanvas.Handle, B.ClientRect,
      Ribbon.ActiveTab.Context.Color, AIsQuickAccessToolbarAtBottom,
      AIsInPopup);
...
ATransparent: Boolean = False);
begin
  dxDrawTextOnGlass(DC, AText, AFont, ABounds, AColor, AFlags,
    dxCaptionGlowRadius, ATransparent);
end;

procedure TdxRibbonPainter.DrawGlyph(ACanvas: TcxCanvas; AGlyph: TdxSmartGlyph;
const ABounds: TRect);
begin
  if not cxRectIsEmpty(ABounds) then
    cxDrawImage(ACanvas.Handle, ABounds, ABounds, AGlyph, nil, -1,
      idmNormal, True);
end;
...
begin
  if AIsAllowContextPaint and ViewInfo.HasActiveContextTab then
    DrawContextGroupsArea(ACanvas, ABounds, QuickAccessToolbarViewInfo.AtBottom,
      AIsInPopup)
  else
    ColorScheme.DrawTabGroupsArea(ACanvas.Handle, ABounds,
      QuickAccessToolbarViewInfo.AtBottom, AIsInPopup);
end;
...
const
  StateMap: array [TcxButtonState] of Integer = (DXBAR_NORMAL, DXBAR_NORMAL,
    DXBAR_HOT, DXBAR_PRESSED, DXBAR_NORMAL);
begin
  ColorScheme.DrawGroupScrollButton(ACanvas.Handle, ABounds, ALeft,
    StateMap[AState]);
end;
...
procedure TdxRibbonPainter.DrawTabAreaBackground(ACanvas: TcxCanvas;
const ABounds: TRect);
var
  ARect: TRect;
begin
  ...
  begin
    ...
    try
      ...
      ARect := ABounds;
      ARect.Top := 0;
      ColorScheme.DrawRibbonTopFrameArea(ACanvas.Handle, ARect,
...
procedure TdxRibbonPainter.DrawTabBase(ACanvas: TcxCanvas;
const ABounds: TRect);
begin
...
const
  StateMap: array [TcxButtonState] of Integer = (DXBAR_NORMAL, DXBAR_NORMAL,
    DXBAR_HOT, DXBAR_PRESSED, DXBAR_NORMAL);
begin
  ColorScheme.DrawTabScrollButton(ACanvas.Handle, ABounds, ALeft,
    StateMap[AState]);
end;
...
begin
  ColorScheme.DrawMinimizeRibbonButton(ACanvas.Handle, ABounds, AState,
    AMinimized);
end;
...
begin
  Result := Ribbon.ScaleFactor.Apply(cxRect(dxRibbonTabTextOffset, 7,
    dxRibbonTabTextOffset, 4));
...
begin
  if cxRectIsEmpty(ABounds) then
    Exit;
  ACanvas.Font := ViewInfo.GetFormCaptionFont
    (GetRibbonFormPaintData.GetIsActive);
  R := ABounds;
  if UseAeroNCPaint(GetRibbonFormPaintData) then
    DrawRibbonGlassFormCaptionText(ACanvas, ABounds, ADocumentName,
      ACaption, True)
  else
  begin
    AFlags := CXTO_PREVENT_LEFT_EXCEED or CXTO_CENTER_HORIZONTALLY or
    ...
    cxTextOut(ACanvas.Handle, ADocumentName + ACaption, R, AFlags, 0,
      Length(ADocumentName), nil, clNone,
...
begin
  ...
  begin
    ...
    if Ribbon.UseRightToLeftAlignment then
      ...
      R.Right := R.Left + cxTextWidth(ACanvas.Font, S) + 2 *
        dxCaptionGlowRadius;
    ...
    AFlags := DT_CENTER or DT_SINGLELINE or DT_END_ELLIPSIS or DT_VCENTER or
      DT_NOPREFIX;
    if Ribbon.UseRightToLeftReading then
...
begin
  ColorScheme.InitializePaintData(Self);
  ColorScheme.UseExternalBorders := Ribbon.UseExternalFormBorders;
  ...
  DrawRibbonFormCaptionText(ACanvas, ViewInfo.FormCaptionBounds,
    ViewInfo.DocumentName, ViewInfo.Caption);
end;

procedure TdxRibbonPainter.DrawRibbonFormCaptionFrameArea(ACanvas: TcxCanvas;
R: TRect);
var
  ABorderWidths: TRect;
begin
  ...
  try
    ...
    ColorScheme.DrawRibbonTopFrameArea(ACanvas.Handle, R,
      ViewInfo.IsTabsOnGlass);
...
const ABounds: TRect; AIcon: TdxRibbonBorderDrawIcon;
...
function TdxRibbonPainter.GetApplicationMenuState
  : TdxRibbonApplicationMenuState;
begin
...
function TdxRibbonPainter.GetGroupCaptionBottomOffset
  (AInPopup: Boolean): Integer;
begin
  Result := Ribbon.SkinGetPartSize
    (IfThen(AInPopup, rspTabGroupInPopupBottomOffset, rspTabGroupBottomOffset));
end;
...
function TdxRibbonPainter.GetQuickAccessToolbarViewInfo
  : TdxRibbonQuickAccessToolbarViewInfo;
begin
...
function TdxRibbonCustomViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
begin
  Result := False;
end;
...
procedure TdxRibbonCustomViewInfo.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
begin
  // do nothing
end;
...
constructor TdxRibbonCustomButtonAccessibilityHelper.CreateEx(AButtonViewInfo
  : TdxRibbonCustomButtonViewInfo);
begin
...
function TdxRibbonCustomButtonAccessibilityHelper.HandleNavigationKey
...
function TdxRibbonCustomButtonAccessibilityHelper.IsNavigationKey
  (AKey: Word): Boolean;
begin
  Result := inherited IsNavigationKey(AKey) or
    (AKey in [VK_ESCAPE, VK_SPACE, VK_RETURN]);
end;
...
procedure TdxRibbonCustomButtonAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
...
procedure TdxRibbonCustomButtonAccessibilityHelper.GetKeyTipData
  (AKeyTipsData: TList);
begin
  // nothing
end;
...
function TdxRibbonCustomButtonAccessibilityHelper.GetParent
...
constructor TdxRibbonCustomButtonViewInfo.Create
  (AOwner: TdxRibbonCustomContainerViewInfo);
begin
  inherited Create;
  FState := cxbsNormal;
  FOwner := AOwner;
...
begin
  if not Enabled then
    State := cxbsDisabled
  else if Ribbon.Controller.PressedButton = Self then
    State := cxbsPressed
  else if Ribbon.Controller.HotButton = Self then
    State := cxbsHot
  else
    State := cxbsNormal;
end;
...
function TdxRibbonCustomButtonViewInfo.CreateAccessibilityHelper
  : TdxRibbonCustomButtonAccessibilityHelper;
begin
...
procedure TdxRibbonCustomButtonViewInfo.DrawButton(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
end;

procedure TdxRibbonCustomButtonViewInfo.DrawButtonGlyph(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
end;
...
function TdxRibbonCustomButtonViewInfo.GetAccessibilityHelper
  : IdxBarAccessibilityHelper;
begin
...
procedure TdxRibbonCustomButtonViewInfo.GetFadingImages(out AFadeOutImage,
...
function TdxRibbonCustomButtonViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
begin
...
procedure TdxRibbonTabViewInfo.Calculate(const ABounds: TRect;
ASeparatorAlpha: Byte);
begin
  FIsRightToLeftConverted := False;
  FBounds := ABounds;
  ...
  if HasSeparator then
    FSeparatorBounds := cxRect(Bounds.Right - dxRibbonTabSeparatorWidth,
...
begin
  ...
  if HasSeparator then
    ColorScheme.DrawTabSeparator(ACanvas.Handle, SeparatorBounds,
      SeparatorAlphaValue);
  if ContextBegin then
    ColorScheme.DrawContextTabSeparator(ACanvas.Handle,
      cxRectSetWidth(Bounds, ContextTabSeparatorWidth), True);
  if ContextEnd then
    ColorScheme.DrawContextTabSeparator(ACanvas.Handle,
      cxRectSetRight(Bounds, Bounds.Right, ContextTabSeparatorWidth), False);
...
const R: TRect; AState: TdxRibbonTabState);
begin
  if Tab.Context <> nil then
    ColorScheme.DrawContextTabBackground(ACanvas.Handle, R, AState,
      Tab.Context.Color)
  else
    ColorScheme.DrawTab(ACanvas.Handle, R, AState);
end;

procedure TdxRibbonTabViewInfo.DrawText(ACanvas: TcxCanvas;
AHasSeparator: Boolean);
const
  CustomFlags: array [Boolean] of Integer = (cxAlignVCenter or cxSingleLine or
    cxAlignHCenter, cxAlignVCenter or cxSingleLine);
  GlassFlags: array [Boolean] of Integer = (DT_VCENTER or DT_SINGLELINE or
    DT_CENTER, DT_VCENTER or DT_SINGLELINE);
var
  AFlags: Cardinal;
begin
  ...
  begin
    ...
    dxDrawTextOnGlass(ACanvas.Handle, Tab.DisplayCaption, ACanvas.Font,
      TextBounds, ACanvas.Font.Color, AFlags, 0, True);
  end
  else
  begin
    ACanvas.Brush.Style := bsClear;
...
procedure TdxRibbonTabViewInfo.CalculateWidths(ACanvas: TcxCanvas;
  ...
  begin
    ...
    if ContextEnd then
      FOptimalWidth := Max(OptimalWidth,
        AViewInfo.ContextsViewInfo.CalculateCaptionWidth(ACanvas,
        Tab.Context.Caption) + dxExtraContextGap);
  end;
...
begin
  FBounds := TdxRightToLeftLayoutConverter.ConvertRect(FBounds, ABounds);
  FSeparatorBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FSeparatorBounds, ABounds);
...
const
  ActiveHotMap: array [Boolean] of TdxRibbonTabState = (rtsActive,
    rtsActiveHot);
...
begin
  ...
  else if not(Tab.Highlighted or Tab.Active) then
    Result := rtsNormal
  else if IsTabActive then
...
begin
  Result := Tab.Active and Ribbon.AreGroupsVisible and
    (Ribbon.ApplicationMenuState <= ramsShownAsMenu);
end;
...
function TdxRibbonTabViewInfo.PrepareFadeImage(ADrawHot: Boolean): TcxBitmap;
  ...
  const
    ActiveStatesMap: array [Boolean] of TdxRibbonTabState = (rtsActive,
      rtsActiveHot);
...
begin
  if Ribbon.IsDesigning then
    BarDesignController.ShowCustomCustomizePopup(Ribbon.BarManager,
      PopulateMenuItems, Ribbon.GroupsPainter,
      Ribbon.ClientToScreen(Point(Bounds.Left, Bounds.Bottom)))
  else
    ShowMenu;
end;

procedure TdxRibbonTabsDropDownButtonViewInfo.DrawButton(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
const
  TabStateMap: array [TcxButtonState] of TdxRibbonTabState = (rtsNormal,
    rtsNormal, rtsHot, rtsActive, rtsNormal);
begin
...
procedure TdxRibbonTabsDropDownButtonViewInfo.ClearMenu
...
procedure TdxRibbonTabsDropDownButtonViewInfo.PopulateMenuItemContent
  (AItemLinks: TdxBarItemLinks; ASource: TdxRibbonTab);
var
  AButton: TdxBarButton;
begin
  AButton := BarDesignController.AddInternalItem(AItemLinks, TdxBarButton,
    ASource.DisplayCaption, ActivateMenuItem, NativeInt(ASource))
...
procedure TdxRibbonTabsDropDownButtonViewInfo.PopulateMenuItems
  (AItemLinks: TdxBarItemLinks);
var
  ATab: TdxRibbonTab;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if (AViewInfoIndex < 0) or ATab.Active and
            (AViewInfoIndex = ATabsViewInfo.Count - 1) and (AItemLinks.Count > 0)
          then
...
procedure TdxRibbonTabsScrollButtonViewInfo.DrawButton(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawTabScrollButton(ACanvas, R, (FScrollButton = rsbLeft)
    xor Ribbon.UseRightToLeftAlignment, AState);
end;

procedure TdxRibbonTabsScrollButtonViewInfo.DrawButtonGlyph(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawTabScrollButtonGlyph(ACanvas, R, (FScrollButton = rsbLeft)
    xor Ribbon.UseRightToLeftAlignment);
end;

function TdxRibbonTabsScrollButtonViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
const
  HitTestMap: array [TdxRibbonScrollButton] of TdxRibbonHitTest =
...
constructor TdxRibbonTabsViewInfo.Create(AOwner: TdxRibbonViewInfo);
  ...
  function CreateButton(AScrollButton: TdxRibbonScrollButton)
    : TdxRibbonTabsScrollButtonViewInfo;
  begin
...
function TdxRibbonTabsViewInfo.CreateTabViewInfo(ATab: TdxRibbonTab)
  : TdxRibbonTabViewInfo;
begin
...
begin
  ...
  AMaxSeparatorIndex := Count -
    IfThen(Owner.TabsAreaViewInfo.ButtonsViewInfo.Count = 0, 1);
...
begin
  ...
  else if FTotalMinimalWidth <= cxRectWidth(Bounds) then
...
begin
  ...
  FScrollButtonLeft.Bounds := cxRectSetWidth(Bounds,
    IfThen(rsbLeft in ScrollButtons, AButtonWidth));
  FScrollButtonRight.Bounds := cxRectSetRight(Bounds, Bounds.Right,
    IfThen(rsbRight in ScrollButtons, AButtonWidth));
end;
...
begin
  ...
  FScrollButtonLeft.Bounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FScrollButtonLeft.Bounds, ABounds);
  FScrollButtonRight.Bounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FScrollButtonRight.Bounds, ABounds);
end;
...
begin
  ...
  cxBitBlt(Bitmap.Canvas.Handle, ACanvas.Handle, Bounds,
    Bounds.TopLeft, SRCCOPY);
  DrawCore(Bitmap.cxCanvas);
  cxBitBlt(ACanvas.Handle, Bitmap.Canvas.Handle, Bounds,
    Bounds.TopLeft, SRCCOPY);
end;
...
begin
  Result := ACanvas.FontHeight(Ribbon.Fonts.TabHeader) +
    cxMarginsHeight(FTabContentOffset);
end;

procedure TdxRibbonTabsViewInfo.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
var
  I: Integer;
begin
...
begin
  FSeparatorAlpha := MulDiv(ATotalDelta, 255,
    Count * (ScaleFactor.Apply(dxRibbonOptimalTabSpace) -
    ScaleFactor.Apply(dxRibbonTabTextOffset) * 2));
...
function TdxRibbonTabsViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
var
  I: Integer;
begin
  Result := FScrollButtonLeft.GetHitInfo(AHitInfo) or
    FScrollButtonRight.GetHitInfo(AHitInfo);
...
function TdxRibbonTabsViewInfo.GetTabViewInfo(Index: Integer)
  : TdxRibbonTabViewInfo;
begin
...
begin
  ...
  CalculateSimpleTabLayoutCore;
  while (Count > 1) and ((Items[Count - 1].Bounds.Right + ADropDownButtonWidth)
    > Bounds.Right) do
  begin
    for I := Count - 1 downto 0 do
    begin
      ...
      if ((AActiveTab = nil) or (ATab <> AActiveTab)) and
        ((APrevActiveTab = nil) or (ATab <> APrevActiveTab) or (Count <= 2))
      then
      begin
  ...
  FDropDownButton.Bounds := cxRectSetLeft(Bounds, Items[Count - 1].Bounds.Right,
    ADropDownButtonWidth);
  FBounds.Right := FDropDownButton.Bounds.Right;
end;

procedure TdxRibbonTabsViewInfo2016Tablet.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  FDropDownButton.Bounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FDropDownButton.Bounds, ABounds);
end;

function TdxRibbonTabsViewInfo2016Tablet.GetDesignHitTest
  (const P: TPoint): Boolean;
begin
  Result := inherited or cxRectPtIn(DropDownButton.Bounds, P);
end;

function TdxRibbonTabsViewInfo2016Tablet.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
begin
...
procedure TdxRibbonTabsViewInfo2016Tablet.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ATextWidth := cxTextWidth(AActiveTabViewInfo.Font,
        AActiveTabViewInfo.Tab.DisplayCaption);
      if ATextWidth < cxRectWidth(FTabBaseRectLTR) then
        FTabBaseRectLTR := cxRectCenterHorizontally(FTabBaseRectLTR,
          ATextWidth);
    end;
  end
  else
...
function TdxRibbonTabsViewInfo2019.CreateTabBaseAnimation(const AOldRect,
  ANewRect: TRect): TdxRectAnimationTransition;
begin
  if cxRectIntersect(AOldRect, ANewRect) then
    Result := TdxRectAnimationTransition.Create(AOldRect, ANewRect,
      dxRibbonTabStateChangeAnimationTime, ateAccelerateDecelerate)
  else
    Result := TdxRectAnimationTransition.Create(AOldRect, ANewRect,
      dxRibbonTabStateChangeAnimationTime);
end;

procedure TdxRibbonTabsViewInfo2019.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  FTabBaseRectLTR := TdxRightToLeftLayoutConverter.ConvertRect
    (FTabBaseRectLTR, ABounds);
  UpdateTabBaseRect(FTabBaseRectLTR);
  dxAnimationController.Update;
end;
...
begin
  ...
  begin
    ...
    else if CanAnimateTabBaseRectChange then
    begin
...
procedure TdxRibbonHelpButtonViewInfo.DrawButton(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawHelpButton(ACanvas, R, AState);
end;

procedure TdxRibbonHelpButtonViewInfo.DrawButtonGlyph(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
...
function TdxRibbonHelpButtonViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
begin
  Result := inherited GetHitInfo(AHitInfo);
  if Result then
...
begin
  if (HelpButton.Glyph = nil) or HelpButton.Glyph.Empty then
    AIconSize := cxSize(dxGetSystemMetrics(SM_CXSMICON, ScaleFactor),
  ...
  if HelpButton.StretchGlyph then
    AIconSize := cxSize(cxRectProportionalStretch(Result, AIconSize.cx,
      AIconSize.cy));
...
procedure TdxRibbonMinimizeButtonViewInfo.DrawButton(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawMinimizeButton(ACanvas, R, AState, not Ribbon.ShowTabGroups);
end;

procedure TdxRibbonMinimizeButtonViewInfo.DrawButtonGlyph(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
...
begin
  if Ribbon.ShowTabGroups then
    Result := rmbMinimize
  else if Ribbon.IsPopupGroupsMode then
    Result := rmbPin
  else
    Result := rmbRestore;
end;
...
procedure TdxRibbonMDIButtonViewInfo.DrawButton(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawMDIButton(ACanvas, R, ButtonType, AState);
end;

procedure TdxRibbonMDIButtonViewInfo.DrawButtonGlyph(ACanvas: TcxCanvas;
const R: TRect; AState: TcxButtonState);
begin
...
begin
  Result := inherited GetIsEnabled and ViewInfo.IsMDIButtonEnabled
    (ButtonType, 0);
end;

{ TdxRibbonButtonsContainerViewInfo }

procedure TdxRibbonButtonsContainerViewInfo.Add
  (AClass: TdxRibbonCustomButtonViewInfoClass);
begin
...
procedure TdxRibbonButtonsContainerViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
var
  I: Integer;
begin
  inherited DoRightToLeftConversion(ABounds);
  for I := 0 to Count - 1 do
    Items[I].Bounds := TdxRightToLeftLayoutConverter.ConvertRect
      (Items[I].Bounds, ABounds);
end;
...
function TdxRibbonButtonsContainerViewInfo.GetItem(Index: Integer)
  : TdxRibbonCustomButtonViewInfo;
begin
  Result := TdxRibbonCustomButtonViewInfo(inherited Items[Index]);
end;

function TdxRibbonButtonsContainerViewInfo.GetSelectedButton
  : TdxRibbonCustomButtonViewInfo;
var
  I: Integer;
begin
...
procedure TdxRibbonButtonsContainerViewInfo.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
var
  I: Integer;
begin
...
function TdxRibbonButtonsContainerViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
var
  I: Integer;
begin
...
procedure TdxRibbonTabsAreaButtonsViewInfo.CalculateButtonsBounds
  (const ABounds: TRect);
var
  I: Integer;
  R: TRect;
...
begin
  ...
  begin
    ...
    try
      cxBitBlt(ABitmap.cxCanvas.Handle, ACanvas.Handle, ABitmap.ClientRect,
        Bounds.TopLeft, SRCCOPY);
      ABitmap.cxCanvas.WindowOrg := Bounds.TopLeft;
      inherited Draw(ABitmap.cxCanvas);
      ABitmap.cxCanvas.WindowOrg := cxNullPoint;
      cxBitBlt(ACanvas.Handle, ABitmap.cxCanvas.Handle, Bounds,
        cxNullPoint, SRCCOPY);
    finally
      ABitmap.Free;
    end;
...
procedure TdxRibbonContextViewInfo.Calculate(ACanvas: TcxCanvas;
const ABounds: TRect);
begin
  FIsRightToLeftConverted := False;
  FNeedShowHint := False;
  ...
  begin
    FTextBounds := CalculateTextBounds(ACanvas,
      IfThen(IsPaintOnGlass, dxCaptionGlowRadius, dxRibbonTabsLeftSpace));
    FNeedShowHint := cxRectWidth(Bounds) < Owner.CalculateCaptionWidth(ACanvas,
      Context.Caption);
  end;
end;

function TdxRibbonContextViewInfo.CalculateTextBounds(ACanvas: TcxCanvas;
AIndent: Integer): TRect;
begin
  Result := Bounds;
  FText := cxGetStringAdjustedToWidth(ACanvas.Handle, 0, Context.Caption,
    cxRectWidth(Bounds) - 2 * AIndent);
end;

procedure TdxRibbonContextViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  FBounds := TdxRightToLeftLayoutConverter.ConvertRect(FBounds, ABounds);
  FTextBounds := TdxRightToLeftLayoutConverter.ConvertRect(FTextBounds,
...
begin
  if IsPaintOnGlass then
    ColorScheme.DrawContextBackgroundGlass(ACanvas.Handle, Bounds,
      Context.Color)
  else
...
begin
  ...
  ACanvas.Font := Font;
  ACanvas.Brush.Style := bsClear;
  InternalDrawText(ACanvas, cxRectOffset(TextBounds, 1, 1),
...
procedure TdxRibbonContextViewInfo.InternalDrawText(ACanvas: TcxCanvas;
const R: TRect; ATextColor: TColor; AHasGlowing: Boolean);
var
  AFlags: Cardinal;
begin
  ...
  begin
    ...
    begin
      ...
      if FContext.Ribbon.UseRightToLeftReading then
        AFlags := AFlags or DT_RTLREADING;
      dxDrawTextOnGlass(ACanvas.Handle, Text, Font, R, ATextColor, AFlags,
        IfThen(AHasGlowing, dxCaptionGlowRadius), True);
    end
    else
    begin
      AFlags := cxAlignHCenter or cxShowEndEllipsis or cxAlignVCenter or
        cxSingleLine;
      if FContext.Ribbon.UseRightToLeftReading then
        AFlags := AFlags or cxRtlReading;
...
begin
  ...
  begin
    ...
    try
      ...
      cxAlphaBlend(ACanvas.Handle, ABitmap, Bounds, ABitmap.ClientRect, False,
        Context.Alpha);
    finally
      ABitmap.Free;
    end;
...
const
  PartsMap: array [Boolean] of Integer = (rspContextTextShadow,
...
function TdxRibbonContextViewInfo2007.CalculateTextBounds(ACanvas: TcxCanvas;
AIndent: Integer): TRect;
var
  ATextSize: TSize;
begin
...
function TdxRibbonContextViewInfo2010OrNewer.CalculateTextBounds
  (ACanvas: TcxCanvas; AIndent: Integer): TRect;
begin
  Result := inherited;
...
function TdxRibbonContextsViewInfo.CalculateCaptionWidth(ACanvas: TcxCanvas;
const ACaption: string): Integer;
begin
  Result := 0;
...
function TdxRibbonContextsViewInfo.NeedShowHint
  (AContext: TdxRibbonContext): Boolean;
var
...
function TdxRibbonContextsViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
var
  I: Integer;
begin
...
function TdxRibbonContextsViewInfo.Find(AContext: TdxRibbonContext)
  : TdxRibbonContextViewInfo;
var
  I: Integer;
begin
...
function TdxRibbonContextsViewInfo.CreateContextViewInfo
  (AContext: TdxRibbonContext): TdxRibbonContextViewInfo;
begin
  Result := TdxRibbonContextViewInfo.Create(Self, AContext);
end;

procedure TdxRibbonContextsViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
var
  I: Integer;
begin
...
function TdxRibbonContextsViewInfo.GetItem(Index: Integer)
  : TdxRibbonContextViewInfo;
begin
...
function TdxRibbonContextsViewInfo2007.CreateContextViewInfo
  (AContext: TdxRibbonContext): TdxRibbonContextViewInfo;
begin
...
function TdxRibbonContextsViewInfo2010OrNewer.CreateContextViewInfo
  (AContext: TdxRibbonContext): TdxRibbonContextViewInfo;
begin
...
procedure TdxRibbonApplicationButtonViewInfo.CalculateMetrics
  (ACanvas: TcxCanvas);
begin
  FClientOffsets := Ribbon.SkinGetContentOffsets(DXBAR_APPLICATIONBUTTON);
  FGlyphOffsets := Ribbon.SkinGetContentOffsets
...
procedure TdxRibbonApplicationButtonViewInfo.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
begin
...
procedure TdxRibbonApplicationButtonViewInfo.GetFadingImages(out AFadeOutImage,
  AFadeInImage: TcxBitmap);
begin
  AFadeInImage := TcxBitmap32.CreateSize(ClientBounds, True);
  AFadeOutImage := TcxBitmap32.CreateSize(ClientBounds, True);
  Painter.DrawApplicationButton(AFadeInImage.cxCanvas,
    AFadeInImage.ClientRect, rabsHot);
  Painter.DrawApplicationButton(AFadeOutImage.cxCanvas,
    AFadeOutImage.ClientRect, rabsNormal);
end;

procedure TdxRibbonApplicationButtonViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  FGlyphBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FGlyphBounds, ABounds);
end;
...
begin
  Painter.DrawApplicationButtonGlyph(ACanvas, GlyphBounds,
...
function TdxRibbonApplicationButtonViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
begin
...
function TdxRibbonApplicationButtonViewInfo.GetApplicationButton
  : TdxRibbonApplicationButton;
begin
...
function TdxRibbonApplicationButtonViewInfo.GetState
  : TdxRibbonApplicationButtonState;
begin
  if ApplicationButton.IAccessibilityHelper.IsSelected then
    Result := rabsHot
  else if IsPressed or (Ribbon.ApplicationMenuState <> ramsHidden) then
    Result := rabsPressed
  else if IsHot then
    Result := rabsHot
  else
...
procedure TdxRibbonApplicationButtonViewInfo2010OrNewer.CalculateMetrics
  (ACanvas: TcxCanvas);
begin
  inherited;
...
procedure TdxRibbonApplicationButtonViewInfo2010OrNewer.DrawContent
  (ACanvas: TcxCanvas);
begin
  if HasText then
    DrawText(ACanvas)
...
procedure TdxRibbonApplicationButtonViewInfo2010OrNewer.DrawText
  (ACanvas: TcxCanvas);
var
  AFlags: Cardinal;
begin
  ...
  begin
    ...
    if Ribbon.UseRightToLeftReading then
      ACanvas.TextFlags := ACanvas.TextFlags or ETO_RTLREADING;
    ACanvas.DrawText(ApplicationButton.Text, TextBounds, taCenter, vaCenter,
      False, False);
    ACanvas.Brush.Style := bsSolid;
  end;
end;
...
begin
  ...
  ARect := ABounds;
  ARect.Left := Max(Owner.ApplicationButtonViewInfo.Bounds.Right,
    dxRibbonTabsLeftSpace);
...
begin
  ...
  cxRedrawWindow(SearchToolbar.DockControl.Handle, RDW_INVALIDATE or
    RDW_ALLCHILDREN);
end;
...
function TdxRibbonTabsAreaViewInfo.GetHitInfo(const AHitInfo
  : TdxRibbonHitInfo): Boolean;
begin
  Result := TabsViewInfo.GetHitInfo(AHitInfo) or
    ButtonsViewInfo.GetHitInfo(AHitInfo);
end;

function TdxRibbonTabsAreaViewInfo.IsSearchToolbarVisible: Boolean;
begin
  Result := Owner.CanShowBarControls and IsTabsVisible and
    SearchToolbar.ActualVisible
end;
...
procedure TdxRibbonTabsAreaViewInfo.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
begin
...
begin
  ButtonsViewInfo.Calculate(cxRectSetHeight(ARect,
    MeasuredHeight - dxRibbonTabsAreaBottomIndent));
  ARect.Right := Min(ButtonsViewInfo.Bounds.Left - 1,
    ARect.Right - dxRibbonTabsRightSpace);
end;
...
begin
  ...
  begin
    ...
    FToolbarBounds := cxRectSetRight(ToolbarBounds, ToolbarBounds.Right,
      ASize.cx);
    ARect.Right := ToolbarBounds.Left;
  end;
end;

function TdxRibbonTabsAreaViewInfo.CalculateToolbarSize(ABar: TdxBar;
AAvailableWidth: Integer): TSize;
var
...
begin
  ...
  if AControl is TdxRibbonCustomToolbarBarControl then
    Result := TdxRibbonCustomToolbarBarControl(AControl)
      .GetSize(AAvailableWidth)
  else
    Result := cxNullSize;
end;
...
function TdxRibbonTabsAreaViewInfo.CreateButtonsViewInfo
  : TdxRibbonTabsAreaButtonsViewInfo;
begin
...
procedure TdxRibbonTabsAreaViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);

  ButtonsViewInfo.RightToLeftConversion(ABounds);
  FToolbarBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FToolbarBounds, ABounds);
  if IsTabsVisible and (TabsViewInfo.Count > 0) then
    TabsViewInfo.RightToLeftConversion(ABounds);
  FSearchToolbarBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FSearchToolbarBounds, ABounds);
  FExtendedCaptionAreaRect := TdxRightToLeftLayoutConverter.ConvertRect
    (FExtendedCaptionAreaRect, ABounds);
end;
...
begin
  ...
  if IsSearchToolbarVisible then
    Result := Max(Result, GetBarHeight(SearchToolbar) +
      ScaleFactor.Apply(dxRibbonTabsAreaBottomIndent));
  if IsToolbarVisible then
    Result := Max(Result, GetBarHeight(Toolbar) +
      ScaleFactor.Apply(dxRibbonTabsAreaBottomIndent));
end;

procedure TdxRibbonTabsAreaViewInfo.UpdateToolbarDockControls;
begin
  Owner.UpdateToolbarDockControl(Toolbar.DockControl,
    not cxRectIsEmpty(ToolbarBounds), ToolbarBounds);
  Owner.UpdateToolbarDockControl(SearchToolbar.DockControl,
    not cxRectIsEmpty(SearchToolbarBounds), SearchToolbarBounds);
end;
...
procedure TdxRibbonTabsAreaViewInfo2010OrNewer.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
...
function TdxRibbonTabsAreaViewInfo2010OrNewer.MeasureHeight
  (ACanvas: TcxCanvas): Integer;
begin
  Result := inherited;
  if ApplicationButtonViewInfo.IsVisible and ApplicationButtonViewInfo.HasText
  then
    Result := Max(Result, ApplicationButtonViewInfo.FTextSize.cy +
      cxMarginsHeight(Owner.Painter.GetTabContentOffset));
end;

function TdxRibbonTabsAreaViewInfo2010OrNewer.GetApplicationButtonViewInfo
  : TdxRibbonApplicationButtonViewInfo2010OrNewer;
begin
  Result := Owner.ApplicationButtonViewInfo as
    TdxRibbonApplicationButtonViewInfo2010OrNewer;
end;
...
procedure TdxRibbonTabsAreaViewInfo2016Tablet.CalculateContent
  (var ARect: TRect);
begin
  ...
  begin
    ...
    ARect.Left := Max(ARect.Left, QuickAccessToolbarViewInfo.Bounds.Right +
      Ribbon.ColorScheme.GetQuickAccessToolbarRightIndent
      (Ribbon.ViewInfo.IsApplicationButtonNearQAT) +
      TabsViewInfo.FTabContentOffset.Left);
  end;
  inherited;
end;

function TdxRibbonTabsAreaViewInfo2016Tablet.CreateTabsViewInfo
  : TdxRibbonTabsViewInfo;
begin
  Result := TdxRibbonTabsViewInfo2016Tablet.Create(Owner);
end;

function TdxRibbonTabsAreaViewInfo2016Tablet.GetQuickAccessToolbarViewInfo
  : TdxRibbonQuickAccessToolbarViewInfo;
begin
...
function TdxRibbonTabsAreaViewInfo2019.CreateTabsViewInfo
  : TdxRibbonTabsViewInfo;
begin
...
begin
  ...
  if Visible then
  begin
    CalculateControlSize(cxRectWidth(ABounds) -
      cxMarginsWidth(DockControlOffsets));

    FBounds := cxRectSetHeight(Bounds, ControlSize.cy +
      cxMarginsHeight(DockControlOffsets));
    if AtTop then
      FBounds := cxRectSetWidth(Bounds, ControlSize.cx +
        cxMarginsWidth(DockControlOffsets));
    ...
    if Toolbar.Toolbar.Control <> nil then
      Toolbar.Toolbar.Control.Enabled := Ribbon.ApplicationMenuState <=
        ramsShownAsMenu;
  end
  else
  begin
...
procedure TdxRibbonQuickAccessToolbarViewInfo.CalculateMetrics
  (ACanvas: TcxCanvas);
begin
  inherited;
...
begin
  ...
  begin
    ...
    ColorScheme.DrawQuickAccessToolbar(ACanvas.Handle, Bounds, AtBottom,
      AtNonClientArea, Owner.IsApplicationButtonNearQAT, IsActive,
      not IsPaintOnGlass);
    ColorScheme.UseRightToLeftAlignment := False;
  end;
end;

procedure TdxRibbonQuickAccessToolbarViewInfo.DrawBackground
  (ACanvas: TcxCanvas);
begin
  ACanvas.SaveClipRegion;
  try
...
begin
  Result := not AtNonClientArea or Owner.Painter.GetRibbonFormPaintData.
    GetIsActive;
end;

function TdxRibbonQuickAccessToolbarViewInfo.
  IsNonClientDrawingSupported: Boolean;
begin
...
procedure TdxRibbonQuickAccessToolbarViewInfo.CalculateControlSize
...
begin
  ...
  begin
    ...
    begin
      FControlSize := TdxRibbonQuickAccessToolbarBarControl(AControl)
        .GetSize(AAvailableWidth);
      if Ribbon.ApplicationMenuState = ramsShownAsFullScreenFrame then
        FControlSize.cx := 0
    end;
...
begin
  ...
  begin
    ...
    FDockControlOffsets.Left :=
      1 + ColorScheme.GetQuickAccessToolbarOverrideWidth
      (Owner.IsApplicationButtonNearQAT, IsPaintOnGlass);
    ...
    FDockControlOffsets := Ribbon.SkinGetContentOffsets
      (DXBAR_QUICKACCESSTOOLBAR);
end;

procedure TdxRibbonQuickAccessToolbarViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  FDockControlBounds := TdxRightToLeftLayoutConverter.ConvertRect
...
function TdxRibbonQuickAccessToolbarViewInfo.GetColorScheme
  : TdxCustomRibbonSkin;
begin
  Result := Ribbon.ColorScheme;
end;

function TdxRibbonQuickAccessToolbarViewInfo.GetDockControl
  : TdxRibbonQuickAccessToolbarDockControl;
begin
...
function TdxRibbonQuickAccessToolbarViewInfo.GetToolbar
  : TdxRibbonQuickAccessToolbar;
begin
...
procedure TdxRibbonQuickAccessToolbarViewInfo2016Tablet.DrawBackground
  (ACanvas: TcxCanvas);
begin
  Owner.DrawRibbonBackground(ACanvas);
end;

function TdxRibbonQuickAccessToolbarViewInfo2016Tablet.
  IsNonClientDrawingSupported: Boolean;
begin
  Result := False;
end;

procedure TdxRibbonQuickAccessToolbarViewInfo2016Tablet.
  CalculateDockControlOffsets;
begin
  if AtTop then
    FDockControlOffsets := cxNullRect
...
procedure TdxRibbonViewInfo.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
begin
...
procedure TdxRibbonViewInfo.UpdateToolbarDockControl(ADockControl
  : TdxCustomRibbonDockControl; AIsVisible: Boolean; const ABounds: TRect);
begin
  if not(Ribbon.IsLoading or Ribbon.IsBarManagerValid and
    TdxBarManagerAccess(Ribbon.BarManager).IsUpdateLocked) then
  begin
...
begin
  ...
  begin
    if AForm.HandleAllocated and (AForm.WindowState = wsMinimized) and
      dxIsWindowStyleSet(AForm.Handle, WS_CHILD) then
...
begin
  ...
  else if IsTabGroupsVisible and not Ribbon.IsPopupGroupsMode then
...
begin
  ...
  else if QuickAccessToolbarViewInfo.AtTop then
...
begin
  ...
  if SupportNonClientDrawing and IsTabsVisible and
    (TabsAreaViewInfo.TabsViewInfo.Count > 0) then
  begin
    ...
    begin
      ...
      if IsApplicationButtonNearQAT then
        Result.Left := ApplicationButtonViewInfo.Bounds.Right -
          ColorScheme.GetQuickAccessToolbarOverrideWidth(True, AOnGlass)
      else if SupportNonClientDrawing then
        Result.Left := Ribbon.FormCaptionHelper.TextBounds.Left + 4
      else
        Result.Left := Result.Left + 2;

      Inc(Result.Left, ColorScheme.GetQuickAccessToolbarLeftIndent
        (IsApplicationButtonNearQAT, AOnGlass));
      ...
      begin
        ...
        if AOnGlass then
          ...
          ARightIndent := ColorScheme.GetQuickAccessToolbarRightIndent
            (IsApplicationButtonNearQAT);

        Inc(ARect.Left, ARightIndent);
        if AOnGlass then
          Dec(ARect.Right, 2 * dxCaptionGlowRadius);
        Result.Left := Max(Result.Left, ARect.Left);
        Result.Right := ARect.Right - ScaleFactor.Apply
          (dxRibbonFormCaptionMinWidth);
...
begin
  TabsAreaViewInfo.Calculate(cxRectSetTop(Bounds, GetTabsVerticalOffset,
    TabsAreaViewInfo.MeasuredHeight));
end;
...
begin
  if Hidden then
    ...
    Result := not ApplicationButtonViewInfo.IsVisible or
      (Painter.GetRibbonFormPaintData.GetState = wsMinimized);
end;

function TdxRibbonViewInfo.CanShowBarControls(AIgnoreHidden
...
function TdxRibbonViewInfo.CreateApplicationButtonViewInfo
  : TdxRibbonApplicationButtonViewInfo;
begin
...
function TdxRibbonViewInfo.CreateQuickAccessToolbarViewInfo
  : TdxRibbonQuickAccessToolbarViewInfo;
begin
...
begin
  FBounds := TdxRightToLeftLayoutConverter.ConvertRect(FBounds, ABounds);
  FGroupsDockControlSiteBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FGroupsDockControlSiteBounds, ABounds);
  FTabGroupsDockControlBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FTabGroupsDockControlBounds, ABounds);
  ...
  FFormCaptionBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FFormCaptionBounds, ABounds);
...
begin
  ...
  begin
    ...
    if QuickAccessToolbarViewInfo.AtNonClientArea then
      Result.Left := QuickAccessToolbarViewInfo.Bounds.Right +
...
begin
  ...
  if not UseGlass then
    InflateRect(ACenterRect,
      -ScaleFactor.Apply(dxRibbonFormCaptionTextSpace), 0);

  W := cxRectWidth(ACenterRect) - ATextWidth;
  Inc(ACenterRect.Left, W div 2);
  Dec(ACenterRect.Right, W div 2 - 1);
  if ContextsViewInfo.Count > 0 then
    Result := GetRibbonFormCaptionTextBoundsWithContext(Result, ACenterRect,
      ATextWidth)
  else if (W >= 0) and cxRectContain(Result, ACenterRect) then
    Result := ACenterRect;
end;
...
begin
  // try center
  ALeftRect := cxRect(ABounds.Left, ABounds.Top, ContextsViewInfo.Bounds.Left,
    ABounds.Bottom);
  if not UseGlass then
  ...
  ARightRect := cxRect(ContextsViewInfo.Bounds.Right, ABounds.Top,
...
const
  Parts: array [Boolean] of Integer = (DXBAR_RIBBONTABGROUP,
...
function TdxRibbonViewInfo.GetTabGroupsHeight(AIgnoreHidden
  : Boolean = False): Integer;
begin
  if IsTabGroupsVisible or AIgnoreHidden then
    Result := Ribbon.GetGroupHeight + cxMarginsHeight
      (GetTabGroupsDockControlOffset)
  else
    Result := 0;
end;
...
begin
  if not Hidden and IsTabsVisible and Ribbon.IsBarManagerValid then
    Result := Ribbon.BarManager.IsMDIMaximized and
      (GetSystemMenu(Ribbon.BarManager.ActiveMDIChild, False) <> 0)
  else
    Result := False;
end;
...
begin
  Result := IsTabsVisible and (TabsAreaViewInfo.TabsViewInfo.Count > 0) and
    (ContextsViewInfo.Count > 0);
end;

function TdxRibbonViewInfo.IsTabGroupsVisible(AIgnoreHidden
  : Boolean = False): Boolean;
begin
  Result := CanShowBarControls(AIgnoreHidden) and
    (Ribbon.ShowTabGroups or Ribbon.IsPopupGroupsMode) and
    (TabsAreaViewInfo.TabsViewInfo.Count > 0);
end;
...
begin
  Result := IsTabsVisible and not(QuickAccessToolbarViewInfo.AtBottom or
    IsTabGroupsVisible or IsTabsOnGlass);
end;
...
begin
  ...
  begin
    ...
    Result := (AForm <> nil) and
      ((IsIconic(AForm.Handle) and IsShowMinimizedOnDesktop(AForm.Handle)) or
      (AForm.Width < ScaleFactor.Apply(dxRibbonOwnerMinimalWidth)) or
      (AForm.Height < ScaleFactor.Apply(dxRibbonOwnerMinimalHeight))) or
      Ribbon.IsAutoHidden;
  end;
end;
...
begin
  Result := Ribbon.EnableTabAero and UseGlass and
    ColorScheme.ExtendCaptionAreaOnTabs;
end;
...
begin
  if ApplicationButtonViewInfo.IsVisible then
    ApplicationButtonViewInfo.Calculate
      (cxRect(ApplicationButtonViewInfo.MeasureSize))
  else
...
function TdxRibbonViewInfo2007.CreateContextsViewInfo
  : TdxRibbonContextsViewInfo;
begin
...
function TdxRibbonViewInfo2010OrNewer.CreateApplicationButtonViewInfo
  : TdxRibbonApplicationButtonViewInfo;
begin
  Result := TdxRibbonApplicationButtonViewInfo2010OrNewer.Create(Self);
end;

function TdxRibbonViewInfo2010OrNewer.CreateContextsViewInfo
  : TdxRibbonContextsViewInfo;
begin
  Result := TdxRibbonContextsViewInfo2010OrNewer.Create(Self);
end;

function TdxRibbonViewInfo2010OrNewer.CreateTabsAreaViewInfo
  : TdxRibbonTabsAreaViewInfo;
begin
...
function TdxRibbonViewInfo2010.CreateTabsAreaViewInfo
  : TdxRibbonTabsAreaViewInfo;
begin
...
procedure TdxRibbonViewInfo2013OrNewer.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  FBackgroundImageBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FBackgroundImageBounds, ABounds);
  FBackgroundImageClipBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FBackgroundImageClipBounds, ABounds);
  inherited DoRightToLeftConversion(ABounds);
end;
...
begin
  Result := inherited;
  Inc(Result.Right, GroupsDockControlSiteViewInfo.
    GetMinimizeRibbonButtonSize.cx + 2);
end;
...
function TdxRibbonViewInfo2016Tablet.CreateQuickAccessToolbarViewInfo
  : TdxRibbonQuickAccessToolbarViewInfo;
begin
  Result := TdxRibbonQuickAccessToolbarViewInfo2016Tablet.Create(Self);
end;

function TdxRibbonViewInfo2016Tablet.CreateTabsAreaViewInfo
  : TdxRibbonTabsAreaViewInfo;
begin
...
procedure TdxRibbonViewInfo2016Tablet.PopulateHorizontalNavigationList
  (AProc: TdxRibbonAddAccessibilityHelperProc);
begin
  inherited;
  dxRibbonAccessibilityEnumChildren
    (QuickAccessToolbarViewInfo.Toolbar.IAccessibilityHelper, AProc);
end;
...
function TdxRibbonViewInfo2019.CreateTabsAreaViewInfo
  : TdxRibbonTabsAreaViewInfo;
begin
...
function TdxRibbonBarControlAccessibilityHelper.LogicalNavigationGetNextChild
  (AChildIndex: Integer; AShift: TShiftState): TdxBarAccessibilityHelper;
begin
...
constructor TdxRibbonBarButtonLikeControlDrawParams.Create
  (ARibbon: TdxCustomRibbon);
begin
...
begin
  ...
  try
    ...
    (ABarControl as TdxRibbonCustomBarControl).DrawBarParentBackground
      (cxPaintCanvas);
...
begin
  ...
  FCollapsedGroupElementSizeNumerator := GetSmallIconSize +
    cxMarginsHeight(Skin.GetContentOffsets
    (DXBAR_COLLAPSEDTOOLBARGLYPHBACKGROUND)) +
    cxMarginsHeight(dxRibbonCollapsedGroupGlyphBackgroundOffsets);
  FCollapsedGroupElementSizeDenominator :=
    Skin.GetContentOffsets(DXBAR_COLLAPSEDTOOLBAR).Top +
    GetButtonHeight(GetSmallIconSize, DefaultFontHeight +
    dxRibbonGroupRowHeightCorrection, DrawParams.ScaleFactor) * GetGroupRowCount
    + GetGroupCaptionHeight(DefaultFontHeight) +
    (Ribbon.Painter.GetGroupCaptionBottomOffset + 1) -
    cxMarginsHeight(Skin.GetContentOffsets(DXBAR_COLLAPSEDTOOLBAR));

  FGroupCaptionTextHeight := TdxTextMeasurer.TextLineHeight
    (Ribbon.Fonts.GetGroupHeaderFont);
  FGroupRowTextHeight := TdxTextMeasurer.TextLineHeight
    (Ribbon.Fonts.GetGroupFont);
end;

function TdxRibbonBarPainter.CalculateRowHeight(AIconSize, ATextHeight: Integer;
AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := GetButtonHeight(AIconSize,
    ATextHeight + AScaleFactor.Apply(dxRibbonGroupRowHeightCorrection),
    AScaleFactor);
end;

function TdxRibbonBarPainter.GetGlyphColorPalette(ABarItemControl
  : TdxBarItemControl; APaintType: TdxBarPaintType; AState: Integer)
  : IdxColorPalette;
begin
...
begin
  Result := CalculateRowHeight(AIconSize, FGroupRowTextHeight,
    DrawParams.ScaleFactor);
end;

function TdxRibbonBarPainter.DropDownGalleryGetScrollBarWidth
  (AControlHeight: Integer; const AControlMargins: TRect): Integer;
begin
  if IsSimplifiedGroupsLayout then
  begin
...
begin
  if (ABar.Control is TdxRibbonGroupBarControl) and
    TdxRibbonGroupBarControl(ABar.Control).Collapsed then
    Result := cxEmptyRect
  else
    Result := inherited GetToolbarContentOffsets(ABar, ADockingStyle,
      AScaleFactor, AHasSizeGrip);
end;
...
procedure TdxRibbonBarPainter.DrawCollapsedToolbarBackgroundPart
  (ABarControl: TdxRibbonGroupBarControl; ACanvas: TcxCanvas;
AGroupState: Integer);
begin
  if not dxFader.DrawFadeImage(ABarControl, ACanvas.Handle,
    ABarControl.ClientRect) then
    Skin.DrawBackground(ACanvas.Handle, ABarControl.ClientRect,
      DXBAR_COLLAPSEDTOOLBAR, AGroupState);
end;
...
begin
  ...
  AGroupGlyphBackgroundSize := GetCollapsedGroupGlyphBackgroundSize
    (ABarControl);
  R := cxRectContent(ABarControl.ClientRect,
    Skin.GetContentOffsets(DXBAR_COLLAPSEDTOOLBAR));
  ...
  Skin.DrawBackground(ACanvas.Handle, R, DXBAR_COLLAPSEDTOOLBARGLYPHBACKGROUND,
    AGroupState);
  ...
  begin
    ...
    AGroupGlyph.StretchDraw(ACanvas.Handle,
      cxRectCenter(R, GetCollapsedGroupGlyphSize(ABarControl)));
  end;
end;

procedure TdxRibbonBarPainter.DrawToolbarContentPart(ABarControl: TdxBarControl;
ACanvas: TcxCanvas);

  function DrawFadeBackground(AGroupBarControl
    : TdxRibbonGroupBarControl): Boolean;
  var
  ...
  begin
    ...
    try
      Result := dxFader.DrawFadeImage(AGroupBarControl, ACanvas.Handle,
...
begin
  ...
  else if not DrawFadeBackground(AGroupBarControl) then
...
begin
  ...
  if not FCollapsedGroupWidthCache.TryGetValue(ABarControl.GetCaption, Result)
  then
  begin
...
function TdxRibbonBarPainter.DropDownGalleryGetSizingBandHeight(AIconSize,
  ATextSize: Integer; AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := CalculateRowHeight(AIconSize, ATextSize, AScaleFactor);
end;

function TdxRibbonBarPainter.GetButtonPartState(const ADrawParams
  : TdxBarButtonLikeControlDrawParams; AControlPart: Integer): Integer;

  function GetCustomButtonPartState: Integer;
  begin
    if ADrawParams.IsPressed then
      Result := DXBAR_PRESSED
    else if ADrawParams.Downed then
    begin
      if ADrawParams.DroppedDown then
        Result := DXBAR_ACTIVE
      else if (ADrawParams.HotPartIndex = AControlPart) or
        ADrawParams.SelectedByKey and not ADrawParams.IsDropDown then
        Result := DXBAR_HOTCHECK
      else
        Result := DXBAR_CHECKED
    end
    else if ADrawParams.DroppedDown then
  ...
  begin
    if ADrawParams.DroppedDown then
      Result := DXBAR_DROPPEDDOWN
    else if ADrawParams.Downed and not ADrawParams.SelectedByKey and
      (ADrawParams.HotPartIndex <> AControlPart) then
      Result := DXBAR_CHECKED
    else
      Result := GetPartState(ADrawParams, AControlPart);
  end;
...
function TdxRibbonBarPainter.GetCollapsedGroupCaptionRect(const AGroupRect
  : TRect): TRect;
begin
  Result := cxRectContent(AGroupRect,
    Skin.GetContentOffsets(DXBAR_COLLAPSEDTOOLBAR));
  Result := cxRectInflate(Result, -1, 0);
  if IsSimplifiedGroupsLayout then
    Result.Top := Result.Bottom
  else
    Inc(Result.Top, cxRectHeight(Result) * FCollapsedGroupElementSizeNumerator
      div FCollapsedGroupElementSizeDenominator);
end;

function TdxRibbonBarPainter.GetGroupMinWidth(ABarControl
  : TdxRibbonGroupBarControl): Integer;
begin
  if IsSimplifiedGroupsLayout then
    Exit(ScaleFactor.Apply(dxRibbonGroupMinContentWidth));

  Result := cxTextWidth(ABarControl.Ribbon.Fonts.GetGroupHeaderFont,
    ABarControl.GetCaption) + 2 * cxTextSpace;
...
function TdxRibbonBarPainter.GetCollapsedGroupGlyph(ABarControl: TdxBarControl)
  : TdxSmartGlyph;
begin
...
begin
  AGroupContentHeight := ABarControl.Height -
    cxMarginsWidth(Skin.GetContentOffsets(DXBAR_COLLAPSEDTOOLBAR));
  Result.cy := cxMarginsHeight
    (Skin.GetContentOffsets(DXBAR_COLLAPSEDTOOLBARGLYPHBACKGROUND)) +
    GetSmallIconSize;
  Result.cy := Result.cy * AGroupContentHeight div
    FCollapsedGroupElementSizeDenominator;
  Result.cx := Result.cy;
end;

function TdxRibbonBarPainter.GetCollapsedGroupGlyphSize
  (ABarControl: TdxBarControl): TSize;
var
  AGlyphSize: TSize;
  AGroupGlyph: TdxSmartGlyph;
begin
  ...
  if (AGlyphSize.cx <= GetSmallIconSize) and (AGlyphSize.cy <= GetSmallIconSize)
  then
    Result := AGlyphSize
  else if AGlyphSize.cx > AGlyphSize.cy then
  begin
...
const
  GroupStates: array [TdxBarMarkState] of Integer = (DXBAR_NORMAL, DXBAR_HOT,
    DXBAR_PRESSED);
begin
  if ABarControl.IAccessibilityHelper.IsSelected then
    Result := DXBAR_ACTIVE
  else
    Result := GroupStates[(ABarControl as TdxRibbonGroupBarControl)
      .MarkDrawState];
end;
...
begin
  ...
  cxPaintControlTo(ADockControl, FImage.cxCanvas, cxNullPoint,
    cxGetClientRect(ADockControl), True, False, True, False);
  FImage.MakeOpaque;
end;
...
procedure TdxRibbonGroupsDockControlContentChangeAnimation.Draw
...
procedure TdxRibbonGroupsDockControl.SetBounds(ALeft, ATop, AWidth,
  AHeight: Integer);
var
  R: TRect;
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment then
      SiteViewInfo.RightToLeftConversion
        (GetControlRect(Ribbon.GroupsDockControlSite));
    R := Tab.GetDockControlBounds;
    if UseRightToLeftAlignment then
    begin
      Dec(AWidth, Max(0, SiteViewInfo.TabGroupsScrollButtonRight.Bounds.Right
        - R.Left));
      Dec(AWidth, Max(0, R.Right - SiteViewInfo.TabGroupsScrollButtonLeft.
        Bounds.Left));
      Inc(ALeft, Max(0, SiteViewInfo.TabGroupsScrollButtonRight.Bounds.Right
        - R.Left));
    end
    else
    begin
      Dec(AWidth, Max(0, SiteViewInfo.TabGroupsScrollButtonLeft.Bounds.Right
        - R.Left));
      Dec(AWidth, Max(0, R.Right - SiteViewInfo.TabGroupsScrollButtonRight.
        Bounds.Left));
      Inc(ALeft, Max(0, SiteViewInfo.TabGroupsScrollButtonLeft.Bounds.Right
        - R.Left));
    end;
...
procedure TdxRibbonGroupsDockControl.CalcRowToolbarPositions(ARowIndex,
  AClientSize: Integer);
begin
  if Visible then
...
function TdxRibbonGroupsDockControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonGroupsDockControlAccessibilityHelper;
end;

function TdxRibbonGroupsDockControl.GetDockedBarControlClass
  : TdxBarControlClass;
begin
...
function TdxRibbonGroupsDockControl.GetViewInfoClass
  : TdxRibbonGroupsDockControlViewInfoClass;
begin
...
begin
  ...
  begin
    ...
    if ANewLeft <> R.Left then
      ViewInfo.InternalScrollGroups(ANewLeft - R.Left,
        cxRectWidth(Ribbon.ViewInfo.GetTabGroupsDockControlBounds));
  end;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          DeferWindowPos(AHandle, AToolbar.Handle, 0, ARect.Left, ARect.Top,
            cxRectWidth(ARect), cxRectHeight(ARect), SWP_DRAWFRAME or
            SWP_NOACTIVATE or SWP_NOOWNERZORDER or SWP_NOZORDER or
...
begin
  ...
  begin
    ...
    begin
      FContentChangeAnimation :=
        TdxRibbonGroupsDockControlContentChangeAnimation.Create(Self,
...
begin
  Result := (dxRibbonTabGroupContentShowAnimationTime > 0) and
    (Ribbon.Style >= rs2019) and not Ribbon.IsDesigning and
    (not Ribbon.IsPopupGroupsMode or
    Ribbon.TabGroupsPopupWindow.SwitchingBetweenTabs) and
    not BarNavigationController.KeyTipsHandlingMode and
    not Ribbon.IsAutoHideAnimationIsInProgress;
end;
...
function TdxRibbonGroupsDockControl.GetSiteViewInfo
  : TdxRibbonGroupsDockControlSiteViewInfo;
begin
...
procedure TdxRibbonGroupsDockControl.HandlerAnimationTerminated
  (Sender: TObject);
begin
...
begin
  case TdxBarButton(Sender).Tag of
    0:
      Ribbon.Tabs.Add.DesignSelectionHelper.SelectComponent;
    1:
      Ribbon.DesignAddTabGroup(Tab, False);
    2:
      Ribbon.DesignAddTabGroup(Tab, True);
  end;
end;

procedure TdxRibbonGroupsDockControl.InitDesignMenu
  (AItemLinks: TdxBarItemLinks);
begin
...
begin
  BarDesignController.ShowCustomCustomizePopup(BarManager,
    InitDesignMenu, Painter);
end;

procedure TdxRibbonGroupsDockControl.WMGestureNotify
  (var Message: TWMGestureNotify);
begin
  Message.Result := DefWindowProc(Handle, Message.Msg, Message.Unused,
    lParam(Message.NotifyStruct));
end;
...
procedure TdxRibbonGroupsDockControlViewInfo.Calculate
  ...
  begin
    for I := 0 to GroupCount - 1 do
      GroupViewInfos[I].BeforeCalculate
        (GroupViewInfos[I].BarControl.Group.Restriction = rtgrNoExpand);
  end;
  ...
  function ReduceGroups(AStage: TdxRibbonGroupsReduceStage;
  AUpToViewLevel: TdxBarItemRealViewLevel): Boolean;
  var
    I: Integer;
  begin
...
begin
  ...
  try
    ...
    begin
      ReduceGroupsInit;
      for AGroupsReduceStage := Low(TdxRibbonGroupsReduceStage)
        to High(TdxRibbonGroupsReduceStage) do
      begin
        if AGroupsReduceStage = rgrsItemControlsViewLevel then
        begin
          for AUpToViewLevel := Succ(Low(TdxBarItemRealViewLevel))
            to High(TdxBarItemRealViewLevel) do
            if ReduceGroups(AGroupsReduceStage, AUpToViewLevel) then
              Break;
        end
        else if ReduceGroups(AGroupsReduceStage, ivlLargeIconWithText) then
          Break;
...
procedure TdxRibbonGroupsDockControlViewInfo.RightToLeftConversion
  (const ABounds: TRect);
begin
  if not FIsRightToLeftConverted then
  begin
...
begin
  ...
  begin
    if FScrollButtons = [] then
      FScrollButtons := [rsbRight]
    else if FScrollButtons = [rsbLeft] then
      FScrollPosition := AMaxContentWidth - ATotalGroupsWidth
    else if FScrollButtons = [rsbLeft, rsbRight] then
    begin
...
procedure TdxRibbonGroupsDockControlViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
var
  I: Integer;
begin
...
procedure TdxRibbonGroupsDockControlViewInfo.InternalScrollGroups
  ...
  begin
    if FScrollPosition > 0 then
      ...
      FScrollPosition := Max(FScrollPosition,
        AMaxContentWidth - TotalGroupsWidth);
  end;
...
begin
  ...
  begin
    ATabGroupsDockControlOffset :=
      DockControl.Ribbon.ViewInfo.GetTabGroupsDockControlOffset;
    if DockControl.UseRightToLeftAlignment then
      Dec(Result, DockControl.Parent.ClientWidth - DockControl.BoundsRect.Right
        - ATabGroupsDockControlOffset.Left)
    else
...
function TdxRibbonGroupsDockControlViewInfo.GetGroupViewInfo(AIndex: Integer)
  : TdxRibbonGroupBarControlViewInfo;
var
  AToolbar: TdxBar;
  I: Integer;
...
function TdxRibbonGroupsDockControlViewInfo.IsValidToolbar
  (AToolbar: TdxBar): Boolean;
begin
  Result := (AToolbar <> nil) and (AToolbar.Control <> nil) and
    (AToolbar.Control.DockControl = DockControl);
end;
...
begin
  ...
  if GroupCount > 1 then
    Inc(Result, DockControl.Painter.GetToolbarsOffsetForAutoAlign *
      (GroupCount - 1));
end;

function TdxRibbonGroupsDockControlViewInfo.TryPlaceGroups(AMaxContentWidth
  : Integer): Boolean;
var
...
function TdxRibbonTabGroupsPopupWindow.CalculatePosition
...
begin
  if (ActiveBarControl = nil) or
    not(bsHideAll in TCustomdxBarControlAccess(ActiveBarControl).FState) then
...
procedure TdxRibbonTabGroupsPopupWindow.KeyDown(var Key: Word;
Shift: TShiftState);
begin
...
function TdxRibbonTabGroupsPopupWindow.NeedIgnoreMouseMessageAfterCloseUp
  (AWnd: THandle; AMsg: Cardinal; AShift: TShiftState;
...
begin
  ...
  begin
    ...
    SelectFirstSelectableAccessibleObject
      (GroupsDockControlSite.DockControl.IAccessibilityHelper.GetBarHelper);
  end;
end;
...
begin
  ...
  begin
    ...
    if Ribbon.UseRightToLeftAlignment then
      ATabGroupsDockControlOffset :=
        TdxRightToLeftLayoutConverter.ConvertOffsets
        (Ribbon.ViewInfo.GetTabGroupsDockControlOffset)
    else
      ATabGroupsDockControlOffset :=
        Ribbon.ViewInfo.GetTabGroupsDockControlOffset;
    R := cxRectContent(GroupsDockControlSite.BoundsRect,
      ATabGroupsDockControlOffset);
...
begin
  ...
  ATabsRect := dxMapWindowRect(Ribbon.Handle, 0,
    Ribbon.ViewInfo.TabsAreaViewInfo.TabsViewInfo.Bounds);
  Result := cxRect(ARibbonRect.Left, ATabsRect.Top - 1, ARibbonRect.Right,
    ATabsRect.Bottom);
  ...
  if (Result.Bottom + ATabGroupsHeight > AMonitorRect.Bottom) and
    (Ribbon.Style < rs2013) then
    Result := cxRect(Result.Left, Result.Top - ATabGroupsHeight, Result.Right,
      Result.Top)
  else
    Result := cxRect(Result.Left, Result.Bottom, Result.Right,
      Result.Bottom + ATabGroupsHeight);
end;

function TdxRibbonTabGroupsPopupWindow.GetGroupsDockControlSite
  : TdxRibbonGroupsDockControlSite;
begin
...
begin
  ...
  if Message.Result = 0 then
    Message.Result := GetParentForm(Ribbon).Perform(Message.Msg, Message.wParam,
      Message.lParam);
end;
...
procedure TdxRibbonTabGroupsPopupWindow.WMShowWindow
  (var Message: TWMShowWindow);
const
  FlagsMap: array [Boolean] of Integer = (AW_SLIDE or AW_VER_NEGATIVE or
    AW_HIDE, AW_SLIDE or AW_VER_POSITIVE);
var
  AAnimationTime: Integer;
begin
  if AllowShowHideAnimation and not FSwitchingBetweenTabs and
...
begin
  ...
  if Ribbon.UseRoundedWindowCorners then
    SetWindowRgn(Handle, CreateRoundRectRgn(0, 0, Message.Width + 1,
      Message.Height + 1, 4, 4), True);
end;
...
function TdxRibbonCustomBarControl.CanAlignControl
...
function TdxRibbonCustomBarControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
...
function TdxRibbonCustomBarControl.GetFullItemRect
  (Item: TdxBarItemControl): TRect;
begin
...
function TdxRibbonCustomBarControl.NotHandleMouseMove(ACheckLastMousePos
  : Boolean = True): Boolean;
begin
  Result := inherited NotHandleMouseMove(ACheckLastMousePos) or
    dxBarHasPopupWindowAbove(Self, True);
end;
...
procedure TdxRibbonCustomBarControl.InitCustomizationPopup
  (AItemLinks: TdxBarItemLinks);
begin
...
begin
  ...
  begin
    ...
    if (AItemLink <> nil) or ClickAtHeader then
      BarDesignController.ShowCustomCustomizePopup(BarManager,
        InitCustomizationPopup, Painter, Self, AItemLink);
  end
  else
    inherited;
end;

function TdxRibbonCustomBarControl.GetQuickAccessToolbar
  : TdxRibbonQuickAccessToolbar;
var
  ARibbon: TdxCustomRibbon;
begin
...
begin
  ...
  begin
    ...
    if (AItemLink.VisibleIndex = -1) or (AItemLink.Control <> nil) and
      IsRectEmpty(AItemLink.ItemRect) then
    begin
      Result := False;
      Break;
    end;
...
procedure TdxRibbonCustomToolbarBarControl.CalcControlsPositions;
  ...
  begin
    ...
    try
      for I := 0 to ItemLinks.VisibleItemCount - 1 do
        AItemControlViewInfos.Add
          (Pointer(IdxBarItemControlViewInfo(ItemLinks.VisibleItems[I]
          .Control.ViewInfo)));
      dxRibbonGroupLayoutCalculator.CalcItemControlsRealPositionInButtonGroup
...
begin
  ...
  try
    ...
    begin
      ...
      Truncated := X + ASeparatorWidth + AItemControlWidth > ClientWidth -
        GetMarkSize;
      if Truncated then
      begin
        if I > 0 then
      ...
      AItemRect := Rect(X, R.Top, X + AItemControlWidth,
        ClientHeight - R.Bottom);
      if UseRightToLeftAlignment then
        AItemRect := TdxRightToLeftLayoutConverter.ConvertRect(AItemRect,
          cxRectBounds(0, 0, ClientWidth, 0));
...
procedure TdxRibbonCustomToolbarBarControl.DoPaintItem
...
begin
  ...
  try
    ...
    begin
      ...
      FBitmap.MakeOpaque;
      cxBitBlt(ACanvas.Handle, FBitmap.cxCanvas.Handle, AItemRect,
        cxNullPoint, SRCCOPY);
    end
    else
...
function TdxRibbonCustomToolbarBarControl.GetMinHeight
...
function TdxRibbonCustomToolbarBarControl.GetMinWidth
  (AStyle: TdxBarDockingStyle): Integer;
begin
  Result := 0;
end;
...
function TdxRibbonCustomToolbarBarControl.GetSizeForWidth
...
function TdxRibbonCustomToolbarBarControl.GetSeparatorWidth
...
const
  StatesMap: array [TdxBarMarkState] of Integer = (DXBAR_NORMAL, DXBAR_HOT,
    DXBAR_PRESSED);
var
  AState: Integer;
begin
  AState := StatesMap[TdxRibbonCustomToolbarBarControl(ABarControl)
    .MarkDrawState];
...
procedure TdxRibbonCustomToolbarPainter.DrawBarMarkState
  (ABarControl: TdxBarControl; DC: HDC; const R: TRect;
...
function TdxRibbonCustomToolbarPainter.BarMarkItemRect
  (ABarControl: TdxBarControl): TRect;
begin
  Result := BarMarkRect(ABarControl);
end;

function TdxRibbonCustomToolbarPainter.BarMarkRect
  (ABarControl: TdxBarControl): TRect;
begin
  Result := cxGetClientRect(ABarControl);
...
begin
  ...
  begin
    ...
    try
      ...
      ABitmap.cxCanvas.WindowOrg := cxNullPoint;
      ABitmap.MakeOpaque;
      cxBitBlt(ADrawParams.Canvas.Handle, ABitmap.cxCanvas.Handle, ARect,
        cxNullPoint, SRCCOPY);
    finally
      ABitmap.Free;
    end;
...
function TdxRibbonCustomToolbarPainter.MarkSizeX(ABarControl
  : TdxBarControl): Integer;
begin
  Result := MarkButtonWidth + MarkButtonOffset;
end;

procedure TdxRibbonCustomToolbarPainter.DrawButtonBackground(const ADrawParams
  : TdxBarButtonLikeControlDrawParams);
  ...
  begin
    if AControlPart >= 0 then
      ARect := TdxBarItemControlAccess(ADrawParams.BarItemControl)
        .FParts[AControlPart]
    else
    begin
    ...
    Skin.DrawBackground(ADrawParams.Canvas.Handle, ARect, ASkinPart,
      GetButtonPartState(ADrawParams, AControlPart));
  end;
...
procedure TdxRibbonCustomToolbarPainter.DrawToolbarContentPart
  (ABarControl: TdxBarControl; ACanvas: TcxCanvas);
begin
  if Ribbon.ViewInfo.UseGlass and ABarControl.IsOnGlass then
    FillRect(ACanvas.Handle, ABarControl.ClientRect,
      GetStockObject(BLACK_BRUSH));
end;

function TdxRibbonCustomToolbarPainter.GetButtonBorderHeight
  (AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := cxMarginsHeight(Skin.GetContentOffsets(SkinGetButtonID));
end;

function TdxRibbonCustomToolbarPainter.GetButtonBorderWidth
  (AScaleFactor: TdxScaleFactor): Integer;
begin
...
function TdxRibbonQuickAccessToolbarBarControl.CalcSize
  (AMaxWidth: Integer): TSize;
begin
  Result := inherited CalcSize(AMaxWidth);
  if MarkExists and (Result.cx = GetMarkSize + cxMarginsWidth(GetClientOffset))
  then
    Dec(Result.cx, TdxRibbonQuickAccessToolbarPainter(Painter)
      .MarkButtonOffset);
end;
...
function TdxRibbonQuickAccessToolbarBarControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper;
end;

function TdxRibbonQuickAccessToolbarBarControl.GetDefaultItemGlyph
  : TdxSmartGlyph;
begin
  Result := FDefaultGlyph;
end;

function TdxRibbonQuickAccessToolbarBarControl.GetMarkAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonQuickAccessToolbarBarControlMarkAccessibilityHelper;
end;

function TdxRibbonQuickAccessToolbarBarControl.GetPopupMenuItems
  : TdxRibbonPopupMenuItems;
begin
...
function TdxRibbonQuickAccessToolbarBarControl.GetQuickControlClass
  : TdxBarPopupControlClass;
begin
  Result := TdxRibbonQuickAccessPopupBarControl;
end;

function TdxRibbonQuickAccessToolbarBarControl.GetViewInfoClass
  : TCustomdxBarControlViewInfoClass;
begin
...
procedure TdxRibbonQuickAccessToolbarBarControl.InitQuickCustomizeItemLinks
...
begin
  ...
  begin
    ASubItem := BarDesignController.AddInternalItem(AQuickControl.ItemLinks,
      TdxRibbonQuickAccessPopupSubItem, BarManager, InternalItems)
      .Item as TdxRibbonQuickAccessPopupSubItem;
    ASubItem.OnPopup := HandleQuickAccessSubItemPopup;
  end;
end;

procedure TdxRibbonQuickAccessToolbarBarControl.InitAddRemoveSubItemPopup
  (AItemLinks: TdxBarItemLinks);
var
  I: Integer;
  AItemLink: TdxBarItemLink;
...
begin
  ...
  begin
    ASeparator := BarDesignController.AddInternalItem(AItemLinks,
      TdxBarSeparator, BarManager, InternalItems).Item;
    ...
    begin
      ...
      ASubItemButton := TdxRibbonQuickAccessPopupSubItemButton
        (BarDesignController.AddInternalItem(AItemLinks,
        TdxRibbonQuickAccessPopupSubItemButton, BarManager,
        InternalItems).Item);
      ...
      if AItemLink.Visible then
        ASubItemButton.Hint :=
          RemoveAccelChars(cxGetResourceString(@dxSBAR_REMOVEFROMQAT))
      else
        ASubItemButton.Hint :=
          RemoveAccelChars(cxGetResourceString(@dxSBAR_ADDTOQAT));
    end;
  end;
  QuickAccessToolbar.UpdateMenuItems(AItemLinks);
end;

procedure TdxRibbonQuickAccessToolbarBarControl.InitCustomizationPopup
  (AItemLinks: TdxBarItemLinks);
begin
  Ribbon.PopulatePopupMenuItems(AItemLinks, GetPopupMenuItems, PopupMenuClick);
end;
...
begin
  if BarDesignController.CustomizingItemLink.Item is TdxRibbonQuickAccessGroupButton
  then
    BarDesignController.DeleteCustomizingItem
  else if BarDesignController.CustomizingItemLink.OriginalItemLink <> nil then
    BarDesignController.CustomizingItemLink.OriginalItemLink.Free
  else
    BarDesignController.DeleteCustomizingItemLink;
end;

procedure TdxRibbonQuickAccessToolbarBarControl.ShowPopup
...
begin
  ...
  begin
    if AItem <> nil then
      ...
      AItemLink := nil;
    BarDesignController.ShowCustomCustomizePopup(BarManager,
      InitCustomizationPopup, Ribbon.GroupsPainter, Self, AItemLink);
  end
  else
    inherited;
end;

procedure TdxRibbonQuickAccessToolbarBarControl.UpdateDefaultGlyph
...
begin
  ...
  try
    Ribbon.ColorScheme.DrawQuickAccessToolbarDefaultGlyph(ABitmap.Canvas.Handle,
      ABitmap.ClientRect);
...
function TdxRibbonQuickAccessToolbarBarControl.GetViewInfo
  : TdxRibbonQuickAccessToolbarBarControlViewInfo;
begin
...
function TdxRibbonQuickAccessToolbarBarControlViewInfo.IsLastVisibleItemControl
...
function TdxRibbonQuickAccessToolbarPainter.GetGlyphColorPalette(ABarItemControl
  : TdxBarItemControl; APaintType: TdxBarPaintType; AState: Integer)
  : IdxColorPalette;
begin
  if APaintType = ptMenu then
    ...
    Result := ColorScheme.GetQuickAccessToolbarColorPalette(AState,
      QuickAccessToolbarViewInfo.AtBottom);
end;
...
function TdxRibbonQuickAccessToolbarPainter.SkinGetButtonDropDownPartID
  : Integer;
begin
...
procedure TdxRibbonQuickAccessToolbarPainter.DrawGroupButtonControl
  (ADrawParams: TdxBarButtonLikeControlDrawParams; const ARect: TRect);
var
  R: TRect;
begin
  Skin.DrawBackground(ADrawParams.Canvas.Handle, ARect,
    DXRIBBON_QAT_GROUPBUTTON, GetButtonPartState(ADrawParams, bcpButton));
  R := cxRectContent(ARect, Skin.GetContentOffsets(DXRIBBON_QAT_GROUPBUTTON));
  DrawGlyph(ADrawParams.BarItemControl, ADrawParams.Canvas.Handle, R, R, ptHorz,
    False, False, False, False, False, True, False, False);
end;

procedure TdxRibbonQuickAccessToolbarPainter.DrawToolbarContentPart
  (ABarControl: TdxBarControl; ACanvas: TcxCanvas);
begin
  inherited DrawToolbarContentPart(ABarControl, ACanvas);
  ...
  ACanvas.SaveDC;
  try
    ACanvas.WindowOrg := dxMapWindowPoint(ABarControl.Handle, Ribbon.Handle,
      ACanvas.WindowOrg);
    if QuickAccessToolbarViewInfo.AtNonClientArea and
      Assigned(Ribbon.FormCaptionHelper) then
...
function TdxRibbonQuickAccessToolbarPainter.GetQuickAccessToolbarViewInfo
  : TdxRibbonQuickAccessToolbarViewInfo;
begin
  Result := Ribbon.ViewInfo.QuickAccessToolbarViewInfo;
end;
...
function TdxRibbonQuickAccessToolbarDockControl.CreatePainter
  : TdxRibbonCustomToolbarPainter;
begin
  Result := TdxRibbonQuickAccessToolbarPainter.Create(TdxNativeUInt(Ribbon));
end;

function TdxRibbonQuickAccessToolbarDockControl.GetDockedBarControlClass
  : TdxBarControlClass;
begin
...
class procedure TdxRibbonQuickAccessToolbarBarControlDesignHelper.GetEditors
  (AEditors: TList);
begin
...
begin
  AAccessibilityHelper := TdxBarControlMarkAccessibilityHelper
    (BarControl.MarkIAccessibilityHelper.GetHelper);
...
begin
  ...
  begin
    ...
    R := TdxBarAccessibilityHelperAccess
      (BarControl.MarkIAccessibilityHelper.GetHelper)
      .GetScreenBounds(cxAccessibleObjectSelfID);
...
function TdxRibbonQuickAccessPopupBarControl.GetBarControl
  : TdxRibbonQuickAccessToolbarBarControl;
begin
...
function TdxRibbonQuickAccessPopupBarControl.GetMarkSubItem
  : TCustomdxBarSubItem;
begin
...
function TdxRibbonQuickAccessPopupPainter.MarkSizeX
...
procedure TdxRibbonQuickAccessPopupPainter.DrawQuickAccessPopupSubItem(DC: HDC;
const ARect: TRect; AState: Integer);
begin
  if AState <> DXBAR_NORMAL then
...
procedure TdxRibbonQuickAccessPopupSubMenuControl.ShowPopup
  (AItem: TdxBarItemControl);
begin
  // do nothing
end;
...
procedure TdxRibbonQuickAccessPopupSubItemControl.DoCloseUp(AHadSubMenuControl
  : Boolean);
var
...
begin
  ...
  begin
    ACloseUpReason := TdxBarSubMenuControlAccess(Item.ItemLinks.BarControl)
      .CloseUpReason;
    inherited;
    ...
    else if (BarNavigationController.AssignedSelectedObject <> nil) and
      (BarNavigationController.AssignedSelectedObject.GetHelper = ABarControl.
      MarkIAccessibilityHelper.GetHelper) then
...
begin
  TdxRibbonQuickAccessPopupPainter(Painter).DrawQuickAccessPopupSubItem
    (Canvas.Handle, ARect, GetState);
end;
...
procedure TdxRibbonTabAreaToolbarBarControl.InitCustomizationPopup
  (AItemLinks: TdxBarItemLinks);
begin
  // do nothing
end;
...
function TdxRibbonTabAreaToolbarBarControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
...
function TdxRibbonTabAreaToolbarBarControl.GetQuickControlClass
  : TdxBarPopupControlClass;
begin
...
function TdxRibbonTabAreaToolbarBarControlAccessibilityHelper.
  GetNextAccessibleObject(AItemLinkHelper: TdxBarItemLinkAccessibilityHelper;
ADirection: TcxAccessibilityNavigationDirection): IdxBarAccessibilityHelper;
begin
  Result := TdxRibbonCustomToolbarBarControl(FOwnerObject)
    .Ribbon.GetNextHorizontalAccessibleObject(AItemLinkHelper, ADirection)
end;

{ TdxRibbonTabAreaToolbarDockControl }

function TdxRibbonTabAreaToolbarDockControl.CreatePainter
  : TdxRibbonCustomToolbarPainter;
begin
  Result := TdxRibbonTabAreaToolbarPainter.Create(TdxNativeUInt(Ribbon));
end;

function TdxRibbonTabAreaToolbarDockControl.GetDockedBarControlClass
  : TdxBarControlClass;
begin
...
procedure TdxRibbonTabAreaToolbarPainter.BarDrawBackground
  (ABarControl: TdxBarControl; ADC: HDC; const ADestRect, ASourceRect: TRect;
ABrush: HBRUSH; AColor: TColor);
begin
  // do nothing
end;

procedure TdxRibbonTabAreaToolbarPainter.EditControlDrawBorder(const ADrawParams
  : TdxBarEditLikeControlDrawParams; var ARect: TRect);
begin
...
begin
  Result := Ribbon.ColorScheme.GetPartColor(rtatpEditBackground,
    GetPartState(ADrawParams, icpControl));
end;

function TdxRibbonTabAreaToolbarPainter.EditControlGetTextColor
  (const ADrawParams: TdxBarItemControlDrawParams): TColor;
var
  AStyle: TcxStyle;
begin
  ...
  if (AStyle = nil) or not(svTextColor in AStyle.AssignedValues) then
    Result := Ribbon.ColorScheme.GetPartColor(rtatpEditText,
      GetPartState(ADrawParams, icpControl))
  else
    Result := inherited EditControlGetTextColor(ADrawParams);
end;

function TdxRibbonTabAreaToolbarPainter.GetGlyphColorPalette(ABarItemControl
  : TdxBarItemControl; APaintType: TdxBarPaintType; AState: Integer)
  : IdxColorPalette;
begin
  if APaintType = ptMenu then
...
procedure TdxRibbonTabAreaToolbarPainter.DoDrawText(ADC: HDC;
const AText: string; var ARect: TRect; AFormat: Cardinal);
begin
...
procedure TdxRibbonTabAreaToolbarPainter.DrawToolbarContentPart
  (ABarControl: TdxBarControl; ACanvas: TcxCanvas);
begin
  inherited DrawToolbarContentPart(ABarControl, ACanvas);
...
function TdxRibbonTabAreaToolbarPainter.GetDefaultEnabledTextColor
  (ABarItemControl: TdxBarItemControl; ASelected: Boolean;
AFlat: Boolean): TColor;
var
...
begin
  ...
  if ADrawParams.PaintType <> ptMenu then
    Result := Ribbon.ColorScheme.GetPartColor(DXRIBBON_TAT_SMALLBUTTON,
      GetPartState(ADrawParams, icpControl))
  else
    Result := inherited GetDefaultEnabledTextColor(ABarItemControl,
      ASelected, AFlat);
end;

{ TdxRibbonTabAreaToolbarPopupBarControl }

procedure TdxRibbonTabAreaToolbarPopupBarControl.InitializeForDock
  (ABar: TdxBar);
begin
  inherited InitializeForDock(ABar);
...
function TdxRibbonTabAreaToolbarPopupBarControl.GetBarControl
  : TdxRibbonTabAreaToolbarBarControl;
begin
  if ParentBar <> nil then
...
begin
  ...
  begin
    TdxRibbonTabAccessibilityHelper(Group.Tab.IAccessibilityHelper.GetHelper)
      .CloseUpHandler(CloseUpReason);
    TdxRibbonTabGroupsPopupWindow(DockControl.Parent.Parent).CloseUp;
  end
  else
  begin
    AAccessibilityHelper := TdxRibbonGroupBarControlAccessibilityHelper
      (ParentBar.IAccessibilityHelper.GetHelper);
    AReason := CloseUpReason;
    inherited CloseUp;
    AAccessibilityHelper.CloseUpHandler(AReason);
  end;
...
procedure TdxRibbonGroupBarControl.FadingGetFadingImages(out AFadeOutImage,
  AFadeInImage: TcxBitmap);
  ...
  const
    CollapsedStateMap: array [TdxBarViewState] of Integer = (DXBAR_NORMAL,
      DXBAR_HOT);
  var
  ...
  begin
    ...
    try
      ...
      try
        ...
        if Collapsed then
          Ribbon.GroupsPainter.DrawCollapsedToolbarBackgroundPart(Self, ACanvas,
            CollapsedStateMap[AState])
        else
...
begin
  ...
  begin
    ...
    APos.Y := Min(APos.Y, ClientToScreen(cxNullPoint).Y - AHeight - 2 *
      HintIndent);
  end;
end;
...
begin
  ...
  try
    inherited;
    if (ALinkSelf.Ref <> nil) and (dxBarGetParentPopupWindow(Self, True) <> nil)
    then
    begin
...
begin
  ...
  try
    ...
    begin
      P := dxMapWindowPoint(Handle, Ribbon.FGroupsDockControlSite.Handle,
        cxNullPoint);
...
begin
  ...
  begin
    ...
    AOffsets := Painter.GetToolbarContentOffsets(Bar, dsNone,
      ScaleFactor, False);
    if UseRightToLeftAlignment then
...
procedure TdxRibbonGroupBarControl.DrawGroupsArea(ACanvas: TcxCanvas;
const ABounds: TRect);
begin
  Ribbon.ViewInfo.Painter.DrawGroupsArea(ACanvas, ABounds,
    IsAllowContextPaint, IsPopup);
end;
...
procedure TdxRibbonGroupBarControl.DoBarMouseDown(Button: TMouseButton;
...
begin
  ...
  begin
    ...
    inherited DoBarMouseDown(Button, Shift, APoint, AItemControl,
      APointInClientRect);
end;

function TdxRibbonGroupBarControl.ClickAtHeader: Boolean;
begin
  Result := Collapsed and cxRectPtIn(WindowRect, GetMouseCursorPos) or
    inherited ClickAtHeader;
end;

function TdxRibbonGroupBarControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
...
function TdxRibbonGroupBarControl.GetViewInfoClass
  : TCustomdxBarControlViewInfoClass;
begin
...
procedure TdxRibbonGroupBarControl.InitQuickControl(AQuickControl
  : TdxBarPopupControl);
begin
  // do nothing
end;
...
procedure TdxRibbonGroupBarControl.ViewStateChanged
  (APrevValue: TdxBarViewState);
begin
  if Ribbon.CanFade then
  begin
    if ViewState = bvsHot then
      dxFader.FadeIn(Self)
    else if MarkState <> msPressed then
      dxFader.FadeOut(Self);
  end;
  FullInvalidate;
end;

procedure TdxRibbonGroupBarControl.UpdateCaptionButton(ACaptionButton
  : TdxBarCaptionButton);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      if Ribbon.UseRightToLeftAlignment then
        Bar.CaptionButtons[I].Rect := TdxRightToLeftLayoutConverter.ConvertRect
          (AButtonRect, ACaptionRect)
      else
...
procedure TdxRibbonGroupBarControl.WindowPosChanged
  (var Message: TWMWindowPosMsg);
begin
...
begin
  case TdxBarButton(Sender).Tag of
    0:
      Ribbon.DesignAddTabGroup(Group.Tab, False);
    1:
      Ribbon.DesignAddTabGroup(Group.Tab, True);
    2:
      BarDesignController.DeleteSelectedObjects(True, True)
  end;
end;

procedure TdxRibbonGroupBarControl.DrawCaptionButton(ACanvas: TcxCanvas;
AButton: TdxBarCaptionButton);
var
...
begin
  Ribbon.SkinDrawBackground(ACanvas.Handle, AButton.Rect,
    DXBAR_LAUNCHBUTTONBACKGROUND, AButton.State);

  ADefaultGlyphSize := Ribbon.SkinGetPartSize(DXBAR_LAUNCHBUTTONDEFAULTGLYPH);
  AGlyphRectSize := Min(cxRectWidth(AButton.Rect), cxRectHeight(AButton.Rect)) -
    2 { BorderSize } * 2;
  ...
  begin
    ...
    if UseRightToLeftAlignment then
      AGlyphRect := TdxRightToLeftLayoutConverter.ConvertRect(AGlyphRect,
        AButton.Rect);
    Ribbon.SkinDrawBackground(ACanvas.Handle, AGlyphRect,
      DXBAR_LAUNCHBUTTONDEFAULTGLYPH, AButton.State);
  end;
end;
...
begin
  ...
  DrawLine(R.Left, R.Bottom - dxRibbonGroupSelectionFrameSize, R.Right,
    R.Bottom);
...
begin
  Result := Ribbon.SkinGetCaptionRect(NCRect,
    Ribbon.GroupsPainter.GetToolbarSkinPart(Self));
end;
...
begin
  ...
  begin
    Result := WindowRect;
    OffsetRect(Result, -(Result.Left - ScaleFactor.Apply(3)),
      -(Result.Top + ScaleFactor.Apply(3)));
...
procedure TdxRibbonGroupBarControl.WMGestureNotify
  (var Message: TWMGestureNotify);
begin
  Message.Result := DefWindowProc(Handle, Message.Msg, Message.Unused,
    lParam(Message.NotifyStruct));
...
begin
  ...
  if Collapsed then
  begin
    ContentSize := cxSize(Ribbon.GroupsPainter.GetCollapsedGroupWidth
  ...
  begin
    if Ribbon.IsSimplifiedGroupsLayout then
      ...
      begin
        AItemControlViewInfo := IdxBarItemControlViewInfo
          (ItemControlViewInfos[I]);
        AItemControlViewInfo.SetViewLevel
          (AItemControlViewInfo.GetViewLevelForButtonGroup);
      end;
...
function TdxRibbonGroupBarControlViewInfo.Reduce
  (AStage: TdxRibbonGroupsReduceStage;
AUpToViewLevel: TdxBarItemRealViewLevel): Boolean;
begin
  Result := False;
  if BarControl.Group.Restriction <> rtgrNoExpand then
    case AStage of
      ...
      rgrsItemControlsViewLevel:
        Result := LayoutCalculator.Reduce(CreateCalculateHelper,
          AUpToViewLevel);
    end;

  if (AStage = rgrsGroupsCollapsing) and
    (BarControl.Group.Restriction <> rtgrNoCollapse) then
  begin
    Collapsed := True;
    Calculate;
  end;
...
begin
  if not BarControl.HandleAllocated or not IsWindowVisible(BarControl.Handle)
  then
    raise EdxException.Create
      ('TdxRibbonGroupBarControlViewInfo.CalculateKeyTipsBaseLinePositions fails');
...
begin
  ...
  begin
    if Collapsed then
      BarControl.Group.Tab.Ribbon.DoTabGroupCollapsed(BarControl.Group.Tab,
        BarControl.Group)
    else
      BarControl.Group.Tab.Ribbon.DoTabGroupExpanded(BarControl.Group.Tab,
        BarControl.Group);
  end;
end;

function TdxRibbonGroupBarControlViewInfo.CreateLayoutCalculator
  : IdxRibbonGroupLayoutCalculator;
begin
  FGroupRowHeight := Ribbon.GetGroupRowHeight;
  Result := TdxRibbonGroupLayoutCalculator.Create(FGroupRowHeight,
    Ribbon.GroupsPainter.GetGroupRowCount, Ribbon.ScaleFactor);
end;

procedure TdxRibbonGroupBarControlViewInfo.DoCalculateKeyTipsBaseLinePositions;
begin
  SetLength(FKeyTipsBaseLinePositions.RowKeyTipsBaseLinePositions,
    Ribbon.GroupsPainter.GetGroupRowCount);
  ...
  begin
    FKeyTipsBaseLinePositions.RowKeyTipsBaseLinePositions[2] :=
      BarControl.GroupCaptionRect.Top;
    with FKeyTipsBaseLinePositions do
      RowKeyTipsBaseLinePositions[1] :=
        (RowKeyTipsBaseLinePositions[0] + RowKeyTipsBaseLinePositions[2]) div 2;
  end;
  FKeyTipsBaseLinePositions.BottomKeyTipsBaseLinePosition := BarControl.Height -
    Ribbon.SkinGetContentOffsets(DXBAR_COLLAPSEDTOOLBAR).Bottom;
end;
...
var
  R: TRect;
begin
  R := BarControl.Painter.GetToolbarContentOffsets(BarControl.Bar, dsNone,
    BarControl.ScaleFactor, False);
...
begin
  ...
  if IsAllControlsCreated(AItemLinks) then
    ...
    begin
      ...
      TdxBarItemControlAccess(AItemControl).LastInRow :=
        I = AItemLinks.CanVisibleItemCount - 1;
      AItemControlViewInfo := IdxBarItemControlViewInfo(AItemControl.ViewInfo);
      if not Ribbon.IsSimplifiedGroupsLayout or
        (AItemControlViewInfo.GetViewLevelForButtonGroup
        in AItemControlViewInfo.GetAllowedViewLevels) then
...
begin
  ...
  try
    ...
    begin
      ...
      ANeedsInvalidateBarControl := ANeedsInvalidateBarControl or
        not EqualRect(AItemLink.ItemRect, GetItemControlBounds(I));
...
function TdxRibbonGroupBarControlViewInfo.CreateCalculateHelper
  : IdxRibbonGroupViewInfo;
begin
  Result := TdxRibbonGroupBarControlViewInfoHelper.Create(Self);
end;

procedure TdxRibbonGroupBarControlViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
var
  I: Integer;
  AItemBounds: TRect;
...
begin
  ...
  begin
    ...
    IdxBarItemControlViewInfo(ItemControlViewInfos[I])
      .SetBounds(TdxRightToLeftLayoutConverter.ConvertRect(AItemBounds,
      ABounds));
  end;
  for I := 0 to SeparatorCount - 1 do
  begin
    ASeparatorInfo := SeparatorInfos[I];
    ASeparatorInfo.Bounds := TdxRightToLeftLayoutConverter.ConvertRect
      (SeparatorInfos[I].Bounds, ABounds);
    SeparatorInfos[I] := ASeparatorInfo;
  end;
end;

function TdxRibbonGroupBarControlViewInfo.GetBarControl
  : TdxRibbonGroupBarControl;
begin
  Result := FBarControl as TdxRibbonGroupBarControl;
end;

function TdxRibbonGroupBarControlViewInfo.
  GetBottomKeyTipsBaseLinePosition: Integer;
begin
...
begin
  CalculateKeyTipsBaseLinePositions;
  if (ARowIndex < 0) or
    (ARowIndex > High(FKeyTipsBaseLinePositions.RowKeyTipsBaseLinePositions))
  then
    raise EdxException.Create
      ('TdxRibbonGroupBarControlViewInfo.GetRowKeyTipsBaseLinePosition fails');
...
class function TdxRibbonGroupBarControlDesignHelper.
  CanDynamicallyChangeViewLevels: Boolean;
begin
  Result := True;
end;

class function TdxRibbonGroupBarControlDesignHelper.GetForbiddenActions
  : TdxBarCustomizationActions;
begin
...
procedure TdxRibbonCollapsedGroupPopupBarControl.InitializeForPopup
  (AParentBarControl: TdxBarControl; ABar: TdxBar);
begin
...
function TdxRibbonCollapsedGroupPopupBarControl.
  IgnoreClickAreaWhenHidePopup: TRect;
begin
...
begin
  ...
  FDesignSelectionHelper := GetSelectableItem
    (TdxDesignSelectionHelper.Create(Ribbon, Self, Tab));
end;
...
procedure TdxRibbonTabGroup.Assign(Source: TPersistent);
  ...
  begin
    if (TdxRibbonTabGroup(Source).Toolbar <> nil) and IsInheritanceUpdating then
      Result := Tab.Ribbon.BarManager.BarByComponentName
        (TdxRibbonTabGroup(Source).Toolbar.Name)
    else
...
begin
  ...
  Filer.DefineProperty('Caption', ReadCaption, WriteCaption,
    GetIsCaptionAssigned and (Caption = ''));
  Filer.DefineProperty('ToolbarName', ReadToolbarName, WriteToolbarName,
    NeedWriteToolbarName);
end;
...
begin
  Result := Visible or (TdxBarManagerAccess(Ribbon.BarManager)
    .GetBarIndexInRestoringList(Toolbar) <> -1);
end;
...
begin
  Result := (AToolbar = nil) or (Ribbon.BarManager <> nil) and
    (Ribbon.BarManager = AToolbar.BarManager);
end;
...
begin
  TdxRibbonMergeHelper.MergeToolbarContainers(Self, AGroup, rmoCanCreateNewGroup
    in AMergeOptions);
end;
...
function TdxRibbonTabGroup.GetIniSection(const ABaseSection,
  ADelimiter: string): string;
begin
  Result := ABaseSection + ADelimiter + 'Group' + IntToStr(Index);
end;

procedure TdxRibbonTabGroup.LoadFromIni(ASource: TCustomIniFile;
const ABaseSection, ADelimiter: string);
var
...
begin
  ...
  if Toolbar <> nil then
    Toolbar.Visible := ASource.ReadBool(ASection, 'ToolBarVisible',
      Toolbar.Visible);
end;

procedure TdxRibbonTabGroup.SaveToIni(ADestination: TCustomIniFile;
const ABaseSection, ADelimiter: string);
var
  ASection: string;
begin
  ...
  begin
    ...
    ADestination.WriteBool(ASection, 'ToolBarVisible', IsActuallyVisible or
      VisibleBeforeMerging);
  end
  else
  begin
...
function TdxRibbonTabGroup.GetAnotherTabWithOurToolbar
  (out ATab: TdxRibbonTab): Boolean;
var
  ATabIndex: Integer;
...
procedure TdxRibbonTabGroup.SetAssignedValues(const AValue
  : TdxRibbonTabGroupAssignedValues);
begin
...
procedure TdxRibbonTabGroup.SetRestriction
  (AValue: TdxRibbonTabGroupRestriction);
begin
...
function TdxRibbonTabGroups.Find(const ACaption: string;
out AGroup: TdxRibbonTabGroup): Boolean;
var
  I: Integer;
begin
...
function TdxRibbonTabGroups.GetIniSection(const AGroupIndex: Integer;
ASource: TCustomIniFile): string;
begin
  Result := TdxRibbonTabCollection(Tab.Collection).GetIniSection(Tab.Name,
    ASource) + '.' + 'Group' + IntToStr(AGroupIndex);
end;
...
begin
  ...
  begin
    AItem := Items[I];
    if (AGroup = nil) or TdxRibbonMergeHelper.IsMergedWith(AGroup.Toolbar,
      AItem.Toolbar) then
...
begin
  ...
  if (Self <> Ribbon.QuickAccessToolbar) and
    (Ribbon.QuickAccessToolbar.Toolbar = AValue) then
    Ribbon.QuickAccessToolbar.Toolbar := nil;
  if (Self <> Ribbon.TabAreaToolbar) and (Ribbon.TabAreaToolbar.Toolbar = AValue)
  then
    Ribbon.TabAreaToolbar.Toolbar := nil;
  if (Self <> Ribbon.TabAreaSearchToolbar) and
    (Ribbon.TabAreaSearchToolbar.Toolbar = AValue) then
...
function TdxRibbonCustomToolbar.GetIAccessibilityHelper
  : IdxBarAccessibilityHelper;
begin
  if (Toolbar <> nil) and (Toolbar.Control <> nil) then
...
procedure TdxRibbonTabAreaToolbar.Merge(ATabAreaToolbar
  : TdxRibbonTabAreaToolbar;
AMergeOptions: TdxRibbonMergeOptions = dxRibbonDefaultMergeOptions);
begin
  if (ATabAreaToolbar <> nil) and (ATabAreaToolbar.Toolbar <> nil) and
    ATabAreaToolbar.DockControl.Visible then
    TdxRibbonMergeHelper.MergeToolbarContainers(Self, ATabAreaToolbar,
      rmoCanCreateTabAreaToolbar in AMergeOptions);
end;

procedure TdxRibbonTabAreaToolbar.Unmerge(ATabAreaToolbar
  : TdxRibbonTabAreaToolbar = nil);
begin
...
procedure TdxRibbonTabAreaSearchToolbar.Merge(ATabAreaSearchToolbar
  : TdxRibbonTabAreaSearchToolbar;
AMergeOptions: TdxRibbonMergeOptions = dxRibbonDefaultMergeOptions);
begin
  if (ATabAreaSearchToolbar <> nil) and (ATabAreaSearchToolbar.Toolbar <> nil)
    and ATabAreaSearchToolbar.DockControl.Visible then
    TdxRibbonMergeHelper.MergeToolbarContainers(Self, ATabAreaSearchToolbar,
      rmoCanCreateTabAreaSearchToolbar in AMergeOptions);
end;

procedure TdxRibbonTabAreaSearchToolbar.Unmerge(ATabAreaSearchToolbar
  : TdxRibbonTabAreaSearchToolbar = nil);
begin
  TdxRibbonMergeHelper.UnmergeToolbarContainers(Self, ATabAreaSearchToolbar);
end;

function TdxRibbonTabAreaSearchToolbar.CreateDockControl
  : TdxCustomRibbonDockControl;
begin
...
function TdxRibbonQuickAccessToolbar.AddGroupButton(AToolbar: TdxBar)
  : TdxRibbonQuickAccessGroupButton;
begin
  Result := nil;
  if AToolbar <> nil then
  begin
    Result := TdxRibbonQuickAccessGroupButton
      (Toolbar.ItemLinks.AddItem(TdxRibbonQuickAccessGroupButton).Item);
...
procedure TdxRibbonQuickAccessToolbar.BarManagerLoadIni(const AEventData
  : TdxBarIniFileEventData);
var
  ABar: TdxBar;
...
begin
  if Toolbar <> nil then
  begin
    ABarSection := TdxBarAccess(Toolbar).GetIniSection(AEventData.BaseSection,
      Toolbar.Index);
    ALinkCount := AEventData.IniFile.ReadInteger(ABarSection,
      'ItemLinkCount', 0);
    for I := 0 to ALinkCount - 1 do
    begin
      AItemSection := TdxBarItemLinkAccess.GetIniSection(ABarSection, I,
        AEventData.StoringKind);
      ABar := Ribbon.BarManager.BarByComponentName
...
procedure TdxRibbonQuickAccessToolbar.BarManagerSaveIni(const AEventData
  : TdxBarIniFileEventData);
var
  AItemLink: TdxBarItemLink;
  ASection: string;
  ...
  if Toolbar <> nil then
    ...
    begin
      ...
      begin
        ...
        begin
          ASection := TdxBarAccess(Toolbar)
            .GetIniSection(AEventData.BaseSection, Toolbar.Index);
          ASection := TdxBarItemLinkAccess(AItemLink).GetIniSection(ASection, I,
            AEventData.StoringKind);
          if TdxRibbonQuickAccessGroupButton(AItemLink.Item).Toolbar <> nil then
            AToolBarName := TdxRibbonQuickAccessGroupButton(AItemLink.Item)
              .Toolbar.Name
          else
            AToolBarName := '';
...
function TdxRibbonQuickAccessToolbar.HasGroupButtonForToolbar
  (AToolbar: TdxBar): Boolean;
begin
  Result := (Toolbar <> nil) and TdxRibbonQuickAccessToolbarHelper.
    HasGroupButtonForToolbar(Toolbar.ItemLinks, AToolbar);
end;
...
function TdxRibbonQuickAccessToolbar.CreateDockControl
  : TdxCustomRibbonDockControl;
begin
  Result := TdxRibbonQuickAccessToolbarDockControl.Create(Ribbon);
end;

function TdxRibbonQuickAccessToolbar.GetMenuItemsForMark
  : TdxRibbonPopupMenuItems;
begin
...
procedure TdxRibbonQuickAccessToolbar.Merge(AQuickAccessToolbar
  : TdxRibbonQuickAccessToolbar;
AMergeOptions: TdxRibbonMergeOptions = dxRibbonDefaultMergeOptions);
begin
  if (AQuickAccessToolbar <> nil) and (AQuickAccessToolbar.Toolbar <> nil) and
    AQuickAccessToolbar.DockControl.Visible then
    TdxRibbonMergeHelper.MergeToolbarContainers(Self, AQuickAccessToolbar,
      rmoCanCreateQATToolbar in AMergeOptions);
end;

procedure TdxRibbonQuickAccessToolbar.Unmerge(AQuickAccessToolbar
  : TdxRibbonQuickAccessToolbar = nil);
begin
...
begin
  Ribbon.PopulatePopupMenuItems(AItems, GetMenuItemsForMark,
    Ribbon.PopupMenuItemClick);
end;

procedure TdxRibbonQuickAccessToolbar.SetPosition
  (AValue: TdxQuickAccessToolbarPosition);
begin
...
procedure TdxRibbonCustomButtonPersistent.SetScreenTip
  (const Value: TdxScreenTip);
begin
...
function TdxRibbonApplicationButton.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
...
procedure TdxRibbonApplicationButton.Notification(AComponent: TComponent;
...
function TdxRibbonApplicationButton.GetIAccessibilityHelper
  : IdxBarAccessibilityHelper;
begin
...
begin
  if FMenu <> Value then
  begin
    if (Value = nil) or Supports(Value, IdxRibbonApplicationMenu, ANewIMenu)
    then
    begin
      if FMenu <> nil then
      begin
...
begin
  Result := (dxRibbonContextualTabLabelShowAnimationTime > 0) and
    (Ribbon.Style >= rs2019) and not Ribbon.IsDesigning;
end;
...
begin
  ...
  begin
    ...
    else if AActivateFirstTab then
      Tabs[0].Active := True;
...
begin
  ...
  begin
    ...
    begin
      FAlphaAnimation := TdxRibbonContextAnimation.Create
        (dxRibbonContextualTabLabelShowAnimationTime, Self);
...
constructor TdxRibbonContextAnimation.Create(ATime: Cardinal;
AContext: TdxRibbonContext);
begin
...
function TdxRibbonTab.GetCollectionFromParent(AParent: TComponent)
...
begin
  Result := Caption;
  if dxDefaultBooleanToBoolean(Ribbon.CapitalizeTabCaptions,
    Ribbon.Style = rs2013) then
...
procedure TdxRibbonTab.Notification(AComponent: TComponent;
Operation: TOperation);
begin
...
AMergeOptions: TdxRibbonMergeOptions = dxRibbonDefaultMergeOptions);
  ...
  begin
    Result := (AGroup.MergeKind = ATargetGroup.MergeKind) and
      (ATargetGroup.Caption = AGroup.Caption) and
      not ATargetGroup.IsToolBarShared;
  end;

  function FindTargetGroup(AGroup: TdxRibbonTabGroup;
  out ATargetGroup: TdxRibbonTabGroup): Boolean;
  var
    I: Integer;
  begin
    Result := False;
...
function TdxRibbonTab.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      if not Ribbon.IsDestroying and (APrevContext <> nil) and APrevContext.FCreatedByMerging
      then
      begin
...
begin
  ...
  begin
    ...
    begin
      FDesignSelectionHelper := nil;
      FDesignSelectionHelper :=
        GetSelectableItem(TdxDesignSelectionHelper.Create(Ribbon, Self,
        Ribbon));
...
begin
  if Ribbon.UseRightToLeftAlignment then
    ATabGroupsDockControlOffset := TdxRightToLeftLayoutConverter.ConvertOffsets
      (Ribbon.ViewInfo.GetTabGroupsDockControlOffset)
  else
    ATabGroupsDockControlOffset :=
      Ribbon.ViewInfo.GetTabGroupsDockControlOffset;
...
begin
  DockControl.ViewInfo.ScrollGroups(AScrollLeft,
    Ribbon.GroupsDockControlSite.Width -
...
begin
  ...
  begin
    ...
    begin
      AIsDockControlVisible := Visible and not Ribbon.Hidden and
        Ribbon.ShowTabGroups and Active;
...
function TdxRibbonTab.GetIniSection(const ABaseSection,
  ADelimiter: string): string;
begin
  Result := ABaseSection + ADelimiter + 'Tab' + IntToStr(Index);
end;

procedure TdxRibbonTab.LoadFromIni(ASource: TCustomIniFile;
const ABaseSection, ADelimiter: string);
var
...
begin
  ...
  begin
    ...
    if AGroup.Toolbar <> nil then
      AGroup.Toolbar.Visible := (I < AGroupCount) or
        (AGroup.IsToolBarShared and AGroup.Toolbar.Visible);
  end;
  ...
  begin
    ...
    AToolbar := Ribbon.BarManager.BarByComponentName
      (ASource.ReadString(AGroupSection, 'ToolBar', ''));
...
procedure TdxRibbonTab.SaveToIni(ADestination: TCustomIniFile;
const ABaseSection, ADelimiter: string);
var
  ASection: string;
  I: Integer;
...
begin
  Result := Visible and (ViewInfo <> nil) and
    not cxRectIsEmpty(ViewInfo.Bounds);
end;
...
procedure TdxRibbonTab.FadingGetFadingImages(out AFadeOutImage,
  AFadeInImage: TcxBitmap);
var
...
function TdxRibbonTabCollection.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonTabCollectionAccessibilityHelper;
end;

function TdxRibbonTabCollection.GetIniSection(const ABaseSection,
  ADelimiter: string; const ATabIndex: Integer): string;
begin
  Result := ABaseSection + ADelimiter + 'Tab' + IntToStr(ATabIndex);
end;

function TdxRibbonTabCollection.GetIniSection(const ATabName: string;
ASource: TCustomIniFile): string;
var
  ABaseSection: string;
  I: Integer;
begin
  ABaseSection := Owner.GetIniSection('.', TdxBarManagerAccess(Owner.BarManager)
    .GetBaseIniSection);
  for I := 0 to ASource.ReadInteger(ABaseSection, 'TabCount', 0) - 1 do
    if SameText(ATabName, ASource.ReadString(GetIniSection(ABaseSection, '.',
      I), 'Name', '')) then
      Break;
...
function TdxRibbonTabCollection.FindByComponentName(const AName: string)
  : TdxRibbonTab;
var
  I: Integer;
begin
...
function TdxRibbonTabCollection.FindByLoadedIndex(AIndex: Integer)
  : TdxRibbonTab;
var
  I: Integer;
begin
...
begin
  for I := 0 to Count - 1 do
    Items[I].Context := FOwner.Contexts.GetItemFromIndex
      (Items[I].FContextIndex);
end;
...
function TdxRibbonFonts.GetApplicationButtonFont
  (AState: TdxRibbonApplicationButtonState): TFont;
begin
  Result := GetInternalFont(afApplicationButton);
  Painter.AdjustApplicationButtonFont(Result, AState);
end;

function TdxRibbonFonts.GetContextFont(AContextColor
  : TColor = clDefault): TFont;
begin
...
function TdxRibbonFonts.GetTabHeaderFont(AState: Integer;
AContext: TdxRibbonContext): TFont;
begin
  Result := GetInternalFont(afTabHeader);
...
function TdxRibbonFonts.GetPartColor(APart: Integer;
...
var
  ...
  AMasterFont: TFont;
begin
  FCaptionFont.Handle := CreateFontIndirect
    (dxSystemInfo.NonClientMetrics.lfCaptionFont);
  FCaptionFont.Height := Ribbon.ScaleFactor.Apply(FCaptionFont.Height,
    dxSystemScaleFactor);
  ...
  FLocked := True;
  try
    for AFontIndex := Low(TdxRibbonAssignedFont)
      to High(TdxRibbonAssignedFont) do
...
begin
  ...
  begin
    if IsWinSevenOrLater then
      Result := clBlack
    else if IsFormZoomed(Ribbon.RibbonForm) then
      Result := clWindow
    else if AIsActive then
...
function TdxBarApplicationMenu.ApplicationMenuPopup(ARibbon: TdxCustomRibbon;
var AClosedByEscape: Boolean): Boolean;
var
  AOwnerBounds: TRect;
...
begin
  ...
  try
    ...
    PopupEx(P.X, P.Y, cxRectWidth(AOwnerBounds), AOwnerOffset, False,
      @AOwnerBounds, True, ARibbon);
...
begin
  inherited DoCloseUp;
  FClosedByEscape := TdxBarSubMenuControlAccess(ItemLinks.BarControl)
    .CloseUpReason = bcrEscape;
end;
...
function TdxBarApplicationMenu.GetDisplayMode
  : TdxRibbonApplicationMenuDisplayMode;
begin
  Result := amdmPopup;
end;
...
begin
  if FRibbonPainter = nil then
    raise EdxException.Create
      (cxGetResourceString(@dxSBAR_APPMENUOUTSIDERIBBON));
  inherited Popup(X, Y);
end;
...
function TdxBarApplicationMenu.GetRootAccessibilityHelper
  : IdxBarAccessibilityHelper;
begin
  Result := nil;
end;
...
begin
  SelectFirstSelectableAccessibleObject
    (ItemLinks.BarControl.IAccessibilityHelper.GetBarHelper);
end;

procedure TdxBarApplicationMenu.ShowKeyTips;
begin
  BarNavigationController.SetKeyTipsShowingState
    (ItemLinks.BarControl.IAccessibilityHelper, '');
end;

{ TdxRibbonApplicationMenuControl }

function TdxRibbonApplicationMenuControl.GetBehaviorOptions
  : TdxBarBehaviorOptions;
begin
  Result := inherited GetBehaviorOptions + [bboItemCustomizePopup];
end;

procedure TdxRibbonApplicationMenuControl.InitCustomizationPopup
  (AItemLinks: TdxBarItemLinks);
begin
  if Ribbon <> nil then
    Ribbon.PopulatePopupMenuItems(AItemLinks, GetPopupMenuItems,
      PopupMenuClick);
end;

function TdxRibbonApplicationMenuControl.GetPopupMenuItems
  : TdxRibbonPopupMenuItems;
begin
  Result := Ribbon.GetValidPopupMenuItems;
  if ExtraPaneItemLinks.IndexOf(BarDesignController.CustomizingItemLink) <> -1
  then
    Exclude(Result, rpmiQATAddRemoveItem);
end;

procedure TdxRibbonApplicationMenuControl.PopupMenuClick(Sender: TObject);
// see TdxRibbonCustomBarControl
var
  ALinkSelf: TcxObjectLink;
begin
  ALinkSelf := cxAddObjectLink(Self);
...
procedure TdxRibbonApplicationMenuControl.WMNCHitTest
...
begin
  ...
  begin
    ARect := dxMapWindowRect(Ribbon.Handle, 0,
      Ribbon.ApplicationButtonViewInfo.Bounds);
...
begin
  ...
  BarDesignController.AddInternalItem(AItemLinks, TdxBarButton,
    cxGetResourceString(@dxSBAR_RIBBONADDEMPTYGROUP),
    DesignTabMenuClick, 2, True);
  BarDesignController.AddInternalItem(AItemLinks, TdxBarButton,
    cxGetResourceString(@dxSBAR_RIBBONADDGROUPWITHTOOLBAR),
    DesignTabMenuClick, 3);
end;
...
begin
  ...
  begin
    case AObject of
      rhtTab:
        Result := (HintInfo.HitObjectAsTab <> nil) and
          ViewInfo.TabsAreaViewInfo.TabsViewInfo.NeedShowHint;
      rhtApplicationMenu:
        Result := (ApplicationButton.ScreenTip <> nil) and
          not IsApplicationMenuDropped;
      rhtHelpButton:
        Result := Ribbon.HelpButton.ScreenTip <> nil;
      rhtContext:
        Result := ViewInfo.ContextsViewInfo.NeedShowHint
          (HintInfo.HitObjectAsContext);
      rhtCustomButton:
        Result := HintInfo.HitObjectAsButton.Enabled and
          (HintInfo.HitObjectAsButton.Hint <> '');
    end;
  end;
end;

function TdxRibbonController.IsOwnerForHintObject
  (AObject: TdxRibbonHitTest): Boolean;
begin
  Result := AObject in [rhtTab, rhtApplicationMenu, rhtHelpButton,
    rhtCustomButton, rhtContext];
end;
...
begin
  ...
  case HitInfo.HitTest of
    ...
    if Button = mbRight then
    begin
      if cxRectPtIn(ViewInfo.TabsAreaViewInfo.TabsViewInfo.GetRealBounds, X, Y)
        or (Ribbon.IsQuickAccessToolbarValid and
        ViewInfo.QuickAccessToolbarViewInfo.AtBottom and
        cxRectPtIn(ViewInfo.QuickAccessToolbarViewInfo.Bounds, X, Y)) then
...
function TdxRibbonController.NextTabCore(ATab: TdxRibbonTab;
ADirection: Integer): TdxRibbonTab;
var
  AIndex: Integer;
...
function TdxRibbonController.CreateHintViewInfo(const AHintText,
...
function TdxRibbonController.GetHintPosition(const ACursorPos: TPoint;
AHeight: Integer): TPoint;
var
  R: TRect;
  ALeftPos: Integer;
begin
  ...
  begin
    ...
    Result := cxPoint(ACursorPos.X, ACursorPos.Y + Ribbon.ScaleFactor.Apply
      (dxRibbonHintOffset));
end;
...
begin
  Result := not Ribbon.IsDesigning and Ribbon.MinimizeOnTabDblClick and
    (not Ribbon.ShowTabGroups or (not FSkipDblClick and
    HitInfo.HitObjectAsTab.Active));
end;
...
begin
  case TdxBarButton(Sender).Tag of
    ...
    1:
      BarDesignController.DeleteSelectedObjects(True, True);
    2:
      Ribbon.DesignAddTabGroup(nil, False);
    3:
      Ribbon.DesignAddTabGroup(nil, True);
  end;
  Ribbon.Modified;
end;
...
const
  ScrollDelta: array [Boolean, Boolean] of Integer =
    ((-dxRibbonTabMinWidth div 2, dxRibbonTabMinWidth div 2), (-3, 3));
begin
  with ViewInfo.TabsAreaViewInfo.TabsViewInfo do
    ScrollPosition := ScrollPosition + ScaleFactor.Apply
      (ScrollDelta[AOnTimer, AScrollLeft]);
end;
...
begin
  ...
  begin
    ...
    else if ANeedHide then
      HideHint;
  end;
end;

procedure TdxRibbonController.SetHotButton
  (AValue: TdxRibbonCustomButtonViewInfo);
begin
...
procedure TdxRibbonController.SetPressedButton
  (AValue: TdxRibbonCustomButtonViewInfo);
begin
...
function TdxRibbonController.NotHandleMouseMove(P: TPoint): Boolean;
  ...
  begin
    Result := IsFormActive(Ribbon.BarManager.ParentForm) or
      IsFormActive(Ribbon.TabGroupsPopupWindow) or (Ribbon.RibbonForm <> nil)
      and Ribbon.RibbonForm.IsActive;
  end;

begin
  if Ribbon.IsBarManagerValid and not Ribbon.IsDesigning then
    Result := (FScrollKind <> rhtNone) or not IsOwnedFormActive or
      dxBarHasPopupWindowAbove(nil, False)
  else
    Result := True;
end;
...
begin
  ...
  begin
    ...
    else if not CloseApplicationMenu then
    begin
...
procedure TdxRibbonController.ProcessContextsOnMouseDown(AIsDoubleClick
  : Boolean);
var
  AForm: TForm;
begin
  ...
  begin
    ...
    else if not Ribbon.ShowTabGroups and (Ribbon.TabGroupsPopupWindow = nil)
    then
      Ribbon.ShowTabGroups := True;
  end
  else
  ...
  case Button of
    ...
    mbLeft:
      if (ssDouble in Shift) or Ribbon.ShowTabGroups then
        Ribbon.ActiveTab := ATab
      else if (Ribbon.ActiveTab <> ATab) or (Ribbon.TabGroupsPopupWindow = nil)
        or not Ribbon.TabGroupsPopupWindow.JustClosed then
      begin
        Ribbon.ActiveTab := ATab;
...
procedure TdxRibbonController.ProcessTabOnMouseDown(AButton: TMouseButton;
AShift: TShiftState);
var
  APrevActiveTab: TdxRibbonTab;
begin
  ...
  begin
    ...
    if (AButton = mbLeft) and (ssDouble in AShift) and CanSwitchMinimizedOnDblClick
    then
      Ribbon.ShowTabGroups := not Ribbon.ShowTabGroups
    else
...
function TdxRibbonController.GetApplicationButtonViewInfo
  : TdxRibbonApplicationButtonViewInfo;
begin
  Result := ViewInfo.ApplicationButtonViewInfo;
end;

function TdxRibbonController.GetApplicationMenuState
  : TdxRibbonApplicationMenuState;
const
  StateMap: array [TdxRibbonApplicationMenuDisplayMode]
    of TdxRibbonApplicationMenuState = (ramsShownAsMenu, ramsShownAsFrame,
    ramsShownAsFullScreenFrame);
begin
...
procedure TdxRibbonGroupsDockControlScrollButtonViewInfo.Draw
  (ACanvas: TcxCanvas);
begin
  inherited Draw(ACanvas);
  ACanvas.ExcludeClipRect(Bounds);
...
procedure TdxRibbonGroupsDockControlScrollButtonViewInfo.DrawButton
  (ACanvas: TcxCanvas; const R: TRect; AState: TcxButtonState);
begin
  Painter.DrawGroupsScrollButton(ACanvas, R, (FScrollButton = rsbLeft)
    xor Ribbon.UseRightToLeftAlignment, AState);
end;
...
function TdxRibbonGroupsDockControlScrollButtonViewInfo.GetHitInfo
...
constructor TdxRibbonGroupsDockControlSiteViewInfo.Create
  (AOwner: TdxRibbonViewInfo);

  function AddButton(AScrollButton: TdxRibbonScrollButton)
    : TdxRibbonGroupsDockControlScrollButtonViewInfo;
  begin
...
begin
  if RibbonViewInfo.IsTabGroupsVisible and (RibbonActiveTab <> nil) then
    FTabGroupsScrollButtons := RibbonActiveTab.DockControl.ViewInfo.
      ScrollButtons
  else
...
procedure TdxRibbonGroupsDockControlSiteViewInfo.CalculateButtonsBounds
  (const ABounds: TRect);
  ...
  begin
    ...
    Result := cxRectOffset(Result,
      RibbonViewInfo.GetTabGroupsContentOffset.BottomRight, False);
  end;
  ...
  begin
    ...
    TabGroupsScrollButtonLeft.Bounds := cxRectSetWidth(ABounds,
      IfThen(rsbLeft in TabGroupsScrollButtons, AButtonWidth));
    TabGroupsScrollButtonRight.Bounds := cxRectSetRight(ABounds, ARightSide,
      IfThen(rsbRight in TabGroupsScrollButtons, AButtonWidth));
  end;
...
procedure TdxRibbonGroupsDockControlSiteViewInfo.DrawRibbonParts
  (ACanvas: TcxCanvas);
var
  ASavedOrg: TPoint;
begin
  ...
  try
    ACanvas.WindowOrg := dxMapWindowPoint(FSite.Handle, Ribbon.Handle,
      ASavedOrg);
    ...
    begin
      ...
      try
        ACanvas.IntersectClipRect
          (RibbonViewInfo.TabsAreaViewInfo.TabsViewInfo.GetRealBounds);
...
function TdxRibbonGroupsDockControlSiteViewInfo.
  GetMinimizeRibbonButtonSize: TSize;
begin
...
function TdxRibbonGroupsDockControlSiteViewInfo.GetRibbonActiveTab
  : TdxRibbonTab;
begin
  Result := FSite.Ribbon.ActiveTab;
end;

function TdxRibbonGroupsDockControlSiteViewInfo.GetRibbonViewInfo
  : TdxRibbonViewInfo;
begin
...
// IdxGestureClient
function TdxRibbonGroupsDockControlSite.AllowPan(AScrollKind
  : TScrollBarKind): Boolean;
begin
  Result := (AScrollKind = sbHorizontal) and
    (Ribbon.ViewInfo.GroupsDockControlSiteViewInfo.
    TabGroupsScrollButtons <> []);
end;

procedure TdxRibbonGroupsDockControlSite.GestureScroll(ADeltaX,
  ADeltaY: Integer);
begin
...
function TdxRibbonGroupsDockControlSite.IsPanArea(const APoint: TPoint)
  : Boolean;
begin
  Result := True;
end;

function TdxRibbonGroupsDockControlSite.NeedPanningFeedback
...
procedure TdxRibbonGroupsDockControlSite.CreateParams
  (var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  Params.WindowClass.Style := Params.WindowClass.Style and
    not(CS_VREDRAW or CS_HREDRAW);
end;
...
function TdxRibbonGroupsDockControlSite.GetDockControl
  : TdxRibbonGroupsDockControl;
begin
...
procedure TdxRibbonTouchModeHelper.AdjustMargins(var AMargins: TRect;
AMaxTargetValue: Integer);
var
  AMaxValue: Integer;
begin
  if IsEnabled then
  begin
    AMaxValue := MaxIntValue([1, AMargins.Left, AMargins.Top, AMargins.Right,
      AMargins.Bottom]);
...
procedure TdxRibbonTouchModeHelper.AdjustPartOffsets(APart: Integer;
var AValue: TRect);
const
  TargetValue = 7;
begin
  ...
  begin
    ...
    if APart = DXBAR_APPLICATIONMENUCONTENT then
      AValue.Bottom := Ribbon.ScaleFactor.Apply(25) +
        cxMarginsHeight(Ribbon.SkinGetContentOffsets
        (DXBAR_APPLICATIONMENUBUTTON));
  end;
end;

procedure TdxRibbonTouchModeHelper.AdjustPartSize(APart: Integer;
var AValue: Integer);
begin
  // do nothing
end;
...
begin
  if risCreating in FInternalState then
    Exit;
  UnmergeBeforeDestroy;
  ...
  begin
    ...
    try
      ...
      try
        ...
        else if ApplicationButton.CanShowPopup then
        begin
          BeforeApplicationMenuPopup;
          Result := ApplicationButton.IMenu.Popup(Self, AClosedByEscape);
          if AObjectLink.Ref <> nil then
          begin
            if Result and AClosedByEscape and BarNavigationController.NavigationMode
            then
...
begin
  Result := (QuickAccessToolbar.Toolbar = AToolbar) or
    Tabs.ContainsToolBar(AToolbar);
end;
...
function TdxCustomRibbon.ShowCustomizationForm
  (AMode: TdxRibbonCustomizationFormMode): Boolean;
begin
  Result := True;
...
begin
  ...
  begin
    ...
    begin
      if QuickAccessToolbar.Toolbar.Control is TdxRibbonQuickAccessToolbarBarControl
      then
      begin
        with TdxRibbonQuickAccessToolbarBarControl
          (QuickAccessToolbar.Toolbar.Control) do
          UpdateDefaultGlyph(FDefaultGlyph);
      end;
...
begin
  Result := (AScrollKind = sbHorizontal) and
    (ViewInfo.TabsAreaViewInfo.TabsViewInfo.ScrollButtons <> []);
end;
...
function TdxCustomRibbon.NeedPanningFeedback(AScrollKind
  : TScrollBarKind): Boolean;
begin
  Result := False;
end;
...
procedure TdxCustomRibbon.AdjustRibbonFormBorderIconSize
  (AIcon: TdxRibbonBorderDrawIcon; AIsToolWindow: Boolean;
ACaptionHeight: Integer; var ASize: TSize);
begin
  ColorScheme.AdjustRibbonFormBorderIconSize(AIcon, AIsToolWindow,
    ACaptionHeight, ASize);
  TouchModeHelper.AdjustFormBorderIconSize(AIcon, AIsToolWindow, ASize);
end;

function TdxCustomRibbon.AreBordersThin: Boolean;
begin
  Result := ColorScheme.AreBordersThin or UseExternalFormBorders;
end;
...
begin
  ...
  ColorScheme.DrawRibbonFormBackground(DC, ARect, ViewInfo.Bounds.Bottom -
    ARect.Top);
  ColorScheme.UseRightToLeftAlignment := False;
end;
...
const ABounds: TRect; const ACaption: string);
begin
...
begin
  if ColorScheme.ExtendCaptionAreaOnTabs and
    (not ViewInfo.UseGlass or EnableTabAero) then
    Result := ViewInfo.TabsAreaViewInfo.MeasuredHeight -
      SkinGetPartSize(DXBAR_TABSGROUPSOVERLAPHEIGHT)
  else
    Result := 0;
end;
  ...
  begin
    ...
    begin
      ...
      for I := 0 to ABar.ItemLinks.Count - 1 do
        if ABar.ItemLinks[I].Item = BarDesignController.CustomizingItemLink.Item
        then
        begin
          Result := False;
          Break;
...
begin
  if UseExternalFormBorders then
    Result := Rect(1, 1, 1, 1)
  else
...
function TdxCustomRibbon.UseExternalFormBorders: Boolean;
begin
  Result := (RibbonForm <> nil) and
    (RibbonForm.UseNavigationControlCompatibleBorders);
end;

function TdxCustomRibbon.UseRoundedWindowCorners: Boolean;
begin
  Result := not UseExternalFormBorders and ColorScheme.UseRoundedWindowCorners;
end;
...
begin
  ...
  try
    ...
    ColorScheme.DrawTabGroupHeaderBackground(DC, Rect(R.Left, R.Bottom, R.Right,
      ARect.Bottom), AState, AIsInPopup);
  finally
    ColorScheme.UseRightToLeftAlignment := False;
  end;
end;
...
begin
  Result := GetGroupContentHeight + cxMarginsHeight
    (SkinGetContentOffsets(DXBAR_TOOLBAR));
end;
...
procedure TdxCustomRibbon.SkinDrawBackground(DC: HDC; const ARect: TRect;
APart, AState: Integer);
begin
...
begin
  ...
  case APart of
    ...
    DXBAR_SCROLLBARHORZ_BACKGROUND, DXBAR_SCROLLBARVERT_BACKGROUND:
      ColorScheme.DrawScrollBarBackground(DC, ARect,
        APart = DXBAR_SCROLLBARHORZ_BACKGROUND);
    DXBAR_SCROLLBARHORZ_LINEDOWN, DXBAR_SCROLLBARVERT_LINEDOWN:
      ColorScheme.DrawScrollBarPart(DC, ARect, sbpLineDown, AState,
        APart = DXBAR_SCROLLBARHORZ_LINEDOWN);
    DXBAR_SCROLLBARHORZ_LINEUP, DXBAR_SCROLLBARVERT_LINEUP:
      ColorScheme.DrawScrollBarPart(DC, ARect, sbpLineUp, AState,
        APart = DXBAR_SCROLLBARHORZ_LINEUP);
    DXBAR_SCROLLBARHORZ_THUMBNAIL, DXBAR_SCROLLBARVERT_THUMBNAIL:
      ColorScheme.DrawScrollBarPart(DC, ARect, sbpThumbnail, AState,
        APart = DXBAR_SCROLLBARHORZ_THUMBNAIL);
    DXBAR_SCROLLBARHORZ_PAGEDOWN, DXBAR_SCROLLBARVERT_PAGEDOWN:
      ColorScheme.DrawScrollBarPart(DC, ARect, sbpPageDown, AState,
        APart = DXBAR_SCROLLBARHORZ_PAGEDOWN);
    DXBAR_SCROLLBARHORZ_PAGEUP, DXBAR_SCROLLBARVERT_PAGEUP:
      ColorScheme.DrawScrollBarPart(DC, ARect, sbpPageUp, AState,
        APart = DXBAR_SCROLLBARHORZ_PAGEUP);
    ...
    DXBAR_INRIBBONGALLERYSCROLLBAR_LINEUPBUTTON:
      ColorScheme.DrawInRibbonGalleryScrollBarButton(DC, ARect,
        gsbkLineUp, AState);
    ...
    DXBAR_INRIBBONGALLERYSCROLLBAR_DROPDOWNBUTTON_TOUCH:
      ColorScheme.DrawInRibbonGalleryScrollBarDropDownTouchButton(DC,
        ARect, AState);
    ...
    DXBAR_MENUEXTRAPANE_SEPARATOR, DXBAR_MENUEXTRAPANE_SEPARATOR_VERT:
      ColorScheme.DrawMenuExtraSeparator(DC, ARect,
        APart = DXBAR_MENUEXTRAPANE_SEPARATOR);
    ...
    DXRIBBON_TAT_SMALLBUTTON:
      ColorScheme.DrawTabAreaButton(DC, ARect,
        RibbonStateToButtonState(AState));
    ...
    DXRIBBON_QAT_ARROWDOWN:
      ColorScheme.DrawQuickAccessToolbarArrowDown(DC, ARect, AState,
    ...
    DXRIBBON_QAT_SMALLBUTTON_GLYPH:
      ColorScheme.DrawQuickAccessToolbarSmallButtonDropButtonMainPart(DC,
        ARect, AState);
    DXRIBBON_QAT_SMALLBUTTON_DROPBUTTON:
      ColorScheme.DrawQuickAccessToolbarSmallButtonDropButtonArrowPart(DC,
        ARect, AState);
    DXRIBBON_QAT_MARKARROW:
      ColorScheme.DrawQuickAccessToolbarMarkArrow(DC, ARect, AState,
        ViewInfo.QuickAccessToolbarViewInfo.AtBottom);
    DXRIBBON_QAT_MARKTRUNCATED:
      ColorScheme.DrawQuickAccessToolbarMarkTruncated(DC, ARect, AState,
        ViewInfo.QuickAccessToolbarViewInfo.AtBottom);
...
function TdxCustomRibbon.SkinGetCaptionRect(const ARect: TRect;
APart: Integer): TRect;
var
  AOffsets: TRect;
begin
  case APart of
    DXBAR_TOOLBAR, DXBAR_TOOLBARINPOPUP:
      begin
        ...
        AOffsets := Rect(AOffsets.Left, 0, AOffsets.Right,
          Painter.GetGroupCaptionBottomOffset(APart = DXBAR_TOOLBARINPOPUP));
        if UseRightToLeftAlignment then
          AOffsets := TdxRightToLeftLayoutConverter.ConvertOffsets(AOffsets);
...
begin
  case APart of
    DXBAR_APPLICATIONMENUCONTENT:
      Result := ColorScheme.GetApplicationMenuContentOffset
        (ViewInfo.TabsAreaViewInfo.TabsViewInfo.Bounds);

    DXBAR_TOOLBAR, DXBAR_TOOLBARINPOPUP:
      begin
        Result := ColorScheme.GetPartContentOffsets(APart);
        Inc(Result.Bottom, GetGroupCaptionHeight +
          Painter.GetGroupCaptionBottomOffset(APart = DXBAR_TOOLBARINPOPUP));
      end
...
function TdxCustomRibbon.SkinGetPartColor(APart: Integer;
AState: Integer = 0): TColor;
begin
  ColorScheme.InitializePaintData(Painter);
...
function TdxCustomRibbon.SkinGetPartColorPalette(APart: Integer;
AState: Integer = 0): IdxColorPalette;
begin
  ColorScheme.InitializePaintData(Painter);
  case APart of
    DXBAR_BACKSTAGEVIEW_MENUBAR_ITEM:
...
begin
  Result := IsWindowEnabled(Handle) and not Hidden and
    (ShowTabHeaders or ShowTabGroups) and
...
begin
  ...
  if AOperation = bmoUnmerge then
    AHandled := (ABarManager = nil) or (ARibbon <> nil) and
...
begin
  if Assigned(OnMinimizedChanged) then
    OnMinimizedChanged(Self);
end;
...
procedure TdxCustomRibbon.DoContextPopup(MousePos: TPoint;
var Handled: Boolean);
begin
  if IsDesigning then
...
function TdxCustomRibbon.DoMouseWheelDown(Shift: TShiftState;
MousePos: TPoint): Boolean;
begin
  Result := inherited DoMouseWheelDown(Shift, MousePos);
  if not Result then
...
function TdxCustomRibbon.DoMouseWheelUp(Shift: TShiftState;
MousePos: TPoint): Boolean;
begin
  Result := inherited DoMouseWheelUp(Shift, MousePos);
  if not Result then
...
function TdxCustomRibbon.GetDesignHitTest(X, Y: Integer;
Shift: TShiftState): Boolean;
begin
  Result := inherited GetDesignHitTest(X, Y, Shift) or
    ViewInfo.GetDesignHitTest(Point(X, Y));
end;
...
procedure TdxCustomRibbon.MouseDown(Button: TMouseButton; Shift: TShiftState;
X, Y: Integer);
begin
  inherited MouseDown(Button, Shift, X, Y);
...
begin
  ...
  begin
    if ViewInfo.UseGlass then
      Canvas.FillRect(cxRectSetHeight(ClientRect,
        GetRibbonFormCaptionHeight), clBlack);
...
procedure TdxCustomRibbon.Notification(AComponent: TComponent;
Operation: TOperation);
...
begin
  ...
  begin
    ...
    if Assigned(AParent) and not(AParent is TCustomForm) then
      raise EdxException.CreateFmt(cxGetResourceString(@dxSBAR_RIBBONBADPARENT),
        [ClassName]);
  end;
...
procedure TdxCustomRibbon.CheckDrawRibbonFormStatusBarBorders
  (ACanvas: TcxCanvas; const ABordersWidth: TRect);
var
...
begin
  ...
  if (AFormData.GetState <> wsNormal) or
    (ApplicationMenuState >= ramsShownAsFrame) then
    Exit;
  if Supports(GetStatusBarInterface, IdxRibbonFormStatusBar, AIntf) then
  begin
    ATop := AFormData.GetBounds.Bottom - AIntf.GetHeight -
      GetWindowBordersWidth.Bottom;
    ...
    try
      ...
      ColorScheme.DrawFormStatusBarPart(ACanvas.Handle, ALeftStatusBarBounds,
        not UseRightToLeftAlignment, AFormData.GetIsActive,
        AIntf.GetIsRaised(not UseRightToLeftAlignment), AIsRectangular);
      ACanvas.ExcludeClipRect(ALeftStatusBarBounds);

      ARightStatusBarBounds :=
        cxRect(AFormData.GetBounds.Right - ABordersWidth.Right, ATop,
        AFormData.GetBounds.Right, ABottom);
      ColorScheme.DrawFormStatusBarPart(ACanvas.Handle, ARightStatusBarBounds,
        UseRightToLeftAlignment, AFormData.GetIsActive,
...
procedure TdxCustomRibbon.DrawApplicationMenuHeader(ADC: THandle;
...
function TdxCustomRibbon.AddGroupButtonToQAT(ABar: TdxBar)
  : TdxRibbonQuickAccessGroupButton;
begin
  Result := QuickAccessToolbar.AddGroupButton(ABar);
end;

function TdxCustomRibbon.CalculateFormCaptionHeight
  (AIsQuickAccessToolbarAtNonClientArea: Boolean): Integer;
var
  H: Integer;
begin
  ...
  begin
    ...
    if RibbonForm.IsUseAeroNCPaint then
      ...
      H := Max(dxGetSystemMetrics(SM_CYCAPTION, ScaleFactor) - 1,
        dxGetSystemMetrics(SM_CYSIZE, ScaleFactor)) + ScaleFactor.Apply(6);

    H := Max(H, Abs(Fonts.GetFormCaptionFont.Height) * 2);

    // quick access toolbar
    if AIsQuickAccessToolbarAtNonClientArea and IsBarManagerValid then
      H := Max(H, GetGroupRowHeight +
        IfThen(RibbonForm.IsUseAeroNCPaint, 7, 9));
    Result := H + RibbonForm.GetCaptionHeightDelta
      (AIsQuickAccessToolbarAtNonClientArea);
...
begin
  Result := Fading and dxFader.IsReady and
    not(IsLocked or IsDesigning or TdxCustomRibbonSkinAccess(ColorScheme)
    .LowColors);
end;
...
function TdxCustomRibbon.CreateTabAreaSearchToolbar
  : TdxRibbonTabAreaSearchToolbar;
begin
...
function TdxCustomRibbon.CreateContextAsByMerging(ASourceContext
  : TdxRibbonContext): TdxRibbonContext;
...
function TdxCustomRibbon.CreateTabAsByMerging(ASourceTab: TdxRibbonTab)
  : TdxRibbonTab;
  ...
  function GetTargetTabContext(ASourceContext: TdxRibbonContext)
    : TdxRibbonContext;
  var
    AContext: TdxRibbonContext;
    I: Integer;
  begin
    ...
    begin
      ...
      begin
        AContext := Contexts[I];
        if AContext.FCreatedByMerging and
          (AContext.FCreatedByMergingWith = ASourceContext) then
        begin
          Result := AContext;
          Break;
        end;
...
const
  ClassMap: array [TdxRibbonStyle] of TdxRibbonViewInfoClass =
    (TdxRibbonViewInfo2007, TdxRibbonViewInfo2010, TdxRibbonViewInfo2013OrNewer,
    TdxRibbonViewInfo2013OrNewer, TdxRibbonViewInfo2016Tablet,
    TdxRibbonViewInfo2019);
begin
  Result := ClassMap[Style].Create(Self);
end;

procedure TdxCustomRibbon.DesignAddTabGroup(ATab: TdxRibbonTab;
ANewToolbar: Boolean);
var
  AGroup: TdxRibbonTabGroup;
begin
  if ATab = nil then
    ATab := ActiveTab;
  if (ATab = nil) or not IsDesigning then
    Exit;
  if ANewToolbar then
  begin
...
function TdxCustomRibbon.DoHideMinimizedByClick(AWnd: HWND; AShift: TShiftState;
const APos: TPoint): Boolean;
begin
  Result := True;
...
procedure TdxCustomRibbon.AddAffiliatedBarControlForAccessibility
  (ABarControl: TCustomdxBarControl);
begin
  FAffiliatedBarControlsForAccessibility.Add(ABarControl);
end;

function TdxCustomRibbon.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
  Result := TdxRibbonAccessibilityHelper;
end;

function TdxCustomRibbon.GetBar(ACustomizingBarControl
  : TCustomdxBarControl): TdxBar;
var
...
function TdxCustomRibbon.GetNextHorizontalAccessibleObject(ACurrentObject
  : IdxBarAccessibilityHelper; ADirection: TcxAccessibilityNavigationDirection)
  : IdxBarAccessibilityHelper;
var
  AIndex: Integer;
begin
  ...
  begin
    ...
    else if AIndex > HorizontalNavigationList.Count - 1 then
      AIndex := 0;
...
begin
  Result := IsAutoHideModeActive and
    not RibbonForm.RibbonAutoHideMode.IsUIShown;
end;

function TdxCustomRibbon.IsAutoHideAnimationIsInProgress: Boolean;
begin
  Result := IsAutoHideModeActive and TdxRibbonAutoHideModeAccess
    (RibbonForm.RibbonAutoHideMode).IsAnimationInProgress;
end;
...
procedure TdxCustomRibbon.InternalCloseTabGroupsPopupWindow(AAllowAnimation
  : Boolean);
begin
...
AItems: TdxRibbonPopupMenuItems; AOnClick: TNotifyEvent);
  ...
  begin
    ALink := BarDesignController.AddInternalItem(ALinks, TdxBarButton,
      BarManager, FInternalItems);
...
begin
  ...
  if rpmiQATAddRemoveItem in AItems then
    if QuickAccessToolbar.Contains(BarDesignController.CustomizingItemLink) then
      AddMenuItem(cxGetResourceString(@dxSBAR_REMOVEFROMQAT), False,
        rpmiQATAddRemoveItem)
    else
      AddMenuItem(GetRibbonQATOptionAddItemCaption, False, rpmiQATAddRemoveItem,
        BarDesignController.CustomizingBarControl).Enabled :=
        GetRibbonQATOptionAddItemEnabled;
  ANeedSeparator := ALinks.Count > 0;
  if rpmiMoreCommands in AItems then
  begin
    AddMenuItem(cxGetResourceString(@dxSBAR_MORECOMMANDS), ANeedSeparator,
      rpmiMoreCommands);
    ANeedSeparator := False;
  end;
  if rpmiCustomizeQAT in AItems then
  begin
    AddMenuItem(cxGetResourceString(@dxSBAR_CUSTOMIZERIBBONQAT), ANeedSeparator,
      rpmiCustomizeQAT);
    ANeedSeparator := False;
  end;
  if rpmiQATPosition in AItems then
    AddMenuItem(GetRibbonQATPositionButtonCaption, ANeedSeparator,
      rpmiQATPosition);
  ANeedSeparator := True;
  if rpmiCustomizeRibbon in AItems then
  begin
    AddMenuItem(cxGetResourceString(@dxSBAR_CUSTOMIZERIBBON), ANeedSeparator,
      rpmiCustomizeRibbon);
    ANeedSeparator := False;
  end;
  if rpmiMinimizeRibbon in AItems then
  begin
    AButton := AddMenuItem(cxGetResourceString(@dxSBAR_MINIMIZERIBBON),
      ANeedSeparator, rpmiMinimizeRibbon);
...
begin
  case TdxRibbonPopupMenuItem(TdxBarButton(Sender).Tag) of
    ...
    rpmiQATAddRemoveItem:
      if QuickAccessToolbar.Contains(BarDesignController.CustomizingItemLink)
      then
        RemoveItemFromQAT
      else
        AddItemToQAT;
...
begin
  if IsBarManagerValid then
    BarDesignController.ShowCustomCustomizePopup(BarManager,
      InitCustomizePopupMenu, GroupsPainter);
end;
...
begin
  ...
  begin
    if ASetControl then
    begin
      if (ARibbonForm.FormStyle = fsMDIForm) and
        Assigned(ARibbonForm.ActiveMDIChild) then
        ARibbonForm.PrevActiveControl :=
          ARibbonForm.ActiveMDIChild.ActiveControl
      else
...
function TdxCustomRibbon.GetApplicationButtonViewInfo
  : TdxRibbonApplicationButtonViewInfo;
begin
  Result := ViewInfo.ApplicationButtonViewInfo;
end;
...
function TdxCustomRibbon.GetNextActiveTab(ATab: TdxRibbonTab): TdxRibbonTab;
  ...
  begin
    if ATab = nil then
      Result := 0
    else if not(csDestroying in ATab.ComponentState) then
      Result := ATab.Index
    else
...
begin
  ...
  begin
    ...
    if (AForm <> nil) and AForm.HandleAllocated then
      ...
      begin
        if Supports(RibbonFormNonClientParts[I], IdxRibbonFormStatusBar, AIntf)
          and AIntf.GetActive(AForm) then
        begin
          Result := AIntf;
          Break;
        end;
...
begin
  ...
  else if (FActiveTab <> Value) and not(csDestroying in ComponentState) then
  begin
    if not IsDesigning then
    begin
      if IsBarManagerValid then
      ...
      if not DoTabChanging(Value) then
        Exit;
    end;
...
procedure TdxCustomRibbon.SetApplicationButton
  (AValue: TdxRibbonApplicationButton);
begin
  FApplicationButton.Assign(AValue);
end;
...
procedure TdxCustomRibbon.SetBarManager(Value: TdxBarManager);
  ...
  procedure RemoveChangeHandler(AHandlerCollection: TcxEventHandlerCollection;
  AEvent: TcxEventHandler);
  begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        RemoveChangeHandler(FBarManager.MDIStateChangedHandlers,
          MDIStateChanged);
        RemoveChangeHandler(FBarManager.SystemFontChangedHandlers,
          SystemFontChanged);
        RemoveChangeHandler(FBarManager.ReadIniFileHandlers, BarManagerLoadIni);
        RemoveChangeHandler(FBarManager.WriteIniFileHandlers,
          BarManagerSaveIni);
...
procedure TdxCustomRibbon.SetCapitalizeTabCaptions
...
procedure TdxCustomRibbon.SetColorSchemeEx(const AName: string;
AStyle: TdxRibbonStyle);
begin
  ColorScheme := dxRibbonSkinsManager.GetMostSuitable(AName, AStyle,
    ScaleFactor.Apply(dxDefaultDPI));
end;
...
procedure TdxCustomRibbon.SetColorSchemeAccent
  (AValue: TdxRibbonColorSchemeAccent);
begin
...
begin
  ...
  begin
    FDocumentName := Value;
    AForm := RibbonForm;
    if (AForm <> nil) and (Application.MainForm = AForm) and
      (AForm.HandleAllocated) then
      TdxFormHelper.SetWindowTextWithoutRedrawing(AForm.Handle,
        ViewInfo.GetFormCaptionText);
    Changed;
  end;
end;
...
procedure TdxCustomRibbon.SetPopupMenuItems(const Value
  : TdxRibbonPopupMenuItems);
begin
...
procedure TdxCustomRibbon.SetQuickAccessToolbar(const Value
  : TdxRibbonQuickAccessToolbar);
begin
...
procedure TdxCustomRibbon.SetTabAreaSearchToolbar
  (const Value: TdxRibbonTabAreaSearchToolbar);
begin
...
procedure TdxCustomRibbon.SetTabAreaToolbar(const Value
  : TdxRibbonTabAreaToolbar);
begin
...
begin
  ...
  begin
    QuickAccessToolbar.Position := TdxQuickAccessToolbarPosition
      (AEventData.IniFile.ReadInteger(ASection,
      'QuickAccessToolbarPosition', 0));
    ShowTabGroups := AEventData.IniFile.ReadBool(ASection,
      'ShowTabGroups', True);
...
begin
  AEventData := TdxBarIniFileEventData(AEventArgs);
  ASection := GetIniSection(AEventData.Delimiter, AEventData.BaseSection);
  AEventData.IniFile.WriteInteger(ASection, 'QuickAccessToolbarPosition',
    Ord(QuickAccessToolbar.Position));
...
AMergeOptions: TdxRibbonMergeOptions = dxRibbonDefaultMergeOptions);
  ...
  function FindTargetTab(ASourceTab: TdxRibbonTab;
  out ATargetTab: TdxRibbonTab): Boolean;
  var
    I: Integer;
  begin
    Result := False;
...
begin
  ...
  try
    AForm := GetParentForm(Self, False);
    AValid := Assigned(AForm) and AForm.HandleAllocated and
...
begin
  ...
  if (AQATLinks <> nil) and (AToolbar <> nil) then
    ...
    begin
      ...
      if not ALink.IsMarkedForDeletion and
        (ALink.Item is TdxRibbonQuickAccessGroupButton) then
      begin
        AGroupButton := TdxRibbonQuickAccessGroupButton(ALink.Item);
        if (AGroupButton.Toolbar = AToolbar) or
          TdxRibbonMergeHelper.IsMergedWith(AToolbar, AGroupButton.Toolbar) then
        begin
          Result := True;
          Break;
        end;
...
begin
  Result := (AToolbar <> nil) and
    (AToolbar.DockControl is TdxRibbonGroupsDockControl) and
...
var
  AQATBar: TdxBar;
begin
  Result := TdxBarManagerAccess(BarManager).IsInitializing or (AToolbar = nil)
    or (LinkCount = 0);
  if not Result then
  begin
    AQATBar := TdxBar(Links[0].Owner.Owner);
    Result := TdxRibbonQuickAccessToolbarHelper.IsToolbarDockedInRibbon
      (TdxRibbonQuickAccessToolbarDockControl(AQATBar.RealDockControl).Ribbon,
      AToolbar) and not HasGroupButtonForToolbar(AQATBar, AToolbar);
  end;
end;
...
out AErrorText: string): Boolean;
  ...
  begin
    ...
    else if AToolbar.DockControl is TdxRibbonQuickAccessToolbarDockControl then
      ARibbon := TdxRibbonQuickAccessToolbarDockControl
        (AToolbar.DockControl).Ribbon
    else
      ARibbon := nil;
  ...
  begin
    Result := (Toolbar.DockControl is TdxRibbonGroupsDockControl) and
      (TdxRibbonGroupsDockControl(Toolbar.DockControl).BarManager = BarManager)
      and (IsDockedInRibbon(Toolbar) or IsDockedInRibbon(TdxBarAccess(Toolbar)
      .MergeData.MergedWith));
  end;
...
begin
  ...
  if (AParentKind <> pkBar) or not GetBarControlClass(AToolbar)
    .InheritsFrom(TdxRibbonQuickAccessToolbarBarControl) then
  begin
  ...
  begin
    AErrorText := cxGetResourceString
      (@dxSBAR_QUICKACCESSGROUPBUTTONTOOLBARNOTDOCKEDINRIBBON);
    Exit;
  end;
...
begin
  if (Toolbar <> nil) and (TdxBarAccess(Toolbar).MergeData.MergedWith <> nil)
  then
...
function TdxRibbonQuickAccessGroupButton.HasGroupButtonForToolbar(AParentBar,
  AToolbar: TdxBar): Boolean;
var
...
begin
  ...
  Result := TdxRibbonQuickAccessToolbarHelper.HasGroupButtonForToolbar
    (AParentBarItemLinks, AToolbar);
end;
...
procedure TdxRibbonQuickAccessGroupButtonControl.CalcDrawParams
  (AFull: Boolean = True);
begin
  inherited CalcDrawParams(AFull);
  DrawParams.DroppedDown := IsDroppedDown;
  DrawParams.Enabled := DrawParams.Enabled and
    (Item.IsDesigning or (Item.Toolbar <> nil));
end;
...
procedure TdxRibbonQuickAccessGroupButtonControl.DoCloseUp(AHadSubMenuControl
  : Boolean);
begin
  ClosePopup;
  Repaint;
...
begin
  ...
  if AToolbar <> nil then
  begin
    FPopupBarControl := TdxRibbonQuickAccessGroupButtonPopupBarControl.
      CreateForPopup(Self);
    R := ItemLink.ItemRect;
    ...
    if BarNavigationController.NavigationMode and not BarNavigationController.KeyTipsHandlingMode
    then
      SelectFirstSelectableAccessibleObject
        (FPopupBarControl.IAccessibilityHelper.GetBarHelper);
    Repaint;
  end;
end;
...
PaintType: TdxBarPaintType);
begin
  TdxRibbonQuickAccessToolbarPainter(Painter).DrawGroupButtonControl
    (DrawParams, ARect);
end;
...
function TdxRibbonQuickAccessGroupButtonControl.GetAccessibilityHelperClass
  : TdxBarAccessibilityHelperClass;
begin
...
out AImageIndex: Integer; out AIsImageEnabled: TdxDefaultBoolean): Boolean;
begin
...
function TdxRibbonQuickAccessGroupButtonControl.GetItem
  : TdxRibbonQuickAccessGroupButton;
begin
  Result := TdxRibbonQuickAccessGroupButton(inherited Item);
end;

function TdxRibbonQuickAccessGroupButtonControl.GetViewStructure
  : TdxBarItemControlViewStructure;
begin
  Result := [cpIcon];
end;
...
function TdxRibbonQuickAccessGroupButtonPopupBarControl.CanActiveChange
  : Boolean;
begin
...
begin
  if FGroupButtonControl.MousePressed then
    TdxRibbonQuickAccessToolbarBarControl(FGroupButtonControl.Parent)
...
function TdxRibbonQuickAccessGroupButtonPopupBarControl.GetBehaviorOptions
  : TdxBarBehaviorOptions;
begin
...
function TdxRibbonQuickAccessGroupButtonPopupBarControl.
  IsAllowContextPaint: Boolean;
begin
  Result := False;
end;
...
class function TdxAddGroupButtonEditor.GetAddedItemClass(const AAddedItemName
  : string): TdxBarItemClass;
begin
  Result := TdxRibbonQuickAccessGroupButton;
end;
...
function TdxRibbonAccessibilityHelper.GetDefaultAccessibleObject
  : IdxBarAccessibilityHelper;
begin
  Result := nil;
  if Ribbon.AreGroupsVisible then
    Result := Ribbon.TabsIAccessibilityHelper.GetDefaultAccessibleObject;
  if (Result = nil) and Ribbon.ApplicationButton.IAccessibilityHelper.GetHelper.Visible
  then
    Result := Ribbon.ApplicationButton.IAccessibilityHelper;
end;

function TdxRibbonAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
...
begin
  ...
  for I := 0 to Ribbon.ViewInfo.TabsAreaViewInfo.ButtonsViewInfo.Count - 1 do
    AddAccessibilityChild(Ribbon.ViewInfo.TabsAreaViewInfo.ButtonsViewInfo[I]
      .AccessibilityHelper);
  ...
  for I := 0 to Ribbon.FAffiliatedBarControlsForAccessibility.Count - 1 do
    AddAccessibilityChild
      (TCustomdxBarControl(Ribbon.FAffiliatedBarControlsForAccessibility[I])
...
begin
  Result := FAccessibilityChildren.IndexOf
    (Pointer(dxBar.GetAccessibilityHelper(AChild)));
end;
...
function TdxRibbonAccessibilityHelper.LogicalNavigationGetChild(AIndex: Integer)
  : TdxBarAccessibilityHelper;
begin
...
function TdxRibbonAccessibilityHelper.GetTabsAreaButtonsViewInfo
  : TdxRibbonButtonsContainerViewInfo;
begin
...
function TdxRibbonTabCollectionAccessibilityHelper.GetDefaultAccessibleObject
  : IdxBarAccessibilityHelper;
begin
...
function TdxRibbonTabCollectionAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
...
function TdxRibbonTabCollectionAccessibilityHelper.GetChildIndex
  (AChild: TcxAccessibilityHelper): Integer;
begin
  if (AChild is TdxRibbonTabAccessibilityHelper) and
    (TdxRibbonTabAccessibilityHelper(AChild).Parent = Self) then
    Result := TabsViewInfo.FCells.IndexOf
      (TdxRibbonTabAccessibilityHelper(AChild).Tab)
  else
    Result := -1;
end;
...
function TdxRibbonTabCollectionAccessibilityHelper.GetParent
  : TcxAccessibilityHelper;
begin
...
function TdxRibbonTabCollectionAccessibilityHelper.
  LogicalNavigationGetChildCount: Integer;
var
...
function TdxRibbonTabCollectionAccessibilityHelper.
  LogicalNavigationGetChildIndex(AChild: TdxBarAccessibilityHelper): Integer;
var
  I: Integer;
begin
  Result := -1;
...
function TdxRibbonTabCollectionAccessibilityHelper.GetTabCollection
  : TdxRibbonTabCollection;
begin
  Result := TdxRibbonTabCollection(FOwnerObject);
end;

function TdxRibbonTabCollectionAccessibilityHelper.GetTabsViewInfo
  : TdxRibbonTabsViewInfo;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Ribbon.ShowTabGroupsPopupWindow;
      SelectFirstSelectableAccessibleObject
        (Tab.DockControl.IAccessibilityHelper.GetBarHelper);
    end;
  end;
end;
...
function TdxRibbonTabAccessibilityHelper.
  LogicalNavigationGetNextAccessibleObject(AShift: TShiftState)
  : IdxBarAccessibilityHelper;
var
  AGoForward: Boolean;
begin
...
procedure TdxRibbonTabAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
  inherited Unselect(ANextSelectedObject);
  Tab.Invalidate;
...
function TdxRibbonTabAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
...
begin
  Result := TdxRibbonTabCollection(Tab.Collection)
    .IAccessibilityHelper.GetHelper;
end;
...
procedure TdxRibbonTabAccessibilityHelper.GetKeyTipInfo(out AKeyTipInfo
  : TdxBarKeyTipInfo);
var
  ABasePoint: TPoint;
...
// IdxBarAccessibilityHelper
function TdxRibbonApplicationButtonAccessibilityHelper.GetBarManager
  : TdxBarManager;
begin
  Result := Ribbon.BarManager;
end;
...
procedure TdxRibbonApplicationButtonAccessibilityHelper.Select
  (ASetFocus: Boolean);
begin
  inherited Select(ASetFocus);
...
function TdxRibbonApplicationButtonAccessibilityHelper.
  GetOwnerObjectWindow: HWND;
begin
  Result := Parent.OwnerObjectWindow;
end;

function TdxRibbonApplicationButtonAccessibilityHelper.GetParent
  : TcxAccessibilityHelper;
begin
  Result := Ribbon.IAccessibilityHelper.GetHelper;
end;
...
procedure TdxRibbonApplicationButtonAccessibilityHelper.GetKeyTipInfo
  (out AKeyTipInfo: TdxBarKeyTipInfo);
begin
  inherited GetKeyTipInfo(AKeyTipInfo);
  AKeyTipInfo.BasePoint :=
    cxRectCenter(GetScreenBounds(cxAccessibleObjectSelfID));
  AKeyTipInfo.HorzAlign := taCenter;
  AKeyTipInfo.VertAlign := vaCenter;
  AKeyTipInfo.Enabled := True;
end;

procedure TdxRibbonApplicationButtonAccessibilityHelper.KeyTipHandler
  (Sender: TObject);
begin
...
function TdxRibbonApplicationButtonAccessibilityHelper.
  GetApplicationButtonViewInfo: TdxRibbonApplicationButtonViewInfo;
begin
  Result := Ribbon.ApplicationButtonViewInfo;
end;

function TdxRibbonApplicationButtonAccessibilityHelper.GetRibbon
  : TdxCustomRibbon;
begin
...
function TdxRibbonGroupsDockControlAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
  Result := DockControl.ViewInfo.GroupViewInfos[AIndex]
    .BarControl.IAccessibilityHelper.GetHelper;
end;
...
function TdxRibbonGroupsDockControlAccessibilityHelper.GetParent
  : TcxAccessibilityHelper;
begin
  if dxBarGetParentPopupWindow(DockControl, False) <> nil then
    Result := nil
  else
    Result := TdxRibbonGroupsDockControl(DockControl)
      .Tab.IAccessibilityHelper.GetHelper;
end;
...
begin
  ...
  if not TdxRibbonGroupsDockControl(DockControl).Tab.Ribbon.AreGroupsVisible
  then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;

function TdxRibbonGroupsDockControlAccessibilityHelper.GetParentForKeyTip
  : TdxBarAccessibilityHelper;
begin
  if dxBarGetParentPopupWindow(DockControl, False) <> nil then
...
function TdxRibbonGroupsDockControlAccessibilityHelper.GetDockControl
  : TdxRibbonGroupsDockControl;
begin
...
function TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.GetChild
...
function TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.
...
function TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.GetChildIndex
  (AChild: TcxAccessibilityHelper): Integer;
begin
...
function TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.GetParent
  : TcxAccessibilityHelper;
begin
  Result := TdxRibbonQuickAccessToolbarBarControl(BarControl)
    .Ribbon.IAccessibilityHelper.GetHelper;
end;
...
procedure TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.
  DoGetKeyTipsData(AKeyTipsData: TList);
  ...
  begin
    ...
    begin
      AChild := ABarControl.ItemLinks.VisibleItems[I]
        .Control.IAccessibilityHelper.GetBarHelper;
  ...
  begin
    ...
    begin
      case I of
        0 .. 8:
          TdxBarKeyTipData(AItemKeyTipsData[I]).KeyTip := IntToStr(I + 1);
          // '1'..'9'
        9 .. 17:
          TdxBarKeyTipData(AItemKeyTipsData[I]).KeyTip :=
            '0' + IntToStr(18 - I); // '09'..'01'
        18 .. 44:
          TdxBarKeyTipData(AItemKeyTipsData[I]).KeyTip :=
            '0' + Char(Ord('A') + (I - 18)); // '0A'..'0Z'
      else
        TdxBarKeyTipData(AItemKeyTipsData[I]).KeyTip := '';
      end;
...
begin
  ...
  try
    VisibleItemCount := TdxBarItemLinksAccess(BarControl.ItemLinks)
      .VisibleItemCount;
    if BarControl.IsPopup then
    begin
      ARealVisibleItemCount := TdxBarItemLinksAccess
        (BarControl.ParentBar.ItemLinks).RealVisibleItemCount;
      GetItemsKeyTipsData(BarControl.ParentBar, 0, ARealVisibleItemCount - 1,
        AItemKeyTipsData, False);
      GetItemsKeyTipsData(BarControl, 0, VisibleItemCount - 1 - 1 { Mark!!! } ,
        AItemKeyTipsData, True);
    end
    else
    begin
      ARealVisibleItemCount := TdxBarItemLinksAccess(BarControl.ItemLinks)
        .RealVisibleItemCount;
      GetItemsKeyTipsData(BarControl, 0, ARealVisibleItemCount - 1,
        AItemKeyTipsData, True);
      GetItemsKeyTipsData(BarControl, ARealVisibleItemCount,
        VisibleItemCount - 1, AItemKeyTipsData, False);
      if not BarControl.AllItemsVisible then
        TdxBarAccessibilityHelperAccess
          (BarControl.MarkIAccessibilityHelper.GetBarHelper)
...
procedure TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.
  InitializeItemKeyTipPosition(AItemLinkHelper
  : TdxBarItemLinkAccessibilityHelper; var AKeyTipInfo: TdxBarKeyTipInfo);
begin
  inherited;
  AKeyTipInfo.VertAlign := vaBottom;
end;
...
begin
  ...
  try
    GetChildrenForNavigation(AItemLinkHelper, InternalGetRootObject,
      AItemLinkHelper.GetScreenBounds(cxAccessibleObjectSelfID), ADirection,
      True, AObjects);
    Result := dxBar.GetNextAccessibleObject(AItemLinkHelper, AObjects,
      ADirection, True);
  finally
    AObjects.Free;
  end;
...
begin
  ...
  begin
    ASelectedControl := TdxRibbonQuickAccessToolbarBarControl(BarControl)
      .SelectedControl;
    inherited KeyTipsEscapeHandler;
    BarNavigationController.ChangeSelectedObject(True,
      ASelectedControl.IAccessibilityHelper);
  end
  else
  begin
    BarNavigationController.SetKeyTipsShowingState
      (GetKeyTipContainerParent(Self), '');
    AMarkAccessibleObject := TdxRibbonQuickAccessToolbarBarControl
      (BarControl.ParentBar).MarkIAccessibilityHelper;
    TdxRibbonQuickAccessToolbarBarControl(BarControl.ParentBar).MarkState
      := msNone;
...
function TdxRibbonQuickAccessToolbarBarControlAccessibilityHelper.GetBarControl
  : TdxRibbonQuickAccessToolbarBarControl;
begin
...
// IdxBarAccessibilityHelper
function TdxRibbonQuickAccessToolbarBarControlMarkAccessibilityHelper.
  HandleNavigationKey(var AKey: Word): Boolean;
begin
  Result := inherited HandleNavigationKey(AKey);
...
procedure TdxRibbonQuickAccessToolbarBarControlMarkAccessibilityHelper.
  GetKeyTipInfo(out AKeyTipInfo: TdxBarKeyTipInfo);
begin
  inherited;
  AKeyTipInfo.BasePoint :=
    cxRectCenter(GetScreenBounds(cxAccessibleObjectSelfID));
...
function TdxRibbonQuickAccessToolbarBarControlMarkAccessibilityHelper.
  GetBarControl: TdxRibbonQuickAccessToolbarBarControl;
begin
  Result := TdxRibbonQuickAccessToolbarBarControl(FOwnerObject);
end;
...
function TdxRibbonGroupBarControlAccessibilityHelper.IsNavigationKey
  (AKey: Word): Boolean;
begin
...
procedure TdxRibbonGroupBarControlAccessibilityHelper.Select
  (ASetFocus: Boolean);
begin
  ...
  begin
    ...
    TdxRibbonGroupsDockControl(BarControl.DockControl)
      .MakeRectFullyVisible(BarControl.BoundsRect);
  end;
end;
...
function TdxRibbonGroupBarControlAccessibilityHelper.Expand
  : TCustomdxBarControlAccessibilityHelper;
begin
  if not IsCollapsed then
    raise EdxException.Create
      ('TdxRibbonGroupBarControlAccessibilityHelper.Expand fails');
  ShowPopupBarControl;
  Result := TCustomdxBarControlAccessibilityHelper
    (BarDesignController.QuickControl.IAccessibilityHelper.GetHelper);
end;

procedure TdxRibbonGroupBarControlAccessibilityHelper.
  GetCaptionButtonKeyTipPosition(ACaptionButton: TdxBarCaptionButton;
out ABasePointY: Integer; out AVertAlign: TcxAlignmentVert);
begin
...
begin
  ...
  AOneRowHeightItemControl := not BarControl.Ribbon.IsSimplifiedGroupsLayout and
    (AItemControl.ViewInfo.ViewLevel <> ivlLargeIconWithText);

  if AOneRowHeightItemControl then
  begin
    ARow := IdxBarItemControlViewInfo(AItemControl.ViewInfo).GetRow;
    if (IdxBarItemControlViewInfo(AItemControl.ViewInfo).GetColumnRowCount = 2)
      and (ARow = 1) then
      ARow := 2;
    if IdxBarItemControlViewInfo(AItemControl.ViewInfo).GetColumnRowCount = 1
    then
      ARow := 1;
    AKeyTipInfo.BasePoint.Y :=
      BarControl.ViewInfo.RowKeyTipsBaseLinePositions[ARow];
  end
  else
    AKeyTipInfo.BasePoint.Y := BarControl.ViewInfo.RowKeyTipsBaseLinePositions
      [BarControl.Ribbon.GroupsPainter.GetGroupRowCount - 1];

  AKeyTipInfo.VertAlign := vaCenter;

  if AOneRowHeightItemControl and
    (cpIcon in TdxBarItemControlAccess(AItemControl).FDrawParams.ViewStructure)
  then
  begin
    ProcessPaintMessages;
    // AItemControl.ViewInfo.ImageBounds are calculated on painting
    AKeyTipInfo.BasePoint.X :=
      cxRectCenter(AItemControl.ViewInfo.ImageBounds).X;
    AKeyTipInfo.BasePoint.X := AItemControl.Parent.ClientToScreen
      (AKeyTipInfo.BasePoint).X;
  ...
  begin
    AKeyTipInfo.BasePoint.X :=
      cxRectCenter(GetItemScreenBounds(AItemLinkHelper)).X;
    AKeyTipInfo.HorzAlign := taCenter;
  end;
end;
...
procedure TdxRibbonGroupBarControlAccessibilityHelper.GetKeyTipInfo
  (out AKeyTipInfo: TdxBarKeyTipInfo);
var
...
procedure TdxRibbonGroupBarControlAccessibilityHelper.GetKeyTipData
  (AKeyTipsData: TList);
  ...
  begin
    ...
    begin
      ...
      with TdxBarItemControlAccessibilityHelperAccess
        (AItemControl.IAccessibilityHelper.GetBarHelper) do
        if CanSelect then
          GetKeyTipData(AKeyTipsData);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      TdxBarCaptionButtonAccessibilityHelperAccess
        (ACaptionButton.IAccessibilityHelper.GetHelper)
        .GetKeyTipData(AKeyTipsData);
    end;
  end;
...
ADirection: TcxAccessibilityNavigationDirection): IdxBarAccessibilityHelper;
  ...
  function FindAmongItemControlsAndCollapsedBarControls(ASelectedObject
    : TdxBarAccessibilityHelper; const ASelectedObjectScreenBounds: TRect)
    : IdxBarAccessibilityHelper;
    ...
    begin
      ...
      begin
        ...
        GetChildrenForNavigation(ASelectedObject,
          AItemControl1.IAccessibilityHelper.GetBarHelper,
          ASelectedObjectScreenBounds, ADirection, False, AObjects);
      end;
    end;
  ...
  begin
    ...
    try
      if BarControl.IsPopup then
        ...
        begin
          ABarControl := TdxRibbonGroupBarControlAccessibilityHelper
            (Parent.Childs[I]).BarControl;
          if ABarControl.Collapsed then
            GetChildrenForNavigation(ASelectedObject,
              ABarControl.IAccessibilityHelper.GetBarHelper,
              ASelectedObjectScreenBounds, ADirection, False, AObjects)
          else if not((ADirection in [andUp, andDown]) and
            (ABarControl <> BarControl)) then
            GetBarControlChildren(ABarControl, AObjects);
        end;
      Result := dxBar.GetNextAccessibleObject(ASelectedObject, AObjects,
        ADirection, True);
    finally
      AObjects.Free;
    end;
...
begin
  ...
  AScreenBounds := TdxBarAccessibilityHelperAccess(ASelectedObject)
    .GetScreenBounds(cxAccessibleObjectSelfID);
  Result := FindAmongItemControlsAndCollapsedBarControls(ASelectedObject,
    AScreenBounds);
  if Result <> nil then
    Exit;
  case ADirection of
    andUp:
      if not(BarControl.IsPopup or TdxRibbonGroupBarControl(BarControl)
        .Ribbon.IsPopupGroupsMode) then
        Result := TdxRibbonGroupBarControl(BarControl)
          .Ribbon.ActiveTab.IAccessibilityHelper;
    andDown:
      begin
        ACaptionButtonIndex := -1;
        if not BarControl.Collapsed then
          ACaptionButtonIndex :=
            BarControl.Bar.CaptionButtons.IAccessibilityHelper.GetHelper.
            GetNextSelectableChildIndex(-1, False);
        if ACaptionButtonIndex <> -1 then
          Result := BarControl.Bar.CaptionButtons[ACaptionButtonIndex]
            .IAccessibilityHelper
        else if not(BarControl.IsPopup or TdxRibbonGroupBarControl(BarControl)
          .Ribbon.IsPopupGroupsMode) and
          (BarControl.Ribbon.QuickAccessToolbar.IAccessibilityHelper <> nil)
        then
        begin
          AObjects := TList.Create;
          try
            GetChildrenForNavigation(ASelectedObject,
              BarControl.Ribbon.QuickAccessToolbar.IAccessibilityHelper.
              GetBarHelper, AScreenBounds, ADirection, False, AObjects);
            Result := dxBar.GetNextAccessibleObject(ASelectedObject, AObjects,
              ADirection, True);
          finally
            AObjects.Free;
...
function TdxRibbonGroupBarControlAccessibilityHelper.GetParentForKeyTip
  : TdxBarAccessibilityHelper;
begin
  if BarControl.IsPopup then
    Result := BarControl.ParentBar.IAccessibilityHelper.GetBarHelper
...
begin
  ...
  begin
    ...
    begin
      TdxRibbonGroupBarControlAccessibilityHelper
        (BarControl.ParentBar.IAccessibilityHelper.GetHelper)
        .KeyTipsEscapeHandler;
    ...
    begin
      ...
      begin
        ASelectedItemControl := TCustomdxBarControlAccess(BarControl.ParentBar)
          .SelectedControl;
        BarControl.Hide;
        BarNavigationController.ChangeSelectedObject(True,
          ASelectedItemControl.IAccessibilityHelper);
        BarNavigationController.SetKeyTipsShowingState
          (ASelectedItemControl.Parent.IAccessibilityHelper, '');
      end;
    end;
  end
  else
...
function TdxRibbonGroupBarControlAccessibilityHelper.GetBarControl
  : TdxRibbonGroupBarControl;
begin
...
function TdxRibbonQuickAccessGroupButtonControlAccessibilityHelper.
...
begin
  ...
  FRibbon := ARibbon;
  Canvas.Font.Height :=
    GetScaled(dxGetFontHeightForDefaultDPI(Canvas.Font.Size));
end;

procedure TdxRibbonKeyTipWindow.ShowKeyTip;
begin
  SetWindowRgn(Handle, CreateRoundRectRgn(0, 0, Width + 1, Height + 1,
    GetScaled(2), GetScaled(2)), True);
  Invalidate;
end;
...
const ABasePoint: TPoint; AHorzAlign: TAlignment; AVertAlign: TcxAlignmentVert;
AEnabled: Boolean; out AWindow: TObject);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        DeferWindowPos(WP, AWindow.Handle, HWND_TOPMOST, AWindow.Left,
          AWindow.Top, AWindow.Width, AWindow.Height, SWP_NOACTIVATE or
...
procedure TdxRibbonCustomKeyTipWindows.FreeNotificationHandler
  (Sender: TComponent);
begin
...
procedure TdxRibbonScrollBarPainter.DoDrawScrollBarPart(ACanvas: TcxCanvas;
const R: TRect; APart: TcxScrollBarPart; AState: TcxButtonState);
const
  StateMap: array [TcxButtonState] of Integer = (DXBAR_NORMAL, DXBAR_NORMAL,
    DXBAR_HOT, DXBAR_PRESSED, DXBAR_DISABLED);
  PartMap: array [TScrollBarKind, TcxScrollBarPart] of Integer =
    ((0, DXBAR_SCROLLBARHORZ_LINEUP, DXBAR_SCROLLBARHORZ_LINEDOWN,
    DXBAR_SCROLLBARHORZ_THUMBNAIL, DXBAR_SCROLLBARHORZ_PAGEUP,
    DXBAR_SCROLLBARHORZ_PAGEDOWN), (0, DXBAR_SCROLLBARVERT_LINEUP,
    DXBAR_SCROLLBARVERT_LINEDOWN, DXBAR_SCROLLBARVERT_THUMBNAIL,
    DXBAR_SCROLLBARVERT_PAGEUP, DXBAR_SCROLLBARVERT_PAGEDOWN));
begin
  if Skin <> nil then
    Skin.DrawBackground(ACanvas.Handle, R, PartMap[ScrollBar.Kind, APart],
      StateMap[AState])
  else
    inherited DoDrawScrollBarPart(ACanvas, R, APart, AState);
end;

procedure TdxRibbonScrollBarPainter.DrawScrollBarBackground(ACanvas: TcxCanvas;
const R: TRect);
const
  PartMap: array [TScrollBarKind] of Integer = (DXBAR_SCROLLBARHORZ_BACKGROUND,
    DXBAR_SCROLLBARVERT_BACKGROUND);
begin
  if Skin <> nil then
...
begin
  Result := TdxRibbonScrollBarHelper(inherited ScrollBar)
    .Owner.GetControl as TdxRibbonScrollBar;
end;
...
begin
  if Supports(Ribbon, IdxSkin, ASkin) then
    ASkin.DrawBackground(ACanvas.Handle, ClientRect,
      DXBAR_SCROLLBOX_SIZEGRIPAREA)
  else
    inherited Draw(ACanvas);
end;
...
BarDesignController.RegisterBarControlDesignHelper(TdxRibbonGroupBarControl,
  TdxRibbonGroupBarControlDesignHelper);
...
BarDesignController.UnregisterBarControlDesignHelper
  (TdxRibbonQuickAccessToolbarBarControl,
  TdxRibbonQuickAccessToolbarBarControlDesignHelper);
BarDesignController.UnregisterBarControlDesignHelper(TdxRibbonGroupBarControl,
  TdxRibbonGroupBarControlDesignHelper);
