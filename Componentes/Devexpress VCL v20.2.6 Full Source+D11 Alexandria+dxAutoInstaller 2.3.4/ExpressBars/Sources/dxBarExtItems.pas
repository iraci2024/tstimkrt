...
{$ENDIF}
  ...
  dxForms, cxDateUtils, cxGeometry;
...
type
  ...
  published
    ...
    property AllowClick: Boolean read FAllowClick write FAllowClick
      default False;
    property BorderStyle: TdxBarStaticBorderStyle read FBorderStyle
      write SetBorderStyle default sbsNone;
    property Glyph;
    property Height: Integer index 4 read FHeight write SetSizeValue default 0;
    property ImageIndex;
    property LeftIndent: Integer index 1 read FLeftIndent write SetSizeValue
      default 0;
    property RightIndent: Integer index 2 read FRightIndent write SetSizeValue
      default 0;
  ...
  published
    ...
    property CustomColorText: string read FCustomColorText
      write SetCustomColorText stored IsCustomColorTextStored;
  ...
  protected
    ...
    function CheckKeyForDropDownWindow(Key: Word; Shift: TShiftState)
      : Boolean; override;
  ...
  published
    property DateOnStart: TdxBarDateOnStart read FDateOnStart
      write SetDateOnStart default bdsToday;
    property MaxDate: TDateTime read FMaxDate write SetMaxDate
      stored IsMaxDateStored;
    property MinDate: TDateTime read FMinDate write SetMinDate
      stored IsMinDateStored;
    property ShowTodayButton: Boolean read FShowTodayButton
      write FShowTodayButton default True;
    property ShowClearButton: Boolean read FShowClearButton
      write FShowClearButton default True;
    property ShowDayText: Boolean read FShowDayText write SetShowDayText
      default True;
  ...
  published
    property AllowResizing: Boolean read FAllowResizing write FAllowResizing
      default True;
    property ChooseByDblClick: Boolean read FChooseByDblClick
      write FChooseByDblClick default True;
    property DropDownHeight: Integer read GetDropDownHeight
    ...
    property ShowEditor default False;
    property ShowImageInEdit: Boolean read FShowImageInEdit
      write SetShowImageInEdit default True;
    ...
    property OnExpanded: TTVExpandedEvent read GetOnExpanded
      write SetOnExpanded;
    property OnExpanding: TTVExpandingEvent read GetOnExpanding
      write SetOnExpanding;
    property OnCanSelectNode: TdxBarTreeViewComboCanSelectNodeEvent
      read FOnCanSelectNode write FOnCanSelectNode;
    property OnChanging: TTVChangingEvent read GetOnChanging
      write SetOnChanging;
    property OnCollapsed: TTVExpandedEvent read GetOnCollapsed
      write SetOnCollapsed;
    property OnCollapsing: TTVCollapsingEvent read GetOnCollapsing
      write SetOnCollapsing;
    property OnCompare: TTVCompareEvent read GetOnCompare write SetOnCompare;
    property OnGetImageIndex: TTVExpandedEvent read GetOnGetImageIndex
      write SetOnGetImageIndex;
    property OnGetSelectedIndex: TTVExpandedEvent read GetOnGetSelectedIndex
      write SetOnGetSelectedIndex;
    property OnTreeViewChange: TTVChangedEvent read GetOnTreeViewChange
      write SetOnTreeViewChange;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    procedure DoClick; override;
    property ImageIndexes[Index: Integer]: Integer read GetImageIndexes
  ...
  published
    property ValueType: TdxBarSpinEditValueType read FValueType
      write SetValueType default svtInteger; // must be loaded before all
    ...
    property PrefixPlace: TdxBarSpinEditPrefixPlace read FPrefixPlace
      write SetPrefixPlace default ppEnd;
    property Text stored False;
  ...
  protected
    ...
    property ActiveButtonIndex: Integer read FActiveButtonIndex
      write SetActiveButtonIndex;
  ...
  TdxBarInPlaceSubItemEvent = procedure(Sender: TdxBarInPlaceSubItem;
    Link: TdxBarItemLink) of object;
  ...
  protected
    ...
    procedure ChangeNextItemLinkBeginGroup(ALink: TdxBarItemLink;
      Value: Boolean);
  ...
  published
    ...
    property KeepBeginGroupWhileExpanded: Boolean
      read FKeepBeginGroupWhileExpanded write FKeepBeginGroupWhileExpanded;
  ...
  protected
    ...
    property DrawParams: TdxBarInPlaceSubItemControlDrawParams
      read GetDrawParams;
  public
  ...
  TdxBarInPlaceSubItemControlAccessibilityHelper = class
...
uses
  Math, ActiveX, Printers, dxBarStrs, dxCoreGraphics,
  cxClasses, cxGraphics, cxFormats, cxLookAndFeels, dxDPIAwareUtils;

const
  Colors: array [0 .. 15] of TColor = (clBlack, clMaroon, clGreen, clOlive,
...
type
  ...
  TPlaceForm = class(TdxCustomForm, IdxPlaceForm, IdxSkinSupport,
    IdxSkinSupport2)
  strict private
    procedure WMEraseBkgnd(var Message: TWMEraseBkgnd); message WM_ERASEBKGND;
  protected
...
begin
  ...
  begin
    R := ClientRect;
    BeforeDrawBackground(Self, FBarItemControl.Parent, Message.DC, R,
      APrevWindowOrg);
    try
      cxPaintCanvas.BeginPaint(Message.DC);
      try
        FBarItemControl.Painter.DrawItemBackground(FBarItemControl,
          cxPaintCanvas, R, Brush.Handle);
      finally
...
begin
  ...
  case Index of
    1:
      PrevValue := FLeftIndent;
    2:
      PrevValue := FRightIndent;
    3:
      PrevValue := FWidth;
    4:
      PrevValue := FHeight;
...
begin
  case Index of
    1:
      Result := Item.LeftIndent;
    2:
      Result := Item.RightIndent;
    3:
      Result := Item.Width;
    4:
      Result := Item.Height;
...
begin
  Painter.DrawStaticLikeControl(DrawParams, ARect,
    cxRect(LeftIndent, LeftIndent, RightIndent, RightIndent));
end;
...
function TdxBarStaticControl.GetAutoHeight(ADefaultButtonSize: Integer)
  : Integer;
begin
...
begin
  ...
  begin
    if (cpIcon in DrawParams.ViewStructure) then
      if DrawParams.PaintType <> ptMenu then
        ...
        Result := Painter.SubMenuControlGetItemTextIndent(DrawParams) +
          GetCaptionWidth + 4
    else
  ...
  Inc(Result, LeftIndent + RightIndent + BorderOffsets.Left +
    BorderOffsets.Right);
end;
...
begin
  if Height = 0 then
    Result := GetAutoHeight(GetRotationDependentHeight
      (DrawParams.DefaultButtonSize))
  else
    Result := Height;
end;
...
begin
  if Width = 0 then
    Result := GetAutoWidth(GetRotationDependentWidth
      (DrawParams.DefaultButtonSize))
  else
    Result := Width;
end;
...
begin
  ...
  begin
    ...
    for Result := Low(Colors) + Byte(FShowAutoColor) to High(Colors) +
      Byte(FShowAutoColor) do
      if ColorToRGB(Colors[Result - Byte(FShowAutoColor)]) = AColor then
        Exit;
    Result := -1;
  end;
end;
...
procedure TdxBarColorCombo.DrawItem(APainter: TdxBarPainter; AIndex: Integer;
  ...
  if Assigned(OnDrawItem) then
    ...
    begin
      ...
      if odFocused in AState then
        Windows.DrawFocusRect(Handle, ARect); // for hiding focus rect
    end;
end;
...
procedure TdxBarColorCombo.MeasureItemWidth(AIndex: Integer;
  var AWidth: Integer);
begin
  inherited;
  Inc(AWidth, 30);
...
function TdxBarColorComboControl.GetDrawParams
  : TdxBarColorComboControlDrawParams;
begin
  ...
  begin
    ...
    begin
      ...
      AAdjacentZoneRect := cxRectInflate(FParts[clcpCustomColorButton],
        AIndentsRect.Left, AIndentsRect.Top, AIndentsRect.Right,
        AIndentsRect.Bottom);

      // TODO: Regions
      Canvas.SaveClipRegion;
      Canvas.SetClipRegion(TcxRegion.Create(AAdjacentZoneRect), roSet);
      Painter.ColorComboDrawCustomButtonAdjacentZone(DrawParams,
        AAdjacentZoneRect);
      Canvas.RestoreClipRegion;

      Painter.ColorComboDrawCustomButton(DrawParams,
        FParts[clcpCustomColorButton]);
    end;
  end;
...
function TdxBarColorComboControl.GetDrawParamsClass
...
begin
  with Message do
    if (Msg = WM_LBUTTONDOWN) and (FHotPartIndex = clcpCustomColorButton) or
      (Msg = WM_KEYDOWN) and (wParam = VK_RETURN) and
...
procedure TdxBarFontNameCombo.DrawItem(APainter: TdxBarPainter; AIndex: Integer;
  ARect: TRect; AState: TOwnerDrawState);
var
  W, H: Integer;
  ...
  if Assigned(OnDrawItem) or (AIndex = -1) then
    ...
    begin
      ...
      TransparentDraw(Handle, Brush.Handle, ARect, R, CurrentBitmap, nil, -1,
        clNone, True, False, False, False, False, False, False,
        False { Faded } , True);
...
begin
  TStrings(Data).AddObject(LogFont.lfFaceName,
    Pointer(FontType and TRUETYPE_FONTTYPE <> 0));
  Result := 1;
end;
...
procedure TdxBarFontNameCombo.LoadFontNames;
  ...
  begin
    ...
    EnumFontFamiliesEx(DC, ALogFont, @EnumFontsProc, LPARAM(Items), 0);
  end;
...
procedure TdxBarFontNameCombo.MeasureItemWidth(AIndex: Integer;
  var AWidth: Integer);
begin
  inherited;
...
procedure AMonthListBoxTimerProc(Wnd: HWND; Msg: UINT; TimerID: UINT_PTR;
...
begin
  ...
  begin
    ...
    begin
      ...
      FTimer := dxSetTimer(Handle, FTimerID, Times[Abs(FTimerID - 5)],
...
begin
  ...
  begin
    ...
    begin
      ...
      cxDrawText(Handle, S, R, DT_SINGLELINE or DT_NOCLIP or DT_CENTER or
        DT_VCENTER);
    end;
...
begin
  inherited;
  with Message do
    if (LPARAM <> 0) and (HWND(LPARAM) <> Handle) then
      CancelAll;
end;
...
begin
  ...
  begin
    ...
    SelFinish := ADate;
    if FSelFinish = PrevSelFinish then
      Repaint;
  end;
end;
...
begin
  ...
  if (ssLeft in Shift) and (GetCapture = Handle) then
    if ADate <> NullDate then
      SelFinish := ADate
    else if not PtInRect(ClientRect, Point(X, Y)) then
...
begin
  SetRect(Result, ClientWidth - FColWidth, 1, ClientWidth - 1 - Byte(not Flat),
    FHeaderHeight - 1);
end;
...
begin
  Result := Bounds((ClientWidth - FTodayButtonWidth - Byte(ShowClearButton) *
    FClearButtonWidth) div (3 - Byte(not ShowClearButton)),
    ClientHeight - FButtonsRegionHeight + FButtonsOffset, FTodayButtonWidth,
    FButtonsHeight);
end;

function TdxBarDateNavigator.GetClearButtonRect: TRect;
begin
  Result := Bounds(ClientWidth - FClearButtonWidth -
    (ClientWidth - Byte(ShowTodayButton) * FTodayButtonWidth -
    FClearButtonWidth) div (3 - Byte(not ShowTodayButton)),
    ClientHeight - FButtonsRegionHeight + FButtonsOffset, FClearButtonWidth,
    FButtonsHeight);
end;
...
begin
  ...
  FTodayButtonWidth := Canvas.TextWidth(sdxBarDatePopupToday) + FColWidth;
  FClearButtonWidth := Canvas.TextWidth(sdxBarDatePopupClear) + FColWidth;
...
procedure ADateNavigatorTimerProc(Wnd: HWND; Msg: UINT; TimerID: UINT_PTR;
  SysTime: DWORD); stdcall;
var
...
begin
  ...
  case TimerID of
    1:
      if PtInRect(AControl.GetLeftArrowRect, P) then
        AControl.StepToPast;
    2:
      if PtInRect(AControl.GetRightArrowRect, P) then
        AControl.StepToFuture;
  end;
end;
...
begin
  if (Button = mbLeft) and IsPopup then
    ...
    else if ShowClearButton and PtInRect(GetClearButtonRect, Point(X, Y)) then
    begin
    ...
    else if ShowButtonsArea and (Y >= ClientHeight - FButtonsRegionHeight) then
      Exit;
  inherited MouseDown(Button, Shift, X, Y);
  if Button = mbLeft then
  begin
    ...
    begin // shift by month to past
      StepToPast;
      if FTimer = 0 then
        FTimer := dxSetTimer(Handle, 1, ADateNavigatorTime,
    ...
    begin // shift by month to future
      StepToFuture;
      if FTimer = 0 then
        FTimer := dxSetTimer(Handle, 2, ADateNavigatorTime,
          ADateNavigatorTimerProc);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if SelStart < FirstDate then
        CheckSelection(FirstDate)
      else if SelStart > LastDate then
        CheckSelection(LastDate)
      else
        Repaint;
    end;
  ...
  begin
    P := Point(X, Y);
    if PtInRect(ClientRect, P) and ((PosToDateTime(P) <> NullDate) or
      ShowTodayButton and PtInRect(GetTodayButtonRect, P) or ShowClearButton and
      PtInRect(GetClearButtonRect, P)) then
      DoDateTimeChanged
    else
      DoInternalSelectPeriod(FDragDate);
  end;
...
procedure TdxBarDateNavigator.Paint;
  ...
  begin
    ...
    P[1] := Point(X + ASign * (FSideWidth - 1),
      (R.Top + R.Bottom - AArrowHeight) div 2);
    P[2] := Point(P[1].X, P[1].Y + AArrowHeight - 1);
    P[3] := Point(P[1].X - ASign * AArrowHeight div 2,
      P[1].Y + AArrowHeight div 2);
  ...
  begin
    ...
    DrawArrow(R, False);
    S := dxFormatSettings.LongMonthNames[AMonth] + ' ' + IntToStr(AYear);
    cxDrawText(cxPaintCanvas, S, R, DT_VCENTER or DT_CENTER or DT_SINGLELINE,
      clBtnText);
  end;
  ...
  begin
    ...
    begin
      ...
      cxDrawText(cxPaintCanvas, S, ALettersRect, DT_VCENTER or DT_CENTER or
        DT_SINGLELINE, clBtnText);
    end;
  end;
    ...
    for I := 0 to 6 do
      ...
      begin
        ...
        if ASelected and UltraFlat then
          cxPaintCanvas.Brush.Color := TBarManagerAccess(FCombo.BarManager)
        ...
        cxDrawText(cxPaintCanvas, S, ATextRect, DT_VCENTER or DT_CENTER or
          DT_SINGLELINE);
      end;
  end;
...
begin
  ...
  try
    ...
    begin
      cxPaintCanvas.FrameRect(cxRectInflate(ClientRect, -FSideWidth,
        -FButtonsRegionHeight - 1), clBtnShadow, 1, [bBottom]);
...
begin
  ...
  OffsetRect(WR, -WR.Left, -WR.Top);
  CR := cxGetClientRect(Handle);
  Result := WR.Bottom - CR.Bottom + FHeaderHeight + Byte(not Flat) +
    FDaysOfWeekHeight + 6 * FRowHeight + 1;
  if IsPopup and ShowButtonsArea then
...
begin
  case TWinControl(Sender).Tag of
    1:
      FDateNavigator.SelStart := SysUtils.Date;
    2:
      FDateNavigator.SelStart := NullDate;
  end;
...
function TdxBarDateCombo.CheckKeyForDropDownWindow(Key: Word;
  Shift: TShiftState): Boolean;
begin
  Result := (Key = VK_RETURN) or inherited CheckKeyForDropDownWindow
    (Key, Shift);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ButtonCancel.SetBounds(ButtonOk.Left, ButtonOk.Top + ButtonOk.Height
        + D, W, H);
      if ButtonToday <> nil then
        ButtonToday.SetBounds(ButtonOk.Left, ClientHeight - D - H - D -
          H, W, H);
...
var
  ...
  var
    ANode: TTreeNode;
  begin
    if AnsiCompareText(AText, ARootNode.Text) = 0 then
      Result := ARootNode
    else
    begin
      Result := nil;
...
begin
  inherited;
  if IsPopup then
    FCombo.UpdateEx; // DoImageListChanged;
end;

procedure TdxBarTreeView.TVMSetItem(var Message: TMessage);
begin
  inherited;
  if not(csDestroying in ComponentState) and (FCombo.SelectedNode <> nil) and
...
begin
  if IsPopup then
    Painter.SysPanelCalcSize(Message.CalcSize_Params^.rgrc[0], FGripRect,
      FCloseButtonRect, FCorner, FCombo, FCombo.AllowResizing,
      TBarManagerAccess(FCombo.BarManager).ScaleFactor);
  inherited;
end;
...
begin
  inherited;
  if IsPopup then
    Painter.SysPanelDraw(Handle, FCombo.AllowResizing, FMouseAboveCloseButton,
      FCloseButtonIsTracking, FCloseButtonRect, FGripRect, FCorner,
      TBarManagerAccess(FCombo.BarManager).ScaleFactor);
end;
...
begin
  ...
  begin
    ...
    if IsPopup then
    begin
      ExStyle := ExStyle and not WS_EX_CLIENTEDGE or WS_EX_TOOLWINDOW or
        WS_EX_TOPMOST;
      WindowClass.Style := WindowClass.Style or CS_SAVEBITS;
    end;
  end;
...
begin
  ...
  begin
    P := GetMouseCursorPos;
    Windows.ScreenToClient(Handle, P);
    if GetHitTestInfoAt(P.X, P.Y) * [htOnItem, htOnIcon, htOnLabel,
      htOnStateIcon] <> [] then
      SaveAndHide;
  end;
end;
...
procedure TdxBarTreeView.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
  if not FCombo.ChooseByDblClick and (FMouseDownHitTestInfo <> [htOnButton]) and
    (GetHitTestInfoAt(X, Y) * [htOnItem, htOnIcon, htOnLabel, htOnStateIcon]
    <> []) then
    SaveAndHide;
end;
...
begin
  if IsPopup and (Combo.CurItemLink <> nil) and
    (Combo.CurItemLink.Control <> nil) then
  begin
    AEditRect := Combo.CurItemLink.ItemRect;
    AEditRect.TopLeft := Combo.CurItemLink.Control.Parent.ClientToScreen
      (AEditRect.TopLeft);
    AEditRect.BottomRight := Combo.CurItemLink.Control.Parent.ClientToScreen
      (AEditRect.BottomRight);
    ADropDownRect := cxRectBounds(ADropDownPosition.X, ADropDownPosition.Y,
      Width, Height);
...
begin
  ...
  begin
    ...
    if (Images <> nil) or (StateImages <> nil) then
      UpdateEx;
  end;
end;
...
begin
  ...
  FButtonCancel.SetBounds(FButtonOk.Left, FButtonOk.Top + FButtonOk.Height
    + D, W, H);
end;
...
function TdxBarTreeViewCombo.CheckKeyForDropDownWindow(Key: Word;
  Shift: TShiftState): Boolean;
begin
  Result := (Key = VK_RETURN) or inherited CheckKeyForDropDownWindow
    (Key, Shift);
...
procedure TdxBarTreeViewCombo.DrawInterior(ABarEditControl: TdxBarEditControl;
  ACanvas: TCanvas; R: TRect; ItemLink: TdxBarItemLink);
var
  DC: HDC;
  ANode: TTreeNode;
  AIndex: Integer;
...
begin
  ...
  begin
    ...
    begin
      Inc(Left);
      if (StateImages <> nil) and (ANode <> nil) and (0 <= ANode.StateIndex) and
        (ANode.StateIndex < StateImages.Count) then
        with StateImages do
        begin
      ...
      if Images <> nil then
        ...
        begin
          if ANode = nil then
            AIndex := -1
          else if (0 <= ANode.SelectedIndex) and (ANode.SelectedIndex < Count)
          then
            AIndex := ANode.SelectedIndex
          else if (0 <= ANode.ImageIndex) and (ANode.ImageIndex < Count) then
...
procedure TdxBarTreeViewCombo.Notification(AComponent: TComponent;
...
procedure TdxBarImageCombo.DialogListBoxDrawItem(Control: TWinControl;
...
procedure TdxBarImageCombo.DialogListBoxMeasureItem(Control: TWinControl;
...
begin
  inherited;
  Filer.DefineProperty('ImageIndexes', ReadImageIndexes,
    WriteImageIndexes, True);
end;

procedure TdxBarImageCombo.DrawItem(APainter: TdxBarPainter; AIndex: Integer;
  ARect: TRect; AState: TOwnerDrawState);
var
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if FocusedItemLink = nil then
            R.Right := R.Left;
        end
        else if FocusedItemLink = nil then
          R.Right := R.Left;
...
procedure TdxBarImageCombo.MeasureItemWidth(AIndex: Integer;
  var AWidth: Integer);
begin
  inherited;
  if Images <> nil then
...
procedure TdxBarImageCombo.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited;
...
begin
  ...
  begin
    ...
    if ShowModal = mrOk then
      ItemIndex := FDialogListBox.ItemIndex;
    Free;
  end;
end;
...
function TdxBarToolbarsListItem.InternalCanMergeWith(AItem: TdxBarItem)
  : Boolean;
begin
...
begin
  inherited;
  if (Key in [VK_UP, VK_DOWN]) and (FocusedItemLink <> nil) and not ReadOnly
  then
  begin
...
begin
  ...
  else if FActiveButtonIndex <> Value then
  begin
...
begin
  ...
  begin
    ...
    DrawParams.ArrowSize := Painter.GetSpinEditArrowSize
      (cxRectHeight(ItemBounds));
    DrawParams.IsPressed := Pressed;
  end;
end;
...
procedure TdxBarSpinEditControl.DrawTextField;
  ...
  begin
    Painter.SpinEditControlDrawButton(DrawParams, FParts[AButtonIndex],
      AButtonIndex);
  end;
...
begin
  ...
  begin
    ...
    AAdjacentZoneRect := cxRectUnion(FParts[secButtonUp],
      FParts[secButtonDown]);
    ...
    Painter.SpinEditControlDrawButtonsAdjacentZone(DrawParams,
      AAdjacentZoneRect);
...
function TdxBarSpinEditControl.GetDrawParamsClass
  : TdxBarItemControlDrawParamsClass;
begin
  ...
  with Message do
    case Msg of
          ...
          begin
            ...
            try
              ...
              if (ALinkSelf.Ref <> nil) and (GetCapture = Handle) then
                FTimerID := dxSetTimer(Handle, 1, GetDoubleClickTime -
                  100, nil);
            finally
              cxRemoveObjectLink(ALinkSelf);
            end;
      ...
      WM_MOUSEMOVE:
        ...
        begin
          CheckHotTrack
            (ClientToParent(SmallPointToPoint(TWMMouse(Message).Pos)));
...
begin
  for I := 0 to LinkCount - 1 do
  begin
    Result := (Links[I].Control <> nil) and TdxBarControlContainerControl
      (Links[I].Control).InPlaceControl;
    if Result then
      Exit;
  end;
  Result := False;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not(csDestroying in FControl.ComponentState) and not BarManager.Designing
      then
        FControl.Parent := nil;
    end;
    FControl := Value;
...
procedure TdxBarControlContainerItem.ControlWndProc(var Message: TMessage);
  ...
  begin
    Result := (Control.Width <> FPrevControlSize.X) or
      (Control.Height <> FPrevControlSize.Y);
  end;
...
function TdxBarControlContainerItem.IsControlAssigned
...
begin
  ...
  begin
    ...
    if (ABarItem is TdxBarControlContainerItem) and (ABarItem <> Self) and
      (TdxBarControlContainerItem(ABarItem).Control = AControl) then
      Exit;
  end;
  Result := False;
end;
...
function TdxBarControlContainerItem.HasAccel(AItemLink: TdxBarItemLink)
  : Boolean;
begin
...
begin
  ...
  else if (Place <> nil) and FPlacedControl then
...
begin
  ...
  begin
    ...
    Painter.DrawGlyph(Self, DC, R, cxEmptyRect, PaintType, False, DrawSelected,
      False, APressed, False, False, False, False);
    ...
    Painter.DrawItemText(Self, DC, Caption, R, DT_LEFT, Enabled, False, False,
      False, False);
  end
  else if (Control = nil) or BarManager.Designing then
  begin
    FrameRect(DC, R, GetSysColorBrush(COLOR_BTNSHADOW));
    InflateRect(R, -1, -1);
    ...
    Painter.DrawItemText(Self, DC, S, R, DT_CENTER, True, False,
...
begin
  ...
  begin
    if Parent.Kind = bkSubMenu then
      Result := 2 * Parent.TextSize + 3 + GetTextWidth(GetTextOf(Item.Name)) + 3
        + Painter.ContainerControlSubMenuOffset
    else if Parent.Kind = bkBarQuickControl then
    begin
...
function TdxBarControlContainerControl.GetPossibleViewLevels
  : TdxBarItemViewLevels;
begin
...
begin
  ...
  else if FPosition + FStep < FMin then
...
function TdxBarProgressControl.CalculateBandDrawPosition(const ABandRect
  : TRect): Integer;
var
  ALength: Integer;
begin
...
begin
  ...
  begin
    ...
    Result := AWidth - GetAutoWidth
      (GetRotationDependentWidth(DrawParams.DefaultButtonSize)) -
      GetProgressBarIndent;
    if Result < 0 then
      Result := 0;
  end;
end;
...
begin
  ...
  try
    ...
    if AIndent <> 0 then
      with ARect do
        Painter.DrawBackground(Self, Canvas.Handle,
          Rect(Left, Top, Left + AIndent, Bottom), Parent.BkBrush, False);
...
function TdxBarProgressControl.GetDrawParamsClass
  : TdxBarItemControlDrawParamsClass;
begin
...
begin
  ...
  ABarHeight := Painter.ProgressControlBarHeight(Self) + BorderOffsets.Top +
    BorderOffsets.Bottom + 2;
...
begin
  ...
  if Width = 0 then
    Inc(Result, (Byte(not(cpText in DrawParams.ViewStructure)) + 1) *
      ProgressBarIndent + ProgressBarDefaultWidth);
...
begin
  Result := cxGetStringAdjustedToWidth(0, 0, inherited GetDisplayText(AText),
    300, mstPathEllipsis);
end;
...
begin
  if FExpanded and not FExpandedChanging or not FExpanded and FExpandedChanging
  then
    inherited;
end;
...
procedure TdxBarInPlaceSubItem.ChangeNextItemLinkBeginGroup
  (ALink: TdxBarItemLink; Value: Boolean);
var
...
begin
  if Assigned(FOnBeforeCollapse) then
    FOnBeforeCollapse(Self, ALink);
  ChangeNextItemLinkBeginGroup(ALink, False);
end;

{ TdxBarInPlaceSubItemControl }

function TdxBarInPlaceSubItemControl.GetDrawParams
  : TdxBarInPlaceSubItemControlDrawParams;
begin
...
procedure TdxBarInPlaceSubItemControl.ControlClick(AByMouse: Boolean;
  AKey: Char = #0);
var
    ...
    if ASelectedItemLinkIndex <> -1 then
      with TCustomdxBarControlAccess(AParent) do
        if (ASelectedItemLinkIndex < ItemLinks.VisibleItemCount) and
          (ItemLinks[ASelectedItemLinkIndex]
          .OriginalItemLink = AOriginalItemLink) then
...
procedure TdxBarInPlaceSubItemControl.DoPaint(ARect: TRect;
  PaintType: TdxBarPaintType);
begin
...
function TdxBarInPlaceSubItemControl.GetAccessibilityHelperClass
...
begin
  if Parent is TdxBarSubMenuControl then
    Result := Painter.InPlaceSubItemGetTextIndent + GetCaptionWidth +
      DrawParams.ArrowSize.cx
  else
    Result := inherited GetDefaultWidth;
end;

function TdxBarInPlaceSubItemControl.GetDrawParamsClass
  : TdxBarItemControlDrawParamsClass;
begin
...
procedure TdxBarInPlaceSubItemControl.KeyDown(var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_LEFT, VK_RIGHT:
...
begin
  if Parent is TdxBarSubMenuControl then
    Result := (Key = VK_LEFT) and Item.Expanded or (Key = VK_RIGHT) and
      not Item.Expanded
  else
...
function TdxBarInPlaceSubItemControlAccessibilityHelper.
  ShowDropDownWindow: Boolean;
begin
  Result := inherited ShowDropDownWindow or
    (ItemControl.Parent.Kind = bkSubMenu);
end;
...
dxBarRegisterItem(TdxBarControlContainerItem,
  TdxBarControlContainerControl, True);
