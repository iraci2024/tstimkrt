...
{$ENDIF}
  ...
  cxGraphics,
  StdCtrls, ComCtrls, ExtCtrls, Menus, Buttons, ActnList, ToolWin, ImgList,
  Contnrs,
  dxCore, cxControls, cxClasses, dxMessages, dxBar, cxLookAndFeelPainters,
  cxLookAndFeels,
...
type
  ...
  protected
    ...
    class procedure InternalDrawCaption(ACanvas: TCanvas; var ATextRect: TRect;
      const AText: string; AItem: TdxCustomBarEdit;
    ...
    class procedure DrawButtonOrSubItemArrowBackground(ACanvas: TCanvas;
      R: TRect; AItem: TdxBarItem; Selected: Boolean); virtual;
    ...
    class procedure DrawSubItemArrow(ACanvas: TCanvas; X, Y: Integer;
      ASelected: Boolean); virtual;
  public
    class procedure DrawButtonOrSubItem(ACanvas: TCanvas; ARect: TRect;
      AItem: TdxBarItem; ACaption: string; Selected: Boolean); virtual;
    class procedure DrawCheckBox(ACanvas: TCanvas; R: TRect;
      AChecked, AEnabled: Boolean); virtual;
    ...
    class procedure DrawFocusedRect(ACanvas: TCanvas; ARect: TRect;
      AItem: TdxBarItem); virtual;

    class function BrushColors(Selected: Boolean; AItem: TdxBarItem)
      : TColor; virtual;
    class function FontColors(Selected: Boolean): TColor; virtual;
    class function GetButtonColor(AItem: TdxBarItem; ASelected: Boolean)
      : Integer; virtual;
    class function SideStripColor(Selected: Boolean; AItem: TdxBarItem)
      : TColor; virtual;
  ...
  TdxBarCustomizationFormOffice11Painter = class
    (TdxBarCustomizationFormFlatPainter)
  protected
    class procedure DrawEditEdge(ACanvas: TCanvas; var AItemRect: TRect;
      AItem: TdxCustomBarEdit; ASelected: Boolean); override;
  ...
  strict private
    ...
    procedure SelectionListNotify(Sender: TObject; AComponent: TComponent;
      AAction: TListNotification);
    procedure SelectionListChanged(Sender: TObject; AComponent: TComponent;
  ...
  public
    constructor Create(AOwner: TdxBarCustomCustomizationForm;
      AListBox: TListBox); virtual;
    destructor Destroy; override;
    ...
    property PainterClass: TdxBarCustomizationFormPainterClass
  ...
  TdxBarCustomizationFormBarListHelper = class
    (TdxBarCustomizationFormListBoxHelper)
  strict private
  ...
  TdxBarCustomizationFormItemsListHelper = class
    (TdxBarCustomizationFormListBoxHelper)
  private
    procedure DoListClick(Sender: TObject);
  ...
  TdxBarCustomizationFormAllCommandsListHelper = class
    (TdxBarCustomizationFormListBoxHelper)
  strict private
    ...
    procedure LAllCommandsKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure LAllDrawItem(Control: TWinControl; Index: Integer; Rect: TRect;
      State: TOwnerDrawState);
  protected
  ...
  TdxBarCustomizationFormGroupsListHelper = class
    (TdxBarCustomizationFormListBoxHelper)
  strict private
  ...
  TdxBarCustomizationGroupItemsListBoxHelper = class
    (TdxBarCustomizationFormListBoxHelper)
  strict private
  ...
  private
    ...
    procedure WMDeferredCallSynchronizationListBox(var Message: TMessage);
      message DXM_BAR_LB_DEFERREDCALLSYNCHRONIZATION;
  protected
    ...
    function GetDisableParent(ABarManager: TdxBarManager;
      out AParent: TWinControl): Boolean;
    ...
    procedure SynchronizeListBox(AListBox: TListBox;
      AChangedObject: TObject = nil;
    ...
    function PrepareMenuAnimationsComboBox(AComboBoxFont: TFont;
      AComboBoxStrings: TStrings): Integer;
    ...
    procedure SynchronizeDesigner(ANewSelection: IdxBarSelectableItem);
      overload;
    ...
    property BarListBoxHelper: TdxBarCustomizationFormListBoxHelper
      read FBarListBoxHelper;
    property CategoryListBoxHelper: TdxBarCustomizationFormListBoxHelper
      read FCategoryListBoxHelper;
    ...
    property GroupItemsListBoxHelper: TdxBarCustomizationGroupItemsListBoxHelper
      read FGroupItemsListBoxHelper;
    property GroupsListBox: TListBox read GetGroupsListBox;
    property GroupsListBoxHelper: TdxBarCustomizationFormGroupsListHelper
      read FGroupsListBoxHelper;
    ...
    property ItemsListBoxHelper: TdxBarCustomizationFormListBoxHelper
      read FItemsListBoxHelper;
...
procedure PrepareLookAndFeel(ALookAndFeel: TcxLookAndFeel;
  ABarManager: TdxBarManager);
...
uses
  Types, dxBarNameEd, dxBarPopupMenuEd, dxBarItemEd, dxBarStrs,
  dxBarAddGroupItemsEd,
  TypInfo, dxUxTheme, dxThemeManager, dxThemeConsts, dxOffice11, cxContainer,
  Math, dxBarCustForm, dxDPIAwareUtils, dxThreading,
  dxCoreGraphics;
...
const
  ...
  dxBarDefaultTextFlags = DT_NOCLIP or DT_NOPREFIX or DT_SINGLELINE or
    ...
    for I := 0 to dxBarManagerList.Count - 1 do
      if dxBarManagerList[I] <> ABarManager then
        ...
        begin
          ...
          if ABar.Visible and (ABar.Control <> nil) and ABar.Control.HandleAllocated
          then
          begin
...
const
  LookAndFeelKindMap: array [TdxBarManagerStyle] of TcxLookAndFeelKind =
    (lfStandard, lfStandard, lfUltraFlat, cxDefaultLookAndFeelKind, lfOffice11,
...
constructor TdxBarCustomizationFormAllCommandsListHelper.Create
...
begin
  ...
  AListBox.ItemHeight := TdxTextMeasurer.TextLineHeight(AListBox.Canvas.Handle)
    + ScaleFactor.Apply(2);
end;

procedure TdxBarCustomizationFormAllCommandsListHelper.WndProc
...
begin
  ...
  for I := 0 to ListBox.Items.Count - 1 do
    ...
    begin
      FAllCommandsNameWidth := Max(FAllCommandsNameWidth,
        ACanvas.TextWidth(Name));
      FAllCommandsCaptionWidth := Max(FAllCommandsCaptionWidth,
        ACanvas.TextWidth(Caption));
      FAllCommandsLinksCountWidth := Max(FAllCommandsLinksCountWidth,
        ACanvas.TextWidth(IntToStr(LinkCount) + ' link(s)'));
      FAllCommandsShortCutWidth := Max(FAllCommandsShortCutWidth,
        ACanvas.TextWidth(ShortCutToText(ShortCut)));
    end;
  ListBox.Invalidate;
...
procedure TdxBarCustomizationFormAllCommandsListHelper.SynchronizeCommandList
  (AItem: TdxBarItem);
var
  I: Integer;
begin
  ...
  begin
    ...
    try
      for I := 0 to BarManager.ItemCount - 1 do
        ...
        begin
          if not Owner.GetShowCommandsWithShortCut or
            Assigned(GetPropInfo(BarManager.Items[I].ClassInfo, 'ShortCut'))
          then
...
procedure TdxBarCustomizationFormAllCommandsListHelper.LAllCommandsClick
  (Sender: TObject);
begin
...
procedure TdxBarCustomizationFormAllCommandsListHelper.LAllDrawItem
  ...
  procedure DrawSeparator(ACanvas: TCanvas; AColor: TColor;
    APos, ATop, ABottom: Integer);
  begin
  ...
  procedure DrawColumn(ACanvas: TCanvas; const AText: string;
    ATextWidth: Integer; var AColumnPos: Integer;
    ANeedDrawSeparator, ALastRow: Boolean);
  begin
    ...
    begin
      DrawSeparator(ACanvas, ACanvas.Font.Color, AColumnPos, Rect.Top,
        Rect.Bottom);
      if ALastRow then
        DrawSeparator(ACanvas, clWindowText, AColumnPos, Rect.Bottom,
...
begin
  ...
  begin
    DrawColumn(TListBox(Control).Canvas, Name, FAllCommandsNameWidth, W, True,
      ALastRow);
    DrawColumn(TListBox(Control).Canvas, Caption, FAllCommandsCaptionWidth, W,
      True, ALastRow);
    DrawColumn(TListBox(Control).Canvas, IntToStr(LinkCount) + ' link(s)',
      FAllCommandsLinksCountWidth, W, True, ALastRow);
    DrawColumn(TListBox(Control).Canvas, ShortCutToText(ShortCut),
      FAllCommandsShortCutWidth, W, False, ALastRow);
  end;
end;

procedure TdxBarCustomizationFormAllCommandsListHelper.LAllCommandsKeyDown
...
function TdxBarCustomizationFormAllCommandsListHelper.GetAllCommandList
  (Index: Integer): TdxBarItem;
begin
  Result := TdxBarItem(Owner.GetObjectFromListBox(ListBox, Index));
end;

function TdxBarCustomizationFormAllCommandsListHelper.
  GetAllCommandsIndent: Integer;
begin
...
constructor TdxBarCustomizationFormGroupsListHelper.Create
  (AOwner: TdxBarCustomCustomizationForm; AListBox: TListBox);
begin
  inherited Create(AOwner, AListBox);
...
procedure TdxBarCustomizationFormGroupsListHelper.WndProc
  (var Message: TMessage);
  ...
  begin
    ...
    Owner.UpdateCommonEvents(ListBox, Owner.actAddGroup, Owner.actDeleteGroup,
      Owner.actMoveGroupUp, Owner.actMoveGroupDown);
  end;
...
procedure TdxBarCustomizationFormGroupsListHelper.lbGroupsClick
  (Sender: TObject);
begin
  Owner.SynchronizeDesigner(ListBox);
end;
...
begin
  ...
  begin
    ...
    cxExtTextOut(Canvas.Handle, Items[Index],
      Point(Rect.Left + 2, Rect.Top + 1), Rect, ETO_OPAQUE);
  end;
end;

procedure TdxBarCustomizationFormGroupsListHelper.lbGroupsKeyDown
  (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_INSERT:
...
procedure TdxBarCustomizationFormGroupsListHelper.lbGroupsMeasureItem
  (Control: TWinControl; Index: Integer; var Height: Integer);
begin
  Height := TdxTextMeasurer.TextLineHeight(TListBox(Control).Font) + 2;
end;

{ TdxBarCustomizationGroupItemsListBoxHelper }

constructor TdxBarCustomizationGroupItemsListBoxHelper.Create
  (AOwner: TdxBarCustomCustomizationForm; AListBox: TListBox);
begin
  inherited Create(AOwner, AListBox);
...
begin
  Owner.actAddGroupItem.Enabled :=
    Owner.GroupsListBoxHelper.SelectedGroup <> nil;
  Owner.UpdateCommonEvents(ListBox, nil, Owner.actDeleteGroupItem,
    Owner.actMoveGroupItemUp, Owner.actMoveGroupItemDown);
end;

procedure TdxBarCustomizationGroupItemsListBoxHelper.WndProc
  (var Message: TMessage);
...
procedure TdxBarCustomizationGroupItemsListBoxHelper.lbGroupItemsClick
  (Sender: TObject);
begin
...
begin
  ...
  begin
    if Items.Objects[Index] is TdxBarGroup then
      Canvas.Font.Style := Canvas.Font.Style + [fsBold];
    cxExtTextOut(Canvas.Handle, Items[Index],
      Point(Rect.Left + Self.ScaleFactor.Apply(2),
      Rect.Top + Self.ScaleFactor.Apply(1)), Rect, ETO_OPAQUE);
  end;
end;

procedure TdxBarCustomizationGroupItemsListBoxHelper.lbGroupItemsKeyDown
  (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_INSERT:
...
procedure TdxBarCustomizationGroupItemsListBoxHelper.lbGroupItemsMeasureItem
  (Control: TWinControl; Index: Integer; var Height: Integer);
begin
  Height := TdxTextMeasurer.TextLineHeight(TListBox(Control).Font) +
    ScaleFactor.Apply(2);
end;
...
class function TdxBarCustomizationFormPainter.SideStripColor(Selected: Boolean;
  AItem: TdxBarItem): TColor;
begin
...
class procedure TdxBarCustomizationFormPainter.CalcButtonOrSubItemRects
  (AItem: TdxBarItem; const R: TRect; out ASideStripRect, ATextRect,
  AArrowRect: TRect);
begin
  ATextRect := R;
  AArrowRect := R;
  ...
  if (AItem is TdxBarCustomButton) and
    (TdxBarCustomButton(AItem).ButtonStyle = bsDropDown) then
  ...
  begin
    ...
    ASideStripRect := TdxRightToLeftLayoutConverter.ConvertRect
      (ASideStripRect, R);
  end;
end;

class procedure TdxBarCustomizationFormPainter.DrawButtonOrSubItem
...
begin
  ...
  DrawButtonOrSubItemBackground(ACanvas, AItem, ARect, ASideStripRect,
    AArrowRect, Selected);
  ...
  if GetIsNeedDrawSubItemArrow(AItem) then
    cxRightToLeftDependentDraw(ACanvas.Handle, ARect,
      UseRightToLeftAlignment(AItem), procedure
      begin
        DrawSubItemArrow(ACanvas, AArrowRect.Right - (ScaleFactor.Apply(4) +
          ScaleFactor.Apply(5)), AArrowRect.Top + (GetBarButtonHeight -
          ScaleFactor.Apply(7)) div 2, False);
      end);
end;

class procedure TdxBarCustomizationFormPainter.
  DrawButtonOrSubItemArrowBackground(ACanvas: TCanvas; R: TRect;
AItem: TdxBarItem; Selected: Boolean);
begin
  if Selected then
  begin
...
class procedure TdxBarCustomizationFormPainter.DrawButtonOrSubItemBackground
  (ACanvas: TCanvas; AItem: TdxBarItem; const ABounds, ASideStripRect,
  AArrowRect: TRect; ASelected: Boolean);
begin
  FillRectByColor(ACanvas.Handle, ABounds, BrushColors(ASelected, AItem));
  FillRectByColor(ACanvas.Handle, ASideStripRect,
    SideStripColor(ASelected and not GetIsGlyphItem(AItem), AItem));
  if (AItem is TdxBarCustomButton) and
    (TdxBarCustomButton(AItem).ButtonStyle = bsDropDown) then
...
begin
  ...
  TransparentDraw(ACanvas.Handle, 0, R, R, AItem.Glyph, AItem.GetCurrentImages,
    AItem.ImageIndex, AItem.BarManager.ImageListBkColor, True, False,
    False { Flat } , ASelected, False, False, False { Shadow } ,
    False { Faded } ,
...
class procedure TdxBarCustomizationFormPainter.DrawButtonOrSubItemText
  (ACanvas: TCanvas; R: TRect; AItem: TdxBarItem; const ACaption: string;
Selected: Boolean);
var
  AText: string;
  AFlags: Cardinal;
begin
  ...
  if (AItem is TCustomdxBarContainerItem) and TCustomdxBarContainerItemAccess
    (AItem).HideWhenRun then
    AText := '(' + ACaption + ')'
  else
    AText := ACaption;
...
begin
  InflateRect(R, -1, -1);
  DrawFrameControl(ACanvas.Handle, R, DFC_BUTTON, DFCS_BUTTONCHECK or
    CheckState[AChecked] or EnabledState[AEnabled]);
end;
...
AItemRect: TRect; AItem: TdxCustomBarEdit;
ASelected, ADrawArrowButton: Boolean);
var
...
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment(AItem) then
    begin
      AContentRect := TdxRightToLeftLayoutConverter.ConvertRect(AContentRect,
        AItemRect);
...
class procedure TdxBarCustomizationFormPainter.DrawSubItemArrow
  (ACanvas: TCanvas; X, Y: Integer; ASelected: Boolean);
var
  ASize: Integer;
begin
  ...
  ACanvas.Polygon([Point(X, Y), Point(X, Y + ASize * 2),
    Point(X + ASize, Y + ASize)]);
end;

class procedure TdxBarCustomizationFormPainter.InternalDrawCaption
...
class procedure TdxBarCustomizationFormPainter.InternalDrawEditContent
  (ACanvas: TCanvas; AItemRect: TRect; AItem: TdxCustomBarEdit;
ASelected: Boolean);
begin
  ACanvas.Font.Color := FontColors(False);
  TdxCustomBarEditAccess(AItem).DrawCustomizingImageContent(ACanvas, AItemRect,
    ASelected);
end;

class function TdxBarCustomizationFormPainter.FontColors
...
class function TdxBarCustomizationFormPainter.GetButtonColor(AItem: TdxBarItem;
ASelected: Boolean): Integer;
begin
  Result := clBtnFace;
end;
...
class function TdxBarCustomizationFormPainter.UseRightToLeftAlignment
  (AItem: TdxBarItem): Boolean;
begin
  Result := SysLocale.MiddleEast and
    (AItem.BarManager.BiDiMode = bdRightToLeft);
end;

class function TdxBarCustomizationFormPainter.GetGlyphSize
  (AItem: TdxBarItem): TSize;
begin
  Result := dxGetImageSize(AItem.Glyph, AItem.GetCurrentImages,
    AItem.ImageIndex, ScaleFactor);
end;

class function TdxBarCustomizationFormPainter.GetIsGlyphItem
  (AItem: TdxBarItem): Boolean;
begin
...
class function TdxBarCustomizationFormPainter.GetIsNeedDrawSubItemArrow
  (AItem: TdxBarItem): Boolean;
begin
  Result := (AItem is TdxBarCustomButton) and
    (TdxBarCustomButton(AItem).ButtonStyle = bsDropDown) or
    (AItem is TCustomdxBarSubItem) and not(AItem is TCustomdxBarContainerItem)
    or (AItem is TCustomdxBarContainerItem) and
    not TCustomdxBarContainerItemAccess(AItem).HideWhenRun;
end;
...
class procedure TdxBarCustomizationFormStandardPainter.DrawFocusedRect
...
class function TdxBarCustomizationFormFlatPainter.BrushColors(Selected: Boolean;
AItem: TdxBarItem): TColor;
begin
  if Selected then
...
class procedure TdxBarCustomizationFormFlatPainter.
  DrawButtonOrSubItemArrowBackground(ACanvas: TCanvas; R: TRect;
AItem: TdxBarItem; Selected: Boolean);
begin
...
begin
  ...
  TransparentDraw(ACanvas.Handle, 0, R, R, AItem.Glyph, AItem.GetCurrentImages,
    AItem.ImageIndex, AItem.BarManager.ImageListBkColor, True, False,
    True { Flat } , ASelected, False, False, True { Shadow } , False { Faded } ,
    AItem.BarManager.ImageOptions.UseLeftBottomPixelAsTransparent);
end;
...
begin
  ...
  InflateRect(ARect, -1, -1);
  if ASelected then
    ACanvas.Brush.Color := TdxBarManagerAccess(AItem.BarManager)
      .FlatToolbarsSelColor
  else
  ...
  if ASelected then
    InflateRect(ARect, -1, -1);
  InflateRect(ARect, 2, 0);
  DrawItemArrow(ACanvas.Handle, ARect, adDown, True, ASelected and
    IsHighContrastWhite, True { Flat } );
end;
...
class function TdxBarCustomizationFormFlatPainter.FontColors
  (Selected: Boolean): TColor;
begin
  Result := clBtnText;
...
class procedure TdxBarCustomizationFormFlatPainter.DrawEditEdge
  (ACanvas: TCanvas; var AItemRect: TRect; AItem: TdxCustomBarEdit;
ASelected: Boolean);
begin
  ACanvas.Brush.Color := BrushColors(False, AItem);
  if ASelected then
    ...
    Windows.FrameRect(ACanvas.Handle, AItemRect,
      GetSysColorBrush(COLOR_BTNSHADOW));
  InflateRect(AItemRect, -1, -1);
end;

class procedure TdxBarCustomizationFormFlatPainter.InternalDrawCaption
  (ACanvas: TCanvas; var ATextRect: TRect; const AText: string;
AItem: TdxCustomBarEdit; ASelected: Boolean);
const
  TextOffset = 5;
...
class procedure TdxBarCustomizationFormOffice11Painter.
  DrawButtonOrSubItemArrowBackground(ACanvas: TCanvas; R: TRect;
AItem: TdxBarItem; Selected: Boolean);
begin
  if Selected then
...
class procedure TdxBarCustomizationFormOffice11Painter.
  DrawButtonOrSubItemBackground(ACanvas: TCanvas; AItem: TdxBarItem;
...
begin
  ...
  begin
    ...
    if ASelected then
      InflateRect(ARect, -1, -1);
    InflateRect(ARect, 2, 0);
    DrawItemArrow(Handle, ARect, adDown, True, ASelected and
      IsHighContrastWhite, True { Flat } );
  end;
end;
...
class procedure TdxBarCustomizationFormOffice11Painter.DrawEditEdge
  (ACanvas: TCanvas; var AItemRect: TRect; AItem: TdxCustomBarEdit;
ASelected: Boolean);
begin
  ACanvas.Brush.Color := BrushColors(False, AItem);
  if ASelected then
    ...
    Windows.FrameRect(ACanvas.Handle, AItemRect,
      dxOffice11OwnerControlDownedBrush);
  InflateRect(AItemRect, -1, -1);
end;
...
begin
  ...
  ACanvas.FillRect(ARect);
  if not ASelected then
    Office11FillTubeGradientRect(ACanvas.Handle,
      cxRectSetWidth(ARect, cxRectHeight(ARect)), dxOffice11MenuIndentColor1,
      dxOffice11MenuIndentColor2, True)
  else
  begin
    if UseRightToLeftAlignment(AItem) then
      Dec(ARect.Left)
...
class procedure TdxBarCustomizationFormXPPainter.
  DrawButtonOrSubItemArrowBackground(ACanvas: TCanvas; R: TRect;
AItem: TdxBarItem; Selected: Boolean);
begin
  ACanvas.Brush.Color := BrushColors(False, AItem);
  ACanvas.FillRect(R);
  dxDrawThemeBackground(ACanvas.Handle, totToolBar, ScaleFactor, R,
    TP_BUTTON, TS_HOT);
end;
...
class procedure TdxBarCustomizationFormXPPainter.DrawComboBoxButton
  (ACanvas: TCanvas; AItem: TdxBarItem; ARect: TRect; ASelected: Boolean);
begin
  dxDrawThemeBackground(ACanvas.Handle, totComboBox, ScaleFactor, ARect,
    CP_DROPDOWNBUTTON, CBXS_NORMAL);
end;

class procedure TdxBarCustomizationFormXPPainter.DrawFocusedRect
  (ACanvas: TCanvas; ARect: TRect; AItem: TdxBarItem);
var
  IsGlyph: Boolean;
begin
  with TdxBarItemAccess(AItem) do
    IsGlyph := not(AItem is TdxBarEdit) and
      (not Glyph.Empty or CurImageIndexLinked);
  if IsGlyph then
...
begin
  ...
  InflateRect(AItemRect, 1, 1);
  dxDrawThemeBackground(ACanvas.Handle, totEdit, ScaleFactor, AItemRect,
...
constructor TdxBarCustomCustomizationForm.CreateEx(ABarManager: TdxBarManager);
  ...
  begin
    ...
    begin
      ...
      TdxBarManagerAccess(BarManager).FdxBarDesignHelper :=
        ARunTimeSelectionController;
    ...
    FItemsOldChangeEvent := TdxBarManagerAccess(BarManager)
      .ItemList.OnComponentListChanged;
    TdxBarManagerAccess(BarManager).ItemList.OnComponentListChanged :=
      ItemsChange;

    FGroupsOldChangeEvent := TdxBarManagerAccess(BarManager)
      .GroupList.OnComponentListChanged;
...
begin
  ...
  FAllCommandsHelper := TdxBarCustomizationFormAllCommandsListHelper.Create
    (Self, AllCommandsListBox);
  FBarListBoxHelper := TdxBarCustomizationFormBarListHelper.Create(Self,
    BarListBox);
  FCategoryListBoxHelper := TdxBarCustomizationFormCategoryListHelper.Create
    (Self, CategoriesList);
  FItemsListBoxHelper := TdxBarCustomizationFormItemsListHelper.Create(Self,
    ItemsListBox);
  FGroupsListBoxHelper := TdxBarCustomizationFormGroupsListHelper.Create(Self,
    GroupsListBox);
  FGroupItemsListBoxHelper := TdxBarCustomizationGroupItemsListBoxHelper.Create
    (Self, GroupItemsListBox);
...
begin
  ...
  Params.Style := WS_POPUP or WS_CLIPSIBLINGS or WS_SYSMENU or WS_CAPTION or
    WS_THICKFRAME;
...
procedure TdxBarCustomCustomizationForm.MouseWheelHandler
  (var Message: TMessage);
var
...
function TdxBarCustomCustomizationForm.GetLookAndFeelPainter
...
procedure TdxBarCustomCustomizationForm.FormCloseEvent(Sender: TObject;
...
procedure TdxBarCustomCustomizationForm.WMDeferredCallSynchronizationListBox
  (var Message: TMessage);
begin
...
begin
  Result := IsSelectableItem(ABar, ASelectableItem) and
  ...
  begin
    if FDisabledWindows <> nil then
      raise EdxException.Create
        ('TdxBarCustomCustomizationForm.EnableWindows fails');
...
function TdxBarCustomCustomizationForm.GetExclusiveObject
  (AListBox: TListBox): TObject;
var
  AIndex: Integer;
begin
...
function TdxBarCustomCustomizationForm.GetNextSelectedObject
  (AListBox: TListBox): TObject;
var
...
function TdxBarCustomCustomizationForm.GetObjectFromListBox(AListBox: TListBox;
...
function TdxBarCustomCustomizationForm.GetPainterClass
  : TdxBarCustomizationFormPainterClass;
begin
...
procedure TdxBarCustomCustomizationForm.GetSelection(AListBox: TListBox;
AList: TList);
var
  I: Integer;
begin
...
function TdxBarCustomCustomizationForm.PrepareMenuAnimationsComboBox
  ...
  function CalcOptimalWidth(AComboBoxFont: TFont;
  AComboBoxStrings: TStrings): Integer;
  var
    I: Integer;
  begin
...
begin
  ...
  TdxBarManagerAccess(BarManager).ItemList.OnComponentListChanged :=
    FItemsOldChangeEvent;
  TdxBarManagerAccess(BarManager).GroupList.OnComponentListChanged :=
    FGroupsOldChangeEvent;
end;
...
procedure TdxBarCustomCustomizationForm.ShowCategoryPopupMenu
  (const APoint: TPoint);
begin
    ...
    begin
      ...
      begin
        ADeleteAction.Enabled := ADeleteAction.Enabled and
          (not Assigned(ADeletePermissiveProc) and IdxBarDesigner(BarManager)
          .CanDeleteComponent(ASelectedObjects[I]) or
          Assigned(ADeletePermissiveProc) and
          ADeletePermissiveProc(ASelectedObjects[I]));
      end;
    end;
...
procedure TdxBarCustomCustomizationForm.UpdateItemDescription
  (const AText: string);
begin
end;
...
begin
  aSubMenuEditor.Enabled := (SelectedItem is TdxBarSubItem) or
    (SelectedItem is TdxBarContainerItem);
  aClearItemList.Enabled := (ItemsListBox.Items.Count > 0) and
    CanDeleteSelectedCategoryCommands;
  UpdateCommonEvents(ItemsListBox, aAddItem, aDeleteItem, aMoveUpItem,
    aMoveDownItem);
...
begin
  ...
  UpdateCommonEvents(BarListBoxHelper.ListBox, aNewToolBar, aDeleteToolBar, nil,
    nil, CanDeleteBar);
end;

procedure TdxBarCustomCustomizationForm.SynchronizeDesigner
  (ANewSelection: IdxBarSelectableItem);
begin
...
procedure TdxBarCustomCustomizationForm.UpdateVisibility(const AWindowPos
  : TWindowPos);
begin
  if AWindowPos.flags and SWP_SHOWWINDOW <> 0 then
    ShowWindow(Handle, SW_SHOW)
  else if AWindowPos.flags and SWP_HIDEWINDOW <> 0 then
...
begin
  ...
  if not(csDestroying in ComponentState) and
    not(bisFormActivating in TdxBarManagerAccess(BarManager).InternalState) then
  begin
    if BarManager.Designing then
    ...
    if // (Message.wParam <> WA_INACTIVE) and
      ...
      SendMessage(AParentForm.Handle, Message.Msg, Message.WParam,
        Message.LParam);
    { #DG
...
procedure TdxBarCustomCustomizationForm.DeleteSelectedObjects
  (AListBox: TListBox; ADeleteProc: TNotifyEvent = nil;
ASynchronizeDesigner: Boolean = True);
var
  I: Integer;
  ASelectedObjects: TObjectList;
  ANextObject: TObject;
begin
  ...
  try
    ...
    SetSelection(AListBox, ANextObject);
    // must be after deleting, because deleting causes update of ListBox
...
begin
  ...
  else if AListBox.ItemIndex <> -1 then
    Result := 1
  else
    Result := 0;
end;

function TdxBarCustomCustomizationForm.GetVisibleItemsCount
  (AListBox: TListBox): Integer;
begin
  Result := AListBox.Height div AListBox.ItemHeight;
end;

procedure TdxBarCustomCustomizationForm.GroupsChange(Sender: TObject;
...
procedure TdxBarCustomCustomizationForm.SetSelection(AListBox: TListBox;
AList: TList);
var
  I, AIndex: Integer;
begin
...
procedure TdxBarCustomCustomizationForm.DeferredCallSynchronizationListBox
  (AListBox: TListBox);
begin
  PostMessage(Handle, DXM_BAR_LB_DEFERREDCALLSYNCHRONIZATION,
    WParam(AListBox), 0);
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          SendMessage(AListBox.Handle, DXM_BAR_LB_SYNCHRONIZE,
            WParam(AChangedObject), AObjectIndex);
          if AListBox.MultiSelect then
...
procedure TdxBarCustomCustomizationForm.SynchronizeListBoxSelection
  (AListBox: TListBox);
var
  AList: TList;
begin
...
begin
  ...
  begin
    ...
    else if AAnchorIndex >
      (AListBox.TopIndex + GetVisibleItemsCount(AListBox) - 1) then
...
function TdxBarRunTimeSelectionController.IsComponentSelected
...
procedure TdxBarRunTimeSelectionController.ShowDefaultEventHandler
  (AItem: TdxBarItem);
begin
  // do nothing
end;

function TdxBarRunTimeSelectionController.UniqueName(const BaseName
  : string): string;
begin
  Result := '';
end;
...
function TdxBarCustomizationFormListBoxHelper.GetPainterClass
  : TdxBarCustomizationFormPainterClass;
begin
...
begin
  ...
  ListBox.ItemHeight := TdxTextMeasurer.TextLineHeight(ListBox.Font) +
    ScaleFactor.Apply(2);
  ListBox.MultiSelect := BarManager.Designing;
end;

function TdxBarCustomizationFormBarListHelper.IsCheckBoxArea(X,
  Y: Integer): Boolean;
var
  AIndex: Integer;
begin
  AIndex := ListBox.ItemAtPos(Point(X, Y), True);
  Result := (AIndex >= 0) and ListBox.UseRightToLeftAlignment and
    (ListBox.ItemRect(AIndex).Right - X < ListBox.ItemHeight) or
    not ListBox.UseRightToLeftAlignment and
...
begin
  ...
  begin
    ...
    cxDrawText(ListBox.Canvas.Handle, ListBox.Items[Index], R,
      ListBox.DrawTextBiDiModeFlags(DT_SINGLELINE or DT_VCENTER or
      DT_NOPREFIX));
  end;
end;

procedure TdxBarCustomizationFormBarListHelper.DoListMouseDown(Sender: TObject;
...
procedure TdxBarCustomizationFormBarListHelper.DoListKeyDown(Sender: TObject;
...
procedure TdxBarCustomizationFormCategoryListHelper.DoListBoxClick
  (Sender: TObject);
begin
  Owner.SynchronizeListBox(Owner.ItemsListBox);
end;

procedure TdxBarCustomizationFormCategoryListHelper.DoListDragOver(Sender,
...
procedure TdxBarCustomizationFormCategoryListHelper.DoListEndDrag(Sender,
...
begin
  case Button of
    mbLeft:
      if BarManager.Designing and (ListBox.ItemAtPos(Point(X, Y), True) > -1)
      then
      begin
...
procedure TdxBarCustomizationFormCategoryListHelper.WndProc
  (var Message: TMessage);
...
procedure TdxBarCustomizationFormItemsListHelper.DoListDblClick
  (Sender: TObject);
begin
...
var
  AIsDragging: Boolean;
begin
  AIsDragging := (Button = mbLeft) and Windows.DragDetect(ListBox.Handle,
    GetMouseCursorPos);
  if Button = mbRight then
  begin
    ListBox.SetFocus;
    Owner.SetSelection(ListBox,
      Owner.ItemList[ListBox.ItemAtPos(Point(X, Y), True)]);
  end;
  Owner.SynchronizeDesigner(ListBox);
...
procedure TdxBarCustomizationFormItemsListHelper.WndProc(var Message: TMessage);
  ...
  var
    AItemIndex: Integer;
  begin
    AItemIndex := ListBox.ItemAtPos
      (ListBox.ScreenToClient(GetMouseCursorPos), True);
    if AItemIndex > -1 then
      ListBox.ItemIndex := AItemIndex;
  end;
  ...
  begin
    ...
    begin
      if odFocused in TOwnerDrawState(LongRec(ADrawStruct.itemState).Lo) then
        DrawFocusRect(ADrawStruct.hDC, ADrawStruct.rcItem);
      // hide default draw focus rect
    end;
  end;
...
begin
  try
    if GetSelCount(BarListBox) > 1 then
      ...
      AConfirmString := Format(cxGetResourceString(@dxSBAR_WANTTODELETETOOLBAR),
        [SelectedBar.Caption]);

    if dxBarMessageBox(AConfirmString, MB_ICONEXCLAMATION or MB_OKCANCEL) = ID_OK
    then
...
begin
  Result := (CategoriesList.Items.Count > 1) and (CategoriesList.ItemIndex > -1)
    and CanDeleteSelectedCategoryCommands;
end;

function TdxBarCustomCustomizationForm.
  CanDeleteSelectedCategoryCommands: Boolean;
var
  I: Integer;
begin
  Result := True;
...
begin
  ...
  begin
    ...
    if dxBarMessageBox(Format(cxGetResourceString(@dxSBAR_WANTTODELETECATEGORY),
      [CategoriesList.Items[AItemIndex]]), MB_ICONEXCLAMATION or MB_OKCANCEL) = ID_OK
    then
    begin
      SelectBarManager;
...
begin
  BarManager.CategoryVisible[CategoriesList.ItemIndex] :=
    TdxBarCustomButton(Sender).Down;
  CategoriesList.SetFocus;
end;

procedure TdxBarCustomCustomizationForm.CategoriesItemsVisibleChange
  (Sender: TObject);
begin
...
procedure TdxBarCustomCustomizationForm.CategoriesPopupMenuPopup
  (Sender: TObject);
begin
  ...
  begin
    ...
    Down := Enabled and Self.BarManager.CategoryVisible
  ...
  begin
    ...
    if Enabled then
      ItemIndex := Ord(Self.BarManager.CategoryItemsVisible
        [CategoriesList.ItemIndex]);
  end;
...
procedure TdxBarCustomCustomizationForm.CommandsSubMenuEditorClick
  (Sender: TObject);
begin
...
begin
  ItemsListBox.SetFocus;
  ASubItem := TCustomdxBarSubItem(ItemsListBox.Items.Objects
    [ItemsListBox.ItemIndex]);
...
begin
  if dxBarMessageBox(Format(cxGetResourceString(@dxSBAR_WANTTOCLEARCOMMANDS),
    [CategoriesList.Items[CategoriesList.ItemIndex]]), MB_ICONEXCLAMATION or
...
procedure TdxBarCustomCustomizationForm.actDeleteGroupItemExecute
  (Sender: TObject);
begin
  try
...
begin
  MoveItems(GroupsListBox, TdxBarManagerAccess(BarManager).GroupList,
    TAction(Sender).Tag);
end;

procedure TdxBarCustomCustomizationForm.actMoveGroupItemExecute
  (Sender: TObject);
begin
  MoveItems(GroupItemsListBox,
    TdxBarGroupAccess(GroupsListBoxHelper.SelectedGroup).ItemList,
    TAction(Sender).Tag);
  GroupStructureChange;
end;
