...
type
  ...
  published
    ...
    property EnableTypes: TdxBarDBEnableTypes read FEnableTypes
      write SetEnableTypes;
  end;
  ...
  TdxBarDBNavButtonType = (dxbnFirst, dxbnPrior, dxbnNext, dxbnLast, dxbnInsert,
    dxbnDelete, dxbnEdit, dxbnPost, dxbnCancel, dxbnRefresh, dxbnAppend);
  ...
  published
    ...
    property BarDBNavigator: TdxBarDBNavigator read FBarDBNavigator
      write FBarDBNavigator;
    property Caption stored False;
    property Hint stored False;
    property NavButton: TdxBarDBNavButtonType read FNavButton
      write SetNavButton;
  end;
  ...
  published
    ...
    property DBCheckLinks: TdxBarDBCheckLinks read FDBCheckLinks
      write SetDBNavItems;
...
begin
  ...
  begin
    ...
    if (ADBNavItem.Item <> nil) and (DBNavigator <> nil) and
      (csUpdating in DBNavigator.ComponentState) and
      (DBNavigator.BarManager <> nil) then
...
const
  PResStrs: array [TdxBarDBNavButtonType] of Pointer =
    (@dxSBAR_BTNCAPTION_FIRST, @dxSBAR_BTNCAPTION_PRIOR,
    @dxSBAR_BTNCAPTION_NEXT, @dxSBAR_BTNCAPTION_LAST, @dxSBAR_BTNCAPTION_INSERT,
    @dxSBAR_BTNCAPTION_DELETE, @dxSBAR_BTNCAPTION_EDIT, @dxSBAR_BTNCAPTION_POST,
    @dxSBAR_BTNCAPTION_CANCEL, @dxSBAR_BTNCAPTION_REFRESH,
...
const
  dxBarNames: array [TdxBarDBNavButtonType] of string = ('First', 'Prev',
    'Next', 'Last', 'Insert', 'Delete', 'Edit', 'Post', 'Cancel', 'Refresh',
    'Append');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Glyph.LoadFromResource(HInstance,
          PChar(BtnResStr + IntToStr(Integer(FNavButton) + 1)), RT_BITMAP);
      end;
      if BarManager.Designing then
        Name := (BarManager as IdxBarDesigner)
          .UniqueName(dxBarDBNavBtnName + dxBarNames[FNavButton]);
...
procedure TdxBarDBNavButton.DefineProperties(Filer: TFiler);
  ...
  begin
    Result := ((Filer.Ancestor = nil) or (TdxBarDBNavButton(Filer.Ancestor)
      .Caption <> Caption)) and IsCaptionStored;
  end;

  function IsHintNeedStored: Boolean;
  begin
    Result := ((Filer.Ancestor = nil) or (TdxBarDBNavButton(Filer.Ancestor).Hint
      <> Hint)) and IsHintStored;
  end;
      ...
      case FNavButton of
        ...
        dxbnDelete:
          if not BarDBNavigator.ConfirmDelete or
            ...
            (PChar(cxGetResourceString(@dxSBAR_DELETERECORD)),
            PChar(Application.Title), MB_ICONQUESTION or MB_YESNO) = ID_YES)
          then
            Delete;
        dxbnEdit:
          Edit;
...
procedure TdxBarDBNavigator.Notification(AComponent: TComponent;
...
begin
  ...
  if Buttons[dxbnDelete] <> nil then
    Buttons[dxbnDelete].Enabled := FDataLink.Active and
...
begin
  ...
  begin
    ...
    if not(csLoading in ComponentState) and (BarManager <> nil) then
      for I := Low(Buttons) to High(Buttons) do
      begin
        if (Buttons[I] <> nil) and
        // not (csDestroying in Buttons[I].ComponentState) and
          not(I in Value) then
        begin
          Buttons[I].Free;
          Buttons[I] := nil;
