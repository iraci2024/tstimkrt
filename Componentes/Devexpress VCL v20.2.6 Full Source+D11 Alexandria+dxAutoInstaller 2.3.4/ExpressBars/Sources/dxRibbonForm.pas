...
uses
  ...
  dxCore, dxMessages, cxClasses, cxGraphics, cxControls, dxShadowWindow,
  dxRibbonSkins, dxRibbonFormCaptionHelper,
  cxDWMApi, dxAnimation, dxCoreClasses, dxCustomFluentDesignForm, cxGeometry,
  cxLookAndFeels, dxFluentDesignFormInterfaces;
...
type
  ...
  public
    constructor Create(AAutoHideMode: TdxRibbonAutoHideMode;
      AActivating: Boolean); reintroduce;
  ...
  protected
    procedure HookKeyboard(AKey: Word; AData: LPARAM;
      var AHookResult: LRESULT); virtual;
    procedure HookMouse(wParam: wParam; LPARAM: PMouseHookStruct;
      var AHookResult: LRESULT); virtual;
  ...
  public
    destructor Destroy; override;
    //
    property ActiveRibbonForm: TdxCustomRibbonForm read FActiveRibbonForm
      write SetActiveRibbonForm;
  end;

  { TdxCustomRibbonForm }

  TdxCustomRibbonForm = class(TdxCustomFluentDesignForm,
    IcxDialogMetricsClientSize, IdxRibbonFormControllerHelper,
  ...
  strict private
    ...
    procedure AfterResize(APrevRibbonVisible: Boolean;
      APrevRibbonHeight: Integer; AIsZoomed: Boolean);
    procedure BeforeResize(out ARibbonVisible: Boolean;
    ...
    procedure InternalDrawNonClientArea(ADC: HDC;
      ADrawCaption, ADrawBorder: Boolean);
    ...
    procedure DXMRibbonFormNCChanged(var Message: TMessage);
      message DXM_RIBBONFORM_NCCHANGED;
    procedure DXMRibbonFormPostUpdateRegion(var Message: TMessage);
      message DXM_RIBBONFORM_POSTUPDATEREGION;
    procedure DXMRibbonFormSysCommand(var Message: TMessage);
      message DXM_RIBBONFORM_SYSCOMMAND;
    procedure DXMSkinsPostRedraw(var Message: TMessage);
      message DXM_SKINS_POSTREDRAW;
    procedure WMDestroy(var Message: TWMDestroy); message WM_DESTROY;
    procedure WMDisplayChange(var Message: TMessage); message WM_DISPLAYCHANGE;
    procedure WMDWMCompositionChanged(var Message: TMessage);
    ...
    procedure WMNCRButtonUp(var Message: TWMNCRButtonUp);
    ...
    procedure WMStyleChanging(var Message: TWMStyleChanging);
  ...
  protected
    ...
    function HandleWithHelper(ADown: Boolean; AButton: TMouseButton)
      : Boolean; virtual;
    ...
    procedure NavigationControlChanged
      (AChanges: TdxFluentDesignNavigationControlChangeTypes); override;

    property ApplicationMenuState: TdxRibbonApplicationMenuState
      read GetApplicationMenuState;
    property DisableAero: Boolean read FDisableAero write SetDisableAero
      default False;
    property DisableMDIClientScrollBars: Boolean
      read FDisableMDIClientScrollBars write SetDisableMDIClientScrollBars
      default True;
  ...
  public
    ...
    function GetCaptionHeightDelta(AHasQuickAccessToolbar: Boolean)
      : Integer; virtual;
    procedure GetTabOrderList(List: TList); override;
    procedure Invalidate; override;
    function IsUseAeroNCPaint: Boolean;
    procedure ResetWindowRegion;
    procedure ScaleForPPI(ATargetDPI: Integer;
      AWindowRect: PRect = nil); override;
    ...
    property PrevActiveControl: TWinControl read FPrevActiveControl
      write SetPrevActiveControl;
    property RibbonAlwaysOnTop: Boolean read FRibbonAlwaysOnTop
      write SetRibbonAlwaysOnTop;
    property RibbonAutoHideMode: TdxRibbonAutoHideMode read FRibbonAutoHideMode
      write SetRibbonAutoHideMode;
    property RibbonControl: TWinControl read FRibbonControl
      write SetRibbonControl;
  ...
  published
    property AdjustLayoutForNonClientDrawing: Boolean
      read FAdjustLayoutForNonClientDrawing
      write FAdjustLayoutForNonClientDrawing default True;
    ...
    property BorderStyle: TFormBorderStyle read GetBorderStyle
      write SetBorderStyle default bsSizeable;
    property EnableAcrylicEffects;
    property KeyPreview default True;
    property NavigationControl;
    property UseSkinColor: Boolean read FUseSkinColor write SetUseSkinColor
      default True;
  end;
...
uses
  ...
  dxAcrylicEffect, cxLookAndFeelPainters, dxForms;
...
const
  ...
  dxAdjustFormSizeFlags = SWP_FRAMECHANGED or SWP_NOMOVE or SWP_NOZORDER or
    SWP_NOACTIVATE;
  dxFullRedrawFlags = RDW_INVALIDATE or RDW_ERASE or RDW_FRAME or
    RDW_ALLCHILDREN;
...
procedure DrawAcrylicNCPart(DC: HDC; const R: TRect; AMakeOpaque: Boolean;
  AProc: TDrawOnAcrylicProc);
var
  ACachedDC: HDC;
...
procedure dxRibbonAutoHideControllerMouseHook(ACode: Integer; wParam: wParam;
  LPARAM: LPARAM; var AHookResult: LRESULT);
begin
  if ACode = HC_ACTION then
    FAutoHideModeController.HookMouse(wParam, PMouseHookStruct(LPARAM),
      AHookResult);
end;

procedure dxRibbonAutoHideControllerKeyboardHook(ACode: Integer; wParam: wParam;
  LPARAM: LPARAM; var AHookResult: LRESULT);
begin
  if ACode = HC_ACTION then
...
constructor TdxCustomRibbonForm.CreateNew(AOwner: TComponent;
  Dummy: Integer = 0);
begin
...
function TdxCustomRibbonForm.GetCaptionHeightDelta(AHasQuickAccessToolbar
  : Boolean): Integer;
begin
  Result := 0;
end;
...
begin
  ...
  begin
    ...
    FFakeClientHandle := Windows.CreateWindowEx(WS_EX_CLIENTEDGE, 'MDICLIENT',
      ...
      WS_CLIPCHILDREN or WS_CLIPSIBLINGS or MDIS_ALLCHILDSTYLES, 0, 0,
      ClientWidth, ClientHeight, Handle, 0, HInstance, @ClientCreateStruct);
    SetWindowPos(FFakeClientHandle, 0, -20, -20, 10, 10, SWP_NOACTIVATE or
      SWP_NOZORDER);
...
begin
  Result := DisableAero or (ParentWindow <> 0) or IsWin10v1809OrNewer and
    (Self <> Application.MainForm) and HandleAllocated and IsIconic(Handle);
end;
...
begin
  if HandleAllocated and IsIconic(Handle) then
    Result := wsMinimized
  else if HandleAllocated and IsZoomed(Handle) then
    Result := wsMaximized
  else
    Result := wsNormal;
end;
...
procedure TdxCustomRibbonForm.NavigationControlChanged
...
procedure TdxCustomRibbonForm.AdjustLayout;
  ...
  begin
    Result := GetSystemMetrics(SM_CYCAPTION) + GetDefaultWindowBordersWidth
      (Handle, ScaleFactor).Top;
  end;
...
procedure TdxCustomRibbonForm.AlignControls(AControl: TControl;
  var Rect: TRect);
...
begin
  ...
  begin
    if RibbonControl <> nil then
      TControlAccess(RibbonControl).UpdateBoundsRect
        (cxRectSetTop(RibbonControl.BoundsRect, -1));
  end;
...
procedure TdxCustomRibbonForm.NewClientWndProc(var Message: TMessage);
  ...
  begin
    with Message do
      Result := CallWindowProc(FOldClientProc, ClientHandle, Msg,
        wParam, LPARAM);
  end;
...
var
  ...
  ARect: TRect;
begin
  if (Message.Msg = WM_MDIACTIVATE) and dxBarLockMainFormOnMergingMDIChildForm
  then
  begin
...
procedure TdxCustomRibbonForm.DrawNonClientArea(ADrawCaption: Boolean;
  AUpdateRegion: HRGN = 1);
var
  DC: HDC;
  AFlags: Integer;
  ARgn: HRGN;
...
begin
  ...
  try
    ...
    if IsAcrylic then
    begin
      DrawAcrylicNCPart(DC, GetBounds,
        not UseNavigationControlCompatibleBorders,
...
procedure TdxCustomRibbonForm.DXMRibbonFormPostUpdateRegion
...
procedure TdxCustomRibbonForm.DXMRibbonFormSysCommand(var Message: TMessage);
  ...
  const
    Commands: array [Boolean, Boolean] of Word = ((SC_MINIMIZE, SC_RESTORE),
...
begin
  Result := inherited IsAcrylicSupported and
    (RootLookAndFeel.ActiveStyle = lfsSkin);
end;
...
begin
  ...
  try
    ...
    begin
      AControl := Controls[I];
      if (AControl <> NavigationControl) and
        (AControl.Align in [alNone, alCustom]) then
      begin
...
begin
  ...
  begin
    if not AIsMaximized then
      ...
      ARgn := CreateRectRgnIndirect(cxRectContent(cxGetWindowBounds(Handle),
        GetDefaultWindowBordersWidth(Handle)));
...
procedure TdxCustomRibbonForm.AfterResize(APrevRibbonVisible: Boolean;
APrevRibbonHeight: Integer; AIsZoomed: Boolean);
...
begin
  ...
  if (ApplicationMenuState = ramsHidden) and
    IsWindowVisible(RibbonControl.Handle) and
    (APrevRibbonHeight <> RibbonControl.Height) then
  begin
...
procedure TdxCustomRibbonForm.BeforeResize(out ARibbonVisible: Boolean;
out ARibbonHeight: Integer);
...
procedure TdxCustomRibbonForm.CalculateCornerRegions;
  ...
  procedure CalculateRegion(ACornerRgn: HRGN; DX, DY: Integer;
  const ACornerRect: TRect);
  var
    R1, R2: HRGN;
  begin
...
procedure TdxCustomRibbonForm.InternalDrawNonClientArea(ADC: HDC;
ADrawCaption, ADrawBorder: Boolean);
var
  R: TRect;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          dxGPPaintCanvas.FillRectangle(R,
            GetNavigationControl.GetBackgroundColor);
...
begin
  ...
  Result := ((SHAppBarMessage(ABM_GETSTATE, ABData) and ABS_AUTOHIDE) > 0) and
    (MonitorFromWindow(FindWindow('Shell_TrayWnd', nil),
    MONITOR_DEFAULTTONEAREST) = Monitor.Handle);
end;
...
begin
  Result := (RibbonNonClientHelper <> nil) and
    RibbonNonClientHelper.IsRibbonHelpButtonPlacedOnCaption;
end;
...
procedure TdxCustomRibbonForm.ScaleForPPI(ATargetDPI: Integer;
AWindowRect: PRect = nil);
begin
...
procedure TdxCustomRibbonForm.SetDisableMDIClientScrollBars
  (const Value: Boolean);
begin
...
procedure TdxCustomRibbonForm.SetRegion(ARegion: HRGN;
ARedraw: Boolean = False);
begin
  ...
  else if ARegion <> 0 then
    DeleteObject(ARegion);
end;

procedure TdxCustomRibbonForm.SetRibbonAutoHideMode(const AValue
  : TdxRibbonAutoHideMode);
begin
...
procedure TdxCustomRibbonForm.SetRibbonNonClientHelper
  (AValue: TdxRibbonFormCaptionHelper);
begin
  if IsDestroying then
    AValue := nil;
...
begin
  ...
  ACommand := LongWord(TrackPopupMenu(AMenu, TPM_RETURNCMD or TPM_TOPALIGN or
    TPM_LEFTALIGN, P.X, P.Y, 0, Handle, nil));
...
begin
  Result := UseSkin and IsNavigationControlVisible and
    GetNavigationControl.GetAcrylicEnabled;
end;
...
procedure TdxCustomRibbonForm.CMActionUpdate(var Message: TMessage);
  ...
  begin
    Result := (AControl <> nil) and AControl.UpdateAction
    ...
    if AContainer.Showing then
      ...
      begin
        ...
        if AControl.Visible and UpdateControlAction(AControl) or
          (AControl is TWinControl) and ProcessChildren(TWinControl(AControl))
        then
        begin
          Result := True;
          Exit;
...
begin
  ...
  else if UpdateControlAction(ActiveControl) or UpdateControlAction(Self) or
    ProcessChildren(Self) then
    Message.Result := 1;
end;
...
begin
  ANeedHideRibbonControls := Visible and FVisibleChanging and
...
begin
  if UseSkin and Visible and (WindowState = wsMaximized) and IsUseAeroNCPaint
  then
  begin
...
var
  R, R1: TRect;
begin
  if UseSkin and (IsUseAeroNCPaint or cxIsDrawToMemory(Message) or
    not DoubleBuffered and not IsAcrylic) then
  begin
    R := ClientRect;
    // reduce flickering
    ...
    begin
      ...
      dxGPPaintCanvas.FillRectangle(R1,
        GetNavigationControl.GetBackgroundColor);
...
var
  ...
  ATop: Integer;
begin
  ACanCalcSize := UseSkin and not IsIconic(Handle) and
    ([csReading, csDestroying] * ComponentState = []);

  if ACanCalcSize and IsWinXP and IsZoomed(Handle) and IsThemeActive and not Visible
  then
    ACanCalcSize := False;
  ...
  begin
    ...
    begin
      ...
      ARect := cxRectContent(Message.CalcSize_Params^.rgrc[0],
        GetCurrentBordersWidth);

    if IsZoomed(Handle) then
    begin
      if FormStyle = fsMDIChild then
        Inc(ARect.Top, GetDefaultWindowNCSize(Handle, ScaleFactor).Top -
...
begin
  ...
  try
    ...
    if IsMDIChild(Self) then
      PostMessage(Handle, DXM_RIBBONFORM_POSTUPDATEREGION,
        Ord(AIsMaximizing), 0)
    else
...
begin
  ...
  begin
    ...
    if (ACommand = SC_MINIMIZE) and
      (RibbonNonClientHelper.RibbonHelper.GetBarManager <> nil) then
    ...
    begin
      if AIsChild and ((ACommand = SC_MAXIMIZE) or (ACommand = SC_MINIMIZE) or
        (ACommand = SC_RESTORE) or (ACommand = SC_CLOSE)) then
      begin
        AAnimation := TdxFormHelper.SetAnimation(False);
        inherited;
        TdxFormHelper.SetAnimation(AAnimation);
      end
      else if ACommand = SC_CONTEXTHELP then
      begin
        LockWindowUpdate(Handle);
        inherited;
        LockWindowUpdate(0);
      ...
      case ACommand of
        ...
        SC_MAXIMIZE, SC_RESTORE:
          begin
            RecalculateCaptionArea;
            // need to recalculate, if the caption was changed in IsIconic
            UpdateNonClientArea;
          end
...
procedure TdxCustomRibbonForm.WMWindowPosChanged(var Message
  : TWMWindowPosChanged);
begin
  UpdateWindowStates;
  inherited;
  ...
  if IsWin10FallCreatorsUpdate and
    ((Message.WindowPos^.flags and SWP_NOMOVE = 0) or
    ...
    begin
      case Msg of
        ...
        WM_DWMNCRENDERINGCHANGED:
          begin
            FExtendFrameAtTopHeight := -1;
            // Windows 7 can reset a non-client settings
            UpdateGlassFrame;
            Result := 0;
          end;
        WM_ENTERSIZEMOVE:
        ...
        WM_MOUSEMOVE:
          begin
            if not UseRightToLeftAlignment or
              not FRibbonNonClientHelper.CanProcessFormCaptionHitTest
              (SmallPointToPoint(TWMMouseMove(Message).Pos)) then
              inherited WndProc(Message);
          end;
        WM_NCLBUTTONDOWN:
          begin
...
begin
  Result := not DontUseAero and (FormStyle <> fsMDIChild) and (GetHandle <> 0)
    and IsCompositionEnabled;
end;

procedure TdxCustomRibbonForm.CheckResizingNCHitTest(var AHitTest: LRESULT;
const P: TPoint);
const
  CornerHitTests: array [0 .. 3] of DWORD = (HTTOPLEFT, HTTOPRIGHT,
  ...
  begin
    ...
    begin
      ...
      if cxRectPtIn(R, P) then
        AHitTest := HTTOP
      else if not IsUseAeroNCPaint then
      begin
        R := RW;
...
function TdxCustomRibbonForm.GetFormBorderIcons: TdxRibbonBorderIcons;
  ...
  function ConverterBorderIconsToRibbonBorderIcons(const AIcons: TBorderIcons)
    : TdxRibbonBorderIcons;
  const
    Map: array [TBorderIcon] of TdxRibbonBorderIcon = (rbiSystemMenu,
      rbiMinimize, rbiMaximize, rbiHelp);
  var
    I: TBorderIcon;
  begin
    Result := [];
...
begin
  ...
  begin
    ...
    if RibbonAutoHideMode.Active and not RibbonAutoHideMode.IsUIShown then
      Result := Result * [rbiDisplayOptions, rbiSystemMenu] +
        [rbiAutoHideModeShowUI];
  end;
...
function TdxCustomRibbonForm.GetRibbonNonClientHelper
  : TdxRibbonFormCaptionHelper;
begin
  Result := FRibbonNonClientHelper;
  // CBuilder 2009 workaround (Buggy CB must die!)
end;
...
function TdxCustomRibbonForm.GetApplicationMenuState
...
procedure TdxCustomRibbonForm.WMSetText(var Message: TWMSetText);
  ...
  begin
    ...
    begin
      ...
      DefFrameProc(Handle, ClientHandle, WM_SETTEXT, 0,
...
begin
  ...
  begin
    ...
    begin
      ...
      if GetWindowLong(Handle, GWL_EXSTYLE) and WS_EX_APPWINDOW = WS_EX_APPWINDOW
      then
        TdxFormHelper.SetWindowTextWithoutRedrawing(Handle,
          RibbonNonClientHelper.GetTaskBarCaption);
    end
    else
    begin
      inherited;
...
begin
  Result := inherited CalculateVisibility and
    not OwnerWindow.IsUseAeroNCPaint and OwnerWindow.UseSkin and
    Assigned(OwnerWindow.RibbonNonClientHelper) and
    not OwnerWindow.RibbonNonClientHelper.UseRoundedWindowCorners and
    dxDefaultBooleanToBoolean(OwnerWindow.ShowFormShadow,
    OwnerWindow.RibbonNonClientHelper.RibbonHelper.AreBordersThin);
end;
...
begin
  Result := Enabled and RibbonForm.UseSkin and
    not RibbonForm.IsUseAeroNCPaint and RibbonForm.HandleAllocated and
    (GetParent(RibbonForm.Handle) = 0);
end;
...
procedure TdxRibbonAutoHideMode.UpdateUIVisibility(AVisible,
  AAnimated: Boolean);
begin
...
begin
  ...
  begin
    if Supports(RibbonForm.RibbonNonClientHelper.RibbonHelper.
      GetStatusBarInterface, IdxRibbonFormStatusBar, AIntf) then
...
constructor TdxRibbonAutoHideModeAnimation.Create(AAutoHideMode
  : TdxRibbonAutoHideMode; AActivating: Boolean);
...
begin
  ...
  SetWindowPos(Ribbon.Handle, 0, R1.Left, R1.Top, 0, 0, SWP_NOSIZE or
    SWP_NOZORDER);
  ...
  begin
    ...
    R1 := cxRectOffset(R1, 0, MulDiv(AStatusBar.Height,
      100 - AVisibility, 100));
    SetWindowPos(AStatusBar.Handle, 0, R1.Left, R1.Top, 0, 0,
...
begin
  ...
  if AControl.HandleAllocated then
    SetWindowPos(AControl.Handle, HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE or
      SWP_NOMOVE);
end;
...
procedure TdxRibbonAutoHideModeController.HookKeyboard(AKey: Word;
AData: LPARAM; var AHookResult: LRESULT);
...
procedure TdxRibbonAutoHideModeController.HookMouse(wParam: wParam;
LPARAM: PMouseHookStruct; var AHookResult: LRESULT);
begin
...
function TdxRibbonAutoHideModeController.IsChildHandle(AHandle: HWND): Boolean;
  ...
  begin
    ...
    if (Result = nil) and (BarDesignController.CustomizingBarManager <> nil)
    then
    begin
  ...
  function GetControlOwner(AControl: TWinControl;
  out AOwnerHandle: HWND): Boolean;
  var
...
var
  AStatusBar: TWinControl;
begin
  Result := HasNativeHandle(ActiveRibbonForm.RibbonAutoHideMode.Ribbon,
    AHandle);
  if not Result and ActiveRibbonForm.RibbonAutoHideMode.GetStatusBar(AStatusBar)
  then
    Result := HasNativeHandle(AStatusBar, AHandle);
end;

function TdxRibbonAutoHideModeController.IsInCaptionArea
...
procedure TdxRibbonAutoHideModeController.Notification(AComponent: TComponent;
...
procedure TdxRibbonAutoHideModeController.SetActiveRibbonForm
  (const AValue: TdxCustomRibbonForm);
begin
