...
uses
  ...
  dxBarSkinConsts, dxRibbon, cxGeometry,
  dxCoreGraphics;
...
type
  ...
  public
    ...
    procedure Popup(P: TPoint;
      AContextMenu: TdxBarCustomPopupMenu = nil); overload;
  end;
  ...
  protected
    ...
    // TdxBarPopupControl
    function GetPopupPosition(const ASize: TSize; const AOwnerRect: TRect;
      APopupDirection: TXDirection): TPoint; override;
...
procedure TdxRibbonCustomMiniToolbar.Popup(P: TPoint;
  AContextMenu: TdxBarCustomPopupMenu);

  function CanShowBar: Boolean;
  begin
    Result := TdxBarManagerAccess(BarManager).IsOwnerVisible and
      IsFormActive(BarManager.ParentForm) and (Ribbon <> nil);
  end;

  procedure CorrectPosForContext(AContextMenu, AMiniToolbarControl
    : TCustomdxBarControl);
...
begin
  ...
  begin
    ...
    AMiniToolbarControl := GetMiniToolbarControl;
    BarDesignController.ShowQuickControl(AMiniToolbarControl, FPainter,
      cxRect(P, P));

    if AContextMenu <> nil then
    begin
      AMiniToolbarControl.FContextMenu := AContextMenu;
      TdxBarCustomPopupMenuAccess(AContextMenu).OnCloseUpEx :=
        AMiniToolbarControl.OnContextMenuClose;

      TdxBarCustomPopupMenuAccess(AContextMenu).FUseOwnMessageLoop := False;
      TdxBarCustomPopupMenuAccess(AContextMenu).FMinWidth :=
        ItemLinks.BarControl.Width;
      ...
      CorrectPosForContext
        (AContextMenu.ItemLinks.BarControl as TdxBarSubMenuControl,
        AMiniToolbarControl);
    end;
...
function TdxRibbonCustomMiniToolbar.GetMiniToolbarControl
  : TdxRibbonMiniToolbarControl;
begin
...
begin
  ...
  else if AMinMouseDistance > 176 then
    Result := -1
  else
  begin
...
procedure TdxRibbonMiniToolbarControl.OnContextMenuClose(Sender: TObject;
...
begin
  Result := Point(AOwnerRect.Right, AOwnerRect.Bottom - ASize.cy -
    ScaleFactor.Apply(11));
end;
...
var
  ...
  AOffsets: TRect;
begin
  ALineHeight := GetSizeForHeight(dsNone,
    TdxTextMeasurer.TextLineHeight(Font)).cy;
  Result := GetSizeForHeight(dsNone, 2 * ALineHeight + cxTextOffset, True);
  AOffsets := Painter.GetToolbarContentOffsets(Bar, DockingStyle,
    ScaleFactor, False);
...
begin
  Result := inherited IsChildWindow(AWnd) or (GetContextPopup <> nil) and
    TCustomdxBarControlAccess(GetContextPopup).IsChildWindow(AWnd);
end;
...
begin
  if AReason <> bcrEnter then
    CloseUp
  else if SelectedControl <> nil then
...
begin
  Result := (FContextMenu = nil) and
    ((SelectedControl = nil) or not SelectedControl.HasWindow);
end;
...
function TdxRibbonMiniToolbarControl.CanShowPopupMenuOnMouseClick
  (AMousePressed: Boolean): Boolean;
begin
  Result := False;
end;

function TdxRibbonMiniToolbarControl.GetItemControlDefaultViewLevel
...
procedure TdxRibbonMiniToolbarPainter.DrawToolbarContentPart
...
procedure TdxRibbonMiniToolbarPainter.DrawToolbarNonContentPart
  (ABarControl: TdxBarControl; DC: HDC);
begin
  with ABarControl.ClientBounds do
    ExcludeClipRect(DC, Left, Top, Right, Bottom);
  Skin.DrawBackground(DC, TCustomdxBarControlAccess(ABarControl).NCRect,
    DXBAR_DROPDOWNBORDER);
end;

function TdxRibbonMiniToolbarPainter.GetGlyphColorPalette(ABarItemControl
