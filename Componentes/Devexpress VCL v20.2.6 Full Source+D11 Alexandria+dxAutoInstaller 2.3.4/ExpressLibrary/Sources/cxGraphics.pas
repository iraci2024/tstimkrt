...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Classes, SysUtils, Controls, Graphics, Menus, CommCtrl,
  ComCtrls, ImgList,
  dxCore, dxCoreGraphics, dxMessages, cxClasses, cxGeometry, dxGDIPlusClasses,
  dxGDIPlusApi,
...
const
  ...
  SystemAlignmentsHorz: array [TAlignment] of Integer = (DT_LEFT, DT_RIGHT,
    DT_CENTER);
  SystemAlignmentsVert: array [TcxAlignmentVert] of Integer = (DT_TOP,
    DT_BOTTOM, DT_VCENTER);
  cxAlignmentsHorz: array [TAlignment] of Integer = (cxAlignLeft, cxAlignRight,
    cxAlignHCenter);
  cxAlignmentsVert: array [TcxAlignmentVert] of Integer = (cxAlignTop,
...
{$ENDIF}
  ...
  public
    ...
    constructor CreateRoundCorners(ALeft, ATop, ARight, ABottom, AWidthEllipse,
      AHeightEllipse: Integer); overload;
    constructor CreateRoundCorners(const ABounds: TRect;
      AWidthEllipse, AHeightEllipse: Integer); overload;
    destructor Destroy; override;

    function Clone: TcxRegion;
    procedure Combine(ARegion: TcxRegion; AOperation: TcxRegionOperation;
      ADestroyRegion: Boolean = True); overload;
    procedure Combine(ARegionHandle: TcxRegionHandle;
      AOperation: TcxRegionOperation); overload;
    ...
    function RectInRegion(ALeft, ATop, ARight, ABottom: Integer)
      : Boolean; overload;
...
{$ENDREGION}
  ...
  public
    ...
    procedure Polygon(const Points: array of TPoint;
      ABrushColor, APenColor: TColor); overload; override;
    procedure Polyline(const Points: array of TPoint); reintroduce; overload;
    procedure Polyline(const Points: array of TPoint; AColor: TColor;
    ...
    procedure FillRect(const R: TRect; const AParams: TcxViewParams;
      AExcludeRect: Boolean = False); overload;
    procedure FillRegion(ARegion: TcxRegion;
      AColor: TColor = clDefault); overload;
    procedure FillRegion(ARegion: TcxRegionHandle;
      AColor: TColor = clDefault); overload;
    procedure FrameRect(const R: TRect; Color: TColor = clDefault;
      ALineWidth: Integer = 1; const ABorders: TcxBorders = cxBordersAll;
      AExcludeFrame: Boolean = False); reintroduce; overload;
    ...
    procedure BrushCopy(const Dest: TRect; Bitmap: TBitmap; const Source: TRect;
      Color: TColor);
    procedure CopyRect(const Dest: TRect; ACanvas: TCanvas;
      const Source: TRect);
    procedure Draw(X, Y: Integer; Graphic: TGraphic); overload;
    procedure Draw(X, Y: Integer; Graphic: TGraphic; Opacity: Byte); overload;
    procedure DrawDesignSelection(ARect: TRect;
      AWidth: Integer = cxDesignSelectionWidth);
    procedure DrawFocusRect(const R: TRect);
    procedure DrawFocusRectEx(const R: TRect; ABorders: TcxBorders;
      AExclude: Boolean = False);
    procedure DrawGlyph(X, Y: Integer; AGlyph: TBitmap;
      AEnabled: Boolean = True; ABackgroundColor: TColor = clNone);
    ...
    procedure DrawRegion(ARegion: TcxRegion; AContentColor: TColor = clDefault;
      ABorderColor: TColor = clDefault; ABorderWidth: Integer = 1;
      ABorderHeight: Integer = 1); overload;
    procedure DrawRegion(ARegion: TcxRegionHandle;
      AContentColor: TColor = clDefault; ABorderColor: TColor = clDefault;
      ABorderWidth: Integer = 1; ABorderHeight: Integer = 1); overload;
    ...
    procedure DrawTexT(const Text: string; R: TRect; AAlignmentHorz: TAlignment;
      ...
      AMultiLine, AShowEndEllipsis: Boolean); overload;
    ...
    procedure AlignMultiLineTextRectVertically(var R: TRect;
    ...
    procedure TextExtent(const Text: string; var R: TRect;
      Flags: Integer); overload;
  ...
  private
    ...
    procedure CompressByBlock(ASourceStream, ADestStream: TStream;
      ASize, ABlockSize: Integer);
    procedure DecompressByBlock(ASourceStream, ADestStream: TStream;
      ASize, ABlockSize: Integer);
  ...
  protected
    ...
    procedure Initialize(AWidth, AHeight: Integer;
      APixelFormat: TPixelFormat); virtual;
  ...
  public
    ...
    constructor CreateSize(const ARect: TRect;
      APixelFormat: TPixelFormat); overload;
    ...
    procedure CopyBitmap(ASrcBitmap: TBitmap;
      ACopyMode: DWORD = SRCCOPY); overload;
    procedure CopyBitmap(ASrcBitmap: TBitmap; const ADestRect: TRect;
      const ASrcTopLeft: TPoint; ACopyMode: DWORD = SRCCOPY); overload;
    procedure Dormant;
    procedure Flip(AFlipHorizontally, AFlipVertically: Boolean);
    procedure Rotate(ARotationAngle: TcxRotationAngle;
      AFlipVertically: Boolean = False);
  ...
  TcxImageDrawMode = (idmNormal, idmDisabled, idmFaded, idmGrayScale, idmDingy,
    idmShadowed);
  TcxImageFitMode = (ifmNormal, ifmStretch, ifmProportionalStretch,
    ifmFit, ifmFill);
  TcxBitmapTransformationMode = (btmDingy, btmDirty, btmGrayScale, btmSetOpaque,
    btmMakeMask, btmFade, btmDisable, btmCorrectBlend, btmHatch, btmClear,
    btmResetAlpha, btmExtractLayer);
  TcxBitmapTransformationProc = procedure(var AColor: TRGBQuad) of object;
  TcxDrawImageProc = function(ACanvas: TCanvas; AImages: TCustomImageList;
    AImageIndex: Integer; AGlyph: TGraphic; ARect: TRect;
    ADrawMode: TcxImageDrawMode): Boolean;
  ...
  protected
    ...
    procedure Resize(ANewWidth, ANewHeight: Integer; AStretch, ASmooth: Boolean;
      AFillColor: TColor = clNone);
  public
    ...
    procedure AlphaBlend(ADestBitmap: TcxBitmap32; const ADestRect: TRect;
      ASmoothImage: Boolean; AConstantAlpha: Byte = $FF);
  ...
  protected
    procedure Initialize(AWidth, AHeight: Integer;
      APixelFormat: TPixelFormat); override;

    procedure ScaleColor(var AColor: TRGBQuad;
      const AColorMap: TcxColorTransitionMap);

    property HatchData: TcxHatchData read FHatchData write FHatchData;
    property TransparentBkColor: TRGBQuad read FTransparentBkColor
      write FTransparentBkColor;
    property TransparentPixels: TcxColorList read FTransparentPixels;
  public
    constructor CreateSize(AWidth, AHeight: Integer); overload; override;
    constructor CreateSize(AWidth, AHeight: Integer;
      ATransparentBkColor: TRGBQuad); overload;
    destructor Destroy; override;

    procedure Clear; override;
    procedure DrawHatch(const AHatchData: TcxHatchData); overload;
    procedure DrawHatch(AColor1, AColor2: TColor; AStep: Byte;
      AAlpha1: Byte = $FF; AAlpha2: Byte = $FF); overload;
    procedure DrawShadow(AMaskBitmap: TcxAlphaBitmap; AShadowSize: Integer;
      AShadowColor: TColor; AInflateSize: Boolean = False);
    procedure ExtractLayer(ATransparentColor: TColor);
    procedure RecoverAlphaChannel(ATransparentColor: TColor); overload;
    procedure RecoverAlphaChannel(ARect: TRect;
      ATransparentColor: TColor); overload;
  ...
  public
    ...
    procedure DrawAlphaBlend(DC: HDC; const P: TPoint;
      AAlpha: Byte = 255); overload;
    procedure DrawAlphaBlend(DC: HDC; const R: TRect;
      AAlpha: Byte = 255); overload;
    //
  ...
  private
    ...
    function GetFrame(AFrameIndex: Integer; const ASize: TSize;
  ...
  public
    ...
    function GetAsMultiFrameTIFF(const ASize: TSize; AMode: TcxImageFitMode)
      : TdxSmartImage; overload;
  ...
  public
    class function ChangeLightness(AColor: TColor;
      const ALightnessDelta: Double): TColor;
    class function ChangeSaturation(AColor: TColor;
      const ASaturationDelta: Double): TColor;
    class function MultiplyLightness(AColor: TColor;
      const AFactor: Double): TColor;

    class function AlphaColorToHexCode(AValue: TdxAlphaColor;
      AIsDelphiNotation: Boolean; AIncludeAlphaValue: Boolean = True): string;
    class function CompleteHexCode(const AValue: string;
      AIsDelphiNotation: Boolean): string;
    class function HexCodeToAlphaColor(AHexCode: string;
      AIsDelphiNotation: Boolean): TdxAlphaColor;
  end;
  ...
  protected type
    TCheckParamFunc = function(ALastSuitableValue, AOriginalValue,
      AMinValue: Integer): Integer of object;
    TMeasureTextFunc = function(const AText: string; AParam: Integer)
      : Integer of object;
  strict private
    ...
    function DoMeasureTextHeight1(const AText: string;
      ALineSpacing: Integer): Integer;
    function DoMeasureTextHeight2(const AText: string;
      AFontSize: Integer): Integer;
    function DoMeasureTextWidth(const AText: string;
      AFontSize: Integer): Integer;
  protected
    FFlags: Cardinal;
    ...
    function CalculateCore(AAvailableSize, AParam, AParamLow: Integer;
      const AText: string; AMeasureFunc: TMeasureTextFunc;
      ACheckParamFunc: TCheckParamFunc): Integer;
    function CheckFontSize(ALastSuitableSize, AOriginalSize, AMinSize: Integer)
      : Integer; virtual;

    function GetMinFontSize: Integer; virtual; abstract;
    function GetMinLineSpacing: Single; virtual;
    function GetTextHeight(const AText: string; AWidth: Integer;
      ALineSpacing: Single): Integer; virtual; abstract;
    function GetTextWidth(const AText: string): Integer; virtual; abstract;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Calculate(const AAvailableWidth: Integer;
      const AText: string); overload;
    procedure Calculate(const AAvailableSize: TSize; const AText: string;
      AMultiLine: Boolean; AFlags: Cardinal); overload;

    property AllowAdjustLineSpacing: Boolean read FAllowAdjustLineSpacing
      write FAllowAdjustLineSpacing;
  ...
  protected
    ...
    function GetTextHeight(const AText: string; AWidth: Integer;
      ALineSpacing: Single): Integer; override;
  ...
  TdxDrawImageCacheID = packed record
    ...
    constructor Create(AGlyph: TGraphic; AImageList: TCustomImageList;
...
{$REGION 'internal types'}
  protected type
    ...
    strict private
      ...
      FSizes: TQueue<TSize>;
    protected
      function GetImage(const ABounds: TRect; AIndex: Integer;
    ...
    public
      constructor Create(AImages: TCustomImageList; AMaxSizeCount: Integer = 0);
        reintroduce;
      destructor Destroy; override;
      procedure Draw(ACanvas: TcxGdiBasedCanvas; const ABounds: TRect;
        AIndex: Integer; ADrawMode: TcxImageDrawMode;
        AUseLeftBottomPixelAsTransparent: Boolean;
        const APalette: IdxColorPalette; ASmoothImage: Boolean); overload;
      procedure Draw(AGraphic: TdxGPGraphics; const ABounds: TRect;
        AIndex: Integer; ADrawMode: TcxImageDrawMode;
        AUseLeftBottomPixelAsTransparent: Boolean;
        const APalette: IdxColorPalette); overload;
...
{$HINTS ON}
  ...
  public
    class procedure Draw(ACanvas: TcxGdiBasedCanvas; const ABounds: TRect;
      AImages: TCustomImageList; AIndex: Integer; ADrawMode: TcxImageDrawMode;
      AUseLeftBottomPixelAsTransparent: Boolean; APalette: IdxColorPalette;
      ASmoothImage: Boolean); overload; static;
    class procedure Draw(AGraphic: TdxGPGraphics; const ABounds: TRect;
      AImages: TCustomImageList; AIndex: Integer; ADrawMode: TcxImageDrawMode;
      AUseLeftBottomPixelAsTransparent: Boolean; APalette: IdxColorPalette);
      overload; static;
...
const
  DisableMap: TcxColorTransitionMap = (RedScale: 0.0729; GreenScale: 0.7146;
    BlueScale: 0.2125; SrcAlpha: 105; SrcConstantAlpha: 151);
  FadeMap: TcxColorTransitionMap = (RedScale: 0.299; GreenScale: 0.587;
    BlueScale: 0.114; SrcAlpha: 192; SrcConstantAlpha: 64);
  GrayMap: TcxColorTransitionMap = (RedScale: 0.299; GreenScale: 0.587;
    BlueScale: 0.114; SrcAlpha: $FF; SrcConstantAlpha: $FF);

  EnabledImageDrawModeMap: array [Boolean] of TcxImageDrawMode = (idmDisabled,
    idmNormal);
...
const
  cxColorsByName: array [0 .. 168] of TcxColorInfo = ((Name: 'aliceblue';
    Color: $00FFF8F0), (Name: 'antiquewhite'; Color: $00D7EBFA), (Name: 'aqua';
    Color: clAqua), (Name: 'aquamarine'; Color: $007FD4FF), (Name: 'azure';
    Color: $00FFFFF0), (Name: 'beige'; Color: $00DCF5F5), (Name: 'bisque';
    Color: $00C4E4FF), (Name: 'black'; Color: clBlack), (Name: 'blanchedalmond';
    Color: $00CDFFFF), (Name: 'blue'; Color: clBlue), (Name: 'blueviolet';
    Color: $00E22B8A), (Name: 'brown'; Color: $002A2AA5), (Name: 'burlywood';
    Color: $0087B8DE), (Name: 'cadetblue'; Color: $00A09E5F),
    (Name: 'chartreuse'; Color: $0000FF7F), (Name: 'chocolate';
    Color: $001E69D2), (Name: 'coral'; Color: $00507FFF),
    (Name: 'cornflowerblue'; Color: $00ED9564), (Name: 'cornsilk';
    Color: $00DCF8FF), (Name: 'crimson'; Color: $003C14DC), (Name: 'cyan';
    Color: $00FFFF00), (Name: 'darkblue'; Color: $008B0000), (Name: 'darkcyan';
    Color: $008B8B00), (Name: 'darkgoldenrod'; Color: $000B86B8),
    (Name: 'darkgray'; Color: $00A9A9A9), (Name: 'darkgreen'; Color: $00006400),
    (Name: 'darkkhaki'; Color: $006BB7BD), (Name: 'darkmagenta';
    Color: $008B008B), (Name: 'darkolivegreen'; Color: $002F6B55),
    (Name: 'darkorange'; Color: $00008CFF), (Name: 'darkorchid';
    Color: $00CC3299), (Name: 'darkred'; Color: $0000008B), (Name: 'darksalmon';
    Color: $007A96E9), (Name: 'darkseagreen'; Color: $008FBC8F),
    (Name: 'darkslateblue'; Color: $008B3D48), (Name: 'darkslategray';
    Color: $004F4F2F), (Name: 'darkturquoise'; Color: $00D1CE00),
    (Name: 'darkviolet'; Color: $00D30094), (Name: 'deeppink';
    Color: $009314FF), (Name: 'deepskyblue'; Color: $00FFBF00),
    (Name: 'dimgray'; Color: $00696969), (Name: 'dodgerblue'; Color: $00FF901E),
    (Name: 'firebrick'; Color: $002222B2), (Name: 'floralwhite';
    Color: $00F0FAFF), (Name: 'forestgreen'; Color: $00228B22),
    (Name: 'fuchsia'; Color: $00FF00FF), (Name: 'gainsboro'; Color: $00DCDCDC),
    (Name: 'ghostwhite'; Color: $00FFF8F8), (Name: 'gold'; Color: $0000D7FF),
    (Name: 'goldenrod'; Color: $0020A5DA), (Name: 'gray'; Color: clGray),
    (Name: 'green'; Color: clGreen), (Name: 'greenyellow'; Color: $002FFFAD),
    (Name: 'honeydew'; Color: $00F0FFF0), (Name: 'hotpink'; Color: $00B469FF),
    (Name: 'indianred'; Color: $005C5CCD), (Name: 'indigo'; Color: $0082004B),
    (Name: 'ivory'; Color: $00F0F0FF), (Name: 'khaki'; Color: $008CE6F0),
    (Name: 'lavender'; Color: $00FAE6E6), (Name: 'lavenderblush';
    Color: $00F5F0FF), (Name: 'lawngreen'; Color: $0000FC7C),
    (Name: 'lemonchiffon'; Color: $00CDFAFF), (Name: 'lightblue';
    Color: $00E6D8AD), (Name: 'lightcoral'; Color: $008080F0),
    (Name: 'lightcyan'; Color: $00FFFFE0), (Name: 'lightgoldenrodyellow';
    Color: $00D2FAFA), (Name: 'lightgreen'; Color: $0090EE90),
    (Name: 'lightgrey'; Color: $00D3D3D3), (Name: 'lightpink';
    Color: $00C1B6FF), (Name: 'lightsalmon'; Color: $007AA0FF),
    (Name: 'lightseagreen'; Color: $00AAB220), (Name: 'lightskyblue';
    Color: $00FACE87), (Name: 'lightslategray'; Color: $00998877),
    (Name: 'lightsteelblue'; Color: $00DEC4B0), (Name: 'lightyellow';
    Color: $00E0FFFF), (Name: 'lime'; Color: clLime), (Name: 'limegreen';
    Color: $0032CD32), (Name: 'linen'; Color: $00E6F0FA), (Name: 'magenta';
    Color: $00FF00FF), (Name: 'maroon'; Color: clMaroon),
    (Name: 'mediumaquamarine'; Color: $00AACD66), (Name: 'mediumblue';
    Color: $00CD0000), (Name: 'mediumorchid'; Color: $00D355BA),
    (Name: 'mediumpurple'; Color: $00DB7093), (Name: 'mediumseagreen';
    Color: $0071B33C), (Name: 'mediumpurple'; Color: $00DB7093),
    (Name: 'mediumslateblue'; Color: $00EE687B), (Name: 'mediumspringgreen';
    Color: $009AFA00), (Name: 'mediumturquoise'; Color: $00CCD148),
    (Name: 'mediumvioletred'; Color: $008515C7), (Name: 'midnightblue';
    Color: $00701919), (Name: 'mintcream'; Color: $00FAFFF5),
    (Name: 'mistyrose'; Color: $00E1E4FF), (Name: 'moccasin'; Color: $00B5E4FF),
    (Name: 'navajowhite'; Color: $00ADDEFF), (Name: 'navy'; Color: clNavy),
    (Name: 'oldlace'; Color: $00E6F5FD), (Name: 'olive'; Color: $00008080),
    (Name: 'olivedrab'; Color: $00238E6B), (Name: 'orange'; Color: $0000A5FF),
    (Name: 'orangered'; Color: $000045FF), (Name: 'orchid'; Color: $00D670DA),
    (Name: 'palegoldenrod'; Color: $00AAE8EE), (Name: 'palegreen';
    Color: $0098FB98), (Name: 'paleturquoise'; Color: $00EEEEAF),
    (Name: 'palevioletred'; Color: $009370DB), (Name: 'papayawhip';
    Color: $00D5EFFF), (Name: 'peachpuff'; Color: $00BDDBFF), (Name: 'peru';
    Color: $003F85CD), (Name: 'pink'; Color: $00CBC0FF), (Name: 'plum';
    Color: $00DDA0DD), (Name: 'powderblue'; Color: $00E6E0B0), (Name: 'purple';
    Color: $00800080), (Name: 'red'; Color: clRed), (Name: 'rosybrown';
    Color: $008F8FBC), (Name: 'royalblue'; Color: $00E16941),
    (Name: 'saddlebrown'; Color: $0013458B), (Name: 'salmon'; Color: $007280FA),
    (Name: 'sandybrown'; Color: $0060A4F4), (Name: 'seagreen';
    Color: $00578B2E), (Name: 'seashell'; Color: $00EEF5FF), (Name: 'sienna';
    Color: $002D52A0), (Name: 'silver'; Color: $00C0C0C0), (Name: 'skyblue';
    Color: $00EBCE87), (Name: 'slateblue'; Color: $00CD5A6A),
    (Name: 'slategray'; Color: $00908070), (Name: 'snow'; Color: $00FAFAFF),
    (Name: 'springgreen'; Color: $007FFF00), (Name: 'steelblue';
    Color: $00B48246), (Name: 'tan'; Color: $008CB4D2), (Name: 'teal';
    Color: clTeal), (Name: 'thistle'; Color: $00D8BFD8), (Name: 'tomato';
    Color: $004763FD), (Name: 'turquoise'; Color: $00D0E040), (Name: 'violet';
    Color: $00EE82EE), (Name: 'wheat'; Color: $00B3DEF5), (Name: 'white';
    Color: clWhite), (Name: 'whitesmoke'; Color: $00F5F5F5), (Name: 'yellow';
    Color: clYellow), (Name: 'yellowgreen'; Color: $0032CD9A),

    (Name: 'activeborder'; Color: clActiveBorder), (Name: 'activecaption';
    Color: clActiveCaption), (Name: 'appworkspace'; Color: clAppWorkSpace),
    (Name: 'background'; Color: clBackground), (Name: 'buttonface';
    Color: clBtnFace), (Name: 'buttonhighlight'; Color: clBtnHighlight),
    (Name: 'buttonshadow'; Color: clBtnShadow), (Name: 'buttontext';
    Color: clBtnText), (Name: 'captiontext'; Color: clCaptionText),
    (Name: 'graytext'; Color: clGrayText), (Name: 'highlight';
    Color: clHighlight), (Name: 'highlighttext'; Color: clHighlightText),
    (Name: 'inactiveborder'; Color: clInactiveBorder), (Name: 'inactivecaption';
    Color: clInactiveCaption), (Name: 'inactivecaptiontext';
    Color: clInactiveCaptionText), (Name: 'infobackground'; Color: clInfoBk),
    (Name: 'infotext'; Color: clInfoText), (Name: 'menu'; Color: clMenu),
    (Name: 'menutext'; Color: clMenuText), (Name: 'scrollbar';
    Color: clScrollBar), (Name: 'threeddarkshadow'; Color: cl3DDkShadow),
    (Name: 'threedface'; Color: clBtnFace), (Name: 'threedhighlight';
    Color: clHighlightText), (Name: 'threedlightshadow'; Color: cl3DLight),
    (Name: 'threedshadow'; Color: clBtnShadow), (Name: 'window';
    Color: clWindow), (Name: 'windowframe'; Color: clWindowFrame),
    (Name: 'windowtext'; Color: clWindowText));

const
  dxDefaultFontSizeCount = 16;
  dxDefaultFontSizes: array [0 .. dxDefaultFontSizeCount - 1] of Integer = (8,
...
procedure cxSetImageList(const AValue: TCustomImageList;
...
function IsImageAssigned(AGlyph: TGraphic; AImageList: TCustomImageList;
  AImageIndex: Integer): Boolean; overload;
function IsImageAssigned(AImage: TGraphic): Boolean; overload;
function IsImageAssigned(AImageList: TCustomImageList; AImageIndex: Integer)
  : Boolean; overload;
function IsPictureAssigned(APicture: TPicture): Boolean;
function dxGetImageSize(AImage: TGraphic): TSize; overload;
function dxGetImageSize(AImage: TGraphic; AScaleFactor: TdxScaleFactor)
  : TSize; overload;
function dxGetImageSize(AImage: TGraphic; AImageList: TCustomImageList;
  AImageIndex: Integer): TSize; overload;
function dxGetImageSize(AImage: TGraphic; AImageList: TCustomImageList;
  AImageIndex: Integer; AScaleFactor: TdxScaleFactor): TSize; overload;
function dxGetImageSize(AImageList: TCustomImageList;
  AScaleFactor: TdxScaleFactor): TSize; overload;
function dxGetImageSize(APicture: TPicture; AScaleFactor: TdxScaleFactor)
  : TSize; overload;
...
// light colors
function GetLightColor(ABtnFaceColorPart, AHighlightColorPart, AWindowColorPart
  : TcxColorPart): TColor;
...
procedure FillGradientRect(DC: HDC; const ARect: TRect;
  AColor1, AColor2: TColor; AHorizontal: Boolean);
procedure FillTubeGradientRect(DC: HDC; const ARect: TRect;
  AColor1, AColor2: TColor; AGradientPercent: Integer; AHorizontal: Boolean);
...
function GetGradientColorRect(const ARect: TRect; X: Integer;
  AColor1, AColor2: TColor; AHorizontal: Boolean): TColor;
...
procedure cxAlphaBlend(ADestBitmap, ASrcBitmap: TBitmap;
  const ADestRect, ASrcRect: TRect; ASmoothImage: Boolean = False;
  AConstantAlpha: Byte = $FF); overload;
procedure cxAlphaBlend(ADestDC: HDC; ASrcBitmap: TBitmap;
  const ADestRect, ASrcRect: TRect; ASmoothImage: Boolean = False;
  AConstantAlpha: Byte = $FF); overload;
procedure cxAlphaBlend(ADestDC, ASrcDC: HDC; const ADestRect, ASrcRect: TRect;
  ASmoothImage: Boolean = False; AConstantAlpha: Byte = $FF); overload;
procedure cxBitBlt(ADestDC, ASrcDC: HDC; const ADestRect: TRect;
  const ASrcTopLeft: TPoint; ROP: DWORD);
// procedure cxBitmapToTrueColorBitmap(ABitmap: TBitmap); -> cxMakeTrueColorBitmap
procedure cxBlendFunction(const ASource: TRGBQuad; var ADest: TRGBQuad;
  ASourceConstantAlpha: Byte);
function cxCreateBitmap(const ASize: TSize; AFormat: TPixelFormat = pf24bit)
  : TBitmap; overload;
function cxCreateBitmap(const ARect: TRect; AFormat: TPixelFormat = pf24bit)
  : TBitmap; overload;
function cxCreateBitmap(AWidth, AHeight: Integer;
  AFormat: TPixelFormat = pf24bit): TBitmap; overload;
...
function cxCreateTrueColorBitmapHandle(AWidth, AHeight: Integer;
  ABPP: Integer = 32): HBITMAP;
function cxCreateRegionFromBitmap(ABitmap: TBitmap; ATransparentColor: TColor)
  : HRGN; overload;
function cxCreateRegionFromBitmap(APixels: PRGBQuad; AWidth, AHeight: Integer;
  ATransparentColor: TColor): HRGN; overload;
procedure cxDrawHatchRect(ACanvas: TcxCanvas; const ARect: TRect;
  AColor: TColor);
procedure cxDrawDesignRect(ACanvas: TcxCanvas; const ARect: TRect;
  ASelected: Boolean);
...
procedure cxDrawImage(ADC: THandle; AGlyphRect, ABackgroundRect: TRect;
  AGlyph: TGraphic; AImages: TCustomImageList; AImageIndex: Integer;
  ADrawMode: TcxImageDrawMode; ASmoothImage: Boolean = False;
  ABrush: THandle = 0; ATransparentColor: TColor = clNone;
  AUseLeftBottomPixelAsTransparent: Boolean = True;
...
procedure cxDrawImage(ACanvas: TcxGdiBasedCanvas; const ARect: TRect;
  ...
  AUseLeftBottomPixelAsTransparent: Boolean = True;
  APalette: IdxColorPalette = nil; AScaleFactor: TdxScaleFactor = nil;
  ASmoothImage: Boolean = False); overload;
procedure cxDrawImage(ACanvas: TcxGdiBasedCanvas; const ARect: TRect;
  AGlyph: TGraphic; AImages: TCustomImageList; AImageIndex: Integer;
  AEnabled: Boolean; APalette: IdxColorPalette = nil;
  AScaleFactor: TdxScaleFactor = nil); overload;
procedure cxDrawImage(ACanvas: TcxGdiBasedCanvas; const ARect: TRect;
  AImage: TGraphic; AFitMode: TcxImageFitMode; APalette: IdxColorPalette = nil;
  AScaleFactor: TdxScaleFactor = nil; ASmoothResize: Boolean = False); overload;
procedure cxDrawImageList(AImageListHandle: HIMAGELIST; AImageIndex: Integer;
  ADC: HDC; APoint: TPoint; ADrawingStyle: TDrawingStyle;
...
procedure cxDrawHatch(ADC: HDC; const ARect: TRect; AColor1, AColor2: TColor;
  AStep: Byte; AAlpha1: Byte = $FF; AAlpha2: Byte = $FF);
procedure cxDrawTransparencyCheckerboard(DC: HDC; const ARect: TRect;
  ACellSize: Integer = 8); overload;
procedure cxDrawTransparencyCheckerboard(ACanvas: TcxCanvas; const ARect: TRect;
  ACellSize: Integer = 8); overload;
procedure cxTransformImage(ABitmap: TBitmap;
  AMode: TcxBitmapTransformationMode); overload;
procedure cxTransformImage(AImage: TdxSmartImage;
  AMode: TcxBitmapTransformationMode); overload;
function cxGetAsBitmap(AGraphic: TGraphic): TBitmap;
function cxGetImageRect(const ADrawRect: TRect; const AImageSize: TSize;
  AFitMode: TcxImageFitMode): TRect; overload;
function cxGetImageRect(const ADrawRect: TRect; const AImageSize: TSize;
  AFitMode: TcxImageFitMode; ACentre: Boolean; AScale: Integer): TRect;
  overload;

function cxPrepareBitmapForDrawing(AGlyph: TGraphic; AImages: TCustomImageList;
  AImageIndex: Integer; AUseLeftBottomPixelAsTransparent: Boolean;
  ATransparentColor: TColor; APalette: IdxColorPalette = nil;
  AGlyphWidth: Integer = 0; AGlyphHeight: Integer = 0): TcxAlphaBitmap;
  overload;
function cxPrepareBitmapForDrawing(AGlyph: TGraphic; AImages: TCustomImageList;
  AImageIndex: Integer; AUseLeftBottomPixelAsTransparent: Boolean;
  ATransparentColor: TColor; APalette: IdxColorPalette; AGlyphWidth: Integer;
  AGlyphHeight: Integer; ASmoothResize: Boolean;
  out AMaskBitmap: TcxAlphaBitmap; out AIsAlphaUsed: Boolean)
  : TcxAlphaBitmap; overload;

function cxSmoothDrawingNeeded(const ADest: TRect; AGraphic: TGraphic;
  AScaleFactor: TdxScaleFactor): Boolean;

procedure cxSmoothResizeBitmap(ASource, ADestination: TBitmap;
  AForceUseLanczos3Filter: Boolean = False);
procedure cxStretchBlt(ADestDC, ASrcDC: HDC; const ADestRect, ASrcRect: TRect;
  ROP: DWORD);
...
procedure cxStretchGraphic(ADestination: TBitmap; ASource: TGraphic;
  const ADestRect, ASourceRect: TRect; ASmoothStretch: Boolean = False;
  APalette: IdxColorPalette = nil); overload;
...
function SystemAlphaBlend(ADestDC, ASrcDC: HDC;
  const ADestRect, ASrcRect: TRect; AAlpha: Byte = $FF;
  AHasPerPixelAlpha: Boolean = True): Boolean;
procedure cxAlphaBlend(ASource: TBitmap; ARect: TRect; const ASelColor: TColor;
  Alpha: Byte = 170); overload;
procedure cxAlphaBlend(ADest, ABkSource, ASource: TBitmap;
  Alpha: Byte = cxDefaultAlphaValue); overload;
procedure cxApplyViewParams(ACanvas: TcxCanvas;
  const AViewParams: TcxViewParams);
procedure cxCopyImage(ASource, ADest: TBitmap;
  const ASrcOffset, ADstOffset: TPoint; const ARect: TRect); overload;
...
procedure cxFillHalfToneRect(Canvas: TCanvas; const ARect: TRect;
  ABkColor, AColor: TColor);
procedure cxFillRectWithCustomBrush(ACanvas: TCanvas; ABrush: TBrush;
  const ARect: TRect; ABkColor, AColor: TColor;
  ATransparentBackground: Boolean = False);

function cxGetTextExtentPoint32(ADC: THandle; const AText: string;
  out ASize: TSize; ACharCount: Integer = -1): Boolean;
...
function cxDrawText(ADC: THandle; const AText: string; var ARect: TRect;
  AFormat: UINT; ACharCount: Integer = -1; ATextColor: TColor = clDefault;
  ABkMode: Integer = TRANSPARENT): Integer; overload;
...
function cxDrawText(ACanvas: TcxCanvas; const AText: string; const ARect: TRect;
  AFormat: UINT; ATextColor: TColor = clDefault;
  ARotationAngle: TcxRotationAngle = ra0): Integer; overload;
function cxDrawMultilineText(ACanvas: TcxCanvas; const AText: string;
  const ARect: TRect; AFormat: UINT; ATextColor: TColor = clDefault)
  : Integer; overload;
function cxDrawMultilineText(ACanvas: TcxCanvas; const AText: string;
  const ARect: TRect; ATextAlignHorz: TAlignment = taLeftJustify;
  ATextAlignVert: TcxAlignmentVert = vaTop; ATextColor: TColor = clDefault)
  : Integer; overload;
function cxExtTextOut(ADC: THandle; const AText: string; const APoint: TPoint;
  const ARect: TRect; AOptions: UINT; ACharCount: Integer = -1)
  : Boolean; overload;
function cxExtTextOut(ADC: THandle; const AText: string; const APoint: TPoint;
...
function cxTextHeight(AFont: TFont; const S: string; AFontSize: Integer = 0)
  : Integer; overload; inline;
function cxTextHeight(AFont: TFont; const S: string; ARect: TRect;
  AFlags: Integer): Integer; overload; inline;
function cxTextWidth(AFont: TFont; const S: string; AFontSize: Integer = 0)
  : Integer; inline;
function cxTextExtent(AFontHandle: THandle; const S: string): TSize;
  overload; inline;
function cxTextExtent(AFont: TFont; const S: string; AFontSize: Integer = 0)
  : TSize; overload; inline;
function cxTextSize(ADC: THandle; const AText: string): TSize; overload; inline;
function cxTextSize(AFont: TFont; const AText: string; AFlags: Integer = 0)
  : TSize; overload; inline;
function cxGetTextRect(ADC: THandle; const AText: string; ARowCount: Integer;
  ...
  overload; inline;
function cxGetTextRect(AFont: TFont; const AText: string; ARowCount: Integer;
  AConsiderLineBreaks: Boolean = False): TRect; overload;
procedure cxGetTextRect(var ARect: TRect; const AText: string; AFont: TFont;
  ADTFlags: Integer); overload; inline;
function cxGetStringAdjustedToWidth(ADC: HDC; AFontHandle: HFONT;
  const S: string; AWidth: Integer;
  AModifyStringType: TcxModifyStringType = mstEndEllipsis): string;
  overload; inline;
function cxGetStringAdjustedToWidth(AFont: TFont; const S: string;
  AWidth: Integer; AModifyStringType: TcxModifyStringType = mstEndEllipsis)
...
function cxGetBitmapData(ABitmapHandle: HBITMAP;
  out ABitmapData: Windows.TBitmap): Boolean;
function cxGetImageClientRect(AImage: TGraphic): TRect; inline;
function cxGetBrushData(ABrushHandle: HBRUSH; out ALogBrush: TLogBrush)
  : Boolean; overload;
...
function cxGetWritingDirection(AFontCharset: TFontCharset; const AText: string)
  : TCanvasOrientation;
procedure cxDrawThemeParentBackground(AControl: TWinControl; ACanvas: TcxCanvas;
  const ARect: TRect); overload;
procedure cxDrawThemeParentBackground(AControl: TWinControl; ACanvas: TCanvas;
  const ARect: TRect); overload;

procedure cxDrawTransparentControlBackground(AControl: TWinControl;
  ACanvas: TcxCanvas; ARect: TRect;
  APaintParentWithChildren: Boolean = True); overload;
procedure cxDrawTransparentControlBackground(AControl: TWinControl;
  ACanvas: TCanvas; const ARect: TRect;
  APaintParentWithChildren: Boolean = True); overload;
procedure cxDrawTransparentControlBackground(AControl: TWinControl;
  ACanvas: TcxCanvas; const ASourceRect: TRect; const ADestinationPoint: TPoint;
  APaintParentWithChildren: Boolean = True); overload;
function cxIsTransparentBackgroundDrawing(ABackgroundControl
  : TWinControl): Boolean;

procedure cxRightToLeftDependentDraw(ACanvas: TcxCanvas; const R: TRect;
  AProc: TProc); overload;
procedure cxRightToLeftDependentDraw(DC: HDC; const R: TRect;
  AIsRightToLeftLayout: Boolean; AProc: TProc;
...
procedure cxPaintControlTo(ADrawingControl: TWinControl; ACanvas: TcxCanvas;
  const ADestinationPoint: TPoint; const ADrawingRect: TRect;
...
procedure cxPaintTo(ASourceControl: TWinControl; ADestinationCanvas: TcxCanvas;
  const ADestinationPoint: TPoint; const ASourceRect: TRect;
  ASkipList: TList = nil);
procedure cxPaintToBitmap(AControl: TWinControl; ABitmap: TcxBitmap;
  ADrawNCArea: Boolean = False);
...
procedure cxTransformImages(AImageList: TcxImageList; ABkColor: TColor;
  AEnabled: Boolean = True); overload;
procedure cxTransformImages(ASourceImageList, ADestinationImageList
  : TcxImageList; ABkColor: TColor; AEnabled: Boolean = True); overload;

procedure cxAdvancedDrawPopupMenuItem(AMenuItem: TMenuItem; ACanvas: TCanvas;
  ARect: TRect; AState: TOwnerDrawState;
  AExternalImages: TCustomImageList = nil);

procedure dxSetFontAsNonAntialiased(AFont: TFont);
procedure dxSetZoomFactor(ACanvas: TcxCanvas; AZoomFactor: Integer;
  var APrevTransform: XFORM);

function dxPictureToImage(APicture: TPicture; ASize: TSize;
  AMode: TcxImageFitMode; var AImage: TdxSmartImage): Boolean;
function dxImageFrameToGraphic(ASource: TdxSmartImage; AFrameIndex: Integer)
  : TdxSmartImage;

function cxRectExcludeBorders(const ABounds, ABordersWidth: TRect;
  ABorders: TcxBorders): TRect;

type
  TdxGetImageFrameProc = function(AIndex: Integer; const ASize: TSize;
    AMode: TcxImageFitMode; var AFrame: TdxSmartImage): Boolean of object;

function dxCreateMultiFrameTIFF(AFrameCount: Integer; const ASize: TSize;
  AMode: TcxImageFitMode; AProc: TdxGetImageFrameProc): TdxSmartImage;
...
function cxGraphicFilter(AGraphic: TGraphic; AForExport: Boolean = False)
  : string; overload;
...
function SetLayout(DC: HDC; Layout: DWORD): DWORD; stdcall;
...
{$ENDIF}
  ...
  cxControls, dxUxTheme, dxOffice11, dxThemeConsts, dxThemeManager,
  cxDrawTextUtils, dxSVGImage, dxDPIAwareUtils,
...
const
  ...
  DefaultBlendFunction: TBlendFunction = (BlendOp: AC_SRC_OVER; BlendFlags: 0;
    SourceConstantAlpha: cxDefaultAlphaValue; AlphaFormat: $0);
...
begin
  Result := cxPoint(cxGetValueCurrentDPI(AValue.X),
    cxGetValueCurrentDPI(AValue.Y));
end;
...
begin
  Result := cxSize(cxGetValueCurrentDPI(AValue.CX),
    cxGetValueCurrentDPI(AValue.CY));
end;
...
begin
  ...
  begin
    ...
    else if cxNoFullWidthCharBreak and Flags <> 0 then
...
function IsImageAssigned(AImageList: TCustomImageList;
  AImageIndex: Integer): Boolean;
begin
  Result := (AImageList <> nil) and (0 <= AImageIndex) and
    (AImageIndex < AImageList.Count);
end;
...
function dxGetImageSize(AImage: TGraphic; AImageList: TCustomImageList;
  AImageIndex: Integer; AScaleFactor: TdxScaleFactor): TSize;
begin
  if IsImageAssigned(AImage) then
    Result := AScaleFactor.Apply(dxGetImageSize(AImage),
      dxGetImageSourceDPI(AImage), dxDefaultDPI)
  else if IsImageAssigned(AImageList, AImageIndex) then
...
begin
  Result := dxGetImageSize(AImage, AImageList, AImageIndex,
    dxSystemScaleFactor);
end;
...
{$ENDIF}
      Result := AScaleFactor.Apply(Result, dxGetImageSourceDPI(AImageList),
        dxDefaultDPI);
  end
  else
    Result := cxNullSize;
...
function dxGetImageSize(APicture: TPicture;
  AScaleFactor: TdxScaleFactor): TSize;
begin
...
begin
  ...
  if Red > High(Byte) then
    Red := High(Byte);
  if Red < Low(Byte) then
    Red := Low(Byte);

  Green := GetGValue(AColor) + AGreen;
  if Green > High(Byte) then
    Green := High(Byte);
  if Green < Low(Byte) then
    Green := Low(Byte);

  Blue := GetBValue(AColor) + ABlue;
  if Blue > High(Byte) then
    Blue := High(Byte);
  if Blue < Low(Byte) then
    Blue := Low(Byte);
...
var
  ...
  function GetLightIndex(ABtnFaceValue, AHighlightValue,
    AWindowValue: Byte): Integer;
  begin
    Result := GetChannelValue(MulDiv(ABtnFaceValue, ABtnFaceColorPart, 100) +
      MulDiv(AHighlightValue, AHighlightColorPart, 100) + MulDiv(AWindowValue,
      AWindowColorPart, 100));
  end;
...
begin
  ...
  if ((ABtnFaceColor = 0) or (ABtnFaceColor = $FFFFFF)) and
    (AHighlightColor <> 0) then
    Result := AHighlightColor
  else
    Result := RGB(GetLightIndex(GetRValue(ABtnFaceColor),
      GetRValue(AHighlightColor), GetRValue(AWindowColor)),
      GetLightIndex(GetGValue(ABtnFaceColor), GetGValue(AHighlightColor),
      GetGValue(AWindowColor)), GetLightIndex(GetBValue(ABtnFaceColor),
      GetBValue(AHighlightColor), GetBValue(AWindowColor)));
end;
...
begin
  ...
  Result := RGB(CalcValue(GetRValue(AColor1), GetRValue(AColor2)),
...
function IsHighContrast2: Boolean;
begin
  Result := (GetSysColor(COLOR_BTNFACE) = 0) and
    (GetSysColor(COLOR_BTNTEXT) <> $FFFFFF); // #2
end;
...
procedure FillGradientRect(DC: HDC; const ARect: TRect;
  ...
  procedure SystemGradientFill(ARGBColor1, ARGBColor2: DWORD);
    ...
    procedure SetVertex(var AVertex: TTriVertex; const APoint: TPoint;
      ARGBColor: DWORD);
    begin
      AVertex.X := APoint.X;
      AVertex.Y := APoint.Y;
  ...
  const
    ModesMap: array [Boolean] of DWORD = (GRADIENT_FILL_RECT_V,
  ...
  begin
    ...
    GradientFill(DC, @AVertices[0], 2, @AGradientRect, 1,
      ModesMap[AHorizontal]);
  end;
...
begin
  ...
  else if Assigned(GradientFill) then
...
begin
  ...
  Result := CanApplySystemAlphaBlending and
    VCLAlphaBlend(ADestDC, ADestRect.Left, ADestRect.Top,
    cxRectWidth(ADestRect), cxRectHeight(ADestRect), ASrcDC, ASrcRect.Left,
    ASrcRect.Top, cxRectWidth(ASrcRect), cxRectHeight(ASrcRect),
    ABlendFunction);
end;
...
procedure CommonAlphaBlend(ADestDC, ASrcDC: HDC;
  const ADestRect, ASrcRect: TRect; ASmoothImage: Boolean;
  AConstantAlpha: Byte = MaxByte; ADestBitmap: TBitmap = nil;
  ASrcBitmap: TBitmap = nil);

  function CreateDirectBitmap(ASrcDC: HDC; ASrcBitmap: TBitmap;
    const ASrcRect: TRect): TBitmap;
  begin
    ...
    begin
      ...
      cxBitBlt(Result.Canvas.Handle, ASrcDC, cxGetImageClientRect(Result),
        ASrcRect.TopLeft, SRCCOPY);
    end;
  end;
  ...
  begin
    ...
    if not SystemAlphaBlend(ADestBitmap.Canvas.Handle, ASrcBitmap.Canvas.Handle,
      AClientRect, AClientRect, AConstantAlpha) then
  ...
  begin
    ...
    try
      ...
      begin
        ADirectDestBitmap := CreateDirectBitmap(ADestDC, ADestBitmap,
          ADestRect);
        try
          AStretchedSrcBitmap := TcxBitmap.CreateSize(ADestRect, pf32bit);
          try
            cxStretchGraphic(AStretchedSrcBitmap, ADirectSrcBitmap,
              ASmoothImage);
            InternalAlphaBlend(ADirectDestBitmap, AStretchedSrcBitmap);
            cxBitBlt(ADestDC, ADirectDestBitmap.Canvas.Handle, ADestRect,
              cxNullPoint, SRCCOPY);
          finally
...
begin
  ...
  begin
    ...
    if IsWin64Bit or IsWin9X or not CanApplySystemAlphaBlending or ASmoothImage
    then
      ComplexAlphaBlend
    else
...
procedure cxAlphaBlend(ADestBitmap, ASrcBitmap: TBitmap;
  ...
  AConstantAlpha: Byte = $FF); overload;
begin
  CommonAlphaBlend(ADestBitmap.Canvas.Handle, ASrcBitmap.Canvas.Handle,
    ADestRect, ASrcRect, ASmoothImage, AConstantAlpha, ADestBitmap, ASrcBitmap);
end;
...
procedure cxAlphaBlend(ADestDC: HDC; ASrcBitmap: TBitmap;
  ...
  AConstantAlpha: Byte = $FF); overload;
begin
  CommonAlphaBlend(ADestDC, ASrcBitmap.Canvas.Handle, ADestRect, ASrcRect,
    ASmoothImage, AConstantAlpha, nil, ASrcBitmap);
end;

procedure cxAlphaBlend(ADestDC, ASrcDC: HDC; const ADestRect, ASrcRect: TRect;
  ASmoothImage: Boolean = False; AConstantAlpha: Byte = $FF); overload;
begin
  CommonAlphaBlend(ADestDC, ASrcDC, ADestRect, ASrcRect, ASmoothImage,
    AConstantAlpha);
end;
...
procedure cxBitBlt(ADestDC, ASrcDC: HDC; const ADestRect: TRect;
  const ASrcTopLeft: TPoint; ROP: DWORD);
begin
  BitBlt(ADestDC, ADestRect.Left, ADestRect.Top, cxRectWidth(ADestRect),
    cxRectHeight(ADestRect), ASrcDC, ASrcTopLeft.X, ASrcTopLeft.Y, ROP);
end;
...
begin
  ...
  ADest.rgbGreen := GetValue(ASource.rgbGreen * ASCA + ASrcAlpha *
    ADest.rgbGreen);
  ADest.rgbBlue := GetValue(ASource.rgbBlue * ASCA + ASrcAlpha * ADest.rgbBlue);
  ADest.rgbReserved := GetValue(ASource.rgbReserved * ASCA + ASrcAlpha *
    ADest.rgbReserved);
end;

const
  ClrNone: TRGBQuad = (rgbBlue: $FF; rgbGreen: $FF; rgbRed: $FF;
    rgbReserved: $FF);
  ClrTransparent: TRGBQuad = (rgbBlue: 0; rgbGreen: 0; rgbRed: 0;
    rgbReserved: 0);
...
function cxCreateRegionFromBitmap(ABitmap: TBitmap;
  ATransparentColor: TColor): HRGN;
var
  AColors: TRGBColors;
begin
  GetBitmapBits(ABitmap, AColors, True);
  Result := cxCreateRegionFromBitmap(@AColors[0], ABitmap.Width, ABitmap.Height,
    ATransparentColor);
end;
...
function cxCreateRegionFromBitmap(APixels: PRGBQuad; AWidth, AHeight: Integer;
  ...
  procedure CombineRegion(X, Y: Integer; var ACount: Integer;
    var ACombined: HRGN);
  var
    ARgn: HRGN;
  begin
...
procedure cxSetBitmapParams(ABitmap: TBitmap; AWidth, AHeight: Integer;
  AFormat: TPixelFormat); inline;
begin
  ABitmap.PixelFormat := AFormat;
  ABitmap.SetSize(AWidth, AHeight);
end;

function cxCreateBitmap(const ASize: TSize;
  AFormat: TPixelFormat = pf24bit): TBitmap;
begin
  Result := cxCreateBitmap(ASize.CX, ASize.CY, AFormat);
end;

function cxCreateBitmap(const ARect: TRect;
  AFormat: TPixelFormat = pf24bit): TBitmap;
begin
  ...
  begin
    ...
    try
      ...
      for I := 1 to MulDiv(Max(ARect.Right, ARect.Bottom), 3, 2) do
        ACanvas.Polyline([Point(ARect.Right - I * 3, ARect.Top),
          Point(ARect.Right, ARect.Top + I * 3)]);
...
procedure cxDrawBitmap(ADestDC: THandle; ASrcBitmap: TBitmap;
  ...
  procedure InternalDrawBitmap;
    ...
    begin
      ...
      try
        cxBitBlt(ABitmap.Canvas.Handle, ASrcBitmap.Canvas.Handle,
          ABitmap.ClientRect, ASrcPoint, SRCCOPY);
  ...
  begin
    if IsWin9X and (GetDeviceCaps(ADestDC, BITSPIXEL) = 32) and
      (cxGetBitmapPixelFormat(ASrcBitmap) < 32) then
      SoftwareBitBlt
    else
...
procedure cxDrawImage(ADC: THandle; AGlyphRect, ABackgroundRect: TRect;
  ...
  ABrush: THandle = 0; ATransparentColor: TColor = clNone;
  AUseLeftBottomPixelAsTransparent: Boolean = True;
  APalette: IdxColorPalette = nil);

  procedure DrawBackground(ABitmap: TcxAlphaBitmap);
  begin
    if ABrush = 0 then
      cxBitBlt(ABitmap.Canvas.Handle, ADC, ABitmap.ClientRect,
        ABackgroundRect.TopLeft, SRCCOPY)
    else
  ...
  begin
    ...
    if not Assigned(CustomDrawImageProc) or
      not CustomDrawImageProc(ABitmap.Canvas, AImages, AImageIndex, AGlyph,
      AGlyphRect, ADrawMode) then
    begin
      AImageBitmap := cxPrepareBitmapForDrawing(AGlyph, AImages, AImageIndex,
        AUseLeftBottomPixelAsTransparent, ATransparentColor, APalette,
        cxRectWidth(AGlyphRect), cxRectHeight(AGlyphRect), ASmoothImage,
        AMaskBitmap, AIsAlphaUsed);
      ...
      case ADrawMode of
        ...
        idmShadowed:
          begin
            ...
            begin
              AMaskBitmap := GetMaskBitmap(cxRectWidth(AGlyphRect),
                cxRectHeight(AGlyphRect));
            ...
            AImageBitmap.DrawShadow(AMaskBitmap, ImageShadowSize,
              clBtnShadow, True);
            AGlyphRect := cxRectInflate(AGlyphRect, 0, 0, ImageShadowSize,
              ImageShadowSize);
            OffsetRect(AGlyphRect, -ImageShadowSize div 2,
              -ImageShadowSize div 2);
          end;
...
begin
  ...
  begin
    ADrawBitmap := GetDrawBitmap(cxRectWidth(ABackgroundRect),
      cxRectHeight(ABackgroundRect));
...
function cxGetImageRect(const ADrawRect: TRect; const AImageSize: TSize;
  AFitMode: TcxImageFitMode): TRect;
begin
...
begin
  ...
  try
    ...
    case AFitMode of
      ...
      AGlyphSize := Size(MulDiv(AImageSize.CX, AScale, 100),
        MulDiv(AImageSize.CY, AScale, 100));
    end;
  finally
  ...
  begin
    ...
    if not IsGlyphAssigned(AGlyph) and (AImages is TcxCustomImageList) then
      ...
      cxDrawImage(ACanvas.Handle, AGlyphRect, AGlyphRect, AGlyph, AImages,
        AImageIndex, ADrawMode, ASmoothImage, 0, clNone,
...
procedure cxDrawImage(ACanvas: TcxGdiBasedCanvas; const ARect: TRect;
  ...
  AScaleFactor: TdxScaleFactor = nil);
begin
  cxDrawImage(ACanvas, ARect, AGlyph, AImages, AImageIndex, ifmNormal,
    EnabledImageDrawModeMap[AEnabled], True, APalette, AScaleFactor);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if AScaleFactor = nil then
        AScaleFactor := dxDefaultScaleFactor;
      cxDrawImage(ACanvas, ARect, APicture.Graphic, nil, -1, AFitMode,
        idmNormal, (APicture.Graphic is TBitmap) and TBitmap(APicture.Graphic)
        .TRANSPARENT, nil, AScaleFactor, cxSmoothDrawingNeeded(ARect,
        APicture.Graphic, AScaleFactor));
    finally
...
procedure cxDrawImage(ACanvas: TcxGdiBasedCanvas; const ARect: TRect;
  AImage: TGraphic; AFitMode: TcxImageFitMode; APalette: IdxColorPalette = nil;
  AScaleFactor: TdxScaleFactor = nil; ASmoothResize: Boolean = False);
begin
  cxDrawImage(ACanvas, ARect, AImage, nil, -1, AFitMode, idmNormal,
    AImage.TRANSPARENT and not(AImage is TMetaFile), APalette, AScaleFactor,
    ASmoothResize);
end;
...
procedure cxDrawImageList(AImageListHandle: HIMAGELIST; AImageIndex: Integer;
  ...
  AImageType: TImageType);
begin
  ImageList_Draw(AImageListHandle, AImageIndex, ADC, APoint.X, APoint.Y,
    cxGetImageListStyle(ADrawingStyle, AImageType));
end;
...
begin
  ...
  try
    cxBitBlt(ADrawBitmap.Canvas.Handle, ADC, ADrawBitmap.ClientRect,
...
procedure cxTransformImage(ABitmap: TBitmap;
  AMode: TcxBitmapTransformationMode);
var
...
begin
  Result := cxPrepareBitmapForDrawing(AGlyph, AImages, AImageIndex,
    AUseLeftBottomPixelAsTransparent, ATransparentColor, APalette, AGlyphWidth,
    AGlyphHeight, True, AMask, AIsAlphaUsed);
end;
...
function cxPrepareBitmapForDrawing(AGlyph: TGraphic; AImages: TCustomImageList;
  ...
  begin
    ...
    else if (AImages <> nil) and
      (ASizeIsEmpty or not(AImages is TcxCustomImageList)) then
    begin
  ...
  begin
    ...
    begin
      if TcxImageList.GetPixelFormat(AImages.Handle) < 32 then
        AHasAlpha := bFalse;
      TcxImageListAccess.GetImageInfoCore(AImages, AImageIndex, AImageBitmap,
        nil, APalette, AHasAlpha);
    end;
  ...
  begin
    Result := not AIsAlphaUsed or AUseLeftBottomPixelAsTransparent and
      IsGlyphAssigned(AGlyph) and (AGlyph is TdxSmartGlyph) and
      not TdxSmartGlyphAccess(AGlyph).FTransparent and
      (TdxSmartGlyphAccess(AGlyph).AlphaState = asOpaque);
  end;

  function MakeMask(AImageBitmap, AMaskBitmap: TcxAlphaBitmap;
    AIsAlphaUsed: Boolean): TcxAlphaBitmap;
  var
...
procedure BuildFilter(out AContributorList: TContributorList; AScale: Single;
  ASrcSize, ADestSize: Integer);
var
...
begin
  ...
  for I := 0 to ADestSize - 1 do
    ...
    begin
      ...
      begin
        AWeight := Round(Lanczos3Filter((ACenter - J) / AScaleFactor) /
          AScaleFactor * 256);
        if AWeight = 0 then
          continue;
        if J < 0 then
          APixel := -J
        else if (J >= ASrcSize) then
...
begin
  ...
  if (ADestWidth = 0) or (ADestHeight = 0) or (ASrcWidth = 0) or (ASrcHeight = 0)
  then
    Exit;
  ASource.Canvas.Lock;
  ...
  try
    ...
    begin
      ...
      ApplyFilter(AContributorList, ABuffer1, ASrcHeight, ASrcWidth, ABuffer2,
        ADestWidth, ADestWidth, True);
      ...
      ApplyFilter(AContributorList, ABuffer2, ADestWidth, ADestWidth, ABuffer1,
        ADestHeight, ADestWidth, False);
...
begin
  StretchBlt(ADestDC, ADestRect.Left, ADestRect.Top,
    ADestRect.Right - ADestRect.Left, ADestRect.Bottom - ADestRect.Top, ASrcDC,
    ASrcRect.Left, ASrcRect.Top, ASrcRect.Right - ASrcRect.Left,
    ASrcRect.Bottom - ASrcRect.Top, ROP);
end;

procedure cxStretchGraphic(ADestination: TBitmap; ASource: TGraphic;
  ASmoothStretch: Boolean; APalette: IdxColorPalette);
begin
  cxStretchGraphic(ADestination, ASource, cxGetImageClientRect(ADestination),
    cxGetImageClientRect(ASource), ASmoothStretch, APalette);
end;

procedure cxStretchGraphic(ADestination: TBitmap; ASource: TGraphic;
  const ADestRect, ASourceRect: TRect; ASmoothStretch: Boolean;
  ...
  begin
    ...
    begin
      if cxRectIsEqual(ADestRect, cxGetImageClientRect(ADestination)) and
        cxRectIsEqual(ASourceRect, cxGetImageClientRect(ASource)) then
      ...
      begin
        ...
        try
          ATempSrc.CopyBitmap(ASource, ATempSrc.ClientRect,
            ASourceRect.TopLeft);
          cxSmoothResizeBitmap(ATempSrc, ATempDst, True);
          cxBitBlt(ADestination.Canvas.Handle, ATempDst.Canvas.Handle,
            ADestRect, cxNullPoint, SRCCOPY);
        finally
          ATempSrc.Free;
          ATempDst.Free;
        end;
    ...
    else if AIsSizeEqual then
      cxBitBlt(ADestination.Canvas.Handle, ASource.Canvas.Handle, ADestRect,
        ASourceRect.TopLeft, SRCCOPY)
    else
      cxStretchBlt(ADestination.Canvas.Handle, ASource.Canvas.Handle, ADestRect,
        ASourceRect, SRCCOPY);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      AGraphics.PixelOffsetMode := PixelOffsetModeHalf;
      TdxSmartImage(ASource).StretchDraw(AGraphics, ADestRect, ASourceRect, nil,
        APalette);
    finally
      dxGpEndPaint(AGraphics);
    end;
    ...
    TdxCustomSmartImage(ASource).StretchDraw(ADestination.Canvas.Handle,
      ADestRect, ASourceRect, MaxByte, APalette)
  else
  begin
...
begin
  Result := TcxAlphaBitmap.CreateSize(ASourceBitmap.Width,
    ASourceBitmap.Height);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        CalculateValue(APixel^.rgbGreen, AColorValue.rgbGreen,
          APixel^.rgbReserved);
        CalculateValue(APixel^.rgbBlue, AColorValue.rgbBlue,
          APixel^.rgbReserved);
      end;
      Inc(APixel);
    end;
...
begin
  ...
  try
    ...
    cxBitBlt(AMaskBitmap.Canvas.Handle, ABitmap.Canvas.Handle,
      ABitmap.ClientRect, cxNullPoint, SRCCOPY);
  finally
    ABitmap.Free;
  end;
end;
        ...
        begin
          ...
          begin
            ...
            if (Result.CY = 0) and (biHeight >= 2 * biWidth) then
              Result.CY :=
                FindScanline(Pointer(TdxNativeUInt(Bits) - ImageSize),
                ImageSize, $00);
...
begin
  ...
  begin
    ...
    begin
      WriteByte(SrcLine, AColorValues[ACol mod 4] + Round(ReadByte(SrcLine,
        ACol) * C2), ACol);
      Inc(ACol);
    end;
  end;
...
procedure cxAlphaBlend(ADest, ABkSource, ASource: TBitmap;
  Alpha: Byte = cxDefaultAlphaValue);
  ...
  begin
    ...
    begin
      ...
      with ADest do
      begin
        Canvas.Draw(0, 0, ABkSource);
        // Assign(ABkSource); todo: graphics bug image not copying but _AddRef called
        Result := VCLAlphaBlend(Canvas.Handle, 0, 0, Width, Height,
          ASource.Canvas.Handle, 0, 0, Width, Height, ABlendFunction);
      end;
    end
    else
      Result := False;
  ...
  begin
    ...
    for K := 0 to ACount - 1 do
      WriteByte(DstLine, Round(ReadByte(SrcLine, K) * C1) +
        Round(ReadByte(BkSrcLine, K) * C2), K);
  end;
...
function cxCopyImage(ASrcHandle: HBITMAP): HBITMAP; overload;
  ...
  begin
    ...
    begin
      dxFillBitmapInfoHeader(ABitmapInfo.bmiHeader, ABitmapData.bmWidth,
        ABitmapData.bmHeight, False);
      if ABitmapData.bmBits = nil then
      begin
        SetLength(ABits, ABitmapData.bmWidth * ABitmapData.bmHeight * 4);
        GetDIBits(cxScreenCanvas.Handle, ASrcHandle, 0, ABitmapData.bmHeight,
          ABits, ABitmapInfo, 0);
        cxScreenCanvas.Dormant;
        ABitmapData.bmBits := ABits;
      end;
      Result := CreateDIBSection(0, ABitmapInfo, DIB_RGB_COLORS,
        ADestinationBits, 0, 0);
      cxCopyData(ABitmapData.bmBits, ADestinationBits,
        ABitmapData.bmWidth * ABitmapData.bmHeight * 4);
    end
    else
...
procedure cxClearBitmap(ABitmap: TBitmap);
begin
  FillRect(ABitmap.Canvas.Handle, cxGetImageClientRect(ABitmap),
...
procedure cxFillRectWithCustomBrush(ACanvas: TCanvas; ABrush: TBrush;
  ...
  procedure DoFillRect(ACanvas: TCanvas; const ARect: TRect;
...
procedure cxGetTextLines(const AText: string; ACanvas: TcxCanvas;
  ...
  procedure GetNextWordPos(const S: string; ALength: Integer;
    const ACurrentWord: TcxWordPosition; var ANextWord: TcxWordPosition);
    ...
    begin
      Result := (AIndex > 1) and IsDelimiter(AIndex) and
        IsDelimiter(AIndex - 1);
    end;
  ...
  begin
    ...
    while (ANextWord.Start < ALength) and not IsWordStart(ANextWord.Start) and
      not IsDoubleDelimiter(ANextWord.Start) do
      Inc(ANextWord.Start);
    ACharPos := ANextWord.Start;
    while (ACharPos + 1 <= ALength) and not IsWordEnd(ACharPos + 1) and
      not IsDoubleDelimiter(ACharPos) do
      Inc(ACharPos);
    ANextWord.Finish := ACharPos;
  end;

  procedure FindNextWordStart(const S: string; var ACharIndex: Integer;
    var ANextWord: TcxWordPosition);
  var
  ...
  function CheckNewLineSymbol(const S: string; var ANextWord: TcxWordPosition;
    var ALineStartIndex: Integer): Boolean;
  var
    I: Integer;
  begin
    Result := False;
  ...
  repeat
    ...
    if not CheckNewLineSymbol(AText, ANextWord, ALineStart) then
      ...
      begin
        ...
        begin
          ALines.Add(Copy(AText, ALineStart, ACurrentWord.Finish -
            ALineStart + 1));
...
function cxDrawText(ADC: THandle; const AText: string; var ARect: TRect;
  AFormat: UINT; ACharCount: Integer = -1; ATextColor: TColor = clDefault;
  ABkMode: Integer = TRANSPARENT): Integer;
var
...
function cxDrawText(ACanvas: TcxCanvas; const AText: string; const ARect: TRect;
  AFormat: UINT; ATextColor: TColor = clDefault;
  ARotationAngle: TcxRotationAngle = ra0): Integer;
var
  ABitmap: TcxBitmap;
begin
  ...
  begin
    ...
    try
      ...
      Result := cxDrawText(ABitmap.Canvas, AText, ABitmap.ClientRect, AFormat,
        ATextColor);
...
begin
  ...
  try
    ...
    if AFormat and DT_VCENTER = DT_VCENTER then
      AOffset := Max(ARect.Top, cxRectCenterVertically(ARect,
        ALineHeight * Result).Top)
    else if AFormat and DT_BOTTOM = DT_BOTTOM then
    ...
    begin
      cxDrawText(ACanvas.Handle, ALines[I], ALineRect, AFormat or DT_SINGLELINE,
        -1, ATextColor);
...
function cxDrawMultilineText(ACanvas: TcxCanvas; const AText: string;
  const ARect: TRect; ATextAlignHorz: TAlignment = taLeftJustify;
  ATextAlignVert: TcxAlignmentVert = vaTop;
  ATextColor: TColor = clDefault): Integer;
const
  TextAlignHorzMap: array [TAlignment] of Integer = (DT_LEFT, DT_RIGHT,
    DT_CENTER);
  TextAlignVertMap: array [TcxAlignmentVert] of Integer = (DT_TOP, DT_BOTTOM,
    DT_VCENTER);
begin
  Result := cxDrawMultilineText(ACanvas, AText, ARect,
    TextAlignHorzMap[ATextAlignHorz] or TextAlignVertMap[ATextAlignVert],
    ATextColor);
end;
...
begin
  if ACharCount = -1 then
    ACharCount := Length(AText);
  Result := ExtTextOut(ADC, APoint.X, APoint.Y, AOptions, @ARect, PChar(AText),
    ACharCount, nil);
end;
...
begin
  if ACharCount = -1 then
    ACharCount := Length(AText);
  Result := ExtTextOut(ADC, APoint.X, APoint.Y, AOptions, nil, PChar(AText),
    ACharCount, nil);
end;
...
function cxTextHeight(AFont: TFont; const S: string;
  AFontSize: Integer = 0): Integer;
begin
  Result := TdxTextMeasurer.TextHeightTO(AFont, S, AFontSize);
end;

function cxTextWidth(AFont: TFont; const S: string;
  AFontSize: Integer = 0): Integer;
begin
  Result := TdxTextMeasurer.TextWidthTO(AFont, S, AFontSize);
end;

function cxTextExtent(AFont: TFont; const S: string;
  AFontSize: Integer = 0): TSize;
begin
...
function cxGetTextRect(ADC: THandle; const AText: string; ARowCount: Integer;
  AReturnMaxRectHeight: Boolean = False; ADTFlags: Integer = 0): TRect;
begin
  Result := TdxTextMeasurer.TextRectDT(ADC, AText, ARowCount,
    AReturnMaxRectHeight, ADTFlags);
end;

function cxGetTextRect(AFont: TFont; const AText: string; ARowCount: Integer;
  AConsiderLineBreaks: Boolean): TRect;
begin
  Result := TdxTextMeasurer.TextRectDT(AFont, AText, ARowCount,
    AConsiderLineBreaks);
end;
...
function cxGetStringAdjustedToWidth(ADC: HDC; AFontHandle: HFONT;
  const S: string; AWidth: Integer; AModifyStringType: TcxModifyStringType)
  : string; overload;
begin
  Result := TdxTextMeasurer.GetStringAdjustedToWidthDT(ADC, AFontHandle, S,
    AWidth, AModifyStringType);
end;

function cxGetStringAdjustedToWidth(AFont: TFont; const S: string;
  AWidth: Integer; AModifyStringType: TcxModifyStringType): string; overload;
begin
  Result := TdxTextMeasurer.GetStringAdjustedToWidthDT(AFont, S, AWidth,
    AModifyStringType);
end;
...
begin
  ...
  Result := (ABitmap1 <> nil) and (ABitmap2 <> nil) and
    ...
    and (cxGetBitmapPixelFormat(ABitmap1) = cxGetBitmapPixelFormat(ABitmap2));
  if Result and not ABitmap1.Empty then
  begin
    AHeight := ABitmap2.Height;
    APixelFormat := cxGetBitmapPixelFormat(ABitmap2);
    Result := CompareMem(ABitmap1.ScanLine[AHeight - 1],
      ABitmap2.ScanLine[AHeight - 1], ABitmap2.Height * ABitmap2.Width *
      APixelFormat div 8);
...
begin
  ...
  Result := (AGraphic1 <> nil) and (AGraphic2 <> nil) and
    (AGraphic1.Width = AGraphic2.Width) and
    (AGraphic1.Height = AGraphic2.Height);
...
function cxGetBitmapData(ABitmapHandle: HBITMAP;
  out ABitmapData: Windows.TBitmap): Boolean;
begin
  Result := GetObject(ABitmapHandle, SizeOf(Windows.TBitmap),
    @ABitmapData) <> 0;
end;
...
begin
  ...
  if not IsStandardASCIIChar and
    (AFontCharset in [ARABIC_CHARSET, CHINESEBIG5_CHARSET, GB2312_CHARSET]) then
...
function GetChildWindowsProc(AHandle: THandle; AInfo: PcxChildInfo)
  : BOOL; stdcall;
begin
  Result := True;
...
function cxGetPrintClientOffset(AWindow: THandle;
  const AWindowRect, AClientRect: TRect): TPoint;
var
  P: TPoint;
begin
...
procedure cxPrintWindow(AWindow, ADC: THandle;
  ADrawNC, ADrawChildren: Boolean); overload;

  function HasNonClientArea(AWindow: THandle;
    var AWindowRect, AClientRect: TRect): Boolean;
  begin
  ...
  procedure DoPrintWindow(AWindow, ADC: THandle; AFlags: DWORD;
    const AClientOffset: TPoint);
  var
    AOrigin: TPoint;
  begin
    GetWindowOrgEx(ADC, AOrigin);
    SetWindowOrgEx(ADC, AOrigin.X - AClientOffset.X,
      AOrigin.Y - AClientOffset.Y, nil);
  ...
  procedure DoPrintChildren(AWindow, ADC: THandle; AFlags: DWORD;
    const AWindowRect: TRect);
  var
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          AOrigin := cxPointOffset(cxGetWindowRect(AChildWindow).TopLeft,
            AWindowRect.TopLeft, False);
...
begin
  ...
  begin
    ...
    AMemBmp := CreateCompatibleBitmap(ADC, cxRectWidth(AWindowRect),
      cxRectHeight(AWindowRect));
    try
      SelectObject(AMemDC, AMemBmp);

      AControl := FindControl(AWindow);
      if AControl <> nil then
        FillRectByColor(AMemDC, cxRectSetNullOrigin(AWindowRect),
          TWinControlAccess(AControl).Color);

      AClientOffset := cxGetPrintClientOffset(AWindow, AWindowRect,
        AClientRect);
      ...
      cxBitBlt(ADC, AMemDC, cxRectSetNullOrigin(AWindowRect),
        cxNullPoint, SRCCOPY);
    finally
  ...
  begin
    ...
    DoPrintWindow(AWindow, ADC, AFlags, cxGetPrintClientOffset(AWindow,
      AWindowRect, AClientRect));
  end;
end;

procedure cxPrintWindow(AControl: TWinControl; ACanvas: TCanvas;
  ADrawNC: Boolean); overload;
begin
  ACanvas.Lock;
...
procedure cxPaintControlTo(ADrawingControl: TWinControl; ACanvas: TcxCanvas;
  ...
  begin
    ...
    try
      if GetWindowRgn(ADrawingControl.Handle, AWindowRegion.Handle)
        in [NULLREGION, SIMPLEREGION, COMPLEXREGION] then
  ...
  begin
    ...
    ADrawingRect := dxMapWindowRect(AChildControl.Handle, AParentControl.Handle,
      AControlRect, False);
    if RectVisible(ACanvas.Handle, ADrawingRect) then
      cxPaintControlTo(AChildControl, ACanvas, ADrawingRect.TopLeft,
        AControlRect, ADrawParentWithChildren, True);
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (AChildControl is TWinControl) and TWinControl(AChildControl).HandleAllocated
        then
        begin
          AChilds.Remove(Pointer(TWinControl(AChildControl).Handle));
          if TWinControl(AChildControl).Visible and
            (PaintSkipList.IndexOf(Pointer(TWinControl(AChildControl).Handle)
            ) < 0) then
      ...
      begin
        ...
        if (PaintSkipList.IndexOf(Pointer(AChildWndHandle)) < 0) and
          (not ACheckVisibility or IsWindowVisible(AChildWndHandle)) then
...
var
  ...
  P: TPoint;
begin
  if (csDestroying in ADrawingControl.ComponentState) or not ADrawingControl.HandleAllocated
  then
    Exit;
  ...
  try
    ...
    if not RectVisible(ACanvas.Handle, ADrawingRect) then
      Exit;

    ASaveControlState := ADrawingControl.ControlState;
    ADrawingControl.ControlState := ADrawingControl.ControlState +
      [csPaintCopy];
    try
      ACanvas.SaveState;
      try
      ...
      begin
        if ADrawNC and GetWindowRect(ADrawingControl.Handle, AWindowRect) and
          GetClientRect(ADrawingControl.Handle, AClientRect) and
          not cxSizeIsEqual(AWindowRect, AClientRect) then
        begin
          AClientRect := cxRectOffset(AClientRect,
            cxGetPrintClientOffset(ADrawingControl.Handle, AWindowRect,
            AClientRect));
          if (cxRectWidth(AClientRect) <> ADrawingControl.Width) or
            (cxRectHeight(AClientRect) <> ADrawingControl.Height) then
...
begin
  cxDrawTransparentControlBackground(AControl, ACanvas, ARect, ARect.TopLeft,
...
begin
  AcxCanvas := TcxCanvas.Create(ACanvas);
  try
    cxDrawTransparentControlBackground(AControl, AcxCanvas, ARect,
      ARect.TopLeft, APaintParentWithChildren);
  finally
    AcxCanvas.Free;
  end;
end;
...
begin
  if (AControl <> nil) and (AControl.Parent <> nil) and
    not cxIsTransparentBackgroundDrawing(AControl.Parent) then
  begin
    ARect := dxMapWindowRect(AControl.Handle, AControl.Parent.Handle,
      ASourceRect, False);
    BeginDrawTransparentBackground(AControl);
    try
{$IFDEF DELPHIXE2}
      AStyle := StyleServices;
      if AStyle.Enabled and not AStyle.IsSystemStyle then
        AStyle.DrawParentBackground(AControl.Handle, ACanvas.Handle, nil,
          False, ARect)
      else
{$ENDIF}
      begin
        ...
        try
          cxPaintControlTo(AControl.Parent, ACanvas, ADestinationPoint, ARect,
            APaintParentWithChildren, False);
        finally
...
procedure cxRightToLeftDependentDraw(ACanvas: TcxCanvas; const R: TRect;
  AProc: TProc);
begin
  cxRightToLeftDependentDraw(ACanvas.Handle, R,
    ACanvas.UseRightToLeftAlignment, AProc);
end;

procedure cxRightToLeftDependentDraw(DC: HDC; const R: TRect;
  AIsRightToLeftLayout: Boolean; AProc: TProc; ACorrectNeeded: Boolean = True);
var
  APrevMode: Integer;
  APrevXForm: TXForm;
begin
  ...
  begin
    ...
    try
      ModifyWorldTransform(DC, TXForm.CreateFlip(True, False,
        (R.Left + R.Right - IfThen(ACorrectNeeded, 1)) / 2, 0),
        MWT_LEFTMULTIPLY);
      AProc;
    finally
...
begin
  ...
  cxPaintControlTo(ASourceControl, ADestinationCanvas, ADestinationPoint,
    ASourceRect, True, True);
  if ASkipList <> nil then
...
begin
  ...
  begin
    ...
    cxPaintTo(AControl, ABitmap.cxCanvas, cxPointInvert(AOffset),
      cxGetWindowBounds(AControl));
  end;
end;
...
constructor TcxRegion.CreateRoundCorners(const ABounds: TRect;
  AWidthEllipse, AHeightEllipse: Integer);
begin
  CreateRoundCorners(ABounds.Left, ABounds.Top, ABounds.Right, ABounds.Bottom,
    AWidthEllipse, AHeightEllipse);
end;

constructor TcxRegion.CreateRoundCorners(ALeft, ATop, ARight, ABottom,
  AWidthEllipse, AHeightEllipse: Integer);
begin
  Create(CreateRoundRectRgn(ALeft + 1, ATop + 1, ARight, ABottom, AWidthEllipse,
    AHeightEllipse));
end;
...
procedure TcxRegion.Combine(ARegionHandle: TcxRegionHandle;
  AOperation: TcxRegionOperation);
const
  RegionOperationMap: array [TcxRegionOperation] of Integer = (RGN_COPY, RGN_OR,
    RGN_DIFF, RGN_AND);
begin
...
begin
  Result := (ARegion <> nil) and ((IsEmpty and ARegion.IsEmpty) or
    IsEqual(ARegion.Handle));
end;
...
begin
  Result := (Canvas is TControlCanvas) and TControlCanvas(Canvas)
...
procedure TcxCanvas.SynchronizeObjects(ACanvas: TCanvas; ADC: THandle);
  ...
  begin
    ...
    ACanvas.Brush.Color := ALogBrush.lbColor;
    // required: set Color before Style
  ...
  procedure AssignPen;
    ...
    const
      PenModes: array [TPenMode] of Integer = (R2_BLACK, R2_WHITE, R2_NOP,
        R2_NOT, R2_COPYPEN, R2_NOTCOPYPEN, R2_MERGEPENNOT, R2_MASKPENNOT,
        R2_MERGENOTPEN, R2_MASKNOTPEN, R2_MERGEPEN, R2_NOTMERGEPEN, R2_MASKPEN,
...
begin
  ...
  begin
    ...
    ASizeR.Bottom := Max(ARowHeight,
      ((R.Bottom - R.Top - Ord(not AEnabled)) div ARowHeight) * ARowHeight);
  end
  else
...
procedure TcxCanvas.DrawFocusRectEx(const R: TRect; ABorders: TcxBorders;
  AExclude: Boolean = False);
...
procedure TcxCanvas.DrawGlyph(X, Y: Integer; AGlyph: TBitmap;
  AEnabled: Boolean = True; ABackgroundColor: TColor = clNone);
var
...
var
  ...
  begin
    ...
    begin
      if (Flags and cxSingleLine = 0) and
        (Flags and (cxAlignBottom or cxAlignVCenter) <> 0) and
        (ARotationAngle = ra0) then
      begin
        ...
        AlignMultiLineTextRectVertically(R, Text, AAlignmentVert,
          ...
          (cxShowEndEllipsis or cxShowWordEllipsis) and Flags <> 0);
      end;
...
begin
  ...
  AUseDrawText := (Flags and cxSingleLine = 0) or (ARotationAngle <> ra0) or
    (Flags and (cxShowPrefix or cxShowEndEllipsis or cxShowPathEllipsis or
    cxShowWordEllipsis) <> 0);
  ProcessFlags;
...
begin
  if Text = '' then
    Exit;
  DrawTexT(Text, R, cxAlignmentsHorz[AAlignmentHorz] or
...
procedure TcxCanvas.FillRect(const R: TRect; const AParams: TcxViewParams;
  AExcludeRect: Boolean = False);
begin
...
procedure TcxCanvas.DrawDesignSelection(ARect: TRect;
  AWidth: Integer = cxDesignSelectionWidth);
var
  I: Integer;
begin
...
procedure TcxCanvas.DrawRegion(ARegion: TcxRegion;
  AContentColor: TColor = clDefault; ABorderColor: TColor = clDefault;
  ABorderWidth: Integer = 1; ABorderHeight: Integer = 1);
begin
  DrawRegion(ARegion.Handle, AContentColor, ABorderColor, ABorderWidth,
    ABorderHeight);
end;

procedure TcxCanvas.DrawRegion(ARegion: TcxRegionHandle;
  AContentColor: TColor = clDefault; ABorderColor: TColor = clDefault;
  ABorderWidth: Integer = 1; ABorderHeight: Integer = 1);
begin
...
procedure TcxCanvas.FillRegion(ARegion: TcxRegionHandle;
...
procedure TcxCanvas.FloodFill(X, Y: Integer; Color: TColor;
...
procedure TcxCanvas.FrameRegion(ARegion: TcxRegionHandle;
  AColor: TColor = clDefault; ABorderWidth: Integer = 1;
  ABorderHeight: Integer = 1);
begin
  if AColor <> clNone then
  begin
    SetBrushColor(AColor);
...
begin
  with R do
    Canvas.Pie(Left, Top, Right, Bottom, ARadial1.X, ARadial1.Y, ARadial2.X,
      ARadial2.Y);
end;
...

{
  ...
  function CalculateRadial(A, B: Integer; const ACenter: TPoint;
    AnAngle: Integer): TPoint;
  var
    Sin, Cos, V: Extended;
  begin
...
begin
  if IsRectEmpty(R) or (ASweepAngle = 0) then
    Exit;
  with R do
  begin
    A := (Right - Left) div 2;
  ...
  if (Radial1.X <> Radial2.X) or (Radial1.Y <> Radial2.Y) or (ASweepAngle > 180)
  then
...
begin
  if IsRectEmpty(R) or (Color = clNone) then
    Exit;
  ...
  begin
    if bLeft in ABorders then
      FillRect(Rect(Left, Top, Min(Left + ALineWidth, Right), Bottom), nil,
        AExcludeFrame);
    if bRight in ABorders then
      FillRect(Rect(Max(Right - ALineWidth, Left), Top, Right, Bottom), nil,
        AExcludeFrame);
    if bTop in ABorders then
      FillRect(Rect(Left, Top, Right, Min(Top + ALineWidth, Bottom)), nil,
        AExcludeFrame);
    if bBottom in ABorders then
      FillRect(Rect(Left, Max(Bottom - ALineWidth, Top), Right, Bottom), nil,
        AExcludeFrame);
  end;
end;
...
begin
  ...
  begin
    ...
    InvertRect(Rect(Left + ABorderSize, Top, Right - ABorderSize,
      Top + ABorderSize));
...
procedure TcxCanvas.Polyline(const Points: array of TPoint; AColor: TColor;
  APenWidth: Integer = 1; APenStyle: TPenStyle = psSolid);
begin
  Pen.Style := APenStyle;
  Pen.Color := AColor;
...
procedure TcxCanvas.Rectangle(const R: TRect; ABrushColor: TColor;
...
procedure TcxCanvas.RotateBitmap(ABitmap: TBitmap;
  ARotationAngle: TcxRotationAngle; AFlipVertically: Boolean = False);
var
  SourceRGBs, DestRGBs: TRGBColors;
  ARow, ACol, H, W, ASourceI, ADestI: Integer;
begin
  SourceRGBs := nil; // to remove compiler's warning
  if (ARotationAngle = ra0) and not AFlipVertically then
    Exit;
  H := ABitmap.Height;
  W := ABitmap.Width;
...
procedure TcxCanvas.TextExtent(const Text: string; var R: TRect;
  Flags: Integer);
var
  ...
  begin
    ...
    else if Flags and DT_BOTTOM <> 0 then
...
begin
  ...
  if (RWidth <= 0) and (Text <> '') then // A2079
    R.Right := R.Left + 1;
  if cxDrawText(Canvas.Handle, Text, R, Flags and not DT_VCENTER or
    DT_CALCRECT) = 0 then
  begin
    R.Right := R.Left;
    R.Bottom := R.Top;
...
procedure TcxCanvas.TransparentDraw(X, Y: Integer; ABitmap: TBitmap;
  AAlpha: Byte; ABackground: TBitmap = nil);

  function BlendValues(ASource, ADestination: DWORD): DWORD;
  begin
    Result := MulDiv(ASource, AAlpha, 255) + MulDiv(ADestination,
      255 - AAlpha, 255);
  end;
  ...
  begin
    ...
    ADestination.rgbGreen := BlendValues(ASource.rgbGreen,
      ADestination.rgbGreen);
...
begin
  ...
  try
    ...
    if ABackground = nil then
      ABackgroundBitmap.Canvas.CopyRect(Rect(0, 0, W, H), Canvas,
        Bounds(X, Y, W, H))
    else
    ...
    begin
      ABlendFunction := DefaultBlendFunction;
      ABlendFunction.SourceConstantAlpha := AAlpha;
      VCLAlphaBlend(ABackgroundBitmap.Canvas.Handle, 0, 0, W, H,
        ABitmap.Canvas.Handle, 0, 0, W, H, ABlendFunction);
    end
    else
    begin
...
var
    ...
    begin
      ...
      for I := 1 to AMaxCharCount do
        ...
        begin
          AMaxCharCount := I - 1;
          ACharCount := 1;
          if (I < Length(Text)) and dxCharInSet(Text[I + 1],
            dxBreakLineCharacters) and (Text[I] <> Text[I + 1]) then
            Inc(ACharCount);
  ...
  begin
    cxGetTextExtentPoint32(Handle, Copy(Text, 1, AMaxCharCount), AStringSize,
      AMaxCharCount);
  end;
...
procedure TcxCanvas.AngleArc(X, Y: Integer; ARadius: Cardinal;
  AStartAngle, ASweepAngle: Single);
begin
{$IFDEF DELPHI15}
  Canvas.AngleArc(X, Y, ARadius, AStartAngle, ASweepAngle);
{$ELSE}
  Windows.AngleArc(Handle, X, Y, ARadius, AStartAngle, ASweepAngle);
{$ENDIF}
end;

procedure TcxCanvas.Arc(X1, Y1, X2, Y2, X3, Y3, X4, Y4: Integer; AColor: TColor;
  APenWidth: Integer);
begin
  Pen.Color := AColor;
  Pen.Width := APenWidth;
...
procedure TcxCanvas.SetClipRegion(ARegion: TcxRegion;
  AOperation: TcxRegionOperation; ADestroyRegion: Boolean = True;
  AConsiderOrigin: Boolean = True);
var
...
begin
  ...
  try
    ...
    Result := AClipRegion.IsEqual(ARegion) and PtVisible(Handle, R.Left, R.Top)
      and PtVisible(Handle, R.Right, R.Bottom);
  finally
    ARegion.Free;
    AClipRegion.Free;
  end;
...
begin
  ...
  FDeviceContext := GetDCEx(FWindowHandle, 0, DCX_CACHE or
    DCX_LOCKWINDOWUPDATE); // B96653
...
procedure cxTransformImages(ASourceImageList, ADestinationImageList
  : TcxImageList; ABkColor: TColor; AEnabled: Boolean = True);
var
  AAlphaBitmap: TcxAlphaBitmap;
  ABitmap: TcxBitmap;
  I: Integer;
begin
  ...
  AAlphaBitmap := TcxAlphaBitmap.CreateSize(ASourceImageList.Width,
    ASourceImageList.Height, dxColorToRGBQuad(ABkColor, $FF));
  ABitmap := TcxBitmap.CreateSize(AAlphaBitmap.ClientRect, pf24bit);
  try
    for I := 0 to ASourceImageList.Count - 1 do
    begin
      AAlphaBitmap.Clear;
      ASourceImageList.Draw(AAlphaBitmap.Canvas, AAlphaBitmap.ClientRect, I,
        True, False, AEnabled);
...
procedure DrawMenuItemText(AMenuItem: TMenuItem; ACanvas: TCanvas;
  const ACaption: string; var ARect: TRect; ASelected: Boolean;
  AFlags: LongInt);
var
  AText: string;
  R: TRect;
begin
  AText := ACaption;
  if (AFlags and DT_CALCRECT <> 0) and
    ((AText = '') or (AText[1] = cHotkeyPrefix) and (AText[2] = #0)) then
    AText := AText + ' ';
  with ACanvas do
  begin
    ...
    begin
      ...
      begin
        ...
        if ASelected and (ColorToRGB(clHighlight) = ColorToRGB(clBtnShadow))
        then
          Font.Color := clBtnHighlight
        else
          Font.Color := clBtnShadow;
...
begin
  ...
  begin
    ...
    DrawThemeTextEx(ATheme, ACanvas.Handle, MENU_POPUPITEM,
      MenuStates[AMenuItem.Enabled], AText, Length(AText), AFlags, ARect,
      AOptions);
  end;
end;
...
var
  ...
  const
    ...
    CheckMarkStates: array [Boolean, Boolean] of Cardinal =
      ((MC_CHECKMARKDISABLED, MC_BULLETDISABLED),
      (MC_CHECKMARKNORMAL, MC_BULLETNORMAL));
  var
  ...
  begin
    ...
    begin
      ...
      GetThemePartSize(ATheme, Handle, MENU_POPUPCHECK,
        CheckMarkStates[AMenuItem.Enabled, AMenuItem.RadioItem], nil, TS_TRUE,
        @ACheckSize);
      GetThemeMargins(ATheme, Handle, MENU_POPUPCHECK,
        CheckMarkStates[AMenuItem.Enabled, AMenuItem.RadioItem],
        TMT_CONTENTMARGINS, nil, ACheckMargins);
      with ARect, ACheckMargins do
        ACheckRect := Rect(Left, Top, Left + ACheckSize.CX + cxRightWidth +
          cxRightWidth, Top + ACheckSize.CY + cyBottomHeight + cyBottomHeight);
      ...
      GetThemePartSize(ATheme, Handle, MENU_POPUPGUTTER, 0, nil,
        TS_TRUE, @ASize);
      GetThemeMargins(ATheme, Handle, MENU_POPUPGUTTER, 0, TMT_SIZINGMARGINS,
        nil, AMargins);
      ...
      begin
        ADrawImage := ADrawImageListItem or
          (AMenuItem.Checked and AMenuItem.Bitmap.Empty);
        if ADrawImage or not AMenuItem.Bitmap.Empty then
        begin
          if AMenuItem.Checked then
            DrawThemeBackground(ATheme, Handle, MENU_POPUPCHECKBACKGROUND,
              CheckMarkBkgs[AMenuItem.Enabled],
              GetActualRectangle(ACheckRect), nil);
          if ADrawImage then
          begin
            if ADrawImageListItem then
              with GetActualRectangle(AGlyphRect) do
                AImageList.Draw(ACanvas,
                  Left + ((Right - Left - AImageList.Width) div 2),
                  Top + ((Bottom - Top - AImageList.Height) div 2),
            ...
            begin
              DrawThemeBackground(ATheme, Handle, MENU_POPUPCHECKBACKGROUND,
                CheckMarkBkgs[AMenuItem.Enabled],
                GetActualRectangle(ACheckRect), nil);
              DrawThemeBackground(ATheme, Handle, MENU_POPUPCHECK,
                CheckMarkStates[AMenuItem.Enabled, AMenuItem.RadioItem],
                GetActualRectangle(ACheckRect), nil);
            end;
          end
      ...
      if ASelected then
        DrawThemeBackground(ATheme, Handle, MENU_POPUPITEM, MPI_HOT,
          GetActualRectangle(ARect), nil);
      GetThemeMargins(ATheme, Handle, MENU_POPUPITEM, MPI_NORMAL,
      ...
      if not AIsMenuSeparator then
      begin
        GetThemePartSize(ATheme, Handle, MENU_POPUPSUBMENU, MSM_NORMAL, nil,
          TS_TRUE, @ASize);
        Dec(ARect.Right, ASize.CX);
        ASaveRect := ARect;
        GetThemeColor(ATheme, MENU_POPUPITEM, ItemStates[AMenuItem.Enabled],
          TMT_TEXTCOLOR, AColorRef);
        Font.Color := AColorRef;
        DrawVistaMenuItemText(AMenuItem, ACanvas, AMenuItem.Caption, ARect,
          ASelected, ATextFlags or DT_CALCRECT or DT_NOCLIP, ATheme);
        OffsetRect(ARect, 0, ((ASaveRect.Bottom - ASaveRect.Top) -
          (ARect.Bottom - ARect.Top)) div 2);

        R := GetActualRectangle(ARect);
        DrawVistaMenuItemText(AMenuItem, ACanvas, AMenuItem.Caption, R,
          ASelected, ATextFlags, ATheme);
        ...
        begin
          ...
          R := GetActualRectangle(ARect);
          DrawMenuItemText(AMenuItem, ACanvas, AShortCutText, R, ASelected,
            ActualRightAlignment[ABiDiMode = bdRightToLeft]);
        end;
      end
      else
      begin
        ARect.Left := AGutterRect.Right + 1;
        GetThemeMargins(ATheme, Handle, MENU_POPUPSEPARATOR, 0,
          TMT_SIZINGMARGINS, nil, AMargins);
        Dec(ARect.Bottom, AMargins.cyBottomHeight);
        DrawThemeBackground(ATheme, Handle, MENU_POPUPSEPARATOR, 0,
          GetActualRectangle(ARect), nil);
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ADrawImage := ADrawImageListItem or
          (AMenuItem.Checked and AMenuItem.Bitmap.Empty);
        ACheckBkgRect := cxRectCenterVertically(ARect,
          Min(IfThen(AImageList <> nil, AImageList.Height + 6, 22),
          cxRectHeight(ARect)));
        ACheckBkgRect.Right := ACheckBkgRect.Left + cxRectHeight(ACheckBkgRect);
        if ADrawImage or not AMenuItem.Bitmap.Empty then
        begin
          if ADrawImage then
            AGlyphRect := cxRectCenter(ACheckBkgRect, AImageList.Width,
              AImageList.Height)
          else
          ...
          begin
            ...
            if ADrawImageListItem then
              AImageList.Draw(ACanvas, R.Left, R.Top, AMenuItem.ImageIndex,
                AMenuItem.Enabled)
            else
            begin
          ...
          begin
            ...
            if AMenuItem.Bitmap.Width < AGlyphRect.Right - AGlyphRect.Left then
              with AGlyphRect do
              begin
                Left := Left + ((Right - Left) - AMenuItem.Bitmap.Width)
                  div 2 + 1;
                Right := Left + AMenuItem.Bitmap.Width;
              end;
            if AMenuItem.Bitmap.Height < AGlyphRect.Bottom - AGlyphRect.Top then
              with AGlyphRect do
              begin
                Top := Top + ((Bottom - Top) - AMenuItem.Bitmap.Height)
                  div 2 + 1;
                Bottom := Top + AMenuItem.Bitmap.Height;
              end;
      ...
      if odDefault in AState then
        Font.Style := [fsBold];
      DrawMenuItemText(AMenuItem, ACanvas, AMenuItem.Caption, ARect, ASelected,
        ATextFlags or DT_CALCRECT or DT_NOCLIP);
      OffsetRect(ARect, 0, ((ASaveRect.Bottom - ASaveRect.Top) -
        (ARect.Bottom - ARect.Top)) div 2);

      R := GetActualRectangle(ARect);
      DrawMenuItemText(AMenuItem, ACanvas, AMenuItem.Caption, R, ASelected,
        ATextFlags);
      if (AMenuItem.ShortCut <> 0) and
        not(IsWinVistaOrLater and (AMenuItem.Count > 0)) then
      begin
        ARect.Left := ARect.Right;
        ARect.Right := ASaveRect.Right - 10;
        R := GetActualRectangle(ARect);
        DrawMenuItemText(AMenuItem, ACanvas, ShortCutToText(AMenuItem.ShortCut),
          R, ASelected, ActualRightAlignment[ABiDiMode = bdRightToLeft]);
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    cxDrawPicture(ABitmap.cxCanvas, cxRect(0, 0, ASize.CX, ASize.CY),
      APicture, AMode);
    ...
    for I := 0 to AFrameCount - 1 do
      ...
      begin
        ...
        begin
          Result := AFrame;
          AParameterValue := Cardinal(TGpEncoderValues.evMultiFrame);
          GdipCheck(GdipSaveImageToStream(Result.Handle, AStreamAdapter,
            @TIFFEncoder, @AParams));
        end
        else
        begin
...
begin
  if IsGlyphAssigned(AGraphic) and (AGraphic is TdxCustomSmartImage) then
    Result := TdxSmartImageCodecsRepository.GetDefaultExt
      (TdxCustomSmartImage(AGraphic).ImageCodec)
  else
...
function cxGraphicFileMask(AGraphicClass: TGraphicClass): string;
begin
  if (AGraphicClass <> nil) and AGraphicClass.InheritsFrom(TdxCustomSmartImage)
  then
...
function cxGraphicFilter(AGraphic: TGraphic;
  AForExport: Boolean = False): string;
begin
  if AForExport and IsGlyphAssigned(AGraphic) and
    (AGraphic is TdxCustomSmartImage) then
    Result := TdxSmartImageCodecsRepository.GetSaveDialogFilter
      (TdxCustomSmartImageAccess(AGraphic).Handle)
  else
...
procedure TcxBitmap.CopyBitmap(ASrcBitmap: TBitmap; const ADestRect: TRect;
  const ASrcTopLeft: TPoint; ACopyMode: DWORD);
begin
  cxBitBlt(Canvas.Handle, ASrcBitmap.Canvas.Handle, ADestRect, ASrcTopLeft,
    ACopyMode);
end;
  ...
  begin
    ...
    try
      ...
      SetWorldTransform(ABitmap.Canvas.Handle,
        TXForm.CreateFlip(AFlipHorizontally, AFlipVertically,
        (ABitmap.Width - 1) / 2, (ABitmap.Height - 1) / 2));
      cxBitBlt(ABitmap.Canvas.Handle, Canvas.Handle, ABitmap.ClientRect,
        cxNullPoint, SRCCOPY);
      SetWorldTransform(ABitmap.Canvas.Handle, TXForm.CreateIdentityMatrix);
      SetGraphicsMode(ABitmap.Canvas.Handle, APrevMode);
      cxBitBlt(Canvas.Handle, ABitmap.Canvas.Handle, ClientRect,
        cxNullPoint, SRCCOPY);
    finally
      ABitmap.Free;
    end;
...
procedure TcxBitmap.Rotate(ARotationAngle: TcxRotationAngle;
...
procedure TcxBitmap.Initialize(AWidth, AHeight: Integer;
  APixelFormat: TPixelFormat);
begin
  BeginUpdate;
  try
...
begin
  ...
  begin
    ...
    try
      case AVersion of
        1:
          Decompress1(Stream, AMemoryStream, ASize);
        2:
          Decompress2(Stream, AMemoryStream, ASize);
      end;
...
procedure TcxBitmap.WriteData(Stream: TStream);
  ...
  procedure WriteSignature(AStream: TStream;
    ASize, ASignaturePosition: Integer);
  var
    ACurrentPos: Integer;
  begin
...
begin
  ...
  begin
    ...
    try
      ...
      ADataOffset := SizeOf(ASize) + SizeOf(ADXBMSignature) +
        SizeOf(ADXBMVersion);
...
begin
  Result := (Length(ABlock1) = Length(ABlock2)) and
    CompareMem(ABlock1, ABlock2, Length(ABlock1));
end;

function ReadBlock(AStream: TStream; AMaxPos: Integer; var ABlock: TBytes;
  ABlockSize: Integer): Boolean;
var
  I: Integer;
begin
  Result := True;
...
procedure TcxBitmap.CompressByBlock(ASourceStream, ADestStream: TStream;
  ASize, ABlockSize: Integer);

  function GetCounter(ASeekBlock: TBytes; AMode: TSeekMode;
    AMaxPos: Integer): Integer;
  var
    ABlock: TBytes;
  begin
    Result := 1;
    SetLength(ABlock, ABlockSize);
    while (Result < 125) and ReadBlock(ASourceStream, AMaxPos, ABlock,
      ABlockSize) do
    begin
      if (AMode = smDup) and CompareBlock(ABlock, ASeekBlock) or
        (AMode = smUnique) and not CompareBlock(ABlock, ASeekBlock) then
        Inc(Result)
      else
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      AMode := AModeMap[(AReadedCount = ABlockSize) or
        not CompareBlock(AReadBlock1, AReadBlock2)];
      ASourceStream.Position := ASourceStream.Position -
        (AReadedCount - ABlockSize);
...
procedure TcxBitmap.DecompressByBlock(ASourceStream, ADestStream: TStream;
  ASize, ABlockSize: Integer);
var
  ACode: Byte;
  AReadBlob: TBytes;
...
procedure TcxBitmap.Compress(ASourceStream, ADestStream: TStream;
  ASize: Integer);
var
...
procedure TcxBitmap.Decompress1(ASourceStream, ADestStream: TStream;
  ASize: Integer);
begin
  DecompressByBlock(ASourceStream, ADestStream, ASize, 1);
end;

procedure TcxBitmap.Decompress2(ASourceStream, ADestStream: TStream;
  ASize: Integer);
var
  ABlockSize, AShift: Byte;
begin
  ...
  if AShift > 0 then
    ADestStream.CopyFrom(ASourceStream, AShift);
  DecompressByBlock(ASourceStream, ADestStream, ASize - (2 + AShift),
    ABlockSize);
end;
...
procedure TcxBitmap32.AlphaBlend(ADestBitmap: TcxBitmap32;
  const ADestRect: TRect; ASmoothImage: Boolean; AConstantAlpha: Byte = $FF);
begin
  cxAlphaBlend(ADestBitmap, Self, ADestRect, ClientRect, ASmoothImage,
    AConstantAlpha);
end;
...
begin
  ...
  begin
    ...
    else if AColor.rgbReserved = 0 then
...
procedure TcxBitmap32.Resize(ANewWidth, ANewHeight: Integer;
  AStretch, ASmooth: Boolean; AFillColor: TColor = clNone);
var
  ABitmap: TcxBitmap32;
begin
  ...
  try
    ...
    if AStretch then
      ...
      CopyBitmap(ABitmap, cxRectCenter(ClientRect, ABitmap.Width,
        ABitmap.Height), cxNullPoint);
  finally
    ABitmap.Free;
  end;
end;
...
procedure TcxAlphaBitmap.DrawHatch(AColor1, AColor2: TColor;
  AStep, AAlpha1, AAlpha2: Byte);
var
...
procedure TcxAlphaBitmap.DrawShadow(AMaskBitmap: TcxAlphaBitmap;
  AShadowSize: Integer; AShadowColor: TColor; AInflateSize: Boolean);
const
  DPSnaa = $00200F09;
var
  AShadowBitmap, ASelfCopy: TcxAlphaBitmap;
begin
  AShadowBitmap := TcxAlphaBitmap.CreateSize(Width + AShadowSize * 2,
    Height + AShadowSize * 2, ClrNone);
  try
    AShadowBitmap.Clear;
    AShadowBitmap.CopyBitmap(AMaskBitmap, cxRectOffset(ClientRect, AShadowSize,
      AShadowSize), cxNullPoint);
    ...
    ASelfCopy := TcxAlphaBitmap.CreateSize(Width + AShadowSize,
      Height + AShadowSize, True);
    try
      ASelfCopy.CopyBitmap(Self);
      ASelfCopy.CopyBitmap(AShadowBitmap, ASelfCopy.ClientRect,
        Point(AShadowSize, AShadowSize), SRCPAINT);
...
procedure TcxAlphaBitmap.RecoverAlphaChannel(ARect: TRect;
  ATransparentColor: TColor);
var
  ABitmap: TcxAlphaBitmap;
begin
  ABitmap := TcxAlphaBitmap.CreateSize(cxRectWidth(ARect), cxRectHeight(ARect));
  try
...
procedure TcxAlphaBitmap.Initialize(AWidth, AHeight: Integer;
  APixelFormat: TPixelFormat);
begin
...
begin
  if Odd(FCurrentColorIndex.X div FHatchData.Step +
    FCurrentColorIndex.Y div FHatchData.Step) then
...
procedure TcxAlphaBitmap.ScaleColor(var AColor: TRGBQuad;
  const AColorMap: TcxColorTransitionMap);
var
  AResultValue: Byte;
begin
  AResultValue := GetChannelValue(Round(AColorMap.RedScale * AColor.rgbRed +
    AColorMap.GreenScale * AColor.rgbGreen + AColorMap.BlueScale *
    AColor.rgbBlue));
...
begin
  Result := cxColorIsEqual(AColor, TransparentBkColor) or
    IsTransparentPixel(cxColorEssence(AColor));
end;
...
begin
  cxBitBlt(DC, CanvasHandle, Bounds(P.X, P.Y, Width, Height),
    cxNullPoint, SRCCOPY);
end;
...
procedure TcxAlphaDIB.DrawAlphaBlend(DC: HDC; const R: TRect;
  AAlpha: Byte = 255);
begin
...
begin
  ...
  begin
    ...
    FBitmap := CreateDIBSection(0, AInfo, DIB_RGB_COLORS,
      Pointer(FPixels), 0, 0);
...
function TcxImageCollection.GetAsMultiFrameTIFF(const ASize: TSize;
  AMode: TcxImageFitMode): TdxSmartImage;
begin
...
procedure TcxImageCollection.RemoveListener
  (AListener: IcxImageCollectionListener);
begin
...
function TcxImageCollection.GetFrame(AFrameIndex: Integer; const ASize: TSize;
  AMode: TcxImageFitMode; var AFrame: TdxSmartImage): Boolean;
begin
...
begin
  ...
  try
    ...
    cxDrawImage(ACanvas.Handle, ARect, R, ABitmap, nil, 0,
      EnabledImageDrawModeMap[AEnabled], ASmoothResize);
  finally
    ABitmap.Free;
  end;
end;
...
function TcxImageCollectionItem.GetCollectionFromParent(AParent: TComponent)
...
function TcxImageCollectionItems.FindItemByName(const AName: string;
  out AItem: TcxImageCollectionItem): Boolean;
var
  I: Integer;
begin
...
function TcxImageCollectionItems.FindItemByID(ID: Integer)
  : TcxImageCollectionItem;
begin
  Result := TcxImageCollectionItem(inherited FindItemByID(ID));
end;

function TcxImageCollectionItems.GetItem(Index: Integer)
  : TcxImageCollectionItem;
begin
...
procedure TcxImageCollectionItems.SetItem(Index: Integer;
  AValue: TcxImageCollectionItem);
begin
...
begin
  AItem.Name := CreateUniqueName(FCollection.Owner, FCollection, AItem,
    'TcxImageCollection', '', ABaseIndex);
end;

procedure TcxImageCollectionItems.Update(AItem: TcxComponentCollectionItem;
...
begin
  ...
  case ASector of
    0:
      Result := GetColor(AMax, AMid_2, AMin);
    1:
      Result := GetColor(AMid_1, AMax, AMin);
    2:
      Result := GetColor(AMin, AMax, AMid_2);
    3:
      Result := GetColor(AMin, AMid_1, AMax);
    4:
      Result := GetColor(AMid_2, AMin, AMax);
  else
...
class function TdxColorHelper.ChangeLightness(AColor: TColor;
  const ALightnessDelta: Double): TColor;
var
  HSL: TdxHSL;
begin
...
class function TdxColorHelper.ChangeSaturation(AColor: TColor;
  const ASaturationDelta: Double): TColor;
var
  HSL: TdxHSL;
begin
...
class function TdxColorHelper.MultiplyLightness(AColor: TColor;
  const AFactor: Double): TColor;
var
  AValue: TdxHSL;
begin
  AValue := TdxColorSpaceConverter.ColorToHSL(AColor);
  Result := TdxColorSpaceConverter.HSLToColor(AValue.H, AValue.S,
    AValue.L * AFactor);
end;

class function TdxColorHelper.AlphaColorToHexCode(AValue: TdxAlphaColor;
  AIsDelphiNotation, AIncludeAlphaValue: Boolean): string;
var
  ARGBQuad: TRGBQuad;
begin
  ARGBQuad := dxAlphaColorToRGBQuad(AValue);
  if AIsDelphiNotation then
    Result := IntToHex(ARGBQuad.rgbBlue, 2) + IntToHex(ARGBQuad.rgbGreen, 2) +
      IntToHex(ARGBQuad.rgbRed, 2)
  else
    Result := IntToHex(ARGBQuad.rgbRed, 2) + IntToHex(ARGBQuad.rgbGreen, 2) +
      IntToHex(ARGBQuad.rgbBlue, 2);

  if AIncludeAlphaValue then
    Result := IntToHex(ARGBQuad.rgbReserved, 2) + Result;
end;

class function TdxColorHelper.CompleteHexCode(const AValue: string;
  AIsDelphiNotation: Boolean): string;
begin
  Result := AValue;
...
class function TdxColorHelper.HexCodeToAlphaColor(AHexCode: string;
  AIsDelphiNotation: Boolean): TdxAlphaColor;
var
  V1, V2, V3, V4: Byte;
begin
...
procedure TdxAdjustFontSizeCustomHelper.Calculate(const AAvailableWidth
...
procedure TdxAdjustFontSizeCustomHelper.Calculate(const AAvailableSize: TSize;
  const AText: string; AMultiLine: Boolean; AFlags: Cardinal);
begin
  BeforeCalculate;
  try
    FFlags := AFlags;
    ...
    begin
      ...
      begin
        FLineSpacing := CalculateCore(AAvailableSize.CY, LineSpacingScaleFactor,
          Round(GetMinLineSpacing * LineSpacingScaleFactor), AText,
          DoMeasureTextHeight1, nil) / LineSpacingScaleFactor;
      end;
      Font.Size := CalculateCore(AAvailableSize.CY, Font.Size, GetMinFontSize,
        AText, DoMeasureTextHeight2, CheckFontSize);
    end
    else
      Font.Size := CalculateCore(AAvailableSize.CX, Font.Size, GetMinFontSize,
        AText, DoMeasureTextWidth, CheckFontSize);
  finally
    AfterCalculate;
  end;
end;
...
function TdxAdjustFontSizeCustomHelper.CalculateCore(AAvailableSize, AParam,
  AParamLow: Integer; const AText: string; AMeasureFunc: TMeasureTextFunc;
  ACheckParamFunc: TCheckParamFunc): Integer;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if ATextSize > AAvailableSize then
        AParamHigh := Result - 1
      else if ATextSize < AAvailableSize then
      begin
...
function TdxAdjustFontSizeCustomHelper.CheckFontSize(ALastSuitableSize,
  AOriginalSize, AMinSize: Integer): Integer;
begin
...
function TdxAdjustFontSizeCustomHelper.DoMeasureTextHeight1(const AText: string;
  ALineSpacing: Integer): Integer;
begin
  Result := GetTextHeight(AText, FWidth, ALineSpacing / LineSpacingScaleFactor);
end;

function TdxAdjustFontSizeCustomHelper.DoMeasureTextHeight2(const AText: string;
  AFontSize: Integer): Integer;
begin
  Font.Size := AFontSize;
...
function TdxAdjustFontSizeCustomHelper.DoMeasureTextWidth(const AText: string;
  AFontSize: Integer): Integer;
begin
  Font.Size := AFontSize;
...
function TdxAdjustFontSizeHelper.GetTextHeight(const AText: string;
  AWidth: Integer; ALineSpacing: Single): Integer;
var
  ATextRect: TRect;
  AMeasureDC: HDC;
begin
  ...
  Windows.DrawTexT(AMeasureDC, PChar(AText), Length(AText), ATextRect,
    FFlags or DT_CALCRECT);
...
constructor TdxImageListPaintCache.TImageListCacheHelper.Create
  (AImages: TCustomImageList; AMaxSizeCount: Integer = 0);
begin
  inherited Create(nil);
  FImageList := AImages;
...
function TdxImageListPaintCache.TImageListCacheHelper.GetImage
  (const ABounds: TRect; AIndex: Integer; ADrawMode: TcxImageDrawMode;
  AUseLeftBottomPixelAsTransparent: Boolean; const APalette: IdxColorPalette;
  ASmoothImage: Boolean): TdxGPImage;
var
...
procedure TdxImageListPaintCache.TImageListCacheHelper.Draw
  (ACanvas: TcxGdiBasedCanvas; const ABounds: TRect; AIndex: Integer;
  ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent: Boolean;
...
procedure TdxImageListPaintCache.TImageListCacheHelper.Draw
  (AGraphic: TdxGPGraphics; const ABounds: TRect; AIndex: Integer;
  ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent: Boolean;
  const APalette: IdxColorPalette);
begin
  AGraphic.Draw(GetImage(ABounds, AIndex, ADrawMode,
    AUseLeftBottomPixelAsTransparent, APalette, True), ABounds);
end;
...
constructor TdxDrawImageCacheID.Create(AGlyph: TGraphic;
  AImageList: TCustomImageList; AImageIndex: Integer;
  ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent,
  ASmoothImage: Boolean; ATransparentColor: TColor;
...
class procedure TdxImageListPaintCache.RemoveHelper
...
class procedure TdxImageListPaintCache.Draw(ACanvas: TcxGdiBasedCanvas;
  const ABounds: TRect; AImages: TCustomImageList; AIndex: Integer;
  ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent: Boolean;
  APalette: IdxColorPalette; ASmoothImage: Boolean);
begin
  SetupHelper(AImages);
  FLastHelper.Draw(ACanvas, ABounds, AIndex, ADrawMode,
    AUseLeftBottomPixelAsTransparent, APalette, ASmoothImage)
end;

class procedure TdxImageListPaintCache.Draw(AGraphic: TdxGPGraphics;
  const ABounds: TRect; AImages: TCustomImageList; AIndex: Integer;
  ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent: Boolean;
  APalette: IdxColorPalette);
begin
  SetupHelper(AImages);
  FLastHelper.Draw(AGraphic, ABounds, AIndex, ADrawMode,
    AUseLeftBottomPixelAsTransparent, APalette)
end;

class procedure TdxImageListPaintCache.InvalidateImageList
...
class procedure TdxImageListPaintCache.PrepareImage(ATarget: TdxGPImage;
  const ASize: TSize; const ID: TdxDrawImageCacheID;
  const AColorPalette: IdxColorPalette);
begin
  TransferBitmap.RefreshImage(ASize.CX, ASize.CY);
  cxDrawImage(TransferBitmap.cxCanvas.Handle, TransferBitmap.ClientRect,
    TransferBitmap.ClientRect, ID.Glyph, ID.ImageList, ID.ImageIndex,
    ID.DrawMode, ID.SmoothImage, ID.BrushHandle, ID.TransparentColor,
...
const
  Pattern: array [0 .. 7] of Word = ($00AA, $0055, $00AA, $0055, $00AA, $0055,
    $00AA, $0055);
var
  ABitmap: HBITMAP;
begin
