...
{$EXTERNALSYM CAL_UMALQURA}
  CAL_SSHORTESTDAYNAME1 = $00000060;
  // Windows Vista or later: Short native name of the first day of the week.
{$EXTERNALSYM CAL_SSHORTESTDAYNAME1}
  CAL_SSHORTESTDAYNAME2 = $00000061;
  // Windows Vista or later: Short native name of the second day of the week.
{$EXTERNALSYM CAL_SSHORTESTDAYNAME2}
  CAL_SSHORTESTDAYNAME3 = $00000062;
  // Windows Vista or later: Short native name of the third day of the week.\
{$EXTERNALSYM CAL_SSHORTESTDAYNAME3}
  CAL_SSHORTESTDAYNAME4 = $00000063;
  // Windows Vista or later: Short native name of the fourth day of the week.
{$EXTERNALSYM CAL_SSHORTESTDAYNAME4}
  CAL_SSHORTESTDAYNAME5 = $00000064;
  // Windows Vista or later: Short native name of the fifth day of the week.
{$EXTERNALSYM CAL_SSHORTESTDAYNAME5}
  CAL_SSHORTESTDAYNAME6 = $00000065;
  // Windows Vista or later: Short native name of the sixth day of the week.
{$EXTERNALSYM CAL_SSHORTESTDAYNAME6}
  CAL_SSHORTESTDAYNAME7 = $00000066;
  // Windows Vista or later: Short native name of the seventh day of the week.
{$EXTERNALSYM CAL_SSHORTESTDAYNAME7}
  CAL_RETURN_NUMBER = $20000000;
  // Windows 98/Me, Windows 2000 and later: Returns the result from GetCalendarInfo as a number instead of a string. This is only valid for CALTYPES beginning with CAL_I.
{$EXTERNALSYM CAL_RETURN_NUMBER}
  CAL_ITWODIGITYEARMAX = $00000030;
  // Windows 98/Me, Windows 2000 and later: An integer value indicating the upper boundary of the two-digit year range.
{$EXTERNALSYM CAL_ITWODIGITYEARMAX}
  CAL_SYEARMONTH = $0000002F;
  // Windows 98/Me, Windows 2000 and later: Enumerates the year/month formats for the specified calendars.
...
type
  ...
  public
    ...
    function GetDaysInMonth(AEra: Integer; AYear, AMonth: Cardinal)
      : Cardinal; override;
    ...
    function GetMonthsInYear(AEra: Integer; AYear: Cardinal): Cardinal;
      override;
    function IsLeapDay(AEra: Integer; AYear, AMonth, ADay: Cardinal)
      : Boolean; override;
    function IsLeapMonth(AEra: Integer; AYear, AMonth: Cardinal)
      : Boolean; override;
  ...
  TcxGregorianTransliteratedEnglishCalendarTable = class
    (TcxGregorianCalendarTable)
  protected
    function GetCalendarID: CALID; override;
  public
  ...
  TcxGregorianTransliteratedFrenchCalendarTable = class
    (TcxGregorianCalendarTable)
  protected
    function GetCalendarID: CALID; override;
  public
  ...
  TcxHebrewCalendarTableTypeYear = (hctyDeficient = 1, hctyNormal = 2,
    hctyPerfect = 3);

  TcxHebrewCalendarTable = class(TcxCustomCalendarTable)
  private
    FDefaultEra: TcxEra;
    function GetDayDifference(ALunarYearType, AMonth, ADay, ALunarMonth,
      ALunarDay: Integer): Integer;
...
function cxIsGregorianCalendar(ACalendar: TcxCustomCalendarTable = nil)
  : Boolean;
...
const
  cxHebrewTable: array [0 .. 1315] of Integer = (7, 3, 17, 3,
    // 1583-1584  (Hebrew year: 5343 - 5344)
  ...
  cxHebrewTableYear = cxHebrewLastGregorianTableYear -
    cxHebrewFirstGregorianTableYear;
...
function TcxGregorianCalendarTable.GetCalendarAlgorithmType
...
function TcxGregorianCalendarTable.GetWeekNumber(ADate: TcxDateTime;
...
function TcxGregorianCalendarTable.GetDaysInMonth(AEra: Integer;
  AYear, AMonth: Cardinal): Cardinal;
begin
  case AMonth of
    2:
      begin
...
function TcxGregorianCalendarTable.GetDaysInYear(AEra: Integer; AYear: Cardinal)
  : Cardinal;
begin
  if IsLeapYear(AEra, AYear) then
    Result := 366
...
function TcxGregorianCalendarTable.GetFullWeeksInYear(AYear: Cardinal)
  : Cardinal;
var
  ADate: TcxDateTime;
  ADay: Integer;
...
function TcxGregorianCalendarTable.GetMonthsInYear(AEra: Integer;
  AYear: Cardinal): Cardinal;
begin
  Result := 12;
end;

function TcxGregorianCalendarTable.IsLeapDay(AEra: Integer;
  AYear, AMonth, ADay: Cardinal): Boolean;
begin
  Result := IsLeapMonth(AEra, AYear, ADay) and (ADay = 29);
end;

function TcxGregorianCalendarTable.IsLeapMonth(AEra: Integer;
  AYear, AMonth: Cardinal): Boolean;
begin
  Result := IsLeapYear(AEra, AYear) and (AMonth = 2);
end;

function TcxGregorianCalendarTable.IsLeapYear(AEra: Integer;
...
function TcxGregorianCalendarTable.ToDateTime(ADateTime: TcxDateTime)
  : TDateTime;
begin
  with ADateTime do
    Result := EncodeDateTime(Year, Month, Day, Hours, Minutes, Seconds,
      Milliseconds);
end;
...
begin
  ...
  FEras.Add(TcxEra.Create(1, EncodeDate(1868, 9, 8), 1867, 1, 1912 - 1867));
  // Meiji
  FEras.Add(TcxEra.Create(2, EncodeDate(1912, 7, 30), 1911, 1, 1926 - 1911));
  // Taisho
  FEras.Add(TcxEra.Create(3, EncodeDate(1926, 12, 25), 1925, 1, 1989 - 1925));
  // Showa
  FEras.Add(TcxEra.Create(4, EncodeDate(1989, 1, 8), 1988, 1, 9999 - 1988));
  // Heisei. Most recent
end;

function TcxJapaneseCalendarTable.GetCalendarAlgorithmType
  : TcxCalendarAlgorithmType;
begin
  Result := catSolarCalendar;
end;
...
begin
  ...
  begin
    ...
    Result := EncodeDate(AYear, Month, Day) + EncodeTime(Hours, Minutes,
...
function TcxTaiwanCalendarTable.GetCalendarAlgorithmType
  : TcxCalendarAlgorithmType;
begin
  Result := catSolarCalendar;
end;
...
function TcxKoreanCalendarTable.GetCalendarAlgorithmType
  : TcxCalendarAlgorithmType;
begin
  Result := catSolarCalendar;
end;
...
function TcxHijriCalendarTable.FromDateTime(ADate: TDateTime): TcxDateTime;
var
  I: Integer;
  H, M, S, MS: Word;
  ACountDays: Integer;
...
function TcxHijriCalendarTable.GetCalendarAlgorithmType
  : TcxCalendarAlgorithmType;
begin
...
function TcxHijriCalendarTable.GetDaysInMonth(AEra: Integer;
  AYear, AMonth: Cardinal): Cardinal;
begin
  if IsLeapMonth(AYear, AMonth) then
    Result := 30
  else if AMonth in [2, 4, 6, 8, 10, 12] then
    Result := 29
  else
    Result := 30;
end;

function TcxHijriCalendarTable.GetDaysInYear(AEra: Integer; AYear: Cardinal)
  : Cardinal;
begin
...
function TcxHijriCalendarTable.GetFullWeeksInYear(AYear: Cardinal): Cardinal;
begin
  Result := 50;
end;
...
function TcxHijriCalendarTable.GetMonthsInYear(AEra: Integer; AYear: Cardinal)
  : Cardinal;
begin
  Result := 12;
end;

function TcxHijriCalendarTable.GetWeekNumber(ADate: TcxDateTime;
  AStartOfWeek: TDay; AFirstWeekOfYear: TcxFirstWeekOfYear): Cardinal;
var
...
begin
  if AFirstWeekOfYear = fwySystem then
    AFirstWeekOfYear := TcxFirstWeekOfYear
      (StrToInt(dxGetLocaleInfo(GetThreadLocale, LOCALE_IFIRSTWEEKOFYEAR,
      '0')) + 1);
  with ATmpDate do
  begin
    Year := ADate.Year;
  ...
  case AFirstWeekOfYear of
    fwyFirstFourDays:
      if FromDateTime(AStart + 3).Year < ADate.Year then
        AStart := AStart + 7;
    fwyFirstFullWeek:
      if FromDateTime(AStart).Year < ADate.Year then
        AStart := AStart + 7;
  end;
...
function TcxHijriCalendarTable.IsLeapDay(AEra: Integer;
  AYear, AMonth, ADay: Cardinal): Boolean;
begin
  Result := IsLeapMonth(AYear, AMonth) and (ADay = 30);
end;

function TcxHijriCalendarTable.IsLeapMonth(AEra: Integer;
  AYear, AMonth: Cardinal): Boolean;
begin
  Result := IsLeapYear(AYear) and (AMonth = 12);
end;

function TcxHijriCalendarTable.IsLeapYear(AEra: Integer;
  AYear: Cardinal): Boolean;
begin
  Result := (AYear mod 30) in [2, 5, 7, 10, 13, 15, 18, 21, 24, 26, 29];
end;

function TcxHijriCalendarTable.ToDateTime(ADateTime: TcxDateTime): TDateTime;
var
  I: Integer;
begin
  Result := 227013;
  ...
  Result := Result + EncodeTime(ADateTime.Hours, ADateTime.Minutes,
    ADateTime.Seconds, ADateTime.Milliseconds);
end;

function TcxHijriCalendarTable.GetMonthNumber(AYear: Integer;
...
function TcxThaiCalendarTable.GetCalendarAlgorithmType
  : TcxCalendarAlgorithmType;
begin
  Result := catSolarCalendar;
end;
...
function TcxHebrewCalendarTable.GetCalendarAlgorithmType
  : TcxCalendarAlgorithmType;
begin
...
function TcxHebrewCalendarTable.GetDaysInMonth(AEra: Integer;
  AYear, AMonth: Cardinal): Cardinal;
var
  AYearIndex: Integer;
begin
...
function TcxHebrewCalendarTable.GetDaysInYear(AEra: Integer; AYear: Cardinal)
  : Cardinal;
begin
...
function TcxHebrewCalendarTable.GetYearType(AYear: Cardinal)
...
function TcxHebrewCalendarTable.GetMonthsInYear(AEra: Integer; AYear: Cardinal)
  : Cardinal;
begin
  if IsLeapYear(AYear) then
    Result := 13
...
function TcxHebrewCalendarTable.IsLeapDay(AEra: Integer;
  AYear, AMonth, ADay: Cardinal): Boolean;
begin
  Result := IsValidDay(AYear, AMonth, ADay) and IsLeapMonth(AYear, AMonth);
end;

function TcxHebrewCalendarTable.IsLeapMonth(AEra: Integer;
  AYear, AMonth: Cardinal): Boolean;
begin
  Result := IsValidMonth(AYear, AMonth) and IsLeapYear(AYear) and (AMonth = 7);
end;

function TcxHebrewCalendarTable.IsLeapYear(AEra: Integer;
  AYear: Cardinal): Boolean;
var
  AIndex: Integer;
begin
...
begin
  ...
  Result := EncodeDateTime(AYear, 1, 1, ADateTime.Hours, ADateTime.Minutes,
    ADateTime.Seconds, ADateTime.Milliseconds);
  if (ADateTime.Month = ALunarDate.Month) and (ADateTime.Day = ALunarDate.Day)
  then
    Exit;
...
function TcxHebrewCalendarTable.GetMonthNumber(AYear: Integer;
  const S: string): Integer;
var
  I: Integer;
begin
...
procedure TcxHebrewCalendarTable.GetLunarMonthDay(AYear: Integer;
  var ADate: TcxDate);
var
  AIndex: Integer;
begin
...
begin
  case ACalendType of
    ...
    CAL_GREGORIAN_US:
      Result := TcxGregorianUSEnglishCalendarTable;
    CAL_GREGORIAN_ME_FRENCH:
      Result := TcxGregorianMiddleEastFrenchCalendarTable;
    CAL_GREGORIAN_ARABIC:
      Result := TcxGregorianArabicCalendarTable;
    CAL_GREGORIAN_XLIT_ENGLISH:
      Result := TcxGregorianTransliteratedEnglishCalendarTable;
    CAL_GREGORIAN_XLIT_FRENCH:
      Result := TcxGregorianTransliteratedFrenchCalendarTable;
    CAL_JAPAN:
      Result := TcxJapaneseCalendarTable;
    CAL_TAIWAN:
      Result := TcxTaiwanCalendarTable;
    CAL_KOREA:
      Result := TcxKoreanCalendarTable;
    CAL_HIJRI:
      Result := TcxHijriCalendarTable;
    CAL_THAI:
      Result := TcxThaiCalendarTable;
    CAL_HEBREW:
      Result := TcxHebrewCalendarTable;
  else
    Result := TcxGregorianCalendarTable;
  end;
end;
...
begin
  ...
  Result := ACalType in [CAL_GREGORIAN, CAL_GREGORIAN_US,
    CAL_GREGORIAN_ME_FRENCH, CAL_GREGORIAN_ARABIC, CAL_GREGORIAN_XLIT_ENGLISH,
    CAL_GREGORIAN_XLIT_FRENCH];
end;
