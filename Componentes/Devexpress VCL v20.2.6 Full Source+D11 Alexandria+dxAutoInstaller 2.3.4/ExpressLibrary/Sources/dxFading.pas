...
uses
  ...
  dxCore, dxCoreGraphics, dxCoreClasses, cxGraphics, dxGDIPlusClasses,
  dxGDIPlusApi, cxClasses,
...
type
  ...
  published
    property FadeInFrameCount: Integer read FFadeInFrameCount
      write SetFadeInFrameCount stored IsFadeInFrameCountStored;
    property FadeInFrameDelay: Integer read FFadeInFrameDelay
      write SetFadeInFrameDelay stored IsFadeInFrameDelayStored;
    property FadeOutFrameCount: Integer read FFadeOutFrameCount
      write SetFadeOutFrameCount stored IsFadeOutFrameCountStored;
    property FadeOutFrameDelay: Integer read FFadeOutFrameDelay
      write SetFadeOutFrameDelay stored IsFadeOutFrameDelayStored;
  end;
  ...
  protected
    ...
    procedure SetAnimationParams(AAnimationFrameCount, AAnimationFrameDelay
      : Integer);
  ...
  public
    ...
    class procedure CorrectAlphaChannel(AColors: PRGBQuad;
      AColorCount: Integer); overload;
  end;
  ...
  protected
    ...
    function CanFade(AObject: TObject; out AFadingObject: IdxFadingObject)
      : Boolean; overload;
  ...
  public
    ...
    function Find(AObject: TObject;
      out AFadingElement: TdxFadingElement): Boolean;
    ...
    property MaxAnimationCount: Integer read FMaxAnimationCount
      write SetMaxAnimationCount;
...
begin
  ...
  try
    AAlpha2 := MulDiv(Stage, dxFadeMaxAlpha, AnimationFrameCount) /
      dxFadeMaxAlpha;
    AAlpha1 := 1 - AAlpha2;
    ...
    begin
      ATempColor^.rgbBlue :=
        Round(Min(MaxByte, AColor1^.rgbBlue * AAlpha1 + AColor2^.rgbBlue *
        AAlpha2));
      ATempColor^.rgbGreen :=
        Round(Min(MaxByte, AColor1^.rgbGreen * AAlpha1 + AColor2^.rgbGreen *
        AAlpha2));
      ATempColor^.rgbRed :=
        Round(Min(MaxByte, AColor1^.rgbRed * AAlpha1 + AColor2^.rgbRed *
        AAlpha2));
      ATempColor^.rgbReserved :=
        Round(Min(MaxByte, AColor1^.rgbReserved * AAlpha1 + AColor2^.rgbReserved
        * AAlpha2));
      Inc(ATempColor);
      Inc(AColor1);
      Inc(AColor2);
    end;
    WorkImage := TdxGPImage.CreateFromBits(ImageSize.cx, ImageSize.cy,
      ATempImageColors, True);
  finally
...
procedure TdxFadingElement.SetAnimationParams(AAnimationFrameCount,
  AAnimationFrameDelay: Integer);
begin
  FAnimationFrameCount :=
    Max(1, Min(AAnimationFrameCount, dxFadeMaxAnimationFrameCount));
  FAnimationFrameDelay :=
    Max(1, Min(AAnimationFrameDelay, dxFadeMaxAnimationFrameDelay));
...
class procedure TdxFadingHelper.CorrectAlphaChannel(AColors: PRGBQuad;
  AColorCount: Integer);
begin
  while AColorCount > 0 do
  begin
    if (AColors^.rgbReserved = 0) and
      (AColors^.rgbBlue + AColors^.rgbGreen + AColors^.rgbRed > 0) then
...
function TdxFader.CanFade(AObject: TObject;
  out AFadingObject: IdxFadingObject): Boolean;
begin
  Result := IsReady and Supports(AObject, IdxFadingObject, AFadingObject) and
    AFadingObject.CanFade;
end;
...
function TdxFader.DrawFadeImage(AObject: TObject; DC: HDC;
...
begin
  ...
  begin
    ...
    try
      if Supports(AFadingObject, IdxFadingObjectFadingOptions,
        AFadingObjectFadingOptions) and
        (AFadingObjectFadingOptions.GetFadingOptions <> nil) then
        AFadingOptions.Assign(AFadingObjectFadingOptions.GetFadingOptions);

      if AState = fsFadeIn then
        Fade(AObject, ATemp1, ATemp2, AFadingOptions.FadeInFrameCount,
          AFadingOptions.FadeInFrameDelay)
      else
        Fade(AObject, ATemp2, ATemp1, AFadingOptions.FadeOutFrameCount,
          AFadingOptions.FadeOutFrameDelay);
    finally
...
procedure TdxFader.Fade(AObject: TObject; AImage1: TBitmap; AImage2: TBitmap;
  AAnimationFrameCount, AAnimationFrameDelay: Integer);

  function CanStartFading: Boolean;
  begin
    Result := CanFade(AObject) and (AImage1 <> nil) and (AImage2 <> nil) and
      (AImage1.Width = AImage2.Width) and (AImage1.Height = AImage2.Height) and
      not(AImage1.Empty or AImage2.Empty) and (AAnimationFrameCount > 0);
  end;
...
begin
  try
    ...
    begin
      ...
      else if (AElement.ImageSize.cx = AImage1.Width) and
        (AElement.ImageSize.cy = AImage1.Height) then
      begin
        AImage1.Free;
...
function TdxFader.Find(AObject: TObject;
  out AFadingElement: TdxFadingElement): Boolean;
var
  I: Integer;
begin
...
begin
  Result := Active and (MaxAnimationCount > 0) and CheckGdiPlus and
...
procedure TdxFadingObjectHelper.CheckStartFading(APrevState,
...
begin
  ...
  begin
    if (AState = cxbsHot) and (APrevState in [cxbsNormal, cxbsDefault]) then
      FadeIn
    else if (AState in [cxbsNormal, cxbsDefault]) and (APrevState = cxbsHot)
    then
      FadeOut
    else
      StopFading;
...
procedure TdxFadingObjectHelper.GetFadingImages(out AFadeOutImage,
...
begin
  FFadeInFrameDelay := Max(dxFadeTimerInterval,
    Min(AValue, dxFadeMaxAnimationFrameDelay));
end;
...
begin
  FFadeOutFrameDelay := Max(dxFadeTimerInterval,
    Min(AValue, dxFadeMaxAnimationFrameDelay));
end;
