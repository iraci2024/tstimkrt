...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Types, TypInfo, Windows, Classes, Variants, Rtti, Graphics,
  Generics.Collections, Generics.Defaults, Contnrs,
  dxCore, dxSmartImage, dxXMLDoc, dxCoreGraphics, cxGraphics, cxGeometry,
...
type
  ...
  TdxSVGFill = record
    ...
    class function Create(const AColor: TdxAlphaColor): TdxSVGFill;
      overload; static;
    class function Create(const AReference: string): TdxSVGFill;
      overload; static;
  ...
  TdxSVGValue = record
    ...
    class function Create(const AValue: Single; AUnitsType: TdxSVGUnitsType)
      : TdxSVGValue; static;
    function IsEmpty: Boolean; inline;
    function ToPixels(ATargetDPI: Integer = dxDefaultDPI): Single; overload;
    function ToPixels(const AParentSize: Single;
      ATargetDPI: Integer = dxDefaultDPI): Single; overload;
  end;
...
{$REGION 'Renderers'}
  ...
  public
    property GradientAutoScale: Boolean read FGradientAutoScale
      write FGradientAutoScale;
  ...
  public
    ...
    // Primitives
    procedure Ellipse(const ACenterX, ACenterY, ARadiusX, ARadiusY: Single);
      virtual; abstract;
    ...
    procedure RoundRect(const X, Y, AWidth, AHeight, ARadiusX,
      ARadiusY: Single); virtual; abstract;
    procedure TextOut(const X, Y: Single; const AText: string;
      AFont: TdxGPFont); virtual; abstract;
    // Opacity
    ...
    procedure SetClipRegion(APath: TdxGPPath;
      AMode: TdxGPCombineMode = gmIntersect); virtual; abstract;
    // WorldTransform
  ...
  public
    ...
    // Primitives
    procedure Ellipse(const ACenterX, ACenterY, ARadiusX,
      ARadiusY: Single); override;
...
{$REGION 'Elements'}
  ...
  protected
    ...
    procedure UpdateReference(const AOldReference, ANewReference
      : string); virtual;
    // Clipping
  ...
  public
    ...
    function FindByID(const ID: string; out AElement: TdxSVGElement)
      : Boolean; virtual;
    //
  ...
  protected
    ...
    procedure Save(const ANode: TdxXMLNode); override;
  public
    property CornerRadiusX: TdxSVGValue read FCornerRadiusX
      write FCornerRadiusX;
    property CornerRadiusY: TdxSVGValue read FCornerRadiusY
      write FCornerRadiusY;
  ...
  protected
    ...
    function TryCreateFontFamily(const AFamilyName: string;
      out AFamily: TdxGPFontFamily): Boolean;
  public
    destructor Destroy; override;
    //
...
{$REGION 'NeverRendered Elements'}
  ...
  strict private
    ...
    procedure CalculateGradientOffsets(const APatternRect,
      AGradientLine: TdxRectF; out AStartOffset, AFinishOffset: Single;
      out AInvertOrder: Boolean);
    procedure PopulateGradientPoints(APoints: TdxGPBrushGradientPoints;
      AStartOffset, AFinishOffset: Single; AInvertOrder: Boolean);
  protected
    procedure InitializeBrush(ARenderer: TdxSVGCustomRenderer); override;
    procedure InitializeBrushCore(ARenderer: TdxSVGCustomRenderer;
      ABrush: TdxSVGBrush);
    procedure InitializePen(ARenderer: TdxSVGCustomRenderer); override;
    procedure Load(const ANode: TdxXMLNode); override;
    procedure Save(const ANode: TdxXMLNode); override;
  public
    ...
    procedure AfterConstruction; override;
    //
    property Elements[Index: Integer]: TdxSVGElementLinearGradientStop
      read GetElement;
  public
    property X1: TdxSVGValue read FX1 write FX1;
    property X2: TdxSVGValue read FX2 write FX2;
    property Y1: TdxSVGValue read FY1 write FY1;
    property Y2: TdxSVGValue read FY2 write FY2;
    property GradientTransform: TdxMatrix read FGradientTransform;
...
{$ENDREGION}
  ...
  const
    StyleAttrName = 'style';
  protected
    class procedure ImportCore(AElement: TdxSVGElement; AStyles: TdxSVGStyles;
      ANode: TdxXMLNode);
    class procedure ImportNode(AParent: TdxSVGElement; AStyles: TdxSVGStyles;
      ANode: TdxXMLNode);
    class procedure ImportStyles(AStyles: TdxSVGStyles; ANode: TdxXMLNode);
  public
    class function GetSize(ADocument: TdxXMLDocument; out ASize: TSize)
      : Boolean;
    class function Import(ADocument: TdxXMLDocument;
      out ARoot: TdxSVGElementRoot): Boolean;
  end;
...
type
  ...
  public
    procedure SetAttr(const AName: TdxXMLString;
      const AValue, ADefaultValue: Single); overload;
    procedure SetAttr(const AName: TdxXMLString; const AValue: Single);
      overload;
    procedure SetAttr(const AName: TdxXMLString; const AValue: string);
      overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxAlphaColor); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxGpLineJoin); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxMatrix); overload;
    procedure SetAttr(const AName: TdxXMLString; const AValue: TdxRectF;
      AAddNewPrefix: Boolean = False); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGContentUnits); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGFill); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGFillMode); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGLineCapStyle); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGTextAnchor); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGValue); overload;
    procedure SetAttr(const AName: TdxXMLString;
      const AValue: TdxSVGValues); overload;
  end;
  ...
  protected const
    ...
    URLPart2 = ')';
  protected const
    ContentUnitsToString: array [TdxSVGContentUnits] of string =
      ('userSpaceOnUse', 'objectBoundingBox');
    FillRuleToString: array [TdxSVGFillMode] of string = ('nonzero', 'evenodd',
      'inherit');
    LineCapToString: array [TdxSVGLineCapStyle] of string = ('', 'butt',
      'square', 'round');
    LineJoinToString: array [TdxGpLineJoin] of string = ('miter', 'bevel',
      'round', '');
    TextAnchorToString: array [TdxSVGTextAnchor] of string = ('start', 'middle',
      'end', 'inherit');
  protected
    class function AlphaColorToString(const AValue: TdxAlphaColor): string;
    class function ReferenceToString(const AValue: string;
      AIsLocalReference: Boolean): string;
  ...
  public
    ...
    property ValueAsColor: TdxAlphaColor read GetValueAsColor
      write SetValueAsColor;
    property ValueAsContentUnits: TdxSVGContentUnits read GetValueAsContentUnits
      write SetValueAsContentUnits;
    property ValueAsFill: TdxSVGFill read GetValueAsFill write SetValueAsFill;
    property ValueAsFillMode: TdxSVGFillMode read GetValueAsFillMode
      write SetValueAsFillMode;
    property ValueAsLineCap: TdxSVGLineCapStyle read GetValueAsLineCap
      write SetValueAsLineCap;
    property ValueAsLineJoin: TdxGpLineJoin read GetValueAsLineJoin
      write SetValueAsLineJoin;
    property ValueAsRectF: TdxRectF read GetValueAsRectF write SetValueAsRectF;
    property ValueAsSvgValue: TdxSVGValue read GetValueAsSvgValue
      write SetValueAsSvgValue;
    property ValueAsTextAnchor: TdxSVGTextAnchor read GetValueAsTextAnchor
      write SetValueAsTextAnchor;
  end;
...
function dxGpApplyOpacity(AColor: TdxAlphaColor; AOpacity: Single)
  : TdxAlphaColor;
begin
  if (AOpacity < 1) and not TdxAlphaColors.IsTransparentOrEmpty(AColor) then
  begin
    with dxAlphaColorToRGBQuad(AColor) do
      Result := dxMakeAlphaColor(Trunc(rgbReserved * AOpacity), rgbRed,
        rgbGreen, rgbBlue);
  end
  else
    Result := AColor;
...
procedure TdxSVGFill.UpdateReference(const AOldReference,
  ANewReference: string);
begin
  if IsReference then
  begin
...
class function TdxSVGValue.Create(const AValue: Single;
  AUnitsType: TdxSVGUnitsType): TdxSVGValue;
begin
  Result.Data := AValue;
...
function TdxSVGValue.ToPixels(const AParentSize: Single;
  ATargetDPI: Integer = dxDefaultDPI): Single;
const
  CMPerInch = 2.54;
begin
  case UnitsType of
...
begin
  if (GradientPoints.Count = 0) or GradientAutoScale and
    ((FTargetRect.Width = 0) or (FTargetRect.Height = 0)) then
  begin
  ...
  TdxGPBrushGradientPointsAccess(GradientPoints).CalculateParams(AColors,
    AOffsets, ACount);
  GdipCheck(GdipCreateLineBrush(@AStartPoint, @AFinishPoint, 0, 0,
...
function TdxSVGCustomRenderer.ApplyOpacity(AColor: TdxAlphaColor)
  : TdxAlphaColor;
begin
...
constructor TdxSVGRenderer.Create(ACanvas: TdxGPCanvas;
...
procedure TdxSVGRenderer.Ellipse(const ACenterX, ACenterY, ARadiusX,
  ARadiusY: Single);
begin
  FCanvas.Ellipse(dxRectF(ACenterX - ARadiusX, ACenterY - ARadiusY,
    ACenterX + ARadiusX, ACenterY + ARadiusY), Pen, Brush);
end;
...
procedure TdxSVGRenderer.RoundRect(const X, Y, AWidth, AHeight, ARadiusX,
  ARadiusY: Single);
var
  APath: TdxGPPath;
begin
  ...
  try
    APath.AddRoundRect(dxRectF(X, Y, X + AWidth, Y + AHeight), ARadiusX,
      ARadiusY);
    Path(APath);
  finally
    APath.Free;
...
procedure TdxSVGRenderer.TextOut(const X, Y: Single; const AText: string;
  AFont: TdxGPFont);
begin
...
procedure TdxSVGRenderer.SetClipRegion(APath: TdxGPPath;
  AMode: TdxGPCombineMode = gmIntersect);
begin
...
begin
  ...
  with FCanvas.GetWorldTransform do
    try
      ...
      Result := Sqrt(Sqr(APoint.X) + Sqr(APoint.Y)) / Sqrt(2 * AWidth);
    finally
      Free;
    end;
end;
...
procedure TdxSVGRendererClipPath.Ellipse(const ACenterX, ACenterY, ARadiusX,
  ARadiusY: Single);
begin
  ClipPath.AddEllipse(dxRectF(ACenterX - ARadiusX, ACenterY - ARadiusY,
    ACenterX + ARadiusX, ACenterY + ARadiusY));
end;
...
procedure TdxSVGRendererClipPath.RoundRect(const X, Y, AWidth, AHeight,
  ARadiusX, ARadiusY: Single);
begin
  ClipPath.AddRoundRect(dxRectF(X, Y, X + AWidth, Y + AHeight), ARadiusX,
    ARadiusY);
end;

procedure TdxSVGRendererClipPath.TextOut(const X, Y: Single;
  const AText: string; AFont: TdxGPFont);
var
  ATextSize: TdxSizeF;
begin
  ATextSize := dxGpMeasureString(AFont, AText);
  ClipPath.AddString(AText, AFont, nil, dxRectF(X, Y, X + ATextSize.cx,
    Y + ATextSize.cy));
end;
...
procedure TdxSVGRendererClipPath.SetClipRegion(APath: TdxGPPath;
...
function TdxSVGElement.FindByID(const ID: string;
  out AElement: TdxSVGElement): Boolean;
var
  I: Integer;
begin
  if ID = '' then
...
procedure TdxSVGElement.UpdateReference(const AOldReference,
  ANewReference: string);
var
  I: Integer;
begin
...
procedure TdxSVGElement.ApplyClipping(ARenderer: TdxSVGCustomRenderer);
  ...
  function GetEffect(const ID: string;
    out AClipPath: TdxSVGElementClipPath): Boolean;
  var
    AElement: TdxSVGElement;
  begin
    Result := (ID <> '') and Root.FindByID(ID, AElement) and
      (AElement is TdxSVGElementClipPath);
    if Result then
...
begin
  ...
  begin
    ...
    if FillOpacity <> 1 then
      AColor := dxMakeAlphaColor(dxAlphaColorToColor(AColor),
        Round(MaxByte * FillOpacity));
...
const
  LineCapMap: array [TdxSVGLineCapStyle] of TdxGPPenLineCapStyle = (gpcsFlat,
    gpcsFlat, gpcsSquare, gpcsRound);
var
...
begin
  ...
  ANode.SetAttr('clip-path', TdxSVGAttributeConverter.ReferenceToString
    (ClipPath, False));
...
begin
  if ARenderer.Brush.IsEmpty then
...
begin
  if CornerRadiusX.IsEmpty and CornerRadiusY.IsEmpty then
    ...
    ARenderer.RoundRect(GetX(X), GetY(Y), GetX(Width), GetY(Height),
      GetX(CornerRadiusX), GetY(CornerRadiusY));
end;
...
begin
  Result := Max(dxGpMeasureString(AFont, '|').cx - dxGpMeasureString(AFont,
    '||').cx / 2, 0);
end;
...
begin
  if TextAssigned and TryCreateFont(AFont) then
    try
      AOffset := AFont.Size * AFont.FontFamily.GetCellAscent(AFont.Style) /
        AFont.FontFamily.GetEmHeight(AFont.Style);
      APoint := dxPointF(GetX(X), GetY(Y) - AOffset);
      case TextAnchor of
        taMiddle:
          APoint.X := APoint.X - dxGpMeasureString(AFont, Text).cx / 2;
        taEnd:
          APoint.X := APoint.X - dxGpMeasureString(AFont, Text).cx +
            CalculateTextPadding(AFont);
        taStart:
          APoint.X := APoint.X - CalculateTextPadding(AFont);
      end;
...
begin
  ...
  if TdxSVGFontStyleString.TryStringToFontStyle
    (ANode.Attributes.GetValueAsString('font-style'), AStyle) then
    FontStyles := FontStyles + [AStyle];
  if TdxSVGFontStyleString.TryStringToFontStyle
    (ANode.Attributes.GetValueAsString('font-weight'), AStyle) then
    FontStyles := FontStyles + [AStyle];
...
begin
  ...
  try
    // font
    ABuffer.Append('font: ');
    if fsItalic in ActualFontStyles then
      ABuffer.Append(TdxSVGFontStyleString.NameMap[fsItalic]).Append(' ');
    if fsBold in ActualFontStyles then
      ABuffer.Append(TdxSVGFontStyleString.NameMap[fsBold]).Append(' ');
    ABuffer.Append(dxFloatToStr(ActualFontSize)).Append('px').Append(' ');
    ABuffer.Append(ActualFontName);
    ABuffer.Append('; ');
...
begin
  ...
  begin
    AFontName := ActualFontName;
    if not TryCreateFontFamily(AFontName, FCachedFontFamily) then
      repeat
    ...
    if (FCachedFontFamily = nil) and ContainsText('sans-serif', ActualFontName)
    then
      FCachedFontFamily := TdxGPFontFamily.GenericSansSerif.Clone;
    if (FCachedFontFamily = nil) and ContainsText('serif', ActualFontName) then
...
function TdxSVGElementText.TryCreateFontFamily(const AFamilyName: string;
  out AFamily: TdxGPFontFamily): Boolean;
begin
...
procedure TdxSVGElementText.SetFontStyles(const Value: TFontStyles);
begin
  FFontStyles := Value;
...
begin
  ...
  begin
    ARenderer.SaveWorldTransform;
    try
      ARenderer.ModifyWorldTransform(TXForm.CreateTranslateMatrix(GetX(X),
        GetY(Y)));
...
begin
  ...
  if ANode.Attributes.Find('y', AAttr) then
    Y := AAttr.ValueAsSvgValue;
  if ANode.Attributes.Find('href', AAttr) or ANode.Attributes.Find('xlink:href',
    AAttr) then
    Reference := TdxSVGAttributeConverter.StringToReference
      (AAttr.ValueAsString);
end;
...
begin
  ...
  ANode.SetAttr('x', X);
  ANode.SetAttr('y', Y);
  ANode.SetAttr('href', TdxSVGAttributeConverter.ReferenceToString
    (Reference, True));
end;

procedure TdxSVGElementUse.UpdateReference(const AOldReference,
  ANewReference: string);
begin
...
procedure TdxSVGElementClipPath.ApplyTo(ARenderer: TdxSVGCustomRenderer;
  AElement: TdxSVGElement);
var
...
constructor TdxSVGElementLinearGradient.Create;
begin
  inherited;
  FGradientTransform := TdxMatrix.Create;
end;

destructor TdxSVGElementLinearGradient.Destroy;
begin
  FreeAndNil(FGradientTransform);
  inherited;
end;
...
procedure TdxSVGElementLinearGradient.InitializeBrush
  (ARenderer: TdxSVGCustomRenderer);
begin
  InitializeBrushCore(ARenderer, ARenderer.Brush);
end;

procedure TdxSVGElementLinearGradient.InitializeBrushCore
  (ARenderer: TdxSVGCustomRenderer; ABrush: TdxSVGBrush);
...
begin
  ...
  APatternRect := GradientTransform.Transform(APatternRect);
  ...
  begin
    ...
    if ABrush.GradientPoints.Count > 0 then
      ABrush.Color := ARenderer.ApplyOpacity
        (ABrush.GradientPoints.Colors[ABrush.GradientPoints.Count - 1])
    else
  ...
  begin
    CalculateGradientOffsets(APatternRect, ABrush.GradientLine, AStartOffset,
      AFinishOffset, AInvertOrder);
    PopulateGradientPoints(ABrush.GradientPoints, AStartOffset,
      AFinishOffset, False);
    for I := 0 to ABrush.GradientPoints.Count - 1 do
      ABrush.GradientPoints.Colors[I] :=
        ARenderer.ApplyOpacity(ABrush.GradientPoints.Colors[I]);
    ABrush.Style := gpbsGradient;
  end;
  ABrush.HandleNeeded;
end;

procedure TdxSVGElementLinearGradient.InitializePen
  (ARenderer: TdxSVGCustomRenderer);
begin
...
begin
  ...
  if ANode.Attributes.Find('y2', AAttr) then
    Y2 := AAttr.ValueAsSvgValue;
  if ANode.Attributes.Find('gradientTransform', AAttr) then
    AAttr.GetValueAsMatrix(GradientTransform);
...
begin
  ...
  ANode.SetAttr('x2', X2);
  ANode.SetAttr('y2', Y2);
  ANode.SetAttr('gradientTransform', GradientTransform);
  ANode.SetAttr('gradientUnits', UnitsType);
end;

function TdxSVGElementLinearGradient.CalculateGradientLine(const APatternRect
  : TdxRectF): TdxRectF;

  procedure AdjustPoint(var X, Y: Single; const ACanvasRect: TdxRectF;
...
procedure TdxSVGElementLinearGradient.CalculateGradientOffsets
  (const APatternRect, AGradientLine: TdxRectF;
...
begin
  ...
  AFinishOffset := CalculatePosition(APatternRect.Right - AGradientLine.Left,
    APatternRect.Bottom - AGradientLine.Top) / ALength;
  AStartOffset := CalculatePosition(APatternRect.Left - AGradientLine.Left,
    APatternRect.Top - AGradientLine.Top) / ALength;
  AInvertOrder := False;
end;
...
procedure TdxSVGElementLinearGradient.PopulateGradientPoints
  ...
  function GetActualColor(AElement: TdxSVGElementLinearGradientStop)
    : TdxAlphaColor;
  begin
...
function TdxSVGElementLinearGradient.GetElement(Index: Integer)
  : TdxSVGElementLinearGradientStop;
begin
...
function TdxSVGStyles.TryGetStyle(const AName: string;
  out AStyle: TdxSVGStyle): Boolean;
begin
...
class function TdxSVGImporter.GetSize(ADocument: TdxXMLDocument;
...
class function TdxSVGImporter.Import(ADocument: TdxXMLDocument;
  out ARoot: TdxSVGElementRoot): Boolean;
var
  ANode: TdxXMLNode;
...
class procedure TdxSVGImporter.ImportCore(AElement: TdxSVGElement;
  AStyles: TdxSVGStyles; ANode: TdxXMLNode);
var
  AAttr: TdxXMLNodeAttribute;
begin
  AElement.Load(ANode);
  if ANode.Attributes.Find(StyleAttrName, AAttr) then
    TdxSVGParserInlineStyle.Parse(AStyles.AddInline(AElement),
      AAttr.ValueAsString);
...
class procedure TdxSVGImporter.ImportNode(AParent: TdxSVGElement;
  AStyles: TdxSVGStyles; ANode: TdxXMLNode);
var
  AElement: TdxSVGElement;
...
begin
  ...
  else if FElementMap.TryGetValue(ANode.Name, AElementClass) then
  begin
...
class procedure TdxSVGImporter.ImportStyles(AStyles: TdxSVGStyles;
  ANode: TdxXMLNode);
...
class procedure TdxSVGExporter.Export(ARoot: TdxSVGElementRoot;
...
class procedure TdxSVGExporter.ExportCore(AElement: TdxSVGElement;
  ANode: TdxXMLNode);
var
  I: Integer;
begin
...
class procedure TdxSVGExporter.ExportElement(AElement: TdxSVGElement;
  AParent: TdxXMLNode);
var
  ANodeName: TdxXMLString;
begin
  if FElementMap.TryGetKey(TdxSVGElementClass(AElement.ClassType), ANodeName)
  then
...
class function TdxSVGAttributeConverter.AlphaColorToString
  (const AValue: TdxAlphaColor): string;
begin
...
class function TdxSVGAttributeConverter.ReferenceToString(const AValue: string;
  AIsLocalReference: Boolean): string;
begin
  if AValue <> '' then
  begin
...
class function TdxSVGAttributeConverter.StringToAlphaColor(const AValue: string)
  : TdxAlphaColor;
var
  ARgbMacro: Integer;
...
begin
  ...
  if ARgbMacro > 0 then
  begin
    ARgbValues := TdxSVGParserNumbers.AsNumbers
      (Copy(AValue, ARgbMacro + Length(RGBMacro), MaxInt), MaxByte);
    try
      if ARgbValues.Count = 3 then
        Result := TdxAlphaColors.FromArgb(MaxByte, Round(ARgbValues[0]),
          Round(ARgbValues[1]), Round(ARgbValues[2]))
      else
  ...
  else if AValue = 'inherit' then
    Result := TdxAlphaColors.Default
  else
    Result := TdxAlphaColors.FromHtml(AValue);
end;

class function TdxSVGAttributeConverter.StringToContentUnits(const S: string)
  : TdxSVGContentUnits;
var
...
class function TdxSVGAttributeConverter.StringToFillRule(const S: string)
  : TdxSVGFillMode;
var
...
class function TdxSVGAttributeConverter.StringToLineCap(const S: string)
  : TdxSVGLineCapStyle;
var
...
class function TdxSVGAttributeConverter.StringToLineJoin(const S: string)
  : TdxGpLineJoin;
var
...
class function TdxSVGAttributeConverter.StringToTextAnchor(const S: string)
  : TdxSVGTextAnchor;
var
...
begin
  ...
  if StartsText(URLPart1, S) and EndsText(URLPart2, S) then
    S := Copy(S, Length(URLPart1) + 1, Length(S) - Length(URLPart1) -
      Length(URLPart2));
...
begin
  ...
  begin
    ...
    Result := TdxSVGFill.Create
      (TdxSVGAttributeConverter.StringToAlphaColor(AValue));
end;
...
begin
  if AValue.IsDefault then
    ...
    ValueAsString := TdxSVGAttributeConverter.ReferenceToString
      (AValue.AsReference, False)
  else if AValue.IsEmpty then
    ValueAsString := 'none'
  else
    ValueAsString := TdxSVGAttributeConverter.AlphaColorToString
      (AValue.AsColor);
end;

procedure TdxSVGAttributeAdapter.SetValueAsFillMode
  (const Value: TdxSVGFillMode);
begin
  ValueAsString := TdxSVGAttributeConverter.FillRuleToString[Value];
end;

procedure TdxSVGAttributeAdapter.SetValueAsLineCap
  (const Value: TdxSVGLineCapStyle);
begin
...
begin
  ValueAsString := dxSVGFloatToString(Value.Data) + TdxSVGParserValue.NameMap
    [Value.UnitsType];
end;

procedure TdxSVGAttributeAdapter.SetValueAsTextAnchor
  (const Value: TdxSVGTextAnchor);
begin
  ValueAsString := TdxSVGAttributeConverter.TextAnchorToString[Value];
end;

procedure TdxSVGAttributeAdapter.SetValueAsContentUnits
  (const Value: TdxSVGContentUnits);
begin
...
procedure TdxSVGNodeAdapter.SetAttr(const AName: TdxXMLString;
  const AValue: Single);
begin
...
procedure TdxSVGNodeAdapter.SetAttr(const AName: TdxXMLString;
  const AValue, ADefaultValue: Single);
begin
...
procedure TdxSVGNodeAdapter.SetAttr(const AName: TdxXMLString;
  const AValue: TdxSVGLineCapStyle);
begin
...
procedure TdxSVGNodeAdapter.SetAttr(const AName: TdxXMLString;
  const AValue: TdxSVGContentUnits);
begin
...
procedure TdxSVGNodeAdapter.SetAttr(const AName: TdxXMLString;
  const AValue: TdxRectF; AAddNewPrefix: Boolean);
var
  AAttr: TdxXMLNodeAttribute;
begin
