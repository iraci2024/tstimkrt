...
function CustomizationTreeViewWndProcHandler(ATreeView: TTreeView;
  var Message: TMessage): Boolean;
function cxTreeViewFindNodeByData(ATreeView: TTreeView; AData: Pointer;
  var ANode: TTreeNode): Boolean;
function cxTreeViewGetHitNode(ATreeView: TTreeView; const ACursorPos: TPoint)
  : TTreeNode;
...
function cxTreeViewFindNodeByData(ATreeView: TTreeView; AData: Pointer;
  ...
  begin
    ...
    if ANode.Data = AData then
      Result := ANode
    else if ANode.HasChildren then
...
procedure cxTreeViewSetSelection(ATreeView: TTreeView; AList: TList);
  ...
  procedure InternalSetSelection(ANode: TTreeNode; ASelection: TList);
  var
    I: Integer;
  begin
    if (ANode = nil) or (ASelection.Count = 0) then
      Exit;
    if ANode.Data <> nil then
    begin
      I := ASelection.IndexOf(ANode.Data);
      if I >= 0 then
      begin
    ...
    if ASelection.Count > 0 then
      for I := 0 to ANode.Count - 1 do
        InternalSetSelection(ANode[I], ASelection);
  end;
...
begin
  ...
  try
    ...
    for I := ATreeView.SelectionCount - 1 downto 0 do
    begin
      ANode := ATreeView.Selections[I];
    ...
    if ASelection.Count > 0 then
      for I := 0 to ATreeView.Items.Count - 1 do
        InternalSetSelection(ATreeView.Items[I], ASelection);
