...
type
  ...
  TdxCalloutPopupAlignment = (cpaLeftBottom, cpaLeftCenter, cpaLeftTop,
    cpaTopLeft, cpaTopCenter, cpaTopRight, cpaRightTop, cpaRightCenter,
    cpaRightBottom, cpaBottomRight, cpaBottomCenter, cpaBottomLeft);
  ...
  protected
    ...
    procedure DoCalculateCalloutPositionAndVisibility(const APopupPos,
      AOrigin: TPoint; const APopupSize: TSize; var ACalloutPosition: Integer;
      var AIsCalloutVisible: Boolean); virtual;
  ...
  public
    ...
    procedure CalculateCalloutPositionAndVisibility(const APopupPos,
      AOrigin: TPoint; const APopupSize: TSize); virtual;
  ...
  published
    property FadeEffect: Boolean read FFadeEffect write FFadeEffect
      default True;
    property HidingAnimationTime: Cardinal read FHidingAnimationTime
      write FHidingAnimationTime default 100;
    property MoveEffect: Boolean read FMoveEffect write FMoveEffect
      default True;
    property ShowingAnimationTime: Cardinal read FShowingAnimationTime
      write FShowingAnimationTime default 200;
  end;
  ...
  strict private
    ...
    function GetAnimationTime(AnimationType: TdxCalloutPopupAnimationType)
      : Cardinal;
    procedure RestorePopupControlData;
    procedure SavePopupControlData;
    procedure SetAnimationTime(AnimationType: TdxCalloutPopupAnimationType;
  ...
  protected
    ...
    procedure DoHideAnimation(Sender: TdxAnimationTransition;
      var APosition: Integer; var AFinished: Boolean); virtual;
    procedure DoShowAnimation(Sender: TdxAnimationTransition;
      var APosition: Integer; var AFinished: Boolean); virtual;
    ...
    property AnimationTime[AnimationType: TdxCalloutPopupAnimationType]
      : Cardinal read GetAnimationTime write SetAnimationTime;
    ...
    property FlyoutPanelAlign: TdxFlyoutPanelAlign read FFlyoutPanelAlign
      write FFlyoutPanelAlign;
    property FlyoutPanelMode: Boolean read FFlyoutPanelMode
      write FFlyoutPanelMode;
  ...
  protected
    function CreatePopupWindow(AOwnerControl: TWinControl)
      : TdxCustomCalloutPopupWindow; virtual;
  ...
  public
    ...
    procedure Popup(AOwnerControl: TWinControl;
      const AOwnerBounds: TRect); overload;
    procedure Popup(AOwnerControl: TWinControl); overload;

    property AnimationOptions: TdxCalloutPopupAnimationOptions
      read FAnimationOptions write SetAnimationOptions;
    property BorderColor: TColor read FBorderColor write FBorderColor
      default clDefault;
    property HideCallout: Boolean read FHideCallout write FHideCallout
      default False;
    ...
    property Alignment: TdxCalloutPopupAlignment read FAlignment
      write FAlignment default cpaBottomCenter;
...
function dxRightToLeftConvertCalloutPopupAlignment(const AAlignment
  : TdxCalloutPopupAlignment): TdxCalloutPopupAlignment;
begin
  Result := AAlignment;
  case Result of
    cpaLeftBottom:
      Result := cpaRightBottom;
    cpaLeftCenter:
      Result := cpaRightCenter;
    cpaLeftTop:
      Result := cpaRightTop;
    cpaTopLeft:
      Result := cpaTopRight;
    ...
    cpaRightTop:
      Result := cpaLeftTop;
    cpaRightCenter:
      Result := cpaLeftCenter;
    cpaRightBottom:
      Result := cpaLeftBottom;
    cpaBottomRight:
      Result := cpaBottomLeft;
    cpaBottomLeft:
      Result := cpaBottomRight;
  end;
end;

function dxRightToLeftConvertFlyoutPanelAlign(const AAlign: TdxFlyoutPanelAlign)
  : TdxFlyoutPanelAlign;
begin
  Result := AAlign;
  case Result of
...
constructor TdxCalloutPopupViewInfo.Create(APopup: TdxCustomCalloutPopupWindow);
begin
  inherited Create;
  FPopup := APopup;
end;

procedure TdxCalloutPopupViewInfo.Calculate(const ABounds: TRect);
begin
  FBounds := ABounds;
...
begin
  if FPopup.CalloutVisible then
    DoCalculateCalloutPositionAndVisibility(APopupPos, AOrigin, APopupSize,
      FCalloutPosition, FIsCalloutVisible)
  else
    FIsCalloutVisible := False;
end;

procedure TdxCalloutPopupViewInfo.CalculateRequiredSize
...
begin
  if FPopup.Rounded then
    Result := CreateRoundRectRgn(FBorderBounds.Left, FBorderBounds.Top,
      FBorderBounds.Right + 1, FBorderBounds.Bottom + 1, FPopup.RoundRadius,
      FPopup.RoundRadius)
  else
...
begin
  ...
  else if FPopup.AlignHorz = pahLeft then
...
begin
  ...
  begin
    ...
    Result[1] := cxPoint(FCalloutBounds.Left, FCalloutBounds.Top +
      ACalloutHalfSize);
    Result[2] := cxPoint(FCalloutBounds.Right, FCalloutBounds.Top);
  end;
end;
...
procedure TdxCalloutPopupViewInfo.DoCalculateCalloutPositionAndVisibility
  ...
  begin
    ...
    AIsCalloutVisible := ((ACalloutPopupSize - 2 * ACalloutElementMinOffset) >=
      ACalloutElementSize) and
      ((ACalloutPopupStartCoordinate + ACalloutElementMinOffset) <=
      (AOwnerStartCoordinate + AOwnerSize - ACalloutElementHalfSize)) and
      ((ACalloutPopupStartCoordinate + ACalloutPopupSize -
      ACalloutElementMinOffset) >= (AOwnerStartCoordinate +
      ACalloutElementHalfSize));
    if AIsCalloutVisible then
    begin
      AOwnerCenterCoordinate := AOwnerStartCoordinate +
        cxHalfCoordinate(AOwnerSize);
      ACalloutPosition := AOwnerCenterCoordinate - ACalloutPopupStartCoordinate
        - ACalloutElementHalfSize;
      ACalloutPosition := EnsureRange(ACalloutPosition,
        ACalloutElementMinOffset, ACalloutPopupSize - ACalloutElementMinOffset -
        ACalloutElementSize);
    end;
  end;
...
begin
  ...
  if FPopup.Direction = pdVertical then
    DirectionIndependentCalculateCalloutPositionAndVisibility(APopupSize.cx,
      APopupPos.X, cxRectWidth(R), R.Left, ACalloutPosition, AIsCalloutVisible)
  else
    DirectionIndependentCalculateCalloutPositionAndVisibility(APopupSize.cy,
      APopupPos.Y, cxRectHeight(R), R.Top, ACalloutPosition, AIsCalloutVisible);
end;
...
function TdxCalloutPopupViewInfo.DoCalculateRequiredSize(const AAvailableSize
  : TSize): TSize;
var
...
const
  AAnchors: array [TdxFlyoutPanelAlign] of TAnchors = ([akRight, akBottom],
    [akLeft, akTop], [akRight, akBottom], [akLeft, akBottom]);
...
procedure TdxCustomCalloutPopupWindow.AdjustClientRect(var Rect: TRect);
begin
  Rect := FViewInfo.PopupControlBounds;
end;

procedure TdxCustomCalloutPopupWindow.CalculateCommonPosition
  (var APosition: TPoint; var AOrigin: TPoint; const ASize: TSize);
begin
...
function TdxCustomCalloutPopupWindow.CalculatePosition
...
begin
  ...
  begin
    ...
    FViewInfo.CalculateCalloutPositionAndVisibility(Result,
      OwnerScreenBounds.TopLeft, ASize);
...
begin
  ...
  else if (Direction = pdHorizontal) and (AlignVert = pavCenter) then
  begin
...
begin
  ...
  dxAssignFont(Font, TWinControlAccess(PopupControl).Font, ScaleFactor,
    dxGetScaleFactor(GetParentForm(PopupControl)));
  PopupControl.Parent := Self;
  PopupControl.Visible := True;
  Width := 0;
end;

procedure TdxCustomCalloutPopupWindow.
  InternalCalculateHorizontalDirectionPosition(var APosition, AOrigin: TPoint;
  const ASize: TSize);
begin
  inherited;
  if cxRectHeight(OwnerScreenBounds) < ViewInfo.GetCalloutElementSize.cx then
    if AlignVert = pavTop then
      APosition.Y := cxRectCenter(OwnerScreenBounds).Y -
        cxHalfCoordinate(ViewInfo.GetCalloutElementSize.cx)
    else if AlignVert = pavBottom then
      APosition.Y := cxRectCenter(OwnerScreenBounds).Y +
        cxHalfCoordinate(ViewInfo.GetCalloutElementSize.cx) - ASize.cy;
end;
...
begin
  ...
  if cxRectWidth(OwnerScreenBounds) < ViewInfo.GetCalloutElementSize.cx then
    if AlignHorz = pahLeft then
      APosition.X := cxRectCenter(OwnerScreenBounds).X -
        cxHalfCoordinate(ViewInfo.GetCalloutElementSize.cx)
    else if AlignHorz = pahRight then
      APosition.X := cxRectCenter(OwnerScreenBounds).X +
        cxHalfCoordinate(ViewInfo.GetCalloutElementSize.cx) - ASize.cx;
end;
...
procedure TdxCustomCalloutPopupWindow.DoAnimate(AAnimationType
  : TdxCalloutPopupAnimationType);
begin
  FPosition := -1;
  with TdxAnimationTransition.Create(AnimationTime[AAnimationType],
    TransitionEffect, 255) do
  begin
...
procedure TdxCustomCalloutPopupWindow.DoFlyoutPanelMoveAnimation
  (APosition: Integer);
var
...
function TdxCustomCalloutPopupWindow.GetAnimationTime(AnimationType
  : TdxCalloutPopupAnimationType): Cardinal;
begin
...
begin
  ...
  begin
    ...
    if PopupControl is TCustomForm then
      TCustomForm(PopupControl).BorderStyle :=
        FPrevPopupControlData.BorderStyle;
...
procedure TdxCustomCalloutPopupWindow.SetAnimationTime(AnimationType
  : TdxCalloutPopupAnimationType; const Value: Cardinal);
begin
...
procedure TdxCustomCalloutPopup.Popup(AOwnerControl: TWinControl;
  const AOwnerBounds: TRect);
begin
  if CanPopup(AOwnerControl) then
  begin
    if (PopupControl = AOwnerControl) or IsRelatedWindow(PopupControl,
      AOwnerControl.Handle) then
...
function TdxCustomCalloutPopup.CreatePopupWindow(AOwnerControl: TWinControl)
  : TdxCustomCalloutPopupWindow;
begin
...
begin
  ...
  if FCallout.UseRightToLeftAlignment then
    FCallout.FlyoutPanelAlign := dxRightToLeftConvertFlyoutPanelAlign
      (FlyoutPanel.Align)
  else
...
begin
  ...
  if (Operation = opRemove) then
    if AComponent = FPopupControl then
      PopupControl := nil
    else if (FCallout <> nil) and (AComponent = FCallout.OwnerControl) then
      DestroyPopup;
end;
...
procedure TdxCustomCalloutPopup.RecreatePopupIfNeeded(AOwnerControl
  : TWinControl);
begin
  if (FCallout = nil) or (FCallout.OwnerControl <> AOwnerControl) then
  begin
    if FCallout <> nil then
    begin
