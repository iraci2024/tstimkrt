...
type
  TfrmGalleryDesigner = class(TcxDesignFormEditor)
    ...
    procedure tvGalleryItemsDragOver(Sender, Source: TObject; X, Y: Integer;
    ...
    procedure tvGalleryItemsEdited(Sender: TObject; Node: TTreeNode;
      var S: string);
    procedure tvGalleryItemsEditing(Sender: TObject; Node: TTreeNode;
  ...
  private
    ...
    function CalculateNextNodeIndex(ANode: TTreeNode;
      AIsMovingUp: Boolean): Integer;
    ...
    procedure GroupItemNodeDragDrop(ASourceNode: TTreeNode;
      var ATargetNode: TTreeNode; AIsCoping: Boolean);
    ...
    procedure MoveItem(AItem: IdxGalleryItem; ATargetGroup: IdxGalleryGroup;
      AIndex: Integer);
    function CopyItem(AItem: IdxGalleryItem; ATargetGroup: IdxGalleryGroup;
      AIndex: Integer): IdxGalleryItem;
...
procedure EditGallery(AGalleryOwner: TComponent);
begin
  ShowFormEditorClass(GetObjectDesigner(AGalleryOwner), AGalleryOwner,
    TfrmGalleryDesigner);
end;
...
procedure TfrmGalleryDesigner.SelectionsChanged(const ASelection
...
begin
  ...
  actMoveNodeUp.Enabled := (ASelectedNode <> nil) and
    (IsGroup(ASelectedNode) and (ASelectedNode.AbsoluteIndex > 0) or
    IsGroupItem(ASelectedNode) and (ASelectedNode.AbsoluteIndex > 1)) and
    (tvGalleryItems.SelectionCount <= 1) and CanModify;
  actMoveNodeDown.Enabled := (ASelectedNode <> nil) and
    not((ASelectedNode.AbsoluteIndex = tvGalleryItems.Items.Count - 1) or
    IsGroup(ASelectedNode) and (ASelectedNode.getNextSibling = nil)) and
    (tvGalleryItems.SelectionCount <= 1) and CanModify;
  ...
  begin
    if not Supports(AValue, IdxGallery, FGallery) then
      if Supports(AValue, IdxGalleryOwner, AGalleryOwner) then
        ...
        raise EdxException.Create('Gallery Designer fails: ' +
          AValue.ClassName);
  end
  else
    FGallery := nil;
...
procedure TfrmGalleryDesigner.UpdatePopupMenuItemsVisibility
  (ASelectedNode: TTreeNode);
var
  I: Integer;
begin
  if ASelectedNode <> nil then
    for I := 0 to pmGalleryItems.Items.Count - 1 do
      pmGalleryItems.Items[I].Visible := IsGroup(ASelectedNode) or
        IsGroupItem(ASelectedNode) and
        (pmGalleryItems.Items[I].Tag = dxcGroupItemLevel);
end;
...
function TfrmGalleryDesigner.CalculateNextNodeIndex(ANode: TTreeNode;
  AIsMovingUp: Boolean): Integer;
begin
...
begin
  ...
  try
    AAddedNode := tvGalleryItems.Items.Add(nil, FGallery.Groups.GetDisplayName +
      IntToStr(FGallery.Groups.Count));
...
procedure TfrmGalleryDesigner.AddGroupItemNode(ANode: TTreeNode);
  ...
  function GetDefaultItemCaption(const APrefix: string;
...
procedure TfrmGalleryDesigner.MoveGroupNode(ANode: TTreeNode;
  AIsMovingUp: Boolean);
var
  AIndex: Integer;
begin
...
procedure TfrmGalleryDesigner.MoveGroupItemNode(ANode: TTreeNode;
  AIsMovingUp: Boolean);
type
  TdxGroupChangeType = (gcNone, gcIncreased, gcDecreased);

  function CalculateGroupChangeType(AParentNode: TTreeNode;
    out ANodeIndex, AGroupIndex: Integer): TdxGroupChangeType;
  begin
    Result := gcNone;
    ...
    else if ANodeIndex > ANode.Parent.Count - 1 then
    begin
      Inc(AGroupIndex);
      Result := gcIncreased;
      ANodeIndex := 0;
  ...
  function CreateNode(AParentNode: TTreeNode; ANodeIndex: Integer;
    AGroup: IdxGalleryGroup; AChangeType: TdxGroupChangeType): TTreeNode;
  begin
    if (ANodeIndex = AGroup.GetItems.Count - 1) and
      not((AChangeType = gcIncreased) and (AParentNode.Count = 1)) or
    ...
    begin
      ...
      Result := tvGalleryItems.Items.Insert
        (AParentNode.Item[ANodeIndex + IfThen(AIsMovingUp or
        (AChangeType = gcIncreased), 0, 1)], ANode.Text);
  end;
...
begin
  ...
  try
    ...
    AGroupChangeType := CalculateGroupChangeType(AParentNode, ANodeIndex,
      AGroupIndex);
    ...
    begin
      case AGroupChangeType of
        ...
        gcDecreased:
          AParentNode := AParentNode.getPrevSibling;
      end;
      ...
      tvGalleryItems.Items.BeginUpdate;
      try
        ACreatedNode := CreateNode(AParentNode, ANodeIndex, AGroup,
          AGroupChangeType);
        AGroupItem := GetGalleryItem(ANode);
        MoveItem(AGroupItem, AGroup,
          ANodeIndex + IfThen(AGroupChangeType = gcDecreased, 1, 0));
...
procedure TfrmGalleryDesigner.SelectNextNode(ADeletingNodes: TList);
  ...
  begin
    Result := (ACurrentNode <> nil) and
      (ADeletingNodes.IndexOf(ACurrentNode) <> -1);
  end;
...
begin
  ...
  begin
    ...
    else if AFirstNode.Parent <> nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        for I := 0 to tvGalleryItems.Items.Count - 1 do
          if tvGalleryItems.Items[I].Selected and
            IsGroupItem(tvGalleryItems.Items[I]) then
            ASourceNodes.Add(tvGalleryItems.Items[I]);

        for I := 0 to ASourceNodes.Count - 1 do
          GroupItemNodeDragDrop(TTreeNode(ASourceNodes[I]), ATargetNode,
            AIsCoping);
      finally
        ASourceNodes.Free;
      end;
...
procedure TfrmGalleryDesigner.GroupItemNodeDragDrop(ASourceNode: TTreeNode;
  var ATargetNode: TTreeNode; AIsCoping: Boolean);

  procedure ChangeGalleryItem(ACreatedNode: TTreeNode; AGroup: IdxGalleryGroup;
    AIndex: Integer; AIsCoping: Boolean);
  var
  ...
  begin
    Result := IsGroupItem(ATargetNode) and
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ACreatedNode := tvGalleryItems.Items.AddChild(ATargetNode,
          ASourceNode.Text);
      end
      else
      begin
        ACreatedNode := tvGalleryItems.Items.Insert(ATargetNode,
          ASourceNode.Text);
    ...
    begin
      ...
      AIndex := ATargetNode.Count - 1 -
        IfThen((ASourceNode.Parent = ATargetNode) and not AIsCoping, 1, 0);
...
procedure TfrmGalleryDesigner.GroupNodeDragDrop(ASourceNode,
...
procedure TfrmGalleryDesigner.SynchronizeSelection(const ASelection
  : TDesignerSelectionList);
var
  AList: TList;
begin
...
procedure TfrmGalleryDesigner.SetGalleryGroupIndex(ANode: TTreeNode;
  AIndex: Integer);
...
procedure TfrmGalleryDesigner.SetObjectCaption(ANode: TTreeNode;
  var ACaption: string);
begin
  if IsGroup(ANode) then
...
procedure TfrmGalleryDesigner.MoveItem(AItem: IdxGalleryItem;
  ATargetGroup: IdxGalleryGroup; AIndex: Integer);
begin
  AItem.Items := ATargetGroup.GetItems;
  AItem.Index := AIndex;
end;

function TfrmGalleryDesigner.CopyItem(AItem: IdxGalleryItem;
  ATargetGroup: IdxGalleryGroup; AIndex: Integer): IdxGalleryItem;
begin
...
function TfrmGalleryDesigner.TreeViewWndProcHandler(ATreeView: TTreeView;
  var Message: TMessage): Boolean;
...
procedure TfrmGalleryDesigner.tvGalleryItemsDragOver(Sender, Source: TObject;
...
begin
  ...
  Accept := (Sender = tvGalleryItems) and (ASelectedNode <> nil) and
    (ATargetNode <> nil) and (IsGroupItem(ASelectedNode) or IsGroup(ATargetNode)
    ) and CanModify;
end;

procedure TfrmGalleryDesigner.tvGalleryItemsDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
...
procedure TfrmGalleryDesigner.tvGalleryItemsEditing(Sender: TObject;
...
begin
  ppmiCopy.Enabled := (tvGalleryItems.Selected <> nil) and
    IsGroupItem(tvGalleryItems.Selected);
end;
