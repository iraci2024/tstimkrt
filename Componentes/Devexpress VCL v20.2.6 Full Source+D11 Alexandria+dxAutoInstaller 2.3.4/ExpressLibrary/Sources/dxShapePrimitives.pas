...
type
  ...
  TdxShapeTransformationType = (dxsttScale, dxsttSkew, dxsttRotate,
    dxsttTranslate);
  ...
  published
    property RenderTransformOrigin: string read GetRenderTransformOrigin
      write SetRenderTransformOrigin;
  end;
  ...
  private
    ...
    procedure ApplyTransformation(ATransformation
      : TdxShapeCustomTransformation);
    procedure BrushChangeHandler(ASender: TObject);
    procedure CalculateTransformMatrix;
    procedure DoApplyTransformation(AType: TdxShapeTransformationType;
  ...
  protected
    ...
    procedure SetShapeColor(AAlphaColor: TdxAlphaColor;
  ...
  private
    ...
    procedure ReadAttribute(ABaseShape: TdxShapeObject; APropList: PPropList;
      APropCount: Integer; AAttribute: TdxXMLNodeAttribute);
  protected
    function CreateShapeFromNode(AParentShape: TdxShapeObject;
      ANode: TdxXMLNode): TdxShapeObject;
    procedure ReadAttributes(ABaseShape: TdxShapeObject;
      ANode: TdxXMLNode); virtual;
    procedure ReadChildNodes(AParentShape: TdxShapeObject;
      ANode: TdxXMLNode); virtual;
  public
  ...
  public
    ...
    property Count: Integer read GetCount;
    property Items[Index: Integer]: TdxShapeCustomTransformation
      read GetTransformation;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  end;
  ...
  public
    property TransformationType: TdxShapeTransformationType read FType
      write FType;
  end;
  ...
  TdxShapePathPointType = (dxpsptStart = 0, dxpsptLine = 1, dxpsptBezier = 3,
    dxpsptPathTypeMask = 7, dxpsptDashMode = 16, dxpsptPathMarker = 32,
    dxpsptCloseSubpath = 128);
  ...
  public
    procedure Parse(const AData: AnsiString; var APoints: TdxGpPointFDynArray;
      var APointTypes: TBytes);
  end;
  ...
  public
    ...
    function GetShapeClass(const AName: TdxXMLString)
      : TdxShapeObjectClass; overload;
...
uses
  Types, StrUtils, Graphics, Math, cxGraphics, dxCustomTree, dxShapeReaders,
  dxShapeTransformations, dxShapeBrushes;
...
begin
  ...
  begin
    if AColorName[1] = '#' then
      Result := TdxColorHelper.HexCodeToAlphaColor
        (Copy(AColorName, 2, Length(AColorName) - 1), False)
    else if SameText(AColorName, 'Transparent') then
...
function dxStrToPointF(const S: string): TdxPointF;
  ...
  function GetCoordinate(const AString: string;
    AFirstCoordinate: Boolean): Single;
  var
  ...
  begin
    ...
    if AFirstCoordinate then
      ...
      Result := dxStrToFloat(Copy(AString, ASeparatorPosition + 1,
        Length(AString) - ASeparatorPosition + 1));
  end;
...
procedure TdxCustomVectorShape.ApplyTransformation(ATransformation
  : TdxShapeCustomTransformation);
  ...
  case ATransformation.TransformationType of
    dxsttScale:
      if IsTransformationAvailable then
        DoApplyTransformation(dxsttScale,
          TdxShapeScaleTransformation(ATransformation).ScaleX, 0, 0,
          TdxShapeScaleTransformation(ATransformation).ScaleY, 0, 0);

    dxsttTranslate:
      if IsTransformationAvailable then
        DoApplyTransformation(dxsttTranslate, 1, 0, 0, 1,
          TdxShapeTranslateTransformation(ATransformation).X + FBounds.Left,
          TdxShapeTranslateTransformation(ATransformation).Y + FBounds.Top);

    dxsttSkew:
      if IsTransformationAvailable then
        DoApplyTransformation(dxsttSkew, 1, 0, 0, 1,
          DegToRad(TdxShapeSkewTransformation(ATransformation).AngleX),
    ...
    dxsttRotate:
      ...
      begin
        ...
        APoint := dxPointF(APoint.X * cxRectWidth(FBounds),
          APoint.Y * cxRectHeight(FBounds));
        DoApplyTransformation(dxsttRotate,
          TdxShapeRotateTransformation(ATransformation).Angle, 0, 0, 1,
          APoint.X, APoint.Y);
      end;
  end;
end;
...
procedure TdxCustomVectorShape.CalculateTransformMatrix;
  ...
  begin
    ...
    try
      ...
      DoApplyTransformation(dxsttTranslate, 1, 0, 0, 1,
        ATranslate.X + FBounds.Left, ATranslate.Y + FBounds.Top);
    finally
...
begin
  ...
  if FTransformations.Count > 0 then
    for I := 0 to FTransformations.Count - 1 do
      ApplyTransformation(TdxShapeCustomTransformation
        (FTransformations.Items[I]))
  else
    DoTranslateTransformation;
  CancelUpdate;
end;

procedure TdxCustomVectorShape.DoApplyTransformation
  (AType: TdxShapeTransformationType; M11, M12, M21, M22, DX, DY: Single);
var
  AMatrix: TdxGPMatrix;
begin
...
begin
  if not SameText(dxAnsiStringToString(FData), dxAnsiStringToString(AValue))
  then
  begin
    FData := AValue;
    Changed;
...
procedure TdxShapeCanvas.SetShapeColor(AAlphaColor: TdxAlphaColor;
  const AShapeName: string);
var
  I: Integer;
begin
  for I := 0 to FShapes.Count - 1 do
    ...
    else if SameText(Shapes[I].Name, AShapeName) then
...
class function TdxShapeFakeGradientStops.GetReaderClass
  : TdxShapeObjectReaderClass;
begin
...
class function TdxShapeFakeCustomTransformation.GetReaderClass
  : TdxShapeObjectReaderClass;
begin
...
procedure TdxShapeObjectReader.ReadFromNode(ABaseObject: TdxShapeObject;
  ANode: TdxXMLNode);
begin
...
function TdxShapeObjectReader.CreateShapeFromNode(AParentShape: TdxShapeObject;
  ANode: TdxXMLNode): TdxShapeObject;
var
...
procedure TdxShapeObjectReader.ReadAttributes(ABaseShape: TdxShapeObject;
  ANode: TdxXMLNode);
var
  I: Integer;
...
procedure TdxShapeObjectReader.ReadChildNodes(AParentShape: TdxShapeObject;
  ANode: TdxXMLNode);
begin
  if ANode <> nil then
    ANode.ForEach(procedure(ANode: TdxXMLNode; AUserData: Pointer)
      ...
      begin
        AShape := dxShapesFactory.GetShapeClass(ANode.Name)
          .Create(AParentShape);
        try
...
function TdxShapeObjectReader.GetShapePropertyName(const AAttributeName
  : string): string;
var
  AIndex: Integer;
begin
  ...
  if AIndex = 0 then
    ...
    Result := Copy(AAttributeName, AIndex + 1, Length(AAttributeName) -
      AIndex + 1);
end;

procedure TdxShapeObjectReader.ReadAttribute(ABaseShape: TdxShapeObject;
APropList: PPropList; APropCount: Integer; AAttribute: TdxXMLNodeAttribute);
...
procedure TdxShapeTransformations.AddTransformation(ATransformation
  : TdxShapeObject);
begin
...
function TdxShapeTransformations.GetTransformation(AIndex: Integer)
  : TdxShapeCustomTransformation;
begin
...
procedure TdxShapePathDataParser.Parse(const AData: AnsiString;
var APoints: TdxGpPointFDynArray; var APointTypes: TBytes);
type
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        APointTypes[Length(APointTypes) - 1] :=
          Byte(APointTypes[Length(APointTypes) - 1]) or
          Byte(dxpsptCloseSubpath);
    end;
  end;
end;
...
begin
  Result := FPosition;
  while (Result <= FDataSize) and
    ((FData[Result] = ' ') or (FData[Result] = ',') or (FData[Result] = '\r') or
    (FData[Result] = '\n') or (FData[Result] = '\t')) do
    Inc(Result);
...
function TdxShapePathDataParser.GetPointType(ACommand: Char)
  : TdxShapePathPointType;
begin
  case ACommand of
    'c':
...
begin
  Result := dxCharInSet(AChar, ['0' .. '9']) or (AChar = '.') or (AChar = '-')
    or (AChar = '+') or (AChar = 'e');
end;
...
function TdxShapesFactory.GetShapeClass(const AName: TdxXMLString)
  : TdxShapeObjectClass;
