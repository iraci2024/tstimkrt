...
type
  ...
  public
    ...
    procedure SetStyleSheet(AStyleSheet: TcxCustomStyleSheet); virtual;
      abstract;
  end;
  ...
  private
    ...
    procedure UpdateStyles(AStyleSheet: TcxCustomStyleSheet;
...
procedure RegisterStyleSheetEditorPreview(APreviewClass
  : TcxStyleSheetEditorPreviewClass);
procedure UnregisterStyleSheetEditorPreview(APreviewClass
  : TcxStyleSheetEditorPreviewClass);
function GetPreviewByStyleSheetClass(AStyleSheetClass: TcxCustomStyleSheetClass)
  : TcxStyleSheetEditorPreviewClass;
...
class function TcxStyleSheetEditorPreview.GetStyleSheetClass
...
function TfrmcxStyleSheetEditor.GetCheckBoxStyleValue(ACheckBox: TCheckBox)
  : TcxStyleValue;
begin
  if ACheckBox = cbBitmap then
    Result := svBitmap
  else if ACheckBox = cbColor then
    Result := svColor
  else
    Result := svFont;
end;
...
procedure TfrmcxStyleSheetEditor.SetSelectedStylesAssignValue
  (ACheckBox: TCheckBox);

  function cxStyleValueTocxStyleValues(AStyleValue: TcxStyleValue)
    : TcxStyleValues;
  begin
    case AStyleValue of
      svBitmap:
...
begin
  ...
  try
    ...
    for I := 0 to ACount - 1 do
      ...
      begin
        AStyle := TcxStyle(GetObjectProp(AStyleSheet.GetStyles,
  ...
  if GetPreviewByStyleSheetClass(TcxCustomStyleSheetClass(FStyleSheet.ClassType)
    ) <> nil then
  begin
    FPreview := GetPreviewByStyleSheetClass
      (TcxCustomStyleSheetClass(FStyleSheet.ClassType)).Create(Self);
...
procedure TfrmcxStyleSheetEditor.UpdateState;
  ...
  procedure UpdateCheckBox(ACheckBox: TCheckBox; AStyleValue: TcxStyleValue);
    ...
    var
      ...
      I: Integer;
    begin
      AIsStyleValueAssigned := AStyleValue
        in GetFirstSelectedStyle.AssignedValues;
      Result := True;
      for I := 0 to lbStyles.Items.Count - 1 do
        if lbStyles.Selected[I] and
          ((AStyleValue in TcxStyle(FStyleList[I]).AssignedValues) <>
          AIsStyleValueAssigned) then
        begin
          Result := False;
          Break;
        end;
    ...
    const
      ACheckBoxStateMap: array [Boolean] of TCheckBoxState = (cbUnchecked,
        cbChecked);
    begin
      Result := cbUnchecked;
      if ACheckBox.Enabled then
        if SelectedStylesHaveSameAssignedValue then
          Result := ACheckBoxStateMap
            [AStyleValue in GetFirstSelectedStyle.AssignedValues]
        else
          Result := cbGrayed
    end;
...
var
  AcxCanvas: TcxCanvas;
begin
  if Index >= FStyleList.Count then
    exit;
...
begin
  ...
  if FStyleSheetPreviews <> nil then
    for I := 0 to FStyleSheetPreviews.Count - 1 do
      if (TcxStyleSheetEditorPreviewClass(FStyleSheetPreviews[I])
        .GetStyleSheetClass = AStyleSheetClass) then
      begin
  ...
  try
    ...
    if AOpenPictureDialog.Execute then
      try
        for I := 0 to lbStyles.Items.Count - 1 do
          if lbStyles.Selected[I] then
            TcxStyle(FStyleList[I]).Bitmap.LoadFromFile
              (AOpenPictureDialog.FileName);
      finally
        lbStyles.Invalidate;
        UpdateState;
      end;
