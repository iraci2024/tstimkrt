...
{$EXTERNALSYM DWM_SIT_DISPLAYFRAME }
  DWM_SIT_DISPLAYFRAME = $00000001;
  // Display a window frame around the provided bitmap
{$EXTERNALSYM DWMWA_HAS_ICONIC_BITMAP}
  DWMWA_HAS_ICONIC_BITMAP = 10;
  // The window can provide a bitmap for use by DWM as an iconic thumbnail or peek representation
...
{$EXTERNALSYM DWMWA_EXCLUDED_FROM_PEEK}
  ...
  // Prevents a window from fading to a glass sheet when the peek preview of another window is shown
...
type
  ...
  TDWMNCRENDERINGPOLICY = (DWMNCRP_USEWINDOWSTYLE,
...
var
  ...
  DwmEnableBlurBehindWindow: function(wnd: HWND; pBlurBehind: PDWM_BLURBEHIND)
    : HRESULT; stdcall;
  ...
  DwmExtendFrameIntoClientArea: function(wnd: HWND; pMarInset: PdxMargins)
    : HRESULT; stdcall;
  ...
  DwmSetWindowAttribute: function(HWND: HWND; dwAttribute: DWORD;
    pvAttribute: Pointer; cbAttribute: DWORD): HRESULT; stdcall;
  DwmSetIconicThumbnail: function(HWND: HWND; hbmp: HBITMAP;
    dwSITFlags: DWORD): HRESULT;
  DwmSetIconicLivePreviewBitmap: function(HWND: HWND; hbmp: HBITMAP;
...
begin
  ...
  begin
    ...
    DwmEnableBlurBehindWindow := GetProcAddress(dwmapiDLL,
    ...
    DwmExtendFrameIntoClientArea := GetProcAddress(dwmapiDLL,
      'DwmExtendFrameIntoClientArea');
    DwmGetColorizationColor := GetProcAddress(dwmapiDLL,
      'DwmGetColorizationColor');
    DwmGetWindowAttribute := GetProcAddress(dwmapiDLL, 'DwmGetWindowAttribute');
    DwmIsCompositionEnabled := GetProcAddress(dwmapiDLL,
    ...
    DwmSetIconicLivePreviewBitmap := GetProcAddress(dwmapiDLL,
      'DwmSetIconicLivePreviewBitmap');
    DwmInvalidateIconicBitmaps := GetProcAddress(dwmapiDLL,
