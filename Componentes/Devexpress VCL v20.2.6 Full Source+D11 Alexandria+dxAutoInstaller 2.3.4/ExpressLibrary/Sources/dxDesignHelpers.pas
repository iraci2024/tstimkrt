...
type
  ...
  TdxOTACustomRepositoryWizard = class(TNotifierObject, IOTARepositoryWizard,
    IOTARepositoryWizard60, IOTARepositoryWizard80, IOTAWizard)
  protected
    // IOTAWizard
    ...
    function IOTARepositoryWizard80.GetGalleryCategory =
      Wizard80GetGalleryCategory;
  ...
  private
    ...
    function CreateOTAFile(const ATemplate, AModuleIdent, AFormIdent,
      AAncestorIdent: string): IOTAFile;
  protected
    function ExpandTemplate(const ATemplate, AModuleIdent, AFormIdent,
      AAncestorIdent: string): string; virtual;
    // IOTACreator
  ...
  public
    constructor Create(const AFormTemplate, AImplTemplate,
      AIntfTemplate: string);
    //
  ...
  TdxOTACustomProjectCreator = class(TdxOTACustomCreator, IOTAProjectCreator50,
...
const
  CategoriesMap: array [TdxOTARepositoryCategory] of string =
    ('Projects', 'New');
begin
...
function TdxOTACustomRepositoryWizard.Wizard80GetGalleryCategory
  : IOTAGalleryCategory;
const
  CategoriesMap: array [TdxOTAPersonality, TdxOTARepositoryCategory]
    of string = ((sCategoryDelphiNew, sCategoryDelphiNewFiles),
    (sCategoryCBuilderNew, sCategoryCBuilderNewFiles));
var
  ACategoryManager: IOTAGalleryCategoryManager;
begin
  if Supports(BorlandIDEServices, IOTAGalleryCategoryManager, ACategoryManager)
  then
    Result := ACategoryManager.FindCategory(CategoriesMap[Personality,
      Category])
  else
    Result := nil;
end;
...
const
  PersonalitiesMap: array [TdxOTAPersonality] of string = (sDelphiPersonality,
...
constructor TdxOTAFormCreator.Create(const AFormTemplate, AImplTemplate,
  AIntfTemplate: string);
begin
  inherited Create;
...
function TdxOTAFormCreator.ExpandTemplate(const ATemplate, AModuleIdent,
  AFormIdent, AAncestorIdent: string): string;
begin
  Result := ATemplate;
  Result := StringReplace(Result, '%FormIdent%', AFormIdent, [rfReplaceAll]);
  Result := StringReplace(Result, '%AncestorIdent%', AAncestorIdent,
    [rfReplaceAll]);
  Result := StringReplace(Result, '%ModuleIdent%', AModuleIdent,
    [rfReplaceAll]);
end;
...
function TdxOTAFormCreator.NewFormFile(const FormIdent, AncestorIdent: string)
  : IOTAFile;
begin
  Result := CreateOTAFile(FormTemplate, '', FormIdent, AncestorIdent);
end;

function TdxOTAFormCreator.NewImplSource(const ModuleIdent, FormIdent,
  AncestorIdent: string): IOTAFile;
begin
  Result := CreateOTAFile(ImplTemplate, ModuleIdent, FormIdent, AncestorIdent);
end;

function TdxOTAFormCreator.NewIntfSource(const ModuleIdent, FormIdent,
  AncestorIdent: string): IOTAFile;
begin
...
function TdxOTAFormCreator.CreateOTAFile(const ATemplate, AModuleIdent,
  AFormIdent, AAncestorIdent: string): IOTAFile;
begin
  if ATemplate <> '' then
    Result := TOTAFile.Create(ExpandTemplate(ATemplate, AModuleIdent,
      AFormIdent, AAncestorIdent))
  else
    Result := nil;
end;
...
function TdxOTACustomProjectCreator.NewOptionSource(const ProjectName: string)
  : IOTAFile;
begin
  Result := nil;
end;

function TdxOTACustomProjectCreator.NewProjectSource(const ProjectName: string)
  : IOTAFile;
begin
...
procedure TdxOTACustomProjectCreator.NewProjectResource(const Project
  : IOTAProject);
begin
  // do nothing
end;

procedure TdxOTACustomProjectCreator.NewDefaultProjectModule
...
procedure TdxOTACustomProjectCreator.SetInitialOptions(const NewProject
  : IOTAProject);
begin
  // do nothing
end;
