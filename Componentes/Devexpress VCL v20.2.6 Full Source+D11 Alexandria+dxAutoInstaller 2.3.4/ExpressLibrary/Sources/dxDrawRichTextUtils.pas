...
uses
  Windows, Controls, Messages, Types, Classes, Math, SysUtils, Graphics,
  RichEdit, ComCtrls, cxGeometry;
...
type
  ...
  public
    ...
    procedure Init(ACanvas: TCanvas; const AText: string;
...
procedure dxDrawRichEdit(ADC: HDC; ARect: TRect; ARichHandle: HWND;
  AScaleFactor: TdxScaleFactor; AMinCharIndex, AMaxCharIndex: Integer;
  ACalculateHeight: Boolean; out AHeight: Integer); overload;
procedure dxDrawRichEdit(ACanvas: TCanvas; const ARect: TRect;
  ARichEdit: TRichEdit; AMinCharIndex, AMaxCharIndex: Integer;
  ACalculateHeight: Boolean; out AHeight: Integer); overload;
...
procedure dxDrawRichEdit(ADC: HDC; const ARect: TRect; ARichHandle: HWND;
  ...
  out AHeight: Integer);
begin
  dxDrawRichEdit(ADC, ARect, ARichHandle, dxDefaultScaleFactor, AMinCharIndex,
    AMaxCharIndex, ACalculateHeight, AHeight);
end;

procedure dxInternalDrawRichEdit(ADC: HDC; ARect: TRect; ARichHandle: HWND;
  AZoomFactor: TdxScaleFactor; AMinCharIndex, AMaxCharIndex: Integer;
  ACalculateHeight: Boolean; out AHeight: Integer);
type
  TState = record
    Assigned: Boolean;
    MapMode: Integer;
  ...
  begin
    ...
    begin
      ...
      SetViewportExtEx(ADC, AZoomFactor.Apply(CanvasSize),
        AZoomFactor.Apply(CanvasSize), @AState.ViewPortExt);
...
begin
  ...
  try
    ...
    try
      ...
      repeat
        AFormatRange.rc := PrepareRect(AZoomFactor.Revert(ARect));
      ...
      if ACalculateHeight then
        AHeight := dxTwipsToPixels(AZoomFactor.Apply(AHeight));
    finally
      SendMessage(ARichHandle, EM_FORMATRANGE, 0, 0);
    end;
  finally
...
begin
  ...
  try
    if Boolean(SendMessage(ARichHandle, EM_GETZOOM, WPARAM(@N), LPARAM(@D))) and
      (D > 0) and (N <> D) then
      AZoomFactor.Assign(N, D)
    else
      AZoomFactor.Assign(AScaleFactor.TargetDPI, dxSystemScaleFactor.TargetDPI);
    dxInternalDrawRichEdit(ADC, ARect, ARichHandle, AZoomFactor, AMinCharIndex,
      AMaxCharIndex, ACalculateHeight, AHeight);
  finally
    AZoomFactor.Free;
  end;
...
procedure dxDrawRichEdit(ACanvas: TCanvas; const ARect: TRect;
  ...
  ACalculateHeight: Boolean; out AHeight: Integer);
begin
  dxDrawRichEdit(ACanvas.Handle, ARect, ARichEdit.Handle, AMinCharIndex,
    AMaxCharIndex, ACalculateHeight, AHeight);
end;
...
procedure TdxRichTextHelper.CalculateTextHeight(ACanvas: TCanvas;
  var ARect: TRect);
var
  AHeight: Integer;
begin
  ACanvas.Lock;
  try
    dxDrawRichEdit(ACanvas.Handle, ARect, FRichEdit.Handle, FScaleFactor, 0, -1,
      True, AHeight);
...
begin
  ...
  try
    ...
    SetWindowLong(FRichEdit.Handle, GWL_EXSTYLE, GetWindowLong(FRichEdit.Handle,
