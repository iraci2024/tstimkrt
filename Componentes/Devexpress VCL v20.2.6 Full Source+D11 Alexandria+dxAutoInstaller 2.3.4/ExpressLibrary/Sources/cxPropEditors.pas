...
type
  ...
  protected
    constructor Create(Parent: TPropertyEditor; AOwner: TPersistent;
  ...
  private
    ...
    function GetInstance: TPersistent;
  protected
    constructor Create(Parent: TPropertyEditor; APropInfo: PPropInfo);
      reintroduce;
...
begin
  ...
  begin
    ...
    if not AClassType.InheritsFrom(TComponent) and
      ...
      Proc(TcxNestedParentElementEventProperty.Create(APropertyEditor,
        AInstance, APropList[I]));
  end;
end;
...
constructor TcxNestedElementEventProperty.Create(Parent: TPropertyEditor;
  APropInfo: PPropInfo);
begin
  inherited Create(Parent);
  FPropInfo := APropInfo;
...
begin
  ...
  if (CurValue <> '') and (AValue <> '') and
    ((CompareText(CurValue, AValue) = 0) or not Designer.MethodExists(AValue))
    and not Designer.MethodFromAncestor(GetMethodValue) then
    Designer.RenameMethod(CurValue, AValue)
  else
  begin
    NewMethod := (AValue <> '') and not Designer.MethodExists(AValue);
    OldMethod := GetMethodValue;
    SetMethodProp(Instance, FPropInfo, Designer.CreateMethod(AValue,
      GetTypeData(FPropInfo.PropType^)));
    if NewMethod then
    begin
...
begin
  ...
  begin
    if APropertyEditor is TcxNestedParentElementEventProperty then
      APropertyEditor := TcxNestedParentElementEventProperty
        (APropertyEditor).FParent
    else if APropertyEditor is TcxNestedElementEventProperty then
...
function TcxStyleControllerStyleProperty.IsPropertyVisible(const APropertyName
  : string): Boolean;
begin
