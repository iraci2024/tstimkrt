...
{$ENDIF}
  ...
  Windows, SysUtils, Classes, ImgList, ComCtrls, Controls, Graphics, Forms,
  Dialogs,
...
type
  ...
  public
    constructor Create(AImageListEditor: TcxImageListEditor);
      reintroduce; virtual;
  ...
  strict private
  const
    sMsgChangeImagesSizeConfirm =
      'This will change the image dimensions and scale the existing images in the list. Scaling raster images may reduce their visual quality. Do you want to proceed?';
    sMsgReplaceConfirm =
      'File %s is already exists.'#13#10'Do you want to replace it?';
    sMsgSplitConfirm =
      'The bitmap in the file %s is too large.'#13#10'Do you want to split it into smaller bitmaps?';
  strict private
    FChanged: Boolean;
    ...
    procedure AddImage(AImage: TGraphic; AMask: TBitmap; AMaskColor: TColor;
      var AInsertedImageIndex: Integer);
    procedure AddImageCore(AImage: TGraphic; AMask: TBitmap;
      const AFileName: string; var AInsertedItemIndex: Integer;
    ...
    procedure SelectDataItem(Sender: TObject; Item: TListItem;
      Selected: Boolean);
  ...
  protected
    ...
    function GetImageInfoFromFile(const AFileName: string;
      AImageInfo: TcxImageInfo): Boolean;
    function GetImageListEditorCaption(AImageList: TcxImageList)
      : string; virtual;
  ...
  public
    ...
    property FocusedImageIndex: Integer read GetFocusedImageIndex
      write SetFocusedImageIndex;
    ...
    property ImagesInfo[Index: Integer]: TcxImageInfo read GetImagesInfo
      write SetImagesInfo;
    ...
    property ShowImageTypes: Boolean read FShowImageTypes
      write SetShowImageTypes;
  ...
  public
    ...
    function Edit(AGlyph: TdxSmartGlyph;
      AMultiPartGlyphSupport: IdxMultiPartGlyphSupport): Boolean;
  end;

function cxImageFileFormats: TcxImageFileFormats;
function cxEditImageList(AImageList: TcxImageList;
  AImportList: TStrings): Boolean;
procedure PngImageListTocxImageList(APngImages: TComponent;
  AImages: TcxImageList);

implementation

uses
  Types, Math, cxImageListEditorView, dxCore, dxCoreGraphics, cxFormats,
  dxSVGImage, dxSmartImage, Generics.Collections;
...
type
  ...
  strict private
    class procedure AddImageFromBinaryData(WriteData: TStreamProc;
      AImages: TcxImageList);
    class procedure ProcessPngImageList(AInputStream: TMemoryStream;
      AImages: TcxImageList);
  public
...
class procedure TPngImageListImporter.AddImageFromBinaryData
  (WriteData: TStreamProc; AImages: TcxImageList);
var
...
class procedure TPngImageListImporter.ProcessPngImageList
  (AInputStream: TMemoryStream; AImages: TcxImageList);
var
  ASaveSeparator: Char;
  AParser: TParser;
...
procedure ConvertProperty; forward;
  ...
  procedure ConvertValue(const APropName: string);
    ...
    begin
      ...
      begin
        AParser.NextToken;
        if not dxCharInSet(AParser.Token, [Classes.toString, Classes.toWString])
        then
...
class procedure TPngImageListImporter.Import(APngImages: TComponent;
  AImages: TcxImageList);
var
  S, D: TMemoryStream;
begin
...
procedure TcxImageFileFormats.Register(const AName, AExt: string;
...
function TcxImageFileFormats.GetGraphicClass(const AFileName: string)
  : TGraphicClass;
var
  AExt: string;
  I: Integer;
...
constructor TcxCustomImageListEditorForm.Create(AImageListEditor
  : TcxImageListEditor);
begin
...
procedure TcxImageListEditor.AddImages(AFiles: TStrings;
  AAddMode: TcxImageListEditorAddMode);
var
  ...
  try
    ...
    begin
      if GetImageInfoFromFile(AFiles[I], AImageInfo) then
        AddImageCore(AImageInfo.Image, AImageInfo.Mask, AFiles[I],
          AInsertedItemIndex, AFiles.Count > 1);
    end;
  finally
    AImageInfo.Free;
  end;
...
procedure TcxImageListEditor.ExportImages(const AFileName: string;
  ...
  begin
    Result := MessageDlg(Format(sMsgReplaceConfirm, [AFileName]), mtWarning,
      [mbYes, mbNo], 0) = mrYes;
  end;
...
begin
  ...
  begin
    ...
    AExportImage := TcxBitmap32.CreateSize(ImageList.Width * GetSelectionCount,
      ImageList.Height);
    try
      ...
      for AImageIndex := 0 to ImagesCount - 1 do
        ...
        begin
          TcxImageInfoHelper.CopyRect(AExportImage, ARect,
            ImagesInfo[AImageIndex].Image);
...
procedure TcxImageListEditor.MoveImage(ASourceImageIndex, ADestImageIndex
  : Integer);
var
  AList: TList;
  I: Integer;
...
begin
  ...
  begin
    if not TcxImageListAccess(ImageList).HasRasterImages or ShowRasterImagesResizeConfirmation
    then
    begin
      DataItems.Clear;
...
function TcxImageListEditor.GetEditorFormClass
  : TcxCustomImageListEditorFormClass;
begin
...
function TcxImageListEditor.GetImageInfoFromFile(const AFileName: string;
  AImageInfo: TcxImageInfo): Boolean;
var
  AGraphic: TGraphic;
  AGraphicClass: TGraphicClass;
...
function TcxImageListEditor.GetImageListEditorCaption
  (AImageList: TcxImageList): string;
begin
...
begin
  Result := MessageDlg(sMsgChangeImagesSizeConfirm, mtWarning, [mbYes, mbNo],
    0) = mrYes;
end;
...
procedure TcxImageListEditor.AddImage(AImage: TGraphic; AMask: TBitmap;
  AMaskColor: TColor; var AInsertedImageIndex: Integer);
var
  AImageInfo: TcxImageInfo;
begin
  AImageInfo := TcxImageInfo.Create;
...
procedure TcxImageListEditor.AddImageCore(AImage: TGraphic; AMask: TBitmap;
  ...
  function GetUserPermissionForSplit(const AFileName: string;
  ...
  begin
    ...
    FSplitBitmaps := MessageDlg(Format(sMsgSplitConfirm, [AFileName]),
      mtConfirmation, APossibleAnswers, 0);
  ...
  begin
    Result := CanSplitImage(AImage) and (FSplitBitmaps <> mrNoToAll) and
      ((FSplitBitmaps = mrYesToAll) or GetUserPermissionForSplit(AFileName,
      AMultiSelect));
  end;
...
begin
  ...
  if (ImageWidth = AImage.Width) and (ImageHeight = AImage.Height) and AMask.Empty
  then
  begin
    AddImage(AImage, AMask, TcxImageInfoHelper.GetDefaultTransparentColor
      (AImage, AMask), AInsertedItemIndex);
    Exit;
  end;
  ...
  try
    for ARowIndex := 0 to ARowCount - 1 do
      ...
      begin
        ADestRect := cxRectCenter(ADestBitmap.ClientRect,
          Min(ImageWidth, ASourceImageSize.cx),
          Min(ImageHeight, ASourceImageSize.cy));
        ADestBitmap.Canvas.Brush.Color :=
          TcxImageInfoHelper.GetDefaultTransparentColor(AImage, AMask);
        ADestBitmap.Canvas.FillRect(ADestBitmap.ClientRect);

        ASrcRect := cxRectBounds(AColIndex * ASourceImageSize.cx,
          ARowIndex * ASourceImageSize.cy, ASourceImageSize);
        ...
        AddImage(ADestBitmap, ADestMask,
          TcxImageInfoHelper.GetDefaultTransparentColor(ADestBitmap, ADestMask),
          AInsertedItemIndex);
      end;
  finally
    ADestMask.Free;
...
procedure TcxImageListEditor.SelectDataItem(Sender: TObject; Item: TListItem;
  Selected: Boolean);
begin
  if Selected then
  ...
  try
    ...
    try
      ...
      begin
        ...
        if not IsXPManifestEnabled and (AEditorImageInfo.Image is TBitmap) and
          AEditorImageInfo.IsAlphaUsed then
        begin
          AImageInfo.Assign(AEditorImageInfo);
          MakeSystemBackground(AEditorImageInfo.Image as TBitmap,
            AImageInfo.Image as TBitmap);
...
begin
  ...
  begin
    ...
    if (ACustomImageList.Width = ImageWidth) and
      (ACustomImageList.Height = ImageHeight) and (ACustomImageList.Count > 0)
    then
...
function TdxMultiPartGlyphEditor.Edit(AGlyph: TdxSmartGlyph;
  AMultiPartGlyphSupport: IdxMultiPartGlyphSupport): Boolean;
var
  AImageList: TcxImageList;
begin
...
function TdxMultiPartGlyphEditor.GetImageListEditorCaption
  (AImageList: TcxImageList): string;
begin
  Result := 'Glyph Editor';
end;
...
procedure TdxMultiPartGlyphEditor.CombineGlyphs;
  ...
  var
    ...
    AStream: TStream;
  begin
    ABitmap := TcxBitmap32.CreateSize(ImageList.Width * ImageList.Count,
      ImageList.Height);
    try
      ABitmap.Clear;
  ...
  begin
    ...
    try
      ...
      for I := 0 to ImageList.Count - 1 do
        AList.Add(TdxCustomSmartImageAccess(ImagesInfo[I].Image)
          .Handle as TdxSVGImageHandle);

      TdxCustomSmartImageAccess(FGlyph).Handle :=
        TdxSVGEditingHelper.Combine(AList);
    finally
      AList.Free;
    end;
  ...
  begin
    Result := True;
    for I := 0 to ImageList.Count - 1 do
      if not(ImagesInfo[I].Image is TdxCustomSmartImage) or
        (TdxSmartImage(ImagesInfo[I].Image).ImageCodec <> TdxSVGImageCodec) then
      begin
        Result := False;
        Break;
      end;
...
procedure TdxMultiPartGlyphEditor.PopulateImageList(AImageList: TcxImageList);
  ...
  begin
    AImageList.SetSize(FGlyph.SourceWidth div Max(1,
      FMultiPartGlyphSupport.GlyphCount), FGlyph.SourceHeight);
  ...
  begin
    if TdxSVGEditingHelper.Split(FGlyph, AList) then
      try
        ...
        try
          AImageList.SetSize(FGlyph.SourceWidth div Max(1, AList.Count),
            FGlyph.SourceHeight);
...
begin
  ...
  begin
    if GetClass(TdxPNGImage.ClassName) <> nil then
      cxImageFileFormats.Register('DevExpress PNG (*.png)', '.png',
        TdxPNGImage);
    if TdxSmartImageCodecsRepository.Contains(TdxSVGImageCodec) then
      cxImageFileFormats.Register('DevExpress SVG (*.svg)', '.svg',
        TdxSmartImage);
  end;
end;
