...
type
  ...
  TcxStorageMode = (smChildrenCreating, smChildrenDeleting,
    smSavePublishedClassProperties);
  ...
  TcxTestClassPropertyEvent = function(const AName: string; AObject: TObject)
    : Boolean of object;
  TcxGetComponentByNameEvent = function(const AName: string)
    : TComponent of object;
  TcxGetUseInterfaceOnlyEvent = function: Boolean of object;

  TcxGetStoredPropertiesEvent = procedure(Sender: TObject;
    AProperties: TStrings) of object;
  TcxGetStoredPropertyValueEvent = procedure(Sender: TObject;
    const AName: string; var AValue: Variant) of object;
  TcxInitStoredObjectEvent = procedure(Sender: TObject; AObject: TObject)
    of object;
  TcxSetStoredPropertyValueEvent = procedure(Sender: TObject;
    const AName: string; const AValue: Variant) of object;
  TcxSetStoredPropertyValueErrorEvent = function(Sender: TcxStorage;
    const AName: string; const AValue: Variant): Boolean of object;
  ...
  protected
    ...
    procedure InternalCheckChildrenOrder(AChildrenNames, AChildrenClassNames
      : TStringList);
    procedure InternalRestoreFrom(AReader: TcxCustomReader;
      const ADefaultObjectName: string = ''); virtual;
    procedure InternalStoreTo(AWriter: TcxCustomWriter;
      const ADefaultObjectName: string = ''); virtual;
    procedure SetStoredObject(AObject: TObject);
  public
    constructor Create(const AStorageName: string;
      AStorageStream: TStream); overload;
    constructor Create(const AStorageName: string); overload;
    constructor Create(AStream: TStream); overload;
    procedure RestoreFrom(AObject: TObject;
      AReaderClass: TcxCustomReaderClass); virtual;
    procedure RestoreWithExistingReader(AObject: TObject;
      AReader: TcxCustomReader); virtual;
    ...
    procedure StoreTo(AObject: TObject;
      AWriterClass: TcxCustomWriterClass); virtual;
    procedure StoreWithExistingWriter(AObject: TObject;
      AWriter: TcxCustomWriter); virtual;
    ...
    property SaveComponentPropertiesByName: Boolean
      read FSaveComponentPropertiesByName write FSaveComponentPropertiesByName;
    ...
    property UseInterfaceOnly: Boolean read FUseInterfaceOnly
      write FUseInterfaceOnly;
    property OnGetComponentByName: TcxGetComponentByNameEvent
      read FOnGetComponentByName write FOnGetComponentByName;
    property OnGetStorageModes: TcxGetStorageModesEvent read FOnGetStorageModes
      write FOnGetStorageModes;
    property OnGetUseInterfaceOnly: TcxGetUseInterfaceOnlyEvent
      read FOnGetUseInterfaceOnly write FOnGetUseInterfaceOnly;
    property OnTestClassProperty: TcxTestClassPropertyEvent
      read FOnTestClassProperty write FOnTestClassProperty;
    property OnSetStoredPropertyValueError: TcxSetStoredPropertyValueErrorEvent
      read FOnSetStoredPropertyValueError write FOnSetStoredPropertyValueError;
  end;
  ...
  public
    ...
    constructor Create(const AStorageName: string; AStorageStream: TStream);
      overload; virtual;
    procedure ReadProperties(const AObjectName, AClassName: string;
      AProperties: TStrings); virtual;
    function ReadProperty(const AObjectName, AClassName, AName: string)
      : Variant; virtual;
    procedure ReadChildren(const AObjectName, AClassName: string;
      AChildrenNames, AChildrenClassNames: TStrings); virtual;
  ...
  protected
    ...
    procedure ClearObjectData(const AObjectFullName,
      AClassName: string); virtual;
    procedure DoSetStream(AStream: TStream); virtual;
    procedure BeginWrite; virtual;
    procedure EndWrite; virtual;
  public
    constructor Create(const AStorageName: string; AReCreate: Boolean = True);
      overload; virtual;
    constructor Create(const AStorageName: string; AStorageStream: TStream;
      AReCreate: Boolean = True); overload; virtual;
  ...
  public
    constructor Create(const AStorageName: string;
      AStorageStream: TStream); override;
    ...
    function ReadProperty(const AObjectName, AClassName, AName: string)
      : Variant; override;
    procedure ReadChildren(const AObjectName, AClassName: string;
      AChildrenNames, AChildrenClassNames: TStrings); override;
  end;
  ...
  protected
    procedure ClearObjectData(const AObjectFullName,
      AClassName: string); override;

    property Registry: TRegistry read FRegistry;
  public
    constructor Create(const AStorageName: string; AStream: TStream;
      AReCreate: Boolean = True); overload; override;
    destructor Destroy; override;
  ...
  private
    ...
    procedure GetSectionDetail(const ASection: string;
      var APath, AObjectName, AClassName: string);
  protected
...
type
  ...
  private
    ...
    function GetProperty(AObject: TcxStreamObjectData; const AName: string)
      : TcxStreamPropertyData;
    function InternalGetObject(const AObjectName: string; AParents: TStrings)
      : TcxStreamObjectData;
  protected
    function CanRead: Boolean; override;
  ...
  private
    ...
    FRootObject: TcxStreamObjectData;
    FWriter: TWriter;
    procedure CreateObject(const AObjectName, AClassName: string;
      AParents: TStrings);
  protected
  ...
  public
    ...
    property Properties[AIndex: Integer]: TcxStreamPropertyData
      read GetProperties;
...
function EnumerationToString(const AValue: Integer;
  const AEnumNames: array of string): string;
function StringToEnumeration(const AValue: string;
  const AEnumNames: array of string): Integer;
function SetToString(const ASet; ASize: Integer;
  const AEnumNames: array of string): string;
procedure StringToSet(AString: string; var ASet; ASize: Integer;
  const AEnumNames: array of string);
...
function CompareChildrenNamesAsNumbers(List: TStringList;
  Index1, Index2: Integer): Integer;
begin
  Result := StrToIntDef(List.Strings[Index1], -1) -
    StrToIntDef(List.Strings[Index2], -1);
end;
...
procedure StringToStream(AValue: AnsiString; AStream: TStream);
begin
  if (AStream = nil) or (Length(AValue) < 6) then
    Exit;
  Delete(AValue, 1, 4);
...
const
  AKeyMap: array [0 .. 6] of string = ('HKCR', 'HKCU', 'HKLM', 'HKU', 'HKPD',
    'HKCC', 'HKDD');
var
  I: DWORD;
begin
...
procedure ExtractObjectFullName(const AObjectFullName: string;
  AParents: TStrings; var AObjectName: string);
var
  I: Integer;
  AName: string;
begin
...
procedure TcxStorage.RestoreFrom(AObject: TObject;
...
procedure TcxStorage.RestoreWithExistingReader(AObject: TObject;
  AReader: TcxCustomReader);
var
...
procedure TcxStorage.StoreTo(AObject: TObject;
...
procedure TcxStorage.StoreWithExistingWriter(AObject: TObject;
  AWriter: TcxCustomWriter);
begin
...
begin
  if Supports(FStoredObject, IcxStoredParent, AInterface) and
    (not Supports(AObject, IcxStoredObject2, AInterface2) or
    AInterface2.CanDelete) then
...
begin
  ...
  begin
    ...
    try
      if (FStoredObject is TCollection) and not Supports(FStoredObject,
        IcxStoredParent) then
      begin
...
function TcxStorage.GetFullObjectName(const ADefaultObjectName
  : string = ''): string;
var
  AObjectName: string;
begin
...
procedure TcxStorage.InternalCheckChildrenOrder(AChildrenNames,
    ...
    try
      ...
      try
        ...
        begin
          ...
          AChildrenClassNames.AddObject(APrevChildrenClassNames[AIndex],
            APrevChildrenClassNames.Objects[AIndex]);
        end;
      finally
...
begin
  ...
  try
    ...
    try
      ...
      AReader.ReadProperties(AObjectFullName, FStoredObject.ClassName,
        AProperties);

      AValue := AReader.ReadProperty(AObjectFullName, FStoredObject.ClassName,
        sPropertySourceDPI);
      ...
      begin
        ...
        if not VarIsNull(AValue) then
          try
            ...
            on EAbort do
            else if not(Assigned(FOnSetStoredPropertyValueError) and
              FOnSetStoredPropertyValueError(Self, APropertyName, AValue)) then
              raise;
          end;
      end;

      AReader.ReadChildren(AObjectFullName, FStoredObject.ClassName,
        AChildrenNames, AChildrenClassNames);
      ...
      try
        ...
        begin
          ...
          begin
            ...
            begin
              AChildObject := CreateChild(AChildrenNames[I],
                AChildrenClassNames[I]);
          ...
          begin
            ...
            AStorage.OnSetStoredPropertyValueError :=
              FOnSetStoredPropertyValueError;
            AStorage.FModes := Modes;
            AStorage.SaveComponentPropertiesByName :=
              FSaveComponentPropertiesByName;
            try
...
procedure TcxStorage.InternalStoreTo(AWriter: TcxCustomWriter;
  const ADefaultObjectName: string = '');
var
  AChildren: TStringList;
  AObjectFullName: string;
  ...
  try
    ...
    try
      ...
      if FStoredObjectPrevDPI > 0 then
        AWriter.WriteProperty(AObjectFullName, FStoredObject.ClassName,
          sPropertySourceDPI, FStoredObjectPrevDPI);
...
begin
  if (ATargetPPI > dxMinDPI) and
    (not Supports(FStoredObject, IdxScaleFactor, AScaleFactor) or
    (AScaleFactor.Value.TargetDPI <> ATargetPPI)) then
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if (AClass = TComponent) or (AClass.InheritsFrom(TComponent))
                    then
                    begin
...
procedure TcxCustomReader.ReadProperties(const AObjectName, AClassName: string;
  AProperties: TStrings);
begin
end;

function TcxCustomReader.ReadProperty(const AObjectName, AClassName,
...
constructor TcxCustomWriter.Create(const AStorageName: string;
  AReCreate: Boolean);
begin
...
procedure TcxCustomWriter.BeginWriteObject(const AObjectName,
  AClassName: string);
begin
end;
...
procedure TcxCustomWriter.WriteProperty(const AObjectName, AClassName,
...
procedure TcxCustomWriter.ClearObjectData(const AObjectFullName,
  AClassName: string);
begin
end;
...
procedure TcxStreamReader.ReadProperties(const AObjectName, AClassName: string;
  AProperties: TStrings);
var
  AObject: TcxStreamObjectData;
  I: Integer;
...
function TcxStreamReader.ReadProperty(const AObjectName, AClassName,
  AName: string): Variant;
var
...
function TcxStreamReader.GetObject(const AObjectFullName: string)
  : TcxStreamObjectData;
var
  AObjectName: string;
...
function TcxStreamReader.GetProperty(AObject: TcxStreamObjectData;
  const AName: string): TcxStreamPropertyData;
var
  I: Integer;
begin
  Result := nil;
...
function TcxStreamReader.InternalGetObject(const AObjectName: string;
  AParents: TStrings): TcxStreamObjectData;
var
  I, J: Integer;
  AObject: TcxStreamObjectData;
begin
...
procedure TcxStreamWriter.BeginWriteObject(const AObjectName,
  AClassName: string);
var
  AName: string;
  AParents: TStringList;
...
procedure TcxStreamWriter.WriteProperty(const AObjectName, AClassName,
  AName: string; AValue: Variant);
begin
...
procedure TcxStreamWriter.CreateObject(const AObjectName, AClassName: string;
  AParents: TStrings);
var
  I, J: Integer;
  AObject: TcxStreamObjectData;
...
procedure TcxRegistryReader.ReadProperties(const AObjectName,
  AClassName: string; AProperties: TStrings);
var
  AName: string;
  AParents: TStringList;
  ANewPath: string;
...
function TcxRegistryReader.ReadProperty(const AObjectName, AClassName,
  AName: string): Variant;
var
  AValue: string;
  ARealValue: Double;
...
constructor TcxRegistryWriter.Create(const AStorageName: string;
  AStream: TStream; AReCreate: Boolean = True);
begin
...
procedure TcxRegistryWriter.BeginWriteObject(const AObjectName,
  AClassName: string);
var
...
procedure TcxRegistryWriter.EndWriteObject(const AObjectName,
  AClassName: string);
var
  AName: string;
  AParents: TStringList;
...
procedure TcxRegistryWriter.WriteProperty(const AObjectName, AClassName,
  AName: string; AValue: Variant);
begin
  case VarType(AValue) of
    // CLR: varDecimal TODO
    varInt64, varLongWord, varWord, varShortInt, varInteger,
      varSmallInt, varByte:
...
procedure TcxRegistryWriter.ClearObjectData(const AObjectFullName,
  AClassName: string);
var
  AKeyName: string;
begin
  ...
  if (AKeyName <> '') and Registry.KeyExists(AKeyName) and
    not Registry.DeleteKey(AKeyName) then
...
begin
  ...
  begin
    if not Registry.CreateKey(GenRegistryPath(StorageName)) then
      raise ERegistryException.CreateFmt
        (cxGetResourceString(@scxCantCreateRegistryKey), [StorageName]);
  ...
  begin
    if not Registry.OpenKey(GenRegistryPath(StorageName), False) then
      raise ERegistryException.CreateFmt
        (cxGetResourceString(@scxCantOpenRegistryKey), [StorageName]);
...
procedure TcxIniFileReader.ReadProperties(const AObjectName, AClassName: string;
  AProperties: TStrings);
var
  ASectionName: string;
begin
...
function TcxIniFileReader.ReadProperty(const AObjectName, AClassName,
  AName: string): Variant;
var
  ASectionName: string;
  AValue: string;
...
begin
  ...
  case FStorageVersion of
    1, 2:
      Result := DecodeStringV2;
    0:
      Result := DecodeStringV0;
  else // -1
    Result := S;
  end;
end;
...
procedure TcxIniFileReader.GetSectionDetail(const ASection: string;
  var APath, AObjectName, AClassName: string);
var
  I: Integer;
  AName: string;
...
constructor TcxIniFileWriter.Create(const AStorageName: string;
  AStream: TStream; AReCreate: Boolean = True);
begin
  inherited Create(AStorageName, AStream, AReCreate);
...
procedure TcxIniFileWriter.BeginWriteObject(const AObjectName,
  AClassName: string);
begin
  IniFile.WriteString(AObjectName + ': ' + AClassName, '', '');
end;

procedure TcxIniFileWriter.WriteProperty(const AObjectName, AClassName,
  AName: string; AValue: Variant);
var
  ASectionName: string;
begin
  ASectionName := AObjectName + ': ' + AClassName;
  case VarType(AValue) of
    varShortInt, varWord, varLongWord, varInt64, varSmallInt,
      varInteger, varByte:
...
procedure TcxIniFileWriter.ClearObjectData(const AObjectFullName,
  AClassName: string);
var
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if (Length(ASectionName) > Length(AObjectName)) and
          (dxCharInSet(ASectionName[Length(AObjectName) + 1], [':', '/'])) then
...
constructor TcxStreamPropertyData.Create(const AName: string;
...
begin
  case VarType(FValue) of
    varShortInt, varWord, varLongWord, varInt64, varSmallInt, varInteger:
      begin
...
function TcxStreamObjectData.GetProperties(AIndex: Integer)
  : TcxStreamPropertyData;
begin
