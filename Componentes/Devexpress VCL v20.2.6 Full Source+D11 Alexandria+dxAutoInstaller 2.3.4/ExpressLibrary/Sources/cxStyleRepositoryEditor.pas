...
begin
  ...
  begin
    if TcxCustomStyleSheet(AList[I]).Name = '' then
      TcxCustomStyleSheet(AList[I]).Name := CreateUniqueName(Component.Owner,
        nil, TcxCustomStyleSheet(AList[I]), '', '');
...
procedure TcxStyleRepositoryEditor.DoAddStyleSheetMenuItemClick
  (Sender: TObject);
var
...
begin
  ...
  try
    ...
    AItem := StyleRepository.CreateStyleSheetEx
      (TcxCustomStyleSheetClass(AList[TMenuItem(Sender).Tag]),
...
procedure TcxStyleRepositoryEditor.SelectItem(AListBox: TListBox;
  AItem: TPersistent);
begin
  with AListBox do
...
begin
  ...
  try
    ...
    for I := 0 to StyleRepository.Count - 1 do
      lbStyles.Items.AddObject(StyleRepository.Items[I].Name,
        StyleRepository.Items[I]);
  finally
  ...
  try
    ...
    for I := 0 to StyleRepository.StyleSheetCount - 1 do
      lbStyleSheets.Items.AddObject(StyleRepository.StyleSheets[I].Name,
...
procedure TcxStyleRepositoryEditor.btStyleSheetDeleteClick(Sender: TObject);
begin
  DeleteSelectedComponents(lbStyleSheets);
end;

procedure TcxStyleRepositoryEditor.SelectionsChanged(const ASelection
...
begin
  if ShowcxStyleSheetEditor(TcxCustomStyleSheet(lbStyleSheets.Items.Objects
    [lbStyleSheets.ItemIndex]), DoStyleGetName) then
    Designer.Modified;
end;

procedure TcxStyleRepositoryEditor.btnStyleSheetsSaveClick(Sender: TObject);
var
  I: Integer;
  AList: TList;
begin
...
procedure TcxStyleRepositoryEditor.btnStyleSheetsLoadClick(Sender: TObject);
var
