...
uses
  ...
  ZLib,
  dxCore, dxGenerics, cxGeometry, dxCoreGraphics, dxXMLDoc, dxGDIPlusClasses,
  dxGDIPlusAPI, dxSmartImage, dxSVGCore;
...
type
  ...
  TdxSVGImageHandle = class(TdxSmartImageCustomHandle, IdxGpFlipContent,
    IdxImageDataFormatEx, IdxVectorImage)
  strict private
  const
    ...
    RasterizedImageBufferSize: array [TdxSVGRenderQuality] of Integer = (1024,
      1536, 2048);
    StandardScaleFactors: array [0 .. 9] of Integer = (64, 32, 24, 16, 12, 8,
      6, 4, 2, 1);
  strict private
    ...
    function CalculateScaleFactor(const ASize: TSize): Single;
  ...
  protected
    ...
    procedure DrawCore(ACanvas: TdxGPCanvas; const R: TdxRectF;
      APalette: IdxColorPalette); overload;
    procedure DrawCore(ATarget: TdxGPImageHandle;
      APalette: IdxColorPalette); overload;
  ...
  public
    ...
    function GetRasterizedImage(ASize: TSize; APalette: IdxColorPalette)
      : TdxGPImageHandle;
    // IdxGpFlipContent
  ...
  protected
    ...
    class procedure SaveSourceData(const ADocument: TdxXMLDocument;
      AHandle: TdxSVGImageHandle); overload;
  public
    class function CanLoadStream(AStream: TStream): Boolean; override;
    class function CanSaveImage(AHandle: TdxSmartImageCustomHandle): Boolean;
      override;
    ...
    class function Load(AStream: TStream;
      out AHandle: TdxSmartImageCustomHandle): Boolean; overload; override;
    class function Load(const ADocument: TdxXMLDocument;
  ...
  strict private
    class procedure Append(ATarget, ASource: TdxSVGElementRoot;
      AReferences: TdxSVGReferences; AOffsetX, AOffsetY: Single);
    class function Extract(ASource: TdxSVGElement; const AFrameSize: TSize)
      : TdxSVGImageHandle;
    class function IsMultiframeTexture(ARoot: TdxSVGElementRoot): Boolean;
    class procedure UpdateReferences(AReferences: TdxSVGReferences;
      AElement, ARootElement: TdxSVGElement);
  public
    class function Combine(AList: TList<TdxSVGImageHandle>): TdxSVGImageHandle;
    class function Split(AImage: TdxCustomSmartImage;
      out AList: TList<TdxSVGImageHandle>): Boolean; overload;
    class function Split(AImage: TdxSVGImageHandle;
      out AList: TList<TdxSVGImageHandle>): Boolean; overload;
  end;
...
procedure TdxSVGImageHandle.Draw(DC: HDC; const ADest, ASource: TdxRectF;
...
begin
  ...
  begin
    ...
    try
      ...
      dxGpDrawImage(dxGPPaintCanvas.Handle, ADest,
        cxRectScale(ASource, AImage.Width / Width), AImage.Handle, AAlpha);
    finally
...
procedure TdxSVGImageHandle.Draw(DC: HDC; const ADest, ASource: TRect;
  AAlpha: Byte; APalette: IdxColorPalette);
begin
...
procedure TdxSVGImageHandle.DrawCore(ACanvas: TdxGPCanvas; const R: TdxRectF;
...
begin
  ...
  ACanvas.ScaleWorldTransform(R.Width / ViewBox.Width,
...
procedure TdxSVGImageHandle.DrawCore(ATarget: TdxGPImageHandle;
  APalette: IdxColorPalette);
var
  ACanvas: TdxGPCanvas;
begin
...
function TdxSVGImageHandle.GetRasterizedImage(ASize: TSize;
  APalette: IdxColorPalette): TdxGPImageHandle;
var
  AViewBoxSize: TSize;
begin
  ...
  if not IsEqualGUID(FCachedImagePaletteID, dxGetColorPaletteID(APalette)) then
    FlushCache;
  if (FCachedImage <> nil) and ((FCachedImage.Width <> ASize.cx) or
    (FCachedImage.Height <> ASize.cy)) then
    FlushCache;
  if FCachedImage = nil then
  begin
    FCachedImageQuality := dxSVGRenderQuality;
    FCachedImage := TdxGPImageHandle.CreateSize(cxSizeScale(AViewBoxSize,
      CalculateScaleFactor(AViewBoxSize)));
...
function TdxSVGImageHandle.CalculateScaleFactor(const ASize: TSize): Single;
var
  AMaxScaleFactor: Single;
  AMaxSize: Integer;
  I: Integer;
begin
  AMaxSize := Max(ASize.cx, ASize.cy);
  AMaxScaleFactor := RasterizedImageBufferSize[FCachedImageQuality] / AMaxSize;
  ...
  Result := Max(AMaxScaleFactor, 0.1);
end;
...
begin
  ...
  else if IsWinSevenOrLater then
    Result := imBilinear
  else
...
class function TdxSVGImageCodec.CanSaveImage
...
class function TdxSVGImageCodec.GetSize(AStream: TStream;
...
class function TdxSVGImageCodec.Load(AStream: TStream;
...
class function TdxSVGImageCodec.Load(const ADocument: TdxXMLDocument;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
var
...
class function TdxSVGImageCodec.Load(const AFileName: string;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
var
  AStream: TStream;
begin
...
class function TdxSVGImageCodec.Save(AStream: TStream;
  AHandle: TdxSmartImageCustomHandle): Boolean;
var
...
class function TdxSVGImageCodec.GetGraphicClassForRegistrationInVCL
  : TGraphicClass;
begin
...
class procedure TdxSVGImageCodec.SaveSourceData(const ADocument: TdxXMLDocument;
  AHandle: TdxSVGImageHandle);
var
...
class function TdxSVGEditingHelper.Combine(AList: TList<TdxSVGImageHandle>)
  : TdxSVGImageHandle;
var
  AFrameSize: TSize;
...
begin
  ...
  ARoot.ViewBox.Value := dxRectF(0, 0, AFrameSize.cx * AList.Count,
    AFrameSize.cy);
...
class function TdxSVGEditingHelper.Split(AImage: TdxCustomSmartImage;
  out AList: TList<TdxSVGImageHandle>): Boolean;
begin
  if TdxCustomSmartImageAccess(AImage).Handle is TdxSVGImageHandle then
    Result := Split(TdxSVGImageHandle(TdxCustomSmartImageAccess(AImage)
      .Handle), AList)
  else
    Result := False;
end;

class function TdxSVGEditingHelper.Split(AImage: TdxSVGImageHandle;
  out AList: TList<TdxSVGImageHandle>): Boolean;
var
  AFrameSize: TSize;
  I: Integer;
...
class function TdxSVGEditingHelper.Extract(ASource: TdxSVGElement;
  const AFrameSize: TSize): TdxSVGImageHandle;
var
  ARoot: TdxSVGElementRoot;
  I: Integer;
begin
...
class function TdxSVGEditingHelper.IsMultiframeTexture
  (ARoot: TdxSVGElementRoot): Boolean;
var
...
class procedure TdxSVGEditingHelper.UpdateReferences
  (AReferences: TdxSVGReferences; AElement, ARootElement: TdxSVGElement);
var
  AReference: string;
  I: Integer;
begin
