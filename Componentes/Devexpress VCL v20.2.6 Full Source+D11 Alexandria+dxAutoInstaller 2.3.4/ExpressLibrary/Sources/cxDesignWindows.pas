...
uses
  Types, DesignIntf, DesignWindows, ComponentDesigner, DesignConst, Windows,
  Math,
  TypInfo, Classes, SysUtils, Controls, Graphics, Menus, Forms, StdCtrls,
  ExtCtrls,
  cxClasses, dxCore;
...
type
  ...
  public
    ...
    procedure ChangeSelection(AOwner: TComponent;
      AObject: TPersistent); overload;
    ...
    function UniqueName(AOwner: TComponent; const ABaseName: string)
      : string; overload;
  ...
  TcxSelectionsChanged = procedure(Sender: TObject;
    const ASelection: TDesignerSelectionList) of object;
  ...
  public
    ...
    procedure SelectionsChanged(const ASelection
      : TDesignerSelectionList); virtual;
    procedure SetSelectionList(AList: TList);
    procedure UpdateSelection;
    property LockCount: Integer read FLockCount;
    property OnSelectionsChanged: TcxSelectionsChanged read FOnSelectionsChanged
      write FOnSelectionsChanged;
  end;
  ...
  public
    ...
    procedure SelectComponents(AList: TList;
      ADefaultComponent: TPersistent = nil); virtual;

    procedure ListBoxApplySelection(AListBox: TListBox;
...
function CanAddComponent(AOwner: TComponent;
  const ADesigner: IDesigner = nil): Boolean;
function CanDeleteComponent(AOwner, AComponent: TComponent;
  const ADesigner: IDesigner = nil): Boolean;
function CanChangeComponentList(AOwner: TComponent;
  ADesigner: IDesigner = nil): Boolean;
...
procedure ConvertSelectionToList(const ASelectionList: TDesignerSelectionList;
  AList: TList);
...
procedure ListBoxRestoreSelection(AListBox: TListBox;
  var ASelection: TStringList; AItemIndex, ATopIndex: Integer);
...
procedure ListBoxSavePos(AListBox: TListBox;
  var AItemIndex, ATopIndex: Integer);
...
function ShowFormEditorClass(ADesigner: IDesigner; AComponent: TComponent;
  AFormEditorClass: TcxDesignFormEditorClass): TcxDesignFormEditor;
begin
  Result := ShowFormEditorClass(ADesigner, AComponent, nil, '',
    AFormEditorClass);
end;
...
begin
  ...
  Result := not ADesigner.IsSourceReadOnly and
    not(csInline in AOwner.ComponentState);
end;
...
begin
  ...
  try
    ...
    with AListBox do
      for I := 0 to Items.Count - 1 do
        if Selected[I] then
          ASelection.AddObject(Items[I], Items.Objects[I]);
    AListBox.Items.BeginUpdate;
  except
    ASelection.Free;
    ASelection := nil;
...
begin
  for Result := 0 to AListBox.Items.Count - 1 do
    if AListBox.Selected[Result] then
      Exit;
  Result := -1;
end;
...
procedure ListBoxMoveDownItems(AListBox: TListBox; var APrevDragIndex: Integer;
  AReindexProc: TcxListBoxReindexProc);
begin
  ListBoxMoveItems(AListBox, Min(AListBox.Items.Count - 1,
    ListBoxGetLastSelectedIndex(AListBox) + 1), APrevDragIndex, AReindexProc);
end;
...
procedure TcxDesignHelper.SelectObject(AObject: TPersistent;
  AClearSelection: Boolean = True; AActivateOwner: Boolean = True);
begin
...
procedure TcxDesignHelper.RemoveSelectionChangedListener
...
procedure TcxDesignHelper.ChangeSelection(AOwner: TComponent;
  AObject: TPersistent);
begin
...
function TcxDesignHelper.IsObjectSelected(AOwner: TComponent;
...
procedure TcxDesignHelper.SelectObject(AOwner: TComponent; AObject: TPersistent;
  AClearSelection: Boolean = True; AActivateOwner: Boolean = True);
var
  AList: TList;
  ADesigner: IDesigner;
...
procedure TcxDesignHelper.ShowComponentDefaultEventHandler
...
begin
  ...
  begin
    ...
    AMethod := ADesigner.CreateMethod(AMethodName,
      GetTypeData(APropInfo.PropType^));
...
function TcxDesignHelper.UniqueName(AOwner: TComponent;
...
procedure TcxDesignHelper.UnselectObject(AOwner: TComponent;
  AObject: TPersistent);
var
  AList: TList;
begin
...
function TcxDesignHelper.QueryInterface(const IID: TGUID; out Obj)
  : HResult; stdcall;
begin
...
class function TcxDesignWindow.GetBaseRegKey
  (ADesigner: IComponentDesigner = nil): string;
begin
  if ADesigner = nil then
...
procedure TcxDesignWindow.SelectionsChanged(const ASelection
...
procedure TcxGlobalDesignWindow.SelectionChanged(const ADesigner: IDesigner;
  const ASelection: TDesignerSelectionList);
begin
  if LockCount <> 0 then
    Exit;
...
procedure TcxDesignFormEditor.DesignerClosed(const ADesigner: IDesigner;
...
procedure TcxDesignFormEditor.ItemDeleted(const ADesigner: IDesigner;
  AItem: TPersistent);
begin
...
begin
  if Closing or (Component = nil) or (csDestroying in Component.ComponentState)
  then
    Exit;
  UpdateCaption;
  DoItemsModified;
...
procedure TcxDesignFormEditor.SelectComponents(AList: TList;
  ADefaultComponent: TPersistent);
begin
  if not Active then
    Exit;
...
function TcxDesignFormEditor.CanDeleteComponent(AComponent: TComponent)
  : Boolean;
begin
  Result := cxDesignWindows.CanDeleteComponent(Component.Owner, AComponent,
    Designer);
end;
