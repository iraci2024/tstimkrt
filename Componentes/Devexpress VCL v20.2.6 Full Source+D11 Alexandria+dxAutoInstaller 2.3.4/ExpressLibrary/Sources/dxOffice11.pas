...
uses
  Types, SysUtils, TypInfo, Windows, Messages, Graphics, Controls, Forms,
  Classes, cxGraphics, cxGeometry;
...
var
  ...
  // bar: marks
  dxOffice11BarMarkColors1: array [1 .. 3] of TColor;
  // Normal, Selected, Pressed
  dxOffice11BarMarkColors2: array [1 .. 3] of TColor;
  // Normal, Selected, Pressed
...
procedure Office11FillTubeGradientRect(DC: HDC; const ARect: TRect;
  AColor1, AColor2: TColor; AHorizontal: Boolean);
procedure Office11FrameSelectedRect(DC: HDC; const R: TRect);
procedure Office11DrawFingerElements(DC: HDC; ARect: TRect;
  AHorizontal: Boolean; ABrush1: HBRUSH = 0; ABrush2: HBRUSH = 0;
...
procedure Office11DrawLargeItemArrow(ADC: HDC; R: TRect;
  AArrowType: TcxArrowDirection; ASize: Integer;
...
procedure Office11DrawSizeGrip(DC: HDC; ARect: TRect;
  ...
  AScaleFactor: TdxScaleFactor = nil);
function GetOffice11FingerElementSize(AScaleFactor
  : TdxScaleFactor = nil): Integer;
  ...
  for I := 0 to ABitmap.Width - 1 do
    ...
    begin
      ...
      begin
        ...
        AValue := GetRValue(APixelColor);
        APixelColor := dxGetMiddleRGB(AColor1, AColor2,
...
begin
  ...
  dxOffice11BarSeparatorBrush1 :=
    CreateSolidBrush(dxOffice11BarSeparatorColor1);
  dxOffice11BarSeparatorBrush2 :=
    CreateSolidBrush(dxOffice11BarSeparatorColor2);
  dxOffice11DropDownBorderBrush1 :=
    CreateSolidBrush(dxOffice11DropDownBorderColor1);
  dxOffice11MenuBrush := CreateSolidBrush(dxOffice11MenuColor);
  dxOffice11DropDownBorderBrush2 := dxOffice11MenuBrush;
  dxOffice11SelectedBorderBrush :=
    CreateSolidBrush(dxOffice11SelectedBorderColor);
  dxOffice11OwnerControlDownedBrush :=
    CreateSolidBrush(dxOffice11OwnerControlDownedColor);
  dxOffice11DownedBrush := CreateSolidBrush(dxOffice11DownedColor);
  dxOffice11DownedSelectedBrush :=
    CreateSolidBrush(dxOffice11DownedSelectedColor);
  dxOffice11DetachableBrush := CreateSolidBrush(dxOffice11DetachableColor);
  dxOffice11ToolbarSelectedBrush :=
    CreateSolidBrush(dxOffice11ToolbarSelectedColor);
  ...
  dxOffice11InPlaceSubItemBrush :=
    CreateSolidBrush(dxOffice11InPlaceSubItemColor);
  // TODO
  dxOffice11SubMenuExpandBitmap := TBitmap.Create;
  dxOffice11SubMenuExpandBitmap.Handle :=
    LoadBitmap(HInstance, 'DXSUBMENUEXPAND');
  PrepareSubMenuExpandBitmap(dxOffice11SubMenuExpandBitmap);
  dxOffice11ExpandButtonBitmap1 := TBitmap.Create;
  dxOffice11ExpandButtonBitmap1.LoadFromResourceName(HInstance,
    'DXEXPANDBUTTON_MINUS');
  dxOffice11ExpandButtonBitmap2 := TBitmap.Create;
  dxOffice11ExpandButtonBitmap2.LoadFromResourceName(HInstance,
    'DXEXPANDBUTTON_PLUS');
...
begin
  ...
  if dxOffice11BarFingerBrush1 <> 0 then
    DeleteObject(dxOffice11BarFingerBrush1);
  if dxOffice11BarFingerBrush2 <> 0 then
    DeleteObject(dxOffice11BarFingerBrush2);
  if dxOffice11BarSeparatorBrush1 <> 0 then
    DeleteObject(dxOffice11BarSeparatorBrush1);
  if dxOffice11BarSeparatorBrush2 <> 0 then
    DeleteObject(dxOffice11BarSeparatorBrush2);
  if dxOffice11DropDownBorderBrush1 <> 0 then
    DeleteObject(dxOffice11DropDownBorderBrush1);
  if dxOffice11MenuBrush <> 0 then
    DeleteObject(dxOffice11MenuBrush);
  if dxOffice11SelectedBorderBrush <> 0 then
    DeleteObject(dxOffice11SelectedBorderBrush);
  if dxOffice11OwnerControlDownedBrush <> 0 then
    DeleteObject(dxOffice11OwnerControlDownedBrush);
  if dxOffice11DownedBrush <> 0 then
    DeleteObject(dxOffice11DownedBrush);
  if dxOffice11DownedSelectedBrush <> 0 then
    DeleteObject(dxOffice11DownedSelectedBrush);
  if dxOffice11DetachableBrush <> 0 then
    DeleteObject(dxOffice11DetachableBrush);
  if dxOffice11ToolbarSelectedBrush <> 0 then
    DeleteObject(dxOffice11ToolbarSelectedBrush);
  if dxOffice11InPlaceSubItemBrush <> 0 then
    DeleteObject(dxOffice11InPlaceSubItemBrush);
end;
...
const
  ...
  SMetallic = 'METALLIC';
var
  PThemeFileName, PThemeColor, PThemeSize: array [0 .. SZ_MAX_CHARS]
    of WideChar;
  S: string;
begin
  Result := schUnknown;
  if AreVisualStylesAvailable([]) and
    Succeeded(GetCurrentThemeName(PThemeFileName, SZ_MAX_CHARS, PThemeColor,
  ...
  begin
    ...
    begin
      ...
      else if S = SHomestead then
        Result := schHomestead
      else if S = SMetallic then
        Result := schMetallic;
    end;
  end;
end;
...
begin
  ...
  Result := RGB(TransformColorComponent(GetRValue(Result)),
    TransformColorComponent(GetGValue(Result)),
    TransformColorComponent(GetBValue(Result)));
...
const
  Office11Colors: array [TOffice11SchemeColor, 0 .. 48] of TColor =
    (($FEECDD, $E2A981, $F5BE9E, $F9DAC3, $913500, $F1A675, $76C1FF, $D3F8FF,
    $98DDFF, $5295FE, $9C613B, $E0A47B, $764127, $FFFFFF, $CB8C6A, $FFF9F1,
    $C9662A, $F9DBC4, $962D00, $F6F6F6, $C2EEFF, $800000, $CCF4FF, $91D0FF,
    $8ED3FF, $4E91FE, $FCE1CB, $6FC0FF, $3E80FE, $F0C7A9, $000000, $8D8D8D,
    $FBDABE, $94E6FB, $FADAC4, $962D00, $B96837, $962D00, $FBE5D3, $E4AE88,
    $DCFFFF, $5BC0F7, $94E6FB, $1595EE, $087FE8, $7CDAF7, $D68759, $933803,
    $FFFFFF), ($DEF7F4, $91C6B7, $A7D9D9, $E4F0F2, $6B7760, $8CC2B0, $76C1FF,
    $D3F8FF, $98DDFF, $5295FE, $588060, $8FC4B5, $335E51, $FFFFFF, $588060,
    $DEF7F4, $5E8674, $ADDEE1, $5E8D75, $EEF4F4, $C2EEFF, $385D3F, $CCF4FF,
    $91D0FF, $8ED3FF, $4E91FE, $B6E3D8, $6FC0FF, $3E80FE, $9FD4C5, $000000,
    $8D8D8D, $91BAAF, $94E6FB, $E4F1F2, $588060, $548973, $5E8D75, $DBF5F2,
    $97C9BC, $DCFFFF, $5BC0F7, $94E6FB, $1595EE, $087FE8, $7CDAF7, $82C0AF,
    $447A63, $FFFFFF), ($FAF4F3, $B59799, $E5D7D7, $F7F3F3, $927476, $C8B2B3,
    $76C1FF, $D3F8FF, $98DDFF, $5295FE, $947C7C, $B09193, $755454, $FFFFFF,
    $8F6D6E, $FFFFFF, $99797A, $E4DADB, $947C7C, $FFFAFD, $C2EEFF, $6F4B4B,
    $CCF4FF, $91D0FF, $8ED3FF, $4E91FE, $E7D3D6, $6FC0FF, $3E80FE, $D3C0C0,
    $000000, $8D8D8D, $EBE5E5, $94E6FB, $F7F3F3, $947C7C, $916F70, $947C7C,
    $F8F1F0, $BA9EA0, $DCFFFF, $5BC0F7, $94E6FB, $1595EE, $087FE8, $7CDAF7,
    $BFA7A8, $916F70, $FFFFFF));
  Office11GradientPercents: array [TOffice11SchemeColor] of Integer =
    (41, 125, 40);
var
  I: Integer;
begin
  FOffice11Scheme := dxGetOffice11Scheme;
  FXPStandardScheme := AreVisualStylesAvailable([]) and
    (FOffice11Scheme <> schUnknown) { luna };
  ...
  begin
    ...
    for I := 1 to 3 do
      dxOffice11BarBorderMarkColors[1, I] :=
        dxGetMiddleRGB(dxOffice11BarMarkColors1[I], dxOffice11ToolbarsColor1,
        90); // #6 ???
    for I := 1 to 3 do
      dxOffice11BarBorderMarkColors[2, I] :=
        dxGetMiddleRGB(dxOffice11BarMarkColors2[I],
        dxOffice11BarBorderColors[2], 50); // #9
    // bar: finger
  ...
  begin
    ...
    dxOffice11BarMarkColors1[1] := dxGetMiddleRGB(dxOffice11BarMarkColors2[1],
      clWindow, 74);
    // TODO
    dxOffice11BarMarkColors1[2] :=
      dxGetNearestColor(GetLightColor(-15, 29, 100 - -15 - 29)); // Selected
    // dxOffice11BarMarkColors1[2] := FlatToolbarsSelColor;
    dxOffice11BarMarkColors2[2] := dxOffice11BarMarkColors1[2];
    dxOffice11BarMarkColors1[3] :=
      dxGetNearestColor(GetLightColor(42, 11, 100 - 42 - 11)); // Downed
    ...
    for I := 1 to 3 do
      dxOffice11BarBorderMarkColors[1, I] :=
        dxGetMiddleRGB(dxOffice11BarMarkColors2[I], clWindow, 50); // #6
    for I := 1 to 3 do
      dxOffice11BarBorderMarkColors[2, I] :=
        dxGetMiddleRGB(dxOffice11BarMarkColors2[I],
        dxOffice11BarBorderColors[2], 50); // #9
    ...
    // selected
    dxOffice11ToolbarSelectedColor :=
      dxGetNearestColor(GetLightColor(-2, 30, 72)); // ?
    ...
    dxOffice11DownedSelectedColor :=
      dxGetNearestColor(GetLightColor(14, 44, 40)); // ?
    ...
    dxOffice11NavPaneGroupCaptionColor2 :=
      GetdxOffice11NavPaneGroupCaptionColor(False);
  ...
  dxOffice11MenuNonRecentIndentColor1 :=
    dxGetMiddleRGB(dxOffice11MenuIndentColor1, clBlack, 92);
  dxOffice11MenuNonRecentIndentColor2 :=
    dxGetMiddleRGB(dxOffice11MenuIndentColor2, clBlack, 92);
  // selected
...
begin
  ...
  repeat
    ...
    begin
      ...
      if R1.Bottom > ARect.Bottom then
        Break;
    end
    else
    begin
      OffsetRect(R1, ASize, 0);
      if R1.Right > ARect.Right then
        Break;
    end;
  until False;
end;
...
procedure Office11FillTubeGradientRect(DC: HDC; const ARect: TRect;
  AColor1, AColor2: TColor; AHorizontal: Boolean);
begin
  FillTubeGradientRect(DC, ARect, AColor1, AColor2, GradientPercent,
    AHorizontal);
end;
...
begin
  ...
  ARect := Rect(ARect.Right - 12, ARect.Bottom - AScaleFactor.Apply(3),
    ARect.Right, ARect.Bottom);
  Office11DrawFingerElements(DC, ARect, False, ABrush1, ABrush2, AScaleFactor);
  // 3
  Inc(ARect.Left, 4);
  ...
  // 1
  if ABrush1 <> 0 then
    DeleteObject(ABrush1);
  if ABrush2 <> 0 then
    DeleteObject(ABrush2);
end;
