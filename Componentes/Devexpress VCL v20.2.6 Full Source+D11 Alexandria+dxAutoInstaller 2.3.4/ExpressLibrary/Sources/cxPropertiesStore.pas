...
type
  ...
  TcxPropertiesStoreComponent = class(TcxInterfacedCollectionItem,
    IcxScalableComponent, IcxStoredObject, IcxStoredParent)
  private
    ...
    procedure InternalGetPropertyValue(const AName: string;
      var AValue: Variant);
    procedure InternalSetPropertyValue(const AName: string;
  ...
  public
    ...
    property ComponentItems[Index: Integer]: TcxPropertiesStoreComponent
      read GetComponentItem write SetComponentItem; default;
  end;
  ...
  protected
    ...
    property Components: TcxPropertiesStoreComponents read FComponents
      write SetComponents;
    property StorageName: string read GetStorageName write FStorageName;
    property StorageType: TcxStorageType read FStorageType write FStorageType
      default stIniFile;
  public
...
procedure TcxPropertiesStoreComponent.RestoreFrom(AStorage: TcxStorage;
...
begin
  ...
  if TcxControlHelper.GetCurrentDPIFromControl(Component) <> AStorage.StoredDPI
  then
    TcxControlHelper.SetOriginalParentSize(Component, AOriginalParentSize);
end;

procedure TcxPropertiesStoreComponent.RestoreFromIniFile
...
procedure TcxPropertiesStoreComponent.RestoreFromRegistry
  (const AStorageName: string);
var
  AStorage: TcxStorage;
begin
...
procedure TcxPropertiesStoreComponent.StoreTo(AStorage: TcxStorage;
...
procedure TcxPropertiesStoreComponent.StoreToIniFile(const AStorageName: string;
  const AReCreate: Boolean);
var
  AStorage: TcxStorage;
begin
...
procedure TcxPropertiesStoreComponent.StoreToRegistry(const AStorageName
  : string; const AReCreate: Boolean);
var
  AStorage: TcxStorage;
begin
  ExtractProperties;
...
procedure TcxPropertiesStoreComponent.StoreToStream(const AStream: TStream;
  const AReCreate: Boolean);
var
  AStorage: TcxStorage;
begin
...
procedure TcxPropertiesStoreComponent.DeleteChild(const AObjectName: string;
  AObject: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          AChild := GetCollectionItemByName(APersistent as TCollection,
            APropName);
          if AChild <> nil then
            AChildren.AddObject(FProperties[I] { APropName } , AChild);
        end;
...
function TcxPropertiesStoreComponent.GetPropertiesStore
  : TcxCustomPropertiesStore;
begin
...
begin
  ...
  for I := 0 to FProperties.Count - 1 do
  begin
    APersistent := ExtractPersistentAndPropertyName(FComponent,
      FProperties[I], AName);
...
function TcxPropertiesStoreComponent.ExtractPersistentAndPropertyName
  ...
  begin
    ...
    else if AStartPersistent is TCollection then
  ...
  begin
    ...
    begin
      ...
      if APersistent <> nil then
        Result := ExtractPersistentAndPropertyName(APersistent, AFullName,
          AResultName);
    end;
  end;
end;

function TcxPropertiesStoreComponent.GetCollectionItemByName
  (ACollection: TCollection; const AName: string): TCollectionItem;
var
  AInteger: Integer;
  ACode: Integer;
begin
...
function TcxPropertiesStoreComponent.GetComponentByName(const AName: string)
  : TComponent;
var
...
procedure TcxPropertiesStoreComponent.InternalGetPropertyValue
...
procedure TcxPropertiesStoreComponent.InternalSetPropertyValue
...
procedure TcxPropertiesStoreComponent.AssignStorageProperties
  (AStorage: TcxStorage);
begin
  with AStorage do
  begin
    Modes := [smSavePublishedClassProperties, smChildrenCreating,
      smChildrenDeleting];
...
function TcxPropertiesStoreComponents.GetPropertiesStore
  : TcxCustomPropertiesStore;
begin
  Result := TcxPropertiesStore(Owner);
end;

procedure TcxPropertiesStoreComponents.RemoveComponent(const AComponent
  : TComponent);
var
  AList: TList;
  I: Integer;
...
function TcxPropertiesStoreComponents.GetComponentItem(Index: Integer)
  : TcxPropertiesStoreComponent;
begin
...
begin
  ...
  FComponents := TcxPropertiesStoreComponents.Create(Self,
    TcxPropertiesStoreComponent);
  FStorageName := '';
...
function TcxCustomPropertiesStore.CreateWriter(AReCreate: Boolean)
  : TcxCustomWriter;
begin
  Result := nil;
  case FStorageType of
...
procedure TcxCustomPropertiesStore.SetComponents(const Value
  : TcxPropertiesStoreComponents);
begin
