...
uses
  SysUtils, Classes, Forms, Math, Generics.Defaults, Generics.Collections,
  dxGdiPlusClasses;

type
  TdxHttpRequestMode = (mhrmRead, mhrmWrite);
  TdxHttpRequestEvent = procedure(ASender: TObject;
    ARequestMode: TdxHttpRequestMode; ACount: Int64; var ACancel: Boolean)
    of object;
  ...
  public
    ...
    function Get(const AUri: string; AResponseContent: TStream): Boolean;
      virtual; abstract;
  ...
  strict private
    class var FHandlers: TList<TdxHttpRequestHandlerClass>;
...
{$ENDIF}
  public
    ...
    class function GetContent(const AUri: string;
...
begin
  ...
  try
    Result := (AHandler <> nil) and AHandler.Get(AUri, AResponseContent);
  finally
    AHandler.Free;
  end;
end;
...
class procedure TdxHttpRequest.RegisterHandler(AHandlerClass
  : TdxHttpRequestHandlerClass);
begin
