...
const
  dxTouchInteractiveGestureToGestureID: array [TInteractiveGesture]
    of Integer = (GID_ZOOM, GID_PAN, GID_ROTATE, GID_TWOFINGERTAP,
    GID_PRESSANDTAP);
  dxTouchInteractiveGestureOptionToPanOption
    : array [igoPanSingleFingerHorizontal .. igoPanGutter] of Integer =
  ...
  dxTouchPanOptions = GC_PAN or GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY or
    GC_PAN_WITH_SINGLE_FINGER_VERTICALLY or GC_PAN_WITH_INERTIA or
    GC_PAN_WITH_GUTTER;
  dxSupportedGestureCount = 5;
  dxSupportedGestureOptions: array [0 .. dxSupportedGestureCount - 1]
    of Integer = (dxTouchPanOptions, GC_ZOOM, GC_ROTATE, GC_TWOFINGERTAP,
    GC_PRESSANDTAP);
  dxSupportedGestureIDs: array [0 .. dxSupportedGestureCount - 1] of Integer =
    (GID_PAN, GID_ZOOM, GID_ROTATE, GID_TWOFINGERTAP, GID_PRESSANDTAP);
...
function dxSetGestureConfig(AWindow: THandle;
  const AGestureConfigs: TdxGestureConfigs): Boolean;
function dxLogicalToPhysicalPoint(AWindow: THandle; var APoint: TPoint)
  : Boolean;
function dxPhysicalToLogicalPoint(AWindow: THandle; var APoint: TPoint)
  : Boolean;
function dxLockGestures(AHandle: THandle;
  var Message: TWMGestureNotify): Boolean;
function dxIsTouchEvent(Shift: TShiftState): Boolean;
function GetInteractiveGestureByGestureID(AGestureId: Integer)
  : TInteractiveGesture;
function GetInteractiveGestureOptionByPanOptionID(APanOptionID: Integer)
  : TInteractiveGestureOption;
function GetInteractiveGestureOptionsByPanOptions(APanOptions: Integer)
  : TInteractiveGestureOptions;
function GetPanOptionsByInteractiveGestureOptions(AGestureOptions
  : TInteractiveGestureOptions): Integer;
...
var
  ...
  FLogicalToPhysicalPoint: function(HWND: HWND; var lpPoint: TPoint)
    : BOOL; stdcall;
  FPhysicalToLogicalPoint: function(HWND: HWND; var lpPoint: TPoint)
    : BOOL; stdcall;
...
procedure dxTouchWndProcHook(ACode: Integer; WPARAM: WPARAM; lParam: lParam;
...
begin
  ...
  begin
    FGestureStartPoint := SmallPointToPoint
      (PdxGestureNotifyStruct(Windows.PCWPStruct(lParam)^.lParam)^.ptsLocation);
...
begin
  ...
  case APanOptionID of
    GC_PAN_WITH_SINGLE_FINGER_VERTICALLY:
      Result := igoPanSingleFingerVertical;
    GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY:
      Result := igoPanSingleFingerHorizontal;
    GC_PAN_WITH_GUTTER:
      Result := igoPanGutter;
    GC_PAN_WITH_INERTIA:
      Result := igoPanInertia;
...
const
  APanOptionIDCount = 4;
  APanOptionIDs: array [0 .. APanOptionIDCount - 1] of Integer =
    (GC_PAN_WITH_SINGLE_FINGER_VERTICALLY,
...
begin
  ...
  for I := 0 to APanOptionIDCount - 1 do
    if (APanOptionIDs[I] and APanOptions) <> 0 then
      Include(Result, GetInteractiveGestureOptionByPanOptionID
        (APanOptionIDs[I]));
end;
...
begin
  if Assigned(FSetGestureConfig) then
    Result := FSetGestureConfig(AWindow, 0, Length(AGestureConfigs),
      AGestureConfigs, SizeOf(TdxGestureConfig))
  else
    Result := False;
end;
...
begin
  if NeedPanningFeedback(AScrollKind) and not InRange(ANewDataPos,
    AMinDataPos + 1, AMaxDataPos - 1) then
...
procedure TdxGestureHelper.CheckGestureOptions(var Gestures
...
begin
  ...
  if igPan in Gestures then
    ...
    else if Options * [igoPanSingleFingerHorizontal, igoPanSingleFingerVertical]
      <> [] then
    begin
  ...
  if Options * [igoPanSingleFingerHorizontal, igoPanSingleFingerVertical] = []
  then
    Gestures := Gestures - [igPan];
end;

procedure TdxGestureHelper.DoGesture(const EventInfo: TGestureEventInfo;
  var Handled: Boolean);
begin
  case EventInfo.GestureID of
    igiPan:
      Handled := Pan(EventInfo.Location, gfBegin in EventInfo.Flags,
        gfEnd in EventInfo.Flags, gfInertia in EventInfo.Flags);
    igiZoom:
      Handled := Zoom(EventInfo.Distance, gfBegin in EventInfo.Flags,
        gfEnd in EventInfo.Flags);
    igiRotate:
      Handled := Rotate(EventInfo.Angle, gfBegin in EventInfo.Flags,
        gfEnd in EventInfo.Flags);
  end;
end;
...
begin
  ...
  if NeedPanningFeedback(sbHorizontal) or NeedPanningFeedback(sbVertical) then
...
begin
  ...
  if GetGestureInfo(Message.lParam, AGestureInfo) then
    try
      ...
      if (AEventInfo.GestureID <> igiBegin) and (AEventInfo.GestureID <> igiEnd)
        and ((FTouchControl <> nil) or ((gfBegin in AEventInfo.Flags) and
        (FNotifyTouchControl <> nil))) then
      begin
        ...
        while (AControl.Parent <> nil) and
          ...
          not(AGestureMap[AEventInfo.GestureID - igiZoom]
          in AControl.Touch.InteractiveGestures) do
        ...
        case AEventInfo.GestureID of
          igiPan:
            begin
              ...
              AEventInfo.InertiaVector := InertiaVectorFromArgument
                (AGestureInfo.ullArguments);
            end;
          igiZoom, igiTwoFingerTap:
            AEventInfo.Distance := Cardinal(AGestureInfo.ullArguments);
          igiPressAndTap:
            begin
              APoint := SmallPointToPoint
                (TSmallPoint(Cardinal(AGestureInfo.ullArguments)));
              ...
              AEventInfo.TapLocation :=
                PointToSmallPoint(AControl.ScreenToClient(APoint));
            end;
          igiRotate:
            AEventInfo.Angle := RotateAngleFromArgument
              (AGestureInfo.ullArguments);
        end;
  ...
  begin
    if not IsPanArea(APoint) then
      APanOptions := APanOptions and not GC_PAN_WITH_SINGLE_FINGER_VERTICALLY
        and not GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY
    else
    begin
      if not AllowPan(sbVertical) then
        APanOptions := APanOptions and not GC_PAN_WITH_SINGLE_FINGER_VERTICALLY;
...
function TdxGestureHelper.NeedPanningFeedback(AScrollKind
...
procedure TdxGestureHelper.DoOverpan(AScrollKind: TScrollBarKind;
  ADelta: Integer);
begin
  if AScrollKind = sbHorizontal then
...
function TdxGestureHelper.Pan(APos: TPoint;
  AIsBegin, AIsEnd, AIsInertia: Boolean): Boolean;
  ...
  begin
    ...
    if (AOverpan <> 0) and (ADelta <> 0) and (Sign(ADelta) <> Sign(AOverpan))
      and NeedPanningFeedback(AScrollKind) then
    begin
...
begin
  ...
  else if FGestureClient <> nil then
  begin
...
function TdxGestureHelper.Rotate(AAngle: Double;
  AIsBegin, AIsEnd: Boolean): Boolean;
var
...
function TdxGestureHelper.Zoom(ADistance: Integer;
  AIsBegin, AIsEnd: Boolean): Boolean;
var
...
begin
  ...
  @FCloseGestureInfoHandle := GetProcAddress(FUser32Lib,
    'CloseGestureInfoHandle');
  @FSetGestureConfig := GetProcAddress(FUser32Lib, 'SetGestureConfig');
  @FLogicalToPhysicalPoint := GetProcAddress(FUser32Lib,
    'LogicalToPhysicalPoint');
  @FPhysicalToLogicalPoint := GetProcAddress(FUser32Lib,
