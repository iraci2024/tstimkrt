...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Messages, Classes, Controls, Forms, Dialogs, Graphics, Menus,
  StdCtrls, SysUtils, Math,
  dxCore, dxCoreClasses, dxMessages, dxThemeManager, dxUxTheme, cxClasses,
  cxControls, MultiMon,
...
const
  ...
  cxContainerDefaultBorderExtent: TRect = (Left: cxContainerMaxBorderWidth;
    Top: cxContainerMaxBorderWidth; Right: cxContainerMaxBorderWidth;
...
const
  ...
  cxContainerStyleValueNameA: array [0 .. cxContainerStyleValueCount - 1]
    of string = ('BorderColor', 'BorderStyle', 'Color', 'Edges', 'Font',
    'HotTrack', 'Shadow', 'TextColor', 'TextStyle', 'TransparentBorder');
...
type
  ...
  protected
    ...
    function CalculateBackgroundColor(AStyle: TcxContainerStyle;
      AEnabled: Boolean; AContainer: TcxContainer = nil): TColor; virtual;
    function CalculateBorderColor(AStyle: TcxContainerStyle;
      AEnabled, AIsDesigning: Boolean; AContainer: TcxContainer = nil)
      : TColor; virtual;
    function CalculateTextColor(AStyle: TcxContainerStyle; AEnabled: Boolean;
      AContainer: TcxContainer = nil): TColor; virtual;
    function GetContainerBorderStyle: TcxContainerBorderStyle; virtual;
    function IsStyleValueAssigned(AStyle: TcxContainerStyle;
      AValue: TcxContainerStyleValue): Boolean; inline;
  ...
  private
    ...
    function InternalGetBorderStyle(var BorderStyle
      : TcxContainerBorderStyle): Boolean;
    ...
    function InternalGetTransparentBorder(var TransparentBorder
      : Boolean): Boolean;
  ...
  protected
    ...
    procedure ControllerChangedNotification(AStyleController
      : TcxStyleController); virtual;
    procedure ControllerFreeNotification(AStyleController
      : TcxStyleController); virtual;
    ...
    function InternalGetNotPublishedExtendedStyleValues
      : TcxContainerStyleValues; virtual;
  ...
  public
    ...
    function IsExtendedStylePropertyPublished(const APropertyName
      : string): Boolean;
  ...
  published
    ...
    property Edges: TcxBorders read GetEdges write SetEdges
      stored IsEdgesStored;
    property Font: TFont read GetFont write SetFont stored IsFontStored;
    property HotTrack: Boolean read GetHotTrack write SetHotTrack
      stored IsHotTrackStored;
  ...
  TcxContainer = class(TcxControl, IUnknown, IcxCompoundControl,
  ...
  private
    ...
    procedure GetStandardScrollBarParameters(AKind: TScrollBarKind;
      out AScrollInfo: TScrollInfo);
    function GetStyle: TcxContainerStyle;
    function GetStyleDisabled: TcxContainerStyle;
    function GetStyleFocused: TcxContainerStyle;
    function GetStyleHot: TcxContainerStyle;
    ...
    procedure CMShortRefreshContainer(var Message: TMessage);
      message DXM_SHORTREFRESHCONTAINER;
    ...
    procedure CMUpdateScrollBars(var Message: TMessage);
      message DXM_UPDATESCROLLBARS;

    procedure DXMUpdateWindowRegion(var Message: TMessage);
      message DXM_UPDATEWINDOWREGION;
  protected
    ...
    procedure CalculateViewInfo(const P: TPoint;
      AMouseTracking: Boolean); virtual;
    ...
    function InternalGetNotPublishedStyleValues
      : TcxContainerStyleValues; virtual;
    ...
    function IsScrollBarVisible(AKind: TScrollBarKind;
      out AScrollBarinfo: TScrollBarInfo): Boolean;
    ...
    procedure SetScrollBarVisible(AScrollBar: IcxControlScrollBar;
...
{$ENDIF}
  ...
  private
    ...
    procedure CMClosePopupWindow(var Message: TMessage);
      message DXM_CLOSEPOPUPWINDOW;
    procedure CMMouseEnter(var Message: TMessage); message CM_MOUSEENTER;
    procedure CMMouseLeave(var Message: TMessage); message CM_MOUSELEAVE;
    procedure CMRecreateWnd(var Message: TMessage); message CM_RECREATEWND;
    procedure CMShowingChanged(var Message: TMessage);
      message CM_SHOWINGCHANGED;
    procedure CMShowPopupWindow(var Message: TMessage);
      message DXM_SHOWPOPUPWINDOW;
  ...
  public
    ...
    property CaptureFocus: Boolean read FCaptureFocus write SetCaptureFocus
      default True;
    ...
    property OnBeforeClosing: TNotifyEvent read FOnBeforeClosing
      write FOnBeforeClosing;
...
{$ENDIF}
    ...
    procedure DoScroll(AKind: TScrollBarKind; AScrollCode: TScrollCode;
      AScrollPos: Integer);
...
function cxGetScrollBarInfo(hwnd: hwnd; idObject: Longint;
  var psbi: TcxScrollBarInfo): Boolean; overload;
function cxGetScrollBarInfo(hwnd: hwnd; idObject: Longint;
  var psbi: TcxScrollBarInfo; AScaleFactor: TdxScaleFactor;
  AIsRightToLeftLayout: Boolean): Boolean; overload;
...
procedure ExtendRectByBorders(var R: TRect; ABorderWidth: Integer;
  AEdges: TcxBorders);

function CheckParentsNativeHandle(AControl: TWinControl;
  ANativeHandle: THandle): Boolean;
...
function GetContainerBorderColor(AIsHighlight, AIsOffice11Style
  : Boolean): TColor;
function GetContainerBorderWidth(ABorderStyle: TcxContainerBorderStyle)
  : Integer; overload;
function GetContainerBorderWidth(ALookAndFeelKind: TcxLookAndFeelKind)
  : Integer; overload;
...
procedure InternalInvalidate(AHandle: THandle;
  const AOuterRect, AInternalRect: TRect; AEraseBackground: Boolean = False;
  ARedrawNC: Boolean = False);
function IsRelatedWindow(AParentHandle, AChildHandle: THandle)
  : Boolean; overload;
function IsRelatedWindow(AParent: TWinControl; AChildHandle: THandle)
  : Boolean; overload;
...
function SetWindowRegion(AWindowHandle: THandle; ARegionHandle: TcxRegionHandle;
  ARedraw: Boolean = True): Boolean; overload;
procedure SetWindowRegion(AWindowHandle: THandle;
  ARegionHandle: TcxRegionHandle; AOperation: TcxRegionOperation;
  ARedraw: Boolean = True); overload;
procedure SetWindowRegion(AControl: TWinControl; const ABounds: TRect;
  AOperation: TcxRegionOperation = roSet; ARedraw: Boolean = True); overload;
procedure SetWindowRegion(AControl: TWinControl;
  const ABounds: TcxContainerInnerControlBounds); overload;
...
function ExecuteEmbeddedDialog(ADialog: TCommonDialog;
...
function TcxPopupVisibleController.IsVisible
  (AItem: TcxCustomPopupWindow): Boolean;
begin
...
function TcxPopupVisibleController.GetItems(Index: Integer)
  : TcxCustomPopupWindow;
begin
...
class procedure _TcxContainerAccess.KeyUp(AInstance: TcxContainer;
  var Key: Word; Shift: TShiftState);
begin
...
function ButtonTocxButton(Button: TMouseButton): TcxMouseButton;
const
  AButtonMap: array [TMouseButton] of TcxMouseButton = (cxmbLeft, cxmbRight,
    cxmbMiddle);
begin
  ...
  begin
    repeat
      ...
      if Result = 0 then
        Break;
      AWnd := Result;
      if GetForm(AWnd, AParentForm, AFormStyle) and (AFormStyle = fsMDIChild)
      then
        Break;
    until False;
    Result := AWnd;
...
begin
  ...
  begin
    ...
    Result := not GetForm(ARootParent, AForm, AFormStyle) or AForm.Active or
      (AFormStyle = fsMDIForm) or cxPopupVisibleController.IsAnyVisible and
...
begin
  ...
  begin
    ...
    if AParentHandle <> AParentForm.Handle then
      Result := (AParentHandle = ANativeHandle) or
        IsChild(AParentHandle, ANativeHandle);
  end;
end;

function cxGetScrollBarInfo(hwnd: hwnd; idObject: Longint;
  var psbi: TcxScrollBarInfo): Boolean;
begin
...
function cxGetScrollBarInfo(hwnd: hwnd; idObject: Longint;
  var psbi: TcxScrollBarInfo; AScaleFactor: TdxScaleFactor;
  AIsRightToLeftLayout: Boolean): Boolean;
begin
  ...
  begin
    if idObject = Longint(OBJID_HSCROLL) then
      psbi.rcScrollBar := cxRectSetHeight(psbi.rcScrollBar,
        dxGetSystemMetrics(SM_CYHSCROLL, AScaleFactor))
    else if AIsRightToLeftLayout then
      psbi.rcScrollBar := cxRectSetRight(psbi.rcScrollBar,
        psbi.rcScrollBar.Right, dxGetSystemMetrics(SM_CXVSCROLL, AScaleFactor))
    else
      psbi.rcScrollBar := cxRectSetWidth(psbi.rcScrollBar,
        dxGetSystemMetrics(SM_CXVSCROLL, AScaleFactor));
  end;
end;
...
const
  BorderStyle: array [TcxLookAndFeelKind] of TcxContainerBorderStyle = (cbsFlat,
    cbs3D, cbsUltraFlat, cbsOffice11);
begin
...
begin
  ...
  else if (AControl <> nil) and Supports(AControl, IcxContainerInnerControl,
    AIContainerInnerControl) then
...
begin
  Result := (AControl <> nil) and AControl.HandleAllocated and
    ((AControl.Handle = AHandle) or ACheckChildren and IsRelatedWindow(AControl,
    AHandle));
end;
...
procedure InternalFillRect(ACanvas: TcxCanvas;
  const AOuterRect, AInternalRect: TRect; AColor: TColor);
begin
  ...
  begin
    ...
    begin
      FillRect(Rect(AOuterRect.Left, AOuterRect.Top, AInternalRect.Left,
        AOuterRect.Bottom));
      FillRect(Rect(AInternalRect.Left, AOuterRect.Top, AInternalRect.Right,
        AInternalRect.Top));
      FillRect(Rect(AInternalRect.Right, AOuterRect.Top, AOuterRect.Right,
        AOuterRect.Bottom));
...
begin
  ...
  begin
    InternalInvalidateRect(Rect(AOuterRect.Left, AOuterRect.Top,
      AInternalRect.Left, AOuterRect.Bottom));
    InternalInvalidateRect(Rect(AInternalRect.Left, AOuterRect.Top,
      AInternalRect.Right, AInternalRect.Top));
    InternalInvalidateRect(Rect(AInternalRect.Right, AOuterRect.Top,
      AOuterRect.Right, AOuterRect.Bottom));
    InternalInvalidateRect(Rect(AInternalRect.Left, AInternalRect.Bottom,
      AInternalRect.Right, AOuterRect.Bottom));
  end;
end;
  ...
  begin
    ...
    if AParent is TcxCustomPopupWindow then
      ...
      begin
        ...
        if (APopupWindow = AParent) or
          (GetParentForm(APopupWindow.OwnerControl) <> AParent) then
          Continue;
...
begin
  Result := False;
  if (AParent = nil) or (AChildHandle = 0) or
...
function IsFormActive(AForm: TCustomForm): Boolean;
  ...
  begin
    Result := IsMDIChild(AForm) and IsWindowActive(Application.MainForm.Handle)
      and (Application.MainForm.ActiveMDIChild = AForm);
  end;

  function IsParentActive(AForm: TCustomForm): Boolean;
  begin
    Result := not IsMDIChild(AForm) and
      IsWindowActive(FindFirstNonChildParentWindow(AForm.Handle));
  end;

begin
  Result := (AForm <> nil) and AForm.HandleAllocated and
    (IsWindowActive(AForm.Handle) or IsMDIChildActive(AForm) or
    IsParentActive(AForm));
end;
...
begin
  Result := dxMessagesController.KillMessages(AWnd, AMsgFilterMin,
    AMsgFilterMax, AKillAllMessages);
end;
...
function SetWindowRegion(AWindowHandle: THandle; ARegionHandle: TcxRegionHandle;
  ARedraw: Boolean): Boolean;

  function IsWindowRegionChanged(AWindowHandle: THandle;
    ARegionHandle: TcxRegionHandle): Boolean;
  var
    ACurrentRegion: HRGN;
  begin
    ACurrentRegion := GetWindowRegion(AWindowHandle);
    Result := (ARegionHandle = 0) and (ACurrentRegion <> 0) or
      (ACurrentRegion = 0) and (ARegionHandle <> 0) or (ARegionHandle <> 0) and
      (ACurrentRegion <> 0) and not EqualRgn(ARegionHandle, ACurrentRegion);
...
procedure SetWindowRegion(AControl: TWinControl; const ABounds: TRect;
  AOperation: TcxRegionOperation; ARedraw: Boolean);
begin
  SetWindowRegion(AControl.Handle, CreateRectRgnIndirect(ABounds),
    AOperation, ARedraw);
end;
...
begin
  SetWindowRegion(AWindowHandle, GetWindowShadowRegion(AWindowHandle,
    AShadowBounds, AShadowBoundsExtent, ANativeStyle, AShadow,
    AExcludeRect), True);
end;
...
begin
  ...
  begin
    Result.TopLeft := AContainer.ClientToScreen
      (Point(AContainer.VScrollBar.Left, AContainer.HScrollBar.Top));
  ...
  begin
    ...
    try
      ABrush := CreateSolidBrush
        (ColorToRGB(AContainer.LookAndFeel.Painter.DefaultSizeGripAreaColor));
...
begin
  Painter.DrawContainerBorder(ACanvas, R, BorderStyle, BorderWidth,
    BorderColor, Edges);
end;

function TcxContainerViewInfo.GetUpdateRegion(AViewInfo: TcxContainerViewInfo)
  : TcxRegion;
begin
...
begin
  ...
  begin
    if not(AContainer.ParentColor or IsStyleValueAssigned(AStyle, csvColor))
    then
      Result := Painter.DefaultEditorBackgroundColorEx
        (AContainer.GetEditStateColorKind);
  end;

  if Result = clDefault then
  begin
    if NativeStyle and not(AEnabled or AStyle.IsValueAssigned(csvColor)) then
      Result := cxGetThemeColor(ThemedObjectType, NativePart, NativeState,
        TMT_FILLCOLOR);
  end;
...
function TcxContainerViewInfo.CalculateBorderColor(AStyle: TcxContainerStyle;
  AEnabled, AIsDesigning: Boolean; AContainer: TcxContainer = nil): TColor;
var
...
begin
  ...
  begin
    if AStyle.BorderStyle in [cbsUltraFlat, cbsOffice11] then
      Result := GetContainerBorderColor(AIsHighlightBorder,
        AStyle.BorderStyle = cbsOffice11)
    else
      Result := AStyle.BorderColor;
  end;
end;

function TcxContainerViewInfo.CalculateTextColor(AStyle: TcxContainerStyle;
  AEnabled: Boolean; AContainer: TcxContainer): TColor;
begin
  if UseSkinnedColors and (AContainer <> nil) and
    not IsStyleValueAssigned(AStyle, csvTextColor) then
  begin
    Result := AContainer.GetDefaultSkinnedTextColor(AEnabled);
    if Result = clDefault then
      Result := Painter.DefaultEditorTextColorEx
        (AContainer.GetEditStateColorKind);
  end
  else
...
function TcxContainerViewInfo.IsStyleValueAssigned(AStyle: TcxContainerStyle;
  AValue: TcxContainerStyleValue): Boolean;
begin
  Result := AStyle.IsValueAssigned(AValue) or (AStyle.ParentStyle <> nil) and
    AStyle.ParentStyle.IsValueAssigned(AValue);
end;
...
procedure TcxContainerViewInfo.InternalPaint(ACanvas: TcxCanvas);
  ...
  begin
    ...
    if bRight in Edges then
      Inc(R.Right, BorderWidth);
    if bBottom in Edges then
  ...
  begin
    ...
    begin
      GetThemeBackgroundContentRect(OpenTheme(ThemedObjectType), ACanvas.Handle,
        EP_EDITTEXT, NativeState, Bounds, R);
      ACanvas.Brush.Color := BackgroundColor;
      ACanvas.FillRect(R);
      ...
      cxGetThemeColor(AThemedObjectType, APart, AState,
        TMT_BORDERCOLOR, AColor);
...
function TcxContainerStyles.GetStyle(AState: TcxContainerStateItem)
  : TcxContainerStyle;
begin
...
procedure TcxContainerStyles.SetStyle(AState: TcxContainerStateItem;
  Value: TcxContainerStyle);
begin
...
function TcxStyleController.GetInternalStyle(AState: TcxContainerStateItem)
  : TcxContainerStyle;
begin
  Result := FStyles[AState];
end;
...
begin
  ...
  if DirectAccessMode then
    FAssignedValues := [csvColor, csvFont, csvTextColor, csvTextStyle];
  // TODO ???
  CreateFont;
...
function TcxContainerStyle.GetStyleValueName(AStyleValue
  : TcxContainerStyleValue; out StyleValueName: string): Boolean;
begin
...
function TcxContainerStyle.IsExtendedStylePropertyPublished(const APropertyName
  : string): Boolean;
var
...
function TcxContainerStyle.IsValueAssigned
  (AValue: TcxContainerStyleValue): Boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      else if not DirectAccessMode then
        RestoreFont(StyleData.Font);
      if (Container <> nil) and Container.DefaultParentColor and
        ((ActiveStyleController = nil) or
        not ActiveStyleController.Style.IsValueAssigned(csvColor)) then
...
procedure TcxContainerStyle.DefineProperties(Filer: TFiler);
  ...
  begin
    Result := IsFontStored and
      ((Filer.Ancestor = nil) or (TcxContainerStyle(Filer.Ancestor).Font.Handle
      <> Font.Handle));
  end;
...
begin
  ...
  Filer.DefineProperty('IsFontAssigned', ReadIsFontAssigned,
    WriteIsFontAssigned, HasFontData);
end;
...
begin
  ...
  else if FStyleController = GetDefaultStyleController then
    Result := nil
  else
...
procedure TcxContainerStyle.ControllerChangedNotification(AStyleController
  : TcxStyleController);
begin
  UpdateFont;
  Changed;
...
procedure TcxContainerStyle.ControllerFreeNotification(AStyleController
  : TcxStyleController);
begin
...
const
  ...
  ABorderStyles: array [TcxContainerStateItem, TcxContainerBorderStyle]
    of TcxContainerBorderStyle = ((cbsNone, cbsSingle, cbsThick, cbsFlat, cbs3D,
    cbsUltraFlat, cbsOffice11), (cbsFlat, cbsThick, cbsThick, cbs3D, cbs3D,
    cbsUltraFlat, cbsOffice11), (cbsNone, cbsSingle, cbsThick, cbsFlat, cbs3D,
    cbsUltraFlat, cbsOffice11), (cbsFlat, cbsThick, cbsThick, cbs3D, cbs3D,
...
begin
  ...
  else if (State = csDisabled) and not AIsDefaultParentColor then
    Result := clBtnFace
  else
...
begin
  ...
  begin
    ...
    if (AContainer <> nil) and AContainer.ParentColor and
...
begin
  ...
  else if not InternalGetTextColor(Result) then
...
begin
  if DirectAccessMode then
    Result := StyleData.Font.Style
  else if not InternalGetTextStyle(Result) then
...
function TcxContainerStyle.InternalGetNotPublishedExtendedStyleValues
...
begin
  ...
  else if FFontAssignedValueLockCount > 0 then
  ...
  try
    ...
    begin
      ...
      if ActiveStyleController <> nil then
        StyleData.Font.Height := dxGetScaleFactor(Owner)
          .Apply(AFont.Height, dxGetScaleFactor(ActiveStyleController));
    end
    else if (Container = nil) or not Container.ParentFont then
      RestoreFont(StyleData.Font);
  finally
...
begin
  if DirectAccessMode then
    Result := clDefault
  else if not InternalGetBorderColor(Result) then
...
begin
  ...
  else if not InternalGetBorderStyle(Result) then
    Result := DefaultBorderStyle;
end;
...
begin
  ...
  else if not InternalGetEdges(Result) then
    Result := DefaultEdges;
end;
...
begin
  ...
  else if not InternalGetHotTrack(Result) then
    Result := DefaultHotTrack;
end;
...
begin
  ...
  else if not InternalGetShadow(Result) then
...
begin
  ...
  else if not InternalGetTransparentBorder(Result) then
    Result := DefaultTransparentBorder;
end;

function TcxContainerStyle.InternalGetBorderColor(var BorderColor
  : TColor): Boolean;
begin
  ...
  if Result then
    ...
    Result := ActiveStyleController.Styles[State].InternalGetBorderColor
      (BorderColor);
end;

function TcxContainerStyle.InternalGetBorderStyle(var BorderStyle
  : TcxContainerBorderStyle): Boolean;
begin
  ...
  if Result then
    ...
    Result := ActiveStyleController.Styles[State].InternalGetBorderStyle
      (BorderStyle);
end;
...
begin
  ...
  if Result then
    ...
    Result := ActiveStyleController.Styles[State].InternalGetTextColor
      (TextColor);
end;

function TcxContainerStyle.InternalGetTextStyle(var TextStyle
  : TFontStyles): Boolean;
begin
  ...
  if Result then
    ...
    Result := ActiveStyleController.Styles[State].InternalGetTextStyle
      (TextStyle);
end;

function TcxContainerStyle.InternalGetTransparentBorder(var TransparentBorder
  : Boolean): Boolean;
begin
  ...
  if Result then
    ...
    Result := ActiveStyleController.Styles[State].InternalGetTransparentBorder
      (TransparentBorder);
end;
...
begin
  Result := (csvBorderColor in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('BorderColor',
    State <> csNormal));
end;

function TcxContainerStyle.IsBorderStyleStored: Boolean;
begin
  Result := (csvBorderStyle in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('BorderStyle',
    State <> csNormal));
end;

function TcxContainerStyle.IsColorStored: Boolean;
begin
  Result := (csvColor in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('Color',
    State <> csNormal));
end;

function TcxContainerStyle.IsEdgesStored: Boolean;
begin
  Result := (csvEdges in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('Edges',
    State <> csNormal));
end;

function TcxContainerStyle.IsFontStored: Boolean;
begin
  Result := (csvFont in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('Font',
    State <> csNormal));
end;

function TcxContainerStyle.IsHotTrackStored: Boolean;
begin
  Result := (csvHotTrack in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('HotTrack',
    State <> csNormal));
end;

function TcxContainerStyle.IsShadowStored: Boolean;
begin
  Result := (csvShadow in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('Shadow',
    State <> csNormal));
end;
...
begin
  Result := (csvTextColor in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('TextColor',
    State <> csNormal));
end;

function TcxContainerStyle.IsTextStyleStored: Boolean;
begin
  Result := (csvTextStyle in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished('TextStyle',
    State <> csNormal));
end;

function TcxContainerStyle.IsTransparentBorderStored: Boolean;
begin
  Result := (csvTransparentBorder in FAssignedValues) and
    ((Container = nil) or Container.IsStylePropertyPublished
    ('TransparentBorder', State <> csNormal));
end;
...
begin
  ...
  begin
    if (csvTransparentBorder in FAssignedValues) and (Value = FTransparentBorder)
    then
      Exit;
  ...
  try
    ...
    begin
      if Style.TransparentBorder then
        InflateRect(Result, -cxContainerMaxBorderWidth,
...
function TcxContainer.InnerControlDefaultHandler(var Message: TMessage)
  : Boolean;
begin
...
begin
  ...
  else if FLockAlignControlsCount > 0 then
...
begin
  ...
  else if FPopupMenuLockCount > 0 then
...
procedure TcxContainer.SetScrollBarsParameters(AIsScrolling: Boolean = False);
  ...
  begin
    ...
    else if not AScrollBar.Data.Visible then
    begin
    ...
    if not UseSystemRightToLeftLayoutForInnerControl and
      (AScrollBar.Kind = sbHorizontal) then
      AScrollBar.Control.BiDiMode := bdLeftToRight;
    if IsPopupScrollBars then
    begin
      AScrollBarVisible := AScrollBar.Data.Visible;
      SetScrollBarInfo(AScrollBar.Kind, AScrollInfo.nMin, AScrollInfo.nMax,
        AScrollBar.SmallChange, AScrollInfo.nPage, AScrollInfo.nPos,
        True, True);
      AScrollBar.Data.Visible := AScrollBarVisible;
      AScrollBar.ApplyData;
    end
    else
      AScrollBar.SetScrollParams(AScrollInfo.nMin, AScrollInfo.nMax,
        AScrollInfo.nPos, AScrollInfo.nPage, True);
  end;
  ...
  begin
    ASizeGripData.Visible := HScrollBar.Visible and VScrollBar.Visible and
      HasInnerControlHandle and Supports(InnerControl, IcxContainerInnerControl,
      AIContainerInnerControl);
    if ASizeGripData.Visible then
      ASizeGripData.Bounds := GetSizeGripRect(AIContainerInnerControl);
  end;

  function NeedsRepaintSizeGrip(const APrevSizeGripData, ASizeGripData
    : TcxContainerSizeGripData): Boolean;
  begin
...
begin
{$IFDEF USETCXSCROLLBAR}
  if not HasInnerControl or not NeedsScrollBars or IsDestroying or not UsecxScrollBars
  then
    Exit;
...
var
  ...
  AShift: TShiftState;
begin
  if CanRefreshContainer and HandleAllocated and
    IsChildEx(Handle, WindowFromPoint(GetMouseCursorPos)) then
...
begin
  if IsContainerClass then
  ...
  else if not IsAlignControlsLocked and not IsInternalControl(AControl) and
    (NeedsScrollBars or (AControl <> nil)) then
    if AControl = nil then
      inherited AlignControls(AControl, Rect)
    else if not FIsCreating and (AControl is TWinControl) and
    ...
    begin
      ...
      if IsInnerControlBoundsChanged(TWinControl(AControl), AInnerControlBounds)
      then
      begin
...
begin
  Result := inherited CanFocusOnClick and
    not(HasInnerControlHandle and FInnerControl.Focused or
...
function TcxContainer.GetContainerState(const P: TPoint;
...
begin
  Result := inherited IsGestureTarget(AWnd) or HandleInnerControlGestures and
    (InnerControl.Handle = AWnd);
end;
...
begin
  ...
  case Key of
    VK_TAB:
      if Focused and (Shift * [ssAlt, ssCtrl] = []) and CanContainerHandleTabs
      then
      begin
        Key := 0;
        AParentForm := GetParentForm(Self);
...
procedure TcxContainer.Notification(AComponent: TComponent;
...
begin
  if CanDisableAlignOnCreateInnerControl and TcxControlHelper.CanSetParent(Self,
    AParent) then
  begin
    DisableAlign;
...
procedure TcxContainer.CalculateViewInfo(const P: TPoint;
  AMouseTracking: Boolean);
...
begin
  ...
  if not ViewInfo.NativeStyle and (GetContainerBorderWidth(ViewInfo.BorderStyle)
  ...
  ViewInfo.BorderWidth := ViewInfo.Painter.GetContainerBorderWidth
    (ViewInfo.BorderStyle);
  ViewInfo.Edges := AStyle.Edges;
  ViewInfo.Shadow := HasShadow;
  ViewInfo.BorderColor := ViewInfo.CalculateBorderColor(AStyle, Enabled,
    IsDesigning, Self);
  ViewInfo.BackgroundColor := ViewInfo.CalculateBackgroundColor(AStyle,
    Enabled, Self);
end;
...
begin
  Result := not(ViewInfo.NativeStyle and (ViewInfo.BorderStyle <> cbsNone)) and
    ActiveStyle.TransparentBorder;
end;

function TcxContainer.CanRefreshContainer: Boolean;
begin
  Result := (FRefreshLockCount = 0) and not FIsCreating and not IsDestroying and
    not IsLoading and not(csDestroyingHandle in ControlState) and
    HandleAllocated;
end;
...
begin
  Result := GetWindowShadowRegion(Handle, GetShadowBounds,
    GetShadowBoundsExtent, ViewInfo.NativeStyle and
    (ViewInfo.BorderStyle <> cbsNone), ViewInfo.Shadow, cxEmptyRect);
...
function TcxContainer.DoInnerControlDefaultHandler
...
{$IFNDEF VCLGLASSPAINT}
  case Message.Msg of
    CN_CTLCOLOREDIT:
      if OnGlass and IsCompositionEnabled and not FInnerControlBufferedPaint
      then
      begin
...
begin
  ...
  if not HasInnerControl then
    ...
    InternalInvalidate(Handle, GetControlRect(Self),
      ViewInfo.BorderRect, False);
...
begin
  ...
  else if not ActiveStyle.HasBorder or ANativeStyle and
    (Style.BorderStyle = cbsNone) then
    Result := cxEmptyRect
...
begin
  Result := Style.IsValueAssigned(AValue) or
    ActiveStyle.IsValueAssigned(AValue);
end;
...
begin
  ...
  ABorderWidth := ViewInfo.Painter.GetContainerBorderWidth
    (ViewInfo.GetContainerBorderStyle);
...
function TcxContainer.InternalGetNotPublishedStyleValues
  : TcxContainerStyleValues;
begin
  Result := [];
end;
...
begin
  if HandleAllocated and (Width > 0) and (Height > 0) and
    not FInternalWindowRegionSetting and not IsInplace then
  begin
...
procedure TcxContainer.DoSetSize;
  ...
  begin
    Result := ViewInfo.FCalculated and HasInnerControl and
      not EqualRect(InnerControl.BoundsRect, R);
  end;
  ...
  begin
    ...
    begin
      ...
      else if InnerControl is TcxCustomInnerListBox then
        if UseRightToLeftScrollBar then
...
begin
  AClipRgn := GetWindowShadowRegion(Handle, GetShadowBounds,
    GetShadowBoundsExtent, not CanHaveTransparentBorder, ViewInfo.Shadow,
    cxEmptyRect);
  if AClipRgn <> 0 then
...
var
  ...
  R: TRect;
begin
  AHScrollbarVisible := IsScrollBarVisible(sbHorizontal, AScrollBarinfo) and
    GetScrollBarEnabled(HScrollBar, AScrollBarinfo);
  AVScrollbarVisible := IsScrollBarVisible(sbVertical, AScrollBarinfo) and
    GetScrollBarEnabled(VScrollBar, AScrollBarinfo);
...
begin
  ...
  if (Result.Left < 0) or (Result.Right > Width) or (Result.Top < 0) or
    (Result.Bottom > Height) then
    Result := cxEmptyRect;
end;
...
begin
  Result := Enabled and (AScrollBarinfo.rgstate[0] and
...
const
  AScrollBarObjects: array [TScrollBarKind] of Longword = (OBJID_HSCROLL,
    OBJID_VSCROLL);
begin
  ...
  if Result then
    Result := cxGetScrollBarInfo(FInnerControl.Handle,
      Integer(AScrollBarObjects[AKind]), AScrollBarinfo, ScaleFactor,
...
function TcxContainer.GetInternalStyle(AState: TcxContainerStateItem)
  : TcxContainerStyle;
begin
  Result := FStyles[AState];
end;
...
procedure TcxContainer.GetStandardScrollBarParameters(AKind: TScrollBarKind;
  out AScrollInfo: TScrollInfo);
const
...
function TcxContainer.IsScrollBarVisible(AKind: TScrollBarKind;
  out AScrollBarinfo: TScrollBarInfo): Boolean;
begin
  Result := GetScrollBarInfo(AScrollBarinfo, AKind) and
    (AScrollBarinfo.rgstate[0] and (STATE_SYSTEM_INVISIBLE or
    STATE_SYSTEM_OFFSCREEN) = 0) and (AScrollBarinfo.rcScrollBar.Width > 0) and
...
begin
  if HasInnerControl then
  begin
    if not InnerControl.HandleAllocated or
      (Message.FocusedWnd <> InnerControl.Handle) then
    begin
      inherited;
      Exit;
    end;
...
begin
  with Message do
    if HasInnerControl and (CursorWnd = Handle) and
      (Smallint(HitTest) = HTCLIENT) and not PtInRect(ViewInfo.ClientRect,
      GetMouseCursorClientPos) then
    begin
...
begin
  ...
  begin
    ...
    if (Align = alRight) and (ASize.cx <> Message.WindowPos.cx) then
      Message.WindowPos.X := Message.WindowPos.X + ASize.cx -
        Message.WindowPos.cx;
    if (Align = alBottom) and (ASize.cy <> Message.WindowPos.cy) then
      Message.WindowPos.Y := Message.WindowPos.Y + ASize.cy -
  ...
  begin
    ...
    begin
      if Screen.ActiveControl <> FPrevActiveForm.ActiveControl then
        if (FPrevActiveControl <> nil) and FPrevActiveControl.HandleAllocated
        then
...
begin
  ...
  begin
    ...
    if (AParentForm <> nil) and AParentForm.HandleAllocated and
      ((TCustomFormAccess(AParentForm).FormStyle <> fsMDIForm) and
      not AParentForm.Active) then
  ...
  begin
    ...
    begin
      ...
      if HasHandle(APopupWindow, AActiveWnd) then
        Exit;
      if IsWindowEnabled(APopupWindow.Handle) and not APopupWindow.IsDeactivateLocked
      then
        APopupWindow.CloseUp;
    end;
  end;
...
begin
  ...
  begin
    ...
    if (AParentForm <> nil) and HasNativeHandle(AParentForm, GetFocus) and
      OwnerControl.CanFocus and not(csDesigning in AParentForm.ComponentState)
    then
...
begin
  ...
  begin
    ...
    begin
      ...
      if FIsTopMost and ((AParentForm = nil) or
...
procedure TcxCustomPopupWindow.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ...
      ATargetBounds := nil;
    ScaleForPPI(AScaleFactor.Apply(dxDefaultDPI), ATargetBounds);
  end;
end;
...
procedure TcxCustomPopupWindow.Popup(AFocusedControl: TWinControl);
  ...
  begin
    ...
    if not Result and not CaptureFocus then
      ...
      begin
        if (Screen.Forms[I].FormStyle = fsStayOnTop) and not Screen.Forms[I].Visible
        then
          Exit(True);
      end;
  end;
  ...
  begin
    ...
    try
      ...
      if FShowWithoutActivation then
        ...
        Show;
      // SetBounds(P.X, P.Y, ASize.cx, ASize.cy);  //#Ch is extra here
      if not FShowWithoutActivation then
      begin
        if CaptureFocus then
...
function TcxCustomPopupWindow.GetFirstFocusControl(AControl: TWinControl)
  : TWinControl;
begin
  if AControl = nil then
    Result := Self
  else if AControl.CanFocus and AControl.TabStop then
    Result := AControl
  else
  begin
...
begin
  Result := OwnerControl.HandleAllocated and
    IsWindowVisible(OwnerControl.Handle);
end;
...
begin
  if (RootContainer = nil) or not RootContainer.InnerControlDefaultHandler
    (TMessage(Message)) then
...
begin
  ...
  begin
    ...
    while ByteType(FAutoCompleteFilter, AIndex) = mbTrailByte do
      Dec(AIndex);
    Delete(FAutoCompleteFilter, AIndex, Length(FAutoCompleteFilter) -
      AIndex + 1);
  end
  else if dxCharInSet(Key, LeadBytes) then
  begin
...
function TcxCustomInnerListBox.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
...
begin
  if AKind = sbHorizontal then
    CallWindowProc(DefWndProc, Handle, WM_HSCROLL,
      Word(AScrollCode) + Word(AScrollPos) shl 16, 0)
  else if AScrollCode in [scLineUp, scLineDown] then
    TopIndex := AScrollPos
  else if AScrollCode in [scPosition, scTrack] then
    TopIndex := AScrollPos
  else
    CallWindowProc(DefWndProc, Handle, WM_VSCROLL,
      Word(AScrollCode) + Word(AScrollPos) shl 16, 0);
end;
...
procedure TcxCustomInnerListBox.Scroll(AScrollBarKind: TScrollBarKind;
...
begin
  if (RootContainer <> nil) and RootContainer.InnerControlMenuHandler(Message)
  then
    Exit;
  ...
  inherited WndProc(Message);
  case Message.Msg of
    CM_RECREATEWND, CM_WININICHANGE, LB_ADDSTRING, LB_DELETESTRING,
      LB_INSERTSTRING, LB_RESETCONTENT, LB_SETCARETINDEX, LB_SETCURSEL,
      LB_SETHORIZONTALEXTENT, LB_SETTOPINDEX, WM_MOUSEWHEEL, WM_NCCALCSIZE,
      WM_WINDOWPOSCHANGED:
...
begin
  inherited;
  if (RootContainer <> nil) and not RootContainer.IsDestroying and
    not(csDestroying in ComponentState) and
    (Message.FocusedWnd <> RootContainer.Handle) then
    RootContainer.FocusChanged;
end;
...
procedure TcxCustomInnerListBox.DoGesture(const EventInfo: TGestureEventInfo;
  var Handled: Boolean);
begin
  if EventInfo.GestureID = igiPan then
    if gfBegin in EventInfo.flags then
      Container.ShowTouchScrollUI(Container)
    else if gfEnd in EventInfo.flags then
      Container.HideTouchScrollUI(Container);
  Handled := False;
end;

procedure TcxCustomInnerListBox.DoGetGestureOptions(var Gestures
...
procedure CloseUnrelatedPopupsOnMouseClick(AWnd: hwnd; const APoint: TPoint;
  ...
  function MDIParentOrAnotherMDIChild(APopupWindow: TcxCustomPopupWindow;
    AWnd: hwnd): Boolean;
  var
  ...
  begin
    ...
    begin
      ...
      if (TCustomFormAccess(AMDIParentForm).FormStyle = fsMDIForm) and
        (TCustomFormAccess(AMDIParentForm).ClientHandle <> 0) then
      begin
  ...
  begin
    Result := APopupWindow.NeedIgnoreMouseMessageAfterCloseUp(AWnd,
      AMouseMessage, KeyboardStateToShiftState, APoint);
  end;
  ...
  begin
    ...
    begin
      ...
      if CheckParentsNativeHandle(APopupWindow.OwnerControl, AWnd) or
        ((AParentForm is TcxCustomPopupWindow) and
        not TcxCustomPopupWindow(AParentForm).IsVisible) then
      begin
        if HasNativeHandle(APopupWindow.OwnerControl, AWnd, True) then
          if (AMouseMessage = WM_LBUTTONDOWN) or
            (AMouseMessage = WM_LBUTTONDBLCLK) then
            if PtInRect(APopupWindow.OwnerScreenBounds, APoint) then
            begin
              ACallNextHook := False;
              if InternalNeedIgnoreMouseMessageAfterCloseUp(AWnd, APopupWindow)
              then
        ...
        try
          if not AHandled and ANeedCheckIgnoreMouseMessage then
            AHandled := InternalNeedIgnoreMouseMessageAfterCloseUp(AWnd,
              APopupWindow);
          APopupWindow.CloseUp;
          if not ACallNextHook and
            ((csDestroying in APopupWindow.OwnerControl.ComponentState) or
            not APopupWindow.OwnerControl.Visible) then
            AHandled := True;
        finally
      ...
      begin
        ...
        if (AParentForm <> nil) and
          (TCustomFormAccess(AParentForm).FormStyle = fsMDIChild) and
          MDIParentOrAnotherMDIChild(APopupWindow, AWnd) then
        begin
          AHandled := AHandled or InternalNeedIgnoreMouseMessageAfterCloseUp
            (AWnd, APopupWindow);
          APopupWindow.CloseUp;
          I := 0;
        end
...
procedure cxContainerWndProcHook(ACode: Integer; WPARAM: WPARAM; LPARAM: LPARAM;
  ...
  begin
    ...
    Result := (AForm1 <> AForm2) and (AForm1 <> nil) and (AForm2 <> nil) and
      ...
      (IsChildEx(AForm1.Handle, AForm2.Handle) and Supports(AForm2,
      IdxPlaceForm) or IsChildEx(AForm2.Handle, AForm1.Handle) and
      Supports(AForm1, IdxPlaceForm));
...
begin
  ...
  begin
    ...
    case message of
      WM_SETFOCUS:
        if not dxMessagesController.IsMessageLocked(CM_ENTER) and
          IsParentFormPlaced(WPARAM, hwnd) then
          dxPerformMessageByQueue(dxFindParentControl(hwnd), CM_ENTER);
      WM_KILLFOCUS:
        if not dxMessagesController.IsMessageLocked(CM_EXIT) and
          IsParentFormPlaced(WPARAM, hwnd) then
    ...
    case message of
      ...
      WM_ACTIVATEAPP:
        ...
        begin
          ...
          begin
            ...
            if APopupWindow.CaptureFocus and APopupWindow.Active or
              not IsWindowEnabled(APopupWindow.Handle) then
            begin
              Inc(I);
              Continue;
            end;
        ...
        begin
          ...
          begin
            ...
            if not APopupWindow.IsVisible and
              not(csDestroying in APopupWindow.ComponentState) and APopupWindow.FTerminateOnDestroy
            then
      ...
      WM_CLOSE:
        ...
        begin
          ...
          if HasNativeHandle(APopupWindow, hwnd) then
          begin
            if APopupWindow.CaptureFocus and
              not(csDestroying in APopupWindow.OwnerControl.ComponentState) then
              AParentWindow := FindFirstNonChildParentWindow
                (APopupWindow.OwnerControl.Handle)
            else
              AParentWindow := 0;
          ...
          if not APopupWindow.CaptureFocus and
            not(csDestroying in APopupWindow.OwnerControl.ComponentState) then
          begin
            AParentForm := GetParentForm(APopupWindow.OwnerControl);
            if not AParentForm.HandleAllocated or
              HasNativeHandle(AParentForm, hwnd) then
              APopupWindow.CloseUp;
            Break;
          end;
        end;
      ...
      WM_WINDOWPOSCHANGED:
        begin
          ...
          begin
            APopupWindow := cxPopupVisibleController[I];
            if (csDestroying in APopupWindow.ComponentState) or
              not APopupWindow.HandleAllocated then
            begin
              Inc(I);
              Continue;
            end;
...
procedure cxContainerGetMessageHook(ACode: Integer; WPARAM: WPARAM;
...
begin
  APMsg := PMSG(LPARAM);
  if cxPopupVisibleController.IsAnyVisible and (ACode = HC_ACTION) and
    (WPARAM = PM_REMOVE) and (APMsg^.Message = WM_SYSKEYDOWN) then
  begin
    APopupWindow := cxPopupVisibleController
      [cxPopupVisibleController.Count - 1];
...
procedure cxContainerMouseHook(ACode: Integer; WPARAM: WPARAM; LPARAM: LPARAM;
  var AHookResult: LRESULT);
var
  AHandled: Boolean;
begin
  ...
  begin
    CloseUnrelatedPopupsOnMouseClick(PMouseHookStruct(LPARAM)^.hwnd,
      PMouseHookStruct(LPARAM)^.pt, WPARAM, AHandled);
    if AHandled then
      AHookResult := 1;
  end;
end;
