...
type
  ...
  public
    ...
    function Add(const AName: string; APersistent: TPersistent)
      : TcxPropertiesStoreNode; overload;
    function Add(const AName, APropertyName: string; APersistent: TPersistent)
      : TcxPropertiesStoreNode; overload;
    function Find(const AName: string; out ANode: TcxPropertiesStoreNode)
      : Boolean; overload;
    function Find(const AName: string; var AIndex: Integer): Boolean; overload;
    function FindPersistent(APersistent: TPersistent;
      out ANode: TcxPropertiesStoreNode;
      ACanBePartOfComponent: Boolean = True): Boolean;
    procedure Clear;
    //
    procedure CheckStoredValue;
    procedure SetCheckValue(AValue: TcxCheckBoxState;
      AWithChildren, AWithParents: Boolean);
    ...
    property Nodes[Index: Integer]: TcxPropertiesStoreNode
  ...
  protected
    ...
    procedure SynchronizeTreeViewNode(ATreeNode: TTreeNode;
  ...
  public
    ...
    property Grouping: TfrmPropertiesStoreGrouping read FGrouping
      write SetGrouping;
    property OwnerComponent: TComponent read FOwnerComponent
      write SetOwnerComponent;
    property PropertiesStore: TcxPropertiesStore read FPropertiesStore
      write FPropertiesStore;
  end;
...
type
  ...
  protected
    ...
    procedure SaveComponent(AStore: TcxPropertiesStore;
      ANode: TcxPropertiesStoreNode);
    procedure SaveComponentsByProperties(AStore: TcxPropertiesStore;
      ANode: TcxPropertiesStoreNode);
    procedure SaveComponentProperties(AStoreComponent
      : TcxPropertiesStoreComponent; ANode: TcxPropertiesStoreNode;
      const AName: string);
  public
    constructor Create(AStructure: TcxPropertiesStoreNode;
      AGroupByComponents: Boolean);
...
constructor TcxPropertiesStoreIO.Create(AStructure: TcxPropertiesStoreNode;
  AGroupByComponents: Boolean);
begin
  inherited Create;
...
procedure TcxPropertiesStoreIO.LoadGroupByProperties
  (AStore: TcxPropertiesStore);
var
  AComponent: TcxPropertiesStoreComponent;
  ANode: TcxPropertiesStoreNode;
...
begin
  ...
  begin
    ...
    begin
      ...
      if GetPersistentNodeByProperty(APropertyName, AComponent.Component, ANode)
      then
      begin
...
procedure TcxPropertiesStoreIO.SaveComponent(AStore: TcxPropertiesStore;
  ANode: TcxPropertiesStoreNode);
var
...
procedure TcxPropertiesStoreIO.SaveComponentsByProperties
  ...
  function GetPropertiesStoreComponent(AComponent: TComponent)
    : TcxPropertiesStoreComponent;
  begin
    if not AStore.Components.FindComponentItemByComponent(AComponent, Result)
    then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AStoreComponent := GetPropertiesStoreComponent
          (TComponent(ASubNode.Persistent));
...
procedure TcxPropertiesStoreIO.SaveComponentProperties(AStoreComponent
  : TcxPropertiesStoreComponent; ANode: TcxPropertiesStoreNode;
  const AName: string);
var
  ASubNode: TcxPropertiesStoreNode;
...
begin
  ...
  begin
    ...
    else if ASubNode.IsStored then
      SaveComponentProperties(AStoreComponent, ASubNode,
        AName + ASubNode.Name + '.');
  end;
end;

function TcxPropertiesStoreIO.GetPropertyNode(AParentNode
  : TcxPropertiesStoreNode; APropertyName: string;
  out ANode: TcxPropertiesStoreNode): Boolean;
var
  ACurrentName: string;
begin
  Result := False;
...
function TcxPropertiesStoreIO.GetPersistentNodeByProperty(var APropertyName
  : string; APersistent: TPersistent;
  out ANode: TcxPropertiesStoreNode): Boolean;
var
  ACurrentName: string;
  APropertyNode: TcxPropertiesStoreNode;
...
procedure TcxPropertiesStoreStructBuilder.AddComponent(AComponent: TComponent;
  AGroupByComponents: Boolean);
begin
      ...
      begin
        ...
        begin
          AddPersistent(TPersistent(AObject),
            dxShortStringToString(APropInfo.Name), AGroupByComponents,
            APersistentNode, APersistent);
          Continue;
        end;
      end;
...
begin
  ...
  try
    ...
    begin
      ...
      AddComponentProperties(APersistent, AParentNode.Add(AName, '',
        APersistentObject), AGroupByComponents, APropList, APropCount);
    end
    else
      for I := 0 to APropCount - 1 do
      begin
        ...
        if APropInfo.PropType^.Kind <> tkMethod then
        begin
          ANode := Structure.Add(dxShortStringToString(APropInfo.Name),
            '', nil);
          if APropInfo.PropType^.Kind = tkClass then
          begin
            AObject := GetObjectProp(APersistent, APropInfo);
            if (AObject is TPersistent) and not(AObject is TComponent) then
            begin
              AddPersistent(TPersistent(AObject), AName, True, ANode,
                APersistent);
              Continue;
            end;
          end;
...
procedure TcxPropertiesStoreStructBuilder.FreePropList(APropList: PPropList;
  APropCount: Integer);
begin
...
function TcxPropertiesStoreNode.Add(const AName: string;
  APersistent: TPersistent): TcxPropertiesStoreNode;
begin
  Result := Add(AName, AName, APersistent);
end;

function TcxPropertiesStoreNode.Add(const AName, APropertyName: string;
  APersistent: TPersistent): TcxPropertiesStoreNode;
var
  AIndex: Integer;
begin
...
function TcxPropertiesStoreNode.Find(const AName: string;
...
begin
  ...
  begin
    Result := (Nodes[I].Persistent = APersistent) and
      (ACanBePartOfComponent or not Nodes[I].IsPartOfComponent);
    if Result then
    begin
      ANode := Nodes[I];
      Break;
...
procedure TcxPropertiesStoreNode.SetCheckValue(AValue: TcxCheckBoxState;
  AWithChildren, AWithParents: Boolean);
var
  I: Integer;
begin
...
procedure TcxPropertiesStoreNode.SetStoredValue(AValue, AWithChildren,
  AWithParents: Boolean);
var
...
constructor TfrmPropertiesStoreEditor.Create(AOwner: TComponent);
  ...
  var
    ACheck: TcxBitmap;
  begin
    ACheck := TcxBitmap.CreateSize(cxImageList1.Width,
      cxImageList1.Height, pf32bit);
    try
      ...
      cxLookAndFeelPaintersManager.GetPainter(lfsNative)
        .DrawScaledCheckButton(ACheck.cxCanvas, cxRectCenter(ACheck.ClientRect,
        ACheck.Width - 3, ACheck.Height - 3), cxbsNormal, ACheckState,
        ScaleFactor);
  ...
  begin
    ...
    try
      ...
      for I := 0 to OwnerComponent.ComponentCount - 1 do
        ABuilder.AddComponent(OwnerComponent.Components[I],
...
procedure TfrmPropertiesStoreEditor.SetFilter(const Value
  : TfrmPropertiesStoreFilter);
begin
...
procedure TfrmPropertiesStoreEditor.SetGrouping(const Value
  : TfrmPropertiesStoreGrouping);
var
...
procedure TfrmPropertiesStoreEditor.TreeExpanding(Sender: TObject;
...
begin
  ...
  try
    ...
    for I := 0 to AStoreNode.NodesCount - 1 do
      SynchronizeTreeViewNode(Tree.Items.AddChild(Node, ''),
        AStoreNode.Nodes[I]);
  finally
    EndUpdate;
  end;
...
const
  AGroupingCaptionMap: array [TfrmPropertiesStoreGrouping] of string =
    (scxFindComponent, scxFindProperty);
begin
...
function TfrmPropertiesStoreEditor.FindNode(const AText: string): TTreeNode;
  ...
  function PartialCompare(const AUpperCaseText: string;
...
procedure TfrmPropertiesStoreEditor.TreeKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
