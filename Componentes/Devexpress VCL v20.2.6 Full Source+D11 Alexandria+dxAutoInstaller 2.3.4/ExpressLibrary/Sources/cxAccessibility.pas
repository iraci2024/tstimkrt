...
unit cxAccessibility; // internal for use
...
type
  ...
  TcxAccessibleObjectProperty = (aopDefaultAction, aopDescription, aopFocus,
    aopLocation, aopShortcut, aopValue, aopSelect);
  ...
  IcxAccessible = interface(IDispatch)
    ...
    function Get_accValue(varChild: OleVariant; out pszValue: WideString)
      : HResult; stdcall;
    function Get_accDescription(varChild: OleVariant;
    ...
    function Get_accKeyboardShortcut(varChild: OleVariant;
      out pszKeyboardShortcut: WideString): HResult; stdcall;
    function Get_accFocus(out pvarChild: OleVariant): HResult; stdcall;
    function Get_accSelection(out pvarChildren: OleVariant): HResult; stdcall;
    function Get_accDefaultAction(varChild: OleVariant;
      out pszDefaultAction: WideString): HResult; stdcall;
    function accSelect(flagsSelect: Integer; varChild: OleVariant)
      : HResult; stdcall;
    function accLocation(out pxLeft: Integer; out pyTop: Integer;
    ...
    function accHitTest(xLeft: Integer; yTop: Integer;
  ...
  private
    ...
    procedure CheckStringToBeReturned(const AStr: WideString;
      out AResult: HResult);
  ...
  protected
    ...
    procedure DoDefaultAction(AChildID
      : TcxAccessibleSimpleChildElementID); virtual;
    procedure DoSelect(AFlags: Integer;
      AChildID: TcxAccessibleSimpleChildElementID); virtual;
    ...
    function GetDefaultActionDescription
      (AChildID: TcxAccessibleSimpleChildElementID): string; virtual;
    function GetDescription(AChildID: TcxAccessibleSimpleChildElementID)
      : string; virtual;
    function GetHitTest(AScreenX, AScreenY: Integer; out AChildIndex: Integer)
      : TcxAccessibleObjectHitTest; virtual;
    procedure GetKeyboardAccessParameters
      (AChildID: TcxAccessibleSimpleChildElementID; out AShortCut: TShortCut;
      out ACaptionWithAccelChars: string); virtual;
    ...
    function GetSupportedProperties(AChildID: TcxAccessibleSimpleChildElementID)
      : TcxAccessibleObjectProperties; virtual;
    function GetValue(AChildID: TcxAccessibleSimpleChildElementID)
      : string; virtual;
    function NavigateToChild(ACurrentChildIndex: Integer;
      ADirection: TcxAccessibilityNavigationDirection): Integer; virtual;
    // andPrev, andNext must prevent looping
    procedure SetValue(AChildID: TcxAccessibleSimpleChildElementID;
  ...
  public
    ...
    property States[AChildID: TcxAccessibleSimpleChildElementID]: Integer
      read GetState;
...
function WMGetObjectResultFromIAccessibilityHelper(const AWMGetObjectMessage
  : TMessage; AIHelper: IcxAccessibilityHelper): LRESULT;

function CanReturnAccessibleObject(const AWMGetObjectMessage: TMessage)
  : Boolean;
...
uses
  Math, Menus, SysUtils, cxControls;
...
stdcall = nil;
FcxLResultFromObject:

function(const riid: TGUID; wParam: wParam; punk: IUnknown): LRESULT;
stdcall = nil;
FOleaccLibrary:
HMODULE;
  ...
  function WMGetObjectResultFromIAccessibilityHelper(const AWMGetObjectMessage
    : TMessage; AIHelper: IcxAccessibilityHelper): LRESULT;
  begin
    Result := FcxLResultFromObject(IID_IcxAccessible,
      AWMGetObjectMessage.wParam, AIHelper as IcxAccessible);
  end;

  function CanReturnAccessibleObject(const AWMGetObjectMessage
    : TMessage): Boolean;
  begin
...
{ TcxAccessibilityHelper }
  ...
  function TcxAccessibilityHelper.GetNextSelectableChildIndex
    (AStartIndex: Integer; AGoForward: Boolean): Integer;
  var
  ...
  begin
    ...
    if AStartIndex < 0 then
      if AGoForward then
        ...
        AStartIndex := ACount
    else if AStartIndex >= ACount then
...
// IcxAccessibilityHelper
  ...
  function TcxAccessibilityHelper.ChildIsSimpleElement(AIndex: Integer)
    : Boolean;
  begin
    raise EdxException.Create('');
  end;
  ...
  procedure TcxAccessibilityHelper.DoSelect(AFlags: Integer;
    AChildID: TcxAccessibleSimpleChildElementID);
  begin
    // do nothing
  end;
  ...
  begin
    ...
    begin
      if GetChild(I).Focused(AIsChildFocused, AFocusedChildIndex) then
      begin
  ...
  function TcxAccessibilityHelper.GetChild(AIndex: Integer)
    : TcxAccessibilityHelper;
  begin
    Result := nil;
  end;
  ...
  function TcxAccessibilityHelper.NavigateToChild(ACurrentChildIndex: Integer;
    ADirection: TcxAccessibilityNavigationDirection): Integer;
  begin
    case ADirection of
      andUp:
...
// IDispatch
  function TcxAccessibilityHelper.GetIDsOfNames(const IID: TGUID;
...
// IOleWindow
  function TcxAccessibilityHelper.ContextSensitiveHelp
...
// IcxAccessible
  ...
  begin
    ...
    if CheckIsOwnerObjectLive(Result) then
      ...
      begin
        if PtInRect(GetScreenBounds(cxAccessibleObjectSelfID),
          Point(xLeft, yTop)) then
        begin
  ...
  function TcxAccessibilityHelper.accNavigate(navDir: Integer;
    ...
    begin
      if varStart <> CHILDID_SELF then
        Result := E_INVALIDARG
      else if not(aopLocation in GetSupportedProperties
        (cxAccessibleObjectSelfID)) then
        Result := DISP_E_MEMBERNOTFOUND
      else
      begin
    ...
    begin
      ...
      if (AParent <> nil) and AParent.CheckIsOwnerObjectLive(Result) and
        (aopLocation in AParent.GetSupportedProperties(cxAccessibleObjectSelfID))
      then
      begin
        TVarData(AStartChildID).VType := VT_I4;
        AStartChildID := AParent.GetChildIndex(Self) + 1;
        if (AParent as IcxAccessible).accNavigate(navDir, AStartChildID,
          pvarEndUpAt) = S_OK then
          Result := S_OK;
      end;
    end;
  ...
  begin
    ...
    else if varStart = CHILDID_SELF then
  ...
  begin
    if not CheckIsOwnerObjectLive(Result) then
      Exit;
    if (TVarData(varChild).VType = VT_EMPTY) or (varChild > GetChildCount) or
      (varChild < 1) then
      Result := E_INVALIDARG
    else if ChildIsSimpleElement(varChild - 1) then
      Result := S_FALSE
    else
    begin
  ...
  function TcxAccessibilityHelper.Get_accChildCount(out pcountChildren
    : Integer): HResult;
  begin
    if CheckIsOwnerObjectLive(Result) then
    begin
  ...
  function TcxAccessibilityHelper.Get_accFocus(out pvarChild
    : OleVariant): HResult;
  var
  ...
  function TcxAccessibilityHelper.Get_accParent(out ppdispParent
    : IDispatch): HResult;
  var
    AParentWnd: HWnd;
  begin
    ...
    begin
      ...
      begin
        ...
        if cxGetAccessibleObjectFromWindow(AParentWnd, OBJID_CLIENT,
  ...
  function TcxAccessibilityHelper.Get_accSelection(out pvarChildren
    : OleVariant): HResult;
  begin
    Result := S_FALSE;
  end;
  ...
  function TcxAccessibilityHelper.CheckIsOwnerObjectLive(out AErrorCode
    : HResult): Boolean;
  begin
  ...
  begin
    if (AVarChild <> CHILDID_SELF) and not ChildIsSimpleElement(AVarChild - 1)
    then
    begin
  ...
  procedure TcxAccessibilityHelper.CheckStringToBeReturned
    (const AStr: WideString; out AResult: HResult);
  begin
  ...
  begin
    ...
    else if (AChildID > GetChildCount) or not ChildIsSimpleElement(AChildID - 1)
    then
    begin
...
begin
  FcxAccessibleObjectFromWindow := GetProcAddress(FOleaccLibrary,
