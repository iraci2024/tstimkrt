...
uses
  Types, Windows, Forms, MultiMon, Classes, Contnrs, Controls, Graphics,
  cxGeometry, dxCore;
...
const
  ...
  dxDefaultDPIValues: array [0 .. 7] of Integer = (96, 120, 144, 168, 192, 216,
    240, 288);
...
type
  TdxProcessDpiAwareness = (pdaUnaware, pdaSystemDpiAware,
    pdaPerMonitorDpiAware, pdaPerMonitorDpiAwareV2);
...
{$ENDREGION}
  ...
  public
    ...
    class procedure Register(ACursorIndex: Integer; AResInstance: THandle;
...
procedure dxAssignFont(ATargetFont, ASourceFont: TFont;
  ATargetScaleFactor, ASourceScaleFactor: TdxScaleFactor);
function dxGetFontHeight(AFontSize: Integer;
...
function dxAdjustWindowRectEx(var R: TRect; Style, ExStyle: Cardinal;
  Menu: Boolean; AScaleFactor: TdxScaleFactor): Boolean;
function dxGetCurrentDPI(AComponent: TComponent): Integer;
function dxGetCurrentScaleFactor(AComponent: TComponent;
  out M, D: Integer): Boolean;
...
function dxGetSystemMetrics(AIndex: Integer;
  AScaleFactor: TdxScaleFactor = nil): Integer;
function dxTryGetScaleFactorForControl(AControl: TControl;
...
function dxGetProcessDpiAwareness(AProcess: THandle): TdxProcessDpiAwareness;
  overload; deprecated;
function dxGetProcessDpiAwareness(AProcess: THandle;
  out AValue: TdxProcessDpiAwareness): Boolean; overload;
function dxSetProcessDpiAwareness(AValue: TdxProcessDpiAwareness): Boolean;

implementation

uses
  Registry, SysUtils, Math, dxCoreGraphics;
...
type
  ...
  TProcessDpiAwareness = (PROCESS_DPI_UNAWARE = 0, PROCESS_SYSTEM_DPI_AWARE = 1,
    PROCESS_PER_MONITOR_DPI_AWARE = 2);

  TDpiAwarenessContext = type THandle;

  TAdjustWindowRectExForDPI = function(var lpRect: TRect; dwStyle: DWORD;
    bMenu: BOOL; dwExStyle: DWORD; DPI: UINT): BOOL; stdcall;
  TGetDpiForMonitorFunc = function(Monitor: HMONITOR; dpiType: Cardinal;
    out dpiX: UINT; out dpiY: UINT): HRESULT; stdcall;
  TGetProcessDpiAwarenessFunc = function(Process: THandle;
    out value: TProcessDpiAwareness): HRESULT; stdcall;
  ...
  TSetProcessDpiAwarenessFunc = function(value: TProcessDpiAwareness)
    : HRESULT; stdcall;
  TSetThreadDpiAwarenessContext = function(value: TDpiAwarenessContext)
    : TDpiAwarenessContext; stdcall;
...
begin
  ...
  ATargetFont.Height := ATargetScaleFactor.Apply(ASourceFont.Height,
    ASourceScaleFactor);
end;
...
begin
  if Assigned(FAdjustWindowRectExForDpi) then
    Result := FAdjustWindowRectExForDpi(R, Style, Menu, ExStyle,
...
{$ELSE}
  if (AMonitor <> nil) and Assigned(FGetDpiForMonitor) and
    (FGetDpiForMonitor(AMonitor.Handle, 0, X, Y) = S_OK) then
    Result := X
{$ENDIF}
  else
    Result := dxGetSystemDPI;
...
begin
  ...
  begin
    ...
    begin
      if AComponent is TControl then
        Exit(dxGetMonitorDPI(TControl(AComponent)
          .ClientToScreen(cxRectCenter(TControl(AComponent).ClientRect))));
...
begin
  ...
  begin
    if dxTryGetScaleFactorForControl(TControlCanvas(ACanvas).Control,
      AScaleFactor) then
      Exit(AScaleFactor);
  end;
...
begin
  ...
  else if Assigned(FGetSystemMetricsForDpi) then
    Result := FGetSystemMetricsForDpi(AIndex, AScaleFactor.Apply(dxDefaultDPI))
  else
    Result := AScaleFactor.Apply(GetSystemMetrics(AIndex),
...
function dxGetProcessDpiAwareness(AProcess: THandle;
  out AValue: TdxProcessDpiAwareness): Boolean;
var
  AProcessDpiAwareness: TProcessDpiAwareness;
begin
  Result := Assigned(FGetProcessDpiAwareness) and
    Succeeded(FGetProcessDpiAwareness(AProcess, AProcessDpiAwareness));
  if Result then
...
function dxIsProcessPerMonitorV2Aware: Boolean;
begin
  Result := IsWin10OrLater and
    (dxGetProcessDpiAwareness = pdaPerMonitorDpiAwareV2);
end;

function dxSetProcessDpiAwareness(AValue: TdxProcessDpiAwareness): Boolean;
const
  ThreadDpiAwarenessContextMap: array [TdxProcessDpiAwareness]
    of TDpiAwarenessContext = (TDpiAwarenessContext(-1),
    TDpiAwarenessContext(-2), TDpiAwarenessContext(-3),
    TDpiAwarenessContext(-4));
begin
  if Assigned(FSetThreadDpiAwarenessContext) then
    Result := FSetThreadDpiAwarenessContext(ThreadDpiAwarenessContextMap[AValue]
      ) <> DPI_AWARENESS_CONTEXT_INVALID
  else if Assigned(FSetProcessDpiAwareness) then
    Result := (AValue <= pdaPerMonitorDpiAware) and
      Succeeded(FSetProcessDpiAwareness(TProcessDpiAwareness(AValue)))
  else
    Result := IsWinVistaOrLater and (AValue <> pdaUnaware) and
      SetProcessDPIAware;
...
begin
  ...
  begin
    @FGetDpiForMonitor := GetProcAddress(FShellCoreLibHandle,
      'GetDpiForMonitor');
    @FGetProcessDpiAwareness := GetProcAddress(FShellCoreLibHandle,
      'GetProcessDpiAwareness');
    @FSetProcessDpiAwareness := GetProcAddress(FShellCoreLibHandle,
  ...
  begin
    @FGetSystemMetricsForDpi := GetProcAddress(AUserLibHandle,
      'GetSystemMetricsForDpi');
    @FGetThreadDpiAwarenessContext := GetProcAddress(AUserLibHandle,
      'GetThreadDpiAwarenessContext');
    @FSetThreadDpiAwarenessContext := GetProcAddress(AUserLibHandle,
      'SetThreadDpiAwarenessContext');
    @FAdjustWindowRectExForDpi := GetProcAddress(AUserLibHandle,
...
class procedure TdxCursorManager.Register(ACursorIndex: Integer;
  AResInstance: THandle; const AResName: PChar);
var
  AInfo: TCursorInfo;
begin
