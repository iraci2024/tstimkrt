...
uses
  SysUtils, Windows, Classes, Types, Math, Generics.Defaults,
  Generics.Collections, dxCore, dxCoreClasses;
...
type
  TcxPtInRectType = (ptrtNone, ptrtArea, ptrtLeft, ptrtTop, ptrtRight,
    ptrtBottom);
  ...
  TdxSizeF = record
    ...
    cy: Single;
  public
    class function Create(const AWidth, AHeight: Single): TdxSizeF;
      static; inline;
  ...
  public
    ...
    procedure Offset(const DX, DY: Single; APositiveFactor: Boolean = True);
      overload; inline;
    procedure Offset(const P: TdxPointF; APositiveFactor: Boolean = True);
      overload; inline;
    procedure Offset(const S: TdxSizeF; APositiveFactor: Boolean = True);
      overload; inline;
  ...
  public
    class function Create(const ALeft, ATop, ARight, ABottom: Single): TdxRectF;
      overload; static;
    class function CreateSize(const ALeft, ATop, AWidth, AHeight: Single)
      : TdxRectF; overload; static;
    procedure Init(const ALeft, ATop, ARight, ABottom: Single);
      overload; inline;
    procedure InitSize(const ALeft, ATop, AWidth, AHeight: Single);
      overload; inline;
  ...
  public
    ...
    class function Create(const AWidth, AHeight: Double): TdxSizeDouble;
      static; inline;
  ...
  public
    ...
    Width: Double;
    Height: Double;
    class function CreateFromLTRB(const ALeft, ATop, ARight, ABottom: Double)
      : TdxRectDouble; overload; static;
    class function Create(const AOrigin: TdxPointDouble): TdxRectDouble;
      overload; static;
    class function Create(const ATopLeft, ABottomRight: TdxPointDouble)
      : TdxRectDouble; overload; static;
    class function Create(const ASize: TdxSizeDouble): TdxRectDouble;
      overload; static;
    class function Create(const AOrigin: TdxPointDouble;
      AWidth, AHeight: Double): TdxRectDouble; overload; static;
    class function Create(const ALeft, ATop, AWidth, AHeight: Double)
      : TdxRectDouble; overload; static;
    class function Create(const ALeft, ATop: Double; const ASize: TdxSizeDouble)
      : TdxRectDouble; overload; static;
    class function Create(const AOrigin: TdxPointDouble;
      const ASize: TdxSizeDouble): TdxRectDouble; overload; static;
    ...
    property BottomRight: TdxPointDouble read GetBottomRight
  ...
  public
    constructor Create(AOwner: TPersistent; ADefaultWidth: Integer = 0;
      ADefaultHeight: Integer = 0); reintroduce;
    procedure ChangeScale(M, D: Integer); override;
    procedure LoadFromStream(AStream: TStream); override;
    procedure SaveToStream(AStream: TStream); override;
  ...
  published
    property Height: Integer index 0 read GetValue write SetValue
      stored IsSizeStored;
    property Width: Integer index 1 read GetValue write SetValue
      stored IsSizeStored;
  end;
  ...
  published
    property All: Integer index 0 read GetValue write SetValue
      stored IsMarginStored;
    ...
    property Right: Integer index 3 read GetValue write SetValue
      stored IsMarginStored;
    property Bottom: Integer index 4 read GetValue write SetValue
      stored IsMarginStored;
  end;
  ...
  public
    constructor Create(AOwner: TPersistent; ADefaultX: Integer = 0;
      ADefaultY: Integer = 0); reintroduce;
    procedure ChangeScale(M, D: Integer); override;
    procedure LoadFromStream(AStream: TStream); override;
    procedure SaveToStream(AStream: TStream); override;
  ...
  published
    property X: Integer index 0 read GetValue write SetValue
      stored IsPointStored;
    property Y: Integer index 1 read GetValue write SetValue
      stored IsPointStored;
  end;
  ...
  protected
    procedure DoAssign(Source: TPersistent); override;
  public
    constructor Create(AOwner: TPersistent; const ADefaultX: Double = 0;
      const ADefaultY: Double = 0); reintroduce;
  ...
  published
    property X: Double index 0 read GetValue write SetValue
      stored IsPointStored;
    property Y: Double index 1 read GetValue write SetValue
      stored IsPointStored;
  end;
  ...
  TdxScaleFactorNotifyEvent = procedure(Sender: TObject; M, D: Integer;
    IsLoading: Boolean) of object;
  ...
  strict private
    ...
    function TransformScaleFactor(var ASourceNumerator, ASourceDenominator
      : Integer): Boolean;
  protected
  ...
  public
    ...
    procedure Assign(ANumerator, ADenominator: Integer;
      AIsLoading: Boolean = False); overload;
    ...
    function Apply(const V: Integer; ASourceScaleFactor: TdxScaleFactor)
      : Integer; overload; inline;
    function Apply(const V: TPoint; ASourceScaleFactor: TdxScaleFactor): TPoint;
      overload; inline;
    function Apply(const V: TRect; ASourceScaleFactor: TdxScaleFactor): TRect;
      overload; inline;
    function Apply(const V: TSize; ASourceScaleFactor: TdxScaleFactor): TSize;
      overload; inline;

    function Apply(const V: Integer; ASourceNumerator, ASourceDenominator
      : Integer): Integer; overload;
    function Apply(const V: TPoint; ASourceNumerator, ASourceDenominator
      : Integer): TPoint; overload;
    function Apply(const V: TRect; ASourceNumerator, ASourceDenominator
      : Integer): TRect; overload;
    function Apply(const V: TSize; ASourceNumerator, ASourceDenominator
      : Integer): TSize; overload;
  ...
  strict private
    ...
    procedure OwnerChangeHandler(Sender: TObject; M, D: Integer;
      IsLoading: Boolean);
  ...
  public
    ...
    class function CreateMatrix(M11, M12, M21, M22, DX, DY: Single)
      : TXForm; static;
    ...
    class function CreateScaleMatrix(AScaleX, AScaleY: Single): TXForm;
      overload; static;
    class function CreateTranslateMatrix(AOffsetX, AOffsetY: Single)
      : TXForm; static;
...
function cxRectCenter(const ABounds: TRect; AWidth, AHeight: Integer): TRect;
  overload; inline;
function cxRectCenter(const ABounds: TdxRectF; AWidth, AHeight: Single)
  : TdxRectF; overload; inline;
function cxRectCenter(const R: TdxRectF): TdxPointF; overload; inline;
function cxRectCenterHorizontally(const R: TdxRectF; const AWidth: Double)
  : TdxRectF; overload; inline;
function cxRectCenterHorizontally(const R: TRect; AWidth: Integer): TRect;
  overload; inline;
function cxRectCenterVertically(const R: TdxRectF; const AHeight: Double)
  : TdxRectF; overload; inline;
...
function cxRectOffset(const R: TRect; const P: TPoint;
  APositiveFactor: Boolean = True): TRect; overload; inline;
function cxRectOffset(const R: TRect; const Ps: array of TPoint;
  APositiveFactor: Boolean = True): TRect; overload;
function cxRectOffset(const R: TRect; DX, DY: Integer;
  APositiveFactor: Boolean = True): TRect; overload; inline;
function cxRectOffset(const R: TdxRectF; const P: TdxPointF;
  APositiveFactor: Boolean = True): TdxRectF; overload; inline;
function cxRectOffset(const R: TdxRectF; DX, DY: Single;
  APositiveFactor: Boolean = True): TdxRectF; overload; inline;
procedure cxRectsOffset(var Rs: array of TdxRectF; const AOffset: TdxPointF;
  APositiveFactor: Boolean = True); overload;
...
procedure cxRectOverlapped(const ASrc, ADst: TRect;
  out ASrcH, ASrcV, ADstH, ADstV: TRect);
...
function cxRectPtInEx(const R: TRect; X, Y, ADelta: Integer): TcxPtInRectType;
  overload; inline;
function cxRectPtInEx(const R: TRect; X, Y, ADeltaX, ADeltaY: Integer)
  : TcxPtInRectType; overload; inline;
function cxRectPtInEx(const R: TRect; const X, Y: Integer;
  DL, DT, DR, DB: Integer): TcxPtInRectType; overload; inline;
...
function cxRectScale(const R: TRect; ANumerator, ADenominator: Integer): TRect;
  overload; inline;
function cxRectScale(const R: TRect; ANumeratorX, ADenominatorX, ANumeratorY,
  ADenominatorY: Integer): TRect; overload; inline;
function cxRectScaleSize(const R: TRect; ANumerator, ADenominator: Integer)
  : TRect; overload; inline;
function cxRectScaleHeight(const R: TRect; Y1, Y2, H1, H2: Integer)
  : TRect; inline;
function cxRectScaleWidth(const R: TRect; X1, X2, W1, W2: Integer)
  : TRect; inline;
...
procedure cxRectSplitHorz(const ABounds: TRect;
  var ARect1, ARect2: TRect); inline;
procedure cxRectSplitVert(const ABounds: TRect;
  var ARect1, ARect2: TRect); inline;
function cxRectSquare(const R: TRect): Integer; inline;
function cxRectTransform(const ARect: TRect;
  ALeftOffset, ATopOffset, ARightOffset, ABottomOffset: Integer): TRect;
  overload; inline;
...
function cxPointInLine(const P, LinePoint1, LinePoint2: TdxPointF)
  : Boolean; overload;
function cxPointInLine(const P, LinePoint1, LinePoint2: TPoint)
  : Boolean; overload;
...
function cxPointOffset(const P, DP: TPoint; APositiveFactor: Boolean = True)
  : TPoint; overload; inline;
function cxPointOffset(const P: TPoint; const Ps: array of TPoint;
  APositiveFactor: Boolean = True): TPoint; overload;
function cxPointOffset(const P: TPoint; X, Y: Integer;
  APositiveFactor: Boolean = True): TPoint; overload; inline;
function cxPointOffset(const P: TPoint; ADelta: Integer): TPoint;
  overload; inline;
function cxPointOffset(const P, AOffset: TdxPointF;
  APositiveFactor: Boolean = True): TdxPointF; overload; inline;
function cxPointOffset(const P: TdxPointF; X, Y: Single;
  APositiveFactor: Boolean = True): TdxPointF; overload; inline;
procedure cxPointsOffset(var Ps: array of TdxPointF;
  const AOffset: TdxPointF); overload;
function cxPointReplaceItem(const P: TPoint; const AIndex, AValue: Integer)
  : TPoint; inline;
function cxPointScale(const P: TPoint; ANumerator, ADenominator: Integer)
  : TPoint; overload; inline;
...
procedure cxPointsOffset(var Ps: array of TPoint;
  const AOffset: TPoint); overload;
...
function dxRingPoint(const ABase: TdxPointF; ARadius, AAngle: Single)
  : TdxPointF; // inline;
function dxRingRelativePoint(const ABase, APoint: TdxPointF): TdxPointF; inline;
function dxRingRotatePoint(const ABase, APoint: TdxPointF; AAngle: Single)
  : TdxPointF; inline;
procedure dxRingRotatePoints(const ABase: TdxPointF;
  var APoints: array of TdxPointF; AAngle: Single);
...
function dxSizeDoubleIsEqual(const ASize1, ASize2: TdxSizeDouble)
  : Boolean; inline;
function dxSizeDoubleScale(const ASize: TdxSizeDouble;
  const AScale: TdxPointDouble): TdxSizeDouble; inline;
...
function dxPointDouble(const APoint: TdxPointF): TdxPointDouble;
  overload; inline;
function dxPointDoubleIsEqual(const APoint1, APoint2: TdxPointDouble)
  : Boolean; inline;
...
function dxPointDoubleOffset(const APoint: TdxPointDouble;
  const AOffset: TdxPointDouble; APositiveFactor: Boolean = True)
  : TdxPointDouble; overload; inline;
function dxPointDoubleScale(const APoint: TdxPointDouble;
  const AScale: TdxPointDouble): TdxPointDouble; overload; inline;
function dxRectDouble(ALeft, ATop, AWidth, AHeight: Double): TdxRectDouble;
  overload; inline;
function dxRectDouble(const ATopLeft, ABottomRight: TdxPointDouble)
  : TdxRectDouble; overload; inline;
function dxRectDouble(const ARect: TRect): TdxRectDouble; overload; inline;
function dxRectDoubleIntersect(const ARect1, ARect2: TdxRectDouble)
  : Boolean; inline;
function dxRectDoubleIsEmpty(const ARect: TdxRectDouble): Boolean; inline;
function dxRectDoubleIsEqual(const ARect1, ARect2: TdxRectDouble)
  : Boolean; inline;
function dxRectDoubleOffset(const ARect: TdxRectDouble;
  const APoint: TdxPointDouble; APositiveFactor: Boolean = True)
  : TdxRectDouble; inline;
...
const
  cxFullAreaRect: TRect = (Left: MinInt; Top: MinInt; Right: MaxInt;
    Bottom: MaxInt);
...
type
  ...
  TdxScaleFactorListenerComparer = class(TInterfacedObject,
  ...
  public
    class property Default: IComparer<TdxScaleFactorNotifyEvent>
      read GetDefault;
  end;
{$ENDIF}
...
begin
  Result := (R.Left = R1.Left) and (R.Top = R1.Top) and (R.Right = R1.Right) and
    (R.Bottom = R1.Bottom);
end;
...
function cxRectOffset(const R: TRect; DX, DY: Integer;
  APositiveFactor: Boolean = True): TRect;
begin
  Result := cxRect(cxPointOffset(R.TopLeft, DX, DY, APositiveFactor),
    cxPointOffset(R.BottomRight, DX, DY, APositiveFactor));
end;

function cxRectOffset(const R: TdxRectF; const P: TdxPointF;
  APositiveFactor: Boolean = True): TdxRectF;
begin
  Result := cxRectOffset(R, P.X, P.Y, APositiveFactor);
end;
...
function GetRectCoordinate(const R: TRect;
  ABottomRight, AVertCoordinate: Boolean): Integer;
begin
  if ABottomRight then
    if AVertCoordinate then
      ...
      Result := R.Right
  else if AVertCoordinate then
    Result := R.Top
  else
    Result := R.Left;
end;

procedure SetRectCoordinate(var R: TRect;
  ABottomRight, AVertCoordinate: Boolean; AValue: Integer);
begin
  if ABottomRight then
    if AVertCoordinate then
      R.Bottom := AValue
...
function cxRectPtInEx(const R: TRect; const X, Y: Integer;
  DL, DT, DR, DB: Integer): TcxPtInRectType;
...
begin
  if cxRectPtIn(R, X, Y) then
    Result := ptrtArea
  else if cxRectPtIn(cxRectInflate(R, DL, DT, DR, DB), X, Y) then
  begin
    with R do
    begin
      if InRange(X, Right, DR) then
        Result := ptrtRight
      else if InRange(Y, Bottom, DB) then
        Result := ptrtBottom
      else if InRange(X, Left, DL) then
        Result := ptrtLeft
      else
        Result := ptrtTop;
    end;
...
begin
  if AUseTrunc then
    Result := cxRect(Trunc(ARect.Left), Trunc(ARect.Top), Trunc(ARect.Right),
      Trunc(ARect.Bottom))
  else
    Result := cxRect(Round(ARect.Left), Round(ARect.Top), Round(ARect.Right),
      Round(ARect.Bottom));
end;
...
function cxRectAdjustF(const R: TdxRectF): TdxRectF;
begin
  Result := dxRectF(Min(R.Left, R.Right), Min(R.Top, R.Bottom),
    Max(R.Left, R.Right), Max(R.Top, R.Bottom));
end;
...
function cxRectScaleSize(const R: TRect;
  ANumerator, ADenominator: Integer): TRect;
begin
  Result := R;
  Result.Right := Result.Left + MulDiv(Result.Right - Result.Left, ANumerator,
    ADenominator);
  Result.Bottom := Result.Top + MulDiv(Result.Bottom - Result.Top, ANumerator,
    ADenominator);
end;
...
begin
  Result := R1;
  if (R2.Right - R2.Left <= 0) or (R2.Bottom - R2.Top <= 0) then
    Exit;
...
begin
  Result := cxPoint(cxHalfCoordinate(R.Right + R.Left),
    cxHalfCoordinate(R.Bottom + R.Top));
end;
...
begin
  Result := (ABounds.Left <= AInner.Left) and (ABounds.Right >= AInner.Right)
    and (ABounds.Top <= AInner.Top) and (ABounds.Bottom >= AInner.Bottom);
end;
...
begin
  Result := cxPointInLine(cxPointF(P), cxPointF(LinePoint1),
    cxPointF(LinePoint2));
end;
...
function cxPointOffset(const P, DP: TPoint;
  APositiveFactor: Boolean = True): TPoint;
begin
...
function cxPointOffset(const P, AOffset: TdxPointF;
  APositiveFactor: Boolean = True): TdxPointF;
begin
...
function cxPointReplaceItem(const P: TPoint;
  const AIndex, AValue: Integer): TPoint;
begin
  if AIndex = 0 then
    Result.X := AValue
  else
...
function cxPointScale(const P: TPoint;
  ANumerator, ADenominator: Integer): TPoint;
begin
...
begin
  Result := (cxRectWidth(R1) = cxRectWidth(R2)) and
    (cxRectHeight(R1) = cxRectHeight(R2));
end;
...
begin
  ...
  if (ASign.X = -1) and (ASign.Y = 1) then
    Result := Pi - Result
  else if (ASign.X = -1) and (ASign.Y = -1) then
    Result := Pi + Result
  else if (ASign.X = 1) and (ASign.Y = -1) then
    Result := 2 * Pi - Result;
end;
...
function dxRingPoint(const ABase: TdxPointF; ARadius, AAngle: Single)
  : TdxPointF;
begin
  Result := dxPointF(ABase.X + ARadius * Cos(AAngle),
    ABase.Y - ARadius * Sin(AAngle));
end;
...
begin
  Result := (ASize1.Height = ASize2.Height) and (ASize1.Width = ASize2.Width);
end;

function dxSizeDoubleScale(const ASize: TdxSizeDouble;
  const AScale: TdxPointDouble): TdxSizeDouble;
begin
...
function dxPointDoubleScale(const APoint: TdxPointDouble;
  const AScale: TdxPointDouble): TdxPointDouble;
begin
...
begin
  Result := SameValue(ARect1.Left, ARect2.Left) and
    SameValue(ARect1.Top, ARect2.Top) and SameValue(ARect1.Width, ARect2.Width)
    and SameValue(ARect1.Height, ARect2.Height);
end;

function dxRectDoubleOffset(const ARect: TdxRectDouble;
  const APoint: TdxPointDouble; APositiveFactor: Boolean = True): TdxRectDouble;
begin
  if APositiveFactor then
...
class function TdxRectF.Create(const ALeft, ATop, ARight, ABottom: Single)
  : TdxRectF;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
...
class function TdxRectF.CreateSize(const ALeft, ATop, AWidth, AHeight: Single)
  : TdxRectF;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
...
begin
  Result := Math.IsZero(Right) and Math.IsZero(Bottom) and Math.IsZero(Top) and
    Math.IsZero(Left);
end;
...
begin
  ...
  begin
    ...
    Result := cxRectOffset(Result, ABounds.Left + (ABounds.Width - Result.Width)
      / 2, ABounds.Top);
  end
  else
  begin
    ...
    Result := cxRectOffset(Result, ABounds.Left,
      ABounds.Top + (ABounds.Height - Result.Height) / 2);
  end;
end;
...
class function TdxSizeDouble.Create(const AWidth, AHeight: Double)
  : TdxSizeDouble;
begin
  Result.Width := AWidth;
  Result.Height := AHeight;
...
class function TdxRectDouble.CreateFromLTRB(const ALeft, ATop, ARight,
  ABottom: Double): TdxRectDouble;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
...
class function TdxRectDouble.Create(const ALeft, ATop: Double;
  const ASize: TdxSizeDouble): TdxRectDouble;
begin
...
class function TdxRectDouble.Create(const ALeft, ATop, AWidth, AHeight: Double)
  : TdxRectDouble;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
...
class function TdxRectDouble.Create(const AOrigin: TdxPointDouble)
  : TdxRectDouble;
begin
...
class function TdxRectDouble.Create(const AOrigin: TdxPointDouble;
  AWidth, AHeight: Double): TdxRectDouble;
begin
  Result.TopLeft := AOrigin;
  Result.Width := AWidth;
  Result.Height := AHeight;
...
class function TdxRectDouble.Create(const ATopLeft, ABottomRight
  : TdxPointDouble): TdxRectDouble;
begin
  Result.TopLeft := ATopLeft;
  Result.BottomRight := ABottomRight;
end;
...
constructor TcxSize.Create(AOwner: TPersistent;
  ADefaultWidth, ADefaultHeight: Integer);
begin
...
begin
  case AIndex of
    ...
    if (FMargin.Left = FMargin.Top) and (FMargin.Left = FMargin.Right) and
      (FMargin.Left = FMargin.Bottom) then
      Result := FMargin.Left
    else
      Result := -1;
  end;
...
constructor TdxPointDoublePersistent.Create(AOwner: TPersistent;
  const ADefaultX: Double = 0; const ADefaultY: Double = 0);
begin
  inherited Create(AOwner);
...
function TdxPointDoublePersistent.IsEqual(const APoint: TdxPointDouble)
  : Boolean;
begin
...
begin
  CreateEx(M.XForm.eM11, M.XForm.eM12, M.XForm.eM21, M.XForm.eM22, M.XForm.eDx,
    M.XForm.eDy);
end;
...
procedure TdxMatrix.Multiply(M: TdxMatrix;
  AOrder: TdxTransformationOrder = moPrepend);
begin
...
procedure TdxMatrix.Rotate(AAngle: Single;
  AOrder: TdxTransformationOrder = moPrepend);
begin
  DoMultiply(TXForm.CreateRotationMatrix(AAngle), AOrder);
end;

procedure TdxMatrix.Rotate(AAngle: Single; const APivotPoint: TPoint;
  AOrder: TdxTransformationOrder = moPrepend);
begin
  Rotate(AAngle, dxPointF(APivotPoint), AOrder);
end;

procedure TdxMatrix.Rotate(AAngle: Single; const APivotPoint: TdxPointF;
  AOrder: TdxTransformationOrder = moPrepend);
const
  TranslateDirectionMap: array [TdxTransformationOrder] of Integer = (1, -1);
begin
  Translate(TranslateDirectionMap[AOrder] * APivotPoint.X,
    TranslateDirectionMap[AOrder] * APivotPoint.Y, AOrder);
  Rotate(AAngle, AOrder);
  Translate(-TranslateDirectionMap[AOrder] * APivotPoint.X,
    -TranslateDirectionMap[AOrder] * APivotPoint.Y, AOrder);
end;

procedure TdxMatrix.Scale(AScaleX, AScaleY: Single;
  AOrder: TdxTransformationOrder = moPrepend);
begin
  DoMultiply(TXForm.CreateScaleMatrix(AScaleX, AScaleY), AOrder);
end;

procedure TdxMatrix.Scale(const AScale: TdxPointF;
  AOrder: TdxTransformationOrder = moPrepend);
begin
  Scale(AScale.X, AScale.Y, AOrder);
end;

procedure TdxMatrix.Translate(DX, DY: Single;
  AOrder: TdxTransformationOrder = moPrepend);
begin
  DoMultiply(TXForm.CreateTranslateMatrix(DX, DY), AOrder);
end;

procedure TdxMatrix.Translate(const AOffset: TdxPointF;
  AOrder: TdxTransformationOrder = moPrepend);
begin
  Translate(AOffset.X, AOffset.Y, AOrder);
end;

procedure TdxMatrix.DoMultiply(const AXForm: TXForm;
  AOrder: TdxTransformationOrder = moPrepend);
begin
...
function TdxScaleFactorListenerComparer.Compare(const Left,
  Right: TdxScaleFactorNotifyEvent): Integer;
var
  AResult: NativeInt;
begin
...
class function TdxScaleFactorListenerComparer.GetDefault
  : IComparer<TdxScaleFactorNotifyEvent>;
begin
...
procedure TdxScaleFactor.Assign(ANumerator, ADenominator: Integer;
  AIsLoading: Boolean = False);
var
  M, D: Integer;
begin
...
begin
  Result := TdxScaleFactor(Obj).Denominator * Numerator = TdxScaleFactor(Obj)
...
function TdxScaleFactor.Apply(const V: TPoint;
  ASourceScaleFactor: TdxScaleFactor): TPoint;
begin
  Result := Apply(V, ASourceScaleFactor.Numerator,
    ASourceScaleFactor.Denominator);
end;

function TdxScaleFactor.Apply(const V: Integer;
  ASourceScaleFactor: TdxScaleFactor): Integer;
begin
  Result := Apply(V, ASourceScaleFactor.Numerator,
    ASourceScaleFactor.Denominator);
end;

function TdxScaleFactor.Apply(const V: TSize;
  ASourceScaleFactor: TdxScaleFactor): TSize;
begin
  Result := Apply(V, ASourceScaleFactor.Numerator,
    ASourceScaleFactor.Denominator);
end;

function TdxScaleFactor.Apply(const V: TRect;
  ASourceScaleFactor: TdxScaleFactor): TRect;
begin
  Result := Apply(V, ASourceScaleFactor.Numerator,
    ASourceScaleFactor.Denominator);
end;
...
function TdxScaleFactor.Apply(const V: TPoint;
  ASourceNumerator, ASourceDenominator: Integer): TPoint;
begin
...
function TdxScaleFactor.Apply(const V: Integer;
  ASourceNumerator, ASourceDenominator: Integer): Integer;
begin
  if TransformScaleFactor(ASourceNumerator, ASourceDenominator) then
...
function TdxScaleFactor.Apply(const V: TSize;
  ASourceNumerator, ASourceDenominator: Integer): TSize;
begin
  if TransformScaleFactor(ASourceNumerator, ASourceDenominator) then
...
function TdxScaleFactor.Apply(const V: TRect;
  ASourceNumerator, ASourceDenominator: Integer): TRect;
begin
  if TransformScaleFactor(ASourceNumerator, ASourceDenominator) then
...
{$ELSE}
    FListeners := TList<TdxScaleFactorNotifyEvent>.Create
      (TdxScaleFactorListenerComparer.Default);
{$ENDIF}
  end;
  FListeners.Add(AEventHandler);
end;

procedure TdxScaleFactor.ListenerRemove(AEventHandler
  : TdxScaleFactorNotifyEvent);
begin
...
function TdxScaleFactor.TransformScaleFactor(var ASourceNumerator,
  ASourceDenominator: Integer): Boolean;
var
  M, D: Integer;
begin
...
procedure TdxOwnedScaleFactor.OwnerChangeHandler(Sender: TObject; M, D: Integer;
  IsLoading: Boolean);
begin
...
class function TdxXFormHelper.CreateFlip(AFlipHorizontally, AFlipVertically
  : Boolean; const APivotPointX, APivotPointY: Single): TXForm;
const
  FlipValueMap: array [Boolean] of Integer = (1, -1);
begin
  Result := CreateMatrix(FlipValueMap[AFlipHorizontally], 0, 0,
    FlipValueMap[AFlipVertically], IfThen(AFlipHorizontally, 2 * APivotPointX),
...
class function TdxXFormHelper.CreateMatrix(M11, M12, M21, M22, DX,
  DY: Single): TXForm;
begin
  Result.eM11 := M11;
  Result.eM12 := M12;
...
begin
  ...
  Result := CreateMatrix(Cos(AAngle), Sin(AAngle), -Sin(AAngle),
    Cos(AAngle), 0, 0);
end;
...
class function TdxXFormHelper.CreateScaleMatrix(AScaleX,
  AScaleY: Single): TXForm;
begin
  Result := CreateMatrix(AScaleX, 0, 0, AScaleY, 0, 0);
end;

class function TdxXFormHelper.CreateTranslateMatrix(AOffsetX,
  AOffsetY: Single): TXForm;
begin
  Result := CreateMatrix(1, 0, 0, 1, AOffsetX, AOffsetY);
end;

class function TdxXFormHelper.IsEqual(const AMatrix1, AMatrix2: TXForm)
  : Boolean;
begin
  Result := SameValue(AMatrix1.eM11, AMatrix2.eM11) and
...
begin
  Result := (AMatrix.eM11 = 1) and (AMatrix.eM12 = 0) and (AMatrix.eM21 = 0) and
    (AMatrix.eM22 = 1) and (AMatrix.eDx = 0) and (AMatrix.eDy = 0);
end;
