...
type
  ...
  TdxDynamicItemListForEachProcRef = reference to procedure
  ...
  strict private
    ...
    procedure SetItem(AIndex: Integer;
      const AValue: TdxDynamicListItem); inline;
  protected
    ...
    procedure ForEach(AProc: TdxDynamicItemListForEachProcRef;
      AStartIndex: Integer; AFinishIndex: Integer;
      AGoForward: Boolean = True); overload;
  ...
  public
    ...
    property Items[Index: Integer]: TdxDynamicListItem read GetItem
  ...
  protected
    ...
    procedure AddToHash(var AHash: Integer; const AData: AnsiString);
      overload; inline;
    procedure AddToHash(var AHash: Integer; const AData: string);
      overload; inline;
...
constructor TdxDynamicListItem.Create(AOwner: TdxDynamicItemList;
  AIndex: Integer);
begin
  inherited Create;
  FOwner := AOwner;
...
function TdxDynamicItemList.CreateItem(const AIndex: Integer)
  : TdxDynamicListItem;
begin
...
begin
  ...
  while (Result <> nil) and (Result.Index < AIndex) do
    Result := Result.FNext;
  while (Result <> nil) and (Result.FPrev <> nil) and
    (Result.FPrev.Index = AIndex) do
    Result := Result.FPrev;
...
begin
  ...
  begin
    if (FCurrentItem <> nil) and ((FCurrentItem.Index <= AStartIndex) or
      (Abs(FCurrentItem.Index - AStartIndex) < Abs(FFirst.Index - AStartIndex)))
    then
    begin
      AItem := FCurrentItem;
  ...
  begin
    if (FCurrentItem <> nil) and ((FCurrentItem.Index >= AFinishIndex) or
      (Abs(FCurrentItem.Index - AFinishIndex) < Abs(FLast.Index - AFinishIndex)))
    then
    begin
      AItem := FCurrentItem;
...
procedure TdxDynamicItemList.InsertItem(const ANewItem, ANeighborItem
  : TdxDynamicListItem);
...
function TdxDynamicItemList.ItemNeeded(const AIndex: Integer)
  : TdxDynamicListItem;
begin
  Result := Items[AIndex];
  if Result = nil then
...
procedure TdxDynamicItemList.SetItem(AIndex: Integer;
  const AValue: TdxDynamicListItem);
begin
...
procedure TdxHashTableItem.AddToHash(var AHash: Integer;
...
procedure TdxHashTableItem.AddToHash(var AHash: Integer; const AData;
  ADataSize: Integer);
begin
