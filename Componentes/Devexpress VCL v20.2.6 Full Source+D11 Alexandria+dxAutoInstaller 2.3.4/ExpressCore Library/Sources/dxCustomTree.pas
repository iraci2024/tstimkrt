...
type
  ...
  TdxTreeNodeAttachMode = (namAdd, namAddFirst, namAddChild, namAddChildFirst,
    namInsert);
  TdxTreeNodeAddMode = (amAdd, amAddFirst, amInsert);
  TdxTreeNodeState = (nsCollapsed, nsHasChildren, nsValidIndexes, nsDeleting,
    nsInternalDelete, nsInvisible);
  TdxTreeNodeStates = set of TdxTreeNodeState;

  TdxCustomTreeNodeCompareProc = function(ANode1,
    ANode2: TdxTreeCustomNode): Integer;
  ...
  protected
    ...
    procedure SetNodeParent(ANewNode, ANewParent: TdxTreeCustomNode;
      AMode: TdxTreeNodeAddMode); virtual;
    procedure SetParentFor(AValue: TdxTreeCustomNode;
  ...
  public
    ...
    function AddNode(ANode, ARelative: TdxTreeCustomNode; AData: Pointer;
      AttachMode: TdxTreeNodeAttachMode): TdxTreeCustomNode;
    procedure CustomSort(ACompareProc: TdxCustomTreeNodeCompareProc;
    ...
    procedure MoveTo(ADestNode: TdxTreeCustomNode;
      AMode: TdxTreeNodeAttachMode); virtual;
  ...
  TdxTreeForEachNodeProc = reference to function(ANode: TdxTreeCustomNode;
    AData: Pointer): Boolean;

function dxTreeForEach(ARoot: TdxTreeCustomNode; AProc: TdxTreeForEachNodeProc;
...
const
  IsAddChild: array [TdxTreeNodeAttachMode] of Boolean = (False, False, True,
    True, False);
  AddMode: array [TdxTreeNodeAttachMode] of TdxTreeNodeAddMode = (amAdd,
    amAddFirst, amAdd, amAddFirst, amInsert);
begin
  if ANode = nil then
  begin
  ...
  else if Result.StructureID < 0 then
    Result.StructureID := Result.Parent.Count;
  Notify([tnStructure]);
end;

procedure TdxTreeCustomNode.CustomSort(ACompareProc
  : TdxCustomTreeNodeCompareProc; ARecurse: Boolean = False);
var
  I: Integer;
  AList: TList;
begin
  if not Assigned(ACompareProc) or (Count = 0) then
    Exit;
  BeginUpdate;
  try
    AList := TList.Create;
...
procedure TdxTreeCustomNode.MoveTo(ADestNode: TdxTreeCustomNode;
...
begin
  ...
  if (ADestNode = nil) or ADestNode.HasAsParent(Self) or
    ((ADestNode = Parent) and (AMode = namAddChild)) or (ADestNode = Self) then
    Exit;
...
procedure TdxTreeCustomNode.SetNodeParent(ANewNode,
  ANewParent: TdxTreeCustomNode; AMode: TdxTreeNodeAddMode);
begin
  case AMode of
    amAdd:
...
procedure TdxTreeCustomNode.SetParentFor(AValue: TdxTreeCustomNode;
  AValidateIndexes: Boolean = True);
begin
  AValue.FParent := Self;
...
begin
  ...
  begin
    ...
    try
      if AValue then
      begin
        if not FOwner.CanExpand(Self) then
          Exit;
        LoadChildren;
      ...
      begin
        if not FOwner.CanCollapse(Self) then
          Exit;
