...
uses
  Classes, SysUtils, Generics.Defaults, Generics.Collections, dxCore,
  dxCoreClasses, dxStringHelper, dxXMLClasses;

type
  TdxXmlReader = class;

  TdxXmlReadState = (Initial, Interactive, Error, EndOfFile, Closed);

  TdxXmlNodeType = (None, Element, Attribute, Text, CDATA, EntityReference,
    Entity, ProcessingInstruction, Comment, Document, DocumentType,
    DocumentFragment, Notation, Whitespace, SignificantWhitespace, EndElement,
    EndEntity, XmlDeclaration);

  TdxXmlSpace = (None, Default, Preserve);

  TdxWhitespaceHandling = (All, Significant, None);
  ...
  public
    ...
    function Add(const AKey: TCharArray; AStart: Integer; ALen: Integer)
      : string; overload;
    function Get(const AValue: string): string; overload;
    function Get(const AKey: TCharArray; AStart: Integer; ALen: Integer)
      : string; overload;
  end;
  ...
  public
    ...
    property LineNumberOffset: Integer read FLineNumberOffset
      write FLineNumberOffset;
    property LinePositionOffset: Integer read FLinePositionOffset
      write FLinePositionOffset;
    property ConformanceLevel: TdxXmlConformanceLevel read FConformanceLevel
      write FConformanceLevel;
    property CheckCharacters: Boolean read FCheckCharacters
      write FCheckCharacters;
    property MaxCharactersInDocument: Int64 read FMaxCharactersInDocument
      write FMaxCharactersInDocument;
    property IgnoreWhitespace: Boolean read FIgnoreWhitespace
      write FIgnoreWhitespace;
    property IgnoreProcessingInstructions: Boolean read FIgnorePIs
      write FIgnorePIs;
  ...
  public
    ...
    function GetAttribute(const AAttribute: string): string; overload;
      virtual; abstract;
    function GetAttribute(const AAttribute, ANamespaceURI: string): string;
      overload; virtual; abstract;
    function IsEmptyElement: Boolean; virtual; abstract;
    function LookupNamespace(const ANameSpace: string): string; overload;
      virtual; abstract;
    function MoveToElement: Boolean; virtual; abstract;
    function MoveToNextAttribute: Boolean; virtual; abstract;
    function ReadToFollowing(const ALocalName: string): Boolean;
      overload; virtual;
    function ReadToFollowing(const ALocalName, ANamespaceURI: string): Boolean;
      overload; virtual;
...
resourcestring
  SXmlBadAttributeChar =
    '''%s'', hexadecimal value %s, is an invalid attribute character.';
  SXmlBadDecimalEntity =
  ...
  SXmlNamespaceDeclXmlXmlns =
    'Prefix ''&s'' cannot be mapped to namespace name reserved for ''xml'' or ''xmlns''.';
  SXmlReadOnlyProperty = 'The ''%s'' property is read only and cannot be set.';
  SXmlTagMismatchEx =
    'The ''%s'' start tag on line ''%s'' position ''%s'' does not match the end tag of ''%s''.';
  ...
  SXmlUnexpectedEOFInElementContent =
    'Unexpected end of file has occurred. The following elements are not closed: %s';
  SXmlUnexpectedTokenEx =
    '''%s'' is an unexpected token. The expected token is ''%s''.';
  SXmlUnexpectedTokens2 =
    '''%s'' is an unexpected token. The expected token is ''%s'' or ''%s''.';
...
type
  ...
  protected type
    TNamespaceDeclaration = record
      ...
      procedure &Set(const APrefix, AUri: string;
        AScopeId, APreviousNsIndex: Integer);
    end;
  ...
  public
    ...
    procedure RemoveNamespace(const APrefix: string;
      const AUri: string); virtual;
    ...
    function GetNamespaceDeclaration(AIdx: Integer; out APrefix: string;
  ...
  protected
    ...
    procedure SetValue(const AChars: TCharArray;
      AStartPos, ALength: Integer); overload;
    procedure SetNamedNode(AType: TdxXmlNodeType;
      const ALocalName: string); overload;
    procedure SetNamedNode(AType: TdxXmlNodeType;
      const ALocalName, APrefix, ANameWPrefix: string); overload;
    procedure SetValueNode(AType: TdxXmlNodeType;
      const AValue: string); overload;
    procedure SetValueNode(AType: TdxXmlNodeType; const AChars: TCharArray;
      AStartPos, ALength: Integer); overload;
    ...
    property XmlContextPushed: Boolean read FXmlContextPushed
      write FXmlContextPushed;
  public
    constructor Create;
    ...
    property IsEmptyElement: Boolean read GetIsEmptyElement
      write SetIsEmptyElement;
    property IsDefaultAttribute: Boolean read GetIsDefaultAttribute
      write SetIsDefaultAttribute;
...
{$REGION 'Private helper types'}
    TParsingFunction = (ElementContent, NoData, SwitchToInteractive,
      SwitchToInteractiveXmlDecl, DocumentContent, MoveToElementContent,
      PopElementContext, PopEmptyElementContext, ResetAttributesRootLevel,
      Error, Eof, ReaderClosed, EntityReference, InIncrementalRead,
      FragmentAttribute, ReportEndEntity, AfterResolveEntityInContent,
      AfterResolveEmptyEntityInContent, XmlDeclarationFragment, GoToEof,
      PartialTextValue,

      InReadAttributeValue, InReadValueChunk, InReadContentAsBinary,
      InReadElementContentAsBinary);

    TParsingMode = (Full, SkipNode, SkipContent);

    TEntityType = (CharacterDec, CharacterHex, CharacterNamed, Expanded,
      Skipped, FakeExpanded, Unexpanded, ExpandedInAttribute);

    TEntityExpandType = (All, OnlyGeneral, OnlyCharacter);
...
{$ENDREGION}
  private
    ...
    function AddAttribute(AEndNamePos, AColonPos: Integer)
      : TdxNodeData; overload;
    function AddAttribute(const ALocalName, APrefix: string;
      const ANameWPrefix: string): TdxNodeData; overload;
    function AddAttributeNoChecks(const AName: string; AAttrDepth: Integer)
      : TdxNodeData;
    ...
    procedure InvalidCharRecovery(var ABytesCount: Integer;
      out ACharsCount: Integer);
    ...
    procedure ParseAttributeValueSlow(ACurPosition: Integer; AQuoteChar: Char;
      AAttribute: TdxNodeData);
    procedure PopXmlContext;
    procedure PushXmlContext;

    function ParseCDataOrComment(AType: TdxXmlNodeType;
      out AOutStartPosition, AOutEndPosition: Integer): Boolean; overload;
    function ParseCharRefInline(AStartPosition: Integer;
    ...
    function ParseNamedCharRef(AExpand: Boolean;
      AInternalSubsetBuilder: TStringBuilder): Integer;
    function ParseNamedCharRefInline(AStartPosition: Integer; AExpand: Boolean;
      AInternalSubsetBuilder: TStringBuilder): Integer;
    function ParseNumericCharRef(AExpand: Boolean;
      AInternalSubsetBuilder: TStringBuilder;
      out AEntityType: TEntityType): Integer;
    function ParseNumericCharRefInline(AStartPosition: Integer;
      AExpand: Boolean; AInternalSubsetBuilder: TStringBuilder;
      out ACharCount: Integer; out AEntityType: TEntityType): Integer;
    function ParsePI(APiInDtdStringBuilder: TStringBuilder = nil): Boolean;
    function ParsePIValue(out AOutStartPosition, AOutEndPosition
      : Integer): Boolean;
    function ParseQName(AIsQName: Boolean; AStartOffset: Integer;
      out AColonPosition: Integer): Integer; overload;
    ...
    function ParseText(out AStartPosition, AEndPosition: Integer;
      var AOutOrChars: Integer): Boolean; overload;
    ...
    procedure Throw(const ARes, AArg: string;
      ALineNo, ALinePos: Integer); overload;
    ...
    procedure Throw(APosition: Integer; const ARes: string;
      const AArgs: array of const); overload;
    procedure Throw(APosition: Integer; const ARes: string;
      const AArgs: TArray<string>); overload;
    procedure ThrowExpectingWhitespace(APosition: Integer);
    procedure ThrowInvalidChar(const AData: TCharArray;
      ALength, AInvCharPos: Integer);
    procedure ThrowTagMismatch(AStartTag: TdxNodeData);
    procedure ThrowUnexpectedToken(APosition: Integer;
      const AExpectedToken1: string;
      const AExpectedToken2: string = ''); overload;
    procedure ThrowUnexpectedToken(const AExpectedToken1, AExpectedToken2
      : string); overload;
  ...
  protected
    ...
    class procedure BlockCopyChars(ASource: TCharArray; ASourceOffset: Integer;
      ADestination: TCharArray; ADestinationOffset, ACount: Integer);
      static; inline;
    class function ConvertToConstArray(const AArgs: TArray<string>)
      : TArray<TVarRec>;
    class function StrEqual(const AChars: TCharArray;
      AStrPos1, AStrLen1: Integer; const AStr2: string): Boolean; static;
    class function StripSpaces(const AValue: string): string; overload; static;
    class procedure StripSpaces(var AValue: TCharArray; AIndex: Integer;
      var ALen: Integer); overload; static;
  ...
  public
    ...
    constructor Create(AStream: TStream; const ABytes: TBytes;
      AByteCount: Integer; ASettings: TdxXmlReaderSettings;
      ACloseInput: Boolean); overload;
    destructor Destroy; override;
...
function IfThen(AValue: Boolean; const ATrue: Boolean;
  const AFalse: Boolean = False): Boolean; overload; inline;
begin
  if AValue then
    Result := ATrue
  else
...
function GetRemainingUTF8EncodedCharacterByteCount(ABuffer: PByte;
  ABytesInBuffer: Integer): Integer;
begin
...
procedure TEncodingHelper.Convert(const ABytes: TBytes;
  AByteIndex, AByteCount: Integer; const AChars: TCharArray;
...
begin
  ...
  begin
    while (AByteCount > 0) and
      (GetRemainingUTF8EncodedCharacterByteCount(@ABytes[AByteIndex],
      AByteCount) <> 0) do
      Dec(AByteCount);
  end
  else
  ...
  begin
    while (AByteCount > 0) and
      (UnicodeFromLocaleChars(TMBCSEncoding(Self).CodePage,
      MB_ERR_INVALID_CHARS, PAnsiChar(@ABytes[AByteIndex]), AByteCount, nil,
      0) = 0) do
      Dec(AByteCount);
  end;
...
procedure TdxXmlNamespaceManager.TNamespaceDeclaration.&Set(const APrefix,
  AUri: string; AScopeId, APreviousNsIndex: Integer);
begin
  Prefix := APrefix;
  Uri := AUri;
  ScopeId := AScopeId;
...
begin
  ...
  if (Pointer(FXml) = Pointer(APrefix)) and (AUri <> TdxXmlReservedNs.NsXml)
  then
  ...
  else if FLastDecl >= MinDeclsCountForHashTable then
  begin
...
procedure TdxXmlNamespaceManager.RemoveNamespace(const APrefix: string;
  const AUri: string);
var
  ADeclIndex: Integer;
begin
  ...
  begin
    if (FNsdecls[ADeclIndex].ScopeId = FScopeId) and
      (FNsdecls[ADeclIndex].Uri = AUri) then
...
function TdxXmlNamespaceManager.LookupNamespaceDecl(const APrefix
  : string): Integer;
var
...
begin
  ...
  begin
    ...
    begin
      if (Pointer(FNsdecls[AThisDecl].Prefix) = Pointer(APrefix)) and
        (FNsdecls[AThisDecl].Uri <> '') then
        Exit(AThisDecl);
      Dec(AThisDecl);
    end;
    AThisDecl := FLastDecl;
    while AThisDecl >= 0 do
    begin
      if (FNsdecls[AThisDecl].Uri <> '') and
        (FNsdecls[AThisDecl].Prefix = APrefix) then
        Exit(AThisDecl);
      Dec(AThisDecl);
    end;
  end;
...
begin
  ...
  begin
    if (FNsdecls[AThisDecl].Uri <> '') and (FNsdecls[AThisDecl].Prefix = APrefix)
    then
    begin
...
function TdxXmlNamespaceManager.GetNamespaceDeclaration(AIdx: Integer;
  out APrefix: string; out AUri: string): Boolean;
begin
  AIdx := FLastDecl - AIdx;
  if AIdx < 0 then
  begin
...
begin
  ...
  else if ALen > MaxStreamLengthForDefaultBufferSize then
...
begin
  ...
  while Read do
    if (NodeType = TdxXmlNodeType.Element) and
      (Pointer(ALocalName) = Pointer(Name)) then
      Exit(True);
  Result := False;
end;

function TdxXmlReader.ReadToFollowing(const ALocalName, ANamespaceURI
  : string): Boolean;
var
...
begin
  ...
  while Read do
    if (NodeType = TdxXmlNodeType.Element) and
      (Pointer(ALocalNameValue) = Pointer(LocalName)) and
...
class function TdxXmlReader.HasValueInternal(ANodeType: TdxXmlNodeType)
  : Boolean;
begin
...
begin
  Assert((FValueStartPos >= 0) or (FValueLocation = TValueLocation.ValueString),
    'Value not ready.');
...
procedure TdxNodeData.SetValueNode(AType: TdxXmlNodeType;
  const AChars: TCharArray; AStartPos, ALength: Integer);
begin
  FType := AType;
  ClearName;
...
procedure TdxNodeData.SetNamedNode(AType: TdxXmlNodeType;
...
procedure TdxNodeData.SetValue(const AChars: TCharArray; AStartPos: Integer;
  ALength: Integer);
begin
  FValue := '';
  FValueLocation := TValueLocation.CharsBuffer;
...
begin
  ...
  begin
    ...
    Assert(FChars <> nil);
    ASb.Append(FChars, FValueStartPos + AValueOffset,
      FValueLength - AValueOffset);
  end
  else if AValueOffset <= 0 then
    ASb.Append(FValue)
  else
...
class procedure TdxXmlTextReaderImpl.BlockCopyChars(ASource: TCharArray;
  ASourceOffset: Integer; ADestination: TCharArray;
  ADestinationOffset, ACount: Integer);
begin
  Move(ASource[ASourceOffset], ADestination[ADestinationOffset],
    ACount * SizeOf(Char));
end;

class function TdxXmlTextReaderImpl.StrEqual(const AChars: TCharArray;
  AStrPos1, AStrLen1: Integer; const AStr2: string): Boolean;
var
  I: Integer;
begin
...
class procedure TdxXmlTextReaderImpl.StripSpaces(var AValue: TCharArray;
  AIndex: Integer; var ALen: Integer);
var
...
function TdxXmlTextReaderImpl.CheckEncoding(const ANewEncodingName: string)
  : TEncoding;
begin
...
constructor TdxXmlTextReaderImpl.Create(AStream: TStream; const ABytes: TBytes;
  AByteCount: Integer; ASettings: TdxXmlReaderSettings; ACloseInput: Boolean);
begin
  Create(ASettings);
...
begin
  InitStreamInput(FLaterInitParam.InputStream, FLaterInitParam.InputBytes,
    FLaterInitParam.InputByteCount, nil);
...
function TdxXmlTextReaderImpl.GetAttribute(const AAttribute,
  ANamespaceURI: string): string;
var
...
begin
  ...
  for I := FIndex + 1 to FIndex + FAttributeCount + 1 - 1 do
    if (FNodes[I].LocalName = ALocalName) and (FNodes[I].Namespace = ANameSpace)
    then
...
begin
  Assert((FParsingState.Stream <> nil) and (FParsingState.Decoder <> nil) and
    (FParsingState.Bytes <> nil));
  Assert(AMaxCharsCount <= Length(FParsingState.Chars) -
    FParsingState.CharsUsed - 1);

  ABytesCount := FParsingState.BytesUsed - FParsingState.BytePos;
  if ABytesCount = 0 then
    Exit(0);
  try
    FParsingState.Decoder.Convert(FParsingState.Bytes, FParsingState.BytePos,
      ABytesCount, FParsingState.Chars, FParsingState.CharsUsed, AMaxCharsCount,
...
begin
  Result := (FAttributeCount > 0) and
    (FParsingFunction >= TParsingFunction.InReadAttributeValue);
end;

function TdxXmlTextReaderImpl.GetIndexOfAttributeWithoutPrefix
...
begin
  ...
  if ANameValue = '' then
    Exit(-1);
  for I := FIndex + 1 to FIndex + FAttributeCount + 1 - 1 do
    if (FNodes[I].LocalName = ANameValue) and (Length(FNodes[I].Prefix) = 0)
    then
      Exit(I);
  Result := -1;
end;

function TdxXmlTextReaderImpl.GetIndexOfAttributeWithPrefix
  (const AName: string): Integer;
var
  I: Integer;
  ANameValue: string;
...
begin
  ...
  begin
    ...
    if (FParsingState.Bytes = nil) or (Length(FParsingState.Bytes) < ABufferSize)
    then
      SetLength(FParsingState.Bytes, ABufferSize);
  end;

  if (FParsingState.Chars = nil) or
    (Length(FParsingState.Chars) < ABufferSize + 1) then
    SetLength(FParsingState.Chars, ABufferSize + 1);

  FParsingState.BytePos := 0;
  while (FParsingState.BytesUsed < 4) and
    (Length(FParsingState.Bytes) - FParsingState.BytesUsed > 0) do
  begin
    ARead := AStream.Read(FParsingState.Bytes[FParsingState.BytesUsed],
      Length(FParsingState.Bytes) - FParsingState.BytesUsed);
    if ARead = 0 then
    begin
...
procedure TdxXmlTextReaderImpl.InvalidCharRecovery(var ABytesCount: Integer;
  out ACharsCount: Integer);
var
...
begin
  ...
  try
    ...
    begin
      FParsingState.Decoder.Convert(FParsingState.Bytes, FParsingState.BytePos +
        ABytesDecoded, 1, FParsingState.Chars, FParsingState.CharsUsed +
        ACharsDecoded, 1, ABDec, AChDec);
...
begin
  ...
  begin
    ...
    if ParseCDataOrComment(AType, AStartPos, AEndPos) then
      FCurrentNode.SetValueNode(AType, FParsingState.Chars, AStartPos,
        AEndPos - AStartPos)
    else
    begin
      repeat
        FStringBuilder.Append(FParsingState.Chars, AStartPos,
          AEndPos - AStartPos);
...
function TdxXmlTextReaderImpl.ParseCDataOrComment(AType: TdxXmlNodeType;
...
begin
  ...
  begin
    if ReadData = 0 then
      Throw(SXmlUnexpectedEOF, IfThen(AType = TdxXmlNodeType.Comment, 'Comment',
        'CDATA'));
  end;
  ...
  begin
    ...
    while (ACh <> AStopChar) and
      ((TdxXmlCharType.CharProperties[ACh] and TdxXmlCharType.Text) <> 0) do
    begin
      Inc(APosition);
      ACh := AChars[APosition];
    end;
    ...
    begin
      ...
      begin
        ...
        else if APosition + 2 = FParsingState.CharsUsed then
          goto ReturnPartial
        else if AType = TdxXmlNodeType.Comment then
          Throw(APosition, SXmlInvalidCommentChars);
      end
      else if APosition + 1 = FParsingState.CharsUsed then
        goto ReturnPartial;
      Inc(APosition);
      Continue;
    end
      ...
      case AChars[APosition] of
        ...
        #$000D:
          begin
            ...
            begin
              if not FParsingState.EolNormalized and
                (FParsingMode = TParsingMode.Full) then
              begin
                ...
                begin
                  ...
                  begin
                    ShiftBuffer(ARpos + ARcount, ARpos,
                      APosition - ARpos - ARcount);
            ...
            else if (APosition + 1 < FParsingState.CharsUsed) or FParsingState.IsEof
            then
            begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Throw(SDTDNotImplemented)
    else if (APosition = FParsingState.CharsUsed) or
      (AMangoQuirks and (AChars[APosition] = #0)) then
      goto LblReadData
    else
    begin
      if FFragmentType = TdxXmlNodeType.Document then
      ...
      begin
        if ParseText then
        begin
          if (FFragmentType = TdxXmlNodeType.None) and
            (FCurrentNode.&Type = TdxXmlNodeType.Text) then
            FFragmentType := TdxXmlNodeType.Element;
          Exit(True);
        end;
      end;
  ...
  LblReadData:
    ...
    begin
      ...
      if not FRootElementParsed and (FFragmentType = TdxXmlNodeType.Document)
      then
...
function TdxXmlTextReaderImpl.AddNode(ANodeIndex, ANodeDepth: Integer)
  : TdxNodeData;
begin
...
function TdxXmlTextReaderImpl.AllocNode(ANodeIndex, ANodeDepth: Integer)
  : TdxNodeData;
begin
  Assert(ANodeIndex < Length(FNodes));
...
function TdxXmlTextReaderImpl.AddAttribute(const ALocalName, APrefix: string;
  const ANameWPrefix: string): TdxNodeData;
var
  ANewAttr, AAttr: TdxNodeData;
  AAttrHash, I: Integer;
begin
  ANewAttr := AddNode(FIndex + FAttributeCount + 1, FIndex + 1);
  ANewAttr.SetNamedNode(TdxXmlNodeType.Attribute, ALocalName, APrefix,
    ANameWPrefix);
...
function TdxXmlTextReaderImpl.AddAttribute(AEndNamePos, AColonPos: Integer)
  : TdxNodeData;
var
...
begin
  ...
  begin
    ALocalName := FNameTable.Add(FParsingState.Chars, FParsingState.CharPos,
      AEndNamePos - FParsingState.CharPos);
  ...
  begin
    ...
    if (APrefixLen = Length(FLastPrefix)) and StrEqual(FParsingState.Chars,
      AStartPos, APrefixLen, FLastPrefix) then
      Result := AddAttribute(FNameTable.Add(FParsingState.Chars, AColonPos + 1,
        AEndNamePos - AColonPos - 1), FLastPrefix, '')
    else
    begin
      APrefix := FNameTable.Add(FParsingState.Chars, AStartPos, APrefixLen);
      FLastPrefix := APrefix;
      Result := AddAttribute(FNameTable.Add(FParsingState.Chars, AColonPos + 1,
        AEndNamePos - AColonPos - 1), APrefix, '');
    end;
  end;
end;

function TdxXmlTextReaderImpl.AddAttributeNoChecks(const AName: string;
  AAttrDepth: Integer): TdxNodeData;
var
...
procedure TdxXmlTextReaderImpl.ParseAttributeValueSlow(ACurPosition: Integer;
  AQuoteChar: Char; AAttribute: TdxNodeData);
label
  LblReadData;
var
  APosition: Integer;
...
begin
  ...
  AValueChunkLineInfo := TdxLineInfo.Create(FParsingState.LineNo,
    FParsingState.LinePos);

  Assert(FStringBuilder.Length = 0);

  while True do
  begin
    while (TdxXmlCharType.CharProperties[AChars[APosition]] and
      TdxXmlCharType.AttrValue) <> 0 do
      Inc(APosition);

    if APosition - FParsingState.CharPos > 0 then
    begin
      FStringBuilder.Append(AChars, FParsingState.CharPos,
        APosition - FParsingState.CharPos);
      FParsingState.CharPos := APosition;
    end;
    ...
    begin
      case AChars[APosition] of
        ...
        '<':
          Throw(APosition, SXmlBadAttributeChar,
            EdxXmlException.BuildCharExceptionArgs('<', #0));
        '&':
          begin
            if APosition - FParsingState.CharPos > 0 then
              FStringBuilder.Append(AChars, FParsingState.CharPos,
            ...
            AEntityLineInfo := TdxLineInfo.Create(FParsingState.LineNo,
              FParsingState.LinePos + 1);

            if not(HandleEntityReference(True, TEntityExpandType.All, APosition)
              in [TEntityType.CharacterDec, TEntityType.CharacterHex,
              TEntityType.CharacterNamed]) then
              APosition := FParsingState.CharPos;
            AChars := FParsingState.Chars;
            Continue;
          end;
...
procedure TdxXmlTextReaderImpl.AddNamespace(const APrefix, AUri: string;
  AAttribute: TdxNodeData);
begin
  if AUri = TdxXmlReservedNs.NsXmlNs then
  begin
    if APrefix = XmlNs then
      Throw(SXmlXmlnsPrefix, AAttribute.LineInfo2.LineNo,
        AAttribute.LineInfo2.LinePos)
    else
      Throw(SXmlNamespaceDeclXmlXmlns, APrefix, AAttribute.LineInfo2.LineNo,
        AAttribute.LineInfo2.LinePos);
  end
  else if AUri = TdxXmlReservedNs.NsXml then
  begin
    if APrefix <> XML then
      Throw(SXmlNamespaceDeclXmlXmlns, APrefix, AAttribute.LineInfo2.LineNo,
        AAttribute.LineInfo2.LinePos);
  ...
  if (AUri = '') and (APrefix <> '') then
    Throw(SXmlBadNamespaceDecl, AAttribute.LineInfo.LineNo,
      AAttribute.LineInfo.LinePos);
...
begin
  ...
  ANameSpace := FNameTable.Add(AAttribute.StringValue);
  if ANameSpace = '' then
    Throw(SXmlBadNamespaceDecl, AAttribute.LineInfo2.LineNo,
      AAttribute.LineInfo2.LinePos - 1);
...
begin
  ...
  begin
    ...
    if AValue = 'preserve' then
      ...
      Throw(SXmlInvalidXmlSpace, AAttribute.StringValue,
        AAttribute.LineInfo.LineNo, AAttribute.LineInfo.LinePos);
  end
  else if AAttribute.LocalName = 'lang' then
  begin
    if not FCurrentNode.XmlContextPushed then
      PushXmlContext;
...
var
  APosition, ALineNoDelta, AStartNameCharSize, AAttrNameLinePos,
    AColonPos: Integer;
  AChars: TCharArray;
  AAttr: TdxNodeData;
...
begin
  ...
  begin
    ...
    while (TdxXmlCharType.CharProperties[ATmpCh0] and
      TdxXmlCharType.Whitespace) <> 0 do
    begin
    ...
    if (TdxXmlCharType.CharProperties[ATmpCh1] and
      TdxXmlCharType.NCStartNameSC) <> 0 then
    ...
    begin
      ...
      else if APosition = FParsingState.CharsUsed then
        goto LblReadData
      else if (ATmpCh1 <> ':') or (FSupportNamespaces) then
        Throw(APosition, SXmlBadStartNameChar,
          EdxXmlException.BuildCharExceptionArgs(AChars,
  ...
  lbContinueParseName:
    repeat
      ...
      if (TdxXmlCharType.CharProperties[ATmpCh2] and TdxXmlCharType.NCNameSC) <> 0
      then
        Inc(APosition)
      else
        Break;
    ...
    begin
      ...
      begin
        if FSupportNamespaces then
          Throw(APosition, SXmlBadNameChar,
            EdxXmlException.BuildCharExceptionArgs(':', #0))
        else
        begin
          Inc(APosition);
      ...
      begin
        ...
        if (TdxXmlCharType.CharProperties[AChars[APosition]] and
          TdxXmlCharType.NCStartNameSC) <> 0 then
        begin
          Inc(APosition);
          goto lbContinueParseName;
    ...
    repeat
      ...
      if (TdxXmlCharType.CharProperties[ATmpCh3] and TdxXmlCharType.AttrValue) = 0
      then
        Break;
      Inc(APosition);
    until False;
    ...
    begin
      AAttr.SetValue(AChars, FParsingState.CharPos,
        APosition - FParsingState.CharPos);
      Inc(APosition);
      FParsingState.CharPos := APosition;
    end
...
SetElement:
  ...
  if (AColonPos = -1) or not FSupportNamespaces then
    FCurrentNode.SetNamedNode(TdxXmlNodeType.Element,
      FNameTable.Add(AChars, FParsingState.CharPos,
      APosition - FParsingState.CharPos))
  else
  begin
    AStartPos := FParsingState.CharPos;
    APrefixLen := AColonPos - AStartPos;
    if (APrefixLen = Length(FLastPrefix)) and StrEqual(AChars, AStartPos,
      APrefixLen, FLastPrefix) then
      FCurrentNode.SetNamedNode(TdxXmlNodeType.Element,
  ...
  AIsWs := (TdxXmlCharType.CharProperties[ACh] and
    TdxXmlCharType.Whitespace) <> 0;
  ...
  begin
    ...
    begin
      ...
      Throw(APosition, SXmlBadNameChar, EdxXmlException.BuildCharExceptionArgs
        (AChars, FParsingState.CharsUsed, APosition));
...
begin
  ...
  while FParsingState.CharsUsed - FParsingState.CharPos < APrefLen +
    ALocLen + 1 do
    if ReadData = 0 then
      Break;
  ...
  begin
    if not StrEqual(AChars, FParsingState.CharPos, ALocLen,
      AStartTagNode.LocalName) then
  ...
  begin
    ...
    if (not StrEqual(AChars, FParsingState.CharPos, APrefLen,
      AStartTagNode.Prefix)) or (AChars[AColonPos] <> ':') or
      (not StrEqual(AChars, AColonPos + 1, ALocLen, AStartTagNode.LocalName))
    then
      ThrowTagMismatch(AStartTagNode);
    ANameLen := ALocLen + APrefLen + 1;
  end;

  AEndTagLineInfo := TdxLineInfo.Create(FParsingState.LineNo,
    FParsingState.LinePos);
  ...
  repeat
    ...
    if APosition = FParsingState.CharsUsed then
      goto LblReadData;
    if (((TdxXmlCharType.CharProperties[AChars[APosition]] and
      TdxXmlCharType.NCNameSC) <> 0) or (AChars[APosition] = ':')) then
      ThrowTagMismatch(AStartTagNode);
          ...
          #$000D:
            begin
              if AChars[APosition] = #$000A then
                Inc(APosition)
              else if (APosition = FParsingState.CharsUsed) and
                not FParsingState.IsEof then
                Break;
              OnNewLine(APosition);
              Continue;
            end;
      ...
      case AChars[APosition] of
        ...
        #$000D:
          begin
            ...
            else if (APosition + 1 < FParsingState.CharsUsed) or
              (FParsingState.IsEof) then
            begin
              if not FParsingState.EolNormalized then
                AChars[APosition] := #$000A;
              Inc(APosition);
...
procedure TdxXmlTextReaderImpl.ShiftBuffer(ASourcePosition, ADestPosition,
  ACount: Integer);
begin
  Move(FParsingState.Chars[ASourcePosition], FParsingState.Chars[ADestPosition],
    ACount * SizeOf(Char));
end;

function TdxXmlTextReaderImpl.ParsePIValue(out AOutStartPosition,
  AOutEndPosition: Integer): Boolean;
var
  APosition, ARcount, ARpos: Integer;
  AChars: TCharArray;
  ATmpCh, ACh: Char;
begin
  ...
  begin
    ...
    begin
      ATmpCh := AChars[APosition];
      if not((ATmpCh <> '?') and (TdxXmlCharType.CharProperties[ATmpCh] and
        TdxXmlCharType.Text <> 0)) then
        Break;
      Inc(APosition);
    end;
...
function TdxXmlTextReaderImpl.ParsePI(APiInDtdStringBuilder
  : TStringBuilder): Boolean;
var
...
begin
  ...
  ATarget := FNameTable.Add(FParsingState.Chars, FParsingState.CharPos,
    ANameEndPos - FParsingState.CharPos);

  if SameText(ATarget, 'xml') then
    Throw(IfThen(ATarget = 'xml', SXmlXmlDeclNotFirst,
      SXmlInvalidPIName), ATarget);
  ...
  begin
    if FParsingState.CharsUsed - FParsingState.CharPos < 2 then
      ReadData;
    if (ACh <> '?') or (FParsingState.Chars[FParsingState.CharPos + 1] <> '>')
    then
      Throw(SXmlBadNameChar,
        ConvertToConstArray(EdxXmlException.BuildCharExceptionArgs
  ...
  begin
    ...
    begin
      if FIgnorePIs then
        Exit(False);
      if FParsingMode = TParsingMode.Full then
        FCurrentNode.SetValue(FParsingState.Chars, AStartPos,
          AEndPos - AStartPos);
    end
    else
      APiInDtdStringBuilder.Append(FParsingState.Chars, AStartPos,
        AEndPos - AStartPos);
  end
  else
  begin
...
function TdxXmlTextReaderImpl.ParseQName(AIsQName: Boolean;
  AStartOffset: Integer; out AColonPosition: Integer): Integer;
label
...
ContinueStartName:
  ...
  begin
    ...
    if (AChars[APosition] <> ':') or FSupportNamespaces then
      Throw(APosition, SXmlBadStartNameChar,
        EdxXmlException.BuildCharExceptionArgs(AChars, FParsingState.CharsUsed,
        APosition));
  end;
...
begin
  ...
  begin
    EatWhitespaces(nil);
    if (FParsingState.Chars[FParsingState.CharPos] = '<') or
      (FParsingState.CharsUsed - FParsingState.CharPos = 0) then
      Exit(False);
  end
  else
  begin
  ...
  if TdxXmlCharType.IsCharData(FParsingState.Chars[FParsingState.CharPos]) then
    ...
    ThrowInvalidChar(FParsingState.Chars, FParsingState.CharsUsed,
...
function TdxXmlTextReaderImpl.GetTextNodeType(AOrChars: Integer)
  : TdxXmlNodeType;
begin
  if AOrChars > $20 then
...
begin
  ...
  begin
    ...
    FCurrentNode.SetValueNode(ANodeType, FParsingState.Chars, AStartPos,
      AEndPos - AStartPos);
  end
  else
  begin
    ...
    begin
      Assert(AEndPos - AStartPos > 0);
      FCurrentNode.SetValueNode(TdxXmlNodeType.Text, FParsingState.Chars,
        AStartPos, AEndPos - AStartPos);
      FNextParsingFunction := FParsingFunction;
    ...
    repeat
      ...
      if AEndPos - AStartPos > 0 then
        FStringBuilder.Append(FParsingState.Chars, AStartPos,
          AEndPos - AStartPos);
    until not(not AFullValue and (AOrChars <= $20) and
      (FStringBuilder.Length < MinWhitespaceLookahedCount));
...
function TdxXmlTextReaderImpl.ParseNumericCharRefInline(AStartPosition: Integer;
  AExpand: Boolean; AInternalSubsetBuilder: TStringBuilder;
  out ACharCount: Integer; out AEntityType: TEntityType): Integer;
label
  Return;
var
  ...
  ACh, ALow, AHigh: Char;
begin
  Assert((FParsingState.Chars[AStartPosition] = '&') and
    (FParsingState.Chars[AStartPosition + 1] = '#'));
...
{$Q-}
  ...
  begin
    ...
    if not TdxXmlCharType.IsCharData(ACh) and FCheckCharacters then
      Throw(IfThen(FParsingState.Chars[AStartPosition + 2] = 'x',
        AStartPosition + 3, AStartPosition + 2), SXmlInvalidCharacter,
        EdxXmlException.BuildCharExceptionArgs(ACh, #0));

    if AExpand then
    begin
      if AInternalSubsetBuilder <> nil then
        AInternalSubsetBuilder.Append(FParsingState.Chars,
          FParsingState.CharPos, APosition - FParsingState.CharPos + 1);
...
function TdxXmlTextReaderImpl.ParseNamedCharRefInline(AStartPosition: Integer;
  AExpand: Boolean; AInternalSubsetBuilder: TStringBuilder): Integer;
label
  FoundCharRef;
var
  APosition: Integer;
...
begin
  ...
  case AChars[APosition] of
    'a':
      begin
        ...
        begin
          ...
          begin
            if (AChars[APosition + 1] = 'p') and (AChars[APosition + 2] = ';')
            then
            begin
              Inc(APosition, 3);
              ACh := '&';
        ...
        else if AChars[APosition] = 'p' then
        begin
          if FParsingState.CharsUsed - APosition >= 4 then
          begin
            if (AChars[APosition + 1] = 'o') and (AChars[APosition + 2] = 's')
              and (AChars[APosition + 3] = ';') then
            begin
              Inc(APosition, 4);
              ACh := #$27;
              goto FoundCharRef;
    ...
    'q':
      ...
      begin
        if (AChars[APosition + 1] = 'u') and (AChars[APosition + 2] = 'o') and
          (AChars[APosition + 3] = 't') and (AChars[APosition + 4] = ';') then
        begin
          Inc(APosition, 5);
          ACh := '"';
          goto FoundCharRef;
...
FoundCharRef:
  ...
  begin
    if AInternalSubsetBuilder <> nil then
      AInternalSubsetBuilder.Append(FParsingState.Chars, FParsingState.CharPos,
        APosition - FParsingState.CharPos);
...
function TdxXmlTextReaderImpl.ParseCharRefInline(AStartPosition: Integer;
  out ACharCount: Integer; out AEntityType: TEntityType): Integer;
begin
  Assert(FParsingState.Chars[AStartPosition] = '&');
  if FParsingState.Chars[AStartPosition + 1] = '#' then
    Result := ParseNumericCharRefInline(AStartPosition, True, nil, ACharCount,
      AEntityType)
  else
  begin
    ACharCount := 1;
...
function TdxXmlTextReaderImpl.ParseNumericCharRef(AExpand: Boolean;
  AInternalSubsetBuilder: TStringBuilder; out AEntityType: TEntityType)
  : Integer;
var
...
begin
  while True do
  begin
    ANewPos := ParseNumericCharRefInline(FParsingState.CharPos, AExpand,
      AInternalSubsetBuilder, ACharCount, AEntityType);
    case ANewPos of
      - 2:
        begin
          if ReadData = 0 then
...
function TdxXmlTextReaderImpl.ParseNamedCharRef(AExpand: Boolean;
  AInternalSubsetBuilder: TStringBuilder): Integer;
var
  ANewPos: Integer;
begin
  while True do
  begin
    ANewPos := ParseNamedCharRefInline(FParsingState.CharPos, AExpand,
      AInternalSubsetBuilder);
    case ANewPos of
      - 1:
        Exit(-1);
      -2:
...
function TdxXmlTextReaderImpl.HandleEntityReference(AIsInAttributeValue
  : Boolean; AExpandType: TEntityExpandType; out ACharRefEndPos: Integer)
  : TEntityType;
var
...
begin
  ...
  begin
    ACharRefEndPos := ParseNumericCharRef
      (AExpandType <> TEntityExpandType.OnlyGeneral, nil, AEntityType);
    Assert((AEntityType = TEntityType.CharacterDec) or
      (AEntityType = TEntityType.CharacterHex));
    Result := AEntityType;
  end
  else
  begin
    ACharRefEndPos := ParseNamedCharRef
      (AExpandType <> TEntityExpandType.OnlyGeneral, nil);
...
function TdxXmlTextReaderImpl.ParseText(out AStartPosition,
  AEndPosition: Integer; var AOutOrChars: Integer): Boolean;
label
  LblReadData, ReturnPartialValue;
var
  AChars: TCharArray;
  APosition, ARcount, ARpos, AOrChars, ACharRefEndPos, ACharCount,
    AOffset: Integer;
  AEntityType: TEntityType;
  C, ACh: Char;
begin
  ...
  begin
    ...
    case C of
      ...
      '&':
        begin
          ACharRefEndPos := ParseCharRefInline(APosition, ACharCount,
            AEntityType);
          if ACharRefEndPos > 0 then
          begin
            if ARcount > 0 then
            ...
            if not TdxXmlCharType.IsWhiteSpace
              (AChars[ACharRefEndPos - ACharCount]) then
          ...
          begin
            if APosition > FParsingState.CharPos then
              goto ReturnPartialValue;
            case HandleEntityReference(False, TEntityExpandType.All,
              APosition) of
              TEntityType.Unexpanded:
                Throw(SDTDNotImplemented);
              TEntityType.CharacterDec, TEntityType.CharacterHex,
                TEntityType.CharacterNamed:
                begin
                  if not TdxXmlCharType.IsWhiteSpace
                    (FParsingState.Chars[APosition - 1]) then
                    AOrChars := AOrChars or $FF;
                end;
            else
              APosition := FParsingState.CharPos;
      ...
      ']':
        begin
          if (FParsingState.CharsUsed - APosition < 3) and not FParsingState.IsEof
          then
            goto LblReadData;

          if (AChars[APosition + 1] = ']') and (AChars[APosition + 2] = '>')
          then
  ...
  LblReadData:
    ...
    begin
      if FParsingState.CharsUsed - FParsingState.CharPos > 0 then
      begin
        if (FParsingState.Chars[FParsingState.CharPos] <> Char($D)) and
          (FParsingState.Chars[FParsingState.CharPos] <> ']') then
          Throw(SXmlUnexpectedEOF1);
        Assert(FParsingState.IsEof);
      end
      else
...
function TdxXmlTextReaderImpl.ParseXmlDeclaration(AIsTextDecl: Boolean)
  : Boolean;
...
begin
  ...
  if (not StrEqual(FParsingState.Chars, FParsingState.CharPos, 5,
    XmlDeclarationBeginning)) or
    (TdxXmlCharType.IsNameSingleChar(FParsingState.Chars[FParsingState.CharPos
    + 5])) then
    goto NoXmlDecl;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (((AEncodingName <> 'utf-8') and (AEncodingName <> 'utf-16')) and
              (AEncodingName <> 'utf-16be')) and
              not(FParsingState.Encoding is TMBCSEncoding) then
              Throw(SXmlEncodingSwitchAfterResetState,
                IfThen(FParsingState.Encoding.GetByteCount('A') = 1, 'UTF-8',
                'UTF-16'));
          end;
      ...
      else if FParsingState.CharPos + 1 = FParsingState.CharsUsed then
        goto LblReadData
      else
    ...
    case FParsingState.Chars[FParsingState.CharPos] of
      'v':
        begin
          if StrEqual(FParsingState.Chars, FParsingState.CharPos,
            ANameEndPos - FParsingState.CharPos, 'version') and
            (AXmlDeclState = 0) then
          begin
            if not AIsTextDecl then
      ...
      'e':
        begin
          if StrEqual(FParsingState.Chars, FParsingState.CharPos,
            ANameEndPos - FParsingState.CharPos, 'encoding') and
            ((AXmlDeclState = 1) or ((AIsTextDecl and (AXmlDeclState = 0))))
          then
          begin
            if not AIsTextDecl then
      ...
      's':
        begin
          if StrEqual(FParsingState.Chars, FParsingState.CharPos,
            ANameEndPos - FParsingState.CharPos, 'standalone') and
            ((AXmlDeclState = 1) or (AXmlDeclState = 2)) and not AIsTextDecl
          then
          begin
            if not AIsTextDecl then
    ...
    ASb.Append(FParsingState.Chars, FParsingState.CharPos,
      ANameEndPos - FParsingState.CharPos);
    FParsingState.CharPos := ANameEndPos;
  ...
  LblContinue:
    ...
    begin
      case AXmlDeclState of
        0:
          if StrEqual(FParsingState.Chars, FParsingState.CharPos,
            APosition - FParsingState.CharPos, '1.0') then
          begin
            if not AIsTextDecl then
              AAttr.SetValue(FParsingState.Chars, FParsingState.CharPos,
                APosition - FParsingState.CharPos);
            AXmlDeclState := 1;
          end
          else
          begin
            SetString(ABadVersion,
              PChar(@FParsingState.Chars[FParsingState.CharPos]),
              APosition - FParsingState.CharPos);
            Throw(SXmlInvalidVersionNumber, ABadVersion);
          end;
        1:
          begin
            SetString(AEncodingName,
              PChar(@FParsingState.Chars[FParsingState.CharPos]),
              APosition - FParsingState.CharPos);
        ...
        2:
          begin
            if StrEqual(FParsingState.Chars, FParsingState.CharPos,
              APosition - FParsingState.CharPos, 'yes') then
              FStandalone := True
            else if StrEqual(FParsingState.Chars, FParsingState.CharPos,
              APosition - FParsingState.CharPos, 'no') then
              FStandalone := False
            else
            begin
              Assert(not AIsTextDecl);
              Throw(SXmlInvalidXmlDecl, FParsingState.LineNo,
                FParsingState.LinePos - 1);
            end;
            if not AIsTextDecl then
              AAttr.SetValue(FParsingState.Chars, FParsingState.CharPos,
      ...
      ASb.Append(AChars, FParsingState.CharPos,
        APosition - FParsingState.CharPos);
      ASb.Append(AQuoteChar);
      FParsingState.CharPos := APosition + 1;
      Continue;
...
var
  AStartPos, AEndPos, AOrChars: Integer;
begin
  Assert(FParsingFunction in [TParsingFunction.PartialTextValue,
    TParsingFunction.InReadValueChunk, TParsingFunction.InReadContentAsBinary,
...
procedure TdxXmlTextReaderImpl.ReThrow(E: Exception;
  ALineNo, ALinePos: Integer);
begin
    ...
    case FParsingFunction of
      ...
      TParsingFunction.PopElementContext:
        begin
          PopElementContext;
          FParsingFunction := FNextParsingFunction;
          Assert(FParsingFunction in [TParsingFunction.ElementContent,
            TParsingFunction.DocumentContent]);
          Continue;
        end;
      ...
      TParsingFunction.Error, TParsingFunction.Eof,
...
var
  ACharsRead, I, ACopyCharsCount, ABytesLeft, ARead, AOriginalBytePos,
    ACharsLen: Integer;
  ABytesRead: Integer;
begin
  ...
  begin
    ...
    if FParsingState.Stream <> nil then
      if FParsingState.BytesUsed - FParsingState.BytePos < MaxByteSequenceLen
      then
        if Length(FParsingState.Bytes) - FParsingState.BytesUsed < MaxByteSequenceLen
        then
  ...
  begin
    ...
    begin
      ...
      begin
        FParsingState.LineStartPos := FParsingState.LineStartPos -
          FParsingState.CharPos;
        if ACopyCharsCount > 0 then
          BlockCopyChars(FParsingState.Chars, FParsingState.CharPos,
            FParsingState.Chars, 0, ACopyCharsCount);
    ...
    begin
      ...
      begin
        ...
        begin
          Move(FParsingState.Bytes[FParsingState.BytePos],
            FParsingState.Bytes[0], ABytesLeft);
  ...
  begin
    ...
    begin
      ...
      begin
        ARead := FParsingState.Stream.
          Read(FParsingState.Bytes[FParsingState.BytesUsed], ABytesRead);
...
begin
  if (ANewEncoding.WebName <> FParsingState.Encoding.WebName) or
    (FParsingState.Decoder = TEncoding.ASCII) and not FAfterResetState then
  begin
...
procedure TdxXmlTextReaderImpl.ThrowUnexpectedToken(APosition: Integer;
  const AExpectedToken1, AExpectedToken2: string);
begin
  FParsingState.CharPos := APosition;
...
begin
  Throw(EdxXmlException.Create(ARes, AArg, FParsingState.LineNo,
    FParsingState.LinePos, ''));
end;
...
begin
  ...
  begin
    ...
    Throw(FParsingState.CharsUsed, SXmlUnexpectedEOFInElementContent,
      FStringBuilder.ToString);
  end;
end;

procedure TdxXmlTextReaderImpl.ThrowUnexpectedToken(const AExpectedToken1,
  AExpectedToken2: string);
var
...
begin
  ...
  if AExpectedToken2 <> '' then
    Throw(SXmlUnexpectedTokens2, [AUnexpectedToken, AExpectedToken1,
      AExpectedToken2])
  else
...
class function TdxXmlTextReaderImpl.ConvertToConstArray
  (const AArgs: TArray<string>): TArray<TVarRec>;
var
  I: Integer;
begin
...
begin
  Assert(((FParsingState.Stream <> nil) and (FParsingState.Decoder <> nil)) and
    (FParsingState.Bytes <> nil));
  Assert(FParsingState.AppendMode,
    'UnDecodeChars cannot be called after ps.appendMode has been changed to false');

  Assert(FParsingState.CharsUsed >= FParsingState.CharPos,
    'The current position must be in the valid character range.');
  if FMaxCharactersInDocument > 0 then
  begin
    Assert(FCharactersInDocument >= FParsingState.CharsUsed -
      FParsingState.CharPos,
      'We didn'#$27't correctly count some of the decoded characters against the MaxCharactersInDocument.');
    FCharactersInDocument := FCharactersInDocument - FParsingState.CharsUsed -
      FParsingState.CharPos;
  end;
  FParsingState.BytePos := FDocumentStartBytePos;
  if FParsingState.CharPos > 0 then
    Inc(FParsingState.BytePos, FParsingState.Encoding.GetByteCount
      (FParsingState.Chars, 0, FParsingState.CharPos));
...
constructor TdxXmlTextReaderImpl.TXmlContext.Create(APreviousContext
  : TXmlContext);
begin
  inherited Create;
...
procedure TdxXmlTextReaderImpl.Throw(APosition: Integer;
  const ARes, AArg: string);
begin
  FParsingState.CharPos := APosition;
  Throw(ARes, AArg);
end;
...
procedure TdxXmlTextReaderImpl.ThrowInvalidChar(const AData: TCharArray;
  ALength, AInvCharPos: Integer);
begin
  Throw(AInvCharPos, SXmlInvalidCharacter,
    EdxXmlException.BuildCharExceptionArgs(AData, ALength, AInvCharPos));
end;

procedure TdxXmlTextReaderImpl.Throw(const ARes: string;
  const AArgs: array of const);
begin
  Throw(EdxXmlException.Create(ARes, AArgs, FParsingState.LineNo,
    FParsingState.LinePos, ''));
end;
...
begin
  ...
  begin
    ...
    SetString(AArg3, PChar(@FParsingState.Chars[FParsingState.CharPos]),
      AEndPos - FParsingState.CharPos);
...
function TdxXmlNameTable.Add(const AKey: TCharArray;
  AStart, ALen: Integer): string;
var
  S: string;
begin
...
function TdxXmlNameTable.Get(const AKey: TCharArray;
  AStart, ALen: Integer): string;
var
  AValue: string;
begin
