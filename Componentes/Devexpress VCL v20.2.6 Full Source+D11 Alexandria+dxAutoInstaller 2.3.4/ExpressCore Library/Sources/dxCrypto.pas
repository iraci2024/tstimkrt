...
type
  ...
  public
    constructor Create(AProvider: IdxCryptoProvider; AAlgorithm: Integer;
      const AKey: TBytes); overload;
    constructor Create(AProvider: IdxCryptoProvider; AAlgorithm: Integer;
      const APasswordHash: HCRYPTHASH); overload;
    destructor Destroy; override;
    // IdxCryptoKey
    function GetBlockSize: Integer;
    function GetChainingMode: Integer;
  ...
  protected
    class procedure Transform(AKey: IdxCryptoKey; ACrypt: Boolean;
      const ASource: TBytes; out ATarget: TBytes;
      AIsFinalBlock: Boolean); overload;
    class procedure Transform(AKey: IdxCryptoKey; ACrypt: Boolean;
      const ASourceStream, ATargetStream: TStream;
      AIsFinalBlock: Boolean); overload;
  public
    class function Decrypt(AKey: IdxCryptoKey; const ASource: TBytes;
      AIsFinalBlock: Boolean = True): TBytes; overload;
    class procedure Decrypt(AKey: IdxCryptoKey;
      const ASourceStream, ATargetStream: TStream;
      AIsFinalBlock: Boolean = True); overload;
    class function Encrypt(AKey: IdxCryptoKey; const ASource: TBytes;
      AIsFinalBlock: Boolean = True): TBytes; overload;
    class procedure Encrypt(AKey: IdxCryptoKey;
      const ASourceStream, ATargetStream: TStream;
      AIsFinalBlock: Boolean = True); overload;
  end;
...
{$REGION 'Private consts'}
    FMap: array [0 .. 7] of TMapItem = ((Name: 'RC4'; Provider: PROV_RSA_FULL;
      Algorithm: CALG_RC4; KeySize: 64), (Name: 'AES'; Provider: PROV_RSA_AES;
      Algorithm: CALG_AES_128; KeySize: 128), (Name: 'AES';
      Provider: PROV_RSA_AES; Algorithm: CALG_AES_192; KeySize: 192),
      (Name: 'AES'; Provider: PROV_RSA_AES; Algorithm: CALG_AES_256;
      KeySize: 256),

      (Name: 'SHA1'; Provider: PROV_RSA_AES; Algorithm: CALG_SHA1;
      KeySize: 160), (Name: 'SHA256'; Provider: PROV_RSA_AES;
      Algorithm: CALG_SHA_256; KeySize: 256), (Name: 'SHA384';
      Provider: PROV_RSA_AES; Algorithm: CALG_SHA_384; KeySize: 384),
      (Name: 'SHA512'; Provider: PROV_RSA_AES; Algorithm: CALG_SHA_512;
      KeySize: 512));
{$ENDREGION}
  public
    class function GetInfo(const AName: string; AKeySize: Integer;
      out AAlgorithmID, AProviderID: Integer): Boolean;
  end;
  ...
  public
    class function DeriveKey(const APassword, ASalt: TBytes;
      AIterationCount: Integer; AKeyLength: Integer): TBytes;
  end;

function dxGenerateSalt(ASize: Integer): TBytes;

procedure dxRC4Crypt(var AKey: TdxRC4Key; AInData, AOutData: PByteArray;
  ADataSize: Integer);
procedure dxRC4Initialize(out AKey: TdxRC4Key; APassword: PByteArray;
  APasswordLength: Integer); overload;
...
constructor TdxCryptoProvider.Create(const ACSPName: PWideChar;
  AProviderType: Integer);
begin
  CryptCheck(CryptAcquireContext(FHandle, nil, ACSPName, AProviderType,
    CRYPT_VERIFYCONTEXT));
end;
...
constructor TdxCryptoKey.Create(AProvider: IdxCryptoProvider;
  AAlgorithm: Integer; const APasswordHash: HCRYPTHASH);
begin
  FProvider := AProvider;
  CryptCheck(CryptDeriveKey(FProvider.Handle, AAlgorithm, APasswordHash, 0,
    FHandle));
end;

constructor TdxCryptoKey.Create(AProvider: IdxCryptoProvider;
  AAlgorithm: Integer; const AKey: TBytes);
var
  AKeyBlob: TBytes;
...
begin
  ...
  Move(AKey[0], AKeyBlob[SizeOf(AKeyBlobHeader) + SizeOf(Integer)],
    Length(AKey));

  CryptCheck(CryptImportKey(FProvider.Handle, @AKeyBlob[0], Length(AKeyBlob), 0,
    0, FHandle));
end;
...
begin
  ...
  if CryptGetKeyParam(FHandle, KP_BLOCKLEN, @ABufferSize, ABufferCapacity, 0)
  then
...
class function TdxCipher.Decrypt(AKey: IdxCryptoKey; const ASource: TBytes;
  AIsFinalBlock: Boolean = True): TBytes;
begin
  Transform(AKey, False, ASource, Result, AIsFinalBlock);
end;

class procedure TdxCipher.Decrypt(AKey: IdxCryptoKey;
  const ASourceStream, ATargetStream: TStream; AIsFinalBlock: Boolean = True);
begin
  Transform(AKey, False, ASourceStream, ATargetStream, AIsFinalBlock);
end;

class function TdxCipher.Encrypt(AKey: IdxCryptoKey; const ASource: TBytes;
  AIsFinalBlock: Boolean = True): TBytes;
begin
  Transform(AKey, True, ASource, Result, AIsFinalBlock);
end;

class procedure TdxCipher.Encrypt(AKey: IdxCryptoKey;
  const ASourceStream, ATargetStream: TStream; AIsFinalBlock: Boolean = True);
begin
  Transform(AKey, True, ASourceStream, ATargetStream, AIsFinalBlock);
end;

class procedure TdxCipher.Transform(AKey: IdxCryptoKey; ACrypt: Boolean;
  const ASource: TBytes; out ATarget: TBytes; AIsFinalBlock: Boolean);
var
...
class procedure TdxCipher.Transform(AKey: IdxCryptoKey; ACrypt: Boolean;
  const ASourceStream, ATargetStream: TStream; AIsFinalBlock: Boolean);
const
...
begin
  ...
  try
    repeat
      ...
      if ACrypt then
        CryptCheck(CryptEncrypt(AKey.Handle, 0, AIsFinalChunk and AIsFinalBlock,
          0, ABuffer, ABufferUsage, ABufferSize))
      else
        CryptCheck(CryptDecrypt(AKey.Handle, 0, AIsFinalChunk and AIsFinalBlock,
          0, ABuffer, ABufferUsage) or AIsFinalChunk);
...
class function TdxCryptoAlgorithms.GetInfo(const AName: string;
  AKeySize: Integer; out AAlgorithmID, AProviderID: Integer): Boolean;
var
  I: Integer;
begin
...
class function TdxPBKDF2.DeriveKey(const APassword, ASalt: TBytes;
  AIterationCount, AKeyLength: Integer): TBytes;
