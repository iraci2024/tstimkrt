...
type
  ...
  public
    class function Create(const ALeft, ATop, ARight, ABottom: Integer;
      ANormalize: Boolean = False): TRect; overload; static;
    class function Create(const AOrigin: TPoint): TRect; overload; static;
    class function Create(const ATopLeft, ABottomRight: TPoint;
      ANormalize: Boolean = False): TRect; overload; static;
    class function CreateSize(const ASize: TSize): TRect; overload; static;
    class function CreateSize(const AOrigin: TPoint; AWidth, AHeight: Integer)
      : TRect; overload; static;
    class function CreateSize(const ALeft, ATop, AWidth, AHeight: Integer)
      : TRect; overload; static;
    class function CreateSize(const ALeft, ATop: Integer; const ASize: TSize)
      : TRect; overload; static;
    class function CreateSize(const AOrigin: TPoint; const ASize: TSize): TRect;
      overload; static;

    class function Round(const ARect: TdxRectF): TRect; static;
    class function Trunc(const ARect: TdxRectF): TRect; static;

    procedure Init(const ALeft, ATop, ARight, ABottom: Integer;
      ANormalize: Boolean = False); overload; inline;
    procedure Init(const AOrigin: TPoint); overload; inline;
    procedure Init(const ATopLeft, ABottomRight: TPoint;
      ANormalize: Boolean = False); overload; inline;
    procedure InitSize(const ASize: TSize); overload; inline;
    procedure InitSize(const AOrigin: TPoint; const ASize: TSize);
      overload; inline;
    procedure InitSize(const AOrigin: TPoint; AWidth, AHeight: Integer);
      overload; inline;
    procedure InitSize(const ALeft, ATop, AWidth, AHeight: Integer);
      overload; inline;
    procedure InitSize(const ALeft, ATop: Integer; const ASize: TSize);
      overload; inline;
    ...
    procedure Deflate(const ADeltaLeft, ADeltaTop, ADeltaRight,
      ADeltaBottom: Integer); overload; inline;
    ...
    procedure Inflate(const ADeltaLeft, ADeltaTop, ADeltaRight,
      ADeltaBottom: Integer); overload; inline;
...
{$IFNDEF DELPHIXE8}
  private
    class procedure CheckArrays(ASource, ADestination: Pointer;
      ASourceIndex, ASourceLength, ADestIndex, ADestLength,
  ...
  public
    class procedure Copy<T>(const ASource: array of T;
      ...
      ACount: NativeInt); overload; static;
    class procedure Copy<T>(const ASource: array of T;
      var ADestination: array of T; ACount: NativeInt); overload; static;
{$ENDIF}
    class procedure Reverse<T>(var ASource: array of T); overload; static;
    class procedure Concat<T>(const ASource: array of T;
      var ADestination: TArray<T>; ASourceIndex, ACount: NativeInt);
      overload; static;
    class procedure Concat<T>(const ASource: array of T;
      var ADestination: TArray<T>); overload; static;
  end;
...
begin
  ...
  T := EnsureRange(T, 0, 1);
  Result := Sqrt(Sqr(A.X - X + (B.X - A.X) * T) +
    Sqr(A.Y - Y + (B.Y - A.Y) * T));
end;
...
class function TdxRectHelper.Create(const ALeft, ATop, ARight, ABottom: Integer;
  ANormalize: Boolean = False): TRect;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
  Result.Right := ARight;
...
class function TdxRectHelper.Create(const ATopLeft, ABottomRight: TPoint;
  ANormalize: Boolean = False): TRect;
begin
...
class function TdxRectHelper.CreateSize(const AOrigin: TPoint;
...
class function TdxRectHelper.CreateSize(const ALeft, ATop, AWidth,
...
class function TdxRectHelper.CreateSize(const ALeft, ATop: Integer;
  const ASize: TSize): TRect;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
...
procedure TdxRectHelper.Init(const ALeft, ATop, ARight, ABottom: Integer;
  ANormalize: Boolean = False);
begin
  Left := ALeft;
  Top := ATop;
  Right := ARight;
...
procedure TdxRectHelper.Init(const ATopLeft, ABottomRight: TPoint;
  ANormalize: Boolean);
begin
  TopLeft := ATopLeft;
...
procedure TdxRectHelper.InitSize(const AOrigin: TPoint;
...
procedure TdxRectHelper.InitSize(const ALeft, ATop: Integer;
  const ASize: TSize);
begin
  Left := ALeft;
  Top := ATop;
...
begin
  Result := (Left <= R.Left) and (Right >= R.Right) and (Top <= R.Top) and
    (Bottom >= R.Bottom);
end;
...
procedure TdxRectHelper.Deflate(const ADeltaLeft, ADeltaTop, ADeltaRight,
  ADeltaBottom: Integer);
begin
  Inc(Left, ADeltaLeft);
...
procedure TdxRectHelper.Inflate(const ADeltaLeft, ADeltaTop, ADeltaRight,
  ADeltaBottom: Integer);
begin
  Dec(Left, ADeltaLeft);
...
begin
  Result := not((Right < R.Left) or (Bottom < R.Top) or (R.Right < Left) or
    (R.Bottom < Top));
end;
...
begin
  Result := (Left = R.Left) and (Top = R.Top) and (Right = R.Right) and
    (Bottom = R.Bottom);
end;
...
var
  I, AWidth, ARemainder, AColumnWidth, AColumnsAreaWidth, AHeight,
    AOffset: Integer;
begin
...
var
  ATypeInfo: PTypeInfo;
begin
  CheckArrays(Pointer(@ASource[0]), Pointer(@ADestination[0]), ASourceIndex,
    Length(ASource), ADestIndex, Length(ADestination), ACount);
  ATypeInfo := TypeInfo(T);
  if ATypeInfo.Kind in [tkLString, tkWString, tkUString, tkVariant, tkArray,
    tkRecord, tkInterface, tkDynArray] then
    System.CopyArray(Pointer(@ADestination[ADestIndex]),
      Pointer(@ASource[ASourceIndex]), ATypeInfo, ACount)
  else
    System.Move(Pointer(@ASource[ASourceIndex])^,
      Pointer(@ADestination[ADestIndex])^, ACount * SizeOf(T));
end;
...
class procedure TdxArrayHelper.CheckArrays(ASource, ADestination: Pointer;
  ASourceIndex, ASourceLength, ADestIndex, ADestLength, ACount: NativeInt);
begin
  if (ASourceIndex < 0) or (ADestIndex < 0) or (ASourceIndex >= ASourceLength)
    or (ADestIndex >= ADestLength) or (ASourceIndex + ACount > ASourceLength) or
    (ADestIndex + ACount > ADestLength) then
...
class procedure TdxArrayHelper.Concat<T>(const ASource: array of T;
  var ADestination: TArray<T>; ASourceIndex, ACount: NativeInt);
var
