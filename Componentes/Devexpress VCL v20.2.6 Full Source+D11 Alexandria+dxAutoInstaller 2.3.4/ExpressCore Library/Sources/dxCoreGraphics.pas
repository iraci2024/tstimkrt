...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Windows, Messages, Types, SysUtils, Classes, Graphics, Generics.Defaults,
...
{$REGION 'private types'}
    // "System" colors
    TSystemColor = (cActiveBorder, cActiveCaption, cActiveCaptionText,
      cAppWorkspace, cButtonFace, cButtonHighlight, cButtonShadow, cControl,
      cControlDark, cControlDarkDark, cControlLight, cControlLightLight,
      cControlText, cDesktop, cGradientActiveCaption, cGradientInactiveCaption,
      cGrayText, cHighlight, cHighlightText, cHotTrack, cInactiveBorder,
      cInactiveCaption, cInactiveCaptionText, cInfo, cInfoText, cMenu, cMenuBar,
      cMenuHighlight, cMenuText, cScrollBar, cWindow, cWindowFrame,
      cWindowText);
...
{$REGION 'private const'}
    SystemColorsData: array [TSystemColor] of TSystemColorData =
      ...
      (Name: 'ActiveCaptionText'; Index: $09), (Name: 'AppWorkspace';
      Index: $0C), (Name: 'ButtonFace'; Index: $0F), (Name: 'ButtonHighlight';
      Index: $14), (Name: 'ButtonShadow'; Index: $10), (Name: 'Control';
      Index: $0F), (Name: 'ControlDark'; Index: $10), (Name: 'ControlDarkDark';
      Index: $15), (Name: 'ControlLight'; Index: $16),
      (Name: 'ControlLightLight'; Index: $14), (Name: 'ControlText';
      Index: $12), (Name: 'Desktop'; Index: $01),
      (Name: 'GradientActiveCaption'; Index: $1B),
      (Name: 'GradientInactiveCaption'; Index: $1C), (Name: 'GrayText';
      Index: $11), (Name: 'Highlight'; Index: $0D), (Name: 'HighlightText';
      Index: $0E), (Name: 'HotTrack'; Index: $1A), (Name: 'InactiveBorder';
      Index: $0B), (Name: 'InactiveCaption'; Index: $03),
...
{$ENDIF}
    ...
    class procedure AddNamedColor(const AName: string;
      AColor: TdxAlphaColor); static;
    class function GetSystemAlphaColor(AColor: TSystemColor)
  ...
  public const
    ...
    class function Blend(AColor, ABackgroundColor: TdxAlphaColor)
      : TdxAlphaColor; overload; static; inline;
    class function CalculateNearestColor(const AColorsToChooseFrom
      : array of TdxAlphaColor; AValue: TdxAlphaColor): TdxAlphaColor; static;
    class function ChangeBrightness(AColor: TdxAlphaColor; ABrightness: Single)
      : TdxAlphaColor; static;
    ...
    class function IsEmpty(AColor: TdxAlphaColor): Boolean; overload;
      static; inline;
    class function IsTransparentOrEmpty(const AColor: TdxAlphaColor): Boolean;
      overload; static; inline;
    class function FromArgb(ARed, AGreen, ABlue: Byte): TdxAlphaColor; overload;
      static; inline;
    class function FromArgb(AAlpha, ARed, AGreen, ABlue: Byte): TdxAlphaColor;
      overload; static; inline;
    class function FromArgb(AAlpha: Byte; ABaseColor: TdxAlphaColor)
      : TdxAlphaColor; overload; static; inline;
    class function FromColor(const AColor: TColor): TdxAlphaColor; overload;
      static; inline;
    class function FromHSL(H, S, L: Single): TdxAlphaColor; overload; static;
    class function ToArgb(const AColor: TdxAlphaColor): Cardinal;
      static; inline;
    class function ToColor(const AColor: TdxAlphaColor): TColor; overload;
      static; inline;
    ...
    class function FromHexCode(const AHexCode: string): TdxAlphaColor; static;
    // for internal use
    class function FromHtml(const AHtmlColor: string): TdxAlphaColor; static;
    class function ToHexCode(AColor: TdxAlphaColor; AUseAlpha: Boolean;
      APrefix: Char = #0000): string; static; // for internal use
    class function ToHtml(AColor: TdxAlphaColor;
      AUseNamedColors: Boolean = True): string; static;
  end;

  TRGBColors = array of TRGBQuad;

  TdxGraphicUnit = (guWorld, // 0 -- World coordinate (non-physical unit)
  ...
  strict private
    ...
    procedure HandlerKeyChanged(Sender: TObject; const Item: string;
      Action: TCollectionNotification);
    procedure HandlerValueChanged(Sender: TObject; const Item: TdxAlphaColor;
      Action: TCollectionNotification);
  ...
  public
    ...
    function GetStrokeColor(const ID: string): TdxAlphaColor; override;
    //
    property FillColors[const ID: string]: TdxAlphaColor read GetFillColor
      write SetFillColor;
    property StrokeColors[const ID: string]: TdxAlphaColor read GetStrokeColor
...
{$ENDREGION}
  strict private
    ...
    FQueueFirst: TQueueItem;
    FQueueLast: TQueueItem;
    FQueueSize: Integer;

    procedure RemoveQueueItem(AItem: TQueueItem);
    procedure ValueHandler(Sender: TObject; const Item: TValue;
      Action: TCollectionNotification);
  protected
  ...
  public
    ...
    constructor Create(ACapacity: Integer;
      AEqualityComparer: IEqualityComparer<TKey>); overload;
    destructor Destroy; override;
  ...
  public
    ...
    class procedure AddRectangleToRegion(var ARegion: HRGN;
      const R: TRect); static;
    class procedure ExcludeRectangleFromRegion(var ARegion: HRGN;
      const R: TRect); static;
  ...
  private const
    GCPFlags: array [Boolean] of Integer = (GCP_USEKERNING or GCP_LIGATE,
      GCP_USEKERNING or GCP_LIGATE or GCP_REORDER);
    class function CalculateCharactersBounds(const AGcpResults: TGCPResults;
      ALength: Integer; const ABounds: TRect): TArray<TRect>; static;
    class function TryMeasureCharactersSlow(ADC: THandle; const AText: PChar;
      ALength: Integer; var AGcpResults: TGCPResults; AFlags: Integer)
      : Integer; static;
  public
    class function GetSelectionRangeRegion(ADC: HDC; const AText: PChar;
      ALength: Integer; const ABounds: TRect; ASelStart, ASelLength: Integer;
      AFont: HFONT = 0; AIsRTLReading: Boolean = False): HRGN; static;
    class function MeasureCharactersBounds(ADC: HDC; const AText: PChar;
      ALength: Integer; const ABounds: TRect; AFont: HFONT = 0;
      AIsRTLReading: Boolean = False): TArray<TRect>; static;
  end;
  ...
  protected type
    TCalcRectProc = procedure(ADC: HDC; const AText: string; var R: TRect;
      AFlags: DWORD);
  ...
  strict private
    ...
    class procedure CalcTextRectDT(ADC: HDC; const AText: string; var R: TRect;
      AFlags: DWORD); static; // inline;
    class function GetMeasureCanvas: TCanvas; static;
  protected
    class function TextRectCore(ADC: THandle; const AText: string;
      ARowCount: Integer; ACalcRectFunc: TCalcRectProc;
      ASingleLineFlags, AMultilineFlags: Integer; AReturnMaxRectHeight: Boolean)
...
{$ENDIF}
    ...
    class function TextLineHeight(ADC: THandle): Integer; overload; static;
    // inline;
    class function TextLineHeight(AFont: TFont): Integer; overload; static;
    class function TextLineHeight(AFont: TFont; AFontStyle: TFontStyles)
      : Integer; overload; static;
    class function TextMetrics(AFont: TFont; out ATextMetric: TTextMetric)
      : Boolean; static;

    class function TextSizeTO(AFont: TFont; const AText: string;
      AFontSize: Integer = 0): TSize; overload; static;
    class function TextSizeTO(AFontHandle: THandle; const AText: string): TSize;
      overload; static;
    class function TextHeightTO(AFont: TFont; const AText: string;
      AFontSize: Integer = 0): Integer; overload; static; // inline;
    class function TextWidthTO(AFont: TFont; const AText: string;
      AFontSize: Integer = 0): Integer; overload; static; // inline;
    class function TextWidthTO(AFont: TFont; const AText: string;
      AFontStyle: TFontStyles): Integer; overload; static; // inline;

    class function GetStringAdjustedToWidthDT(AFont: TFont; const S: string;
      AWidth: Integer; AModifyStringType: TcxModifyStringType): string;
      overload; static;
    class function GetStringAdjustedToWidthDT(ADC: HDC; AFontHandle: HFONT;
      const S: string; AWidth: Integer; AModifyStringType: TcxModifyStringType)
      : string; overload; static;
    class procedure TextRectDT(var ARect: TRect; const AText: string;
      AFont: TFont; ADTFlags: Integer = 0); overload; static;
    class function TextRectDT(AFont: TFont; const AText: string;
      ARowCount: Integer; AConsiderLineBreaks: Boolean = False): TRect;
      overload; static;
    class function TextRectDT(ADC: THandle; const AText: string;
      ARowCount: Integer; AReturnMaxRectHeight: Boolean = False;
      ADTFlags: Integer = 0): TRect; overload; static;
    class function TextSizeDT(AFont: TFont; const AText: string;
      AFlags: Integer = 0): TSize; overload; static;
    class function TextSizeDT(ADC: THandle; const AText: string): TSize;
      overload; static;
    class function TextHeightDT(AFont: TFont; const AText: string; R: TRect;
      AFlags: Integer): Integer; overload; static;
...
function dxAlphaColorToColor(AColor: TdxAlphaColor; out AAlpha: Byte)
  : TColor; overload;
...
function dxMakeAlphaColor(AColor: TColor; AAlpha: Byte = 255)
  : TdxAlphaColor; overload;
...
procedure dxFillBitmapInfoHeader(out AHeader: TBitmapInfoHeader;
  ABitmap: TBitmap; ATopDownDIB: WordBool); overload;
procedure dxFillBitmapInfoHeader(out AHeader: TBitmapInfoHeader;
  AWidth, AHeight: Integer; ATopDownDIB: WordBool); overload;
function GetBitmapBits(ABitmap: TBitmap; var AColors: TRGBColors;
  ATopDownDIB: Boolean): Boolean;
procedure GetBitmapBitsByScanLine(ABitmap: TBitmap; var AColors: TRGBColors);
procedure SetBitmapBits(ABitmap: TBitmap; const AColors: TRGBColors;
  ATopDownDIB: Boolean);
...
function dxGetBitmapData(ABitmapHandle: HBITMAP;
  out ABitmapData: Windows.TBitmap): Boolean;
function dxGetBrushData(ABrushHandle: HBRUSH; out ALogBrush: TLogBrush)
  : Boolean; overload;
function dxGetBrushData(ABrushHandle: HBRUSH): TLogBrush; overload;
function dxGetFontData(AFontHandle: HFONT; out ALogFont: TLogFont)
  : Boolean; overload;
...
begin
  Result := (AColor <> TdxAlphaColors.Default) and
...
begin
  ...
  if AColor = dxacNone then
    Result := clNone
  else if AColor = dxacDefault then
    Result := clDefault
  else
  begin
    AAlpha := AColor shr AlphaShift;
    Result := RGB(Byte(AColor shr RedShift), Byte(AColor shr GreenShift),
      Byte(AColor shr BlueShift));
  end;
end;
...
begin
  case ABitmap.PixelFormat of
    pf1Bit .. pf32Bit:
      Result := ABitCounts[ABitmap.PixelFormat]
  else
...
procedure dxFillBitmapInfoHeader(out AHeader: TBitmapInfoHeader;
  AWidth, AHeight: Integer; ATopDownDIB: WordBool);
begin
...
procedure dxFillBitmapInfoHeader(out AHeader: TBitmapInfoHeader;
  ABitmap: TBitmap; ATopDownDIB: WordBool);
begin
  dxFillBitmapInfoHeader(AHeader, ABitmap.Width, ABitmap.Height, ATopDownDIB);
end;

function InternalGetDIB(ABitmap: TBitmap; const AColors: TRGBColors;
  ATopDownDIB: WordBool): Boolean;
...
begin
  ...
  begin
    ...
    try
      Result := GetDIBits(ADC, ABitmap.Handle, 0, ABitmap.Height, AColors,
        ABitmapInfo, DIB_RGB_COLORS) <> 0;
      if not Result then
      begin
        Result := True;
        for I := 0 to ABitmap.Height - 1 do
        begin
          AScanLineResult := GetDIBits(ADC, ABitmap.Handle, GetStartScan(I), 1,
            @AColors[ABitmap.Width * I], ABitmapInfo, DIB_RGB_COLORS) <> 0;
          if not AScanLineResult then
          begin
            P := cxAllocMem(ABitmap.Width * SizeOf(TRGBQuad));
            try
              AScanLineResult := GetDIBits(ADC, ABitmap.Handle, GetStartScan(I),
                1, P, ABitmapInfo, DIB_RGB_COLORS) <> 0;
              cxCopyData(P, @AColors[ABitmap.Width * I],
                ABitmap.Width * SizeOf(TRGBQuad));
            finally
              cxFreeMem(P);
            end;
          end;
...
function InternalSetDIB(ABitmap: TBitmap; const AColors: TRGBColors;
  ATopDownDIB: WordBool): Boolean;
var
  ADC: HDC;
  ABitmapInfo: TBitmapInfo;
begin
  ...
  begin
    ...
    ADC := CreateCompatibleDC(0);
    try
      Result := SetDIBits(ADC, ABitmap.Handle, 0, ABitmap.Height, AColors,
        ABitmapInfo, DIB_RGB_COLORS) <> 0;
    finally
      DeleteDC(ADC);
    end;
...
begin
  ...
  begin
    ...
    AHasTransparentValues := AHasTransparentValues or
      ((AAlpha > 0) and (AAlpha < $FF));
...
begin
  if IsWin8OrLater and (AInstance <> 0) then
    Result := LoadImage(AInstance, ACursorName, IMAGE_CURSOR, 0, 0,
      LR_DEFAULTCOLOR or LR_DEFAULTSIZE)
  else
...
begin
  Result := (AGraphic1 <> nil) and (AGraphic2 <> nil) and
    (AGraphic1.ClassType = AGraphic2.ClassType) and
...
begin
  ...
  Result := (AFont1.Color = AFont2.Color) and dxGetFontData(AFont1.Handle,
    ALogFont1) and dxGetFontData(AFont2.Handle, ALogFont2) and
...
procedure TdxAlphaColors.TUpdateSystemColorsListener.WndProc
...
class function TdxAlphaColors.GetSystemAlphaColor(AColor: TSystemColor)
  : TdxAlphaColor;
var
  ATemp: TRGBA;
begin
...
class procedure TdxAlphaColors.AddNamedColor(const AName: string;
  AColor: TdxAlphaColor);
var
  AInfo: TColorInfo;
begin
...
class function TdxAlphaColors.Blend(AColor, ABackgroundColor: TdxAlphaColor)
  : TdxAlphaColor;
var
...
begin
  ...
  begin
    ...
    Result := FromArgb(Round(R(AColor) * Alpha + R(ABackgroundColor) *
      AOneAlpha), Round(G(AColor) * Alpha + G(ABackgroundColor) * AOneAlpha),
      Round(B(AColor) * Alpha + B(ABackgroundColor) * AOneAlpha));
  end;
end;

class function TdxAlphaColors.CalculateNearestColor(const AColorsToChooseFrom
  : array of TdxAlphaColor; AValue: TdxAlphaColor): TdxAlphaColor;
var
  AValueHue, AValueBrightness, AValueSaturation, AMinDistance, AHue,
    ASaturation, ABrightness, ADistance: Single;
  AColor: TdxAlphaColor;
begin
  ...
  begin
    ...
    ADistance := AHue * AHue + ASaturation * ASaturation + ABrightness *
      ABrightness;
...
class function TdxAlphaColors.ChangeBrightness(AColor: TdxAlphaColor;
  ABrightness: Single): TdxAlphaColor;
var
  H, S, L: Single;
begin
  ...
  if ABrightness > 0 then
    ...
    L := L + L * ABrightness;
  Result := FromArgb(TAlphaColorChannelMap(AColor).Alpha,
    FromHSL(H, S, Min(1, Max(0, L))));
end;
...
begin
  ...
  else if AGreenF = AMax then
    Result := 2 + (ABlueF - ARedF) / ADelta
  else if ABlueF = AMax then
...
class function TdxAlphaColors.IsTransparentOrEmpty(const AColor
  : TdxAlphaColor): Boolean;
begin
  Result := (AColor = Empty) or (AColor = Transparent);
end;

class function TdxAlphaColors.FromArgb(ARed, AGreen, ABlue: Byte)
  : TdxAlphaColor;
begin
  Result := FromArgb(255, ARed, AGreen, ABlue);
end;

class function TdxAlphaColors.FromArgb(AAlpha, ARed, AGreen, ABlue: Byte)
  : TdxAlphaColor;
begin
  Result := dxMakeAlphaColor(AAlpha, ARed, AGreen, ABlue);
end;

class function TdxAlphaColors.FromArgb(AAlpha: Byte; ABaseColor: TdxAlphaColor)
  : TdxAlphaColor;
begin
  Result := ABaseColor;
...
class procedure TdxAlphaColors.ToHSL(AColor: TdxAlphaColor;
  out H, S, L: Single);
var
...
class function TdxAlphaColors.FromHexCode(const AHexCode: string)
  : TdxAlphaColor;
begin
  case Length(AHexCode) of
    6:
      Result := FromArgb(StrToInt('$' + Copy(AHexCode, 1, 2)),
        StrToInt('$' + Copy(AHexCode, 3, 2)),
        StrToInt('$' + Copy(AHexCode, 5, 2)));
    7:
      Result := FromArgb(StrToInt('$' + Copy(AHexCode, 2, 2)),
...
begin
  ...
  if AHtmlColor = '' then
    Exit;
  if (AHtmlColor[1] = '#') and
    ((Length(AHtmlColor) = 7) or (Length(AHtmlColor) = 4)) then
  begin
    if Length(AHtmlColor) = 7 then
    begin
      Result := FromArgb(StrToInt('$' + Copy(AHtmlColor, 2, 2)),
        StrToInt('$' + Copy(AHtmlColor, 4, 2)),
        StrToInt('$' + Copy(AHtmlColor, 6, 2)));
    end
    else
    begin
      Result := FromArgb(StrToInt('$' + AHtmlColor[2] + AHtmlColor[2]),
...
class function TdxAlphaColors.ToHexCode(AColor: TdxAlphaColor;
  AUseAlpha: Boolean; APrefix: Char = #0000): string;
begin
  if AUseAlpha then
    Result := Format('%.2x%.2x%.2x%.2x', [Alpha(AColor), R(AColor), G(AColor),
      B(AColor)])
  else
...
class function TdxAlphaColors.ToHtml(AColor: TdxAlphaColor;
  AUseNamedColors: Boolean = True): string;
begin
  Result := '';
  ...
  begin
    ...
    begin
      ...
      else if (AColor = GradientInactiveCaption) and (AColor = InactiveCaption)
      then
...
constructor TdxSimpleColorPalette.Create(AFillColor, AStrokeColor
  : TdxAlphaColor);
begin
...
function TdxAdvancedColorPalette.GetStrokeColor(const ID: string)
  : TdxAlphaColor;
begin
...
procedure TdxAdvancedColorPalette.HandlerKeyChanged(Sender: TObject;
  const Item: string; Action: TCollectionNotification);
begin
  Changed;
end;

procedure TdxAdvancedColorPalette.HandlerValueChanged(Sender: TObject;
  const Item: TdxAlphaColor; Action: TCollectionNotification);
begin
  Changed;
end;

procedure TdxAdvancedColorPalette.SetFillColor(const ID: string;
...
procedure TdxAdvancedColorPalette.SetStrokeColor(const ID: string;
  const Value: TdxAlphaColor);
begin
...
begin
  ...
  FData := TDictionary<TKey, TValue>.Create(ACapacity, FComparer);
...
procedure TdxValueCacheManager<TKey, TValue>.BeforeDestruction;
begin
  inherited;
  Clear;
end;

procedure TdxValueCacheManager<TKey, TValue>.Add(const Key: TKey;
  const Value: TValue);
var
  AItem: TQueueItem;
begin
  if FQueueSize = FCapacity then
    RemoveQueueItem(FQueueFirst);
  FData.Add(Key, Value);

  AItem := TQueueItem.Create;
  AItem.Key := Key;
  AItem.Prev := FQueueLast;

  if FQueueLast <> nil then
    FQueueLast.Next := AItem;
  if FQueueFirst = nil then
    FQueueFirst := AItem;
  FQueueLast := AItem;
  Inc(FQueueSize);
end;
...
begin
  ...
  FData.OnValueNotify := ValueHandler;
  while FQueueFirst <> nil do
    RemoveQueueItem(FQueueFirst);
end;

function TdxValueCacheManager<TKey, TValue>.Get(const Key: TKey;
...
begin
  ...
  while AItem <> nil do
  begin
    if FComparer.Equals(Key, AItem.Key) then
    begin
      RemoveQueueItem(AItem);
      Break;
    end;
    AItem := AItem.Next;
  end;
...
procedure TdxValueCacheManager<TKey, TValue>.RemoveQueueItem(AItem: TQueueItem);
begin
  Dec(FQueueSize);
  if AItem = FQueueFirst then
    FQueueFirst := AItem.Next;
  if AItem = FQueueLast then
    FQueueLast := AItem.Prev;
  if AItem.Prev <> nil then
    AItem.Prev.Next := AItem.Next;
  if AItem.Next <> nil then
    AItem.Next.Prev := AItem.Prev;
  FData.Remove(AItem.Key);
  AItem.Free;
end;

procedure TdxValueCacheManager<TKey, TValue>.ValueHandler(Sender: TObject;
...
class procedure TdxGdiClippingHelper.AddClippingRectangle(DC: HDC;
  const R: TRect);
var
  ARegion: HRGN;
begin
...
class procedure TdxGdiClippingHelper.AddRectangleToRegion(var ARegion: HRGN;
  const R: TRect);
var
  ATempRegion: HRGN;
begin
...
class procedure TdxGdiClippingHelper.ExcludeRectangleFromRegion
  (var ARegion: HRGN; const R: TRect);
var
  ATempRegion: HRGN;
begin
  ATempRegion := CreateRectRgnIndirect(R);
...
begin
  ...
  try
    ...
    try
      ASize := GetCharacterPlacement(ADC, PChar(AText), ALength, 0, AGcpResults,
        GCPFlags[AIsRTLReading]);
      if (ASize = 0) and (ALength > 0) then
        if TryMeasureCharactersSlow(ADC, AText, ALength, AGcpResults,
          GCPFlags[AIsRTLReading]) = 0 then
          Exit(nil);
...
class function TdxRTLReadingCharacterPlacementHelper.TryMeasureCharactersSlow
  (ADC: THandle; const AText: PChar; ALength: Integer;
  var AGcpResults: TGCPResults; AFlags: Integer): Integer;
var
...
begin
  ...
  begin
    ...
    Result := GetCharacterPlacement(ADC, PChar(AText), ALength, 0,
      AGcpResults, AFlags);
    if Result <> 0 then
      Break;
  end;
...
class function TdxRTLReadingCharacterPlacementHelper.CalculateCharactersBounds
  (const AGcpResults: TGCPResults; ALength: Integer; const ABounds: TRect)
  : TArray<TRect>;
var
  I, APrevPos: Integer;
  ADX: PINT;
...
begin
  ...
  begin
    ADisplayOrderBounds[I].InitSize(APrevPos, ABounds.Top, ADX^,
      ABounds.Height);
    Inc(APrevPos, ADX^);
    Inc(ADX);
  end;
...
class function TdxRTLReadingCharacterPlacementHelper.GetSelectionRangeRegion
  (ADC: HDC; const AText: PChar; ALength: Integer; const ABounds: TRect;
  ASelStart, ASelLength: Integer; AFont: HFONT = 0;
  AIsRTLReading: Boolean = False): HRGN;
var
...
begin
  ...
  ACharacterBounds := MeasureCharactersBounds(ADC, AText, ALength, ABounds,
    AFont, AIsRTLReading);
...
class procedure TdxTextMeasurer.Initialize;
{$ENDIF}
begin
...
class constructor TdxTextMeasurer.Finalize;
{$ELSE}

class procedure TdxTextMeasurer.Finalize;
{$ENDIF}
begin
...
class function TdxTextMeasurer.TextLineHeight(AFont: TFont;
  AFontStyle: TFontStyles): Integer;
begin
...
class function TdxTextMeasurer.TextMetrics(AFont: TFont;
  out ATextMetric: TTextMetric): Boolean;
begin
  FMeasureCanvas.SetFont(AFont);
  Result := GetTextMetrics(FMeasureCanvas.Handle, ATextMetric);
end;

class function TdxTextMeasurer.TextSizeTO(AFontHandle: THandle;
...
class function TdxTextMeasurer.TextSizeTO(AFont: TFont; const AText: string;
...
class function TdxTextMeasurer.TextHeightTO(AFont: TFont; const AText: string;
  AFontSize: Integer = 0): Integer;
begin
  Result := TextSizeTO(AFont, AText, AFontSize).cy;
end;

class function TdxTextMeasurer.TextWidthTO(AFont: TFont; const AText: string;
  AFontSize: Integer = 0): Integer;
begin
...
class procedure TdxTextMeasurer.CalcTextRectDT(ADC: HDC; const AText: string;
  var R: TRect; AFlags: DWORD);
begin
  DrawText(ADC, PChar(AText), Length(AText), R, AFlags or DT_CALCRECT);
end;

class function TdxTextMeasurer.TextRectCore(ADC: THandle; const AText: string;
  ARowCount: Integer; ACalcRectFunc: TCalcRectProc;
  ASingleLineFlags, AMultilineFlags: Integer;
  AReturnMaxRectHeight: Boolean): TRect;
...
begin
  ...
  begin
    ...
    begin
      if GetTextSize(AWidth, AMultilineFlags).Bottom > AMaxTextHeight then
        AMinWidth := AWidth
      else
        AMaxWidth := AWidth;
...
class function TdxTextMeasurer.TextHeightDT(AFont: TFont; const AText: string;
  R: TRect; AFlags: Integer): Integer;
begin
  FMeasureCanvas.SetFont(AFont);
  Result := DrawText(FMeasureCanvas.Handle, PChar(AText), Length(AText), R,
    AFlags or DT_CALCRECT);
end;

class function TdxTextMeasurer.TextRectDT(ADC: THandle; const AText: string;
  ARowCount: Integer; AReturnMaxRectHeight: Boolean = False;
  ADTFlags: Integer = 0): TRect;
const
  DT_NOFULLWIDTHCHARBREAK = $80000;
begin
  Result := TextRectCore(ADC, AText, ARowCount, CalcTextRectDT, DT_SINGLELINE or
    ADTFlags, DT_WORDBREAK or DT_NOFULLWIDTHCHARBREAK or ADTFlags,
    AReturnMaxRectHeight);
end;

class function TdxTextMeasurer.TextRectDT(AFont: TFont; const AText: string;
  ARowCount: Integer; AConsiderLineBreaks: Boolean = False): TRect;

  function GetTextRectWithLineBreaks(AFont: TFont; const AText: string;
    ARowCount: Integer): TRect;
  var
    ATextLines: TStrings;
    I: Integer;
    AWidth: Integer;
...
class procedure TdxTextMeasurer.TextRectDT(var ARect: TRect;
  const AText: string; AFont: TFont; ADTFlags: Integer = 0);
begin
  FMeasureCanvas.SetFont(AFont);
  CalcTextRectDT(FMeasureCanvas.Handle, AText, ARect, ADTFlags);
end;

class function TdxTextMeasurer.TextSizeDT(ADC: THandle; const AText: string)
  : TSize; // differs from TextExtent
begin
  Result := TextRectDT(ADC, AText, 1).Size;
end;

class function TdxTextMeasurer.TextSizeDT(AFont: TFont; const AText: string;
  AFlags: Integer = 0): TSize;
var
  ARect: TRect;
begin
  ARect.Empty;
...
class function TdxTextMeasurer.GetStringAdjustedToWidthDT(ADC: HDC;
  AFontHandle: HFONT; const S: string; AWidth: Integer;
  AModifyStringType: TcxModifyStringType): string;
const
  AModifyStringMap: array [TcxModifyStringType] of DWORD = (DT_END_ELLIPSIS,
    DT_PATH_ELLIPSIS);
var
...
begin
  ...
  DrawText(ACalculateDC, @ABuffer[0], -1,
    // D12 DrawText(ACalculateDC, ABuffer, -1,
    R, DT_SINGLELINE or DT_CALCRECT or DT_MODIFYSTRING or
    AModifyStringMap[AModifyStringType]);
  SelectObject(ACalculateDC, AOldFont);
  Result := ABuffer;
end;

class function TdxTextMeasurer.GetStringAdjustedToWidthDT(AFont: TFont;
  const S: string; AWidth: Integer;
  AModifyStringType: TcxModifyStringType): string;
begin
  FMeasureCanvas.SetFont(AFont);
  Result := GetStringAdjustedToWidthDT(0, AFont.Handle, S, AWidth,
    AModifyStringType);
end;
...
TdxTextMeasurer.Initialize;

finalization

TdxAlphaColors.Finalize;
TdxTextMeasurer.Finalize;
