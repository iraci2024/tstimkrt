...
{$ENDIF}
  ...
  TdxDirectedOrientation = (doLeftToRight, doTopToBottom, doBottomToTop,
    doRightToLeft);
  ...
  IdxLocalizerListener = interface
    ['{8412D6F4-1A56-4599-0946-9D4E0E182032}']
  ...
  public
    ...
    class function Clone(const ABytes: TBytes; AMaxLength: Integer)
      : TBytes; overload;
    class function Compare(const AB1, AB2: TBytes): Boolean;
    class function Concatenate(ANum: Integer; const ABytes: TBytes)
      : TBytes; overload;
    class function Concatenate(const AB1: TBytes; const AB2: TBytes)
      : TBytes; overload;
    class function Concatenate(const ABytes: TBytes; ANum: Integer)
      : TBytes; overload;
    class function Resize(const ABytes: TBytes; ATargetSize: Integer;
      AFillBy: Byte = 0): TBytes;
  end;
  ...
  TdxAddResourceStringsProcedure = procedure
  ...
  public
    constructor Create(const AName: string;
      AInitializeProc: TdxAddResourceStringsProcedure); virtual;
  ...
  TdxLocalizationTranslateResStringEvent = procedure(const AResStringName
    : string; AResString: Pointer) of object;
  ...
  public
    ...
    procedure RegisterProduct(const AProductName: string;
      AAddStringsProc: TdxAddResourceStringsProcedure);
    ...
    procedure UnRegisterProduct(const AProductName: string;
      AAddStringsProc: TdxAddResourceStringsProcedure = nil);

    property Products[Index: Integer]: TdxProductResourceStrings
      read GetProducts;
    property ProductsCount: Integer read GetProductsCount;
    property OnTranslateResString: TdxLocalizationTranslateResStringEvent
      read FOnTranslateResString write FOnTranslateResString;
  end;
...
procedure dxCheckOrientation(var AValue: TdxOrientation;
  ADefaultOrientation: TdxOrientation);
...
function dxDefaultBooleanToBoolean(AValue: TdxDefaultBoolean;
...
function dxBinToHex(const ABuffer: PAnsiChar; ABufSize: Integer)
  : AnsiString; overload;
...
function dxStrCopy(ADest: PAnsiChar; const ASource: PAnsiChar)
  : PAnsiChar; inline;
function dxStrLCopy(ADest: PAnsiChar; const ASource: PAnsiChar;
  AMaxLen: Cardinal): PAnsiChar; inline;
function dxStrLen(const AStr: PAnsiChar): Cardinal; inline;

{$IFDEF MSWINDOWS}
function dxGetCodePageFromCharset(ACharSet: Integer): Integer;
function dxGetStringTypeA(Locale: LCID; dwInfoType: DWORD;
  const lpSrcStr: PAnsiChar; cchSrc: Integer; var lpCharType): BOOL;
...
{$ENDIF}
// string conversions
function dxAnsiStringToWideString(const ASource: AnsiString;
  ACodePage: Cardinal = CP_ACP): WideString; inline;
function dxWideStringToAnsiString(const ASource: WideString;
  ACodePage: Cardinal = CP_ACP): AnsiString; inline;

function dxAnsiStringToString(const S: AnsiString;
  ACodePage: Integer = CP_ACP): string;
function dxStringToAnsiString(const S: string; ACodePage: Integer = CP_ACP)
  : AnsiString;
...
function dxStrToFloat(const S: string; const ADecimalSeparator: Char = '.')
  : Extended;
function dxStrToFloatDef(const S: string; const ADecimalSeparator: Char = '.';
  ADefaultValue: Extended = 0): Extended;
function dxFloatToStr(AValue: Extended; ADecimalSeparator: Char = '.'): string;
function RemoveAccelChars(const S: string;
  AAppendTerminatingUnderscore: Boolean = True): string;
{$IFNDEF DELPHIXE2}
...
function dxAnsiStringReplace(const AString, AOldPattern,
  ANewPattern: AnsiString; AFlags: TReplaceFlags): AnsiString;
...
procedure dxCompressStream(ASourceStream, ADestStream: TStream;
  ACompressMethod: Byte; ASize: Integer = 0);
...
procedure cxSetResourceString(AResString: TcxResourceStringID;
  const Value: string);
function cxGetResourceString(const AResString: string): string; overload;
  deprecated;
...
function dxGetSystemResourceString(ALibraryHandle: THandle;
  const AStringID: Integer; const ADefaultValue: string = ''): string;
// for internal use only
function dxGetLocalizedSystemResourceString(const AdxResourceString: string;
  ALibraryHandle: THandle; const AStringID: Integer): string;
// for internal use only
...
{$ENDIF}
function dxExcludeTrailingPathDelimiter(const APath: string;
  const APathDelimiter: Char = PathDelim): string;
function dxExtractFileName(const APath: string;
  const APathDelimiter: Char = PathDelim): string;
function dxIncludeTrailingPathDelimiter(const APath: string;
  const APathDelimiter: Char = PathDelim): string;
function dxIsPathDelimiter(const S: string; Index: Integer;
  const APathDelimiter: Char = PathDelim): Boolean;
function dxReplacePathDelimiter(const APath: AnsiString;
  const AOldDelimiter, ANewDelimiter: AnsiChar): AnsiString; overload;
function dxReplacePathDelimiter(const APath: string;
  const AOldDelimiter, ANewDelimiter: Char): string; overload;
...
procedure cxCopyData(ASource, ADestination: Pointer; ACount: Integer);
  overload; inline;
procedure cxCopyData(ASource, ADestination: Pointer;
  ASourceOffSet, ADestinationOffSet, ACount: Integer); overload; inline;
...
procedure WriteBoolean(ADestination: Pointer; AValue: WordBool;
  AOffset: Integer = 0); inline;
procedure WriteByte(ADestination: Pointer; AValue: Byte;
  AOffset: Integer = 0); inline;
procedure WriteInteger(ADestination: Pointer; AValue: Integer;
  AOffset: Integer = 0); inline;
procedure WritePointer(ADestination: Pointer; AValue: Pointer); inline;
procedure WriteWord(ADestination: Pointer; AValue: Word;
  AOffset: Integer = 0); inline;

function ReadBufferFromStream(AStream: TStream; ABuffer: Pointer;
  Count: Integer): Boolean;
function ReadStringFromStream(AStream: TStream; out AValue: AnsiString)
  : Longint;
function WriteBufferToStream(AStream: TStream; ABuffer: Pointer;
...
function WriteStringToStream(AStream: TStream;
  const AValue: AnsiString): Longint;
...
procedure dxGetLocaleFormatSettings(ALocale: Integer;
  var AFormatSettings: TFormatSettings);
function dxGetLocaleInfo(ALocale, ALocaleType: Integer;
...
function dxShellExecute(AHandle: HWND; const AFileName: string;
  AShowCmd: Integer = SW_SHOWNORMAL): Boolean; overload;
function dxShellExecute(const AFileName: string;
  AShowCmd: Integer = SW_SHOWNORMAL): Boolean; overload;
{$ENDIF}
// compare functions
...
procedure dxFactorizeBuildNumber(ABuildNumber: Cardinal;
  out AMajor, AMinor, ABuild: Integer);
...
type
  TGetUserNameExW = function(NameFormat: DWORD; lpNameBuffer: LPWSTR;
    var nSize: ULONG): BOOL; stdcall;
  TRtlGetVersion = function(var lpVersionInformation: TOSVersionInfoExW)
    : Integer; stdcall;
{$ENDIF}
...
{$ENDIF}
{$IFDEF MSWINDOWS}
function GetStringTypeA(ALocale: Cardinal; dwInfoType: DWORD;
  const lpSrcStr: PAnsiChar; cchSrc: Integer; var lpCharType): BOOL; stdcall;
  external kernel32 name 'GetStringTypeA';
function GetStringTypeW(dwInfoType: DWORD; const lpSrcStr: PWideChar;
  cchSrc: Integer; var lpCharType): BOOL; stdcall;
  external kernel32 name 'GetStringTypeW';
{$ENDIF}
...
var
{$IFNDEF CPUX64}
  AIsWow64Process: function(AHandle: THandle; AWow64Process: PBOOL)
    : BOOL; stdcall;
  AIsWow64: BOOL;
{$ENDIF CPUX64}
  ARtlHandle: THandle;
    ...
    if ARtlHandle > 32 then
      try
        ...
        begin
          ...
          begin
            IsWin10 := (AVersionInfo.dwMajorVersion = 10) and
              (AVersionInfo.dwMinorVersion = 0);
            IsWin10OrLater := IsWin10 or (AVersionInfo.dwMajorVersion > 10);
            IsWin10FallCreatorsUpdate := IsWin10 and
              (AVersionInfo.dwBuildNumber = 16299);
            IsWinSupportsAcrylicEffect := (AVersionInfo.dwMajorVersion > 10) or
              IsWin10 and (AVersionInfo.dwBuildNumber >= 17064);
            IsWin10v1809OrNewer := IsWin10 and
              (AVersionInfo.dwBuildNumber >= 17763);
          end;
        end;
      finally
...
{$IFDEF CPUX64}
  IsWin64Bit := True;
{$ELSE}
  AIsWow64Process := GetProcAddress(GetModuleHandle(kernel32),
    'IsWow64Process');
  if Assigned(AIsWow64Process) and AIsWow64Process(GetCurrentProcess, @AIsWow64)
  then
    IsWin64Bit := AIsWow64
  else
    IsWin64Bit := False;
...
begin
  if TdxNativeUInt(A) < TdxNativeUInt(B) then
    Result := -1
  else if TdxNativeUInt(A) > TdxNativeUInt(B) then
    Result := 1
  else
    Result := 0;
end;
...
function ReadStreamByte(AStream: TStream; AMaxPos: Integer;
  var AByte: Byte): Boolean;
begin
...
function ReadStreamBlock(AStream: TStream; AMaxPos: Integer; var ABlock: TBytes;
...
procedure WriteStreamBlock(AStream: TStream; const ABlock: TBytes;
  ABlockSize: Integer);
var
  I: Integer;
begin
...
procedure dxCompressStream(ASourceStream, ADestStream: TStream;
  ACompressMethod: Byte; ASize: Integer);

  procedure CompressByBlock(ASourceStream, ADestStream: TStream;
    ASize, ABlockSize: Integer);

    function GetCounter(const ASeekBlock: TBytes; AMode: TSeekMode;
    ...
    begin
      ...
      while (Result < 125) and ReadStreamBlock(ASourceStream, AMaxPos, ABlock,
        ABlockSize) do
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        AMode := AModeMap[(AReadCount = ABlockSize) or
...
begin
  ...
  begin
    ...
    if AShift > 0 then
      ADestStream.CopyFrom(ASourceStream, AShift);
    DecompressByBlock(ASourceStream, ADestStream, ASize - 6 - AShift,
      ACompressMethod);
  end
  else
...
function dxIsPathDelimiter(const S: string; Index: Integer;
  const APathDelimiter: Char = PathDelim): Boolean;
begin
  Result := (Index > 0) and (Index <= Length(S)) and (S[Index] = APathDelimiter)
...
function dxReplacePathDelimiter(const APath: AnsiString;
  const AOldDelimiter, ANewDelimiter: AnsiChar): AnsiString;
begin
  if AOldDelimiter <> ANewDelimiter then
    Result := AnsiStrings.StringReplace(APath, AOldDelimiter, ANewDelimiter,
      [rfReplaceAll])
  else
    Result := APath;
end;
...
function dxReplacePathDelimiter(const APath: string;
  const AOldDelimiter, ANewDelimiter: Char): string;
begin
  if AOldDelimiter <> ANewDelimiter then
...
begin
  ...
  if {$IFDEF MSWINDOWS} not IsWinNT and {$ENDIF}
    ...
    Exit(UTF8ToString(S));
{$IFDEF DELPHIXE}
  ADestLength := UnicodeFromLocaleChars(ACodePage, 0, PAnsiChar(S),
    Length(S), nil, 0);
  SetLength(Result, ADestLength);
  UnicodeFromLocaleChars(ACodePage, ConversionFlags[ACodePage = CP_UTF8],
    PAnsiChar(S), Length(S), PWideChar(Result), ADestLength);
{$ELSE}
  ADestLength := MultiByteToWideChar(ACodePage, 0, PAnsiChar(S),
    Length(S), nil, 0);
  SetLength(Result, ADestLength);
...
begin
  ...
  if {$IFDEF MSWINDOWS} not IsWinNT and {$ENDIF}
    ...
    Exit(UTF8Encode(S));
{$IFDEF DELPHIXE}
  ADestLength := LocaleCharsFromUnicode(ACodePage, 0, PWideChar(S), Length(S),
    nil, 0, nil, nil);
  SetLength(Result, ADestLength);
  LocaleCharsFromUnicode(ACodePage, 0, PWideChar(S), Length(S),
    PAnsiChar(Result), ADestLength, nil, nil);
{$ELSE}
  ADestLength := WideCharToMultiByte(ACodePage, 0, PWideChar(S), Length(S), nil,
    0, nil, nil);
  SetLength(Result, ADestLength);
  WideCharToMultiByte(ACodePage, 0, PWideChar(S), Length(S), PAnsiChar(Result),
    ADestLength, nil, nil);
{$ENDIF}
end;
...
procedure TdxResourceStringsRepository.AddListener
  (AListener: IdxLocalizerListener);
begin
  if FListeners.IndexOf(Pointer(AListener)) = -1 then
    FListeners.Add(Pointer(AListener));
end;

procedure TdxResourceStringsRepository.RemoveListener
  (AListener: IdxLocalizerListener);
begin
...
procedure TdxResourceStringsRepository.RegisterProduct(const AProductName
  : string; AAddStringsProc: TdxAddResourceStringsProcedure);
var
  AIndex: Integer;
begin
  ...
  if AIndex >= 0 then
    ...
    FProducts.Add(TdxProductResourceStrings.Create(AProductName,
      AAddStringsProc));
end;

function TdxResourceStringsRepository.GetProductIndexByName
...
function TdxResourceStringsRepository.GetOriginalValue
...
procedure TdxResourceStringsRepository.UnRegisterProduct(const AProductName
  : string; AAddStringsProc: TdxAddResourceStringsProcedure = nil);
var
  AIndex: Integer;
  AProduct: TdxProductResourceStrings;
begin
...
function TdxResourceStringsRepository.GetProducts(AIndex: Integer)
  : TdxProductResourceStrings;
begin
...
begin
  ...
  cxCopyData(ASource.List, ADestination.List, 0, APrevDestinationCount *
    SizeOf(Pointer), ASource.Count * SizeOf(Pointer));
end;
...
begin
  if IsWinXPOrLater then
    ...
    Result := dxMakeLCID(dxMakeLangID(LANG_ENGLISH, SUBLANG_ENGLISH_US),
      SORT_DEFAULT);
end;
...
var
  APrevLocale: Integer;
begin
  if not IsValidLocale(ALocale, LCID_INSTALLED) and
    IsValidLocale(ALocale, LCID_SUPPORTED) then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FGetUserNameExProc := GetProcAddress(LoadLibrary('secur32.dll'),
        'GetUserNameExW');
    end;
    ...
    begin
      ...
      if FGetUserNameExProc(AFormat, @ABuffer[0], ABufferSize) and
        (ABufferSize > 0) and (ABuffer[0] <> #0) then
      begin
...
function dxShellExecute(AHandle: HWND; const AFileName: string;
  AShowCmd: Integer = SW_SHOWNORMAL): Boolean;
begin
  Result := ShellExecute(AHandle, 'open', PChar(AFileName), nil, nil,
    AShowCmd) >= 32;
end;
...
const
  ABoolToDefBool: array [Boolean] of TdxDefaultBoolean = (bFalse, bTrue);
  ADefBoolToBool: array [TdxDefaultBoolean] of Boolean = (False, True, False);

function dxBooleanToDefaultBoolean(AValue: Boolean): TdxDefaultBoolean;
begin
  Result := ABoolToDefBool[AValue];
end;
...
constructor TdxProductResourceStrings.Create(const AName: string;
  AInitializeProc: TdxAddResourceStringsProcedure);
begin
  inherited Create;
  FName := AName;
...
procedure TdxProductResourceStrings.Add(const AResStringName: string;
  AResStringAddr: Pointer);
begin
...
procedure TdxProductResourceStrings.AddProcedure
  (AProc: TdxAddResourceStringsProcedure);
begin
...
procedure TdxProductResourceStrings.RemoveProcedure
  (AProc: TdxAddResourceStringsProcedure);
begin
...
begin
  dxResourceStringsRepository.OnTranslateResString(Names[AIndex],
    FResStringNames.Objects[AIndex]);
end;
...
class function TdxByteArray.Clone(const ABytes: TBytes;
  AMaxLength: Integer): TBytes;
begin
...
class function TdxByteArray.Concatenate(ANum: Integer;
  const ABytes: TBytes): TBytes;
var
  ACountBytes: TBytes;
...
class function TdxByteArray.Concatenate(const ABytes: TBytes;
  ANum: Integer): TBytes;
var
  ACountBytes: TBytes;
  AArrayLength: Integer;
...
class function TdxByteArray.Resize(const ABytes: TBytes; ATargetSize: Integer;
  AFillBy: Byte): TBytes;
begin
...
type
  TcxResourceStringsModificationMode = (rmmByResStringValue, rmmByResStringID,
    rmmUndefined);
...
var
  ...
  FResStringsModificationMode: TcxResourceStringsModificationMode =
    rmmUndefined;
...
begin
  ...
  if (FResStringsModificationMode <> rmmUndefined) and
    ...
    raise EdxException.Create
      ('You cannot mix cxSetResourceString and cxSetResourceStringNet calls');
...
begin
  ...
  begin
    FResOriginalStrings.AddObject(LoadResString(AResString),
      TObject(AResString));
...
begin
{$WARNINGS OFF}
  Result := cxGetResourceString(AdxResourceString);
{$WARNINGS ON}
  if Result = AdxResourceString then
...
function dxStrLCopy(ADest: PAnsiChar; const ASource: PAnsiChar;
  AMaxLen: Cardinal): PAnsiChar;
begin
  Result := {$IFDEF DELPHI18}AnsiStrings.{$ENDIF}StrLCopy(ADest,
    ASource, AMaxLen)
end;
...
function dxAnsiStringToWideString(const ASource: AnsiString;
  ACodePage: Cardinal = CP_ACP): WideString;
begin
  Result := dxAnsiStringToString(ASource, ACodePage);
end;

function dxWideStringToAnsiString(const ASource: WideString;
  ACodePage: Cardinal = CP_ACP): AnsiString;
begin
...
function dxAnsiStringReplace(const AString, AOldPattern,
  ANewPattern: AnsiString; AFlags: TReplaceFlags): AnsiString;
begin
  Result := AnsiStrings.StringReplace(AString, AOldPattern,
    ANewPattern, AFlags);
end;
...
procedure cxCopyData(ASource, ADestination: Pointer;
  ASourceOffSet, ADestinationOffSet, ACount: Integer);
begin
  cxCopyData(PByte(ASource) + ASourceOffSet, PByte(ADestination) +
    ADestinationOffSet, ACount);
end;
