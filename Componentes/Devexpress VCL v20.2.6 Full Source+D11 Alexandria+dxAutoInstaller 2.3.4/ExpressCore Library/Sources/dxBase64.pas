...
type
  ...
  const
    ...
    class function FromPChar(AInputPtr: PChar; AInputLength: Integer)
      : TBytes; static;
  public
    class function CalculateDecodedOutputLength(AInputPtr: PChar;
      AInputLength: Integer): Integer; static;
    class function CalculateEncodedOutputLength(AInputLength: Integer;
      AInsertLineBreaks: Boolean): Integer; static;
    class function EncodeArray(AOutChars: PChar; AInData: PByte;
      AOffset: Integer; ALength: Integer; AInsertLineBreaks: Boolean)
      : Integer; static;
    class function DecodeArray(AStartInputPtr: PChar; AInputLength: Integer;
      AStartDestPtr: PByte; ADestLength: Integer): Integer; static;

    class function FromBase64CharArray(const AInArray: TCharArray;
      AOffset, ALength: Integer): TBytes; static;
    class function FromBase64String(const S: string): TBytes; static;

    class function ToBase64CharArray(const AInArray: TBytes; AOffsetIn: Integer;
      ALength: Integer; const AOutArray: TCharArray; AOffsetOut: Integer)
      : Integer; overload; static;
    class function ToBase64CharArray(const AInArray: TBytes; AOffsetIn: Integer;
      ALength: Integer; const AOutArray: TCharArray; AOffsetOut: Integer;
      AInsertLineBreaks: Boolean): Integer; overload; static;
    class function ToBase64String(const AInArray: TBytes): string;
      overload; static;
    class function ToBase64String(const AInArray: TBytes;
      AInsertLineBreaks: Boolean): string; overload; static;
    class function ToBase64String(const AInArray: TBytes; AOffset: Integer;
      ALength: Integer): string; overload; static;
    class function ToBase64String(const AInArray: TBytes; AOffset: Integer;
      ALength: Integer; AInsertLineBreaks: Boolean): string; overload; static;
  end;
...
class function TdxBase64.CalculateEncodedOutputLength(AInputLength: Integer;
...
class function TdxBase64.EncodeArray(AOutChars: PChar; AInData: PByte;
  AOffset, ALength: Integer; AInsertLineBreaks: Boolean): Integer;
var
...
begin
  ...
  begin
    ...
    Inc(AOutChars);
    AOutChars^ := Base64Table[((AInData[I] and $03) shl 4) or
      ((AInData[I + 1] and $F0) shr 4)];
    Inc(AOutChars);
    AOutChars^ := Base64Table[((AInData[I + 1] and $0F) shl 2) or
      ((AInData[I + 2] and $C0) shr 6)];
    Inc(AOutChars);
  ...
  I := ACalcLength;
  if AInsertLineBreaks and (ALengthMod3 <> 0) and
    (ACharCount = Base64LineBreakPosition) then
  begin
    AOutChars^ := dxCR;
    Inc(AOutChars);
    AOutChars^ := dxLF;
...
class function TdxBase64.FromBase64CharArray(const AInArray: TCharArray;
  AOffset, ALength: Integer): TBytes;
var
  AInArrayPtr: PChar;
begin
...
class function TdxBase64.ToBase64CharArray(const AInArray: TBytes;
  AOffsetIn: Integer; ALength: Integer; const AOutArray: TCharArray;
  AOffsetOut: Integer): Integer;
begin
  Result := ToBase64CharArray(AInArray, AOffsetIn, ALength, AOutArray,
    AOffsetOut, False);
end;

class function TdxBase64.ToBase64CharArray(const AInArray: TBytes;
  AOffsetIn: Integer; ALength: Integer; const AOutArray: TCharArray;
  AOffsetOut: Integer; AInsertLineBreaks: Boolean): Integer;
var
...
begin
  ...
  ANumElementsToCopy := CalculateEncodedOutputLength(ALength,
    AInsertLineBreaks);

  if AOffsetOut > Integer((AOutArrayLength - ANumElementsToCopy)) then
    raise EArgumentOutOfRangeException.Create('offsetOut');
  Result := EncodeArray(@AOutArray[AOffsetOut], @AInArray[0], AOffsetIn,
    ALength, AInsertLineBreaks);
end;
...
class function TdxBase64.ToBase64String(const AInArray: TBytes;
  AInsertLineBreaks: Boolean): string;
begin
  if AInArray = nil then
    raise EdxException.Create('inArray');
...
class function TdxBase64.ToBase64String(const AInArray: TBytes;
  AOffset: Integer; ALength: Integer): string;
begin
  Result := ToBase64String(AInArray, AOffset, ALength, False);
end;

class function TdxBase64.ToBase64String(const AInArray: TBytes;
  AOffset: Integer; ALength: Integer; AInsertLineBreaks: Boolean): string;
var
...
class function TdxBase64.FromPChar(AInputPtr: PChar;
  AInputLength: Integer): TBytes;
var
...
begin
  ...
  AActualResultLength := DecodeArray(AInputPtr, AInputLength,
    Pointer(@Result[0]), AResultLength);
...
class function TdxBase64.DecodeArray(AStartInputPtr: PChar;
  AInputLength: Integer; AStartDestPtr: PByte; ADestLength: Integer): Integer;
const
  Ord_Eq = Ord('=');
  Ord_Plus = Ord('+');
  Ord_Slash = Ord('/');
...
begin
  ...
  begin
    ...
    if ACurrCode - Ord('A') <= Ord_AtoZ then
      Dec(ACurrCode, Ord('A'))
    else if ACurrCode - Ord('a') <= Ord_AtoZ then
      Dec(ACurrCode, Ord('a') - 26)
    else if ACurrCode - Ord('0') <= Ord_0to9 then
...
class function TdxBase64.CalculateDecodedOutputLength(AInputPtr: PChar;
  AInputLength: Integer): Integer;
const
  Ord_Eq = Ord('=');
  Ord_Space = Ord(' ');
var
...
procedure TdxBase64Encoder.Encode(const ABuffer: TBytes; AIndex: Integer;
  ACount: Integer);
var
...
begin
  ...
  begin
    ...
    ALeftOverChars := TdxBase64.ToBase64CharArray(FLeftOverBytes, 0, 3,
      FCharsLine, 0);
  ...
  begin
    ...
    ACharCount := TdxBase64.ToBase64CharArray(ABuffer, AIndex, AChunkSize,
      FCharsLine, 0);
...
begin
  if FLeftOverBytesCount > 0 then
  begin
    ALeftOverChars := TdxBase64.ToBase64CharArray(FLeftOverBytes, 0,
      FLeftOverBytesCount, FCharsLine, 0);
    WriteChars(FCharsLine, 0, ALeftOverChars);
