...
{$IFDEF DELPHI16}
  UITypes,
{$ENDIF}
  Classes, Types, Windows, Graphics, cxGraphics, dxLayoutLookAndFeels,
  dxLayoutContainer,
...
type
  ...
  protected
    ...
    procedure DrawFrame(ACanvas: TcxCanvas); virtual;
    property LayoutLookAndFeel: TdxLayoutWebLookAndFeel
  ...
  TdxLayoutGroupCaptionCxLookAndFeelPainter = class
    (TdxLayoutGroupCaptionPainter)
  strict private
    function GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
  public
    property LookAndFeelPainter: TcxCustomLookAndFeelPainter
...
const
  dxCaptionPositionMap: array [Boolean, TdxLayoutSide]
    of TcxGroupBoxCaptionPosition = ((cxgpCenter, cxgpCenter, cxgpCenter,
    cxgpCenter), (cxgpLeft, cxgpRight, cxgpTop, cxgpBottom));
...
function TdxLayoutGroupStandardPainter.GetCaptionPainterClass
...
begin
  ...
  if cxIsVCLThemesEnabled then
    cxStyleServices.DrawElement(ACanvas.Handle,
      cxStyleServices.GetElementDetails(tbGroupBoxNormal), R)
  else
  begin
...
function TdxLayoutGroupOfficePainter.GetCaptionPainterClass
  : TdxCustomLayoutItemCaptionPainterClass;
begin
...
begin
  R := LayoutLookAndFeel.GetGroupFrameBounds(ViewInfo);
  ACanvas.DrawEdge(R, True, True,
    BordersMap[GroupViewInfo(ViewInfo).IsVerticalCaption]);
  ...
  case GroupViewInfo(ViewInfo).CaptionSide of
    sdLeft:
      begin
        ...
        Dec(Result.Left, LayoutLookAndFeel.GroupOptions.CaptionOptions.
          SeparatorWidth);
      end;
    sdRight:
      begin
        Result.Left := Result.Right;
        Inc(Result.Right, LayoutLookAndFeel.GroupOptions.CaptionOptions.
          SeparatorWidth);
      end;
    sdTop:
      begin
        Result.Bottom := Result.Top;
        Dec(Result.Top, LayoutLookAndFeel.GroupOptions.CaptionOptions.
          SeparatorWidth);
      end;
    sdBottom:
      begin
        Result.Top := Result.Bottom;
        Inc(Result.Bottom, LayoutLookAndFeel.GroupOptions.CaptionOptions.
          SeparatorWidth);
      end;
  end;
end;
...
begin
  ...
  if not LayoutLookAndFeel.GroupOptions.OffsetCaption and
    not LayoutLookAndFeel.GroupOptions.OffsetItems and
    (LayoutLookAndFeel.GroupOptions.FrameWidth = 0) and
    (ViewInfo.CaptionViewInfo.Color = ViewInfo.Color) then
  begin
...
begin
  ...
  Result := AGroupViewInfo.IsExpanded and
    LayoutLookAndFeel.GroupOptions.HasCaptionSeparator
    (AGroupViewInfo.HasCaption or AGroupViewInfo.HasButtons);
end;
...
begin
  ...
  begin
    ...
    R := cxRectInflate(ViewInfo.Bounds,
      -LayoutLookAndFeel.GroupOptions.FrameWidth,
      -LayoutLookAndFeel.GroupOptions.FrameWidth);
    case GroupViewInfo(ViewInfo).CaptionSide of
      sdLeft:
        R.Right := R.Left + Max(ViewInfo.CaptionViewInfo.CalculateWidth,
          ViewInfo.ButtonsViewInfo.CalculateWidth);
      sdRight:
        R.Left := R.Right - Max(ViewInfo.CaptionViewInfo.CalculateWidth,
          ViewInfo.ButtonsViewInfo.CalculateWidth);
      sdTop:
        R.Bottom := R.Top + Max(ViewInfo.CaptionViewInfo.CalculateHeight,
          ViewInfo.ButtonsViewInfo.CalculateHeight);
      sdBottom:
        R.Top := R.Bottom - Max(ViewInfo.CaptionViewInfo.CalculateHeight,
          ViewInfo.ButtonsViewInfo.CalculateHeight);
    end;
    ACanvas.FillRect(R);
  end;
...
procedure TdxLayoutGroupCaptionStandardPainter.DoDrawText(ACanvas: TcxCanvas);
  ...
  const
    AElementMap: array [Boolean] of TThemedButton = (tbGroupBoxNormal,
      tbGroupBoxDisabled);
  var
  ...
  begin
    ...
    R := ARect;
    cxStyleServices.DrawText(ADC, cxStyleServices.GetElementDetails
      (AElementMap[ViewInfo.Enabled]), TdxCustomLayoutItemCaptionViewInfoAccess
      (ViewInfo).FormattedText.GetDisplayText, R, AFlags{$IFNDEF DELPHI16},
      0{$ENDIF});
  end;
    ...
    begin
      ...
      try
        ABitmap.cxCanvas.FillRect(ABitmap.ClientRect,
          TdxCustomLayoutItemViewInfoAccess(TdxLayoutGroupCaptionViewInfoAccess
          (ViewInfo).ItemViewInfo).GetBackgroundColor);
        TdxLayoutGroupCaptionViewInfoAccess(ViewInfo)
          .PrepareCanvas(ABitmap.cxCanvas, True);
        DrawThemeText(ABitmap.cxCanvas.Handle, ABitmap.ClientRect);
        ABitmap.Rotate(TdxLayoutGroupCaptionViewInfoAccess(ViewInfo)
          .GetRotationAngle);
        cxDrawImage(ACanvas.Handle, ATextRect, ATextRect, ABitmap, nil, -1,
          idmNormal);
      finally
        ABitmap.Free;
      end;
...
function TdxLayoutGroupCaptionCxLookAndFeelPainter.GetLookAndFeelPainter
...
begin
  ...
  ACaptionPosition := dxCaptionPositionMap[AHasCaption,
    AGroupViewInfo.CaptionSide];
  LookAndFeelPainter.DrawGroupBoxFrame(ACanvas, AFrameBounds, ViewInfo.Enabled,
    ACaptionPosition);
  ...
  begin
    ...
    LookAndFeelPainter.DrawGroupBoxCaption(ACanvas, ACaptionRect, cxNullRect,
      ACaptionPosition);

    if LookAndFeelPainter.IsGroupBoxCaptionTextDrawnOverBorder(ACaptionPosition)
    then
    begin
      ACanvas.SaveClipRegion;
      try
        ACanvas.SetClipRegion
          (TcxRegion.Create(ViewInfo.CaptionViewInfo.Bounds), roSet);
        ACanvas.SetClipRegion
          (TcxRegion.Create(ViewInfo.ButtonsViewInfo.Bounds), roAdd);
        ACaptionRect := cxRectUnion(ViewInfo.CaptionViewInfo.Bounds,
          ViewInfo.ButtonsViewInfo.Bounds);
        LookAndFeelPainter.DrawGroupBoxCaption(ACanvas, ACaptionRect,
          ACaptionRect, ACaptionPosition);
      finally
...
begin
  AGroupViewInfo := GroupViewInfo(ViewInfo);
  Result := not AGroupViewInfo.IsSkinPainterUsed and
    not(AGroupViewInfo.IsSkinPainterUsed and AGroupViewInfo.Group.IsRoot) and
    inherited CanDrawBackground;
end;

function TdxLayoutGroupCxLookAndFeelPainter.GetCaptionPainterClass
  : TdxCustomLayoutItemCaptionPainterClass;
begin
...
function TdxLayoutGroupCxLookAndFeelPainter.GetLookAndFeelPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := TdxLayoutCxLookAndFeel(LayoutLookAndFeel).LookAndFeelPainter;
end;
