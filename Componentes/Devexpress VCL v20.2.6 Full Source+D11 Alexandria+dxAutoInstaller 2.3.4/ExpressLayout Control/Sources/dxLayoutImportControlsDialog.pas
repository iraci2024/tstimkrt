...
uses
  ...
  Types, cxLookAndFeels, cxSplitter, Menus, cxDropDownEdit, cxCheckBox,
  cxButtons,
...
type
  ...
  public
    ...
    property LayoutHorizontalCenter: Integer read FLayoutHorizontalCenter
      write FLayoutHorizontalCenter;
  end;
  ...
  private
    ...
    procedure PopulateItemsAtRect(AItems: TdxAutoLayoutItemList;
      const R: TRect);
    procedure ProcessLabels;
    function UpdateUtmostItems: TRect;
  protected
    procedure CheckRootDirection(AItems: TdxAutoLayoutItemList;
    ...
    procedure Initialize(AContainer: TWinControl;
      ARoot: TdxLayoutGroup); virtual;
  ...
  protected
    ...
    procedure DoExecute(AContainer: TWinControl;
      ARecursive: Boolean = True); virtual;
  ...
  public
    ...
    property ConvertCheckBoxes: Boolean read FConvertCheckBoxes
      write FConvertCheckBoxes default False;
    property ConvertPageControls: Boolean read FConvertPageControls
      write FConvertPageControls default True;
    property ConvertRadioButtons: Boolean read FConvertRadioButtons
      write FConvertRadioButtons default False;
    property FocusControlDistanceX: Byte read FFocusControlDistanceX
      write FFocusControlDistanceX default 48;
    property FocusControlDistanceY: Byte read FFocusControlDistanceY
      write FFocusControlDistanceY default 16;
    property SmartFindFocusControl: Boolean read FSmartFindFocusControl
      write FSmartFindFocusControl default True;
    property UseLabeledItems: Boolean read FUseLabeledItems
      write FUseLabeledItems default True;
  end;
  ...
  public
    ...
    procedure PopulateContainersList(ALayoutControl: TdxCustomLayoutControl;
      AParentContainer: TWinControl);
    function CanExport(ALayoutControl: TdxCustomLayoutControl;
      AContainer: TWinControl): Boolean;
  end;

procedure ImportLayout(ALayoutControl: TdxLayoutControl;
  ADefaultImport: Boolean = False); overload;

implementation

uses
  dxCore, cxGeometry, Math, dxLayoutConvertControlsDialog, dxBevel;
...
begin
  Result := TdxAutoLayoutItem(Item1).BoundsRect.Top - TdxAutoLayoutItem(Item2)
    .BoundsRect.Top;
end;

function CompareBottom(Item1, Item2: Pointer): Integer;
begin
  Result := TdxAutoLayoutItem(Item1).BoundsRect.Bottom -
    TdxAutoLayoutItem(Item2).BoundsRect.Bottom;
end;

function CompareLeft(Item1, Item2: Pointer): Integer;
begin
  Result := TdxAutoLayoutItem(Item1).BoundsRect.Left - TdxAutoLayoutItem(Item2)
    .BoundsRect.Left;
end;

function CompareRight(Item1, Item2: Pointer): Integer;
begin
  Result := TdxAutoLayoutItem(Item1).BoundsRect.Right - TdxAutoLayoutItem(Item2)
    .BoundsRect.Right;
end;
...
function TdxSideItemList.Add(AItems: TdxAutoLayoutItemList; ASide: TcxBorder)
  : TdxSideItem;
begin
...
procedure TdxSideItemList.BuildLayout(out APreviousGroupBounds: TRect);
  ...
  begin
    ...
    begin
      ...
      begin
        Helper.ConvertGroup(Items[I].Items, Items[I].Side, APreviousGroupBounds,
          AGroupBounds);
...
constructor TdxAutoLayoutItem.Create(AAutoLayout: TdxAutoLayout;
  AControl: TControl);
begin
...
function TdxAutoLayoutItem.CreateItem(AGroup: TdxLayoutGroup)
  : TdxCustomLayoutItem;
begin
  Result := AGroup.CreateItemForControl(Control);
  if (FAutoLayout.ConvertCheckBoxes and ((Control is TCheckBox) or
    (Control is TcxCheckBox))) or (FAutoLayout.UseLabeledItems and
    IsStandaloneLabel and ((Control is TLabel) or (Control is TcxLabel))) or
    (FAutoLayout.ConvertRadioButtons and (Control is TRadioButton)) then
    Result := TdxLayoutControlEmbeddedControlItemConversionHelper.Convert
      (TdxLayoutItem(Result));
end;

function TdxAutoLayoutItem.CreateSeparator(AGroup: TdxLayoutGroup)
  : TdxCustomLayoutItem;
begin
  Result := AGroup.CreateItem(TdxLayoutSeparatorItem);
end;

function TdxAutoLayoutItem.CreateSplitter(AGroup: TdxLayoutGroup)
  : TdxCustomLayoutItem;
begin
  Result := AGroup.CreateItem(TdxLayoutSplitterItem);
end;

function TdxAutoLayoutItem.CreateTabbedGroup(AGroup: TdxLayoutGroup)
  : TdxCustomLayoutItem;
var
...
begin
  ...
  begin
    ...
    else if Control is TcxPageControl then
    begin
...
begin
  Result := ((Control is TCustomGroupBox) or (Control is TCustomPanel) or
    (Control is TcxCustomGroupBox)) and not(Control is TcxCustomButtonGroup);
  // and (TWinControl(Control).ControlCount > 0);
end;
...
begin
  Result := ((Control is TBevel) and (TBevel(Control).Shape in [bsTopLine,
    bsBottomLine, bsLeftLine, bsRightLine])) or
    ((Control is TdxBevel) and (TdxBevel(Control).Shape in [dxbsLineTop,
    dxbsLineBottom, dxbsLineLeft, dxbsLineRight, dxbsLineCenteredHorz,
...
procedure TdxAutoLayoutItem.SetLabelInfo(ALabel: TControl);
  ...
  begin
    if ALabel.BoundsRect.Right <= Control.BoundsRect.Left then
      Result := clLeft
    else if ALabel.BoundsRect.Left >= Control.BoundsRect.Right then
      Result := clRight
    else if ALabel.BoundsRect.Bottom <= Control.BoundsRect.Top then
      Result := clTop
    else
      Result := clBottom;
  end;
  ...
  begin
    ...
    FLabeledBoundsRect.Bottom := Max(FBoundsRect.Bottom,
      ALabel.BoundsRect.Bottom);
  end;
...
begin
  ALayoutItem := Safe<TdxLayoutItem>.Cast(LayoutControlItem);
...
begin
  Result := not(csAncestor in FControl.ComponentState) and
    (IsGroup or IsSplitter or IsSeparator or
    (IsPageControl and FAutoLayout.ConvertPageControls));
end;

procedure TdxAutoLayoutItem.SetAlignments;
begin
  if LayoutControlItem.IsRoot then
    Exit;
  if Abs(LayoutHorizontalCenter - cxRectCenter(Control.BoundsRect).X) < CenterAlignmentAccuracy
  then
    LayoutControlItem.AlignHorz := ahCenter
  else
    LayoutControlItem.AlignHorz := AlignHorzs[akLeft in FAnchors,
      akRight in FAnchors];
  LayoutControlItem.AlignVert := AlignVerts[akTop in FAnchors,
    akBottom in FAnchors];
end;
...
function TdxAutoLayoutOwnedItemList.AddAlignedItems(AItem: TdxAutoLayoutItem)
  : TdxAutoLayoutItem;
begin
...
procedure TdxAutoLayoutHelper.CheckRootDirection(AItems: TdxAutoLayoutItemList;
  ADirection: TdxLayoutDirection);
...
procedure TdxAutoLayoutHelper.ConvertGroup(AItems: TdxAutoLayoutItemList;
  ...
  begin
    ...
    if AGroup.LayoutDirection = ldVertical then
      for I := 0 to AItems.Count - 1 do
        if (Abs(AItems[I].Control.Left - ABounds.Left) >
          BorderAlignmentAccuracy) and
          (Abs(AItems[I].Control.BoundsRect.Right - ABounds.Right) >
          BorderAlignmentAccuracy) then
          AItems[I].LayoutHorizontalCenter := ACenter;
  end;

  procedure AddItems;
  const
    Directions: array [TcxBorder] of TdxLayoutDirection = (ldVertical,
      ldHorizontal, ldVertical, ldHorizontal);
  var
    I: Integer;
  ...
  begin
    ...
    if (APreviousGroupBounds.Left <> AGroupBounds.Right) and
      (Abs(AGroupBounds.Left - APreviousGroupBounds.Left) >
      BorderAlignmentAccuracy) and
      (Abs(AGroupBounds.Right - APreviousGroupBounds.Right) >
      BorderAlignmentAccuracy) and
      (Abs(cxRectCenter(AGroupBounds).X - cxRectCenter(APreviousGroupBounds).X)
      < CenterAlignmentAccuracy) then
...
begin
  ...
  try
    ...
    if L.Count > 0 then
      case AAlign of
        alTop:
          FNonAlignedBoundsRect.Top := L.Last.BoundsRect.Bottom;
        alBottom:
          FNonAlignedBoundsRect.Bottom := L.First.BoundsRect.Top;
        alLeft:
          FNonAlignedBoundsRect.Left := L.Last.BoundsRect.Right;
        alRight:
          FNonAlignedBoundsRect.Right := L.First.BoundsRect.Left;
      end;
...
var
  ...
  begin
    Result := 0;
    if not(ASide in ATestSides) then
      Exit;
  ...
  procedure ProcessUngroupedItems(ASide: TcxBorder;
    const APreviousGroupBounds: TRect);
  var
  ...
  begin
    ...
    begin
      case ASide of
        ...
        Result := cxRect(Left + 1, Top,
          FRightmostItem.BoundsRect.Right, Bottom);
      end;
    end;
  end;
...
function TdxAutoLayoutHelper.GetFocusControl(AItem: TdxAutoLayoutItem)
  : TWinControl;
var
  I, X, Y, DX, DY: Integer;
...
function TdxAutoLayoutHelper.NewGroup(AItems: TdxAutoLayoutItemList)
  : TdxLayoutGroup;
var
...
procedure TdxAutoLayout.DoExecute(AContainer: TWinControl;
...
procedure TdxAutoLayout.Execute(AContainer: TWinControl;
  ARecursive: Boolean = True);
begin
...
procedure TdxAutoLayout.CorrectAlign(AGroup: TdxLayoutGroup);
  ...
  begin
    if (AItem.AlignHorz = TdxCustomLayoutItemAccess(AItem)
      .GetParentManagedAlignHorz) and not((AItem.AlignHorz = ahClient) and
      (AItem.Parent.LayoutDirection = ldVertical)) then
      AItem.AlignHorz := ahParentManaged;
    if (AItem.AlignVert = TdxCustomLayoutItemAccess(AItem)
      .GetParentManagedAlignVert) and not((AItem.AlignVert = avClient) and
      (AItem.Parent.LayoutDirection = ldHorizontal)) then
...
procedure ImportLayout(ALayoutControl: TdxLayoutControl;
  AContainer: TWinControl; AConvertPageControls, ASmartFindFocusControl,
  AUseLabeledItems, AConvertCheckBoxes, AConvertRadioButtons: Boolean;
  ...
  try
    ...
    if ADefaultImport or (AImportForm.ShowModal = mrOk) then
      ImportLayout(ALayoutControl, AImportForm.GetImportContainer,
        AImportForm.cbConvertPageControls.Checked,
        AImportForm.cbAssociate.Checked, AImportForm.cbUseLabeledItems.Checked,
        AImportForm.cbConvertCheckBoxes.Checked,
        AImportForm.cbConvertRadioButtons.Checked, AImportForm.seDeltaX.Value,
        AImportForm.seDeltaY.Value);
  finally
    AImportForm.Free;
  end;
end;
...
begin
  if (cbContainers.ItemIndex >= 0) and
    (cbContainers.ItemIndex < cbContainers.Properties.Items.Count) then
    Result := cbContainers.Properties.Items.Objects[cbContainers.ItemIndex]
      as TWinControl
  else
    Result := nil;
end;

procedure TfmImportControlsDialog.PopulateContainersList(ALayoutControl
  : TdxCustomLayoutControl; AParentContainer: TWinControl);

  function InternalPopulateContainersList(AParentContainer: TWinControl;
    AList: TStrings; AInsertionIndex: Integer): Boolean;
  var
    I: Integer;
  begin
    Result := CanExport(ALayoutControl, AParentContainer) and
      ((csAcceptsControls in AParentContainer.ControlStyle) or
      (AParentContainer is TPageControl));
    if Result then
    begin
      AList.InsertObject(AInsertionIndex, AParentContainer.Name,
        AParentContainer);
      ...
      for I := 0 to AParentContainer.ControlCount - 1 do
        if AParentContainer.Controls[I] is TWinControl then
          if InternalPopulateContainersList
            (TWinControl(AParentContainer.Controls[I]), AList, AInsertionIndex)
          then
            Inc(AInsertionIndex);
    end;
  end;
...
begin
  InternalPopulateContainersList(AParentContainer,
    cbContainers.Properties.Items, 0);
...
function TfmImportControlsDialog.CanExport(ALayoutControl
  : TdxCustomLayoutControl; AContainer: TWinControl): Boolean;
begin
  Result := (AContainer <> ALayoutControl) and
    ((ALayoutControl.Parent = AContainer) or not IsParent(AContainer,
    ALayoutControl)) and not(csNoDesignVisible in AContainer.ControlStyle) and
    not(csSubComponent in AContainer.ComponentStyle);
end;
