...
type
  ...
  TdxDbOrgChart = class;
  TdxOcNewKeyEvent = procedure(Sender: TObject; MaxValue: Variant;
  ...
  protected
    ...
    function InternalAdd(AParent: TdxOcNode; Data: TdxOcNodeData; Idx: Integer)
      : TdxOcNode; override;
  ...
  public
    ...
    property WidthFieldName: String read FWidthFieldName
      write SetWidthFieldName;
    property HeightFieldName: String read FHeightFieldName
      write SetHeightFieldName;
    property ColorFieldName: String read FColorFieldName
      write SetColorFieldName;
    property ShapeFieldName: String read FShapeFieldName
      write SetShapeFieldName;
    property ChAlignFieldName: String read FChAlignFieldName
      write SetChAlignFieldName;
    property ImAlignFieldName: String read FImAlignFieldName
      write SetImAlignFieldName;
  published
    property Anchors;
    ...
    property OrderFieldName: String read FOrderFieldName
      write SetOrderFieldName;
    property ImageFieldName: String read FImIndexFieldName
      write SetImIndexFieldName;
...
begin
  try
    if V1 = V2 then
      Result := 0
    else if VarIsNull(V1) then
      Result := -1
    else if VarIsNull(V2) then
      Result := 1
    else if V1 < V2 then
      Result := -1
    else
...
begin
  Result := (AField <> nil) and
    (AField.DataType in [ftInteger, ftSmallint, ftWord, ftAutoInc, ftLargeint,
...
begin
  if (FFiltered <> DataSet.Filtered) or (FFilter <> DataSet.Filter) then
    FTree.ActiveChanged
  else
    FTree.DataChanged;
end;
...
begin
  ...
  then
    SetLength(TheText, DbOwner.TextField.Size);
...
begin
  if ABookmark <> FCurRec then
    FCurRec := FCurRec + DataSet.MoveBy(ABookmark - FCurRec);
end;

procedure TdxDbOrgChart.ShowEditor;
begin
  if Active and DataSet.CanModify then
...
procedure TdxDbOrgChart.AssignFields;
  ...
  begin
    if FName <> '' then
      Field := DataSet.FindField(FName);
  end;

  procedure ChkInt(var Field: TField);
  begin
    if not IsIntegerField(Field) then
      Field := nil;
  end;
...
begin
  if KeyField.IsNull then
    Exit;
  if VarIsNull(FMaxKey) or VarGT(KeyField.Value, FMaxKey) then
    FMaxKey := KeyField.Value;
  ParKey := ParentField.Value;
  if VarEQ(ParKey, KeyField.Value) then
    ParKey := Null;
  Node := GetNodeByKey(KeyField.Value);
  Par := GetNodeByKey(ParKey);
  if Node = nil then
    Node := CreateDBNode(KeyField.Value, Par);
  if Node = nil then
    Exit;
  if VarIsNull(Node.ParentKey) then
    Node.FParentKey := ParKey;
  if Node.Parent <> Par then
    MoveDBNode(Node, Par);
  with Node do
  begin
...
begin
  ...
  begin
    ...
    if VarEQ(AKey, CKey) then
    begin
      Result := True;
      Exit;
    end;
    if VarGT(AKey, CKey) then
      Min := AIndex + 1
    else
      Max := AIndex;
  end;
...
var
  I: Integer;
begin
  if FindNearest(AKey, I) then
    Result := TdxDbOcNode(FKeyList[I])
  else
    Result := nil;
end;
...
begin
  ...
  if AParent = nil then
    ...
    Node := TdxDbOcNode(AParent.GetFirstChild);
  while Node <> nil do
  begin
    if not VarGT(AKey, Node.FOrder) then
      Exit;
    Inc(Result);
...
function TdxDbOrgChart.CreateDBNode(AKey: Variant; AParent: TdxDbOcNode)
  : TdxDbOcNode;
var
  Order: Variant;
begin
...
begin
  if (AParent = ANode) or (AParent <> nil) and AParent.HasAsParent(ANode) then
    Exit;
...
function TdxDbOrgChart.InternalAdd(AParent: TdxOcNode; Data: TdxOcNodeData;
  Idx: Integer): TdxOcNode;

  function IsAutoIncField(AField: TField): Boolean;
  begin
    Result := (AField is TAutoIncField) or
      (AField.AutoGenerateValue = arAutoInc);
  end;
...
var
  ...
  Bm: Integer;
begin
  if FEnableDB and not DataSet.CanModify then
    Exit;
  inherited InternalMoveTo(AParent, ANode, Idx);
  Node := TdxDbOcNode(ANode);
  if Node.Parent = nil then
    Node.FParentKey := Null
  else
    Node.FParentKey := TdxDbOcNode(Node.Parent).Key;
  if FEnableDB then
  begin
    Bm := FCurRec;
    FEnableDB := False;
...
var
  ...
  Loc: Boolean;
begin
  if not KeyOrder and (OrderField = nil) then
    Exit;
  Node := TdxDbOcNode(ANode.getNextSibling);
  if Node = nil then
    Exit;
  if VarGE(ANode.FOrder, Node.FOrder) then
  begin
    Loc := LocateToNode(Node);
    if OrderField <> nil then
      Node.FOrder := ANode.FOrder + 1
    else
    begin
      Node.FKey := NewKey;
    ...
    begin
      ...
      begin
        ...
        if LocateToNode(TdxDbOcNode(Node[I])) then
        begin
          DataSet.Edit;
...
begin
  ...
  begin
    Node := TdxDbOcNode(FKeyList[I]);
    if Node.FRecNum > OldNum then
      Dec(Node.FRecNum);
    if Node.FRecNum >= NewNum then
      Inc(Node.FRecNum);
  end;
...
begin
  Result := Null;
  if Assigned(OnNewKey) then
  begin
    OnNewKey(Self, FMaxKey, Result);
    if not VarGT(Result, FMaxKey) then
      Result := Null;
  end;
  if VarIsNull(Result) and IsIntegerField(KeyField) then
    if VarIsNull(FMaxKey) then
      Result := 0
    else
      Result := FMaxKey + 1;
  if not VarIsNull(Result) then
    ...
    raise EVariantError.Create('Cannot create new key.');
end;
...
begin
  ...
  begin
    ...
    Bm := FCurRec;
    try
      if LocateToNode(TdxDbOcNode(ANode)) then
      begin
        DataSet.Delete;
        Dec(FRecCount);
        if FCurRec > FRecCount then
          FCurRec := FRecCount;
        if Bm > FCurRec then
          Dec(Bm);
        Node := TdxDbOcNode(GetFirstNode);
        while Node <> nil do
        begin
          if Node.FRecNum > FCurRec then
            Dec(Node.FRecNum);
          if (Node = ANode.Parent) and Node.Deleting then
            Node := TdxDbOcNode(ANode);
  ...
  if FKeyList <> nil then
    DelFromKeyList(TdxDbOcNode(ANode));
...
var
  Sel: TdxDbOcNode;
begin
  if FEnableDB and (KeyField <> nil) then
  begin
    FEnableDB := False;
    FCurRec := FCurRec + Dist;
    Sel := GetNodeByKey(KeyField.Value);
    if (Sel <> nil) and not VarEQ(KeyField.Value, Sel.Key) then
      Sel := nil;
    Selected := Sel;
    if Sel <> nil then
      Sel.MakeVisible;
...
begin
  if FEnableDB and (Selected <> nil) and (Field <> nil) and
    (DataSet.State = dsEdit) then
  begin
    FEnableDB := False;
    if Field = TextField then
      Selected.Text := TextField.AsString;
    if Field = ImageField then
      Selected.ImageIndex := ImageField.AsInteger;
    FEnableDB := True;
  end;
end;
...
var
  ...
  Info: TdxOcNodeInfo;
begin
  if FEnableDB and DataSet.CanModify then
  begin
    FEnableDB := False;
    Bm := FCurRec;
    try
      ...
      begin
        ...
        DataSet.Edit;
        if TextField <> nil then
          TextField.AsString := ANode.Text;
        if WidthField <> nil then
          WidthField.AsInteger := Info.Width;
        if HeightField <> nil then
          HeightField.AsInteger := Info.Height;
        if ColorField <> nil then
          ColorField.AsInteger := Info.Color;
        if ShapeField <> nil then
          ShapeField.AsInteger := Ord(Info.Shape);
        if ChAlignField <> nil then
          ChAlignField.AsInteger := Ord(Info.Align);
        if ImAlignField <> nil then
          ImAlignField.AsInteger := Ord(Info.IAlign);
        if ImageField <> nil then
          ImageField.AsInteger := Info.Index;
        DataSet.Post;
...
begin
  if not FEnableDB or (DataSet.State in [dsEdit, dsInsert]) then
    Exit;
  RefreshItems;
  DoScroll(0);
end;
...
begin
  if FEnableDB and (Node <> nil) then
  begin
    FEnableDB := False;
    try
...
begin
  ...
  if Allow and FEnableDB and (Node <> nil) then
  begin
    Bm := FCurRec;
    FEnableDB := False;
    try
...
begin
  if FHeightFieldName <> Value then
  begin
...
begin
  if FChAlignFieldName <> Value then
  begin
...
begin
  if FImIndexFieldName <> Value then
  begin
...
begin
  if FImAlignFieldName <> Value then
  begin
...
begin
  ...
  if TextField <> nil then
    Result.MaxLength := TextField.Size;
end;
