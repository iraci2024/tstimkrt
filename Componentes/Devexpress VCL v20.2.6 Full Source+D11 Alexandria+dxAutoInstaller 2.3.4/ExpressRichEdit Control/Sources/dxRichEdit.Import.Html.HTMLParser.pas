...
type
  ...
  TdxHtmlElementType = (Content, OpenTag, CloseTag, EmptyTag, Comment);
  ...
  TdxHtmlTagNameID = (Unknown, LI, TD, TR, TH, Table, NumberingList, BulletList,
    Style, Abbr, Acronym, Address, Area, BaseFont, Bdo, BgSound, Button, Cite,
    Dd, Del, Dfn, Dl, Dt, Embed, Fieldset, Form, Frame, FrameSet, Hr, Iframe,
    Input, Ins, Kbd, &Label, Legend, Map, Nobr, Noembed, NoFrames, NoScript,
    &Object, OptGroup, Option, Param, Q, Samp, Select, TextArea, TT, &Var, Wbr,
    Xmp, Html, Head, Base, Meta, Title, Link, Anchor, Body, Bold, Italic,
    Underline, Paragraph, Strong, Big, Small, Preformatted, Font, LineBreak,
    Emphasized, Img, Heading1, Heading2, Heading3, Heading4, Heading5, Heading6,
    SuperScript, SubScript, Center, S, Strike, Code, Span, &Div, Script,
    Blockquote, Caption, Thead, Tfoot, Tbody, Col, ColGroup, Namespace);
  ...
  TdxState = (StartState, ReadContent, ReadTag, ReadOpenTag, ReadEmptyTag,
    ReadComment, ReadCommentContent, ReadTagName, ReadAttributeName,
    WaitAttributeName, WaitAttrNameOrEqualSymbol, WaitAttributeValue,
    ReadAttributeValue, ReadValueInQuotes, ReadValueInApostrophe, EndTag,
    StopReading);
  ...
  TdxHtmlParser = class
  strict private
    class var FHtmlTagNameIDTable: TdxHtmlTagNameIDTable;
  ...
  protected
    ...
    function CreateCommentElement(const AComment: string)
      : TdxHtmlElement; virtual;
    function CreateTagElement(ATag: TdxTag): TdxHtmlElement; overload; virtual;
    function ReplaceSpecialSymbolCore(const ARawText: string;
      AStartIndex: Integer; AIsPrevWhiteSpace: Boolean): string;
    ...
    property ElementType: TdxHtmlElementType read FElementType
      write FElementType;
    ...
    property IsEncodingDetectionBufferFull: Boolean
      read GetIsEncodingDetectionBufferFull;
    property HtmlTagNameIDTable: TdxHtmlTagNameIDTable
      read GetHtmlTagNameIDTable;
  public
    constructor Create;
    destructor Destroy; override;
    function GetTagNameID(const AName: string): TdxHtmlTagNameID;
    procedure ParseNextScript(AStream: TStreamReader;
      out AScriptContent, ACloseScriptTag: TdxHtmlElement);
    procedure ParseScriptContent(AStream: TStreamReader;
      out AScriptContent, ACloseScriptTag: TdxHtmlElement);
    function TryParseCloseScriptTag(AStream: TStreamReader;
      out AElement: TdxHtmlElement): Boolean;
...
procedure TdxHtmlParser.ParseNextScript(AStream: TStreamReader;
  out AScriptContent, ACloseScriptTag: TdxHtmlElement);
begin
  ParseScriptContent(AStream, AScriptContent, ACloseScriptTag);
end;

procedure TdxHtmlParser.ParseScriptContent(AStream: TStreamReader;
  out AScriptContent, ACloseScriptTag: TdxHtmlElement);
var
...
function TdxHtmlParser.TryParseCloseScriptTag(AStream: TStreamReader;
  out AElement: TdxHtmlElement): Boolean;
var
  ATag: TdxTag;
begin
...
begin
  case ACh of
    ...
    if not {$IFDEF DELPHIXE4}ACh.IsLetter{$ELSE}TCharacter.IsLetter
      (ACh){$ENDIF} and (ACh <> '?') then
    begin
      ParserState.ChangeState(TdxState.ReadContent);
      ToggleReadContentState(ACh);
    end
...
begin
  ...
  if ACh = '<' then
    EndContentElement
  else if {$IFDEF DELPHIXE4}ACh.IsWhiteSpace{$ELSE}TCharacter.IsWhiteSpace
    (ACh){$ENDIF} then
  begin
...
begin
  if IsTagClosed(ACh) then
    Exit;
  if {$IFDEF DELPHIXE4}ACh.IsWhiteSpace{$ELSE}TCharacter.IsWhiteSpace
    (ACh){$ENDIF} then
    Exit;
  if ACh = '<' then
  begin
...
function TdxHtmlParser.CreateCommentElement(const AComment: string)
  : TdxHtmlElement;
var
  AElement: TdxComment;
begin
...
function TdxHtmlParser.ReplaceSpecialSymbolCore(const ARawText: string;
  AStartIndex: Integer; AIsPrevWhiteSpace: Boolean): string;
var
  I: Integer;
  ACh: char;
...
begin
  ...
  try
    ...
    begin
      ACh := ARawText[I];
      if (ACh = #10) or (ACh = #13) or (ACh = #9) or (ACh = ' ') or (ACh = #0)
      then
      begin
...
begin
  ...
  begin
    ACh := ARawText[I];
    AIsSpecialWhiteSpace := (ACh = #10) or (ACh = #13) or (ACh = #9) or
      (ACh = #0);
...
begin
  if HtmlTagNameIDTable.TryGetValue(AName, Result) then
    Exit
  else if TdxStringHelper.Contains(AName, ':') then
