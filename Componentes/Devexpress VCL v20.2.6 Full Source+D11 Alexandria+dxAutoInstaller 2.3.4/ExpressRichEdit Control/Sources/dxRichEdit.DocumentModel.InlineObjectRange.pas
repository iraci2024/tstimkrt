...
type
  ...
  public
    class function GetScale(ADesired, AOriginalInModelUnits: Integer;
      const ADefaultScale: Single): Single; overload; static;
    class function GetScale(const ADesiredSize, AOriginalSize: TSize;
      const ADefaultScale: Single): TdxSizeF; overload; static;
    class function GetDesiredImageSizeInModelUnits(ADesiredSizeInPixels: TSize;
      AUnitConverter: TdxDocumentModelUnitConverter): TSize; overload; static;
    class function GetDesiredImageSizeInModelUnits(AWidthInPixels,
      AHeightInPixels: Integer): TSize; overload; static;
  end;

  { TdxInlineObjectRun }

  TdxInlineObjectRun = class abstract(TdxTextRunBase, IdxRectangularObject,
  ...
  public
    constructor Create(AParagraph: TdxSimpleParagraph;
      const AOriginalSize: TSize); reintroduce; virtual;

    function GetPlainText(AGrowBuffer: TdxChunkedStringBuilder)
      : string; override;
    function GetPlainText(AGrowBuffer: TdxChunkedStringBuilder;
      AFrom, ATo: Integer): string; override;
  ...
  TdxInlineObjectRunBase<T: TdxInlineObjectInfo> = class abstract
    (TdxInlineObjectRun)
  strict private
    FProperties: TdxInlineObjectProperties<T>;
  protected
    function CreateProperties(APieceTable: TdxCustomPieceTable)
      : TdxInlineObjectProperties<T>; virtual; abstract;
    procedure InternalSetPropertiesScaleX(const Value: Single);
      virtual; abstract;
    procedure InternalSetPropertiesScaleY(const Value: Single);
      virtual; abstract;
  ...
  public
    constructor Create(AParagraph: TdxSimpleParagraph;
      const AOriginalSize: TSize); override;
  ...
  TdxInlineCustomObjectRun = class
    (TdxInlineObjectRunBase<TdxInlineCustomObjectInfo>) // for internal use
  strict private
    FOriginalSize: TSize;
  ...
  protected
    ...
    function CreateProperties(APieceTable: TdxCustomPieceTable)
      : TdxInlineObjectProperties<TdxInlineCustomObjectInfo>; override;
    procedure SubscribeCustomObjectEvents; virtual;
    procedure SubscribePropertiesEvents; virtual;
    property CustomObjectProperties: TdxInlineCustomObjectProperties
      read GetCustomObjectProperties;
    property Properties: TdxInlineCustomObjectProperties read GetProperties;
  public
    constructor Create(AParagraph: TdxSimpleParagraph;
      const ACustomObject: IdxInlineCustomObject); reintroduce;
  ...
  TdxInlinePictureRun = class(TdxInlineObjectRunBase<TdxInlinePictureInfo>,
    IdxInlinePicturePropertiesContainer, IdxPictureContainerRun,
    IdxInlineObjectRun, IdxHighlightableTextRun)
  strict private
    ...
    function GetLockAspectRatio: Boolean;
    function IdxPictureContainerRun.GetDocumentModel =
      PictureContainerRunGetDocumentModel;
    function PictureContainerRunGetDocumentModel: TdxCustomDocumentModel;
    procedure SetSizing(const Value: TdxImageSizeMode);
    procedure SetResizingShadowDisplayMode(const Value
  ...
  protected
    ...
    function CreatePictureObjectContent(AImage: TdxOfficeImageReference)
      : TdxPictureFloatingObjectContent; virtual;
    ...
    procedure CopyOriginalSize(const AOriginalSize: TSize); override;
    function CreateProperties(APieceTable: TdxCustomPieceTable)
      : TdxInlineObjectProperties<TdxInlinePictureInfo>; override;
    procedure InsertInlinePicture(ATargetPieceTable: TdxSimplePieceTable;
      ALogPosition: TdxDocumentLogPosition); virtual;

    function GetPieceTable: TdxCustomPieceTable;
    function CreateInlinePicturePropertiesChangedHistoryItem
      : TdxIndexChangedHistoryItemCore;
    procedure OnInlineCharacterPropertiesChanged;
  public
    constructor Create(AParagraph: TdxSimpleParagraph;
      AImage: TdxOfficeImageReference); reintroduce;
    destructor Destroy; override;

    class function ShouldExportInlinePictureRunCharacterProperties
      (AInfo: TdxCharacterFormattingInfo;
      const AOptions: TdxCharacterFormattingOptions): Boolean; static;
    function CanPlaceCaretBefore: Boolean; override;
    function Copy(ACopyManager: TdxCustomDocumentModelCopyManager)
      : TdxTextRunBase; override;
    ...
    property PictureProperties: TdxInlinePictureProperties
      read GetPictureProperties;
    property ResizingShadowDisplayMode: TdxResizingShadowDisplayMode
      read GetResizingShadowDisplayMode write SetResizingShadowDisplayMode;
  ...
  protected
    ...
    procedure OnImageChanged(Sender: TObject;
      E: TdxNativeImageChangedEventArgs); virtual;
  ...
  public
    constructor Create(const ARun: IdxPictureContainerRun;
      AImage: TdxOfficeImageReference); reintroduce;
    destructor Destroy; override;
    ...
    property InvalidActualSize: Boolean read FInvalidActualSize
      write FInvalidActualSize;
  end;
...
class function TdxImageScaleCalculator.GetScale(ADesired, AOriginalInModelUnits
  : Integer; const ADefaultScale: Single): Single;
begin
...
class function TdxImageScaleCalculator.GetDesiredImageSizeInModelUnits
  (ADesiredSizeInPixels: TSize;
  AUnitConverter: TdxDocumentModelUnitConverter): TSize;
begin
  Result := AUnitConverter.PixelsToModelUnits(ADesiredSizeInPixels,
    TdxCustomDocumentModel.DpiX, TdxCustomDocumentModel.DpiY);
end;

class function TdxImageScaleCalculator.GetDesiredImageSizeInModelUnits
  (AWidthInPixels, AHeightInPixels: Integer): TSize;
begin
  Result := PixelsToTwips(TSize.Create(AWidthInPixels, AHeightInPixels),
    TdxCustomDocumentModel.DpiX, TdxCustomDocumentModel.DpiY);
end;

class function TdxImageScaleCalculator.GetScale(const ADesiredSize,
  AOriginalSize: TSize; const ADefaultScale: Single): TdxSizeF;
begin
  Result := dxSizeF(GetScale(ADesiredSize.Width, AOriginalSize.Width,
    ADefaultScale), GetScale(ADesiredSize.Height, AOriginalSize.Height,
    ADefaultScale));
end;
...
constructor TdxInlineObjectRun.Create(AParagraph: TdxSimpleParagraph;
...
function TdxInlineObjectRun.GetPlainText(AGrowBuffer: TdxChunkedStringBuilder;
  AFrom, ATo: Integer): string;
begin
  Result := GetPlainText(AGrowBuffer);
end;

function TdxInlineObjectRun.GetPlainText(AGrowBuffer
  : TdxChunkedStringBuilder): string;
begin
  Result := EmptyStr;
end;

procedure TdxInlineObjectRun.Measure(ABoxInfo: TdxBoxInfo;
  const AMeasurer: IdxObjectMeasurer);
begin
  ABoxInfo.Size := OriginalSize;
end;

function TdxInlineObjectRun.TryAdjustEndPositionToFit(ABoxInfo: TdxBoxInfo;
  AMaxWidth: Integer; const AMeasurer: IdxObjectMeasurer): Boolean;
begin
  Result := False;
end;
...
constructor TdxInlineObjectRunBase<T>.Create(AParagraph: TdxSimpleParagraph;
  const AOriginalSize: TSize);
begin
...
procedure TdxInlineObjectRunBase<T>.SetActualSizeInternal
...
begin
  ...
  try
    ScaleX := TdxImageScaleCalculator.GetScale(AActualSize.Width,
      OriginalSize.Width, 100);
    ScaleY := TdxImageScaleCalculator.GetScale(AActualSize.Height,
      OriginalSize.Height, 100);
  finally
...
constructor TdxInlineCustomObjectRun.Create(AParagraph: TdxSimpleParagraph;
  const ACustomObject: IdxInlineCustomObject);
begin
  inherited Create(AParagraph, cxSize(1, 1));
  Assert(ACustomObject <> nil);
  FCustomObject := ACustomObject;

  FOriginalSize := ACustomObject.CalculateOriginalSize
    (AParagraph.DocumentModel.UnitConverter);
...
function TdxInlineCustomObjectRun.Copy(ACopyManager
...
begin
  ...
  ATargetPieceTable.InsertInlineCustomObject(ATargetPosition.LogPosition,
    CustomObject.Clone);
  ARun := TdxInlineCustomObjectRun(ATargetPieceTable.Runs
    [ATargetPosition.RunIndex]);
...
function TdxInlineCustomObjectRun.CreateProperties
  ...
  : TdxInlineObjectProperties<TdxInlineCustomObjectInfo>;
begin
...
procedure TdxInlineCustomObjectRun.InternalSetPropertiesScaleX
  (const Value: Single);
begin
  Properties.ScaleX := Value;
end;

procedure TdxInlineCustomObjectRun.InternalSetPropertiesScaleY
  (const Value: Single);
begin
...
function TdxInlineCustomObjectRun.GetCustomObjectProperties
  : TdxInlineCustomObjectProperties;
begin
  Result := TdxInlineCustomObjectProperties(Properties);
end;

function TdxInlineCustomObjectRun.GetProperties
  : TdxInlineCustomObjectProperties;
begin
...
constructor TdxInlinePictureRun.Create(AParagraph: TdxSimpleParagraph;
...
begin
  ...
  PictureProperties.OnObtainAffectedRange.Add
    (OnCharacterPropertiesObtainAffectedRange);
end;
...
procedure TdxInlinePictureRun.Measure(ABoxInfo: TdxBoxInfo;
  const AMeasurer: IdxObjectMeasurer);
begin
...
function TdxInlinePictureRun.CreateInlinePicturePropertiesChangedHistoryItem
  : TdxIndexChangedHistoryItemCore;
begin
  Result := TdxRunInlinePicturePropertiesChangedHistoryItem.Create
    (Paragraph.PieceTable, GetRunIndex);
end;

function TdxInlinePictureRun.CreateProperties(APieceTable: TdxCustomPieceTable)
  : TdxInlineObjectProperties<TdxInlinePictureInfo>;
begin
  Result := TdxInlinePictureProperties.Create(Self);
end;

procedure TdxInlinePictureRun.Export(const AExporter
...
function TdxInlinePictureRun.GetBatchUpdateChangeActions
  : TdxDocumentModelChangeActions;
begin
  Result := TdxInlinePicturePropertiesAccess(PictureProperties)
...
function TdxInlinePictureRun.PictureContainerRunGetDocumentModel
...
function TdxInlinePictureRun.Copy(ACopyManager
  : TdxCustomDocumentModelCopyManager): TdxTextRunBase;
var
  ATargetPieceTable: TdxSimplePieceTable;
  ATargetPosition: TdxDocumentModelPosition;
...
function TdxInlinePictureRun.CreatePictureObjectContent
  (AImage: TdxOfficeImageReference): TdxPictureFloatingObjectContent;
begin
...
function TdxInlinePictureRun.GetResizingShadowDisplayMode
  : TdxResizingShadowDisplayMode;
begin
...
procedure TdxInlinePictureRun.InsertInlinePicture(ATargetPieceTable
  : TdxSimplePieceTable; ALogPosition: TdxDocumentLogPosition);
begin
  ATargetPieceTable.InsertInlinePicture(ALogPosition, Image);
end;

function TdxInlinePictureRun.TryAdjustEndPositionToFit(ABoxInfo: TdxBoxInfo;
  AMaxWidth: Integer; const AMeasurer: IdxObjectMeasurer): Boolean;
begin
  Result := False;
end;

function TdxInlinePictureRun.MeasureRun(const AMeasurer
  : IdxObjectMeasurer): TSize;
begin
...
procedure TdxInlinePictureRun.SetResizingShadowDisplayMode
  (const Value: TdxResizingShadowDisplayMode);
begin
...
class function TdxInlinePictureRun.
  ShouldExportInlinePictureRunCharacterProperties
  (AInfo: TdxCharacterFormattingInfo;
  const AOptions: TdxCharacterFormattingOptions): Boolean;
begin
  Result := (AOptions.UseHidden and AInfo.Hidden) or
    (AOptions.UseFontUnderlineType and (AInfo.FontUnderlineType <>
    TdxUnderlineType.None)) or (AOptions.UseFontStrikeoutType and
    (AInfo.FontStrikeoutType <> TdxStrikeoutType.None)) or
    (AOptions.UseBackColor and not TdxAlphaColors.IsTransparentOrEmpty
    (AInfo.BackColor));
end;
...
constructor TdxPictureFloatingObjectContent.Create
  (const ARun: IdxPictureContainerRun; AImage: TdxOfficeImageReference);
var
...
begin
  ...
  AImageSizeInModelUnits := AImage.CalculateImageSizeInModelUnits
    (Paragraph.DocumentModel.UnitConverter);
...
procedure TdxPictureFloatingObjectContent.EnsureActualSize
  (AUseScreenDpi: Boolean);
begin
...
procedure TdxPictureFloatingObjectContent.HandleImageChanged
  (E: TdxNativeImageChangedEventArgs);
var
...
begin
  ...
  AImageSize := FImage.CalculateImageSizeInModelUnits
  ...
  RunPieceTable.ApplyChangesCore
    (FPictureContainerRun.GetBatchUpdateChangeActions, ARunIndex, ARunIndex);
end;

function TdxPictureFloatingObjectContent.
  GetImageSizeWithDocumentModelDpi: TSize;
var
...
procedure TdxPictureFloatingObjectContent.OnImageChanged(Sender: TObject;
  E: TdxNativeImageChangedEventArgs);
begin
...
procedure TdxPictureFloatingObjectContent.OnImageChanging(Sender: TObject;
  E: TdxEventArgs);
begin
  if DocumentModel.IsDestroying then
    Exit;
...
function TdxPictureFloatingObjectContent.ApplyScales(const AImageSize
  : TSize): TSize;
var
...
function TdxPictureFloatingObjectContent.CalculateActualSize(const ADesiredSize
  : TSize): TSize;
var
...
begin
  ...
  AAspect := FImage.SizeInOriginalUnits.Width /
    FImage.SizeInOriginalUnits.Height;
...
procedure TdxPictureFloatingObjectContent.SetActualSize
  (const ADesiredSize: TSize);
var
  AActualSize: TSize;
begin
