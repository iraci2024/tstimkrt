...
type
  ...
  type
    ...
    public
      ...
      property PositionsMapping: TDictionary<Integer, TdxDocumentLogPosition>
        read FPositionsMapping;
      property SkippedPositionsMapping
        : TDictionary<Integer, TdxDocumentLogPosition>
        read FSkippedPositionsMapping;
      property PositionIndex: Integer read FCurrentPositionIndex
        write FCurrentPositionIndex;
    end;
  ...
  public
    ...
    function TryConvert(APosition: Integer; ASkipDeletedPosition: Boolean;
      out ALogPosition: TdxDocumentLogPosition): Boolean; overload;
    function TryConvert(APosition: Integer;
      out ALogPosition: TdxDocumentLogPosition): Boolean; overload;
    function GetNextObtainablePosition(APosition: Integer)
      : TdxDocumentLogPosition;
    function GetPrevObtainablePosition(APosition: Integer)
      : TdxDocumentLogPosition;
  end;
...
constructor TdxPositionConverter.TPositionConverterState.Create(AStart: Integer;
  AEnd: Integer);
begin
  FStart := AStart;
  FEnd := AEnd;
  FPositionsMapping := TDictionary<Integer, TdxDocumentLogPosition>.Create;
  FSkippedPositionsMapping :=
    TDictionary<Integer, TdxDocumentLogPosition>.Create;
end;
...
procedure TdxPositionConverter.BeginEmbeddedContent(AStart: Integer;
  AEnd: Integer);
var
  AStartIndex: Integer;
begin
  ...
  if (AStartIndex < FOriginalPositions.Count) and
    (FOriginalPositions[AStartIndex] <= AEnd) then
...
function TdxPositionConverter.CalculatePositionIndex(ACharacterPosition
  : Integer): Integer;
begin
...
procedure TdxPositionConverter.AdvanceNext(ALogPosition: TdxDocumentLogPosition;
  AOriginalPosition: Integer; ALength: Integer);
var
...
begin
  ...
  if AIndex < 0 then
    Exit;
  while (AIndex < FOriginalPositions.Count) and
    (FOriginalPositions[AIndex] < AOriginalPosition) do
  begin
  ...
  while (AIndex < FOriginalPositions.Count) and
    (FOriginalPositions[AIndex] < AOriginalPosition + ALength) do
  begin
    ACurrentPosition := FOriginalPositions[AIndex];
    FCurrentState.PositionsMapping.Add(ACurrentPosition,
      ALogPosition + ACurrentPosition - AOriginalPosition);
    Inc(AIndex);
  end;
...
begin
  Result := (APosition >= FCurrentState.Start) and
    (APosition <= FCurrentState.&End);
end;

function TdxPositionConverter.TryConvert(APosition: Integer;
  ASkipDeletedPosition: Boolean;
  out ALogPosition: TdxDocumentLogPosition): Boolean;
begin
  if not FCurrentState.PositionsMapping.TryGetValue(APosition, ALogPosition)
  then
  begin
    if not ASkipDeletedPosition then
      Result := FCurrentState.SkippedPositionsMapping.TryGetValue(APosition,
        ALogPosition)
    else
      Result := False;
  end
...
function TdxPositionConverter.GetNextObtainablePosition(APosition: Integer)
  : TdxDocumentLogPosition;
var
...
function TdxPositionConverter.GetPrevObtainablePosition(APosition: Integer)
  : TdxDocumentLogPosition;
var
  APositionsMapping: TDictionary<Integer, TdxDocumentLogPosition>;
  AIndex, I: Integer;
...
function TdxPositionConverter.CreateState(AStart, AEnd: Integer)
  : TPositionConverterState;
begin
