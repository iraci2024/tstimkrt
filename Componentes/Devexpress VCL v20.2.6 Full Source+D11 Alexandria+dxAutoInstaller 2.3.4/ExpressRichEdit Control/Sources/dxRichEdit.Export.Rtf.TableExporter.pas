...
type
  ...
  protected
    function CreateExporterState(ATable: TdxTable)
      : TdxRtfTableExporterStateBase; virtual;
  ...
  public
    ...
    property ExportAsNestedTable: Boolean read FExportAsNestedTable
      write FExportAsNestedTable;
  end;
  ...
  private
    ...
    function GetActualBorderInfoCore(ACellBorderInfo: TdxBorderInfo;
      AUse: Boolean): TdxBorderInfo;
  ...
  protected
    ...
    procedure ExportInTableParagraph(AParIndex: TdxParagraphIndex;
    ...
    procedure ExportCellParagraphs(ACell: TdxTableCell;
      AParentRowIndex: Integer);
    function ExportNestedTable(ACell: TdxTableCell;
      AParIndex: TdxParagraphIndex; var ALastParIndex: TdxParagraphIndex)
      : Boolean; overload;
    procedure ExportNestedTable(ATable: TdxTable;
      var ALastParIndex: TdxParagraphIndex); overload;
    function CreateTableHelper(ATable: TdxTable): TdxRtfTableExportHelper;
    procedure ExportRow(ARow: TdxTableRow; ARowIndex: Integer);
      virtual; abstract;
    procedure WriteParagraphEndMark; virtual; abstract;
    procedure ExportRowProperties(ARow: TdxTableRow; ARowIndex: Integer);
    function CalculateRowLeft(ARow: TdxTableRow; AIndent: TdxWidthUnit)
      : Integer;
    function CalculateRowLeftOffset(ARow: TdxTableRow): Integer;
    function GetActualWidth(AUnit: TdxWidthUnit): Integer;
    procedure ExportOwnRowProperties(ARow: TdxTableRow; ARowIndex: Integer;
      ALeft: Integer);
  ...
  public
    ...
    destructor Destroy; override;
    function GetCellWidth(ALeftSideIndex: Integer;
...
function TdxRtfTableExporter.CreateExporterState(ATable: TdxTable)
  : TdxRtfTableExporterStateBase;
var
  ARootTable: TdxTable;
begin
...
constructor TdxRtfTableExporterStateBase.Create(ARtfExporter
  : TdxRtfContentExporter; ATable: TdxTable; ANestingLevel: Integer);
begin
  inherited Create;
  ...
  FTableRowPropertiesExporter := TdxRtfTableRowPropertiesExporter.Create
    (DocumentModel, RtfContentExporter.RtfExportHelper, RtfBuilder);
  FTableCellPropertiesExporter := TdxRtfTableCellPropertiesExporter.Create
    (DocumentModel, RtfContentExporter.RtfExportHelper, RtfBuilder);
  FTablePropertiesExporter := TdxRtfTablePropertiesExporter.Create
    (DocumentModel, RtfContentExporter.RtfExportHelper, RtfBuilder);
...
function TdxRtfTableExporterStateBase.GetActualLeftBorderInfo
  (ACell: TdxTableCell): TdxBorderInfo;
begin
  Result := GetActualBorderInfoCore(ACell.GetActualLeftCellBorder.Info,
    ACell.Properties.Borders.UseLeftBorder);
end;

function TdxRtfTableExporterStateBase.GetActualRightBorderInfo
  (ACell: TdxTableCell): TdxBorderInfo;
begin
  Result := GetActualBorderInfoCore(ACell.GetActualRightCellBorder.Info,
    ACell.Properties.Borders.UseRightBorder);
end;

function TdxRtfTableExporterStateBase.GetActualTopBorderInfo
  (ACell: TdxTableCell): TdxBorderInfo;
begin
  Result := GetActualBorderInfoCore(ACell.GetActualTopCellBorder.Info,
    ACell.Properties.Borders.UseTopBorder);
end;

function TdxRtfTableExporterStateBase.GetActualBottomBorderInfo
  (ACell: TdxTableCell): TdxBorderInfo;
begin
  Result := GetActualBorderInfoCore(ACell.GetActualBottomCellBorder.Info,
    ACell.Properties.Borders.UseBottomBorder);
end;

function TdxRtfTableExporterStateBase.GetActualBorderInfoCore(ACellBorderInfo
  : TdxBorderInfo; AUse: Boolean): TdxBorderInfo;
begin
...
function TdxRtfTableExporterStateBase.GetUnitConverter
...
begin
  if FRtfExporter.RtfExportHelper.SupportStyle and
    (FTableHelper.Table.StyleIndex <>
    TdxTableStyleCollection.DefaultTableStyleIndex) then
  begin
...
procedure TdxRtfTableExporterStateBase.ExportRowCells(ARow: TdxTableRow;
  ARowIndex: Integer);
var
...
procedure TdxRtfTableExporterStateBase.ExportInTableParagraph
  (AParIndex: TdxParagraphIndex; ATableNestingLevel: Integer;
...
begin
  ...
  RtfContentExporter.ExportParagraphCore(AParagraph, ATableNestingLevel,
    ACondTypes, FTableStyleIndex);
  FinishParagraph(AIsEndParagraph);
end;

procedure TdxRtfTableExporterStateBase.FinishParagraph(AIsEndParagraph
  : Boolean);
begin
...
procedure TdxRtfTableExporterStateBase.ExportCellParagraphs(ACell: TdxTableCell;
  AParentRowIndex: Integer);
var
  AStartParagraphIndex, AEndParagraphIndex, AParIndex, ANextParIndex
    : TdxParagraphIndex;
  AIsEndParagraph: Boolean;
  ARow: TdxTableRow;
begin
  ...
  begin
    ...
    begin
      if (NestingLevel = 1) and (AParIndex = AStartParagraphIndex) and ACell.IsFirstCellInRow
      then
        ExportRowProperties(ACell.Row, AParentRowIndex);
      AIsEndParagraph := AParIndex = AEndParagraphIndex;
      ExportInTableParagraph(AParIndex, NestingLevel, AIsEndParagraph,
        ACell.CellConditionalFormattingMasks);
    end;
    Inc(AParIndex);
  end;
end;

function TdxRtfTableExporterStateBase.ExportNestedTable(ACell: TdxTableCell;
  AParIndex: TdxParagraphIndex; var ALastParIndex: TdxParagraphIndex): Boolean;
var
  AParagraph: TdxParagraph;
...
procedure TdxRtfTableExporterStateBase.ExportNestedTable(ATable: TdxTable;
  var ALastParIndex: TdxParagraphIndex);
var
  ANestedTableExporter: TdxRtfNestedTableExporterState;
begin
  ANestedTableExporter := TdxRtfNestedTableExporterState.Create
    (RtfContentExporter, ATable, NestingLevel + 1);
  try
...
function TdxRtfTableExporterStateBase.CreateTableHelper(ATable: TdxTable)
  : TdxRtfTableExportHelper;
begin
  Result := TdxRtfTableExportHelper.Create(ATable);
end;

procedure TdxRtfTableExporterStateBase.ExportRowProperties(ARow: TdxTableRow;
  ARowIndex: Integer);
var
...
function TdxRtfTableExporterStateBase.CalculateRowLeft(ARow: TdxTableRow;
  AIndent: TdxWidthUnit): Integer;
var
...
function TdxRtfTableExporterStateBase.CalculateRowLeftOffset
...
function TdxRtfTableExporterStateBase.GetActualWidth
  (AUnit: TdxWidthUnit): Integer;
begin
...
procedure TdxRtfTableExporterStateBase.ExportOwnRowProperties(ARow: TdxTableRow;
  ARowIndex: Integer; ALeft: Integer);
var
  ATable: TdxTable;
begin
  ...
  ATable := ARow.Table;
  FTablePropertiesExporter.WriteTableBorders(ATable.GetActualTopBorder.Info,
    ATable.GetActualLeftBorder.Info, ATable.GetActualBottomBorder.Info,
    ATable.GetActualRightBorder.Info,
    ATable.GetActualInsideHorizontalBorder.Info,
    ATable.GetActualInsideVerticalBorder.Info);
  FTablePropertiesExporter.WriteTableFloatingPosition
    (ATable.TableProperties.FloatingPosition.Info);
  ...
  FTablePropertiesExporter.WriteTableCellMargins(ATable.LeftMargin.Info,
    ATable.RightMargin.Info, ATable.BottomMargin.Info, ATable.TopMargin.Info);
  ...
  FTablePropertiesExporter.WriteBandSizes
    (ATable.TableProperties.GeneralSettings.Info,
    ATable.TableStyle.HasRowBandingStyleProperties,
    ATable.TableStyle.HasColumnBandingStyleProperties);
end;
...
begin
  ...
  FTableRowPropertiesExporter.WriteHalfSpaceBetweenCells
...
procedure TdxRtfTableExporterStateBase.ExportCellProperties(ACell: TdxTableCell;
  ACellRight: Integer);
begin
  ...
  FTableCellPropertiesExporter.WriteCellBasicBorders
    (GetActualTopBorderInfo(ACell), GetActualLeftBorderInfo(ACell),
    GetActualRightBorderInfo(ACell), GetActualBottomBorderInfo(ACell));
  ...
  FTableCellPropertiesExporter.WriteCellPreferredWidth
    (ACell.PreferredWidth.Info);
  FTableCellPropertiesExporter.WriteCellMargins(ACell.GetActualTopMargin.Info,
...
function TdxRtfTableExportHelper.GetCellWidth(ALeftSideIndex: Integer;
  AColumnSpan: Integer): Integer;
var
  I: Integer;
begin
...
begin
  ...
  try
    ...
    try
      Result := ACalculator.CalculateTableGrid(Table,
        ADocumentModel.ToDocumentLayoutUnitConverter.ToLayoutUnits
        (TdxRtfTableWidthsCalculator.DefaultPercentBaseWidthInTwips));
    finally
      ACalculator.Free;
    end;
  finally
...
constructor TdxRtfTableExporterState.Create(ARtfExporter: TdxRtfContentExporter;
  ATable: TdxTable);
begin
...
procedure TdxRtfTableExporterState.ExportRow(ARow: TdxTableRow;
  ARowIndex: Integer);
begin
...
procedure TdxRtfNestedTableExporterState.ExportRow(ARow: TdxTableRow;
  ARowIndex: Integer);
begin
  ExportRowCells(ARow, ARowIndex);
