...
type
  TdxDocBoolWrapper = (False = $00, True = $01, Leave = $80, Inverse = $81);
  ...
  public
    ...
    property DoubleStrike: TdxDocBoolWrapper read FDoubleStrike
      write FDoubleStrike;
    property FontUnderlineType: TdxUnderlineType read FFontUnderlineType
      write FFontUnderlineType;
    property AllCaps: TdxDocBoolWrapper read FAllCaps write FAllCaps;
    property UnderlineWordsOnly: Boolean read FUnderlineWordsOnly
      write FUnderlineWordsOnly;
    property StrikeoutWordsOnly: Boolean read FStrikeoutWordsOnly
      write FStrikeoutWordsOnly;
    ...
    property UnderlineColor: TdxAlphaColor read FUnderlineColor
      write FUnderlineColor;
    property StrikeoutColor: TdxAlphaColor read FStrikeoutColor
      write FStrikeoutColor;
...
class function TdxDocCharacterFormattingInfo.CreateDefault
  : TdxDocCharacterFormattingInfo;
begin
...
procedure TdxDocCharacterFormattingInfo.SetDoubleFontSize
...
begin
  Result := (DoubleFontSize = AInfo.DoubleFontSize) and
    (FontBold = AInfo.FontBold) and (FontItalic = AInfo.FontItalic) and
    (Strike = AInfo.Strike) and (DoubleStrike = AInfo.DoubleStrike) and
    (FontUnderlineType = AInfo.FontUnderlineType) and (AllCaps = AInfo.AllCaps)
    and (ForeColor = AInfo.ForeColor) and (BackColor = AInfo.BackColor) and
    ...
    (UnderlineWordsOnly = AInfo.UnderlineWordsOnly) and (Script = AInfo.Script)
    and (Hidden = AInfo.Hidden) and (StyleIndex = AInfo.StyleIndex) and
...
begin
  Result := (FAllCaps = TdxDocBoolWrapper.Inverse) or
