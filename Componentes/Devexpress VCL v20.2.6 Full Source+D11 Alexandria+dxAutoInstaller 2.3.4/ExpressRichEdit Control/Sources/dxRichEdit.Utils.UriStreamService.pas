...
uses
  SysUtils, Classes, Generics.Defaults, Generics.Collections, dxCoreClasses,
  dxThreading,
...
type
  ...
  IdxUriStreamProvider2 = interface(IdxUriStreamProvider)
    ...
    function AsyncGetStream(const AUri: string;
      const ACancelStatus: TdxTaskCancelCallback): TStream;
  end;
  ...
  TdxUriStreamService = class(TInterfacedObject, IdxUriStreamService,
  ...
  TdxWebUriStreamProvider = class(TInterfacedObject, IdxUriStreamProvider,
  ...
  TdxDataStringUriStreamProvider = class(TInterfacedObject,
    IdxUriStreamProvider)
  protected
    // IdxUriStreamProvider
    function GetStream(const AUri: string): TStream;
  ...
  IdxUriProvider = interface
    ...
    function CreateImageUri(const ARootUri: string; AImage: TdxOfficeImage;
      const ARelativeUri: string): string;
    function CreateCssUri(const ARootUri, AStyleText,
  ...
  protected
    function GetRelativeFileName(const ARootUri, ARelativeUri,
      AFileName: string): string;
    function TryToSaveImageInNativeFormat(AImage: TdxOfficeImage;
      const AFileName: string): Boolean; virtual;
    function TryToSaveImageAsPng(AImage: TdxOfficeImage;
      const AFileName: string): Boolean; virtual;
    ...
    function CreateNextFileName(const APath, AName, AExtension: string)
      : string; virtual;
    function GetInitialFileIndex(const APath, AName, AExtension: string)
      : Integer; virtual;
...
{$ENDREGION}
  strict private
    ...
    function GetImageBytesCore(AImage: TdxOfficeImage;
...
{$ENDREGION}
  ...
  public
    class property ApplicationDataStringPattern: string
      read GetApplicationDataStringPattern;
    class property ImageDataStringPattern: string
      read GetImageDataStringPattern;
  end;
...
begin
  Result := Format
    ('%0:s?data:%0:s?application%0:s?%4:s?%0:s?%1:s?%0:s?;%0:s?%2:s%0:s?,%0:s?%3:s%0:s?',
    [SpacePattern, MediaTypePattern, CapacityPattern, ContentPattern,
    SlashPattern]);
end;
...
begin
  Result := Format
    ('%0:s?data:%0:s?image%0:s?%4:s?%0:s?%1:s?%0:s?;%0:s?%2:s%0:s?,%0:s?%3:s%0:s?',
    [SpacePattern, ImageTypePattern, CapacityPattern, ImagePattern,
    SlashPattern]);
end;
...
function TdxUriStreamService.AsyncGetStream(const AUri: string;
  const ACancelStatus: TdxTaskCancelCallback): TStream;
var
  ACount, I: Integer;
...
procedure TdxUriStreamService.RegisterProvider(const AProvider
  : IdxUriStreamProvider);
begin
...
procedure TdxUriStreamService.UnregisterProvider(const AProvider
  : IdxUriStreamProvider);
var
  AIndex: Integer;
begin
...
function TdxUriStreamService.IsAsyncLoadingSupported
...
begin
  ...
  else if AUriRecord.IsRelativeUri then
    AFile := AUri
  else
    AFile := '';
...
function TdxLocalUriStreamProvider.IsAsyncLoadingSupported
...
function TdxWebUriStreamProvider.AsyncGetStream(const AUri: string;
  const ACancelStatus: TdxTaskCancelCallback): TStream;
...
function TdxWebUriStreamProvider.IsAsyncLoadingSupported
  (const AUrl: string): Boolean;
begin
  Result := True;
end;
...
begin
  ...
  try
    ARegex := TRegEx.Create(TDataStringUriPattern.ImageDataStringPattern,
      [{$IFDEF DELPHIXE6}roNotEmpty, {$ENDIF}roSingleLine]);
    ...
    begin
      ARegex := TRegEx.Create
        (TDataStringUriPattern.ApplicationDataStringPattern);
      AMatch := ARegex.Match(AUri);
      if AMatch.Success then
      begin
        if (AMatch.Groups[1 { 'mediatype' } ].Value = 'octet-stream') and
          (AMatch.Groups['capacity'].Value = 'base64') then
...
function TdxDataStringUriStreamProvider.IsAsyncLoadingSupported
  (const AUrl: string): Boolean;
begin
  Result := True;
end;
...
function TdxUriProviderService.CreateCssUri(const ARootUri, AStyleText,
  ARelativeUri: string): string;
var
  I, ACount: Integer;
begin
...
procedure TdxUriProviderService.RegisterProvider(const AProvider
  : IdxUriProvider);
begin
  if AProvider = nil then
    Exit;
...
procedure TdxUriProviderService.UnregisterProvider(const AProvider
  : IdxUriProvider);
var
  AIndex: Integer;
begin
...
function TdxFileBasedUriProvider.CreateImageUri(const ARootUri: string;
  AImage: TdxOfficeImage; const ARelativeUri: string): string;
var
  AFileName: string;
  S: string;
...
function TdxFileBasedUriProvider.CreateCssUri(const ARootUri, AStyleText,
  ARelativeUri: string): string;
var
  AFileName: string;
  S: string;
...
function TdxFileBasedUriProvider.GetRelativeFileName(const ARootUri,
  ARelativeUri, AFileName: string): string;
var
  AStr: TStringBuilder;
begin
  ...
  try
    try
      ...
      Result := FileUriPrefix + AFileName;
      TdxRichEditExceptions.ThrowArgumentException('invalid relativeUri',
        ARelativeUri);
    end;
...
function TdxFileBasedUriProvider.TryToSaveImageInNativeFormat
  (AImage: TdxOfficeImage; const AFileName: string): Boolean;
begin
...
function TdxFileBasedUriProvider.TryToSaveImageAsPng(AImage: TdxOfficeImage;
...
procedure TdxFileBasedUriProvider.SaveCssProperties(const AFileName,
  AStyleText: string);
var
  AStream: TFileStream;
...
function TdxFileBasedUriProvider.EnsureDirectoryExists
...
function TdxFileBasedUriProvider.CreateNextFileName(const APath, AName,
...
function TdxFileBasedUriProvider.GetInitialFileIndex(const APath, AName,
  AExtension: string): Integer;
var
  S: string;
begin
  S := TPath.GetFullPath(APath);
  if (CompareText(S, FLastPath) = 0) and (CompareText(AName, FLastName) = 0) and
...
function TdxFileBasedUriProvider.GetImageFileExtension
...
begin
  ...
  try
    ...
    Result := Format('data:%s;base64,%s',
      [AContentType, TdxBase64.ToBase64String(ABytes.Bytes)]);
  except
    Result := '';
  end;
end;

function TdxDataStringUriProvider.GetImageBytes(AImage: TdxOfficeImage)
  : TImageBytes;
begin
  try
