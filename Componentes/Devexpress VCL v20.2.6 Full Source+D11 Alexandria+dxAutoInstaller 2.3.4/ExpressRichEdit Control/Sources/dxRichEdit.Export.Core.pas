...
type
  ...
  IdxExporterOptions = interface
    (IdxSupportsCopyFrom<TObject { IdxExporterOptions } >)
  ...
  TdxExportersCalculator = function(const AExportManagerService
    : IdxExportManagerService): TdxExporterList;
  ...
  public
    constructor Create(AFormat: TdxRichEditDocumentFormat;
  ...
  TdxBeforeExportEvent = procedure(Sender: TObject; E: TdxBeforeExportEventArgs)
    of object;
  ...
  public
    ...
    procedure UnregisterAllExporters;
    function GetExporter(AFormat: TdxRichEditDocumentFormat)
      : IdxExporter; virtual;
    function GetExporters: TdxExporterList; virtual;

    property Exporters: TDictionary<TdxRichEditDocumentFormat, IdxExporter>
      read FExporters;
  end;

  { TdxDocumentExportManagerService }

  TdxDocumentExportManagerService = class(TdxExportManagerService,
    IdxDocumentExportManagerService)
  protected
  ...
  protected
    procedure ApplyEncoding(const AOptions: IdxExporterOptions;
      AEncoding: TEncoding); virtual; abstract;
    function CalculateCurrentFilterIndex(AExporters: TdxExporterList)
      : Integer; virtual;
    function ChooseExporter(const AFileName: string; AFilterIndex: Integer;
      AExporters: TdxExporterList): IdxExporter;
    function ChooseExporterByFileName(const AFileName: string;
      AExporters: TdxExporterList): IdxExporter;
    function ChooseExporterByFilterIndex(AFilterIndex: Integer;
      AExporters: TdxExporterList): IdxExporter;
    function CreateExportFilters(AExporters: TdxExporterList)
      : TdxFileDialogFilterCollection; virtual;
    function CreateSaveFileDialog(AFilters: TdxFileDialogFilterCollection;
      ACurrentFilterIndex: Integer): TSaveDialog; virtual;
    function GetCurrentDocumentFormat: TdxRichEditDocumentFormat;
      virtual; abstract;
    ...
    function GetPredefinedOptions(AFormat: TdxRichEditDocumentFormat)
      : IdxExporterOptions; virtual; abstract;
    procedure PreprocessContentBeforeExport(AFormat: TdxRichEditDocumentFormat);
      virtual; abstract;
    procedure SetDirectoryName(ADialog: TSaveDialog;
      const ADirectoryName: string);
    procedure SetFileName(ADialog: TSaveDialog; const AFileName: string);
    function ShowSaveFileDialog(ADialog: TSaveDialog;
      const AParent: TWinControl): Boolean; virtual;
  public
    function Export(AStream: TStream; AFormat: TdxRichEditDocumentFormat;
      const ATargetUri: string;
      const AExportManagerService: IdxExportManagerService): Boolean; overload;
    function Export(AStream: TStream; AFormat: TdxRichEditDocumentFormat;
      const ATargetUri: string;
      const AExportManagerService: IdxExportManagerService;
      AEncoding: TEncoding): Boolean; overload;
    function InvokeExportDialog(const AParent: TWinControl;
      const AExportManagerService: IdxExportManagerService)
      : TdxExportTarget; overload;
    function InvokeExportDialog(const AParent: TWinControl;
      const AExportManagerService: IdxExportManagerService;
  ...
  public
    constructor Create(ADocumentModel: TdxCustomDocumentModel;
      const AOptions: IdxExporterOptions); virtual;
    destructor Destroy; override;
    ...
    property LastParagraphRunNotSelected: Boolean
      read FLastParagraphRunNotSelected write FLastParagraphRunNotSelected;
    property Options: IdxExporterOptions read FOptions;
...
constructor TdxBeforeExportEventArgs.Create(AFormat: TdxRichEditDocumentFormat;
  const AOptions: IdxExporterOptions);
begin
  FFormat := AFormat;
  FOptions := AOptions;
...
begin
  inherited Create;
  FExporters := TObjectDictionary<TdxRichEditDocumentFormat,
    IdxExporter>.Create;
...
procedure TdxExportManagerService.RegisterExporter(const AExporter
  : IdxExporter);
begin
  Exporters.Add(AExporter.Format, AExporter);
end;

procedure TdxExportManagerService.UnregisterExporter(const AExporter
  : IdxExporter);
begin
...
function TdxExportManagerService.GetExporter(AFormat: TdxRichEditDocumentFormat)
  : IdxExporter;
begin
...
function TdxExportHelper.Export(AStream: TStream;
...
begin
  ...
  AEncryptionPropertiesContainer :=
    DocumentModel as IdxDocumentEncryptionPropertiesContainer;
  if (AEncryptionPropertiesContainer <> nil) and
    (AEncryptionPropertiesContainer.EncryptionProperties.Password <> '') and
    not AExporter.SupportsEncryption then
    if Application.MessageBox
      (PChar(cxGetResourceString
      (@sdxRichEditConfirmSaveDocumentWithPasswordProtection)),
    ...
    try
      ...
      try
        if not ShowSaveFileDialog(ADialog, AParent) then
          Exit(nil);
        AParent.Repaint;
        AExporter := ChooseExporter(GetFileName(ADialog),
          ADialog.FilterIndex - 1, AExporters);
        if AExporter = nil then
          ThrowUnsupportedFormatException;
...
function TdxExportHelper.CreateSaveFileDialog
  (AFilters: TdxFileDialogFilterCollection; ACurrentFilterIndex: Integer)
  : TSaveDialog;
var
...
begin
  ...
  Result.Options := Result.Options - [ofFileMustExist, ofNoDereferenceLinks,
    ofNoValidate] + [ofPathMustExist, ofOverwritePrompt];
...
function TdxExportHelper.ChooseExporterByFilterIndex(AFilterIndex: Integer;
  AExporters: TdxExporterList): IdxExporter;
begin
...
function TdxExportHelper.CreateExportFilters(AExporters: TdxExporterList)
...
function TdxExportHelper.CalculateCurrentFilterIndex
  (AExporters: TdxExporterList): Integer;
var
...
procedure TdxExportHelper.SetFileName(ADialog: TSaveDialog;
  const AFileName: string);
begin
  ADialog.FileName := AFileName;
end;

procedure TdxExportHelper.SetDirectoryName(ADialog: TSaveDialog;
  const ADirectoryName: string);
begin
...
constructor TdxExportTarget.Create(const AFileName: string;
  const AExporter: IdxExporter);
begin
  inherited Create;
  FExporter := AExporter;
  FFileName := AFileName;
...
constructor TdxCustomDocumentModelExporter.Create(ADocumentModel
...
function TdxCustomDocumentModelExporter.ExportSaveMemory
  : TdxChunkedStringBuilder;
begin
  raise ENotSupportedException.CreateFmt
    ('%s doesn''t implement ExportSaveMemory', [ClassName]);
end;

function TdxCustomDocumentModelExporter.GetUnitConverter
