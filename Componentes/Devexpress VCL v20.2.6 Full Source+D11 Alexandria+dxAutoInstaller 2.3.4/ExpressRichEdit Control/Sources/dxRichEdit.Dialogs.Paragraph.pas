...
{$ENDIF}
  ...
  Controls, Forms, Dialogs, dxLayoutLookAndFeels, cxClasses, ActnList, dxCore,
  cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters, dxLayoutContainer, dxLayoutControl,
  Menus, dxLayoutControlAdapters,
  StdCtrls, cxButtons, dxLayoutcxEditAdapters, cxContainer, cxEdit, cxLabel,
...
type
  ...
  TdxRichEditParagraphDialogForm = class(TdxRichEditCustomDialogForm,
    IdxFormOwner)
    btnCancel: TcxButton;
    btnOK: TcxButton;
    btnTabs: TcxButton;
    ...
    procedure seFirstLineIndentPropertiesValidate(Sender: TObject;
  ...
  private
    ...
    FLockCount: Integer;
    function IsLineSpacingInsignificant(const ALineSpacingType
      : TdxNullableParagraphLineSpacing): Boolean;
    procedure PopulateAlignment;
    procedure PopulateFirstLineIndent;
    procedure PopulateLineSpacing;
    procedure PopulateOutlineLevel;
    procedure RefreshLineSpacingMeasurementUnit(const ALineSpacingType
      : TdxNullableParagraphLineSpacing; AForceUpdate: Boolean = False);
  ...
  public
    ...
    procedure Initialize(AControllerParameters
      : TdxFormControllerParameters); override;
...
procedure TdxRichEditParagraphDialogForm.
  ccbFirstLineIndentTypePropertiesEditValueChanged(Sender: TObject);
begin
  BeginUpdate;
  try
    if (ccbFirstLineIndentType.ItemIndex <=
      Ord(TdxParagraphFirstLineIndent.None)) then
...
procedure TdxRichEditParagraphDialogForm.
  seFirstLineIndentPropertiesPropertiesChange(Sender: TObject);
begin
  BeginUpdate;
  try
    if Trim(seFirstLineIndent.Text) = '' then
      ccbFirstLineIndentType.ItemIndex := Ord(TdxParagraphFirstLineIndent.None)
    else if ccbFirstLineIndentType.ItemIndex <=
      Ord(TdxParagraphFirstLineIndent.None) then
      ccbFirstLineIndentType.ItemIndex :=
        Ord(TdxParagraphFirstLineIndent.Indented);
  finally
    EndUpdate;
  end;
end;

procedure TdxRichEditParagraphDialogForm.seFirstLineIndentPropertiesValidate
  (Sender: TObject; var DisplayValue: Variant; var ErrorText: TCaption;
  var Error: Boolean);
begin
  if ccbFirstLineIndentType.ItemIndex > Ord(TdxParagraphFirstLineIndent.None)
  then
    MeasurementUnitEditValidateHandler(Sender, DisplayValue, ErrorText, Error);
end;

procedure TdxRichEditParagraphDialogForm.
  ccbLineSpacingPropertiesEditValueChanged(Sender: TObject);
var
  V: Variant;
...
begin
  ...
  try
    ...
    if ccbLineSpacing.ItemIndex >= 0 then
      ALineSpacingType.Value := TdxParagraphLineSpacing
        (ccbLineSpacing.ItemIndex);

    if not FLastLineSpacingType.IsNull and
      (FLastLineSpacingType.Value in [TdxParagraphLineSpacing.Multiple,
    ...
    else if (seAtSpacing.Text = '') or not IsLineSpacingInsignificant
      (FLastLineSpacingType) then
...
function TdxRichEditParagraphDialogForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxParagraphFormController.Create
    (AControllerParameters as TdxParagraphFormControllerParameters);
end;

procedure TdxRichEditParagraphDialogForm.seAtSpacingPropertiesChange
  (Sender: TObject);
var
  ALineSpacingType: TdxNullableParagraphLineSpacing;
begin
  ...
  if ALineSpacingType.IsNull or
    not(ALineSpacingType.Value in [TdxParagraphLineSpacing.Multiple,
    TdxParagraphLineSpacing.Exactly, TdxParagraphLineSpacing.AtLeast]) then
    ccbLineSpacing.ItemIndex := Ord(TdxParagraphLineSpacing.Multiple);
end;

procedure TdxRichEditParagraphDialogForm.Initialize(AControllerParameters
...
begin
  ...
  InitializeMeasurementUnitEdit(seLeftIndent, ToMeasurementType(UnitType),
    TdxMeasurementUnitEditHelper.Create(UnitTypeDescription, 0.1, 2, AMinIndent,
    AMaxIndent));
  ...
  InitializeMeasurementUnitEdit(seFirstLineIndent, ToMeasurementType(UnitType),
    TdxMeasurementUnitEditHelper.Create(UnitTypeDescription, 0.1, 2, 0,
    AMaxIndent));

  InitializeMeasurementUnitEdit(seSpacingBefore, TdxMeasurementType.mtPoint,
    TdxMeasurementUnitEditHelper.Create(GetUnitTypeDescription
    (TdxMeasurementUnit.Point), 6, 2, 0, 1584));
...
begin
  ...
  lcgIndentsAndSpacing.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogIndentsAndSpacing);
  lcgLineAndPageBreaks.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogLineAndPageBreaks);
  lciAlignment.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogAlignment);
  lciOutlineLevel.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogOutlineLevel);
  ...
  lciSpecial.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogSpecial);
  lciBy.CaptionOptions.Text := cxGetResourceString
    (@sdxRichEditParagraphDialogBy);
  lciBefore.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogBefore);
  lciLineSpacing.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogLineSpacing);
  lciAfter.CaptionOptions.Text := cxGetResourceString
    (@sdxRichEditParagraphDialogAfter);
  lciAtSpacing.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditParagraphDialogAt);
...
function TdxRichEditParagraphDialogForm.GetController
  : TdxParagraphFormController;
begin
  Result := TdxParagraphFormController(inherited Controller);
end;

function TdxRichEditParagraphDialogForm.GetUnitConverter
...
begin
  ...
  try
    ...
    else if not Controller.FirstLineIndent.IsNull and
      not Controller.FirstLineIndentType.IsNull and
      (Controller.FirstLineIndentType.Value = TdxParagraphFirstLineIndent.
      Hanging) then
      SetValueToEditor(seLeftIndent, Controller.LeftIndent.Value -
        Controller.FirstLineIndent.Value)
    else
      SetValueToEditor(seLeftIndent, Controller.LeftIndent.Value);
    ...
    if Controller.FirstLineIndent.IsNull or
      (not Controller.FirstLineIndentType.IsNull and
      (Controller.FirstLineIndentType.Value = TdxParagraphFirstLineIndent.None))
    then
      SetValueToEditor(seFirstLineIndent, Null)
    ...
    if Controller.FirstLineIndentType.IsNull then
      ...
      ccbFirstLineIndentType.ItemIndex :=
        Ord(Controller.FirstLineIndentType.Value);
  finally
    EndUpdate;
  end;
end;
...
begin
  ...
  try
    ...
    if Controller.LineSpacing.IsNull or IsLineSpacingInsignificant
      (Controller.LineSpacingType) then
      SetValueToEditor(seAtSpacing, Null)
    else
...
begin
  if not cbContextualSpacing.CheckBoxOptions.AllowGrayed or
    (cbContextualSpacing.State <> TcxCheckBoxState.cbsGrayed) then
...
begin
  if not cbKeepLinesTogether.CheckBoxOptions.AllowGrayed or
    (cbKeepLinesTogether.State <> TcxCheckBoxState.cbsGrayed) then
...
begin
  if not cbPageBreakBefore.CheckBoxOptions.AllowGrayed or
    (cbPageBreakBefore.State <> TcxCheckBoxState.cbsGrayed) then
...
begin
  ...
  if not Controller.FirstLineIndent.IsNull and
    (Controller.FirstLineIndent.Value = 0) then
    Controller.FirstLineIndentType.Value := TdxParagraphFirstLineIndent.None;

  if not Controller.FirstLineIndentType.IsNull and
    (Controller.FirstLineIndentType.Value = TdxParagraphFirstLineIndent.Hanging)
    and not Controller.FirstLineIndent.IsNull and not Controller.LeftIndent.IsNull
  then
    Controller.LeftIndent.Value := Controller.LeftIndent.Value +
      Controller.FirstLineIndent.Value;
end;
...
begin
  ...
  if not Controller.LineSpacing.IsNull and
    not Controller.LineSpacingType.IsNull and
  ...
  begin
    ...
    else if SameValue(Controller.LineSpacing.Value, 1.5) then
      Controller.LineSpacingType.Value := TdxParagraphLineSpacing.Sesquialteral
    else if SameValue(Controller.LineSpacing.Value, 2) then
      Controller.LineSpacingType.Value := TdxParagraphLineSpacing.Double;
  end;
  if not Controller.LineSpacingType.IsNull and
    not(Controller.LineSpacingType.Value in [TdxParagraphLineSpacing.Multiple,
    TdxParagraphLineSpacing.Exactly, TdxParagraphLineSpacing.AtLeast]) then
    Controller.LineSpacing.Reset;
end;
...
function TdxRichEditParagraphDialogForm.ValidateFromControls: Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        Result := False;
        MessageDlg(cxGetResourceString(@sdxRichEditInvalidMeasurement),
          mtWarning, [mbOK], 0);
      end
      else if VarIsNull(GetValueFromEditor(AEditor)) then
      begin
        Result := False;
        AHelper := GetMeasurementUnitEditHelper(AEditor);
        MessageDlg
          (Format(cxGetResourceString(@sdxRichEditInvalidMeasurementValue),
          [AHelper.GetTextFromValue(AHelper.MinValue),
          AHelper.GetTextFromValue(AHelper.MaxValue)]), mtWarning, [mbOK], 0);
      end
      else
        Result := True;
    end;
...
begin
  Result := DoValidateEditor(seLeftIndent) and DoValidateEditor(seRightIndent)
    and DoValidateEditor(seFirstLineIndent) and
    DoValidateEditor(seSpacingBefore) and DoValidateEditor(seSpacingAfter) and
...
begin
  ccbFirstLineIndentType.Properties.OnEditValueChanged :=
    ccbFirstLineIndentTypePropertiesEditValueChanged;
  seFirstLineIndent.Properties.OnChange :=
    seFirstLineIndentPropertiesPropertiesChange;
  seFirstLineIndent.Properties.OnValidate :=
    seFirstLineIndentPropertiesValidate;
  ccbLineSpacing.Properties.OnEditValueChanged :=
    ccbLineSpacingPropertiesEditValueChanged;
...
begin
  Populate(edtAlignment, procedure(ACombobox: TcxCustomComboBox)
    ...
    begin
      for AAlignment := Low(TdxParagraphAlignment)
        to High(TdxParagraphAlignment) do
        ACombobox.Properties.Items.Add(cxGetResourceString
          (dxParagraphAlignmentNames[AAlignment]));
    end);
end;
...
begin
  Populate(ccbFirstLineIndentType, procedure(ACombobox: TcxCustomComboBox)
    ...
    begin
      for AFirstLineIndent := Low(TdxParagraphFirstLineIndent)
        to High(TdxParagraphFirstLineIndent) do
        ACombobox.Properties.Items.Add(cxGetResourceString
          (dxParagraphFirstLineIndentNames[AFirstLineIndent]));
    end);
end;
...
begin
  Populate(ccbLineSpacing, procedure(ACombobox: TcxCustomComboBox)
    ...
    begin
      for ALineSpacing := Low(TdxParagraphLineSpacing)
        to High(TdxParagraphLineSpacing) do
        ACombobox.Properties.Items.Add(cxGetResourceString
          (dxParagraphLineSpacingNames[ALineSpacing]));
    end);
end;
...
begin
  Populate(ccbOutlineLevel, procedure(ACombobox: TcxCustomComboBox)
    var
      AIndex: Integer;
    begin
      for AIndex := Low(dxParagraphOutlineLevelNames)
        to High(dxParagraphOutlineLevelNames) do
        ACombobox.Properties.Items.Add(cxGetResourceString
          (dxParagraphOutlineLevelNames[AIndex]));
    end);
end;

procedure TdxRichEditParagraphDialogForm.RefreshLineSpacingMeasurementUnit
  (const ALineSpacingType: TdxNullableParagraphLineSpacing;
AForceUpdate: Boolean = False);
begin
  if not AForceUpdate and
    (ALineSpacingType.IsNull = FLastLineSpacingType.IsNull) and
    (ALineSpacingType.IsNull or
    (ALineSpacingType.Value = FLastLineSpacingType.Value)) then
    Exit;
  if not ALineSpacingType.IsNull and
    (ALineSpacingType.Value in [TdxParagraphLineSpacing.Exactly,
    TdxParagraphLineSpacing.AtLeast]) then
    InitializeMeasurementUnitEdit(seAtSpacing, TdxMeasurementType.mtPoint,
      TdxMeasurementUnitEditHelper.Create(GetUnitTypeDescription
      (TdxMeasurementUnit.Point), 1, 2, 1, 1584))
  else
    InitializeMeasurementUnitEdit(seAtSpacing, TdxMeasurementType.mtCustom,
      TdxMeasurementUnitEditHelper.Create('', 0.5, 2, MinMultiplierValue,
      MaxMultiplierValue));
end;
