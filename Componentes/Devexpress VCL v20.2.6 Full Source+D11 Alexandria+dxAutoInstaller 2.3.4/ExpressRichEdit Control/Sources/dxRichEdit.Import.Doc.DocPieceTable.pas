...
type
  ...
  protected
    ...
    function GetPieceDescriptors(const APieceTable: TBytes)
      : TArray<TdxPieceDescriptor>;
  public
    constructor Create;
    destructor Destroy; override;
    class function FromByteArray(const APieceTable: TBytes)
      : TdxDocPieceTable; static;
    class function CreateDefault(ATextStartOffset: Integer;
      ALastCharacterPosition: Integer): TdxDocPieceTable; static;
    ...
    procedure AddEntry(ACharacterPosition: Integer;
      APieceDescriptor: TdxPieceDescriptor);
...
class function TdxDocPieceTable.FromByteArray(const APieceTable: TBytes)
  : TdxDocPieceTable;
begin
...
class function TdxDocPieceTable.CreateDefault(ATextStartOffset: Integer;
  ALastCharacterPosition: Integer): TdxDocPieceTable;
var
...
begin
  FPcdCount := (Length(APieceTable) - TdxDocConstants.CharacterPositionSize)
    div (TdxDocConstants.CharacterPositionSize + PieceDescriptorSize);
...
var
  APositionsCount, APieceDescriptorsCount, I, APieceDescriptorsOffset,
    AValue: Integer;
begin
  APositionsCount := FCharacterPositions.Count;
  APieceDescriptorsCount := FPieceDescriptors.Count;
  SetLength(Result, APositionsCount * TdxDocConstants.CharacterPositionSize +
    APieceDescriptorsCount * PieceDescriptorSize);
  for I := 0 to APositionsCount - 1 do
  begin
    AValue := FCharacterPositions[I];
    Move(AValue, Result[I * TdxDocConstants.CharacterPositionSize],
      TdxDocConstants.CharacterPositionSize);
  end;
  APieceDescriptorsOffset := APositionsCount *
    TdxDocConstants.CharacterPositionSize;
  for I := 0 to APieceDescriptorsCount - 1 do
    TArray.Copy<Byte>(FPieceDescriptors[I].ToByteArray, Result, 0,
      APieceDescriptorsOffset + I * PieceDescriptorSize, PieceDescriptorSize);
end;

function TdxDocPieceTable.GetCharacterPositions(const APieceTable: TBytes)
  : TArray<Integer>;
var
  I: Integer;
begin
...
function TdxDocPieceTable.GetPieceDescriptors(const APieceTable: TBytes)
  : TArray<TdxPieceDescriptor>;
var
...
begin
  ...
  begin
    ...
    TArray.Copy<Byte>(APieceTable, APieceDescriptor, (I shl 3) + APcdOffset, 0,
      TdxPieceDescriptor.PieceDescriptorSize);
...
var
  ALength, AFc: Integer;
begin
  ALength := FCharacterPositions[APcdIndex + 1] - FCharacterPositions
    [APcdIndex];
...
procedure TdxDocPieceTable.AddEntry(ACharacterPosition: Integer;
  APieceDescriptor: TdxPieceDescriptor);
begin
