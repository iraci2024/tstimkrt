...
uses
  ...
  Forms, Dialogs, ActnList, Menus, StdCtrls,
  dxCore, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
  dxLayoutContainer, dxLayoutControl,
  dxLayoutControlAdapters, cxButtons, dxLayoutLookAndFeels, cxClasses,
...
type
  ...
  protected
    ...
    procedure OnSearchComplete(ASender: TObject;
      E: TdxSearchCompleteEventArgs); overload;
  ...
  TdxRichEditSearchTextDialogForm = class(TdxRichEditCustomDialogForm)
    ...
    procedure ReplaceStringAddingMRUItem(Sender: TObject;
  ...
  public
    property ActivePage: TdxSearchFormActivePage read GetActivePage
      write SetActivePage;
...
type
  ...
  public
    function AddMenuItem(AEditor: TcxCustomEdit; ACaptionId: Pointer;
      const AInsertStr: string; AShowInsertStr: Boolean = True)
      : TMenuItem; overload;
    function AddMenuItem(AEditor: TcxCustomEdit;
      const ACaption, AInsertStr: string; AShowInsertStr: Boolean = True)
      : TMenuItem; overload;
...
procedure TdxSearchHelper.OnSearchComplete(ASender: TObject;
...
function TdxSearchHelper.ShowDialog(const AMessage: string;
  AFlags: Integer): Integer;
begin
  Result := Application.MessageBox(PChar(AMessage),
    PChar(Application.Title), AFlags);
end;
...
function TdxFindAndReplaceFormController.CreateSearchHelper
  : TdxSearchHelperBase;
begin
...
begin
  if not FIsReplaceStringChange then
    Exit;
  if (Controller.ReplaceString <> '') and
    (ReplaceStrings.IndexOf(Controller.ReplaceString) = -1) then
...
procedure TdxRichEditSearchTextDialogForm.cmbFndFindDirectionPropertiesChange
  (Sender: TObject);
begin
  Controller.Direction := GetSearchDirection(cmbFndFindDirection);
end;

procedure TdxRichEditSearchTextDialogForm.cmbRplFindDirectionPropertiesChange
  (Sender: TObject);
begin
  Controller.Direction := GetSearchDirection(cmbRplFindDirection);
end;

class procedure TdxRichEditSearchTextDialogForm.CopyToMRUList(ASource,
...
function TdxRichEditSearchTextDialogForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxFindAndReplaceFormController.Create
    (AControllerParameters as TdxSearchFormControllerParameters);
end;

procedure TdxRichEditSearchTextDialogForm.CreateParams
...
procedure TdxRichEditSearchTextDialogForm.RegExButtonsEnabled
  (AChecked: Boolean);
begin
...
procedure TdxRichEditSearchTextDialogForm.edSearchStringPropertiesChange
  (Sender: TObject);
var
  AText: string;
begin
...
procedure TdxRichEditSearchTextDialogForm.SearchStringAddingMRUItem
...
procedure TdxRichEditSearchTextDialogForm.
  edRplReplaceStringPropertiesButtonClick(Sender: TObject);
begin
  if aRegex.Checked then
    pmReplaceRegex.Popup(Sender as TControl);
end;

procedure TdxRichEditSearchTextDialogForm.edRplReplaceStringPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxRichEditSearchTextDialogForm.ReplaceStringAddingMRUItem
  (Sender: TObject; const AText: TCaption);
begin
...
procedure TdxRichEditSearchTextDialogForm.edSearchStringPropertiesButtonClick
  (Sender: TObject);
begin
  if aRegex.Checked then
  begin
...
procedure TdxRichEditSearchTextDialogForm.aFindWholeWordExecute
  (Sender: TObject);
begin
...
function TdxRichEditSearchTextDialogForm.GetController
  : TdxFindAndReplaceFormController;
begin
...
function TdxRichEditSearchTextDialogForm.GetSearchDirection
  (AComboBox: TcxComboBox): TdxTextSearchDirection;
begin
...
begin
  ...
  cmbRplFindDirection.Properties.Items.Assign
    (cmbFndFindDirection.Properties.Items);
  cmbFndFindDirection.ItemIndex := Ord(TdxTextSearchDirection.All);
  cmbRplFindDirection.ItemIndex := Ord(TdxTextSearchDirection.All);
end;

procedure TdxRichEditSearchTextDialogForm.PopulateReplaceStringComboBox
  (AComboBox: TcxCustomComboBox);
begin
  CopyToMRUList(ReplaceStrings, AComboBox.ActiveProperties.LookupItems);
end;

procedure TdxRichEditSearchTextDialogForm.PopulateSearchStringComboBox
  (AComboBox: TcxCustomComboBox);
begin
...
var
  ...
  I: Integer;
begin
  ATaggedExpression := cxGetResourceString
    (@sdxRichEditSearchTextDialogTaggedExpression);
  for I := 1 to 9 do
    pmReplaceRegex.AddMenuItem(edRplReplaceString,
      Format('%s %d', [ATaggedExpression, I]), Format('$%d', [I]), False);
end;

procedure TdxRichEditSearchTextDialogForm.SearchRegexPopupMenuBuild;
begin
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogAnySingleCharacter), '.');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogZeroOrMore), '*');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogOneOrMore), '+');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogBeginningOfLine), '^');
  pmSearchRegex.AddMenuSeparator;
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogEndOfLine), '$');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogBeginningOfWord), '\b');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString
  ...
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogOr), '|');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString
    (@sdxRichEditSearchTextDialogEscapeSpecialCharacter), '\');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogTagExpression), '( )');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogWordCharacter), '\w');
  pmSearchRegex.AddMenuSeparator;
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogSpaceOrTab), '\s');
  pmSearchRegex.AddMenuItem(nil,
    cxGetResourceString(@sdxRichEditSearchTextDialogInteger), '\d');
end;

procedure TdxRichEditSearchTextDialogForm.SetActivePage
  (const Value: TdxSearchFormActivePage);
begin
...
procedure TdxRichEditSearchTextDialogForm.UpdateMRUList(AMRUEdit: TcxMRUEdit;
const AText: string);
var
  AList: TStrings;
  I: Integer;
...
begin
  ...
  aReplaceNext.Caption := cxGetResourceString
    (@sdxRichEditSearchTextDialogButtonReplaceNext);
  aReplaceAll.Caption := cxGetResourceString
    (@sdxRichEditSearchTextDialogButtonReplaceAll);
  aFindNext.Caption := cxGetResourceString
    (@sdxRichEditSearchTextDialogButtonFindNext);

  aMatchCase.Caption := cxGetResourceString
    (@sdxRichEditSearchTextDialogMatchCase);
  aFindWholeWord.Caption := cxGetResourceString
  ...
  lcgFind.CaptionOptions.Text := cxGetResourceString
    (@sdxRichEditSearchTextDialogFind);
  lcgReplace.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSearchTextDialogReplace);
  lciFndSearchString.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSearchTextDialogSearchString);
  lcilFndDirection.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSearchTextDialogDirection);
  lciRplSearchString.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSearchTextDialogSearchString);
  lciRplReplaceString.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSearchTextDialogRplReplaceString);
  lciRplDirection.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSearchTextDialogDirection);
...
procedure TdxRichEditSearchTextDialogForm.lcgTabControlTabChanged
  (Sender: TObject);
begin
...
function TSearchTextDialogPopupMenuHelper.AddMenuItem(AEditor: TcxCustomEdit;
ACaptionId: Pointer; const AInsertStr: string; AShowInsertStr: Boolean)
  : TMenuItem;
begin
  Result := AddMenuItem(AEditor, cxGetResourceString(ACaptionId), AInsertStr,
    AShowInsertStr);
end;

function TSearchTextDialogPopupMenuHelper.AddMenuItem(AEditor: TcxCustomEdit;
const ACaption, AInsertStr: string; AShowInsertStr: Boolean): TMenuItem;
begin
  Result := TInsertRegexMenuItem.Create(Self, nil, AEditor, ACaption,
    AInsertStr, AShowInsertStr);
  Items.Add(Result);
end;
