...
type
  ...
  TdxRichEditViewType = (Simple, Draft, PrintLayout);

  TdxRichEditBaseValueSource = (Auto, Document, Control);

  TdxPageBreakInsertMode = (NewLine, CurrentLine);

  TdxLineBreakSubstitute = (None, Paragraph, Space);

  TdxFieldsHighlightMode = (Auto, Never, Always);

  TdxRichEditTableGridLinesVisibility = (Auto, Visible, Hidden);

  TdxRichEditBookmarkVisibility = (Auto, Visible, Hidden);
  ...
  TdxPageFormattedEvent = procedure(Sender: TObject;
    E: TdxPageFormattedEventArgs) of object;
  ...
  TdxDocumentLayoutInvalidatedEvent = procedure(Sender: TObject;
    E: TdxDocumentLayoutInvalidatedEventArgs) of object;
  TdxDocumentLayoutInvalidatedEventHandler =
    TdxMulticastMethod<TdxDocumentLayoutInvalidatedEvent>;
  ...
  strict private
    ...
    FShift: TShiftState;
  public
    constructor Create(ACommand: TdxRichEditCommandId; AKeyData: Word;
      AAltGr: Boolean; const AShift: TShiftState);
  ...
  TdxRichEditShortCutEvent = procedure(Sender: TObject;
    const Args: TdxRichEditShortCutEventArgs) of object;
  ...
  public
    constructor Create(const AHyperlink: IdxRichEditHyperlink;
  ...
  TdxHyperlinkClickEvent = procedure(Sender: TObject;
    const Args: TdxHyperlinkClickEventArgs) of object;
  TdxHyperlinkClickEventHandler = TdxMulticastMethod<TdxHyperlinkClickEvent>;
  TdxCloseQueryEvent = procedure(Sender: TObject; var CanClose: Boolean)
    of object;
  ...
  public
    constructor Create(const AMergeFieldsNames
      : TArray<IdxRichEditMergeFieldName>);

    property MergeFieldsNames: TArray<IdxRichEditMergeFieldName>
      read FMergeFieldsNames write FMergeFieldsNames;
  end;

  TdxCustomizeMergeFieldsEvent = procedure(Sender: TObject;
    const Args: TdxCustomizeMergeFieldsEventArgs) of object;
  TdxCustomizeMergeFieldsEventHandler =
    TdxMulticastMethod<TdxCustomizeMergeFieldsEvent>;
  ...
  public
    ...
    class function CreateDocumentServerForExistingDocumentModel
      (const ADocumentServerOwner: IdxInnerRichEditDocumentContainerOwner;
      ADocumentModel: TdxCustomDocumentModel)
      : IdxRichEditDocumentContainer; static;
    procedure Clear;

    property OperationDescription: string read FOperationDescription
      write FOperationDescription;
    property Document: IdxRichEditDocument read GetDocument;
  end;

  TdxMailMergeStartedEventArgs = class(TdxMailMergeCustomEventArgs);
  TdxMailMergeStartedEvent = procedure(Sender: TObject;
    const Args: TdxMailMergeStartedEventArgs) of object;
  TdxMailMergeStartedEventHandler =
    TdxMulticastMethod<TdxMailMergeStartedEvent>;

  TdxMailMergeFinishedEventArgs = class(TdxMailMergeCustomEventArgs);
  TdxMailMergeFinishedEvent = procedure(Sender: TObject;
    const Args: TdxMailMergeFinishedEventArgs) of object;
  TdxMailMergeFinishedEventHandler =
    TdxMulticastMethod<TdxMailMergeFinishedEvent>;
  ...
  public
    property TargetDocument: IdxRichEditDocument read FTargetDocument
      write FTargetDocument;
  end;

  TdxMailMergeGetTargetDocumentEvent = procedure(Sender: TObject;
    const Args: TdxMailMergeGetTargetDocumentEventArgs) of object;

  TdxMailMergeGetTargetDocumentEventHandler =
    TdxMulticastMethod<TdxMailMergeGetTargetDocumentEvent>;
  ...
  TdxMailMergeRecordStartedEvent = procedure(Sender: TObject;
    const Args: TdxMailMergeRecordStartedEventArgs) of object;
  TdxMailMergeRecordStartedEventHandler =
    TdxMulticastMethod<TdxMailMergeRecordStartedEvent>;

  TdxMailMergeRecordFinishedEventArgs = class
    (TdxMailMergeRecordCustomEventArgs);
  TdxMailMergeRecordFinishedEvent = procedure(Sender: TObject;
    const Args: TdxMailMergeRecordFinishedEventArgs) of object;
  TdxMailMergeRecordFinishedEventHandler =
    TdxMulticastMethod<TdxMailMergeRecordFinishedEvent>;
  ...
  TdxCalculateDocumentVariableEvent = procedure(Sender: TObject;
    E: TdxCalculateDocumentVariableEventArgs) of object;
  TdxCalculateDocumentVariableEventHandler =
    TdxMulticastMethod<TdxCalculateDocumentVariableEvent>;
...
constructor TdxHyperlinkClickEventArgs.Create(const AHyperlink
  : IdxRichEditHyperlink; AModifierKeys: TShortCut);
begin
  inherited Create;
...
constructor TdxCustomizeMergeFieldsEventArgs.Create(const AMergeFieldsNames
  : TArray<IdxRichEditMergeFieldName>);
begin
  inherited Create;
...
constructor TdxMailMergeCustomEventArgs.Create(const ADocumentServerOwner
  : IdxInnerRichEditDocumentContainerOwner;
  ATargetDocumentModel: TdxCustomDocumentModel);
begin
  inherited Create;
...
begin
  ...
  if FTargetServer = nil then
    FTargetServer := CreateDocumentServerForExistingDocumentModel
      (FDocumentServerOwner, FTargetDocumentModel);
  Result := FTargetServer.Document;
end;

class function TdxMailMergeCustomEventArgs.
  CreateDocumentServerForExistingDocumentModel(const ADocumentServerOwner
  : IdxInnerRichEditDocumentContainerOwner;
...
constructor TdxMailMergeRecordCustomEventArgs.Create(const ADocumentServerOwner
  : IdxInnerRichEditDocumentContainerOwner;
  ATargetDocumentModel, ARecordDocumentModel: TdxCustomDocumentModel);
begin
  inherited Create;
...
begin
  if FTargetDocumentModel = nil then
    Exit(nil);
  if FTargetServer = nil then
    FTargetServer := TdxMailMergeCustomEventArgs.
      CreateDocumentServerForExistingDocumentModel(FDocumentServerOwner,
      FTargetDocumentModel);
  Result := FTargetServer.Document;
end;

function TdxMailMergeRecordCustomEventArgs.GetRecordDocument
  : IdxRichEditDocument;
begin
  if FRecordDocumentModel = nil then
    Exit(nil);
  if FRecordServer = nil then
    FRecordServer := TdxMailMergeCustomEventArgs.
      CreateDocumentServerForExistingDocumentModel(FDocumentServerOwner,
      FRecordDocumentModel);
...
constructor TdxCalculateDocumentVariableEventArgs.Create(const AVariableName
  : string; AArguments: TdxArgumentCollection);
begin
  inherited Create;
