...
function TdxRtfDocumentExporter.FormatEquals(const AValue
  : TdxRichEditDocumentFormat): Boolean;
begin
  Result := Format = AValue;
end;

function TdxRtfDocumentExporter.SaveDocument(const ADocumentModel
...
begin
  AModel.InternalAPI.SaveDocumentRtfContent(AStream,
    TdxRtfDocumentExporterOptions(AOptions));
  Result := True;
end;
