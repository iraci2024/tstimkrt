...
uses
  ...
  cxClasses, ActiveX, dxCoreClasses,
...
type
  ...
  public
    ...
    function CanActivate(const AMousePosition: TPoint): Boolean;
      virtual; abstract;
    procedure PerformAutoScroll; virtual; abstract;
    function CalculateHotZoneBounds: TRect; virtual; abstract;
    function AdjustHotZoneBounds(const ABounds: TRect;
      const AMousePosition: TPoint): TRect; virtual; abstract;

    property Bounds: TRect read FBounds write FBounds;
  end;

  TdxAutoScrollerHotZoneCollection = class
    (TdxObjectList<TdxAutoScrollerHotZone>);
...
type
  ...
  TdxRichEditVerticalScrollHotZone = class abstract
    (TdxRichEditAutoScrollerHotZone)
  protected
    function CalculatePhysicalOffset(const Args: TdxMouseEventArgs): Integer;
      virtual; abstract;
  ...
  TdxRichEditVerticalScrollForwardHotZone = class
    (TdxRichEditVerticalScrollHotZone)
  protected
    function CalculatePhysicalOffset(const Args: TdxMouseEventArgs)
      : Integer; override;
  public
    function AdjustHotZoneBounds(const ABounds: TRect;
      const AMousePosition: TPoint): TRect; override;
  ...
  TdxRichEditVerticalScrollBackwardHotZone = class
    (TdxRichEditVerticalScrollHotZone)
  protected
    function CalculatePhysicalOffset(const Args: TdxMouseEventArgs)
      : Integer; override;
  ...
  TdxRichEditHorizontalScrollHotZone = class abstract
    (TdxRichEditAutoScrollerHotZone)
  protected
    function CalculatePhysicalOffset(const Args: TdxMouseEventArgs): Integer;
      virtual; abstract;
  ...
  TdxRichEditHorizontalScrollForwardHotZone = class
    (TdxRichEditHorizontalScrollHotZone)
  protected
    function CalculatePhysicalOffset(const Args: TdxMouseEventArgs)
      : Integer; override;
  ...
  TdxRichEditHorizontalScrollBackwardHotZone = class
    (TdxRichEditHorizontalScrollHotZone)
  protected
    function CalculatePhysicalOffset(const Args: TdxMouseEventArgs)
      : Integer; override;
  ...
  TdxRichEditAutoScrollerHotZoneHelper = class helper for
    TdxRichEditAutoScrollerHotZone
  private
...
function TdxRichEditAutoScrollerHotZoneHelper.GetController
...
function TdxAutoScrollerHotZone.Initialize(const AMousePosition
  : TPoint): Boolean;
begin
...
function TdxAutoScroller.CalculateActiveHotZone(const P: TPoint)
  : TdxAutoScrollerHotZone;
var
  ACount, I: Integer;
begin
...
begin
  if AMousePosition.Y >= ABounds.Top then
    Result.Init(ABounds.Left, AMousePosition.Y + 1, ABounds.Right,
      ABounds.Bottom)
  else
    Result := ABounds;
end;
...
begin
  AUnitConverter := Control.InnerControl.DocumentModel.LayoutUnitConverter;
  if (Args.MousePos.Y - Bounds.Top) > AUnitConverter.PixelsToLayoutUnits(10,
    TdxDocumentModel.DpiY) then
...
begin
  ...
  if AGenerator.TopInvisibleHeight >= AGenerator.TotalHeight - AGenerator.VisibleHeight
  then
    Result := False
  else
...
begin
  AUnitConverter := Control.InnerControl.DocumentModel.LayoutUnitConverter;
  if (Bounds.Bottom - Args.MousePos.Y) > AUnitConverter.PixelsToLayoutUnits(10,
    TdxDocumentModel.DpiY) then
    Result := -AUnitConverter.DocumentsToLayoutUnits(150)
  else
    Result := -AUnitConverter.DocumentsToLayoutUnits(50);
...
begin
  if AMousePosition.X >= ABounds.Left then
    Result.Init(AMousePosition.X + 1, ABounds.Top, ABounds.Right,
      ABounds.Bottom)
  else
    Result := ABounds;
end;

function TdxRichEditHorizontalScrollForwardHotZone.
  CalculateHotZoneBounds: TRect;
var
  AWidth: Integer;
begin
  Result := CalculateCompatibleViewBounds;
...
begin
  AUnitConverter := Control.InnerControl.DocumentModel.LayoutUnitConverter;
  if (Args.MousePos.X - Bounds.Left) > AUnitConverter.PixelsToLayoutUnits(10,
    TdxDocumentModel.DpiX) then
    Result := AUnitConverter.DocumentsToLayoutUnits(150)
  else
    Result := AUnitConverter.DocumentsToLayoutUnits(50);
end;
...
begin
  ...
  if AGenerator.VisibleWidth >= AGenerator.TotalWidth then
    Exit(False);
  if AGenerator.LeftInvisibleWidth >= AGenerator.TotalWidth - AGenerator.VisibleWidth
  then
    Result := False
  else
...
function TdxRichEditHorizontalScrollBackwardHotZone.
  CalculateHotZoneBounds: TRect;
begin
  Result := CalculateCompatibleViewBounds;
...
begin
  AUnitConverter := Control.InnerControl.DocumentModel.LayoutUnitConverter;
  if (Bounds.Right - Args.MousePos.X) > AUnitConverter.PixelsToLayoutUnits(10,
    TdxDocumentModel.DpiX) then
    Result := -AUnitConverter.DocumentsToLayoutUnits(150)
  else
    Result := -AUnitConverter.DocumentsToLayoutUnits(50);
