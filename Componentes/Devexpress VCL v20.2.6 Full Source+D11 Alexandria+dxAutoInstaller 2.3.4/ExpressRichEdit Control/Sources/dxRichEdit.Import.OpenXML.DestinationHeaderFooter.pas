...
type
  ...
  TdxHeaderFooterDestinationBase<T: TdxSectionHeaderFooterBase> = class abstract
    (TdxBodyDestinationBase)
  strict private
    FNewHeaderFooter: T;
  protected
    function CalculateTargetHeaderFooter(AReader: TdxXmlReader): T;
      virtual; abstract;
  public
  ...
  TdxHeaderFooterDestination<T: TdxSectionHeaderFooterBase> = class abstract
    (TdxHeaderFooterDestinationBase<T>)
  strict private
  ...
  TdxHeaderFooterReferenceDestinationBase<T: TdxSectionHeaderFooterBase> = class
    abstract(TdxLeafElementDestination)
  protected type
    TdxHeaderFooterTypeReference = (Unknown, Default, First, Even);
  protected
    function GetHeaderFooterTypeReference(const AType: string)
    ...
    function CreatePieceTableDestination(APieceTable: T)
      : TdxHeaderFooterDestination<T>; virtual; abstract;
  ...
  TdxHeaderReferenceDestination = class
    (TdxHeaderFooterReferenceDestinationBase<TdxSectionHeader>)
  protected
    function GetRootPieceTableTag: string; override;
    function CreatePieceTableDestination(APieceTable: TdxSectionHeader)
      : TdxHeaderFooterDestination<TdxSectionHeader>; override;
    function CreatePieceTable(const AType: string): TdxSectionHeader; override;
  end;

  { TdxFooterReferenceDestination }

  TdxFooterReferenceDestination = class
    (TdxHeaderFooterReferenceDestinationBase<TdxSectionFooter>)
  protected
    function GetRootPieceTableTag: string; override;
    function CreatePieceTableDestination(APieceTable: TdxSectionFooter)
      : TdxHeaderFooterDestination<TdxSectionFooter>; override;
  ...
  protected
    function CalculateTargetHeaderFooter(AReader: TdxXmlReader)
...
procedure TdxHeaderFooterDestinationBase<T>.ProcessElementOpen
...
procedure TdxHeaderFooterDestinationBase<T>.ProcessElementClose
  (AReader: TdxXmlReader);
begin
...
class function TdxHeaderFooterDestination<T>.CreateElementHandlerTable
...
function TdxHeaderFooterDestination<T>.GetElementHandlerTable
  : TdxElementHandlerTable;
begin
  Result := FHandlerTable;
end;

class function TdxHeaderFooterDestination<T>.OnTable
...
class function TdxHeaderFooterDestination<T>.OnParagraph
  (AImporter: TdxRichEditDestinationAndXmlBasedImporter; AReader: TdxXmlReader)
  : TdxDestination;
begin
...
function TdxHeaderFooterReferenceDestinationBase<T>.GetHeaderFooterTypeReference
  (const AType: string): TdxHeaderFooterTypeReference;
begin
  if (AType = '') or (AType = 'default') then
    Result := TdxHeaderFooterTypeReference.Default
  else if AType = 'first' then
    Result := TdxHeaderFooterTypeReference.First
  else if AType = 'even' then
    Result := TdxHeaderFooterTypeReference.Even
  else
    Result := TdxHeaderFooterTypeReference.Unknown;
end;

function TdxHeaderFooterReferenceDestinationBase<T>.GetImporter
  : TdxOpenXmlImporter;
begin
  Result := TdxOpenXmlImporter(inherited Importer);
end;

procedure TdxHeaderFooterReferenceDestinationBase<T>.ProcessElementOpen
  (AReader: TdxXmlReader);
var
...
begin
  ...
  APieceTableStream := AImporter.LookupPackageFileStreamByRelationId(AId,
    AImporter.DocumentRootFolder, True);
  if APieceTableStream = nil then
    Importer.ThrowInvalidFile;

  AHeaderFooterFileName := Importer.LookupRelationTargetById
    (Importer.DocumentRelations, AId, AImporter.DocumentRootFolder, '');
  Importer.DocumentRelationsStack.Push
    (Importer.ImportRelations(Importer.DocumentRootFolder + '/_rels/' +
    TPath.GetFileName(AHeaderFooterFileName) + '.rels'));
  Importer.PackageFileStreamsStack.Push(TdxPackageFileStreams.Create);
  Importer.PackageImagesStack.Push
  ...
  try
    ...
    ANewPieceTable := CreatePieceTable(AReader.GetAttribute('type',
...
function TdxHeaderReferenceDestination.CreatePieceTableDestination
  (APieceTable: TdxSectionHeader): TdxHeaderFooterDestination<TdxSectionHeader>;
begin
  Result := TdxHeaderDestination.Create(Importer, APieceTable);
end;

function TdxHeaderReferenceDestination.CreatePieceTable(const AType: string)
  : TdxSectionHeader;
var
  ASection: TdxSection;
...
function TdxFooterReferenceDestination.CreatePieceTableDestination
  (APieceTable: TdxSectionFooter): TdxHeaderFooterDestination<TdxSectionFooter>;
begin
  Result := TdxFooterDestination.Create(Importer, APieceTable);
end;

function TdxFooterReferenceDestination.CreatePieceTable(const AType: string)
  : TdxSectionFooter;
var
  ASection: TdxSection;
  ATypeValue: TdxHeaderFooterTypeReference;
...
constructor TdxHeaderDestination.Create(AImporter
  : TdxWordProcessingMLBaseImporter; AHeader: TdxSectionHeader);
begin
...
function TdxHeaderDestination.CalculateTargetHeaderFooter(AReader: TdxXmlReader)
  : TdxSectionHeader;
begin
  Result := FHeader;
end;
...
constructor TdxFooterDestination.Create(AImporter
  : TdxWordProcessingMLBaseImporter; AFooter: TdxSectionFooter);
begin
  inherited Create(AImporter);
...
function TdxFooterDestination.CalculateTargetHeaderFooter(AReader: TdxXmlReader)
  : TdxSectionFooter;
begin
  Result := FFooter;
end;
