...
type
  ...
  public
    ...
    class function TryCreate(ADocument: TdxNativeSubDocument;
      ...
      : TdxNativeDocumentImage; static;
    class function CanCreateImage(ADocument: TdxNativeSubDocument;
      ARun: TdxTextRunBase): Boolean; static;
    class function CreateUnsafe(ADocument: TdxNativeSubDocument;
      ARunIndex: TdxRunIndex): TdxNativeDocumentImage; static;
  ...
  strict private
  type
    TProcessImageProc = reference to function(ARun: TdxTextRunBase;
  ...
  public
    ...
    property Self[Index: Integer]: IdxRichEditDocumentImage
...
constructor TdxNativeDocumentImage.Create(ADocument: TdxNativeSubDocument;
...
begin
  ...
  begin
    ...
    begin
      AContent := Safe<TdxPictureFloatingObjectContent>.Cast
        (AAnchorRun.Content);
...
class function TdxNativeDocumentImage.CanCreateImage
  (ADocument: TdxNativeSubDocument; ARun: TdxTextRunBase): Boolean;
var
  AAnchorRun: TdxFloatingObjectAnchorRun;
  AContent: TdxPictureFloatingObjectContent;
begin
...
class function TdxNativeDocumentImage.CreateUnsafe
  (ADocument: TdxNativeSubDocument; ARunIndex: TdxRunIndex)
  : TdxNativeDocumentImage;
begin
  Result := TdxNativeDocumentImage.Create(ADocument, ARunIndex);
end;

class function TdxNativeDocumentImage.CreateImage(AImage: TGraphic)
  : TdxOfficeImage;
var
  AStream: TStream;
begin
...
begin
  Supports(FDocument.PieceTable.Runs[TdxNativeDocumentPosition(FRange.Start)
    .Position.RunIndex], IdxPictureContainerRun, Result);
end;
...
function TdxNativeDocumentImage.GetRectangleScalableObject
...
begin
  ...
  Result := TdxSizeF.Create(FDocument.ModelUnitsToUnits(AOriginalSize.cx),
    FDocument.ModelUnitsToUnits(AOriginalSize.cy));
end;
...
begin
  ...
  Result := TdxSizeF.Create(FDocument.ModelUnitsToUnitsF(AActualSize.cx),
    FDocument.ModelUnitsToUnitsF(AActualSize.cy));
end;
...
function TdxNativeDocumentImageCollection.Append(AImage: TGraphic)
  : IdxRichEditDocumentImage;
begin
  Result := Insert(FDocument.EndPosition, AImage);
end;

function TdxNativeDocumentImageCollection.Get(const ARange
  : IdxRichEditDocumentRange): IdxRichEditReadOnlyDocumentImageCollection;
begin
...
function TdxNativeDocumentImageCollection.GetItem(Index: Integer)
  : IdxRichEditDocumentImage;
var
  I: Integer;
...
function TdxNativeDocumentImageCollection.Insert
  (const APos: IdxRichEditDocumentPosition; AImage: TGraphic)
  : IdxRichEditDocumentImage;
var
...
begin
  ...
  try
    ...
    ARun := TdxInlinePictureRun(PieceTable.InsertInlinePicture(ALogPosition,
      AOfficeImage));
    try
      if ARun = nil then
        Exit(nil);
      PieceTable.CalculateRunInfoStart(ALogPosition, ARunInfo);
      Result := TdxNativeDocumentImage.CreateUnsafe(FDocument,
