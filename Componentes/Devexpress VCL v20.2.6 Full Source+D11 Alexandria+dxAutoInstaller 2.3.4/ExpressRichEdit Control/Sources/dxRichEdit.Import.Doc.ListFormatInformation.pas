...
type
  ...
  public
    ...
    property LevelStyleIdentifiers: TArray<SmallInt> read FLevelStyleIdentifiers
      write FLevelStyleIdentifiers;
  ...
  public
    class function FromStream(AReader: TBinaryReader)
      : TdxDocListLevelProperties; static;
    procedure Write(AWriter: TBinaryWriter);
    ...
    property RestartAfterLevelLimit: Boolean read FRestartAfterLevelLimit
      write FRestartAfterLevelLimit;
    property Alignment: TdxListNumberAlignment read FAlignment write FAlignment;
    property PlaceholderIndices: TBytes read FPlaceholderIndices
      write FPlaceholderIndices;
    ...
    property CharacterUPXSize: Byte read FCharacterUPXsize
      write FCharacterUPXsize;
    property ParagraphUPXSize: Byte read FParagraphUPXsize
      write FParagraphUPXsize;
    property RestartLevelLimit: Byte read FRelativeRestartLevel
  ...
  protected
    ...
    class function IsDoublePercent(const ADisplayFormatString: string;
      I: Integer): Boolean; static;
    procedure Read(AReader: TBinaryReader);
  ...
  public
    ...
    property ListLevelProperties: TdxDocListLevelProperties
      read FListLevelProperties write FListLevelProperties;
    ...
    property DisplayTextPlaceHolder: string read FDisplayTextPlaceHolder
      write FDisplayTextPlaceHolder;
  end;
  ...
  public
    ...
    property ListFormatting: TdxDocListFormatting read FListFormatting
      write FListFormatting;
    property LevelsFormatting: TdxObjectList<TdxDocListLevel>
      read FLevelsFormatting;
  end;
  ...
  public
    ...
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer): TdxDocListFormatInformation; static;
    procedure Write(AWriter: TBinaryWriter);
...
class function TdxDocListFormatting.FromStream(AReader: TBinaryReader)
  : TdxDocListFormatting;
begin
...
class function TdxDocListLevelProperties.FromStream(AReader: TBinaryReader)
  : TdxDocListLevelProperties;
begin
...
begin
  ...
  if ANumberFormatCode = BulletFormatCode then
    FBulletedList := True;
  FNumberingFormat := TdxNumberingFormatCalculator.CalcNumberingFormat
    (ANumberFormatCode);
  AFlags := AReader.ReadByte;
  FAlignment := TdxAlignmentCalculator.CalcListNumberAlignment
    (Byte((AFlags and $03)));
  FLevelFormatFlags := AFlags and $FC;
  FConvertPreviousLevelNumberingToDecimal :=
    (FLevelFormatFlags and TdxDocListLevelFormatFlags.
    ConvertPreviousLevelNumberingToDecimal) <> 0;
  FRestartAfterLevelLimit :=
    (FLevelFormatFlags and TdxDocListLevelFormatFlags.NoRestart) <> 0;
...
begin
  ...
  ANumberFormatCode := TdxNumberingFormatCalculator.CalcNumberingFormatCode
    (FNumberingFormat);
  ...
  if FConvertPreviousLevelNumberingToDecimal then
    FLevelFormatFlags := FLevelFormatFlags or
      TdxDocListLevelFormatFlags.ConvertPreviousLevelNumberingToDecimal;
  if FLegacy then
    FLevelFormatFlags := FLevelFormatFlags or TdxDocListLevelFormatFlags.Legacy;
  if FRestartAfterLevelLimit then
    FLevelFormatFlags := FLevelFormatFlags or
      TdxDocListLevelFormatFlags.NoRestart;
...
class function TdxDocListLevel.IsDoublePercent(const ADisplayFormatString
  : string; I: Integer): Boolean;
begin
  if I > Length(ADisplayFormatString) then
    Exit(False);
  Result := (ADisplayFormatString[I] = '%') and
    (ADisplayFormatString[I + 1] = '%');
end;

class function TdxDocListLevel.FromStream(AReader: TBinaryReader)
  : TdxDocListLevel;
begin
...
function TdxDocListLevel.GetDisplayTextPlaceHolder
  (AReader: TBinaryReader): string;
var
  ATextPlaceholderSize: Integer;
  ABuffer: TBytes;
begin
  ATextPlaceholderSize := AReader.ReadUInt16 * 2;
  if ATextPlaceholderSize >= AReader.BaseStream.Size - AReader.BaseStream.Position
  then
    Exit('');
...
begin
  ...
  try
    ADisplayString := TdxListLevelDisplayTextHelper.
...
procedure TdxDocListLevel.SetDisplayFormatString
...
begin
  DisplayTextPlaceHolder := '';
  SetLength(FListLevelProperties.FPlaceholderIndices,
    TdxDocListFormatting.MaxLevelCount);
  FCurrentLevel := 0;
...
function TdxDocListLevel.AddLevelNumber(const ADisplayFormatString: string;
  AIndex: Integer): Integer;
begin
  if IsDoublePercent(ADisplayFormatString, AIndex) then
  begin
    DisplayTextPlaceHolder := DisplayTextPlaceHolder +
      ADisplayFormatString[AIndex];
    Exit(AIndex + 2);
  end;
  Result := AddLevelNumberCore(ADisplayFormatString, AIndex);
end;

function TdxDocListLevel.AddLevelNumberCore(const ADisplayFormatString: string;
  AIndex: Integer): Integer;
var
  AValue: string;
...
begin
  ...
  DisplayTextPlaceHolder := DisplayTextPlaceHolder +
    Chr(StrToIntDef(AValue, 1));
  FListLevelProperties.PlaceholderIndices[FCurrentLevel] :=
    Byte(Length(DisplayTextPlaceHolder));
  Inc(FCurrentLevel);
...
class function TdxDocListData.FromStream(AReader: TBinaryReader)
  : TdxDocListData;
begin
...
class function TdxDocListFormatInformation.FromStream(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer): TdxDocListFormatInformation;
begin
  Result := TdxDocListFormatInformation.Create;
  Result.Read(AReader, AOffset, ASize);
end;

procedure TdxDocListFormatInformation.Read(AReader: TBinaryReader;
...
begin
  ...
  begin
    if FListData[AListIndex].ListFormatting.SimpleList then
      FListData[AListIndex].LevelsFormatting.Add
        (TdxDocListLevel.FromStream(AReader))
    else
