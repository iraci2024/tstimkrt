...
type
  ...
  protected
    ...
    procedure DrawItemRect(const R: TRect;
      AItem: TdxCustomListBoxItem); virtual;
  ...
  public
    ...
    property SelectedCharCode: Word read GetSelectedCharCode
      write SetSelectedCharCode;
    property SelectedIndexSymbol: Integer read FSelectedIndexSymbol
      write SetSelectedIndexSymbol;
    property OnSelectedIndexSymbolChanged: TNotifyEvent
      read FOnSelectedIndexSymbolChanged write FOnSelectedIndexSymbolChanged;
  end;
  ...
  protected
    procedure DrawItemRect(const R: TRect;
      AItem: TdxCustomListBoxItem); override;
...
begin
  Font.Name := Value;
  Font.Size := MulDiv(MulDiv(ItemHeight - 2 * GetLineThickness, 2, 3), 72,
    Font.PixelsPerInch);
end;
...
function TdxCustomSymbolListBox.DoMouseWheel(Shift: TShiftState;
...
procedure TdxCustomSymbolListBox.DrawItem(const R: TRect;
  ...
  var
    ...
    begin
      ...
      Result.Left := ATextRect.Left + (SelectedIndexSymbol mod ColumnCount) *
        ItemHeight;
  ...
  begin
    ...
    for I := 0 to ColumnCount - 1 do
      ...
      begin
        ...
        Canvas.DrawTexT(OrdToString(FUnicodeChars[AIndex]), ATextRect,
          GetTextFlags, True, ra0);
...
procedure TdxCustomSymbolListBox.DrawItemRect(const R: TRect;
...
begin
  if (SelectedIndexSymbol >= 0) and (SelectedIndexSymbol < FUnicodeChars.Count)
  then
...
begin
  if (SelectedIndexSymbol < 0) or (SelectedIndexSymbol >= FUnicodeChars.Count)
  then
    Result := 0
  else
...
procedure TdxCustomSymbolListBox.KeyDown(var Key: Word; Shift: TShiftState);
  ...
  begin
    ...
    begin
      SelectedIndexSymbol := FSelectedIndexSymbol - FItemsAreaRowCount *
        ColumnCount;
...
procedure TdxCustomSymbolListBox.MouseDown(Button: TMouseButton;
...
begin
  ...
  begin
    ...
    begin
      ...
      AIsCharCategoryControl :=
      {$IFDEF DELPHIXE4}AChar.IsControl{$ELSE}TCharacter.IsControl
        (AChar){$ENDIF};
      AIsCharCategoryPrivateUse :=
      {$IFDEF DELPHIXE4}AChar.GetUnicodeCategory{$ELSE}TCharacter.
        GetUnicodeCategory(AChar){$ENDIF} = TUnicodeCategory.ucPrivateUse;
      if AFontCharacterSet.ContainsChar(AChar) and
        not AIsCharCategoryControl and not AIsCharCategoryPrivateUse then
        FUnicodeChars.Add(I);
    end;
  end;
  CalculateRowCount;
...
begin
  ...
  if TdxFontCache.SystemTrueTypeFonts.TryGetValue
    ({$IFDEF DELPHIXE4}Value.ToUpper{$ELSE}ToUpper(Value){$ENDIF}, AFontInfo)
  then
  begin
    FIsSymbolFont := (AFontInfo.FontType and
      TdxTrueTypeFontInfo.SymbolFontType) <> 0;
...
procedure TdxSimpleSymbolListBox.DrawItemBackground(const R: TRect;
  AItem: TdxCustomListBoxItem; AState: TcxButtonState);
begin
...
procedure TdxSimpleSymbolListBox.DrawItemRect(const R: TRect;
  AItem: TdxCustomListBoxItem);
begin
...
begin
  ...
  Canvas.FrameRect(ARect, LookAndFeelPainter.DefaultSelectionColor,
    SelectRectangleThickness);
end;
