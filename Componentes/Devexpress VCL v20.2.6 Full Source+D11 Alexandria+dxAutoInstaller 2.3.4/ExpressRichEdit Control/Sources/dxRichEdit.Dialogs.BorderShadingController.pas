...
type
  ...
  TdxSetMode = (None, Box, All, Grid, Custom);
  ...
  protected
    ...
    procedure SetBorders(ACommandClass
      : TdxToggleTableCellsBordersCommandBaseClass; ABorder: TdxBorderInfo);
    ...
    class function AreEqual(ABorderFirst, ABorderSecond: TdxBorderInfo)
      : Boolean; static;
  public
    ...
    property BorderLineHorizontalIn: TdxBorderInfo read FBorderLineHorizontalIn
      write FBorderLineHorizontalIn;
    property BorderLineDown: TdxBorderInfo read FBorderLineDown
      write FBorderLineDown;
    property BorderLineLeft: TdxBorderInfo read FBorderLineLeft
      write FBorderLineLeft;
    property BorderLineRight: TdxBorderInfo read FBorderLineRight
      write FBorderLineRight;
    property BorderLineVerticalIn: TdxBorderInfo read FBorderLineVerticalIn
      write FBorderLineVerticalIn;
    property BorderLineHorizontalInVisible: Boolean
      read FBorderLineHorizontalInVisible write FBorderLineHorizontalInVisible;
    property BorderLineVerticalInVisible: Boolean
      read FBorderLineVerticalInVisible write FBorderLineVerticalInVisible;
...
constructor TdxBorderShadingFormControllerParameters.Create(const AControl
  : IdxRichEditControl; ADocumentModel: TdxDocumentModel;
  ASelectedCells: TdxSelectedCellsCollection);
begin
...
begin
  ...
  try
    ...
    if Assigned(BorderLineHorizontalIn) then
      SetBorders(TdxToggleTableCellsInsideHorizontalBorderCommand,
        BorderLineHorizontalIn);
    ...
    if Assigned(BorderLineVerticalIn) then
      SetBorders(TdxToggleTableCellsInsideVerticalBorderCommand,
        BorderLineVerticalIn);
...
class function TdxBorderShadingFormController.AreEqual(ABorderFirst,
  ABorderSecond: TdxBorderInfo): Boolean;
begin
  Result := (ABorderFirst.Style = ABorderSecond.Style) and
    (ABorderFirst.Color = ABorderSecond.Color) and
    (ABorderFirst.Width = ABorderSecond.Width);
end;

constructor TdxBorderShadingFormController.Create(AControllerParameters
  : TdxBorderShadingFormControllerParameters);
begin
  inherited Create;
...
var
  ...
  I: Integer;
begin
  ABorders := TdxBorderArray.Create(BorderLineUp, BorderLineRight,
    BorderLineDown, BorderLineLeft, BorderLineVerticalIn,
    BorderLineHorizontalIn);
  for I := Low(ABorders) to High(ABorders) do
    if Assigned(ABorders[I]) and (ABorders[I].Style <> TdxBorderLineStyle.None)
    then
      Exit(ABorders[I]);
...
begin
  ...
  else if IsModeStateAll then
    Result := TdxSetMode.All
  else if IsModeStateBox then
    Result := TdxSetMode.Box
  else if IsModeStateGrid then
...
begin
  ...
  BorderLineHorizontalIn :=
    AFirstInterval.StartCell.GetActualBottomCellBorder.Info;
  ...
  BorderLineVerticalIn := AFirstInterval.StartCell.
    GetActualRightCellBorder.Info;

  ARowsCount := FSelectedCells.RowsCount;
  for J := 0 to ARowsCount - 1 do
    if (FSelectedCells[J].NormalizedEndCellIndex - FSelectedCells[J]
      .NormalizedStartCellIndex + 1) >= 2 then
    begin
  ...
  begin
    ...
    for I := ASelectedCell.NormalizedStartCellIndex to ASelectedCell.
      NormalizedEndCellIndex do
  ...
  for I := AFirstInterval.NormalizedStartCellIndex to AFirstInterval.
    NormalizedEndCellIndex do
    if not AFirstInterval.Row.Cells[I].GetActualTopCellBorder.Info.Equals
      (BorderLineUp) then
    begin
      BorderLineUp := nil;
      Break;
    end;
  for I := ALastInterval.NormalizedStartCellIndex to ALastInterval.
    NormalizedEndCellIndex do
    if not ALastInterval.Row.Cells[I].GetActualBottomCellBorder.Info.Equals
      (BorderLineDown) then
    begin
      BorderLineDown := nil;
      Break;
  ...
  begin
    ...
    begin
      ...
      for I := ASelectedCell.NormalizedStartCellIndex to ASelectedCell.
        NormalizedEndCellIndex do
        if not ASelectedCell.Row.Cells[I].GetActualBottomCellBorder.Info.Equals
          (BorderLineHorizontalIn) then
        begin
  ...
  for I := 0 to ARowsCount - 1 do
    if not FSelectedCells[I].StartCell.GetActualLeftCellBorder.Info.Equals
      (BorderLineLeft) then
    begin
      BorderLineLeft := nil;
      Break;
  ...
  for I := 0 to ARowsCount - 1 do
    if not FSelectedCells[I].EndCell.GetActualRightCellBorder.Info.Equals
      (BorderLineRight) then
    begin
      BorderLineRight := nil;
      Break;
    end;
  ...
  begin
    ASelectedCell := FSelectedCells[J];
    for I := ASelectedCell.NormalizedStartCellIndex to ASelectedCell.
      NormalizedEndCellIndex - 1 do
      if not ASelectedCell.Row.Cells[I].GetActualRightCellBorder.Info.Equals
        (BorderLineVerticalIn) then
      begin
...
begin
  ...
  if (ABorders[0] = nil) or (ABorders[0].Style = TdxBorderLineStyle.None) then
    Exit(False);
  if (not BorderLineVerticalInVisible) and (not BorderLineHorizontalInVisible)
  then
    Exit(False);
...
begin
  ABorders := TdxBorderArray.Create(BorderLineUp, BorderLineRight,
    BorderLineDown, BorderLineLeft);
  ABordersIn := TdxBorderArray.Create(BorderLineVerticalIn,
    BorderLineHorizontalIn);
  if (ABorders[0] = nil) or (ABorders[0].Style = TdxBorderLineStyle.None) then
    Exit(False);
  for I := 1 to Length(ABorders) - 1 do
...
begin
  ...
  then
    Exit(False);
  for ABorder in ABordersIn do
    if (ABorder = nil) or (ABorder.Style <> TdxBorderLineStyle.Single) or
      (ABorder.Color <> ABorders[0].Color) or (ABorder.Width <> FGridWidth) then
      Exit(False);
  Result := True;
end;
...
procedure TdxBorderShadingFormController.SetBorders(ACommandClass
  : TdxToggleTableCellsBordersCommandBaseClass; ABorder: TdxBorderInfo);
var
  ...
  begin
    ...
    begin
      ...
      for I := ASelectedCell.StartCellIndex to ASelectedCell.EndCellIndex do
        ASelectedCell.Row.Cells[I].Properties.ResetUse
