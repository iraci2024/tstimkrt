...
type
  ...
  public
    ...
    property HorisontalPositionType: TdxFloatingObjectHorizontalPositionType
      read FHorizontalPositionType write FHorizontalPositionType;
    property VericalPositionType: TdxFloatingObjectVerticalPositionType
  ...
  protected
    ...
    property ShapeAddresses: TObjectList<TdxFileShapeAddress>
      read FShapeAddresses;
  public
    constructor Create;
    destructor Destroy; override;
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer): TdxFileShapeAddressTable; static;
    ...
    property TranslationTable: TDictionary<Integer, TdxFileShapeAddress>
      read FTranslationTable;
  end;
...
class function TdxFileShapeAddress.FromStream(AReader: TBinaryReader)
  : TdxFileShapeAddress;
begin
...
begin
  ...
  FHorizontalPositionType :=
    ...
    CalcHorizontalPositionType97((AFlags and $06) shr 1);
  FVerticalPositionType := TdxDocFloatingObjectVerticalPositionTypeCalculator.
    CalcVerticalPositionType97((AFlags and $18) shr 3);
  FTextWrapType := TdxDocFloatingObjectTextWrapTypeCalculator.CalcTextWrapType
    ((AFlags and $01E0) shr 5);
  FTextWrapSide := TdxDocFloatingObjectTextWrapSideCalculator.CalcTextWrapSide
    ((AFlags and $1E00) shr 9);
  ABelowText := AFlags and $4000 <> 0;

  if TextWrapType = TdxDocFloatingObjectTextWrapTypeCalculator.WrapTypeBehindText
  then
  begin
...
begin
  ...
  AFlags := 0;
  AFlags := AFlags or
    Word(TdxDocFloatingObjectHorizontalPositionTypeCalculator.
    CalcHorizontalPositionTypeCode97(HorisontalPositionType) shl 1);
  AFlags := AFlags or
    Word(TdxDocFloatingObjectVerticalPositionTypeCalculator.
    CalcVerticalPositionTypeCode97(VericalPositionType) shl 3);
  AFlags := AFlags or
    Word(TdxDocFloatingObjectTextWrapTypeCalculator.CalcTextWrapTypeCode
    (TextWrapType) shl 5);
  AFlags := AFlags or
    Word(TdxDocFloatingObjectTextWrapSideCalculator.CalcTextWrapSideTypeCode
    (TextWrapSide) shl 9);

  if (TextWrapType = TdxDocFloatingObjectTextWrapTypeCalculator.
    WrapTypeBehindText) and IsBehindDoc then
...
class function TdxFileShapeAddressTable.FromStream(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer): TdxFileShapeAddressTable;
begin
...
procedure TdxFileShapeAddressTable.Read(AReader: TBinaryReader;
...
begin
  ...
  ACount := (ASize - TdxDocConstants.CharacterPositionSize)
    div (TdxDocConstants.CharacterPositionSize + TdxFileShapeAddress.Size);
...
procedure TdxFileShapeAddressTable.AddEntry(ACharacterPosition: Integer;
  AAddress: TdxFileShapeAddress);
begin
