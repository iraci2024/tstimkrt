...
type
  ...
  public
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer): TdxComplexFileInformation; static;
...
class function TdxComplexFileInformation.FromStream(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer): TdxComplexFileInformation;
begin
...
procedure TdxComplexFileInformation.Read(AReader: TBinaryReader;
...
function TdxComplexFileInformation.ExtractPieceTable
  (const AComplexFileInformation: TBytes): TBytes;
var
...
begin
  ...
  Result := nil;
  while not AIsPieceTableRetrieved and
    (ACurrentPosition < Length(AComplexFileInformation)) do
  begin
    if ACurrentEntryType = PieceTableByteCode then
    begin
      APieceTableSize :=
        PInteger(@AComplexFileInformation[ACurrentPosition + 1])^;
      SetLength(Result, APieceTableSize);
      Move(AComplexFileInformation[ACurrentPosition + ComplexTypeSize +
        SizeOfPieceTableSize], Result[0], APieceTableSize);
      AIsPieceTableRetrieved := True;
    end
    else
    begin
      AGrpplSize := PSmallInt(@AComplexFileInformation[ACurrentPosition + 1])^;
      ACurrentPosition := ACurrentPosition + ComplexTypeSize + SizeOfGrpprlSize
        + AGrpplSize;
