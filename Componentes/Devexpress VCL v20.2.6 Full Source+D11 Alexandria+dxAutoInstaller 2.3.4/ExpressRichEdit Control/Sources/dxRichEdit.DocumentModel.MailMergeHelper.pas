...
type
  ...
  protected
    ...
    procedure ExecuteMailMergeCore(ATargetModel: TdxDocumentModel;
      const AFieldDataService: IdxMailMergeDataService); virtual;
    procedure PrepareTargetModel(ATargetModel: TdxDocumentModel);
    function MailMergeRecord(ATargetModel: TdxDocumentModel;
      const AFieldDataService: IdxMailMergeDataService;
      const AFieldCalculatorService: IdxFieldCalculatorService;
      const AUriStreamService: IdxUriStreamService; AFirstRecord: Boolean)
      : Boolean; virtual;
    function MailMergeRecordCore(ATargetModel: TdxDocumentModel;
      AIntermediateResult: TdxDocumentModel;
      const AFieldDataService: IdxMailMergeDataService; AFirstRecord: Boolean)
      : Boolean; virtual;
    procedure AppendRecordResult(ATargetModel: TdxDocumentModel;
      AIntermediateResult: TdxDocumentModel;
      const AFieldDataService: IdxMailMergeDataService;
      AFirstRecord: Boolean); virtual;
    function PrepareRecordResult(ATargetModel: TdxDocumentModel;
      AIntermediateResult: TdxDocumentModel;
      const AFieldDataService: IdxMailMergeDataService): Boolean;
    procedure PrepareRecordData(ARecordModel: TdxDocumentModel); virtual;
    procedure PrepareRecordResultCore(AIntermediateResult: TdxDocumentModel;
      const AFieldDataService: IdxMailMergeDataService); virtual;
    procedure BeforeRecordInserted(ATargetModel: TdxDocumentModel;
      AFirstRecord: Boolean); virtual;
    procedure AfterRecordInserted(AIntermediateResult: TdxDocumentModel;
      ATargetModel: TdxDocumentModel); virtual;
    function NewSectionInserted: Boolean; virtual;
    procedure CopyFromIntermediateResult(AIntermediateResult: TdxPieceTable;
      ATarget: TdxPieceTable);
    function CalculateTargetPosition(ATarget: TdxPieceTable)
      : TdxDocumentModelPosition; virtual;
    procedure ApplyLastSectionSize(ATargetModel: TdxDocumentModel;
      ASourceModel: TdxDocumentModel);
    function RaiseMailMergeStarted(const AFieldDataService
      : IdxMailMergeDataService; ATargetModel: TdxDocumentModel): Boolean;
      virtual; abstract;
    function RaiseMailMergeRecordStarted(const AFieldDataService
      : IdxMailMergeDataService; ATargetModel: TdxDocumentModel;
      ARecordModel: TdxDocumentModel): Boolean; virtual;
    function RaiseMailMergeRecordFinished(const AFieldDataService
      : IdxMailMergeDataService; ATargetModel: TdxDocumentModel;
      ARecordModel: TdxDocumentModel): Boolean; virtual;
    procedure RaiseMailMergeFinished(const AFieldDataService
      : IdxMailMergeDataService; ATargetModel: TdxDocumentModel); virtual;
    procedure CopyTemplateStyles(ATargetModel: TdxDocumentModel;
      ASourceModel: TdxDocumentModel); virtual;
    procedure ReplaceServices(ADocumentModel: TdxDocumentModel;
      const AMailMergeDataService: IdxMailMergeDataService;
      const AFieldCalculatorService: IdxFieldCalculatorService;
      const AUriStreamService: IdxUriStreamService); virtual;
    procedure UpdateFields(ADocumentModel: TdxDocumentModel); virtual;
    procedure UpdateFieldsCore(ADocumentModel: TdxDocumentModel);
      overload; virtual;
    procedure UpdateFieldsCore(APieceTable: TdxPieceTable); overload; virtual;
    function AllowFixLastParagraph(APieceTable: TdxPieceTable)
      : Boolean; virtual;
    procedure UpdateSectionHeadersFooters(AIntermediate: TdxSection;
      ATarget: TdxSection);
    procedure UpdateSectionHeaderFooter(ASource: TdxSectionHeaderFooterBase;
      ATarget: TdxSection; AOwnerList: TdxSectionHeadersFootersBase;
      ATargetProvider: TdxFunc<TdxSection, TdxPieceTable>);

    property DocumentServerOwner: IdxInnerRichEditDocumentServerOwner
      read FDocumentServerOwner;
  ...
  public
    constructor Create(const ADocumentServerOwner
      : IdxInnerRichEditDocumentServerOwner; ASourceModel: TdxDocumentModel;
      const AOptions: IdxMailMergeOptions;
...
constructor TdxMailMergeHelperBase.Create(const ADocumentServerOwner
  : IdxInnerRichEditDocumentServerOwner; ASourceModel: TdxDocumentModel;
  const AOptions: IdxMailMergeOptions;
  const AProgressIndication: IdxProgressIndicationService = nil);
begin
...
procedure TdxMailMergeHelperBase.ExecuteMailMerge(ATargetModel
  : TdxDocumentModel);
var
...
procedure TdxMailMergeHelperBase.ExecuteMailMergeCore
...
begin
  ...
  while True do
  begin
    if not MailMergeRecord(ATargetModel, AFieldDataService,
      AFieldCalculatorService, AUriStreamService, AFirstRecord) then
      Break;
    AFirstRecord := False;
...
procedure TdxMailMergeHelperBase.PrepareTargetModel(ATargetModel
  : TdxDocumentModel);
begin
  ATargetModel.DocumentProperties.CopyFrom(SourceModel.DocumentProperties.Info);
  TdxDocumentModelCopyCommand.ReplaceDefaultProperties(ATargetModel,
    SourceModel);
...
function TdxMailMergeHelperBase.MailMergeRecord(ATargetModel: TdxDocumentModel;
  const AFieldDataService: IdxMailMergeDataService;
  const AFieldCalculatorService: IdxFieldCalculatorService;
  const AUriStreamService: IdxUriStreamService; AFirstRecord: Boolean): Boolean;
var
  AIntermediateResult: TdxDocumentModel;
...
begin
  ...
  try
    ...
    AEventRouter := TdxCalculateDocumentVariableEventRouter.Create
      (ATargetModel);
    AIntermediateResult.CalculateDocumentVariable.Add
    ...
    try
      ...
      AOptions := TdxDocumentModelCopyOptions.Create(AStartLogPosition,
        AEndLogPosition - AStartLogPosition + 1);
      try
        AOptions.CopyDocumentVariables := True;
        ACopyCommand := SourceModel.CreateDocumentModelCopyCommand
          (AMainPieceTable, AIntermediateResult, AOptions)
          as TdxDocumentModelCopyCommand;
        try
          ACopyCommand.FixLastParagraph :=
            AllowFixLastParagraph(AMainPieceTable);
      ...
      AIntermediateResult.DocumentSaveOptions.CurrentFileName :=
        SourceModel.DocumentSaveOptions.CurrentFileName;
      ReplaceServices(AIntermediateResult, AFieldDataService,
        AFieldCalculatorService, AUriStreamService);
    finally
      AIntermediateResult.EndSetContent
        (TdxDocumentModelChangeType.LoadNewDocument, False, nil);
      Result := MailMergeRecordCore(ATargetModel, AIntermediateResult,
        AFieldDataService, AFirstRecord);
      AIntermediateResult.CalculateDocumentVariable.Remove
        (AEventRouter.OnCalculateDocumentVariable);
    end;
  finally
...
function TdxMailMergeHelperBase.MailMergeRecordCore(ATargetModel
  : TdxDocumentModel; AIntermediateResult: TdxDocumentModel;
  const AFieldDataService: IdxMailMergeDataService;
  AFirstRecord: Boolean): Boolean;
var
  AResult: Boolean;
begin
  AResult := PrepareRecordResult(ATargetModel, AIntermediateResult,
    AFieldDataService);
  if AResult then
    AppendRecordResult(ATargetModel, AIntermediateResult, AFieldDataService,
      AFirstRecord);
  Result := AResult;
end;

procedure TdxMailMergeHelperBase.AppendRecordResult(ATargetModel
  : TdxDocumentModel; AIntermediateResult: TdxDocumentModel;
  const AFieldDataService: IdxMailMergeDataService; AFirstRecord: Boolean);
begin
  BeforeRecordInserted(ATargetModel, AFirstRecord);
  CopyFromIntermediateResult(AIntermediateResult.MainPieceTable,
    ATargetModel.MainPieceTable);
  AfterRecordInserted(AIntermediateResult, ATargetModel);
end;

function TdxMailMergeHelperBase.PrepareRecordResult(ATargetModel
  : TdxDocumentModel; AIntermediateResult: TdxDocumentModel;
  const AFieldDataService: IdxMailMergeDataService): Boolean;
begin
  PrepareRecordData(AIntermediateResult);
  if not RaiseMailMergeRecordStarted(AFieldDataService, ATargetModel,
    AIntermediateResult) then
    Exit(False);

  AIntermediateResult.MailMergeOptions.CustomSeparators.Assign
    (SourceModel.MailMergeOptions.CustomSeparators);

  PrepareRecordResultCore(AIntermediateResult, AFieldDataService);
  if not RaiseMailMergeRecordFinished(AFieldDataService, ATargetModel,
    AIntermediateResult) then
    Exit(False);
  Result := True;
end;

procedure TdxMailMergeHelperBase.PrepareRecordData(ARecordModel
  : TdxDocumentModel);
begin
end;

procedure TdxMailMergeHelperBase.PrepareRecordResultCore(AIntermediateResult
  : TdxDocumentModel; const AFieldDataService: IdxMailMergeDataService);
begin
  UpdateFields(AIntermediateResult);
end;

procedure TdxMailMergeHelperBase.BeforeRecordInserted
  (ATargetModel: TdxDocumentModel; AFirstRecord: Boolean);
begin
end;

procedure TdxMailMergeHelperBase.AfterRecordInserted(AIntermediateResult
  : TdxDocumentModel; ATargetModel: TdxDocumentModel);
begin
  if NewSectionInserted and not Options.HeaderFooterLinkToPrevious then
    UpdateSectionHeadersFooters(AIntermediateResult.Sections.Last,
      ATargetModel.Sections.Last);
end;
...
procedure TdxMailMergeHelperBase.CopyFromIntermediateResult(AIntermediateResult
  : TdxPieceTable; ATarget: TdxPieceTable);
var
  ACopyManager: TdxDocumentModelCopyManager;
  AOperation: TdxCopySectionOperation;
begin
  ACopyManager := TdxDocumentModelCopyManager.Create(AIntermediateResult,
    ATarget, TdxParagraphNumerationCopyOptions.CopyAlways);
  try
    ACopyManager.TargetPosition.CopyFrom(CalculateTargetPosition(ATarget));
    AOperation := AIntermediateResult.DocumentModel.CreateCopySectionOperation
      (ACopyManager) as TdxCopySectionOperation;
    try
      AOperation.FixLastParagraph := True;
      AOperation.Execute(AIntermediateResult.DocumentStartLogPosition,
        AIntermediateResult.DocumentEndLogPosition -
        AIntermediateResult.DocumentStartLogPosition + 1, False);
      ApplyLastSectionSize(ATarget.DocumentModel,
        AIntermediateResult.DocumentModel);
    finally
      AOperation.Free;
    end;
  finally
...
function TdxMailMergeHelperBase.CalculateTargetPosition(ATarget: TdxPieceTable)
  : TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.FromDocumentEnd(ATarget);
end;

procedure TdxMailMergeHelperBase.ApplyLastSectionSize
  (ATargetModel: TdxDocumentModel; ASourceModel: TdxDocumentModel);
begin
  ATargetModel.Sections.Last.CopyFromCore(ASourceModel.Sections.Last);
end;

function TdxMailMergeHelperBase.RaiseMailMergeRecordStarted
  (const AFieldDataService: IdxMailMergeDataService;
  ATargetModel: TdxDocumentModel; ARecordModel: TdxDocumentModel): Boolean;
var
  ARecordStartedArgs: TdxMailMergeRecordStartedEventArgs;
begin
  ARecordStartedArgs := TdxMailMergeRecordStartedEventArgs.Create
    (DocumentServerOwner, ATargetModel, ARecordModel);
  try
...
function TdxMailMergeHelperBase.RaiseMailMergeRecordFinished
  (const AFieldDataService: IdxMailMergeDataService;
  ATargetModel: TdxDocumentModel; ARecordModel: TdxDocumentModel): Boolean;
var
  ARecordFinishedArgs: TdxMailMergeRecordFinishedEventArgs;
begin
  ARecordFinishedArgs := TdxMailMergeRecordFinishedEventArgs.Create
    (DocumentServerOwner, ATargetModel, ARecordModel);
  try
...
procedure TdxMailMergeHelperBase.RaiseMailMergeFinished(const AFieldDataService
  : IdxMailMergeDataService; ATargetModel: TdxDocumentModel);
var
  AFinishedArgs: TdxMailMergeFinishedEventArgs;
begin
  AFinishedArgs := TdxMailMergeFinishedEventArgs.Create(DocumentServerOwner,
    ATargetModel);
  try
...
procedure TdxMailMergeHelperBase.CopyTemplateStyles(ATargetModel
  : TdxDocumentModel; ASourceModel: TdxDocumentModel);
var
...
procedure TdxMailMergeHelperBase.ReplaceServices(ADocumentModel
  : TdxDocumentModel; const AMailMergeDataService: IdxMailMergeDataService;
  const AFieldCalculatorService: IdxFieldCalculatorService;
...
procedure TdxMailMergeHelperBase.UpdateFieldsCore(ADocumentModel
  : TdxDocumentModel);
var
...
begin
  ...
  try
    ...
    for I := 0 to ACount - 1 do
      AUpdater.PrepareFieldUpdate(AFields[I],
...
function TdxMailMergeHelperBase.AllowFixLastParagraph
  (APieceTable: TdxPieceTable): Boolean;
begin
  Result := True;
end;

procedure TdxMailMergeHelperBase.UpdateSectionHeadersFooters
  (AIntermediate: TdxSection; ATarget: TdxSection);
begin
  UpdateSectionHeaderFooter(AIntermediate.InnerOddPageHeader, ATarget,
    ATarget.Headers, function(const ASection: TdxSection): TdxPieceTable
    begin
      Result := TdxPieceTable(ASection.InnerOddPageHeader.PieceTable);
    end);
  UpdateSectionHeaderFooter(AIntermediate.InnerEvenPageHeader, ATarget,
    ATarget.Headers, function(const ASection: TdxSection): TdxPieceTable
    begin
      Result := TdxPieceTable(ASection.InnerEvenPageHeader.PieceTable);
    end);

  UpdateSectionHeaderFooter(AIntermediate.InnerFirstPageHeader, ATarget,
    ATarget.Headers, function(const ASection: TdxSection): TdxPieceTable
    begin
      Result := TdxPieceTable(ASection.InnerFirstPageHeader.PieceTable);
    end);

  UpdateSectionHeaderFooter(AIntermediate.InnerOddPageFooter, ATarget,
    ATarget.Footers, function(const ASection: TdxSection): TdxPieceTable
    begin
      Result := TdxPieceTable(ASection.InnerOddPageFooter.PieceTable);
    end);

  UpdateSectionHeaderFooter(AIntermediate.InnerEvenPageFooter, ATarget,
    ATarget.Footers, function(const ASection: TdxSection): TdxPieceTable
    begin
      Result := TdxPieceTable(ASection.InnerEvenPageFooter.PieceTable);
    end);

  UpdateSectionHeaderFooter(AIntermediate.InnerFirstPageFooter, ATarget,
    ATarget.Footers, function(const ASection: TdxSection): TdxPieceTable
    begin
...
procedure TdxMailMergeHelperBase.UpdateSectionHeaderFooter
  (ASource: TdxSectionHeaderFooterBase; ATarget: TdxSection;
...
begin
  ...
  CopyFromIntermediateResult(TdxPieceTable(ASource.PieceTable),
    ATargetProvider(ATarget));
end;
...
procedure TdxMailMergeHelper.BeforeRecordInserted(ATargetModel
  : TdxDocumentModel; AFirstRecord: Boolean);
begin
  if (MergeMode = TdxMergeMode.NewSection) and not AFirstRecord then
  begin
    ATargetModel.Sections.Last.GeneralSettings.StartType :=
      TdxSectionStartType.NextPage;
    ATargetModel.InsertSection
      (ATargetModel.MainPieceTable.DocumentEndLogPosition, True);
  end;
end;

function TdxMailMergeHelper.AllowFixLastParagraph
  (APieceTable: TdxPieceTable): Boolean;
var
...
begin
  ...
  AIsTableBeforeParagraph := AParagraphs[ALastParagraph.Index - 1]
    .GetCell <> nil;
  Result := ALastParagraph.IsEmpty and AIsTableBeforeParagraph;
end;

function TdxMailMergeHelper.RaiseMailMergeStarted(const AFieldDataService
  : IdxMailMergeDataService; ATargetModel: TdxDocumentModel): Boolean;
begin
  FStartedArgs.Free;
  FStartedArgs := TdxMailMergeStartedEventArgs.Create(DocumentServerOwner,
    ATargetModel);
