...
type
  ...
  IdxBoxHitTestCalculator = interface(IdxBoxHitTestCustomCalculator)
    ...
    function CreatePageAreaHitTestManager(APageArea: TdxPageArea)
      : TdxBoxHitTestManager;
    function CreateColumnHitTestManager(AColumn: TdxColumn)
      : TdxBoxHitTestManager;
    function CreateNumberingListBoxHitTestManager(ABox: TdxNumberingListBox)
      : TdxBoxHitTestManager;
    function CreatePageBreakBoxHitTestManager(ABox: TdxPageBreakBox)
      : TdxBoxHitTestManager;
    function CreateColumnBreakBoxHitTestManager(ABox: TdxColumnBreakBox)
      : TdxBoxHitTestManager;
    function CreateSectionMarkBoxHitTestManager(ABox: TdxSectionMarkBox)
      : TdxBoxHitTestManager;
    function CreateTabSpaceBoxHitTestManager(ABox: TdxTabSpaceBox)
      : TdxBoxHitTestManager;
  ...
  TdxCommentLocationType = (CommentContent, CommentMoreButton, None);
  ...
  public
    constructor Create(ADocumentLayout: TdxDocumentLayout;
      APieceTable: TdxPieceTable); reintroduce;
    ...
    property CommentViewInfo: TdxCommentViewInfo read FCommentViewInfo
      write FCommentViewInfo;
    property CommentLocation: TdxCommentLocationType read FCommentLocation
      write FCommentLocation;
  end;

  { TdxBoxHitTestCalculator }

  TdxBoxHitTestCalculator = class(TdxBoxHitTestCustomCalculator,
    IdxBoxHitTestCalculator)
  private
    FHitTestRequest: TdxRichEditHitTestRequest;
    FHitTestResult: TdxRichEditHitTestResult;
  protected
    procedure CalcHitTestCore(AManager: TdxBoxHitTestManager;
      AForceStrictHitTest: Boolean);
    ...
    procedure ProcessPageAreaCollection(ACollection
      : TdxPageAreaCollection); virtual;
    procedure ProcessColumnCollection(ACollection
      : TdxColumnCollection); virtual;
...
{$REGION 'IdxBoxHitTestCustomCalculator'}
    function CreateCharacterBoxHitTestManager(ABox: TdxCharacterBox)
      : TdxBoxHitTestCustomManager; override;
    ...
    procedure ProcessCharacterBoxCollection(ACollection
      : TdxCharacterBoxCollection); override;
{$ENDREGION}
    procedure CalcHitTest(ABox: TdxBox; AForceStrictHitTest: Boolean = False);

    procedure FastHitTestCharacter(ACollection: TdxCharacterBoxCollection;
      AStrictHitTest: Boolean);
    function FastHitTestAssumingArrangedHorizontally
      (ACollection: TdxCustomBoxCollection; AStrictHitTest: Boolean): Integer;
    function FastHitTestAssumingArrangedVertically
      (ACollection: TdxCustomBoxCollection; AStrictHitTest: Boolean): Integer;

    function FastHitTestIndexCore(ACollection: TdxBoxList;
      const APredicate: TdxBoxComparableFunc; AStrictHitTest: Boolean)
      : Integer; overload;
    function FastHitTestIndexCore<T: class>(ACollection: TdxList<T>;
      ...
      overload;
    function FastHitTestIndexCore<T: class>(ACollection: TdxList<T>;
      AStartIndex, ACount: Integer; const APredicate: TdxComparableFunc<T>;
      AStrictHitTest: Boolean): Integer; overload;
  ...
  public type
    TPageAreaKind = (Autodetect = 0, Main, Header, Footer, TextBox);
  strict private
...
type
  ...
  protected
    function CalcHitTestAmongNestedBoxesCore(ATable: TdxTableViewInfo;
      AStrictTableMatch: Boolean): Boolean; overload;
    function CalcHitTestAmongNestedBoxesCore(ATable: TdxTableViewInfo;
      AStrictTableMatch, AStrictTableRowMatch, AStrictTableCellMatch: Boolean)
      : Boolean; overload;
...
constructor TdxRichEditHitTestResult.Create(ADocumentLayout: TdxDocumentLayout;
...
procedure TdxRichEditHitTestResult.SetFloatingObjectBox
...
constructor TdxBoxHitTestCalculator.Create(const ARequest
...
procedure TdxBoxHitTestCalculator.CalcHitTest(ABox: TdxBox;
  AForceStrictHitTest: Boolean = False);
var
...
procedure TdxBoxHitTestCalculator.CalcHitTestCore
  (AManager: TdxBoxHitTestManager; AForceStrictHitTest: Boolean);
var
  AStrictHitTest, AStrictMatch: Boolean;
begin
  AStrictHitTest :=
    ((Ord(HitTestRequest.Accuracy) and Ord(AManager.Box.HitTestAccuracy)) <> 0)
    or AForceStrictHitTest;
  AStrictMatch := AManager.Box.Bounds.Contains(HitTestRequest.LogicalPoint);
  if not AStrictHitTest and
    (HitTestRequest.DetailsLevel >= AManager.Box.DetailsLevel) then
  begin
...
function TdxBoxHitTestCalculator.FastHitTestIndexCore(ACollection: TdxBoxList;
  const APredicate: TdxBoxComparableFunc; AStrictHitTest: Boolean): Integer;
begin
  Result := FastHitTestIndexCore<TdxBoxBase>(ACollection, APredicate,
    AStrictHitTest);
end;

function TdxBoxHitTestCalculator.FastHitTestIndexCore<T>
  (ACollection: TdxList<T>; AStartIndex, ACount: Integer;
  const APredicate: TdxComparableFunc<T>; AStrictHitTest: Boolean): Integer;
begin
  if ACollection.Count <= 0 then
    Exit(-1);

  if TdxAlgorithms1<T>.BinarySearch(ACollection, APredicate, AStartIndex,
    ACount - 1, Result) then
    Exit;
...
function TdxBoxHitTestCalculator.FastHitTestIndexCore<T>
  (ACollection: TdxList<T>; const APredicate: TdxComparableFunc<T>;
  AStrictHitTest: Boolean): Integer;
begin
  Result := FastHitTestIndexCore<T>(ACollection, 0, ACollection.Count,
    APredicate, AStrictHitTest);
end;

function TdxBoxHitTestCalculator.FastHitTestRowIndex
...
function TdxBoxHitTestCalculator.FastHitTestCore(ACollection
  : TdxCustomBoxCollection; const APredicate: TdxBoxComparableFunc;
  AStrictHitTest: Boolean): Integer;
begin
  ...
  begin
    ...
    ACollection.RegisterSuccessfulItemHitTest(Self,
      TdxBox(ACollection[Result]));
  end
  else
...
procedure TdxBoxHitTestCalculator.RegisterCharacterHitTest
  (ABox: TdxCharacterBox);
begin
...
procedure TdxBoxHitTestCalculator.FastHitTestCharacter
  (ACollection: TdxCharacterBoxCollection; AStrictHitTest: Boolean);
var
...
begin
  ACharacterIndex := FastHitTestAssumingArrangedHorizontally(ACollection,
    AStrictHitTest);
  if not AStrictHitTest and (HitTestResult.Character <> nil) and
    (ACharacterIndex >= 0) then
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          HitTestResult.Accuracy := HitTestResult.Accuracy and
            not ExactCharacter;
        end;
      end;
    end;
...
function TdxBoxHitTestCalculator.CreatePageHitTestManager(APage: TdxPage)
  : TdxBoxHitTestManager;
begin
  Result := TdxPageHitTestManager.Create(Self, APage);
end;

function TdxBoxHitTestCalculator.CreatePageAreaHitTestManager
  (APageArea: TdxPageArea): TdxBoxHitTestManager;
begin
  Result := TdxPageAreaHitTestManager.Create(Self, APageArea);
end;

function TdxBoxHitTestCalculator.CreateColumnHitTestManager(AColumn: TdxColumn)
  : TdxBoxHitTestManager;
begin
  Result := TdxColumnHitTestManager.Create(Self, AColumn);
end;

function TdxBoxHitTestCalculator.CreateRowHitTestManager(ARow: TdxSimpleRow)
  : TdxBoxHitTestCustomManager;
begin
  Result := TdxRowHitTestManager.Create(Self, ARow);
end;

function TdxBoxHitTestCalculator.CreateTextBoxHitTestManager(ABox: TdxTextBox)
  : TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateCharacterBoxHitTestManager
  (ABox: TdxCharacterBox): TdxBoxHitTestCustomManager;
begin
  Result := TdxCharacterBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateHyphenBoxHitTestManager
  (ABox: TdxHyphenBox): TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateInlinePictureBoxHitTestManager
  (ABox: TdxInlinePictureBox): TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateSeparatorMarkBoxHitTestManager
  (ABox: TdxSeparatorBox): TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateSpaceBoxHitTestManager(ABox: TdxBox)
  : TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateTabSpaceBoxHitTestManager
  (ABox: TdxTabSpaceBox): TdxBoxHitTestManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateNumberingListBoxHitTestManager
  (ABox: TdxNumberingListBox): TdxBoxHitTestManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateLineBreakBoxHitTestManager
  (ABox: TdxLineBreakBox): TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreatePageBreakBoxHitTestManager
  (ABox: TdxPageBreakBox): TdxBoxHitTestManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateColumnBreakBoxHitTestManager
  (ABox: TdxColumnBreakBox): TdxBoxHitTestManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateParagraphMarkBoxHitTestManager
  (ABox: TdxParagraphMarkBox): TdxBoxHitTestCustomManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

function TdxBoxHitTestCalculator.CreateSectionMarkBoxHitTestManager
  (ABox: TdxSectionMarkBox): TdxBoxHitTestManager;
begin
  Result := TdxBoxHitTestManager.Create(Self, ABox);
end;

procedure TdxBoxHitTestCalculator.ProcessPageCollection
  (ACollection: TdxPageCollection);
begin
  // do nothing
end;

procedure TdxBoxHitTestCalculator.ProcessPageAreaCollection
...
procedure TdxBoxHitTestCalculator.ProcessColumnCollection
...
procedure TdxBoxHitTestCalculator.ProcessRowCollection
  (ACollection: TdxRowCollection);
var
  AStrictHitTest: Boolean;
begin
  AStrictHitTest := (HitTestRequest.Accuracy and ExactRow) <> 0;
...
procedure TdxBoxHitTestCalculator.ProcessBoxCollection
  (ACollection: TdxBoxCollection);
var
  AStrictHitTest: Boolean;
begin
...
procedure TdxBoxHitTestCalculator.ProcessCharacterBoxCollection
  (ACollection: TdxCharacterBoxCollection);
var
  AStrictHitTest: Boolean;
begin
...
var
  AStrictHitTest, AStrictMatch: Boolean;
begin
  AStrictHitTest := ((HitTestRequest.Accuracy and Box.HitTestAccuracy) <> 0) or
    AForceStrictHitTest;
  AStrictMatch := Box.Bounds.Contains(HitTestRequest.LogicalPoint);
  if not AStrictHitTest and (HitTestRequest.DetailsLevel >= Box.DetailsLevel)
  then
  begin
...
procedure TdxPageHitTestManager.RegisterSuccessfulHitTest
...
procedure TdxPageAreaHitTestManager.RegisterSuccessfulHitTest
  (AStrictMatch: Boolean);
begin
...
begin
  ...
  begin
    if not HitTestRequest.IgnoreInvalidAreas and not ShouldIgnoreFailedHitTest
    then
    begin
      case AreaKind of
        TPageAreaKind.Main:
          if ((HitTestResult.Page.Header <> nil) and
            (HitTestRequest.LogicalPoint.Y < PageArea.Bounds.Top)) or
            ((HitTestResult.Page.Footer <> nil) and
            (HitTestRequest.LogicalPoint.Y > PageArea.Bounds.Bottom)) then
          begin
            RegisterFailedHitTest;
            Exit;
          end;
...
begin
  ...
  if AFloatingObjectBox = nil then
    Exit(False);
  Result := AFloatingObjectBox.GetFloatingObjectRun.PieceTable = HitTestRequest.
    PieceTable;
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if ARow.Bounds.Contains(HitTestRequest.LogicalPoint) or not ARow.IsTableCellRow
        then
...
function TdxColumnHitTestManager.CalcHitTestAmongNestedBoxesCore
  (ATable: TdxTableViewInfo; AStrictTableMatch: Boolean): Boolean;
var
...
begin
  ...
  Result := CalcHitTestAmongNestedBoxesCore(ATable, AStrictTableMatch,
    AStrictTableRowMatch, AStrictTableCellMatch);
end;

function TdxColumnHitTestManager.CalcHitTestAmongNestedBoxesCore
  (ATable: TdxTableViewInfo; AStrictTableMatch, AStrictTableRowMatch,
  AStrictTableCellMatch: Boolean): Boolean;
var
...
begin
  ...
  ATableRowAnchorComparable := TdxTableRowAnchorComparable.Create
    (ALogicalPoint.Y, ATable.Table.Rows.Last);
  ATableRowIndex := Calculator.FastHitTestIndexCore<TdxTableRowViewInfoBase>
    (ATable.Rows, 0, ATable.RowCount, ATableRowAnchorComparable.CompareTo,
    AStrictTableRowMatch);
  ...
  ATableCellAnchorComparable := TdxTableCellAnchorComparable.Create
    (ALogicalPoint.X);
  ATableCellIndex := Calculator.FastHitTestIndexCore<TdxTableCellViewInfo>
    (ATableRow.Cells, ATableCellAnchorComparable.CompareTo,
    AStrictTableCellMatch);
  ...
  begin
    ...
    begin
      ...
      if CalcHitTestAmongNestedBoxesCore(ACurrentTable, AStrictTableMatch, True,
        True) then
        Exit(True);
    end;
  end;
...
begin
  ...
  begin
    ...
    if (ACurrentTable.Anchors.First.VerticalPosition <= ALogicalY) and
      (AActualTableBottom >= ALogicalY) then
      Exit(ACurrentTable);
  end;
  Result := nil;
end;
...
function TdxColumnHitTestManager.GetTable(ATableCellRow: TdxTableCellRow)
  : TdxTableViewInfo;
begin
...
procedure TdxCharacterBoxHitTestManager.RegisterSuccessfulHitTest
  (AStrictMatch: Boolean);
begin
...
procedure TdxEmptyHitTestManager.RegisterSuccessfulHitTest
  (AStrictMatch: Boolean);
begin
  // do nothing
end;
