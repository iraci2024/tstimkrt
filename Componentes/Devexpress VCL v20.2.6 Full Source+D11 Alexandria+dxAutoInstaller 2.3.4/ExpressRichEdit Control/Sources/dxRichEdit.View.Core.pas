...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Windows, Types, SysUtils, Generics.Defaults, Generics.Collections, Controls,
  Graphics, SyncObjs, Classes, ActiveX,
  cxGeometry, cxGraphics, dxSkinsCore, dxSkinInfo, dxCoreClasses,
  cxLookAndFeels, cxControls,
...
type
  ...
  TdxRefreshAction = (AllDocument, Zoom, Transforms, Selection);

  TdxScrollEventType = (EndScroll, First, LargeDecrement, LargeIncrement, Last,
    SmallDecrement, SmallIncrement, ThumbPosition, ThumbTrack);

  TdxScrollOrientation = (HorizontalScroll, VerticalScroll);
  ...
  public
    constructor Create(AOwner: TPersistent; const ADefaultValue: TRect);
      reintroduce; overload;
    constructor Create(AOwner: TPersistent; ADefaultValue: Integer = 0);
      reintroduce; overload;
  ...
  published
    property All: Integer index 0 read GetProperty write SetProperty
      stored IsPropertyStored;
    property Left: Integer index 1 read GetProperty write SetProperty
      stored IsPropertyStored;
    property Top: Integer index 2 read GetProperty write SetProperty
      stored IsPropertyStored;
    property Right: Integer index 3 read GetProperty write SetProperty
      stored IsPropertyStored;
    property Bottom: Integer index 4 read GetProperty write SetProperty
      stored IsPropertyStored;
  end;
  ...
  public
    constructor Create(AType: TdxScrollEventType; ANewValue: Integer;
      AScrollOrientation: TdxScrollOrientation = TdxScrollOrientation.
      HorizontalScroll); overload;
  ...
  IdxPlatformSpecificScrollBarAdapter = interface
    ...
    procedure OnScroll(AAdapter: TdxScrollBarAdapter; ASender: TObject;
    ...
    function SetRawScrollBarValue(AAdapter: TdxScrollBarAdapter;
      Value: Integer): Boolean;
    function GetPageUpRawScrollBarValue(AAdapter: TdxScrollBarAdapter): Integer;
    function GetPageDownRawScrollBarValue
      (AAdapter: TdxScrollBarAdapter): Integer;
    function CreateLastScrollEventArgs(AAdapter: TdxScrollBarAdapter)
      : TdxScrollEventArgs;
  end;
  ...
  IdxDecoratorPainter = interface
    ...
    procedure DrawDecorators(APainter: TdxPainter;
  ...
  public
    ...
    function CreateDecorator(APainter: TdxPainter): IdxTableViewInfoDecorator;
      virtual; abstract;
    procedure Update(const APhysicalPoint: TPoint;
      AHitTestResult: TdxRichEditHitTestResult); virtual; abstract;
  ...
  IdxCommandExecutionListenerService = interface
    ...
    procedure BeginCommandExecution(ACommand: TdxCommand;
      const AState: IdxCommandUIState);
    procedure EndCommandExecution(ACommand: TdxCommand;
      const AState: IdxCommandUIState);
  end;
  ...
  public
    ...
    property CommandSourceType: TdxCommandSourceType read GetCommandSourceType
      write SetCommandSourceType;
  ...
  protected
    function GetCommandExecutionListener
      : IdxCommandExecutionListenerService; virtual;
    procedure NotifyBeginCommandExecution(const AState
      : IdxCommandUIState); virtual;
    procedure NotifyEndCommandExecution(const AState
      : IdxCommandUIState); virtual;
  ...
  protected
    ...
    procedure ApplyCommandsRestriction(const AState: IdxCommandUIState;
      AOption: TdxDocumentCapability; AdditionEnabledCondition: Boolean = True);
    procedure ApplyCommandRestrictionOnEditableControl
      (const AState: IdxCommandUIState; AOption: TdxDocumentCapability);
      overload; virtual;
    procedure ApplyCommandRestrictionOnEditableControl
      (const AState: IdxCommandUIState; AOption: TdxDocumentCapability;
      AdditionEnabledCondition: Boolean); overload; virtual;
    procedure ApplyCommandRestrictionOnReadOnlyControl
      (const AState: IdxCommandUIState); virtual;
    procedure ApplyDocumentProtectionToSelectedCharacters
      (const AState: IdxCommandUIState); virtual;
    procedure ApplyDocumentProtectionToSelectedParagraphs
      (const AState: IdxCommandUIState);
    procedure ApplyDocumentProtectionToSelectedSections
      (const AState: IdxCommandUIState); virtual;
    procedure ApplyDocumentProtectionToTable(const AState: IdxCommandUIState;
      ATable: TdxTable);
    ...
    function ExtendToParagraphBoundary(ASelectionItem: TdxSelectionItem)
      : TdxSelectionItem;
  ...
  private
    ...
    function CalculateCharacterMouseCursor
      (AResult: TdxRichEditHitTestResult): TCursor;
    function IsEditable: Boolean;
  public
    constructor Create(AView: TdxRichEditView);
    function Calculate(AHitTestResult: TdxRichEditHitTestResult;
  ...
  IdxInnerControl = interface(IdxBoxMeasurerProvider)
    ...
    function CreateCommand(ACommandID: TdxRichEditCommandId)
      : TdxRichEditCommand;
    ...
    function OnHyperlinkClick(AField: TdxField;
      AAllowForModifiers: Boolean): Boolean;
    ...
    property DocumentModel: TdxDocumentModel read GetDocumentModel;
    property DocumentModelTemplate: TdxDocumentModel
      read GetDocumentModelTemplate;
    property Formatter: TdxBackgroundFormatter read GetFormatter;
    property IsEditable: Boolean read GetIsEditable;
    property Modified: Boolean read GetModified write SetModified;
    property Options: TdxRichEditControlOptionsBase read GetOptions;
    property PredefinedFontSizeCollection: TdxPredefinedFontSizeCollection
      read GetPredefinedFontSizeCollection;

    property UIUnit: TdxMeasurementUnit read GetUIUnit;

    property HorizontalScrollBar: IdxOfficeScrollbar
      read GetHorizontalScrollBar;
    ...
    property OnSearchComplete: TdxSearchCompleteEvent read GetOnSearchComplete
      write SetOnSearchComplete;
    // for internal use
    property MouseController: TdxRichEditCustomMouseController
      read GetMouseController;
  end;

  TdxShowFontFormCallback = reference to procedure
    (AProperties: TdxMergedCharacterProperties; AData: TObject);
  TdxShowParagraphFormCallback = reference to procedure
    (AProperties: TdxMergedParagraphProperties; AData: TObject);
  TdxShowEditStyleFormCallback = reference to procedure(ASourceStyle,
    ATargetStyle: TdxStyleBase);
  TdxShowTableStyleFormCallback = reference to procedure(ASourceStyle,
    ATargetStyle: TdxTableStyle);
  TdxShowLineNumberingFormCallback = reference to procedure
    (AProperties: TdxLineNumberingInfo; AData: TObject);
  TdxShowPageSetupFormCallback = reference to procedure
    (AProperties: TdxPageSetupInfo; AData: TObject);
  TdxShowColumnsSetupFormCallback = reference to procedure(const AProperties
    : TdxColumnsInfoUI; AData: TObject);
  TdxShowTabsFormCallback = reference to procedure
    (ATabInfo: TdxTabFormattingInfo; ADefaultTabWidth: Integer; AData: TObject);
  TdxShowNumberingListFormCallback = reference to procedure
    (AParagraphs: TdxParagraphList; AData: TObject);
  TdxShowSymbolFormCallback = reference to procedure(const ASymbolProperties
    : TdxSymbolProperties; AData: TObject);
  TdxShowHyperlinkFormCallback = reference to procedure(AHyperlinkInfo
    : TdxHyperlinkInfo; ASource: TdxTextToDisplaySource; ARunInfo: TdxRunInfo;
    const AText: string);
  TdxShowInsertTableFormCallback = reference to procedure(const AParameters
    : TdxCreateTableParameters; AData: TObject);
  TdxShowInsertDeleteTableCellsFormCallback = reference to procedure
    (const AParameters: TdxTableCellsParameters; AData: TObject);
  TdxShowSplitTableCellsFormCallback = reference to procedure(const AParameters
    : TdxSplitTableCellsParameters; AData: TObject);
  TdxPasswordFormCallback = reference to procedure(const APasswordInfo: string);
  TdxShowFloatingInlineObjectLayoutOptionsFormCallback = reference to procedure
    (const AFloatingObjectParameters: TdxFloatingInlineObjectParameters;
    AData: TObject);
  TdxShowMergeDatabaseRecordsFormCallback = reference to procedure
  ...
  IdxRichEditControl = interface(IdxBatchUpdatable)
    ...
    function GetPixelPhysicalBounds(APageViewInfo: TdxPageViewInfo;
      ALogicalBounds: TRect): TRect;
    ...
    function CreateRichEditViewVerticalScrollController(ARichEditView
      : TdxRichEditView): TdxRichEditViewVerticalScrollController;
    function CreateRichEditViewHorizontalScrollController
      (ARichEditView: TdxRichEditView)
      : TdxRichEditViewHorizontalScrollController;
    function CreatePlatformSpecificScrollBarAdapter
      : IdxPlatformSpecificScrollBarAdapter;
    ...
    function DoDragDrop(const AData: IDataObject;
    ...
    procedure ShowNumberingListForm(AParagraphs: TdxParagraphList;
      const ACallback: TdxShowNumberingListFormCallback;
      ACallbackData: TObject);
    procedure ShowEditStyleForm(AParagraphSourceStyle: TdxParagraphStyle;
      AIndex: TdxParagraphIndex;
      const ACallback: TdxShowEditStyleFormCallback); overload;
    procedure ShowEditStyleForm(ACharacterSourceStyle: TdxCharacterStyle;
      AIndex: TdxParagraphIndex;
      const ACallback: TdxShowEditStyleFormCallback); overload;
    procedure ShowTableStyleForm(AStyle: TdxTableStyle);
    procedure ShowParagraphForm(AParagraphProperties
      : TdxMergedParagraphProperties;
      const ACallback: TdxShowParagraphFormCallback; ACallbackData: TObject);
    procedure ShowFontForm(ACharacterProperties: TdxMergedCharacterProperties;
      const ACallback: TdxShowFontFormCallback; ACallbackData: TObject);
    procedure ShowTabsForm(ATabInfo: TdxTabFormattingInfo;
      ADefaultTabWidth: Integer; const ACallback: TdxShowTabsFormCallback;
      ACallbackData: TObject);
    procedure ShowHyperlinkForm(AHyperlinkInfo: TdxHyperlinkInfo;
      ARunInfo: TdxRunInfo; const ATitle: string;
      const ACallback: TdxShowHyperlinkFormCallback);
    procedure ShowSymbolForm(const ASymbolProperties: TdxSymbolProperties;
      const ACallback: TdxShowSymbolFormCallback; ACallbackData: TObject);
    procedure ShowInsertMergeFieldForm;
    procedure ShowInsertTableForm(const AParameters: TdxCreateTableParameters;
      const ACallback: TdxShowInsertTableFormCallback; ACallbackData: TObject);
    procedure ShowInsertTableCellsForm(const AParameters
      : TdxTableCellsParameters;
      const ACallback: TdxShowInsertDeleteTableCellsFormCallback;
      ACallbackData: TObject);
    procedure ShowDeleteTableCellsForm(const AParameters
      : TdxTableCellsParameters;
      const ACallback: TdxShowInsertDeleteTableCellsFormCallback;
      ACallbackData: TObject);
    procedure ShowSplitTableCellsForm(const AParameters
      : TdxSplitTableCellsParameters;
      const ACallback: TdxShowSplitTableCellsFormCallback;
      ACallbackData: TObject);
    procedure ShowRangeEditingPermissionsForm;
    procedure ShowDocumentEncryptQueryNewPasswordForm(const APassword: string;
      const ACallback: TdxPasswordFormCallback);
    procedure ShowDocumentProtectionQueryNewPasswordForm(const APasswordInfo
      : string; const ACallback: TdxPasswordFormCallback);
    procedure ShowDocumentProtectionQueryPasswordForm(const APasswordInfo
      : string; const ACallback: TdxPasswordFormCallback);
    procedure ShowLineNumberingForm(AProperties: TdxLineNumberingInfo;
      const ACallback: TdxShowLineNumberingFormCallback;
      ACallbackData: TObject);
    procedure ShowPageSetupForm(AProperties: TdxPageSetupInfo;
      const ACallback: TdxShowPageSetupFormCallback; ACallbackData: TObject;
      AInitialTabPage: TdxPageSetupFormInitialTabPage);
    procedure ShowColumnsSetupForm(const AProperties: TdxColumnsInfoUI;
      const ACallback: TdxShowColumnsSetupFormCallback; ACallbackData: TObject);
    procedure ShowTablePropertiesForm(ASelectedCells
      : TdxSelectedCellsCollection);
    procedure ShowFloatingInlineObjectLayoutOptionsForm
      (const AFloatingObjectParameters: TdxFloatingInlineObjectParameters;
    ...
    procedure ShowMergeDatabaseRecordsForm(const AMergeRecordsParameters
    ...
    property BackgroundPainter: TdxRichEditViewBackgroundPainter
      read GetBackgroundPainter;
    property DocumentModel: TdxDocumentModel read GetDocumentModel;
    property MeasurementAndDrawingStrategy: TdxMeasurementAndDrawingStrategy
      read GetMeasurementAndDrawingStrategy;
    ...
    property LayoutUnit: TdxDocumentLayoutUnit read GetLayoutUnit
      write SetLayoutUnit;
  ...
  public type
    ...
    public const
      ShowHiddenText = TdxSimpleFormattingMarkVisibilityOptions.TAction.
        ShowHiddenText;
    end;
  strict private
  ...
  private
    ...
    procedure SetFormattingMarkVisibility(const Value
  ...
  protected
    ...
    property ShowHiddenText: Boolean read GetShowHiddenText
      write SetShowHiddenText default False;
    property SpellChecker: TdxSpellCheckerOptions read GetSpellChecker
      write SetSpellChecker;
    property RichEditDocumentFormats: TdxRichEditDocumentFormats
      read FRichEditDocumentFormats write FRichEditDocumentFormats;
  public
    constructor Create(const ADocumentServer: IdxRichEditDocumentServer);
      reintroduce; virtual;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;

    property Authentication: TdxAuthenticationOptions read GetAuthentication
      write SetAuthentication;
    property AutoCorrect: TdxAutoCorrectOptions read GetAutoCorrect
      write SetAutoCorrect;
    property Behavior: TdxRichEditBehaviorOptions read GetBehavior
      write SetBehavior;
    property Bookmarks: TdxBookmarkOptions read GetBookmarks write SetBookmarks;
    property CopyPaste: TdxCopyPasteOptions read GetCopyPaste
      write SetCopyPaste;
    ...
    property DocumentSaveOptions: TdxDocumentSaveOptions
      read GetDocumentSaveOptions write SetDocumentSaveOptions;
    property Export: TdxRichEditDocumentExportOptions read GetExport
      write SetExport;
    property Fields: TdxFieldOptions read GetFields write SetFields;
    property FormattingMarkVisibility: TdxFormattingMarkVisibilityOptions
      read GetFormattingMarkVisibility write SetFormattingMarkVisibility;
    property HorizontalRuler: TdxHorizontalRulerOptions read FHorizontalRuler
      write SetHorizontalRuler;
    property Hyperlinks: TdxHyperlinkOptions read FHyperlinks
      write SetHyperlinks;
    property Import: TdxRichEditDocumentImportOptions read GetImport
      write SetImport;
    property Layout: TdxRichEditLayoutOptions read GetLayout write SetLayout;
    property MailMerge: TdxRichEditMailMergeOptions read GetMailMerge
      write SetMailMerge;
    property Printing: TdxPrintingOptions read GetPrinting write SetPrinting;
    property RangePermissions: TdxRangePermissionOptions
      read GetRangePermissions write SetRangePermissions;
    property Search: TdxDocumentSearchOptions read GetSearch write SetSearch;
    property TableOptions: TdxTableOptions read GetTableOptions
      write SetTableOptions;
    property VerticalRuler: TdxVerticalRulerOptions read FVerticalRuler
  ...
  private
    ...
    function GetActualLogPosition(AEndPosition: TdxDocumentLogPosition)
  ...
  protected
    ...
    function UpdatePositionTrySetUsePreviousBoxBounds(ADetailsLevel
      : TdxDocumentLayoutDetailsLevel): Boolean; virtual;
    function UpdateCaretDocumentLayoutPosition(ADetailsLevel
      : TdxDocumentLayoutDetailsLevel): TdxDocumentLayoutPosition;
    procedure UpdateModelPosition(ALogPosition: TdxDocumentLogPosition);
    function GetCaretBoundsDocumentLayoutPosition(ALogPosition
      : TdxDocumentLogPosition): TdxDocumentLayoutPosition;
    function CalculateCurrentFormatingPosition(ALogPosition
      : TdxDocumentLogPosition): TdxDocumentModelPosition;
    function ShouldUpdateModelPosition(ALogPosition
      : TdxDocumentLogPosition): Boolean;

    property View: TdxRichEditView read FView;
    property VirtualLogPosition: TdxDocumentLogPosition
      read GetVirtualLogPosition;
    property UsePreviousBoxBounds: Boolean read GetUsePreviousBoxBounds;
    property CaretBoundsPosition: TdxDocumentLayoutPosition
      read FCaretBoundsPosition write SetCaretBoundsPosition;
  public
    constructor Create(AView: TdxRichEditView; APreferredPageIndex: Integer);
    destructor Destroy; override;

    function CreateCaretDocumentLayoutPosition
      : TdxDocumentLayoutPosition; virtual;
    function Update(ADetailsLevel: TdxDocumentLayoutDetailsLevel)
      : Boolean; virtual;
    ...
    function CreateExplicitCaretPosition(ALogPosition: TdxDocumentLogPosition)
      : TdxCaretPosition; virtual;
    function CreateInputPosition: TdxInputPosition; overload; virtual;
    function CreateInputPosition(ALogPosition: TdxDocumentLogPosition)
      : TdxInputPosition; overload; virtual;
    function GetRowBoundsRelativeToPage: TRect; virtual;

    property PageViewInfo: TdxPageViewInfo read FPageViewInfo
      write SetPageViewInfo;
    property X: Integer read FX write FX;
    property TableCellTopAnchorIndex: Integer read FTableCellTopAnchorIndex
      write FTableCellTopAnchorIndex;
  ...
  protected
    ...
    function GetUsePreviousBoxBounds: Boolean; override;
    function UpdatePositionTrySetUsePreviousBoxBounds(ADetailsLevel
      : TdxDocumentLayoutDetailsLevel): Boolean; override;
  public
    constructor Create(AView: TdxRichEditView;
  ...
  public
    function CreateCaretDocumentLayoutPosition
      : TdxDocumentLayoutPosition; override;
    function CreateExplicitCaretPosition(ALogPosition: TdxDocumentLogPosition)
      : TdxCaretPosition; override;
  end;
  ...
  TdxHeaderFooterCaretDocumentLayoutPosition = class
  ...
  TdxTextBoxCaretDocumentLayoutPosition = class
  ...
  public
    constructor Create(AView: TdxRichEditView;
      APreferredPageIndex: Integer); overload;
    constructor Create(AView: TdxRichEditView;
      ATextBoxPieceTable: TdxTextBoxContentType;
      APreferredPageIndex: Integer); overload;
    function IsValid(ADetailsLevel: TdxDocumentLayoutDetailsLevel)
      : Boolean; override;
    function Update(APages: TdxPageCollection;
      ADetailsLevel: TdxDocumentLayoutDetailsLevel): Boolean; override;
  end;
  ...
  public
    ...
    function ShouldDrawCaret(ADocumentModel: TdxDocumentModel)
      : Boolean; virtual;
    function GetCaretPosition(AView: TdxRichEditView)
      : TdxCaretPosition; virtual;
  ...
  private
    ...
    function FindCellViewInfo(AModelCell: TdxTableCell; AColumn: TdxColumn)
      : TdxTableCellViewInfo;
    function GetView: TdxRichEditView;
    function GetDocumentModel: TdxDocumentModel;
    function FindFloatingObject(AFloatingObjects: TdxFloatingObjectBoxList;
      AIndex: TdxRunIndex): TdxFloatingObjectBox; overload;
    function FindFloatingObject(APage: TdxPage; AIndex: TdxRunIndex)
      : TdxFloatingObjectBox; overload;
    function TryToCreateRectangularObjectSelectionLayoutForSinglePosition
      (AStart, AEnd: TdxDocumentLayoutPosition)
      : TdxRectangularObjectSelectionLayout;
    function TryToCreateRectangularObjectSelectionLayoutByBox
      (ALayoutPos: TdxDocumentLayoutPosition; ALogPos: TdxDocumentLogPosition)
      : TdxRectangularObjectSelectionLayout;
    ...
    function TryGetNextPositionCore(AParagraph: TdxParagraph;
      ACell: TdxTableCell; var APos: TdxDocumentLayoutPosition): Boolean;
    function TryCreateRectangularObjectSelectionLayout(AStart,
      AEnd: TdxDocumentLayoutPosition): TdxRectangularObjectSelectionLayout;
    function TryCreateRectangularObjectSelectionLayoutByAnchorRun
      (ARunInfo: TdxDocumentModelPosition; out APos: TdxDocumentLayoutPosition)
      : TdxRectangularObjectSelectionLayout;
    function TryToCreateRectangularObjectSelectionLayoutByField
      (AField: TdxField): TdxRectangularObjectSelectionLayout;
    function TryGetNextPositionBySelectionLayoutItem
    ...
    function TryGetNextPositionBySelectionLayoutItemCore
      (const AItem: IdxSelectionLayoutItem; AStop: TdxDocumentLayoutPosition;
      AVisibleEndLogPosition: TdxDocumentLogPosition;
      var APos: TdxDocumentLayoutPosition): Boolean;

    property SelectionLayout: TdxSelectionLayout read FSelectionLayout;
    property NestedItems: TObjectDictionary<Integer, TdxSelectionLayoutItemList>
      read FNestedItems;
  protected
    function GetBox(APos: TdxDocumentLayoutPosition): TdxPage;
    function CreateDocumentLayoutPosition(ALogPosition: TdxDocumentLogPosition)
      : TdxDocumentLayoutPosition; virtual;
    procedure AddNestedItem(APageIndex: Integer;
      const AItem: IdxSelectionLayoutItem);
    ...
    property DocumentModel: TdxDocumentModel read GetDocumentModel;
  public
    constructor Create(ASelectionLayout: TdxSelectionLayout;
      AStart, AEnd: TdxDocumentLayoutPosition; APieceTable: TdxPieceTable);
    destructor Destroy; override;
    procedure ForceUpdateForEmptySelection; virtual;
    procedure AddToPageSelection(APage: TdxPage;
      ATarget: TdxPageSelectionLayoutsCollection); virtual;
    procedure Draw(const ASelectionPainter: IdxSelectionPainter);
    function Update: Boolean; virtual;
    function HitTest(ALogPosition: TdxDocumentLogPosition;
  ...
  protected
    function CreateDocumentSelectionLayout(AStart,
      AEnd: TdxDocumentLayoutPosition): TdxDocumentSelectionLayout; virtual;

    property FirstDocumentSelectionLayout: TdxDocumentSelectionLayout
      read GetFirstDocumentSelectionLayout;
    property DocumentLayout: TdxDocumentLayout read GetDocumentLayout;
  public
    constructor Create(AView: TdxRichEditView; APreferredPageIndex: Integer);
    destructor Destroy; override;

    function CalculateHotZone(AResult: TdxRichEditHitTestResult;
      AView: TdxRichEditView): TdxHotZone; virtual;
    ...
    function CreateDocumentLayoutPosition(ALogPosition: TdxDocumentLogPosition)
      : TdxDocumentLayoutPosition; virtual;
    function HitTest(ALogPosition: TdxDocumentLogPosition;
      ALogicalPoint: TPoint): Boolean;
    function GetPageSelection(APage: TdxPage)
      : TdxPageSelectionLayoutsCollection; virtual;

    property PreferredPageIndex: Integer read GetPreferredPageIndex
      write SetPreferredPageIndex;
    property StartLayoutPosition: TdxDocumentLayoutPosition
      read GetStartLayoutPosition;
    property EndLayoutPosition: TdxDocumentLayoutPosition
      read GetEndLayoutPosition;
    property LastDocumentSelectionLayout: TdxDocumentSelectionLayout
      read GetLastDocumentSelectionLayout;
    property View: TdxRichEditView read FView;
  end;
  ...
  public const
    Infinite = -1;
  public type
    TCommand = (Shutdown, PerformSecondaryLayout, PerformPrimaryLayout, None);
  ...
  private
    ...
    function PredicateDocumentModelPosition(const ACurrentFormatterPosition
      : TdxDocumentModelPosition): Boolean;
    ...
    function IsTableFormattingComplete(const ANextPosition
      : TdxDocumentModelPosition): Boolean;
    procedure RefreshSecondaryLayout(AFirstPageIndex: Integer);
    procedure SetSpellCheckerController(const Value
      : TdxSpellCheckerControllerBase);
  protected
    function CreateParagraphFinalFormatter(ADocumentLayout: TdxDocumentLayout)
      : TdxParagraphFinalFormatter; virtual;
    ...
    function IsPrimaryLayoutComplete(const ACurrentFormatterPosition
      : TdxDocumentModelPosition): Boolean; virtual;
    function ShouldResetPrimaryLayout(const AFrom,
      ATo: TdxDocumentModelPosition): Boolean; virtual;
    function ShouldResetSecondaryLayout(const AFrom,
      ATo: TdxDocumentModelPosition): Boolean; virtual;
    procedure ResetSecondaryLayout; virtual;

    procedure ResetCalculators;
    procedure ResetPrimaryLayout; overload; virtual;
    procedure ResetPrimaryLayout(const AFrom, ATo: TdxDocumentModelPosition);
      overload; virtual;

    property ContinueLayout: TSimpleEvent read FContinueLayout;
    property SecondaryLayoutComplete: TSimpleEvent
      read FSecondaryLayoutComplete;
  ...
  public
    constructor Create(AController: TdxDocumentFormattingController;
      const ACommentPadding: TdxCommentPadding);
    destructor Destroy; override;
    procedure Start; virtual;

    procedure BeginDocumentRendering(APerformPrimaryLayoutUntil
      : TdxPredicate<TdxDocumentModelPosition>);
    procedure EndDocumentRendering;
    procedure BeginDocumentUpdate;
    procedure EndDocumentUpdate;
    procedure PerformPageSecondaryFormatting(APage: TdxPage); virtual;
    procedure WaitForPrimaryLayoutReachesLogPosition(ALogPosition
      : TdxDocumentLogPosition);
    procedure WaitForPrimaryLayoutReachesPreferredPage(APreferredPageIndex
      : Integer);
    procedure WaitForPagePrimaryLayoutComplete(APage: TdxPage);
    procedure WaitForPrimaryLayoutComplete;
    procedure WaitForSecondaryLayoutReachesPosition
      (const APos: TdxDocumentModelPosition);
    procedure NotifyDocumentChanged(const AFrom, ATo: TdxDocumentModelPosition;
      ADocumentLayoutResetType: TdxDocumentLayoutResetType);
    procedure UpdateSecondaryPositions(const AFrom,
      ATo: TdxDocumentModelPosition);
    ...
    procedure ResetSecondaryFormattingForPage(APage: TdxPage;
      APageIndex: Integer);

    procedure RaisePageFormattingComplete
    ...
    procedure OnPageFormattingComplete(ASender: TObject;
      E: TdxPageFormattingCompleteEventArgs);
    ...
    property DocumentLayout: TdxDocumentLayout read GetDocumentLayout;
    property SecondaryLayoutStart: TdxDocumentModelPosition
      read FSecondaryLayoutStart;
    property SecondaryLayoutEnd: TdxDocumentModelPosition
      read FSecondaryLayoutEnd;
    property SpellCheckerController: TdxSpellCheckerControllerBase
      read FSpellCheckerController write SetSpellCheckerController;
  ...
  TdxDrawDelegate = reference to procedure(AGraphics: TdxGraphics;
    const ACustomMarkExporter: IdxCustomMarkExporter);
  TdxDrawAtPageDelegate = reference to procedure(AGraphics: TdxGraphics);
  TdxDrawReversibleDelegate = reference to procedure(AHdc: THandle;
    const ABounds: TRect);
  ...
  TdxRichEditSelectionPainters = class
    (TdxRichEditPaintersBase<IdxSelectionLayoutItem,
  ...
  strict private
    ...
    function BeginDrawPageContentInPixels(APageViewInfo: TdxPageViewInfo)
      : TdxGPMatrix;
    function ClipPageContentForDrawingInPixels(APageViewInfo: TdxPageViewInfo;
      APainter: TdxPainter): TdxRectF;
  ...
  protected
    ...
    procedure BeginDrawPageContent(APage: TdxPageViewInfo;
      ATransform: TdxGPMatrix); virtual;
    procedure ClipPageContent(APage: TdxPageViewInfo;
      APainter: TdxPainter); virtual;
    procedure EndDrawPageContent(APage: TdxPageViewInfo;
      ATransform: TdxGPMatrix); virtual;
    procedure BeginDrawInPixels(AGraphics: TdxGraphics); virtual;
    procedure EndDrawInPixels; virtual;
    procedure DrawPageContent(APageViewInfo: TdxPageViewInfo;
      APainter: TdxPainter); virtual;
    procedure DrawPageComment(APageViewInfo: TdxPageViewInfo;
      APainter: TdxPainter); virtual;
    function CommentsVisible(ACount: Integer): Boolean;
    procedure DrawCommentsBackground(APageViewInfo: TdxPageViewInfo;
      AGraphics: TdxGraphics); virtual;
    ...
    function GetNewClientBounds(const AClipBounds: TRect;
      const AOldClipBounds: TdxRectF): TdxRectF; virtual;
    procedure DrawBoxesInPixels(const ACustomMarkExporter
      : IdxCustomMarkExporter); virtual;
    procedure IntersectClipBounds(APainter: TdxPainter;
      const AOldClipBounds: TdxRectF; X: Single; Y: Single; AWidth: Single;
      AHeight: Single);
    function CalculateRulerHeight: Integer; virtual;
    procedure DecorateTables(APainter: TdxPainter); virtual;
    procedure DecorateTablesCore(AController: TdxTableViewInfoController;
      APainter: TdxPainter);
    procedure DrawDecorators(APainter: TdxPainter); overload; virtual;
    procedure ApplyGraphicsTransform(AGraphics: TdxGraphics;
      ACustomMarkVisualInfoCollection
      : TdxCustomMarkVisualInfoCollection); virtual;
    procedure DrawCaretCore(AGraphics: TdxGraphics; ACaret: TdxCaret); virtual;
    procedure DrawReversibleHorizontalLine(AGraphics: TdxGraphics;
      const ABounds: TRect); overload; virtual;
    procedure DrawReversibleHorizontalLine(AHdc: THandle; const ABounds: TRect);
      overload; virtual;
    procedure DrawReversibleVerticalLine(AGraphics: TdxGraphics;
      const ABounds: TRect); overload; virtual;
    procedure DrawReversibleVerticalLine(AHdc: THandle; const ABounds: TRect);
      overload; virtual;
    procedure DrawReversibleFrame(AGraphics: TdxGraphics; const ABounds: TRect);
      overload; virtual;
    procedure DrawReversibleFrame(AHdc: THandle; const ABounds: TRect);
      overload; virtual;
    procedure DrawReversible(AHdc: THandle; const ABounds: TRect;
      const ADraw: TdxDrawReversibleDelegate); virtual;
    procedure DrawReversibleFrameCore(AHdc: THandle;
      const ABounds: TRect); virtual;
    procedure DrawReversibleHorizontalLineCore(AHdc: THandle;
      const ABounds: TRect); virtual;
    procedure DrawReversibleVerticalLineCore(AHdc: THandle;
      const ABounds: TRect); virtual;
    procedure DrawPageSelection(AGraphics: TdxGraphics;
      APage: TdxPageViewInfo); virtual;
    procedure DrawPageSelectionCore(const ASelection: IdxSelectionLayoutItem);
    procedure DrawHover(APainter: TdxPainter); virtual;
    procedure DrawHoverCore(const AHoverLayoutItem: IdxHoverLayoutItem;
      APainter: TdxPainter);
    procedure DrawEmptyPages(AGraphics: TdxGraphics); virtual;
    procedure ResetCache; virtual;
    procedure DrawEmptyPage(AGraphics: TdxGraphics; APage: TdxPageViewInfo);
      virtual; abstract;
    procedure DrawEmptyComment(AGraphics: TdxGraphics;
      AComment: TdxCommentViewInfo); virtual; abstract;
    procedure DrawEmptyExtensionComment(AGraphics: TdxGraphics;
      AComment: TdxCommentViewInfo); virtual; abstract;

    property DecoratorPainters: TList<IdxDecoratorPainter>
      read FDecoratorPainters;
    property SelectionPainters: TdxRichEditSelectionPainters
      read FSelectionPainters;
  ...
  public
    ...
    procedure Draw(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); virtual;
    function GetCommentBounds(ACommentViewInfo: TdxCommentViewInfo): TRect;
    procedure DrawAtPageCore(AGraphics: TdxGraphics; APage: TdxPageViewInfo;
      const ADraw: TdxDrawAtPageDelegate); virtual;
    procedure DrawCaretAtPage(AGraphics: TdxGraphics; ACaret: TdxCaret;
      APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleHorizontalLineAtPage(AGraphics: TdxGraphics;
      Y: Integer; APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleVerticalLineAtPage(AGraphics: TdxGraphics;
      X: Integer; APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleFrameAtPage(AGraphics: TdxGraphics;
      const ABounds: TRect; APage: TdxPageViewInfo); virtual;
  ...
  TdxRichEditViewColorBasedBackgroundPainter = class
    (TdxRichEditViewBackgroundPainter);
  ...
  private
    ...
    procedure DocumentModelActivePieceTableTextBoxHitTest
      (var ARequest: TdxRichEditHitTestRequest;
      var AResult: TdxRichEditHitTestResult; APageViewInfo: TdxPageViewInfo);
    function IsPaddingStored: Boolean;
    function IsPrimaryFormattingCompleteForVisibleHeight2
      (const ACurrentFormatterPosition: TdxDocumentModelPosition): Boolean;
    procedure PaddingChanged(Sender: TObject);
    function EnsurePositionNotBeforeSectionBreakAfterParagraphBreak
      (const AModelPosition: TdxDocumentModelPosition)
      : TdxDocumentModelPosition;
    function IsActivePieceTableFloatingObjectBox
      (const ABox: TdxFloatingObjectBox): Boolean;
    function IsActiveFloatingObjectTextBox(const ABox
      : TdxFloatingObjectBox): Boolean;
    function IsFloatingObjectHit(ABox: TdxFloatingObjectBox;
    ...
    class function SetNewPosition(const APos: TdxDocumentModelPosition;
  ...
  protected
    ...
    function GetMatchHorizontalTableIndentsToTextEdge: Boolean;
      virtual; abstract;
    ...
    function CreateDocumentLayout(ADocumentModel: TdxDocumentModel;
      const AMeasurerProvider: IdxBoxMeasurerProvider)
      : TdxDocumentLayout; virtual;
    function CreateDocumentFormattingController
      : TdxDocumentFormattingController; virtual; abstract;
    function CreatePageViewInfoGenerator: TdxPageViewInfoGenerator;
      virtual; abstract;
    ...
    function CalculateDocumentLayoutResetType
      (AChanges: TdxDocumentModelDeferredChanges)
      : TdxDocumentLayoutResetType; virtual;
    function CalculateResetFromPosition(APieceTable: TdxPieceTable;
      AChanges: TdxDocumentModelDeferredChanges;
      AResetType: TdxDocumentLayoutResetType)
      : TdxDocumentModelPosition; virtual;
    function EnsureTopLevelParagraph(const APos: TdxDocumentModelPosition)
      : TdxDocumentModelPosition; virtual;
    ...
    procedure OnResetSecondaryFormattingForPage(ASender: TObject;
      E: TdxResetSecondaryFormattingForPageArgs); virtual;
    procedure OnPageFormattingStarted(ASender: TObject;
      E: TdxPageFormattingCompleteEventArgs); virtual;
    procedure OnPageFormattingComplete(ASender: TObject;
      E: TdxPageFormattingCompleteEventArgs); virtual;
    procedure SetOptimalHorizontalScrollbarPosition; virtual;
    procedure SetOptimalHorizontalScrollbarPositionCore; virtual;
    function CalculateOptimalHorizontalScrollbarPosition(APageViewInfo
      : TdxPageViewInfo): Integer; virtual;
    function ChoosePageViewInfoForOptimalHorizontalScrollbarPosition
      : TdxPageViewInfo; virtual;
    ...
    procedure EnsureFormattingCompleteForLogPosition(ALogPosition
      : TdxDocumentLogPosition); virtual;
    procedure EnsurePageSecondaryFormattingComplete(APage: TdxPage); virtual;
    function CreateTransformMatrix(const AClientBounds: TRect)
      : TdxTransformMatrix; virtual;
    function GetPageViewInfoRowFromPoint(const APoint: TPoint;
      AStrictSearch: Boolean): TdxPageViewInfoRow; virtual;
    procedure HitTest(APage: TdxPage; const ARequest: TdxRichEditHitTestRequest;
      AResult: TdxRichEditHitTestResult); virtual;
    function PerformStrictPageViewInfoHitTest(APageViewInfo: TdxPageViewInfo;
      const APt: TPoint): Boolean; virtual;
    ...
    function HitTestByPhysicalPoint(const APt: TPoint;
      ADetailsLevel: TdxDocumentLayoutDetailsLevel): TRect; virtual;
    function GetPhysicalBounds(APageViewInfo: TdxPageViewInfo;
      const ABounds: TRect): TRect; virtual;
    function ExpandClipBoundsToPaddings(const AClipBounds: TRect)
      : TRect; virtual;
    function CalculateVisiblePageBounds(const APageBounds: TRect;
      APageViewInfo: TdxPageViewInfo): TRect; virtual;
    procedure ResetBackColor; virtual;
    procedure LayoutOptionsChanged(ASender: TObject;
      Args: TdxRichEditNotificationOptionsChangedArgs); virtual;
    ...
    property FixedLeftTextBorderOffset: Integer
      read GetFixedLeftTextBorderOffset;
    property MinHeight: Integer read GetMinHeight;
    property MinWidth: Integer read GetMinWidth;
    property TextColors: TdxTextColors read GetTextColors;
    property PageViewInfoGenerationIsAboutToComplete: Boolean
      read FPageViewInfoGenerationIsAboutToComplete
      write FPageViewInfoGenerationIsAboutToComplete;
    property PageViewInfoGenerationComplete: Boolean
      read FPageViewInfoGenerationComplete
      write SetPageViewInfoGenerationComplete;
  ...
  public
    ...
    class function EnsurePositionVisibleWhenHiddenTextNotShown
      (ADocumentModel: TdxDocumentModel;
      const AModelPosition: TdxDocumentModelPosition)
      : TdxDocumentModelPosition; static;

    procedure CheckExecutedAtUIThread; inline;
    procedure CheckExecutedAtBackgroundThread; inline;

    function CreateLogicalPoint(const AClientBounds: TRect;
      const APoint: TPoint): TPoint; virtual;
    function CreateDocumentLayoutExporter(APainter: TdxPainter;
      AAdapter: TdxGraphicsDocumentLayoutExporterAdapter;
      APageViewInfo: TdxPageViewInfo; const ABounds: TRect)
      : TdxDocumentLayoutExporter; virtual; abstract;
    ...
    function CalculatePageContentClipBounds(APage: TdxPageViewInfo)
      : TRect; virtual;
    ...
    function CreateBackgroundPainter: TdxRichEditViewBackgroundPainter;
      virtual; abstract;
    function CreatePhysicalPoint(APageViewInfo: TdxPageViewInfo;
      const APoint: TPoint): TPoint; virtual;
    ...
    procedure EnsureCaretVisible(ACheckCaretVisibility
      : Boolean = True); virtual;
    procedure OnEndDocumentUpdate; virtual;
    procedure OnResize(const ABounds: TRect;
      AEnsureCaretVisibleOnResize: Boolean); overload;
    procedure OnResize(const ABounds: TRect; AEnsureCaretVisibleOnResize,
      AEnsureOptimalHorizontalScrollbarPosition: Boolean); overload; virtual;
    function CreateLogicalRectangle(APageViewInfo: TdxPageViewInfo;
      const ABounds: TRect): TRect; virtual;
    function CreatePhysicalRectangle(APageViewInfo: TdxPageViewInfo;
      const ABounds: TRect): TRect; overload; virtual;
    function CreatePhysicalRectangle(const APageViewInfoClientBounds,
      ABounds: TRect): TRect; overload; virtual;
    procedure ResetPages(AStrategy: TdxPageGenerationStrategyType); virtual;
    procedure GeneratePages; virtual;

    procedure EnsureFormattingCompleteForPreferredPage(APreferredPageIndex
      : Integer); virtual;
    function GetPageViewInfoFromPoint(const APoint: TPoint;
      AStrictSearch: Boolean): TdxPageViewInfo; virtual;
    function LookupPageViewInfoByPage(APage: TdxPage): TdxPageViewInfo; virtual;
    procedure OnSelectionChanged; virtual;
    procedure ProcessSelectionChanges(const AChangeActions
    ...
    function CreatePhysicalRectangleFast(APageViewInfo: TdxPageViewInfo;
      const ABounds: TRect): TRect; virtual;
    function CalculateNearestCharacterHitTest(const APoint: TPoint;
      APieceTable: TdxPieceTable): TdxRichEditHitTestResult; virtual;
    function CalculateNearestPageHitTest(const APoint: TPoint;
      AStrictHitIntoPageBounds: Boolean): TdxRichEditHitTestResult; virtual;
    function CalculateFloatingObjectHitTest(APage: TdxPage;
      const APoint: TPoint;
      const APredicate: TdxPredicate<TdxFloatingObjectBox>)
      : TdxFloatingObjectBox; virtual;
    function CalculateFloatingObjectHitTestCore(const APoint: TPoint;
      AList: TdxFloatingObjectBoxList;
      const APredicate: TdxPredicate<TdxFloatingObjectBox>)
      : TdxFloatingObjectBox; virtual;
    function CalculateHitTest(const APoint: TPoint;
      ADetailsLevel: TdxDocumentLayoutDetailsLevel)
      : TdxRichEditHitTestResult; virtual;
    function HitTestCore(var ARequest: TdxRichEditHitTestRequest;
      AStrictHitIntoPageBounds: Boolean): TdxRichEditHitTestResult; virtual;
    ...
    function GetHyperlinkField(AHitTestResult: TdxRichEditHitTestResult)
      : TdxField;
    procedure Visit(const AVisitor: IdxRichEditViewVisitor); virtual; abstract;

    property VerticalScrollController: TdxRichEditViewVerticalScrollController
      read FVerticalScrollController;
    property HorizontalScrollController
      : TdxRichEditViewHorizontalScrollController
      read FHorizontalScrollController;
    property MatchHorizontalTableIndentsToTextEdge: Boolean
      read GetMatchHorizontalTableIndentsToTextEdge;

    property ActualBackColor: TdxAlphaColor read GetActualBackColor;
    property ActualPadding: TRect read GetActualPadding;
    property AllowDisplayLineNumbers: Boolean read FAllowDisplayLineNumbers
      write SetAllowDisplayLineNumbers stored IsAllowDisplayLineNumbersStored;
    property Bounds: TRect read FBounds write SetBounds;
    property CaretPosition: TdxCaretPosition read FCaretPosition
      write SetCaretPosition;
    property Control: IdxRichEditControl read FControl;
    property DefaultHitTestPageAccuracy: TdxHitTestAccuracy
      read GetDefaultHitTestPageAccuracy;
    property DocumentLayout: TdxDocumentLayout read FDocumentLayout;
    property DocumentModel: TdxDocumentModel read GetDocumentModel;
    property FormattingController: TdxDocumentFormattingController
      read FFormattingController;
    property HoverLayout: IdxHoverLayoutItem read FHoverLayout
      write FHoverLayout;
    property LookAndFeel: TcxLookAndFeel read GetLookAndFeel;
    property PageViewInfoGenerator: TdxPageViewInfoGenerator
      read FPageViewInfoGenerator;
    ...
    property ZoomFactor: Single read FZoomFactor write SetZoomFactor
      stored IsZoomFactorStored;
    property Padding: TdxRichEditControlPadding read FPadding write SetPadding
      stored IsPaddingStored;
    property BackColor: TdxAlphaColor read FBackColor write SetBackColor
      stored IsBackColorStored;
    property BackColorChanged: TdxEventHandler read FOnBackColorChanged;
    property SelectionLayout: TdxSelectionLayout read FSelectionLayout
      write SetSelectionLayout;
    property ScaleFactor: Single read GetScaleFactor;
    property ShowHorizontalRulerByDefault: Boolean
      read GetShowHorizontalRulerByDefault;
    property ShowVerticalRulerByDefault: Boolean
      read GetShowVerticalRulerByDefault;
  ...
  protected
    ...
    property RichEditControl: IdxRichEditControl read FRichEditControl;
  public
    constructor Create(const ARichEditControl: IdxRichEditControl);
      reintroduce; virtual;
    destructor Destroy; override;
    function GetViewByType(AType: TdxRichEditViewType)
      : TdxRichEditView; virtual;
  ...
  TdxScrollBarAdapter = class abstract(TcxIUnknownObject, IdxBatchUpdateHandler,
    IdxBatchUpdatable)
  private
  ...
  public
    constructor Create(const AScrollBar: IdxOfficeScrollbar;
      const AAdapter: IdxPlatformSpecificScrollBarAdapter);
    destructor Destroy; override;
    ...
    function CreateEmulatedScrollEventArgs(AEventType: TdxScrollEventType)
      : TdxScrollEventArgs; virtual;
  ...
  protected
    ...
    procedure OnScroll(ASender: TObject; E: TdxScrollEventArgs);
      virtual; abstract;
  ...
  TdxRichEditViewHorizontalScrollController = class abstract
    (TdxOfficeScrollControllerBase)
  protected
  ...
  public
    ...
    procedure ScrollByLeftInvisibleWidthDelta(ALeftInvisibleWidthDelta
      : Int64); virtual;
  ...
  TdxRichEditViewVerticalScrollController = class abstract
    (TdxOfficeScrollControllerBase)
  protected
    function CreateScrollBarAdapter: TdxScrollBarAdapter; override;
    ...
    procedure ApplyNewScrollValueToScrollEventArgs(E: TdxScrollEventArgs;
  ...
  public
    procedure ScrollByTopInvisibleHeightDelta(ATopInvisibleHeightDelta
      : Int64); virtual;
    procedure ScrollToAbsolutePosition(AValue: Int64); virtual;
  ...
  strict private
    FView: TdxRichEditView;
  protected
    function CalculateScrollDeltaForInvisibleRows(APhysicalVerticalOffset
      : Integer): Int64;
    function CalculateScrollDeltaForVisibleRows(APhysicalVerticalOffset
      : Integer; out ADelta: Int64): Boolean; virtual; abstract;
    function LookupIntersectingRowIndexInInvisibleRows
      (AGenerator: TdxInvisiblePageRowsGenerator; Y: Integer): Integer;
    ...
    function CalculateInvisiblePhysicalVerticalOffset(APhysicalVerticalOffset
    ...
    function CalculateRowsTotalVisibleHeight(AGenerator
      : TdxPageViewInfoGeneratorBase; ALastRowIndex: Integer;
      ABottomY: Integer): Int64;
  public
    constructor Create(AView: TdxRichEditView);
    function CalculateScrollDelta(APhysicalVerticalOffset: Integer)
      : Int64; virtual;
  ...
  TdxScrollUpByPhysicalHeightCalculator = class
    (TdxScrollByPhysicalHeightCalculator)
  protected
    function CalculateScrollDeltaForVisibleRows(APhysicalVerticalOffset
      : Integer; out ADelta: Int64): Boolean; override;
  ...
  TdxScrollDownByPhysicalHeightCalculator = class
    (TdxScrollByPhysicalHeightCalculator)
  protected
    function CalculateScrollDeltaForVisibleRows(APhysicalVerticalOffset
      : Integer; out ADelta: Int64): Boolean; override;
  ...
  protected
    function CreateDocumentModel(AParagraphNumerationCopyOptions
      : TdxParagraphNumerationCopyOptions;
      AFormattingCopyOptions: TdxFormattingCopyOptions;
      ASourcePieceTable: TdxPieceTable;
      ASelectionRanges: TdxSelectionRangeCollection;
      ASuppressFieldsUpdate: Boolean = False;
      AGetTextOptions: PdxTextFragmentOptions = nil): TdxDocumentModel;
    function IsLastParagraphRunSelected(APieceTable: TdxPieceTable;
      ASelectionRanges: TdxSelectionRangeCollection): Boolean;
    procedure SetDataObject(APieceTable: TdxPieceTable;
    ...
    procedure DoCalculateDocumentVariable(Sender: TObject;
      E: TdxCalculateDocumentVariableEventArgs);
  ...
  public
    ...
    procedure CopyDocumentRange(APieceTable: TdxPieceTable;
      ASelectionCollection: TdxSelectionRangeCollection);

    function GetRtfText(APieceTable: TdxPieceTable;
      ASelectionRanges: TdxSelectionRangeCollection;
    ...
    function GetPlainText(APieceTable: TdxPieceTable;
      ASelectionCollection: TdxSelectionRangeCollection;
      AOptions: TdxPlainTextDocumentExporterOptions = nil;
      AGetTextOptions: PdxTextFragmentOptions = nil): string;
    function GetOpenXmlBytes(APieceTable: TdxPieceTable;
      ASelectionCollection: TdxSelectionRangeCollection;
    ...
    property FixLastParagraph: Boolean read FFixLastParagraph
      write FFixLastParagraph;
    property KeepFieldCodeViewState: Boolean read FKeepFieldCodeViewState
      write FKeepFieldCodeViewState;
  end;
...
type
  ...
  protected
    function CreateDocumentSelectionLayout(AStart: TdxDocumentLayoutPosition;
      AEnd: TdxDocumentLayoutPosition): TdxDocumentSelectionLayout; override;
  public
    function CreateDocumentLayoutPosition(ALogPosition: TdxDocumentLogPosition)
      : TdxDocumentLayoutPosition; override;
  end;
  ...
  protected
    ...
    procedure TryToAddTextBoxFrameSelectionLayout
      (AStart: TdxDocumentLayoutPosition; AEnd: TdxDocumentLayoutPosition);
  public
    procedure AddToPageSelection(APage: TdxPage;
      AWhere: TdxPageSelectionLayoutsCollection); override;
...
constructor TdxTableCellSelectionLayout.Create(ACell: TdxTableCellViewInfo;
  APage: TdxPage);
begin
...
procedure TdxSemitransparentSelectionPainter.FillRectangle
  (const ABounds: TRect);
begin
  Graphics.FillRectangle(ABounds, SelectionColor);
end;

procedure TdxSemitransparentSelectionPainter.DrawRectangle
  (const ABounds: TRect);
begin
  Graphics.Rectangle(ABounds, SelectionColor, dxacNone);
end;

procedure TdxSemitransparentSelectionPainter.DrawLine(const AFrom: TPoint;
  const ATo: TPoint);
begin
...
procedure TdxRectangularObjectSelectionPainter.DrawLine(const AFrom: TPoint;
  const ATo: TPoint);
begin
  Graphics.Line(AFrom.X, AFrom.Y, ATo.X, ATo.Y, ShapeBorderColor,
    ShapeBorderWidth);
end;

procedure TdxRectangularObjectSelectionPainter.FillRectangle
  (const ABounds: TRect);
begin
  Graphics.FillRectangle(ABounds, SelectionColor);
end;

procedure TdxRectangularObjectSelectionPainter.DrawRectangle
  (const ABounds: TRect);
begin
...
constructor TdxRichEditControlPadding.Create(AOwner: TPersistent;
  const ADefaultValue: TRect);
begin
...
begin
  Create(AOwner, TRect.Create(ADefaultValue, ADefaultValue, ADefaultValue,
    ADefaultValue));
end;
...
begin
  case AIndex of
    ...
    3:
      Result := FValue.Right;
    4:
      Result := FValue.Bottom;
  else
    if (FValue.Left = FValue.Top) and (FValue.Left = FValue.Right) and
      (FValue.Left = FValue.Bottom) then
      Result := FValue.Left
    else
      Result := -1;
  end;
...
function TdxRichEditControlPadding.IsEqual(const APadding
  : TdxRichEditControlPadding): Boolean;
begin
...
begin
  case AIndex of
    1:
      Result := FValue.Left <> FDefaultValue.Left;
    2:
      Result := FValue.Top <> FDefaultValue.Top;
    3:
      Result := FValue.Right <> FDefaultValue.Right;
    4:
      Result := FValue.Bottom <> FDefaultValue.Bottom;
  else
...
constructor TdxScrollEventArgs.Create(AType: TdxScrollEventType;
  ANewValue: Integer; AScrollOrientation: TdxScrollOrientation);
begin
  inherited Create;
  FType := AType;
  FNewValue := ANewValue;
...
constructor TdxScrollEventArgs.Create(AType: TdxScrollEventType;
  AOldValue, ANewValue: Integer; AScrollOrientation: TdxScrollOrientation);
begin
  inherited Create;
  FType := AType;
  FOldValue := AOldValue;
...
constructor TdxControlCommand.Create(const ARichEditControl
  : IdxRichEditControl);
begin
  inherited Create;
...
function TdxControlCommand.GetCommandExecutionListener
  : IdxCommandExecutionListenerService;
begin
  Result := nil;
end;
...
begin
  ...
  Result := ATable.PieceTable.CanEditRange(AStartParagraph.LogPosition,
    AEndParagraph.EndLogPosition);
end;
...
function TdxRichEditCommand.ExtendSelectionToParagraphBoundary
...
begin
  ...
  AFirstSectionIndex := DocumentModel.FindSectionIndex
    (ASelection.NormalizedStart);
  ...
  Result.Start := AMainPieceTable.Paragraphs[AFirstSection.FirstParagraphIndex]
    .LogPosition;
...
function TdxRichEditCommand.ExtendToParagraphBoundary(ASelectionItem
  : TdxSelectionItem): TdxSelectionItem;
var
...
begin
  ...
  Result.Start := APieceTable.Paragraphs[ANormalizedStart.ParagraphIndex]
    .LogPosition;
  ALastParagraph := APieceTable.Paragraphs[ANormalizedEnd.ParagraphIndex];
  if (ANormalizedStart.ParagraphIndex <> ALastParagraph.Index) and
    (ALastParagraph.LogPosition = ANormalizedEnd.LogPosition) then
...
procedure TdxRichEditCommand.ApplyCommandsRestriction
  (const AState: IdxCommandUIState; AOption: TdxDocumentCapability;
  AdditionEnabledCondition: Boolean = True);
begin
  AState.Enabled := AdditionEnabledCondition and
    (AOption <> TdxDocumentCapability.Disabled) and
    (AOption <> TdxDocumentCapability.Hidden);
  AState.Visible := (AOption <> TdxDocumentCapability.Hidden);
end;

procedure TdxRichEditCommand.ApplyCommandRestrictionOnEditableControl
  (const AState: IdxCommandUIState; AOption: TdxDocumentCapability);
begin
  AState.Enabled := IsContentEditable and
    (AOption <> TdxDocumentCapability.Disabled) and
    (AOption <> TdxDocumentCapability.Hidden);
  AState.Visible := AOption <> TdxDocumentCapability.Hidden;
end;
...
procedure TdxRichEditCommand.ApplyDocumentProtectionToSelectedCharacters
  (const AState: IdxCommandUIState);
begin
  AState.Enabled := AState.Enabled and CanEditSelection;
end;

procedure TdxRichEditCommand.ApplyDocumentProtectionToSelectedParagraphs
  (const AState: IdxCommandUIState);
var
...
procedure TdxRichEditCommand.ApplyDocumentProtectionToSelectedSections
  (const AState: IdxCommandUIState);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        AState.Enabled := DocumentModel.MainPieceTable.CanEditRange
          (AItem.NormalizedStart, AItem.NormalizedEnd);
      finally
        AItem.Free;
      end;
    end;
...
procedure TdxRichEditCommand.ApplyDocumentProtectionToTable
  (const AState: IdxCommandUIState; ATable: TdxTable);
begin
  if AState.Enabled then
...
constructor TdxCaretPosition.Create(AView: TdxRichEditView;
...
var
  AWidth: Integer;
begin
  AWidth := View.DocumentModel.LayoutUnitConverter.PixelsToLayoutUnits(1,
    DocumentModel.DpiX);
  ...
  begin
    if (FCaretBoundsPosition = FPosition) or
      (FCaretBoundsPosition.Row <> FPosition.Row) then
    begin
      Result := FPosition.Character.TightBounds;
      Result.Intersect(FPosition.Row.Bounds);
      Result.Width := AWidth;
...
function TdxCaretPosition.ShouldUpdateModelPosition(ALogPosition
  : TdxDocumentLogPosition): Boolean;
var
...
begin
  ...
  if ARunIndex >= FModelPosition.PieceTable.Runs.Count then
    Exit(True);
  if (ALogPosition < FModelPosition.RunStartLogPosition) or
    (ALogPosition > FModelPosition.RunEndLogPosition) then
    Exit(True);
  AParagraph := AParagraphs[FModelPosition.ParagraphIndex];
  if (ARunIndex > AParagraph.LastRunIndex) or
    (ARunIndex < AParagraph.FirstRunIndex) then
    Exit(True);
  Result := False;
end;

function TdxCaretPosition.CreateCaretDocumentLayoutPosition
...
function TdxCaretPosition.CreateExplicitCaretPosition
  (ALogPosition: TdxDocumentLogPosition): TdxCaretPosition;
begin
  Result := TdxExplicitCaretPosition.Create(View, ALogPosition,
    PreferredPageIndex);
end;

function TdxCaretPosition.CreateInputPosition(ALogPosition
  : TdxDocumentLogPosition): TdxInputPosition;
var
...
begin
  ...
  begin
    AField := DocumentModel.ActivePieceTable.FindFieldByRunIndex
      (AFormattingRunIndex);
    ARun := DocumentModel.ActivePieceTable.Runs[AField.FirstRunIndex]
  end
  else if not DocumentModel.ActivePieceTable.VisibleTextFilter.IsRunVisible
    (AFormattingRunIndex) then
  begin
    AFormattingRunIndex := DocumentModel.ActivePieceTable.VisibleTextFilter.
      GetPrevVisibleRunIndex(ARunIndex);
    ARun := DocumentModel.ActivePieceTable.Runs[AFormattingRunIndex];
    if not(ARun is TdxTextRun) then
    begin
      AFormattingRunIndex := DocumentModel.ActivePieceTable.VisibleTextFilter.
        GetNextVisibleRunIndex(ARunIndex);
      ARun := DocumentModel.ActivePieceTable.Runs[AFormattingRunIndex];
    end;
  end;
...
function TdxCaretPosition.GetActualLogPosition(AEndPosition
...
begin
  ...
  begin
    ...
    AEnd := AEndPosition + ASelection.ActiveSelection.LeftOffset +
      AVirtualEndLogPositionCompensation;
  end;
...
begin
  ACurrentFormattingPosition := CalculateCurrentFormatingPosition(ALogPosition);
  if (FCaretBoundsPosition <> nil) and
    (FCaretBoundsPosition.LogPosition = ACurrentFormattingPosition.LogPosition)
  then
...
procedure TdxCaretPosition.SetCaretBoundsPosition
  (const Value: TdxDocumentLayoutPosition);
begin
...
begin
  ...
  if LayoutPosition is TdxHeaderFooterDocumentLayoutPosition then
    TdxHeaderFooterDocumentLayoutPosition(LayoutPosition).PreferredPageIndex :=
      FPreferredPageIndex;
  if CaretBoundsPosition is TdxHeaderFooterDocumentLayoutPosition then
    TdxHeaderFooterDocumentLayoutPosition(CaretBoundsPosition)
      .PreferredPageIndex := FPreferredPageIndex;
  if LayoutPosition is TdxTextBoxDocumentLayoutPosition then
    TdxTextBoxDocumentLayoutPosition(LayoutPosition).PreferredPageIndex :=
      FPreferredPageIndex;
  if CaretBoundsPosition is TdxTextBoxDocumentLayoutPosition then
    TdxTextBoxDocumentLayoutPosition(CaretBoundsPosition).PreferredPageIndex :=
      FPreferredPageIndex;
end;
...
function TdxCaretPosition.Update(ADetailsLevel
  : TdxDocumentLayoutDetailsLevel): Boolean;
begin
  View.CheckExecutedAtUIThread;
  FPosition.SetLogPosition(VirtualLogPosition);
  UpdateModelPosition(Min(FPosition.LogPosition,
    FPosition.PieceTable.DocumentEndLogPosition));

  if not FPosition.Update(View.FormattingController.PageController.Pages,
    ADetailsLevel) then
  begin
  ...
  CaretBoundsPosition := UpdateCaretDocumentLayoutPosition
    (FPosition.DetailsLevel);
...
begin
  ...
  if APos.LogPosition = APos.PieceTable.Paragraphs[APos.ParagraphIndex].LogPosition
  then
    Exit(FPosition);
  ATextFilter := TdxPieceTable(APos.PieceTable).VisibleTextFilter;
  ALogPosition := ATextFilter.GetPrevVisibleLogPosition
    (VirtualLogPosition, False);
...
procedure TdxCaretPosition.UpdateModelPosition(ALogPosition
  : TdxDocumentLogPosition);
begin
  if not FModelPosition.IsValid or
    (FModelPosition.PieceTable <> DocumentModel.ActivePieceTable) then
    FModelPosition := TdxDocumentModelPosition.Create
      (DocumentModel.ActivePieceTable);
...
begin
  Result := False;
  ASelection := DocumentModel.Selection;
  if (not UsePreviousBoxBounds) and (ASelection.Length > 0) and
    (ASelection.&End > ASelection.Start) then
    if FPosition.IsValid(TdxDocumentLayoutDetailsLevel.Row) then
      if FPosition.Row.GetFirstPosition(FPosition.PieceTable)
        .LogPosition = FPosition.LogPosition then
      begin
...
procedure TdxDocumentSelectionLayout.AddNestedItem(APageIndex: Integer;
  const AItem: IdxSelectionLayoutItem);
begin
...
procedure TdxDocumentSelectionLayout.AddToPageSelection(APage: TdxPage;
  ATarget: TdxPageSelectionLayoutsCollection);
var
...
constructor TdxDocumentSelectionLayout.Create(ASelectionLayout
  : TdxSelectionLayout; AStart, AEnd: TdxDocumentLayoutPosition;
  APieceTable: TdxPieceTable);
begin
  inherited Create;
  FUpdatedSuccessfully := False;
  FHotZones := TdxHotZoneCollection.Create;
  FNestedItems := TObjectDictionary<Integer, TdxSelectionLayoutItemList>.Create
    ([doOwnsValues]);
...
procedure TdxDocumentSelectionLayout.Draw(const ASelectionPainter
  : IdxSelectionPainter);
begin
  NotImplemented;
end;

function TdxDocumentSelectionLayout.FindCellViewInfo(AModelCell: TdxTableCell;
  AColumn: TdxColumn): TdxTableCellViewInfo;
var
...
function TdxDocumentSelectionLayout.FindFloatingObject(APage: TdxPage;
  AIndex: TdxRunIndex): TdxFloatingObjectBox;
var
  AResultBox: TdxFloatingObjectBox;
begin
  AResultBox := FindFloatingObject
    (APage.InnerBackgroundFloatingObjects, AIndex);
...
function TdxDocumentSelectionLayout.FindFloatingObject(AFloatingObjects
  : TdxFloatingObjectBoxList; AIndex: TdxRunIndex): TdxFloatingObjectBox;
var
  APieceTable: TdxCustomPieceTable;
...
begin
  ...
  begin
    ...
    if (ABox.StartPos.RunIndex = AIndex) and (APieceTable = ABox.PieceTable)
    then
    begin
      Result := ABox;
      Break;
...
function TdxDocumentSelectionLayout.GetBox
  (APos: TdxDocumentLayoutPosition): TdxPage;
begin
...
function TdxDocumentSelectionLayout.HitTest(ALogPosition
  : TdxDocumentLogPosition; const ALogicalPoint: TPoint): Boolean;
begin
  Result := (ALogPosition >= Start.LogPosition) and
    (ALogPosition < &End.LogPosition);
end;

function TdxDocumentSelectionLayout.
  TryToCreateRectangularObjectSelectionLayoutForSinglePosition(AStart,
  AEnd: TdxDocumentLayoutPosition): TdxRectangularObjectSelectionLayout;
var
  ASelection: TdxSelection;
...
begin
  ...
  if (AStart.LogPosition > ASelection.NormalizedStart) or
    (ASelection.NormalizedStart > AEnd.LogPosition) then
    Exit(nil);
  ...
  try
    ...
    begin
      ABox := FindFloatingObject(APos.Page,
        ASelection.Interval.NormalizedStart.RunIndex);
      if (ABox <> nil) and
        (ABox.PieceTable = DocumentLayout.DocumentModel.GetActivePieceTableCore)
      then
      begin
        Result := TdxRectangularObjectSelectionLayout.CreateResizableLayout
          (View, ABox, ASelection.NormalizedStart, PieceTable);
        Result.HitTestTransform := ABox.CreateBackwardTransformUnsafe;
        Exit;
      end;
    end;
    Result := TryToCreateRectangularObjectSelectionLayoutByBox(APos,
      ASelection.NormalizedStart);
  finally
    APos.Free;
  end;
...
function TdxDocumentSelectionLayout.
  TryToCreateRectangularObjectSelectionLayoutByBox
  (ALayoutPos: TdxDocumentLayoutPosition; ALogPos: TdxDocumentLogPosition)
  : TdxRectangularObjectSelectionLayout;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := TdxRectangularObjectSelectionLayout.CreateResizableLayout(View,
        AInlinePicture, ALogPos, PieceTable);
    end
  end;
end;
...
begin
  ASelectionRunsInfo := PieceTable.FindRunInfo(FStart.LogPosition,
    FEnd.LogPosition - FStart.LogPosition);
  try
    ARunInfo := ASelectionRunsInfo.Start;
  finally
    ASelectionRunsInfo.Free;
  end;
  while ARunInfo.LogPosition < Min(FEnd.LogPosition,
    PieceTable.DocumentEndLogPosition + 1) do
  begin
    AFloatSelection := TryCreateRectangularObjectSelectionLayoutByAnchorRun
      (ARunInfo, AAnchorPos);
    try
      if AFloatSelection <> nil then
        AddNestedItem(AAnchorPos.Page.PageIndex, AFloatSelection);
      ASelectionRunsInfo := PieceTable.FindRunInfo
        (ARunInfo.RunEndLogPosition + 1, 0);
      try
        ARunInfo := ASelectionRunsInfo.Start;
      finally
        ASelectionRunsInfo.Free;
...
function TdxDocumentSelectionLayout.TryGetNextPosition
  (var APos: TdxDocumentLayoutPosition): Boolean;
var
  AParagraph: TdxParagraph;
  ACell: TdxTableCell;
...
begin
  if not APos.Update(DocumentLayout.Pages, TdxDocumentLayoutDetailsLevel.Row,
    True) then
  begin
...
function TdxDocumentSelectionLayout.TryGetNextPositionCore
  (AParagraph: TdxParagraph; ACell: TdxTableCell;
  var APos: TdxDocumentLayoutPosition): Boolean;
var
  AStop: TdxDocumentLayoutPosition;
...
begin
  ...
  if (ACell <> nil) and
    ((Start.LogPosition > PieceTable.Paragraphs[ACell.StartParagraphIndex]
    .LogPosition) or (&End.LogPosition <= PieceTable.Paragraphs
    [ACell.EndParagraphIndex].EndLogPosition)) then
  begin
    ACell := nil;
    ...
    AVisibleEndLogPosition := PieceTable.VisibleTextFilter.
      GetPrevVisibleLogPosition(AEndLogPosition, True);
  ...
  begin
    ...
    begin
      ...
      while (AParent <> nil) and
        ((Start.LogPosition <= PieceTable.Paragraphs
        [AParent.StartParagraphIndex].LogPosition) or
        (&End.LogPosition > PieceTable.Paragraphs[AParent.EndParagraphIndex]
        .EndLogPosition)) do
      begin
        ACell := AParent;
        AParent := ACell.Table.ParentCell;
    ...
    begin
      AVisibleEndLogPosition :=
        PieceTable.VisibleTextFilter.GetPrevVisibleLogPosition
        (&End.LogPosition, True);
      AStop := CreateDocumentLayoutPosition(AVisibleEndLogPosition);
    end
    else
    begin
      AField := PieceTable.FindFieldByRunIndex(AParagraph.LastRunIndex);
      if (AField <> nil) and AField.IsCodeView and
        AField.Result.Contains(AParagraph.LastRunIndex) and
  ...
  try
    if not AStop.Update(DocumentLayout.Pages, TdxDocumentLayoutDetailsLevel.Row,
      True) then
      Exit(False);
    Result := TryGetNextPositionBySelectionLayoutItem(AStop,
      AVisibleEndLogPosition, ACell, APos);
  finally
    AStop.Free;
  end;
end;

function TdxDocumentSelectionLayout.TryCreateRectangularObjectSelectionLayout
  (AStart, AEnd: TdxDocumentLayoutPosition)
  : TdxRectangularObjectSelectionLayout;
var
  ASelection: TdxSelection;
...
begin
  ...
  try
    if (ASelection.Length = 1) or ((ASelectedParagraphs.Count > 0) and
      (ASelectedParagraphs.First.FrameProperties <> nil) and
      not ASelectedParagraphs.First.IsInCell) then
      Exit(TryToCreateRectangularObjectSelectionLayoutForSinglePosition
        (AStart, AEnd));
...
function TdxDocumentSelectionLayout.
  TryCreateRectangularObjectSelectionLayoutByAnchorRun
  (ARunInfo: TdxDocumentModelPosition; out APos: TdxDocumentLayoutPosition)
  : TdxRectangularObjectSelectionLayout;
var
  ASelection: TdxSelection;
...
begin
  ...
  then
  begin
    Result := TdxRectangularObjectSelectionLayout.Create(View, ABox,
      ASelection.NormalizedStart, FPieceTable);
    Result.HitTestTransform := ABox.CreateBackwardTransformUnsafe;
    Exit;
  end;
  Result := nil;
...
function TdxDocumentSelectionLayout.
  TryToCreateRectangularObjectSelectionLayoutByField(AField: TdxField)
  : TdxRectangularObjectSelectionLayout;
var
  ASelection: TdxSelection;
...
function TdxDocumentSelectionLayout.TryGetNextPositionBySelectionLayoutItem
  (AStop: TdxDocumentLayoutPosition;
  AVisibleEndLogPosition: TdxDocumentLogPosition; ACell: TdxTableCell;
  var APos: TdxDocumentLayoutPosition): Boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        AFracture := CreateDocumentLayoutPosition
          (APos.Row.GetLastPosition(PieceTable).LogPosition);
        if not AFracture.Update(DocumentLayout.Pages,
          TdxDocumentLayoutDetailsLevel.Row, True) then
          Exit(False);
    ...
    begin
      ...
      begin
        ...
        begin
          AFracture := CreateDocumentLayoutPosition
            (AInfo.GetLastRow(APos.Column).GetLastPosition(PieceTable)
            .LogPosition);
          if not AFracture.Update(DocumentLayout.Pages,
            TdxDocumentLayoutDetailsLevel.Row, True) then
            Exit(False);
        ...
        if (ACell.Table.ParentCell <> nil) and
          (PieceTable.Paragraphs[ACell.Table.ParentCell.StartParagraphIndex]
          .LogPosition >= Start.LogPosition) and
          (PieceTable.Paragraphs[ACell.Table.ParentCell.EndParagraphIndex]
          .LogPosition < &End.LogPosition) then
        begin
          APos := CreateDocumentLayoutPosition
            (PieceTable.Paragraphs[ACell.Table.ParentCell.EndParagraphIndex]
            .LogPosition + 1);
          Exit(True);
        end
    ...
    Result := TryGetNextPositionBySelectionLayoutItemCore(AItem, AStop,
      AVisibleEndLogPosition, APos);
  finally
    AFracture.Free;
  end;
end;

function TdxDocumentSelectionLayout.TryGetNextPositionBySelectionLayoutItemCore
  (const AItem: IdxSelectionLayoutItem; AStop: TdxDocumentLayoutPosition;
  AVisibleEndLogPosition: TdxDocumentLogPosition;
  var APos: TdxDocumentLayoutPosition): Boolean;
var
...
begin
  ...
  begin
    ...
    begin
      if not APos.Update(DocumentLayout.Pages,
        TdxDocumentLayoutDetailsLevel.Character, True) or
        not AStop.Update(DocumentLayout.Pages,
        TdxDocumentLayoutDetailsLevel.Character, True) then
        Exit(False);
    ...
    begin
      if not APos.Update(DocumentLayout.Pages,
        TdxDocumentLayoutDetailsLevel.Row, True) or
        not AStop.Update(DocumentLayout.Pages,
        TdxDocumentLayoutDetailsLevel.Row, True) then
        Exit(False);
  ...
  ANextLogPosition := PieceTable.VisibleTextFilter.GetNextVisibleLogPosition
    (AStop.LogPosition, True, True);
...
procedure TdxTextBoxDocumentSelectionLayout.AddToPageSelection(APage: TdxPage;
  AWhere: TdxPageSelectionLayoutsCollection);
var
  ABox: TdxFloatingObjectBox;
  APos: TdxDocumentLayoutPosition;
begin
  ...
  begin
    ...
    APos := TdxDocumentLayoutPosition
      (DocumentLayout.CreateLayoutPosition(ABox.PieceTable,
      FTextBoxFrameSelection.LogStart, 0));
    try
      if APos.Update(DocumentLayout.Pages, TdxDocumentLayoutDetailsLevel.Page)
        and (APage = APos.Page) then
...
procedure TdxTextBoxDocumentSelectionLayout.TryToAddTextBoxFrameSelectionLayout
  (AStart: TdxDocumentLayoutPosition; AEnd: TdxDocumentLayoutPosition);
var
...
begin
  ATextBoxContentType := Safe<TdxTextBoxContentType>.Cast
    (DocumentModel.ActivePieceTable.ContentType);
  ...
  begin
    ...
    begin
      ...
      AResult := TdxRectangularObjectSelectionLayout.CreateResizableLayout(View,
        ABox, ALogPosition, TdxPieceTable(ATextBoxContentType.PieceTable));
...
function TdxSelectionLayout.GetFirstDocumentSelectionLayout
  : TdxDocumentSelectionLayout;
begin
...
function TdxSelectionLayout.GetLastDocumentSelectionLayout
  : TdxDocumentSelectionLayout;
begin
  if (FDocumentSelectionLayouts = nil) or (FDocumentSelectionLayouts.Count = 0)
  then
    Update;
  Result := FDocumentSelectionLayouts[FDocumentSelectionLayouts.Count - 1];
end;

function TdxSelectionLayout.GetPageSelection(APage: TdxPage)
  : TdxPageSelectionLayoutsCollection;
var
  I: Integer;
begin
  if FDocumentSelectionLayouts = nil then
...
function TdxSelectionLayout.HitTest(ALogPosition: TdxDocumentLogPosition;
  ALogicalPoint: TPoint): Boolean;
var
  I: Integer;
begin
...
begin
  Result := (StartLayoutPosition.Row.Boxes[0].StartPos = StartLayoutPosition.
    Character.StartPos);
end;
...
begin
  if (FDocumentSelectionLayouts <> nil) and
    (FDocumentSelectionLayouts.Count > 0) and LastDocumentSelectionLayout.UpdatedSuccessfully
  then
    Exit;
  ...
  begin
    ...
    if (ASelection.Length <= 0) and (ASelection.Items.Count = 1) and
      (ASelectionItem.LeftOffset = 0) and (ASelectionItem.RightOffset = 0) then
...
function TdxSelectionLayout.CalculateHotZone(AResult: TdxRichEditHitTestResult;
  AView: TdxRichEditView): TdxHotZone;
var
...
begin
  ...
  begin
    ...
    if AHotZone.HitTest(AResult.LogicalPoint, AView.DocumentModel.Dpi,
      AView.ScaleFactor) then
    begin
      Result := AHotZone;
      Break;
    end;
...
constructor TdxSelectionLayout.Create(AView: TdxRichEditView;
  APreferredPageIndex: Integer);
begin
  inherited Create;
  Assert(AView <> nil);
...
function TdxSelectionLayout.CreateDocumentLayoutPosition
  (ALogPosition: TdxDocumentLogPosition): TdxDocumentLayoutPosition;
begin
  Result := TdxDocumentLayoutPosition.Create(DocumentLayout,
    TdxDocumentModel(DocumentLayout.DocumentModel).Selection.PieceTable,
    ALogPosition);
end;

function TdxSelectionLayout.CreateDocumentSelectionLayout(AStart,
  AEnd: TdxDocumentLayoutPosition): TdxDocumentSelectionLayout;
begin
  Result := TdxDocumentSelectionLayout.Create(Self, AStart, AEnd,
    View.DocumentModel.ActivePieceTable);
end;

{ TdxTextBoxSelectionLayout }

function TdxTextBoxSelectionLayout.CreateDocumentLayoutPosition
  (ALogPosition: TdxDocumentLogPosition): TdxDocumentLayoutPosition;
begin
  Result := TdxTextBoxDocumentLayoutPosition.Create(DocumentLayout,
    TdxTextBoxContentType(TdxDocumentModel(DocumentLayout.DocumentModel)
    .Selection.PieceTable.ContentType), ALogPosition, PreferredPageIndex);
end;

function TdxTextBoxSelectionLayout.CreateDocumentSelectionLayout
  (AStart: TdxDocumentLayoutPosition; AEnd: TdxDocumentLayoutPosition)
  : TdxDocumentSelectionLayout;
begin
  Result := TdxTextBoxDocumentSelectionLayout.Create(Self, AStart, AEnd,
    View.DocumentModel.ActivePieceTable);
end;

{ TdxHeaderFooterSelectionLayout }

function TdxHeaderFooterSelectionLayout.CreateDocumentLayoutPosition
  (ALogPosition: TdxDocumentLogPosition): TdxDocumentLayoutPosition;
begin
  Result := TdxHeaderFooterDocumentLayoutPosition.Create(DocumentLayout,
    TdxDocumentModel(DocumentLayout.DocumentModel).Selection.PieceTable,
    ALogPosition, PreferredPageIndex);
end;
...
begin
  inherited Create;
  FPageViewInfos := TdxRichEditViewPageViewInfoCollection.Create
    (ADocumentLayout);
end;
...
procedure TdxRichEditPaintersBase<TLayoutItem, TPainter>.Add<T>
  (APainter: TPainter);
begin
...
function TdxRichEditPaintersBase<TLayoutItem, TPainter>.Get(AType: TClass)
  : TPainter;
begin
...
procedure TdxRichEditPaintersBase<TLayoutItem, TPainter>.AddDefault
  (APainter: TPainter);
begin
  FreeAndNil(FDefaultPainter);
...
procedure TdxRichEditPaintersBase<TLayoutItem, TPainter>.InternalAdd
  (AType: TClass; APainter: TPainter);
begin
  Painters.AddOrSetValue(AType, APainter);
end;

procedure TdxRichEditPaintersBase<TLayoutItem, TPainter>.InternalRemove
  (AType: TClass);
begin
...
function TdxRichEditViewPainter.BeginDrawPageContentInPixels
  (APageViewInfo: TdxPageViewInfo): TdxGPMatrix;
var
  AOrigin: TPoint;
...
begin
  ...
  AOrigin := DocumentModel.LayoutUnitConverter.LayoutUnitsToPixels
    (APageViewInfo.ClientBounds.Location, FGraphics.DpiX, FGraphics.DpiY);
  AOrigin.X := AOrigin.X - DocumentModel.LayoutUnitConverter.LayoutUnitsToPixels
    (FPhysicalLeftInvisibleWidth, FGraphics.DpiX);
...
function TdxRichEditViewPainter.ClipPageContentForDrawingInPixels
  (APageViewInfo: TdxPageViewInfo; APainter: TdxPainter): TdxRectF;
var
  AClipBounds: TRect;
...
begin
  ...
  AClipBounds := DocumentModel.LayoutUnitConverter.LayoutUnitsToPixels
    (AClipBounds, FGraphics.DpiX, FGraphics.DpiY);
  ...
  APainter.ClipBounds := GetNewClientBounds(cxRect(AActualClipBounds),
    AOldClipBounds);
  Result := AOldClipBounds;
end;

procedure TdxRichEditViewPainter.EndDrawPageContentInPixels
...
procedure TdxRichEditViewPainter.AddDecoratorPainter(const APainter
  : IdxDecoratorPainter);
begin
...
begin
  ...
  FMinReadableTextHeight :=
    Round(DocumentModel.LayoutUnitConverter.PixelsToLayoutUnits(6,
    FGraphics.DpiY) / View.ZoomFactor);
...
begin
  SelectionPainters.AddDefault(TdxSemitransparentSelectionPainter.Create
    (AGraphics));
  SelectionPainters.Add<TdxRectangularObjectSelectionLayout>
    (TdxRectangularObjectSelectionPainter.Create(AGraphics));
end;
...
procedure TdxRichEditViewPainter.BeginDrawPageContent(APage: TdxPageViewInfo;
...
begin
  ...
  AOrigin.X := AMeasurer.SnapToPixels(AOrigin.X, FGraphics.DpiX);
  FOriginModifier := TdxHdcOriginModifier.Create(FGraphics, AOrigin,
    View.ScaleFactor, TdxHdcOriginModifier.TMode.Combine);
end;

procedure TdxRichEditViewPainter.ClipPageContent(APage: TdxPageViewInfo;
  APainter: TdxPainter);
var
  AClipBounds: TRect;
...
procedure TdxRichEditViewPainter.EndDrawPageContent(APage: TdxPageViewInfo;
  ATransform: TdxGPMatrix);
begin
...
begin
  ...
  ATransform.Translate(ATransform.OffsetX * (AScaleX - 1),
    ATransform.OffsetY * (AScaleY - 1));

  FGraphics.PageUnit := TdxGraphicUnit.guPixel;
  FGraphics.PageScale := 1.0;

  FPixelHdcDpiModifier := TdxHdcDpiModifier.Create(FGraphics,
    TSize.Create(4096, 4096), Round(View.Control.DpiX));
...
procedure TdxRichEditViewPainter.DrawPageContent(APageViewInfo: TdxPageViewInfo;
  APainter: TdxPainter);
var
  AAdapter: TdxGraphicsDocumentLayoutExporterAdapter;
  APageBounds: TRect;
...
begin
  ...
  AExporter := View.CreateDocumentLayoutExporter(APainter, AAdapter,
    APageViewInfo, APageBounds);
  try
    AExporter.ShowWhitespace :=
      FControl.DocumentModel.FormattingMarkVisibilityOptions.ShowHiddenText;
    AExporter.MinReadableTextHeight := FMinReadableTextHeight;
    AExporter.SetBackColor(Control.BackgroundPainter.GetActualPageBackColor,
      APageBounds);
    ...
    try
      AArgs := TdxRichEditBeforePagePaintEventArgs.Create(APagePainter,
        APageViewInfo.Page.PageIndex);
      try
        ...
        begin
          ...
          AArgs.Painter.Page := Control.DocumentLayout.GetPage
            (APageViewInfo.Page.PageIndex);
          AArgs.Painter.Draw;
        end;
      finally
...
procedure TdxRichEditViewPainter.DrawPageComment(APageViewInfo: TdxPageViewInfo;
  APainter: TdxPainter);
begin
  NotImplemented;
end;
...
procedure TdxRichEditViewPainter.DrawCommentsBackground(APageViewInfo
  : TdxPageViewInfo; AGraphics: TdxGraphics);
begin
  NotImplemented;
end;
...
begin
  APainter := TdxPainter(Control.MeasurementAndDrawingStrategy.
    CreateDocumentPainter(Graphics));
  try
...
function TdxRichEditViewPainter.GetNewClientBounds(const AClipBounds: TRect;
  const AOldClipBounds: TdxRectF): TdxRectF;
var
  R: TRect;
begin
...
procedure TdxRichEditViewPainter.DrawBoxesInPixels(const ACustomMarkExporter
  : IdxCustomMarkExporter);
var
  APainter: TdxPainter;
...
begin
  APainter := TdxPainter(FControl.MeasurementAndDrawingStrategy.
    CreateDocumentPainter(Graphics));
  try
    AExporter := TdxWinFormsNotPrintableGraphicsBoxExporter.Create
      (DocumentModel, APainter, View, ACustomMarkExporter);
    try
      ACount := View.PageViewInfos.Count;
      ...
      begin
        ...
        try
          AOldClipBounds := ClipPageContentForDrawingInPixels(APageViewInfo,
            APainter);
...
procedure TdxRichEditViewPainter.IntersectClipBounds(APainter: TdxPainter;
  const AOldClipBounds: TdxRectF; X: Single; Y: Single; AWidth: Single;
  AHeight: Single);
var
...
begin
  APainter := TdxPainter(FControl.MeasurementAndDrawingStrategy.
    CreateDocumentPainter(AGraphics));
  try
...
procedure TdxRichEditViewPainter.DecorateTablesCore
  (AController: TdxTableViewInfoController; APainter: TdxPainter);
var
...
procedure TdxRichEditViewPainter.Draw(AGraphics: TdxGraphics;
  const ACustomMarkExporter: IdxCustomMarkExporter);
var
  AOldTransform: TdxGPMatrix;
begin
  ...
  ApplyGraphicsTransform(AGraphics,
    ACustomMarkExporter.CustomMarkVisualInfoCollection);
  EndDrawInPixels;
end;

function TdxRichEditViewPainter.GetCommentBounds(ACommentViewInfo
  : TdxCommentViewInfo): TRect;
begin
  NotImplemented;
end;

procedure TdxRichEditViewPainter.ApplyGraphicsTransform(AGraphics: TdxGraphics;
...
procedure TdxRichEditViewPainter.DrawAtPageCore(AGraphics: TdxGraphics;
  APage: TdxPageViewInfo; const ADraw: TdxDrawAtPageDelegate);
var
...
begin
  ...
  try
    ...
    AGraphics.TranslateWorldTransform(APageViewInfoClientBounds.X -
      View.GetPhysicalLeftInvisibleWidth, APageViewInfoClientBounds.Y);
    ...
    try
      ...
      AOriginModifier := TdxHdcOriginModifier.Create(AGraphics, AOrigin,
        AScaleFactor, TdxHdcOriginModifier.TMode.Combine);
      try
        ADraw(AGraphics);
      finally
...
procedure TdxRichEditViewPainter.DrawCaretCore(AGraphics: TdxGraphics;
  ACaret: TdxCaret);
begin
  ACaret.Draw(AGraphics);
end;

procedure TdxRichEditViewPainter.DrawCaretAtPage(AGraphics: TdxGraphics;
  ACaret: TdxCaret; APage: TdxPageViewInfo);
var
  ADraw: TdxDrawAtPageDelegate;
begin
  ADraw := procedure(C: TdxGraphics)
    begin
...
procedure TdxRichEditViewPainter.DrawReversibleHorizontalLineAtPage
  (AGraphics: TdxGraphics; Y: Integer; APage: TdxPageViewInfo);
var
  ADraw: TdxDrawAtPageDelegate;
begin
  ADraw := procedure(G: TdxGraphics)
    var
      ABounds: TRect;
    begin
...
procedure TdxRichEditViewPainter.DrawReversibleVerticalLineAtPage
  (AGraphics: TdxGraphics; X: Integer; APage: TdxPageViewInfo);
var
  ADraw: TdxDrawAtPageDelegate;
begin
  ADraw := procedure(G: TdxGraphics)
...
procedure TdxRichEditViewPainter.DrawReversibleHorizontalLine
...
procedure TdxRichEditViewPainter.DrawReversibleHorizontalLine(AHdc: THandle;
  const ABounds: TRect);
begin
  DrawReversible(AHdc, ABounds, DrawReversibleHorizontalLineCore);
end;

procedure TdxRichEditViewPainter.DrawReversibleVerticalLine
  (AGraphics: TdxGraphics; const ABounds: TRect);
var
  AHdc: THandle;
begin
...
procedure TdxRichEditViewPainter.DrawReversibleVerticalLine(AHdc: THandle;
  const ABounds: TRect);
begin
  DrawReversible(AHdc, ABounds, DrawReversibleVerticalLineCore);
end;

procedure TdxRichEditViewPainter.DrawReversibleFrameAtPage
  (AGraphics: TdxGraphics; const ABounds: TRect; APage: TdxPageViewInfo);
var
  ADraw: TdxDrawAtPageDelegate;
begin
  ADraw := procedure(G: TdxGraphics)
...
procedure TdxRichEditViewPainter.DrawReversibleFrame(AGraphics: TdxGraphics;
  const ABounds: TRect);
var
  AHdc: THandle;
begin
...
procedure TdxRichEditViewPainter.DrawReversibleFrame(AHdc: THandle;
  const ABounds: TRect);
begin
  DrawReversible(AHdc, ABounds, DrawReversibleFrameCore);
end;

procedure TdxRichEditViewPainter.DrawReversible(AHdc: THandle;
  const ABounds: TRect; const ADraw: TdxDrawReversibleDelegate);
var
  AOldRop2: Integer;
...
procedure TdxRichEditViewPainter.DrawReversibleFrameCore(AHdc: THandle;
  const ABounds: TRect);
begin
...
procedure TdxRichEditViewPainter.DrawReversibleHorizontalLineCore(AHdc: THandle;
  const ABounds: TRect);
begin
  MoveToEx(AHdc, ABounds.Left, ABounds.Top, nil);
  LineTo(AHdc, ABounds.Right, ABounds.Top);
...
procedure TdxRichEditViewPainter.DrawReversibleVerticalLineCore(AHdc: THandle;
  const ABounds: TRect);
begin
  MoveToEx(AHdc, ABounds.Left, ABounds.Top, nil);
  LineTo(AHdc, ABounds.Left, ABounds.Bottom);
...
procedure TdxRichEditViewPainter.DrawPageSelection(AGraphics: TdxGraphics;
...
procedure TdxRichEditViewPainter.DrawPageSelectionCore(const ASelection
...
procedure TdxRichEditViewPainter.DrawHoverCore(const AHoverLayoutItem
  : IdxHoverLayoutItem; APainter: TdxPainter);
begin
  NotImplemented;
end;
...
procedure TdxRichEditViewBackgroundPainter.Draw(AGraphics: TdxGraphics;
  const ABounds: TRect);
begin
...
begin
  ...
  if ADocumentProperties.DisplayBackgroundShape and
    not TdxAlphaColors.IsEmpty(APageBackColor) then
...
function TdxRichEditViewSkinBackgroundPainter.GetBackground
  (const ABounds: TRect): TBitmap;
var
  ASize: TSize;
begin
  if (Background = nil) or (Background.Width <> ABounds.Width) or
    (Background.Height <> ABounds.Height) then
  begin
    FreeAndNil(FBackground);
    ASize.Init(Max(8, ABounds.Width), Max(8, ABounds.Height));
    FBackground := TBitmap.Create;
    FBackground.SetSize(ASize.Width, ASize.Height);
    SkinInfo.PrintingPreviewBackground.Draw(FBackground.Canvas.Handle,
      TRect.Create(0, 0, ASize.cx, ASize.cy));
  end;
  Result := FBackground;
end;
...
begin
  ...
  FVerticalScrollController :=
    AControl.CreateRichEditViewVerticalScrollController(Self);
  FHorizontalScrollController :=
    AControl.CreateRichEditViewHorizontalScrollController(Self);
  ...
  FFormattingController.ResetSecondaryFormattingForPage.Add
    (OnResetSecondaryFormattingForPage);
...
begin
  ...
  begin
    ...
    DocumentModel.LayoutOptions.PrintLayoutView.Changed.Remove
...
begin
  ...
  ATo := TdxDocumentModelPosition.FromParagraphEnd(APieceTable,
    APieceTable.Paragraphs.Last.Index);
  Formatter.UpdateSecondaryPositions(AFrom, ATo);
  ApplyChanges(APieceTable);
  Formatter.NotifyDocumentChanged(AFrom, ATo,
...
begin
  ...
  begin
    ...
    AMinGapInLayoutUnit := Ceil(AUnitConverter.DocumentsToLayoutUnits(30) /
      ScaleFactor);
    PageViewInfoGenerator.HorizontalPageGap :=
      Max(AMinGapInLayoutUnit,
      Ceil(AUnitConverter.PixelsToLayoutUnits(Ceil(Control.SkinLeftMargin +
      Control.SkinRightMargin)) / ScaleFactor));
    PageViewInfoGenerator.VerticalPageGap :=
      Max(AMinGapInLayoutUnit,
      Ceil(AUnitConverter.PixelsToLayoutUnits(Ceil(Control.SkinTopMargin +
      Control.SkinBottomMargin)) / ScaleFactor));
  end;
  Formatter.BeginDocumentRendering
    (IsPrimaryFormattingCompleteForVisibleHeight2);
  ...
  begin
    ...
    PageViewInfoGenerator.FirstPageOffsetAnchor.PageIndex :=
      DocumentLayout.Pages.Count - 1;
    ALastRow := ALastPage.Areas.Last.Columns.Last.Rows.Last;
    FPageViewInfoGenerator.FirstPageOffsetAnchor.VerticalOffset :=
      ALastRow.Bounds.Top;
    GeneratePages;
  end;
...
begin
  ...
  if APageViewInfo <> nil then
    Result := FormattingController.PageController.Pages.IndexOf
      (APageViewInfo.Page) - 1;
end;
...
begin
  ...
  if APageViewInfo = nil then
    ...
    Result := 1 + FormattingController.PageController.Pages.IndexOf
      (APageViewInfo.Page);
end;

function TdxRichEditView.CalculateFloatingObjectHitTest(APage: TdxPage;
  const APoint: TPoint; const APredicate: TdxPredicate<TdxFloatingObjectBox>)
...
begin
  ...
  try
    Result := CalculateFloatingObjectHitTestCore(APoint, AFloatingObjects,
      APredicate);
    if Result = nil then
      Result := CalculateFloatingObjectHitTestCore(APoint,
        APage.InnerBackgroundFloatingObjects, APredicate);
  finally
    AFloatingObjects.Free;
  end;
end;

function TdxRichEditView.CalculateFloatingObjectHitTestCore
  (const APoint: TPoint; AList: TdxFloatingObjectBoxList;
...
begin
  ...
  ARequest.Accuracy := DefaultHitTestPageAccuracy or NearestPageArea or
    NearestColumn or NearestTableRow or NearestTableCell or NearestRow or
    NearestBox or NearestCharacter;
...
function TdxRichEditView.CalculateNearestCharacterHitTest(const APoint: TPoint;
  APieceTable: TdxPieceTable): TdxRichEditHitTestResult;
var
  ARequest: TdxRichEditHitTestRequest;
begin
  ...
  Result := HitTestCore(ARequest, (DefaultHitTestPageAccuracy and
    ExactPage) <> 0);
...
begin
  ...
  ARequest.Accuracy := NearestPage or NearestPageArea or NearestColumn or
    NearestTableRow or NearestTableCell or NearestRow or NearestBox or
    NearestCharacter;
...
function TdxRichEditView.CalculateOptimalHorizontalScrollbarPosition
...
begin
  ...
  if ALogicalVisibleWidth > APageViewInfo.Page.Bounds.Width +
    PageViewInfoGenerator.HorizontalPageGap then
    Result := 0
  else if ALogicalVisibleWidth > APageViewInfo.Page.Bounds.Width then
    Result := Trunc((APageViewInfo.Page.Bounds.Width +
      PageViewInfoGenerator.HorizontalPageGap - ALogicalVisibleWidth) / 2 -
      FixedLeftTextBorderOffset)
  else if ALogicalVisibleWidth > APageViewInfo.Page.Bounds.Right -
    APageClientBounds.Left + AHalfOfHorizontalPageGap then
    Result := APageViewInfo.Page.Bounds.Width +
      PageViewInfoGenerator.HorizontalPageGap - ALogicalVisibleWidth -
      FixedLeftTextBorderOffset
  else
    Result := AHalfOfHorizontalPageGap + APageClientBounds.Left -
      FixedLeftTextBorderOffset;
end;

function TdxRichEditView.CalculatePageContentClipBounds
  (APage: TdxPageViewInfo): TRect;
var
  AScaleFactor: Single;
...
function TdxRichEditView.CalculateResetFromPosition(APieceTable: TdxPieceTable;
  AChanges: TdxDocumentModelDeferredChanges;
  AResetType: TdxDocumentLayoutResetType): TdxDocumentModelPosition;
var
...
begin
  ...
  begin
    AModelPosition := TdxDocumentModelPosition.FromParagraphStart(APieceTable,
      AChangeStart.ParagraphIndex);
    AModelPosition := EnsurePositionVisibleWhenHiddenTextNotShown(DocumentModel,
      AModelPosition);
    AModelPosition := EnsurePositionNotBeforeSectionBreakAfterParagraphBreak
      (AModelPosition);
...
function TdxRichEditView.CalculateVisiblePageBounds(const APageBounds: TRect;
  APageViewInfo: TdxPageViewInfo): TRect;
var
  APageViewInfoTotalHeight: Integer;
  AViewPort, AExtendedViewPortBounds, APageViewInfoVisibleBounds,
...
begin
  ...
  begin
    ...
    AExtendedViewPortBounds := TRect.CreateSize(0, AViewPort.Top, MaxInt,
      AViewPort.Height);
    ...
    AVisibleBounds := cxRectSetTop(AVisibleBounds,
      Math.Floor(APageBounds.Height / APageViewInfoTotalHeight *
      (APageViewInfoVisibleBounds.Top - APageViewInfo.Bounds.Top)));
    AVisibleBounds.Height :=
      Math.Ceil(APageBounds.Height / APageViewInfoTotalHeight *
      APageViewInfoVisibleBounds.Height);
...
procedure TdxRichEditView.LayoutOptionsChanged(ASender: TObject;
  Args: TdxRichEditNotificationOptionsChangedArgs);
begin
  if TdxRichEditOptionsAction.MatchHorizontalTableIndentsToTextEdge in Args.Actions
  then
  begin
    DocumentModel.BeginUpdate;
    try
      DocumentModel.OnMatchHorizontalTableIndentsToTextEdgeOptionsChanged;
      FFormattingController.RowsController.MatchHorizontalTableIndentsToTextEdge
        := TdxViewLayoutOptionsBase(ASender)
...
function TdxRichEditView.ChoosePageViewInfoForOptimalHorizontalScrollbarPosition
  : TdxPageViewInfo;
var
...
begin
  ...
  begin
    ...
    AOffset := APageViewInfo.Page.ClientBounds.Left * ScaleFactor +
      APageViewInfo.Bounds.Left;
    if AOffset > X then
    begin
      X := AOffset;
...
function TdxRichEditView.CreateHitTestCalculator(const ARequest
  : TdxRichEditHitTestRequest; AResult: TdxRichEditHitTestResult)
  : TdxBoxHitTestCalculator;
begin
  Result := FormattingController.PageController.CreateHitTestCalculator
    (ARequest, AResult);
end;

function TdxRichEditView.CreateLogicalPoint(const AClientBounds: TRect;
...
function TdxRichEditView.CreateLogicalRectangle(APageViewInfo: TdxPageViewInfo;
...
function TdxRichEditView.CreatePhysicalPoint(APageViewInfo: TdxPageViewInfo;
  const APoint: TPoint): TPoint;
var
  AMatrix: TdxTransformMatrix;
begin
...
function TdxRichEditView.CreatePhysicalRectangle(APageViewInfo: TdxPageViewInfo;
  const ABounds: TRect): TRect;
begin
  Result := CreatePhysicalRectangle(APageViewInfo.ClientBounds, ABounds);
end;

function TdxRichEditView.CreatePhysicalRectangle
  (const APageViewInfoClientBounds, ABounds: TRect): TRect;
var
  AMatrix: TdxTransformMatrix;
begin
...
function TdxRichEditView.CreatePhysicalRectangleFast(APageViewInfo
  : TdxPageViewInfo; const ABounds: TRect): TRect;
var
  AClientBounds: TRect;
  X, Y, W, H: Single;
begin
  AClientBounds := APageViewInfo.ClientBounds;
  X := ScaleFactor * ABounds.Left + AClientBounds.Left -
    GetPhysicalLeftInvisibleWidth;
...
function TdxRichEditView.CreateTransformMatrix(const AClientBounds: TRect)
  : TdxTransformMatrix;
begin
  Result := TdxTransformMatrix.Create;
  Result.Translate(AClientBounds.Left - GetPhysicalLeftInvisibleWidth,
    AClientBounds.Top);
...
procedure TdxRichEditView.EnsureCaretVisible(ACheckCaretVisibility
  : Boolean = True);
var
...
procedure TdxRichEditView.EnsureFormattingCompleteForLogPosition
  (ALogPosition: TdxDocumentLogPosition);
begin
...
procedure TdxRichEditView.EnsureFormattingCompleteForPreferredPage
  (APreferredPageIndex: Integer);
begin
  CheckExecutedAtUIThread;
...
class function TdxRichEditView.EnsurePositionVisibleWhenHiddenTextNotShown
  (ADocumentModel: TdxDocumentModel;
  const AModelPosition: TdxDocumentModelPosition): TdxDocumentModelPosition;
var
  APieceTable: TdxPieceTable;
...
begin
  ...
  begin
    if AVisibleTextFilter.IsRunVisible
      (APieceTable.Paragraphs[AParagraphIndex - 1].LastRunIndex) then
      Break;
    Dec(AParagraphIndex);
  end;
  if AParagraphIndex = AModelPosition.ParagraphIndex then
    ...
    Result := TdxDocumentModelPosition.FromParagraphStart(APieceTable,
      AParagraphIndex);
end;

function TdxRichEditView.EnsureTopLevelParagraph
...
function TdxRichEditView.ExpandClipBoundsToPaddings(const AClipBounds
  : TRect): TRect;
var
  APadding: TRect;
...
begin
  ...
  ALeftPadding :=
    MulDiv(3, DocumentModel.LayoutUnitConverter.PixelsToLayoutUnits
    (APadding.Left), 4);
  ATopPadding := MulDiv(3, DocumentModel.LayoutUnitConverter.PixelsToLayoutUnits
    (APadding.Top), 4);
  Result := AClipBounds;
...
begin
  ...
  begin
    ...
    if APage.PrimaryFormattingComplete then
      PageViewInfoGenerationComplete := PageViewInfoGenerator.ProcessPage(APage,
        I) = TdxProcessPageResult.VisiblePagesGenerationComplete;
  end;
  ...
  begin
    ...
    begin
      ...
      AFrom := AFirstProcessedPage.Page.GetFirstPosition
        (DocumentModel.MainPieceTable);
      ATo := ALastProcessedPage.Page.GetLastPosition
        (DocumentModel.MainPieceTable);
      Formatter.UpdateSecondaryPositions(AFrom, ATo);
      Formatter.NotifyDocumentChanged(AFrom, ATo,
        TdxDocumentLayoutResetType.SecondaryLayout);

      if (FOldFirstPageIndex <> PageViewInfos.First.Index) or
        (FOldLastPageIndex <> PageViewInfos.Last.Index) then
...
function TdxRichEditView.GetHyperlinkField(AHitTestResult
  : TdxRichEditHitTestResult): TdxField;
begin
  if (AHitTestResult.DetailsLevel < TdxDocumentLayoutDetailsLevel.Box) or
    ((AHitTestResult.Accuracy and ExactBox) = 0) then
    Result := nil
  else
    Result := AHitTestResult.PieceTable.GetHyperlinkField
      (AHitTestResult.Box.StartPos.RunIndex);
end;

function TdxRichEditView.GetPageViewInfoFromPoint(const APoint: TPoint;
  AStrictSearch: Boolean): TdxPageViewInfo;
var
...
function TdxRichEditView.GetPageViewInfoRowFromPoint(const APoint: TPoint;
  AStrictSearch: Boolean): TdxPageViewInfoRow;
begin
  Result := PageViewInfoGenerator.GetPageRowAtPoint(APoint, AStrictSearch);
end;

function TdxRichEditView.GetPhysicalBounds(APageViewInfo: TdxPageViewInfo;
  const ABounds: TRect): TRect;
...
var
  ...
  AUnitConverter: TdxDocumentLayoutUnitConverter;
  X, Y, AScreenWidth, AScreenHeight, AHorizontalRulerHeight,
    AVerticalRulerWidth: Integer;
begin
  ...
  AScreenPosition := AUnitConverter.LayoutUnitsToPixels(APosition,
    DocumentModel.DpiX, DocumentModel.DpiY);
  AScreenWidth := AUnitConverter.LayoutUnitsToPixels(ABounds.Width,
    DocumentModel.DpiX);
  AScreenHeight := AUnitConverter.LayoutUnitsToPixels(ABounds.Height,
    DocumentModel.DpiY);
  ...
  Result.InitSize(AScreenPosition.X + ActualPadding.Left + AVerticalRulerWidth,
...
begin
  ...
  if ASelection.Length > 0 then
    ALastSelectedSymbol :=
      Max(AEnd - 1, ASelection.PieceTable.DocumentStartLogPosition)
  else
  ...
  if (ASelection.Interval.&End.RunOffset = 0) and (AEndRunIndex > 0) then
    Dec(AEndRunIndex);
  AEndVisible := ASelection.PieceTable.VisibleTextFilter.IsRunVisible
    (AEndRunIndex);
  if AEndVisible then
    Result := AEnd
  else
    Result := ASelection.PieceTable.VisibleTextFilter.GetNextVisibleLogPosition
      (ALastSelectedSymbol, False);
end;
...
begin
  ...
  AStartVisible := ASelection.PieceTable.VisibleTextFilter.IsRunVisible
    (ASelection.Interval.Start.RunIndex);
...
procedure TdxRichEditView.HitTest(APage: TdxPage;
  const ARequest: TdxRichEditHitTestRequest; AResult: TdxRichEditHitTestResult);
var
  ACalculator, ARowCalculator, ABoxCalculator, ADetailRowCalculator
    : TdxBoxHitTestCalculator;
  ...
  try
    ...
    begin
      ...
      try
        ...
        ABoxResult := TdxRichEditHitTestResult.Create(DocumentLayout,
          ARequest.PieceTable);
        try
          ...
          try
            ...
            try
              ...
              ADetailRowCalculator := CreateHitTestCalculator(ARequest,
                AResult);
              try
                ADetailRowCalculator.FastHitTestCharacter(ADetailRow.Characters,
                  AStrictHitTest);
              finally
...
function TdxRichEditView.HitTestByPhysicalPoint(const APt: TPoint;
  ADetailsLevel: TdxDocumentLayoutDetailsLevel): TRect;
var
  ARow: TdxRow;
  APageViewInfo: TdxPageViewInfo;
...
begin
  ...
  APageViewInfo := GetPageViewInfoFromPoint(ARequest.PhysicalPoint,
    AStrictHitIntoPageBounds);
  if APageViewInfo = nil then
    Exit;

  P := ARequest.PhysicalPoint;
  P.X := P.X + GetPhysicalLeftInvisibleWidth;
  if AStrictHitIntoPageBounds and not PerformStrictPageViewInfoHitTest
    (APageViewInfo, P) then
    Exit;
  ...
  begin
    Result.LogicalPoint := CreateLogicalPoint(APageViewInfo.ClientBounds,
      ARequest.PhysicalPoint);
    Result.FloatingObjectBox := CalculateFloatingObjectHitTest(Result.Page,
      Result.LogicalPoint, IsActivePieceTableFloatingObjectBox);
    Result.FloatingObjectBoxPage := Result.Page;
    Exit;
  end;

  ARequest.LogicalPoint := CreateLogicalPoint(APageViewInfo.ClientBounds,
    ARequest.PhysicalPoint);
  ...
  begin
    ...
    begin
      Result.FloatingObjectBox := CalculateFloatingObjectHitTest(Result.Page,
        ARequest.LogicalPoint, IsActivePieceTableFloatingObjectBox);
      HitTest(APageViewInfo.Page, ARequest, Result);
    end;
  end;
end;

function TdxRichEditView.IsActiveFloatingObjectTextBox
  (const ABox: TdxFloatingObjectBox): Boolean;
var
...
function TdxRichEditView.IsActivePieceTableFloatingObjectBox
  (const ABox: TdxFloatingObjectBox): Boolean;
begin
...
function TdxRichEditView.IsFloatingObjectHit(ABox: TdxFloatingObjectBox;
  APoint: TPoint): Boolean;
begin
  Result := ABox.Bounds.Contains(ABox.TransformPointBackward(APoint));
end;

procedure TdxRichEditView.DocumentModelActivePieceTableTextBoxHitTest
  (var ARequest: TdxRichEditHitTestRequest;
  var AResult: TdxRichEditHitTestResult; APageViewInfo: TdxPageViewInfo);
var
  ABox: TdxFloatingObjectBox;
...
begin
  ABox := CalculateFloatingObjectHitTest(AResult.Page, ARequest.LogicalPoint,
    IsActiveFloatingObjectTextBox);
  if ABox <> nil then
  begin
  ...
  begin
    ...
    if (AResult.PageArea <> nil) and
      (AResult.PageArea.PieceTable <> AResult.PieceTable) then
    begin
      ACopy := TdxRichEditHitTestResult.Create(DocumentLayout,
        TdxPieceTable(AResult.PageArea.PieceTable));
...
function TdxRichEditView.LookupPageViewInfoByPage(APage: TdxPage)
  : TdxPageViewInfo;
var
...
begin
  PageViewInfoGenerator.LeftInvisibleWidth :=
    HorizontalScrollController.GetLeftInvisibleWidth;
...
procedure TdxRichEditView.OnPageFormattingComplete(ASender: TObject;
  E: TdxPageFormattingCompleteEventArgs);
var
  ALastPage: TdxPage;
...
begin
  ...
  PageViewInfoGenerationComplete := PageViewInfoGenerator.ProcessPage(ALastPage,
    FormattingController.PageController.Pages.Count - 1)
    = TdxProcessPageResult.VisiblePagesGenerationComplete;
  if (PageViewInfoGenerationComplete <> APrevPageViewInfoGenerationComplete) or
    E.DocumentFormattingComplete then
  begin
    PageViewInfoGenerator.CalculateWidthParameters;
  ...
  begin
    Formatter.UpdateSecondaryPositions(Formatter.SecondaryLayoutStart,
      ALastPage.GetLastPosition(DocumentModel.MainPieceTable));
...
procedure TdxRichEditView.OnPageFormattingStarted(ASender: TObject;
  E: TdxPageFormattingCompleteEventArgs);
var
  ALastPage: TdxPage;
begin
  ...
  if not PageViewInfoGenerationIsAboutToComplete then
    PageViewInfoGenerationIsAboutToComplete :=
      (PageViewInfoGenerator.PreProcessPage(ALastPage,
      FormattingController.PageController.Pages.Count - 1)
      = TdxProcessPageResult.VisiblePagesGenerationComplete);
end;
...
procedure TdxRichEditView.OnResize(const ABounds: TRect;
  AEnsureCaretVisibleOnResize: Boolean);
begin
  OnResize(ABounds, AEnsureCaretVisibleOnResize, True);
end;

procedure TdxRichEditView.OnResize(const ABounds: TRect;
  AEnsureCaretVisibleOnResize, AEnsureOptimalHorizontalScrollbarPosition
  : Boolean);
begin
...
procedure TdxRichEditView.ProcessSelectionChanges(const AChangeActions
  : TdxDocumentModelChangeActions);
begin
  CaretPosition.Invalidate;
  if TdxDocumentModelChangeAction.ResetCaretInputPositionFormatting in AChangeActions
  then
    CaretPosition.InvalidateInputPosition;

  if TdxDocumentModelChangeAction.ValidateSelectionInterval in AChangeActions
  then
...
begin
  ...
  try
    ResetPages(TdxPageGenerationStrategyType.RunningHeight);
    PageViewInfoGenerator.TopInvisibleHeight :=
      VerticalScrollController.GetTopInvisibleHeight;
    GeneratePages;
  finally
    ResumeFormatting;
  end;
...
function TdxRichEditView.PerformStrictPageViewInfoHitTest
  (APageViewInfo: TdxPageViewInfo; const APt: TPoint): Boolean;
begin
...
begin
  ...
  try
    ...
    if DocumentModel.IsUpdateLocked then
      DocumentModel.MainPieceTable.ApplyChangesCore
        ([TdxDocumentModelChangeAction.Redraw], dxRunIndexDontCare,
        dxRunIndexDontCare);
  finally
    DocumentModel.EndUpdate;
  end;
...
begin
  ...
  begin
    ...
    if APageViewInfo = nil then
      ...
      PageViewInfoGenerator.LeftInvisibleWidth :=
        Max(0, CalculateOptimalHorizontalScrollbarPosition(APageViewInfo));
  end;
  HorizontalScrollController.UpdateScrollBar;
end;

procedure TdxRichEditView.SetPageViewInfoGenerationComplete
  (const Value: Boolean);
begin
...
class function TdxRichEditView.SetNewPosition
  (const APos: TdxDocumentModelPosition; AParagraph: TdxParagraph)
  : TdxDocumentModelPosition;
begin
  Result := APos;
...
var
  ASelection: TdxSelection;
  AStart, AEnd, AVisibleStart, AVisibleEnd, ANewPosition
    : TdxDocumentLogPosition;
...
begin
  ...
  begin
    ...
    ANonEmptyVisibleSelection := (AStart < AEnd) and
      (AVisibleStart < AVisibleEnd) or (AStart > AEnd) and
      (AVisibleStart > AVisibleEnd);
...
begin
  ...
  try
    if not APosition.IsValid(TdxDocumentLayoutDetailsLevel.Page) then
      APosition.Update(FormattingController.PageController.Pages,
        TdxDocumentLayoutDetailsLevel.Page);
...
procedure TdxPageBasedRichEditView.OnPageCountChanged(ASender: TObject;
  E: TdxEventArgs);
begin
...
constructor TdxRichEditCustomViewRepository.Create(const ARichEditControl
  : IdxRichEditControl);
begin
...
function TdxRichEditCustomViewRepository.GetViewByType
...
function TdxRichEditCustomViewRepository.GetView(Index: Integer)
  : TdxRichEditView;
begin
...
procedure TdxOfficeScrollControllerBase.EmulateScroll
...
begin
  Result := (ScrollBarAdapter.Maximum - ScrollBarAdapter.Minimum >
    ScrollBarAdapter.LargeChange) and
    (ScrollBarAdapter.Value >= ScrollBarAdapter.Minimum) and
    (ScrollBarAdapter.Value <= ScrollBarAdapter.Maximum -
    ScrollBarAdapter.LargeChange + 1);
end;
...
function TdxRichEditViewHorizontalScrollController.CreateScrollBarAdapter
  : TdxScrollBarAdapter;
begin
  Result := TdxHorizontalScrollBarAdapter.Create(ScrollBar,
    View.Control.CreatePlatformSpecificScrollBarAdapter);
end;
...
function TdxRichEditViewHorizontalScrollController.
  GetPhysicalLeftInvisibleWidth: Integer;
begin
  if ScrollBar = nil then
    Exit(0);
  Result := Round(View.PageViewInfoGenerator.LeftInvisibleWidth *
    View.ScaleFactor);
end;

function TdxRichEditViewHorizontalScrollController.GetScrollBar
  : IdxOfficeScrollbar;
begin
  Result := nil;
...
procedure TdxRichEditViewHorizontalScrollController.OnScrollCore
  (E: TdxScrollEventArgs);
begin
end;

procedure TdxRichEditViewHorizontalScrollController.
  ScrollByLeftInvisibleWidthDelta(ALeftInvisibleWidthDelta: Int64);
begin
  ScrollToAbsolutePosition(ScrollBarAdapter.Value + ALeftInvisibleWidthDelta);
end;

procedure TdxRichEditViewHorizontalScrollController.ScrollToAbsolutePosition
  (AValue: Int64);
begin
...
begin
  ...
  try
    ...
    ScrollBarAdapter.SmallChange :=
      Round(View.DocumentLayout.UnitConverter.DocumentsToLayoutUnits(210));
...
procedure TdxRichEditViewVerticalScrollController.ApplyNewScrollValue
  (AValue: Integer);
begin
  dxAbstractError;
end;

procedure TdxRichEditViewVerticalScrollController.
  ApplyNewScrollValueToScrollEventArgs(E: TdxScrollEventArgs; AValue: Integer);
begin
  dxAbstractError;
end;
...
function TdxRichEditViewVerticalScrollController.CreateScrollBarAdapter
  : TdxScrollBarAdapter;
begin
  Result := TdxVerticalScrollBarAdapter.Create(ScrollBar,
    View.Control.CreatePlatformSpecificScrollBarAdapter);
end;

function TdxRichEditViewVerticalScrollController.IsScrollTypeValid
...
function TdxRichEditViewVerticalScrollController.GetScrollBar
  : IdxOfficeScrollbar;
begin
  Result := nil;
  if (View.Control = nil) or (View.Control.InnerControl = nil) then
...
procedure TdxRichEditViewVerticalScrollController.OnScroll(ASender: TObject;
  E: TdxScrollEventArgs);
var
...
procedure TdxRichEditViewVerticalScrollController.
  ScrollByTopInvisibleHeightDelta(ATopInvisibleHeightDelta: Int64);
begin
...
begin
  Result := ScrollLineUpDown
    (View.DocumentLayout.UnitConverter.DocumentsToLayoutUnits(50));
end;
...
function TdxRichEditViewVerticalScrollController.ScrollLineUpDown
  (APhysicalOffset: Integer): Integer;
var
...
begin
  Result := ScrollBarAdapter.SetRawScrollBarValue
    (ScrollBarAdapter.GetPageDownRawScrollBarValue);
end;

function TdxRichEditViewVerticalScrollController.ScrollPageUp: Boolean;
begin
  Result := ScrollBarAdapter.SetRawScrollBarValue
    (ScrollBarAdapter.GetPageUpRawScrollBarValue);
end;

procedure TdxRichEditViewVerticalScrollController.ScrollToAbsolutePosition
  (AValue: Int64);
begin
  ScrollBarAdapter.BeginUpdate;
  try
...
begin
  ...
  try
    ...
    ScrollBarAdapter.Value := Max(0, Min(AGenerator.TopInvisibleHeight,
      ScrollBarAdapter.Maximum - ScrollBarAdapter.LargeChange + 1));
    ScrollBarAdapter.Enabled := IsScrollPossible;
    if not ScrollBarAdapter.Enabled and
      (ScrollBarAdapter.Value <> AGenerator.TopInvisibleHeight) then
      View.OnVerticalScroll;
  finally
    ScrollBarAdapter.EndUpdate;
  end;
...
constructor TdxScrollBarAdapter.Create(const AScrollBar: IdxOfficeScrollbar;
  const AAdapter: IdxPlatformSpecificScrollBarAdapter);
begin
  inherited Create;
  FFactor := 1.0;
...
function TdxScrollBarAdapter.CreateEmulatedScrollEventArgs
  (AEventType: TdxScrollEventType): TdxScrollEventArgs;
begin
  EnsureSynchronized;
  case AEventType of
    ...
    TdxScrollEventType.SmallIncrement:
      Result := TdxScrollEventArgs.Create(AEventType,
...
begin
  ...
  begin
    ...
    AActualRelativePos := ScrollBar.Value /
      Max(1, ScrollBar.Maximum - ScrollBar.LargeChange + 1 - ScrollBar.Minimum);
...
function TdxMouseCursorCalculator.Calculate(AHitTestResult
  : TdxRichEditHitTestResult; const APhysicalPoint: TPoint): TCursor;
begin
...
function TdxMouseCursorCalculator.CalculateCore
  (AResult: TdxRichEditHitTestResult): TCursor;
var
...
begin
  ...
  if Result <> TdxRichEditCursors.Default then
    Exit;
  ASelectionManager := TdxEnhancedSelectionManager.Create
    (View.DocumentModel.ActivePieceTable);
  try
    ATableRow := ASelectionManager.CalculateTableRowToResize(AResult);
    if ASelectionManager.ShouldResizeTableRow(View.Control, AResult, ATableRow)
    then
      Exit(TdxRichEditCursors.ResizeTableRow);

    AVirtualColumn :=
      ASelectionManager.CalculateTableCellsToResizeHorizontally(AResult);
    try
      if ASelectionManager.ShouldResizeTableCellsHorizontally(View.Control,
        AResult, AVirtualColumn) then
...
function TdxMouseCursorCalculator.CalculateHotZoneCursor
  (AResult: TdxRichEditHitTestResult): TCursor;
var
  AHotZone: TdxHotZone;
  ASelectionManager: TdxEnhancedSelectionManager;
begin
  ...
  begin
    ...
    try
      ...
      if ASelectionManager.ShouldSelectFloatingObject(AResult) and IsEditable
      then
...
function TdxMouseCursorCalculator.CalculateHotZoneCursorCore
  (AHotZone: TdxHotZone): TCursor;
begin
...
function TdxMouseCursorCalculator.CalculateCharacterMouseCursor
  (AResult: TdxRichEditHitTestResult): TCursor;
...
function TdxCaretDocumentLayoutPosition.CreateEmptyClone
  : TdxDocumentLayoutPosition;
begin
  Result := TdxCaretDocumentLayoutPosition.Create(FView);
end;
...
procedure TdxCaretDocumentLayoutPosition.EnsurePageSecondaryFormattingComplete
  (APage: TdxPage);
begin
...
function TdxScrollByPhysicalHeightCalculator.CalculateRowsTotalVisibleHeight
  (AGenerator: TdxPageViewInfoGeneratorBase; ALastRowIndex: Integer;
  ABottomY: Integer): Int64;
var
...
begin
  ...
  Inc(Result, ALayoutManager.CalculatePagesTotalVisibleLogicalHeight
    (ARows[ALastRowIndex], AViewPortBounds));
end;

function TdxScrollByPhysicalHeightCalculator.CalculateScrollDelta
  (APhysicalVerticalOffset: Integer): Int64;
var
...
begin
  ...
  if APhysicalVerticalOffset <= 0 then
    Exit(0);
  if not CalculateScrollDeltaForVisibleRows(APhysicalVerticalOffset, Result)
  then
  begin
    AInvisiblePhysicalVerticalOffset := CalculateInvisiblePhysicalVerticalOffset
      (APhysicalVerticalOffset);
    Result := CalculateScrollDeltaForInvisibleRows
      (AInvisiblePhysicalVerticalOffset);
  end;
end;

function TdxScrollByPhysicalHeightCalculator.
  CalculateScrollDeltaForInvisibleRows(APhysicalVerticalOffset: Integer): Int64;
var
...
begin
  ...
  if AFirstInvisiblePageIndex < 0 then
    Exit(0);
  AGenerator := TdxInvisiblePageRowsGenerator.Create
    (View.FormattingController.PageController.Pages,
    View.PageViewInfoGenerator);
  try
    AGenerator.FirstPageIndex := AFirstInvisiblePageIndex;
    AGenerator.FirstInvalidPageIndex := CalculateFirstInvalidPageIndex;
    AIndex := LookupIntersectingRowIndexInInvisibleRows(AGenerator,
      APhysicalVerticalOffset);
    if AIndex < 0 then
      Exit(GetDefaultScrollDeltaForInvisibleRows);
    Result := CalculateRowsTotalVisibleHeight(AGenerator.Generator, AIndex,
      APhysicalVerticalOffset);
...
function TdxScrollByPhysicalHeightCalculator.
  LookupIntersectingRowIndexInInvisibleRows(AGenerator
  : TdxInvisiblePageRowsGenerator; Y: Integer): Integer;
var
  ARow: TdxPageViewInfoRow;
  ARowIndex: Integer;
begin
...
function TdxScrollUpByPhysicalHeightCalculator.
  CalculateScrollDeltaForVisibleRows(APhysicalVerticalOffset: Integer;
  out ADelta: Int64): Boolean;
var
  AFirstRow: TdxPageViewInfoRow;
  AViewPortBounds: TRect;
  APageIndex: Integer;
begin
  ...
  if AFirstRow.IntersectsWithHorizontalLine(-APhysicalVerticalOffset) or
    (AFirstRow.Bounds.Top = -APhysicalVerticalOffset) then
  begin
    AViewPortBounds.InitSize(0, -APhysicalVerticalOffset, 0,
      APhysicalVerticalOffset);
    ADelta := View.PageViewInfoGenerator.CalculatePagesTotalVisibleLogicalHeight
      (AFirstRow, AViewPortBounds);
    Exit(True);
  end
  else
  begin
    APageIndex := View.FormattingController.PageController.Pages.IndexOf
      (AFirstRow.First.Page);
    Result := APageIndex = 0;
    if Result then
      ADelta := View.PageViewInfoGenerator.CalculatePagesTotalLogicalHeightAbove
        (AFirstRow, 0);
  end;
end;

function TdxScrollUpByPhysicalHeightCalculator.
  CalculateFirstInvisiblePageIndex: Integer;
begin
  Result := View.CalculateFirstInvisiblePageIndexBackward;
end;

function TdxScrollUpByPhysicalHeightCalculator.
  CalculateFirstInvalidPageIndex: Integer;
begin
  Result := -1;
end;

function TdxScrollUpByPhysicalHeightCalculator.
  CalculateInvisiblePhysicalVerticalOffset(APhysicalVerticalOffset
  : Integer): Integer;
var
  AFirstRow: TdxPageViewInfoRow;
begin
...
function TdxScrollUpByPhysicalHeightCalculator.
  CalculateVisibleRowsScrollDelta: Int64;
var
  AViewPortBounds: TRect;
  ALayoutManager: TdxPageGeneratorLayoutManager;
  AFirstRow: TdxPageViewInfoRow;
begin
  ...
  Result := ALayoutManager.CalculatePagesTotalVisibleLogicalHeight(AFirstRow,
    AViewPortBounds);
end;

function TdxScrollUpByPhysicalHeightCalculator.
  GetDefaultScrollDeltaForInvisibleRows: Int64;
begin
...
function TdxScrollDownByPhysicalHeightCalculator.
  CalculateScrollDeltaForVisibleRows(APhysicalVerticalOffset: Integer;
  out ADelta: Int64): Boolean;
var
...
begin
  ...
  if Result then
    ADelta := CalculateRowsTotalVisibleHeight
      (View.PageViewInfoGenerator.ActiveGenerator, AIntersectingRowIndex,
      APhysicalVerticalOffset);
end;
...
begin
  ...
  Result := ALayoutManager.CalculatePagesTotalLogicalHeightBelow(ARows[0],
    ALayoutManager.ViewPortBounds.Top);
...
begin
  try
    ...
    if ALastRow = nil then
      ...
      Result := CalculateRowsTotalVisibleHeight
        (View.PageViewInfoGenerator.ActiveGenerator, ARows.Count - 1,
        ALastRow.Bounds.Bottom);
  except
    Result := 0;
  end;
end;

function TdxScrollDownByPhysicalHeightCalculator.LookupIntersectingRowIndex
  (Y: Integer): Integer;
var
  ARows: TdxPageViewInfoRowCollection;
  I: Integer;
...
constructor TdxNonPrintViewPageControllerBase.Create(ADocumentLayout
  : TdxDocumentLayout);
begin
  inherited Create(ADocumentLayout, nil, nil);
end;

function TdxNonPrintViewPageControllerBase.GetColumnBottom
  (AColumn: TdxColumn): Integer;
var
  I: Integer;
  ALastRow: TdxRow;
  AColumnBounds: TRect;
...
begin
  ...
  AFloatingObjects := FloatingObjectsLayout.GetAllObjectsInRectangle
    (AColumnBounds);
  try
    for I := 0 to AFloatingObjects.Count - 1 do
  ...
  begin
    AParagraphFrames := ParagraphFramesLayout.GetObjectsInRectangle
      (AColumnBounds);
    try
...
constructor TdxBackgroundFormatter.TWorkThread.Create
  (AFormatter: TdxBackgroundFormatter);
begin
...
function TdxBackgroundFormatter.TWorkThread.GetWaitCommandHandles
  : TWaitCommandHandles;
var
  I: TCommand;
begin
...
function TdxBackgroundFormatter.TWorkThread.GetWaitContinueLayoutHandles
  : TWaitContinueLayoutHandles;
begin
...
function TdxBackgroundFormatter.TWorkThread.
  WaitForContinueLayoutAllowed: Boolean;
var
...
constructor TdxBackgroundFormatter.Create(AController
  : TdxDocumentFormattingController; const ACommentPadding: TdxCommentPadding);
begin
  ...
  FSecondaryFormatter := CreateParagraphFinalFormatter
    (AController.DocumentLayout);
...
function TdxBackgroundFormatter.CreateParagraphFinalFormatter(ADocumentLayout
  : TdxDocumentLayout): TdxParagraphFinalFormatter;
begin
...
begin
  ...
  FCommands[TCommand.PerformPrimaryLayout] := TSimpleEvent.Create(nil, True,
    False, '');
  FCommands[TCommand.PerformSecondaryLayout] := TSimpleEvent.Create(nil, True,
    False, '');
...
function TdxBackgroundFormatter.IsPrimaryLayoutComplete
  (const ACurrentFormatterPosition: TdxDocumentModelPosition): Boolean;
begin
...
function TdxBackgroundFormatter.IsTableFormattingComplete(const ANextPosition
  : TdxDocumentModelPosition): Boolean;
begin
  Result := not DocumentFormatter.ParagraphFormatter.RowsController.
    TablesController.IsInsideTable;
end;

procedure TdxBackgroundFormatter.NotifyDocumentChanged(const AFrom,
  ATo: TdxDocumentModelPosition;
  ADocumentLayoutResetType: TdxDocumentLayoutResetType);
begin
...
function TdxBackgroundFormatter.DoPerformPrimaryLayout
...
begin
  ...
  begin
    ...
    FCurrentPosition.RunIndex := FDocumentFormatter.ParagraphFormatter.
      Iterator.RunIndex;
    AOffset := 0;
    if Result.FormattingProcess = TdxFormattingProcess.ContinueFromParagraph
    then
...
begin
  ...
  begin
    ...
    UpdateSecondaryPositions(SecondaryLayoutStart,
      FController.PageController.Pages.Last.GetLastPosition
      (FDocumentFormatter.DocumentModel.MainPieceTable));
  end;
end;
...
begin
  ...
  else if FPerformPrimaryLayoutUntil(FCurrentPosition) and
    IsTableFormattingComplete(FCurrentPosition) then
    PerformSecondaryLayoutCore
  else
    PerformPrimaryLayout;
end;
...
begin
  ...
  AComparable := TdxBoxLogPositionComparable.Create
    (DocumentFormatter.DocumentModel.MainPieceTable,
    FSecondaryLayoutStart.LogPosition);
  if not TdxAlgorithms1<TdxBoxBase>.BinarySearch(APages,
    AComparable.BoxAndLogPositionCompare, AFirstPageIndex) then
    AFirstPageIndex := 0;

  AComparable := TdxBoxLogPositionComparable.Create
    (DocumentFormatter.DocumentModel.MainPieceTable,
    FSecondaryLayoutEnd.LogPosition);
  if not TdxAlgorithms1<TdxBoxBase>.BinarySearch(APages,
    AComparable.BoxAndLogPositionCompare, ALastPageIndex) then
...
procedure TdxBackgroundFormatter.RefreshSecondaryLayout(AFirstPageIndex
  : Integer);
var
  I: Integer;
  APage: TdxPage;
...
procedure TdxBackgroundFormatter.SetSpellCheckerController
  (const Value: TdxSpellCheckerControllerBase);
begin
  Assert(Value <> nil);
...
begin
  ...
  FDocumentFormatter.ChangeState
...
procedure TdxBackgroundFormatter.ResetPrimaryLayout(const AFrom,
  ATo: TdxDocumentModelPosition);
begin
  FDocumentFormatter.ParagraphIndex := AFrom.ParagraphIndex - 1;
  FDocumentFormatter.ChangeState
    (TdxDocumentFormatterStateType.BeginParagraphFormatting);
  FCurrentPosition := TdxDocumentModelPosition.FromParagraphStart(PieceTable,
    AFrom.ParagraphIndex);
  FPrimaryLayoutComplete := False;
  Commands[TCommand.PerformPrimaryLayout].SetEvent;
end;

procedure TdxBackgroundFormatter.ResetSecondaryFormattingForPage(APage: TdxPage;
  APageIndex: Integer);
begin
  if FResetSecondaryLayoutFromPage >= 0 then
    FResetSecondaryLayoutFromPage :=
      Math.Min(APageIndex, FResetSecondaryLayoutFromPage)
  else
    FResetSecondaryLayoutFromPage := APageIndex;
end;

procedure TdxBackgroundFormatter.RaisePageFormattingComplete
  (E: TdxPageFormattingCompleteEventArgs);
begin
...
procedure TdxBackgroundFormatter.OnPageFormattingComplete(ASender: TObject;
  E: TdxPageFormattingCompleteEventArgs);
begin
...
function TdxBackgroundFormatter.ShouldResetPrimaryLayout(const AFrom,
  ATo: TdxDocumentModelPosition): Boolean;
begin
  Result := (AFrom.LogPosition <= FCurrentPosition.LogPosition) or
    (ATo.LogPosition <= FCurrentPosition.LogPosition);
end;

function TdxBackgroundFormatter.ShouldResetSecondaryLayout(const AFrom,
  ATo: TdxDocumentModelPosition): Boolean;
begin
...
procedure TdxBackgroundFormatter.UpdateSecondaryPositions(const AFrom,
  ATo: TdxDocumentModelPosition);
begin
...
procedure TdxBackgroundFormatter.WaitForPagePrimaryLayoutComplete
  (APage: TdxPage);
begin
  CheckExecutedAtUIThread;
  BeginDocumentUpdate;
...
procedure TdxBackgroundFormatter.WaitForPrimaryLayoutReachesLogPosition
  (ALogPosition: TdxDocumentLogPosition);
begin
  CheckExecutedAtUIThread;
  BeginDocumentUpdate;
  try
    while ((FCurrentPosition.LogPosition <= ALogPosition) or
      not IsTableFormattingComplete(FCurrentPosition)) and
      not FPrimaryLayoutComplete do
      PerformPrimaryLayoutCore;
  finally
    EndDocumentUpdate;
  end;
...
procedure TdxBackgroundFormatter.WaitForPrimaryLayoutReachesPreferredPage
  (APreferredPageIndex: Integer);
begin
  CheckExecutedAtUIThread;
  BeginDocumentUpdate;
...
function TdxBackgroundFormatter.PredicateDocumentModelPosition
  (const ACurrentFormatterPosition: TdxDocumentModelPosition): Boolean;
begin
  Result := ACurrentFormatterPosition.LogPosition > FPredicatePos.LogPosition;
end;

procedure TdxBackgroundFormatter.WaitForSecondaryLayoutReachesPosition
  (const APos: TdxDocumentModelPosition);
var
  AOriginalStartPos, AOriginalEndPos, AStartPos,
...
begin
  ...
  try
    ...
    NotifyDocumentChanged(AStartPos, AEndPos,
      TdxDocumentLayoutResetType.SecondaryLayout);
  finally
    EndDocumentUpdate;
  end;
  ...
  NotifyDocumentChanged(AOriginalStartPos, AOriginalEndPos,
    TdxDocumentLayoutResetType.SecondaryLayout);
  EndDocumentRendering;
end;
...
function TdxDragCaret.GetCaretPosition(AView: TdxRichEditView)
  : TdxCaretPosition;
begin
  Result := FPosition;
end;
...
function TdxDragCaret.ShouldDrawCaret(ADocumentModel: TdxDocumentModel)
  : Boolean;
begin
  Result := True;
end;
...
procedure TdxDragCaretPosition.SetLogPosition(ALogPosition
  : TdxDocumentLogPosition);
begin
...
begin
  ...
  FLogPosition :=
    Max(0, Min(DocumentModel.MainPieceTable.DocumentEndLogPosition,
    ALogPosition));
end;
...
function TdxHeaderFooterCaretPosition.CreateCaretDocumentLayoutPosition
  : TdxDocumentLayoutPosition;
begin
  Result := TdxHeaderFooterCaretDocumentLayoutPosition.Create(View,
    PreferredPageIndex);
end;

function TdxHeaderFooterCaretPosition.CreateExplicitCaretPosition
  (ALogPosition: TdxDocumentLogPosition): TdxCaretPosition;
begin
  Result := TdxExplicitHeaderFooterCaretPosition.Create(View, ALogPosition,
    PreferredPageIndex);
end;

function TdxHeaderFooterCaretPosition.CreateDragCaretPosition
  : TdxDragCaretPosition;
begin
...
function TdxTextBoxCaretPosition.CreateCaretDocumentLayoutPosition
  : TdxDocumentLayoutPosition;
begin
  Result := TdxTextBoxCaretDocumentLayoutPosition.Create(View,
    PreferredPageIndex);
end;

function TdxTextBoxCaretPosition.CreateExplicitCaretPosition
  (ALogPosition: TdxDocumentLogPosition): TdxCaretPosition;
begin
  Result := TdxExplicitTextBoxCaretPosition.Create(View, ALogPosition,
    PreferredPageIndex);
end;
...
constructor TdxExplicitTextBoxCaretPosition.Create(AView: TdxRichEditView;
  ALogPosition: TdxDocumentLogPosition; APreferredPageIndex: Integer);
begin
  inherited Create(AView, APreferredPageIndex);
  FLogPosition :=
    Max(0, Min(DocumentModel.ActivePieceTable.DocumentEndLogPosition,
    ALogPosition));
end;
...
function TdxExplicitTextBoxCaretPosition.GetVirtualLogPosition
  : TdxDocumentLogPosition;
begin
  Result := FLogPosition;
end;
...
function TdxExplicitTextBoxCaretPosition.
  UpdatePositionTrySetUsePreviousBoxBounds(ADetailsLevel
  : TdxDocumentLayoutDetailsLevel): Boolean;
begin
  Result := False;
end;
...
function TdxHeaderFooterDragCaretPosition.CreateCaretDocumentLayoutPosition
  : TdxDocumentLayoutPosition;
begin
  Result := TdxHeaderFooterCaretDocumentLayoutPosition.Create(View,
    PreferredPageIndex);
...
function TdxHeaderFooterDragCaretPosition.CreateExplicitCaretPosition
  (ALogPosition: TdxDocumentLogPosition): TdxCaretPosition;
begin
  Result := TdxExplicitHeaderFooterCaretPosition.Create(View, ALogPosition,
    PreferredPageIndex);
...
constructor TdxHeaderFooterCaretDocumentLayoutPosition.Create
  (AView: TdxRichEditView; APreferredPageIndex: Integer);
begin
  inherited Create(AView.DocumentLayout, AView.DocumentModel.MainPieceTable, 0,
    APreferredPageIndex);
  Assert(AView <> nil);
...
function TdxHeaderFooterCaretDocumentLayoutPosition.GetPieceTable
  : TdxPieceTable;
begin
  Result := FView.DocumentModel.ActivePieceTable;
end;
...
procedure TdxHeaderFooterCaretDocumentLayoutPosition.
  EnsurePageSecondaryFormattingComplete(APage: TdxPage);
begin
  Assert(FView.DocumentLayout.Pages[PreferredPageIndex] = APage);
  FView.EnsurePageSecondaryFormattingComplete(APage);
end;

function TdxHeaderFooterCaretDocumentLayoutPosition.CreateEmptyClone
  : TdxDocumentLayoutPosition;
begin
  Result := TdxHeaderFooterCaretDocumentLayoutPosition.Create(FView,
    PreferredPageIndex);
end;

{ TdxExplicitHeaderFooterCaretPosition }

constructor TdxExplicitHeaderFooterCaretPosition.Create(AView: TdxRichEditView;
  ALogPosition: TdxDocumentLogPosition; APreferredPageIndex: Integer);
begin
  inherited Create(AView, APreferredPageIndex);
  FLogPosition :=
...
function TdxExplicitHeaderFooterCaretPosition.GetLogPosition
  : TdxDocumentLogPosition;
begin
  Result := FLogPosition;
end;

function TdxExplicitHeaderFooterCaretPosition.GetVirtualLogPosition
  : TdxDocumentLogPosition;
begin
  Result := FLogPosition;
end;
...
function TdxExplicitHeaderFooterCaretPosition.
  UpdatePositionTrySetUsePreviousBoxBounds(ADetailsLevel
  : TdxDocumentLayoutDetailsLevel): Boolean;
begin
  Result := False;
...
constructor TdxTextBoxCaretDocumentLayoutPosition.Create(AView: TdxRichEditView;
  ATextBoxPieceTable: TdxTextBoxContentType; APreferredPageIndex: Integer);
begin
  inherited Create(AView.DocumentLayout, ATextBoxPieceTable, 0,
    APreferredPageIndex);
  Assert(AView <> nil);
  FView := AView;
...
constructor TdxTextBoxCaretDocumentLayoutPosition.Create(AView: TdxRichEditView;
  APreferredPageIndex: Integer);
begin
  Create(AView, TdxTextBoxContentType(AView.DocumentModel.ActivePieceTable.
    ContentType), APreferredPageIndex);
end;
...
function TdxTextBoxCaretDocumentLayoutPosition.Update(APages: TdxPageCollection;
  ADetailsLevel: TdxDocumentLayoutDetailsLevel): Boolean;
begin
...
begin
  FView.EnsureFormattingCompleteForLogPosition
    (FTextBoxContentType.AnchorRun.Paragraph.EndLogPosition);
end;

function TdxTextBoxCaretDocumentLayoutPosition.CreateEmptyClone
  : TdxDocumentLayoutPosition;
begin
  Result := TdxTextBoxCaretDocumentLayoutPosition.Create(FView,
    FTextBoxContentType, PreferredPageIndex);
end;

function TdxTextBoxCaretDocumentLayoutPosition.IsValid(ADetailsLevel
  : TdxDocumentLayoutDetailsLevel): Boolean;
begin
  if (FTextBoxContentType = nil) or (FTextBoxContentType.AnchorRun = nil) then
    Result := False
...
function TdxTextBoxDragCaretPosition.CreateCaretDocumentLayoutPosition
  : TdxDocumentLayoutPosition;
begin
  Result := TdxTextBoxCaretDocumentLayoutPosition.Create(View,
    PreferredPageIndex);
...
function TdxTextBoxDragCaretPosition.CreateExplicitCaretPosition
  (ALogPosition: TdxDocumentLogPosition): TdxCaretPosition;
begin
  Result := TdxExplicitTextBoxCaretPosition.Create(View, ALogPosition,
    PreferredPageIndex);
...
begin
  if (ASelectionCollection.Count <= 0) or (ASelectionCollection[0].Length <= 0)
  then
    Exit;
...
constructor TdxCopySelectionManager.Create(const ADocumentServer
  : IdxInnerControl);
begin
  inherited Create;
...
function TdxCopySelectionManager.CreateDocumentModel
...
begin
  ...
  begin
    Result.MainPieceTable.PreCalculatedNumberingListTexts :=
      TDictionary<TdxParagraph, string>.Create;
  end;
  if not ATextOptions.AllowExtendingDocumentRange then
    ASelectionRanges := UpdateSelectionCollection(ASourcePieceTable,
      ASelectionRanges);
  try
    Result.InternalAPI.ImporterFactory :=
      ASourcePieceTable.DocumentModel.InternalAPI.ImporterFactory;
    Result.InternalAPI.ExporterFactory :=
      ASourcePieceTable.DocumentModel.InternalAPI.ExporterFactory;
    Result.FieldOptions.UpdateFieldsOnPaste :=
      ASourcePieceTable.DocumentModel.FieldOptions.UpdateFieldsOnPaste;
    ...
    try
      ...
      try
        AOptions.ParagraphNumerationCopyOptions :=
          AParagraphNumerationCopyOptions;
        AOptions.FormattingCopyOptions := AFormattingCopyOptions;
        AOptions.DefaultPropertiesCopyOptions := DefaultPropertiesCopyOptions;
        ACopyCommand := TdxDocumentModelCopyCommand
          (ASourcePieceTable.DocumentModel.CreateDocumentModelCopyCommand
          (ASourcePieceTable, Result, AOptions));
        try
...
begin
  try
    ATarget := CreateDocumentModel
      (TdxParagraphNumerationCopyOptions.CopyIfWholeSelected,
      TdxFormattingCopyOptions.UseDestinationStyles, APieceTable,
      ASelectionRanges, True, nil);
    try
      SubscribeTargetModelEvents(ATarget);
      ALastParagraphRunSelected := IsLastParagraphRunSelected(APieceTable,
        ASelectionRanges);
      Result := ATarget.InternalAPI.GetDocumentRtfContent(AOptions,
        not ALastParagraphRunSelected, KeepFieldCodeViewState,
        AForceRaiseBeforeExport, AForceRaiseAfterExport);
...
function TdxCopySelectionManager.GetOpenXmlBytes(APieceTable: TdxPieceTable;
  ASelectionCollection: TdxSelectionRangeCollection;
...
function TdxCopySelectionManager.GetSuppressStoreImageSizeCollection
  (APieceTable: TdxPieceTable;
  ASelectionCollection: TdxSelectionRangeCollection): string;
var
  ATarget: TdxDocumentModel;
      ...
      begin
        ...
        begin
          ...
          if APictureRun.Image.SuppressStore then
            ASuppressStoreImageScale.Append(TdxStringHelper.Format('%g,%g',
              [APictureRun.ScaleX, APictureRun.ScaleY]));
        end;
      end;
      UnsubscribeTargetModelEvents(ATarget);
    finally
...
function TdxCopySelectionManager.IsLastParagraphRunSelected
  (APieceTable: TdxPieceTable;
  ASelectionRanges: TdxSelectionRangeCollection): Boolean;
var
...
begin
  ...
  try
    ...
    try
      ...
      ARtfText := GetRtfText(APieceTable, ASelectionCollection, AOptions,
        True, True);
      if ARtfText <> '' then
      begin
    ...
    ASuppressStoreImageScale := GetSuppressStoreImageSizeCollection(APieceTable,
      ASelectionCollection);
    if ASuppressStoreImageScale <> '' then
      ADataObject.SetData(TdxOfficeDataFormats.SuppressStoreImageSize,
        ASuppressStoreImageScale);
  finally
    ADataObject.Close;
  end;
end;

function TdxCopySelectionManager.ShouldExtendRange(AInfo: TdxRunInfo;
  AField: TdxField): Boolean;
begin
  if AField = nil then
    Exit(False);
  Result := not TdxFieldsOperation.IsFieldCodeTextAffectedOnly(AInfo, AField)
    and not TdxFieldsOperation.IsFieldResultTextAffectedOnly(AInfo, AField) and
    not TdxFieldsOperation.IsEntireFieldAffected(AInfo, AField);
end;

function TdxCopySelectionManager.UpdateSelectionCollection(ASourcePieceTable
  : TdxPieceTable; ASelection: TdxSelectionRangeCollection)
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        AStart := Max(AStart, ASourcePieceTable.GetRunLogPosition
          (AField.Result.Start));
        AEnd := Min(AEnd, ASourcePieceTable.GetRunLogPosition
          (AField.Result.&End));
        if AStart < AEnd then
...
procedure TdxCopySelectionManager.DoBeforeExport(Sender: TObject;
  E: TdxBeforeExportEventArgs);
begin
  DocumentServer.OnBeforeExport(Sender, E);
end;

procedure TdxCopySelectionManager.DoCalculateDocumentVariable(Sender: TObject;
  E: TdxCalculateDocumentVariableEventArgs);
begin
  DocumentServer.OnCalculateDocumentVariable(Sender, E);
end;

procedure TdxCopySelectionManager.SubscribeTargetModelEvents
...
procedure TdxCopySelectionManager.UnsubscribeTargetModelEvents
  (ATargetModel: TdxDocumentModel);
begin
...
constructor TdxRichEditControlOptionsBase.Create(const ADocumentServer
...
function TdxRichEditControlOptionsBase.GetAuthentication
...
function TdxRichEditControlOptionsBase.GetMailMerge
...
function TdxRichEditControlOptionsBase.GetRangePermissions
...
function TdxRichEditControlOptionsBase.GetDocumentCapabilities
  : TdxDocumentCapabilitiesOptions;
begin
  if DocumentServer = nil then
    Result := nil
...
function TdxRichEditControlOptionsBase.GetDocumentSaveOptions
...
function TdxRichEditControlOptionsBase.GetExport
...
function TdxRichEditControlOptionsBase.GetImport
...
function TdxRichEditControlOptionsBase.GetFormattingMarkVisibility
  : TdxFormattingMarkVisibilityOptions;
begin
  if DocumentServer = nil then
    Result := nil
...
procedure TdxRichEditControlOptionsBase.SetAuthentication
  (const Value: TdxAuthenticationOptions);
begin
  Authentication.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetAutoCorrect
  (const Value: TdxAutoCorrectOptions);
begin
  AutoCorrect.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetBehavior
  (const Value: TdxRichEditBehaviorOptions);
begin
  Behavior.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetDocumentCapabilities
  (const Value: TdxDocumentCapabilitiesOptions);
begin
  DocumentCapabilities.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetDocumentSaveOptions
  (const Value: TdxDocumentSaveOptions);
begin
  DocumentSaveOptions.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetExport
  (const Value: TdxRichEditDocumentExportOptions);
begin
  Export.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetImport
  (const Value: TdxRichEditDocumentImportOptions);
begin
  Import.Assign(Value);
end;
...
procedure TdxRichEditControlOptionsBase.SetFormattingMarkVisibility
  (const Value: TdxFormattingMarkVisibilityOptions);
begin
  FormattingMarkVisibility.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetHorizontalRuler
  (const Value: TdxHorizontalRulerOptions);
begin
  FHorizontalRuler.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetHyperlinks
  (const Value: TdxHyperlinkOptions);
begin
  FHyperlinks.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetTableOptions
  (const Value: TdxTableOptions);
begin
  TableOptions.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetLayout
  (const Value: TdxRichEditLayoutOptions);
begin
  Layout.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetMailMerge
  (const Value: TdxRichEditMailMergeOptions);
begin
  MailMerge.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetPrinting
  (const Value: TdxPrintingOptions);
begin
  Printing.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetRangePermissions
  (const Value: TdxRangePermissionOptions);
begin
  RangePermissions.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetBookmarks
  (const Value: TdxBookmarkOptions);
begin
  Bookmarks.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetCopyPaste
  (const Value: TdxCopyPasteOptions);
begin
  CopyPaste.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetSearch
  (const Value: TdxDocumentSearchOptions);
begin
  Search.Assign(Value);
end;
...
procedure TdxRichEditControlOptionsBase.SetSpellChecker
  (const Value: TdxSpellCheckerOptions);
begin
  SpellChecker.Assign(Value);
end;

procedure TdxRichEditControlOptionsBase.SetVerticalRuler
  (const Value: TdxVerticalRulerOptions);
begin
