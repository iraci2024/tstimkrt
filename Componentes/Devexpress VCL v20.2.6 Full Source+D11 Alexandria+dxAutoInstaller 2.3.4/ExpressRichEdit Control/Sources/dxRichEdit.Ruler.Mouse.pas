...
type
  ...
  public
    constructor Create(AController: TdxRichEditMouseController;
      ADragState: TdxRichEditMouseCustomState; const APoint: TPoint;
      AHotZone: TdxRulerHotZone); reintroduce;
  ...
  TdxDragAndDropMouseHandlerState = class
    (TdxRichEditMouseCustomVisualFeedbackState, IdxKeyboardHandlerService,
  ...
  protected
    function ConvertMouseEventArgs(const Args: TdxMouseEventArgs)
      : TdxMouseEventArgs; override;
  public
    function CreateLineVisualFeedback(const AValueProvider
      : TdxRichEditMouseCustomVisualFeedbackState): IdxVisualFeedback; override;
...
type
  ...
  TdxRichEditReversibleLineVisualFeedback<T> = class abstract
    (TdxVisualFeedback<T>)
  strict private
  ...
  public
    constructor Create(AControl: TdxCustomRichEditControl;
  ...
  TdxRichEditHorizontalLineVisualFeedback = class
    (TdxRichEditReversibleLineVisualFeedback<TdxRectangleVisualFeedbackValue>)
  protected
  ...
  TdxRichEditVerticalLineVisualFeedback = class
    (TdxRichEditReversibleLineVisualFeedback<TdxRectangleVisualFeedbackValue>)
  protected
    procedure DrawReversibleLine; override;
  end;
...
function TdxDefaultRulerMouseHandlerState.GetController
  : TdxRulerMouseController;
begin
  Result := TdxRulerMouseController(inherited Controller);
end;

procedure TdxDefaultRulerMouseHandlerState.HandleMouseDown
...
begin
  ...
  if (AHotZone <> nil) and AHotZone.Enabled and AHotZone.CanEdit and
    AHotZone.CanActivate(Args) then
  begin
...
procedure TdxDefaultRulerMouseHandlerState.HandleMouseDoubleClick
  (const Args: TdxMouseEventArgs);
var
  APoint: TPoint;
  AHotZone: TdxRulerHotZone;
...
procedure TdxDefaultRulerMouseHandlerState.HandleMouseMove
  (const Args: TdxMouseEventArgs);
var
  APoint: TPoint;
  AHotZone: TdxRulerHotZone;
...
function TdxDefaultRulerMouseHandlerState.CreateZone(const APoint: TPoint)
  : TdxRulerHotZone;
var
...
begin
  ...
  begin
    APosition := Controller.GetSnappedPosition
      (Controller.GetPrimaryCoordinate(APoint));
    AHotZone := ARulerControl.ViewInfo.TabTypeToggleHotZone.HotZone.Clone;
    AHotZone.SetNewValue(Control.ViewInfo.GetRulerModelPosition(APosition -
      ARulerControl.ViewInfo.GetAdditionalCellIndent(True)));
    AHotZone.IsNew := True;
...
function TdxDefaultRulerMouseHandlerState.GetRulerMovableElementIndex
  (const APoint: TPoint): TdxRulerHotZone;
var
...
function TdxDefaultRulerMouseHandlerState.GetHitActiveAreaStartPosition
...
constructor TdxBeginDragHotZoneMouseDragHelperState.Create
...
procedure TdxBeginDragHotZoneMouseDragHelperState.HandleMouseUp
...
constructor TdxDragAndDropMouseHandlerState.Create
  (AController: TdxRichEditMouseController; AHotZone: TdxRulerHotZone;
  const AStartMousePosition: TPoint);
var
...
procedure TdxDragAndDropMouseHandlerState.HandleMouseMove
  (const E: TdxMouseEventArgs);
begin
  HideVisualFeedback;
...
var
  APrimary: Integer;
begin
  APrimary := Controller.GetSnappedPosition
    (Controller.GetPrimaryCoordinate(APt));
  FPoint := Controller.CreatePoint(APrimary,
    Controller.GetSecondaryCoordinate(APt));
  FHotZone.OnMove(FPoint);
  Control.Repaint;
end;

procedure TdxDragAndDropMouseHandlerState.HandleMouseUp
  (const E: TdxMouseEventArgs);
var
  APt: TPoint;
  APrimary: Integer;
...
function TdxDragAndDropMouseHandlerState.CanKeyDownModifyEdit
  (const Args: TdxKeyEventArgs): Boolean;
begin
  Result := Args.KeyData = VK_RETURN;
end;

function TdxDragAndDropMouseHandlerState.HandleKeyDown
  (const E: TdxKeyEventArgs): Boolean;
begin
  Result := True;
...
function TdxDragAndDropMouseHandlerState.HandleKeyPress
  (const E: TdxKeyPressEventArgs): Boolean;
begin
  Result := False;
end;

function TdxDragAndDropMouseHandlerState.HandleKeyUp
  (const E: TdxKeyEventArgs): Boolean;
begin
  Result := False;
end;
...
function TdxDragAndDropMouseHandlerState.GetVisualFeedbackValue
  : TdxRectangleVisualFeedbackValue;
var
  AValue: Integer;
begin
  AValue := FHotZone.GetVisualFeedbackValue(FPoint, FPageViewInfo);
  Result := TdxRectangleVisualFeedbackValue.Create(FPageViewInfo,
    TRect.CreateSize(Controller.CreatePoint(AValue, 0), 0, 0));
end;
...
function TdxHorizontalRulerMouseHandler.GetHotZoneBounds
  (AHotZone: TdxRulerHotZone): TRect;
begin
  Result := AHotZone.Bounds;
end;

procedure TdxHorizontalRulerMouseHandler.MouseDown(Button: TMouseButton;
...
function TdxHorizontalRulerMouseHandler.ConvertMouseEventArgs
...
begin
  ...
  ALocation := Control.RichEditControl.ActiveView.CreateLogicalPoint
    (Control.ViewInfo.Bounds, ALocation);
  ALocation.Init(Trunc(ALocation.X * Control.ZoomFactor),
    Trunc(ALocation.Y * Control.ZoomFactor));
  Result.MousePos := ALocation;
end;

function TdxHorizontalRulerMouseHandler.CreateLineVisualFeedback
  (const AValueProvider: TdxRichEditMouseCustomVisualFeedbackState)
  : IdxVisualFeedback;
begin
  Result := TdxRichEditVerticalLineVisualFeedback.Create
    (Control.RichEditControl, AValueProvider);
end;

{ TdxVerticalRulerMouseHandler }

function TdxVerticalRulerMouseHandler.ConvertMouseEventArgs
  (const Args: TdxMouseEventArgs): TdxMouseEventArgs;
var
  ALocation: TPoint;
begin
...
function TdxVerticalRulerMouseHandler.CreateLogicalPoint(const AClientBounds
  : TRect; const APoint: TPoint): TPoint;
var
  Y: Integer;
  AMatrix: TdxGPMatrix;
begin
  Y := Control.RichEditControl.ActiveView.CaretPosition.PageViewInfo.
    ClientBounds.Y;
...
function TdxVerticalRulerMouseHandler.CreateLineVisualFeedback
  (const AValueProvider: TdxRichEditMouseCustomVisualFeedbackState)
  : IdxVisualFeedback;
begin
  Result := TdxRichEditHorizontalLineVisualFeedback.Create
    (Control.RichEditControl, AValueProvider);
end;

{ TdxRichEditReversibleLineVisualFeedback }

constructor TdxRichEditReversibleLineVisualFeedback<T>.Create
...
begin
  ...
  try
    if AValue.PageViewInfo <> nil then
      Control.Painter.DrawReversibleHorizontalLine(AValue.Bounds.Y,
        AValue.PageViewInfo);
  finally
    AValue.Free;
  end;
...
begin
  ...
  try
    if AValue.PageViewInfo <> nil then
      Control.Painter.DrawReversibleVerticalLine(AValue.Bounds.X,
        AValue.PageViewInfo);
  finally
    AValue.Free;
  end;
