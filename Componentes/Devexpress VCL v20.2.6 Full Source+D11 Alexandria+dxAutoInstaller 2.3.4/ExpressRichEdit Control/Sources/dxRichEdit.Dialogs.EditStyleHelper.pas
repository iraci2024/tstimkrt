...
type
  ...
  public
    class procedure ChangeConditionalType(ATable: TdxTable;
      AStyleType: TdxConditionalTableStyleFormattingTypes); static;
    class function IsFontSizeValid(AEditValue: Variant; out AText: string;
      out AValue: Integer): Boolean; overload;
    class function IsFontSizeValid(AEditValue: Variant; out AText: string)
      : Boolean; overload;
    class function IsValidFontSize(AFontSize: Integer): Boolean;
    class function IsValidStyleName(const AName: string;
      ASourceStyle: TdxStyleBase; ACollection: TdxStyleCollectionBase)
      : Boolean; static;
  end;
  ...
  public
    constructor Create(ACharacterProperties: TdxCharacterProperties;
    ...
    property ParentCharacterProperties: TdxCharacterFormattingInfo
      read FParentCharacterProperties;
  end;
  ...
  protected
    function GetOptionsUseValue(AUseOption
      : TdxUsedParagraphFormattingOption): Boolean;
  public
    constructor Create(AParagraphProperties: TdxParagraphProperties;
      AParentParagraphProperties: TdxParagraphFormattingInfo); overload;
    constructor Create(AParagraphProperties: TdxParagraphProperties;
    ...
    property ParentParagraphProperties: TdxParagraphFormattingInfo
      read FParentParagraphProperties;
  end;
  ...
  public
    constructor Create(ATableCellProperties: TdxTableCellProperties;
      AParentCellProperties: TdxTableCellGeneralSettingsInfo;
      AParentTableCellOptions: TdxTableCellPropertiesOptions);
    destructor Destroy; override;
    ...
    property ParentTableCellOptions: TdxTableCellPropertiesOptions
      read FParentTableCellOptions;
    property TableCellProperties: TdxTableCellProperties
      read FTableCellProperties;
    property ParentCellProperties: TdxTableCellGeneralSettingsInfo
      read FParentCellProperties;
  end;
...
class function TdxEditStyleHelper.IsFontSizeValid(AEditValue: Variant;
  out AText: string; out AValue: Integer): Boolean;
var
  AIsIntValue: Boolean;
begin
  AText := '';
  AIsIntValue := TdxRichEditFontSizeHelper.TryGetHalfSizeValue
    (AEditValue, AValue);
  Result := True;
  if AIsIntValue then
  begin
    ...
    begin
      AText := Format(cxGetResourceString(@sdxRichEditInvalidSize),
        [TdxPredefinedFontSizeCollection.MinFontSize,
        TdxPredefinedFontSizeCollection.MaxFontSize]);
      Result := False;
    end;
  end
  else
...
class function TdxEditStyleHelper.IsValidStyleName(const AName: string;
  ASourceStyle: TdxStyleBase; ACollection: TdxStyleCollectionBase): Boolean;
var
  AStyle: TdxStyleBase;
  I: Integer;
begin
...
class procedure TdxEditStyleHelper.ChangeConditionalType(ATable: TdxTable;
...
begin
  ...
  if TdxConditionalTableStyleFormattingType.TopLeftCell in AStyleType then
    ATableLook := ATableLook + [TdxTableLookType.ApplyFirstColumn,
      TdxTableLookType.ApplyFirstRow];

  if TdxConditionalTableStyleFormattingType.TopRightCell in AStyleType then
    ATableLook := ATableLook + [TdxTableLookType.ApplyLastColumn,
      TdxTableLookType.ApplyFirstRow];
...
constructor TdxCharacterPropertyAccessor.Create(ACharacterProperties
  : TdxCharacterProperties; AParentCharacterProperties
...
begin
  CharacterProperties.AllCaps :=
    TdxDocumentModel(CharacterProperties.DocumentModel)
    .DefaultCharacterProperties.AllCaps;
  CharacterProperties.ResetUse(TdxUsedCharacterFormattingOption.UseAllCaps);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.AllCaps) then
    CharacterProperties.AllCaps := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeDoubleFontSize
  (const AValue: Integer);
begin
  CharacterProperties.ResetUse
    (TdxUsedCharacterFormattingOption.UseDoubleFontSize);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.DoubleFontSize) then
...
begin
  ...
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.FontBold) then
    CharacterProperties.FontBold := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeForeColor
  (const AValue: TdxAlphaColor);
begin
  CharacterProperties.ResetUse(TdxUsedCharacterFormattingOption.UseForeColor);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.ForeColor) then
    CharacterProperties.ForeColor := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeHidden(const AValue: Boolean);
begin
  CharacterProperties.Hidden :=
    TdxDocumentModel(CharacterProperties.DocumentModel)
    .DefaultCharacterProperties.Hidden;
  CharacterProperties.ResetUse(TdxUsedCharacterFormattingOption.UseHidden);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.Hidden) then
    CharacterProperties.Hidden := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeScript(const AValue
  : TdxCharacterFormattingScript);
begin
  CharacterProperties.Script :=
    TdxDocumentModel(CharacterProperties.DocumentModel)
    .DefaultCharacterProperties.Script;
  CharacterProperties.ResetUse(TdxUsedCharacterFormattingOption.UseScript);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.Script) then
    CharacterProperties.Script := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeUnderlineColor
  (const AValue: TdxAlphaColor);
begin
  CharacterProperties.UnderlineColor :=
    TdxDocumentModel(CharacterProperties.DocumentModel)
    .DefaultCharacterProperties.UnderlineColor;
  CharacterProperties.ResetUse
    (TdxUsedCharacterFormattingOption.UseUnderlineColor);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.UnderlineColor) then
    CharacterProperties.UnderlineColor := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeUnderlineWordsOnly
  (const AValue: Boolean);
begin
  CharacterProperties.UnderlineWordsOnly :=
    TdxDocumentModel(CharacterProperties.DocumentModel)
    .DefaultCharacterProperties.UnderlineWordsOnly;
  CharacterProperties.ResetUse
    (TdxUsedCharacterFormattingOption.UseUnderlineWordsOnly);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.UnderlineWordsOnly) then
...
begin
  ...
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.FontItalic) then
...
begin
  ...
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.FontName) then
    CharacterProperties.FontName := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeFontStrikeoutType
  (const AValue: TdxStrikeoutType);
begin
  CharacterProperties.FontStrikeoutType :=
    TdxDocumentModel(CharacterProperties.DocumentModel)
    .DefaultCharacterProperties.FontStrikeoutType;
  CharacterProperties.ResetUse
    (TdxUsedCharacterFormattingOption.UseFontStrikeoutType);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.FontStrikeoutType) then
    CharacterProperties.FontStrikeoutType := AValue;
end;

procedure TdxCharacterPropertyAccessor.ChangeFontUnderlineType
  (const AValue: TdxUnderlineType);
begin
  CharacterProperties.ResetUse
    (TdxUsedCharacterFormattingOption.UseFontUnderlineType);
  if (ParentCharacterProperties = nil) or
    (AValue <> ParentCharacterProperties.FontUnderlineType) then
    CharacterProperties.FontUnderlineType := AValue;
end;

{ TdxParagraphPropertyAccessor }

procedure TdxParagraphPropertyAccessor.ChangeAfterAutoSpacing
  (const AValue: Boolean);
begin
  ParagraphProperties.AfterAutoSpacing :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.AfterAutoSpacing;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseAfterAutoSpacing]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.AfterAutoSpacing) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseAfterAutoSpacing)
  then
    ParagraphProperties.AfterAutoSpacing := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeAlignment
  (const AValue: TdxParagraphAlignment);
begin
  ParagraphProperties.Alignment :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.Alignment;
  ParagraphProperties.ResetUse([TdxUsedParagraphFormattingOption.UseAlignment]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.Alignment) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseAlignment) then
    ParagraphProperties.Alignment := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeBackColor
  (const AValue: TdxAlphaColor);
begin
  ParagraphProperties.BackColor :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.BackColor;
  ParagraphProperties.ResetUse([TdxUsedParagraphFormattingOption.UseBackColor]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.BackColor) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseBackColor) then
    ParagraphProperties.BackColor := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeBeforeAutoSpacing
  (const AValue: Boolean);
begin
  ParagraphProperties.BeforeAutoSpacing :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.BeforeAutoSpacing;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseBeforeAutoSpacing]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.BeforeAutoSpacing) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseBeforeAutoSpacing)
  then
    ParagraphProperties.BeforeAutoSpacing := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeContextualSpacing
  (const AValue: Boolean);
begin
  ParagraphProperties.ContextualSpacing :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.ContextualSpacing;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseContextualSpacing]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.ContextualSpacing) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseContextualSpacing)
  then
    ParagraphProperties.ContextualSpacing := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeFirstLineIndent
  (const AValue: Integer);
begin
  ParagraphProperties.FirstLineIndent :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.FirstLineIndent;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseFirstLineIndent]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.FirstLineIndent) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseFirstLineIndent)
  then
    ParagraphProperties.FirstLineIndent := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeKeepLinesTogether
  (const AValue: Boolean);
begin
  ParagraphProperties.KeepLinesTogether :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.KeepLinesTogether;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseKeepLinesTogether]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.KeepLinesTogether) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseKeepLinesTogether)
  then
    ParagraphProperties.KeepLinesTogether := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeKeepWithNext
  (const AValue: Boolean);
begin
  ParagraphProperties.KeepWithNext :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.KeepWithNext;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseKeepWithNext]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.KeepWithNext) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseKeepWithNext)
  then
...
begin
  ParagraphProperties.LeftIndent :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.LeftIndent;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseLeftIndent]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.LeftIndent) or
...
begin
  ParagraphProperties.LineSpacing :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.LineSpacing;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseLineSpacing]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.LineSpacing) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseLineSpacing) then
    ParagraphProperties.LineSpacing := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeOutlineLevel
  (const AValue: Integer);
begin
  ParagraphProperties.OutlineLevel :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.OutlineLevel;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseOutlineLevel]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.OutlineLevel) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseOutlineLevel)
  then
    ParagraphProperties.OutlineLevel := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangePageBreakBefore
  (const AValue: Boolean);
begin
  ParagraphProperties.PageBreakBefore :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.PageBreakBefore;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UsePageBreakBefore]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.PageBreakBefore) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UsePageBreakBefore)
  then
...
begin
  ParagraphProperties.RightIndent :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.RightIndent;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseRightIndent]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.RightIndent) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseRightIndent) then
    ParagraphProperties.RightIndent := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeSpacingAfter
  (const AValue: Integer);
begin
  ParagraphProperties.SpacingAfter :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.SpacingAfter;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseSpacingAfter]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.SpacingAfter) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseSpacingAfter)
  then
    ParagraphProperties.SpacingAfter := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeSpacingBefore
  (const AValue: Integer);
begin
  ParagraphProperties.SpacingBefore :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.SpacingBefore;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseSpacingBefore]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.SpacingBefore) or
    not GetOptionsUseValue(TdxUsedParagraphFormattingOption.UseSpacingBefore)
  then
    ParagraphProperties.SpacingBefore := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeSuppressHyphenation
  (const AValue: Boolean);
begin
  ParagraphProperties.SuppressHyphenation :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.SuppressHyphenation;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseSuppressHyphenation]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.SuppressHyphenation) or
    not GetOptionsUseValue
    (TdxUsedParagraphFormattingOption.UseSuppressHyphenation) then
    ParagraphProperties.SuppressHyphenation := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeSuppressLineNumbers
  (const AValue: Boolean);
begin
  ParagraphProperties.SuppressLineNumbers :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.SuppressLineNumbers;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseSuppressLineNumbers]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.SuppressLineNumbers) or
    not GetOptionsUseValue
    (TdxUsedParagraphFormattingOption.UseSuppressLineNumbers) then
    ParagraphProperties.SuppressLineNumbers := AValue;
end;

procedure TdxParagraphPropertyAccessor.ChangeWidowOrphanControl
  (const AValue: Boolean);
begin
  ParagraphProperties.WidowOrphanControl :=
    TdxDocumentModel(ParagraphProperties.DocumentModel)
    .DefaultParagraphProperties.WidowOrphanControl;
  ParagraphProperties.ResetUse
    ([TdxUsedParagraphFormattingOption.UseWidowOrphanControl]);
  if (ParentParagraphProperties = nil) or
    (AValue <> ParentParagraphProperties.WidowOrphanControl) or
    not GetOptionsUseValue
    (TdxUsedParagraphFormattingOption.UseWidowOrphanControl) then
    ParagraphProperties.WidowOrphanControl := AValue;
end;

constructor TdxParagraphPropertyAccessor.Create(AParagraphProperties
  : TdxParagraphProperties; AParentParagraphProperties
  : TdxParagraphFormattingInfo;
  const AParentParagraphOptions: TdxParagraphFormattingOptions);
begin
...
function TdxParagraphPropertyAccessor.GetOptionsUseValue
  (AUseOption: TdxUsedParagraphFormattingOption): Boolean;
begin
...
constructor TdxTableCellPropertyAccessor.Create(ATableCellProperties
...
procedure TdxTableCellPropertyAccessor.ChangeBackgroundColor
  (const AValue: TdxAlphaColor);
begin
  TableCellProperties.BackgroundColor :=
    TdxDocumentModel(TableCellProperties.DocumentModel)
    .DefaultTableCellProperties.BackgroundColor;
  TableCellProperties.ResetUse
    (TdxTableCellPropertiesOptions.MaskUseBackgroundColor);
  if (ParentCellProperties = nil) or
    (AValue <> ParentCellProperties.BackgroundColor) or
    (not ParentTableCellOptions.UseBackgroundColor) then
    TableCellProperties.BackgroundColor := AValue;
end;

procedure TdxTableCellPropertyAccessor.ChangeVerticalAlignment
  (const AValue: TdxVerticalAlignment);
begin
  TableCellProperties.VerticalAlignment :=
    TdxDocumentModel(TableCellProperties.DocumentModel)
    .DefaultTableCellProperties.VerticalAlignment;
  TableCellProperties.ResetUse
    (TdxTableCellPropertiesOptions.MaskUseVerticalAlignment);
  if (ParentCellProperties = nil) or
    (AValue <> ParentCellProperties.VerticalAlignment) or
