...
type
  ...
  IdxInternalRichEditDocumentServerOwner = interface
    ...
    property InternalServer: TdxInternalRichEditDocumentServer
      read GetInternalServer;
  end;

  { TdxInternalRichEditDocumentServer }

  TdxInternalRichEditDocumentServer = class abstract(TInterfacedObject,
    IdxRichEditDocumentContainer, IdxRichEditDocumentServer, IdxServiceProvider,
    IdxServiceContainer, IdxInnerRichEditDocumentContainerOwner,
    IdxInnerRichEditDocumentServerOwner, IdxRichEditDocumentLayoutProvider)
  strict private
  ...
  protected
    ...
    function CreateInnerServer(ADocumentModel: TdxDocumentModel)
      : TdxInnerRichEditDocumentServer; virtual;
    ...
    function CreateOptionsCore(ADocumentServer: TdxInnerRichEditDocumentServer)
      : TdxRichEditControlOptionsBase; virtual;
  ...
  public
    ...
    class function TryConvertInternalRichEditDocumentServer(const AObj: TValue)
      : TdxInternalRichEditDocumentServer; static;
    ...
    property LayoutCalculationMode: TdxCalculationModeType
      read GetLayoutCalculationMode write SetLayoutCalculationMode;
...
constructor TdxInternalRichEditDocumentServer.Create(ADocumentModel
  : TdxDocumentModel = nil);
begin
  inherited Create;
...
class function TdxInternalRichEditDocumentServer.
  TryConvertInternalRichEditDocumentServer(const AObj: TValue)
  : TdxInternalRichEditDocumentServer;
var
...
begin
  ...
  if Result <> nil then
    Exit;
  if Supports(AObj.AsObject, IdxInternalRichEditDocumentServerOwner, AOwner)
  then
    Result := AOwner.InternalServer;
end;

function TdxInternalRichEditDocumentServer.GetLayoutCalculationMode
...
function TdxInternalRichEditDocumentServer.GetDocumentLayoutAsync
  : TdxDocumentLayout;
begin
  Result := FInnerServer.ModelDocumentLayout;
end;

procedure TdxInternalRichEditDocumentServer.PerformPageSecondaryFormatting
  (APage: TdxPage);
begin
...
procedure TdxInternalRichEditDocumentServer.AddPageFormatted(const AHandler
  : TdxDocumentLayoutInvalidatedEvent);
begin
  FInnerServer.PageFormatted.Add(AHandler);
end;

procedure TdxInternalRichEditDocumentServer.AddDocumentFormatted
...
procedure TdxInternalRichEditDocumentServer.RemovePageFormatted
  (const AHandler: TdxDocumentLayoutInvalidatedEvent);
begin
  FInnerServer.PageFormatted.Add(AHandler);
end;

procedure TdxInternalRichEditDocumentServer.RemoveDocumentFormatted
  (const AHandler: TdxEvent);
begin
  FInnerServer.DocumentFormatted.Add(AHandler);
end;
...
function TdxInternalRichEditDocumentServer.GetMeasurementUnit
  : TdxMeasurementUnit;
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.SetMeasurementUnit
  (Value: TdxMeasurementUnit);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.SetLayoutUnit
  (Value: TdxDocumentLayoutUnit);
begin
  if InnerServer <> nil then
    InnerServer.LayoutUnit := Value;
end;

procedure TdxInternalRichEditDocumentServer.SetLayoutCalculationMode
  (const Value: TdxCalculationModeType);
begin
  InnerServer.LayoutCalculationMode := Value;
end;

procedure TdxInternalRichEditDocumentServer.AddAfterExportHandler
...
procedure TdxInternalRichEditDocumentServer.RemoveAfterExportHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddSelectionChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveSelectionChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddDocumentLoadedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveDocumentLoadedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddEmptyDocumentCreatedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveEmptyDocumentCreatedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddDocumentClosingHandler
...
procedure TdxInternalRichEditDocumentServer.RemoveDocumentClosingHandler
  (const AHandler: TdxCloseQueryEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddContentChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveContentChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddModifiedChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveModifiedChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddUnitChangingHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveUnitChangingHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddUnitChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveUnitChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddCalculateDocumentVariableHandler
...
procedure TdxInternalRichEditDocumentServer.
  RemoveCalculateDocumentVariableHandler(const AHandler
  : TdxCalculateDocumentVariableEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddBeforeImportHandler
...
procedure TdxInternalRichEditDocumentServer.RemoveBeforeImportHandler
  (const AHandler: TdxBeforeImportEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddBeforeExportHandler
...
procedure TdxInternalRichEditDocumentServer.RemoveBeforeExportHandler
  (const AHandler: TdxBeforeExportEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddInitializeDocumentHandler
  (const AHandler: TdxEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveInitializeDocumentHandler
  (const AHandler: TdxEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddRtfTextChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveRtfTextChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.AddHtmlTextChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.RemoveHtmlTextChangedHandler
  (const AHandler: TNotifyEvent);
begin
  if InnerServer <> nil then
...
function TdxInternalRichEditDocumentServer.GetOptions
...
function TdxInternalRichEditDocumentServer.CreateMeasurementAndDrawingStrategy
...
function TdxInternalRichEditDocumentServer.CreateOptions(const ADocumentServer
  : TObject { TdxInnerRichEditDocumentServer } )
  : TObject { TdxRichEditControlOptionsBase };
begin
...
function TdxInternalRichEditDocumentServer.CreateOptionsCore(ADocumentServer
  : TdxInnerRichEditDocumentServer): TdxRichEditControlOptionsBase;
begin
  Result := TdxRichEditDocumentServerOptions.Create(ADocumentServer);
end;

procedure TdxInternalRichEditDocumentServer.RaiseDeferredEvents
  (AChangeActions: TdxDocumentModelChangeActions);
begin
  InnerServer.RaiseDeferredEventsCore(AChangeActions);
end;

function TdxInternalRichEditDocumentServer.GetBatchUpdateHelper
...
procedure TdxInternalRichEditDocumentServer.AddService(const AServiceType
...
begin
  if InnerServer <> nil then
    (InnerServer as IdxServiceContainer).AddService(AServiceType,
...
procedure TdxInternalRichEditDocumentServer.RemoveService(const AServiceType
...
function TdxInternalRichEditDocumentServer.GetService(const AServiceType
...
function TdxInternalRichEditDocumentServer.CreateNewDocument
  (ARaiseDocumentClosing: Boolean = False): Boolean;
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.LoadDocument(AStream: TStream;
...
procedure TdxInternalRichEditDocumentServer.SaveDocument(AStream: TStream;
  ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.LoadDocumentTemplate
  (AStream: TStream; ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerServer <> nil then
    InnerServer.LoadDocumentTemplate(AStream, ADocumentFormat);
end;

procedure TdxInternalRichEditDocumentServer.LoadDocument
...
procedure TdxInternalRichEditDocumentServer.LoadDocument(const AFileName
...
procedure TdxInternalRichEditDocumentServer.LoadDocumentTemplate(const AFileName
  : string; ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.SaveDocument(const AFileName
  : string; ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerServer <> nil then
...
function TdxInternalRichEditDocumentServer.CreateMailMergeOptions
...
procedure TdxInternalRichEditDocumentServer.MailMerge(const ADocument
  : IdxRichEditDocument);
begin
  if InnerServer <> nil then
...
procedure TdxInternalRichEditDocumentServer.MailMerge(const AOptions
...
procedure TdxInternalRichEditDocumentServer.MailMerge(const AFileName: string;
...
procedure TdxInternalRichEditDocumentServer.MailMerge(AStream: TStream;
  AFormat: TdxRichEditDocumentFormat);
begin
  if InnerServer <> nil then
