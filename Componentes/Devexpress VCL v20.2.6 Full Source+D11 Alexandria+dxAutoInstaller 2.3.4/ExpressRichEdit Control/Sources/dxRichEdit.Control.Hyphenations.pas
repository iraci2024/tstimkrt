...
type
  ...
  protected
    ...
    class function GetPatternStringLength(const APattern: string)
      : Integer; static;
    procedure ApplySubPatterns(const APattern: string;
      var APatternArray: TArray<Integer>);
    procedure AddPattern(const APattern: string);
    class function CreatePatternString(const APattern: string;
      var APatternArray: TArray<Integer>): string; static;
    class procedure ApplyPattern(var AValues: TArray<Integer>;
      const APattern: TArray<Integer>; APosition: Integer); static;
    class function IsOdd(ANumber: Integer): Boolean; static;
    procedure ApplyPatternToSubwords(AMaxSubwordLength, ASubwordsStartIndex
      : Integer);
    function GetHyphenPositions(const AWord: string; AOffset: Integer)
      : TdxIntegerList;
    function HyphenatePart(const AOriginalWord: string;
      AOffset, ALength: Integer): TdxIntegerList;
    function HyphenateCore(const AOriginalWord: string; AOffset: Integer)
      : TdxIntegerList;
  public
    constructor Create(const APatternFileName: string); overload;
    constructor Create(const APatternsFileName, AExceptionsFileName
      : string); overload;
...
constructor TdxHyphenator.Create(const APatternsFileName,
  AExceptionsFileName: string);
begin
  inherited Create;
...
class function TdxHyphenator.GetPatternStringLength(const APattern
  : string): Integer;
var
  ACount, I: Integer;
begin
  ...
  begin
    if not {$IFDEF DELPHIXE4}APattern[I].IsDigit{$ELSE}TCharacter.IsDigit
      (APattern[I]){$ENDIF} then
      Inc(Result);
  end;
end;

procedure TdxHyphenator.ApplySubPatterns(const APattern: string;
  var APatternArray: TArray<Integer>);
var
...
begin
  ...
  begin
    ...
    if FPatternTable.TryGetValue(APatternString, ASubPatternArray) and
      (ASubPatternArray <> nil) then
...
class function TdxHyphenator.CreatePatternString(const APattern: string;
  var APatternArray: TArray<Integer>): string;
var
...
begin
  ...
  try
    ...
    begin
      ANumericValue :=
        Trunc({$IFDEF DELPHIXE4}APattern[I].GetNumericValue{$ELSE}TCharacter.
        GetNumericValue(APattern[I]){$ENDIF});
...
class procedure TdxHyphenator.ApplyPattern(var AValues: TArray<Integer>;
  const APattern: TArray<Integer>; APosition: Integer);
var
  ALength, I: Integer;
begin
...
procedure TdxHyphenator.ApplyPatternToSubwords(AMaxSubwordLength,
  ASubwordsStartIndex: Integer);
var
  ASubStrLength: Integer;
  ASubword: string;
  APattern: TArray<Integer>;
begin
  AMaxSubwordLength := Min(AMaxSubwordLength, FExtendedWordLength -
    (ASubwordsStartIndex - 1));
...
function TdxHyphenator.GetHyphenPositions(const AWord: string; AOffset: Integer)
  : TdxIntegerList;
var
...
begin
  ...
  begin
    ACanAddHyphen := (IsOdd(FPatternCharsValues[I + 1]) and (I >= LeftHyphenMin)
      ) and (I <= AMaxHyphenPosition);
...
begin
  Result := TdxIntegerList.Create;
  AWord := {$IFDEF DELPHIXE4}AOriginalWord.ToLower{$ELSE}TCharacter.ToLower
    (AOriginalWord){$ENDIF};
  ALength := Length(AWord);
  AStartOfWord := 1;
  for I := 1 to ALength do
  begin
    if {$IFDEF DELPHIXE4}AWord[I].IsLetter{$ELSE}TCharacter.IsLetter(AWord[I]
      ){$ENDIF} then
      Continue;
...
function TdxHyphenator.HyphenatePart(const AOriginalWord: string;
  AOffset, ALength: Integer): TdxIntegerList;
begin
  Result := HyphenateCore(Copy(AOriginalWord, AOffset, ALength), AOffset - 1);
end;

function TdxHyphenator.HyphenateCore(const AOriginalWord: string;
  AOffset: Integer): TdxIntegerList;
var
...
begin
  ...
  ZeroMemory(@FPatternCharsValues[0], (FExtendedWordLength + 1) *
    SizeOf(Integer));
...
function TdxEmptyHyphenator.Hyphenate(const AOriginalWord: string)
  : TdxIntegerList;
begin
  Result := TdxIntegerList.Create;
end;
...
function TdxHyphenationService.Hyphenate(const AOriginalWord: string)
  : TdxIntegerList;
begin
