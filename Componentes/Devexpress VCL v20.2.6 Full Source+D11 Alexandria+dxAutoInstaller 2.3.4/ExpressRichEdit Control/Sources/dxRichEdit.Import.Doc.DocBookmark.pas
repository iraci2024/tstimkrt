...
type
  ...
  public
    constructor Create(AOriginalStartPosition: Integer;
      AOriginalEndPosition: Integer; const AName: string);
  ...
  public
    ...
    class function FromStream(AReader: TBinaryReader;
      ABookmarkIndexSize: Integer): TdxDocBookmarkFirstDescriptor; static;
    procedure Write(AWriter: TBinaryWriter; ABookmarkIndexSize: Integer);

    property BookmarkLimitDescriptorIndex: Integer
      read FBookmarkLimitDescriptorIndex;
  ...
  protected
    procedure Read(AReader: TBinaryReader; AOffset: Integer; ASize: Integer;
      ABookmarkIndexSize: Integer);
  public
    constructor Create;
    destructor Destroy; override;
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer; ABookmarkIndexSize: Integer)
      : TdxDocBookmarkFirstTable; static;
    procedure Write(AWriter: TBinaryWriter; ABookmarkIndexSize: Integer);
    procedure AddEntry(ACharacterPosition: Integer;
      AFirstDescriptor: TdxDocBookmarkFirstDescriptor);
    procedure Finish(ALastCharacterPosition: Integer);

    property CharacterPositions: TdxIntegerList read FCharacterPositions;
    property BookmarkFirstDescriptors
      : TdxObjectList<TdxDocBookmarkFirstDescriptor>
      read FBookmarkFirstDescriptors;
  end;
  ...
  public
    ...
    destructor Destroy; override;
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer): TdxDocBookmarkLimTable; static;
  ...
  public
    ...
    constructor Create(AFib: TdxFileInformationBlock; AReader: TBinaryReader);
      overload; virtual;
    destructor Destroy; override;
    procedure AdvanceNext(ALogPosition: TdxDocumentLogPosition;
      AOriginalPosition: Integer; ALength: Integer);
    procedure Finish(ALastCharacterPosition: Integer); virtual;
    procedure Write(AFib: TdxFileInformationBlock; AWriter: TBinaryWriter);
      virtual; abstract;
    procedure BeginEmbeddedContent(AStart: Integer; AEnd: Integer);
    procedure EndEmbeddedContent;

    property FirstTable: TdxDocBookmarkFirstTable read FFirstTable
      write FFirstTable;
  end;
  ...
  public
    constructor Create; overload;
    constructor Create(AFib: TdxFileInformationBlock; AReader: TBinaryReader;
      AKeepBookmarksForRemovedRanges: Boolean); reintroduce; overload;
    destructor Destroy; override;
...
function TDocBookmarkComparer.Compare(const Left,
  Right: TdxDocBookmark): Integer;
begin
...
constructor TdxDocBookmark.Create(AOriginalStartPosition: Integer;
  AOriginalEndPosition: Integer; const AName: string);
begin
...
constructor TdxDocBookmarkFirstDescriptor.Create(ABookmarkLimitDescriptorIndex
  : SmallInt = 0);
begin
  inherited Create;
...
class function TdxDocBookmarkFirstDescriptor.FromStream(AReader: TBinaryReader;
  ABookmarkIndexSize: Integer): TdxDocBookmarkFirstDescriptor;
begin
  Result := TdxDocBookmarkFirstDescriptor.Create;
  Result.Read(AReader, ABookmarkIndexSize);
end;

procedure TdxDocBookmarkFirstDescriptor.Read(AReader: TBinaryReader;
  ABookmarkIndexSize: Integer);
var
  ABitField: Byte;
begin
...
procedure TdxDocBookmarkFirstDescriptor.Write(AWriter: TBinaryWriter;
  ABookmarkIndexSize: Integer);
var
  ABitField: Byte;
begin
...
begin
  ...
  FBookmarkFirstDescriptors :=
    TdxObjectList<TdxDocBookmarkFirstDescriptor>.Create;
end;
...
class function TdxDocBookmarkFirstTable.FromStream(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer; ABookmarkIndexSize: Integer)
  : TdxDocBookmarkFirstTable;
begin
...
procedure TdxDocBookmarkFirstTable.Read(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer; ABookmarkIndexSize: Integer);
var
  ABookmarkSize, ACount, I: Integer;
begin
  if (ASize <= 0) or (AOffset < 0) or (AReader.BaseStream.Size - AOffset < ASize)
  then
    Exit;
  ...
  for I := 0 to ACount - 1 do
    FBookmarkFirstDescriptors.Add(TdxDocBookmarkFirstDescriptor.FromStream
      (AReader, ABookmarkIndexSize));
end;

procedure TdxDocBookmarkFirstTable.Write(AWriter: TBinaryWriter;
  ABookmarkIndexSize: Integer);
var
  ACount, I: Integer;
begin
...
procedure TdxDocBookmarkFirstTable.AddEntry(ACharacterPosition: Integer;
  AFirstDescriptor: TdxDocBookmarkFirstDescriptor);
begin
...
class function TdxDocBookmarkLimTable.FromStream(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer): TdxDocBookmarkLimTable;
begin
  Result := TdxDocBookmarkLimTable.Create;
  Result.Read(AReader, AOffset, ASize);
end;

procedure TdxDocBookmarkLimTable.Read(AReader: TBinaryReader; AOffset: Integer;
  ASize: Integer);
var
  ACount, I: Integer;
begin
...
constructor TdxDocBookmarkIteratorBase.Create(AFib: TdxFileInformationBlock;
...
procedure TdxDocBookmarkIteratorBase.AdvanceNext(ALogPosition
...
procedure TdxDocBookmarkIteratorBase.BeginEmbeddedContent(AStart: Integer;
  AEnd: Integer);
begin
...
constructor TdxDocBookmarkIterator.Create(AFib: TdxFileInformationBlock;
  AReader: TBinaryReader; AKeepBookmarksForRemovedRanges: Boolean);
begin
...
var
  ...
  ASkipDeletedPosition, AStartPositionObtainable,
...
begin
  ...
  try
    ...
    begin
      ...
      if not PositionConverter.ContainsPosition(ABookmark.OriginalStartPosition)
        or not PositionConverter.ContainsPosition(ABookmark.OriginalEndPosition)
      then
        Continue;
      AStartPositionObtainable := PositionConverter.TryConvert
        (ABookmark.OriginalStartPosition, ASkipDeletedPosition, AStartPosition);
      AEndPositionObtainable := PositionConverter.TryConvert
        (ABookmark.OriginalEndPosition, ASkipDeletedPosition, AEndPosition);
      if AStartPositionObtainable and AEndPositionObtainable then
      begin
        APieceTable.CreateBookmarkCore(AStartPosition,
          AEndPosition - AStartPosition, ABookmark.Name);
        AProcessedBookmarks.Add(ABookmark);
        Continue;
      end;
      if AStartPositionObtainable then
        PositionConverter.TryConvert(ABookmark.OriginalEndPosition, False,
          AEndPosition);
      if AEndPositionObtainable then
        PositionConverter.TryConvert(ABookmark.OriginalStartPosition, False,
          AStartPosition);
...
procedure TdxDocBookmarkIterator.RemoveProcessedBookmarks(AProcessedBookmarks
  : TdxDocBookmarks);
var
...
procedure TdxDocBookmarkIterator.Read(AFib: TdxFileInformationBlock;
  AReader: TBinaryReader);
begin
  FBookmarkNames := TdxDocStringTable.FromStream(AReader,
    AFib.BookmarkNamesTableOffset, AFib.BookmarkNamesTableSize);
  FirstTable := TdxDocBookmarkFirstTable.FromStream(AReader,
    AFib.BookmarkStartInfoOffset, AFib.BookmarkStartInfoSize,
    BookmarkIndexSize);
  LimTable := TdxDocBookmarkLimTable.FromStream(AReader,
    AFib.BookmarkEndInfoOffset, AFib.BookmarkEndInfoSize);
  InitBookmarks;
  InitConverter;
end;

procedure TdxDocBookmarkIterator.Write(AFib: TdxFileInformationBlock;
  AWriter: TBinaryWriter);
begin
  AFib.BookmarkNamesTableOffset := AWriter.BaseStream.Position;
  FBookmarkNames.Write(AWriter);
  AFib.BookmarkNamesTableSize := AWriter.BaseStream.Position -
    AFib.BookmarkNamesTableOffset;
  AFib.BookmarkStartInfoOffset := AWriter.BaseStream.Position;
  FirstTable.Write(AWriter, BookmarkIndexSize);
  AFib.BookmarkStartInfoSize := AWriter.BaseStream.Position -
    AFib.BookmarkStartInfoOffset;
  AFib.BookmarkEndInfoOffset := AWriter.BaseStream.Position;
  LimTable.Write(AWriter);
  AFib.BookmarkEndInfoSize := AWriter.BaseStream.Position -
    AFib.BookmarkEndInfoOffset;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if AEnd <> -1 then
        begin
          FBookmarks.Add(TdxDocBookmark.Create(AItem.Value.Key, AEnd,
            AItem.Key));
      ...
      begin
        ...
        AEndPositionIndex := AEndPositions.IndexOf
          (ABookmark.OriginalEndPosition);
        ...
        FirstTable.AddEntry(ABookmark.OriginalStartPosition,
          TdxDocBookmarkFirstDescriptor.Create(AEndPositionIndex + AOffset));
...
var
  ACount, I, ABookmarkStartPosition, AEndPositionIndex,
    ABookmarkEndPosition: Integer;
...
begin
  ...
  begin
    ...
    AEndPositionIndex := FirstTable.BookmarkFirstDescriptors[I]
      .BookmarkLimitDescriptorIndex;
    if AEndPositionIndex >= ACount then
      Continue;
    ABookmarkEndPosition := LimTable.CharacterPositions[AEndPositionIndex];
    FBookmarks.Add(TdxDocBookmark.Create(ABookmarkStartPosition,
      ABookmarkEndPosition, ABookmarkName));
  end;
end;

procedure TdxDocBookmarkIterator.AddBookmarkStart(const AName: string;
  AStartPosition: Integer);
begin
  if not FBookmarkDescriptions.ContainsKey(AName) then
    FBookmarkDescriptions.Add(AName,
      TdxIntegersPair.Create(AStartPosition, -1));
end;

procedure TdxDocBookmarkIterator.AddBookmarkEnd(const AName: string;
