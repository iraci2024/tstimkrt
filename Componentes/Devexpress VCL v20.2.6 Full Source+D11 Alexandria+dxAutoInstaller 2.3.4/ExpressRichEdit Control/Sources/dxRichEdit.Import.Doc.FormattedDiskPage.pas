...
type
  ...
  protected
    ...
    procedure WriteByteProperties(AWriter: TBinaryWriter;
      AStartPosition: Int64); virtual; abstract;
  ...
  protected
    ...
    procedure WriteByteProperties(AWriter: TBinaryWriter;
      AStartPosition: Int64); override;
  public
    constructor Create;
    destructor Destroy; override;
    class function FromStream(AReader: TBinaryReader; AOffset: Integer)
      : TdxPAPXFormattedDiskPage; static;
    function TryToAddGrpprlAndPosition(AFilePosition: Integer;
      AStyleIndex: Integer; const AGrpprl: TBytes): Boolean;
  end;
  ...
  protected
    ...
    property LastActiveInnerOffset: Integer read FLastActiveInnerOffset
      write FLastActiveInnerOffset;
  public
    constructor Create;
    destructor Destroy; override;
    class function FromStream(AReader: TBinaryReader; AOffset: Integer)
      : TdxCHPXFormattedDiskPage; static;
    function TryToAddGrpprlAndPosition(AFilePosition: Integer;
      const AGrpprl: TBytes): Boolean;
  end;
...
procedure TdxFormattedDiskPageBase.Read(AReader: TBinaryReader;
  AOffset: Integer);
begin
...
begin
  ...
  AWriter.BaseStream.Seek(AStartPosition +
    TdxDocConstants.LastByteOffsetInSector, TSeekOrigin.soBeginning);
...
class function TdxPAPXFormattedDiskPage.FromStream(AReader: TBinaryReader;
  AOffset: Integer): TdxPAPXFormattedDiskPage;
begin
...
function TdxPAPXFormattedDiskPage.TryToAddGrpprlAndPosition
  (AFilePosition: Integer; AStyleIndex: Integer; const AGrpprl: TBytes)
  : Boolean;
var
  APapxLength, AGrpprlOffset: Integer;
begin
  APapxLength := Length(AGrpprl) + SizeOfGrpprlSizeAndStyleDescriptor +
    StyleDescriptorSize;
  ...
  if AGrpprlOffset < (TdxDocConstants.CharacterPositionSize + BxSize) *
    (InnerOffsets.Count + 1) + TdxDocConstants.CharacterPositionSize then
    Exit(False);
...
procedure TdxPAPXFormattedDiskPage.WriteByteProperties(AWriter: TBinaryWriter;
  AStartPosition: Int64);
var
  ACount, I: Integer;
begin
  ...
  for I := 0 to ACount - 1 do
  begin
    AWriter.BaseStream.Seek(AStartPosition + InnerOffsets[I] * 2,
    ...
    begin
      ...
      AWriter.Write(Byte((Length(FGrppapx[I]) + StyleDescriptorSize +
        PadByteSize) div 2));
...
class function TdxCHPXFormattedDiskPage.FromStream(AReader: TBinaryReader;
  AOffset: Integer): TdxCHPXFormattedDiskPage;
begin
...
function TdxCHPXFormattedDiskPage.TryToAddGrpprlAndPosition
  (AFilePosition: Integer; const AGrpprl: TBytes): Boolean;
var
...
begin
  ...
  if LastActiveInnerOffset < (FilePositionSize + InnerOffsetSize) *
    (InnerOffsets.Count + 1) + LastFilePositionSize then
    Exit(False);
  FC.Add(AFilePosition);
  InnerOffsets.Add(Byte(AGrpprlOffset div 2));
...
procedure TdxCHPXFormattedDiskPage.WriteByteProperties(AWriter: TBinaryWriter;
  AStartPosition: Int64);
var
  ACount, I: Integer;
begin
