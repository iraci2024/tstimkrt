...
type
  ...
  TdxFieldType = (None, CreateDate, Date, EditTime, PrintDate, SaveDate, Time,
    Compare, DocVariable, GotoButton, &If, MacroButton, Print, Author, Comments,
    DocProperty, FileName, FileSize, Info, Keywords, LastSavedBy, NumChars,
    NumPages, NumWords, Subject, Template, Title, Formula, Advance, Eq, Symbol,
    Index, RD, TA, TC, TOA, TOC, XE, AutoText, AutoTextList, Bibliography,
    Citation, Hyperlink, IncludePicture, IncludeText, Link, NoteRef, PageRef,
    Quote, Ref, StyleRef, AddressBlock, Ask, DataBase, FillIn, GreetingLine,
    MergeField, MergeRec, MergeSeq, Next, NextIf, &Set, SkipIf, AutoNum,
    AutoNumLGL, AutoNumOut, BarCode, ListNum, Page, RevNum, Section,
    SectionPages, Seq, UserAddress, UserInitials, UserName, FormCheckbox,
    FormDropdown, FormText);
  ...
  public
    constructor Create(const ADisplayName: string;
      const ATargetFrameName: string); overload;
  ...
  TdxDocFieldSeparatorDescriptor = class(TInterfacedObject,
  ...
  public
    class function CreateInstance(AReader: TBinaryReader)
      : IdxDocFieldDescriptor; static;
  end;
...
constructor TdxDocFieldBinaryData.Create(AReader: TBinaryReader;
  AOffset: Integer);
begin
...
constructor TdxDocHyperlinkInfo.Create(const ADisplayName: string;
  const ATargetFrameName: string);
begin
  FHyperlinkObjectFlags := TdxDocHyperlinkObjectFlags.HasDisplayName and
    TdxDocHyperlinkObjectFlags.HasFrameName;
...
begin
  ...
  if (FHyperlinkObjectFlags and TdxDocHyperlinkObjectFlags.HasDisplayName)
    = TdxDocHyperlinkObjectFlags.HasDisplayName then
    FDisplayName := ReadHyperlinkString(AReader);
  if (FHyperlinkObjectFlags and TdxDocHyperlinkObjectFlags.HasFrameName)
    = TdxDocHyperlinkObjectFlags.HasFrameName then
...
begin
  Result := BaseSize + CalcHyperlinkStringLength(FDisplayName) +
    CalcHyperlinkStringLength(FTargetFrameName);
end;

function TdxDocHyperlinkInfo.ReadHyperlinkString
...
function TdxDocHyperlinkInfo.CalcHyperlinkStringLength(const AHyperLinkString
  : string): Integer;
begin
...
class function TdxDocFieldDescriptorFactory.CreateInstance
  (AReader: TBinaryReader): IdxDocFieldDescriptor;
var
...
begin
  ...
  case AFieldCode and $7 of
    $3:
      Result := TdxDocFieldBeginDescriptor.Create(AInfo);
    $4:
      Result := TdxDocFieldSeparatorDescriptor.Create;
    $5:
      Result := TdxDocFieldEndDescriptor.Create(AInfo);
  else
    TdxDocImporter.ThrowInvalidDocFile;
    Result := nil;
  end;
