...
type
  ...
  strict private
    class var FEmptyGenericFormatting: TArray<string>;
    class constructor Initialize;
  protected
    class function GetPageNumbering(AFormatter: TdxParagraphBoxFormatter)
      : TdxSectionPageNumbering; static;
    function GetGeneralFormatting: TArray<string>; override;
    function GetValueCore(AFormatter: TObject { TdxParagraphBoxFormatter };
      ADocumentModel: TdxCustomDocumentModel): Integer; override;
    function GetPageOrdinal(AFormatter: TdxParagraphBoxFormatter)
      : Integer; virtual;
  ...
  public
    ...
    function GetAllowedUpdateFieldTypes(const AOptions
      : TdxFieldUpdateOnLoadOptions): TdxUpdateFieldOperationTypes; override;
    ...
    function GetPageIndex(APieceTable: TdxCustomPieceTable;
      ALogPosition: TdxDocumentLogPosition): Integer; overload;
    function GetPageIndex(APieceTable: TdxCustomPieceTable;
      ADocumentLayout: TdxDocumentLayout; ALogPosition: TdxDocumentLogPosition)
      : Integer; overload;
  end;
...
function TdxPageFieldResultFormatting.
...
class function TdxPageFieldResultFormatting.GetPageNumbering
  (AFormatter: TdxParagraphBoxFormatter): TdxSectionPageNumbering;
begin
  Result := AFormatter.RowsController.ColumnController.PageAreaController.
    PageController.CurrentSection.PageNumbering;
end;

function TdxPageFieldResultFormatting.ApplyImplicitFormatting
...
begin
  ...
  AConverter := TdxOrdinalBasedNumberConverter.CreateConverter
    (APageNumbering.NumberingFormat, TdxLanguageId.English);
  try
...
function TdxPageFieldResultFormatting.GetPageOrdinal
...
begin
  if AFormatter.PageNumberSource <> nil then
    ...
    Exit(AFormatter.RowsController.ColumnController.PageAreaController.
...
function TdxPageField.GetCalculatedValueCore(ASourcePieceTable
...
begin
  ALogPosition := TdxDocumentModelPosition.FromRunStart
    (TdxPieceTable(ASourcePieceTable), ADocumentField.FirstRunIndex)
    .LogPosition;
  ...
  ATargetModel := TdxPieceTable(ASourcePieceTable)
    .DocumentModel.GetFieldResultModel;
  ATargetModel.BeginUpdate;
  try
    AFormatting := TdxPageFieldResultFormatting.Create(NumericFormatting,
      GeneralFormatting);
  ...
  Result := TdxCalculatedFieldValue.Create
    (TValue.From<TdxDocumentModel>(ATargetModel));
end;

function TdxPageField.GetPageIndex(APieceTable: TdxCustomPieceTable;
...
begin
  ADocumentLayoutService := APieceTable.DocumentModel.
    GetService<IdxDocumentLayoutService>;
...
begin
  ALayoutPosition := TdxDocumentLayoutPosition
    (ADocumentLayout.CreateLayoutPosition(APieceTable, ALogPosition, -1));
  try
    ALayoutPosition.Update(ADocumentLayout.Pages,
...
function TdxPageField.GetAllowedUpdateFieldTypes(const AOptions
  : TdxFieldUpdateOnLoadOptions): TdxUpdateFieldOperationTypes;
begin
...
procedure TdxPageField.ApplyFieldFormatting(AValue: TdxCalculatedFieldValue;
