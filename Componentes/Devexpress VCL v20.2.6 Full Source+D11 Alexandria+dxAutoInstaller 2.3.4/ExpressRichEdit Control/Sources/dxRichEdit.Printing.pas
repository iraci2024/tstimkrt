...
type
  ...
  public
    function BeginFormat(ADocumentModel: TdxDocumentModel): TdxGraphics;
      virtual; abstract;
  ...
  protected
    ...
    function Format(AGraphics: TdxGraphics; AMaxHeight: Integer)
      : Integer; overload;
    procedure SubscribeToFormattingControllerEvents; virtual;
    procedure UnsubscribeFromFormattingControllerEvents; virtual;
    function CreateDocumentLayout(AMeasurer: TdxBoxMeasurer)
      : TdxDocumentLayout; virtual;
    function CreateDocumentFormattingController(ADocumentLayout
      : TdxDocumentLayout): TdxDocumentFormattingController; virtual; abstract;
    function CreateDocumentPrinterController: TdxDocumentPrinterController;
      virtual; abstract;
    function CreateMeasurer(AGraphics: TdxGraphics): TdxBoxMeasurer;
      virtual; abstract;
    function PerformPrimaryFormat(AMaxHeight: Integer): Integer; virtual;
    procedure PerformFinalFormat; virtual;
    procedure PerformFinalFormatCore(AFinalFormatter
  ...
  protected
    ...
    function CreateDocumentFormattingController(ADocumentLayout
      : TdxDocumentLayout): TdxDocumentFormattingController; override;
    function CreateDocumentPrinterController
      : TdxDocumentPrinterController; override;
  end;
  ...
  protected
    function CreateDocumentFormattingController(ADocumentLayout
      : TdxDocumentLayout): TdxDocumentFormattingController; override;
    function CreateDocumentPrinterFormattingController(ADocumentLayout
    ...
    function CalculateFloatingObjectsBottom(AFloatingObjects
      : TdxFloatingObjectBoxList): Integer; overload;
    procedure Export(const AExporter: IdxDocumentLayoutExporter); override;
    procedure PerformFinalFormatCore(AFinalFormatter
      : TdxParagraphFinalFormatter); override;
  public
  ...
  public
    constructor Create(ATextBoxContentType: TdxContentTypeBase;
...
type
  ...
  TdxDocumentPrinterFormattingController = class
  ...
  public
    ...
    property ColumnLocation: TPoint read GetColumnLocation
      write SetColumnLocation;
    property ColumnSize: TSize read GetColumnSize write SetColumnSize;
  end;

  { TdxTextBoxDocumentPrinterFormattingController }

  TdxTextBoxDocumentPrinterFormattingController = class
    (TdxDocumentPrinterFormattingController)
  protected
    function CreatePageController: TdxPageController; override;
  end;
  ...
  public
    function BeginFormat(ADocumentModel: TdxDocumentModel)
      : TdxGraphics; override;
...
function TdxDocumentPrinter.Format(AGraphics: TdxGraphics;
...
function TdxDocumentPrinter.CreateDocumentLayout(AMeasurer: TdxBoxMeasurer)
  : TdxDocumentLayout;
begin
  Result := TdxDocumentLayout.Create(DocumentModel,
    TdxExplicitBoxMeasurerProvider.Create(AMeasurer) as IdxBoxMeasurerProvider);
end;
...
begin
  ...
  try
    ...
    AFormatter := TdxSimplePieceTablePrimaryFormatter.Create(PieceTable,
      FDocumentLayout.Measurer, Controller.RowsController,
      PieceTable.VisibleTextFilter, APage);
...
procedure TdxDocumentPrinter.PerformFinalFormatCore(AFinalFormatter
...
function TdxBrickDocumentPrinter.CreateMeasurer(AGraphics: TdxGraphics)
  : TdxBoxMeasurer;
begin
  Result := TdxGdiBoxMeasurer.Create(DocumentModel, AGraphics);
end;

function TdxBrickDocumentPrinter.CreateDocumentFormattingController
...
begin
  Result := TdxPrintLayoutViewDocumentFormattingController.Create
    (ADocumentLayout, PieceTable);
end;

function TdxBrickDocumentPrinter.CreateDocumentPrinterController
  : TdxDocumentPrinterController;
begin
...
function TdxPlatformDocumentPrinterController.BeginFormat(ADocumentModel
  : TdxDocumentModel): TdxGraphics;
begin
  FGraphics := TdxGraphics.Create;
  FModifier := TdxGraphicsToLayoutUnitsModifier.Create(FGraphics,
    ADocumentModel.LayoutUnitConverter);
  Result := FGraphics;
end;
...
function TdxPrintColumnController.GetNextColumnCore(AColumn: TdxColumn)
  : TdxColumn;
begin
  Result := TdxColumn.Create;
  if AColumn = nil then
    Result.Bounds := TRect.CreateSize(FColumnLocation.X, FColumnLocation.Y,
      FSize.Width, FSize.Height)
  else
    Result.Bounds := TRect.CreateSize(FColumnLocation.X,
      AColumn.Rows.Last.Bounds.Bottom, FSize.Width, FSize.Height);
end;

function TdxPrintColumnController.CalculateColumnBoundsCore
  (AColumnIndex: Integer): TRect;
begin
  Result := ColumnsBounds[AColumnIndex];
end;

function TdxPrintColumnController.GetCurrentPageBounds(ACurrentPage: TdxPage;
...
function TdxPrintColumnController.GetCurrentPageClientBounds
  (ACurrentPage: TdxPage; ACurrentColumn: TdxColumn): TRect;
var
  APageClientBounds: TRect;
begin
  APageClientBounds := inherited GetCurrentPageClientBounds(ACurrentPage,
    ACurrentColumn);
...
function TdxPrintPageController.CreatePageBoundsCalculator
...
function TdxPrintPageController.CreateHitTestCalculator(const ARequest
...
function TdxPrintPageController.CompleteCurrentPageFormatting
...
function TdxPrintPageController.GetNextPage(AKeepFloatingObjects
  : Boolean): TdxPage;
begin
  if Pages.Count > 0 then
    Result := Pages.Last
...
function TdxPrintPageAreController.CompleteCurrentAreaFormatting
  : TdxCompleteFormattingResult;
begin
...
function TdxPrintPageAreController.GetNextPageArea(AKeepFloatingObjects
  : Boolean): TdxPageArea;
begin
  if (PageController.Pages.Count > 0) and (Areas.Count > 0) then
    Exit(Areas.Last);
...
constructor TdxDocumentPrinterFormattingController.Create(ADocumentLayout
  : TdxDocumentLayout; APieceTable: TdxPieceTable);
begin
...
procedure TdxDocumentPrinterFormattingController.SetColumnLocation
  (const AValue: TPoint);
var
  AColumnController: TdxPrintColumnController;
begin
...
procedure TdxDocumentPrinterFormattingController.SetColumnSize
  (const AValue: TSize);
var
  AColumnController: TdxPrintColumnController;
begin
...
function TdxDocumentPrinterFormattingController.CreatePageController
  : TdxPageController;
begin
  Result := TdxPrintPageController.Create(DocumentLayout);
end;

function TdxDocumentPrinterFormattingController.CreateColumnController
  : TdxColumnController;
begin
  Result := TdxPrintColumnController.Create(PageAreaController);
end;

function TdxDocumentPrinterFormattingController.CreatePageAreaController
  : TdxPageAreaController;
begin
  Result := TdxPrintPageAreController.Create(PageController);
end;

function TdxDocumentPrinterFormattingController.CreateRowController
  : TdxRowsController;
begin
...
begin
  AFormattingController := TdxDocumentPrinterFormattingController
    (CreateDocumentPrinterFormattingController(ADocumentLayout, PieceTable));
...
function TdxSimpleDocumentPrinter.CreateDocumentPrinterFormattingController
  (ADocumentLayout: TdxDocumentLayout; APieceTable: TdxPieceTable)
  : TdxDocumentFormattingController;
begin
  Result := TdxDocumentPrinterFormattingController.Create(ADocumentLayout,
    APieceTable);
end;
...
begin
  ...
  if (((AColumnCount = 1) and (ARowsCount = 1)) and (ALastRow.Boxes.Count = 1))
    and (ALastRow.Boxes[0] is TdxParagraphMarkBox) then
    Exit(0);
...
function TdxSimpleDocumentPrinter.CalculateFloatingObjectsBottom
  (APage: TdxPage): Integer;
begin
  Result := 0;
  Result := Math.Max(CalculateFloatingObjectsBottom
    (APage.BackgroundFloatingObjects), Result);
  Result := Math.Max(CalculateFloatingObjectsBottom
    (APage.FloatingObjects), Result);
  Result := Math.Max(CalculateFloatingObjectsBottom
    (APage.ForegroundFloatingObjects), Result);
end;

function TdxSimpleDocumentPrinter.CalculateFloatingObjectsBottom
  (AFloatingObjects: TdxFloatingObjectBoxList): Integer;
var
  ACount, I: Integer;
begin
  Result := 0;
...
procedure TdxSimpleDocumentPrinter.Export(const AExporter
...
procedure TdxSimpleDocumentPrinter.PerformFinalFormatCore(AFinalFormatter
  : TdxParagraphFinalFormatter);
var
  I: Integer;
  AColumns: TdxColumnCollection;
...
constructor TdxTextBoxPrintPageController.Create(ADocumentLayout
  : TdxDocumentLayout; APieceTable: TdxPieceTable);
begin
...
function TdxTextBoxPrintPageController.AppendFloatingObjectsToPage
...
function TdxTextBoxDocumentPrinterFormattingController.CreatePageController
  : TdxPageController;
begin
...
constructor TdxTextBoxFloatingObjectContentPrinter.Create(ATextBoxContentType
  : TdxContentTypeBase; APageNumberSource: TdxPage; AMeasurer: TdxBoxMeasurer);
begin
...
function TdxTextBoxFloatingObjectContentPrinter.Format
  (AMaxHeight: Integer): Integer;
var
...
function TdxTextBoxFloatingObjectContentPrinter.CreateDocumentPrinterController
  : TdxDocumentPrinterController;
begin
  Result := nil;
end;

function TdxTextBoxFloatingObjectContentPrinter.
  CreateDocumentPrinterFormattingController(ADocumentLayout: TdxDocumentLayout;
  APieceTable: TdxPieceTable): TdxDocumentFormattingController;
begin
  Result := TdxTextBoxDocumentPrinterFormattingController.Create
    (ADocumentLayout, APieceTable);
end;

function TdxTextBoxFloatingObjectContentPrinter.CreateMeasurer(AGr: TdxGraphics)
  : TdxBoxMeasurer;
begin
  Result := FMeasurer;
end;
...
constructor TdxExtensionCommentInfo.Create(AComment: TdxComment;
