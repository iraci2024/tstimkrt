...
{$ENDIF}
  ...
  Types,
  Printers, SysUtils, Generics.Defaults, Generics.Collections, dxCoreClasses,
...
type
  ...
  TdxScreenCharacterLinePainterParameters = class
  ...
  protected
    procedure AddItem(AUnderlineType: TdxUnderlineType;
      const ADescription: string = '');
    class function CalculateCenteredUnderlineBounds(AUnderline: TdxUnderline;
      const ABounds: TRect; AThickness: Integer): TdxRectF; static;
    procedure DrawUnderlineItem(AControl: TcxCustomComboBox; ACanvas: TcxCanvas;
      AIndex: Integer; const ARect: TRect; AState: TOwnerDrawState);
    class function GetUnderlineTypeName(AUnderlineType: TdxUnderlineType)
      : string; static;
    class function GetUnderlineThickness(AUnderline: TdxUnderline)
      : Integer; static;
    procedure InitItems;
  ...
  public
    ...
    class procedure DrawUnderlineItemCore(AUnderline: TdxUnderline;
      ACanvas: TcxCanvas; const ABounds: TRect; AForeColor: TdxAlphaColor;
      AThickness, AUnderlineBoxHeight: Integer); static;
    procedure Subscribe(AControl: TcxCustomComboBox);
    property UnderlineRepository: TdxUnderlineRepository
      read FUnderlineRepository write SetUnderlineRepository;
    property UnderlineType: TdxNullableUnderlineType read GetUnderlineType
...
function TdxScreenCharacterLinePainterParameters.PixelsToUnits(AValue: Single;
  ADpi: Single): Single;
begin
  Result := AValue;
end;
...
function TdxScreenCharacterLinePainter.CreateParameters
  : TdxPatternLinePainterParameters;
begin
...
function TdxScreenCharacterLinePainter.GetParametersTable
  : TdxPatternLinePainterParametersTable;
begin
  Result := FParametersTable;
end;

procedure TdxScreenCharacterLinePainter.InitializeParameters
  (AParameters: TdxPatternLinePainterParameters);
begin
  AParameters.Initialize(PixelGraphics.DpiX);
end;

function TdxScreenCharacterLinePainter.PixelsToUnits(AVal,
  ADpi: Single): Single;
begin
  Result := AVal;
end;

function TdxScreenCharacterLinePainter.UnitsToPixels(AVal,
  ADpi: Single): Single;
begin
  Result := AVal;
end;
...
begin
  if FControl.ItemIndex >= 0 then
    Result := TdxUnderline(FControl.Properties.Items.Objects
      [FControl.ItemIndex]).Id
  else
    Result := TdxNullableUnderlineType.Null;
end;

class function TdxUnderlinePainterHelper.GetUnderlineTypeName
  (AUnderlineType: TdxUnderlineType): string;
var
  AInfo: PTypeInfo;
begin
...
class function TdxUnderlinePainterHelper.GetUnderlineThickness
  (AUnderline: TdxUnderline): Integer;
begin
...
procedure TdxUnderlinePainterHelper.AddItem(AUnderlineType: TdxUnderlineType;
  const ADescription: string);
begin
  if ADescription = '' then
    Items.AddObject(GetUnderlineTypeName(AUnderlineType),
      UnderlineRepository.GetPatternLineByType(AUnderlineType))
  else
    Items.AddObject(ADescription, UnderlineRepository.GetPatternLineByType
      (AUnderlineType));
end;

class function TdxUnderlinePainterHelper.CalcClipBounds(const ABounds: TRect)
  : TdxRectF;
var
  ARect: TRect;
begin
...
class function TdxUnderlinePainterHelper.CalculateCenteredUnderlineBounds
  (AUnderline: TdxUnderline; const ABounds: TRect; AThickness: Integer)
  : TdxRectF;
var
  ARect: TRect;
  Ady: Single;
...
procedure TdxUnderlinePainterHelper.DrawUnderlineItem
...
begin
  ...
  AItemUnderlineObj := AControl.Properties.Items.Objects[AIndex]
    as TdxUnderline;
  if AItemUnderlineObj.Id <> TdxUnderlineType.None then
    DrawUnderlineItemCore(AItemUnderlineObj, ACanvas, ARect,
      TdxAlphaColors.FromColor(clWindowText),
      GetUnderlineThickness(AItemUnderlineObj), UnderlineBoxHeight)
  else
  begin
...
class procedure TdxUnderlinePainterHelper.DrawUnderlineItemCore
  (AUnderline: TdxUnderline; ACanvas: TcxCanvas; const ABounds: TRect;
  AForeColor: TdxAlphaColor; AThickness, AUnderlineBoxHeight: Integer);
var
...
begin
  ...
  try
    ...
    try
      ...
      ActualUnderlineBounds := CalculateCenteredUnderlineBounds(AUnderline,
        ABounds, AThickness);
      AEmptyDocumentLayoutUnitConverter :=
        TdxEmptyDocumentLayoutUnitConverter.Create;
      ALinePainter := TdxScreenCharacterLinePainter.Create(APainter,
        AEmptyDocumentLayoutUnitConverter);
      try
...
begin
  AddItem(TdxUnderlineType.None,
    cxGetResourceString(@sdxRichEditFontDialogUnderlineStyleNone));
...
procedure TdxUnderlinePainterHelper.SetUnderlineRepository
  (const Value: TdxUnderlineRepository);
begin
...
procedure TdxUnderlinePainterHelper.SetUnderlineType
...
begin
  ...
  begin
    ...
    while AIndex >= 0 do
    begin
      if TdxUnderline(FControl.Properties.Items.Objects[AIndex]).Id = Value.Value
      then
        Break;
      Dec(AIndex);
    end;
