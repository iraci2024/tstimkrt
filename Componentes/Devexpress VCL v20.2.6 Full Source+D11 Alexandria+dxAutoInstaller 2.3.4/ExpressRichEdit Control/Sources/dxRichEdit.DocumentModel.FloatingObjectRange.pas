...
type
  ...
  protected
    ...
    function CalculateAnchorPieceTableStartRunIndex(ARunIndex: TdxRunIndex)
      : TdxRunIndex;
    function CalculateAnchorPieceTableEndRunIndex(ARunIndex: TdxRunIndex)
      : TdxRunIndex;
  public
    ...
    procedure ApplyChanges(AChangeType: TdxDocumentModelChangeType;
      AStartRunIndex: TdxRunIndex; AEndRunIndex: TdxRunIndex); override; final;
    ...
    function LookupSectionIndexByParagraphIndex(AParagraphIndex
      : TdxParagraphIndex): TdxSectionIndex; override; final;
  ...
  public
    ...
    function Clone(const ARun: TdxTextRunBase;
      ACopyManager: TdxCustomDocumentModelCopyManager)
      : TdxCustomObjectContent; override;
  end;

  { TdxTextBoxFloatingObjectContent }

  TdxTextBoxFloatingObjectContent = class(TdxFloatingObjectContent,
    IdxTextBoxPropertiesContainer) // for internal use
  strict private
  ...
  protected
    ...
    function IdxTextBoxPropertiesContainer.GetPieceTable =
      GetTextBoxPropertiesContainerPieceTable;
    function GetTextBoxPropertiesContainerPieceTable: TdxCustomPieceTable;
  public
    constructor Create(AAnchor: TdxFloatingObjectAnchorRun;
      AContent: TdxTextBoxContentType);
    destructor Destroy; override;

    function Clone(const ARun: TdxTextRunBase;
      ACopyManager: TdxCustomDocumentModelCopyManager)
      : TdxCustomObjectContent; override;
    function GetInsertOptions(AFormattingCopyOptions: TdxFormattingCopyOptions)
  ...
  TdxExplicitFloatingObjectLocation = class(TInterfacedObject,
    IdxFloatingObjectLocation) // for internal use
  strict private
  ...
  TdxFloatingObject = class(TcxIUnknownObject,
    IdxFloatingObjectPropertiesContainer, IdxShapeContainer) // for internal use
  strict private
    FAnchorRun: TdxFloatingObjectAnchorRun;
  ...
  public
    ...
    procedure CopyFrom(AFloatingObject: TdxFloatingObject;
      ACopyManager: TdxCustomDocumentModelCopyManager); virtual;
  ...
  TdxFloatingObjectAnchorRun = class(TdxTextRunBase, IdxPictureContainerRun,
    IdxRectangularObject, IdxRectangularScalableObject)
  strict private
  ...
  public
    ...
    function Copy(ACopyManager: TdxCustomDocumentModelCopyManager)
      : TdxTextRunBase; override;
    function CanJoinWith(ANextRun: TdxTextRunBase): Boolean; override;
    procedure Measure(ABoxInfo: TdxBoxInfo;
      const AMeasurer: IdxObjectMeasurer); override;
    procedure SetContent(AContent: TdxCustomObjectContent);
    function TryAdjustEndPositionToFit(ABoxInfo: TdxBoxInfo; AMaxWidth: Integer;
      const AMeasurer: IdxObjectMeasurer): Boolean; override;
...
{$REGION 'interfaces'}
    ...
    function IdxPictureContainerRun.GetPictureContent =
      PictureContainerRunGetPictureContent;
    function PictureContainerRunGetPictureContent
...
{$ENDREGION}
    property PictureContent: TdxPictureFloatingObjectContent
      read GetPictureContent;
    function CanPlaceCaretBefore: Boolean; override;

    property FloatingObjectProperties: TdxFloatingObjectProperties
      read GetFloatingObjectProperties;
    ...
    property ExcludeFromLayout: Boolean read FExcludeFromLayout
      write FExcludeFromLayout;
  end;
...
constructor TdxTextBoxContentType.Create(ADocumentModel
  : TdxSimpleDocumentModel);
begin
  inherited Create(ADocumentModel);
  TdxDocumentModel(ADocumentModel).UnsafeEditor.InsertFirstParagraph
    (PieceTable);
  TdxPieceTable(PieceTable).SpellCheckerManager :=
    TdxPieceTable(ADocumentModel.MainPieceTable)
    .SpellCheckerManager.CreateInstance(TdxPieceTable(PieceTable));
...
function TdxTextBoxContentType.CalculateAnchorPieceTableStartRunIndex
  (ARunIndex: TdxRunIndex): TdxRunIndex;
begin
  Result := AnchorRun.Paragraph.FirstRunIndex;
end;

function TdxTextBoxContentType.CalculateAnchorPieceTableEndRunIndex
  (ARunIndex: TdxRunIndex): TdxRunIndex;
begin
  Result := AnchorRun.Paragraph.LastRunIndex;
end;

function TdxTextBoxContentType.LookupSectionIndexByParagraphIndex
  (AParagraphIndex: TdxParagraphIndex): TdxSectionIndex;
begin
  Result := dxSectionIndexDontCare;
end;

procedure TdxTextBoxContentType.ApplyChanges(AChangeType
  : TdxDocumentModelChangeType; AStartRunIndex: TdxRunIndex;
  AEndRunIndex: TdxRunIndex);
begin
  if not IsReferenced then
    inherited ApplyChanges(AChangeType, AStartRunIndex, AEndRunIndex)
  else
    AnchorRun.PieceTable.ApplyChanges(AChangeType,
      CalculateAnchorPieceTableStartRunIndex(AStartRunIndex),
      CalculateAnchorPieceTableEndRunIndex(AEndRunIndex));
end;

procedure TdxTextBoxContentType.ApplyChangesCore
...
begin
  ...
  inherited ApplyChangesCore(AActions, CalculateAnchorPieceTableStartRunIndex
    (AStartRunIndex), CalculateAnchorPieceTableEndRunIndex(AEndRunIndex));
end;

procedure TdxTextBoxContentType.FixLastParagraphOfLastSection
...
constructor TdxTextBoxFloatingObjectContent.Create
  (AAnchor: TdxFloatingObjectAnchorRun; AContent: TdxTextBoxContentType);
begin
...
procedure TdxTextBoxFloatingObjectContent.SetAnchorRun
  (AAnchor: TdxFloatingObjectAnchorRun);
begin
...
begin
  ...
  try
    ...
    try
      ...
      ATempDocumentModel.FieldOptions.CopyFrom
        (TdxDocumentModel(ARun.DocumentModel).FieldOptions);
      ATextBoxPieceTable := TdxPieceTable(TextBox.PieceTable);
      AOptions := TdxDocumentModelCopyOptions.Create
        (ATextBoxPieceTable.DocumentStartLogPosition,
        ATextBoxPieceTable.DocumentEndLogPosition -
        ATextBoxPieceTable.DocumentStartLogPosition + 1);
      try
        AOptions.DefaultPropertiesCopyOptions :=
          TdxDefaultPropertiesCopyOptions.Always;
        ACopyCommand := TdxDocumentModelCopyCommand.Create(ATextBoxPieceTable,
          ATempDocumentModel, AOptions);
        try
    ...
    ACommand := TdxPieceTableInsertContentConvertedToDocumentModelCommand.Create
      (TdxPieceTable(AContentTextBox.PieceTable),
  ...
  Result := TdxTextBoxFloatingObjectContent.Create
    (ARun as TdxFloatingObjectAnchorRun, AContentTextBox);
  TdxTextBoxFloatingObjectContent(Result).TextBoxProperties.CopyFrom
    (TextBoxProperties.Info);
end;

function TdxTextBoxFloatingObjectContent.GetInsertOptions(AFormattingCopyOptions
  : TdxFormattingCopyOptions): TdxInsertOptions;
begin
...
function TdxTextBoxFloatingObjectContent.GetTextBoxPropertiesContainerPieceTable
  : TdxCustomPieceTable;
begin
  Result := PieceTable;
end;

function TdxTextBoxFloatingObjectContent.CreateTextBoxChangedHistoryItem
  : TdxIndexChangedHistoryItemCore;
begin
  Result := TdxIndexChangedHistoryItem.Create(Run.PieceTable,
    TextBoxProperties);
end;
...
function TdxExplicitFloatingObjectLocation.GetHorizontalPositionAlignment
  : TdxFloatingObjectHorizontalPositionAlignment;
begin
  Result := FHorizontalPositionAlignment;
end;

function TdxExplicitFloatingObjectLocation.GetHorizontalPositionType
...
function TdxExplicitFloatingObjectLocation.GetRelativeHeight
  : TdxFloatingObjectRelativeHeight;
begin
  Result := FRelativeHeight;
end;

function TdxExplicitFloatingObjectLocation.GetRelativeWidth
  : TdxFloatingObjectRelativeWidth;
begin
  Result := FRelativeWidth;
end;

function TdxExplicitFloatingObjectLocation.GetTextWrapType
...
function TdxExplicitFloatingObjectLocation.GetVerticalPositionAlignment
  : TdxFloatingObjectVerticalPositionAlignment;
begin
  Result := FVerticalPositionAlignment;
end;

function TdxExplicitFloatingObjectLocation.GetVerticalPositionType
...
procedure TdxFloatingObject.CopyFrom(AFloatingObject: TdxFloatingObject;
  ACopyManager: TdxCustomDocumentModelCopyManager);
var
  ...
  begin
    if TdxDocumentModel(DocumentModel).ModelForExport then
      TdxPieceTable(ATextBoxContent.TextBox.PieceTable)
        .FieldUpdater.UpdateFields
        (TdxUpdateFieldOperationType.CreateModelForExport)
    else
      TdxPieceTable(ATextBoxContent.TextBox.PieceTable)
        .FieldUpdater.UpdateFields(TdxUpdateFieldOperationType.Copy);
  end;
end;
...
procedure TdxFloatingObject.AssignTextBoxAnchorRun
...
function TdxFloatingObject.CreateFloatingObjectPropertiesChangedHistoryItem
  : TdxIndexChangedHistoryItemCore;
begin
  Result := TdxIndexChangedHistoryItem.Create(PieceTable, Properties);
end;

function TdxFloatingObject.CreateShapeChangedHistoryItem
  : TdxIndexChangedHistoryItemCore;
begin
...
begin
  ...
  begin
    AHeaderFooter := Safe<TdxSectionHeaderFooterBase>.Cast
      (PieceTable.ContentType);
    TdxDocumentModel(DocumentModel).SetActivePieceTable
      (TdxPieceTable(PieceTable), AHeaderFooter.GetSection);
  end
  else
...
function TdxFloatingObject.IsTextBoxActive
  (ATextBox: TdxTextBoxContentType): Boolean;
begin
...
procedure TdxFloatingObject.OnObtainAffectedRange(ASender: TObject;
  E: TdxObtainAffectedRangeEventArgs);
begin
...
begin
  if (FContent is TdxTextBoxFloatingObjectContent) and
    (TdxTextBoxFloatingObjectContent(FContent).TextBox <> nil) then
    TdxPieceTable(PieceTable).TextBoxes.Add
      (TdxTextBoxFloatingObjectContent(FContent).TextBox);
end;
...
begin
  ...
  AAnchorPosition := TdxDocumentModelPosition.FromRunStart(PieceTable,
    ARunIndex);
...
function TdxFloatingObjectAnchorRun.Copy(ACopyManager
  : TdxCustomDocumentModelCopyManager): TdxTextRunBase;
var
...
begin
  ...
  if not ACopyManager.TargetModel.DocumentCapabilities.FloatingObjectsAllowed
  then
  begin
  ...
  try
    ARun := ATargetPieceTable.InsertFloatingObjectAnchorCore
      (ATargetPosition.ParagraphIndex, ATargetPosition.LogPosition);
...
function TdxFloatingObjectAnchorRun.CanJoinWith
  (ANextRun: TdxTextRunBase): Boolean;
begin
  Result := False;
end;
...
constructor TdxFloatingObjectAnchorRun.Create(AParagraph: TdxParagraphBase;
...
function TdxFloatingObjectAnchorRun.GetBatchUpdateChangeActions
...
function TdxFloatingObjectAnchorRun.TryAdjustEndPositionToFit
  (ABoxInfo: TdxBoxInfo; AMaxWidth: Integer;
  const AMeasurer: IdxObjectMeasurer): Boolean;
begin
  Result := False;
end;
...
function TdxFloatingObjectAnchorRun.GetFloatingObjectProperties
...
function TdxFloatingObjectAnchorRun.GetPictureContent
  : TdxPictureFloatingObjectContent;
begin
...
function TdxFloatingObjectAnchorRun.PictureContainerRunGetPictureContent
  : TdxPictureFloatingObjectContent;
begin
...
begin
  if PictureContent = nil then
    ...
    Result := 100 * FloatingObjectProperties.ActualSize.Width /
      Max(1, Content.OriginalSize.Width);
end;
...
begin
  if PictureContent = nil then
    ...
    Result := 100 * FloatingObjectProperties.ActualSize.Height /
      Max(1, Content.OriginalSize.Height);
end;
...
procedure TdxFloatingObjectAnchorRun.Measure(ABoxInfo: TdxBoxInfo;
...
procedure TdxFloatingObjectAnchorRun.SetActualSizeInternal
  (const AActualSize: TSize);
begin
  ActualSize := AActualSize;
end;

procedure TdxFloatingObjectAnchorRun.SetContent
...
begin
  ...
  if FloatingObjectProperties.ActualSize.Width = AVal then
    Exit;
  FloatingObjectProperties.ActualSize :=
    cxSize(AVal, FloatingObjectProperties.ActualSize.Height);
end;
...
begin
  ...
  if FloatingObjectProperties.ActualSize.Height = AVal then
    Exit;
  FloatingObjectProperties.ActualSize :=
    cxSize(FloatingObjectProperties.ActualSize.Width, AVal);
end;
