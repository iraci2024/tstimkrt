...
uses
  ...
  RegularExpressions,
  dxCore, dxCoreClasses, dxCoreGraphics, dxCultureInfo, dxGDIPlusAPI,
  dxGDIPlusClasses,
...
{$ENDIF}
  private
    ...
    class function GetTypeFromString(const AValue: string)
      : TdxWebUnitType; static;
  ...
  public
    ...
    constructor Create(const AValue: string;
      const ACulture: TdxCultureInfo); overload;
    constructor Create(const AValue: string; const ACulture: TdxCultureInfo;
      ADefaultType: TdxWebUnitType); overload;
    ...
    class function Parse(const S: string; const ACulture: TdxCultureInfo)
      : TdxWebUnit; overload; static;
    ...
    class property Empty: TdxWebUnit read
    {$IFDEF DELPHIXE3}FEmpty{$ELSE}GetEmpty{$ENDIF};
  end;
...
{$ENDIF}
  ...
  public
    ...
    class function Parse(const S: string; const ACulture: TdxCultureInfo)
      : TdxWebFontUnit; overload; static;
    ...
    class property Empty: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FEmpty{$ELSE}GetEmpty{$ENDIF};
    class property Smaller: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FSmaller{$ELSE}GetSmaller{$ENDIF};
    class property Larger: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FLarger{$ELSE}GetLarger{$ENDIF};
    class property XXSmall: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FXXSmall{$ELSE}GetXXSmall{$ENDIF};
    class property XSmall: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FXSmall{$ELSE}GetXSmall{$ENDIF};
    class property Small: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FSmall{$ELSE}GetSmall{$ENDIF};
    class property Medium: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FMedium{$ELSE}GetMedium{$ENDIF};
    class property Large: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FLarge{$ELSE}GetLarge{$ENDIF};
    class property XLarge: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FXLarge{$ELSE}GetXLarge{$ENDIF};
    class property XXLarge: TdxWebFontUnit read
    {$IFDEF DELPHIXE3}FXXLarge{$ELSE}GetXXLarge{$ENDIF};
  end;
  ...
  protected
    ...
    function FindControl(const AId: string; APathOffset: Integer)
      : TdxWebControlBase; overload; virtual;
    ...
    function OnBubbleEvent(ASource: TObject; AArgs: TdxEventArgs)
      : Boolean; virtual;
    procedure AddedControl(AControl: TdxWebControlBase;
    ...
    procedure FillNamedControlsTable(ANamingContainer: TdxWebControlBase;
      AControls: TdxWebControlCollection);
    ...
    property ChildControlsCreated: Boolean read GetChildControlsCreated
      write SetChildControlsCreated;
    property Flags: TdxSimpleBitVector32 read FFlags;
    property IdSeparator: Char read GetIdSeparator;
    property IsChildControlStateCleared: Boolean
      read GetIsChildControlStateCleared;
  ...
  public
    ...
    function FindControl(const AId: string): TdxWebControlBase;
      overload; virtual;
    ...
    procedure RenderChildrenInternal(AWriter: TdxHtmlTextWriter;
      const AChildren: TdxWebControlCollection);
    ...
    property EnableTheming: Boolean read GetEnableTheming
      write SetEnableTheming;
  ...
  protected const
    StyleKey = 'style';
  strict private
    class var FStyleAttribRegex: TRegex;
    class constructor Initialize;
  strict private
  ...
  public
    ...
    property Items[const AKey: string]: TdxNullableString read GetItem
  ...
  public
    ...
    property BorderStyle: TdxWebBorderStyle read GetBorderStyle
...
{$REGION 'private const'}
    BorderStyles: array [0 .. 9] of string = ('NotSet', 'None', 'Dotted',
      'Dashed', 'Solid', 'Double', 'Groove', 'Ridge', 'Inset', 'Outset');
    CssClassStr = 'class';
{$ENDREGION}
  public type
{$REGION 'public type'}
    TAssignedValue = (BackColor, BorderColor, BorderStyle, BorderWidth,
      CssClass, ForeColor, Height, Width, FontBold, FontItalic, FontNames,
      FontOverline, FontSize, FontStrikeout, FontUnderline);
...
{$ENDREGION}
  strict private
    ...
    class function FormatStringArray(const AArray: TArray<string>;
      ADelimiter: Char): string; static;
  ...
  public
    ...
    procedure AddAttributesToRender(AWriter: TdxHtmlTextWriter;
      AOwner: TdxHtmlControl = nil); virtual;
  ...
  public
    class function GetHtmlStyle(AFont: TdxGPFont;
      AForeColor, ABackColor: TdxAlphaColor): string; overload; static;
    class function GetHtmlStyle(const AFontFamilyName: string; ASize: Single;
      AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout, AUnderline: Boolean;
      AForeColor, ABackColor: TdxAlphaColor): string; overload; static;
    class function GetHtmlStyle(const AFontFamilyName: string; ASize: Single;
      AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout, AUnderline: Boolean;
      AForeColor, ABackColor: TdxAlphaColor; AUseFontSizeInPixels: Boolean)
      : string; overload; static;
    class procedure GetHtmlStyle(AFont: TdxGPFont;
      AForeColor, ABackColor: TdxAlphaColor; AStyle: TdxCssStyleCollection);
      overload; static;
    class procedure GetHtmlStyle(const AFontFamilyName: string; ASize: Single;
      AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout, AUnderline: Boolean;
      AForeColor, ABackColor: TdxAlphaColor; AStyle: TdxCssStyleCollection);
      overload; static;
    class procedure GetHtmlStyle(const AFontFamilyName: string; ASize: Single;
    ...
    class function GetHtmlStyle(AFont: TdxGPFont; AForeColor: TdxAlphaColor)
      : string; overload; static;
    ...
    class function GetFontHtml(const AFontFamilyName: string; ASize: Single;
      AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout, AUnderline: Boolean;
      AInPixels: Boolean): string; overload; static;
    class function GetFontHtml(const AFontFamilyName: string; ASize: Single;
      AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout, AUnderline: Boolean)
      : string; overload; static;
    class procedure GetFontHtml(AFont: TdxGPFont;
      AStyle: TdxCssStyleCollection); overload; static;
    class procedure GetFontHtml(const AFontFamilyName: string; ASize: Single;
      AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout, AUnderline: Boolean;
      AInPixels: Boolean; AStyle: TdxCssStyleCollection); overload; static;
    class function GetCorrectedFamilyName(const AFamilyName: string)
      : string; static;
    class function GetFontWeight(AIsBold: Boolean): string; static;
    class function GetFontStyle(AIsItalic: Boolean): string; static;
    class function GetTextDecoration(AStrikeout, AUnderline: Boolean)
      : string; static;
    class function GetTextDecorationValue(AStrikeout, AUnderline: Boolean)
      : string; static;
  end;
...
{$ENDREGION}
  ...
  class var
    ...
    FAttributeNameLookup: array [TdxHtmlTextWriterStyle]
      of TAttributeInformation;
    class constructor Initialize;
    class destructor Finalize;
  strict private
    class procedure WriteUrlAttribute(AWriter: TTextWriter;
      const AUrl: string); static;
    class procedure WriteAttribute(AWriter: TTextWriter;
      AKey: TdxHtmlTextWriterStyle; const AName, AValue: string);
      overload; static;
  protected
    class procedure WriteAttributes(AWriter: TTextWriter;
      AStyles: TList<TdxWebRenderStyle>); static;
  public
    FWriter: TTextWriter;
    constructor Create(AWriter: TTextWriter);
    procedure Close; override;
    procedure Flush; override;
    class function GetStyleKey(const AStyleName: string)
      : TdxHtmlTextWriterStyle; static;
    class function GetStyleName(AStyleKey: TdxHtmlTextWriterStyle)
      : string; static;
    class function IsStyleEncoded(AStyleKey: TdxHtmlTextWriterStyle)
      : Boolean; static;
    class procedure RegisterAttribute(const AName: string;
      AKey: TdxHtmlTextWriterStyle; AEncode: Boolean = False;
      AIsUrl: Boolean = False); static;
    ...
    procedure Write(const ABuffer: TArray<Char>; AIndex: Integer;
      ACount: Integer); override;
    procedure WriteAttribute(const AName: string;
      const AValue: string); overload;
    procedure WriteAttribute(AKey: TdxHtmlTextWriterStyle;
      const AValue: string); overload;
    ...
    procedure WriteLine(const ABuffer: TArray<Char>; AIndex: Integer;
      ACount: Integer); override;
  end;
...
{$REGION 'class types'}
  protected type
    ...
    TTagInformation = record
      ...
      TagType: TTagType;
      ClosingTag: string;
      constructor Create(const AName: string; ATagType: TTagType;
...
{$ENDREGION}
  ...
  class var
    ...
    FAttributeNameLookup: array [TdxHtmlTextWriterAttribute]
      of TAttributeInformation;
  ...
  strict private
    ...
    FTabString: string;
    FWriter: TTextWriter;
    procedure OpenDiv(ALayout: TLayout; AWriteHorizontalAlign: Boolean;
      AWriteWrapping: Boolean); overload;
  ...
  protected
    procedure AddAttribute(const AName: string; const AValue: TdxNullableString;
      AKey: TdxHtmlTextWriterAttribute; AEncode: Boolean = False;
      AIsUrl: Boolean = False); overload; virtual;
    procedure AddStyleAttribute(const AName: string; const AValue: string;
      AKey: TdxHtmlTextWriterStyle); overload; virtual;
    function EncodeAttributeValue(const AValue: TdxNullableString;
      AEncode: Boolean): TdxNullableString; overload;
    function EncodeAttributeValue(AAttrKey: TdxHtmlTextWriterAttribute;
      const AValue: TdxNullableString): TdxNullableString; overload; virtual;
    function EncodeUrl(const AUrl: string): string;
    procedure FilterAttributes; virtual;
    function GetAttributeKey(const AAttrName: string)
    ...
    function IsAttributeDefined(AKey: TdxHtmlTextWriterAttribute)
      : Boolean; overload;
    function IsAttributeDefined(AKey: TdxHtmlTextWriterAttribute;
      out AValue: string): Boolean; overload;
    function IsStyleAttributeDefined(AKey: TdxHtmlTextWriterStyle)
      : Boolean; overload;
    function IsStyleAttributeDefined(AKey: TdxHtmlTextWriterStyle;
      out AValue: string): Boolean; overload;
    function OnAttributeRender(const AName: string; const AValue: string;
      AKey: TdxHtmlTextWriterAttribute): Boolean; virtual;
    function OnStyleAttributeRender(const AName: string; const AValue: string;
      AKey: TdxHtmlTextWriterStyle): Boolean; virtual;
    function OnTagRender(const AName: string; AKey: TdxHtmlTextWriterTag)
      : Boolean; virtual;
    ...
    class procedure RegisterAttribute(const AName: string;
      AKey: TdxHtmlTextWriterAttribute; AEncode: Boolean = False;
      AIsUrl: Boolean = False); static;
    class procedure RegisterStyle(const AName: string;
      AKey: TdxHtmlTextWriterStyle); static;
    class procedure RegisterTag(const AName: string; AKey: TdxHtmlTextWriterTag;
      AType: TTagType = TTagType.Other); static;
  ...
  public
    ...
    constructor Create(AWriter: TTextWriter; const ATabString: string);
      overload;
    destructor Destroy; override;
    procedure AddAttribute(const AName: string;
      const AValue: TdxNullableString); overload;
    procedure AddAttribute(const AName: string; const AValue: string); overload;
    procedure AddAttribute(AKey: TdxHtmlTextWriterAttribute;
      const AValue: TdxNullableString); overload;
    procedure AddAttribute(AKey: TdxHtmlTextWriterAttribute;
      const AValue: string); overload;
    procedure AddAttribute(const AName: string; const AValue: TdxNullableString;
      AEndode: Boolean); overload;
    procedure AddAttribute(AKey: TdxHtmlTextWriterAttribute;
      const AValue: TdxNullableString; AEncode: Boolean); overload;
    procedure AddStyleAttribute(const AName: string;
      const AValue: string); overload;
    procedure AddStyleAttribute(AKey: TdxHtmlTextWriterStyle;
      const AValue: string); overload;
    procedure BeginRender; virtual;
    procedure Close; override;
    procedure EndRender; virtual;
    procedure EnterStyle(AStyle: TdxWebStyle;
      ATag: TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span); virtual;
    procedure ExitStyle(AStyle: TdxWebStyle;
      ATag: TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span); virtual;
    procedure Flush; override;
    ...
    procedure Write(AValue: Single); override;
    procedure Write(const S: string); override;
    procedure Write(const ABuffer: TArray<Char>;
      AIndex, ACount: Integer); override;
    procedure WriteAttribute(const AName, AValue: string;
      AEncode: Boolean = False); virtual;
    ...
    procedure WriteLine(AValue: Single); override;
    procedure WriteLine(const S: string); override;
    procedure WriteLine(const ABuffer: TArray<Char>;
      AIndex, ACount: Integer); override;
    procedure WriteLineNoTabs(const S: string);
    procedure WriteObsoleteBreak;
    procedure WriteStyleAttribute(const AName, AValue: string;
      AEncode: Boolean = False); overload; virtual;

    property Indent: Integer read FIndentLevel write SetIndent;
    property InnerWriter: TTextWriter read FWriter write FWriter;
    property RenderDivAroundHiddenInputs: Boolean
      read GetRenderDivAroundHiddenInputs;
  end;
...
constructor TdxWebUnit.Create(const AValue: string;
  const ACulture: TdxCultureInfo; ADefaultType: TdxWebUnitType);
var
  S, AText: string;
...
class function TdxWebUnit.GetTypeFromString(const AValue: string)
  : TdxWebUnitType;
begin
  if AValue = '' then
...
class function TdxWebUnit.Parse(const S: string; const ACulture: TdxCultureInfo)
  : TdxWebUnit;
begin
...
begin
  ...
  if AValue <> '' then
  begin
    C := {$IFDEF DELPHIXE4}AValue[1].ToLower{$ELSE}TCharacter.ToLower
      (AValue[1]){$ENDIF};
    case C of
      's':
        begin
...
class function TdxWebFontUnit.Parse(const S: string;
  const ACulture: TdxCultureInfo): TdxWebFontUnit;
begin
...
const
  Sizes: array [TdxWebFontSize] of string = ('', // NotSet,
    '', // AsUnit,
  ...
  begin
    ...
    begin
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontNames) then
        ResetNames;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontSize) and
        (F.Size <> TdxWebFontUnit.Empty) then
        ResetFontSize;
...
begin
  ...
  begin
    ...
    if Length(AOwner.RegisteredCssClass) = 0 then
    begin
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontNames) and
        not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontNames) then
        Names := F.Names;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontSize) and
        (not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontSize) or
        (Size = TdxWebFontUnit.Empty)) then
        Size := F.Size;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontBold) and
        not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontBold) then
        Bold := F.Bold;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontItalic) and
        not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontItalic) then
        Italic := F.Italic;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontOverline) and
        not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontOverline) then
        Overline := F.Overline;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontStrikeout) and
        not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontStrikeout) then
        Strikeout := F.Strikeout;
      if AOwner.IsSet(TdxWebStyle.TAssignedValue.FontUnderline) and
        not FOwner.IsSet(TdxWebStyle.TAssignedValue.FontUnderline) then
        Underline := F.Underline;
    end;
  end;
end;
...
begin
  Result := FOwner.IsSet(TdxWebStyle.TAssignedValue.FontBold) and
    FOwner.ViewState['Font_Bold'].AsBoolean;
end;
...
begin
  Result := FOwner.IsSet(TdxWebStyle.TAssignedValue.FontItalic) and
    FOwner.ViewState['Font_Italic'].AsBoolean;
end;
...
begin
  Result := FOwner.IsSet(TdxWebStyle.TAssignedValue.FontOverline) and
    FOwner.ViewState['Font_Overline'].AsBoolean;
end;
...
begin
  Result := FOwner.IsSet(TdxWebStyle.TAssignedValue.FontStrikeout) and
    FOwner.ViewState['Font_Strikeout'].AsBoolean;
end;
...
begin
  Result := FOwner.IsSet(TdxWebStyle.TAssignedValue.FontUnderline) and
    FOwner.ViewState['Font_Underline'].AsBoolean;
end;
...
begin
  if FControlState >= TdxWebControlState.FrameworkInitialized then
    TdxRichEditExceptions.ThrowInvalidOperationException
      ('PropertySetBeforePreInitOrAddToControls');
  if not AValue then
    FFlags.&Set($1000)
  else
    FFlags.Clear($1000);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AControl.ID = '') and (ANamingContainer <> nil) and
        not AControl.Flags[$40] then
...
function TdxWebControlBase.IsDescendentOf(AAncestor: TdxWebControlBase)
  : Boolean;
var
...
procedure TdxWebControlBase.RenderChildrenInternal(AWriter: TdxHtmlTextWriter;
  const AChildren: TdxWebControlCollection);
var
  I: Integer;
begin
...
procedure TdxWebControlBase.AddedControl(AControl: TdxWebControlBase;
  AIndex: Integer);
var
...
begin
  ...
  begin
    ...
    if (AControl.FId = '') and not AControl.FFlags[$40] then
      AControl.GenerateAutomaticID
    else if (AControl.ID <> '') or (AControl.FControls <> nil) then
...
function TdxWebControlBase.FindControl(const AId: string; APathOffset: Integer)
  : TdxWebControlBase;
var
  AStr: string;
...
begin
  Result := (FControls <> nil) and (FControls.Count = 1) and
    Controls[0].IsLiteral;
end;
...
function TdxWebControlBase.OnBubbleEvent(ASource: TObject;
  AArgs: TdxEventArgs): Boolean;
begin
  Result := False;
end;
...
procedure TdxWebControlBase.RaiseBubbleEvent(ASource: TObject;
  AArgs: TdxEventArgs);
var
  AControl: TdxWebControlBase;
begin
...
procedure TdxWebControlBase.UpdateNamingContainer(ANamingContainer
  : TdxWebControlBase);
begin
...
procedure TdxWebControlBase.FillNamedControlsTable(ANamingContainer
  : TdxWebControlBase; AControls: TdxWebControlCollection);
var
  AControl: TdxWebControlBase;
  I: Integer;
begin
...
procedure TdxWebControlCollection.AddAt(AIndex: Integer;
  AChild: TdxWebControlBase);
begin
  if AIndex = -1 then
  begin
    Add(AChild);
...
procedure TdxWebEmptyControlCollection.AddAt(AIndex: Integer;
  AChild: TdxWebControlBase);
begin
...
begin
  FStyleAttribRegex := TRegex.Create
    ('\G(\s*(;\s*)*(?<stylename>[^:]+?)\s*:\s*(?<styleval>[^;]*))*\s*(;\s*)*$',
...
function TdxCssStyleCollection.GetItem(const AKey
  : TdxHtmlTextWriterStyle): string;
begin
  if FHtmlStyleTable = nil then
    Exit('');
...
procedure TdxCssStyleCollection.Add(AKey: TdxHtmlTextWriterStyle;
  const AValue: string);
var
  AStyleName: string;
begin
...
begin
  if ((FStyleTable = nil) or (FStyleTable.Count = 0)) and
    ((FHtmlStyleTable = nil) or (FHtmlStyleTable.Count = 0)) then
    Exit('');
...
function TdxWebAttributeCollection.GetItem(const AKey: string)
  : TdxNullableString;
begin
  if (FStyleCollection <> nil) and SameText(AKey, 'style') then
    Result := FStyleCollection.Value
  else if FBag[AKey].IsEmpty then
...
procedure TdxWebAttributeCollection.SetItem(const AKey: string;
  const Value: TdxNullableString);
begin
  Add(AKey, Value);
end;

procedure TdxWebAttributeCollection.Add(const AKey: string;
  const Value: TdxNullableString);
begin
  if (FStyleCollection <> nil) and SameText(AKey, 'style') then
    FStyleCollection.Value := Value
  else if Value.HasValue then
...
constructor TdxHtmlControl.Create
  (ATagKey: TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span);
begin
  inherited Create;
  FTagKey := ATagKey;
end;
...
class function TdxHtmlControl.MapStringAttributeToString
...
const
  ResultMap: array [TdxHtmlTextWriterTag] of string = ('unknown', 'a',
    'acronym', 'address', 'area', 'b', 'base', 'basefont', 'bdo', 'bgsound',
    'big', 'blockquote', 'body', 'br', 'button', 'caption', 'center', 'cite',
    'code', 'col', 'colgroup', 'dd', 'del', 'dfn', 'dir', 'div', 'dl', 'dt',
    'em', 'embed', 'o:p', 'fieldset', 'font', 'form', 'frame', 'frameset', 'h1',
    'h2', 'h3', 'h4', 'h5', 'h6', 'head', 'hr', 'html', 'i', 'iframe', 'img',
    'input', 'ins', 'isindex', 'kbd', 'label', 'legend', 'li', 'link', 'map',
    'marquee', 'menu', 'meta', 'nobr', 'noframes', 'noscript', 'object', 'ol',
    'option', 'p', 'param', 'pre', 'q', 'rt', 'ruby', 's', 'samp', 'script',
...
procedure TdxHtmlControl.AddDisplayInlineBlockIfNeeded
...
begin
  ...
  if (TagKey = TdxHtmlTextWriterTag.Span) or (TagKey = TdxHtmlTextWriterTag.A)
  then
...
procedure TdxHtmlControl.SetAttribute(const AName: string;
  const AValue: string);
begin
...
class function TdxWebStyle.FormatStringArray(const AArray: TArray<string>;
...
procedure TdxWebStyle.AddAttributesToRender(AWriter: TdxHtmlTextWriter;
  AOwner: TdxHtmlControl = nil);
var
...
begin
  ...
  begin
    ...
    if Length(ARegisteredCssClass) <> 0 then
      ARegisteredCssClass :=
        Format('%s %s', [ARegisteredCssClass, FRegisteredCssClass])
    else
...
begin
  if Length(RegisteredCssClass) <> 0 then
    TdxRichEditExceptions.ThrowInvalidOperationException
      ('Style_RegisteredStylesAreReadOnly');
  ...
  begin
    ...
    begin
      ...
      if S.IsSet(TAssignedValue.BackColor) and
        (S.BackColor <> TdxAlphaColors.Empty) then
      begin
        ViewState.Remove('BackColor');
        ClearBit(TAssignedValue.BackColor);
      end;
      if S.IsSet(TAssignedValue.ForeColor) and
        (S.ForeColor <> TdxAlphaColors.Empty) then
      begin
        ViewState.Remove('ForeColor');
        ClearBit(TAssignedValue.ForeColor);
      end;
      if S.IsSet(TAssignedValue.BorderColor) and
        (S.BorderColor <> TdxAlphaColors.Empty) then
      begin
        ViewState.Remove('BorderColor');
        ClearBit(TAssignedValue.BorderColor);
      end;
      if S.IsSet(TAssignedValue.BorderWidth) and
        (S.BorderWidth <> TdxWebUnit.Empty) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if S.IsSet(TAssignedValue.CssClass) and not IsSet(TAssignedValue.CssClass)
      then
        CssClass := S.CssClass;
      if Length(S.RegisteredCssClass) = 0 then
      begin
        if S.IsSet(TAssignedValue.BackColor) and
          (not IsSet(TAssignedValue.BackColor) or
          (BackColor = TdxAlphaColors.Empty)) then
          BackColor := S.BackColor;
        if S.IsSet(TAssignedValue.ForeColor) and
          (not IsSet(TAssignedValue.ForeColor) or
          (ForeColor = TdxAlphaColors.Empty)) then
          ForeColor := S.ForeColor;
        if S.IsSet(TAssignedValue.BorderColor) and
          (not IsSet(TAssignedValue.BorderColor) or
          (BorderColor = TdxAlphaColors.Empty)) then
          BorderColor := S.BorderColor;
        if S.IsSet(TAssignedValue.BorderWidth) and
          (not IsSet(TAssignedValue.BorderWidth) or
          (BorderWidth = TdxWebUnit.Empty)) then
          BorderWidth := S.BorderWidth;
        if S.IsSet(TAssignedValue.BorderStyle) and
          not IsSet(TAssignedValue.BorderStyle) then
          BorderStyle := S.BorderStyle;
        if S.IsSet(TAssignedValue.Height) and
          (not IsSet(TAssignedValue.Height) or (Height = TdxWebUnit.Empty)) then
          Height := S.Height;
        if S.IsSet(TAssignedValue.Width) and
          (not IsSet(TAssignedValue.Width) or (Width = TdxWebUnit.Empty)) then
          Width := S.Width;
      end
      else if IsSet(TAssignedValue.CssClass) then
        CssClass := Format('%s %s', [CssClass, S.RegisteredCssClass])
      else
        CssClass := S.RegisteredCssClass;
    end;
...
begin
  ...
  begin
    ...
    if not TdxAlphaColors.IsEmpty(AColor) then
      AAttributes.Add(TdxHtmlTextWriterStyle.Color,
        TdxAlphaColors.ToHtml(AColor));
  end;
  if IsSet(TAssignedValue.BackColor) then
  begin
    AColor := ViewState['BackColor'].AsType<TdxAlphaColor>;
    if not TdxAlphaColors.IsEmpty(AColor) then
      AAttributes.Add(TdxHtmlTextWriterStyle.BackgroundColor,
        TdxAlphaColors.ToHtml(AColor));
  end;
  if IsSet(TAssignedValue.BorderColor) then
  begin
    ...
    if not TdxAlphaColors.IsEmpty(AColor) then
      AAttributes.Add(TdxHtmlTextWriterStyle.BorderColor,
        TdxAlphaColors.ToHtml(AColor));
  end;
  ...
  begin
    AAttributes.Add(TdxHtmlTextWriterStyle.BorderWidth,
      ABorderWidth.ToString(TdxCultureInfo.InvariantCulture));
    ...
    begin
      ...
      AAttributes.Add(TdxHtmlTextWriterStyle.BorderStyle,
        BorderStyles[Integer(ABorderStyle)]);
  end
  else if ABorderStyle <> TdxWebBorderStyle.NotSet then
    AAttributes.Add(TdxHtmlTextWriterStyle.BorderStyle,
      BorderStyles[Integer(ABorderStyle)]);
  AFont := Font;
  ANames := AFont.Names;
  if Length(ANames) > 0 then
    AAttributes.Add(TdxHtmlTextWriterStyle.FontFamily,
      FormatStringArray(ANames, ','));
  ASize := AFont.Size;
  if not ASize.IsEmpty then
    AAttributes.Add(TdxHtmlTextWriterStyle.FontSize,
      ASize.ToString(TdxCultureInfo.InvariantCulture));
  ...
  else if IsSet(TAssignedValue.FontUnderline) or
    IsSet(TAssignedValue.FontOverline) or IsSet(TAssignedValue.FontStrikeout)
  then
  ...
  begin
    ...
    if not AUnit.IsEmpty then
      AAttributes.Add(TdxHtmlTextWriterStyle.Height,
        AUnit.ToString(TdxCultureInfo.InvariantCulture));
  end;
  if IsSet(TAssignedValue.Width) then
  begin
    AUnit := ViewState['Width'].AsType<TdxWebUnit>;
    if not AUnit.IsEmpty then
      AAttributes.Add(TdxHtmlTextWriterStyle.Width,
        AUnit.ToString(TdxCultureInfo.InvariantCulture));
  end;
end;
...
class function TdxHtmlStyleRender.GetHtmlStyle(AFont: TdxGPFont;
  AForeColor, ABackColor: TdxAlphaColor): string;
begin
  Result := Format('color:%s;background-color:%s;%s',
    [TdxHtmlConvert.ToHtml(AForeColor), TdxHtmlConvert.ToHtml(ABackColor),
    GetFontHtml(AFont)]);
end;

class function TdxHtmlStyleRender.GetHtmlStyle(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean; AForeColor, ABackColor: TdxAlphaColor): string;
begin
  Result := GetHtmlStyle(AFontFamilyName, ASize, AUnit, ABold, AItalic,
    AStrikeout, AUnderline, AForeColor, ABackColor, False);
end;

class function TdxHtmlStyleRender.GetHtmlStyle(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean; AForeColor, ABackColor: TdxAlphaColor;
  AUseFontSizeInPixels: Boolean): string;
var
  AFontHtml: string;
begin
  AFontHtml := GetFontHtml(AFontFamilyName, ASize, AUnit, ABold, AItalic,
    AStrikeout, AUnderline, AUseFontSizeInPixels);
  Result := Format('color:%s;background-color:%s;%s',
    [TdxHtmlConvert.ToHtml(AForeColor), TdxHtmlConvert.ToHtml(ABackColor),
    AFontHtml]);
end;

class procedure TdxHtmlStyleRender.GetHtmlStyle(AFont: TdxGPFont;
  AForeColor, ABackColor: TdxAlphaColor; AStyle: TdxCssStyleCollection);
begin
...
class procedure TdxHtmlStyleRender.GetHtmlStyle(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean; AForeColor, ABackColor: TdxAlphaColor;
  AStyle: TdxCssStyleCollection);
begin
  GetHtmlStyle(AFontFamilyName, ASize, AUnit, ABold, AItalic, AStrikeout,
    AUnderline, AForeColor, ABackColor, False, AStyle);
end;

class procedure TdxHtmlStyleRender.GetHtmlStyle(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean; AForeColor, ABackColor: TdxAlphaColor;
  AUseFontSizeInPixels: Boolean; AStyle: TdxCssStyleCollection);
begin
  AStyle.Add('color', TdxHtmlConvert.ToHtml(AForeColor));
  AStyle.Add('background-color', TdxHtmlConvert.ToHtml(ABackColor));
  GetFontHtml(AFontFamilyName, ASize, AUnit, ABold, AItalic, AStrikeout,
    AUnderline, AUseFontSizeInPixels, AStyle);
end;

class function TdxHtmlStyleRender.GetHtmlStyle(AFont: TdxGPFont;
  AForeColor: TdxAlphaColor): string;
begin
  Result := Format('color:%s;%s', [TdxHtmlConvert.ToHtml(AForeColor),
    GetFontHtml(AFont)]);
end;
...
class function TdxHtmlStyleRender.GetFontHtmlInPixels(AFont: TdxGPFont)
  : TObject;
begin
  // if AFont = nil then
  // Result := ''
...
class function TdxHtmlStyleRender.GetFontHtml(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean; AInPixels: Boolean): string;
var
  AFontString: string;
begin
  if AInPixels then
  ...
  Result := Format
    ('font-family:%s; font-size:%s; font-weight:%s; font-style:%s; %s',
    [GetCorrectedFamilyName(AFontFamilyName), AFontString, GetFontWeight(ABold),
    GetFontStyle(AItalic), GetTextDecoration(AStrikeout, AUnderline)]);
end;

class function TdxHtmlStyleRender.GetFontHtml(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean): string;
begin
  Result := GetFontHtml(AFontFamilyName, ASize, AUnit, ABold, AItalic,
    AStrikeout, AUnderline, False);
end;

class procedure TdxHtmlStyleRender.GetFontHtml(AFont: TdxGPFont;
  AStyle: TdxCssStyleCollection);
begin
  // if AFont = nil then
  // Exit;
...
class procedure TdxHtmlStyleRender.GetFontHtml(const AFontFamilyName: string;
  ASize: Single; AUnit: TdxGraphicUnit; ABold, AItalic, AStrikeout,
  AUnderline: Boolean; AInPixels: Boolean; AStyle: TdxCssStyleCollection);
var
...
class function TdxHtmlStyleRender.GetCorrectedFamilyName(const AFamilyName
  : string): string;
begin
...
class function TdxHtmlStyleRender.GetTextDecoration(AStrikeout,
  AUnderline: Boolean): string;
var
...
class function TdxHtmlStyleRender.GetTextDecorationValue(AStrikeout,
  AUnderline: Boolean): string;
begin
  if AStrikeout then
    Result := ' line-through'
...
constructor TdxCssTextWriter.TAttributeInformation.Create(const AName: string;
  AEncode, AIsUrl: Boolean);
begin
  Name := AName;
  Encode := AEncode;
  IsUrl := AIsUrl;
...
begin
  ...
  RegisterAttribute('background-image', TdxHtmlTextWriterStyle.BackgroundImage,
    True, True);
  ...
  RegisterAttribute('list-style-image', TdxHtmlTextWriterStyle.ListStyleImage,
    True, True);
...
class procedure TdxCssTextWriter.WriteUrlAttribute(AWriter: TTextWriter;
  const AUrl: string);
var
  S: string;
  ALength: Integer;
...
class function TdxCssTextWriter.GetStyleKey(const AStyleName: string)
  : TdxHtmlTextWriterStyle;
begin
  if (AStyleName <> '') and FStyleLookup.TryGetValue(LowerCase(AStyleName),
    Result) then
    Exit;
  Result := TdxHtmlTextWriterStyle.Invalid;
end;

class function TdxCssTextWriter.GetStyleName
  (AStyleKey: TdxHtmlTextWriterStyle): string;
begin
  Result := FAttributeNameLookup[AStyleKey].Name;
end;

class function TdxCssTextWriter.IsStyleEncoded
  (AStyleKey: TdxHtmlTextWriterStyle): Boolean;
begin
  Result := FAttributeNameLookup[AStyleKey].Encode;
end;

class procedure TdxCssTextWriter.RegisterAttribute(const AName: string;
  AKey: TdxHtmlTextWriterStyle; AEncode: Boolean = False;
  AIsUrl: Boolean = False);
begin
  if AKey <> TdxHtmlTextWriterStyle.Invalid then
    FStyleLookup.Add(LowerCase(AName), AKey);
  FAttributeNameLookup[AKey] := TAttributeInformation.Create(AName,
    AEncode, AIsUrl);
end;
...
procedure TdxCssTextWriter.Write(const ABuffer: TArray<Char>; AIndex: Integer;
  ACount: Integer);
begin
  FWriter.Write(ABuffer, AIndex, ACount);
end;

procedure TdxCssTextWriter.WriteAttribute(const AName: string;
  const AValue: string);
begin
  WriteAttribute(FWriter, GetStyleKey(AName), AName, AValue);
end;

procedure TdxCssTextWriter.WriteAttribute(AKey: TdxHtmlTextWriterStyle;
  const AValue: string);
begin
  WriteAttribute(FWriter, AKey, GetStyleName(AKey), AValue);
end;

class procedure TdxCssTextWriter.WriteAttribute(AWriter: TTextWriter;
  AKey: TdxHtmlTextWriterStyle; const AName, AValue: string);
var
  AIsUrl: Boolean;
begin
...
class procedure TdxCssTextWriter.WriteAttributes(AWriter: TTextWriter;
  AStyles: TList<TdxWebRenderStyle>);
var
  I: Integer;
...
constructor TdxHtmlTextWriter.TTagInformation.Create(const AName: string;
  ATagType: TTagType; const AClosingTag: string);
begin
  Name := AName;
  TagType := ATagType;
...
constructor TdxHtmlTextWriter.TAttributeInformation.Create(const AName: string;
  AEncode, AIsUrl: Boolean);
begin
  Name := AName;
  Encode := AEncode;
...
constructor TdxHtmlTextWriter.TLayout.Create(AAlignment: TdxWebHorizontalAlign;
  AWrapping: Boolean);
begin
  Align := AAlignment;
  Wrap := AWrapping;
...
begin
  ...
  FAttributeLookup :=
    TdxNamedOrdinalDictionary<TdxHtmlTextWriterAttribute>.Create;
  ...
  RegisterAttribute('autocomplete',
    TdxHtmlTextWriterAttribute.AutoComplete, False);
  RegisterAttribute('axis', TdxHtmlTextWriterAttribute.Axis, True);
  RegisterAttribute('background', TdxHtmlTextWriterAttribute.Background,
    True, True);
  ...
  RegisterAttribute('bordercolor',
    TdxHtmlTextWriterAttribute.BorderColor, False);
  RegisterAttribute('cellpadding',
    TdxHtmlTextWriterAttribute.Cellpadding, False);
  RegisterAttribute('cellspacing',
    TdxHtmlTextWriterAttribute.Cellspacing, False);
  ...
  RegisterAttribute('longdesc', TdxHtmlTextWriterAttribute.Longdesc,
    True, True);
  ...
  RegisterAttribute('_designerRegion',
    TdxHtmlTextWriterAttribute.DesignerRegion, False);
end;
...
procedure TdxHtmlTextWriter.AddAttribute(const AName: string;
  const AValue: TdxNullableString);
var
  AAttributeKey: TdxHtmlTextWriterAttribute;
begin
  AAttributeKey := GetAttributeKey(AName);
  AddAttribute(AName, EncodeAttributeValue(AAttributeKey, AValue),
    AAttributeKey);
end;
...
procedure TdxHtmlTextWriter.AddAttribute(AKey: TdxHtmlTextWriterAttribute;
  const AValue: TdxNullableString);
var
...
begin
  ...
  begin
    ...
    AddAttribute(AInformation.Name, AValue, AKey, AInformation.Encode,
      AInformation.IsUrl);
  end;
end;
...
procedure TdxHtmlTextWriter.AddAttribute(const AName: string;
  const AValue: TdxNullableString; AEndode: Boolean);
var
  AAttributeKey: TdxHtmlTextWriterAttribute;
begin
  AAttributeKey := GetAttributeKey(AName);
...
procedure TdxHtmlTextWriter.AddAttribute(AKey: TdxHtmlTextWriterAttribute;
  const AValue: TdxNullableString; AEncode: Boolean);
var
  AInformation: TAttributeInformation;
begin
  if AKey <> TdxHtmlTextWriterAttribute.Invalid then
...
procedure TdxHtmlTextWriter.AddAttribute(const AName: string;
  const AValue: TdxNullableString; AKey: TdxHtmlTextWriterAttribute;
...
procedure TdxHtmlTextWriter.AddStyleAttribute(const AName: string;
  const AValue: string);
begin
  AddStyleAttribute(AName, AValue, TdxCssTextWriter.GetStyleKey(AName));
end;

procedure TdxHtmlTextWriter.AddStyleAttribute(AKey: TdxHtmlTextWriterStyle;
  const AValue: string);
begin
...
procedure TdxHtmlTextWriter.AddStyleAttribute(const AName: string;
  const AValue: string; AKey: TdxHtmlTextWriterStyle);
var
  AStyle: TdxWebRenderStyle;
  AStr: string;
begin
...
function TdxHtmlTextWriter.EncodeAttributeValue(const AValue: TdxNullableString;
  AEncode: Boolean): TdxNullableString;
begin
...
function TdxHtmlTextWriter.EncodeAttributeValue
  (AAttrKey: TdxHtmlTextWriterAttribute; const AValue: TdxNullableString)
  : TdxNullableString;
begin
...
procedure TdxHtmlTextWriter.EnterStyle(AStyle: TdxWebStyle;
  ATag: TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span);
begin
...
procedure TdxHtmlTextWriter.ExitStyle(AStyle: TdxWebStyle;
  ATag: TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span);
begin
  if not AStyle.IsEmpty or (ATag <> TdxHtmlTextWriterTag.Span) then
    RenderEndTag;
...
function TdxHtmlTextWriter.GetAttributeKey(const AAttrName: string)
  : TdxHtmlTextWriterAttribute;
begin
  if (AAttrName <> '') and FAttributeLookup.TryGetValue(LowerCase(AAttrName),
    Result) then
    Exit(Result);
  Result := TdxHtmlTextWriterAttribute.Invalid;
end;

function TdxHtmlTextWriter.GetAttributeName
  (AAttrKey: TdxHtmlTextWriterAttribute): string;
begin
  Result := FAttributeNameLookup[AAttrKey].Name;
end;

function TdxHtmlTextWriter.GetStyleKey(const AStyleName: string)
  : TdxHtmlTextWriterStyle;
begin
  Result := TdxCssTextWriter.GetStyleKey(AStyleName);
end;

function TdxHtmlTextWriter.GetStyleName(AStyleKey
  : TdxHtmlTextWriterStyle): string;
begin
  Result := TdxCssTextWriter.GetStyleName(AStyleKey);
end;

function TdxHtmlTextWriter.GetTagKey(const ATagName: string)
  : TdxHtmlTextWriterTag;
begin
  if (ATagName <> '') and FTagsLookup.TryGetValue(LowerCase(ATagName), Result)
  then
    Exit;
...
function TdxHtmlTextWriter.IsAttributeDefined
  (AKey: TdxHtmlTextWriterAttribute): Boolean;
var
  AAttribute: TRenderAttribute;
begin
...
function TdxHtmlTextWriter.IsAttributeDefined(AKey: TdxHtmlTextWriterAttribute;
...
function TdxHtmlTextWriter.IsStyleAttributeDefined
  (AKey: TdxHtmlTextWriterStyle): Boolean;
var
  AStyle: TdxWebRenderStyle;
begin
...
function TdxHtmlTextWriter.IsStyleAttributeDefined(AKey: TdxHtmlTextWriterStyle;
  out AValue: string): Boolean;
var
  AStyle: TdxWebRenderStyle;
begin
...
function TdxHtmlTextWriter.IsValidFormAttribute(const AAttribute
  : string): Boolean;
begin
  Result := True;
end;

function TdxHtmlTextWriter.OnAttributeRender(const AName: string;
  const AValue: string; AKey: TdxHtmlTextWriterAttribute): Boolean;
begin
  Result := True;
end;

function TdxHtmlTextWriter.OnStyleAttributeRender(const AName: string;
  const AValue: string; AKey: TdxHtmlTextWriterStyle): Boolean;
begin
  Result := True;
end;

function TdxHtmlTextWriter.OnTagRender(const AName: string;
  AKey: TdxHtmlTextWriterTag): Boolean;
begin
  Result := True;
end;
...
begin
  OpenDiv(FCurrentLayout, (FCurrentLayout <> nil) and
    (FCurrentLayout.Align <> TdxWebHorizontalAlign.NotSet),
    (FCurrentLayout <> nil) and not FCurrentLayout.Wrap);
end;

procedure TdxHtmlTextWriter.OpenDiv(ALayout: TLayout;
  AWriteHorizontalAlign: Boolean; AWriteWrapping: Boolean);
var
  AStr: string;
begin
  WriteBeginTag('div');
...
class procedure TdxHtmlTextWriter.RegisterAttribute(const AName: string;
  AKey: TdxHtmlTextWriterAttribute; AEncode: Boolean = False;
  AIsUrl: Boolean = False);
begin
  if AKey <> TdxHtmlTextWriterAttribute.Invalid then
...
class procedure TdxHtmlTextWriter.RegisterStyle(const AName: string;
  AKey: TdxHtmlTextWriterStyle);
begin
  TdxCssTextWriter.RegisterAttribute(AName, AKey);
end;

class procedure TdxHtmlTextWriter.RegisterTag(const AName: string;
  AKey: TdxHtmlTextWriterTag; AType: TTagType = TTagType.Other);
var
  S: string;
begin
  S := LowerCase(AName);
  ...
  if (AType <> TTagType.NonClosing) and (AKey <> TdxHtmlTextWriterTag.Unknown)
  then
    S := Format('</%s>', [S])
  else
    S := '';
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if AAttribute.IsUrl and
            ((AAttribute.Key <> TdxHtmlTextWriterAttribute.Href) or
            not TdxStringHelper.StartsWith(AUrl, 'javascript:')) then
            AUrl := EncodeUrl(AUrl);
...
procedure TdxHtmlTextWriter.WriteAttribute(const AName, AValue: string;
  AEncode: Boolean = False);
begin
  FWriter.Write(' ');
  FWriter.Write(AName);
  if AValue <> '' then
  ...
  begin
    ...
    begin
      if AStartIndex = 0 then
        ...
        TdxHttpUtility.HtmlEncode(TdxStringHelper.Substring(AText, AStartIndex,
          ALength - AStartIndex), Self);
      AStartIndex := ALength;
    end
    else
    begin
...
procedure TdxHtmlTextWriter.WriteStyleAttribute(const AName, AValue: string;
  AEncode: Boolean = False);
begin
  FWriter.Write(AName);
  FWriter.Write(':');
...
procedure TdxHtmlTextWriter.WriteUrlEncodedString(const AText: string;
  AArgument: Boolean);
var
  I: Integer;
  C: Char;
begin
  ...
  begin
    ...
    if TdxHttpUtility.IsSafe(C) then
      Write(C)
    else if not AArgument and ((C = '/') or (C = ':') or (C = '#') or (C = ','))
    then
      Write(C)
    else if (C = ' ') and AArgument then
      Write('+')
    else if (Ord(C) and $FF80) = 0 then
    begin
      Write('%');
