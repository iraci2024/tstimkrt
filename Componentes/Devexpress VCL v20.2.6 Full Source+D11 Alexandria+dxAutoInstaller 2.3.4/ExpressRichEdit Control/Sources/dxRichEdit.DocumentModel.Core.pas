...
type
  ...
  TdxRunVisibility = (Hidden, Visible, ForceVisible);

  TdxParagraphIteratorResult = (Success, RunFinished, Finished);

  TdxDocumentLayoutDetailsLevel = (None = -1, Page = 0, PageArea = 1,
    Column = 2, TableRow = 3, TableCell = 4, Row = 5, Box = 6, Character = 7,
    Max = MaxInt);

  TdxResetFormattingCacheType = (Character, Paragraph, All);
  ...
  TdxFormatterPosition = record
    ...
    class operator GreaterThanOrEqual(const A, B: TdxFormatterPosition)
      : Boolean;
  end;

  TdxDocumentModelChangeAction = (Redraw, ResetPrimaryLayout,
    ResetAllPrimaryLayout, ResetSecondaryLayout, ResetSelectionLayout,
    ResetCaretInputPositionFormatting, RaiseSelectionChanged,
    RaiseContentChanged, ScrollToBeginOfDocument, RaiseEmptyDocumentCreated,
    RaiseDocumentLoaded, RaiseModifiedChanged, ResetUncheckedIntervals,
    ResetIgnoredList, ForceResize, ValidateSelectionInterval,
    PerformActionsOnIdle, SplitRunByCharset, ResetRuler,
    SuppressBindingsNotifications, ActivePieceTableChanged,
    RaiseDocumentProtectionChanged, ForceResetHorizontalRuler,
    ForceResetVerticalRuler, ForceSyntaxHighlight, ApplyAutoCorrect,
    SuppressRaiseContentChangedCalculationByCurrentTransactionChanges, Fields);
  ...
  public
    procedure SetIndex(AIndex: Integer;
      const AChangeActions: TdxDocumentModelChangeActions); virtual; abstract;
  ...
  strict protected
    ...
    property InnerFontCacheIndex: Integer read FFontCacheIndex
      write FFontCacheIndex;
  protected
    // IUnknown
    function QueryInterface(const IID: TGUID; out Obj): HResult; stdcall;
    function _AddRef: Integer; stdcall;
  ...
  public
    ...
    property Items[Index: TdxRunIndex]: TdxRunBase read GetItem
  ...
  public
    ...
    function SearchByLogPosition(ALogPosition: TdxDocumentLogPosition)
  ...
  public
    ...
    class operator LessThanOrEqual(const A, B: TdxDocumentModelPosition)
      : Boolean;
    class operator GreaterThan(const A, B: TdxDocumentModelPosition): Boolean;
    class operator GreaterThanOrEqual(const A, B: TdxDocumentModelPosition)
      : Boolean;

    class function FromDocumentEnd(APieceTable: TdxCustomPieceTable)
      : TdxDocumentModelPosition; static;
    class function FromRunEnd(APieceTable: TdxCustomPieceTable;
      ARunIndex: TdxRunIndex): TdxDocumentModelPosition; static;
    class function FromRunStart(APieceTable: TdxCustomPieceTable;
      ARunIndex: TdxRunIndex): TdxDocumentModelPosition; static;
    class function FromParagraphEnd(APieceTable: TdxCustomPieceTable;
      AParagraphIndex: TdxParagraphIndex): TdxDocumentModelPosition; static;
    class function FromParagraphStart(APieceTable: TdxCustomPieceTable;
      AParagraphIndex: TdxParagraphIndex): TdxDocumentModelPosition; static;

    function GetHashCode: Integer;
    class procedure SetParagraphStart(var APos: TdxDocumentModelPosition;
      AParagraphIndex: TdxParagraphIndex); static;
    class procedure SetParagraphEnd(APos: PdxDocumentModelPosition;
      AParagraphIndex: TdxParagraphIndex); static;
    class procedure SetRunStart(var APos: TdxDocumentModelPosition;
      ARunIndex: TdxRunIndex); static;
    class procedure SetRunEnd(var APos: TdxDocumentModelPosition;
      ARunIndex: TdxRunIndex); static;
    class function MoveBackward(const APos: TdxDocumentModelPosition)
      : TdxDocumentModelPosition; static; inline;
    class procedure MoveBackwardCore(var APos
      : TdxDocumentModelPosition); static;
    class function MoveForward(const APos: TdxDocumentModelPosition)
      : TdxDocumentModelPosition; static; inline;
    ...
    property PieceTable: TdxCustomPieceTable read FPieceTable;
    property RunEndLogPosition: TdxDocumentLogPosition
      read GetRunEndLogPosition;
    property RunIndex: TdxRunIndex read FRunIndex write SetRunIndex;
    property RunOffset: Integer read GetRunOffset;
    property RunStartLogPosition: TdxDocumentLogPosition
      read FRunStartLogPosition write SetRunStartLogPosition;
  end;
  ...
  protected
    ...
    function CreateDocumentModelPosition(APieceTable: TdxCustomPieceTable)
      : TdxDocumentModelPosition;
  ...
  public
    class function ToDocumentModelPosition(APieceTable: TdxCustomPieceTable;
      APos: TdxDocumentLogPosition): TdxDocumentModelPosition; overload; static;
    class function ToDocumentModelPosition(APieceTable: TdxCustomPieceTable;
      APos: TdxFormatterPosition): TdxDocumentModelPosition; overload; static;
    class function GetRunStartLogPosition(ARunIndex: TdxRunIndex;
      AParagraph: TdxParagraphBase): TdxDocumentLogPosition; static;
    class function ToFormatterPosition(const APos: TdxDocumentModelPosition)
      : TdxFormatterPosition; overload; static;
    class function ToFormatterPosition(APieceTable: TdxCustomPieceTable;
      APos: TdxDocumentLogPosition): TdxFormatterPosition; overload; static;
  end;
  ...
  public
    procedure ModifySection(ASection: TObject; ASectionIndex: TdxSectionIndex);
      virtual; abstract;
  end;
  ...
  strict protected
    function CreateParagraphCollection: TdxParagraphBaseCollection;
      virtual; abstract;
    function CreateRuns: TdxRunCollection; virtual; abstract;

    function GetDocumentStartLogPosition: TdxDocumentLogPosition;
      virtual; abstract;
    function GetDocumentEndLogPosition: TdxDocumentLogPosition;
      virtual; abstract;
  ...
  public
    constructor Create(const ADocumentModel: TdxCustomDocumentModel;
      const AContentType: TdxContentTypeBase); virtual;
    destructor Destroy; override;

    procedure AddPieceTables(AResult: TdxFastList;
      AIncludeUnreferenced: Boolean); virtual; abstract;
    procedure ApplyChanges(AChangeType: TdxDocumentModelChangeType;
      AStartRunIndex, AEndRunIndex: TdxRunIndex);
    procedure ApplyChangesCore(const Actions: TdxDocumentModelChangeActions;
      AStartRunIndex, AEndRunIndex: Integer);
    procedure ApplySectionFormatting(ALogPositionStart: TdxDocumentLogPosition;
      ALength: Integer; AModifier: TdxSectionPropertyModifierBase);
    function CalculateRunIndex(ARun: TdxRunBase;
      ADefaultResultWhenNotFound: TdxRunIndex = 0): TdxRunIndex;
    procedure Clear; virtual;
    function FindParagraphIndex(ALogPosition: TdxDocumentLogPosition;
      AStrictSearch: Boolean = True): TdxParagraphIndex;
    function FindRunStartLogPosition(AParagraph: TdxParagraphBase;
      ALogPosition: TdxDocumentLogPosition; out ARunIndex: TdxRunIndex)
      : TdxDocumentLogPosition;
    function GetRunLogPosition(ARunIndex: TdxRunIndex)
      : TdxDocumentLogPosition; overload;
    function GetRunLogPosition(ARun: TdxRunBase)
      : TdxDocumentLogPosition; overload;

    procedure CalculateRunInfoStart(ALogPosition: TdxDocumentLogPosition;
      AResult: TdxRunInfo);
    procedure CalculateRunInfoEnd(ALogPosition: TdxDocumentLogPosition;
      AResult: TdxRunInfo);
    function FindRunInfo(ALogPositionStart: TdxDocumentLogPosition;
    ...
    procedure ResetParagraphs(AFrom, ATo: TdxParagraphIndex); overload;
      virtual; abstract;

    function FindParagraphIndexCore(ALogPosition: TdxDocumentLogPosition)
      : TdxParagraphIndex;
    ...
    property DocumentStartLogPosition: TdxDocumentLogPosition
      read GetDocumentStartLogPosition;
    property DocumentEndLogPosition: TdxDocumentLogPosition
      read GetDocumentEndLogPosition;
  ...
  private
    ...
    function CheckIsTransactionChangeModifiedForward(AIndex, AStartInnerIndex
      : Integer): Boolean;
    function CheckIsTransactionChangeModifiedBackward(AIndex, AStartInnerIndex
      : Integer): Boolean;
  ...
  protected
    ...
    procedure SetModifiedTextAppended(FForceRaiseModifiedChanged
      : Boolean); virtual;
    ...
    function CommitAsSingleItemCore(ASingleItem: TdxHistoryItem)
      : TdxHistoryItem; virtual;
    ...
    property DocumentModel: TdxCustomDocumentModel read FDocumentModel;
    property UnmodifiedIndex: Integer read FUnmodifiedIndex
      write FUnmodifiedIndex;
  public
    ...
    function IsModified(AUnmodifiedIndex, AUnmodifiedTransactionIndex: Integer)
      : Boolean; overload;
    ...
    property SuppressRaiseOperationComplete: Boolean
      read FSuppressRaiseOperationComplete
      write FSuppressRaiseOperationComplete;
  ...
  public
    procedure Initialize(ADocumentModel: TdxCustomDocumentModel);
      virtual; abstract;
  end;
  ...
  public
    constructor Create(ADocumentModel: TdxCustomDocumentModel); virtual;
    property DocumentModel: TdxCustomDocumentModel read FDocumentModel;
    property IsSetContentMode: Boolean read FIsSetContentMode
      write FIsSetContentMode;
    property SuppressSyntaxHighlight: Boolean read FSuppressSyntaxHighlight
      write FSuppressSyntaxHighlight;
  end;
  ...
  TdxCustomDocumentModel = class abstract(TdxDpiSupport, IdxBatchUpdatable,
    IdxBatchUpdateHandler, IdxServiceProvider, IdxServiceContainer,
    IdxFontsContainer)
  public const
  ...
  strict private
    ...
    procedure OnDocumentCapabilitiesChanged(ASender: TObject;
      E: TdxRichEditNotificationOptionsChangedArgs);
  protected
    function GetDefaultLayoutUnit: TdxDocumentLayoutUnit; virtual;

    function CreateDocumentModelUnitConverter
      : TdxDocumentModelUnitConverter; virtual;
    function CreateDeferredChanges: TdxCustomDocumentModelDeferredChanges;
      virtual; abstract;
    function CreateServiceManager: TdxServiceManager; virtual;
    function CreateSectionCollection: TdxCustomSectionCollection;
      virtual; abstract;
    ...
    procedure OnHistoryOperationCompleted(ASender: TObject; E: TdxEventArgs);
      virtual; abstract;
    procedure OnHistoryModifiedChanged(ASender: TObject; E: TdxEventArgs);
      virtual; abstract;
  ...
  public
    ...
    function AddHistoryItem(AItem: TdxHistoryItem): TdxHistoryItem;
      virtual; abstract;
    function AutodetectDocumentFormat(const AFileName: string;
      AUseFormatFallback: Boolean = True): TdxRichEditDocumentFormat;
      virtual; abstract;
    procedure ApplyChanges(APieceTable: TdxCustomPieceTable;
      AChangeType: TdxDocumentModelChangeType;
      AStartRunIndex, AEndRunIndex: TdxRunIndex); virtual; abstract;
    procedure ApplyChangesCore(APieceTable: TdxCustomPieceTable;
    ...
    function CreatePieceTable(AContentType: TdxContentTypeBase)
      : TdxCustomPieceTable; virtual; abstract;
    ...
    procedure ResetDocumentFormattingCaches(AResetFormattingCacheType
      : TdxResetFormattingCacheType); virtual; abstract;

    // for internal use
    function CreateSection: TdxCustomSection; virtual; abstract;
    function FindSectionIndex(ALogPosition: TdxDocumentLogPosition;
      AStrictSearch: Boolean = True): TdxSectionIndex;
    ...
    function LookupSectionIndexByParagraphIndex(AParagraphIndex
      : TdxParagraphIndex): TdxSectionIndex; virtual; abstract;

    property Cache: TdxCustomDocumentCache read FCache;
    property DeferredChanges: TdxCustomDocumentModelDeferredChanges
      read FDeferredChanges;
    property DocumentCapabilities: TdxCustomDocumentCapabilitiesOptions
      read FDocumentCapabilities write SetDocumentCapabilities;
    ...
    property IsUpdateLockedOrOverlapped: Boolean
      read GetIsUpdateLockedOrOverlapped;
    property IsUpdateLocked: Boolean read GetIsUpdateLocked;
    property LayoutUnit: TdxDocumentLayoutUnit read FLayoutUnit
      write SetLayoutUnit;
    property LayoutUnitConverter: TdxDocumentLayoutUnitConverter
      read FLayoutUnitConverter;
    ...
    property ToDocumentLayoutUnitConverter
      ...
      read FToDocumentLayoutUnitConverter;
  ...
  TdxRequestSectionIndexEvent = procedure(ASender: TObject;
    AArgs: TdxRequestSectionIndexEventArgs) of object;
  TdxRequestSectionIndexEventHandler =
    TdxMulticastMethod<TdxRequestSectionIndexEvent>;
  ...
  public
    ...
    procedure ApplyChanges(AChangeType: TdxDocumentModelChangeType;
      AStartRunIndex: TdxRunIndex; AEndRunIndex: TdxRunIndex); virtual;
    procedure ApplyChangesCore(AActions: TdxDocumentModelChangeActions;
      AStartRunIndex: TdxRunIndex; AEndRunIndex: TdxRunIndex); virtual;
    procedure ApplySectionFormatting(ALogPositionStart: TdxDocumentLogPosition;
      ALength: Integer; AModifier: TdxSectionPropertyModifierBase); virtual;
    procedure FixLastParagraphOfLastSection(AOriginalParagraphCount: Integer);
      virtual; abstract;
    function LookupSectionIndexByParagraphIndex(AParagraphIndex
      : TdxParagraphIndex): TdxSectionIndex; virtual;
    procedure SetPageCount(APageCount: Integer); virtual;

    // for internal use
    function CreateSpellCheckerManager(APieceTable: TdxCustomPieceTable)
      : TObject { TdxSpellCheckerManager }; virtual;
    property DocumentModel: TdxCustomDocumentModel read GetDocumentModel;
...
function TdxSectionAndLogPositionComparable.CompareTo(const ASection
  : TdxCustomSection): Integer;
var
...
begin
  AFirstParagraphLogPosition := ASection.DocumentModel.MainPart.Paragraphs
    [ASection.FirstParagraphIndex].LogPosition;
  if LogPosition < AFirstParagraphLogPosition then
    Result := 1
  else if LogPosition > AFirstParagraphLogPosition then
  begin
    ALastParagraph := ASection.DocumentModel.MainPart.Paragraphs
      [ASection.LastParagraphIndex];
...
procedure TdxFormatterPosition.Init(ARunIndex: TdxRunIndex;
  AOffset, ABoxIndex: Integer);
begin
  RunIndex := ARunIndex;
  Offset := AOffset;
  BoxIndex := ABoxIndex;
...
constructor TdxFormatterPosition.Create(ARunIndex: TdxRunIndex;
  AOffset, ABoxIndex: Integer);
begin
...
class operator TdxFormatterPosition.Equal(const A,
  B: TdxFormatterPosition): Boolean;
begin
  Result := (A.Offset = B.Offset) and (A.RunIndex = B.RunIndex);
end;

class operator TdxFormatterPosition.NotEqual(const A,
  B: TdxFormatterPosition): Boolean;
begin
  Result := not(A = B);
end;

class operator TdxFormatterPosition.LessThan(const A, B: TdxFormatterPosition)
  : Boolean;
begin
  Result := (A.RunIndex < B.RunIndex) or
    ((A.RunIndex = B.RunIndex) and (A.Offset < B.Offset));
end;

class operator TdxFormatterPosition.LessThanOrEqual(const A,
  B: TdxFormatterPosition): Boolean;
begin
  Result := not(A > B);
end;

class operator TdxFormatterPosition.GreaterThan(const A,
  B: TdxFormatterPosition): Boolean;
begin
  Result := (A.RunIndex > B.RunIndex) or
    ((A.RunIndex = B.RunIndex) and (A.Offset > B.Offset));
...
class operator TdxFormatterPosition.GreaterThanOrEqual(const A,
  B: TdxFormatterPosition): Boolean;
begin
  Result := not(A < B);
end;
...
procedure TdxDocumentModelPosition.SetLogPosition
  (Value: TdxDocumentLogPosition);
begin
  FLogPosition := Value;
end;
...
procedure TdxDocumentModelPosition.SetRunStartLogPosition
  (Value: TdxDocumentLogPosition);
begin
  FRunStartLogPosition := Value;
end;

procedure TdxDocumentModelPosition.CopyFrom(const APos
  : TdxDocumentModelPosition);
begin
...
begin
  ...
  RunStartLogPosition := PieceTable.FindRunStartLogPosition
    (PieceTable.Paragraphs[ParagraphIndex], LogPosition, ARunIndex);
  RunIndex := ARunIndex;
end;
...
class function TdxDocumentModelPosition.FromParagraphEnd
...
class operator TdxDocumentModelPosition.Equal(const A,
  B: TdxDocumentModelPosition): Boolean;
begin
  Result := A.LogPosition = B.LogPosition;
end;

class operator TdxDocumentModelPosition.NotEqual(const A,
  B: TdxDocumentModelPosition): Boolean;
begin
  Result := A.LogPosition <> B.LogPosition;
end;

class operator TdxDocumentModelPosition.LessThan(const A,
  B: TdxDocumentModelPosition): Boolean;
begin
  Result := A.LogPosition < B.LogPosition;
end;

class operator TdxDocumentModelPosition.LessThanOrEqual(const A,
  B: TdxDocumentModelPosition): Boolean;
begin
  Result := A.LogPosition <= B.LogPosition;
end;

class operator TdxDocumentModelPosition.GreaterThan(const A,
  B: TdxDocumentModelPosition): Boolean;
begin
  Result := A.LogPosition > B.LogPosition;
end;

class operator TdxDocumentModelPosition.GreaterThanOrEqual(const A,
  B: TdxDocumentModelPosition): Boolean;
begin
  Result := A.LogPosition >= B.LogPosition;
end;

class function TdxDocumentModelPosition.FromParagraphStart
  (APieceTable: TdxCustomPieceTable; AParagraphIndex: TdxParagraphIndex)
  : TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.Create(APieceTable);
...
class function TdxDocumentModelPosition.FromDocumentEnd
  (APieceTable: TdxCustomPieceTable): TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.Create(APieceTable);
...
class function TdxDocumentModelPosition.FromRunEnd
  (APieceTable: TdxCustomPieceTable; ARunIndex: TdxRunIndex)
  : TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.Create(APieceTable);
...
class function TdxDocumentModelPosition.FromRunStart
  (APieceTable: TdxCustomPieceTable; ARunIndex: TdxRunIndex)
  : TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.Create(APieceTable);
...
class procedure TdxDocumentModelPosition.SetParagraphEnd
  (APos: PdxDocumentModelPosition; AParagraphIndex: TdxParagraphIndex);
var
...
begin
  ...
  APos^.RunStartLogPosition := APos^.LogPosition - APieceTable.Runs
    [AParagraph.LastRunIndex].Length;
end;

class procedure TdxDocumentModelPosition.SetRunStart
  (var APos: TdxDocumentModelPosition; ARunIndex: TdxRunIndex);
var
  I, AOffset: Integer;
  APieceTable: TdxCustomPieceTable;
  AParagraph: TdxParagraphBase;
...
class procedure TdxDocumentModelPosition.SetRunEnd
  (var APos: TdxDocumentModelPosition; ARunIndex: TdxRunIndex);
var
  I, AOffset: Integer;
  APieceTable: TdxCustomPieceTable;
...
class function TdxDocumentModelPosition.MoveBackward
...
class procedure TdxDocumentModelPosition.MoveBackwardCore
...
class function TdxDocumentModelPosition.MoveForward
  (const APos: TdxDocumentModelPosition): TdxDocumentModelPosition;
begin
  Result := APos;
...
class procedure TdxDocumentModelPosition.MoveForwardCore
  (var APos: TdxDocumentModelPosition);
begin
  APos.LogPosition := APos.LogPosition + 1;
...
class procedure TdxDocumentModelPosition.SetParagraphStart
  (var APos: TdxDocumentModelPosition; AParagraphIndex: TdxParagraphIndex);
var
  AParagraph: TdxParagraphBase;
  APieceTable: TdxCustomPieceTable;
begin
...
function TdxRunInfo.CreateDocumentModelPosition(APieceTable
  : TdxCustomPieceTable): TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.Create(APieceTable);
end;
...
begin
  ...
  begin
    ...
    Result := (AInfo.Start = Start) and (AInfo.&End = &End);
  end;
end;
...
class function TdxPositionConverter.ToDocumentModelPosition
  (APieceTable: TdxCustomPieceTable; APos: TdxDocumentLogPosition)
  : TdxDocumentModelPosition;
begin
  Result := TdxDocumentModelPosition.Create(APieceTable);
...
class function TdxPositionConverter.ToDocumentModelPosition
  (APieceTable: TdxCustomPieceTable; APos: TdxFormatterPosition)
  : TdxDocumentModelPosition;
var
  ARunIndex: TdxRunIndex;
  AParagraph: TdxParagraphBase;
...
class function TdxPositionConverter.GetRunStartLogPosition
  (ARunIndex: TdxRunIndex; AParagraph: TdxParagraphBase)
  : TdxDocumentLogPosition;
var
...
class function TdxPositionConverter.ToFormatterPosition
  (const APos: TdxDocumentModelPosition): TdxFormatterPosition;
begin
...
constructor TdxRunBase.Create(AParagraph: TdxParagraphBase;
...
constructor TdxCustomPieceTable.Create(const ADocumentModel
...
procedure TdxCustomPieceTable.ApplyChanges(AChangeType
  : TdxDocumentModelChangeType; AStartRunIndex, AEndRunIndex: TdxRunIndex);
begin
  FContentType.ApplyChanges(AChangeType, AStartRunIndex, AEndRunIndex);
end;

procedure TdxCustomPieceTable.ApplyChangesCore(const Actions
  : TdxDocumentModelChangeActions; AStartRunIndex, AEndRunIndex: Integer);
begin
  FContentType.ApplyChangesCore(Actions, AStartRunIndex, AEndRunIndex);
end;

procedure TdxCustomPieceTable.ApplySectionFormatting(ALogPositionStart
...
function TdxCustomPieceTable.CalculateRunIndex(ARun: TdxRunBase;
  ADefaultResultWhenNotFound: TdxRunIndex = 0): TdxRunIndex;
var
  AParagraph: TdxParagraphBase;
...
function TdxCustomPieceTable.FindParagraphIndex(ALogPosition
  : TdxDocumentLogPosition; AStrictSearch: Boolean = True): TdxParagraphIndex;
begin
...
function TdxCustomPieceTable.FindRunStartLogPosition
  (AParagraph: TdxParagraphBase; ALogPosition: TdxDocumentLogPosition;
  out ARunIndex: TdxRunIndex): TdxDocumentLogPosition;
var
...
function TdxCustomPieceTable.GetRunLogPosition(ARunIndex: TdxRunIndex)
  : TdxDocumentLogPosition;
var
  I: Integer;
  ARun: TdxRunBase;
...
function TdxCustomPieceTable.GetRunLogPosition(ARun: TdxRunBase)
  : TdxDocumentLogPosition;
var
  I: Integer;
  AParagraph: TdxParagraphBase;
...
procedure TdxCustomPieceTable.CalculateRunInfoStart(ALogPosition
  : TdxDocumentLogPosition; AResult: TdxRunInfo);
var
  ARunIndex: TdxRunIndex;
begin
  ...
  AResult.Start.RunStartLogPosition := FindRunStartLogPosition
    (Paragraphs[AResult.Start.ParagraphIndex], ALogPosition, ARunIndex);
  AResult.Start.RunIndex := ARunIndex;
end;

procedure TdxCustomPieceTable.CalculateRunInfoEnd(ALogPosition
  : TdxDocumentLogPosition; AResult: TdxRunInfo);
var
  ARunIndex: TdxRunIndex;
begin
  ...
  AResult.&End.RunStartLogPosition := FindRunStartLogPosition
    (Paragraphs[AResult.&End.ParagraphIndex], ALogPosition, ARunIndex);
  AResult.&End.RunIndex := ARunIndex;
end;

function TdxCustomPieceTable.FindRunInfo(ALogPositionStart
...
function TdxCustomPieceTable.FindParagraphIndexCore(ALogPosition
  : TdxDocumentLogPosition): TdxParagraphIndex;
begin
  Result := Paragraphs.SearchByLogPosition(ALogPosition);
end;

function TdxCustomPieceTable.LookupSectionIndexByParagraphIndex(AParagraphIndex
  : TdxParagraphIndex): TdxSectionIndex;
begin
...
function TdxDocumentHistory.CheckIsTransactionChangeModifiedBackward(AIndex,
  AStartInnerIndex: Integer): Boolean;
var
...
function TdxDocumentHistory.CheckIsTransactionChangeModifiedForward(AIndex,
  AStartInnerIndex: Integer): Boolean;
var
  ATransaction: TdxCompositeHistoryItem;
  I: Integer;
...
function TdxDocumentHistory.CommitAsSingleItemCore(ASingleItem: TdxHistoryItem)
  : TdxHistoryItem;
begin
  Add(ASingleItem);
  Result := ASingleItem;
...
function TdxDocumentHistory.IsModified(AUnmodifiedIndex,
  AUnmodifiedTransactionIndex: Integer): Boolean;
var
  I: Integer;
begin
  if CurrentIndex = AUnmodifiedIndex then
  begin
    Result := (AUnmodifiedTransactionIndex >= 0) and
      (CheckIsTransactionChangeModifiedBackward(CurrentIndex,
      AUnmodifiedTransactionIndex));
    Exit;
  end;
  ...
  begin
    ...
    begin
      if CheckIsTransactionChangeModifiedForward(AUnmodifiedIndex,
        AUnmodifiedTransactionIndex + 1) then
        Exit(True);
      Inc(AUnmodifiedIndex);
    end;
    I := AUnmodifiedIndex;
  ...
  begin
    if AUnmodifiedTransactionIndex >= 0 then
    begin
      CheckIsTransactionChangeModifiedBackward(AUnmodifiedIndex,
        AUnmodifiedTransactionIndex);
...
procedure TdxDocumentHistory.SetModifiedTextAppended(FForceRaiseModifiedChanged
  : Boolean);
begin
...
procedure TdxHistoryTransaction.HistoryDestroyingHandler(Sender: TObject;
  Args: TdxEventArgs);
begin
...
begin
  Result := (LeftMargin <> 0) or (TopMargin <> 0) or (RightMargin <> 0) or
    (BottomMargin <> 0);
end;

{ TdxCustomDocumentModelDeferredChanges }

constructor TdxCustomDocumentModelDeferredChanges.Create(ADocumentModel
  : TdxCustomDocumentModel);
begin
...
function TdxCustomDocumentModel.FindSectionIndex(ALogPosition
  : TdxDocumentLogPosition; AStrictSearch: Boolean = True): TdxSectionIndex;
var
  APredicate: TdxSectionAndLogPositionComparable;
begin
  APredicate := TdxSectionAndLogPositionComparable.Create(ALogPosition);
  if not TdxAlgorithms1<TdxCustomSection>.BinarySearch(Sections,
    APredicate.CompareTo, Result) then
  begin
...
procedure TdxCustomDocumentModel.SetActivePieceTable
  (APieceTable: TdxCustomPieceTable);
begin
end;
...
procedure TdxCustomDocumentModel.SwitchToNormalHistory(ADisposeHistory
  : Boolean);
begin
  if ADisposeHistory then
    DisposeHistory;
...
function TdxCustomDocumentModel.GetService(const AServiceType: TdxServiceType)
  : IInterface;
begin
...
procedure TdxCustomDocumentModel.AddService(const AServiceType: TdxServiceType;
...
procedure TdxCustomDocumentModel.RemoveService(const AServiceType
  : TdxServiceType);
begin
  if FServiceManager <> nil then
...
function TdxCustomDocumentModel.GetPieceTables(AIncludeUnreferenced: Boolean)
  : TdxFastList;
begin
...
begin
  Result := BatchUpdateHelper.IsUpdateLocked or
    BatchUpdateHelper.OverlappedTransaction;
end;
...
procedure TdxCustomDocumentModel.SetDocumentCapabilities
  (const Value: TdxCustomDocumentCapabilitiesOptions);
begin
...
function TdxCustomDocumentModel.CreateDocumentModelUnitConverter
...
procedure TdxCustomDocumentModel.SetLayoutUnit(const Value
...
begin
  FreeAndNil(FToDocumentLayoutUnitConverter);
  FToDocumentLayoutUnitConverter := FUnitConverter.CreateConverterToLayoutUnits
    (LayoutUnit);
  FreeAndNil(FLayoutUnitConverter);
  FLayoutUnitConverter := TdxDocumentLayoutUnitConverter.CreateConverter
    (LayoutUnit, ScreenDpi);
end;
...
procedure TdxContentTypeBase.ApplyChanges(AChangeType
  : TdxDocumentModelChangeType; AStartRunIndex, AEndRunIndex: TdxRunIndex);
begin
  DocumentModel.ApplyChanges(PieceTable, AChangeType, AStartRunIndex,
    AEndRunIndex);
end;

procedure TdxContentTypeBase.ApplyChangesCore
  (AActions: TdxDocumentModelChangeActions;
  AStartRunIndex, AEndRunIndex: TdxRunIndex);
begin
  DocumentModel.ApplyChangesCore(PieceTable, AActions, AStartRunIndex,
    AEndRunIndex);
end;
...
procedure TdxContentTypeBase.ApplySectionFormatting(ALogPositionStart
  : TdxDocumentLogPosition; ALength: Integer;
  AModifier: TdxSectionPropertyModifierBase);
begin
...
function TdxContentTypeBase.LookupSectionIndexByParagraphIndex(AParagraphIndex
  : TdxParagraphIndex): TdxSectionIndex;
begin
  Result := DocumentModel.LookupSectionIndexByParagraphIndex(AParagraphIndex);
end;

function TdxContentTypeBase.CreateSpellCheckerManager
  (APieceTable: TdxCustomPieceTable): TObject { TdxSpellCheckerManager };
begin
  Result := APieceTable.DocumentModel.CreateEmptySpellCheckerManager
    (APieceTable);
end;
