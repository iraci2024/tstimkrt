...
uses
  SysUtils, Character, Generics.Defaults, Generics.Collections, Classes,
  Controls, Rtti,
  dxSpellCheckerCore,
  dxRichEdit.Types,
...
type
  ...
  public
    ...
    property DocumentServer: TdxInnerRichEditDocumentServer
      read FDocumentServer;
  ...
  protected
    ...
    procedure ReplaceWithText(const AInfo: TdxAutoCorrectInfo;
      const AReplaceString: string);
    procedure ReplaceWithTextCore(const AInfo: TdxAutoCorrectInfo;
      const AReplaceString: string; ALength: Integer);
    procedure UpdateCaretPositionX(const AInfo: TdxAutoCorrectInfo);
    procedure ReplaceWithHyperlink(const AInfo: TdxAutoCorrectInfo;
      const AHyperlinkInfo: TdxHyperlinkInfo);
    procedure ReplaceWithHyperlinkCore(const AInfo: TdxAutoCorrectInfo;
      const AHyperlinkInfo: TdxHyperlinkInfo; ALength: Integer);
    procedure ReplaceWithPicture(const AInfo: TdxAutoCorrectInfo;
  ...
  TdxConditionalWordReplaceAutoCorrectProvider = class abstract
    (TInterfacedObject, IdxAutoCorrectProvider)
  strict private
    FDocumentServer: TdxInnerRichEditDocumentServer;
    function GetDocumentModel: TdxDocumentModel;
  protected
    function IsValidInitialText(const AText: string): Boolean; virtual;
    function BeforeFirstGetText(const AInfo: TdxAutoCorrectInfo)
      : Boolean; virtual;
    function IsAutoCorrectAllowed(const AInfo: TdxAutoCorrectInfo)
      : Boolean; virtual;
    function IsSeparator(ACh: Char): Boolean; virtual;
    function IsTriggerChar(ACh: Char): Boolean; virtual;
    function CalculateWordReplacement(const AInfo: TdxAutoCorrectInfo): TValue;
      virtual; abstract;
  public
...
type
  ...
  TdxUrlAutoCorrectProvider = class
    (TdxConditionalWordReplaceAutoCorrectProvider)
  strict private
  class var
    FUrlRegex: TRegEx;
  ...
  protected
    ...
    function CalculateWordReplacement(const AInfo: TdxAutoCorrectInfo)
      : TValue; override;
  ...
  TdxSpellCheckerAutoCorrectProvider = class
    (TdxConditionalWordReplaceAutoCorrectProvider)
  strict private
    FRule: IdxSpellCheckerAutoCorrectCustomRule;
  protected
...
function TdxAutoCorrectRichEditTextController.GetWordIterator
...
constructor TdxAutoCorrectInfo.Create(const ADocumentServer
...
function TdxAutoCorrectInfo.CreateIterator
  : TdxVisibleCharactersStopAtFieldsDocumentModelIterator;
begin
  Result := TdxVisibleCharactersStopAtFieldsDocumentModelIterator.Create
    (PieceTable);
end;
...
begin
  ...
  AValue := TdxTextRunBaseAccess(ARun).GetPlainText(PieceTable.TextBuffer,
    FEnd.RunOffset, FEnd.RunOffset);
...
begin
  ...
  begin
    ...
    AValue := TdxTextRunBaseAccess(ARun).GetPlainText(PieceTable.TextBuffer,
      FStart.RunOffset, FStart.RunOffset);
    if Length(AValue) > 1 then
      AValue := TdxStringHelper.Substring(AValue, 0, 1);
...
constructor TdxAutoCorrectService.Create(const ADocumentServer
  : TdxInnerRichEditDocumentServer);
begin
  inherited Create;
  FDocumentServer := ADocumentServer;
...
begin
  ...
  InnerRegisterProvider(TdxSpellCheckerAutoCorrectProvider.Create
    (DocumentServer));
end;
...
procedure TdxAutoCorrectService.ApplyAutoCorrectInfo
  (const AInfo: TdxAutoCorrectInfo);
var
...
procedure TdxAutoCorrectService.ReplaceWithText(const AInfo: TdxAutoCorrectInfo;
  const AReplaceString: string);
var
  ALength: Integer;
...
begin
  ...
  AInnerInfo := AInfo;
  TdxUIThreadSyncService.EnqueueInvokeInUIThread(FDocumentServer, procedure
    begin
      ReplaceWithTextCore(AInnerInfo, AReplaceString, ALength);
    end);
end;

procedure TdxAutoCorrectService.ReplaceWithTextCore
  (const AInfo: TdxAutoCorrectInfo; const AReplaceString: string;
ALength: Integer);
begin
  AInfo.DocumentServer.BeginUpdate;
  try
    AInfo.PieceTable.ApplyAutoCorrect(AInfo.Start.LogPosition, ALength,
      AReplaceString, AInfo.Rule);
...
procedure TdxAutoCorrectService.UpdateCaretPositionX
  (const AInfo: TdxAutoCorrectInfo);
var
...
procedure TdxAutoCorrectService.ReplaceWithHyperlink
  (const AInfo: TdxAutoCorrectInfo; const AHyperlinkInfo: TdxHyperlinkInfo);
var
  ALength: Integer;
  AInnerInfo: TdxAutoCorrectInfo;
begin
...
procedure TdxAutoCorrectService.ReplaceWithHyperlinkCore
  (const AInfo: TdxAutoCorrectInfo; const AHyperlinkInfo: TdxHyperlinkInfo;
ALength: Integer);
begin
  AInfo.DocumentServer.BeginUpdate;
  try
    AInfo.PieceTable.CreateHyperlink(AInfo.Start.LogPosition, ALength,
      AHyperlinkInfo);
    UpdateCaretPositionX(AInfo);
  finally
    AInfo.DocumentServer.EndUpdate;
...
procedure TdxAutoCorrectService.ReplaceWithPicture
  (const AInfo: TdxAutoCorrectInfo; AImage: TdxOfficeImageReference);
var
  ALength: Integer;
  AInnerInfo: TdxAutoCorrectInfo;
begin
...
procedure TdxAutoCorrectService.ReplaceWithPictureCore
  (const AInfo: TdxAutoCorrectInfo; AImage: TdxOfficeImageReference;
ALength: Integer);
begin
  AInfo.DocumentServer.BeginUpdate;
  try
    AInfo.PieceTable.ReplaceTextWithPicture(AInfo.Start.LogPosition,
      ALength, AImage);
    UpdateCaretPositionX(AInfo);
  finally
    AInfo.DocumentServer.EndUpdate;
...
procedure TdxAutoCorrectService.RegisterProvider(const AProvider
  : IdxAutoCorrectProvider);
begin
...
procedure TdxAutoCorrectService.InnerRegisterProvider(const AProvider
  : IdxAutoCorrectProvider);
begin
  if AProvider = nil then
    Exit;
...
procedure TdxAutoCorrectService.UnregisterProvider(const AProvider
  : IdxAutoCorrectProvider);
var
  AIndex: Integer;
begin
...
constructor TdxConditionalWordReplaceAutoCorrectProvider.Create(ADocumentServer
  : TdxInnerRichEditDocumentServer);
begin
  inherited Create;
  FDocumentServer := ADocumentServer;
...
function TdxConditionalWordReplaceAutoCorrectProvider.GetDocumentModel
  : TdxDocumentModel;
begin
  Result := FDocumentServer.DocumentModel;
end;

function TdxConditionalWordReplaceAutoCorrectProvider.CalculateAutoCorrectInfo
  : TdxAutoCorrectInfo;
var
  AInfo: TdxAutoCorrectInfo;
  AText: string;
...
function TdxConditionalWordReplaceAutoCorrectProvider.IsValidInitialText
  (const AText: string): Boolean;
begin
  Result := (AText <> '') and not IsTriggerChar(AText[1]);
end;

function TdxConditionalWordReplaceAutoCorrectProvider.BeforeFirstGetText
  (const AInfo: TdxAutoCorrectInfo): Boolean;
begin
  AInfo.DecrementEndPosition;
  Result := AInfo.DecrementStartPosition;
end;

function TdxConditionalWordReplaceAutoCorrectProvider.IsAutoCorrectAllowed
  (const AInfo: TdxAutoCorrectInfo): Boolean;
begin
  Result := True;
end;

function TdxConditionalWordReplaceAutoCorrectProvider.IsSeparator
  (ACh: Char): Boolean;
begin
{$IFDEF DELPHIXE4}
  Result := ACh.IsPunctuation or ACh.IsSeparator or ACh.IsWhiteSpace or
    CharInSet(ACh, [#13, #10, TdxCharacters.NonBreakingSpace,
    TdxCharacters.PageBreak, TdxCharacters.ColumnBreak]);
{$ELSE}
  Result := IsPunctuation(ACh) or TCharacter.IsSeparator(ACh) or
    IsWhiteSpace(ACh) or CharInSet(ACh,
    [#13, #10, TdxCharacters.NonBreakingSpace, TdxCharacters.PageBreak,
    TdxCharacters.ColumnBreak]);
{$ENDIF}
end;

function TdxConditionalWordReplaceAutoCorrectProvider.IsTriggerChar
  (ACh: Char): Boolean;
begin
{$IFDEF DELPHIXE4}
...
begin
  FUrlRegex := TRegEx.Create
    ('(?:[a-z][\w-]+:(?:/{1,3}([^./]*:[^./]*@){0,1})|www\d{0,3}[.]|ftp[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\([^\s<>]*\))+(?:\([^\s<>]*\)|[^\s`!()\[\]{};:'#$27'".,<>?«»“”‘’])',
    [roIgnoreCase]);
  FEmailRegex := TRegEx.Create
    ('(mailto:)?[-\w!#$%&'#$27'*+/=?^_`{|}~]+(?:\.[-\w!#$%&'#$27'*+/=?^_`{|}~]+)*@(?:\w+([-\w]*\w)?\.)*[\w]+',
    [roIgnoreCase]);
end;
...
function TdxUrlAutoCorrectProvider.CalculateWordReplacement
  (const AInfo: TdxAutoCorrectInfo): TValue;
var
...
function TdxUrlAutoCorrectProvider.CreateNavigateUri
  (const AWord: string): string;
begin
  if not TdxStringHelper.Contains(AWord, ':') and
    TdxStringHelper.Contains(AWord, '@') then
    Exit('mailto:' + AWord)
  else
    Exit(AWord);
end;
...
function TdxUrlAutoCorrectProvider.IsAutoCorrectAllowed
  (const AInfo: TdxAutoCorrectInfo): Boolean;
var
  AField: TdxField;
begin
  AField := DocumentServer.DocumentModel.Selection.PieceTable.
    FindFieldByRunIndex(AInfo.&End.RunIndex);
  Result := AField = nil;
end;
...
function TdxSpellCheckerAutoCorrectProvider.CalculateWordReplacement
  (const AInfo: TdxAutoCorrectInfo): TValue;
var
...
begin
  ...
  try
    ...
    AAutoCorrectInfo.SpellingAreaFinish := TdxDocumentPosition.Create
      (TdxDocumentModelPosition.FromDocumentEnd(AInfo.&End.PieceTable));
    AAutoCorrectInfo.SpellingAreaStart := TdxDocumentPosition.Create
      (TdxDocumentModelPosition.Create(AInfo.&End.PieceTable));
    AAutoCorrectInfo.Word := AInfo.Text;
    AAutoCorrectInfo.WordPositionFinish := TdxDocumentPosition.Create
      (AInfo.&End);
    AAutoCorrectInfo.WordPositionStart := TdxDocumentPosition.Create
      (AInfo.Start);
    if ASpellChecker.AutoCorrect(TdxInnerRichEditControl(AInnerControl)
      .Owner.Control, ATextController, AAutoCorrectInfo) then
    begin
...
function TdxSpellCheckerAutoCorrectProvider.CalculateAutoCorrectInfo
  : TdxAutoCorrectInfo;
var
  ASpellChecker: IdxSpellChecker3;
begin
...
constructor TdxRichEditAutoCorrectHistoryItem.Create
...
begin
  if (FSpellChecker = nil) or
    (FSpellChecker <> TdxSpellCheckerInstance.ISpellChecker3) or (FRule = nil)
  then
    Exit;
  FRule.Undo;
end;
