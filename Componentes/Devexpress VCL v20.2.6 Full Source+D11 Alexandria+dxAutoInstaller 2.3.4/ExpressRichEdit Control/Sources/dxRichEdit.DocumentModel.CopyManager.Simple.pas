...
type
  ...
  protected
    ...
    function ShouldProcessRunParent(AInfo: TdxRunInfo): Boolean;
      virtual; abstract;
    function ShouldProcessContentInSameParent(AInfo: TdxRunInfo): Boolean;
      virtual; abstract;
    procedure ProcessRunParent(AInfo: TdxRunInfo;
      ADocumentLastParagraphSelected: Boolean); virtual; abstract;
    ...
    function ProcessHead(AInfo: TdxRunInfo;
      ADocumentLastParagraphSelected: Boolean): Integer; virtual; abstract;
  ...
  TdxDeleteContentOperationBase = class abstract
  ...
  protected
    ...
    function CreateDeleteContentOperation: TdxSelectionBasedOperation;
      virtual; abstract;
    procedure DeleteContent(const AStart, AEnd: TdxDocumentModelPosition;
      ADocumentLastParagraphSelected: Boolean); overload; virtual;
    procedure DeleteContent(AStartPosition: TdxDocumentLogPosition;
  ...
  protected
    procedure ApplySourceParagraphFormatting(ASourceParagraph, ATargetParagraph
      : TdxSimpleParagraph);
    procedure ApplySourceCharacterFormatting(ASourceRun,
      ATargetRun: TdxTextRunBase);
  public
    procedure OnTargetParagraphInserted(ASourceParagraph, ATargetParagraph
      : TdxSimpleParagraph; ASourceRun, ATargetRun: TdxTextRunBase);
...
function TdxSelectionBasedOperation.Execute(AStartLogPosition
...
begin
  if ShouldProcessContentInSameParent(AInfo) then
    Result := ProcessContentSameParent(AInfo, True,
...
begin
  ...
  if Result then
    ...
    ProcessContentInsideParent(AInfo, AAllowMergeWithNextParagraph,
      ADocumentLastParagraphSelected)
end;
...
procedure TdxDeleteContentOperationBase.DeleteContent(AStartPosition
  : TdxDocumentLogPosition; ALength: Integer;
...
begin
  ...
  try
    FDeletingResult := AOperation.Execute(AStartPosition, ALength,
      ADocumentLastParagraphSelected) or FDeletingResult;
  finally
    AOperation.Free;
  end;
end;
...
function TdxDeleteContentOperationBase.Execute(AStartLogPosition
  : TdxDocumentLogPosition; ALength: Integer;
  ADocumentLastParagraphSelected: Boolean): Boolean;
var
  ARunInfo: TdxRunInfo;
...
procedure TdxSimpleDocumentModelCopyManager.OnTargetRunInserted(ASourceRun,
  ATargetRun: TdxTextRunBase);
var
  ARunLength: Integer;
begin
  ARunLength := ASourceRun.Length;
  TargetPosition.RunStartLogPosition := TargetPosition.RunStartLogPosition +
    ARunLength;
  ...
  if TargetPieceTable = TargetModel.MainPieceTable then
    Assert(TargetPosition.RunIndex = TdxPositionConverter.
      ToDocumentModelPosition(TargetModel.MainPieceTable,
      TargetPosition.LogPosition).RunIndex);
  if FormattingCopyOptions = TdxFormattingCopyOptions.KeepSourceFormatting then
    ApplySourceCharacterFormatting(ASourceRun, ATargetRun);
end;

procedure TdxSimpleDocumentModelCopyManager.ApplySourceParagraphFormatting
  (ASourceParagraph, ATargetParagraph: TdxSimpleParagraph);
var
  ATargetFormatting: TdxRunParagraphFormatting;
  ASourceFormatting: TdxRunParagraphFormattingKey;
begin
  ASourceFormatting := TdxRunParagraphFormattingKey.Create
    (ASourceParagraph.ParagraphProperties.Index,
    ASourceParagraph.ParagraphStyleIndex,
    ASourceParagraph.GetOwnNumberingListIndex,
    ASourceParagraph.GetOwnListLevelIndex);
  if MapSourceToTargetParagraphFormatting.TryGetValue(ASourceFormatting,
    ATargetFormatting) then
  begin
    ATargetParagraph.ParagraphStyleIndex :=
      ATargetFormatting.ParagraphStyleIndex;
    ATargetParagraph.ParagraphProperties.ChangeIndexCore
      (ATargetFormatting.ParagraphPropertiesIndex, []);
  end
  else
  begin
    ...
    try
      ...
      if ASourceParagraph.LineSpacingType <> ATargetParagraph.LineSpacingType
      then
      ...
      if ASourceParagraph.FirstLineIndentType <> ATargetParagraph.FirstLineIndentType
      then
        ATargetParagraph.FirstLineIndentType :=
          ASourceParagraph.FirstLineIndentType;

      if ASourceParagraph.FirstLineIndent <> ATargetParagraph.FirstLineIndent
      then
        ATargetParagraph.FirstLineIndent := ASourceParagraph.FirstLineIndent;

      if ASourceParagraph.SuppressHyphenation <> ATargetParagraph.SuppressHyphenation
      then
        ATargetParagraph.SuppressHyphenation :=
          ASourceParagraph.SuppressHyphenation;

      if ASourceParagraph.SuppressLineNumbers <> ATargetParagraph.SuppressLineNumbers
      then
        ATargetParagraph.SuppressLineNumbers :=
          ASourceParagraph.SuppressLineNumbers;

      if ASourceParagraph.ContextualSpacing <> ATargetParagraph.ContextualSpacing
      then
        ATargetParagraph.ContextualSpacing :=
          ASourceParagraph.ContextualSpacing;

      if ASourceParagraph.PageBreakBefore <> ATargetParagraph.PageBreakBefore
      then
        ATargetParagraph.PageBreakBefore := ASourceParagraph.PageBreakBefore;

      if ASourceParagraph.BeforeAutoSpacing <> ATargetParagraph.BeforeAutoSpacing
      then
        ATargetParagraph.BeforeAutoSpacing :=
          ASourceParagraph.BeforeAutoSpacing;

      if ASourceParagraph.AfterAutoSpacing <> ATargetParagraph.AfterAutoSpacing
      then
      ...
      if ASourceParagraph.KeepLinesTogether <> ATargetParagraph.KeepLinesTogether
      then
        ATargetParagraph.KeepLinesTogether :=
          ASourceParagraph.KeepLinesTogether;

      if ASourceParagraph.WidowOrphanControl <> ATargetParagraph.WidowOrphanControl
      then
        ATargetParagraph.WidowOrphanControl :=
          ASourceParagraph.WidowOrphanControl;
    ...
    MapSourceToTargetParagraphFormatting.Add(ASourceFormatting,
      TdxRunParagraphFormatting.Create(ATargetParagraph.ParagraphStyleIndex,
      ATargetParagraph.ParagraphProperties.Index));
  end;
end;

procedure TdxSimpleDocumentModelCopyManager.ApplySourceCharacterFormatting
  (ASourceRun, ATargetRun: TdxTextRunBase);
var
  ATargetFormatting, ASourceFormatting: TdxRunCharacterFormatting;
begin
  ASourceFormatting := TdxRunCharacterFormatting.Create
    (ASourceRun.CharacterProperties.Index, ASourceRun.CharacterStyleIndex);
  if MapSourceToTargetCharacterFormatting.TryGetValue(ASourceFormatting,
    ATargetFormatting) then
  begin
    ATargetRun.CharacterStyleIndex := ATargetFormatting.CharacterStyleIndex;
    ATargetRun.CharacterProperties.ChangeIndexCore
      (ATargetFormatting.CharacterPropertiesIndex, []);
  end
  else
  begin
    ...
    MapSourceToTargetCharacterFormatting.Add(ASourceFormatting,
      TdxRunCharacterFormatting.Create(ATargetRun.CharacterStyleIndex,
      ATargetRun.CharacterProperties.Index));
  end;
end;

function TdxSimpleDocumentModelCopyManager.GetSourceModel
  : TdxSimpleDocumentModel;
begin
  Result := TdxSimpleDocumentModel(inherited SourceModel);
end;

function TdxSimpleDocumentModelCopyManager.GetSourcePieceTable
  : TdxSimplePieceTable;
begin
  Result := TdxSimplePieceTable(inherited SourcePieceTable);
end;

function TdxSimpleDocumentModelCopyManager.GetTargetModel
  : TdxSimpleDocumentModel;
begin
  Result := TdxSimpleDocumentModel(inherited TargetModel);
end;

function TdxSimpleDocumentModelCopyManager.GetTargetPieceTable
  : TdxSimplePieceTable;
begin
