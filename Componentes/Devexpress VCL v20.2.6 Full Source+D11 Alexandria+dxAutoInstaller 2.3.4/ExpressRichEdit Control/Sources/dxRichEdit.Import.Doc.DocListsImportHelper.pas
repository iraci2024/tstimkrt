...
type
  ...
  protected
    ...
    property AbstractListMapping
      : TDictionary<Integer, TdxAbstractNumberingListIndex>
      read FAbstractListMapping;
  public
    ...
    procedure InitializeListLevel(AListLevel: TdxListLevel;
      ADocListLevel: TdxDocListLevel; AListLevelIndex: Integer);
    procedure SetListLevelFormatting(AListLevel: TdxListLevel;
      ADocListLevel: TdxDocListLevel);
    procedure SetListLevelProperties(AListLevel: TdxListLevel;
      ADocListLevel: TdxDocListLevel; AListLevelIndex: Integer);
    procedure CreateList(AListOverrideFormat: TdxDocListOverrideFormat;
      AListOverrideLevelInformation: TdxDocListOverrideLevelInformation);
...
constructor TdxDocListsImportHelper.Create(AContentBuilder
  : TdxDocContentBuilder; ADocumentModel: TdxDocumentModel);
begin
  inherited Create;
  FContentBuilder := AContentBuilder;
  FDocumentModel := ADocumentModel;
  FAbstractListMapping :=
    TDictionary<Integer, TdxAbstractNumberingListIndex>.Create;
end;
...
begin
  ...
  for I := 0 to ACount - 1 do
    CreateList(AListOverrideInfo.FormatOverride[I],
      AListOverrideInfo.FormatOverrideData[I]);
end;

procedure TdxDocListsImportHelper.CreateAbstractList(ACurrentListData
  : TdxDocListData);
var
  ...
  ACount, I: Integer;
begin
  if FAbstractListMapping.ContainsKey
    (ACurrentListData.ListFormatting.ListIdentifier) then
    Exit;
  ...
  FAbstractListMapping.Add(ACurrentListData.ListFormatting.ListIdentifier,
    DocumentModel.AbstractNumberingLists.Count);
  ...
  for I := 0 to ACount - 1 do
    InitializeListLevel(TdxListLevel(AAbstractList.Levels[I]),
      ALevelsFormatting[I], I);
end;

procedure TdxDocListsImportHelper.InitializeListLevel(AListLevel: TdxListLevel;
  ADocListLevel: TdxDocListLevel; AListLevelIndex: Integer);
begin
...
procedure TdxDocListsImportHelper.SetListLevelFormatting
  (AListLevel: TdxListLevel; ADocListLevel: TdxDocListLevel);
var
...
begin
  AContainer := TdxDocCommandHelper.Traverse(ADocListLevel.CharacterUPX,
    ContentBuilder.Factory, ContentBuilder.DataReader);
  ContentBuilder.AddToGC(AContainer);

  TdxDocCommandHelper.Traverse(ADocListLevel.ParagraphUPX, AContainer,
    ContentBuilder.DataReader);
  ContentBuilder.FontManager.SetFontName(AContainer);

  if AContainer.CharacterInfo <> nil then
  begin
    ADefaultFormattingInfo := DocumentModel.Cache.CharacterFormattingCache
      [TdxCharacterFormattingInfoCache.DefaultItemIndex].Info;
    ACharacterFormattingInfo := TdxDocCharacterFormattingHelper.
      GetMergedCharacterFormattingInfo(AContainer.CharacterInfo.FormattingInfo,
      ADefaultFormattingInfo, DocumentModel);
    try
      ...
      AListLevel.ListLevelProperties.SuppressBulletResize :=
        AContainer.CharacterInfo.PictureBulletInformation.SuppressBulletResize;
    finally
  ...
  begin
    AMergedParagraphProperties := TdxMergedParagraphProperties.Create
      (AContainer.ParagraphInfo.FormattingInfo,
      AContainer.ParagraphInfo.FormattingOptions);
    try
...
procedure TdxDocListsImportHelper.SetListLevelProperties
  (AListLevel: TdxListLevel; ADocListLevel: TdxDocListLevel;
...
begin
  ...
  AListLevelProperties.ConvertPreviousLevelNumberingToDecimal :=
    ADocListLevelProperties.ConvertPreviousLevelNumberingToDecimal;
  AListLevelProperties.DisplayFormatString :=
    ADocListLevel.GetDisplayFormatString;
  ...
  if ADocListLevelProperties.RestartAfterLevelLimit then
    AListLevelProperties.RelativeRestartLevel := AListLevelIndex -
      ADocListLevelProperties.RestartLevelLimit
  else
  ...
  begin
    ...
    AListLevelProperties.TemplateCode :=
      TdxNumberingListHelper.GenerateNewTemplateCode(FDocumentModel);
  end;
end;

procedure TdxDocListsImportHelper.CreateList(AListOverrideFormat
  : TdxDocListOverrideFormat; AListOverrideLevelInformation
...
begin
  if not AbstractListMapping.TryGetValue(AListOverrideFormat.ListIdentifier,
    AIndex) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      InitializeListLevel(AOverrideLevel, ALevelFormat.OverrideLevelFormatting,
        AOverriddenLevel);
      AList.Levels[AOverriddenLevel] := AOverrideLevel;
      AOverrideLevel.SetOverrideStart(ALevelFormat.OverrideStart);
    end
    else if ALevelFormat.OverrideStart then
    begin
      (AList.Levels[AOverriddenLevel] as IdxOverrideListLevel)
        .SetOverrideStart(True);
      (AList.Levels[AOverriddenLevel] as IdxOverrideListLevel).NewStart :=
        ALevelFormat.StartAt;
    end;
  end;
end;

procedure TdxDocListsImportHelper.LinkNumberingListStyles
  (AHelper: TdxDocStylesImportHelper);
var
  AStyles: TList<TdxListStylesRecordItem>;
  ACount, AAbstractListsCount, ANumberingListStylesCount, I,
    AStyleIndex: Integer;
