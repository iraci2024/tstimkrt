...
type
  ...
  public
    constructor Create(AImporter: TdxRtfImporter;
      ANestedState: Boolean = False); reintroduce;
...
constructor TdxFontTableDestination.Create(AImporter: TdxRtfImporter;
  ANestedState: Boolean = False);
begin
...
begin
  ...
  else if AKeyword = 'fcharset' then
    FontCharsetHandler(AParameterValue)
  else if AKeyword = 'bin' then
    Result := inherited ProcessKeywordCore(AKeyword, AParameterValue,
      AHasParameter)
  else
    Result := False;
end;
...
begin
  ...
  if Length(FFontInfo.Name) = 0 then
    FFontInfo.Name := Importer.DocumentModel.Cache.CharacterFormattingInfoCache.
      DefaultItem.FontName;
...
begin
  ...
  AFontInfo := AProperties.Fonts.GetRtfFontInfoById
    (AProperties.DefaultFontNumber);
  Importer.Position.CharacterFormatting.FontName := AFontInfo.Name;
  if (AFontInfo <> AProperties.Fonts.DefaultRtfFontInfo) and
    (AFontInfo.Charset >= 0) then
    Importer.DocumentProperties.DefaultCodePage :=
      TdxEncoding.CodePageFromCharset(AFontInfo.Charset);
  Importer.SetCodePage(Importer.DocumentProperties.DefaultCodePage);
end;
