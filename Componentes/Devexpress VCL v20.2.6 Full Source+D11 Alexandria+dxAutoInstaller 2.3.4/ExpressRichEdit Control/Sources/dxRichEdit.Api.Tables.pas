...
type
  ...
  TdxNativeTableCell = class(TInterfacedObject, IdxRichEditTableCell)
  strict private
  ...
  TdxNativeTable = class(TInterfacedObject, IdxRichEditTable, IdxBatchUpdatable)
  strict private
  type
  ...
  strict private
    ...
    function GetCellsForMerging(const AMergeFrom,
      AMergeTo: IdxRichEditTableCell): TdxSelectedCellsCollection;
  ...
  TdxNativeTableCollection = class(TInterfacedObject,
    IdxRichEditTableCollection, IdxRichEditReadOnlyTableCollection)
  strict private
    FDocument: TdxNativeSubDocument;
  ...
  public
    ...
    procedure TablesCollectionChanged(ATable: TdxTable;
...
type
  ...
  TdxNativeTableBorder = class abstract(TdxNativeTableBorderBase,
  ...
  private
    class var FInstance: TdxNativeTableComparer;
...
function TdxNativeTableComparer.Compare(const Left,
  Right: IdxRichEditTable): Integer;
begin
  Result := TdxNativeTable(Left).ModelTable.Index - TdxNativeTable(Right)
    .ModelTable.Index;
end;
...
constructor TdxNativeTableCell.Create(ARow: TdxNativeTableRow;
...
begin
  ...
  try
    APieceTable.SplitTableCellsHorizontally(FRow.ModelRow.Cells[FColumnIndex],
      AColumnCount, Document.DocumentServer.Owner);
    APieceTable.SplitTableCellsVertically(FRow.ModelRow.Cells[FColumnIndex],
      ARowCount, AColumnCount, False);
  finally
...
begin
  ...
  if not FIsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedTableCellError));
end;
...
begin
  ...
  Result := Document.CreateRange(AStart.LogPosition,
    AEnd.LogPosition + AEnd.Length - AStart.LogPosition - 1);
end;
...
begin
  ...
  AEnd := Document.PieceTable.Paragraphs[ModelCell.EndParagraphIndex];
  Result := Document.CreateRange(AStart.LogPosition,
    AEnd.LogPosition + AEnd.Length - AStart.LogPosition);
end;
...
function TdxNativeTableCell.GetVerticalAlignment
  : TdxRichEditTableCellVerticalAlignment;
begin
...
begin
  Document.SetWidthUnitFixedValue
    (ModelCell.Properties.CellMargins.Bottom, Value);
end;
...
procedure TdxNativeTableCell.SetPreferredWidthType
  (const Value: TdxRichEditWidthType);
begin
...
begin
  Document.SetWidthUnitFixedValue
    (ModelCell.Properties.CellMargins.Right, Value);
end;
...
begin
  ...
  if Value <> nil then
    ...
    AStyle := nil;
  ModelCell.StyleIndex := TdxDocumentModel(ModelTable.DocumentModel)
    .TableCellStyles.IndexOf(AStyle);
end;
...
procedure TdxNativeTableCell.SetVerticalAlignment
  (const Value: TdxRichEditTableCellVerticalAlignment);
begin
  ModelCell.Properties.VerticalAlignment :=
    TdxNativeProperties.ModelVerticalAlignmentMap[Value];
end;
...
constructor TdxNativeTableRow.Create(ATable: TdxNativeTable;
  ARowIndex: Integer);
begin
  inherited Create;
  FTable := ATable;
...
begin
  ...
  if not FIsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedTableRowError));
end;
...
var
  AStart, AEnd: TdxParagraph;
begin
  AStart := Document.PieceTable.Paragraphs
    [ModelRow.FirstCell.StartParagraphIndex];
...
procedure TdxNativeTableRow.SetTableRowAlignment(const Value
...
constructor TdxNativeTable.Create(ADocument: TdxNativeSubDocument;
  ATable: TdxTable);
begin
  inherited Create;
  FDocument := ADocument;
...
procedure TdxNativeTable.MergeCells(const AMergeFrom,
  AMergeTo: IdxRichEditTableCell);
var
...
procedure TdxNativeTable.SetPreferredWidth(AWidth: Single;
  AWidthType: TdxRichEditWidthType);
begin
  PreferredWidthType := AWidthType;
  PreferredWidth := AWidth;
end;

procedure TdxNativeTable.ForEachCell(const ACellProcessor
  : TdxRichEditTableCellProcessorDelegate);
var
...
procedure TdxNativeTable.ForEachRow(const ARowProcessor
  : TdxRichEditTableRowProcessorDelegate);
var
  ARows: IdxRichEditTableRowCollection;
  I, ARowCount: Integer;
...
begin
  ModelTable.TableProperties.ResetUse
    (TdxNativeProperties.ApiMaskToModelMask(AMask));
end;
...
begin
  if not IsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedTableError));
end;
...
function TdxNativeTable.GetCellsForMerging(const AMergeFrom,
  AMergeTo: IdxRichEditTableCell): TdxSelectedCellsCollection;
var
...
begin
  ASelectionCalculator := TdxTableStructureBySelectionCalculator.Create
    (PieceTable);
  Result := ASelectionCalculator.Calculate(TdxNativeTableCell(AMergeFrom)
    .ModelCell, TdxNativeTableCell(AMergeTo).ModelCell);
end;
...
begin
  ...
  Result := TdxNativeStyleCollections.GetStyle
    (Document.MainDocument.TableStyles, ModelTable.TableStyle);
end;
...
begin
  Document.SetWidthUnitFixedValue
    (ModelTable.TableProperties.CellMargins.Bottom, Value);
end;

procedure TdxNativeTable.SetIndent(const Value: Single);
begin
  Document.SetWidthUnitFixedValue
    (ModelTable.TableProperties.TableIndent, Value);
end;

procedure TdxNativeTable.SetLeftPadding(const Value: Single);
begin
  Document.SetWidthUnitFixedValue
    (ModelTable.TableProperties.CellMargins.Left, Value);
end;
...
procedure TdxNativeTable.SetPreferredWidthType(const Value
...
begin
  Document.SetWidthUnitFixedValue
    (ModelTable.TableProperties.CellMargins.Right, Value);
end;
...
procedure TdxNativeTable.SetTableAlignment(const Value
  : TdxRichEditTableRowAlignment);
begin
...
begin
  ...
  try
    ...
    ForEachCell(
      procedure(const ACell: IdxRichEditTableCell;
...
begin
  Document.SetWidthUnitFixedValue
    (ModelTable.TableProperties.CellSpacing, Value);
end;

procedure TdxNativeTable.SetTableLayout(const Value
...
begin
  Document.SetWidthUnitFixedValue
    (ModelTable.TableProperties.CellMargins.Top, Value);
end;
...
ARowCount, AColumnCount: Integer;
AAutoFitBehavior: TdxRichEditAutoFitBehaviorType; AFixedColumnWidths: Integer)
  : IdxRichEditTable;
var
...
begin
  ...
  try
    ANewTable := PieceTable.InsertTable(ALogPosition, ARowCount, AColumnCount,
      TdxTableAutoFitBehaviorType(AAutoFitBehavior),
      FDocument.UnitsToModelUnits(AFixedColumnWidths));
    AStyleIndex := DocumentModel.TableStyles.GetStyleIndexByName
      (TdxTableStyleCollection.TableSimpleStyleName);
...
function TdxNativeTableCollection.Get(const ARange: IdxRichEditDocumentRange)
  : IdxRichEditReadOnlyTableCollection;
var
  AParagraphsRange: TdxParagraphRange;
  ANativeRange: TdxNativeDocumentRange;
  AFirstIndex, ALastIndex, ATableStartParagraphIndex, ATableEndParagraphIndex
    : TdxParagraphIndex;
...
begin
  ...
  AFirstIndex := TdxNativeDocumentPosition(ANativeRange.NormalizedStart)
    .Position.ParagraphIndex;
  ALastIndex := TdxNativeDocumentPosition(ANativeRange.NormalizedEnd)
    .Position.ParagraphIndex;
  ...
  begin
    ...
    if (ATableStartParagraphIndex >= AFirstIndex) and
      (ATableEndParagraphIndex <= ALastIndex) then
...
function TdxNativeTableCollection.IndexOf(const ATable
  : IdxRichEditTable): Integer;
begin
...
procedure TdxNativeTableCollection.TablesCollectionChanged(ATable: TdxTable;
AAction: TListNotification);
begin
  case AAction of
    lnAdded:
...
function TdxNativeTableRowCollection.InsertBefore(ARowIndex: Integer)
  : IdxRichEditTableRow;
begin
  if (ARowIndex < 0) or (ARowIndex >= Count) then
    Exit(nil);
...
function TdxNativeTableRowCollection.InsertAfter(ARowIndex: Integer)
  : IdxRichEditTableRow;
begin
  if (ARowIndex < 0) or (ARowIndex >= Count) then
    Exit(nil);
...
function TdxNativeTableRowCollection.GetItem(Index: Integer)
  : IdxRichEditTableRow;
begin
  Result := nil;
  if Index < 0 then
...
begin
  if (AColumnIndex < 0) or (AColumnIndex >= Count) then
    Exit;
  PieceTable.DeleteTableCellWithContent(FRow.ModelRow.Cells[AColumnIndex],
    Document.DocumentServer.Owner);
end;

function TdxNativeTableCellCollection.InsertBefore(AColumnIndex: Integer)
  : IdxRichEditTableCell;
var
  ACell: TdxTableCell;
begin
...
function TdxNativeTableCellCollection.InsertAfter(AColumnIndex: Integer)
  : IdxRichEditTableCell;
var
  ACell: TdxTableCell;
begin
...
function TdxNativeTableCellCollection.GetItem(Index: Integer)
  : IdxRichEditTableCell;
begin
  Result := nil;
  if Index < 0 then
...
begin
  SetBorderValues(procedure(const ABorder: TdxBorderBase)
    begin
      ABorder.Color := Value;
    end);
end;

procedure TdxNativeTableBorderBase.SetLineStyleCore
...
procedure TdxNativeTableBorderBase.SetBorderValues(const ASetter
  : TBorderValueSetter);
var
...
function TdxNativeTableBorders.CreateInsideHorizontalBorder
  : IdxRichEditTableBorder;
begin
  Result := TdxNativeTableInsideHorizontalBorder.Create(Table);
end;

function TdxNativeTableBorders.CreateInsideVerticalBorder
  : IdxRichEditTableBorder;
begin
