...
type
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      const ASuggestion: string); reintroduce;
...
procedure TdxSpellCheckerCommandBase.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
  AState.Enabled := (IsContentEditable and (SpellChecker <> nil)) and
    CanExecuteCommand;
  AState.Visible := True;
end;

function TdxSpellCheckerCommandBase.GetMisspelledInterval
  : TdxMisspelledInterval;
var
  ALogPosition: TdxDocumentLogPosition;
  AIntervals: TdxMisspelledIntervalCollection;
begin
  ALogPosition := ActivePieceTable.DocumentModel.Selection.Interval.
    NormalizedStart.LogPosition;
  AIntervals := ActivePieceTable.SpellCheckerManager.MisspelledIntervals;
  Result := AIntervals.FindInterval(ALogPosition);
end;

function TdxSpellCheckerCommandBase.GetMisspelledWord
  (AInterval: TdxMisspelledInterval): string;
begin
  Result := ActivePieceTable.GetFilteredPlainText(AInterval.Interval.Start,
    AInterval.Interval.&End, ActivePieceTable.VisibleTextFilter.IsRunVisible);
end;

{ TdxReplaceMisspellingCommand }

constructor TdxReplaceMisspellingCommand.Create(const AControl
  : IdxRichEditControl; const ASuggestion: string);
begin
...
procedure TdxReplaceMisspellingCommand.ForceExecute(const AState
  : IdxCommandUIState);
var
  ASuggestions: TArray<string>;
  AValueState: TdxDefaultValueBasedCommandUIState<string>;
begin
  AValueState := Safe < TdxDefaultValueBasedCommandUIState < string >>
    .Cast(TObject(AState));
...
function TdxReplaceMisspellingCommand.GetSuggestions
  (AInterval: TdxMisspelledInterval): TArray<string>;
var
...
function TdxReplaceMisspellingCommand.HasSuggestions
  (AInterval: TdxMisspelledInterval): Boolean;
var
  ASuggestions: TArray<string>;
begin
...
function TdxReplaceMisspellingCommand.CreateDefaultCommandUIState
  : IdxCommandUIState;
begin
...
procedure TdxAutoCorrectPlaceholderMenuCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
end;
...
begin
  ...
  try
    ...
    try
      ...
      ActivePieceTable.DeleteContent(AStart.LogPosition,
        AEnd.LogPosition - AStart.LogPosition, False);
    finally
      AController.Free;
    end;
  finally
...
begin
  ...
  SpellChecker.Ignore(Control, AWord,
    TdxDocumentPosition.Create(AInterval.Interval.Start),
    TdxDocumentPosition.Create(AInterval.Interval.&End));
end;
