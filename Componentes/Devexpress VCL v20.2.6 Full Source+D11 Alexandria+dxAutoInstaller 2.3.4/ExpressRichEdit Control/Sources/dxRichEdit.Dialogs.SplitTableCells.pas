...
{$ENDIF}
  ...
  cxLookAndFeelPainters, Menus, dxLayoutControlAdapters, dxLayoutcxEditAdapters,
  cxContainer, cxEdit, cxCheckBox,
  cxTextEdit, cxMaskEdit, cxSpinEdit, dxMeasurementUnitEdit, dxLayoutContainer,
...
type
  ...
  public
    ...
    function Validate(AValue: Integer; AControl: TWinControl): Boolean;
      overload;
...
function TdxRichEditSplitTableCellsDialogForm.GetController
...
procedure TdxRichEditSplitTableCellsDialogForm.cbMergeBeforeSplitClick
  (Sender: TObject);
begin
...
procedure TdxRichEditSplitTableCellsDialogForm.EditPropertiesValidate
...
begin
  ...
  begin
    ...
    ErrorText := Format(cxGetResourceString(@sdxRichEditInvalidSize),
      [AMinValue, AMaxValue]);
  end;
end;

procedure TdxRichEditSplitTableCellsDialogForm.edtNumberOfRowsPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxRichEditSplitTableCellsDialogForm.EditKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TdxRichEditSplitTableCellsDialogForm.
  edtNumberOfColumnsPropertiesChange(Sender: TObject);
begin
  Controller.ColumnsCount := edtNumberOfColumns.Value;
end;

procedure TdxRichEditSplitTableCellsDialogForm.FormCloseQuery(Sender: TObject;
...
begin
  ...
  cbMergeBeforeSplit.Caption := cxGetResourceString
    (@sdxRichEditSplitTableCellsDialogMergeBeforeSplit);
  lciNumberOfColumns.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSplitTableCellsDialogNumberOfColumns);
  lciNumberOfRows.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditSplitTableCellsDialogNumberOfRows);
end;
...
begin
  Result := TdxSplitTableCellsFormController.Create
    (AControllerParameters as TdxSplitTableCellsFormControllerParameters);
end;
...
begin
  ...
  FValidationEquals := TdxValidationEquals.Create(0,
    Format(cxGetResourceString(@sdxRichEditExceptionInvalidDivisor),
    [ARowsCountAfterMerge]));
...
begin
  ...
  edtNumberOfRows.Properties.MaxValue := IfThen(ARowsCountAfterMerge > 1,
    ARowsCountAfterMerge, Controller.MaxRowsCount);
...
function TdxRichEditSplitTableCellsDialogForm.Validate
  (AEditor: TcxSpinEdit): Boolean;
begin
...
begin
  ...
  FIsRowsCountValid := FValidationEquals.Validate
    (Controller.RowsCountAfterMerge mod ACurrentValue, edtNumberOfRows);
  AAllowedValues := Controller.AllowedRowsCount;
  if Assigned(AAllowedValues) and not AAllowedValues.Contains(ACurrentValue)
  then
...
constructor TdxValidationEquals.Create(AValue: Integer;
...
function TdxValidationEquals.Validate(AValue: Integer;
