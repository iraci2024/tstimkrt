...
type
  ...
  TdxDocumentViewKind = (NormalView, OutlineView, PageView);

  TdxZoomType = (None = $0000, FullPage = $1000, PageWidth = $2000);

  TdxGutterPosition = (Side = $0000, Top = $8000);

  TdxAutoformatDocumentType = (Normal, Letter, Email);
  ...
  public
    ...
    class function FromStream(AReader: TBinaryReader)
      : TdxDocumentTypographyInfo; static;
  ...
  protected
    ...
    procedure ReadOddEvenPageSettingsAndFootNoteSettings
  ...
  public
    ...
    class function FromStream(AReader: TBinaryReader; AOffset: Integer)
      : TdxDocDocumentProperties; static;
    ...
    procedure WritePageBackgroundAndDocumentProtection(AWriter: TBinaryWriter;
      AOffset: Integer);
    ...
    property DifferentOddAndEvenPages: Boolean read GetDifferentOddAndEvenPages
      write SetDifferentOddAndEvenPages;
    property DisplayBackgroundShape: Boolean read GetDisplayBackgroundShape
      write SetDisplayBackgroundShape;
    property FootNotePosition: TdxFootNotePosition read FFootNotePosition
      write FFootNotePosition;
    property FootNoteNumberingRestartType: TdxLineNumberingRestart
      read FFootNoteNumberingRestartType write FFootNoteNumberingRestartType;
    property FootNoteInitialNumber: Integer read FFootnoteInitialNumber
      write FFootnoteInitialNumber;
    ...
    property DisplayFormFieldsSelection: Boolean
      read GetDisplayFormFieldsSelection;
    ...
    property EnforceProtection: Boolean read GetEnforceProtection
      write SetEnforceProtection;
    property PasswordHash: TBytes read FPasswordHash write FPasswordHash;
    property ProtectionType: TdxDocumentProtectionType read FProtectionType
      write FProtectionType;
    property DefaultTabWidth: SmallInt read FDefaultTabWidth
      write FDefaultTabWidth;
    property HyphenationHotZone: SmallInt read FHyphenationHotZone
      write FHyphenationHotZone;
    property HyphenationConsecutiveLimit: SmallInt
      read FHyphenationConsecutiveLimit write FHyphenationConsecutiveLimit;
    ...
    property DateLastPrinter: TDateTime read FDateLastPrinted
      write FDateLastPrinted;
    property RevisionNumber: SmallInt read FRevisionNumber
      write FRevisionNumber;
    property CharactersCount: Integer read FCharactersCount
      write FCharactersCount;
    property WordsCount: Integer read FWordsCount write FWordsCount;
    property ParagraphsCount: Integer read FParagraphsCount
      write FParagraphsCount;
    property PagesCount: SmallInt read FPagesCount write FPagesCount;
    property EndNoteNumberingRestartType: TdxLineNumberingRestart
      read FEndNoteNumberingRestartType write FEndNoteNumberingRestartType;
    property EndnoteInitialNumber: Integer read FEndnoteInitialNumber
      write SetEndnoteInitialNumber;
    property EndNotePosition: TdxFootNotePosition read FEndNotePosition
      write SetEndNotePosition;
    property IncludeFootnotesAndEndnotesInWordCount: Boolean
      read GetIncludeFootnotesAndEndnotesInWordCount;
    property GutterPosition: TdxGutterPosition read FGutterPosition
      write SetGutterPosition;
    property DocumentType: TdxAutoformatDocumentType read FDocumentType
      write SetDocumentType;
    property Typography: TdxDocumentTypographyInfo read FDoptypography
      write SetTypography;
  end;
...
class function TdxDocumentTypographyInfo.FromStream(AReader: TBinaryReader)
  : TdxDocumentTypographyInfo;
begin
...
begin
  ...
  for I := 0 to 101 - 1 do
    AWriter.Write(TdxEncoding.Unicode.GetBytes
      (TArray<Char>.Create(FFollowingPuncts[I])));
...
class function TdxDocDocumentProperties.FromStream(AReader: TBinaryReader;
  AOffset: Integer): TdxDocDocumentProperties;
begin
...
procedure TdxDocDocumentProperties.SetDifferentOddAndEvenPages
...
procedure TdxDocDocumentProperties.SetDisplayBackgroundShape
  (const AValue: Boolean);
begin
  if AValue then
...
procedure TdxDocDocumentProperties.SetEndnoteInitialNumber
  (const AValue: Integer);
begin
  FEndnoteInitialNumber := AValue;
end;

procedure TdxDocDocumentProperties.SetEndNotePosition
  (const AValue: TdxFootNotePosition);
begin
  FEndNotePosition := AValue;
end;

function TdxDocDocumentProperties.
  GetIncludeFootnotesAndEndnotesInWordCount: Boolean;
begin
  Result := FIncludeFootnotesAndEndNotesInWordsCount <> 0;
end;

procedure TdxDocDocumentProperties.SetGutterPosition(const AValue
  : TdxGutterPosition);
begin
  FGutterPosition := AValue;
end;

procedure TdxDocDocumentProperties.SetDocumentType(const AValue
  : TdxAutoformatDocumentType);
begin
  FDocumentType := AValue;
end;

procedure TdxDocDocumentProperties.SetTypography(const AValue
  : TdxDocumentTypographyInfo);
begin
  FDoptypography.Free;
...
procedure TdxDocDocumentProperties.Read(AReader: TBinaryReader;
  AOffset: Integer);
begin
...
procedure TdxDocDocumentProperties.ReadOddEvenPageSettingsAndFootNoteSettings
  (AReader: TBinaryReader);
var
  AByteFlags: Byte;
  AShortFlags: Word;
begin
  ...
  FFootNotePosition := TdxFootNotePositionCalculator.CalcFootNotePosition
    (AByteFlags and $60);
  AReader.BaseStream.Seek(1, TSeekOrigin.soCurrent);
  AShortFlags := AReader.ReadUInt16;
  FFootNoteNumberingRestartType := TdxFootNoteNumberingRestartCalculator.
    CalcFootNoteNumberingRestart(AShortFlags and $0003);
...
procedure TdxDocDocumentProperties.ReadHyphenationSettings
  (AReader: TBinaryReader);
begin
...
begin
  ...
  FEndNoteNumberingRestartType := TdxFootNoteNumberingRestartCalculator.
    CalcFootNoteNumberingRestart(AShortFlags and $0003);
  FEndnoteInitialNumber := AShortFlags shr 2;
  AShortFlags := AReader.ReadWord;
  FEndNotePosition := TdxFootNotePositionCalculator.CalcFootNotePosition
    (AShortFlags and $0003);
...
procedure TdxDocDocumentProperties.ReadDocZoomAndPositionInfo
  (AReader: TBinaryReader);
var
  AShortFlags: Word;
begin
...
procedure TdxDocDocumentProperties.ReadDocumentProtection
  (AReader: TBinaryReader; AOffset: Integer);
var
...
begin
  ...
  AReader.BaseStream.Seek(AOffset + PasswordHashPosition,
  ...
  ProtectionType := TdxDocumentProtectionTypeCalculator.
    CalcDocumentProtectionType(SmallInt(AShortFlags and $0070) shr 4);
end;

procedure TdxDocDocumentProperties.ReadPageBackground(AReader: TBinaryReader;
  AOffset: Integer);
var
...
var
  ...
  AShortFlags: Word;
begin
  AByteFlags := FDifferentOddAndEvenPage or (FWidowControl shl 1) or
    (TdxFootNotePositionCalculator.
    CalcFootNotePositionTypeCodeForDocumentProperties(FootNotePosition) shl 5);
  AWriter.Write(AByteFlags);
  AWriter.BaseStream.Seek(1, TSeekOrigin.soCurrent);
  AShortFlags := Word(FFootnoteInitialNumber shl 2) or
    Word(FFootNoteNumberingRestartType);
  AWriter.Write(AShortFlags);
  AShortFlags := FOutlineDirtySave or FOnlyMacPics or FOnlyWinPics or
    FLabelDoc or FHyphCapitals or FAutoHyphen or FFormNoFields or FLinkStyles or
    FRevMarking;
  AWriter.Write(AShortFlags);

  if EnforceProtection then
    FLockAnnotations := $0010;
  AShortFlags := FBackup or FExactWordsCount or FDisplayHiddenContent or
    FDisplayFieldResults or FLockAnnotations or FMirrorMargins or
    FTrueTypeFontsByDefault or FProtectedFromEditing or
    FDisplayFormFieldsSelection or FShowRevisionMarkings or
    FPrintRevisionMarkings or FLockRevisionMarkings or FContainsEmbeddedFonts;
  AWriter.Write(AShortFlags);
end;
...
procedure TdxDocDocumentProperties.WriteHyphenationSettings
...
procedure TdxDocDocumentProperties.WritePageBackgroundAndDocumentProtection
  (AWriter: TBinaryWriter; AOffset: Integer);
var
  AOriginalOffset: Int64;
  AFlags: Word;
begin
  AOriginalOffset := AWriter.BaseStream.Position;
  AWriter.BaseStream.Seek(AOffset + PasswordHashPosition,
    TSeekOrigin.soBeginning);
  ...
  AFlags := AFlags or
    Word(TdxDocumentProtectionTypeCalculator.CalcDocumentProtectionTypeCode
    (ProtectionType) shl 4);
  AFlags := AFlags or Word(FDisplayBackgroundShape shl 7);
  AWriter.BaseStream.Seek(AOffset + BackgroundPosition,
    TSeekOrigin.soBeginning);
...
var
  AShortFlags: Word;
begin
  AShortFlags := Word(FEndnoteInitialNumber shl 2) or
    Byte(EndNoteNumberingRestartType <> TdxLineNumberingRestart.Continuous);
  AWriter.Write(AShortFlags);
  AShortFlags := Word(TdxFootNotePositionCalculator.CalcFootNotePositionTypeCode
    (EndNotePosition)) or FShadeFormFields or
    FIncludeFootnotesAndEndNotesInWordsCount;
  AWriter.Write(AShortFlags);
end;
...
procedure TdxDocDocumentProperties.WriteDocZoomAndPosiitonInfo
  (AWriter: TBinaryWriter);
var
  AShortFlags: Word;
  I: Integer;
begin
  ...
  AShortFlags := Word(FViewKind) or Word(FZoomPercentage shl 3) or
    Word(FZoomType) or Word(FGutterPosition);
  AWriter.Write(AShortFlags);
...
begin
  ...
  if (AMonths = 0) or (ADaysOfMonth = 0) then
    Exit(EncodeDate(1601, 1, 1));
  Result := EncodeDate(AYears, AMonths, ADaysOfMonth) +
    EncodeTime(AHours, AMinutes, 0, 0);
end;

procedure TdxDocDocumentProperties.WriteDTTM(AWriter: TBinaryWriter;
  ADateTime: TDateTime);
var
