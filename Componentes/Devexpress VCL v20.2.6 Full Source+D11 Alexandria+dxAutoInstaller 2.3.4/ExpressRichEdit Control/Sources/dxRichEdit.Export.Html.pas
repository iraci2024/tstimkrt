...
type
  ...
  protected
    procedure Initialize(const ATargetUri: string;
      AUseAbsolutePath: Boolean); virtual;

    procedure CreateHtmlDocument(AWriter: TdxHtmlTextWriter;
      ABody: TdxWebControlBase); virtual;
    ...
    function ExportCssPropertiesToSeparateFile
      (AStyle: TdxStyleWebControl): string;
  ...
  protected
    ...
    function CreateContentExporter(ADocumentModel: TdxDocumentModel;
      ...
      : TdxHtmlContentExporter; virtual;
  ...
  public
    constructor Create(ADocumentModel: TdxDocumentModel;
      AOptions: TdxHtmlDocumentExporterOptions); reintroduce;
    destructor Destroy; override;
...
type
  ...
  TdxServiceBasedImageRepository = class(TInterfacedObject,
    IdxOfficeImageRepository)
  strict private
    FServiceProvider: IdxServiceProvider;
    FRootUri: string;
    FRelativeUri: string;
...
constructor TdxServiceBasedImageRepository.Create(const AServiceProvider
  : IdxServiceProvider; const ARootUri, ARelativeUri: string);
begin
  inherited Create;
...
function TdxServiceBasedImageRepository.GetImageSource
  (AImage: TdxOfficeImageReference): string;
var
  AService: IdxUriProviderService;
begin
  Result := '';
  AService := FServiceProvider.GetService(IdxUriProviderService)
    as IdxUriProviderService;
...
procedure TdxHtmlExporterBase.CreateHtmlDocument(AWriter: TdxHtmlTextWriter;
  ...
  try
    ...
    try
      ...
      try
        ...
        AMeta.Attributes.Add('content', Format('text/html; charset=%s',
          [Encoding.WebName]));
...
begin
  Result := ServiceProvider.GetService(IdxScriptContainer)
    as IdxScriptContainer;
  if Result <> nil then
    Exit;
  Result := StyleControl;
end;
...
function TdxHtmlExporterBase.ExportCssPropertiesToSeparateFile
  (AStyle: TdxStyleWebControl): string;
var
  ASb: TStringBuilder;
...
begin
  ...
  try
    ...
    AService := ServiceProvider.GetService(IdxUriProviderService)
      as IdxUriProviderService;
    if AService <> nil then
...
procedure TdxHtmlExporterBase.Initialize(const ATargetUri: string;
  AUseAbsolutePath: Boolean);
var
  S: string;
begin
  if ATargetUri = '' then
  ...
  begin
    ...
    FFilesPath := S + '/' + TPath.GetFileNameWithoutExtension(ATargetUri) +
      '_files/';
  end;
  ...
  FImageRepository := TdxServiceBasedImageRepository.Create(ServiceProvider,
    FilesPath, RelativeUri);
end;

function TdxHtmlExporterBase.ShouldWriteStyles: Boolean;
begin
  Result := (StyleControl.Styles.Count > 0) or
    (TObject(ScriptContainer) <> StyleControl);
end;
...
begin
  inherited Create(ADocumentModel, AOptions);
  FOptions := AOptions;
  Initialize(FOptions.TargetUri,
    FOptions.UriExportType = TdxUriExportType.Absolute);
...
function TdxHtmlExporter.CreateContentExporter(ADocumentModel: TdxDocumentModel;
  AOptions: TdxHtmlDocumentExporterOptions): TdxHtmlContentExporter;
begin
  Result := TdxHtmlContentExporter.Create(ADocumentModel, ScriptContainer,
    ImageRepository, AOptions);
end;
...
begin
  ...
  if not TdxAlphaColors.IsEmpty(APageBackColor) then
    ABodyControl.Attributes.Add('bgcolor',
      TdxHtmlConvert.ToHtml(APageBackColor));
  ...
  begin
    ...
    if AWebSettings.LeftMargin <> 0 then
      AStyle.Add(TdxHtmlTextWriterStyle.MarginLeft,
        IntToStr(AUnitConverter.ModelUnitsToPixels(AWebSettings.LeftMargin,
        DocumentModel.DPI)) + 'px');
    if AWebSettings.TopMargin <> 0 then
      AStyle.Add(TdxHtmlTextWriterStyle.MarginTop,
        IntToStr(AUnitConverter.ModelUnitsToPixels(AWebSettings.TopMargin,
        DocumentModel.DPI)) + 'px');
    if AWebSettings.RightMargin <> 0 then
      AStyle.Add(TdxHtmlTextWriterStyle.MarginRight,
        IntToStr(AUnitConverter.ModelUnitsToPixels(AWebSettings.RightMargin,
        DocumentModel.DPI)) + 'px');
    if AWebSettings.BottomMargin <> 0 then
      AStyle.Add(TdxHtmlTextWriterStyle.MarginBottom,
        IntToStr(AUnitConverter.ModelUnitsToPixels(AWebSettings.BottomMargin,
        DocumentModel.DPI)) + 'px');
  end;
end;
...
begin
  Result := TdxHtmlExporter.Create(TdxDocumentModel(ADocumentModel),
    AOptions as TdxHtmlDocumentExporterOptions);
end;
