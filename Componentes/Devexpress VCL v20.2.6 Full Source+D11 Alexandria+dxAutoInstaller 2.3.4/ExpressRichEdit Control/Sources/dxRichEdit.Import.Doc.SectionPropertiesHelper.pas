...
type
  ...
  protected
    procedure Read(AMainStreamReader: TBinaryReader;
      ATableReader: TBinaryReader; AOffset, ASize: Integer);
  public
    constructor Create;
    destructor Destroy; override;
    class function FromStream(AMainStreamReader: TBinaryReader;
      ATableStreamReader: TBinaryReader; AOffset, ASize: Integer)
      : TdxSectionPropertiesHelper; static;
    procedure Write(AWriter: TBinaryWriter);
    procedure UpdateCurrentSectionProperties(AMainStreamReader: TBinaryReader;
      ADataStreamReader: TBinaryReader;
      APropertyContainer: TdxDocPropertyContainer);
    function GetSectionStartPositions(AReader: TBinaryReader; ACount: Integer)
      : TdxIntegerList;
    function GetSectionPropertyDescriptorOffsets(AReader: TBinaryReader;
      ACount: Integer): TdxIntegerList;
...
class function TdxSectionPropertiesHelper.FromStream(AMainStreamReader
  : TBinaryReader; ATableStreamReader: TBinaryReader; AOffset, ASize: Integer)
  : TdxSectionPropertiesHelper;
begin
...
procedure TdxSectionPropertiesHelper.Read(AMainStreamReader: TBinaryReader;
  ATableReader: TBinaryReader; AOffset, ASize: Integer);
var
...
begin
  ...
  ASectionsCount := (ASize - TdxDocConstants.CharacterPositionSize)
    div (TdxDocConstants.CharacterPositionSize + SectionPropertyDescriptorSize);
  FSectionStartPositions.Free;
  FSectionStartPositions := GetSectionStartPositions(ATableReader,
    ASectionsCount + 1);
  FSectionPropertyDescriptorOffsets.Free;
  FSectionPropertyDescriptorOffsets := GetSectionPropertyDescriptorOffsets
    (ATableReader, ASectionsCount);
end;
...
procedure TdxSectionPropertiesHelper.UpdateCurrentSectionProperties
  (AMainStreamReader: TBinaryReader; ADataStreamReader: TBinaryReader;
  APropertyContainer: TdxDocPropertyContainer);
var
  AGrpprlSize: Word;
  AGrpprl: TBytes;
begin
  if (FCurrentSectionIndex >= FSectionPropertyDescriptorOffsets.Count) or
    (FSectionPropertyDescriptorOffsets[FCurrentSectionIndex] = -1) then
    Exit;

  AMainStreamReader.BaseStream.Seek(FSectionPropertyDescriptorOffsets
    [FCurrentSectionIndex], TSeekOrigin.soBeginning);
...
function TdxSectionPropertiesHelper.GetSectionStartPositions
  (AReader: TBinaryReader; ACount: Integer): TdxIntegerList;
var
  I: Integer;
begin
...
function TdxSectionPropertiesHelper.GetSectionPropertyDescriptorOffsets
  (AReader: TBinaryReader; ACount: Integer): TdxIntegerList;
var
  I: Integer;
begin
  ...
  begin
    if FSectionStartPositions[I] = FSectionStartPositions[I + 1] then
      AReader.BaseStream.Seek(SectionPropertyDescriptorSize,
...
procedure TdxSectionPropertiesHelper.AddEntry(ASectionStartPosition: Integer;
  ASepxOffset: Integer);
begin
...
begin
  ...
  begin
    if FSectionPropertyDescriptorOffsets[I] <> -1 then
      FSectionPropertyDescriptorOffsets[I] := FSectionPropertyDescriptorOffsets
        [I] + AOffset;
  end;
end;
