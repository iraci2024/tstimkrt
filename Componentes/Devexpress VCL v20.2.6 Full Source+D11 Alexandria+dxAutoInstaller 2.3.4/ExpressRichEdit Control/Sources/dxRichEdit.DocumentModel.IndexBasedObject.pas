...
type
  ...
  TdxObtainAffectedRangeEvent = procedure(Sender: TObject;
    E: TdxObtainAffectedRangeEventArgs) of object;
  TdxObtainAffectedRangeEventHandler =
    TdxMulticastMethod<TdxObtainAffectedRangeEvent>;
  ...
  protected
    procedure InitItems(const AUnitConverter
      : IdxDocumentModelUnitConverter); virtual;
    function CreateDefaultItem(const AUnitConverter
      : IdxDocumentModelUnitConverter): T; virtual; abstract;
  ...
  public
    constructor Create(const AUnitConverter: IdxDocumentModelUnitConverter;
  ...
  TdxIndexBasedObjectValueSetter<T, U> = reference to procedure(const AInfo: T;
    const ANewValue: U); // for internal use

  TdxIndexBasedObject<TInfo: TdxCloneable; TOptions: record > = class
  ...
  protected
    ...
    procedure SetPropertyValue<U>(ASetter: TdxIndexBasedObjectValueSetter<TInfo,
      U>; const ANewValue: U);
    procedure SetInfo(const ANewInfo: TInfo);

    function PropertyEquals(const AOther: TdxIndexBasedObject<TInfo, TOptions>)
  ...
  public
    ...
    procedure ReplaceInfo(const ANewInfo: TInfo; const ANewOptions: TOptions);
      overload; {$IFDEF DELPHIXE3}inline; {$ENDIF}
  ...
  TdxIndexBasedObjectB<TInfo: TdxCloneable; TOptions: record > = class
    (TdxIndexBasedObject<TInfo, TOptions>)
  private
  ...
  TdxUndoableIndexBasedObject<T: TdxCloneable> = class(TdxIndexBasedObject,
    IdxBatchUpdateHandler, IdxBatchInitHandler)
  private
    FDocumentModelPart: TdxCustomPieceTable;
    FIndex: Integer;
    FBatchUpdateHelper: TdxBatchUpdateHelper<T>;
    function ChangeIndex(AIndex: Integer;
      const AChangeActions: TdxDocumentModelChangeActions): Boolean;
    function GetDeferredInfo: T;
    function GetDocumentModel: TdxCustomDocumentModel; {$IFDEF DELPHIXE3}inline;
    {$ENDIF}
  ...
  protected
    ...
    function CreateIndexChangedHistoryItem
      : TdxIndexChangedHistoryItemCore; virtual;
    function GetCache(const ADocumentModel: TdxCustomDocumentModel)
      : TdxUniqueItemsCache<T>; virtual; abstract;
    ...
    function GetBatchUpdateChangeActions: TdxDocumentModelChangeActions;
      virtual; abstract;
    function ReplaceInfoCore(const ANewValue: T;
      const AChangeActions: TdxDocumentModelChangeActions): Boolean;

    property DeferredInfo: T read GetDeferredInfo;

    property IsDirectNotificationsEnabled: Boolean
      read GetIsDirectNotificationsEnabled;
  public
    ...
    {$IFNDEF DELPHI17} virtual; {$ENDIF}
    procedure CopyFrom(const Source: T); overload; {$IFNDEF DELPHI17} virtual;
    {$ENDIF}
    function ReplaceInfo(const ANewValue: T;
      const AChangeActions: TdxDocumentModelChangeActions): Boolean;
    ...
    procedure ChangeIndexCore(ANewIndex: Integer;
      const AChangeActions: TdxDocumentModelChangeActions); virtual;
    ...
    property BatchUpdateChangeActions: TdxDocumentModelChangeActions
      read GetBatchUpdateChangeActions;
  ...
  TdxRichEditIndexBasedObjectValueSetter<T, U> = reference to function
    (const AInfo: T; const AValue: U): TdxDocumentModelChangeActions;
  // for internal use

  TdxRichEditIndexBasedObject<T: TdxCloneable> = class
    (TdxUndoableIndexBasedObject<T>)
  private
    FOnObtainAffectedRange: TdxObtainAffectedRangeEventHandler;
  protected
    function CanSetPropertyValue: Boolean; virtual;
    procedure SetPropertyValue<U>
    ...
    procedure RaiseObtainAffectedRange(const AArgs
      : TdxObtainAffectedRangeEventArgs); virtual;
    function GetObtainAffectedRangeListener
      : IdxObtainAffectedRangeListener; virtual;
  public
    destructor Destroy; override;
    property OnObtainAffectedRange: TdxObtainAffectedRangeEventHandler
      read FOnObtainAffectedRange;
  end;
...
constructor TdxUniqueItemsCache<T>.Create(const AUnitConverter
...
procedure TdxUniqueItemsCache<T>.InitItems(const AUnitConverter
  : IdxDocumentModelUnitConverter);
var
  ADefaultItem: T;
begin
...
constructor TdxIndexBasedObject<TInfo, TOptions>.Create(const ADocumentModel
  : TdxCustomDocumentModel; const AFormattingInfo: TInfo;
  const AFormattingOptions: TOptions);
begin
  inherited Create;
...
procedure TdxIndexBasedObject<TInfo, TOptions>.ReplaceInfo(const ANewInfo
  : TInfo; const ANewOptions: TOptions);
begin
  ReplaceInfo(ANewInfo);
  Options := ANewOptions;
end;

procedure TdxIndexBasedObject<TInfo, TOptions>.ReplaceInfo
  (const ANewInfo: TInfo);
begin
...
begin
  ...
  begin
    ...
    if AOther.DocumentModel = DocumentModel then
      Result := FInfo.Equals(AOther.Info) and
        (TComparer<TOptions>.Default.Compare(Options, AOther.Options) = 0)
    else
...
procedure TdxIndexBasedObject<TInfo, TOptions>.SetPropertyValue<U>
  (ASetter: TdxIndexBasedObjectValueSetter<TInfo, U>; const ANewValue: U);
var
  AInfo: TInfo;
...
procedure TdxIndexBasedObject<TInfo, TOptions>.CopyFromCore(const ANewInfo
  : TInfo; const ANewOptions: TOptions);
begin
...
constructor TdxIndexBasedObjectB<TInfo, TOptions>.Create
  (APieceTable: TdxCustomPieceTable;
  const ADocumentModel: TdxCustomDocumentModel; const AFormattingInfo: TInfo;
  const AFormattingOptions: TOptions);
begin
...
constructor TdxUndoableIndexBasedObject<T>.Create(const ADocumentModelPart
  : TdxCustomPieceTable);
begin
  inherited Create;
...
function TdxUndoableIndexBasedObject<T>.GetBatchUpdateHelper
...
function TdxUndoableIndexBasedObject<T>.GetDocumentModel
...
function TdxUndoableIndexBasedObject<T>.GetIsDirectNotificationsEnabled
  : Boolean;
begin
  Result := not IsUpdateLocked or
    FBatchUpdateHelper.IsDirectNotificationsEnabled;
end;
...
procedure TdxUndoableIndexBasedObject<T>.CopyFromCore
  (const Source: TdxUndoableIndexBasedObject<T>);
begin
  if Index <> Source.Index then
    SetIndex(Source.Index, BatchUpdateChangeActions);
end;

procedure TdxUndoableIndexBasedObject<T>.CopyFrom(const Source
  : TdxUndoableIndexBasedObject<T>);
begin
...
begin
  if FBatchUpdateHelper = nil then
    FBatchUpdateHelper := TdxBatchUpdateHelper<T>.Create(Self,
...
function TdxUndoableIndexBasedObject<T>.GetInfoForModification(out AIsDeferred
  : Boolean): T;
begin
...
function TdxUndoableIndexBasedObject<T>.CreateIndexChangedHistoryItem
...
begin
  Assert(FBatchUpdateHelper is TdxBatchUpdateHelper<T>);
  if not ReplaceInfo(FBatchUpdateHelper.DeferredNotifications,
    BatchUpdateChangeActions) then
...
function TdxUndoableIndexBasedObject<T>.ReplaceInfoCore(const ANewValue: T;
  const AChangeActions: TdxDocumentModelChangeActions): Boolean;
var
  AIndex: Integer;
begin
...
function TdxUndoableIndexBasedObject<T>.ReplaceInfo(const ANewValue: T;
  const AChangeActions: TdxDocumentModelChangeActions): Boolean;
begin
  if IsUpdateLocked then
    Result := False
...
function TdxUndoableIndexBasedObject<T>.GetDocumentModelPart
  : TdxCustomPieceTable;
begin
...
procedure TdxRichEditIndexBasedObject<T>.SetPropertyValue<U>
  (ASetter: TdxRichEditIndexBasedObjectValueSetter<T, U>; const ANewValue: U);
var
  AInfo: T;
...
procedure TdxRichEditIndexBasedObject<T>.ApplyChanges(const AChangeActions
...
begin
  ...
  try
    ...
    if AArgs.Start >= 0 then
      DocumentModelPart.ApplyChangesCore(AChangeActions, AArgs.Start,
        AArgs.&End);
  finally
    FreeAndNil(AArgs);
  end;
...
procedure TdxRichEditIndexBasedObject<T>.RaiseObtainAffectedRange
...
function TdxRichEditIndexBasedObject<T>.GetObtainAffectedRangeListener
  : IdxObtainAffectedRangeListener;
begin
  Result := nil;
end;
...
function TdxCloneableIUnknownObject.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
