...
type
  ...
  private
    ...
    function GetNextUnmergedCellFromCoveredCell(ACell: TdxTableCell)
      : TdxTableCell;
  ...
  protected
    ...
    procedure SelectEntireCell(ACell: TdxTableCell;
      AShouldUpdateCaretY: Boolean); virtual;
    procedure AppendTableRowAtBottom(ATable: TdxTable); virtual;

    procedure ChangeIndent; virtual;
    function CreateIncrementIndentByTheTabCommand
      : TdxIncrementIndentByTheTabCommand; virtual;
    ...
    function ShouldAppendTableRowAtBottom(ACurrentCell: TdxTableCell)
      : Boolean; virtual;
  ...
  protected
    ...
    function CreateDecrementIndentByTheTabCommand
      : TdxDecrementIndentByTheTabCommand; virtual;
...
function TdxTabKeyCommand.CreateIncrementIndentByTheTabCommand
...
begin
  ...
  begin
    if IsSelectionStartFromBeginRow and SelectedWholeRow and
      not IsSelectedOneParagraphWithTab and DocumentModel.DocumentCapabilities.ParagraphFormattingAllowed
    then
      ChangeIndent
    else
    begin
...
begin
  if IsLastCellInDirection(ACell) and
    (ACell.VerticalMerging = TdxMergingState.Restart) then
  begin
    ...
    ACellIndexInTargetRow := ACell.Table.GetAbsoluteCellIndexInRow(ATargetRow,
...
begin
  CaretPosition.TableCellTopAnchorIndex :=
...
begin
  ...
  begin
    ...
    begin
      ANextCell := ANextCell.Table.GetFirstCellInVerticalMergingGroup
        (ANextCell);
...
begin
  ...
  AStartParagraph := ActivePieceTable.Paragraphs
    [DocumentModel.Selection.Interval.Start.ParagraphIndex];
  if DocumentModel.Selection.Length = 0 then
    Result := (AStartParagraph.Length > 1) or AStartParagraph.IsInList
  else
  begin
    AEqualsStartAndEndRow := ASelectionLayout.StartLayoutPosition.Row.Equals
      (ASelectionLayout.EndLayoutPosition.Row);
    ACharacter := ASelectionLayout.EndLayoutPosition.Character;
    ASelectedUpToEndRow := (ACharacter <> nil) and
      (ACharacter.EndPos = ASelectionLayout.EndLayoutPosition.Row.Boxes.
      Last.EndPos);
...
begin
  ...
  try
    ACommand.Cell := ACell;
    ACommand.ShouldUpdateCaretTableAnchorVerticalPositionY :=
      AShouldUpdateCaretY;
    AState := ACommand.CreateDefaultCommandUIState;
    ACommand.ForceExecute(AState);
  finally
...
function TdxShiftTabKeyCommand.CreateDecrementIndentByTheTabCommand
...
function TdxShiftTabKeyCommand.GetNextCell(ACurrentCell: TdxTableCell)
  : TdxTableCell;
begin
...
begin
  if ACell.IsFirstCellInTable and (CaretPosition.TableCellTopAnchorIndex = 0)
  then
    Exit;
