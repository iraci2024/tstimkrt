...
type
  ...
  protected
    ...
    procedure InsertResult(ASourcePieceTable: TdxPieceTable;
    ...
    function CompareNumericExpression(AValue1: Single; AValue2: Single)
      : Boolean;
...
procedure TdxIfField.Initialize(APieceTable: TdxCustomPieceTable;
...
function TdxIfField.GetCalculatedValueCore(ASourcePieceTable
...
begin
  ...
  else if FFalseResult.IsValid then
...
procedure TdxIfField.InsertResult(ASourcePieceTable: TdxPieceTable;
  ATargetModel: TdxDocumentModel;
  const ASourceInterval: TdxDocumentLogInterval);
var
...
begin
  AOptions := TdxDocumentModelCopyOptions.Create(ASourceInterval.Start,
    ASourceInterval.Length);
  try
    AOptions.CopyDocumentVariables := True;
    ACopyCommand := TdxDocumentModelCopyCommand
      (ASourcePieceTable.DocumentModel.CreateDocumentModelCopyCommand
      (ASourcePieceTable, ATargetModel, AOptions));
    try
...
function TdxIfField.CompareNumericExpression(AValue1: Single;
