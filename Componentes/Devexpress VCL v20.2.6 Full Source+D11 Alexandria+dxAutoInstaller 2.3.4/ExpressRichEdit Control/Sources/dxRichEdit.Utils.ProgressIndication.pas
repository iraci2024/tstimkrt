...
type
  ...
  IdxProgressIndicationService = interface
    ...
    procedure &Begin(const ADisplayName: string;
      AMinProgress, AMaxProgress, ACurrentProgress: Integer);
    procedure &End;
  ...
  TdxRichEditProgressIndicationService = class(TInterfacedObject,
    IdxRichEditProgressIndicationService, IdxProgressIndicationService)
  strict private
  ...
  protected
    ...
    property IsProgressIndicationSuspended: Boolean
      read GetIsProgressIndicationSuspended;
  public
  ...
  public
    constructor Create(const AProvider: IdxServiceProvider);
    procedure &Begin(const ADisplayName: string; AMinProgress: Integer;
      AMaxProgress: Integer; ACurrentProgress: Integer); virtual;
...
constructor TdxRichEditProgressIndicationService.Create(const AProvider
  : IdxServiceProvider);
begin
...
function TdxRichEditProgressIndicationService.
  GetIsProgressIndicationSuspended: Boolean;
begin
  Result := FSuspendCount <> 0;
end;

procedure TdxRichEditProgressIndicationService.&Begin(const ADisplayName
  : string; AMinProgress: Integer; AMaxProgress: Integer;
  ACurrentProgress: Integer);
var
...
begin
  ...
  begin
    AService := IdxProgressIndicationService
      (FProvider.GetService(IdxProgressIndicationService));
    if AService <> nil then
      AService.&Begin(ADisplayName, AMinProgress, AMaxProgress,
        ACurrentProgress);
  end;
end;

procedure TdxRichEditProgressIndicationService.SetProgress(ACurrentProgress
  : Integer);
var
  AService: IdxProgressIndicationService;
begin
...
procedure TdxProgressIndication.&Begin(const ADisplayName: string;
  AMinProgress: Integer; AMaxProgress: Integer; ACurrentProgress: Integer);
begin
...
begin
  ...
  begin
    ...
    begin
      ANow := Now;
      if (ANow - FIndicationTime >= MinIndicationInterval) or
        (FNormalizedProgress = 100) then
      begin
        IndicateProgressCore;
        FIndicationTime := ANow;
      end;
...
begin
  Result := IdxProgressIndicationService
    (Provider.GetService(IdxProgressIndicationService));
end;
