...
type
  ...
  TdxOperationType = (Addition, Subtraction, Multiplication, Division, Power,
    Percentage, Negation);

  TdxMathTokenType = (Constant, &Operator);
  ...
  TdxInfixNotationParserState = (Start, Constant, AfterOperand, AfterPercentage,
    WaitOperator, Operator, Operand, OpenBracket, CloseBracket);

  TdxOperationPriority = (Lowest, Low, Medium, High, Highest);

  TdxAssociation = (Left, Right);
  ...
  private
    class var Operations: array [TdxOperationType] of TdxOperationHandler;
  strict private
    class constructor Initialize;
    class procedure ProcessAdditionOperation(AOperands: TStack<Double>); static;
    class procedure ProcessSubstructionOperation
      (AOperands: TStack<Double>); static;
    class procedure ProcessMultiplicationOperation
      (AOperands: TStack<Double>); static;
    ...
    class procedure ProcessPercentageOperation
      (AOperands: TStack<Double>); static;
    ...
    class procedure SetOperand(AValue: Double;
      AOperands: TStack<Double>); static;
  public
    function Calculate(const ANotation: string; const ACulture: TdxCultureInfo)
      : Double; virtual;
    function ParseInfixNotation(const ANotation: string;
      const ACulture: TdxCultureInfo): TdxMathTokenCollection;
    function ProcessTokens(AResult: TdxMathTokenCollection): TArray<Double>;
    procedure ProcessToken(const AToken: IdxMathToken;
  ...
  TdxOperationBase = class abstract(TInterfacedObject, IdxOperationBase,
    IdxMathToken)
  protected
    // IdxOperationBase
  ...
  strict private
    class var FDelimiters: TdxCharList;
  strict private
  ...
  public
    ...
    function ProcessChar(const ANotation: string; AIndex: Integer): Boolean;
      virtual; abstract;
  ...
  protected
    ...
    function ShouldEjectOperation(const AOperation: IdxOperationBase)
      : Boolean; virtual;
...
type
  ...
  public
    function ProcessChar(const ANotation: string; AIndex: Integer)
      : Boolean; override;
  end;
  ...
  TdxAfterOperandInfixNotationParserState = class
    (TdxInfixNotationParserStateBase)
  protected
  ...
  TdxAfterPercentageInfixNotationParserState = class
    (TdxAfterOperandInfixNotationParserState)
  public
    function ProcessChar(const ANotation: string; AIndex: Integer)
      : Boolean; override;
  ...
  TdxWaitOperatorInfixNotationParserState = class
    (TdxInfixNotationParserStateBase)
  public
    function ProcessChar(const ANotation: string; AIndex: Integer)
      : Boolean; override;
  ...
  strict private
    ...
    class function TryGetOperation(const AOperation: string;
      var AOperationType: TdxOperationType): Boolean; static;
  protected
    procedure TryAddOperation(ACh: Char); virtual;
    procedure ChangeStateByOperationType(AType: TdxOperationType); virtual;
    function CreateOperation(const AOperation: string)
      : IdxOperationBase; virtual;
    function CreateOperationCore(AType: TdxOperationType)
      : IdxOperationBase; virtual;
  public
    constructor Create(AParser: TdxInfixNotationParser);
    destructor Destroy; override;
  ...
  TdxOpenBracketInfixNotationParserState = class
    (TdxInfixNotationParserStateBase)
  public
    function ProcessChar(const ANotation: string; AIndex: Integer)
      : Boolean; override;
  ...
  TdxCloseBracketInfixNotationParserState = class
    (TdxInfixNotationParserStateBase)
  public
    function ProcessChar(const ANotation: string; AIndex: Integer)
      : Boolean; override;
...
const
  ResultMap: array [TdxOperationType] of string = ('Addition', 'Subtraction',
    'Multiplication', 'Division', 'Power', 'Percentage', 'Negation');
begin
...
constructor TdxInfixNotationParserStateBase.Create
...
class function TdxInfixNotationParserStateBase.CreateDelimitersTable
  : TdxCharList;
begin
...
procedure TdxInfixNotationParserStateBase.ChangeState
  (AType: TdxInfixNotationParserState);
begin
...
function TdxStartInfixNotationParserState.ProcessChar(const ANotation: string;
...
begin
  ...
  if {$IFDEF DELPHIXE4}ACh.IsDigit{$ELSE}TCharacter.IsDigit(ACh){$ENDIF} or
    (ACh = ADecimalSeparator) then
  begin
...
constructor TdxConstantInfixNotationParserState.Create
  (AParser: TdxInfixNotationParser);
begin
...
function TdxConstantInfixNotationParserState.ProcessChar(const ANotation
...
begin
  Parser.AddToken(TdxConstant.Create(StrToFloat(FNumber.ToString,
    Culture.FormatSettings)));
end;

{ TdxAfterOperandInfixNotationParserState }

function TdxAfterOperandInfixNotationParserState.ProcessChar(const ANotation
  : string; AIndex: Integer): Boolean;
begin
  if IsDelimiter(ANotation[AIndex]) then
    Exit(True);

  if {$IFDEF DELPHIXE4}ANotation[AIndex].IsDigit{$ELSE}TCharacter.IsDigit
    (ANotation[AIndex]){$ENDIF} then
...
function TdxAfterPercentageInfixNotationParserState.ProcessChar(const ANotation
  : string; AIndex: Integer): Boolean;
begin
...
function TdxOperandInfixNotationParserState.ProcessChar(const ANotation: string;
  AIndex: Integer): Boolean;
begin
  if IsDelimiter(ANotation[AIndex]) then
    Exit(True);
  ...
  else if not TryChangeState(ANotation[AIndex]) then
    TdxFieldFormatter.ThrowSyntaxError(ANotation[AIndex]);
  Result := False;
end;
...
function TdxWaitOperatorInfixNotationParserState.ProcessChar(const ANotation
  : string; AIndex: Integer): Boolean;
var
  ACh: Char;
begin
...
constructor TdxOperatorInfixNotationParserState.Create
  (AParser: TdxInfixNotationParser);
begin
  inherited Create(AParser);
...
function TdxOperatorInfixNotationParserState.ProcessChar(const ANotation
  : string; AIndex: Integer): Boolean;
var
  ACh: Char;
begin
...
class function TdxOperatorInfixNotationParserState.TryGetOperation
  (const AOperation: string; var AOperationType: TdxOperationType): Boolean;
begin
...
procedure TdxOperatorInfixNotationParserState.ChangeStateByOperationType
...
function TdxOperatorInfixNotationParserState.CreateOperation(const AOperation
  : string): IdxOperationBase;
var
...
function TdxOperatorInfixNotationParserState.CreateOperationCore
  (AType: TdxOperationType): IdxOperationBase;
begin
  case AType of
...
function TdxOpenBracketInfixNotationParserState.ProcessChar(const ANotation
  : string; AIndex: Integer): Boolean;
begin
...
function TdxCloseBracketInfixNotationParserState.ProcessChar(const ANotation
  : string; AIndex: Integer): Boolean;
begin
...
function TdxInfixNotationParser.Parse(const ANotation: string)
  : TdxMathTokenCollection;
begin
  BeginParse;
  try
    try
...
procedure TdxInfixNotationParser.ChangeState
  (AType: TdxInfixNotationParserState);
begin
  FState.Free;
  case AType of
...
procedure TdxInfixNotationParser.AddOperation(const AOperation
  : IdxOperationBase);
begin
...
function TdxInfixNotationParser.ShouldEjectOperation(const AOperation
  : IdxOperationBase): Boolean;
begin
...
class procedure TdxMathematicalCalculator.ProcessAdditionOperation
  (AOperands: TStack<Double>);
begin
  SetOperand(GetOperand(AOperands) + GetOperand(AOperands), AOperands);
end;

class procedure TdxMathematicalCalculator.ProcessSubstructionOperation
  (AOperands: TStack<Double>);
var
...
class procedure TdxMathematicalCalculator.ProcessMultiplicationOperation
  (AOperands: TStack<Double>);
begin
  SetOperand(GetOperand(AOperands) * GetOperand(AOperands), AOperands);
end;

class procedure TdxMathematicalCalculator.ProcessDivisionOperation
  (AOperands: TStack<Double>);
var
  ASecondOperand, AFirstOperand: Double;
begin
...
class procedure TdxMathematicalCalculator.ProcessPowerOperation
  (AOperands: TStack<Double>);
var
  ASecondOperand, AFirstOperand: Double;
begin
...
class procedure TdxMathematicalCalculator.ProcessPercentageOperation
  (AOperands: TStack<Double>);
begin
  SetOperand(GetOperand(AOperands) / 100, AOperands);
end;

class procedure TdxMathematicalCalculator.ProcessNegationOperation
  (AOperands: TStack<Double>);
begin
  SetOperand(-GetOperand(AOperands), AOperands);
end;

class function TdxMathematicalCalculator.GetOperand
  (AOperands: TStack<Double>): Double;
begin
...
class procedure TdxMathematicalCalculator.SetOperand(AValue: Double;
  AOperands: TStack<Double>);
begin
  AOperands.Push(AValue);
end;

function TdxMathematicalCalculator.Calculate(const ANotation: string;
  const ACulture: TdxCultureInfo): Double;
var
...
function TdxMathematicalCalculator.ParseInfixNotation(const ANotation: string;
  const ACulture: TdxCultureInfo): TdxMathTokenCollection;
var
  AParser: TdxInfixNotationParser;
begin
...
function TdxMathematicalCalculator.ProcessTokens
  (AResult: TdxMathTokenCollection): TArray<Double>;
var
  AOperands: TStack<Double>;
  AToken: IdxMathToken;
begin
...
procedure TdxMathematicalCalculator.ProcessToken(const AToken: IdxMathToken;
  AOperands: TStack<Double>);
var
  AType: TdxOperationType;
begin
