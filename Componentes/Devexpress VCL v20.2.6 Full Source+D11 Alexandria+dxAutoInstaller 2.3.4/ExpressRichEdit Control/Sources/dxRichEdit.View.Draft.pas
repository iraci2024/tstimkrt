...
uses
  ...
  cxGraphics, dxCoreClasses, cxControls, dxCoreGraphics, cxGeometry,
  dxRichEdit.Types,
...
type
  ...
  public
    ...
    procedure PopulateEqualWidthColumnsBoundsCore(const AResult: TdxRectList;
      ...
      ASpaceBetweenColumns: Integer); override;
  end;
  ...
  public
    procedure FinalizePagePrimaryFormatting(APage: TdxPage;
      ADocumentEnded: Boolean); override;
  ...
  TdxDraftViewDocumentFormattingController = class
...
procedure TdxDraftViewColumnsBoundsCalculator.PopulateColumnsBounds
...
begin
  ...
  AColumnBounds.Width := UnitConverter.ToLayoutUnits
    (AColumnInfoCollection[0].Width);
  AResult.Add(AColumnBounds);
end;

procedure TdxDraftViewColumnsBoundsCalculator.
  PopulateEqualWidthColumnsBoundsCore(const AResult: TdxRectList;
...
function TdxDraftViewColumnController.
  GetShouldZeroSpacingBeforeWhenMoveRowToNextColumn: Boolean;
begin
  Result := False;
end;

function TdxDraftViewColumnController.CalculateColumnBoundsCore
  (AColumnIndex: Integer): TRect;
begin
  Result := ColumnsBounds[0];
end;

function TdxDraftViewColumnController.CreateColumnBoundsCalculator
  : TdxColumnsBoundsCalculator;
begin
  Result := TdxDraftViewColumnsBoundsCalculator.Create
...
function TdxDraftPageBoundsCalculator.CalculatePageBounds
  (ASection: TdxSection): TRect;
begin
  Result := CalculatePageClientBounds(ASection);
end;

function TdxDraftPageBoundsCalculator.CalculatePageClientBoundsCore
  (APageWidth: Integer; APageHeight: Integer; AMarginLeft: Integer;
  AMarginTop: Integer; AMarginRight: Integer; AMarginBottom: Integer): TRect;
var
...
procedure TdxDraftViewPageController.FinalizePagePrimaryFormatting
  (APage: TdxPage; ADocumentEnded: Boolean);
var
...
function TdxDraftViewPageController.CreatePageBoundsCalculator
  : TdxPageBoundsCalculator;
begin
  Result := TdxDraftPageBoundsCalculator.Create
    (DocumentLayout.DocumentModel.ToDocumentLayoutUnitConverter);
end;

function TdxDraftViewPageController.CreateHitTestCalculator(const ARequest
...
function TdxDraftViewRowsController.GetEffectiveLineNumberingRestartType
  (ASection: TdxSection): TdxLineNumberingRestart;
begin
  Result := GetEffectiveLineNumberingRestartTypeBase(ASection);
end;

function TdxDraftViewRowsController.CreateTableGridCalculator
...
begin
  Result := TdxTableGridCalculator.Create(AWidthsCalculator, AMaxTableWidth,
    DocumentModel.LayoutOptions.DraftView.
    AllowTablesToExtendIntoMargins, False);
end;

{ TdxDraftViewBoxHitTestCalculator }

procedure TdxDraftViewBoxHitTestCalculator.ProcessColumnCollection
...
constructor TdxDraftViewDocumentFormattingController.Create(ADocumentLayout
  : TdxDocumentLayout; APieceTable: TdxPieceTable);
begin
  inherited Create(ADocumentLayout, APieceTable, nil, nil);
end;

function TdxDraftViewDocumentFormattingController.CreatePageController
  : TdxPageController;
begin
  Result := TdxDraftViewPageController.Create(DocumentLayout);
end;

function TdxDraftViewDocumentFormattingController.CreateColumnController
  : TdxColumnController;
begin
  Result := TdxDraftViewColumnController.Create(PageAreaController);
end;

function TdxDraftViewDocumentFormattingController.CreateRowController
  : TdxRowsController;
begin
  Result := TdxDraftViewRowsController.Create(PieceTable, ColumnController,
    DocumentModel.LayoutOptions.DraftView.
...
procedure TdxDraftViewPageViewInfoGenerator.SetHorizontalPageGap
  (AValue: Integer);
begin
end;
...
function TdxDraftViewPageViewInfoGenerator.CanFitPageToPageRow(APage: TdxPage;
  ARow: TdxPageViewInfoRow): Boolean;
begin
  Result := False;
end;

function TdxDraftViewPageViewInfoGenerator.CreateEmptyClone
  : TdxPageGeneratorLayoutManager;
begin
  Result := TdxDraftViewPageViewInfoGenerator.Create(FView, FViewInfo);
end;

function TdxDraftViewPageViewInfoGenerator.CreateInitialPageViewInfoRowBounds
  (Y: Integer): TRect;
begin
  Result.Init(0 { ViewPortBounds.Left } , Y, 0, 0);
end;

function TdxDraftViewPageViewInfoGenerator.CalculateFirstPageLeftOffset
...
begin
  ...
  if ADocumentProperties.DisplayBackgroundShape and
    not TdxAlphaColors.IsTransparentOrEmpty(APageBackColor) then
...
function TdxDraftView.CalculatePageContentClipBounds
...
begin
  Result := Control.InnerControl.Options.Layout.DraftView.
    MatchHorizontalTableIndentsToTextEdge;
end;
...
function TdxDraftView.CreateDocumentFormattingController
  : TdxDocumentFormattingController;
begin
  Result := TdxDraftViewDocumentFormattingController.Create(DocumentLayout,
...
function TdxDraftView.CreateDocumentLayoutExporter(APainter: TdxPainter;
  AAdapter: TdxGraphicsDocumentLayoutExporterAdapter;
  APageViewInfo: TdxPageViewInfo; const ABounds: TRect)
...
function TdxDraftView.GetPageViewInfoRowFromPoint(const APoint: TPoint;
  AStrictSearch: Boolean): TdxPageViewInfoRow;
begin
  Result := inherited GetPageViewInfoRowFromPoint(APoint, False);
end;

function TdxDraftView.PerformStrictPageViewInfoHitTest(APageViewInfo
