...
type
  ...
  public
    ...
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer; AStreamLength: Int64): TdxBinTable; static;
    procedure Write(AWriter: TBinaryWriter);
    function GetPositions(AReader: TBinaryReader; ACount: Integer)
      : TArray<Integer>;
    function GetSectorsOffsets(AReader: TBinaryReader; ACount: Integer;
      AStreamLength: Int64): TArray<Integer>;
...
class function TdxBinTable.FromStream(AReader: TBinaryReader; AOffset: Integer;
  ASize: Integer; AStreamLength: Int64): TdxBinTable;
begin
  Result := TdxBinTable.Create;
  Result.Read(AReader, AOffset, ASize, AStreamLength);
end;

procedure TdxBinTable.Read(AReader: TBinaryReader; AOffset: Integer;
  ASize: Integer; AStreamLength: Int64);
var
...
begin
  ...
  FSectorsOffsets.AddRange(GetSectorsOffsets(AReader, ABinEntriesCount,
    AStreamLength));
end;
...
begin
  ...
  for I := 0 to ACount - 1 do
    AWriter.Write(Cardinal(FSectorsOffsets[I]
      div TdxDocConstants.ContentBuilderSectorSize));
end;

function TdxBinTable.GetPositions(AReader: TBinaryReader; ACount: Integer)
  : TArray<Integer>;
var
  I: Integer;
begin
...
function TdxBinTable.GetSectorsOffsets(AReader: TBinaryReader; ACount: Integer;
  AStreamLength: Int64): TArray<Integer>;
var
...
begin
  ...
  for I := 0 to ACount - 1 do
  begin
    ACurrentOffset := TdxDocConstants.ContentBuilderSectorSize *
      (AReader.ReadInt32 and SectorOffsetBitMask);
...
begin
  ACount := FSectorsOffsets.Count;
  for I := 0 to ACount - 1 do
    FSectorsOffsets[I] := FSectorsOffsets[I] + AOffset *
      TdxDocConstants.ContentBuilderSectorSize;
end;
