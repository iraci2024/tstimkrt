...
type
  ...
  TdxScannerStateType = (Simple, Expression, Instructions, EqSwitches,
    EqInstructions);
  ...
  TdxFieldCalculatorService = class(TInterfacedObject,
    IdxFieldCalculatorService)
  strict private
  type
    TFieldTypes = class(TdxNamedDelegateDictionary<TdxCreateFieldDelegate>);
    class var FFieldTypes: TFieldTypes;
  strict private
    class constructor Initialize;
    class destructor Finalize;
  protected
    FUpdateOnLoadOptions: TdxFieldUpdateOnLoadOptions;
    class procedure RegisterFieldType(const AFieldType: string;
      const ACreator: TdxCreateFieldDelegate); static;
    procedure PrepareSimpleField(APieceTable: TdxPieceTable;
      AFirstToken: IdxToken; AScanner: TdxFieldScanner;
    ...
    function CreateInitializedCalculatedField(APieceTable: TdxSimplePieceTable;
      AFirstToken: IdxToken; AScanner: TdxFieldScanner)
      : TdxCalculatedFieldBase; virtual;
    function CalculateSimpleField(APieceTable: TdxPieceTable;
      AFirstToken: IdxToken; AScanner: TdxFieldScanner;
      AMailMergeDataMode: TdxMailMergeDataMode; ADocumentField: TdxField;
      AUpdateType: TdxUpdateFieldOperationType)
      : TdxCalculateFieldResult; virtual;
    function CalculateExpressionValue(APieceTable: TdxPieceTable;
      AScanner: TdxFieldScanner; AField: TdxField)
      : TdxCalculateFieldResult; virtual;
    function CalculateInvalidField(APieceTable: TdxPieceTable)
      : TdxCalculateFieldResult; virtual;
    function GetActualUpdateDocVariablesBeforePrint(AModel: TdxDocumentModel;
      AUpdateType: TdxUpdateFieldOperationType): Boolean;
    class function IsSwitchWithArgument(AField: TdxCalculatedFieldBase;
      AToken: IdxToken): Boolean; static;
    class function IsFieldArgument(ATokenKind: TdxTokenKind): Boolean; static;
  public
    procedure BeginUpdateFieldsOnLoad
      (AOptions: TdxFieldUpdateOnLoadOptions); virtual;
    ...
    procedure PrepareField(APieceTable: TdxCustomPieceTable; AField: TdxField;
      AUpdateType: TdxUpdateFieldOperationType);
    function ParseField(APieceTable: TdxSimplePieceTable; AField: TdxField)
      : TdxCalculatedFieldBase; virtual;
    function GetFirstToken(APieceTable: TdxPieceTable; AField: TdxField)
      : IdxToken; virtual;

    class function CreateField(const AFieldType: string)
      : TdxCalculatedFieldBase; static;
  ...
  protected
    function CreateToken(AKind: TdxTokenKind;
      APosition: TdxDocumentLogPosition = 0; const AValue: string = '';
      ALength: Integer = 1): IdxToken;
  ...
  public
    ...
    function GetNextState(AToken: IdxToken): TdxScannerStateType;
      virtual; abstract;
  ...
  strict private
    class constructor Initialize;
    class destructor Finalize;
    class procedure AddTokensToDictionary(const ATokens: TArray<string>;
      AKind: TdxTokenKind); static;
    class procedure AddTokenToDictionary(const AToken: string;
      AKind: TdxTokenKind); static;
  strict private
  ...
  protected
    function CreateToken(AKind: TdxTokenKind;
      APosition: TdxDocumentLogPosition = 0; const Value: string = '';
      ALength: Integer = 1): IdxToken;
    function GetSeparatorChar: Char; virtual;
    function GetDecimalSeparator: Char; virtual;
  public
    constructor Create(const AIterator: IdxFieldIterator;
      APieceTable: TdxPieceTable); overload;
    constructor Create(const AIterator: IdxFieldIterator;
      AMaxFieldSwitchLength: Integer; AEnableFieldNames: Boolean;
      ASupportFieldCommonStringFormat: Boolean); overload;
    ...
    property SupportFieldCommonStringFormat: Boolean
      read FSupportFieldCommonStringFormat;
...
begin
  ...
  RegisterFieldType(TdxMergefieldField.FieldType,
    TdxMergefieldField.CreateField);
  ...
  RegisterFieldType(TdxCreateDateField.FieldType,
    TdxCreateDateField.CreateField);
  RegisterFieldType(TdxIncludePictureField.FieldType,
    TdxIncludePictureField.CreateField);
  ...
  RegisterFieldType(TdxDocVariableField.FieldType,
    TdxDocVariableField.CreateField);
...
class procedure TdxFieldCalculatorService.RegisterFieldType(const AFieldType
  : string; const ACreator: TdxCreateFieldDelegate);
begin
  if not FFieldTypes.ContainsKey(AFieldType) then
    FFieldTypes.Add(AFieldType, ACreator);
end;

class function TdxFieldCalculatorService.CreateField(const AFieldType: string)
...
procedure TdxFieldCalculatorService.BeginUpdateFieldsOnLoad
...
function TdxFieldCalculatorService.CalculateField
  (APieceTable: TdxCustomPieceTable; AField: TdxField;
  AMailMergeDataMode: TdxMailMergeDataMode;
  AUpdateType: TdxUpdateFieldOperationType): TdxCalculateFieldResult;
var
...
begin
  ...
  try
    AScanner := TdxFieldScanner.Create(AIterator,
      ATable.DocumentModel.MaxFieldSwitchLength,
      ATable.DocumentModel.EnableFieldNames,
      ATable.SupportFieldCommonStringFormat);
    try
      ...
      case AToken.ActualKind of
        ...
        Result := CalculateSimpleField(ATable, AToken, AScanner,
          AMailMergeDataMode, AField, AUpdateType);
      end;
    finally
      AScanner.Free;
    end;
...
procedure TdxFieldCalculatorService.PrepareField(APieceTable
  : TdxCustomPieceTable; AField: TdxField;
  AUpdateType: TdxUpdateFieldOperationType);
var
  AIterator: TdxDocumentFieldIterator;
  AScanner: TdxFieldScanner;
...
begin
  ...
  try
    ...
    try
      AToken := AScanner.Scan;
      if (AToken.ActualKind <> TdxTokenKind.OpEQ) and
        (AToken.ActualKind <> TdxTokenKind.Eq) then
...
procedure TdxFieldCalculatorService.PrepareSimpleField
  (APieceTable: TdxPieceTable; AFirstToken: IdxToken; AScanner: TdxFieldScanner;
  ADocumentField: TdxField; AUpdateType: TdxUpdateFieldOperationType);
var
...
begin
  try
    ANeedFreeField := True;
    AField := CreateInitializedCalculatedField(APieceTable, AFirstToken,
      AScanner);
    try
      if (AField = nil) or (not AField.CanPrepare) then
        Exit;
      if not(AUpdateType in AField.GetAllowedUpdateFieldTypes
        (FUpdateOnLoadOptions)) then
        Exit;
...
function TdxFieldCalculatorService.CreateInitializedCalculatedField
  (APieceTable: TdxSimplePieceTable; AFirstToken: IdxToken;
  AScanner: TdxFieldScanner): TdxCalculatedFieldBase;
var
...
function TdxFieldCalculatorService.ParseField(APieceTable: TdxSimplePieceTable;
  AField: TdxField): TdxCalculatedFieldBase;
var
  AIterator: TdxDocumentFieldIterator;
  AScanner: TdxFieldScanner;
  AToken: IdxToken;
begin
  try
    ...
    try
      AScanner := TdxFieldScanner.Create(AIterator,
        APieceTable.DocumentModel.MaxFieldSwitchLength,
        APieceTable.DocumentModel.EnableFieldNames,
        APieceTable.SupportFieldCommonStringFormat);
      try
        AToken := AScanner.Scan;
        if (AToken.ActualKind <> TdxTokenKind.OpEQ) and
          (AToken.ActualKind <> TdxTokenKind.Eq) then
...
function TdxFieldCalculatorService.CalculateSimpleField
  (APieceTable: TdxPieceTable; AFirstToken: IdxToken; AScanner: TdxFieldScanner;
  AMailMergeDataMode: TdxMailMergeDataMode; ADocumentField: TdxField;
  AUpdateType: TdxUpdateFieldOperationType): TdxCalculateFieldResult;
var
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Exit(TdxCalculateFieldResult.Create(TdxCalculatedFieldValue.Create(nil,
          [TdxFieldResultOption.KeepOldResult]),
    ...
    if not(AUpdateType in AField.GetAllowedUpdateFieldTypes
      (FUpdateOnLoadOptions)) then
      Exit(nil);
    if (AUpdateType = TdxUpdateFieldOperationType.Copy) and
      not APieceTable.DocumentModel.FieldOptions.UpdateFieldsOnPaste then
      Exit(nil);
    if AField is TdxDocVariableField then
    begin
      AModel := APieceTable.DocumentModel;
      AUpdateBeforePrint := GetActualUpdateDocVariablesBeforePrint(AModel,
        AUpdateType);
      if not AUpdateBeforePrint then
        Exit(nil);
      if not AModel.FieldOptions.UpdateDocVariablesBeforeCopy and
        (AUpdateType = TdxUpdateFieldOperationType.Copy) then
        Exit(nil);
    end;
    AResult := AField.Update(APieceTable, AMailMergeDataMode, ADocumentField);
    Result := TdxCalculateFieldResult.Create(AResult,
      AField.GetAllowedUpdateFieldTypes(FUpdateOnLoadOptions));
  finally
    AField.Free;
  end;
end;

function TdxFieldCalculatorService.CalculateExpressionValue
  (APieceTable: TdxPieceTable; AScanner: TdxFieldScanner; AField: TdxField)
  : TdxCalculateFieldResult;
begin
  Result := nil;
end;

function TdxFieldCalculatorService.CalculateInvalidField
  (APieceTable: TdxPieceTable): TdxCalculateFieldResult;
begin
...
begin
  case AModel.DocumentProperties.UpdateDocVariablesBeforePrint of
    ...
    if not AModel.PrintingOptions.UpdateDocVariablesBeforePrint and
      ...
      Result := False
    else if not AModel.FieldOptions.UpdateDocVariablesBeforePrint and
      (AUpdateType in [TdxUpdateFieldOperationType.CreateModelForExport,
      TdxUpdateFieldOperationType.Copy]) then
      Result := False
    else
      Result := True;
  end;
...
function TdxFieldCalculatorService.GetFirstToken(APieceTable: TdxPieceTable;
  AField: TdxField): IdxToken;
var
  AIterator: TdxDocumentFieldIterator;
  AScanner: TdxFieldScanner;
begin
...
class function TdxFieldCalculatorService.ParseInstructions
  (AScanner: TdxFieldScanner; AField: TdxCalculatedFieldBase)
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            AInterval := TdxDocumentLogInterval.Create(AToken.Position,
              AFieldArgument.Position - AToken.Position);
        ...
        begin
          ...
          begin
            AInterval := TdxDocumentLogInterval.Create(AToken.Position,
              ANextToken.Position - AToken.Position);
...
class function TdxFieldCalculatorService.IsSwitchWithArgument
  (AField: TdxCalculatedFieldBase; AToken: IdxToken): Boolean;
var
  AKind: TdxTokenKind;
begin
  AKind := AToken.ActualKind;
  Result := (AKind in [TdxTokenKind.NumericFormattingSwitchBegin,
    TdxTokenKind.GeneralFormattingSwitchBegin,
    TdxTokenKind.DateAndTimeFormattingSwitchBegin,
    TdxTokenKind.CommonStringFormatSwitchBegin]) or
    AField.IsSwitchWithArgument(AToken.Value);
end;

class function TdxFieldCalculatorService.IsFieldArgument
  (ATokenKind: TdxTokenKind): Boolean;
begin
  Result := (ATokenKind = TdxTokenKind.Text) or
    (ATokenKind = TdxTokenKind.QuotedText);
end;
...
function TdxScannerStateBase.CreateToken(AKind: TdxTokenKind;
  APosition: TdxDocumentLogPosition; const AValue: string; ALength: Integer)
  : IdxToken;
begin
...
begin
  ...
  begin
    ...
    Exit(CreateToken(TdxTokenKind.OpEQ, AStart, '=',
      Iterator.Position - AStart));
  end;
...
function TdxScannerStateSimpleToken.GetNextState(AToken: IdxToken)
  : TdxScannerStateType;
begin
  case AToken.ActualKind of
    TdxTokenKind.OpEQ:
...
begin
  ...
  try
    ...
    begin
      Iterator.AdvanceNextChar;
      Exit(CreateToken(TdxTokenKind.Percent, AStart, '%',
        Iterator.Position - AStart));
    end;
    ...
    begin
      ACh := Iterator.PeekNextChar;
      if (not AFieldNameStarted and IsEndOfToken(ACh)) or
        (AFieldNameStarted and (ACh = #0)) then
        Break;
      Iterator.AdvanceNextChar;
      AVal.Append(ACh);
...
function TdxScannerStateExpression.GetNextState(AToken: IdxToken)
  : TdxScannerStateType;
begin
  ...
  try
    ...
    begin
      ...
      if not IsConstantChar(ACh) then
        Break;
      Iterator.AdvanceNextChar;
      AVal.Append({$IFDEF DELPHIXE4}ACh.ToUpper{$ELSE}TCharacter.ToUpper
        (ACh){$ENDIF});
    end;
    AStrVal := AVal.ToString;
    Result := CreateToken(TdxTokenKind.Constant, AStart, AStrVal,
      Iterator.Position - AStart);
  finally
    AVal.Free;
  end;
    ...
    '<':
      case ANextChar of
        '>':
          begin
            Iterator.AdvanceNextChar;
            Exit(CreateToken(TdxTokenKind.OpNEQ, AStart, '<>',
              Iterator.Position - AStart));
          end;
        '=':
          begin
            Iterator.AdvanceNextChar;
            Exit(CreateToken(TdxTokenKind.OpLOWEQ, AStart, '<=',
              Iterator.Position - AStart));
          end
      else
        Exit(CreateToken(TdxTokenKind.OpLOW, AStart, '<',
          Iterator.Position - AStart));
      end;
    '>':
      if ANextChar = '=' then
      begin
        Iterator.AdvanceNextChar;
        Exit(CreateToken(TdxTokenKind.OpHIEQ, AStart, '>=',
          Iterator.Position - AStart));
      end
      else
        Exit(CreateToken(TdxTokenKind.OpHI, AStart, '>',
          Iterator.Position - AStart));
    '+':
      Exit(CreateToken(TdxTokenKind.OpPLUS, AStart, '+',
        Iterator.Position - AStart));
    '-':
      Exit(CreateToken(TdxTokenKind.OpMINUS, AStart, '-',
        Iterator.Position - AStart));
    '*':
      Exit(CreateToken(TdxTokenKind.OpMUL, AStart, '*',
        Iterator.Position - AStart));
    '/':
      Exit(CreateToken(TdxTokenKind.OpDIV, AStart, '/',
        Iterator.Position - AStart));
    '^':
      Exit(CreateToken(TdxTokenKind.OpPOW, AStart, '^',
        Iterator.Position - AStart));
    '(':
      Exit(CreateToken(TdxTokenKind.OpenParenthesis, AStart, '(',
        Iterator.Position - AStart));
    ')':
      Exit(CreateToken(TdxTokenKind.CloseParenthesis, AStart, ')',
        Iterator.Position - AStart));
  else
    if ACh = Scanner.SeparatorChar then
      Exit(CreateToken(TdxTokenKind.SeparatorChar, AStart,
        Scanner.SeparatorChar, Iterator.Position - AStart))
    else
...
function TdxScannerStateExpression.ReadSwitchToken
  (AStart: TdxDocumentLogPosition): IdxToken;
var
  ACh: Char;
begin
  ACh := Iterator.PeekNextChar;
  case ACh of
    '@':
      begin
        Iterator.AdvanceNextChar;
        Exit(CreateToken(TdxTokenKind.DateAndTimeFormattingSwitchBegin, AStart,
          '\@', Iterator.Position - AStart));
      end;
    '#':
      begin
        Iterator.AdvanceNextChar;
        Exit(CreateToken(TdxTokenKind.NumericFormattingSwitchBegin, AStart,
          '\#', Iterator.Position - AStart));
      end;
    '*':
      begin
        Iterator.AdvanceNextChar;
        Exit(CreateToken(TdxTokenKind.GeneralFormattingSwitchBegin, AStart,
          '\*', Iterator.Position - AStart));
      end;
    '!':
      begin
        Iterator.AdvanceNextChar;
        Exit(CreateToken(TdxTokenKind.FieldSwitchCharacter, AStart, '\!',
          Iterator.Position - AStart));
      end;
    '$':
      ...
      begin
        Iterator.AdvanceNextChar;
        Exit(CreateToken(TdxTokenKind.CommonStringFormatSwitchBegin, AStart,
          '\$', Iterator.Position - AStart));
      end
      else
...
function TdxScannerStateExpression.ReadFieldSwitchCharacter
  (AStart: TdxDocumentLogPosition): IdxToken;
var
  AVal: string;
...
begin
  ...
  begin
    ACh := {$IFDEF DELPHIXE4}Iterator.PeekNextChar.ToLower{$ELSE}TCharacter.
      ToLower(Iterator.PeekNextChar){$ENDIF};
    if not IsLatinLetter(ACh) then
      Break;
    AVal := AVal + ACh;
    Iterator.AdvanceNextChar;
  end;
  Result := CreateToken(TdxTokenKind.FieldSwitchCharacter, AStart, AVal,
    Iterator.Position - AStart);
end;
...
begin
  ...
  try
    ...
    begin
      ACh := Iterator.PeekNextChar;
      if (Iterator.IsEnd) or (Scanner.IsWhiteSpace(ACh)) or (IsSwitchStart(ACh))
      then
        Break;
      Iterator.AdvanceNextChar;
      AVal.Append(ACh);
    ...
    AStrVal := AVal.ToString;
    Result := CreateToken(TdxTokenKind.Text, AStart, AStrVal,
      Iterator.Position - AStart);
  finally
    AVal.Free;
  end;
...
function TdxScannerStateInstructions.GetNextState(AToken: IdxToken)
  : TdxScannerStateType;
begin
  Result := StateType;
end;
...
function TdxScannerStateInstructions.ReadSwitchToken
  (AStart: TdxDocumentLogPosition): IdxToken;
var
  ACh: Char;
begin
...
function TdxScannerStateInstructions.ReadFieldSwitchCharacter
  (AStart: TdxDocumentLogPosition): IdxToken;
var
  AVal: string;
  AMaxResultLength: Integer;
...
function TdxScannerStateEqSwitches.GetNextState(AToken: IdxToken)
  : TdxScannerStateType;
begin
...
begin
  ...
  try
    ...
    begin
      Iterator.AdvanceNextChar;
      Exit(CreateToken(TdxTokenKind.SeparatorChar, AStart, ACh,
        Iterator.Position - AStart));
    end;
    while True do
    begin
...
function TdxScannerStateEqInstructions.GetNextState(AToken: IdxToken)
  : TdxScannerStateType;
begin
...
begin
  SimpleTokens := TArray<string>.Create('IF', 'COMPARE', 'CREATEDATE', 'DATE',
    'EDITTIME', 'PRINTDATE', 'SAVEDATE', 'TIME', 'DOCVARIABLE', 'GOTOBUTTON',
    'MACROBUTTON', 'PRINT', 'ADVANCE', 'SYMBOL', 'INDEX', 'RD', 'TA', 'TC',
    'TOA', 'TOC', 'XE', 'AUTOTEXT', 'AUTOTEXTLIST', 'BIBLIOGRAPHY', 'CITATION',
    'HYPERLINK', 'INCLUDEPICTURE', 'INCLUDETEXT', 'LINK', 'NOTEREF', 'PAGEREF',
    'QUOTE', 'REF', 'STYLEREF', 'ADDRESSBLOCK', 'ASK', 'DATABASE', 'FILLIN',
    'GREETINGLINE', 'MERGEFIELD', 'MERGEREC', 'MERGESEQ', 'NEXT', 'SET',
    'SKIPIF', 'NEXTIF', 'AUTONUM', 'AUTONUMLGL', 'AUTONUMOUT', 'BARCODE',
    'LISTNUM', 'PAGE', 'REVNUM', 'SECTION', 'SECTIONPAGES', 'SEQ',
    'USERADDRESS', 'USERINITIALS', 'USERNAME', 'FORMCHECKBOX', 'FORMDROPDOWN',
    'FORMTEXT');
  DocPropertyInfoCommonTokens := TArray<string>.Create('AUTHOR', 'COMMENTS',
    'KEYWORDS', 'LASTSAVEDBY', 'SUBJECT', 'TEMPLATE', 'TITLE');
  DocPropertyCategoryTokens := TArray<string>.Create('BYTES', 'CATEGORY',
    'CHARACTERS', 'CHARACTERSWITHSPACES', 'COMPANY', 'CREATETIME',
    'HYPERLINKBASE', 'LASTPRINTED', 'LASTSAVEDTIME', 'LINES', 'MANAGER',
    'NAMEOFAPPLICATION', 'ODMADOCID', 'PAGES', 'PARAGRAPHS', 'REVISIONNUMBER',
    'SECURITY', 'TOTALEDITINGTIME', 'WORDS');
  DocumentInformationTokens := TArray<string>.Create('FILENAME', 'FILESIZE',
    'INFO', 'NUMCHARS', 'NUMPAGES', 'NUMWORDS');
  TokenDictionary := TdxNamedOrdinalDictionary<TdxTokenKind>.Create;
  AddTokensToDictionary(SimpleTokens, TdxTokenKind.Simple);
  AddTokensToDictionary(DocPropertyInfoCommonTokens,
    TdxTokenKind.DocPropertyInfoCommon);
  AddTokensToDictionary(DocPropertyCategoryTokens,
    TdxTokenKind.DocPropertyCategory);
  AddTokenToDictionary(DocPropertyToken, TdxTokenKind.DocProperty);
  AddTokensToDictionary(DocumentInformationTokens,
    TdxTokenKind.DocumentInformation);
...
constructor TdxFieldScanner.Create(const AIterator: IdxFieldIterator;
  AMaxFieldSwitchLength: Integer; AEnableFieldNames: Boolean;
  ASupportFieldCommonStringFormat: Boolean);
begin
  inherited Create;
...
begin
  Create(AIterator, APieceTable.DocumentModel.MaxFieldSwitchLength,
    APieceTable.DocumentModel.EnableFieldNames,
    APieceTable.SupportFieldCommonStringFormat);
end;
...
class procedure TdxFieldScanner.AddTokensToDictionary(const ATokens
  : TArray<string>; AKind: TdxTokenKind);
var
  ACount, I: Integer;
begin
...
class procedure TdxFieldScanner.AddTokenToDictionary(const AToken: string;
  AKind: TdxTokenKind);
begin
  TokenDictionary.Add(AToken, AKind);
end;

function TdxFieldScanner.CreateToken(AKind: TdxTokenKind;
  APosition: TdxDocumentLogPosition = 0; const Value: string = '';
  ALength: Integer = 1): IdxToken;
begin
...
begin
  ...
  Result := CreateToken(TdxTokenKind.Template, AStart + 1, '',
    AEnd - AStart - 1);
end;

function TdxFieldScanner.IsWhiteSpace(ACh: Char): Boolean;
begin
  Result := {$IFDEF DELPHIXE4}ACh.IsWhiteSpace{$ELSE}TCharacter.IsWhiteSpace
    (ACh){$ENDIF};
end;
...
constructor TdxDocumentFieldIterator.Create(APieceTable: TdxSimplePieceTable;
  AField: TdxField);
var
...
begin
  ...
  FPosition := TdxDocumentModelPosition.FromRunStart(PieceTable,
    AField.FirstRunIndex + 1);
  SkipNestedFields;
  ARange := PieceTable.Runs[FPosition.RunIndex];
  FCurrentRunText := TdxTextRunBaseAccess(ARange)
    .GetRawTextFast(PieceTable.TextBuffer);
...
begin
  Result := not IsEnd and
    (PieceTable.Runs[FPosition.RunIndex] is TdxFieldCodeStartRun);
end;

function TdxDocumentFieldIterator.IsFieldEnd: Boolean;
begin
  Result := not IsEnd and
    (PieceTable.Runs[FPosition.RunIndex] is TdxFieldResultEndRun);
end;

function TdxDocumentFieldIterator.IsFieldCodeEnd: Boolean;
begin
  Result := not IsEnd and
    (PieceTable.Runs[FPosition.RunIndex] is TdxFieldCodeEndRun);
end;

procedure TdxDocumentFieldIterator.AdvanceNextChar(ASkipNestedFields
  : Boolean = True);
var
...
begin
  ...
  begin
    ...
    if FPosition.RunIndex <> FLastRunIndex then
      FCurrentRunText := TdxTextRunBaseAccess
        (PieceTable.Runs[FPosition.RunIndex])
        .GetRawTextFast(PieceTable.TextBuffer);
  end;
end;

function TdxDocumentFieldIterator.IsNestedFieldBegin
  (ARange: TdxTextRunBase): Boolean;
begin
  Result := ARange is TdxFieldCodeStartRun;
end;

function TdxDocumentFieldIterator.IsNestedFieldEnd
  (ARange: TdxTextRunBase): Boolean;
begin
