...
type
  ...
  TdxTextRunChangeCaseHistoryItemClass = class of
    TdxTextRunChangeCaseHistoryItem;

  TdxTextRunChangeCaseHistoryItem = class abstract(TdxRichEditHistoryItem)
  public const
    ChangeActions = [TdxDocumentModelChangeAction.ResetPrimaryLayout,
  ...
  protected
    ...
    function ChangeCase(const AValue: Char; AAllCaps: Boolean): Char;
      virtual; abstract;
  public
    constructor Create(APieceTable: TdxPieceTable; ARunIndex: TdxRunIndex);
      reintroduce;
  ...
  public
    ...
    property DocumentModel: TdxDocumentModel read GetDocumentModel;
    property NumberingList: TdxNumberingList read FNumberingList
      write FNumberingList;
  end;
  ...
  strict private
    ...
    function IsBookmarkStartPositionAffected(ABookmark: TdxBookmarkBase;
      AStartRunIndex: TdxRunIndex; AEndRunIndex: TdxRunIndex): Boolean;
    function IsBookmarkEndPositionAffected(ABookmark: TdxBookmarkBase;
      AStartRunIndex: TdxRunIndex; AEndRunIndex: TdxRunIndex): Boolean;
    function CalculateAffectedBookmarkList(ABookmarks
      : TdxBookmarkBaseCollection): TdxBookmarkInfoList;
...
constructor TdxTextRunChangeCaseHistoryItem.Create(APieceTable: TdxPieceTable;
...
function TdxTextRunMakeLowerCaseHistoryItem.ChangeCase(const AValue: Char;
  AAllCaps: Boolean): Char;
begin
{$IFDEF DELPHIXE4}
...
function TdxTextRunMakeUpperCaseHistoryItem.ChangeCase(const AValue: Char;
  AAllCaps: Boolean): Char;
begin
{$IFDEF DELPHIXE4}
...
function TdxTextRunToggleCaseHistoryItem.ChangeCase(const AValue: Char;
  AAllCaps: Boolean): Char;
begin
{$IFDEF DELPHIXE4}
  if AAllCaps then
...
function TdxSectionRunInsertedHistoryItem.CreateRun
...
function TdxAddAbstractNumberingListHistoryItem.GetDocumentModel
  : TdxDocumentModel;
begin
...
begin
  ...
  FreeAndNil(FAffectedRangePermissions);
  FAffectedRangePermissions := CalculateAffectedBookmarkList
    (PieceTable.RangePermissions);
end;
...
begin
  ...
  begin
    ...
    AInfo := nil;
    if IsBookmarkStartPositionAffected(ABookmark, AStartDeletedIndex,
      AEndDeletedIndex) then
    begin
      AInfo := TdxBookmarkInfo.Create(ABookmark);
      AInfo.StartPosition := ABookmark.Interval.Start;
      AInfo.EndPosition.Invalidate;
    end;
    if IsBookmarkEndPositionAffected(ABookmark, AStartDeletedIndex,
      AEndDeletedIndex) then
    begin
      if AInfo = nil then
      begin
...
function TdxTextRunsDeletedHistoryItem.IsBookmarkEndPositionAffected
  (ABookmark: TdxBookmarkBase;
  AStartRunIndex, AEndRunIndex: TdxRunIndex): Boolean;
var
  AEndBookmarkIndex: TdxRunIndex;
begin
  AEndBookmarkIndex := ABookmark.Interval.&End.RunIndex;
  Result := (AStartRunIndex <= AEndBookmarkIndex) and
    (AEndRunIndex >= AEndBookmarkIndex);
end;
...
begin
  ...
  Result := (AStartRunIndex <= AStartBookmarkIndex) and
    (AEndRunIndex >= AStartBookmarkIndex);
end;

procedure TdxTextRunsDeletedHistoryItem.RepairBookmarks(AAffectedBookmarks
  : TdxBookmarkInfoList);
var
  I: Integer;
  AInfo: TdxBookmarkInfo;
