...
type
  ...
  protected
    ...
    procedure DeleteContentCore(ASelectionStart: TdxDocumentLogPosition;
      ASelectionLength: Integer; ADocumentLastParagraphSelected: Boolean);
      overload; virtual;
    procedure DeleteContentCore(ASelectionStart: TdxDocumentLogPosition;
      ASelectionLength: Integer; ADocumentLastParagraphSelected: Boolean;
      ABackspacePressed: Boolean); overload; virtual;
    procedure ModifyModel; virtual; abstract;
    function ValidateSelectionRanges(ASorted: TdxSelectionRangeCollection)
      : Boolean; virtual;
  public
  ...
  private
    function GetSelectionStartBySelectionRange(ARange: TdxSelectionRange)
  ...
  protected
    ...
    procedure DeleteBackProcess(const ASelectionStart: TdxDocumentModelPosition;
      ASelectionStartPos: TdxDocumentLogPosition);
    function IsDeletedNumeration(const ASelectionStart
      : TdxDocumentModelPosition; AParagraph: TdxParagraph): Boolean; virtual;
  public
  ...
  protected
    ...
    function IsWholeDocumentSelected(ASelectionRange
      : TdxSelectionRange): Boolean;
  ...
  protected
    ...
    function CalculateSelectionEnd(const AStart: TdxDocumentModelPosition)
      : TdxDocumentModelPosition;
  ...
  public
    ...
    property RestoreInputPositionFormatting: Boolean
      read FRestoreInputPositionFormatting
      write FRestoreInputPositionFormatting;
  end;

  { TdxDeleteNonEmptySelectionCommandKeepSingleImage }

  TdxDeleteNonEmptySelectionCommandKeepSingleImage = class
    (TdxDeleteNonEmptySelectionCommand)
  protected
  ...
  TdxDeleteSelectionKeepLastParagraphCommand = class
    (TdxDeleteNonEmptySelectionCommandKeepSingleImage)
  protected
    procedure DeleteContentCore(ASelectionStart: TdxDocumentLogPosition;
      ASelectionLength: Integer;
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      const ATab: TdxTabInfo); reintroduce; virtual;
...
function TdxDeleteCommandBase.CanChangePosition
  (const APos: TdxDocumentModelPosition): Boolean;
begin
  Result := IsContentEditable and CanEditSelection;
end;

function TdxDeleteCommandBase.ChangePosition(const APos
  : TdxDocumentModelPosition): Integer;
begin
  Result := APos.LogPosition;
end;

procedure TdxDeleteCommandBase.DeleteContentCore(ASelectionStart
  : TdxDocumentLogPosition; ASelectionLength: Integer;
  ADocumentLastParagraphSelected: Boolean);
begin
  DeleteContentCore(ASelectionStart, ASelectionLength,
    ADocumentLastParagraphSelected, False);
end;

procedure TdxDeleteCommandBase.DeleteContentCore(ASelectionStart
  : TdxDocumentLogPosition; ASelectionLength: Integer;
  ADocumentLastParagraphSelected: Boolean; ABackspacePressed: Boolean);
begin
  if ABackspacePressed then
    ActivePieceTable.DeleteBackContent(ASelectionStart, ASelectionLength,
      ADocumentLastParagraphSelected)
  else
    ActivePieceTable.DeleteContent(ASelectionStart, ASelectionLength,
      ADocumentLastParagraphSelected);
...
begin
  ...
  begin
    AStart := Max(ActivePieceTable.DocumentStartLogPosition,
      ASelection.Items[0].Start - 1);
    Result := IsContentEditable and ActivePieceTable.CanEditRangeLength
      (AStart, 1);
  end
  else
...
procedure TdxDeleteBackCoreCommand.DeleteBackProcess(const ASelectionStart
  : TdxDocumentModelPosition; ASelectionStartPos: TdxDocumentLogPosition);
var
...
begin
  ...
  begin
    AItem := AItems[I];
    if (AItem.NormalizedStart = ARange.From) and (AItem.Length = ARange.Length)
    then
...
function TdxDeleteBackCoreCommand.IsDeletedNumeration(const ASelectionStart
  : TdxDocumentModelPosition; AParagraph: TdxParagraph): Boolean;
begin
  Result := AParagraph.IsInList and
    (AParagraph.LogPosition = ASelectionStart.LogPosition);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      ADocumentLastParagraphSelected := False;
      if ASelectionStartPos + ASelectionLength > ActivePieceTable.DocumentEndLogPosition
      then
      begin
        ASelectionLength := ActivePieceTable.DocumentEndLogPosition -
          ASelectionStartPos;
        ADocumentLastParagraphSelected := True;
      end;
      if ASelectionLength > 0 then
        DeleteContentCore(ASelectionStartPos, ASelectionLength,
          ADocumentLastParagraphSelected, True)
      else
      begin
  ...
  ActivePieceTable.ApplyChangesCore
    ([TdxDocumentModelChangeAction.ValidateSelectionInterval], ARunIndex,
    ARunIndex);
end;

{ TdxDeleteCoreCommand }

function TdxDeleteCoreCommand.CanChangePosition
  (const APos: TdxDocumentModelPosition): Boolean;
var
  ASelection: TdxSelection;
begin
...
function TdxDeleteCoreCommand.IsResetLastParagraph(ASelectionRange
  : TdxSelectionRange): Boolean;
begin
...
function TdxDeleteCoreCommand.IsWholeDocumentSelected(ASelectionRange
  : TdxSelectionRange): Boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      ADocumentLastParagraphSelected := False;
      if ASelectionStart + ASelectionLength > ActivePieceTable.DocumentEndLogPosition
      then
      begin
        ASelectionLength := ActivePieceTable.DocumentEndLogPosition -
          ASelectionStart;
...
begin
  ...
  begin
    APosition := Min(DocumentModel.Selection.NormalizedStart + 1,
      ActivePieceTable.DocumentEndLogPosition);
...
begin
  ...
  if (AFirstCell = nil) or (ALastCell = nil) or
    (AFirstCell.Table <> ALastCell.Table) then
    Exit(nil);

  ATable := AFirstCell.Table;
  if (AFirstCell = ATable.FirstRow.FirstCell) and
    (ALastCell = ATable.LastRow.LastCell) then
    Exit(ATable);
  Result := nil;
end;

procedure TdxDeleteNonEmptySelectionCommand.DeleteContentCore(ASelectionStart
  : TdxDocumentLogPosition; ASelectionLength: Integer;
  ADocumentLastParagraphSelected: Boolean);
begin
  DeleteContentCore(ASelectionStart, ASelectionLength,
    ADocumentLastParagraphSelected, AllowDeleteTableRows);
end;

procedure TdxDeleteNonEmptySelectionCommand.UpdateUIStateCore
...
function TdxDeleteNonEmptySelectionCommandKeepSingleImage.
...
procedure TdxDeleteSelectionKeepLastParagraphCommand.DeleteContentCore
  (ASelectionStart, ASelectionLength: Integer;
  ADocumentLastParagraphSelected: Boolean);
var
  APieceTable: TdxPieceTable;
...
begin
  ...
  if (AParagraph.EndLogPosition = AEndSelectionLogPosition) and (ACell = nil)
  then
    if not ADocumentLastParagraphSelected or not APieceTable.Paragraphs.Last.IsEmpty
    then
      ASelectionLength := Max(0, ASelectionLength - 1);
  inherited DeleteContentCore(ASelectionStart, ASelectionLength,
    ADocumentLastParagraphSelected);
end;
...
begin
  ...
  Result := not AParagraph.IsInList and
    ((AParagraph.LeftIndent <> 0) or (AParagraph.FirstLineIndent <> 0));
end;

function TdxBackSpaceKeyCommand.CreateDecrementParagraphIndentCommand
...
begin
  ...
  try
    if ADecrementIndentCommand.SelectionBeginFirstRowStartPos and CanDecrementParagraphIndent
    then
    begin
...
begin
  ...
  Result := (ACell <> nil) and
    (ASelection.Start = ActivePieceTable.Paragraphs[ACell.StartParagraphIndex]
    .LogPosition);
  if not Result then
  begin
...
procedure TdxBackSpaceKeyCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
...
function TdxDeleteBackCommand.GetUpdateUIStateMode
...
function TdxDeleteWordCoreCommand.CalculateSelectionEnd
  (const AStart: TdxDocumentModelPosition): TdxDocumentModelPosition;
var
  AIterator: TdxWordsDocumentModelIterator;
begin
  ...
  try
    Result := TryCalculateSelectionEndAssumingSelectionStartAtTheEndOfWord
      (AIterator, AStart);
...
begin
  ...
  if ALength > 0 then
  begin
    if IsContentEditable and ActivePieceTable.CanEditRange(AStart.LogPosition,
      AEnd.LogPosition) then
      DeleteContentCore(AStart.LogPosition, ALength, False);
  end;
end;

function TdxDeleteWordCoreCommand.
  TryCalculateSelectionEndAssumingSelectionStartAtTheEndOfWord
  (AIterator: TdxWordsDocumentModelIterator;
  const AStart: TdxDocumentModelPosition): TdxDocumentModelPosition;
var
...
begin
  if AIterator.IsStartOfDocument(AStart) or AIterator.IsEndOfDocument(AStart)
  then
    Exit(TdxDocumentModelPosition.Null);
  if AIterator.IsInsideWord(AStart) then
  begin
    ACharacterIterator := TdxCharactersDocumentModelIterator.Create
      (ActivePieceTable);
    try
      Result := AStart;
      AIterator.SkipForward(ACharacterIterator, Result,
        AIterator.IsNotNonWordsSymbols);
    finally
  ...
  begin
    ...
    try
      ...
      if not AIterator.IsNotNonWordsSymbols
        (AIterator.GetCharacter(APreviousChar)) then
        Exit(TdxDocumentModelPosition.Null);
      Result := AStart;
...
begin
  ...
  try
    ...
    if ALength > 0 then
    begin
      if IsContentEditable and ActivePieceTable.CanEditRange
        (ANewStart.LogPosition, AStart.LogPosition) then
...
function TdxDeleteWordBackCommand.GetExecutionMode
...
function TdxDeleteWordBackCommand.GetUpdateUIStateMode
  : TdxMultiCommandUpdateUIStateMode;
begin
  Result := TdxMultiCommandUpdateUIStateMode.EnableIfAnyAvailable;
end;
...
function TdxDeleteCommand.GetUpdateUIStateMode
  : TdxMultiCommandUpdateUIStateMode;
begin
  Result := TdxMultiCommandUpdateUIStateMode.EnableIfAnyAvailable;
end;
...
function TdxDeleteWordCommand.GetUpdateUIStateMode
  : TdxMultiCommandUpdateUIStateMode;
begin
  Result := TdxMultiCommandUpdateUIStateMode.EnableIfAnyAvailable;
end;
...
constructor TdxDeleteTabAtParagraphCommand.Create(const AControl
...
procedure TdxDeleteTabAtParagraphCommand.ForceExecute
  (const AState: IdxCommandUIState);
var
  ASelection: TdxSelection;
        ...
        begin
          ...
          try
            ...
            try
              ...
              begin
                FTab := TdxTabInfo.Create(FTab.Position, FTab.Alignment,
                  FTab.Leader, True, False);
...
procedure TdxDeleteTabAtParagraphCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
