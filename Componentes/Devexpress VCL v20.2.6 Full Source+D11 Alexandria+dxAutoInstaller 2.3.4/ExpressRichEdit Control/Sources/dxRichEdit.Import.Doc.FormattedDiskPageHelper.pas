...
type
  ...
  protected
    ...
    property CharacterFormattedDiskPages
      : TObjectDictionary<Integer, TdxCHPXFormattedDiskPage>
      read FCharacterFKPs;
    property ParagraphFormattedDiskPages
      : TObjectDictionary<Integer, TdxPAPXFormattedDiskPage>
      read FParagraphsFKPs;
  public
    constructor Create(AMainStreamReader: TdxVirtualStreamBinaryReader;
      ADataStreamReader: TBinaryReader; AParagraphsBinTable: TdxBinTable;
      ACharactersBinTable: TdxBinTable);
    destructor Destroy; override;
    function UpdateCharacterProperties(AFc: Integer;
      AFactory: TdxDocCommandFactory): TdxDocPropertyContainer;
    procedure UpdateParagraphProperties(AFc: Integer;
      APropertyContainer: TdxDocPropertyContainer);
    function GetTextRunBorders: TdxObjectList<TdxTextRunBorder>;
    procedure AddTextRunBorders(ABinTable: TdxBinTable;
      AReason: TdxTextRunStartReason;
...
constructor TdxFormattedDiskPageHelper.Create(AMainStreamReader
  : TdxVirtualStreamBinaryReader; ADataStreamReader: TBinaryReader;
...
begin
  ...
  FCharacterFKPs := TObjectDictionary<Integer, TdxCHPXFormattedDiskPage>.Create
    ([doOwnsValues]);
  FParagraphsFKPs := TObjectDictionary<Integer, TdxPAPXFormattedDiskPage>.Create
    ([doOwnsValues]);
...
function TdxFormattedDiskPageHelper.UpdateCharacterProperties(AFc: Integer;
  AFactory: TdxDocCommandFactory): TdxDocPropertyContainer;
var
...
procedure TdxFormattedDiskPageHelper.UpdateParagraphProperties(AFc: Integer;
  APropertyContainer: TdxDocPropertyContainer);
var
...
begin
  ...
  begin
    APapFkp := TdxPAPXFormattedDiskPage.FromStream(MainStreamReader,
      AFkpOffset);
  ...
  APropertyContainer.ParagraphInfo.ParagraphStyleIndex :=
    SmallInt(MainStreamReader.ReadUInt16);
  if (AGrpprlSize <= 0) or (MainStreamReader.BaseStream.Position + AGrpprlSize >
    MainStreamReader.BaseStream.Size) then
    Exit;
  AGrpprl := MainStreamReader.ReadBytes(AGrpprlSize);
  TdxDocCommandHelper.Traverse(AGrpprl, APropertyContainer, FDataStreamReader);
end;

function TdxFormattedDiskPageHelper.GetTextRunBorders
  : TdxObjectList<TdxTextRunBorder>;
begin
  Result := TdxObjectList<TdxTextRunBorder>.Create;
  AddTextRunBorders(FCharactersBinTable,
    TdxTextRunStartReason.TextRunMark, Result);
  AddTextRunBorders(FParagraphsBinTable,
    TdxTextRunStartReason.ParagraphMark, Result);
end;

procedure TdxFormattedDiskPageHelper.AddTextRunBorders(ABinTable: TdxBinTable;
  AReason: TdxTextRunStartReason;
  ATextRunBorders: TdxObjectList<TdxTextRunBorder>);
var
...
begin
  ...
  try
    ...
    begin
      ...
      if TdxAlgorithms1<TdxTextRunBorder>.BinarySearch(ATextRunBorders,
        AComparable.CompareTo, AIndex) then
        ATextRunBorders[AIndex].Reason := ATextRunBorders[AIndex].Reason
          + [AReason]
      else
      begin
