...
type
  ...
  protected
    ...
    procedure ProcessParagraph(ARunIndex: TdxRunIndex;
      AParagraphIndex: TdxParagraphIndex);
    procedure ProcessRuns(AFirstRunIndex, ALastRunIndex: TdxRunIndex);
    procedure ProcessMiddleCore(AStartIndex, AEndParagraphIndex
      : TdxParagraphIndex);
...
constructor TdxCopyParagraphOperation.Create(ACopyManager
...
function TdxCopyParagraphOperation.ShouldProcessContentInSameParent
...
function TdxCopyParagraphOperation.ShouldProcessRunParent
  (AInfo: TdxRunInfo): Boolean;
var
...
procedure TdxCopyParagraphOperation.ProcessContentInsideParent
...
begin
  ...
  if (AParagraph.IsInNonStyleList or
    (AParagraph.GetOwnNumberingListIndex = NumberingListIndexNoNumberingList))
    and (AStartRunIndex = AParagraph.FirstRunIndex) and
    (CopyManager.ParagraphNumerationCopyOptions =
    TdxParagraphNumerationCopyOptions.CopyAlways) then
    AParagraph.CopyNumberingListProperties
      (CopyManager.TargetPieceTable.Paragraphs
      [CopyManager.TargetPosition.ParagraphIndex]);
end;

function TdxCopyParagraphOperation.ProcessHead(AInfo: TdxRunInfo;
...
function TdxCopyParagraphOperation.ProcessTail(AInfo: TdxRunInfo;
  ADocumentLastParagraphSelected: Boolean): Integer;
var
...
begin
  ...
  ATargetParagraph := ATargetPieceTable.Paragraphs
    [ATargetPosition.ParagraphIndex];

  AOptions := ATargetParagraph.DocumentModel.DocumentCapabilities;
  if AOptions.ParagraphStyleAllowed then
    ATargetParagraph.ParagraphStyleIndex := AParagraph.ParagraphStyle.Copy
      (CopyManager.TargetModel);
  ATargetParagraph.ParagraphStyle.ParagraphProperties.Alignment :=
  ...
  if ((AParagraph.IsInNonStyleList) or
    (AParagraph.GetOwnNumberingListIndex = NumberingListIndexNoNumberingList))
    and (CopyManager.ParagraphNumerationCopyOptions =
    TdxParagraphNumerationCopyOptions.CopyAlways) then
    AParagraph.CopyNumberingListProperties(ATargetParagraph);
  Result := 0;
end;

function TdxCopyParagraphOperation.NeedInsertParagraphBeforeTable
  (AInfo: TdxRunInfo): Boolean;
var
  ASourceStartCell: TdxTableCell;
  ATargetParagraphLogPosition: TdxDocumentLogPosition;
begin
  ASourceStartCell := SourcePieceTable.Paragraphs
    [AInfo.Start.ParagraphIndex].GetCell;
  ATargetParagraphLogPosition := TargetPieceTable.Paragraphs
    [TargetPosition.ParagraphIndex].LogPosition;
  Result := (ASourceStartCell <> nil) and
    (ATargetParagraphLogPosition <> TargetPosition.LogPosition);
end;
...
begin
  ...
  if TargetModel.DocumentCapabilities.CharacterFormattingAllowed then
    ATargetRun.CharacterProperties.CopyFrom
      (ASourceRun.CharacterProperties.Info);
  ...
  if TargetPieceTable.DocumentModel.DocumentCapabilities.ParagraphsAllowed then
    CopyManager.OnTargetParagraphInserted(ASourceParagraph, ATargetParagraph,
      ASourceRun, ATargetRun)
  else
    InsertSpaceInsteadParagraph;
end;

procedure TdxCopyParagraphOperation.ProcessRuns(AFirstRunIndex,
  ALastRunIndex: TdxRunIndex);
var
  I: Integer;
...
procedure TdxCopyParagraphOperation.ProcessMiddleCore(AStartIndex,
...
begin
  ...
  CopyManager.TableCopyHelper.TargetStartParagraphIndex :=
    CopyManager.TableCopyHelper.TargetStartParagraphIndex + 1;
