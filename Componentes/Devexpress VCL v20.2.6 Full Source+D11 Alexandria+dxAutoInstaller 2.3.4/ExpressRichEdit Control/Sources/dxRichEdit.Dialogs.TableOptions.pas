...
uses
  ...
  Dialogs,
  dxCore, dxRichEdit.Dialogs.CustomTableOptions, cxGraphics, cxControls,
  cxLookAndFeels,
  cxLookAndFeelPainters, Menus, cxContainer, cxEdit, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters,
  dxLayoutContainer, cxTextEdit, cxMaskEdit, cxSpinEdit, dxMeasurementUnitEdit,
...
type
  TdxRichEditTableOptionsDialogForm = class
...
begin
  ...
  lblMargins.Caption := cxGetResourceString
    (@sdxRichEditTableOptionsDialogMargins);
  lblDefaultCellSpacing.Caption :=
    cxGetResourceString(@sdxRichEditTableOptionsDialogDefaultCellSpacing);
  cbAllowCellSpacing.Caption := cxGetResourceString
    (@sdxRichEditTableOptionsDialogAllowCellSpacing);
  lblOptions.Caption := cxGetResourceString
    (@sdxRichEditTableOptionsDialogOptions);
  cbResizeToFitContent.Caption := cxGetResourceString
    (@sdxRichEditTableOptionsDialogResizeToFitContent);
end;

procedure TdxRichEditTableOptionsDialogForm.cbAllowCellSpacingClick
  (Sender: TObject);
var
  ACellSpacingChecked: Boolean;
begin
  ACellSpacingChecked := cbAllowCellSpacing.Checked;
  if ACellSpacingChecked and (GetValueFromEditor(edtSpacingBetweenCells) = 0)
  then
    SetValueToEditor(edtSpacingBetweenCells,
      TdxTableOptionsFormHelper.MinCellSpacing);

  Controller.AllowCellSpacing := ACellSpacingChecked;
  edtSpacingBetweenCells.Enabled := ACellSpacingChecked;
end;

procedure TdxRichEditTableOptionsDialogForm.cbResizeToFitContentClick
  (Sender: TObject);
begin
...
begin
  Result := TdxTableOptionsFormController.Create
    (AControllerParameters as TdxTableOptionsFormControllerParameters);
end;

procedure TdxRichEditTableOptionsDialogForm.edtBottomMarginPropertiesChange
  (Sender: TObject);
var
  AValue: Integer;
begin
...
procedure TdxRichEditTableOptionsDialogForm.edtLeftMarginPropertiesChange
  (Sender: TObject);
var
  AValue: Integer;
begin
...
procedure TdxRichEditTableOptionsDialogForm.edtRightMarginPropertiesChange
  (Sender: TObject);
var
  AValue: Integer;
begin
...
procedure TdxRichEditTableOptionsDialogForm.
  edtSpacingBetweenCellsPropertiesChange(Sender: TObject);
var
  AValue: Integer;
begin
...
procedure TdxRichEditTableOptionsDialogForm.edtTopMarginPropertiesChange
  (Sender: TObject);
var
  AValue: Integer;
begin
...
procedure TdxRichEditTableOptionsDialogForm.FormCloseQuery(Sender: TObject;
...
begin
  ...
  CanClose := IsValidEdit(edtTopMargin) and IsValidEdit(edtBottomMargin) and
    IsValidEdit(edtLeftMargin) and IsValidEdit(edtRightMargin) and
    IsValidEdit(edtSpacingBetweenCells);
  if CanClose then
    Controller.ApplyChanges;
end;

function TdxRichEditTableOptionsDialogForm.GetController
...
begin
  ...
  InitializeMeasurementUnitEdit(edtSpacingBetweenCells,
    ToMeasurementType(UnitType), TdxMeasurementUnitEditHelper.Create
    (UnitTypeDescription, 0.1, 2,
    ModelUnitsToUIUnit(TdxTableOptionsFormHelper.MinCellSpacingByDefault),
    ModelUnitsToUIUnit(TdxTableOptionsFormHelper.MaxCellSpacingByDefault)));
end;
...
begin
  ...
  edtSpacingBetweenCells.Properties.OnChange :=
    edtSpacingBetweenCellsPropertiesChange;
