...
uses
  ...
  dxCoreReg, cxClasses, cxControls, cxDesignWindows, dxGaugeControl,
  dxGaugeControlSelection, dxGaugeCustomScale;
...
type
  ...
  public
    procedure PrepareItem(AIndex: Integer;
  ...
  TdxGaugeControlDesignTimeSelectionHelper = class
    (TdxGaugeControlCustomSelectionHelper, IcxDesignSelectionChanged)
  private
...
uses
  SysUtils, cxComponentCollectionEditor, dxGaugeControlScalesEditor,
  dxGaugeQuantitativeScale, dxGaugeCircularScale,
...
procedure TdxGaugeControlComponentEditor.PrepareItem(AIndex: Integer;
...
function TdxGaugeControlComponentEditor.InternalGetVerb
...
procedure TdxGaugeControlComponentEditor.MenuItemOnClickHandler
  (ASender: TObject);
...
begin
  if GetComponent(0) is TdxGaugeControl then
    ShowFormEditorClass(Designer, TdxGaugeControl(GetComponent(0)),
...
begin
  ...
  Result := Result - [paSubProperties, paReadOnly] + [paValueList, paSortList,
    paRevertable, paVolatileSubProperties];
end;
...
begin
  ...
  try
    dxGaugeGetRegisteredStyleNames(TdxGaugeCustomScaleAccess(GetComponent(0))
      .GetScaleType, AStyles);
...
constructor TdxGaugeControlDesignTimeSelectionHelper.Create
  (AControl: TcxControl);
begin
...
procedure TdxGaugeControlDesignTimeSelectionHelper.SelectComponent
...
begin
  ShowFormEditorClass(FDesignHelper.Designer, Control as TdxCustomGaugeControl,
    TfmGaugeControlScalesEditor);
end;

procedure TdxGaugeControlDesignTimeSelectionHelper.DesignSelectionChanged
  (ASelection: TList);
begin
  Selections.Clear;
...
function TdxGaugeControlDesignTimeSelectionHelper.QueryInterface
...
begin
  ...
  RegisterNoIcon([TdxGaugeCircularScaleRange, TdxGaugeCircularWideScaleRange,
    TdxGaugeDigitalScaleCaption, TdxGaugeLinearScaleRange,
  ...
  RegisterPropertyEditor(TypeInfo(Integer), TdxGaugeCustomScale,
    'AnchorScaleIndex', nil);
  RegisterPropertyEditor(TypeInfo(string), TdxGaugeCustomScale, 'StyleName',
    TdxGaugeStyleNameProperty);
  RegisterPropertyEditor(TypeInfo(TdxGaugeScaleCollection), TdxGaugeControl,
    'Scales', TdxGaugeScaleCollectionProperty);
  RegisterPropertyEditor(TypeInfo(TdxGaugeCaptionCollection),
    TdxGaugeCustomScale, 'Captions', nil);
  RegisterPropertyEditor(TypeInfo(TdxGaugeRangeCollection),
    TdxGaugeQuantitativeScale, 'Ranges', nil);
  RegisterPropertyEditor(TypeInfo(Integer), TdxGaugeScaleOptionsArcLayout,
    'Radius', nil);
  RegisterPropertyEditor(TypeInfo(Single), TdxGaugeScaleOptionsArcLayout,
    'RadiusFactor', nil);
end;
