...
type
  ...
  TdxGaugeControlCustomSelectionHelperClass = class of
    TdxGaugeControlCustomSelectionHelper;
  TdxGaugeControlMoveSelectionEvent = procedure(const ADelta: TdxPointF)
    of object;
  ...
  private
    ...
    function IsCaption(AScale: TdxGaugeCustomScale;
    ...
    function IsRange(AScale: TdxGaugeCustomScale;
      AComponent: TComponent): Boolean;
    ...
    procedure InternalSelect(AComponent: TComponent;
      AIsSelectionKeyPressed: Boolean);
  ...
  protected
    function FindActiveHelper(out AHelper
      : TdxGaugeControlCustomSelectionHelper): Boolean;
  public
    constructor Create;
    destructor Destroy; override;

    procedure RegistrySelectionHelper
      (AHelper: TdxGaugeControlCustomSelectionHelper);
    procedure UnregistrySelectionHelper
      (AHelper: TdxGaugeControlCustomSelectionHelper);
  end;
...
procedure TdxGaugeControlCustomSelectionHelper.SelectComponent
...
function TdxGaugeControlCustomSelectionHelper.GetActive
  (ASelection: TList): Boolean;
begin
  Result := (ASelection.Count > 0) and
    IsGaugeElement(TComponent(ASelection.Last));
end;
...
function TdxGaugeControlCustomSelectionHelper.KeyDown(AKey: Integer): Boolean;
  ...
  begin
    Result := TdxCustomGaugeControlControllerAccess
      (TdxCustomGaugeControlAccess(Control).Controller).IsScalePopupMenuShowing;
  end;
...
begin
  ...
  case AKey of
    ...
    VK_ESCAPE:
      begin
        Result := not IsScalePopupMenuShowing and
          not(Control.DragAndDropState <> ddsNone);
        if Result then
        begin
...
procedure TdxGaugeControlCustomSelectionHelper.PopulateSelections
  (ASelection: TList);
var
  I: Integer;
begin
...
procedure TdxGaugeControlCustomSelectionHelper.Select(AComponent: TComponent;
...
function TdxGaugeControlCustomSelectionHelper.IsCaption
  (AScale: TdxGaugeCustomScale; AComponent: TComponent): Boolean;
var
  I: Integer;
begin
  ...
  begin
    ...
    begin
      Result := TdxGaugeCustomScaleAccess(AScale)
        .Captions.IndexOf(TdxGaugeCustomCaption(AComponent)) <> -1;
      if Result then
        Break;
    end;
  end;
...
function TdxGaugeControlCustomSelectionHelper.IsGaugeElement
  (AComponent: TComponent): Boolean;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Result := IsRange(TdxCustomGaugeControl(Control).Scales[I], AComponent)
          or IsCaption(TdxCustomGaugeControl(Control).Scales[I], AComponent);
        if Result then
          Break;
      end;
    end;
...
function TdxGaugeControlCustomSelectionHelper.IsRange
  (AScale: TdxGaugeCustomScale; AComponent: TComponent): Boolean;
begin
  Result := (AComponent is TdxGaugeCustomRange) and
    (AScale is TdxGaugeQuantitativeScale) and
    (TdxGaugeQuantitativeScaleAccess(AScale)
    .Ranges.IndexOf(TdxGaugeCustomRange(AComponent)) <> -1);
end;

function TdxGaugeControlCustomSelectionHelper.IsScale
  (AComponent: TComponent): Boolean;
begin
  Result := (AComponent is TdxGaugeCustomScale) and
    (TdxCustomGaugeControl(Control).Scales.IndexOf
    (TdxGaugeCustomScale(AComponent)) <> -1);
end;

function TdxGaugeControlCustomSelectionHelper.GetScale(AScaleIndex: Integer)
  : TdxGaugeCustomScale;
begin
  Result := TdxGaugeCustomScale(TdxCustomGaugeControl(Control)
    .Scales[AScaleIndex]);
end;
...
procedure TdxGaugeControlCustomSelectionHelper.AddSelection
  (AComponent: TComponent);
begin
  Selections.Add(AComponent);
end;

procedure TdxGaugeControlCustomSelectionHelper.InternalSelect
  (AComponent: TComponent; AIsSelectionKeyPressed: Boolean);
begin
  if (Selections.Count = 1) and not AIsSelectionKeyPressed or
    (IsMultiSelection and (Selections.IndexOf(AComponent) = -1) and
    not AIsSelectionKeyPressed) then
    Selections.Clear;
  if (Selections.IndexOf(AComponent) = -1) and
    (IsEmptySelection or ((Selections.Count >= 1) and AIsSelectionKeyPressed))
  then
    AddSelection(AComponent)
  else if AIsSelectionKeyPressed then
...
begin
  Result := False;
  if KeyPressed and dxGaugeControlSelectionHelpers.FindActiveHelper(AHelper)
  then
  begin
...
procedure dxGaugeControlKeyboardHook(ACode: Integer; WPARAM: WPARAM;
...
procedure TdxGaugeControlSelectionHelpers.RegistrySelectionHelper
  (AHelper: TdxGaugeControlCustomSelectionHelper);
begin
...
procedure TdxGaugeControlSelectionHelpers.UnregistrySelectionHelper
  (AHelper: TdxGaugeControlCustomSelectionHelper);
begin
  Remove(AHelper);
end;

function TdxGaugeControlSelectionHelpers.FindActiveHelper
  (out AHelper: TdxGaugeControlCustomSelectionHelper): Boolean;
var
  I: Integer;
begin
  Result := False;
  ...
  begin
    ...
    Result := AHelper.IsActive and AHelper.Control.HandleAllocated and
      ((AHelper.Control.Handle = GetFocus) or ((AHelper.Control.Parent <> nil)
      and AHelper.Control.Parent.HandleAllocated and
      cxIsParentFocused(AHelper.Control.Parent.Handle)));
    if Result then
      Break;
  end;
end;
