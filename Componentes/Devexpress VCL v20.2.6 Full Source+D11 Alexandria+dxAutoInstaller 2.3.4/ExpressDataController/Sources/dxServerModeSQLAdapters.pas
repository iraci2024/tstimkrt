...
type
  ...
  protected
    ...
    function GetInsertSQLString(const AFrom, AOutRowKey, AFields,
      AValues: string): string; override;
  ...
  protected
    procedure CheckParams(AParams: TdxServerModeParams;
      var ACommandText: string); override;
    ...
    function DateDec(APart: TdxSQLDatePart;
      const ADateExpression, ADelta: string): string; override;
    function DateDiff(APart: TdxSQLDatePart;
      const ADateExpression1, ADateExpression2: string): string; override;
  ...
  protected
    ...
    function TruncGroupingDate(const ADate: string;
      ADateTimeGrouping: TdxDateTimeGrouping): string; override;
  ...
  public
    ...
    function GetSQLQuerySchemaNames: string; override;
  published
    property NullsSortOrder: TdxNullsSortOrder read FNullsSortOrder
      write SetNullsSortOrder default nsoFirstIfDesc;
  end;
  ...
  protected
    ...
    procedure CheckFilterFieldCaption(var AFieldCaption: string;
      AOperatorKind: TcxFilterOperatorKind); override;
...
function TdxServerModeMSSQLAdapter.GetSQLQueryKeyFieldNames(const ATableName
  : string): string;
begin
  Result := Format
    ('select c.COLUMN_NAME, COLUMNPROPERTY(OBJECT_ID(c.TABLE_SCHEMA + ''.'' + c.TABLE_NAME), c.COLUMN_NAME, ''IsIdentity'') '
...
function TdxServerModeMSSQLAdapter.GetInsertingRowOutKey
  (const AKey: string): string;
begin
  Result := Format('INSERTED.%s', [AKey]);
end;

function TdxServerModeMSSQLAdapter.GetInsertSQLString(const AFrom, AOutRowKey,
  AFields, AValues: string): string;
begin
  Result := Format('INSERT INTO %s (%s) OUTPUT %s VALUES (%s)',
    [AFrom, AFields, AOutRowKey, AValues]);
end;
...
begin
  ...
  if ASkipRecords = 0 then
    Result := Format('SELECT %s%s FROM %s %s %s %s', [AModificatorsSql, AFields,
      AFrom, AWhere, AGroup, ASortInfo])
  else
  begin
    Result := Format
      ('SELECT %s FROM (SELECT %0:s, ROW_NUMBER() OVER(%3:s) as "DXROWNUMBER" FROM %1:s %2:s)DXRESULTSET '
      + 'WHERE DXRESULTSET.DXROWNUMBER > %4:d', [AFields, AFrom, AWhere,
      ASortInfo, ASkipRecords]);
    if ATopRecords > 0 then
      Result := Format('%s AND DXRESULTSET.DXROWNUMBER <= %d',
        [Result, ASkipRecords + ATopRecords]);
  end;
end;

function TdxServerModeMSSQLAdapter.GetUpdateSQLString(const AFrom, AOutRowKey,
  AValues, AWhere: string): string;
begin
  Result := Format('UPDATE %s SET %s OUTPUT %s %s',
    [AFrom, AValues, AOutRowKey, AWhere]);
end;

function TdxServerModeMSSQLAdapter.MakeEscapedValue(const AValue: string;
...
function TdxServerModeMySQLAdapter.GetSQLQueryKeyFieldNames(const ATableName
  : string): string;
begin
  Result := Format('SHOW COLUMNS FROM `%s` WHERE `Key`=''PRI''', [ATableName]);
end;

procedure TdxServerModeMySQLAdapter.CheckParams(AParams: TdxServerModeParams;
...
procedure TdxServerModeMySQLAdapter.CheckRepeatedParams
...
begin
  ...
  begin
    ...
    begin
      ...
      if APos > 0 then
      begin
        AParam := CreateParam(AParams, AParams[I].Value, AParams[I].DataType,
          ADisplayValue);
...
procedure TdxServerModeMySQLAdapter.RenameParams(AParams: TdxServerModeParams;
  var ACommandText: string);
var
  I: Integer;
...
begin
  ...
  if ADistinct then
    Result := Format('SELECT DISTINCT %s FROM %s %s %s %s %s',
      [AFields, AFrom, AWhere, AGroup, ASortInfo, AModificatorsSql])
  else
    Result := Format('SELECT %s FROM %s %s %s %s %s',
      [AFields, AFrom, AWhere, AGroup, ASortInfo, AModificatorsSql]);
end;
...
function TdxServerModeMySQLAdapter.QuotedString(const AParamValue
  : string): string;
begin
...
function TdxServerModeMySQLAdapter.DateDec(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('SUBDATE(%s, INTERVAL %s %s)', [ADateExpression, ADelta,
    GetDatePart(APart)]);
end;

function TdxServerModeMySQLAdapter.DateDiff(APart: TdxSQLDatePart;
  const ADateExpression1, ADateExpression2: string): string;
begin
  case APart of
    dpYear:
      Result := Format('(Year(%s) - Year(%s))',
        [ADateExpression2, ADateExpression1]);
    dpMonth:
      Result := Format
        ('(((Year(%s) - Year(%s)) * 12) + Month(%0:s) - Month(%1:s))',
        [ADateExpression2, ADateExpression1]);
    dpDay:
      Result := Format('DATEDIFF(%s, %s)',
        [ADateExpression2, ADateExpression1]);
    dpHour:
      Result := Format('(Hour(%s) - Hour(%s))',
        [ADateExpression2, ADateExpression1]);
  else
    Result := '(' + DatePart(APart, ADateExpression2) + ' - ' +
      DatePart(APart, ADateExpression1) + ')';
  end;
end;

function TdxServerModeMySQLAdapter.DateInc(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('ADDDATE(%s, INTERVAL %s %s)', [ADateExpression, ADelta,
    GetDatePart(APart)]);
end;

function TdxServerModeMySQLAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  case APart of
    dpYear:
      Result := Format('Year(%s)', [ADateExpression]);
    dpMonth:
      Result := Format('Month(%s)', [ADateExpression]);
    dpDay:
      Result := Format('Day(%s)', [ADateExpression]);
    dpHour:
      Result := Format('Hour(%s)', [ADateExpression]);
    dpMinute:
      Result := Format('Minute(%s)', [ADateExpression]);
    dpSecond:
      Result := Format('Second(%s)', [ADateExpression]);
    dpMicrosecond:
      Result := Format('Microsecond(%s)', [ADateExpression]);
  else
    Result := Format('EXTRACT(%s FROM %s)',
      [GetDatePart(APart), ADateExpression]);
  end;
end;
...
function TdxServerModeFirebirdAdapter.GetSQLQueryKeyFieldNames(const ATableName
  : string): string;
begin
  Result := Format
    ('SELECT RDB$FIELD_NAME,RDB$FIELD_POSITION FROM RDB$INDEX_SEGMENTS WHERE RDB$INDEX_NAME = ( '
...
function TdxServerModeFirebirdAdapter.CanUseParam
  (AParam: TdxServerModeParam): Boolean;
begin
  Result := inherited CanUseParam(AParam) and
    not(AParam.CriteriaOperatorKind in [foLike, foNotLike, foContains,
    foNotContains, foBeginsWith, foEndsWith]);
end;

procedure TdxServerModeFirebirdAdapter.CheckParams(AParams: TdxServerModeParams;
  var ACommandText: string);
begin
  CheckRepeatedParams(AParams, ACommandText);
  SortParams(AParams, ACommandText);
...
procedure TdxServerModeFirebirdAdapter.CheckRepeatedParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  APos: Integer;
...
procedure TdxServerModeFirebirdAdapter.RenameParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  AParamName, ANewName: string;
...
function TdxServerModeFirebirdAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  Result := Format('EXTRACT(%s FROM %s)',
    [GetDatePart(APart), ADateExpression]);
...
function TdxServerModeFirebirdAdapter.GetInsertSQLString(const AFrom,
  AOutRowKey, AFields, AValues: string): string;
begin
  Result := Format('INSERT INTO %s (%s) VALUES (%s) RETURNING %s',
    [AFrom, AFields, AValues, AOutRowKey]);
end;
...
function TdxServerModeFirebirdAdapter.GetUpdateSQLString(const AFrom,
  AOutRowKey, AValues, AWhere: string): string;
begin
  Result := Format('UPDATE %s SET %s %s RETURNING %s',
    [AFrom, AValues, AWhere, AOutRowKey]);
end;
...
function TdxServerModeInterBaseAdapter.CanUseParam
  (AParam: TdxServerModeParam): Boolean;
begin
  Result := inherited CanUseParam(AParam) and
    not(AParam.CriteriaOperatorKind in [foLike, foNotLike, foContains,
...
function TdxServerModeInterBaseAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  Result := Format('EXTRACT(%s FROM %s)',
    [GetDatePart(APart), ADateExpression]);
...
begin
  ...
  begin
    ...
    AModificatorsSql := Format('ROWS %d TO %d',
      [ASkipRecords + 1, ATopRecords]);
  end;
  if ADistinct then
    Result := Format('SELECT DISTINCT %s FROM %s %s %s %s %s',
      [AFields, AFrom, AWhere, AGroup, ASortInfo, AModificatorsSql])
...
constructor TdxServerModeOracleAdapter.Create(ADataSource
...
function TdxServerModeOracleAdapter.GetSQLQueryKeyFieldNames(const ATableName
  : string): string;
begin
...
begin
  Result := 'SELECT DISTINCT OWNER FROM ALL_OBJECTS ' + 'ORDER BY 1';
end;

procedure TdxServerModeOracleAdapter.CheckParams(AParams: TdxServerModeParams;
  var ACommandText: string);
begin
  CheckRepeatedParams(AParams, ACommandText);
  SortParams(AParams, ACommandText);
...
procedure TdxServerModeOracleAdapter.CheckRepeatedParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  APos: Integer;
...
procedure TdxServerModeOracleAdapter.RenameParams(AParams: TdxServerModeParams;
  var ACommandText: string);
var
  I: Integer;
  AParamName: string;
begin
  ...
  begin
    AParamName := Format(GetParamFormat, [AParams[I].Name]);
    AParams[I].Name := Format(GetParamNameFormat, [AParams[I].Index + 1]);
    ACommandText := StringReplace(ACommandText, AParamName, '?',
      [rfReplaceAll]);
  end;
end;
...
function TdxServerModeOracleAdapter.DateDiff(APart: TdxSQLDatePart;
  const ADateExpression1, ADateExpression2: string): string;
begin
  case APart of
    dpMonth:
    ...
    dpDay:
      Result := Format('(TRUNC(%s) - TRUNC(%s))',
        [ADateExpression2, ADateExpression1]);
    dpHour:
      Result := Format('(TRUNC(%s, ''HH24'') - TRUNC(%s, ''HH24'')) * 24',
        [ADateExpression2, ADateExpression1]);
  else
    Result := '(' + DatePart(APart, ADateExpression2) + ' - ' +
      DatePart(APart, ADateExpression1) + ')';
...
function TdxServerModeOracleAdapter.DateInc(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  case APart of
    dpYear:
...
function TdxServerModeOracleAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  if APart in [dpHour, dpMinute, dpSecond] then
    Result := Format('EXTRACT(%s FROM CAST((%s) as TIMESTAMP))',
      [GetDatePart(APart), ADateExpression])
  else
    Result := Format('EXTRACT(%s FROM %s)',
      [GetDatePart(APart), ADateExpression]);
...
function TdxServerModeOracleAdapter.TruncGroupingDate(const ADate: string;
...
begin
  ...
  begin
    if ATopRecords > 0 then
      Result := Format
        ('SELECT * FROM (SELECT %s%s FROM %s %s %s %s) WHERE RowNum <= %d',
        [AModificatorsSql, AFields, AFrom, AWhere, AGroup, ASortInfo,
        ATopRecords])
    else
      Result := Format('SELECT %s%s FROM %s %s %s %s',
        [AModificatorsSql, AFields, AFrom, AWhere, AGroup, ASortInfo]);
    Exit;
  end
  else
  begin
    Result := Format
      ('SELECT %1:s FROM(SELECT %1:s, RowNum rNum FROM(SELECT %0:s%1:s FROM %2:s %3:s %4:s %5:s)) WHERE rNum > %6:d',
      [AModificatorsSql, AFields, AFrom, AWhere, AGroup, ASortInfo,
      ASkipRecords]);
    if ATopRecords > 0 then
      Result := Format('%s and rNum <= %d',
        [Result, ATopRecords + ASkipRecords]);
  end;
end;
...
constructor TdxServerModePostgreSQLAdapter.Create
...
function TdxServerModePostgreSQLAdapter.GetSQLQueryKeyFieldNames
...
begin
  if GetSchemaName = '' then
    Result := Format('select a.attname from pg_constraint c ' +
      ...
      'where c.contype = ''p'' and tc.relname = ''%s'' ' + 'order by a.attnum',
      [ATableName])
  else
    Result := Format('select a.attname from pg_constraint c ' +
      'join pg_attribute a on c.conrelid=a.attrelid and a.attnum = ANY (c.conkey) '
...
function TdxServerModePostgreSQLAdapter.CanUseGroupingByDateRangeParams
  : Boolean;
begin
  Result := False;
end;

procedure TdxServerModePostgreSQLAdapter.CheckParams
  (AParams: TdxServerModeParams; var ACommandText: string);
begin
  CheckRepeatedParams(AParams, ACommandText);
  SortParams(AParams, ACommandText);
...
procedure TdxServerModePostgreSQLAdapter.CheckRepeatedParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  APos: Integer;
...
function TdxServerModePostgreSQLAdapter.GetOrderFormat
  (AIsDesc: Boolean): string;
begin
  Result := inherited GetOrderFormat(AIsDesc);
  if NullsSortOrder = nsoFirstIfAsc then
  begin
...
procedure TdxServerModePostgreSQLAdapter.RenameParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  AParamName, ANewName: string;
...
function TdxServerModePostgreSQLAdapter.QuotedString(const AParamValue
  : string): string;
begin
...
function TdxServerModePostgreSQLAdapter.DateDec(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('%s - %s', [ADateExpression, ADelta]);
end;

function TdxServerModePostgreSQLAdapter.DateDiff(APart: TdxSQLDatePart;
  const ADateExpression1, ADateExpression2: string): string;
begin
  Result := Format('(date_trunc(''%s'', %s) - date_trunc(''%0:s'', %2:s))',
    [GetDatePart(APart), ADateExpression2, ADateExpression1]);
end;

function TdxServerModePostgreSQLAdapter.DateInc(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('%s + %s', [ADateExpression, ADelta]);
end;

function TdxServerModePostgreSQLAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  Result := Format('date_part(''%s'', %s) * interval ''1 %0:s''',
    [GetDatePart(APart), ADateExpression]);
end;
...
function TdxServerModePostgreSQLAdapter.GetInsertSQLString(const AFrom,
  AOutRowKey, AFields, AValues: string): string;
begin
  Result := Format('INSERT INTO %s (%s) VALUES (%s) RETURNING %s',
    [AFrom, AFields, AValues, AOutRowKey]);
...
function TdxServerModePostgreSQLAdapter.GetUpdateSQLString(const AFrom,
  AOutRowKey, AValues, AWhere: string): string;
begin
  Result := Format('UPDATE %s SET %s %s RETURNING %s',
    [AFrom, AValues, AWhere, AOutRowKey]);
...
function TdxServerModePostgreSQLAdapter.TruncGroupingDate(const ADate: string;
  ADateTimeGrouping: TdxDateTimeGrouping): string;
begin
  case ADateTimeGrouping of
    dgByHour:
...
procedure TdxServerModePostgreSQLAdapter.CheckFilterFieldCaption
  (var AFieldCaption: string; AOperatorKind: TcxFilterOperatorKind);
begin
  if AOperatorKind in [foLike, foContains, foNotContains, foBeginsWith,
    foEndsWith] then
...
function TdxServerModePostgreSQLAdapter.GetSelectSQLString(const AFields, AFrom,
  AWhere, AGroup, ASortInfo: string; ATopRecords, ASkipRecords: Integer;
  ADistinct: Boolean): string;
var
  AModificatorsSql: string;
begin
  if ATopRecords > 0 then
    ...
    AModificatorsSql := '';
  if ASkipRecords > 0 then
    AModificatorsSql := Format('%s OFFSET %d ',
      [AModificatorsSql, ASkipRecords]);
  if ADistinct then
    Result := Format('SELECT DISTINCT %s FROM %s %s %s %s %s',
      [AFields, AFrom, AWhere, AGroup, ASortInfo, AModificatorsSql])
  else
...
procedure TdxServerModePostgreSQLAdapter.SetNullsSortOrder
  (const Value: TdxNullsSortOrder);
begin
  if FNullsSortOrder <> Value then
  begin
...
function TdxServerModeAdvantageAdapter.GetSQLQueryKeyFieldNames(const ATableName
  : string): string;
begin
  Result := Format
    ('SELECT i.Index_Expression FROM system.indexes i INNER JOIN system.tables t ON t.Table_Primary_Key = i.Name WHERE t.Name = ''%s''',
    [ATableName]);
end;
...
function TdxServerModeAdvantageAdapter.DateDec(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('TIMESTAMPADD(%s, -(%s), CAST( %s as SQL_TIMESTAMP))',
    [GetDatePart(APart), ADelta, ADateExpression]);
end;

function TdxServerModeAdvantageAdapter.DateDiff(APart: TdxSQLDatePart;
  const ADateExpression1, ADateExpression2: string): string;
begin
  if APart in [dpHour, dpMinute, dpSecond] then
    ...
    Result := Format
      ('TIMESTAMPDIFF(%s, CAST(%s as SQL_DATE), CAST(%s as SQL_DATE))',
      [GetDatePart(APart), ADateExpression1, ADateExpression2]);
end;

function TdxServerModeAdvantageAdapter.DateInc(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('TIMESTAMPADD(%s, %s, CAST( %s as SQL_TIMESTAMP))',
    [GetDatePart(APart), ADelta, ADateExpression]);
...
function TdxServerModeAdvantageAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  case APart of
    dpYear:
...
function TdxServerModeAdvantageAdapter.GetDatePart
...
procedure TdxServerModeAdvantageAdapter.CheckFilterFieldCaption
  (var AFieldCaption: string; AOperatorKind: TcxFilterOperatorKind);
begin
  if AOperatorKind in [foLike, foContains, foNotContains, foBeginsWith,
    foEndsWith] then
...
procedure TdxServerModeAdvantageAdapter.CheckParams
  (AParams: TdxServerModeParams; var ACommandText: string);
begin
  CheckRepeatedParams(AParams, ACommandText);
  SortParams(AParams, ACommandText);
...
procedure TdxServerModeAdvantageAdapter.CheckRepeatedParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  APos: Integer;
...
procedure TdxServerModeAdvantageAdapter.RenameParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  AParamName, ANewName: string;
...
function TdxServerModeAdvantageAdapter.GetSelectSQLString(const AFields, AFrom,
  AWhere, AGroup, ASortInfo: string; ATopRecords, ASkipRecords: Integer;
  ADistinct: Boolean): string;
var
  AModificatorsSql: string;
begin
  if ATopRecords > 0 then
    ...
    AModificatorsSql := '';
  if ASkipRecords > 0 then
    AModificatorsSql := Format('%s START AT %d ',
      [AModificatorsSql, ASkipRecords + 1]);
  if ADistinct then
    AModificatorsSql := AModificatorsSql + 'DISTINCT ';
  Result := Format('SELECT %s %s FROM %s %s %s %s', [AModificatorsSql, AFields,
    AFrom, AWhere, AGroup, ASortInfo]);
...
procedure TdxServerModeMSAccessAdapter.CheckParams(AParams: TdxServerModeParams;
  var ACommandText: string);
begin
  CheckRepeatedParams(AParams, ACommandText);
  SortParams(AParams, ACommandText);
...
procedure TdxServerModeMSAccessAdapter.CheckRepeatedParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  APos: Integer;
...
function TdxServerModeMSAccessAdapter.DateDec(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
...
function TdxServerModeMSAccessAdapter.DateDiff(APart: TdxSQLDatePart;
  const ADateExpression1, ADateExpression2: string): string;
begin
...
function TdxServerModeMSAccessAdapter.DateInc(APart: TdxSQLDatePart;
  const ADateExpression, ADelta: string): string;
begin
  Result := Format('DateAdd(''%s'', %s, %s)', [GetDatePart(APart), ADelta,
    ADateExpression]);
...
function TdxServerModeMSAccessAdapter.DatePart(APart: TdxSQLDatePart;
  const ADateExpression: string): string;
begin
  Result := Format('DatePart(''%s'', %s)',
    [GetDatePart(APart), ADateExpression]);
end;

function TdxServerModeMSAccessAdapter.DateTimeToString(const ADateTime
  : TDateTime): string;
begin
...
function TdxServerModeMSAccessAdapter.GetDatePart
  (APart: TdxSQLDatePart): string;
begin
  case APart of
    dpYear:
...
function TdxServerModeMSAccessAdapter.MakeEscapedValue(const AValue: string;
  AEscapePercentWildcard, AEscapeUnderscoreWildcard: Boolean): string;
...
begin
  ...
  if AEscapePercentWildcard then
    Result := StringReplace(Result, GetPercentWildcard, '[' + GetPercentWildcard
      + ']', [rfReplaceAll]);
  if AEscapeUnderscoreWildcard then
    Result := StringReplace(Result, GetUnderscoreWildcard,
      '[' + GetUnderscoreWildcard + ']', [rfReplaceAll]);
...
function TdxServerModeMSAccessAdapter.NeedEscapeFakeParamName
  (AParam: TdxServerModeParam): Boolean;
begin
  Result := False;
end;

function TdxServerModeMSAccessAdapter.CanUseParam
  (AParam: TdxServerModeParam): Boolean;
begin
...
procedure TdxServerModeMSAccessAdapter.RenameParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  AParamName, ANewName: string;
...
function TdxServerModeMSAccessAdapter.GetSelectSQLString(const AFields, AFrom,
  AWhere, AGroup, ASortInfo: string; ATopRecords, ASkipRecords: Integer;
  ADistinct: Boolean): string;
var
  AModificatorsSql: string;
...
procedure TdxServerModeSQLiteAdapter.CheckParams(AParams: TdxServerModeParams;
  var ACommandText: string);
begin
  CheckRepeatedParams(AParams, ACommandText);
  SortParams(AParams, ACommandText);
...
procedure TdxServerModeSQLiteAdapter.CheckRepeatedParams
  (AParams: TdxServerModeParams; var ACommandText: string);
var
  I: Integer;
  APos: Integer;
...
procedure TdxServerModeSQLiteAdapter.RenameParams(AParams: TdxServerModeParams;
  var ACommandText: string);
var
  I: Integer;
  AParamName, ANewName: string;
