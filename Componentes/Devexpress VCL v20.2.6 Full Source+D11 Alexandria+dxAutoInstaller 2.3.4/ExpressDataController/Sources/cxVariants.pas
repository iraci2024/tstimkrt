...
uses
  Types, SysUtils, Classes, dxCore, dxGenerics, Variants, Generics.Defaults,
...
type
  ...
  TdxVariantToObjectDictionary<TValue: class> = class
    (TObjectDictionary<Variant, TValue>)
  public
...
begin
  for Result := 0 to Length(AList) - 1 do
    if VarEquals(AList[Result], AValue) then
      Exit;
  Result := -1;
end;
...
procedure ReadVariantProc(AStream: TStream; var Value: Variant);
const
  ValToVarT: array [TValueType] of Integer = (varNull, varError, varShortInt,
...
procedure WriteVariantProc(AStream: TStream; const AValue: Variant);
  ...
  begin
    ...
    else if (Value >= Low(Integer)) and (Value <= High(Integer)) then
    begin
      WriteValue(vaInt32);
      I := Value;
...
begin
  ...
  case VType and varTypeMask of
    ...
    varShortInt, varWord, varLongWord, varInt64, varByte, varSmallInt,
      varInteger:
      WriteInteger(AValue);
    varDouble:
      WriteFloat(AValue);
...
begin
  if AVarData.VType = varByRef or varVariant then
    Result := GetVarDataArrayInfo(PVarData(AVarData.VPointer)^, AVarType,
      AVarArray)
  else
  begin
...
begin
  case AVarType of
    ...
    else if AVarType = VarFMTBcd then
    begin
...
begin
  ...
  begin
    ...
    for I := AVarArray.Bounds[0].LowBound to AVarArray.Bounds[0]
      .ElementCount - 1 do
    begin
      ...
      begin
        ...
        VarResultCheck(SafeArrayGetElement(AVarArray, @AIndex,
          @AData.VPointer));
...
constructor TdxVariantToObjectDictionary<TValue>.Create(AOwnsValues: Boolean;
  ACapacity: Integer = 0);
var
