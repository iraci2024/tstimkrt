...
type
  ...
  public
    function IsCurrentQuery(ADataSet: TDataSet; const AParamNames: string;
      const AParamValues: Variant): Boolean; virtual;
    procedure ReopenSQL(ADataSet: TDataSet; const AParamNames: string;
      const AParamValues: Variant; var AReopened: Boolean); virtual;
    procedure SetFilter(ADataSet: TDataSet;
      const AMasterDetailKeyFieldNames: string;
      const AMasterDetailKeyValues: Variant; AIsSQLMode: Boolean;
      var AReopened: Boolean); virtual;
  end;
  ...
  TcxDBDetailDataLink = class(TcxDBDataLink)
  end;

  TcxDBDetailPatternDataLink = class(TcxDBDataLink)
  end;
  ...
  public
    ...
    procedure AddFieldsByFieldNames(ADataSet: TDataSet;
  ...
  protected
    ...
    function CompareBookmarks(const AItem1, AItem2: TcxBookmark)
      : Integer; virtual;
    ...
    procedure DoInitInsertingRecord(AInsertingRecordIndex: Integer;
      const AGroupValues: TcxDataSummaryValues); override;
    // Notification
    ...
    function DoLocate(const ASubText: string; AForward, ANext: Boolean)
      : Boolean;
    ...
    // Master Detail
    function GetDetailHasChildren(ARecordIndex: Integer;
      ARelationIndex: Integer): Boolean; override;
    // Structure
  ...
  public
    ...
    function IsOtherDataControllerLocating: Boolean;
    ...
    property IsDataSetCurrent: Boolean read GetIsDataSetCurrent
      write SetIsDataSetCurrent;
  ...
  TcxDBDataFilterFormatFilterTextValueEvent = procedure
  ...
  published
    ...
    property AutoDataSetFilter: Boolean read FAutoDataSetFilter
      write SetAutoDataSetFilter default False;
    property DateTimeFormat;
    property IgnoreOrigin: Boolean read FIgnoreOrigin write SetIgnoreOrigin
      default True;
    property TranslateBetween;
    property TranslateIn;
    property TranslateLike;
    property SupportedLike;
    property OnBeforeChange: TcxDBDataFilterBeforeChangeEvent
      read FOnBeforeChange write FOnBeforeChange;
    property OnFormatFilterTextValue: TcxDBDataFilterFormatFilterTextValueEvent
      read FOnFormatFilterTextValue write FOnFormatFilterTextValue;
  end;
  ...
  TcxDBDataModeControllerDetailIsCurrentQueryEvent = function
    (Sender: TcxDBDataModeController; ADataSet: TDataSet;
    const AMasterDetailKeyFieldNames: string;
    const AMasterDetailKeyValues: Variant): Boolean of object;

  TcxDBDataModeControllerDetailFirstEvent = procedure
    (Sender: TcxDBDataModeController; ADataSet: TDataSet;
    const AMasterDetailKeyFieldNames: string;
  ...
  protected
    function DetailIsCurrentQuery(const AMasterDetailKeyFieldNames: string;
      const AMasterDetailKeyValues: Variant): Boolean; virtual;
    procedure DoDetailFirst(const AMasterDetailKeyFieldNames: string;
      const AMasterDetailKeyValues: Variant; var AReopened: Boolean); virtual;

    property DetailInSyncMode: Boolean read FDetailInSyncMode
      write FDetailInSyncMode default True;
  public
    constructor Create(ADataController: TcxDBDataController);
    procedure Assign(Source: TPersistent); override;
    property DataController: TcxDBDataController read FDataController;
    property SyncInsert: Boolean read FSyncInsert write FSyncInsert
      default True;
  published
    property DetailInSQLMode: Boolean read FDetailInSQLMode
      write FDetailInSQLMode default False;
    property GridMode: Boolean read FGridMode write SetGridMode default False;
    property GridModeBufferCount: Integer read FGridModeBufferCount
      write SetGridModeBufferCount default 0;
    property SmartRefresh: Boolean read FSmartRefresh write SetSmartRefresh
      default False;
    property SyncMode: Boolean read FSyncMode write SetSyncMode default True;
    property OnDetailFirst: TcxDBDataModeControllerDetailFirstEvent
      read FOnDetailFirst write FOnDetailFirst;
    property OnDetailIsCurrentQuery
      : TcxDBDataModeControllerDetailIsCurrentQueryEvent
      read FOnDetailIsCurrentQuery write FOnDetailIsCurrentQuery;
  end;

  TcxCompareBookmarksFunc = function(const AItem1, AItem2: TcxBookmark)
    : Integer of object;
  ...
  protected
    ...
    procedure InternalAdd(AIndex, ARowIndex, ARecordIndex,
      ALevel: Integer); override;
    procedure InternalClear; override;
    procedure InternalDelete(AIndex: Integer); override;
    function FindBookmark(const ABookmark: TcxBookmark;
  ...
  public
    ...
    function FindByRowIndex(ARowIndex: Integer; var AIndex: Integer)
      : Boolean; override;
    procedure SelectAll;
    procedure SelectFromAnchor(AToBookmark: TcxBookmark;
  ...
  TcxDBDataControllerGridModeChangedEvent = procedure
    (Sender: TcxDBDataController) of object;
  ...
  protected
    ...
    function DoSearchInGridMode(const ASubText: string;
      AForward, ANext: Boolean): Boolean; override;
    ...
    procedure NotifyDetailAfterFieldsRecreating(ADataController
    ...
    procedure RetrieveField(ADataField: TcxDBDataField;
      AIsLookupKeyOnly: Boolean);
    ...
    procedure UpdateField(ADataField: TcxDBDataField; const AFieldNames: string;
      AIsLookup: Boolean);
    procedure UpdateFields; override;
    procedure UpdateFocused; override;
    procedure UpdateInternalKeyFields(const AFieldNames: string;
  ...
  public
    ...
    function IsDisplayFormatDefined(AItemIndex: Integer;
      AIgnoreSimpleCurrency: Boolean): Boolean; override;
    ...
    // Navigation
    function CanFocusedRecordIndexChangePostData: Boolean; override;
    // for internal use
    ...
    procedure SelectFromAnchor(ARowIndex: Integer;
      AKeepSelection: Boolean); override;
    ...
    procedure ForEachRow(ASelectedRows: Boolean;
      AProc: TcxDataControllerEachRowProc); override;
    function IsSequenced: Boolean;

    property DataControllerInfo: TcxDBDataControllerInfo
      read GetDataControllerInfo;
    property DataModeController: TcxDBDataModeController
      read FDataModeController write SetDataModeController;
    property DataSet: TDataSet read GetDataSet;
    property DataSource: TDataSource read GetDataSource write SetDataSource;
    ...
    property DataSetRecordCount: Integer read GetDataSetRecordCount;
    // Sequenced
    property OnDetailHasChildren: TcxDBDataDetailHasChildrenEvent
      read FOnDetailHasChildren write FOnDetailHasChildren;
    property OnGridModeChanged: TcxDBDataControllerGridModeChangedEvent
      read FOnGridModeChanged write FOnGridModeChanged;
  end;
...
procedure cxDataSetRestoreBookmark(ADataSet: TDataSet;
  var ABookmark: TcxDataSetBookmark);
...
begin
  try
    with TDataSetAccess(ADataSet) do
    begin
      if (Length(ABookmark.SaveBookmark) > 0) and
        BookmarkValid(TBookmark(ABookmark.SaveBookmark)) then
        try
          Bookmark := ABookmark.SaveBookmark;
          if ActiveRecord > ABookmark.SaveActiveRecord then
          begin
            MoveBy(BufferCount - ActiveRecord - 1 + ActiveRecord -
              ABookmark.SaveActiveRecord);
            MoveBy(ABookmark.SaveActiveRecord - BufferCount + 1);
          end
          else if ActiveRecord < ABookmark.SaveActiveRecord then
          begin
            MoveBy(-ActiveRecord + ActiveRecord - ABookmark.SaveActiveRecord);
            MoveBy(ABookmark.SaveActiveRecord);
          end;
          if ABookmark.SaveBOF and not Bof then
            Prior;
          if ABookmark.SaveEOF and not Eof then
            Next;
        except
...
function CanCallDataSetLocate(ADataSet: TDataSet; const AKeyFieldNames: string;
  ...
  function TryGetFieldList(ADataSet: TDataSet; const AFieldNames: string;
    AList: TList): Boolean;
  var
    AField: TField;
    APos: Integer;
...
begin
  ...
  begin
    ...
    Result := (AField <> nil) and
      (not VarIsNull(AValue) or IsNullValidToLocate(AField));
  end;
end;
...
procedure TcxDBProviderDetailFilterAdapter.ReopenSQL(ADataSet: TDataSet;
  const AParamNames: string; const AParamValues: Variant;
  var AReopened: Boolean);
var
  AParams: TParams;
begin
  if IsPublishedProp(ADataSet, 'Params') then
...
begin
  if AIsSQLMode then
    ReopenSQL(ADataSet, AMasterDetailKeyFieldNames, AMasterDetailKeyValues,
      AReopened)
  else
    ADataSet.Locate(AMasterDetailKeyFieldNames, AMasterDetailKeyValues, []);
  // must be sort by Detail Keys
end;

{ TcxDBDataModeController }

constructor TcxDBDataModeController.Create(ADataController
  : TcxDBDataController);
begin
  inherited Create;
...
begin
  ...
  begin
    ...
    OnDetailIsCurrentQuery := TcxDBDataModeController(Source)
...
function TcxDBDataModeController.DetailIsCurrentQuery
  (const AMasterDetailKeyFieldNames: string;
...
procedure TcxDBDataModeController.DoDetailFirst(const AMasterDetailKeyFieldNames
  : string; const AMasterDetailKeyValues: Variant; var AReopened: Boolean);
var
  ADetailFilterAdapter: TcxDBProviderDetailFilterAdapter;
begin
  AReopened := False;
  ...
  begin
    ADetailFilterAdapter := DataController.GetDetailFilterAdapter;
    ADetailFilterAdapter.SetFilter(DataController.DataSet,
      AMasterDetailKeyFieldNames, AMasterDetailKeyValues, DetailInSQLMode,
      AReopened);
  end;
end;
...
begin
  ...
  begin
    System.Move(FItems[AIndex + 1], FItems[AIndex], (Count - AIndex - 1) *
      SizeOf(Pointer));
...
begin
  ...
  if AIndex < Count - 1 then
  begin
    Move(FItems[AIndex], FItems[AIndex + 1], (Count - AIndex - 1) *
      SizeOf(Pointer));
...
procedure TcxBookmarkList.QuickSort(L, R: Integer;
  ACompareFunc: TcxCompareBookmarksFunc);
var
  I, J: Integer;
...
function TcxDBDataSelection.FindByRowIndex(ARowIndex: Integer;
  var AIndex: Integer): Boolean;
begin
...
procedure TcxDBDataSelection.SelectFromAnchor(AToBookmark: TcxBookmark;
  AKeepSelection: Boolean);

  function CheckBookmarks(const ABkm1, ABkm2: TcxBookmark;
    ADirectionDown: Boolean): Boolean;
  begin
    Result := False;
    if AKeepSelection or (FBookmarks.Count = 0) then
      Exit;
    with DataController.Provider do
    begin
      ...
      else if ADirectionDown then
      begin
        if (FBookmarks.Count > 1) and
          (CompareBookmarks(FBookmarks[FBookmarks.Count - 2], ABkm2) = 0) then
        // dec last
        begin
        ...
        begin
          ...
          DataSet.Prior;
          if CompareBookmarks(FBookmarks[FBookmarks.Count - 1],
            DataSet.Bookmark) = 0 then // inc
          begin
            if CompareBookmarks(FBookmarks[0], ABkm1) = 0 then
            begin
      ...
      begin
        ...
        begin
          ...
          begin
            if CompareBookmarks(FBookmarks[FBookmarks.Count - 1], ABkm2) = 0
            then
            begin
...
function TcxDBDataSelection.CompareBookmarks(const AItem1,
  AItem2: TcxBookmark): Integer;
begin
  Result := DataController.Provider.CompareBookmarks(AItem1, AItem2);
end;

procedure TcxDBDataSelection.InternalAdd(AIndex, ARowIndex, ARecordIndex,
  ALevel: Integer);
begin
  inherited InternalAdd(AIndex, ARowIndex, ARecordIndex, ALevel);
  if not DataController.IsGridMode or FInSelectAll then
    Exit;
...
function TcxDBDataSelection.FindBookmark(const ABookmark: TcxBookmark;
  var AIndex: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
...
procedure TcxDBDataLink.DataEvent(Event: TDataEvent; Info: TdxNativeInt);
begin
...
begin
  ...
  if Provider.IsOtherDataControllerLocating then
    Exit;
  if Provider.IsOtherDataControllerUpdating then
    Exit;
  if Provider.IsOtherDetailChanged then
    Exit;
  {
  ...
  begin
    ...
    begin
      ...
      else if (DataSet.State = dsInsert) and not Provider.FInserting then
      begin
        if (not Provider.DataController.IsDetailMode or
          Provider.IsDataSetCurrent) and Provider.IsSyncInsert and not Provider.FInInserting
        then
          DoInsertingRecord;
      end;
    end;
...
begin
  if not Provider.IsSyncMode then
    Exit;
  ...
  begin
    if { not Provider.IsGridMode and } Provider.IsSmartRefresh and
      ...
      (Provider.EditingRecordIndex = Provider.DataController.NewItemRecordIndex)
    then
    begin
      ...
      begin
        ...
        try
          Provider.FUpdatingNewRecordIndex :=
            Provider.DataController.AppendRecord;
          Provider.DataController.CopyRecord
            (Provider.DataController.NewItemRecordIndex,
            Provider.FUpdatingNewRecordIndex);
        finally
  ...
  begin
    ...
    else if (DataSet.State = dsInsert) and not DataSet.ControlsDisabled and
      (Provider.FInSetDataSource or
      Provider.DataController.FInUpdateGridModeBufferCount) then
    begin
...
begin
  ...
  begin
    ...
    if (AItemIndex <> -1) and (AItemIndex < Provider.DataController.GetItemCount)
    then
    begin
...
procedure TcxDBDataAssignedFields.AddFieldsByFieldNames(ADataSet: TDataSet;
  const AFieldNames: string);
begin
...
begin
  ...
  begin
    ...
    if (ADBDataProvider.DataSet = ADataSet) and ADBDataProvider.FInCanInitEditing
    then
      Exit(True);
  end;
  Result := False;
...
begin
  ...
  begin
    ...
    if (ADBDataProvider.DataSet = ADataSet) and
      ...
      (ADBDataProvider.LocateCount <> 0) and
      (IsKeyNavigation or DataController.IsGridMode)) then
    begin
      ...
      else if not FPostLocateFlag then
        Result := not(DataController.IsGridMode and
          (DataController.LockGridModeNotifyCount <> 0));
      Break;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if (ADBDataProvider.DataSet = Self.DataSet) and
      ...
      ADBDataProvider.DataController.IsDetailMode { ADBDataProvider.FInFirst }
    then
    begin
      Result := True;
      Break;
...
function TcxDBDataProvider.IsOtherDataControllerLocating: Boolean;
var
  I: Integer;
  ADBDataProvider: TcxDBDataProvider;
begin
  Exit(False);
  Result := DataController.SkipLocateNotification;
  if not Result then
    Exit;
  for I := 0 to DBDataProviders.Count - 1 do
  begin
    ADBDataProvider := TcxDBDataProvider(DBDataProviders[I]);
    if (ADBDataProvider <> Self) and (ADBDataProvider.DataSet = DataSet) and
      ADBDataProvider.IsLocating then
      Exit(True);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (ADBDataProvider.DataLink <> nil) and
        (ADBDataProvider.DataLink.DataSet = ADataSet) and
        ((ADBDataProvider.LockCount <> 0) or (ADBDataProvider.LocateCount <> 0)
        and IsKeyNavigation) then
      begin
...
begin
  ...
  begin
    ADBDataProvider := TcxDBDataProvider(DBDataProviders[I]);
    if (ADBDataProvider <> Self) and (ADBDataProvider.DataSet = Self.DataSet)
      and ADBDataProvider.FInInsert then
    begin
      Result := True;
      Break;
    end;
...
begin
  Result := not DataController.IsPattern and
    DataController.DataModeController.SyncInsert and IsSyncMode;
end;
...
begin
  ...
  begin
    if not DataController.DataModeController.DetailInSyncMode then
      Result := False
    else if DataController.DataModeController.DetailInSQLMode then
    begin
      if not IsDataSetCurrent then
        Result := False;
    end
    else if not(IsKeyNavigation or IsGridMode) then
      Result := False;
  end;
end;
...
begin
  ...
  // WARNING: DataLink.Active <> DataSet.Active!
    ...
    begin
      ...
      begin
        if not VarIsEmpty(MasterDetailKeyValues) then
          Result := DataSet.Eof or (not(DataSet.State in dsEditModes) and
            not VarEquals(GetDataSetValues(DataSet, MasterDetailKeyFields),
            MasterDetailKeyValues))
        else
          Result := True;
      end
      else
...
begin
  Result := inherited IsModified { for Unbound column } or IsDataSetModified;
end;
...
function TcxDBDataProvider.CompareBookmarks(const AItem1,
  AItem2: TcxBookmark): Integer;
begin
        ...
        begin
          ...
          begin
            ...
            if RecreatedFieldsAfterFirst then
              NotifyDetailsAfterFieldsRecreating
                (not DataController.FInCheckCurrentQuery);
          end;
        end
...
begin
  if not IsGridMode or (ADistance = 0) or not IsActive then
    Exit;
  if ADistance < 0 then
  begin
  ...
  begin
    ARecordIndex := DataLink.RecordCount - DataLink.ActiveRecord - 1 +
      ADistance;
    DataSet.MoveBy(ARecordIndex);
  end;
...
begin
  if Assigned(DataSet) then
    ...
    begin
      ...
      FSavePosInfo.ActiveRecord := DataSet_ActiveRecord;
      // save offset in DataSet buffer
    ...
    begin
      ...
      begin
        try
          ...
          begin
            MoveBy(DataSet_BufferCount - DataSet_ActiveRecord - 1 +
              DataSet_ActiveRecord - FSavePosInfo.ActiveRecord);
            MoveBy(FSavePosInfo.ActiveRecord - DataSet_BufferCount + 1);
          end
          else if DataSet_ActiveRecord < FSavePosInfo.ActiveRecord then
          begin
            MoveBy(-DataSet_ActiveRecord + DataSet_ActiveRecord -
              FSavePosInfo.ActiveRecord);
            MoveBy(FSavePosInfo.ActiveRecord);
          end;
          // BOF and EOF
          if FSavePosInfo.Bof and not Bof then
            Prior;
          if FSavePosInfo.Eof and not Eof then
            Next;
        except
          on EDatabaseError do;
        end;
...
begin
  Result := Assigned(DataSet) and DataSet.Active and DataSet.CanModify and
    IsSyncMode;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ARecordIndex := DataController.GetRowInfo(DataController.GetRowCount -
          2).RecordIndex;
...
procedure TcxDBDataProvider.DeleteRecords(AList: TList);
  ...
  begin
    if IsSmartRefresh then
    begin
      ARecordIndex := DataController.LocateRecordIndex
        (DataController.GetKeyFields);
      if ARecordIndex <> -1 then
      begin
        DataSet.Delete;
  ...
  begin
    ...
    try
      ...
      try
        ...
        else if not DataController.IsKeyNavigation then
          DataController.ClearSelection;
      end;
    finally
...
function TcxDBDataProvider.GetEditValue(ARecordIndex: Integer;
...
begin
  ...
  begin
    ...
    else if (AEditValueSource = evsKey) and (ADataSetField.KeyFields <> '') then
      Result := ADataSetField.DataSet.FieldValues[ADataSetField.KeyFields]
    else if not ADataSetField.IsNull then
      Result := ADataSetField.Value; // evsValue
  end
  else if AField.IsUnbound then
...
procedure TcxDBDataProvider.AssignItemValue(ARecordIndex: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      FAssignedFields.AddFieldsByFieldNames(ADataSetField.DataSet,
        ADataSetField.KeyFields);
    end
    else
    begin
...
procedure TcxDBDataProvider.DoInitInsertingRecord(AInsertingRecordIndex
...
begin
  ...
  if DataController.GetAssignMasterDetailKeysSetting and DataController.IsDetailMode
  then
  begin
    SetDataSetValues(DataSet, DataController.MasterDetailKeyFields,
      DataController.MasterDetailKeyValues);
...
function TcxDBDataProvider.SetEditValue(ARecordIndex: Integer;
  AField: TcxCustomDataField; const AValue: Variant;
...
begin
  ...
  if Assigned(ADataSetField) then
  begin
    if (AEditValueSource = evsText) and not VarIsNull(AValue) or
      Assigned(ADataSetField.OnSetText) then
    ...
    Result := inherited SetEditValue(ARecordIndex, AField, AValue,
      AEditValueSource);
end;
  ...
  try
    ...
    begin
      if DataController.FNearestRecordIndex < DataController.RecordCount then
        DataController.ChangeFocusedRecordIndex
          (DataController.FNearestRecordIndex);
    end;
  finally
    inherited;
  end;
...
procedure TcxDBDataProvider.LayoutChanged(ADataLayoutChanges
...
begin
  if DataController.IsDetailMode and
    DataController.DataModeController.DetailInSyncMode and not IsDataSetCurrent
  then
...
procedure TcxDBDataProvider.SmartDataChanged;
  ...
  begin
    ...
    try
      ...
      if (AKeyFields.Count = 1) and (TField(AKeyFields[0]).DataType = ftAutoInc)
      then
        Result := True;
    finally
      AKeyFields.Free;
  ...
  function IsNewRecordInserted(const AFocusedRecordValues, AKeyFieldsValues
    : Variant): Boolean;
  begin
    Result := False;
    if not FUpdatingRecordIsInserting then
      Exit;
    // check auto inc
    if VarEquals(AFocusedRecordValues, FNewRecordKeyValue) and IsKeyFieldAutoInc
    then
    begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              DataController.CopyRecord(DataController.NewItemRecordIndex,
                AFocusedRecordIndex);
            end;
          end;
...
begin
  ...
  for I := 0 to DataController.RecordCount - 1 do
    if (I <> AFocusedRecordIndex) and DataController.IsDetailDataControllerExist
      (I, -1) then
    begin
      DataController.CollapseDetails;
      Break;
...
function TcxDBDataProvider.DoLocate(const ASubText: string;
...
var
  ...
  begin
    ...
    try
      S := DataController.GetIncrementalSearchText
        (-1 { set FIncrementalSearching flag! } , ASearchItemIndex);
...
begin
  Result := False;
  if not Assigned(DataSet) or IsEditing then
    Exit;
  with DataSet do
  begin
      ...
      begin
        ...
        try
          ...
          while (not Eof or ANext or AStartEOF) and
            ((Length(AStartPos) = 0) or (Self.CompareBookmarks(Bookmark,
            AStartPos) <> 0)) do
          begin
            AStartEOF := False;
...
begin
  ...
  begin
    ...
    else if IsInserting or FIncrementalSearching then
      Result := ADataSetField.DisplayText
    else
      Result := '';
  end
...
function TcxDBDataProvider.GetDetailHasChildren(ARecordIndex: Integer;
...
begin
  ...
  if APatternDataController <> nil then
    ADetailDataController := APatternDataController.Relations[ARelationIndex]
  ...
  begin
    ...
    if Assigned(DataController.FOnDetailHasChildren) then
      DataController.FOnDetailHasChildren(DataController, ARecordIndex,
        ARelationIndex, ADataController.MasterDetailKeyFieldNames,
        AMasterDetailKeyValues, Result)
    else if not(ADataController.IsGridMode or
      ADataController.DataModeController.DetailInSQLMode) then
    begin
      if Assigned(ADataController.DataSet) and ADataController.DataSet.Active
      then
      begin
        ...
        try
          ...
          try
            Result := ADataController.DataSet.Locate
              (ADataController.MasterDetailKeyFieldNames,
...
begin
  ...
  else if FInUpdateGridMode and IsInserting and DataController.NewItemRowFocused
  then
  ...
  begin
    ...
    try
      ...
      begin
        ...
        AOperation;
      if IsSmartRefresh and (EditingRecordIndex <> cxNullEditingRecordIndex)
      then
...
begin
  if DataController.IsDetailMode and not DataController.DataModeController.
    DetailInSQLMode and IsSyncMode then
    Result := FInCanInitEditing or
      ((FInInsert or (((DataController.GetFocusedRowIndex <> -1) or
      FIsDataSetCurrent) and not IsOtherDetailDataSetInInsert) or
      (DataController.UseNewItemRowForEditing and
      DataController.NewItemRowFocused and IsInserting)) and
...
function TcxDBDataField.CanModify(AEditValueSource
...
begin
  Result := not IsInternal and (FieldName = '') and (Field = nil) and
    (ValueDef <> nil);
end;
...
begin
  ...
  begin
    if Provider.FInFirst and Provider.DataLink.Active and not Provider.DataSet.Active
    then
...
begin
  if FMasterKeyField = nil then
    Result := 0
  else if FMasterKeyField.FieldCount = 0 then
    Result := 1
  else
...
function TcxDBDataFilterCriteriaItem.GetFilterOperatorClass
...
constructor TcxDBDataFilterCriteria.Create(ADataController
  : TcxCustomDataController);
begin
  inherited Create(ADataController);
  FIgnoreOrigin := True;
...
begin
  ...
  begin
    ...
    OnFormatFilterTextValue := TcxDBDataFilterCriteria(Source)
...
procedure TcxDBDataFilterCriteria.FormatFilterTextValue
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ADataSet.Filter <> AFilterText) or
        (ADataSet.FilterOptions <> AFilterOptions) or
...
begin
  ...
  if (DataField <> nil) and (DataController.DBFields[DataField.Index].Field
    <> nil) then
...
begin
  ...
  begin
    ...
    if (AMasterRelation <> nil) and AMasterRelation.IsLinked then
      try
        Result := (AMasterRelation.MasterKeyFieldCount =
          MasterDetailKeyFields.Count);
      except
        on E: Exception do
        begin
          Error(E);
...
function TcxDBDataController.CreateDataControllerInfo
...
procedure TcxDBDataController.DoInitInsertingRecord(AInsertingRecordIndex
  : Integer);
begin
end;
...
function TcxDBDataController.FindRecordByFields(ABufferRecordIndex: Integer;
...
begin
  ...
  try
    if FFocusingRecord and (FFocusingRecordIndex >= 0) and
      (FFocusingRecordIndex < RecordCount) and
      AreFieldValuesEqual(ABufferRecordIndex, FFocusingRecordIndex, AFields)
    then
    begin
...
function TcxDBDataController.FindRecordIndexInGridMode(const AKeyFieldValues
  : Variant): Integer;
var
  AFieldList: TList;
  I: Integer;
...
procedure TcxDBDataController.UpdateField(ADataField: TcxDBDataField;
...
procedure TcxDBDataController.ChangeFieldName(AItemIndex: Integer;
...
function TcxDBDataController.GetItemByFieldName(const AFieldName
  : string): TObject;
var
  I: Integer;
begin
...
function TcxDBDataController.GetGroupValue(ARecordIndex: Integer;
  AField: TcxCustomDataField): Variant;
var
  ADBField: TcxDBDataField;
  I: Integer;
...
begin
  Result := inherited CanFocusedRecordIndexChangePostData and
    Provider.IsDataSetModified and (DataSet.State in dsEditModes);
end;
...
function TcxDBDataController.GetFilterDataValue(ARecordIndex: Integer;
  AField: TcxCustomDataField): Variant;
var
  ADBField: TcxDBDataField;
    ...
    if Assigned(ADBField.Field) then
      ...
      begin
        ...
        if ADBField.FieldCount > 0 then
          ...
          begin
            // Multiply Lookup Keys
            Result := GetFilterFieldName(ADBField[0].Field,
              Filter.IgnoreOrigin);
            for I := 1 to AFieldCount - 1 do
              Result := Result + ';' + GetFilterFieldName(ADBField[I].Field,
                Filter.IgnoreOrigin);
          end
        else
          Result := '';
...
function TcxDBDataController.FindRecordIndexByKey(const AKeyFieldValues
  : Variant): Integer;

  function GetVarArrayElementCount(const AVarArray: Variant): Integer;
  begin
    Result := VarArrayHighBound(AVarArray, 1) -
      VarArrayLowBound(AVarArray, 1) + 1;
  end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        if (ALocateObject.FieldList.Count = 1) and VarIsArray(AKeyFieldValues)
          or (ALocateObject.FieldList.Count <> 1) and
          (not VarIsArray(AKeyFieldValues) or
          (GetVarArrayElementCount(AKeyFieldValues) <
          ALocateObject.FieldList.Count)) then
        begin
          Result := -1;
          Exit;
        end;
        if ALocateObject.FieldList.Count = 1 then
          AValueDefReader.SetValue(TcxDBDataField(ALocateObject.FieldList[0])
            .ValueDef, AKeyFieldValues)
        else
        begin
          for I := 0 to ALocateObject.FieldList.Count - 1 do
            AValueDefReader.SetValue(TcxDBDataField(ALocateObject.FieldList[I])
              .ValueDef, AKeyFieldValues[I]);
        end;
...
function TcxDBDataController.LocateByKey(const AKeyFieldValues
  : Variant): Boolean;
  ...
  begin
    ...
    try
      if DataSet <> nil then
        Result := CanCallDataSetLocate(DataSet, KeyFieldNames, AKeyFieldValues)
          and DataSet.Locate(KeyFieldNames, AKeyFieldValues, []);
    finally
      EndLocate;
    end;
  end;
...
function TcxDBDataController.GetDetailFilterAdapter
  : TcxDBProviderDetailFilterAdapter;
var
  AIndex: Integer;
begin
  if cxDetailFilterControllers.FindAdapter(TDataSetClass(DataSet.ClassType),
    AIndex) then
    Result := cxDetailFilterControllers[AIndex]
      as TcxDBProviderDetailFilterAdapter
  else
    InvalidOperation(cxSDBDetailFilterControllerNotFound);
end;
{$WARNINGS ON}

procedure TcxDBDataController.SetMasterRelation(AMasterRelation
...
begin
  if not IsGridMode then
    InvalidOperation(cxSDBNotInGridMode);
...
begin
  ...
  begin
    ...
    if (ASelection.CompareBookmarks(ASelection.GetRowBookmark(0), ABookmark) <=
      0) and (ASelection.CompareBookmarks(ABookmark,
      ASelection.GetRowBookmark(ARowCount - 1)) <= 0) then
    begin
      for I := 0 to ARowCount - 1 do
        if ASelection.CompareBookmarks(ASelection.GetRowBookmark(I),
          ABookmark) = 0 then
        begin
          Result := I;
          Break;
...
procedure TcxDBDataController.SelectFromAnchor(ARowIndex: Integer;
  AKeepSelection: Boolean);
var
...
begin
  ...
  begin
    ...
    begin
      ASelection := DBSelection;
      ASelection.SelectFromAnchor(ASelection.GetRowBookmark(ARowIndex),
        AKeepSelection);
    end;
  end;
end;
...
procedure TcxDBDataController.ForEachRow(ASelectedRows: Boolean;
...
function TcxDBDataController.InternalCheckBookmark(ADeletedRecordIndex
  : Integer): Boolean;
begin
  if IsGridMode then
...
function TcxDBDataController.IsFieldSupportsExpression
  (AField: TcxCustomDataField): Boolean;
var
  ADBField: TcxDBDataField absolute AField;
begin
  Result := inherited IsFieldSupportsExpression(AField) and
    (ADBField.FieldName = '') and (ADBField.Field = nil)
end;
...
function TcxDBDataController.LocateRecordIndex(AGetFieldsProc
  : TGetListProc): Integer;
var
  ALocateObject: TcxDataControllerKeyLocateObject;
  AValueDefReader: TcxValueDefReader;
begin
...
var
  I: Integer;
  AListener: TObject;
  AProvider: TcxDBDataProvider absolute AListener;
begin
  if Provider.FInNotify then
    Exit;
  Provider.FInNotify := True;
  try
    for I := Provider.FListeners.Count - 1 downto 0 do
    begin
      AListener := Provider.FListeners[I];
      if not AProvider.DataController.IsDestroying then
...
procedure TcxDBDataController.NotifyDetailAfterFieldsRecreating(ADataController
  : TcxCustomDataController);
var
  ADBDataController: TcxDBDataController;
begin
  if ADataController = FCreatedDataController then
    Exit;
...
procedure TcxDBDataController.NotifyDetailsAfterFieldsRecreating
  (ACreatingLinkObject: Boolean);
begin
...
procedure TcxDBDataController.ResetDataSetCurrent(ADataController
  : TcxCustomDataController);
var
  ADBDataController: TcxDBDataController;
begin
  ADBDataController := ADataController as TcxDBDataController;
  if ADBDataController.FInResetDataSetCurrent then
    Exit;
  ...
  begin
    ...
    begin
      if ADBDataProvider.DataController.FInCheckCurrentQuery or
        ...
        ADBDataProvider.DataController.GetMasterDataController.
        IsCreatingLinkObject) then
      begin
        Result := True;
        Break;
      end;
...
procedure TcxDBDataController.RetrieveField(ADataField: TcxDBDataField;
  AIsLookupKeyOnly: Boolean);

  function IsLookupKeyField(ADataField: TcxDBDataField): Boolean;
  begin
    Result := Assigned(ADataField.ParentField) and
      Assigned(ADataField.ParentField.Field) and
...
var
  APrevField: TField;
begin
  if Assigned(ADataField.ReferenceField) then
    Exit;
  if AIsLookupKeyOnly and not IsLookupKeyField(ADataField) then
    Exit;
  ...
  begin
    ...
    begin
      ADataField.Field := FindField(Provider.DataSet.Fields,
        ADataField.FieldName);
      if ADataField.Field = nil then
        ADataField.Field := FindField(Provider.DataSet.AggFields,
          ADataField.FieldName);
    end
    else
...
begin
  ...
  begin
    ...
    begin
{$WARNINGS OFF}
      Provider.DataSet.GetFieldList(FMasterDetailKeyFields,
        MasterDetailKeyFieldNames);
{$WARNINGS ON}
...
procedure TcxDBDataController.MasterDetailKeyFieldsRemoveNotification
...
procedure TcxDBDataController.SetDataModeController
  (Value: TcxDBDataModeController);
begin
...
begin
  ...
  begin
    if not IsLoading and not Provider.ActiveChanging and not Provider.IsDataSetBusy
    then // !!!
      SyncMasterDetailDataSetPos;
  end
  else if IsKeyNavigation then
  begin
...
function TcxDBDataController.SyncMasterDetail: TcxCustomDataController;
  ...
  function ExpandDetails(ADataController: TcxCustomDataController;
    ARelationIndex: Integer; const AMasterKeyValues: Variant)
    : TcxCustomDataController;
  var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ACurRelationIndex := ADataController.GetDetailActiveRelationIndex
          (ARecordIndex);
        if ARelationIndex <> ACurRelationIndex then
        begin
          ACurDetailDataController := ADataController.GetDetailDataController
            (ARecordIndex, ACurRelationIndex) as TcxDBDataController;
          if not ACurDetailDataController.Provider.IsActiveDataSet then
            Exit;
          ACurMasterRelation := ACurDetailDataController.GetMasterRelation as
            TcxDBDataRelation;
          ACurMasterDetailKeyValues :=
            ACurDetailDataController.Provider.DataSet.FieldValues
            [ACurMasterRelation.DetailKeyFieldNames];
        ...
        ADataController.ChangeDetailActiveRelationIndex(ARecordIndex,
          ARelationIndex);
        Result := ADataController.GetDetailDataController(ARecordIndex,
          ARelationIndex)
      end;
    end;
  end;

  function IsMastersInSyncMode(AMasterDataController
    : TcxDBDataController): Boolean;
  var
    AMasterRelation: TcxDBDataRelation;
  begin
    ...
    if Result then
    begin
      AMasterRelation := AMasterDataController.GetMasterRelation as
        TcxDBDataRelation;
      if AMasterRelation <> nil then
...
begin
  ...
  begin
    Result := nil;
    if not(GetMasterRelation is TcxDBDataRelation) then
      Exit;
    ...
    begin
      ...
      try
        AMasterDetailKeyValues := Provider.DataSet.FieldValues
          [AMasterRelation.DetailKeyFieldNames];
        AMasterKeyValues := AMasterDataController.Provider.DataSet.FieldValues
          [AMasterRelation.MasterKeyFieldNames];
        ...
        begin
          ...
          begin
            ...
            try
              AEqualFlag := AMasterDataController.Provider.DataSet.Locate
                (AMasterRelation.MasterKeyFieldNames,
...
begin
  if FUpdateDataSetPos or FInCheckBrowseMode then
    Exit;
  SyncMasterDetail;
end;
...
begin
  for I := 0 to Relations.Count - 1 do
    ...
    begin
      AMasterKeyFieldNames := (Relations[I] as TcxDBDataRelation)
        .MasterKeyFieldNames;
...
begin
  ...
  cxDetailFilterControllers.RegisterAdapter(TDataSet,
    TcxDBProviderDetailFilterAdapter);
end;
