...
function LikeOperatorByPattern(var APatternStr: string; APercent: Char)
  : TcxFilterLikeOperator;
function LikeStr(const AStr, APatternStr: string; APercent, AUnderline: Char;
  AEscape: Char = #0): Boolean;
...
begin
  ...
  repeat
    I := Pos(S, PatternStr);
    if I > 0 then
      PatternStr := Copy(PatternStr, 1, I - 1) + APercent +
        Copy(PatternStr, I + 2, MaxInt);
  until I = 0;
end;
...
function Like(AText: PChar; ATextLen: Integer; APattern: PChar;
  APatternLen: Integer; const APercentChar, AUnderlineChar,
  AEscapeChar: Char): Boolean;
var
  C: Char;
  AEscapeFlag: Boolean;
begin
  ...
  repeat
    ...
    begin
      ...
      while ATextLen > 0 do
      begin
        if Like(AText, ATextLen, APattern, APatternLen, APercentChar,
          AUnderlineChar, AEscapeChar) then
        begin
          Result := True;
          Exit;
        end;
