...
{$ENDIF}
  ...
  public
    ...
    procedure Clear;
    function FindAdapter(ADataSetClass: TDataSetClass; var AIndex: Integer)
      : Boolean; virtual;
    procedure RegisterAdapter(ADataSetClass: TDataSetClass;
      AItemClass: TcxDBAdapterItemClass); virtual;
    procedure UnregisterAdapter(ADataSetClass: TDataSetClass;
      AItemClass: TcxDBAdapterItemClass); virtual;
  ...
  public
    ...
    function GetStoredValue(AValueSource: TcxDataEditValueSource;
      AFocused: Boolean): Variant; override;
    ...
    procedure SetStoredValue(AValueSource: TcxDataEditValueSource;
...
begin
{$IFDEF DELPHI20}
  Result := (TDataSetAccess(ADataSet).FieldOptions.AutoCreateMode <>
    acExclusive) or not(lcPersistent in ADataSet.Fields.LifeCycles);
{$ELSE}
...
begin
  Result := AField.CanModify and
    (AIsValueSource or not(AField.DataType in ftNonTextTypes) or
    Assigned(AField.OnSetText));
end;
...
begin
  ...
  begin
    ...
    begin
      Result := (TNumericField(AField).DisplayFormat <> '') or
        (TNumericField(AField).EditFormat <> '');
      if not Result then
      begin
      ...
      Result := (TAggregateField(AField).DisplayFormat <> '') or
        TAggregateField(AField).Currency
...
begin
  ...
  begin
    ...
    if Result then
    begin
      if (TNumericField(AField).DisplayFormat <> '') or
        Assigned(AField.OnGetText) then
        Result := False;
    end;
  end;
end;
...
procedure SetFieldValue(AField: TField; const AValue: Variant);
begin
  if (AField is TDateTimeField) and (VarType(AValue) = varDouble) then
  // bug in Delphi
...
begin
  ...
  begin
    ...
    else if AItem.DataSetClass.InheritsFrom(ADataSetClass) then
      AIndex := I;
  end;
end;
...
begin
  ...
  begin
    AItem := TcxDBAdapterItem(FItems[I]);
    if (AItem.DataSetClass = ADataSetClass) and (AItem.ClassType = AItemClass)
    then
    begin
      AItem.Free;
      FItems.Delete(I);
...
begin
  Result := not ReadOnly and (Field <> nil) and
...
function TcxCustomDBDataBinding.GetStoredValue(AValueSource
  : TcxDataEditValueSource; AFocused: Boolean): Variant;
begin
...
procedure TcxCustomDBDataBinding.SetStoredValue(AValueSource
  : TcxDataEditValueSource; const Value: Variant);

  procedure SetFieldValueEx(AField: TField; const AValue: Variant);
  begin
    if VarIsStr(Value) and (Value = '') and
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        if (* ( *) AValueSource = evsText (* ) or Assigned(Field.OnSetText) *)
        then
          Field.Text := VarToStr(Value)
        else if (AValueSource = evsKey) and (Field.KeyFields <> '') then
