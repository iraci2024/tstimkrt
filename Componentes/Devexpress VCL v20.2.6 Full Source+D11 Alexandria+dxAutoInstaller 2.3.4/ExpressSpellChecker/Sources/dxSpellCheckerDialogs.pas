...
uses
  cxLookAndFeels, dxSpellChecker, dxCustomDictionaryDialog,
...
const
  dxCustomDictionaryDialogClass: TdxCustomDictionaryFormClass =
    TfmCustomDictionaryForm;
  dxSpellCheckerAutoCorrectExceptionsDialogClass
    : TdxCustomSpellCheckerAutoCorrectFormClass =
    TfmSpellCheckerAutoCorrectExceptionsForm;
  dxSpellCheckerAutoCorrectOptionsDialogClass
    : TdxCustomSpellCheckerAutoCorrectFormClass =
    TfmSpellCheckerAutoCorrectOptionsForm;
  dxSpellCheckerOutlookDialogClass: TdxCustomSpellCheckerFormClass =
    TfmSpellCheckerOutlookForm;
  dxSpellCheckerWordDialogClass: TdxCustomSpellCheckerFormClass =
    TfmSpellCheckerWordForm;
  dxSpellCheckerSpellingOptionsDialogClass
    : TfmSpellCheckerSpellingOptionsFormClass =
    TfmSpellCheckerSpellingOptionsForm;

function dxShowCustomDictionaryDialog(ADictionary
  : TdxUserSpellCheckerDictionary; ALookAndFeel: TcxLookAndFeel = nil): Integer;
...
begin
  ADialog := dxSpellCheckerAutoCorrectExceptionsDialogClass.CreateEx
    (ASpellChecker.AutoCorrectOptions);
  try
    if ALookAndFeel = nil then
...
var
  ADialog: TdxCustomSpellCheckerAutoCorrectForm;
begin
  ADialog := dxSpellCheckerAutoCorrectOptionsDialogClass.CreateEx
    (ASpellChecker.AutoCorrectOptions);
  try
    if ALookAndFeel = nil then
      ALookAndFeel := ASpellChecker.DialogLookAndFeel;
