...
{$ENDIF}
  ...
  cxContainer, cxEdit, cxTextEdit, cxMemo, dxSpellChecker,
...
function TfmSpellCheckerWordForm.IsModifyMisspelledWordOnly
  (out ANewWord: string): Boolean;
var
  ...
  ANewLength: Integer;
begin
  APrevPart := Copy(MisspelledSentence.Text, 1,
    MisspelledSentence.MisspelledWordPositionInText);
  ANextPartStart := MisspelledSentence.MisspelledWordPositionInText +
    Length(MisspelledWord) + 1;
  ...
  ANewNextPart := Copy(reMisspelledText.Text, ANextPartStart,
    ANewLength - ANextPartStart + 1);

  Result := (Length(APrevPart) + Length(ANextPart) <= ANewLength) and
    WideSameStr(APrevPart, ANewPrevPart) and
    WideSameStr(ANextPart, ANewNextPart);
...
procedure TfmSpellCheckerWordForm.SetSuggestions(ASuggestions
...
begin
  ...
  try
    ...
    begin
      lbxSuggestions.Items.Add
        (cxGetResourceString(@sdxSpellCheckerNoSuggestions));
...
begin
  case CheckMode.LastCode of
    scMisspelled:
      begin
        lbMisspelled.Caption := cxGetResourceString
...
begin
  ...
  btnChangeAll.Enabled := (HasSuggestions or IsDeleteMisspelled or IsChanged)
    and not IsDeleteMisspelled;
  lbxSuggestions.Enabled := not liUndoEdit.Visible;
  liIgnore.Visible := not liUndoEdit.Visible;
  btnIgnoreAll.Enabled := not liUndoEdit.Visible and
    not(CheckMode.LastCode = scRepeatedWords);
  btnAdd.Enabled := not liUndoEdit.Visible and
    (CheckMode.LastCode <> scRepeatedWords) and
    SpellChecker.HasEnabledUserDictionary;
...
begin
  ...
  try
    reMisspelledText.SetSelection
      (MisspelledSentence.MisspelledWordPositionInText, Length(MisspelledWord));
    reMisspelledText.SelAttributes.Color := clRed;
    reMisspelledText.SelAttributes.Style := [fsBold];
    reMisspelledText.SetSelection
      (MisspelledSentence.MisspelledWordPositionInText +
      Length(MisspelledWord), 0);
...
begin
  ...
  begin
    ...
    btnChangeAll.Caption := cxGetResourceString
