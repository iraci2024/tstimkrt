...
uses
  ...
  cxGraphics, cxLookAndFeels, dxSpellCheckerCore, dxLayoutcxEditAdapters,
  dxLayoutContainer, dxLayoutControlAdapters,
  cxClasses, dxLayoutControl, dxLayoutLookAndFeels;

type

  TfrmSpellCheckerCandidateEvent = procedure(var S: string;
  ...
  public
    constructor CreateEx(AOwner: TComponent;
      AExceptions: TdxSpellCheckerAutoCorrectExceptions;
      ALocalizeIndex: Integer); virtual;

    property Exceptions: TdxSpellCheckerAutoCorrectExceptions read FExceptions;
    property OnCandidateExist: TfrmSpellCheckerCandidateEvent
      read FOnCandidateExist write FOnCandidateExist;
    property OnCandidateValidate: TfrmSpellCheckerCandidateEvent
      read FOnCandidateValidate write FOnCandidateValidate;
  end;
...
constructor TfrmSpellCheckerExceptions.CreateEx(AOwner: TComponent;
  AExceptions: TdxSpellCheckerAutoCorrectExceptions; ALocalizeIndex: Integer);
begin
...
begin
  S := teCandidate.Text;
  AAddButtonEnabled := (Length(teCandidate.Text) > 0) and
    DoCandidateValidate(S);
...
begin
  ...
  begin
    if btnAdd.Enabled then
      btnAddClick(nil)
    else if btnDelete.Enabled then
      btnDeleteClick(nil);
  end;
end;
