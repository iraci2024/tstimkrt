...
type
  ...
  public
    ...
    function Decode(AWordStemManager: TdxHunspellWordStemManager;
      AFlags: PWideChar): Boolean;
    function InitializeByAlias(AWordStemManager: TdxHunspellWordStemManager;
      AAlias: PWideChar): Boolean;
    procedure Sort;
  ...
  public
    constructor Create(const AWord: PWideChar; AWordLength: Integer;
      const AIgnoreChars: PWideChar; AReverse: Boolean);
    destructor Destroy; override;
    function IsCompatibleWithFlag(AFlag: Word;
      CompatibleIfNull: Boolean = False): Boolean; inline;
    ...
    property NextHomonym: TdxHunspellWordStem read FNextHomonym
      write FNextHomonym;
  ...
  private
    ...
    //
    procedure AddWord(const AWord: PWideChar; AWordLength: Integer;
      AAffixFlags: TdxHunspellFlags; const AMorphologocalDescription: PWideChar;
      AOnlyUpperCase: Boolean);
    ...
    function InitializeAffixFlags(AAffixFlags: TdxHunspellFlags;
      ADescription: PWideChar): Boolean;
  ...
  public
    ...
    function GetNextItem(var ATableIndex: Integer;
    ...
    property AffixFlagMode: TdxAffixFlagMode read FAffixFlagMode
      write FAffixFlagMode;
    property AffixMorphologicAliases: PdxPWideCharArray
      read FAffixMorphologicAliases;
    property AffixMorphologicAliasesCount: Integer
      read FAffixMorphologicAliasesCount;
    property CodePage: Cardinal read FCodePage write FCodePage;
    property ComplexPrefixes: Boolean read FComplexPrefixes
      write FComplexPrefixes;
    property ForbiddenWordFlag: Word read FForbiddenWordFlag
      write FForbiddenWordFlag;
...
function TdxHunspellFlags.InitializeByAlias(AWordStemManager
  : TdxHunspellWordStemManager; AAlias: PWideChar): Boolean;
var
  AAliasNum: Integer;
begin
  AAliasNum := StrInt(AAlias);
  Result := (AAliasNum > 0) and
    (AAliasNum <= AWordStemManager.AffixFlagAliases.Count);
  DisposeData;
  if Result then
  begin
    ...
    afmTwoCharacters:
      begin
        ...
        for I := 0 to Length - 1 do
          Data[I] := Ord((AFlags + I * 2)^) shl 8 + Ord((AFlags + I * 2 + 1)^);
      end;
    afmNumber:
      begin
        ALength := 1;
...
constructor TdxHunspellWordStem.Create(const AWord: PWideChar;
  AWordLength: Integer; const AIgnoreChars: PWideChar; AReverse: Boolean);
begin
  inherited Create;
...
procedure TdxHunspellWordStem.ProcessFlags(AWordStemManager
  : TdxHunspellWordStemManager);
begin
...
procedure TdxHunspellWordStem.SetMorphology(const AMorphologocalDescription
  : PWideChar);
begin
...
function TdxHunspellReader.GetNextLine(var ABuffer;
...
procedure TdxHunspellWordStemManager.AllocateAffixMorphologicAliases
  (ASize: Integer);
begin
...
function TdxHunspellWordStemManager.Load(ADictionaryStream: TStream): Boolean;
begin
...
function TdxHunspellWordStemManager.Lookup(const AWord: PWideChar)
  : TdxHunspellWordStem;
begin
...
function TdxHunspellWordStemManager.GetFirstItem(out ATableIndex: Integer)
  : TdxHunspellWordStem;
begin
  ATableIndex := -1;
...
procedure TdxHunspellWordStemManager.AddWord(const AWord: PWideChar;
  ...
  function InitializeWordStem(AWordStem: TdxHunspellWordStem)
    : TdxHunspellWordStem;
  var
...
begin
  ...
  begin
    if not ProcessItems(ANewWordStem, AExistingWordStem, AHasUpperCaseHomonym,
      True) then
      Exit;
...
begin
  ACapitalizationType := GetWordCapitalizationType(AWord, AWordLength,
    Language);
...
begin
  ...
  FreeMem(FAffixMorphologicAliases, FAffixMorphologicAliasesCount *
    SizeOf(PWideChar));
...
function TdxHunspellWordStemManager.GetAffixesDescription(ALine: PWideChar)
  : PWideChar;
var
  ATemp: PWideChar;
begin
...
function TdxHunspellWordStemManager.GetAffixMorphologyByAlias(AAlias: Integer)
  : PWideChar;
begin
...
function TdxHunspellWordStemManager.GetMorphologyDescription(ALine: PWideChar)
  : PWideChar;
var
...
begin
  ...
  while Result <> nil do
  begin
    if (Result > ALine + 3) and (((Result - 3)^ = ' ') or ((Result - 3)^ = #9))
    then
    begin
      Dec(Result, 4);
...
function TdxHunspellWordStemManager.CreateWordStem(const AWord: PWideChar;
  AWordLength: Integer; const AMorphologyDescription: PWideChar)
  : TdxHunspellWordStem;
begin
  Result := TdxHunspellWordStem.Create(AWord, AWordLength, FIgnoredChars,
    FComplexPrefixes);
  SetupMorphology(Result, AMorphologyDescription);
end;

procedure TdxHunspellWordStemManager.SetupMorphology(var AWordStem
  : TdxHunspellWordStem; const AMorphologyDescription: PWideChar);
begin
  if AMorphologyDescription = nil then
    Exit;
  if HasAffixMorphologicAliases then
    AWordStem.SetMorphologyByAlias
      (GetAffixMorphologyByAlias(StrInt(AMorphologyDescription)))
  else
