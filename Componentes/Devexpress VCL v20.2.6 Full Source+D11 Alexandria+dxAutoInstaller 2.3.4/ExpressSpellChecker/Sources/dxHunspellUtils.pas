...
function CopyMorphology(ADestination: PWideChar;
  const AMorphologicalDescription, AMorphologicalField: PWideChar): PWideChar;
...
function StrReplace(S: PWideChar; APattern: PWideChar; AReplacement: PWideChar)
  : PWideChar;
...
begin
  ...
  begin
    ...
    while not((APosition^ = #0) or (APosition^ = ' ') or (APosition^ = #9) or
      (APosition^ = #10)) do
    begin
...
begin
  ...
  if AIndex >= L then
    Exit;
  LCMapStringW(ALangID, LCMAP_LOWERCASE + LCMAP_LINGUISTIC_CASING, S + AIndex,
    1, S + AIndex, 1);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      while (ADelimiterPos^ <> #0) and (ADelimiterPos^ <> ' ') and
        (ADelimiterPos^ <> #9) do
        Inc(ADelimiterPos);
