...
uses
  ...
  dxCore, dxSpellCheckerBaseForm, dxSpellChecker, cxLookAndFeelPainters,
  cxControls, cxContainer, cxEdit, cxGroupBox,
  cxCheckBox, cxLabel, cxTextEdit, ComCtrls, cxListView, cxGraphics,
  cxLookAndFeels, dxLayoutLookAndFeels, cxClasses,
...
type
  ...
  TdxCustomSpellCheckerAutoCorrectFormClass = class of
    TdxCustomSpellCheckerAutoCorrectForm;
  ...
  TfmSpellCheckerAutoCorrectOptionsForm = class
    (TdxCustomSpellCheckerAutoCorrectForm)
    btnCancel: TcxButton;
    btnDelete: TcxButton;
...
uses
  dxSpellCheckerStrs, dxSpellCheckerUtils, dxSpellCheckerDialogs, DateUtils,
  dxSpellCheckerCore;
...
constructor TfmSpellCheckerAutoCorrectOptionsForm.CreateEx
  (AOptions: TdxSpellCheckerAutoCorrectOptions);
begin
...
begin
  ...
  gbCapitalize.Caption := cxGetResourceString
    (@sdxSpellCheckerAutoCorrectCapitalize);
  cbCorrectSentenceCaps.Caption :=
    cxGetResourceString(@sdxSpellCheckerAutoCorrectCorrectSentenceCaps);
  cbCorrectInitialCaps.Caption := cxGetResourceString
    (@sdxSpellCheckerAutoCorrectCorrectInitialCaps);
  cbCorrectCapsLock.Caption := cxGetResourceString
    (@sdxSpellCheckerAutoCorrectCorrectCapsLock);
  cbDisableCapsLock.Caption := cxGetResourceString
    (@sdxSpellCheckerAutoCorrectDisableCapsLock);
  cbAutomaticallyUseSuggestions.Caption :=
    cxGetResourceString(@sdxSpellCheckerAutoCorrectAutomaticallyUseSuggestions);
  gbReplaceTextAsYouType.Caption :=
    cxGetResourceString(@sdxSpellCheckerAutoCorrectReplaceTextAsYouType);
  cbReplaceTextAsYouType.Caption := cxGetResourceString(@sdxSpellCheckerActive);
  btnExceptions.Caption := cxGetResourceString
...
begin
  ...
  btnOperation.Enabled := (Length(teReplace.Text) > 0) and
    (not btnDelete.Enabled or (WideCompareStr(AItem.Replacement,
    teWith.Text) <> 0));
end;
