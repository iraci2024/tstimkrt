...
uses
  Windows, Messages, Comctrls, Controls, Forms, Classes, Menus, ShlObj,
  StdCtrls,
  dxCOre, dxMessages, cxGraphics, cxGeometry, cxContainer, cxControls,
  cxDataUtils,
  cxCustomData, cxScrollBar, cxLookAndFeels, cxLookAndFeelPainters, cxListView,
  cxShellCommon, cxShellControls, cxHeader, cxEdit;

type
  TcxShellObjectPathType = (sptAbsolutePhysical, sptRelativePhysical, sptUNC,
    sptVirtual, sptInternalAbsoluteVirtual, sptInternalRelativeVirtual,
    sptIncorrect);
  ...
  TcxBeforeNavigationEvent = procedure(Sender: TcxCustomShellListView;
    ANewAbsolutePIDL: PItemIDList) of object;
  TcxCurrentFolderChangedEvent = procedure(Sender: TcxCustomShellListView)
    of object;
  ...
  published
    property Size: TcxShellIconSize read GetSize write SetSize
      default isDefault;
  end;
  ...
  TcxCustomShellListView = class(TcxListViewContainer, IcxShellDependedControls,
    IcxShellRoot)
  private
    ...
    procedure ExecuteItemHandler(Sender: TObject; APIDL: PItemIDList;
  ...
  protected
    ...
    procedure CurrentFolderChangedHandler(Sender: TObject;
      Root: TcxCustomShellRoot); virtual;
    ...
    property DragDropSettings: TcxDragDropSettings read GetDragDropSettings
      write SetDragDropSettings;
    property IconOptions: TcxShellIconOptions read GetIconOptions
      write SetIconOptions;
    property ListHotTrack: Boolean read GetListHotTrack write SetListHotTrack
      default False;
    ...
    property ThumbnailOptions: TcxShellThumbnailOptions read GetThumbnailOptions
      write SetThumbnailOptions;
    property OnAddFolder: TcxShellAddFolderEvent read FOnAddFolder
      write FOnAddFolder;
    property OnBeforeNavigation: TcxBeforeNavigationEvent
...
function GetPIDLDisplayName(APIDL: PItemIDList;
  AShowFullPath: Boolean = False): string;
...
function PathToAbsolutePIDL(APath: string; ARoot: TcxCustomShellRoot;
  AViewOptions: TcxShellViewOptions; ACheckIsSubPath: Boolean = True)
  : PItemIDList;
...
var
  cxShellSpecialFolderInfoTable: array [CSIDL_DESKTOP .. CSIDL_HISTORY]
    of TcxShellSpecialFolderInfoTableItem;
...
begin
  ...
  if (APathLength > 1) and (APath[APathLength] = '\') and
    (APath[APathLength - 1] <> ':') then
  begin
    Dec(APathLength);
  ...
  begin
    S := AnsiUpperCase(Copy(APath, 1,
      cxShellObjectInternalVirtualPathPrefixLength));
  ...
  begin
    if (Length(ACurrentPath) < 3) or (ACurrentPath[2] <> ':') or
      (ACurrentPath[3] <> '\') or not dxCharInSet(ACurrentPath[1],
      ['A' .. 'Z', 'a' .. 'z']) then
      Exit;
    if (APath[1] <> '\') and (UpperCase(APath[1]) <> UpperCase(ACurrentPath[1]))
    then
      Exit;
    ...
    else if not AIsDisplayText then
      Exit
    else
    begin
...
begin
  ...
  begin
    ...
    if AShowFullPath then
    begin
      if AParentIFolder.GetDisplayNameOf(ATempPIDL, SHGDN_INFOLDER, AStrRet) <> S_OK
      then
        Break;
      if Result <> '' then
        Result := Result + '\';
    ...
    AParentIFolder.BindToObject(ATempPIDL, nil, IID_IShellFolder,
      Pointer(AParentIFolder));
    DisposePidl(ATempPIDL);
  end;

  if not AShowFullPath and (AParentIFolder.GetDisplayNameOf(APIDL, SHGDN_NORMAL,
    AStrRet) = S_OK) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if AnsiUpperCase(GetShellItemDisplayName(AParentIFolder, ATempPIDL,
          True)) = ADisplayName then
        begin
          Result := ATempPIDL;
          Break;
        end
...
function PathToAbsolutePIDL(APath: string; ARoot: TcxCustomShellRoot;
  ...
  begin
    ...
    APathType := CheckShellObjectPath(APath,
      AnsiUpperCase(GetPidlName(ARoot.PIDL)), False);
    case APathType of
      sptIncorrect:
        Exit;
      sptAbsolutePhysical, sptUNC, sptVirtual:
        Result := InternalParseDisplayName(GetDesktopIShellFolder, APath,
          AViewOptions);
      sptInternalAbsoluteVirtual, sptInternalRelativeVirtual:
        Result := ShellObjectInternalVirtualPathToPIDL(APath, ARoot,
          AViewOptions);
      sptRelativePhysical:
        begin
          ATempPIDL := InternalParseDisplayName(ARoot.ShellFolder, APath,
            AViewOptions);
...
begin
  ...
  if (Result <> nil) and ACheckIsSubPath and not IsSubPath(ARoot.PIDL, Result)
  then
  begin
    DisposePidl(Result);
    Result := nil;
...
begin
  ...
  if Copy(APath, 1, cxShellObjectInternalVirtualPathPrefixLength) = cxShellObjectInternalAbsoluteVirtualPathPrefix
  then
  begin
    AParentIFolder := GetDesktopIShellFolder;
    ATempPIDL := nil;
  ...
  for ACSIDL := CSIDL_DESKTOP to CSIDL_HISTORY do
    ...
    begin
      ...
      begin
        ...
        if ADesktopIFolder.GetAttributesOf(1, ATempPIDL, Attributes) <> NOERROR
        then
...
begin
  Result := inherited UpdateAction(Action) or TcxCustomDataBindingAccess
...
function TcxInnerShellListView.NeedCheckScrollBars
  (var Message: TMessage): Boolean;
begin
  Result := inherited NeedCheckScrollBars(Message) or
    (Message.Msg = DSM_NOTIFYUPDATECONTENTS) or (Message.Msg = DSM_NOTIFYUPDATE)
    or (Message.Msg = WM_SETREDRAW) and (Message.WParam <> 0);
end;
...
var
  APIDL: PItemIDList;
begin
  APIDL := PathToAbsolutePIDL(AValue, Root,
    Container.GetViewOptions(True), False);
  if APIDL <> nil then
    try
      AbsolutePIDL := APIDL;
    finally
...
function TcxCustomShellListView.GetItemAbsolutePIDL(AIndex: Integer)
  : PItemIDList;
begin
  CheckShellRoot(Root);
...
procedure TcxCustomShellListView.Sort(AColumnIndex: Integer;
...
procedure TcxCustomShellListView.CurrentFolderChangedHandler(Sender: TObject;
...
function TcxCustomShellListView.GetViewOptions(AForNavigation: Boolean = False)
  : TcxShellViewOptions;
begin
...
begin
  case Message.Msg of
    DSM_FIRST .. DSM_LAST:
      ...
      begin
        Message.Result := SendMessage(InnerControl.Handle, Message.Msg,
...
procedure TcxCustomShellListView.BeforeNavigationHandler
  (Sender: TcxCustomInnerShellListView; APItemIDList: PItemIDList;
  AFolderPath: WideString);
begin
...
procedure TcxCustomShellListView.EditingHandler(Sender: TObject;
...
procedure TcxCustomShellListView.ExecuteItemHandler(Sender: TObject;
...
procedure TcxCustomShellListView.SelectItemHandler(Sender: TObject;
...
procedure TcxCustomShellListView.SetDragDropSettings
  (Value: TcxDragDropSettings);
begin
...
begin
  Result := TcxCustomShellListView(ListViewContainer)
    .InnerListView.LargeIconSize;
end;

procedure TcxShellIconOptions.SetSize(const Value: TcxShellIconSize);
begin
  TcxCustomShellListView(ListViewContainer).InnerListView.LargeIconSize
    := Value;
end;
