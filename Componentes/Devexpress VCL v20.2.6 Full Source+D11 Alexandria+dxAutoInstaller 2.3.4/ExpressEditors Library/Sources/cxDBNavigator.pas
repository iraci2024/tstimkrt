...
type
  ...
  protected
    ...
    property OnGetDataLink: TcxDBNavigatorButtonsGetDataLink read FOnGetDataLink
      write FOnGetDataLink;
  public
  ...
  published
    ...
    property OnApplyFilter: TcxDBNavigatorButtonsFilterEvent read FOnApplyFilter
  ...
  TcxCustomDBNavigator = class(TcxCustomNavigator, IcxNavigatorRecordPosition)
  private
  ...
  protected
    ...
    property OnShowFilterDialog: TcxDBNavigatorFilterEvent
      read FOnShowFilterDialog write SetOnShowFilterDialog;
  public
...
var
  AFilterControl: TcxDBFilterControl;
begin
  AFilterControl := TfmFilterControlDialog(Sender)
    .FilterControl as TcxDBFilterControl;
...
begin
  ...
  case ADefaultIndex of
    NBDI_FIRST:
      DataSet.First;
    NBDI_PRIORPAGE:
      DataSet.MoveBy(-InternalPageSize);
    NBDI_PRIOR:
      DataSet.Prior;
    NBDI_LAST:
      DataSet.Last;
...
function TcxDBNavigatorButtons.GetButtonEnabled(ADefaultIndex: Integer)
  : Boolean;
begin
  ...
  begin
    case ADefaultIndex of
      ...
      NBDI_DELETE:
        Result := DataSet.CanModify and not(DataSet.Bof and DataSet.Eof);
      NBDI_EDIT:
        Result := DataSet.CanModify and not DataLink.Editing;
      NBDI_POST, NBDI_CANCEL:
        Result := DataSet.CanModify and DataLink.Editing;
      NBDI_GOTOBOOKMARK:
        Result := Bookmark <> nil;
      NBDI_SAVEBOOKMARK:
...
begin
  ...
  begin
    if (DataSource <> nil) and not(csDestroying in DataSource.ComponentState)
    then
...
procedure TcxCustomDBNavigator.SetInfoPanel
  (Value: TcxNavigatorControlInfoPanel);
begin
  CustomInfoPanel.Assign(Value);
end;

procedure TcxCustomDBNavigator.SetOnApplyFilter
...
procedure TcxCustomDBNavigator.SetOnShowFilterDialog
  (Value: TcxDBNavigatorFilterEvent);
begin
...
procedure TcxCustomDBNavigator.Notification(AComponent: TComponent;
