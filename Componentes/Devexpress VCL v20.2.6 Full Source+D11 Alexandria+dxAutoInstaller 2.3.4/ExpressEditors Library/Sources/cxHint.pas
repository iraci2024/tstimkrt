...
{$ENDIF}
  ...
  cxGraphics, cxLookAndFeels, cxLookAndFeelPainters, dxCustomHint, ComCtrls,
  CommCtrl, dxTypeHelpers, cxGeometry;

type
  TcxHintWindow = class;
  TcxHintAnimationDelay = 0 .. 1000;
  TcxCallOutPosition = (cxbpNone, cxbpAuto, cxbpLeftBottom, cxbpLeftTop,
    cxbpTopLeft, cxbpTopRight, cxbpRightBottom, cxbpRightTop, cxbpBottomRight,
    cxbpBottomLeft);
  ...
  protected
    ...
    procedure ControllerChangedNotification(AStyleController
      : TcxCustomHintStyleController); override;
    ...
    property HintStyleController: TcxCustomHintStyleController
      read BaseGetHintStyleController write BaseSetHintStyleController;
  ...
  public
    ...
    procedure EndUpdate;
    class function GetDefaultHintStyleController
      : TcxCustomHintStyleController; virtual;
    function GetHintWindowClass: TcxCustomHintWindowClass; override;
    procedure RestoreDefaults; virtual;
  published
    property Animate: TcxHintAnimate read FAnimate write SetAnimate
      default cxhaAuto;
    property AnimationDelay: TcxHintAnimationDelay read FAnimationDelay
      write SetAnimationDelay default 100;
    property BorderColor: TColor read FBorderColor write SetBorderColor
      default clWindowFrame;
    property CallOutPosition: TcxCallOutPosition read FCallOutPosition
      write SetCallOutPosition default cxbpNone;
    property CaptionFont: TFont read FCaptionFont write SetCaptionFont;
    property Color: TColor read FColor write SetColor default clInfoBk;
    property Font: TFont read GetFont write SetFont;
    property Icon: TIcon read FIcon write SetIcon;
    property IconSize: TcxHintIconSize read FIconSize write SetIconSize
      default cxisDefault;
    property IconType: TcxHintIconType read FIconType write SetIconType
      default cxhiNone;
  ...
  private
    ...
    procedure GetTextBounds(var ARect: TRect; const AText: string;
      AFont: TFont);
  protected
    ...
    procedure AdjustHintPositionByPoint(const APoint: TPoint;
    ...
    function GetAutoCallOutPosition(const ARect: TRect)
      : TcxCallOutPosition; virtual;
    ...
    property CalculatedCallOutPos: TcxCallOutPosition
      read FCalculatedCallOutPos;
  ...
  protected
    ...
    property AbsolutePosition: Boolean read FAbsolutePosition
  ...
  public
    ...
    property Animate: TcxHintAnimate read GetAnimate write SetAnimate;
    // obsolete
    property BorderColor: TColor read FBorderColor write FBorderColor;
    property CallOutPosition: TcxCallOutPosition read FCallOutPosition
      write FCallOutPosition;
...
uses
  ...
  dxCoreGraphics, cxEditUtils, cxExtEditUtils, dxDPIAwareUtils;
...
class function TcxHintStyle.GetDefaultHintStyleController
...
procedure TcxHintStyle.BaseSetHintStyleController
  (Value: TcxCustomHintStyleController);

  function CheckHintStyleController(AHintStyleController
    : TcxCustomHintStyleController): Boolean;
  var
    AOwner: TPersistent;
  begin
    Result := False;
    ...
    while AOwner <> nil do
    begin
      if (AOwner is TcxCustomHintStyleController) and
        (AOwner = AHintStyleController) then
        Exit;
...
begin
  ...
  if (Value <> nil) and (not CheckHintStyleController(Value)) then
    Exit;
...
procedure TcxHintStyle.ControllerChangedNotification(AStyleController
  : TcxCustomHintStyleController);
begin
  Changed;
end;

procedure TcxHintStyle.ControllerFreeNotification(AHintStyleController
  : TcxCustomHintStyleController);
begin
  if (AHintStyleController <> nil) and
    (AHintStyleController = FHintStyleController) then
...
begin
  ...
  ARegion := CreateRoundRectRgn(R.Left, R.Top, R.Right, R.Bottom,
    Owner.RoundRadius, Owner.RoundRadius);
...
procedure TcxHintViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    ...
    begin
      ...
      DrawIconEx(ACanvas.Handle, R.Left, R.Top, Owner.Icon.Handle, IconSize,
        IconSize, 0, 0, DI_NORMAL);
    end;
  end;

  procedure InternalDrawHintText(ABounds: TRect; const AText: string;
    AFont: TFont; AColor: TColor; AFlags: Cardinal);
  begin
  ...
  begin
    ...
    begin
      ...
      if Owner.InternalUseRightToLeftAlignment then
        R := TdxRightToLeftLayoutConverter.ConvertRect(R, FHintWindowRect);
      InternalDrawHintText(R, Caption, CaptionFont,
        FPainter.ScreenTipGetTitleTextColor, DT_WORDBREAK or DT_NOPREFIX or
        DT_VCENTER);
    end;

    InternalDrawHintText(FHintBounds, Hint, HintFont,
      FPainter.ScreenTipGetDescriptionTextColor, DT_WORDBREAK or DT_NOPREFIX);
...
function TcxHintViewInfo.GetAutoCallOutPosition(const ARect: TRect)
  : TcxCallOutPosition;
var
  ACursorPos: TPoint;
  AMonitorRect: TRect;
...
procedure TcxHintViewInfo.AdjustHintPositionByPoint(const APoint: TPoint;
  var ARect: TRect);
var
  AMonitorRect: TRect;
...
begin
  ...
  ACalloutOffset := cxPoint(CallOutSize + CalculatedCallOutOffset.X,
    CallOutSize + CalculatedCallOutOffset.Y);
  case CalculatedCallOutPos of
    cxbpLeftBottom:
      begin
...
begin
  ...
  case CalculatedCallOutPos of
    cxbpLeftBottom:
      begin
        FCallOutTops[0] := Point(HintWindowRect.Left, HintWindowRect.Bottom -
          CallOutSize);
        FCallOutTops[1] := Point(HintWindowRect.Left, HintWindowRect.Bottom -
          CallOutSize * 2);
        FCallOutTops[2] := Point(HintWindowRect.Left - CallOutSize,
          HintWindowRect.Bottom - CallOutSize);
        cxPointsOffset(FCallOutTops, 0, -FCalculatedCallOutOffset.Y);
      end;
    cxbpLeftTop:
      begin
        FCallOutTops[0] := Point(HintWindowRect.Left,
          HintWindowRect.Top + CallOutSize);
        FCallOutTops[1] := Point(HintWindowRect.Left,
          HintWindowRect.Top + CallOutSize * 2);
        FCallOutTops[2] := Point(HintWindowRect.Left - CallOutSize,
          HintWindowRect.Top + CallOutSize);
        cxPointsOffset(FCallOutTops, 0, FCalculatedCallOutOffset.Y);
      end;
    cxbpTopRight:
      begin
        FCallOutTops[0] := Point(HintWindowRect.Right - CallOutSize,
          HintWindowRect.Top);
        FCallOutTops[1] := Point(HintWindowRect.Right - CallOutSize * 2,
          HintWindowRect.Top);
        FCallOutTops[2] := Point(HintWindowRect.Right - CallOutSize,
          HintWindowRect.Top - CallOutSize);
        cxPointsOffset(FCallOutTops, -FCalculatedCallOutOffset.X, 0);
      end;
    cxbpTopLeft:
      begin
        FCallOutTops[0] := Point(HintWindowRect.Left + CallOutSize,
          HintWindowRect.Top);
        FCallOutTops[1] := Point(HintWindowRect.Left + CallOutSize * 2,
          HintWindowRect.Top);
        FCallOutTops[2] := Point(HintWindowRect.Left + CallOutSize,
          HintWindowRect.Top - CallOutSize);
        cxPointsOffset(FCallOutTops, FCalculatedCallOutOffset.X, 0);
    ...
    cxbpRightBottom:
      begin
        FCallOutTops[0] := Point(HintWindowRect.Right - 1,
          HintWindowRect.Bottom - CallOutSize);
        FCallOutTops[1] := Point(HintWindowRect.Right - 1,
          HintWindowRect.Bottom - CallOutSize * 2);
        FCallOutTops[2] := Point(HintWindowRect.Right + CallOutSize,
          HintWindowRect.Bottom - CallOutSize);
        cxPointsOffset(FCallOutTops, 0, -FCalculatedCallOutOffset.Y);
      end;
...
begin
  ...
  if CalculatedCallOutPos in [cxbpTopLeft, cxbpTopRight, cxbpBottomLeft,
    cxbpBottomRight] then
  begin
    AStraightSize := Max(AWidth - 2 * RoundRadius, CallOutSize);
    if AStraightSize < 3 * CallOutSize then
      FCalculatedCallOutOffset.X := (AStraightSize - 3 * CallOutSize) div 2 +
        RoundRadius;
  ...
  begin
    ...
    if AStraightSize < 3 * CallOutSize then
      FCalculatedCallOutOffset.Y := (AStraightSize - 3 * CallOutSize) div 2 +
        RoundRadius;
...
procedure TcxHintViewInfo.GetTextBounds(var ARect: TRect; const AText: string;
  AFont: TFont);
begin
...
function TcxHintViewInfo.GetMaxCaptionWidth(AIsCaption: Boolean = True)
  : Integer;
var
  ADec: Integer;
begin
...
procedure TcxHintWindow.CalculateController;
  ...
  begin
    ...
    Result := (AController <> nil) and Supports(AController.HintStyle, IcxHint,
      AHintData);
    if Result then
...
procedure TcxHintWindow.LoadHintStyleData(const AHintData: IcxHint);
  ...
  begin
    ...
    ATargetFont.Height := ScaleFactor.Apply(ATargetFont.Height,
...
procedure TcxHintWindow.ShowHint(X, Y: Integer; ACaption, AHint: string;
  AMaxWidth: Integer = 0);
var
  R: TRect;
begin
...
type
  TcxRealHintIconType = (IDIAPPLICATION, IDIINFORMATION, IDIWARNING, IDIERROR,
    IDIQUESTION, IDIWINLOGO);
const
  LI_LARGE = 1;
  LI_SMALL = 0;
  ...
  function InternalLoadIcon(AIconID: MakeIntResource;
    out AHandle: HICON): Boolean;
  begin
    Result := Succeeded(LoadIconMetric(0, MakeIntResourceW(AIconID),
      IconSize[FIconSize], AHandle));
  end;
...
begin
  ...
  begin
    ATargetDPI := dxGetMonitorDPI(cxRectCenter(ARect));
    if ATargetDPI <> ScaleFactor.TargetDPI then
    begin
      ScaleForPPI(ATargetDPI);
      ARect := cxRectSetOrigin(ViewInfo.Calculate(ViewInfo.FMaxWidth),
        ARect.TopLeft);
    end;
  end;
