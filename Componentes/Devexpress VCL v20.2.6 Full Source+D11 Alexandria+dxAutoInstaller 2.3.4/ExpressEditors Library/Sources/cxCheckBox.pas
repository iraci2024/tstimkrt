...
type
  ...
  TcxCheckBoxNullValueShowingStyle = (nssUnchecked, nssInactive,
    nssGrayedChecked);
  TcxCheckStatesValueFormatEx = (cvfCaptions, cvfIndices, cvfInteger,
    cvfStatesString, cvfCustom);
  ...
  protected
    function DoCompareItems(AItem1, AItem2: TcxInterfacedCollectionItem)
      : Integer; override;
    ...
    function GetCount: Integer;
  public
    property Items[AIndex: Integer]: TcxCaptionItem read GetItem
  ...
  protected
    ...
    function GetCheckControlFadingHelperClass
      : TcxCheckControlFadingHelperClass; virtual;
    ...
    class function GetContentOffset(AIsInplace, AIsCaptionVisible: Boolean;
      AItemAlignment: TLeftRight; AScaleFactor: TdxScaleFactor): TRect; virtual;
    class function GetTextGap(AIsInplace, ANativeStyle: Boolean;
      AScaleFactor: TdxScaleFactor): Integer; virtual;
    class function GetTextRect(const ABounds, ACheckBoxRect: TRect;
      AItemAlignment: TAlignment; AIsCaptionVisible, AIsInplace,
      ANativeStyle: Boolean; AFont: TFont; AScaleFactor: TdxScaleFactor): TRect;
  public
  ...
  protected
    ...
    function CalculateCheckBoxRect(AViewInfo: TcxCustomCheckBoxViewInfo)
      : TRect; virtual;
    function CalculateCheckBoxState(AViewInfo: TcxCustomCheckBoxViewInfo;
      const P: TPoint; AButton: TcxMouseButton; AShift: TShiftState)
      : TcxEditCheckState; virtual;
    procedure CalculateFocusRect(AViewInfo: TcxCustomCheckBoxViewInfo); virtual;
    function CalculateTextRect(AViewInfo: TcxCustomCheckBoxViewInfo)
      : TRect; virtual;

    property Alignment: TAlignment read FAlignment;
    property BiDiModeDependedAlignment: TAlignment
      read GetBiDiModeDependedAlignment;
  public
    ...
    function GetTextGap(AIsInplace, ANativeStyle: Boolean;
      AScaleFactor: TdxScaleFactor): Integer; virtual;
    ...
    function GetEditContentSize(ACanvas: TcxCanvas; const AEditValue: Variant;
      ...
      AErrorData: TcxEditValidateInfo = nil): TSize; override;
  ...
  private
    ...
    procedure SetStateValues(const AValueChecked, AValueGrayed, AValueUnchecked
      : TcxEditValue);
  ...
  public
    ...
    // obsolete
    property DisplayChecked: WideString index cbsChecked
      read GetDisplayCheckState write SetDisplayCheckState
      stored IsDisplayCheckStateStored;
    property DisplayGrayed: WideString index cbsGrayed read GetDisplayCheckState
      write SetDisplayCheckState stored IsDisplayCheckStateStored;
    property DisplayUnchecked: WideString index cbsUnchecked
      read GetDisplayCheckState write SetDisplayCheckState
      stored IsDisplayCheckStateStored;
    property FullFocusRect: Boolean read FFullFocusRect write SetFullFocusRect
      default False;
    ...
    property MultiLine: Boolean read FMultiLine write SetMultiLine
      default False;
    property NullStyle: TcxCheckBoxNullValueShowingStyle read FNullStyle
      write SetNullStyle default nssGrayedChecked;
    ...
    property ValueUnchecked: TcxEditValue read FValueUnchecked
      write SetValueUnchecked stored IsValueUncheckedStored;
  end;
  ...
  public
    ...
    //
    property ActiveProperties: TcxCustomCheckBoxProperties
  ...
  public
    ...
    class procedure GetFilterValue(AEdit: TcxCustomEdit;
      AEditProperties: TcxCustomEditProperties; var V: Variant;
      var S: TCaption); override;
...
function CalculateCheckStates(const AValue: TcxEditValue; AItems: IcxCheckItems;
...
procedure DrawEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ...
  AGlyph: TdxSmartGlyph; AGlyphCount: Integer;
  ABorderStyle: TcxEditCheckBoxBorderStyle; ANativeStyle: Boolean;
  ABorderColor: TColor; ABackgroundColor: TColor; ADrawBackground, AIsDesigning,
  AFocused, ASupportGrayed: Boolean; APainter: TcxCustomLookAndFeelPainter;
...
procedure DrawScaledEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ...
  AGrayedShowingStyle: TcxCheckBoxNullValueShowingStyle =
  nssGrayedChecked); overload;
...
function GetEditCheckBorderOffset(ACheckBorderStyle: TcxContainerBorderStyle;
  ANativeStyle, AHasGlyph: Boolean; APainter: TcxCustomLookAndFeelPainter)
  : Integer; overload;
...
function GetEditCheckSize(ACanvas: TcxCanvas; ANativeStyle: Boolean;
  AGlyph: TdxSmartGlyph; AGlyphCount: Integer;
  APainter: TcxCustomLookAndFeelPainter): TSize; overload;
...
procedure DrawCheckBoxText(ACanvas: TcxCanvas; AText: string; AFont: TFont;
  ATextColor: TColor; ATextRect: TRect; ADrawTextFlags: Integer;
  AEnabled: Boolean; APaintOnGlass: Boolean = False);
...
function CalculateCheckStates(const AValue: TcxEditValue; AItems: IcxCheckItems;
  ...
  function GetNumber(var ANumber, AIndex: Integer; const S: string): Boolean;
    ...
    begin
      ...
      repeat
        ...
        if AIndex + ACaptionLength - 1 > AValueLength then
          Exit;
        ACaptions.AddObject(Copy(S, AIndex, ACaptionLength), Pointer(AChecked));
  ...
  begin
    ...
    repeat
      ...
      if not GetNumber(AItemIndex, AIndex, S) or (AItemIndex >= AItems.Count)
      then
        Exit;
      if AChecked then
...
begin
  if APaintOnGlass then
    dxDrawTextOnGlass(ACanvas.Handle, AText, AFont, ATextRect, ATextColor,
      cxFlagsToDTFlags(ADrawTextFlags), 0, True)
  else
  begin
    ACanvas.Font := AFont;
...
function GetCheckNativeState(AState: TcxCheckBoxState;
  ACheckState: TcxEditCheckState): Integer;
const
  ANativeCheckStateMap: array [TcxCheckBoxState, TcxEditCheckState]
    of Integer = ((CBS_UNCHECKEDNORMAL, CBS_UNCHECKEDHOT, CBS_UNCHECKEDPRESSED,
    CBS_UNCHECKEDDISABLED), (CBS_CHECKEDNORMAL, CBS_CHECKEDHOT,
    CBS_CHECKEDPRESSED, CBS_CHECKEDDISABLED), (CBS_MIXEDNORMAL, CBS_MIXEDHOT,
    CBS_MIXEDPRESSED, CBS_MIXEDDISABLED));
begin
  Result := ANativeCheckStateMap[AState, ACheckState];
end;

procedure DrawCustomCheckBox(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomCheckBoxViewInfo; AScaleFactor: TdxScaleFactor); overload;

  function IsCheckTransparent: Boolean;
  begin
    Result := AViewInfo.IsBackgroundTransparent and
      (AViewInfo.UseSkins or AViewInfo.HasGlyph or AViewInfo.NativeStyle and
      IsThemeBackgroundPartiallyTransparent(OpenTheme(totButton), BP_CHECKBOX,
      GetCheckNativeState(AViewInfo.State, AViewInfo.CheckBoxState)));
  end;
...
begin
  ...
  InflateRect(ACheckRect, -AViewInfo.CheckBorderOffset,
    -AViewInfo.CheckBorderOffset);
  AViewInfo.DrawEditBackground(ACanvas, AViewInfo.Bounds, cxNullRect,
    IsCheckTransparent);
  ...
  begin
    if not AViewInfo.FadingHelper.DrawImage(ACanvas.Handle, ACheckRect) then
      DrawScaledEditCheck(ACanvas, AViewInfo.CheckBoxRect,
        AViewInfo.CheckBoxState, AViewInfo, AScaleFactor);

    if AViewInfo.Alignment <> taCenter then
      DrawCheckBoxText(ACanvas, AViewInfo.Text, AViewInfo.Font,
        AViewInfo.TextColor, AViewInfo.TextRect, AViewInfo.DrawTextFlags,
        AViewInfo.IsTextEnabled, AViewInfo.PaintOnGlass);
  end;
...
begin
  DrawScaledEditCheck(ACanvas, ACheckRect, AViewInfo.State, ACheckState,
    ...
    AViewInfo.CheckBoxBorderStyle, AViewInfo.NativeStyle, AViewInfo.BorderColor,
    AViewInfo.BackgroundColor, not AViewInfo.IsBackgroundTransparent,
    AViewInfo.IsDesigning, AViewInfo.Focused, True, GetPainter, AScaleFactor,
...
procedure DrawEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ACheckState: TcxEditCheckState; AViewInfo: TcxCustomCheckBoxViewInfo);
begin
  DrawScaledEditCheck(ACanvas, ACheckRect, ACheckState, AViewInfo,
    dxSystemScaleFactor);
end;
...
procedure DrawScaledEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ...
  begin
    ...
    try
      ...
      begin
        ABitmap.SetSize(AScaleFactor.Apply(AGlyphWidth),
          AScaleFactor.Apply(AGlyph.SourceHeight));
      ...
      AGlyph.StretchDraw(ABitmap.Canvas.Handle, ABitmap.ClientRect,
        cxRectBounds((AGlyph.Width div AGlyphCount) * AGlyphIndex, 0,
        (AGlyph.Width div AGlyphCount), AGlyph.Height));
      ...
      cxDrawImage(ACanvas, ACheckRect, ABitmap, nil, -1, ifmStretch,
        EnabledImageDrawModeMap[ACheckState <> ecsDisabled], True, nil,
        AScaleFactor);
    finally
      ABitmap.Free;
    end;
  ...
  begin
    ...
    begin
      cxLookAndFeelPaintersManager.GetPainter(lfsNative)
        .DrawScaledCheck(ACanvas, ACheckRect,
        CheckStateToButtonState[ACheckState], AState, clDefault, AScaleFactor);
      Exit;
    end;

    R := ACheckRect;
    ACheckBorderOffset := GetEditCheckBorderOffset(ABorderStyle, False, False,
      APainter);
      ...
      case ABorderStyle of
        ...
        ebsUltraFlat, ebsOffice11:
          begin
            ...
            else if (ACheckState in [ecsHot, ecsPressed]) or AIsDesigning and
              (ACheckState <> ecsDisabled) or (ACheckState = ecsNormal) and AFocused
            then
  ...
  begin
    ...
    try
      ...
      cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat)
        .DrawScaledCheck(ACanvas, R, CheckStateToButtonState[ACheckState],
        AState, AColor, AScaleFactor);
    finally
  ...
  const
    AColors: array [TcxEditCheckState] of TColor = (clWindow, clWindow,
      clBtnFace, clBtnFace);
  begin
    if ABorderStyle in [ebsUltraFlat, ebsOffice11] then
      case ACheckState of
        ecsHot, ecsPressed:
          Result := GetEditButtonHighlightColor(ACheckState = ecsPressed,
            ABorderStyle = ebsOffice11);
        ecsNormal:
    ...
    else if (AState = cbsGrayed) and (AGrayedShowingStyle = nssInactive) then
      Result := clBtnFace
    else
  ...
  begin
    ...
    ACanvas.Brush.Style := bsClear;
    if (AState = cbsUnchecked) or (AState = cbsGrayed) and
      (AGrayedShowingStyle <> nssGrayedChecked) then
      Exit;
  ...
  const
    ...
    AGrayedShowingStyleMap: array [TcxCheckBoxNullValueShowingStyle]
      of Integer = (0, DFCS_INACTIVE, DFCS_CHECKED);
  var
  ...
  begin
    ...
    begin
      ...
      if (AState = cbsUnchecked) or
        ((AState = cbsGrayed) and (AGrayedShowingStyle <> nssGrayedChecked))
      then
    ...
    begin
      ...
      case AState of
        cbsGrayed:
          AFlags := DFCS_BUTTON3STATE or AGrayedShowingStyleMap
            [AGrayedShowingStyle];
        cbsChecked:
          AFlags := DFCS_CHECKED;
      end;
    ...
    InflateRect(AContentRect, cxEditMaxCheckBoxBorderWidth,
      cxEditMaxCheckBoxBorderWidth);

    DrawFrameControl(ACanvas.Handle, AContentRect, DFC_BUTTON,
      DFCS_BUTTONCHECK or AFlags or ABorder3DStyleMap[ABorderStyle = ebs3D]);
...
begin
  ...
  begin
    ...
    begin
      if ADrawBackground then
        ACanvas.FillRect(ACheckRect, ABackgroundColor);
      APainter.DrawScaledCheckButton(ACanvas, ACheckRect,
        CheckStateToButtonState[ACheckState], AState, AScaleFactor);
    end
    else
    begin
      DrawCheckBoxBorder;
...
begin
  case AIndex of
    ...
    2:
      Result := 'Grayed';
    3:
      Result := 'Unchecked Pressed';
    4:
      Result := 'Checked Pressed';
    5:
      Result := 'Grayed Pressed';
  else
    Result := 'Not Used';
  end;
end;
...
procedure DrawEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ...
  AGrayedShowingStyle: TcxCheckBoxNullValueShowingStyle = nssGrayedChecked);
begin
  DrawScaledEditCheck(ACanvas, ACheckRect, AState, ACheckState, AGlyph,
    AGlyphCount, ABorderStyle, ANativeStyle, ABorderColor, ABackgroundColor,
    ADrawBackground, AIsDesigning, AFocused, ASupportGrayed, APainter,
    dxSystemScaleFactor, AGrayedShowingStyle);
end;

function GetEditCheckBorderOffset(ACheckBorderStyle: TcxContainerBorderStyle;
  ANativeStyle, AHasGlyph: Boolean;
  APainter: TcxCustomLookAndFeelPainter): Integer;
begin
  if ANativeStyle or AHasGlyph or (APainter <> nil) then
    Result := 0
  else
    Result := cxContainerMaxBorderWidth - GetContainerBorderWidth
      (ACheckBorderStyle);
end;
...
function GetEditCheckBorderOffset(ACheckBorderStyle: TcxEditBorderStyle;
  ...
  : Integer; overload;
begin
  Result := GetEditCheckBorderOffset(TcxContainerBorderStyle(ACheckBorderStyle),
    ANativeStyle, AHasGlyph, APainter);
end;
...
begin
  ...
  begin
    ...
    if not ASupportGrayed and (AGlyphCount mod 3 <> 0) and
      (AGlyphCount mod 2 = 0) then
      Dec(AStateCount);
    case AState of
      cbsUnchecked:
        Result := 0;
...
function GetScaledEditCheckSize(ACanvas: TcxCanvas; ANativeStyle: Boolean;
  AGlyph: TdxSmartGlyph; AGlyphCount: Integer;
  APainter: TcxCustomLookAndFeelPainter; AScaleFactor: TdxScaleFactor): TSize;
var
  AIntf: IdxSourceSize;
begin
  if IsGlyphValid(AGlyph, AGlyphCount) then
  begin
    ...
    Result := AScaleFactor.Apply(Result, dxGetImageSourceDPI(AGlyph),
      dxDefaultDPI);
  end
  else
  begin
...
function GetEditCheckSize(ACanvas: TcxCanvas; ANativeStyle: Boolean;
  AGlyph: TdxSmartGlyph; AGlyphCount: Integer;
  APainter: TcxCustomLookAndFeelPainter): TSize;
begin
  Result := GetScaledEditCheckSize(ACanvas, ANativeStyle, AGlyph, AGlyphCount,
    APainter, dxSystemScaleFactor);
end;
...
function TcxCaptionItems.DoCompareItems(AItem1,
  AItem2: TcxInterfacedCollectionItem): Integer;
begin
  Result := AnsiCompareText(TcxCaptionItem(AItem1).Caption,
    TcxCaptionItem(AItem2).Caption);
end;
...
function TcxCustomCheckBoxViewInfo.GetUpdateRegion
...
begin
  ...
  Result := Result or (ACheckBoxViewInfo <> nil) and
    ((CheckBoxState <> ACheckBoxViewInfo.CheckBoxState) or
    (State <> ACheckBoxViewInfo.State));
  if (ACheckBoxViewInfo = nil) or
    (CheckBoxState <> ACheckBoxViewInfo.CheckBoxState) or
    (State <> ACheckBoxViewInfo.State) then
    cxRedrawWindow(AControl.Handle, cxRectOffset(CheckBoxRect, Left,
      Top), False);
end;

function TcxCustomCheckBoxViewInfo.IsTextEnabled: Boolean;
begin
  Result := IsContainerInnerControl or Enabled or IsTextColorAssigned or
    NativeStyle;
end;
...
begin
  Result := GetTextRect(ClientRect, CheckBoxRect, Alignment, IsCaptionVisible,
...
class function TcxCustomCheckBoxViewInfo.GetContentOffset(AIsInplace,
  AIsCaptionVisible: Boolean; AItemAlignment: TLeftRight;
  AScaleFactor: TdxScaleFactor): TRect;
begin
  if AIsInplace then
    Result := Rect(1, 1, 1, 1)
  else if not AIsCaptionVisible then
    Result := Rect(0, 2, 0, 2)
  else
...
class function TcxCustomCheckBoxViewInfo.GetTextGap(AIsInplace,
  ANativeStyle: Boolean; AScaleFactor: TdxScaleFactor): Integer;
const
...
class function TcxCustomCheckBoxViewInfo.GetTextRect(const ABounds,
  ACheckBoxRect: TRect; AItemAlignment: TAlignment;
  AIsCaptionVisible, AIsInplace, ANativeStyle: Boolean; AFont: TFont;
  AScaleFactor: TdxScaleFactor): TRect;
var
  AOffset: TRect;
begin
  Result := ABounds;
  AOffset := GetContentOffset(AIsInplace, AIsCaptionVisible, AItemAlignment,
    AScaleFactor);
  InflateRect(Result, -AOffset.Left, 0);

  if IsOldStyleSupported and not AIsInplace and not ANativeStyle and
    not Odd(cxRectHeight(Result) - TdxTextMeasurer.TextLineHeight(AFont)) then
    OffsetRect(Result, 0, -1);

  if cxRectWidth(ACheckBoxRect) < cxRectWidth(ABounds) then
    case AItemAlignment of
      taLeftJustify:
        Result.Left := ACheckBoxRect.Right + GetTextGap(AIsInplace,
          ANativeStyle, AScaleFactor);
      taRightJustify:
        Result.Right := ACheckBoxRect.Left - GetTextGap(AIsInplace,
          ANativeStyle, AScaleFactor);
    end
  else
...
function TcxCustomCheckBoxViewInfo.GetCheckControlFadingHelperClass
  : TcxCheckControlFadingHelperClass;
begin
...
procedure TcxCustomCheckBoxViewData.Calculate(ACanvas: TcxCanvas;
  ...
  function GetCheckBoxBorderStyle(AEditHotState: TcxContainerHotState)
    : TcxEditBorderStyle;
  const
    ABorderStyles: array [TcxContainerHotState, TcxEditBorderStyle]
      of TcxEditBorderStyle = ((ebsNone, ebsSingle, ebsThick, ebsFlat, ebs3D,
      ebsUltraFlat, ebsOffice11), (ebsNone, ebsSingle, ebsThick, ebsFlat, ebs3D,
      ebsUltraFlat, ebsOffice11), (ebsFlat, ebsThick, ebsThick, ebs3D, ebs3D,
      ebsUltraFlat, ebsOffice11));
  begin
...
begin
  ...
  ACheckBoxViewInfo.TextRect := cxRectContent(AViewInfo.ClientRect,
    EditContentParams.Offsets);
  ...
  ACheckBoxViewInfo.CheckBoxBorderStyle :=
    GetCheckBoxBorderStyle(AViewInfo.HotState);
  ...
  ACheckBoxViewInfo.HasGlyph := IsGlyphValid(Properties.Glyph,
    Properties.GlyphCount);
  ACheckBoxViewInfo.CheckBorderOffset := GetEditCheckBorderOffset
    (ACheckBoxViewInfo.CheckBoxBorderStyle, NativeStyle,
  ...
  ACheckBoxViewInfo.CheckBoxSize := GetScaledEditCheckSize(ACanvas, NativeStyle,
    Properties.Glyph, Properties.GlyphCount, AViewInfo.Painter, ScaleFactor);
  ...
  ACheckBoxViewInfo.CheckBoxState := CalculateCheckBoxState(ACheckBoxViewInfo,
    P, Button, Shift);
  ...
  begin
    if (ACheckBoxViewInfo.CheckBoxState = ecsHot) or
      (IsDesigning and (ACheckBoxViewInfo.CheckBoxState <> ecsDisabled)) then
...
procedure TcxCustomCheckBoxViewData.EditValueToDrawValue(const AEditValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
begin
  if PreviewMode then
    TcxCustomCheckBoxViewInfo(AViewInfo).State := cbsChecked
  else
    TcxCustomCheckBoxViewInfo(AViewInfo).State :=
      Properties.GetState(AEditValue);
...
function TcxCustomCheckBoxViewData.GetTextGap(AIsInplace, ANativeStyle: Boolean;
  AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := TcxCustomCheckBoxViewInfo.GetTextGap(AIsInplace, ANativeStyle,
    AScaleFactor);
end;

function TcxCustomCheckBoxViewData.GetContentOffset(AIsCaptionVisible
  : Boolean): TRect;
begin
  Result := TcxCustomCheckBoxViewInfo.GetContentOffset(IsInplace,
    AIsCaptionVisible, BiDiModeDependedAlignment, ScaleFactor);
end;

function TcxCustomCheckBoxViewData.InternalGetEditConstantPartSize
...
begin
  ...
  ASize1 := GetScaledEditCheckSize(ACanvas, NativeStyle, Properties.Glyph,
    Properties.GlyphCount, Style.LookAndFeel.Painter, ScaleFactor);
  AOffset := GetContentOffset((AViewInfo as TcxCustomCheckBoxViewInfo)
    .IsCaptionVisible);
...
function TcxCustomCheckBoxViewData.GetEditContentSize(ACanvas: TcxCanvas;
  const AEditValue: Variant; const AEditSizeProperties: TcxEditSizeProperties;
...
begin
  ...
  try
    ...
    if AText <> '' then
    begin
      ARect := Rect(0, 0, Max(AEditSizeProperties.Width - GetTextGap(IsInplace,
        NativeStyle, ScaleFactor), 1), 1);
      ACanvas.Font := Style.GetVisibleFont;
      cxDrawText(ACanvas.Handle, AText, ARect, DT_CALCRECT or
        cxFlagsToDTFlags(GetCalculateTextFlags or cxDontBreakChars));
...
function TcxCustomCheckBoxViewData.CalculateCheckBoxRect
  (AViewInfo: TcxCustomCheckBoxViewInfo): TRect;
var
...
begin
  ...
  Result := cxRectCenter(AAvailableArea,
    Min(ACheckSize.cx, cxRectWidth(AAvailableArea)),
    Min(ACheckSize.cy, cxRectHeight(AAvailableArea)));
...
begin
  if not Enabled then
    Result := ecsDisabled
  else if IsCheckPressed then
    Result := ecsPressed
  else if not IsDesigning and PtInRect(AViewInfo.BorderRect, P) then
  begin
    if ([ssLeft, ssRight, ssMiddle] * AShift) = [] then
      Result := ecsHot
    else if (ssLeft in AShift) and
      ((AButton = cxmbLeft) or (AViewInfo.CheckBoxState = ecsPressed)) then
      Result := ecsPressed
    else
      Result := ecsNormal
  end
...
procedure TcxCustomCheckBoxViewData.CalculateFocusRect
  (AViewInfo: TcxCustomCheckBoxViewInfo);

  procedure CheckFocusRectBounds;
  begin
    AViewInfo.FocusRect.Left := Max(AViewInfo.FocusRect.Left,
      AViewInfo.ClientRect.Left);
    AViewInfo.FocusRect.Right := Min(AViewInfo.FocusRect.Right,
      AViewInfo.ClientRect.Right);
    if AViewInfo.IsCaptionVisible then
    begin
      if AViewInfo.Alignment = taLeftJustify then
        AViewInfo.FocusRect.Left := Max(AViewInfo.FocusRect.Left,
          AViewInfo.TextRect.Left - 1)
      else
        AViewInfo.FocusRect.Right := Min(AViewInfo.FocusRect.Right,
          AViewInfo.TextRect.Right + 1);
    end;
    AViewInfo.FocusRect.Top := Max(AViewInfo.FocusRect.Top,
      AViewInfo.ClientRect.Top);
    AViewInfo.FocusRect.Bottom := Min(AViewInfo.FocusRect.Bottom,
      AViewInfo.ClientRect.Bottom);
  end;

begin
  if (Focused and ((not IsInplace or Properties.IsEmbeddedEdit) and
    (AViewInfo.Alignment <> taCenter) or IsInplace and
    (AViewInfo.Alignment = taCenter) and (epoShowFocusRectWhenInplace
    in PaintOptions))) then
  begin
    if AViewInfo.Alignment = taCenter then
    begin
    ...
    else if Length(AViewInfo.Text) <> 0 then
    begin
      ...
      begin
        ...
        cxScreenCanvas.TextExtent(AViewInfo.Text, AViewInfo.FocusRect,
          AViewInfo.DrawTextFlags);
...
function TcxCustomCheckBoxViewData.CalculateTextRect
  (AViewInfo: TcxCustomCheckBoxViewInfo): TRect;
begin
...
const
  AHorzAlignmentFlags: array [TcxEditHorzAlignment] of Integer = (cxAlignLeft,
    cxAlignLeft, cxAlignHCenter);
begin
  Result := cxShowPrefix;
  if UseRightToLeftAlignment then
    Result := Result or TdxRightToLeftLayoutConverter.ConvertcxDrawTextAlignment
      (AHorzAlignmentFlags[Properties.InternalAlignment.Horz]);
  if ADraw then
...
function TcxCustomCheckBoxProperties.CompareDisplayValues(const AEditValue1,
...
function TcxCustomCheckBoxProperties.GetDisplayText(const AEditValue
...
class function TcxCustomCheckBoxProperties.GetStyleClass
...
function TcxCustomCheckBoxProperties.GetSupportedOperations
...
class function TcxCustomCheckBoxProperties.GetViewInfoClass
...
function TcxCustomCheckBoxProperties.IsEditValueValid(var EditValue
...
begin
  ...
  else if VarEqualsExact(AEditValue, FValueUnchecked) then
...
procedure TcxCustomCheckBoxProperties.DoAssign(AProperties
...
class function TcxCustomCheckBoxProperties.GetViewDataClass
...
function TcxCustomCheckBoxProperties.CheckValue(const AValue
  : TcxEditValue): Boolean;
begin
  Result := not(InternalVarEqualsExact(AValue, FValueChecked) or
    InternalVarEqualsExact(AValue, FValueGrayed) or
    InternalVarEqualsExact(AValue, FValueUnchecked));
end;

function TcxCustomCheckBoxProperties.GetState(const AEditValue: TcxEditValue)
  : TcxCheckBoxState;
var
...
function TcxCustomCheckBoxProperties.GetDefaultDisplayCheckState
  (AState: TcxCheckBoxState): string;
begin
  case AState of
    cbsChecked:
...
function TcxCustomCheckBoxProperties.GetDisplayCheckState(const Index: Integer)
  : WideString;
var
...
function TcxCustomCheckBoxProperties.IsDisplayCheckStateStored
...
begin
  Result := (GetOwnerComponent(Self) <> nil) and
    (csLoading in GetOwnerComponent(Self).ComponentState);
end;
...
begin
  ACheckState := TcxCheckBoxState(Index);
  if not InternalCompareString(Value, FDisplayCheckState[ACheckState], True)
  then
  begin
    FDisplayCheckState[ACheckState] := Value;
    FIsDisplayCheckStateAssigned[ACheckState] :=
      not InternalCompareString(Value,
...
procedure TcxCustomCheckBoxProperties.SetNullStyle
  (Value: TcxCheckBoxNullValueShowingStyle);
begin
...
procedure TcxCustomCheckBoxProperties.SetStateValues(const AValueChecked,
...
procedure TcxCustomCheckBoxProperties.SetUseAlignmentWhenInplace
  (Value: Boolean);
begin
...
procedure TcxCustomCheckBoxProperties.SetValueChecked
...
procedure TcxCustomCheckBoxProperties.SetValueUnchecked
  (const Value: TcxEditValue);
begin
  if IsLoading or CheckValue(Value) and not VarIsNull(Value) then
  begin
...
class function TcxCustomCheckBox.GetPropertiesClass
...
procedure TcxCustomCheckBox.ActionChange(Sender: TObject;
  CheckDefaults: Boolean);
begin
  inherited ActionChange(Sender, CheckDefaults);
  if (IsLoading or FIsLoaded) and FIsLoadingStateAssigned or
    not(Sender is TCustomAction) then
    Exit;
  if not CheckDefaults or not Checked then
    TcxCheckBoxActionLink(ActionLink).InternalSetChecked
      (TCustomAction(Sender).Checked);
end;
...
function TcxCustomCheckBox.GetDefaultSkinnedTextColor
...
begin
  ...
  if IsInplace and (Key = dxSpace) and
    (ViewInfo.CheckBoxState in [ecsNormal, ecsHot]) then
  begin
    Toggle;
    Key := #0;
  end;
...
begin
  if (ViewInfo.CheckBoxLastState = ecsPressed) and
    (ViewInfo.CheckBoxState = ecsHot) and (IsInplace or Focused) then
    Toggle;
end;

procedure TcxCustomCheckBox.PopulateSizeProperties(var AEditSizeProperties
...
function TcxCustomCheckBox.InternalGetNotPublishedStyleValues
  : TcxEditStyleValues;
begin
...
class function TcxFilterCheckBoxHelper.GetFilterDataType(AValueTypeClass
...
constructor TcxCustomCheckControlFadingHelper.Create
  (AViewInfo: TcxCustomCheckBoxViewInfo);
begin
  inherited Create;
  FViewInfo := AViewInfo;
...
begin
  ...
  begin
    CheckStartFading(CheckStateToButtonState[State],
...
function TcxCustomCheckControlFadingHelper.GetEditViewInfo
...
procedure TcxCheckBoxFadingHelper.GetFadingImages(out AFadeOutImage,
  ...
  begin
    ...
    Result.Canvas.Lock;
    try
      DrawScaledEditCheck(Result.cxCanvas, Result.ClientRect, AState, ViewInfo,
        ViewInfo.ScaleFactor);
...
GetRegisteredEditProperties.Register(TcxCheckBoxProperties,
  scxSEditRepositoryCheckBoxItem);
FilterEditsController.Register(TcxCheckBoxProperties, TcxFilterCheckBoxHelper);

finalization

FilterEditsController.Unregister(TcxCheckBoxProperties,
  TcxFilterCheckBoxHelper);
