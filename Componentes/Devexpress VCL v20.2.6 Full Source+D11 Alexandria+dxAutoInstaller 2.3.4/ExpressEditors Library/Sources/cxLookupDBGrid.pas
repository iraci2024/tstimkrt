...
type
  ...
  TcxLookupDBGridDefaultValuesProvider = class
  ...
  protected
    ...
    property DataController: TcxLookupGridDBDataController
  ...
  public
    ...
    function Equals(Obj: TObject): Boolean; override;
    //
    property Items[Index: Integer]: TcxLookupDBGridColumn read GetColumn
  ...
  public
    ...
    property DataController: TcxLookupGridDBDataController
      read GetDataController write SetDataController; // for internal use
    property Font;
...
function TcxLookupDBGridDefaultValuesProvider.IsDisplayFormatDefined
...
function TcxLookupDBGridColumn.DefaultWidth: Integer;
  ...
  begin
    Result := 0;
    for I := Max(0, DataController.FocusedRecordIndex - 100)
      to Min(DataController.RecordCount - 1, Index + 100) do
      Result := Max(Result,
        ACanvas.TextWidth(DataController.GetDisplayText(I, Index)));
  end;
...
begin
  ...
  begin
    ...
    Result := Max(GetMaxTextWidthNearCurrentField(ACanvas),
      AField.DisplayWidth * ACanvas.TextWidth('0')) + 2 * cxTextOffset;
    if Grid.Options.ShowHeader then
    begin
      Result := Max(Result, Grid.Painter.LFPainterClass.ScaledHeaderWidth
        (ACanvas, cxBordersAll, Caption, Grid.ViewInfo.GetHeaderFont,
        TcxCustomLookupGridAccess(Grid).ScaleFactor));
    end;
  end;
...
begin
  Result := inherited Equals(Obj) and (Obj is TcxLookupDBGridColumn) and
    (FieldName = TcxLookupDBGridColumn(Obj).FieldName);
end;

function TcxLookupDBGridColumn.GetDefaultValuesProviderClass
...
begin
  TcxCustomDBEditDefaultValuesProvider(DefaultValuesProvider.GetInstance)
    .Field := Field;
end;
...
function TcxLookupDBGridColumns.ColumnByFieldName(const AFieldName: string)
  : TcxLookupDBGridColumn;
var
  I: Integer;
begin
...
var
  I: Integer;
begin
  Result := (Obj is TcxLookupDBGridColumns) and
    (TcxLookupDBGridColumns(Obj).Count = Count);
  if Result then
  begin
    for I := 0 to Count - 1 do
      Result := Result and Items[I].Equals(TcxLookupDBGridColumns(Obj)
        .Items[I]);
  end;
end;

function TcxLookupDBGridColumns.GetColumn(Index: Integer)
  : TcxLookupDBGridColumn;
begin
  Result := inherited Items[Index] as TcxLookupDBGridColumn;
end;

procedure TcxLookupDBGridColumns.SetColumn(Index: Integer;
  Value: TcxLookupDBGridColumn);
begin
...
procedure TcxCustomLookupDBGrid.CreateColumnsByFieldNames
...
function TcxCustomLookupDBGrid.GetDataControllerClass
...
begin
  ...
  begin
    SetScrollBarInfo(sbVertical, 0, (DataController.DataSetRecordCount - 1) +
      (ViewInfo.VisibleRowCount - 1), 1, ViewInfo.VisibleRowCount,
      DataController.RecNo - 1, True, True);
  end
  else
...
procedure TcxCustomLookupDBGrid.SetDataController
  (Value: TcxLookupGridDBDataController);
begin
