...
type
  ...
  TcxMRUFontNameAction = (mfaInvalidFontName, mfaNone, mfaMoved, mfaAdded,
    mfaDeleted);
  TcxFontPreviewType = (cxfpFontName, cxfpCustom, cxfpFullAlphabet);
  TcxDeleteMRUFontEvent = procedure(Sender: TObject;
    const DeletedMRUFontName: string) of object;
  ...
  protected
    ...
    function AddMRUFontName(const AFontName: TFontName)
      : TcxMRUFontNameItem; virtual;
    function InsertMRUFontName(Index: Integer; const AFontName: TFontName)
      : TcxMRUFontNameItem; virtual;
    procedure Move(CurIndex, NewIndex: Integer); virtual;
  public
    constructor Create(AOwner: TPersistent; ItemClass: TCollectionItemClass);
    destructor Destroy; override;
    property Items[Index: Integer]: TcxMRUFontNameItem read GetItems
      write SetItems; default;
    function FindFontName(const AFontName: TFontName)
      : TcxMRUFontNameItem; virtual;
  end;
  ...
  TcxFontButtonClickEvent = procedure(Sender: TObject;
    ButtonType: TcxFontButtonType) of Object;
  ...
  published
    ...
    property PreviewType: TcxFontPreviewType read FPreviewType
      write SetPreviewType default cxfpFontName;
    ...
    property OnButtonClick: TcxFontButtonClickEvent read FOnButtonClick
  ...
  public
    ...
    property ShowEndEllipsis: Boolean read FShowEndEllipsis
      write SetShowEndEllipsis default True;
    property Edges: TcxBorders read FEdges write SetEdges
      default [bLeft, bTop, bRight, bBottom];
    property FontStyle: TFontStyles read FFontStyle write SetFontStyle
      default [];
  ...
  protected
    ...
    function AddMRUFontName(const AFontName: TFontName)
      : TcxMRUFontNameAction; virtual;
    function DelMRUFontName(const AFontName: TFontName)
      : TcxMRUFontNameAction; virtual;
...
uses
  dxCoreGraphics, dxTypeHelpers, cxListBox;
...
begin
  Result := ((cxftTTF in AFontTypes) and
    (AFontType and TRUETYPE_FONTTYPE = TRUETYPE_FONTTYPE)) or
    ((cxftDevice in AFontTypes) and
    (AFontType and DEVICE_FONTTYPE = DEVICE_FONTTYPE)) or
    ((cxftRaster in AFontTypes) and
    (AFontType and RASTER_FONTTYPE = RASTER_FONTTYPE)) or
    ((cxftFixed in AFontTypes) and (ALogFont.lfPitchAndFamily and
    FIXED_PITCH = FIXED_PITCH)) or
...
function EnumFontsProc1(var ALogFont: TLogFont; var ATextMetric: TTextMetric;
  AFontType: DWORD; AData: LPARAM): Integer; stdcall;
begin
...
function GetFontGlyph(AIsTrueType: Boolean;
  ...
  begin
    ...
    begin
      ...
      if UseRightToLeftAlignment then
        FButtons[I].BoundsRect := TdxRightToLeftLayoutConverter.ConvertRect(R,
          ClientRect)
      else
...
begin
  ...
  begin
    DrawText(Canvas.Handle, PChar(Caption), Length(Caption), FTextRect,
      GetTextFlag(DT_CALCRECT or DT_NOPREFIX));
    if (cxRectHeight(FTextRect) <= cxRectHeight(ARect)) and
      (cxRectWidth(FTextRect) <= cxRectWidth(ARect)) then
      Canvas.Font.Size := Canvas.Font.Size + 1
    else if Canvas.Font.Size > 8 then
      Canvas.Font.Size := Canvas.Font.Size - 1;
  end;
end;
...
begin
  ...
  if Assigned(FontPreview.OnButtonClick) then
    FontPreview.OnButtonClick(Self,
      TcxFontButtonType((Sender as TComponent).Tag));
...
constructor TcxMRUFontNameItems.Create(AOwner: TPersistent;
...
procedure TcxMRUFontNameItems.SetItems(Index: Integer;
  const Value: TcxMRUFontNameItem);
begin
...
function TcxMRUFontNameItems.AddMRUFontName(const AFontName: TFontName)
  : TcxMRUFontNameItem;
begin
  Result := nil;
...
function TcxMRUFontNameItems.InsertMRUFontName(Index: Integer;
  const AFontName: TFontName): TcxMRUFontNameItem;
begin
  Result := nil;
  if (AFontName = '') or (FindFontName(AFontName) <> nil) then
    Exit;
...
function TcxMRUFontNameItems.FindFontName(const AFontName: TFontName)
  : TcxMRUFontNameItem;
var
  I: Integer;
begin
...
procedure TcxCustomFontNameComboBoxViewData.Calculate(ACanvas: TcxCanvas;
  ...
  begin
    Result.Y := (cxRectHeight(AClientRect) - GetFontGlyphSize(ScaleFactor)
      .cy) div 2;
    if IsInplace then
  ...
  begin
    AViewInfo.ImageRect := cxRectSetSize(AViewInfo.ClientRect,
      GetFontGlyphSize(ScaleFactor));
...
begin
  ...
  inherited Calculate(ACanvas, ABounds, P, Button, Shift, AViewInfo,
    AIsMouseEvent);
  if (ABounds.Right = cxMaxRectSize) or (ABounds.Bottom = cxMaxRectSize) then
    Exit;

  AEditViewInfo := TcxCustomFontNameComboBoxViewInfo(AViewInfo);
  AEditViewInfo.ShowFontTypeIcon := Properties.ShowFontTypeIcon;

  AEditViewInfo.TextRect.Left := IconBorderWidth +
    ScaleFactor.Apply(IconTextOffset);
  ...
  begin
    ...
    AEditViewInfo.TextRect.Left := AEditViewInfo.ImageRect.Right +
      AEditViewInfo.TextRect.Left;
    if UseRightToLeftAlignment then
    begin
      AEditViewInfo.ImageRect := TdxRightToLeftLayoutConverter.ConvertRect
        (AEditViewInfo.ImageRect, AViewInfo.ClientRect);
      AEditViewInfo.TextRect := TdxRightToLeftLayoutConverter.ConvertRect
        (AEditViewInfo.TextRect, AViewInfo.ClientRect);
    end;
  end;
...
begin
  ...
  if AViewInfoAccess.FCurrentIndex <> -1 then
    AViewInfoAccess.IsTrueTypeFont := cxftTTF in Properties.ItemTypes
      [AViewInfoAccess.FCurrentIndex];
end;
...
function TcxCustomFontNameComboBoxViewData.GetEditContentSize
...
function TcxCustomFontNameComboBoxViewData.GetProperties
  : TcxCustomFontNameComboBoxProperties;
begin
...
function TcxFontNameComboBoxListBox.GetItemHeight(AIndex: Integer = -1)
  : Integer;
begin
  ...
  begin
    ...
    if ActiveProperties.UseOwnFont then
      Inc(Result, 4)
    else if Result <= GetFontGlyphSize(ScaleFactor).cy then
...
procedure TcxFontNameComboBoxListBox.MouseUp(Button: TMouseButton;
...
procedure TcxFontNameComboBoxListBox.DrawItem(Index: Integer; Rect: TRect;
  ...
  begin
    ...
    if (ftiShowInList in ActiveProperties.ShowFontTypeIcon) and (AGlyph <> nil)
    then
    begin
      Inc(ATextRect.Left, IconBorderWidth);
      AGlyphSize := GetFontGlyphSize(ActiveProperties.ScaleFactor);
      AGlyphRect := cxRectCenterVertically(cxRectSetWidth(ATextRect,
        AGlyphSize.cx), AGlyphSize.cy);
      ATextRect.Left := AGlyphRect.Right + IconBorderWidth +
        ScaleFactor.Apply(IconTextOffset);
      if UseRightToLeftAlignment then
      begin
        AGlyphRect := TdxRightToLeftLayoutConverter.ConvertRect
          (AGlyphRect, Rect);
        ATextRect := TdxRightToLeftLayoutConverter.ConvertRect(ATextRect, Rect);
      end;
      cxDrawImage(Canvas, AGlyphRect, AGlyph, nil, -1, ifmNormal, idmNormal,
        True, nil, ActiveProperties.ScaleFactor);
    end
    else
  ...
  begin
    ...
    begin
      ...
      Inc(ATextRect.Left, Canvas.TextWidth(AText) +
        ItemSymbolFontExampleOffset);
      Canvas.Font.Name := AText;
      Canvas.Font.Charset := SYMBOL_CHARSET;
      AFlags := AFlags and not DT_END_ELLIPSIS;
    end;

    if IsHighlightSearchText and not(ActiveProperties.UseOwnFont and
      IsSymbolFontType(Index)) then
      DrawItemText(AText, ATextRect)
    else
      DrawText(Canvas.Handle, PChar(AText), Length(AText), ATextRect, AFlags);
...
function TcxFontNameComboBoxListBox.GetActiveProperties
  : TcxCustomFontNameComboBoxProperties;
begin
...
function TcxFontNameComboBoxListBox.IsSymbolFontType
  (AItemIndex: Integer): Boolean;
begin
  Result := Integer(ActiveProperties.Items.Objects
    [Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(AItemIndex)]) and
    SYMBOL_FONTTYPE <> 0;
end;
...
begin
  if ActiveProperties.FontPreview.Visible then
    AFontPreviewPanelHeight := ActiveProperties.ScaleFactor.Apply
...
function TcxFontNameComboBoxLookupData.GetVisualAreaPreferredSize
...
begin
  ...
  if ftiShowInCombo in ActiveProperties.ShowFontTypeIcon then
    AWidthDelta := GetFontGlyphSize(ScaleFactor).cx + IconBorderWidth * 2 +
      ScaleFactor.Apply(IconTextOffset)
  else
...
procedure TcxFontNameComboBoxLookupData.DoInitialize(AVisualControlsParent
  : TWinControl);
var
...
begin
  ...
  if AProperties.FontPreview.Visible and Assigned(AVisualControlsParent) and
    AVisualControlsParent.HandleAllocated then
  begin
    if not Assigned(FPanel) then
    begin
      FPanel := TFontPreviewPanel.Create(AVisualControlsParent);
      FPanel.FontPreview := AProperties.FontPreview;
      FPanel.LookAndFeel.MasterLookAndFeel :=
        (Edit as TcxCustomFontNameComboBox).PopupControlsLookAndFeel;
    end;
    ...
    try
      ...
      FPanel.ShowEndEllipsis := AProperties.FontPreview.ShowEndEllipsis and
        (AProperties.FontPreview.PreviewType <> cxfpFontName);
      AAlignment := AProperties.FontPreview.Alignment;
      if FPanel.UseRightToLeftAlignment then
        ChangeBiDiModeAlignment(AAlignment);
      FPanel.Alignment := AAlignment;
      FPanel.WordWrap := AProperties.FontPreview.WordWrap and
        (AProperties.FontPreview.PreviewType <> cxfpFontName);
      FPanel.ShowButtons := AProperties.FontPreview.ShowButtons;
      FPanel.Parent := AVisualControlsParent;
      FPanel.Height := AProperties.ScaleFactor.Apply
        (cxFontPreviewPanelDefaultHeight);
      FPanel.Visible := True;
    finally
      FPanel.Locked := False;
...
procedure TcxFontNameComboBoxLookupData.DoPositionVisualArea
  (const ARect: TRect);
var
  R: TRect;
begin
  R := ARect;
  if ActiveProperties.FontPreview.Visible and Assigned(FPanel) and FPanel.HandleAllocated
  then
  begin
    FPanel.SetBounds(R.Left, R.Top, R.Right - R.Left,
      ActiveProperties.ScaleFactor.Apply(cxFontPreviewPanelDefaultHeight));
...
function TcxFontNameComboBoxLookupData.GetListBoxClass
...
procedure TcxFontNameComboBoxLookupData.
  InternalChangeCurrentMRUFontNamePosition;
var
  FIndex: Integer;
begin
...
function TcxFontNameComboBoxLookupData.GetActiveProperties
  : TcxCustomFontNameComboBoxProperties;
begin
...
begin
  case ActiveProperties.FontPreview.PreviewType of
    cxfpCustom:
      Result := ActiveProperties.FontPreview.PreviewText;
    cxfpFullAlphabet:
      Result := 'ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 1234567890';
  else
    Result := GetFontName;
  end;
end;
...
class function TcxCustomFontNameComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
...
procedure TcxCustomFontNameComboBoxProperties.DoAssign
...
class function TcxCustomFontNameComboBoxProperties.GetLookupDataClass
  : TcxInterfacedPersistentClass;
begin
  Result := TcxFontNameComboBoxLookupData;
end;

class function TcxCustomFontNameComboBoxProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TcxCustomFontNameComboBoxViewData;
end;

class function TcxCustomFontNameComboBoxProperties.GetViewInfoClass
...
procedure TcxCustomFontNameComboBoxProperties.SetShowFontTypeIcon
  (Value: TcxShowFontIconTypes);
begin
...
function TcxCustomFontNameComboBoxProperties.FindItemByValue(const AEditValue
  : TcxEditValue): Integer;
begin
  if not LoadFontComplete then
    Result := -1
  else if IsVarEmpty(AEditValue) then
    Result := -1
  else
    Result := Items.IndexOf(VarToStr(AEditValue));
end;

function TcxCustomFontNameComboBoxProperties.GetItemTypes(Index: Integer)
  : TcxFontTypes;
begin
  Result := RealFontTypeToCxTypes(Integer(Items.Objects[Index]));
end;

procedure TcxCustomFontNameComboBoxProperties.SetFontPreview
  (Value: TcxFontPreview);
begin
...
procedure TcxCustomFontNameComboBoxProperties.DoUpdate
  (AProperties: TcxCustomEditProperties);
var
...
begin
  ...
  begin
    AFontNameComboBoxProperties :=
      AProperties as TcxCustomFontNameComboBoxProperties;
    AFontNameComboBoxProperties.Items.Assign(Items);
    AFontNameComboBoxProperties.MRUFontNames.Assign(MRUFontNames);
  end;
end;

procedure TcxCustomFontNameComboBoxProperties.FontLoaderCompleteHandler
  (Sender: TObject);
begin
  Items.BeginUpdate;
  try
    ...
    if Assigned(Sender) then
      Items.Assign((Sender as TcxFontLoader).FontList);
  finally
    Items.EndUpdate;
  end;
  FLoadFontComplete := True;
...
procedure TcxCustomFontNameComboBoxProperties.FontLoaderDestroyHandler
  (Sender: TObject);
begin
  FFontLoader.OnCompleteThread := nil;
  FFontLoader.OnDestroyThread := nil;
...
function TcxCustomFontNameComboBoxProperties.AddMRUFontName(const AFontName
  : TFontName): TcxMRUFontNameAction;
var
  FIndex: Integer;
begin
  Result := mfaNone;
...
function TcxCustomFontNameComboBoxProperties.DelMRUFontName(const AFontName
  : TFontName): TcxMRUFontNameAction;
var
  FIndex: Integer;
begin
...
class function TcxCustomFontNameComboBox.GetPropertiesClass
...
begin
  ...
  TcxCustomFontNameComboBoxProperties(inherited Properties)
    .FOnInternalLoadFontComplete := InternalLoadFontCompleteHandler;
end;
...
begin
  FNeedsUpdateMRUList := FNeedsUpdateMRUList or
    (AReason in [crTab, crEnter, crClose]);
  try
...
function TcxCustomFontNameComboBox.AddMRUFontName(const AFontName: TFontName)
  : TcxMRUFontNameAction;
begin
  Result := ActiveProperties.AddMRUFontName(AFontName);
end;

function TcxCustomFontNameComboBox.DelMRUFontName(const AFontName: TFontName)
  : TcxMRUFontNameAction;
begin
  Result := ActiveProperties.DelMRUFontName(AFontName);
end;

function TcxCustomFontNameComboBox.GetProperties
  : TcxCustomFontNameComboBoxProperties;
begin
  Result := TcxCustomFontNameComboBoxProperties(inherited Properties);
end;

function TcxCustomFontNameComboBox.GetActiveProperties
  : TcxCustomFontNameComboBoxProperties;
begin
...
procedure TcxCustomFontNameComboBox.SetProperties
  (Value: TcxCustomFontNameComboBoxProperties);
begin
  Properties.Assign(Value);
end;

procedure TcxCustomFontNameComboBox.InternalLoadFontCompleteHandler
  (Sender: TObject);
var
...
begin
  ...
  begin
    ...
    LookupData.InternalSetCurrentKey
      (ActiveProperties.Items.IndexOf(FLocalFontName));
  end;
end;
...
begin
  try
    if FNeedsUpdateMRUList and (FDontCheckModifiedWhenUpdatingMRUList or
      ModifiedAfterEnter) then
    begin
...
procedure TcxCustomFontNameComboBox.InternalSetEditValue
  (const Value: TcxEditValue; AValidateEditValue: Boolean);
begin
...
class function TcxFontNameComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
class function TcxFilterFontNameComboBoxHelper.GetFilterEditClass
  : TcxCustomEditClass;
begin
  Result := TcxFontNameComboBox;
end;
...
GetRegisteredEditProperties.Register(TcxFontNameComboBoxProperties,
...
FilterEditsController.Register(TcxFontNameComboBoxProperties,
  TcxFilterFontNameComboBoxHelper);

finalization

FilterEditsController.Unregister(TcxFontNameComboBoxProperties,
  TcxFilterFontNameComboBoxHelper);
