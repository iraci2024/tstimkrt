...
{$ENDIF}
  ...
  StdCtrls, Forms, Dialogs, Types,
  cxClasses, cxControls, cxContainer, cxGraphics, cxDataStorage, cxDataUtils,
  cxVariants, dxCoreClasses, cxEdit,
  cxTextEdit, cxEditUtils, cxMaskEdit, cxDropDownEdit, cxImageComboBox,
...
type
  ...
  TcxColorPrepareList = (cxplNone, cxplDelphi, cxplHTML4, cxplX11,
    cxplX11Ordered);
  TcxColorValueFormat = (cxcvRGB, cxcvHexadecimal, cxcvInteger);
  TcxDefaultColorStyle = (cxdcClear, cxdcColor, cxdcHatched, cxdcText,
    cxdcCustom);
  ...
  public
    ...
    function AddColor(const AColor: TColor; const ADescription: string)
      : TcxColorComboBoxItem; virtual;
    ...
    property Items[Index: Integer]: TcxColorComboBoxItem read GetItems
  ...
  TcxChangeItemIndexEvent = procedure(Sender: TObject; const AIndex: Integer)
    of object;
  TcxGetDefaultColorEvent = procedure(Sender: TObject; var AColor: TColor)
    of object;
  ...
  private
    ...
    procedure InternalPrepareColorList(APrepareList
      : TcxColorPrepareList); overload;
  ...
  protected
    ...
    function GetColorByDescription(const ADescription: string;
    ...
    procedure TranslateValues(const AEditValue: TcxEditValue;
      ...
      ANeedDescription: Boolean = False);
  public
    ...
    procedure PrepareColorList(APrepareList: TcxColorPrepareList;
      ASaveCustom, ASaveMRU: Boolean);
    ...
    property ColorDialogShowFull: Boolean read FColorDialogShowFull
      write FColorDialogShowFull default False;
    ...
    property DropDownListStyle: TcxEditDropDownListStyle
      read GetDropDownListStyle write SetDropDownListStyle default lsFixedList;

    property CustomColors: TcxColorComboBoxItems read FCustomColors
    ...
    property MaxMRUColors: Byte read FMaxMRUColors write SetMaxMRUColors
      default 10;
    property MRUColors: TcxColorComboBoxItems read FMRUColors
      write SetMRUColors;
  ...
  protected
    ...
    function DoSelectCustomColor(AColor: TColor; ANeedFireEvent: Boolean)
      : Integer; virtual;
    procedure ClearEditValue; virtual;
    function DoOnSelectCustomColor(var AColor: TColor;
      out AColorDescription: string; ANeedFireEvent: Boolean): Boolean;
  ...
  public
    class procedure DrawColorBox(ACanvas: TcxCanvas; ARect: TRect;
      const AFrameColor, ABoxColor, ABkColor: TColor;
      const DefaultColorStyle: TcxDefaultColorStyle); overload;
    class procedure DrawColorBox(ACanvas: TcxCanvas; ARect: TRect;
      const AFrameColor, ABoxColor, ABkColor: TColor;
      const DefaultColorStyle: TcxDefaultColorStyle;
...
uses
  dxCore, cxExtEditUtils, dxThemeConsts, dxThemeManager, dxUxTheme, Math,
  dxColorDialog, dxCoreGraphics, dxColorPicker,
  dxDPIAwareUtils;
...
function TcxColorDialogHelper.ExecuteAdvancedDialog(var AColor: TColor)
  : Boolean;
var
...
function TcxColorDialogHelper.ExecuteStandardDialog(var AColor: TColor)
  : Boolean;
var
...
begin
  ...
  else if FDialog is TdxColorDialog then
...
function TcxColorComboBoxItems.FindColorItem(const AColor: TColor)
  : TcxColorComboBoxItem;
var
  I: Integer;
begin
...
procedure TcxColorComboBoxItems.SetItems(Index: Integer;
  const Value: TcxColorComboBoxItem);
begin
...
begin
  ...
  TcxColorComboBoxHelper.DrawColorBox(ACanvas, ColorBoxRect, ColorBoxFrameColor,
    ColorBoxColor, BkColor, ARealDefaultColorStyle, ScaleFactor);

  if not IsInplace and not ShowDescriptions and Focused and not HasPopupWindow
  then
  begin
...
begin
  ...
  if IsRectEmpty(ABounds) or (ABounds.Right = cxMaxRectSize) or
    (ABounds.Bottom = cxMaxRectSize) then
    Exit;
  ...
  if (AColorViewInfo.DefaultColorStyle = cxdcText) and not AColorViewInfo.FoundItem
  then
  ...
  begin
    ...
    begin
      ...
      begin
        AColorViewInfo.FColorBoxRect.Right := AColorViewInfo.FColorBoxRect.Left
          + AColorViewInfo.ColorBoxWidth;
        R.Left := AColorViewInfo.FColorBoxRect.Right;
      end
      else
      begin
        AColorViewInfo.FColorBoxRect.Left := AColorViewInfo.FColorBoxRect.Right
          - AColorViewInfo.ColorBoxWidth;
...
begin
  ...
  Properties.InternalGetColorComboBoxDisplayValue(TcxCustomColorComboBox(Edit)
    .ILookupData.CurrentKey, Edit.EditingValue, AViewInfoAccess.FColorBoxColor,
    AViewInfoAccess.Text, AViewInfoAccess.FFoundItem);
  inherited DisplayValueToDrawValue(ADisplayValue, AViewInfo);
end;

procedure TcxCustomColorComboBoxViewData.EditValueToDrawValue(const AEditValue
...
procedure TcxCustomColorComboBoxViewData.CalculateViewInfoProperties
...
begin
  ...
  begin
    ...
    ColorBoxWidth := ScaleFactor.Apply(AProperties.ColorBoxWidth,
      AProperties.ScaleFactor);
...
function TcxCustomColorComboBoxViewData.InternalGetEditConstantPartSize
...
begin
  ...
  if AProperties.ShowDescriptions then
    Inc(Result.cx, ScaleFactor.Apply(AProperties.ColorBoxWidth,
      AProperties.ScaleFactor) + 6);
end;
...
function TcxCustomColorComboBoxViewData.GetProperties
  : TcxCustomColorComboBoxProperties;
begin
...
function TcxCustomColorComboBoxListBox.GetItemHeight
...
procedure TcxCustomColorComboBoxListBox.DrawItem(Index: Integer; Rect: TRect;
...
begin
  ...
  try
    ...
    TcxColorComboBoxHelper.DrawColorBox(Canvas, AColorBoxRect,
      Edit.ActiveProperties.ColorBoxFrameColor,
      Edit.ActiveProperties.GetColorByIndex
      (Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(Index)),
...
procedure TcxCustomColorComboBoxListBox.MouseUp(Button: TMouseButton;
...
function TcxColorComboBoxLookupData.GetActiveProperties
  : TcxCustomColorComboBoxProperties;
begin
...
begin
  ...
  FDefaultDescription := cxGetResourceString
    (@cxSColorComboBoxDefaultDescription);
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        Self.ColorBoxWidth := Self.ScaleFactor.Apply(ColorBoxWidth,
          ScaleFactor);
...
begin
  GetColorComboBoxDisplayValue(AEditValue1, AColor1, ADescription1,
    AColorFound1);
  GetColorComboBoxDisplayValue(AEditValue2, AColor2, ADescription2,
    AColorFound2);

  Result := ShowColorBox(AColorFound1) = ShowColorBox(AColorFound2);
  if Result then
    if ShowColorBox(AColorFound1) then
      Result := (AColor1 = AColor2) and
        (not ShowDescriptions or InternalCompareString(ADescription1,
        ADescription2, True))
    else
      Result := not ShowDescriptions or InternalCompareString(ADescription1,
        ADescription2, True);
end;

class function TcxCustomColorComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxColorComboBox;
end;

class function TcxCustomColorComboBoxProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxCustomColorComboBoxViewInfo;
end;

function TcxCustomColorComboBoxProperties.GetEditValueSource
  (AEditFocused: Boolean): TcxDataEditValueSource;
begin
  Result := evsValue;
end;

function TcxCustomColorComboBoxProperties.IsDisplayValueValid(var DisplayValue
  : TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := True;
end;

procedure TcxCustomColorComboBoxProperties.PrepareDisplayValue(const AEditValue
...
function TcxCustomColorComboBoxProperties.GetSupportedOperations
...
function TcxCustomColorComboBoxProperties.GetDisplayText(const AEditValue
...
procedure TcxCustomColorComboBoxProperties.DoUpdate
...
procedure TcxCustomColorComboBoxProperties.GetColorComboBoxDisplayValue
  (const AEditValue: TcxEditValue; out AColor: TColor; out ADescription: string;
  out AColorFound: Boolean);
begin
...
function TcxCustomColorComboBoxProperties.ShowColorBox
  (AColorFound: Boolean): Boolean;
begin
  Result := not(not AColorFound and (DefaultColorStyle = cxdcText));
end;

class function TcxCustomColorComboBoxProperties.GetLookupDataClass
  : TcxInterfacedPersistentClass;
begin
  Result := TcxColorComboBoxLookupData;
end;

class function TcxCustomColorComboBoxProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
  Result := TcxColorComboBoxPopupWindow;
end;

class function TcxCustomColorComboBoxProperties.GetViewDataClass
...
function TcxCustomColorComboBoxProperties.EditValueToColorValue(const AEditValue
  : Variant): TColor;
begin
...
function TcxCustomColorComboBoxProperties.GetColorByIndex
...
function TcxCustomColorComboBoxProperties.GetColorByDescription
  (const ADescription: string; out AColor: TColor): Boolean;
var
  I: Integer;
  ACount: Integer;
begin
...
function TcxCustomColorComboBoxProperties.GetIndexByColor
...
function TcxCustomColorComboBoxProperties.GetDescriptionByIndex
...
function TcxCustomColorComboBoxProperties.IndexByValue
...
function TcxCustomColorComboBoxProperties.StrToColor(const S: string;
  out AColor: TColor): Boolean;
begin
  Result := (Length(Trim(S)) > 0) and (GetColorByDescription(S, AColor) or
    cxStrToColor(S, AColor));
end;

function TcxCustomColorComboBoxProperties.AddMRUColor(const AColor: TColor)
  : TcxMRUColorAction;
var
  AIndex: Integer;
begin
  Result := mcaNone;
  if MaxMRUColors = 0 then
    Exit;
...
function TcxCustomColorComboBoxProperties.DelMRUColor(const AColor: TColor)
  : TcxMRUColorAction;
var
  AIndex: Integer;
begin
...
procedure TcxCustomColorComboBoxProperties.DoGetDefaultColor
  (var AColor: TColor);
begin
...
procedure TcxCustomColorComboBoxProperties.TranslateValues(const AEditValue
  : TcxEditValue; var AColor: TColor; var ADescription: string;
  ANeedDescription: Boolean = False);
var
  AFoundIndex: Integer;
  AValid: Boolean;
...
begin
  ...
  begin
    ...
    if AValid then
    begin
      S := TcxColorComboBoxHelper.GetColorName(AColor, '', NamingConvention,
        ColorValueFormat);
...
function TcxCustomColorComboBoxProperties.ColorItemByIndex(AIndex: Integer)
  : TcxColorComboBoxItem;
begin
  if AIndex = -1 then
    Result := nil
  else if AIndex <= MRUColors.Count - 1 then
...
function TcxCustomColorComboBoxProperties.DoConvertNaming
  (AIndex: Integer): string;
var
...
begin
  ...
  if FItem = nil then
    ...
    Result := TcxColorComboBoxHelper.GetColorName(FItem.Color,
      FItem.Description, NamingConvention, ColorValueFormat);
...
function TcxCustomColorComboBoxProperties.GetDropDownListStyle
...
begin
  ...
  begin
    if (AItemIndex = -1) and (ColorComboStyle = cxccsComboList) and AColorFound
    then
      AColor := DefaultColor
    else
    begin
...
procedure TcxCustomColorComboBoxProperties.InternalPrepareColorList
  (APrepareList: TcxColorPrepareList);
begin
...
begin
  Result := DefaultDescription <> cxGetResourceString
    (@cxSColorComboBoxDefaultDescription);
end;
...
procedure TcxCustomColorComboBoxProperties.SetColorBoxAlign
  (Value: TcxColorBoxAlign);
begin
...
procedure TcxCustomColorComboBoxProperties.SetColorComboStyle
  (Value: TcxColorComboStyle);
begin
...
procedure TcxCustomColorComboBoxProperties.SetColorValueFormat
  (Value: TcxColorValueFormat);
begin
...
procedure TcxCustomColorComboBoxProperties.SetDropDownListStyle
  (AValue: TcxEditDropDownListStyle);
begin
...
procedure TcxCustomColorComboBoxProperties.SetDefaultColorStyle
  (Value: TcxDefaultColorStyle);
begin
...
procedure TcxCustomColorComboBoxProperties.SetCustomColors
  (Value: TcxColorComboBoxItems);
begin
  FCustomColors.Assign(Value);
end;

procedure TcxCustomColorComboBoxProperties.SetItems
...
procedure TcxCustomColorComboBoxProperties.SetMRUColors
  (Value: TcxColorComboBoxItems);
begin
  FMRUColors.Assign(Value);
end;

procedure TcxCustomColorComboBoxProperties.SetNamingConvention
  (Value: TcxColorNamingConvention);
begin
...
procedure TcxCustomColorComboBoxProperties.SetPrepareList
  (Value: TcxColorPrepareList);
begin
...
procedure TcxCustomColorComboBoxProperties.SetShowDescriptions
  (const Value: Boolean);
begin
...
class function TcxCustomColorComboBox.GetPropertiesClass
...
procedure TcxCustomColorComboBox.PrepareEditValue(const ADisplayValue
  : TcxEditValue; out EditValue: TcxEditValue; AEditFocused: Boolean);
var
  AColor: TColor;
  AIndex: Integer;
begin
...
function TcxCustomColorComboBox.DoSelectCustomColor(AColor: TColor;
  ANeedFireEvent: Boolean): Integer;
var
...
procedure TcxCustomColorComboBox.InternalValidateDisplayValue
...
function TcxCustomColorComboBox.DoOnSelectCustomColor(var AColor: TColor;
  out AColorDescription: string; ANeedFireEvent: Boolean): Boolean;
begin
...
begin
  ...
  begin
    ...
    LookupData.InternalSetCurrentKey
      (ActiveProperties.GetIndexByColor(ColorValue));
    UpdateDrawValue;
  end;
end;

function TcxCustomColorComboBox.
  NeedResetInvalidTextWhenPropertiesChanged: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  AKey := DoSelectCustomColor(ASelectedColor,
    ActiveProperties.ColorDialogType = cxcdtCustom);
  if VarEqualsExact(AKey, LookupData.CurrentKey) and
    (ActiveProperties.GetColorByIndex(LookupData.CurrentKey) <> APrevColor) then
    LookupData.InternalSetCurrentKey
      (ActiveProperties.GetIndexByColor(APrevColor));
...
function TcxCustomColorComboBox.GetActiveProperties
  : TcxCustomColorComboBoxProperties;
begin
...
function TcxCustomColorComboBox.AddMRUColor(const AColor: TColor)
  : TcxMRUColorAction;
begin
  Result := ActiveProperties.AddMRUColor(AColor);
end;

function TcxCustomColorComboBox.DelMRUColor(const AColor: TColor)
  : TcxMRUColorAction;
begin
  Result := ActiveProperties.DelMRUColor(AColor);
end;

function TcxCustomColorComboBox.LookupKeyToEditValue(const AKey: TcxEditValue)
  : TcxEditValue;
begin
...
procedure TcxCustomColorComboBox.DoEditKeyDown(var Key: Word;
  Shift: TShiftState);
var
  APrevKey: Word;
  AColor: TColor;
begin
  ...
  if (APrevKey = VK_RETURN) and not(ssAlt in Shift) and (ItemIndex = -1) and
    ActiveProperties.StrToColor(Text, AColor) then
    LookupData.SetCurrentKey(DoSelectCustomColor(AColor, False));
end;

procedure TcxCustomColorComboBox.PropertiesLoadColorListHandler
  (Sender: TObject);
begin
...
procedure TcxCustomColorComboBox.PrepareColorList(APrepareList
  : TcxColorPrepareList; ASaveCustom, ASaveMRU: Boolean);
var
...
procedure TcxCustomColorComboBox.PrepareDelphiColorList(const ASaveCustom,
  ASaveMRU: Boolean);
begin
  PrepareColorList(cxplDelphi, ASaveCustom, ASaveMRU);
end;
...
class function TcxColorComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
class function TcxFilterColorComboBoxHelper.GetFilterEditClass
  : TcxCustomEditClass;
begin
  Result := TcxColorComboBox;
end;
...
begin
  ...
  TcxCustomColorComboBoxProperties(AProperties).DropDownListStyle :=
    lsFixedList;
end;

{ TcxColorComboBoxHelper }

class procedure TcxColorComboBoxHelper.DrawColorBox(ACanvas: TcxCanvas;
  ARect: TRect; const AFrameColor, ABoxColor, ABkColor: TColor;
  const DefaultColorStyle: TcxDefaultColorStyle);
begin
  DrawColorBox(ACanvas, ARect, AFrameColor, ABoxColor, ABkColor,
    DefaultColorStyle, dxSystemScaleFactor);
end;

class procedure TcxColorComboBoxHelper.DrawColorBox(ACanvas: TcxCanvas;
  ARect: TRect; const AFrameColor, ABoxColor, ABkColor: TColor;
  const DefaultColorStyle: TcxDefaultColorStyle; AScaleFactor: TdxScaleFactor);
begin
    ...
    case DefaultColorStyle of
      ...
      cxdcHatched:
        begin
          ...
          ACanvas.Polyline([Point(ARect.Left - 1, ARect.Top - 1),
            Point(ARect.Right, ARect.Top - 1), Point(ARect.Right, ARect.Bottom),
            Point(ARect.Left - 1, ARect.Bottom), Point(ARect.Left - 1,
            ARect.Top - 1)]);
        end;
    end;
    ACanvas.Brush.Style := bsSolid;
...
class function TcxColorComboBoxHelper.GetColorName(AColor: TColor;
  const ADescription: string; const ANamingConvention: TcxColorNamingConvention;
  const AColorValueFormat: TcxColorValueFormat): string;
  ...
  begin
    ...
    case AColorValueFormat of
      cxcvRGB:
        Result := Format('%d.%d.%d', [GetRValue(RGB), GetGValue(RGB),
          GetBValue(RGB)]);
      cxcvHexadecimal:
  ...
  function DoGetColorName(const AColorValues: array of TColor;
    const AColorNames: array of string): string;
  var
    I: Integer;
  begin
    for I := Low(AColorValues) to High(AColorValues) do
