...
uses
  cxCustomConverter, cxBlobEdit, StdCtrls, cxConverterUtils, cxImage,
  cxButtonEdit,
  cxEdit, Classes, Graphics, cxCalc, cxCheckBox, cxDropDownEdit, cxCurrencyEdit,
  cxCalendar, cxDBLookupComboBox, cxHyperLinkEdit, cxImageComboBox, Controls,
  SysUtils,
...
type
  ...
  protected
    function ConvertPictureTransparency(const AValue: string)
      : TcxImageTransparency;
    function ConvertVertAlignment(const AVertAlignment: string)
  ...
  private
    function ConvertNullFieldStyle(const AValue: string)
...
function TcxCustomPropertiesConverter.ConvertVertAlignment(const AVertAlignment
  : string): TcxEditVertAlignment;
begin
...
begin
  with Destination do
  begin
    AlwaysSaveData := GetBooleanProperty(Source, 'AlwaysSaveText',
      AlwaysSaveData);
    BlobEditKind := ConvertBlobEditKind(GetEnumProperty(Source, 'BlobKind'));
    BlobPaintStyle := ConvertBlobPaintStyle(GetEnumProperty(Source,
      'BlobPaintStyle'));
    ...
    MemoOEMConvert := GetBooleanProperty(Source, 'MemoOEMConvert',
      MemoOEMConvert);
    MemoScrollBars := ConvertScrollStyle(GetEnumProperty(Source,
      'MemoScrollBars'));
    MemoWantReturns := GetBooleanProperty(Source, 'MemoWantReturns',
      MemoWantReturns);
    ...
    PictureAutoSize := GetBooleanProperty(Source, 'PictureAutoSize',
      PictureAutoSize);
    PictureFilter := GetStringProperty(Source, 'PictureFilter', PictureFilter);
    PictureTransparency := ConvertPictureTransparency
      (GetEnumProperty(Source, 'PictureTransparency'));
    ReadOnly := GetBooleanProperty(Source, 'ReadOnly', ReadOnly);
    ShowExPopupItems := GetBooleanProperty(Source, 'ShowExPopupItems',
      ShowExPopupItems);
    ShowPicturePopup := GetBooleanProperty(Source, 'ShowPicturePopup',
      ShowPicturePopup);
  end;
  inherited DoImport;
end;

function TcxBlobEditPropertiesConverter.ConvertBlobEditKind(const AKind: string)
  : TcxBlobEditKind;
begin
    ...
    begin
      if AButtons is TCollection then
        ...
        begin
          ...
          begin
            AButton := Buttons.Add;
            AButton.Default := GetBooleanProperty(Items[I], 'Default',
              AButton.Default);
            AButton.Glyph.Assign(GetClassProperty(Items[I], 'Glyph')
              as TBitmap);
            AButton.Kind := ConvertButtonKind(GetEnumProperty(Items[I],
              'Kind'));
            AButton.LeftAlignment := GetBooleanProperty(Items[I],
              'LeftAlignment', AButton.LeftAlignment);
            AButton.Visible := GetBooleanProperty(Items[I], 'Visible',
              AButton.Visible);
            AButton.Width := GetIntegerProperty(Items[I], 'Width',
              AButton.Width);
          end;
        end;
    end;
...
function TcxButtonEditPropertiesConverter.ConvertButtonKind(const AButtonKind
  : string): TcxEditButtonKind;
begin
...
function TcxButtonEditPropertiesConverter.GetDestination
...
begin
  ...
  begin
    Alignment.Horz := ConvertAlignment(GetEnumProperty(Source, 'Alignment'));
    Alignment.Vert := ConvertVertAlignment(GetEnumProperty(Source,
      'VertAlignment'));
...
begin
  ...
  begin
    ...
    DisplayChecked := GetStringProperty(Source, 'DisplayChecked',
      DisplayChecked);
    DisplayGrayed := GetStringProperty(Source, 'DisplayNull', DisplayGrayed);
    DisplayUnchecked := GetStringProperty(Source, 'DisplayUnchecked',
      DisplayUnchecked);
    NullStyle := ConvertNullFieldStyle(GetEnumProperty(Source,
      'ShowNullFieldStyle'));
  end;
  inherited DoImport;
end;

function TcxCheckBoxPropertiesConverter.ConvertNullFieldStyle
  (const AValue: string): TcxCheckBoxNullValueShowingStyle;
begin
...
begin
  ...
  begin
    ...
    ImmediateDropDown := GetBooleanProperty(Source, 'ImmediateDropDown',
      ImmediateDropDown);
    ...
    PopupAlignment := ConvertAlignment(GetEnumProperty(Source,
      'PopupAlignment'));
    ReadOnly := GetBooleanProperty(Source, 'ReadOnly', ReadOnly);
...
begin
  ...
  begin
    ...
    UseThousandSeparator := GetBooleanProperty(Source, 'UseThousandSeparator',
      UseThousandSeparator);
  end;
  inherited DoImport;
end;

function TcxCurrencyEditPropertiesConverter.GetDestination
...
function TcxHyperLinkEditPropertiesConverter.GetDestination
...
begin
  ...
  begin
    ...
    if AObject <> nil then
    begin
      PopupMenuLayout.CustomMenuItemCaption :=
        GetStringProperty(AObject, 'CustomButtonCaption',
        PopupMenuLayout.CustomMenuItemCaption);
...
begin
  ...
  begin
    ...
    ShowDescriptions := GetBooleanProperty(Source, 'ShowDescription',
      ShowDescriptions);
...
function TcxImageComboBoxPropertiesConverter.GetDestination
...
begin
  ...
  begin
    ...
    ListFieldNames := GetStringProperty(Source, 'ListFieldName',
      ListFieldNames);
    PopupAlignment := ConvertAlignment(GetEnumProperty(Source,
      'PopupAlignment'));
...
function TcxLookupComboBoxPropertiesConverter.GetDestination
...
begin
  ...
  begin
    ...
    IgnoreMaskBlank := GetBooleanProperty(Source, 'IgnoreMaskBlank',
      IgnoreMaskBlank);
...
begin
  ...
  begin
    ...
    PopupClientEdge := GetBooleanProperty(Source, 'PopupFormClientEdge',
      PopupClientEdge);
    ...
    PopupMinHeight := GetIntegerProperty(Source, 'PopupMinHeight',
      PopupMinHeight);
    ...
    PopupSizeable := GetBooleanProperty(Source, 'PopupFormSizeable',
      PopupSizeable);
...
function TcxTimeEditPropertiesConverter.ConvertTimeFormat(const AFormat: string)
