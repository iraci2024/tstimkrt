...
uses
  Types, Variants, Windows, Classes, Controls, Forms, Graphics, Messages,
  SysUtils,
...
type
  ...
  TcxProgressBarBarStyle = (cxbsSolid, cxbsLEDs, cxbsGradient, cxbsGradientLEDs,
    cxbsBitmap, cxbsBitmapLEDs, cxbsAnimation, cxbsAnimationLEDs);
...
type
  ...
  private
    ...
    procedure CalcDrawingParams(out ADrawProgressBarRect, ADrawOverloadBarRect,
      ADrawPeakBarRect, ADrawAnimationBarRect, ASolidRect: TRect;
      out ALEDsWidth: Integer);
    function CanAnimationBarShow: Boolean;
    procedure CreateBarBmp;
    procedure CreateNativeBitmap(var ABitmap: TcxAlphaBitmap;
      const ASize: TSize);
    ...
    procedure DrawBackground(ACanvas: TcxCanvas; const ACanvasParent: TcxCanvas;
      const ABounds: TRect);
    ...
    procedure DrawGradientBar(ACanvas: TcxCanvas;
      const ANormalRect, AOverloadRect, ABarRect: TRect);
    procedure DrawSolidBar(ACanvas: TcxCanvas;
      const ANormalRect, AOverloadRect: TRect);
    procedure DrawVistaStyleMarquee(ACanvas: TcxCanvas; const ARect: TRect);
    procedure DrawAnimationBar(ACanvas: TcxCanvas;
      const ABar, ASolidRect: TRect);
    procedure DrawAnimationBarBackground(ACanvas: TcxCanvas;
      const ASolidRect: TRect; ASolidColor: TColor; ADrawBar: Boolean);
    function CalcLEDsWidth: Integer;
    procedure AdjustForLEDsBarBounds(var ABarRect, AOverloadBarRect: TRect;
      const ALEDsWidth: Integer);
    procedure DrawPeak(ACanvas: TcxCanvas; const APeakRect: TRect);
    procedure DrawBorderLEDs(ACanvas: TcxCanvas; const ABarRect: TRect;
      ALEDsWidth: Integer);
  ...
  protected
    ...
    property AnimationRestartDelay: Cardinal read FAnimationRestartDelay
      write FAnimationRestartDelay;
    property AnimationRestartDelayTimer: TcxTimer
      read FAnimationRestartDelayTimer;
    ...
    property ForegroundImage: TBitmap read FForegroundImage
      write FForegroundImage;
  ...
  protected
    procedure CalculateViewInfoProperties
  ...
  protected
    ...
    procedure CorrectPositionWithMaxMin(AViewInfo
      : TcxCustomProgressBarViewInfo); virtual;
    function InternalGetDisplayText(const AEditValue: TcxEditValue;
      AMarquee: Boolean; AShowTextStyle: TcxProgressBarTextStyle;
      AFullText: Boolean = False; AIsInplace: Boolean = True)
      : WideString; virtual;

    property AssignedValues: TcxProgressBarPropertiesValues
      read GetAssignedValues write SetAssignedValues;
    property IsForegroundImageChanged: Boolean read FIsForegroundImageChanged
      write FIsForegroundImageChanged;
    property IsPositionChanged: Boolean read FIsPositionChanged
      write FIsPositionChanged;
  public
    ...
    property AnimationRestartDelay: Cardinal read FAnimationRestartDelay
      ...
      default cxProgressBarDefaultAnimationRestartDelay;
    property AnimationSpeed: TcxProgressBarAnimationSpeed read FAnimationSpeed
      write SetAnimationSpeed default cxProgressBarDefaultAnimationSpeed;
    property BarBevelOuter: TcxProgressBarBevelOuter read FBarBevelOuter
...
uses
  ...
  dxUxTheme, cxGeometry, dxOffice11, cxLookAndFeels, dxCoreClasses,
...
procedure CalculateCustomProgressBarViewInfo
  (AViewData: TcxCustomProgressBarViewData;
...
function CalculateDelta(const APositionDelta, ARectWidth,
  AMaxMinDelta: Double): Integer;
var
  ACalc: Double;
begin
...
function TcxCustomProgressBarViewInfo.GetUpdateRegion
  (AViewInfo: TcxContainerViewInfo): TcxRegion;
begin
  Result := inherited GetUpdateRegion(AViewInfo);
  if not(AViewInfo is TcxCustomProgressBarViewInfo) then
    Exit;
end;
...
procedure TcxCustomProgressBarViewInfo.DrawVistaStyleMarquee(ACanvas: TcxCanvas;
  const ARect: TRect);
var
  ASize: TSize;
  AGradientRect: TRect;
...
begin
  ...
  if not Assigned(FVistaStyleMarqueeBitmap) or
    not cxSizeIsEqual(ASize, cxRectSize(FVistaStyleMarqueeBitmap.ClientRect))
  then
  begin
    ...
    cxStretchBlt(FVistaStyleMarqueeBitmap.Canvas.Handle,
      FNativeBitmap.Canvas.Handle, FVistaStyleMarqueeBitmap.ClientRect,
      FNativeBitmap.ClientRect, SRCCOPY);
    AGradientBitmap := TcxAlphaBitmap.CreateSize(ASize.cx, ASize.cy);
    try
      if FOrientation = cxorHorizontal then
      begin
        AGradientRect := cxRectSetWidth(AGradientBitmap.ClientRect,
          cxRectWidth(AGradientBitmap.ClientRect) div 2);
        FillGradientRect(AGradientBitmap.Canvas.Handle, AGradientRect,
          RGB(0, 0, 0), RGB(0, 255, 0), True);
        AGradientRect.Left := AGradientRect.Right;
        AGradientRect.Right := AGradientBitmap.ClientRect.Right;
        FillGradientRect(AGradientBitmap.Canvas.Handle, AGradientRect,
          RGB(0, 255, 0), RGB(0, 0, 0), True);
      end
      else
      begin
        AGradientRect := cxRectSetHeight(AGradientBitmap.ClientRect,
          cxRectHeight(AGradientBitmap.ClientRect) div 2);
        FillGradientRect(AGradientBitmap.Canvas.Handle, AGradientRect,
          RGB(0, 0, 0), RGB(0, 255, 0), False);
        AGradientRect.Top := AGradientRect.Bottom;
        AGradientRect.Bottom := AGradientBitmap.ClientRect.Bottom;
        FillGradientRect(AGradientBitmap.Canvas.Handle, AGradientRect,
          RGB(0, 255, 0), RGB(0, 0, 0), False);
      end;
      ...
      begin
        ARGBColors[I].rgbRed := ARGBGradientColors[I].rgbGreen * ARGBColors[I]
          .rgbRed div 255;
        ARGBColors[I].rgbGreen := ARGBGradientColors[I].rgbGreen * ARGBColors[I]
          .rgbGreen div 255;
        ARGBColors[I].rgbBlue := ARGBGradientColors[I].rgbGreen * ARGBColors[I]
          .rgbBlue div 255;
  ...
  cxAlphaBlend(ACanvas.Handle, FVistaStyleMarqueeBitmap, ARect,
    FVistaStyleMarqueeBitmap.ClientRect);
end;
...
procedure TcxCustomProgressBarViewInfo.Paint(ACanvas: TcxCanvas);
  ...
  begin
    Result := FIsBoundsChanged or IsDoubleBuffered or
      (ShowText and (GetDrawText <> ''));
  end;
...
begin
  ACanvas.SaveState;
  try
    CalcDrawingParams(ADrawProgressBarRect, ADrawOverloadBarRect,
      ADrawPeakBarRect, ADrawAnimationBarRect, ASolidRect, ALEDsWidth);
    ABarRect := BarRect;
    ...
    try
      ...
      case FBarStyle of
        cxbsSolid, cxbsLEDs, cxbsGradient, cxbsGradientLEDs:
          begin
            if (FBarStyle in [cxbsSolid, cxbsLEDs]) and not NativeStyle then
              DrawSolidBar(FUsualBitmap.cxCanvas, ADrawProgressBarRect,
                ADrawOverloadBarRect)
            else
              DrawGradientBar(FUsualBitmap.cxCanvas, ADrawProgressBarRect,
                ADrawOverloadBarRect, ABarRect);
        ...
        cxbsAnimation, cxbsAnimationLEDs:
          begin
            if not(FMarquee and IsDesigning) and
              not(FMarquee and (FBarStyle = cxbsAnimation) and NativeStyle) then
              DrawAnimationBarBackground(FUsualBitmap.cxCanvas, ASolidRect,
                FBeginColor, True);
            if not FMarquee then
              DrawAnimationBar(FUsualBitmap.cxCanvas, ADrawAnimationBarRect,
                ASolidRect)
            else
              DrawVistaStyleMarquee(FUsualBitmap.cxCanvas,
                ADrawAnimationBarRect);
          end;
      end;
      if IsLEDStyle then
    ...
    cxBitBlt(ACanvas.Canvas.Handle, FUsualBitmap.cxCanvas.Handle, BarRect,
      cxNullPoint, SRCCOPY);
  finally
...
procedure TcxCustomProgressBarViewInfo.PaintProgressBarByPainter
  (ACanvas: TcxCanvas);
var
  AChunkRect: TRect;
  AContentRect: TRect;
  ...
  var
    AOverloadRect: TRect;
  begin
    AOverloadRect := CalcRect(AVertical, AContentRect, RelativeOverloadValue /
      MaxMinDiff);
    if AVertical then
    begin
...
begin
  ...
  AContentRect := cxRectContent(ARect,
    Painter.ProgressBarBorderSize(AVertical));
  if FMarquee and not IsInplace then
  begin
    ...
    AChunkRect := CalcRect(AVertical, AContentRect,
      RelativePosition / MaxMinDiff);
  ...
  cxBitBlt(ACanvas.Handle, FPainterBitmap.cxCanvas.Handle, Bounds,
    cxNullPoint, SRCCOPY);
end;

procedure TcxCustomProgressBarViewInfo.CalcDrawingParams
  (out ADrawProgressBarRect, ADrawOverloadBarRect, ADrawPeakBarRect,
  ADrawAnimationBarRect, ASolidRect: TRect; out ALEDsWidth: Integer);
begin
  ...
  if IsLEDStyle then
    AdjustForLEDsBarBounds(ADrawProgressBarRect, ADrawOverloadBarRect,
      ALEDsWidth);

  if not FRealShowOverload then
    ASolidRect := ADrawProgressBarRect
  else if FOrientation = cxorHorizontal then
...
begin
  Result := (((FBarStyle in [cxbsAnimation, cxbsAnimationLEDs]) and
    not FMarquee) or FMarquee) and (FAnimationSpeed > 0) and not IsDesigning and
    not IsInplace;
end;
...
procedure TcxCustomProgressBarViewInfo.CreateNativeBitmap
  (var ABitmap: TcxAlphaBitmap; const ASize: TSize);
var
  ATheme: TdxTheme;
  ARect: TRect;
begin
  ...
  else if cxSizeIsEqual(cxRectSize(ABitmap.ClientRect), ASize) then
    Exit;
...
procedure TcxCustomProgressBarViewInfo.ExcludeRects(ACanvas: TcxCanvas;
  const ABounds: TRect);
begin
  if (FBarStyle in [cxbsAnimation, cxbsAnimationLEDs]) and NativeStyle then
  begin
    if FOrientation = cxorHorizontal then
    begin
      ACanvas.ExcludeClipRect(Rect(ABounds.Right, Bounds.Top, Bounds.Right,
        Bounds.Bottom));
      ACanvas.ExcludeClipRect(Rect(Bounds.Left, Bounds.Top, ABounds.Left,
        Bounds.Bottom));
    end
    else
    begin
      ACanvas.ExcludeClipRect(Rect(Bounds.Left, Bounds.Top, Bounds.Right,
        ABounds.Top));
      ACanvas.ExcludeClipRect(Rect(Bounds.Left, ABounds.Bottom, Bounds.Right,
        Bounds.Bottom))
    end;
  end
  else
  ...
  begin
    ...
    begin
      ...
      for I := 1 to ALEDsMaxCount do
        ACanvas.ExcludeClipRect(Rect(ABounds.Left + I * ALEDsWidth - 2,
          ABounds.Top, ABounds.Left + I * ALEDsWidth, ABounds.Bottom));
    end
    else
    begin
      ALEDsMaxCount := cxRectHeight(ABounds) div ALEDsWidth;
      for I := 1 to ALEDsMaxCount do
        ACanvas.ExcludeClipRect(Rect(ABounds.Left,
          ABounds.Bottom - I * ALEDsWidth, ABounds.Right,
          ABounds.Bottom - I * ALEDsWidth + 2));
    end;
  end;
end;
...
begin
  ...
  case AnimationPath of
    cxapCycle:
      Inc(Result, GetAnimationBarDimension);
    cxapPingPong:
      Dec(Result, GetAnimationBarDimension);
  end;
end;
...
procedure TcxCustomProgressBarViewInfo.DrawBackground(ACanvas: TcxCanvas;
  const ACanvasParent: TcxCanvas; const ABounds: TRect);
const
  BarThemeTypeMap: array [TcxProgressBarOrientation] of Integer = (PP_BAR,
    PP_BARVERT);
var
...
begin
  ...
  begin
    if IsInplace or (csPaintCopy in Edit.ControlState) then
      cxBitBlt(ACanvas.Handle, ACanvasParent.Handle, ABounds,
        BarRect.TopLeft, SRCCOPY)
    else
    begin
  ...
  else if not(FBarStyle in [cxbsAnimation, cxbsAnimationLEDs]) then
  begin
  ...
  begin
    if NativeStyle then
      dxDrawThemeBackground(ACanvas.Handle, totProgress, ScaleFactor, ABounds,
        BarThemeTypeMap[FOrientation], 1)
    else if (FBarStyle in [cxbsAnimation, cxbsAnimationLEDs]) and not FPropTransparent
    then
...
begin
  ...
  try
    ...
    ATextRect := Rect(0, 0, ACanvas.TextWidth(ABarText),
      ACanvas.TextHeight(ABarText));
    if FTextOrientation = cxorVertical then
      ATextRect := Rect(ATextRect.Top, ATextRect.Left, ATextRect.Bottom,
        ATextRect.Right);
    OffsetRect(ATextRect, (cxRectWidth(ABarRect) - cxRectWidth(ATextRect))
      div 2, (cxRectHeight(ABarRect) - cxRectHeight(ATextRect)) div 2);

    if UseRightToLeftAlignment then
    begin
      Painter.DrawProgressBarText(ACanvas, TextOrientation = cxorVertical,
        SolidTextColor, ABarText, TdxRightToLeftLayoutConverter.ConvertRect
        (ATextRect, ABarRect), ABarRect,
        TdxRightToLeftLayoutConverter.ConvertRect(AProgressBarRect, ABarRect),
        TextColor);
    end
    else
      Painter.DrawProgressBarText(ACanvas, TextOrientation = cxorVertical,
        SolidTextColor, ABarText, ATextRect, ABarRect, AProgressBarRect,
        TextColor);
  finally
...
procedure TcxCustomProgressBarViewInfo.PaintBarBevelOuter(ACanvas: TcxCanvas;
  ABBORect: TRect);
begin
...
procedure TcxCustomProgressBarViewInfo.DrawBarBitmap(ACanvas: TcxCanvas;
  ARect: TRect);
begin
  ACanvas.SaveState;
  try
...
procedure TcxCustomProgressBarViewInfo.DrawGradientBar(ACanvas: TcxCanvas;
  const ANormalRect, AOverloadRect, ABarRect: TRect);
var
  ASize: TSize;
  R: TRect;
    ...
    begin
      if FOrientation = cxorHorizontal then
        ...
        begin
          ...
          if not Assigned(FNativeBitmap) then
            Exit;
          StretchBlt(Handle, ANormalRect.Left, ANormalRect.Top,
            cxRectWidth(ANormalRect), cxRectHeight(ANormalRect),
            FNativeBitmap.Canvas.Handle, 0, 0, FNativeBitmap.Width,
            FNativeBitmap.Height, SRCCOPY);
        end
      else
        with ANormalRect do
        begin
...
procedure TcxCustomProgressBarViewInfo.DrawSolidBar(ACanvas: TcxCanvas;
  const ANormalRect, AOverloadRect: TRect);
begin
  with ACanvas do
  begin
...
procedure TcxCustomProgressBarViewInfo.DrawAnimationBar(ACanvas: TcxCanvas;
  const ABar, ASolidRect: TRect);

  procedure LightCanvasByGradient(ACanvas: TcxCanvas; const ARect: TRect;
    AIsHorizontal: Boolean = True);
  var
    I: Integer;
    ABeginColor: TColor;
    AEndColor: TColor;
  ...
  begin
    ...
    begin
      ...
      for I := ARect.Top to (ARect.Top + ScaleFactor.Apply
        (cxAnimationBarTopPartWidth) - 1) do
      begin
        ABeginColor := ACanvas.Canvas.Pixels[ASolidRect.Left + 1, I];
        AEndColor := dxGetLighterColor(ABeginColor,
          cxAnimationBarColorLightPercentage);
        FillGradientRect(ACanvas.Handle, Rect(ARect.Left - AWidth, I,
          ARect.Left + AWidth - ScaleFactor.Apply(cxAnimationBarMiddlePartWidth)
          div 2, I + 1), ABeginColor, AEndColor, True);
        FillRectByColor(ACanvas.Handle,
          Rect(ARect.Left + AWidth - ScaleFactor.Apply
          (cxAnimationBarMiddlePartWidth) div 2, I, ARect.Left + AWidth +
          ScaleFactor.Apply(cxAnimationBarMiddlePartWidth) div 2, I + 1),
          AEndColor);
        FillGradientRect(ACanvas.Handle,
          Rect(ARect.Left + AWidth + ScaleFactor.Apply
          (cxAnimationBarMiddlePartWidth) div 2, I, ARect.Right + AWidth,
          I + 1), AEndColor, ABeginColor, True);
      end;
      ...
      try
        ...
        begin
          ...
          ADeltaPercentage := 0;
          if I < (AWidth + ARect.Left - ScaleFactor.Apply
            (cxAnimationBarMiddlePartWidth) div 2) then
            ADeltaPercentage := (100 - cxAnimationBarColorLightPercentage) *
              (AWidth + ARect.Left -
              (ScaleFactor.Apply(cxAnimationBarMiddlePartWidth) div 2) - I)
              div (AWidth - ScaleFactor.Apply
              (cxAnimationBarMiddlePartWidth) div 2)
          else if I > (AWidth + ARect.Left + ScaleFactor.Apply
            (cxAnimationBarMiddlePartWidth) div 2) then
            ADeltaPercentage :=
              (100 - ScaleFactor.Apply(cxAnimationBarColorLightPercentage)) *
              (I - AWidth - ARect.Left - ScaleFactor.Apply
              (cxAnimationBarMiddlePartWidth) div 2)
              div (AWidth - ScaleFactor.Apply
              (cxAnimationBarMiddlePartWidth) div 2);
          Inc(ACurrentPercentage, ADeltaPercentage);
          ABeginColor := ACanvas.Canvas.Pixels
            [I, ARect.Top + ScaleFactor.Apply(cxAnimationBarTopPartWidth)];
          AEndColor := dxGetLighterColor(ABeginColor, ACurrentPercentage);
          ACanvas.FillRect
            (Rect(I, ARect.Top + ScaleFactor.Apply(cxAnimationBarTopPartWidth),
            I + 1, ARect.Bottom), AEndColor);
        end;
      finally
    ...
    begin
      ...
      for I := ARect.Left to (ARect.Left + ScaleFactor.Apply
        (cxAnimationBarTopPartWidth) - 1) do
      begin
        ...
        FillGradientRect(ACanvas.Handle, Rect(I, ARect.Top - AWidth, I + 1,
          ARect.Top + AWidth - ScaleFactor.Apply(cxAnimationBarMiddlePartWidth)
          div 2), ABeginColor, AEndColor, False);
        ACanvas.FillRect(Rect(I, ARect.Top + AWidth -
          ScaleFactor.Apply(cxAnimationBarMiddlePartWidth) div 2, I + 1,
          ARect.Top + AWidth + ScaleFactor.Apply(cxAnimationBarMiddlePartWidth)
          div 2), AEndColor);
        FillGradientRect(ACanvas.Handle,
          Rect(I, ARect.Top + AWidth + ScaleFactor.Apply
          (cxAnimationBarMiddlePartWidth) div 2, I + 1, ARect.Bottom + AWidth),
          AEndColor, ABeginColor, False);
      end;
      ACanvas.SaveClipRegion;
      try
        ACanvas.SetClipRegion(TcxRegion.Create(Rect(ARect.Left, ARect.Top,
        ...
        begin
          ACurrentPercentage := cxAnimationBarColorLightPercentage;
          ADeltaPercentage := 0;
          if I < (AWidth + ARect.Top - ScaleFactor.Apply
            (cxAnimationBarMiddlePartWidth) div 2) then
            ADeltaPercentage := (100 - cxAnimationBarColorLightPercentage) *
              (AWidth + ARect.Top -
              (ScaleFactor.Apply(cxAnimationBarMiddlePartWidth) div 2) - I)
              div (AWidth - ScaleFactor.Apply
              (cxAnimationBarMiddlePartWidth) div 2)
          else if I > (AWidth + ARect.Top + ScaleFactor.Apply
            (cxAnimationBarMiddlePartWidth) div 2) then
            ADeltaPercentage := (100 - cxAnimationBarColorLightPercentage) *
              (I - AWidth - ARect.Top - ScaleFactor.Apply
              (cxAnimationBarMiddlePartWidth) div 2)
              div (AWidth - ScaleFactor.Apply
              (cxAnimationBarMiddlePartWidth) div 2);
          Inc(ACurrentPercentage, ADeltaPercentage);
          ABeginColor := ACanvas.Canvas.Pixels
            [ARect.Left + ScaleFactor.Apply(cxAnimationBarTopPartWidth), I];
          if (ABeginColor < 0) or (ABeginColor >= (1 shl 24)) then
            Continue;
          AEndColor := dxGetLighterColor(ABeginColor, ACurrentPercentage);
          ACanvas.FillRect
            (Rect(ARect.Left + ScaleFactor.Apply(cxAnimationBarTopPartWidth), I,
            ARect.Right, I + 1), AEndColor);
        end;
      finally
        ACanvas.RestoreClipRegion;
      end;
...
procedure TcxCustomProgressBarViewInfo.DrawAnimationBarBackground
  (ACanvas: TcxCanvas; const ASolidRect: TRect; ASolidColor: TColor;
  ADrawBar: Boolean);
var
...
begin
  ATopBorderBeginColor := dxGetLighterColor(ASolidColor,
    cxAnimationBarTopBeginColorLightPercentage);
  ATopBorderEndColor := dxGetLighterColor(ASolidColor,
    cxAnimationBarTopEndColorLightPercentage);
  ABorderBeginColor := dxGetDarkerColor(ASolidColor,
    cxAnimationBarBorderExtPartColorLightPercentage);
  ABorderEndColor := dxGetDarkerColor(ASolidColor,
  ...
  begin
    ...
    begin
      ...
      begin
        ABorderExtPathWidth := ABorderExtPathWidth * cxRectWidth(ASolidRect)
          div (3 * ABorderWidth);
        ABorderIntPathWidth := ABorderIntPathWidth * cxRectWidth(ASolidRect)
          div (3 * ABorderWidth);
      end;
      ...
      begin
        ...
        FillRectByGradient(Rect(Right - ABorderExtPathWidth, Top, Right,
          Bottom), ABorderEndColor, ABorderBeginColor, True);

        FillRectByGradient(Rect(Left + ABorderExtPathWidth, Top,
          Left + ABorderWidth, Bottom), ABorderEndColor, ASolidColor, True);
        FillRectByGradient(Rect(Right - ABorderWidth, Top,
          Right - ABorderExtPathWidth, Bottom), ASolidColor,
          ABorderEndColor, True);

        FillRectByGradient(Rect(Left, Top, Right,
          Top + ScaleFactor.Apply(cxAnimationBarTopPartWidth)),
    ...
    begin
      ...
      begin
        ABorderExtPathWidth := ABorderExtPathWidth * cxRectHeight(ASolidRect)
          div (3 * ABorderWidth);
        ABorderIntPathWidth := ABorderIntPathWidth * cxRectHeight(ASolidRect)
          div (3 * ABorderWidth);
      end;
      ABorderWidth := ABorderIntPathWidth + ABorderExtPathWidth;
      ...
      begin
        ...
        FillRectByGradient(Rect(Left, Bottom - ABorderExtPathWidth,
          ASolidRect.Right, Bottom), ABorderEndColor, ABorderBeginColor, False);

        FillRectByGradient(Rect(Left, Top + ABorderExtPathWidth, Right,
          Top + ABorderWidth), ABorderEndColor, ASolidColor, False);
        FillRectByGradient(Rect(Left, Bottom - ABorderWidth, Right,
          Bottom - ABorderExtPathWidth), ASolidColor, ABorderEndColor, False);

        FillRectByGradient(Rect(Left, Top,
          Left + ScaleFactor.Apply(cxAnimationBarTopPartWidth), Bottom),
          ATopBorderBeginColor, ATopBorderEndColor, True);
      end;
    end;
  end;
end;
...
procedure TcxCustomProgressBarViewInfo.DrawPeak(ACanvas: TcxCanvas;
  const APeakRect: TRect);
begin
...
begin
  ...
  begin
    ...
    begin
      PaintBarBevelOuter(ACanvas, Rect(ABarRect.Left + (I - 1) * ALEDsWidth,
        ABarRect.Top, ABarRect.Left + I * ALEDsWidth - 2, ABarRect.Bottom));
    end;
  ...
  begin
    ...
    for I := 1 to AMaxCount do
    begin
      PaintBarBevelOuter(ACanvas, Rect(ABarRect.Left, ABarRect.Bottom - (I - 1)
        * ALEDsWidth, ABarRect.Right, ABarRect.Bottom - I * ALEDsWidth + 2));
    end;
    if (ABarRect.Bottom - AMaxCount * ALEDsWidth) > ABarRect.Top then
      PaintBarBevelOuter(ACanvas, Rect(ABarRect.Left,
        ABarRect.Bottom - AMaxCount * ALEDsWidth, ABarRect.Right,
        ABarRect.Top));
  end;
end;
...
begin
  ...
  if FAnimationRestartDelay = 0 then
    Exit;
  FAnimationRestartDelayTimer := cxCreateTimer(DoAnimationRestartDelayTimer,
...
procedure TcxCustomProgressBarViewInfo.SetAnimationPath
  (AValue: TcxProgressBarAnimationPath);
begin
...
begin
  ...
  begin
    ...
    else if FAnimationSpeed > 0 then
      StartAnimationTimer;
  end;
end;
  ...
  case FAnimationPath of
    ...
    cxapPingPong:
      begin
        ...
        else if FAnimationPosition < 0 then
        begin
          SetAnimationFirstPosition;
          StartAnimationRestartDelayTimer;
        end;
...
begin
  ...
  if FOrientation = cxorHorizontal then
    ...
    AAnimationBarRect.Bottom := AAnimationBarRect.Top +
      GetAnimationBarDimension;
...
procedure TcxCustomProgressBarViewInfo.SetPeakSize
  (const Value: TcxNaturalNumber);
begin
...
procedure TcxCustomProgressBarViewData.CalculateViewInfoProperties
...
procedure TcxCustomProgressBarViewData.Calculate(ACanvas: TcxCanvas;
...
begin
  ...
  AProgressBarViewInfo.NativeStyle := AreVisualStylesMustBeUsed(NativeStyle,
    totProgress);
  ...
  begin
    ...
    ABarRect := ABounds;
    if IsInplace then
      ARect := cxRectTransform(ARect, (AViewInfo.BorderWidth + ADelta),
        (AViewInfo.BorderWidth + ADelta), -(AViewInfo.BorderWidth + ADelta),
        -(AViewInfo.BorderWidth + ADelta))
    else if not(Properties.BarStyle in [cxbsAnimation, cxbsAnimationLEDs]) then
      ARect := cxRectTransform(ARect, (AViewInfo.BorderWidth + ADelta + 1),
        (AViewInfo.BorderWidth + ADelta + 1), -(AViewInfo.BorderWidth + ADelta),
        -(AViewInfo.BorderWidth + ADelta))
    else
      ARect := cxRectTransform(ARect, (AViewInfo.BorderWidth + 1),
        (AViewInfo.BorderWidth + 1), -(AViewInfo.BorderWidth + 1),
        -(AViewInfo.BorderWidth + 1));
  ...
  begin
    ...
    ARect := cxRectInflate(AProgressBarViewInfo.BorderRect,
      -AViewInfo.BorderWidth, -AViewInfo.BorderWidth);
  ...
  begin
    if AProgressBarViewInfo.FOrientation = cxorHorizontal then
      AProgressBarRect.Right := ARect.Left +
        CalculateDelta(AProgressBarViewInfo.Position - AProgressBarViewInfo.Min,
        cxRectWidth(ARect), AProgressBarViewInfo.MaxMinDiff)
    else
      AProgressBarRect.Top := ARect.Bottom -
        CalculateDelta((AProgressBarViewInfo.Position -
        AProgressBarViewInfo.Min), cxRectHeight(ARect),
  ...
  begin
    ...
    begin
      AOverloadBarRect.Left := ARect.Left +
        CalculateDelta(AProgressBarViewInfo.RelativeOverloadValue,
        cxRectWidth(ARect), AProgressBarViewInfo.MaxMinDiff);
    ...
    begin
      AOverloadBarRect.Top := AOverloadBarRect.Bottom -
        CalculateDelta(AProgressBarViewInfo.RelativePosition,
        cxRectHeight(ARect), AProgressBarViewInfo.MaxMinDiff);
  ...
  AProgressBarViewInfo.FRealShowPeak := AProgressBarViewInfo.ShowPeak and
    not Properties.Marquee;
  ...
  begin
    ...
    begin
      APeakBarRect.Left := ARect.Left +
        CalculateDelta(AProgressBarViewInfo.RelativePeakValue,
        cxRectWidth(ARect), AProgressBarViewInfo.MaxMinDiff);
      APeakBarRect.Left := Math.Min(APeakBarRect.Left,
        ARect.Right - AProgressBarViewInfo.PeakSize);
    ...
    begin
      APeakBarRect.Bottom := ARect.Bottom -
        CalculateDelta(AProgressBarViewInfo.RelativePeakValue,
        cxRectHeight(ARect), AProgressBarViewInfo.MaxMinDiff);
      APeakBarRect.Bottom := Math.Max(APeakBarRect.Bottom,
        ARect.Top + AProgressBarViewInfo.PeakSize);
      APeakBarRect.Top := APeakBarRect.Bottom - AProgressBarViewInfo.PeakSize;
    end;
  end;
  if Properties.IsForegroundImageChanged or
    not IsGlyphAssigned(AProgressBarViewInfo.ForegroundImage) then
  begin
  ...
  if not IsInplace then
    AProgressBarViewInfo.DrawSelectionBar := False;
  ...
  if Properties.IsPositionChanged and not AIsMouseEvent and
    ...
    (not AProgressBarViewInfo.FRealShowOverload or
    cxRectIsEqual(AOverloadBarRect, AProgressBarViewInfo.OverloadBarRect)) and
    (not AProgressBarViewInfo.FRealShowPeak or cxRectIsEqual(APeakBarRect,
    AProgressBarViewInfo.PeakBarRect)) then
  begin
...
procedure TcxCustomProgressBarViewData.CalculateButtonsViewInfo
...
procedure TcxCustomProgressBarViewData.EditValueToDrawValue(const AEditValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
var
  ADisplayValue: TcxEditValue;
begin
  CalculateCustomProgressBarViewInfo(Self,
    AViewInfo as TcxCustomProgressBarViewInfo);
  if PreviewMode then
...
function TcxCustomProgressBarViewData.InternalGetEditConstantPartSize
...
begin
  ...
  try
    ...
    begin
      ...
      if not(IsInplace or AreVisualStylesMustBeUsed(AViewInfo.NativeStyle,
        totButton)) then
      begin
      ...
      ASize2 := GetTextEditContentSize(ACanvas, Self, AText,
        DrawTextFlagsTocxTextOutFlags(cxTextOutFlagsToDrawTextFlags
        (GetDrawTextFlags) and not(CXTO_CENTER_VERTICALLY or CXTO_BOTTOM) or
        CXTO_TOP), AEditSizeProperties, 0, False);
...
function TcxCustomProgressBarViewData.GetProperties
  : TcxCustomProgressBarProperties;
begin
...
class function TcxCustomProgressBarProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxProgressBar;
end;

function TcxCustomProgressBarProperties.GetDisplayText(const AEditValue
  : TcxEditValue; AFullText: Boolean = False;
  AIsInplace: Boolean = True): string;
begin
  Result := InternalGetDisplayText(AEditValue, Marquee, ShowTextStyle,
    AFullText, AIsInplace);
end;

function TcxCustomProgressBarProperties.GetSupportedOperations
  : TcxEditSupportedOperations;
begin
  Result := [esoAlwaysHotTrack, esoFiltering, esoSorting];
end;

class function TcxCustomProgressBarProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxCustomProgressBarViewInfo;
end;

function TcxCustomProgressBarProperties.IsEditValueValid(var EditValue
...
begin
  ...
  else if VarIsNumericEx(AEditValue) or VarIsDate(AEditValue) then
...
function TcxCustomProgressBarProperties.InternalGetDisplayText(const AEditValue
  : TcxEditValue; AMarquee: Boolean; AShowTextStyle: TcxProgressBarTextStyle;
...
procedure TcxCustomProgressBarProperties.DoAssign
...
class function TcxCustomProgressBarProperties.GetAssignedValuesClass
...
class function TcxCustomProgressBarProperties.GetViewDataClass
...
procedure TcxCustomProgressBarProperties.ForegroundImageChanged
  (Sender: TObject);
begin
...
function TcxCustomProgressBarProperties.GetAssignedValues
  : TcxProgressBarPropertiesValues;
begin
...
procedure TcxCustomProgressBarProperties.SetAnimationPath
  (AValue: TcxProgressBarAnimationPath);
begin
  if AValue <> FAnimationPath then
  begin
...
procedure TcxCustomProgressBarProperties.SetAnimationRestartDelay
  (AValue: Cardinal);
begin
...
procedure TcxCustomProgressBarProperties.SetAnimationSpeed
  (AValue: TcxProgressBarAnimationSpeed);
begin
...
procedure TcxCustomProgressBarProperties.SetBarBevelOuter
  (Value: TcxProgressBarBevelOuter);
begin
...
function TcxCustomProgressBarProperties.GetRealPeakValue
...
procedure TcxCustomProgressBarProperties.SetOrientation
  (Value: TcxProgressBarOrientation);
begin
...
procedure TcxCustomProgressBarProperties.SetShowTextStyle
  (Value: TcxProgressBarTextStyle);
begin
  if (FShowTextStyle <> Value) and
    (not Marquee or (Marquee and (Value = cxtsText))) then
  begin
...
procedure TcxCustomProgressBarProperties.SetTextOrientation
  (Value: TcxProgressBarOrientation);
begin
...
procedure TcxCustomProgressBarProperties.SetBarStyle
  (Value: TcxProgressBarBarStyle);
begin
...
class function TcxCustomProgressBar.GetPropertiesClass
...
procedure TcxCustomProgressBar.PopulateSizeProperties(var AEditSizeProperties
...
function TcxCustomProgressBar.InternalGetNotPublishedStyleValues
  : TcxEditStyleValues;
begin
...
procedure TcxCustomProgressBar.UnRegisterListener
  (AValue: TcxProgressBarListener);
begin
  if FListeners <> nil then
...
function TcxCustomProgressBar.GetActiveProperties
  : TcxCustomProgressBarProperties;
begin
...
GetRegisteredEditProperties.Register(TcxProgressBarProperties,
  scxSEditRepositoryProgressBarItem);
FilterEditsController.Register(TcxProgressBarProperties,
  TcxFilterProgressBarHelper);

finalization

FilterEditsController.Unregister(TcxProgressBarProperties,
  TcxFilterProgressBarHelper);
