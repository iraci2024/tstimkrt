...
type
  ...
  public
    ...
    property UseTimeFormatWhenUnfocused: Boolean
      read FUseTimeFormatWhenUnfocused write SetUseTimeFormatWhenUnfocused
      default True;
  end;
  ...
  public
    ...
    property ActiveProperties: TcxCustomTimeEditProperties
...
var
  cxTimeEditFormats: array [TcxTimeEditTimeFormat, Boolean, 0 .. 1]
    of string = ((('hh:nn:ss ampm', '00:00:00 LL;1;0'),
    ('hh:nn:ss', '00:00:00;1;0')), (('hh:nn ampm', '00:00 LL;1;0'),
    ('hh:nn', '00:00;1;0')), (('hh ampm', '00 LL;1;0'), ('hh', '00;1;0')));
...
type
  ...
  TcxTimeEditFormatListener = class(TcxInterfacedPersistent,
...
var
  cxTimeEditHalfDayIncrement, cxTimeEditOneHourIncrement,
    cxTimeEditOneMinIncrement, cxTimeEditOneSecIncrement: Double;
...
function IsCharValidForTimeEdit(ATimeEdit: TcxCustomMaskEdit; var AChar: Char;
  ...
  begin
    ...
    AFirstDifferenceIndex := ATimeZoneInfo.Start + GetFirstDifferenceIndex
      (ATimeSuffix1, ATimeSuffix2) - 1;
    ...
    else if APos > AFirstDifferenceIndex then
    begin
    ...
    begin
      ...
      else if EqualChars(AChar, ATimeSuffix2[APos - ATimeZoneInfo.Start + 1])
      then
  ...
  begin
    ...
    else if ADigit <= IntToStr(AMaxTimeZoneValue)[1] then
    begin
  ...
  begin
    ...
    GetTimeValueItemValueRange(AMinTimeZoneValue, AMaxTimeZoneValue);
    Result := (StrToInt(S) >= AMinTimeZoneValue) and
      (StrToInt(S) <= AMaxTimeZoneValue);
    if not Result then
...
begin
  if TcxCustomTimeEditProperties(Properties).GetEditingPlace(APos) = tzTimeSuffix
  then
    Result := #0
  else
    Result := '0';
...
function TcxCustomTimeEditProperties.IsDisplayValueValid(var DisplayValue
...
begin
  ...
  try
    ...
    DisplayValue := FormatDateTime(cxTimeEditFormats[TimeFormat,
      FUse24HourFormat, 0], ADateTime);
...
function TcxCustomTimeEditProperties.IsEditValueValid(var EditValue
  : TcxEditValue; AEditFocused: Boolean): Boolean;
begin
...
procedure TcxCustomTimeEditProperties.DoPrepareDisplayValue(const AEditValue
  ...
  begin
    ...
    if AEditFocused or (AValue = 0) or not ShowDate or (dxDateOf(AValue) = 0)
    then
      ADateFormat := ''
    else
...
begin
  if not VarIsDate(AEditValue) and not VarIsNumericEx(AEditValue) and
    not VarIsStr(AEditValue) and not VarIsNull(AEditValue) then
    raise EConvertError.CreateFmt
      (cxGetResourceString(@cxSEditTimeConvertError), [])
  else
    try
...
procedure TcxCustomTimeEditProperties.ValidateDisplayValue(var ADisplayValue
...
function TcxCustomTimeEditProperties.GetEditValueSource(AEditFocused: Boolean)
...
begin
  Result := FormatDateTime(cxTimeEditFormats[TimeFormat,
    FUse24HourFormat, 0], 0);
...
function TcxCustomTimeEditProperties.GetDisplayFormatOptions
  : TcxEditDisplayFormatOptions;
begin
  Result := [dfoSupports, dfoNoCurrencyValue];
end;

function TcxCustomTimeEditProperties.GetModeClass(AMaskKind: TcxEditMaskKind)
...
function TcxCustomTimeEditProperties.GetTimePartLength
  (AKind: TcxTimeEditZoneKind): Integer;
begin
  Result := 2;
end;

function TcxCustomTimeEditProperties.GetTimePartPos
  (AKind: TcxTimeEditZoneKind): Integer;
begin
...
function TcxCustomTimeEditProperties.PrepareValue(const AValue
...
function TcxCustomTimeEditProperties.GetEditingPlace(APos: Integer)
  : TcxTimeEditZoneKind;
var
...
begin
  ...
  if not((ATimeZoneChar = 'H') or (ATimeZoneChar = 'N') or (ATimeZoneChar = 'S'))
  then
  begin
    Inc(APos);
    if APos > Length(S) then
...
procedure TcxCustomTimeEditProperties.SetTimeFormat
  (Value: TcxTimeEditTimeFormat);
begin
...
class function TcxCustomTimeEdit.GetPropertiesClass
...
begin
  ...
  if not ActiveProperties.Use24HourFormat then
    AText := AText + GetCorrectTimeSuffix
      (Copy(ADisplayValue, ATimeStringLength + 1, Length(ADisplayValue) -
      ATimeStringLength));
...
begin
  ...
  case AEditingPlace of
    ...
    tzHour:
      Result := cxTimeEditOneHourIncrement * AMultiplier;
    tzMin:
      Result := cxTimeEditOneMinIncrement * AMultiplier;
    tzSec:
      Result := cxTimeEditOneSecIncrement * AMultiplier;
  end;
end;
...
begin
  ...
  if not ActiveProperties.Use24HourFormat and
    InternalCompareString(Copy(S, Length(S) - ATimeSuffixLength + 1,
    ATimeSuffixLength), dxFormatSettings.TimePMString, True) then
    Inc(AValue, 12 * 60 * 60);
  Result := AValue;
end;

function TcxCustomTimeEdit.IncrementValueToStr(const AValue
  : TcxEditValue): string;
var
  ATime: TDateTime;
begin
...
begin
  AValue := Value;
  AIsEditValueValid := not AValidateEditValue or
    ActiveProperties.IsEditValueValid(AValue, InternalFocused);
...
begin
  InternalEditValue := VarToDateTime(Abs(Frac(Value)) * cxSign(FSavedDate) +
    FSavedDate);
end;
...
class function TcxFilterTimeEditHelper.GetFilterDataType(AValueTypeClass
...
GetRegisteredEditProperties.Register(TcxTimeEditProperties,
  scxSEditRepositoryTimeItem);
FilterEditsController.Register(TcxTimeEditProperties, TcxFilterTimeEditHelper);

finalization

FilterEditsController.Unregister(TcxTimeEditProperties,
  TcxFilterTimeEditHelper);
