...
uses
  ...
  cxEdit, cxExtEditConsts, cxExtEditUtils, cxGraphics, cxHeader,
...
type
  ...
  private
    ...
    function CalcCellTextRect(AApproximateRect: TRect;
      AItemIndex, AColumnIndex: Integer): TRect;
    procedure DrawCellTextEx(var ARect: TRect;
      AFlags, AItemIndex, AColumnIndex: Integer);
  ...
  protected
    ...
    function CalcItemHeight(AIndex: Integer; AVScrollBarVisible: Boolean)
      : Integer; virtual;
    ...
    procedure SectionSortChangedHandler(Sender: TObject;
      const Section: TcxHeaderSection;
      const ASortOrder: TcxHeaderSortOrder); virtual;
...
uses
  Variants, Math, cxVariants, dxThemeConsts, dxThemeManager, dxUxTheme,
...
type
  ...
  public
    function GetComparedString(AIndex: Integer;
      ASection: TcxHeaderSection): string;
...
function TMCStringList.GetComparedString(AIndex: Integer;
  ASection: TcxHeaderSection): string;
begin
...
begin
  ...
  for I := AStartIndex to AEndIndex do
    Perform(LB_SETITEMHEIGHT, I, MCListBox.CalcItemHeight(I,
      FVScrollBarVisible));
end;
...
function TcxMCInnerListBoxContainer.IsMouseWheelHandleNeeded(Shift: TShiftState;
...
procedure TcxMCInnerListBoxContainer.Scroll(AScrollBarKind: TScrollBarKind;
  AScrollCode: TScrollCode; var AScrollPos: Integer);
begin
  inherited Scroll(AScrollBarKind, AScrollCode, AScrollPos);
  if Enabled then
    (InnerControl as TcxMCInnerListBox).Scroll(AScrollBarKind, AScrollCode,
      AScrollPos);
end;
...
begin
  ...
  if UseRightToLeftScrollBar then
    ...
    R := Rect(MCListBox.InnerHeader.Width, 0, ClientWidth,
...
begin
  ...
  FInnerListBoxContainer.Style.LookAndFeel.MasterLookAndFeel :=
    Style.LookAndFeel;
  CreateInnerListBox;
...
function TcxMCListBox.ItemAtPos(const APos: TPoint; AExisting: Boolean)
  : Integer;
var
  P: TPoint;
begin
...
begin
  if HeaderSections.FirstSortedSection <> nil then
    InnerHeader.OnSectionChangedSortOrder(FInnerHeader,
      HeaderSections.FirstSortedSection,
      HeaderSections.FirstSortedSection.SortOrder);
end;
...
begin
  ...
  if (AActiveControl <> Self) and CanFocus and (InnerListBox = AActiveControl)
  then
  begin
...
function TcxMCListBox.CalcCellTextRect(AApproximateRect: TRect;
  AItemIndex, AColumnIndex: Integer): TRect;
begin
  Result := AApproximateRect;
  if MultiLines then
    DrawCellTextEx(Result, DT_CALCRECT or DT_NOPREFIX or DT_VCENTER, AItemIndex,
      AColumnIndex);
end;

procedure TcxMCListBox.DrawCellTextEx(var ARect: TRect;
  AFlags, AItemIndex, AColumnIndex: Integer);
  ...
  begin
    Result := AStartFlag or SystemAlignmentsHorz
      [HeaderSections[AColumnIndex].Alignment] or WordWrapArray[MultiLines] or
      ShowEndEllipsisArray[ShowEndEllipsis];
...
begin
  ...
  DrawTextEx(InnerListBox.Canvas.Handle, PChar(AText), Length(AText), ARect,
    GetTextFlag(AFlags), @ADrawTextParams);
end;

procedure TcxMCListBox.DrawCellText(ARect: TRect;
  AItemIndex, AColumnIndex: Integer);
begin
  DrawCellTextEx(ARect, DT_NOPREFIX or DT_VCENTER, AItemIndex, AColumnIndex);
end;

function TcxMCListBox.GetCellRect(AItemIndex, AColumnIndex, ATop,
  ABottom: Integer; AVScrollBarVisible: Boolean): TRect;
var
  I: Integer;
  AHeaderSectionRect: TRect;
begin
  AHeaderSectionRect := GetHeaderSectionRect(AColumnIndex, AVScrollBarVisible);
  Result := Rect(AHeaderSectionRect.Left + 2, ATop, AHeaderSectionRect.Right -
    2, ABottom);
  if OverflowEmptyColumn and
    (HeaderSections.Items[AColumnIndex].Alignment = taLeftJustify) then
    for I := AColumnIndex + 1 to HeaderSections.Count - 1 do
      if GetTextPart(AItemIndex, I) = '' then
        if not UseRightToLeftAlignment then
          Result.Right := Result.Right +
            cxRectWidth(GetHeaderSectionRect(I, AVScrollBarVisible))
        else
...
function TcxMCListBox.GetCellTextRect(AItemIndex, AColumnIndex, ATop,
  ABottom: Integer; AVScrollBarVisible: Boolean): TRect;
begin
  Result := CalcCellTextRect(GetCellRect(AItemIndex, AColumnIndex, ATop,
    ABottom, AVScrollBarVisible), AItemIndex, AColumnIndex);
end;
...
procedure TcxMCListBox.MeasureItem(Control: TWinControl; Index: Integer;
  var Height: Integer);
begin
...
begin
  ...
  FInnerListBoxContainer.SetBounds(0, InnerHeader.Height,
    FInnerPanel.ClientWidth, FInnerPanel.ClientHeight - InnerHeader.Height);
end;

procedure TcxMCListBox.CalcHeaderSectionRects;

  procedure InternalCalcHeaderSectionRects(AHeaderWidth: Integer;
    out ASectionRects: TRects);
  var
  ...
  begin
    ...
    for I := 0 to InnerHeader.Sections.Count - 1 do
      ASectionRects[I] := InnerHeader.GetSectionRectBySectionWidths
        (AHeaderWidth, ASectionWidths, I);
  end;

begin
  InternalCalcHeaderSectionRects(FInnerPanel.ClientWidth,
    FInnerHeaderSectionRectsWithoutScrollbar);
  InternalCalcHeaderSectionRects(FInnerPanel.ClientWidth -
    FInnerListBoxContainer.GetVScrollBarDefaultAreaWidth,
    FInnerHeaderSectionRectsWithScrollbar);
end;

function TcxMCListBox.CalcItemHeight(AIndex: Integer;
  AVScrollBarVisible: Boolean): Integer;
var
  I, ATextHeight: Integer;
  ACalcRect: TRect;
begin
  Result := ItemHeight;
  ATextHeight := TdxTextMeasurer.TextLineHeight(InnerListBox.Font);
