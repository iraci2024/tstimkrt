...
{$ENDIF}
  ...
  TcxInnerTreeViewWndProcEvent = procedure(ATreeView: TTreeView;
    var Message: TMessage; var ADone: Boolean) of object;
  ...
  protected
    ...
    function DoInnerTreeViewWndProcHandler(var Message: TMessage)
      : Boolean; virtual;
    ...
    property OnInnerTreeViewWndProc: TcxInnerTreeViewWndProcEvent
      read FOnInnerTreeViewWndProc write FOnInnerTreeViewWndProc;
    //
  ...
  private
    ...
    procedure SetOnAdvancedCustomDrawItem
      (Value: TTVAdvancedCustomDrawItemEvent);
  ...
  protected
    ...
    property OnAddition: TTVExpandedEvent read GetOnAddition
      write SetOnAddition;
    property OnCancelEdit: TTVChangedEvent read GetOnCancelEdit
      write SetOnCancelEdit;
    ...
    property OnCustomDraw: TTVCustomDrawEvent read GetOnCustomDraw
      write SetOnCustomDraw;
    property OnCustomDrawItem: TTVCustomDrawItemEvent read GetOnCustomDrawItem
      write SetOnCustomDrawItem;
    property OnDeletion: TTVExpandedEvent read GetOnDeletion
      write SetOnDeletion;
    ...
    property OnCreateNodeClass: TTVCreateNodeClassEvent
      read GetOnCreateNodeClass write SetOnCreateNodeClass;
  public
    constructor Create(AOwner: TComponent); override;
...
function cxTreeViewGetHitNode(ATreeView: TcxTreeView; const ACursorPos: TPoint)
  : TTreeNode;
...
function TcxBaseInnerTreeView.DoMouseWheel(Shift: TShiftState;
...
procedure TcxBaseInnerTreeView.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
begin
  ...
  begin
    ...
    if FContainer.InnerControlMenuHandler(Message) or
      FContainer.DoInnerTreeViewWndProcHandler(Message) then
      Exit;
  ...
  case Message.Msg of
    WM_HSCROLL, WM_MOUSEWHEEL, WM_VSCROLL, WM_WINDOWPOSCHANGED, CM_WININICHANGE,
      TVM_ENSUREVISIBLE, TVM_EXPAND, TVM_INSERTITEM, TVM_SELECTITEM:
      FContainer.SetScrollBarsParameters;
  end;
end;

procedure TcxBaseInnerTreeView.HScrollHandler(Sender: TObject;
  ScrollCode: TScrollCode; var ScrollPos: Integer);
begin
  CallWindowProc(DefWndProc, Handle, WM_HSCROLL,
    Word(ScrollCode) + Word(ScrollPos) shl 16,
    FContainer.GetScrollBarHandle(sbHorizontal));
  ScrollPos := GetScrollPos(Handle, SB_HORZ);
end;

procedure TcxBaseInnerTreeView.VScrollHandler(Sender: TObject;
  ScrollCode: TScrollCode; var ScrollPos: Integer);
var
...
begin
  ...
  begin
    ...
    if FContainer.HScrollBar.Visible and (FContainer.HScrollBar.Control <> nil)
    then
      FContainer.HScrollBar.Control.Repaint;
    if FContainer.VScrollBar.Visible and (FContainer.VScrollBar.Control <> nil)
    then
...
procedure TcxBaseInnerTreeView.WMWindowPosChanged
...
begin
  ...
  then
    Params.ExStyle := (Params.ExStyle or WS_EX_LAYOUTRTL or
      WS_EX_NOINHERITLAYOUT) and not(WS_EX_LEFTSCROLLBAR or WS_EX_RIGHT or
      WS_EX_RTLREADING);
end;
...
procedure TcxCustomInnerTreeView.CreateWindowHandle(const Params
  : TCreateParams);
begin
...
function TcxTreeViewContainer.DoInnerTreeViewWndProcHandler
...
procedure TcxTreeViewContainer.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
function TcxTreeViewContainer.UseSystemRightToLeftLayoutForInnerControl
  : Boolean;
begin
  Result := True;
end;
...
function TcxTreeViewContainer.GetScrollBarBounds(const AScrollBarRect
  : TRect): TRect;
begin
  if UseRightToLeftScrollBar and not IsWinVistaOrLater then
  // #Ch WinXp TreeView RTL bug
  begin
...
function TcxCustomTreeView.CustomSort(SortProc: TTVCompare; Data: Longint;
  ARecurse: Boolean = True): Boolean;
begin
...
begin
  Result := InnerTreeView.GetNodeAt(X - InnerTreeView.Left,
    Y - InnerTreeView.Top);
end;
...
procedure TcxCustomTreeView.SetScrollBarsParameters(AIsScrolling
  : Boolean = False);
begin
...
procedure TcxCustomTreeView.Select(Node: TTreeNode;
...
procedure TcxCustomTreeView.Subselect(Node: TTreeNode;
...
function TcxCustomTreeView.GetOnAdvancedCustomDrawItem
...
procedure TcxCustomTreeView.SetOnAdvancedCustomDraw
  (Value: TTVAdvancedCustomDrawEvent);
begin
  InnerTreeView.OnAdvancedCustomDraw := Value;
end;

procedure TcxCustomTreeView.SetOnAdvancedCustomDrawItem
  (Value: TTVAdvancedCustomDrawItemEvent);
begin
...
procedure TcxCustomTreeView.SetOnCreateNodeClass
  (Value: TTVCreateNodeClassEvent);
begin
