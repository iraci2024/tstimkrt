...
uses
  ...
  cxClasses, cxControls, cxGraphics, cxGeometry, cxLookAndFeels,
...
type
  ...
  protected
    procedure AdjustAutoRange(AControl: TControl;
      var ARange, AAlignMargin: Integer);
  ...
  public
    constructor Create(AScrollBox: TcxCustomScrollBox;
      AKind: TScrollBarKind); virtual;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    //
    property Kind: TScrollBarKind read FKind;
  published
    property Increment: TScrollBarInc read FIncrement write SetIncrement
      default 8;
  ...
  protected
    ...
    function CreateScrollBarOptions(AKind: TScrollBarKind)
      : TcxScrollBoxScrollBarOptions; virtual;
    ...
    property SelectorHelper: TcxScrollBoxDesignSelectorHelper
      read FSelectorHelper;
  ...
  public
    ...
    property AutoScroll: Boolean read FAutoScroll write SetAutoScroll
      default True;
    property HorzScrollBar: TcxScrollBoxScrollBarOptions read FHorzScrollBar
      write SetHorzScrollBar;
    property LookAndFeel;
    property Transparent;
    property VertScrollBar: TcxScrollBoxScrollBarOptions read FVertScrollBar
...
constructor TcxScrollBoxScrollAnimationHelper.Create
  (AOwner: TcxScrollBoxScrollBarOptions);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
constructor TcxScrollBoxScrollBarOptions.Create(AScrollBox: TcxCustomScrollBox;
...
procedure TcxScrollBoxScrollBarOptions.AdjustAutoRange(AControl: TControl;
  var ARange, AAlignMargin: Integer);
  ...
  procedure AdjustHorizontalAutoRange(AControl: TControl;
    var ARange, AAlignMargin: Integer);
  begin
    if GetLTRAlign(AControl) = alRight then
      Inc(AAlignMargin, AControl.Width)
    else if (GetLTRAlign(AControl) = alLeft) or (AControl.Align = alNone) and
      IsLTRLeftAnchor(AControl) then
      if ScrollBox.UseRightToLeftAlignment then
  ...
  procedure AdjustVerticalAutoRange(AControl: TControl;
    var ARange, AAlignMargin: Integer);
  begin
    if AControl.Align = alBottom then
      Inc(AAlignMargin, AControl.Height)
    else if (AControl.Align = alTop) or (AControl.Align = alNone) and
      (AControl.Anchors * [akTop, akBottom] = [akTop]) then
...
procedure TcxScrollBoxScrollBarOptions.Scroll(ACode: TScrollCode;
...
begin
  ...
  if ScrollBox.IsScrollAnimationEnabled and (ACode in [scLineUp .. scPageDown])
  then
    FScrollAnimationHelper.SetPosition(APosition)
  else if Tracking or ScrollBox.IsGestureScrolling or (ACode <> scTrack) then
    Position := APosition;
  APosition := Position;
end;
...
procedure TcxCustomScrollBoxViewInfo.DrawBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
...
function TcxCustomScrollBoxViewInfo.DoCalculateClientRect
...
begin
  Result := TdxControlsDesignSelectorHelper.CalculateBounds(ClientRect,
    ScaleFactor);
end;
...
function TcxScrollBoxDesignSelectorHelper.IsHitTestTransparent
...
begin
  ...
  ARect.BottomRight := ScreenToClient
    (AControl.ClientToScreen(ARect.BottomRight));
...
procedure TcxCustomScrollBox.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
procedure TcxCustomScrollBox.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TcxCustomScrollBox.Scroll(AScrollBarKind: TScrollBarKind;
...
begin
  ...
  begin
    AControl := Controls[I];
    if not(AControl is TWinControl) or not TWinControl(AControl).HandleAllocated
    then
    begin
      with AControl do
        SetBounds(Left + ADeltaX, Top + ADeltaY, Width, Height);
    end
    else if not AIsVisible then
      with TWinControl(AControl) do
        SetWindowPos(Handle, 0, Left + ADeltaX, Top + ADeltaY, Width, Height,
...
begin
  ...
  case Message.Msg of
    WM_WINDOWPOSCHANGING:
      if Transparent then
        PWindowPos(Message.LParam).flags := PWindowPos(Message.LParam).flags or
          SWP_NOCOPYBITS;
  end;
end;
...
function TcxCustomScrollBox.CreateScrollBarOptions(AKind: TScrollBarKind)
  : TcxScrollBoxScrollBarOptions;
begin
...
procedure TcxCustomScrollBox.DrawOpaqueBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
...
begin
  ...
  try
    ...
    TcxControlCustomScrollBarsAccess(MainScrollBars).CheckSizeGripVisible
      (IsSizeGripVisible);
...
begin
  ...
  Result := Rect(ABorderSize, ABorderSize,
    ABorderSize + IfThen(not IsTouchScrollUIMode and VertScrollBar.IsVisible,
    GetVScrollBarAreaWidth), ABorderSize + IfThen(not IsTouchScrollUIMode and
    HorzScrollBar.IsVisible, GetHScrollBarAreaHeight));
  if UseRightToLeftScrollBar then
    Result := TdxRightToLeftLayoutConverter.ConvertOffsets(Result);
end;

function TcxCustomScrollBox.GetMainScrollBarsClass
...
function TcxCustomScrollBox.GetMouseWheelScrollingKind
...
begin
  SetScrollBarInfo(sbHorizontal, 0, HorzScrollBar.Range - 1,
    HorzScrollBar.Increment, HorzScrollBar.PageSize, HorzScrollBar.Position,
    True, True);
  SetScrollBarInfo(sbVertical, 0, VertScrollBar.Range - 1,
    VertScrollBar.Increment, VertScrollBar.PageSize, VertScrollBar.Position,
    True, True);
end;
...
procedure TcxCustomScrollBox.SetHorzScrollBar
  (Value: TcxScrollBoxScrollBarOptions);
begin
  HorzScrollBar.Assign(Value);
end;

procedure TcxCustomScrollBox.SetVertScrollBar
  (Value: TcxScrollBoxScrollBarOptions);
begin
...
begin
  ...
  if (AControl <> nil) and AControl.HandleAllocated and
    IsChildEx(Handle, AControl.Handle) then
...
begin
  ...
  if not IsDesigning and (APrevUseRightToLeftAlignment <>
    UseRightToLeftAlignment) then
    for I := 0 to ControlCount - 1 do
    begin
      if Controls[I].Align = alRight then
        Controls[I].Align := alLeft
      else if Controls[I].Align = alLeft then
      ...
      begin
        ...
        else if Controls[I].Anchors * [akLeft, akRight] = [akRight] then
...
begin
  ...
  R := cxRectOffset(ViewInfo.ClientRect, Message.CalcSize_Params^.rgrc
    [0].TopLeft);
...
begin
  if not IsTouchScrollUIMode and
    IsScrollBarsArea(ScreenToClient(Point(Message.XPos, Message.YPos))) then
