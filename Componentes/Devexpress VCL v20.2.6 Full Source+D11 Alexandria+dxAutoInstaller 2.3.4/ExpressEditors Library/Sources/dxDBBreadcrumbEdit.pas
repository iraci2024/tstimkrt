...
type
  ...
  public
    constructor Create(AOwner: TPersistent; AControl: IdxDBBreadCrumbEdit);
      reintroduce; virtual;
  ...
  published
    ...
    property ParentKeyField: string read FParentKeyFieldName
  ...
  public
    ...
    //
    property Items[AIndex: Integer]: TdxDBBreadcrumbEditNode
  ...
  protected
    ...
    function FindLoadedNode(AList: TList; const AParentKeyValue: Variant;
      out ANode: TdxDBBreadcrumbEditNode): Boolean;
  ...
  TdxDBBreadcrumbEditControllerState = (dxdbbecsSynchronizingSelection,
    dxdbbecsUpdatingRecord, dxdbbecsLocating);
  TdxDBBreadcrumbEditControllerStates = set of
    TdxDBBreadcrumbEditControllerState;
  ...
  public
    function FindNodeByKeyValue(const AKeyValue: Variant)
      : TdxDBBreadcrumbEditNode; overload;
    function FindNodeByKeyValue(const AKeyValue: Variant;
      out ANode: TdxDBBreadcrumbEditNode): Boolean; overload; virtual;
    function ProcessParentKeyValueChanges(const ANewParentKeyValue: Variant)
      : Boolean; virtual;
  ...
  TdxCustomDBBreadCrumbEdit = class(TdxCustomBreadcrumbEdit,
    IdxDBBreadCrumbEdit)
  private
  ...
  public
    ...
    property Properties: TdxDBBreadcrumbEditProperties read GetDBProperties
      write SetDBProperties;
    property Root: TdxDBBreadcrumbEditNode read GetDBRoot;
    property RootValue: Variant read GetRootValue write SetRootValue;
    property Selected: TdxDBBreadcrumbEditNode read GetSelected
      write SetSelected;
  end;
...
function cxCompareByParentKeyValue(ANode1,
  ANode2: TdxDBBreadcrumbEditNode): Integer;
begin
...
constructor TdxDBBreadcrumbEditDataBinding.Create(AOwner: TPersistent;
  AControl: IdxDBBreadCrumbEdit);
begin
...
function TdxDBBreadcrumbEditDataBinding.CreateDataLink
  : TdxDBBreadcrumbEditDataLink;
begin
  Result := TdxDBBreadcrumbEditDataLink.Create(Self);
end;

procedure TdxDBBreadcrumbEditDataBinding.DoSetFieldValue(const AValue: string;
  var AFieldValue: string);
begin
...
begin
  Result := (FieldKey <> nil) and (FieldParentKey <> nil) and
    (FieldName <> nil);
end;

function TdxDBBreadcrumbEditDataBinding.GetFieldByName(const AFieldName
  : string): TField;
begin
  if DataSet <> nil then
...
procedure TdxDBBreadcrumbEditDataBinding.SetImageIndexFieldName
  (const AValue: string);
begin
...
procedure TdxDBBreadcrumbEditDataBinding.SetParentKeyFieldName
  (const AValue: string);
begin
...
function TdxDBBreadcrumbEditDataLoader.FindLoadedNode(AList: TList;
  const AParentKeyValue: Variant; out ANode: TdxDBBreadcrumbEditNode): Boolean;
var
  L, H, I, C: Integer;
begin
  L := 0;
  ...
  begin
    I := (L + H) shr 1;
    C := VarCompare(TdxDBBreadcrumbEditNode(AList[I]).ParentKeyValue,
      AParentKeyValue);
    if C < 0 then
      L := I + 1
    else
    begin
      ...
      if C = 0 then
      begin
        while (I > 0) and VarEquals(TdxDBBreadcrumbEditNode(AList[I - 1])
          .ParentKeyValue, AParentKeyValue) do
          Dec(I);
...
procedure TdxDBBreadcrumbEditDataLoader.LoadNodeData
  (ANode: TdxDBBreadcrumbEditNode);
begin
...
begin
  ...
  try
    RootNode.Clear;
    if (DataSet <> nil) and DataSet.Active and DataBinding.IsRequeredFieldsAssigned
    then
    begin
...
function TdxDBBreadcrumbEditDataLoader.GetDataBinding
  : TdxDBBreadcrumbEditDataBinding;
begin
...
function TdxDBBreadcrumbEditDataLoader.GetImageIndexAsInteger
...
function TdxDBBreadcrumbEditController.CanSelectNode
  (ANode: TdxBreadcrumbEditNode): Boolean;
begin
  Result := (ANode = nil) or inherited CanSelectNode(ANode);
end;

function TdxDBBreadcrumbEditController.FindNodeByKeyValue(const AKeyValue
  : Variant; out ANode: TdxDBBreadcrumbEditNode): Boolean;
...
function TdxDBBreadcrumbEditController.FindNodeByKeyValue(const AKeyValue
  : Variant): TdxDBBreadcrumbEditNode;
begin
...
function TdxDBBreadcrumbEditController.GetDataBinding
  : TdxDBBreadcrumbEditDataBinding;
begin
  Result := Control.GetDataBinding;
end;
...
function TdxDBBreadcrumbEditNode.GetItem(AIndex: Integer)
  : TdxDBBreadcrumbEditNode;
begin
...
function TdxCustomDBBreadCrumbEdit.CreateController
  : TdxBreadcrumbEditController;
begin
  Result := TdxDBBreadcrumbEditController.Create(ViewInfo);
end;

function TdxCustomDBBreadCrumbEdit.CreateDataBinding
  : TdxDBBreadcrumbEditDataBinding;
begin
  Result := TdxDBBreadcrumbEditDataBinding.Create(Self, Self);
end;

function TdxCustomDBBreadCrumbEdit.CreateDataLoader
  : TdxDBBreadcrumbEditDataLoader;
begin
  Result := TdxDBBreadcrumbEditDataLoader.Create(Self);
end;

function TdxCustomDBBreadCrumbEdit.CreateProperties
...
function TdxCustomDBBreadCrumbEdit.GetDBController
  : TdxDBBreadcrumbEditController;
begin
  Result := TdxDBBreadcrumbEditController(inherited Controller);
end;

function TdxCustomDBBreadCrumbEdit.GetDataBinding
  : TdxDBBreadcrumbEditDataBinding;
begin
  Result := FDataBinding;
end;
...
function TdxCustomDBBreadCrumbEdit.GetDBProperties
  : TdxDBBreadcrumbEditProperties;
begin
...
procedure TdxCustomDBBreadCrumbEdit.SetDataBinding
  (AValue: TdxDBBreadcrumbEditDataBinding);
begin
  FDataBinding.Assign(AValue);
end;

procedure TdxCustomDBBreadCrumbEdit.SetDBProperties
  (AValue: TdxDBBreadcrumbEditProperties);
begin
...
procedure TdxCustomDBBreadCrumbEdit.SetSelected
  (AValue: TdxDBBreadcrumbEditNode);
begin
