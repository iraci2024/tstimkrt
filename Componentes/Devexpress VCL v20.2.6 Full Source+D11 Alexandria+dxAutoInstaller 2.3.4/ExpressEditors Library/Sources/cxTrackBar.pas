...
type
  ...
  TcxTrackBarGetCustomTextEvent = procedure(Sender: TObject;
    const APosition: Integer; var AText: string) of object;
  TcxTrackBarGetCustomHintEvent = procedure(Sender: TObject;
    const APosition: Integer; var AHintText: string;
    var ACanShow, AIsHintMultiLine: Boolean) of object;
  ...
  TcxTrackBarRightButtonFadingHelper = class
    (TcxTrackBarCustomButtonFadingHelper)
  protected
    procedure DrawButton(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState); override;
  ...
  protected
    ...
    procedure CalculateCustomThumbSize(ACustomRect: TRect;
      out AThumbSize, AThumbLargeSize: Integer); virtual;
    procedure DrawTickAsLineBottomRight(ACanvas: TcxCanvas;
      ATickPosition, ATickSize: Integer); virtual;
    procedure DrawTickAsLineTopLeft(ACanvas: TcxCanvas;
      ATickPosition, ATickSize: Integer); virtual;
    function GetClientRect: TRect; virtual;
    function GetLeftButtonRect: TRect; virtual;
    function GetPositionHintTextRect(ACanvas: TcxCanvas;
      const AHintText: string; AIsHintMultiLine: Boolean): TRect; virtual;
    function GetPositionHintSize(ACanvas: TcxCanvas; const AHintText: string)
      : Integer; virtual;
    function GetRightButtonRect: TRect; virtual;
    function GetSelectionRect: TRect; virtual;
    function GetTextTopRect(ATickPosition, ATextWidth, ATextHeight,
      ADelta: Integer): TRect; virtual;
    function GetTextBottomRect(ATickPosition, ATextWidth, ATextHeight,
      ADelta: Integer): TRect; virtual;
  ...
  protected
    ...
    procedure CalculateCustomThumbSize(ACustomRect: TRect;
      out AThumbSize, AThumbLargeSize: Integer); override;
    procedure DrawTickAsLineBottomRight(ACanvas: TcxCanvas;
      ATickPosition, ATickSize: Integer); override;
    procedure DrawTickAsLineTopLeft(ACanvas: TcxCanvas;
      ATickPosition, ATickSize: Integer); override;
    function GetPositionHintSize(ACanvas: TcxCanvas; const AHintText: string)
      : Integer; override;
  ...
  protected
    ...
    procedure DoGetPositionHint(var AHintText: string; var ACanShow: Boolean;
      var AIsHintMultiLine: Boolean); virtual;

    procedure DrawButtons(ACanvas: TcxCanvas);
    procedure DrawButtonLeft(ACanvas: TcxCanvas); overload;
    procedure DrawButtonLeft(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState); overload;
    procedure DrawButtonRight(ACanvas: TcxCanvas); overload;
    procedure DrawButtonRight(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState); overload;
    ...
    procedure DrawTickAsLine(ACanvas: TcxCanvas;
      AIndex, ATickPosition: Integer);
    procedure DrawTickAsText(ACanvas: TcxCanvas;
      AIndex, ATickPosition: Integer);
    procedure DrawTicks(ACanvas: TcxCanvas); virtual;
    procedure DrawThumb(ACanvas: TcxCanvas); overload; virtual;
    procedure DrawThumb(ACanvas: TcxCanvas; const R: TRect; AState: Integer);
      overload; virtual;
    function DrawingThumbRectToRealThumbRect(ACanvas: TcxCanvas;
      const AThumbRect: TRect): TRect; overload; virtual;
    function DrawingThumbRectToRealThumbRect(ACanvas: TcxCanvas): TRect;
      overload; virtual;
    ...
    function IsPtToTheRightFromThumb(AThumb: TcxTrackBarThumbViewInfo;
      X, Y: Integer): Boolean; virtual;
    function IsPtToTheLeftFromThumb(AThumb: TcxTrackBarThumbViewInfo;
      X, Y: Integer): Boolean; virtual;
    ...
    function GetPositionHintText(var ACanShow: Boolean;
      var AIsHintMultiLine: Boolean): string;
    ...
    procedure CalculateCustomThumbSize(ACustomRect: TRect;
      out AThumbSize, AThumbLargeSize: Integer);
    procedure DrawTickAsLineBottomRight(ACanvas: TcxCanvas;
      ATickPosition, ATickSize: Integer);
    procedure DrawTickAsLineTopLeft(ACanvas: TcxCanvas;
      ATickPosition, ATickSize: Integer);
    ...
    function GetThumbFromPos(const APoint: TPoint)
      : TcxTrackBarThumbViewInfo; virtual;

    procedure PaintTrackBar(ACanvas: TcxCanvas); virtual;
    //
    property ShowSelection: Boolean read FShowSelection write FShowSelection;
    property ThumbFadingHelper: TcxTrackBarThumbFadingHelper
      read GetThumbFadingHelper;
  ...
  public
    ...
    property ButtonLeftFadingHelper: TcxTrackBarLeftButtonFadingHelper
      read FButtonLeftFadingHelper; // for internal use
    property ButtonLeftState: TcxButtonState read FButtonLeftState
      write SetButtonLeftState;
    property ButtonRightFadingHelper: TcxTrackBarRightButtonFadingHelper
      read FButtonRightFadingHelper; // for internal use
    property ButtonRightState: TcxButtonState read FButtonRightState
      write SetButtonRightState;
    property LeftButtonSize: TSize read GetLeftButtonSize;
    property OrientationHelper: TcxHorizontalTrackBarViewInfoHelper
      read FOrientationHelper; // for internal use
    property Position: Integer read GetPosition write SetPosition;
    property PositionHintHelper: TcxTrackBarPositionHintHelper
      read FPositionHintHelper; // for internal use
    ...
    property TrackBarProperties: TcxCustomTrackBarProperties
      read GetTrackBarProperties;
    property TrackBarState: Integer read GetTrackBarState
      write SetTrackBarState;
    ...
    // for internal use
    property PressedThumb: TcxTrackBarThumbViewInfo read FPressedThumb
      write FPressedThumb; // for internal use
  end;
  ...
  private
    procedure GetCustomRealTrackZoneSize(AViewInfo: TcxCustomTrackBarViewInfo;
  ...
  protected
    ...
    function CalculateTickSize(ACanvas: TcxCanvas;
      AViewInfo: TcxCustomTrackBarViewInfo; ALeftTop: Boolean)
      : Integer; virtual;
    procedure CalculateTrackBarViewInfoProperties
      (AViewInfo: TcxCustomEditViewInfo); virtual;
    function ThumbHalfSize(AViewInfo: TcxCustomTrackBarViewInfo): Integer;
    procedure CalculateButtonLeftRect(AViewInfo
      : TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateButtonRightRect(AViewInfo
      : TcxCustomTrackBarViewInfo); virtual;
    procedure CalculatePixelsPerPositionStep
      (AViewInfo: TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateSelectionRect(AViewInfo
      : TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateThumbSize(AViewInfo: TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateThumbRect(ACanvas: TcxCanvas;
      AViewInfo: TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateTrackBarRect(AViewInfo
      : TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateTrackRect(AViewInfo: TcxCustomTrackBarViewInfo); virtual;
    procedure CalculateTrackSize(AViewInfo: TcxCustomTrackBarViewInfo;
      ARealTrackZoneSize: Integer); virtual;
    ...
    procedure UpdateValue(AViewInfo: TcxCustomTrackBarViewInfo;
      AValue: TcxEditValue); virtual;
  public
  ...
  private
    ...
    procedure DoDrawThumb(Sender: TObject; ACanvas: TcxCanvas;
      const ARect: TRect);
  protected
    FMin: Integer;
    FMax: Integer;
  ...
  public
    ...
    property BorderWidth: Integer read FBorderWidth write SetBorderWidth
      default 0;
    property ReverseDirection: Boolean read FReverseDirection
      write SetReverseDirection default False;
    ...
    property ShowChangeButtons: Boolean read FShowChangeButtons
      write SetShowChangeButtons default False;
    property ShowPositionHint: Boolean read FShowPositionHint
      write SetShowPositionHint default False;
    ...
    property OnGetPositionHint: TcxTrackBarGetCustomHintEvent
      read FOnGetPositionHint write FOnGetPositionHint;
  ...
  protected
    ...
    procedure UpdatePos(AValue: Integer;
      AThumb: TcxTrackBarThumbViewInfo); virtual;
  ...
  TcxCustomTrackBar = class(TcxCustomEdit, IcxTrackBar, IcxTrackBarPostProvider)
  private
...
const
  TrackBarTicksToTicksAlign: array [TcxTrackBarTickMarks]
    of TcxTrackBarTicksAlign = (tbtaBoth, tbtaUp, tbtaDown);
  TrackBarStateToButtonStates: array [1 .. 5] of TcxButtonState = (cxbsNormal,
    cxbsHot, cxbsPressed, cxbsHot, cxbsDisabled);
...
procedure CalculateCustomTrackBarViewInfo(ACanvas: TcxCanvas;
  AViewData: TcxCustomTrackBarViewData; AViewInfo: TcxCustomTrackBarViewInfo);
...
constructor TcxHorizontalTrackBarViewInfoHelper.Create
  (AViewInfo: TcxCustomTrackBarViewInfo);
begin
  inherited Create;
  FViewInfo := AViewInfo;
...
function TcxHorizontalTrackBarViewInfoHelper.GetCurrentMousePos(X,
  Y: Integer): Integer;
begin
  Result := X;
end;
...
function TcxHorizontalTrackBarViewInfoHelper.GetMouseDownPos
  (AMouseDownCoordinate: TPoint): Integer;
begin
  Result := AMouseDownCoordinate.X;
end;

procedure TcxHorizontalTrackBarViewInfoHelper.CalculateCustomThumbSize
  (ACustomRect: TRect; out AThumbSize, AThumbLargeSize: Integer);
begin
  AThumbSize := cxRectWidth(ACustomRect);
  AThumbLargeSize := cxRectHeight(ACustomRect);
end;

procedure TcxHorizontalTrackBarViewInfoHelper.DrawTickAsLineBottomRight
  (ACanvas: TcxCanvas; ATickPosition, ATickSize: Integer);
begin
  ACanvas.Line(ATickPosition, GetThumbRect.Bottom + TrackAndTickIndent + 1,
    ATickPosition, GetThumbRect.Bottom + TrackAndTickIndent + ATickSize + 1);
end;

procedure TcxHorizontalTrackBarViewInfoHelper.DrawTickAsLineTopLeft
  (ACanvas: TcxCanvas; ATickPosition, ATickSize: Integer);
begin
  ACanvas.Line(ATickPosition, GetThumbRect.Top - TrackAndTickIndent - ATickSize,
    ATickPosition, GetThumbRect.Top - TrackAndTickIndent);
end;

function TcxHorizontalTrackBarViewInfoHelper.GetClientRect: TRect;
begin
  Result := GetRealRect(cxRectOffset(ViewInfo.ClientRect, -ViewInfo.Bounds.Left,
    -ViewInfo.Bounds.Top));
end;
...
function TcxHorizontalTrackBarViewInfoHelper.GetPositionHintTextRect
  (ACanvas: TcxCanvas; const AHintText: string;
...
begin
  ...
  else if ViewInfo.PressedThumb <> nil then
  ...
  if ViewInfo.TickMarks = cxtmTopLeft then
    ...
    Result.Top := R.Top - PositionHintIndent - GetPositionHintSize(ACanvas,
      AHintText);
...
function TcxHorizontalTrackBarViewInfoHelper.GetPositionHintSize
  (ACanvas: TcxCanvas; const AHintText: string): Integer;
begin
...
function TcxHorizontalTrackBarViewInfoHelper.GetTextTopRect(ATickPosition,
  ATextWidth, ATextHeight, ADelta: Integer): TRect;
begin
  Result := cxRectBounds(ATickPosition - ATextWidth div 2,
    GetThumbRect.Top - ADelta - TrackAndTickIndent - ATextHeight, ATextWidth,
    ATextHeight);
end;

function TcxHorizontalTrackBarViewInfoHelper.GetTextBottomRect(ATickPosition,
  ATextWidth, ATextHeight, ADelta: Integer): TRect;
begin
  Result := cxRectBounds(ATickPosition - ATextWidth div 2,
    ViewInfo.ThumbRect.Bottom + TrackAndTickIndent + ADelta, ATextWidth,
    ATextHeight);
end;
...
procedure TcxHorizontalTrackBarViewInfoHelper.InflateTrackRect
  (var ARect: TRect);
begin
  Inc(ARect.Right);
end;
...
function TcxVerticalTrackBarViewInfoHelper.GetCurrentMousePos(X,
  Y: Integer): Integer;
begin
  Result := Y;
end;

function TcxVerticalTrackBarViewInfoHelper.GetMouseDownPos
  (AMouseDownPos: TPoint): Integer;
begin
  Result := AMouseDownPos.Y;
end;

procedure TcxVerticalTrackBarViewInfoHelper.CalculateCustomThumbSize
  (ACustomRect: TRect; out AThumbSize, AThumbLargeSize: Integer);
begin
...
procedure TcxVerticalTrackBarViewInfoHelper.DrawTickAsLineBottomRight
  (ACanvas: TcxCanvas; ATickPosition, ATickSize: Integer);
begin
  ACanvas.Line(GetThumbRect.Right + TrackAndTickIndent + 1, ATickPosition,
    GetThumbRect.Right + TrackAndTickIndent + ATickSize + 1, ATickPosition);
end;

procedure TcxVerticalTrackBarViewInfoHelper.DrawTickAsLineTopLeft
  (ACanvas: TcxCanvas; ATickPosition, ATickSize: Integer);
begin
  ACanvas.Line(GetThumbRect.Left - (TrackAndTickIndent + ATickSize),
    ATickPosition, GetThumbRect.Left - TrackAndTickIndent, ATickPosition);
end;

function TcxVerticalTrackBarViewInfoHelper.GetPositionHintSize
  (ACanvas: TcxCanvas; const AHintText: string): Integer;
begin
...
function TcxVerticalTrackBarViewInfoHelper.GetTextTopRect(ATickPosition,
  ATextWidth, ATextHeight, ADelta: Integer): TRect;
begin
  Result := cxRectBounds(GetThumbRect.Left - TrackAndTickIndent - ATextWidth -
    ADelta, ATickPosition - ATextHeight div 2, ATextWidth, ATextHeight);
end;

function TcxVerticalTrackBarViewInfoHelper.GetTextBottomRect(ATickPosition,
  ATextWidth, ATextHeight, ADelta: Integer): TRect;
begin
  Result := cxRectBounds(GetThumbRect.Right + ADelta + TrackAndTickIndent,
    ATickPosition - ATextHeight div 2, ATextWidth, ATextHeight);
end;
  ...
  begin
    ...
    begin
      if IsInplace or (csPaintCopy in Edit.ControlState) then
        cxBitBlt(FTBBitmap.cxCanvas.Handle, ACanvas.Handle,
          FTBBitmap.ClientRect, Bounds.TopLeft, SRCCOPY)
      else
        cxDrawTransparentControlBackground(Edit, FTBBitmap.cxCanvas, Bounds);
    end
    else if IsInplace then
    begin
    ...
    else if (NativeStyle or UseSkins) and not IsDesigning then
      cxDrawTransparentControlBackground(Edit, FTBBitmap.cxCanvas, Bounds)
    else
...
begin
  ...
  cxBitBlt(ACanvas.Handle, FTBBitmap.cxCanvas.Handle, Bounds,
    cxNullPoint, SRCCOPY);
...
procedure TcxCustomTrackBarViewInfo.AddMinorTicks(AMin, AMax,
  AFrequency: Integer);
var
  I: Integer;
begin
  ...
  begin
    MinorTicks.Capacity := MinorTicks.Capacity + (AMax - AMin)
      div AFrequency + 1;
...
procedure TcxCustomTrackBarViewInfo.DoGetPositionHint(var AHintText: string;
  var ACanShow: Boolean; var AIsHintMultiLine: Boolean);
begin
  if Assigned(TrackBarProperties.OnGetPositionHint) then
    TrackBarProperties.OnGetPositionHint(Edit, Position, AHintText, ACanShow,
      AIsHintMultiLine);
end;

procedure TcxCustomTrackBarViewInfo.DrawButtonLeft(ACanvas: TcxCanvas);
begin
  if not ButtonLeftFadingHelper.DrawImage(ACanvas.Handle, GetLeftButtonRect)
  then
...
begin
  if ReverseDirection then
    ...
    LookAndFeel.Painter.DrawScaledZoomOutButton(ACanvas, R, AState,
      ScaleFactor);
end;

procedure TcxCustomTrackBarViewInfo.DrawButtonRight(ACanvas: TcxCanvas);
begin
  if not ButtonRightFadingHelper.DrawImage(ACanvas.Handle, GetRightButtonRect)
  then
...
begin
  ...
  LookAndFeel.Painter.DrawTrackBarScaledTrack(ACanvas, ARect, GetSelectionRect,
    FShowSelection, Enabled, Orientation = tboHorizontal,
    TrackBarProperties.TrackColor, ScaleFactor);
end;

procedure TcxCustomTrackBarViewInfo.DrawSelection(ACanvas: TcxCanvas);
begin
  if not UseSkins then
    cxEditFillRect(ACanvas, GetSelectionRect,
      TrackBarProperties.SelectionColor);
end;

procedure TcxCustomTrackBarViewInfo.DrawTickAsLine(ACanvas: TcxCanvas;
  AIndex, ATickPosition: Integer);
var
  ATickSize: Integer;
begin
...
procedure TcxCustomTrackBarViewInfo.DrawTickAsText(ACanvas: TcxCanvas;
  AIndex, ATickPosition: Integer);

  procedure InternalDrawTickText(const ARect: TRect; const ACaption: string);
  const
    ARotationAngle: array [TcxTrackBarTextOrientation] of TcxRotationAngle =
      (ra0, raPlus90);
  begin
    if ACanvas.RectFullyVisible(ARect) then
    begin
      cxDrawText(ACanvas, ACaption, ARect, 0, clDefault,
        ARotationAngle[TextOrientation]);
      ACanvas.ExcludeClipRect(ARect);
    end;
  end;

const
  AOrientation: array [TcxTrackBarTextOrientation] of TcxTrackBarOrientation =
    (tboHorizontal, tboVertical);
var
  ATextSize: TSize;
  ADelta: Integer;
...
begin
  ...
  if TickMarks in [cxtmTopLeft, cxtmBoth] then
    InternalDrawTickText(GetTextTopRect(ATickPosition, ATextWidth, ATextHeight,
      ADelta), ACaption);
  if TickMarks in [cxtmBottomRight, cxtmBoth] then
    InternalDrawTickText(GetTextBottomRect(ATickPosition, ATextWidth,
      ATextHeight, ADelta), ACaption);
end;
...
procedure TcxCustomTrackBarViewInfo.DrawTicks(ACanvas: TcxCanvas);
  ...
  procedure DrawTick(ACanvas: TcxCanvas; AIndex, ATickPosition: Integer;
    ADrawTextTicks, ADrawLineTicks: Boolean);
  ...
  begin
    for I := 0 to MajorTicks.Count - 1 do
      DrawTick(ACanvas, Integer(MajorTicks[I]),
        GetCoordinate(Integer(MajorTicks[I])), True, True);
  end;
  ...
  begin
    ...
    begin
      ...
      if (ATickPosition > APrevTickPosition + 1) or
        (ATickPosition < APrevTickPosition - 1) then
      begin
        DrawTick(ACanvas, AIndex, ATickPosition, ADrawTextTicks,
          ADrawLineTicks);
...
begin
  LookAndFeel.Painter.DrawTrackBarScaledThumb(ACanvas, R,
    ...
    TrackBarTicksToTicksAlign[TickMarks], GetThumbRealColor(AState),
    ScaleFactor);
end;
...
begin
  Result := MulDiv(APosition - Min, PixelsPerPositionStep.Numerator,
    PixelsPerPositionStep.Denominator) + ThumbSize div 2;
end;
  ...
  case AState of
    ...
    TUS_PRESSED, TUS_HOT:
      if LookAndFeel.Kind in [lfUltraFlat, lfOffice11] then
        Result := GetEditButtonHighlightColor(AState = TUS_PRESSED,
...
begin
  ...
  begin
    ...
    else if tbmpUnderThumb in MouseStates then
...
function TcxCustomTrackBarViewInfo.GetCalculatedPosition(APosition: Integer;
  AIsInc: Boolean; AIsLineSize: Boolean; ANeedRound: Boolean = False): Integer;
var
...
function TcxCustomTrackBarViewInfo.GetMouseDelta(X, Y: Integer;
  AMouseDownPos: TPoint): Integer;
begin
...
begin
  ...
  begin
    ...
    else if IsPtToTheLeftFromThumb(AThumb, X, Y) then
...
function TcxCustomTrackBarViewInfo.GetPositionAfterJump(X, Y: Integer): Integer;
  ...
  begin
    Result := GetCurrentMousePos(X, Y) - TrackRect.Left -
      cxRectWidth(VisibleThumbRect) div 2
  end;

var
  ADeltaPosition: Integer;
begin
  ADeltaPosition := MulDiv(GetMouseOffset, PixelsPerPositionStep.Denominator,
    PixelsPerPositionStep.Numerator);
  if ReverseDirection then
...
function TcxCustomTrackBarViewInfo.GetPositionHintText(var ACanShow: Boolean;
  var AIsHintMultiLine: Boolean): string;
begin
  ...
  begin
    ...
    if ACanShow then
      PositionHintHelper.ShowHint(TrackBarRect,
        GetPositionHintTextRect(Edit.Canvas, AHintText, AIsMultiLine),
        AHintText, AIsMultiLine, Self, Font)
    else
...
begin
  ...
  begin
    if (OrientationHelper = nil) or
      (OrientationHelper is TcxVerticalTrackBarViewInfoHelper) then
    begin
      FreeAndNil(FOrientationHelper);
  ...
  begin
    if (OrientationHelper = nil) or
      not(OrientationHelper is TcxVerticalTrackBarViewInfoHelper) then
    begin
      FreeAndNil(FOrientationHelper);
...
function TcxCustomTrackBarViewInfo.GetMouseDownPos(AMouseDownPos
  : TPoint): Integer;
begin
  Result := OrientationHelper.GetMouseDownPos(AMouseDownPos);
end;

procedure TcxCustomTrackBarViewInfo.CalculateCustomThumbSize(ACustomRect: TRect;
  out AThumbSize, AThumbLargeSize: Integer);
begin
  OrientationHelper.CalculateCustomThumbSize(ACustomRect, AThumbSize,
    AThumbLargeSize);
end;

procedure TcxCustomTrackBarViewInfo.DrawTickAsLineBottomRight
  (ACanvas: TcxCanvas; ATickPosition, ATickSize: Integer);
begin
  OrientationHelper.DrawTickAsLineBottomRight(ACanvas, ATickPosition,
    ATickSize);
end;

procedure TcxCustomTrackBarViewInfo.DrawTickAsLineTopLeft(ACanvas: TcxCanvas;
  ATickPosition, ATickSize: Integer);
begin
...
function TcxCustomTrackBarViewInfo.GetPositionHintTextRect(ACanvas: TcxCanvas;
  const AHintText: string; AIsHintMultiLine: Boolean): TRect;
begin
  Result := OrientationHelper.GetPositionHintTextRect(ACanvas, AHintText,
    AIsHintMultiLine);
end;
...
function TcxCustomTrackBarViewInfo.GetTextTopRect(ATickPosition, ATextWidth,
  ATextHeight, ADelta: Integer): TRect;
begin
  Result := OrientationHelper.GetTextTopRect(ATickPosition, ATextWidth,
    ATextHeight, ADelta);
end;

function TcxCustomTrackBarViewInfo.GetTextBottomRect(ATickPosition, ATextWidth,
  ATextHeight, ADelta: Integer): TRect;
begin
  Result := OrientationHelper.GetTextBottomRect(ATickPosition, ATextWidth,
    ATextHeight, ADelta);
end;
...
function TcxCustomTrackBarViewInfo.CreateThumbViewInfo
  : TcxTrackBarThumbViewInfo;
begin
...
function TcxCustomTrackBarViewInfo.GetNearestThumb(X, Y: Integer)
  : TcxTrackBarThumbViewInfo;
var
  AThumb: TcxTrackBarThumbViewInfo;
...
function TcxCustomTrackBarViewInfo.GetThumbFromPos(const APoint: TPoint)
  : TcxTrackBarThumbViewInfo;
var
  AThumb: TcxTrackBarThumbViewInfo;
begin
...
function TcxCustomTrackBarViewInfo.GetTrackBarProperties
  : TcxCustomTrackBarProperties;
begin
...
function TcxCustomTrackBarViewInfo.GetThumbFadingHelper
  : TcxTrackBarThumbFadingHelper;
begin
  Result := FThumbs[0].FFadingHelper;
end;

function TcxCustomTrackBarViewInfo.GetThumb(Index: Integer)
  : TcxTrackBarThumbViewInfo;
begin
...
procedure TcxCustomTrackBarViewData.CalculateTrackBarViewInfoProperties
...
begin
  ...
  begin
    ...
    TrackSize := ScaleFactor.Apply(Properties.TrackSize,
    ...
    ThumbHeight := ScaleFactor.Apply(Properties.ThumbHeight,
      Properties.ScaleFactor);
    ThumbWidth := ScaleFactor.Apply(Properties.ThumbWidth,
      Properties.ScaleFactor);
    ...
    if Orientation = tboHorizontal then
      ALength := cxRectWidth(ClientRect)
    else
      ALength := cxRectHeight(ClientRect);
    Frequency := Math.Max(Frequency, (Max - Min) div Math.Max(1, ALength));
...
function TcxCustomTrackBarViewData.ThumbHalfSize
  (AViewInfo: TcxCustomTrackBarViewInfo): Integer;
begin
  Result := AViewInfo.ThumbSize div 2;
end;

procedure TcxCustomTrackBarViewData.CalculateButtonLeftRect
  (AViewInfo: TcxCustomTrackBarViewInfo);
begin
  AViewInfo.LeftButtonRect.Left := AViewInfo.TrackZoneRect.Left;
  AViewInfo.LeftButtonRect.Right := AViewInfo.TrackZoneRect.Left +
    AViewInfo.GetLeftButtonSize.cx;
  AViewInfo.LeftButtonRect.Top := AViewInfo.TrackZoneRect.Top +
    (cxRectHeight(AViewInfo.TrackZoneRect) -
    AViewInfo.GetLeftButtonSize.cy) div 2;
  AViewInfo.LeftButtonRect.Bottom := AViewInfo.LeftButtonRect.Top +
    AViewInfo.GetLeftButtonSize.cy;
end;

procedure TcxCustomTrackBarViewData.CalculateButtonRightRect
  (AViewInfo: TcxCustomTrackBarViewInfo);
begin
  AViewInfo.RightButtonRect.Left := AViewInfo.TrackZoneRect.Right -
    AViewInfo.GetRightButtonSize.cx;
  if not AViewInfo.UseSkins then
    Inc(AViewInfo.RightButtonRect.Left);
  AViewInfo.RightButtonRect.Right := AViewInfo.RightButtonRect.Left +
    AViewInfo.GetRightButtonSize.cx;
  AViewInfo.RightButtonRect.Top := AViewInfo.TrackZoneRect.Top +
    (cxRectHeight(AViewInfo.TrackZoneRect) -
    AViewInfo.GetRightButtonSize.cy) div 2;
  AViewInfo.RightButtonRect.Bottom := AViewInfo.RightButtonRect.Top +
    AViewInfo.GetRightButtonSize.cy;
end;

procedure TcxCustomTrackBarViewData.CalculatePixelsPerPositionStep
  (AViewInfo: TcxCustomTrackBarViewInfo);
...
begin
  AViewInfo.PixelsPerPositionStep.Numerator := cxRectWidth(AViewInfo.TrackRect)
    - AViewInfo.ThumbSize;
  AViewInfo.PixelsPerPositionStep.Denominator := GetMinMaxDiff;
end;

function TcxCustomTrackBarViewData.CalculateTickSize(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomTrackBarViewInfo; ALeftTop: Boolean): Integer;
  ...
  begin
    ...
    begin
      AViewInfo.TickSize :=
        Max((ATrackBarHeight - MaxRealTrackZoneSize - AMultiplier *
        (ATextHeight + TrackAndTickIndent + TickAndTextIndent))
        div AMultiplier, 0);
      Result := AViewInfo.TickSize + TrackAndTickIndent + TickAndTextIndent +
        ATextHeight;
    end;
  end;

const
  Orientation: array [TcxTrackBarTextOrientation] of TcxTrackBarOrientation =
    (tboHorizontal, tboVertical);
var
...
begin
  Result := 0;
  ATextHeight := 0;
  AIsCorrectSide := ((Properties.TickMarks <> cxtmBottomRight) and ALeftTop) or
    (Properties.TickMarks <> cxtmTopLeft) and not ALeftTop;

  if AIsCorrectSide then
  begin
    Result := AViewInfo.TickSize + TrackAndTickIndent;
    if Properties.TickType in [tbttNumbers, tbttValueNumber, tbttTicksAndNumbers]
    then
    begin
...
procedure TcxCustomTrackBarViewData.CalculateSelectionRect
  ...
  procedure CalculateSelectionBounds(out ASelectionStartCoordinate,
    ASelectionEndCoordinate: Integer);
  ...
  begin
    ASelectionStartCoordinate := AViewInfo.GetCoordinate
      (AViewInfo.SelectionStart);
...
begin
  ...
  begin
    CalculateSelectionBounds(AViewInfo.SelectionRect.Left,
      AViewInfo.SelectionRect.Right);
...
begin
  ...
  begin
    ...
    AViewInfo.CalculateCustomThumbSize(ACustomRect, AThumbSize,
      AThumbLargeSize);
    AViewInfo.ThumbWidth := AThumbSize;
    AViewInfo.ThumbHeight := AThumbLargeSize;
  end
  else if AViewInfo.UseSkins then
  begin
    APainterThumbSize := AViewInfo.Painter.TrackBarScaledThumbSize(True,
      ScaleFactor);
    AThumbSize := APainterThumbSize.cx;
    AThumbLargeSize := Min(APainterThumbSize.cy,
      cxRectHeight(AViewInfo.TrackZoneRect));
  end
  else if Properties.AutoSize then
  begin
...
procedure TcxCustomTrackBarViewData.CalculateThumbRect(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomTrackBarViewInfo);
begin
  AViewInfo.ThumbRect.Left := AViewInfo.GetCoordinate(AViewInfo.Position) -
    ThumbHalfSize(AViewInfo);
  ...
  AViewInfo.ThumbRect.Bottom := AViewInfo.ThumbRect.Top +
    AViewInfo.ThumbLargeSize;
  AViewInfo.VisibleThumbRect :=
    AViewInfo.DrawingThumbRectToRealThumbRect(ACanvas);
...
begin
  ...
  AViewInfo.TrackBarBorderWidth := Min(AViewInfo.TrackBarBorderWidth,
    cxRectWidth(AViewInfo.TrackBarRect) div 2);
  AViewInfo.TrackBarBorderWidth := Min(AViewInfo.TrackBarBorderWidth,
    cxRectHeight(AViewInfo.TrackBarRect) div 2);
end;
...
procedure TcxCustomTrackBarViewData.CalculateTrackRect
  ...
  begin
    AViewInfo.TrackRect.Left := AViewInfo.TrackRect.Left +
      cxRectWidth(AViewInfo.LeftButtonRect) + ScaleFactor.Apply(ButtonIndent);
    AViewInfo.TrackRect.Right := AViewInfo.TrackRect.Right -
      cxRectWidth(AViewInfo.RightButtonRect) - ScaleFactor.Apply(ButtonIndent);
  end;
...
procedure TcxCustomTrackBarViewData.CalculateTrackSize
  (AViewInfo: TcxCustomTrackBarViewInfo; ARealTrackZoneSize: Integer);
begin
  if AViewInfo.UseSkins then
    AViewInfo.TrackSize := AViewInfo.Painter.TrackBarScaledTrackSize
      (ScaleFactor)
  else if Properties.AutoSize then
...
begin
  ...
  ATrackZoneSize := cxRectHeight(AViewInfo.TrackBarRect) - ATopLeftIndent -
    ABottomRightIndent;
  ARealTrackZoneSize := Min(Max(ATrackZoneSize, 10),
    ScaleFactor.Apply(MaxRealTrackZoneSize));

  GetCustomRealTrackZoneSize(AViewInfo as TcxCustomTrackBarViewInfo,
    ARealTrackZoneSize);

  AViewInfo.TrackZoneRect.Top := AViewInfo.TrackBarRect.Top +
    ((ATrackZoneSize - ARealTrackZoneSize) div 2) + ATopLeftIndent;
  AViewInfo.TrackZoneRect.Bottom := AViewInfo.TrackZoneRect.Top +
    ARealTrackZoneSize;
...
procedure TcxCustomTrackBarViewData.UpdateSelection
  (AViewInfo: TcxCustomTrackBarViewInfo);
begin
...
procedure TcxCustomTrackBarViewData.CalculateCustomTrackBarRects
...
procedure TcxCustomTrackBarViewData.Calculate(ACanvas: TcxCanvas;
...
procedure TcxCustomTrackBarViewData.EditValueToDrawValue(const AEditValue
...
function TcxCustomTrackBarViewData.InternalGetEditConstantPartSize
...
procedure TcxCustomTrackBarViewData.GetCustomRealTrackZoneSize
  (AViewInfo: TcxCustomTrackBarViewInfo; var ARealTrackZoneSize: Integer);
var
  ACustomRect: TRect;
begin
  if (Properties.ThumbType = cxttCustom) and IsOnGetThumbRectEventAssigned then
...
procedure TcxCustomTrackBarViewData.GetOnGetThumbRect
  (out AValue: TcxGetThumbRectEvent);
begin
  if Edit = nil then
...
function TcxCustomTrackBarProperties.GetDisplayText(const AEditValue
...
class function TcxCustomTrackBarProperties.GetStyleClass
  : TcxCustomEditStyleClass;
begin
  Result := TcxTrackBarStyle;
end;

function TcxCustomTrackBarProperties.GetSupportedOperations
...
class function TcxCustomTrackBarProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxCustomTrackBarViewInfo;
end;

function TcxCustomTrackBarProperties.IsEditValueValid(var EditValue
...
function TcxCustomTrackBarProperties.EditValueToPosition(const AEditValue
  : TcxEditValue): Integer;
begin
  if not IsVarEmpty(AEditValue) and
    (VarIsNumeric(AEditValue) or VarIsStr(AEditValue)) then
...
function TcxCustomTrackBarProperties.GetValidateErrorText
  (AErrorKind: TcxEditErrorKind): string;
begin
  Result := cxGetResourceString(@cxSEditValueOutOfBounds);
end;

class function TcxCustomTrackBarProperties.GetViewDataClass
...
procedure TcxCustomTrackBarProperties.DoAssign(AProperties
...
procedure TcxCustomTrackBarProperties.SetOrientation
  (Value: TcxTrackBarOrientation);
begin
...
procedure TcxCustomTrackBarProperties.SetTextOrientation
  (Value: TcxTrackBarTextOrientation);
begin
...
function TcxCustomTrackBarProperties.FixPosition(const APosition
  : Integer): Integer;
begin
  Result := Math.Max(Math.Min(APosition, Max), Min);
end;

procedure TcxCustomTrackBarProperties.DoDrawThumb(Sender: TObject;
...
constructor TcxTrackBarPositionHintHelper.Create
  (AViewInfo: TcxCustomTrackBarViewInfo);
begin
  inherited Create;
  FTrackBarViewInfo := AViewInfo;
  FPositionHintHidingTimer := cxCreateTimer(PositionHintHidingTimerHandler,
    1000, False);
end;
...
begin
  ...
  if AOwnerControl.ActiveProperties.TickMarks in [cxtmBottomRight, cxtmBoth]
  then
  begin
    ATopLeft := AOwnerControl.ClientToScreen
      (AOwnerControl.ViewInfo.GetThumbRect.TopLeft);
    if AOwnerControl.ActiveProperties.Orientation = tboHorizontal then
      ARect := cxRectSetTop(ARect, ATopLeft.Y - cxRectHeight(ARect) -
        PositionHintIndent)
    else
      ARect := cxRectSetLeft(ARect, ATopLeft.X - cxRectWidth(ARect) -
        PositionHintIndent);
  end;
end;
...
procedure TcxTrackBarController.MouseDown(Button: TMouseButton;
...
begin
  ...
  if (tbmpSliding in ViewInfo.MouseStates) and
    not(ssLeft in KeyboardStateToShiftState) then
  begin
...
procedure TcxTrackBarController.MouseMove(Shift: TShiftState; X, Y: Integer);
  ...
  function GetButtonState(AIsPressed: Boolean; const ABounds: TRect)
    : TcxButtonState;
  begin
    Result := cxbsNormal;
  ...
  begin
    ViewInfo.ButtonLeftState := GetButtonState(FIsMouseDownOnLeftButton,
      ViewInfo.GetLeftButtonRect);
    ViewInfo.ButtonRightState := GetButtonState(FIsMouseDownOnRightButton,
      ViewInfo.GetRightButtonRect);
  end;
...
procedure TcxTrackBarController.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
  ...
  begin
    ...
    else if PtInRect(ViewInfo.GetRightButtonRect, Point(X, Y)) then
...
begin
  Result := ActiveProperties.FixPosition
    (StoredPosition + MulDiv(ViewInfo.GetMouseDelta(X, Y, GetMouseDownPos),
    ViewInfo.PixelsPerPositionStep.Denominator,
    ViewInfo.PixelsPerPositionStep.Numerator));
end;

procedure TcxTrackBarController.UpdatePos(AValue: Integer;
  AThumb: TcxTrackBarThumbViewInfo);
begin
  if AValue <> Position then
  begin
    if (ssShift in KeyboardStateToShiftState) and
...
begin
  Exclude(ViewInfo.MouseStates, tbmpSliding);
  if (TrackBar.GetPosition <> StartSlidingPosition) and
    Supports(TrackBar, IcxTrackBarPostProvider, APostProvider) then
...
begin
  if ActiveProperties.ReadOnly { or not FTrackBar.DataBinding.IsDataAvailable }
  then
    Exit;
...
function TcxTrackBarController.GetPositionAfterKeyDown
  (AThumb: TcxTrackBarThumbViewInfo; var Key: Word; Shift: TShiftState)
  : Integer;
begin
  case Key of
    VK_PRIOR:
      Result := ActiveProperties.FixPosition
        (ViewInfo.GetCalculatedPosition(AThumb.Position, False, False));
    VK_NEXT:
      Result := ActiveProperties.FixPosition
        (ViewInfo.GetCalculatedPosition(AThumb.Position, True, False));
    VK_END:
      begin
    ...
    VK_LEFT, VK_UP:
      Result := ActiveProperties.FixPosition
        (ViewInfo.GetCalculatedPosition(AThumb.Position, False, True));
    VK_RIGHT, VK_DOWN:
      Result := ActiveProperties.FixPosition
        (ViewInfo.GetCalculatedPosition(AThumb.Position, True, True));
  else
    Result := AThumb.Position;
  end;
end;
...
begin
  ...
  else if FIsMouseDownOnRightButton and (ViewInfo.ButtonRightState = cxbsPressed)
  then
...
procedure TcxTrackBarController.ProcessButtons(var ANewPosition: Integer;
  X, Y: Integer);
begin
  if PtInRect(ViewInfo.GetLeftButtonRect, Point(X, Y)) then
  begin
...
procedure TcxTrackBarController.SetNewSelectionPosition(const ANewPosition
  : Integer);
begin
...
class function TcxCustomTrackBar.GetPropertiesClass
...
procedure TcxCustomTrackBar.DoValidateDisplayValue(var ADisplayValue
...
procedure TcxCustomTrackBar.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
...
procedure TcxCustomTrackBar.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TcxCustomTrackBar.PopulateSizeProperties(var AEditSizeProperties
...
constructor TcxCustomTrackBarFadingHelper.Create
  (AViewInfo: TcxCustomTrackBarViewInfo);
begin
  inherited Create;
  FViewInfo := AViewInfo;
...
procedure TcxTrackBarThumbFadingHelper.GetFadingImages(out AFadeOutImage,
...
procedure TcxTrackBarCustomButtonFadingHelper.GetFadingImages(out AFadeOutImage,
  AFadeInImage: TcxBitmap);
...
procedure TcxTrackBarLeftButtonFadingHelper.DrawButton(ACanvas: TcxCanvas;
  const R: TRect; AState: TcxButtonState);
begin
...
procedure TcxTrackBarRightButtonFadingHelper.DrawButton(ACanvas: TcxCanvas;
  const R: TRect; AState: TcxButtonState);
begin
...
function TcxTrackBarThumbViewInfo.CreateFadingHelper
  : TcxTrackBarThumbFadingHelper;
begin
...
begin
  ...
  begin
    FFadingHelper.CheckStartFading(TrackBarStateToButtonStates[FState],
...
GetRegisteredEditProperties.Register(TcxTrackBarProperties,
  scxSEditRepositoryTrackBarItem);
FilterEditsController.Register(TcxTrackBarProperties, TcxFilterTrackBarHelper);

finalization

FilterEditsController.Unregister(TcxTrackBarProperties,
  TcxFilterTrackBarHelper);
