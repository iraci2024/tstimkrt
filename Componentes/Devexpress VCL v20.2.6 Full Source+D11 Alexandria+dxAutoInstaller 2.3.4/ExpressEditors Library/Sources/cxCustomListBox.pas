  ...
  repeat
    ...
    begin
      ...
      begin
        AListClientSize.cy := AListClientSize.cy - AItemHeight;
        AListSize.cy := AListClientSize.cy;
        AListClientSize.cx := Width -
          (ABorderExtent.Left + ABorderExtent.Right);
        AVScrollBar := I < Count;
        if AVScrollBar then
          AListClientSize.cx := AListClientSize.cx - AScrollBarSize.cx;
        if AListClientSize.cx < AScrollWidth then
...
begin
  ...
  if Key = dxSpace then
    DataBinding.Reset
  else if NeedIgnorePressedKey(Key) then
    Key := #0;
end;
...
procedure TcxCustomListBox.Scroll(AScrollBarKind: TScrollBarKind;
  AScrollCode: TScrollCode; var AScrollPos: Integer);
begin
  inherited Scroll(AScrollBarKind, AScrollCode, AScrollPos);
  if Enabled then
    TcxCustomInnerListBoxAccess(FInnerListBox).Scroll(AScrollBarKind,
  ...
  if InnerListBox <> nil then
    case Message.Msg of
      LB_ADDSTRING .. LB_MSGMAX:
        begin
          Message.Result := SendMessage(InnerListBox.Handle, Message.Msg,
