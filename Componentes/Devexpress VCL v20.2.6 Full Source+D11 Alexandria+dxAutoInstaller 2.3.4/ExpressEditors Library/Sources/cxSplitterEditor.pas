  ...
  with TcxSplitEditor.Create(Application) do
    try
      if ASplitter.Owner <> nil then
        ...
        AOwnerCaption := '';
      Caption := Format('%s%s - Splitter Control',
        [AOwnerCaption, ASplitter.Name]);
...
begin
  ...
  vHzcl := TcxHotZoneStyleClass
    (GetRegisteredHotZoneStyles.FindByClassName(vText));
  if vHzcl = nil then
    vHzcl := TcxHotZoneStyleClass
      (GetRegisteredHotZoneStyles.FindByDescription(vText));
...
Begin
  ...
  begin
    ...
    vHzt := GetRegisteredHotZoneStyles.GetIndexByClass
      (cxSplit.HotZone.ClassType);
    case vHzt of
      0:
        cxRbHzMp8.Checked := True;
      1:
        cxRbHzMp9.Checked := True;
      2:
        cxRbHzXp.Checked := True;
      3:
        cxRbHzSimple.Checked := True;
    end;
  end
  else
  begin
