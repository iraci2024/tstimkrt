...
uses
  ...
  cxGraphics, Menus, cxLabel, dxLayoutcxEditAdapters, dxLayoutControlAdapters,
...
type
  ...
  protected
    procedure AddFilterItem(AParent: TcxFilterCriteriaItemList;
    ...
    property FilterEditHelper: TcxCustomFilterEditHelperClass
      read FFilterEditHelper;
  ...
  public
    constructor Create(ACriteria: TcxFilterCriteria; AItemLink: TObject;
      ...
      ALookAndFeel: TcxLookAndFeel; AFont: TFont); reintroduce; virtual;
    procedure ApplyChanges; virtual;
    class function GetFilterEditHelper(AEditProperties: TcxCustomEditProperties)
      : TcxCustomFilterEditHelperClass; // for internal use
...
function ShowFilterDialog(ACriteria: TcxFilterCriteria; AItemLink: TObject;
  AEditProperties: TcxCustomEditProperties; const ACriteriaItemCaption: string;
  AValueTypeClass: TcxValueTypeClass; ALookAndFeel: TcxLookAndFeel = nil;
  AFont: TFont = nil; AControl: TControl = nil): Boolean;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        AItem := Items[I];
        if AItem.IsItemList or not(TcxFilterCriteriaItem(AItem).OperatorKind
          in SupportedKinds) then
          Exit;
      end;
  ...
  begin
    if AItem.IsItemList then
      ...
      begin
        Result := (OperatorKind in SupportedKinds) and
          (AItemLinks.IndexOf(ItemLink) = -1);
        if Result then
...
begin
  ...
  try
    ...
    if Result then
      ADialog.ApplyChanges;
  finally
    ADialog.Free;
  end;
end;

procedure FillOperatorComboBox(AComboBox: TcxComboBox;
  AOperators: TcxFilterControlOperators);
var
  ...
  for AOperator := Low(TcxFilterControlOperator)
    ...
    if AOperator in AOperators then
      AComboBox.Properties.Items.AddObject
        (GetFilterControlOperatorText(AOperator), TObject(AOperator));
end;

function GetOperatorIndex(AComboBox: TcxComboBox;
...
constructor TcxFilterDialog.Create(ACriteria: TcxFilterCriteria;
  AItemLink: TObject; AEditProperties: TcxCustomEditProperties;
  const ACriteriaItemCaption: string; AValueTypeClass: TcxValueTypeClass;
  ALookAndFeel: TcxLookAndFeel; AFont: TFont);
begin
...
begin
  ...
  Criteria.AddItem(AParent, ItemLink, GetFilterOperatorKind(AOperator), AValue,
    ADisplayValue);
end;
...
procedure TcxFilterDialog.GetFilterValues(AEdit: TcxCustomEdit;
  var AValue: Variant; var ADisplayValue: TCaption);
begin
  AValue := Null;
  ADisplayValue := '';
  if GetOperatorComboBox(AEdit).ItemIndex <> -1 then
    FilterEditHelper.GetFilterValue(AEdit, EditProperties, AValue,
      ADisplayValue);
...
begin
  Result := FilterEditHelper.GetSupportedFilterOperators(EditProperties,
    ValueTypeClass);
...
procedure TcxFilterDialog.InitControls(const ACriteriaItemCaption: string);
  ...
  function CreateValueEdit(APlaceHolder: TdxLayoutItem;
    AOperatorComboBox: TcxComboBox): TcxCustomEdit;
  begin
...
begin
  ...
  lblSingle.Caption := cxGetResourceString(@cxSFilterDialogUse) + ' ' +
    Criteria.UnderscoreWildcard + ' ' + cxGetResourceString
    (@cxSFilterDialogSingleCharacter);
  lblSeries.Caption := cxGetResourceString(@cxSFilterDialogUse) + ' ' +
    Criteria.PercentWildcard + ' ' + cxGetResourceString
...
begin
  // default values
  if (fcoLike in SupportedOperators) and (ValueTypeClass <> nil) and
    ValueTypeClass.IsString then
    cbOperator1.ItemIndex := GetOperatorIndex(cbOperator1, fcoLike)
  else if fcoEqual in SupportedOperators then
...
procedure TcxFilterDialog.InitEdits(AComboBox: TcxComboBox;
  AEdit: TcxCustomEdit; AItem: TcxFilterCriteriaItem);
var
  AOperator: TcxFilterControlOperator;
begin
  AOperator := GetFilterControlOperator(AItem.OperatorKind,
    AItem.ValueIsNull(AItem.Value));
...
begin
  AWildcardChars := [AnsiChar(Criteria.UnderscoreWildcard),
    AnsiChar(Criteria.PercentWildcard)];
  if AEdit is TcxCustomTextEdit then
    with TcxCustomTextEdit(AEdit) do
      if GetOperator(GetOperatorComboBox(AEdit))
        in [fcoLike, fcoNotLike, fcoContains, fcoNotContains, fcoBeginsWith,
        fcoEndsWith] then
        ActiveProperties.ValidChars := ActiveProperties.ValidChars +
          AWildcardChars
      else
        ActiveProperties.ValidChars := ActiveProperties.ValidChars -
          AWildcardChars;
...
procedure TcxFilterDialog.ValidateValue(AEdit: TcxCustomEdit;
  var AValue: Variant);
begin
  FilterControlValidateValue(AEdit, AValue,
    GetOperator(GetOperatorComboBox(AEdit)), ValueTypeClass, FilterEditHelper);
end;
...
begin
  ...
  try
    ...
    begin
      AItemList := Criteria.Root.AddItemList
        (TcxFilterBoolOperatorKind(rbOr.Checked));
...
class function TcxFilterDialog.GetFilterEditHelper(AEditProperties
  : TcxCustomEditProperties): TcxCustomFilterEditHelperClass;
begin
