...
type
  ...
  private
    ...
    procedure CorrectTextBounds(var ATextBounds: TRect;
      const AHorzAlignment: TAlignment; const ARect: TRect);
  protected
    ...
    function CalculateTextColor(AStyle: TcxContainerStyle; AEnabled: Boolean;
      AContainer: TcxContainer = nil): TColor; override;
  ...
  public
    ...
    procedure ValidateDisplayValue(var ADisplayValue: TcxEditValue;
      var AErrorText: TCaption; var Error: Boolean;
  ...
  protected
    class procedure OpenHyperLink(const AHyperLink: string;
      AActiveProperties: TcxCustomHyperLinkEditProperties);
  ...
  public
    ...
    property ActiveProperties: TcxCustomHyperLinkEditProperties
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TcxHyperLinkEditProperties
      read GetActiveProperties;
  published
    property Anchors;
    property AutoSize;
...
function TcxHyperLinkEditViewInfo.CanOpenHyperLink(const P: TPoint;
  AIsFirstClick: Boolean): Boolean;
begin
  if FSingleClick and AIsFirstClick or not FSingleClick and not AIsFirstClick
  then
...
function TcxHyperLinkEditViewInfo.CalculateTextColor(AStyle: TcxContainerStyle;
...
procedure TcxHyperLinkEditViewInfo.CorrectTextBounds(var ATextBounds: TRect;
  const AHorzAlignment: TAlignment; const ARect: TRect);
begin
  case AHorzAlignment of
    taRightJustify:
      ATextBounds := cxRectOffsetHorz(ATextBounds,
        ARect.Right - ATextBounds.Right);
    taCenter:
...
begin
  if Edit <> nil then
  begin
    Result := cxRect(Bounds.Left, InnerEditRect.Top, Bounds.Right,
      InnerEditRect.Bottom);
    if FEdit is TcxCustomTextEdit then
    begin
      AHorzAlignment := FEdit.ActiveProperties.Alignment.Horz;
      ATextWidth := TdxTextMeasurer.TextWidthTO(TcxCustomTextEditAccess(Edit)
        .Font, TcxCustomTextEditAccess(Edit).InnerTextEdit.EditValue);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result.Right := Max(Result.Right, ARow.TextOriginX +
          ARow.TextExtents.cx);
...
function TcxHyperLinkEditViewInfo.GetCurrentCursor(const AMousePos
  : TPoint): TCursor;
begin
  Result := crDefault;
...
procedure TcxHyperLinkEditViewInfo.InplaceMouseDown(AButton: TMouseButton;
  const AShift: TShiftState; X, Y: Integer);
begin
  if (AButton = mbLeft) and CanOpenHyperLink(Point(X, Y),
    not(ssDouble in AShift)) then
    TcxHyperLinkEdit.OpenHyperLink(Text,
      EditProperties as TcxCustomHyperLinkEditProperties);
end;

function TcxHyperLinkEditViewInfo.IsMouseHoveredOverText(const AMousePos
  : TPoint): Boolean;
begin
...
begin
  Result := inherited IsHotTrack or TcxCustomHyperLinkEditProperties
    (EditProperties).SingleClick;
end;
...
procedure TcxCustomHyperLinkEditProperties.SetLinkColor(const Value: TColor);
begin
...
class function TcxCustomHyperLinkEditProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxHyperLinkEdit;
end;

class function TcxCustomHyperLinkEditProperties.GetStyleClass
  : TcxCustomEditStyleClass;
begin
  Result := TcxHyperLinkStyle;
end;

function TcxCustomHyperLinkEditProperties.GetSupportedOperations
...
class function TcxCustomHyperLinkEditProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TcxHyperLinkEditViewData;
end;

class function TcxCustomHyperLinkEditProperties.GetViewInfoClass
...
procedure TcxCustomHyperLinkEditProperties.DoAssign
...
function TcxCustomHyperLinkEditProperties.AddPrefixTo
...
begin
  ...
  begin
    if (Container = nil) or (TcxCustomHyperLinkEdit(Container)
      .ActiveProperties = nil) then
...
procedure TcxCustomHyperLinkEdit.ActivateByMouse(Shift: TShiftState;
...
function TcxCustomHyperLinkEdit.GetActiveProperties
  : TcxCustomHyperLinkEditProperties;
begin
...
function TcxCustomHyperLinkEdit.InnerControlMenuHandler
  (var Message: TMessage): Boolean;
begin
  if Message.Msg = WM_LBUTTONDOWN then
    TcxHyperLinkEditViewInfo(ViewInfo).FWasFocusedBeforeClick :=
      ViewInfo.Focused;
  Result := inherited InnerControlMenuHandler(Message);
end;

class procedure TcxCustomHyperLinkEdit.OpenHyperLink(const AHyperLink: string;
  AActiveProperties: TcxCustomHyperLinkEditProperties);
var
  ALink: string;
begin
...
procedure TcxCustomHyperLinkEdit.SetProperties(const Value
  : TcxCustomHyperLinkEditProperties);
begin
...
begin
  if (ShortCut(Key, Shift) <> 0) and
    (ActiveProperties.StartKey = ShortCut(Key, Shift)) then
  begin
    DoStart;
    Key := 0;
  end
...
begin
  ...
  if (Button = mbLeft) and (ssDouble in Shift) and
    TcxHyperLinkEditViewInfo(ViewInfo).CanOpenHyperLink(Point(X, Y), False) then
    DoStart;
end;
...
begin
  inherited;
  if (Button = mbLeft) and (SelLength = 0) and
    TcxHyperLinkEditViewInfo(ViewInfo).CanOpenHyperLink(Point(X, Y),
...
class function TcxCustomHyperLinkEdit.GetPropertiesClass
...
class function TcxHyperLinkEdit.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
class function TcxFilterHyperLinkEditHelper.GetFilterEditClass
  : TcxCustomEditClass;
begin
  Result := TcxHyperLinkEdit;
end;
...
begin
  ...
  AHyperLinkViewInfo.FIsStandardAlignment :=
    not AProperties.UseLeftAlignmentOnEditing or
    (AProperties.Alignment.Horz = taLeftJustify);
end;

initialization

GetRegisteredEditProperties.Register(TcxHyperLinkEditProperties,
...
FilterEditsController.Unregister(TcxHyperLinkEditProperties,
  TcxFilterHyperLinkEditHelper);
