...
const
  cxShellObjectInternalAbsoluteVirtualPathPrefix =
    '::{9C211B58-E6F1-456A-9F22-7B3B418A7BB1}';
  cxShellObjectInternalRelativeVirtualPathPrefix =
    '::{63BE9ADB-E4B5-4623-96AA-57440B4EF5A8}';
  ...
  CLSID_ShellItem: TGUID = (D1: $43826D1E; D2: $E718; D3: $42EE;
    D4: ($BC, $55, $A1, $E2, $61, $C3, $7B, $FE));
...
type
  ITEMIDLISTARRAY = array [0 .. MaxInt div SizeOf(PItemIDList) - 1]
    of PItemIDList;
  ...
  IExtractImage = interface(IUnknown)
    [SID_IExtractImage]
    function GetLocation(pszPathBuffer: LPWSTR; cch: DWORD;
  ...
  IShellItemImageFactory = interface(IUnknown)
    ...
    function GetImage(size: TSize; flags: UINT; out phbm: HBITMAP)
      : HRESULT; stdcall;
  end;
  ...
  published
    property ShowFolders: Boolean read FShowFolders write SetShowFolders
      default True;
    property ShowNonFolders: Boolean read FShowNonFolders
      write SetShowNonFolders default True;
  ...
  published
    ...
    property ShowThumbnails: Boolean read FShowThumbnails
      write SetShowThumbnails default False;
  ...
  private
    ...
    function HasShellAttribute(AAttributes, AAttribute: LongWord)
      : Boolean; overload;
  ...
  TcxRootChangedEvent = procedure(Sender: TObject; Root: TcxCustomShellRoot)
    of object;
  ...
  public
    ...
    destructor Destroy; override;
    procedure CheckUpdate(AShellFolder: IShellFolder; AFolderPidl: PItemIDList;
      AFast: Boolean);
    procedure CheckInitialize(AIFolder: IShellFolder; APIDL: PItemIDList);
    procedure FetchDetails(wnd: HWND; ShellFolder: IShellFolder;
      DetailsMap: TcxShellDetails);
    ...
    procedure SetNewPidl(pFolder: IShellFolder;
      AFolderPidl, APIDL: PItemIDList);
  ...
  protected
    ...
    function CreateShellItemInfo(APIDL: PItemIDList; AFast: Boolean)
      : TcxShellItemInfo;
    ...
    function SlowInitializationDone(AItem: TcxShellItemInfo): Boolean;
      virtual; abstract;
  ...
  public
    ...
    procedure DoGetInfoTip(Handle: HWND; ItemIndex: Integer; InfoTip: PChar;
      cch: Integer);
  ...
  published
    ...
    property Scroll: Boolean read FScroll write FScroll stored False;
    // deprecated
  end;
  ...
  public
    ...
    procedure AddDefaultShellItems(AIShellFolder: IShellFolder;
      AItemPIDLList: TList); overload;
    procedure Popup(APos: TPoint); virtual;
  end;

function GetDesktopIShellFolder: IShellFolder;
function GetTextFromStrRet(var AStrRet: TStrRet; APIDL: PItemIDList)
  : WideString;
function GetShellDetails(pFolder: IShellFolder; Pidl: PItemIDList;
  out sd: IShellDetails): HRESULT;
...
function GetShellItemDisplayName(AIFolder: IShellFolder; APIDL: PItemIDList;
  ACheckIsFolder: Boolean): WideString;
...
var
  SHChangeNotifyRegister: function(HWND: HWND; dwFlags: DWORD;
    wEventMask: DWORD; uMsg: UINT; cItems: DWORD; lpItems: PShChangeNotifyEntry)
    : Cardinal; stdcall;
  ...
  SHGetImageList: function(iImageList: Integer; const riid: TIID; out ppv)
    : HRESULT; stdcall;
  dxSHGetViewStatePropertyBag: function(Pidl: PItemIDList; pszBagName: PChar;
    dwFlags: DWORD; const riid: TIID; out ppvOut): HRESULT; stdcall;
  dxSHGetFolderTypeFromCanonicalName: function(AName: PChar;
    var AFolderTypeID: TGUID): HRESULT; stdcall;
...
uses
  cxContainer, cxControls, cxEdit, dxUxTheme, dxCore, Contnrs, ComCtrls,
  CommCtrl;
...
type
  ...
  TSHGetPathFromIDList = function(APIDL: PItemIDList; APath: PChar)
    : BOOL; stdcall;
  TSHGetPathFromIDListW = function(APIDL: PItemIDList; APath: PWideChar)
    : BOOL; stdcall;
...
begin
  ...
  if not AItemProducer.DoCompareItems(AItemInfo1.Folder, AItemInfo2.Folder,
    Result) then
  begin
...
begin
  Result := (cxShellGetThreadSafeFileInfo(PChar(APIDL), 0, AFileInfo,
    SizeOf(AFileInfo), SHGFI_PIDL or SHGFI_ATTRIBUTES) <> 0) and
    (AFileInfo.dwAttributes and (SFGAO_FOLDER or SFGAO_STREAM) = SFGAO_FOLDER or
    SFGAO_STREAM);
end;
...
function HasSubItems(AParentIFolder: IShellFolder; AFullPIDL: PItemIDList;
  ...
  begin
    ...
    try
      ...
      if FindFirst(S + PathDelim + '*.*', AFileSearchAttributes, ASearchRec) = 0
      then
      begin
        repeat
          AHasSubItems := (ASearchRec.Name <> '.') and
            (ASearchRec.Name <> '..');
...
begin
  Result := HasAttributes(SFGAO_FOLDER) and
    Succeeded(GetDesktopIShellFolder.BindToObject(AFullPIDL, nil,
    IID_IShellFolder, ATempIFolder));
  if Result then
    if AEnumSettings and SHCONTF_NONFOLDERS = 0 then
      Result := HasAttributes(SFGAO_HASSUBFOLDER)
    else if not CheckLocalFolder(Result) then
    begin
      Result := HasAttributes(SFGAO_HASSUBFOLDER);
      if not Result and (ATempIFolder <> nil) and
        Succeeded(ATempIFolder.EnumObjects(0, AEnumSettings, AIEnum)) and
        Assigned(AIEnum) and (AIEnum.Next(1, ATempPIDL, AFetchedItemCount)
        = S_OK) then
        try
...
begin
  try
    ...
    if Result = S_OK then
      Exit;
    Result := pFolder.GetUIObjectOf(0, Integer(Pidl <> nil) (* 1 *) , Pidl,
      IID_IShellDetails, nil, sd);
  finally
    if sd = nil then
...
function CreateCallbackWnd(AContextMenu: IContextMenu2)
  : TcxContextMenuMessageWindow;
begin
...
begin
  ...
  begin
    cxShellGetThreadSafeFileInfo('C:\CXDUMMYFILE.TXT', FILE_ATTRIBUTE_NORMAL,
      AFileInfo, SizeOf(AFileInfo), SHGFI_SYSICONINDEX or
...
begin
  ...
  begin
    cxShellGetThreadSafeFileInfo('C:\CXDUMMYFOLDER', FILE_ATTRIBUTE_DIRECTORY,
      AFileInfo, SizeOf(AFileInfo), SHGFI_SYSICONINDEX or
      SHGFI_USEFILEATTRIBUTES);
...
begin
  ...
  begin
    cxShellGetThreadSafeFileInfo('C:\CXDUMMYFOLDER', FILE_ATTRIBUTE_DIRECTORY,
      AFileInfo, SizeOf(AFileInfo), SHGFI_SYSICONINDEX or
      SHGFI_USEFILEATTRIBUTES or SHGFI_OPENICON);
...
begin
  ...
  if not Result then
    if (APidl1 = nil) or (APidl2 = nil) then
      Exit
    else if Assigned(dxILIsEqual) then
...
begin
  if (APidl1 = nil) and (APidl2 = nil) then
    Result := nil
  else if APidl1 = nil then
    Result := GetPidlCopy(APidl2)
  else if APidl2 = nil then
...
procedure UnregisterShellItemsInfoGatherer
  (AGatherer: TcxShellItemsInfoGatherer);
begin
...
const
  ACSIDLs: array [TcxBrowseFolder] of Integer = (CSIDL_DESKTOP, CSIDL_STARTUP,
    CSIDL_BITBUCKET, CSIDL_COMMON_DESKTOPDIRECTORY, CSIDL_COMMON_DOCUMENTS,
    CSIDL_COMMON_FAVORITES, CSIDL_COMMON_PROGRAMS, CSIDL_COMMON_STARTMENU,
    CSIDL_COMMON_STARTUP, CSIDL_COMMON_TEMPLATES, CSIDL_CONTROLS, CSIDL_DESKTOP,
    CSIDL_DESKTOPDIRECTORY, CSIDL_DRIVES, CSIDL_PRINTERS, CSIDL_FAVORITES,
    CSIDL_FONTS, CSIDL_HISTORY, CSIDL_MYMUSIC, CSIDL_MYPICTURES, CSIDL_NETHOOD,
    CSIDL_PROFILE, CSIDL_PROGRAM_FILES, CSIDL_PROGRAMS, CSIDL_RECENT,
    CSIDL_STARTMENU, CSIDL_STARTUP, CSIDL_TEMPLATES, CSIDL_PERSONAL,
    CSIDL_NETWORK);
var
  ...
  try
    try
      ...
      begin
        ...
        OleCheck(cxGetFolderLocation(ParentWindow, ACSIDLs[ABrowseFolder], 0, 0,
          ATempPIDL));
    except
      on E: Exception do
...
procedure TcxCustomShellRoot.SetPidl(const Value: PItemIDList);
  ...
  function GetDesktopPidl(out APIDL: PItemIDList): Boolean;
  begin
    if IsWinVistaOrLater then
      Result := Succeeded(SHGetKnownFolderIDList(FOLDERID_Desktop, 0, 0, APIDL))
    else
      Result := Succeeded(cxGetFolderLocation(ParentWindow, CSIDL_DESKTOP, 0,
        0, APIDL));
  end;

  function CheckPidlEnumerable(AValue: PItemIDList; out AFolder: IShellFolder;
    out AIsDesktop: Boolean): Boolean;
  var
    pEnum: IEnumIDList;
  ...
  begin
    ...
    AFolder := nil;
    try
      Result := Succeeded(GetDesktopIShellFolder.BindToObject(Value, nil,
        IID_IShellFolder, AFolder)) and
        Succeeded(AFolder.EnumObjects(ParentWindow, 0, pEnum)) and
        Assigned(pEnum) or GetDesktopPidl(ADesktopPidl) and
        EqualPIDLs(Value, ADesktopPidl);
...
var
  ...
  AIsDesktop: Boolean;
begin
  if (Value = nil) or not CheckPidlEnumerable(Value, AFolder, AIsDesktop) then
    Exit;
  if FPidl <> nil then
  begin
    DisposePidl(FPidl);
  ...
  begin
    FAttributes := 0;
    if Failed(GetDesktopIShellFolder.GetAttributesOf(1, FPidl, FAttributes))
    then
      FAttributes := 0;
  end;
  RootUpdated;
...
begin
  Result := '';
  if (Pidl <> nil) and Succeeded(GetDesktopIShellFolder.GetDisplayNameOf(Pidl,
    SHGDN_NORMAL or SHGDN_FORPARSING, AStrName)) then
...
function TcxCustomItemProducer.CreateShellItemInfo(APIDL: PItemIDList;
  AFast: Boolean): TcxShellItemInfo;
begin
  Result := TcxShellItemInfo.Create(Self, ShellFolder, FFolderPidl,
    APIDL, AFast);
end;
...
function TcxCustomItemProducer.GetThumbnailIndex
...
function TcxCustomItemProducer.InternalAddItem(APIDL: PItemIDList)
  : TcxShellItemInfo;
begin
...
begin
  Result := (ShellFolder <> nil) and
    Succeeded(ShellFolder.EnumObjects(Owner.ParentWindow, GetEnumFlags, pEnum))
    and Assigned(pEnum);
end;
...
begin
  ...
  try
    if Failed(GetDesktopIShellFolder.GetAttributesOf(1, ATempPIDL, Attr)) then
      Attr := 0;
    Details.ProcessDetails(CharWidth, ShellFolder, (Attr and SFGAO_FILESYSTEM)
      = SFGAO_FILESYSTEM);
  finally
    DisposePidl(ATempPIDL);
  end;
...
begin
  ...
  begin
    ...
    try
      if Succeeded(ShellFolder.GetUIObjectOf(Handle, 1, ATempPIDL, IQueryInfo,
        nil, AQueryInfo)) and Succeeded(AQueryInfo.GetInfoTip(0, AInfoStr)) and
        (AInfoStr <> nil) then
      begin
        GetInfoTip(AInfoStr);
...
function TcxCustomItemProducer.GetItemByPidl(APIDL: PItemIDList)
  : TcxShellItemInfo;
var
  AItemIndex: Integer;
begin
...
procedure TcxShellItemsInfoGatherer.ClearFetchQueue(AItemProducer
  : TcxCustomItemProducer);
...
begin
  ...
  if FIsLink and Succeeded(FParentShellFolder.BindToObject(FRelativePIDL, nil,
    IID_IShellLink, ALink)) and Succeeded(ALink.GetIDList(ATargetPidl)) then
  begin
    if IsWinVistaOrLater then
      FIsFolderLink := Succeeded(SHCreateItemFromIDList(ATargetPidl,
        IID_IShellItem, AShellItem)) and
        Succeeded(AShellItem.GetAttributes(SFGAO_FOLDER or SFGAO_STREAM, AAttr))
    else
    begin
      AParentTargetPidl := GetPidlParent(ATargetPidl);
      ARelativeTargetPidl := GetLastPidlItem(ATargetPidl);
      try
        FIsFolderLink :=
          Succeeded(GetDesktopIShellFolder.BindToObject(AParentTargetPidl, nil,
          IID_IShellFolder, AParentTargetShellFolder)) and
          Succeeded(AParentTargetShellFolder.GetAttributesOf(1,
          ARelativeTargetPidl, AAttr));
      finally
        DisposePidl(ARelativeTargetPidl);
        DisposePidl(AParentTargetPidl);
      end;
    end;
    FIsFolderLink := FIsFolderLink and
      ((AAttr and SFGAO_FOLDER) = SFGAO_FOLDER);
    FIsZipFolderLink := FIsFolderLink and
      ((AAttr and (SFGAO_FOLDER or SFGAO_STREAM)) = (SFGAO_FOLDER or
      SFGAO_STREAM));
  end;
end;
...
function TcxShellFolder.GetCapabilities: TcxShellFolderCapabilities;
  ...
  procedure CheckCapability(AShellAttributes, ACapabilityShellAttribute
    : LongWord; ACapability: TcxShellFolderCapability);
  begin
...
begin
  Result := InternalGetDisplayName(ParentShellFolder, RelativePIDL,
    SHGDN_INFOLDER);
end;
...
function TcxShellFolder.GetPathName: string;
  ...
  begin
    Result := InternalGetDisplayName(GetDesktopIShellFolder, AbsolutePIDL,
      ANameType);
  end;
...
begin
  AShellAttributes := GetShellAttributes(SFGAO_BROWSABLE or SFGAO_COMPRESSED or
    SFGAO_ENCRYPTED or SFGAO_NEWCONTENT or SFGAO_NONENUMERATED or
    SFGAO_REMOVABLE);
  Result := [];
...
function TcxShellFolder.GetShellAttributes(ARequestedAttributes: LongWord)
  : LongWord;
begin
...
begin
  if GetPidlItemsCount(AbsolutePIDL) = 0 then
    ...
    GetDesktopIShellFolder.BindToObject(AbsolutePIDL, nil,
      IID_IShellFolder, Result);
end;

function TcxShellFolder.GetStorageCapabilities
  : TcxShellFolderStorageCapabilities;

  procedure CheckStorageCapability(AShellAttributes,
    AStorageCapabilityShellAttribute: LongWord;
    AStorageCapability: TcxShellFolderStorageCapability);
  begin
    if HasShellAttribute(AShellAttributes, AStorageCapabilityShellAttribute)
    then
...
begin
  ...
  Result := [];
  CheckStorageCapability(AShellAttributes, SFGAO_FILESYSANCESTOR,
    sfscFileSysAncestor);
  ...
  CheckStorageCapability(AShellAttributes, SFGAO_STORAGEANCESTOR,
    sfscStorageAncestor);
...
function TcxShellFolder.HasShellAttribute(AAttributes,
  AAttribute: LongWord): Boolean;
begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          ...
          try
            ...
            cxShellGetThreadSafeFileInfo(pszName, FILE_ATTRIBUTE_NORMAL,
              AFileInfo, SizeOf(TSHFileInfo), SHGFI_SYSICONINDEX or
              SHGFI_USEFILEATTRIBUTES);
        ...
        begin
          ...
          cxShellGetThreadSafeFileInfo(PChar(AfqPidl), 0, AFileInfo,
            SizeOf(AFileInfo), AFlags);
          FIconIndex := AFileInfo.iIcon;
        end;
      end
      else
      begin
        ...
        if FIsFolder then
          cxShellGetThreadSafeFileInfo(PChar(AfqPidl), 0, AFileInfo,
            SizeOf(AFileInfo), AFlags or SHGFI_OPENICON);
        FOpenIconIndex := AFileInfo.iIcon;
...
procedure TcxShellItemInfo.FetchDetails(wnd: HWND; ShellFolder: IShellFolder;
  DetailsMap: TcxShellDetails);
  ...
  begin
    Result := GetRegStringValue
      (GetRegStringValue(ExtractFileExt(AFileName), ''), '');
  end;
...
var
  ...
  AFileSize: record case Integer of 0: (l, h: Cardinal);
  1: (c: int64);
end;
AFindFileHandle:
THandle;
...
begin
  ...
  begin
    ...
    begin
      ...
      if AShellFolder2.GetDetailsOf(Pidl, APDetailItem.ID, AColumnDetails) = S_OK
      then
        Details.Add(GetTextFromStrRet(AColumnDetails.str, Pidl))
      else if MapColumnToSCID(AShellFolder2, APDetailItem.ID, AShColumnID) and
  ...
  else if Succeeded(GetShellDetails(ShellFolder, Pidl, AShellDetails)) then
  begin
    for I := 0 to DetailsMap.Count - 1 do
    begin
      APDetailItem := DetailsMap[I];
      if APDetailItem.ID = 0 then
        Continue; // Name column already exists
      if AShellDetails.GetDetailsOf(Pidl, APDetailItem.ID, AColumnDetails) = S_OK
      then
        Details.Add(GetTextFromStrRet(AColumnDetails.str, Pidl))
      else
        Details.Add('');
  ...
  else if IsFilesystem then
  begin
    if Failed(ShellFolder.GetDisplayNameOf(Pidl, SHGDN_NORMAL or
      SHGDN_FORPARSING, AStrPath)) then
      Exit;
    ...
    try
      ...
      if AFindFileHandle <> INVALID_HANDLE_VALUE then
        try
          ...
          Details.Add
            (DateTimeToStr(cxFileTimeToDateTime(AFileInfo.ftLastWriteTime)));
        finally
...
begin
  ...
  begin
    ...
    cxShellGetThreadSafeFileInfo(PChar(FullPIDL), 0, AFileInfo,
      SizeOf(AFileInfo), AFlags);
...
procedure TcxShellItemInfo.SetNewPidl(pFolder: IShellFolder;
  AFolderPidl, APIDL: PItemIDList);
begin
  if APIDL = nil then
    Exit;
...
var
  ...
  procedure StoreCurrentState(const AMaskPos, ANamePos: Integer;
    AAnySym: Boolean);
  begin
  ...
  procedure RestoreCurrentState(out AMaskPos, ANamePos: Integer;
    out AAnySym: Boolean);
  begin
  ...
  begin
    ...
    begin
      case AMask[AMaskPos] of
        ...
        begin
          AFirstSym := AMaskPos;
          while (AMaskPos < AMaskLength) and (AMask[AMaskPos + 1] <> '*') and
            (AMask[AMaskPos + 1] <> '?') do
            Inc(AMaskPos);
          P := Pos(Copy(AMask, AFirstSym, AMaskPos - AFirstSym + 1),
            Copy(AFileName, ANamePos, 260));
    ...
    Result := (ANamePos > ANameLength) or AAnySym or (AParseStates.Count > 0)
      and Parse(AMask, AFileName);
  end;
...
const
  AAlignment: array [0 .. 2] of TAlignment = (taLeftJustify, taRightJustify,
    taCenter);
var
  AColumnDetails: TShellDetails;
  ...
  begin
    ANewColumn := Add;
    SetItemInfo(ANewColumn, AText, ADetails.cxChar,
      AAlignment[ADetails.fmt], AIndex);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Succeeded(SF2.GetDefaultColumnState(AColumnIndex - 1, AColumnFlags))
      then
      begin
        ADefaultColumns := ADefaultColumns or
          (AColumnFlags and SHCOLSTATE_ONBYDEFAULT = SHCOLSTATE_ONBYDEFAULT);
        if not IsWinXPOrLater and ADefaultColumns and
          (AColumnFlags and SHCOLSTATE_ONBYDEFAULT <> SHCOLSTATE_ONBYDEFAULT)
        then
          Break;
        if (AColumnFlags and SHCOLSTATE_ONBYDEFAULT <> SHCOLSTATE_ONBYDEFAULT)
          or (AColumnFlags and SHCOLSTATE_HIDDEN = SHCOLSTATE_HIDDEN) then
          Continue;
      end;
      AddItem(AColumnDetails, AColumnIndex - 1, AText);
    end;
  end
  else if GetShellDetails(AShellFolder, nil, sd) = S_OK then
  begin
...
begin
  ...
  else if ((grfKeyState and MK_LBUTTON) <> MK_LBUTTON) and
...
begin
  ...
  cxSHGetPathFromIDListW := GetProcAddress(ShellLibrary,
  ...
  dxSHGetFolderTypeFromCanonicalName := GetProcAddress(ShellLibrary,
    PChar(824));
  ShlwapiLibrary := LoadLibrary('shlwapi.dll');
  dxSHGetViewStatePropertyBag := GetProcAddress(ShlwapiLibrary,
...
procedure TcxShellCustomContextMenu.AddDefaultShellItems(AIShellFolder
  : IShellFolder);
begin
  FContextMenu := nil;
  if Succeeded(AIShellFolder.CreateViewObject(WindowHandle, IID_IContextMenu,
    FContextMenu)) then
    DoAddDefaultShellItems;
end;

procedure TcxShellCustomContextMenu.AddDefaultShellItems(AIShellFolder
  : IShellFolder; AItemPIDLList: TList);
var
...
begin
  if (FContextMenu <> nil) and
    Succeeded(FContextMenu.QueryInterface(IID_IContextMenu2, AContextMenu2))
  then
...
procedure TcxShellCustomContextMenu.ExecuteMenuItemCommand
  (ACommandId: Cardinal);
var
  AInvokeCommandInfo: TCMInvokeCommandInfo;
begin
  if (FContextMenu <> nil) and (ACommandId >= FFirstInvokeShellCommandIndex)
  then
  begin
    ...
    AInvokeCommandInfo.lpVerb :=
      MakeIntResourceA(ACommandId - FFirstInvokeShellCommandIndex);
