...
{$ENDIF}
  ...
  cxGeometry, cxGraphics, dxGDIPlusClasses, dxCoreGraphics,
  cxLookAndFeelPainters, cxFormats, cxContainer, cxEdit,
  cxEditConsts, cxFilterControlUtils, cxDateUtils, dxWheelPicker,
...
type
  ...
  TdxDateTimeWheelPickerItemViewInfo = class
  ...
  TdxDateTimeWheelPickerWheelViewInfo = class
  ...
  protected
    ...
    procedure SetValueByDateTime(ADateTime: TcxDateTime;
      AUse24HourFormat: Boolean);
    procedure SynchronizeDateTime(var ADateTime: TcxDateTime;
      var ATimeSuffix: Integer);
  end;
  ...
  strict private
  const
    TimeFormatItemKindToDateTimePickerWheel: array [dtikYear .. dtikSec]
      of TdxDateTimePickerWheel = (pwYear, pwMonth, pwDay, pwHour, pwMinute,
      pwSecond);
  strict private
    ...
    function GetRightToLeftConvertedWheelKind
    ...
    procedure InitializeDayWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
    procedure InitializeHourWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
    procedure InitializeMinuteWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
    procedure InitializeMonthWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
    procedure InitializeSecondWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
    procedure InitializeTimeSuffixWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
    procedure InitializeYearWheel(AWheelViewInfo
      : TdxDateTimeWheelPickerWheelViewInfo);
  protected
    ...
    procedure CalculateTextSize(AViewInfo
      : TdxCustomWheelPickerViewInfo); override;
    procedure CalculateWheelWidth(AViewInfo
      : TdxCustomWheelPickerViewInfo); override;
  ...
  public
    ...
    function GetDisplayText(const AEditValue: Variant;
  ...
  published
    property AssignedValues: TcxDateTimeWheelPickerPropertiesValues
    ...
    property Wheels: TdxDateTimePickerWheels read FWheels write SetWheels
      stored IsWheelsStored;
    property LineCount;
    property OnChange;
  ...
  public
    ...
    property ActiveProperties: TdxDateTimeWheelPickerProperties
...
constructor TdxDateTimeWheelPickerItemViewInfo.Create(AWheelPickerViewInfo
...
begin
  ...
  begin
    ...
    FLowerText := (WheelViewInfo as TdxDateTimeWheelPickerWheelViewInfo)
      .GetItemLowerText(Bounds, FIndex);
  end
  else
    FLowerText := '';
...
procedure TdxDateTimeWheelPickerItemViewInfo.DrawText(AGPCanvas: TdxGPCanvas);
  ...
  begin
    ...
    begin
      AFullSize := cxRectHeight(TdxDateTimeWheelPickerWheelViewInfo
        (WheelViewInfo).FCentralRowRect);
      AOffset := Abs(Bounds.Top - TdxDateTimeWheelPickerWheelViewInfo
        (WheelViewInfo).FCentralRowRect.Top);
      Result := Trunc((AFullSize - AOffset) / AFullSize * 255);
    end;
  end;

const
  ADefaultAlignment: array [Boolean] of TAlignment = (taLeftJustify,
    taRightJustify);
var
  ARTLReading: Boolean;
begin
  ...
  begin
    ...
    dxGPDrawText(dxGPPaintCanvas, FLowerText, FLowerTextRect, FLowerFont,
      dxColorToAlphaColor(GetTextColor, GetTextColorAlpha),
      ADefaultAlignment[ARTLReading], taVerticalCenter, False,
      TextRenderingHintSystemDefault, StringTrimmingNone, ARTLReading);
  end;
end;
...
function TdxDateTimeWheelPickerWheelViewInfo.GetItemLowerText
  (const ABounds: TRect; AIndex: Integer): string;
var
...
begin
  if TdxDateTimeWheelPickerViewInfo(WheelPickerViewInfo).FIsNullEditValue then
    ...
    ADateTime := (WheelPickerViewInfo as TdxDateTimeWheelPickerViewInfo)
      .FDateTime;
  ...
  if cxRectIntersect(ABounds, GetCentralRowRect) and
    cxGetLocalCalendar.IsValidDay(ADateTime.Year, ADateTime.Month, ADateTime.Day)
  then
    case FKind of
      dtikMin:
        Result := cxGetResourceString(@sdxDateTimeWheelPickerMinutes);
      dtikDay:
        Result := FormatDateTime('dddd',
          cxGetLocalCalendar.GetWeekDay(ADateTime) + 1);
      dtikSec:
        Result := cxGetResourceString(@sdxDateTimeWheelPickerSeconds);
      dtikHour:
        Result := cxGetResourceString(@sdxDateTimeWheelPickerHours);
      dtikMonth:
        Result := cxGetLocalMonthName(ADateTime.Year, ADateTime.Month,
          cxGetLocalCalendar);
    end
  else
    Result := '';
...
procedure TdxDateTimeWheelPickerWheelViewInfo.SetValueByDateTime
  (ADateTime: TcxDateTime; AUse24HourFormat: Boolean);
...
procedure TdxDateTimeWheelPickerWheelViewInfo.SynchronizeDateTime
  (var ADateTime: TcxDateTime; var ATimeSuffix: Integer);
begin
  case FKind of
    dtikSec:
...
class function TdxDateTimeWheelPickerWheelViewInfo.
  GetWheelPickerItemViewInfoClass: TdxCustomWheelPickerItemViewInfoClass;
begin
  Result := TdxDateTimeWheelPickerItemViewInfo;
end;

function TdxDateTimeWheelPickerWheelViewInfo.GetItemText
  (AItemIndex: Integer): string;

  procedure CalculateHoursText(out AText: string);
  begin
    if not TdxDateTimeWheelPickerViewInfo(WheelPickerViewInfo).FUse24HourFormat
    then
    begin
...
begin
  ...
  case FKind of
    ...
    dtikMonth:
      if TdxDateTimeWheelPickerViewInfo(WheelPickerViewInfo).FIsSmallItemSize
      then
...
function TdxDateTimeWheelPickerWheelViewInfo.CanTyping(AValue: Integer)
  : Boolean;
var
...
begin
  ...
  AWheel := TdxCustomWheelPickerViewInfoAccess(WheelPickerViewInfo)
    .SelectedWheel as TdxDateTimeWheelPickerWheelViewInfo;
  Result := (AValue * 10 <= MaxValue) and (ATypingStringLength <> 4) and
    ((ATypingStringLength <> 1) or (AWheel.FKind <> dtikTimeSuffix)) and
    ((ATypingStringLength <> 2) or (AWheel.FKind = dtikYear));
end;

function TdxDateTimeWheelPickerWheelViewInfo.NeedProcessKey(AKey: Word)
  : Boolean;
begin
...
class function TdxDateTimeWheelPickerViewInfo.GetWheelPickerWheelViewInfoClass
...
function TdxDateTimeWheelPickerViewInfo.GetInternalValue: TcxEditValue;
  ...
  begin
    ...
    FDateTime.Day := Max(1, Min(FDateTime.Day, DaysPerMonth(FDateTime.Year,
      FDateTime.Month)));
...
begin
  for I := 0 to FWheels.Count - 1 do
    (FWheels[I] as TdxDateTimeWheelPickerWheelViewInfo)
      .SynchronizeDateTime(FDateTime, ATimeSuffix);
...
begin
  for I := 0 to FWheels.Count - 1 do
    (FWheels[I] as TdxDateTimeWheelPickerWheelViewInfo)
      .SetValueByDateTime(FDateTime, FUse24HourFormat);
end;

procedure TdxDateTimeWheelPickerViewInfo.SetDateInValidRangeByDay;
begin
  if (FDateTime.Year = FMaxDate.Year) and (FDateTime.Month = FMaxDate.Month) and
    (FDateTime.Day > FMaxDate.Day) then
    FDateTime.Month := Min(FDateTime.Month, FMaxDate.Month - 1)
  else if (FDateTime.Year = FMinDate.Year) and
    (FDateTime.Month = FMinDate.Month) and (FDateTime.Day < FMinDate.Day) then
...
procedure TdxDateTimeWheelPickerViewData.Calculate(ACanvas: TcxCanvas;
...
procedure TdxDateTimeWheelPickerViewData.CalculateTextSize
  ...
  begin
    Result := 0;
    for I := 1 to 12 do
      Result := Max(Result, dxGPGetTextSize(dxFormatSettings.LongMonthNames[I],
        Style.Font).cx);
  end;
  ...
  begin
    Result := 0;
    for I := 1 to 7 do
      Result := Max(Result, dxGPGetTextSize(dxFormatSettings.LongDayNames[I],
        Style.Font).cx);
  end;
...
procedure TdxDateTimeWheelPickerViewData.InitializeWheel(AWheelViewInfo
  : TdxCustomWheelPickerWheelViewInfo; AIndex: Integer);
var
  ADateTimeWheelViewInfo: TdxDateTimeWheelPickerWheelViewInfo;
begin
  inherited;
  ADateTimeWheelViewInfo :=
    AWheelViewInfo as TdxDateTimeWheelPickerWheelViewInfo;
...
begin
  Result := (FWheelWidth + GetWheelIndent) * (GetWheelCount - 1) +
    (FMonthWheelWidth + GetWheelIndent) + GetWheelIndent
end;
...
procedure TdxDateTimeWheelPickerViewData.CalculateWheelWidth
  (AViewInfo: TdxCustomWheelPickerViewInfo);
begin
...
function TdxDateTimeWheelPickerViewData.GetProperties
  : TdxDateTimeWheelPickerProperties;
begin
...
function TdxDateTimeWheelPickerViewData.GetRightToLeftConvertedWheelKind
  ...
  begin
    ...
    else if AWheelKind = ASwapWheelKind2 then
...
begin
  ...
  AIsDayWheelVisible := TimeFormatItemKindToDateTimePickerWheel[dtikDay]
    in Properties.Wheels;
  AIsMonthWheelVisible := TimeFormatItemKindToDateTimePickerWheel[dtikMonth]
    in Properties.Wheels;
  AIsYearWheelVisible := TimeFormatItemKindToDateTimePickerWheel[dtikYear]
    in Properties.Wheels;

  if AIsDayWheelVisible and AIsYearWheelVisible then
    SwapWheelKinds(Result, dtikDay, dtikYear)
  else if AIsMonthWheelVisible then
  begin
    if AIsDayWheelVisible then
      SwapWheelKinds(Result, dtikDay, dtikMonth)
    else if AIsYearWheelVisible then
      SwapWheelKinds(Result, dtikMonth, dtikYear);
  end;
end;

function TdxDateTimeWheelPickerViewData.GetWheelKind(AIndex: Integer)
...
begin
  ...
  begin
    ...
    if (AKind in [dtikYear .. dtikSec]) and
      (TimeFormatItemKindToDateTimePickerWheel[AKind] in Properties.Wheels) then
    begin
...
procedure TdxDateTimeWheelPickerViewData.InitializeDayWheel(AWheelViewInfo
  : TdxDateTimeWheelPickerWheelViewInfo);
var
  ADayCount: Integer;
  AViewInfo: TdxDateTimeWheelPickerViewInfo;
begin
  AViewInfo := AWheelViewInfo.WheelPickerViewInfo as
    TdxDateTimeWheelPickerViewInfo;
  if AViewInfo.FIsNullEditValue then
    ADayCount := DaysPerMonth(cxGetLocalCalendar.FromDateTime(Today).Year,
      cxGetLocalCalendar.FromDateTime(Today).Month)
  else
  ...
  begin
    AWheelViewInfo.FWheelItemCount := Min(ADayCount, MaxDate.Day) -
      MinDate.Day + 1;
    AWheelViewInfo.FFirstItemValue := MinDate.Day;
  end;
end;

procedure TdxDateTimeWheelPickerViewData.InitializeHourWheel(AWheelViewInfo
  : TdxDateTimeWheelPickerWheelViewInfo);
begin
...
procedure TdxDateTimeWheelPickerViewData.InitializeMinuteWheel
  (AWheelViewInfo: TdxDateTimeWheelPickerWheelViewInfo);
begin
  AWheelViewInfo.FWheelItemCount := MinsPerHour;
  AWheelViewInfo.FFirstItemValue := 0;
end;

procedure TdxDateTimeWheelPickerViewData.InitializeMonthWheel
  (AWheelViewInfo: TdxDateTimeWheelPickerWheelViewInfo);
begin
...
procedure TdxDateTimeWheelPickerViewData.InitializeSecondWheel
  (AWheelViewInfo: TdxDateTimeWheelPickerWheelViewInfo);
begin
...
procedure TdxDateTimeWheelPickerViewData.InitializeTimeSuffixWheel
  (AWheelViewInfo: TdxDateTimeWheelPickerWheelViewInfo);
begin
  AWheelViewInfo.FWheelItemCount := 2;
end;

procedure TdxDateTimeWheelPickerViewData.InitializeYearWheel(AWheelViewInfo
  : TdxDateTimeWheelPickerWheelViewInfo);
begin
...
class function TdxDateTimeWheelPickerProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TdxDateTimeWheelPicker;
end;

function TdxDateTimeWheelPickerProperties.GetDisplayText(const AEditValue
  : Variant; AFullText, AIsInplace: Boolean): string;
var
...
class function TdxDateTimeWheelPickerProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TdxDateTimeWheelPickerViewInfo;
end;

procedure TdxDateTimeWheelPickerProperties.PrepareDisplayValue(const AEditValue
...
begin
  ...
  else if VarIsNumericEx(AEditValue) then
    DisplayValue := VarToDateTime(AEditValue)
  else if VarIsNull(AEditValue) then
    DisplayValue := Null
  else
    DisplayValue := NullDate;
...
procedure TdxDateTimeWheelPickerProperties.DoAssign
...
class function TdxDateTimeWheelPickerProperties.GetAssignedValuesClass
  : TcxCustomEditPropertiesValuesClass;
begin
  Result := TcxDateTimeWheelPickerPropertiesValues;
end;

class function TdxDateTimeWheelPickerProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TdxDateTimeWheelPickerViewData;
end;

function TdxDateTimeWheelPickerProperties.CorrectDateByRange
...
begin
  if AssignedValues.MaxDate and (MaxDate <> NullDate) and (MinDate < MaxDate)
  then
    Result := MaxDate
  else
    Result := MaxDateTime;
...
begin
  if AssignedValues.MinDate and (MinDate <> NullDate) and (MinDate < MaxDate)
  then
    Result := MinDate
  else
    Result := MinDateTime;
...
function TdxDateTimeWheelPickerProperties.GetAssignedValues
  : TcxDateTimeWheelPickerPropertiesValues;
begin
...
procedure TdxDateTimeWheelPickerProperties.SetAssignedValues
  (AValue: TcxDateTimeWheelPickerPropertiesValues);
begin
...
procedure TdxDateTimeWheelPickerProperties.SetWheels
  (AValue: TdxDateTimePickerWheels);
begin
...
class function TdxCustomDateTimeWheelPicker.GetPropertiesClass
...
function TdxCustomDateTimeWheelPicker.GetActiveProperties
  : TdxDateTimeWheelPickerProperties;
begin
  Result := InternalGetActiveProperties as TdxDateTimeWheelPickerProperties;
end;

function TdxCustomDateTimeWheelPicker.GetProperties
  : TdxDateTimeWheelPickerProperties;
begin
  Result := inherited Properties as TdxDateTimeWheelPickerProperties;
end;

procedure TdxCustomDateTimeWheelPicker.SetProperties
  (AValue: TdxDateTimeWheelPickerProperties);
begin
...
class function TdxDateTimeWheelPicker.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TdxDateTimeWheelPickerProperties;
end;

function TdxDateTimeWheelPicker.GetActiveProperties
  : TdxDateTimeWheelPickerProperties;
begin
  Result := InternalGetActiveProperties as TdxDateTimeWheelPickerProperties;
end;
...
procedure TdxDateTimeWheelPicker.SetProperties
  (AValue: TdxDateTimeWheelPickerProperties);
begin
  Properties.Assign(AValue);
end;
...
GetRegisteredEditProperties.Register(TdxDateTimeWheelPickerProperties,
  scxSEditRepositoryDateTimeWheelPickerItem);
FilterEditsController.Register(TdxDateTimeWheelPickerProperties,
  TcxFilterDateEditHelper);

finalization

FilterEditsController.Unregister(TdxDateTimeWheelPickerProperties,
  TcxFilterDateEditHelper);
