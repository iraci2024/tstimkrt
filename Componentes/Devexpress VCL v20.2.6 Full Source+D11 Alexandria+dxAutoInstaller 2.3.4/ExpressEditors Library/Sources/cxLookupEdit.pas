...
type
  ...
  TcxCustomLookupEditLookupData = class(TcxInterfacedPersistent, IUnknown,
  ...
  protected
    ...
    procedure Go(ADirection: TcxEditLookupDataGoDirection;
      ACircular: Boolean); virtual;
  ...
  protected
    ...
    function FindByText(AItemIndex: Integer; const AText: string;
      APartialCompare: Boolean): Integer; virtual;
    function FindLookupText(const AText: string): Boolean; override;
    function GetDefaultIncrementalFilteringOptions
      : TcxTextEditIncrementalFilteringOptions; override;
    ...
    property DataController: TcxCustomDataController read GetDataController;
    property DisplayColumnIndex: Integer read GetDisplayColumnIndex
      write SetDisplayColumnIndex default 0;
  ...
  public
    ...
    property DropDownAutoSize: Boolean read GetDropDownAutoSize
      write SetDropDownAutoSize default False;
    property DropDownHeight: Integer read GetDropDownHeight
      write SetDropDownHeight default 0;
  ...
  protected
    ...
    property LookupData: TcxCustomLookupEditLookupData read GetLookupData
      write SetLookupData;
  public
    ...
    property ActiveProperties: TcxCustomLookupEditProperties
...
begin
  ...
  try
    ...
    Properties.LookupGridInitEvents(GridClick, GridFocusedRowChanged,
      GridCloseUp);
    Properties.LookupGridInitLookAndFeel(Edit.PopupControlsLookAndFeel,
      Edit.ViewInfo.BackgroundColor, Edit.VisibleFont);
...
function TcxCustomLookupEditLookupData.GetDisplayText
...
function TcxCustomLookupEditLookupData.GetVisualAreaPreferredSize
...
procedure TcxCustomLookupEditLookupData.Go(ADirection
  : TcxEditLookupDataGoDirection; ACircular: Boolean);
begin
  if (DataController = nil) or (DataController.RecordCount = 0) then
    Exit;
  DisableChanging;
  try
    ResetIncrementalFilter;
    ...
    try
      ...
      else if ADirection = egdEnd then
...
procedure TcxCustomLookupEditLookupData.Initialize(AVisualControlsParent
  : TWinControl);
begin
  if Properties.GetLookupGridControl <> nil then
...
function TcxCustomLookupEditLookupData.IsMouseOverList(const P: TPoint)
  : Boolean;
var
  APoint: TPoint;
begin
    ...
    begin
      ...
      begin
        if Properties.DropDownListStyle <> lsEditList then
          DataController.SetIncrementalFilter(AItemIndex,
            APrevIncrementalFilterText, IsLikeTypeFiltering);
        Result := -1;
      end;
    end;
    UpdateDropDownCount;
...
begin
  ...
  try
    ...
    begin
      // TODO: Next
      if FVisible and
        Properties.GetIncrementalFiltering { and (Properties.DropDownListStyle <> lsFixedList) }
      then
...
procedure TcxCustomLookupEditLookupData.PositionVisualArea
  (const AClientRect: TRect);
begin
  if Properties.GetLookupGridControl <> nil then
  begin
    with AClientRect do
      Properties.GetLookupGridControl.SetBounds(Left, Top, Right - Left,
        Bottom - Top);
...
procedure TcxCustomLookupEditLookupData.SetOnCurrentKeyChanged
  (Value: TNotifyEvent);
begin
...
procedure TcxCustomLookupEditLookupData.DoSetKeySelection
  (AKeySelected: Boolean);
begin
  if DataController <> nil then
...
procedure TcxCustomLookupEditLookupData.GridCloseUp(Sender: TObject;
  AAccept: Boolean);
begin
  if AAccept then
  begin
...
begin
  Result := (DataController <> nil) and (DataController.GetSelectedCount = 1)
    and (DataController.GetFocusedRowIndex = DataController.
...
var
  ...
  S: string;
begin
  if Properties.FInGridDataChanged then
    Exit;
  ASelected := False;
  AItemIndex := GetListIndex;
  ...
  begin
    if FVisible and Properties.GetIncrementalFiltering then
      ...
      else if Edit.FIsTextChanging and
        DataController.ChangeIncrementalFilterText(Edit.Text) then
        UpdateDropDownCount;
    ...
    // Properties.EditingStyle in [esEditList, esFixedList]
    begin
...
function TcxCustomLookupEditLookupData.GetDataController
...
function TcxCustomLookupEditLookupData.GetProperties
  : TcxCustomLookupEditProperties;
begin
...
begin
  Result := IsIncrementalFiltering and
    (ifoUseContainsOperator in Properties.IncrementalFilteringOptions);
end;
...
class function TcxCustomLookupEditProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxCustomLookupEdit;
end;

function TcxCustomLookupEditProperties.GetEditValueSource(AEditFocused: Boolean)
...
function TcxCustomLookupEditProperties.GetSupportedOperations
...
function TcxCustomLookupEditProperties.IsEditValueValid(var EditValue
...
procedure TcxCustomLookupEditProperties.PrepareDisplayValue(const AEditValue
...
procedure TcxCustomLookupEditProperties.DoAssign(AProperties
...
function TcxCustomLookupEditProperties.GetLookupGridColumnProperties
...
function TcxCustomLookupEditProperties.GetLookupGridDataController
  : TcxCustomDataController;
begin
  Result := nil;
end;

function TcxCustomLookupEditProperties.
...
function TcxCustomLookupEditProperties.GetLookupGridPopupHeight
  (ADropDownRowCount: Integer): Integer;
begin
  Result := 0;
end;

function TcxCustomLookupEditProperties.IsLookupGridMouseOverList
...
procedure TcxCustomLookupEditProperties.LookupGridDroppedDown
  (const AFindStr: string);
begin
end;

procedure TcxCustomLookupEditProperties.LookupGridInitEvents(AOnClick,
  AOnFocusedRowChanged: TNotifyEvent; AOnCloseUp: TcxLookupGridCloseUpEvent);
begin
end;
...
procedure TcxCustomLookupEditProperties.LookupGridInitLookAndFeel
...
begin
  Result := DataController.FindRecordIndexByText(0, AItemIndex, AText,
    APartialCompare, False, True);
end;

function TcxCustomLookupEditProperties.FindLookupText
...
function TcxCustomLookupEditProperties.GetDisplayFormatOptions
  : TcxEditDisplayFormatOptions;
begin
  Result := [];
end;

function TcxCustomLookupEditProperties.GetDisplayLookupText
  (const AKey: TcxEditValue): string;
begin
  Result := '';
end;
...
class function TcxCustomLookupEditProperties.GetLookupDataClass
...
begin
  if FInGridDataChanged or IsChangeLocked then
    Exit;
  if IsDataChangedPostponed then
    Exit;
...
function TcxCustomLookupEditProperties.
  IsEditValueConversionDependOnFocused: Boolean;
begin
  Result := False;
end;
...
function TcxCustomLookupEditProperties.IsPopupKey(Key: Word;
...
begin
  if Value >= GetLookupGridColumnCount then
    Value := GetLookupGridColumnCount - 1;
  if Value < 0 then
    Value := 0;
...
function TcxCustomLookupEditProperties.GetDataController
  : TcxCustomDataController;
begin
  Result := GetLookupGridDataController;
end;

function TcxCustomLookupEditProperties.GetDefaultIncrementalFilteringOptions
...
class function TcxCustomLookupEdit.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxCustomLookupEditProperties;
end;

procedure TcxCustomLookupEdit.PrepareEditValue(const ADisplayValue
  : TcxEditValue; out EditValue: TcxEditValue; AEditFocused: Boolean);
begin
  if ActiveProperties.IsPickMode then
    EditValue := ADisplayValue
  else if InternalCompareString
    (ILookupData.GetDisplayText(ILookupData.CurrentKey), ADisplayValue,
    ActiveProperties.CharCase = ecNormal) then
...
procedure TcxCustomLookupEdit.SetLookupData
  (Value: TcxCustomLookupEditLookupData);
begin
  FLookupData.Assign(Value);
end;

procedure TcxCustomLookupEdit.SetProperties
  (Value: TcxCustomLookupEditProperties);
begin
