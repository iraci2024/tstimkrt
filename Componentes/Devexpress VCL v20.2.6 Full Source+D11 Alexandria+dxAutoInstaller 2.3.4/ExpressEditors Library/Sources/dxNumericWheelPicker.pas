...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Graphics, Classes, cxGraphics, dxGDIPlusClasses, cxLookAndFeelPainters,
  cxEdit, cxContainer, cxDataStorage, cxSpinEdit,
...
type
  ...
  TdxCustomNumericWheelPickerItemViewInfo = class
  ...
  TdxCustomNumericWheelPickerWheelViewInfo = class
  ...
  strict private
    function GetFontSize(AViewInfo
      : TdxCustomNumericWheelPickerViewInfo): Integer;
  ...
  protected
    ...
    procedure CalculateWheels(AViewInfo: TdxCustomWheelPickerViewInfo);
      override;
  ...
  public
    ...
    function GetDisplayText(const AEditValue: Variant;
      AFullText, AIsInplace: Boolean): string; override;
  ...
  public
    ...
    property ActiveProperties: TdxNumericWheelPickerProperties
      read GetActiveProperties;
  published
    property Properties: TdxNumericWheelPickerProperties read GetProperties
      write SetProperties;
...
function dxGPGetTextSize(AGPCanvas: TdxGPGraphics; const AText: string;
  AFont: TFont): TSize; overload;
function dxGPGetTextSize(const AText: string; AFont: TFont): TSize; overload;

implementation

uses
  SysUtils, Math, Variants, Windows, dxCore, cxVariants, cxGeometry,
  dxCoreGraphics, cxEditConsts,
...
function dxNumericWheelPickerGetNullValue(AMinValue, AMaxValue: Double)
  : Variant;
begin
...
var
  ARect: TRect;
begin
  dxGPGetTextRect(AGPCanvas, AText, AFont, False, Rect(0, 0, cxMaxRectSize,
    cxMaxRectSize), ARect);
...
constructor TdxCustomNumericWheelPickerItemViewInfo.Create(AWheelPickerViewInfo
...
procedure TdxCustomNumericWheelPickerItemViewInfo.DrawText
  (AGPCanvas: TdxGPCanvas);

  function GetTextColorAlpha: Byte;
  begin
    if (IsItemSelected or
      (WheelViewInfo as TdxCustomNumericWheelPickerWheelViewInfo).IsSelected)
      and not TdxCustomNumericWheelPickerViewInfo(WheelPickerViewInfo).FIsNullEditValue
    then
      Result := 255
    else
      Result := 130;
...
begin
  if HasText then
    dxGPDrawText(AGPCanvas, FText, FTextRect, FFont,
      dxColorToAlphaColor(GetTextColor, GetTextColorAlpha));
end;

function TdxCustomNumericWheelPickerItemViewInfo.GetTextColor: TColor;
begin
  if WheelPickerViewInfo.IsInplace and
    (IsItemSelected or
    (WheelViewInfo as TdxCustomNumericWheelPickerWheelViewInfo).IsSelected) then
    Result := clBtnText
  else
    Result := WheelPickerViewInfo.TextColor;
end;

procedure TdxCustomNumericWheelPickerItemViewInfo.InternalDrawContent
  (AGPCanvas: TdxGPCanvas);
begin
  DrawText(AGPCanvas);
end;

procedure TdxCustomNumericWheelPickerItemViewInfo.Calculate
  (const ABounds: TRect);
begin
...
procedure TdxCustomNumericWheelPickerItemViewInfo.DrawContent
...
class function TdxCustomNumericWheelPickerWheelViewInfo.
  GetWheelPickerItemViewInfoClass: TdxCustomWheelPickerItemViewInfoClass;
begin
  Result := TdxCustomNumericWheelPickerItemViewInfo;
end;

procedure TdxCustomNumericWheelPickerWheelViewInfo.Calculate
  (const ABounds: TRect);
begin
...
procedure TdxCustomNumericWheelPickerWheelViewInfo.InitializeItem
  (AItem: TdxCustomWheelPickerItemViewInfo; AIndex: Integer);
begin
  inherited InitializeItem(AItem, AIndex);
  (AItem as TdxCustomNumericWheelPickerItemViewInfo).FText :=
    GetItemText(AIndex);
end;
...
function TdxCustomNumericWheelPickerWheelViewInfo.GetItemText
  (AItemIndex: Integer): string;
begin
  Result := IntToStr(FFirstItemValue + AItemIndex);
end;

function TdxCustomNumericWheelPickerWheelViewInfo.CanTyping
  (AValue: Integer): Boolean;
var
  AMaxValue: Double;
begin
  AMaxValue := GetMaxValue;
  Result := (Length(TypingString) <> Max(Length(FloatToStr(AMaxValue)),
    Length(FloatToStr(GetMinValue)))) and (Abs(AValue) <= Abs(AMaxValue));
end;

function TdxCustomNumericWheelPickerWheelViewInfo.NeedProcessKey
  (AKey: Word): Boolean;
begin
...
procedure TdxCustomNumericWheelPickerWheelViewInfo.CalculateCyclic
  (const ABounds: TRect);
begin
...
class function TdxCustomNumericWheelPickerViewInfo.
  GetWheelPickerWheelViewInfoClass: TdxCustomWheelPickerWheelViewInfoClass;
begin
...
begin
  ...
  if FWheels.Count > 0 then
    Result := IntToStr
      ((FWheels[0] as TdxCustomNumericWheelPickerWheelViewInfo).Value);
end;

procedure TdxCustomNumericWheelPickerViewInfo.KeyDown(var AKey: Word;
  AShift: TShiftState);
begin
  inherited KeyDown(AKey, AShift);
  if (SelectedWheel <> nil) and not TdxCustomNumericWheelPickerWheelViewInfo
    (SelectedWheel).CanContinueTyping then
    SelectNextWheel(1);
end;
...
procedure TdxCustomNumericWheelPickerViewData.CalculateWheelWidth
  (AViewInfo: TdxCustomWheelPickerViewInfo);
begin
  FWheelWidth := FMaxTextSize.cx + 2 * ScaleFactor.Apply
    (dxWheelPickerItemContentMargin)
end;

procedure TdxCustomNumericWheelPickerViewData.CalculateTextSize
  (AViewInfo: TdxCustomWheelPickerViewInfo);
var
  AFont: TFont;
begin
  inherited;
  (AViewInfo as TdxCustomNumericWheelPickerViewInfo).FFontSize :=
    GetFontSize(AViewInfo as TdxCustomNumericWheelPickerViewInfo);
  AFont := TFont.Create;
  try
...
procedure TdxCustomNumericWheelPickerViewData.CalculateWheels
  (AViewInfo: TdxCustomWheelPickerViewInfo);
var
...
begin
  ...
  AWheelPickerViewInfo.FIsSmallItemSize := AWheelPickerViewInfo.FRowHeight <
    GetOptimalRowHeight;
...
procedure TdxCustomNumericWheelPickerViewData.InitializeWheel
  (AWheelViewInfo: TdxCustomWheelPickerWheelViewInfo; AIndex: Integer);
var
...
begin
  ...
  AViewInfo.FWheelItemCount :=
    Max(Round(GetRealMaxValue) - AViewInfo.FFirstItemValue, 0) + 1;
end;

function TdxCustomNumericWheelPickerViewData.InternalGetEditConstantPartSize
  (ACanvas: TcxCanvas; AIsInplace: Boolean;
  AEditSizeProperties: TcxEditSizeProperties; var MinContentSize: tagSIZE;
  AViewInfo: TcxCustomEditViewInfo): tagSIZE;
begin
...
begin
  ...
  Result := FWheelWidth * AWheelCount + GetWheelIndent * (AWheelCount + 1) + 2 *
...
function TdxCustomNumericWheelPickerViewData.GetWheelWidth
  (AIndex: Integer): Integer;
begin
  Result := FWheelWidth;
end;

function TdxCustomNumericWheelPickerViewData.GetFontSize
...
begin
  if TcxCustomEditPropertiesAccess(Properties).MinValue >
    TcxCustomEditPropertiesAccess(Properties).MaxValue then
...
class function TdxCustomNumericWheelPickerProperties.GetViewDataClass
...
class function TdxCustomNumericWheelPickerProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TdxCustomNumericWheelPicker;
end;

class function TdxCustomNumericWheelPickerProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TdxCustomNumericWheelPickerViewInfo;
end;

function TdxCustomNumericWheelPickerProperties.GetSupportedOperations
  : TcxEditSupportedOperations;
begin
  Result := [esoEditing, esoFiltering, esoSorting];
end;

function TdxCustomNumericWheelPickerProperties.GetDisplayText(const AEditValue
...
class function TdxCustomNumericWheelPicker.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TdxCustomNumericWheelPickerProperties;
end;

function TdxCustomNumericWheelPicker.GetEditStateColorKind
...
class function TdxNumericWheelPickerProperties.GetContainerClass
  : TcxContainerClass;
begin
...
class function TdxNumericWheelPicker.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TdxNumericWheelPickerProperties;
end;

function TdxNumericWheelPicker.GetActiveProperties
  : TdxNumericWheelPickerProperties;
begin
...
procedure TdxNumericWheelPicker.SetProperties
  (AValue: TdxNumericWheelPickerProperties);
begin
...
GetRegisteredEditProperties.Register(TdxNumericWheelPickerProperties,
  scxSEditRepositoryNumericWheelPickerItem);
FilterEditsController.Register(TdxNumericWheelPickerProperties,
  TcxFilterSpinEditHelper);

finalization

FilterEditsController.Unregister(TdxNumericWheelPickerProperties,
  TcxFilterSpinEditHelper);
