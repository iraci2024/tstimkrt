...
type
  ...
  public
    ...
    property Items[Index: Integer]: TcxCheckComboBoxItem read GetItems
  ...
  TcxCustomCheckComboBoxListBox = class(TcxCustomComboBoxListBox, IUnknown,
  ...
  protected
    ...
    procedure SynchronizeCheckStates(ANewHotCheckIndex, ANewPressedIndex
      : Integer);
  ...
  TcxCheckComboBoxIncrementalFilteringHelper = class
  ...
  protected
    function CreateIncrementalFilteringHelper
      : TcxComboBoxIncrementalFilteringHelper; override;
  ...
  protected
    ...
    function CalcTextRect(const ARect: TRect;
      const ACheckState: TcxCheckBoxState): TRect; virtual;
    function CalcCheckPoint(const ARect: TRect;
      const ACheckState: TcxCheckBoxState): TPoint; virtual;
    function IsClickInCheck(const AItemRect: TRect;
  ...
  protected
    ...
    procedure CalculateCheckStatesByEditValue(Sender: TObject;
      ...
      var ACheckStates: TcxCheckStates); virtual;
    function CalculateDisplayValueByCheckStates(const ACheckStates
      : TcxCheckStates): string; virtual;
  ...
  public
    ...
    property ShowEmptyText: Boolean read FShowEmptyText write SetShowEmptyText
      default True;
  ...
  protected
    ...
    property StatesItems: Boolean read FStatesItems write FStatesItems
      stored False;
  public
  ...
  protected
    class function GetEditValueFormat(AEditProperties: TcxCustomEditProperties)
      : TcxCheckStatesValueFormat; virtual;
    class function GetItems(AEditProperties: TcxCustomEditProperties)
      : IcxCheckItems; virtual;
    class procedure InitializeItems(AProperties, AEditProperties
  ...
  protected
    class function GetEditValueFormat(AEditProperties: TcxCustomEditProperties)
      : TcxCheckStatesValueFormat; override;
    class function GetItems(AEditProperties: TcxCustomEditProperties)
      : IcxCheckItems; override;
  end;
...
procedure TcxCheckComboBoxItems.SetItems(Index: Integer;
  const Value: TcxCheckComboBoxItem);
begin
...
function TcxCustomCheckComboBoxViewData.GetProperties
  : TcxCustomCheckComboBoxProperties;
begin
...
function TcxCustomCheckComboBoxListBox.GetItemHeight
...
procedure TcxCustomCheckComboBoxListBox.DrawItem(Index: Integer; Rect: TRect;
  ...
  begin
    if not AIsItemEnabled then
      Result := ecsDisabled
    else if FHotCheckIndex = Index then
      Result := ecsHot
    else if FPressedCheckIndex = Index then
      Result := ecsPressed
    else
      Result := ecsNormal;
  end;

  function GetCheckBorderStyle(ACheckState: TcxEditCheckState)
    : TcxEditCheckBoxBorderStyle;
  begin
...
function TcxCustomCheckComboBoxListBox.GetCheckRect(const R: TRect;
...
procedure TcxCustomCheckComboBoxListBox.InternalMouseMove(Shift: TShiftState;
  X, Y: Integer);
var
...
begin
  ...
  begin
    if Key = dxSpace then
    begin
      if not(GetCheckComboBoxItem(ItemIndex).Enabled) then
        Exit;
...
procedure TcxCustomCheckComboBoxListBox.LookAndFeelChanged
...
procedure TcxCustomCheckComboBoxListBox.MouseMove(Shift: TShiftState;
  X, Y: Integer);
var
...
procedure TcxCustomCheckComboBoxListBox.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  AItemIndex: Integer;
  ANewHotCheckIndex, ANewPressedCheckIndex: Integer;
...
begin
  FCheckSize := GetScaledEditCheckSize(Self.Canvas,
    Edit.PopupControlsLookAndFeel.NativeStyle, Edit.ActiveProperties.Glyph,
    Edit.ActiveProperties.GlyphCount, Edit.PopupControlsLookAndFeel.SkinPainter,
    Edit.ScaleFactor);
  FCheckBorderOffset := GetEditCheckBorderOffset
    (TcxContainerBorderStyle(Edit.FCheckBorderStyle), Edit.FNativeStyle,
    IsGlyphAssigned(Edit.ActiveProperties.Glyph) and
    (Edit.ActiveProperties.GlyphCount > 0), LookAndFeel.SkinPainter);
...
procedure TcxCustomCheckComboBoxListBox.SynchronizeCheckStates
  (ANewHotCheckIndex, ANewPressedIndex: Integer);
begin
...
function TcxCustomCheckComboBoxListBox.GetCheckComboBoxItem(AIndex: Integer)
  : TcxCheckComboBoxItem;
begin
  Result := Edit.ActiveProperties.Items
    [Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(AIndex)];
end;
...
function TcxCustomCheckComboBoxListBox.GetItemCheckState(AIndex: Integer)
  : TcxCheckBoxState;
begin
  Result := Edit.FStates
    [Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(AIndex)];
...
begin
  Edit.FStates[Edit.LookupData.GetLookupItemIndexFromFilteredItemIndex(AIndex)]
    := AState;
end;
...
begin
  if (Key = dxSpace) and (TcxCheckComboBoxLookupData(LookupData).ItemIndex <> -1)
  then
...
function TcxCheckComboBoxLookupData.CreateIncrementalFilteringHelper
  : TcxComboBoxIncrementalFilteringHelper;
begin
...
class function TcxCheckComboBox.GetPropertiesClass
...
function TcxCheckPaintHelper.GetNextState(Value: TcxCheckBoxState)
  : TcxCheckBoxState;
begin
  case Value of
    cbsChecked:
...
begin
  ...
  if not AUseRightToLeftAlignment then
    FCheckRect := Bounds(AItemRect.Left + FCenterPos,
      AItemRect.Top + FCenterPos, GetCheckWidth, GetCheckWidth)
  else
    FCheckRect := Bounds(AItemRect.Right - (FCenterPos * 3),
      AItemRect.Top + FCenterPos, GetCheckWidth, GetCheckWidth);
...
begin
  ...
  FShowEmptyText := True;
  FEmptySelectionText := cxGetResourceString
    (@cxSCheckComboBoxEmptySelectionText);
...
begin
  Result := FEmptySelectionText <> cxGetResourceString
    (@cxSCheckComboBoxEmptySelectionText);
end;
...
class function TcxCustomCheckComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxCheckComboBox;
end;

function TcxCustomCheckComboBoxProperties.GetEditValueSource
  (AEditFocused: Boolean): TcxDataEditValueSource;
begin
  Result := evsValue;
end;

function TcxCustomCheckComboBoxProperties.GetSupportedOperations
...
procedure TcxCustomCheckComboBoxProperties.DoAssign
...
class function TcxCustomCheckComboBoxProperties.GetLookupDataClass
  : TcxInterfacedPersistentClass;
begin
  Result := TcxCheckComboBoxLookupData;
end;

class function TcxCustomCheckComboBoxProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
  Result := TcxCheckComboBoxPopupWindow;
end;

class function TcxCustomCheckComboBoxProperties.GetViewDataClass
...
function TcxCustomCheckComboBoxProperties.GetStateCaption
...
procedure TcxCustomCheckComboBoxProperties.SetEditValueFormat
  (Value: TcxCheckStatesValueFormat);
begin
...
procedure TcxCustomCheckComboBoxProperties.SetItems
  (const Value: TcxCheckComboBoxItems);
begin
  FItems.Assign(Value);
  Changed;
end;
...
begin
  if (GetRealEditValueFormat = cvfInteger) and (Items.Count > 64) then
    raise EdxException.Create
      (cxGetResourceString(@cxSCheckControlIncorrectItemCount));
end;

function TcxCustomCheckComboBoxProperties.GetRealEditValueFormat
...
procedure TcxCustomCheckComboBoxProperties.ItemsChanged(Sender: TObject;
...
function TcxCustomCheckComboBoxProperties.IsEditValueValid
  (var EditValue: TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := True;
end;

procedure TcxCustomCheckComboBoxProperties.PrepareDisplayValue(const AEditValue
...
class function TcxCustomCheckComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
procedure TcxCustomCheckComboBox.SetItemState(AIndex: Integer;
  AState: TcxCheckBoxState);
begin
  if AState = cbsGrayed then
    Exit;
...
begin
  ...
  (PopupWindow as TcxCheckComboBoxPopupWindow).SysPanelStyle :=
...
procedure TcxCustomCheckComboBox.InternalValidateDisplayValue
...
begin
  Result := (ActiveProperties.EditValueFormat = cvfInteger) and (EditValue <> 0)
    or (ActiveProperties.EditValueFormat <> cvfInteger) and
    (VarToStr(EditValue) <> '');
end;
...
function TcxCustomCheckComboBox.GetActiveProperties
  : TcxCustomCheckComboBoxProperties;
begin
...
procedure TcxCustomCheckComboBox.SetProperties
  (Value: TcxCustomCheckComboBoxProperties);
begin
  Properties.Assign(Value);
end;
...
begin
  ...
  begin
    FNativeStyle := NativeStyle and AreVisualStylesAvailable
      ([totButton, totComboBox]);
...
begin
  with ActiveProperties.Items do
    if ItemChanged and (ChangedItemOperation = copDelete) and
      (ChangedItemIndex < Length(FStates) - 1) then
      Move(FStates[ChangedItemIndex + 1], FStates[ChangedItemIndex],
        (ActiveProperties.Items.Count - 1 - ChangedItemIndex) *
        SizeOf(TcxCheckBoxState));
  ...
  if not IsInplaceInitializing then
    if ModifiedAfterEnter then
      UpdateDrawValue
    else if IsDBEdit then
      SynchronizeDisplayValue
    else
      FEditValue := ActiveProperties.CalculateEditValueByCheckStates
...
begin
  ...
  Self.ActiveProperties.DisplayValueToDisplayText
    (TcxCustomTextEditViewInfo(ViewInfo).Text);
...
class function TcxFilterChecksHelper.GetEditValueFormat(AEditProperties
  : TcxCustomEditProperties): TcxCheckStatesValueFormat;
begin
  Result := cvfCaptions;
end;

class function TcxFilterChecksHelper.GetItems(AEditProperties
  : TcxCustomEditProperties): IcxCheckItems;
begin
  Result := nil;
end;
...
class function TcxFilterCheckComboBoxHelper.GetEditValueFormat(AEditProperties
  : TcxCustomEditProperties): TcxCheckStatesValueFormat;
begin
  Result := TcxCustomCheckComboBoxProperties(AEditProperties).EditValueFormat
end;

class function TcxFilterCheckComboBoxHelper.GetItems(AEditProperties
  : TcxCustomEditProperties): IcxCheckItems;
begin
...
GetRegisteredEditProperties.Register(TcxCheckComboBoxProperties,
  scxSEditRepositoryCheckComboBox);
FilterEditsController.Register(TcxCheckComboBoxProperties,
  TcxFilterCheckComboBoxHelper);

finalization

FilterEditsController.Unregister(TcxCheckComboBoxProperties,
  TcxFilterCheckComboBoxHelper);
