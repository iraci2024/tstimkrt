...
uses
  ...
  dxCore, cxContainer, cxGraphics, cxGeometry, cxLookAndFeels, cxEdit,
  cxGroupBox, dxGDIPlusClasses,
...
type
  ...
  public
    ...
    property Items[Index: Integer]: TcxCheckGroupItem read GetItem
  ...
  protected
    ...
    procedure CalculateEditValueByCheckStates(Sender: TObject;
      const ACheckStates: TcxCheckStates;
...
function TcxCheckGroupButtonViewInfo.GetGlyphRect(const AGlyphSize: TSize;
...
procedure TcxCheckGroupViewInfo.DrawButtonCaption(ACanvas: TcxCanvas;
  ...
  begin
    Result := TcxCustomCheckBoxViewInfoAccess.GetTextRect
      (AButtonViewInfo.Bounds, AGlyphRect, ItemAlignment, True, IsInplace,
      NativeStyle, Font, ScaleFactor);
  end;

begin
  DrawCheckBoxText(ACanvas, AButtonViewInfo.Caption, Font,
    AButtonViewInfo.TextColor, GetCaptionRect, DrawTextFlags, Enabled);
end;
...
const
  ...
  begin
    Result := not IsBackgroundTransparent and
      (IsInplace or (GetSkinPainter = nil));
  end;
...
begin
  if (CheckBorderStyle = ebsFlat) and (AButtonViewInfo.Data.State = ebsSelected)
  then
  ...
  DrawScaledEditCheck(ACanvas, AGlyphRect,
    TcxCheckGroupButtonViewInfo(AButtonViewInfo).State,
    ACheckStateByButtonState[AButtonViewInfo.Data.State], AEditProperties.Glyph,
    AEditProperties.GlyphCount, ACheckBorderStyle,
    AButtonViewInfo.Data.NativeStyle, clBtnText, BackgroundColor,
    CanDrawBackground, IsDesigning, False, True, GetSkinPainter, ScaleFactor);
end;

function TcxCheckGroupViewInfo.GetButtonViewInfoClass
  : TcxEditButtonViewInfoClass;
begin
  Result := TcxCheckGroupButtonViewInfo;
end;

function TcxCheckGroupViewInfo.IsButtonGlyphTransparent(AButtonViewInfo
  : TcxGroupBoxButtonViewInfo): Boolean;
begin
  Result := IsBackgroundTransparent;
end;

procedure TcxCheckGroupViewInfo.SetOnDrawBackground
...
begin
  ...
  if Edit <> nil then
    for I := 0 to Edit.InternalButtons.Count - 1 do
      TcxCheckGroupButton(Edit.InternalButtons[I]).ViewInfo.OnDrawBackground
        := AValue;
end;
...
begin
  ...
  TcxCheckGroupViewInfo(AViewInfo).GlyphSize := GetScaledEditCheckSize(ACanvas,
    IsButtonNativeStyle(Style.LookAndFeel), ACheckGroupProperties.Glyph,
    ACheckGroupProperties.GlyphCount, GetSkinPainter, ScaleFactor);
  TcxCheckGroupViewInfo(AViewInfo).ItemAlignment :=
    GetBiDiModeDependentAlignment(ACheckGroupProperties.ItemAlignment);
  TcxCheckGroupViewInfo(AViewInfo).CheckBorderStyle :=
    ABorderStyle[Style.LookAndFeel.Kind];
  AViewInfo.BackgroundColor := Style.Color;
end;

procedure TcxCheckGroupViewData.EditValueToDrawValue(const AEditValue
  ...
  begin
    ...
    begin
      if Items.Count > 0 then
        TcxCheckGroupButtonViewInfo(AViewInfo.ButtonsInfo[0]).State :=
          cbsChecked
    end
    else
    begin
      ...
      for I := 0 to Items.Count - 1 do
        TcxCheckGroupButtonViewInfo(AViewInfo.ButtonsInfo[I]).State :=
          ACheckStates[I];
    end;
  end;
...
begin
  Result.Top := 0;
  Result.Bottom := 0;
  if TcxCustomCheckGroupProperties(Properties).ItemAlignment = taLeftJustify
  then
  begin
    Result.Left := 3;
    Result.Right := 2;
...
begin
  ...
  AMetrics.AutoHeightColumnWidthCorrection := ACaptionRectExtent.Left +
    ACaptionRectExtent.Right;
...
function TcxCustomCheckGroupProperties.CompareDisplayValues(const AEditValue1,
...
begin
  ...
  Result := (Length(ACheckStates1) = Length(ACheckStates2)) and
    CompareMem(@ACheckStates1[0], @ACheckStates2[0], SizeOf(TcxCheckBoxState) *
    Length(ACheckStates1));
end;

class function TcxCustomCheckGroupProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxCheckGroup;
end;

function TcxCustomCheckGroupProperties.GetDisplayText(const AEditValue
...
begin
  ...
  for I := 0 to Items.Count - 1 do
  begin
    Result := Result + Items[I].Caption + cxGetResourceString
      (@cxSEditCheckGroupCaptionStateDelimiter) + ACheckStateNames
      [ACheckStates[I]];
...
function TcxCustomCheckGroupProperties.GetSpecialFeatures
  : TcxEditSpecialFeatures;
begin
  Result := inherited GetSpecialFeatures + [esfMultiRow];
end;

class function TcxCustomCheckGroupProperties.GetViewInfoClass
...
procedure TcxCustomCheckGroupProperties.DoAssign(AProperties
...
class function TcxCustomCheckGroupProperties.GetViewDataClass
...
begin
  if Assigned(OnStatesToEditValue) then
    ...
    AEditValue := cxCheckBox.CalculateCheckStatesValue(ACheckStates, Items,
      EditValueFormat);
end;
...
procedure TcxCustomCheckGroupProperties.SetEditValueFormat
...
function TcxCustomCheckGroupProperties.GetRealEditValueFormat
...
procedure TcxCustomCheckGroupProperties.ItemsChanged(Sender: TObject;
...
begin
  Result := (TcxCheckGroupButton(Owner).Owner as TcxCustomCheckGroup)
...
class function TcxCheckGroupButtonProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
...
class function TcxCheckGroupButton.GetPropertiesClass
...
begin
  with Message do
    if ((Msg = WM_LBUTTONDOWN) or (Msg = WM_LBUTTONDBLCLK)) and
      (CheckGroup.DragMode = dmAutomatic) and
      not(csDesigning in CheckGroup.ComponentState) then
    begin
...
class function TcxCustomCheckGroup.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
procedure TcxCustomCheckGroup.InternalSetEditValue(const Value: TcxEditValue;
...
procedure TcxCustomCheckGroup.InternalValidateDisplayValue(const ADisplayValue
  : TcxEditValue);
var
...
begin
  ...
  ActiveProperties.CalculateEditValueByCheckStates
    (GetStandaloneEventSender(Self), ACheckStates, FEditValue);
  ...
  begin
    ...
    try
      ...
      if Length(ViewInfo.ButtonsInfo) > 0 then
        AButton.LookAndFeel.Kind := ALookAndFeelKindMap
...
function TcxCustomCheckGroup.GetFirstEnabledItemIndex
...
begin
  ...
  if AStartIndex = -1 then
    AStartIndex := 0;
  for I := 0 to InternalButtons.Count - 1 do
    if TWinControl(InternalButtons[(I + AStartIndex) mod InternalButtons.Count])
      .CanFocus then
    begin
...
procedure TcxCustomCheckGroup.DoButtonEditing(Sender: TObject;
  var CanEdit: Boolean);
begin
  CanEdit := DoEditing;
end;
...
procedure TcxCustomCheckGroup.SetProperties
  (Value: TcxCustomCheckGroupProperties);
begin
...
class function TcxFilterCheckGroupHelper.GetEditValueFormat(AEditProperties
  : TcxCustomEditProperties): TcxCheckStatesValueFormat;
begin
  Result := TcxCustomCheckGroupProperties(AEditProperties).EditValueFormat;
end;

class function TcxFilterCheckGroupHelper.GetItems(AEditProperties
