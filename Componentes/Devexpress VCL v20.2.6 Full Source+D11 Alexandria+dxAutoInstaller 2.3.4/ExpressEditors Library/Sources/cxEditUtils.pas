...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Types, Windows, Variants, Classes, Controls, Forms, Graphics, ImgList,
  StdCtrls, SysUtils,
  dxThemeManager, cxContainer, cxControls, cxGraphics, cxLookAndFeels,
...
function CalculateEditDefaultButtonWidth(ACanvas: TcxCanvas;
  AViewInfo: TcxEditButtonViewInfo): Integer;
procedure cxEditFillRect(ACanvas: TCanvas; const R: TRect;
  AColor: TColor); overload;
procedure cxEditFillRect(ACanvas: TcxCanvas; const R: TRect;
  AColor: TColor); overload;
...
procedure DrawArrow(ACanvas: TcxCanvas; AArrowSize: Integer;
  AContentRect: TRect; AArrowDirection: TcxArrowDirection;
  AIsDoubleArrow: Boolean; AOffsetContent: Boolean; AColor: TColor);
...
procedure DrawGlyph(ACanvas: TCanvas; X, Y: Integer; AGlyph: TBitmap;
  AEnabled: Boolean; ABrushColor: TColor = clNone;
...
procedure DrawGlyph(ACanvas: TcxCanvas; AImageList: TCustomImageList;
  ...
  AEnabled: Boolean; ABackgroundBitmap: TBitmap = nil); overload;
function GetArrowSize(const AContentSize: TSize;
  AArrowDirection: TcxArrowDirection): TSize;
...
function GetEditButtonRegion(ACanvas: TcxCanvas;
  AViewInfo: TcxEditButtonViewInfo): TcxRegion;
...
function GetPainterClass(ANativeStyle: Boolean;
  ALookAndFeel: TcxLookAndFeelKind): TcxCustomLookAndFeelPainter;
procedure InternalFillRect(ACanvas: TcxCanvas;
  const AOuterRect, AInternalRect: TRect; ABrush: TBrushHandle);
...
const
  cxEditButtonContentVerticalOffset: array [TcxEditButtonStyle] of Integer = (0,
    4, 2, 2, 2, 2, 2);
...
begin
  ...
  begin
    InternalPolyLine(ACanvas, [AWorkRect.TopLeft, Point(AWorkRect.Left,
      AWorkRect.Bottom)], AColor);
    Inc(ARect.Left);
  end;
  if bRight in ABorders then
  begin
    InternalPolyLine(ACanvas, [Point(AWorkRect.Right, AWorkRect.Top),
      AWorkRect.BottomRight], AColor);
    Dec(ARect.Right);
  end;
  if bTop in ABorders then
  begin
    InternalPolyLine(ACanvas, [AWorkRect.TopLeft, Point(AWorkRect.Right,
      AWorkRect.Top)], AColor);
    Inc(ARect.Top);
  end;
  if bBottom in ABorders then
  begin
    InternalPolyLine(ACanvas, [Point(AWorkRect.Left, AWorkRect.Bottom),
      AWorkRect.BottomRight], AColor);
    Dec(ARect.Bottom);
  end;
end;
...
procedure DrawGlyph(ACanvas: TcxCanvas; X, Y: Integer; AGlyph: TBitmap;
  ...
  ABackgroundBitmap: TBitmap = nil);
begin
  DrawGlyph(ACanvas.Canvas, X, Y, AGlyph, AEnabled, ABrushColor,
    ABackgroundBitmap);
end;
...
begin
  ...
  try
    ...
    if ABackgroundBitmap <> nil then
    begin
      AImageList.Draw(ABackgroundBitmap.Canvas, 0, 0, AImageIndex, AEnabled);
      // ??? itMask TODO
...
begin
  if ANativeStyle and AreVisualStylesAvailable then
    ...
    Result := cxLookAndFeelPaintersManager.GetPainter
      (cxLookAndFeelStyleMap[ALookAndFeel]);
end;
    ...
    with AViewInfo do
      ...
      begin
        ...
        begin
          ...
          GetThemeBackgroundContentRect(ATheme, ACanvas.Handle,
            CP_DROPDOWNBUTTON, CBXS_NORMAL, R, CR);
        end
        else
        begin
...
begin
  ...
  begin
    ...
    begin
      if Data.ComboBoxStyle then
        Data.NativeStyle := AreVisualStylesMustBeUsed(Data.NativeStyle,
          totComboBox)
      else
        Data.NativeStyle := AreVisualStylesMustBeUsed(Data.NativeStyle,
          totButton);
    ...
    begin
      if (Data.Kind = bkGlyph) and IsImageAssigned(Glyph, Images, ImageIndex)
      then
        Result := dxGetImageSize(Glyph, Images, ImageIndex, ScaleFactor).cx +
          AButtonTotalBorderExtent
      else
      begin
        if Data.Kind = bkText then
          ACaptionWidth := ACanvas.TextWidth(Data.VisibleCaption) +
            AButtonTotalBorderExtent + ScaleFactor.Apply(2) + 1
        else
          ACaptionWidth := 0;

        Result := Max(ACaptionWidth, dxGetSystemMetrics(SM_CYHSCROLL,
          ScaleFactor));
      end;
...
begin
  ...
  begin
    ...
    cxEditFillRect(ACanvas.Handle, Rect(AInternalRect.Left,
      AInternalRect.Bottom, AInternalRect.Right, AOuterRect.Bottom), ABrush);
  end;
end;
...
procedure DrawCustomEditBackground(ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      AViewInfo.Painter.DrawContainerBorder(ACanvas, R,
        TcxContainerBorderStyle(AViewInfo.BorderStyle), ABorderWidth,
        AViewInfo.BorderColor, AViewInfo.Edges);
    end;
  end;
  ...
  begin
    ...
    ABorderWidth := AViewInfo.Painter.GetContainerBorderWidth
    ...
    begin
      ...
      else if ACanDrawBackground then
...
begin
  ...
  begin
    ...
    if AViewInfo.NativeStyle then
      AViewInfo.DrawNativeStyleEditBackground(ACanvas, ACanDrawBackground,
        ABackgroundBrush)
    else
...
function GetEditButtonHighlightColor(APressed: Boolean;
  AIsOffice11Style: Boolean): TColor;
const
  PaintersMap: array [Boolean] of TcxLookAndFeelStyle = (lfsUltraFlat,
    lfsOffice11);
var
  APainter: TcxCustomLookAndFeelPainter;
begin
  APainter := cxLookAndFeelPaintersManager.GetPainter
    (PaintersMap[AIsOffice11Style]);
  if APressed then
...
function IsShadowDrawingNeeded(AViewData: TcxCustomEditViewData): Boolean;
begin
  Result := AViewData.Style.Shadow and not AViewData.IsNativeStyle
    (AViewData.Style.LookAndFeel);
end;
