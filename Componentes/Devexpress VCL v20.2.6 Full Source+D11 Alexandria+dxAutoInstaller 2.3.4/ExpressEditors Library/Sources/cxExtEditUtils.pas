...
function IncColor(const AColor: TColor; const AR, AG, AB: Integer)
  : TColor; overload;
function IncColor(const AColor: TColor; const X: Integer): TColor; overload;
function CalcCenterPosHeight(const ARect: TRect; const ADrawHeight: Integer)
  : Integer;
...
function cxStrToFloat(const AValue: string; AToFirstNonNum: Boolean = False)
  : Extended;
...
function StringToCheckState(const Value: string; const AllowGrayed: Boolean)
  : TcxCheckBoxState;
function GetWord(const APosition: Integer; const S: string;
  const Delimiter: Char): string;
function AdjustCanvasFont(ACanvas: TCanvas; AFont: TFont;
  AAngle: Integer): Boolean;

implementation

uses
  Types, Math, cxDrawTextUtils, dxThemeConsts, dxUxTheme, dxCore, cxFormats,
  StrUtils, cxGeometry;
...
function CalcCenterPosHeight(const ARect: TRect;
  const ADrawHeight: Integer): Integer;
begin
...
begin
  ...
  if Result then
    ...
    begin
      if not dxCharInSet(AString[I],
        ['0' .. '9', dxFormatSettings.DecimalSeparator]) and
        ((AString[I] <> '-') or ((AString[I] = '-') and (I > 1))) then
        Result := False
      else if AString[I] = dxFormatSettings.DecimalSeparator then
      begin
...
begin
  S := '';
  for I := 1 to Length(AValue) do
    if dxCharInSet(AValue[I], ['0' .. '9', '-',
      dxFormatSettings.DecimalSeparator]) then
      S := S + AValue[I]
    else if AToFirstNonNum then
      Break;
  if S = '' then
...
begin
  AColor := 0;
  APos := 0;
  Result := ScanPart(R) and ScanPart(G) and ScanPart(B) and
    (S[Length(S)] <> RGBSeparatorChar);
  if Result then
    AColor := RGB(R, G, B);
end;
