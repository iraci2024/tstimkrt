...
uses
  ...
  cxGeometry, cxLookAndFeels, cxLookAndFeelPainters, cxScrollBar,
  cxExtEditConsts, cxHeader, cxEdit, dxTypeHelpers;
...
type
  ...
  published
    property Arrangement: TIconArrangement read GetArrangement
      write SetArrangement default cxiaTop;
    property AutoArrange: Boolean read GetAutoArrange write SetAutoArrange
      default False;
  ...
  private
    ...
    procedure LVMGetHeaderItemInfo(var Message: TDXMHeaderItemInfo);
      message DXM_GETHEADERITEMINFO;
  protected
...
{$ENDIF}
  ...
  private
    ...
    procedure SetOnAdvancedCustomDrawItem
      (Value: TLVAdvancedCustomDrawItemEvent);
    procedure SetOnAdvancedCustomDrawSubItem
      (Value: TLVAdvancedCustomDrawSubItemEvent);
  ...
  protected
    ...
    property ColumnClick: Boolean read GetColumnClick write SetColumnClick
      default True;
    ...
    property OwnerData: Boolean read GetOwnerData write SetOwnerData
      default False;
    ...
    property RowSelect: Boolean read GetRowSelect write SetRowSelect
      default False;
    property ShowWorkAreas: Boolean read GetShowWorkAreas write SetShowWorkAreas
      default False;
    ...
    property OnAdvancedCustomDraw: TLVAdvancedCustomDrawEvent
      read GetOnAdvancedCustomDraw write SetOnAdvancedCustomDraw;
    ...
    property OnChanging: TLVChangingEvent read GetOnChanging
      write SetOnChanging;
    property OnColumnClick: TLVColumnClickEvent read GetOnColumnClick
      write SetOnColumnClick;
    property OnColumnDragged: TNotifyEvent read GetOnColumnDragged
      write SetOnColumnDragged;
    ...
    property OnCustomDraw: TLVCustomDrawEvent read GetOnCustomDraw
    ...
    property OnCustomDrawSubItem: TLVCustomDrawSubItemEvent
      read GetOnCustomDrawSubItem write SetOnCustomDrawSubItem;
    property OnData: TLVOwnerDataEvent read GetOnData write SetOnData;
    property OnDataFind: TLVOwnerDataFindEvent read GetOnDataFind
      write SetOnDataFind;
    property OnDataHint: TLVOwnerDataHintEvent read GetOnDataHint
      write SetOnDataHint;
    property OnDataStateChange: TLVOwnerDataStateChangeEvent
      read GetOnDataStateChange write SetOnDataStateChange;
    property OnDeletion: TLVDeletedEvent read GetOnDeletion write SetOnDeletion;
    property OnDrawItem: TLVDrawItemEvent read GetOnDrawItem
      write SetOnDrawItem;
    ...
    property OnGetImageIndex: TLVNotifyEvent read GetOnGetImageIndex
    ...
    property OnSelectItem: TLVSelectItemEvent read GetOnSelectItem
      write SetOnSelectItem;
    property OnCreateItemClass: TLVCreateItemClassEvent
      read GetOnCreateItemClass write SetOnCreateItemClass;
  public
    constructor Create(AOwner: TComponent); override;
    ...
    function GetNearestItem(Point: TPoint; Direction: TSearchDirection)
      : TListItem;
    ...
    property HotTrackStyles: TListHotTrackStyles read GetHotTrackStyles
      write SetHotTrackStyles default [];
...
begin
  ...
  begin
    Result := (not Container.ReadOnly) { and (not OwnerData) };
    { <- Prevent bug, when Caption not saved after CreateWnd in "OwnerData" mode }
    if Result then
...
begin
  ...
  begin
    ...
    AHintInfo.HintPos := Point(AItemRect.Left + GetSystemMetrics(SM_CXCURSOR),
...
begin
  ...
  else if Message.NMHdr.Code = NM_CUSTOMDRAW then
  begin
...
function TcxListViewContainer.IsPanArea(const APoint: TPoint): Boolean;
  ...
  begin
    Result := WindowFromPoint(ClientToScreen(APoint))
      = InnerListView.FHeaderHandle;
  end;
...
function TcxListViewContainer.UseSystemRightToLeftLayoutForInnerControl
  : Boolean;
begin
  Result := True;
end;
...
function TcxListViewContainer.GetScrollBarBounds(const AScrollBarRect
  : TRect): TRect;
begin
  if UseRightToLeftScrollBar and not IsWinVistaOrLater then
  // #Ch WinXp ListView RTL bug
  begin
...
function TcxCustomListView.FindCaption(StartIndex: Integer; Value: string;
  Partial, Inclusive, Wrap: Boolean): TListItem;
begin
  Result := InnerListView.FindCaption(StartIndex, Value, Partial,
    Inclusive, Wrap);
end;
...
function TcxCustomListView.GetNearestItem(Point: TPoint;
...
procedure TcxCustomListView.SetOnAdvancedCustomDraw
  (Value: TLVAdvancedCustomDrawEvent);
begin
  InnerListView.OnAdvancedCustomDraw := Value;
end;

function TcxCustomListView.GetOnAdvancedCustomDrawItem
  : TLVAdvancedCustomDrawItemEvent;
begin
  Result := InnerListView.OnAdvancedCustomDrawItem;
end;

procedure TcxCustomListView.SetOnAdvancedCustomDrawItem
  (Value: TLVAdvancedCustomDrawItemEvent);
begin
  InnerListView.OnAdvancedCustomDrawItem := Value;
end;

function TcxCustomListView.GetOnAdvancedCustomDrawSubItem
  : TLVAdvancedCustomDrawSubItemEvent;
begin
  Result := InnerListView.OnAdvancedCustomDrawSubItem;
end;

procedure TcxCustomListView.SetOnAdvancedCustomDrawSubItem
  (Value: TLVAdvancedCustomDrawSubItemEvent);
begin
...
procedure TcxCustomListView.SetOnCustomDrawSubItem
  (Value: TLVCustomDrawSubItemEvent);
begin
...
procedure TcxCustomListView.SetOnDataStateChange
...
procedure TcxCustomListView.SetOnCreateItemClass
  (Value: TLVCreateItemClassEvent);
begin
...
begin
  if (FContainer = nil) or not FContainer.InnerControlDefaultHandler
...
begin
  if not(sfWidth in ScalingFlags) then
    ...
    begin
      AColumn := Columns[I];
      if (AColumn.Width <> LVSCW_AUTOSIZE) and
        (AColumn.Width <> LVSCW_AUTOSIZE_USEHEADER) then
...
procedure TcxBaseInnerListView.DoMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
begin
  Result := (FContainer <> nil) and FContainer.DoMouseWheel(Shift, WheelDelta,
    MousePos);
  if not Result then
...
procedure TcxBaseInnerListView.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  ...
  try
    ...
    if ALink.Ref <> nil then
      case Message.Msg of
        ...
        WM_PARENTNOTIFY:
          if (Message.WParamLo = WM_CREATE) and
            (cxGetClassName(Message.lParam) = 'SysHeader32') then
          begin
            FHeaderHandle := Message.lParam;
            FWindowProcObject := cxWindowProcController.Add(FHeaderHandle,
              HeaderWndProc);
...
begin
  ...
  for I := 0 to Columns.Count do
    DrawScaledHeaderSection(FHeaderHandle, I, Canvas, GetLookAndFeel,
      AScaleFactor, SmallImages);
end;
...
function TcxBaseInnerListView.NeedCheckScrollBars
  (var Message: TMessage): Boolean;
begin
  case Message.Msg of
    WM_HSCROLL, WM_MOUSEWHEEL, WM_VSCROLL, WM_WINDOWPOSCHANGED, CM_WININICHANGE,
      LVM_SETITEMCOUNT:
      Result := True
  else
    Result := False;
...
function TcxBaseInnerListView.GetLookAndFeelPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := FContainer.LookAndFeelPainter;
end;

procedure TcxBaseInnerListView.HScrollHandler(Sender: TObject;
...
begin
  ...
  begin
    CallWindowProc(DefWndProc, Handle, WM_HSCROLL, MakeWParam(Word(ScrollCode),
      Word(ScrollPos)), FContainer.GetScrollBarHandle(sbHorizontal));
    ScrollPos := GetScrollPos(Handle, SB_HORZ);
  end;
end;

procedure TcxBaseInnerListView.VScrollHandler(Sender: TObject;
  ScrollCode: TScrollCode; var ScrollPos: Integer);
...
begin
  ...
  begin
    ...
    begin
      ...
      AScrollParam := (ScrollPos - ListView_GetTopIndex(Handle)) *
        GetLineHeight;
  ...
  begin
    CallWindowProc(DefWndProc, Handle, WM_VSCROLL,
      Word(ScrollCode) + Word(ScrollPos) shl 16,
      FContainer.GetScrollBarHandle(sbVertical));
...
begin
  inherited;
  if not FContainer.IsTouchScrollUIMode and UsecxScrollBars and not FContainer.ScrollBarsCalculating
  then
    FContainer.SetScrollBarsParameters;
end;
...
begin
  ...
  begin
    Message.Result := 1;
    if FContainer.HScrollBarVisible and FContainer.VScrollBarVisible then
...
procedure TcxBaseInnerListView.WMWindowPosChanged
  (var Message: TWMWindowPosChanged);
begin
  inherited;
  if not FContainer.IsTouchScrollUIMode and not(csDestroying in ComponentState)
    and FContainer.HScrollBarVisible and FContainer.VScrollBarVisible then
...
begin
  inherited;
  if UseRightToLeftAlignment and FContainer.UseSystemRightToLeftLayoutForInnerControl
  then
    Params.ExStyle :=
      (Params.ExStyle or WS_EX_LAYOUTRTL { or WS_EX_NOINHERITLAYOUT } ) and
...
procedure TcxBaseInnerListView.LVMGetHeaderItemInfo
  (var Message: TDXMHeaderItemInfo);
  ...
  begin
    ...
    begin
      ...
      if AHeaderItemIndex = FPressedHeaderItemIndex then
        Result := cxbsPressed
      else if CanHotTrack and (AHeaderItemIndex = GetHeaderHotItemIndex) then
        Result := cxbsHot
      else
        Result := cxbsNormal;
    end;
