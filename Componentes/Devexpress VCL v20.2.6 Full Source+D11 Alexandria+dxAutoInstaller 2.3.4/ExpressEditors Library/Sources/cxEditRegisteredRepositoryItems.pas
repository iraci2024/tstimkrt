...
type
  ...
  TcxEditRegisteredRepositoryItemsDataArray = array of
    TcxEditRegisteredRepositoryItemsData;

  TcxEditRegisteredRepositoryItems = class(TcxInterfacedPersistent, IUnknown,
    IcxEditRepositoryItemListener) // singleton
  private
    FDestroying: Boolean;
    ...
    function Find(const AItems: TcxEditRegisteredRepositoryItemsDataArray;
      ...
      AFindItemWithMaxVersion: Boolean; out AIndex: Integer): Boolean;
  ...
  protected
    ...
    procedure Delete(var AItems: TcxEditRegisteredRepositoryItemsDataArray;
      AIndex: Integer);
    function FindIndexForInsertion(const AItems
      : TcxEditRegisteredRepositoryItemsDataArray; ADataType: TdxNativeInt;
      AVersion: Integer; out AIndex: Integer): Boolean;
    function FindItemWithMaxVersion(const AItems
      : TcxEditRegisteredRepositoryItemsDataArray;
  ...
  public
    ...
    procedure RegisterDefaultItem(AVersion: Integer;
      ARepositoryItem: TcxEditRepositoryItem); virtual;
    procedure RegisterItem(ADataType: TdxNativeInt; AVersion: Integer;
      ARepositoryItem: TcxEditRepositoryItem); virtual;
    procedure UnregisterDefaultItem(AVersion: Integer); virtual;
    procedure UnregisterItem(ADataType: TdxNativeInt;
...
function TcxEditRegisteredRepositoryItems.GetItem(ADataType: TdxNativeInt)
...
procedure TcxEditRegisteredRepositoryItems.UnregisterDefaultItem
  (AVersion: Integer);
var
  AIndex: Integer;
begin
...
procedure TcxEditRegisteredRepositoryItems.UnregisterItem
  (ADataType: TdxNativeInt; AVersion: Integer);
var
  AIndex: Integer;
begin
...
procedure TcxEditRegisteredRepositoryItems.ItemRemoved
...
procedure TcxEditRegisteredRepositoryItems.PropertiesChanged
  (Sender: TcxEditRepositoryItem);
begin
  Changed;
end;
...
begin
  ...
  if Result < Length(AItems) - 1 then
    Move(AItems[Result], AItems[Result + 1], (Length(AItems) - Result - 1) *
      SizeOf(TcxEditRegisteredRepositoryItemsData));
...
begin
  ...
  if AIndex < Length(AItems) - 1 then
    Move(AItems[AIndex + 1], AItems[AIndex], (Length(AItems) - AIndex - 1) *
      SizeOf(TcxEditRegisteredRepositoryItemsData));
  SetLength(AItems, Length(AItems) - 1);
  Changed;
end;
...
function TcxEditRegisteredRepositoryItems.Find(const AItems
  : TcxEditRegisteredRepositoryItemsDataArray; ADataType: TdxNativeInt;
  AVersion: Integer; AFindItemWithMaxVersion: Boolean;
  out AIndex: Integer): Boolean;

  function Compare(const AItem1,
    AItem2: TcxEditRegisteredRepositoryItemsData): Integer;
  begin
...
begin
  ...
  if AFindItemWithMaxVersion then
    while (AIndex < AItemCount - 1) and
      (AItems[AIndex + 1].DataType = AItem.DataType) do
      Inc(AIndex);

  Result := (AIndex < AItemCount) and (AItems[AIndex].DataType = AItem.DataType)
