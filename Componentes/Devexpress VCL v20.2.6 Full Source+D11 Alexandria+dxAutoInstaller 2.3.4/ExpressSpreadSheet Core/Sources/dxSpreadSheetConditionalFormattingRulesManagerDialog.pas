...
{$ENDIF}
  ...
  dxCore, cxGraphics, cxControls, cxLookAndFeels, cxClasses, cxContainer,
  cxStyles,
  cxCustomData, cxData, cxDataStorage, dxForms, cxImageList,
  cxLookAndFeelPainters, dxLayoutControlAdapters, dxLayoutLookAndFeels,
  dxLayoutControl,
  dxLayoutcxEditAdapters, dxLayoutContainer,
  cxEdit, cxButtons, cxListBox, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxMCListBox,
...
type
  ...
  TfrmSpreadSheetConditionalFormattingRulesManagerDialogClass = class of
    TfrmSpreadSheetConditionalFormattingRulesManagerDialog;
  ...
  TfrmSpreadSheetConditionalFormattingRulesManagerDialog = class(TdxForm,
    ...
    procedure tlRulesCustomDrawDataCell(Sender: TcxCustomTreeList;
      ACanvas: TcxCanvas; AViewInfo: TcxTreeListEditCellViewInfo;
      var ADone: Boolean);
    procedure tlRulesDblClick(Sender: TObject);
    procedure tlRulesContextPopup(Sender: TObject; MousePos: TPoint;
      var Handled: Boolean);
    procedure tlRulesDragOver(Sender, Source: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
    procedure tlRulesEditing(Sender: TcxCustomTreeList;
      AColumn: TcxTreeListColumn; var Allow: Boolean);
    procedure tlRulesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure tlRulesMoveTo(Sender: TcxCustomTreeList;
  ...
  protected
    ...
    function GetHelperClass
      : TdxSpreadSheetConditionalFormattingRulesManagerDialogHelperClass;
      virtual;
    ...
    procedure SaveFocus(out AIndex: Integer; out ATopIndex: Integer;
      out AData: Pointer);

    // IdxSpreadSheetConditionalFormattingRulesManagerDialogHelperListener
    procedure IdxSpreadSheetConditionalFormattingRulesManagerDialogHelperListener.
      NotifyDataChanged = PopulateRules;
    procedure NotifyModified;
  public
    constructor Create(AOwner: IdxDialogOwner); reintroduce;
    destructor Destroy; override;
    //
    property FocusedRuleInfo
      : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
      read GetFocusedRuleInfo;
    property Helper: TdxSpreadSheetConditionalFormattingRulesManagerDialogHelper
      read FHelper;
  end;
...
procedure ShowConditionalFormattingRulesManagerDialog(AOwner: TObject);
  overload; // Special for CBuilder //#doc'd

implementation

uses
  dxSpreadSheetCoreDialogsStrs, cxGeometry, dxSpreadSheetUtils, Math,
...
begin
  ...
  begin
    ADialog := dxSpreadSheetConditionalFormattingRulesManagerDialogClass.
      Create(AOwner);
    try
...
constructor TfrmSpreadSheetConditionalFormattingRulesManagerDialog.Create
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  ApplyLocalization;
var
  AIndex: Integer;
begin
...
function TfrmSpreadSheetConditionalFormattingRulesManagerDialog.GetHelperClass
  : TdxSpreadSheetConditionalFormattingRulesManagerDialogHelperClass;
begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  PopulateRulesInArea(const AArea: TRect);
begin
  Helper.PopulateRulesInArea(AArea,
    ...
    begin
      ...
      ANode.Values[2] := Helper.ConditionalFormatting.ReferencesToString
        (AInfo.Rule.Areas);
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.RestoreFocus
  (AIndex, ATopIndex: Integer; AData: Pointer);
var
  I: Integer;
begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.SaveFocus
  (out AIndex: Integer; out ATopIndex: Integer; out AData: Pointer);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  AdapterClickHandler(Sender: TObject);
begin
  TMenuItem((Sender as TComponent).Tag).Click;
end;

function TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  GetFocusedRuleInfo
  : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo;
begin
  Result := TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
    (tlRules.FocusedNode.Data);
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  WMUpdateSelection(var AMessage: TMessage);
begin
  if tlRules.FocusedNode = nil then
  begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acCloneExecute
  (Sender: TObject);
begin
  if tlRules.FocusedNode <> nil then
  begin
    Helper.Add(FocusedRuleInfo.Rule.Clone);
    tlRules.FocusedNode := tlRules.AbsoluteVisibleItems
      [tlRules.AbsoluteVisibleCount - 1];
    tlRules.FocusedNode.MakeVisible;
  end;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acCloneUpdate
  (Sender: TObject);
begin
  acClone.Enabled := tlRules.FocusedNode <> nil;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acCreateExecute
  (Sender: TObject);
var
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acDeleteExecute
  (Sender: TObject);
begin
  Helper.Delete(FocusedRuleInfo);
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acDeleteUpdate
  (Sender: TObject);
begin
  acDelete.Enabled := tlRules.FocusedNode <> nil;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acEditExecute
  (Sender: TObject);
var
  ARule: TdxSpreadSheetCustomConditionalFormattingRule;
begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acEditUpdate
  (Sender: TObject);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  acMoveDownExecute(Sender: TObject);
begin
  Helper.MoveTo(FocusedRuleInfo,
    TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
    (tlRules.FocusedNode.getNextSibling.Data), True);
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  acMoveDownUpdate(Sender: TObject);
begin
  acMoveDown.Enabled := (tlRules.FocusedNode <> nil) and
    not tlRules.FocusedNode.IsLast;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acMoveUpExecute
  (Sender: TObject);
begin
  Helper.MoveTo(FocusedRuleInfo,
    TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
    (tlRules.FocusedNode.getPrevSibling.Data), False);
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.acMoveUpUpdate
  (Sender: TObject);
begin
  acMoveUp.Enabled := (tlRules.FocusedNode <> nil) and
    not tlRules.FocusedNode.IsFirst;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.btnApplyClick
  (Sender: TObject);
begin
  Helper.ApplyChanges;
end;
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  cbDisplayModePropertiesChange(Sender: TObject);
begin
  PopulateRules;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  tlcRuleAreaPropertiesEditValueChanged(Sender: TObject);
var
...
begin
  ...
  try
    ...
    AAreaListAsString := StringReplace(AAreaListAsString, '$', '',
      [rfReplaceAll]);
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  tlcRuleStopIfTrueGetEditProperties(Sender: TcxTreeListColumn;
ANode: TcxTreeListNode; var EditProperties: TcxCustomEditProperties);
begin
  if TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo(ANode.Data).StopIfTrueSupported
  then
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  tlcRuleStopIfTruePropertiesEditValueChanged(Sender: TObject);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  tlRulesContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean);
var
...
begin
  ...
  begin
    ...
    try
      ...
      TdxBuiltInPopupMenuAdapterHelper.AddMenu(AAdapter, pmRules,
        AdapterClickHandler);
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.
  tlRulesCustomDrawDataCell(Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
AViewInfo: TcxTreeListEditCellViewInfo; var ADone: Boolean);
begin
  ADone := AViewInfo.Column = tlcRuleFormat;
  if ADone then
    TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
      (AViewInfo.Node.Data).DrawPreview(ACanvas,
      cxRectInflate(AViewInfo.BoundsRect, -cxTextOffset));
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.tlRulesDblClick
  (Sender: TObject);
begin
...
var Accept: Boolean);
begin
  Accept := not(tlRules.HitTest.HitAtNodePreview or
    tlRules.HitTest.HitAtColumn);
end;
...
begin
  Allow := (AColumn = tlcRuleArea) or (AColumn = tlcRuleStopIfTrue) and
    FocusedRuleInfo.StopIfTrueSupported;
end;

procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.tlRulesKeyDown
...
procedure TfrmSpreadSheetConditionalFormattingRulesManagerDialog.tlRulesMoveTo
  (Sender: TcxCustomTreeList; AttachNode: TcxTreeListNode;
...
begin
  case AttachMode of
    tlamAddChild:
      Helper.MoveTo
        (TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
        (TcxTreeListNode(Nodes[0]).Data),
        TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo
        (AttachNode.GetLastChild.Data), True);
