...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Character, Generics.Collections, Generics.Defaults, Graphics, Variants,
  SysUtils, Math, StrUtils,
  dxCore, dxCoreGraphics, dxCultureInfo, cxGraphics,
  dxSpreadSheetNumberFormatCore;
...
type
  ...
  protected
    class procedure GenerateInvariant(ADesignators
      : TdxNumberFormatDesignatorMap);
  public
  ...
  type
    ...
    TdxNumberFormatDesignatorCheckMethod = reference to function(AChar: Char)
      : TdxNumberFormatDesignatorCheckResult;
    TdxNumberFormatDesignatorParseMethod = procedure of object;
    TdxNumberFormatDesignatorParseSeparatorMethod = function
      (ADesignator: Integer): Integer of object;
...
{$ENDREGION}
  ...
  protected
    ...
    function ParseSeparator(ADesignator: Integer;
      ASeparators: array of Integer): Integer;
    ...
    procedure ParseNumericExponent(AIntegerCount, ADecimalSeparatorIndex,
      ADecimalCount: Integer; AGrouping: Boolean);
    ...
    procedure ProcessSequence(AMethods
      : TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>;
      ASeparatorParser: TdxNumberFormatDesignatorParseSeparatorMethod;
      ACheckDesignator: TdxNumberFormatDesignatorCheckMethod = nil);

    function GetDateTimeBlockLength: Integer; overload;
    function GetDateTimeBlockLength(const AFormatString: string;
      ACurrentIndex: Integer): Integer; overload;
...
begin
  ...
  begin
    ...
    CheckAddChar(ADesignator,
      TdxNumberFormatDesignator.FractionOrDateSeparator, AKey);
...
procedure TdxNumberFormatLocalizer.AddSeparator(ASymbol: Char;
  ADesignator: Integer);
begin
...
class procedure TdxNumberFormatLocalizer.GenerateInvariant
  (ADesignators: TdxNumberFormatDesignatorMap);
begin
  ...
  ADesignators.Add('m', TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month);
  ADesignators.Add('s', TdxNumberFormatDesignator.Second);
  ADesignators.Add('y', TdxNumberFormatDesignator.Year);
  ADesignators.Add('g', TdxNumberFormatDesignator.JapaneseEra or
    TdxNumberFormatDesignator.General);
end;
...
begin
  ...
  FLocalizer := TdxNumberFormatLocalizer.Create
    (TdxCultureInfo.InvariantCulture);
...
class function TdxNumberFormatParser.Parse(const S: string; ID: Integer)
  : TdxNumberFormat;
begin
  if S = '' then
...
procedure TdxNumberFormatParser.ParseNumericExponent(AIntegerCount,
  ADecimalSeparatorIndex, ADecimalCount: Integer; AGrouping: Boolean);
begin
  Inc(FCurrentIndex);
  if FCurrentIndex >= Length(FormatString) then
    Exit;
...
function TdxNumberFormatParser.ParseSeparator(ADesignator: Integer;
ASeparators: array of Integer): Integer;
var
  I: Integer;
begin
...
begin
  if (ADesignator and TdxNumberFormatDesignator.DecimalSeparator <> 0) and OnMillisecondCore
  then
    Exit;
  ...
  begin
    ...
    if ADesignator and TdxNumberFormatDesignator.FractionOrDateSeparator <> 0
  then
...
begin
  Result := SameText(Copy(FormatString, FCurrentIndex,
    Length(Localizer.GeneralDesignator)), Localizer.GeneralDesignator);
end;
...
begin
  ...
  begin
    ...
    if FDecimalSeparatorIndex >= 0 then
      FDecimalSeparatorIndex := FDecimalSeparatorIndex + FIntegerCount -
        APrevIntegerCount;
  end;
end;
...
begin
  ...
  begin
    FCurrentSymbol := FormatString[FCurrentIndex];
    if not ProcessDesignator(FCurrentSymbol, AMethods, ASeparatorParser,
      ACheckDesignator) or FErrorState then
    begin
      FreeAndNil(FPart);
      Exit;
    end;
...
function TdxNumberFormatParser.GetDateTimeBlockLength(const AFormatString
  : string; ACurrentIndex: Integer): Integer;
var
  AStartIndex: Integer;
begin
...
function TdxNumberFormatParser.TryParseColor(const S: string)
...
begin
  ...
  else if SameText(ColorPrefix, Copy(S, 1, Length(ColorPrefix))) then
  begin
    AIndex := StrToIntDef(Copy(S, Length(ColorPrefix) + 1, MaxInt), 0);
    if InRange(AIndex - 1, Low(dxExcelStandardColors),
      High(dxExcelStandardColors)) then
      Result := TdxNumberFormatElementColor.Create
        (dxExcelStandardColors[AIndex - 1]);
  end
end;
...
function TdxNumberFormatParser.TryParseDateTimeCondition
  (var AElapsed: Boolean): Integer;
var
  AElement: TdxNumberFormatElement;
...
begin
  ...
  begin
    ...
    begin
      if FLocale <> nil then
        Exit(-1)
      else if AElement is TdxNumberFormatElementDisplayLocale then
...
function TdxNumberFormatParser.TryParseElapsed(const S: string)
  : TdxNumberFormatElement;
var
...
begin
  ...
  if (FCurrentSymbol = 'h') or (FCurrentSymbol = 'm') or (FCurrentSymbol = 's')
  then
  begin
    ...
    if ABlockLength = Length(S) then
      case FCurrentSymbol of
        'h':
          Result := TdxNumberFormatElementHours.Create(ABlockLength,
            True, False);
        'm':
...
function TdxNumberFormatParser.TryParseExpr(const S: string)
  : TdxNumberFormatElement;
begin
  Result := TdxNumberFormatElementExprCondition.Create(S);
end;

function TdxNumberFormatParser.TryParseLocale(const S: string)
  : TdxNumberFormatElement;
var
  ADashIndex: Integer;
begin
  ...
  if ADashIndex > 0 then
    Result := TdxNumberFormatElementDisplayLocale.Create
      (StrToIntDef('$' + Copy(S, ADashIndex + 1, MaxInt), -1),
      Copy(S, 2, ADashIndex - 2))
  else
    Result := TdxNumberFormatElementDisplayLocale.Create(-1,
      Copy(S, 2, MaxInt));
end;
...
var
  ...
  S: string;
begin
  if (FCurrentIndex + 5 <= Length(FormatString)) and
    SameText(Copy(FormatString, FCurrentIndex, 5), 'am/pm') then
  begin
    if FElapsed then
    begin
      FErrorState := True;
  ...
  begin
    ...
    Inc(FCurrentIndex, 2);
    FElements.Add(TdxNumberFormatElementAmPm.CreateEx(dxIsLowerCase(S[1]),
      dxIsLowerCase(S[2])));
  end
  else
    Exit(False);
  ...
  begin
    ...
    else if AElement is TdxNumberFormatElementDateBase then
      Break;
  end;
  Result := True;
end;
...
begin
  Inc(FCurrentIndex);
  FElements.Add(TdxNumberFormatElementAsterisk.Create
    (FormatString[FCurrentIndex]));
end;
...
begin
  if (FElements.Count > 0) and (FElements.Last is TdxNumberFormatElementTimeBase)
  then
    OnTimeSeparator
  else
    OnDateSeparator;
...
begin
  ...
  begin
    FCurrentSymbol := FormatString[FCurrentIndex];
    if not FLocalizer.Designators.TryGetValue(dxLowerCase(FCurrentSymbol),
      FDesignator) then
      FDesignator := TdxNumberFormatDesignator.Default;
...
begin
  FElements.Add(TdxNumberFormatElementDay.Create
    (Min(GetDateTimeBlockLength, 4)));
end;
...
begin
  ...
  if Result then
    FElements.Add(TdxNumberFormatElementDayOfWeek.Create(IfThen(AElementLength >
      3, 4, 3)))
  else
    FCurrentIndex := AIndex;
end;
...
begin
  FPart := TdxSimpleNumericNumberFormat.Create(FElements, 0, 0, 0, 0, -1, False,
    FFormats.Count = 1);
end;

procedure TdxNumberFormatParser.OnEndOfPart2;

  function CreatePart(ALocale: TdxNumberFormatElementDisplayLocale)
  ...
  begin
    ...
    Result := TdxDateTimeNumberFormat.Create(FElements, ALocale,
      FHasMilliseconds);
  end;
...
begin
  Inc(FIntegerCount, FDividendCount);
  PrepareGrouping;
  FPart := TdxSimpleNumericNumberFormat.Create(FElements, FPercentCount,
    FIntegerCount, FDecimalCount, FDisplayFactor, FDecimalSeparatorIndex,
    FGrouping, FFormats.Count = 1);
end;

procedure TdxNumberFormatParser.OnEndOfPart4;
begin
  FPart := TdxNumericNumberFormatExponent.Create(FElements, FIntegerCount,
    FDecimalCount, FDecimalSeparatorIndex, FExpIndex, FExpCount, FExplicitSign,
    FGrouping, FFormats.Count = 1);
end;
...
begin
  ...
  begin
    ...
    FPart := TdxExplicitNumericFractionNumberFormat.Create(FElements,
      FPercentCount, FIntegerCount, FPreFractionIndex, FFractionSeparatorIndex,
      FDisplayFactor, FDividendCount, FDivisor, FGrouping, FFormats.Count = 1);
  end
  else if FDivisorCount = 0 then
    FErrorState := True
  else
    FPart := TdxNumericFractionNumberFormat.Create(FElements, FPercentCount,
      FIntegerCount, FPreFractionIndex, FFractionSeparatorIndex, FDisplayFactor,
      FDividendCount, FDivisorCount, FGrouping, FFormats.Count = 1);
end;
...
begin
  Inc(FIntegerCount, FDividendCount);
  PrepareGrouping;
  ParseNumericExponent(FIntegerCount, FDecimalSeparatorIndex, FDecimalCount,
    FGrouping);
end;
    ...
    begin
      ...
      begin
        if AElement is TdxNumberFormatElementAmPm then
          Exit(True);
        if (AElement is TdxNumberFormatElementHours) or
          not(AElement is TdxNumberFormatElementTimeBase) then
          Break;
      end;
    end;
  end;
...
begin
  FElements.Add(TdxNumberFormatElementHours.Create(IfThen(GetDateTimeBlockLength
    > 1, 2, 1), False, Is12HourTime));
end;

procedure TdxNumberFormatParser.OnInvariantYear;
begin
  FElements.Add(TdxNumberFormatElementInvariantYear.Create
    (Min(GetDateTimeBlockLength, 2)));
end;

procedure TdxNumberFormatParser.OnJapaneseEra;
begin
  FElements.Add(TdxNumberFormatElementJapaneseEra.Create
    (Min(GetDateTimeBlockLength, 3)));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FElements.Add(TdxNumberFormatElementMilliseconds.Create
        (AElementLength, False));
    end;
    Result := True;
  end;
end;
...
begin
  FElements.Add(TdxNumberFormatElementMinutes.Create(Min(GetDateTimeBlockLength,
    2), False));
end;
...
begin
  ...
  FElements.Add(TdxNumberFormatElementMonth.Create(IfThen(ALength > 5, 4,
    ALength)));
end;
...
procedure TdxNumberFormatParser.OnMonthOrMinute;
  ...
  begin
    ...
    begin
      ...
      if AElement is TdxNumberFormatElementTimeBase then
        Exit(True);
      if (AElement is TdxNumberFormatElementDateBase) and
        not(AElement is TdxNumberFormatElementAmPm) then
        Exit(False);
    end;
  end;
...
begin
  ...
  if ABlockLength > 2 then
    FElements.Add(TdxNumberFormatElementMonth.Create(IfThen(ABlockLength > 5, 4,
      ABlockLength)))
  else if CheckIsMinute then
...
begin
  ...
  begin
    ...
    if FCurrentSymbol = '"' then
    begin
      FElements.Add(TdxNumberFormatElementQuotedText.Create(Copy(FormatString,
        AIndex, FCurrentIndex - AIndex)));
      Break;
    end;
    Inc(FCurrentIndex);
  end;
  ...
  begin
    ...
    begin
      ...
      if AElement is TdxNumberFormatElementMonth then
        FElements[I] := TdxNumberFormatElementMinutes.Create
          (TdxNumberFormatElementMonth(AElement).Count, False);
      Break;
    end;
  end;
  FElements.Add(TdxNumberFormatElementSeconds.Create
    (IfThen(GetDateTimeBlockLength > 1, 2, 1), False));
end;
...
begin
  ...
  Dec(FCurrentIndex);
  FElements.Add(TdxNumberFormatElementThaiYear.Create(IfThen(AElementLength >
    2, 4, 2)));
end;
...
begin
  Inc(FCurrentIndex);
  FElements.Add(TdxNumberFormatElementUnderline.Create
    (FormatString[FCurrentIndex]));
end;

procedure TdxNumberFormatParser.OnYear;
begin
  FElements.Add(TdxNumberFormatElementYear.Create(IfThen(GetDateTimeBlockLength
    > 2, 4, 2)));
end;
...
begin
  FParseMethods1 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods1.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnDateTimeSymbol);
  FParseMethods1.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnDateTimeSymbol);
  FParseMethods1.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Year, OnDateTimeSymbol);
  FParseMethods1.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods1[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods1.Add(TdxNumberFormatDesignator.DecimalSeparator,
    OnNumericSymbol);
  ...
  FParseMethods1.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnError);
  ...
  FParseMethods1.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnGeneralOrDateTime);
  FParseMethods1.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods1[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods1.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnGeneralOrDateTime);
  FParseMethods1.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.InvariantYear, OnGeneralOrDateTime);
  FParseMethods1.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.JapaneseEra, OnGeneralOrDateTime);
  FParseMethods1.Add(TdxNumberFormatDesignator.Hour, OnDateTimeSymbol);
  FParseMethods1.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods1[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods1.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnDateTimeSymbol);
...
begin
  FParseMethods2 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods2.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnAmPmOrDayOfWeek);
  FParseMethods2.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnAmPmOrMonth);
  FParseMethods2.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Year, OnAmPmOrYear);
  FParseMethods2.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods2[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods2.Add(TdxNumberFormatDesignator.DayOfWeek,
    OnDayOfWeekOrDefault2);
  ...
  FParseMethods2.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnDefaultDateSeparator);
  ...
  FParseMethods2.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnGeneralOrDay);
  FParseMethods2.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods2[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods2.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnGeneralOrSecond);
  FParseMethods2.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.InvariantYear, OnGeneralOrInvariantYear);
  FParseMethods2.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.JapaneseEra, OnGeneralOrJapaneseEra);
  FParseMethods2.Add(TdxNumberFormatDesignator.Hour, OnHour);
  FParseMethods2.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods2[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods2.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnMonthOrMinute);
...
begin
  FParseMethods3 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods3.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Year, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods3[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods3.Add(TdxNumberFormatDesignator.DayOfWeek,
    OnDayOfWeekOrDefault3);
  FParseMethods3.Add(TdxNumberFormatDesignator.Default, OnDefault);
  FParseMethods3.Add(TdxNumberFormatDesignator.DecimalSeparator,
    OnDecimalSeparator);
  ...
  FParseMethods3.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnFractionSeparator);
  FParseMethods3.Add(TdxNumberFormatDesignator.GroupSeparator,
    OnGroupSeparator);
  FParseMethods3.Add(TdxNumberFormatDesignator.General, OnGeneral3);
  FParseMethods3.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods3[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods3.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.InvariantYear, OnESymbol3);
  FParseMethods3.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.JapaneseEra, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.Hour, OnError);
  FParseMethods3.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods3[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods3.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnError);
...
begin
  FParseMethods4 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods4.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.AmPm or
  ...
  FParseMethods4.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods4[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods4.Add(TdxNumberFormatDesignator.DayOfWeek,
    OnDayOfWeekOrDefault3);
  FParseMethods4.Add(TdxNumberFormatDesignator.Default, OnDefault);
  FParseMethods4.Add(TdxNumberFormatDesignator.DecimalSeparator,
    OnDecimalSeparator);
  ...
  FParseMethods4.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnError);
  ...
  FParseMethods4.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods4[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods4.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.InvariantYear, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.JapaneseEra, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.Hour, OnError);
  FParseMethods4.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods4[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods4.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnError);
...
begin
  FParseMethods5 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods5.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnError);
  FParseMethods5.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnError);
  FParseMethods5.Add(TdxNumberFormatDesignator.AmPm or
  ...
  FParseMethods5.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods5[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods5.Add(TdxNumberFormatDesignator.DayOfWeek,
    OnDayOfWeekOrDefault3);
  ...
  FParseMethods5.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnError);
  ...
  FParseMethods5.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnError);
  FParseMethods5.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods5[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods5.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnError);
  FParseMethods5.Add(TdxNumberFormatDesignator.General or
  ...
  FParseMethods5.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods5[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods5.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnError);
...
begin
  FParseMethods6 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods6.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnError);
  FParseMethods6.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnError);
  FParseMethods6.Add(TdxNumberFormatDesignator.AmPm or
  ...
  FParseMethods6.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods6[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods6.Add(TdxNumberFormatDesignator.DayOfWeek,
    OnDayOfWeekOrDefault3);
  ...
  FParseMethods6.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnError);
  ...
  FParseMethods6.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnError);
  FParseMethods6.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods6[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods6.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnError);
  FParseMethods6.Add(TdxNumberFormatDesignator.General or
  ...
  FParseMethods6.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods6[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods6.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnError);
...
begin
  FParseMethods7 :=
    TDictionary<Integer, TdxNumberFormatDesignatorParseMethod>.Create;
  FParseMethods7.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek, OnError);
  FParseMethods7.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month, OnError);
  FParseMethods7.Add(TdxNumberFormatDesignator.AmPm or
  ...
  FParseMethods7.Add(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.DayOfWeek or TdxNumberFormatDesignator.Month,
    FParseMethods7[(TdxNumberFormatDesignator.AmPm or
    TdxNumberFormatDesignator.Month)]);
  ...
  FParseMethods7.Add(TdxNumberFormatDesignator.DayOfWeek,
    OnDayOfWeekOrDefault3);
  ...
  FParseMethods7.Add(TdxNumberFormatDesignator.FractionOrDateSeparator,
    OnDefault);
  ...
  FParseMethods7.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day, OnGeneralOrDateTime7);
  FParseMethods7.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day or TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods7[(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Day)]);
  FParseMethods7.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.Second, OnGeneralOrDateTime7);
  FParseMethods7.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.InvariantYear, OnGeneralOrDateTime7);
  FParseMethods7.Add(TdxNumberFormatDesignator.General or
    TdxNumberFormatDesignator.JapaneseEra, OnGeneralOrDateTime7);
  FParseMethods7.Add(TdxNumberFormatDesignator.Hour, OnError);
  FParseMethods7.Add(TdxNumberFormatDesignator.Hour or
    TdxNumberFormatDesignator.JapaneseEra,
    FParseMethods7[TdxNumberFormatDesignator.Hour]);
  ...
  FParseMethods7.Add(TdxNumberFormatDesignator.Minute or
    TdxNumberFormatDesignator.Month, OnError);
