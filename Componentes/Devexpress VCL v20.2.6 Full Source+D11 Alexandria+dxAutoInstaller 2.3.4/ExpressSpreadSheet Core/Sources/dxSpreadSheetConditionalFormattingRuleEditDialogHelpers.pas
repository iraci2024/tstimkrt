...
uses
  ...
  cxGraphics, cxDropDownEdit, cxMaskEdit, cxTL, cxColorComboBox,
  cxImageComboBox, cxEdit,
  dxSpreadSheetConditionalFormatting, dxSpreadSheetConditionalFormattingRules,
  dxSpreadSheetConditionalFormattingIconSet,
  dxSpreadSheetStyles, dxSpreadSheetCoreFormulas, dxSpreadSheetCoreDialogsStrs,
  dxSpreadSheetCoreStrs;

type
  TdxSpreadSheetConditionalFormattingRuleType = (cfrtCellIs, cfrtExpression,
    cfrtDuplicateValues, cfrtUniqueValues, cfrtAboveOrBelowAverage,
    cfrtTopBottomValues, cfrtTwoColorScale, cfrtThreeColorScale, cfrtIconSet,
    cfrtDataBar);
  ...
  protected
    ...
    property ConditionalFormatting: TdxSpreadSheetCustomConditionalFormatting
      read GetConditionalFormatting;
  public
    constructor Create(AHelper
    ...
    property RuleClass: TdxSpreadSheetCustomConditionalFormattingRuleClass
      read GetRuleClass;
    property RulePreview: TdxSpreadSheetCustomConditionalFormattingRule
      read GetRulePreview;
    property RuleType: TdxSpreadSheetConditionalFormattingRuleType
      read FRuleType;
  end;
  ...
  strict private
    ...
    FRuleInfos
      : TObjectList<TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo>;
    FSelectedArea: TRect;
    FSelectedRuleInfo
      : TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo;

    function GetSelectedRulePreview
      : TdxSpreadSheetCustomConditionalFormattingRule;
    procedure SetSelectedRuleInfo
  ...
  public
    constructor Create(AConditionalFormatting
      : TdxSpreadSheetCustomConditionalFormatting);
    destructor Destroy; override;
    procedure BeforeDestruction; override;
    procedure SaveChanges(var ARule
      : TdxSpreadSheetCustomConditionalFormattingRule);

    // Validation
    function AreExpressionsValid
      (ARule: TdxSpreadSheetConditionalFormattingRuleCellIs): Boolean; overload;
    function AreExpressionsValid
      (ARule: TdxSpreadSheetConditionalFormattingRuleExpression)
      : Boolean; overload;
    function IsExpressionValid(AExpression
      : TdxSpreadSheetCustomFormula): Boolean;

    property RuleInfos
      : TObjectList<TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo>
      read FRuleInfos;
    property SelectedArea: TRect read FSelectedArea write FSelectedArea;
    property SelectedRuleInfo
      : TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo
      read FSelectedRuleInfo write SetSelectedRuleInfo;
    property SelectedRulePreview: TdxSpreadSheetCustomConditionalFormattingRule
      read GetSelectedRulePreview;
    property ConditionalFormatting: TdxSpreadSheetCustomConditionalFormatting
  ...
  TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper = record
    ...
    Stops: array [0 .. 2]
      of TdxSpreadSheetConditionalFormattingRuleCustomScaleStop;

    function AreActualValuesInValidOrder: Boolean;
    procedure BindEditors(ATypeEditor: TcxComboBox; AValueEditor: TcxMaskEdit;
      AColorEditor: TcxColorComboBox);
    procedure Initialize(ARule
  ...
  strict private
    ...
    function GetValueFromComboBox(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn): TObject;
    procedure LoadStop(ANode: TcxTreeListNode;
      AStop: TdxSpreadSheetConditionalFormattingRuleIconSetStop);
  public
    constructor Create(ATreeList: TcxTreeList);
    function AreActualValuesInValidOrder: Boolean;
    function GetIconSetIndex(AItems: TcxImageComboBoxItems;
      const APresetName: string): Integer;
    function GetStopType(ANode: TcxTreeListNode;
      AValueTypeColumn: TcxTreeListColumn)
    ...
    procedure SetComparisonOperator(ANode: TcxTreeListNode;
    ...
    property Presets: TdxSpreadSheetConditionalFormattingIconSetPresets
      read GetPresets;
  ...
  public
    class function FindRuleType(ATreeList: TcxTreeList; AClass: TClass;
      out ANode: TcxTreeListNode): Boolean;
    class procedure PopulateCellIsOperators(AItems: TStrings);
    class procedure PopulateRuleTypes(ATreeList: TcxTreeList;
    ...
    // Scales
    class function GetScaleStopValueType(ACombobox: TcxComboBox)
      : TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType;
    class procedure PopulateScaleStopTypes(ACombobox: TcxComboBox); overload;
    class procedure PopulateScaleStopTypes(AItems: TStrings;
      AStopIndex: Integer); overload;
  end;

implementation

uses
  SysUtils, dxCore, cxGeometry, Math, cxFormats, cxVariants,
  dxSpreadSheetFormatCellsDialog, dxSpreadSheetFormulas,
  dxSpreadSheetTypes, dxCoreClasses;

const
  RuleClasses: array [TdxSpreadSheetConditionalFormattingRuleType]
    of TdxSpreadSheetCustomConditionalFormattingRuleClass =
...
type
  TdxSpreadSheetCustomConditionalFormattingAccess = class
    (TdxSpreadSheetCustomConditionalFormatting);
  TdxSpreadSheetConditionalFormattingRuleExpressionAccess = class
    (TdxSpreadSheetConditionalFormattingRuleExpression);
  TdxSpreadSheetCustomConditionalFormattingRuleAccess = class
    (TdxSpreadSheetCustomConditionalFormattingRule);

function GetScaleStopValueTypeName
...
function GetCellIsRuleOperatorName(AOperator
  : TdxSpreadSheetConditionalFormattingRuleCellIsComparisonOperator): string;
var
  AMessagePtr: Pointer;
begin
  AMessagePtr := nil;
  case AOperator of
    ...
    cicoGreaterThanOrEqual:
      AMessagePtr :=
        @sdxConditionalFormattingCellIsRuleComparisonOperatorGreaterThanOrEqual;
    cicoLessThan:
      AMessagePtr :=
        @sdxConditionalFormattingCellIsRuleComparisonOperatorLessThan;
    cicoLessThanOrEqual:
      AMessagePtr :=
        @sdxConditionalFormattingCellIsRuleComparisonOperatorLessThanOrEqual;
    cicoNotBetween:
      AMessagePtr :=
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo.DrawPreview
  (ACanvas: TcxCanvas; const R: TRect);
begin
  TdxSpreadSheetCustomConditionalFormattingRuleAccess(RulePreview)
    .DrawPreview(ACanvas, R);
end;

function TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo.GetStyle
  (out AStyle: TdxSpreadSheetCellStyle): Boolean;
begin
  Result := RulePreview is TdxSpreadSheetConditionalFormattingRuleStyleBased;
  if Result then
    AStyle := TdxSpreadSheetConditionalFormattingRuleStyleBased
      (RulePreview).Style;
end;

function TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo.
  GetConditionalFormatting: TdxSpreadSheetCustomConditionalFormatting;
begin
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo.GetRuleClass
  : TdxSpreadSheetCustomConditionalFormattingRuleClass;
begin
  Result := RuleClasses[RuleType];
end;

function TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo.
  GetRulePreview: TdxSpreadSheetCustomConditionalFormattingRule;
begin
...
constructor TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.Create
...
begin
  ...
  FRuleInfos :=
    TObjectList<
    TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo>.Create;
  FCancelChanges := True;
  PopulateRulesInfo;
  BeginEditing;
end;
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.
  BeforeDestruction;
begin
...
begin
  ...
  try
    ...
    begin
      FreeAndNil(ARule);
      ARule := TdxSpreadSheetCustomConditionalFormattingRuleClass
        (SelectedRulePreview.ClassType).Create(ConditionalFormatting);
    end;
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.
  AreExpressionsValid(ARule
  : TdxSpreadSheetConditionalFormattingRuleCellIs): Boolean;
begin
  Result := AreExpressionsValid
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.
  AreExpressionsValid(ARule
  : TdxSpreadSheetConditionalFormattingRuleExpression): Boolean;
begin
  Result := IsExpressionValid
    (TdxSpreadSheetConditionalFormattingRuleExpressionAccess(ARule)
    .Formulas[0]);
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.
  IsExpressionValid(AExpression: TdxSpreadSheetCustomFormula): Boolean;
var
...
begin
  ...
  begin
    if (FSelectedRuleInfo <> nil) and (AValue <> nil) and
      (AValue.FRulePreview = nil) then
    begin
      if AValue.GetStyle(ATargetStyle) and FSelectedRuleInfo.GetStyle
        (ASourceStyle) then
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.
  CanValidateExpressionResult: Boolean;
begin
  Result := TdxSpreadSheetCustomConditionalFormattingAccess
    (ConditionalFormatting).CanValidateExpressionRuleResultValue;
end;

procedure TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.EndEditing
  (ACancel: Boolean);
begin
...
begin
  for AType := Low(TdxSpreadSheetConditionalFormattingRuleType)
    to High(TdxSpreadSheetConditionalFormattingRuleType) do
    RuleInfos.Add(TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo.
      Create(Self, AType));
end;

function TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.
  GetSelectedRulePreview: TdxSpreadSheetCustomConditionalFormattingRule;
begin
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper.
  AreActualValuesInValidOrder: Boolean;

  function CanProcessStop
    (AStop: TdxSpreadSheetConditionalFormattingRuleCustomScaleStop): Boolean;
  begin
    Result := (AStop <> nil) and
      not(AStop.ValueType in [cssvtLimitValue, cssvtFormula]);
  end;
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper.
  BindEditors(ATypeEditor: TcxComboBox; AValueEditor: TcxMaskEdit;
  AColorEditor: TcxColorComboBox);
begin
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper.
  Initialize(ARule: TdxSpreadSheetConditionalFormattingRuleCustomScale);
var
  AStop: TdxSpreadSheetConditionalFormattingRuleCustomScaleStop;
  I: Integer;
begin
  ...
  begin
    Stops[0] := TdxSpreadSheetConditionalFormattingRuleTwoColorScale
      (ARule).MinValue;
    Stops[2] := TdxSpreadSheetConditionalFormattingRuleTwoColorScale
      (ARule).MaxValue;
  end
  else
  ...
  begin
    Stops[0] := TdxSpreadSheetConditionalFormattingRuleThreeColorScale
      (ARule).MinValue;
    Stops[1] := TdxSpreadSheetConditionalFormattingRuleThreeColorScale(ARule)
      .MiddleValue;
    Stops[2] := TdxSpreadSheetConditionalFormattingRuleThreeColorScale
      (ARule).MaxValue;
  end;
  ...
  begin
    ...
    if AStop <> nil then
    begin
      EditorType[I].ItemIndex :=
        Max(0, EditorType[I].Properties.Items.IndexOfObject
        (TObject(AStop.ValueType)));
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper.
  GetStopColor(AStop
  : TdxSpreadSheetConditionalFormattingRuleCustomScaleStop): TColor;
begin
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper.
  SetStopColor(AIndex: Integer; AColor: TColor);
begin
  SetStopColor(Stops[AIndex], AColor);
end;

procedure TdxSpreadSheetConditionalFormattingRuleEditDialogScaleHelper.
  SetStopColor(AStop: TdxSpreadSheetConditionalFormattingRuleCustomScaleStop;
  AColor: TColor);
begin
  if AStop is TdxSpreadSheetConditionalFormattingRuleColorScaleStop then
    TdxSpreadSheetConditionalFormattingRuleColorScaleStop(AStop).Color
      := AColor;
end;

{ TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper }

constructor TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  Create(ATreeList: TcxTreeList);
begin
  inherited Create;
  FTreeList := ATreeList;
end;

function TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  AreActualValuesInValidOrder: Boolean;

  function CanProcessStop
    (AStop: TdxSpreadSheetConditionalFormattingRuleIconSetStop): Boolean;
  begin
    Result := (AStop <> nil) and
      not(AStop.ValueType in [cssvtLimitValue, cssvtFormula]);
  end;
...
begin
  Result := True;
  AIndex := 1;
  while (AIndex < FRule.StopCount) and
    not CanProcessStop(FRule.Stops[AIndex]) do
    Inc(AIndex);
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  GetIconSetIndex(AItems: TcxImageComboBoxItems;
  const APresetName: string): Integer;
var
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  GetStopType(ANode: TcxTreeListNode; AValueTypeColumn: TcxTreeListColumn)
  : TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType;
begin
  Result := TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType
    (GetValueFromComboBox(ANode, AValueTypeColumn));
end;

procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.Load
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  PopulateComparisonOperators(AProperties: TcxComboBoxProperties);
begin
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  PopulateIcons(AProperties: TcxImageComboBoxProperties);
var
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  PopulateIconSets(AIconSets: TcxImageComboBox);
var
  AImageIndex: Integer;
  AItem: TcxImageComboBoxItem;
  APreset: TdxSpreadSheetConditionalFormattingIconSetPreset;
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  PopulateStopValueTypes(AProperties: TcxComboBoxProperties);
var
  AType: TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType;
begin
  AProperties.Items.BeginUpdate;
  try
    ...
    begin
      if AType <> cssvtLimitValue then
        AProperties.Items.AddObject(GetScaleStopValueTypeName(AType),
          TObject(AType));
    end;
  finally
    AProperties.Items.EndUpdate;
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetComparisonOperator(ANode: TcxTreeListNode; AColumn: TcxTreeListColumn);
var
  AStop: TdxSpreadSheetConditionalFormattingRuleIconSetStop;
begin
  AStop := TdxSpreadSheetConditionalFormattingRuleIconSetStop(ANode.Data);
  AStop.ComparisonOperator :=
    TdxSpreadSheetConditionalFormattingRuleIconSetStopComparisonOperator
    (GetValueFromComboBox(ANode, AColumn));
  LoadStop(ANode, AStop);
  UpdateRowInfo;
end;

procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetIconIndex(ANode: TcxTreeListNode; AColumn: TcxTreeListColumn);
begin
  TdxSpreadSheetConditionalFormattingRuleIconSetStop(ANode.Data).IconIndex :=
    ANode.Values[AColumn.ItemIndex];
end;

procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetIconSet(ASetIndex: Integer);
begin
  if ASetIndex >= 0 then
  begin
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetReverseIconOrder(AReverseIconOrder: Boolean);
const
  Map: array [Boolean] of TdxSpreadSheetConditionalFormattingRuleIconSetOrder =
    (isioNormal, isioReversed);
begin
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetShowIconsOnly(AShowIconsOnly: Boolean);
begin
  FRule.ShowValue := not AShowIconsOnly;
end;

procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetStopValue(ANode: TcxTreeListNode; const AValue: Variant);
var
  AStop: TdxSpreadSheetConditionalFormattingRuleIconSetStop;
begin
  AStop := TdxSpreadSheetConditionalFormattingRuleIconSetStop(ANode.Data);
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  SetStopValueType(ANode: TcxTreeListNode;
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  ...
  begin
    if TdxSpreadSheetConditionalFormattingRuleIconSetStop(ANode.Data)
      .ValueType = cssvtFormula then
      Result := cxGetResourceString
        (@sdxConditionalFormattingRuleEditDialogScaleValueTypeFormula)
    else if VarIsSoftNull(ANode.Values[3]) then
      Result := ''
    else
  ...
  begin
    ...
    else if TdxSpreadSheetConditionalFormattingRuleIconSetStop
      (ANode.getPrevSibling.Data).ComparisonOperator = isscoGreaterThan then
    begin
  ...
  begin
    ...
    if ANode.IsFirst then
      ...
      ANode.Values[1] := Format(AFormatString,
        [GetValue(ANode.getPrevSibling)]);
  end;
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.GetIcons
  : TcxImageList;
begin
...
function TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  GetPresets: TdxSpreadSheetConditionalFormattingIconSetPresets;
begin
  Result := ConditionalFormattingIconSet.Presets;
end;

function TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  GetValueFromComboBox(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn): TObject;
var
  AItems: TStrings;
  AValue: string;
...
procedure TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.
  LoadStop(ANode: TcxTreeListNode;
  AStop: TdxSpreadSheetConditionalFormattingRuleIconSetStop);
  ...
  var
    AItems: TStrings;
  begin
    AItems := (FTreeList.Columns[AColumnIndex]
...
class function TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  FindRuleType(ATreeList: TcxTreeList; AClass: TClass;
  out ANode: TcxTreeListNode): Boolean;
var
  I: Integer;
begin
  ...
  begin
    ...
    if TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo(ANode.Data)
      .RuleClass = AClass then
      Exit(True);
  end;
  Result := False;
end;

class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  PopulateCellIsOperators(AItems: TStrings);
var
  AComparisonOperator
...
begin
  ...
  try
    ...
    for AComparisonOperator := Low(AComparisonOperator)
      ...
      AItems.AddObject(GetCellIsRuleOperatorName(AComparisonOperator),
        TObject(AComparisonOperator));
  finally
    AItems.EndUpdate;
  end;
end;
...
class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  PopulateRuleTypes(ATreeList: TcxTreeList;
  AHelper: TdxSpreadSheetConditionalFormattingRuleEditDialogHelper);
var
  I: Integer;
begin
  ...
  try
    ...
    for I := 0 to AHelper.RuleInfos.Count - 1 do
      ATreeList.AddChild(nil, AHelper.RuleInfos[I]).Texts[0] :=
        AHelper.RuleInfos[I].Name;
  finally
    ATreeList.EndUpdate;
  end;
end;
...
class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  PopulateTopBottomDirections(AItems: TStrings);
begin
  AItems.BeginUpdate;
  try
    AItems.Clear;
    AItems.AddObject
      (Format(cxGetResourceString(@sdxConditionalFormattingTopValues), ['']),
      TObject(tbvdTop));
    AItems.AddObject
      (Format(cxGetResourceString(@sdxConditionalFormattingBottomValues), ['']),
      TObject(tbvdBottom));
  finally
    AItems.EndUpdate;
  end;
...
class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  LoadAboveOrBelowAverageRuleInfo(ACombobox: TcxComboBox;
  ARule: TdxSpreadSheetConditionalFormattingRuleAboveOrBelowAverage);
var
  AIndex: Integer;
begin
  ...
  ACombobox.ItemIndex := ACombobox.Properties.Items.IndexOfObject
    (TObject(AIndex));
end;

class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  SaveAboveOrBelowAverageRuleInfo(ACombobox: TcxComboBox;
  ARule: TdxSpreadSheetConditionalFormattingRuleAboveOrBelowAverage);
var
  AIndex: Integer;
begin
...
class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  PopulateAboveOrBelowAverageOperators(AItems: TStrings);
var
  I: Integer;
begin
  AItems.BeginUpdate;
  try
    AItems.Clear;
    AItems.AddObject(cxGetResourceString(@sdxConditionalFormattingAboveAverage),
      TObject(abacoAboveAverage));
    AItems.AddObject(cxGetResourceString(@sdxConditionalFormattingBelowAverage),
      TObject(abacoBelowAverage));
    AItems.AddObject(cxGetResourceString
      (@sdxConditionalFormattingAboveOrEqualAverage),
      TObject(abacoAboveOrEqualAverage));
    AItems.AddObject(cxGetResourceString
      (@sdxConditionalFormattingBelowOrEqualAverage),
      TObject(abacoBelowOrEqualAverage));

    for I := TdxSpreadSheetConditionalFormattingRuleAboveOrBelowAverage.
      MinStandardDeviationLevel to
      TdxSpreadSheetConditionalFormattingRuleAboveOrBelowAverage.
      MaxStandardDeviationLevel do
    begin
      AItems.AddObject
        (Format(cxGetResourceString
        (@sdxConditionalFormattingAboveAverageOnStandardDeviation), [I]),
        TObject(I * 10));
      AItems.AddObject
        (Format(cxGetResourceString
        (@sdxConditionalFormattingBelowAverageOnStandardDeviation), [I]),
        TObject(-I * 10));
    end;
  finally
    AItems.EndUpdate;
...
class function TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  GetScaleStopValueType(ACombobox: TcxComboBox)
  : TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType;
begin
  Result := TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType
    (ACombobox.ItemObject);
end;

class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  PopulateScaleStopTypes(ACombobox: TcxComboBox);
begin
  PopulateScaleStopTypes(ACombobox.Properties.Items, ACombobox.Tag);
end;

class procedure TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
  PopulateScaleStopTypes(AItems: TStrings; AStopIndex: Integer);
var
  AString: string;
  AType: TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType;
begin
