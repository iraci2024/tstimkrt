...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, SysUtils, Controls, Variants, Menus, StdCtrls, Classes, ActnList,
  ImgList, Forms,
  cxControls, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters, cxContainer,
  cxEdit, cxCustomData, cxStyles, cxTL,
  cxTextEdit, cxDropDownEdit, dxForms, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, dxLayoutContainer, cxCheckBox,
  cxEditRepositoryItems, cxImageList, cxClasses, dxLayoutLookAndFeels,
  cxInplaceContainer, cxMaskEdit, cxButtons,
...
type
  ...
  TcxDataControllerConditionalFormattingRulesManagerDialogHelper = class
  ...
  TfrmDataControllerConditionalFormattingRulesManagerDialog = class
    ...
    procedure cxEditRepositoryCheckBoxApplyToTheRowPropertiesEditValueChanged
      (Sender: TObject);
    procedure tlcRuleApplyToTheRecordGetEditProperties
...
type
  ...
  TcxDataControllerConditionalFormattingRulesManagerDialogAdapter = class
    (TcxDataControllerConditionalFormattingRulesManagerDialogProvider.TAdapter)
  public
    procedure Execute(AProvider
      : TcxDataControllerConditionalFormattingProvider); override;
  end;

procedure TcxDataControllerConditionalFormattingRulesManagerDialogAdapter.
  Execute(AProvider: TcxDataControllerConditionalFormattingProvider);
var
...
begin
  ...
  ADialog := TfrmDataControllerConditionalFormattingRulesManagerDialog.Create
    (AProvider);
  try
...
function TcxDataControllerConditionalFormattingRulesManagerDialogHelper.
  ApplyToTheRowSupported(ARule
  : TdxSpreadSheetCustomConditionalFormattingRule): Boolean;
begin
  Result := TdxSpreadSheetCustomConditionalFormattingRuleAccess(ARule)
    .ApplyToTheRowSupported;;
end;
...
function TcxDataControllerConditionalFormattingRulesManagerDialogHelper.
  GetConditionalFormatting: TcxDataControllerConditionalFormatting;
begin
  Result := TcxDataControllerConditionalFormatting
    (inherited ConditionalFormatting);
end;

procedure TcxDataControllerConditionalFormattingRulesManagerDialogHelper.
  EndEditing(ACanceled: Boolean);
begin
...
procedure TfrmDataControllerConditionalFormattingRulesManagerDialog.
  ApplyLocalization;
begin
  ...
  tlcRuleApplyToTheRecord.Caption.Text :=
    cxGetResourceString
    (@sdxConditionalFormattingRulesManagerDialogColumnApplyToTheRecord);
end;

procedure TfrmDataControllerConditionalFormattingRulesManagerDialog.
  cxEditRepositoryCheckBoxApplyToTheRowPropertiesEditValueChanged
  (Sender: TObject);

  function GetToken(AExpression: TdxSpreadSheetConditionalFormattingExpression;
    out AToken: TdxSpreadSheetFormulaToken): Boolean;
  begin
...
begin
  ...
  begin
    ARule := TdxSpreadSheetConditionalFormattingRuleExpressionAccess
      (FocusedRuleInfo.Rule);
    if GetToken(ARule.Formulas[0], AToken) or GetToken(ARule.Formulas[1], AToken)
    then
    begin
...
begin
  ...
  for I := 0 to ConditionalFormatting.DataController.ItemCount - 1 do
    if ConditionalFormatting.IsItemVisible(I) then
    begin
      AProperties.Items.AddObject(ConditionalFormatting.GetItemDisplayName(I),
        TObject(I));
      cbDisplayMode.Properties.Items.AddObject
        (ConditionalFormatting.GetItemDisplayName(I), TObject(I));
    end;
...
begin
  ...
  begin
    AItemIndex := Integer(cbDisplayMode.Properties.Items.Objects
      [cbDisplayMode.ItemIndex]);
    AArea := TRect.Create(AItemIndex, 0, AItemIndex, MaxInt);
  end;
  TcxDataControllerConditionalFormattingProviderAccess
    (TcxDataControllerConditionalFormattingAccess(ConditionalFormatting)
    .Provider).CustomSelectedArea := AArea;
  if cbDisplayMode.ItemIndex in [0, 1] then
...
function TfrmDataControllerConditionalFormattingRulesManagerDialog.
  GetConditionalFormatting: TcxDataControllerConditionalFormatting;
begin
  Result := Helper.ConditionalFormatting;
end;

function TfrmDataControllerConditionalFormattingRulesManagerDialog.GetHelper
  : TcxDataControllerConditionalFormattingRulesManagerDialogHelper;
begin
...
procedure TfrmDataControllerConditionalFormattingRulesManagerDialog.
  RuleAreaPropertiesEditValueChangedHandler(Sender: TObject);
var
  AComboBox: TcxComboBox;
  AItemIndex: Integer;
begin
  AComboBox := Sender as TcxComboBox;
  AItemIndex := Integer(AComboBox.Properties.Items.Objects
    [AComboBox.ItemIndex]);
  Helper.SetArea(FocusedRuleInfo, TRect.Create(AItemIndex, 0,
    AItemIndex, MaxInt));
end;

procedure TfrmDataControllerConditionalFormattingRulesManagerDialog.
  tlcRuleApplyToTheRecordGetEditProperties(Sender: TcxTreeListColumn;
  ANode: TcxTreeListNode; var EditProperties: TcxCustomEditProperties);
begin
  if Helper.ApplyToTheRowSupported
    (TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo(ANode.Data)
    .Rule) then
...
procedure TfrmDataControllerConditionalFormattingRulesManagerDialog.
  tlRulesEditing(Sender: TcxCustomTreeList; AColumn: TcxTreeListColumn;
  var Allow: Boolean);
begin
  inherited;
  Allow := Allow or (AColumn = tlcRuleApplyToTheRecord) and
    Helper.ApplyToTheRowSupported(FocusedRuleInfo.Rule);
end;
...
TcxDataControllerConditionalFormattingRulesManagerDialogProvider.
  RegisterRulesManagerDialog
  (TcxDataControllerConditionalFormattingRulesManagerDialogAdapter);

finalization

TcxDataControllerConditionalFormattingRulesManagerDialogProvider.
  UnregisterRulesManagerDialog
  (TcxDataControllerConditionalFormattingRulesManagerDialogAdapter);
