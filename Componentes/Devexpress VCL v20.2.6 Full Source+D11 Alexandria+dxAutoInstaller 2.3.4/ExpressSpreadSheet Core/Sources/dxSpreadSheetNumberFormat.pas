...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Character, Classes, Graphics, Variants, SysUtils, Generics.Collections,
  Generics.Defaults,
...
type
  ...
  TdxSpreadSheetNumberFormatCategory = (nfcGeneral, nfcNumber, nfcCurrency,
    nfcAccounting, nfcDate, nfcTime, nfcPercentage, nfcFraction, nfcScientific,
    nfcText, nfcCustom);
  ...
  public
    ...
    function Format(const AValue: Variant;
      const AFormatSettings: TFormatSettings)
      : TdxSpreadSheetNumberFormatResult; overload;
  ...
  public
    class function GetFormat(ASettings: TdxSpreadSheetFormatSettings;
    ...
    class function GetPositiveFormat(ACurrency: string; const APattern: string;
      AFormatCode: Byte; AIncludeCurrencyIntoMarkChar: Boolean = False): string;
    class function IsCurrencySymbolFollowsValue(ANegFormatCode
      : Integer): Boolean;
  end;
  ...
  public
    class function ConvertDateTimeDisplayFormat(const S: string;
      const AValue: TDateTime): string;
    class function ConvertFloatValueDisplayFormat(const S: string): string;
    class function ConvertFloatValueDisplayFormatSection
      (const S: string): string;
  end;

function dxGetDataTypeByVariantValue(const AValue: Variant)
...
uses
  dxSpreadSheetCoreStrs, dxSpreadSheetUtils, dxSpreadSheetNumberFormatParser,
  cxFormats, StrUtils, dxStringHelper;
...
begin
  ...
  try
    AFormat.Format(AValue, dxGetDataTypeByVariantValue(AValue),
      AFormatSettings, Result);
  finally
    AFormat.Free;
  end;
end;
...
function TdxSpreadSheetCustomNumberFormat.Format(const AValue: Variant;
  const AFormatSettings: TFormatSettings): TdxSpreadSheetNumberFormatResult;
begin
  Format(AValue, dxGetDataTypeByVariantValue(AValue), dts1900,
    AFormatSettings, Result);
end;
...
procedure TdxSpreadSheetCustomNumberFormat.Format(const AValue: Variant;
  AValueType: TdxSpreadSheetCellDataType;
  ADateTimeSystem: TdxSpreadSheetDateTimeSystem;
  const AFormatSettings: TFormatSettings;
...
constructor TdxSpreadSheetNumberFormat.Create(const AFormatCode: string;
...
begin
  Result := GetPositiveFormat(ASettings.CurrencyFormat, APattern,
    ASettings.Data.CurrencyFormat, True) + ';[Red]' +
    GetNegativeFormat(ASettings.CurrencyFormat, APattern,
    ASettings.Data.NegCurrFormat, True);
end;

class function TdxSpreadSheetCurrencyFormatHelper.GetNegativeFormat
  (ACurrency: string; const APattern: string; AFormatCode: Byte;
  AIncludeCurrencyIntoMarkChar: Boolean): string;

  function Make(const APrefix, ACurrency, APattern, ASuffix: string): string;
  begin
    if APattern[1] = '*' then
      Result := ACurrency + Copy(APattern, 1, 2) + APrefix +
        Copy(APattern, 3, MaxInt) + ASuffix
    else
...
class function TdxSpreadSheetCurrencyFormatHelper.IsCurrencySymbolFollowsValue
  (ANegFormatCode: Integer): Boolean;
begin
...
class procedure TdxSpreadSheetNumberFormatSplitter.Split(const AFormatCode
    ...
    begin
      ...
      case AChar of
        ...
        ';':
          if AHasBackSlash then
            AHasBackSlash := False
          else if not AHasQuotationMark then
          begin
            AList.Add(ATemp);
            ATemp := EmptyStr;
            Continue;
...
class function TdxSpreadSheetDateTimeFormatHelper.GetDefaultFormat
  (const AValue: TDateTime): string;
begin
...
class function TdxSpreadSheetDisplayFormatConverter.ConvertDateTimeDisplayFormat
  ...
  procedure ProcessSequence(var ASequenceSize, AIndex: Integer;
    ALength: Integer);
  var
    AChar: Char;
  begin
    ...
    begin
      case Result[AIndex] of
        'd', 'D':
          begin
            ...
            case ACount of
              ...
              5:
                ABuilder.Append
                  (ConvertDateTimeDisplayFormat
                  (dxFormatSettings.ShortDateFormat, AValue));
            else
              ABuilder.Append(ConvertDateTimeDisplayFormat
                (dxFormatSettings.LongDateFormat, AValue));
            end;
          end;
        ...
        'C':
          begin
            ...
            begin
              ABuilder.Append(' ');
              ABuilder.Append(ConvertDateTimeDisplayFormat
                (dxFormatSettings.LongTimeFormat, AValue));
            end;
          end;
        ...
        'Z', 'z':
          begin
            ProcessSequence(ACount, AIndex, ALength);
            if (ABuilder.Length > 0) and
              not dxCharIsNumeric(ABuilder.Chars[ABuilder.Length - 1]) then
...
class function TdxSpreadSheetDisplayFormatConverter.
  ConvertFloatValueDisplayFormat(const S: string): string;
var
  AList: TStringList;
...
class function TdxSpreadSheetDisplayFormatConverter.
  ConvertFloatValueDisplayFormatSection(const S: string): string;
var
  ABuilder: TStringBuilder;
  AIndex, I: Integer;
  ALength: Integer;
  ...
  try
    ...
    begin
      case S[AIndex] of
        ...
        ',':
          if (ABuilder.Length > 0) and (ABuilder[ABuilder.Length - 1] = '.')
          then
          begin
        ...
        begin
          I := AIndex + 1;
          while (I <= ALength) and not dxCharInSet(S[I],
            ['0', '#', '''', '"']) do
          begin
...
class procedure TdxSpreadSheetDisplayFormatConverter.CorrectDigitsPlaces
  (AData: TStringBuilder);
var
  ADigitCount: Integer;
  AIndex: Integer;
begin
  ...
  begin
    while (AIndex > 0) and (ADigitCount < 3) and
      IsDigitPlaceHolder(AData[AIndex - 1]) do
    begin
...
class function TdxSpreadSheetDisplayFormatConverter.IsDigitPlaceHolder
