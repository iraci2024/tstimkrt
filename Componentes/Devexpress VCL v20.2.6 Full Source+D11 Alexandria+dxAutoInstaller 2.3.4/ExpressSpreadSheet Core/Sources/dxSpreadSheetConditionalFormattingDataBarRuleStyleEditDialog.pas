...
type
  ...
  TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialogClass = class of
    TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog;
  ...
  protected
    ...
    procedure LoadStyle(AStyle
      : TdxSpreadSheetConditionalFormattingRuleDataBarStyle);
    procedure SaveStyle(AStyle
      : TdxSpreadSheetConditionalFormattingRuleDataBarStyle);
...
function ShowConditionalFormattingDataBarRuleStyleEditDialog
  ...
  AStyle: TdxSpreadSheetConditionalFormattingRuleDataBarStyle): Boolean;
  overload;
function ShowConditionalFormattingDataBarRuleStyleEditDialog(AOwner: TObject;
  AStyle: TdxSpreadSheetConditionalFormattingRuleDataBarStyle): Boolean;
  overload;
...
function GetBarDirectionName(ADirection
  : TdxSpreadSheetConditionalFormattingRuleDataBarDirection): string;
var
  AMessagePtr: Pointer;
begin
  AMessagePtr := nil;
...
function GetFillModeName(AFillMode
  : TdxSpreadSheetConditionalFormattingRuleDataBarFillMode): string;
var
  AMessagePtr: Pointer;
begin
  AMessagePtr := nil;
...
begin
  ...
  begin
    ADialog :=
      dxSpreadSheetConditionalFormattingDataBarRuleStyleEditDialogClass.Create
      (AOwner.GetParentForm);
    try
...
begin
  ...
  Assert(Result);
  Result := Result and ShowConditionalFormattingDataBarRuleStyleEditDialog
    (AOwnerIntf, AStyle);
end;

{ TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog }

procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  ApplyLocalizations;
var
...
begin
  ...
  for AFillMode := Low(AFillMode) to High(AFillMode) do
    cbbFillMode.Properties.Items.AddObject(GetFillModeName(AFillMode),
      TObject(AFillMode));

  cbbBarDirection.Properties.Items.Clear;
  for ADirection := Low(ADirection) to High(ADirection) do
    cbbBarDirection.Properties.Items.AddObject(GetBarDirectionName(ADirection),
      TObject(ADirection));

  cbbBorderStyle.Properties.Items.Clear;
  cbbBorderStyle.Properties.Items.Add
    (cxGetResourceString
...
procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  Initialize(AConditionalFormatting: TdxSpreadSheetCustomConditionalFormatting;
  ALookAndFeel: TcxLookAndFeel);
begin
...
procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  LoadStyle(AStyle: TdxSpreadSheetConditionalFormattingRuleDataBarStyle);
begin
  cbbFillMode.ItemIndex :=
    Max(0, cbbFillMode.Properties.Items.IndexOfObject
    (TObject(AStyle.FillMode)));
  cbbBarDirection.ItemIndex :=
    Max(0, cbbBarDirection.Properties.Items.IndexOfObject
    (TObject(AStyle.Direction)));
  ...
  rbNegativeBarBorderColor.Checked := AStyle.NegativeBarBorderColor <>
    clDefault;
  rbNegativeBarBorderColorAuto.Checked :=
...
procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  SaveStyle(AStyle: TdxSpreadSheetConditionalFormattingRuleDataBarStyle);
begin
  AStyle.FillMode := TdxSpreadSheetConditionalFormattingRuleDataBarFillMode
    (cbbFillMode.ItemObject);
  AStyle.Direction := TdxSpreadSheetConditionalFormattingRuleDataBarDirection
    (cbbBarDirection.ItemObject);
  ...
  else if rbAxisMidpoint.Checked then
...
begin
  ...
  ccbNegativeBarBorderColor.Enabled := (cbbBorderStyle.ItemIndex > 0) and
    rbNegativeBarBorderColor.Checked;
...
procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  cbbPositiveBarBorderStylePropertiesChange(Sender: TObject);
begin
  UpdateControlsState;
end;

procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  rbAxisAutoClick(Sender: TObject);
begin
  ccbAxisColor.Enabled := not rbAxisNone.Checked;
end;

procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  rbNegativeBarBorderColorClick(Sender: TObject);
begin
  UpdateControlsState;
end;

procedure TfrmSpreadSheetConditionalFormattingDataBarRuleStyleEditDialog.
  rbNegativeBarColorClick(Sender: TObject);
begin
