...
type
  ...
  public
    property EditControl: TcxCustomTreeList read FEditControl
      write SetEditControl;
...
begin
  ...
  begin
    SetValues(IntToStr(FocusedNode.ImageIndex),
      IntToStr(FocusedNode.SelectedIndex), IntToStr(FocusedNode.StateIndex),
      IntToStr(FocusedNode.OverlayIndex),
  ...
  begin
    SetValues(edtDefImage.Text, edtDefSelected.Text, edtDefState.Text,
      edtDefOverlay.Text, edtDefOverlayState.Text);
...
procedure TfrmItemsEditor.edtImageKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmItemsEditor.cxTLSiteDragOver(Sender, Source: TObject;
...
begin
  if FocusedNode <> nil then
    FocusedNode.Parent.CheckGroupType := TcxTreeListNodeCheckGroupType
      (cbCheckGroupType.ItemIndex);
end;
