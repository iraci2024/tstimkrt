...
uses
  ...
  cxDesignWindows, Forms, Controls, ComCtrls, Graphics, Dialogs, Menus,
  StdCtrls,
...
type
  ...
  TcxTreeListSetIndexProc = procedure(AItem: TObject; AIndex: Integer)
    of object;
  TcxTreeListAddItemProc = procedure(AListBox: TcxListBox; AIndex: Integer)
    of object;

  TcxTreeListDesignerChange = (tldsgStructure, tldsgSelection);
  TcxTreeListDesignerChanges = set of TcxTreeListDesignerChange;

  TcxTreeListBandColumnDesigner = class(TcxDesignFormEditor,
    IcxTreeListDesigner)
    btnBAdd: TcxButton;
    btnBDel: TcxButton;
    btnBMoveD: TcxButton;
    ...
    procedure lbxBandsDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
      AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
    procedure lbxColumnsDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
      AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
  ...
  protected
    ...
    procedure SelectAllItems(AListBox: TcxListBox;
      ADefaultComponent: TPersistent);
    procedure UpdateItems(AListBox: TcxListBox; AItemCount: Integer;
    ...
    procedure DrawListBoxItem(AListBox: TcxListBox; ACanvas: TcxCanvas;
      AIndex: Integer; R: TRect; AParentBand: TcxTreeListBand);

    procedure MoveBy(AListBox: TcxListBox; AIncrement: Integer;
...
begin
  ...
  else if AItem is TcxTreeListBand then
    DeleteItem(lbxBands);
end;
...
procedure TcxTreeListBandColumnDesigner.SelectionsChanged(const ASelection
...
procedure TcxTreeListBandColumnDesigner.AddBand(AListBox: TcxListBox;
  AIndex: Integer);
...
procedure TcxTreeListBandColumnDesigner.AddColumn(AListBox: TcxListBox;
  AIndex: Integer);
begin
  AListBox.Items.AddObject(TreeList.Columns[AIndex].Name,
    TreeList.Columns[AIndex]);
  // TreeList.Columns[AIndex].FreeNotification(Self);
end;

function TcxTreeListBandColumnDesigner.GetOperations
  ...
  begin
    ...
    begin
      if AComponent.Components[I] is TcxControl then
        SetControlLookAndFeel(TWinControl(AComponent.Components[I]),
...
procedure TcxTreeListBandColumnDesigner.MoveBy(AListBox: TcxListBox;
...
procedure TcxTreeListBandColumnDesigner.SyncEnabled;
  ...
  procedure SyncEnabled(AButton: TcxButton; AMenuItem: TMenuItem;
    AEnabled: Boolean);
  begin
...
procedure TcxTreeListBandColumnDesigner.SelectAllItems(AListBox: TcxListBox;
  ADefaultComponent: TPersistent);
begin
  AListBox.Items.BeginUpdate;
  try
...
procedure TcxTreeListBandColumnDesigner.UpdateItems(AListBox: TcxListBox;
  AItemCount: Integer; AAddItemProc: TcxTreeListAddItemProc);

  function IsSelected(AItem: TObject; ASelections: IDesignerSelections)
    : Boolean;
  var
    I: Integer;
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      if AListBox.Selected[I] xor IsSelected(AListBox.Items.Objects[I],
        ASelections) then
...
const
  AChangeMap: array [Boolean] of TcxTreeListDesignerChange = (tldsgSelection,
    tldsgStructure);
var
...
procedure TcxTreeListBandColumnDesigner.CalculateListBoxItemHeight
  (AListBox: TcxListBox);
begin
  with AListBox do
  begin
...
procedure TcxTreeListBandColumnDesigner.DrawListBoxItem(AListBox: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; R: TRect; AParentBand: TcxTreeListBand);
...
procedure TcxTreeListBandColumnDesigner.SetBandIndex(AItem: TObject;
  AIndex: Integer);
begin
...
procedure TcxTreeListBandColumnDesigner.SetColumnIndex(AItem: TObject;
  AIndex: Integer);
begin
...
procedure TcxTreeListBandColumnDesigner.lbxBandsDrawItem(AControl: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect,
    TcxTreeListBand(lbxBands.Items.Objects[AIndex]).ParentBand);
end;

procedure TcxTreeListBandColumnDesigner.lbxColumnsDrawItem(AControl: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  DrawListBoxItem(AControl, ACanvas, AIndex, ARect,
    TcxTreeListColumn(lbxColumns.Items.Objects[AIndex]).Position.Band);
end;
