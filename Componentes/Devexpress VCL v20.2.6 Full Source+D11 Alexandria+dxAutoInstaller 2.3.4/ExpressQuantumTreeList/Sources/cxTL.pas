...
{$ENDIF}
  dxCore, dxCoreClasses, dxMessages, cxControls, cxContainer, cxGraphics,
  cxVariants, dxForms,
  cxStorage, cxClasses, cxLookAndFeels, cxLookAndFeelPainters, dxCustomHint,
  cxInplaceContainer, cxCheckBox, cxCheckListBox, cxNavigator, cxEdit,
  cxListBox,
  cxData, cxDataUtils, cxCustomData, cxStyles, cxGeometry, cxMemo,
  dxGDIPlusClasses,
  cxTLStrs, cxScrollBar, cxLibraryConsts, cxDataStorage, cxPC, cxLookupEdit,
  dxUxTheme,
  dxSpreadSheetTypes, dxSpreadSheetConditionalFormatting,
  dxUIElementPopupWindow,
  cxDataControllerConditionalFormatting, cxFilter, cxFilterControl,
  dxFilterPopupWindow,
  dxScrollbarAnnotations, cxFindPanel,
  cxDataControllerSpreadSheetExpressionProvider,
  dxAnimation, dxTypeHelpers, dxScrollAnimation, dxCoreGraphics;
...
type
  ...
  TcxTreeListImageIndexType = (tlitImageIndex, tlitSelectedIndex,
    tlitStateIndex, tlitOverlayIndex, tlitOverlayStateIndex);

  TcxTreeListImageIndexes = set of TcxTreeListImageIndexType;

  TcxTreeListNodeAttachMode = (tlamAdd, tlamAddFirst, tlamAddChild,
    tlamAddChildFirst, tlamInsert);
  TcxTreeListNodeAddMode = (tlnaAdd, tlnaAddFirst, tlnaInsert);
  TcxTreeListNodeImageIndexes = array [TcxTreeListImageIndexType]
    of TcxImageIndex;
  ...
  TcxTreeListFindFunc = function(ANode: TcxTreeListNode;
  ...
  TcxTreeListChange = (tcStructure, tcData, tcLayout, tcImages, tcColumns,
    ...
    tcFilterCriteria, tcFindCriteria);
  ...
  TcxTreeListNodeState = (nsDeleting, nsHasChildren, nsValuesAssigned,
    nsCollapsed, nsValidIndexes, nsInternalDelete, nsHidden, nsHeightAssigned,
    nsInserting, nsEditing, nsCheck, nsCheckStateInvalid, nsDisabled,
    nsSaveExpanded, nsHiddenByFilter);

  TcxTreeListNodeStates = set of TcxTreeListNodeState;

  TcxTreeListNodeCheckInfo = (nciChecked, nciGrayed, nciAllowGrayed,
    nciCheckGroup, nciRadioGroup, nciCheckValid, nciChangeCheck);
  ...
  private
    ...
    procedure SetParentFor(AValue: TcxTreeListNode;
      AValidateIndexes: Boolean = True);
  ...
  protected
    ...
    function GetNextSiblingEx(ANode: TcxTreeListNode; AForward: Boolean)
      : TcxTreeListNode;
    ...
    procedure SetChildrenCheckState(AValue: TcxCheckBoxState;
      AExclude: TcxTreeListNode);
    ...
    property HiddenByFilter: Boolean read GetHiddenByFilter
      write SetHiddenByFilter;
  ...
  public
    ...
    function CanMove(ADest: TcxTreeListNode;
      AMode: TcxTreeListNodeAttachMode): Boolean;
    procedure CheckClick;
    procedure Collapse(Recurse: Boolean);
    procedure CustomSort(ASortProc: TcxTreeListCompareFunc;
    ...
    function getNextSibling: TcxTreeListNode; inline;
    ...
    function getPrevSibling: TcxTreeListNode; inline;
    { GetPrevSibling conflicts with a C++ macro }
    function GetPrevSiblingVisible: TcxTreeListNode;
    function GetPrevVisible: TcxTreeListNode;
    function getFirstChild: TcxTreeListNode; inline;
    { GetFirstChild conflicts with C++ macro }
    ...
    function InsertChild(ABeforeNode: TcxTreeListNode)
      : TcxTreeListNode; virtual;
    procedure Invalidate;
    function IsMultiThreadedSorting: Boolean;
    function IsSibling(ANode: TcxTreeListNode): Boolean;
    procedure LoadChildren;
    procedure MakeVisible;
    procedure MoveTo(ADestNode: TcxTreeListNode;
      AMode: TcxTreeListNodeAttachMode); virtual;
    ...
    property CheckGroupType: TcxTreeListNodeCheckGroupType
      read GetCheckGroupType write SetCheckGroupType;
    ...
    property ImageIndex: TcxImageIndex index 0 read GetNodeImageIndex
      write SetNodeImageIndex;
    ...
    property OverlayIndex: TcxImageIndex index 3 read GetNodeImageIndex
      write SetNodeImageIndex;
    property OverlayStateIndex: TcxImageIndex index 4 read GetNodeImageIndex
      write SetNodeImageIndex;
    property Parent: TcxTreeListNode read FParent;
    property Root: TcxTreeListNode read GetRoot;
    property Selected: Boolean read GetSelected write SetSelected;
    property SelectedIndex: TcxImageIndex index 1 read GetNodeImageIndex
      write SetNodeImageIndex;
    property StateIndex: TcxImageIndex index 2 read GetNodeImageIndex
      write SetNodeImageIndex;
  ...
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
    procedure SetPosition(AColIndex, ARowIndex: Integer;
      IsInsertRow: Boolean = False);
    procedure SetPositionEx(ABandIndex, AColIndex, ARowIndex: Integer;
      IsInsertRow: Boolean = False);
  ...
  published
    ...
    property GlyphAlignHorz: TAlignment read FGlyphAlignHorz
      write SetGlyphAlignHorz default taLeftJustify;
    property GlyphAlignVert: TcxAlignmentVert read FGlyphAlignVert
      write SetGlyphAlignVert stored IsGlyphAlignVertStored;
  ...
  TcxTreeListColumnFilterPopupIncrementalFilteringOptions = class
  ...
  TcxTreeListColumnFilterPopupOptions = class
    (TcxCustomEditContainerItemFilterPopupOptions)
  private
    function GetIncrementalFiltering
      : TcxTreeListColumnFilterPopupIncrementalFilteringOptions;
    procedure SetIncrementalFiltering
      (AValue: TcxTreeListColumnFilterPopupIncrementalFilteringOptions);
  protected
    function GetIncrementalFilteringOptionsClass
      : TcxCustomEditContainerItemFilterPopupIncrementalFilteringOptionsClass;
      override;
  published
    ...
    property IncrementalFiltering
      : TcxTreeListColumnFilterPopupIncrementalFilteringOptions
      read GetIncrementalFiltering write SetIncrementalFiltering;
  ...
  private
    ...
    procedure SetExcelFilterPopup
      (AValue: TcxTreeListColumnExcelFilterPopupOptions);
  ...
  protected
    ...
    function GetExcelFilterPopupOptionsClass
      : TcxCustomEditContainerItemExcelFilterPopupOptionsClass; override;
    function GetFilterPopupOptionsClass
      : TcxCustomEditContainerItemFilterPopupOptionsClass; override;
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
  ...
  published
    property CellEndEllipsis: Boolean read FCellEndEllipsis
      write SetCellEndEllipsis default True;
    ...
    property GroupFooter: Boolean read FGroupFooter write SetGroupFooter
      default True;
    ...
    property VertSizing: Boolean read FVertSizing write SetVertSizing
      default True;
    property Customizing;
    property Editing;
    property ExcelFilterPopup: TcxTreeListColumnExcelFilterPopupOptions
      read GetExcelFilterPopup write SetExcelFilterPopup;
    property Filtering;
    property FilterPopup: TcxTreeListColumnFilterPopupOptions
      read GetFilterPopup write SetFilterPopup;
  ...
  protected
    ...
    function GetValueFormat(AValueType: TcxSummaryValueType;
      const AValue: Variant; AIsFooter: Boolean): string; override;
    procedure Notification(AComponent: TComponent; Operation: TOperation);
  public
    constructor Create(Collection: TCollection); override;
    function FormatValue(const AValue: Variant; AIsFooter: Boolean)
      : string; override;

    property AbsoluteIndex: Integer read FAbsoluteIndex write FAbsoluteIndex;
    property AlignHorzAssigned: Boolean read FAlignHorzAssigned
      write FAlignHorzAssigned;
    property AlignVertAssigned: Boolean read FAlignVertAssigned
      write FAlignVertAssigned;
    property Column: TcxTreeListColumn read GetColumn;
    property VisibleIndexInColumn: Integer read FVisibleIndexInColumn;
  published
    property AlignHorz: TAlignment read GetAlignHorz write SetAlignHorz
      stored IsAlignHorzStored;
    property AlignVert: TcxAlignmentVert read GetAlignVert write SetAlignVert
      stored IsAlignVertStored;
    property AllNodes: Boolean read FAllNodes write SetAllNodes default True;
    property CalculatedColumn: TcxTreeListColumn read GetCalculatedColumn
      write SetCalculatedColumn stored IsCalculatedColumnStored;
    property MultiLine: Boolean read FMultiLine write SetMultiLine
      default False;
    property Visible: Boolean read FVisible write SetVisible default True;
    property Format;
    property Kind;
    property OnGetText: TcxTreeListSummaryItemGetTextEvent read FOnGetText
      write FOnGetText;
  end;
  ...
  published
    property FooterSummaryItems: TcxTreeListSummaryItems
      read FFooterSummaryItems write SetFooterSummaryItems;
    property GroupFooterSummaryItems: TcxTreeListSummaryItems
      read FGroupFooterSummaryItems write SetGroupFooterSummaryItems;
  end;
  ...
  public
    constructor Create(ASummary: TcxTreeListColumnSummary;
      AItemClass: TcxTreeListSummaryItemClass); virtual;
    function Add: TcxTreeListSummaryItem;
    function GetItemByKind(AKind: TcxSummaryKind): TcxTreeListSummaryItem;

    property Column: TcxTreeListColumn read GetColumn;
    property Items[Index: Integer]: TcxTreeListSummaryItem read GetItem
      write SetItem; default;
  ...
  TcxTreeListColumn = class(TcxCustomInplaceEditContainer, IcxDragSizing,
    IcxStoredObject, IcxDataControllerSpreadSheetExpressionItem)
  private
  ...
  protected
    ...
    function GetSummaryBestFitSize(ANode: TcxTreeListNode;
    ...
    function GetSizingBoundsRect(ADirection: TcxDragSizingDirection)
      : TRect; virtual;
    function GetSizingIncrement(ADirection: TcxDragSizingDirection)
      : Integer; virtual;
    function IsDynamicUpdate: Boolean; virtual;
    procedure SetSizeDelta(ADirection: TcxDragSizingDirection;
    ...
    function IcxDataControllerSpreadSheetExpressionItem.CanUseInExpression =
      IsVisibleForExpressionEditor;
    function IcxDataControllerSpreadSheetExpressionItem.GetReferenceName =
      GetReferenceNameForExpression;
    function GetReferenceNameForExpression: string;
    // column methods
    procedure ChangeSortOrder(ASortOrder: TcxDataSortOrder;
      AShift: TShiftState);
    procedure ConvertOldFooterSummaries;
    procedure DoGetDisplayText(ARecordIndex: TdxNativeInt;
      var AText: string); override;
    function DoGetNodeDisplayText(ANode: TcxTreeListNode; const AValue: Variant)
      : Variant; virtual;
    function DoOnGetDisplayText(ANode: TcxTreeListNode; AsText: Boolean = False;
      AValueOnly: Boolean = False): Variant; virtual;
    procedure DoValidateDrawValue(ANode: TcxTreeListNode; const AValue: Variant;
      AData: TcxEditValidateInfo); virtual;
    ...
    procedure ValidateDrawValue(const AValue: Variant;
      AEditViewInfo: TcxEditCellViewInfo); override;
  ...
  public
    ...
    property Value;
    property ValueCount;
    property Values[ANode: TcxTreeListNode]: Variant read GetValueByNode
      write SetValueByNode;
  published
    property SummaryFooter: TPersistent read FSummaryFooter
      write SetSummaryFooter stored False;
    property Visible: Boolean read FVisible write SetVisible default True;
    ...
    property DataBinding: TcxItemDataBinding read GetDataBinding
      write SetDataBinding;
    property MinWidth: Integer read FMinWidth write SetMinWidth
      default cxTreeListDefMinWidth;
    property Options: TcxTreeListColumnOptions read GetOptions write SetOptions;
    property Width: Integer read GetWidth write SetWidth
      stored GetIsWidthStored;
    property Position: TcxTreeListColumnPosition read FPosition
      write SetPosition;
    property SortOrder: TcxDataSortOrder read FSortOrder write SetSortOrder
      default soNone;
    ...
    property OnValidateDrawValue: TcxTreeListValidateDrawValueEvent
      read FOnValidateDrawValue write FOnValidateDrawValue;
  ...
  protected
    function CanCustomize: Boolean; override;
    function CanDrop: Boolean; override;
    procedure CheckDropColumnIndex(ARow: TcxTreeListBandRow;
  ...
  private
    ...
    function GetCellIndex(AColumn: TcxTreeListColumn;
      ARow: TcxTreeListBandRow): Integer;
    function GetController: TcxTreeListController;
    function GetFocusedNode: TcxTreeListNode;
    function GetTreeList: TcxCustomTreeList;
    function GoToNextCellInBand(AForward: Boolean; AColumn: TcxTreeListColumn;
      var ACellIndex: Integer): Boolean;
    function GoToNextNodeCell(AForward: Boolean; ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn; var ACellIndex: Integer): Boolean;
  protected
    procedure AppendRecord(var ARowIndex, ACellIndex: Integer);
    procedure CalcNextRow(AForward: Boolean;
      var ARowIndex, ACellIndex: Integer); override;
    function GetCount(ARecordIndex: Integer): Integer; override;
    function GetCellContainer(ARowIndex, ACellIndex: Integer)
      : TcxCustomInplaceEditContainer; override;
    ...
    procedure SetFocusCell(ARowIndex, ACellIndex: Integer;
      AShift: TShiftState); override;
  public
    function FocusNextCell(AForward, ANextRow: Boolean; AShift: TShiftState = []
      ): Boolean; override;
  ...
  public
    ...
    property CustomizationControl: TdxQuickCustomizationCustomControl
  ...
  TdxCustomTreeListQuickCustomizationControl = class
  ...
  TcxNodeDragInfoState = (ndiNone, ndiInsertBefore, ndiInsertAfter,
    ndiAddChild);
  ...
  protected
    ...
    HScrollPos: Integer;
    VScrollPos: Integer;
    ScrollControllers: array [TcxBorder]
      of TcxAutoScrollingEditingControlObject;
    ...
    procedure SetFocusedNodeItem(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn); virtual;
    ...
    function CreateAutoScrollObject(Kind: TScrollBarKind; const ARect: TRect;
      ACode: TScrollCode): TcxAutoScrollingEditingControlObject;
    procedure CheckButtonTimer; virtual;
    procedure DragDrop(Source: TObject; X, Y: Integer); override;
    procedure DragEnter; virtual;
    procedure DragLeave; virtual;
    procedure DragMove(Source: TObject; const P: TPoint;
      var Accepted: Boolean); virtual;
    procedure DragOver(Source: TObject; X, Y: Integer; State: TDragState;
      var Accept: Boolean); override;
    procedure DropTo(ANode: TcxTreeListNode;
      AMode: TcxTreeListNodeAttachMode); virtual;
    ...
    function IsKeyForController(AKey: Word; AShift: TShiftState)
      : Boolean; override;
    function IsNodeKeyHandle(ANode: TcxTreeListNode; var AKey: Word;
    ...
    function GetFocusedCellViewInfo(AEditContainer
      : TcxCustomInplaceEditContainer): TcxEditCellViewInfo; override;
    ...
    function GetNextNodeForIncSearch(ANode: TcxTreeListNode;
      AGoForward: Boolean): TcxTreeListNode;
    procedure CheckLocate(AFound: Boolean);
    procedure SearchLocate(AItem: TcxCustomInplaceEditContainer;
      const AValue: string); override;
    procedure SearchLocateNext(AItem: TcxCustomInplaceEditContainer;
      AForward: Boolean); override;
    ...
    property DataController: TcxTreeListDataController read GetDataController;
    property DragCursorWasChanged: Boolean read FDragCursorWasChanged
      write FDragCursorWasChanged;
    property EditingController: TcxTreeListEditingController
      read GetEditingController;
    property IsNavigationKeyAutoRepeat: Boolean read FIsNavigationKeyAutoRepeat;
    property PostMakeVisible: Boolean read FPostMakeVisible
      write FPostMakeVisible;
    property SearchNotification: Boolean read FSearchNotification
      write FSearchNotification;
    property SelectionLockCount: Integer read FSelectionLockCount
      write FSelectionLockCount;
  ...
  public
    ...
    function DoHeaderMouseDown(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer): Boolean; virtual;
    function DoNodeMouseDown(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer): Boolean; virtual;
    ...
    function FindNearestFocusableColumn(AVisibleIndex: Integer)
      : TcxTreeListColumn;
    ...
    property BandsCustomizationPopup: TcxTreeListBandsCustomizationPopup
      read GetBandsCustomizationPopup;
    property ClickedObject: TObject read FClickedObject write FClickedObject;
    property ColumnsCustomizationPopup: TcxTreeListColumnsCustomizationPopup
      read GetColumnsCustomizationPopup;
    property DragDropInProcess: Boolean read FDragDropInProcess
      write FDragDropInProcess;
    property FilterPopup;
    property FocusedNode: TcxTreeListNode read FFocusedNode
      write SetFocusedNode;
    property FocusedNodeIndex: Integer read GetFocusedNodeIndex
      write FFocusedNodeIndex;
    property ForceSelectionNode: TcxTreeListNode read FForceSelectionNode
      write FForceSelectionNode;
    property ForcingWidthBand: TcxTreeListBand read FForcingWidthBand
      write FForcingWidthBand;
    property ForcingWidthColumn: TcxTreeListColumn read FForcingWidthColumn
      write SetForcingWidthColumn;
    property HitTest: TcxTreeListHitTest read GetHitTest;
    property HotTrackColumn: TcxTreeListColumn read FHotTrackColumn
      write FHotTrackColumn;
    property HotTrackNode: TcxTreeListNode read FHotTrackNode
      write FHotTrackNode;
    property HotTrackShift: TShiftState read FHotTrackShift
      write FHotTrackShift;
    property IncSearchNode: TcxTreeListNode read FIncSearchNode
      write SetIncSearchNode;
    ...
    property OptionsSelection: TcxTreeListOptionsSelection
      read GetOptionsSelection;
    property PressedHeader: TcxTreeListCustomHeaderCellViewInfo
      read FPressedHeader write SetPressedHeader;
  ...
  private
    ...
    function GetViewInfo: TcxTreeListViewInfo;
  protected
    function AllowDesignMouseEvents(X, Y: Integer; AShift: TShiftState)
      : Boolean; override;
  ...
  public
    ...
    property HitAtBandContainer: Boolean index tlhc_HitAtBandContainer
      read GetState;
    property HitAtBandCustomizing: Boolean index tlhc_HitAtBandCustomizing
      read GetState;
    ...
    property HitAtCheckButton: Boolean index tlhc_HitAtCheckButton
      read GetState;
    property HitAtColumn: Boolean index tlhc_HitAtColumn read GetState;
    property HitAtColumnCustomizing: Boolean index tlhc_HitAtColumnCustomizing
      read GetState;
    property HitAtColumnHeader: Boolean index tlhc_HitAtColumnHeader
      read GetState;
    property HitAtColumnHeaderFilterButton: Boolean
      index tlhc_HitAtColumnHeaderFilterButton read GetState;
    ...
    property HitAtGroupFooter: Boolean index tlhc_HitAtGroupFooter
      read GetState;
    property HitAtGroupFooterItem: Boolean index tlhc_HitAtGroupFooterItem
      read GetState;
    ...
    property HitAtNodePreview: Boolean index tlhc_HitAtNodePreview
      read GetState;
  ...
  public
    constructor CreateEx(ALink: TcxTreeListCustomCellViewInfo;
  ...
  protected
    ...
    function GetHitTest(AHitTest: TcxCustomHitTestController): Boolean;
      override;
  ...
  protected
    ...
    procedure Initialize(ANode, AttachNode: TcxTreeListNode;
      AColumn: TcxTreeListColumn); virtual;
    procedure Scroll(const DX, DY: Integer); override;
    // IcxHotTrackElement
  ...
  protected
    ...
    function GetItem: TcxTreeListSummaryItem; override;
    function GetSelected: Boolean; override;
    procedure Initialize(ADataNode, AttachNode: TcxTreeListNode;
      AColumn: TcxTreeListColumn; ASummaryItems: TcxTreeListSummaryItems);
      reintroduce; overload;
    function IsNeedHint(ACanvas: TcxCanvas; const P: TPoint;
      out AText: TCaption; out AIsMultiLine: Boolean; out ATextRect: TRect;
  ...
  protected
    ...
    function CanNeighborFor(ACandidate: TcxTreeListHeaderCellViewInfo)
      : Boolean; virtual;
    procedure CheckClipping(const ADisplayRect, AAvailableRect: TRect);
      override;
    function CheckNeighbors(ACandidate: TcxTreeListHeaderCellViewInfo;
      AShift: Integer): Boolean; virtual;
  ...
  TcxTreeListIndicatorPosition = (tlipBands, tlipColumns, tlipContent,
    tlipFooter);
  ...
  protected
    ...
    function GetPopup: TcxTreeListCustomizationPopup;
    procedure Initialize(ANode: TcxTreeListNode;
      APosition: TcxTreeListIndicatorPosition); virtual;
    ...
    property SizingArea: TcxTreeListNodeSizingArea read FSizingArea
      write FSizingArea;
  public
    destructor Destroy; override;
    class function CustomDrawID: Integer; override;
  ...
  TcxTreeListColumnHeaderCellViewInfo = class(TcxTreeListHeaderCellViewInfo,
    IdxUIElementPopupWindowOwner, IdxFilterPopupWindowOwner)
  private
    function GetBand: TcxTreeListBand;
    function GetColumn: TcxTreeListColumn;
  protected
    { IdxUIElementPopupWindowOwner }
    function IdxUIElementPopupWindowOwner.ClosePopupWhenSetNil =
      CloseFilterPopupOnDestruction;
    ...
    function IdxFilterPopupWindowOwner.GetLinkComponent =
      GetFilterPopupLinkComponent;
  ...
  TcxTreeListBandPart = (tlbpHeader, tlbpContent, tlbpGroupFooter, tlbpFooter,
    tlbpSeparator);
  ...
  protected
    ...
    function GetHitTest(AHitTest: TcxCustomHitTestController): Boolean;
      override;
    procedure Initialize(ABand: TcxTreeListBand;
  ...
  protected
    ...
    procedure CalculateCellEditorBounds(AViewInfo: TcxCustomEditViewInfo;
    ...
    procedure GetFindTextInfo(out AStart: Integer;
      out ALength: Integer); override;
    ...
    property ConditionalFormattingProvider
      : TcxTreeListConditionalFormattingProvider
      read GetConditionalFormattingProvider;
  public
    class function CustomDrawID: Integer; override;
    procedure Invalidate(const R: TRect;
      AEraseBackground: Boolean = False); override;
  ...
  TcxTreeListNodeIndentKind = (nikLevel, nikImage, nikState, nikCheck,
    nikFooter);
  ...
  protected
    ...
    function GetHitTest(AHitTest: TcxCustomHitTestController): Boolean;
      override;
    procedure InitializeLevelIndent(ANode, AAttachNode: TcxTreeListNode;
  ...
  public
    ...
    constructor Create(AViewInfo: TcxTreeListViewInfo; ANode: TcxTreeListNode;
      ACapacity: Integer; AFake: Boolean = False); virtual;
    destructor Destroy; override;
    ...
    procedure Scroll(DX, DY: Integer;
      ALeftFixed, ARightFixed: Boolean); virtual;
  ...
  TcxTreeListNodeScrollAnimationHelper = class
    (TdxCustomScrollAnimationHelper<TcxTreeListNodePixelScrollData>)
  strict private
    FOwner: TcxTreeListViewInfo;
  protected
    procedure AfterAnimationEnded(AIsRaiseEvent: Boolean); override;
    function CalculateAnimatedPosition(var APosition: Integer)
      : TcxTreeListNodePixelScrollData; override;
    function CalculateScrollDelta(AValue: TcxTreeListNodePixelScrollData)
      : Integer; override;
    function GetOwnerPosition: TcxTreeListNodePixelScrollData; override;
    function GetUnassignedTargetPosition
      : TcxTreeListNodePixelScrollData; override;
    function IsEqual(AValue1, AValue2: TcxTreeListNodePixelScrollData)
      : Boolean; override;
    procedure BeforeAnimationStarted; override;
    procedure UpdateOwnerPosition
      (AValue: TcxTreeListNodePixelScrollData); override;
  ...
  protected
    ...
    function AddBackgroundPart(const ABounds: TRect)
      : TcxTreeListBackgroundCellViewInfo;
    function AddBandHeader(ABand: TcxTreeListBand; var ABounds: TRect;
      const AClipRect: TRect): TcxTreeListBandHeaderCellViewInfo;
    function AddBandPart(APart: TcxTreeListBandPart; ABand: TcxTreeListBand;
      ANode, AAttachNode: TcxTreeListNode; ABounds, AClipRect: TRect)
      : TcxTreeListBandCellViewInfo;
    function AddBandSeparator(ANode: TcxTreeListNode; var ABounds: TRect;
      const AClipBounds: TRect): TcxTreeListBandCellViewInfo;
    procedure AddBandsHeaders(AKind: TcxTreeListBandFixedKind;
      const ABounds, AClipBounds: TRect);
    ...
    function AddEditCell(AViewData: TcxTreeListNodeViewData;
    ...
    procedure AddFooter(AAttachNode: TcxTreeListNode; AIndex, AIndent: Integer;
      const ABounds: TRect);
    procedure AddHeaderHitTestAreas(ACell: TcxTreeListHeaderCellViewInfo;
      AHorz, AVert: Boolean);
    function AddImageIndent(ANode: TcxTreeListNode; AImages: TCustomImageList;
      var ABounds: TRect; const AClipRect: TRect;
      ACheckDynamic: Boolean = False): TcxTreeListIndentCellViewInfo;
    function AddIndent(ANode, AAttachNode: TcxTreeListNode; ALevel: Integer;
      AKind: TcxTreeListNodeIndentKind; var ABounds: TRect;
      const AClipRect: TRect): TcxTreeListIndentCellViewInfo;
    function AddIndentCheck(ANode: TcxTreeListNode; var ABounds: TRect;
      const AClipRect: TRect): TcxTreeListIndentCellViewInfo;
    function AddIndicator(ANode: TcxTreeListNode;
      APosition: TcxTreeListIndicatorPosition; var ABounds: TRect)
      : TcxTreeListIndicatorCellViewInfo;
    ...
    function AddNodePreview(AViewData: TcxTreeListNodeViewData)
      : TcxTreeListEditCellViewInfo;
    procedure AddNodeStandardContent(AViewData: TcxTreeListNodeViewData);
    function AddNodeViewData(ANode: TcxTreeListNode; AInsert: Boolean;
      AFake: Boolean = False): TcxTreeListNodeViewData;
    ...
    procedure NodeScrollAnimated(ATopNode: TcxTreeListNode;
      ATopRecordPixelScrollOffset: Integer);
    ...
    procedure CalculateHorzScrollInfo(AAvailableWidth, AContentWidth
      : Integer); virtual;
    function CalculateNodeAutoHeight(AViewData
      : TcxTreeListNodeViewData): Integer;
    procedure CalculateNodesOrigin; virtual;
    procedure CalculateNodesViewData; virtual;
    procedure CalculateNodesViewDataBackward(ANode: TcxTreeListNode;
      ACheckHeight: Boolean; var AvailableHeight: Integer); virtual;
    procedure CalculateNodesViewDataForward(ANode: TcxTreeListNode;
      var AvailableHeight: Integer); virtual;
    procedure CalculatePixelScrollInfo(var ANode: TcxTreeListNode;
      var APixelScrollNodeOffset: Integer; AMaxTopNode: TcxTreeListNode;
      AMaxPixelScrollTopNodeOffset: Integer; ADelta: Integer;
      out AOverPan: Boolean);
    procedure CellsChanged;
    procedure CheckBiDiMode; virtual;
    function CheckScrollPosition(AScrollCode: TScrollCode;
      APos, APage, AInc, AMax: Integer; var AScrollPos: Integer)
      : Boolean; virtual;
    procedure Clear;
    procedure DeleteNodeViewData(AIndex: Integer);
    procedure DoCalculate; override;
    procedure DoCalculateBackgroundParts(AHScrollPos: Integer);
    procedure DoSetTopNodeWithOffset(ANode: TcxTreeListNode;
      APixelScrollNodeOffset: Integer; ARaiseEvent: Boolean);
    function GetContentBounds: TRect; virtual;
    function GetPixelScrollOffset(AFirstNode: TcxTreeListNode;
      AFirstNodeOffset: Integer; ALastNode: TcxTreeListNode;
      ALastNodeOffset: Integer): Integer;
    function GetRealContentBounds: TRect;
    function GetFilterBoxViewInfoClass
      : TcxControlFilterBoxViewInfoClass; override;
    function GetFindPanelViewInfoClass
      : TcxControlFindPanelViewInfoClass; override;
    ...
    function GetLastNodeByTopNode(ATopNode: TcxTreeListNode;
      ATopNodePixelScrollOffset: Integer): TcxTreeListNode;
    ...
    function GetNavigatorSiteViewInfoClass
      : TcxCustomNavigatorSiteViewInfoClass; override;
    ...
    procedure GetPixelScrollTopNodeAndOffsetByBottomNode
      (ABottomNode: TcxTreeListNode; out ATopNode: TcxTreeListNode;
      out ATopNodePixelScrollOffset: Integer);
    ...
    procedure InitializeRows(const ANewHeight: Integer;
      var AViewData: TcxTreeListNodeViewData);
    ...
    procedure SetTopNodeWithOffset(ANode: TcxTreeListNode;
      APixelScrollNodeOffset: Integer; AAnimated: Boolean);
    ...
    procedure ValidateScrollPos(var APosition: Integer;
      const APage, AMax: Integer);

    property CalculateInProcess: Boolean read GetCalculateInProcess
      write SetCalculateInProcess;
    property CanOffsetContent: Boolean read FCanOffsetContent
      write FCanOffsetContent;
    property Controller: TcxTreeListController read GetController;
    property EditingController: TcxTreeListEditingController
      read GetEditingController;
    property FakeCell: TcxTreeListEditCellViewInfo read FFakeCell;
    property FilterBoxViewInfo: TcxTreeListFilterBoxViewInfo
      read GetFilterBoxViewInfo;
    property HitTestCells: TcxObjectList read FHitTestCells;
    property IsPrinting: Boolean read GetIsPrinting;
    property LockCount: Integer read FLockCount write FLockCount;
    property RowOffset[Index: Integer]: Integer read GetRowOffset
      write SetRowOffset;
  public
    constructor Create(AOwner: TcxEditingControl); override;
    destructor Destroy; override;
    function GetEditCell(ANode: TcxTreeListNode; AColumn: TcxTreeListColumn)
      : TcxTreeListEditCellViewInfo;
    procedure SetDirty;
    ...
    property ContentParts[AKind: TcxTreeListBandFixedKind]: TRect
      read GetContentPart;
    ...
    property HorzBackgroundCell: TcxTreeListBackgroundCellViewInfo
      read FHorzBackgroundCell; // for internal use
    ...
    property NodeViewData[Index: Integer]: TcxTreeListNodeViewData
      read GetNodeViewData; // for internal use
    property OptionsView: TcxTreeListOptionsView read GetOptionsView;
  ...
  protected
    ...
    procedure CustomDrawCell(ACanvas: TcxCanvas; ACell: TcxCustomViewInfoItem;
      var ADone: Boolean); virtual;
  public
    property Cells: TcxCustomControlCells read GetCells;
    property CustomDrawSupported: Boolean read FCustomDrawSupported
      write FCustomDrawSupported;
    property TreeList: TcxCustomTreeList read GetTreeList;
    property ViewInfo: TcxTreeListViewInfo read GetViewInfo;
  end;
  ...
  public
    ...
    procedure RestoreDefaults; virtual;
  published
    property Customizing: Boolean read FCustomizing write FCustomizing
      default True;
    property Hidden: Boolean read FHidden write FHidden default False;
    property Moving: Boolean read FMoving write FMoving default True;
    property OnlyOwnColumns: Boolean read FOnlyOwnColumns write FOnlyOwnColumns
      default False;
    property Sizing: Boolean read FSizing write SetSizing default True;
    property VertSizing: Boolean read FVertSizing write SetVertSizing
      default True;
  ...
  protected
    ...
    procedure DoGetContentParams(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn; var AParams: TcxViewParams); virtual;
    procedure GetDefaultViewParams(Index: Integer; AData: TObject;
      out AParams: TcxViewParams); override;
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
    function GetContentParams(ANode: TcxTreeListNode;
  ...
  published
    ...
    property HeaderBackground: TcxStyle index tlbs_HeaderBackground
  ...
  published
    ...
    property ColIndex: Integer read GetColIndex write SetColIndex
      stored IsColIndexStored;
  end;

  { TcxTreeListBand }

  TcxTreeListBand = class(TcxInterfacedCollectionItem, IcxDragSizing,
    IcxStoredObject)
  private
  ...
  protected
    ...
    function CanDropColumnAt(const APoint: TPoint;
      out ARowIndex, AColIndex: Integer): Boolean; virtual;
    procedure ChangeScale(M, D: Integer); virtual;
    ...
    property CalculatedWidth: Integer read GetCalculatedWidth
      write SetCalculatedWidth;
    ...
    property HeaderCell: TcxTreeListBandHeaderCellViewInfo read FHeaderCell
      write FHeaderCell;
    ...
    property ParentBandWidthAssigned: TcxTreeListBand
      read GetParentBandWidthAssigned;
  public
    constructor Create(Collection: TCollection); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  ...
  published
    property Caption: TcxTreeListCaption read FCaption write SetCaption;
    property Expandable: TcxTreeListBandExpandable read FExpandable
      write SetExpandable default tlbeDefault;
    property FixedKind: TcxTreeListBandFixedKind read FFixedKind
      write SetFixedKind default tlbfNone;
    property MinWidth: Integer read FMinWidth write SetMinWidth
      default cxTreeListDefMinWidth;
  ...
  protected
    ...
    function GetFirstVisibleIndex(AFixedKind: TcxTreeListBandFixedKind)
      : Integer;
    function GetFirstVisibleRootIndex(AFixedKind
      : TcxTreeListBandFixedKind): Integer;
    function GetLastVisibleIndex(AFixedKind: TcxTreeListBandFixedKind): Integer;
    function GetLastVisibleRootIndex(AFixedKind
      : TcxTreeListBandFixedKind): Integer;
    function GetVisibleCountByKind(AFixedKind
      : TcxTreeListBandFixedKind): Integer;
    function GetOwner: TPersistent; override;
  ...
  public
    ...
    property Items[AIndex: Integer]: TcxTreeListBand read GetItem
      write SetItem; default;
  ...
  TcxTreeListFilteringColumnPopupOptions = class
  ...
  private
    ...
    procedure SetColumnExcelPopup
      (AValue: TcxTreeListFilteringColumnExcelPopupOptions);
  ...
  protected
    function GetItemExcelPopupClass
      : TcxControlOptionsFilteringItemExcelPopupClass; override;
    function GetItemPopupClass
      : TcxControlOptionsFilteringItemPopupClass; override;
  public
    procedure RunCustomizeDialog(AColumn: TcxTreeListColumn = nil);
      reintroduce; virtual;
  published
    property ColumnExcelPopup: TcxTreeListFilteringColumnExcelPopupOptions
      read GetColumnExcelPopup write SetColumnExcelPopup;
    property ColumnPopup: TcxTreeListFilteringColumnPopupOptions
      read GetColumnPopup write SetColumnPopup;
    property ColumnPopupMode: TdxFilterPopupWindowMode read GetColumnPopupMode
      write SetColumnPopupMode default fpmDefault;
  end;

  { TcxTreeListOptionsView }

  TcxTreeListGroupFootersMode = (tlgfInvisible, tlgfVisibleWhenExpanded,
    tlgfAlwaysVisible);
  ...
  published
    property BandLineHeight: Integer read FBandLineHeight
      write SetBandLineHeight default 0;
    ...
    property CategorizedColumn: TcxTreeListColumn read FCategorizedColumn
      write SetCategorizedColumn;
    property ColumnAutoWidth: Boolean read FColumnAutoWidth
      write SetColumnAutoWidth default False;
    property CheckGroups: Boolean read FCheckGroups write SetCheckGroups
      default False;
    property DropArrowColor: TColor read FDropArrowColor write FDropArrowColor
      default cxTreeListDefDropArrowColor;
    property DropNodeIndicator: Boolean read FDropNodeIndicator
      write FDropNodeIndicator default False;
    property DynamicIndent: Boolean read FDynamicIndent write SetDynamicIndent
      default False;
    property DynamicFocusedStateImages: Boolean read FDynamicFocusedStateImages
      write SetDynamicFocusedStateImages default False;
    ...
    property ExtPaintStyle: Boolean read FExtPaintStyle write SetExtPaintStyle
      default False;
    property FixedSeparatorColor: TColor read FFixedSeparatorColor
      write SetFixedSeparatorColor default clDefault;
    property FixedSeparatorWidth: Integer read FFixedSeparatorWidth
      write SetFixedSeparatorWidth default cxTreeListDefSeparatorWidth;
    ...
    property GridLineColor: TColor read FGridLineColor write SetGridLineColor
      default clDefault;
    property GridLines: TcxTreeListGridLines read FGridLines write SetGridLines
      default tlglNone;
    property GroupFooters: TcxTreeListGroupFootersMode read FGroupFooters
      write SetGroupFooters default tlgfInvisible;
    property HeaderAutoHeight: Boolean read FHeaderAutoHeight
      write SetHeaderAutoHeight default False;
    property HeaderFilterButtonShowMode: TcxFilterButtonShowMode
      read GetHeaderFilterButtonShowMode write SetHeaderFilterButtonShowMode
      default fbmDefault;
    ...
    property IndicatorWidth: Integer read FIndicatorWidth
      write SetIndicatorWidth default cxTreeListDefIndicatorWidth;
    property NavigatorOffset;
    property PaintStyle: TcxTreeListPaintStyle read FPaintStyle
      write SetPaintStyle default tlpsStandard;
    property ShowColumnFilterButtons: TcxShowFilterButtons
      read GetShowColumnFilterButtons write SetShowColumnFilterButtons
      default sfbDefault;
    property ShowRoot: Boolean read FShowRoot write SetShowRoot default True;
    property SimpleCustomizeBox: Boolean read FSimpleCustomizeBox
      write SetSimpleCustomizeBox default False;
    property TreeLineColor: TColor read FTreeLineColor write SetTreeLineColor
      default clDefault;
    property TreeLineStyle: TcxTreeListTreeLineStyle read FTreeLineStyle
      write SetTreeLineStyle default tllsDot;
    property UseImageIndexForSelected: Boolean read FUseImageIndexForSelected
      write SetUseImageIndexForSelected default True;
    property UseNodeColorForIndent: Boolean read FUseNodeColorForIndent
      write SetUseNodeColorForIndent default True;
  ...
  protected
    ...
    property BandsQuickCustomizationMultiColumnMode: Boolean
      read FBandsQuickCustomizationMultiColumnMode
      write FBandsQuickCustomizationMultiColumnMode default True;
    property ColumnsQuickCustomizationMultiColumnMode: Boolean
      read FColumnsQuickCustomizationMultiColumnMode
      write FColumnsQuickCustomizationMultiColumnMode default True;
  public
    constructor Create(AOwner: TPersistent); override;
    ...
    property TreeList: TcxCustomTreeList read GetTreeList;
  published
    property BandCustomizing: Boolean read FBandCustomizing
      write SetBandCustomizing default True;
    property BandHorzSizing: Boolean read FBandHorzSizing
      write SetBandHorzSizing default True;
    property BandHiding: Boolean read FBandHiding write SetBandHiding
      default False;
    property BandMoving: Boolean read FBandMoving write SetBandMoving
      default True;
    ...
    property BandsQuickCustomizationMaxDropDownCount: Integer
      read FBandsQuickCustomizationMaxDropDownCount
      write FBandsQuickCustomizationMaxDropDownCount default 0;
    property BandsQuickCustomizationShowCommands: Boolean
      read FBandsQuickCustomizationShowCommands
      write FBandsQuickCustomizationShowCommands default True;
    property BandsQuickCustomizationSorted: Boolean
      read FBandsQuickCustomizationSorted write FBandsQuickCustomizationSorted
      default False;
    property BandVertSizing: Boolean read FBandVertSizing
      write SetBandVertSizing default True;
    property ColumnCustomizing: Boolean read FColumnCustomizing
      write SetColumnCustomizing default True;
    property ColumnFiltering: TdxDefaultBoolean read GetColumnFiltering
      write SetColumnFiltering default bDefault;
    property ColumnHiding: Boolean read FColumnHiding write SetColumnHiding
      default False;
    property ColumnHorzSizing: Boolean read FColumnHorzSizing
      write SetColumnHorzSizing default True;
    property ColumnMoving: Boolean read FColumnMoving write SetColumnMoving
      default True;
    property ColumnsQuickCustomization: Boolean read FColumnsQuickCustomization
      write SetColumnsQuickCustomization default False;
    property ColumnsQuickCustomizationMaxDropDownCount: Integer
      read FColumnsQuickCustomizationMaxDropDownCount
      write FColumnsQuickCustomizationMaxDropDownCount default 0;
    property ColumnsQuickCustomizationShowCommands: Boolean
      read FColumnsQuickCustomizationShowCommands
      write FColumnsQuickCustomizationShowCommands default True;
    property ColumnsQuickCustomizationSorted: Boolean
      read FColumnsQuickCustomizationSorted
      write FColumnsQuickCustomizationSorted default False;
    property ColumnVertSizing: Boolean read FColumnVertSizing
      write SetColumnVertSizing default True;
    property DynamicSizing: Boolean read FDynamicSizing write SetDynamicSizing
      default False;
    property NestedBands: Boolean read FNestedBands write SetNestedBands
      default True;
    property NodeSizing: Boolean read FNodeSizing write SetNodeSizing
      default False;
    property RowSizing: Boolean read FRowSizing write SetRowSizing
      default False;
    property StackedColumns: Boolean read FStackedColumns
      write SetStackedColumns default True;
  end;
  ...
  public
    ...
    property TreeList: TcxCustomTreeList read GetTreeList;
  published
    property CellSelect: Boolean read FCellSelect write FCellSelect
      default True;
    ...
    property HideSelection: Boolean read FHideSelection write FHideSelection
      default False;
    property InvertSelect: Boolean read FInvertSelect write FInvertSelect
      default True;
  ...
  public
    ...
    property TreeList: TcxCustomTreeList read GetTreeList;
  published
    property AutoDragCopy: Boolean read FAutoDragCopy write FAutoDragCopy
      default False;
    property BestFitMaxRecordCount: Integer read FBestFitMaxRecordCount
      write FBestFitMaxRecordCount default -1;
    ...
    property CopyCaptionsToClipboard: Boolean read FCopyCaptionsToClipboard
      write FCopyCaptionsToClipboard default True;
    property DragCollapse: Boolean read FDragCollapse write FDragCollapse
      default True;
    property DragDropText;
    property DragExpand: Boolean read FDragExpand write FDragExpand
      default True;
    property DragFocusing: Boolean read FDragFocusing write FDragFocusing
      default False;
    property EditAutoHeight: TcxInplaceEditAutoHeight read FEditAutoHeight
      write SetEditAutoHeight default eahNone;
    property ExpandOnDblClick: Boolean read FExpandOnDblClick
      write FExpandOnDblClick default True;
    property ExpandOnIncSearch: Boolean read FExpandOnIncSearch
      write FExpandOnIncSearch default False;
    property FocusCellOnCycle;
    property FocusFirstCellOnNewRecord;
    property FooterHints: Boolean read FFooterHints write SetFooterHints
      default False;
    property HeaderHints: Boolean read FHeaderHints write SetHeaderHints
      default False;
    property HotTrack: Boolean read FHotTrack write FHotTrack default False;
    property IncSearch;
    property IncSearchItem: TcxTreeListColumn read GetIncSearchItem
      write SetIncSearchItem default nil;
    ...
    property ShowHourGlass: Boolean read FShowHourGlass write FShowHourGlass
      default True;
    property Sorting: Boolean read FSorting write FSorting default True;
    property WaitForExpandNodeTime: Integer read FWaitExpandingTime
      write FWaitExpandingTime default 500;
  end;
  ...
  published
    ...
    property ImmediatePost: Boolean read FImmediatePost write SetImmediatePost
      default False;
    property Inserting: Boolean read FInserting write SetInserting
      default False;
    property MultiThreadedSorting: TdxDefaultBoolean read FMultiThreadedSorting
      write FMultiThreadedSorting default bDefault;
    property SummaryNullIgnore: Boolean read FSummaryNullIgnore
      write SetSummaryNullIgnore default False;
  end;
  ...
  published
    ...
    property MaxLineCount: Integer read FMaxLineCount write SetMaxLineCount
      default 3;
    property Place: TcxTreeListPreviewPlace read FPlace write SetPlace
      default tlppBottom;
  ...
  TcxTreeListGetBandContentStyleEvent = procedure(Sender: TcxCustomTreeList;
    ABand: TcxTreeListBand; ANode: TcxTreeListNode; var AStyle: TcxStyle)
    of object;
  ...
  TcxTreeListGetColumnFooterStyleEvent = procedure(Sender: TcxCustomTreeList;
    ...
    AFooterItem: TcxTreeListSummaryItem; var AStyle: TcxStyle) of object;
  ...
  protected
    ...
    procedure DoGetBandBackgroundParams(ABand: TcxTreeListBand;
      var AParams: TcxViewParams); virtual;
    procedure DoGetBandContentParams(ABand: TcxTreeListBand;
      ANode: TcxTreeListNode; var AParams: TcxViewParams); virtual;
    procedure DoGetBandFooterParams(ABand: TcxTreeListBand;
      ANode: TcxTreeListNode; var AParams: TcxViewParams); virtual;
    procedure DoGetBandHeaderParams(ABand: TcxTreeListBand;
      var AParams: TcxViewParams); virtual;
    procedure DoGetColumnFooterParams(AColumn: TcxTreeListColumn;
      ANode: TcxTreeListNode; ASummaryItem: TcxTreeListSummaryItem;
      var AParams: TcxViewParams); virtual;
    procedure DoGetColumnHeaderParams(AColumn: TcxTreeListColumn;
      var AParams: TcxViewParams); virtual;
    procedure DoGetContentParams(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn; var AParams: TcxViewParams); virtual;
    procedure DoGetNodeContentParams(ANode: TcxTreeListNode; AStyle: TcxStyle;
      var AParams: TcxViewParams); virtual;
    procedure DoGetPreviewParams(ANode: TcxTreeListNode;
      var AParams: TcxViewParams); virtual;
    procedure GetDefaultViewParams(Index: Integer; AData: TObject;
      out AParams: TcxViewParams); override;
    function GetCellSelectionParams(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn): TcxViewParams;
    procedure PrepareConditionalFormattingParams(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn; var AParams: TcxViewParams);
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
    function GetBandBackgroundParams(ABand: TcxTreeListBand): TcxViewParams;
    function GetBandContentParams(ABand: TcxTreeListBand;
      ANode: TcxTreeListNode): TcxViewParams;
    function GetBandFooterParams(ABand: TcxTreeListBand; ANode: TcxTreeListNode)
      : TcxViewParams;
    function GetBandHeaderParams(ABand: TcxTreeListBand): TcxViewParams;
    function GetColumnFooterParams(AColumn: TcxTreeListColumn;
      ANode: TcxTreeListNode; ASummaryItem: TcxTreeListSummaryItem = nil)
      : TcxViewParams;
    ...
    function GetIncSearchParams: TcxViewParams;
    function GetIndentParams(ANode: TcxTreeListNode; AIndent: Integer)
      : TcxViewParams;
    ...
    property TreeList: TcxCustomTreeList read GetTreeList;
  published
    property BandBackground: TcxStyle index tlsv_BandBackground read GetValue
      write SetValue;
    property BandContent: TcxStyle index tlsv_BandContent read GetValue
      write SetValue;
    property BandHeader: TcxStyle index tlsv_BandHeader read GetValue
      write SetValue;
    property ColumnFooter: TcxStyle index tlsv_ColumnFooter read GetValue
      write SetValue;
    property ColumnHeader: TcxStyle index tlsv_ColumnHeader read GetValue
      write SetValue;
    property ContentEven: TcxStyle index tlsv_ContentEven read GetValue
      write SetValue;
    property ContentOdd: TcxStyle index tlsv_ContentOdd read GetValue
      write SetValue;
    property Footer: TcxStyle index tlsv_Footer read GetValue write SetValue;
    property HotTrack: TcxStyle index tlsv_HotTrack read GetValue
      write SetValue;
    property IncSearch: TcxStyle index tlsv_IncSearch read GetValue
      write SetValue;
    property Indicator: TcxStyle index tlsv_Indicator read GetValue
      write SetValue;
    property Navigator;
  ...
  protected
    ...
    procedure DoDrawItemEx(Control: TWinControl; Index: Integer; Rect: TRect;
    ...
    procedure MakeItemVisible(AHeader: TcxTreeListHeaderCellViewInfo);
      virtual; abstract;
    procedure MeasureItem(AControl: TcxListBox; Index: Integer;
    ...
    property Headers[Index: Integer]: TcxTreeListHeaderCellViewInfo
      read GetHeader;
    property Painter: TcxCustomLookAndFeelPainter read GetPainter;
  public
    constructor CreateEx(AOwner: TComponent; ATreeList: TcxCustomTreeList;
      AParent: TWinControl);
    destructor Destroy; override;
  ...
  public
    ...
    property DragDropListBox: TcxTreeListItemsCustomizeListBox
      read FDragDropListBox;
    property TreeList: TcxCustomTreeList read GetTreeList;
  end;
  ...
  TcxTreeListCustomizing = class(TcxOwnedInterfacedPersistent,
    IcxTreeListDesigner)
  private
  ...
  protected
    ...
    procedure FinishCustomizing(const APosition: TPoint;
      const ASize: TSize); virtual;
    ...
    property HeaderListBox: TcxTreeListItemsCustomizeListBox
      read FHeaderListBox;
  ...
  public
    ...
    property IsPositionAssigned: Boolean read FIsPositionAssigned
      write FIsPositionAssigned;
  ...
  protected
    ...
    property DataController: TcxTreeListDataController read GetDataController;
    property NodeVisibleIndexes[AMatchIndex: Integer]: Integer
      read GetMatchNodeVisibleIndex;
    property TreeList: TcxCustomTreeList read GetTreeList;
  public
    function GetTextStartPositionByNode(ANode: TcxTreeListNode;
      AItemIndex: Integer; out AHighlightedText: string): Integer; virtual;

    property MatchNodes[AMatchIndex: Integer]: TcxTreeListNode
      read GetMatchNode;
  end;

  { TcxTreeListDataController }

  TcxTreeListForEachNodeProc = procedure(ANode: TcxTreeListNode; Data: Pointer)
    of object;
  ...
  protected
    ...
    procedure ClearNodeValue(ANode: TcxTreeListNode;
      AField: TcxCustomDataField); virtual;
    procedure ClearValue(ARecordIndex: Integer;
      AField: TcxCustomDataField); override;
    function Compare(AValueDef: TcxValueDef;
      AHandle1, AHandle2: Pointer): Integer;
    ...
    function GetFilteredIndexByRecordIndex(ARecordIndex: Integer)
      : Integer; override;
    ...
    function GetFilterDisplayText(ARecordIndex: Integer; AItemIndex: Integer)
      : string; override;
    function GetFilterRecordIndexByFilteringRecordIndex(AUseFiltered: Boolean;
      AIndex: Integer): Integer; override;
    function GetFilterRecordIndexByNode(ANode: TcxTreeListNode)
      : Integer; virtual;
    function GetInternalValue(ARecordIndex: Integer; AField: TcxCustomDataField)
      : Variant; override;
    function GetNodeByFilterRecordIndex(ARecordIndex: Integer)
      : TcxTreeListNode; virtual;
    function GetNodeByRecordIndex(ARecordIndex: Integer)
      : TcxTreeListNode; virtual;
    function GetNodeErrorCode(ANode: TcxTreeListNode; AIndex: Integer)
      : Integer; virtual;
    function GetNodeExpressionDisplayText(ANode: TcxTreeListNode;
      AIndex: Integer): string; virtual;
    function GetNodeExpressionValue(ANode: TcxTreeListNode; AIndex: Integer)
      : Variant; virtual;
    function GetNodeStoredDisplayText(ANode: TcxTreeListNode; AIndex: Integer)
      : string; virtual;
    function GetNodeStoredValue(ANode: TcxTreeListNode; AIndex: Integer)
      : Variant; virtual;
    function GetRecordIndexByNode(ANode: TcxTreeListNode): Integer; virtual;
    function GetStoredDisplayText(ARecordIndex: Integer;
      AField: TcxCustomDataField): string; override;
    function GetStoredErrorCode(ARecordIndex: Integer;
      AField: TcxCustomDataField): Integer; override;
    function GetStoredValue(ARecordIndex: Integer; AField: TcxCustomDataField)
      : Variant; override;
    ...
    function IsStoredValueEmpty(ARecordIndex: Integer;
      AField: TcxCustomDataField): Boolean; override;
    procedure ResetNodesFiltering; virtual;
    procedure SetNodeDisplayText(ANode: TcxTreeListNode; AIndex: Integer;
      const Value: string); virtual;
    procedure SetNodeStoredValue(ANode: TcxTreeListNode; AIndex: Integer;
      const Value: Variant); virtual;
    procedure SetStoredErrorCode(ARecordIndex: Integer;
      AField: TcxCustomDataField; ACode: Integer); override;
    procedure SetStoredValue(ARecordIndex: Integer; AField: TcxCustomDataField;
      const Value: Variant); override;
    // Bookmark
    function InternalCheckBookmark(ADeletedRecordIndex: Integer)
      : Boolean; override;
    ...
    property DataChangedBusy: Boolean read FDataChangedBusy
      write FDataChangedBusy;
    property EditingNode: TcxTreeListNode read FEditingNode
      write SetEditingNode;
    property Filtered: Boolean read FFiltered;
    property IsValueChanged: Boolean read FIsValueChanged
      write SetIsValueChanged;
    property TreeList: TcxCustomTreeList read GetTreeList;
  ...
  public
    ...
    function CompareNodesByColumns(ANode1, ANode2: TcxTreeListNode;
      AColumns: TList): Integer; virtual;
    ...
    function GetNodeDisplayText(ANode: TcxTreeListNode; AIndex: Integer)
      : Variant; virtual;
    function GetNodeValue(ANode: TcxTreeListNode; AIndex: Integer)
      : Variant; virtual;
    function GetValue(ARecordIndex, AItemIndex: Integer): Variant; override;
    function GetValueForCompare(ANode: TcxTreeListNode;
    ...
    procedure SetNodeValue(ANode: TcxTreeListNode; AIndex: Integer;
      const AValue: Variant); virtual;
    procedure SetValue(ARecordIndex, AItemIndex: Integer;
      const Value: Variant); override;
  ...
  protected
    function CreateMenuItem(AOwner: TComponent; const ACaption: string;
      ACommand: TcxTag; AEnabled: Boolean = True;
      AItemType: TcxTreeListBuiltInMenuItemType = tlmitDefault;
      AChecked: Boolean = False; AImageIndex: Integer = -1;
      AWithSeparator: Boolean = False; AInternal: Boolean = True): TComponent;
      virtual; abstract;
  ...
  protected
    function CreateMenuItem(AOwner: TComponent; const ACaption: string;
      ...
      AItemType: TcxTreeListBuiltInMenuItemType = tlmitDefault;
      AChecked: Boolean = False; AImageIndex: Integer = -1;
      AWithSeparator: Boolean = False; AInternal: Boolean = True)
      : TComponent; override;
  ...
  protected
    function CreateInternalMenuItem(AOwner: TComponent; const ACaption: string;
      ACommand: TcxTag; AEnabled: Boolean = True;
      AItemType: TcxTreeListBuiltInMenuItemType = tlmitDefault;
      AChecked: Boolean = False; AImageIndex: Integer = -1;
      AWithSeparator: Boolean = False): TComponent;
  ...
  published
    property PopupMenu;
    property UseBuiltInMenu: Boolean read FUseBuiltInMenu
      write SetUseBuiltInMenu default False;
    property OnClick: TcxTreeListPopupMenuClickEvent read FOnClick
      write FOnClick;
    property OnPopup: TcxTreeListPopupMenuPopupEvent read FOnPopup
      write FOnPopup;
  end;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property Items: TcxTreeListFooterPopupMenuItems read FItems write FItems
  ...
  TcxTreeListColumnHeaderPopupMenuItem = (tlchmiSortAscending,
    tlchmiSortDescending, tlchmiClearSorting, tlchmiFooter, tlchmiGroupFooters,
    tlchmiRemoveThisColumn, tlchmiFieldChooser, tlchmiHorzAlignment,
    tlchmiVertAlignment, tlchmiBestFit, tlchmiBestFitAllColumns);
  TcxTreeListColumnHeaderPopupMenuItems = set of
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property Items: TcxTreeListColumnHeaderPopupMenuItems read FItems
      write FItems stored IsItemsStored;
  end;
  ...
  published
    property ColumnHeaderMenu: TcxTreeListColumnHeaderPopupMenu
      read FColumnHeaderMenu write SetColumnHeaderMenu;
    property FooterMenu: TcxTreeListFooterPopupMenu read FFooterMenu
      write SetFooterMenu;
    property GroupFooterMenu: TcxTreeListGroupFooterPopupMenu
      read FGroupFooterMenu write SetGroupFooterMenu;
  end;
  ...
  private
    ...
    function GetFooterSummaryValue(ASummaryItem
      : TcxTreeListSummaryItem): Variant;
    function GetGroupFooterSummaryCount: Integer;
    function GetGroupFooterSummaryItem(AIndex: Integer): TcxTreeListSummaryItem;
    function GetGroupFooterSummaryText(ASummaryItem: TcxTreeListSummaryItem;
      ANode: TcxTreeListNode): string;
    function GetGroupFooterSummaryValue(ASummaryItem: TcxTreeListSummaryItem;
      ANode: TcxTreeListNode): Variant;
    function GetRoot: TcxTreeListNode;
    procedure SetFooterSummaryValue(ASummaryItem: TcxTreeListSummaryItem;
      const Value: Variant);
    procedure SetGroupFooterSummaryValue(ASummaryItem: TcxTreeListSummaryItem;
      ANode: TcxTreeListNode; const Value: Variant);
  protected
    procedure AddSummaryItems(AAbsoluteSummaryItems: TList;
      ASummaryItems: TcxTreeListSummaryItems; AIsVisible: Boolean;
      var AVisibleCount, AVisibleRowCount: Integer);
    procedure Calculate;
    procedure CalculateNode(ADestNode, AValueNode: TcxTreeListNode;
      AItems: TList);
    procedure CheckChanges; virtual;
    function DoProcessSummaryValue(ANode: TcxTreeListNode;
      ASummaryItem: TcxTreeListSummaryItem; var ASummaryValue: Variant;
      var ASummaryCount: Integer; var AValue: Variant): Boolean;
    ...
    procedure ProcessSummaryValue(ANode: TcxTreeListNode;
      ASummaryItem: TcxTreeListSummaryItem; var ASummaryValue: Variant;
      var ASummaryCount: Integer);
    procedure ProcessSummaryValues(ADestNode, AValueNode: TcxTreeListNode;
      AItems: TList);
  ...
  public
    ...
    property GroupFooterSummaryRowCount: Integer
      read FGroupFooterSummaryRowCount;
    property GroupFooterSummaryVisibleCount: Integer
      read FGroupFooterSummaryVisibleCount;
    property GroupFooterSummaryItems[Index: Integer]: TcxTreeListSummaryItem
      read GetGroupFooterSummaryItem;
    property GroupFooterSummaryTexts[ASummaryItem: TcxTreeListSummaryItem;
      ANode: TcxTreeListNode]: string read GetGroupFooterSummaryText;
    property GroupFooterSummaryValues[ASummaryItem: TcxTreeListSummaryItem;
      ANode: TcxTreeListNode]: Variant read GetGroupFooterSummaryValue
      write SetGroupFooterSummaryValue;
    property TreeList: TcxCustomTreeList read FTreeList;
  end;
  ...
  TcxTreeListNodeChangedEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode) of object;

  TcxTreeListNodeCheckChangedEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode; AState: TcxCheckBoxState) of object;

  TcxTreeListNodeCheckChangingEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode; AState: TcxCheckBoxState; var Allow: Boolean)
    of object;

  TcxTreeListNodeChangingEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode; var Allow: Boolean) of object;

  TcxTreeListColumnChangedEvent = procedure(Sender: TcxCustomTreeList;
    AColumn: TcxTreeListColumn) of object;

  TcxTreeListBandChangedEvent = procedure(Sender: TcxCustomTreeList;
    ABand: TcxTreeListBand) of object;

  TcxTreeListMoveToEvent = procedure(Sender: TcxCustomTreeList;
    AttachNode: TcxTreeListNode; AttachMode: TcxTreeListNodeAttachMode;
    Nodes: TList; var IsCopy, Done: Boolean) of object;
  ...
  TcxTreeListGetCellHintEvent = procedure(Sender: TcxCustomTreeList;
    ACell: TObject; var AText: string; var ANeedShow: Boolean) of object;

  TcxTreeListGetDragDropTextEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode; var AText: string) of object;

  TcxTreeListGetLevelImagesEvent = procedure(Sender: TcxCustomTreeList;
    ALevel: Integer; var AImages, AStateImages: TCustomImageList) of object;
  ...
  TcxTreeListGetNodeImageIndexEvent = procedure(Sender: TcxCustomTreeList;
    ...
    var AIndex: TcxImageIndex) of object;
  ...
  TcxTreeListGetNodePreviewHeightEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode; const ADisplayValue: Variant; var AHeight: Integer)
    of object;
  ...
  TcxTreeListHotTrackNodeEvent = procedure(Sender: TcxCustomTreeList;
  ...
  TcxTreeListCustomDrawBackgroundCellEvent = procedure
    (Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
    AViewInfo: TcxTreeListCustomCellViewInfo; var ADone: Boolean) of object;
  ...
  TcxTreeListFilterNodeEvent = procedure(Sender: TcxCustomTreeList;
    ANode: TcxTreeListNode; var Accept: Boolean) of object;
  ...
  protected
    function GetDateTimeRelativeFilterDisplayText(AKind: TcxFilterOperatorKind)
      : string; override;
  end;
  ...
  TcxCustomTreeList = class(TcxExtEditingControl, IcxNavigator, IcxStoredParent,
    IcxStoredObject, IcxTreeListDesignTimeOperations,
    IcxTreeListDesignTimeOperations2, IdxSkinSupport,
    IcxDataControllerConditionalFormattingProviderOwner, IcxFilterControl,
    IcxFilterControlEx)
  private
    ...
    function GetOnFindCriteriaBeforeChange
      : TcxDataFindCriteriaBeforeChangeEvent;
    ...
    procedure SetOnFindCriteriaBeforeChange
      (Value: TcxDataFindCriteriaBeforeChangeEvent);
    ...
    procedure SetScrollbarAnnotations
      (Value: TcxTreeListScrollbarAnnotationOptions);
  ...
  protected
    ...
    procedure CheckImageListReferences(AComponent: TComponent;
    ...
    function CreateScrollbarAnnotations
      : TcxControlScrollbarAnnotationOptions; override;
    ...
    procedure DoCompare(ANode1, ANode2: TcxTreeListNode;
      var ACompare: Integer); virtual;
    ...
    procedure DoEditValueChanged(AItem: TcxCustomInplaceEditContainer);
      override;
    procedure DoExpand(ANode: TcxTreeListNode); virtual;
    function DoFilterNodeEvent(ANode: TcxTreeListNode): Boolean; virtual;
    procedure DoFilterNodes; virtual;
    procedure DoFocusedItemChanged(APrevFocusedItem, AFocusedItem
      : TcxCustomInplaceEditContainer); virtual;
    procedure DoFocusedNodeChanged(APrevNode, AFocusedNode
      : TcxTreeListNode); virtual;
    procedure DoGetCellHint(ACell: TObject; var AText: string;
      var ANeedShow: Boolean); override;
    procedure DoGetLevelImages(AInfo: TcxTreeListLevelInfo;
      ALevel: Integer); virtual;
    procedure DoGetNodeHeight(ANode: TcxTreeListNode;
      var AHeight: Integer); virtual;
    function DoGetNodeImageIndex(ANode: TcxTreeListNode;
      AIndex: TcxTreeListImageIndexType): TcxImageIndex; virtual;
    procedure DoGetPreviewHeight(ANode: TcxTreeListNode;
      var AHeight: Integer); virtual;
    procedure DoHotTrackNode(ANode: TcxTreeListNode; AShift: TShiftState;
      var ACursor: TCursor); virtual;
    procedure DoInplaceEditContainerItemAdded
      (AItem: TcxCustomInplaceEditContainer); override;
    procedure DoInplaceEditContainerItemRemoved
      (AItem: TcxCustomInplaceEditContainer); override;
    ...
    function DoMoveToEvent(AttachNode: TcxTreeListNode;
      AttachMode: TcxTreeListNodeAttachMode; Nodes: TList; var IsCopy: Boolean)
      : Boolean; virtual;
    procedure DoNodeChanged(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn); virtual;
    procedure DoNodeCheckChanged(ANode: TcxTreeListNode;
      AState: TcxCheckBoxState); virtual;
    function DoNodeCheckChanging(ANode: TcxTreeListNode;
      AState: TcxCheckBoxState): Boolean; virtual;
    ...
    procedure DoSetNodeFocused(ANode: TcxTreeListNode; AValue: Boolean;
    ...
    function GetDateTimeHandlingClass
      : TcxControlOptionsDateTimeHandlingClass; override;
    ...
    function GetOptionsCustomizingClass
      : TcxTreeListOptionsCustomizingClass; virtual;
    function GetOptionsDataClass: TcxControlOptionsDataClass; override;
    function GetOptionsFilterBoxClass: TcxControlOptionsFilterBoxClass;
      override;
    function GetOptionsFilteringClass: TcxControlOptionsFilteringClass;
      override;
    function GetOptionsSelectionClass
      : TcxTreeListOptionsSelectionClass; virtual;
    ...
    function IsScrollBarBasedGestureScroll(AScrollKind: TScrollBarKind)
      : Boolean; override;
    ...
    procedure InternalMove(ANode, ADestNode: TcxTreeListNode;
      AMode: TcxTreeListNodeAttachMode); virtual;
    ...
    procedure SetImageList(const AValue: TCustomImageList;
      var AFieldValue: TCustomImageList; const AChangeLink: TChangeLink);
    procedure SetNodeParent(ANewNode, ANewParent: TcxTreeListNode;
    ...
    procedure DoWriteToClipboard(AOnlySelected: Boolean;
      ACheckVisible: Boolean = False); virtual;
    function DoWriteToText(AOnlySelected: Boolean; out AText: string;
      ACheckVisible: Boolean = False): Boolean;
    ...
    function InsertNode(ARelative: TcxTreeListNode; IsAppend: Boolean)
      : Boolean; virtual;
    procedure MakeNodeVisible(ANode: TcxTreeListNode); virtual;
    procedure RestoreCursor;
    procedure SetGlassCursor;
    procedure SortingStateChanged(AColumn: TcxTreeListColumn;
      AShift: TShiftState);
    ...
    procedure DrawDragDropImage(ADragBitmap: TBitmap;
      ACanvas: TcxCanvas); override;
    ...
    //
    function InternalCollapseNode(ANode: TcxTreeListNode; ARecursive: Boolean)
      : Boolean; virtual;
    function InternalExpandNode(ANode: TcxTreeListNode; ARecursive: Boolean)
      : Boolean; virtual;
    ...
    procedure SetStoredPropertyValue(const AName: string;
      const AValue: Variant); virtual;
    function StoredCreateChild(const AObjectName, AClassName: string)
      : TObject; virtual;
    procedure StoredDeleteChild(const AObjectName: string;
    ...
    property AppendToFocused: Boolean read FAppendToFocused
      write FAppendToFocused;
    ...
    property ChangesLocked: Boolean read GetChangesLocked
      write SetChangesLocked;
    property ColumnsList: TList read GetColumnsList;
    property ConditionalFormattingProvider
      : TcxTreeListConditionalFormattingProvider
      read FConditionalFormattingProvider;
    property Controller: TcxTreeListController read GetController;
    property DataController: TcxTreeListDataController read GetDataController;
    property DateTimeHandling: TcxTreeListDateTimeHandling
      read GetDateTimeHandling write SetDateTimeHandling;
    property FilterBox: TcxTreeListFilterBox read GetFilterBox
      write SetFilterBox;
    property Filtering: TcxTreeListFiltering read GetFiltering
      write SetFiltering;
    property IgnoreLoadingStatus: Boolean read FIgnoreLoadingStatus
      write FIgnoreLoadingStatus;
    property IsCancelOperation: Boolean read GetIsCancelOperation
      write FIsCancelOperation;
    property IndentWidth: Integer read GetIndentWidth;
    property IsRefreshFields: Boolean read FIsRefreshFields
      write FIsRefreshFields;
    ...
    property LevelsInfo[ALevel: Integer]: TcxTreeListLevelInfo
      read GetLevelInfo;
    ...
    property OnAfterSummary: TNotifyEvent read FOnAfterSummary
      write FOnAfterSummary;
    property OnBandHeaderClick: TcxTreeListBandChangedEvent
      read FOnBandHeaderClick write FOnBandHeaderClick;
    property OnBandPosChanged: TcxTreeListBandChangedEvent
      read FOnBandPosChanged write FOnBandPosChanged;
    property OnBandSizeChanged: TcxTreeListBandChangedEvent
      read FOnBandSizeChanged write FOnBandSizeChanged;
    property OnBeginDragNode: TcxTreeListNodeChangingEvent read FOnBeginDragNode
      write FOnBeginDragNode;
    property OnCanFocusNode: TcxTreeListNodeChangingEvent read FOnCanFocusNode
      write FOnCanFocusNode;
    property OnCanSelectNode: TcxTreeListNodeChangingEvent read FOnCanSelectNode
      write FOnCanSelectNode;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
    property OnCollapsed: TcxTreeListNodeChangedEvent read FOnCollapsed
      write FOnCollapsed;
    property OnCollapsing: TcxTreeListNodeChangingEvent read FOnCollapsing
      write FOnCollapsing;
    property OnColumnHeaderClick: TcxTreeListColumnChangedEvent
      read FOnColumnHeaderClick write FOnColumnHeaderClick;
    property OnColumnPosChanged: TcxTreeListColumnChangedEvent
      read FOnColumnPosChanged write FOnColumnPosChanged;
    property OnColumnSizeChanged: TcxTreeListColumnChangedEvent
      read FOnColumnSizeChanged write FOnColumnSizeChanged;
    property OnCompare: TcxTreeListNodeCompareEvent read FOnCompare
      write FOnCompare;
    property OnCustomDrawBackgroundCell
      : TcxTreeListCustomDrawBackgroundCellEvent
      read FOnCustomDrawBackgroundCell write FOnCustomDrawBackgroundCell;
    property OnCustomDrawBandCell: TcxTreeListCustomDrawBandCellEvent
      read FOnCustomDrawBandCell write FOnCustomDrawBandCell;
    property OnCustomDrawBandHeaderCell: TcxTreeListCustomDrawHeaderCellEvent
      read FOnCustomDrawBandHeaderCell write FOnCustomDrawBandHeaderCell;
    property OnCustomDrawDataCell: TcxTreeListCustomDrawEditCellEvent
      read FOnCustomDrawDataCell write FOnCustomDrawDataCell;
    property OnCustomDrawFooterCell: TcxTreeListCustomDrawFooterCellEvent
      read FOnCustomDrawFooterCell write FOnCustomDrawFooterCell;
    ...
    property OnCustomDrawIndentCell: TcxTreeListCustomDrawIndentCellEvent
      read FOnCustomDrawIndentCell write FOnCustomDrawIndentCell;
    property OnCustomDrawIndicatorCell: TcxTreeListCustomDrawIndicatorCellEvent
      read FOnCustomDrawIndicatorCell write FOnCustomDrawIndicatorCell;
    property OnCustomDrawPreviewCell: TcxTreeListCustomDrawEditCellEvent
      read FOnCustomDrawPreviewCell write FOnCustomDrawPreviewCell;
    property OnCustomizationFormVisibleChanged: TNotifyEvent
      read FOnCustomizationVisibleChanged write FOnCustomizationVisibleChanged;
    property OnDeletion: TcxTreeListNodeChangedEvent read FOnDeletion
      write FOnDeletion;
    property OnExpanded: TcxTreeListNodeChangedEvent read FOnExpanded
      write FOnExpanded;
    property OnExpanding: TcxTreeListNodeChangingEvent read FOnExpanding
      write FOnExpanding;
    property OnFilterNode: TcxTreeListFilterNodeEvent read FOnFilterNode
      write FOnFilterNode;
    property OnFindCriteriaBeforeChange: TcxDataFindCriteriaBeforeChangeEvent
      read GetOnFindCriteriaBeforeChange write SetOnFindCriteriaBeforeChange;
    property OnFindCriteriaChanged: TcxDataFindCriteriaChangedEvent
      read GetOnFindCriteriaChanged write SetOnFindCriteriaChanged;
    property OnGetCellHint: TcxTreeListGetCellHintEvent read FOnGetCellHint
      write FOnGetCellHint;
    property OnGetDragDropText: TcxTreeListGetDragDropTextEvent
      read FOnGetDragDropText write FOnGetDragDropText;
    property OnGetLevelImages: TcxTreeListGetLevelImagesEvent
      read FOnGetLevelImages write FOnGetLevelImages;
    property OnGetNodeHeight: TcxTreeListGetNodeHeightEvent
      read FOnGetNodeHeight write FOnGetNodeHeight;
    property OnGetNodeImageIndex: TcxTreeListGetNodeImageIndexEvent
      read FOnGetNodeImageIndex write FOnGetNodeImageIndex;
    property OnGetNodePreviewHeight: TcxTreeListGetNodePreviewHeightEvent
      read FOnGetNodePreviewHeight write FOnGetNodePreviewHeight;
    property OnHotTrackNode: TcxTreeListHotTrackNodeEvent read FOnHotTrackNode
      write FOnHotTrackNode;
    property OnIsGroupNode: TcxTreeListIsGroupNodeEvent read FOnIsGroupNode
      write FOnIsGroupNode;
    property OnLeftPosChanged: TNotifyEvent read FOnLeftPosChanged
      write FOnLeftPosChanged;
    property OnMoveTo: TcxTreeListMoveToEvent read FOnMoveTo write FOnMoveTo;
    property OnNodeChanged: TcxTreeListNodeDataChangedEvent read FOnNodeChanged
      write FOnNodeChanged;
    property OnNodeCheckChanged: TcxTreeListNodeCheckChangedEvent
      read FOnNodeCheckChanged write FOnNodeCheckChanged;
    property OnNodeCheckChanging: TcxTreeListNodeCheckChangingEvent
      read FOnNodeCheckChanging write FOnNodeCheckChanging;
    ...
    property OnSummary: TcxTreeListSummaryEvent read FOnSummary
      write FOnSummary;
    property OnTopRecordIndexChanged: TNotifyEvent read FOnTopRecordIndexChanged
      write FOnTopRecordIndexChanged;
    property NavigatorEvents;
    property PopupMenusEvents: TNotifyEvent read FPopupMenusEvents
      write FPopupMenusEvents;
    ...
    property OnEdited: TcxTreeListColumnChangedEvent read GetOnEdited
      write SetOnEdited;
    property OnEditing: TcxTreeListEditingEvent read GetOnEditing
      write SetOnEditing;
    property OnEditValueChanged: TcxTreeListColumnChangedEvent
      read GetOnEditValueChanged write SetOnEditValueChanged;
    property OnFocusedColumnChanged: TcxTreeListFocusedColumnChanged
      read FOnFocusedColumnChanged write FOnFocusedColumnChanged;
    ...
    property OnInitStoredObject: TcxInitStoredObjectEvent
      read FOnInitStoredObject write FOnInitStoredObject;
    property OnSetStoredPropertyValue: TcxSetStoredPropertyValueEvent
      read FOnSetStoredPropertyValue write FOnSetStoredPropertyValue;
    //
  public
    ...
    procedure Cancel;
    procedure Clear;
    function CreateColumn(ABand: TcxTreeListBand = nil)
      : TcxTreeListColumn; virtual;
    ...
    function Find(AData: Pointer; AStart: TcxTreeListNode;
      AExpandedOnly, AForward: Boolean; AFilter: TcxTreeListFindFunc;
      AIgnoreStartNode: Boolean = False): TcxTreeListNode; virtual;
    function FindNodeByText(const AText: string; AColumn: TcxTreeListColumn;
      ...
      AForward: Boolean = True; ACaseSensitive: Boolean = True;
      AMode: TcxTreeListFindMode = tlfmNormal;
      ALikeParams: TcxTreeListLikeParams = nil;
      AIgnoreStartNode: Boolean = False): TcxTreeListNode;
    procedure SelectAll;
    procedure Select(Node: TcxTreeListNode; ShiftState: TShiftState = []);
      overload; virtual;
    ...
    // positions
    function CellRect(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn): TRect;
    function GetEditRect(ANode: TcxTreeListNode;
      AColumn: TcxTreeListColumn): TRect;
    ...
    property AbsoluteItems[Index: Integer]: TcxTreeListNode read GetAbsoluteItem
      write SetAbsoluteItem;
    ...
    property AbsoluteVisibleItems[Index: Integer]: TcxTreeListNode
      read GetAbsoluteVisibleItem write SetAbsoluteVisibleItem;
    ...
    property Count: Integer read GetCount;
    property Customizing: TcxTreeListCustomizing read FCustomizing
      write FCustomizing;
    property DefaultIndentSize: TSize read FDefaultIndentSize;
    property DefaultLayout: Boolean read FDefaultLayout write SetDefaultLayout
      default False;
    property DefaultRowHeight: Integer read GetDefaultRowHeight
    ...
    property FindCriteria: TcxTreeListDataFindCriteria read GetFindCriteria;
    property FindPanel;
    property FocusedColumn: TcxTreeListColumn read GetFocusedColumn
      write SetFocusedColumn;
    property FocusedNode: TcxTreeListNode read GetFocusedNode
      write SetFocusedNodeProp;
    property HitTest: TcxTreeListHitTest read GetHitTest;
    ...
    property LastPartVisibleNode: TcxTreeListNode read GetLastPartVisibleNode;
    property LastVisibleNode: TcxTreeListNode read GetLastVisibleNode
      write SetLastVisibleNode;
    property Navigator;
    property OptionsBehavior: TcxTreeListOptionsBehavior read GetOptionsBehavior
      write SetOptionsBehavior;
    property OptionsCustomizing: TcxTreeListOptionsCustomizing
      read FOptionsCustomizing write SetOptionsCustomizing;
    property OptionsData: TcxTreeListOptionsData read GetOptionsData
      write SetOptionsData;
    property OptionsSelection: TcxTreeListOptionsSelection
      read FOptionsSelection write SetOptionsSelection;
    property OptionsView: TcxTreeListOptionsView read GetOptionsView
      write SetOptionsView;
    property PopupMenus: TcxTreeListPopupMenus read FPopupMenus
      write SetPopupMenus;
    property Preview: TcxTreeListPreview read FPreview write SetPreview;
    property Root: TcxTreeListNode read FRoot;
    property ScrollbarAnnotations: TcxTreeListScrollbarAnnotationOptions
      read GetScrollbarAnnotations write SetScrollbarAnnotations;
  ...
  public
    ...
    function Add(ASibling: TcxTreeListNode; AData: Pointer = nil)
      : TcxTreeListNode; overload;
    function AddChild(AParent: TcxTreeListNode; AData: Pointer = nil)
      : TcxTreeListNode;
    function AddChildFirst(AParent: TcxTreeListNode; AData: Pointer = nil)
      : TcxTreeListNode;
    function AddFirst: TcxTreeListNode; overload;
    function AddFirst(ASibling: TcxTreeListNode; AData: Pointer = nil)
      : TcxTreeListNode; overload;
    function AddNode(ANode, ARelative: TcxTreeListNode; AData: Pointer;
      AttachMode: TcxTreeListNodeAttachMode): TcxTreeListNode; override;
    function Insert(ASibling: TcxTreeListNode; AData: Pointer = nil)
      : TcxTreeListNode;
    function InsertEx(ANode, ASibling: TcxTreeListNode; AData: Pointer = nil)
      : TcxTreeListNode;
...
var
  cxTreeListBuiltInMenuClass: TcxTreeListCustomBuiltInMenuClass =
    TcxTreeListBuiltInMenu;
  DesignerNavigatorProc: procedure(AListener: TObject; AddListener: Boolean)
    of object;

implementation

uses
  Types, CommCtrl, RTLConsts, cxLike, cxFormats, dxThreading, dxDPIAwareUtils,
...
const
  ...
  ColumnPropertiesName: array [0 .. 11] of string = ('Visible', 'Index',
    'LineCount', 'ColIndex', 'RowIndex', 'BandIndex', 'Caption', 'SortOrder',
    'SortIndex', 'RealMinWidth', 'RealWidth', 'Expression');
  BandPropertiesName: array [0 .. 6] of string = ('Caption', 'RealMinWidth',
    'RealWidth', 'Visible', 'Index', 'BandIndex', 'ColIndex');

  GridLineBorders: array [TcxTreeListGridLines] of TcxBorders = ([],
    [bTop, bBottom], [bLeft, bRight], cxBordersAll);

  OddEvenStyleIndexes: array [Boolean] of Integer = (tlsv_ContentEven,
    tlsv_ContentOdd);
...
type
  ...
  protected
    ...
    function GetFocused: Boolean; override;
    function GetSelected: Boolean; override;
    procedure Initialize(ANode: TcxTreeListNode; AHeight, AWidth: Integer;
      ACalcHeight: Boolean);
    function IsAutoHeight: Boolean; override;
    function IsEndEllipsis: Boolean; override;
  public
    constructor Create(AOwner: TObject); override;
    destructor Destroy; override;
    function MeasureHeight(AColumn: TcxTreeListColumn; ANode: TcxTreeListNode;
      AIsCategory: Boolean): Integer;
    function MeasureWidth(AColumn: TcxTreeListColumn;
...
function cxFind(ATreeList: TcxCustomTreeList; AData: Pointer;
  ...
  begin
    with ANode do
    begin
      if (FCount = 0) and HasChildren and not Expanded and not AExpandedOnly
      then
        ANode.LoadChildren;
...
function cxGetLatest(ANode: TcxTreeListNode; ACheckExpanded: Boolean = True)
  : TcxTreeListNode;
begin
  Result := ANode;
  if Result <> nil then
...
function cxGetNativeColor(AColor: TColor;
  ADefColorFunc: TcxDefColorFunc): TColor;
begin
...
function cxCompareBandsByPosition(ABand1, ABand2: TcxTreeListBand): Integer;
begin
  Result := ABand1.FIndex - ABand2.FIndex;
end;

function cxCompareBandsByActualPositionEx(ABand1,
  ABand2: TcxTreeListBand): Integer;

  function IsBandInheritedFromParent(ABand, AParentBand
    : TcxTreeListBand): Boolean;
  begin
    Result := False;
  ...
  function GetParentBandInheritedFromDestination(ABand, ADestinationBand
    : TcxTreeListBand): TcxTreeListBand;
  begin
    Result := ABand;
    while (Result.Position.ParentBand <> nil) and
      (Result.Position.ParentBand <> ADestinationBand) do
...
begin
  ...
  if IsBandInheritedFromParent(ABand1, ABand2) then
    Result := 1
  else if IsBandInheritedFromParent(ABand2, ABand1) then
    Result := -1;
  if Result = 0 then
  begin
    AParentBand1 := nil;
    AParentBand2 := nil;
    repeat
      AParentBand1 := GetParentBandInheritedFromDestination(ABand1,
        AParentBand1);
      AParentBand2 := GetParentBandInheritedFromDestination(ABand2,
        AParentBand2);
      if AParentBand1 <> AParentBand2 then
      begin
        Result := AParentBand1.Position.ColIndex -
          AParentBand2.Position.ColIndex;
        if Result = 0 then
          Result := AParentBand1.Index - AParentBand2.Index;
      end;
    until (Result <> 0) or ((AParentBand1 = ABand1) and
      (AParentBand2 = ABand2));
  end;
end;

function cxCompareBandsByPositionEx(ABand1, ABand2: TcxTreeListBand): Integer;
begin
...
function cxCompareColumnsByActualPosition(AColumn1,
  AColumn2: TcxTreeListColumn): Integer;
begin
  Result := 0;
  if AColumn1 = AColumn2 then
    Exit;
  if AColumn1.Position.Band <> nil then
    Result := cxCompareBandsByActualPositionEx(AColumn1.Position.Band,
      AColumn2.Position.Band);
  if Result = 0 then
...
function cxCompareColumnsByPosition(AColumn1,
  AColumn2: TcxTreeListColumn): Integer;
begin
...
function cxCompareColumnsBySortIndex(AColumn1,
  AColumn2: TcxTreeListColumn): Integer;
begin
...
function cxCompareNodesByAbsoluteIndex(ANode1, ANode2: TcxTreeListNode)
  : Integer;
begin
  Result := dxCompareValues(ANode1.AbsoluteIndex, ANode2.AbsoluteIndex);
end;

function cxCompareNodesByRecordIndex(ANode1, ANode2: TcxTreeListNode): Integer;
begin
...
procedure cxError(Condition: Boolean; const Description: string;
...
constructor TcxTreeListCustomCellViewInfo.CreateEx(ATreeList: TcxCustomTreeList;
...
begin
  ...
  begin
    ...
    ACanvas.Rectangle(cxRectOffset(BoundsRect, GetCellOrigin), ViewParams,
      Borders, BorderColor);
    ACanvas.WindowOrg := AWindowOrg;
  end;
end;

procedure TcxTreeListCustomCellViewInfo.DoRightToLeftConversion
...
begin
  if CellHasOrigin then
    Result := Visible and PtInRect(cxRectOffset(ClipRect, GetCellOrigin),
      AHitTest.HitPoint)
  else
...
procedure TcxTreeListCustomCellViewInfo.SetBounds(const ABounds,
...
begin
  ...
  if not Transparent then
    Painter.DrawFooterCellContent(ACanvas, BoundsRect, AlignHorz, AlignVert,
      MultiLine, '', ViewParams.Font, ViewParams.TextColor, ViewParams.Color,
...
begin
  ...
  ACanvas.DrawText(FText, GetTextBounds, AlignHorz, AlignVert, MultiLine,
    ShowEndEllipsis);
...
begin
  ...
  Result := PtInRect(cxRectInflate(R, ScaleFactor.Apply(cxTextOffset)),
    AHitTest.HitPoint);
  if not Result then
    Exit;
  if not VisibleInfoCalculated then
...
begin
  Result := (Column.Position.LineCount > 1) and (GetItem <> nil) and
    GetItem.MultiLine;
end;
...
begin
  Result := BoundsRect;
  AContentOffset.Init(cxHeaderTextOffset, cxHeaderTextOffset,
    cxHeaderTextOffset, cxHeaderTextOffset);
  TdxVisualRefinements.Padding.InflatePadding(AContentOffset, ScaleFactor);
  Result.Deflate(AContentOffset);
end;
...
procedure TcxTreeListFooterCellViewInfo.Initialize(ANode,
  AttachNode: TcxTreeListNode; AColumn: TcxTreeListColumn);
begin
...
begin
  if Column.Position.Band.FixedKind <> tlbfNone then
    Exit;
...
procedure TcxTreeListFooterSingleCellViewInfo.Initialize(ANode,
  AttachNode: TcxTreeListNode; AColumn: TcxTreeListColumn;
...
begin
  Result := True { Assigned(Styles.FOnGetColumnFooterStyle) } and
    (VisibleCount > 1);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      FHintRect := R;
      R := cxRectSetHeight(R, TdxTextMeasurer.TextLineHeight(ViewParams.Font) +
      ...
      begin
        if not SummaryItems[I].Visible then
          Continue;
        Inc(J);
        Inc(R.Left, AWidth);
        ...
        if (ACountInLine = 0) or (cxRectWidth(R) >= AWidth) or not MultiLine
        then
          Inc(ACountInLine)
        else if (cxRectWidth(R) < AWidth) then
        begin
          R1 := cxRectSetTop(cxTextRect(DisplayRect), R.Bottom,
            cxRectHeight(R));
          ACountInLine := 1;
          FNeedShowHint := FNeedShowHint or
            (R1.Bottom > cxTextRect(DisplayRect).Bottom);
        ...
        FNeedShowHint := FNeedShowHint or
          ((R.Left + AWidth) > cxTextRect(DisplayRect).Right);
        ACanvas.Font.Color := Styles.GetColumnFooterParams(Column, Node,
          SummaryItems[I]).TextColor;
        ACanvas.Brush.Style := bsClear;
        ACanvas.DrawText(AText, R, AlignHorz, AlignVert, False,
          ShowEndEllipsis);
        ACanvas.Brush.Style := bsSolid;
      end;
...
function TcxTreeListFooterMultiItemsCellViewInfo.GetItem
  : TcxTreeListSummaryItem;
var
  I: Integer;
begin
...
begin
  Result := False;
  for I := 0 to SummaryItems.Count - 1 do
    if SummaryItems[I].Visible and TreeList.Controller.IsObjectSelected
      (SummaryItems[I]) then
    begin
      Result := True;
      Break;
...
procedure TcxTreeListFooterMultiItemsCellViewInfo.Initialize(ADataNode,
  AttachNode: TcxTreeListNode; AColumn: TcxTreeListColumn;
...
begin
  ...
  begin
    ACanvas.Font := ViewParams.Font;
    AIsMultiLine := MultiLine;
    Result := FNeedShowHint and
      PtInRect(cxRectOffset(FHintRect, GetCellOrigin), P);
    AText := FText;
    ATextRect := FHintRect;
    IsNeedOffsetHint := False;
  end;
...
begin
  ...
  if AShift = 0 then
    Exit;
  if (BoundsRect.Right = ACandidate.BoundsRect.Left) and
    cxRectIsEqual(VisibleRect, ACandidate.VisibleRect) then
  begin
  ...
  if CanNeighborFor(ACandidate) and
    (BoundsRect.Bottom = ACandidate.BoundsRect.Top) then
  begin
...
begin
  FTextBounds := cxRectInflate(BoundsRect, -1, -1);
...
begin
  ...
  if ExtPaintStyle then
    Painter.DrawScaledHeaderEx(ACanvas, R, TextBounds, Neighbors, Borders,
      State, GetRealAlignHorz, GetAlignVert, GetMultiline, GetShowEndEllipsis,
      GetText, ACanvas.Font, ViewParams.TextColor, ViewParams.Color,
      ScaleFactor, DrawBackgroundHandler)
  else
    Painter.DrawScaledHeader(ACanvas, R, TextBounds, Neighbors, Borders, State,
      GetRealAlignHorz, GetAlignVert, GetMultiline, GetShowEndEllipsis, GetText,
      ACanvas.Font, ViewParams.TextColor, ViewParams.Color, ScaleFactor,
      DrawBackgroundHandler, IsLast);
end;

procedure TcxTreeListCustomHeaderCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  ...
  BordersMargins := TdxRightToLeftLayoutConverter.ConvertOffsets
    (BordersMargins);
end;
...
function TcxTreeListCustomHeaderCellViewInfo.
  NeedRecalculateOnStateChanged: Boolean;
begin
  Result := False;
end;
...
begin
  if (AValue = Pressed) or not GetIsVisible then
    Exit;
  if AValue then
...
function TcxTreeListCustomHeaderCellViewInfo.HasHintPoint
...
begin
  if Pressed or not GetIsVisible then
    Exit;
  if AValue then
    State := cxbsHot
  else
...
begin
  ...
  begin
    ...
    if TreeList.IsNodeInserting(Node) then
      Result := ikInsert
    else if TreeList.IsNodeEdited(Node) then
      Result := ikEdit;
  end
...
begin
  ...
  begin
    if ExtPaintStyle then
      Painter.DrawScaledIndicatorItemEx(ACanvas, DisplayRect, Kind,
        ViewParams.Color, ScaleFactor, DrawBackgroundHandler)
    else
      Painter.DrawScaledIndicatorItem(ACanvas, DisplayRect, Kind,
        ViewParams.Color, ScaleFactor, DrawBackgroundHandler);

    ACanvas.ExcludeClipRect(ClipRect);
  end
  else if Position = tlipFooter then
    Painter.DrawFooterPanel(ACanvas, DisplayRect, ViewParams, Borders)
  else
  begin
    inherited DoDraw(ACanvas);
    if (Position in [tlipBands, tlipColumns]) and IsQuickCustomizationEnabled
    then
      Painter.DrawScaledIndicatorCustomizationMark(ACanvas, DisplayRect,
        ViewParams.TextColor, ScaleFactor);
  end;
end;
...
function TcxTreeListIndicatorCellViewInfo.GetPopup
  : TcxTreeListCustomizationPopup;
begin
...
constructor TcxTreeListHeaderCellViewInfo.CreateEx(ATreeList: TcxCustomTreeList;
  const ABounds, AVisibleRect: TRect);
begin
  inherited CreateEx(ATreeList, ABounds, AVisibleRect);
...
begin
  ...
  if IsFilterButtonSmartTag then
    FFilterButtonBounds.Bottom := FFilterButtonBounds.Top +
      GetFilterSmartTagHeight;
  if IsRightToLeftConverted then
  begin
    FFilterButtonBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FFilterButtonBounds, BoundsRect);
    FTextBounds := TdxRightToLeftLayoutConverter.ConvertRect(FTextBounds,
      BoundsRect);
  end;
end;
...
begin
  ...
  case GlyphAlignVert of
    ...
    vaCenter:
      FGlyphPosition.Y := (FTextBounds.Bottom + FTextBounds.Top -
        AHeight) div 2;
  end;
  GlyphClipping := not cxInRange(FGlyphPosition.X, ClipRect.Left,
    ClipRect.Right - AWidth) or not cxInRange(FGlyphPosition.Y, ClipRect.Top,
    ClipRect.Bottom - AHeight);
end;
...
begin
  ...
  if IsRightToLeftConverted then
  begin
    FSortMarkBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FSortMarkBounds, BoundsRect);
    FTextBounds := TdxRightToLeftLayoutConverter.ConvertRect(FTextBounds,
      BoundsRect);
  end;
end;
...
begin
  ...
  if Selected then
  begin
    ACanvas.DrawFocusRect(cxRectInflate(cxRectInflate(DisplayRect,
      BordersMargins), -1));
    ACanvas.DrawFocusRect(cxRectInflate(cxRectInflate(DisplayRect,
      BordersMargins), -2));
  end;
...
procedure TcxTreeListHeaderCellViewInfo.DoDrawGlyph(ACanvas: TcxCanvas);
  ...
  begin
    Glyph.StretchDraw(ACanvas.Handle, cxRectSetOrigin(cxRect(GetGlyphSize),
      FGlyphPosition), MaxByte, GlyphColorPalette);
  end;
...
begin
  if IsFilterButtonSmartTag then
    Painter.DrawScaledFilterSmartTag(ACanvas, FilterButtonBounds,
      GetFilterSmartTagState, IsFilterButtonActive, ScaleFactor)
  else
    Painter.DrawScaledFilterDropDownButton(ACanvas, FilterButtonBounds,
      FilterButtonState, IsFilterButtonActive, ScaleFactor);
end;

procedure TcxTreeListHeaderCellViewInfo.DoDrawSortMark(ACanvas: TcxCanvas);
begin
  Painter.DrawScaledSortingMark(ACanvas, FSortMarkBounds,
    SortOrder = soAscending, ScaleFactor);
end;
...
procedure TcxTreeListHeaderCellViewInfo.Initialize
...
begin
  Result := IsFilterable and (IsFilterButtonAlwaysVisible or
    (State in [cxbsHot, cxbsPressed]) or (FilterButtonState = cxbsPressed) or
    (IsFilterButtonSmartTag and IsFilterButtonActive));
end;
...
procedure TcxTreeListHeaderCellViewInfo.ReleaseReference(var AReference
  : TObject);
begin
...
procedure TcxTreeListHeaderCellViewInfo.UpdateFilterButtonHotTrackState
  (const APoint: TPoint);
begin
  if (FilterButtonState = cxbsPressed) or (TreeList.Controller.DragItem <> nil)
  then
    Exit;
...
procedure TcxTreeListHeaderCellViewInfo.UpdateHotTrackState
  (const APoint: TPoint);
begin
...
function TcxTreeListHeaderCellViewInfo.GetFilterSmartTagState
...
procedure TcxTreeListHeaderCellViewInfo.SetFilterButtonState
...
function TcxTreeListColumnHeaderCellViewInfo.
  CloseFilterPopupOnDestruction: Boolean;
begin
  Result := False;
end;
...
procedure TcxTreeListColumnHeaderCellViewInfo.InitFilterPopup
...
function TcxTreeListColumnHeaderCellViewInfo.GetFilterPopupLinkComponent
  : TComponent;
begin
  Result := Column;
end;

function TcxTreeListColumnHeaderCellViewInfo.GetFilterPopupMode
...
begin
  if IsFilterPopupExist and FilterPopup.Visible and (FilterPopup.Owner = nil)
    and (FilterPopup.LinkComponent = GetFilterPopupLinkComponent) then
  begin
...
function TcxTreeListColumnHeaderCellViewInfo.GetFilterPopup
  : TdxFilterPopupWindow;
begin
  Result := TreeList.Controller.FilterPopup;
end;

function TcxTreeListColumnHeaderCellViewInfo.GetFilterPopupAlignHorz
  : TcxPopupAlignHorz;
begin
...
function TcxTreeListColumnHeaderCellViewInfo.
  IsFilterButtonAlwaysVisible: Boolean;
begin
...
begin
  ...
  case Part of
    ...
    tlbpContent:
      begin
        ACanSelect := TreeList.OptionsSelection.CellSelect and
          TreeList.OptionsSelection.InvertSelect;
        if (Node <> nil) and Node.Selected and (not Node.Focused or ACanSelect)
        then
    ...
    tlbpSeparator:
      begin
        ...
        ItemViewParams.Color :=
          ColorToRgb(cxGetNativeColor(OptionsView.FFixedSeparatorColor,
...
begin
  if (Band = nil) or (Band.FixedKind <> tlbfNone) then
    Exit;
...
procedure TcxTreeListEditCellViewInfo.AfterDrawCellBackground
  (ACanvas: TcxCanvas);
begin
  inherited AfterDrawCellBackground(ACanvas);
  ConditionalFormattingProvider.AfterDrawCellBackground(Self,
    ViewParams.Color, ACanvas);
end;
...
procedure TcxTreeListEditCellViewInfo.CalculateCellEditorBounds
...
function TcxTreeListEditCellViewInfo.FormatDisplayValue
...
begin
  ...
  if Focused and OptionsView.FocusRect and (Node.ViewData <> nil) and
    not Node.ViewData.DrawFocusRect and
...
function TcxTreeListEditCellViewInfo.GetButtonTransparency
...
procedure TcxTreeListEditCellViewInfo.GetFindTextInfo(out AStart: Integer;
  out ALength: Integer);
var
  AText: string;
begin
  AStart := TreeList.FindCriteria.GetTextStartPositionByNode(Node,
    EditContainer.ItemIndex, AText);
...
function TcxTreeListEditCellViewInfo.GetHitTest
  (AHitTest: TcxCustomHitTestController): Boolean;
begin
  Result := PtInRect(cxRectOffset(ClipRect, NodeViewData.Origin),
    AHitTest.HitPoint);
  if not Result then
    Exit;
...
begin
  ...
  if not Node.Focused or not Result then
    Exit;
...
procedure TcxTreeListEditCellViewInfo.Invalidate(const R: TRect;
...
function TcxTreeListEditCellViewInfo.IsEditHotTrack(const APoint
  : TPoint): Boolean;
begin
...
function TcxTreeListEditCellViewInfo.GetConditionalFormattingProvider
  : TcxTreeListConditionalFormattingProvider;
begin
...
begin
  DY := 0;
  ARect := DisplayRect;
  if (Kind = nikCheck) or (Kind = nikState) or
    ((Images <> nil) and TreeList.OptionsView.DynamicIndent) then
  begin
    ARect.Right := ARect.Left + TreeList.ViewInfo.LevelInfo[Index + 1].Size.cx;
    ACenter := cxRectCenter(ARect);
  end;
  if (Node.ViewData.PreviewHeight > 0) and (TreeList.Preview.Place = tlppTop)
  then
    DY := 1;
  X2 := ARect.Right;
  if IsRightToLeftConverted then
  ...
  begin
    ...
    if (Images <> nil) and
      (FVertTreeLine.Right >= (DisplayRect.Right -
      ScaleFactor.Apply(cxTextOffset))) then
...
begin
  ...
  FButton := TreeList.OptionsView.Buttons and (Images = nil) and
    (Kind = nikLevel) and (Index = Node.Level) and Node.HasVisibleChildren;
  ...
  else if Kind = nikCheck then
  begin
    if Node.Parent.CheckGroupType = ncgRadioGroup then
      FGlyphRect := cxRectCenter(DisplayRect,
        Painter.ScaledRadioButtonSize(ScaleFactor))
    else
      FGlyphRect := cxRectCenter(DisplayRect,
        Painter.ScaledCheckButtonSize(ScaleFactor));
  end;

  if (OptionsView.TreeLineStyle <> tllsNone) and (Lines <> []) then
    CalculateTreeLines(cxRectCenter(cxRectSetWidth(DisplayRect,
      TreeList.DefaultIndentSize.cx)));
end;
...
procedure TcxTreeListIndentCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  FGlyphRect := TdxRightToLeftLayoutConverter.ConvertRect(FGlyphRect,
    AClientBounds);
end;
...
begin
  ...
  if Node.Parent.CheckGroupType = ncgRadioGroup then
    Painter.DrawScaledRadioButton(ACanvas, GlyphRect.Left, GlyphRect.Top,
      StateMap[Node.Enabled, HotTrack], CheckState = cbsChecked, False,
      clDefault, ScaleFactor)
  else
...
begin
  ...
  begin
    ...
    cxDrawImage(ACanvas, GlyphRect, nil, Images, ImageIndex, True,
      AColorPalette, ScaleFactor);
    cxDrawImage(ACanvas, GlyphRect, nil, Images, OverlayIndex, True,
      AColorPalette, ScaleFactor);
  end;
end;
...
procedure TcxTreeListIndentCellViewInfo.DrawTreeLine(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
  if OptionsView.TreeLineStyle = tllsDot then
    cxFillHalfToneRect(ACanvas.Canvas, ARect, ViewParams.Color,
      TreeList.ViewInfo.TreeLineColor)
  else
    ACanvas.FillRect(ARect, TreeList.ViewInfo.TreeLineColor);
end;

function TcxTreeListIndentCellViewInfo.GetCheckBoxViewInfo(ACanvas: TcxCanvas)
...
var
  R: TRect;
const
  AImages: array [Boolean] of Integer = (tlhc_HitAtImage, tlhc_HitAtStateImage);
begin
  Result := inherited GetHitTest(AHitTest);
  if not Result then
    Exit;
  ...
  begin
    ...
    else if Button and PtInRect(R, AHitTest.HitPoint) then
      SetHitTestCodes([tlhc_HitAtButton])
  end;
end;

procedure TcxTreeListIndentCellViewInfo.InitializeLevelIndent(ANode,
  AAttachNode: TcxTreeListNode; AIndex: Integer);
begin
  FIndex := AIndex;
  FNode := ANode;
  FAttachNode := AAttachNode;
...
begin
  ...
  FImageIndex := -1;
  FOverlayIndex := -1;
  if (Images = nil) or not(Kind in [nikImage, nikState]) then
    Exit;
  if Kind = nikImage then
  begin
    FImageIndex := TreeList.DoGetNodeImageIndex(Node,
      TcxTreeListImageIndexType(Node.Selected));
  ...
  begin
    ...
    if FImageIndex <> -1 then
      FOverlayIndex := TreeList.DoGetNodeImageIndex(Node,
...
function TcxTreeListIndentCellViewInfo.IsNeedHint(ACanvas: TcxCanvas;
  const P: TPoint; out AText: TCaption; out AIsMultiLine: Boolean;
  out ATextRect: TRect; var IsNeedOffsetHint: Boolean): Boolean;
begin
  AText := '';
...
procedure TcxTreeListIndentCellViewInfo.UpdateHotTrackState
  (const APoint: TPoint);
begin
...
function TcxTreeListFindPanelViewInfo.GetHitTest
  (AHitTest: TcxCustomHitTestController): Boolean;
begin
  Result := inherited GetHitTest(AHitTest);
  if Result then
...
function TcxTreeListFilterBoxViewInfo.GetHitTest
  (AHitTest: TcxCustomHitTestController): Boolean;
begin
  Result := inherited GetHitTest(AHitTest);
  if Result then
...
function TcxTreeListNavigatorSiteViewInfo.GetHitTest
  (AHitTest: TcxCustomHitTestController): Boolean;
begin
  Result := inherited GetHitTest(AHitTest);
  if Result then
    TcxCustomTreeList(OwnerControl).HitTest.SetHitState
      (tlhc_HitAtNavigator, True);
end;
...
function TcxTreeListPreviewCellViewInfo.GetButtonTransparency
  : TcxEditButtonTransparency;
begin
  Result := ebtHideInactive;
end;
...
begin
  ...
  begin
    ACustomizationControl :=
      CustomizationControl as TdxCustomTreeListQuickCustomizationControl;
    AMaxRowCount := (ADesktopSpace - ACustomizationControl.CheckListBox.Top)
      div ACustomizationControl.CheckListBox.ItemHeight - 1;
    ...
    begin
      AColumnCount := ACustomizationControl.CheckListBox.Count div AMaxRowCount
        + Integer(ACustomizationControl.CheckListBox.Count mod
        AMaxRowCount <> 0);
      repeat
        RestoreControlsBounds;
        SetVisibleRowCount(Min(AMaxRowCount,
          ACustomizationControl.CheckListBox.Count div AColumnCount + 1));
...
procedure TcxTreeListCustomizationPopup.CheckListBoxAction
...
begin
  ...
  if CustomizationControl.HasCommands then
    CustomizationControl.CommandListBox.States[0] :=
      TdxCustomTreeListQuickCustomizationControl(CustomizationControl)
      .GetCheckingAllState;
end;

procedure TcxTreeListCustomizationPopup.CheckListBoxCheckClicked
  (AIndex: Integer; AChecked: Boolean);
begin
  ItemClicked(CheckListBox.Items[AIndex].Data, AChecked);
end;

procedure TcxTreeListCustomizationPopup.CheckListBoxInitializeSelection
  (Sender: TObject);
var
  I: Integer;
begin
  ...
  try
    if TreeList.IsDesigning then
      for I := 0 to CheckListBox.Count - 1 do
        if TreeList.Controller.IsObjectSelected
...
procedure TcxTreeListCustomizationPopup.CheckListBoxSelectionChanged
  (Sender: TObject);
var
  AItems: TList;
  AFocusedItem: TObject;
begin
  if not TreeList.IsDesigning or FSelectionInitializingInProcess then
    Exit;
  AItems := TList.Create;
  try
...
procedure TcxTreeListCustomizationPopup.
...
begin
  ...
  for I := 0 to FChangingItems.Count - 1 do
    FChangingItemsCheckStates[I] :=
      Integer(IsVisibleTreeListItem(FChangingItems[I]));
end;

procedure TcxTreeListCustomizationPopup.CheckListSelectedItemCheckedStateChanged
  (Sender: TObject);
var
  I, AIndex: Integer;
  AItems: TList;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (AIndex = -1) or
          (Integer((IsVisibleTreeListItem(CheckListBox.Items[AIndex].Data))) <>
          FChangingItemsCheckStates[I]) then
          HandleItemClicked(FChangingItems[I],
            IsVisibleTreeListItem(FChangingItems[I]));
      end;
...
begin
  FCustomizationControl := TdxCustomTreeListQuickCustomizationControl.
    Create(Self);
...
function TcxTreeListCustomizationPopup.GetCheckListBox
  : TdxQuickCustomizationCustomCheckListBox;
begin
  Result := CustomizationControl.CheckListBox;
end;

procedure TcxTreeListCustomizationPopup.GetCheckListBoxSelectedItems
...
procedure TcxTreeListCustomizationPopup.ItemClicked(AItem: TObject;
  AChecked: Boolean);
begin
...
procedure TcxTreeListCustomizationPopup.SetCheckListBoxSelectedItems
...
begin
  TdxCustomTreeListQuickCustomizationControl(CustomizationControl)
    .CheckListBoxVisibleRowCount := ACount;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        CheckListBox.Items.AddObject(AColumn.Caption.Text, AColumn).Checked :=
          AColumn.Visible;
      end;
    finally
      AList.Free;
...
procedure TcxTreeListColumnsCustomizationPopup.HandleItemClicked(AItem: TObject;
  AChecked: Boolean);
begin
...
begin
  Result := TreeList.OptionsCustomizing.
    ColumnsQuickCustomizationMultiColumnMode;
end;

function TcxTreeListColumnsCustomizationPopup.IsVisibleTreeListItem
  (AItem: TObject): Boolean;
begin
  Result := (AItem is TcxTreeListColumn) and TcxTreeListColumn(AItem).Visible;
end;

procedure TcxTreeListColumnsCustomizationPopup.SetQuickCustomizationSorted
  (AValue: Boolean);
begin
  TreeList.OptionsCustomizing.ColumnsQuickCustomizationSorted := AValue;
end;

procedure TcxTreeListColumnsCustomizationPopup.
  SynchronizeTreeListItemsVisibilityWithListBox;
var
  I: Integer;
begin
  for I := 0 to CheckListBox.Count - 1 do
    (CheckListBox.Items[I].Data as TcxTreeListColumn).Visible :=
  ...
  try
    ...
    try
      ...
      begin
        ...
        CheckListBox.Items.AddObject(ABand.Caption.Text, ABand).Checked :=
          ABand.Visible;
      end;
    finally
      AList.Free;
...
procedure TcxTreeListBandsCustomizationPopup.HandleItemClicked(AItem: TObject;
  AChecked: Boolean);
var
  AItems: TList;
  AFocusedItem: TObject;
...
begin
  ...
  begin
    ...
    ASize := cxSize(BorderWidths[bLeft] + CustomizationControl.Width +
      BorderWidths[bRight], BorderWidths[bTop] + CustomizationControl.Height +
      BorderWidths[bBottom]);
...
function TcxTreeListBandsCustomizationPopup.IsVisibleTreeListItem
  (AItem: TObject): Boolean;
begin
  Result := (AItem is TcxTreeListBand) and TcxTreeListBand(AItem).Visible;
end;

procedure TcxTreeListBandsCustomizationPopup.SetQuickCustomizationSorted
  (AValue: Boolean);
begin
  TreeList.OptionsCustomizing.BandsQuickCustomizationSorted := AValue;
end;

procedure TcxTreeListBandsCustomizationPopup.
  SynchronizeTreeListItemsVisibilityWithListBox;
var
  I: Integer;
begin
  for I := 0 to CheckListBox.Count - 1 do
    (CheckListBox.Items[I].Data as TcxTreeListBand).Visible :=
      CheckListBox.Checked[I];
end;

{ TdxCustomTreeListQuickCustomizationControl }

procedure TdxCustomTreeListQuickCustomizationControl.DoCheckAllItems
  (AValue: Boolean);
var
...
procedure TdxCustomTreeListQuickCustomizationControl.CheckAllItems
  (Sender: TObject);
begin
...
function TdxCustomTreeListQuickCustomizationControl.GetTreeList
  : TcxCustomTreeList;
begin
  Result := Popup.TreeList;
end;

function TdxCustomTreeListQuickCustomizationControl.GetPopup
  : TcxTreeListCustomizationPopup;
begin
  Result := Owner as TcxTreeListCustomizationPopup;
end;

function TdxCustomTreeListQuickCustomizationControl.GetCheckingAllState
  : TcxCheckBoxState;
const
...
procedure TdxCustomTreeListQuickCustomizationControl.KeyDown(var Key: Word;
  Shift: TShiftState);
begin
...
constructor TcxTreeListEditingController.Create(AController
...
procedure TcxTreeListEditingController.EditKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TcxTreeListEditingController.CheckMultilineEditBounds
  (var ABounds: TRect);
begin
...
function TcxTreeListEditingController.CreateMultilineEdit
  : TcxAutoHeightInplaceEdit;
begin
  Result := TcxAutoHeightInplaceEdit.Create(Self);
end;

function TcxTreeListEditingController.GetAdjustedMultilineEditBounds
...
begin
  if TreeList.OptionsView.FocusRect then
    Result := cxRectOffset(TreeList.FocusedNode.ViewData.FocusRectBounds,
      TreeList.FocusedNode.ViewData.Origin)
  else
    Result := cxNullRect;
end;
...
var
  R: TRect;
begin
  if not CanUpdateMultilineEditHeight then
    Exit;
...
procedure TcxTreeListEditingController.StartEditAutoHeight(AHeightChanged
  : Boolean);
begin
...
begin
  if TreeList.IsDestroying or TreeList.LockChanges then
    Exit;
  Update(0);
  TreeList.ImagesChanged(Sender);
end;

procedure TcxTreeListLevelInfo.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
begin
  ...
  begin
    ...
    FSize.cy := Max(FSize.cy, AImageSize.cy + ScaleFactor.Apply
      (cxTreeListIndentOffsetSize) div 2);
    if not DynamicIndent then
    begin
      FSize.cx := Max(AImageSize.cx + ScaleFactor.Apply
        (cxTreeListIndentOffsetSize) div 2, FSize.cx);
      Inc(FWidth, FSize.cx);
    end
    else
      Inc(FWidth, AImageSize.cx + ScaleFactor.Apply
        (cxTreeListIndentOffsetSize) div 2);
  end;
...
constructor TcxTreeListConditionalFormattingProvider.Create
...
function TcxTreeListConditionalFormattingProvider.EditCellViewInfoToPoint
...
function TcxTreeListConditionalFormattingProvider.GetController
  : TcxCustomControlController;
begin
  Result := FOwner.Controller;
end;

function TcxTreeListConditionalFormattingProvider.GetItemDisplayName
...
function TcxTreeListConditionalFormattingProvider.GetLookAndFeel
  : TcxLookAndFeel;
begin
...
function TcxTreeListConditionalFormattingProvider.GetScaleFactor
  : TdxScaleFactor;
begin
  Result := FOwner.ScaleFactor;
end;

function TcxTreeListConditionalFormattingProvider.IsItemVisible
  (AItem: TObject): Boolean;
var
  AColumn: TcxTreeListColumn absolute AItem;
begin
...
function TcxTreeListConditionalFormattingProvider.IsRowVisible
...
function TcxTreeListFilterValueList.GetDateTimeRelativeFilterDisplayText
  (AKind: TcxFilterOperatorKind): string;
begin
  case AKind of
    foYesterday:
...
procedure TcxTreeListPainter.CustomDrawCell(ACanvas: TcxCanvas;
  ACell: TcxCustomViewInfoItem; var ADone: Boolean);
begin
  ADone := False;
  if not CustomDrawSupported then
    Exit;
  with TreeList do
  begin
    case ACell.CustomDrawID of
      cxtlBackgroundCell:
        if Assigned(OnCustomDrawBackgroundCell) then
          OnCustomDrawBackgroundCell(TreeList, ACanvas,
            TcxTreeListCustomCellViewInfo(ACell), ADone);
      cxtlColumnHeaderCell:
        if Assigned(OnCustomDrawHeaderCell) then
          OnCustomDrawHeaderCell(TreeList, ACanvas,
            TcxTreeListHeaderCellViewInfo(ACell), ADone);
      cxtlBandHeaderCell:
        if Assigned(OnCustomDrawBandHeaderCell) then
          OnCustomDrawBandHeaderCell(TreeList, ACanvas,
            TcxTreeListHeaderCellViewInfo(ACell), ADone);
      cxtlBandPartCell:
        if Assigned(OnCustomDrawBandCell) then
          OnCustomDrawBandCell(TreeList, ACanvas,
            TcxTreeListBandCellViewInfo(ACell), ADone);
      cxtlEditCell:
        if Assigned(OnCustomDrawDataCell) then
          OnCustomDrawDataCell(TreeList, ACanvas,
            TcxTreeListEditCellViewInfo(ACell), ADone);
      cxtlFooterCell:
        if Assigned(OnCustomDrawFooterCell) then
          OnCustomDrawFooterCell(TreeList, ACanvas,
            TcxTreeListFooterCellViewInfo(ACell), ADone);
      cxtlIndentCell:
        if Assigned(OnCustomDrawIndentCell) then
          OnCustomDrawIndentCell(TreeList, ACanvas,
            TcxTreeListIndentCellViewInfo(ACell), ADone);
      cxtlIndicatorCell:
        if Assigned(OnCustomDrawIndicatorCell) then
          OnCustomDrawIndicatorCell(TreeList, ACanvas,
            TcxTreeListIndicatorCellViewInfo(ACell), ADone);
      cxtlPreviewCell:
        if Assigned(OnCustomDrawPreviewCell) then
          OnCustomDrawPreviewCell(TreeList, ACanvas,
            TcxTreeListEditCellViewInfo(ACell), ADone);
    end;
  end;
end;
...
begin
  Result := Visible and (FColumn <> nil) and (TreeList.VisibleColumnCount > 0)
    and ((MaxLineCount <> 0) or AutoHeight);
end;
...
begin
  ...
  Result := (FImages.Count > 0) and IsImageIndexValid and
    (not cxPointIsEqual(APosition, FPrevPosition) or not Visible or
    (FPrevImageIndex <> ImageIndex));
  if Result then
  begin
    Hide;
    ...
    SetBounds(AWindowPosition.X, AWindowPosition.Y, FImages.Width,
      FImages.Height);
    Show;
  end;
end;
...
procedure TcxTreeListNodeDragInfoImage.SetState(const Value
  : TcxNodeDragInfoState);
begin
...
function TcxTreeListController.CheckAutoScrolling(const APoint: TPoint)
  : Boolean;
var
  ASide: TcxBorder;
begin
...
begin
  if (ANode <> nil) and ANode.IsHidden or not TreeList.DoCanFocusNode(ANode)
  then
    Exit;
...
procedure TcxTreeListController.SetFocusedNodeItem(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn);
begin
...
begin
  if TcxTreeListNode(Value) = FocusedNode then
    Exit;
  if Value = 0 then
    FocusedNode := nil
  else
...
begin
  ...
  if (TreeList.Bands.ExpandableBand <> nil) and
    (TreeList.Bands.ExpandableBand.FixedKind = tlbfNone) then
  begin
  ...
  ScrollControllers[bRight] := CreateAutoScrollObject(sbHorizontal,
    cxRectSetWidth(R, R.Right - cxtlScrollDelta * 2, cxtlScrollDelta * 2),
    GetRealScrollCode(scLineDown));
...
procedure TcxTreeListController.DragMove(Source: TObject; const P: TPoint;
...
begin
  ProcessDragMove;
  HitTest.Recalculate(P);
  if CanDrop and TreeList.OptionsBehavior.DragFocusing and not HitTest.HitNode.Focused
  then
...
procedure TcxTreeListController.DragOver(Source: TObject; X, Y: Integer;
...
procedure TcxTreeListController.DropTo(ANode: TcxTreeListNode;
...
begin
  ...
  if TreeList.OptionsBehavior.AlwaysShowEditor or
    TreeList.OptionsBehavior.ImmediateEditor and
    ((FocusedItem <> nil) and (FocusedNode <> nil)) then
...
function TcxTreeListController.GetRealScrollCode(ACode: TScrollCode)
  : TScrollCode;
begin
  Result := ACode;
...
begin
  if (FocusedNode <> nil) and (FocusedNode.GetNextVisibleEx(AForward) <> nil)
  then
    FocusedNode := FocusedNode.GetNextVisibleEx(AForward);
end;

procedure TcxTreeListController.DoNextPage(AForward: Boolean;
  Shift: TShiftState);
begin
...
procedure TcxTreeListController.DoShowNextPageEx(AGoForward,
  ASetCursor: Boolean; AShift: TShiftState);
...
begin
  if TreeList.AbsoluteVisibleCount = 0 then
    Exit;
  if TreeList.ViewInfo.IsRecordPixelScrollMode then
    if AGoForward then
      ...
      begin
        ...
        begin
          ...
          if FocusedNode.VisibleIndex >=
            TreeList.ViewInfo.FMaxPixelScrollTopNode.VisibleIndex then
    ...
    else if ASetCursor then
      if TreeList.ViewInfo.IsScrollAnimationEnabled then
      begin
        ANode := GetNext(FocusedNode, False);
        if ANode <> nil then
          TreeList.ViewInfo.GetPixelScrollTopNodeAndOffsetByBottomNode(ANode,
            ANextFocusedNode, ANextTopNodePixelScrollOffset)
        else
  ...
  begin
    if (TreeList.LastVisibleNode <> nil) and
      TreeList.LastVisibleNode.IsLastVisible and
      TreeList.LastVisibleNode.Focused and ASetCursor then
      Exit;
    if ASetCursor then
    begin
      TreeList.TopVisibleNode := GetNext(FocusedNode, True);
  ...
  begin
    if (TreeList.TopVisibleNode <> nil) and
      TreeList.TopVisibleNode.IsFirstVisible and
      TreeList.TopVisibleNode.Focused and ASetCursor then
      Exit;
    if ASetCursor then
    begin
      TreeList.LastVisibleNode := GetNext(FocusedNode, False);
...
procedure TcxTreeListController.FocusedRecordChanged(APrevFocusedRecordIndex,
...
procedure TcxTreeListController.FocusedItemChanged(APrevFocusedItem
...
function TcxTreeListController.IsKeyForController(AKey: Word;
...
begin
  ...
  Result := inherited IsKeyForController(AKey, AShift) or
    ((IsEditing and (ssCtrl in AShift) and Result) or
    (not IsEditing and Result));
  if ((AKey = VK_LEFT) or (AKey = VK_RIGHT)) and
    (IsEditing and not OptionsBehavior.AlwaysShowEditor) then
    Result := False;
end;
  ...
  case AKey of
    ...
    VK_HOME:
      begin
        ...
        else if TreeList.VisibleColumnCount > 0 then
    ...
    VK_END:
      begin
        if ssCtrl in Shift then
          ...
          TreeList.VisibleColumns[TreeList.VisibleColumnCount - 1]
            .Focused := True
        else
          Result := False;
      end;
    VK_DELETE:
      ...
      begin
        Result := (ssCtrl in Shift) and CanDeleteSelection and
          DeleteConfirmation;
        if Result then
    ...
    VK_INSERT:
      begin
        if (FocusedNode = nil) and (TreeList.AbsoluteCount > 0) or not CanInsertNode
        then
          Result := False
        else
        begin
        ...
        begin
          if ANode.IsGroupNode then
            SetFocusedNodeItem(FocusedNode,
              TreeList.OptionsView.GetCategorizedColumn)
          else
...
begin
  ...
  if not Result then
    Exit;
  HitTest.HitPoint := Point(X, Y);
  Result := not HitTest.CanSizing and not TreeList.IsDesigning and
    not IsEditing and (HitTest.HitAtNode and HitTest.HitNode.Selected and
    not HitTest.HitAtFooterArea);
  if (HitTest.HitAtIndent and not HitTest.HitAtImages) or not HitTest.CanStartDrag
  then
    Result := False;
  if Result then
    TreeList.FDragNode := HitTest.HitNode;
...
function TcxTreeListController.DoHeaderMouseDown(Button: TMouseButton;
...
begin
  ...
  begin
    if HitTest.HitAtHeader and not HitTest.CanSizing and not(ssDouble in Shift)
    ...
    if HitTest.HitAtHeader and not HitTest.CanSizing and
      (ClickedObject = TcxTreeListHeaderCellViewInfo(HitTest.HitTestItem).Item)
    then
    begin
      TcxTreeListCustomHeaderCellViewInfo(HitTest.HitTestItem).Pressed := True;
      Result := True;
    end
    else if HitTest.HitAtColumnHeaderFilterButton then
    begin
      TcxTreeListHeaderCellViewInfo(HitTest.HitTestItem)
        .UpdateFilterButtonStateOnMouseDown;
      Result := True;
    end;
  end;
end;

function TcxTreeListController.DoNodeMouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer): Boolean;
begin
  Result := False;
  ...
  else if HitTest.HitAtButton and (Button = mbLeft) then
  begin
    if (HitTest.HitNode.Expanded and TreeList.InternalCollapseNode
      (HitTest.HitNode, False)) or (not HitTest.HitNode.Expanded and
      TreeList.InternalExpandNode(HitTest.HitNode, False)) then
      Exit;
...
procedure TcxTreeListController.DoMouseDown(Button: TMouseButton;
...
begin
  ...
  if HitTest.HitAtNavigator then
    Exit;
  ...
  AIsHotTrack := HitTest.HitAtColumn and not HitTest.HitAtNodePreview and
    HitTest.IsItemEditCell and TcxTreeListEditCellViewInfo(HitTest.HitTestItem)
    .IsEditHotTrack(Point(X, Y));
  if not DoHeaderMouseDown(Button, Shift, X, Y) and not HitTest.CanSizing then
    DoNodeMouseDown(Button, Shift, X, Y);
  WasFocusedBeforeClick := HitTest.HitAtColumn and
    not HitTest.HitAtNodePreview and HitTest.HitAtNode and
    HitTest.HitNode.Focused and (HitTest.HitNode = APrevFocusedNode) and
...
procedure TcxTreeListController.DoMouseMove(Shift: TShiftState; X, Y: Integer);
begin
  inherited DoMouseMove(Shift, X, Y);
  UpdateHotTrackNode(X, Y, Shift);
end;

procedure TcxTreeListController.DoMouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  ...
  HitTest.RecalculateOnMouseEvent(X, Y, Shift);
  if not IsEditing and HitTest.HitAtNode and
    (HitTest.HitNode = TreeList.FocusedNode) and (Button = mbLeft) then
  begin
    if ([ssCtrl, ssShift, ssLeft] * Shift = []) and CanClearSelection and
      (Selection.Count <> 1) then
      CancelSelection();
    if (OptionsBehavior.ImmediateEditor and HitTest.HitAtColumn and
      HitTest.HitAtNode) or OptionsBehavior.AlwaysShowEditor then
      ShowEdit(Shift, X, Y)
    else if WasFocusedBeforeClick and not IsDblClick then
...
var
  Var1: TcxTreeListNode;
begin
  if (FocusedNode = nil) and (FocusedNodeIndex >= 0) then
  begin
    Var1 :=
      FindNearestFocusableNode(FocusedNodeIndex);
    if Var1 <> nil then
      Var1.Focused := True;
  end;
end;
...
begin
  if not HitTest.HitAtColumnHeader or HitTest.CanSizing or
    ...
    not OptionsBehavior.Sorting then
    Exit;
...
begin
  ...
  begin
    ...
    try
      ...
      if HitTest.HitAtColumn then
        FocusedItem := FindNearestFocusableColumn
          (HitTest.HitColumn.VisibleIndex);
      if FocusedItem = nil then
        FocusedItem := FindNearestFocusableColumn(0);
    finally
  ...
  else if HitTest.HitAtBackground then
    CancelSelection;
end;
...
function TcxTreeListController.CanInsertNode
  (AIsAppend: Boolean = False): Boolean;
begin
  Result := (not AIsAppend and TreeList.OptionsData.Inserting) or
    (AIsAppend and TreeList.OptionsData.Appending);
  if not Result then
    Exit;
...
function TcxTreeListController.CanSelectNode(ANode: TcxTreeListNode): Boolean;
begin
...
begin
  ...
  if Result and TcxTreeListIndicatorCellViewInfo(HitTest.HitTestItem)
    .IsQuickCustomizationEnabled and not(ColumnsCustomizationPopup.JustClosed or
    BandsCustomizationPopup.JustClosed) then
...
begin
  HitTest.Recalculate;
  if IsEditing and (FocusedNode <> nil) and FocusedNode.HasChildren and not GetAlwaysShowEditor
  then
  begin
  ...
  if HitTest.HitAtNode and not(HitTest.HitAtButton or HitTest.HitAtFooterArea or
    HitTest.HitAtCheckButton or IsEditing) and OptionsBehavior.ExpandOnDblClick
  then
  begin
    HitTest.HitNode.Expanded := not HitTest.HitNode.Expanded;
    SkipButtonDown := True;
...
function TcxTreeListController.FindNearestFocusableNode(AVisibleIndex: Integer)
  : TcxTreeListNode;
begin
  Result := nil;
  if TreeList.AbsoluteVisibleCount <= 0 then
    Exit;
  AVisibleIndex := Max(0, Min(TreeList.AbsoluteVisibleCount - 1,
    AVisibleIndex));
...
begin
  ...
  begin
    ...
    if (HScrollPos <> TreeList.ViewInfo.HScrollPos) or
      (VScrollPos <> TreeList.ViewInfo.VScrollPos) then
      TreeList.Invalidate
    else
      TreeList.ViewInfo.UpdateSelection;
    if not ACancel then
...
function TcxTreeListController.GetDragAndDropObjectClass
...
begin
  ...
  else if HitTest.CanMoving then
...
begin
  Result := HitTest.HitAtNode and
    (HitTest.HitAtIndicator or HitTest.HitAtImages or HitTest.HitAtColumn or
    HitTest.HitAtBand or HitTest.HitAtNodePreview) and
    not HitTest.HitAtFooterArea;
end;

procedure TcxTreeListController.KeyDown(var Key: Word; Shift: TShiftState);
var
  AKey: Word;
...
begin
  ...
  try
    ...
    if (TreeList.VisibleColumnCount = 1) and (FocusedNode <> nil) and
      (FocusedNode.HasChildren) and
      (((Key = VK_LEFT) and FocusedNode.Expanded) or
      ((Key = VK_RIGHT) and not FocusedNode.Expanded)) then
    begin
    ...
    case AKey of
      VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT, VK_NEXT, VK_PRIOR, VK_HOME,
        ...
        begin
          ...
          else if (FocusedNode <> nil) and not FocusedNode.Selected or
            (TreeList.OptionsSelection.MultiSelect and (Selection.Count > 1))
          then
            Select(FocusedNode, Shift);
        end;
...
begin
  if TreeList.IsLocked or (AScrollCode = scEndScroll) then
    Exit;
...
begin
  if not CanCreateSelectionHelper then
    Exit;
...
begin
  ...
  begin
    if (ARecordIndex <> 0) and ((EditingController.EditingItem <> AItem) or
      not TcxTreeListNode(ARecordIndex).Focused) then
    begin
...
procedure TcxTreeListController.SetHotTrackNode(ANode: TcxTreeListNode;
  AShift: TShiftState);
var
  ACursor: TCursor;
  APrevNode: TcxTreeListNode;
begin
  ...
  if (TreeList.IsDestroying or not TreeList.OptionsBehavior.HotTrack) or
    ((ANode = HotTrackNode) and (HotTrackColumn = HitTest.HitColumn)) then
    Exit;
...
procedure TcxTreeListController.UpdateHotTrackNode(X, Y: Integer;
  AShift: TShiftState);
begin
...
function TcxTreeListController.GetFocusedCellViewInfo(AEditContainer
  : TcxCustomInplaceEditContainer): TcxEditCellViewInfo;
begin
  Result := nil;
  if (AEditContainer = nil) or TreeList.ViewInfo.IsDirty then
    Exit;
  Result := TreeList.ViewInfo.GetEditCell(FocusedNode,
    TcxTreeListColumn(AEditContainer));
end;
...
begin
  if TreeList.IsDestroying or TreeList.IsLoading or (IncSearchNode = nil) or SearchNotification
  then
    Exit;
  IncSearchText := '';
  IncSearchNode := nil;
...
function TcxTreeListController.GetIncSearchingItem
  : TcxCustomInplaceEditContainer;
begin
...
function TcxTreeListController.GetNextNodeForIncSearch(ANode: TcxTreeListNode;
  AGoForward: Boolean): TcxTreeListNode;
begin
  Result := nil;
  ...
  else if cxInRange(ANode.VisibleIndex + cxIntOffs[AGoForward], 0,
    TreeList.AbsoluteVisibleCount - 1) then
    Result := TreeList.AbsoluteVisibleItems
      [ANode.VisibleIndex + cxIntOffs[AGoForward]];
end;
...
begin
  ...
  if (AItem = nil) or (ANode = nil) then
    Exit;
  IncSearchText := AValue;
  CheckLocate(DoSearch(ANode, TcxTreeListColumn(AItem).ItemIndex,
    IncSearchText, True));
end;
...
var
  ANode: TcxTreeListNode;
begin
  if (AItem = nil) or (TreeList.FocusedNode = nil) then
    Exit;
  ANode := GetNextNodeForIncSearch(TreeList.FocusedNode, AForward);
  if ANode = nil then
    ANode := TreeList.FocusedNode;
  CheckLocate(DoSearch(ANode, TcxTreeListColumn(AItem).ItemIndex, IncSearchText,
    AForward));
end;

function TcxTreeListController.CanCreateSelectionHelper: Boolean;
begin
  Result := TreeList.IsDesigning and not TreeList.IsDestroying and
    not TreeList.IsLoading and (cxDesignSelectionHelperClass <> nil) and
    TreeList.HandleAllocated;
end;
...
procedure TcxTreeListController.SelectObject(AObject: TPersistent;
  AShift: TShiftState);
var
  AHelper: TcxCustomDesignSelectionHelper;
begin
...
procedure TcxTreeListController.Select(ANode: TcxTreeListNode;
  AShift: TShiftState);
begin
  ...
  try
    BeginUpdateSelection;
    if (FSelectionAnchor = nil) and OptionsSelection.MultiSelect and
      (ssShift in AShift) then
      FSelectionAnchor := FocusedNode;
    if ANode = nil then
    begin
    ...
    if not OptionsSelection.MultiSelect or
      (not(ssShift in AShift) and ((ssCtrl in AShift) or not ANode.Selected))
    then
      SelectionAnchor := ANode;
    if not ANode.Selected and (not OptionsSelection.MultiSelect or
      ([ssShift, ssCtrl] * AShift = [])) then
    begin
      Selection.Clear;
      AddNodeToSelection(ANode);
    end
    else if [ssCtrl, ssShift] * AShift = [ssCtrl] then
    begin
    ...
    else if (ssLeft in AShift) and ANode.Selected then
    begin
...
procedure TcxTreeListController.SelectRange(const AStartNode,
  AFinishNode: TcxTreeListNode);
var
  I: Integer;
...
begin
  ...
  if PressedHeader <> nil then
    PressedHeader.Pressed := False;
  if not TreeList.HandleAllocated then
    Exit;
...
function TcxTreeListController.GetBandsCustomizationPopup
  : TcxTreeListBandsCustomizationPopup;
begin
  if FBandsCustomizationPopup = nil then
    FBandsCustomizationPopup := TcxTreeListBandsCustomizationPopup.Create
      (TreeList);
  Result := FBandsCustomizationPopup;
end;

function TcxTreeListController.GetColumnsCustomizationPopup
  : TcxTreeListColumnsCustomizationPopup;
begin
  if FColumnsCustomizationPopup = nil then
    FColumnsCustomizationPopup := TcxTreeListColumnsCustomizationPopup.Create
      (TreeList);
...
begin
  if HitTest.HitAtNodePreview or HitTest.HitAtColumn then
    Result := ndiAddChild
  else if Y < cxRectCenter(HitTest.HitNode.ViewData.GetRealBounds).Y then
    Result := ndiInsertBefore
  else
    Result := ndiInsertAfter;
end;

function TcxTreeListController.GetEditingController
  : TcxTreeListEditingController;
begin
...
procedure TcxTreeListController.CheckKeyAutoRepeat(ACharCode: Word;
  ARepeatCount: Cardinal);
begin
  ...
  begin
    ...
    begin
      if GetResizeDirection = dsdHorz then
        FOffset.X := TcxTreeListCustomCellViewInfo(DragItem).DisplayRect.Right -
          CurMousePos.X
      else
        FOffset.Y := TcxTreeListCustomCellViewInfo(DragItem).DisplayRect.Bottom
          - CurMousePos.Y
    end
    else if DragItem is TcxTreeListNode then
...
procedure TcxTreeListSizingDragAndDropObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
begin
...
function TcxTreeListFindPanel.GetItemCaption
  (AItem: TcxFindPanelItemKind): string;
begin
  case AItem of
    fpikFindButton:
...
begin
  if TreeList.AbsoluteVisibleCount = 0 then
    Exit;
  ANode := TreeList.AbsoluteVisibleItems[0];
...
begin
  ...
  if Result or (DestBand = nil) or (DestBand = Band) then
    Exit;
    ...
    case Band.FixedKind of
      tlbfLeft:
        Result := (ActualDropPosition = posLeft) and
          (DestBand.VisibleRootIndex = Bands.GetFirstVisibleRootIndex
          (tlbfNone));
      tlbfNone:
        Result := (ActualDropPosition = posRight) and
          (DestBand.VisibleRootIndex = Bands.GetLastVisibleRootIndex(tlbfLeft))
          or (ActualDropPosition = posLeft) and
          (DestBand.VisibleRootIndex = Bands.GetFirstVisibleRootIndex
          (tlbfRight));
      tlbfRight:
        Result := (ActualDropPosition = posRight) and
  ...
  if not Result or not Band.Visible then
    Exit;
  case DropInfo.Position of
    posLeft, posRight:
      Result := (DestBand.ParentBand <> Band.ParentBand) or
        (DestBand.Position.VisibleColIndex <> GetNearestColIndex
        (ActualDropPosition = posLeft));
    posTop:
      Result := (DestBand.ParentBand <> Band) or
        (Band.FChildVisibleBands.Count > 1);
    posBottom:
      Result := (Band.ParentBand <> DestBand) or
        (DestBand.FChildVisibleBands.Count > 1);
  end;
end;

function TcxTreeListDragAndDropBandObject.CanRemove: Boolean;
begin
  Result := CanCustomize and
    (Customizing or TreeList.OptionsCustomizing.BandHiding) and
    not Band.Options.Hidden and
    (not Band.IsRoot or (Bands.VisibleItemCount > 1));
end;
...
begin
  ...
  begin
    DropInfo.Band := ABand;
    DropInfo.Position := GetPointPosition(ABand.HeaderCell.GetClipRect,
      DropInfo.DropPos, True, TreeList.OptionsCustomizing.NestedBands);
  end
  else if TreeList.OptionsCustomizing.NestedBands and not ABand.IsBottom then
  begin
...
begin
  ...
  for I := 0 to TreeList.Bands.VisibleRootItemCount - 1 do
    if CheckBandInsertAt(TreeList.Bands.VisibleRootItems[I]) then
      Break;
end;
...
begin
  ...
  else if (DestBand = nil) and CanRemove then
    Band.Visible := False
  else if Bands.VisibleItemCount = 0 then
  begin
      ...
      begin
        ...
        while I < DestBand.ChildVisibleBands.Count do
          if DestBand.ChildVisibleBands[I] <> Band then
            TcxTreeListBand(DestBand.ChildVisibleBands[I]).Position.BandIndex :=
              Band.Index
          else
            Inc(I);
      end;
...
begin
  ...
  begin
    ...
    if ActualDropPosition = posRight then
      Inc(Result);
    if Band.Position.ColIndex < Result then
      Dec(Result);
  end;
end;
...
begin
  Result := Column.Options.Customizing and OptionsCustomizing.ColumnCustomizing;
end;
...
begin
  Result := CanCustomize and (Customizing or OptionsCustomizing.ColumnHiding)
    and not Column.Options.Hidden and (TreeList.VisibleColumnCount > 1);
end;
...
begin
  DropInfo.Area := cxRectSetTop(ABand.HeaderCell.GetClipRect,
    ABand.HeaderCell.DisplayRect.Bottom, ViewInfo.HeadersHeight +
    cxTreeListHeaderMovingZoneSize);
  Result := PtInRect(DropInfo.Area, DropInfo.DropPos) and
    ((ABand <> Column.Position.Band) or not Column.Visible or
    (ABand.VisibleColumnCount > 1));
  if not Result then
    Exit;
  DropInfo.Band := ABand;
  ...
  begin
    ...
    begin
      ...
      begin
        Inc(DropInfo.Area.Bottom, ABand.BandRows.VisibleItems[I].LineCount *
          ViewInfo.HeaderLineHeight);
...
begin
  ...
  begin
    DropInfo.Position := HorzPos
      [DropInfo.DropPos.X <= cxRectCenter(DropInfo.Area).X];
    if (ActualDropPosition = posRight) then
...
begin
  ...
  else if (DestBand <> nil) and (TreeList.OptionsCustomizing.StackedColumns or
...
function TcxTreeListDragAndDropColumnObject.GetDropInfo
...
begin
  ...
  OrgOffset := FOrigin;
  FArrows := TcxPlaceArrows.CreateArrows(TreeList.OptionsView.DropArrowColor,
    clBtnText);
...
const
  Position2Border: array [TcxPosition] of TcxBorder = (bLeft, bLeft, bRight,
    bTop, bBottom);
begin
...
procedure TcxTreeListDragAndDropObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
var
  APos: TPoint;
begin
  ...
  Accepted := DropInfo.Customizing or (DropInfo.Band <> nil) or
    (Bands.VisibleItemCount = 0);
...
begin
  ...
  try
    DropInfo.Accepted := Accepted and
      (DropInfo.Accepted or DropInfo.Customizing or CanRemove) and
      not TreeList.Controller.DragCancel;
...
begin
  if cxRectIsNull(Area) then
    Exit;
  AddAutoScrollingObject(cxRectSetLeft(Area, Area.Left, cxtlScrollDelta),
    sbHorizontal, TreeList.Controller.GetRealScrollCode(scLineUp));
  AddAutoScrollingObject(cxRectSetRight(Area, Area.Right, cxtlScrollDelta),
    sbHorizontal, TreeList.Controller.GetRealScrollCode(scLineDown));
end;
...
function TcxTreeListDragAndDropObject.GetDragHeader
  : TcxTreeListHeaderCellViewInfo;
begin
...
function TcxTreeListDragAndDropObject.GetOptionsCustomize
  : TcxTreeListOptionsCustomizing;
begin
  Result := TreeList.OptionsCustomizing;
end;
...
function TcxTreeListDataController.CompareNodesByColumns(ANode1,
  ANode2: TcxTreeListNode; AColumns: TList): Integer;
var
  I: Integer;
  V1, V2: Variant;
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := Compare(AColumn.ValueDef, GetNodeHandle(ANode1),
        GetNodeHandle(ANode2));
...
function TcxTreeListDataController.GetDisplayText(ARecordIndex,
  AItemIndex: Integer): string;
begin
  Result := GetNodeByRecordIndex(ARecordIndex).Texts[AItemIndex]
end;

function TcxTreeListDataController.GetFilterDataValue(ARecordIndex: Integer;
  AField: TcxCustomDataField): Variant;
begin
  Result := GetNodeByFilterRecordIndex(ARecordIndex).Values[AField.Index];
end;

function TcxTreeListDataController.GetNodeDisplayText(ANode: TcxTreeListNode;
  AIndex: Integer): Variant;
begin
  if ANode.Handle = nil then
    Result := ''
  else if IsExpression(AIndex) then
    Result := GetNodeExpressionDisplayText(ANode, AIndex)
  else
    Result := GetNodeStoredDisplayText(ANode, AIndex);
end;

function TcxTreeListDataController.GetNodeValue(ANode: TcxTreeListNode;
  AIndex: Integer): Variant;
begin
  if ANode.Handle = nil then
    Result := Null
...
function TcxTreeListDataController.GetValue(ARecordIndex,
  AItemIndex: Integer): Variant;
var
  ANode: TcxTreeListNode;
begin
...
function TcxTreeListDataController.GetValueForCompare(ANode: TcxTreeListNode;
  AField: TcxCustomDataField): Variant;
begin
...
procedure TcxTreeListDataController.InitializeNodeFromRecordIndex
  (ANode: TcxTreeListNode);
begin
  ANode.Handle := TcxDataStorageHelper.RemoveRecord(DataStorage,
    Integer(ANode.Handle));
  if ANode.Handle <> nil then
    Inc(FAllocatedRecords);
end;
...
procedure TcxTreeListDataController.InitializeRecordIndexFromNode
  (ANode: TcxTreeListNode);
begin
  ANode.Handle := Pointer(TcxDataStorageHelper.AppendRecord(DataStorage,
    ANode.Handle));
end;
...
begin
  if not TreeList.OptionsData.Editing then
    Exit;
...
begin
  if (EditingNode = nil) or (Root.Handle = nil) or
    (nsDeleting in EditingNode.State) then
    Exit;
  TreeList.UpdateData;
...
procedure TcxTreeListDataController.SetNodeValue(ANode: TcxTreeListNode;
  AIndex: Integer; const AValue: Variant);
begin
  if (ANode = nil) or IsExpression(AIndex) then
    Exit;
  if ANode.Handle = nil then
    ANode.InitializeHandle;
  if ANode.Handle = nil then
    Exit;
  if ValueDefs[AIndex] = nil then
    Exit;
...
procedure TcxTreeListDataController.SetValue(ARecordIndex, AItemIndex: Integer;
  const Value: Variant);
begin
  cxTLUnused;
end;
...
begin
  if EditingNode = nil then
    Exit;
  try
    if not EditingNode.Deleting and not EditingNode.Inserting and IsValueChanged
    then
...
procedure TcxTreeListDataController.ClearFieldValueDef
  (AField: TcxCustomDataField);
begin
  RemoveValueDef(GetValueDef(AField));
  TcxCustomDataHelper.SetTextStored(AField, not AField.IsInternal and
    TcxTreeListColumn(AField.Item).GetIsTextStored);
end;

procedure TcxTreeListDataController.ClearFieldValues
  (AField: TcxCustomDataField);
begin
  ForEachNode(DoClearNodeValueProc, AField);
end;

procedure TcxTreeListDataController.ClearNodeExpressionFieldValues
  (ANode: TcxTreeListNode);
begin
  ClearExpressionFieldValues(ANode.FAbsoluteIndex)
end;

procedure TcxTreeListDataController.ClearNodeValue(ANode: TcxTreeListNode;
  AField: TcxCustomDataField);
var
...
procedure TcxTreeListDataController.ClearValue(ARecordIndex: Integer;
  AField: TcxCustomDataField);
begin
...
function TcxTreeListDataController.Compare(AValueDef: TcxValueDef;
  AHandle1, AHandle2: Pointer): Integer;
begin
...
begin
  ...
  for I := 0 to TreeList.ColumnCount - 1 do
  begin
    TcxDataStorageHelper(DataStorage).SetValue(Result, ValueDefs[I],
      GetNodeValue(ASource, I));
  end;
end;
...
begin
  if (ANode.FAbsoluteIndex >= 0) and
    (ANode.FAbsoluteIndex < TreeList.FAbsoluteItems.Count) then
...
function TcxTreeListDataController.DoFilterRecordEvent
  (ARecordIndex: Integer): Boolean;
begin
  Result := TreeList.DoFilterNodeEvent
    (GetNodeByFilterRecordIndex(ARecordIndex));
end;
...
function TcxTreeListDataController.GetErrorCode(ARecordIndex,
  AItemIndex: Integer): Integer;
begin
  Result := GetInternalErrorCode(ARecordIndex, AItemIndex);
end;

function TcxTreeListDataController.GetFilteredIndexByRecordIndex
...
function TcxTreeListDataController.GetFilteredRecordIndex
...
function TcxTreeListDataController.GetFilteringRecordCount
  (AUseFiltered: Boolean): Integer;
begin
...
function TcxTreeListDataController.GetFilterDisplayText(ARecordIndex: Integer;
  AItemIndex: Integer): string;
begin
  Result := GetNodeByFilterRecordIndex(ARecordIndex).Texts[AItemIndex];
end;

function TcxTreeListDataController.GetFilterRecordIndexByFilteringRecordIndex
  (AUseFiltered: Boolean; AIndex: Integer): Integer;
var
  ARecordIndex: Integer;
  ANode: TcxTreeListNode;
begin
  ARecordIndex := inherited GetFilterRecordIndexByFilteringRecordIndex
    (AUseFiltered, AIndex);
  ANode := GetNodeByRecordIndex(ARecordIndex);
  Result := GetFilterRecordIndexByNode(ANode);
end;

function TcxTreeListDataController.GetFilterRecordIndexByNode
  (ANode: TcxTreeListNode): Integer;
begin
  Result := ANode.FAbsoluteIndex;
end;

function TcxTreeListDataController.GetInternalValue(ARecordIndex: Integer;
  AField: TcxCustomDataField): Variant;
begin
  Result := GetNodeByRecordIndex(ARecordIndex).Values[AField.Index];
end;

function TcxTreeListDataController.GetNodeByFilterRecordIndex
  (ARecordIndex: Integer): TcxTreeListNode;
begin
  Result := GetNodeByRecordIndex(ARecordIndex);
end;

function TcxTreeListDataController.GetNodeByRecordIndex(ARecordIndex: Integer)
  : TcxTreeListNode;
begin
  Result := TreeList.FAbsoluteItems[ARecordIndex];
end;

function TcxTreeListDataController.GetNodeErrorCode(ANode: TcxTreeListNode;
...
function TcxTreeListDataController.GetNodeExpressionDisplayText
  (ANode: TcxTreeListNode; AIndex: Integer): string;
begin
  Result := GetExpressionFieldDisplayText(GetRecordIndexByNode(ANode),
    Fields[AIndex]);
end;

function TcxTreeListDataController.GetNodeExpressionValue
  (ANode: TcxTreeListNode; AIndex: Integer): Variant;
begin
  Result := GetExpressionFieldValue(GetRecordIndexByNode(ANode),
    Fields[AIndex]);
end;

function TcxTreeListDataController.GetNodeStoredDisplayText
  (ANode: TcxTreeListNode; AIndex: Integer): string;
var
  AValueDef: TcxValueDef;
begin
...
function TcxTreeListDataController.GetNodeStoredValue(ANode: TcxTreeListNode;
  AIndex: Integer): Variant;
var
  AValueDef: TcxValueDef;
begin
...
function TcxTreeListDataController.GetRecordIndexByNode
  (ANode: TcxTreeListNode): Integer;
begin
  Result := ANode.AbsoluteIndex;
end;

function TcxTreeListDataController.GetStoredDisplayText(ARecordIndex: Integer;
  AField: TcxCustomDataField): string;
begin
  Result := GetNodeStoredDisplayText(GetNodeByRecordIndex(ARecordIndex),
    AField.Index);
end;

function TcxTreeListDataController.GetStoredErrorCode(ARecordIndex: Integer;
...
begin
  ...
  if AValueDef <> nil then
    Result := TcxDataStorageHelper.GetErrorCode(AValueDef,
      GetNodeByRecordIndex(ARecordIndex).Handle)
  else
    Result := 0;
end;

function TcxTreeListDataController.GetStoredValue(ARecordIndex: Integer;
  AField: TcxCustomDataField): Variant;
begin
  Result := GetNodeStoredValue(GetNodeByRecordIndex(ARecordIndex),
    AField.Index);
end;
...
procedure TcxTreeListDataController.InitFieldValueDef
  (AField: TcxCustomDataField);
begin
  InsertValueDef(AField);
end;

function TcxTreeListDataController.IsHiddenByFilter
  (ANode: TcxTreeListNode): Boolean;
begin
  Result := not ANode.IsRoot and not DoGlobalFilterRecord
    (GetFilterRecordIndexByNode(ANode));
end;

function TcxTreeListDataController.IsStoredValueEmpty(ARecordIndex: Integer;
  AField: TcxCustomDataField): Boolean;
var
  AValueDef: TcxValueDef;
begin
  AValueDef := TcxCustomDataHelper.GetValueDef(AField);
  Result := (AValueDef = nil) or TcxDataStorageHelper.IsValueEmpty(AValueDef,
    GetNodeByRecordIndex(ARecordIndex).Handle);
end;
...
procedure TcxTreeListDataController.SetNodeDisplayText(ANode: TcxTreeListNode;
...
procedure TcxTreeListDataController.SetNodeStoredValue(ANode: TcxTreeListNode;
  AIndex: Integer; const Value: Variant);
var
  AValueDef: TcxValueDef;
begin
  AValueDef := GetValueDef(AIndex);
...
procedure TcxTreeListDataController.SetStoredErrorCode(ARecordIndex: Integer;
  AField: TcxCustomDataField; ACode: Integer);
var
  AValueDef: TcxValueDef;
begin
  AValueDef := TcxCustomDataHelper.GetValueDef(AField);
  if AValueDef <> nil then
    TcxDataStorageHelper.SetErrorCode(GetNodeByRecordIndex(ARecordIndex).Handle,
      AValueDef, ACode);
end;

procedure TcxTreeListDataController.SetStoredValue(ARecordIndex: Integer;
  AField: TcxCustomDataField; const Value: Variant);
begin
  SetNodeStoredValue(GetNodeByRecordIndex(ARecordIndex), AField.Index, Value);
end;

function TcxTreeListDataController.InternalCheckBookmark(ADeletedRecordIndex
  : Integer): Boolean;
begin
  Result := False;
end;
...
procedure TcxTreeListDataController.SetNodeData(ASource,
...
function TcxTreeListDataController.StringCompare(const V1, V2: Variant)
  : Integer;
begin
...
procedure TcxTreeListDataController.DoClearNodeValueProc(ANode: TcxTreeListNode;
  Data: Pointer);
begin
  ClearNodeValue(ANode, Data);
end;

procedure TcxTreeListDataController.DoFilterNodeProc(ANode: TcxTreeListNode;
  Data: Pointer);
begin
...
function TcxTreeListDataController.GetValueDef(AField: TcxCustomDataField)
  : TcxValueDef;
begin
  Result := TcxCustomDataHelper.GetValueDef(AField);
end;

procedure TcxTreeListDataController.FreeRecordProc(ANode: TcxTreeListNode;
  AData: Pointer);
begin
...
procedure TcxTreeListDataController.InsertValueDefProc(ANode: TcxTreeListNode;
  AData: Pointer);
var
  PSource: PAnsiChar;
begin
  if ANode.FHandle = nil then
    Exit;
  ReallocMem(ANode.FHandle, PcxTreeListValueDefData(AData)
    ^.ValueDef.ValueDefs.RecordSize);
  PSource := PAnsiChar(ANode.FHandle) + PcxTreeListValueDefData(AData)
    ^.ValueDef.Offset;
...
procedure TcxTreeListDataController.RemoveValueDefProc(ANode: TcxTreeListNode;
  AData: Pointer);
var
  AFreeRecord: Boolean;
  ARecordSize: Integer;
...
begin
  if (ANode.FHandle = nil) or (PcxTreeListValueDefData(AData)^.ValueDef = nil)
  then
    Exit;
  ...
  if AFreeRecord then
    ...
    begin
      ...
      System.Move(PSource^, PDest^,
        ARecordSize - (PSource - PAnsiChar(ANode.FHandle)));
      ReallocMem(ANode.FHandle, ARecordSize - ValueDef.BufferSize);
    end;
end;

procedure TcxTreeListDataController.ResetNodeFilteringProc
...
procedure TcxTreeListNode.AssignValues(const AValues: array of Variant);
var
  I, K: Integer;
begin
  K := 0;
...
function TcxTreeListNode.CanMove(ADest: TcxTreeListNode;
  AMode: TcxTreeListNodeAttachMode): Boolean;
begin
  if (ADest = nil) or
    ((ADest = Self) and (AMode in [tlamAddChild, tlamAddChildFirst, tlamInsert])
    ) or ADest.HasAsParent(Self) or ADest.Deleting then
    Result := False
  else
    Result := True;
end;
...
const
  ANewState: array [Boolean, TcxCheckBoxState] of TcxCheckBoxState =
    ((cbsChecked, cbsUnchecked, cbsChecked), (cbsChecked, cbsGrayed,
    cbsUnchecked));
begin
  if not Enabled or not HasCheckbox or
    ((Parent.CheckGroupType = ncgRadioGroup) and Checked) then
    Exit;
...
begin
  ...
  TreeList.BeginUpdate;
  try
    if TreeList.InternalCollapseNode(Self, Recurse) then
      Exit;
    Expanded := False;
    if Recurse then
    begin
...
procedure TcxTreeListNode.CustomSort(ASortProc: TcxTreeListCompareFunc;
  ARecurse: Boolean = False);
var
  I: Integer;
  AList: TdxFastList;
...
begin
  if not Assigned(ASortProc) and not TreeList.CanCompare and
    not TreeList.Sorted and not(tcSortOrder in TreeList.Changes) then
    Exit;
  TreeList.BeginUpdate;
  try
...
var
  ANode: TcxTreeListNode;
begin
  if not HasChildren or TreeList.IsCancelOperation then
    Exit;
  TreeList.BeginUpdate;
  try
    if TreeList.InternalExpandNode(Self, Recurse) then
      Exit;
    Expanded := True;
    if Recurse then
    begin
...
function TcxTreeListNode.InsertChild(ABeforeNode: TcxTreeListNode)
  : TcxTreeListNode;
begin
...
begin
  if (Count <> 0) or not HasChildren or Expanded or TreeList.IsDestroying then
    Exit;
  TreeList.BeginUpdate;
  try
...
procedure TcxTreeListNode.MoveTo(ADestNode: TcxTreeListNode;
  AMode: TcxTreeListNodeAttachMode);
begin
  if (ADestNode = nil) or (ADestNode = Self) then
    Exit;
...
begin
  if (FViewData = nil) or Deleting then
    Exit;
  ViewData.Update(True);
end;

function TcxTreeListNode.GetNextSiblingEx(ANode: TcxTreeListNode;
  AForward: Boolean): TcxTreeListNode;
begin
  if AForward then
    Result := ANode.FNext
  else
...
begin
  ...
  if (ASource.Parent <> nil) and (Parent <> nil) then
    Parent.CheckInfo := Parent.CheckInfo + ASource.Parent.CheckInfo *
...
begin
  cxError(not cxInRange(AIndex, 0, FooterSummaryCount - 1), SListIndexError,
    [AIndex]);
...
begin
  Result := (FVisibleIndex >= 0) and
    (FVisibleIndex = TreeList.AbsoluteVisibleCount - 1);
end;
...
procedure TcxTreeListNode.SetChildrenCheckState(AValue: TcxCheckBoxState;
  AExclude: TcxTreeListNode);
var
  ANode: TcxTreeListNode;
begin
...
begin
  ...
  if not(nsCheckStateInvalid in State) then
    Exit;
  State := State - [nsCheckStateInvalid];
  AState := CheckState;
  if CheckGroupType = ncgNone then
  ...
  begin
    ...
    while AChild <> nil do
    begin
      if AChild.Visible and not AChild.HiddenByFilter and
        (AState <> AChild.CheckState) then
      begin
        AState := cbsGrayed;
        Break;
      end;
...
function TcxTreeListNode.CanSizing(ADirection: TcxDragSizingDirection): Boolean;
begin
  with TreeList do
  begin
...
function TcxTreeListNode.GetSizingBoundsRect(ADirection
  : TcxDragSizingDirection): TRect;
begin
  if ViewData = nil then
    Result := cxEmptyRect
  else
  begin
    Result := cxRectSetTop(TreeList.ClientBounds, FViewData.Origin.Y +
      TreeList.ViewInfo.ColumnsRowCount * (TreeList.ViewInfo.DefaultCellHeight +
      cxTextOffset * 2) + TreeList.ViewInfo.GroupFooterHeight *
      ViewData.GroupFooterCount, TreeList.ClientHeight);
  end;
end;

function TcxTreeListNode.GetSizingIncrement(ADirection
  : TcxDragSizingDirection): Integer;
begin
  Result := 1;
end;
...
procedure TcxTreeListNode.SetSizeDelta(ADirection: TcxDragSizingDirection;
  ADelta: Integer);
begin
  if ADelta = 0 then
    Exit;
  ...
  if TreeList.OptionsCustomizing.RowSizing then
    TreeList.DefaultRowHeight := Max(TreeList.ViewInfo.DefaultCellHeight,
      Max(TreeList.ViewInfo.DefaultCellHeight,
      TreeList.DefaultRowHeight) + ADelta)
  else
  begin
    if Height = 0 then
      Height := TreeList.ViewInfo.DefaultRowHeight *
        TreeList.ViewInfo.ColumnsRowCount;
    Height := Max(TreeList.ViewInfo.DefaultCellHeight *
      TreeList.ViewInfo.ColumnsRowCount, Height + ADelta);
  end;
  TreeList.Modified;
end;
...
begin
  ...
  while ANode <> nil do
  begin
    if ANode.Visible and not ANode.HiddenByFilter and
      (ANode.CheckState = cbsChecked) then
      Inc(Result);
    ANode := ANode.FNext;
  end;
end;
...
begin
  ...
  else if nciRadioGroup in CheckInfo then
...
begin
  ...
  if nciGrayed in CheckInfo then
    Result := cbsGrayed
  else if nciChecked in CheckInfo then
    Result := cbsChecked;
end;
...
begin
  ...
  if not HasChildren or (Count = 0) then
    Exit;
  ANode := FFirst;
  while not Result and (ANode <> nil) do
  begin
...
begin
  Result := FParent <> nil;
  ANode := Self;
  while Result and (ANode.FParent <> nil) and
    (ANode.FParent <> TreeList.Root) do
  begin
...
begin
  Result := TreeList.DoGetNodeImageIndex(Self,
    TcxTreeListImageIndexType(AIndex));
end;
...
var
  ...
  ANode: TcxTreeListNode;
begin
  if nsValidIndexes in State then
    Exit;
  AIndex := 0;
  ANode := FFirst;
  while ANode <> nil do
...
const
  ...
  AParentCheckState: array [Boolean] of TcxCheckBoxState = (cbsGrayed,
    cbsChecked);
begin
  ...
  CheckInfo := CheckInfo - [nciChecked, nciGrayed] + AState[AValue] +
    [nciChangeCheck];
  try
    if (CheckState in [cbsChecked, cbsUnchecked]) and HasChildren then
    begin
      LoadChildren;
      if not IsRadioGroup or (CheckState = cbsUnchecked) or (CheckedCount = 0)
      then
    ...
    begin
      ...
      begin
        ...
        if (ACheckedCount = 0) and (Parent.GrayedCount = 0) then
          ...
          Parent.CheckState := AParentCheckState
            [(ACheckedCount = Parent.ChildVisibleCount) or
...
begin
  ...
  begin
    AExpanded := Expanded;
    ANeedMoveFocus := not AValue and (TreeList.FocusedNode <> nil) and
      TreeList.FocusedNode.HasAsParent(Self);
    if Parent = nil then
      Exit;
    TreeList.BeginUpdate;
    try
      ...
      if AValue then
      begin
        if not TreeList.DoCanNodeExpand(Self) then
          Exit;
        TreeList.DoExpand(Self);
        Exclude(State, nsCollapsed);
      ...
      begin
        if not TreeList.DoCanNodeCollapse(Self) then
          Exit;
...
procedure TcxTreeListNode.SetFooterSummaryValue(AIndex: Integer;
  AValue: Variant);
begin
  cxError(not cxInRange(AIndex, 0, FooterSummaryCount - 1), SListIndexError,
    [AIndex]);
...
procedure TcxTreeListNode.SetNodeImageIndex(AIndex: Integer;
  AValue: TcxImageIndex);
begin
...
procedure TcxTreeListNode.SetParentFor(AValue: TcxTreeListNode;
  AValidateIndexes: Boolean = True);
begin
  AValue.FParent := Self;
...
begin
  if Selected <> AValue then
    try
      ...
      else if TreeList.OptionsSelection.MultiSelect then
...
function TcxTreeListRootNode.GetFooterSummaryText(AIndex: Integer): string;
begin
  Result := Summary.FooterSummaryTexts[Summary.FooterSummaryItems[AIndex]];
end;
...
begin
  for I := 0 to ASummaryItems.Count - 1 do
    ASummaryItems[I].AbsoluteIndex := AAbsoluteSummaryItems.Add
      (ASummaryItems[I]);
  if AIsVisible then
  begin
...
begin
  ...
  begin
    AddSummaryItems(FAbsoluteFooterSummaryItems,
      FTreeList.Columns[I].Summary.FooterSummaryItems,
      FTreeList.Columns[I].ActuallyVisible and FTreeList.Columns[I]
      .Options.Footer, FFooterSummaryVisibleCount, FFooterSummaryRowCount);
    AddSummaryItems(FAbsoluteGroupFooterSummaryItems,
      FTreeList.Columns[I].Summary.GroupFooterSummaryItems,
      FTreeList.Columns[I].ActuallyVisible and FTreeList.Columns[I]
      .Options.GroupFooter, FGroupFooterSummaryVisibleCount,
      FGroupFooterSummaryRowCount);
  end;
  IsDirty := True;
end;

procedure TcxTreeListSummary.CalculateNode(ADestNode,
  AValueNode: TcxTreeListNode; AItems: TList);
begin
...
function TcxTreeListSummary.DoProcessSummaryValue(ANode: TcxTreeListNode;
  ASummaryItem: TcxTreeListSummaryItem; var ASummaryValue: Variant;
  var ASummaryCount: Integer; var AValue: Variant): Boolean;
var
...
procedure TcxTreeListSummary.FinalizeValues(ANode: TcxTreeListNode;
  AItems: TList);
var
  ASummaryItem: TcxTreeListSummaryItem;
  I: Integer;
...
begin
  ...
  begin
    ...
    case ASummaryItem.Kind of
      ...
      skAverage:
        if (ASummaryInfos[I].Count > 0) and
          not VarIsEmpty(ASummaryInfos[I].TempValue) then
        begin
          AVarIsDate := VarIsDate(ASummaryInfos[I].TempValue);
          ASummaryInfos[I].Value := ASummaryInfos[I].TempValue /
            ASummaryInfos[I].Count;
          if AVarIsDate then
            VarCast(ASummaryInfos[I].Value, ASummaryInfos[I].TempValue,
              varDate);
        end;
    end;
  end;
...
procedure TcxTreeListSummary.InitializeValues(ANode: TcxTreeListNode;
  AItems: TList);
var
  I: Integer;
  ASummaryItem: TcxTreeListSummaryItem;
...
procedure TcxTreeListSummary.Notification(AComponent: TComponent;
  Operation: TOperation);
var
  I: Integer;
begin
  ...
  for I := 0 to FAbsoluteFooterSummaryItems.Count - 1 do
    TcxTreeListSummaryItem(FAbsoluteFooterSummaryItems[I])
      .Notification(AComponent, Operation);
  for I := 0 to FAbsoluteGroupFooterSummaryItems.Count - 1 do
    TcxTreeListSummaryItem(FAbsoluteGroupFooterSummaryItems[I])
      .Notification(AComponent, Operation);
end;
...
procedure TcxTreeListSummary.ProcessSummaryValue(ANode: TcxTreeListNode;
  ASummaryItem: TcxTreeListSummaryItem; var ASummaryValue: Variant;
  var ASummaryCount: Integer);
var
  AValue: Variant;
begin
  ...
  if not DoProcessSummaryValue(ANode, ASummaryItem, ASummaryValue,
    ASummaryCount, AValue) and not(TreeList.OptionsData.SummaryNullIgnore and
...
procedure TcxTreeListSummary.ProcessSummaryValues(ADestNode,
  AValueNode: TcxTreeListNode; AItems: TList);
var
  I: Integer;
  ASummaryItem: TcxTreeListSummaryItem;
...
begin
  ...
  begin
    ...
    begin
      ...
      ProcessSummaryValue(AValueNode, ASummaryItem,
        ADestNode.FSummaryInfo[AAbsoluteItemIndex].TempValue,
...
function TcxTreeListSummary.GetFooterSummaryItem(AIndex: Integer)
  : TcxTreeListSummaryItem;
begin
  Result := TcxTreeListSummaryItem(FAbsoluteFooterSummaryItems[AIndex]);
end;

function TcxTreeListSummary.GetFooterSummaryText(ASummaryItem
  : TcxTreeListSummaryItem): string;
begin
  Result := GetGroupFooterSummaryText(ASummaryItem, Root);
end;

function TcxTreeListSummary.GetFooterSummaryValue(ASummaryItem
  : TcxTreeListSummaryItem): Variant;
begin
...
function TcxTreeListSummary.GetGroupFooterSummaryItem(AIndex: Integer)
  : TcxTreeListSummaryItem;
begin
...
begin
  Result := ASummaryItem.FormatValue(GetGroupFooterSummaryValue(ASummaryItem,
    ANode), True);
end;
...
procedure TcxTreeListSummary.SetFooterSummaryValue(ASummaryItem
  : TcxTreeListSummaryItem; const Value: Variant);
begin
...
function TcxTreeListStyles.GetBandBackgroundParams(ABand: TcxTreeListBand)
  : TcxViewParams;
begin
...
function TcxTreeListStyles.GetBandFooterParams(ABand: TcxTreeListBand;
  ANode: TcxTreeListNode): TcxViewParams;
begin
...
function TcxTreeListStyles.GetBandHeaderParams(ABand: TcxTreeListBand)
  : TcxViewParams;
begin
  if ABand = nil then
...
function TcxTreeListStyles.GetColumnHeaderParams(AColumn: TcxTreeListColumn)
  : TcxViewParams;
begin
  if AColumn = nil then
...
begin
  AStyle := nil;
  if (ANode <> nil) and ANode.HotTrack and not TreeList.IsEditing and
    ((AColumn = nil) or not AColumn.IsPreview) then
  begin
...
function TcxTreeListStyles.GetIndentParams(ANode: TcxTreeListNode;
  AIndent: Integer): TcxViewParams;
var
  AStyle: TcxStyle;
begin
  AStyle := nil;
...
function TcxTreeListStyles.GetPreviewParams(ANode: TcxTreeListNode)
  : TcxViewParams;
begin
...
begin
  Result := ((UseOddEvenStyles = bDefault) and (Assigned(ContentOdd) or
    Assigned(ContentEven) or
    LookAndFeelPainter.GridLikeControlDefaultUseOddEvenStyle)) or
    (UseOddEvenStyles = bTrue);
end;

procedure TcxTreeListStyles.DoGetBandBackgroundParams(ABand: TcxTreeListBand;
...
procedure TcxTreeListStyles.DoGetBandHeaderParams(ABand: TcxTreeListBand;
  var AParams: TcxViewParams);
var
  AStyle: TcxStyle;
begin
...
procedure TcxTreeListStyles.DoGetColumnHeaderParams(AColumn: TcxTreeListColumn;
  var AParams: TcxViewParams);
var
  AStyle: TcxStyle;
begin
...
begin
  ...
  begin
    ...
    if (AStyle = nil) and (AColumn <> nil) and (AColumn.Position.Band <> nil)
      and Assigned(FOnGetBandContentStyle) then
      FOnGetBandContentStyle(TreeList, AColumn.Position.Band, ANode, AStyle);
    DoGetNodeContentParams(ANode, AStyle, AParams);
  end;
end;

procedure TcxTreeListStyles.DoGetNodeContentParams(ANode: TcxTreeListNode;
  AStyle: TcxStyle; var AParams: TcxViewParams);
begin
  if (ANode <> nil) and (GetValue(OddEvenStyleIndexes[Odd(ANode.VisibleIndex)])
    <> nil) and ActuallyUseOddEvenStyles then
    GetViewParams(OddEvenStyleIndexes[Odd(ANode.VisibleIndex)], ANode,
      AStyle, AParams)
  else
    GetViewParams(ecs_Content, ANode, AStyle, AParams);
end;

procedure TcxTreeListStyles.DoGetPreviewParams(ANode: TcxTreeListNode;
  var AParams: TcxViewParams);
var
  AStyle: TcxStyle;
begin
...
procedure TcxTreeListStyles.GetDefaultViewParams(Index: Integer; AData: TObject;
...
begin
  ...
  begin
    ...
    else if AData is TcxTreeListNode then
  ...
  else if ((Index = tlsv_ContentEven) or (Index = tlsv_ContentOdd)) and not ActuallyUseOddEvenStyles
  then
    Index := tlsv_Content;
  ...
  begin
    ...
    case Index of
      ...
      ecs_Inactive, ecs_Selection:
        if AData <> nil then
          Font := TcxTreeListColumn(TcxTreeListCellPos(AData).Item)
            .Styles.GetContentParams(TcxTreeListCellPos(AData).Node).Font;
    end;
  end;
end;

function TcxTreeListStyles.GetCellSelectionParams(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn): TcxViewParams;
var
  ACellPos: TcxTreeListCellPos;
begin
...
procedure TcxTreeListStyles.PrepareConditionalFormattingParams
  (ANode: TcxTreeListNode; AColumn: TcxTreeListColumn;
  var AParams: TcxViewParams);
begin
  if (ANode <> nil) and (AColumn <> nil) then
    TreeList.ConditionalFormattingProvider.CalculateViewParams(AParams,
      ANode.AbsoluteIndex, AColumn.ItemIndex);
end;
...
procedure TcxTreeListStyles.SetUseOddEvenStyles(const AValue
  : TdxDefaultBoolean);
begin
...
function TcxTreeListDataFindCriteria.GetTextStartPositionByNode
  (ANode: TcxTreeListNode; AItemIndex: Integer;
...
begin
  ...
  if not IsActive or (Items.IndexOf(AField) = -1) or
    not DoFilterRecord(ARecordIndex) then
    Exit;
...
function TcxTreeListDataFindCriteria.GetDataController
  : TcxTreeListDataController;
begin
  Result := TcxTreeListDataController(inherited DataController);
end;

function TcxTreeListDataFindCriteria.GetMatchNode(AMatchIndex: Integer)
  : TcxTreeListNode;
var
...
function TcxTreeListDataFindCriteria.GetMatchNodeVisibleIndex
  (AMatchIndex: Integer): Integer;
begin
...
procedure TcxTreeListCustomBuiltInMenu.GetBitmapFromImageList
  (AImages: TCustomImageList; AImageIndex: Integer; ABitmap: TBitmap);
var
  AImage: TcxAlphaBitmap;
begin
  if IsImageAssigned(AImages, AImageIndex) then
  begin
    AImage := cxPrepareBitmapForDrawing(nil, AImages, AImageIndex,
      False, clNone);
    ABitmap.PixelFormat := pf32bit;
    ABitmap.SetSize(AImage.Width, AImage.Height);
    cxBitBlt(ABitmap.Canvas.Handle, AImage.Canvas.Handle,
      Rect(0, 0, ABitmap.Width, ABitmap.Height), cxNullPoint, SRCCOPY);
  end;
end;

function TcxTreeListCustomBuiltInMenu.GetImages(AInternal: Boolean)
  : TCustomImageList;
begin
  if AInternal then
...
type
  ...
  protected
    ...
    procedure MeasureItem(ACanvas: TCanvas;
      var Width, Height: Integer); override;
  public
    constructor Create(ABuiltInMenu: TcxTreeListBuiltInMenu;
      AInternal: Boolean); reintroduce; virtual;
  end;

constructor TcxTreeListBuiltInMenuItem.Create(ABuiltInMenu
  : TcxTreeListBuiltInMenu; AInternal: Boolean);
begin
...
procedure TcxTreeListBuiltInMenuItem.AdvancedDrawItem(ACanvas: TCanvas;
...
begin
  if not IsWinXPOrLater or FBuiltInMenu.PopupMenu.IsRightToLeft then
    ...
    cxAdvancedDrawPopupMenuItem(Self, ACanvas, ARect, State,
      FBuiltInMenu.GetImages(FInternal));
end;

procedure TcxTreeListBuiltInMenuItem.MeasureItem(ACanvas: TCanvas;
...
function TcxTreeListBuiltInMenu.CreateMenuItem(AOwner: TComponent;
  const ACaption: string; ACommand: TcxTag; AEnabled: Boolean;
  AItemType: TcxTreeListBuiltInMenuItemType; AChecked: Boolean;
  AImageIndex: Integer; AWithSeparator: Boolean; AInternal: Boolean)
  : TComponent;
var
...
function TcxTLPopupMenuImagesManager.GetImages(const AResourceName: string)
  : TCustomImageList;
...
function TcxTreeListPopupMenu.CreateMenuItem(AOwner: TComponent;
  const ACaption: string; ACommand: TcxTag; AEnabled: Boolean;
  AItemType: TcxTreeListBuiltInMenuItemType; AChecked: Boolean;
  AImageIndex: Integer; AWithSeparator: Boolean): TComponent;
begin
  Result := BuiltInMenu.CreateMenuItem(AOwner, ACaption, ACommand, AEnabled,
    AItemType, AChecked, AImageIndex, AWithSeparator, False);
end;

function TcxTreeListPopupMenu.CreateInternalMenuItem(AOwner: TComponent;
  const ACaption: string; ACommand: TcxTag; AEnabled: Boolean = True;
  AItemType: TcxTreeListBuiltInMenuItemType = tlmitDefault;
  AChecked: Boolean = False; AImageIndex: Integer = -1;
  AWithSeparator: Boolean = False): TComponent;
begin
...
procedure TcxTreeListPopupMenu.DoClick(AItem: TObject; var AHandled: Boolean);
begin
  AHandled := False;
...
begin
  Result := IsPopupSuitable(TcxTreeListPopupMenus(Owner).HitTest) and
    Popup(P.X, P.Y);
end;
...
function TcxTreeListPopupMenu.IsPopupSuitable
...
begin
  if (Operation = opRemove) then
    if (AComponent = PopupMenu) then
      PopupMenu := nil
    else if (AComponent = UserImages) then
      UserImages := nil;
end;
...
begin
  inherited Create(AOwner);
  FItems := [tlfmiSum, tlfmiMin, tlfmiMax, tlfmiAverage, tlfmiCount, tlfmiNone,
    tlfmiAllNodes];
end;
...
begin
  ...
  if tlfmiSum in Items then
    CreateInternalMenuItem(Root, cxGetResourceString(@scxTreeListSumMenuItem),
      tlcmSum, IsItemEnabled(skSum), tlmitChecked, skSum in ASummaryKinds,
      tlcmSum - 1);
  if tlfmiMin in Items then
    CreateInternalMenuItem(Root, cxGetResourceString(@scxTreeListMinMenuItem),
      tlcmMin, IsItemEnabled(skMin), tlmitChecked, skMin in ASummaryKinds,
      tlcmMin - 1);
  if tlfmiMax in Items then
    CreateInternalMenuItem(Root, cxGetResourceString(@scxTreeListMaxMenuItem),
      tlcmMax, IsItemEnabled(skMax), tlmitChecked, skMax in ASummaryKinds,
      tlcmMax - 1);
  if tlfmiCount in Items then
    CreateInternalMenuItem(Root, cxGetResourceString(@scxTreeListCountMenuItem),
      tlcmCount, IsItemEnabled(skCount), tlmitChecked, skCount in ASummaryKinds,
      tlcmCount - 1);
  if tlfmiAverage in Items then
    CreateInternalMenuItem(Root, cxGetResourceString(@scxTreeListAvgMenuItem),
      tlcmAverage, IsItemEnabled(skAverage), tlmitChecked,
      skAverage in ASummaryKinds, tlcmAverage - 1);
  if tlfmiNone in Items then
    CreateInternalMenuItem(Root, cxGetResourceString(@scxTreeListNoneMenuItem),
      tlcmNone, IsItemEnabled(skNone), tlmitChecked, not ADoesVisibleItemExist,
      -1, True);
  if tlfmiAllNodes in Items then
    CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListAllNodesMenuItem), tlcmAllNodes,
      ADoesVisibleItemExist, tlmitChecked, ADoesVisibleItemExist and IsAllNodes,
      -1, True);
end;
...
begin
  ...
  try
    case ACommand of
      tlcmNone .. tlcmAverage:
        SetItemsVisibility(TcxSummaryKind(ACommand));
      tlcmAllNodes:
        SetAllNodes(not IsAllNodes);
    end;
  finally
...
function TcxTreeListFooterPopupMenu.IsPopupSuitable
  (AHitTest: TcxTreeListHitTest): Boolean;
begin
...
function TcxTreeListFooterPopupMenu.IsItemEnabled
  (AKind: TcxSummaryKind): Boolean;

  function IsSummaryKindAllowedForValueType(AValueTypeClass
    : TcxValueTypeClass): Boolean;
  const
    ANumberVarTypes = [varSmallint, varInteger, varSmallint, varSingle, varByte,
      varDouble, varCurrency, varDate, varLongWord, varWord, varInt64,
      varShortInt];
  ...
  begin
    ...
    begin
      ...
      Result := ((AVarType in ANumberVarTypes) or
        (AVarType = TcxFMTBcdValueType.GetVarType)) and
...
begin
  ...
  Result := (AKind in [skCount, skNone]) or
    not(AColumn.Properties is TcxCustomLookupEditProperties) and
...
function TcxTreeListGroupFooterPopupMenu.IsPopupSuitable
  (AHitTest: TcxTreeListHitTest): Boolean;
begin
...
begin
  inherited Create(AOwner);
  FItems := [tlchmiSortAscending, tlchmiSortDescending, tlchmiClearSorting,
    tlchmiFooter, tlchmiGroupFooters, tlchmiRemoveThisColumn,
...
begin
  ...
  begin
    if tlchmiSortAscending in Items then
      CreateInternalMenuItem(Root,
        ...
        Column.SortOrder = soAscending, 0);
    if tlchmiSortDescending in Items then
      CreateInternalMenuItem(Root,
        cxGetResourceString(@scxTreeListSortDescendingMenuItem),
        tlcmSortDescending, True, tlmitChecked,
        Column.SortOrder = soDescending, 1);
    if tlchmiClearSorting in Items then
      CreateInternalMenuItem(Root,
        cxGetResourceString(@scxTreeListClearSortingMenuItem), tlcmClearSorting,
        Column.SortOrder <> soNone, tlmitDefault, False, -1);
    FSeparatorNeeded := True;
  end;
  if tlchmiFooter in Items then
    CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListFooterMenuItem), tlcmFooter, True,
      tlmitChecked, TreeList.OptionsView.Footer, 4, IsSeparatorNeeded);
  if tlchmiGroupFooters in Items then
  begin
    ASubMenu := CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListGroupFootersMenuItem), -1, True,
      tlmitSubItem, False, 5, IsSeparatorNeeded);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListGroupFootersInvisibleMenuItem),
      tlcmGroupFootersInvisible, True, tlmitChecked,
      TreeList.OptionsView.GroupFooters = tlgfInvisible);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListGroupFootersVisibleWhenExpandedMenuItem),
      tlcmGroupFootersVisibleWhenExpanded, True, tlmitChecked,
      TreeList.OptionsView.GroupFooters = tlgfVisibleWhenExpanded);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListGroupFootersAlwaysVisibleMenuItem),
      tlcmGroupFootersAlwaysVisible, True, tlmitChecked,
  ...
  if (tlchmiRemoveThisColumn in Items) and not Column.Options.Hidden and
    ...
    CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListRemoveThisColumnMenuItem),
      tlcmRemoveThisColumn, True, tlmitDefault, False, -1, IsSeparatorNeeded);
  if tlchmiFieldChooser in Items then
    CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListFieldChooserMenuItem), tlcmFieldChooser,
      True, tlmitDefault, False, 2, IsSeparatorNeeded);
  FSeparatorNeeded := True;
  ...
  if tlchmiBestFit in Items then
    CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListBestFitMenuItem), tlcmBestFit, True,
      tlmitDefault, False, 3, IsSeparatorNeeded);
...
begin
  ...
  case ACommand of
    ...
    tlcmGroupFootersInvisible .. tlcmGroupFootersAlwaysVisible:
      TreeList.OptionsView.GroupFooters := TcxTreeListGroupFootersMode
        (ACommand - tlcmGroupFootersInvisible);
    ...
    tlcmHorzAlignmentLeft .. tlcmHorzAlignmentCenter:
      Column.Properties.Alignment.Horz :=
        TcxEditHorzAlignment(ACommand - tlcmHorzAlignmentLeft);
    tlcmVertAlignmentTop .. tlcmVertAlignmentCenter:
      Column.Properties.Alignment.Vert :=
        TcxEditVertAlignment(ACommand - tlcmVertAlignmentTop);
    tlcmBestFit:
      Column.ApplyBestFit;
...
function TcxTreeListColumnHeaderPopupMenu.IsPopupSuitable
  (AHitTest: TcxTreeListHitTest): Boolean;
begin
...
begin
  ...
  begin
    ASubMenu := CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListHorizontalAlignmentMenuItem), -1, True,
      tlmitSubItem, False, -1, IsSeparatorNeeded);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListHorizontalAlignmentLeftMenuItem),
      tlcmHorzAlignmentLeft, True, tlmitChecked,
      Column.Properties.Alignment.Horz = taLeftJustify);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListHorizontalAlignmentCenterMenuItem),
      tlcmHorzAlignmentCenter, True, tlmitChecked,
      Column.Properties.Alignment.Horz = taCenter);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListHorizontalAlignmentRightMenuItem),
      tlcmHorzAlignmentRight, True, tlmitChecked,
      Column.Properties.Alignment.Horz = taRightJustify);
  end;
  if tlchmiVertAlignment in Items then
  begin
    ASubMenu := CreateInternalMenuItem(Root,
      cxGetResourceString(@scxTreeListVerticalAlignmentMenuItem), -1, True,
      tlmitSubItem, False, -1, IsSeparatorNeeded);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListVerticalAlignmentTopMenuItem),
      tlcmVertAlignmentTop, True, tlmitChecked,
      Column.Properties.Alignment.Vert = taTopJustify);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListVerticalAlignmentCenterMenuItem),
      tlcmVertAlignmentCenter, True, tlmitChecked,
      Column.Properties.Alignment.Vert = taVCenter);
    CreateInternalMenuItem(ASubMenu,
      cxGetResourceString(@scxTreeListVerticalAlignmentBottomMenuItem),
      tlcmVertAlignmentBottom, True, tlmitChecked,
      Column.Properties.Alignment.Vert = taBottomJustify);
  end;
end;

function TcxTreeListColumnHeaderPopupMenu.IsItemsStored: Boolean;
begin
  Result := FItems <> [tlchmiSortAscending, tlchmiSortDescending,
    tlchmiClearSorting, tlchmiFooter, tlchmiGroupFooters,
    tlchmiRemoveThisColumn, tlchmiFieldChooser, tlchmiHorzAlignment,
    tlchmiVertAlignment, tlchmiBestFit, tlchmiBestFitAllColumns];
end;
...
procedure TcxTreeListPopupMenus.SetColumnHeaderMenu
  (AValue: TcxTreeListColumnHeaderPopupMenu);
begin
...
function TcxTreeList.Add(ASibling: TcxTreeListNode; AData: Pointer = nil)
  : TcxTreeListNode;
begin
...
function TcxTreeList.AddChild(AParent: TcxTreeListNode; AData: Pointer = nil)
  : TcxTreeListNode;
begin
...
function TcxTreeList.AddNode(ANode, ARelative: TcxTreeListNode; AData: Pointer;
  AttachMode: TcxTreeListNodeAttachMode): TcxTreeListNode;
begin
  Result := inherited AddNode(ANode, ARelative, AData, AttachMode);
end;

function TcxTreeList.Insert(ASibling: TcxTreeListNode; AData: Pointer = nil)
  : TcxTreeListNode;
begin
...
begin
  ...
  for I := 0 to FAbsoluteItems.Count - 1 do
    DataController.InitializeNodeFromRecordIndex
      (TcxTreeListNode(FAbsoluteItems.List[I]));
...
begin
  ...
  TcxDataStorageHelper.SetRecordsCapacity(DataController.DataStorage,
    AbsoluteCount);
  for I := 0 to AbsoluteCount - 1 do
    DataController.InitializeRecordIndexFromNode
      (TcxTreeListNode(FAbsoluteItems[I]));
  try
...
constructor TcxTreeListNodeViewData.Create(AViewInfo: TcxTreeListViewInfo;
  ANode: TcxTreeListNode; ACapacity: Integer; AFake: Boolean = False);
begin
  ViewInfo := AViewInfo;
  Node := ANode;
  FFake := AFake;
...
procedure TcxTreeListNodeViewData.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  Bounds := TdxRightToLeftLayoutConverter.ConvertRect(Bounds, AClientBounds);
  ContentBounds := TdxRightToLeftLayoutConverter.ConvertRect(ContentBounds,
    AClientBounds);
  FocusRectBounds := TdxRightToLeftLayoutConverter.ConvertRect(FocusRectBounds,
    AClientBounds);
  Cells.RightToLeftConversion(AClientBounds);
end;

procedure TcxTreeListNodeViewData.RightToLeftConversion
  (const AClientBounds: TRect);
begin
...
begin
  for I := 0 to Cells.Count - 1 do
    if Cells[I].Visible and (Cells[I].CustomDrawID in [cxtlIndentCell,
      cxtlIndicatorCell]) then
      Cells[I].Invalidate(cxRectOffset(Cells[I].VisibleRect, Origin));
end;

procedure TcxTreeListNodeViewData.Paint(ACanvas: TcxCanvas;
...
begin
  if not ACanvas.RectVisible(cxRectInflate(GetRealBounds, 1, 1)) and
    (GetRealBounds.Top <> 39) then
    Exit;
  ...
  try
    ...
    if DrawFocusRect and Node.TreeList.OptionsView.FocusRect and
      Node.Focused and (not Node.TreeList.OptionsSelection.HideFocusRect or
      Node.TreeList.IsFocused) then
    begin
      for I := 0 to Cells.Count - 1 do
    ...
    begin
      for I := 0 to Cells.Count - 1 do
        if Cells[I].Visible and (Cells[I].CustomDrawID = cxtlBandPartCell) and
          (TcxTreeListBandCellViewInfo(Cells[I]).Part in [tlbpGroupFooter,
          tlbpFooter]) then
        begin
          AFooterRgn := TcxRegion.Create
            (cxRectOffset(TcxTreeListBandCellViewInfo(Cells[I])
            .ClipRect, Origin));
...
var
  ...
  R: TRect;
begin
  Result := (Node <> nil) and ((Selected or Node.Selected) or Focused or
    Node.Focused or AForceUpdate);
...
procedure TcxTreeListColumnScrollAnimationHelper.AfterAnimationEnded
...
procedure TcxTreeListColumnScrollAnimationHelper.UpdateOwnerPosition
  (AValue: Integer);
begin
...
begin
  Result := FOwner.GetPixelScrollOffset(GetOwnerPosition.Node,
    GetOwnerPosition.ScrollOffset, AValue.Node, AValue.ScrollOffset);
end;
...
procedure TcxTreeListNodeScrollAnimationHelper.AfterAnimationEnded
  (AIsRaiseEvent: Boolean);
begin
...
function TcxTreeListNodeScrollAnimationHelper.CalculateAnimatedPosition
  (var APosition: Integer): TcxTreeListNodePixelScrollData;
var
  AOverPan: Boolean;
begin
  Result := GetOwnerPosition;
  FOwner.CalculatePixelScrollInfo(Result.Node, Result.ScrollOffset, nil, -1,
    -Sign(AnimationLength) * (APosition - PrevAnimatedPosition), AOverPan);
end;

function TcxTreeListNodeScrollAnimationHelper.GetOwnerPosition
  : TcxTreeListNodePixelScrollData;
begin
...
function TcxTreeListNodeScrollAnimationHelper.GetUnassignedTargetPosition
  : TcxTreeListNodePixelScrollData;
begin
  Result.Node := nil;
...
function TcxTreeListNodeScrollAnimationHelper.IsEqual(AValue1,
  AValue2: TcxTreeListNodePixelScrollData): Boolean;
begin
  Result := (AValue1.Node = AValue2.Node) and
    (AValue1.ScrollOffset = AValue2.ScrollOffset);
end;
...
procedure TcxTreeListNodeScrollAnimationHelper.UpdateOwnerPosition
  (AValue: TcxTreeListNodePixelScrollData);
begin
...
function TcxCustomTreeList.CreateColumn(ABand: TcxTreeListBand = nil)
  : TcxTreeListColumn;
begin
  BeginUpdate;
  try
...
function TcxCustomTreeList.Find(AData: Pointer; AStart: TcxTreeListNode;
  AExpandedOnly, AForward: Boolean; AFilter: TcxTreeListFindFunc;
  AIgnoreStartNode: Boolean = False): TcxTreeListNode;
begin
...
function TcxCustomTreeList.FindNodeByText(const AText: string;
  ...
  ALikeParams: TcxTreeListLikeParams = nil; AIgnoreStartNode: Boolean = False)
  : TcxTreeListNode;
var
...
begin
  ...
  Result := Find(@AFindInfo, AStartNode, AExpandedOnly, AForward,
    FindTextFilter, AIgnoreStartNode);
end;

procedure TcxCustomTreeList.SetFocusedNode(ANode: TcxTreeListNode;
  AShift: TShiftState);
begin
  if ANode = Root then
    ANode := nil;
...
begin
  if (not OptionsSelection.MultiSelect and (Controller.FocusedNode <> nil)) or
    (AbsoluteVisibleCount = 0) then
    Exit;
  BeginUpdate;
  try
...
begin
  if not OptionsData.Editing then
    Exit;
  Edit;
...
procedure TcxCustomTreeList.RestoreFromStream(AStream: TStream;
  ...
  const ARestoreTreeListName: string = '');
begin
  RestoreFrom(stStream, '', AStream, AChildrenCreating, AChildrenDeleting,
    ARestoreTreeListName);
end;
...
function TcxCustomTreeList.CellRect(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn): TRect;
var
...
function TcxCustomTreeList.GetEditRect(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn): TRect;
var
  AViewInfo: TcxTreeListEditCellViewInfo;
begin
...
function TcxCustomTreeList.GetNodeAt(X, Y: Integer): TcxTreeListNode;
begin
...
procedure TcxCustomTreeList.CopySelectedToClipboard(AVisibleOnly
  : Boolean = False);
begin
...
function TcxCustomTreeList.GoToNextScrollbarAnnotation
  (AKinds: TdxScrollbarAnnotationKinds = dxAllScrollbarAnnotationKinds;
  AGoForward: Boolean = True; AGoOnCycle: Boolean = False): Boolean;
begin
...
function TcxCustomTreeList.ColumnByName(const AName: string): TcxTreeListColumn;
var
  I: Integer;
begin
  Result := nil;
...
function cxCompareColumnsVisibleOrder(AColumn1,
  AColumn2: TcxTreeListColumn): Integer;
begin
  Result := Weights[AColumn1.Position.Band.FixedKind] -
    Weights[AColumn2.Position.Band.FixedKind];
  if Result <> 0 then
    Exit;
  Result := AColumn1.Position.Band.VisibleIndex -
    AColumn2.Position.Band.VisibleIndex;
  if Result = 0 then
  begin
    Result := AColumn1.Position.VisibleRowIndex -
      AColumn2.Position.VisibleRowIndex;
    if Result = 0 then
    begin
      Result := AColumn1.Position.VisibleColIndex -
        AColumn2.Position.VisibleColIndex;
      if Result = 0 then
...
function TcxCustomTreeList.DoShowPopupMenu(AMenu: TComponent;
  X, Y: Integer): Boolean;
begin
...
begin
  if ([csLoading, csDestroying, csUpdating, csReading] * ComponentState <> [])
    or DataController.IsLoading or (Root = nil) then
    Exit;
...
function TcxCustomTreeList.IsScrollbarAnnotationVisible(AKind: Integer)
  : Boolean;
begin
  case AKind of
...
procedure TcxCustomTreeList.ScrollContentByGesture(AScrollKind: TScrollBarKind;
  ADelta: Integer);
begin
...
procedure TcxCustomTreeList.PopulateScrollbarAnnotations
...
procedure TcxCustomTreeList.SetCurrentScrollbarAnnotationRecordIndex
  (AIndex: Integer);
begin
  SetFocusedVisibleNode(AbsoluteVisibleItems[AIndex]);
end;

function TcxCustomTreeList.InternalCollapseNode(ANode: TcxTreeListNode;
  ARecursive: Boolean): Boolean;
begin
  Result := False;
end;

function TcxCustomTreeList.InternalExpandNode(ANode: TcxTreeListNode;
  ARecursive: Boolean): Boolean;
begin
  Result := False;
end;

procedure TcxCustomTreeList.AssignItemsPosition(ABandIndexOnly: Boolean);
var
  I: Integer;
  AList: TList;
begin
  ...
  try
    ...
    for I := 0 to Bands.Count - 1 do
      AList.Add(Bands[I]);
    if (Bands.Count > 0) and (Bands[0].FIndex <> Bands[Bands.Count - 1].FIndex)
    then
    begin
...
begin
  ...
  begin
    ...
    Result.Right := Max(VisibleColumns[0].DisplayWidth,
      cxTextWidth(GetDragDropViewParams.Font, GetDragDropText) +
      ScaleFactor.Apply(cxTextOffset) * 2);
...
procedure TcxCustomTreeList.DrawDragDropImage(ADragBitmap: TBitmap;
  ACanvas: TcxCanvas);
var
  R: TRect;
begin
  ...
  ACanvas.Brush.Style := bsClear;
  ACanvas.DrawText(GetDragDropText, cxTextRect(R), cxAlignLeft or
...
begin
  ...
  if not ViewInfo.IsRightToLeftConverted then
    Result := ClientToScreen
      (Point(ADestNodeBounds.Left + ViewInfo.IndicatorWidth,
      cxRectCenter(ADestNodeBounds).Y))
  else
    Result := ClientToScreen
      (Point(ADestNodeBounds.Right - ViewInfo.IndicatorWidth -
      Controller.FDragNodeInfoImage.Image.Width,
      cxRectCenter(ADestNodeBounds).Y));
end;
...
procedure TcxCustomTreeList.GetStoredPropertyValue(const AName: string;
  var AValue: Variant);
begin
  if AName = 'ColumnsQuickCustomizationSorted' then
    AValue := OptionsCustomizing.ColumnsQuickCustomizationSorted
  else if AName = 'BandsQuickCustomizationSorted' then
...
procedure TcxCustomTreeList.SetStoredPropertyValue(const AName: string;
...
function TcxCustomTreeList.StoredCreateChild(const AObjectName,
...
procedure TcxCustomTreeList.StoredDeleteChild(const AObjectName: string;
  AObject: TObject);
begin
...
procedure TcxCustomTreeList.GetChildren(Proc: TGetChildProc; Root: TComponent);
...
procedure TcxCustomTreeList.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  ...
  begin
    if (Summary <> nil) and (Operation = opRemove) and
      (AComponent is TcxTreeListColumn) then
...
const
  IsAddChild: array [TcxTreeListNodeAttachMode] of Boolean = (False, False,
    True, True, False);
...
begin
  ...
  else if Result.FOriginalIndex < 0 then
...
procedure TcxCustomTreeList.AfterMouseDown(AButton: TMouseButton;
  X, Y: Integer);
begin
  if (DragMode = dmAutomatic) and (AButton = mbLeft) and MouseCapture and
    ...
    CanDrag(X, Y) and (cxDragDetect(Handle, ClientToScreen(Point(X, Y)))
    = ddDrag) then
    BeginDrag(True);
...
begin
  ...
  for I := 0 to Min(ContainerList.Count, ASource.ContainerList.Count) - 1 do
    TcxTreeListColumn(ContainerList[I])
      .Assign(TPersistent(ASource.ContainerList[I]));
  AssignItemsPosition(False);
...
begin
  if Bands.IsUpdateLocked then
    Exit;
  ABand := Bands.ExpandableBand;
  if (ABand = nil) or IsLoading then
    Exit;
  AForceDelta := ABand.BandRows.RowMinWidth + IndentWidth - ABand.DisplayWidth;
  W := ABand.Width;
  if AForceDelta > 0 then
    Bands.ExpandableBand.ForceWidth(Bands.ExpandableBand.DisplayWidth +
      AForceDelta);
  for I := 0 to ABand.BandRows.VisibleItemCount - 1 do
  begin
...
begin
  Result := dxDefaultBooleanToBoolean(OptionsData.MultiThreadedSorting,
    dxDefaultMultiThreadedSorting) and dxCanUseMultiThreading and
    IsSortedColumnsSupportMultiThreading;
end;
...
begin
  if (Changes = []) or ChangesLocked or IsLoading or IsDestroying or ViewInfo.CalculateInProcess
  then
    Exit;
  ...
  try
    ...
    try
      ...
      if (Changes * [tcFilterCriteria, tcSortOrder, tcData, tcStructure,
        tcFindCriteria] <> []) and ViewInfo.IsNodeScrollAnimationActive then
        ViewInfo.DestroyNodeScrollAnimation;
      if ([tcFindCriteria, tcFilterCriteria] * Changes = []) and
        not(Changes = SelectionChanges) then
      ...
      AStructureChanged := (tcStructure in Changes) or
        (DataController.NodesCount <> FAbsoluteItems.Count);
      ...
      begin
        ...
        AChangedAnnotationKinds := AChangedAnnotationKinds +
          [dxSelectedRowScrollbarAnnotationID,
          dxFocusedRowScrollbarAnnotationID];
      end;
      ...
      else if (ADataChanged or AStructureChanged or AFocusChanged) and
        Controller.IsFindPanelVisible then
...
procedure TcxCustomTreeList.CheckImageListReferences(AComponent: TComponent;
  Operation: TOperation);
var
  I: Integer;
begin
...
begin
  FDefaultIndentSize.cy := Max(ViewInfo.DefaultCellHeight,
    ...
    ScaleFactor.Apply(cxTreeListIndentOffsetSize));
  FDefaultIndentSize.cx := LookAndFeelPainter.ScaledExpandButtonSize
    (ScaleFactor) + ScaleFactor.Apply(cxTreeListIndentOffsetSize);
  ...
  try
    ...
    begin
      ...
      if not OptionsView.DynamicIndent then
        FDefaultIndentSize.cx := Max(FDefaultIndentSize.cx,
          LevelsInfo[I].Size.cx);
      if not OptionsView.CellAutoHeight then
        FDefaultIndentSize.cy := Max(FDefaultIndentSize.cy,
          LevelsInfo[I].Size.cy);
    end;
...
begin
  if (Controller.FocusedNode = nil) and cxInRange(Controller.FocusedNodeIndex,
    0, FAbsoluteVisibleItems.Count - 1) then
    TcxTreeListNode(FAbsoluteVisibleItems[Controller.FocusedNodeIndex])
      .Focused := True;
end;
...
begin
  ...
  FAbsoluteItems.Capacity := Max(FAbsoluteItems.Capacity,
    DataController.NodesCount);
...
begin
  ...
  CheckStructureAbsoluteItems;
  if IsDestroying or (Root = nil) or (DataController = nil) then
    Exit;
  FAbsoluteVisibleItems.Capacity := Max(FAbsoluteVisibleItems.Capacity,
    DataController.NodesCount);
  ANode := Root.FFirst;
  ...
  begin
    ...
    begin
      Inc(ALevel);
      if (AHiddenLevel > 0) or (nsCollapsed in ANode.State) or
        not ANode.Visible or ANode.HiddenByFilter then
        Inc(AHiddenLevel);
      ANode := ANode.FFirst;
    end
    else if ANode.FNext <> nil then
...
procedure TcxCustomTreeList.ColumnSortOrderChanged(AColumn: TcxTreeListColumn;
  AShift: TShiftState);
var
  AIndex: Integer;
begin
  ...
  try
    ...
    begin
      if not IsLoading and (not OptionsBehavior.MultiSort or
      ...
      else if AColumn.SortOrder = soNone then
...
function TcxCustomTreeList.CreateExpressionProvider
  : TcxDataCustomExpressionProvider;
begin
  Result := TcxDataControllerSpreadSheetExpressionProvider.Create
    (DataController);
end;
...
function TcxCustomTreeList.CreateScrollbarAnnotations
  : TcxControlScrollbarAnnotationOptions;
begin
...
function TcxCustomTreeList.DoBeginDragNode(ANode: TcxTreeListNode): Boolean;
begin
  Result := True;
...
function TcxCustomTreeList.DoCanFocusNode(ANode: TcxTreeListNode): Boolean;
begin
  Result := True;
...
function TcxCustomTreeList.DoCanNodeCollapse(ANode: TcxTreeListNode): Boolean;
begin
...
function TcxCustomTreeList.DoCanNodeExpand(ANode: TcxTreeListNode): Boolean;
begin
  Result := ANode.HasChildren;
...
function TcxCustomTreeList.DoCanSelectNode(ANode: TcxTreeListNode): Boolean;
begin
  Result := True;
...
begin
  if IsLoading or IsDestroying or
    (BlockChangedEvent and (OptionsBehavior.ChangeDelay > 0)) or FPostChanged
  then
    Exit;
  try
    FPostChanged := True;
...
procedure TcxCustomTreeList.DoCompare(ANode1, ANode2: TcxTreeListNode;
...
begin
  ...
  begin
    if ACompare <> 0 then
      ACompare := DataController.CompareNodesByColumns(ANode1, ANode2,
        FSortedColumns)
  end
  else
...
procedure TcxCustomTreeList.DoFocusedItemChanged(APrevFocusedItem,
  AFocusedItem: TcxCustomInplaceEditContainer);
begin
  if Assigned(FOnFocusedColumnChanged) then
    FOnFocusedColumnChanged(Self, TcxTreeListColumn(APrevFocusedItem),
      TcxTreeListColumn(AFocusedItem));
end;

procedure TcxCustomTreeList.DoFocusedNodeChanged(APrevNode,
...
procedure TcxCustomTreeList.DoGetLevelImages(AInfo: TcxTreeListLevelInfo;
  ALevel: Integer);
var
...
procedure TcxCustomTreeList.DoGetNodeHeight(ANode: TcxTreeListNode;
  var AHeight: Integer);
begin
...
function TcxCustomTreeList.DoGetNodeImageIndex(ANode: TcxTreeListNode;
  AIndex: TcxTreeListImageIndexType): TcxImageIndex;
begin
  LockChanges := True;
  try
    if ANode = nil then
    ...
    begin
      ...
      if (AIndex = tlitSelectedIndex) and OptionsView.UseImageIndexForSelected
      then
...
procedure TcxCustomTreeList.DoGetPreviewHeight(ANode: TcxTreeListNode;
  var AHeight: Integer);
begin
  if Assigned(FOnGetNodePreviewHeight) then
    FOnGetNodePreviewHeight(Self, ANode,
      ANode.Values[Preview.Column.ItemIndex], AHeight);
end;

procedure TcxCustomTreeList.DoHotTrackNode(ANode: TcxTreeListNode;
...
procedure TcxCustomTreeList.DoInplaceEditContainerItemAdded
  (AItem: TcxCustomInplaceEditContainer);
begin
...
procedure TcxCustomTreeList.DoInplaceEditContainerItemRemoved
  (AItem: TcxCustomInplaceEditContainer);
begin
  if TcxTreeListColumn(AItem).StoreID = FNextStoreID - 1 then
    Dec(FNextStoreID);
  if TcxTreeListCellNavigator(Controller.Navigator).FCurrentNavigationColumn = AItem
  then
    TcxTreeListCellNavigator(Controller.Navigator)
      .FCurrentNavigationColumn := nil;
  inherited DoInplaceEditContainerItemRemoved(AItem);
end;

function TcxCustomTreeList.DoIsGroupNode(ANode: TcxTreeListNode): Boolean;
begin
  Result := (ANode <> nil) and (OptionsView.PaintStyle = tlpsCategorized) and
    ANode.HasChildren;
...
procedure TcxCustomTreeList.DoNodeChanged(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn);
begin
...
procedure TcxCustomTreeList.DoNodeCheckChanged(ANode: TcxTreeListNode;
  AState: TcxCheckBoxState);
begin
...
function TcxCustomTreeList.DoNodeCheckChanging(ANode: TcxTreeListNode;
  AState: TcxCheckBoxState): Boolean;
begin
  Result := True;
...
procedure TcxCustomTreeList.DoSetNodeFocused(ANode: TcxTreeListNode;
  AValue: Boolean; Shift: TShiftState = []);
begin
  if AValue then
    FocusedNode := ANode
  else if ANode.Focused then
    FocusedNode := nil;
end;

procedure TcxCustomTreeList.DoSetTopVisibleNode(ANode: TcxTreeListNode;
  ARaiseEvent: Boolean);
begin
...
procedure TcxCustomTreeList.SetImageList(const AValue: TCustomImageList;
  var AFieldValue: TCustomImageList; const AChangeLink: TChangeLink);
var
  AImages: TCustomImageList;
begin
...
begin
  ...
  if ((DX = 0) and (DY = 0)) or not HandleAllocated then
    Exit;
  if not Styles.CanOffsetContent or ((DX <> 0) and not ViewInfo.CanOffsetContent)
  then
  begin
  ...
  AMemBitmap := CreateCompatibleBitmap(Canvas.Handle, cxRectWidth(AUpdateRect),
    cxRectHeight(AUpdateRect));
  try
...
begin
  if (FChanges - SelectionChanges <> []) and not ChangesLocked then
    ForceLayoutChanged
  else if not ChangesLocked and (Bands <> nil) then
    Bands.RefreshInformation;
end;
...
begin
  ...
  for I := 0 to AbsoluteVisibleCount - 1 do
  begin
    if not DoCanSelectNode(AbsoluteVisibleItems[I]) then
      Continue;
...
begin
  if (not(tcSortOrder in Changes) and not Sorted) or IsLoading or
    IsDestroying or IsUpdating or (tcLoading in Changes) then
    Exit;
  if IsInserting then
...
begin
  if ViewInfo.CalculateInProcess or IsLoading or IsDestroying then
    Exit;
  BeforeUpdate;
  DoLayoutChanged;
end;
...
function TcxCustomTreeList.GetRecordPixelScrollPosition
  (ARecordIndex: Integer): Integer;
begin
...
function TcxCustomTreeList.
  IsRecordIndexBasedScrollbarAnnotationRendering: Boolean;
begin
...
function TcxCustomTreeList.GetOptionsBehaviorClass
...
function TcxCustomTreeList.GetOptionsFilterBoxClass
  : TcxControlOptionsFilterBoxClass;
begin
  Result := TcxTreeListFilterBox;
end;

function TcxCustomTreeList.GetOptionsFilteringClass
  : TcxControlOptionsFilteringClass;
begin
  Result := TcxTreeListFiltering;
end;

function TcxCustomTreeList.GetOptionsSelectionClass
  : TcxTreeListOptionsSelectionClass;
begin
...
function TcxCustomTreeList.GetTreeListCustomizingClass
  : TcxTreeListCustomizingClass;
begin
...
function TcxCustomTreeList.GetDateTimeHandlingClass
  : TcxControlOptionsDateTimeHandlingClass;
begin
...
function TcxCustomTreeList.GetFilterValueListClass
  : TcxControlFilterValueListClass;
begin
...
begin
  if Bands.Count = 0 then
    Result := Bands.Add
  else if Bands.BottomItemCount > 0 then
...
function TcxCustomTreeList.GetSameColumn(AColumn: TcxTreeListColumn)
  : TcxTreeListColumn;
begin
  Result := nil;
...
function TcxCustomTreeList.IsScrollBarBasedGestureScroll
...
begin
  Result := (LockUpdate > 0) or
    ([csLoading, csDestroying, csUpdating, csReading] * ComponentState <> []);
end;
...
procedure TcxCustomTreeList.InternalCopy(ANode, ADestNode: TcxTreeListNode);
var
...
procedure TcxCustomTreeList.InternalMove(ANode, ADestNode: TcxTreeListNode;
  AMode: TcxTreeListNodeAttachMode);
begin
  if (ANode = nil) or (ADestNode = nil) or ADestNode.HasAsParent(ANode) or
    ((ADestNode = ANode.Parent) and (AMode = tlamAddChild)) or
    (ANode = ADestNode) then
    Exit;
...
begin
  Result := (DataController.EditingNode = ANode) and (nsEditing in ANode.State);
end;
...
begin
  ...
  begin
    Result := DataController.IsItemSupportMultiThreading
      (SortedColumns[I].ItemIndex);
    if not Result then
      Exit;
  end;
end;

procedure TcxCustomTreeList.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
begin
  ...
  if ANode = FDragNode then
    FDragNode := nil;
  if not IsDestroying and (ANode <> Root) and (Root.FCount = 1) and
    (Root.FFirst = ANode) then
...
begin
  ...
  if not CheckNodesList or DoMoveToEvent(AttachNode, AttachMode, ANodes, IsCopy)
  then
    Exit;
...
procedure TcxCustomTreeList.DoOnColumnSizeChanged(AColumn: TcxTreeListColumn);
begin
...
function TcxCustomTreeList.DoWriteToText(AOnlySelected: Boolean;
  out AText: string; ACheckVisible: Boolean = False): Boolean;
var
  ANode: TcxTreeListNode;
begin
  ANode := Root.getFirstChild;
  ...
  while ANode <> nil do
  begin
    if (not AOnlySelected or ANode.Selected) and
      (not ACheckVisible or ANode.IsVisible) then
    begin
      if AText <> '' then
        AText := AText + dxCRLF;
...
begin
  Result := 0;
  if (Bands.FirstVisible <> nil) and (Bands.FirstVisible.FixedKind = tlbfLeft)
  then
    Inc(Result, Bands.FirstVisible.DisplayWidth);
  if (Bands.LastVisible <> nil) and (Bands.LastVisible.FixedKind = tlbfRight)
  then
...
function TcxCustomTreeList.InsertNode(ARelative: TcxTreeListNode;
  IsAppend: Boolean): Boolean;
var
  ANode: TcxTreeListNode;
const
  AMode: array [Boolean] of TcxTreeListNodeAttachMode = (tlamInsert,
    tlamAddChild);
begin
  HideEdit;
  BeginUpdate;
...
var
  AAnimated: Boolean;
begin
  if (ANode = nil) or not ANode.IsVisible then
    Exit;
  AAnimated := ViewInfo.IsScrollAnimationEnabled and
    not Controller.EditingController.EditingItemSetting;
  if FTopVisibleNode = nil then
  begin
    FTopVisibleNode := ANode;
    DoTopRecordIndexChanged;
  ...
  begin
    ...
    begin
      if (FTopVisibleNode = nil) or
        (ANode.VisibleIndex <= FTopVisibleNode.VisibleIndex) then
        ViewInfo.SetTopNodeWithOffset(ANode, 0, AAnimated)
      else if AAnimated then
...
begin
  if not OptionsBehavior.ShowHourGlass then
    Exit;
...
procedure TcxCustomTreeList.SortingStateChanged(AColumn: TcxTreeListColumn;
  AShift: TShiftState);
begin
end;
...
begin
  case AButtonIndex of
    ...
    NBDI_DELETE:
      if Controller.CanDeleteSelection and Controller.DeleteConfirmation then
        DeleteSelection;
    NBDI_EDIT:
      ShowEdit;
    NBDI_POST:
...
function TcxCustomTreeList.GetOnFindCriteriaBeforeChange
  : TcxDataFindCriteriaBeforeChangeEvent;
begin
  Result := DataController.OnFindCriteriaBeforeChange;
end;

function TcxCustomTreeList.GetOnFindCriteriaChanged
  : TcxDataFindCriteriaChangedEvent;
begin
...
function TcxCustomTreeList.GetAbsoluteItem(AIndex: Integer): TcxTreeListNode;
begin
...
function TcxCustomTreeList.GetAbsoluteVisibleItem(AIndex: Integer)
  : TcxTreeListNode;
begin
  CheckChanges;
...
procedure TcxCustomTreeList.SetAbsoluteItem(AIndex: Integer;
  AValue: TcxTreeListNode);
begin
  AbsoluteItems[AIndex].Assign(AValue);
end;

procedure TcxCustomTreeList.SetAbsoluteVisibleItem(AIndex: Integer;
  AValue: TcxTreeListNode);
begin
...
function TcxCustomTreeList.GetColumn(AIndex: Integer): TcxTreeListColumn;
begin
...
function TcxCustomTreeList.GetConditionalFormatting
...
begin
  CheckChanges;
  if InRange(ExpansionLevel, 0, FLevelsInfo.Count - 1) and
    (Bands.ExpandableBand <> nil) then
...
function TcxCustomTreeList.GetScrollbarAnnotations
  : TcxTreeListScrollbarAnnotationOptions;
begin
  Result := TcxTreeListScrollbarAnnotationOptions
    (inherited ScrollbarAnnotations);
end;

function TcxCustomTreeList.GetSelection(AIndex: Integer): TcxTreeListNode;
begin
  if (AIndex = 0) and not OptionsSelection.MultiSelect and (FocusedNode <> nil)
  then
    Result := FocusedNode
  else
...
function TcxCustomTreeList.GetSortedColumn(Index: Integer): TcxTreeListColumn;
begin
...
begin
  ...
  begin
    if Result.GetNextVisible <> nil then
      Result := Result.GetNextVisible
    else if Result.GetPrevVisible <> nil then
      Result := Result.GetPrevVisible;
  end;
end;
...
function TcxCustomTreeList.GetVisibleColumn(AIndex: Integer): TcxTreeListColumn;
begin
...
procedure TcxCustomTreeList.SetColumn(AIndex: Integer;
  Value: TcxTreeListColumn);
begin
  TcxTreeListColumn(ColumnsList[AIndex]).Assign(Value);
end;

procedure TcxCustomTreeList.SetDateTimeHandling
  (AValue: TcxTreeListDateTimeHandling);
begin
...
procedure TcxCustomTreeList.SetOnFindCriteriaBeforeChange
  (Value: TcxDataFindCriteriaBeforeChangeEvent);
begin
  DataController.OnFindCriteriaBeforeChange := Value;
end;

procedure TcxCustomTreeList.SetOnFindCriteriaChanged
  (Value: TcxDataFindCriteriaChangedEvent);
begin
...
procedure TcxCustomTreeList.SetScrollbarAnnotations
  (Value: TcxTreeListScrollbarAnnotationOptions);
begin
...
procedure TcxCustomTreeList.SetVisibleColumn(Index: Integer;
  AValue: TcxTreeListColumn);
begin
  VisibleColumns[Index].Assign(AValue);
end;

procedure TcxCustomTreeList.DoAddAbsoluteItem(ANode: TcxTreeListNode;
  AData: Pointer);
begin
  if ANode <> Root then
...
procedure TcxCustomTreeList.StoreTo(AStorageType: TcxStorageType;
...
function TcxCustomTreeList.GetConditionalFormattingProvider
  : TcxDataControllerConditionalFormattingProvider;
begin
...
procedure TcxCustomTreeList.GetPropertyValue(const AName: string;
  var AValue: Variant);
begin
  GetStoredPropertyValue(AName, AValue);
end;

procedure TcxCustomTreeList.SetPropertyValue(const AName: string;
  const AValue: Variant);
begin
...
var
  C: Integer;
const
  AcceptedCursors: array [Boolean, Boolean] of TCursor = ((crDrag, crDragCopy),
    (crcxDragMulti, crcxMultiDragCopy));
begin
  C := 0;
  ...
  begin
    DragCursor := AcceptedCursors[C > 1, Controller.IsDragCopy];
...
function TcxTreeListHitTest.AllowDesignMouseEvents(X, Y: Integer;
  AShift: TShiftState): Boolean;
begin
  Result := not TreeList.IsLocked and
    (inherited AllowDesignMouseEvents(X, Y, AShift) or HitAtHeader or
    CanSizing or HitAtButton or (HitCell is TcxTreeListFooterCellViewInfo) and
    not TcxTreeListFooterCellViewInfo(HitCell).Hidden);
end;
...
begin
  ...
  else if AItem is TcxTreeListFooterCellViewInfo then
...
begin
  if (HitBand = nil) or not(HitTestItem is TcxTreeListBandCellViewInfo) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      if cxInRange(HitX, AColumnRect.Left, AColumnRect.Right) and
        cxInRange(HitY, R.Top + MulDiv(cxRectHeight(R), ARow.LineOffset,
        AVisibleRowCount), R.Bottom) then
      begin
        FHitColumn := AColumn;
...
begin
  ...
  else if HitAtColumnHeader then
    AController.SelectObject(HitColumn, AShift)
  else if HitCell is TcxTreeListFooterSingleCellViewInfo then
    AController.SelectObject(TcxTreeListFooterSingleCellViewInfo(HitCell)
      .SummaryItem, AShift)
  else if HitCell is TcxTreeListFooterMultiItemsCellViewInfo then
    for I := 0 to TcxTreeListFooterMultiItemsCellViewInfo(HitCell)
      .SummaryItems.Count - 1 do
      if TcxTreeListFooterMultiItemsCellViewInfo(HitCell).SummaryItems[I].Visible
      then
      begin
        AController.SelectObject(TcxTreeListFooterMultiItemsCellViewInfo
          (HitCell).SummaryItems[I], AShift);
        AShift := [ssShift];
      end;
end;
...
const
  ACursors: array [Boolean, Boolean] of TCursor = ((crcxHorzSize, crcxVertSize),
    (crHSplit, crVSplit));
var
  P: TPoint;
...
begin
  ...
  if (TreeList.DragAndDropState = ddsInProcess) and
    not(TreeList.DragAndDropObject is TcxSizingDragAndDropObject) then
    Exit;
  if (Result = crDefault) and CanSizing then
    Result := ACursors[TreeList.DragAndDropState <> ddsNone, HitAtSizingVert]
  else if HitAtNode and not HitAtFooterArea and (HitTestItem <> nil) and
    (HitTestItem is TcxTreeListEditCellViewInfo) and HitColumn.Options.Focusing
  then
  begin
    P := HitPoint;
    ...
    Result := TcxCustomEditViewInfoAccess(ACellViewInfo.ViewInfo)
      .GetCurrentCursor(P);
  end;
end;
...
begin
  Result := HitAtFooter or HitAtFooterItem or HitAtGroupFooter or
...
begin
  Result := (HitAtBandHeader or HitAtColumnHeader or HitAtBandCustomizing or
    HitAtColumnCustomizing) and
    (HitTestItem is TcxTreeListCustomHeaderCellViewInfo);
end;
...
constructor TcxTreeListHitTestArea.Create(ALink: TcxTreeListCustomCellViewInfo);
begin
  FLink := ALink;
end;
...
procedure TcxTreeListHitTestArea.RightToLeftConversion
  (const AClientBounds: TRect);
begin
...
begin
  ...
  begin
    ...
    else if FLink.DisplayRect.Left < FLink.VisibleRect.Left then
      FArea := cxNullRect
    else
  ...
  if FLink.HasClipping then
    IntersectRect(FArea, FArea, cxRectInflate(FLink.VisibleRect, 0,
      cxtlHitDelta));
...
const
  AStates: array [TcxDragSizingDirection] of Integer = (tlhc_HitAtSizingHorz,
...
constructor TcxTreeListNodeSizingArea.Create
...
begin
  inherited Create(AOwner);
  FNodeViewData := TcxTreeListNodeViewData.Create
    (AOwner as TcxTreeListViewInfo, nil, 0);
end;
  ...
  begin
    if esoAutoHeight in Properties.GetSupportedOperations then
      Result := Max(TreeListViewInfo.DefaultEditHeight,
        Max(Column.GetEditHeight(Self), AHeight) +
        OptionsView.VertIncrement * 2)
    else
...
begin
  FColumn := AColumn;
  W := Column.DisplayWidth - OptionsView.HorzIncrement * 2;
  // # Exclude GridLines
  if AIsCategory then
  ...
  begin
    if IsEditing and IsEditAutoHeight and (AutoHeight = eahRow) and
      (Column = AColumn) and (ANode = TreeList.DataController.EditingNode) then
    begin
      CellValue := MultilineEdit.EditingValue;
      Result := CalculateHeight(MultilineEdit.CalculatedHeight +
...
function TcxFakeCellViewInfo.MeasureWidth(AColumn: TcxTreeListColumn;
  ANode: TcxTreeListNode): Integer;
var
  AIndent: Integer;
begin
...
constructor TcxTreeListCellPos.Create(ANode: TcxTreeListNode; AItem: TObject);
begin
  Item := AItem;
  Node := ANode;
end;
...
function TcxTreeListItemsCustomizeListBox.GetItemHeight(AItem: Integer)
  : Integer;
begin
  Result := Painter.ScaledHeaderHeight(TdxTextMeasurer.TextLineHeight(Font),
    ScaleFactor);
end;

function TcxTreeListItemsCustomizeListBox.GetOrigin
  (AHeader: TcxTreeListHeaderCellViewInfo): TPoint;
begin
  Result := ClientToScreen(AHeader.BoundsRect.TopLeft);
end;

procedure TcxTreeListItemsCustomizeListBox.KeyDown(var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  begin
    ...
    IntersectClipRect(Handle, BorderSize, BorderSize, ClientWidth - BorderSize,
...
begin
  if (csDestroying in ComponentState) or (DragAndDropState = ddsInProcess) then
    Exit;
  Items.BeginUpdate;
  try
    Clear;
...
begin
  Headers[AIndex].SetBounds(Rect(0, (AIndex - TopIndex) * ItemHeight,
    cxRectWidth(InnerListBox.ClientRect), (AIndex - TopIndex + 1) * ItemHeight),
    ClientRect);
end;
...
function TcxTreeListItemsCustomizeListBox.GetHeader(AIndex: Integer)
  : TcxTreeListHeaderCellViewInfo;
begin
  Result := Items.Objects[AIndex] as TcxTreeListHeaderCellViewInfo;
end;

function TcxTreeListItemsCustomizeListBox.GetPainter
...
function TcxTreeListColumnsCustomizeListBox.CreateCell
  : TcxTreeListHeaderCellViewInfo;
begin
  Result := TcxTreeListColumnHeaderCellViewInfo.CreateEx(TreeList, ClientRect,
    ClientRect);
end;

function TcxTreeListColumnsCustomizeListBox.GetDragAndDropObjectClass
  : TcxDragAndDropObjectClass;
begin
...
begin
  for I := 0 to TreeList.ColumnCount - 1 do
    if not TreeList.Columns[I].ActuallyVisible and not TreeList.Columns[I]
      .Options.Hidden then
...
function TcxTreeListBandsCustomizeListBox.CreateCell
  : TcxTreeListHeaderCellViewInfo;
begin
  Result := TcxTreeListBandHeaderCellViewInfo.CreateEx(TreeList, ClientRect,
    ClientRect);
end;

function TcxTreeListBandsCustomizeListBox.GetDragAndDropObjectClass
  : TcxDragAndDropObjectClass;
begin
...
begin
  for I := 0 to TreeList.Bands.Count - 1 do
    if not TreeList.Bands[I].ActuallyVisible and not TreeList.Bands[I].Options.Hidden
    then
...
constructor TcxTreeListCustomizationForm.CreateEx
  (AOwner: TcxTreeListCustomizing);
begin
  CreateNew(nil);
  FOwner := AOwner;
...
begin
  Customizing.FinishCustomizing(BoundsRect.TopLeft,
    ScaleFactor.Revert(cxSize(BoundsRect)));
...
begin
  if IsIconic(Application.Handle) then
    Visible := False
  else if not IsControlVisible(TreeList) then
    Customizing.Visible := False
...
function TcxTreeListCustomizing.PtInCustomizingBox(const APoint
  : TPoint): Boolean;
begin
...
procedure TcxTreeListCustomizing.Close(Sender: TObject;
...
begin
  ...
  begin
    ...
    FBandListBox := TcxTreeListBandsCustomizeListBox.CreateEx(FForm, TreeList,
      FTabSheetBands);
    // columns
    ...
    FHeaderListBox := TcxTreeListColumnsCustomizeListBox.CreateEx(FForm,
      TreeList, FTabSheetHeaders);
  end
  else
    FHeaderListBox := TcxTreeListColumnsCustomizeListBox.CreateEx(FForm,
      TreeList, FForm);
...
const
  UnusedItems: array [0 .. 4, 0 .. 1] of Integer = ((7, MF_BYPOSITION),
    (5, MF_BYPOSITION), (SC_MAXIMIZE, MF_BYCOMMAND),
...
begin
  ...
  for I := 0 to High(UnusedItems) do
    DeleteMenu(GetSystemMenu(FForm.Handle, False), UnusedItems[I, 0],
      UnusedItems[I, 1]);
  CreateControls;
end;

procedure TcxTreeListCustomizing.FinishCustomizing(const APosition: TPoint;
  const ASize: TSize);
begin
  FPosition := APosition;
...
procedure TcxTreeListCustomizing.SetControlParent(AControl,
  AParent: TWinControl);
begin
...
begin
  ...
  if IsGroupRow(ARecordIndex) then
    Result := 1
  else if cxInRange(ARecordIndex, 0, TreeList.AbsoluteVisibleCount - 1) then
...
begin
  ...
  begin
    ...
    begin
      ARowIndex := ANewRow;
      GoToNextNodeCell(AForward, TreeList.AbsoluteVisibleItems[ARowIndex],
        AColumn, ACellIndex);
    end
...
procedure TcxTreeListCellNavigator.AppendRecord(var ARowIndex,
  ACellIndex: Integer);
begin
  if not TreeList.OptionsData.Appending or
    (TreeList.IsInserting and not TreeList.DataController.IsValueChanged) then
    Exit;
  ...
  try
    ...
    if TcxTreeListController(Controller).CanInsertNode(True) and
      TreeList.InsertNode(nil, True) then
    begin
...
function TcxTreeListCellNavigator.GetCellContainer(ARowIndex,
...
function TcxTreeListCellNavigator.GetNode(ARowIndex: Integer): TcxTreeListNode;
begin
  Result := nil;
  if (ARowIndex < 0) or (ARowIndex >= TreeList.AbsoluteVisibleCount) then
    Exit;
  Result := TreeList.AbsoluteVisibleItems[ARowIndex];
end;

procedure TcxTreeListCellNavigator.Init(var ARowIndex, ACellIndex,
  ARowCount: Integer);
var
  ANode: TcxTreeListNode;
begin
...
procedure TcxTreeListCellNavigator.SetFocusCell(ARowIndex, ACellIndex: Integer;
  AShift: TShiftState);
var
  ANode: TcxTreeListNode;
begin
  ...
  if ANode.IsGroupNode then
    Controller.SetFocusedNodeItem(ANode,
      TreeList.OptionsView.GetCategorizedColumn)
  else
...
begin
  ...
  Result := (TreeList.VisibleColumnCount > 0) and
...
begin
  SaveCurrentNavigationColumn;
  if TreeList.OptionsSelection.CellSelect and ([ssShift, ssCtrl] * Shift = [])
    and (TreeList.VisibleColumnCount > 0) then
  begin
...
function TcxTreeListCellNavigator.GetCellIndex(AColumn: TcxTreeListColumn;
  ARow: TcxTreeListBandRow): Integer;
var
...
begin
  ...
  if AColumnIndex < 0 then
    AColumnIndex := 0;
  Result := ARow.VisibleItems[Min(AColumnIndex, ARow.VisibleItemCount - 1)
    ].VisibleIndex;
end;
...
begin
  ...
  if ARows.VisibleItemCount <= 1 then
    Exit;
  AIndex := ACellIndex;
...
begin
  ...
  Result := (ANode <> nil) and (AColumn <> nil) and
    (AColumn.Position.Band <> nil);
  if Result then
  begin
    if ANode.IsGroupNode then
      ACellIndex := 0
...
begin
  ...
  TcxCheckBoxProperties(FCheckBoxProperties).ValueChecked :=
    Integer(cbsChecked);
  TcxCheckBoxProperties(FCheckBoxProperties).ValueUnchecked :=
    Integer(cbsUnchecked);
  TcxCheckBoxProperties(FCheckBoxProperties).ValueGrayed := Integer(cbsGrayed);
  TcxCheckBoxPropertiesAccess(FCheckBoxProperties).ScaleFactor.Owner :=
    ScaleFactor;
  FCheckBoxViewInfo := TcxCustomEditViewInfo
    (FCheckBoxProperties.GetViewInfoClass.Create);
...
function TcxTreeListViewInfo.AddBackgroundPart(const ABounds: TRect)
  : TcxTreeListBackgroundCellViewInfo;
begin
...
begin
  ...
  if cxRectHeight(ABounds) > 0 then
    AddHeaderHitTestAreas(Result, ABand.CanSizing(dsdHorz),
      ABand.CanSizing(dsdVert));
...
procedure TcxTreeListViewInfo.AddBandsHeaders(AKind: TcxTreeListBandFixedKind;
  const ABounds, AClipBounds: TRect);
...
procedure TcxTreeListViewInfo.AddCell(ACellClass
  : TcxTreeListCustomCellViewInfoClass; const ABounds, AVisibleRect: TRect;
  var AInstance);
begin
...
function TcxTreeListViewInfo.AddColumnHeader(AColumn: TcxTreeListColumn;
  var ABounds: TRect; const AClipBounds: TRect)
  : TcxTreeListColumnHeaderCellViewInfo;
var
  R: TRect;
begin
  ...
  FHeaderCells.Add(Result);
  if cxRectHeight(ABounds) > 0 then
    AddHeaderHitTestAreas(Result, AColumn.CanSizing(dsdHorz),
      AColumn.CanSizing(dsdVert));
end;
...
procedure TcxTreeListViewInfo.AddColumnFooterItems(ADataNode,
  AttachNode: TcxTreeListNode; AColumn: TcxTreeListColumn;
...
var
  ...
  AOffset: Integer;
begin
  R := cxRectSetTop(ABounds, AClipRect.Top + AColumn.Position.Row.LineOffset *
    FooterLineHeight, AColumn.Position.LineCount * FooterLineHeight);
  AOffset := 0;
  ...
  begin
    ...
    TcxTreeListFooterMultiItemsCellViewInfo(ACell).Initialize(ADataNode,
      AttachNode, AColumn, AItems);
  end
  else
    for I := 0 to AItems.Count - 1 do
      if AItems[I].Visible then
      begin
        AddCell(TcxTreeListFooterSingleCellViewInfo,
          cxRectSetHeight(R, cxRectHeight(R) - AOffset), AClipRect, ACell);
        TcxTreeListFooterSingleCellViewInfo(ACell).Initialize(ADataNode,
          AttachNode, AColumn, AItems[I]);
...
begin
  ...
  if (Bands.VisibleRootRightFixedCount > 0) and
    ((R.Right <= ContentParts[tlbfRight].Left) or
...
const
  BandPart: array [Boolean] of TcxTreeListBandPart = (tlbpGroupFooter,
    tlbpFooter);
begin
  ...
  begin
    ...
    R := cxRectSetYPos(ABand.HeaderCell.DisplayRect, ABounds.Top,
      ABounds.Bottom);
    AClipRect := cxRectSetYPos(ABand.HeaderCell.VisibleRect, ABounds.Top,
      ABounds.Bottom);
    ...
    APart := AddBandPart(BandPart[ADataNode = Root], ABand, ADataNode,
      AAttachNode, R, AClipRect);
    ...
    begin
      ...
      if ((ADataNode = Root) and not AColumn.Options.Footer) or
        ((ADataNode <> Root) and not AColumn.Options.GroupFooter) then
        Continue;
      if cxRectIntersect(R, ACellsClipRect,
        cxRectSetYPos(AColumn.HeaderCell.DisplayRect, AClipRect.Top,
        AClipRect.Bottom)) then
      begin
        if (ABand = Bands.ExpandableBand) and (I <> 0) and not IsPrinting then
          Inc(R.Left);
        AddColumnFooterItems(ADataNode, AAttachNode, ABand.VisibleColumns[J], R,
          AClipRect);
      end;
    end;
  end;
...
function TcxTreeListViewInfo.AddImageIndent(ANode: TcxTreeListNode;
  AImages: TCustomImageList; var ABounds: TRect; const AClipRect: TRect;
  ACheckDynamic: Boolean = False): TcxTreeListIndentCellViewInfo;
begin
  if not ACheckDynamic or not TreeList.OptionsView.DynamicIndent then
    ...
    ABounds.Right := ABounds.Left + dxGetImageSize(AImages, ScaleFactor).cx +
      ScaleFactor.Apply(cxTreeListIndentOffsetSize) div 2;
...
function TcxTreeListViewInfo.AddIndent(ANode, AAttachNode: TcxTreeListNode;
  ALevel: Integer; AKind: TcxTreeListNodeIndentKind; var ABounds: TRect;
  const AClipRect: TRect): TcxTreeListIndentCellViewInfo;
begin
  if ALevel <> -1 then
  ...
  if (bBottom in GridLines) and ANode.IsLastVisible and
    ((AKind = nikFooter) or not HasFooter(ANode)) then
...
begin
  ABounds.Right := ABounds.Left + Max(LevelInfo[ANode.Level].Size.cx,
    GetCheckBoxSize.cx);
...
begin
  ...
  begin
    ABand := Bands.BottomItems[I];
    R := cxRectSetTop(ABand.HeaderCell.DisplayRect, RowOffset[0],
      AViewData.RowsHeight);
    AClipRect := cxRectSetTop(ABand.HeaderCell.VisibleRect, RowOffset[0],
      AViewData.RowsHeight);
    if I = 0 then
    ...
    AddBandPart(tlbpContent, ABand, AViewData.Node, AViewData.Node, R,
      AClipRect);
  end;
...
begin
  ...
  begin
    ...
    AHeight := TcxFakeCellViewInfo(FakeCell)
      .MeasureHeight(OptionsView.GetCategorizedColumn, AViewData.Node, True);
  end;
  ...
  Inc(AViewData.ContentBounds.Left, AViewData.IndentWidth + IndicatorWidth +
    Byte(bLeft in GridLines));
  AViewData.Bounds.Right := Min(AViewData.Bounds.Right, Bounds.Right) +
    Byte(HScrollSize > cxRectWidth(ClientRect));
  AViewData.ContentBounds.Right := AViewData.Bounds.Right;
  AddEditCell(AViewData, OptionsView.GetCategorizedColumn,
    AViewData.ContentBounds, AViewData.Bounds,
    TcxTreeListGroupNodeEditViewInfo);
...
procedure TcxTreeListViewInfo.AddNodeColumns
  (AViewData: TcxTreeListNodeViewData);
var
  I: Integer;
  AClipRect, R: TRect;
...
begin
  ...
  begin
    ...
    R := cxRectSetYPos(AColumn.HeaderCell.DisplayRect,
      RowOffset[ARow.LineOffset],
...
procedure TcxTreeListViewInfo.AddNodeFooters
  (AViewData: TcxTreeListNodeViewData);
var
  R, ABounds: TRect;
...
begin
  ...
  if not ANode.HasChildren or ANode.Expanded or
    (OptionsView.GroupFooters <> tlgfAlwaysVisible) and (ANode.Parent <> Root)
  then
    ANode := ANode.Parent
  else
    AIndex := 0;
  ...
  repeat
    ...
    begin
      R := cxRectSetLeft(ABounds,
        Bands.ExpandableBand.HeaderCell.DisplayRect.Left);
      ABounds := cxRectSetLeft(ABounds,
        Bands.ExpandableBand.HeaderCell.VisibleRect.Left);
      for ALevel := Byte(not OptionsView.ShowRoot) to AFooterLevel do
        AddIndent(AViewData.Node, AViewData.Node, ALevel, nikFooter, R,
          ABounds);
    end;
    Inc(AViewData.Height, GroupFooterHeight);
    if ANode.GetNextSiblingVisible <> nil then
      Break;
    ANode := ANode.Parent;
    AAttachLevelNode := AAttachLevelNode.Parent;
    Dec(AFooterLevel);
    Inc(AIndex);
  until (ANode = nil) or (ANode = Root)
  { or ((AAttachLevelNode.GetNextSiblingVisible <> nil) };
end;

procedure TcxTreeListViewInfo.AddNodeIndents
  (AViewData: TcxTreeListNodeViewData);
var
  I, ALevel: Integer;
...
begin
  ...
  for I := Byte(not OptionsView.ShowRoot) to ALevel do
    AIndent := AddIndent(AViewData.Node, AViewData.Node, I, nikLevel, ABounds,
      AClipRect);
  ...
  begin
    AIndent := AddImageIndent(AViewData.Node, LevelInfo[ALevel].StateImages,
      ABounds, AClipRect);
  ...
  begin
    AIndent := AddImageIndent(AViewData.Node, LevelInfo[ALevel].Images, ABounds,
      AClipRect, True);
...
function TcxTreeListViewInfo.AddNodePreview(AViewData: TcxTreeListNodeViewData)
  : TcxTreeListEditCellViewInfo;
var
...
begin
  ...
  if Preview.MaxLineCount > 0 then
    AHeight := Preview.MaxLineCount * PreviewLineHeight +
      ScaleFactor.Apply(cxTextOffset) + OptionsView.VertIncrement
  else
  ...
  if Preview.AutoHeight then
    AHeight := Min(AHeight, Result.CalculateEditHeight -
      ScaleFactor.Apply(cxTextOffset) - OptionsView.VertIncrement * 2);
  ...
  begin
    ...
    begin
      if Preview.AutoHeight then
        Inc(AHeight, OptionsView.VertIncrement * 2 +
          ScaleFactor.Apply(cxTextOffset));
...
begin
  ...
  Result.ContentBounds := cxRectSetTop(Result.ContentBounds, 0,
    Result.Height - Byte(bRight in GridLines));
...
procedure TcxTreeListViewInfo.NodeScrollAnimated(ATopNode: TcxTreeListNode;
  ATopRecordPixelScrollOffset: Integer);
begin
  if ATopNode <> nil then
    FNodeScrollAnimationHelper.SetPosition
      (TcxTreeListNodePixelScrollData.Create(ATopNode,
...
begin
  ...
  begin
    for I := 0 to ColumnCount - 1 do
      ...
      begin
        FDefaultCellHeight := Max(FDefaultCellHeight,
          GetColumnDefaultEditHeight(Columns[I]));
  ...
  if FDefaultCellHeight = 0 then
    FDefaultCellHeight := GetFontHeight(TreeList.Font) +
      ScaleFactor.Apply(cxTextOffset);
...
begin
  ...
  begin
    ...
    for I := 0 to Bands.VisibleItemCount - 1 do
      FBandHeaderLineHeight := Max(FBandHeaderLineHeight,
        Painter.ScaledHeaderHeight(GetFontHeight(Styles.GetBandHeaderParams
        (Bands.VisibleItems[I]).Font), ScaleFactor));
  ...
  begin
    if OptionsView.Headers then
      FHeaderLineHeight := Max(FHeaderLineHeight,
        Painter.ScaledHeaderHeight(GetFontHeight(Styles.GetColumnHeaderParams
        (Columns[I]).Font), ScaleFactor));
    if OptionsView.Footer and Columns[I].Options.Footer then
      FFooterLineHeight := Max(FFooterLineHeight,
        GetFooterLineHeight(Styles.GetColumnFooterParams(Columns[I],
        Root).Font));
  end;
  if FHeaderLineHeight = 0 then
    FHeaderLineHeight := Painter.ScaledHeaderHeight
      (GetFontHeight(TreeList.Font), ScaleFactor);
...
begin
  ...
  FTreeLineColor := ColorToRgb(cxGetNativeColor(OptionsView.TreeLineColor,
...
begin
  ...
  Inc(FContentWidth, ALengths[tlbfLeft] + ALengths[tlbfNone] +
    ALengths[tlbfRight]);
  if Bands.VisibleLeftFixedCount > 0 then
  begin
  ...
  FContentParts[tlbfNone] := cxRectSetLeft(ABounds, ABounds.Left,
    ALengths[tlbfNone]);
  if Bands.VisibleRightFixedCount > 0 then
  begin
    FContentParts[tlbfRight] := cxRectSetLeft(ABounds,
      Min(FContentParts[tlbfNone].Right, Bounds.Right - ALengths[tlbfRight] -
      FixedSeparatorWidth), ALengths[tlbfRight] + FixedSeparatorWidth);
  ...
  for AKind := Low(TcxTreeListBandFixedKind)
    to High(TcxTreeListBandFixedKind) do
...
begin
  ...
  begin
    ...
    AClipBounds := cxRectSetTop(ABand.HeaderCell.VisibleRect, ATop,
      HeadersHeight);
    ...
    begin
      ...
      ARowBounds := cxRectSetYPos(ABounds, RowOffset[ARow.LineOffset],
        RowOffset[ARow.LineOffset + ARow.LineCount]);
      for AColIndex := 0 to ARow.VisibleItemCount - 1 do
      begin
        AColumn := ARow.VisibleItems[AColIndex];
        ARowBounds.Bottom := RowOffset
          [ARow.LineOffset + AColumn.Position.LineCount];
...
procedure TcxTreeListViewInfo.CalculateHitTest(AHitTest: TcxTreeListHitTest);
var
  I: Integer;
begin
  Validate;
  ...
  begin
    for I := 0 to Count - 1 do
      if NodeViewData[I].Cells.CalculateHitTest(AHitTest) then
        Break;
  end;
  for I := 0 to HitTestCells.Count - 1 do
    if TcxTreeListHitTestArea(HitTestCells[I]).GetHitTest(AHitTest) then
      Break;
  (NavigatorSiteViewInfo as TcxTreeListNavigatorSiteViewInfo)
    .GetHitTest(AHitTest);
  (FindPanelViewInfo as TcxTreeListFindPanelViewInfo).GetHitTest(AHitTest);
  FilterBoxViewInfo.GetHitTest(AHitTest);
end;

procedure TcxTreeListViewInfo.CalculateHorzScrollInfo(AAvailableWidth,
...
begin
  ...
  if (TreeList.GetScrollbarMode = sbmHybrid) and
    (TreeList.IsScrollBarActive(sbVertical)) and
    (Bands.VisibleRightFixedCount = 0) then
  ...
  begin
    FHScrollArea := cxRectSetXPos(ContentParts[tlbfNone],
      Max(ContentParts[tlbfNone].Left, Bounds.Left),
      Min(ContentParts[tlbfNone].Right, Bounds.Right));
    if Bands.VisibleRightFixedCount > 0 then
      FHScrollArea.Right := ContentParts[tlbfRight].Left -
        FixedSeparatorWidth - 1;
    if Bands.VisibleLeftFixedCount > 0 then
...
begin
  ...
  begin
    ...
    for J := 0 to FHeaderCells.Count - 1 do
      ACell.CheckNeighbors(TcxTreeListHeaderCellViewInfo(FHeaderCells[J]
        ), AShift);
  end;
end;
...
begin
  ...
  begin
    ARow := Columns[I].Position.Row;
    AHeight := TcxFakeCellViewInfo(FakeCell).MeasureHeight(Columns[I],
      AViewData.Node, False);
    AFinishRow := ARow.LineOffset + Columns[I].Position.LineCount;
    RowOffset[AFinishRow] :=
      Max(RowOffset[AFinishRow] - RowOffset[ARow.LineOffset],
      RowOffset[ARow.LineOffset] + AHeight);
  end;
...
begin
  ...
  begin
    ...
    else if AOffset.Y <= ALastPos then
    begin
      Inc(FNodesVisibleCount);
  ...
  begin
    ...
    if FHorzBackgroundCell = nil then
      FHorzBackgroundCell := AddBackgroundPart(cxRectSetYPos(ContentBounds,
        AOffset.Y, ALastPos))
    else
      FHorzBackgroundCell.SetBounds(cxRectSetYPos(ContentBounds, AOffset.Y,
        ALastPos), Bounds);
  end
  else if FHorzBackgroundCell <> nil then
    FHorzBackgroundCell.SetBounds(cxEmptyRect, cxEmptyRect);
  FContentHeight := FContentHeight - ContentBounds.Top;
  if (NodesVisibleCount > 0) and
    (TreeList.FTopVisibleNode <> NodeViewData[0].Node) then
  begin
...
procedure TcxTreeListViewInfo.CalculatePixelScrollInfo
  (var ANode: TcxTreeListNode; var APixelScrollNodeOffset: Integer;
  AMaxTopNode: TcxTreeListNode; AMaxPixelScrollTopNodeOffset: Integer;
  ADelta: Integer; out AOverPan: Boolean);

  function NextRecordNeeded(ANode: TcxTreeListNode; var ASize: Integer)
    : Boolean;
  var
    ANodeSize: Integer;
  begin
...
begin
  ...
  begin
    ...
    if ANode = AMaxTopNode then
      APixelScrollNodeOffset := Max(APixelScrollNodeOffset,
        AMaxPixelScrollTopNodeOffset);
...
begin
  ...
  begin
    ...
    FHScrollArea := TdxRightToLeftLayoutConverter.ConvertRect
      (FHScrollArea, Bounds);
...
begin
  ...
  if AScrollPos < 0 then
    AScrollPos := 0
  else if AScrollPos + APage > AMax then
    AScrollPos := AMax - APage;
  if AScrollCode = scPosition then
    cxRange(AScrollPos, 0, AMax);
...
begin
  ...
  else if not cxRectIsEmpty(ABounds) then
...
procedure TcxTreeListViewInfo.DoSetTopNodeWithOffset(ANode: TcxTreeListNode;
  APixelScrollNodeOffset: Integer; ARaiseEvent: Boolean);
begin
  if (ANode <> TopNode) or (APixelScrollNodeOffset <> PixelScrollNodeOffset)
  then
  begin
    TreeList.ShowTouchScrollUI(TreeList, True);
...
function TcxTreeListViewInfo.GetPixelScrollOffset(AFirstNode: TcxTreeListNode;
  AFirstNodeOffset: Integer; ALastNode: TcxTreeListNode;
  ALastNodeOffset: Integer): Integer;
var
  ANode: TcxTreeListNode;
...
function TcxTreeListViewInfo.GetFilterBoxViewInfoClass
  : TcxControlFilterBoxViewInfoClass;
begin
  Result := TcxTreeListFilterBoxViewInfo;
end;

function TcxTreeListViewInfo.GetFindPanelViewInfoClass
  : TcxControlFindPanelViewInfoClass;
begin
...
begin
  Result := GetFontHeight(AFont) + 2 *
    (Painter.FooterCellBorderSize + cxHeaderTextOffset) +
    TdxVisualRefinements.Padding.ScaledHeight(ScaleFactor);
...
begin
  if (TreeList.GetScrollbarMode = sbmHybrid) and
    TreeList.IsScrollBarActive(sbHorizontal) then
...
function TcxTreeListViewInfo.GetLastNodeByTopNode(ATopNode: TcxTreeListNode;
  ATopNodePixelScrollOffset: Integer): TcxTreeListNode;
var
  AOverPan: Boolean;
begin
  ...
  CalculatePixelScrollInfo(Result, ATopNodePixelScrollOffset, nil, -1,
    -GetPixelScrollContentSize, AOverPan);
end;
...
function TcxTreeListViewInfo.GetMultilineEditorBounds(const ACellEditBounds
  : TRect; ACalculatedHeight: Integer;
...
begin
  ...
  begin
    ...
    ANodeIndex := TreeList.FocusedNode.VisibleIndex -
      TreeList.TopVisibleNode.VisibleIndex;
    for I := ANodeIndex to NodesVisibleCount - 1 do
    begin
      ANodeViewData := NodeViewData[I];
      Result.Bottom := Min(ANodeViewData.GetRealContentBounds.Bottom,
        AMaxBottom);
...
function TcxTreeListViewInfo.GetNavigatorSiteViewInfoClass
  : TcxCustomNavigatorSiteViewInfoClass;
begin
  Result := TcxTreeListNavigatorSiteViewInfo;
end;

function TcxTreeListViewInfo.GetNodeContentOffset
  (ANode: TcxTreeListNode): Integer;
begin
...
function TcxTreeListViewInfo.GetNodePixelScrollSize
  (ANode: TcxTreeListNode): Integer;
var
  AViewData: TcxTreeListNodeViewData;
  ACells: TcxCustomControlCells;
...
procedure TcxTreeListViewInfo.GetPixelScrollTopNodeAndOffsetByBottomNode
  (ABottomNode: TcxTreeListNode; out ATopNode: TcxTreeListNode;
  out ATopNodePixelScrollOffset: Integer);
var
  AOverPan: Boolean;
begin
  ...
  CalculatePixelScrollInfo(ATopNode, ATopNodePixelScrollOffset, nil, -1,
    GetPixelScrollContentSize, AOverPan);
end;
...
begin
  ...
  if OptionsView.GroupFooters = tlgfAlwaysVisible then
    Result := (ANode.HasChildren and not ANode.Expanded) or
      (not ANode.HasChildren and (ANode.GetNextSiblingVisible = nil) and
      (ANode.Parent <> Root))
  else if OptionsView.GroupFooters = tlgfVisibleWhenExpanded then
    Result := not ANode.Expanded and (ANode.GetNextSiblingVisible = nil) and
      (ANode.Level > 0);
end;
...
begin
  ...
  if LevelInfo[ANode.Level].StateImages = nil then
    ...
    Result := (not OptionsView.DynamicFocusedStateImages or ANode.Focused) and
      cxInRange(ANode.StateIndex, 0,
      LevelInfo[ANode.Level].StateImages.Count - 1);
end;

procedure TcxTreeListViewInfo.InflateBoundsForGridLines(var ABounds,
  AClipRect: TRect);
begin
...
procedure TcxTreeListViewInfo.InitializeRows(const ANewHeight: Integer;
  var AViewData: TcxTreeListNodeViewData);
var
  I: Integer;
  AScaleFactor: Double;
begin
  ...
  begin
    ...
    if ANewHeight = 0 then
      Exit;
    for I := 1 to ColumnsRowCount do
      FRows^[I] := MulDiv(ANewHeight, I, ColumnsRowCount) +
        AViewData.RowsOffset;
  end
  else if AViewData.RowsHeight <> ANewHeight then
  begin
...
begin
  ...
  TreeList.SetScrollBarInfo(sbVertical, 0, AVScrollSize - 1, 1, VScrollPage,
    FCacheVScrollPos, True, True);
  TreeList.SetScrollBarInfo(sbHorizontal, 0, HScrollSize - 1, HScrollInc,
    HScrollPage, HScrollPos, True, CanHScrollBarHide);
end;
...
begin
  Result := IsRecordPixelScrollMode and
    (TreeList.AbsoluteVisibleCount < GetPixelScrollContentSize);
end;
...
begin
  Result := (TreeList.OptionsBehavior.RecordScrollMode = rsmByPixel) or
    (TreeList.OptionsBehavior.RecordScrollMode = rsmDefault) and
    (cxIsTouchModeEnabled or TreeList.IsScrollAnimationEnabled);
end;
...
begin
  ...
  begin
    ...
    begin
      AWasFullyVisible := (ANode.VisibleIndex = ATopNode.VisibleIndex) and
        (ATopNodePixelScrollOffset = 0);
      SetTopNodeWithOffset(ANode, 0, True);
    end
    else
    begin
      GetPixelScrollTopNodeAndOffsetByBottomNode(ANode, ANextTopNode,
        ANextTopNodePixelScrollOffset);
      if (ANextTopNode.VisibleIndex > ATopNode.VisibleIndex) or
        (ANextTopNode.VisibleIndex = ATopNode.VisibleIndex) and
        (ANextTopNodePixelScrollOffset < ATopNodePixelScrollOffset) then
      begin
  ...
  else if IsRecordPixelScrollMode and
    ((ANode.ViewData = nil) or (ANode.ViewData.GetRealBounds.Bottom >
    ContentBounds.Bottom)) then
    DoSetLastNode(ANode, False)
  else
  begin
    if (ANode = TreeList.TopVisibleNode) then
      Exit;
    APos := VScrollPos + 1;
    while (ANode.ViewData <> nil) and
      (ANode.ViewData.GetRealBounds.Bottom > ContentBounds.Bottom) and
...
begin
  ...
  AWidth := AColumn.DisplayWidth - Painter.HeaderBorderSize * 2 -
    ScaleFactor.Apply(cxTextOffset) * 2;
  ...
  begin
    ...
    Result := Max(Result, AColumn.Caption.Glyph.Height +
      ScaleFactor.Apply(cxTextOffset) * 2);
  end;
  ...
  with TcxScreenCanvas.Create do
    try
      ...
      Result := Max(Result, Painter.ScaledHeaderHeight(R.Bottom - R.Top,
        ScaleFactor));
    finally
      Free;
    end;
...
begin
  FreeAndNil(FCheckBoxViewData);
  FCheckBoxViewData := FCheckBoxProperties.CreateViewData
    (TreeList.EditStyle, True);
  FCheckBoxViewData.ScaleFactor.Assign(ScaleFactor);
end;

procedure TcxTreeListViewInfo.SetTopNodeWithOffset(ANode: TcxTreeListNode;
  APixelScrollNodeOffset: Integer; AAnimated: Boolean);
begin
  if AAnimated then
...
procedure TcxTreeListViewInfo.ScrollContentByGesture
...
begin
  if ADelta = 0 then
    Exit;
  if (TopNode = nil) or (Count = 0) then
    Exit;
  ANode := TopNode;
  APixelScrollNodeOffset := PixelScrollNodeOffset;
  CalculatePixelScrollInfo(ANode, APixelScrollNodeOffset,
    FMaxPixelScrollTopNode, FMaxPixelScrollTopNodeOffset, ADelta, AOverPan);
...
procedure TcxTreeListViewInfo.ScrollHorz(AScrollCode: TScrollCode;
  var AScrollPos: Integer);
var
  AHScrollPos: Integer;
begin
  if IsColumnScrollAnimationActive and (AScrollCode in [scLineUp .. scPageDown])
  then
...
procedure TcxTreeListViewInfo.ScrollVert(AScrollCode: TScrollCode;
  var AScrollPos: Integer);
...
begin
  ...
  begin
    ...
    begin
      ...
      ATopNodePixelScrollOffset := FNodeScrollAnimationHelper.TargetPosition.
        ScrollOffset;
    end
    else
    begin
    ...
    case AScrollCode of
      ...
      scPageUp:
        begin
          ...
          begin
            GetPixelScrollTopNodeAndOffsetByBottomNode(ATopNode, ANextTopNode,
              ANextTopNodePixelScrollOffset);
            if (ANextTopNode = ATopNode) and (ATopNodePixelScrollOffset = 0)
            then
              ANextTopNode := ATopNode.GetPrevVisible
            else
              ANextTopNodePixelScrollOffset := 0;
      ...
      scPageDown:
        begin
          ...
          begin
            ALastNode := GetLastNodeByTopNode(ATopNode,
              ATopNodePixelScrollOffset);
            if ALastNode.VisibleIndex > FMaxPixelScrollTopNode.VisibleIndex then
            begin
              ANextTopNode := FMaxPixelScrollTopNode;
              ANextTopNodePixelScrollOffset := FMaxPixelScrollTopNodeOffset;
            end
            else if (ALastNode = ATopNode) and (ALastNode.GetNextVisible <> nil)
            then
    ...
    case AScrollCode of
        ...
        begin
          ...
          begin
            if AScrollCode = scPageUp then
              AHeight := Max(1, GetPixelScrollContentSize - NodeViewData
                [NodesVisibleCount - 1].Height);
            CalculateNodesViewDataBackward(TopNode.GetPrevVisible,
              False, AHeight);
          end
          else if APrevPixelScrollOffset <> 0 then
            TreeList.LayoutChanged;
        end;
      scLineDown:
        begin
          ...
          begin
            ...
            FPixelScrollNodeOffset := 0;
            AOriginDisplacement := -APrevPixelScrollOffset;
            if (ANode <> TopNode) and
              (not ANode.IsLastVisible or (Count > NodesVisibleCount)) then
            begin
            ...
            else if (ANode = TopNode) and not ANode.IsLastVisible then
      ...
      scPageDown:
        begin
          FPixelScrollNodeOffset := 0;
          AOriginDisplacement :=
            -(NodeViewData[0].Height + APrevPixelScrollOffset);
...
begin
  ...
  for I := 0 to Count - 1 do
  begin
    if not NodeViewData[I].Update(False) then
      Continue;
  ...
  begin
    DoCalculate;
    if OptionsView.CellAutoHeight then
      InvalidateRect(cxRectSetYPos(Bounds, TopNonContentHeight,
        Bounds.Bottom - BottomNonContentHeight));
  end;
...
var
  AOverPan: Boolean;
begin
  FMaxPixelScrollTopNode := TreeList.AbsoluteVisibleItems
    [TreeList.AbsoluteVisibleCount - 1];
  FMaxPixelScrollTopNodeOffset := -GetNodePixelScrollSize
    (FMaxPixelScrollTopNode) - GetHScrollBarAreaHeight;
...
procedure TcxTreeListViewInfo.ValidateScrollPos(var APosition: Integer;
  const APage, AMax: Integer);
begin
...
begin
  ...
  begin
    ...
    for I := 0 to Count - 1 do
    begin
      NodeViewData[I].Scroll(DX, 0, Bands.VisibleLeftFixedCount > 0,
        Bands.VisibleRightFixedCount > 0);
    end;
    RecalculateHitTestCells;
...
procedure TcxTreeListViewInfo.DoSetLastNode(ANode: TcxTreeListNode;
  AAnimated: Boolean);
var
  ATopNode: TcxTreeListNode;
  ATopNodePixelScrollOffset: Integer;
begin
  GetPixelScrollTopNodeAndOffsetByBottomNode(ANode, ATopNode,
    ATopNodePixelScrollOffset);
...
begin
  ...
  if (Result = 0) or IsPrinting then
    Exit;
  Result := Result + Painter.FooterSeparatorSize + Painter.FooterCellOffset +
    Painter.FooterBorderSize;
end;
...
begin
  Result := (IndentBand <> nil) and (IndentBand = Bands.BottomItems[0]);
end;
...
function TcxTreeListViewInfo.GetLevelInfo(ALevel: Integer)
  : TcxTreeListLevelInfo;
begin
  Result := TreeList.LevelsInfo[ALevel];
end;

function TcxTreeListViewInfo.GetNodeViewData(AIndex: Integer)
  : TcxTreeListNodeViewData;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      AIsAnyNodeHeightChanged := AIsAnyNodeHeightChanged or
...
procedure TcxTreeListViewInfo.SetRowOffset(AIndex: Integer;
...
begin
  ...
  begin
    ...
    TreeList.TopVisibleNode := TreeList.AbsoluteVisibleItems
      [Min(TreeList.AbsoluteVisibleCount - 1, Max(0, APosition))];
end;
...
function TcxTreeListColumnStyles.GetContentParams(ANode: TcxTreeListNode)
  : TcxViewParams;
begin
  GetViewParams(ecs_Content, ANode, nil, Result);
end;

procedure TcxTreeListColumnStyles.GetDefaultViewParams(Index: Integer;
  ...
  case Index of
    ecs_Content:
      begin
        ...
        else if Band <> nil then
          Band.Styles.DoGetContentParams(TcxTreeListNode(AData),
            Column, AParams)
        else
          TreeList.Styles.DoGetContentParams(TcxTreeListNode(AData),
            Column, AParams);
      end;
    tlcs_Footer:
      if Band <> nil then
...
procedure TcxTreeListColumnPosition.SetPosition(AColIndex, ARowIndex: Integer;
  IsInsertRow: Boolean = False);
begin
  SetPositionEx(BandIndex, AColIndex, ARowIndex, IsInsertRow);
end;

procedure TcxTreeListColumnPosition.SetPositionEx(ABandIndex, AColIndex,
  ARowIndex: Integer; IsInsertRow: Boolean = False);
begin
  TreeList.BeginUpdate;
  try
    ...
    if Band <> nil then
    begin
      if not IsInsertRow or not cxInRange(ARowIndex, 0, Band.BandRows.Count)
      then
        RowIndex := ARowIndex
      else
...
begin
  Result := Column.IsLoading or Column.IsUpdating or TreeList.IsRestoring or
    TreeList.IsLoading or TreeList.IsUpdating;
end;
...
begin
  if Column.IsReading or Column.IsUpdating or
    (TreeList.IsRestoring and not Column.IsDestroying and
    ((Band = nil) or not Band.FIsDestroying)) then
  begin
    FBandIndex := AValue;
    Exit;
  end;
  if BandIndex = AValue then
    Exit;
...
begin
  if IsUpdating then
    FColIndex := AValue
  else if (FBand <> nil) and (ColIndex >= 0) and (AValue >= 0) then
  begin
...
begin
  ...
  begin
    if (FBand = nil) or (RowIndex = AValue) then
      Exit;
    if AValue >= 0 then
...
function TcxTreeListColumnFilterPopupOptions.GetIncrementalFilteringOptionsClass
  : TcxCustomEditContainerItemFilterPopupIncrementalFilteringOptionsClass;
begin
  Result := TcxTreeListColumnFilterPopupIncrementalFilteringOptions;
end;

function TcxTreeListColumnFilterPopupOptions.GetIncrementalFiltering
  : TcxTreeListColumnFilterPopupIncrementalFilteringOptions;
begin
  Result := TcxTreeListColumnFilterPopupIncrementalFilteringOptions
    (inherited IncrementalFiltering);
end;

procedure TcxTreeListColumnFilterPopupOptions.SetIncrementalFiltering
  (AValue: TcxTreeListColumnFilterPopupIncrementalFilteringOptions);
begin
...
function TcxTreeListColumnOptions.GetExcelFilterPopupOptionsClass
  : TcxCustomEditContainerItemExcelFilterPopupOptionsClass;
begin
  Result := TcxTreeListColumnExcelFilterPopupOptions;
end;

function TcxTreeListColumnOptions.GetFilterPopupOptionsClass
  : TcxCustomEditContainerItemFilterPopupOptionsClass;
begin
...
function TcxTreeListColumnOptions.GetExcelFilterPopup
  : TcxTreeListColumnExcelFilterPopupOptions;
begin
  Result := TcxTreeListColumnExcelFilterPopupOptions
    (inherited ExcelFilterPopup);
end;

function TcxTreeListColumnOptions.GetFilterPopup
  : TcxTreeListColumnFilterPopupOptions;
begin
...
procedure TcxTreeListColumnOptions.SetEditAutoHeight
  (AValue: TcxItemInplaceEditAutoHeight);
begin
...
procedure TcxTreeListColumnOptions.SetExcelFilterPopup
  (AValue: TcxTreeListColumnExcelFilterPopupOptions);
begin
  inherited ExcelFilterPopup := AValue;
end;

procedure TcxTreeListColumnOptions.SetFilterPopup
  (AValue: TcxTreeListColumnFilterPopupOptions);
begin
...
function TcxTreeListSummaryItem.FormatValue(const AValue: Variant;
...
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('AlignHorzAssigned', ReadAlignHorz, WriteAlignHorz,
    AlignHorzAssigned);
  Filer.DefineProperty('AlignVertAssigned', ReadAlignVert, WriteAlignVert,
    AlignVertAssigned);
end;
...
function TcxTreeListSummaryItem.GetValueFormat(AValueType: TcxSummaryValueType;
  const AValue: Variant; AIsFooter: Boolean): string;
begin
  Result := inherited GetValueFormat(AValueType, AValue, AIsFooter);
  if Result = '' then
    Result := cxDataGetDataSummaryValueDefaultFormat(AValueType, AValue,
      AIsFooter);
end;

procedure TcxTreeListSummaryItem.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
begin
  Result := FAlignHorz;
  if AlignHorzAssigned or (Column.PropertiesValue = nil) then
    Exit;
...
begin
  Result := FAlignVert;
  if AlignVertAssigned or (Column.PropertiesValue = nil) then
    Exit;
...
begin
  ...
  begin
    GroupFooterSummaryItems := TcxTreeListColumnSummary(Source)
...
begin
  inherited Create(AOwner);
  FFooterSummaryItems := TcxTreeListSummaryItems.Create(Self,
    TcxTreeListSummaryItem);
  FGroupFooterSummaryItems := TcxTreeListSummaryItems.Create(Self,
    TcxTreeListSummaryItem);
end;
...
constructor TcxTreeListSummaryItems.Create(ASummary: TcxTreeListColumnSummary;
  AItemClass: TcxTreeListSummaryItemClass);
begin
...
function TcxTreeListSummaryItems.GetItemByKind(AKind: TcxSummaryKind)
  : TcxTreeListSummaryItem;
var
  I: Integer;
begin
...
function TcxTreeListSummaryItems.GetItem(AIndex: Integer)
  : TcxTreeListSummaryItem;
begin
...
procedure TcxTreeListDeprecatedColumnSummaryFooter.Assign(Source: TPersistent);
var
...
begin
  if (TreeList = nil) or TreeList.IsLoading or TreeList.IsDestroying then
    Exit;
  TreeList.ValidateStates;
  ...
  try
    ...
    begin
      AWidth := Max(AWidth, TcxFakeCellViewInfo(TreeList.ViewInfo.FakeCell)
        .MeasureWidth(Self, ANode));
      ANode := ANode.GetNextVisible;
      Dec(ACount);
    end;
...
procedure TcxTreeListColumn.MakeVisible;
  ...
  begin
    ...
    if TreeList.FocusedNode = nil then
      Exit;
    Result := TreeList.ViewInfo.GetLevelContentOffset
      (TreeList.FocusedNode.Level);
    if TreeList.FocusedNode.Level > 0 then
      Result := Result - TreeList.ViewInfo.GetLevelContentOffset
        (TreeList.FocusedNode.Level - 1);
  end;
  ...
  begin
    if Abs(APosition) <= 2 then
      APosition := 0;
    if TreeList.ViewInfo.IsScrollAnimationEnabled and
      not TreeList.Controller.EditingController.EditingItemSetting then
...
begin
  ...
  begin
    ...
    if (cxRectWidth(AEditRect) > cxRectWidth(R)) and (FocusedCellViewInfo <> nil)
    then
    begin
    ...
    begin
      ...
      begin
        if not HasIndent then
          SetPosition(TreeList.ViewInfo.HScrollPos +
            (AEditRect.Right - R.Right))
        else if R.Right - AEditRect.Left < AMinWidth then
        begin
      ...
      else if AEditRect.Left < R.Left then
    ...
    begin
      ...
      begin
        ...
        else if AEditRect.Right - R.Left < AMinWidth then
        begin
          if cxRectWidth(AEditRect) <= cxRectWidth(R) then
            SetPosition(TreeList.ViewInfo.HScrollPos + AEditRect.Left -
              cxRectWidth(R))
      ...
      else if AEditRect.Right > R.Right then
...
begin
  if (TreeList <> nil) and TreeList.IsRefreshFields then
    Exit;
  inherited Changed;
  if TreeList <> nil then
...
begin
  Result := (Options.EditAutoHeight <> ieahNone) and
    (TreeList.GetEditAutoHeight <> eahNone) and
...
begin
  ...
  begin
    ...
    Result := cxTextWidth(TreeList.Styles.GetColumnHeaderParams(Self).Font,
      Caption.Text) + (AOffset.Left + AOffset.Right) + cxHeaderTextOffset;
    if not Caption.Glyph.Empty and (Byte(Caption.GlyphAlignHorz) < 2) then
      Inc(Result, dxGetImageSize(Caption.Glyph, ScaleFactor).cx +
        ScaleFactor.Apply(cxTextOffset));
    if SortOrder <> soNone then
      Inc(Result, TreeList.LookAndFeelPainter.ScaledSortingMarkAreaSize
        (ScaleFactor).X);
  end;
  if Options.Footer and TreeList.OptionsView.Footer and
    (Summary.FooterVisibleCount > 0) then
    Result := Max(Result, GetSummaryBestFitSize(TreeList.Root,
      Summary.FooterSummaryItems));
end;

function TcxTreeListColumn.GetOptionsClass
...
begin
  ...
  begin
    ...
    AItemWidth := cxTextWidth(AViewParams.Font, AText) +
      ScaleFactor.Apply
      ((cxHeaderTextOffset + TreeList.LookAndFeelPainter.FooterCellBorderSize +
      TreeList.LookAndFeelPainter.FooterCellOffset) * 2 +
      TdxVisualRefinements.Padding.Width);
...
function TcxTreeListColumn.CanSizing(ADirection
  : TcxDragSizingDirection): Boolean;
begin
  if ADirection = dsdVert then
    Result := Options.VertSizing and
      TreeList.OptionsCustomizing.ColumnVertSizing
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if I = Position.VisibleColIndex then
          Continue;
...
begin
  if ADirection = dsdVert then
    Result := cxRectSetTop(TreeList.ClientRect, HeaderCell.DisplayRect.Top +
      TreeList.ViewInfo.HeaderLineHeight)
  else
  begin
    if not HeaderCell.IsRightToLeftConverted then
      Result := cxRectSetLeft(TreeList.ClientRect, HeaderCell.DisplayRect.Left +
        GetRealMinSize, GetAvailableMaxWidth - GetRealMinSize)
    else
      Result := cxRectSetRight(TreeList.ClientRect, HeaderCell.DisplayRect.Right
        - GetRealMinSize, GetAvailableMaxWidth - GetRealMinSize);
  end;
end;
...
procedure TcxTreeListColumn.SetSizeDelta(ADirection: TcxDragSizingDirection;
  ADelta: Integer);
begin
  if ADelta = 0 then
    Exit;
...
procedure TcxTreeListColumn.GetPropertyValue(const AName: string;
  var AValue: Variant);
begin
...
procedure TcxTreeListColumn.SetPropertyValue(const AName: string;
  const AValue: Variant);
begin
  case GetPropertyIndex(AName) of
    0:
...
procedure TcxTreeListColumn.ChangeSortOrder(ASortOrder: TcxDataSortOrder;
  AShift: TShiftState);
begin
...
procedure TcxTreeListColumn.DoGetDisplayText(ARecordIndex: TdxNativeInt;
  var AText: string);
begin
  if ARecordIndex <> 0 then
...
function TcxTreeListColumn.DoGetNodeDisplayText(ANode: TcxTreeListNode;
...
function TcxTreeListColumn.DoOnGetDisplayText(ANode: TcxTreeListNode;
  AsText: Boolean = False; AValueOnly: Boolean = False): Variant;
var
  AText: string;
  AValue: Variant;
  AUseValue: Boolean;
...
begin
  ...
  AUseValue := (AProperties.GetEditValueSource(False) = evsValue) or
    (AProperties.GetEditValueSource(False) = evsText) and
    AProperties.IsValueFormattedByProperties or VarIsNull(AValue) and
    (not IsExpression or (DataController.GetNodeErrorCode(ANode,
    ItemIndex) = 0));
  if AUseValue then
...
function TcxTreeListColumn.GetEditingProperties
...
begin
  Result := Visible and (Position.Band <> nil) and
    Position.Band.ActuallyVisible;
end;
...
begin
  ...
  Result := (TreeList.Controller.ForcingWidthColumn = Self) or
    not Options.Sizing;
  ...
  begin
    ...
    Result := (APos = Position) or not AForcingColumn.IsRight and
      (Position.Row = APos.Row) and
      (Position.VisibleColIndex < APos.VisibleColIndex);
  end;
end;
...
begin
  if (AValue < 0) or (AValue >= TreeList.ContainerList.Count) or
    (AValue = ItemIndex) then
    Exit;
  TreeList.BeginUpdate;
  try
...
begin
  ...
  begin
    FSortIndex := AValue;
    if IsLoading or TreeList.IsRestoring then
      Exit;
    if AValue >= 0 then
    begin
...
procedure TcxTreeListColumn.SetValueByNode(ANode: TcxTreeListNode;
  const AValue: Variant);
begin
...
function TcxTreeListBandStyles.GetContentParams(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn): TcxViewParams;
begin
  if AColumn <> nil then
    Result := AColumn.Styles.GetContentParams(ANode)
...
procedure TcxTreeListBandStyles.DoGetContentParams(ANode: TcxTreeListNode;
  AColumn: TcxTreeListColumn; var AParams: TcxViewParams);
var
  ACellPos: TcxTreeListCellPos;
begin
...
procedure TcxTreeListBandStyles.GetDefaultViewParams(Index: Integer;
  AData: TObject; out AParams: TcxViewParams);
begin
  inherited GetDefaultViewParams(Index, AData, AParams);
  case Index of
    tlbs_Content:
      ...
      begin
        ...
        begin
          if Item <> nil then
            TreeList.Styles.DoGetContentParams(Node,
              TcxTreeListColumn(Item), AParams)
          else
    ...
    tlbs_Footer:
      if AData is TcxTreeListCellPos then
        TreeList.Styles.GetViewParams(tlsv_ColumnFooter,
          TcxTreeListCellPos(AData).Item, nil, AParams)
      else
        TreeList.Styles.DoGetBandFooterParams(Band,
          TcxTreeListNode(AData), AParams);
    tlbs_Header:
...
begin
  ...
  if TreeList.IsDestroying or not Result then
    Exit;
  Result := (AIndex = -1) or
    ((AIndex >= 0) and (AIndex < TreeList.Bands.Count));
  if not Result or (AIndex = -1) then
    Exit;
  ABand := TreeList.Bands[AIndex];
  if not ABand.HasAsParent(Band) then
    Exit;
  AColIndex := ColIndex;
  TreeList.BeginUpdate;
  try
...
begin
  ...
  if not CheckBandIndex(AValue) or (BandIndex = AValue) then
    Exit;
  TreeList.BeginUpdate;
  try
    if ParentBand <> nil then
...
begin
  ...
  Result := (AForcingBand <> nil) and
    ((Self = AForcingBand) or not AForcingBand.IsLastAsChild and
    (ParentBand = AForcingBand.ParentBand) and
...
function TcxTreeListBand.CanDropColumnAt(const APoint: TPoint;
  out ARowIndex, AColIndex: Integer): Boolean;
begin
  Result := True;
  ARowIndex := 0;
  AColIndex := 0;
...
begin
  if Expandable = tlbeNotExpandable then
    Exit;
  if (ABand = nil) or not(ABand.Expandable = tlbeExpandable) and
    (Expandable <> tlbeDefault) then
    ABand := Self;
end;
...
begin
  if (Bands = nil) or TreeList.IsUpdating or TreeList.IsDestroying then
    Exit;
...
var
  I: Integer;
begin
  if TreeList.IsDestroying or (FColumns = nil) then
    Exit;
  TreeList.BeginUpdate;
  try
    for I := ColumnCount - 1 downto 0 do
...
begin
  if ADirection = dsdVert then
    Result := Options.VertSizing and TreeList.OptionsCustomizing.BandVertSizing
      and IsBottom
  else
    Result := TreeList.OptionsCustomizing.BandHorzSizing and Options.Sizing;
end;

function TcxTreeListBand.GetSizingBoundsRect(ADirection
  : TcxDragSizingDirection): TRect;
begin
  if ADirection = dsdVert then
    Result := cxRectSetTop(TreeList.ClientRect, HeaderCell.DisplayRect.Top +
      TreeList.ViewInfo.BandHeaderLineHeight)
  else
  begin
    if not HeaderCell.IsRightToLeftConverted then
      Result := cxRectSetLeft(TreeList.ClientRect,
        Max(HeaderCell.DisplayRect.Left + ActualMinWidth + IndentWidth,
        HeaderCell.VisibleRect.Left))
    else
      Result := cxRectSetRight(TreeList.ClientRect,
        Min(HeaderCell.DisplayRect.Right - ActualMinWidth - IndentWidth,
        HeaderCell.VisibleRect.Right))
  end;
end;

function TcxTreeListBand.GetSizingIncrement(ADirection
  : TcxDragSizingDirection): Integer;
begin
  Result := 1;
end;
...
procedure TcxTreeListBand.SetSizeDelta(ADirection: TcxDragSizingDirection;
  ADelta: Integer);
begin
  if ADelta = 0 then
    Exit;
...
function TcxTreeListBand.GetProperties(AProperties: TStrings): Boolean;
var
  I: Integer;
begin
...
procedure TcxTreeListBand.GetPropertyValue(const AName: string;
  var AValue: Variant);
begin
  case GetPropertyIndex(AName) of
    0:
...
procedure TcxTreeListBand.SetPropertyValue(const AName: string;
  const AValue: Variant);
begin
  case GetPropertyIndex(AName) of
    0:
...
begin
  Result := FHasEmptyArea or (BandRows.LineCount <> Bands.ColumnsLineCount);
end;
...
begin
  ...
  begin
    Result := ABand = Self;
    if ABand.ChildVisibleBands.Count > 0 then
      ABand := TcxTreeListBand(ABand.ChildVisibleBands
        [ABand.ChildVisibleBands.Count - 1])
    else
      ABand := nil;
  end;
end;
...
begin
  Result := IsFirstInGroup and (Bands.GetFirstVisibleIndex(FixedKind) = 0);
end;
...
begin
  Result := IsLastInGroup and (Bands.GetLastVisibleIndex(FixedKind)
    = Bands.VisibleItemCount - 1);
end;
...
begin
  ...
  Result := not Options.Sizing or IsBandFixedDuringSizing or
    ((AForcingColumn <> nil) and
    (AForcingColumn.Position.Band.ParentBandWidthAssigned = Self));
  if not Result and (TreeList.Controller.ForcingWidthBand <> nil) and
    ...
    Result := Position.ColIndex <= TreeList.Controller.ForcingWidthBand.
      Position.ColIndex;
end;
...
begin
  if not IsRoot or TreeList.IsDestroying then
    Exit;
...
procedure TcxTreeListBand.SetVisibleColumn(AIndex: Integer;
  AValue: TcxTreeListColumn);
begin
...
begin
  ...
  else if ABand.FixedKind = tlbfRight then
...
procedure TcxTreeListBands.Adjust(ABands: TList = nil; AWidth: Integer = 0);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    try
      ...
      for I := 0 to ABands.Count - 1 do
        TcxTreeListBand(ABands[I]).CalculatedWidth := AAutoWidth.Items[I]
          .AutoWidth;
    finally
      AAutoWidth.Free;
    end;
...
begin
  for I := 0 to BottomItemCount - 1 do
    if BottomItems[I].BandRows.VisibleItemCount > ARow.VisibleIndex then
      BottomItems[I].BandRows.VisibleItems[ARow.VisibleIndex]
        .AssignColumnsWidth;
end;

function TcxTreeListBands.CanExpandableLeftMostOnly: Boolean;
begin
  Result := TreeList.OptionsView.IsCategorizedPaint or TreeList.Preview.Active;
end;
...
procedure TcxTreeListBands.Notify(Item: TCollectionItem;
...
procedure TcxTreeListBands.GetPropertyValue(const AName: string;
  var AValue: Variant);
begin
end;

procedure TcxTreeListBands.SetPropertyValue(const AName: string;
  const AValue: Variant);
begin
end;

// IcxStoredParent
function TcxTreeListBands.CreateChild(const AObjectName,
  AClassName: string): TObject;
begin
...
procedure TcxTreeListBands.DeleteChild(const AObjectName: string;
  AObject: TObject);
begin
  AObject.Free;
end;
...
function TcxTreeListBandRows.CheckRowIndex(ARowIndex: Integer)
  : TcxTreeListBandRow;
begin
...
begin
  if (ARowIndex >= 0) and (ARowIndex < Count) and (Items[ARowIndex].Count = 0)
  then
  begin
...
function TcxTreeListBandRows.GetVisibleItem(AIndex: Integer)
  : TcxTreeListBandRow;
begin
...
procedure TcxTreeListFiltering.RunCustomizeDialog
  (AColumn: TcxTreeListColumn = nil);
begin
  inherited RunCustomizeDialog(AColumn);
end;

function TcxTreeListFiltering.GetItemExcelPopupClass
  : TcxControlOptionsFilteringItemExcelPopupClass;
begin
  Result := TcxTreeListFilteringColumnExcelPopupOptions;
end;

function TcxTreeListFiltering.GetItemPopupClass
  : TcxControlOptionsFilteringItemPopupClass;
begin
  Result := TcxTreeListFilteringColumnPopupOptions;
end;

function TcxTreeListFiltering.GetColumnExcelPopup
  : TcxTreeListFilteringColumnExcelPopupOptions;
begin
  Result := TcxTreeListFilteringColumnExcelPopupOptions(ItemExcelPopup);
end;

function TcxTreeListFiltering.GetColumnPopup
...
procedure TcxTreeListFiltering.SetColumnExcelPopup
  (AValue: TcxTreeListFilteringColumnExcelPopupOptions);
begin
  ItemExcelPopup := AValue;
end;

procedure TcxTreeListFiltering.SetColumnPopup
  (AValue: TcxTreeListFilteringColumnPopupOptions);
begin
  ItemPopup := AValue;
end;

procedure TcxTreeListFiltering.SetColumnPopupMode
  (AValue: TdxFilterPopupWindowMode);
begin
...
function TcxTreeListOptionsView.GetHeaderFilterButtonShowMode
...
function TcxTreeListOptionsView.GetShowColumnFilterButtons
...
procedure TcxTreeListOptionsView.SetCategorizedColumn
  (AValue: TcxTreeListColumn);
begin
...
procedure TcxTreeListOptionsView.SetHeaderFilterButtonShowMode
  (AValue: TcxFilterButtonShowMode);
begin
...
procedure TcxTreeListOptionsView.SetShowColumnFilterButtons
  (Value: TcxShowFilterButtons);
begin
...
begin
  if Source is TcxTreeListOptionsCustomizing then
    ...
    begin
      ...
      Self.BandsQuickCustomizationMaxDropDownCount :=
        BandsQuickCustomizationMaxDropDownCount;
      Self.BandsQuickCustomizationMultiColumnMode :=
        BandsQuickCustomizationMultiColumnMode;
      Self.BandsQuickCustomizationShowCommands :=
      ...
      Self.ColumnsQuickCustomizationMaxDropDownCount :=
        ColumnsQuickCustomizationMaxDropDownCount;
      Self.ColumnsQuickCustomizationMultiColumnMode :=
        ColumnsQuickCustomizationMultiColumnMode;
      Self.ColumnsQuickCustomizationShowCommands :=
...
procedure TcxTreeListOptionsCustomizing.SetColumnFiltering
...
procedure TcxTreeListOptionsBehavior.SetEditAutoHeight
  (AValue: TcxInplaceEditAutoHeight);
begin
  if FEditAutoHeight <> AValue then
  begin
    FEditAutoHeight := AValue;
...
procedure TcxTreeListOptionsBehavior.SetFooterHints(AValue: Boolean);
begin
...
procedure TcxTreeListOptionsBehavior.SetHeaderHints(AValue: Boolean);
begin
...
procedure TcxTreeListOptionsBehavior.SetIncSearchItem
  (AValue: TcxTreeListColumn);
begin
  inherited IncSearchItem := AValue;
end;

procedure TcxTreeListOptionsBehavior.SetRecordScrollMode
  (AValue: TcxRecordScrollMode);
begin
...
begin
  ...
  begin
    ...
    FMultiThreadedSorting := TcxTreeListOptionsData(Source)
...
begin
  Result := dcoAnsiSort in TcxCustomTreeList(GetOwner).DataController.Options;
end;

function TcxTreeListOptionsData.GetCaseInsensitive: Boolean;
begin
  Result := dcoCaseInsensitive in TcxCustomTreeList(GetOwner)
