...
{$ENDIF}
  ...
  cxDBEdit, cxControls, cxClasses, cxDataStorage, cxCustomData, cxDB, cxDBData,
  cxDataUtils,
...
type
  ...
  protected
    ...
    procedure SetNodeParentValue(ANode: TcxDBTreeListNode;
    ...
    property UpdateInserting: Boolean read FUpdateInserting
      write FUpdateInserting;
  public
  ...
  published
    ...
    property ImageIndexField: string read FImageIndexFieldName
      write SetImageIndexFieldName;
    property ParentField: string read FParentKeyFieldName
      write SetParentKeyFieldName;
    property KeyField: string read FKeyFieldName write SetKeyFieldName;
    property StateIndexField: string read FStateIndexFieldName
      write SetStateIndexFieldName;
  end;
  ...
  private
    ...
    function GetDefaultValuesProviderProperty
      : TcxDBTreeListDefaultValuesProvider;
  ...
  protected
    ...
    function GetDefaultValuesProviderClass
      : TcxCustomEditDefaultValuesProviderClass; override;
    ...
    function IsDisplayFormatDefinedForColumn(AIgnoreSimpleCurrency
      : Boolean): Boolean;
    ...
    property DefaultValuesProvider: TcxDBTreeListDefaultValuesProvider
      read GetDefaultValuesProviderProperty;
  public
    procedure Assign(Source: TPersistent); override;
  ...
  published
    property DataBinding: TcxDBItemDataBinding read GetDataBinding
  ...
  public
    ...
    property TreeList: TcxCustomDBTreeList read GetTreeList;
  published
    property AutoCalcKeyValue: Boolean read FAutoCalcKeyValue
      write FAutoCalcKeyValue default False;
  ...
  protected
    ...
    function FindLoadedNode(const AParentValue: Variant;
      var AIndex: Integer): Boolean;
    function FindNode(var ANode: TcxDBTreeListNode;
      const AValue: Variant): Boolean;
    function GetImageIndex(ANode: TcxTreeListNode;
      AIndexType: TcxTreeListImageIndexType): Integer; override;
    ...
    procedure LoadLevel(ANode: TcxDBTreeListNode; const AParentValue: Variant;
      ANewNodes: TList);
    procedure LoadLevelBack(AParentValue: Variant);
    function LoadValues(const AKeyValue, AParentKeyValue: Variant)
      : TcxDBTreeListNode;
  ...
  TcxDBTreeListGetUniqueKeyValueEvent = procedure(Sender: TcxCustomDBTreeList;
    var KeyValue: Variant) of object;
  TcxDBTreeListInitInsertingRecordEvent = procedure(Sender: TcxCustomDBTreeList;
    AFocusedNode: TcxDBTreeListNode; var AHandled: Boolean) of object;
  ...
  protected
    ...
    function InsertNode(ARelative: TcxTreeListNode; IsAppend: Boolean)
      : Boolean; override;
    function InternalFindNode(const AKeyValue: Variant;
      AStartNode: TcxTreeListNode = nil; AExpandedOnly: Boolean = False;
      AForward: Boolean = True; ARefreshAll: Boolean = True)
      : TcxDBTreeListNode; virtual;
    ...
    property IsRefreshValues: Boolean read FIsRefreshValues
      write FIsRefreshValues;
    ...
    property UpdatingNode: TcxDBTreeListNode read FUpdatingNode
      write FUpdatingNode;
    property OnGetUniqueKeyValue: TcxDBTreeListGetUniqueKeyValueEvent
      read FOnGetUniqueKeyValue write FOnGetUniqueKeyValue;
    property OnInitInsertingRecord: TcxDBTreeListInitInsertingRecordEvent
      read FOnInitInsertingRecord write FOnInitInsertingRecord;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    function FindNodeByKeyValue(const AKeyValue: Variant;
      AStartNode: TcxTreeListNode = nil; AExpandedOnly: Boolean = False;
      AForward: Boolean = True): TcxDBTreeListNode;
    function GetColumnByFieldName(const AFieldName: string)
      : TcxDBTreeListColumn;
    //
...
uses
  cxTLStrs, cxEditDataRegisteredRepositoryItems, Math, RTLConsts;
...
constructor TcxTreeListDataLink.Create(ADataController
  : TcxDBTreeListDataController);
begin
  inherited Create;
...
procedure TcxTreeListDataLink.DataEvent(Event: TDataEvent; Info: TdxNativeInt);
begin
...
begin
  ...
  if not DataController.IsDataLoaded and not DataController.LoadingInProcess and
    (TreeList.IsRefresh or IsDataSetBusy) then
  ...
  if DataBusy or TreeList.IsRefresh or (DataSet.State = dsEdit) or
    (DataSet.State = dsSetKey) then
    Exit;
...
var
  ...
  ADone: Boolean;
begin
  if not Assigned(Field) or not Assigned(Field^) then
    Exit;
  ADone := False;
  for I := 0 to TreeList.ColumnCount - 1 do
    if TcxDBTreeListColumn(TreeList.Columns[I]).DataBinding.Field = Field^ then
    begin
      DataController.FocusControl(TcxDBTreeListColumn(TreeList.Columns[I])
        .ItemIndex, ADone);
      if ADone then
        Break;
    end;
...
begin
  if not(DataSet.State in dsEditModes) or
    ((DataSet.State = dsInsert) and (TreeList.UpdatingNode = nil)) or
    DataController.UpdateInserting then
    Exit;
  if (TreeList.FocusedNode <> nil) and (Field = DataController.DBParentField)
  { and TreeList.FocusedNode.Inserting } then
  begin
    TcxDBTreeListNode(TreeList.FocusedNode).ParentKeyValue :=
      GetFieldValue(DataController.DBParentField);
    AParentNode := TreeList.InternalFindNode
      (TcxDBTreeListNode(TreeList.FocusedNode).ParentKeyValue, nil, False,
      True, False);
...
begin
  ...
  begin
    ...
    if not DataSet.Modified or ((DataSet.State = dsInsert) and
      not IsValueChanged) then
      DataSet.Cancel
    else
      DataSet.Post;
  end;
...
procedure TcxDBTreeListDataController.CreateAllItems(AMissingItemsOnly
  : Boolean = False);
var
  I: Integer;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        if AMissingItemsOnly and
          (TreeList.GetColumnByFieldName(DataSet.Fields[I].FieldName) <> nil)
        then
          Continue;
...
begin
  if (DataSet = nil) or (DataSet.State = dsEdit) then
    Exit;
...
procedure TcxDBTreeListDataController.FocusControl(AItemIndex: Integer;
  var Done: Boolean);
begin
...
function TcxDBTreeListDataController.GetFilterItemFieldName
...
function TcxDBTreeListDataController.LocateByKey(const AKeyValue
  : Variant): Boolean;
var
...
begin
  ...
  begin
    ...
    try
      CheckBrowseMode;
      Result := TreeList.UpdateFocused or VarEquals(DBKeyField.Value, AKeyValue)
        or DataSet.Locate(KeyField, AKeyValue, []);
      if Result then
      begin
...
function TcxDBTreeListDataController.LocateByKeyEx(const AKeyValue
  : Variant): Boolean;
begin
  Result := False;
  if DataLink.Active then
  begin
    ...
    try
      CheckBrowseMode;
      Result := TreeList.UpdateFocused or DataSet.Locate(KeyField,
        AKeyValue, []);
    finally
      TreeList.DataLocate := False;
      TreeList.DataBusy := False;
...
begin
  ...
  if (DataSet.State in [dsInsert, dsEdit]) and (AColumn.Field <> nil) and
    (EditingNode <> nil) then
  begin
    if (AEditValueSource = evsText) or Assigned(AColumn.Field.OnSetText) then
      AColumn.Field.Text := VarToStr(AValue)
    else if (AEditValueSource = evsKey) and (AColumn.Field.KeyFields <> '') then
...
function TcxDBTreeListDataController.CanLocateByNode(AOldNode,
...
begin
  ...
  if not Result or ((AOldNode = ANode) and
    (GetFieldValue(DBKeyField) = ANode.KeyValue)) then
    Exit;
  Result := not((ANode = TreeList.UpdatingNode) and DataLink.Active and
...
begin
  if not TreeList.OptionsData.SyncMode then
    Exit;
    ...
    try
      ...
      try
        ...
        begin
          ...
          else if (F = AKeyField) or (F = AParentField) or
        ...
        begin
          ...
          for I := ANodes.Count - 1 downto 1 do
            if TcxTreeListNode(ANodes[I])
              .HasAsParent(TcxTreeListNode(ANodes[I - 1])) then
              ANodes.Delete(I);
          // make Buffer List
...
function TcxDBTreeListDataController.GetFieldByName(const AFieldName
  : string): TField;
begin
  Result := nil;
  if DataSet <> nil then
...
begin
  ...
  else if not TreeList.OptionsData.SmartLoad then
    try
      AList := TcxDBTreeListLoader(Loader)
        .CloneList(TreeList.AbsoluteItemsList);
      try
      ...
      if VarType(Result) in [varSmallint, varInteger, varSingle, varDouble,
        varCurrency] then
...
function TcxDBTreeListDataController.IsFieldSupportsExpression
...
begin
  ...
  begin
    ...
    Result := (AColumn.DataBinding.FieldName = '') and
      (AColumn.DataBinding.Field = nil);
  end;
end;
...
procedure TcxDBTreeListDataController.SetFieldName(var AField: string;
  const AValue: string);
begin
...
begin
  if TreeList.DataLocate or not CanLocateByNode
    (TcxDBTreeListNode(TreeList.Controller.PrevFocusedNode),
    TcxDBTreeListNode(ANode)) then
    Exit;
  try
    TreeList.DataLocate := True;
    CheckBrowseMode;
    if (ANode = nil) or not VarEquals(TcxDBTreeListNode(ANode).KeyValue,
      GetFieldValue(DBKeyField)) then
...
begin
  if not(DataLink.Active and DataLink.DataSet.Active) or TreeList.IsLoading then
    Exit;
  TreeList.FindNodeById;
  TreeList.UpdateData;
end;
...
begin
  Result := TcxDBItemDataBinding(Owner).IsDisplayFormatDefinedForColumn
...
procedure TcxDBItemDataBinding.CheckFreeNotification(AComponent: TComponent);
begin
...
function TcxDBItemDataBinding.GetDefaultValuesProviderClass
...
function TcxDBItemDataBinding.IsDisplayFormatDefinedForColumn
...
begin
  TcxCustomDataFieldAccess(DataController.Fields[EditContainer.ItemIndex])
    .UpdateExpressionState;
end;
...
function TcxDBItemDataBinding.QueryInterface(const IID: TGUID; out Obj)
  : HResult;
begin
...
function TcxDBItemDataBinding.GetDefaultValuesProviderProperty
  : TcxDBTreeListDefaultValuesProvider;
begin
  Result := inherited DefaultValuesProvider as
    TcxDBTreeListDefaultValuesProvider;
end;
...
begin
  ...
  begin
    ...
    else if (AEditValueSource = evsKey) and (Field.KeyFields <> '') then
      Result := Field.DataSet.FieldValues[Field.KeyFields]
    else if not Field.IsNull then
  ...
  try
    ...
    begin
      if Assigned(DBImageIndexField) then
        TcxDBTreeListNode(ANode).FImageIndexes[tlitImageIndex] :=
          GetImageIndexAsInteger(DBImageIndexField);
      if Assigned(DBStateIndexField) then
        TcxDBTreeListNode(ANode).FImageIndexes[tlitStateIndex] :=
          GetImageIndexAsInteger(DBStateIndexField);
    end;
    ...
    DataController.IsValueChanged := APrevIsChanged or
      (not ANode.Inserting and (DataSet.State in dsEditModes));
  finally
...
begin
  ...
  begin
    ...
    begin
      ...
      if not ANode.Deleting and FindLoadedNode(ANode.ParentKeyValue, AIndex) and
        ANode.CanMove(TcxTreeListNode(Loaded.List[AIndex]), tlamAddChildFirst)
      then
        InternalMove(ANode, TcxTreeListNode(Loaded.List[AIndex]),
          tlamAddChildFirst);
      ANode := APrev;
    end;
    for I := 0 to Loaded.Count - 1 do
  ...
  begin
    ...
    if (KeyField <> ParentKeyField) and IsExistKeyRoot(AParentKeyValue) and
      not FindLoadedNode(AKeyValue, I) and
      (AParentKeyValue <> TreeList.RootValue) then
      LoadLevelBack(AParentKeyValue);
  end
  else
    LoadLevel(TcxDBTreeListNode(ALoadingNode), TcxDBTreeListNode(ALoadingNode)
      .KeyValue, nil);
end;

function TcxDBTreeListLoader.FindLoadedNode(const AParentValue: Variant;
  var AIndex: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
...
function TcxDBTreeListLoader.GetImageIndex(ANode: TcxTreeListNode;
  AIndexType: TcxTreeListImageIndexType): Integer;
begin
  Result := -1;
end;
...
begin
  ...
  repeat
    ...
    if not DataSet.Locate(KeyFieldName, AParentValue, []) or
      (GetFieldValue(KeyField) = GetFieldValue(ParentKeyField)) then
      Break;
...
begin
  if not DataSet.Locate(ParentKeyFieldName, AParentValue, []) then
    Exit;
  AList := TList.Create;
  try
    ...
    begin
      ...
      begin
        ...
        else if IsChildrenExist(AItem) then
          HasChildren.Add(AItem);
      end;
    end
    else
    begin
      ...
      begin
        ...
        else if IsChildrenExist(ANode) then
          HasChildren.Add(ANode);
  ...
  try
    repeat
      ...
      begin
        ...
        if not DataSet.Locate(KeyFieldName, AParentValue, []) then
          Break;
        AParentValue := GetFieldValue(ParentKeyField);
      end;
    until False;
...
function TcxDBTreeListLoader.LoadValues(const AKeyValue, AParentKeyValue
  : Variant): TcxDBTreeListNode;
var
  I: Integer;
  AFound: Boolean;
begin
  ...
  begin
    ...
    begin
      if (Result.ParentKeyValue <> AParentKeyValue) or
        ((Result.Parent <> Root) and (TcxDBTreeListNode(Result.Parent).KeyValue
        <> AParentKeyValue)) then
      begin
        if FindLoadedNode(AParentKeyValue, I) and
          Result.CanMove(TcxTreeListNode(Loaded.List[I]), tlamAddChild) then
    ...
    begin
      if ((Result.ParentKeyValue <> AParentKeyValue) or
        ((Result.Parent <> nil) and (Result.Parent <> Root) and
        (TcxDBTreeListNode(Result.Parent).KeyValue <> AParentKeyValue))) and
        FindLoadedNode(AParentKeyValue, I) and
        Result.CanMove(TcxTreeListNode(Loaded.List[I]), tlamAddChild) then
        InternalMove(Result, TcxTreeListNode(Loaded.List[I]), tlamAddChild)
      else
        InternalMove(Result, Root, tlamAdd);
...
function TcxCustomDBTreeList.FindNodeByKeyValue(const AKeyValue: Variant;
  AStartNode: TcxTreeListNode = nil; AExpandedOnly: Boolean = False;
  AForward: Boolean = True): TcxDBTreeListNode;
begin
  Result := InternalFindNode(AKeyValue, AStartNode, AExpandedOnly,
    AForward, False);
end;

function TcxCustomDBTreeList.GetColumnByFieldName(const AFieldName: string)
  : TcxDBTreeListColumn;
var
  I: Integer;
begin
  ...
  for I := 0 to ColumnCount - 1 do
    if AnsiCompareText(TcxDBTreeListColumn(Columns[I]).DataBinding.FieldName,
      AFieldName) = 0 then
    begin
...
begin
  ...
  if ASource is TcxCustomDBTreeList then
    DataController.DataSource := TcxCustomDBTreeList(ASource)
...
procedure TcxCustomDBTreeList.CreateAllItems(AMissingItemsOnly
  : Boolean = False);
begin
...
begin
  ...
  begin
    if (ANode.Parent <> Root) and VarIsNull(DataController.DBParentField.Value)
    then
    begin
...
begin
  ...
  begin
    ...
    try
      for I := 0 to ANodes.Count - 1 do
        DataController.SetNodeParentValue(TcxDBTreeListNode(ANodes.List[I]),
          NewParentValue);
    finally
  ...
  begin
    BeginUpdate;
    try
      DataController.DoCopyTo(TcxDBTreeListNode(AttachNode),
        AttachMode, ANodes);
    finally
      EndUpdate;
    end
...
function TcxCustomDBTreeList.FindLoadedNodeByKeyValue(const AValue: Variant)
  : TcxDBTreeListNode;
var
  I: Integer;
  AList: TList;
begin
  Result := nil;
  if (FocusedNode <> nil) and VarEquals(TcxDBTreeListNode(FocusedNode).KeyValue,
    AValue) then
    Exit(TcxDBTreeListNode(FocusedNode))
  else if (UpdatingNode <> nil) and VarEquals(UpdatingNode.KeyValue, AValue)
  then
    Exit(UpdatingNode);
...
var
  ANode: TcxDBTreeListNode;
begin
  if DataLocate or DataBusy or DataController.IsKeyFieldsEmpty or
    not OptionsData.SyncMode or (DataSet.State in dsEditModes) then
    Exit;
...
function TcxCustomDBTreeList.GetDataControllerClass
...
begin
  Result := inherited HasData and not DataController.IsKeyFieldsEmpty and
    DataController.DataLink.Active;
end;
...
function TcxCustomDBTreeList.InsertNode(ARelative: TcxTreeListNode;
...
begin
  ...
  else if (Loader.CursorNode <> nil) and VarEquals(Loader.CursorNode.KeyValue,
    AKeyValue) then
    Result := Loader.CursorNode
  else if not((OptionsData.SmartLoad and VarEquals(AKeyValue, RootValue)) or
    VarIsNull(AKeyValue)) then
  begin
    Result := Find(@AKeyValue, AStartNode, AExpandedOnly, AForward,
      TcxTreeListFindFunc(@cxFindKey)) as TcxDBTreeListNode;
    if ARefreshAll and (Result = nil) and (UpdatingNode = nil) and
      not DataBusy and not FIsRefresh then
    begin
      RefreshTree;
      Result := Find(@AKeyValue, AStartNode, AExpandedOnly, AForward,
        TcxTreeListFindFunc(@cxFindKey)) as TcxDBTreeListNode;
...
var
  ANode: TcxDBTreeListNode;
begin
  if not IsDataSettingsValid or IsTreeListLocked then
    Exit;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        UpdatingNode := TcxDBTreeListNode(AddNode(CreateNode, FocusedNode, nil,
          tlamInsert)); // FocusedNode.AddChild as TcxDBTreeListNode;
    end
    else
    ...
    begin
      if not IsAppend and (FocusedNode <> nil) and (FocusedNode <> UpdatingNode)
      then
...
begin
  Result := inherited IsDataSettingsValid and not IsLoading and
    not IsDestroying and (DataController.DataSet <> nil) and
    DataController.DataSet.Active and not DataController.IsKeyFieldsEmpty and
    not DataController.DataSet.ControlsDisabled;
end;

function TcxCustomDBTreeList.IsNodeInserting(ANode: TcxTreeListNode): Boolean;
begin
  Result := (ANode <> nil) and DataController.DataLink.Active and
    (DataSet.State = dsInsert) and
    (TcxDBTreeListNode(ANode).KeyValue = GetFieldValue
    (DataController.DBKeyField));
end;
...
procedure TcxCustomDBTreeList.UpdateFocusedNode(AColumn: TcxTreeListColumn);
begin
...
procedure TcxCustomDBTreeList.LoadListByNode(ANode: TcxDBTreeListNode;
  AList: TList);
var
  P: PVariant;
  I: Integer;
...
begin
  ...
  begin
    if AComponent is TField then
      for I := 0 to ColumnCount - 1 do
        TcxDBTreeListColumn(Columns[I]).DataBinding.CheckFreeNotification
          (AComponent);
  end;
end;
...
procedure TcxCustomDBTreeList.LoadListByID(const AKeyValue: Variant;
  AList: TList);
var
  P: PVariant;
  ATempList: TList;
...
begin
  ...
  P^ := AKeyValue;
  AList.Add(P);
  if not DataSet.Locate(DataController.ParentField, AKeyValue, []) then
    Exit;
  ATempList := TList.Create;
  try
    while not DataSet.EOF and (GetFieldValue(DataController.DBParentField)
      = AKeyValue) do
    begin
      New(P);
