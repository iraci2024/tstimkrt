...
{$ENDIF}
  ...
  dxCore, dxCoreClasses, cxControls, cxClasses, cxGraphics,
  cxInplaceContainer, cxTL, cxTLStrs, cxExport, cxMaskEdit, cxImage,
  cxImageCombobox, RTLConsts;

procedure cxExportTLToHTML(const AFileName: string;
  ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; const AFileExt: string = 'html';
...
procedure cxExportTLToText(const AFileName: string;
  ATreeList: TcxCustomTreeList; AExpand: Boolean; ASaveAll: Boolean = True;
  const AFileExt: string = 'txt'; AHandler: TObject = nil;
  AEncoding: TEncoding = nil); overload;
procedure cxExportTLToText(const AFileName: string;
  ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; const ASeparator: string = '';
  const ABeginString: string = ''; const AEndString: string = '';
  const AFileExt: string = 'txt'; AHandler: TObject = nil;
  AEncoding: TEncoding = nil); overload;

procedure cxExportTLToExcel(const AFileName: string;
  ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; AUseNativeFormat: Boolean = True;
  const AFileExt: string = 'xls'; AHandler: TObject = nil);
procedure cxExportTLToXLSX(const AFileName: string;
  ATreeList: TcxCustomTreeList; AExpand: Boolean = True;
  ASaveAll: Boolean = True; AUseNativeFormat: Boolean = True;
...
uses
  Math, cxEdit, cxDataUtils, cxCalendar, cxCurrencyEdit, cxSpinEdit, cxCalc,
  cxTimeEdit, cxTextEdit, cxDBTL,
...
type
  ...
  protected
    ...
    function AddVisualItem(AItemClass: TcxExportDataItemClass;
      const ABounds: TRect; AIsBackground: Boolean): TcxExportCustomItem;
    function AddVisualItemEx(const AItemBounds: TRect;
      AVisualInfo: TcxItemVisualInfo; AIsBackground: Boolean = False)
      : TcxExportCustomItem; overload;
    function AddVisualItemEx(const AItemBounds: TRect; AStyle: Integer;
      AIsBackground: Boolean = False): TcxExportCustomItem; overload;
    ...
    procedure DrawIndentImage(AItem: TcxExportDataItem;
      ACell: TcxTreeListIndentCellViewInfo);
    ...
    function GetDisplayFormatType(AProperties: TcxCustomEditProperties)
      : TcxValueDisplayFormatType;
    ...
    procedure SetRealCellStyleAndValue(const ASearchArea: TRect;
      AInfo: TcxExportCustomItem);

    procedure SetRealCellStyleAndValueEx(const ARealBounds, ASearchArea: TRect;
      AStyleIndex: Integer; AGraphic: TGraphic; const AValue: Variant;
      const AValueDisplayFormat: string = '';
      const AValueDisplayFormatType: TcxValueDisplayFormatType = vdftAuto;
    ...
    procedure ViewParamsToExportStyle(const AViewParams: TcxViewParams;
      ...
      ABorderColor: TColor = clDefault; AFontSize: Integer = 0);
...
begin
  ...
  FProgressHelper := TcxProgressCalculationHelper.Create(3, ATreeList,
    ProgressHandler);
...
function TcxTreeListExportHelper.AddVisualItem
  (AItemClass: TcxExportDataItemClass; const ABounds: TRect;
  AIsBackground: Boolean): TcxExportCustomItem;
begin
  ...
  else if FVisualItemsList.Count > 54 then
  ...
  begin
    for ASide := bLeft to bBottom do
      ...
      begin
        ...
        AStyle.Borders[Integer(ASide)].Color :=
          ColorToRgb(AVisualInfo.BorderColor);
        AStyle.Borders[Integer(ASide)].Width := 1;
      end;
  end;
  Result := AddVisualItemEx(AItemBounds, Provider.RegisterStyle(AStyle),
    AIsBackground);
...
const
  AClass: array [Boolean] of TcxExportDataItemClass = (TcxExportDataItem,
    TcxExportCustomItem);
begin
...
function TcxTreeListExportHelper.CanHasBorders
  (ACell: TcxCustomViewInfoItem): Boolean;
begin
  Result := (ACell is TcxTreeListBandCellViewInfo) or
    (ACell is TcxTreeListCustomHeaderCellViewInfo) or
    (ACell is TcxTreeListEditCellViewInfo) or
    (ACell is TcxTreeListFooterCellViewInfo);
end;
...
begin
  ...
  try
    ...
    try
      ...
      try
        FVisualItemsList.Capacity := FNodes.Count *
          (TreeList.VisibleColumnCount + 1) * 2;
        FViewInfo.Calculate;
        PrepareExportCells;
      finally
...
function TcxTreeListExportHelper.GetCellVisualInfo(ACell: TcxCustomViewInfoItem)
  : TcxItemVisualInfo;
const
  ABorders: array [TcxTreeListGridLines] of TcxBorders = ([], [bTop, bBottom],
    [bLeft, bRight], cxBordersAll);
begin
  Result := FVisualInfo;
  Result.FontSize := 0;
  ...
  else if ACell is TcxTreeListIndentCellViewInfo then
  begin
    Result.Alignment := taCenter;
    Result.FontSize := 12;
  end
  else if ACell is TcxTreeListHeaderCellViewInfo then
    Result.Alignment := TcxTreeListHeaderCellViewInfo(ACell).AlignHorz
  else if ACell is TcxTreeListEditCellViewInfo then
    Result.Alignment := TcxTreeListEditCellViewInfoAccess(ACell)
      .Column.PropertiesValue.Alignment.Horz
  else
  ...
  begin
    ...
    else if (ACell is TcxTreeListBandCellViewInfo) and
      (TcxTreeListBandCellViewInfo(ACell).Part in [tlbpGroupFooter, tlbpFooter])
    then
    begin
      Result.Borders := cxBordersAll;
      if TcxTreeListBandCellViewInfo(ACell).Band.VisibleIndex > 0 then
        Exclude(Result.Borders, bLeft);
      if TcxTreeListBandCellViewInfo(ACell).Band.VisibleIndex <
        (TreeList.Bands.BottomItemCount - 1) then
...
function TcxTreeListExportHelper.GetDisplayFormat
...
begin
  ...
  if (Result = '') and (AProperties.Owner is TcxDBTreeListColumn) then
    Result := cxGetFieldDisplayFormat(TcxDBTreeListColumn(AProperties.Owner)
      .DataBinding.Field);
end;

function TcxTreeListExportHelper.GetDisplayFormatType
  (AProperties: TcxCustomEditProperties): TcxValueDisplayFormatType;
begin
  if AProperties.Owner is TcxDBTreeListColumn then
    Result := cxGetDisplayFormatType(AProperties,
      TcxDBTreeListColumn(AProperties.Owner).DataBinding.Field)
  else
    Result := cxGetDisplayFormatType(AProperties)
end;
...
begin
  Result := IsNativeFormat and ((AProperties is TcxDateEditProperties) or
  ...
  Result := Result or (Provider.SupportGraphic and
    IsGraphicProperties(AProperties)) or
    (IsRTFSupported(AProperties) and Provider.SupportRTF);
end;

procedure TcxTreeListExportHelper.FillArea(const ABounds: TRect;
...
procedure TcxTreeListExportHelper.FillRealArea(const ABounds: TRect;
  AStyleIndex: Integer; ABorderColor: TColor = clDefault;
  ABorders: TcxBorders = cxBordersAll);
var
...
function TcxTreeListExportHelper.IsGraphicProperties
  (AProperties: TcxCustomEditProperties): Boolean;
begin
  Result := (AProperties is TcxCustomImageProperties) or
...
procedure TcxTreeListExportHelper.DrawIndentImage(AItem: TcxExportDataItem;
  ACell: TcxTreeListIndentCellViewInfo);
var
  ABitmap: TcxBitmap;
begin
  ...
  begin
    ABitmap := TcxBitmap.CreateSize(ACell.Images.Width,
      ACell.Images.Height, pf24bit);
    try
      ABitmap.cxCanvas.FillRect(Rect(0, 0, ABitmap.Width, ABitmap.Height),
        ACell.ViewParams);
...
begin;
  ...
  begin
    ...
    AExportVisualItem := TcxExportDataItem(AddVisualItemEx(ACell.BoundsRect,
      GetCellVisualInfo(ACell), IsBackgroundCell(ACell)));
    ...
    begin
      ...
      if IsNativeFormatProperties(AEditCell.Properties) then
        Provider.AddValue(AExportVisualItem,
          AEditCell.Node.Values[AEditCell.Column.ItemIndex],
          AEditCell.Properties, GetDisplayFormat(AEditCell.Properties),
          GetDisplayFormatType(AEditCell.Properties))
      else
        Provider.AddText(AExportVisualItem,
          AEditCell.Node.Texts[AEditCell.Column.ItemIndex])
    end
    else
    ...
    begin
      if TcxTreeListIndentCellViewInfo(ACell).Button then
        Provider.AddText(AExportVisualItem,
          AExpandChar[TcxTreeListIndentCellViewInfo(ACell).Node.Expanded])
      else if TcxTreeListIndentCellViewInfo(ACell).HasImage and Provider.SupportGraphic
      then
      begin
        if TcxTreeListIndentCellViewInfo(ACell).OverlayIndex <> -1 then
          DrawIndentImage(AExportVisualItem,
            TcxTreeListIndentCellViewInfo(ACell))
        else
          Provider.AddImageListItem(AExportVisualItem,
            TcxTreeListIndentCellViewInfo(ACell).Images,
            TcxTreeListIndentCellViewInfo(ACell).ImageIndex,
            ACell.ViewParams.Color);
      end;
    end
    else
      ...
      Provider.AddText(AExportVisualItem,
        TcxTreeListHeaderCellViewInfo(ACell).Text)
    else

      if ACell is TcxTreeListFooterCellViewInfo then
    begin
      AProperties := TcxTreeListFooterCellViewInfo(ACell)
        .Column.PropertiesValue;
      if IsNativeFormatProperties(AProperties) and
        not IsGraphicProperties(AProperties) then
      begin
        AFormat := TcxTreeListFooterCellViewInfo(ACell).DisplayFormat;
        if AFormat = '' then
          AFormat := GetDisplayFormat(AProperties);
        Provider.AddValue(AExportVisualItem,
          TcxTreeListFooterCellViewInfo(ACell).Value, AProperties, AFormat);
      end
      else
        Provider.AddText(AExportVisualItem,
          TcxTreeListFooterCellViewInfo(ACell).Text);
    end;
  end;
...
procedure TcxTreeListExportHelper.RealBoundsToLogicalBoundsEx(const ABounds,
  ASearchArea: TRect; out ALogicalBounds: TRect);
begin
  with ALogicalBounds do
  begin
    Columns.GetPositionEx(ABounds.Left, ABounds.Right, ASearchArea.Left,
      ASearchArea.Right, Left, Right);
    Rows.GetPositionEx(ABounds.Top, ABounds.Bottom, ASearchArea.Top,
      ASearchArea.Bottom, Top, Bottom);
  end;
end;
...
procedure TcxTreeListExportHelper.SetRealCellStyleAndValue(const ASearchArea
  : TRect; AInfo: TcxExportCustomItem);
begin
...
procedure TcxTreeListExportHelper.SetRealCellStyleAndValueEx(const ARealBounds,
  ASearchArea: TRect; AStyleIndex: Integer; AGraphic: TGraphic;
...
begin
  ...
  begin
    Provider.SetCellValue(R.Left, R.Top, AValue, AValueDisplayFormat,
...
function TcxTreeListExportHelper.SkipExportCell
  (ACell: TcxCustomViewInfoItem): Boolean;
begin
  Result := not ACell.Visible or not((ACell is TcxTreeListCustomCellViewInfo) or
    (ACell is TcxTreeListEditCellViewInfo)) or
    ((ACell is TcxTreeListFooterCellViewInfo) and TcxTreeListFooterCellViewInfo
    (ACell).Hidden);
end;

procedure TcxTreeListExportHelper.ViewParamsToExportStyle(const AViewParams
...
begin
  ...
  begin
    ...
    for I := 0 to 3 do
    begin
      Borders[I].IsDefault := (ABorderColor = clDefault) or
...
function TcxTreeListExportHelper.GetVisualItem(AIndex: Integer)
  : TcxExportDataItem;
begin
...
procedure TcxTreeListExportHelper.ProgressHandler(Sender: TObject;
  Percents: Integer);
var
...
procedure cxExportTLToHTML(const AFileName: string;
  ...
  AHandler: TObject = nil);
begin
  cxExportTLToFile(AFileName, ATreeList, cxExportToHtml, AExpand, ASaveAll,
    False, [], AFileExt, AHandler);
end;
...
procedure cxExportTLToXML(const AFileName: string; ATreeList: TcxCustomTreeList;
  ...
  const AFileExt: string = 'xml'; AHandler: TObject = nil);
begin
  cxExportTLToFile(AFileName, ATreeList, cxExportToXML, AExpand, ASaveAll,
    False, [], AFileExt, AHandler);
end;
...
procedure cxExportTLToCSV(const AFileName: string; ATreeList: TcxCustomTreeList;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  cxExportTLToFile(AFileName, ATreeList, cxExportToCSV, AExpand, ASaveAll,
    False, [ASeparator], AFileExt, AHandler, AEncoding);
end;
...
begin
  cxExportTLToText(AFileName, ATreeList, AExpand, ASaveAll, '', '', '',
...
procedure cxExportTLToText(const AFileName: string;
  ...
  AEncoding: TEncoding = nil); overload;
begin
  cxExportTLToFile(AFileName, ATreeList, cxExportToText, AExpand, ASaveAll,
    False, [ASeparator, ABeginString, AEndString], AFileExt, AHandler,
    AEncoding);
end;

function Supports(const Instance: IUnknown; const IID: TGUID; out Intf)
  : Boolean; overload;
begin
...
begin
  ...
  with TcxTreeListExportHelper.Create(ATreeList, AExportType, AFileName,
    AHandler) do
    try
      Expand := AExpand;
      IsNativeFormat := AUseNativeFormat;
      SaveAll := ASaveAll;
      if cxTLExportLink.Supports(Provider, IcxExportProviderEncoding,
        AEncodingIntf) then
