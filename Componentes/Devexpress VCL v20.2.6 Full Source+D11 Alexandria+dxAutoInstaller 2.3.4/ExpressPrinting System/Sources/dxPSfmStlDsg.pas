...
type
  ...
  public
    ...
    property Controller: TdxPrintStyleManager read FController
      write SetController;
    property ControllerDesigner: TAbstractdxStyleManagerDesigner
...
procedure dxShowPrintStylesDesigner(AStyleController: TdxPrintStyleManager;
...
type
  ...
  public
    constructor Create(AStyleController: TdxPrintStyleManager;
      AFormDesigner: TFormDesigner);
    destructor Destroy; override;
...
constructor TdxStyleManagerDesigner.Create(AStyleController
  : TdxPrintStyleManager; AFormDesigner: TFormDesigner);
begin
...
procedure TdxfmPrintStylesDesignWindow.lbxStylesEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
  DrawDragRect;
end;
...
procedure TdxfmPrintStylesDesignWindow.lbxStylesDrawItem(AControl: TcxListBox;
  ACanvas: TcxCanvas; AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
begin
  dxPSDrawStyleItem(Styles[AIndex], AControl.InnerListBox, AIndex, AState,
    ARect, False, True, ScaleFactor);
end;
...
begin
  ...
  if (ADesigner = Designer) and (Item is TBasedxPrintStyle) and
    Assigned(Controller) and not(csDestroying in Controller.ComponentState) then
...
procedure TdxfmPrintStylesDesignWindow.SelectionChanged(const ADesigner
...
function TdxfmPrintStylesDesignWindow.GetControllerDesigner
...
function TdxfmPrintStylesDesignWindow.GetStyle(Index: Integer)
  : TBasedxPrintStyle;
begin
  Result := TBasedxPrintStyle(lbxStyles.Items.Objects[Index]);
end;

procedure TdxfmPrintStylesDesignWindow.SetController
  (Value: TdxPrintStyleManager);
begin
...
procedure TdxfmPrintStylesDesignWindow.SetSelected(Index: Integer;
  Value: Boolean);
begin
    ...
    try
      ...
      try
        ...
        begin
          ...
          if not(csAncestor in AItem.ComponentState) and
            (AItem is TBasedxPrintStyle) then
            AItem.Free;
        end;
...
begin
  ...
  Result.Y := btnRestoreDefaults.Top + btnRestoreDefaults.Height +
    dxGetSystemMetrics(SM_CYCAPTION, ScaleFactor) + 2 *
    dxGetSystemMetrics(SM_CXFRAME, ScaleFactor) + ScaleFactor.Apply(4);
end;

procedure TdxfmPrintStylesDesignWindow.GetSelections(const ASelections
...
function TdxfmPrintStylesDesignWindow.IndexOf(AItem: TBasedxPrintStyle)
  : Integer;
begin
  Result := lbxStyles.Items.IndexOfObject(AItem);
end;

procedure TdxfmPrintStylesDesignWindow.InternalAddStyle
...
procedure TdxfmPrintStylesDesignWindow.PrepareAddStandardItem
  (MenuItem: TMenuItem);
begin
...
procedure TdxfmPrintStylesDesignWindow.Select(AItem: TPersistent;
...
begin
  if (Controller <> nil) and (Controller.Owner <> nil) then
    NewCaption := Format(sdxStyleDesigner, [Controller.Owner.Name, '.',
      Controller.Name]);
...
begin
  ...
  begin
    ...
    begin
      ...
      AWidth := Max(AWidth, Self.ScaleFactor.Apply(dxStyleGlyphSize.X) +
        Self.ScaleFactor.Apply(4) + Canvas.TextWidth(Items[I]));
    end;
...
procedure TdxfmPrintStylesDesignWindow.UpdateSelections(const ASelections
  : TdxDesignSelectionList);
...
begin
  if (ASelections = nil) or (Controller = nil) or
    ...
    (TdxStyleManagerDesigner(ControllerDesigner).FUpdateCount <> 0) then
    Exit;
  for I := 0 to StyleCount - 1 do
...
procedure TdxfmPrintStylesDesignWindow.WMGetMinMaxInfo
