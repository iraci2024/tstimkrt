...
uses
  Windows, Types, Classes, Controls, Graphics, Generics.Collections,
  Generics.Defaults,
  dxCore, dxPSBaseGridLnk, dxPSCore, dxPSReportRenderCanvas,
  dxPSdxSpreadSheetLnkCore, dxPSEdgePatterns,
  dxSpreadSheetCore, dxSpreadSheetPrinting, dxSpreadSheetCoreStyles,
  dxSpreadSheetTypes, dxSpreadSheetGraphics,
  dxSpreadSheetContainers, dxPSGlbl, dxPrnPg, dxPgsDlg, dxPSHFLibrary,
  dxSpreadSheet;
...
type
  ...
  TdxSpreadSheetMacroConverter = class
  ...
  protected
    ...
    class function EvalFuncDate(const S: string; var AIndex: Integer;
      ALength: Integer): string; override;
    class function EvalFuncPageNumber(const S: string; var AIndex: Integer;
      ALength: Integer): string; override;
    class function EvalFuncPages(const S: string; var AIndex: Integer;
      ALength: Integer): string; override;
    class function EvalFuncSheetName(const S: string; var AIndex: Integer;
      ALength: Integer): string; override;
    class function EvalFuncTime(const S: string; var AIndex: Integer;
      ALength: Integer): string; override;
    class function EncodeCore(const S: string): string;
    class function EncodeFont: string;
    class function GetDefinition(const FunctionClass
      : TdxHFCustomFunctionClass): string;
  public
    class procedure Decode(AReportLink: TdxSpreadSheetDocumentBasedReportLink;
      ATarget: TCustomdxPageObject;
      ASource: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText);
    class procedure Encode(AReportLink: TdxSpreadSheetDocumentBasedReportLink;
      ATarget: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText;
      ASource: TCustomdxPageObject);
  ...
  public
    constructor Create(AReportLink: TdxSpreadSheetDocumentBasedReportLink;
  ...
  TdxSpreadSheetDocumentBasedReportLink = class
    (TdxSpreadSheetAbstractReportLink, IdxSpreadSheetListener,
  ...
  protected
    ...
    function GetBorderStylePatternClass(AStyle: TdxSpreadSheetCellBorderStyle)
      : TdxPSEdgePatternClass; override;
    ...
    function GetSourceCellContentBkColor(ACol: Integer; ARow: Integer)
      : TColor; override;
    ...
    function ConstructHeadersOnEveryPage(AHost: TdxReportCell; AArea: TRect)
    ...
    procedure InitializeContainerCell(ACell: TdxReportCell;
      AViewInfo: TdxSpreadSheetContainerViewInfo); override;
    // Comments
    procedure AddComments(AReportCells: TdxReportCells); virtual;
    function CreateCommentsProducer(AReportCells: TdxReportCells)
      : TdxSpreadSheetDocumentBasedReportLinkCommentProducer; virtual;
    // Render
  ...
  protected
    ...
    function IsReportFitsToPages(APrinterPage: TdxPrinterPage)
      : Boolean; override;
  public
    function CalculateActualScaleFactor(AFitToPageHorizontally,
      AFitToPageVertically, AReportDPI: Integer): Integer; override;
    ...
    //
    property ReportLink: TdxSpreadSheetDocumentBasedReportLink
      read GetReportLink;
  end;
  ...
  public
    ...
    //
    property RenderInfo: TdxSpreadSheetDocumentBasedReportRenderInfo
      read GetRenderInfo;
    property ReportLink: TdxSpreadSheetDocumentBasedReportLink
      read GetReportLink;
  end;
...
uses
  dxPSExcelEdgePatterns, dxPSExcelFillPatterns, cxGeometry, dxSpreadSheetUtils,
  dxSpreadSheetDialogStrs,
  dxCoreGraphics, dxSpreadSheetStrs, cxDrawTextUtils, Math, SysUtils, dxPrnDev,
  dxPrintUtils, dxMeasurementUnits,
...
class procedure TdxSpreadSheetMacroConverter.Decode
  (AReportLink: TdxSpreadSheetDocumentBasedReportLink;
  ATarget: TCustomdxPageObject;
  ASource: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText);
begin
  ATarget.BeginUpdate;
...
class procedure TdxSpreadSheetMacroConverter.Encode
  (AReportLink: TdxSpreadSheetDocumentBasedReportLink;
  ATarget: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText;
  ASource: TCustomdxPageObject);
...
begin
  FReportLink := AReportLink;
  try
    ATarget.LeftSection := CheckEncode(ATarget.LeftSection,
      ASource.LeftTitle.Text);
    ATarget.CenterSection := CheckEncode(ATarget.CenterSection,
      ASource.CenterTitle.Text);
    ATarget.RightSection := CheckEncode(ATarget.RightSection,
      ASource.RightTitle.Text);
    ATarget.Assigned := (ATarget.LeftSection <> '') and
      (ATarget.CenterSection <> '') and (ATarget.RightSection <> '');
  finally
    FReportLink := nil;
  end;
end;
...
begin
  ...
  AExpander := TdxHFTemplateExpander.Create(StringReplace(S, '&', '&&',
    [rfReplaceAll]));
  try
...
class function TdxSpreadSheetMacroConverter.EvalFuncPageNumber(const S: string;
  var AIndex: Integer; ALength: Integer): string;
begin
  Result := GetDefinition(TdxHFPageNumberFunction);
end;

class function TdxSpreadSheetMacroConverter.EvalFuncPages(const S: string;
  var AIndex: Integer; ALength: Integer): string;
begin
  Result := GetDefinition(TdxHFTotalPagesFunction);
end;

class function TdxSpreadSheetMacroConverter.EvalFuncDate(const S: string;
  var AIndex: Integer; ALength: Integer): string;
begin
  Result := GetDefinition(TdxHFDateFunction);
end;

class function TdxSpreadSheetMacroConverter.EvalFuncTime(const S: string;
  var AIndex: Integer; ALength: Integer): string;
begin
  Result := GetDefinition(TdxHFTimeFunction);
end;

class function TdxSpreadSheetMacroConverter.EvalFuncSheetName(const S: string;
  var AIndex: Integer; ALength: Integer): string;
begin
  Result := FReportLink.Sheet.Caption;
end;

class function TdxSpreadSheetMacroConverter.GetDefinition(const FunctionClass
  : TdxHFCustomFunctionClass): string;
begin
  Result := dxFunctionDelimiters[False] +
    FunctionClass.DefaultTemplateStringValue + dxFunctionDelimiters[True];
end;
...
begin
  ...
  FHost.BoundsRect := cxRectSetTop(ReportCells.BoundsRect,
    ReportCells.BoundsRect.Bottom, 0);

  ReportLink.AddVerticalHardDelimiter(FHost);
end;

procedure TdxSpreadSheetDocumentBasedReportLinkCommentProducer.Produce
  (AComment: TdxSpreadSheetCommentContainer);

  function CreateString(AHost: TdxReportCell; const AText: string;
    AFontIndex: Integer): TdxReportCellString;
  begin
  ...
  function CreateHeader(AHost: TdxReportCell; const AText: string)
    : TdxReportCellString;
  begin
  ...
  function CreateContent(AHost: TdxReportCell; const AText: string)
    : TdxReportCellString;
  begin
...
begin
  ...
  AOffset := Host.Height;
  AHeaderWidth := Max(ALine1Header.MeasureWidth(ScreenCanvas),
    ALine2Header.MeasureWidth(ScreenCanvas));
  ALine1Header.BoundsRect := cxRectBounds(0, AOffset, AHeaderWidth,
    ALine1Header.MeasureHeight(ScreenCanvas));
  ALine1Content.BoundsRect := cxRectBounds(AHeaderWidth, AOffset,
    Host.Width - AHeaderWidth, 0);
  ALine1Content.Height := ALine1Content.MeasureHeight(ScreenCanvas);
  Inc(AOffset, ALine1Content.Height);

  ALine2Header.BoundsRect := cxRectBounds(0, AOffset, AHeaderWidth,
    ALine2Header.MeasureHeight(ScreenCanvas));
  ALine2Content.BoundsRect := cxRectBounds(AHeaderWidth, AOffset,
    Host.Width - AHeaderWidth, 0);
...
procedure TdxSpreadSheetDocumentBasedReportLinkCommentProducer.SetupBoundsRect
  (ACell: TdxReportCell);
begin
  ReportLink.SetupBoundsRect(ACell);
end;

function TdxSpreadSheetDocumentBasedReportLinkCommentProducer.GetScreenCanvas
...
procedure TdxSpreadSheetDocumentBasedReportLink.AddHorizontalPageBreak
  (AColumn: Integer);
var
...
begin
  ...
  begin
    FRowsToRepeatArea := SpreadSheetAreaToReportArea
      (Options.Source.RowsToRepeat.Rect);
    FSelectionRect.Top := Min(FSelectionRect.Top, FRowsToRepeatArea.Top);
    FSelectionRect.Bottom := Max(FSelectionRect.Bottom,
      FRowsToRepeatArea.Bottom);
  end
  else
  ...
  begin
    FColumnsToRepeatArea := SpreadSheetAreaToReportArea
      (Options.Source.ColumnsToRepeat.Rect);
    FSelectionRect.Left := Min(FSelectionRect.Left, FColumnsToRepeatArea.Left);
    FSelectionRect.Right := Max(FSelectionRect.Right,
      FColumnsToRepeatArea.Right);
  end
  else
...
function TdxSpreadSheetDocumentBasedReportLink.
  GetBreakPagesByHardDelimiters: Boolean;
begin
  Result := True;
end;

function TdxSpreadSheetDocumentBasedReportLink.GetCellTextCore
...
function TdxSpreadSheetDocumentBasedReportLink.GetSourceCellColor(ACol,
  ARow: Integer): TColor;
begin
...
function TdxSpreadSheetDocumentBasedReportLink.GetSourceCellContentBkColor(ACol,
  ARow: Integer): TColor;
begin
  if IsSuppressSourceFormats then
    Result := clWindow
...
function TdxSpreadSheetDocumentBasedReportLink.GetSourceColWidth
  (ACol: Integer): Integer;

  function IsInPrintArea(C: Integer): Boolean;
  begin
    Result := dxSpreadSheetContainsColumn(FPrintArea, C) or
      dxSpreadSheetContainsColumn(FColumnsToRepeatArea, C);
  end;
...
function TdxSpreadSheetDocumentBasedReportLink.GetSourceRowHeight
  (ARow: Integer): Integer;

  function IsInPrintArea(R: Integer): Boolean;
  begin
    Result := dxSpreadSheetContainsRow(FPrintArea, R) or
      dxSpreadSheetContainsRow(FRowsToRepeatArea, R);
  end;
...
function TdxSpreadSheetDocumentBasedReportLink.GetUseHardHorzDelimiters
  : Boolean;
begin
  Result := True;
end;

function TdxSpreadSheetDocumentBasedReportLink.GetUseHardVertDelimiters
  : Boolean;
begin
  Result := True;
end;
...
function TdxSpreadSheetDocumentBasedReportLink.
  GetDefaultCellBorderColor: TColor;
begin
...
begin
  Result := (IsShowGridLines or IsShowRowAndColumnHeadings) and
    not IsDraftQuality;
end;

function TdxSpreadSheetDocumentBasedReportLink.
  IsDrawHeaderCornersOnEveryPage: Boolean;
begin
  Result := IsDrawRowHeadersOnEveryPage and IsDrawHeadersOnEveryPage;
end;

function TdxSpreadSheetDocumentBasedReportLink.IsDrawHeadersOnEveryPage
  : Boolean;
begin
...
function TdxSpreadSheetDocumentBasedReportLink.
  IsDrawRowHeadersOnEveryPage: Boolean;
begin
...
function TdxSpreadSheetDocumentBasedReportLink.
  IsShowRowAndColumnHeadings: Boolean;
begin
...
function TdxSpreadSheetDocumentBasedReportLink.GetRebuildOnPageParamsChange
...
procedure TdxSpreadSheetDocumentBasedReportLink.PrepareConstruct
  (AReportCells: TdxReportCells);
begin
  CalculatePrintAreas;
...
procedure TdxSpreadSheetDocumentBasedReportLink.UnprepareConstruct
  (AReportCells: TdxReportCells);
var
  AIndex: Integer;
  I: Integer;
...
procedure TdxSpreadSheetDocumentBasedReportLink.AssignOptionsToPrinterPage
...
procedure TdxSpreadSheetDocumentBasedReportLink.AssignOptionsToPrinterPageCore
  (APage: TdxPrinterPage);
...
const
  OrientationMap: array [Boolean] of TdxPrinterOrientation = (poPortrait,
    poLandscape);
  PageOrderMap: array [Boolean] of TdxPageOrder = (poDownThenOver,
    poOverThenDown);
begin
  APage.GrayShading := Options.Printing.ActualBlackAndWhite;
  APage.PageOrder := PageOrderMap
    [Options.Printing.ActualPageOrder = opppOverThenDown];
  ...
  begin
    ...
    begin
      APage.PageSize.Point :=
        Point(ConvertValue(Options.Page.Paper.CustomSize.X),
  ...
  TdxSpreadSheetMacroConverter.Decode(Self, APage.PageFooter,
    Options.HeaderFooter.CommonFooter);
  TdxSpreadSheetMacroConverter.Decode(Self, APage.PageHeader,
    Options.HeaderFooter.CommonHeader);
  TCustomdxPageObjectAccess(APage.PageFooter).AdjustOnScale :=
    Options.HeaderFooter.ActualScaleWithDocument;
  TCustomdxPageObjectAccess(APage.PageFooter).AlignWithMargins :=
    Options.HeaderFooter.ActualAlignWithMargins;
  TCustomdxPageObjectAccess(APage.PageHeader).AdjustOnScale :=
    Options.HeaderFooter.ActualScaleWithDocument;
  TCustomdxPageObjectAccess(APage.PageHeader).AlignWithMargins :=
    Options.HeaderFooter.ActualAlignWithMargins;
end;

procedure TdxSpreadSheetDocumentBasedReportLink.AssignPrinterPageToOptions
  (APage: TdxPrinterPage);
begin
  if not FIsPrintOptionsLocked and (Sheet <> nil) then
  begin
...
procedure TdxSpreadSheetDocumentBasedReportLink.AssignPrinterPageToOptionsCore
  (APage: TdxPrinterPage);
...
const
  OrientationMap: array [Boolean]
    of TdxSpreadSheetTableViewOptionsPrintPageOrientation = (oppoPortrait,
    oppoLandscape);
  ScaleModeMap: array [TdxScaleMode]
    of TdxSpreadSheetTableViewOptionsPrintPageScaleMode = (oppsmAdjustToScale,
    oppsmFitToPage);
  PageOrderMap: array [TdxPageOrder]
    of TdxSpreadSheetTableViewOptionsPrintPrintingPageOrder = (opppOverThenDown,
    opppDownThenOver);
begin
  ...
  Options.Printing.BlackAndWhite := dxBooleanToDefaultBoolean
    (APage.GrayShading);
  Options.Printing.HorizontalCentered := dxBooleanToDefaultBoolean
    (APage.CenterOnPageH);
  Options.Printing.VerticalCentered := dxBooleanToDefaultBoolean
    (APage.CenterOnPageV);
  ...
  TdxSpreadSheetMacroConverter.Encode(Self, Options.HeaderFooter.CommonFooter,
    APage.PageFooter);
  TdxSpreadSheetMacroConverter.Encode(Self, Options.HeaderFooter.CommonHeader,
    APage.PageHeader);
  Options.HeaderFooter.AlignWithMargins := dxBooleanToDefaultBoolean
    (TCustomdxPageObjectAccess(APage.PageHeader).AlignWithMargins);
  Options.HeaderFooter.ScaleWithDocument := dxBooleanToDefaultBoolean
    (TCustomdxPageObjectAccess(APage.PageHeader).AdjustOnScale);
...
procedure TdxSpreadSheetDocumentBasedReportLink.SetStartPageIndex
  (Value: Integer);
begin
...
procedure TdxSpreadSheetDocumentBasedReportLink.PageParamsChanged
  (Sender: TdxPrinterPage; AStyle: TBasedxPrintStyle;
...
procedure TdxSpreadSheetDocumentBasedReportLink.DataChanged
  (Sender: TdxCustomSpreadSheet);
begin
  // do nothing
end;

procedure TdxSpreadSheetDocumentBasedReportLink.OptionsChanged
...
procedure TdxSpreadSheetDocumentBasedReportLink.ConstructHeadersOnEveryPage;
  ...
  begin
    ...
    if HasSelection then
      Result := cxRectOffset(Result, ReportAreaToSpreadSheetArea(SelectionRect)
        .TopLeft, False);
    if IsShowRowAndColumnHeadings then
      Result := cxRectOffset(Result, Ord(AOrientation = orVertical),
        Ord(AOrientation = orHorizontal));
...
begin
  ...
  if IsDrawHeadersOnEveryPage and IsDrawRowHeadersOnEveryPage then
    AHeadersCorner := cxRect(ARowHeaders.Left, AHeaders.Top, ARowHeaders.Right,
      AHeaders.Bottom)
  else
  ...
  begin
    if ConstructHeadersOnEveryPage(ReportCells.HeaderCells, AHeaders) then
      OffsetChildren(ReportCells.HeaderCells,
        cxPoint(0, -ReportCells.HeaderCells.FirstCell.Top));
  end;
  ...
  begin
    if ConstructHeadersOnEveryPage(ReportCells.RowHeaderCells, ARowHeaders) then
      OffsetChildren(ReportCells.RowHeaderCells,
        cxPoint(-ReportCells.RowHeaderCells.FirstCell.Left,
        ReportCells.HeaderCells.Height));
  end;

  if dxSpreadSheetIsValidArea(AHeadersCorner) then
  begin
    if ConstructHeadersOnEveryPage(ReportCells.HeaderCornerCells, AHeadersCorner)
    then
      OffsetChildren(ReportCells.HeaderCornerCells,
        cxPoint(0, -ReportCells.HeaderCornerCells.Cells[0].Top));
  end;
end;

function TdxSpreadSheetDocumentBasedReportLink.ConstructHeadersOnEveryPage
  (AHost: TdxReportCell; AArea: TRect): Boolean;
  ...
  procedure CloneAffectedContainers(AContainers: TdxReportCell;
...
var
  ADataOffset: Integer;
begin
  if dxSpreadSheetIntersects(AArea, cxRect(0, 0, MaxInt, FGridAdapter.RowCount -
    1), AArea) then
  begin
    ADataOffset := 0;
...
procedure TdxSpreadSheetDocumentBasedReportLink.AddContainers
  (AReportCells: TdxReportCells);
begin
  inherited;
...
function TdxSpreadSheetDocumentBasedReportLink.CanPrintContainer
...
begin
  ...
  if (Options.Source.CellComments = psccAsDisplayed) and
    (AViewInfo.Owner is TdxSpreadSheetCommentContainer) then
  begin
    AContainer := TdxSpreadSheetCommentContainer(AViewInfo.Owner);
    if GetCellBoundsInReport(AContainer.Cell.RowIndex,
      AContainer.Cell.ColumnIndex, True, ACellBounds) then
    begin
      ...
      if not(cxRectPtIn(AContainerBounds, ALinePoint1) and
        cxRectPtIn(AContainerBounds, ALinePoint2)) then
      begin
...
procedure TdxSpreadSheetDocumentBasedReportLink.AddComments
  (AReportCells: TdxReportCells);
var
...
function TdxSpreadSheetDocumentBasedReportLink.CreateCommentsProducer
  ...
  : TdxSpreadSheetDocumentBasedReportLinkCommentProducer;
begin
  Result := TdxSpreadSheetDocumentBasedReportLinkCommentProducer.Create(Self,
    AReportCells);
end;

function TdxSpreadSheetDocumentBasedReportLink.GetRenderInfoClass
  : TdxPSReportRenderInfoClass;
begin
  Result := TdxSpreadSheetDocumentBasedReportRenderInfo;
end;

function TdxSpreadSheetDocumentBasedReportLink.GetOptions
  : TdxSpreadSheetTableViewOptionsPrint;
begin
  Result := Sheet.OptionsPrint;
end;

procedure TdxSpreadSheetDocumentBasedReportLink.OffsetChildren
  (AHost: TdxReportCell; const AOffset: TPoint);
var
  ACell: TdxReportCell;
  I: Integer;
begin
  ...
  begin
    ACell := AHost.Cells[I];
    ACell.SetBounds(ACell.Left + AOffset.X, ACell.Top + AOffset.Y, ACell.Width,
      ACell.Height);
  end;
...
function TdxSpreadSheetDocumentBasedReportRenderInfo.GetPageRenderInfoClass
  : TdxPSPageRenderInfoClass;
begin
...
begin
  Result := (ReportLink.Options.Source.CellComments = psccAtEnd) and
    HasComments;
end;

function TdxSpreadSheetDocumentBasedReportRenderInfo.IsCommentsPage
...
begin
  ...
  begin
    ACommentsPageOffset := ReportCells.Overlays
      [ReportCells.OverlayCount - 1][0].Top;
...
function TdxSpreadSheetDocumentBasedReportRenderInfo.IsHeaderOnPage
  (APageIndex: Integer): Boolean;
begin
  Result := (APageIndex >= PageColCount) and not IsCommentsPage(APageIndex);
end;

function TdxSpreadSheetDocumentBasedReportRenderInfo.IsReportFitsToPages
  (APrinterPage: TdxPrinterPage): Boolean;
begin
  Result := ((APrinterPage.FitToPagesVertically = 0) or
    (PageRowCount <= AdjustFitToPagesVerticallyValue
    (APrinterPage.FitToPagesVertically))) and
    ((APrinterPage.FitToPagesHorizontally = 0) or
    (PageColCount <= APrinterPage.FitToPagesHorizontally));
end;

function TdxSpreadSheetDocumentBasedReportRenderInfo.IsRowHeaderOnPage
  (APageIndex: Integer): Boolean;
begin
  Result := (APageIndex > 0) and not IsCommentsPage(APageIndex);
end;

function TdxSpreadSheetDocumentBasedReportRenderInfo.
  AdjustFitToPagesVerticallyValue(AValue: Integer): Integer;
begin
  Result := AValue;
  if HasCommentsPage then
    Result := Max(Result, 2);
...
function TdxSpreadSheetDocumentBasedReportRenderInfo.GetReportLink
  : TdxSpreadSheetDocumentBasedReportLink;
begin
...
function TdxSpreadSheetDocumentBasedReportPageRenderInfo.
  HasHeaderCorner: Boolean;
begin
...
function TdxSpreadSheetDocumentBasedReportPageRenderInfo.IsCommentsPage
  : Boolean;
begin
...
procedure TdxSpreadSheetDocumentBasedReportPageRenderInfo.
...
function TdxSpreadSheetDocumentBasedReportPageRenderInfo.GetRenderInfo
  : TdxSpreadSheetDocumentBasedReportRenderInfo;
begin
  Result := TdxSpreadSheetDocumentBasedReportRenderInfo(inherited RenderInfo);
end;

function TdxSpreadSheetDocumentBasedReportPageRenderInfo.GetReportLink
  : TdxSpreadSheetDocumentBasedReportLink;
begin
  Result := TdxSpreadSheetDocumentBasedReportLink(inherited ReportLink);
end;
