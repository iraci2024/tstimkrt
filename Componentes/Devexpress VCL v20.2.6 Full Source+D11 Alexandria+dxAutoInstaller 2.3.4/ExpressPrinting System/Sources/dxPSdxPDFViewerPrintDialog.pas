...
{$ENDIF}
  ...
  cxLabel, cxGroupBox, cxContainer, cxEdit,
  cxDropDownEdit, cxTextEdit, cxRadioGroup, cxCheckBox, cxMaskEdit, cxListBox,
  cxSpinEdit, dxPrnDev, dxPrnDlg, dxPgsDlg,
  dxPSPrVw, dxPSCore, dxLayoutControlAdapters, dxLayoutContainer,
...
type
  ...
  protected
    ...
    function DoShowStylePageSetup(AStyle: TBasedxPrintStyle;
      ...
      out APreviewBtnClicked, APrintBtnClicked: Boolean): Boolean; override;
    procedure CallOnPageSetup(Sender: TObject; var ADone: Boolean;
      APreviewBtnClicked, APrintBtnClicked: PBoolean); override;
    procedure DefinePrintStyleDlg(out APreviewBtnClicked: Boolean); override;
    procedure LoadStrings; override;
    procedure Initialize;

    property OnGetPrintingResult: TdxPDFOnGetPrintingResult
      read FOnGetPrintingResult write FOnGetPrintingResult;
  public
...
uses
  cxGeometry, dxCore, dxCustomPreview, dxPrnPg, dxPSUtl, dxPSGlbl,
  dxPSdxPDFViewerLnk, dxPSRes, dxPreVw, dxPDFViewer;
...
type
  ...
  TdxPDFViewerReportLinkRenderInfoAccess = class
...
function TdxPDFPrintDialog.DoShowStylePageSetup(AStyle: TBasedxPrintStyle;
  APageIndex: Integer; AShowPreviewBtn, AShowPrintBtn: Boolean;
  out APreviewBtnClicked, APrintBtnClicked: Boolean): Boolean;
begin
  Result := inherited DoShowStylePageSetup(AStyle, APageIndex, False, False,
...
procedure TdxPDFPrintDialog.DefinePrintStyleDlg(out APreviewBtnClicked
  : Boolean);
begin
...
begin
  ...
  gbxPlacementAndScaling.Caption :=
    cxGetResourceString(@sdxPDFViewerPagePlacementAndScaling);
...
begin
  ...
  if FUseOwnPrintDlgData then
    GetComponentPrinter.InitializeDefaultPrintDlgData(Preview.ReportLink,
      APrintDlgData)
  else
...
begin
  ...
  begin
    ...
    TdxPrinterPageAccess(Preview.ReportLink.PrinterPage)
...
begin
  (Preview.ReportLink.PrinterPage as TdxPDFViewerPrinterPage).PrintAsImage :=
    cbPrintAsImage.Checked;
end;
...
begin
  ARebuildReport := Length(TdxPDFViewerReportLinkAccess(Preview.ReportLink)
    .PagesToPrint) <> Length(AValue);
  if not ARebuildReport then
    for I := 0 to Length(AValue) - 1 do
    begin
      ARebuildReport := AValue[I] <> TdxPDFViewerReportLinkAccess
        (Preview.ReportLink).PagesToPrint[I];
...
begin
  ...
  rbtnAutoOrientation.Checked := Preview.ReportLink.PrinterPage.
    Orientation = poAuto;
  rbtnLandscape.Checked := Preview.ReportLink.PrinterPage.Orientation =
    poLandscape;
  rbtnPortrait.Checked := Preview.ReportLink.PrinterPage.Orientation =
    poPortrait;
  cbPrintAsImage.Checked :=
    (Preview.ReportLink.PrinterPage as TdxPDFViewerPrinterPage).PrintAsImage;
...
begin
  ...
  begin
    SetLength(APages, 1);
    APages[0] := (Preview.ReportLink.Component as TdxPDFCustomViewer)
      .CurrentPageIndex + 1;
  end
  else
    SetLength(APages, 0);
...
begin
  ...
  Preview.Preview.OptionsHint := [pohShowForMargins, pohShowOnDrag,
    pohShowOnScroll];
...
procedure TdxPDFPrintDialog.seScaleFactorPropertiesEditValueChanged
  (Sender: TObject);
begin
  if Assigned(Preview.ReportLink) then
    UpdateScaleFactor(seScaleFactor.Value);
...
procedure TdxPDFPrintDialog.FormCloseQuery(Sender: TObject;
  ...
  begin
    ...
    begin
      if not FUseOwnPrintDlgData then
        TdxPDFViewerReportLinkAccess(Preview.ReportLink).PrintDlgData.DialogData
          := DialogData;
      ...
      begin
        ...
        ARenderInfo := TdxPDFViewerReportLinkRenderInfoAccess
          (TdxPDFViewerReportLinkAccess(Preview.ReportLink)
          .RenderInfo as TdxPDFViewerReportLinkRenderInfo);
        GetComponentPrinter.PrintPagesEx
          (TIntegers(ARenderInfo.PageIndexesToPrint), DialogData.PageNums,
          DialogData.Copies, DialogData.Collate, Preview.ReportLink);
      end
      else
        GetComponentPrinter.PrintEx(DialogData.PageNums, DialogData.Copies,
          DialogData.Collate, Preview.ReportLink);
...
procedure AddPDFViewerPrintingResourceStrings
  (AProduct: TdxProductResourceStrings);
begin
  AProduct.Add('sdxPDFViewerPagePlacementAndScaling',
    @sdxPDFViewerPagePlacementAndScaling);
...
dxResourceStringsRepository.RegisterProduct('ExpressPrinting System',
  @AddPDFViewerPrintingResourceStrings);
