...
type
  ...
  protected
    procedure AddItem(AParent: TdxPSExplorerFolder;
      AnItem: TCustomdxPSExplorerItem); override;
    ...
    function AddNode(AParent: TTreeNode; AnItem: TCustomdxPSExplorerItem)
      : TTreeNode; virtual;
  ...
  public
    ...
    procedure CollapseItem(AnItem: TCustomdxPSExplorerItem;
      ARecursive: Boolean = False); override;
  ...
  private
    ...
    function GetIsMouseInScrollAreaBound(ScrollArea
      : TdxPSExplorerTreeViewScrollArea): Boolean;
    function GetScrollActiveAreas: TdxPSExplorerTreeViewScrollAreas;
    function GetScrollAreaBounds(ScrollArea
      : TdxPSExplorerTreeViewScrollArea): TRect;
  ...
  protected
    ...
    property Container: TCustomdxPSExplorerTreeViewContainer read FContainer
      write FContainer;
    property IsMouseInScrollArea: Boolean read GetIsMouseInScrollArea;
    property IsMouseInScrollAreaBounds
      [ScrollArea: TdxPSExplorerTreeViewScrollArea]: Boolean
      read GetIsMouseInScrollAreaBound;
    property ScrollActiveAreas: TdxPSExplorerTreeViewScrollAreas
      read GetScrollActiveAreas;
    property ScrollAreaBounds[ScrollArea: TdxPSExplorerTreeViewScrollArea]
      : TRect read GetScrollAreaBounds;
  public
    constructor Create(AOwner: TComponent); override;
    constructor CreateEx(AOwner: TComponent;
      AContainer: TCustomdxPSExplorerTreeViewContainer); virtual;
  ...
  protected
    ...
    //
    property TreeViewContainer: TCustomdxPSExplorerTreeViewContainer
      read GetTreeViewContainer;
  end;
...
function TCustomdxPSExplorerTreeViewContainer.BeginEdit
  (AnImmediate: Boolean = True): Boolean;
var
...
begin
  ...
  begin
    ...
    if AnImmediate then
      ...
      Result := PostMessage(TreeView.Handle, TVM_EDITLABEL, 0,
        lParam(SelectedNode.ItemId));
    Host.UpdateState;
  end;
end;
...
begin
  ...
  begin
    ...
    if Node <> nil then
      Node.EndEdit(ACancel);
  end;
end;

procedure TCustomdxPSExplorerTreeViewContainer.CollapseItem
  (AnItem: TCustomdxPSExplorerItem; ARecursive: Boolean = False);
var
  Node: TTreeNode;
begin
  Node := GetNodeByItem(AnItem);
  if Node <> nil then
    Node.Collapse(ARecursive);
end;

procedure TCustomdxPSExplorerTreeViewContainer.ExpandItem
  (AnItem: TCustomdxPSExplorerItem; ARecursive: Boolean = False);
var
  Node: TTreeNode;
begin
...
procedure TCustomdxPSExplorerTreeViewContainer.ItemDataLoaded
  (AnItem: TCustomdxPSExplorerItem);
begin
  TreeView.Refresh;
end;

procedure TCustomdxPSExplorerTreeViewContainer.ItemDataUnloaded
  (AnItem: TCustomdxPSExplorerItem);
begin
  TreeView.Refresh;
end;

procedure TCustomdxPSExplorerTreeViewContainer.MakeItemVisible
  (AnItem: TCustomdxPSExplorerItem);
var
  Node: TTreeNode;
begin
...
begin
  if lParam2 <> 0 then
    Result := TCustomdxPSExplorerItemComparator.CompareItems
      (TTreeNode(lParam1).Data, TTreeNode(lParam2).Data)
  else
    Result := 0;
end;
...
procedure TCustomdxPSExplorerTreeViewContainer.RefreshSorting
...
class function TCustomdxPSExplorerTreeViewContainer.ControlClass
  : TWinControlClass;
begin
  Result := TreeViewClass;
end;

class function TCustomdxPSExplorerTreeViewContainer.TreeViewClass
  : TcxTreeViewClass;
begin
  Result := TcxTreeView;
end;

function TCustomdxPSExplorerTreeViewContainer.GetItemAt(X, Y: Integer)
  : TCustomdxPSExplorerItem;
begin
  Result := GetItemByNode(TreeView.GetNodeAt(X, Y))
end;

function TCustomdxPSExplorerTreeViewContainer.GetItemAtMousePos
  : TCustomdxPSExplorerItem;
begin
...
function TCustomdxPSExplorerTreeViewContainer.GetDropTarget(X, Y: Integer)
  : TdxPSExplorerFolder;
var
...
procedure TCustomdxPSExplorerTreeViewContainer.AddItem
...
begin
  ...
  if AnItem is TdxPSExplorerFolder then
    Node.HasChildren := TdxPSExplorerFolderHelper.GetHasChildren
      (TdxPSExplorerFolder(AnItem));
end;
...
procedure TCustomdxPSExplorerTreeViewContainer.DeleteItem
  (AnItem: TCustomdxPSExplorerItem);
begin
  inherited;
...
procedure TCustomdxPSExplorerTreeViewContainer.InvalidateItem
  (AnItem: TCustomdxPSExplorerItem);
var
  ANode: TTreeNode;
  R: TRect;
...
procedure TCustomdxPSExplorerTreeViewContainer.MoveItem
  (AnItem: TCustomdxPSExplorerItem);
var
  Node: TTreeNode;
begin
...
procedure TCustomdxPSExplorerTreeViewContainer.RenameItem
  (AnItem: TCustomdxPSExplorerItem);
begin
  GetNodeByItem(AnItem).Text := AnItem.DisplayName;
end;

function TCustomdxPSExplorerTreeViewContainer.GetCreationParent
  : TdxPSExplorerFolder;
var
  Item: TCustomdxPSExplorerItem;
begin
  ...
  if Item = nil then
    Result := Explorer.Root
  else if not(Item is TdxPSExplorerFolder) then
    Result := Item.Parent
  else
    Result := TdxPSExplorerFolder(Item);
end;

function TCustomdxPSExplorerTreeViewContainer.GetFocusedItem
  : TCustomdxPSExplorerItem;
begin
...
function TCustomdxPSExplorerTreeViewContainer.GetSelectedFolder
  : TdxPSExplorerFolder;
var
  Item: TCustomdxPSExplorerItem;
begin
...
function TCustomdxPSExplorerTreeViewContainer.GetSelectedItem
  : TCustomdxPSExplorerItem;
begin
...
procedure TCustomdxPSExplorerTreeViewContainer.SetFocusedItem
  (Value: TCustomdxPSExplorerItem);
var
  Node: TTreeNode;
begin
...
procedure TCustomdxPSExplorerTreeViewContainer.SetSelectedItem
  (Value: TCustomdxPSExplorerItem);
var
  Node: TTreeNode;
begin
...
procedure TCustomdxPSExplorerTreeViewContainer.SetSelectedItemText
  (const Value: string);
var
  Node: TTreeNode;
begin
...
begin
  ...
  Result.SelectedIndex := TCustomdxPSExplorerItemHelper.
    GetSelectedIndex(AnItem);
  // RefreshSorting(AParent); //3.1
end;

function TCustomdxPSExplorerTreeViewContainer.CanDragNode
...
function TCustomdxPSExplorerTreeViewContainer.GetHitTestInfoAt(X, Y: Integer)
  : THitTests;
begin
  Result := TreeView.GetHitTestInfoAt(X, Y);
end;

function TCustomdxPSExplorerTreeViewContainer.GetItemByNode(ANode: TTreeNode)
  : TCustomdxPSExplorerItem;
begin
  if ANode <> nil then
...
function TCustomdxPSExplorerTreeViewContainer.GetNodeByItem
  (AnItem: TCustomdxPSExplorerItem): TTreeNode;
...
procedure TCustomdxPSExplorerTreeViewContainer.TreeCustomDrawItem
...
begin
  if (SelectedItem <> nil) and (GetItemAtMousePos = SelectedItem) and CanLoadSelectedItemData
  then
    LoadSelectedItemData;
end;

procedure TCustomdxPSExplorerTreeViewContainer.TreeDragDrop(Sender,
  Source: TObject; X, Y: Integer);
begin
  SelectedItem.Parent := GetDropTarget(X, Y);
end;

procedure TCustomdxPSExplorerTreeViewContainer.TreeDragOver(Sender,
  Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := (Source = Sender) and SelectedItem.CanMoveTo(GetDropTarget(X, Y));
end;

procedure TCustomdxPSExplorerTreeViewContainer.TreeCancelEdit(Sender: TObject;
  ANode: TTreeNode);
begin
  Host.UpdateState;
end;

procedure TCustomdxPSExplorerTreeViewContainer.TreeChanged(Sender: TObject;
  ANode: TTreeNode);
begin
  Host.UpdateState;
end;

procedure TCustomdxPSExplorerTreeViewContainer.TreeEdited(Sender: TObject;
...
procedure TCustomdxPSExplorerTreeViewContainer.TreeEditing(Sender: TObject;
...
procedure TCustomdxPSExplorerTreeViewContainer.TreeExpanding(Sender: TObject;
...
procedure TdxPSExplorerTreeView.DragOver(Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
begin
  inherited;
...
procedure TdxPSExplorerTreeView.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TdxPSExplorerTreeView.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  Pt: TPoint;
  Node: TTreeNode;
...
class function TdxPSExplorerTreeView.GetTreeViewClass
...
function TdxPSExplorerTreeView.MakeDragBitmap(ANode: TTreeNode)
  : Graphics.TBitmap;
const
...
function TdxPSExplorerTreeView.GetIsMouseInScrollAreaBound
  (ScrollArea: TdxPSExplorerTreeViewScrollArea): Boolean;
begin
  Result := PtInRect(ScrollAreaBounds[ScrollArea], GetMouseCursorClientPos);
end;

function TdxPSExplorerTreeView.GetScrollActiveAreas
...
function TdxPSExplorerTreeView.GetScrollAreaBounds
  (ScrollArea: TdxPSExplorerTreeViewScrollArea): TRect;
type
  TScrollBarKind = (sbkHorz, sbkVert);

  function HasScrollBar(AScrollBarKind: TScrollBarKind): Boolean;
  const
    ScrollBarKindMap: array [TScrollBarKind] of Integer = (WS_HSCROLL,
      WS_VSCROLL);
  begin
    Result := Windows.GetWindowLong(Handle, GWL_STYLE) and
      ScrollBarKindMap[AScrollBarKind] <> 0;
  end;

  function ScrollBarWidth(AScrollBarKind: TScrollBarKind): Integer;
  const
    ScrollBarWidthIndexMap: array [TScrollBarKind] of Integer = (SM_CYHSCROLL,
      SM_CXVSCROLL);
  begin
...
const
  ScrollMessages: array [TdxPSExplorerTreeViewScrollArea] of Cardinal =
    (WM_HSCROLL, WM_VSCROLL, WM_HSCROLL, WM_VSCROLL);
  ScrollCodes: array [TdxPSExplorerTreeViewScrollArea] of Integer =
...
begin
  ...
  TreeView.Style.LookAndFeel.MasterLookAndFeel :=
    TcxControlAccess(Host.TreeContainerParent).LookAndFeel;
end;
...
function TcxPSExplorerInnerTreeView.GetTreeViewContainer
  : TCustomdxPSExplorerTreeViewContainer;
begin
