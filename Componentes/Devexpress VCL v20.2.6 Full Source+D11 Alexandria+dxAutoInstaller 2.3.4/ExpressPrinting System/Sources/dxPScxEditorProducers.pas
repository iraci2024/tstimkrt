...
uses
  ...
  cxContainer, cxEdit, cxMemo, cxTextEdit,
  cxCheckBox, dxCheckGroupBox, cxRadioGroup, cxBlobEdit, cxHyperLinkEdit,
  cxCurrencyEdit, cxGroupBox, cxDropDownEdit,
  cxImageComboBox, dxToggleSwitch, cxImage, dxThemeManager, dxPSGlbl,
  dxPScxCommon, dxPSContainerLnk, dxPSCore,
  cxLookAndFeelPainters, cxDrawTextUtils, dxPSReportRenderCanvas, dxSparkline,
  dxBarCode, dxBarCodeUtils, dxTokenEdit,
  dxFormattedLabel;
...
type
  ...
  protected
    ...
    function CanUseNativeStyle(AThemedObjectType: TdxThemedObjectType)
      : Boolean; virtual;
  ...
  TdxPScxNativePrintableControlProducer = class
  ...
  protected
    ...
    procedure InitializeNativePrintableControlHost
      (AnItem: TdxReportVisualItem); override;
    property BorderColor: TColor read GetBorderColor;
  public
    function Control: TcxContainer; reintroduce; overload;
    class function ControlClass: TControlClass; override;
  ...
  public
    ...
    class function ControlClass: TControlClass; override;
    function Properties: TcxCustomHyperLinkEditProperties; reintroduce;
      overload;
  end;
  ...
  public
    ...
    class function ControlClass: TControlClass; override;
    function Properties: TcxCustomImageComboBoxProperties; reintroduce;
      overload;
  end;
  ...
  protected
    function CanProcessChild(AChildControl: TControl): Boolean; override;
    procedure InitializeItem(AnItem: TdxReportVisualItem); override;
    procedure InitializeItemLookAndFeel(AnItem: TdxReportVisualItem;
      ALookAndFeel: TdxPSReportGroupLookAndFeel); virtual;
    function ItemClass: TdxReportVisualItemClass; override;
  ...
  protected
    ...
    procedure InitializeItem(AnItem: TdxReportVisualItem); override;
    procedure InitializeItemLookAndFeel(AnItem: TdxReportVisualItem;
      ALookAndFeel: TdxPSReportGroupLookAndFeel); override;
    function ItemClass: TdxReportVisualItemClass; override;
  public
  ...
  public
    ...
    property SymbologyClassName: string read FSymbologyClassName
      write SetSymbologyClassName;
...
const
  CheckBorderStyleMap: array [TcxContainerBorderStyle]
    of TdxCheckButtonEdgeStyle = (cbesNone, { cbesUltraFlat } cbesSingle,
    cbesBoldFlat, cbesSoft3D, cbes3D, cbesUltraFlat, cbesUltraFlat);
  CheckPosMap: array [TAlignment] of TdxCellCheckPos = (ccpLeft, ccpRight,
    ccpCenter);
  ContainerBorderStyleMap: array [TcxContainerBorderStyle]
    of TdxPSCellBorderClass = (TdxPSCellNullBorder, TdxPSCellUltraFlatBorder,
    TdxPSCellBoldFlatBorder, TdxPSCellSunkenSoftBorder, TdxPSCellSunkenBorder,
    TdxPSCellUltraFlatBorder, TdxPSCellUltraFlatBorder);
  ...
  RadioLookAndFeelKindMap: array [TcxLookAndFeelKind]
    of TdxCheckButtonEdgeStyle = (cbesSoft3D, cbes3D, cbesUltraFlat,
    cbesUltraFlat);
  RadioBorderStyleMap: array [TcxContainerBorderStyle]
    of TdxCheckButtonEdgeStyle = (cbesNone, cbesUltraFlat, cbesBoldFlat,
    cbesSoft3D, cbes3D, cbesUltraFlat, cbesUltraFlat);
  GroupBoxCaptionAlignmentMap: array [TcxCaptionAlignment] of TcxTextAlignX =
    (taLeft, taCenterX, taRight, taLeft, taLeft, taLeft, taLeft, taLeft, taLeft,
    taLeft, taLeft, taLeft, taLeft);
...
uses
  ...
  cxClasses,
  dxPScxEditorLnks, dxPScxListBoxLnk, cxGeometry, dxGDIPlusClasses, cxEditUtils;
...
begin
  ...
  else if Style.BorderStyle in [cbsSingle, cbsThick] then
...
function TdxPScxContainerProducer.CanProcessChild(AChildControl
  : TControl): Boolean;
begin
  Result := inherited CanProcessChild(AChildControl) and
    not Supports(AChildControl, IcxContainerInnerControl);
end;

function TdxPScxContainerProducer.CanUseNativeStyle(AThemedObjectType
  : TdxThemedObjectType): Boolean;
begin
  Result := IsNativeStyle and dxThemeManager.AreVisualStylesAvailable
    (AThemedObjectType);
end;
...
class function TdxPScxNativePrintableControlProducer.ControlClass
  : TControlClass;
begin
  Result := TcxContainer;
end;
...
class function TdxPScxNativePrintableControlProducer.
...
procedure TdxPScxNativePrintableControlProducer.
...
const
  ...
  // Actually should be borrowed from AnItem.ShadowDepth
begin
...
begin
  Result := cxTextEditProperties_GetDisplayText(Properties,
    Control.EditValue, False);
end;

procedure TdxPScxCustomTextEditProducer.InitializeItem
...
begin
  ...
  TdxReportCellString(AnItem).TextAlignX := EditTextAlignXMap
    [EditAlignment.Horz];
  TdxReportCellString(AnItem).TextAlignY := EditTextAlignYMap
    [EditAlignment.Vert];
end;
...
procedure TdxPScxCustomMemoEditProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
  TdxReportCellString(AnItem).MultiLine := True;
  TdxReportCellString(AnItem).Text := Control.Text;
  TdxReportCellString(AnItem).TextAlignX :=
    TextAlignXMap[cxMemoProperties_GetAlignment(Properties)];
...
function TcxPScxCustomHyperLinkEditProducer.Properties
...
function TcxPScxCustomCurrencyEditProducer.Properties
...
function TdxPScxCustomImageComboBoxProducer.Properties
  : TcxCustomImageComboBoxProperties;
begin
  Result := inherited Properties as TcxCustomImageComboBoxProperties;
end;

procedure TdxPScxCustomImageComboBoxProducer.GetImageLists
...
procedure TdxPScxCustomImageComboBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
var
  ItemIndex: Integer;
...
begin
  ...
  begin
    TdxReportCellImage(AnItem).ImageIndex :=
      cxImageComboBoxProperties_GetDefaultImageIndex(Properties);
    TdxReportCellImage(AnItem).Text :=
      cxImageComboBoxProperties_GetDefaultDescription(Properties);
  end;

  TdxReportCellImage(AnItem).ImageLayout := ImageLayoutMap
    [cxImageComboBoxProperties_GetImageAlignment(Properties)];
  TdxReportCellImage(AnItem).ImageList := cxImageComboBoxProperties_GetImages
    (Properties);
  TdxReportCellImage(AnItem).MakeSpaceForEmptyImage := True;
  TdxReportCellImage(AnItem).MultiLine :=
    cxImageComboBoxProperties_GetIsMultilined(Properties);
...
procedure TdxPScxCustomCheckBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
var
...
begin
  ...
  TdxReportCellCheckImage(AnItem).Checked := (CheckState = cbsChecked) or
    ((CheckState = cbsGrayed) and (CheckNullStyle = nssGrayedChecked));
  TdxReportCellCheckImage(AnItem).ButtonEdgeStyle := CheckBorderStyleMap
    [Style.BorderStyle];
  TdxReportCellCheckImage(AnItem).CheckPos :=
    CheckPosMap[cxCheckBoxProperties_GetAlignment(Properties)];
  TdxReportCellCheckImage(AnItem).Enabled := Control.Enabled and
    not((CheckState = cbsGrayed) and (CheckNullStyle in [nssInactive,
    nssGrayedChecked]));
  ...
  begin
    ...
    TdxReportCellCheckImage(AnItem).GlyphCount :=
      cxCheckBoxProperties_GetGlyphCount(Properties);
  end;
  TdxReportCellCheckImage(AnItem).MultiLine :=
    cxCheckBoxProperties_GetIsMultilined(Properties);
...
begin
  ...
  begin
    TdxSparklineImage(AnItem).ViewData := Control.Properties.CreateViewData
      (DefaultEditStyleController.Style, True);
    TdxSparklineImage(AnItem).ViewInfo :=
      Control.Properties.GetViewInfoClass.Create as TdxSparkLineViewInfo;
    TdxSparklineImage(AnItem).Transparent := True;
    TdxSparklineImage(AnItem).ViewData.EditValueToDrawValue(Control.EditValue,
      TdxSparklineImage(AnItem).ViewInfo);
  end;
  inherited;
...
begin
  ...
  dxPSDataMaps.InitializeItem(AnItem as TdxReportCellTokens,
    Control.ActiveProperties, Control.EditValue, nil, AViewParams);
end;
...
procedure TdxPSdxFormattedLabelProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
const
...
procedure TdxPSdxToggleSwitchProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
  ...
  var
    ...
    ARect: TRect;
  begin
    AViewInfo := TdxToggleSwitchViewInfoAccess(TdxToggleSwitchAccess(Control)
      .ViewInfo);
    ...
    ARect.Left := ARect.Left + cxRectWidth(AViewInfo.TextRect) +
      cxGetValueCurrentDPI(dxToggleSwitchTextOffset);
...
procedure TdxPScxRadioButtonProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
  TdxReportCellRadio(AnItem).ButtonEdgeStyle := RadioLookAndFeelKindMap
    [Control.LookAndFeel.Kind];
...
function TdxPScxCustomBlobEditProducer.CanProcessChild(AChildControl
  : TControl): Boolean;
begin
  Result := False;
end;
...
function TdxPScxCustomBlobEditProducer.Producer
  : TdxPSCustomContainerItemProducer;
const
  ProducerClasses: array [TcxBlobPaintStyle]
    of TdxPSCustomContainerItemProducerClass = (TdxPScxDefaultBlobEditProducer,
    TdxPScxPictureBlobProducer, TdxPScxTextBlobEditProducer);
var
  ProducerClass: TdxPSCustomContainerItemProducerClass;
begin
  ProducerClass := ProducerClasses
    [cxBlobEditProperties_GetPaintStyle(cxEdit_GetProperties(Control))];
  Result := TdxPSCustomContainerItemProducer
    (ReportLink.ProducersByClass[ProducerClass, Control]);
end;
...
procedure TdxPScxDefaultBlobEditProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
  TdxReportCellString(AnItem).Text := cxBlobEdit.GetBlobText(Control.EditValue,
    TcxCustomBlobEditProperties(cxEdit_GetProperties(Control)), False);
end;
...
procedure TdxPScxPictureBlobProducer.GetImageLists
  (AProc: TdxPSGetImageListProc);
begin
  inherited;
  AProc(GetBlobImages);
...
procedure TdxPScxPictureBlobProducer.InitializeItem
  (AnItem: TdxReportVisualItem);

  function HasEditValue: Boolean;
  begin
    Result := not VarIsNull(Control.EditValue) and
      not VarIsEmpty(Control.EditValue);
  end;
...
begin
  ...
  TdxReportCellGraphic(AnItem).ImageIndex := Ord(HasEditValue) + ImageIndexes
    [cxBlobEditProperties_GetEditKind(cxEdit_GetProperties(Control))];
end;
...
procedure TdxPScxTextBlobEditProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
...
procedure TdxPScxCustomImageEditProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
  if HasImage then
...
procedure TdxPScxCustomImageEditProducer.InitializeItemAsImage
...
begin
  ...
  AnItem.ImageTransparent :=
    ((GraphicClass <> nil) and GraphicClass.InheritsFrom(TIcon)) or
    ReportLink.OptionsTransparent.Graphics;
  AnItem.Stretch := (GraphicClass <> nil) and not GraphicClass.InheritsFrom
    (TIcon) and cxImageProperties_GetStretch(Properties);
end;

procedure TdxPScxCustomImageEditProducer.InitializeItemAsText
...
function TdxPScxCustomGroupBoxProducer.CanProcessChild(AChildControl
  : TControl): Boolean;
begin
  Result := True;
end;
...
procedure TdxPScxCustomGroupBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited InitializeItem(AnItem);
  TdxReportGroup(AnItem).Transparent := OptionsTransparent.Containers;
  TdxReportGroup(AnItem).BorderClass := ContainerBorderStyleMap
    [Self.Style.BorderStyle];
  TdxReportGroup(AnItem).CaptionAlignment := GroupBoxCaptionAlignmentMap
    [cxGroupBox_GetAlignment(Control)];
  TdxReportGroup(AnItem).CaptionText := Control_GetText(Control);
  TdxReportGroup(AnItem).CaptionTransparent := TdxReportGroup(AnItem)
    .Transparent;
  TdxReportGroup(AnItem).UseOwnBorderClass := True;

  TdxReportGroup(AnItem).LookAndFeel := ReportLink.CreateGroupLookAndFeel
    (LookAndFeelClass, False);
...
procedure TdxPScxCustomGroupBoxProducer.InitializeItemLookAndFeel
...
function TdxPScxCustomGroupBoxProducer.LookAndFeelClass
...
function TdxPSdxCustomCheckGroupBoxProducer.Properties
  : TdxCheckGroupBoxProperties;
begin
  Result := inherited Properties as TdxCheckGroupBoxProperties;
end;

procedure TdxPSdxCustomCheckGroupBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited InitializeItem(AnItem);
  TdxReportGroup(AnItem).CheckBoxVisible := TdxCustomCheckGroupBoxAccess
    (Control).CheckBox.Visible;
  TdxReportGroup(AnItem).CheckBoxChecked := TdxCustomCheckGroupBoxAccess
...
function TdxPScxCustomButtonGroupProducer.Properties
  : TcxCustomButtonGroupProperties;
begin
  Result := inherited Properties as TcxCustomButtonGroupProperties;
end;

procedure TdxPScxCustomButtonGroupProducer.CreateItems
  (AButtonGroup: TdxCustomReportButtonGroup);
var
...
procedure TdxPScxCustomButtonGroupProducer.InitializeButton
  (AGroup: TdxCustomReportButtonGroup; AButton: TdxCustomReportCellCheck;
  AnIndex: Integer);
begin
...
procedure TdxPScxCustomButtonGroupProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
  TdxCustomReportButtonGroup(AnItem).ButtonEdgeStyle := RadioBorderStyleMap
    [Style.BorderStyle];
  TdxCustomReportButtonGroup(AnItem).ColumnCount :=
    cxButtonGroupProperties_GetColumnCount(Properties);
  CreateItems(TdxCustomReportButtonGroup(AnItem));
  TdxCustomReportButtonGroup(AnItem).ShowCaption :=
    (TdxCustomReportButtonGroup(AnItem).CaptionText <> '') and
    (cxGroupBox_GetAlignment(Control) <> alCenterCenter);
  TdxCustomReportButtonGroup(AnItem).AdjustContent(Canvas);
end;

procedure TdxPScxCustomButtonGroupProducer.InitializeItemLookAndFeel
  (AnItem: TdxReportVisualItem; ALookAndFeel: TdxPSReportGroupLookAndFeel);
begin
  inherited;
end;
...
function TdxPScxCustomRadioGroupProducer.CanProcessChild(AChildControl
  : TControl): Boolean;
begin
  Result := inherited CanProcessChild(AChildControl) and
    not(AChildControl is TcxCustomRadioGroupButton);
  // RadioGroup buttons are processed inside
end;

procedure TdxPScxCustomRadioGroupProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
...
procedure TdxPScxCustomRadioGroupProducer.InitializeButton
  (AGroup: TdxCustomReportButtonGroup; AButton: TdxCustomReportCellCheck;
  AnIndex: Integer);
begin
  inherited;
...
procedure TdxPSBarCodePainter.DrawText(const ARect: TRect; const AText: string;
...
procedure TdxReportCellBarCode.DrawContent
...
begin
  ...
  begin
    ...
    try
      ...
      TdxCustomBarCodeSymbologyAccess(ASymbology).ModuleWidth :=
        ConvertValue(ModuleWidth);
      ...
      try
        ...
        begin
          ADifferenceSize :=
            (cxRectWidth(BoundsRect) - cxRectHeight(BoundsRect)) div 2;
          ARect := cxRectInflate(BoundsRect, -ADifferenceSize + 1,
            ADifferenceSize + 1, -ADifferenceSize + 1, ADifferenceSize + 1);
        end;
        APainter := TdxPSBarCodePainter.Create;
        try
          APainter.Canvas := ACanvas;
          TdxCustomBarCodeSymbologyAccess(ASymbology).Paint(APainter, ARect,
            Text, Font);
        finally
          APainter.Free;
        end;
...
function TdxReportCellBarCode.MeasureContentHeight
  (ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin
  Result := GetBarCodeSize.cy;
end;

function TdxReportCellBarCode.MeasureContentWidth
  (ACanvas: TdxPSReportRenderCustomCanvas): Integer;
begin
...
procedure TdxReportCellBarCode.ReadData(AReader: TdxPSDataReader);
  ...
  begin
    ...
    begin
      TdxBarCodeInterleaved2Of5Symbology(Symbology).Checksum :=
        AReader.ReadBoolean;
      TdxBarCodeInterleaved2Of5Symbology(Symbology).WideNarrowRatio :=
        AReader.ReadSingle;
    end;
    ...
    begin
      TdxBarCodeQRCodeSymbology(Symbology).CompactionMode :=
        TdxQRCodeCompactionMode(AReader.ReadInteger);
      TdxBarCodeQRCodeSymbology(Symbology).ErrorCorrectionLevel :=
        TdxQRCodeErrorCorrectionLevel(AReader.ReadInteger);
...
procedure TdxReportCellBarCode.WriteData(AWriter: TdxPSDataWriter);
  ...
  begin
    if FSymbologyClass = TdxBarCodeInterleaved2Of5Symbology then
    begin
      AWriter.WriteBoolean(TdxBarCodeInterleaved2Of5Symbology(Symbology)
        .Checksum);
      AWriter.WriteSingle(TdxBarCodeInterleaved2Of5Symbology(Symbology)
        .WideNarrowRatio);
    end;
    if FSymbologyClass = TdxBarCode39Symbology then
    begin
    ...
    if FSymbologyClass = TdxBarCodeQRCodeSymbology then
    begin
      AWriter.WriteInteger(Integer(TdxBarCodeQRCodeSymbology(Symbology)
        .CompactionMode));
      AWriter.WriteInteger(Integer(TdxBarCodeQRCodeSymbology(Symbology)
...
begin
  ...
  begin
    ...
    try
      if (TdxCustomBarCodeSymbologyAccess(Symbology).CalculateSize(Text, Font,
        AWidth, AHeight) = bceNone) then
        Result := cxSize(MulDiv(AWidth, PixelsDenominator, PixelsNumerator),
          MulDiv(AHeight, PixelsDenominator, PixelsNumerator));
    finally
      TdxCustomBarCodeSymbologyAccess(Symbology).ModuleWidth :=
        MulDiv(TdxCustomBarCodeSymbologyAccess(Symbology).ModuleWidth,
        PixelsDenominator, PixelsNumerator);
    end;
  end;
end;

function TdxReportCellBarCode.GetTransformMatrix(ARotationAngle
  : TcxRotationAngle): XFORM;
var
  ACenter: TPoint;
begin
...
begin
  ...
  begin
    ...
    FreeAndNil(FSymbology);
    FSymbologyClass := TdxCustomBarCodeSymbologyClass
      (dxGetRegisteredBarCodeSymbologies.FindByClassName(SymbologyClassName));
...
class procedure TdxPSBarCodeDataMap.InitializeItem
...
class function TdxPSBarCodeDataMap.ItemClass(AProperties
  : TcxCustomEditProperties; AnIsPreview: Boolean): TdxReportCellDataClass;
begin
  Result := TdxReportCellBarCode;
end;

class function TdxPSBarCodeDataMap.PropertiesClass
