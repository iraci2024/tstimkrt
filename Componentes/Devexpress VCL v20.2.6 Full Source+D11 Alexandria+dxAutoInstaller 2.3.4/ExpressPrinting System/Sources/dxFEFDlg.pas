...
{$IFDEF DELPHI16}
  UITypes,
{$ENDIF}
  Types, Windows, Messages, SysUtils, Classes, Graphics, Controls, ExtDlgs,
  Registry,
  StdCtrls, ExtCtrls, ComCtrls, Grids, Forms, Menus, Consts, ImgList, IniFiles,
  dxCore, dxPSForm, dxBkgnd, cxLookAndFeelPainters, cxButtons, cxControls,
  cxContainer,
  cxEdit, cxGroupBox, cxLabel, cxGraphics, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxColorComboBox,
  cxPC, dxExtCtrls, dxPSReportRenderCanvas, cxLookAndFeels, dxLayoutContainer,
  dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, dxLayoutLookAndFeels, cxClasses, dxLayoutControl,
  cxImageList, cxGeometry;
...
type
  ...
  TdxFEFDialog = class(TCustomdxPSForm)
    ...
    procedure dgPatternDrawCell(Sender: TObject; Col, Row: Integer; Rect: TRect;
      State: TGridDrawState);
    procedure dgPatternMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    procedure dgTextureClick(Sender: TObject);
    procedure dgTextureDblClick(Sender: TObject);
    procedure dgTextureDrawCell(Sender: TObject; Col, Row: Integer; Rect: TRect;
      State: TGridDrawState);
    procedure dgTextureMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
  ...
  private
    ...
    function InternalLoadImage(var AImage: TGraphic;
    ...
    property PaintMode: TdxPicturePaintMode read GetPaintMode
      write SetPaintMode;
...
begin
  ...
  try
    ...
    Result := Dialog.Execute or not Dialog.FFirstApplied;
    { at least one time button "Apply" was pressed }
...
{$ENDIF}
      if CompareText(Extention, '.' + cxGraphicExtension(TMetafile)) = 0 then
        Result := TMetafile
      else if CompareText(Extention, '.wmf') = 0 then
        Result := TMetafile;
end;
      ...
      case FBackground.Mode of
        ...
        bmPicture:
          begin
            ...
            GetOtherPicture(TGraphicClass(FBackground.Picture.ClassType))
              .Assign(FBackground.Picture);
            FPictureExists := True;
...
begin
  ...
  btnApply.Enabled := FModified and Assigned(FPicture) and
    not(FPicture.Empty or FApplied);
...
procedure TdxFEFDialog.dgTextureDrawCell(Sender: TObject; Col, Row: Integer;
...
begin
  ...
  if not((gdSelected in State) and FTextureWasSelected) then
    if (FOtherTexture = nil) or ((Row < DrawGrid.RowCount - 1) or (Col = 0))
    then
    begin
  ...
  begin
    ...
    DoHalftone := (BPP <= 8) and
      (BPP < (ABitmap.bmBitsPixel * ABitmap.bmPlanes));
    if DoHalftone then
    begin
      GetBrushOrgEx(DC, Pt);
  ...
  if FOtherTexture = nil then
    Windows.StretchBlt(DC, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top,
      FbmpTexture.Canvas.Handle, Col * TextureSize.X, Row * TextureSize.Y,
      TextureSize.X, TextureSize.Y, SRCCOPY)
  ...
  if gdSelected in State then
    if (FTextureWasSelected and (FOtherTexture = nil)) or
      ((FOtherTexture <> nil) and ((Row < DrawGrid.RowCount - 1) or
      (DrawGrid.Col = 0))) then
      DrawSelectedFrame(DrawGrid, Rect);
end;

procedure TdxFEFDialog.dgPatternDrawCell(Sender: TObject; Col, Row: Integer;
  Rect: TRect; State: TGridDrawState);
var
  APrevBrush: TBrush;
...
procedure TdxFEFDialog.LoadFromIniFile(AIniFile: TCustomIniFile;
...
procedure TdxFEFDialog.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
    ...
    try
      ...
      begin
        ...
        begin
          ...
          FileName := { Dialog.FileName;// } ChangeFileExt
          ...
          case AWhat of
            0: // textures
              begin
                ...
                pnlTextureName.Caption := FileName;
...
function TdxFEFDialog.InternalLoadImage(var AImage: TGraphic;
  const AFileName: string): Boolean;
var
  Picture: TPicture;
begin
...
procedure TdxFEFDialog.PaintPreview(ACanvas: TCanvas; const R: TRect);
  ...
  begin
    ...
    for I := 0 to cxRectWidth(R) div AImageSize.cx do
      for J := 0 to cxRectHeight(R) div AImageSize.cy do
        ACanvas.StretchDraw(Bounds(I * AImageSize.cx, J * AImageSize.cy,
          AImageSize.cx, AImageSize.cy), AGraphic);
  end;
  ...
  begin
    case AMode of
      ...
      ppmProportional:
        ACanvas.StretchDraw(cxRectProportionalStretch(R, GetImageSize(AImage)
          ), AImage);
    end;
  end;
...
begin
  dgTexture.DefaultColWidth := (dgTexture.Width - GetSystemMetrics(SM_CXHSCROLL)
    - ScaleFactor.Apply(1)) div dgTexture.ColCount - ScaleFactor.Apply(1);
  dgTexture.DefaultRowHeight := dgTexture.DefaultColWidth;

  dgPattern.DefaultColWidth := (dgPattern.Width - ScaleFactor.Apply(1))
    div dgPattern.ColCount - ScaleFactor.Apply(1);
  dgPattern.DefaultRowHeight := (dgPattern.Height - ScaleFactor.Apply(1))
    div dgPattern.RowCount - ScaleFactor.Apply(1);
end;
...
begin
  ...
  else if (DrawGrid.Row = DrawGrid.RowCount - 1) and (DrawGrid.Col > 0) then
    Exit
  ...
  FPreviewWhat := faTexture;
  if Assigned(FOtherTexture) and (DrawGrid.Col = 0) and
    (DrawGrid.Row = DrawGrid.RowCount - 1) then
    pnlTextureName.Caption := FOtherTextureName
  else
    pnlTextureName.Caption := FTextureNames[DrawGrid.Col * TextureCount.Y +
      DrawGrid.Row];
  CheckModified;
  pbxPreview.Invalidate;
end;
...
begin
  ...
  FPreviewWhat := faPattern;
  pbxPreview.Refresh;
  pnlPatternName.Caption := FPatternNames[DrawGrid.Col * TextureCount.Y +
    DrawGrid.Row];
  CheckModified;
end;
...
begin
  PaintPreview(pbxPreview.Canvas, cxRectInflate(pbxPreview.ClientRect,
    ScaleFactor.Apply(2)));
end;
...
begin
  FreeAndNil(FbmpCurrentPattern);
  FbmpCurrentPattern := dxCreateMappedBmp(FbmpPattern, [clWhite, clBlack],
    [BackColor, ForeColor]);
end;
  ...
  case FPreviewWhat of
    ...
    faTexture:
      ...
      begin
        ...
        else if dgTexture.Row < dgTexture.RowCount - 1 then
          FPicture := CopyTexture(dgTexture.Col, dgTexture.Row)
        else if dgTexture.Col = 0 then
          SetPicture(OtherTexture);
      end;
  end;
  pbxPreview.Invalidate;
...
begin
  if FTextureWasSelected and (lgTabbed.ItemIndex = 0) then
    Result := faTexture
  else if FPatternWasSelected and (lgTabbed.ItemIndex = 1) then
    Result := faPattern
  else if FPictureExists and (lgTabbed.ItemIndex = 2) then
    Result := faPicture
  else
    Result := faNone;
end;
...
procedure TdxFEFDialog.dgTextureMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
const
...
begin
  ...
  begin
    ...
    if (Col > -1) and (Row > -1) then
      ...
      else if (FOtherTexture <> nil) and (Col = 0) and
        (Row = TDrawGrid(Sender).RowCount - 1) then
        S := FOtherTextureName
      else if Row < TDrawGrid(Sender).RowCount - 1 then
        S := FTextureNames[Col * TextureCount.Y + Row];
...
procedure TdxFEFDialog.dgPatternMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
const
