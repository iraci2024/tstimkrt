...
uses
  Types, Classes, Windows, Messages, Graphics, Controls, Forms, Dialogs,
  StdCtrls, ImgList, CommCtrl, ExtCtrls,
  ComCtrls, {$IFDEF REGISTERSHELLCTRLS}ShellCtrls, {$ENDIF} dxPSCore, dxPSGlbl,
  dxPSRes, cxDrawTextUtils, cxPC,
  cxControls, cxContainer, cxEdit, cxCheckBox, cxLabel, cxGraphics, cxSpinEdit,
  cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxColorComboBox, Menus, cxLookAndFeelPainters, cxButtons,
  dxPSReportRenderCanvas, cxLookAndFeels, cxClasses,
...
type
  ...
  TdxTreeViewPaintOption = (tvpoBorder, tvpoGrid, tvpoStateImages, tvpoImages,
    tvpoTreeLines, tvpoButtons);
  TdxTreeViewPaintOptions = set of TdxTreeViewPaintOption;

  TCustomdxNativeTreeViewReportLink = class(TBasedxReportLink,
  ...
  protected
    ...
    procedure AssignData(ANode: HTREEITEM;
    ...
    procedure InitializeItem(ANode: HTREEITEM;
      ADataItem: TAbstractdxReportCellData); virtual;
    ...
    function TV_GetNodeParentAtLevel(AItem: HTREEITEM; ALevel: Integer)
      : HTREEITEM;
    ...
    property TreeViewHandle: THandle read GetTreeViewHandle
      write SetTreeViewHandle;
  public
    ...
    property AutoNodesExpand: Boolean read FAutoNodesExpand
      write SetAutoNodesExpand default False;
    ...
    property ExpandLevel: Integer read FExpandLevel write SetExpandLevel
      default -1;
    property ExplicitTreeViewExpand: Boolean read FExplicitTreeViewExpand
      write SetExplicitTreeViewExpand default False;
    property Font;
    ...
    property NodeAutoHeight: Boolean read FNodeAutoHeight
      write SetNodeAutoHeight default False;
    property ScaleFonts;
  ...
  TdxTVInitializeNodeEvent = procedure(Sender: TBasedxReportLink;
    ANode: TTreeNode; AnItem: TdxReportCellString) of object;

  TdxTVCustomDrawNodeEvent = procedure(Sender: TBasedxReportLink;
    ANode: TTreeNode; ACanvas: TCanvas; ABoundsRect, AClientRect: TRect;
  ...
  protected
    ...
    procedure InternalRestoreDefaults; override;
    function IsSupportedCustomDraw(Item: TAbstractdxReportCellData): Boolean;
      override;
    ...
    procedure DoInitializeNode(ANode: TTreeNode;
    ...
    property OnInitializeNode: TdxTVInitializeNodeEvent read FOnInitializeNode
      write FOnInitializeNode;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
...
const
  dxDefaultTreeViewPaintOptions: TdxTreeViewPaintOptions = [tvpoStateImages,
    tvpoImages, tvpoTreeLines, tvpoButtons];
...
uses
  ClipBrd, SysUtils, Math, dxPrnDev, dxPSUtl, dxExtCtrls, cxGeometry, dxPSImgs,
  dxCore, dxDPIAwareUtils;
...
begin
  case AnIndex of
    0:
      Result := cxGetResourceString(@sdxTechnicalDepartment);
    1:
      Result := cxGetResourceString(@sdxSoftwareDepartment);
    2:
      Result := cxGetResourceString(@sdxSystemProgrammers);
    3:
      Result := cxGetResourceString(@sdxEndUserProgrammers);
    4:
      Result := cxGetResourceString(@sdxBetaTesters);
  else
...
procedure TCustomdxNativeTreeViewReportLink.ConstructReport
  (AReportCells: TdxReportCells);

  procedure InsertNodeIndent(AParentCell: TdxReportCell; ANode: HTREEITEM;
    var ALeftOffset: Integer);

    function GetIndentShowButton(AtLevel: Integer; ANode: HTREEITEM): Boolean;
    begin
      Result := (tvpoButtons in Options) and
        (TV_GetNodeParentAtLevel(ANode, AtLevel) = ANode) and
    ...
    begin
      if AutoNodesExpand then
        if ExplicitTreeViewExpand then
          Result := TV_IsNodeExpanded(ANode) and
            ((ExpandLevel = -1) or (ExpandLevel > TV_GetNodeLevel(ANode)))
        else
    ...
    function GetIndentTreeLineMode(AtLevel: Integer; ANode: HTREEITEM)
      : TdxPSTreeLineMode;
    var
      Parent: HTREEITEM;
    begin
      ...
      begin
        ...
        if Parent = ANode then
          ...
          else if not TV_IsNodeParentExists(ANode) and TV_IsNodeFirstChild(ANode)
          then
            Result := tlmTopRightCorner
          else
            Result := tlmCross
        else if TV_IsNodeLastChild(Parent) then
          Result := tlmNone
        else
          Result := tlmVertical;
      end
    ...
    function GetIndentCellSides(I, ALevel: Integer; ANode: HTREEITEM)
      : TdxCellSides;
    begin
      Result := [];
      if IsDrawGrid then
      begin
        ...
        { 2.0 } if (I = ALevel) or (not TV_IsNodeParentExists(ANode) and
          (not IsFirstNode(ANode) or IsDrawBorder)) then
          Include(Result, csTop);

        if (TV_GetNodeCount(ANode) = 0) or
          (not TV_IsNodeExpanded(ANode) and not AutoNodesExpand) then
        begin
          if I = ALevel then
          begin
          ...
          else if TV_IsNodeLastChild(ANode) then
            Include(Result, csBottom);
        end;
      end;
  ...
  procedure InsertImage(AParentCell: TdxReportCell; ANode: HTREEITEM;
    ...
    function GetImageCellSides(ANode: HTREEITEM; AnItem: TdxReportVisualItem)
      : TdxCellSides;
    begin
      Result := [];
      if IsDrawBorder then
  ...
  begin
    ...
    if IsDrawNodeStateImages(ANode) then
      InsertImage(Cell, ANode, StateImages, TV_GetNodeStateIndex(ANode),
        LeftOffset);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if IsDrawNodeStateImages(Node) then
            Inc(W, StateImages.Width);
          if IsDrawNodeImages(Node) then
            Inc(W, Images.Width);
  ...
  begin
    FNodeList.Clear;
    if TreeView_GetCount(TreeViewHandle) = 0 then
      Exit;
...
begin
  ...
  if not AbortBuilding then
    with AReportCells do
      Cells.BoundsRect := Rect(0, 0, FActualWidth,
        Cells.LastCell.BoundsRect.Bottom);
end;

procedure TCustomdxNativeTreeViewReportLink.GetImageLists
...
procedure TCustomdxNativeTreeViewReportLink.MakeDelimiters
...
function TCustomdxNativeTreeViewReportLink.GetDataClass(ANode: HTREEITEM)
...
begin
  if FStateImages.Count = 0 then
    CopyImages(TreeView_GetImageList(TreeViewHandle, TVSIL_STATE),
      FStateImages);
  Result := FStateImages;
end;
...
procedure TCustomdxNativeTreeViewReportLink.DoExplicitTreeViewExpand
  (ALevel: Integer = -1);
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeCount
  (AItem: HTREEITEM): Integer;
var
  Child: HTREEITEM;
begin
  Result := 0;
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeHasChildren
  (AItem: HTREEITEM): Boolean;
var
  Item: TTVItem;
begin
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeImageIndex
  (AItem: HTREEITEM): Integer;
var
  Item: TTVItem;
begin
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeLevel
  (AItem: HTREEITEM): Integer;
var
  Parent: HTREEITEM;
begin
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeParent(AItem: HTREEITEM)
  : HTREEITEM;
begin
  Result := TreeView_GetParent(TreeViewHandle, AItem);
end;

function TCustomdxNativeTreeViewReportLink.TV_GetNodeParentAtLevel
  (AItem: HTREEITEM; ALevel: Integer): HTREEITEM;
begin
  Result := AItem;
  while ALevel > 0 do
  begin
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeStateIndex
  (AItem: HTREEITEM): Integer;
var
  Item: TTVItem;
begin
...
function TCustomdxNativeTreeViewReportLink.TV_GetNodeText
  (AItem: HTREEITEM): string;
const
  BufferLength = 4096;
var
  Item: TTVItem;
...
function TCustomdxNativeTreeViewReportLink.TV_IsNodeExpanded
  (AItem: HTREEITEM): Boolean;
var
  Item: TTVItem;
begin
...
function TCustomdxNativeTreeViewReportLink.TV_IsNodeParentExists
  (AItem: HTREEITEM): Boolean;
begin
  Result := TV_GetNodeParent(AItem) <> nil;
end;

function TCustomdxNativeTreeViewReportLink.TV_IsNodeFirstChild
  (AItem: HTREEITEM): Boolean;
begin
  Result := TreeView_GetPrevSibling(TreeViewHandle, AItem) = nil;
end;

function TCustomdxNativeTreeViewReportLink.TV_IsNodeLastChild
  (AItem: HTREEITEM): Boolean;
begin
...
begin
  Result := GetWindowLong(TreeViewHandle, GWL_STYLE) and
...
procedure TCustomdxNativeTreeViewReportLink.SetExplicitTreeViewExpand
  (Value: Boolean);
begin
...
procedure TCustomdxNativeTreeViewReportLink.SetOptions
  (Value: TdxTreeViewPaintOptions);
begin
...
begin
  ...
  begin
    ...
    if CanDrawStateImages then
      Dec(W, StateImages.Width + 2);
    if CanDrawImages then
      Dec(W, Images.Width + 2);

    R := Rect(0, 0, W, 0);
    H := Renderer.CalcTextRect(FScreenCanvas, TV_GetNodeText(Node), R,
      False, nil);
    if H < FNodeHeight then
      H := FNodeHeight;
    if not Odd(H) then
      Inc(H);
...
begin
  Result := (tvpoStateImages in Options) and CheckStateImages and
    StateImages.HandleAllocated;
end;

function TCustomdxNativeTreeViewReportLink.GetCellSides(ANode: HTREEITEM)
  : TdxCellSides;
var
...
begin
  ...
  begin
    ...
    if not IsLastNode(ANode) or IsDrawBorder then
      Include(Result, csBottom);
    if IsDrawBorder and (TV_GetNodeLevel(ANode) = 0) and not IsAnyImagesExists
    then
      Include(Result, csLeft);
  end;
  if IsDrawBorder then
...
function TCustomdxNativeTreeViewReportLink.GetNodeHeight
...
function TCustomdxNativeTreeViewReportLink.IsDrawNodeImages
  (ANode: HTREEITEM): Boolean;
begin
  Result := CanDrawImages and IsValidNodeImageIndex(ANode);
end;

function TCustomdxNativeTreeViewReportLink.IsDrawNodeStateImages
  (ANode: HTREEITEM): Boolean;
begin
  Result := CanDrawStateImages and IsValidNodeStateIndex(ANode);
end;

function TCustomdxNativeTreeViewReportLink.IsFirstNode
  (ANode: HTREEITEM): Boolean;
begin
  Result := TObject(ANode) = FNodeList.First;
end;

function TCustomdxNativeTreeViewReportLink.IsLastNode(ANode: HTREEITEM)
  : Boolean;
begin
  Result := TObject(ANode) = FNodeList.Last;
end;

function TCustomdxNativeTreeViewReportLink.IsValidNodeImageIndex
  (ANode: HTREEITEM): Boolean;
var
  ImageIndex: Integer;
begin
...
function TCustomdxNativeTreeViewReportLink.IsValidNodeStateIndex
  (ANode: HTREEITEM): Boolean;
var
  StateIndex: Integer;
begin
...
procedure TdxNativeTreeViewReportLink.Notification(AComponent: TComponent;
...
function TdxNativeTreeViewReportLink.GetDesignerClass
...
begin
  ...
  if Source is TCustomdxTreeViewReportLink then
    SupportedCustomDraw := TCustomdxTreeViewReportLink(Source)
      .SupportedCustomDraw;
end;
...
function TCustomdxTreeViewReportLink.IsSupportedCustomDraw
...
procedure TCustomdxTreeViewReportLink.CustomDraw
...
begin
  ...
  begin
    ...
    DoCustomDrawNode(TreeViewGetItems(CustomTreeView).GetNode(HTREEITEM(Data)),
      ACanvas, ABoundsRect, AClientRect, AText, FSaveFont, AColor, ATextAlignX,
      ATextAlignY, ADone);
    if not ADone then
    begin
...
begin
  if Assigned(FOnCustomDrawNode) then
    FOnCustomDrawNode(Self, ANode, ACanvas, ABoundsRect, AClientRect, AText,
...
procedure TdxfmTVReportLinkDesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
const
  ...
  begin
    ...
    R := cxRectSetHeight(R, Trunc((cxRectHeight(R) - 2) / ItemsCount) *
      ItemsCount);
  ...
  begin
    ...
    cxDrawImage(ACanvas, cxRectSetWidth(R, AImageSize.cX), nil, ilPreview,
      AIndex, True, nil, ScaleFactor);
    Inc(R.Left, AImageSize.cX);
  end;

  procedure DrawItemGrid(ACanvas: TcxCanvas; R: TRect; ALevel: Integer;
  ...
  begin
    ...
    ACanvas.FrameRect(cxRectInflate(R, 1, 1, 0, 0), ReportLink.GridLineColor, 1,
      ABorders);
  end;
  ...
  begin
    ...
    dxAssignFont(ACanvas.Font, ReportLink.Font, ScaleFactor,
      ReportLink.ScaleFactor);
    ACanvas.Brush.Style := bsClear;
    cxDrawText(ACanvas.Handle, GetDesignerString(AIndex), R, DT_SINGLELINE or
      DT_VCENTER);
  end;
  ...
  begin
    ...
    begin
      ...
      AExpandButtonRect := cxRectCenter(AExpandButtonRect,
        ScaleFactor.Apply(11), ScaleFactor.Apply(11));
      cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat)
        .DrawScaledExpandButton(ACanvas, AExpandButtonRect, True, ScaleFactor);
    end;
  end;
...
begin
  ...
  case TTagToInt(TcxColorComboBox(Sender).Tag) of
    ...
    2:
      ReportLink.TreeLineColor := AColor;
  end;
  Modified := True;
  UpdatePreview;
end;
...
function TdxfmTVReportLinkDesignWindow.GetReportLink
  : TCustomdxNativeTreeViewReportLink;
begin
...
procedure TdxfmTVReportLinkDesignWindow.CMDialogChar
...
procedure TdxfmTVReportLinkDesignWindow.chbxNodeAutoHeightClick
  (Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
procedure TdxfmTVReportLinkDesignWindow.chbxAutoNodesExpandClick
  (Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
dxPSRegisterReportLink(TdxTreeViewReportLink, TTreeView,
  TdxfmTVReportLinkDesignWindow);
{$IFDEF REGISTERSHELLCTRLS}
dxPSRegisterReportLink(TdxShellTreeViewReportLink, TShellTreeView,
  TdxfmTVReportLinkDesignWindow);
{$ENDIF}
...
{$IFDEF REGISTERSHELLCTRLS}
  dxPSUnregisterReportLink(TdxShellTreeViewReportLink, TShellTreeView,
  TdxfmTVReportLinkDesignWindow);
{$ENDIF}
dxPSUnregisterReportLink(TdxTreeViewReportLink, TTreeView,
  TdxfmTVReportLinkDesignWindow);
