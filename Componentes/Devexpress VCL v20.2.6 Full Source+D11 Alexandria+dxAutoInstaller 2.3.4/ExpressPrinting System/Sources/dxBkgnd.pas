...
type
  ...
  TdxBackgroundChangeEvent = procedure(Sender: TObject;
    AChangeWhat: TdxBackgroundParams) of object;
  ...
  public
    ...
    procedure PaintEx(ACanvas: TdxPSReportRenderCustomCanvas; const R: TRect;
      APixelsNumerator, APixelsDenominator: Integer); virtual;
    ...
    property OnPaint: TCustomdxBackgroundPaintEvent read FOnPaint
      write FOnPaint;
    property OnPaintEx: TCustomdxBackgroundPaintExEvent read FOnPaintEx
      write FOnPaintEx;
  published
    ...
    property PictureMode: TdxPicturePaintMode read FPictureMode
      write SetPictureMode default ppmCenter;
  end;
...
begin
  ...
  if Result then
    with TdxBackground(ABaseObject) do
      Result := (Self.BkColor = BkColor) and (Self.Mode = Mode) and
        (Self.PictureMode = PictureMode) and dxAreBrushesEqual(Self.Brush,
        Brush) and dxAreGraphicsEqual(Self.Picture, Picture);
end;
...
begin
  ...
  case Mode of
      ...
      case Brush.Style of
        ...
        begin
          ...
          try
            ...
            ACanvas.DrawPicture(APattern, R, ppmTile, APixelsNumerator,
              APixelsDenominator);
          finally
            APattern.Free;
          end;
    ...
    bmPicture, bmBrushBitmap:
      ACanvas.DrawPicture(Picture, R, StretchDrawMode, APixelsNumerator,
        APixelsDenominator);
  end;
...
function TdxBackground.DoPaintEx(ACanvas: TdxPSReportRenderCustomCanvas;
  Rect: TRect; Sequence: TdxPaintSequence;
  PixelsNumerator, PixelsDenominator: Integer): Boolean;
begin
  Result := False;
  if Assigned(OnPaintEx) then
    OnPaintEx(Self, ACanvas, Rect, Sequence, PixelsNumerator,
      PixelsDenominator, Result);
  if not Result then
...
function TdxBackground.RepaintNeeded(AChangeWhats: TdxBackgroundParams)
  : Boolean;
begin
  if bpMode in AChangeWhats then
    ...
    case Mode of
      bmBrush:
        Result := (bpBrush in AChangeWhats) or
          ((bpBkColor in AChangeWhats) and (Brush.Style > bsClear));
      bmBrushBitmap:
