...
uses
  Types, Classes, Controls, Windows, Graphics, cxFontNameComboBox,
  cxColorComboBox,
...
type
  ...
  public
    ...
    function Properties: TcxCustomColorComboBoxProperties; reintroduce;
      overload;
  end;
...
{$IFDEF REGISTERCXSHELLCTRLS}
  ...
  public
    ...
    class function ControlClass: TControlClass; override;
    function Properties: TcxCustomShellComboBoxProperties; reintroduce;
      overload;
  end;
...
{$ENDIF}
  ...
  public
    ...
    class function ControlClass: TControlClass; override;
    function Properties: TcxCustomFontNameComboBoxProperties;
  ...
  public
    ...
    class function ControlClass: TControlClass; override;
    function Properties: TcxCustomCheckComboBoxProperties; reintroduce;
      overload;
  end;

  { TdxPScxCustomSpinButtonAsMetafileProducer }

  TdxPScxCustomSpinButtonAsMetafileProducer = class
...
{$IFDEF REGISTERCXSHELLCTRLS} dxPScxExtComCtrlsLnk, {$ENDIF}
  dxPScxMCListBoxLnk, cxCheckBox, dxPSRELnk, cxDrawTextUtils, cxGraphics,
  cxGeometry;
...
function TdxPScxCustomColorComboBoxProducer.Properties
  : TcxCustomColorComboBoxProperties;
begin
  Result := inherited Properties as TcxCustomColorComboBoxProperties;
end;

procedure TdxPScxCustomColorComboBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
  with TcxCustomColorComboBoxPropertiesAccess(Properties) do
  begin
    TdxReportCellColorBox(AnItem).ColorBoxAlignment := ColorBoxAlignmentMap
      [ColorBoxAlign];
    ...
    TdxReportCellColorBox(AnItem).ColorValue := TcxCustomColorComboBoxAccess
      (Control).ColorValue;
...
function TdxPScxCustomShellComboBoxProducer.Properties
  : TcxCustomShellComboBoxProperties;
begin
  Result := inherited Properties as TcxCustomShellComboBoxProperties;
end;

procedure TdxPScxCustomShellComboBoxProducer.GetImageLists
...
procedure TdxPScxCustomShellComboBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
var
  AText: string;
...
begin
  ...
  begin
    ...
    begin
      TdxReportCellImage(AnItem).ImageList := dxPSUtl.ShellSmallImages;
...
function TdxPScxCustomFontNameComboBoxProducer.Control
  : TcxCustomFontNameComboBox;
begin
  Result := inherited Control as TcxCustomFontNameComboBox;
end;

class function TdxPScxCustomFontNameComboBoxProducer.ControlClass
  : TControlClass;
begin
  Result := TcxCustomFontNameComboBox;
end;

function TdxPScxCustomFontNameComboBoxProducer.Properties
  : TcxCustomFontNameComboBoxProperties;
begin
...
begin
  if not IsFontSubstitutable then
    Result := TcxCustomFontNameComboBoxPropertiesAccess(Properties)
      .Items[TcxCustomFontNameComboBoxAccess(Control).ItemIndex]
  else
    Result := inherited GetFontName;
end;

procedure TdxPScxCustomFontNameComboBoxProducer.GetImageLists
  (AProc: TdxPSGetImageListProc);
begin
  inherited;
  AProc(FontImages);
...
procedure TdxPScxCustomFontNameComboBoxProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
const
    ...
    begin
      ...
      if ItemIndex > -1 then
        TdxReportCellImage(AnItem).ImageIndex :=
          ImageIndexes[cxftTTF in ItemTypes[ItemIndex]];
    end;
    TdxReportCellImage(AnItem).MakeSpaceForEmptyImage := ftiShowInCombo
      in ShowFontTypeIcon;
  end;
end;
...
begin
  Result := not(TcxCustomFontNameComboBoxPropertiesAccess(Properties)
    .UseOwnFont and (TcxCustomFontNameComboBoxAccess(Control).ItemIndex <> -1));
end;

function TdxPScxCustomFontNameComboBoxProducer.ItemClass
...
function TdxPScxCustomCheckComboBoxProducer.Properties
...
class function TdxPScxCustomSpinButtonAsMetafileProducer.BuddyClass
...
function TdxPScxCustomCheckGroupProducer.Properties
  : TcxCustomCheckGroupProperties;
begin
  Result := inherited Properties as TcxCustomCheckGroupProperties;
end;

function TdxPScxCustomCheckGroupProducer.CanProcessChild(AChildControl
  : TControl): Boolean;
begin
...
procedure TdxPScxCustomCheckGroupProducer.InitializeButton
...
begin
  ...
  begin
    CheckPos := LeftRightMap[TcxCustomCheckGroupPropertiesAccess(Properties)
      .ItemAlignment];
...
procedure TdxPScxCustomCheckGroupProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
begin
  inherited;
...
begin
  ...
  if not IsDesigning and (TcxCustomRichEditAccess(Control).Text <> '') then
    Result := dxPSContainerLnk.dxPSMakeFriendlyNameFromStrings
      (TcxCustomRichEditAccess(Control).Lines);
  if Result = '' then
...
begin
  Result := GetRichEditAsGraphic(TcxCustomRichEditAccess(Control)
    .InnerRich.Handle, ContentColor, GetMargins, TMetafile, Control.Width, -1,
    GetMaxHeight);
end;

procedure TdxPScxCustomRichEditProducer.InitializeItem
  (AnItem: TdxReportVisualItem);
var
  Graphic: TGraphic;
begin
...
const
  TextAlignXMap: array [TcxEditHorzAlignment] of TcxTextAlignX = (taLeft,
    taRight, taCenterX);
  TextAlignYMap: array [TcxEditVertAlignment] of TcxTextAlignY = (taTop,
    taBottom, taCenterY);
var
...
begin
  ...
  AItem.PreventAutoIndents := not Control.Style.TransparentBorder or
    (Control.Style.LookAndFeel.SkinPainter <> nil);

  if Properties.Angle <> 0 then
  begin
    cxScreenCanvas.Font := Font;
    try
      AItem.Indents.TopLeft := TcxLabelHelper.CalculatePositionOfRotatedText
        (cxScreenCanvas.Canvas, Control.Caption,
        cxRectInflate(cxRectSetNullOrigin(AItem.BoundsRect), -cxTextOffset),
        Properties);
...
begin
  Result := ReportLink.OptionsBehavior.LabelAutoHeight and
    (Properties.Angle = 0);
end;
