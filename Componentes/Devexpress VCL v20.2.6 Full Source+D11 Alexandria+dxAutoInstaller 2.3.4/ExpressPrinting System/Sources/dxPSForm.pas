...
function dxPSFormGetActualSectionName(AForm: TCustomForm;
  const ABaseSection: string = ''): string;
begin
  Result := dxValidatePath(ABaseSection) + sdxFormLayouts + '\' +
    DropT(AForm.ClassName);
end;
...
function dxPSFormLoadPositionFromIniFile(AForm: TCustomForm;
  ...
  function ReadSettingValue(const AValueName: string; ADefaultValue: Integer;
    var APositionChanged: Boolean): Integer;
  begin
  ...
  begin
    ...
    if not AIsFormSizeable then
      ...
      ABounds := cxRectSetSize(ABounds, ReadSettingValue(sdxWidth, AForm.Width,
        Result), ReadSettingValue(sdxHeight, AForm.Height, Result));
    AState := TWindowState(ReadSettingValue(sdxWindowState,
      Integer(wsNormal), Result));
  end;
  ...
  begin
    Result := ABounds;
    AScreenWidth := AIniFile.ReadInteger(ASectionName, sdxScreenWidth,
      Screen.Width);
    AScreenHeight := AIniFile.ReadInteger(ASectionName, sdxScreenHeight,
      Screen.Height);
...
begin
  ...
  begin
    ...
    begin
      ...
      AForm.BoundsRect := CheckBounds(cxRectScaleSize(AFormBounds,
        dxGetFormDPI(AForm), dxDefaultDPI));
    end;
...
begin
  ...
  AIniFile.WriteInteger(ASectionName, sdxWindowState,
    Integer(AForm.WindowState));
  ...
  begin
    R := cxRectScaleSize(AWindowPlacement.rcNormalPosition, dxDefaultDPI,
      dxGetFormDPI(AForm));
...
procedure TCustomdxPSForm.LoadFromIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  if dxPSEngine.SaveFormsPosition then
    dxPSFormLoadPositionFromIniFile(Self, AIniFile, ASectionName,
      IsFormSizeable);
end;

procedure TCustomdxPSForm.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  if dxPSEngine.SaveFormsPosition then
...
begin
  ...
  begin
    ...
    Params.WindowClass.Style := Params.WindowClass.Style or CS_VREDRAW or
      CS_HREDRAW;
  end;
...
begin
  Result := HandleAllocated and dxIsWindowStyleSet(Handle, WS_THICKFRAME);
  // and (dwoSizeable in Options);
end;
