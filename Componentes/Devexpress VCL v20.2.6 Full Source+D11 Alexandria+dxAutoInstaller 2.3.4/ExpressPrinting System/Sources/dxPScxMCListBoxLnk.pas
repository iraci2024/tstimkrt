...
uses
  ...
  dxCore, cxMCListBox, cxHeader, dxPSGlbl, dxPSCore, dxPSForm, dxPSBaseGridLnk,
  dxExtCtrls, cxDrawTextUtils,
  cxPC, cxControls, cxContainer, cxEdit, cxCheckBox, cxLabel,
...
type
  TcxMCListBoxPaintOption = (mclbpoBorder, mclbpoHorzLines, mclbpoVertLines,
    mclbpoColumnHeaders);
  ...
  published
    ...
    property OnCustomDrawItem: TcxMCListBoxCustomDrawItemEvent
...
const
  dxDefaultMCListBoxOptions = [Low(TcxMCListBoxPaintOption)
    .. High(TcxMCListBoxPaintOption)];
...
const
  SortOrderMap: array [TcxHeaderSortOrder] of TdxCellSortOrder = (csoNone,
    csoUp, csoDown);
...
function TcxMCListBoxReportLink.CanCalculateRowAutoHeight
  (ARow: Integer): Boolean;
begin
  Result := inherited CanCalculateRowAutoHeight(ARow) or
    cxMCListBox.MultiLines and (ARow > 0);
end;
...
function TcxMCListBoxReportLink.GetDataItemClass(ACol: Integer;
...
function TcxMCListBoxReportLink.GetCellImageList(ACol, ARow: Integer)
  : TCustomImageList;
begin
  if HasHeaderSections and IsFixedRow(ARow) then
...
begin
  ...
  else if HasHeaderSections then
    Result := Columns[ACol].Text
  else
    Result := '';
end;

function TcxMCListBoxReportLink.GetCellTextAlignX(ACol, ARow: Integer)
  : TcxTextAlignX;
begin
...
function TcxMCListBoxReportLink.GetColSortOrder(ACol: Integer)
  : TdxCellSortOrder;
begin
  if HasHeaderSections then
...
procedure TcxMCListBoxReportLink.SetOnCustomDrawItem
  (Value: TcxMCListBoxCustomDrawItemEvent);
begin
...
begin
  ...
  dxPSDrawGridPreview(ACanvas, R, ReportLink, False,
    ReportLink.AreColumnHeadersShown, ScaleFactor);
end;
...
dxPSRegisterReportLink(TcxMCListBoxReportLink, TcxMCListBox,
  TcxfmMCListBoxDesignWindow);

finalization

dxPSUnregisterReportLink(TcxMCListBoxReportLink, TcxMCListBox,
  TcxfmMCListBoxDesignWindow);
