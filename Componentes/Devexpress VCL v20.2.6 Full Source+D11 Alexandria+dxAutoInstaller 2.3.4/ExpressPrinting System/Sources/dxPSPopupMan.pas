...
type
  ...
  protected
    ...
    procedure InvokePopup(X, Y: Integer; AControl: TControl;
      APopupMenu: TComponent); virtual; abstract;
  ...
  public
    ...
    procedure ShowPopupAtMousePos(const AControl: TControl;
      const APopupMenu: TPopupMenu);
    ...
    property Builders[Index: Integer]: TdxPSPopupMenuBuilderClass
      read GetBuilder;
...
procedure dxPSPopupManKeyboardHook(ACode: Integer; wParam: wParam;
  lParam: lParam; var AHookResult: LRESULT);

  function IsProcessKey(AKey, AKeyData: Longint; AKeyPressed: Boolean): Boolean;
  begin
    Result := ((AKey = VK_F10) and (GetAsyncKeyState(VK_SHIFT) < 0)) or
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        dxPSPopupMenuController.TryShowPopup(Control,
...
procedure dxPSPopupManMouseHook(ACode: Integer; wParam: wParam; lParam: lParam;
  ...
  function FindControl(Wnd: HWND; const Pt: TPoint): TControl;
    ...
    begin
      ...
      begin
        ...
        if IsRegisteredControl(Result, Pt) then
          Exit;
      end;
      Result := nil;
    end;

    function FindInParents(AControl: TWinControl; const Pt: TPoint)
      : TWinControl;
    begin
      Result := AControl;
      while Result <> nil do
  ...
  begin
    ...
    begin
      ...
      if dxPSPopupMenuController.IndexOfControl(Result) > -1 then
        Exit;
...
var
  ...
  Control: TControl;
begin
  if (ACode >= 0) and ((wParam = WM_RBUTTONUP) or (wParam = WM_RBUTTONDOWN))
  then
  begin
      ...
      if Assigned(APopupMenuComponent) then
        try
          try
            ...
            APopupMenuBuilder.InvokePopup(APoint.X, APoint.Y, AControl,
              APopupMenuComponent);
          except
...
procedure TdxPSPopupMenuController.RegisterBuilder
  (ABuilder: TdxPSPopupMenuBuilderClass);
begin
  FBuilders.Add(ABuilder);
end;

procedure TdxPSPopupMenuController.UnregisterBuilder
  (ABuilder: TdxPSPopupMenuBuilderClass);
begin
...
function TdxPSPopupMenuController.TryShowPopup(AControl: TControl;
  Pt: TPoint): Boolean;
  ...
  begin
    ...
    if (Result <> nil) and (Pt.X >= 0) and not PtInRect(AControl.ClientRect,
      AControl.ScreenToClient(Pt)) then
      Result := nil;
  end;
...
begin
  ...
  if Result then
  begin
    dxPSUtl.Control_DoContextPopup(AControl,
      AControl.ScreenToClient(Pt), Result);
    if not Result then
    begin
      Result := True;
      if Pt.X < 0 then
...
function TdxPSPopupMenuController.GetBuilder(Index: Integer)
  : TdxPSPopupMenuBuilderClass;
begin
...
class function TAbstractdxPSPopupMenuBuilder.CanShowPopup(const APopupMenu
  : TPopupMenu): Boolean;
begin
  Result := True;
end;
...
procedure TdxStandardPSPopupMenuBuilder.InvokePopup(X, Y: Integer;
