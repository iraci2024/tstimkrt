...
uses
  ...
  dxCore, dxFlChrt, dxPSGraphicLnk, dxPSCore,
  ImgList, cxPC, cxControls, cxContainer, cxEdit, cxLabel, cxGraphics,
  cxTextEdit, cxMaskEdit, cxDropDownEdit, Menus,
  cxColorComboBox, cxCheckBox, cxLookAndFeels, cxLookAndFeelPainters,
  dxPSReportLinkDesignWindow, dxLayoutControlAdapters,
  dxLayoutLookAndFeels, dxLayoutContainer, cxClasses, cxButtons,
...
function dxGetFlowChartAsGraphic(const AData: PdxFlowChartGetAsGraphicData)
  : TGraphic;
...
begin
  Result := nil;
  if (AData = nil) or (AData^.FlowChart = nil) or (AData^.GraphicClass = nil)
  then
    Exit;
...
begin
  Result := nil;
  if (FlowChart = nil) or ((FlowChart.ObjectCount = 0) and
    (FlowChart.ConnectionCount = 0)) then
    Exit;
...
begin
  ...
  ACanvas.Font.Height := ScaleFactor.Apply(ReportLink.Font.Height,
  ...
  { Plan }
  R2 := Bounds(R.Left + ScaleFactor.Apply(2), ScaleFactor.Apply(4), R.Right,
    ScaleFactor.Apply(12));
  ACanvas.Font.Style := [fsBold];
  DrawText(ACanvas, R2, cxGetResourceString(@sdxPlan));
  R2 := Rect(R.Left + ScaleFactor.Apply(30), R2.Bottom + ScaleFactor.Apply(2),
    R.Right - ScaleFactor.Apply(30), R2.Bottom + ScaleFactor.Apply(3));
  ACanvas.Brush.Style := bsSolid;
  ...
  ACanvas.Brush.Style := bsClear;
  { Swimming-pool }
  R2 := Bounds(R.Left + ScaleFactor.Apply(2), ScaleFactor.Apply(27),
    R.Left + W div 2 - ScaleFactor.Apply(22), H div 4);
  ACanvas.RoundRect(R2.Left, R2.Top, R2.Right, R2.Bottom, ScaleFactor.Apply(10),
    ScaleFactor.Apply(10));
  ...
  try
    ...
    cxDrawImage(cxPaintCanvas, cxRectSetOrigin(R3,
      Point(cxRectCenter(R2).X - R3.Right div 2, R2.Top + ScaleFactor.Apply(10))
      ), nil, ilFlowChart, 0, Enabled, nil, ScaleFactor);
    cxDrawImage(cxPaintCanvas, cxRectSetOrigin(R3,
      Point(cxRectCenter(R2).X - 2 * R3.Right, R2.Top + R3.Bottom +
      ScaleFactor.Apply(20))), nil, ilFlowChart, 0, Enabled, nil, ScaleFactor);
    cxDrawImage(cxPaintCanvas, cxRectSetOrigin(R3,
      Point(cxRectCenter(R2).X + R3.Right, R2.Top + R3.Bottom +
      ScaleFactor.Apply(20))), nil, ilFlowChart, 0, Enabled, nil, ScaleFactor);
  finally
  ...
  R2 := Rect(R.Left + W div 2 - ScaleFactor.Apply(10), ScaleFactor.Apply(27),
    R.Left + W div 2 + ScaleFactor.Apply(8), R.Bottom - ScaleFactor.Apply(2));
  ACanvas.FillRect(R2);
  R2 := Rect(R.Left + ScaleFactor.Apply(2), R.Top + ScaleFactor.Apply(28) +
    H div 4, R.Right - ScaleFactor.Apply(2), R.Top + ScaleFactor.Apply(27) +
    H div 4 + ScaleFactor.Apply(20));
  ACanvas.FillRect(R2);
...
dxPSRegisterReportLink(TdxFlowChartReportLink, TdxCustomFlowChart,
  TdxFCReportLinkDesignWindow);

finalization

dxPSUnregisterReportLink(TdxFlowChartReportLink, TdxCustomFlowChart,
  TdxFCReportLinkDesignWindow);
