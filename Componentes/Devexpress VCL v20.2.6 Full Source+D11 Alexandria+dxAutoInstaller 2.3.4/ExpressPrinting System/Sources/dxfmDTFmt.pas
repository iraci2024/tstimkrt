...
uses
  ...
  ComCtrls, StdCtrls, ExtCtrls, dxCore,
  dxPSForm, cxControls, cxContainer, cxEdit, cxLabel, Menus,
  cxLookAndFeelPainters, cxButtons, cxCheckBox, cxListBox,
  cxGroupBox, cxGraphics, cxLookAndFeels, dxLayoutContainer,
...
type
  ...
  public
    ...
    property AutoUpdateDateTime: Boolean read FAutoUpdateDateTime
      write FAutoUpdateDateTime;
    property DateFormatIndex: Integer read FDateFormatIndex
      write FDateFormatIndex;
    property DateFormats: TStrings read FDateFormats write SetDateFormats;
    property SetDateTimeFormatAsDefault: Boolean
      read FSetDateTimeFormatAsDefault write FSetDateTimeFormatAsDefault;
    property TimeFormatIndex: Integer read FTimeFormatIndex
      write FTimeFormatIndex;
...
function dxShowDateTimeFormatDlg(var AFormatsDlgData
  : TdxDateTimeFormatDlgData): Boolean;
...
function dxShowDateTimeFormatDlg(var AFormatsDlgData
  ...
  begin
    Result := (AFormatsDlgData.DateFormats <> nil) and
      (AFormatsDlgData.TimeFormats <> nil) and
      (AFormatsDlgData.DateFormatIndex > -1) and
      (AFormatsDlgData.DateFormatIndex < AFormatsDlgData.DateFormats.Count) and
      (AFormatsDlgData.TimeFormatIndex > -1) and
      (AFormatsDlgData.TimeFormatIndex < AFormatsDlgData.TimeFormats.Count);
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      AFormatsDlgData.SetDateTimeFormatAsDefault :=
        Dialog.SetDateTimeFormatAsDefault;
    end;
  finally
    Dialog.Free;
  end;
...
begin
  btnDefault.Enabled := (lbxDateFormats.ItemIndex > -1) and
    (lbxTimeFormats.ItemIndex > -1);
...
begin
  ...
  lblAvailableDateFormats.Caption :=
    cxGetResourceString(@sdxDTFormatsAvailableDateFormats);
  lblAvailableTimeFormats.Caption :=
    cxGetResourceString(@sdxDTFormatsAvailableTimeFormats);
