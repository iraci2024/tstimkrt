...
uses
  ...
  StdCtrls, ExtCtrls, ComCtrls, ImgList, Menus, IniFiles, dxCore, dxPSCore,
  dxPSForm,
  cxLookAndFeelPainters, cxButtons, cxControls, cxContainer, cxListView, cxPC,
  cxEdit,
  cxGroupBox, cxLabel, cxGraphics, cxLookAndFeels, dxLayoutContainer,
...
uses
  Math, CommCtrl, dxPSGlbl, dxPSUtl, dxPSRes, dxPSPopupMan, dxPSfmLnkAdd,
  dxPSCompsProvider, dxDPIAwareUtils, cxGeometry;

procedure dxShowCPDesignerDlg(const AData: TdxCPDesignerDlgData);
var
  Dialog: TdxfmCPDesigner;
begin
  if AData.ComponentPrinter = nil then
    Exit;
...
begin
  ...
  try
    ...
    AData.Options := [adoShowDesignButton, adoShowDescriptionColumn,
      adoAllowMultiSelect, adoShowCaptionEdit, adoAllowSelectLinkClass];
    ...
    if dxShowAddComponentsDlg(AData) then
      ...
      begin
        ALink := ComponentPrinter.AddEmptyLink
          (TdxReportLinkClass(AData.ComponentsLinkClasses[I]));
...
procedure TdxfmCPDesigner.FormCloseQuery(Sender: TObject;
...
procedure TdxfmCPDesigner.LoadFromIniFile(AIniFile: TCustomIniFile;
...
procedure TdxfmCPDesigner.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
begin
  ...
  try
    ...
    AItem.Width := 2 * (lvItems.Width - dxGetSystemMetrics(SM_CXHSCROLL,
      ScaleFactor) - 2) div 3 - ScaleFactor.Apply(3);
    AItem.Caption := cxGetResourceString(@sdxItemName);

    if doShowDescription in FOptions then
    begin
      AItem := lvItems.Columns.Add;
      AItem.Width := (lvItems.Width - dxGetSystemMetrics(SM_CXHSCROLL,
        ScaleFactor) - 2) div 3 - ScaleFactor.Apply(3);
      AItem.Caption := cxGetResourceString(@sdxItemDescription);
    end;

    for I := 0 to MinIntValue([lvItems.Columns.Count - 1,
      Length(ColumnWidths) - 1]) do
