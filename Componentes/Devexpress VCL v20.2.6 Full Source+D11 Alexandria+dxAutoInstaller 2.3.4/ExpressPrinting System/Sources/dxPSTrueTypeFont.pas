...
type
  ...
  public
    ...
    //
    property CacheItem[Index: Integer]: TdxPSTTFCharCacheListItem
      read GetCacheItem;
  ...
  protected
    function GetSize: Integer; override;
    function CalculateEntriesInfo(ACharList: TdxPSTTFCharCacheList)
      : TdxPSTTFMapEntryInfoList;
    function CalculateSegmentCount(ACharList: TdxPSTTFCharCacheList): Integer;
    procedure ReadData(Stream: TdxPSTTFStream); override;
    procedure ReadUnicodeTable(Stream: TdxPSTTFStream;
  ...
  protected
    function GetGlyphInfoByGlyphIndex(AGlyphIndex: Integer;
      out AGlyphInfo: TdxPSTTFGlyphInfo): Boolean;
    function GetSize: Integer; override;
    procedure ReadData(Stream: TdxPSTTFStream); override;
    procedure WriteData(Stream: TdxPSTTFStream); override;
    //
  ...
  protected
    ...
    function FindSection(const ATag: TdxPSTTFSectionTag)
      : TdxPSTTFAbstractSection;
    function ReadSectionEntry(Stream: TdxPSTTFStream;
  ...
  public
    ...
    //
    property CharCodeByGlyphIndex[AGlyphIndex: Word]: Word
      read GetCharCodeByGlyphIndex;
    property CharWidthByCode[ACharCode: Word]: Integer read GetCharWidthByCode;
    property CharWidthByGlyphIndex[AIndex: Word]: Integer
      read GetCharWidthByGlyphIndex;
...
function dxPSGetFontCharsMap(AFont: TFont;
  out AMap: TdxPSTTFCMapSection): Boolean;
function dxPSGetFontData(AStream: TMemoryStream; AFont: TFont;
  ATable: Integer = 0): Boolean;
...
const
  ...
  sdxCMapUnicodeTableMissing =
    'The unicode CMap doesn''t exist in the font file';
...
function CompareCacheListItemByGlyphIndex(Item1,
  Item2: TdxPSTTFCharCacheListItem): Integer;
begin
...
function TdxPSTTFStream.CalculateCheckSum(AStartsPosition, ALength: Cardinal)
  : Cardinal;
var
...
procedure TdxPSTTFStream.Seek(AOffset: Integer;
  AOrigin: TSeekOrigin = soCurrent);
begin
...
function TdxPSTTFTableDirectory.FindSection(const ATag: TdxPSTTFSectionTag)
  : TdxPSTTFAbstractSection;
var
  I: Integer;
begin
...
function TdxPSTTFTableDirectory.GetSection(AIndex: Integer)
  : TdxPSTTFAbstractSection;
begin
...
function TdxPSTTFCMapSection.CalculateSegmentCount
  (ACharList: TdxPSTTFCharCacheList): Integer;
var
  I: Integer;
begin
  Result := 0;
  ...
  begin
    if ACharList.CacheItem[I + 1].CharCode <> ACharList.CacheItem[I].CharCode + 1
    then
      Inc(Result);
  end;
  Inc(Result, 2);
...
function TdxPSTTFCMapSection.CalculateEntriesInfo
  ...
  begin
    ...
    begin
      ...
      for I := 0 to ACharList.Count - 1 do
      begin
        if ACharList.CacheItem[I + 1].CharCode <> ACharList.CacheItem[I]
          .CharCode + 1 then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Inc(AStartGlyphIdOffset, (AEntryInfo.EntryEnd - AEntryInfo.EntryStarts
          + 1) * 2);
        AEntryInfo.Delta := 0;
      end;
...
begin
  Result := 28 + CalculateMapSize + Length(FEntryInfoList) *
    SizeOf(TdxPSTTFMapEntryInfo);
end;
...
procedure TdxPSTTFCMapSection.ReadUnicodeTable(Stream: TdxPSTTFStream;
  AUnicodeTableOffset: Integer);
begin
...
procedure TdxPSTTFCMapSection.WriteGlyphIndexes(Stream: TdxPSTTFStream;
  const AEntriesInfo: TdxPSTTFMapEntryInfoList);
var
  AEntryInfo: TdxPSTTFMapEntryInfo;
  I, ACode: Integer;
...
procedure TdxPSTTFCMapSection.WriteUnicodeTable(Stream: TdxPSTTFStream;
  const AEntriesInfo: TdxPSTTFMapEntryInfoList);
var
...
procedure TdxPSTTFLocaSection.CalculateOffsets
  (ACharList: TdxPSTTFCharCacheList);
  ...
  begin
    ...
    begin
      ...
      if AFlags and GLYPH_WE_HAVE_A_SCALE = 0 then
        AOffset := 0
      else if AFlags and GLYPH_WE_HAVE_A_TWO_BY_TWO <> 0 then
        AOffset := 32
      else if AFlags and GLYPH_WE_HAVE_AN_X_AND_Y_SCALE <> 0 then
        AOffset := 16
      else
        AOffset := 4;

      Stream.Seek(AOffset + OffsetMap[AFlags and
        GLYPH_ARG_1_AND_2_ARE_WORDS <> 0]);
    end;
  end;
end;
...
function TdxPSTTFGlyfSection.GetGlyphInfoByGlyphIndex(AGlyphIndex: Integer;
  out AGlyphInfo: TdxPSTTFGlyphInfo): Boolean;
var
  I: Integer;
begin
...
procedure TdxPSTTFGlyfSection.RemoveUnusedGlyphs
  (ACharList: TdxPSTTFCharCacheList);
...
function TdxPSTTFCharCacheList.GetCacheItem(AIndex: Integer)
  : TdxPSTTFCharCacheListItem;
begin
...
begin
  Result := 23 * SizeOf(Word) + 2 * SizeOf(Byte) + SizeOf(Panose) +
    SizeOf(FUnicodeRange) + SizeOf(FVendor);
  if FVersion >= 1 then
