...
{$ENDIF}
  ...
  Dialogs, dxPSCore, StdCtrls, ExtCtrls,
...
type
  ...
  protected
    ...
    property AtLeastOneTimeApplied: Boolean read FAtLeastOneTimeApplied
      write SetAtLeastOneTimeApplied;
...
uses
  dxPSUtl, dxCoreClasses, dxCore, dxPSRes, dxThemeConsts, dxThemeManager,
  dxUxTheme, Themes;
...
procedure TStandarddxReportLinkDesignWindow.FootnotesPropertiesClick
  (Sender: TObject);
begin
...
procedure TStandarddxReportLinkDesignWindow.RestoreOriginalClick
  (Sender: TObject);
begin
  BeginUpdateControls;
  try
...
procedure TStandarddxReportLinkDesignWindow.RestoreDefaultsClick
  (Sender: TObject);
begin
  BeginUpdateControls;
  try
...
procedure TStandarddxReportLinkDesignWindow.TitlePropertiesClick
  (Sender: TObject);
begin
...
begin
  ...
  libtnRestoreOriginal.Visible := TBasedxReportLinkAccess(ReportLink)
    .IsDesigning;
  libtnRestoreDefaults.Visible := TBasedxReportLinkAccess(ReportLink)
    .IsDesigning;
  libtnFootnoteProperties.Visible := libtnFootnoteProperties.Visible and
    ReportLink.CanChangeFootnotes and not TBasedxReportLinkAccess(ReportLink)
    .IsDesigning;
  libtnTitleProperties.Visible := libtnTitleProperties.Visible and
    ReportLink.CanChangeTitle and not TBasedxReportLinkAccess(ReportLink)
    .IsDesigning;
end;
...
begin
  ...
  btnFootnoteProperties.Caption :=
    cxGetResourceString(@sdxBtnFootnoteProperties);
end;
...
begin
  ...
  if btnRestoreOriginal <> nil then
    btnRestoreOriginal.Enabled := (ReportLink <> nil) and
      ReportLink.DataProviderPresent;
end;
...
procedure TStandarddxReportLinkDesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
var
...
begin
  ...
  begin
    ...
    cxStyleServices.DrawElement(ACanvas.Handle, Details,
      PreviewHost.Parent.ClientRect);
    Exit;
  end;
...
procedure TStandarddxReportLinkDesignWindow.SetAtLeastOneTimeApplied
  (Value: Boolean);
begin
