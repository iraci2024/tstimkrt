...
uses
  ...
  dxPrnPg, dxPSContainerLnk, cxDrawTextUtils, cxGeometry,
...
type
  ...
  protected
    ...
    property PageRenderInfos[Index: Integer]: TdxPSREPageRenderInfo
  ...
  published
    property OnlySelected: Boolean read FOnlySelected write SetOnlySelected
      default False;
    property RichEditVersion: TdxRichEditVersion read FRichEditVersion
      write SetRichEditVersion default dxDefaultRichEditVersion;
  end;
...
function GetRichEditAsGraphic(AWnd: THandle; ABkColor: TColor;
  const AMargins: TRect; AGraphicClass: TGraphicClass = nil; { TMetafile }
...
function GetTextLengthEx(AHandle: HWND;
  const ATextLengthEx: TGetTextLengthEx): Integer;
begin
  Result := SendMessage(AHandle, EM_GETTEXTLENGTHEX, WPARAM(@ATextLengthEx), 0);
end;

function GetRichEditTextLenght(AHandle: HWND;
  ARichEditVersion: Integer): Integer;
var
...
const
  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        FormatRange.rc.Right :=
          MulDiv(AWidth - (AMargins.Left + AMargins.Right),
          TwipsPerInch, APPI.X);
        FormatRange.rc.Bottom := TwipsPerInch;
        FormatRange.rcPage := FormatRange.rc;
        FormatRange.chrg.cpMax := -1;

        FormatRange.chrg.cpMin := SendMessage(AWnd, EM_FORMATRANGE, 0,
          LPARAM(@FormatRange));
        Inc(Result, FormatRange.rc.Bottom - FormatRange.rc.Top);
      until (FormatRange.chrg.cpMin >= CharCount) or
        (FormatRange.chrg.cpMin = -1);
      AMaxChars := -1;
    end
    else
    begin
      FormatRange.HDC := DC;
      FormatRange.hdcTarget := DC;
      FormatRange.rc.Right := MulDiv(AWidth - 2 * dxTextSpace,
        TwipsPerInch, APPI.X);
    ...
    Result := MulDiv(Result, APPI.Y, TwipsPerInch) + AMargins.Top +
      AMargins.Bottom;
  end;
  ...
  function GetRichEditAsMetafile(DC: HDC; const APPI: TPoint;
    AWidth, AHeight: Integer; AMaxChars: Integer): TMetafile;
  const
  ...
  begin
    ...
    try
      ...
      try
        ...
        FormatRange := PrepareFormatRange(MetaCanvas.Handle, APPI, AWidth,
          AHeight, AMaxChars);
  ...
  function GetRichEditAsBitmap(DC: HDC; const APPI: TPoint;
    AWidth, AHeight: Integer; AMaxChars: Integer): TBitmap;
  var
    FormatRange: TFormatRange;
  begin
    ...
    try
      ...
      FormatRange := PrepareFormatRange(Result.Canvas.Handle, APPI, AWidth,
        AHeight, AMaxChars);
    ...
    begin
      ...
      begin
        if W = -1 then
          W := Right - Left;
        if H = -1 then
          H := CalculateRichEditHeight(DC, PPI, W, MaxChars);
      end;
    end;
...
begin
  ...
  if not IsDesigning and (RichEdit_GetLines(Control).Count <> 0) then
    Result := dxPSContainerLnk.dxPSMakeFriendlyNameFromStrings
      (RichEdit_GetLines(Control));
  if Result = '' then
...
procedure TdxPSCustomRichEditProducer.InitializeItem
...
function TdxPSREReportLinkRenderInfo.GetPageRenderInfoClass
...
function TdxPSREReportLinkRenderInfo.LoMetricValueToInternalUnits
...
function TdxPSREReportLinkRenderInfo.GetPageRenderInfo(Index: Integer)
  : TdxPSREPageRenderInfo;
begin
...
function TdxPSREReportLinkRenderInfo.GetReportLink
  : TAbstractdxRichEditReportLink;
begin
...
begin
  VirtualPageCount := 0;
  repeat
    APageRenderInfo := CreatePageRenderInfo(VirtualPageCount)
      as TdxPSREPageRenderInfo;
    if VirtualPageCount = 0 then
      APageRenderInfo.FirstChar := FirstChar
    else
      APageRenderInfo.FirstChar := PageRenderInfos
        [VirtualPageCount - 1].LastChar;
    ...
    APageRenderInfo.LastChar :=
      Min(LastChar, SendMessage(REHandle, EM_FORMATRANGE, 0,
      LPARAM(@FFormatRange)));
    APageRenderInfo.ContentBounds := cxRectScale(FFormatRange.rc,
      PixelsDenominator, 1440);
    Inc(VirtualPageCount);
  until (APageRenderInfo.LastChar >= LastChar) or
    (APageRenderInfo.LastChar = -1);
end;
...
begin
  FPageTwipsRect.BottomRight := ScalePoint(PrinterPage.RealPageSizeLoMetric,
    1440, 254);
  FDetailsTwipsRect := cxRectScale(PrinterPage.PaintRectLoMetric, 1440, 254,
    1440, 254);
...
procedure TdxPSREReportRenderer.PrepareFont(AFont: TFont;
...
begin
  RenderInfo.FPageSize.X := MulDiv(RenderInfo.FPageSize.X, PixelsPerInch,
    UnitsPerInch);
  RenderInfo.FPageSize.Y := MulDiv(RenderInfo.FPageSize.Y, PixelsPerInch,
    UnitsPerInch);
...
begin
  ...
  try
    ...
    try
      ...
      SendMessage(REHandle, EM_FORMATRANGE, 0, 0);
      try
        ALastChar := SendMessage(REHandle, EM_FORMATRANGE, 1,
          LPARAM(@AFormatRange));
        ...
        begin
          ...
          if RenderingPageIndex + 1 < RenderInfo.VirtualPageCount then
            RenderInfo.PageRenderInfos[RenderingPageIndex + 1].FirstChar :=
              ALastChar;
    ...
    if (AMetafileSize.cx > APageSize.cx) or (AMetafileSize.cy > APageSize.cy)
    then
    begin
      Canvas.SaveClipRgn;
...
begin
  RenderInfo.FPageSize.X := MulDiv(RenderInfo.FPageSize.X, UnitsPerInch,
    PixelsPerInch);
  RenderInfo.FPageSize.Y := MulDiv(RenderInfo.FPageSize.Y, UnitsPerInch,
    PixelsPerInch);
  RenderInfo.FootnotesBounds := cxRectScale(RenderInfo.FootnotesBounds,
...
procedure TAbstractdxRichEditReportLink.ConstructReport
  ...
  if ADefaultDrawText or ADefaultDrawBackground then
    ...
    begin
      if Assigned(OnCustomDrawPageHeader) then
        OnCustomDrawPageHeader(Self, ACanvas, APageIndex, R, APixelsNumerator,
          PixelsDenominator, ADefaultDrawText, ADefaultDrawBackground)
    end
    else if Assigned(OnCustomDrawPageFooter) then
      OnCustomDrawPageFooter(Self, ACanvas, APageIndex, R, APixelsNumerator,
        PixelsDenominator, ADefaultDrawText, ADefaultDrawBackground);
end;
...
begin
  APixelsNumerator := Renderer.PixelsPerInch;
  DoParentCustomDrawReportTitle(ACanvas, R, ATextAlignX, ATextAlignY, AColor,
    AFont, ADone, APixelsNumerator);
...
function TAbstractdxRichEditReportLink.GetRendererClass
  : TdxPSReportRendererClass;
begin
  Result := TdxPSREReportRenderer;
end;

function TAbstractdxRichEditReportLink.GetRenderInfo
  : TdxPSREReportLinkRenderInfo;
begin
  Result := TdxPSREReportLinkRenderInfo(inherited RenderInfo);
end;

function TAbstractdxRichEditReportLink.GetRenderInfoClass
...
function TAbstractdxRichEditReportLink.TryLoadRichEditDLL
...
begin
  Result := (REHandle <> 0) and
    (GetRichEditTextLenght(REHandle, RichEditVersion) > 0);
end;
...
procedure TAbstractdxRichEditReportLink.SetRichEditVersion
  (Value: TdxRichEditVersion);
begin
...
procedure TAbstractdxRichEditReportLink.GetCharRange(var ASelStart,
  ASelLength: Integer);
var
