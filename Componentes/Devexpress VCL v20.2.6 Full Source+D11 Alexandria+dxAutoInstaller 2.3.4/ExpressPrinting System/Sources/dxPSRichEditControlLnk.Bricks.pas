...
{$ENDIF}
  ...
  cxGraphics, cxControls, cxClasses, dxCore,
  dxMessages, dxGDIPlusAPI, dxGDIPlusClasses, cxLookAndFeelPainters,
  dxCoreGraphics, dxPSCore, dxPSReportRenderCanvas,
  dxPSEdgePatterns,
...
type
  ...
  public
    property AbsoluteLocation: TdxPointF read FAbsoluteLocation
      write FAbsoluteLocation;
  end;
  ...
  public
    ...
    procedure SetText(const AText: string; AGlyphs: PWord;
      AGlyphWidths: PInteger; AGlyphCount: Integer); overload;
  ...
  public
    constructor Create(AParent: TdxReportCell); override;
    //
    class function CreateStrikeoutBrick(AParent: TdxReportCell;
      ALineType: TdxStrikeoutType): TdxLineBrick;
    class function CreateUnderlineBrick(AParent: TdxReportCell;
      ALineType: TdxUnderlineType; AHorizontal: Boolean): TdxLineBrick;
    //
  ...
  public
    constructor Create(AParent: TdxReportCell; const APattern: array of Word;
      AThickness: Integer; AHorizontal: Boolean); reintroduce;
    procedure DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
      AStage: TdxPSRenderStages); override;
  end;
  ...
  protected
    function GetPatternSize: Integer; override;
  public
    constructor Create(AParent: TdxReportCell; ALineCount: Integer);
      reintroduce;
    procedure DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
      AStage: TdxPSRenderStages); override;
    //
  ...
  public
    constructor Create(AParent: TdxReportCell; ALineWidth: Integer;
      AHorizontal: Boolean); reintroduce;
    procedure DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
      AStage: TdxPSRenderStages); override;
    //
...
procedure TdxOfficeRectBrick.DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
  AStage: TdxPSRenderStages);
begin
...
procedure TdxOfficeImageBrick.DrawContent
...
procedure TdxOfficeTextBrick.DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
  AStage: TdxPSRenderStages);
begin
  inherited DrawContent(ACanvas, AStage);
...
begin
  ACanvas.ExtTextOut(BoundsRect, Font, Text, @FGlyphs[0], Length(FGlyphs),
    @FGlyphWidths[0]);
end;

procedure TdxOfficeTextBrick.SetText(const AText: string; AGlyphs: PWord;
  AGlyphWidths: PInteger; AGlyphCount: Integer);
begin
  FText := AText;
...
procedure TdxOfficeTextBrick.ConvertCoords(APixelsNumerator, APixelsDenominator
  : Integer);
var
  I: Integer;
begin
  ...
  for I := 0 to Length(FGlyphWidths) - 1 do
    FGlyphWidths[I] := MulDiv(FGlyphWidths[I], APixelsNumerator,
      APixelsDenominator);
end;
...
class function TdxLineBrick.CreateStrikeoutBrick(AParent: TdxReportCell;
  ALineType: TdxStrikeoutType): TdxLineBrick;
begin
  case ALineType of
...
class function TdxLineBrick.CreateUnderlineBrick(AParent: TdxReportCell;
  ALineType: TdxUnderlineType; AHorizontal: Boolean): TdxLineBrick;
begin
  case ALineType of
    ...
    TdxUnderlineType.DashDotted:
      Result := TdxPatternLineBrick.Create(AParent, [8, 2, 3, 2], 1,
        AHorizontal);
    TdxUnderlineType.ThickDashDotted:
      Result := TdxPatternLineBrick.Create(AParent, [8, 2, 3, 2], 2,
        AHorizontal);
    TdxUnderlineType.DashDotDotted:
      Result := TdxPatternLineBrick.Create(AParent, [6, 2, 2, 2, 2, 2], 1,
        AHorizontal);
    TdxUnderlineType.ThickDashDotDotted:
      Result := TdxPatternLineBrick.Create(AParent, [6, 2, 2, 2, 2, 2], 2,
        AHorizontal);
...
constructor TdxSolidLineBrick.Create(AParent: TdxReportCell;
  ALineCount: Integer);
begin
  inherited Create(AParent);
...
procedure TdxSolidLineBrick.DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
  AStage: TdxPSRenderStages);
var
  ALineRect: TRect;
...
constructor TdxPatternLineBrick.Create(AParent: TdxReportCell;
  const APattern: array of Word; AThickness: Integer; AHorizontal: Boolean);
var
  I: Integer;
begin
  inherited Create(AParent);
...
procedure TdxPatternLineBrick.DrawContent
  (ACanvas: TdxPSReportRenderCustomCanvas; AStage: TdxPSRenderStages);
var
  AIndex: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      ADrawRect.Init(ARect.Left, ARect.Top, Min(ARect.Left + ASize,
  ...
  begin
    ...
    begin
      ASize := MulDiv(FPattern[AIndex], PixelsNumerator, PatternDPI);
      ADrawRect.Init(ARect.Left, ARect.Top, ARect.Right,
        Min(ARect.Top + ASize, ARect.Bottom));
      ACanvas.FillRect(ADrawRect, Color);
      Inc(ASize, MulDiv(FPattern[AIndex + 1], PixelsNumerator, PatternDPI));
      Inc(ARect.Top, ASize);
      AIndex := (AIndex + 2) mod Length(FPattern);
...
constructor TdxWavyLineBrick.Create(AParent: TdxReportCell; ALineWidth: Integer;
  AHorizontal: Boolean);
begin
  inherited Create(AParent);
...
procedure TdxWavyLineBrick.DrawContent(ACanvas: TdxPSReportRenderCustomCanvas;
  AStage: TdxPSRenderStages);
var
  APoints: TPoints;
begin
...
procedure TdxWavyLineBrick.CalculatePoints(const R: TRect;
  out APoints: TPoints);
var
  ACount: Integer;
  ASum: Integer;
...
procedure TdxDoubleWavyLineBrick.DrawContent
  (ACanvas: TdxPSReportRenderCustomCanvas; AStage: TdxPSRenderStages);
var
  APoints: TPoints;
  ARect: TRect;
