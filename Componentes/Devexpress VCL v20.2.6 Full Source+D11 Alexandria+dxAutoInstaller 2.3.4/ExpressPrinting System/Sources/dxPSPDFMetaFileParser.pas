...
{$ENDIF}
  ...
  dxGDIPlusClasses,
  dxPSCore, dxPSPDFExportCore, dxPSPDFExport, dxPSGlbl, dxPSFillPatterns,
...
type
  ...
  public
    constructor Create(AHandle: THandle; ASourceObject: TObject;
      AColor: TColor);
  ...
  public
    function CreateBrush(const ABrushInfo:
      {$IFDEF DELPHI16}TLogBrush32{$ELSE}TLogBrush{$ENDIF}): HBRUSH;
    function CreateFont(const AFontInfo: TExtLogFontW): HFONT;
    function CreatePen(const APenInfo: TLogPen): HPEN;
    function CreatePenEx(const APenInfo:
      {$IFDEF DELPHI16}TExtLogPen32{$ELSE}TExtLogPen{$ENDIF}): HPEN;
    function FindObject(AHandle: THandle;
      var AObject: TdxPSPDFMetaFileGdiObjectItem): Boolean;
    //
  ...
  strict private
    ...
    function ConvertTextOrigin(const P: TPoint; const ABox: TRect;
      ATextWidth, ATextHeight, AAlignMode: Integer): TPoint;
    function GetNoClip: Boolean;
  public
    constructor Create;
    destructor Destroy; override;
    function Concatenate(AInfo: TdxPSPDFMetaFileTextInfo): Boolean;
    procedure Initialize(const R: PEMRExtTextOut; AIsUnicode: Boolean;
      AFont: TFont; ATextAlign: Integer);
    //
  ...
  protected
    ...
    function ExtractBitmap(ARecordAddress: Pointer; const ARect: TRect;
      ABitmapInfoOffset, ABitmapBitsOffset, ABitmapBitsSize,
      AColorUsage: Cardinal): TcxBitmap;
    procedure DrawBitmap(const ADest, ASource: TRect; ARecordAddress: Pointer;
      ABitmapInfoOffset, ABitmapBitsOffset, ABitmapBitsSize,
      AColorUsage: Cardinal);
    ...
    function ConvertPoints(P: PByte; ACount: Integer;
      AIsSmallPoints: Boolean): TPoints;
    ...
    procedure EmfCreateBrush(const ARecord: PEMRCreateBrushIndirect;
      ATable: PHandleTable);
    procedure EmfCreateFont(const ARecord: PEMRExtCreateFontIndirect;
      ATable: PHandleTable);
    procedure EmfCreatePen(const ARecord: PEMRCreatePen; ATable: PHandleTable);
    procedure EmfCreatePenEx(const ARecord: PEMRExtCreatePen;
      ATable: PHandleTable);
    procedure EmfDeleteObject(const ARecord: PEMRDeleteObject;
      ATable: PHandleTable);
    ...
    procedure EmfPolyBezier(const ARecord: PEMRPolyBezier;
      ASmallPoints: Boolean);
    procedure EmfPolyBezierTo(const ARecord: PEMRPolyBezierTo;
      ASmallPoints: Boolean);
    ...
    procedure EmfPolyPolygon(const APolygon: PEMRPolyPolygon;
      ASmallPoints: Boolean);
    procedure EmfPolyPolyline(const APolyline: PEMRPolyPolyline;
      ASmallPoints: Boolean);
    ...
    procedure EmfSelectObject(const ARecord: PEMRSelectObject;
      ATable: PHandleTable);
    procedure EmfSetPixel(const ARecord: TEMRSetPixelV);
    procedure EmfSetTextAlign(const ARecord: PEMRSetTextAlign;
      ATable: PHandleTable);
  ...
  public
    ...
    destructor Destroy; override;
    procedure Render(AMetaFile: TMetafile; R: TRect;
      AScaleFactor: Single); virtual;
    //
...
procedure dxPDFExportMetaFile(APDFCanvas: TdxPSPDFReportRenderCanvas;
  AMetaFile: TMetafile; const R: TRect);
...
const
  ROP_NOOP = $AA0029;
  PointSizeMap: array [Boolean] of Cardinal = (SizeOf(TPoint),
    SizeOf(TSmallPoint));
...
constructor TdxPSPDFMetaFileExportProvider.Create
  (APDFCanvas: TdxPSPDFReportRenderCanvas);
begin
  inherited Create;
...
procedure TdxPSPDFMetaFileExportProvider.Render(AMetaFile: TMetafile; R: TRect;
  AScaleFactor: Single);
begin
  FreeAndNil(FBaseClipRegion);
...
function TdxPSPDFMetaFileExportProvider.ConvertPoints(P: PByte; ACount: Integer;
  AIsSmallPoints: Boolean): TPoints;
var
  I: Integer;
begin
...
function TdxPSPDFMetaFileExportProvider.ConvertValue(AValue: Integer;
  const AMapScale: Double): Integer;
var
  ATempValue: Double;
begin
  ATempValue := AValue;
...
function TdxPSPDFMetaFileExportProvider.CreateRegion(ARgnData: PRgnData)
  : TcxRegion;
var
  ARgnRect: PRect;
  I: Integer;
...
begin
  ...
  PDFCanvas.WindowOrg := cxPointOffset(cxPointOffset(State.WindowOrg,
    State.ViewPortOrg, False), BaseWindowOrg);
end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfAbortPath(const ARecord
  : PEMRAbortPath);
begin
  if FPath <> nil then
    FPath.Reset;
...
procedure TdxPSPDFMetaFileExportProvider.EmfAlphaBlend(const ARecord
  : PEMRAlphaBlend);
var
...
begin
  ...
  begin
    ASourceBitmap := ExtractBitmap(ARecord, Bounds(ARecord^.xSrc, ARecord^.ySrc,
      ARecord^.cxSrc, ARecord^.cySrc), ARecord^.offBmiSrc, ARecord^.offBitsSrc,
      ARecord^.cbBitsSrc, ARecord^.iUsageSrc);
    try
      ...
      try
        ...
        if cxGetBitmapPixelFormat(ASourceBitmap) = 32 then
          cxAlphaBlend(ABitmap, ASourceBitmap, ABitmap.ClientRect,
            ASourceBitmap.ClientRect)
        else
        begin
          ASourceBitmap.Transparent := True;
          ABitmap.Canvas.Draw(0, 0, ASourceBitmap);
        end;
        DrawPicture(ABitmap, Bounds(ARecord^.xDest, ARecord^.yDest,
          ARecord^.cxDest, ARecord^.cyDest));
      finally
        ABitmap.Free;
      end;
    finally
...
procedure TdxPSPDFMetaFileExportProvider.EmfBeginPath(const ARecord
  : PEMRBeginPath);
begin
  Inc(FPathLevel);
...
begin
  ...
  if CheckROP(ARecord^.dwRop) then
  begin
    R := cxRectAdjust(Bounds(ARecord^.xDest, ARecord^.yDest, ARecord^.cxDest,
      ARecord^.cyDest));
    ...
    begin
      ...
      DrawBitmap(R, Bounds(ARecord^.xSrc, ARecord^.ySrc, cxRectWidth(R),
        cxRectHeight(R)), ARecord, ARecord^.offBmiSrc, ARecord^.offBitsSrc,
        ARecord^.cbBitsSrc, ARecord^.iUsageSrc);
  end;
end;

procedure TdxPSPDFMetaFileExportProvider.EmfCloseFigure(const ARecord
  : PEMRCloseFigure);
begin
  if FPath <> nil then
  begin
...
procedure TdxPSPDFMetaFileExportProvider.EmfCreateBrush(const ARecord
  : PEMRCreateBrushIndirect; ATable: PHandleTable);
begin
  ATable^.objectHandle[ARecord^.ihBrush] := GdiObjectList.CreateBrush
    (ARecord^.lb);
end;

procedure TdxPSPDFMetaFileExportProvider.EmfCreateFont(const ARecord
  : PEMRExtCreateFontIndirect; ATable: PHandleTable);
begin
  ATable^.objectHandle[ARecord^.ihFont] :=
    GdiObjectList.CreateFont(ARecord^.elfw);
end;

procedure TdxPSPDFMetaFileExportProvider.EmfCreatePen(const ARecord
  : PEMRCreatePen; ATable: PHandleTable);
begin
  ATable^.objectHandle[ARecord^.ihPen] :=
    GdiObjectList.CreatePen(ARecord^.lopn);
end;

procedure TdxPSPDFMetaFileExportProvider.EmfSetTextAlign(const ARecord
  : PEMRSetTextAlign; ATable: PHandleTable);
begin
  State.TextAlign := ARecord.iMode;
end;

procedure TdxPSPDFMetaFileExportProvider.EmfCreatePenEx(const ARecord
  : PEMRExtCreatePen; ATable: PHandleTable);
begin
  ATable^.objectHandle[ARecord^.ihPen] := GdiObjectList.CreatePenEx
    (ARecord^.elp);
end;

procedure TdxPSPDFMetaFileExportProvider.EmfDeleteObject(const ARecord
  : PEMRDeleteObject; ATable: PHandleTable);
var
  AObject: TdxPSPDFMetaFileGdiObjectItem;
begin
  if GdiObjectList.FindObject(ATable^.objectHandle[ARecord^.ihObject], AObject)
  then
    GdiObjectList.Remove(AObject);
end;

procedure TdxPSPDFMetaFileExportProvider.EmfExtTextOut(const ARecord
  : PEMRExtTextOut; AIsUnicode: Boolean);
var
...
procedure TdxPSPDFMetaFileExportProvider.EmfFillPath(const ARecord
  : PEMRFillPath);
var
  ARegion: TcxRegion;
  ARegionData: PRgnData;
...
begin
  if FPath <> nil then
  begin
    ARegion := TcxRegion.Create(FPath.GetGdiRegionHandle(dxGPPaintCanvas));
    try
      if cxGetRegionData(ARegion.Handle, ARegionData) then
        try
          BeginOutput;
          try
            PDFCanvas.PreparePageContent;
            try
              ARegionRect := PRect(@ARegionData^.Buffer[0]);
              for I := 0 to ARegionData^.rdh.nCount - 1 do
              begin
                PageContent.WriteRectangle(ARegionRect^);
                Inc(ARegionRect);
              end;
              PageContent.FillPolygon(clNone, BackgroundColor,
                State.PolyFillMode);
            finally
...
procedure TdxPSPDFMetaFileExportProvider.EmfFillRgn(const ARecord: TEMRFillRgn;
  ATable: PHandleTable);
var
  AObject: TdxPSPDFMetaFileGdiObjectItem;
  ARegion: TcxRegion;
begin
  if IsInPath then
    Exit;
  if GdiObjectList.FindObject(ATable^.objectHandle[ARecord.ihBrush], AObject)
  then
  begin
    ...
    try
      BeginOutput;
      try
        PDFCanvas.FillRegion(ARegion.Handle, AObject.Color, clNone,
...
begin
  ...
  if IsInPath then
  begin
    FPath.AddLine(Min(ATargetPoint.X, FMoveToPoint.X),
      Min(ATargetPoint.Y, FMoveToPoint.Y), Max(ATargetPoint.X, FMoveToPoint.X),
      Max(ATargetPoint.Y, FMoveToPoint.Y));
  end
  else
  begin
    BeginOutput;
...
procedure TdxPSPDFMetaFileExportProvider.EmfMaskBlt(const ARecord: PEMRMaskBlt);
  ...
  var
    ...
    AOffset: TPoint;
  begin
    AMaskRegion := ConvertRegion(cxCreateRegionFromBitmap(AMask,
      ATransparentColor));
    if AMaskRegion <> nil then
      try
        AOffset := ConvertPoint(Point(ARecord^.xDest, ARecord^.yDest));
        AOffset := cxPointOffset(AOffset, cxPointOffset(State.WindowOrg,
          BaseWindowOrg), False);
        AMaskRegion.Offset(AOffset);
        PDFCanvas.ClipRegion.Combine(AMaskRegion, roIntersect,
          GetActualWorldTransform);
      finally
        AMaskRegion.Free;
      end;
  end;
  ...
  begin
    ...
    ABitmap := ExtractBitmap(ARecord, Bounds(0, 0, ARecord^.cxDest,
      ARecord^.cyDest), ARecord^.offBmiSrc, ARecord^.offBitsSrc,
      ARecord^.cbBitsSrc, ARecord^.iUsageSrc);
    try
...
begin
  ...
  begin
    ...
    try
      ARect := Bounds(ARecord^.xDest, ARecord^.yDest, ARecord^.cxDest,
        ARecord^.cyDest);
...
procedure TdxPSPDFMetaFileExportProvider.EmfPaintRgn(const ARecord
  : EMRPaintRgn);
var
  ARegion: TcxRegion;
begin
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolyline(const P: PEMRPolyline;
  ASmallPoints: Boolean);
var
  APoints: TPoints;
begin
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolyBezier(const ARecord
  : PEMRPolyBezier; ASmallPoints: Boolean);
var
  APoints: TPoints;
  AIndex: Integer;
begin
  ...
  begin
    ...
    begin
      FPath.AddBezier(dxPointF(APoints[AIndex]), dxPointF(APoints[AIndex + 1]),
        dxPointF(APoints[AIndex + 2]), dxPointF(APoints[AIndex + 3]));
      Inc(AIndex, 4);
    end;
  end
  // TODO:
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolyBezierTo(const ARecord
  : PEMRPolyBezierTo; ASmallPoints: Boolean);
var
  APoints: TPoints;
  AIndex: Integer;
begin
  ...
  begin
    ...
    begin
      FPath.AddBezier(dxPointF(FMoveToPoint), dxPointF(APoints[AIndex]),
        dxPointF(APoints[AIndex + 1]), dxPointF(APoints[AIndex + 2]));
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolyPolygon(const APolygon
  : PEMRPolyPolygon; ASmallPoints: Boolean);
var
  ACount: Integer;
  ACursor: PByte;
  I: Integer;
  ...
  begin
    ...
    begin
      if APolygon^.aPolyCounts[I] > 1 then
        FPath.AddPolygon(ConvertPoints(ACursor, APolygon^.aPolyCounts[I],
          ASmallPoints));
  ...
  begin
    ...
    try
      ...
      try
        ...
        begin
          if APolygon^.aPolyCounts[I] > 1 then
            PageContent.WritePoints(ConvertPoints(ACursor,
              APolygon^.aPolyCounts[I], ASmallPoints));
          Inc(ACursor, APolygon^.aPolyCounts[I] * PointSizeMap[ASmallPoints]);
        end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolyPolyline(const APolyline
  : PEMRPolyPolyline; ASmallPoints: Boolean);
var
  ACount: Integer;
  ACursor: PByte;
  I: Integer;
begin
  ...
  begin
    for I := 0 to ACount - 1 do
    begin
      FPath.AddPolyline(ConvertPoints(ACursor, APolyline^.aPolyCounts[I],
        ASmallPoints));
  ...
  begin
    ...
    try
      ...
      try
        for I := 0 to ACount - 1 do
        begin
          PageContent.Polyline(ConvertPoints(ACursor, APolyline^.aPolyCounts[I],
            ASmallPoints), PenWidth, Pen.Color);
          Inc(ACursor, APolyline^.aPolyCounts[I] * PointSizeMap[ASmallPoints]);
        end;
      finally
        PDFCanvas.UnpreparePageContent;
...
procedure TdxPSPDFMetaFileExportProvider.EmfPie(const ARecord: EMRPie;
  ATable: PHandleTable);
begin
  if IsInPath then
    // FPath.AddPie();
  ...
  begin
    BeginOutput;
    try
      PDFCanvas.Pie(ConvertRect(ARecord.rclBox), ConvertPoint(ARecord.ptlStart),
        ConvertPoint(ARecord.ptlEnd), BackgroundColor);
    finally
      EndOutput;
    end;
  end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfPolygon(const APolygon
  : PEMRPolyline; ASmallPoints: Boolean);
var
  APoints: TPoints;
begin
  ...
  begin
    BeginOutput;
    try
      PDFCanvas.Polygon(APoints, Pen.Color, BackgroundColor, PenWidth,
        State.PolyFillMode);
    finally
      EndOutput;
    end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfRoundRect(R: TRect;
  ACorners: TSize);
begin
  R := ConvertRect(R);
    ...
    try
      if HasBackground then
        PDFCanvas.FillRoundRect(R, ACorners.cx, ACorners.cy, BackgroundColor,
          State.TextColor, nil, Brush);
      PDFCanvas.DrawRoundFrame(R, ACorners.cx, ACorners.cy, Pen.Color,
        PenWidth);
    finally
      EndOutput;
    end;
...
procedure TdxPSPDFMetaFileExportProvider.EmfSelectClipPath
  (const ARecord: PEMRSelectClipPath);
var
  ARegion: TcxRegion;
begin
  if FPath <> nil then
...
procedure TdxPSPDFMetaFileExportProvider.EmfSelectObject(const ARecord
  : PEMRSelectObject; ATable: PHandleTable);
var
  AObject: TdxPSPDFMetaFileGdiObjectItem;
begin
  if ARecord^.ihObject and $80000000 <> 0 then
    SelectStockObject(ARecord^.ihObject and $7FFFFFFF)
  else if GdiObjectList.FindObject(ATable^.objectHandle[ARecord^.ihObject],
    AObject) then
  begin
...
procedure TdxPSPDFMetaFileExportProvider.EmfSetPixel(const ARecord
  : TEMRSetPixelV);
var
  R: TRect;
begin
...
procedure TdxPSPDFMetaFileExportProvider.EmfSetWorldTransform
  (const AInfo: PEMRSetWorldTransform);
begin
  State.Transform := AInfo^.xform;
end;

procedure TdxPSPDFMetaFileExportProvider.EmfStretctBlt(const ARecord
  : PEMRStretchBlt);
begin
  if not IsInPath and (ARecord^.cbBitsSrc <> 0) and CheckROP(ARecord^.dwRop)
  then
  begin
    DrawBitmap(Bounds(ARecord^.xDest, ARecord^.yDest, ARecord^.cxDest,
      ARecord^.cyDest), Bounds(ARecord^.xSrc, ARecord^.ySrc, ARecord^.cxSrc,
      ARecord^.cySrc), ARecord, ARecord^.offBmiSrc, ARecord^.offBitsSrc,
      ARecord^.cbBitsSrc, ARecord^.iUsageSrc);
  end;
end;

procedure TdxPSPDFMetaFileExportProvider.EmfStretctDIBits
  (const ARecord: PEMRStretchDIBits);
begin
  if not IsInPath and (ARecord^.cbBitsSrc <> 0) and CheckROP(ARecord^.dwRop)
  then
  begin
...
function TdxPSPDFMetaFileExportProvider.ExtractBitmap(ARecordAddress: Pointer;
  const ARect: TRect; ABitmapInfoOffset, ABitmapBitsOffset, ABitmapBitsSize,
  AColorUsage: Cardinal): TcxBitmap;
...
begin
  ...
  AHandle := CreateDIBSection(cxScreenCanvas.Handle, AInfo^, DIB_RGB_COLORS,
    ABits, 0, 0);
  try
    cxCopyData(Pointer(TdxNativeUInt(ARecordAddress) + ABitmapBitsOffset),
      ABits, ABitmapBitsSize);
    Result := TcxBitmap.CreateSize(ARect, GetPixelFormat(AInfo));
    StretchDIBits(Result.Canvas.Handle, 0, 0, Result.Width, Result.Height,
      ARect.Left, ARect.Top, cxRectWidth(ARect), cxRectHeight(ARect), ABits,
      AInfo^, AColorUsage, SRCCOPY);
  finally
    DeleteObject(AHandle);
  end;
end;

procedure TdxPSPDFMetaFileExportProvider.TextOut
  (AInfo: TdxPSPDFMetaFileTextInfo);
var
  ATempFont: TFont;
  AText: string;
...
begin
  ...
  try
    ...
    if dxAreFontsEqual(PDFCanvas.Font, AInfo.Font) then
      PDFCanvas.DrawTextLine(AText, ATextOrigin, ATextRect, ATextWidth,
        State.TextColor, 0, 0, AInfo.NoClip)
    else
    begin
...
procedure TdxPSPDFMetaFileExportProvider.DrawBitmap(const ADest, ASource: TRect;
  ARecordAddress: Pointer; ABitmapInfoOffset, ABitmapBitsOffset,
  ABitmapBitsSize, AColorUsage: Cardinal);
var
  ABitmap: TcxBitmap;
begin
  ABitmap := ExtractBitmap(ARecordAddress, ASource, ABitmapInfoOffset,
    ABitmapBitsOffset, ABitmapBitsSize, AColorUsage);
  try
...
procedure TdxPSPDFMetaFileExportProvider.DrawPicture(APicture: TGraphic;
  const ARect: TRect);
begin
  if IsInPath then
    Exit;
  ...
  BeginOutput;
  try
    PDFCanvas.DrawPicture(APicture, ConvertRect(ARect), ppmStretch,
...
procedure TdxPSPDFMetaFileExportProvider.ProcessMetaFileObject
  (ATable: PHandleTable; AObjectsInTable: Integer;
  const ARecord: PEnhMetaRecord);
begin
  ...
  case ARecord^.iType of
    ...
    EMR_ROUNDRECT:
      EmfRoundRect(PEMRRoundRect(ARecord)^.rclBox, PEMRRoundRect(ARecord)
        ^.szlCorner);
    EMR_POLYGON, EMR_POLYGON16:
      EmfPolygon(PEMRPolyline(ARecord), ARecord^.iType = EMR_POLYGON16);
    EMR_POLYPOLYGON, EMR_POLYPOLYGON16:
      EmfPolyPolygon(PEMRPolyPolygon(ARecord),
        ARecord^.iType = EMR_POLYPOLYGON16);
    EMR_POLYPOLYLINE16, EMR_POLYPOLYLINE:
      EmfPolyPolyline(PEMRPolyPolyline(ARecord),
        ARecord^.iType = EMR_POLYPOLYLINE16);
    ...
    EMR_POLYBEZIERTO, EMR_POLYBEZIERTO16:
      EmfPolyBezierTo(PEMRPolyBezierTo(ARecord),
        ARecord^.iType = EMR_POLYBEZIERTO16);
    EMR_SETPIXELV:
    ...
    EMR_SETVIEWPORTORGEX:
      State.ViewPortOrg := ConvertPoint(PEMRSetViewportOrgEx(ARecord)
        ^.ptlOrigin);
    EMR_SETWINDOWEXTEX:
...
function TdxPSPDFMetaFileGdiObjectList.CreateBrush(const ABrushInfo:
  {$IFDEF DELPHI16}TLogBrush32{$ELSE}TLogBrush{$ENDIF}): HBRUSH;
var
  ABrush: TBrush;
...
begin
  ...
  Add(TdxPSPDFMetaFileGdiObjectItem.Create(ABrush.Handle, ABrush,
    ABrushInfo.lbColor));
  Result := ABrush.Handle;
end;

function TdxPSPDFMetaFileGdiObjectList.CreateFont(const AFontInfo
  : EXTLOGFONTW): HFONT;
var
  AFont: TFont;
  ALogFont: TLogFontW;
...
begin
  ...
  Add(TdxPSPDFMetaFileGdiObjectItem.Create(APen.Handle, APen,
    APenInfo.lopnColor));
  Result := APen.Handle;
end;

function TdxPSPDFMetaFileGdiObjectList.CreatePenEx(const APenInfo:
  {$IFDEF DELPHI16}TExtLogPen32{$ELSE}TExtLogPen{$ENDIF}): HPEN;
var
  APen: TPen;
begin
...
function TdxPSPDFMetaFileGdiObjectList.FindObject(AHandle: THandle;
  var AObject: TdxPSPDFMetaFileGdiObjectItem): Boolean;
var
  I: Integer;
begin
...
function TdxPSPDFMetaFileGdiObjectList.GetItem(Index: Integer)
  : TdxPSPDFMetaFileGdiObjectItem;
begin
  Result := TdxPSPDFMetaFileGdiObjectItem(inherited Items[Index]);
end;

procedure TdxPSPDFMetaFileGdiObjectList.Notify(Ptr: Pointer;
...
constructor TdxPSPDFMetaFileGdiObjectItem.Create(AHandle: THandle;
  ASourceObject: TObject; AColor: TColor);
begin
  inherited Create;
  FHandle := AHandle;
...
procedure TdxPSPDFMetaFileCanvasState.Assign
  (ASource: TdxPSPDFMetaFileCanvasState);
begin
...
function TdxPSPDFMetaFileTextInfo.Concatenate
  (AInfo: TdxPSPDFMetaFileTextInfo): Boolean;
begin
  Result := (Options = AInfo.Options) and dxAreFontsEqual(Font, AInfo.Font) and
    (TextBounds.Top = AInfo.TextBounds.Top) and
    (TextBounds.Bottom = AInfo.TextBounds.Bottom) and
...
procedure TdxPSPDFMetaFileTextInfo.Initialize(const R: PEMRExtTextOut;
  AIsUnicode: Boolean; AFont: TFont; ATextAlign: Integer);
  ...
  begin
    if AIsUnicode then
      SetString(Result, PWideChar(TdxNativeUInt(R) + R^.emrtext.offString),
        R^.emrtext.nChars)
    else
    begin
      SetString(ATextA, PAnsiChar(TdxNativeUInt(R) + R^.emrtext.offString),
        R^.emrtext.nChars);
      Result := dxAnsiStringToString(ATextA,
        dxGetCodePageFromCharset(AFont.Charset));
    end;
    ...
    begin
      ...
      try
        for I := 1 to Length(Result) do
          Result[I] := WideChar(AFontData.CharCodeByGlyphIndex
            [Word(Result[I])]);
      finally
        AFontData.Free;
      end;
...
begin
  ...
  else if FFont.Orientation = 0 then
  begin
    FTextOrigin := ConvertTextOrigin(FTextOrigin, TextBounds, FTextWidth,
      AFontHeight, ATextAlign);
    FTextBounds := cxRectSetOrigin(TextBounds, TextOrigin);
    FTextBounds.Bottom := TextBounds.Bottom + AFontHeight;
  end;
end;

function TdxPSPDFMetaFileTextInfo.CalculateTextWidth
  (const R: PEMRExtTextOut): Integer;
var
  AOffsets: PInteger;
  I: Integer;
begin
...
function TdxPSPDFMetaFileTextInfo.ConvertTextOrigin(const P: TPoint;
  const ABox: TRect; ATextWidth, ATextHeight, AAlignMode: Integer): TPoint;
begin
  Result := P;
  ...
  if (AAlignMode and TA_BOTTOM = TA_BOTTOM) or
    (AAlignMode and TA_CENTER = TA_CENTER) then
    Result.Y := ABox.Top;
end;
