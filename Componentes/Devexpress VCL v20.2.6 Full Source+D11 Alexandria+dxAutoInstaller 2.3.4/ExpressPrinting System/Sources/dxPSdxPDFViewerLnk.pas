...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Classes, Windows, Graphics, cxGeometry, dxCore, cxGraphics, dxPrnPg,
  dxPSReportRenderCanvas, dxPSCore, dxPrnDev,
  dxBase, dxPSGlbl, dxPgsDlg, dxPrnDlg, dxPDFCore, dxPDFCommandInterpreter,
  dxPDFDocument, dxPDFViewer, dxPDFFontUtils,
...
type
  ...
  strict private
    ...
    function GetPageOrientation(AInfo: TcxPDFViewerReportLinkPageRenderInfo)
  ...
  published
    property PrintAsImage: Boolean read FPrintAsImage write SetPrintAsImage
      default False;
  end;
  ...
  public
    property PageRenderInfo: TcxPDFViewerReportLinkPageRenderInfo
  ...
  protected
    ...
    function DoBeforeExportToPDF(const AFileName: string;
      AOptions: TdxPSPDFReportExportOptions): Boolean; override;
...
uses
  Math, Forms, SysUtils, dxTypeHelpers, cxClasses, dxPrinting, dxCoreGraphics,
  dxGDIPlusAPI, dxGDIPlusClasses, dxPSUtl,
...
type
  ...
  strict private
    function CreateRenderParameters(APage: TdxPDFPage;
      AParameters: TdxPDFPrintParameters; var ARealBounds: TRect)
      : TdxPDFPrintParameters;
  protected
    function CreateGraphics: TdxGPCanvas; override;
    function IsPrinting: Boolean; override;
    procedure DestroyGraphics; override;
  public
    procedure Export(APage: TdxPDFPage;
      AParameters: TdxPDFExportParameters); override;
    procedure ExportAsImage(APage: TdxPDFPage;
      AParameters: TdxPDFExportParameters);
  end;

  { TdxPDFPrinting }

procedure TdxPDFPrinting.Export(APage: TdxPDFPage;
  AParameters: TdxPDFExportParameters);
var
  ARealBounds: TRect;
  ARenderParameters: TdxPDFRenderParameters;
begin
  ARenderParameters := CreateRenderParameters(APage,
    AParameters as TdxPDFPrintParameters, ARealBounds);
  try
...
procedure TdxPDFPrinting.ExportAsImage(APage: TdxPDFPage;
  AParameters: TdxPDFExportParameters);
var
  ABitmap: TBitmap;
...
begin
  ...
  ARenderParameters := CreateRenderParameters(APage, APrintParameters,
    ARealBounds) as TdxPDFPrintParameters;
  ...
  if APrintParameters.FitToPageWidth then
    ARenderParameters.ContentBounds :=
      cxRectSetOrigin(ARenderParameters.ContentBounds, cxNullPoint);
  APrinterCanvas.Lock;
  try
    inherited Export(APage, ARenderParameters);
    ADestRect := cxRect(ARealBounds.TopLeft, ARealBounds.TopLeft);
    ADestRect := cxRectSetSize(ADestRect, ABitmap.Width, ABitmap.Height);
    cxBitBlt(APrinterCanvas.Handle, ABitmap.Canvas.Handle, ADestRect,
      cxNullPoint, SRCCOPY);
  finally
...
function TdxPDFPrinting.CreateRenderParameters(APage: TdxPDFPage;
  AParameters: TdxPDFPrintParameters; var ARealBounds: TRect)
  : TdxPDFPrintParameters;
var
...
begin
  ...
  APreviewFactor := Result.ZoomFactor / 100;
  ADPIFactor := AParameters.DPI / cxGetCurrentDPI;
  Result.ScaleFactor := Result.ScaleFactor * APreviewFactor * ADPIFactor;

  Result.Position := cxPointF(AParameters.ContentBounds.Left * ADPIFactor,
    AParameters.ContentBounds.Top * ADPIFactor);
  APageSize := TdxPDFViewerViewState.CalculatePageSize(APage.Size,
    AParameters.DocumentState.RotationAngle);
  ARealBounds := cxRect(cxRectCenter(AParameters.ContentBounds),
    cxRectCenter(AParameters.ContentBounds));
  W := Trunc(APageSize.X * AParameters.ScaleFactor / 2);
  H := Trunc(APageSize.Y * AParameters.ScaleFactor / 2);
  ARealBounds := cxRectScale(cxRectInflate(ARealBounds, W, H), AParameters.DPI,
    cxGetCurrentDPI);
...
begin
  case PrinterPage.ScaleMode of
    smFit:
      begin
        ASize := TdxPDFViewerViewState.CalculatePageSize(FPDFPage.Size,
          FRotationAngle);
        FScaleFactor := Min(cxRectWidth(ContentBounds) / ASize.X,
          cxRectHeight(ContentBounds) / ASize.Y);
      end;
    smAdjust:
...
begin
  ...
  ADetailsTwipsRect := cxRectScale(PrinterPage.PaintRectLoMetric, 1440, 254,
    1440, 254);
...
function TcxPDFViewerReportLinkPageRenderInfo.CreatePrinterPage
  (ARenderInfo: TdxPSReportRenderInfo): TdxPrinterPage;
begin
...
begin
  ...
  begin
    ...
    TdxPDFUtils.CopyData(ReportLink.PagesToPrint, 0, FPageIndices, 0,
      Length(ReportLink.PagesToPrint));
...
begin
  ...
  begin
    ARenderInfo := CreatePageRenderInfo(VirtualPageCount)
      as TcxPDFViewerReportLinkPageRenderInfo;
...
function TdxPDFViewerReportLinkRenderInfo.GetPageRenderInfoClass
...
begin
  Result := (PageRenderInfos[ReportLink.Renderer.RenderingPageIndex]
    as TcxPDFViewerReportLinkPageRenderInfo).PageSize;
end;
...
function TdxPDFViewerReportLinkRenderInfo.GetWindowScalePair
  : TdxWindowScalePair;
begin
...
function TdxPDFViewerReportLinkRenderInfo.LoMetricValueToInternalUnits
  (Value: Integer): Integer;
begin
  Result := MulDiv(Value, UnitsPerInch, 254);
end;

function TdxPDFViewerReportLinkRenderInfo.GetPageOrientation
  (AInfo: TcxPDFViewerReportLinkPageRenderInfo): TdxPrinterOrientation;
var
  ASize: TdxPointF;
begin
  if ReportLink.PrinterPage.Orientation = poAuto then
  begin
    ASize := TdxPDFViewerViewState.CalculatePageSize(AInfo.FPDFPage.Size,
      AInfo.FRotationAngle);
...
procedure TdxPDFViewerReportLink.PageParamsChanged(Sender: TdxPrinterPage;
...
function TdxPDFViewerReportLink.PrintDialog(var APrintDlgData
...
begin
  ...
  begin
    TdxComponentPrinterAccess(ComponentPrinter).InitializeDefaultPrintDlgData
      (Self, APrintDlgData);
    try
      if Length(APageIndexes) > 0 then
      begin
        APages := TdxPDFUtils.ConvertToPageRanges
          (TIntegerDynArray(@APageIndexes[0]));
        if APages <> '' then
        begin
...
function TdxPDFViewerReportLink.CreatePreviewWindowForm
...
begin
  ...
  TdxPDFPrintDialogAccess(ADialog).OnGetPrintingResult :=
    OnGetPrintingResultHandler;
...
begin
  ...
  RenderInfo.FootnotesBounds := cxRectScale(RenderInfo.FootnotesBounds,
    UnitsPerInch, PixelsPerInch);
  RenderInfo.TitleBounds := cxRectScale(RenderInfo.TitleBounds, UnitsPerInch,
    PixelsPerInch);
  PageRenderInfo.FRealPageSize.X := MulDiv(PageRenderInfo.FRealPageSize.X,
    UnitsPerInch, PixelsPerInch);
  PageRenderInfo.FRealPageSize.Y := MulDiv(PageRenderInfo.FRealPageSize.Y,
    UnitsPerInch, PixelsPerInch);
  PageRenderInfo.TitleOffset := ScalePoint(PageRenderInfo.TitleOffset,
    UnitsPerInch, PixelsPerInch);
  PageRenderInfo.FootnotesOffset := ScalePoint(PageRenderInfo.FootnotesOffset,
    UnitsPerInch, PixelsPerInch);
  PageRenderInfo.PageHeaderBounds :=
    cxRectScale(PageRenderInfo.PageHeaderBounds, UnitsPerInch, PixelsPerInch);
  PageRenderInfo.PageFooterBounds :=
    cxRectScale(PageRenderInfo.PageFooterBounds, UnitsPerInch, PixelsPerInch);
end;

procedure TdxPDFViewerReportRenderer.PrepareFont(AFont: TFont;
...
begin
  RenderInfo.TitleBounds := cxRectScale(RenderInfo.TitleBounds, PixelsPerInch,
    UnitsPerInch);
  RenderInfo.FootnotesBounds := cxRectScale(RenderInfo.FootnotesBounds,
    PixelsPerInch, UnitsPerInch);
  PageRenderInfo.FRealPageSize.X := MulDiv(PageRenderInfo.FRealPageSize.X,
    PixelsPerInch, UnitsPerInch);
  PageRenderInfo.FRealPageSize.Y := MulDiv(PageRenderInfo.FRealPageSize.Y,
    PixelsPerInch, UnitsPerInch);
  ...
  PageRenderInfo.FootnotesOffset := ScalePoint(PageRenderInfo.FootnotesOffset,
    PixelsPerInch, UnitsPerInch);
  PageRenderInfo.PageHeaderBounds :=
    cxRectScale(PageRenderInfo.PageHeaderBounds, PixelsPerInch, UnitsPerInch);
  PageRenderInfo.PageFooterBounds :=
    cxRectScale(PageRenderInfo.PageFooterBounds, PixelsPerInch, UnitsPerInch);
...
procedure TdxPDFViewerReportRenderer.RenderPageContent;
  ...
  begin
    Result := IsPrinting and (PrinterPage.PrintAsImage or
      ReportLink.PrinterPage.GrayShading);
  end;

  procedure SetLogicalCoordinates(ACanvas: TcxCanvas);
  begin
    SetMapMode(ACanvas.Handle, 0);
    SetWindowExtEx(ACanvas.Handle, PageRenderInfo.PageSize.X,
      PageRenderInfo.PageSize.Y, nil);
    ScaleWindowExtEx(ACanvas.Handle, 1, 1, 1, 1, nil);
    SetViewPortExtEx(ACanvas.Handle, PageRenderInfo.PageSize.X,
      PageRenderInfo.PageSize.Y, nil);
...
begin
  ...
  begin
    ...
    try
      ...
      AParameters := TdxPDFPrintParameters.Create
        (TdxPDFDocumentAccess(Document).State);
...
function TdxPDFViewerReportRenderer.GetImageDataStorage
  : TdxPDFDocumentImageDataStorage;
begin
  Result := TdxPDFDocumentAccess(Document).State.ImageDataStorage;
end;

function TdxPDFViewerReportRenderer.GetPageRenderInfo
  : TcxPDFViewerReportLinkPageRenderInfo;
begin
...
function TdxPDFViewerReportRenderer.GetRenderInfo
  : TdxPDFViewerReportLinkRenderInfo;
begin
