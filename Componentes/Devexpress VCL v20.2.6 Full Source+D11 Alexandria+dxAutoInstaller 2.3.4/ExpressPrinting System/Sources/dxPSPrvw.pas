...
uses
  ...
  IniFiles,
  dxMessages, dxCustomPreview, dxPSGlbl, dxPSCore, dxPSESys, dxPrevw,
  dxPSPrVwOpt, dxPrnPg, dxExtCtrls, dxBkgnd,
  cxLookAndFeels, cxControls, cxGraphics, cxGeometry, cxSplitter,
  cxLookAndFeelPainters, dxBase, cxProgressBar,
  cxButtons;
...
type
  ...
  TdxPSPreviewWindowAddExplorerCommandEvent = procedure(Sender: TObject;
    ACommand: TCustomdxPSExplorerContextCommand) of object;
  TdxPSPreviewWindowCanShowMarginHint = procedure(Sender: TObject;
  ...
  TdxPSPreviewWindowCustomHostOnSize = procedure(Sender: TObject;
  ...
  public
    ...
    //
    property OnResize;
    property OnSizeChanging: TdxPSPreviewWindowCustomHostOnSize
      read FOnSizeChanging write FOnSizeChanging;
  end;
  ...
  published
    property ShowMarginBar: Boolean read FShowMarginBar write SetShowMarginBar
      default True;
    property ShowStatusBar: Boolean read FShowStatusBar write SetShowStatusBar
      default True;
    property ZoomOnClick: Boolean read FZoomOnClick write SetZoomOnClick
      default True;
  end;

  { TdxPSPreviewWindow }

  TdxPSPreviewWindowLoadPropertiesEvent = procedure(Sender: TObject;
    AIniFile: TCustomIniFile; const ASectionName: string) of object;

  TdxPSPreviewWindow = class(TdxPSCustomPreviewWindow,
    IdxComponentPrinterListener, IdxSkinSupport, IdxPSExplorerTreeContainerHost)
  strict private
    ...
    function CreateSplitter(AAlign: TAlign; const AName: string; ALeft: Integer;
      AParent: TWinControl; AResizeControl: TControl;
      OnCanResizeEvent: TCanResizeEvent): TcxCustomSplitter;
    procedure FillEffectsApply(Sender: TObject);
    procedure ExplorerCloseHandler(Sender: TObject);
    procedure ExplorerSplitterCanResize(Sender: TObject; var NewSize: Integer;
      var Accept: Boolean);
    procedure ExplorerToolBarResize(Sender: TObject);
    procedure MarginBarDblClick(Sender: TObject);
    procedure MarginBarGetDrawParams(ASender: TdxPSCustomStatusBar;
      APanel: TdxPSStatusBarPanel; var ABkgColor, ATextColor: TColor);
    procedure PreviewAfterDragMargin(Sender: TObject;
      AMargin: TdxPreviewPageMargin);
    procedure PreviewBeforeDragMargin(Sender: TObject;
      AMargin: TdxPreviewPageMargin);
    procedure PreviewCanShowMarginHint(Sender: TObject;
    ...
    procedure PreviewMarginsChanged(Sender: TObject;
      AMargin: TdxPreviewPageMargin);
    ...
    procedure StatusBarMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
    procedure ThumbnailsPreviewSelectedPageChanged(Sender: TObject;
      APageIndex: Integer);
    procedure ThumbnailsSplitterCanResize(Sender: TObject; var NewSize: Integer;
      var Accept: Boolean);

    procedure EndGenerateReport(Sender: TObject;
      AReportLink: TBasedxReportLink);
    procedure GenerateReportProgress(Sender: TObject;
      AReportLink: TBasedxReportLink; APercentDone: Double);
    procedure StartGenerateReport(Sender: TObject;
      AReportLink: TBasedxReportLink);

    procedure EndPrint(Sender: TObject; AReportLink: TBasedxReportLink);
    procedure NewPage(Sender: TObject; AReportLink: TBasedxReportLink;
      APageIndex: Integer);
  ...
  protected
    ...
    procedure AddExplorerContextCommand
      (ACommand: TCustomdxPSExplorerContextCommand); override;
...
{$ENDIF}
    ...
    procedure ExplorerPaneSizeChanging(Sender: TObject;
      var AWidth, AHeight: Integer);
    procedure ThumbnailsPreviewSizeChanging(Sender: TObject;
      var AWidth, AHeight: Integer);
    procedure UpdateThumbnailsSize;

    function MarginStatusPanel(AMargin: TdxPreviewPageMargin)
      : TdxPSStatusBarPanel;
    function ProgressBarGetMaxValue: Integer;
    function SectionStatusPanel(AStatusSection: TfmPreviewStatusSection)
      : TdxPSStatusBarPanel;
    ...
    procedure SectionStatusPanelSetText(AStatusSection: TfmPreviewStatusSection;
      const AText: string);
    ...
    procedure LoadProperties(AIniFile: TCustomIniFile;
      const ASectionName: string);
    procedure SaveProperties(AIniFile: TCustomIniFile;
      const ASectionName: string);
    ...
    procedure RefreshStatusPanels(AStatusBarMode
      : TdxPSPreviewWindowStatusBarMode); virtual;

    procedure DoExplorerButtonsPosUpdate;
    procedure DoExplorerShowToggled(Value: Boolean);
    procedure DoLoadFromIniFile(AIniFile: TCustomIniFile;
      const ASectionName: string);
    procedure DoPreviewAfterDragMargin(APreview: TdxPreview;
      AMargin: TdxPreviewPageMargin); virtual;
    procedure DoPreviewBeforeDragMargin(APreview: TdxPreview;
      AMargin: TdxPreviewPageMargin); virtual;
    procedure DoPreviewDblClick(APreview: TdxPreview); virtual;
    procedure DoPreviewDragMargin(APreview: TdxPreview;
      AMargin: TdxPreviewPageMargin); virtual;
    procedure DoPreviewMarginChanged(APreview: TdxPreview;
      AMargin: TdxPreviewPageMargin); virtual;
    procedure DoSaveToIniFile(AIniFile: TCustomIniFile;
      const ASectionName: string);
    ...
    function IdxPSExplorerTreeContainerHost.GetFlat =
      IdxPSExplorerTreeContainerHost_GetFlat;
    function IdxPSExplorerTreeContainerHost.GetReportLink =
      IdxPSExplorerTreeContainerHost_GetReportLink;
  ...
  public
    ...
    property ActualEnableOptions: TdxPreviewEnableOptions
      read GetActualEnableOptions;
    property ActualVisibleOptions: TdxPreviewVisibleOptions
      read GetActualVisibleOptions;
    ...
    property ProgressStatusPanel: TdxPSStatusBarPanel
      read GetProgressStatusPanel;
    ...
    property ExplorerCloseButton: TdxPSExplorerCloseButton
      read FExplorerCloseButton;
    property ExplorerPane: TdxPSPreviewWindowCustomHost read FExplorerPane;
    property ExplorerPaneWidth: Integer read GetExplorerPaneWidth
      write SetExplorerPaneWidth;
    ...
    property ThumbnailsPaneWidth: Integer read GetThumbnailsPaneWidth
      write SetThumbnailsPaneWidth;
  ...
  published
    ...
    property Options: TdxPSPreviewWindowOptions read GetOptions
      write SetOptions;
    property PreviewPopupMenu: TComponent read FPreviewPopupMenu
      write SetPreviewPopupMenu;
    property Transparent: Boolean read FTransparent write SetTransparent
      default False;

    property OnAddExplorerCommand: TdxPSPreviewWindowAddExplorerCommandEvent
      read FOnAddExplorerCommand write FOnAddExplorerCommand;
    property OnCanShowMarginHint: TdxPSPreviewWindowCanShowMarginHint
      read FOnCanShowMarginHint write FOnCanShowMarginHint;
    property OnHFTextEntriesChanged: TNotifyEvent read FOnHFTextEntriesChanged
      write FOnHFTextEntriesChanged;
    property OnInitContent: TNotifyEvent read FOnInitContent
      write FOnInitContent;
    property OnLoadProperties: TdxPSPreviewWindowLoadPropertiesEvent
      read FOnLoadProperties write FOnLoadProperties;
    property OnPreviewDblClick: TNotifyEvent read FOnPreviewDblClick
      write FOnPreviewDblClick;
    property OnSaveProperties: TdxPSPreviewWindowLoadPropertiesEvent
      read FOnSaveProperties write FOnSaveProperties;
    property OnStyleListChanged: TNotifyEvent read FOnStyleListChanged
      write FOnStyleListChanged;
    property OnUpdateControls: TNotifyEvent read FOnUpdateControls
      write FOnUpdateControls;
    property OnUpdateExplorerCommands: TNotifyEvent
      read FOnUpdateExplorerCommands write FOnUpdateExplorerCommands;
  ...
  TdxPSStatusBarPanelGetDrawParamsEvent = procedure
    (ASender: TdxPSCustomStatusBar; APanel: TdxPSStatusBarPanel;
    var ABkgColor, ATextColor: TColor) of object;
  ...
  protected
    ...
    procedure CalculateSizeGripBounds(AViewInfo
      : TdxPSStatusBarViewInfo); virtual;
    procedure CreateParams(var Params: TCreateParams); override;
    function CreateViewInfo: TdxPSStatusBarViewInfo; virtual;
    procedure DoGetPanelDrawParams(APanel: TdxPSStatusBarPanel;
      var ABkgColor, ATextColor: TColor); virtual;
    procedure DrawBackground(ACanvas: TcxCanvas; const R: TRect); override;
    procedure DrawItem(ACanvas: TcxCanvas;
      APanel: TdxPSStatusBarPanel); virtual;
    procedure DrawPanelItem(ACanvas: TcxCanvas;
      APanel: TdxPSStatusBarPanel); virtual;
    procedure DrawPanelItemText(ACanvas: TcxCanvas; APanel: TdxPSStatusBarPanel;
      ATextColor: TColor); virtual;
    procedure DrawSepartorItem(ACanvas: TcxCanvas;
      APanel: TdxPSStatusBarPanel); virtual;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function PanelFromPoint(APoint: TPoint; var APanel: TdxPSStatusBarPanel)
      : Boolean; virtual;
    ...
    procedure SetBounds(ALeft, ATop, AWidth, AHeight: Integer); override;
    //
    property OnPanelGetDrawParams: TdxPSStatusBarPanelGetDrawParamsEvent
      read FOnPanelGetDrawParams write FOnPanelGetDrawParams;
...
function AddStatusPanel(AStatusBar: TdxPSCustomStatusBar;
  AAlignment: TAlignment; AShowBorder: Boolean; const AText: string;
  AWidth: Integer; AStyle: TdxPSStatusBarPanelStyle = sbpsPanel)
  : TdxPSStatusBarPanel;
...
{$ENDIF}
  ...
  dxPrnDev, dxPSUtl, dxPgsDlg, dxfmClr, dxfmMnPg, dxfmDTFmt, dxfmPNFmt,
  dxPSEngn,
  dxPSExtDlgs, dxPSPopupMan, dxPSEvnt, dxfmZoom, Types, dxPSForm, dxPSHFLibrary,
  StrUtils, dxDPIAwareUtils;
...
const
  ...
  pssAll: TfmPreviewStatusSections = [ssCurrentPage, ssPageCount, ssPaperSize,
    ssStatus];
...
function MeasureStatusPanelWidth(AStatusBar: TdxPSCustomStatusBar;
...
begin
  I := Length(S);
  while (I > 0) and (S[I] = PercentSymbol) do
    Dec(I);
  Result := Copy(S, 1, I);
end;
...
constructor TdxPSPreviewExplorerChangeNotifier.Create
  (APreview: TdxPSPreviewWindow);
begin
  FPreview := APreview;
  inherited Create(Preview.Explorer);
end;

procedure TdxPSPreviewExplorerChangeNotifier.ItemDataLoaded
  (AnItem: TdxPSExplorerItem);
begin
  Preview.UpdateControls;
end;

procedure TdxPSPreviewExplorerChangeNotifier.ItemDataUnloaded
  (AnItem: TdxPSExplorerItem);
begin
  Preview.UpdateControls;
end;
...
procedure TdxPSPreviewWindowCustomHost.DoSizeChanging(var AWidth,
  AHeight: Integer);
begin
...
procedure TdxPSPreviewWindowCustomHost.DrawBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
  ACanvas.FillRect(R, LookAndFeelPainter.DefaultControlColor);
end;

procedure TdxPSPreviewWindowCustomHost.EraseBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
...
procedure TdxPSPreviewWindowCustomHost.SetBounds(ALeft, ATop, AWidth,
  AHeight: Integer);
begin
...
begin
  Result := 2 * ScaleFactor.Apply(LookAndFeelPainter.BorderSize) + 2 *
    (3 + Ord(Flat)) + 2 * cxTextOffset + cxTextHeight(Font);
end;
...
begin
  FPrintEventsSubscriber := TdxPSPrintReportSubscriber.Create
    ([TdxPSPrintEvent]);
  ...
  FBuildEventsSubscriber := TdxPSBuildReportSubscriber.Create
    ([TdxPSBuildEvent]);
  TdxPSBuildReportSubscriber(FBuildEventsSubscriber).OnEndGenerateReport :=
    EndGenerateReport;
  TdxPSBuildReportSubscriber(FBuildEventsSubscriber).OnGenerateReportProgress :=
    GenerateReportProgress;
  TdxPSBuildReportSubscriber(FBuildEventsSubscriber).OnStartGenerateReport :=
    StartGenerateReport;

  FStyleEventsSubscriber := TdxStyleListChangedSubscriber.Create
    ([TdxSMStyleListChangedEvent]);
  TdxStyleListChangedSubscriber(FStyleEventsSubscriber).OnStyleListChanged :=
    StyleListChanged;

  FHFTextEntriesChangedSubscriber := TdxHFTextEntriesChangedSubscriber.Create
    ([TdxHFTextEntriesChangedEvent]);
  TdxHFTextEntriesChangedSubscriber(FHFTextEntriesChangedSubscriber)
...
procedure TdxPSPreviewWindow.CreateExplorerToolBar;
  ...
  begin
    ...
    FExplorerCloseButton.SetBounds(0, (FExplorerToolBar.Height - ButtonSize)
      div 2, ButtonSize, ButtonSize);
...
begin
  ...
  AddStatusPanel(FMarginBar, taLeftJustify, False,
    DropAmpersand(cxGetResourceString(@sdxMargins)), 55);
  AddStatusPanel(FMarginBar, taRightJustify, False,
    DropAmpersand(cxGetResourceString(@sdxLeft)), 40);
  AddStatusPanel(FMarginBar, taRightJustify, not Flat, '', 70);
  AddStatusPanel(FMarginBar, taRightJustify, False,
    DropAmpersand(cxGetResourceString(@sdxTop)), 40);
  AddStatusPanel(FMarginBar, taRightJustify, not Flat, '', 70);
  AddStatusPanel(FMarginBar, taRightJustify, False,
    DropAmpersand(cxGetResourceString(@sdxRight)), 50);
  AddStatusPanel(FMarginBar, taRightJustify, not Flat, '', 70);
  AddStatusPanel(FMarginBar, taRightJustify, False,
    DropAmpersand(cxGetResourceString(@sdxBottom)), 60);
  AddStatusPanel(FMarginBar, taRightJustify, not Flat, '', 70);
  AddStatusPanel(FMarginBar, taRightJustify, False,
    DropAmpersand(cxGetResourceString(@sdxHeader2)), 50);
  AddStatusPanel(FMarginBar, taRightJustify, not Flat, '', 70);
  AddStatusPanel(FMarginBar, taRightJustify, False,
    DropAmpersand(cxGetResourceString(@sdxFooter2)), 50);
  AddStatusPanel(FMarginBar, taRightJustify, not Flat, '', 70);
...
function TdxPSPreviewWindow.CreateSplitter(AAlign: TAlign; const AName: string;
  ALeft: Integer; AParent: TWinControl; AResizeControl: TControl;
  OnCanResizeEvent: TCanResizeEvent): TcxCustomSplitter;
begin
...
begin
  ...
  ThumbnailsPreview.OptionsBehavior := ThumbnailsPreview.OptionsBehavior +
    [pobNonCenterizePages];
  if IsWin9X then
    ThumbnailsPreview.OptionsBehavior := ThumbnailsPreview.OptionsBehavior -
      [pobThumbTracking];
  ThumbnailsPreview.OptionsView := ThumbnailsPreview.OptionsView - [povMargins];
  ThumbnailsPreview.OptionsZoom := ThumbnailsPreview.OptionsZoom -
    [pozZoomOnClick];
  ThumbnailsPreview.ZoomFactor := dxThumbnailsZoomFactors
    [Options.ThumbnailsOptions.Size];

  ThumbnailsPreview.OnSelectedPageChanged :=
    ThumbnailsPreviewSelectedPageChanged;
  ThumbnailsPreview.OnDrawPageContent := PaintThumbnailPage;
end;

procedure TdxPSPreviewWindow.RefreshStatusPanels(AStatusBarMode
  : TdxPSPreviewWindowStatusBarMode);
...
begin
  ...
  try
    ...
    case AStatusBarMode of
      psbmNormal:
        begin
          AddStatusPanel(StatusBar, taRightJustify, False,
            DropAmpersand(cxGetResourceString(@sdxPage)) + ':', 40);
          AddStatusPanel(StatusBar, taRightJustify, not Flat, '', 45);
          AddStatusPanel(StatusBar, taCenter, False,
            cxGetResourceString(@sdxOf), 30);
          AddStatusPanel(StatusBar, taRightJustify, not Flat, '', 45);
          AddStatusPanel(StatusBar, taRightJustify, False,
            cxGetResourceString(@sdxPages), 50);
          AddStatusPanel(StatusBar, taLeftJustify, not Flat, '', 4,
            sbpsSeparator);
          AddStatusPanel(StatusBar, taRightJustify, False,
            cxGetResourceString(@sdxPaperSize), 80);
          AddStatusPanel(StatusBar, taRightJustify, not Flat, '', 150);
          AddStatusPanel(StatusBar, taRightJustify, False,
            cxGetResourceString(@sdxStatus), 60);
          AddStatusPanel(StatusBar, taLeftJustify, not Flat,
            cxGetResourceString(@sdxStatusReady), -1);
      ...
      psbmProgress:
        begin
          AddStatusPanel(StatusBar, taLeftJustify, False, '',
            Max(Min(300, Width div 2), 100));
          AddStatusPanel(StatusBar, taLeftJustify, False,
            GetProgressModeCaption, -1);
...
begin
  ...
  begin
    ...
    AddObject(cxGetResourceString(@sdxWidenToSourceWidth),
      TObject(Integer(-1)));
  end;
end;
...
begin
  ...
  begin
    ...
    FReleased := not Application.Terminated and
      (IsBuilding or (IsPrinting and
      MessageQuestion(cxGetResourceString(@sdxAbortPrinting))));
    if FReleased then
...
begin
  Result := (dxPSEngine.DialogsLookAndFeel.Kind in [lfFlat, lfOffice11]) and
    not(dxPSEngine.IsSkinsStyle or dxPSEngine.IsNativeStyle or
    cxIsVCLThemesEnabled);
end;
...
procedure TdxPSPreviewWindow.EraseBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
  if IsTransparentBackground then
...
procedure TdxPSPreviewWindow.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
begin
  if ExplorerSplitter <> nil then
    TcxSplitterAccess(ExplorerSplitter).Color :=
      LookAndFeelPainter.DefaultControlColor;
  if ThumbnailsSplitter <> nil then
    TcxSplitterAccess(ThumbnailsSplitter).Color :=
      LookAndFeelPainter.DefaultControlColor;
end;
...
begin
  ...
  begin
    ...
    ThumbnailsSplitter.Left := ThumbnailsPane.Left -
      ThumbnailsSplitter.Width - 1;
  end;
end;
...
begin
  ...
  if Supports(Explorer, IdxPSExplorerContextCommands, AExplorerContextCommands)
  then
...
function TdxPSPreviewWindow.IdxPSExplorerTreeContainerHost_GetReportLink
  : TBasedxReportLink;
begin
  Result := Self.ReportLink;
end;

procedure TdxPSPreviewWindow.BeforeDestroyReport(AReportLink
  : TBasedxReportLink);
begin
...
procedure TdxPSPreviewWindow.UnprepareBuildReport
  (AReportLink: TBasedxReportLink);
begin
  if AReportLink = ReportLink then
  begin
...
procedure TdxPSPreviewWindow.InitContent;
  ...
  procedure SetupPageSize(APageSize: TdxPreviewPageSizeOptions;
  ...
  begin
    ...
    APageSize.MinUsefulSize := Point(APage.MinPrintableAreaLoMetric,
      APage.MinPrintableAreaLoMetric);
  ...
  procedure SetupPreview(APreview: TdxPreview; ALink: TBasedxReportLink;
    APrinterPage: TdxPrinterPage);
  const
    PreviewPaperOrientationMap: array [TdxPrinterOrientation]
      of TdxPreviewPaperOrientation = (ppoPortrait, ppoLandscape, ppoPortrait);
  var
    ...
    begin
      ...
      try
        ...
        begin
          ...
          APrinterPageForPage := ReportLink.Renderer.RenderInfo.
            GetActualPageRenderInfo(I).PrinterPage;
...
begin
  ...
  begin
    FPreview.MeasurementUnits := TdxPreviewMeasurementUnits
      (PrinterPage.MeasurementUnits);
    if Supports(ReportLink, IdxPSPreviewMeasurementUnitsProvider, AIntf) then
      TdxPreviewAccess(FPreview).DefaultMeasurementUnits :=
        AIntf.GetMeasurementUnits;
    ...
    TdxPreviewAccess(FPreview).PageBackgroundApplyToEntirePage :=
      TdxReportLinkAccess(ReportLink).IsApplyBackgroundToEntirePage;
  end;
  ...
  begin
    ...
    try
      if CanChangeMargins then
        Preview.OptionsBehavior := Preview.OptionsBehavior +
          [pobAllowDragMargins]
      else
        Preview.OptionsBehavior := Preview.OptionsBehavior -
          [pobAllowDragMargins];
...
procedure TdxPSPreviewWindow.ExplorerSplitterCanResize(Sender: TObject;
...
procedure TdxPSPreviewWindow.PreviewMarginsChanged(Sender: TObject;
  AMargin: TdxPreviewPageMargin);
var
  APage: TdxPrinterPage;
  AValue: Integer;
...
procedure TdxPSPreviewWindow.PreviewAfterDragMargin(Sender: TObject;
  AMargin: TdxPreviewPageMargin);
begin
...
procedure TdxPSPreviewWindow.PreviewBeforeDragMargin(Sender: TObject;
  AMargin: TdxPreviewPageMargin);
begin
...
procedure TdxPSPreviewWindow.PreviewDragMargin(Sender: TObject;
  AMargin: TdxPreviewPageMargin);
begin
...
procedure TdxPSPreviewWindow.PreviewCanShowMarginHint(Sender: TObject;
  var ACanShowHint: Boolean);
begin
  if CanCallEvents then
  begin
...
procedure TdxPSPreviewWindow.PreviewSelectedPageChanged(Sender: TObject;
  APageIndex: Integer);
begin
  if ReportLink <> nil then
...
begin
  ...
  if (phtNoWhere in HitTests) and CanDesign then
    DoDesignReport
  else if (HitTests * phtMargins <> []) and CanChangeMargins and CanPageSetup
  then
    DoPageSetupReport(1);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if PtInRect then
        StatusBar.Hint := cxGetResourceString
          (@sdxHintDoubleClickForChangePaperSize)
      else
        StatusBar.Hint := '';
    end;
  end;
...
procedure TdxPSPreviewWindow.ThumbnailsSplitterCanResize(Sender: TObject;
  var NewSize: Integer; var Accept: Boolean);
var
  ATempValue: Integer;
begin
...
begin
  ...
  begin
    if dxChooseBackgroundDlg(Preview.PageBackground, Pt,
      dxDefaultBackgroundDlgData) then
      DoSyncPrintingPageBackground;
  end;
end;

procedure TdxPSPreviewWindow.DoShowMultiplySelectPagesDlg
  (AImageList: TCustomImageList; AImageIndex: Integer; const Pt: TPoint;
  AYShift: Integer);
var
  AColCount: Integer;
...
begin
  ...
  APageSize := cxPointScale(TdxPreviewAccess(FPreview)
    .PageSize.ActualSizeInPixels, FPreview.MinZoomFactor, 100);
  AMaxColCount :=
    Max(1, Floor(cxRectWidth(TdxPreviewAccess(FPreview).GetPagesArea) /
    (TdxPreviewAccess(FPreview).Indent + APageSize.X)));
  AMaxRowCount :=
    Max(1, Floor(cxRectHeight(TdxPreviewAccess(FPreview).GetPagesArea) /
    (TdxPreviewAccess(FPreview).Indent + APageSize.Y)));

  AColCount := Min(AMaxColCount, 3);
  ARowCount := Min(AMaxRowCount, 2);

  if dxChooseMultiplePages(AImageList, AImageIndex, AOrigin, AYShift,
    AMaxColCount, AMaxRowCount, AColCount, ARowCount) then
...
procedure TdxPSPreviewWindow.ExplorerPaneSizeChanging(Sender: TObject;
  var AWidth, AHeight: Integer);
begin
  AWidth := Min(ClientWidth - MinPreviewSize.X, AWidth);
end;

procedure TdxPSPreviewWindow.ThumbnailsPreviewSizeChanging(Sender: TObject;
  var AWidth, AHeight: Integer);
begin
...
begin
  ThumbnailsPreview.ZoomFactor := dxPSCore.dxThumbnailsZoomFactors
    [Options.ThumbnailsOptions.Size];
end;
...
begin
  if CanPageSetup then
  begin
    if ComponentPrinter.PageSetupEx(APageIndex, False, CanPrintDialog,
      PreviewBtnClicked, PrintBtnClicked) then
    begin
      InitContent;
      UpdateControls;
    end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ADialog.FileName := ChangeFileExt(ADialog.FileName,
          '.' + dxPSCore.dxPSReportFileShortExtension);
...
begin
  Result := IsVisible(pvoReportDesign) and IsEnabled(peoReportDesign) and
    (ReportLink <> nil) and ReportLink.CheckToDesign and not IsPrinting and
    not IsBuilding;
end;

function TdxPSPreviewWindow.CanPrint: Boolean;
begin
  Result := IsVisible(pvoPrint) and IsEnabled(peoPrint) and (ReportLink <> nil)
    and (ReportLink.PageCount > 0) and (dxPrintDevice.CurrentDevice <> '') and
    not IsPrinting;
end;
...
begin
  Result := (ReportLink <> nil) and ReportLink.CanExportToPDF and
    (ReportLink.PageCount > 0) and not IsPrinting;
end;

function TdxPSPreviewWindow.CanLoadReport: Boolean;
begin
  if IsExplorerAvailable then
    Result := (ExplorerTree <> nil) and ExplorerTree.CanLoadSelectedItemData and
      not IsBuilding
  else
...
begin
  Result := IsVisible(pvoPageSetup) and IsEnabled(peoPageSetup) and
    (ComponentPrinter <> nil) and not IsPrinting;
end;

function TdxPSPreviewWindow.CanChangeMargins: Boolean;
begin
  Result := IsEnabled(peoCanChangeMargins) and not IsPrinting and
    not IsBuilding;
end;
...
begin
  ...
  begin
    if IsExplorerAvailable and (ReportLink = nil) then
      ...
      Result := (ReportLink <> nil) and IsVisible(pvoReportFileOperations) and
        ReportLink.CanSaveData;
  end
  else
    Result := False;
end;
...
function TdxPSPreviewWindow.IsVisible(AOption: TdxPreviewVisibleOption)
  : Boolean;
begin
...
begin
  Result := ((ComponentPrinter <> nil) and
    (cpsPrinting in ComponentPrinter.State)) or dxPrintDevice.Printing;
end;
...
procedure TdxPSPreviewWindow.RefreshStatusBar(AStatusSections
  : TfmPreviewStatusSections);
begin
  if not IsProgressState then
  begin
    if ssCurrentPage in AStatusSections then
      SectionStatusPanelSetText(ssCurrentPage, IfThen(ActivePageIndex >= 0,
        IntToStr(ActivePageIndex + 1), ''));
    if ssPageCount in AStatusSections then
      SectionStatusPanelSetText(ssPageCount,
        IfThen(PageCount > 0, IntToStr(PageCount), ''));
    if ssPaperSize in AStatusSections then
      SectionStatusPanelSetText(ssPaperSize, TdxPreviewAccess(FPreview)
        .PageSizeToString);
...
begin
  ...
  begin
    if Assigned(AMargin) and (AMargin.DraggingValue >= 0) then
      MarginStatusPanel(AMargin).Text :=
        MarginValueToString(AMargin.DraggingValue)
    else
    begin
      MarginStatusPanel(Margins.Left).Text :=
        MarginValueToString(Margins.Left.Value);
      MarginStatusPanel(Margins.Top).Text :=
        MarginValueToString(Margins.Top.Value);
      MarginStatusPanel(Margins.Right).Text :=
        MarginValueToString(Margins.Right.Value);
      MarginStatusPanel(Margins.Bottom).Text :=
        MarginValueToString(Margins.Bottom.Value);
      MarginStatusPanel(Margins.Header).Text :=
        MarginValueToString(Margins.Header.Value);
      MarginStatusPanel(Margins.Footer).Text :=
        MarginValueToString(Margins.Footer.Value);
    end;
  end;
  MarginBar.Invalidate;
end;

function TdxPSPreviewWindow.MarginStatusPanel(AMargin: TdxPreviewPageMargin)
  : TdxPSStatusBarPanel;
...
function TdxPSPreviewWindow.SectionStatusPanel(AStatusSection
  : TfmPreviewStatusSection): TdxPSStatusBarPanel;
const
...
procedure TdxPSPreviewWindow.SectionStatusPanelSetText(AStatusSection
  : TfmPreviewStatusSection; const AText: string);
var
...
procedure TdxPSPreviewWindow.DoPreviewMarginChanged(APreview: TdxPreview;
  AMargin: TdxPreviewPageMargin);
begin
...
procedure TdxPSPreviewWindow.DoLoadFromIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  if CanCallEvents then
  begin
...
procedure TdxPSPreviewWindow.DoSaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  if CanCallEvents then
  begin
...
procedure TdxPSPreviewWindow.DoPreviewAfterDragMargin(APreview: TdxPreview;
  AMargin: TdxPreviewPageMargin);
var
...
procedure TdxPSPreviewWindow.DoPreviewDragMargin(APreview: TdxPreview;
  AMargin: TdxPreviewPageMargin);
begin
  RefreshMarginBar(AMargin);
end;
...
begin
  FExplorerCloseButton.Left := FExplorerToolBar.Width -
    FExplorerCloseButton.Width - 1;
end;
...
begin
  ...
  begin
    ...
    else if Assigned(ExplorerTree) then
    begin
...
procedure TdxPSPreviewWindow.EndPrint(Sender: TObject;
  AReportLink: TBasedxReportLink);
begin
  if Sender <> ComponentPrinter then
    Exit;
...
procedure TdxPSPreviewWindow.NewPage(Sender: TObject;
...
procedure TdxPSPreviewWindow.StartPrint(Sender: TObject;
  ...
  if I > -1 then
    ...
    begin
      ...
      case I - PredefinedZoomValueCount of
        1:
          TdxPreviewAccess(FPreview).SetPageXYCount(1, 1);
        2:
          TdxPreviewAccess(FPreview).SetPageXYCount(2, 1);
        3:
          TdxPreviewAccess(FPreview).SetPageXYCount(2, 2);
        4:
          begin
...
procedure TdxPSPreviewWindow.UpdateStatusBarPanelWidths
  ...
  begin
    if APanel.Width >= 0 then
      APanel.Width := Max(APanel.Width, MeasureStatusPanelWidth(AStatusBar,
        APanel.Text));
  end;
...
begin
  ...
  Preview.MeasurementUnits := TdxPreviewMeasurementUnits
    (AData.MeasurementUnits);
  PrinterPage.MeasurementUnits := AData.MeasurementUnits;
end;

procedure TdxPSPreviewWindow.LoadFromIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxPSPreviewWindow.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxPSPreviewWindow.LoadProperties(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  BeginUpdate;
  try
    TdxPreviewAccess(Preview).LoadFromIniFile(AIniFile,
      dxValidatePath(ASectionName) + sdxPreviewControl);
    Options.ShowMarginBar := AIniFile.ReadBool(ASectionName, sdxShowMarginBar,
      Options.ShowMarginBar);
    Options.ShowStatusBar := AIniFile.ReadBool(ASectionName, sdxShowStatusBar,
      Options.ShowStatusBar);

    ExplorerPaneWidth := ScaleFactor.Apply(AIniFile.ReadInteger(ASectionName,
      sdxExplorerPaneWidth, DefaultExplorerPaneWidth));
    Options.ShowExplorer := AIniFile.ReadInteger(ASectionName,
      sdxExplorerVisibilityState, 1) > 1;

    ThumbnailsPaneWidth := ScaleFactor.Apply(AIniFile.ReadInteger(ASectionName,
      sdxThumbnailsPaneWidth, DefaultThumbnailsPaneWidth));
    Options.ThumbnailsOptions.Visible := AIniFile.ReadInteger(ASectionName,
      sdxThumbnailsVisibilityState, 1) > 1;
  finally
    EndUpdate;
  end;
end;

procedure TdxPSPreviewWindow.SaveProperties(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  TdxPreviewAccess(Preview).SaveToIniFile(AIniFile, dxValidatePath(ASectionName)
    + sdxPreviewControl);
  ...
  AIniFile.WriteInteger(ASectionName, sdxExplorerVisibilityState,
    Ord(Options.ShowExplorer) + 1);
  if ExplorerPaneWidth <> 0 then
    AIniFile.WriteInteger(ASectionName, sdxExplorerPaneWidth,
      ScaleFactor.Revert(ExplorerPaneWidth));

  AIniFile.WriteInteger(ASectionName, sdxThumbnailsVisibilityState,
    Ord(Options.ThumbnailsOptions.Visible) + 1);
  if ThumbnailsPaneWidth <> 0 then
    AIniFile.WriteInteger(ASectionName, sdxThumbnailsPaneWidth,
      ScaleFactor.Revert(ThumbnailsPaneWidth));
end;
...
procedure TdxPSCustomStatusBar.CalculateSizeGripBounds
  (AViewInfo: TdxPSStatusBarViewInfo);
begin
...
begin
  ...
  Result.BorderWidths := cxRect(ABorderSize, ABorderSize, ABorderSize,
    ABorderSize);
...
procedure TdxPSCustomStatusBar.DrawBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
...
procedure TdxPSCustomStatusBar.DrawItem(ACanvas: TcxCanvas;
  APanel: TdxPSStatusBarPanel);
begin
...
procedure TdxPSCustomStatusBar.DrawPanelItem(ACanvas: TcxCanvas;
  APanel: TdxPSStatusBarPanel);
var
...
procedure TdxPSCustomStatusBar.DrawSepartorItem(ACanvas: TcxCanvas;
  APanel: TdxPSStatusBarPanel);
begin
  LookAndFeelPainter.DrawLabelLine(ACanvas, APanel.Bounds, clDefault,
    clDefault, True);
end;

procedure TdxPSCustomStatusBar.DrawPanelItemText(ACanvas: TcxCanvas;
  APanel: TdxPSStatusBarPanel; ATextColor: TColor);
const
  TextAlignFlagsMap: array [TAlignment] of Integer = (cxAlignLeft, cxAlignRight,
    cxAlignHCenter);
begin
  ACanvas.Font := Font;
  ACanvas.Font.Color := ATextColor;
  ACanvas.Brush.Style := bsClear;
  ACanvas.DrawTexT(APanel.Text, APanel.TextRect, cxAlignVCenter or
    cxShowEndEllipsis or TextAlignFlagsMap[APanel.Alignment]);
end;
...
function TdxPSCustomStatusBar.GetDesignHitTest(X, Y: Integer;
...
function TdxPSCustomStatusBar.PanelFromPoint(APoint: TPoint;
  var APanel: TdxPSStatusBarPanel): Boolean;
var
  I: Integer;
begin
  Result := False;
...
begin
  ...
  begin
    if PtInRect(SizeGripRect, ScreenToClient(SmallPointToPoint(Message.Pos)))
    then
