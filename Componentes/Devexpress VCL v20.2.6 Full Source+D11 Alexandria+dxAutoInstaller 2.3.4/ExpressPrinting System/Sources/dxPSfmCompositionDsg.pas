...
uses
  ...
  Menus, ExtCtrls, ComCtrls, IniFiles,
  dxCore, dxPSCore, cxLookAndFeelPainters, cxButtons, cxControls, cxContainer,
  cxListView, cxPC, cxEdit, cxLabel,
  cxGraphics, cxLookAndFeels, cxGroupBox, cxCheckBox, cxClasses,
...
type
  ...
  public
    ...
    property ActiveItem: TdxCompositionLinkItem read GetActiveItem
      write SetActiveItem;
    property Composition: TdxCompositionReportLink read GetComposition;
    property HasOnlyBuiltInsAreInSelection: Boolean
      read GetHasOnlyBuiltInsAreInSelection;
    property IsSelected[Index: Integer]: Boolean read GetIsSelected
      write SetIsSelected;
...
uses
  SysUtils, CommCtrl, Math, dxPSRes, dxPSGlbl, dxPSUtl, dxPSForm, dxPSPopupMan,
  dxPSfmCompositionAdd, dxPSImgs;
...
procedure TdxfmCompositionDesignWindow.LoadFromIniFile(AIniFile: TCustomIniFile;
...
procedure TdxfmCompositionDesignWindow.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
begin
  if not lvItems.IsEditing and
    MessageQuestion(Format(cxGetResourceString(@sdxConfirmDeleteItem),
    [GetSelectedItemsAsString])) then
    DeleteSelection;
end;
...
procedure TdxfmCompositionDesignWindow.lvItemsDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
  MoveSelection(FSaveDragIndex - TcxListView(Sender).ItemFocused.Index);
end;

procedure TdxfmCompositionDesignWindow.lvItemsDragOver(Sender, Source: TObject;
...
begin
  ...
  begin
    ...
    Accept := (Item <> nil) and
      (FSaveDragIndex <> TcxListView(Sender).ItemFocused.Index);
    if Item = nil then
      FSaveDragIndex := -1
    else
...
begin
  ...
  cbStartEachItemFromNewPage.Caption :=
    cxGetResourceString(@sdxCompositionStartEachItemFromNewPage);
...
function TdxfmCompositionDesignWindow.GetItem(Index: Integer)
...
begin
  Result := (Composition <> nil) and
    (coShowDescription in Composition.DesignerOptions);
end;

procedure TdxfmCompositionDesignWindow.SetActiveItem
...
procedure TdxfmCompositionDesignWindow.SetIsSelected(Index: Integer;
  Value: Boolean);
begin
...
procedure TdxfmCompositionDesignWindow.cbStartEachItemFromNewPageClick
  (Sender: TObject);
begin
...
begin
  ...
  try
    ...
    begin
      Width := 2 *
      // + Ord(not(coShowDescription in Composition.DesignerOptions))) *
