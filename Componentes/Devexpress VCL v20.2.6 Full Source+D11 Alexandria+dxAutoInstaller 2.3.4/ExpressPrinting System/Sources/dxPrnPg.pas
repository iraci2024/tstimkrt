...
uses
  Types, Classes, SysUtils, Windows, Controls, Graphics, Dialogs, CommDlg,
  Messages,
  dxBase, dxPSGlbl, dxPSUtl, dxWrap, dxPrnDev, dxBkgnd, cxDrawTextUtils,
  cxGeometry, cxGraphics, dxMeasurementUnits,
  dxPrintUtils;
...
type
  ...
  TdxPrinterPageUpdateCode = (ucMarginLeft, ucMarginTop, ucMarginRight,
    ucMarginBottom, ucMarginHeader, ucMarginFooter, ucScale);
  TdxPrinterPageUpdateCodes = set of TdxPrinterPageUpdateCode;

const
  mtAll: TdxMarginTypes = [mtLeft, mtTop, mtRight, mtBottom, mtHeader,
    mtFooter];
  ucAll = [ucMarginLeft, ucMarginTop, ucMarginRight, ucMarginBottom,
    ucMarginHeader, ucMarginFooter, ucScale];
  ucMargins = [ucMarginLeft, ucMarginTop, ucMarginRight, ucMarginBottom,
    ucMarginHeader, ucMarginFooter];
...
type
  ...
  public
    ...
    property CenterTextAlignY: TcxTextAlignY Index 1 read GetPartialTextAlignY
      write SetPartialTextAlignY default taCenterY;
    property CenterTitle: TStrings Index 1 read GetPartialTitle
      write SetPartialTitle stored IsTitleStored;
    property Font: TFont read FFont write SetFont stored IsFontStored;
    property LeftTextAlignY: TcxTextAlignY Index 0 read GetPartialTextAlignY
      write SetPartialTextAlignY default taCenterY;
    property LeftTitle: TStrings Index 0 read GetPartialTitle
      write SetPartialTitle stored IsTitleStored;
    property Page;
    property RightTextAlignY: TcxTextAlignY Index 2 read GetPartialTextAlignY
      write SetPartialTextAlignY default taCenterY;
    property RightTitle: TStrings Index 2 read GetPartialTitle
      write SetPartialTitle stored IsTitleStored;
    property TextAlignY[Index: TdxPageTitlePart]: TcxTextAlignY
      read GetTextAlignY write SetTextAlignY;
    property Titles[Index: TdxPageTitlePart]: TStrings read GetTitle
      write SetTitle; default;
  published
    property Background;
  end;
  ...
  protected
    ...
    procedure PageParamsChanged(AUpdateCodes
      : TdxPrinterPageUpdateCodes); virtual;
  ...
  public
    ...
    procedure GetRealMinMargins(var AMinLeft, AMinRight, AMinTop,
      AMinBottom: Integer);
    function IsEqual(ABaseObject: TdxBaseObject): Boolean; override;
    ...
    property RealMeasurementUnits: TdxMeasurementUnits
      read GetRealMeasurementUnits;
    property RealPageSize: TPoint read GetRealPageSize write SetRealPageSize;
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
    property OnMarginChange: TNotifyEvent read FOnMarginChange
      write FOnMarginChange;
  published
    property AutoSwapMargins: Boolean read FAutoSwapMargins
      write FAutoSwapMargins default True;
    property Background;
    ...
    property DMPaper: Integer read FDMPaper
      write SetDMPaper { default dxDefaultDMPaper };
    { DMPAPER_FIRST = DMPAPER_LETTER }
    property FitToPagesHorizontally: Integer read FFitToPagesHorizontally
      write SetFitToPagesHorizontally default 1;
    property FitToPagesVertically: Integer read FFitToPagesVertically
      write SetFitToPagesVertically default 0;
    ...
    property ImageCollection: TcxImageCollection read FImageCollection
      write SetImageCollection;
    ...
    property MinMargins: TdxRectWrapper read FMinMargins write SetMinMargins
      stored IsMinMarginsStored;
    property Orientation: TdxPrinterOrientation read FOrientation
    ...
    property ScaleFactor: Integer read FScaleFactor write SetScaleFactor
      default 100;
    property ScaleMode: TdxScaleMode read FScaleMode write SetScaleMode
      default smAdjust;
  end;

var
  dxPSPageTitlePartMap: array [0 .. 2] of TdxPageTitlePart = (
    tpLeft,
    tpCenter,
    tpRight
  );
...
function TCustomdxPageObject.GetPartialTextAlignY(Index: Integer)
  : TcxTextAlignY;
begin
...
function TCustomdxPageObject.GetTextAlignY(Index: TdxPageTitlePart)
  : TcxTextAlignY;
begin
...
begin
  ...
  begin
    for I := Low(TdxPageTitlePart) to High(TdxPageTitlePart) do
    begin
      Result := Result and Titles[I].Equals(TCustomdxPageObject(ABaseObject)
        .Titles[I]) and (FTextAlignY[I] = TCustomdxPageObject(ABaseObject)
        .FTextAlignY[I]);
    end;
  end;
end;
...
procedure TCustomdxPageObject.SetPartialTextAlignY(Index: Integer;
  Value: TcxTextAlignY);
begin
...
procedure TCustomdxPageObject.SetTextAlignY(Index: TdxPageTitlePart;
  Value: TcxTextAlignY);
begin
...
procedure TCustomdxPageObject.SetTitle(Index: TdxPageTitlePart;
  Value: TStrings);
begin
...
begin
  ...
  begin
    ...
    begin
      APaperInfo := TdxPaperInfo(dxPrnDev.dxPrintDevice.Papers.Objects
        [dxPrnDev.dxPrintDevice.PaperIndex]);
...
procedure TdxPrinterPage.GetRealMinMargins(var AMinLeft, AMinRight, AMinTop,
  AMinBottom: Integer);
begin
  AMinLeft := 0;
  AMinRight := 0;
  ...
  begin
    AMinLeft := MulDiv(dxPrintDevice.PhysOffsetX, 1000,
      GetDeviceCaps(dxPrintDevice.Handle, LOGPIXELSX));
    AMinTop := MulDiv(dxPrintDevice.PhysOffsetY, 1000,
      GetDeviceCaps(dxPrintDevice.Handle, LOGPIXELSY));
...
begin
  Result := inherited IsEqual(ABaseObject) and
    ...
    (ReverseTitlesOnEvenPages = TdxPrinterPage(ABaseObject)
    .ReverseTitlesOnEvenPages) and FMargins.IsEqual(TdxPrinterPage(ABaseObject)
    .Margins.Rect) and FMinMargins.IsEqual(TdxPrinterPage(ABaseObject)
    .MinMargins.Rect) and PageSize.IsEqual(TdxPrinterPage(ABaseObject)
    .PageSize.Point) and (MeasurementUnits = TdxPrinterPage(ABaseObject)
    .MeasurementUnits) and (Orientation = TdxPrinterPage(ABaseObject)
    .Orientation) and (DMPaper = TdxPrinterPage(ABaseObject).DMPaper) and
...
begin
  ...
  Result := Rect(Result.Left, RealPageSize.Y - Margins.Bottom,
    RealPageSize.X - Result.Right, RealPageSize.Y - Footer);
end;
...
begin
  ...
  Result := Rect(Result.Left, Header, RealPageSize.X - Result.Right,
    Margins.Top);
end;
...
begin
  case GetInnerMeasurementUnits of
    muInches:
      Result := cxRectScale(TdxInchesUnits.ToMM(MinMargins.Rect), 1,
        100, 1, 100);
  else { muMillimeters }
...
begin
  ...
  if dxPrnDev.dxIsPrintDeviceInitialized and (dxPrintDevice.PrinterCount > 0)
  then
...
procedure TdxPrinterPage.PageParamsChanged(AUpdateCodes
...
begin
  ...
  begin
    ...
    if CanSwapMargins and ((APrevOrientation = poLandscape) and
      (FOrientation in [poAuto, poPortrait]) or
      (APrevOrientation in [poPortrait, poAuto]) and
      (FOrientation = poLandscape)) then
      DoSwapMargins;
  end;
end;
...
begin
  ...
  if rsTop in ASides then
    Values[1] := MinMax(Values[1], FMinMargins.Top,
      RestPageSizeY - Margins.Bottom);
  if rsBottom in ASides then
    Values[3] := MinMax(Values[3], FMinMargins.Bottom,
      RestPageSizeY - Margins.Top);
end;

procedure TdxPrinterPage.LockUpdated(Sender: TdxBaseObject;
  ALockState: TdxLockState);
begin
  if ALockState = lsUnLock then
...
begin
  if IsLoading or (UpdateCount <> 0) then
    Exit;
  if rsLeft in ASides then
    Values[0] := MinMax(Values[0], FMinMargins.Left,
      RestPageSizeX - IfThen(rsRight in ASides, Values[2], Margins.Right));
  if rsTop in ASides then
    Values[1] := MinMax(Values[1], Header,
      RestPageSizeY - IfThen(rsBottom in ASides, Values[3], Margins.Bottom));
  if rsRight in ASides then
    Values[2] := MinMax(Values[2], FMinMargins.Right,
      RestPageSizeX - IfThen(rsLeft in ASides, Values[0], Margins.Left));
  if rsBottom in ASides then
    Values[3] := MinMax(Values[3], Footer,
      RestPageSizeY - IfThen(rsTop in ASides, Values[1], Margins.Top));
end;

procedure TdxPrinterPage.MinMarginsChanged(Sender: TObject;
  ASides: TdxRectSides);
begin
  if IsLoading or (UpdateCount <> 0) then
    Exit;
  ...
  begin
    ...
    FMargins.Right := MinMax(FMargins.Right, FMinMargins.Right,
      RestPageSizeX - FMargins.Left);
  end;
  ...
  begin
    ...
    FMargins.Left := MinMax(FMargins.Left, FMinMargins.Left,
      RestPageSizeX - FMargins.Right);
  end;
...
procedure TdxPrinterPage.MinMarginsChanging(Sender: TObject;
  ASides: TdxRectSides; var Values: array of Integer);
begin
  if IsLoading or (UpdateCount <> 0) then
    Exit;
  if rsLeft in ASides then
...
procedure TdxPrinterPage.PageSizeChanged(Sender: TObject;
...
procedure TdxPrinterPage.PageSizeChanging(Sender: TObject;
  ACoords: TdxPointCoords; var Values: array of Integer);
begin
  if IsLoading then
    Exit;
...
begin
  ...
  Footer := MinMax(Footer, FMinMargins.Bottom, RestPageSizeY - Header);
  FMargins.Left := MinMax(FMargins.Left, FMinMargins.Left,
    RestPageSizeX - FMinMargins.Right);
  FMargins.Right := MinMax(FMargins.Right, FMinMargins.Right,
    RestPageSizeX - FMargins.Left);
  FMargins.Top := MinMax(FMargins.Top, Header, RestPageSizeY - Footer);
  FMargins.Bottom := MinMax(FMargins.Bottom, Footer, RestPageSizeY - Header);
...
begin
  ...
  if RestPageSizeX = 0 then
    ...
    FMinMargins.Right := MinMax(FMinMargins.Right, 0,
      RestPageSizeX - FMinMargins.Left);

  FMinMargins.Top := MinMax(FMinMargins.Top, 0, RestPageSizeY);
  if RestPageSizeY = 0 then
    FMinMargins.Bottom := 0
  else
    FMinMargins.Bottom := MinMax(FMinMargins.Bottom, 0,
      RestPageSizeY - FMinMargins.Top);
end;
...
begin
  ...
  begin
    ...
    if PaperIndex <> -1 then
      PaperSize := ScalePoint
        (TdxPaperInfo(dxPrintDevice.Papers.Objects[PaperIndex]).Size, 100, 1)
  end
  else
