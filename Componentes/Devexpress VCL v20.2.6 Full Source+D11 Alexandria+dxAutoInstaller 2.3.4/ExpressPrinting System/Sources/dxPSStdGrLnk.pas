...
uses
  ...
  CheckLst, dxCore, dxPSRes, dxPSCore, dxPSGrLnks, dxPSReportRenderCanvas,
  dxPSGlbl, dxPSBaseGridLnk, cxDrawTextUtils,
  cxControls, cxContainer, cxEdit, cxLabel, cxCheckBox, Menus,
  cxLookAndFeelPainters, cxPC, cxButtons, cxTextEdit,
  cxGraphics, cxMaskEdit, cxDropDownEdit, cxColorComboBox, cxLookAndFeels,
  cxImageComboBox, dxPSReportLinkDesignWindow,
...
type
  ...
  published
    ...
    property OnCustomDrawCell: TdxCustomDrawTextCellEvent read FOnCustomDrawCell
      write FOnCustomDrawCell;
  ...
  TdxCustomDrawCellExEvent = procedure(Sender: TBasedxReportLink;
    ACol, ARow: Integer; ACanvas: TCanvas; AFont: TFont;
    ABoundsRect, AClientRect: TRect; var ADone: Boolean) of object;
  ...
  published
    ...
    property OnCustomDrawCell: TdxCustomDrawCellEvent read FOnCustomDrawCell
      write FOnCustomDrawCell;
    property OnCustomDrawCellEx: TdxCustomDrawCellExEvent
      read FOnCustomDrawCellEx write FOnCustomDrawCellEx;
    property OnGetCustomPageBreaks;
    property OnInitializeItem;
  end;
...
function TdxStringGridReportLink.GetCellTextAlignX(ACol, ARow: Integer)
  : TcxTextAlignX;
begin
...
function TdxStringGridReportLink.GetCellTextAlignY(ACol, ARow: Integer)
  : TcxTextAlignY;
begin
  if DrawMode <> gdmBorrowSource then
    Result := FTextAlignY
...
procedure TdxStringGridReportLink.DoCustomDrawCell(ACol, ARow: Integer;
...
begin
  if Assigned(FOnCustomDrawCell) then
    FOnCustomDrawCell(Self, ACol, ARow, ACanvas, ABoundsRect, AClientRect,
      AText, AFont, AColor, ATextAlignX, ATextAlignY, ADone);
end;
...
function TdxReportCustomDrawCellData.CustomDraw
...
function TdxDrawGridReportLink.GetDataItemClass(ACol: Integer;
...
begin
  ...
  begin
    GetCellColRow(AItem, ACol, ARow);
    FSaveFont.Assign(Font);
    DoCustomDrawCellEx(ACol, ARow, ACanvas, FSaveFont, ABoundsRect,
      AClientRect);
  end;
  ADone := True;
end;
...
begin
  ...
  if Assigned(FOnCustomDrawCellEx) then
    FOnCustomDrawCellEx(Self, ACol, ARow, ACanvas, AFont, ABoundsRect,
      AClientRect, ADone);
  if not ADone then
    DoCustomDrawCell(ACol, ARow, ACanvas, ABoundsRect, AClientRect);
end;

procedure TdxDrawGridReportLink.DoCustomDrawCell(ACol, ARow: Integer;
...
function TdxDrawGridReportLink.IsSupportedCustomDraw
  (Item: TAbstractdxReportCellData): Boolean;
begin
  Result := inherited IsSupportedCustomDraw(Item) and
    (Item is TdxReportCustomDrawCellData) and
...
begin
  ...
  chbxFixedRowsOnEveryPage.Caption :=
    cxGetResourceString(@sdxFixedRowOnEveryPage);
...
begin
  ...
  if lichbxRowAutoHeight.Visible then
    chbxRowAutoHeight.Checked := TdxStringGridReportLink(ReportLink)
      .RowAutoHeight;
...
procedure TdxGridReportLinkDesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
begin
  inherited;
...
procedure TdxGridReportLinkDesignWindow.CMDialogChar
...
begin
  ...
  begin
    if ReportLink.IsStringGridLink then
      TdxStringGridReportLink(ReportLink).RowAutoHeight :=
...
procedure TdxGridReportLinkDesignWindow.chbxUse3DEffectsClick(Sender: TObject);
begin
  if not LockControlsUpdate then
  begin
    ...
    with TcxCheckBox(Sender) do
      if Checked then
        ReportLink.Options := ReportLink.Options +
          [TdxGridPaintOption(TTagToInt(Tag))]
      else
        ReportLink.Options := ReportLink.Options -
          [TdxGridPaintOption(TTagToInt(Tag))];
    Modified := True;
...
procedure TdxGridReportLinkDesignWindow.stFixedTransparentClick
  (Sender: TObject);
begin
...
dxPSRegisterReportLink(TdxStringGridReportLink, TStringGrid,
  TdxGridReportLinkDesignWindow);
dxPSRegisterReportLink(TdxDrawGridReportLink, TDrawGrid,
  TdxGridReportLinkDesignWindow);

finalization

dxPSUnregisterReportLink(TdxDrawGridReportLink, TDrawGrid,
  TdxGridReportLinkDesignWindow);
dxPSUnregisterReportLink(TdxStringGridReportLink, TStringGrid,
  TdxGridReportLinkDesignWindow);
