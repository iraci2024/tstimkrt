...
uses
  Types, Windows, SysUtils, Classes, Graphics, Controls, Forms, ExtCtrls,
  StdCtrls,
  ...
  cxSpinEdit, cxGraphics, cxGeometry, cxLookAndFeels, dxLayoutControlAdapters,
...
uses
  // Variants,
  Math, dxCustomPreview, dxPSGlbl, dxPSImgs, dxPSRes, dxPSUtl, dxfmMnPg, dxCore,
  cxDrawTextUtils;
...
begin
  ...
  try
    ...
    if (APreview.Owner <> nil) and (APreview.Owner is TControl) then
      AForm.FPreviewOwnerSize := Point(TControl(APreview.Owner).Width,
        TControl(APreview.Owner).Height)
    else
    ...
    if Result then
    begin
      TdxPreviewAccess(APreview).SetPageXYCount(AForm.FPreview.PageXCount,
        AForm.FPreview.PageYCount);
    ...
    pzmNone:
      begin
        ...
        begin
          ...
          if (Control is TcxRadioButton) and TcxRadioButton(Control).Checked
          then
          begin
...
const
  cZoomFactors: array [0 .. 7] of Integer = (500, 200, 150, 100, 75,
    50, 25, 10);
var
  T: Longint;
begin
...
begin
  ...
  AMaxColCount := FPreview.Width div MulDiv(TdxPreviewAccess(FPreview)
    .PageSize.ActualSizeInPixels.X, FPreview.MinZoomFactor, 100);
  AMaxRowCount := FPreview.Height div MulDiv(TdxPreviewAccess(FPreview)
    .PageSize.ActualSizeInPixels.Y, 2 * FPreview.MinZoomFactor, 100);
...
const
  ...
  begin
    cxDrawImage(ACanvas, GetPreviewBounds, nil, ilMonitor, AImageIndex, True,
      nil, ScaleFactor);
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        APageBounds := cxRectScale(APageBounds, cxRectWidth(AScreenBounds),
          FPreview.Width, cxRectHeight(AScreenBounds), FPreview.Height);
        APageBounds := cxRectOffset(APageBounds, AScreenBounds.Left,
          AScreenBounds.Top);
...
const
  Format = CXTO_PATTERNEDTEXT or CXTO_CENTER_HORIZONTALLY or CXTO_WORDBREAK or
    CXTO_CHARBREAK;
var
  Template, S: string;
  I: Integer;
