  ...
  if (AEvent <> nil) and AEvent.Registered then
    try
      ...
      begin
        ...
        if Subscriber.Enabled and Subscriber.SupportsEventClass
          (AEvent.EventClass) then
        begin
          try
...
function TdxPSEventSystem.IndexOfEventClass(AEventClass: TdxEventClass)
  : Integer;
begin
...
begin
  if AEventClass <> nil then
    FEventClasses.Add(AEventClass, True);
end;
...
function TdxPSEventSystem.IndexOfSubscriber(ASubscriber
  : TdxEventSubscriber): Integer;
begin
...
procedure TdxPSEventSystem.UnregisterSubscriber(ASubscriber
  : TdxEventSubscriber);
begin
...
constructor TdxEventSubscriber.Create(const AEventClasses
  : array of TdxEventClass);
var
  I: Integer;
begin
  inherited Create;
...
begin
  if not SupportsEventClass(AEventClass) then
...
function TdxEventSubscriber.SupportsEventClass(AEventClass
  : TdxEventClass): Boolean;
begin
