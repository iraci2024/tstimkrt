...
uses
  ...
  cxColorComboBox, cxLabel, cxSpinEdit, cxButtons, cxPC, cxLookAndFeels,
...
function dxShowPSPreviewOptionsDlg(var AData: TdxPreviewOptionsDlgData)
  : Boolean;
...
begin
  ...
  tshGeneral.Caption := dxPSUtl.DropAmpersand
    (cxGetResourceString(@sdxPreferenceDlgTab1));
  lblMeasurementUnits.Caption := cxGetResourceString
  ...
  chbxShowMarginsHints.Caption := cxGetResourceString
    (@sdxPreferenceDlgMarginsHints);
  chbxShowMarginsHintsWhileDragging.Caption :=
    cxGetResourceString(@sdxPreferenceDlgMargingWhileDragging);
...
begin
  ...
  try
    ...
    chbxShowMarginsHintsWhileDragging.Checked :=
      FData.ShowMarginsHintWhileDragging;
...
begin
  ...
  if Result then
  begin
    FData.MeasurementUnits := TdxMeasurementUnits
      (cbxMeasurementUnits.ItemIndex);
