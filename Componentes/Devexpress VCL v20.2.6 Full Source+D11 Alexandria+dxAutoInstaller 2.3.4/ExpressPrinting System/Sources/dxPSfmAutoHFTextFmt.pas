...
uses
  ...
  ToolWin, Menus, dxCore, dxPSForm, dxPgsDlg, cxLookAndFeelPainters, cxButtons,
  cxControls, cxContainer, cxEdit,
  cxLabel, cxGroupBox, cxGraphics, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxListBox, cxGeometry, cxLookAndFeels,
  dxPSHFToolBarBld, dxLayoutcxEditAdapters, dxLayoutContainer,
...
type
  TdxfmAutoText = class(TCustomdxPSForm)
    ...
    procedure NewAutoTextKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  public
    ...
    property SelectedEntryIndex: Integer read FSelectedEntryIndex
      write SetSelectedEntryIndex;
  end;
...
uses
  Registry, Math, dxPSCore, dxPSRes, dxPSUtl, dxPsGlbl, dxPSESys, dxPSEvnt,
  dxPSHFLibrary, dxDPIAwareUtils;
...
procedure TdxfmAutoText.pbxPreviewPaint(Sender: TObject);
  ...
  procedure DrawPart(ACanvas: TcxCanvas; S: string; ARect: TRect;
    AFormat: LongInt);
  begin
...
procedure TdxfmAutoText.NewAutoTextKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_ESCAPE:
...
procedure TdxfmAutoText.DoInsertMacrosValue(Sender: TObject;
...
begin
  ...
  gbxEnterAutoTextEntriesHere.Caption :=
    cxGetResourceString(@sdxEnterAutoTextEntriesHere);
...
procedure TdxfmAutoText.cbxAutoEntriesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case Key of
    VK_HOME:
    ...
    VK_DOWN:
      SelectedEntryIndex := Min(SelectedEntryIndex + 1,
        cbxAutoEntriesList.Count - 1);
  end;
end;
