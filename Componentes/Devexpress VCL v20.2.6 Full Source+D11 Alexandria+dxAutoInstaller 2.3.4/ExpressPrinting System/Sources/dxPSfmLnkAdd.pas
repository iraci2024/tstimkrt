...
uses
  ...
  Menus, ImgList, dxCore, dxPSForm, dxPSCompsProvider, dxPSCore, cxControls,
  cxContainer, cxEdit, cxLabel,
  cxCheckBox, IniFiles, cxLookAndFeelPainters, cxButtons, cxTextEdit,
  cxListView, cxPC, cxGroupBox,
  cxGraphics, cxLookAndFeels, dxLayoutcxEditAdapters, dxLayoutControlAdapters,
  dxLayoutContainer,
  dxLayoutLookAndFeels, cxClasses, dxLayoutControl, cxImageList, cxMaskEdit,
  cxDropDownEdit,
...
type
  ...
  TdxAddReportLinkDlgDataOption = (adoShowDesignButton,
    adoShowDescriptionColumn, adoShowOnlyComponentsWOLinks,
    adoShowOnlyComponentsInActiveForm, adoAllowMultiSelect, adoShowNameEdit,
    adoShowCaptionEdit, adoShowHideCustomContainers, adoShowCreatorEdit,
    adoAllowSelectLinkClass);
  ...
  TdxfmAddComponent = class(TCustomdxPSForm)
    ...
    procedure cbReportLinkClassesPropertiesValidate(Sender: TObject;
  ...
  protected
    ...
    procedure LoadComponentState(AIniFile: TCustomIniFile;
      const ASectionName: string);
    procedure LoadListViewSorting(AIniFile: TCustomIniFile;
      const ASectionName: string);
    procedure SaveComponentState(AIniFile: TCustomIniFile;
      const ASectionName: string);
    procedure SaveListViewSorting(AIniFile: TCustomIniFile;
      const ASectionName: string);
  ...
  public
    ...
    property OriginalReportLinkName: string read FOriginalReportLinkName
      write FOriginalReportLinkName;
    property ReportLinkCaption: string read GetReportLinkCaption
      write SetReportLinkCaption;
    property ReportLinkCreator: string read GetReportLinkCreator
      write SetReportLinkCreator;
    property ReportLinkDescription: string read FReportLinkDescription
      write FReportLinkDescription;
    property ReportLinkName: string read GetReportLinkName
      write SetReportLinkName;
...
procedure TdxfmAddComponent.cbReportLinkClassesPropertiesChange
  (Sender: TObject);
var
...
procedure TdxfmAddComponent.lvItemsCompare(Sender: TObject;
...
procedure TdxfmAddComponent.lvItemsSelectItem(Sender: TObject; Item: TListItem;
  Selected: Boolean);
begin
  PopulateLinkClasses;
  UpdateControlsState;
...
procedure TdxfmAddComponent.LoadFromIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxfmAddComponent.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxfmAddComponent.LoadComponentState(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  chbxOnlyUnLinked.Checked := AIniFile.ReadBool(ASectionName,
    sdxOnlyWithoutLinks, chbxOnlyUnLinked.Checked);
  chbxOnlyInCurrentModule.Checked := AIniFile.ReadBool(ASectionName,
    sdxOnlyInCurrentModule, chbxOnlyInCurrentModule.Checked);
  chbxHideCustomContainers.Checked := AIniFile.ReadBool(ASectionName,
    sdxHideCustomContainers_, chbxHideCustomContainers.Checked);
end;

procedure TdxfmAddComponent.LoadListViewSorting(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  FSortedColumnIndex := AIniFile.ReadInteger(ASectionName, sdxSortedColumnIndex,
    FSortedColumnIndex);
  FSortOrder := TSortOrder(AIniFile.ReadInteger(ASectionName, sdxSortOrder,
    Integer(FSortOrder)));
end;

procedure TdxfmAddComponent.SaveListViewSorting(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxfmAddComponent.SaveComponentState(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  AIniFile.WriteBool(ASectionName, sdxOnlyWithoutLinks,
    chbxOnlyUnLinked.Checked);
  AIniFile.WriteBool(ASectionName, sdxOnlyInCurrentModule,
    chbxOnlyInCurrentModule.Checked);
  AIniFile.WriteBool(ASectionName, sdxHideCustomContainers_,
    chbxHideCustomContainers.Checked);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not Result then
        MessageError(Format(cxGetResourceString(@sdxComponentAlreadyExists),
          [ReportLinkName]));
    end
    else
      MessageError(Format(cxGetResourceString(@sdxInvalidComponentName),
        [ReportLinkName]));
...
begin
  ...
  lichbxOnlyInCurrentModule.Visible := adoShowOnlyComponentsInActiveForm
    in FOptions;
...
begin
  if (FComponentPrinter = nil) or
    not(csDesigning in FComponentPrinter.ComponentState) then
  ...
  chbxHideCustomContainers.Caption :=
    cxGetResourceString(@sdxHideCustomContainers);
  chbxOnlyInCurrentModule.Caption :=
    cxGetResourceString(@sdxOnlyComponentsInActiveForm);
  chbxOnlyUnLinked.Caption := cxGetResourceString
...
begin
  Result := [];
  if (adoShowOnlyComponentsWOLinks in FOptions) and chbxOnlyUnLinked.Checked
  then
    Include(Result, gcoExcludeExisting);
  if (adoShowOnlyComponentsInActiveForm in FOptions) and
    chbxOnlyInCurrentModule.Checked then
    Include(Result, gcoExcludeOutOfActiveForm);
  if (adoShowHideCustomContainers in FOptions) and chbxHideCustomContainers.Checked
  then
  ...
  try
    ...
    begin
      for I := 0 to AInfo.LinkClasses.Count - 1 do
        AItems.AddObject(TClass(AInfo.LinkClasses[I]).ClassName,
          TObject(AInfo.LinkClasses[I]));
      cbReportLinkClasses.ItemIndex :=
        AItems.IndexOfObject(TObject(AInfo.LinkClass));
    end;
    if cbReportLinkClasses.ItemIndex < 0 then
      cbReportLinkClasses.ItemIndex := 0;
    liReportLinkClasses.Visible := (AItems.Count > 1) and
      (adoAllowSelectLinkClass in FOptions);
  finally
    AItems.EndUpdate;
  end;
...
procedure TdxfmAddComponent.RefreshList;
  ...
  begin
    ...
    try
      dxPSComponentProvidersFactory.GetComponents(FComponentPrinter,
        FReportLink, AStrings, PreparedOptions);
      for I := 0 to AStrings.Count - 1 do
      begin
        AComponentItem := AStrings.Objects[I] as TdxComponentItem;
        if not FComponentInfo.TryGetValue(AComponentItem.Component,
          TObject(AItemInfo)) then
        begin
...
begin
  if (FSortedColumnIndex > -1) and (FSortedColumnIndex < lvItems.Columns.Count)
    and (FSortOrder <> soNone) then
...
begin
  ...
  else if edName.CanFocus then
  begin
    ActiveControl := edName;
    edName.SelectAll;
  end
  else if edCaption.CanFocus then
  begin
    ActiveControl := edCaption;
    edCaption.SelectAll;
  end
  else if edCreator.CanFocus then
  begin
...
procedure TdxfmAddComponent.SetColumnSortMark(AIndex: Integer;
...
procedure TdxfmAddComponent.SortColumn(Column: TListColumn;
  ASortOrder: TSortOrder);
var
  AItem: TListItem;
begin
...
begin
  if FLinkClass <> nil then
    Result := FLinkClass
  else if LinkClasses.Count = 1 then
