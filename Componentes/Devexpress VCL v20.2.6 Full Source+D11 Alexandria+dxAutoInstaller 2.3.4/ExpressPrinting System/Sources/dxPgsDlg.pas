...
{$ENDIF}
  ...
  Buttons, Menus, ToolWin, Registry, ImgList, dxMessages, cxClasses,
  dxCustomPreview, dxPSESys, dxPSForm, dxBkgnd,
  dxPreVw, dxPSGlbl, dxfmClr, dxPrnPg, dxPrnDev, dxExtCtrls,
  cxLookAndFeelPainters, cxButtons, cxRadioGroup, cxPC,
  cxControls, cxContainer, cxEdit, cxCheckBox, cxLabel, cxTextEdit, cxMemo,
  cxListBox, cxGraphics, cxMaskEdit,
  cxDropDownEdit, cxGroupBox, cxSpinEdit, cxDrawTextUtils, dxCore, cxGeometry,
  cxLookAndFeels, IniFiles,
  dxPSHFToolBarBld, dxLayoutContainer, dxLayoutControl, dxLayoutLookAndFeels,
...
type
  ...
  TdxPageSetupDlgOption = (psoCenterOnPage, psoMargins, psoPageOrder,
    psoShading, psoStyleCaption, psoHFAutoText, psoHFBackground, psoHFFont,
    psoHFText, psoHFFunctions, psoHFMargins, psoHFReverse, psoHFVertAlignment);
...
type
  ...
  protected
    ...
    procedure PageParamsChanged(AUpdateCodes
      : TdxPrinterPageUpdateCodes); dynamic;
  ...
  published
    property AllowChangeHFText: Boolean read GetAllowChangeHFText
      write SetAllowChangeHFText default True;
    property AllowChangeMargins: Boolean read GetAllowChangeMargins
      write SetAllowChangeMargins default True;
    property AllowChangeOrientation: Boolean read GetAllowChangeOrientation
      write SetAllowChangeOrientation default True;
    property AllowChangePaper: Boolean read GetAllowChangePaper
      write SetAllowChangePaper default True;
    property AllowChangeScale: Boolean read GetAllowChangeScale
      write SetAllowChangeScale default True;
    property AllowCustomPaperSizes: Boolean read GetAllowCustomPaperSizes
      write SetAllowCustomPaperSizes default True;
    property Description: string read GetDescription write SetDescription
      stored IsDescriptionStored;
    property ImageIndex: Integer read FImageIndex write SetImageIndex
      default -1;
    property Index: Integer read GetIndex write SetIndex stored False;
    property IsCurrentStyle: Boolean read GetIsCurrentStyle
      write SetIsCurrentStyle stored False;
    property PrinterPage: TdxPrinterPage read FPrinterPage write SetPrinterPage;
    property ShowPageSetupDlg: Boolean read FShowPageSetupDlg
      write FShowPageSetupDlg stored False;
    property StyleCaption: string read GetStyleCaption write SetStyleCaption
      stored IsStyleCaptionStored;
    property StyleGlyph: TBitmap read GetStyleGlyph write SetStyleGlyph
      stored IsStyleGlyphStored; { 32 x 32 }

    property OnDestroy: TNotifyEvent read FOnDestroy write FOnDestroy;
    property OnFilterPaper: TdxFilterPaperEvent read FOnFilterPaper
      write FOnFilterPaper;
  end;
  ...
  private
    ...
    procedure SetNameHandler(Reader: TReader; Component: TComponent;
      var Name: string);
  ...
  public
    ...
    function AddStyleEx(AStyleClass: TdxPrintStyleClass; AOwner: TComponent)
      : TBasedxPrintStyle;
    ...
    procedure DefinePrintStylesDlg(out APreviewBtnClicked, APrintBtnClicked
      : Boolean); overload;
    ...
    property CurrentStyleIndex: Integer read GetCurrentStyleIndex
      write SetCurrentStyleIndex;
    property Designer: TAbstractdxStyleManagerDesigner read FDesigner;
    { accessible only in DesignTime }
    ...
    property Styles[Index: Integer]: TBasedxPrintStyle read GetStyle
      write SetStyle; default;
    property UpdateCount: Integer read FUpdateCount;
  published
    property AutoHFTextEntries: TStrings read FAutoHFTextEntries
      write SetAutoHFTextEntries stored IsAutoHFTextEntriesStored;
    property AutoSave: Boolean read FAutoSave write FAutoSave default False;
    property CloneStyleCaptionPrefix: string read GetCloneStyleCaptionPrefix
      write SetCloneStyleCaptionPrefix stored IsCloneStyleCaptionPrefixStored;
    property CurrentStyle: TBasedxPrintStyle read FCurrentStyle
      write SetCurrentStyle;
    property HelpContext: THelpContext read FHelpContext write FHelpContext
      default 0;
    property Images: TImageList read FImages write SetImages;
    property PageSetupDialog: TdxPageSetupDialog read FPageSetupDialog
      write SetPageSetupDialog;
    ...
    property OnChangeCurrentStyle: TNotifyEvent read FOnChangeCurrentStyle
      write FOnChangeCurrentStyle;
    property OnStyleListChanged: TNotifyEvent read FOnStyleListChanged
      write FOnStyleListChanged;
  end;
  ...
  TdxCustomDrawPreviewEvent = procedure(APrintStyle: TBasedxPrintStyle;
    ACanvas: TCanvas; APageRect, AContentRect, AHeaderRect, AFooterRect: TRect)
    of object;
  ...
  published
    property ActivePageIndex: Integer read FActivePageIndex
      write FActivePageIndex default 0;
    property ButtonsEnabled: TdxPageSetupDlgButtons read FButtonsEnabled
      write FButtonsEnabled default [psbStyleOptions, psbPreview, psbPrint];
    property ButtonsVisible: TdxPageSetupDlgButtons read FButtonsVisible
      write FButtonsVisible default [psbStyleOptions, psbPreview, psbPrint];
    property HelpContext: THelpContext read FHelpContext write FHelpContext
      default 0;
    property HFMode: TdxHFMode read FHFMode write FHFMode
      default hfmThreeSections;
    property OptionsEnabled: TdxPageSetupDlgOptions read FOptionsEnabled
      write FOptionsEnabled default [Low(TdxPageSetupDlgOption)
      .. High(TdxPageSetupDlgOption)]; { psoDefaultOptionsEnabled }
    property OptionsVisible: TdxPageSetupDlgOptions read FOptionsVisible
      write FOptionsVisible default psoDefaultOptionsVisible;
  ...
  private
    ...
    procedure DrawBackgroundBitmapContent(ACanvas: TcxCanvas; R: TRect;
    ...
    procedure PreviewAfterDragMargin(Sender: TObject;
    ...
    property MarginsInvalid: Boolean read FMarginsInvalid
      write SetMarginsInvalid;
    property MarginsOutside: Boolean read FMarginsOutside
      write SetMarginsOutside;
    ...
    property SelectedTitlePartEditor: TcxCustomMemo
      read GetSelectedTitlePartEditor;
    property SelectedTitlePartTextAlignY: TcxTextAlignY
      read GetSelectedTitlePartTextAlignY write SetSelectedTitlePartTextAlignY;
...
function GetFormatedDate(const ADateTime: TDateTime;
  const AFormat: string): string;
function GetFormatedTime(const ADateTime: TDateTime;
  const AFormat: string): string;
...
procedure dxPSDrawStyleItem(AStyle: TBasedxPrintStyle; AListBox: TListBox;
  ...
  AMultiline, ABoldedCurrent: Boolean; AScaleFactor: TdxScaleFactor);
procedure dxPSDefaultDrawPagePreview(APrintStyle: TBasedxPrintStyle;
  ACanvas: TCanvas; APageBounds, AContentBounds, AHeaderBounds,
  AFooterBounds: TRect; AScaleFactor: TdxScaleFactor);
function dxPSPrintStyleUniqueName(AStyleController: TdxPrintStyleManager;
...
begin
  Result := False;
  if APageSetupDlgData.PrintStyle = nil then
    Exit;
...
function dxGetPreviewPaperOrientation(APrinterOrientation
  : TdxPrinterOrientation): TdxPreviewPaperOrientation;
begin
...
const
  uFormat: array [Boolean] of UINT = (DT_LEFT or DT_WORDBREAK,
...
begin
  ...
  AImageRect := cxRectCenter(AImageRect, dxGetImageSize(AStyle.StyleGlyph,
    AImages, AStyle.ImageIndex, AScaleFactor));

  cxDrawImage(AListBox.Canvas.Handle, AImageRect, AImageRect, AStyle.StyleGlyph,
    AImages, AStyle.ImageIndex, idmNormal, True);
  ...
  begin
    R := ABounds;
    cxDrawText(AListBox.Canvas.Handle, AListBox.Items[Index], R,
      DT_LEFT or DT_SINGLELINE or DT_CALCRECT);
    cxDrawText(AListBox.Canvas, AListBox.Items[Index], ABounds,
      uFormat[cxRectWidth(R) <= cxRectWidth(ABounds)]);
  end
  else
...
begin
  dxDrawDefaultPagePreview(ACanvas, AContentBounds,
    APrintStyle.PrinterPage.CenterOnPageH,
    APrintStyle.PrinterPage.CenterOnPageV, AScaleFactor);
end;
...
begin
  ...
  if AStyleController.IsDesigning then
    raise EdxException.CreateFmt(sdxCantCreateUniqueName,
...
begin
  ...
  Filer.DefineProperty('IsTitleAssigned', ReadIsTitleAssigned,
    WriteIsTitleAssigned, FIsTitleAssigned and (Title = ''));
end;

procedure TdxPageSetupDialog.Notification(AComponent: TComponent;
...
procedure TdxfmPageSetupDialog.SetSelectedTitlePartTextAlignY
  (AAlign: TcxTextAlignY);
begin
  SelectedTitle.TextAlignY[SelectedTitlePart] := AAlign;
end;

procedure TdxfmPageSetupDialog.SetupDialog(const APageSetupDlgData
  : TdxPageSetupDlgData);
  ...
  begin
    AEdit.Properties.DisplayFormat := DisplayFormatMap
      [AEdit.Properties.ValueType] + ALegendText;
  end;
  ...
  begin
    ...
    begin
      sePaperWidth.Properties.MinValue := dxPrintDevice.MinExtentY /
        Denominator;
      sePaperWidth.Properties.MaxValue := dxPrintDevice.MaxExtentY /
        Denominator;
      sePaperHeight.Properties.MinValue := dxPrintDevice.MinExtentX /
        Denominator;
      sePaperHeight.Properties.MaxValue := dxPrintDevice.MaxExtentX /
        Denominator;
    end
    else
    begin
      sePaperWidth.Properties.MinValue := dxPrintDevice.MinExtentX /
        Denominator;
      sePaperWidth.Properties.MaxValue := dxPrintDevice.MaxExtentX /
        Denominator;
      sePaperHeight.Properties.MinValue := dxPrintDevice.MinExtentY /
        Denominator;
      sePaperHeight.Properties.MaxValue := dxPrintDevice.MaxExtentY /
        Denominator;
    end;
  ...
  begin
    ...
    if libtnHeaderBackground.Visible then
      btnHeaderBackground.Enabled :=
        (psoHFBackground in APageSetupDlgData.OptionsEnabled);

    if pnlHeaderMemos.Visible then
    begin
      pnlHeaderMemos.Enabled := PrintStyle.AllowChangeHFText and
  ...
  begin
    ...
    begin
      tbPredefined.Enabled := psoHFFunctions
        in APageSetupDlgData.OptionsEnabled;
      if FStyleManager = nil then
        AutoHFTextEntries := nil
      else
        AutoHFTextEntries := FStyleManager.AutoHFTextEntries;

      FHFToolBarHelper.Build(FHFFunctionList, AutoHFTextEntries,
        PrintStyle.PrinterPage.ImageCollection, Assigned(FStyleManager) and
        (psoHFAutoText in APageSetupDlgData.OptionsVisible));
      SetControlLookAndFeel(tbPredefined, dxPSEngine.DialogsLookAndFeel);
    end;

    if gbxVertAlignment.Visible then
      gbxVertAlignment.Enabled := psoHFVertAlignment
        in APageSetupDlgData.OptionsEnabled;

    if pnlReverse.Visible then
      chbxReverseOnEvenPages.Enabled := psoHFReverse
        in APageSetupDlgData.OptionsEnabled;
  end;
  ...
  var
    AIndex: Integer;
  begin
    AIndex := Min(Max(APageSetupDlgData.ActivePageIndex, 0),
      pgctrlMain.Count - 1);
  ...
  procedure UpdateControlEnableState(AControl: TdxLayoutGroup;
    AEnabled: Boolean);
  var
    I: Integer;
  begin
  ...
  begin
    lichbxCenterHorz.Visible := psoCenterOnPage
      in APageSetupDlgData.OptionsVisible;
    lichbxCenterVert.Visible := psoCenterOnPage
      in APageSetupDlgData.OptionsVisible;
    lilblCenterOnPage.Visible := psoCenterOnPage
      in APageSetupDlgData.OptionsVisible;
    ...
    libtnHeaderBackground.Visible := psoHFBackground
      in APageSetupDlgData.OptionsVisible;
    pnlHeaderFont.Visible := libtnHeaderFont.Visible or
      libtnHeaderBackground.Visible;
    pnlHeaderMemos.Visible := (psoHFText in APageSetupDlgData.OptionsVisible);
    if pnlHeaderMemos.Visible and (APageSetupDlgData.HFMode = hfmOneSection)
    then
    begin
    ...
    liedFooterFontInfo.Visible :=
      (psoHFFont in APageSetupDlgData.OptionsVisible);
    libtnFooterBackground.Visible :=
      (psoHFBackground in APageSetupDlgData.OptionsVisible);

    pnlFooterFont.Visible := libtnHeaderFont.Visible or
      libtnHeaderBackground.Visible;
    pnlFooterMemos.Visible := (psoHFText in APageSetupDlgData.OptionsVisible);

    if pnlFooterMemos.Visible and (APageSetupDlgData.HFMode = hfmOneSection)
    then
    begin
    ...
    litbPredefined.Visible :=
      (psoHFFunctions in APageSetupDlgData.OptionsVisible);
    gbxFunctions.Visible := litbPredefined.Visible;
    gbxVertAlignment.Visible :=
      (psoHFVertAlignment in APageSetupDlgData.OptionsVisible);
    ...
    tshHeaderFooter.Visible := pnlHeader.Visible or pnlFooter.Visible or
      pnlBottom.Visible or pnlReverse.Visible;
    tshScaling.Visible := PrintStyle.AllowChangeScale;

    pnlStyleName.Enabled := pnlStyleName.Visible and
      (not FSavePrintStyle.BuiltIn or IsDesignTime);
    ...
    begin
      gbxPrintOrder.Enabled :=
        (psoPageOrder in APageSetupDlgData.OptionsEnabled);
    ...
    begin
      chbxCenterHorz.Enabled := psoCenterOnPage
        in APageSetupDlgData.OptionsEnabled;
      chbxCenterVert.Enabled := psoCenterOnPage
        in APageSetupDlgData.OptionsEnabled;
      lblCenterOnPage.Enabled := psoCenterOnPage
        in APageSetupDlgData.OptionsEnabled;
    end;
  end;
  ...
  begin
    ...
    pnlHFMargins.Visible := gbxMargins.Visible and
      (psoHFMargins in APageSetupDlgData.OptionsVisible);
    ...
    if gbxMargins.Enabled then
      FPreview.OptionsBehavior := FPreview.OptionsBehavior +
        [pobAllowDragMargins]
    else
      FPreview.OptionsBehavior := FPreview.OptionsBehavior -
        [pobAllowDragMargins];
...
procedure TdxfmPageSetupDialog.DoInsertMacrosValue(Sender: TObject;
  const AMacrosValue: string);
begin
...
procedure TdxfmPageSetupDialog.DrawBackgroundBitmapContent(ACanvas: TcxCanvas;
  R: TRect; ABackground: TdxBackground);

  procedure DrawText(ACanvas: TcxCanvas; const S: string; const R: TRect;
  ...
  procedure DrawByBrush(ACanvas: TcxCanvas; B: HBRUSH; R: TRect;
    APainter: TcxCustomLookAndFeelPainter);
  begin
  ...
  begin
    ...
    case ABackground.Mode of
      bmNone:
        DrawText(ACanvas, '[' + DropAmpersand(cxGetResourceString(@sdxBtnNoFill)
          ) + ']', R, APainter);

      bmPicture:
        DrawText(ACanvas, '[' + DropAmpersand(cxGetResourceString(@sdxPicture))
          + ']', R, APainter);

      bmBrushBitmap:
        DrawByBrush(ACanvas, CreatePatternBrush(TBitmap(ABackground.Picture)
          .Handle), R, APainter);

      bmBrush:
        begin
          PrevColor := SetBkColor(ACanvas.Handle,
            ColorToRGB(ABackground.BkColor));
          DrawByBrush(ACanvas,
            CreateSolidBrush(ColorToRGB(ABackground.Brush.Color)), R, APainter);
...
begin
  ...
  try
    ...
    ARect := cxRectSetWidth(R, cxRectWidth(R) -
      ScaleFactor.Apply(FBmpArrow.Width));
    ...
    APainter.DrawArrow(ACanvas, ARect, adDown,
      APainter.ButtonSymbolColor(cxbsNormal));
  finally
...
begin
  ...
  AGlyph.SetSize(ScaleFactor.Apply(75), AButton.Height - ScaleFactor.Apply(6));
  { 75 = TextArea width + DownArrow width }

  ACanvas := TcxCanvas.Create(AGlyph.Canvas);
  try
    DrawBackgroundBitmapContent(ACanvas, Rect(0, 0, AGlyph.Width,
      AGlyph.Height), ABackground);
  finally
    ACanvas.Free;
  end;
...
begin
  ...
  FOrientationPreview.OptionsHint := FOrientationPreview.OptionsHint -
    [pohShowForMargins, pohShowOnDrag];
  FOrientationPreview.OptionsView := FOrientationPreview.OptionsView -
    [povPageSelection, povMargins];
  FOrientationPreview.OptionsZoom := FOrientationPreview.OptionsZoom -
    [pozZoomOnClick];
  TdxPreviewAccess(FOrientationPreview).PageSize.MinUsefulSize := cxNullPoint;
  TdxPreviewAccess(FOrientationPreview).OnCalcPageCount :=
    OrientationPreviewCalcPageCount;
...
begin
  ...
  btnRestoreOriginalMargins.Caption :=
    cxGetResourceString(@sdxBtnRestoreOriginal);
  ...
  lblPercentOfNormalSize.Caption :=
    cxGetResourceString(@sdxPercentOfNormalSize);
...
begin
  if not FControlsUpdating then
  begin
    Page.RealPageSize := Point(Round(1000 * sePaperWidth.Value),
      Round(1000 * sePaperHeight.Value));
    TdxPreviewAccess(FPreview).PageSize.Size := Page.PageSizeLoMetric;
    TdxPreviewAccess(FOrientationPreview).PageSize.Size :=
      Page.PageSizeLoMetric;
    UpdateMarginsBounds;
...
begin
  ...
  try
    ...
    btnOptions.Enabled := FCanShowStyleOptionsDlg and
      PrintStyle.HasStyleOptionsDlg;
...
begin
  ...
  if (ASelectedMemo <> nil) and (ASelectedMemo.Tag >= 3) and
    (ASelectedMemo.Tag < 6) then
...
begin
  ...
  else if ActiveControl is TcxCustomInnerMemo then
    Result := GetInnerControlContainer(ActiveControl) as TcxCustomMemo
  else
    Result := nil;
end;

function TdxfmPageSetupDialog.GetTitlePart(AComponent: TComponent)
  : TdxPageTitlePart;
var
  ATag: Integer;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        for I := 0 to PaperInfoCount - 1 do
          if PrintStyle.PrinterPage.DMPaper = PaperInfos[I].DMPaper then
          // TODO: FindByDMPaper
          begin
            lbxPaperType.ItemIndex := I;
            Break;
        ...
        if lbxPaperType.ItemIndex = -1 then
          ...
          begin
            ...
            while (I < lbxPaperType.Items.Count) and
              (Pos('Custom', lbxPaperType.Items[I]) = 0) do
              Inc(I);
  ...
  begin
    ...
    try
      ...
      if cbxPaperSource.Enabled then
        cbxPaperSource.ItemIndex :=
          Max(0, cbxPaperSource.Properties.Items.IndexOfObject
          (TObject(Page.PaperSource)));
    finally
...
begin
  ...
  try
    ...
    UpdateMarginsEdits;
    FPreview.MeasurementUnits := TdxPreviewMeasurementUnits
      (Page.MeasurementUnits);
    TdxPreviewAccess(FPreview).PageSize.MinUsefulSize :=
      Point(Page.MinPrintableAreaLoMetric, Page.MinPrintableAreaLoMetric);
    TdxPreviewAccess(FPreview).PageSize.Orientation :=
      dxGetPreviewPaperOrientation(Page.Orientation);
    ...
    FOrientationPreview.MeasurementUnits := TdxPreviewMeasurementUnits
      (Page.MeasurementUnits);
    TdxPreviewAccess(FOrientationPreview).PageSize.Orientation :=
      dxGetPreviewPaperOrientation(Page.Orientation);

    chbxCenterHorz.Checked := PrintStyle.PrinterPage.CenterOnPageH;
    chbxCenterVert.Checked := PrintStyle.PrinterPage.CenterOnPageV;
    rbtnDownThenOver.Checked :=
      (PrintStyle.PrinterPage.PageOrder = poDownThenOver);
    rbtnOverThenDown.Checked :=
      (PrintStyle.PrinterPage.PageOrder = poOverThenDown);
  ...
  else if pgctrlMain.Index = 0 then
  begin
    if lbxPaperType.CanFocus then
      ActiveControl := lbxPaperType
  end
  else if pgctrlMain.Index = 1 then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      seMarginHeader.Properties.MaxValue :=
        (RealPageSize.Y - MinPrintableArea - MinMargins.Bottom) / 1000;
      seMarginFooter.Properties.MinValue := MinMargins.Bottom / 1000;
      seMarginFooter.Properties.MaxValue :=
        (RealPageSize.Y - MinPrintableArea - MinMargins.Top) / 1000;
      seMarginTop.Properties.MinValue := MinMargins.Top / 1000;
      seMarginTop.Properties.MaxValue :=
        (RealPageSize.Y - MinPrintableArea - MinMargins.Bottom) / 1000;
      seMarginBottom.Properties.MinValue := MinMargins.Bottom / 1000;
      seMarginBottom.Properties.MaxValue :=
        (RealPageSize.Y - MinPrintableArea - MinMargins.Top) / 1000;
      seMarginLeft.Properties.MinValue := MinMargins.Left / 1000;
      seMarginLeft.Properties.MaxValue :=
        (RealPageSize.X - MinPrintableArea - MinMargins.Right) / 1000;
      seMarginRight.Properties.MinValue := MinMargins.Right / 1000;
      seMarginRight.Properties.MaxValue :=
        (RealPageSize.X - MinPrintableArea - MinMargins.Left) / 1000;
    end;
  finally
    if not APrevValue then
      FControlsUpdating := False;
...
begin
  ...
  begin
    ...
    UpdateWarningPane(MarginsInvalid, MarginsOutside,
      cxGetResourceString(@sdxInvalidMargins),
      cxGetResourceString(@sdxOutsideMargins));
...
begin
  if FControlsUpdating or FMarginsChanging then
    Exit;
...
begin
  ...
  begin
    ...
    else if TTagToInt(GetInnerControlContainer(ActiveControl).Tag) < 4 then
    begin
      if Part1 <> '' then
        memHeaderLeft.SelText := Part1;
      if Part2 <> '' then
        memHeaderCenter.SelText := Part2;
      if Part3 <> '' then
        memHeaderRight.SelText := Part3;
    end
    else
    begin
      if Part1 <> '' then
        memFooterLeft.SelText := Part1;
      if Part2 <> '' then
        memFooterCenter.SelText := Part2;
      if Part3 <> '' then
        memFooterRight.SelText := Part3;
    end;
    CheckModified;
  end;
end;
...
begin
  if ActiveControl is TcxCustomInnerMemo then
  begin
    SelectedTitlePartTextAlignY :=
      TcxTextAlignY(TTagToInt(TComponent(Sender).Tag));
    CheckModified;
  end;
end;
...
begin
  if FControlsUpdating then
    Exit;
  PrintStyle.PrinterPage.ReverseTitlesOnEvenPages :=
  ...
  begin
    ...
    if povMargins in OptionsView then
      if Assigned(FOnCustomDrawPreview) then
        FOnCustomDrawPreview(PrintStyle, ACanvas, ABounds, AContentBounds,
          AHeaderBounds, AFooterBounds)
      else
        dxPSDefaultDrawPagePreview(PrintStyle, ACanvas, ABounds, AContentBounds,
          AHeaderBounds, AFooterBounds, Self.ScaleFactor);
  end;
end;
...
begin
  ...
  if (AMargin is TdxPreviewPageMarginFooter) or
    (AMargin is TdxPreviewPageMarginHeader) then
    FMarginsChanging := True;
...
begin
  ...
  try
    cxDrawImage(cxPaintCanvas, TPaintBox(Sender).ClientRect, nil, ilPrintOrders,
      ImageIndexes[rbtnDownThenOver.Checked], gbxPrintOrder.Enabled, nil,
      ScaleFactor);
  finally
...
begin
  if not FControlsUpdating then
  begin
    PrintStyle.PrinterPage.PageOrder :=
      TdxPageOrder(TTagToInt(TcxRadioButton(Sender).Tag));
...
var
  ...
  APrevPreviewPaperOrientation, APreviewPaperOrientation
    : TdxPreviewPaperOrientation;
begin
  if FControlsUpdating or FOrientationChanging then
    Exit;

  FOrientationChanging := True;
  try
    APrinterOrientation := TdxPrinterOrientation
      (TTagToInt(TComponent(Sender).Tag));
    APreviewPaperOrientation := dxGetPreviewPaperOrientation
      (APrinterOrientation);
    if not Page.AutoSwapMargins then
    begin
      FPreview.OptionsBehavior := FPreview.OptionsBehavior -
        [pobAutoSwapMargins];
      FOrientationPreview.OptionsBehavior := FOrientationPreview.OptionsBehavior
        - [pobAutoSwapMargins];
    end;
    APrevPreviewPaperOrientation := TdxPreviewAccess(FPreview)
      .PageSize.Orientation;
    TdxPreviewAccess(FPreview).PageSize.Orientation := APreviewPaperOrientation;
    TdxPreviewAccess(FOrientationPreview).PageSize.Orientation :=
      APreviewPaperOrientation;
    SaveMargins;
...
begin
  if FPaperSizeLocked then
    Exit;

  PaperInfo := CurrentPaperInfo;
  TdxPreviewAccess(FPreview).PageSize.Size :=
    Point(PaperInfo.Width, PaperInfo.Height);
  TdxPreviewAccess(FOrientationPreview).PageSize.Size :=
    Point(PaperInfo.Width, PaperInfo.Height);
...
procedure TdxfmPageSetupDialog.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Key = VK_ESCAPE) and cxShiftStateMoveOnly(Shift) and
    (ActiveControl is TcxCustomInnerMemo) then
...
begin
  ...
  begin
    ...
    if (StyleManager <> nil) and (StyleManager.PageSetupDialog <> nil) then
      ...
      Caption := cxGetResourceString(@sdxPageSetupCaption) + ': ' +
        PrintStyle.StyleCaption;
...
function TdxfmPageSetupDialog.ValidateUserInput(out AControl
  : TWinControl): Boolean;
var
...
begin
  ...
  if not Result then
  begin
    MessageWarning(Format(cxGetResourceString(@sdxInvalideStyleCaption),
      [edStyleName.Text]));
  ...
  begin
    ...
    begin
      Result := True;
      Beep;
      IsFixupMarginsOutside := MarginsOutsideMessageDlg
        (cxGetResourceString(@sdxOutsideMarginsMessage2)) = mrYes;
      if IsFixupMarginsOutside then
        FixupMarginsOutside;
    end;
    ...
    begin
      ...
      begin
        Beep;
        Result := True;
        case MarginsMessageDlg
          (cxGetResourceString(@sdxInvalidMarginsMessage)) of
          mrYes:
            FixupMargins;
          mrNo:
...
function TdxfmPageSetupDialog.ValidateMarginsOutside(out AInvalidMarginControl
  : TWinControl): Boolean;
var
  AMin: TRect;
begin
...
function TdxfmPageSetupDialog.ValidateMargins(out AInvalidMarginControl
  : TWinControl): Boolean;
...
procedure TdxfmPageSetupDialog.FixupMarginsOutside;
  ...
  function SetSpinEditValidValue(AEdit: TcxSpinEdit; var AValue: Integer;
    const ADenominator: Integer): Boolean;
  begin
    Result := False;
...
begin
  ...
  begin
    if (ActiveControl is TcxCustomInnerTextEdit) or
      (ActiveControl is TcxCustomEdit) then
...
function TdxPrintStylePrinterPage.IsPageFooterTitleStored
  (Index: Integer): Boolean;
var
  Part: TdxPageTitlePart;
begin
  Part := dxPSPageTitlePartMap[Index];
  Result := PageFooter.Titles[Part].Text <>
    PrintStyle.DefaultPageFooterText(Part);
end;

function TdxPrintStylePrinterPage.IsPageHeaderTitleStored
  (Index: Integer): Boolean;
var
  Part: TdxPageTitlePart;
begin
  Part := dxPSPageTitlePartMap[Index];
  Result := PageHeader.Titles[Part].Text <>
    PrintStyle.DefaultPageHeaderText(Part);
end;

procedure TdxPrintStylePrinterPage.PageParamsChanged(AUpdateCodes
  : TdxPrinterPageUpdateCodes);
begin
  inherited;
...
begin
  ...
  Filer.DefineProperty('IsDescriptonAssigned', ReadIsDescriptionAssigned,
    WriteIsDescriptionAssigned, FIsDescriptionAssigned and (Description = ''));
  Filer.DefineProperty('IsStyleCaptionAssigned', ReadIsStyleCaptionAssigned,
    WriteIsStyleCaptionAssigned, FIsStyleCaptionAssigned and
    (StyleCaption = ''));
  Filer.DefineProperty('IsStyleGlyphAssigned', ReadIsStyleGlyphAssigned,
...
function TBasedxPrintStyle.DefaultPageFooterText
  (APart: TdxPageTitlePart): string;
begin
  Result := '';
end;

function TBasedxPrintStyle.DefaultPageHeaderText
  (APart: TdxPageTitlePart): string;
begin
  Result := '';
end;
...
function TBasedxPrintStyle.IsSupportedPaper(const APaper: TdxPaperInfo)
  : Boolean;
begin
  Result := True;
...
procedure TBasedxPrintStyle.PageParamsChanged(AUpdateCodes
  : TdxPrinterPageUpdateCodes);
begin
  if StyleManager <> nil then
...
begin
  Result := FIsStyleGlyphAssigned and not dxPSUtl.dxAreGraphicsEqual
    (FStyleGlyph, DefaultStyleGlyph);
end;
...
begin
  if StyleManager <> nil then
    TdxPrinterPageAccess(PrinterPage).PixelsPerInch :=
      StyleManager.PixelsPerInch;
end;
...
begin
  if Source is TdxPrintStyleManager then
    ...
    begin
      ...
      Self.FIsCloneStyleCaptionPrefixAssigned :=
        FIsCloneStyleCaptionPrefixAssigned;
      Self.FIsTitleAssigned := FIsTitleAssigned;
    end
  else
    inherited;
...
procedure TdxPrintStyleManager.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited;
...
begin
  ...
  Filer.DefineProperty('IsCloneStyleCaptionPrefixAssigned',
    ReadIsCloneStyleCaptionPrefixAssigned,
    WriteIsCloneStyleCaptionPrefixAssigned,
    FIsCloneStyleCaptionPrefixAssigned and (CloneStyleCaptionPrefix = ''));
end;

procedure TdxPrintStyleManager.GetChildren(Proc: TGetChildProc;
  Root: TComponent);
var
  I: Integer;
...
begin
  ...
  begin
    Style := Styles[I];
    if (Root = Style.Owner) or
      (FInternalStreaming and (Root = Style.StyleManager)) then
      Proc(Style);
  end;
end;
...
begin
  try
    S := AutoHFTextEntries
      [dxPSAutoHFTextMenuBuilderFactory.ActiveBuilder.
      ExtractAutoHFTextEntryIndexFromObj(Sender)];
    ...
    case Msg of
      WM_SETTINGCHANGE:
        ...
        begin
          ...
          for I := 0 to Count - 1 do
            TdxPrintStylePrinterPage(Styles[I].PrinterPage)
...
procedure TdxPrintStyleManager.SetNameHandler(Reader: TReader;
...
procedure TdxPrintStyleManager.ReadIsCloneStyleCaptionPrefixAssigned
  (Reader: TReader);
begin
...
procedure TdxPrintStyleManager.WriteIsCloneStyleCaptionPrefixAssigned
  (Writer: TWriter);
begin
        ...
        begin
          ...
          if P = 0 then
            ...
            AName := Copy(ItemName, 1, P - 1) + Name +
              Copy(ItemName, P + Length(OldName), Length(ItemName) - P -
              Length(OldName) + 1);
          Styles[I].Name := AName;
        end;
      finally
...
begin
  Result := FIsCloneStyleCaptionPrefixAssigned and
    (FCloneStyleCaptionPrefix <> DefaultCloneStyleCaptionPrefix);
end;
...
procedure TdxPrintStyleManager.SetStyle(Index: Integer;
...
function TdxPrintStyleManager.StyleByCaption(const ACaption: string)
  : TBasedxPrintStyle;
var
  I: Integer;
begin
...
function TdxPrintStyleManager.StyleByName(const AName: string)
  : TBasedxPrintStyle;
var
  I: Integer;
begin
...
begin
  Result := nil;
  if (AIndex < -1) or (AIndex > Count - 1) then
    Exit;
  if AIndex = -1 then
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Length(S2) > dxMaxStyleCaption) then
        SetLength(S2, dxMaxStyleCaption);
      I := 2;
...
function TdxPrintStyleManager.AddStyle(AStyleClass: TdxPrintStyleClass)
  : TBasedxPrintStyle;
begin
  ...
  begin
    ...
    begin
      if FLoadedExistingStyles = nil then
        ...
        ALoadedExistingStyleIndex := FLoadedExistingStyles.IndexOf
          (Styles[I].Name);
      if ALoadedExistingStyleIndex = -1 then
        Styles[I].Free
      else
      begin
        AStyle := TBasedxPrintStyle(FLoadedExistingStyles.Objects
          [ALoadedExistingStyleIndex]);
...
begin
  if (FUpdateCount = 0) and not IsLoading and not IsDestroying then
    if Assigned(FOnChangeCurrentStyle) then
      FOnChangeCurrentStyle(Self);
end;
...
begin
  if (FUpdateCount = 0) and not IsLoading and not IsDestroying then
  begin
    if Assigned(FOnStyleListChanged) then
      FOnStyleListChanged(Self);
...
begin
  ...
  try
    try
      MenuBuilder.BuildAutoHFTextEntriesMenu(ARootItem, AData,
        AIncludeSetupAutoHFTextEntriesItem, AutoHFTextEntries,
        OnAutoHFTextEntryClick, OnEditAutoHFTextEntriesClick);
    except
      Application.HandleException(Self);
    end;
  finally
...
procedure TdxPrintStyleManager.LoadFromIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  dxLoadStrings(AIniFile, ASectionName, AutoHFTextEntries);
end;

procedure TdxPrintStyleManager.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxPrintStyleManager.DefinePrintStylesDlg(out APreviewBtnClicked,
...
constructor TAbstractdxStyleManagerDesigner.Create(AStyleManager
  : TdxPrintStyleManager);
begin
  inherited Create;
...
begin
  while (Length(Source) > 0) and ((Source[1] = '.') or (Source[1] = ',') or
    (Source[1] = ' ')) do
    Delete(Source, 1, 1);
end;
...
var
  HourFormat: string;
begin
  HourFormat := HourFormats[StrToIntDef(GetLocaleChar(GetThreadLocale,
    LOCALE_ITLZERO, '0'), 0) = 0];
  with AStrings do
  begin
    BeginUpdate;
    try
...
begin
  ...
  begin
    ...
    FPageNumberFormats.AddObject('1, 2, 3, 4, 5, ...',
      TObject(Integer(pnfNumeral)));
    FPageNumberFormats.AddObject('a, b, c, d, e, ...',
      TObject(Integer(pnfChars)));
    FPageNumberFormats.AddObject('A, B, C, D, E, ...',
      TObject(Integer(pnfUpperChars)));
    FPageNumberFormats.AddObject('i, ii, iii, iv, v, ...',
      TObject(Integer(pnfRoman)));
    FPageNumberFormats.AddObject('I, II, III, IV, V, ...',
      TObject(Integer(pnfUpperRoman)));
  end;
  Result := FPageNumberFormats;
end;

function System_GetFormatedDate(const ASystemTime: TSystemTime;
  const AFormat: string): string;
var
  DefaultLCID: LCID;
  BufferSize: Integer;
...
begin
  ...
  begin
    ...
    BufferSize := GetDateFormat(DefaultLCID, 0, @ASystemTime, PChar(AFormat),
...
function System_GetFormatedTime(const ASystemTime: TSystemTime;
  const AFormat: string): string;
var
  DefaultLCID: LCID;
  BufferSize: Integer;
...
begin
  ...
  begin
    DefaultLCID := GetThreadLocale;
    FillChar(Buffer, SizeOf(Buffer), 0);
    BufferSize := GetTimeFormat(DefaultLCID, 0, @ASystemTime, PChar(AFormat),
      @Buffer, Length(Buffer));
    if BufferSize = 0 then
      Result := ''
    else
...
begin
  ...
  begin
    ...
    if F <> nil then
      Add(dxHFFunctionSeparator + ' ' + '-' + F.TemplateString + '-' + ' ' +
    ...
    F := dxHFFunctionLibrary.FuncsByClass[TdxHFPageNumberFunction];
    if F <> nil then
      S := S + dxHFFunctionSeparator + ' ' +
        DropAmpersand(cxGetResourceString(@sdxPage)) + ' ' + F.TemplateString;
    ...
    if F <> nil then
      Add(DropAmpersand(cxGetResourceString(@sdxPage)) + ' ' +
        F.TemplateString);
  end;
end;
...
begin
  ...
  cxDrawImage(ACanvas, cxRectSetWidth(ARect,
    ScaleFactor.Apply(ilPaperTypes.Width)), nil, ilPapers,
  ...
  ACanvas.Canvas.TextRect(ARect, ARect.Left,
    ARect.Top + (ARect.Bottom - ARect.Top - ACanvas.TextHeight(AControl.Items
    [AIndex])) div 2, AControl.Items[AIndex]);
...
begin
  ...
  cxDrawImage(ACanvas, cxRectSetWidth(R, ScaleFactor.Apply(ilBins.Width)), nil,
    ilBins, Integer(dxPrintDevice.IsAutoSelectBin(AIndex)), True, nil,
    ScaleFactor);
  Inc(R.Left, ScaleFactor.Apply(ilPaperTypes.Width) + ScaleFactor.Apply(2));
  cxDrawText(ACanvas.Handle, AControl.Properties.Items[AIndex], R,
