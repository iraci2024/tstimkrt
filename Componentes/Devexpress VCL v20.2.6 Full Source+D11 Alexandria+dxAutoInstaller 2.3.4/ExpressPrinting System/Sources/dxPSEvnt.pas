...
type
  ...
  public
    ...
    property OnPageParamsChanged: TdxPageParamsChangedEvent
      read FOnPageParamsChanged write FOnPageParamsChanged;
  end;
  ...
  protected
    procedure DoProcessEvent; override;
  public
    procedure EndPrint(Sender: TObject;
    ...
    property OnEndPrint: TdxReportLinkNotifyEvent read FOnEndPrint
      write FOnEndPrint;
    property OnProgressPrint: TdxNewPageEvent read FOnProgressPrint
      write FOnProgressPrint;
  ...
  protected
    procedure DoProcessEvent; override;
  public
    property OnHFTextEntriesChanged: TdxNoParamsEvent
      read FOnHFTextEntriesChanged write FOnHFTextEntriesChanged;
  end;
  ...
  TdxHFTextEntryEvent = procedure(Sender: TObject; const AEntry: string)
    of object;
  ...
  protected
    procedure DoProcessEvent; override;
  public
    property OnHFTextEntryChoose: TdxHFTextEntryEvent read FOnHFTextEntryChoose
      write FOnHFTextEntryChoose;
  end;
...
constructor TdxPSPrintEvent.Create(Sender: TObject;
...
constructor TdxPSBuildEvent.Create(Sender: TObject;
...
begin
  if Assigned(FOnEndPrint) then
    FOnEndPrint(Sender, AReportLink);
end;
...
begin
  if ActiveEvent is TdxHFTextEntriesChangedEvent then
    if Assigned(FOnHFTextEntriesChanged) then
      FOnHFTextEntriesChanged;
end;

{ TdxHFTextEntryChooseEvent }

constructor TdxHFTextEntryChooseEvent.Create(Sender: TObject;
  const AEntry: string);
begin
  inherited Create(Sender);
  FEntry := AEntry;
...
begin
  if ActiveEvent is TdxHFTextEntryChooseEvent then
    if Assigned(FOnHFTextEntryChoose) then
      FOnHFTextEntryChoose(ActiveEvent.Sender,
        TdxHFTextEntryChooseEvent(ActiveEvent).Entry);
end;
