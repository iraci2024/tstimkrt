...
type
  ...
  private
    ...
    procedure ControlChange(Sender: TObject; AComponent: TComponent;
...
procedure TdxCustomContainerListBox.ControlChange(Sender: TObject;
  AComponent: TComponent; AAction: TListNotification);
begin
  if AAction = lnAdded then
  begin
    TWinControl(AComponent).Parent := Self;
    TWinControl(AComponent).SetBounds(0, 0, ColumnWidth - 2 * ItemPadding,
      ItemHeight - 2 * ItemPadding);
  end;
  UpdateItems;
end;
...
procedure TdxCustomContainerListBox.DrawControl(const AControl: TWinControl;
  const APos: TPoint);
var
  ANewPos: TPoint;
begin
  Canvas.FrameRect(cxRectBounds(APos, ColumnWidth - IndentFromEdges,
    ItemHeight - IndentFromEdges), clBlack);
  ANewPos := APos;
...
begin
  ...
  ABounds := TRect.CreateSize(SelectRect.X + FocusRectangleMargin,
    SelectRect.Y + FocusRectangleMargin, ABoundsWidth, ABoundsHeight);
  Canvas.DrawFocusRect(ABounds);
end;

procedure TdxCustomContainerListBox.DrawItem(const R: TRect;
...
begin
  ...
  ADrawRect := TRect.CreateSize(SelectRect.X + SelectRectangleMargin,
    SelectRect.Y + SelectRectangleMargin, ASelectRectWidth, ASelectRectHeight);
...
procedure TdxCustomContainerListBox.MouseDown(Button: TMouseButton;
...
begin
  ...
  Assert(not((FSelectedIndex < -1) or
    (FSelectedIndex > FControlItems.Count - 1)));
...
procedure TdxCustomContainerListBox.UpdateSelectedRectCoordinate
  (AValue: Integer);
begin
...
procedure TdxCustomContainerListBox.UpdateSelectIndexAfterMouseDown
  (const APos: TPoint);
var
  I: Integer;
  ASelPos: TPoint;
...
begin
  ...
  for I := AVisibleItemIndex to Min(ControlItems.Count - 1,
    AVisibleItemIndex + ColumnCount * RowCount) do
  begin
