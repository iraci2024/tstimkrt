...
type
  ...
  public
    ...
    constructor Create(AUid: Integer;
      AProtectionType: TdxDocumentProtectionType); overload;
    class function FromStream(AReader: TBinaryReader)
      : TdxDocProtectionInfo; static;
  ...
  public
    ...
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer): TdxUserProtectionStringTable; static;
  ...
  public
    ...
    destructor Destroy; override;
    class function FromStream(AReader: TBinaryReader; AOffset: Integer;
      ASize: Integer): TdxRangeEditPermissionsStringTable; static;
    procedure Write(AWriter: TBinaryWriter); override;

    property ProtectionInfos: TdxObjectList<TdxDocProtectionInfo>
      read FProtectionInfos write FProtectionInfos;
  end;
  ...
  strict private
    ...
    FProtectionType: TdxDocumentProtectionType;
  public
    constructor Create(AOriginalStartPosition: Integer;
      AOriginalEndPosition: Integer);
    destructor Destroy; override;
  ...
  protected
    procedure Read(AFib: TdxFileInformationBlock;
      AReader: TBinaryReader); override;
    procedure InitRangeEditPermissions;
    function CreatePermissionInfos(AStart: Integer; AEnd: Integer;
      AType: TdxDocumentProtectionType; AUsers: TdxStringList)
      : TdxObjectList<TdxDocImportRangePermissionInfo>;
  public
    constructor Create; overload;
    constructor Create(AFib: TdxFileInformationBlock; AReader: TBinaryReader;
      AKeepPermissionsForRemovedRanges: Boolean); overload;
    destructor Destroy; override;
    procedure InsertRangeEditPermissions(APieceTable: TdxPieceTable);
    procedure RemoveProcessedPermissions(AProcessedPermissions
      : TdxList<TdxDocImportRangePermissionInfo>);
    procedure Write(AFib: TdxFileInformationBlock;
      AWriter: TBinaryWriter); override;
    procedure AddPermissionStart(APermission: TdxRangePermission;
      AStartPosition: Integer);
    procedure AddPermissionEnd(APermission: TdxRangePermission;
      AEndPosition: Integer);
  end;
...
constructor TdxDocProtectionInfo.Create(AUid: Integer;
  AProtectionType: TdxDocumentProtectionType);
begin
  FUid := Word(AUid);
...
class function TdxDocProtectionInfo.FromStream(AReader: TBinaryReader)
  : TdxDocProtectionInfo;
begin
...
begin
  ...
  ProtectionType := TdxDocumentProtectionTypeCalculator.
    CalcRangePermissionProtectionType(AReader.ReadSmallInt);
  AReader.BaseStream.Seek(IgnoredDataSize, TSeekOrigin.soCurrent);
end;
...
begin
  AWriter.Write(FUid);
  AWriter.Write(TdxDocumentProtectionTypeCalculator.
    CalcRangePermissionProtectionTypeCode(ProtectionType));
...
class function TdxUserProtectionStringTable.FromStream(AReader: TBinaryReader;
  AOffset: Integer; ASize: Integer): TdxUserProtectionStringTable;
begin
...
function TdxUserProtectionStringTable.GetUserIdByName(const AUserName
  : string): Integer;
var
  ACount, I: Integer;
begin
...
function TdxUserProtectionStringTable.CalcExtraDataSize
  (AReader: TBinaryReader): Integer;
begin
...
function TdxUserProtectionStringTable.CalcRecordsCount
  (AReader: TBinaryReader): Integer;
begin
  Result := AReader.ReadSmallInt;
end;

procedure TdxUserProtectionStringTable.WriteExtraDataSize
...
procedure TdxUserProtectionStringTable.Read(AReader: TBinaryReader;
...
begin
  ...
  begin
    ...
    else if Roles[I] = TdxDocUserRole.Owner then
      FOwners.Add(Users[I]);
  end;
end;
...
procedure TdxUserProtectionStringTable.WriteString(AWriter: TBinaryWriter;
  AIndex: Integer);
var
  AUser: string;
begin
...
procedure TdxUserProtectionStringTable.WriteExtraData(AWriter: TBinaryWriter;
  AIndex: Integer);
begin
...
class function TdxRangeEditPermissionsStringTable.FromStream
  (AReader: TBinaryReader; AOffset: Integer; ASize: Integer)
  : TdxRangeEditPermissionsStringTable;
begin
...
function TdxRangeEditPermissionsStringTable.CalcExtraDataSize
  (AReader: TBinaryReader): Integer;
begin
...
function TdxRangeEditPermissionsStringTable.CalcRecordsCount
  (AReader: TBinaryReader): Integer;
begin
...
procedure TdxRangeEditPermissionsStringTable.WriteExtraDataSize
  (AWriter: TBinaryWriter);
begin
...
procedure TdxRangeEditPermissionsStringTable.ReadExtraData
  (AReader: TBinaryReader);
begin
  ProtectionInfos.Add(TdxDocProtectionInfo.FromStream(AReader));
end;

procedure TdxRangeEditPermissionsStringTable.WriteString(AWriter: TBinaryWriter;
  AIndex: Integer);
begin
  AWriter.Write(SmallInt($00));
end;

procedure TdxRangeEditPermissionsStringTable.WriteExtraData
  (AWriter: TBinaryWriter; AIndex: Integer);
begin
...
constructor TdxDocImportRangePermissionInfo.Create(AOriginalStartPosition
  : Integer; AOriginalEndPosition: Integer);
begin
...
constructor TdxDocRangeEditPermissionIterator.Create
  (AFib: TdxFileInformationBlock; AReader: TBinaryReader;
  AKeepPermissionsForRemovedRanges: Boolean);
begin
...
procedure TdxDocRangeEditPermissionIterator.InsertRangeEditPermissions
  (APieceTable: TdxPieceTable);
var
  I: Integer;
  AProcessedPermissions: TdxList<TdxDocImportRangePermissionInfo>;
  AStart, AEnd: TdxDocumentLogPosition;
  ASkipDeletedPositions, AStartPositionObtainable,
    AEndPositionObtainable: Boolean;
...
begin
  ...
  try
    ...
    begin
      ...
      if (not PositionConverter.ContainsPosition(AInfo.OriginalStartPosition))
        or (not PositionConverter.ContainsPosition(AInfo.OriginalEndPosition))
        or (not AInfo.Validate) then
        Continue;
      AStartPositionObtainable := PositionConverter.TryConvert
        (AInfo.OriginalStartPosition, ASkipDeletedPositions, AStart);
      AEndPositionObtainable := PositionConverter.TryConvert
        (AInfo.OriginalEndPosition, ASkipDeletedPositions, AEnd);
      if (AStartPositionObtainable and AEndPositionObtainable) and AInfo.Validate
      then
      begin
      ...
      if not AStartPositionObtainable then
        PositionConverter.TryConvert(AInfo.OriginalStartPosition,
          False, AStart);
...
procedure TdxDocRangeEditPermissionIterator.RemoveProcessedPermissions
  (AProcessedPermissions: TdxList<TdxDocImportRangePermissionInfo>);
var
  ACount, I: Integer;
begin
...
procedure TdxDocRangeEditPermissionIterator.Read(AFib: TdxFileInformationBlock;
  AReader: TBinaryReader);
begin
  FProtectionInfos := TdxRangeEditPermissionsStringTable.FromStream(AReader,
    AFib.RangeEditPermissionsInformationOffset,
    AFib.RangeEditPermissionsInformationSize);
  FirstTable := TdxDocBookmarkFirstTable.FromStream(AReader,
    AFib.RangeEditPermissionsStartInfoOffset,
    AFib.RangeEditPermissionsStartInfoSize, BookmarkIndexSize);
  LimTable := TdxDocBookmarkLimTable.FromStream(AReader,
    AFib.RangeEditPermissionsEndInfoOffset,
    AFib.RangeEditPermissionsEndInfoSize);
  FUsers := TdxUserProtectionStringTable.FromStream(AReader,
    AFib.RangeEditPermissionsUsernamesOffset,
    AFib.RangeEditPermissionsUsernamesSize);
...
begin
  ...
  begin
    if FirstTable.BookmarkFirstDescriptors[I].Column then
      Continue;
    AGrantedUsers := FUsers.GetUsersById
      (FProtectionInfos.ProtectionInfos[I].Uid);
    AStart := FirstTable.CharacterPositions[I];
    AEnd := LimTable.CharacterPositions[I];
    APermissions := CreatePermissionInfos(AStart, AEnd,
      FProtectionInfos.ProtectionInfos[I].ProtectionType, AGrantedUsers);
    try
...
function TdxDocRangeEditPermissionIterator.CreatePermissionInfos
  (AStart: Integer; AEnd: Integer; AType: TdxDocumentProtectionType;
...
procedure TdxDocRangeEditPermissionIterator.Write(AFib: TdxFileInformationBlock;
  AWriter: TBinaryWriter);
begin
  AFib.RangeEditPermissionsInformationOffset := AWriter.BaseStream.Position;
  FProtectionInfos.Write(AWriter);
  AFib.RangeEditPermissionsInformationSize := AWriter.BaseStream.Position -
    AFib.RangeEditPermissionsInformationOffset;
  AFib.RangeEditPermissionsStartInfoOffset := AWriter.BaseStream.Position;
  FirstTable.Write(AWriter, BookmarkIndexSize);
  AFib.RangeEditPermissionsStartInfoSize := AWriter.BaseStream.Position -
    AFib.RangeEditPermissionsStartInfoOffset;
  AFib.RangeEditPermissionsEndInfoOffset := AWriter.BaseStream.Position;
  LimTable.Write(AWriter);
  AFib.RangeEditPermissionsEndInfoSize := AWriter.BaseStream.Position -
    AFib.RangeEditPermissionsEndInfoOffset;
  AFib.RangeEditPermissionsUsernamesOffset := AWriter.BaseStream.Position;
  FUsers.Write(AWriter);
  AFib.RangeEditPermissionsUsernamesSize := AWriter.BaseStream.Position -
    AFib.RangeEditPermissionsUsernamesOffset;
end;

procedure TdxDocRangeEditPermissionIterator.AddPermissionStart
  (APermission: TdxRangePermission; AStartPosition: Integer);
var
  AId: Integer;
begin
...
procedure TdxDocRangeEditPermissionIterator.AddPermissionEnd
  (APermission: TdxRangePermission; AEndPosition: Integer);
var
...
begin
  ...
  FProtectionInfos.ProtectionInfos.Add(TdxDocProtectionInfo.Create(AUserId,
    TdxDocumentProtectionType.None));
  FirstTable.AddEntry(AStartPosition, TdxDocBookmarkFirstDescriptor.Create
    (CurrentIndex));
