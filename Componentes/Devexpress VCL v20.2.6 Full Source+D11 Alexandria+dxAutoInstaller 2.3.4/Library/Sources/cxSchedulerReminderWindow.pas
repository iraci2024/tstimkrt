...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Messages, SysUtils, Classes, Controls, Graphics, ExtCtrls,
  Forms,
  StdCtrls, cxLookAndFeels, cxSchedulerStorage, cxListView, cxButtons, ComCtrls,
  Menus,
  dxCore, dxMessages, cxControls, cxContainer, cxClasses, cxGraphics,
  cxLookAndFeelPainters, cxEdit, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxLabel, dxLayoutContainer, dxLayoutControl,
...
type
  ...
  TcxSchedulerReminderForm = class(TcxSchedulerCustomReminderForm, IUnknown,
  ...
  protected
    ...
    procedure AddReminderInfo(AReminder: TcxSchedulerReminder; AItem: TListItem;
      const ADueInText: string);
    function AddSnoozeAdvanceTime(const AText: string;
    ...
    function GetImageIndexForReminder(AReminder: TcxSchedulerReminder)
      : Integer; virtual;
    function GetMaxAdvanceMinutes: Integer;
    function GetResourceNameForReminder(AReminder: TcxSchedulerReminder)
      : string; virtual;
    ...
    property VisibleReminders[Index: Integer]: TcxSchedulerReminder
      read GetVisibleReminder;
  public
...
constructor TcxSchedulerReminderFormItem.Create
...
function TcxSchedulerReminderFormItem.Equals
  (AReminder: TcxSchedulerReminder): Boolean;
begin
  Result := SameEvent(AReminder.Event) and
    (ReminderDate = AReminder.ReminderDate) and
...
function TcxSchedulerReminderForm.AddReminder(AReminder: TcxSchedulerReminder)
  : TListItem;
begin
...
begin
  ...
  lvItems.Columns[1 + GetDueInColumnIndex].Caption :=
    cxGetResourceString(@scxrDueIn);
end;
...
begin
  ...
  begin
    ...
    if AMaxAdvanceTime > 5 then
      cbSnoozeTime.ItemIndex := AddSnoozeAdvanceTime
        (cxGetResourceString(@scxAdvance5m), 5);
...
begin
  AIsAdvanceTime := (lvItems.SelCount = 1) and
    (cbSnoozeTime.ItemIndex < AdvanceCount);
...
begin
  ...
  try
    ...
    for I := 0 to VisibleReminderCount - 1 do
      lvItems.Items[I].SubItems[AColumnIndex] := Reminders.GetEventDueTimeText
        (VisibleReminders[I], ANow);
  finally
    lvItems.Items.EndUpdate;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      AddReminderInfo(AReminder, AItem,
        Reminders.GetEventDueTimeText(AReminder, ANow));
    end;
  finally
    lvItems.Items.EndUpdate;
  end;
...
begin
  if lvItems.SelCount = 1 then
    MenuImages.Draw(pbImage.Canvas, (pbImage.Width - MenuImages.Width) div 2,
...
function TcxSchedulerReminderForm.GetVisibleReminder(Index: Integer)
...
begin
  if (FSelections = nil) or (VisibleReminderCount = 0) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      while (Length(Result) > 0) and
        (cxScreenCanvas.TextWidth(Result) > ATotalWidth) do
...
begin
  ...
  begin
    ...
    lbEventStartTime.Caption := Format(cxGetResourceString(@scxrStartTime),
      [FormatDateTime('dddddd t', AEvent.Start)]);
  end
  else
  begin
    lbEventCaption.Caption := '';
    lbEventStartTime.Caption := Format(cxGetResourceString(@scxrSelected),
      [lvItems.SelCount]);
  end;
...
const
SnoozeTimes:
array [1 .. 15] of record P: Pointer;
M:
Integer
end
= ((P: @scxTime5m; M: 5), (P: @scxTime10m; M: 10), (P: @scxTime15m; M: 15),
  (P: @scxTime30m; M: 30), (P: @scxTime1h; M: MinsPerHour), (P: @scxTime2h;
  M: 2 * MinsPerHour), (P: @scxTime4h; M: 4 * MinsPerHour), (P: @scxTime8h;
  M: 8 * MinsPerHour), (P: @scxTime12h; M: 12 * MinsPerHour), (P: @scxTime1d;
  M: MinsPerDay), (P: @scxTime2d; M: 2 * MinsPerDay), (P: @scxTime3d;
  M: 3 * MinsPerDay), (P: @scxTime4d; M: 4 * MinsPerDay), (P: @scxTime1w;
  M: MinsPerWeek), (P: @scxTime2w; M: 2 * MinsPerWeek));
...
begin
  ...
  try
    ...
    if cbSnoozeTime.ItemIndex = -1 then
      cbSnoozeTime.ItemIndex := 1;
  finally
...
procedure TcxSchedulerReminderForm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
