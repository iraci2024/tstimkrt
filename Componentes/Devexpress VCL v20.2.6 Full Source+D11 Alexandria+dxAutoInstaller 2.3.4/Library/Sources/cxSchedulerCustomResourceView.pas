...
{ ExpressScheduler }
{ }
{ Copyright (c) 2003-2021 Developer Express Inc. }
{ ALL RIGHTS RESERVED }
{ }
...
{$ENDIF}
  ...
  DateUtils, Variants, Math, ImgList, Types, Generics.Defaults,
  Generics.Collections,
  dxCoreClasses, cxClasses, cxGraphics,
  cxGeometry, cxControls, cxStyles, cxEdit, cxContainer, dxGDIPlusClasses,
  cxSchedulerStorage, cxSchedulerUtils, cxSchedulerCustomControls,
...
type
  ...
  TcxSchedulerColumnPositionInResource = (cprIndefinite, cprFirst, cprLast,
    cprSingle);
  TcxSchedulerDayHeaderModernStyleDisplayMode = (hdmDefault, hdmClassic,
    hdmDayAndDate);

  // custom draw event types definition

  TcxSchedulerCustomDrawBackgroundEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerBackgroundCellViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawButtonEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerMoreEventsButtonViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawDayHeaderEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerDayHeaderCellViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawNavigationButtonEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerContentNavigationButtonViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawResourceHeaderEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerHeaderCellViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawContentEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerContentCellViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawEventEvent = procedure(Sender: TObject;
    ACanvas: TcxCanvas; AViewInfo: TcxSchedulerEventCellViewInfo;
    var ADone: Boolean) of object;

  TcxSchedulerCustomDrawGroupSeparatorEvent = procedure(Sender: TObject;
  ...
  IcxSchedulerCommonViewItemsPainter = interface
    ...
    procedure DoCustomDrawBackground(AViewInfo
      : TcxSchedulerBackgroundCellViewInfo; var ADone: Boolean);
    procedure DoCustomDrawButton(AViewInfo
      : TcxSchedulerMoreEventsButtonViewInfo; var ADone: Boolean);
    procedure DoCustomDrawContent(AViewInfo: TcxSchedulerContentCellViewInfo;
      var ADone: Boolean);
    procedure DoCustomDrawDayHeader(AViewInfo
      : TcxSchedulerDayHeaderCellViewInfo; var ADone: Boolean);
    procedure DoCustomDrawEvent(AViewInfo: TcxSchedulerEventCellViewInfo;
      var ADone: Boolean);
    procedure DoCustomDrawGroupSeparator(AViewInfo
      : TcxSchedulerGroupSeparatorCellViewInfo; var ADone: Boolean);
    procedure DoCustomDrawNavigationButton
      (AViewInfo: TcxSchedulerContentNavigationButtonViewInfo;
      var ADone: Boolean);
    procedure DoCustomDrawResourceHeader(AViewInfo
      : TcxSchedulerHeaderCellViewInfo; var ADone: Boolean);
    // cached paint
  ...
  public
    function CalculateHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest): Boolean;
    procedure Draw(ACanvas: TcxCanvas;
      ADrawItemProc: TcxSchedulerCustomDrawItemProc);

    property Items[Index: Integer]: TcxSchedulerCustomViewInfoItem
  ...
  protected
    ...
    function CalculateClipRect(const ABounds, AVisibleRect: TRect)
      : TRect; virtual;
    procedure ClippingCreate(AHasClipping: Boolean); virtual;
    procedure ClippingRestore;
    procedure DoDraw; virtual; abstract;
    procedure DoRightToLeftConversion(const AClientBounds: TRect); virtual;
    procedure DoRightToLeftConversionWithChecking(var ARect: TRect;
      const AClientBounds: TRect);
    function DrawBackground(ACanvas: TcxCanvas; const ABounds: TRect)
      : Boolean; virtual;
    ...
    procedure DrawRect(const ARect: TRect; ABorders: TcxBorders = [];
      ABorderColor: TColor = clDefault; ABorderSize: Integer = 1);
    procedure DrawText(const ARect: TRect; const AText: string; AFlags: Integer;
      AFont: TFont = nil; AColor: TColor = clDefault);
    function GetActiveBounds: TRect; virtual;
    function GetActualRectRTL(const ARect, AClientBounds: TRect;
      AOffsetIfRTL: Integer = 0): TRect;
    function GetBoundsForHitTest: TRect; virtual;
    function GetFont: TFont; virtual;
    function GetHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest;
      var ABreak: Boolean): Boolean; virtual;
    procedure InitHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest); virtual;
    ...
    // class item settings
    property AllowRightToLeftConversion: Boolean
      read FAllowRightToLeftConversion write FAllowRightToLeftConversion;
    property ActiveBounds: TRect read GetActiveBounds;
    property DateTimeHelper: TcxSchedulerDateTimeHelperClass
      read GetDateTimeHelper;
    property DisplayText: string read FDisplayText write FDisplayText;
    property PainterHelper: TcxSchedulerPainterHelperClass
      read GetPainterHelper;
  ...
  public
    constructor Create(APainter: TcxCustomLookAndFeelPainter;
      ...
      AScaleFactor: TdxScaleFactor; AUseRightToLeftAlignment: Boolean); virtual;
  ...
  protected
    procedure InitHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest); override;

    property DateTime: TDateTime read FDateTime write FDateTime;
    property IsResourceAssigned: Boolean read FIsResourceAssigned;
    property Resource: TcxSchedulerResourceViewInfo read FResource
      write FResource;
  ...
  TcxSchedulerCustomHeaderCellViewInfo = class
    (TcxSchedulerCustomResourceViewInfoItem)
  protected
    DisplayBounds: TRect;
    ...
    function DrawRotateBackground(ACanvas: TcxCanvas; const ABounds: TRect)
      : Boolean; virtual;
  ...
  public
    ...
    property ActualModernStyleDisplayMode
      : TcxSchedulerDayHeaderModernStyleDisplayMode
      read FActualModernStyleDisplayMode;
    property Compressed: Boolean read FCompressed write FCompressed;
  end;

  TcxSchedulerDayHeaderCellViewInfoClass = class of
    TcxSchedulerDayHeaderCellViewInfo;

  { TcxSchedulerDayHeaderCellModernViewInfo }

  TcxSchedulerDayHeaderCellModernViewInfo = class
    (TcxSchedulerDayHeaderCellViewInfo)
  protected
  ...
  TcxSchedulerWeekDayHeaderCellViewInfo = class
    (TcxSchedulerDayHeaderCellViewInfo)
  protected
  ...
  TcxSchedulerWeekDayHeaderCellModernViewInfo = class
    (TcxSchedulerWeekDayHeaderCellViewInfo)
  protected
    function DrawBackground(ACanvas: TcxCanvas; const ABounds: TRect)
      : Boolean; override;
  ...
  public
    procedure Calculate(ADate: TDateTime; ASelected: Boolean;
      ACaptionDisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode;
      AHeadersEventsAreaTop: Integer); virtual;

    property DateTime;
    property HeaderEvent: TcxSchedulerEventCellViewInfo read FHeaderEvent
      write FHeaderEvent;
    property LineCount: Integer read FLineCount write FLineCount;
    property ModernStyleCaptionDisplayMode
      : TcxSchedulerDayHeaderModernStyleDisplayMode
      read FModernStyleCaptionDisplayMode;
    property Resource;
    property Selected: Boolean read FSelected write FSelected;
  end;

  TcxSchedulerContainerCellViewInfoClass = class of
    TcxSchedulerContainerCellViewInfo;

  { TcxSchedulerContainerModernCellViewInfo }

  TcxSchedulerContainerModernCellViewInfo = class
    (TcxSchedulerContainerCellViewInfo)
  strict private
    FTextRect: TRect;
  ...
  public
    procedure Calculate(ADate: TDateTime; ASelected: Boolean;
      ACaptionDisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode;
      AHeadersEventsAreaTop: Integer); override;
  ...
  protected
    ...
    procedure DoDraw; override;
    procedure DoRightToLeftConversion(const AClientBounds: TRect); override;
    procedure DrawTimeZone(const ABounds: TRect; const AText: string;
      AIsCurrent: Boolean); virtual;
    procedure DrawTimeZoneLabel(const ABounds: TRect; const AText: string;
    ...
    function GetBorderColor: TColor; virtual;
    function GetCanDrawCurrentTime(ASelectedDays: TcxSchedulerDateList)
      : Boolean; virtual;
    function GetLabelZoneBorders(const AIsCurrentTimeZone: Boolean)
      : TcxBorders; virtual;
  ...
  public
    class function CalculateWidth(ATimeZoneCount, ALineCount: Integer;
      AFont1, AFont2: TFont; AScaleFactor: TdxScaleFactor = nil;
      AShowMinutes: Boolean = False): Integer;
    procedure Calculate(const ALabel1, ALabel2: string;
      ATimeZone, AAdditionalTimeZone: Integer;
      AAdditionalTimeZoneBiasDelta: Integer;
      ASelectedDays: TcxSchedulerDateList = nil); overload; virtual;
    procedure Calculate(AHour, ALineCount: Integer;
  ...
  protected
    ...
    procedure InitHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest); override;
    procedure SetContentSelectionParameters(ASelected: Boolean;
      const ASelectionParams: TcxViewParams);
    procedure SpecialDrawRect(const ARect: TRect; ABorders: TcxBorders;
      AContentColor, ABorderColor: TColor);
  ...
  public
    ...
    property TimeEvent: TcxSchedulerEventCellViewInfo read FTimeEvent
      write FTimeEvent;
    property TimeLineBorders: TcxBorders read FTimeLineBorders
      write FTimeLineBorders;
    property TimeLineParams: TcxViewParams read FTimeLineParams
      write FTimeLineParams;
    property TimeStart: TDateTime read FDateTime;
    property TimeFinish;
    property Resource;
  end;

  TcxSchedulerContentCellViewInfoClass = class of
    TcxSchedulerContentCellViewInfo;
  ...
  TcxSchedulerWeekDayContentCellModernViewInfo = class
    (TcxSchedulerContentCellViewInfo)
  protected
    procedure DoDraw; override;
  ...
  TcxSchedulerMonthDayContentCellCustomViewInfo = class
    (TcxSchedulerContentCellViewInfo)
  private
    function GetIsToday: Boolean;
  ...
  TcxSchedulerMonthDayContentCellViewInfo = class
    (TcxSchedulerMonthDayContentCellCustomViewInfo)
  protected
  ...
  TcxSchedulerMonthDayContentCellModernViewInfo = class
    (TcxSchedulerMonthDayContentCellCustomViewInfo)
  protected
    procedure DoDraw; override;
    procedure DoDrawBackground; override;
  ...
  protected
    ...
    procedure CalculateActualEditRect(AEditViewData: TcxCustomEditViewData;
      var AMessageRect: TRect); virtual;
    ...
    // items layout
    function CalculateAutoLayoutImagesAutoHeight(AVisibleImages: TList)
      : Integer; virtual;
    procedure CalculateHeaderEventContinueArrows(ACompressing
      : Integer); virtual;
    ...
    function CheckAutoLayoutImagesSingleLineCaption(const ACaptionWidth,
      AAvailableWidth: Integer): Boolean; virtual;
    procedure CheckItemsAfterAutoLayoutImagesCalculation
      (AFactuallyImagesRowCount, AImagesWidth: Integer); virtual;
    ...
    function GetHorizontalImagesMessageAutoHeight(AMessageTop: Integer)
      : Integer;
    ...
    function NeedCorrectCaptionBottomWhenMessageAutoLayoutCalculating
      : Boolean; virtual;
    ...
    procedure CalculateHeaderEventNeededCaptionWidth(var AFullWidth,
      ACaptionOnlyWidth: Integer); virtual;
    ...
    function CalculateSingleLineImages(const ABounds: TRect;
      ARightLimit: Integer = 0): Integer;
    ...
    function GetHeaderEventCaptionLeft(const ARect: TRect;
      AContentWidth: Integer): Integer; virtual;
    function GetHeaderImagesPossibleRect(const ACaptionRect: TRect;
      ATextWidth: Integer): TRect; virtual;
    ...
    function GetHitTest(AHitTest: TcxSchedulerCustomResourceViewHitTest;
      var ABreak: Boolean): Boolean; override;
    function GetImagesBounds: TRect;
    function GetImagesHorizontalOffset: Integer; virtual;
    function GetImagesVerticalOffset(AImageHeight: Integer;
      AIsAbsolute: Boolean): Integer; virtual;
    function GetLocationAutoHeight(const R: TRect): Integer;
    function GetMessageRect(const ACaptionRect: TRect;
    ...
    function MeasureAutoLayoutImagesLocationHeight(const R: TRect)
      : Integer; virtual;
    // HitTest
    function HintTriggerArea: TRect; virtual;
    procedure InitHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest); override;
    procedure InitHitTestForHorzEvent
  ...
  public
    constructor Create(AViewData: TcxSchedulerEventViewData;
      ACanResize: Boolean; AScaleFactor: TdxScaleFactor;
      AUseRightToLeftAlignment: Boolean); reintroduce; virtual;
    destructor Destroy; override;

    procedure DrawState(ACanvas: TcxCanvas; const ARect: TRect;
      ABorders: TcxBorders; ABorderColor: TColor); overload;
  ...
  public
    ...
    property Items[Index: Integer]: TcxSchedulerEventCellViewInfo
      read GetItem; default;
  end;

  { TcxSchedulerMoreEventsButtonViewInfo }

  TcxSchedulerMoreEventsButtonClick = procedure
    (Sender: TcxSchedulerMoreEventsButtonViewInfo) of object;

  TcxSchedulerMoreEventsButtonViewInfoClass = class of
    TcxSchedulerMoreEventsButtonViewInfo;
  ...
  protected
    ...
    property OnClick: TcxSchedulerMoreEventsButtonClick read FOnClick
      write FOnClick;
  public
  ...
  TcxSchedulerMoreEventsModernButtonViewInfo = class
    (TcxSchedulerMoreEventsButtonViewInfo)
  protected
  ...
  TcxSchedulerContentNavigationButtonClick = procedure
    (Sender: TcxSchedulerContentNavigationButtonViewInfo) of object;
  ...
  protected
    ...
    property OnClick: TcxSchedulerContentNavigationButtonClick read FOnClick
      write FOnClick;
  public
    constructor Create(APainter: TcxCustomLookAndFeelPainter;
      const ABounds, AVisibleRect: TRect; const AViewParams: TcxViewParams;
    ...
    property Kind: TcxSchedulerContentNavigationButtonKind read FKind
      write FKind;
    property TextRect: TRect read FTextRect;
  end;
  ...
  private
    FDrawRotatedBackground: Boolean;
    FRotated: Boolean;
    function DrawRotateBackground(ACanvas: TcxCanvas;
  ...
  public
    ...
    property VisibleEventCount: Integer read FVisibleEventCount
      write FVisibleEventCount;
  end;

  { TcxSchedulerExternalPainter }

  TcxSchedulerExternalPainter = class(TInterfacedObject,
    IcxSchedulerCommonViewItemsPainter)
  private
  ...
  protected
    procedure DrawCustomCurrentTime(ACanvas: TcxCanvas; AColor: TColor;
      AStart: TDateTime; const ABounds: TRect;
      AUseRightToLeftAlignment: Boolean); virtual;
  ...
  public
    // IcxSchedulerCommonViewItemsPainter
    procedure DoCustomDrawBackground(AViewInfo
      : TcxSchedulerBackgroundCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawButton(AViewInfo
      : TcxSchedulerMoreEventsButtonViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawContent(AViewInfo: TcxSchedulerContentCellViewInfo;
      var ADone: Boolean); virtual;
    procedure DoCustomDrawDayHeader(AViewInfo
      : TcxSchedulerDayHeaderCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawEvent(AViewInfo: TcxSchedulerEventCellViewInfo;
      var ADone: Boolean); virtual;
    procedure DoCustomDrawGroupSeparator(AViewInfo
      : TcxSchedulerGroupSeparatorCellViewInfo; var ADone: Boolean); virtual;
    procedure DoCustomDrawNavigationButton
      (AViewInfo: TcxSchedulerContentNavigationButtonViewInfo;
      var ADone: Boolean); virtual;
    procedure DoCustomDrawResourceHeader(AViewInfo
      : TcxSchedulerHeaderCellViewInfo; var ADone: Boolean); virtual;
    function HasCustomDrawGroupSeparator: Boolean;
    function HasCustomDrawResourceHeader: Boolean;
    //
    procedure DrawAllDayArea(ACanvas: TcxCanvas; const ARect: TRect;
      ...
      ASelected, ATransparent: Boolean); virtual;
    procedure DrawCellFocus(AViewInfo: TcxSchedulerCustomViewInfoItem;
      AColor: TColor); virtual;
    procedure DrawCurrentTime(ACanvas: TcxCanvas; AColor: TColor;
      AStart: TDateTime; ABounds: TRect); virtual;
    procedure DrawCurrentTimeRTL(ACanvas: TcxCanvas; AColor: TColor;
      AStart: TDateTime; ABounds: TRect); virtual;
    function DrawCurrentTimeFirst: Boolean; virtual;
    procedure DrawEvent(AViewInfo: TcxSchedulerEventCellViewInfo); virtual;
    procedure DrawEventAsGroup(AViewInfo
      : TcxSchedulerEventCellViewInfo); virtual;
    procedure DrawEventAsMilestone(AViewInfo
      : TcxSchedulerEventCellViewInfo); virtual;
    procedure DrawEventAsProgress(AViewInfo
      : TcxSchedulerEventCellViewInfo); virtual;
    ...
    procedure DrawEventLabel(AViewInfo: TcxSchedulerEventCellViewInfo;
    ...
    procedure DrawTimeRulerBackground(ACanvas: TcxCanvas; const ARect: TRect;
      ...
      overload; virtual;
    procedure DrawTimeRulerBackground(ACanvas: TcxCanvas; const ARect: TRect;
      ABorders: TcxBorders; ABorderBolor: TColor; AViewParams: TcxViewParams;
      ATransparent: Boolean); overload; virtual;
    procedure DrawShadow(ACanvas: TcxCanvas;
      const ABounds, AVisibleBounds: TRect;
      AScaleFactor: TdxScaleFactor = nil); virtual;
    function DrawShadowFirst: Boolean; virtual;

    procedure AdjustEditStyleViewParams(var AViewParams
      : TcxViewParams); virtual;
    procedure FlushCache; virtual;
    function GetColorizedColor(ASource: TColor;
      AEvent: TcxSchedulerEventCellViewInfo): TColor;
    function GetEventBorderColor(AViewInfo: TcxSchedulerEventCellViewInfo)
      : TColor; virtual;
    function GetEventBorderSize(AViewInfo: TcxSchedulerEventCellViewInfo)
      : Integer; virtual;
    function GetEventLabelSize(AScaleFactor: TdxScaleFactor = nil)
      : TSize; virtual;
    function GetEventSelectionBorderColor
      (AViewInfo: TcxSchedulerEventCellViewInfo): TColor; virtual;
    function GetEventSelectionBorderSize(AViewInfo
      : TcxSchedulerEventCellViewInfo): Integer; virtual;
    function GetEventSelectionExtends: TRect; overload; virtual;
    function GetEventSelectionExtends(AViewInfo: TcxSchedulerEventCellViewInfo)
      : TRect; overload; virtual;
    function MoreButtonSize(ASize: TSize; AScaleFactor: TdxScaleFactor = nil)
      : TSize; virtual;
    function NeedDrawSelection: Boolean; virtual;

    property Painter: TcxCustomLookAndFeelPainter read FPainter
      write SetPainter;
    property CommonPainter: IcxSchedulerCommonViewItemsPainter
      read FCommonPainter write FCommonPainter; // for internal use
  end;
  ...
  private
    ...
    function IsRelevantSelection(out ATime: TDateTime;
      out AResource: TcxSchedulerStorageResourceItem): Boolean;
  ...
  protected
    ...
    function FindEventViewInfo(AEvent: TcxSchedulerEvent;
      const ADate: TDateTime; AResource: TcxSchedulerStorageResourceItem;
      var AViewInfo: TcxSchedulerEventCellViewInfo): Boolean;
    function GetCommonViewItemsPainter
      : IcxSchedulerCommonViewItemsPainter; virtual;
    function GetCompressWeekEnd: Boolean; virtual;
    function GetEditRectForEvent(AEvent: TcxSchedulerControlEvent;
      const ADate: TDateTime; AResource: TcxSchedulerStorageResourceItem)
      : TRect; override;
    function GetEditStyleViewParams(AEvent: TcxSchedulerControlEvent)
      : TcxViewParams; override;
    function GetEventHintText(AEvent: TcxSchedulerControlEvent)
      : string; override;
    function GetEventVisibility(AEvent: TcxSchedulerControlEvent)
      : Boolean; override;
    ...
    property EventImagesLayout: TcxSchedulerEventImagesLayout
      read FEventImagesLayout write SetEventImagesLayout;
    property ExternalPainter: TcxSchedulerExternalPainter read FExternalPainter;
    property ShowEndTime: Boolean read FShowEndTime write SetShowEndTime
      default True;
    property ShowTimeAsClock: Boolean read FShowTimeAsClock
      write SetShowTimeAsClock default False;
    property ViewInfo: TcxSchedulerCustomResourceViewViewInfo read GetViewInfo;

    property OnCustomDrawBackground: TcxSchedulerCustomDrawBackgroundEvent
      read FOnCustomDrawBackground write FOnCustomDrawBackground;
    property OnCustomDrawButton: TcxSchedulerCustomDrawButtonEvent
      read FOnCustomDrawButton write FOnCustomDrawButton;
    property OnCustomDrawContent: TcxSchedulerCustomDrawContentEvent
      read FOnCustomDrawContent write FOnCustomDrawContent;
    property OnCustomDrawDayHeader: TcxSchedulerCustomDrawDayHeaderEvent
      read FOnCustomDrawDayHeader write FOnCustomDrawDayHeader;
    property OnCustomDrawEvent: TcxSchedulerCustomDrawEventEvent
      read FOnCustomDrawEvent write FOnCustomDrawEvent;
    ...
    property OnCustomDrawNavigationButton
      : TcxSchedulerCustomDrawNavigationButtonEvent
      read FOnCustomDrawNavigationButton write FOnCustomDrawNavigationButton;
    property OnCustomDrawResourceHeader
      : TcxSchedulerCustomDrawResourceHeaderEvent
      read FOnCustomDrawResourceHeader write FOnCustomDrawResourceHeader;
  public
    ...
    property GroupingKind: TcxSchedulerGroupingKind read FGroupingKind
      write SetGroupingKind default gkDefault;
  end;
  ...
  protected
    ...
    property PainterHelper: TcxSchedulerPainterHelperClass
      read GetPainterHelper;
    property ResourceNavigator: TcxSchedulerResourceNavigator
      read GetResourceNavigator;
    property ViewInfo: TcxSchedulerCustomResourceViewViewInfo read GetViewInfo;
  public
  ...
  TcxSchedulerEventImageType = (eitReminder, eitRecurrence, eitCustomOccurrence,
    eitCustom);
  ...
  public
    ...
    function Add(AImageIndex: Integer; AutoHide: Boolean = True)
      : TcxSchedulerEventImageItem;
    function AddStandardImage(AImageType: TcxSchedulerEventImageType;
      AutoHide: Boolean = True): TcxSchedulerEventImageItem;

    property Items[Index: Integer]: TcxSchedulerEventImageItem
      read GetImageItem; default;
  ...
  public
    function Add(AItem: TcxSchedulerHeaderCellViewInfo)
      : TcxSchedulerResourceHeaderCachedImage;
    function GetCacheForItem(AItem: TcxSchedulerHeaderCellViewInfo)
      : TcxSchedulerResourceHeaderCachedImage;
  end;
  ...
  public
    ...
    property ResourceHeaders: TcxSchedulerResourceHeaderCachedImageList
      read FResourceHeaders;
  ...
  private
    ...
    procedure SetHitContentNavigationButton
      (AButton: TcxSchedulerContentNavigationButtonViewInfo;
      AHitTest: TcxSchedulerCustomResourceViewHitTest);
    procedure SetHitMoreEventsButton
      (AButton: TcxSchedulerMoreEventsButtonViewInfo;
      AHitTest: TcxSchedulerCustomResourceViewHitTest);
    procedure SetHotContentNavigationButton
      (AButton: TcxSchedulerContentNavigationButtonViewInfo);
    procedure SetHotMoreEventsButton
      (AButton: TcxSchedulerMoreEventsButtonViewInfo);
  protected
    FAdapter: TcxCustomResourceViewAdapter;
    ...
    function AddBackgroundSlot(const ABounds: TRect; ABorders: TcxBorders;
      const AText: string = '')
      : TcxSchedulerBackgroundSlotCellViewInfo; virtual;
    function AddButton(AColumnIndex: Integer; ABounds: TRect;
      const ADateTime: TDateTime; AIsDown: Boolean; AEvent: TcxSchedulerEvent)
      : TcxSchedulerMoreEventsButtonViewInfo; virtual;
    function AddContentCell(const ARect: TRect;
      const AStart, AFinish: TDateTime; AResourceIndex: Integer)
      : TcxSchedulerContentCellViewInfo; virtual;
    procedure AddContentNavigationButton(const AColumnRect: TRect;
    ...
    function AddDayHeader(const ADate: TDateTime; const ABounds: TRect;
      AResourceIndex: Integer;
      ADisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode)
      : TcxSchedulerDayHeaderCellViewInfo; overload; virtual;
    function AddDayHeader(const ADate: TDateTime; ATop, ALeft, ARight: Integer;
      AResourceIndex: Integer;
      ADisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode)
      : TcxSchedulerDayHeaderCellViewInfo; overload;
    function AddEventCell(AViewData: TcxSchedulerEventViewData;
      AImmediateCalculate: Boolean = True)
      : TcxSchedulerEventCellViewInfo; virtual;
    function AddGroupHorzSeparator(APos: Integer)
      : TcxSchedulerGroupSeparatorCellViewInfo; virtual;
    function AddGroupSeparator(const ABounds: TRect)
      : TcxSchedulerGroupSeparatorCellViewInfo; virtual;
    function AddGroupVertSeparator(APos: Integer)
      : TcxSchedulerGroupSeparatorCellViewInfo; virtual;
    procedure AddResourceBounds(AResourceIndex: Integer; const ABounds: TRect);
    function AddResourceHeader(const AIndex: Integer; const ABounds: TRect)
      : TcxSchedulerHeaderCellViewInfo; virtual;
    procedure AfterCalculate; override;
    function AreThereEventsInVisibleInterval(AResourceIndex: Integer)
      : Boolean; virtual;
    ...
    procedure CalculateNavigationButtonParams(AColumnRect: TRect;
    ...
    function CanCacheGroupSeparator
      (AItem: TcxSchedulerGroupSeparatorCellViewInfo): Boolean; virtual;
    function CanCacheResourceHeader(AItem: TcxSchedulerHeaderCellViewInfo)
      : Boolean; virtual;
    ...
    procedure CreateCellInstance(AClass: TcxSchedulerCustomViewInfoItemClass;
      ...
      AUseRightToLeftAlignment: Boolean; var Instance); overload;
    procedure CreateCellInstance(AClass: TcxSchedulerCustomViewInfoItemClass;
      const ABounds, AVisibleBounds: TRect; const AViewParams: TcxViewParams;
      AUseRightToLeftAlignment: Boolean; var Instance); overload;
    function CreateEventViewData(AEvent: TcxSchedulerControlEvent;
      const ABounds: TRect; const AStart, AFinish: TDateTime;
      AResource: TcxSchedulerResourceViewInfo): TcxSchedulerEventViewData;
      overload; virtual;
    function CreateEventViewData(AEvent: TcxSchedulerControlEvent;
      const ABounds: TRect; const AStart, AFinish: TDateTime;
      AResource: TcxSchedulerResourceViewInfo; const ACurrentDate: TDateTime)
      : TcxSchedulerEventViewData; overload; virtual;
    function CreateEventCellViewInfo(AViewData: TcxSchedulerEventViewData;
      ACanResize: Boolean): TcxSchedulerEventCellViewInfo; virtual;
    ...
    procedure DoContentNavigationButtonClick
      (Sender: TcxSchedulerContentNavigationButtonViewInfo); virtual;
    procedure DoMoreEventsButtonClick
      (Sender: TcxSchedulerMoreEventsButtonViewInfo); virtual;
    procedure DoRightToLeftConversion(const AClientBounds: TRect); virtual;
    function DoSetResourceHasVisibleEvent(AEvent: TcxSchedulerControlEvent)
      : Boolean; virtual;
    procedure DrawNavigationButtons(ACanvas: TcxCanvas;
      ADrawItemProc: TcxSchedulerCustomDrawItemProc);
    procedure ExtractResources; virtual;
    function EventCellClass: TcxSchedulerEventCellViewInfoClass; virtual;
    function GetBounds: TRect; override;
    function GetContentNavigationInterval(AContentNavigationButton
      : TcxSchedulerContentNavigationButtonViewInfo): TDateTime;
    ...
    function GetMoreEventButtonClass
      : TcxSchedulerMoreEventsButtonViewInfoClass; virtual;
    ...
    function GetPageHeaderText: string; virtual;
    function GetResourceItemByIndex(AIndex: Integer)
    ...
    procedure HotTrackMoreEventsButton
      (AHitTest: TcxSchedulerCustomResourceViewHitTest);
    procedure HotTrackNavigationButtons
      (AHitTest: TcxSchedulerCustomResourceViewHitTest);
    ...
    function NavigationButtonOffset
      (AKind: TcxSchedulerContentNavigationButtonKind; AResourceIndex: Integer)
      : Integer; virtual;
    function NeedClearResources(AGroupingKind: TcxSchedulerGroupingKind)
      : Boolean; virtual;
    procedure OnContentNavigationButtonClick
      (Sender: TcxSchedulerContentNavigationButtonViewInfo); virtual;
    procedure OnMoreEventsButtonClick
    ...
    procedure CustomProcessPreviouslyCalculatedEvent
      (AEventCell: TcxSchedulerEventCellViewInfo); virtual;
    procedure ReturnVisibleInterval(var AStart, AEnd: TDateTime); virtual;
    function SetAdapter(Adapter: TcxCustomResourceViewAdapter)
      : TcxCustomResourceViewAdapter;
    ...
    procedure SetResourceTextRect(AResource: TcxSchedulerStorageResourceItem;
      const ARect: TRect);

    property HotContentNavigationButton
      : TcxSchedulerContentNavigationButtonViewInfo
      read FHotContentNavigationButton write SetHotContentNavigationButton;
    property HotMoreEventsButton: TcxSchedulerMoreEventsButtonViewInfo
      read FHotMoreEventsButton write SetHotMoreEventsButton;
    property ImageCacheManager: TcxSchedulerImageCacheManager
      read FImageCacheManager;
  ...
  public
    ...
    procedure CalculateHitTest(AHitTest
      : TcxSchedulerCustomResourceViewHitTest); virtual;
    procedure InitScrollBarsParameters; virtual;
    procedure ScrollHorizontal(AScrollCode: TScrollCode;
      var AScrollPos: Integer); virtual;
    procedure ScrollVertical(AScrollCode: TScrollCode;
      var AScrollPos: Integer); virtual;
    procedure SetEventsVisibility(AShowSources, AShowClones: Boolean;
      AForceRepaint: Boolean = False); override;
    ...
    property ShowEventsWithoutResource: Boolean
      read GetShowEventsWithoutResource;
    property GroupByDate: Boolean read GetGroupByDate;
    property GroupByResource: Boolean read GetGroupByResource;
    property GroupSeparatorCells: TcxSchedulerViewInfoCellList
      read FGroupSeparatorCells;
    ...
    property HeaderContainerCells: TcxSchedulerViewInfoCellList
      read FHeaderContainerCells;
    property HideSelection: Boolean read FHideSelection;
    property NavigationButtons: TcxSchedulerViewInfoCellList
      read FNavigationButtons;
    ...
    property Resources[AIndex: Integer]: TcxSchedulerResourceViewInfo
      read GetResource;
    property ResourceCaptions[AIndex: Integer]: string read GetResourceCaption;
    property ResourceCount: Integer read GetResourceCount;
    property ResourceHeaders: TcxSchedulerResourceHeaders
      read GetResourceHeaders;
    property ResourceHeaderCells: TcxSchedulerViewInfoCellList
      read FResourceHeaderCells;
  ...
  private
    ...
    function GetViewInfo: TcxSchedulerCustomResourceViewViewInfo;
  protected
    function GetNextResource(AGoToForward: Boolean)
      : TcxSchedulerStorageResourceItem;
    function GetResourceItem: TcxSchedulerStorageResourceItem; override;
    function GetResourceFromViewInfo(AInfo: TcxSchedulerResourceViewInfo)
      : TcxSchedulerStorageResourceItem;
  ...
  public
    ...
    property ResourceItem: TcxSchedulerStorageResourceItem read GetResourceItem;
    property ResourceObject: TcxSchedulerResourceViewInfo
      read GetResourceObject;
    property ViewInfo: TcxSchedulerCustomResourceViewViewInfo read GetViewInfo;
  end;
  ...
  protected
    ...
    property NavigationButtonClicked: Boolean read FNavigationButtonClicked
      write FNavigationButtonClicked;
    property View: TcxSchedulerCustomResourceView read GetView;
  end;
  ...
  private
    ...
    function GetHitContentNavigationButton
      : TcxSchedulerContentNavigationButtonViewInfo;
  ...
  protected
    ...
    property View: TcxSchedulerCustomResourceView read GetView;
    property HitAtTimeZoneLabel: Boolean index htcTimeZoneLabel
      read GetBitState;
    property HitContentNavigationButton
      : TcxSchedulerContentNavigationButtonViewInfo
      read GetHitContentNavigationButton;
    property HitMoreEventsButton: TcxSchedulerMoreEventsButtonViewInfo
      read GetHitMoreEventsButton;
    property TimeZone: Integer read FTimeZone;
  public
    ...
    property ContentNavigationButton
      : TcxSchedulerContentNavigationButtonViewInfo
      read FContentNavigationButton; // for internal use
    ...
    property HitAtContentNavigationButton: Boolean index htcNavigationButton
      read GetBitState;
    property HitAtDayHeader: Boolean index htcDayHeader read GetBitState;
    property HitAtGroupSeparator: Boolean index htcGroupSeparator
      read GetBitState;
    property HitAtResourceHeader: Boolean index htcResourceHeader
      read GetBitState;
  end;
  ...
  protected
    ...
    procedure DrawBackgroundCell
      (AItem: TcxSchedulerBackgroundCellViewInfo); virtual;
    procedure DrawButtonCell(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean); virtual;
    procedure DrawContentCell(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean); virtual;
    procedure DrawEventCell(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean); virtual;
    procedure DrawGroupSeparatorCell(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean); virtual;
    procedure DrawHeaderCell(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean); virtual;
    procedure DrawResourceHeaderCell(AItem: TcxSchedulerCustomViewInfoItem;
      var ADone: Boolean); virtual;
  public
    constructor Create(AOwner: TcxSchedulerSubControl); override;
  ...
  TcxSchedulerEventLayoutBuilderGetEventPlaceProc = function
    (Sender: TcxSchedulerEventLayoutBuilder; AEvent: TcxSchedulerControlEvent;
    out AStartCol, AFinishCol: Integer; out ALineCount: Integer)
    : Boolean of object;
  ...
  public
    ...
    function AddEventPlace(AEvent: TcxSchedulerEvent;
      AStartCol, AFinishCol: Integer; ALineCount: Integer = 1;
      AResource: TObject = nil): TcxSchedulerEventPlace;
    procedure Clear;
    ...
    property EventPlaces[Index: Integer]: TcxSchedulerEventPlace
      read GetEventPlace;
  end;

var
  ExternalPainterClass: TcxSchedulerExternalPainterClass =
    TcxSchedulerExternalPainter;

function MakeTextOutcxFlags(AlignHorz: TAlignment; AlignVert: TcxAlignmentVert;
  AMultiline: Boolean = False): Integer;

implementation

uses
  RTLConsts, cxDrawTextUtils, cxLibraryConsts, cxVariants,
...
const
  ...
  MultiLines: array [Boolean] of Integer = (0, cxWordBreak or
    cxNoFullWidthCharBreak);
  cxDrawTextFlags: array [Boolean] of Cardinal = (cxAlignLeft or
    cxAlignVCenter or cxSingleLine, cxAlignLeft or cxAlignTop or cxWordBreak or
    cxDontBreakChars or cxNoFullWidthCharBreak);

type
  { TcxSchedulerEventPlaceComparer }

  TcxSchedulerEventPlaceComparer = class(TInterfacedObject,
    IComparer<TcxSchedulerEventPlace>)
  strict private
    FViewStyle: TcxSchedulerViewStyle;
  protected
...
function GetTrueMultilineTextHeight(ACanvas: TcxCanvas; const S: string;
  ABounds: TRect; ATextFlags: Cardinal): Integer;
var
  ARowCount: Integer;
...
begin
  ...
  cxMakeTextRows(ACanvas.Canvas, PChar(S), Length(S), ABounds, ATextParams,
    ATextRows, ARowCount);
  Result := ATextParams.RowHeight * Max(ARowCount, 1);
  cxResetTextRows(ATextRows);
end;

function cxCompareEventPlaces(AEventPlace1, AEventPlace2
  : TcxSchedulerEventPlace): Integer;
var
...
begin
  if AEventPlace1 = AEventPlace2 then
    Exit(0);
  Result := TdxNativeInt(AEventPlace1.Resource) -
    TdxNativeInt(AEventPlace2.Resource);
  if Result <> 0 then
    Exit;
  ...
  if AEventPlace1.ColStart < AEventPlace2.ColStart then
    Result := -1
  else if AEventPlace1.ColStart > AEventPlace2.ColStart then
    Result := 1
  else if AEventPlace1.ColFinish > AEventPlace2.ColFinish then
    Result := -1
  else if AEventPlace1.ColFinish < AEventPlace2.ColFinish then
    Result := 1
...
function cxCompareEventModernPlaces(AEventPlace1, AEventPlace2
  : TcxSchedulerEventPlace): Integer;
var
  AAllDay1, AAllDay2: Boolean;
  AEvent1, AEvent2: TcxSchedulerEvent;
  ...
  begin
    ...
    begin
      if Int(AEvent1.Start) < Int(AEvent2.Start) then
        Result := -1
      else if Int(AEvent1.Start) > Int(AEvent2.Start) then
        Result := 1
      else if Int(AEvent1.Finish) > Int(AEvent2.Finish) then
        Result := -1
      else if Int(AEvent1.Finish) < Int(AEvent2.Finish) then
        Result := 1
    end
...
constructor TcxSchedulerEventPlaceComparer.Create
  (AViewStyle: TcxSchedulerViewStyle);
begin
  inherited Create;
  FViewStyle := AViewStyle;
end;

function TcxSchedulerEventPlaceComparer.Compare(const Left,
  Right: TcxSchedulerEventPlace): Integer;
begin
...
function TcxSchedulerOccupiedArea.CanMergeHorizontal(const ABounds
  : TRect): Boolean;
begin
  Result := (FBounds.Top = ABounds.Top) and
    (cxRectHeight(FBounds) = cxRectHeight(ABounds)) and
    (RightOf(ABounds) or LeftOf(ABounds));
end;

function TcxSchedulerOccupiedArea.CanMergeVertical(const ABounds
  : TRect): Boolean;
begin
  Result := (FBounds.Left = ABounds.Left) and
    (cxRectWidth(FBounds) = cxRectWidth(ABounds)) and
    (BottomOf(ABounds) or TopOf(ABounds));
end;
...
function TcxSchedulerOccupiedArea.Merge(AOccupiedArea
  : TcxSchedulerOccupiedArea): Boolean;
begin
...
function TcxSchedulerOccupiedAreaManager.AddArea(const ABounds: TRect)
  : TcxSchedulerOccupiedArea;
var
  I: Integer;
...
function TcxSchedulerOccupiedAreaManager.MergeRecursively
  (var AArea: TcxSchedulerOccupiedArea): Boolean;
var
  I: Integer;
  AOldArea: TcxSchedulerOccupiedArea;
begin
...
function TcxSchedulerOccupiedAreaManager.GetItem(AIndex: Integer)
  : TcxSchedulerOccupiedArea;
begin
  Result := TcxSchedulerOccupiedArea(FAreas[AIndex]);
end;

function TcxSchedulerOccupiedAreaManager.GetFreeBounds(const AEventPlaceBounds
  : TRect): TRect;
var
  I: Integer;
...
begin
  ...
  begin
    ...
    if (AOccupiedArea.Bounds.Right >= Result.Left) and
      (AOccupiedArea.Bounds.Left <= Result.Right) and
      (AOccupiedArea.Bounds.Bottom >= Result.Top) and
      (AOccupiedArea.Bounds.Top <= Result.Bottom) then
    begin
...
begin
  ...
  for I := Count - 1 downto 0 do
    if Items[I].GetHitTest(AHitTest, Result) and Result then
      Exit;
end;

procedure TcxSchedulerViewInfoCellList.DoRightToLeftConversion
  (const AClientBounds: TRect);
var
  I: Integer;
begin
...
procedure TcxSchedulerViewInfoCellList.Draw(ACanvas: TcxCanvas;
  ADrawItemProc: TcxSchedulerCustomDrawItemProc);
var
  I: Integer;
  ADone: Boolean;
...
begin
  ...
  begin
    AItem := Items[I];
    if AItem.FVisible and not cxRectIsEmpty(AItem.ActiveBounds) and
      ACanvas.RectVisible(AItem.ActiveBounds) then
    begin
...
function TcxSchedulerViewInfoCellList.GetItem(AIndex: Integer)
  : TcxSchedulerCustomViewInfoItem;
begin
  Result := List[AIndex];
end;
...
constructor TcxSchedulerCustomViewInfoItem.Create
  ...
  const AViewParams: TcxViewParams; AScaleFactor: TdxScaleFactor;
...
procedure TcxSchedulerCustomViewInfoItem.CalculateCellBounds(const ABounds,
  AVisibleRect: TRect);
begin
  FBounds := ABounds;
...
function TcxSchedulerCustomViewInfoItem.CalculateClipRect(const ABounds,
  AVisibleRect: TRect): TRect;
begin
...
begin
  Inc(FClipRef);
  if not AHasClipping or (FClipRef > 1) then
    Exit;
...
procedure TcxSchedulerCustomViewInfoItem.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
...
procedure TcxSchedulerCustomViewInfoItem.DoRightToLeftConversionWithChecking
  (var ARect: TRect; const AClientBounds: TRect);
begin
  if not cxRectIsNull(ARect) then
    ARect := TdxRightToLeftLayoutConverter.ConvertRect(ARect, AClientBounds);
end;

function TcxSchedulerCustomViewInfoItem.DrawBackground(ACanvas: TcxCanvas;
  const ABounds: TRect): Boolean;
begin
...
procedure TcxSchedulerCustomViewInfoItem.DrawFrame(const ARect: TRect;
  ABorders: TcxBorders = []; ABorderColor: TColor = clDefault;
  ABorderSize: Integer = 1);
begin
  Canvas.FrameRect(ARect, ABorderColor, ABorderSize, ABorders);
end;

procedure TcxSchedulerCustomViewInfoItem.DrawRect(const ARect: TRect;
  ABorders: TcxBorders = []; ABorderColor: TColor = clDefault;
  ABorderSize: Integer = 1);
begin
  if Transparent then
...
procedure TcxSchedulerCustomViewInfoItem.DrawText(const ARect: TRect;
  const AText: string; AFlags: Integer; AFont: TFont = nil;
...
function TcxSchedulerCustomViewInfoItem.GetActualRectRTL(const ARect,
  AClientBounds: TRect; AOffsetIfRTL: Integer = 0): TRect;
begin
  Result := ARect;
...
procedure TcxSchedulerCustomViewInfoItem.UpdateCachedImage(const AViewParams
  : TcxViewParams);
var
  P: TPoint;
begin
...
function TcxSchedulerCustomViewInfoItem.GetDateTimeHelper
...
function TcxSchedulerCustomViewInfoItem.GetPainterHelper
  : TcxSchedulerPainterHelperClass;
begin
...
constructor TcxSchedulerCustomHeaderCellViewInfo.Create
  (APainter: TcxCustomLookAndFeelPainter; const ABounds, AVisibleRect: TRect;
  const AViewParams: TcxViewParams; AScaleFactor: TdxScaleFactor;
  AUseRightToLeftAlignment: Boolean);
begin
  inherited Create(APainter, ABounds, AVisibleRect, AViewParams, AScaleFactor,
...
begin
  ...
  begin
    ...
    begin
      ...
      InflateRect(FTextRect, -(ATextOffset + ScaleFactor.Apply(1)),
        -(ATextOffset + ScaleFactor.Apply(1)));
    end
  ...
  begin
    ...
    ABounds := MakeRect(0, 0, cxSize(Height - ScaleFactor.Apply(2),
      Width - ScaleFactor.Apply(2)));
  ...
  DisplayBounds := cxRectInflate(ABounds, -ScaleFactor.Apply(1),
    -ScaleFactor.Apply(1));
  ...
  ATopLeft := cxPointOffset(cxRectCenter(ABounds), ScaleFactor.Apply(1),
    -ScaleFactor.Apply(1) + Byte(RotateHeader));
  if ImagePosition in [ipLeft, ipRight] then
  begin
    Dec(ATopLeft.X, (AImageSize.cx + ATextSize.cx + ATextOffset) div 2);
    FTextRect := MakeRect(ATopLeft.X, ATopLeft.Y - ATextSize.cy div 2,
      ATextSize);
    FImageRect := MakeRect(ATopLeft.X, ATopLeft.Y - AImageSize.cy div 2,
      AImageSize);
  end
  else
  begin
    Dec(ATopLeft.Y, (AImageSize.cy + ATextSize.cy + ATextOffset) div 2);
    FTextRect := MakeRect(ATopLeft.X - ATextSize.cx div 2, ATopLeft.Y,
      ATextSize);
    FImageRect := MakeRect(ATopLeft.X - AImageSize.cx div 2, ATopLeft.Y,
      AImageSize);
  end;
...
procedure TcxSchedulerCustomHeaderCellViewInfo.CheckNeighbor
  (APrevCell: TcxSchedulerCustomHeaderCellViewInfo);
begin
  if (APrevCell = nil) or (APrevCell.RotateHeader <> RotateHeader) then
    Exit;
...
begin
  ...
  if Transparent then
    cxBitBlt(FRotateBitmap.Canvas.Handle, Canvas.Handle,
      FRotateBitmap.ClientRect, Bounds.TopLeft, srcCopy);
...
procedure TcxSchedulerCustomHeaderCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
...
procedure TcxSchedulerCustomHeaderCellViewInfo.DrawCaption
  (ACanvas: TcxCanvas = nil);
var
  ABitmap: TcxBitmap;
  AImageBounds: TRect;
begin
      ...
      begin
        ...
        try
          ...
          cxBitBlt(ABitmap.Canvas.Handle, ACanvas.Handle, ABitmap.ClientRect,
            FImageRect.TopLeft, srcCopy);
          ABitmap.Rotate(raPlus90);
          cxDrawImage(ABitmap.cxCanvas, AImageBounds, nil, Images, ImageIndex,
            ifmNormal, idmNormal, True, nil, ScaleFactor);
          ABitmap.Rotate(raMinus90);
        ...
        cxDrawImage(ACanvas, FImageRect, nil, Images, ImageIndex, ifmNormal,
          idmNormal, True, nil, ScaleFactor);
    end;
  finally
...
begin
  if not Transparent then
    Painter.DrawScaledHeader(Canvas, Bounds, TextRect, Neighbors, Borders,
      FButtonState, AlignHorz, AlignVert, MultiLine, ShowEndEllipsis, '', Font,
      TextColor, Color, ScaleFactor, DrawBackground, not(nRight in Neighbors));
...
function TcxSchedulerCustomHeaderCellViewInfo.DrawRotateBackground
...
begin
  ...
  cxBitBlt(FSelectionBitmap.Canvas.Handle, Canvas.Handle,
    FSelectionBitmap.ClientRect, Bounds.TopLeft, srcCopy);
  cxAlphaBlend(FSelectionBitmap, cxRectOffset(FSelectionRect, -Bounds.Left,
    -Bounds.Top), ColorToRgb(SelectionColor));
  cxBitBlt(Canvas.Handle, FSelectionBitmap.Canvas.Handle, Bounds,
    cxNullPoint, srcCopy);
end;
  ...
  begin
    ...
    begin
      ...
      Painter.DrawScaledHeader(FRotateBitmap.cxCanvas, R,
        cxRectInflate(R, -ScaleFactor.Apply(cxTextOffset),
        -ScaleFactor.Apply(cxTextOffset)), Neighbors, Borders, cxbsNormal,
        taCenter, vaCenter, False, False, '', Font, TextColor, Color,
        ScaleFactor, DrawRotateBackground);
      FRotateBitmap.Rotate(AAngles[UseRightToLeftAlignment], True);
    end;
...
begin
  Result := cxRectInflate(PainterHelper.ExcludeBorders(Bounds, Borders),
    -ScaleFactor.Apply(cxTextOffset), 0);
end;

function TcxSchedulerCustomHeaderCellViewInfo.GetImagePositionRTL
...
const
  Horz: array [Boolean, TAlignment] of Integer = ((CXTO_LEFT, CXTO_RIGHT,
    CXTO_CENTER_HORIZONTALLY), (CXTO_RIGHT, CXTO_LEFT,
    CXTO_CENTER_HORIZONTALLY));
  Vert: array [TcxAlignmentVert] of Integer = (CXTO_TOP, CXTO_BOTTOM,
    CXTO_CENTER_VERTICALLY);
begin
  if AutoHeight then
    Result := CXTO_PREVENT_LEFT_EXCEED or CXTO_PREVENT_TOP_EXCEED or
      Horz[UseRightToLeftAlignment, AlignHorz] or Vert[AlignVert] or
      CXTO_WORDBREAK
  else
    Result := CXTO_PREVENT_LEFT_EXCEED or CXTO_PREVENT_TOP_EXCEED or
      Horz[UseRightToLeftAlignment, AlignHorz] or Vert[AlignVert] +
      CXTO_SINGLELINE;
end;
...
procedure TcxSchedulerCustomHeaderCellViewInfo.InitHitTest
  (AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
...
procedure TcxSchedulerCustomHeaderCellViewInfo.TextOut(ACanvas: TcxCanvas;
  ...
  begin
    ...
    begin
      FSelectionRect.Left := Max(FSelectionRect.Left, FSelectionRect.Right -
        PainterHelper.TextWidth(Font, DisplayText) - 2 *
...
function TcxSchedulerDayHeaderCellViewInfo.DrawBackground(ACanvas: TcxCanvas;
  const ABounds: TRect): Boolean;
var
  R: TRect;
begin
...
function TcxSchedulerDayHeaderCellViewInfo.
  GetDefaultTextRectForModernStyle: TRect;
var
  AMinLeft: Integer;
begin
...
function TcxSchedulerDayHeaderCellModernViewInfo.ConvertDateToDisplayText
  (AType: Integer = 0): Integer;
var
  AOffsets: Integer;
  ADay, ADate: string;
  ...
  case ActualModernStyleDisplayMode of
    ...
    begin
      ...
      repeat
        ...
        if cxRectWidth(FTextRect) <= 0 then
          Break;
        ADate := AnsiUpperCase(DateTimeHelper.DayToStr(FDateTime, Result,
          Compressed));
        ADay := '';
        if Pos(AnsiUpperCase('DDD'),
          AnsiUpperCase(DateTimeHelper.GetDayMonthFormat(Result, False))) = 0
        then
...
function TcxSchedulerDayHeaderCellModernViewInfo.DrawBackground
  (ACanvas: TcxCanvas; const ABounds: TRect): Boolean;
begin
  if ActualModernStyleDisplayMode = hdmClassic then
  begin
  ...
  if Result and not Transparent then
    ACanvas.FillRect(ABounds, Bitmap);
  ACanvas.FrameRect(ABounds,
    ExternalPainter.Painter.DefaultSchedulerDayHeaderBorderColor, 1, [bBottom]);
  if not Result then
  begin
    if FButtonState = cxbsHot then
    begin
      ACanvas.FillRect(ABounds,
        ExternalPainter.Painter.
        DefaultSchedulerHeaderContainerAlternateBackgroundColor);
      FBackgroundDrawing := True;
    end
    else
...
procedure TcxSchedulerDayHeaderCellModernViewInfo.DrawCaption
  (ACanvas: TcxCanvas = nil);
var
  AStyles: TFontStyles;
begin
...
function TcxSchedulerDayHeaderCellModernViewInfo.GetDayText
...
begin
  ...
  until (Result = 3) or (cxTextWidth(Font, DisplayText) <=
    cxRectWidth(FTextRect));
...
begin
  ...
  begin
    case AType of
      0:
        Result := dxFormatSettings.LongDayNames[7] + '/' +
          dxFormatSettings.LongDayNames[1];
      1:
        Result := dxFormatSettings.ShortDayNames[7] + '/' +
          dxFormatSettings.ShortDayNames[1];
    else
      Result := AnsiUpperCase(dxFormatSettings.LongDayNames[7][1] + '/' +
        dxFormatSettings.LongDayNames[1][1]);
    end;
  end
  else
    case AType of
...
function TcxSchedulerWeekDayHeaderCellModernViewInfo.DrawBackground
  (ACanvas: TcxCanvas; const ABounds: TRect): Boolean;
begin
  Result := inherited DrawBackground(ACanvas, ABounds);
  if not Result then
...
function TcxSchedulerWeekDayHeaderCellModernViewInfo.GetDayText
  (AType: Integer): string;
begin
  AlignHorz := taLeftJustify;
...
procedure TcxSchedulerContainerCellViewInfo.Calculate(ADate: TDateTime;
  ASelected: Boolean;
  ACaptionDisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode;
  AHeadersEventsAreaTop: Integer);
begin
  FDateTime := ADate;
  FSelected := ASelected;
...
begin
  FExternalPainter.DrawAllDayArea(Canvas, Bounds, GetBorderColor, Borders,
...
procedure TcxSchedulerContainerModernCellViewInfo.Calculate(ADate: TDateTime;
  ASelected: Boolean;
  ACaptionDisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode;
  AHeadersEventsAreaTop: Integer);
var
  AOffset: Integer;
begin
  inherited Calculate(ADate, ASelected, ACaptionDisplayMode,
    AHeadersEventsAreaTop);
...
begin
  if (HeaderEvent <> nil) and
    ((Resource = nil) or VarEquals(Resource.ResourceID,
    HeaderEvent.Event.ResourceID)) and (HeaderEvent.Event.State > 0) and not Selected
  then
  begin
    FExternalPainter.DrawAllDayArea(Canvas, cxRectSetHeight(Bounds,
      FFirstEventTop - Bounds.Top), GetBorderColor, Borders - [bBottom],
      ViewParams, Selected, Transparent);

    AContentColor := ExternalPainter.GetColorizedColor(ViewParams.Color,
      HeaderEvent);
    ...
    begin
      FExternalPainter.DrawAllDayArea(Canvas, ABounds, GetBorderColor, [],
        ViewParams, Selected, Transparent);
      cxFillRectWithCustomBrush(Canvas.Canvas, StateBrushes[9], ABounds,
        AContentColor, ExternalPainter.GetColorizedColor(GetBorderColor,
        HeaderEvent), ViewParams.Bitmap <> nil);
    ...
    begin
      ...
      FExternalPainter.DrawAllDayArea(Canvas, ABounds, GetBorderColor, Borders,
        AViewParams, Selected, Transparent);
      if IsDrawBySkin then
        Canvas.Rectangle(PainterHelper.ExcludeBorders(ABounds, [bRight, bBottom]
          ), AViewParams, [], GetBorderColor, 1);
    end;
  end
  else
    inherited DoDraw;
...
procedure TcxSchedulerContainerModernCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  FTextRect := TdxRightToLeftLayoutConverter.ConvertRect(FTextRect,
    AClientBounds);
end;

procedure TcxSchedulerContainerModernCellViewInfo.DrawCaption;
const
  AFlags: array [Boolean] of Integer = (cxAlignTop or cxAlignLeft,
    cxAlignTop or cxAlignRight);
var
  AStyle: TFontStyles;
begin
  AStyle := Font.Style;
  if IsToday then
    Font.Style := Font.Style + [fsBold];
  DrawText(FTextRect, DisplayText, AFlags[UseRightToLeftAlignment], Font,
    TextColor);
  Font.Style := AStyle;
end;
...
procedure TcxSchedulerTimeRulerCellViewInfo.Calculate(const ALabel1,
  ALabel2: string; ATimeZone, AAdditionalTimeZone: Integer;
  AAdditionalTimeZoneBiasDelta: Integer;
  ASelectedDays: TcxSchedulerDateList = nil);
begin
  Calculate(-1, 1, ATimeZone, AAdditionalTimeZone, nil,
    AAdditionalTimeZoneBiasDelta, ASelectedDays);
  FDisplayTexts[True] := ALabel1;
  FDisplayTexts[False] := ALabel2;
end;

procedure TcxSchedulerTimeRulerCellViewInfo.Calculate(AHour,
  ALineCount: Integer; ATimeZone, AAdditionalTimeZone: Integer;
  ALargeFont: TFont; AAdditionalTimeZoneBiasDelta: Integer;
  ASelectedDays: TcxSchedulerDateList = nil);
begin
  FHour := AHour;
...
class function TcxSchedulerTimeRulerCellViewInfo.CalculateWidth(ATimeZoneCount,
  ALineCount: Integer; AFont1, AFont2: TFont;
...
begin
  ...
  Result := GetBaseWidth(ALineCount, AFont1, AFont2, AScaleFactor,
    AShowMinutes);
  ...
  begin
    if Hour >= 0 then
      ...
      DrawTimeZoneLabel(Bounds[ACurrentTimeZone],
        DisplayTexts[ACurrentTimeZone], GetLabelZoneBorders(ACurrentTimeZone));
  end;
  ClippingRestore;
end;

procedure TcxSchedulerTimeRulerCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  FBounds[True] := TdxRightToLeftLayoutConverter.ConvertRect(FBounds[True],
    AClientBounds);
  FBounds[False] := TdxRightToLeftLayoutConverter.ConvertRect(FBounds[False],
    AClientBounds);
  FVisibleRect := TdxRightToLeftLayoutConverter.ConvertRect(FVisibleRect,
    AClientBounds);
end;
...
begin
  R := ABounds;
  cxRightToLeftDependentDraw(Canvas.Handle, ABounds,
    Canvas.UseRightToLeftAlignment, procedure
    begin
      FExternalPainter.DrawTimeRulerBackground(Canvas, R, ABorders,
        GetBorderColor, ViewParams, Transparent);
    end, False);
  DrawText(GetActualRectRTL(GetLabelZoneTextRect(ABounds), ABounds), AText,
    cxAlignBottom or cxAlignLeft, Font, TextColor);
end;

procedure TcxSchedulerTimeRulerCellViewInfo.DrawTimeZone(const ABounds: TRect;
...
var
  ...
  R, R1, TextR: TRect;
const
  cxRightCenterAlign: array [Boolean] of Integer = (cxAlignVCenter or
    cxAlignRight or cxDontClip, cxAlignVCenter or cxAlignLeft or cxDontClip);
  cxRightTopAlign: array [Boolean] of Integer = (cxAlignTop or cxAlignRight or
    cxDontClip, cxAlignTop or cxAlignLeft or cxDontClip);
begin
  ...
  cxRightToLeftDependentDraw(Canvas.Handle, ABounds,
    Canvas.UseRightToLeftAlignment, procedure
    begin
      FExternalPainter.DrawTimeRulerBackground(Canvas, R, [bRight],
        GetBorderColor, ViewParams, Transparent);
    end, False);
  ...
  begin
    Canvas.FrameRect(GetActualRectRTL(R, ABounds), GetBorderColor,
      ScaleFactor.Apply(1), [bBottom]);
    OffsetRect(R, 0, LH);
  end;
  Canvas.Brush.Style := bsClear;
  TextR := cxRectInflate(PainterHelper.ExcludeBorders(ABounds,
    GetRealRightBorder + [bBottom]), -ScaleFactor.Apply(5),
    -ScaleFactor.Apply(cxTextOffset));
  if CanDrawCurrentTime and AIsCurrent and FExternalPainter.DrawCurrentTimeFirst
  then
  ...
  begin
    ...
    DrawText(GetActualRectRTL(Rect(TextR.Left, TextR.Top,
      R.Left - ScaleFactor.Apply(cxTextOffset), TextR.Bottom), ABounds, 1),
      Copy(AText, 1, APos - 1), cxRightTopAlign[UseRightToLeftAlignment],
      LargeFont, TextColor);
    R1 := Rect(R.Left, TextR.Top, TextR.Right,
      TextR.Top + LH - ScaleFactor.Apply(3));
    DrawText(GetActualRectRTL(R1, ABounds), Copy(AText, APos + 1, J - APos),
      cxAlignCenter, Font, TextColor);
    if ShowMinutes and AIsCurrent then
      ...
      begin
        ...
        if Length(S) < 2 then
          S := '0' + S;
        DrawText(GetActualRectRTL(R1, ABounds), S, cxAlignCenter, Font,
          TextColor);
      end;
  end
  else
    DrawText(GetActualRectRTL(TextR, ABounds, 1), AText,
      cxRightCenterAlign[UseRightToLeftAlignment], Font, TextColor);
  if CanDrawCurrentTime and AIsCurrent and not FExternalPainter.DrawCurrentTimeFirst
  then
    DrawCurrentTime(FDateTime, ABounds);
...
begin
  ...
  FExternalPainter.DrawCustomCurrentTime(Canvas, Color, AStart - ADelta,
    ABounds, UseRightToLeftAlignment);
  Canvas.Brush.Style := bsClear;
end;
...
class function TcxSchedulerTimeRulerCellViewInfo.GetBaseWidth
  (ALineCount: Integer; AFont1, AFont2: TFont; AScaleFactor: TdxScaleFactor;
AShowMinutes: Boolean): Integer;
begin
...
function TcxSchedulerTimeRulerCellViewInfo.GetCanDrawCurrentTime
  (ASelectedDays: TcxSchedulerDateList): Boolean;
begin
  Result := True;
end;

function TcxSchedulerTimeRulerCellViewInfo.GetLabelZoneBorders
  (const AIsCurrentTimeZone: Boolean): TcxBorders;
begin
  Result := [bBottom];
...
function TcxSchedulerTimeRulerCellViewInfo.GetLabelZoneTextRect
...
begin
  ...
  begin
    ...
    Result := Copy(Result, 1, APos - 1) + ' ' +
      AnsiLowerCase(Copy(Result, APos + 1, 2));
  end;
end;
...
begin
  ...
  begin
    ...
    ATime := (AHitTest.HitY - inherited Bounds.Top) div LH * ATimePerLine +
      FHour * HourToTime;
...
procedure TcxSchedulerTimeRulerModernCellViewInfo.DrawBottomLine
  (const ABounds: TRect);
var
  R: TRect;
begin
...
procedure TcxSchedulerTimeRulerModernCellViewInfo.DrawTimeZone
  ...
  begin
    ...
    if not Is24HourTimeFormat then
      Result := cxTextWidth(Font, TcxSchedulerDateTimeHelper.GetMeridiem(True,
        True)) + ScaleFactor.Apply(cxTextOffset);
    if ShowMinutes then
      Result := Max(Result, (ABounds.Right - ABounds.Left) div 4);
  end;

const
  AAlignOnVCenter: array [Boolean] of Integer = (cxAlignVCenter or cxDontClip,
    cxAlignTop or cxDontClip);
  AAlignMinutes: array [Boolean] of Integer = (cxAlignTop or cxAlignRight,
    cxAlignTop or cxAlignLeft);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Length(st) < 2 then
          st := '0' + st;
        DrawText(GetActualRectRTL(R1, ABounds, 1), st,
          AAlignMinutes[UseRightToLeftAlignment], Font, TextColor);
      end;
      OffsetRect(R, 0, LH);
    end;
  end;
  ...
  then
    DrawCurrentTime(FDateTime, ABounds);
  R := cxRectInflate(PainterHelper.ExcludeBorders(ABounds,
    GetRealRightBorder + [bBottom]), -GetBottomLineIndent,
    -ScaleFactor.Apply(cxTextOffset));
  R.Right := ABounds.Right - InternalRightIndent;
  if Is24HourTimeFormat then
    DrawText(GetActualRectRTL(R, ABounds, 1), AText,
      AAlignOnVCenter[LineCount > 1] or cxAlignHCenter, LargeFont, TextColor)
  else
  begin
    st := AText;
    APos := Pos(' ', AText);
    ...
    begin
      ...
      DrawText(GetActualRectRTL(R1, ABounds, 1), st, AAlignOnVCenter[True] or
        AAlignHorz[UseRightToLeftAlignment], Font, TextColor);
      st := Copy(AText, 1, APos - 1);
    end;
    DrawText(GetActualRectRTL(R, ABounds, 1), st,
      AAlignOnVCenter[LineCount > 1] or cxAlignHCenter, LargeFont, TextColor);
  end;
  if CanDrawCurrentTime and AIsCurrent and not FExternalPainter.DrawCurrentTimeFirst
  then
...
procedure TcxSchedulerTimeRulerModernCellViewInfo.DrawTimeZoneLabel
  (const ABounds: TRect; const AText: string; ABorders: TcxBorders);
begin
...
class function TcxSchedulerTimeRulerModernCellViewInfo.GetBaseWidth
  (ALineCount: Integer; AFont1, AFont2: TFont; AScaleFactor: TdxScaleFactor;
AShowMinutes: Boolean): Integer;
begin
  if not Is24HourTimeFormat or AShowMinutes then
    Result := TcxSchedulerPainterHelper.TextWidth(AFont2, ' 24') +
      cxTextWidth(AFont1, 'PM') + AScaleFactor.Apply(cxTextOffset)
  else
    Result := TcxSchedulerPainterHelper.TextWidth(AFont2, ' 24 ');
end;
...
function TcxSchedulerTimeRulerModernCellViewInfo.GetCanDrawCurrentTime
  (ASelectedDays: TcxSchedulerDateList): Boolean;
var
  I: Integer;
  ADate: TDate;
...
function TcxSchedulerTimeRulerModernCellViewInfo.GetLabelZoneBorders
  (const AIsCurrentTimeZone: Boolean): TcxBorders;
begin
  if AIsCurrentTimeZone then
    Result := [bRight]
...
function TcxSchedulerTimeRulerModernCellViewInfo.GetLabelZoneTextRect
  (const ABounds: TRect): TRect;
begin
  Result := cxRectInflate(ABounds, -2 * cxTextOffset, -cxTextOffset);
end;

function TcxSchedulerTimeRulerModernCellViewInfo.GetTimeDisplayText
  (const ATime: TDateTime): string;
var
  AIsPM: Boolean;
  H, M, S: Word;
begin
  ...
  begin
    ...
    begin
      ...
      if InRange(Bounds[True].Top, FVisibleRect.Top,
        FVisibleRect.Top + cxRectHeight(Bounds[True]) - 1) then
        Result := Result + ' ' + TcxSchedulerDateTimeHelper.GetMeridiem
          (AIsPM, True);
    end;
  end;
end;
...
procedure TcxSchedulerContentCellViewInfo.SetTime(const ATimeStart,
  ATimeFinish: TDateTime);
begin
...
begin
  ...
  begin
    ...
    if FTimeEvent = nil then
      FExternalPainter.DrawTimeLine(Canvas, GetActualRectRTL(R, Bounds),
        FTimeLineParams, FTimeLineBorders, FTimeLineParams.TextColor)
    else
      FTimeEvent.DrawState(Canvas, GetActualRectRTL(R, Bounds),
        FTimeLineBorders, FTimeLineParams.TextColor);
  end
  else
...
begin
  ...
  begin
    ABounds := Bounds;
    ABounds.Top := FFirstEventTop;
    SpecialDrawRect(cxRectSetHeight(Bounds, ABounds.Top - Bounds.Top),
      Borders - [bBottom], AContentColor, BorderColor);

    AContentColor := ExternalPainter.GetColorizedColor(AContentColor,
      TimeEvent);
    ABorderColor1 := ExternalPainter.GetColorizedColor
      (ExternalPainter.Painter.DefaultSchedulerHeaderContainerBorderColor,
      TimeEvent);
    SpecialDrawRect(ABounds, Borders - [bTop], AContentColor, BorderColor);
    if TimeEvent.Event.State = 1 then
      cxFillRectWithCustomBrush(Canvas.Canvas, StateBrushes[9],
        PainterHelper.ExcludeBorders(ABounds, Borders), AContentColor,
        ABorderColor1, ViewParams.Bitmap <> nil);
  end
  else
    SpecialDrawRect(Bounds, Borders, AContentColor, BorderColor);
end;

procedure TcxSchedulerContentCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  FTimeLineBorders := TdxRightToLeftLayoutConverter.ConvertBorders
    (FTimeLineBorders);
end;
...
procedure TcxSchedulerContentCellViewInfo.SetContentSelectionParameters
  (ASelected: Boolean; const ASelectionParams: TcxViewParams);
begin
...
procedure TcxSchedulerContentCellViewInfo.SpecialDrawRect(const ARect: TRect;
ABorders: TcxBorders; AContentColor, ABorderColor: TColor);
var
  AViewParams: TcxViewParams;
begin
...
begin
  ...
  begin
    ...
    Canvas.FillRect(PainterHelper.ExcludeBorders(Bounds, Borders),
      SelectionColor);
  end
  else
...
function TcxSchedulerWeekDayContentCellModernViewInfo.
  GetDefaultBorderColor: TColor;
begin
  Result := ExternalPainter.Painter.DefaultSchedulerHeaderContainerBorderColor;
end;
...
procedure TcxSchedulerMonthDayContentCellCustomViewInfo.SetContentState
  (AsDayOfMonth: Boolean; ASelected: Boolean; ATextHeight: Integer;
const ASelectionParams: TcxViewParams);
begin
...
begin
  Result := Max(FTextRect.Left, FTextRect.Right - PainterHelper.TextWidth(Font,
    DisplayText)) - 2 * ScaleFactor.Apply(cxTextOffset);
end;

function TcxSchedulerMonthDayContentCellCustomViewInfo.
  GetTextLeftForSmallFont: Integer;
begin
  Result := Max(FTextRect.Left, FTextRect.Right - PainterHelper.TextWidth
    (FSmallTextFont, DisplayText)) - ScaleFactor.Apply(cxTextOffset);
end;
...
procedure TcxSchedulerMonthDayContentCellCustomViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  FTextRect := TdxRightToLeftLayoutConverter.ConvertRect(FTextRect,
...
procedure TcxSchedulerMonthDayContentCellCustomViewInfo.DrawDisplayText;
  ...
  procedure InternalCalculateTextIndents(var ALeftIndent,
...
begin
  ...
  begin
    ...
    try
      ...
      cxTextOut(Canvas.Handle, DisplayText, FTextRect, GetTextFlags, nil, 0,
        ALeftIndent, ARightIndent, AColor);
    finally
...
begin
  Result := dxFormatSettings.LongMonthNames[MonthOf(TimeStart)] + ', ' +
    IntToStr(DayOf(TimeStart));
  if cxTextWidth(Font, Result) >= (cxRectWidth(FTextRect) div 2) then
    Result := dxFormatSettings.ShortMonthNames[MonthOf(TimeStart)] + ' ' +
      IntToStr(DayOf(TimeStart));
end;
...
procedure TcxSchedulerMonthDayContentCellCustomViewInfo.SetSmallTextFont
  (AFont: TFont);
begin
  if FSmallFontCreated then
    FreeAndNil(FSmallTextFont);
...
function TcxSchedulerMonthDayContentCellCustomViewInfo.
  GetTextLeftIndent: Integer;
begin
  Result := 0;
end;

function TcxSchedulerMonthDayContentCellCustomViewInfo.
  GetTextRightIndent: Integer;
begin
  Result := 0;
end;

function TcxSchedulerMonthDayContentCellCustomViewInfo.SaveFontStyle
  (AFont: TFont): TFontStyles;
begin
  Result := AFont.Style;
end;

function TcxSchedulerMonthDayContentCellCustomViewInfo.UpdateSelection
...
begin
  ...
  begin
    ...
    begin
      R := FTextRect;
      R.Left := Max(Bounds.Left,
        (R.Left - 2 * ScaleFactor.Apply(cxTextOffset)));
      Inc(R.Bottom);
...
function TcxSchedulerMonthDayContentCellModernViewInfo.
  GetDefaultBorderColor: TColor;
begin
  Result := ExternalPainter.Painter.DefaultSchedulerHeaderContainerBorderColor;
end;
...
begin
  Result := CXTO_PREVENT_LEFT_EXCEED or CXTO_PREVENT_TOP_EXCEED or
    CXTO_CENTER_VERTICALLY or AHorizontalAlignment[UseRightToLeftAlignment];
end;
...
function TcxSchedulerMonthDayContentCellModernViewInfo.SaveFontStyle
  (AFont: TFont): TFontStyles;
begin
...
constructor TcxSchedulerEventCellViewInfo.Create
  (AViewData: TcxSchedulerEventViewData; ACanResize: Boolean;
AScaleFactor: TdxScaleFactor; AUseRightToLeftAlignment: Boolean);
begin
  ViewData := AViewData;
  FExternalPainter := AViewData.ExternalPainter;
  inherited Create(AViewData.Painter, AViewData.Bounds, AViewData.VisibleRect,
    AViewData.ViewParams, AScaleFactor, AUseRightToLeftAlignment);
...
procedure TcxSchedulerEventCellViewInfo.DrawSizingHandler(const R: TRect;
var ARgn: TcxRegion);
begin
...
begin
  ...
  if HasLeftBorder and not ActualBorderColorIsNone then
    DrawFrame(Bounds, LeftBorders[UseRightToLeftAlignment], BorderColor,
      BorderSize);
...
function TcxSchedulerEventCellViewInfo.MeasureHeight
...
function TcxSchedulerEventCellViewInfo.CalculateClipRect(const ABounds,
  AVisibleRect: TRect): TRect;
begin
  Result := inherited CalculateClipRect(cxRectInflate(ABounds,
    GetSelectionBoundsExtends), AVisibleRect);
end;
...
begin
  ...
  begin
    ...
    FHintNeeded := (FCaptionRect.Right < R.Right) or
      (FCaptionRect.Bottom < R.Bottom);
  end;
...
begin
  ...
  begin
    if not FCaptionAndLocationCanPlaceOneLineOnly and
      (LocationRect.Bottom > LocationRect.Top) then
    begin
      FMessageRect := GetMessageRect(LocationRect,
        Images.VisibleImageCount > 0);
      FCaptionRect.Bottom := FLocationRect.Top - 1;
      FLocationRect.Bottom := Min(FMessageRect.Top,
        Bounds.Bottom - ScaleFactor.Apply(cxTextOffset));
    end
    else
    begin
      FMessageRect := GetMessageRect(CaptionRect, Images.VisibleImageCount > 0);
      FCaptionRect.Bottom := Min(FMessageRect.Top,
        Bounds.Bottom - ScaleFactor.Apply(cxTextOffset));
    end;
  end;
...
procedure TcxSchedulerEventCellViewInfo.CalculateActualEditRect
  (AEditViewData: TcxCustomEditViewData; var AMessageRect: TRect);
begin
  // do nothing
end;

procedure TcxSchedulerEventCellViewInfo.InitializeEditViewInfo
  (var AMessageRect: TRect);
var
...
begin
  if FEditViewInfo = nil then
    FEditViewInfo := TcxCustomEditViewInfo
      (ViewData.EditProperties.GetViewInfoClass.Create);
  ...
  try
    ...
    AEditViewData.CalculateEx(Canvas, AMessageRect, cxInvalidPoint, cxmbNone,
      [], FEditViewInfo, False);
  finally
    AEditViewData.Free;
  end;
...
begin
  if not EnableModernSelection then
    Exit;
  FSelectionBorderSize := Max(ExternalPainter.GetEventSelectionBorderSize(Self),
    ScaleFactor.Apply(2));
  FSelectionBorderRect := cxRectInflate(Bounds,
    ExternalPainter.GetEventSelectionExtends(Self));
  if not IsDrawBySkin and not cxRectIsNull(TimeLineRect) then
    FSelectionBorderRect.Left := TimeLineRect.Left -
      GetOffsetSelectionBorderFromTimeLineRect;
...
begin
  ...
  begin
    ...
    if NeedTopSizingHandler then
      FSizingHandlerRect1 := cxRectSetOrigin(R, cxPoint(M - ASize div 2,
        FSelectionBorderRect.Top - 1));
    if NeedBottomSizingHandler then
      FSizingHandlerRect2 := cxRectSetOrigin(R, cxPoint(M - ASize div 2,
        FSelectionBorderRect.Bottom + 1 - ASize));
  end
  else if HasLeftBorder or HasRightBorder then
  begin
    M := (FSelectionBorderRect.Top + FSelectionBorderRect.Bottom) div 2;
    if NeedLeftSizingHandler then
      FSizingHandlerRect1 := cxRectSetOrigin(R,
        cxPoint(FSelectionBorderRect.Left - 1, M - ASize div 2));
    if NeedRightSizingHandler then
      FSizingHandlerRect2 := cxRectSetOrigin(R,
        cxPoint(FSelectionBorderRect.Right + 1 - ASize, M - ASize div 2));
  end
end;

function TcxSchedulerEventCellViewInfo.CanBeExcludedSelectionLeftBorder
  : Boolean;
begin
  Result := False;
end;

function TcxSchedulerEventCellViewInfo.
  CanBeExcludedSelectionRightBorder: Boolean;
begin
  Result := False;
end;
...
function TcxSchedulerEventCellViewInfo.CalculateAutoLayoutImagesAutoHeight
  ...
  begin
    ...
    Result := GetCaptionAutoHeight(R) + 2 * ScaleFactor.Apply(cxTextOffset) +
      MeasureAutoLayoutImagesLocationHeight(R);
    if ShowMessage then
    begin
...
var
  I, ANewColCount, ACaptionWidth, ACaptionOnlyWidth, ASpaceWidth, AImagesHeight,
    AImagesWidth, AImagesColCount, AImagesRowCount, ATextsHeight: Integer;
begin
  ASpaceWidth := cxRectWidth(GetAvailableBounds) - 2 *
    ScaleFactor.Apply(cxTextOffset);
  CalculateHeaderEventNeededCaptionWidth(ACaptionWidth, ACaptionOnlyWidth);
  if (Images.TotalVisibleImageCount < 2) or
    (ASpaceWidth >= Images.TotalVisibleWidth +
    ScaleFactor.Apply(cxEventImagesGap) + ACaptionWidth) then
  begin
    Result := CalculateHorizontalImagesAutoHeight;
    Exit;
  end;
  ...
  begin
    AImagesRowCount := I;
    AImagesHeight := Min(Images.TotalVisibleImageCount,
      (AImagesRowCount + ScaleFactor.Apply(1))) *
      (Images.ItemHeight + ScaleFactor.Apply(cxEventImagesGap)) +
      ScaleFactor.Apply(4);
    ANewColCount := GetColCountForRowCount(AImagesRowCount);
    if ANewColCount <> AImagesColCount then
    begin
      AImagesColCount := ANewColCount;
      AImagesWidth := AImagesColCount *
        (Images.ItemWidth + ScaleFactor.Apply(cxEventImagesGap)) +
        ScaleFactor.Apply(1);
      if ASpaceWidth <= AImagesWidth then
        Continue;
      ATextsHeight := GetEventTextsHeight(ASpaceWidth - AImagesWidth);
      if (ATextsHeight < AImagesHeight) and
        CheckAutoLayoutImagesSingleLineCaption(ACaptionOnlyWidth,
        ASpaceWidth - AImagesWidth) then
        Break;
    end
    else
      Continue;
  end;
  Result := AImagesRowCount *
    (Images.ItemHeight + ScaleFactor.Apply(cxEventImagesGap)) +
    ScaleFactor.Apply(1);
  Images.CalculateForCols(AVisibleImages,
    cxPointOffset(GetAvailableBounds.TopLeft,
...
procedure TcxSchedulerEventCellViewInfo.CalculateHeaderEventContinueArrows
  (ACompressing: Integer);
var
  AOffset: Integer;
begin
  ...
  begin
    ...
    FStartContinueArrowRect.Right := FStartContinueArrowRect.Left +
      ScaleFactor.Apply(TcxSchedulerPainterHelper.GetStartContinueArrowSize.cx);
    ...
    begin
      ...
      FStartContinueArrowCaptionRect.Left := FStartContinueArrowRect.Right
        + AOffset;
      FStartContinueArrowCaptionRect.Right :=
        FStartContinueArrowCaptionRect.Left + cxTextWidth(Canvas.Font,
  ...
  begin
    ...
    FEndContinueArrowRect.Left := FEndContinueArrowRect.Right -
      ScaleFactor.Apply(TcxSchedulerPainterHelper.GetStartContinueArrowSize.cx);
    if ACompressing = 0 then
    begin
      FEndContinueArrowCaption := cxGetResourceString(@scxContinueTo) + ' ' +
        FormatDateTime(GetContinueArrowCaptionDateFormat,
        IfThen(TimeOf(EventFinish) > 0, EventFinish, EventFinish - 1));
      FEndContinueArrowCaptionRect := FEndContinueArrowRect;
      FEndContinueArrowCaptionRect.Right := FEndContinueArrowRect.Left
        - AOffset;
...
function TcxSchedulerEventCellViewInfo.
  CalculateHorizontalImagesAutoHeight: Integer;
var
  ARightLimit, ACaptionWidth, ACaptionHeight, ASpaceWidth, AMessageTop,
...
begin
  ...
  AImagesRect := cxRectInflate(Bounds, -ScaleFactor.Apply(cxTextOffset),
    -ScaleFactor.Apply(cxTextOffset));
  if Images.ForceVisibleWidth + ScaleFactor.Apply(cxEventImagesGap) >= ASpaceWidth
  then
  begin
    FCaptionRect := cxNullRect;
    Images.CalculateSingleLineImages(AImagesRect);
    AMessageTop := Bounds.Top + ScaleFactor.Apply
      (cxTextOffset + cxEventImagesGap) + Result;
  end
  else
  begin
    ARightLimit := 0;
    if not(Images.TotalVisibleWidth + ScaleFactor.Apply(cxEventImagesGap) +
      ACaptionWidth <= ASpaceWidth) then
    begin
      R := FCaptionRect;
      ...
      begin
        ...
        ARightLimit := Bounds.Right - ScaleFactor.Apply(cxTextOffset) -
          cxRectWidth(R);
      end;
    end;
    AImagesWidth := Images.CalculateSingleLineImages(AImagesRect, ARightLimit);
    if AImagesWidth > 0 then
      Inc(AImagesWidth);
...
procedure TcxSchedulerEventCellViewInfo.CalculateMessageAutoLayout
  (const R: TRect; var AHeight: Integer);
var
...
begin
  ...
  begin
    ...
    if FCaptionAndLocationCanPlaceOneLineOnly then
      ...
      ACaptionHeight := Max(cxRectHeight(CaptionRect),
        ACaptionHeight + cxRectHeight(FLocationRect));
    AHeight := Max(AHeight, ACaptionHeight);
    if FShowMessage then
    begin
      AMessageTop := FCaptionRect.Top + ACaptionHeight - ScaleFactor.Apply(1);
      FMessageRect := cxRect(R.Left - ScaleFactor.Apply(2), AMessageTop,
        Bounds.Right - ScaleFactor.Apply(1), 0);
...
function TcxSchedulerEventCellViewInfo.
  CalculateVerticalImagesAutoHeight: Integer;
var
  AImagesRect: TRect;
begin
  Result := Images.ItemHeight + ScaleFactor.Apply(cxTextOffset);
  ...
  begin
    ...
    if (cxRectWidth(AImagesRect) > Images.ItemWidth +
      ScaleFactor.Apply(cxEventImagesGap)) or (Images.ForceVisibleHeight > 0)
    then
    begin
      Inc(FCaptionRect.Left, Images.ItemWidth +
        ScaleFactor.Apply(cxTextOffset));
...
var
  AImagesWidth: Integer;
begin
  FCaptionRect := cxRectInflate(Bounds, -ScaleFactor.Apply(cxTextOffset),
    -FDetailCaptionVertOffset);
  AImagesWidth := Images.Calculate(GetImagesBounds);
  Images.Offset(GetImagesHorizontalOffset,
...
procedure TcxSchedulerEventCellViewInfo.CalculateHeaderEventNeededCaptionWidth
...
var
  ACaptionRect: TRect;
  ALeft, AWidth, AImagesWidth, AFullCaptionWidth, ACaptionOnlyWidth,
    AVertOffset, ACompressing: Integer;
  ...
  begin
    ...
    if AImagesWidth <> 0 then
      Inc(AImagesWidth, ScaleFactor.Apply(cxTextOffset));
...
begin
  ...
  begin
    ...
    CalculateHeaderEventNeededCaptionWidth(AFullCaptionWidth,
      ACaptionOnlyWidth);
    if (ACaptionOnlyWidth > cxRectWidth(ACaptionRect)) and (ACompressing < 2)
    then
      Continue;
    ...
    ACanShowTimeText := not ACanShowClock and CanShowHeaderEventTimeText
      (ACaptionRect);
    ...
    begin
      ...
      begin
        FStartRect := Bounds;
        FStartRect.Left := GetHeaderEventStartClockOriginLeft + ATextOffset +
          cxEventBorderWidth;
    ...
    begin
      if ACanShowClock then
        FFinishRect := cxRectOffset(R, GetHeaderEventFinishClockOriginRight -
          R.Right - ScaleFactor.Apply(cxEventImagesOffset) - cxEventBorderWidth,
          AVertOffset)
      else
      begin
        FFinishRect := Bounds;
        FFinishRect.Right := GetHeaderEventFinishClockOriginRight - ATextOffset
          - cxEventBorderWidth;
        FFinishRect.Left := FFinishRect.Right - GetTimeTextMaxWidth -
          ATextOffset;
      end;
    ...
    if (ShowStartTime or ShowFinishTime) and
      (AFullCaptionWidth + Images.TotalVisibleWidth > cxRectWidth(ACaptionRect))
      and not GetForceShowClockInHeaderEvent then
    begin
...
function TcxSchedulerEventCellViewInfo.
  GetNonDetailEventSingleLineCaptionWidth: Integer;
begin
...
begin
  ...
  AImagesWidth := Images.Offset(AOffset,
    GetImagesVerticalOffset(Images.ItemHeight, False));
  if AImagesWidth > 0 then
    Inc(AImagesWidth);
...
procedure TcxSchedulerEventCellViewInfo.CalculateEventTimeAsClockLayout
  (const ABounds: TRect; const ACaptionWidth, AImagesWidth: Integer;
var ALeft: Integer);
var
  R: TRect;
  AVertOffset: Integer;
...
begin
  Result := Max(Canvas.TextWidth('00:00' + dxFormatSettings.TimeAMString),
...
begin
  ...
  begin
    FStartRect := cxRect(ALeft, FCaptionRect.Top, ALeft + AWidth,
      FCaptionRect.Bottom);
...
begin
  ...
  FTimeLineRect := cxRectSetRight(GetTimeLineRectBaseBounds,
    GetTimeLineRectRight, AWidth);
...
procedure TcxSchedulerEventCellViewInfo.
  CheckItemsAfterAutoLayoutImagesCalculation(AFactuallyImagesRowCount,
  AImagesWidth: Integer);
begin
  Inc(FCaptionRect.Left, AImagesWidth);
end;
...
function TcxSchedulerEventCellViewInfo.CanShowHeaderEventClock
  (const ARect: TRect): Boolean;
begin
  Result := (cxRectWidth(ARect) >= EventImages.Width +
    ScaleFactor.Apply(cxEventImagesGap)) and ViewData.ShowTimeAsClock;
end;

function TcxSchedulerEventCellViewInfo.CanShowHeaderEventTimeText
  (const ARect: TRect): Boolean;
var
...
procedure TcxSchedulerEventCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
var
  ALeft: Integer;
begin
  ...
  DoRightToLeftConversionWithChecking(FEndContinueArrowCaptionRect,
    AClientBounds);
  ...
  DoRightToLeftConversionWithChecking(FStartContinueArrowCaptionRect,
    AClientBounds);
...
function TcxSchedulerEventCellViewInfo.
  GetContinueArrowCaptionDateFormat: string;
begin
  Result := 'd mmm';
end;
...
begin
  ...
  if cxRectIsEmpty(FStartContinueArrowRect) and
    cxRectIsEmpty(FEndContinueArrowRect) then
    Result := cxRectInflate(ABounds, -ScaleFactor.Apply(cxTextOffset),
      -ScaleFactor.Apply(1))
  else
  begin
    ...
    else if not cxRectIsEmpty(FStartContinueArrowRect) then
    ...
    else if not cxRectIsEmpty(FEndContinueArrowRect) then
...
function TcxSchedulerEventCellViewInfo.GetHorizontalImagesMessageAutoHeight
  (AMessageTop: Integer): Integer;
var
...
begin
  ...
  if FShowMessage then
  begin
    FMessageRect := cxRect(GetMessageRectLeft,
      AMessageTop - ScaleFactor.Apply(1),
      Bounds.Right - ScaleFactor.Apply(1), 0);
    AMessageHeight := CalculateMessageHeight(FMessageRect);
    FMessageRect.Bottom := FMessageRect.Top + AMessageHeight;
...
function TcxSchedulerEventCellViewInfo.GetHeaderImagesPossibleRect
  (const ACaptionRect: TRect; ATextWidth: Integer): TRect;
begin
  Result := cxRectSetLeft(cxRectInflate(ACaptionRect, -ScaleFactor.Apply(1),
    -ScaleFactor.Apply(1)), 0, cxRectWidth(ACaptionRect) -
    (ATextWidth + ScaleFactor.Apply(cxTextOffset)));
end;

function TcxSchedulerEventCellViewInfo.
  GetHeaderEventStartClockOriginLeft: Integer;
begin
  Result := Bounds.Left;
end;

function TcxSchedulerEventCellViewInfo.
  GetHeaderEventFinishClockOriginRight: Integer;
begin
  Result := Bounds.Right;
end;
...
function TcxSchedulerEventCellViewInfo.
  GetNonDetailEventPossibleCaptionRect: TRect;
begin
  Result := cxRectInflate(ClipRect, -ScaleFactor.Apply(cxTextOffset),
    -ScaleFactor.Apply(1));
end;
...
function TcxSchedulerEventCellViewInfo.
  GetOffsetSelectionBorderFromTimeLineRect: Integer;
begin
  Result := 1;
end;
...
begin
  ...
  begin
    ...
    if UseRightToLeftAlignment and not AllowRightToLeftConversion and
      ((Result.Left <> 0) or (Result.Right <> 0)) then
...
function TcxSchedulerEventCellViewInfo.
  NeedCorrectCaptionBottomWhenMessageAutoLayoutCalculating: Boolean;
begin
  Result := True;
end;

function TcxSchedulerEventCellViewInfo.
  NeedHeaderEventStartContinueArrow: Boolean;
begin
  Result := False;
end;

function TcxSchedulerEventCellViewInfo.
  NeedHeaderEventFinishContinueArrow: Boolean;
begin
  Result := False;
end;
...
begin
  ViewData.ShowFinishTime := (IsDetailCaption and ViewData.AlwaysShowTime) or
    ((ViewData.ContentFinish > EventFinish) and
    (ViewData.ContentStart <> EventFinish)) or
...
begin
  FCaptionRect := cxRectInflate(GetAvailableBounds,
    -ScaleFactor.Apply(cxTextOffset + 1), -FDetailCaptionVertOffset);
end;
...
begin
  Result := (ViewStyle = svsModern) or
    (not Event.AllDayEvent and not IsDetailInfo and
...
begin
  if not UseRightToLeftAlignment then
    TcxSchedulerPainterHelper.DrawEndContinueArrow(Canvas,
      FEndContinueArrowRect, Bounds, ScaleFactor)
  else
    TcxSchedulerPainterHelper.DrawStartContinueArrow(Canvas,
      FEndContinueArrowRect, Bounds, ScaleFactor);
end;
...
begin
  if not UseRightToLeftAlignment then
    TcxSchedulerPainterHelper.DrawStartContinueArrow(Canvas,
      FStartContinueArrowRect, Bounds, ScaleFactor)
  else
    TcxSchedulerPainterHelper.DrawEndContinueArrow(Canvas,
      FStartContinueArrowRect, Bounds, ScaleFactor);
end;
...
const
  ABorders: array [Boolean, Boolean] of TcxBorders =
    (([bTop, bBottom], [bLeft, bTop, bBottom]),
    ([bTop, bBottom], [bTop, bRight, bBottom]));
var
  ABounds: TRect;
begin
  ...
  DrawFrame(ABounds, ABorders[UseRightToLeftAlignment, HasLeftBorder],
...
begin
  if cxRectIsEmpty(FCaptionRect) or ((Caption = '') and (Event.Location = ''))
  then
    Exit;
  Canvas.Brush.Style := bsClear;
...
begin
  ...
  else if ViewData.IsGroup then
    ExternalPainter.DrawEventAsGroup(Self)
  else if ViewData.DrawAsProgress then
    DoDrawAsProgress
  else
  begin
...
begin
  ...
  if FStartContinueArrowCaption <> '' then
    cxTextOut(Canvas.Canvas, FStartContinueArrowCaption,
      FStartContinueArrowCaptionRect, GetCaptionFlags, 0, 0, Canvas.Font,
      clNone, clNone, 0, 0, 0, ViewParams.TextColor);
  if not cxRectIsEmpty(FEndContinueArrowRect) then
    DoDrawEndContinueArrow;
  if FEndContinueArrowCaption <> '' then
    cxTextOut(Canvas.Canvas, FEndContinueArrowCaption,
      FEndContinueArrowCaptionRect, GetCaptionFlags, 0, 0, Canvas.Font, clNone,
      clNone, 0, 0, 0, ViewParams.TextColor);
end;
  ...
  begin
    ...
    for I := 0 to Images.Count - 1 do
      if Images.Items[I].Visible and not FHidden then
        PainterHelper.DrawTransparentImage(Canvas, Images.Items[I].Images,
          Images.Items[I].Bounds, Images.Items[I].ImageIndex, ScaleFactor);
...
begin
  if (FLocation <> '') and (cxRectWidth(LocationRect) > 0) then
    cxTextOut(Canvas.Canvas, FLocation, FLocationRect, GetCaptionFlags, 0, 0,
      Canvas.Font, clNone, clNone, 0, 0, 0, ViewParams.TextColor);
end;
...
begin
  ...
  Result := HasRightBorder and ((AEventFinish = AContentFinish) or
    (AEventFinish + 1 = AContentFinish));
end;
...
function TcxSchedulerEventCellViewInfo.GetCaptionAutoHeight
...
begin
  ...
  if (Caption <> '') and not cxRectIsEmpty(CaptionRect) then
    Result := GetTrueMultilineTextHeight(Canvas, Caption, R,
      CXTO_CALCROWCOUNT or GetCaptionFlags)
  else
...
const
  AFlags: array [Boolean, Boolean] of Cardinal =
    ((cxAlignLeft or cxAlignVCenter or cxSingleLine, cxAlignLeft or
    cxAlignTop or cxWordBreak or cxDontBreakChars or cxNoFullWidthCharBreak),
    (cxAlignRight or cxAlignVCenter or cxSingleLine, cxAlignRight or
    cxAlignTop or cxWordBreak or cxDontBreakChars or cxNoFullWidthCharBreak));
begin
...
begin
  cxRectIntersect(Result, PainterHelper.ExcludeBorders(Bounds, Borders),
    FClipRect);
  with Result do
  begin
...
begin
  ...
  if Result and not Hidden and ShowTimeLine and Selected then
    ABreak := not PtInRect(CaptionRect, AHitTest.HitPoint) and
      not PtInRect(MessageRect, AHitTest.HitPoint)
  else
    ABreak := False;
end;
...
begin
  Result := cxRect(Bounds.Left + ScaleFactor.Apply(cxEventImagesOffset) +
    cxEventBorderWidth - Byte(ViewData.ShowTimeLine),
    Bounds.Top + cxEventBorderWidth,
    (Bounds.Left + Bounds.Right - ScaleFactor.Apply(cxTextOffset)) div 2,
    Bounds.Bottom - (ScaleFactor.Apply(cxEventImagesOffset) +
    cxEventBorderWidth));
end;
...
begin
  ...
  if not FIsDetailCaption then
    Result := Max(Result, (cxRectHeight(Bounds) - 2 * cxEventBorderWidth -
      AImageHeight) div 2);
  if AIsAbsolute then
    Inc(Result, Bounds.Top + cxEventBorderWidth);
end;

function TcxSchedulerEventCellViewInfo.GetLocationAutoHeight
  (const R: TRect): Integer;
begin
  Result := 0;
  if (FLocation <> '') and (R.Right - R.Left > 0) then
    Result := GetTrueMultilineTextHeight(Canvas, FLocation, R,
      CXTO_CALCROWCOUNT or GetCaptionFlags)
end;
...
begin
  ...
  begin
    ...
    ACaptionBottom := R.Bottom + ScaleFactor.Apply(cxTextOffset);
    // bottom text offset
  end
...
begin
  Result := ALeftBorderRTL[UseRightToLeftAlignment and
    AllowRightToLeftConversion and (FCalculationCounter = 0)] in Borders;
end;
...
begin
  Result := ARightBorderRTL[UseRightToLeftAlignment and
    AllowRightToLeftConversion and (FCalculationCounter = 0)] in Borders;
end;
...
begin
  with Event do
  begin
    Result := Storage.Reminders.Active and
      Storage.IsReminderAvailable and Reminder;
    if Result and (ResourceItem <> nil) and Storage.IsReminderByResourceAvailable
    then
...
begin
  Result := not(cxRectIsNull(FSizingHandlerRect1) and
    cxRectIsNull(FSizingHandlerRect2));
end;
...
begin
  ...
  begin
    ...
    TcxCustomSchedulerAccess(AHitTest.Scheduler)
      .EventHitTestController.HideEventHint;
  end;
end;
...
begin
  ...
  if CanMoveLeftBorder and ((not UseRightToLeftAlignment and AHitAtLeft) or
    (UseRightToLeftAlignment and AHitAtRight)) then
    AHitTest.FDragKind := edkResizeStart
  else if CanMoveRightBorder and ((not UseRightToLeftAlignment and AHitAtRight)
    or (UseRightToLeftAlignment and AHitAtLeft)) then
...
begin
  ...
  begin
    ...
    case EventType of
      etOccurrence:
        Images.AddStandardImage(eitRecurrence, AutoHide);
      etCustom:
        Images.AddStandardImage(eitCustomOccurrence, AutoHide);
    end;
  end;
end;
...
function TcxSchedulerEventCellViewInfo.MeasureAutoLayoutImagesLocationHeight
  (const R: TRect): Integer;
begin
  Result := 0;
end;

procedure TcxSchedulerEventCellViewInfo.ShiftRect(var R: TRect;
const X, Y: Integer);
begin
  if not cxRectIsEmpty(R) then
    OffsetRect(R, X, Y);
...
function TcxSchedulerEventCellViewInfo.TruncTime(const ATime: TDateTime)
  : TDateTime;
begin
...
function TcxSchedulerEventCellViewInfo.CalculateMessageHeight
  (const R: TRect): Integer;
...
begin
  Result := R;
  if not cxRectIsNull(LocationRect) and (LocationRect.Top > CaptionRect.Top)
  then
    Canvas.TextExtent(FLocation, Result, cxDrawTextFlags[IsDetailCaption])
  else if Length(ViewData.Caption) > 0 then
    Canvas.TextExtent(ViewData.Caption, Result,
      cxDrawTextFlags[IsDetailCaption])
  else
...
begin
  Result := ViewData.AutoHeight or
    (not IsHeaderEvent and ((dxTimeOf(ContentFinish) <> 0) or
    (dxTimeOf(ContentStart) <> 0)));
end;
...
function TcxSchedulerEventCellViewInfo.GetResourceInfo
  : TcxSchedulerResourceViewInfo;
begin
  Result := ViewData.Resource;
end;

function TcxSchedulerEventCellViewInfo.GetResourceItem
  : TcxSchedulerStorageResourceItem;
begin
  Result := nil;
...
function TcxSchedulerEventCellViewInfo.GetShowingState(AIndex: Integer)
  : Boolean;
begin
  case AIndex of
    0:
...
function TcxSchedulerEventCellViewInfoList.GetItem(Index: Integer)
  : TcxSchedulerEventCellViewInfo;
begin
...
begin
  if not IsModern then
    PainterHelper.DrawTransparentImage(Canvas, MoreEventButtonGlyphs, Bounds,
      Byte(not IsDown), ScaleFactor)
  else
    TcxSchedulerPainterHelper.DrawMoreEventButton(Canvas, Bounds, ScaleFactor,
      Byte(IsDown) + 2 * Byte((FHitTest <> nil) and
      (FHitTest.HitMoreEventsButton = Self)));
end;
...
constructor TcxSchedulerContentNavigationButtonViewInfo.Create
  (APainter: TcxCustomLookAndFeelPainter; const ABounds, AVisibleRect: TRect;
const AViewParams: TcxViewParams; AScaleFactor: TdxScaleFactor;
AUseRightToLeftAlignment: Boolean);
begin
...
procedure TcxSchedulerContentNavigationButtonViewInfo.DoDraw;
  ...
  const
    AKindRTL: array [Boolean] of TcxSchedulerContentNavigationButtonKind =
      (nbkNext, nbkPrevious);
  begin
    Result := Kind;
...
begin
  ...
  begin
    ...
    AState := cxbsDisabled;
  Painter.DrawSchedulerNavigationButton(Canvas, Bounds,
    GetButtonKindRTL = nbkNext, AState, Caption, TextRect, ArrowRect,
    FIsVertical);
end;

procedure TcxSchedulerContentNavigationButtonViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
  FArrowRect := TdxRightToLeftLayoutConverter.ConvertRect(FArrowRect,
    AClientBounds);
  FTextRect := TdxRightToLeftLayoutConverter.ConvertRect(FTextRect,
    AClientBounds);
end;
...
procedure TcxSchedulerContentNavigationButtonViewInfo.SetIsVertical
  (AValue: Boolean);
begin
  FIsVertical := AValue;
end;
...
begin
  ...
  begin
    ...
    Painter.DrawSchedulerScaledGroupSeparator(Canvas, Bounds, Color,
      ScaleFactor, DrawBackground);
end;
...
procedure TcxSchedulerExternalPainter.DrawCellFocus
  (AViewInfo: TcxSchedulerCustomViewInfoItem; AColor: TColor);
var
  R: TRect;
  APenWidth: Integer;
  ASavedWidth: Integer;
...
procedure TcxSchedulerExternalPainter.DrawCustomCurrentTime(ACanvas: TcxCanvas;
AColor: TColor; AStart: TDateTime; const ABounds: TRect;
AUseRightToLeftAlignment: Boolean);
var
  Y: Integer;
  ANow: TDateTime;
...
begin
  ...
  if (ANow < 0) or (ANow >= HourToTime) then
    Exit;
...
procedure TcxSchedulerExternalPainter.DrawCurrentTimeRTL(ACanvas: TcxCanvas;
AColor: TColor; AStart: TDateTime; ABounds: TRect);
begin
...
procedure TcxSchedulerExternalPainter.SetPainter(const Value
...
begin
  AViewInfo.DrawRect(AViewInfo.Bounds, AViewInfo.Borders,
    AViewInfo.ViewData.BorderColor);
end;

procedure TcxSchedulerExternalPainter.DrawEventAsGroup
  (AViewInfo: TcxSchedulerEventCellViewInfo);
begin
  Painter.DrawSchedulerGroup(AViewInfo.Canvas, AViewInfo.Bounds,
    AViewInfo.Color);
end;

procedure TcxSchedulerExternalPainter.DrawEventAsMilestone
  (AViewInfo: TcxSchedulerEventCellViewInfo);
begin
...
begin
  ...
  begin
    ...
    if not AViewInfo.UseRightToLeftAlignment then
      ...
      AProgressRect.Left := AProgressRect.Right -
        Trunc(ViewData.TaskComplete / 100 * cxRectWidth(AProgressRect));

    Painter.DrawSchedulerScaledEventProgress(Canvas, ABounds, AProgressRect,
      ViewParams, Transparent, ScaleFactor);
    Canvas.Brush.Style := bsClear;
...
procedure TcxSchedulerExternalPainter.DrawEventAsProgressText
  (AViewInfo: TcxSchedulerEventCellViewInfo; AContent: TRect;
AProgressRect: TRect; const AText: string);
begin
  Painter.DrawProgressBarText(AViewInfo.Canvas, False, False, AText,
    cxRectCenter(AContent, AViewInfo.Canvas.TextExtent(AText)), AContent,
    AProgressRect);
end;

procedure TcxSchedulerExternalPainter.DrawEventLabel
  (AViewInfo: TcxSchedulerEventCellViewInfo; const R: TRect; AColor: TColor);
var
...
begin
  ...
  try
    ...
    ARegion := TcxRegion.CreateRoundCorners(cxRectInflate(R, 1),
      AWidth, AHeight);
    AViewInfo.Canvas.SetClipRegion(ARegion, roIntersect);
    dxGpFillRect(AViewInfo.Canvas.Handle, R, ABorderColor);

    ARegion := TcxRegion.CreateRoundCorners(cxRectInflate(R, 1), AWidth + 2,
      AHeight + 2);
    AViewInfo.Canvas.SetClipRegion(ARegion, roIntersect);
    dxGpFillRect(AViewInfo.Canvas.Handle, R, ABorderColor);
...
procedure TcxSchedulerExternalPainter.DrawTimeGridCurrentTime
...
procedure TcxSchedulerExternalPainter.DrawTimeRulerBackground
  (ACanvas: TcxCanvas; const ARect: TRect; ABorders: TcxBorders;
AViewParams: TcxViewParams; ATransparent: Boolean);
begin
  DrawTimeRulerBackground(ACanvas, ARect, ABorders, clBtnShadow, AViewParams,
    ATransparent);
end;

procedure TcxSchedulerExternalPainter.DrawTimeRulerBackground
  (ACanvas: TcxCanvas; const ARect: TRect; ABorders: TcxBorders;
ABorderBolor: TColor; AViewParams: TcxViewParams; ATransparent: Boolean);
begin
  if ATransparent then
...
const ABounds, AVisibleBounds: TRect; AScaleFactor: TdxScaleFactor = nil);
begin
  if AScaleFactor = nil then
...
procedure TcxSchedulerExternalPainter.AdjustEditStyleViewParams(var AViewParams
  : TcxViewParams);
begin
  // do nothing
end;
...
function TcxSchedulerExternalPainter.GetColorizedColor(ASource: TColor;
AEvent: TcxSchedulerEventCellViewInfo): TColor;
var
  AEventHSL: TdxHSL;
  ASourceHSL: TdxHSL;
begin
  ...
  if (AEventHSL.H = 0) and (AEventHSL.S = 0) and (ASourceHSL.H = 0) and
    (ASourceHSL.S = 0) then
...
function TcxSchedulerExternalPainter.GetEventBorderColor
  (AViewInfo: TcxSchedulerEventCellViewInfo): TColor;
begin
  Result := AViewInfo.BorderColor;
end;

function TcxSchedulerExternalPainter.GetEventBorderSize
  (AViewInfo: TcxSchedulerEventCellViewInfo): Integer;
begin
  Result := cxEventBorderWidth;
end;

function TcxSchedulerExternalPainter.GetEventLabelSize
...
function TcxSchedulerExternalPainter.GetEventSelectionBorderColor
  (AViewInfo: TcxSchedulerEventCellViewInfo): TColor;
begin
  Result := Painter.DefaultSchedulerSelectedEventBorderColor;
end;

function TcxSchedulerExternalPainter.GetEventSelectionBorderSize
  (AViewInfo: TcxSchedulerEventCellViewInfo): Integer;
begin
  Result := 0;
...
function TcxSchedulerExternalPainter.GetEventSelectionExtends
  (AViewInfo: TcxSchedulerEventCellViewInfo): TRect;
var
  ABorderSize: Integer;
begin
...
function TcxSchedulerExternalPainter.GetPainterHelper
  : TcxSchedulerPainterHelperClass;
begin
  Result := SchedulerHelpersFactory.PainterHelperClass;
end;

function TcxSchedulerExternalPainter.MoreButtonSize(ASize: TSize;
AScaleFactor: TdxScaleFactor = nil): TSize;
begin
  if AScaleFactor = nil then
...
procedure TcxSchedulerCustomResourceView.CheckGroupingKind
  (AStyle: TcxSchedulerGroupingKind;
var ActuallyStyle: TcxSchedulerGroupingKind);
begin
...
begin
  ...
  begin
    ...
    else if (ViewInfo.GroupingKind <> gkNone) and (Scheduler.SelResource = nil)
    then
      with Controller.Navigation do
...
procedure TcxSchedulerCustomResourceView.DoMouseDown(AButton: TMouseButton;
...
function TcxSchedulerCustomResourceView.FindEventViewInfo
  ...
  for I := 0 to ViewInfo.EventCells.Count - 1 do
    ...
    begin
      ...
      if (AResource = nil) or (AViewInfo.ResourceItem = AResource) then
        if (ADate = NullDate) or ((dxDateOf(AViewInfo.ContentStart) = ADate) or
          (AViewInfo.IsHeaderEvent and
          (ADate >= dxDateOf(AViewInfo.ContentStart)) and
          (ADate < dxDateOf(AViewInfo.ContentFinish)))) then
          Break;
    end;
end;

function TcxSchedulerCustomResourceView.GetCommonViewItemsPainter
  : IcxSchedulerCommonViewItemsPainter;
var
  ACustomPainter: IcxSchedulerCommonViewItemsPainter;
begin
  if not Supports(TObject(Scheduler), IcxSchedulerCommonViewItemsPainter,
    ACustomPainter) then
    ACustomPainter := nil;
  InitializePainter;
...
function TcxSchedulerCustomResourceView.GetEditStyleViewParams
  (AEvent: TcxSchedulerControlEvent): TcxViewParams;
begin
...
function TcxSchedulerCustomResourceView.GetEventHintText
...
var
  AViewInfo: TcxSchedulerEventCellViewInfo;
begin
  Result := FindEventViewInfo(AEvent, NullDate, nil, AViewInfo) and
    AViewInfo.Visible;
end;

function TcxSchedulerCustomResourceView.GetGroupingKind
...
function TcxSchedulerCustomResourceView.GetViewInfo
  : TcxSchedulerCustomResourceViewViewInfo;
begin
  Result := TcxSchedulerCustomResourceViewViewInfo(inherited ViewInfo);
end;

function TcxSchedulerCustomResourceView.IsRelevantSelection
  (out ATime: TDateTime;
out AResource: TcxSchedulerStorageResourceItem): Boolean;
begin
  ATime := HitTest.Time;
  ...
  Result := ((ATime >= Scheduler.SelStart) and (ATime < Scheduler.SelFinish))
    and ((Scheduler.SelResource = AResource) or (AResource = nil));
end;
...
procedure TcxSchedulerCustomResourceView.SetShowEndTime(const AValue: Boolean);
begin
...
procedure TcxCustomResourceViewAdapter.GetPageDays(APageIndex: Integer;
ADays: TcxSchedulerDateList);
begin
end;
...
function TcxCustomResourceViewAdapter.GetPrintRange(Index: Integer): TDateTime;
begin
  Result := NullDate;
end;

function TcxCustomResourceViewAdapter.GetStylesAdapter
...
procedure TcxCustomResourceViewAdapter.DoInitialize(var ASelectedDays
...
begin
  ...
  Result := ViewInfo.GetFontHeight(StylesAdapter.GetEventParams(nil)) + 3 *
    ViewInfo.ScaleFactor.Apply(cxTextOffset);
  if AEventHeight = 0 then
  begin
    if AImages <> nil then
      AIconHeight :=
        Max(ViewInfo.PainterHelper.IconsHeight(ViewInfo.ScaleFactor),
        AImages.Height)
    else
      AIconHeight := ViewInfo.PainterHelper.IconsHeight(ViewInfo.ScaleFactor);
    Result := Max(AIconHeight + ViewInfo.ScaleFactor.Apply(cxTextOffset) + 3 *
      ViewInfo.ScaleFactor.Apply(cxEventImagesOffset), Result);
  end
  else
    Result := Max(AEventHeight, Result);
end;

function TcxCustomResourceViewAdapter.GetPainterHelper
  : TcxSchedulerPainterHelperClass;
begin
  Result := TcxSchedulerHelpersFactory.PainterHelperClass;
end;

function TcxCustomResourceViewAdapter.GetResourceNavigator
  : TcxSchedulerResourceNavigator;
begin
  Result := View.Scheduler.ResourceNavigator;
end;

function TcxCustomResourceViewAdapter.GetResources
...
function TcxCustomResourceViewAdapter.GetViewInfo
  : TcxSchedulerCustomResourceViewViewInfo;
begin
...
procedure TcxSchedulerEventImageItem.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  FBounds := TdxRightToLeftLayoutConverter.ConvertRect(FBounds, AClientBounds);
end;
...
constructor TcxSchedulerEventImages.Create
  (ALayout: TcxSchedulerEventImagesLayout; AScaleFactor: TdxScaleFactor);
begin
...
AutoHide: Boolean = True): TcxSchedulerEventImageItem;
begin
  if (FImages = nil) or ((AImageIndex < 0) or (AImageIndex >= FImages.Count))
  then
  begin
    Result := nil;
    Exit;
...
function TcxSchedulerEventImages.AddStandardImage
  (AImageType: TcxSchedulerEventImageType; AutoHide: Boolean = True)
  : TcxSchedulerEventImageItem;
const
...
var
  I, AWidth, AHeight, ACount, ARows, AColIndex, ARowIndex: Integer;
  L: TList;
  RS: TSize;
  ...
  try
    ...
    begin
      SetItemBounds(TcxSchedulerEventImageItem(L[I]),
        R.Left + AColIndex * AWidth, R.Top + ARowIndex * AHeight);
...
function TcxSchedulerEventImages.CalculateSingleLineImages(const R: TRect;
ARightLimit: Integer = 0): Integer;
var
  ...
  var
    ...
    AItem: TcxSchedulerEventImageItem;
  begin
    if ARectWidth >= ATotalWidth then
      Exit;
...
function TcxSchedulerEventImages.CalculateSingleColumnImages
  (const R: TRect): Integer;
var
  ...
  var
    ...
    AItem: TcxSchedulerEventImageItem;
  begin
    if RS.cy + FEventImageGap >= ATotalHeight then
      Exit;
    for I := L.Count - 1 downto 0 do
    begin
      AItem := TcxSchedulerEventImageItem(L.List[I]);
...
begin
  ...
  try
    ...
    RS := cxRectSize(R);
    AWidth := ItemWidth + FEventImageGap;
    if (RS.cx < AWidth) and CanAutoHideAllItems then
      Exit;
    AHeight := ItemHeight + FEventImageGap;
    CheckAutoHideItems;
    if L.Count = 0 then
...
procedure TcxSchedulerEventImages.DoRightToLeftConversion
  (const AClientBounds: TRect);
var
  I: Integer;
begin
...
begin
  ...
  if (FImages <> nil) then
  begin
    if FImages.Height > FItemHeight then
      FItemHeight := FImages.Height;
    if FImages.Width > FItemWidth then
      FItemWidth := FImages.Width;
  end;
end;
...
function TcxSchedulerEventImages.GetImageItem(AIndex: Integer)
  : TcxSchedulerEventImageItem;
begin
...
function TcxSchedulerCachedImage.IsValid
  (AItem: TcxSchedulerCustomViewInfoItem): Boolean;
begin
...
function TcxSchedulerResourceHeaderCachedImage.IsValid
  (AItem: TcxSchedulerCustomViewInfoItem): Boolean;
begin
  Result := inherited IsValid(AItem);
  if Result then
  begin
    Result := (TcxSchedulerHeaderCellViewInfo(AItem).DisplayText = FDisplayText)
      and (TcxSchedulerHeaderCellViewInfo(AItem).DateTime = FDateTime);
  end;
end;

procedure TcxSchedulerResourceHeaderCachedImage.Update
  (AItem: TcxSchedulerCustomViewInfoItem);
begin
...
function TcxSchedulerResourceHeaderCachedImageList.Add
...
function TcxSchedulerResourceHeaderCachedImageList.GetCacheForItem
  ...
  function GetResourceItem(AItem: TcxSchedulerHeaderCellViewInfo)
    : TcxSchedulerStorageResourceItem;
  begin
    if AItem.Resource = nil then
      Result := nil
...
begin
  ...
  begin
    SetHitContentNavigationButton(nil, TcxSchedulerCustomResourceViewHitTest
      (View.HitTest));
    SetHitMoreEventsButton(nil, TcxSchedulerCustomResourceViewHitTest
      (View.HitTest));
  end;
end;

function TcxSchedulerCustomResourceViewViewInfo.AddButton(AColumnIndex: Integer;
ABounds: TRect; const ADateTime: TDateTime; AIsDown: Boolean;
AEvent: TcxSchedulerEvent): TcxSchedulerMoreEventsButtonViewInfo;
var
  AMoreButtonSize: TSize;
begin
  ABounds := cxRectInflate(ABounds, -ScaleFactor.Apply(cxTextOffset),
    -ScaleFactor.Apply(cxTextOffset));
  AMoreButtonSize := View.ExternalPainter.MoreButtonSize
    (Size(PainterHelper.MoreButtonWidth(ViewStyle),
    PainterHelper.MoreButtonHeight), ScaleFactor);
...
function TcxSchedulerCustomResourceViewViewInfo.AddBackgroundSlot
  (const ABounds: TRect; ABorders: TcxBorders; const AText: string = '')
  : TcxSchedulerBackgroundSlotCellViewInfo;
begin
...
function TcxSchedulerCustomResourceViewViewInfo.AddContentCell
  (const ARect: TRect; const AStart, AFinish: TDateTime;
AResourceIndex: Integer): TcxSchedulerContentCellViewInfo;
begin
  CreateCellInstance(ContentCellClass, ARect, GetContentParams(AStart,
    Resources[AResourceIndex]), UseRightToLeftAlignment, Result);
...
AColumnPositionInResource: TcxSchedulerColumnPositionInResource);
  ...
  begin
    ...
    CreateCellInstance(TcxSchedulerContentNavigationButtonViewInfo,
      FButtonBounds, Styles.GetBackgroundParams,
      UseRightToLeftAlignment, AButton);
...
begin
  ...
  if Scheduler.OptionsView.ShowNavigationButtons and
    ...
    (AColumnHeight > ScaleFactor.Apply(ContentNavigationButtonReducedHeight))
  then
  begin
...
ADisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode)
  : TcxSchedulerDayHeaderCellViewInfo;
begin
  CreateCellInstance(DayHeaderClass, ABounds,
...
ADisplayMode: TcxSchedulerDayHeaderModernStyleDisplayMode)
  : TcxSchedulerDayHeaderCellViewInfo;
begin
  Result := AddDayHeader(ADate, cxRect(ALeft, ATop, ARight,
    ATop + FDayHeaderHeight), AResourceIndex, ADisplayMode);
end;

function TcxSchedulerCustomResourceViewViewInfo.AddEventCell
  (AViewData: TcxSchedulerEventViewData; AImmediateCalculate: Boolean = True)
  : TcxSchedulerEventCellViewInfo;
...
var
  ACanResize: Boolean;
begin
  ACanResize := Scheduler.EventOperations.Sizing and
    TcxSchedulerControlEventAccess(AViewData.Event).CanResize(GetResourceItem);
...
begin
  CreateCellInstance(TcxSchedulerDayHeaderCellViewInfo, ABounds,
    StylesAdapter.GetResourceHeaderParams(GetResourceItemByIndex(AIndex)),
    UseRightToLeftAlignment, Result);
...
begin
  ...
  Result := (((AGroupingKind = gkNone) or (AResourceIndex < 0)) and
    (HasVisibleEvents or HasMoreEventsButton(-1))) or
...
procedure TcxSchedulerCustomResourceViewViewInfo.
  ...
  begin
    if ResourceCount > 0 then
      for AResourceIndex := 0 to ResourceCount - 1 do
        AddContentNavigationButton(FResourceBounds[AResourceIndex],
          AResourceIndex, cprSingle)
    else
...
begin
  FHideSelection := View.OptionsView.HideSelection and
    not View.Scheduler.IsFocused;
  ...
  FContentFontHeight := cxTextHeight(StylesAdapter.GetContentParams(Now, nil)
    .Font) + 2 * ScaleFactor.Apply(cxTextOffset);
  FDayHeaderHeight := LookAndFeelPainter.ScaledHeaderHeight
    (GetFontHeight(StylesAdapter.GetDayHeaderStyle), ScaleFactor);
  ...
  FUseResourceImages := HasStorage and
    View.Scheduler.Storage.Resources.AreImagesUsed;
  if HasStorage then
  ...
  begin
    ...
    CalculateResourceHeadersAutoHeight(GetResourceHeaderWidth - 3 *
      ScaleFactor.Apply(cxTextOffset));
    if ResourceHeaders.Height <> 0 then
      FResourceHeaderHeight := ResourceHeaders.Height;
  end;
...
begin
  if IsNavigationButtonsVertical then
    AIsCollapsed := Round(ScaleFactor.ApplyF(ContentNavigationButtonHeight *
      1.5)) >= GetCorrectColumnHeight(AColumnRect)
  else
    AIsCollapsed := Round(ScaleFactor.ApplyF(ContentNavigationButtonHeight *
      1.5)) >= GetCorrectColumnHeight(cxRectRotate(AColumnRect));
  ...
  begin
    ...
    case AKind of
      nbkPrevious:
        FButtonBounds := Rect(AColumnRect.Left, AMiddle - AHeight div 2,
          AColumnRect.Left + ScaleFactor.Apply(ContentNavigationButtonWidth),
          AMiddle + AHeight div 2);
      nbkNext:
        FButtonBounds :=
          Rect(AColumnRect.Right - ScaleFactor.Apply
          (ContentNavigationButtonWidth), AMiddle - AHeight div 2,
          AColumnRect.Right, AMiddle + AHeight div 2);
    else
  ...
  begin
    ...
    case AKind of
      nbkPrevious:
        FButtonBounds := Rect(AMiddle - AHeight div 2, AColumnRect.Top,
          AMiddle + AHeight div 2, AColumnRect.Top +
          ScaleFactor.Apply(ContentNavigationButtonWidth));
      nbkNext:
        FButtonBounds := Rect(AMiddle - AHeight div 2,
          AColumnRect.Bottom - ScaleFactor.Apply(ContentNavigationButtonWidth),
          AMiddle + AHeight div 2, AColumnRect.Bottom);
    else
      FButtonBounds := cxNullRect;
    end;
  end;

  LookAndFeelPainter.CalculateSchedulerNavigationButtonRects(AKind = nbkNext,
    AIsCollapsed, cxTextExtent(Styles.GetBackgroundParams.Font,
...
procedure TcxSchedulerCustomResourceViewViewInfo.
  CalculateResourceHeadersAutoHeight(AWidth: Integer);
  ...
  begin
    ...
    if AHasImage and (ResourceHeaders.ImagePosition in [ipLeft, ipRight]) then
      AWidth := Max(0, AWidth - (GetResourceImagesSize.cx +
    ...
    begin
      if (ResourceHeaders.ImagePosition in [ipTop, ipBottom]) then
        Inc(Result, GetResourceImagesSize.cy + 4 *
          ScaleFactor.Apply(cxTextOffset))
      else
        Result := Max(Result, GetResourceImagesSize.cy + 3 *
          ScaleFactor.Apply(cxTextOffset));
    end;
  end;
...
begin
  ...
  for I := 0 to Adapter.Resources.VisibleResourceCount - 1 do
    FResourceHeaderHeight := Max(FResourceHeaderHeight,
      CalculateResourceHeight(Adapter.Resources.VisibleResources[I], AWidth));
end;

function TcxSchedulerCustomResourceViewViewInfo.
  CalculateResourceHeaderWidth: Integer;
var
  I: Integer;
  AWidth: Integer;
  ...
  begin
    ...
    begin
      ...
      if AHasImage and (ResourceHeaders.ImagePosition in [ipTop, ipBottom]) then
        Dec(R.Bottom, GetResourceImagesSize.cy + 3 *
          ScaleFactor.Apply(cxTextOffset));
      ...
      SetResourceTextRect(AResource, R);
      if AHasImage and (ResourceHeaders.ImagePosition in [ipTop, ipBottom]) then
        Result := Max(Result, ResourceImages.Width + 4 *
          ScaleFactor.Apply(cxTextOffset));
    end;
    if AHasImage and (ResourceHeaders.ImagePosition in [ipLeft, ipRight]) then
...
begin
  ...
  for I := 0 to Adapter.Resources.VisibleResourceCount - 1 do
    Result := Max(Result,
      CalculateWidth(Adapter.Resources.VisibleResources[I]));
end;

function TcxSchedulerCustomResourceViewViewInfo.
  CanCalculateNavigationButtons: Boolean;
begin
  Result := Scheduler.OptionsView.ShowNavigationButtons and
    not Adapter.IsPrinting;
end;
...
begin
  Result := AItem.Rotated and (AItem.Bitmap = nil) and
    not View.HasCustomDrawGroupSeparator;
end;
...
function TcxSchedulerCustomResourceViewViewInfo.ContentCellClass
  : TcxSchedulerContentCellViewInfoClass;
begin
...
const AViewParams: TcxViewParams; AUseRightToLeftAlignment: Boolean;
var Instance);
begin
  CreateCellInstance(AClass, ABounds, Bounds, AViewParams,
    AUseRightToLeftAlignment, Instance);
end;

procedure TcxSchedulerCustomResourceViewViewInfo.CreateCellInstance
  (AClass: TcxSchedulerCustomViewInfoItemClass;
const ABounds, AVisibleBounds: TRect; const AViewParams: TcxViewParams;
AUseRightToLeftAlignment: Boolean; var Instance);
begin
  pointer(Instance) := AClass.Create(LookAndFeelPainter, ABounds,
    AVisibleBounds, AViewParams, ScaleFactor, AUseRightToLeftAlignment);
  TcxSchedulerCustomViewInfoItem(Instance).FDayBorderColor := FDayBorderColor;
  TcxSchedulerCustomViewInfoItem(Instance).FExternalPainter :=
    View.ExternalPainter;
end;
...
function TcxSchedulerCustomResourceViewViewInfo.CreateEventCellViewInfo
  ...
  : TcxSchedulerEventCellViewInfo;
begin
  Result := EventCellClass.Create(AViewData, ACanResize, ScaleFactor,
    UseRightToLeftAlignment);
end;

function TcxSchedulerCustomResourceViewViewInfo.CreateImageCacheManager
  : TcxSchedulerImageCacheManager;
begin
  Result := TcxSchedulerImageCacheManager.Create;
end;

function TcxSchedulerCustomResourceViewViewInfo.DayHeaderClass
  : TcxSchedulerDayHeaderCellViewInfoClass;
const
...
procedure TcxSchedulerCustomResourceViewViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
var
  I: Integer;
begin
  for I := 0 to FCells.Count - 1 do
    if FCells.Items[I] is TcxSchedulerViewInfoCellList then
      TcxSchedulerViewInfoCellList(FCells.Items[I]).DoRightToLeftConversion
        (AClientBounds);
  NavigationButtons.DoRightToLeftConversion(AClientBounds);
  Buttons.DoRightToLeftConversion(AClientBounds);
end;

function TcxSchedulerCustomResourceViewViewInfo.DoSetResourceHasVisibleEvent
  (AEvent: TcxSchedulerControlEvent): Boolean;
var
  AResourceIndex: Integer;
begin
  ...
  for AResourceIndex := 0 to ResourceCount - 1 do
    ...
    begin
      Resources[AResourceIndex].VisibleEventCount := Resources[AResourceIndex]
        .VisibleEventCount + 1;
      Result := True;
    end;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.DrawNavigationButtons
...
begin
  ...
  AHorzSplitterMovingRect := TcxSchedulerSplitterControllerAccess
    (TcxSchedulerSplitterAccess(TcxCustomSchedulerAccess(Scheduler)
    .HorzSplitter).Controller).PrevInvertRect;
  AVertSplitterMovingRect := TcxSchedulerSplitterControllerAccess
    (TcxSchedulerSplitterAccess(TcxCustomSchedulerAccess(Scheduler)
    .VertSplitter).Controller).PrevInvertRect;
  ...
  if AIsHorzSplitterMoving or AIsVertSplitterMoving then
    for I := 0 to NavigationButtons.Count - 1 do
      ...
      begin
        AActualBounds := cxRectOffset(NavigationButtons[I].ActiveBounds,
          View.Bounds.TopLeft);
        if (AIsHorzSplitterMoving and IntersectRect(R, AHorzSplitterMovingRect,
          AActualBounds)) or (AIsVertSplitterMoving and IntersectRect(R,
          AVertSplitterMovingRect, AActualBounds)) then
          InvertRect(ACanvas.Handle, cxRectOffset(R, -View.Bounds.Left,
            -View.Bounds.Top));
      end;
end;
...
function TcxSchedulerCustomResourceViewViewInfo.GetContentNavigationInterval
  ...
  function GetContentNavigationInfo(AIndex: Integer)
    : TcxSchedulerContentNavigationInfo;
  begin
    Result := TcxSchedulerContentNavigationInfo
      (FContentNavigationIntervals[AIndex]);
  end;
...
function TcxSchedulerCustomResourceViewViewInfo.GetContentParams
  (const ATime: TDateTime; AResource: TcxSchedulerResourceViewInfo)
  : TcxViewParams;
begin
  if AResource = nil then
...
function TcxSchedulerCustomResourceViewViewInfo.EventCellClass
  : TcxSchedulerEventCellViewInfoClass;
begin
  Result := TcxSchedulerEventCellViewInfo;
end;

function TcxSchedulerCustomResourceViewViewInfo.
  GetEventWithoutResourceCount: Integer;
begin
  Result := FEventWithoutResourceCount;
end;

function TcxSchedulerCustomResourceViewViewInfo.GetGroupingKind
  : TcxSchedulerGroupingKind;
begin
  ...
  if ResourceCount = 0 then
    Result := gkNone
  else if Result = gkDefault then
    Result := gkByResource;
end;

function TcxSchedulerCustomResourceViewViewInfo.GetMoreEventButtonClass
...
begin
  ...
  if ResourceCount > 0 then
    Result := Max(0, (Result - SeparatorWidth * GetSeparatorCount)
      div ResourceCount);
end;
...
function TcxSchedulerCustomResourceViewViewInfo.
  GetResourcesContentWidth: Integer;
begin
...
function TcxSchedulerCustomResourceViewViewInfo.
  GetShowEventsWithoutResource: Boolean;
begin
  Result := View.OptionsView.ShowEventsWithoutResource or
    (FGroupingKind = gkNone);
end;

function TcxSchedulerCustomResourceViewViewInfo.GetTimeLineParams
  : TcxViewParams;
begin
...
function TcxSchedulerCustomResourceViewViewInfo.GetActualFont
  (AStyle: TcxStyle): TFont;
begin
...
begin
  ...
  if Days[0] <> Days[DayCount - 1] then
    Result := Result + ' - ' + FormatDateTime(dxFormatSettings.LongDateFormat,
      Days[DayCount - 1]);
end;
...
function TcxSchedulerCustomResourceViewViewInfo.GetResourceViewInfoByItem
  (AItem: TcxSchedulerStorageResourceItem; var ResourceViewInfoIndex: Integer)
  : TcxSchedulerResourceViewInfo;
var
  I: Integer;
begin
...
function TcxSchedulerCustomResourceViewViewInfo.GetStartDate(Index: Integer)
  : TDateTime;
begin
  Result := SelectedDays[0];
end;

function TcxSchedulerCustomResourceViewViewInfo.HasClone
...
function TcxSchedulerCustomResourceViewViewInfo.HasEventForResource
  (AResourceIndex: Integer): Boolean;
begin
...
begin
  DrawCommonBorders := LookAndFeelPainter.HeaderBorders([nLeft .. nBottom])
    = cxBordersAll;
  APrevCell := nil;
...
begin
  ...
  begin
    ...
    else if AEventCell.HasSizingHandlers then
    begin
...
function TcxSchedulerCustomResourceViewViewInfo.
  GetUseRightToLeftAlignment: Boolean;
begin
...
begin
  ...
  for I := 0 to FDayHeaderCells.Count - 1 do
    AType := Max(TcxSchedulerDayHeaderCellViewInfo(FDayHeaderCells[I])
      .ConvertDateToDisplayText, AType);
  if ArrangeByType and (AType > 0) then
  begin
    for I := 0 to FDayHeaderCells.Count - 1 do
      TcxSchedulerDayHeaderCellViewInfo(FDayHeaderCells[I])
        .ConvertDateToDisplayText(AType);
  end;
end;
...
procedure TcxSchedulerCustomResourceViewViewInfo.HotTrackMoreEventsButton
  (AHitTest: TcxSchedulerCustomResourceViewHitTest);
var
...
function TcxSchedulerCustomResourceViewViewInfo.
  IsNavigationButtonsVertical: Boolean;
begin
  Result := True;
end;

function TcxSchedulerCustomResourceViewViewInfo.IsTimeSelected(ATime: TDateTime;
AResource: TObject): Boolean;
begin
...
begin
  ...
  Result := AButtonWidth1 + AButtonWidth2 <= AResourceRect.Right -
    AResourceRect.Left;
end;
...
function TcxSchedulerCustomResourceViewViewInfo.NeedClearResources
  (AGroupingKind: TcxSchedulerGroupingKind): Boolean;
begin
...
procedure TcxSchedulerCustomResourceViewViewInfo.
  AddContentNavigationIntervalItem(AResourceIndex: Variant);
var
  AResourceID: Variant;
  I: Integer;
...
begin
  ...
  begin
    AnAlreadyExists := VarEqualsSoft
      (TcxSchedulerContentNavigationInfo(FContentNavigationIntervals[I])
      .FResourceID, AResourceID);
    Inc(I);
  end;
  if not AnAlreadyExists then
    FContentNavigationIntervals.Add(TcxSchedulerContentNavigationInfo.Create
      (AResourceID));
end;
...
function TcxSchedulerCustomResourceViewViewInfo.GetClones
...
function TcxSchedulerCustomResourceViewViewInfo.GetDay(AIndex: Integer)
  : TDateTime;
begin
...
function TcxSchedulerCustomResourceViewViewInfo.GetResource(AIndex: Integer)
  : TcxSchedulerResourceViewInfo;
begin
  if AIndex < 0 then
    Result := nil
...
function TcxSchedulerCustomResourceViewViewInfo.GetResourceHeaders
  : TcxSchedulerResourceHeaders;
begin
  Result := View.OptionsView.ResourceHeaders;
end;

function TcxSchedulerCustomResourceViewViewInfo.GetPrintRange(Index: Integer)
  : TDateTime;
begin
...
function TcxSchedulerCustomResourceViewViewInfo.GetView
  : TcxSchedulerCustomResourceView;
begin
  Result := TcxSchedulerCustomResourceView(Owner);
end;

function TcxSchedulerCustomResourceViewViewInfo.HasMoreEventsButton
  (AResourceIndex: Integer): Boolean;
...
begin
  ...
  for I := 0 to Buttons.Count - 1 do
    ...
    begin
      ...
      if (AResourceIndex = -1) or AButton.Event.IsSharedWithResource
...
function TcxSchedulerCustomResourceViewViewInfo.
  IsDrawContentNavigationButtons: Boolean;
var
  I: Integer;
begin
...
begin
  ...
  begin
    AContentNavigationButton := TcxSchedulerContentNavigationButtonViewInfo
      (FNavigationButtons[I]);
    AContentNavigationButton.Interval := GetContentNavigationInterval
      (AContentNavigationButton);
  end;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.SetHitMoreEventsButton
  (AButton: TcxSchedulerMoreEventsButtonViewInfo;
AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
  ...
  begin
    ...
    begin
      ...
      BeginMouseTracking(Scheduler,
        cxRectOffset(FHotContentNavigationButton.Bounds, View.Left,
        View.Top), Self);
    end;
  end;
end;

procedure TcxSchedulerCustomResourceViewViewInfo.SetHotMoreEventsButton
  (AButton: TcxSchedulerMoreEventsButtonViewInfo);
begin
  ...
  begin
    ...
    begin
      View.RepaintRect(FHotMoreEventsButton.Bounds);
      BeginMouseTracking(Scheduler, cxRectOffset(FHotMoreEventsButton.Bounds,
        View.Left, View.Top), Self);
    end;
  end;
end;
...
begin
  Result := ResourceItem;
  AIndex := ViewInfo.FResources.IndexOf(ResourceObject) +
    Byte(AGoToForward) * 2 - 1;
  if (AIndex < 0) or (AIndex >= ViewInfo.FResources.Count) then
    Exit;
  Result := ViewInfo.GetResourceItemByIndex(AIndex);
end;

function TcxSchedulerCustomResourceViewNavigation.GetResourceItem
  : TcxSchedulerStorageResourceItem;
begin
  Result := nil;
...
function TcxSchedulerCustomResourceViewNavigation.RoundTime(const ADateTime
  : TDateTime): TDateTime;
begin
...
function TcxSchedulerCustomResourceViewNavigation.GetGroupingKind
  : TcxSchedulerGroupingKind;
begin
  Result := ViewInfo.FGroupingKind;
end;

function TcxSchedulerCustomResourceViewNavigation.GetResourceObject
  : TcxSchedulerResourceViewInfo;
begin
  Result := ViewInfo.FSelResource;
end;

function TcxSchedulerCustomResourceViewNavigation.GetViewInfo
  : TcxSchedulerCustomResourceViewViewInfo;
begin
...
function TcxSchedulerCustomResourceViewController.CreateNavigation
  : TcxSchedulerViewNavigation;
begin
  Result := TcxSchedulerCustomResourceViewNavigation.Create(View);
end;

function TcxSchedulerCustomResourceViewController.GetCursor(X,
  Y: Integer): TCursor;
const
...
procedure TcxSchedulerCustomResourceViewController.MouseMove(Shift: TShiftState;
X, Y: Integer);
begin
...
procedure TcxSchedulerCustomResourceViewController.MouseUp(Button: TMouseButton;
Shift: TShiftState; X, Y: Integer);
begin
  inherited MouseUp(Button, Shift, X, Y);
  if NavigationButtonClicked and HitTest.HitAtContentNavigationButton and
    (Button = mbLeft) then
...
procedure TcxSchedulerCustomResourceViewController.
  NavigationButtonDownCustomProcessing;
begin
  //
end;
...
procedure TcxSchedulerCustomResourceViewController.InternalShowHint(P: TPoint;
...
begin
  ...
  begin
    FHintText := AHintText;
    R := HintController.CalcHintRect(cxscMinHintWidth, AHintText,
      AFlags[View.UseRightToLeftAlignment]);
...
function TcxSchedulerCustomResourceViewController.NeedShowDayHeaderHint
  : Boolean;
var
  AGroupingKind: TcxSchedulerGroupingKind;
...
begin
  ...
  Result := Scheduler.OptionsView.ShowHints and HitTest.HitAtDayHeader and
    (HitTest.HitAtTime) and ((AGroupByDate and not HitTest.HitAtResourceHeader)
    or not AGroupByDate);
end;
...
function TcxSchedulerCustomResourceViewController.GetHintController
  : TcxSchedulerHintController;
begin
  Result := TcxCustomSchedulerAccess(Scheduler).HintController;
end;

function TcxSchedulerCustomResourceViewController.GetHitTest
  : TcxSchedulerCustomResourceViewHitTest;
begin
  Result := TcxSchedulerCustomResourceViewHitTest(inherited HitTest);
end;

function TcxSchedulerCustomResourceViewController.GetView
  : TcxSchedulerCustomResourceView;
begin
...
function TcxSchedulerCustomResourceViewHitTest.CanMoveEvent
  (AEvent: TcxSchedulerControlEvent): Boolean;
begin
  Result := Scheduler.EventOperations.Moving and TcxSchedulerControlEventAccess
    (AEvent).CanMove;
end;
...
begin
  if HitAtResource then
    Result := Scheduler.EventOperations.Sizing and
      TcxSchedulerControlEventAccess(AEvent).CanResize(Resource)
  else
    Result := Scheduler.EventOperations.Sizing and
      TcxSchedulerControlEventAccess(AEvent).CanResize;
end;
...
function TcxSchedulerCustomResourceViewHitTest.GetHitEvent
...
procedure TcxSchedulerCustomResourceViewHitTest.SetHitTime(AItemFlag: Integer;
const ATime: TDateTime);
begin
...
begin
  if (Event = nil) or ((FDragKind in [edkEventDragRect, edkMoveEvent]) and
    not CanMoveEvent(Event)) or ((FDragKind in [edkResizeStart, edkResizeEnd])
    and not CanResizeEvent(Event)) then
    FDragKind := edkNone;
end;

function TcxSchedulerCustomResourceViewHitTest.GetContentCell
  : TcxSchedulerContentCellViewInfo;
begin
...
function TcxSchedulerCustomResourceViewHitTest.GetHitContentNavigationButton
  : TcxSchedulerContentNavigationButtonViewInfo;
begin
...
function TcxSchedulerCustomResourceViewHitTest.GetHitMoreEventsButton
  : TcxSchedulerMoreEventsButtonViewInfo;
begin
  if HitAtButton then
    Result := FButton
...
function TcxSchedulerCustomResourceViewHitTest.GetHeaderCell
  : TcxSchedulerHeaderCellViewInfo;
begin
...
function TcxSchedulerCustomResourceViewHitTest.GetView
  : TcxSchedulerCustomResourceView;
begin
...
begin
  ...
  begin
    if Painter <> nil then
      Painter.DoCustomDrawButton(TcxSchedulerMoreEventsButtonViewInfo
        (AItem), ADone);
    if not ADone then
      View.DoCustomDrawButton(TcxSchedulerMoreEventsButtonViewInfo
        (AItem), ADone);
  end
  else
  begin
    if Painter <> nil then
      Painter.DoCustomDrawNavigationButton
        (TcxSchedulerContentNavigationButtonViewInfo(AItem), ADone);
    if not ADone then
      View.DoCustomDrawNavigationButton
        (TcxSchedulerContentNavigationButtonViewInfo(AItem), ADone);
  end;
end;
...
procedure TcxSchedulerCustomViewPainter.DrawEventCell
  (AItem: TcxSchedulerCustomViewInfoItem; var ADone: Boolean);
begin
  if TcxSchedulerEventCellViewInfo(AItem).Hidden or
    not TcxSchedulerEventCellViewInfo(AItem).Visible then
    Exit;
  if Painter <> nil then
...
begin
  if Painter <> nil then
    Painter.DoCustomDrawDayHeader(TcxSchedulerDayHeaderCellViewInfo
      (AItem), ADone);
  if not ADone then
...
begin
  if Painter <> nil then
    Painter.DoCustomDrawGroupSeparator(TcxSchedulerGroupSeparatorCellViewInfo
      (AItem), ADone);
  if not ADone then
    View.DoCustomDrawGroupSeparator(TcxSchedulerGroupSeparatorCellViewInfo
      (AItem), ADone);
end;
...
begin
  if Painter <> nil then
    Painter.DoCustomDrawResourceHeader
      (TcxSchedulerHeaderCellViewInfo(AItem), ADone);
  if not ADone then
    View.DoCustomDrawResourceHeader
      (TcxSchedulerHeaderCellViewInfo(AItem), ADone);
end;
...
function TcxSchedulerEventPlace.IntersectHorz
  (APlace: TcxSchedulerEventPlace): Boolean;
begin
  Result := (APlace.Resource = Resource) and (APlace.ColFinish >= ColStart) and
    (APlace.ColStart <= ColFinish);
end;

function TcxSchedulerEventPlace.IntersectVert
  (APlace: TcxSchedulerEventPlace): Boolean;
begin
  Result := (APlace.Resource = Resource) and (APlace.LineFinish >= LineStart)
    and (APlace.LineStart <= LineFinish);
end;
...
constructor TcxSchedulerEventLayoutBuilder.Create(const AViewStyle
  : TcxSchedulerViewStyle);
var
...
begin
  ...
  try
    ...
    begin
      ...
      ABounds := AManager.GetFreeBounds(cxRect(APlace.ColStart,
        APlace.LineStart, APlace.ColFinish, APlace.LineFinish));
...
procedure TcxSchedulerEventLayoutBuilder.CalculateEx
  (AEventsList: TcxSchedulerFilteredEventList;
APlaceInfoProc: TcxSchedulerEventLayoutBuilderGetEventPlaceProc);
var
  AEvent: TcxSchedulerControlEvent;
...
begin
  ...
  begin
    ...
    ALineCount := 1;
    if APlaceInfoProc(Self, AEventsList[I], AStartCol, AFinishCol, ALineCount)
    then
...
function TcxSchedulerEventLayoutBuilder.CreateComparer
  : IComparer<TcxSchedulerEventPlace>;
begin
  Result := TcxSchedulerEventPlaceComparer.Create(ViewStyle);
end;

function TcxSchedulerEventLayoutBuilder.GetEventPlace(AIndex: Integer)
  : TcxSchedulerEventPlace;
begin
