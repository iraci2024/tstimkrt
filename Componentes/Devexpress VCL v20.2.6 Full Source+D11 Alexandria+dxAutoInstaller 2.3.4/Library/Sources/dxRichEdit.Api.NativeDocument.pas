...
type
  ...
  public
    ...
    function InsertContent(APieceTable: TdxPieceTable;
    ...
    function InsertContentCore(APieceTable: TdxPieceTable;
      ALogPosition: TdxDocumentLogPosition): Boolean; virtual; abstract;
  ...
  TdxNativeDocumentPosition = class(TInterfacedObject,
  ...
  strict private
    ...
    function GetLength: Integer;
  public
    constructor Create(const AStart,
      AEnd: IdxRichEditDocumentPosition); overload;
    constructor Create(const ADocument: IdxRichEditSubDocument;
    ...
    property NormalizedStart: IdxRichEditDocumentPosition
      read GetNormalizedStart;
  ...
  TdxNativeFieldCollection = class(TdxIUnknownList<IdxRichEditField>,
    IdxRichEditFieldCollection, IdxRichEditReadOnlyFieldCollection)
  strict private
  ...
  public
    ...
    function Add(const ACodeRange: IdxRichEditDocumentRange)
      : IdxRichEditField; overload;
    function Add(const AStart: IdxRichEditDocumentPosition; const ACode: string)
      : IdxRichEditField; overload;
    function CreateField(const ACodeRange: IdxRichEditDocumentRange)
      : IdxRichEditField; overload;
    function CreateField(const AStart: IdxRichEditDocumentPosition;
      const ACode: string): IdxRichEditField; overload;
    function Get(const ARange: IdxRichEditDocumentRange)
  ...
  protected
    ...
    procedure OnHyperlinkInfoInserted(ASender: TObject;
      E: TdxHyperlinkInfoEventArgs);
    procedure OnHyperlinkInfoDeleted(ASender: TObject;
      E: TdxHyperlinkInfoEventArgs);
    procedure OnTablesCollectionChanged(const AItem: TObject;
      Action: TListNotification);
    function CreateNativeRange(const AStart, AEnd: TdxDocumentModelPosition)
      : TdxNativeDocumentRange;
    function CreateZeroLengthRange(ALogPosition: TdxDocumentLogPosition)
      : IdxRichEditDocumentRange;
    function CreateNativePosition(const APos: TdxDocumentModelPosition)
      : TdxNativeDocumentPosition;
    function GetSelectionRangeCollection(const ARange: IdxRichEditDocumentRange)
    ...
    function AppendDocumentContentCore(AInserter: TdxDocumentContentInserter)
      : IdxRichEditDocumentRange;
  public
    constructor Create(APieceTable: TdxPieceTable;
      AServer: TdxInnerRichEditDocumentServer); override;

    procedure UnitsChanged; override;

    procedure BeginUpdate;
    procedure EndUpdate;
    function InsertParagraph(const APos: IdxRichEditDocumentPosition)
      : IdxRichEditParagraph; overload;
    function InsertParagraph(const APos: IdxRichEditDocumentPosition;
      const AInsertOptions: TdxRichEditInsertOptions)
      : IdxRichEditParagraph; overload;
    function AppendParagraph: IdxRichEditParagraph;
    function InsertContentCore(const APos: IdxRichEditDocumentPosition;
    ...
    function InsertText(const APos: IdxRichEditDocumentPosition;
      const AText: string): IdxRichEditDocumentRange;

    function AppendSingleLineText(const AText: string)
      : IdxRichEditDocumentRange;
    function InsertSingleLineText(const APos: IdxRichEditDocumentPosition;
      const AText: string): IdxRichEditDocumentRange;

    function AppendRtfText(const ARtfText: string;
      AInsertOptions: TdxRichEditInsertOptions = TdxRichEditInsertOptions.
      MatchDestinationFormatting): IdxRichEditDocumentRange;
    ...
    function InsertImage(const APos: IdxRichEditDocumentPosition;
      AImage: TGraphic): IdxRichEditDocumentImage;

    function InsertPicture(const APos: IdxRichEditDocumentPosition;
      AImage: TGraphic): IdxRichEditShape; virtual;
    function InsertTextBox(const APos: IdxRichEditDocumentPosition)
      : IdxRichEditShape; virtual;

    function InsertTable(const APos: IdxRichEditDocumentPosition;
      ARowCount, AColumnCount: Integer;
      AAutoFitBehavior: TdxRichEditAutoFitBehaviorType =
      TdxRichEditAutoFitBehaviorType.AutoFitToContents;
      AFixedColumnWidths: Integer = MinInt): IdxRichEditTable;

    function CreateRange(AStart, ALength: Integer)
      : IdxRichEditDocumentRange; overload;
    function CreateRange(const AStart: IdxRichEditDocumentPosition;
      ALength: Integer): IdxRichEditDocumentRange; overload;
    function CreatePosition(AStart: Integer): IdxRichEditDocumentPosition;

    procedure Delete(const ARange: IdxRichEditDocumentRange);
    procedure SelectAll;

    function CalculateParagraphsRange(const ARange: IdxRichEditDocumentRange)
      : TdxParagraphRange;
    function BeginUpdateParagraphs(const ARange: IdxRichEditDocumentRange)
      : IdxRichEditParagraphProperties;
    procedure EndUpdateParagraphs(const AProperties
      : IdxRichEditParagraphProperties);
    function BeginUpdateCharacters(AStart, ALength: Integer)
      : IdxRichEditCharacterProperties; overload;
    function BeginUpdateCharacters(const AStart: IdxRichEditDocumentPosition;
      ALength: Integer): IdxRichEditCharacterProperties; overload;
    function BeginUpdateCharacters(const ARange: IdxRichEditDocumentRange)
      : IdxRichEditCharacterProperties; overload;
    function BeginUpdateCharactersCore(AStart, AEnd: Integer)
      : IdxRichEditCharacterProperties;
    procedure EndUpdateCharacters(const AProperties
      : IdxRichEditCharacterProperties);

    procedure ApplyNewRangePermissions(const APermissions
      : IdxRichEditRangePermissionCollection);
    procedure ApplyNewRangePermission(const ARangePermission
      : IdxRichEditRangePermission);
    function CreateRangePermission(const ACollection
      : IdxRichEditRangePermissionCollection;
    ...
    procedure CancelUpdateRangePermissions(const APermissions
      : IdxRichEditRangePermissionCollection);
    procedure EndUpdateRangePermissions(const APermissions
      : IdxRichEditRangePermissionCollection);

    function CreateBookmark(const AStart: IdxRichEditDocumentPosition;
      ALength: Integer; const AName: string): IdxRichEditBookmark; overload;
    function CreateBookmark(const ARange: IdxRichEditDocumentRange;
      const AName: string): IdxRichEditBookmark; overload;
    ...
    function CreateHyperlink(const AStart: IdxRichEditDocumentPosition;
      ALength: Integer): IdxRichEditHyperlink; overload;
    function CreateHyperlink(const ARange: IdxRichEditDocumentRange)
      : IdxRichEditHyperlink; overload;
    procedure RemoveHyperlink(const AHyperlink: IdxRichEditHyperlink);

    function GetParagraph(const APos: IdxRichEditDocumentPosition)
      : IdxRichEditParagraph;
    function GetParagraphs(const ARange: IdxRichEditDocumentRange)
      : IdxRichEditParagraphCollection; overload;
    ...
    function GetImages(const ARange: IdxRichEditDocumentRange)
      : IdxRichEditReadOnlyDocumentImageCollection; overload;
    function GetShapes(const ARange: IdxRichEditDocumentRange)
      : IdxRichEditReadOnlyShapeCollection; overload;
    ...
    function GetText(const ARange: IdxRichEditDocumentRange;
      const AFragmentOptions: TdxRichEditTextFragmentOptions): string; overload;

    function Replace(const ARange: IdxRichEditDocumentRange;
      const AText: string): Integer;
    function FindAll(const ATextToFind: string;
      AOptions: TdxRichEditSearchOptions = [];
      const ARange: IdxRichEditDocumentRange = nil)
      : TArray<IdxRichEditDocumentRange>; overload;
    function FindAll(const ARegex: TRegEx;
      const ARange: IdxRichEditDocumentRange = nil)
      : TArray<IdxRichEditDocumentRange>; overload;
    ...
    function ReplaceAll(const ARegex: TRegEx; const AReplaceWith: string;
      const ARange: IdxRichEditDocumentRange = nil): Integer; overload;
    function StartSearch(const ATextToFind: string;
      AOptions: TdxRichEditSearchOptions = [];
      ADirection: TdxRichEditSearchDirection = TdxRichEditSearchDirection.
      Forward; const ARange: IdxRichEditDocumentRange = nil)
      : IdxRichEditSearchResult; overload;
    function StartSearch(const ARegex: TRegEx;
      const ARange: IdxRichEditDocumentRange = nil)
      : IdxRichEditRegexSearchResult; overload;
    ...
    procedure RemoveNumberingFromParagraph(const AParagraph
      : IdxRichEditParagraph);
    procedure RemoveNumberingFromParagraphs(const ARange
      : IdxRichEditDocumentRange);
  ...
  public
    ...
    procedure CheckDocumentPosition(const ADocumentPosition
      : IdxRichEditDocumentPosition);
    procedure CheckDocumentRange(const ADocumentRange
      : IdxRichEditDocumentRange);
    function CreatePositionCore(APos: TdxDocumentLogPosition)
      : IdxRichEditDocumentPosition;
    procedure SetSelectionCore(AStart, AEnd: TdxDocumentLogPosition;
      AForceUpdateTableSelection: Boolean = True);

    function NormalizeLogPosition(APos: TdxDocumentLogPosition)
      : TdxDocumentLogPosition; overload;
    function NormalizeLogPosition(APieceTable: TdxPieceTable;
      APos: TdxDocumentLogPosition): TdxDocumentLogPosition; overload;
    ...
    function GetInnerTableCellStyle(const AStyle: IdxRichEditTableCellStyle)
      : TdxTableCellStyle;

    function ModelUnitsToUnits(Value: Integer): Single; overload;
    function ModelUnitsToUnitsF(Value: Single): Single; overload;
    function ModelUnitsToUnits(const Value: TdxNullableInteger)
      : TdxNullableSingle; overload;
    function ModelUnitsToUnitsF(const Value: TdxNullableSingle)
      : TdxNullableSingle; overload;
  ...
  TdxNativeSelectionCollection = class(TInterfacedObject,
...
{$ENDREGION}
  ...
  type
{$REGION 'private type'}
    TSelectionRangeType = (Single, Start, Middle, &End);
{$ENDREGION}
  strict private
    FDocument: TdxNativeDocument;

    function AreSelectionItemsIntersected(const AItem1,
      AItem2: IdxRichEditDocumentRange): Boolean;
    procedure CalcColumnsAndCellsCount(ARow: TdxTableRow;
      AStartIndexInRow, AEndIndexInRow: Integer;
      out AColumnCount, ACellCount: Integer);
    function CloneCollectionByValue(ACollection: TdxSelectionItemList)
      : TdxSelectionItemList;
    procedure CorrectEndLogPosByEndOfDocument
      (ARanges: TList<IdxRichEditDocumentRange>);
    function FillGroup(ASortedRanges: TList<IdxRichEditDocumentRange>;
      APrevRangeIndex: Integer; APrevStartCell: TdxTableCell;
      ASelectedCellCount: Integer; ASelectedColumnCount: Integer;
      ACurrentGroup: TList<IdxRichEditDocumentRange>): Integer;
    procedure InternalAdd(const ARange: IdxRichEditDocumentRange;
      ASelElementType: TSelectionRangeType);
    function IsCellPartiallySelected(ACell: TdxTableCell;
      APos: TdxDocumentLogPosition): Boolean;
    function IsEndOfDocument(APos: TdxDocumentLogPosition): Boolean;
    function IsLogPositionSelectAnotherCell(ACell: TdxTableCell;
      APos: TdxDocumentLogPosition): Boolean;
    function IsParent(AParent, AChild: TdxTableCell): Boolean;
    function GetCell(APieceTable: TdxPieceTable; APos: TdxDocumentLogPosition)
      : TdxTableCell;
    ...
    function GetGroupedSortedRanges(ARanges: TList<IdxRichEditDocumentRange>)
      : TdxObjectList<TList<IdxRichEditDocumentRange>>;
    function GetInnerSelection: TdxSelection;
    function GetLastSelectedCell(AStartCell, AEndCell: TdxTableCell;
      AEndLogPosition: TdxDocumentLogPosition): TdxTableCell;
    function GetLastNormalizedCellInRow(ARow: TdxTableRow): TdxTableCell;
    function GetParentCellWithNestedLevel(ACell: TdxTableCell;
      ANestedLevel: Integer): TdxTableCell;
    function GetPreviousCellInRow(ACell: TdxTableCell): TdxTableCell;
    function MergeNeighbourhoodRanges(ASortedRanges
      : TList<IdxRichEditDocumentRange>): TList<IdxRichEditDocumentRange>;
    procedure NormalizeCells(ACell1, ACell2: TdxTableCell;
      out ANormalizedCell1, ANormalizedCell2: TdxTableCell);
    procedure RemoveRangesWithZeroWidth
      (ARanges: TList<IdxRichEditDocumentRange>);
    function SameRowCell(ACell1, ACell2: TdxTableCell): Boolean;
    function ShouldSkipValidation(AStart, AEnd, ALastSelected
      : TdxTableCell): Boolean;
    function SortRanges(AItems: TList<IdxRichEditDocumentRange>)
      : TList<IdxRichEditDocumentRange>;
    function SplitRanges(ASortedRanges: TList<IdxRichEditDocumentRange>)
      : TList<IdxRichEditDocumentRange>;
    function SplitRangesByRowWithChildTables(ASortedRanges
      : TList<IdxRichEditDocumentRange>): TList<IdxRichEditDocumentRange>;
    function SplitRangesByRowSameTable(ASortedRanges
      : TList<IdxRichEditDocumentRange>): TList<IdxRichEditDocumentRange>;
    procedure SplitRows(ARange: TdxNativeDocumentRange; ATable: TdxTable;
      AStartRowIndex, AEndRowIndex: Integer;
      ARanges: TList<IdxRichEditDocumentRange>);
    procedure UnselectCore(ACloneCollection: TdxSelectionItemList;
      ARange: TdxNativeDocumentRange);
    procedure ValidateSelection(const ARange: IdxRichEditDocumentRange;
      ASelElementType: TSelectionRangeType; AIsRemoved: Boolean = False);
    procedure ValidateSelections(AGroupedItems
      : TdxObjectList<TList<IdxRichEditDocumentRange>>;
      AIsRemoved: Boolean = False);
    procedure ValidateSelectionCells(const ARange: IdxRichEditDocumentRange;
      ASelElementType: TSelectionRangeType);
    procedure ValidateCellsWithinSameTable(AFirstSelectedCell, ALastSelectedCell
      : TdxTableCell; ASelStartLogPosition, ASelEndLogPosition
      : TdxDocumentLogPosition; ASelElementType: TSelectionRangeType);
    procedure ValidateCellsWithinDifferentTables(AFirstSelectedCell,
      ALastSelectedCell: TdxTableCell; ASelStartLogPosition, ASelEndLogPosition
      : TdxDocumentLogPosition);
    procedure ValidateTableCellTextBeforeTable(ASelectedCell: TdxTableCell;
      ASelEndLogPosition: TdxDocumentLogPosition;
      ACellForNormalize: TdxTableCell = nil);
  ...
  protected
    function CreateDocumentRange(const AStart, AEnd: TdxDocumentLogPosition)
      : IdxRichEditDocumentRange; overload;
    function CreateDocumentRange(const AItem: TdxSelectionItem)
      : IdxRichEditDocumentRange; overload;
    function CreateSelectionItem(AStart, AEnd: TdxDocumentLogPosition)
      : TdxSelectionItem; overload;
    function CreateSelectionItem(const ARange: IdxRichEditDocumentRange)
      : TdxSelectionItem; overload;
  ...
  public
    ...
    property Items[Index: Integer]: IdxRichEditDocumentRange
  ...
  strict private
    ...
    function GetAbstractNumberingLists
  ...
  protected
    ...
    function CreateNativeSection(ASection: TdxSection)
      : IdxRichEditSection; virtual;
    procedure DoSubscribeInternalAPIEvents; override;
    procedure DoUnsubscribeInternalAPIEvents; override;
    ...
    procedure LoadDocumentCore(AStream: TStream;
      AFormat: TdxRichEditDocumentFormat; const ASourceUri: string); virtual;
    class procedure SaveDocument(ADocumentModel: TdxDocumentModel;
      const AFileName: string; AFormat: TdxRichEditDocumentFormat);
      overload; static;
  ...
  public
    ...
    procedure LoadDocument(AStream: TStream;
      AFormat: TdxRichEditDocumentFormat); overload;
    procedure LoadDocument(AStream: TStream; AFormat: TdxRichEditDocumentFormat;
      const ASourceUri: string); overload;
    procedure LoadDocument(const AFileName: string;
      AFormat: TdxRichEditDocumentFormat); overload;
    procedure LoadDocument(const AFileName: string;
      AFormat: TdxRichEditDocumentFormat; const ASourceUri: string); overload;
    procedure SaveDocument(AStream: TStream;
      AFormat: TdxRichEditDocumentFormat); overload;
    procedure SaveDocumentCore(AStream: TStream;
      AFormat: TdxRichEditDocumentFormat; const ATargetUri: string); overload;
    procedure SaveDocument(const AFileName: string;
      AFormat: TdxRichEditDocumentFormat); overload;

    function InsertSection(const APos: IdxRichEditDocumentPosition)
      : IdxRichEditSection;
    function AppendSection: IdxRichEditSection;
    function InsertSectionCore(const APos: IdxRichEditDocumentPosition;
      AAppend: Boolean): IdxRichEditSection;
    function GetSection(const APos: IdxRichEditDocumentPosition)
      : IdxRichEditSection;

    procedure ChangeActiveDocument(const ADocument: IdxRichEditSubDocument);
    procedure ChangeActivePieceTable(const AControl: IdxRichEditControl;
    ...
    function GetSubDocument(const APos: IdxRichEditDocumentPosition)
      : TdxNativeSubDocument;

    function CreateMailMergeOptions: IdxRichEditMailMergeOptions;
    procedure MailMerge(const AFileName: string;
      AFormat: TdxRichEditDocumentFormat); overload;
    procedure MailMerge(AStream: TStream;
      AFormat: TdxRichEditDocumentFormat); overload;
    procedure MailMerge(const ATargetDocument: IdxRichEditDocument); overload;
    procedure MailMerge(const AOptions: IdxRichEditMailMergeOptions;
      const AFileName: string; AFormat: TdxRichEditDocumentFormat); overload;
    procedure MailMerge(const AOptions: IdxRichEditMailMergeOptions;
      AStream: TStream; AFormat: TdxRichEditDocumentFormat); overload;
    procedure MailMerge(const AOptions: IdxRichEditMailMergeOptions;
      const ATargetDocument: IdxRichEditDocument); overload;
    ...
    property AbstractNumberingLists: IdxRichEditAbstractNumberingListCollection
      read GetAbstractNumberingLists;
    property ActiveSubDocument: IdxRichEditSubDocument
      read GetActiveSubDocument;
    property CaretPosition: IdxRichEditDocumentPosition read GetCaretPosition
      write SetCaretPosition;
    property CharacterStyles: IdxRichEditCharacterStyleCollection
      read GetCharacterStyles;
    property IsDocumentProtected: Boolean read GetIsDocumentProtected;
    property IsEmpty: Boolean read GetIsEmpty;
    property NumberingLists: IdxRichEditNumberingListCollection
      read GetNumberingLists;
    property ParagraphStyles: IdxRichEditParagraphStyleCollection
      read GetParagraphStyles;
    property Sections: IdxRichEditSectionCollection read GetSections;
    property Selection: IdxRichEditDocumentRange read GetSelection
      write SetSelection;
...
type
  ...
  TdxPieceTablePasteTextContentConvertedToDocumentModelCommandBaseAccess = class
    (TdxPieceTablePasteTextContentConvertedToDocumentModelCommandBase);

  { TdxNativeSectionCollection }

  TdxNativeSectionCollection = class(TdxIUnknownList<IdxRichEditSection>,
  ...
  protected
    ...
    function CreateDocumentModel(ATargetDocumentModel: TdxDocumentModel)
  ...
  public
    constructor Create(AStream: TStream; AFormat: TdxRichEditDocumentFormat;
      const ASourceUri: string; AInsertOptions: TdxRichEditInsertOptions);
      reintroduce;
  end;
  ...
  TdxFormattedDocumentContentInserter = class abstract
    (TdxDocumentContentInserter)
  strict private
    FText: string;
    FInsertOptions: TdxRichEditInsertOptions;
  ...
  TdxRangeComparer = class(TInterfacedObject,
    IComparer<IdxRichEditDocumentRange>)
  public
...
function TdxDocumentTextContentInserter.InsertContentCore
...
constructor TdxDocumentModelContentInserter.Create(AInsertOptions
...
function TdxDocumentModelContentInserter.InsertContentCore
  (APieceTable: TdxPieceTable; ALogPosition: TdxDocumentLogPosition): Boolean;
var
...
begin
  ...
  try
    ...
    ACommand := TdxPieceTableInsertContentConvertedToDocumentModelCommand.Create
      (APieceTable, ADocumentModel, ALogPosition, InsertOptions, False);
    try
...
constructor TdxDocumentStreamContentInserterEventRouter.Create
  (ADocumentModel: TdxDocumentModel);
begin
  inherited Create;
...
procedure TdxDocumentStreamContentInserterEventRouter.SubscribeEvents
  (ADocumentModel: TdxDocumentModel);
begin
...
procedure TdxDocumentStreamContentInserterEventRouter.DoBeforeImport
  (ASender: TObject; E: TdxBeforeImportEventArgs);
begin
...
function TdxDocumentStreamContentInserter.CreateDocumentModel
  (ATargetDocumentModel: TdxDocumentModel): TdxDocumentModel;
var
...
begin
  ...
  ARouter := TdxDocumentStreamContentInserterEventRouter.Create
    (ATargetDocumentModel);
  try
    ARouter.SubscribeEvents(Result);
    try
      AImportManagerService :=
        ATargetDocumentModel.GetService<IdxDocumentImportManagerService>;
      ...
      try
        AImportHelper.Import(FStream, FFormat, FSourceUri,
          AImportManagerService);
      finally
        AImportHelper.Free;
      end;
...
constructor TdxDocumentRangeContentInserter.Create(const ASourceRange
  : IdxRichEditDocumentRange; AInsertOptions: TdxRichEditInsertOptions);
begin
  inherited Create(AInsertOptions);
...
function TdxDocumentRangeContentInserter.CreateDocumentModel
  (ATargetDocumentModel: TdxDocumentModel): TdxDocumentModel;
var
...
begin
  ADocument := TdxNativeSubDocument
    (TdxNativeDocumentPosition(FSourceRange.Start).Document);

  ASelectionRanges := ADocument.GetSelectionRangeCollection(FSourceRange);
  if ASelectionRanges = nil then
    Exit(nil);

  AManager := TdxCopySelectionManager.Create
    (ADocument.DocumentServer as IdxInnerControl);
  try
    ...
    Result := TdxCopySelectionManagerAccess(AManager)
      .CreateDocumentModel
      (TdxParagraphNumerationCopyOptions.CopyIfWholeSelected, AOptions,
      ADocument.PieceTable, ASelectionRanges);
  finally
    AManager.Free;
  end;
end;
...
function TdxFormattedDocumentContentInserter.InsertContentCore
  (APieceTable: TdxPieceTable; ALogPosition: TdxDocumentLogPosition): Boolean;
var
...
begin
  ...
  try
    ...
    try
      ...
      try
        TdxPieceTablePasteTextContentConvertedToDocumentModelCommandBaseAccess
          (ACommand).PasteContent(AContent, ALogPosition, '');
      finally
        AContent.Free;
      end;
    finally
...
function TdxDocumentRtfTextContentInserter.CreateCommand
  (APieceTable: TdxPieceTable; AInsertOptions: TdxRichEditInsertOptions)
  : TdxPieceTablePasteTextContentConvertedToDocumentModelCommandBase;
begin
  Result := TdxPieceTablePasteRtfTextCommand.Create(APieceTable,
    AInsertOptions);
end;

{ TdxDocumentSingleLineTextContentInserter }

constructor TdxDocumentSingleLineTextContentInserter.Create
  (const AText: string);
begin
  inherited Create;
  FText := AText;
...
function TdxRangeComparer.Compare(const Left,
  Right: IdxRichEditDocumentRange): Integer;
begin
  Result := TdxNativeDocumentRange(Left).NormalizedStart.CompareTo
    (TdxNativeDocumentRange(Right).NormalizedStart);
end;
...
function TdxDocumentContentInserter.InsertContent(APieceTable: TdxPieceTable;
  ALogPosition: TdxDocumentLogPosition): Boolean;
var
...
begin
  ...
  ALastParagraph.ParagraphProperties.CopyFrom
    (APreviousToLastParagraph.ParagraphProperties);
  ALastParagraph.ParagraphStyleIndex :=
    APreviousToLastParagraph.ParagraphStyleIndex;
end;
...
constructor TdxNativeDocumentPosition.Create(const ADocument
  : IdxRichEditSubDocument; const APos: TdxDocumentModelPosition);
begin
  inherited Create;
  FPosition := APos;
...
begin
  Result := (Self = AObj) or
    ((Self <> nil) and Supports(AObj, IdxRichEditDocumentPosition, APos) and
...
function TdxNativeDocumentPosition.CompareTo(const APos
  : IdxRichEditDocumentPosition): Integer;
begin
...
constructor TdxNativeDocumentRange.Create(const AStart,
  AEnd: IdxRichEditDocumentPosition);
begin
  inherited Create;
  FStart := AStart;
  FEnd := AEnd;
...
constructor TdxNativeDocumentRange.Create(const ADocument
  : IdxRichEditSubDocument; const AStart, AEnd: TdxDocumentModelPosition);
begin
  inherited Create;
...
function TdxNativeDocumentRange.Contains(const APos
  : IdxRichEditDocumentPosition): Boolean;
begin
...
function TdxNativeFieldCollection.Add(const ACodeRange
  : IdxRichEditDocumentRange): IdxRichEditField;
var
  AField: TdxField;
begin
  AField := FDocument.PieceTable.CreateField(ACodeRange.Start.LogPosition,
    ACodeRange.Length);
  Result := Self[AField.Index];
end;

function TdxNativeFieldCollection.Add(const AStart: IdxRichEditDocumentPosition;
  const ACode: string): IdxRichEditField;
var
...
function TdxNativeFieldCollection.CreateField(const ACodeRange
  : IdxRichEditDocumentRange): IdxRichEditField;
var
  AField: TdxField;
begin
...
function TdxNativeFieldCollection.CreateField(const AStart
  : IdxRichEditDocumentPosition; const ACode: string): IdxRichEditField;
var
  ARange: IdxRichEditDocumentRange;
begin
  FDocument.BeginUpdate;
...
function TdxNativeFieldCollection.Get(const ARange: IdxRichEditDocumentRange)
  : IdxRichEditReadOnlyFieldCollection;
var
...
begin
  ...
  AFirstRunIndex := TdxNativeDocumentPosition(ANativeRange.NormalizedStart)
    .Position.RunIndex;
  ALastRunIndex := TdxNativeDocumentPosition(ANativeRange.NormalizedEnd)
    .Position.RunIndex;
...
constructor TdxNativeSubDocument.Create(APieceTable: TdxPieceTable;
...
function TdxNativeSubDocument.GetTableCellStyle(ACell: TdxTableCell)
  : IdxRichEditTableCellStyle;
begin
  Result := nil;
end;

function TdxNativeSubDocument.GetInnerTableCellStyle(const AStyle
  : IdxRichEditTableCellStyle): TdxTableCellStyle;
begin
  Result := nil;
end;
...
begin
  ...
  AParagraphs := APieceTable.Paragraphs;
  Result := CreateNativeRange(TdxDocumentModelPosition.FromParagraphStart
    (APieceTable, AParagraphs.First.Index),
    TdxDocumentModelPosition.FromParagraphEnd(APieceTable,
    AParagraphs.Last.Index));
end;
...
begin
  if Self = nil then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseInvalidDocument));
end;
...
procedure TdxNativeSubDocument.RegisterParagraph(const AParagraph
  : TdxCustomParagraph);
begin
  TdxNativeParagraphCollection(FParagraphs)
    .Add(TdxNativeParagraph.Create(Self, TdxParagraph(AParagraph)));
end;
...
procedure TdxNativeSubDocument.OnParagraphInserted(ASender: TObject;
  E: TdxParagraphEventArgs);
var
...
begin
  ...
  AParagraph := TdxNativeParagraph.Create(Self,
    PieceTable.Paragraphs[E.ParagraphIndex]);
  if FParagraphs.Count <= AParagraphIndex then
    TdxNativeParagraphCollection(FParagraphs).Add(AParagraph)
  else
    TdxNativeParagraphCollection(FParagraphs).Insert(AParagraphIndex,
      AParagraph);
end;

procedure TdxNativeSubDocument.OnParagraphRemoved(ASender: TObject;
  E: TdxParagraphEventArgs);
begin
  if PieceTable <> E.PieceTable then
    Exit;
...
procedure TdxNativeSubDocument.OnParagraphMerged(ASender: TObject;
  E: TdxParagraphEventArgs);
begin
  if PieceTable <> E.PieceTable then
    Exit;
...
procedure TdxNativeSubDocument.OnFieldInserted(ASender: TObject;
  E: TdxFieldEventArgs);
var
  AFieldIndex: Integer;
  AField: TdxNativeField;
...
procedure TdxNativeSubDocument.OnFieldRemoved(ASender: TObject;
  E: TdxFieldEventArgs);
var
  AFieldIndex: Integer;
  AField: TdxNativeField;
...
procedure TdxNativeSubDocument.OnHyperlinkInfoInserted(ASender: TObject;
  E: TdxHyperlinkInfoEventArgs);
var
...
begin
  ...
  AHyperlink := TdxNativeHyperlink.Create(Self, PieceTable,
    PieceTable.Fields[E.FieldIndex]);
  TdxNativeHyperlinkCollection(FHyperlinks).Add(AHyperlink);
end;

procedure TdxNativeSubDocument.OnHyperlinkInfoDeleted(ASender: TObject;
  E: TdxHyperlinkInfoEventArgs);
var
  AHyperlink: IdxRichEditHyperlink;
begin
...
procedure TdxNativeSubDocument.OnTablesCollectionChanged(const AItem: TObject;
  Action: TListNotification);
begin
  TdxNativeTableCollection(FTables).TablesCollectionChanged
    (TdxTable(AItem), Action);
end;
...
function TdxNativeSubDocument.ModelUnitsToUnits(const Value: TdxNullableInteger)
  : TdxNullableSingle;
begin
...
function TdxNativeSubDocument.ModelUnitsToUnitsF(const Value: TdxNullableSingle)
  : TdxNullableSingle;
begin
  if Value.HasValue then
...
function TdxNativeSubDocument.NormalizeLogPosition(APos: TdxDocumentLogPosition)
  : TdxDocumentLogPosition;
begin
...
function TdxNativeSubDocument.InsertParagraph
  (const APos: IdxRichEditDocumentPosition): IdxRichEditParagraph;
begin
  Result := InsertParagraph(APos,
...
begin
  ...
  if AParagraph.IsInList and
    (AInsertOptions = TdxRichEditInsertOptions.KeepSourceFormatting) then
...
function TdxNativeSubDocument.InsertContentCore
  (const APos: IdxRichEditDocumentPosition;
  AInserter: TdxDocumentContentInserter): IdxRichEditDocumentRange;
var
  ANativePosition: TdxNativeDocumentPosition;
...
function TdxNativeSubDocument.InsertText(const APos
  : IdxRichEditDocumentPosition; const AText: string): IdxRichEditDocumentRange;
var
...
function TdxNativeSubDocument.AppendText(const AText: string)
  : IdxRichEditDocumentRange;
var
  AInserter: TdxDocumentTextContentInserter;
begin
...
function TdxNativeSubDocument.AppendSingleLineText(const AText: string)
  : IdxRichEditDocumentRange;
begin
  Result := InsertSingleLineText(EndPosition, AText);
end;

function TdxNativeSubDocument.InsertSingleLineText
  (const APos: IdxRichEditDocumentPosition; const AText: string)
  : IdxRichEditDocumentRange;
var
...
function TdxNativeSubDocument.InsertRtfText(const APos
  : IdxRichEditDocumentPosition; const ARtfText: string;
  AInsertOptions: TdxRichEditInsertOptions = TdxRichEditInsertOptions.
  MatchDestinationFormatting): IdxRichEditDocumentRange;
var
  AInserter: TdxDocumentRtfTextContentInserter;
begin
  AInserter := TdxDocumentRtfTextContentInserter.Create(ARtfText,
    AInsertOptions);
  try
    Result := InsertContentCore(APos, AInserter);
  finally
...
function TdxNativeSubDocument.InsertPicture(const APos
  : IdxRichEditDocumentPosition; AImage: TGraphic): IdxRichEditShape;
begin
  Result := Shapes.InsertPicture(APos, AImage);
end;

function TdxNativeSubDocument.InsertTextBox(const APos
  : IdxRichEditDocumentPosition): IdxRichEditShape;
begin
  Result := Shapes.InsertTextBox(APos);
end;

function TdxNativeSubDocument.InsertImage(const APos
  : IdxRichEditDocumentPosition; AImage: TGraphic): IdxRichEditDocumentImage;
begin
  CheckValid;
  CheckDocumentPosition(APos);
...
function TdxNativeSubDocument.AppendImage(AImage: TGraphic)
  : IdxRichEditDocumentImage;
begin
  Result := InsertImage(EndPosition, AImage);
end;

function TdxNativeSubDocument.InsertTable(const APos
  : IdxRichEditDocumentPosition; ARowCount, AColumnCount: Integer;
  AAutoFitBehavior: TdxRichEditAutoFitBehaviorType =
  TdxRichEditAutoFitBehaviorType.AutoFitToContents;
  AFixedColumnWidths: Integer = MinInt): IdxRichEditTable;
begin
  Result := Tables.Add(APos, ARowCount, AColumnCount, AAutoFitBehavior,
    AFixedColumnWidths);
end;

function TdxNativeSubDocument.CreateRange(AStart, ALength: Integer)
  : IdxRichEditDocumentRange;
var
...
function TdxNativeSubDocument.CreateRange(const AStart
  : IdxRichEditDocumentPosition; ALength: Integer): IdxRichEditDocumentRange;
begin
  Result := CreateRange(AStart.LogPosition, ALength);
end;

function TdxNativeSubDocument.CreateNativeRange(const AStart,
  AEnd: TdxDocumentModelPosition): TdxNativeDocumentRange;
begin
  Result := TdxNativeDocumentRange.Create(Self, AStart, AEnd);
end;

function TdxNativeSubDocument.CreateZeroLengthRange(ALogPosition
  : TdxDocumentLogPosition): IdxRichEditDocumentRange;
begin
  Result := TdxNativeDocumentRange.Create(CreatePositionCore(ALogPosition),
    CreatePositionCore(ALogPosition));
end;

function TdxNativeSubDocument.CreatePosition(AStart: Integer)
  : IdxRichEditDocumentPosition;
begin
  CheckValid;
...
function TdxNativeSubDocument.CreatePositionCore(APos: TdxDocumentLogPosition)
  : IdxRichEditDocumentPosition;
var
  APieceTable: TdxPieceTable;
  ARangeInfo: TdxRunInfo;
...
function TdxNativeSubDocument.CreateNativePosition
  (const APos: TdxDocumentModelPosition): TdxNativeDocumentPosition;
begin
...
begin
  ...
  ANativeRange := TdxNativeDocumentRange(ARange);
  ALength := ANativeRange.NormalizedEnd.LogPosition -
    ANativeRange.NormalizedStart.LogPosition;
  ADocumentLastParagraphSelected := False;
  if ANativeRange.NormalizedEnd.LogPosition > PieceTable.DocumentEndLogPosition
  then
  begin
    Dec(ALength, ANativeRange.NormalizedEnd.LogPosition -
      PieceTable.DocumentEndLogPosition);
    ADocumentLastParagraphSelected := True;
  end;
  if ALength <= 0 then
    Exit;
  PieceTable.DeleteContent(ANativeRange.NormalizedStart.LogPosition, ALength,
...
function TdxNativeSubDocument.CalculateParagraphsRange
  (const ARange: IdxRichEditDocumentRange): TdxParagraphRange;
var
  ANativeRange: TdxNativeDocumentRange;
...
begin
  ...
  AFirstParagraphIndex := TdxNativeDocumentPosition
    (ANativeRange.NormalizedStart).Position.ParagraphIndex;
  ALastPosition := TdxNativeDocumentPosition
    (ANativeRange.NormalizedEnd).Position;
  ALastParagraphIndex := ALastPosition.ParagraphIndex;
  if ALastPosition.LogPosition = PieceTable.Paragraphs[ALastParagraphIndex].LogPosition
  then
...
function TdxNativeSubDocument.BeginUpdateParagraphs(const ARange
  : IdxRichEditDocumentRange): IdxRichEditParagraphProperties;
var
...
begin
  ...
  AList := AModelParagraphs.GetRange(AParagraphsRange.Start,
    AParagraphsRange.Length);
  try
    AParagraph := AList.First;
    AFrom := TdxDocumentModelPosition.FromParagraphStart(PieceTable,
      AParagraph.Index).LogPosition;
    AParagraph := AList.Last;
    ALength := TdxDocumentModelPosition.FromParagraphEnd(PieceTable,
      AParagraph.Index).LogPosition - AFrom;
    Result := TdxNativeProperties.GetNativeParagraphProperties(Self,
      AFrom, ALength);
  finally
    AList.Free;
  end;
...
procedure TdxNativeSubDocument.EndUpdateParagraphs(const AProperties
  : IdxRichEditParagraphProperties);
begin
  CheckValid;
...
function TdxNativeSubDocument.BeginUpdateCharacters(AStart, ALength: Integer)
  : IdxRichEditCharacterProperties;
begin
  CheckValid;
...
function TdxNativeSubDocument.BeginUpdateCharacters(const AStart
  : IdxRichEditDocumentPosition; ALength: Integer)
  : IdxRichEditCharacterProperties;
begin
  CheckValid;
  Result := BeginUpdateCharactersCore(AStart.LogPosition,
    AStart.LogPosition + ALength);
end;

function TdxNativeSubDocument.BeginUpdateCharacters(const ARange
  : IdxRichEditDocumentRange): IdxRichEditCharacterProperties;
var
  ANativeRange: TdxNativeDocumentRange;
begin
  CheckValid;
  CheckDocumentRange(ARange);
  ANativeRange := TdxNativeDocumentRange(ARange);
  Result := BeginUpdateCharactersCore(ANativeRange.NormalizedStart.LogPosition,
    ANativeRange.NormalizedEnd.LogPosition);
end;

function TdxNativeSubDocument.BeginUpdateCharactersCore(AStart, AEnd: Integer)
  : IdxRichEditCharacterProperties;
var
  ALength: Integer;
begin
  ...
  ALength := AEnd - AStart;
  Result := TdxNativeProperties.GetNativeCharacterProperties(Self,
    AStart, ALength);
end;

procedure TdxNativeSubDocument.EndUpdateCharacters(const AProperties
  : IdxRichEditCharacterProperties);
begin
  CheckValid;
...
function TdxNativeSubDocument.BeginUpdateRangePermissions
  : IdxRichEditRangePermissionCollection;
var
...
function TdxNativeSubDocument.CreateRangePermission(const ACollection
  : IdxRichEditRangePermissionCollection;
  const ARangePermission: TdxRangePermission): IdxRichEditRangePermission;
var
  ALength: Integer;
...
procedure TdxNativeSubDocument.EndUpdateRangePermissions(const APermissions
  : IdxRichEditRangePermissionCollection);
var
  AModelPermissions: TdxRangePermissionCollection;
  I: Integer;
...
procedure TdxNativeSubDocument.ApplyNewRangePermissions(const APermissions
  : IdxRichEditRangePermissionCollection);
var
  ACount, I: Integer;
begin
...
procedure TdxNativeSubDocument.ApplyNewRangePermission(const ARangePermission
  : IdxRichEditRangePermission);
var
...
procedure TdxNativeSubDocument.CancelUpdateRangePermissions(const APermissions
  : IdxRichEditRangePermissionCollection);
begin
end;

function TdxNativeSubDocument.CreateBookmark(const AStart
  : IdxRichEditDocumentPosition; ALength: Integer; const AName: string)
  : IdxRichEditBookmark;
begin
  CheckValid;
...
function TdxNativeSubDocument.CreateBookmark(const ARange
  : IdxRichEditDocumentRange; const AName: string): IdxRichEditBookmark;
begin
  Result := CreateBookmark(ARange.Start, ARange.Length, AName);
end;

procedure TdxNativeSubDocument.SelectBookmark(const ABookmark
  : IdxRichEditBookmark);
begin
  CheckValid;
...
procedure TdxNativeSubDocument.SetSelectionCore(AStart,
  AEnd: TdxDocumentLogPosition; AForceUpdateTableSelection: Boolean = True);
var
  ASelection: TdxSelection;
begin
  DocumentModel.BeginUpdate;
  try
    ...
    AEnd := Min(AEnd, DocumentModel.Selection.PieceTable.
      DocumentEndLogPosition + 1);
...
procedure TdxNativeSubDocument.RemoveBookmark(const ABookmark
  : IdxRichEditBookmark);
begin
  CheckValid;
...
function TdxNativeSubDocument.CreateHyperlink(const AStart
  : IdxRichEditDocumentPosition; ALength: Integer): IdxRichEditHyperlink;
begin
  CheckValid;
  Result := FHyperlinks.CreateHyperlink(AStart, ALength);
end;

function TdxNativeSubDocument.CreateHyperlink(const ARange
  : IdxRichEditDocumentRange): IdxRichEditHyperlink;
begin
  Result := CreateHyperlink(ARange.Start, ARange.Length);
end;

procedure TdxNativeSubDocument.RemoveHyperlink(const AHyperlink
...
function TdxNativeSubDocument.GetParagraph(const APos
  : IdxRichEditDocumentPosition): IdxRichEditParagraph;
var
  AParagraphIndex: Integer;
begin
  CheckValid;
...
function TdxNativeSubDocument.GetParagraphs(const ARange
  : IdxRichEditDocumentRange): IdxRichEditParagraphCollection;
var
  AParagraphsRange: TdxParagraphRange;
...
function TdxNativeSubDocument.GetImages(const ARange: IdxRichEditDocumentRange)
...
function TdxNativeSubDocument.GetShapes(const ARange: IdxRichEditDocumentRange)
...
function TdxNativeSubDocument.GetSelectionRangeCollection
  (const ARange: IdxRichEditDocumentRange): TdxSelectionRangeCollection;
var
  ALength: Integer;
  ANativeRange: TdxNativeDocumentRange;
...
begin
  ...
  AEnd := Min(ANativeRange.NormalizedEnd.LogPosition,
    PieceTable.DocumentEndLogPosition + 1);
  ALength := AEnd - AStart;
  if ALength <= 0 then
    Exit(nil);
...
function TdxNativeSubDocument.CreateCopySelectionManagerForGetContent
...
function TdxNativeSubDocument.GetText(const ARange
  : IdxRichEditDocumentRange): string;
var
...
function TdxNativeSubDocument.GetText(const ARange: IdxRichEditDocumentRange;
  const AFragmentOptions: TdxRichEditTextFragmentOptions): string;
var
  AManager: TdxCopySelectionManager;
...
begin
  ...
  try
    ...
    try
      Result := AManager.GetPlainText(PieceTable, ASelection, nil,
        @AFragmentOptions);
    finally
      AManager.Free;
    end;
...
function TdxNativeSubDocument.GetRtfText(const ARange
  : IdxRichEditDocumentRange): string;
var
...
begin
  ...
  try
    ...
    try
      ...
      AManager := CreateCopySelectionManagerForGetContent;
      try
        Result := AManager.GetRtfText(PieceTable, ASelection, AOptions,
          True, True);
      finally
        AManager.Free;
      end;
...
function TdxNativeSubDocument.GetOpenXmlBytes(const ARange
  : IdxRichEditDocumentRange): TBytes;
var
  ASelection: TdxSelectionRangeCollection;
  AManager: TdxCopySelectionManager;
begin
...
function TdxNativeSubDocument.Replace(const ARange: IdxRichEditDocumentRange;
  const AText: string): Integer;
var
  AStart, AEnd, AEndLogPositionBefore, AEndLogPositionAfter
    : TdxDocumentLogPosition;
  ALength: Integer;
begin
  CheckValid;
...
function TdxNativeSubDocument.FindAll(const ATextToFind: string;
  AOptions: TdxRichEditSearchOptions = [];
  const ARange: IdxRichEditDocumentRange = nil)
  : TArray<IdxRichEditDocumentRange>;
var
...
begin
  ...
  try
    ASearchResult := StartSearch(ATextToFind, AOptions,
      TdxRichEditSearchDirection.Forward, ARange);
...
function TdxNativeSubDocument.ReplaceAll(const ATextToFind,
  AReplaceWith: string; AOptions: TdxRichEditSearchOptions = [];
...
function TdxNativeSubDocument.ReplaceAll(const ARegex: TRegEx;
...
function TdxNativeSubDocument.StartSearch(const ATextToFind: string;
  AOptions: TdxRichEditSearchOptions = [];
...
begin
  ...
  if ADirection = TdxRichEditSearchDirection.Forward then
    Result := TdxNativeSearchResultBase.CreateNativeSearchResultForward(Self,
      ATextToFind, AOptions, ARange)
  else
    Result := TdxNativeSearchResultBase.CreateNativeSearchResultBackward(Self,
      ATextToFind, AOptions, ARange);
end;
...
begin
  ...
  CheckValid;
  CheckDocumentRange(ARange);
  AMaxGuaranteedSearchResultLength :=
    DocumentModel.SearchOptions.RegExResultMaxGuaranteedLength;
  Result := TdxNativeRegexSearchResult.Create(Self, ARegex, ARange,
    AMaxGuaranteedSearchResultLength)
end;

procedure TdxNativeSubDocument.CheckDocumentPosition(const ADocumentPosition
  : IdxRichEditDocumentPosition);
var
...
procedure TdxNativeSubDocument.CheckDocumentRange(const ADocumentRange
  : IdxRichEditDocumentRange);
var
  ARange: TdxNativeDocumentRange;
begin
  ARange := TdxNativeDocumentRange(ADocumentRange);
  if TdxNativeDocumentPosition(ARange.Start).Position.PieceTable <> PieceTable
  then
    ThrowDocumentPositionPieceTableMismatch;
end;
...
function TdxNativeSubDocument.InsertDocumentContent
  (const APos: IdxRichEditDocumentPosition; AStream: TStream;
  AFormat: TdxRichEditDocumentFormat): IdxRichEditDocumentRange;
begin
  Result := InsertDocumentContent(APos, AStream, AFormat,
    TdxRichEditInsertOptions.MatchDestinationFormatting);
end;

function TdxNativeSubDocument.InsertDocumentContent
  (const APos: IdxRichEditDocumentPosition; AStream: TStream;
  AFormat: TdxRichEditDocumentFormat; AInsertOptions: TdxRichEditInsertOptions)
  : IdxRichEditDocumentRange;
begin
...
function TdxNativeSubDocument.InsertDocumentContent
  ...
  : IdxRichEditDocumentRange;
begin
  Result := InsertDocumentContent(APos, AStream, AFormat, ASourceUri,
    TdxRichEditInsertOptions.MatchDestinationFormatting);
end;
...
begin
  AInserter := TdxDocumentStreamContentInserter.Create(AStream, AFormat,
    ASourceUri, AInsertOptions);
  try
    Result := InsertContentCore(APos, AInserter);
  finally
    AInserter.Free;
...
function TdxNativeSubDocument.InsertDocumentContent
  (const APos: IdxRichEditDocumentPosition; const AFileName: string;
  AFormat: TdxRichEditDocumentFormat): IdxRichEditDocumentRange;
begin
  Result := InsertDocumentContent(APos, AFileName, AFormat, AFileName,
    TdxRichEditInsertOptions.MatchDestinationFormatting);
end;
...
function TdxNativeSubDocument.InsertDocumentContent
  ...
  : IdxRichEditDocumentRange;
begin
  Result := InsertDocumentContent(APos, AFileName, AFormat, ASourceUri,
    TdxRichEditInsertOptions.MatchDestinationFormatting);
end;
...
begin
  Result := InsertDocumentContent(APos, ARange,
    TdxRichEditInsertOptions.MatchDestinationFormatting);
end;
...
function TdxNativeSubDocument.AppendDocumentContent(AStream: TStream;
  ...
  : IdxRichEditDocumentRange;
begin
  Result := AppendDocumentContent(AStream, AFormat, ASourceUri,
    TdxRichEditInsertOptions.MatchDestinationFormatting);
end;
...
function TdxNativeSubDocument.AppendDocumentContent(const ARange
  : IdxRichEditDocumentRange): IdxRichEditDocumentRange;
begin
  Result := AppendDocumentContent(ARange,
...
function TdxNativeSubDocument.AppendDocumentContentCore
  (AInserter: TdxDocumentContentInserter): IdxRichEditDocumentRange;
begin
...
function TdxNativeSubDocument.GetWidthUnitFixedValue
  (AUnitValue: TdxWidthUnit): Single;
begin
...
procedure TdxNativeSubDocument.SetWidthUnitFixedValue(AUnitValue: TdxWidthUnit;
  AValue: Single);
begin
...
procedure TdxNativeSubDocument.SetWidthValue(AUnitValue: TdxWidthUnit;
  AValue: Single);
begin
  if AUnitValue.&Type = TdxWidthUnitType.ModelUnits then
    AUnitValue.Value := UnitsToModelUnits(AValue)
...
procedure TdxNativeSubDocument.AddParagraphsToList(const ARange
  : IdxRichEditDocumentRange; const AList: IdxRichEditNumberingList;
  ALevelIndex: Integer);
begin
  CheckValid;
  CheckDocumentRange(ARange);
...
procedure TdxNativeSubDocument.AddParagraphToList(const AParagraph
  : IdxRichEditParagraph; const AList: IdxRichEditNumberingList;
  ALevelIndex: Integer);
begin
...
procedure TdxNativeSubDocument.AddParagraphToList(const AParagraph
  : IdxRichEditParagraph; ANumberingListIndex, ALevelIndex: Integer);
begin
  Paragraphs.AddParagraphToList(AParagraph, ANumberingListIndex, ALevelIndex);
end;

procedure TdxNativeSubDocument.RemoveNumberingFromParagraph(const AParagraph
  : IdxRichEditParagraph);
begin
  Paragraphs.RemoveNumberingFromParagraph(AParagraph);
end;

procedure TdxNativeSubDocument.RemoveNumberingFromParagraphs
  (const ARange: IdxRichEditDocumentRange);
begin
  CheckValid;
  CheckDocumentRange(ARange);
...
function TdxNativeSelectionCollection.Add(const ARange
  : IdxRichEditDocumentRange): Integer;
var
...
procedure TdxNativeSelectionCollection.Add(const ARanges
  : TList<IdxRichEditDocumentRange>);
var
...
begin
  if (ARanges = nil) or (ARanges.Count = 0) then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionEmptyCollection));
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            else if J = ASelections.Count - 1 then
      ...
      AStartRunIndex := TdxNativeDocumentPosition
        (TdxNativeDocumentRange(AGroupedRanges[0][0]).NormalizedStart)
        .Position.RunIndex;
      ALastRange := AGroupedRanges[AGroupedRanges.Count - 1];
      AEndRunIndex := TdxNativeDocumentPosition
        (TdxNativeDocumentRange(ALastRange[ALastRange.Count - 1]).NormalizedEnd)
        .Position.RunIndex;
      FDocument.DocumentModel.ApplyChangesCore
        (FDocument.DocumentModel.ActivePieceTable, DocumentModelChangeActions,
...
begin
  ...
  try
    ...
    AEndRunIndex := InnerSelection.Items[InnerSelection.Items.Count - 1]
      .Interval.NormalizedEnd.RunIndex;
...
begin
  if (Index < 0) or (Index >= InnerSelection.Items.Count) then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionOutOfRange));
  if (InnerSelection.Items.Count = 1) and
    (InnerSelection.Items[Index].Length = 0) then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionCannotRemoveCaret));

  if InnerSelection.Items.Count = 1 then
  begin
    FDocument.DocumentModel.BeginUpdate;
    try
      AStartRunIndex := InnerSelection.Items[0]
        .Interval.NormalizedStart.RunIndex;
  ...
  FDocument.DocumentModel.BeginUpdate;
  try
    AStartRunIndex := InnerSelection.Items[Index]
      .Interval.NormalizedStart.RunIndex;
...
procedure TdxNativeSelectionCollection.Unselect(const ARange
  : IdxRichEditDocumentRange);
var
...
begin
  if InnerSelection.Items.Count = 0 then
    TdxRichEditExceptions.ThrowInternalException;
  if (InnerSelection.Items.Count = 1) and (InnerSelection.Items[0].Length = 0)
  then
    Exit;
  if ARange.Length = 0 then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionRangeCannotBeEmpty));
    ...
    try
      ...
      try
        ...
        try
          ...
          FDocument.DocumentModel.ApplyChangesCore
            (FDocument.DocumentModel.ActivePieceTable,
...
function TdxNativeSelectionCollection.CreateDocumentRange(const AStart,
  AEnd: TdxDocumentLogPosition): IdxRichEditDocumentRange;
var
...
function TdxNativeSelectionCollection.CreateDocumentRange
  (const AItem: TdxSelectionItem): IdxRichEditDocumentRange;
begin
  Result := CreateDocumentRange(AItem.Start, AItem.&End);
end;

function TdxNativeSelectionCollection.CreateSelectionItem(AStart,
  AEnd: TdxDocumentLogPosition): TdxSelectionItem;
begin
...
function TdxNativeSelectionCollection.CreateSelectionItem
  (const ARange: IdxRichEditDocumentRange): TdxSelectionItem;
begin
  Result := CreateSelectionItem(ARange.Start.LogPosition,
    ARange.&End.LogPosition);
end;
...
function TdxNativeSelectionCollection.GetItem(Index: Integer)
  : IdxRichEditDocumentRange;
begin
  Result := CreateDocumentRange(InnerSelection.Items[Index]);
end;

function TdxNativeSelectionCollection.AreSelectionItemsIntersected(const AItem1,
  AItem2: IdxRichEditDocumentRange): Boolean;
var
...
begin
  ...
  begin
    if (ANativeItem2.NormalizedStart.CompareTo(ANativeItem1.NormalizedStart) <
      0) and (ANativeItem1.NormalizedStart.CompareTo
      (ANativeItem2.NormalizedEnd) < 0) then
      Result := True;
    Exit;
  end;
  if (ANativeItem1.NormalizedStart.CompareTo(ANativeItem2.NormalizedStart) <= 0)
    and (ANativeItem2.NormalizedStart.CompareTo(ANativeItem1.NormalizedEnd) < 0)
  then
    Result := True;
  if (ANativeItem1.NormalizedStart.CompareTo(ANativeItem2.NormalizedEnd) < 0)
    and (ANativeItem2.NormalizedEnd.CompareTo(ANativeItem1.NormalizedEnd) <= 0)
  then
    Result := True;
  if (ANativeItem2.NormalizedStart.CompareTo(ANativeItem1.NormalizedStart) <= 0)
    and (ANativeItem1.NormalizedEnd.CompareTo(ANativeItem2.NormalizedEnd) <= 0)
  then
    Result := True;
end;

procedure TdxNativeSelectionCollection.CalcColumnsAndCellsCount
  (ARow: TdxTableRow; AStartIndexInRow, AEndIndexInRow: Integer;
  out AColumnCount, ACellCount: Integer);
var
  I: Integer;
begin
...
function TdxNativeSelectionCollection.CloneCollectionByValue
  (ACollection: TdxSelectionItemList): TdxSelectionItemList;
var
  I: Integer;
...
procedure TdxNativeSelectionCollection.CorrectEndLogPosByEndOfDocument
  (ARanges: TList<IdxRichEditDocumentRange>);
var
...
begin
  ...
  if FDocument.DocumentModel.MainPieceTable = InnerSelection.PieceTable then
    ADocEndLogPos := FDocument.Paragraphs[FDocument.Paragraphs.Count - 1]
      .Range.&End.LogPosition;

  for I := 0 to ARanges.Count - 1 do
  begin
    ARange := ARanges[I];
    if (ADocEndLogPos < ARange.Start.LogPosition) or
      (ADocEndLogPos < ARange.&End.LogPosition) then
    begin
...
function TdxNativeSelectionCollection.FillGroup(ASortedRanges
  : TList<IdxRichEditDocumentRange>; APrevRangeIndex: Integer;
  APrevStartCell: TdxTableCell; ASelectedCellCount: Integer;
  ASelectedColumnCount: Integer;
  ACurrentGroup: TList<IdxRichEditDocumentRange>): Integer;
var
  APrevCellIndexInRow, ASelectedColumnCountInCurrentRow,
    ASelectedCellCountInCurrentRow, I, ANormStartCellIndexInRow: Integer;
  ARange: TdxNativeDocumentRange;
  AStartPosCell, AEndPosCell, ANormalizedStartCell, ANormalizedEndCell,
    ANormalizedPrevStartCell, ANormalizedCurrStartCell: TdxTableCell;
...
begin
  ...
  begin
    ...
    AStartPosCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
      ARange.NormalizedStart.LogPosition);
    AEndPosCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
      ARange.NormalizedEnd.LogPosition);

    if AEndPosCell <> nil then
    begin
      if FDocument.DocumentModel.ActivePieceTable.Paragraphs
        [AEndPosCell.StartParagraphIndex].LogPosition = ARange.NormalizedEnd.LogPosition
      then
        AEndPosCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
          ARange.NormalizedEnd.LogPosition - 1);
    end
    else if not IsEndOfDocument(ARange.NormalizedEnd.LogPosition) then
    begin
      ADocModelEndPos := TdxPositionConverter.ToDocumentModelPosition
        (FDocument.DocumentModel.ActivePieceTable,
        ARange.NormalizedEnd.LogPosition);
      AEndPosParagraph := FDocument.DocumentModel.ActivePieceTable.Paragraphs
        [ADocModelEndPos.ParagraphIndex];
    ...
    NormalizeCells(AStartPosCell, AEndPosCell, ANormalizedStartCell,
      ANormalizedEndCell);
    ...
    NormalizeCells(APrevStartCell, ANormalizedStartCell,
      ANormalizedPrevStartCell, ANormalizedCurrStartCell);
    ...
    CalcColumnsAndCellsCount(ANormalizedStartCell.Row, ANormStartCellIndexInRow,
      ANormalizedEndCell.IndexInRow, ASelectedColumnCountInCurrentRow,
      ASelectedCellCountInCurrentRow);
    if (ASelectedCellCount <> ASelectedCellCountInCurrentRow) and
      (ASelectedColumnCount <> ASelectedColumnCountInCurrentRow) then
      Exit(I);
...
procedure TdxNativeSelectionCollection.InternalAdd(const ARange
  : IdxRichEditDocumentRange; ASelElementType: TSelectionRangeType);
var
  AItem: TdxSelectionItem;
begin
...
function TdxNativeSelectionCollection.IsCellPartiallySelected
  (ACell: TdxTableCell; APos: TdxDocumentLogPosition): Boolean;
begin
  Result := (ACell <> nil) and
    (ACell.PieceTable.Paragraphs[ACell.StartParagraphIndex].LogPosition
    <> APos);
end;

function TdxNativeSelectionCollection.IsEndOfDocument
  (APos: TdxDocumentLogPosition): Boolean;
begin
  Result := FDocument.Paragraphs[FDocument.Paragraphs.Count - 1]
    .Range.&End.LogPosition = APos;
end;

function TdxNativeSelectionCollection.IsLogPositionSelectAnotherCell
  (ACell: TdxTableCell; APos: TdxDocumentLogPosition): Boolean;
var
...
begin
  ...
  if FDocument.DocumentModel.ActivePieceTable.Paragraphs
    [ACell.StartParagraphIndex].LogPosition = APos then
    Exit(True);
  ATempCell := nil;
...
function TdxNativeSelectionCollection.IsParent(AParent,
  AChild: TdxTableCell): Boolean;
begin
  Result := False;
...
function TdxNativeSelectionCollection.GetCell(APieceTable: TdxPieceTable;
  APos: TdxDocumentLogPosition): TdxTableCell;
var
...
function TdxNativeSelectionCollection.GetContainerTable(ACell1,
  ACell2: TdxTableCell): TdxTable;
begin
  Result := nil;
...
function TdxNativeSelectionCollection.GetFirstNormalizedCellInRow
  (ARow: TdxTableRow): TdxTableCell;
begin
  if ARow = nil then
    Exit(nil);
...
function TdxNativeSelectionCollection.GetGroupedSortedRanges
  (ARanges: TList<IdxRichEditDocumentRange>)
  : TdxObjectList<TList<IdxRichEditDocumentRange>>;
var
  ASortedRanges, ASortedMergedRanges, ASortedCorrectedRanges,
    ACurrentGroup: TList<IdxRichEditDocumentRange>;
    ...
    try
      ...
      try
        ...
        begin
          ...
          begin
            if FDocument.DocumentModel.ActivePieceTable.Paragraphs
              [AEndPosCell.StartParagraphIndex]
              .LogPosition = ARange.NormalizedEnd.LogPosition then
              AEndPosCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
                ARange.NormalizedEnd.LogPosition - 1);
          end
          else if not IsEndOfDocument(ARange.NormalizedEnd.LogPosition) then
          begin
            ADocModelEndPos := TdxPositionConverter.ToDocumentModelPosition
              (FDocument.DocumentModel.ActivePieceTable,
              ARange.NormalizedEnd.LogPosition);
            AEndPosParagraph := FDocument.DocumentModel.ActivePieceTable.
              Paragraphs[ADocModelEndPos.ParagraphIndex];
            if AEndPosParagraph.LogPosition = ARange.NormalizedEnd.LogPosition
            then
              AEndPosCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
                ARange.NormalizedEnd.LogPosition - 1);
          end;
          ...
          CalcColumnsAndCellsCount(ANormalizedStartCell.Row,
            ANormalizedStartCell.IndexInRow, ANormalizedEndCell.IndexInRow,
            ASelectedColumnsCount, ASelectedCellsCount);
          I := FillGroup(ASortedCorrectedRanges, I, ANormalizedStartCell,
            ASelectedCellsCount, ASelectedColumnsCount, ACurrentGroup);
        end;
      finally
...
function TdxNativeSelectionCollection.GetLastSelectedCell(AStartCell,
  AEndCell: TdxTableCell; AEndLogPosition: TdxDocumentLogPosition)
  : TdxTableCell;
var
  ALastSelectedCell, ATempCell: TdxTableCell;
begin
  if AEndCell = nil then
    Exit(GetCell(FDocument.DocumentModel.ActivePieceTable,
      AEndLogPosition - 1));
  ...
  ALastSelectedCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
    AEndLogPosition - 1);
...
function TdxNativeSelectionCollection.GetLastNormalizedCellInRow
  (ARow: TdxTableRow): TdxTableCell;
begin
  if ARow = nil then
    Exit(nil);
...
function TdxNativeSelectionCollection.GetParentCellWithNestedLevel
  (ACell: TdxTableCell; ANestedLevel: Integer): TdxTableCell;
begin
  Result := nil;
...
function TdxNativeSelectionCollection.GetPreviousCellInRow(ACell: TdxTableCell)
  : TdxTableCell;
begin
  if ACell = nil then
    Exit(nil);
...
function TdxNativeSelectionCollection.MergeNeighbourhoodRanges
  (ASortedRanges: TList<IdxRichEditDocumentRange>)
  : TList<IdxRichEditDocumentRange>;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if TdxNativeDocumentRange(APrevRange).NormalizedEnd.CompareTo
        (TdxNativeDocumentRange(ARange).NormalizedStart) = 0 then
      begin
        APrevRange := CreateDocumentRange(TdxNativeDocumentRange(APrevRange)
          .NormalizedStart.LogPosition, TdxNativeDocumentRange(ARange)
          .NormalizedEnd.LogPosition);
        Continue;
      end
      else
...
procedure TdxNativeSelectionCollection.NormalizeCells(ACell1,
  ACell2: TdxTableCell; out ANormalizedCell1, ANormalizedCell2: TdxTableCell);
var
...
begin
  ...
  begin
    ...
    begin
      ANormalizedCell1 := ACell1;
      ANormalizedCell2 := GetParentCellWithNestedLevel(ACell2,
        ACell1.Table.NestedLevel + 1);
    end
    else if IsParent(ACell2, ACell1) then
    begin
      ANormalizedCell2 := ACell2;
      ANormalizedCell1 := GetParentCellWithNestedLevel(ACell1,
        ACell2.Table.NestedLevel + 1);
    end
    else
    begin
      ANormalizedCell1 := GetParentCellWithNestedLevel(ACell1,
        ABaseParentTable.NestedLevel);
      ANormalizedCell2 := GetParentCellWithNestedLevel(ACell2,
        ABaseParentTable.NestedLevel);
      if ANormalizedCell1 = ANormalizedCell2 then
      begin
        ANormalizedCell1 := GetParentCellWithNestedLevel(ACell1,
          ABaseParentTable.NestedLevel + 1);
        ANormalizedCell2 := GetParentCellWithNestedLevel(ACell2,
          ABaseParentTable.NestedLevel + 1);
      end;
    end;
...
procedure TdxNativeSelectionCollection.RemoveRangesWithZeroWidth
  (ARanges: TList<IdxRichEditDocumentRange>);
var
  I: Integer;
begin
...
function TdxNativeSelectionCollection.SameRowCell(ACell1,
  ACell2: TdxTableCell): Boolean;
begin
  Result := (ACell1 <> nil) and (ACell2 <> nil) and (ACell1.Row = ACell2.Row);
end;

function TdxNativeSelectionCollection.ShouldSkipValidation(AStart, AEnd,
  ALastSelected: TdxTableCell): Boolean;
begin
  Result := True;
  if (AStart = nil) and (AEnd = nil) and (ALastSelected = nil) then
    Exit;
  if (AStart = ALastSelected) and (AEnd <> nil) and
    (AEnd.Table.ParentCell = AStart) then
    Exit;
  Result := False;
end;

function TdxNativeSelectionCollection.SortRanges
  (AItems: TList<IdxRichEditDocumentRange>): TList<IdxRichEditDocumentRange>;
var
...
function TdxNativeSelectionCollection.SplitRanges(ASortedRanges
  : TList<IdxRichEditDocumentRange>): TList<IdxRichEditDocumentRange>;
var
  AResult, APartiallySelectedCells, ASplittedRangesByRow
    : TList<IdxRichEditDocumentRange>;
  I: Integer;
  ARange: TdxNativeDocumentRange;
  AFirstRangePart, ASecondRangePart: IdxRichEditDocumentRange;
  AStartCell, AEndCell, ANormalStartCell, ANormalEndCell, ALastSelectedCell
    : TdxTableCell;
      ...
      try
        repeat
          ...
          begin
            ...
            AStartCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
              ARange.NormalizedStart.LogPosition);
            AEndCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
              ARange.NormalizedEnd.LogPosition);

            ANormalStartCell := nil;
            ANormalEndCell := nil;
            NormalizeCells(AStartCell, AEndCell, ANormalStartCell,
              ANormalEndCell);

            if (ANormalStartCell = nil) or (ANormalEndCell = nil) or
              (ANormalStartCell = ANormalEndCell) then
            begin
              AResult.Add(ARange);
              Continue;
            end;
            ...
            ALastSelectedCell := GetLastSelectedCell(ANormalStartCell,
              ANormalEndCell, ARange.NormalizedEnd.LogPosition);
            ...
            AEndCellStartLogPos := FDocument.DocumentModel.ActivePieceTable.
              Paragraphs[ANormalEndCell.StartParagraphIndex].LogPosition;
            AFirstRangePart := CreateDocumentRange
              (ARange.NormalizedStart.LogPosition, AEndCellStartLogPos);
            ASecondRangePart := CreateDocumentRange(AEndCellStartLogPos,
              AEndLogPos);
            AResult.Add(AFirstRangePart);
            APartiallySelectedCells.Add(ASecondRangePart);
          end;
          ASplittedRangesByRow.Free;
          ASplittedRangesByRow := SplitRangesByRowWithChildTables
            (APartiallySelectedCells);
...
function TdxNativeSelectionCollection.SplitRangesByRowWithChildTables
  (ASortedRanges: TList<IdxRichEditDocumentRange>)
  : TList<IdxRichEditDocumentRange>;
var
...
function TdxNativeSelectionCollection.SplitRangesByRowSameTable
  (ASortedRanges: TList<IdxRichEditDocumentRange>)
  : TList<IdxRichEditDocumentRange>;
var
  ...
  ARange: TdxNativeDocumentRange;
  AStartCell, AEndCell, ANormalStartCell, ANormalEndCell, ALastSelectedCell,
    ALastRangeFirstCell: TdxTableCell;
...
begin
  ...
  begin
    ...
    ALastSelectedCell := GetLastSelectedCell(ANormalStartCell, ANormalEndCell,
      ARange.NormalizedEnd.LogPosition);
    ...
    begin
      ...
      begin
        ...
        begin
          SplitRows(ARange, ANormalStartCell.Table, AStartCellRowIndex + 1,
            AEndCellRowIndex, Result);
          Continue;
        end;
      end
      else if ANormalStartCell.Table.ParentCell = ALastSelectedCell.Table.ParentCell
      then
      begin
        AStartCellRowIndex := ANormalStartCell.RowIndex;
        AFirstTableRows := ANormalStartCell.Table.Rows;
        SplitRows(ARange, ANormalStartCell.Table, AStartCellRowIndex + 1,
          AFirstTableRows.Count - 1, Result);

        repeat
          ALastRange := TdxNativeDocumentRange(Result[Result.Count - 1]);
          Result.Delete(Result.Count - 1);
          ALastRangeFirstCell :=
            GetCell(FDocument.DocumentModel.ActivePieceTable,
            TdxNativeDocumentRange(ALastRange).NormalizedStart.LogPosition);
          if (ALastRangeFirstCell = nil) or
            (ALastRangeFirstCell.Table.NestedLevel <
            ANormalStartCell.Table.NestedLevel) then
          begin
            AEndLogPos := FDocument.DocumentModel.ActivePieceTable.Paragraphs
              [ALastSelectedCell.Table.FirstRow.FirstCell.StartParagraphIndex]
              .LogPosition;
            ANewRange := CreateDocumentRange(TdxNativeDocumentRange(ALastRange)
              .NormalizedStart.LogPosition, AEndLogPos);
            Result.Add(ANewRange);
            ALastRange := CreateDocumentRange(AEndLogPos,
              TdxNativeDocumentRange(ALastRange).NormalizedEnd.LogPosition);
            ALastRangeFirstCell := ALastSelectedCell;
          end
          else
            ALastRangeFirstCell := GetParentCellWithNestedLevel
              (ALastRangeFirstCell, ANormalStartCell.Table.NestedLevel);
          if ALastRangeFirstCell.Table = ALastSelectedCell.Table then
            AEndCellRowIndex := ALastSelectedCell.RowIndex
          else
            AEndCellRowIndex := ALastRangeFirstCell.Table.Rows.Count - 1;
          SplitRows(TdxNativeDocumentRange(ALastRange),
            ALastRangeFirstCell.Table, 1, AEndCellRowIndex, Result);
        until not(ALastRangeFirstCell.Table <> ALastSelectedCell.Table);
        Continue;
      end
      else if ANormalStartCell = ALastSelectedCell.Table.ParentCell then
      begin
        SplitRows(ARange, ALastSelectedCell.Table, 0,
          ALastSelectedCell.RowIndex, Result);
        Continue;
      end
      else if ANormalStartCell.Table.ParentCell = ALastSelectedCell then
      begin
        AStartCellRowIndex := ANormalStartCell.RowIndex;
        ATableRows := ANormalStartCell.Table.Rows;
        SplitRows(ARange, ANormalStartCell.Table, AStartCellRowIndex + 1,
          ATableRows.Count - 1, Result);
        Continue;
      end;
    end
    else if (ANormalStartCell <> nil) and (ALastSelectedCell = nil) then
    begin
      AStartCellRowIndex := ANormalStartCell.RowIndex;
      ATableRows := ANormalStartCell.Table.Rows;
      SplitRows(ARange, ANormalStartCell.Table, AStartCellRowIndex + 1,
...
procedure TdxNativeSelectionCollection.SplitRows(ARange: TdxNativeDocumentRange;
  ATable: TdxTable; AStartRowIndex, AEndRowIndex: Integer;
  ARanges: TList<IdxRichEditDocumentRange>);
var
...
begin
  ...
  begin
    ARow := ARows[I];
    AEndLogPos := FDocument.DocumentModel.ActivePieceTable.Paragraphs
      [ARow.FirstCell.StartParagraphIndex].LogPosition;
  ...
  begin
    AAfterTableLogPos := FDocument.DocumentModel.ActivePieceTable.Paragraphs
      [ARows.Last.LastCell.EndParagraphIndex].EndLogPosition + 1;
    ...
    ALastRange := CreateDocumentRange(AStartLogPos,
      ARange.NormalizedEnd.LogPosition);
    ARanges.Add(ALastRange);
  end;
end;

procedure TdxNativeSelectionCollection.UnselectCore(ACloneCollection
  : TdxSelectionItemList; ARange: TdxNativeDocumentRange);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    else if (ASel.NormalizedStart < ARange.NormalizedStart.LogPosition) and
      (ASel.NormalizedEnd > ARange.NormalizedEnd.LogPosition) then
    begin
      ...
      begin
        ...
        ANewSelItem := CreateSelectionItem(ARange.NormalizedEnd.LogPosition,
          AOldSelEnd);
        ACloneCollection.Insert(I + 1, ANewSelItem);
      end
      else if ASel.Start > ASel.&End then
      begin
        AOldSelEnd := ASel.&End;
        ASel.&End := ARange.NormalizedEnd.LogPosition;
        ANewSelItem := CreateSelectionItem(ARange.NormalizedStart.LogPosition,
          AOldSelEnd);
        ACloneCollection.Insert(I + 1, ANewSelItem);
      end;
    end
    else if (ASel.NormalizedStart < ARange.NormalizedEnd.LogPosition) and
    ...
    else if (ASel.NormalizedEnd > ARange.NormalizedStart.LogPosition) and
      (ASel.NormalizedEnd <= ARange.NormalizedEnd.LogPosition) then
    begin
      if ASel.Start < ASel.&End then
        ASel.&End := ARange.NormalizedStart.LogPosition
...
procedure TdxNativeSelectionCollection.ValidateSelection
...
procedure TdxNativeSelectionCollection.ValidateSelections
  (AGroupedItems: TdxObjectList<TList<IdxRichEditDocumentRange>>;
  AIsRemoved: Boolean = False);
var
  I, J, K, L: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if AreSelectionItemsIntersected(TdxNativeDocumentRange(AItem),
          TdxNativeDocumentRange(AItem2)) then
          TdxRichEditExceptions.ThrowInvalidOperationException
            (cxGetResourceString
...
procedure TdxNativeSelectionCollection.ValidateSelectionCells
  (const ARange: IdxRichEditDocumentRange;
  ASelElementType: TSelectionRangeType);
var
  ANativeRange: TdxNativeDocumentRange;
...
begin
  ...
  ALastSelectedCell := GetLastSelectedCell(ANormalizedStartCell,
    ANormalizedEndCell, AEndLogPos);

  if ShouldSkipValidation(ANormalizedStartCell, ANormalizedEndCell,
    ALastSelectedCell) then
    Exit;
  ...
  begin
    if ANormalizedStartCell.Table = ALastSelectedCell.Table then
      ValidateCellsWithinSameTable(ANormalizedStartCell, ANormalizedEndCell,
        AStartLogPos, AEndLogPos, ASelElementType)
    else if ANormalizedStartCell.Table.ParentCell = ALastSelectedCell.Table.ParentCell
    then
      ValidateCellsWithinDifferentTables(ANormalizedStartCell,
        ANormalizedEndCell, AStartLogPos, AEndLogPos)
    else if ANormalizedStartCell = ALastSelectedCell.Table.ParentCell then
      ValidateTableCellTextBeforeTable(ANormalizedEndCell, AEndLogPos)
    else if ANormalizedStartCell.Table.ParentCell = ALastSelectedCell then
      ValidateTableCellTextAfterTable(ANormalizedStartCell, AStartLogPos);
  end
  else if (ANormalizedStartCell <> nil) and (ALastSelectedCell = nil) then
    ValidateTableCellTextAfterTable(ANormalizedStartCell, AStartLogPos)
  else if (ANormalizedStartCell = nil) and (ALastSelectedCell <> nil) then
    ValidateTableCellTextBeforeTable(ANormalizedEndCell, AEndLogPos);
end;

procedure TdxNativeSelectionCollection.ValidateCellsWithinSameTable
  (AFirstSelectedCell, ALastSelectedCell: TdxTableCell;
...
begin
  AIsStartCellPartiallySelected := IsCellPartiallySelected(AFirstSelectedCell,
    ASelStartLogPosition);
  ALastTableCell := nil;
  if ALastSelectedCell = nil then
    ALastTableCell := GetCell(FDocument.DocumentModel.ActivePieceTable,
      ASelEndLogPosition - 1);
  if ALastSelectedCell = nil then
    AIsEndCellPartiallySelected := False
  else
    AIsEndCellPartiallySelected := IsCellPartiallySelected(ALastSelectedCell,
      ASelEndLogPosition);
  ...
  if ALastSelectedCell = nil then
    ...
    AEndCellRowIndex := ALastSelectedCell.RowIndex;
  if (ALastSelectedCell <> nil) and ALastSelectedCell.IsFirstCellInRow and not AIsEndCellPartiallySelected
  then
  ...
  if (AFirstSelectedCell.VerticalMerging = TdxMergingState.Continue) and
    (ASelElementType in [TSelectionRangeType.Start, TSelectionRangeType.Single])
  then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString
  ...
  if AIsStartCellPartiallySelected or AIsEndCellPartiallySelected then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionPartiallySelectedCells));
  if ALastSelectedCell <> nil then
  begin
    if (ALastSelectedCell.Previous.VerticalMerging = TdxMergingState.Continue)
      and (ASelElementType in [TSelectionRangeType.&End,
      TSelectionRangeType.Single]) then
      TdxRichEditExceptions.ThrowInvalidOperationException
        (cxGetResourceString
        (@sdxRichEditExceptionLastCellContinuesVerticalMerge));
  end
  else if ALastSelectedCell = nil then
  begin
    if (ALastTableCell.VerticalMerging = TdxMergingState.Continue) and
      (ASelElementType in [TSelectionRangeType.&End, TSelectionRangeType.Single])
    then
      TdxRichEditExceptions.ThrowInvalidOperationException
        (cxGetResourceString
        (@sdxRichEditExceptionLastCellContinuesVerticalMerge));
...
procedure TdxNativeSelectionCollection.ValidateCellsWithinDifferentTables
  (AFirstSelectedCell, ALastSelectedCell: TdxTableCell;
  ASelStartLogPosition, ASelEndLogPosition: TdxDocumentLogPosition);
var
...
begin
  ...
  ValidateTableCellTextBeforeTable(ALastSelectedCell, ASelEndLogPosition,
    ACellForNormalize);
end;

procedure TdxNativeSelectionCollection.ValidateTableCellTextBeforeTable
  (ASelectedCell: TdxTableCell; ASelEndLogPosition: TdxDocumentLogPosition;
  ACellForNormalize: TdxTableCell = nil);
var
...
begin
  ...
  begin
    ...
    begin
      ANormCell := nil;
      NormalizeCells(ACellForNormalize, ALastTableCell, ANormCell,
        ALastTableCell);
    end;
  end;
  if ASelectedCell = nil then
    ...
    AIsSelectedCellPartiallySelected := IsCellPartiallySelected(ASelectedCell,
      ASelEndLogPosition);
...
procedure TdxNativeSelectionCollection.ValidateTableCellTextAfterTable
  (ASelectedCell: TdxTableCell; ASelStartLogPosition: TdxDocumentLogPosition);
var
...
begin
  ...
  AIsCellPartiallySel := IsCellPartiallySelected(ASelectedCell,
    ASelStartLogPosition);
  ...
  begin
    if ASelectedCell.VerticalMerging = TdxMergingState.Continue then
      TdxRichEditExceptions.ThrowInvalidOperationException
        ...
        (@sdxRichEditExceptionFirstCellContinuesVerticalMerge));
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionSelectionExtendsOutsideTable));
  end;
...
function TdxNativeDocument.GetDefaultParagraphProperties
...
function TdxNativeDocument.GetDefaultCharacterProperties
...
function TdxNativeDocument.GetDefaultTableProperties
...
begin
  ...
  Result := TdxNativeDocumentPosition.Create(ActiveSubDocument,
    ARangeInfo.&End);
end;

procedure TdxNativeDocument.SetCaretPosition(const Value
  : IdxRichEditDocumentPosition);
var
  ALogPosition: TdxDocumentLogPosition;
begin
  ...
  ALogPosition := Min(Value.LogPosition, TdxNativeSubDocument(ActiveSubDocument)
    .PieceTable.DocumentEndLogPosition);
...
begin
  ...
  FCharacterStyles := TdxNativeStyleCollections.
    GetCharacterStyleCollection(Self);
  FParagraphStyles := TdxNativeStyleCollections.
    GetParagraphStyleCollection(Self);
  FTableStyles := TdxNativeStyleCollections.GetTableStyles(Self);

  FDefaultParagraphProperties :=
    TdxNativeProperties.GetDefaultParagraphProperties(Self,
    DocumentModel.DefaultParagraphProperties);
  FDefaultCharacterProperties :=
    TdxNativeProperties.GetDefaultCharacterProperties
    (DocumentModel.DefaultCharacterProperties);
  FDefaultTableProperties := TdxNativeProperties.GetDefaultTableProperties(Self,
    DocumentModel.DefaultTableProperties);
  FAbstractNumberingLists := TdxNativeAbstractNumberingListCollection.
    Create(Self);
...
function TdxNativeDocument.CreateNativeSection(ASection: TdxSection)
  : IdxRichEditSection;
begin
...
procedure TdxNativeDocument.OnDocumentCleared(ASender: TObject;
  E: TdxEventArgs);
begin
  Invalidate;
end;
...
procedure TdxNativeDocument.LoadDocument(AStream: TStream;
...
procedure TdxNativeDocument.LoadDocumentCore(AStream: TStream;
  AFormat: TdxRichEditDocumentFormat; const ASourceUri: string);
var
  AImportManagerService: IdxDocumentImportManagerService;
  AImportHelper: TdxDocumentImportHelper;
begin
  AImportManagerService :=
    DocumentModel.GetService<IdxDocumentImportManagerService>;
  if AImportManagerService = nil then
    TdxRichEditExceptions.ThrowInvalidOperationException
      ('Could not find service: IdxDocumentImportManagerService');
...
procedure TdxNativeDocument.SaveDocument(AStream: TStream;
  AFormat: TdxRichEditDocumentFormat);
begin
  SaveDocumentCore(AStream, AFormat, '');
end;

procedure TdxNativeDocument.SaveDocumentCore(AStream: TStream;
  AFormat: TdxRichEditDocumentFormat; const ATargetUri: string);
begin
  TdxInnerRichEditDocumentServer.SaveDocumentCore(DocumentModel, AStream,
    AFormat, ATargetUri);
end;

procedure TdxNativeDocument.SaveDocument(const AFileName: string;
  AFormat: TdxRichEditDocumentFormat);
begin
  SaveDocument(DocumentModel, AFileName, AFormat);
end;

class procedure TdxNativeDocument.SaveDocument(ADocumentModel: TdxDocumentModel;
  const AFileName: string; AFormat: TdxRichEditDocumentFormat);
var
  AStream: TFileStream;
begin
  AStream := TFileStream.Create(AFileName, fmCreate or fmOpenWrite or
    fmShareDenyWrite);
  try
    TdxInnerRichEditDocumentServer.SaveDocumentCore(ADocumentModel, AStream,
      AFormat, AFileName);
  finally
    AStream.Free;
  end;
...
function TdxNativeDocument.InsertSection(const APos
  : IdxRichEditDocumentPosition): IdxRichEditSection;
begin
...
function TdxNativeDocument.InsertSectionCore(const APos
  : IdxRichEditDocumentPosition; AAppend: Boolean): IdxRichEditSection;
var
  ANativePosition: TdxNativeDocumentPosition;
  ALogPosition: TdxDocumentLogPosition;
  ASectionIndex: Integer;
...
procedure TdxNativeDocument.ChangeActiveDocument(const ADocument
...
procedure TdxNativeDocument.ChangeActivePieceTable(const AControl
  : IdxRichEditControl; APieceTable: TdxPieceTable);
var
...
begin
  if (APieceTable.IsMain) or (APieceTable.IsTextBox) or (APieceTable.IsComment)
  then
  begin
    ACommand := TdxChangeActivePieceTableCommand.Create(AControl,
      APieceTable, nil, -1);
    try
  ...
  begin
    ...
    begin
      AHeaderCommand := TdxMakeNearestHeaderActiveCommand.Create(AControl,
        TdxSectionHeader(APieceTable.ContentType));
      try
    ...
    else if APieceTable.IsFooter then
    begin
      AFooterCommand := TdxMakeNearestFooterActiveCommand.Create(AControl,
        TdxSectionFooter(APieceTable.ContentType));
      try
...
begin
  ...
  if ShouldCreateNewSubDocument then
    Result := CreateNativeSubDocument
      (TdxPieceTable(DocumentModel.ActivePieceTable.ContentType.PieceTable),
      DocumentServer)
  else
    Result := Self;
end;
...
begin
  Result := (DocumentModel.ActivePieceTable.ContentType is
    TdxSectionHeaderFooterBase) or
...
begin
  ...
  try
    if ASelection = nil then
      Exit;
    AManager := TdxCopySelectionManager.Create
      (DocumentServer as IdxInnerControl);
    try
...
function TdxNativeDocument.GetSection(const APos: IdxRichEditDocumentPosition)
  : IdxRichEditSection;
var
...
begin
  ...
  AHeaderFooter := Safe<TdxSectionHeaderFooterBase>.Cast
    (ASubDocument.PieceTable.ContentType);
...
function TdxNativeDocument.GetSubDocument(const APos
  : IdxRichEditDocumentPosition): TdxNativeSubDocument;
var
  ANativePosition: TdxNativeDocumentPosition;
begin
  ANativePosition := TdxNativeDocumentPosition(APos);
...
procedure TdxNativeDocument.MailMerge(const AFileName: string;
  AFormat: TdxRichEditDocumentFormat);
begin
  DocumentServer.MailMerge(AFileName, AFormat);
end;

procedure TdxNativeDocument.MailMerge(AStream: TStream;
  AFormat: TdxRichEditDocumentFormat);
begin
  DocumentServer.MailMerge(AStream, AFormat);
end;

procedure TdxNativeDocument.MailMerge(const ATargetDocument
  : IdxRichEditDocument);
begin
  DocumentServer.MailMerge(ATargetDocument);
end;

procedure TdxNativeDocument.MailMerge(const AOptions
  : IdxRichEditMailMergeOptions; const AFileName: string;
  AFormat: TdxRichEditDocumentFormat);
begin
...
procedure TdxNativeDocument.MailMerge(const AOptions
  : IdxRichEditMailMergeOptions; AStream: TStream;
  AFormat: TdxRichEditDocumentFormat);
begin
  DocumentServer.MailMerge(AOptions, AStream, AFormat);
end;

function TdxNativeDocument.GetParagraphStyles
...
function TdxNativeDocument.GetAbstractNumberingLists
  : IdxRichEditAbstractNumberingListCollection;
begin
  CheckValid;
...
function TdxNativeDocument.GetNumberingLists
...
function TdxNativeDocument.GetCharacterStyles
...
procedure TdxNativeDocument.MailMerge(const AOptions
  : IdxRichEditMailMergeOptions; const ATargetDocument: IdxRichEditDocument);
begin
