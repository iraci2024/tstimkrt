...
type
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      ABookmark: TdxBookmark); reintroduce;
...
constructor TdxCreateBookmarkCommand.Create(const AControl: IdxRichEditControl;
  const ABookmarkName: string);
begin
...
begin
  ...
  ALength := AEnd - AStart;
  ACommand := TdxCreateBookmarkCoreCommand.Create(RichEditControl, AStart,
    ALength, FBookmarkName);
  try
    ACommand.Execute;
  finally
    ACommand.Free;
...
procedure TdxCreateBookmarkCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  ApplyCommandRestrictionOnEditableControl(AState,
    Options.DocumentCapabilities.Bookmarks);
  ApplyDocumentProtectionToSelectedCharacters(AState);
end;

{ TdxSelectBookmarkCommand }

constructor TdxSelectBookmarkCommand.Create(const AControl: IdxRichEditControl;
...
function TdxSelectBookmarkCommand.
...
procedure TdxSelectBookmarkCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
...
function TdxSelectBookmarkCommand.CanChangePosition
  (const APos: TdxDocumentModelPosition): Boolean;
begin
  Result := True;
end;

function TdxSelectBookmarkCommand.ChangePosition
...
function TdxSelectBookmarkCommand.CreateEnsureCaretVisibleVerticallyCommand
  : TdxRichEditCommand;
var
...
constructor TdxDeleteBookmarkCommand.Create(const AControl: IdxRichEditControl;
  ABookmark: TdxBookmark);
begin
  inherited Create(AControl);
  Assert(ABookmark <> nil, 'Bookmark');
...
procedure TdxDeleteBookmarkCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  ApplyCommandRestrictionOnEditableControl(AState,
...
constructor TdxCreateBookmarkCoreCommand.Create(const AControl
  : IdxRichEditControl; APosition: TdxDocumentLogPosition; ALength: Integer;
  const ABookmarkName: string);
begin
  inherited Create(AControl);
  Assert(ABookmarkName <> '', 'BookmarkName');
...
procedure TdxCreateBookmarkCoreCommand.UpdateUIStateCore
...
procedure TdxShowBookmarkFormCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
  ApplyCommandRestrictionOnEditableControl(AState,
    Options.DocumentCapabilities.Bookmarks);
