...
begin
  ...
  if Message.HitTest <> HTCLOSE then
    Activate
  else if SubMenuControl <> nil then
    SubMenuControl.Hide;
end;
...
procedure TdxBarSubMenuEditor.Notification(AComponent: TComponent;
...
begin
  with TdxBarManagerAccess(ItemLinks.BarManager) do
    if Dragging and not ItemLinks.CanContainItem(DraggingItem, AErrorText) then
      Exit;
  if SubMenuControl = nil then
  begin
