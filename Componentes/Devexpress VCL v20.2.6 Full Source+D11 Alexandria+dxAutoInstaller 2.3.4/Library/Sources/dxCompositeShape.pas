...
uses
  ...
  dxCore, cxClasses, cxGeometry, dxCoreGraphics, cxGraphics, dxGDIPlusClasses,
  dxXMLDoc,
  dxShapePrimitives;
...
type
  ...
  private
    ...
    procedure DoWorldTransform(AGraphics: TdxGPGraphics;
      const ADestRect: TdxRectF);
  ...
  protected
    ...
    procedure LoadFromShape(AShape: TdxCompositeShape;
      const AShapeName: string);
    procedure Draw(ACanvas: TCanvas; const ARect: TRect); override;
    procedure DrawF(AGPGraphics: TdxGPGraphics; const ARect: TdxRectF);
    procedure SetShapesColor(AAlphaColor: TdxAlphaColor;
      const AShapeName: string);
...
uses
  Types, Math, dxGDIPlusAPI, dxCustomTree, dxShapeReaders, dxShapeBrushes,
  dxCoreClasses;
...
procedure TdxCompositeShape.LoadFromClipboardFormat(AFormat: Word;
...
begin
  ...
  try
    ...
    if (AXMLDocument <> nil) and (AXMLDocument.Root <> nil) and
      AXMLDocument.Root.HasChildren then
...
procedure TdxCompositeShape.SaveToClipboardFormat(var AFormat: Word;
...
begin
  ...
  begin
    ...
    if SameText(dxXMLStringToString(ACanvasNode.Name), TdxShapeCanvas.GetName)
    then
    begin
...
procedure TdxCompositeShape.LoadFromShape(AShape: TdxCompositeShape;
  const AShapeName: string);
var
  I: Integer;
...
procedure TdxCompositeShape.DrawF(AGPGraphics: TdxGPGraphics;
...
procedure TdxCompositeShape.SetShapesColor(AAlphaColor: TdxAlphaColor;
  const AShapeName: string);
var
...
procedure TdxCompositeShape.DoWorldTransform(AGraphics: TdxGPGraphics;
  const ADestRect: TdxRectF);
var
  AScale: TdxPointF;
