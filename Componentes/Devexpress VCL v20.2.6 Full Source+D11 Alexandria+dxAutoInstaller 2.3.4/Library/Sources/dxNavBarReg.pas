...
type
  ...
  TdxNavBarOfficeNavigationBarItemImageIndexProperty = class
    (TImageIndexProperty)
  public
    function GetImages: TCustomImageList; override;
  end;
  ...
  protected
    ...
    procedure InternalExecuteVerb(AIndex: Integer); override;
  public
    property OfficeNavigationBar: TdxNavBarCustomOfficeNavigationBar
      read GetOfficeNavigationBar;
  end;
...
begin
  ...
  if Value <> sdxNavBarLookAndFeelView then
    (GetComponent(0) as TdxNavBar).View :=
      dxNavBarViewsFactory.GetIDFromName(Value)
  else
...
function TdxNavBarImageIndexProperty.GetCollectionItem
...
procedure TdxNavBarCursorProperty.ListMeasureHeight(const Value: string;
  ACanvas: TCanvas; var AHeight: Integer);
begin
  AHeight := Max(cxTextHeight(ACanvas.Handle),
    GetSystemMetrics(SM_CYCURSOR) + 4);
end;
...
procedure TdxNavBarCursorProperty.ListDrawValue(const Value: string;
...
begin
  ...
  begin
    ...
    if CursorHandle <> 0 then
      DrawIconEx(ACanvas.Handle, ARect.Left + 2, ARect.Top + 2, CursorHandle, 0,
        0, 0, 0, DI_NORMAL or DI_DEFAULTSIZE);
...
type
  TdxNavBarOfficeNavigationBarItemsAccess = class
    (TdxNavBarOfficeNavigationBarItems);

function TdxNavBarOfficeNavigationBarItemImageIndexProperty.GetImages
  : TCustomImageList;
var
  ACollection: TdxNavBarOfficeNavigationBarItems;
begin
  ACollection := (GetComponent(0) as TdxNavBarOfficeNavigationBarItem)
    .Collection as TdxNavBarOfficeNavigationBarItems;
  Result := TdxNavBarOfficeNavigationBarItemsAccess(ACollection)
...
begin
  case AIndex of
    0:
      Result := sdxItemsEditor;
    1:
      Result := sdxAppearanceEditor;
  else
...
begin
  case AIndex of
    0:
      NavBar.Customization.Visible := True;
    1:
      dxShowNavBarDesigner(NavBar);
  end;
end;
  ...
  case AIndex of
    0:
      ShowCollectionEditor(Designer, Component,
        (Component as TdxNavBarCustomOfficeNavigationBar).Items, 'Items');
  end;
end;
...
function TdxNavBarOfficeNavigationBarComponentEditor.
  InternalGetVerbCount: Integer;
begin
  Result := 1;
end;

function TdxNavBarOfficeNavigationBarComponentEditor.GetOfficeNavigationBar
...
begin
  ...
  RegisterComponents(dxLibraryProductPage,
    [TdxNavBar, TdxNavBarOfficeNavigationBar]);
  ...
  RegisterComponentEditor(TdxNavBarOfficeNavigationBar,
    TdxNavBarOfficeNavigationBarComponentEditor);
  RegisterSelectionEditor(TdxNavBarOfficeNavigationBar,
    TdxNavBarOfficeNavigationBarSelectionEditor);
  ...
  RegisterPropertyEditor(TypeInfo(TcxImageIndex),
    TdxNavBarOfficeNavigationBarItem, 'ImageIndex',
    TdxNavBarOfficeNavigationBarItemImageIndexProperty);

  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TdxNavBar, 'CustomDrawEvents',
    TdxNavBarCustomDrawEventEditor);

  RegisterPropertyEditor(TypeInfo(TCursor), TdxNavBarCursors, 'Cursor',
    TdxNavBarCursorProperty);
  RegisterPropertyEditor(TypeInfo(TCursor), TdxNavBarCursors, 'DragCursor',
    TdxNavBarCursorProperty);
  RegisterPropertyEditor(TypeInfo(TCursor), TdxNavBarCursors, 'DragCopyCursor',
    TdxNavBarCursorProperty);
  RegisterPropertyEditor(TypeInfo(TCursor), TdxNavBarCursors,
    'HotTrackedGroupCursor', TdxNavBarCursorProperty);
  RegisterPropertyEditor(TypeInfo(TCursor), TdxNavBarCursors,
    'HotTrackedLinkCursor', TdxNavBarCursorProperty);

  RegisterPropertyEditor(TypeInfo(TdxNavBarPainter), TdxNavBar, 'ViewStyle',
    TdxNavBarViewStyleProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxNavBarGroup,
    'LargeImageIndex', TdxLargeImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxNavBarGroup,
    'SmallImageIndex', TdxSmallImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxNavBarItem,
    'LargeImageIndex', TdxLargeImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxNavBarItem,
    'SmallImageIndex', TdxSmallImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TdxSkinName),
    TdxNavBarSkinBasedNavPanePainter, 'ColorSchemeName',
    TdxNavBarColorSchemeNameProperty);
  RegisterPropertyEditor(TypeInfo(TdxSkinName),
    TdxNavBarSkinBasedExplorerBarPainter, 'ColorSchemeName',
    TdxNavBarColorSchemeNameProperty);

  HideClassProperties(TdxNavBar, [
    // Cursors
    'Cursor', 'DragCopyCursor', 'DragCursor', 'HotTrackedGroupCursor',
    'HotTrackedLinkCursor',
    // OptionsBehavior
    'AllowSelectLinks', 'DragDropFlags', 'ShowGroupsHint', 'ShowLinksHint',
    'ShowNavigationPaneOverflowPanelHints', 'EachGroupHasSelectedLink',
    // OptionsStyle
    'DefaultStyles', 'StyleBackground', 'StyleHint', 'StyleDropTargetGroup',
    'StyleDropTargetLink', 'StyleButton', 'StyleButtonHotTracked',
    'StyleButtonPressed', 'StyleNavigationPaneHeader',
    // OptionsImage
    'LargeImages', 'SmallImages',
    // OptionsView
    'View', 'ViewReal', 'ShowSpecialGroup', 'SpaceBetweenGroups',
    'ShowGroupCaptions', 'NavigationPaneMaxVisibleGroups',
    'NavigationPaneOverflowPanelUseSmallImages', 'LookAndFeelSchemes',
    // NavigationPane
    'OnNavigationPaneCollapsed', 'OnNavigationPaneExpanded',
    'OnNavigationPanePopupShowed', 'OnNavigationPanePopupShowing',
    // CustomDraw
    'OnCustomDraw', 'OnCustomDrawBackground', 'OnCustomDrawGroupCaption',
    'OnCustomDrawGroupClientBackground', 'OnCustomDrawGroupClientForeground',
    'OnCustomDrawGroupHint', 'OnCustomDrawLink', 'OnCustomDrawLinkHint',
    'OnCustomDrawBottomScrollButton', 'OnCustomDrawTopScrollButton',
    'OnCustomDrawNavigationPaneHeader', 'OnCustomDrawNavigationPaneSplitter',
    'OnCustomDrawNavigationPaneOverflowPanel',
    'OnCustomDrawNavigationPaneOverflowPanelHint']);

  HideClassProperties(TdxNavBarGroup, [
    // CustomStyles
    'StyleBackground', 'StyleControl', 'StyleHeader', 'StyleHeaderActive',
    'StyleHeaderActiveHotTracked', 'StyleHeaderActivePressed',
    'StyleHeaderHotTracked', 'StyleHeaderPressed',
    // OptionsExpand
    'Expandable', 'Expanded', 'ShowExpandButton',
    // OptionsGroupControl
    'ShowControl', 'UseControl']);
  HideClassProperties(TdxNavBarItem, ['Style', 'StyleDisabled',
    'StyleHotTracked', 'StylePressed']);
  HideClassProperties(TdxNavBarSeparator, ['CustomStyles', 'Style',
    'StyleDisabled', 'StyleHotTracked', 'StylePressed', 'Action', 'Caption',
    'Enabled', 'Hint', 'LargeImageIndex', 'SmallImageIndex', 'OnClick']);
...
begin
  TdxCustomNavBarAccess(ASender).FDesignHelper :=
    TdxNavBarDesigner.Create(TComponent(ASender));
end;
