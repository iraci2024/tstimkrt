...
{$ENDIF}
  ...
  dxCore, dxCoreClasses, cxVariants, cxClasses, cxGraphics, cxControls,
  cxContainer,
  cxDataStorage, cxDataUtils, cxEdit, cxDropDownEdit, cxTextEdit,
  cxFilterControlUtils, dxCoreGraphics;
...
type
  ...
  public
    ...
    property Items[Index: Integer]: TcxImageComboBoxItem read GetItems
  ...
  protected
    ...
    property ActiveProperties: TcxCustomImageComboBoxProperties
  ...
  protected
    ...
    function GetItemDescription(AItem: TcxImageComboBoxItem;
      AImageIndex: Integer): string;
    ...
    property ShowImageIndexInsteadDescription: Boolean
      read FShowImageIndexInsteadDescription
      write SetShowImageIndexInsteadDescription;
  public
    ...
    property DefaultImageIndex: TcxImageIndex read FDefaultImageIndex
      write SetDefaultImageIndex default -1;
    property ImageAlign: TcxImageAlign read FImageAlign write SetImageAlign
      default iaLeft;
    ...
    property MultiLineText: Boolean read FMultiLineText write SetMultiLineText
      default False;
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TcxImageComboBoxProperties
      read GetActiveProperties;
  published
    property Anchors;
    property AutoSize;
...
function TcxImageComboBoxListBox.GetHeight(ARowCount: Integer;
...
begin
  ...
  begin
    ...
    begin
      Dec(FClientWidth, TcxControlAccess(Container)
...
procedure TcxImageComboBoxListBox.DrawItem(Index: Integer; Rect: TRect;
  ...
  begin
    ...
    try
      ...
      cxBitBlt(Canvas.Handle, ATempBitmap.Canvas.Handle, Rect,
        cxNullPoint, SRCCOPY);
    finally
      ATempBitmap.Free;
    end;
    ...
    begin
      ...
      begin
        cxDrawImage(Canvas, GetImageRect(AImages, Rect), nil, AImages,
          ...
          TcxImageComboBoxViewInfo(Edit.ViewInfo).GetImageColorPalette,
          ScaleFactor);
      end;
...
begin
  if Properties.MultiLineText and not IsHighlightSearchText then
    Result := DrawTextBiDiModeFlags(DT_EXPANDTABS or DT_NOPREFIX or
      DT_WORDBREAK)
  else
    Result := inherited GetDrawTextFlags;
end;

procedure TcxImageComboBoxListBox.MeasureItem(Index: Integer;
  var Height: Integer);
var
  AData: string;
  AImageSize: TSize;
...
begin
  ...
  begin
    R := Rect(0, 0, W, H);
    AData := GetItem(Index);
    DrawText(Canvas.Handle, PChar(AData), Length(AData), R, GetDrawTextFlags or
      DT_CALCRECT);
    H := Max(H, R.Bottom - R.Top + ScaleFactor.Apply(2));
  end
  else
    H := Max(TdxTextMeasurer.TextLineHeight(Canvas.Handle) +
      ScaleFactor.Apply(2), H);
...
function TcxImageComboBoxListBox.GetImageRect(AImages: TCustomImageList;
  var R: TRect): TRect;
var
  AImageSize: TSize;
begin
...
begin
  ...
  for I := 0 to Properties.Items.Count - 1 do
    Result := Max(Result,
      Canvas.TextWidth(Properties.GetItemDescription(Properties.Items[I],
      Properties.Items[I].ImageIndex)));
...
function TcxImageComboBoxListBox.GetProperties
  : TcxCustomImageComboBoxProperties;
begin
...
begin
  ...
  else if (Index > -1) and (Index < Items.Count) then
...
function TcxImageComboBoxLookupData.GetActiveProperties
  : TcxCustomImageComboBoxProperties;
begin
...
begin
  ...
  begin
    ...
    AViewInfo.ImageRect := cxRectInflate(AViewInfo.ClientRect,
      -cxTextOffset, 0);
    ...
    begin
      ...
      begin
        AViewInfo.ImageRect.Left := AViewInfo.ImageRect.Left +
          (cxRectWidth(AViewInfo.ImageRect) - AImageSize.cx) div 2;
        AViewInfo.ImageRect.Right := AViewInfo.ImageRect.Left + AImageSize.cx;
      end;
    end
    else
    ...
    begin
      AViewInfo.ImageRect.Top := AViewInfo.ImageRect.Top +
        (cxRectHeight(AViewInfo.ImageRect) - AImageSize.cy) div 2;
...
procedure TcxImageComboBoxViewData.EditValueToDrawValue(const AEditValue
...
procedure TcxImageComboBoxViewData.DisplayValueToDrawValue(const ADisplayValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
var
...
function TcxImageComboBoxViewData.GetProperties
  : TcxCustomImageComboBoxProperties;
begin
  ...
  begin
    ...
    try
      ...
      cxDrawImage(ACanvas, ImageRect, nil, Images, ImageIndex, ifmNormal,
        EnabledImageDrawModeMap[Enabled], False, GetImageColorPalette,
        ScaleFactor);
    finally
...
procedure TcxCustomImageComboBoxProperties.SetShowImageIndexInsteadDescription
  (AValue: Boolean);
begin
...
function TcxCustomImageComboBoxProperties.FindLookupText
  (const AText: string): Boolean;
begin
  Result := FindItemByText(AText) <> nil;
end;

procedure TcxCustomImageComboBoxProperties.DoAssign
...
begin
  ...
  if Sender = FImages then
    FImages := nil
  else if Sender = FLargeImages then
    FLargeImages := nil;
  if AChanged then
    Changed;
end;

function TcxCustomImageComboBoxProperties.GetDisplayFormatOptions
  : TcxEditDisplayFormatOptions;
begin
  Result := [];
end;

function TcxCustomImageComboBoxProperties.GetItemDescription
  (AItem: TcxImageComboBoxItem; AImageIndex: Integer): string;
begin
...
class function TcxCustomImageComboBoxProperties.GetLookupDataClass
  : TcxInterfacedPersistentClass;
begin
  Result := TcxImageComboBoxLookupData;
end;

class function TcxCustomImageComboBoxProperties.GetViewDataClass
...
procedure TcxCustomImageComboBoxProperties.InternalGetImageComboBoxDisplayValue
  (AItem: TcxImageComboBoxItem; out AText: string;
  out AImageIndex: TcxImageIndex; AAlwaysShowDescription: Boolean = False);
begin
  if AItem = nil then
...
begin
  ...
  Result := InternalCompareString(AText1, AText2, True) and
    ((Images = nil) or (AImageIndex1 = AImageIndex2));
end;

function TcxCustomImageComboBoxProperties.FindItemByValue(const AValue: Variant)
  : TcxImageComboBoxItem;
var
  I: Integer;
begin
...
class function TcxCustomImageComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxImageComboBox;
end;

function TcxCustomImageComboBoxProperties.GetDisplayText(const AEditValue
...
function TcxCustomImageComboBoxProperties.GetEditValueSource
...
function TcxCustomImageComboBoxProperties.GetSupportedOperations
...
class function TcxCustomImageComboBoxProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxImageComboBoxViewInfo;
end;

function TcxCustomImageComboBoxProperties.IsEditValueValid(var AEditValue
  : TcxEditValue; AEditFocused: Boolean): Boolean;
begin
  Result := True;
end;

procedure TcxCustomImageComboBoxProperties.PrepareDisplayValue(const AEditValue
...
function TcxCustomImageComboBox.GetActiveProperties
  : TcxCustomImageComboBoxProperties;
begin
...
procedure TcxCustomImageComboBox.SetProperties(const Value
  : TcxCustomImageComboBoxProperties);
begin
...
begin
  ...
  begin
    if (LookupData.ActiveControl <> nil) and
      (LookupData.ActiveControl is TcxImageComboBoxListBox) then
...
function TcxCustomImageComboBox.LookupKeyToEditValue(const AKey: TcxEditValue)
  : TcxEditValue;
begin
...
class function TcxCustomImageComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxCustomImageComboBoxProperties;
end;

procedure TcxCustomImageComboBox.PrepareEditValue(const ADisplayValue
...
class function TcxImageComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
class function TcxFilterImageComboBoxHelper.GetFilterEditClass
  : TcxCustomEditClass;
begin
  Result := TcxImageComboBox;
end;
...
GetRegisteredEditProperties.Register(TcxImageComboBoxProperties,
  scxSEditRepositoryImageComboBoxItem);
FilterEditsController.Register(TcxImageComboBoxProperties,
  TcxFilterImageComboBoxHelper);

finalization

FilterEditsController.Unregister(TcxImageComboBoxProperties,
  TcxFilterImageComboBoxHelper);
