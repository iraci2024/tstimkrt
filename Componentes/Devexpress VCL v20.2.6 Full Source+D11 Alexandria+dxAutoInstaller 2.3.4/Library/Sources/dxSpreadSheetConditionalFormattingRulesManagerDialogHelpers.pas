...
uses
  Types, TypInfo, Classes, Generics.Defaults, Generics.Collections, Graphics,
  cxGraphics,
...
type
  ...
  TdxSpreadSheetConditionalFormattingRulesManagerDialogHelperEnumRulesProc =
    reference to procedure(ARuleInfo
    : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo);
  ...
  public
    ...
    procedure PopulateRulesInArea(const AArea: TRect;
      AProc: TdxSpreadSheetConditionalFormattingRulesManagerDialogHelperEnumRulesProc);

    procedure Add(ARule: TdxSpreadSheetCustomConditionalFormattingRule);
    procedure Delete(AInfo
      : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo);
    procedure MoveTo(ASource, ATarget
      : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo;
      AInsertAfter: Boolean);
    procedure Replace(ARuleInfo
      : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo;
      ARule: TdxSpreadSheetCustomConditionalFormattingRule);
    procedure SetAreas(ARuleInfo
      : TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo;
  ...
  TdxSpreadSheetConditionalFormattingRulesManagerDialogHelperClass = class of
    TdxSpreadSheetConditionalFormattingRulesManagerDialogHelper;
...
begin
  ...
  if StopIfTrueSupported then
    StopIfTrue := TdxSpreadSheetCustomConditionalFormattingRuleAccess(Rule)
      .StopIfTrue;
end;

procedure TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo.
  ApplyChanges(AIndex: Integer);
begin
  Rule.Index := AIndex;
  if StopIfTrueSupported then
    TdxSpreadSheetCustomConditionalFormattingRuleAccess(Rule).StopIfTrue :=
      StopIfTrue;
end;

procedure TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo.
  DrawPreview(ACanvas: TcxCanvas; const R: TRect);
begin
  TdxSpreadSheetCustomConditionalFormattingRuleAccess(Rule)
    .DrawPreview(ACanvas, R);
end;

function TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo.
  GetDetails: string;
begin
  Result := TdxSpreadSheetCustomConditionalFormattingRuleAccess(Rule)
    .GetDetails;
end;
...
begin
  ...
  FRulesAdded :=
    TObjectList<TdxSpreadSheetCustomConditionalFormattingRule>.Create;
  FRulesDeleted := TList<TdxSpreadSheetCustomConditionalFormattingRule>.Create;

  FRules := TObjectList<
    TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo>.Create;
  FRules.Capacity := ConditionalFormatting.RuleCount;
  for I := 0 to ConditionalFormatting.RuleCount - 1 do
    FRules.Add(TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo.
      Create(ConditionalFormatting[I]));
...
procedure TdxSpreadSheetConditionalFormattingRulesManagerDialogHelper.
  ApplyChanges;
var
  I: Integer;
begin
  ...
  begin
    ...
    try
      ...
      for I := 0 to FRules.Count - 1 do
        TdxSpreadSheetConditionalFormattingRulesManagerDialogRuleInfo(FRules[I])
          .ApplyChanges(I);
    finally
...
procedure TdxSpreadSheetConditionalFormattingRulesManagerDialogHelper.EndEditing
  (ACanceled: Boolean);
begin
...
procedure TdxSpreadSheetConditionalFormattingRulesManagerDialogHelper.
  InternalDeleteRule(ARule: TdxSpreadSheetCustomConditionalFormattingRule);
begin
...
procedure TdxSpreadSheetConditionalFormattingRulesManagerDialogHelper.
  SetHasChanges(const Value: Boolean);
begin
