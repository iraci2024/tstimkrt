...
{$ENDIF}
  ...
  StdCtrls, Buttons, ExtCtrls, ImgList, dxCore, cxControls, cxGraphics,
  cxGeometry,
...
type
  ...
  TdxOcImageAlign = (iaNone, iaLT, iaLC, iaLB, iaRT, iaRC, iaRB, iaTL, iaTC,
    iaTR, iaBL, iaBC, iaBR);
  TdxOcIvFlags = (ivWidth, ivHeight, ivBoth);
  TdxOcShape = (shRectangle, shRoundRect, shEllipse, shDiamond);
  TdxOcNodeAttachMode = (naAdd, naAddFirst, naAddChild, naAddChildFirst,
    naInsert);
  ...
  private
    ...
    procedure SetAnimXY(LeftX, TopY: Integer; const Clip: TRect;
      First: Boolean);
  ...
  public
    ...
    procedure GetNodeInfo(var AInfo: TdxOcNodeInfo;
      ATargetScaleFactor: TdxScaleFactor = nil);
  ...
  public
    ...
    function GetNodeColor(ACurrectColor: TColor; ASelected: Boolean)
      : TColor; virtual;
    ...
    procedure DrawFrameDiamond(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect); virtual;
    procedure DrawFrameEllipse(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect); virtual;
    procedure DrawFrameRectangle(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect; ARect3D: Boolean); virtual;
    procedure DrawFrameRoundRect(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect; ARadius: Integer); virtual;
    procedure DrawLine(ACanvas: TcxCanvas; X1, Y1, X2, Y2: Integer;
      AWidth: Single); virtual;
    //
  ...
  public
    ...
    procedure DrawFrameEllipse(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect); override;
    procedure DrawFrameRectangle(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect; ARect3D: Boolean); override;
    procedure DrawFrameRoundRect(ACanvas: TcxCanvas; ANode: TdxOcNode;
      const R: TRect; ARadius: Integer); override;
    procedure DrawLine(ACanvas: TcxCanvas; X1, Y1, X2, Y2: Integer;
      AWidth: Single); override;
  end;
  ...
  TdxOcAllowEvent = procedure(Sender: TObject; Node: TdxOcNode;
    var Allow: Boolean) of object;
  TdxOcFontEvent = procedure(Sender: TObject; Node: TdxOcNode; Font: TFont)
    of object;
  TdxOcDrawEvent = procedure(Sender: TObject; Node: TdxOcNode; ACanvas: TCanvas;
    Rect: TRect) of object;
  TdxOcGetTextEvent = procedure(Sender: TObject; Node: TdxOcNode;
    var Text: string) of object;
  TdxOcSetTextEvent = procedure(Sender: TObject; Node: TdxOcNode;
    const Text: string) of object;
  TdxOcNodeFunc = function(Value: TdxOcNode): TdxOcNode;
  TdxOcEditMode = set of (emLeft, emCenter, emRight, emVCenter, emWrap, emUpper,
    emLower, emGrow);
  ...
  TdxOcNavigate = (ocnLineLeft, ocnLineUp, ocnLineRight, ocnLineDown,
    ocnPageLeft, ocnPageUp, ocnPageRight, ocnPageDown, ocnLeft, ocnTop,
    ocnRight, ocnBottom, ocnLeftPosition, ocnTopPosition, ocnSelectNextLeft,
    ocnSelectNextUp, ocnSelectNextRight, ocnSelectNextDown);

  TdxCustomOrgChart = class(TcxScrollingControl, IdxSkinSupport,
    IdxRotateClient)
  private
    ...
    procedure DoDrawText(Handle: HDC; Text: string; var Rect: TRect;
      Flags: Integer);
  ...
  protected
    ...
    function PaintNode(ACanvas: TcxCanvas; AFont: TFont; ANode: TdxOcNode;
      const AUpdateRect: TRect): TPoint;
    ...
    function InternalAdd(ParentNode: TdxOcNode; Data: TdxOcNodeData;
      Idx: Integer): TdxOcNode; virtual;
    procedure InternalMoveTo(ParentNode, Node: TdxOcNode;
      Idx: Integer); virtual;
    ...
    property DefaultImageAlign: TdxOcImageAlign read FDefaultImageAlign
      write FDefaultImageAlign default iaNone;
    property DefaultNodeHeight: Word read FDefaultNodeHeight write SetNodeHeight
      default 40;
    property DefaultNodeWidth: Word read FDefaultNodeWidth write SetNodeWidth
      default 64;
    property EditMode: TdxOcEditMode read FEditMode write SetEditMode
      default [emWrap];
    ...
    property SelectedNodeColor: TColor read FSelectedNodeColor
      write SetSelectedNodeColor default clDefault;
    property SelectedNodeTextColor: TColor read FSelectedNodeTextColor
      write SetSelectedNodeTextColor default clDefault;
    ...
    property OnCollapsing: TdxOcAllowEvent read FOnCollapsing
      write FOnCollapsing;
  ...
  protected
    ...
    function CalculateFormPosition(AOrgChart: TdxCustomOrgChart)
      : TRect; virtual;
...
uses
  Types, Math, cxScrollBar, dxDPIAwareUtils, dxScrollAnimation;
...
function TdxOrgChartCustomizeFormManager.Customize
  (AOrgChart: TdxCustomOrgChart): Boolean;
begin
...
procedure TdxOrgChartCustomizeFormManager.Register
  (AClass: TdxOrgChartCustomCustomizeFormClass);
begin
  FList.Add(Pointer(AClass));
end;

procedure TdxOrgChartCustomizeFormManager.Unregister
  (AClass: TdxOrgChartCustomCustomizeFormClass);
begin
  FList.Remove(Pointer(AClass));
end;

function TdxOrgChartCustomizeFormManager.GetActualClass
  : TdxOrgChartCustomCustomizeFormClass;
begin
...
begin
  ...
  FSizeChanged := True;
  FOptions := [ocSelect, ocFocus, ocButtons, ocDblClick, ocEdit, ocCanDrag,
    ocShowDrag];
  FEditMode := [emWrap];
  FZoom := 1;
...
begin
  ...
  Params.WindowClass.Style := Params.WindowClass.Style or CS_HREDRAW or
    CS_VREDRAW;
  Params.Style := (Params.Style or WS_TABSTOP) and
...
procedure TdxCustomOrgChart.Notification(AComponent: TComponent;
...
begin
  AHandled := True;
  if (AAngle > 0) and not Rotated or (AAngle < 0) and Rotated then
  begin
...
function TdxCustomOrgChart.InternalAdd(ParentNode: TdxOcNode;
  Data: TdxOcNodeData; Idx: Integer): TdxOcNode;
begin
...
procedure TdxCustomOrgChart.InternalMoveTo(ParentNode, Node: TdxOcNode;
  Idx: Integer);
begin
...
function TdxCustomOrgChart.Insert(Node: TdxOcNode; Data: TdxOcNodeData)
  : TdxOcNode;
begin
  if Node = nil then
    Result := nil
...
function TdxCustomOrgChart.AddFirst(Node: TdxOcNode; Data: TdxOcNodeData)
  : TdxOcNode;
begin
  if Node = nil then
...
function TdxCustomOrgChart.AddChild(Node: TdxOcNode; Data: TdxOcNodeData)
  : TdxOcNode;
begin
  Result := InternalAdd(Node, Data, -1);
end;

function TdxCustomOrgChart.AddChildFirst(Node: TdxOcNode; Data: TdxOcNodeData)
  : TdxOcNode;
begin
...
begin
  Result := (ocButtons in Options) and (Node.Count > 0) and
    not(IsZoomed and Node.Expanded);
end;
...
procedure TdxCustomOrgChart.PaintContent(ACanvas: TcxCanvas;
...
function TdxCustomOrgChart.PaintNode(ACanvas: TcxCanvas; AFont: TFont;
  ANode: TdxOcNode; const AUpdateRect: TRect): TPoint;

  function InRect(const R: TRect): Boolean;
  begin
    Result := not((R.Left > AUpdateRect.Right) or (R.Right < AUpdateRect.Left)
      or (R.Top > AUpdateRect.Bottom) or (R.Bottom < AUpdateRect.Top));
  end;

  function CalculateAnimationRect(ANode: TdxOcNode; ACoef: Integer): TRect;
  begin
    Result.Left := ANode.FAnimX + (ANode.FAnimX0 - ANode.FAnimX) *
      ACoef div OtMaxAnimFrames;
    Result.Top := ANode.FAnimY + (ANode.FAnimY0 - ANode.FAnimY) *
      ACoef div OtMaxAnimFrames;
  ...
  begin
    ...
    LookAndFeelPainter.DrawScaledExpandButton(ACanvas, ARect, Value,
      ScaleFactor);
  end;

  procedure PrepareCanvas(ACanvas: TcxCanvas; ANode: TdxOcNode;
    ASelected: Boolean);
  var
    ABrushColor: TColor;
  begin
...
begin
  ...
  if not ANode.FDraw then
    Exit;

  if (APlaceRect.Bottom < AUpdateRect.Bottom) and
    (ANode.Expanded or (FCollapsed = ANode)) then
  begin
    ALineLeft := 0;
    ALineRight := -1;
...
procedure TdxCustomOrgChart.DoDrawText(Handle: HDC; Text: string;
...
procedure TdxCustomOrgChart.DrawNode(Node: TdxOcNode; ACanvas: TCanvas;
  Rect: TRect);
begin
...
procedure TdxCustomOrgChart.DefaultDrawNode(Node: TdxOcNode; ACanvas: TCanvas;
  R: TRect);
var
  AImageRect: TRect;
...
begin
  ...
  begin
    ...
    try
      ...
      AImageRect := cxRectOffset
        (cxRect(dxGetImageSize(Images, AScaleFactor)), P);
      cxDrawImage(ACanvas.Handle, AImageRect, AImageRect, nil, Images,
        Node.ImageIndex, idmNormal, ocImageFiltering in Options);
    finally
      AScaleFactor.Free;
    end;
  end;
...
begin
  Result := not FNoAnim and (ocAnimate in Options) and HandleAllocated and
    Node.IsVisible;
  if Result then
  begin
    FCollapsed := Node;
...
begin
  ...
  if Value < P.X then
    Value := P.X;
  if Value = DefaultNodeWidth then
    Exit;
...
begin
  ...
  if Value < P.Y then
    Value := P.Y;
  if Value = DefaultNodeHeight then
    Exit;
...
begin
  ...
  begin
    ...
    if FZoom = 0 then
      SetZoomRatio
    else if IsPopupScrollBars then
    ...
    begin
      ...
      begin
        ...
        begin
          FHitTests := [htOnRect];
          if Node.Is3D then
            Dec(RN.Bottom, LineWidth);
          if InFrame(Node.Shape) then
            Include(FHitTests, htOnShape);
        end;
        if HasButton(Node) then
        begin
          AExpandButtonAreaSize := LookAndFeelPainter.ScaledExpandButtonAreaSize
            (ScaleFactor);
          I := Node.ExtWidth div 2 - AExpandButtonAreaSize div 2;
          if (X >= RN.Left + I) and (X < RN.Left + I + AExpandButtonAreaSize)
            and (Y >= RN.Bottom - AExpandButtonAreaSize div 2) then
            Include(FHitTests, htOnButton);
        end;
      end;
      if (Y >= R.Bottom) and not(htOnButton in FHitTests) then
        FNodeAt := nil;
    end;
    if FNodeAt = nil then
    begin
...
begin
  if (FHitTests = []) or (X <> FHitX) or (Y <> FHitY) then
    HitTestsAt(X, Y);
  Result := FNodeAt;
end;
...
begin
  ...
  if Assigned(OnChanging) then
    OnChanging(Self, Node, Allow);
end;

procedure TdxCustomOrgChart.DoNavigate(ANavigateCode: TdxOcNavigate;
  AValue: Integer);
...
var
  ...
  ALeft, ATop: Integer;
begin
  if TdxScrollAnimationHelperAccess(HorzScrollAnimationHelper).IsAnimationActive
  then
    ALeft := TdxScrollAnimationHelperAccess(HorzScrollAnimationHelper)
      .TargetPosition
  else
    ALeft := LeftEdge;

  if TdxScrollAnimationHelperAccess(VertScrollAnimationHelper).IsAnimationActive
  then
    ATop := TdxScrollAnimationHelperAccess(VertScrollAnimationHelper)
      .TargetPosition
  else
    ATop := TopEdge;
...
procedure TdxCustomOrgChart.MouseDown(Button: TMouseButton; Shift: TShiftState;
  ...
  procedure DoSelect;
  begin
    if CanSelect(FNodeAt) then
      Selected := FNodeAt;
  end;

begin
  if not IsScrollBarsArea(Point(X, Y)) and (Button = mbLeft) and
    not(ssDouble in Shift) then
  begin
    SetFocus;
    FDrag := True;
    GetNodeAt(X, Y);
    if FHitTests * [htOnShape, htOnButton] = [htOnShape] then
      DoSelect;
...
function TdxCustomOrgChart.DragScroll(X, Y: Integer;
  AState: TDragState): Boolean;
var
...
procedure TdxCustomOrgChart.DragOver(Source: TObject; X, Y: Integer;
...
begin
  ...
  begin
    N := GetNodeAt(X, Y);
    Accept := (N <> nil) and not N.HasAsParent(TdxCustomOrgChart(Source)
      .Selected);
  end;
  if not Accept or AScrolled or not(ocShowDrag in Options) or
    (State <> dsDragMove) then
    FDragParent := nil
  else
  begin
    FDragX0 := X;
    ...
    if not(htUnder in FHitTests) or (N = Selected) then
      N := N.FParent;
...
begin
  ...
  else if (Source = Self) and (Selected <> nil) then
  begin
    Node := GetNodeAt(X, Y);
    if Node = nil then
      Exit;
...
procedure TdxCustomOrgChart.KeyDown(var Key: Word; Shift: TShiftState);
  ...
  begin
    Result := nil;
    if not(ocInsDel in Options) or (Selected = nil) and (Count > 0) then
      Exit;
...
begin
  ...
  if Shift = [ssCtrl] then
    case Key of
      VK_RIGHT:
        DoNavigate(ocnPageRight);
      VK_LEFT:
        DoNavigate(ocnPageLeft);
      VK_PRIOR:
        DoNavigate(ocnTop);
      VK_NEXT:
        DoNavigate(ocnBottom);
      VK_HOME:
        SetLeftTop(cxNullPoint, IsScrollAnimationEnabled);
      VK_END:
        SetLeftTop(Point(FullWidth, FullHeight), IsScrollAnimationEnabled);
      VK_INSERT:
        SelectNode(InsertNode(True));
    end;
  if Shift = [] then
    case Key of
      VK_NEXT:
      ...
      VK_RETURN, VK_F2:
        ShowEditor;
      VK_RIGHT:
        DoNavigate(ocnSelectNextRight);
      VK_LEFT:
        DoNavigate(ocnSelectNextLeft);
    end;
end;
...
begin
  ...
  begin
    ...
    FOptions := Value;
    if ocRect3D in Chgd then
      InvalidateSizes(ivBoth);
    if ocButtons in Chgd then
      Invalidate;
    if ocSelect in Chgd then
      InvalidateSel;
    if (ocFocus in Chgd) and Focused then
      InvalidateSel;
  end;
end;
...
begin
  if not IsScrollBarsArea(ScreenToClient(GetMouseCursorPos)) and
    (ocDblClick in Options) and
    (FHitTests * [htOnShape, htOnButton] = [htOnShape]) then
...
var
  ...
  Node: TdxOcNode;
begin
  if (Value = FImages) or (csDestroying in ComponentState) then
    Exit;
  BeginUpdate;
  if FImages <> nil then
...
{$IFNDEF STREAMANSIFORMAT}
  Stream.WriteBuffer(StreamDescriptionUNICODE[1],
    dxStrLen(PAnsiChar(StreamDescriptionUNICODE)));
{$ENDIF}
...
begin
  ...
  begin
    ...
    if Assigned(OnEdited) then
      OnEdited(Self, Selected, TheText);
...
procedure TdxCustomOrgChart.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
begin
  if GetEditing then
    HideEditor(False); // Fix: by Kirill (ModalWindow)
  inherited;
...
begin
  if ocDblClick in Options then
    GetNodeAt(Msg.XPos, Msg.YPos);
  inherited;
end;
...
begin
  ...
  begin
    ...
    if emLeft in Tree.EditMode then
      Style := Style or ES_LEFT;
    if emCenter in Tree.EditMode then
      Style := Style or ES_CENTER;
    if emRight in Tree.EditMode then
      Style := Style or ES_RIGHT;
    if emWrap in Tree.EditMode then
      Style := Style and not ES_AUTOHSCROLL;
    if emUpper in Tree.EditMode then
      Style := Style or ES_UPPERCASE;
    if emLower in Tree.EditMode then
...
begin
  R := ClientRect;
  Tree.DoDrawText(Tree.Canvas.Handle, Text + 'WW' + #13 + #10 + #13 + #10 +
    'WW', R, DT_CALCRECT);
  W := R.Right - R.Left;
  H := R.Bottom - R.Top;
  if W < FMinW then
...
procedure TdxOrgChartGdiPlusPainter.DrawFrameDiamond(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect);
var
...
begin
  ...
  try
    ...
    AGraphics.Polygon(APoints, GetLineColor, GetBrushColor(ACanvas),
      Chart.LineWidth, psSolid, MaxByte, MaxByte);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawFrameEllipse(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect);
var
  AGraphics: TdxGPGraphics;
begin
  ...
  try
    ...
    AGraphics.Ellipse(R, GetLineColor, GetBrushColor(ACanvas), Chart.LineWidth,
      psSolid, MaxByte, MaxByte);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawFrameRectangle(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect; ARect3D: Boolean);
var
  AGraphics: TdxGPGraphics;
  AShadowSize: Integer;
begin
  AShadowSize := Chart.LineWidth;
  AGraphics := dxGpBeginPaint(ACanvas.Handle, cxRectInflate(R, AShadowSize,
    AShadowSize));
  try
    AGraphics.SmoothingMode := smAntiAlias;
    if ARect3D then
      AGraphics.Rectangle(cxRectOffset(R, AShadowSize, AShadowSize),
        GetLineColor, clNone, Chart.LineWidth, psSolid, 200, MaxByte);
    AGraphics.Rectangle(R, GetLineColor, GetBrushColor(ACanvas),
      Chart.LineWidth, psSolid, MaxByte, MaxByte);
  finally
    AGraphics.Free;
  end;
...
procedure TdxOrgChartGdiPlusPainter.DrawFrameRoundRect(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect; ARadius: Integer);
var
  AGraphics: TdxGPGraphics;
begin
  AGraphics := dxGpBeginPaint(ACanvas.Handle, R);
  try
    AGraphics.SmoothingMode := smAntiAlias;
    AGraphics.RoundRect(R, GetLineColor, GetBrushColor(ACanvas), ARadius div 2,
      ARadius div 2, Chart.LineWidth, MaxByte, MaxByte);
  finally
    dxGpEndPaint(AGraphics);
  end;
end;

procedure TdxOrgChartGdiPlusPainter.DrawLine(ACanvas: TcxCanvas;
  X1, Y1, X2, Y2: Integer; AWidth: Single);
var
  AGraphics: TdxGPGraphics;
begin
  AGraphics := dxGpBeginPaint(ACanvas.Handle, cxRect(Min(X1, X2), Min(Y1, Y2),
    Max(X1, X2), Max(Y1, Y2)));
  try
    AGraphics.SmoothingMode := smAntiAlias;
...
begin
  ...
  begin
    if Expanded then
      Exit;
    Allow := True;
    if Assigned(Owner.OnExpansion) then
      Owner.OnExpansion(Owner, Self, Allow);
    if not Allow then
      Exit;
    ...
    if Allow then
      Owner.Update;
    if Assigned(Owner.OnExpanded) then
      Owner.OnExpanded(Owner, Self);
  end;
end;
...
begin
  if Count = 0 then
    Exit;
  if (Owner.Selected <> nil) and Owner.Selected.HasAsParent(Self) then
    Selected := True;
  FDeleting := True;
  for I := 0 to Count - 1 do
...
begin
  ...
  R := DisplayRect;
  if (cxRectWidth(R) > Owner.ClientBoundsWidth) or
    (cxRectHeight(R) > Owner.ClientBoundsHeight) then
    DisplayToClient(R);
...
begin
  ...
  else if (Dest = Self) or Dest.HasAsParent(Self) or not Owner.IsMyNode(Dest)
  then
    Exit;
  case Mode of
    naInsert:
      SetPar(Dest.Parent, Dest.Index);
    naAdd:
      SetPar(Dest.Parent, -1);
    naAddFirst:
      SetPar(Dest.Parent, 0);
    naAddChild:
      SetPar(Dest, -1);
    naAddChildFirst:
      SetPar(Dest, 0);
  ...
  begin
    if ParIdx > Index then
      Dec(ParIdx);
    if (ParIdx = Index) or (ParIdx < 0) and (getNextSibling = nil) then
      Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      HideEditor(True);
      InvalidateNode(Self);
      if (FNodeAt = Self) and (htOnRect in FHitTests) then
        FHitTests := [];
      NodeChanged(Self);
    end;
  end;
...
begin
  if Assigned(Owner.OnSetFont) then
    Owner.OnSetFont(Owner, Self, Font);
end;
...
procedure TdxOcNode.SetAnimXY(LeftX, TopY: Integer; const Clip: TRect;
  First: Boolean);
var
  R: TRect;
  I: Integer;
begin
  ...
  begin
    ...
    if Expanded then
      I := 999999
    else if Owner.HasButton(Self) then
      I := 6 - Owner.LineWidth div 2;
    if I < 0 then
      I := 0;
    Inc(R.Bottom, I);
    if not((R.Left > Clip.Right) or (R.Right < Clip.Left) or
      (R.Top > Clip.Bottom) or (R.Bottom < Clip.Top)) then
      FDraw := True;
    Dec(R.Bottom, I);
    LeftX := LeftX + ChildOffset;
    TopY := R.Bottom;
    FullToDisplay(R);
    FAnimX := R.Left;
    FAnimY := R.Top;
  ...
  begin
    ...
    begin
      ...
      if FParent.ChildAlign = caCenter then
        I := I div 2;
      Inc(FAnimX, I);
    end;
  end;
  ...
  if not(FDraw or First and (Owner.FCollapsed <> nil)) then
    Exit;
...
begin
  if Text = Value then
    Exit;
  SetText(Value);
  if Owner.EditMode * [emWrap, emGrow] <> [emWrap] then
    AdjustSizes(Text);
  if FParent = nil then
    Exit;
  Owner.InvalidateNode(Self);
...
function TdxOrgChartCustomCustomizeForm.CalculateFormPosition
  (AOrgChart: TdxCustomOrgChart): TRect;
var
  APoint: TPoint;
begin
...
function TdxOrgChartCustomCustomizeForm.Execute
  (AOrgChart: TdxCustomOrgChart): Boolean;
begin
...
procedure TdxOrgChartCustomCustomizeForm.PreparePreview(APreview,
  ASource: TdxCustomOrgChart);
begin
  ...
  APreview.DefaultNodeHeight := APreview.ScaleFactor.Apply
    (ASource.DefaultNodeHeight, ASource.ScaleFactor);
  APreview.DefaultNodeWidth := APreview.ScaleFactor.Apply
    (ASource.DefaultNodeWidth, ASource.ScaleFactor);
  APreview.DefaultImageAlign := ASource.DefaultImageAlign;
  APreview.IndentX := APreview.ScaleFactor.Apply(ASource.IndentX,
    ASource.ScaleFactor);
  APreview.IndentY := APreview.ScaleFactor.Apply(ASource.IndentY,
    ASource.ScaleFactor);
  ...
  APreview.Font.Height := APreview.ScaleFactor.Apply(ASource.Font.Height,
    ASource.ScaleFactor);
...
procedure TdxOrgChartCustomCustomizeForm.SaveChanges
...
function TdxOrgChartCustomPainter.GetNodeColor(ACurrectColor: TColor;
  ASelected: Boolean): TColor;
begin
  if ASelected then
    Result := cxGetActualColor(Chart.SelectedNodeColor,
      LookAndFeelPainter.DefaultSelectionColor)
  else
...
procedure TdxOrgChartCustomPainter.DrawBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
  ACanvas.FillRect(R, GetBackgroundColor);
end;

procedure TdxOrgChartCustomPainter.DrawFrame(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect);
begin
  case ANode.Shape of
    shRoundRect:
...
procedure TdxOrgChartCustomPainter.DrawFrameDiamond(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect);
var
  APoints: array [0 .. 3] of TPoint;
begin
...
procedure TdxOrgChartCustomPainter.DrawFrameEllipse(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect);
begin
...
procedure TdxOrgChartCustomPainter.DrawFrameRectangle(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect; ARect3D: Boolean);
var
  ABorderThin: Integer;
begin
...
procedure TdxOrgChartCustomPainter.DrawFrameRoundRect(ACanvas: TcxCanvas;
  ANode: TdxOcNode; const R: TRect; ARadius: Integer);
begin
  ACanvas.Canvas.RoundRect(R.Left, R.Top, R.Right, R.Bottom, ARadius, ARadius);
end;

procedure TdxOrgChartCustomPainter.DrawLine(ACanvas: TcxCanvas;
  X1, Y1, X2, Y2: Integer; AWidth: Single);
begin
...
function TdxOrgChartCustomPainter.GetLookAndFeelPainter
