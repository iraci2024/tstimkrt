...
{$ENDIF}
  ...
  cxButtons, dxCore, cxImageList, dxForms,
  dxLayoutLookAndFeels, dxLayoutContainer, dxLayoutControl, cxContainer, cxEdit,
  cxListBox, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, cxLabel, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxSpinEdit, cxCheckBox, cxClasses,
  cxFontNameComboBox, cxColorComboBox, cxCheckListBox, dxGalleryControl,
  dxColorGallery, cxImageComboBox, dxColorDialog,
  dxSpreadSheetCoreStrs, dxSpreadSheetGraphics, dxSpreadSheetClasses,
  dxSpreadSheetTypes, dxSpreadSheetNumberFormat,
  dxSpreadSheetStyles, dxSpreadSheetCellStyleEditDialogController,
...
type
  ...
  TdxSpreadSheetCellStyleEditDialogFormClass = class of
    TdxSpreadSheetCellStyleEditDialogForm;
  ...
  TdxSpreadSheetCellStyleEditDialogForm = class(TdxForm)
    ...
    procedure cbColorComboBoxButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure cbColorComboBoxNamingConvention(Sender: TObject;
      const AColor: TColor; var AColorDescription: string);
    ...
    procedure clbFontStylesClickCheck(Sender: TObject; AIndex: Integer;
    ...
    procedure lbLineStylesDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
      AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
    procedure lbNumberFormatTemplatesClick(Sender: TObject);
    procedure NumberFormatChanged(Sender: TObject);
    procedure pbBordersPreviewMouseDown(Sender: TObject; Button: TMouseButton;
    ...
    procedure teFontNameKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure teFontNamePropertiesChange(Sender: TObject);
    procedure teFontSizeKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure teFontSizePropertiesChange(Sender: TObject);
    procedure teFontStyleKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure teFontStylePropertiesChange(Sender: TObject);
  strict private
    FBordersButtons: array [TdxSpreadSheetCellStyleEditDialogBorder]
      of TcxButton;
    ...
    procedure SetNumberFormatCategory(const AValue
      : TdxSpreadSheetNumberFormatCategory);
  protected
    procedure ApplyLocalization;
    procedure SetBordersStyle(ABorders
      : TdxSpreadSheetCellStyleEditDialogBorders;
    ...
    procedure Load(ACellStyle: TdxSpreadSheetCellStyle; ARow, AColumn: Integer;
      const AArea: TRect); overload;
    procedure LoadBorders(ACellStyle: TdxSpreadSheetCellStyle;
    ...
    procedure LoadTextAlignment(ACellStyle: TdxSpreadSheetCellStyle;
      ARow, AColumn: Integer);

    procedure Merge(ACellStyle: TdxSpreadSheetCellStyle; ARow, AColumn: Integer;
      const AArea: TRect);
    ...
    procedure MergeTextAlignment(ACellStyle: TdxSpreadSheetCellStyle;
      ARow, AColumn: Integer);

    procedure Save(ACellStyle: TdxSpreadSheetCellStyle; ARow, AColumn: Integer;
      const AArea: TRect); overload;
    procedure SaveBorders(ACellStyle: TdxSpreadSheetCellStyle;
      ARow, AColumn: Integer; const AArea: TRect);
    ...
    property BordersPreview: TdxSpreadSheetCellStyleEditDialogBordersPreview
      read FBordersPreview;
    property Controller: TdxSpreadSheetCellStyleEditDialogCustomController
      read FController;
    ...
    property NumberFormatCategory: TdxSpreadSheetNumberFormatCategory
      read GetNumberFormatCategory write SetNumberFormatCategory;
  public
    constructor Create(AOwner: IdxDialogOwner; AActivePage: Integer = -1;
      ADummy: Integer = 0); reintroduce; virtual;
    destructor Destroy; override;
    procedure Initialize(AController
...
uses
  Math, dxTypeHelpers, dxHashUtils, cxDrawTextUtils, dxCoreGraphics, cxGeometry,
  dxSpreadSheetUtils,
...
constructor TdxSpreadSheetCellStyleEditDialogForm.Create(AOwner: IdxDialogOwner;
  AActivePage: Integer = -1; ADummy: Integer = 0);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.Initialize
...
begin
  ...
  try
    ...
    lciMergeCells.Enabled := Controller.CanMergeAreas and
      (Controller.HasMultipleCellsAreaAtHorz or
      Controller.HasMultipleCellsAreaAtVert);
...
begin
  ...
  lciCustomFormatCode.Caption := cxGetResourceString
    (@sdxFormatCellsDialogCustomCode);
  lciDecimalPlaces.Caption := cxGetResourceString
    (@sdxFormatCellsDialogDecimalPlaces);
  lciNumberFormatTemplates.Caption :=
    cxGetResourceString(@sdxFormatCellsDialogNumberFormatTemplates);
  cbUseThousandSeparator.Caption :=
    Format(cxGetResourceString(@sdxFormatCellsDialogUseThousandSeparator),
    [dxFormatSettings.ThousandSeparator]);
  Helper.PopulateNumberFormatCategories(lbCategory);
  UpdateNumberFormatCategoryDescription;

  // Text align page
  lcgTextAlignment.Caption := cxGetResourceString
    (@sdxFormatCellsDialogGroupTextAlignment);
  cbMergeCells.Caption := cxGetResourceString(@sdxFormatCellsDialogMergeCells);
  cbShrinkToFit.Caption := cxGetResourceString
    (@sdxFormatCellsDialogShrinkToFit);
  cbWrapText.Caption := cxGetResourceString(@sdxFormatCellsDialogWrapText);
  lsiTextAlignment.Caption := cxGetResourceString
    (@sdxFormatCellsDialogTextAlignment);
  lsiTextControl.Caption := cxGetResourceString
    (@sdxFormatCellsDialogTextControl);
  lciTextAlignHorz.Caption := cxGetResourceString
    (@sdxFormatCellsDialogTextAlignHorz);
  lciTextAlignHorzIndent.Caption :=
    cxGetResourceString(@sdxFormatCellsDialogTextAlignHorzIndent);
  lciTextAlignVert.Caption := cxGetResourceString
    (@sdxFormatCellsDialogTextAlignVert);
  ...
  btnFontReset.Caption := cxGetResourceString
    (@sdxFormatCellsDialogButtonResetFont);
  lcgFontEffects.Caption := cxGetResourceString
    (@sdxFormatCellsDialogGroupFontEffects);
  lcgFontPreview.Caption := cxGetResourceString
    (@sdxFormatCellsDialogFontPreview);
  ...
  lciFontUnderline.Caption := cxGetResourceString
    (@sdxFormatCellsDialogFontUnderline);
  cbFontStrikethrough.Caption := cxGetResourceString
    (@sdxFormatCellsDialogFontStrikethrough);
  ...
  lbBordersHint.Caption := cxGetResourceString
  ...
  lciBorderInside.Caption := cxGetResourceString
    (@sdxFormatCellsDialogBorderInside);
  lciBorderNone.Caption := cxGetResourceString(@sdxFormatCellsDialogBorderNone);
  lciBorderOutline.Caption := cxGetResourceString
    (@sdxFormatCellsDialogBorderOutline);
  lciLineColor.Caption := cxGetResourceString
    (@sdxFormatCellsDialogBorderLineColor);
  lciLineStyles.Caption := cxGetResourceString
    (@sdxFormatCellsDialogBorderLineStyle);
  BordersPreview.PreviewText := cxGetResourceString
    (@sdxFormatCellsDialogPreviewText);
  ...
  lciFillBackgroundColor.Caption :=
    cxGetResourceString(@sdxFormatCellsDialogBackgroundColor);
  btnFillNoColor.Caption := cxGetResourceString(@sdxFormatCellsDialogNoColor);
  btnFillMoreColors.Caption := cxGetResourceString
    (@sdxFormatCellsDialogMoreColors);
  lciFillPatternColor.Caption := cxGetResourceString
    (@sdxFormatCellsDialogPatternColor);
  lciFillPatternStyle.Caption := cxGetResourceString
    (@sdxFormatCellsDialogPatternStyle);
  for AFillStyle := Low(AFillStyle) to High(AFillStyle) do
    icbPatternStyle.Properties.Items[Ord(AFillStyle)].Description :=
      cxGetResourceString(dxCellFillStyleNames[AFillStyle]);
  ...
  lbProtectionNotes.Caption := cxGetResourceString
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SetBordersStyle
  (ABorders: TdxSpreadSheetCellStyleEditDialogBorders;
AStyle: TdxSpreadSheetCellBorderStyle);
var
...
begin
  for ABorder := Low(ABorder) to High(ABorder) do
    ...
    begin
      ...
      BordersPreview.BordersInfo[ABorder].Color :=
        Helper.GetActualColor(ccbBorderLineColor);
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SetupControls;
  ...
  begin
    ...
    AItem.Width := cxTextWidth(AColorComboBox.Style.GetVisibleFont,
      AItem.Caption) + 8 * cxTextSpace;
...
begin
  ...
  lbLineStyles.ItemHeight := cxTextHeight(lbLineStyles.Style.Font) + 2 *
    cxTextSpace;
  pbFillPreview.Tag := clDefault;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.
  SetupDefaultsForBorderLineControls;
var
  ABorder: TdxSpreadSheetCellStyleEditDialogBorder;
...
begin
  ...
  begin
    ...
    if ABorderInfo.Assigned and not(ABorderInfo.Undefined or ABorderInfo.Empty)
    then
    begin
...
begin
  ...
  begin
    ...
    FBordersButtons[AIndex].Down :=
      not((AInfo.Style in [sscbsDefault, sscbsNone]) or AInfo.Undefined);
  end;
end;
...
begin
  if lbFontNames.ItemIndex < 0 then
    lbFontWarning.Caption := cxGetResourceString
...
begin
  case NumberFormatCategory of
    nfcNumber:
      NumberFormat := TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
        BuildCodeForNumber(seDecimalPlaces.Value,
        cbUseThousandSeparator.Checked);
    nfcPercentage:
      NumberFormat := TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
        BuildCodeForPercentage(seDecimalPlaces.Value);
    nfcScientific:
      NumberFormat := TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
        BuildCodeForScientific(seDecimalPlaces.Value);
    nfcCustom:
      NumberFormat := teCustomFormatCode.EditingText;
    nfcText:
      NumberFormat := '@';
    nfcCurrency, nfcAccounting, nfcDate, nfcTime, nfcFraction:
      NumberFormat := Helper.DisplayFormatCodeToFormatCode
        (lbNumberFormatTemplates);
  else
...
begin
  case NumberFormatCategory of
    nfcNumber:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryNumberDescription);
    nfcCurrency:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryCurrencyDescription);
    nfcAccounting:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryAccountingDescription);
    nfcDate:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryDateDescription);
    nfcTime:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryTimeDescription);
    nfcPercentage:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryPercentageDescription);
    nfcCustom:
      lbCategoryDescription.Caption :=
        cxGetResourceString(@sdxFormatCellsDialogCategoryCustomDescription);
  else
    lbCategoryDescription.Caption := '';
  end;
  ...
  case NumberFormatCategory of
    nfcGeneral:
      lbGeneralNotes.Caption := cxGetResourceString
        (@sdxFormatCellsDialogCategoryGeneralNotes);
    nfcText:
      lbGeneralNotes.Caption := cxGetResourceString
...
begin
  ...
  lbSample.Style.TextColor := cxGetActualColor(AResult.Color,
    lbSample.Style.LookAndFeel.Painter.DefaultContentTextColor);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.UpdateShrinkToFitState;
begin
  lciShrinkToFit.Enabled :=
    not(cbWrapText.Checked or
    (TdxSpreadSheetDataAlignHorz(ccbTextAlignHorz.ItemObject) in [ssahJustify,
    ssahFill, ssahDistributed]));
end;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.LoadBrush
...
procedure TdxSpreadSheetCellStyleEditDialogForm.LoadFont
...
procedure TdxSpreadSheetCellStyleEditDialogForm.LoadNumberFormat
  (ADataFormat: TdxSpreadSheetCellDataFormat);
var
...
begin
  if ADataFormat.FormatCode = '@' then
    ...
    if TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.IsGeneralCode
    (ADataFormat.FormatCode) then
    NumberFormatCategory := nfcGeneral
  else

    if TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.IsNumberCode
    (ADataFormat.FormatCode, ADecimalPlaces, AUseThousandSeparator) then
  begin
    ...
    if TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.IsPercentageCode
    (ADataFormat.FormatCode, ADecimalPlaces) then
  begin
    ...
    if TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.IsScientificCode
    (ADataFormat.FormatCode, ADecimalPlaces) then
  begin
  ...
  begin
    case Controller.PredefinedFormats.GetIDByFormatCode
      (ADataFormat.FormatCode) of
      $05, $06, $07, $08:
    ...
    lbNumberFormatTemplates.ItemIndex := lbNumberFormatTemplates.Items.IndexOf
      (Helper.FormatCodeToDisplayFormatCode(ADataFormat));
    teCustomFormatCode.Text := ADataFormat.FormatCode;
  end;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.LoadProtection
...
procedure TdxSpreadSheetCellStyleEditDialogForm.LoadTextAlignment
  (ACellStyle: TdxSpreadSheetCellStyle; ARow, AColumn: Integer);
begin
...
begin
  ...
  begin
    ABorderInfo := BordersPreview.BordersInfo
      [Helper.GetActualBorderType(ABorder, ARow, AColumn, AArea)];
...
procedure TdxSpreadSheetCellStyleEditDialogForm.MergeBrush
  (ABrush: TdxSpreadSheetCellBrush);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.MergeEditValue
  (AEdit: TcxCustomEdit; const AValue: Variant);
begin
  if (AEdit.EditValue <> AValue) and (AEdit.EditValue <> Null) then
    AEdit.EditValue := Null;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.MergeFont
  (AFont: TdxSpreadSheetCellFont);
begin
  ...
  if (cbFontStrikethrough.State <> cbsGrayed) and
    (cbFontStrikethrough.Checked <> (fsStrikeOut in AFont.Style)) then
    cbFontStrikethrough.State := cbsGrayed;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.MergeNumberFormat
  (ADataFormat: TdxSpreadSheetCellDataFormat);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.MergeProtection
  (ACellStyle: TdxSpreadSheetCellStyle);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.MergeTextAlignment
  (ACellStyle: TdxSpreadSheetCellStyle; ARow, AColumn: Integer);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SaveBrush
  (ABrush: TdxSpreadSheetCellBrush);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SaveFont
  (AFont: TdxSpreadSheetCellFont);
var
  AValue: Integer;
begin
  ...
  if not VarIsNull(teFontStyle.EditValue) then
    AFont.Style := AFont.Style - [fsBold, fsItalic] +
      Helper.GetFontStyle(Helper.ListBoxCache.GetValue(lbFontStyles));
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SaveNumberFormat
  (ADataFormat: TdxSpreadSheetCellDataFormat);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SaveProtection
  (ACellStyle: TdxSpreadSheetCellStyle);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SaveTextAlignment
  (ACellStyle: TdxSpreadSheetCellStyle);
begin
  ...
  if ccbTextAlignHorz.ItemIndex >= 0 then
    ACellStyle.AlignHorz := TdxSpreadSheetDataAlignHorz
      (ccbTextAlignHorz.ItemObject);
  if ccbTextAlignVert.ItemIndex >= 0 then
    ACellStyle.AlignVert := TdxSpreadSheetDataAlignVert
      (ccbTextAlignVert.ItemObject);
...
procedure TdxSpreadSheetCellStyleEditDialogForm.NumberFormatChanged
  (Sender: TObject);
begin
  UpdateNumberFormat;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.BordersPreviewChanged
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.FontLoaderCompleted
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.FontLoaderDestroyed
  (Sender: TObject);
begin
  FFontLoader := nil;
end;

function TdxSpreadSheetCellStyleEditDialogForm.GetNumberFormatCategory
  : TdxSpreadSheetNumberFormatCategory;
begin
  Result := TdxSpreadSheetNumberFormatCategory(lbCategory.ItemObject);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.SetNumberFormat
  (const AValue: string);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.SetNumberFormatCategory
  (const AValue: TdxSpreadSheetNumberFormatCategory);
begin
  lbCategory.ItemObject := TObject(AValue);
  lbCategoryClick(nil);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.teFontNameKeyDown
  (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Helper.NavigateOnListbox(Key, lbFontNames);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.teFontNamePropertiesChange
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.teFontSizeKeyDown
  (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Helper.NavigateOnListbox(Key, lbFontSizes);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.teFontSizePropertiesChange
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.teFontStyleKeyDown
  (Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  Helper.NavigateOnListbox(Key, lbFontStyles);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.teFontStylePropertiesChange
  (Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    ABorderInfo := BordersPreview.BordersInfo
      [TdxSpreadSheetCellStyleEditDialogBorder(TComponent(Sender).Tag)];
    ABorderInfo.Assigned := True;
    if (ABorderInfo.Style <> AStyle) or (ABorderInfo.Color <> AColor) or
      ABorderInfo.Undefined then
    begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.btnBordersInsideClick
  (Sender: TObject);
begin
  if lbLineStyles.ItemIndex >= 0 then
    SetBordersStyle([fcdbInsideHorizontal, fcdbInsideVertical],
      TdxSpreadSheetCellBorderStyle(lbLineStyles.ItemObject));
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.btnBordersNoneClick
  (Sender: TObject);
begin
  SetBordersStyle([Low(TdxSpreadSheetCellStyleEditDialogBorder)
    .. High(TdxSpreadSheetCellStyleEditDialogBorder)], sscbsDefault);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.btnBordersOutlineClick
  (Sender: TObject);
begin
  if lbLineStyles.ItemIndex >= 0 then
    SetBordersStyle([fcdbLeft .. fcdbBottom],
      TdxSpreadSheetCellBorderStyle(lbLineStyles.ItemObject));
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.btnFillMoreColorsClick
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.btnFillNoColorClick
  (Sender: TObject);
begin
  pbFillPreview.Tag := clDefault;
  UpdateFillSettings;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.btnFontResetClick
  (Sender: TObject);
begin
  LoadFont(Controller.DefaultFont);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.cbColorComboBoxButtonClick
...
begin
  ...
  else if AColorDescription = '' then
...
procedure TdxSpreadSheetCellStyleEditDialogForm.cbFontChanged(Sender: TObject);
  ...
  begin
    ...
    if not VarIsNull(teFontStyle.EditingValue) then
      AFont.Style := AFont.Style - [fsBold, fsItalic] +
        Helper.GetFontStyle(lbFontStyles.ItemIndex);
...
procedure TdxSpreadSheetCellStyleEditDialogForm.cbWrapTextPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.
  ccbBorderLineColorPropertiesChange(Sender: TObject);
begin
  lbLineStyles.Invalidate;
  pbBordersPreview.Invalidate;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.ccbTextAlignHorzPropertiesChange
  (Sender: TObject);
begin
  lciTextAlignHorzIndent.Enabled := TdxSpreadSheetDataAlignHorz
    (ccbTextAlignHorz.ItemObject) in [ssahLeft, ssahRight, ssahDistributed];
  UpdateShrinkToFitState;
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.cgFillItemClick(Sender: TObject;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.lbCategoryClick
  (Sender: TObject);
begin
  lcMain.BeginUpdate;
  try
    ...
    lciDecimalPlaces.Visible := NumberFormatCategory
      in [nfcNumber, nfcPercentage, nfcScientific];
    lciGeneralNotes.Visible := (lbCategory.ItemIndex >= 0) and
      (NumberFormatCategory in [nfcGeneral, nfcText]);
    lciNumberFormatTemplates.CaptionOptions.Visible := NumberFormatCategory <>
      nfcCustom;
    lciNumberFormatTemplates.Visible := NumberFormatCategory
      in [nfcCurrency, nfcAccounting, nfcDate, nfcTime, nfcCustom, nfcFraction];
    lciUseThousandSeparator.Visible := NumberFormatCategory in [nfcNumber];
    Helper.PopulateNumberFormatTemplates(lbNumberFormatTemplates,
...
procedure TdxSpreadSheetCellStyleEditDialogForm.lbFontNamesClick
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.lbFontSizesClick
  (Sender: TObject);
begin
  Helper.SyncSearchEdit(lbFontSizes, teFontSize);
end;

procedure TdxSpreadSheetCellStyleEditDialogForm.lbFontStylesClick
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogForm.lbNumberFormatTemplatesClick
  (Sender: TObject);
begin
  if NumberFormatCategory = nfcCustom then
    teCustomFormatCode.Text := Helper.DisplayFormatCodeToFormatCode
      (lbNumberFormatTemplates)
  else
    UpdateNumberFormat;
end;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.pbBordersPreviewPaint
  (Sender: TObject);
begin
  UpdateBordersPreview;
...
procedure TdxSpreadSheetCellStyleEditDialogForm.pbFillPreviewPaint
  (Sender: TObject);
begin
  if pbFillPreview.Tag <> clNone then
  begin
    ...
    try
      dxSpreadSheetDrawBackground(cxPaintCanvas, pbFillPreview.ClientRect,
        cxGetActualColor(pbFillPreview.Tag, clWindow),
        ccbPatternColor.ColorValue,
...
procedure TdxSpreadSheetCellStyleEditDialogForm.pbTextPreviewPaint
  (Sender: TObject);
var
  R: TRect;
begin
  ...
  cxTextOut(pbTextPreview.Canvas, cxGetResourceString(@sdxPreviewText), R,
