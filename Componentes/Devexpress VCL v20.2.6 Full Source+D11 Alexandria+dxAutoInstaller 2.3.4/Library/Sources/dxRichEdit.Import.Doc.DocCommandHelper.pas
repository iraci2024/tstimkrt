...
type
  ...
  public
    class function Traverse(const AGrpprl: TBytes;
      AFactory: TdxDocCommandFactoryBase; ADataStreamReader: TBinaryReader)
      : TdxDocPropertyContainer; overload; static;
    class procedure Traverse(const AGrpprl: TBytes;
      AContainer: TdxDocPropertyContainer; ADataStreamReader: TBinaryReader);
      overload; static;
    class function CreateDocCommands(const AGrpprl: TBytes;
      AFactory: TdxDocCommandFactoryBase; ADataStreamReader: TBinaryReader)
      : TList<IdxDocCommand>; static;
    class function CreateSinglePropertyModifier(ASignedOpcode: SmallInt;
      const AParameters: TBytes): TBytes; static;
  end;
...
class function TdxDocCommandHelper.Traverse(const AGrpprl: TBytes;
  AFactory: TdxDocCommandFactoryBase; ADataStreamReader: TBinaryReader)
...
class procedure TdxDocCommandHelper.Traverse(const AGrpprl: TBytes;
  AContainer: TdxDocPropertyContainer; ADataStreamReader: TBinaryReader);
var
  ACommands: TList<IdxDocCommand>;
  ACount, I: Integer;
  AChangeActions: TdxChangeActionTypes;
begin
  ACommands := CreateDocCommands(AGrpprl, AContainer.Factory,
    ADataStreamReader);
  try
    ACount := ACommands.Count;
    AChangeActions := [];
...
class function TdxDocCommandHelper.CreateDocCommands(const AGrpprl: TBytes;
  AFactory: TdxDocCommandFactoryBase; ADataStreamReader: TBinaryReader)
  : TList<IdxDocCommand>;
var
  ASprm: TBytes;
  ASprmSize: SmallInt;
  ALength, AByteCounter, ASpra, AItbdDelMax, AItbdAddMaxOffset,
    AItbdAddMax: Integer;
  AOpcode: Word;
  ...
  begin
    ...
    case ASpra of
      ...
      6:
        ...
        else if AOpcode = SprmPChgTabs then
        begin
...
class function TdxDocCommandHelper.CreateSinglePropertyModifier
  (ASignedOpcode: SmallInt; const AParameters: TBytes): TBytes;
var
  AOpcode: Word;
