...
uses
  ...
  StdCtrls, Types,
  ComCtrls, ExtCtrls, checklst, ImgList, dxCore, dxPSGlbl, dxPSCore,
  dxPSBaseGridLnk,
  dxPSGrLnks, cxControls, cxContainer, cxEdit, cxCheckBox, Menus,
...
type
  ...
  public
    ...
    property PaintItemsGraphics: Boolean read FPaintItemsGraphics
      write SetPaintItemsGraphics default False;
...
const
  ...
  dxListBoxStrings: array [0 .. 5] of string = ('Sample Text Row #1',
    'Sample Text Row #2', 'Sample Text Row #3', 'Sample Text Row #4',
    'Sample Text Row #5', 'Sample Text Row #6');
...
begin
  ...
  begin
    ...
    PaintItemsGraphics := TCustomdxListBoxReportLinkControl(Source)
      .PaintItemsGraphics;
  end;
end;
...
begin
  inherited;
  Options := dxDefaultListBoxPaintOptions;
  { [Low(TdxListBoxPaintOptions)..High(TdxListBoxPaintOptions)] }
...
function TCustomdxListBoxReportLinkControl.GetCellHasImage(ACol,
...
function TCustomdxListBoxReportLinkControl.GetCellImage(ACol, ARow: Integer)
  : TGraphic;
begin
  Result := TGraphic(CustomListBox.Items.Objects[ARow]);
end;

function TCustomdxListBoxReportLinkControl.GetCellImageTransparent(ACol,
  ARow: Integer): Boolean;
begin
...
procedure TCustomdxListBoxReportLinkControl.SetOptions
  (Value: TdxListBoxPaintOptions);
begin
...
procedure TCustomdxListBoxReportLinkControl.SetPaintItemsGraphics
  (Value: Boolean);
begin
...
procedure TCustomdxListBoxReportLinkControl.SetTransparentGraphics
  (Value: Boolean);
begin
...
function TdxLBxReportLinkDesignWindow.GetReportLink
  : TCustomdxListBoxReportLinkControl;
begin
...
begin
  ...
  chbxOnlySelected.Enabled := (ReportLink.CustomListBox = nil) or
    ListBoxGetMultiSelect(ReportLink.CustomListBox);
  chbxPaintItemGraphics.Enabled := (ReportLink.CustomListBox = nil) or
    IsPaintItemsGraphics;
...
begin
  ...
  Brush := SelectObject(DC,
    CreateSolidBrush(ColorToRGB(ReportLink.GridLineColor)));
  for I := 0 to C do
    if (((I = 0) or (I = C)) and ReportLink.IsDrawBorder) or
      ((I > 0) and (I < C) and ReportLink.IsDrawHorzLines) then
    begin
      R := Rect(R2.Left + 1, R2.Top + I * dY, R2.Right - 1,
        R2.Top + I * dY + 1);
  ...
  for I := 0 to C - 1 do
  begin
    R := Rect(R2.Left + 1, R2.Top + I * dY + 1, R2.Right - 1,
      R2.Top + (I + 1) * dY + Ord(not ReportLink.IsDrawHorzLines and
      (I < C - 1)));
    ...
    begin
      ...
      try
        cxDrawImage(cxPaintCanvas, cxRectCenterVertically(cxRectSetWidth(R,
          ScaleFactor.Apply(ilPreview.Width)),
          ScaleFactor.Apply(ilPreview.Height)), nil, ilPreview, 0, Enabled, nil,
          ScaleFactor);
      finally
    ...
    dxAssignFont(FPreviewFont, ReportLink.GetCellFont(0, I), ScaleFactor,
    ...
    Windows.DrawText(DC, PChar(S), Length(S), R, DT_NOPREFIX or DT_SINGLELINE or
      dxDrawTextTextAlignX[ReportLink.TextAlignX] or dxDrawTextTextAlignY
      [ReportLink.TextAlignY]);
  end;
  ...
  with TcxCheckBox(Sender) do
    if Checked then
      ReportLink.Options := ReportLink.Options +
        [TdxListBoxPaintOption(TTagToInt(Tag))]
    else
      ReportLink.Options := ReportLink.Options -
        [TdxListBoxPaintOption(TTagToInt(Tag))];
  Modified := True;
...
dxPSRegisterReportLink(TdxListBoxReportLink, TListBox,
  TdxLBxReportLinkDesignWindow);

finalization

dxPSUnregisterReportLink(TdxListBoxReportLink, TListBox,
  TdxLBxReportLinkDesignWindow);
