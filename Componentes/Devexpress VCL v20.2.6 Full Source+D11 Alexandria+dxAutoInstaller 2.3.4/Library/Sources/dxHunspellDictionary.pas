...
type
  ...
  strict private
    ...
    function CheckWord(const AWord: PWideChar; var AInfo: TdxSpellInfo)
      : TdxHunspellWordStem;
    function GetAbbreviationWordStem(const AWord: PWideChar;
      AWordLength: Integer; var AInfo: TdxSpellInfo;
      ACapitalizationType: TdxCapitalizationType = ctLower)
      : TdxHunspellWordStem;
    function InputConverting(ACleanedWord, AWordForCompound: PWideChar;
    ...
    function SpellSharps(ABase, APosition: PWideChar;
      AIndex, ANumRepeat: Integer; var AInfo: TdxSpellInfo)
      : TdxHunspellWordStem;
    // setters
  ...
  protected
    function CleanWord(ADest: PWideChar; ASource: PWideChar;
      out ACapitalizationType: TdxCapitalizationType;
      out AAbbreviationCount: Integer): Integer;

    procedure AfterLoad; override;
    procedure BeforeLoad; override;
    function CanLoad: Boolean; override;
    function CreateSuggestionBuilder: TdxSpellCheckerSuggestionBuilder;
      override;
    ...
    function ProcessSharps(AWord: PWideChar; AWordLength: Integer;
      AIsAbbreviation: Boolean; var AWordStem: TdxHunspellWordStem;
      var AInfo: TdxSpellInfo): Boolean;
    function ProcessWordBreak(const AWord: PWideChar;
      var AInfo: TdxSpellInfo): Boolean;
  ...
  published
    property DictionaryPath: TFileName read FDictionaryPath
      write SetDictionaryPath;
    property Enabled;
    property GrammarPath: TFileName read FGrammarPath write SetGrammarPath;
    property NgramDistance: Integer read FNgramDistance write FNgramDistance
      default 0;
    property SuggestionsLiteMode: Boolean read FSuggestionsLiteMode
      write FSuggestionsLiteMode default True;
  ...
  TdxPhoneticWordStemScoreArray = array [0 .. dxMaxWordStemCount - 1]
    of Integer;
  ...
  private
    ...
    procedure CheckHungarianCompounding(ACursor: PWideChar;
      const ACandidate: TdxLineBuffer; ACheck1, ACheck2: Integer);
    function CheckWord(const AWord: PWideChar; ALength: Integer;
      ACheckCompound: Boolean): Integer;
    function CommonCharacterPositions(S1, S2: PWideChar;
      out AHasSwap: Boolean): Integer;
    procedure GenerateNgramGuessTables(AWord: PWideChar; AWordLength: Integer;
      var AGuess, AGuessOrig: TdxGuessArray;
      var AGuessScore: TdxGuessScoreArray; const AWordStems: TdxWordStemArray);
  ...
  protected
    ...
    procedure DoNearMissStrategy(AWord: PWideChar;
      var AOnlyCompoundSuggestions: Boolean);
    procedure InsertSuggestion(AWord: PWideChar;
...
procedure TdxHunspellDictionary.LoadFromStreams(AGrammarStream,
  ADictionaryStream: TStream; AFreeStreams: Boolean;
  AMode: TdxSpellCheckerDictionaryLoadMode = dlmDefault);
begin
...
begin
  Result := inherited CanLoad and
    (not FLoadingFromStreams or FileExists(DictionaryPath) and
    FileExists(GrammarPath));
end;

function TdxHunspellDictionary.CreateSuggestionBuilder
...
begin
  ...
  if Result = nil then
    Result := TFileStream.Create(DictionaryPath, fmOpenRead or
      fmShareDenyWrite);
end;
...
begin
  if (WordStemManager = nil) or (AffixManager = nil) or
    (StrLen(AWord) >= dxMaxWordLength) then
  begin
    Result := False;
    Exit;
  end;
  Result := True;
  AWordLength := InputConverting(ACleanedWord, ACandidate, AWord,
    ACapitalizationType, AIsAbbreviation);
  ...
  begin
    ...
    begin
      ...
      if ProcessApostrophe(ACleanedWord, AWordStem, AInfo) then
        Exit;
      if ProcessSharps(ACleanedWord, AWordLength, AIsAbbreviation, AWordStem,
        AInfo) then
        Exit;
    end;
    ...
    begin
      if (AWordStem <> nil) and (ACapitalizationType = ctUpper) and AWordStem.KeepCase
      then
        AWordStem := nil;
      if AWordStem <> nil then
        Exit;
      ...
      begin
        ...
        begin
          AWordStem := GetAbbreviationWordStem(ACleanedWord, AWordLength, AInfo,
            ACapitalizationType);
          if (AWordStem <> nil) and AWordStem.KeepCase and
            (ACapitalizationType = ctUpper) then
            AWordStem := nil;
          ADone := True;
        end;
      end;
      if not ADone and (AWordStem <> nil) and AWordStem.KeepCase and
        ((ACapitalizationType = ctUpper) or not(AffixManager.CheckSharps and
        (StrScan(ACandidate, #$00DF) <> nil))) then
        AWordStem := nil;
    end;
  end;
...
function TdxHunspellDictionary.ProcessSharps(AWord: PWideChar;
  AWordLength: Integer; AIsAbbreviation: Boolean;
  var AWordStem: TdxHunspellWordStem; var AInfo: TdxSpellInfo): Boolean;
var
...
begin
  ...
  begin
    ...
    if (AWordBreakItemLength = 1) or (AWordBreakItemLength > AWordLength) then
      Continue;
    ALocalInfo := [];
    if (AWordBreakItem^ = '^') and
      (StrLComp(AWord, AWordBreakItem + 1, AWordBreakItemLength - 1) = 0) and
      Spell(AWord + AWordBreakItemLength - 1, ALocalInfo) then
      Exit;
    if ((AWordBreakItem + AWordBreakItemLength - 1)^ = '$') and
      (StrLComp(AWord + AWordLength - AWordBreakItemLength + 1, AWordBreakItem,
      AWordBreakItemLength - 1) = 0) then
    begin
...
begin
  ...
  begin
    Result := AffixManager.CompoundCheck(AWord, 0, 0, 100, 0, nil,
      False, False);
    if (Result = nil) and AffixManager.IsHungarian and
      ((AWord + AWordLength - 1)^ = '-') then
    begin
      Move(AWord^, ATempWord, AWordLength * SizeOf(WideChar));
      (ATempWord + AWordLength - 1)^ := #0;
      Result := AffixManager.CompoundCheck(ATempWord, -5, 0, 100, 0, nil,
        True, False);
    end;
    if Result <> nil then
...
function TdxHunspellDictionary.CheckForbiddenAndCompoundOnly
  (var AWordStem: TdxHunspellWordStem; var AInfo: TdxSpellInfo): Boolean;
begin
...
begin
  ...
  begin
    ...
    Result := AffixManager.AffixCheck(ACandidate, AWordLength, ATakenPrefix,
      ATakenSuffix, 0);
    if (Result <> nil) and (Result.InCompoundOnly or
      ((siiInitialCapitalization in AInfo) and Result.UpcaseOnly)) then
      Result := nil;
    if Result <> nil then
    begin
...
function TdxHunspellDictionary.SpellSharps(ABase, APosition: PWideChar;
  AIndex, ANumRepeat: Integer; var AInfo: TdxSpellInfo): TdxHunspellWordStem;
var
...
begin
  ...
  begin
    ...
    Result := SpellSharps(ATestWord, ATestWord + AOffset + 1, AIndex + 1,
      ANumRepeat + 1, AInfo);
    if Result = nil then
      Result := SpellSharps(ABase, APosition + 2, AIndex + 1,
        ANumRepeat, AInfo);
  end
  else if ANumRepeat > 0 then
    Result := CheckWord(ABase, AInfo);
end;

function TdxHunspellDictionary.InputConverting(ACleanedWord, AWordForCompound
  : PWideChar; const ASource: PWideChar;
  out ACapitalizationType: TdxCapitalizationType;
  out AIsAbbreviation: Boolean): Integer;
var
...
begin
  if AffixManager.InputConvertTable.Convert(ASource, AWordForCompound) then
    Result := CleanWord(ACleanedWord, AWordForCompound, ACapitalizationType,
      AAbbreviationCount)
  else
    Result := CleanWord(ACleanedWord, ASource, ACapitalizationType,
      AAbbreviationCount);
  AIsAbbreviation := AAbbreviationCount <> 0;
end;

function TdxHunspellDictionary.IsNumber(AWord: PWideChar;
  AWordLength: Integer): Boolean;
type
...
begin
  ...
  begin
    ...
    else if (AWord[I] = ',') or (AWord[I] = '.') or (AWord[I] = '-') then
    begin
...
begin
  ...
  (ADest + Result)^ := #0;
  ACapitalizationType := GetWordCapitalizationType(ADest, Result,
    AffixManager.Language);
end;

function TdxHunspellDictionary.GetAbbreviationWordStem(const AWord: PWideChar;
  AWordLength: Integer; var AInfo: TdxSpellInfo;
  ACapitalizationType: TdxCapitalizationType = ctLower): TdxHunspellWordStem;
var
  ACandidate: TdxLineBuffer;
begin
  Move(AWord^, ACandidate, AWordLength * SizeOf(WideChar));
...
constructor TdxHunspellSuggestionBuilder.Create(ADictionary
...
begin
  ...
  begin
    ...
    begin
      if (APosition > AffixManager.KeyboardString) and ((APosition - 1)^ <> '|')
      then
      begin
...
var
  ACandidate: TdxLineBuffer;
begin
  if (Length(Word) < 2) or (AffixManager.MapTable.Count = 0) then
    Exit;
...
begin
  ...
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            ProcessMap(AWord, ACandidate, AWordIndex + 1, StrLen(ACandidate),
              AMapTable);
            if IsTimeOut then
              Exit;
          end;
    ...
    begin
      ...
      ProcessMap(AWord, ACandidate, AWordIndex + 1, ACandidateIndex + 1,
        AMapTable);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if ACheck1 <> 0 then
    begin
      ACheck2 := CheckWord(ABreakPos + 1, StrLen(ABreakPos + 1),
        FCheckCompound);
      if ACheck2 <> 0 then
      begin
        ABreakPos^ := ' ';
        ...
        if (AffixManager.TryChars <> nil) and
          ((StrScan(AffixManager.TryChars, 'a') <> nil) or
          (StrScan(AffixManager.TryChars, '-') <> nil)) and
          (StrLen(ABreakPos + 1) > 1) and
          (StrLen(ACandidate) - StrLen(ABreakPos) > 1) then
        begin
          ABreakPos^ := '-';
          if not SuggestionExist(ACandidate) then
            AddSuggestion(ACandidate);
...
procedure TdxHunspellSuggestionBuilder.AddSuggestion(AWord: PWideChar;
  ADistance: Integer = 2);
begin
  Suggestions.Add(AWord, Dictionary, ADistance);
end;

function TdxHunspellSuggestionBuilder.CanAddToSuggestions(const ATestWord
  : string): Boolean;
begin
...
begin
  ...
  if AffixManager.InputConvertTable.Convert(P, ACleanedWord) then
    Dictionary.CleanWord(P, ACleanedWord, FCapitalizationType,
      FAbbreviationCount)
  else
...
begin
  ...
  GenerateNearMissSuggestions(AWord, AExistCapitalizedWords,
    AOnlyCompoundSuggestions);
  GenerateNgramSuggestions(AWord, AExistCapitalizedWords,
    AOnlyCompoundSuggestions);
  StrCopy(ACandidate, AWord);
  ...
  if (FAbbreviationCount <> 0) and AffixManager.SuggestionsWithDots then
    for I := 0 to Suggestions.Count - 1 do
      StrCat(PWideChar(Suggestions[I].Word), AWord + AWordLength -
        FAbbreviationCount);
  RemoveBadSuggestions;
  if AffixManager.OutputConvertTable.Count > 0 then
    for I := 0 to Suggestions.Count - 1 do
      if AffixManager.OutputConvertTable.Convert(PWideChar(Suggestions[I].Word),
        ACandidate) then
        Suggestions[I].Word := PWideChar(@ACandidate);
end;

function TdxHunspellSuggestionBuilder.SuggestionExist(AWord: PWideChar)
  : Boolean;
var
  I: Integer;
begin
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            StrCopy(ATempWord + Integer(AWordPos - AWord),
              PWideChar(Suggestions[I].Word));
            if not ADone then
            begin
              StrCat(ATempWord, '-');
...
procedure TdxHunspellSuggestionBuilder.CheckHungarianCompounding
  (ACursor: PWideChar; const ACandidate: TdxLineBuffer;
  ACheck1, ACheck2: Integer);
...
begin
  if AffixManager.IsHungarian and
    (((ACursor[-1] = ACursor[1]) and (((ACursor > PWideChar(@ACandidate) + 1)
    and (ACursor[-1] = ACursor[-2])) or (ACursor[-1] = ACursor[2]))) or
    MultiplyCompounding) then
    ACursor^ := '-';
end;
...
begin
  ...
  begin
    if AffixManager.IsCompoundWordsAvailable and
      (AffixManager.CompoundCheck(AWord, 0, 0, 100, 0, nil, False, True) <> nil)
    then
      Result := 3;
    Exit;
  end;
  ...
  begin
    ...
    begin
      if AWordStem.NeedAffix or AWordStem.UpcaseOnly or AWordStem.InCompoundOnly
      then
    ...
    AWordStem := AffixManager.ProcessPrefixCheck(AWord, ALength, ATempPrefix,
      ATempSuffix, cwpNone);
  if AWordStem <> nil then
    ANoSuffix := True
  else
    AWordStem := AffixManager.ProcessSuffixCheck(AWord, ALength, [], nil,
      ATempSuffix);
  if (AWordStem = nil) and AffixManager.CompatibleFlagsExist then
  begin
    AWordStem := AffixManager.ProcessTwoSuffixCheck(AWord, ALength, [], nil,
      ATempSuffix);
    if AWordStem = nil then
  ...
  if (AWordStem <> nil) and
    (AWordStem.UpcaseOnly or AffixManager.IsForbiddenOrNoSuggestWord(AWordStem,
    True) or AWordStem.InCompoundOnly) then
    Exit;
  if AWordStem <> nil then
  begin
...
function TdxHunspellSuggestionBuilder.CommonCharacterPositions(S1,
  S2: PWideChar; out AHasSwap: Boolean): Integer;
var
...
begin
  ...
  AHasSwap := (ADiffCount = 2) and (S1^ = #0) and (P2^ = #0) and
    (ADiffChars[0, 0] = ADiffChars[1, 1]) and
    (ADiffChars[0, 1] = ADiffChars[1, 0]);
end;

procedure TdxHunspellSuggestionBuilder.GenerateNgramGuessTables
  (AWord: PWideChar; AWordLength: Integer;
...
var
  ...
  I, J, K, AThreshold, AIndex, ALength, ASwapCharacterWeight, ACount, AScore,
    ALongestCommonSubsequenceLength, AMinScore,
    AEqualCharacterPositionsWeight: Integer;
...
begin
  ...
  begin
    ...
    AThreshold := AThreshold + Ngram(AWord, ACandidate, AWordLength,
      [ngoAnyMismatch, ngoLowering]);
  end;
    ...
    begin
      ...
      begin
        ...
        if AWordStems[I].MorphologyPhone then
          AMorphology := CopyMorphology(AField,
            AWordStems[I].MorphologicalDescription, mlPhon);
        ACount := AffixManager.ExpandWordStem(AWordStemList, dxMaxWordStemCount,
          AWordStems[I].WordStem, AWordStems[I].WordStemLength,
          AWordStems[I].AffixFlags, AWord, AWordLength, AMorphology);
        for J := 0 to ACount - 1 do
        begin
          AScore := Ngram(AWord, AWordStemList[J].Word, AWordLength,
            [ngoAnyMismatch, ngoLowering]) + LeftCommonSubStringLength(AWord,
            AWordStemList[J].Word);
  ...
  begin
    ...
    begin
      ...
      ALongestCommonSubsequenceLength :=
        Similarity.GetLongestCommonSubsequenceLength(AWord, ACandidate);
      if (AWordLength = ALength) and
        (AWordLength = ALongestCommonSubsequenceLength) then
      begin
        Inc(AGuessScore[I], 2000);
        Break;
      end;
      AEqualCharacterPositionsWeight := 0;
      if ALongestCommonSubsequenceLength = CommonCharacterPositions(AWord,
        ACandidate, AIsSwap) then
        AEqualCharacterPositionsWeight := 1;
      ASwapCharacterWeight := 0;
      if AIsSwap then
        ASwapCharacterWeight := 1000;
      Inc(AGuessScore[I], 2 * ALongestCommonSubsequenceLength -
        Abs(AWordLength - ALength) + LeftCommonSubStringLength(AWord,
        ACandidate) + AEqualCharacterPositionsWeight + ASwapCharacterWeight);
    end;
  end;
  Sort(@AGuess, @AGuessOrig, @AGuessScore, dxMaxGuessCount);
end;

procedure TdxHunspellSuggestionBuilder.GenerateNearMissSuggestions
  (AWord: PWideChar; var AExistCapitalizedWords, AOnlyCompoundSuggestions
  : Boolean);
var
  ...
  case FCapitalizationType of
    ...
    ctMixedCapitalized, ctMixed:
      begin
        ...
        begin
          ATempSuggestion := MakeSecondWordCapitalized
            (PWideChar(Suggestions[ASuggestionCount].Word));
...
procedure TdxHunspellSuggestionBuilder.GenerateNgramSuggestions
  (AWord: PWideChar; var AExistCapitalizedWords, AOnlyCompoundSuggestions
  : Boolean);
var
  ACandidate: TdxLineBuffer;
...
begin
  if ((Suggestions.Count = 0) or (not Dictionary.SuggestionsLiteMode and
    AOnlyCompoundSuggestions)) and (MaxNgramSuggestionCount > 0) then
  begin
    case FCapitalizationType of
      ctLower:
      ...
      ctUpper:
        begin
          ...
          NgramSuggestion(ACandidate);
          if ASuggestionCount < Suggestions.Count then
            MakeAllCapital(PWideChar(Suggestions[ASuggestionCount].Word),
              AffixManager.Language);
        end
    end;
  end;
...
function TdxHunspellSuggestionBuilder.GetWordStemManager
...
function TdxHunspellSuggestionBuilder.IsForbidden
  (const AWord: PWideChar): Boolean;
var
  ALength: Integer;
  AWordStem: TdxHunspellWordStem;
  ATempPrefix: TdxPrefix;
...
begin
  ...
  AWordStem := AffixManager.Lookup(AWord);
  if (AWordStem <> nil) and (AWordStem.NeedAffix or AWordStem.InCompoundOnly)
  then
    AWordStem := nil;
  if (AffixManager.ProcessPrefixCheck(AWord, ALength, ATempPrefix, ATempSuffix,
    cwpFirst) = nil) then
    AWordStem := AffixManager.ProcessSuffixCheck(AWord, ALength, [], nil,
      ATempSuffix);
  Result := (AWordStem <> nil) and AWordStem.Forbidden;
...
function TdxHunspellSuggestionBuilder.LeftCommonSubStringLength(AWord1,
  AWord2: PWideChar): Integer;
var
...
begin
  ...
  begin
    if (AWord1^ = AWord2^) or
      (AWord1^ = WideLowCase(AWord2^, AffixManager.Language)) then
    begin
      AStartWord1 := AWord1;
      repeat
        Inc(AWord1);
...
function TdxHunspellSuggestionBuilder.MakeSecondWordCapitalized
  (AWord: PWideChar): PWideChar;
var
...
begin
  ...
  AWordLength := StrLen(AWord);
  CalculateScores(AWord, AWordLength, AWordStems, AScores, APhoneticWordStems,
    APhoneticScores);
  AddNgramSuggestions(AWord, AWordLength, AWordStems);
  AddPhoneticSuggestions(AWord, AWordLength, APhoneticWordStems,
    APhoneticScores);
end;
...
procedure TdxHunspellSuggestionBuilder.AddNgramSuggestions(AWord: PWideChar;
  ...
  function CheckDistance(ACandidate: PWideChar; var ADistance: Integer)
    : Boolean;
  var
  ...
  begin
    ...
    begin
      ADistance := Similarity.GetDistance(ACandidate, StrLen(ACandidate), AWord,
        AWordLength);
...
begin
  GenerateNgramGuessTables(AWord, AWordLength, AGuess, AGuessOrig, AGuessScores,
    AWordStems);
  ASame := False;
...
begin
  ...
  begin
    ...
    Inc(APhoneticScores[I], 2 * Similarity.GetLongestCommonSubsequenceLength
      (AWord, ACandidate) - Abs(AWordLength - ALength) +
      LeftCommonSubStringLength(AWord, ACandidate));
  end;
  Sort(@APhoneticWordStems, nil, @APhoneticScores, dxMaxWordStemCount);
  AAddedSuggestionCount := 0;
  for I := 0 to dxMaxWordStemCount - 1 do
    if (AAddedSuggestionCount < dxMaxPhoneticSuggestionCount) and
      (APhoneticWordStems[I] <> nil) and
      not SuggestionExist(APhoneticWordStems[I]) and
      (CheckWord(APhoneticWordStems[I], StrLen(APhoneticWordStems[I]), False)
      <> 0) then
    begin
...
var
  ...
  AWordStem: TdxHunspellWordStem;
  I, AIndex, AWordStemScore, AWordStemScore2, ATableIndex, AMinScore,
    APhoneticScore, APhoneticIndex: Integer;
begin
  ...
  begin
    ...
    AWordStemScore := Trigram(AWord, AWordStem.WordStem,
      [ngoLongerWorse, ngoLowering]) + LeftCommonSubStringLength(AWord,
      AWordStem.WordStem);
    if AWordStem.MorphologyPhone and
      (CopyMorphology(AField, AWordStem.MorphologicalDescription, mlPhon) <> nil)
    then
    begin
      AWordStemScore2 := Trigram(AWord, AField, [ngoLongerWorse, ngoLowering]) +
        LeftCommonSubStringLength(AWord, AField);
      AWordStemScore := Max(AWordStemScore, AWordStemScore2);
    end;
    if (PhoneTable.Count > 0) and (AWordStemScore > 2) and
      (Abs(AWordLength - AWordStem.WordStemLength) <= 3) then
    begin
    ...
    if (APhoneticScore > 0) and
      (APhoneticScore > APhoneticScores[APhoneticIndex]) then
    begin
...
var
  ...
  I, AReplacementLen, ATextLen, AWordLength, ACount,
    ASavedSuggestionCount: Integer;
begin
  AWordLength := Length(Word);
  ...
  begin
    ...
    begin
      ...
      if ATextPos - AWordStart + AReplacementLen +
        Integer(StrLen(ATextPos + ATextLen)) >= dxMaxWordLength then
        Break;
      StrCopy(@ACandidate[(ATextPos - AWordStart)],
        AReplaceTable[I].Replacement);
      StrCopy(@ACandidate[(ATextPos - AWordStart) + AReplacementLen],
        ATextPos + ATextLen);
      CheckAddToSuggestions(PWideChar(@ACandidate));
...
GetRegisteredDictionaryTypes.Register(TdxHunspellDictionary,
  cxGetResourceString(@sdxSpellCheckerHunspellDictionary));
