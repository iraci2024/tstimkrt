...
type
  ...
  TdxPointChangeEvent = procedure(Sender: TObject; Coords: TdxPointCoords)
    of object;
  ...
  protected
    ...
    procedure DoChanging(ACoords: TdxPointCoords;
      var AValues: array of Integer); dynamic;
  public
    ...
    property OnChanging: TdxPointChangingEvent read FOnChanging
      write FOnChanging;
  ...
  TdxRectChangeEvent = procedure(Sender: TObject; Sides: TdxRectSides)
    of object;
  ...
  protected
    ...
    procedure DoChanging(ASides: TdxRectSides;
      var AValues: array of Integer); dynamic;
  public
    ...
    property Side[ASide: TdxRectSide]: Integer read GetSide
      write SetSide; default;
    property TopLeft: TPoint Index 0 read GetRectPoint write SetRectPoint;
    property Width: Integer read GetWidth write SetWidth;

    property OnChanging: TdxRectChangingEvent read FOnChanging
      write FOnChanging;
...
begin
  if Source = nil then
    Empty
  else if Source is TdxPointWrapper then
...
begin
  if Assigned(FOnChanging) then
    FOnChanging(Self, ACoords, AValues);
end;
...
begin
  if Source = nil then
    Empty
  else if Source is TdxRectWrapper then
...
begin
  if Assigned(FOnChanged) then
    FOnChanged(Self, ASides);
end;

procedure TdxRectWrapper.DoChanging(ASides: TdxRectSides;
  var AValues: array of Integer);
begin
  if Assigned(FOnChanging) then
    FOnChanging(Self, ASides, AValues);
end;
...
begin
  ...
  else if not TdxPointWrapper.ArePointsEqual(FRect.BottomRight, Value) then
  begin
