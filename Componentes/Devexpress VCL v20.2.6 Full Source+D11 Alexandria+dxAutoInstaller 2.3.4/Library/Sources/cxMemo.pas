...
uses
  Windows, Messages, Types, Classes, Controls, Forms, Graphics, Menus, StdCtrls,
  SysUtils,
  dxCore, dxCoreClasses, cxClasses, cxLookAndFeelPainters, cxContainer,
  cxControls, cxDataStorage, cxDataUtils,
  cxEdit, cxDrawTextUtils, cxGraphics, cxLookAndFeels, cxTextEdit,
  cxFilterControlUtils, cxGeometry, dxTypeHelpers;
...
type
  ...
  TcxCustomInnerMemo = class(TMemo, IUnknown, IcxContainerInnerControl,
  ...
  protected
    ...
    function GetInnerEditClass: TControlClass; override;
  public
    constructor Create(AEditingController: TcxCustomEditingController);
      reintroduce; virtual;
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    procedure SetEditingController(AController: TcxCustomEditingController);
    // for internal use

    property AutoHeight: TcxInplaceEditAutoHeight read FAutoHeight
      write FAutoHeight;
    ...
    property EditingController: TcxCustomEditingController
...
uses
  Variants, Clipbrd, Math, dxCoreGraphics, cxEditConsts, cxEditUtils,
  cxScrollBar, dxMessages, dxDPIAwareUtils;
...
begin
  ...
  InternalTextOut(ACanvas.Canvas, AViewInfo, AText, AViewInfo.TextRect,
    AViewInfo.DrawTextFlags, AViewInfo.SelStart, AViewInfo.SelLength,
    AViewInfo.SelBackgroundColor, AViewInfo.SelTextColor,
    AViewInfo.MaxLineCount);
end;
...
const
  AAlignmentFlagMap: array [TAlignment] of DWORD = (CXTO_LEFT, CXTO_RIGHT,
...
begin
  ...
  if not(Properties.ScrollBars in [ssHorizontal, ssBoth]) and Properties.WordWrap
  then
...
begin
  ...
  if (ADisplayText <> '') and dxCharInSet(ADisplayText[Length(ADisplayText)],
    dxBreakLineCharacters) then
...
class function TcxCustomMemoProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxCustomMemoViewInfo;
end;

function TcxCustomMemoProperties.GetEditValueSource(AEditFocused: Boolean)
...
function TcxCustomMemoProperties.GetSupportedOperations
...
procedure TcxCustomMemoProperties.DoAssign(AProperties
...
class function TcxCustomMemoProperties.GetViewDataClass
...
begin
  ...
  try
    ...
    if IsInplace then
      TdxVisualRefinements.Padding.Deflate(AInnerContentBounds, ScaleFactor,
        UseRightToLeftAlignment);
    AInnerEditBounds := cxRectInflate(AInnerContentBounds,
      cxContainerMaxBorderWidth);
    if IsPopupScrollBars then
    begin
      if IsScrollBarVisible(sbVertical, AScrollbarInfo) then
        if UseRightToLeftScrollBar then
          AInnerEditBounds.Left := AInnerEditBounds.Left -
            AScrollbarInfo.rcScrollBar.Width
        else
          AInnerEditBounds.Right := AInnerEditBounds.Right +
            AScrollbarInfo.rcScrollBar.Width;
      if IsScrollBarVisible(sbHorizontal, AScrollbarInfo) then
        AInnerEditBounds.Bottom := AInnerEditBounds.Bottom +
          AScrollbarInfo.rcScrollBar.Height;
    end;
    if not EqualRect(InnerEdit.Control.BoundsRect, AInnerEditBounds) then
      InnerEdit.Control.SetBounds(AInnerEditBounds.Left, AInnerEditBounds.Top,
        AInnerEditBounds.Width, AInnerEditBounds.Height);
...
begin
  Result := inherited CanKeyPressModifyEdit(Key) or (Key = #10) or
    CanMemoKeyModifyEdit(Ord(Key), KeyboardStateToShiftState, True);
end;
...
begin
  ...
  if IsInplace and (AKey = VK_RETURN) and
    ((ssCtrl in Shift) and not ActiveProperties.WantReturns or
...
procedure TcxCustomMemo.Scroll(AScrollBarKind: TScrollBarKind;
...
const
  ...
  begin
    Result := MakeWParam(Ord(AScrollCode),
      IfThen((AScrollBarKind = sbHorizontal) or not(AScrollCode in [scLineDown,
      scLineUp]), AScrollPos));
  end;

begin
  SetScrollPos(InnerControl.Handle, AScrollBarIDs[AScrollBarKind],
    AScrollPos, False);
  InnerMemo.CallDefWndProc(AScrollMessages[AScrollBarKind], GetWParam,
    GetScrollBarHandle(AScrollBarKind));

  if AScrollCode <> scTrack then
    AScrollPos := GetScrollPos(InnerControl.Handle,
      AScrollBarIDs[AScrollBarKind]);
  DoLayoutChanged;
  if AScrollCode <> scTrack then
    SetScrollBarsParameters;
end;
...
begin
  ...
  if Focused and (ActiveProperties.MaxLength > 0) and
    (Length(WideString(Copy(Text, 1, SelStart) + Value)) >
    ActiveProperties.MaxLength) then
  begin
    ANewValueLength := ActiveProperties.MaxLength -
      Length(WideString(Copy(Text, 1, SelStart)));
...
begin
  ...
  begin
    ...
    if (ActiveProperties.ScrollBars in [ssHorizontal, ssBoth]) or
      not ActiveProperties.WordWrap then
...
begin
  ...
  Result := (ATranslatedKey = VK_RETURN) and
    (ActiveProperties.WantReturns or (ssCtrl in Shift));
...
begin
  Result := [fcoLike, fcoNotLike, fcoContains, fcoNotContains, fcoBeginsWith,
    fcoEndsWith, fcoBlanks, fcoNonBlanks];
end;
...
begin
  ...
  begin
    dxTestCheck(Edit.FLockBoundsCount > 0,
      'TcxCustomInnerMemoHelper.LockBounds fails');
...
begin
  if not Container.InnerControlDefaultHandler(TMessage(Message)) then
    if (csGlassPaint in ControlState) and
      (TMessage(Message).Msg = CN_CTLCOLOREDIT) then
    begin
...
procedure TcxCustomInnerMemo.DoGetGestureOptions(var Gestures
...
procedure TcxCustomInnerMemo.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TcxCustomInnerMemo.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
begin
  ...
  try
    ...
    // do not handle WM_SETTEXT from inherited CreateWindowHandle
  finally
...
begin
  if Container.IsPopupScrollBars and FScrollUIActivityHelper.CheckScrollActivity
    (Self, Message) then
...
procedure TcxCustomInnerMemo.WMNCPaint(var Message: TWMNCPaint);
  ...
  begin
    ...
    if AScrollBarState and (STATE_SYSTEM_INVISIBLE or STATE_SYSTEM_OFFSCREEN) <> 0
    then
      Exit;
...
begin
  ...
  begin
    ...
    begin
      SetWindowLong(Handle, GWL_STYLE, AFlags and
...
procedure TcxCustomInnerMemo.WMWindowPosChanged(var Message
  : TWMWindowPosChanged);
begin
  Container.SetScrollBarsParameters;
  inherited;
...
procedure TcxAutoHeightInplaceEditViewInfo.DrawParentFocusRect
  (ACanvas: TcxCanvas);
var
...
class function TcxAutoHeightInplaceEditProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxAutoHeightInplaceEdit;
end;

class function TcxAutoHeightInplaceEditProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TcxAutoHeightInplaceEditViewData;
end;

class function TcxAutoHeightInplaceEditProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
...
constructor TcxAutoHeightInplaceEdit.Create(AEditingController
...
class function TcxAutoHeightInplaceEdit.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxAutoHeightInplaceEditProperties;
end;

procedure TcxAutoHeightInplaceEdit.SetEditingController
  (AController: TcxCustomEditingController);
begin
...
begin
  AText := Text;
  ALineHeight := TdxTextMeasurer.TextLineHeight(Style.Font);
  ...
  begin
    ...
    InflateRect(R, -1, 0);
    ATextHeight := TdxTextMeasurer.TextHeightDT(Style.Font, AText, R,
      DT_EDITCONTROL or DT_WORDBREAK or DT_CALCRECT or DT_NOPREFIX);
...
begin
  ...
  begin
    ...
    inherited;
    if (Message.NotifyCode = EN_CHANGE) and ModifiedAfterEnter and not APrevModified
    then
      TcxEditingControllerAccess(EditingController)
        .StartEditAutoHeight(FPrevHeight <> CalculatedHeight);
  end
  else
    inherited;
end;
...
GetRegisteredEditProperties.Register(TcxMemoProperties,
