...
uses
  ...
  ImgList, Menus, ActnList, Dialogs, ComCtrls,
  Generics.Defaults, Generics.Collections,
  cxGraphics, cxControls, cxDesignWindows, cxContainer, cxTreeView,
  dxGaugeControl, dxGaugeCustomScale, cxLookAndFeels,
...
type
  ...
  TfmGaugeControlScalesEditor = class(TcxDesignFormEditor)
    ...
    procedure tvScalesEdited(Sender: TObject; Node: TTreeNode;
      var AName: string);
    procedure tvScalesDragDrop(ASender, ASource: TObject; X, Y: Integer);
    procedure tvScalesDragOver(ASender, ASource: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
    procedure tvScalesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  private
    ...
    // Scales Tree View
    function FindScaleNode(AScale: TdxGaugeCustomScale;
      var ANode: TTreeNode): Boolean;
  ...
  public
    ...
    procedure SelectionsChanged(const ASelection
      : TDesignerSelectionList); override;
  end;
...
uses
  dxCore, cxGeometry, dxComCtrlsUtils, dxGaugeQuantitativeScale,
  dxGaugeCircularScale, dxGaugeDigitalScale, dxGaugeLinearScale;
...
function cxTreeViewNodeObjectInheritsFrom(ANode: TTreeNode;
...
begin
  ...
  if (AScale1 <> nil) and (AScale2 <> nil) then
    Result := dxCompareValues(TdxGaugeCustomScale(GetScaleFromNode(ANode1)).
      Index, TdxGaugeCustomScale(GetScaleFromNode(ANode2)).Index)
  else
    Result := 0;
end;
...
procedure TfmGaugeControlScalesEditor.SelectionsChanged(const ASelection
...
procedure TfmGaugeControlScalesEditor.AddScale(AScaleClass
...
begin
  ...
  try
    ...
    else if IsGaugeControl(tvScales.Selected) then
      ASelection.Add(GaugeControl);
    if (ASelection.Count > 0) and (LockCount = 0) or
      IsGaugeControl(tvScales.Selected) then
...
procedure TfmGaugeControlScalesEditor.CreateRegisteredScaleMenuItems;
  ...
  function CreateMenuItem(const ACaption: string; AMenuIndex: Integer;
  ...
  try
    ...
    for I := 0 to AScaleClasses.Count - 1 do
      ...
      begin
        ...
        miAddScale.Add(CreateMenuItem(AScaleName, 2001 + I,
...
function TfmGaugeControlScalesEditor.FindScaleNode(AScale: TdxGaugeCustomScale;
  var ANode: TTreeNode): Boolean;
begin
...
function TfmGaugeControlScalesEditor.GetController
  : TdxCustomGaugeControlController;
begin
...
begin
  if not FNodeImageIndexes.TryGetValue(TComponent(AScale).ClassType, Result)
  then
    if not FNodeImageIndexes.TryGetValue(TComponent(AScale).ClassParent, Result)
    then
      Result := -1;
end;
...
function TfmGaugeControlScalesEditor.GetScaleIndex
  (AScale: TdxGaugeCustomScale): Integer;
begin
  if AScale = nil then
    Result := -1
  else
...
function TfmGaugeControlScalesEditor.IsContainerScale(ANode: TTreeNode)
  : Boolean;
begin
...
procedure TfmGaugeControlScalesEditor.ListView1GetImageIndex(Sender: TObject;
  Item: TListItem);
begin
  inherited;
...
procedure TfmGaugeControlScalesEditor.AddNode(AComponent: TComponent);
  ...
  begin
    if AComponent is TdxGaugeCustomScale and
      (TdxGaugeCustomScaleAccess(AComponent).Captions.Count > 0) then
      for I := 0 to TdxGaugeCustomScaleAccess(AComponent).Captions.Count - 1 do
      begin
        AChildNode := tvScales.Items.AddChild(ANode,
          TdxGaugeCustomScaleAccess(AComponent).Captions[I].Name);
  ...
  var
    ...
    AChildNode: TTreeNode;
  begin
    if AComponent is TdxGaugeQuantitativeScale and
      (TdxGaugeQuantitativeScaleAccess(AComponent).Ranges.Count > 0) then
      for I := 0 to TdxGaugeQuantitativeScaleAccess(AComponent)
        .Ranges.Count - 1 do
      begin
        AChildNode := tvScales.Items.AddChild(ANode,
          TdxGaugeQuantitativeScaleAccess(AComponent).Ranges[I].Name);
        AChildNode.Data := TdxGaugeQuantitativeScaleAccess(AComponent)
          .Ranges[I];
        AChildNode.Selected := False;
        AChildNode.Focused := False;
        AChildNode.ImageIndex := GetImageIndex(AChildNode);
...
procedure TfmGaugeControlScalesEditor.Drop(ATargetScale: TdxGaugeCustomScale);
  ...
  begin
    ...
    AScale := GaugeControl.AddScale
      (TdxGaugeCustomScaleClass(ABaseScale.ClassType));
    AScale.Assign(ABaseScale);
    TdxGaugeCustomScaleAccess(AScale).AnchorScaleIndex :=
      GetScaleIndex(ATargetScale);
  end;
  ...
  begin
    ...
    begin
      ...
      TdxGaugeCustomScaleAccess(ASelectedScale).AnchorScaleIndex :=
        GetScaleIndex(ATargetScale);
    end;
  end;
...
begin
  ...
  try
    TdxCustomGaugeControlControllerAccess(Controller)
...
procedure TfmGaugeControlScalesEditor.RefreshTreeView(AForceRefresh
  : Boolean = False);
...
procedure TfmGaugeControlScalesEditor.SyncSelection(const ASelection
  : TDesignerSelectionList);
var
  AList: TList;
begin
...
procedure TfmGaugeControlScalesEditor.Changed(ANeedRefreshTreeView
  : Boolean = True);
begin
  BeginUpdate;
  try
...
begin
  TdxCustomGaugeControlControllerAccess(Controller)
    .AddCaption(GetScaleFromNode(tvScales.Selected));
end;

procedure TfmGaugeControlScalesEditor.acAddCircularHalfScaleExecute
  (Sender: TObject);
begin
  AddScale(TdxGaugeCircularHalfScale);
end;

procedure TfmGaugeControlScalesEditor.acAddCircularQuarterLeftScaleExecute
  (Sender: TObject);
begin
  AddScale(TdxGaugeCircularQuarterLeftScale);
end;

procedure TfmGaugeControlScalesEditor.acAddCircularQuarterRightScaleExecute
  (Sender: TObject);
begin
  AddScale(TdxGaugeCircularQuarterRightScale);
end;

procedure TfmGaugeControlScalesEditor.acAddCircularScaleExecute
  (Sender: TObject);
begin
...
begin
  TdxCustomGaugeControlControllerAccess(Controller)
    .AddRange(GetScaleFromNode(tvScales.Selected));
end;

procedure TfmGaugeControlScalesEditor.acSendBackwardExecute(Sender: TObject);
begin
  TdxCustomGaugeControlControllerAccess(Controller).ChangeSelectionsZOrders
    (octSendBackward);
end;
...
begin
  if not((tvScales.SelectionCount = 1) and
    IsGaugeControl(tvScales.Selections[0])) then
  begin
    BeginUpdate;
    TdxCustomGaugeControlControllerAccess(Controller).DeleteSelection;
    EndUpdate(False);
    cxTreeViewSetSelection(tvScales.InnerTreeView,
      TdxCustomGaugeControlControllerAccess(Controller).Selections);
    SetSelections;
  end;
end;
...
procedure TfmGaugeControlScalesEditor.acRestoreStyleParametersExecute
  (Sender: TObject);
var
  I: Integer;
  AScale: TdxGaugeCustomScale;
...
procedure TfmGaugeControlScalesEditor.pmScalesMainPopup(Sender: TObject);
  ...
  begin
    Result := (GetScaleFromNode(tvScales.Selected) is TdxGaugeQuantitativeScale)
      and (tvScales.SelectionCount = 1);
  end;
...
begin
  AIsScaleElement := TdxCustomGaugeControlControllerAccess(Controller)
    .IsScaleElement;
  AIsNotGaugeControlNodeSelected := not IsGaugeControl(tvScales.Selected);
  acDeleteScale.Visible := AIsNotGaugeControlNodeSelected;
  acRestoreStyleParameters.Visible := AIsNotGaugeControlNodeSelected and
    not AIsScaleElement and not IsContainerScale(tvScales.Selected);
  acBringForward.Visible := AIsNotGaugeControlNodeSelected and
    not AIsScaleElement;
  acSendBackward.Visible := AIsNotGaugeControlNodeSelected and
    not AIsScaleElement;
  acBringToFront.Visible := AIsNotGaugeControlNodeSelected and
    not AIsScaleElement;
  acSendToBack.Visible := AIsNotGaugeControlNodeSelected and
    not AIsScaleElement;
  miAddScale.Visible := not AIsScaleElement;
  acAddContainer.Visible := not AIsScaleElement;
  acAddCaption.Visible := AIsNotGaugeControlNodeSelected and
    not IsContainerScale(tvScales.Selected) and not AIsScaleElement;
  acAddRange.Visible := CanAddRange and not AIsScaleElement;
end;

procedure TfmGaugeControlScalesEditor.tvScalesChange(Sender: TObject;
  Node: TTreeNode);
begin
  if tvScales.IsFocused and not(IsGaugeControl(Node) or
    TdxCustomGaugeControlAccess(GaugeControl).Locked) then
    SetSelections;
end;

procedure TfmGaugeControlScalesEditor.tvScalesEdited(Sender: TObject;
  Node: TTreeNode; var AName: string);
begin
...
procedure TfmGaugeControlScalesEditor.tvScalesDragDrop(ASender,
  ASource: TObject; X, Y: Integer);
begin
...
procedure TfmGaugeControlScalesEditor.tvScalesDragOver(ASender,
  ASource: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
...
begin
  Accept := not IsGaugeControlNodeDragging and
    (ASender = cxExtractDragObjectSource(ASource));
  if Accept then
    ...
    begin
      ...
      Accept := TdxCustomGaugeControlControllerAccess(Controller)
        .CanAnchorScale(AScaleIndex, AAnchorScaleIndex);
    end;
end;

procedure TfmGaugeControlScalesEditor.tvScalesKeyDown(Sender: TObject;
