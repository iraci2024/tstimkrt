...
uses
  Types, Windows, Classes, Graphics, dxGDIPlusClasses, dxCore, dxCoreGraphics,
  dxCoreClasses,
...
type
  ...
  public
    class function Build(AText
      : TdxSpreadSheetTableViewOptionsPrintHeaderFooterText): string;
    class procedure Parse(ATarget
  ...
  public
    class procedure IncreaseOutlineLevelTo(AItems: TdxSpreadSheetTableItems;
      AIndex, ALevel: Integer); overload;
    class procedure IncreaseOutlineLevelTo(AItems: TdxSpreadSheetTableItems;
      AStartIndex, AFinishIndex, ALevel: Integer); overload;
  end;
  ...
  public
    class function BuildPrintTitlesReference
  ...
  TdxSpreadSheetHashTableItemList = class(TList)
  protected
    procedure Notify(Ptr: Pointer; Action: TListNotification); override;
  end;

procedure dxSpreadSheetInitializeBrushPattern(const ABrush: TdxGPBrush;
  const APattern: TdxGPImage; const AForegroundColor, ABackgroundColor
  : TdxAlphaColor; AIsTexture: Boolean = True);
procedure dxSpreadSheetLoadBrushPattern(const ABrush: TdxGPBrush;
  const AInstance: THandle; const AResName: UnicodeString;
  const AForegroundColor, ABackgroundColor: TdxAlphaColor);

implementation

uses
  Math, SysUtils, dxSpreadSheetUtils, cxGraphics, cxGeometry, dxTypeHelpers,
  dxDPIAwareUtils, dxStringHelper,
...
procedure InitializeBrushPattern(const ABrush: TdxGPBrush;
  const AWidth, AHeight: Integer; const AColors: TRGBColors;
  const AForegroundColor, ABackgroundColor: TRGBQuad);
var
  I: Integer;
begin
...
begin
  ...
  try
    ...
    InitializeBrushPattern(ABrush, ABitmap.Width, ABitmap.Height, AColors,
      dxAlphaColorToRGBQuad(AForegroundColor),
      dxAlphaColorToRGBQuad(ABackgroundColor));
  finally
    ABitmap.Free;
  end;
end;
...
begin
  ...
  if not AIsTexture then
    ...
    InitializeBrushPattern(ABrush, APattern.Width, APattern.Height,
      APattern.GetBitmapBits, dxAlphaColorToRGBQuad(AForegroundColor),
      dxAlphaColorToRGBQuad(ABackgroundColor));
end;

{ TdxValueUnitsHelper }

class function TdxValueUnitsHelper.InchesToPixels(const AValue
  : Integer): Integer;
begin
  Result := InchesToPixelsF(AValue);
end;

class function TdxValueUnitsHelper.InchesToPixelsF(const AValue
  : Double): Integer;
begin
  Result := Round(AValue * dxDefaultDPI);
end;

class function TdxValueUnitsHelper.PixelsToInches(const AValue
  : Integer): Integer;
begin
  Result := Round(PixelsToInchesF(AValue));
end;

class function TdxValueUnitsHelper.PixelsToInchesF(const AValue
  : Double): Double;
begin
  Result := AValue / dxDefaultDPI
end;

class function TdxValueUnitsHelper.PixelsToMillimeters
  (const AValue: Integer): Integer;
begin
  Result := Round(PixelsToMillimetersF(AValue));
end;

class function TdxValueUnitsHelper.PixelsToMillimetersF
...
class function TdxValueUnitsHelper.PointsToPixels(const AValue: Double)
  : Integer;
begin
...
class function TdxSpreadSheetColorHelper.ApplyTint(AColor: TColor;
  const ATintValue: Double): TColor;
var
  AColorHSL: TdxHSL;
begin
  Result := AColor;
...
begin
  ...
  begin
    if APosC > 0 then
      APos := APosC
    else if APosR > 0 then
      APos := APosR
    else
      APos := Length(ASource) + 1;

    ATarget.LeftSection :=
      Copy(ASource, APosL + Length(sdxHeaderFooterLeftSectionMacro),
  ...
  begin
    ...
    ATarget.CenterSection :=
      Copy(ASource, APosC + Length(sdxHeaderFooterCenterSectionMacro),
      APos - (APosC + Length(sdxHeaderFooterCenterSectionMacro)));
  end;

  if APosR > 0 then
    ATarget.RightSection :=
      Copy(ASource, APosR + Length(sdxHeaderFooterRightSectionMacro), MaxInt);
end;
...
class procedure TdxSpreadSheetOutlineHelper.IncreaseOutlineLevelTo
  (AItems: TdxSpreadSheetTableItems; AStartIndex, AFinishIndex,
  ALevel: Integer);
var
  I: Integer;
begin
...
class function TdxSpreadSheetPrintAreasHelper.BuildPrintTitlesReference
  (AView: TdxSpreadSheetTableView): string;

  function EncodePrintableArea(const R: TRect): string;
  begin
    Result := dxReferenceToString(R,
      AView.SpreadSheet.OptionsView.R1C1Reference, [croSheetName],
      AView.Caption);
  end;
...
begin
  ...
  try
    if AView.OptionsPrint.Source.ColumnsToRepeat.Assigned then
      ABuffer.Append(EncodePrintableArea
        (AView.OptionsPrint.Source.ColumnsToRepeat.Rect));
    if ABuffer.Length > 0 then
      ABuffer.Append(',');
    if AView.OptionsPrint.Source.RowsToRepeat.Assigned then
      ABuffer.Append(EncodePrintableArea
        (AView.OptionsPrint.Source.RowsToRepeat.Rect));
...
class procedure TdxSpreadSheetPrintAreasHelper.ImportPrintArea
...
begin
  try
    ARanges := TdxSpreadSheetDefinedNameHelper.GetUsedAreas
      (ADefinedName, AView);
    try
      if ARanges.Count > 0 then
      begin
        if AView is TdxSpreadSheetTableView then
          TdxSpreadSheetTableView(AView).OptionsPrint.Source.Area.Rect :=
            ARanges.Union;
      end;
    finally
      ARanges.Free;
...
class procedure TdxSpreadSheetPrintAreasHelper.ImportPrintTitles
  (ADefinedName: TdxSpreadSheetDefinedName);
var
  ARange: TRect;
  ARanges: TdxRectList;
      ...
      begin
        ...
        begin
          ...
          if dxSpreadSheetIsEntireColumn(ARange) then
            TdxSpreadSheetTableView(AView).OptionsPrint.Source.ColumnsToRepeat.
              Rect := ARange;
          if dxSpreadSheetIsEntireRow(ARange) then
            TdxSpreadSheetTableView(AView).OptionsPrint.Source.RowsToRepeat.Rect
              := ARange;
        end;
      end;
    finally
...
procedure TdxSpreadSheetHashTableItemList.Notify(Ptr: Pointer;
