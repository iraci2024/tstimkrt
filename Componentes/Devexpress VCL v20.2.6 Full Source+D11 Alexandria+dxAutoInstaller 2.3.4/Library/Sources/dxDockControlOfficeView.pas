...
function TdxDockControlOfficePainter.GetCaptionFontColor
...
function TdxDockControlOfficePainter.GetCaptionSignColor(IsActive: Boolean;
  AState: TcxButtonState): TColor;
begin
  Result := GetCaptionFontColor(IsActive and
    not(AState in [cxbsHot, cxbsPressed]));
end;

class function TdxDockControlOfficePainter.HasLookAndFeelStyle
...
procedure TdxDockControlOfficePainter.DrawBorder(ACanvas: TcxCanvas;
  ARect: TRect);
var
  ABorderWidths: TRect;
begin
  ...
  begin
    ...
    ACanvas.FillRect(cxRectSetHeight(ARect, ABorderWidths.Top),
      dxOffice11BarFloatingBorderColor2);
    ACanvas.FillRect(cxRectSetBottom(ARect, ARect.Bottom, ABorderWidths.Bottom),
      dxOffice11BarFloatingBorderColor1);
    FillGradientRect(ACanvas.Handle, Rect(Left, Top, Left + ABorderWidths.Left,
      Bottom), dxOffice11BarFloatingBorderColor2,
      dxOffice11BarFloatingBorderColor1, False);
    FillGradientRect(ACanvas.Handle, Rect(Right - ABorderWidths.Right, Top,
      Left + Right, Bottom), dxOffice11BarFloatingBorderColor2,
      dxOffice11BarFloatingBorderColor1, False);
  end;
  if DockControl.AutoHide then
    DrawColorEdge(ACanvas, ARect, dxOffice11BarFloatingBorderColor1, etStandard,
      [epRect]);
end;

procedure TdxDockControlOfficePainter.DrawCaptionButton(ACanvas: TcxCanvas;
  ARect: TRect; AIsActive: Boolean; AState: TcxButtonState);
begin
  case AState of
    cxbsPressed:
      begin
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11SelectedDownColor1,
          dxOffice11SelectedDownColor2, False);
        Office11FrameSelectedRect(ACanvas.Handle, ARect);
      end;

    cxbsHot:
      begin
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11SelectedColor1,
          dxOffice11SelectedColor2, False);
        Office11FrameSelectedRect(ACanvas.Handle, ARect);
      end;
  end;
end;
...
begin
  ...
  APoints[2] := Point(ARect.Left + cxRectWidth(ARect) div 2,
    ARect.Top + cxRectHeight(ARect) div 2);

  ACanvas.Polygon(APoints);
end;

procedure TdxDockControlOfficePainter.DrawClient(ACanvas: TcxCanvas;
  ARect: TRect);
begin
  FillGradientRect(ACanvas.Handle, ARect, dxOffice11ToolbarsColor1,
    dxOffice11ToolbarsColor2, False);
end;

procedure TdxDockControlOfficePainter.DrawHideBar(ACanvas: TcxCanvas;
  ARect: TRect; APosition: TdxAutoHidePosition);
begin
  case APosition of
    ahpLeft:
      begin
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11ToolbarsColor2,
          dxOffice11ToolbarsColor2, True);
        ARect.Right := ARect.Left + GetHideBarVertInterval;
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11DockColor1,
          dxOffice11DockColor1, True);
      end;

    ahpTop:
      begin
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11ToolbarsColor2,
          dxOffice11ToolbarsColor1, True);
        ARect.Bottom := ARect.Top + GetHideBarVertInterval;
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11DockColor1,
          dxOffice11DockColor2, True);
      end;
    ...
    ahpRight:
      begin
        ...
        FillGradientRect(ACanvas.Handle, ARect, dxOffice11DockColor2,
          dxOffice11DockColor2, True);
      end;
...
procedure TdxDockControlOfficePainter.DrawHideBarButton(ACanvas: TcxCanvas;
  AButton: TdxDockSiteHideBarButton; APosition: TdxAutoHidePosition);
const
  Edges: array [TdxAutoHidePosition] of TdxEdgePositions =
    ([epTop, epBottomRight], [epLeft, epBottomRight], [epTopLeft, epBottom],
    [epTopLeft, epRight], []);
begin
  inherited DrawHideBarButton(ACanvas, AButton, APosition);
  DrawColorEdge(ACanvas, AButton.Bounds, dxOffice11SelectedBorderColor,
    etStandard, Edges[APosition]);
end;
...
procedure TdxDockControlOfficePainter.DrawHideBarButtonSection
    ...
    case APosition of
      ahpLeft, ahpTop:
        FillGradientRect(ACanvas.Handle, AButtonSection.Bounds,
          dxOffice11DownedSelectedColor, dxOffice11DownedColor,
          APosition = ahpLeft);
      ahpRight, ahpBottom:
        FillGradientRect(ACanvas.Handle, AButtonSection.Bounds,
          dxOffice11DownedColor, dxOffice11DownedSelectedColor,
          APosition = ahpRight);
    end;
  end;
...
procedure TdxDockControlOfficePainter.DrawHideBarScrollButton
  (ACanvas: TcxCanvas; const ARect: TRect; AState: TcxButtonState;
  AArrow: TcxArrowDirection);
begin
  cxLookAndFeelPaintersManager.GetPainter(lfsOffice11).DrawScaledArrow(ACanvas,
