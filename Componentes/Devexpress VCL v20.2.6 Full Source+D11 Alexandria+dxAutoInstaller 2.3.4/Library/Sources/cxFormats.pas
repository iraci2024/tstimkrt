...
type
  ...
  TcxDateTimeFormatItemKind = (dtikString, dtikYear, // YY YYYY
  ...
  private
    ...
    procedure SetDefinedItems(AKind: TcxDateTimeFormatItemKind;
      AValue: Boolean);
  ...
  public
    ...
    property DefinedItems[AKind: TcxDateTimeFormatItemKind]: Boolean
      read GetDefinedItems write SetDefinedItems;
  ...
  private
    ...
    function GetDateTimeDisplayFormat(AMaskKind
      : TcxDateTimeEditMaskKind): string;
    ...
    function InternalGetDateTimeEditRegExprMask(AFormatInfo
      : TcxDateTimeFormatInfo; AMaskKind: TcxDateTimeEditMaskKind): string;
    function InternalGetDateTimeEditStandardMask(AFormatInfo
      : TcxDateTimeFormatInfo; AMaskKind: TcxDateTimeEditMaskKind): string;
    function InternalGetMaskedDateEditFormat(AFormatInfo
      : TcxDateTimeFormatInfo): string;
  ...
  public
    ...
    class function GetDateTimeFormatItemStandardMaskInfo(const AFormatInfo
      : TcxDateTimeFormatInfo; APos: Integer;
      out AItemInfo: TcxDateTimeFormatItemInfo): Boolean;
    function GetDateTimeStandardMaskStringLength(const AFormatInfo
      : TcxDateTimeFormatInfo): Integer;
    ...
    property AssignedCurrencyFormat: Boolean read FAssignedCurrencyFormat
      write SetAssignedCurrencyFormat;
    property AssignedRegExprDateEditMask: Boolean
      read FAssignedRegExprDateEditMask write SetAssignedRegExprDateEditMask;
    property AssignedRegExprDateTimeEditMask: Boolean
      read FAssignedRegExprDateTimeEditMask
      write SetAssignedRegExprDateTimeEditMask;
    property AssignedStandardDateEditMask: Boolean
      read FAssignedStandardDateEditMask write SetAssignedStandardDateEditMask;
    property AssignedStandardDateTimeEditMask: Boolean
      read FAssignedStandardDateTimeEditMask
      write SetAssignedStandardDateTimeEditMask;
    property AssignedStartOfWeek: Boolean read GetAssignedStartOfWeek
      write SetAssignedStartOfWeek;
    ...
    property DateEditShowZeroTime: Boolean read FDateEditShowZeroTime
      write SetDateEditShowZeroTime;
    ...
    property FirstWeekOfYear: TcxFirstWeekOfYear read FFirstWeekOfYear
      write SetFirstWeekOfYear;
    property IsStandardMaskValid: Boolean read FIsStandardMaskValid;
    // for internal use
    ...
    property RegExprDateEditMask: string read FRegExprDateEditMask
      write SetRegExprDateEditMask;
    property RegExprDateTimeEditMask: string read FRegExprDateTimeEditMask
      write SetRegExprDateTimeEditMask;
    property StandardDateEditMask: string read FStandardDateEditMask
      write SetStandardDateEditMask;
    property StandardDateTimeEditMask: string read FStandardDateTimeEditMask
      write SetStandardDateTimeEditMask;
    ...
    // for internal use
    property UseDelphiDateTimeFormats: Boolean read FUseDelphiDateTimeFormats
      write SetUseDelphiDateTimeFormats;
...
function GetLongTimeFormat: string;
  ...
  function GetCount(const S: string; var AStartIndex: Integer;
...
var
  ...
  begin
    Result := 1;
    while (AStartIndex <= Length(AFormat)) and
      (AFormat[AStartIndex] = AStartSymbol) do
    begin
      Inc(AStartIndex);
      Inc(Result);
    end;
  ...
  function ParseFormat(const AFormat: string; ARecursionDepth: Integer)
    : Boolean;
  var
      ...
      case AToken of
        ...
        'D':
          begin
            ...
            case ACount of
              ...
              5:
                Result := ParseFormat(dxFormatSettings.ShortDateFormat,
                  ARecursionDepth);
            else
              Result := ParseFormat(dxFormatSettings.LongDateFormat,
                ARecursionDepth);
            end;
            if not Result then
              Break;
        ...
        'T':
          begin
            ACount := GetCount(AFormat, I, AStartSymbol);
            if ACount = 1 then
              Result := ParseFormat(dxFormatSettings.ShortTimeFormat,
                ARecursionDepth)
            else
  ...
  begin
    ...
    else if AFormatInfo.DefinedItems[dtikTimeSuffix] then
      for I := 0 to AFormatInfo.ItemCount - 1 do
...
procedure TcxDateTimeFormatInfo.AddItem(AKind: TcxDateTimeFormatItemKind;
  AData: string);
var
...
begin
  ClearItems;
  for AFormatItemKind := Low(TcxDateTimeFormatItemKind)
    to High(TcxDateTimeFormatItemKind) do
...
function TcxDateTimeFormatInfo.GetDefinedItems
  (AKind: TcxDateTimeFormatItemKind): Boolean;
begin
...
procedure TcxDateTimeFormatInfo.SetDefinedItems
  (AKind: TcxDateTimeFormatItemKind; AValue: Boolean);
begin
...
procedure TcxFormatController.CalculateDateEditMasks(AUseSmartInputWhenRegExpr
  : Boolean);
...
begin
  ...
  begin
    FRegExprDateEditMask := InternalGetDateTimeEditRegExprMask(FDateFormatInfo,
      dtmkDate);
  ...
  FMaskedDateTimeEditFormat := InternalGetMaskedDateEditFormat
    (FDateTimeFormatInfo);
end;
...
function TcxFormatController.GetCurrencyFormat: string;
  ...
  begin
    case dxFormatSettings.CurrencyFormat of
      0:
        Result := ACurrStr + AFormat; { '$1' }
      1:
        Result := AFormat + ACurrStr; { '1$' }
      2:
        Result := ACurrStr + ' ' + AFormat; { '$ 1' }
      3:
        Result := AFormat + ' ' + ACurrStr; { '1 $' }
...
function TcxFormatController.InternalGetDateTimeEditRegExprMask
  ...
  begin
    ...
    ASystemTime.wDay := 1;
    if GetDateFormat(GetThreadLocale, 0, @ASystemTime, 'ddMMMM', ABuf, 100) <> 0
    then
    begin
      Result := ABuf;
      Delete(Result, 1, 2);
  ...
  begin
    ...
    if CompareText(AGenitiveForm, dxFormatSettings.LongMonthNames[AIndex]) <> 0
    then
    begin
      ADst := ADst + '|';
        ...
        dtikTimeSuffix:
          begin
            if UpperCase(Data) = 'A/P' then
              ...
              S := S + '(AM|PM)?'
            else if (dxFormatSettings.TimeAMString <> '') or
              (dxFormatSettings.TimePMString <> '') then
            begin
              S := S + '(''';
              if (dxFormatSettings.TimeAMString <> '') and
                (dxFormatSettings.TimePMString <> '') then
                S := S + dxFormatSettings.TimeAMString + '''|''' +
                  dxFormatSettings.TimePMString
              else if dxFormatSettings.TimeAMString <> '' then
                S := S + dxFormatSettings.TimeAMString
              else
...
begin
  ...
  if AFormatInfo.ItemCount = 0 then
    Exit;
  for I := 0 to AFormatInfo.ItemCount - 1 do
    if (AFormatInfo.Items[I].Kind in [dtikString, dtikYear, dtikMonth, dtikDay,
      dtikDateSeparator]) and (AMaskKind in [dtmkDate, dtmkDateTime]) then
      ProcessDateItem(Result, AFormatInfo.Items[I])
    else if (AFormatInfo.Items[I].Kind in [dtikString, dtikHour, dtikMin,
      dtikSec, dtikMSec, dtikTimeSuffix, dtikTimeSeparator]) and
    ...
    case AItem.Kind of
      ...
      dtikTimeSuffix:
        begin
          ...
          begin
            J := Max(Length(dxFormatSettings.TimePMString),
              Length(dxFormatSettings.TimeAMString));
...
function TcxFormatController.RegExprCustomDateEditMask(const AFormatString
  : string): string;
const
  AMaskKindMap: array [Boolean] of TcxDateTimeEditMaskKind = (dtmkDate,
    dtmkDateTime);
var
  AFormatInfo: TcxDateTimeFormatInfo;
begin
  ...
  try
    ...
    Result := InternalGetDateTimeEditRegExprMask(AFormatInfo,
      AMaskKindMap[AFormatInfo.DefinedItems[dtikHour]]);
  finally
...
begin
  ...
  begin
    ...
    Inc(AItemZoneStart, GetDateTimeFormatItemStandardMaskZoneLength
      (AFormatInfo.Items[I]));
    ...
    begin
      ...
      if AItemInfo.Kind = dtikTimeSuffix then
        AItemInfo.TimeSuffixKind :=
          GetTimeSuffixKind(AFormatInfo.Items[I].Data);
      Result := True;
      Break;
    end;
  end;
...
begin
  Result := 0;
  for I := 0 to AFormatInfo.ItemCount - 1 do
    Inc(Result, GetDateTimeFormatItemStandardMaskZoneLength
      (AFormatInfo.Items[I]));
end;
...
procedure TcxFormatController.SetAssignedRegExprDateTimeEditMask
  (Value: Boolean);
begin
...
procedure TcxFormatController.SetAssignedStandardDateTimeEditMask
  (Value: Boolean);
begin
...
procedure TdxFormatSettings.SetLongDayNames(AIndex: Integer;
  const Value: string);
begin
{$IFDEF DELPHI15}
...
procedure TdxFormatSettings.SetLongMonthNames(AIndex: Integer;
  const Value: string);
begin
{$IFDEF DELPHI15}
...
procedure TdxFormatSettings.SetShortDayNames(AIndex: Integer;
  const AValue: string);
begin
{$IFDEF DELPHI15}
...
procedure TdxFormatSettings.SetShortMonthNames(AIndex: Integer;
  const AValue: string);
begin
{$IFDEF DELPHI15}
