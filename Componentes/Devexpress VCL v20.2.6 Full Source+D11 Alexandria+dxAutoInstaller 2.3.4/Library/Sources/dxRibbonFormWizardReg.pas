...
uses
  ...
  dxBar, dxRibbon, dxRibbonReg, dxCoreReg, dxRibbonForm, dxRibbonFormWizard,
  dxDesignHelpers, dxRibbonSkins;
...
const
  ...
  sdxRibbonApplicationWizardName =
    'DevExpress VCL %version% Ribbon Based Application';
...
type
  ...
  TdxOTARibbonApplicationWizard = class(TdxOTACustomRibbonWizard,
    IOTAProjectWizard)
  protected
  ...
  protected
    function ExpandTemplate(const ATemplate, AModuleIdent, AFormIdent,
      AAncestorIdent: string): string; override;
    function LoadTemplate(const AName: string): string;
    procedure LoadTemplates(out AFormTemplate, AImplTemplate,
      AIntfTemplate: string);
  public
    constructor Create(ARibbonStyle: TdxRibbonStyle;
...
begin
  Result := StringReplace(AText, '%version%', dxGetShortBuildNumberAsString,
    [rfReplaceAll]);
end;
...
const
  PersonalitiesMap: array [TdxOTAPersonality] of string = ('Delphi',
    'CBuilder');
begin
...
begin
  ...
  begin
    ...
    if ExecuteSelectRibbonStyleDialog(ARibbonStyle, GetName,
      sdxRibbonApplicationSelectStyleText) then
    begin
      AModuleServices.CreateModule
        (TdxOTAApplicationCreator.Create(Personality));
      AModuleServices.CreateModule(TdxOTARibbonFormCreator.Create(ARibbonStyle,
        Personality));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if ExecuteSelectRibbonStyleDialog(ARibbonStyle, GetName,
      sdxRibbonFormSelectStyleText) then
      AModuleServices.CreateModule(TdxOTARibbonFormCreator.Create(ARibbonStyle,
        Personality));
  end;
end;
...
constructor TdxOTARibbonFormCreator.Create(ARibbonStyle: TdxRibbonStyle;
  APersonality: TdxOTAPersonality);
var
...
function TdxOTARibbonFormCreator.ExpandTemplate(const ATemplate, AModuleIdent,
  AFormIdent, AAncestorIdent: string): string;
const
  SkinNameMap: array [TdxRibbonStyle] of string = ('Office2007Blue',
    'Office2010Blue', 'Office2013White', 'Office2013LightGray',
    'Office2013LightGray', 'Office2019Colorful');
begin
  Result := inherited ExpandTemplate(ATemplate, AModuleIdent, AFormIdent,
    AAncestorIdent);
  Result := StringReplace(Result, '%SkinName%', '''' + SkinNameMap[RibbonStyle]
    + '''', [rfReplaceAll]);
end;
...
begin
  ...
  try
    AStream := TResourceStream.Create(HInstance, AName,
...
procedure TdxOTARibbonFormCreator.LoadTemplates(out AFormTemplate,
  AImplTemplate, AIntfTemplate: string);
begin
  AFormTemplate := LoadTemplate('RIBBON' + dxRibbonStyleNamePrefixMap
    [RibbonStyle] + 'FORM');
  if Personality = dxopCBuilder then
  begin
    AIntfTemplate := LoadTemplate('RIBBON' + dxRibbonStyleNamePrefixMap
      [RibbonStyle] + 'CBUILDERHEADER');
    AImplTemplate := LoadTemplate('RIBBON' + dxRibbonStyleNamePrefixMap
      [RibbonStyle] + 'CBUILDERUNIT');
  end
  else
  begin
    AImplTemplate := LoadTemplate('RIBBON' + dxRibbonStyleNamePrefixMap
      [RibbonStyle] + 'DELPHIUNIT');
    AIntfTemplate := '';
  end;
end;
