...
uses
  Math, Classes, SysUtils, Generics.Defaults, Generics.Collections, dxPDFBase,
  dxJPX, dxGenerics;
...
type
  ...
  TdxJBIG2Corner = (jbcBottomLeft = 0, jbcTopLeft = 1, jbcBottomRight = 2,
    jbcTopRight = 3);
  ...
  IdxJBIG2Image = interface
    ...
    procedure CompositeGeneral(AImage: IdxJBIG2Image;
      X, Y, AComposeOperator: Integer);
    ...
    property GlobalSegments: TDictionary<Integer, TdxJBIG2SegmentHeader>
      read GetGlobalSegments;
    property Height: Integer read GetHeight;
    property Segments: TDictionary<Integer, TdxJBIG2SegmentHeader>
      read GetSegments;
  ...
  strict private
    ...
    procedure InitializeReferredToSegments(AStream: TStream;
  ...
  private
    ...
    function CreateComposeOperatorByte(AValue: Integer; A, B: Byte)
      : Byte; inline;
    function CreateComposeOperator(AValue: Integer; A, B: Boolean)
      : Boolean; inline;
    ...
    class function CalculateGlobalSegments(const AData: TBytes)
      : TObjectList<TdxJBIG2SegmentHeader>;
  public
  ...
  public
    ...
    constructor Create(AStream: TStream; AInfo: TdxJBIG2HeaderInfo;
      AImage: IdxJBIG2Image); overload; virtual;
    destructor Destroy; override;

    class function CreateData(AStream: TStream; AInfo: TdxJBIG2HeaderInfo;
      AImage: IdxJBIG2Image): TdxJBIG2SegmentData;
  ...
  strict private
    ...
    FGBAT1: Integer;
  public
    constructor Create(AImage: IdxJBIG2Image; AGbat0, AGbat1: Integer);
      reintroduce;
    procedure Decode(ADecoder: TdxJBIG2Decoder); override;
  end;
  ...
  protected
    function CreateAdaptiveTemplates(AGBAdaptiveTemplate: TdxIntegerList)
  ...
  public
    constructor Create(AImage: IdxJBIG2Image;
      AGBAdaptiveTemplate: TdxIntegerList); overload;
    destructor Destroy; override;

    class function CreateDecoder(AImage: IdxJBIG2Image;
      AGBAdaptiveTemplate: TdxIntegerList; AGBTemplate: Integer)
      : TdxJBIG2TPGDONDecoder; overload;
  ...
  public
    constructor Create(AStream: TStream; AInfo: TdxJBIG2HeaderInfo;
      AImage: IdxJBIG2Image); overload; override;

    constructor Create(AInfo: TdxJBIG2HeaderInfo; ADecoder: TdxJBIG2Decoder;
      ASymbolInstanceCount, ARefinementTemplate: Integer;
      AAt, ARAt: TdxIntegerList; AGlyphs: TList<IdxJBIG2Image>;
      AImage: IdxJBIG2Image); overload;
    destructor Destroy; override;

    procedure Process; override;

    function RefinementDecode(ADecoder: TdxJBIG2Decoder; AImage: IdxJBIG2Image;
  ...
  public
    constructor Create(AGlyph: IdxJBIG2Image; ADecoder: TdxJBIG2Decoder;
      ADx, ADy, ATemplate: Integer; AAt: TdxIntegerList;
      AImage: IdxJBIG2Image); overload;
    destructor Destroy; override;
  ...
  protected
    ...
    property RegionInfo: TdxJBIG2RegionSegmentInfo read FRegionInfo;
    property UseTypicalPredictionDecoding: Boolean
      read FUseTypicalPredictionDecoding write FUseTypicalPredictionDecoding;
  public
    constructor Create(AStream: TStream; AInfo: TdxJBIG2HeaderInfo;
      AImage: IdxJBIG2Image); overload; override;
    ...
    function CreateDecoder(AImage: IdxJBIG2Image;
      AAdaptiveTemplates: TdxIntegerList): TdxJBIG2GenericRegionDecoder;
  end;
  ...
  protected
    ...
    property RefinementAdaptiveTemplates: TdxIntegerList
      read FRefinementAdaptiveTemplates;
  ...
  public
    ...
    function Decode(AState: TdxArithmeticState)
      : TdxJBIG2ArithmeticDecoderResult;
  end;
  ...
  public
    constructor Create(AHelper: TdxJBIG2StreamReader; AMaxId: Integer);
      overload;
    constructor Create(AHelper: TdxJBIG2StreamReader;
      AIdLength, AGbLength, AGrLength: Integer); overload;
    destructor Destroy; override;
...
function TdxJBIG2StreamReader.CreateAdaptiveTemplates(ALength: Integer)
  : TdxIntegerList;
var
  I: Integer;
begin
...
constructor TdxJBIG2SegmentHeader.Create(AStream: TStream;
...
procedure TdxJBIG2SegmentHeader.InitializeReferredToSegments(AStream: TStream;
  AHelper: TdxJBIG2StreamReader);
var
  ASlant: Byte;
...
class function TdxJBIG2Image.CalculateGlobalSegments(const AData: TBytes)
  : TObjectList<TdxJBIG2SegmentHeader>;

  procedure ReadHeader(AStream: TBytesStream; AImage: IdxJBIG2Image;
    AHeaders: TObjectList<TdxJBIG2SegmentHeader>);
  var
  ...
  begin
    ...
    try
      ...
      until not(not Result.Last.IsEOF and
    ...
    try
      ...
      try
        try
          ...
          until not(not AHeader.IsEOF and
            (AStream.Position <= AStream.Size - 1));
        finally
          AStream.Free;
        end;
...
function TdxJBIG2Image.CreateComposeOperatorByte(AValue: Integer;
  A, B: Byte): Byte;
begin
  case AValue of
    0:
...
function TdxJBIG2Image.CreateComposeOperator(AValue: Integer;
  A, B: Boolean): Boolean;
begin
  case AValue of
    0:
...
function TdxJBIG2Image.GetGlobalSegments
  : TDictionary<Integer, TdxJBIG2SegmentHeader>;
begin
...
procedure TdxJBIG2Image.Composite(AImage: IdxJBIG2Image;
  X, Y, AComposeOperator: Integer);
begin
  if (X = 0) and (Y = 0) and (AImage.Width = FWidth) and
    (AImage.Height = FHeight) then
    CompositeFast(AImage, AComposeOperator)
  else if AComposeOperator <> 0 then
    CompositeGeneral(AImage, X, Y, AComposeOperator)
  else
    CompositeOrFast(AImage, X, Y);
end;

procedure TdxJBIG2Image.CompositeFast(AImage: IdxJBIG2Image;
  AComposeOperator: Integer);
var
  I: Integer;
begin
  for I := 0 to Length(FData) - 1 do
    FData[I] := CreateComposeOperatorByte(AComposeOperator, FData[I],
      AImage.Data[I]);
end;

procedure TdxJBIG2Image.CompositeOrFast(AImage: IdxJBIG2Image; X, Y: Integer);
var
  AMask, ARightMask, W, H, ASs, S, ALeftByte, ARightByte, AShift, Add, D, J,
    I: Integer;
  AOverlap: Boolean;
begin
  W := AImage.Width;
  ...
  D := Add;
  if (D < 0) or (ALeftByte > FStride) or (H * FStride < 0) or
    (D - ALeftByte + H * FStride > FHeight * FStride) then
    TdxPDFUtils.RaiseTestException;
  ...
  begin
    ...
    ARightMask := IfThen(AOverlap, ($100 - ($100 shr ((X + W) and 7)))
      shr (8 - AShift), $100 - ($100 shr (W and 7)));
    for J := 0 to H - 1 do
    begin
      ...
      begin
        FData[D] := FData[D] or
          Byte(((AImage.Data[S] and not AMask) shl (8 - AShift)));
        FData[D] := FData[D] or
          Byte(((AImage.Data[S + 1] and AMask) shr AShift));
        Inc(S);
        Inc(D);
      end;
      if AOverlap then
        FData[D] := FData[D] or
          Byte(((AImage.Data[S] and ARightMask) shl (8 - AShift)))
      else
        FData[D] := FData[D] or
          (Byte(((((AImage.Data[S] and not AMask) shl (8 - AShift))) or
          ((AImage.Data[S + 1] and ARightMask) shr AShift))));
      Inc(Add, FStride);
      D := Add;
      Inc(ASs, AImage.Stride);
      S := ASs;
...
procedure TdxJBIG2Image.CompositeGeneral(AImage: IdxJBIG2Image;
  X, Y, AComposeOperator: Integer);
var
...
begin
  ...
  for J := 0 to ASh - 1 do
    for I := 0 to ASw - 1 do
      SetPixel(I + X, J + Y, CreateComposeOperator(AComposeOperator,
        AImage.GetPixel(I + ASx, J + ASy), GetPixel(I + X, J + Y)));
end;
...
constructor TdxJBIG2SymbolDictionary.Create(AStream: TStream;
  AInfo: TdxJBIG2HeaderInfo; AImage: IdxJBIG2Image);
var
  ASdatLength: Integer;
begin
  ...
  if (FFlags and $000C <> 0) or (FFlags and $0030 <> 0) or
    (FFlags and $0080 <> 0) or (FFlags and $0100 <> 0) then
    TdxPDFUtils.RaiseTestException;
...
var
  ALimit, ASymwidth, AHcheight, AHcdh, ADw, ARefaggninst, AId, ARdx, ARdy, AIi,
    AZeroLength, AExRunLength, K: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ATextRegion := TdxJBIG2TextRegion.Create(HeaderInfo, ADecoder,
              ARefaggninst, FRefinementTemplate, FAdaptiveTemplates,
              FRefinementAdaptiveTemplates, FSymbols, AGlyph);
            try
              ATextRegion.Process;
            finally
              ATextRegion.Free;
          ...
          begin
            ...
            if AId >= FSymbols.Count then
              TdxPDFUtils.RaiseTestException;
            ARefinement := TdxJBIG2RefinementRegion.Create(FSymbols[AId],
              ADecoder, ARdx, ARdy, FRefinementTemplate,
    ...
    begin
      ...
      if (ADecoder.LastCode) or (AExRunLength > ALimit - AIi) or
        (AExRunLength < 0) or (AZeroLength > 4) or AExFlag and
        (AExRunLength > FExportedSymbolCount - FGlyphs.Count) then
        TdxPDFUtils.RaiseTestException;
...
constructor TdxJBIG2SegmentData.Create(AStream: TStream;
  AInfo: TdxJBIG2HeaderInfo; AImage: IdxJBIG2Image);
begin
  Create(AImage);
...
class function TdxJBIG2SegmentData.CreateData(AStream: TStream;
  AInfo: TdxJBIG2HeaderInfo; AImage: IdxJBIG2Image): TdxJBIG2SegmentData;
begin
...
begin
  ...
  begin
    if not ImageRef.Segments.TryGetValue(HeaderInfo.ReferredToSegments[I],
      AHeader) then
      ImageRef.GlobalSegments.TryGetValue(HeaderInfo.ReferredToSegments
        [I], AHeader);
...
constructor TdxJBIG2TPGDONDecoder.Create(AImage: IdxJBIG2Image;
  AGBAdaptiveTemplate: TdxIntegerList);
begin
...
class function TdxJBIG2TPGDONDecoder.CreateDecoder(AImage: IdxJBIG2Image;
  AGBAdaptiveTemplate: TdxIntegerList; AGBTemplate: Integer)
  : TdxJBIG2TPGDONDecoder;
begin
  case AGBTemplate of
  ...
  begin
    ...
    if ALtp = 0 then
      ...
      begin
        ...
        begin
          AContext := AContext or
            (AImage.GetPixelEx(X + AAdaptiveTemplate[I],
            Y + AdaptiveTemplate[I + 1]) shl J);
          Inc(I, 2);
          Inc(J);
        end;
...
function TdxJBIG2TPGDON0Decoder.CreateAdaptiveTemplates(AGBAdaptiveTemplate
  : TdxIntegerList): TdxIntegerList;
begin
  Result := TdxIntegerList.Create;
  Result.Add(-1);
  Result.Add(0);
...
function TdxJBIG2TPGDON1Decoder.CreateAdaptiveTemplates(AGBAdaptiveTemplate
  : TdxIntegerList): TdxIntegerList;
begin
  Result := TdxIntegerList.Create;
  Result.Add(-1);
...
function TdxJBIG2TPGDON2Decoder.CreateAdaptiveTemplates(AGBAdaptiveTemplate
  : TdxIntegerList): TdxIntegerList;
begin
  Result := TdxIntegerList.Create;
  Result.Add(-1);
...
function TdxJBIG2TPGDON3Decoder.CreateAdaptiveTemplates(AGBAdaptiveTemplate
  : TdxIntegerList): TdxIntegerList;
begin
  Result := TdxIntegerList.Create;
  Result.Add(-1);
...
var
  AImage: IdxJBIG2Image;
  AGbw, AGbh, ARowStride, AGbregLine, Y, ALine1, ALine2, APaddedWidth, AContext,
    X, AMinorWidth, AXSlide, AXMinor, AMinorDif: Integer;
  AData: TBytes;
  AResult, ABit: Byte;
begin
  AImage := Image;
  ...
  begin
    ...
    begin
      ...
      if Y >= 1 then
        ALine1 := (ALine1 shl 8) or IfThen((X + 8) < AGbw,
          AData[AGbregLine - ARowStride + AXSlide + 1], 0);
      if Y >= 2 then
        ALine2 := (ALine2 shl 8) or IfThen((X + 8) < AGbw,
          AData[AGbregLine - ARowStride * 2 + AXSlide + 1] shl 6, 0);
      ...
      begin
        ...
        AContext := ((AContext and $7BF7) shl 1) or ABit or
          (ALine1 shr AMinorDif) and $10 or (ALine2 shr AMinorDif) and $800;
      end;
...
var
  AImage: IdxJBIG2Image;
  AGbw, AGbh, ARowStride, AGbregLine, Y, ALine1, ALine2, AContext, X,
    AMinorX: Integer;
  AData: TBytes;
  AResult, ABit: Byte;
begin
  ...
  begin
    ...
    begin
      AResult := 0;
      if Y >= 1 then
        ALine1 := ((ALine1 shl 8)) or
          IfThen(X + 8 < AGbw,
          AData[AGbregLine - ARowStride + (X shr 3) + 1], 0);
      if Y >= 2 then
        ALine2 := ((ALine2 shl 8)) or
          IfThen(X + 8 < AGbw, AData[AGbregLine - (ARowStride shl 1) + (X shr 3)
          + 1] shl 4, 0);
      ...
      begin
        ...
        AContext := (((AContext and $1BD) shl 1)) or (ABit) or
          (((ALine1 shr (10 - AMinorX)) and $4)) or
...
constructor TdxJBIG2Template3Decoder.Create(AImage: IdxJBIG2Image;
  AGbat0, AGbat1: Integer);
begin
  inherited Create(AImage);
  FGBAT0 := AGbat0;
  FGBAT1 := AGbat1;
...
constructor TdxJBIG2TextRegion.Create(AInfo: TdxJBIG2HeaderInfo;
  ADecoder: TdxJBIG2Decoder; ASymbolInstanceCount, ARefinementTemplate: Integer;
  AAt, ARAt: TdxIntegerList; AGlyphs: TList<IdxJBIG2Image>;
  AImage: IdxJBIG2Image);
begin
  inherited Create(AImage);
...
constructor TdxJBIG2TextRegion.Create(AStream: TStream;
  AInfo: TdxJBIG2HeaderInfo; AImage: IdxJBIG2Image);
var
  AFlags: Integer;
begin
...
var
  ...
  ACurs, AStript, AFirsts, ANInstances, ADt, ACurt, T, AId, X, Y, ASs, ATt,
    AIds: Integer;
begin
  inherited Process;
  ...
  begin
    ...
    begin
      ...
      else if (FTransposed) and ((Integer(FCorner) and 1) = 0) then
  ...
  if (HeaderInfo.Flags and 63) <> 4 then
    ImageRef.Composite(AResultImage, FRegionInfo.X, FRegionInfo.Y,
      FRegionInfo.ComposeOperator);
end;

function TdxJBIG2TextRegion.RefinementDecode(ADecoder: TdxJBIG2Decoder;
  AImage: IdxJBIG2Image; AAdaptiveTemplates: TdxIntegerList): IdxJBIG2Image;
var
...
begin
  ...
  ARefinementRegion := TdxJBIG2RefinementRegion.Create(AImage, ADecoder, ADx,
    ADy, FRefinementTemplate, FRefinementAdaptiveTemplates, Result);
  try
...
constructor TdxJBIG2RefinementRegion.Create(AGlyph: IdxJBIG2Image;
  ADecoder: TdxJBIG2Decoder; ADx, ADy, ATemplate: Integer; AAt: TdxIntegerList;
  AImage: IdxJBIG2Image);
begin
  inherited Create(AImage);
  FDecoderRef := ADecoder;
...
function TdxJBIG2RefinementRegion.Template0Context(X: Integer;
  Y: Integer): Integer;
var
  AContext: Integer;
begin
  ...
  AContext := AContext or ImageRef.GetPixelEx(X + FAdaptiveTemplate[0],
    Y + FAdaptiveTemplate[1]) shl 3;
  AContext := AContext or FReferenceGlyph.GetPixelEx(X - FDx + 1,
    Y - FDy + 1) shl 4;
  AContext := AContext or FReferenceGlyph.GetPixelEx(X - FDx + 0,
    Y - FDy + 1) shl 5;
  AContext := AContext or FReferenceGlyph.GetPixelEx(X - FDx - 1,
    Y - FDy + 1) shl 6;
  AContext := AContext or FReferenceGlyph.GetPixelEx(X - FDx + 1,
  ...
  AContext := AContext or FReferenceGlyph.GetPixelEx(X - FDx + 0,
    Y - FDy - 1) shl 11;
  AContext := AContext or FReferenceGlyph.GetPixelEx
    (X - FDx + FAdaptiveTemplate[2], Y - FDy + FAdaptiveTemplate[3]) shl 12;
  Result := AContext;
end;

function TdxJBIG2RefinementRegion.Template1Context(X: Integer;
  Y: Integer): Integer;
var
  AContext: Integer;
begin
...
constructor TdxJBIG2GenericRegion.Create(AStream: TStream;
  AInfo: TdxJBIG2HeaderInfo; AImage: IdxJBIG2Image);
var
  ALength: Integer;
begin
...
constructor TdxJBIG2GenericRegion.Create(AInfo: TdxJBIG2HeaderInfo;
  ATemplate: Integer);
begin
...
begin
  ...
  ImageRef.Composite(AImage, FRegionInfo.X, FRegionInfo.Y,
    FRegionInfo.ComposeOperator);
end;

function TdxJBIG2GenericRegion.CreateDecoder(AImage: IdxJBIG2Image;
  AAdaptiveTemplates: TdxIntegerList): TdxJBIG2GenericRegionDecoder;
begin
  Result := nil;
  if FMMR then
  ...
  if FUseTypicalPredictionDecoding then
    Result := TdxJBIG2TPGDONDecoder.CreateDecoder(AImage, AAdaptiveTemplates,
      FTemplate)
  else
    case FTemplate of
      0:
        if (AAdaptiveTemplates[0] = 3) and (AAdaptiveTemplates[1] = -1) and
          (AAdaptiveTemplates[2] = -3) and (AAdaptiveTemplates[3] = -1) and
          (AAdaptiveTemplates[4] = 2) and (AAdaptiveTemplates[5] = -2) and
      ...
      3:
        Result := TdxJBIG2Template3Decoder.Create(AImage, AAdaptiveTemplates[0],
          AAdaptiveTemplates[1]);
    else
      TdxPDFUtils.RaiseTestException;
    end;
end;
...
constructor TdxJBIG2PageInfo.Create(AStream: TStream; AInfo: TdxJBIG2HeaderInfo;
  AImage: IdxJBIG2Image);
var
...
function TdxJBIG2ArithmeticIntContext.Decode(AState: TdxArithmeticState)
  : TdxJBIG2ArithmeticDecoderResult;
var
...
constructor TdxJBIG2Decoder.Create(AHelper: TdxJBIG2StreamReader;
  AMaxId: Integer);
begin
...
constructor TdxJBIG2Decoder.Create(AHelper: TdxJBIG2StreamReader;
  AIdLength, AGbLength, AGrLength: Integer);
begin
  inherited Create;
