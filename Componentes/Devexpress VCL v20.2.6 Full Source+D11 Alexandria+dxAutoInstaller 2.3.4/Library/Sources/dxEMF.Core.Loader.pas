...
type
  ...
  protected
    ...
    function GetFieldDef(AIndex: Integer): TdxDataLoaderFieldDef;
      virtual; abstract;
    function MoveNext: Boolean; virtual; abstract;
    function ReadCurrentRecordKeys: TArray<TValue>; virtual; abstract;
    procedure ReadCurrentRecord(AInstance: Pointer); overload; virtual;
      abstract;
    procedure ReadCurrentRecord(out AValues: TArray<Variant>); overload;
      virtual; abstract;
    procedure ReadCurrentRecord(out AValues: TArray<TValue>); overload;
      virtual; abstract;
  ...
  public
    ...
    constructor Create(const ARows
      : TArray<TdxSelectStatementResultRow>); overload;
    destructor Destroy; override;
    property Count: Integer read GetCount;
    property Items[AIndex: Integer]: TdxSelectStatementResultRow
...
{$REGION 'ValueAccessor'}
    ...
    protected
      ...
      procedure SetRowValue(const ARow: TArray<TValue>;
    ...
    public
      ...
      class function CreateAccessor(AData: TdxQueryData;
        AMember: TdxMappingMemberInfo; var AIndex: Integer)
        : TdxValueAccessor; static;
    ...
    protected
      function GetValue: TValue; override;
    public
      constructor Create(AData: TdxQueryData; AIndex: Integer;
    ...
    public
      constructor Create(ANested: TList<TdxValueAccessor>;
        AMember: TdxMappingMemberInfo);
    end;
{$ENDREGION}
  strict private
  ...
  protected
    ...
    procedure AddMember(AMemberInfo: TdxMappingMemberInfo; ARoot: TdxQueryData;
      AParentPath: TdxMemberInfoCollection; var AIndex: Integer);
    procedure Init(ARoot: TdxQueryData; AEntityInfo: TdxEntityInfo;
      AParentPath: TdxMemberInfoCollection; var AIndex: Integer);
    procedure CreateInternalEntityInfo(const AProperties
  ...
  public
    ...
    constructor Create(AEntityInfo: TdxEntityInfo;
      const AProperties: TArray<IdxCriteriaOperator>;
  ...
  public
    class function Create(ASelectStatementResult: TdxSelectStatementResult)
      : IdxEMFCollection; overload; static;
  end;
...
type
  ...
  public
    constructor Create(AOwner: TdxEntityInfo; AIndex: Integer;
  ...
  public
    constructor Create(AOwner: TdxEntityInfo;
      const AOperandValue: IdxOperandValue);
    function GetValue(AObject: TObject): TValue; override;
  ...
  public
    constructor Create(AQueryData: TdxQueryData;
      const AProperties: TArray<IdxCriteriaOperator>);
  end;

  { TdxEMFSelectStatementResultCollection }

  TdxEMFSelectStatementResultCollection = class(TdxEMFBaseCollection,
    IdxCollection, IdxList, IdxEMFCollection)
  strict private
  type
...
constructor TdxSelectStatementResultMemberInfo.Create(AOwner: TdxEntityInfo;
  AIndex: Integer; AOriginal: TdxMappingMemberInfo);
begin
  ...
  if AOriginal = nil then
    Attributes := [TdxAttribute.Column]
  else
    Attributes := AOriginal.Attributes;
end;
...
constructor TdxSelectStatementConstantMemberInfo.Create(AOwner: TdxEntityInfo;
  const AOperandValue: IdxOperandValue);
begin
  inherited Create(AOwner);
...
function TdxSelectStatementConstantMemberInfo.GetValue
  (AObject: TObject): TValue;
begin
  Result := FOperandValue.Value;
end;

procedure TdxSelectStatementConstantMemberInfo.SetValue(AObject: TObject;
  const AValue: TValue);
begin
end;
...
constructor TdxSelectStatementResultEntityInfo.Create(AQueryData: TdxQueryData;
  const AProperties: TArray<IdxCriteriaOperator>);
begin
  inherited Create;
...
function TdxSelectStatementResultEntityInfo.IsUniqueName
...
procedure TdxSelectStatementResultEntityInfo.Make(const AProperties
  : TArray<IdxCriteriaOperator>);
var
...
begin
  if ((FQueryData.DataLoader = nil) or (FQueryData.DataLoader.FieldCount = 0))
  then
    Exit;
  AFieldCount := FQueryData.FieldCount;
  AQuery := FQueryData.DataLoader.Query;
  AHasConst := (AQuery.ConstantValues <> nil) and
    (AQuery.ConstantValues.Count > 0);
  AMappingClassInfo := TdxMappingClassInfo.Create(Self,
    FQueryData.DataLoader.EntityInfo.ClassAttributes.PersistentClass);
  TdxMappingClassInfoAccess(AMappingClassInfo)
    .Assign(FQueryData.DataLoader.EntityInfo.ClassAttributes);
  Add(AMappingClassInfo);
  for I := 0 to AFieldCount - 1 do
  begin
    AParentMemberInfo := nil;
    if AProperties[I].QueryInterface(IdxParentCriteria, AParentCriteria) = 0
    then
      if AParentCriteria.ParentCriteria.QueryInterface(IdxMemberOperand,
        AMemberOperand) = 0 then
        AParentMemberInfo := AMemberOperand.Member;
    if AHasConst then
    begin
      if AQuery.ConstantValues.ContainsKey(I) then
        AMemberInfo := TdxSelectStatementConstantMemberInfo.Create(Self,
          AQuery.ConstantValues[I])
      else
        AMemberInfo := TdxSelectStatementResultMemberInfo.Create(Self,
          AQuery.OperandIndexes[I], AParentMemberInfo);
    end
    else
      AMemberInfo := TdxSelectStatementResultMemberInfo.Create(Self, I,
        AParentMemberInfo);
...
function TdxSelectStatementResultEntityInfo.UniqueName
...
procedure TdxDataLoader.AppendObject(const AKeys: TArray<TValue>;
  AObject: TObject);
begin
...
function TdxDataLoader.GetLoadedObjectByKey(const AKeys
  : TArray<TValue>): TObject;
begin
  Assert(FPool <> nil);
...
begin
  ...
  repeat
    ...
    if Result = nil then
    begin
      ANewObject := TdxEMFCustomSessionAccess(ASession)
...
begin
  ...
  FPool := TdxEMFCustomSessionAccess(Session as TdxEMFCustomSession)
    .PoolClasses.GetPool(FLoadClasses);
end;
...
class function TdxQueryData.TdxValueAccessor.CreateAccessor(AData: TdxQueryData;
  AMember: TdxMappingMemberInfo; var AIndex: Integer): TdxValueAccessor;
var
...
procedure TdxQueryData.TdxValueAccessor.SetRowValue(const ARow: TArray<TValue>;
  var AIndex: Integer);
begin
  ARow[AIndex] := Value;
  Inc(AIndex);
...
constructor TdxQueryData.TdxTypedRefAccessor.Create(AValue: TdxValueAccessor;
  AType: TdxValueAccessor);
begin
...
constructor TdxQueryData.TdxSimpleValueAccessor.Create(AData: TdxQueryData;
  AIndex: Integer; AMember: TdxMappingMemberInfo);
begin
...
constructor TdxQueryData.TdxSubValueAccessor.Create
  (ANested: TList<TdxValueAccessor>; AMember: TdxMappingMemberInfo);
begin
  inherited Create(AMember);
  FNested := ANested;
end;
...
procedure TdxQueryData.TdxSubValueAccessor.SetRowValue
  (const ARow: TArray<TValue>; var AIndex: Integer);
begin
  NotImplemented;
end;
...
begin
  inherited Create;
  FNestedDataDictionary := TObjectDictionary<TdxMappingMemberInfo,
    TdxQueryData>.Create([doOwnsValues]);
end;

constructor TdxQueryData.Create(AEntityInfo: TdxEntityInfo;
  const AProperties: TArray<IdxCriteriaOperator>; ADataLoader: TdxDataLoader);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      FLinearMembers.Add(TdxValueAccessor.CreateAccessor(Self,
        APath[APath.Count - 1], FItemCount));
    end;
  end;
end;
...
procedure TdxQueryData.CreateInternalEntityInfo(const AProperties
  : TArray<IdxCriteriaOperator>);
begin
  FInternalEntityInfo := TdxSelectStatementResultEntityInfo.Create(Self,
    AProperties);
end;
...
function TdxQueryData.GetNestedData(AMemberInfo: TdxMappingMemberInfo)
  : TdxQueryData;
begin
...
procedure TdxQueryData.AddMember(AMemberInfo: TdxMappingMemberInfo;
  ARoot: TdxQueryData; AParentPath: TdxMemberInfoCollection;
  var AIndex: Integer);
var
  ACollection: TdxMemberInfoCollection;
begin
  ACollection := TdxMemberInfoCollection.Create
    (ARoot.EntityInfo { , AParentPath.Count + 1 } );
  ACollection.AddRange(AParentPath);
  ACollection.Add(AMemberInfo);
  FProperties.Add(ACollection);
end;

procedure TdxQueryData.Init(ARoot: TdxQueryData; AEntityInfo: TdxEntityInfo;
  AParentPath: TdxMemberInfoCollection; var AIndex: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((((AParentPath.Count <= ADepth) and (ARefType <> nil)) and
        not M.IsKey) and not M.IsDelayed) and
...
function TdxQueryDataPool.GetQueryData(AEntityInfo: TdxEntityInfo)
  : TdxQueryData;
begin
...
constructor TdxSelectStatementResult.Create(const ARows
  : TArray<TdxSelectStatementResultRow>);
begin
  Create;
...
function TdxSelectStatementResult.GetRows(AIndex: Integer)
  : TdxSelectStatementResultRow;
begin
  Result := FRows[AIndex];
end;
...
class function TdxSelectStatementCollections.Create(ASelectStatementResult
  : TdxSelectStatementResult): IdxEMFCollection;
begin
  Result := TdxEMFSelectStatementResultCollection.Create
    (ASelectStatementResult);
end;

{ TdxEMFSelectStatementResultCollection }

constructor TdxEMFSelectStatementResultCollection.Create(ASelectStatementResult
  : TdxSelectStatementResult);
begin
  inherited Create;
...
function TdxEMFSelectStatementResultCollection.Contains
  (AValue: TObject): Boolean;
begin
  Result := FSelectStatementResult.Rows.Contains
    (AValue as TdxSelectStatementResultRow);
end;
...
function TdxEMFSelectStatementResultCollection.
  GetCollectionElementClass: TClass;
begin
...
function TdxEMFSelectStatementResultCollection.GetItems
  (AIndex: Integer): TObject;
begin
  Result := FSelectStatementResult.Rows[AIndex];
end;

function TdxEMFSelectStatementResultCollection.IndexOf(AValue: TObject)
  : Integer;
begin
  Result := FSelectStatementResult.Rows.IndexOf
    (AValue as TdxSelectStatementResultRow);
end;

procedure TdxEMFSelectStatementResultCollection.Insert(AIndex: Integer;
  AObject: TObject);
begin
  raise ENotSupportedException.Create('sNotSupportedException');
end;

function TdxEMFSelectStatementResultCollection.InternalGetEnumerator
  : IEnumerator;
begin
...
procedure TdxEMFSelectStatementResultCollection.SetDeleteObjectOnRemove
  (Value: Boolean);
begin
end;
...
constructor TdxEMFSelectStatementResultCollection.TEnumerator.Create
