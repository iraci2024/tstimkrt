...
type
  ...
  protected
    property CurrentFields: TArray<TdxField> read FCurrentFields
      write FCurrentFields;
    property CurrentFieldIndex: Integer read FCurrentFieldIndex
      write FCurrentFieldIndex;
  public
    constructor Create;
    destructor Destroy; override;
    procedure UpdateFieldsInfo(APieceTable: TdxPieceTable);
    function CreateFieldBeginDescriptor(APieceTable: TdxPieceTable)
      : TdxDocFieldBeginDescriptor;
    function CreateFieldEndDescriptor(AFieldLevel: Integer)
  ...
  protected
    property CurrentPictureIndex: Integer read FCurrentPictureIndex
      write FCurrentPictureIndex;
  public
    constructor Create;
    procedure WritePictureDescriptor(AWriter: TBinaryWriter;
  ...
  protected type
    ...
    public
      ...
      property HeaderTextBoxesTextLength: Integer
        read FHeaderTextBoxesTextLength write FHeaderTextBoxesTextLength;
    end;
  ...
  protected
    ...
    procedure ExportTextBoxesContent(ATextBoxes
    ...
    procedure SetNotesProperties(ADop: TdxDocDocumentProperties;
      ASection: TdxSection);
    ...
    procedure ExportListOverrideFormatInformation
    ...
    procedure ExportParagraphs(AFrom: TdxParagraphIndex;
      ATo: TdxParagraphIndex); override;
    function ExportFirstCommentParagraph(AParagraph: TdxParagraph)
      : TdxParagraphIndex;
    ...
    procedure ExportTextRunBaseCore(ARun: TdxTextRunBase; const AText: string;
      ASpecial: Boolean = False); overload;
  ...
  public
    ...
    procedure Write;
    procedure AddStreamDirectoryEntry(ARoot: TdxOLEDocumentDirectoryEntry;
      const AName: string; ABinaryWriter: TBinaryWriter);

    property CurrentCharacterPosition: Integer read FCurrentCharacterPosition
      write FCurrentCharacterPosition;
    property CurrentDocumentPartStart: Integer read FCurrentDocumentPartStart
      write FCurrentDocumentPartStart;
...
function TdxExportFieldsIterator.CreateFieldBeginDescriptor
  (APieceTable: TdxPieceTable): TdxDocFieldBeginDescriptor;
var
  AField: TdxField;
...
begin
  ...
  try
    ...
    AScanner := TdxFieldScanner.Create(AIterator, AModel.MaxFieldSwitchLength,
      AModel.EnableFieldNames, APieceTable.SupportFieldCommonStringFormat);
    try
...
function TdxExportFieldsIterator.CreateFieldEndDescriptor(AFieldLevel: Integer)
  : TdxDocFieldEndDescriptor;
var
  AField: TdxField;
begin
...
procedure TdxExportImagesIterator.WritePictureDescriptor(AWriter: TBinaryWriter;
  ARun: TdxInlinePictureRun);
var
  APictureDescriptor: TdxPictureDescriptor;
begin
  APictureDescriptor := TdxPictureDescriptor.Create(ARun,
    CalculateCurrentShapeId, CurrentPictureIndex);
  try
...
begin
  if FState = TdxDocContentState.MainDocument then
    Exit(TdxOfficeArtConstants.DefaultMainDocumentShapeIdentifier +
      CurrentPictureIndex);
  if FState = TdxDocContentState.HeadersFootersStory then
    Exit(TdxOfficeArtConstants.DefaultHeaderShapeIdentifier +
      CurrentPictureIndex);
...
function TdxDocExporter.TTextStreamBorders.
  ShouldWriteEmptyParagraphAtEnd: Boolean;
begin
  Result := (FootNotesTextLength <> 0) or (HeadersFootersTextLength <> 0) or
    (CommentsTextLength <> 0) or (EndNotesTextLength <> 0) or
    (TextBoxesTextLength <> 0) or (HeaderTextBoxesTextLength <> 0);
end;
...
begin
  ...
  FDocDataWriter := TdxDocDataWriter.Create(DataStreamWriter, Options,
    DocumentModel);
...
begin
  AOleDocument := TdxOLEDocument.Create(FOutputStream,
    TdxOLEDocumentMode.dmWriting);
  ARoot := AOleDocument.CreateDirEntry(TdxOLEDocument.RootDirName, ET_ROOT);
  try
    AddStreamDirectoryEntry(ARoot, TdxDocumentStreams.Stream1TableName,
      TableStreamWriter);
    AddStreamDirectoryEntry(ARoot, TdxDocumentStreams.MainStreamName,
      MainStreamWriter);

    if DataStreamWriter.BaseStream.Size > 0 then
      AddStreamDirectoryEntry(ARoot, TdxDocumentStreams.DataStreamName,
        DataStreamWriter);
    AOleDocument.Commit;
  finally
    AOleDocument.Free;
...
procedure TdxDocExporter.AddStreamDirectoryEntry
  (ARoot: TdxOLEDocumentDirectoryEntry; const AName: string;
  ABinaryWriter: TBinaryWriter);
begin
  ARoot.Document.CreateStream(ARoot, AName)
    .CopyFrom(ABinaryWriter.BaseStream, 0);
end;
...
begin
  if (not DocumentModel.DocumentCapabilities.FootNotesAllowed) or
    (DocumentModel.FootNotes.Count = 0) then
    Exit;
  ...
  DocDataWriter.NotesExporter.FinishFootNotePositions
    (TextBorders.FootNotesTextLength - 1, CurrentCharacterPosition);
end;
...
begin
  ...
  begin
    ...
    DocDataWriter.NotesExporter.FootNotePositions.Add
      (GetRelativeCharacterPosition);
...
begin
  ...
  HeadersFootersPositions.CharacterPositions.Add
    (TextBorders.HeadersFootersTextLength + 2);
end;
...
begin
  if (DocumentModel.Headers.Count > 0) or (DocumentModel.Footers.Count > 0) then
    Exit(True);
  if (DocumentModel.FootNotes.Count > 0) or (DocumentModel.EndNotes.Count > 0)
  then
    Exit(True);
  Result := False;
end;
...
begin
  if (not DocumentModel.DocumentCapabilities.EndNotesAllowed) or
    (DocumentModel.EndNotes.Count = 0) then
    Exit;
  ...
  DocDataWriter.NotesExporter.FinishEndNotePositions
    (TextBorders.EndNotesTextLength);
end;
...
begin
  ...
  begin
    ...
    DocDataWriter.NotesExporter.EndNotePositions.Add
      (GetRelativeCharacterPosition);
...
procedure TdxDocExporter.ExportTextBoxesContent
  (ATextBoxes: TDictionary<Integer, TdxTextBoxContentType>);
var
...
begin
  ...
  begin
    ATextBoxExporter.AddTextBoxTableEntry(GetRelativeCharacterPosition,
      AKey, False);
...
procedure TdxDocExporter.ExportSectionHeadersFootersFiltered
  (ASection: TdxSection);
begin
...
begin
  ...
  begin
    ...
    ExportEvenPageHeader(ASection.InnerEvenPageHeader,
      ASection.Headers.IsLinkedToPrevious(TdxHeaderFooterType.Even));
    ExportEmptyParagraph;
  end;
  HeadersFootersPositions.CharacterPositions.Add(GetRelativeCharacterPosition);
end;
...
begin
  ...
  begin
    ...
    ExportOddPageHeader(ASection.InnerOddPageHeader,
      ASection.Headers.IsLinkedToPrevious(TdxHeaderFooterType.Odd));
    ExportEmptyParagraph;
  end;
  HeadersFootersPositions.CharacterPositions.Add(GetRelativeCharacterPosition);
end;
...
begin
  ...
  begin
    ...
    ExportEvenPageFooter(ASection.InnerEvenPageFooter,
      ASection.Headers.IsLinkedToPrevious(TdxHeaderFooterType.Even));
    ExportEmptyParagraph;
  end;
  HeadersFootersPositions.CharacterPositions.Add(GetRelativeCharacterPosition);
...
begin
  ...
  begin
    ...
    ExportOddPageFooter(ASection.InnerOddPageFooter,
      ASection.Headers.IsLinkedToPrevious(TdxHeaderFooterType.Odd));
    ExportEmptyParagraph;
  end;
  HeadersFootersPositions.CharacterPositions.Add(GetRelativeCharacterPosition);
...
begin
  ...
  begin
    ...
    ExportFirstPageHeader(ASection.InnerFirstPageHeader,
      ASection.Headers.IsLinkedToPrevious(TdxHeaderFooterType.First));
    ExportEmptyParagraph;
  end;
  HeadersFootersPositions.CharacterPositions.Add(GetRelativeCharacterPosition);
end;
...
begin
  ...
  begin
    ...
    ExportFirstPageFooter(ASection.InnerFirstPageFooter,
      ASection.Headers.IsLinkedToPrevious(TdxHeaderFooterType.First));
    ExportEmptyParagraph;
  end;
  HeadersFootersPositions.CharacterPositions.Add(GetRelativeCharacterPosition);
...
var
  AOffset: Int64;
begin
  if MainStreamWriter.BaseStream.Position mod TdxDocContentBuilder.SectorSize <> 0
  then
  begin
    AOffset := (MainStreamWriter.BaseStream.Position div TdxDocContentBuilder.
      SectorSize + 1) * TdxDocContentBuilder.SectorSize;
...
begin
  ...
  Result.Flags := [TdxFileInformationFlag.TableStreamType,
    TdxFileInformationFlag.ExtendedCharset, TdxFileInformationFlag.QuickSaves];
...
begin
  ...
  AFib.StyleSheetSize := TableStreamWriter.BaseStream.Position -
    AFib.StyleSheetOffset;
end;
...
begin
  ...
  AFib.SectionTableSize := TableStreamWriter.BaseStream.Position -
    AFib.SectionTableOffset;
  AFib.ParagraphHeightsOffset := TableStreamWriter.BaseStream.Position;
  AFib.ParagraphHeightsSize := AFib.ParagraphHeightsOffset -
    TableStreamWriter.BaseStream.Position;
end;

procedure TdxDocExporter.ExportDocumentProperties
  (AFib: TdxFileInformationBlock);
var
...
begin
  ...
  try
    ...
    ADop.DifferentOddAndEvenPages :=
      ADocumentProperties.DifferentOddAndEvenPages;
    ...
    AFib.DocumentPropertiesSize := TableStreamWriter.BaseStream.Position -
...
procedure TdxDocExporter.SetNotesProperties(ADop: TdxDocDocumentProperties;
  ASection: TdxSection);
begin
...
begin
  ...
  ADocVars := TdxDocumentVariables.FromVariablesCollection
    (DocumentModel.Variables, DocumentModel.DocumentProperties);
  try
    ADocVars.Write(TableStreamWriter);
  finally
    ADocVars.Free;
  end;
  AFib.DocumentVariablesSize := TableStreamWriter.BaseStream.Position -
    AFib.DocumentVariablesOffset;
end;
...
begin
  ...
  begin
    ...
    try
      AFontName := DocDataWriter.StylesExporter.GetFontNameByIndex
        (AFontNameIndex);
  ...
  AFib.FontTableSize := TableStreamWriter.BaseStream.Position -
    AFib.FontTableOffset;
end;

procedure TdxDocExporter.ExportComplexFileInformation
  (AFib: TdxFileInformationBlock);
var
  APieceTable: TdxDocPieceTable;
  AClxInfo: TdxComplexFileInformation;
begin
  APieceTable := TdxDocPieceTable.CreateDefault(TextStartOffset,
    CurrentCharacterPosition);
  try
    AClxInfo := TdxComplexFileInformation.Create;
    try
      AClxInfo.PieceTable := APieceTable.ToByteArray;
      AFib.ComplexFileInformationOffset :=
        TableStreamWriter.BaseStream.Position;
      AClxInfo.Write(TableStreamWriter);
      AFib.ComplexFileInformationSize := TableStreamWriter.BaseStream.Position -
        AFib.ComplexFileInformationOffset;
    finally
      AClxInfo.Free;
    end;
...
procedure TdxDocExporter.ExportFormattingBinTables
  (AFib: TdxFileInformationBlock);
begin
  AFib.CharacterTableOffset := TableStreamWriter.BaseStream.Position;
  DocDataWriter.WriteCharactersBinTable(TableStreamWriter);
  AFib.CharacterTableSize := TableStreamWriter.BaseStream.Position -
    AFib.CharacterTableOffset;
  AFib.ParagraphTableOffset := TableStreamWriter.BaseStream.Position;
  DocDataWriter.WriteParagraphsBinTable(TableStreamWriter);
  AFib.ParagraphTableSize := TableStreamWriter.BaseStream.Position -
    AFib.ParagraphTableOffset;
end;

procedure TdxDocExporter.ExportHeadersFootersPositionsTable
  (AFib: TdxFileInformationBlock);
begin
  ...
  AFib.HeadersFootersPositionsSize := TableStreamWriter.BaseStream.Position -
    AFib.HeadersFootersPositionsOffset;
end;

procedure TdxDocExporter.ExportListFormatInformation
  (AFib: TdxFileInformationBlock);
begin
  ...
  AFib.ListFormatInformationSize := TableStreamWriter.BaseStream.Position -
    AFib.ListFormatInformationOffset;
end;

procedure TdxDocExporter.ExportDocumentFormatRecords
  (AFib: TdxFileInformationBlock);
var
...
begin
  ...
  AFib.DocumentFileRecordsSize := TableStreamWriter.BaseStream.Position -
    AFib.DocumentFileRecordsOffset;
end;

procedure TdxDocExporter.ExportListOverrideFormatInformation
  (AFib: TdxFileInformationBlock);
begin
  AFib.ListFormatOverrideInformationOffset :=
    TableStreamWriter.BaseStream.Position;
  DocDataWriter.ListsExporter.WriteListOverrideFormatInformation
    (TableStreamWriter);
...
procedure TdxDocExporter.ExportRangeEditPermissions
  (AFib: TdxFileInformationBlock);
begin
  DocDataWriter.PermissionIterator.Write(AFib, TableStreamWriter);
end;

procedure TdxDocExporter.ExportFloatingObjectsInfo
  (AFib: TdxFileInformationBlock);
begin
  DocDataWriter.FloatingObjectsExporter.ExportFloatingObjectsInfo(AFib,
    TableStreamWriter, MainStreamWriter);
end;
...
begin
  ...
  AFib.LastCharacterFileOffset := TdxFileInformationBlock.FIBSize +
...
begin
  ...
  AFib.RmdThreadingSize := TableStreamWriter.BaseStream.Position -
    AFib.RmdThreadingOffset;
end;
...
procedure TdxDocExporter.ExportParagraphs(AFrom: TdxParagraphIndex;
...
function TdxDocExporter.ExportFirstCommentParagraph(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
begin
...
function TdxDocExporter.ExportParagraph(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
begin
  BeginParagraphExport(AParagraph);
  Result := inherited ExportParagraph(AParagraph);
...
begin
  AParagraphStyleIndex := DocDataWriter.StylesExporter.GetParagraphStyleIndex
    (AParagraph.ParagraphStyleIndex);
  if AParagraph.GetCell <> nil then
    DocDataWriter.WriteInTableParagraph(CurrentCharacterPosition,
      AParagraphStyleIndex, AParagraph)
  else
    DocDataWriter.WriteParagraph(CurrentCharacterPosition, AParagraphStyleIndex,
      AParagraph);
  AListIndex := AParagraph.NumberingListIndex;
  if (AListIndex = NumberingListIndexNoNumberingList) or
    (AListIndex = NumberingListIndexListIndexNotSet) then
    Exit;
  ALevelInfo := DocDataWriter.ListsExporter.ListOverrideInfo.FormatOverrideData
    [AListIndex];
  if ALevelInfo.CharacterPosition <> TdxDocListsExporter.EmptyCharacterPosition
  then
    Exit;
...
begin
  ...
  DocDataWriter.WriteParagraph(CurrentCharacterPosition, AParagraphStyleIndex,
    AParagraph);
  ExportTextRunBaseCore(DocumentModel.MainPieceTable.Runs.Last,
    TdxTextCodes.ParagraphMark);
end;
...
begin
  ExportEmptyParagraphCore;
  MainStreamWriter.
    Write(TdxEncoding.Unicode.GetBytes(TdxTextCodes.ParagraphMark));
...
begin
  AParagraphStyleIndex := DocDataWriter.StylesExporter.GetParagraphStyleIndex
    (DocumentModel.ParagraphStyles.DefaultItemIndex);
  DocDataWriter.WriteParagraph(CurrentCharacterPosition, AParagraphStyleIndex);
end;
...
begin
  ...
  AIsLastParagraphInTableCell := (ACell <> nil) and
    (ACell.EndParagraphIndex = AParagraph.Index);
...
begin
  ...
  ACharacterStyleIndex := DocDataWriter.StylesExporter.GetCharacterStyleIndex
    (ARun.CharacterStyleIndex);
  DocDataWriter.WriteInlinePictureRun(CurrentCharacterPosition,
    ACharacterStyleIndex, DataStreamWriter.BaseStream.Position,
    ARun.CharacterProperties);
  ImagesIterator.WritePictureDescriptor(DataStreamWriter, ARun);
  Inc(FCurrentCharacterPosition);
end;

procedure TdxDocExporter.ExportFloatingObjectAnchorRun
  (ARun: TdxFloatingObjectAnchorRun);
begin
  DocDataWriter.FloatingObjectsExporter.RegisterFloatingObject(ARun,
    GetRelativeCharacterPosition);
...
begin
  ...
  DocDataWriter.BookmarkIterator.AddBookmarkStart(ABookmark.Name,
    CurrentCharacterPosition);
end;
...
begin
  ...
  DocDataWriter.BookmarkIterator.AddBookmarkEnd(ABookmark.Name,
    CurrentCharacterPosition);
end;

procedure TdxDocExporter.ExportRangePermissionStart(ARangePermission
  : TdxRangePermission);
begin
  inherited ExportRangePermissionStart(ARangePermission);
  DocDataWriter.PermissionIterator.AddPermissionStart(ARangePermission,
    CurrentCharacterPosition);
end;

procedure TdxDocExporter.ExportRangePermissionEnd(ARangePermission
  : TdxRangePermission);
begin
  inherited ExportRangePermissionEnd(ARangePermission);
  DocDataWriter.PermissionIterator.AddPermissionEnd(ARangePermission,
    CurrentCharacterPosition);
end;
...
begin
  ...
  AFieldBeginDescriptor := FieldsIterator.CreateFieldBeginDescriptor
    (ActivePieceTable);
  DocDataWriter.FieldTable.AddEntry(GetRelativeCharacterPosition,
    AFieldBeginDescriptor);
...
begin
  ...
  begin
    ...
    DocDataWriter.NotesExporter.AddFootNoteReferenceEntry
      (GetRelativeCharacterPosition, True);
  end;
...
begin
  ...
  begin
    ...
    DocDataWriter.NotesExporter.AddEndNoteReferenceEntry
      (GetRelativeCharacterPosition, True);
  end;
  ExportTextRunBaseCore(ARun, TdxTextCodes.AutoNumberedFootNoteReference, True);
end;
...
function TdxDocExporter.ExportTable(ATableInfo: TdxTableInfo)
  : TdxParagraphIndex;
var
...
begin
  ...
  AParagraphStyleIndex := DocDataWriter.StylesExporter.GetParagraphStyleIndex
    (DocumentModel.ParagraphStyles.DefaultItemIndex);
  DocDataWriter.WriteParagraph(CurrentCharacterPosition,
    AParagraphStyleIndex, ARow);
...
procedure TdxDocExporter.ExportTextRunBaseCore(ARun: TdxTextRunBase;
  const AText: string; ASpecial: Boolean = False);
begin
