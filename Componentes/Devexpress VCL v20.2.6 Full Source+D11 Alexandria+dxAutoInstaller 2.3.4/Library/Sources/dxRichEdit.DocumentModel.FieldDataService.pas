...
type
  ...
  IdxMailMergeDataService = interface(IdxFieldDataService)
    ...
    function StartMailMerge(const AOptions: IdxMailMergeOptions)
      : IdxMailMergeDataService;
  ...
  IdxDynamicObjectPropertyValueProvider = interface
    ...
    function GetPropertyValue(AInstance: TObject;
      const APropertyName: string): TValue;
  end;

  { TdxMailMergeDataService }

  TdxMailMergeDataService = class(TInterfacedObject, IdxMailMergeDataService,
    IdxFieldDataService)
  strict private
  const
  ...
  protected
    ...
    function GetRecordCount: Integer;
    function CreateNew(ADataController: TdxRichEditDataControllerAdapterBase;
      const AOptions: IdxMailMergeOptions): IdxMailMergeDataService; virtual;
    function CreateMailMergeDataController
      : TdxRichEditDataControllerAdapterBase; virtual;
    function GetColumnIndexByName(AMailMergeProperties: TdxMailMergeProperties;
      const AFieldName: string; AMapFieldName: Boolean): Integer; virtual;

    property DataController: TdxRichEditDataControllerAdapterBase
  ...
  public
    ...
    function GetFieldValue(AMailMergeProperties: TdxMailMergeProperties;
      ...
      AField: TdxField): TValue; virtual;

    procedure DestroyDataController;

    property BoundMode: Boolean read GetBoundMode;
    property DynamicObjectPropertyValueProvider
      : IdxDynamicObjectPropertyValueProvider
      read FDynamicObjectPropertyValueProvider
      write FDynamicObjectPropertyValueProvider;
  end;
...
constructor TdxMailMergeDataService.Create(ADataController
...
function TdxMailMergeDataService.GetDataController
  : TdxRichEditDataControllerAdapterBase;
begin
...
function TdxMailMergeDataService.StartMailMerge(const AOptions
  : IdxMailMergeOptions): IdxMailMergeDataService;
var
...
function TdxMailMergeDataService.CreateNew(ADataController
  : TdxRichEditDataControllerAdapterBase; const AOptions: IdxMailMergeOptions)
  : IdxMailMergeDataService;
begin
  Result := TdxMailMergeDataService.Create(ADataController);
end;

function TdxMailMergeDataService.CreateMailMergeDataController
  : TdxRichEditDataControllerAdapterBase;
begin
  Result := TdxRichEditDataControllerAdapter.Create;
end;

procedure TdxMailMergeDataService.EndMailMerge(const AFieldDataService
  : IdxMailMergeDataService);
begin
  TdxMailMergeDataService(AFieldDataService).DestroyDataController;
end;

function TdxMailMergeDataService.GetFieldValue(AMailMergeProperties
  : TdxMailMergeProperties; const AFieldName: string; AMapFieldName: Boolean;
  AOptions: TdxMailMergeDataMode; APieceTable: TdxCustomPieceTable;
...
begin
  ...
  AColumnIndex := GetColumnIndexByName(AMailMergeProperties, AFieldName,
    AMapFieldName);
...
function TdxMailMergeDataService.GetColumnIndexByName(AMailMergeProperties
  : TdxMailMergeProperties; const AFieldName: string;
  AMapFieldName: Boolean): Integer;
var
  ADataSourceObjectProperties: TdxDataSourceObjectProperties;
  AFieldMapData: TdxFieldMapData;
begin
  ADataSourceObjectProperties :=
    AMailMergeProperties.DataSourceObjectProperties;
  if AMapFieldName then
    Exit(ADataSourceObjectProperties.FindMapDataByMapName(AFieldName)
      .ColumnIndex)
  else
  begin
    AFieldMapData := ADataSourceObjectProperties.FindMapDataByColumnName
      (AFieldName);
