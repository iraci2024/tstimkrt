...
uses
  Types, Windows, SysUtils, Messages, Classes, Graphics, Generics.Defaults,
  Generics.Collections, Controls, StdCtrls,
  dxCore, cxClasses, dxMessages, cxControls, cxLookAndFeels, dxSpellChecker,
  dxSpellCheckerCore, dxSpellCheckerAdapters;
...
type
  ...
  TdxSpellCheckerAutoCorrectEngineCheckEvent = function
    ...
    var AInfo: TdxSpellCheckerAutoCorrectWordInfo): Boolean of object;
  ...
  protected
    function CheckRule(var AInfo: TdxSpellCheckerAutoCorrectWordInfo;
      ARule: IdxSpellCheckerAutoCorrectRule): Boolean;
    function DoAutoCorrect(ARule: IdxSpellCheckerAutoCorrectRule;
  ...
  public
    constructor Create(ASpellChecker: TdxCustomSpellChecker;
      ATextController: IdxSpellCheckTextController); reintroduce;
    destructor Destroy; override;
    procedure Check(var AInfo: TdxSpellCheckerAutoCorrectWordInfo);
    //
    property IgnoreCaretReturn: Boolean read FIgnoreCaretReturn
      write FIgnoreCaretReturn;
    property TextController: IdxSpellCheckTextController read FTextController;
    //
    property OnCheckEvent: TdxSpellCheckerAutoCorrectEngineCheckEvent
      read FOnCheckEvent write FOnCheckEvent;
  end;
  ...
  strict private
    ...
    function CheckHandler(ARule: IdxSpellCheckerAutoCorrectRule;
  ...
  TdxSpellCheckerCorrectSentenceCapsRule = class
  ...
  protected
    ...
    function IsFirstWordOfSentence(const ATextController
  ...
  TdxSpellCheckerCorrectCapsLockRule = class
    (TdxSpellCheckerAutoCorrectCustomRule)
  protected
    function GetActive: Boolean; override;
  public
  ...
  TdxSpellCheckerCorrectInitialCapsRule = class
    (TdxSpellCheckerAutoCorrectCustomRule)
  strict private
    FPrevWord: string;
  protected
  ...
  TdxSpellCheckerCorrectReplaceTextAsYouTypeRule = class
    (TdxSpellCheckerAutoCorrectCustomRule)
  protected
    function GetActive: Boolean; override;
    procedure InitializeDelimiters; override;
...
function ExtractChar(const AController: IdxSpellCheckTextController;
  const APosition: IdxSpellCheckerPosition): Char;
var
  AText: string;
begin
...
constructor TdxSpellCheckerAutoCorrectEngine.Create(ASpellChecker
  : TdxCustomSpellChecker; ATextController: IdxSpellCheckTextController);
begin
...
procedure TdxSpellCheckerAutoCorrectEngine.Check
  (var AInfo: TdxSpellCheckerAutoCorrectWordInfo);
var
  ARule: IdxSpellCheckerAutoCorrectRule;
  I: Integer;
begin
...
function TdxSpellCheckerAutoCorrectEngine.CheckRule
  ...
  begin
    Result := (AInfo.WordPositionStart.Compare(AInfo.SpellingAreaStart) <= 0) or
      TdxCharacters.IsWhiteSpace(ExtractChar(TextController,
      AInfo.WordPositionStart.MoveBackward));
  end;
...
begin
  Result := (ARule <> nil) and ARule.Active and
    ((AInfo.LastKey = #0) or ARule.IsWordDelimiter(AInfo.LastKey));
  if Result then
  begin
    ATempInfo := AInfo;
    Result := (Length(ATempInfo.Word) > 0) and IsBeginningOfWord and
      ARule.IsCheckWord(TextController, ATempInfo);
    if Result then
    begin
      ATempInfo.CursorLogPosition := ATempInfo.CursorLogPosition -
        Length(TextController.GetWord(ATempInfo.WordPositionStart,
        ATempInfo.WordPositionFinish)) + Length(ATempInfo.Word) + 1 +
        GetSelStartDelta;
...
begin
  FRules.Clear;
  FRules.Add(TdxSpellCheckerCorrectReplaceTextAsYouTypeRule.Create
    (SpellChecker.AutoCorrectOptions));
  FRules.Add(TdxSpellCheckerCorrectCapsLockRule.Create
    (SpellChecker.AutoCorrectOptions));
  FRules.Add(TdxSpellCheckerCorrectInitialCapsRule.Create
    (SpellChecker.AutoCorrectOptions));
  FRules.Add(TdxSpellCheckerCorrectSentenceCapsRule.Create
    (SpellChecker.AutoCorrectOptions));
end;
...
begin
  ...
  Result := DoAutoCorrect(ARule as TdxSpellCheckerAutoCorrectCustomRule,
    AWordRange);
  if Result and (AWordRange <> AInfo.ToWordRange) then
    AInfo := TdxSpellCheckerAutoCorrectWordInfo.Create(AWordRange,
      Engine.TextController);
end;

function TdxSpellCheckerAutoCorrectManager.GetAdapter
  : IdxSpellCheckerAutoCorrectAdapter;
begin
  Result := inherited Adapter as IdxSpellCheckerAutoCorrectAdapter;
end;

function TdxSpellCheckerAutoCorrectManager.GetAutoCorrectWordRange
  : TdxSpellCheckerAutoCorrectWordInfo;
var
  AChar: Char;
begin
  Result := TdxSpellCheckerAutoCorrectWordInfo.Create;
  Adapter.GetSpellingBounds(Result.SpellingAreaStart,
    Result.SpellingAreaFinish);
  Adapter.UpdateController(Engine.TextController);
  Engine.TextController.SetSpellingArea(Result.SpellingAreaStart,
    Result.SpellingAreaFinish);
  ...
  begin
    AChar := ExtractChar(Engine.TextController,
      Result.WordPositionStart.MoveBackward);
...
function TdxSpellCheckerAutoCorrectManager.GetEngine
  : TdxSpellCheckerAutoCorrectEngine;
begin
  if FEngine = nil then
  begin
    FEngine := TdxSpellCheckerAutoCorrectEngine.Create(SpellChecker,
...
begin
  FPrevWord := '';
  Result := (AInfo.LastKey <> '.') or
    (Options.FirstLetterExceptions.Find(AInfo.Word + '.') = -1);
...
begin
  ...
  while (APosition.Compare(AInfo.SpellingAreaStart) > 0) and
    (ExtractChar(ATextController, APosition.MoveBackward) = ' ') do
...
function TdxSpellCheckerCorrectSentenceCapsRule.IsSentenceDelimiter
  (ACh: WideChar): Boolean;
begin
...
function TdxSpellCheckerCorrectCapsLockRule.IsCheckWord(const ATextController
  : IdxSpellCheckTextController;
  var AInfo: TdxSpellCheckerAutoCorrectWordInfo): Boolean;
var
  I: Integer;
begin
  Result := Active and (Length(AInfo.Word) > 1) and
...
function TdxSpellCheckerCorrectInitialCapsRule.IsCheckWord(const ATextController
  : IdxSpellCheckTextController;
  var AInfo: TdxSpellCheckerAutoCorrectWordInfo): Boolean;
var
  I: Integer;
...
begin
  ...
  else if not WideIsAlphaNumeric(AInfo.LastKey) and
    Options.GetReplacement(AInfo.Word + AInfo.LastKey, AReplacement) then
  begin
    AInfo.Word := WidePatternCase(AInfo.Word, AReplacement);
    AInfo.WordPositionFinish := ATextController.CreatePosition
      (AInfo.WordPositionFinish.ToInteger + 1);
    Result := True;
  end;
end;
