...
begin
  ...
  try
    ...
    if ATarget = nil then
      ATarget := FAdaptersForNewComponents[cbTarget.ItemIndex].CreateComponent
        (FComponent.Owner);
    ...
    begin
      ...
      try
        AAdapter.ActionList :=
          TActionList(cbActionLists.Items.Objects[cbActionLists.ItemIndex]);
        AAdapter.BeginUpdate;
        try
          for I := 0 to clbCategories.Count - 1 do
          begin
            if clbCategories.Checked[I] then
              AAdapter.Generate(clbCategories.Items.Objects[I]
                as TdxUIGeneratorCategoryInfo);
          end;
        finally
          AAdapter.EndUpdate;
        end;
...
begin
  ...
  try
    ...
    for I := 0 to FAdaptersForNewComponents.Count - 1 do
      cbTarget.Items.Add(Format(sCreateNewTarget,
        [FAdaptersForNewComponents[I].GetComponentClass.ClassName]));
    ...
    cbTarget.ItemIndex := FAdaptersForNewComponents.Count -
      Ord(cbTarget.Items.Count <= FAdaptersForNewComponents.Count);
  finally
