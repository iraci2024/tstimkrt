...
{$IFDEF DELPHI16}
  UITypes,
{$ENDIF}
  Types, Windows, Classes, Graphics, Variants, ExtCtrls, dxCore, dxCoreGraphics,
  cxGeometry;
...
type
  ...
  TdxImageDataFormat = (dxImageUnknown, dxImageBitmap, dxImageJpeg, dxImagePng,
    dxImageTiff, dxImageGif, dxImageEmf, dxImageExif, dxImageIcon,
    dxImageMemoryBmp, dxImageWmf);
  ...
  public
    ...
    procedure Draw(DC: HDC; const ADest, ASource: TdxRectF; AAlpha: Byte = 255;
      APalette: IdxColorPalette = nil); overload; virtual;
    procedure Draw(DC: HDC; const ADest, ASource: TRect; AAlpha: Byte = 255;
      APalette: IdxColorPalette = nil); overload; virtual; abstract;
  ...
  public
    ...
    class function Load(AStream: TStream;
      out AHandle: TdxSmartImageCustomHandle): Boolean; overload; virtual;
  ...
  TdxSmartImageAnimationControllerClass = class of
    TdxSmartImageAnimationController;
  ...
  protected
    ...
    procedure LoadFromBits(const ABits: TRGBColors; const ASize: TSize;
  ...
  protected
    ...
    function CanCreateCache(const ASourceSize, ATargetSize: TSize)
      : Boolean; virtual;
    function CheckCache(const ASize: TSize;
      const APalette: IdxColorPalette): Boolean;
    function CreateCache(const ASize: TSize)
      : TdxSmartImageCustomHandle; virtual;
    ...
    procedure CreateHandleFromBits(AWidth, AHeight: Integer;
      const ABits: TRGBColors; AAlphaFormat: TAlphaFormat);
    ...
    function ExportToStream(ACodec: TdxSmartImageCodecClass)
      : TdxSmartImageDataStream;
    ...
    property AnimationController: TdxSmartImageAnimationController
  ...
  public
    ...
    constructor CreateSize(const ARect: TRect;
      AColor: TdxAlphaColor = 0); overload;
    constructor CreateSize(const ASize: TSize;
      AColor: TdxAlphaColor = 0); overload;
    ...
    constructor CreateFromBits(AWidth, AHeight: Integer;
      const ABits: TRGBColors; AHasAlphaChannel: Boolean); overload; virtual;
    constructor CreateFromBits(AWidth, AHeight: Integer;
      const ABits: TRGBColors; AAlphaFormat: TAlphaFormat); overload; virtual;
    constructor CreateFromBits(AWidth, AHeight: Integer; const ABits: PRGBQuad;
      AAlphaFormat: TAlphaFormat); overload; virtual;
    constructor CreateFromFile(const AFileName: string);
    constructor CreateFromStream(AStream: TStream); virtual;
    destructor Destroy; override;

    procedure Assign(ASource: TPersistent); override;
    procedure LoadFromBits(AWidth, AHeight: Integer; const ABits: TRGBColors;
      AHasAlphaChannel: Boolean); overload;
    procedure LoadFromBits(AWidth, AHeight: Integer; const ABits: TRGBColors;
      AAlphaFormat: TAlphaFormat); overload;
    procedure LoadFromClipboardFormat(AFormat: Word; AData: THandle;
      APalette: HPALETTE); override;
    procedure LoadFromFieldValue(const AValue: Variant);
    procedure LoadFromResource(AInstance: THandle; const AResName: string;
      AResType: PChar);
    ...
    procedure SaveToStreamByCodec(AStream: TStream;
      ACodec: TdxSmartImageCodecClass); overload;
    procedure SaveToStreamByCodec(AStream: TStream;
      AFormat: TdxImageDataFormat); overload;
    ...
    property AnimationFrameDelay: Integer read GetAnimationFrameDelay
      write SetAnimationFrameDelay;
    property AnimationLoop: TdxDefaultBoolean read FAnimationLoop
      write FAnimationLoop default bTrue;
    property AnimationLoopIndex: Integer read FAnimationLoopIndex
      write FAnimationLoopIndex;
    ...
    property ImageCodec: TdxSmartImageCodecClass read GetImageCodec
      write SetImageCodec;
    property ImageDataFormat: TdxImageDataFormat read GetImageDataFormat
      write SetImageDataFormat; // deprecated, use ImageCodec instead
  ...
  public
    ...
    class function GetFormatByID(ID: TdxImageDataFormat)
      : TdxSmartImageCodecClass;
    class function GetFormatFromStream(AStream: TStream)
      : TdxSmartImageCodecClass;

    class function GetImageInfo(const AFileName: string; out ASize: TSize;
      out ACodec: TdxSmartImageCodecClass): Boolean; overload;
    class function GetImageInfo(const AImageData: TdxSmartImageData;
      out ASize: TSize; out ACodec: TdxSmartImageCodecClass): Boolean; overload;
    class function GetImageInfo(const AStream: TStream; out ASize: TSize;
      out ACodec: TdxSmartImageCodecClass): Boolean; overload;
    ...
    class function GetSaveDialogFilter
      (AHandle: TdxSmartImageCustomHandle = nil): string;
    class function GetSupportedExtensions: string;

    class function Load(const ABits: TRGBColors; AAlphaFormat: TAlphaFormat;
      AWidth, AHeight: Integer; out AHandle: TdxSmartImageCustomHandle)
      : Boolean; overload;
    class function Load(const AImageData: TdxSmartImageData;
      out AHandle: TdxSmartImageCustomHandle): Boolean; overload;
    ...
    class property RegisterInVCL: Boolean read FRegisterInVCL
      write SetRegisterInVCL;
  end;
  ...
  public
    class function AllocColors(AWidth, AHeight: Integer;
      AColor: TdxAlphaColor = 0): TRGBColors;
...
var
  ImageAnimationControllerClass: TdxSmartImageAnimationControllerClass =
    TdxSmartImageAnimationController;
...
uses
  SysUtils, Math, Clipbrd, dxHash, dxThreading, Consts, StrUtils,
  dxStringHelper;
...
const
  ...
  SupportedFormats: array [0 .. 4] of TdxImageDataFormat = (dxImagePng,
    dxImageGif, dxImageTiff, dxImageJpeg, dxImageBitmap);

  HasAlphaChannelFormatMap: array [Boolean] of TAlphaFormat = (afIgnored,
    afPremultiplied);
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := CompareMem(@AColors1[0], @AColors2[0],
        SizeOf(AColors1[0]) * Length(AColors1));
    end;
  end;
end;
...
procedure TdxSmartImageEmptyHandle.Draw(DC: HDC; const ADest, ASource: TRect;
...
begin
  Result := UpperCase(TdxSmartImageCodecsRepository.GetDefaultExt(Self)) +
    dxSmartImageCodecDescriptionSuffix;
end;
...
class function TdxSmartImageCodec.GetGraphicClassForRegistrationInVCL
  : TGraphicClass;
begin
  Result := nil;
end;
...
class function TdxSmartImageCodec.GetSize(AStream: TStream;
...
class function TdxSmartImageCodec.CanSaveImage
  (AHandle: TdxSmartImageCustomHandle): Boolean;
begin
  Result := False;
end;

class function TdxSmartImageCodec.Load(const ABits: TRGBColors;
...
class function TdxSmartImageCodec.Load(AStream: TStream;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
begin
  Result := False;
end;

class function TdxSmartImageCodec.Save(AStream: TStream;
  AHandle: TdxSmartImageCustomHandle): Boolean;
begin
  Result := False;
end;
...
constructor TdxSmartImageAnimationController.Create
...
begin
  ...
  begin
    ...
    if FBitsFormat = afIgnored then
    begin
      if (cxGetBitmapPixelFormat(ABitmap) = 32) and
        TdxSmartImageHelper.HasAlpha(FBits) then
      begin
...
procedure TdxSmartImageData.LoadFromBits(const ABits: TRGBColors;
  const ASize: TSize; AAlphaFormat: TAlphaFormat);
begin
  FreeImageData;
  FBits := ABits;
...
constructor TdxCustomSmartImage.CreateFromBits(AWidth, AHeight: Integer;
  const ABits: TRGBColors; AHasAlphaChannel: Boolean);
begin
  CreateFromBits(AWidth, AHeight, ABits,
    HasAlphaChannelFormatMap[AHasAlphaChannel]);
end;
...
constructor TdxCustomSmartImage.CreateFromBits(AWidth, AHeight: Integer;
  const ABits: PRGBQuad; AAlphaFormat: TAlphaFormat);
var
  AColors: TRGBColors;
  ACount: Integer;
begin
...
constructor TdxCustomSmartImage.CreateSize(const ARect: TRect;
  AColor: TdxAlphaColor);
begin
  CreateSize(cxRectWidth(ARect), cxRectHeight(ARect), AColor);
end;

constructor TdxCustomSmartImage.CreateSize(const ASize: TSize;
  AColor: TdxAlphaColor);
begin
  CreateSize(ASize.cx, ASize.cy, AColor);
end;

constructor TdxCustomSmartImage.CreateSize(AWidth, AHeight: Integer;
  AColor: TdxAlphaColor);
begin
  CreateFromBits(AWidth, AHeight, TdxSmartImageHelper.AllocColors(AWidth,
    AHeight, AColor), True);
end;
...
var
  ABitmap: TBitmap;
begin
  if not Empty and not(ImageDataFormat in [dxImageBitmap, dxImageMemoryBmp])
  then
  begin
    ABitmap := GetAsBitmap;
    try
...
begin
  ...
  begin
    if IsClipboardFormatAvailable(ImageClipboardFormats[SupportedFormats[I]])
    then
      Exit(True);
  end;
end;

class function TdxCustomSmartImage.IsSupportClipboardFormat
  (AFormat: Word): Boolean;
var
...
procedure TdxCustomSmartImage.LoadFromBits(AWidth, AHeight: Integer;
  const ABits: TRGBColors; AHasAlphaChannel: Boolean);
begin
  LoadFromBits(AWidth, AHeight, ABits,
    HasAlphaChannelFormatMap[AHasAlphaChannel]);
end;
...
procedure TdxCustomSmartImage.LoadFromClipboardFormat(AFormat: Word;
...
begin
  ...
  else if IsSupportClipboardFormat(AFormat) then
  begin
...
procedure TdxCustomSmartImage.LoadFromResource(AInstance: THandle;
  const AResName: string; AResType: PChar);
var
  ABitmap: TBitmap;
  AStream: TStream;
begin
...
procedure TdxCustomSmartImage.SaveToClipboardFormat(var AFormat: Word;
...
begin
  ...
  try
    ACodec := TdxSmartImageCodecsRepository.GetFormatByExt
...
procedure TdxCustomSmartImage.SaveToStreamByCodec(AStream: TStream;
  ACodec: TdxSmartImageCodecClass);
var
...
begin
  SaveToStreamByCodec(AStream, TdxSmartImageCodecsRepository.GetFormatByID
    (AFormat));
end;
...
procedure TdxCustomSmartImage.StretchDraw(DC: HDC; const ADest, ASource: TRect;
  AAlpha: Byte; APalette: IdxColorPalette);
begin
  Handle.Draw(DC, ADest, ASource, AAlpha, APalette);
end;

procedure TdxCustomSmartImage.StretchDraw(DC: HDC; const ADest: TRect;
  AAlpha: Byte; APalette: IdxColorPalette);
begin
...
begin
  Result := Supports(Handle, IdxAnimatedImage) and
    (ImageDataFormat <> dxImageTiff);
end;

function TdxCustomSmartImage.CanCreateCache(const ASourceSize,
  ATargetSize: TSize): Boolean;
begin
  Result := (ASourceSize.cx > ATargetSize.cx) and
    (ASourceSize.cy > ATargetSize.cy) and
    (ASourceSize.cx * ASourceSize.cy > 4 * ATargetSize.cx * ATargetSize.cy);
end;

function TdxCustomSmartImage.CheckCache(const ASize: TSize;
  const APalette: IdxColorPalette): Boolean;
begin
...
function TdxCustomSmartImage.CreateCache(const ASize: TSize)
  : TdxSmartImageCustomHandle;
begin
  Result := nil;
end;
...
begin
  ...
  begin
    ...
    begin
      APalette := 0;
      LoadFromClipboardFormat(ImageClipboardFormats[SupportedFormats[I]], AData,
        APalette);
      Exit(True);
    end;
  end;
...
begin
  if ADest is TdxCustomSmartImage then
    ADest.Assign(Self)
  else if ADest is TMetafile then
    AssignToMetaFile(TMetafile(ADest))
  else if ADest is TBitmap then
  begin
    if Empty then
    begin
...
var
  AStream: TMemoryStream;
begin
  AStream := ExportToStream(TdxSmartImageCodecsRepository.GetFormatByID
    (FormatMap[ADest.Enhanced]));
  try
    AStream.Position := 0;
...
procedure TdxCustomSmartImage.CreateHandleFromBits(AWidth, AHeight: Integer;
  const ABits: TRGBColors; AAlphaFormat: TAlphaFormat);
begin
  FreeHandle;
...
procedure TdxCustomSmartImage.CreateHandleFromHBITMAP(ABitmap: HBITMAP;
  APalette: HPALETTE);
var
  AImage: TBitmap;
begin
...
begin
  if ImageData.Empty then
    ...
    Check(TdxSmartImageCodecsRepository.Load(ImageData, FHandle),
      sdxErrorUnsupportedImageFormat);
...
function TdxCustomSmartImage.ExportToStream(ACodec: TdxSmartImageCodecClass)
  : TdxSmartImageDataStream;
begin
  Check(ACodec <> nil, sdxErrorUnsupportedOperation);
  if (ACodec.ID = ImageDataFormat) and (ImageData.FData <> nil) and
    (ACodec.ID <> dxImageUnknown) then
    Result := ImageData.FData.Clone
  else
  begin
    Result := TdxSmartImageDataStream.Create;
    try
      Check(ACodec.Save(Result, Handle), Format(sdxErrorCannotExportImage,
        [ACodec.ClassName, Ord(ImageDataFormat)]));
      Result.Position := 0;
    except
      FreeAndNil(Result);
      raise;
...
begin
  if not HandleAllocated then
  begin
    if not TdxSmartImageCodecsRepository.GetImageInfo(ImageData, ASize, ACodec)
    then
      HandleNeeded;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      CreateHandleFromBits(ASize.cx, ASize.cy,
        TdxSmartImageHelper.AllocColors(ASize.cx, ASize.cy), afPremultiplied);
      Exit(True);
    end;
...
begin
  ...
  else if Supports(Handle, IdxImageDataFormat, AIntf) then
...
begin
  ...
  begin
    ...
    if TdxSmartImageCodecsRepository.GetImageInfo(ImageData, Result, ACodec)
    then
    begin
...
begin
  ...
  begin
    ...
    begin
      if (AnimationLoop = bTrue) or
        ((AnimationLoop = bDefault) and ((AnimationLoopCount = 0) or
        (AnimationLoopIndex < AnimationLoopCount))) then
      begin
        AValue := 0;
...
procedure TdxCustomSmartImage.SetImageCodec(const AValue
  : TdxSmartImageCodecClass);
var
  AStream: TMemoryStream;
begin
...
class function TdxSmartImageCodecsRepository.Contains
  (ACodec: TdxSmartImageCodecClass): Boolean;
begin
  Result := (Count > 0) and (FList.IndexOf(ACodec) >= 0);
end;

class function TdxSmartImageCodecsRepository.GetFormatFromStream
  (AStream: TStream): TdxSmartImageCodecClass;
var
  I: Integer;
begin
...
class function TdxSmartImageCodecsRepository.GetImageInfo(const AFileName
  : string; out ASize: TSize; out ACodec: TdxSmartImageCodecClass): Boolean;
var
  AStream: TFileStream;
begin
...
class function TdxSmartImageCodecsRepository.GetImageInfo(const AImageData
  : TdxSmartImageData; out ASize: TSize;
  out ACodec: TdxSmartImageCodecClass): Boolean;
var
  I: Integer;
begin
  Result := False;
...
class function TdxSmartImageCodecsRepository.GetImageInfo
  (const AStream: TStream; out ASize: TSize;
  out ACodec: TdxSmartImageCodecClass): Boolean;
var
  APosition: Int64;
  I: Integer;
...
class function TdxSmartImageCodecsRepository.CanLoad(const AFileName
  : string): Boolean;
begin
  Result := GetFormatByExt(ExtractFileExt(AFileName)) <> nil;
end;

class function TdxSmartImageCodecsRepository.CanLoad(const AStream
  : TStream): Boolean;
begin
...
class function TdxSmartImageCodecsRepository.GetSaveDialogFilter
  (AHandle: TdxSmartImageCustomHandle = nil): string;
var
  ACodec: TdxSmartImageCodecClass;
...
begin
  ...
  try
    ...
    Result := Format('%0:s (%1:s)|%1:s|%2:s', [sAllFilter, AExts.ToString,
      AFilter.ToString]);
  finally
...
class function TdxSmartImageCodecsRepository.GetDefaultExt
  (ACodec: TdxSmartImageCodecClass): string;
var
...
class function TdxSmartImageCodecsRepository.GetFormatByExt(const Ext: string)
  : TdxSmartImageCodecClass;
var
  E: string;
  I: Integer;
...
class function TdxSmartImageCodecsRepository.GetFormatByID
  (ID: TdxImageDataFormat): TdxSmartImageCodecClass;
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
...
class function TdxSmartImageCodecsRepository.Load(const ABits: TRGBColors;
  AAlphaFormat: TAlphaFormat; AWidth, AHeight: Integer;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
var
  I: Integer;
...
class function TdxSmartImageCodecsRepository.Load(const AImageData
  : TdxSmartImageData; out AHandle: TdxSmartImageCustomHandle): Boolean;
begin
  if Length(AImageData.FBits) > 0 then
    Result := Load(AImageData.FBits, AImageData.FBitsFormat,
      AImageData.FSize.cx, AImageData.FSize.cy, AHandle)
  else
  begin
    AImageData.FData.Position := 0;
...
class function TdxSmartImageCodecsRepository.Load(const AStream: TStream;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
var
  ASavedPosition: Int64;
  I: Integer;
...
class procedure TdxSmartImageCodecsRepository.Register
  (ACodec: TdxSmartImageCodecClass);
var
...
begin
  ...
  begin
    ...
    if ACodec.GetGraphicClassForRegistrationInVCL <> nil then
      TPicture.RegisterClipboardFormat(AClipboardFormat,
        ACodec.GetGraphicClassForRegistrationInVCL);
  end;
...
class procedure TdxSmartImageCodecsRepository.Unregister
  (ACodec: TdxSmartImageCodecClass);
begin
  if FList <> nil then
  begin
...
class procedure TdxSmartImageCodecsRepository.DoRegisterInVCL
  (ACodec: TdxSmartImageCodecClass);
var
  AExtensions: TStringDynArray;
  ...
  begin
    ...
    begin
      ...
      for I := Low(AExtensions) to High(AExtensions) do
        TPicture.RegisterFileFormat(AExtensions[I], ACodec.Description,
          AGraphicClass);
    end
    else if not AIsAlreadyRegistered then
      TPicture.RegisterFileFormat('', '', AGraphicClass);
  end;
end;

class procedure TdxSmartImageCodecsRepository.DoUnregisterFromVCL
  (ACodec: TdxSmartImageCodecClass);
var
  AGraphicClass: TGraphicClass;
  I: Integer;
...
class function TdxSmartImageCodecsRepository.GetItem(Index: Integer)
  : TdxSmartImageCodecClass;
begin
...
class function TdxSmartImageCodecsRepository.SplitExtensions
  (const ExtList: string): TStringDynArray;
var
  I: Integer;
begin
...
class function TdxSmartImageHelper.AllocColors(AWidth, AHeight: Integer;
  AColor: TdxAlphaColor): TRGBColors;
var
  I: Integer;
begin
...
class function TdxSmartImageHelper.AreColorsPremultiplied
  (const ABits: TRGBColors): Boolean;
var
  Q: TRGBQuad;
  I: Integer;
begin
