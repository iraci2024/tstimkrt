...
type
  ...
  TdxRtfTableCellPropertiesCollection = class
    (TdxRtfTableCustomItemCollection<TdxRtfTableCellProperties>);
  ...
  public
    ...
    procedure OnEndParagraph(AParagraphFormattingInfo
      : TdxRtfParagraphFormattingInfo); virtual; abstract;
  ...
  protected
    ...
    function IsParagraphInTable(AParagraphFormattingInfo
      : TdxRtfParagraphFormattingInfo): Boolean;
  ...
  public
    constructor Create(AReader: TdxRtfTableReader);
    procedure OnEndParagraph(AParagraphFormattingInfo
      : TdxRtfParagraphFormattingInfo); override;
  ...
  public
    ...
    property CellPropertiesCollection: TdxRtfTableCellPropertiesCollection
      read FCellPropertiesCollection;
  end;
  ...
  protected
    procedure SetParagraphIndexCore(ACell: TdxRtfTableCell;
  ...
  TdxRtfTableCellCollection = class
    (TdxRtfTableCustomItemCollection<TdxRtfTableCell>);
  ...
  public
    ...
    property DefineProperties: Boolean read FDefineProperties
      write FDefineProperties;
    property Right: Integer read GetRight;
  end;

  TdxRtfTableRowCollection = class
    (TdxRtfTableCustomItemCollection<TdxRtfTableRow>);
  ...
  public
    constructor Create(ARow: TdxRtfTableRow; AImporter: TObject;
      ARight: Integer = 0);
  ...
  TdxRtfParentCellMap = TObjectDictionary<TdxRtfTableCell,
  ...
  public
    ...
    property ProcessedBorder: TdxBorderBase read FProcessedBorder
      write FProcessedBorder;
    property IsNestedTableProperetiesReading: Boolean
      read FIsNestedTableProperetiesReading;
  ...
  protected
    ...
    procedure PrepareRtfRow(ARow: TdxRtfTableRow; AGrid: TdxRtfTableGrid;
      ATableLayoutType: TdxTableLayoutType); virtual;
    ...
    procedure RemoveCell(ACells: TdxRtfTableCellCollection;
      ACell: TdxRtfTableCell);
    procedure RecalcParagraphIndexes(ARemovedCell: TdxRtfTableCell);
    procedure RecalcParagraphIndexesInRow(ARow: TdxRtfTableRow;
      ACellIndex: Integer; ADelta: Integer);
    procedure RecalcParagraphIndexesInTable(ATable: TdxRtfTable;
      ARowIndex: Integer; ADelta: Integer; AParagraphIndex: TdxParagraphIndex);
    procedure RecalcParagraphIndexesInTables(ADelta: Integer;
      AParagraphIndex: TdxParagraphIndex);
  public
    constructor Create(ATableReader: TdxRtfTableReader);
    destructor Destroy; override;
    procedure ConvertTables(ARtfTables: TdxRtfTableCollection;
      AIsCopySingleCellAsText: Boolean);
...
type
  ...
  protected
    class function Merge(ASource: TdxRtfTableGrid;
      ADestination: TdxRtfTableGrid): TdxRtfTableGrid; virtual;
    class function GetRowColumns(ARow: TdxRtfTableRow)
      : TdxRtfTableGrid; virtual;
  public
    class function Calculate(ATable: TdxRtfTable; ATableIndent: Integer)
      : TdxRtfTableGrid;
  end;
...
constructor TdxRtfTableCell.Create(ARow: TdxRtfTableRow; AImporter: TObject;
  ARight: Integer);
var
...
begin
  ...
  FCellProperties := TdxRtfTableCellProperties.Create
    (ARtfImporter.PieceTable, Self);
...
procedure TdxTableRtfTableManagerState.OnEndParagraph(AParagraphFormattingInfo
  : TdxRtfParagraphFormattingInfo);
begin
  if (IsParagraphInTable(AParagraphFormattingInfo)) or
    (IsCurrentTableNotComplete) then
...
procedure TdxTableRtfTableManagerState.OnEndInTableParagraph
...
function TdxTableRtfTableManagerState.IsParagraphInTable
  (AParagraphFormattingInfo: TdxRtfParagraphFormattingInfo): Boolean;
begin
  Result := AParagraphFormattingInfo.InTableParagraph or
    (AParagraphFormattingInfo.NestingLevel > 0);
end;
...
begin
  Result := Reader.TableController.RowController.IsCurrentRowNotComplete or
    (Reader.TableController.RowController.IsCurrentRowValid and
    (Reader.TableController.RowController.CurrentRow.Cells.Count <
    Reader.CellPropertiesCollection.Count));
end;
...
begin
  ...
  if (not Reader.IsNestedTableProperetiesReading) and (ACurrentTable <> nil) and
    (ACurrentTable.NestingLevel > 1) then
...
begin
  ...
  if (FCellPropertiesCollection.Count = 0) or
    (FCellPropertiesCollection.Last <> CellProperties) then
...
constructor TdxRtfTable.Create(AImporter: TObject;
...
begin
  Result := UseHalfSpace or UseLeftMargin or UseRightMargin or UseTopMargin or
    UseBottomMargin or UseCellSpacing or UseIsTableOverlap or
    UsePreferredWidth or UseTableIndent or
    (TableLayout = TdxTableLayoutType.Autofit) or UseTableLook or
    UseTableStyleColBandSize or UseTableStyleRowBandSize or
    Borders.UseBottomBorder or Borders.UseInsideHorizontalBorder or
    Borders.UseInsideVerticalBorder or Borders.UseLeftBorder or
    Borders.UseRightBorder or Borders.UseTopBorder;
end;
...
constructor TdxRtfTableRowController.Create(ATableController
  : TdxRtfTableController);
begin
  inherited Create;
...
begin
  ...
  begin
    ...
    ATableProperties.FloatingPosition.CopyFrom
      (TableController.Reader.TableProperties.FloatingPosition);
  end;
  ATableProperties.TableStyleIndex := TableController.Reader.TableProperties.
    TableStyleIndex;
  CurrentRow.Properties.CopyFrom(TableController.Reader.RowProperties);
  CurrentRow.Properties.FloatingPosition.CopyFrom
    (TableController.Reader.RowProperties.FloatingPosition);
...
begin
  Result := TableController.CurrentTable.Rows.Contains(CurrentRow) and
    (CurrentRow.Cells.Count > 0);
end;
...
function TdxRtfTableRowController.CreateCellController
  : TdxRtfTableCellController;
begin
...
constructor TdxRtfTableCellController.Create(ARowController
  : TdxRtfTableRowController);
begin
  inherited Create;
...
begin
  Result := not RowController.CurrentRow.Cells.Contains(CurrentCell) and
    not CurrentCell.IsEmpty;
end;
...
begin
  ...
  FCurrentCell := TdxRtfTableCell.Create(ARow,
    RowController.TableController.Reader.Importer);
...
function TdxNoTableRtfTableReaderState.ChangeState
  : TdxTableRtfTableManagerState;
begin
...
procedure TdxNoTableRtfTableReaderState.OnEndParagraph(AParagraphFormattingInfo
  : TdxRtfParagraphFormattingInfo);
var
...
begin
  inherited Create(APieceTable);
  FFloatingPosition := APieceTable.DocumentModel.Cache.
    TableFloatingPositionInfoCache.DefaultItem.Clone;
...
begin
  inherited Create;
  FIndex := -1;
  FRowProperties := TdxRtfTableRowProperties.Create(TdxRtfImporter(AImporter)
    .PieceTable);
...
procedure TdxRtfTableConverter.ConvertTables(ARtfTables: TdxRtfTableCollection;
  AIsCopySingleCellAsText: Boolean);
var
  ATable: TdxRtfTable;
begin
...
begin
  ...
  if (ACell.StartParagraphIndex <> 0) or
    (ACell.EndParagraphIndex <> PieceTable.Paragraphs.Count - 2) then
    Exit(False);
  Result := True;
end;
...
begin
  ...
  begin
    ...
    ATable.Properties.FloatingPosition.CopyFrom
      (ARow.Properties.FloatingPosition);
  end;
  ...
  if (not ATable.Properties.UseRightMargin and
    not ATable.Properties.UseLeftMargin) and ATable.Properties.UseHalfSpace then
  begin
...
function TdxRtfTableConverter.ShouldUseFloatingPosition
  (ATable: TdxRtfTable): Boolean;
var
...
function TdxRtfTableConverter.CalculateTableLeftOffset
  (ATable: TdxRtfTable): Integer;
var
  ARows: TdxRtfTableRowCollection;
...
function TdxRtfTableConverter.CalculateTableIndent(ATable: TdxRtfTable)
  : Integer;
var
...
begin
  ...
  try
    ...
    Result := Max(ABorderWidth div 2, GetActualWidth(ALeftMargin.Info)) +
      ATable.Indent;
  finally
    ACalculator.Free;
  end;
...
function TdxRtfTableConverter.GetCellLeftMargin(ATable: TdxRtfTable)
  : TdxWidthUnit;
var
  ACellProperties: TdxRtfTableCellProperties;
begin
  ACellProperties := ATable.Rows.First.Cells.First.Properties;
  if ACellProperties.CellMargins.Left.Info.&Type = TdxWidthUnitType.ModelUnits
  then
    Exit(ACellProperties.CellMargins.Left);
  Result := ATable.Properties.CellMargins.Left;
end;

function TdxRtfTableConverter.GetActualWidth
  (AUnitInfo: TdxWidthUnitInfo): Integer;
begin
  Result := 0;
...
procedure TdxRtfTableConverter.ConvertTableCore(ATable: TdxTable;
  ARtfTable: TdxRtfTable);
var
...
begin
  ...
  ATable.TableProperties.FloatingPosition.CopyFrom
    (ARtfTable.Properties.FloatingPosition);
...
function TdxRtfTableConverter.CalculateTableGrid(ATable: TdxRtfTable)
  : TdxRtfTableGrid;
begin
  Result := TdxRtfTableColumnsCalculator.Calculate(ATable, ATable.Indent);
end;

procedure TdxRtfTableConverter.PrepareRtfRow(ARow: TdxRtfTableRow;
  AGrid: TdxRtfTableGrid; ATableLayoutType: TdxTableLayoutType);
var
...
begin
  ...
  AGridAfter := Max(0, ALastColumnIndex - CalculateRowColumnSpan(ARow) -
    AGridBefore);
  ...
  begin
    if ARow.Properties.WidthAfter.&Type = TdxWidthUnitType.ModelUnits then
      ...
      AWidthAfter := AGrid[ALastColumnIndex] -
        AGrid[ALastColumnIndex - AGridAfter];
    ARow.Properties.WidthAfter.Value := AWidthAfter;
    ARow.Properties.WidthAfter.&Type := TdxWidthUnitType.ModelUnits;
  end;
end;

function TdxRtfTableConverter.CalculateWidthAfter(ARow: TdxRtfTableRow)
  : Integer;
var
...
function TdxRtfTableConverter.CalculateTotalRowWidth
  (ARow: TdxRtfTableRow): Integer;
var
...
procedure TdxRtfTableConverter.PrepareRtfRowCells(ARow: TdxRtfTableRow;
  AGrid: TdxRtfTableGrid; AGridBefore: Integer;
  ATableLayoutType: TdxTableLayoutType);
var
...
begin
  ...
  begin
    ...
    ABorderIndex := AGrid.BinarySearchRight(ARight) -
      CalculateEquidistantCellOrder(ARow.Cells, I, ARight);
    ...
    if (APreferredWidth.&Type = TdxWidthUnitType.Nil) or
      ((APreferredWidth.&Type = TdxWidthUnitType.Auto) and
      (ATableLayoutType <> TdxTableLayoutType.Autofit)) then
    begin
...
function TdxRtfTableConverter.CalculateEquidistantCellOrder
  (ACells: TdxRtfTableCellCollection; AIndex: Integer; ALeft: Integer): Integer;
var
  ACount, I: Integer;
begin
  Result := 0;
...
function TdxRtfTableConverter.CalculateRowColumnSpan
  (ARow: TdxRtfTableRow): Integer;
var
...
procedure TdxRtfTableConverter.ConvertRow(ARow: TdxTableRow;
  ARtfRow: TdxRtfTableRow);
var
...
procedure TdxRtfTableConverter.ConvertCell(ARow: TdxTableRow;
  ARtfCell: TdxRtfTableCell);
var
  ACell: TdxTableCell;
...
begin
  ...
  PieceTable.TableCellsManager.InitializeTableCell(ACell, AStartParagraphIndex,
    AEndParagraphIndex);
...
begin
  ...
  while (ANextIndex < ACells.Count) and
    (ACells[ANextIndex].Properties.HorizontalMerging = TdxMergingState.
    Continue) do
  begin
    ...
    AFirstCell.Properties.ColumnSpan := AFirstCell.Properties.ColumnSpan +
      ANextRtfCell.Properties.ColumnSpan;
    RemoveCell(ACells, ANextRtfCell);
  end;
end;

procedure TdxRtfTableConverter.RemoveCell(ACells: TdxRtfTableCellCollection;
  ACell: TdxRtfTableCell);
var
...
procedure TdxRtfTableConverter.RecalcParagraphIndexes
  (ARemovedCell: TdxRtfTableCell);
var
...
begin
  ...
  RecalcParagraphIndexesInTable(ATable, ARowIndex + 1, ADelta,
    ARemovedStartParagraphIndex);
  RecalcParagraphIndexesInTables(ADelta, ARemovedStartParagraphIndex);
end;

procedure TdxRtfTableConverter.RecalcParagraphIndexesInRow(ARow: TdxRtfTableRow;
  ACellIndex: Integer; ADelta: Integer);
var
  ACount, I: Integer;
begin
  ...
  begin
    ARow.Cells[I].StartParagraphIndex := ARow.Cells[I]
      .StartParagraphIndex - ADelta;
    ARow.Cells[I].EndParagraphIndex := ARow.Cells[I].EndParagraphIndex - ADelta;
  end;
end;

procedure TdxRtfTableConverter.RecalcParagraphIndexesInTable
  (ATable: TdxRtfTable; ARowIndex: Integer; ADelta: Integer;
  AParagraphIndex: TdxParagraphIndex);
var
  ACount, I: Integer;
begin
...
procedure TdxRtfTableConverter.RecalcParagraphIndexesInTables(ADelta: Integer;
  AParagraphIndex: TdxParagraphIndex);
var
  ACount, I: Integer;
begin
...
class function TdxRtfTableColumnsCalculator.Calculate(ATable: TdxRtfTable;
  ATableIndent: Integer): TdxRtfTableGrid;
var
  ARowsCount, I: Integer;
...
begin
  ...
  if ARowsCount = 0 then
    TdxRichEditExceptions.ThrowArgumentException('table.Rows.Count',
      ARowsCount);
...
class function TdxRtfTableColumnsCalculator.Merge(ASource: TdxRtfTableGrid;
  ADestination: TdxRtfTableGrid): TdxRtfTableGrid;
var
...
begin
  ...
  ASourceIndex := 0;
  ADestinationIndex := 0;
  while (ASourceIndex < ASource.Count) and
    (ADestinationIndex < ADestination.Count) do
  begin
  ...
  else if ASourceIndex < ASource.Count then
  begin
...
class function TdxRtfTableColumnsCalculator.GetRowColumns(ARow: TdxRtfTableRow)
  : TdxRtfTableGrid;
var
...
begin
  ...
  if ACellsCount = 0 then
    TdxRichEditExceptions.ThrowArgumentException('row.Cells.Count',
      ACellsCount);
