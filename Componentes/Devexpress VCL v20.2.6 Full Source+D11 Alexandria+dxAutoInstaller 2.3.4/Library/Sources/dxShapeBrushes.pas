...
type
  ...
  TdxGPLinearGradientBrushMode = (gplgbmHorizontal, gplgbmVertical,
    gplgbmForwardDiagonal, gplgbmBackwardDiagonal, gplgbmLine);
  ...
  public
    ...
    property GradientMode: TdxGPLinearGradientBrushMode read FGradientMode
...
type
  ...
  procedure CalculateGradientPoints(APoints: TList);
    ...
    procedure AddGradientPoint(AList: TList; AOffset: Single;
      const AColor: TdxAlphaColor);
    var
  ...
  begin
    ...
    begin
      ...
      AddGradientPoint(APoints, 1 - AGradientPoints.Offsets[I],
        AGradientPoints.Colors[I]);
    end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        AGradientPoints.Add(PdxGradientPoint(APoints[I]).Offset,
          PdxGradientPoint(APoints[I]).Color);
...
begin
  if Texture.Empty then
    ...
    GdipCheck(GdipCreateTexture2(Texture.Handle, WrapModeTile, 0, 0,
      Texture.Width, Texture.Height, AHandle));
end;
...
begin
  ...
  GdipCheck(GdipTranslateTextureTransform(Handle, TargetRect.X, TargetRect.Y,
    MatrixOrderAppend));
end;
...
begin
  ...
  TdxGPBrushGradientPointsAccess(GradientPoints).CalculateParams(FColors,
    FOffsets, FCount);
end;
...
procedure TdxGpCustomGradientBrush.SetGradientPoints(const AValue
...
procedure TdxGpCustomGradientBrush.SetTransformMatrix
  (const AValue: TdxGPMatrix);
begin
...
const
  GradientModeMap: array [TdxGPLinearGradientBrushMode]
...
begin
  ...
  begin
    ...
    GdipCheck(GdipCreateLineBrush(@AStartPoint, @AEndPoint, 0, 0,
      WrapModeTileFlipX, AHandle));
  end
  else
    GdipCheck(GdipCreateLineBrushFromRect(@TargetRect, 0, 0,
      GradientModeMap[GradientMode], WrapModeTileFlipX, AHandle));
...
procedure TdxGpLinearGradientBrush.SetGradientMode(const AValue
  : TdxGPLinearGradientBrushMode);
begin
...
begin
  Result := dxRectF(TargetRect.X, TargetRect.Y, TargetRect.X + TargetRect.Width,
...
procedure TdxGpPathGradientBrush.DoCreateHandle(out AHandle: GpHandle);
  ...
  var
    ACenter: TdxPointF;
  begin
    ACenter := dxPointF(ARect.Left + cxRectWidth(ARect) / 2,
      ARect.Top + cxRectHeight(ARect) / 2);
...
begin
  ...
  GdipCheck(GdipSetPathGradientPresetBlend(AHandle, TdxAlphaColor(FColors),
    FOffsets, FCount));
  ApplyBrushTransformation;
  FreeAndNil(APath);
end;
...
procedure TdxLinearGradientBrush.CalculateTransformation;
  ...
  procedure CalculateTransformElement(ATransformMatrix: TdxGPMatrix;
    out M11, M12, M21, M22, DX, DY: Single);
  var
    AOffset: TdxPointF;
    AMatrix: TdxGPMatrix;
  begin
...
begin
  ...
  begin
    GetBrushTransformElements(M11, M12, M21, M22, ACurrentOffset.X,
      ACurrentOffset.Y);
    AOffset := cxPointOffset(AOffset, dxPointF(ACurrentOffset.X,
      ACurrentOffset.Y));
  end
  else
  begin
    CalculateTransformElement(TransformMatrix, M11, M12, M21, M22,
      ACurrentOffset.X, ACurrentOffset.Y);
    TransformMatrix.GetElements(M11, M12, M21, M22, ATransformOffset.X,
      ATransformOffset.Y);
...
procedure TdxLinearGradientBrush.CalculateTransformMatrix(M11, M12, M21, M22,
  DX, DY: Single);
begin
...
procedure TdxLinearGradientBrush.GetBrushTransformElements(out M11, M12, M21,
  M22, DX, DY: Single);
var
  AMatrix: TdxGPMatrix;
begin
...
function TdxShapeCustomGradientBrush.GetGradientPoints
  : TdxGPBrushGradientPoints;
begin
...
begin
  APoint := dxStrToPointF(AValue);
  if (GetBrush.StartPoint.X <> APoint.X) or (GetBrush.StartPoint.Y <> APoint.Y)
  then
  begin
...
class function TdxShapeBrushGradientStop.GetReaderClass
