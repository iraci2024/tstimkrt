...
uses
  Types, TypInfo, Windows, Classes, Rtti, Generics.Collections,
  Generics.Defaults, Contnrs, Graphics,
...
type
  ...
  public const
    NameMap: array [TFontStyle] of string = ('bold', 'italic', 'underline',
      'line-through');
  public
    class function Build(const AName: string; ASize: Single;
      AStyles: TFontStyles): string;
    class function Parse(const S: string; out AName: string; out ASize: Single;
      out AStyles: TFontStyles): Boolean;
    class function TryStringToFontStyle(const S: string;
      out AStyle: TFontStyle): Boolean;
  end;
  ...
  public
    class function AsNumbers(const S: string; const AMaxOfNumber: Double = 100)
      : TdxSVGValues; overload;
    ...
    class procedure AsPoints(const V: TdxSVGValues;
      AProc: TEnumPoints); overload;
  end;
  ...
  public const
    NameMap: array [TdxSVGUnitsType] of string = ('px', 'mm', 'cm', 'in', 'pc',
      'pt', '%');
  strict private
...
{$REGION 'TdxSVGParserPath'}
  ...
  public type
    TCommandProc = procedure(const AData: string; AIsRelative: Boolean)
      of object;
  strict private
    ...
    procedure ProcessCommandCurveToSmooth(const AData: string;
      AIsRelative: Boolean);
    ...
    procedure ProcessCommandQuadraticCurveTo(const AData: string;
      AIsRelative: Boolean);
    procedure ProcessCommandQuadraticCurveToSmooth(const AData: string;
      AIsRelative: Boolean);
  ...
  strict protected
    ...
    function GetAbsolutePoint(const P: TdxPointF; AIsRelative: Boolean)
      : TdxPointF; overload;
    function GetAbsolutePoint(const X, Y: Single;
      AIsRelativeX, AIsRelativeY: Boolean): TdxPointF; overload;
  ...
  public
    ...
    //
    property Commands[Index: Integer]: TdxSVGParserPathCommand
  ...
  strict private
    ...
    procedure CalculateVectors(ARotateMatrix: TdxGPMatrix;
      out AVector1, AVector2, ACenterPoint, ARadius: TdxPointF);
  protected
    Angle: Single;
    LargeArc: Boolean;
    Radius: TdxPointF;
    ...
    procedure AppendCore(APath: TdxGPPath; ARotateMatrix: TdxGPMatrix;
      ARadius, AStartPoint, AOffset: TdxPointF;
      AStartAngle, AAngleDelta: Double);
  public
    constructor Create(const AStartPoint, AFinishPoint, ARadius: TdxPointF;
      AAngle: Single; ASweep, ALargeArc: Boolean);
  ...
  protected
    ...
    Point2: TdxPointF;
  public
    constructor Create(const AStartPoint, APoint1, APoint2,
  ...
  protected
    ...
    StartPoint: TdxPointF;
  public
    constructor Create(const AStartPoint, AMiddlePoint,
      AFinishPoint: TdxPointF);
    procedure Append(APath: TdxGPPath); override;
  end;
...
{$ENDREGION}
  ...
  strict private
    ...
    class function GetValue(AValues: TdxSVGValues; AIndex: Integer;
      ADefaultValue: Single = 0): Single;
    // built-in operators
...
class procedure TdxSVGParserInlineStyle.Parse(AStyle: TdxSVGStyle;
  const S: string);
var
  ADelimPos: Integer;
...
class function TdxSVGFontStyleString.Build(const AName: string; ASize: Single;
  AStyles: TFontStyles): string;
var
...
class function TdxSVGFontStyleString.Parse(const S: string; out AName: string;
  out ASize: Single; out AStyles: TFontStyles): Boolean;
var
...
begin
  ...
  begin
    ...
    else if TdxSVGParserValue.Parse(APart, AValue) then
    begin
      if ASize = 0 then
...
class function TdxSVGFontStyleString.TryStringToFontStyle(const S: string;
  out AStyle: TFontStyle): Boolean;
var
  AIndex: TFontStyle;
begin
...
class function TdxSVGParserNumbers.AsNumbers(const S: string;
  const AMaxOfNumber: Double = 100): TdxSVGValues;
  ...
  procedure TryAddValue(ATarget: TdxSVGValues;
    AStartIndex, AIndex, ALength: Integer);
  var
...
class procedure TdxSVGParserNumbers.AsNumbers(const S: string;
  AProc: TEnumNumbers);
var
...
class procedure TdxSVGParserNumbers.AsPoints(const S: string;
  AList: TdxSVGPoints);
var
  AValues: TdxSVGValues;
begin
  ...
  try
    ...
    AsPoints(AValues, procedure(const P: TdxPointF)
      begin
        AList.Add(P);
      end);
  finally
...
class procedure TdxSVGParserNumbers.AsPoints(const V: TdxSVGValues;
AProc: TEnumPoints);
var
  AIndex: Integer;
begin
...
var
  AValues: TdxSVGValues;
begin
  AValues := TdxSVGParserNumbers.AsNumbers
    (Trim(StringReplace(S, 'new', '', [])));
  try
...
class function TdxSVGParserValue.Parse(S: string;
out AValue: TdxSVGValue): Boolean;
var
...
class function TdxSVGParserValue.StringToUnitsType(const S: string)
  : TdxSVGUnitsType;
var
...
function TdxSVGParserPath.GetAbsolutePoint(const P: TdxPointF;
AIsRelative: Boolean): TdxPointF;
begin
  if AIsRelative then
...
function TdxSVGParserPath.GetAbsolutePoint(const X, Y: Single;
AIsRelativeX, AIsRelativeY: Boolean): TdxPointF;
begin
  Result := cxPointF(X, Y);
...
procedure TdxSVGParserPath.ProcessCommandArc(const AData: string;
AIsRelative: Boolean);
var
  AIndex: Integer;
...
begin
  ...
  try
    ...
    begin
      FCommands.Add(TdxSVGParserPathCommandArc.Create(LastPoint,
        GetAbsolutePoint(ANumbers[AIndex + 5], ANumbers[AIndex + 6],
        AIsRelative, AIsRelative), dxPointF(ANumbers[AIndex],
        ANumbers[AIndex + 1]), ANumbers[AIndex + 2],
        not IsZero(ANumbers[AIndex + 4]), not IsZero(ANumbers[AIndex + 3])));
      Inc(AIndex, 7);
    end;
  finally
    ANumbers.Free;
...
procedure TdxSVGParserPath.ProcessCommandClose(const AData: string;
AIsRelative: Boolean);
var
  ACommand: TObject;
  APoint: TdxPointF;
...
procedure TdxSVGParserPath.ProcessCommandCurveTo(const AData: string;
AIsRelative: Boolean);
var
  AIndex: Integer;
...
procedure TdxSVGParserPath.ProcessCommandCurveToSmooth(const AData: string;
AIsRelative: Boolean);
var
...
begin
  ...
  begin
    ...
    if FCommands.Last is TdxSVGParserPathCommandCurveTo then
      AControlPoint :=
        MirrorPoint(TdxSVGParserPathCommandCurveTo(FCommands.Last).Point2,
        AControlPoint);
    FCommands.Add(TdxSVGParserPathCommandCurveTo.Create(LastPoint,
      AControlPoint, GetAbsolutePoint(APoints[AIndex], AIsRelative),
      GetAbsolutePoint(APoints[AIndex + 1], AIsRelative)));
    Inc(AIndex, 2);
  end;
end;

procedure TdxSVGParserPath.ProcessCommandHorzLine(const AData: string;
AIsRelative: Boolean);
begin
  TdxSVGParserNumbers.AsNumbers(AData, procedure(const AValue: Single)
    begin
      FCommands.Add(TdxSVGParserPathCommandLine.Create(LastPoint,
        GetAbsolutePoint(AValue, LastPoint.Y, AIsRelative, False)));
    end);
end;

procedure TdxSVGParserPath.ProcessCommandLineTo(const AData: string;
AIsRelative: Boolean);
var
  APoints: TdxSVGPoints;
  I: Integer;
begin
  ...
  for I := 0 to APoints.Count - 1 do
    FCommands.Add(TdxSVGParserPathCommandLine.Create(LastPoint,
      GetAbsolutePoint(APoints[I], AIsRelative)));
end;

procedure TdxSVGParserPath.ProcessCommandMoveTo(const AData: string;
AIsRelative: Boolean);
var
  APoints: TdxSVGPoints;
  I: Integer;
begin
  APoints := GetPoints(AData);
  FCommands.Add(TdxSVGParserPathCommandMoveTo.Create
    (GetAbsolutePoint(APoints.First, AIsRelative)));
...
procedure TdxSVGParserPath.ProcessCommandOpen(const AData: string;
AIsRelative: Boolean);
begin
  FCommands.Add(TdxSVGParserPathCommandOpen.Create(LastPoint));
end;

procedure TdxSVGParserPath.ProcessCommandQuadraticCurveTo(const AData: string;
AIsRelative: Boolean);
var
  AIndex: Integer;
  APoints: TdxSVGPoints;
...
procedure TdxSVGParserPath.ProcessCommandQuadraticCurveToSmooth
  (const AData: string; AIsRelative: Boolean);
var
  AIndex: Integer;
...
begin
  ...
  begin
    ...
    if FCommands.Last is TdxSVGParserPathCommandQuadraticCurveTo then
      AMiddlePoint := MirrorPoint(TdxSVGParserPathCommandQuadraticCurveTo
        (FCommands.Last).MiddlePoint, AMiddlePoint);
    FCommands.Add(TdxSVGParserPathCommandQuadraticCurveTo.Create(LastPoint,
      AMiddlePoint, GetAbsolutePoint(APoints[AIndex], AIsRelative)));
    Inc(AIndex);
  end;
end;

procedure TdxSVGParserPath.ProcessCommandVertLine(const AData: string;
AIsRelative: Boolean);
begin
  TdxSVGParserNumbers.AsNumbers(AData, procedure(const AValue: Single)
    begin
      FCommands.Add(TdxSVGParserPathCommandLine.Create(LastPoint,
        GetAbsolutePoint(LastPoint.X, AValue, False, AIsRelative)));
    end);
end;
...
constructor TdxSVGParserPathCommandArc.Create(const AStartPoint, AFinishPoint,
  ARadius: TdxPointF; AAngle: Single; ASweep, ALargeArc: Boolean);
begin
...
begin
  ...
  try
    ...
    CalculateVectors(ARotateMatrix, AVector1, AVector2, ACenterPoint,
      ARealRadius);
    ...
    ACenterPoint := cxPointOffset(ACenterPoint, (StartPoint.X + FinishPoint.X) /
      2, (StartPoint.Y + FinishPoint.Y) / 2);

    AppendCore(APath, ARotateMatrix, ARealRadius, StartPoint, ACenterPoint,
      AAngle, AAngleDelta);
  finally
    ARotateMatrix.Free;
  end;
...
procedure TdxSVGParserPathCommandArc.AppendCore(APath: TdxGPPath;
ARotateMatrix: TdxGPMatrix; ARadius, AStartPoint, AOffset: TdxPointF;
AStartAngle, AAngleDelta: Double);
...
begin
  ...
  begin
    ...
    AEndPoint := ARotateMatrix.TransformPoint(dxPointF(ARadius.X * ACosAngle2,
      ARadius.Y * ASinAngle2));
    AEndPoint := cxPointOffset(AEndPoint, AOffset);

    APath.AddBezier(AStartPoint, cxPointOffset(AStartPoint,
      CalculateDelta(ASinAngle1, ACosAngle1, -AScaleFactor)),
      cxPointOffset(AEndPoint, CalculateDelta(ASinAngle2, ACosAngle2,
      AScaleFactor)), AEndPoint);
...
function TdxSVGParserPathCommandArc.CalculateVectorAngle(const AVector1,
  AVector2: TdxPointF): Double;
var
  AAngle1: Double;
  AAngle2: Double;
begin
...
procedure TdxSVGParserPathCommandArc.CalculateVectors(ARotateMatrix
  : TdxGPMatrix; out AVector1, AVector2, ACenterPoint, ARadius: TdxPointF);
var
  APoint1: TdxPointF;
  ARoot: Double;
  AValue1: Double;
...
constructor TdxSVGParserPathCommandCurveTo.Create(const AStartPoint, APoint1,
  APoint2, AFinishPoint: TdxPointF);
begin
  inherited Create(AFinishPoint);
  StartPoint := AStartPoint;
  Point1 := APoint1;
...
constructor TdxSVGParserPathCommandLine.Create(const AStartPoint,
  AFinishPoint: TdxPointF);
begin
  StartPoint := AStartPoint;
  FinishPoint := AFinishPoint;
...
constructor TdxSVGParserPathCommandQuadraticCurveTo.Create(const AStartPoint,
  AMiddlePoint, AFinishPoint: TdxPointF);
begin
  inherited Create(AFinishPoint);
  StartPoint := AStartPoint;
...
begin
  APath.AddBezier(StartPoint, CalcPoint(StartPoint, MiddlePoint),
    CalcPoint(FinishPoint, MiddlePoint), FinishPoint);
end;
...
class procedure TdxSVGParserTransform.Parse(const S: string;
AMatrix: TdxMatrix);
var
  ...
  try
    for I := 0 to AOperators.Count - 1 do
      ...
      begin
        ...
        try
          AMatrix.XForm := TXForm.Combine(AOperatorProc(AValues),
            AMatrix.XForm);
        finally
          AValues.Free;
        end;
...
class function TdxSVGParserTransform.GetValue(AValues: TdxSVGValues;
AIndex: Integer; ADefaultValue: Single = 0): Single;
begin
...
class function TdxSVGParserTransform.OperatorMatrix
  (AValues: TdxSVGValues): TXForm;
begin
  Result := TXForm.CreateMatrix(GetValue(AValues, 0, 1), GetValue(AValues, 1),
    GetValue(AValues, 2), GetValue(AValues, 3, 1), GetValue(AValues, 4),
    GetValue(AValues, 5));
end;

class function TdxSVGParserTransform.OperatorRotate
  (AValues: TdxSVGValues): TXForm;
begin
  Result := TXForm.CreateTranslateMatrix(GetValue(AValues, 1),
    GetValue(AValues, 2));
  Result := TXForm.Combine(TXForm.CreateRotationMatrix(GetValue(AValues, 0)
    ), Result);
  Result := TXForm.Combine(TXForm.CreateTranslateMatrix(-GetValue(AValues, 1),
    -GetValue(AValues, 2)), Result);
end;

class function TdxSVGParserTransform.OperatorScale
  (AValues: TdxSVGValues): TXForm;
var
  X, Y: Single;
begin
...
class function TdxSVGParserTransform.OperatorSkewX
  (AValues: TdxSVGValues): TXForm;
begin
  Result := TXForm.CreateMatrix(1, 0, Tan(DegToRad(GetValue(AValues, 0))
    ), 1, 0, 0);
end;

class function TdxSVGParserTransform.OperatorSkewY
  (AValues: TdxSVGValues): TXForm;
begin
  Result := TXForm.CreateMatrix(1, Tan(DegToRad(GetValue(AValues, 0))),
    0, 1, 0, 0);
end;

class function TdxSVGParserTransform.OperatorTranslate
  (AValues: TdxSVGValues): TXForm;
begin
  Result := TXForm.CreateTranslateMatrix(GetValue(AValues, 0),
    GetValue(AValues, 1));
end;
