...
uses
  Classes, SysUtils, Menus, cxLibraryReg, cxPropEditors,
  ...
  dxCustomMapItemLayer, dxMapControlProjections, dxMapControlImageTileProvider,
  dxMapItemsEditor,
  dxMapControlBingMapImageryDataProvider,
  dxMapControlOpenStreetMapImageryDataProvider,
  dxMapInformationProvidersEditor, dxMapControlInformationProvider,
...
type
  ...
  TdxMapControlBingMapInformationProviderSelectionEditor = class
    (TSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
...
begin
  case AIndex of
    0:
      ShowFormEditorClass(Designer, Component, MapControl.Layers, 'Layers',
...
function TdxMapInformationProvidersProperty.GetEditorClass
  : TcxComponentCollectionEditorClass;
begin
...
function TdxMapControlCustomProjectionProperty.GetAttributes
  : TPropertyAttributes;
begin
...
begin
  if HasSubProperties then
    Result := dxRegisteredMapProjections.GetDescriptionByClass
      (TdxMapControlCustomProjection(GetOrdValue).ClassType)
  else
    Result := '';
end;
...
begin
  ...
  if AMapProjectionClass = nil then
    AMapProjectionClass := TdxMapControlCustomProjectionClass
      (dxRegisteredMapProjections.FindByDescription(Value));

  ObjectInspectorCollapseProperty;
  for I := 0 to PropCount - 1 do
    TdxCustomMapItemLayer(GetComponent(I)).ProjectionClass :=
      AMapProjectionClass;
{$IFNDEF DELPHI15}
...
function TdxMapControlImageTileProviderProperty.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes;
...
begin
  if HasSubProperties then
    Result := dxMapControlImageryDataProviders.GetDescriptionByClass
      (TdxMapControlImageTileProvider(GetOrdValue).ClassType)
  else
    Result := '';
end;
...
begin
  AProviderClass := TdxMapControlImageTileProviderClass
    (dxMapControlImageryDataProviders.FindByClassName(Value));
  if AProviderClass = nil then
    AProviderClass := TdxMapControlImageTileProviderClass
      (dxMapControlImageryDataProviders.FindByDescription(Value));
...
function TdxMapControlCoordinatePatternProperty.GetAttributes
  : TPropertyAttributes;
begin
...
begin
  ...
  try
    Designer.GetComponentNames
      (GetTypeData(PTypeInfo(TdxMapImageTileLayer.ClassInfo)), AddComponent);
...
begin
  ...
  RegisterClasses([TdxMapControl, TdxCustomMapControl, TdxMapLayer, TdxMapItem,
    TdxMapImageTileLayer, TdxMapItemLayer, TdxMapItemFileLayer, TdxMapDot,
    TdxMapRectangle, TdxMapEllipse, TdxMapPolyline, TdxMapPolygon, TdxMapPath,
    TdxMapPushpin, TdxMapCustomElement,
    TdxMapControlBingMapGeoCodingDataProvider,
  ...
  RegisterPropertyEditor(TypeInfo(TFileName), TdxMapItemFileLayer, 'FileName',
    TdxMapItemFileLayerFilenameProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxMapControlInformationProviders), nil, '',
    TdxMapInformationProvidersProperty);
  RegisterPropertyEditor(TypeInfo(TdxMapControlCustomProjection),
    TdxCustomMapItemLayer, 'Projection', TdxMapControlCustomProjectionProperty);
  RegisterPropertyEditor(TypeInfo(TdxMapControlImageTileProvider),
    TdxMapImageTileLayer, 'Provider', TdxMapControlImageTileProviderProperty);
  RegisterPropertyEditor(TypeInfo(string), TdxMapControlNavigationPanel,
    'XCoordinateDisplayMask', TdxMapControlCoordinatePatternProperty);
  RegisterPropertyEditor(TypeInfo(string), TdxMapControlNavigationPanel,
    'YCoordinateDisplayMask', TdxMapControlCoordinatePatternProperty);
  RegisterSelectionEditor(TdxCustomMapControl, TdxMapControlSelectionEditor);
  RegisterSelectionEditor(TdxMapControlBingMapInformationProvider,
    TdxMapControlBingMapInformationProviderSelectionEditor);
