...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Math, Types, SysUtils, Classes, Controls, cxClasses, dxCore, cxVariants,
  dxSpreadSheetUtils, dxCoreClasses,
  dxHashUtils;
...
type
  ...
  TdxSpreadSheetHyperlinkAssignedValue = (havDisplayText, havScreenTip,
    havValue, havExecuted);
  TdxSpreadSheetHyperlinkAssignedValues = set of
  ...
  protected
    ...
    function CreateData(AType: TdxSpreadSheetHyperlinkValueType)
      : TObject; virtual;
    ...
    property DisplayTextAssigned: Boolean read GetDisplayTextAssigned
      write SetDisplayTextAssigned;
  ...
  public
    ...
    property AssignedValues: TdxSpreadSheetHyperlinkAssignedValues
  ...
  protected
    ...
    procedure SaveToStream(AWriter: TcxWriter); virtual;
    //
    property CachedItem: TdxSpreadSheetHyperlink read FCachedItem
      write FCachedItem;
    property View: TObject read FView;
  public
    constructor Create(AView: TObject); virtual;
    destructor Destroy; override;
    function Add(const AArea: TRect): TdxSpreadSheetHyperlink;
      reintroduce; overload;
    function Add(const AArea: string): TdxSpreadSheetHyperlink;
      reintroduce; overload;
    function Add(const ARow, AColumn: Integer): TdxSpreadSheetHyperlink;
      reintroduce; overload;
    procedure Clear; override;
    function FindItem(const ACellReference: string)
      : TdxSpreadSheetHyperlink; overload;
    function FindItem(const ARow, AColumn: Integer)
      : TdxSpreadSheetHyperlink; overload;

    property First: TdxSpreadSheetHyperlink read GetFirst;
    property Last: TdxSpreadSheetHyperlink read GetLast;
    property Items[Index: Integer]: TdxSpreadSheetHyperlink
...
uses
  Windows, Graphics, ShellApi, dxTypeHelpers, cxGeometry, dxSpreadSheetCore,
  dxSpreadSheetStrs, dxSpreadSheetClasses,
  dxSpreadSheetTypes, dxSpreadSheetFormulas, dxSpreadSheetCoreStyles,
  dxSpreadSheetCoreFormulas,
  dxSpreadSheetCoreFormulasParser, dxSpreadSheetCoreFormulasTokens,
  dxSpreadSheetCoreStrs, dxSpreadSheetCoreReferences;
...
type
  ...
  public
    ...
    procedure LoadFromStream(ASpreadSheet: TdxCustomSpreadSheet;
      AReader: TcxReader); virtual;
    procedure SaveToStream(ASpreadSheet: TdxCustomSpreadSheet;
      AWriter: TcxWriter); virtual;
    procedure SetValue(AView: TdxSpreadSheetTableView;
  ...
  public
    ...
    procedure CheckDisplayTextAssigned(AView: TdxSpreadSheetTableView);
      override;
    ...
    procedure LoadFromStream(ASpreadSheet: TdxCustomSpreadSheet;
      AReader: TcxReader); override;
    procedure SaveToStream(ASpreadSheet: TdxCustomSpreadSheet;
      AWriter: TcxWriter); override;
    procedure SetValue(AView: TdxSpreadSheetTableView;
  ...
  TdxSpreadSheetHyperlinkReferenceHelperFormula = class
  ...
  public
    constructor Create(AOwner: TdxSpreadSheetHyperlinkReferenceHelper);
      reintroduce; overload;
  end;

  { TdxSpreadSheetHistoryChangeHyperlinkAction }

  TdxSpreadSheetHistoryChangeHyperlinkAction = class
  ...
  TdxSpreadSheetHistoryChangeHyperlinkCommand = class
  ...
  public
    constructor Create(AHyperlink: TdxSpreadSheetHyperlink;
      ANewItem: Boolean = False);
    destructor Destroy; override;
...
procedure TdxSpreadSheetHyperlinkData.Assign
  (ASource: TdxSpreadSheetHyperlinkData);
begin
...
procedure TdxSpreadSheetHyperlinkData.CheckDisplayTextAssigned
  (AView: TdxSpreadSheetTableView);
begin
end;

function TdxSpreadSheetHyperlinkData.GetDisplayText
  (AView: TdxSpreadSheetTableView): string;
begin
...
function TdxSpreadSheetHyperlinkData.GetValue
  (AView: TdxSpreadSheetTableView): string;
begin
  Result := Value;
end;

procedure TdxSpreadSheetHyperlinkData.LoadFromStream(ASpreadSheet
  : TdxCustomSpreadSheet; AReader: TcxReader);
begin
...
procedure TdxSpreadSheetHyperlinkData.SaveToStream(ASpreadSheet
  : TdxCustomSpreadSheet; AWriter: TcxWriter);
begin
...
procedure TdxSpreadSheetHyperlinkData.SetValue(AView: TdxSpreadSheetTableView;
  const AValue: string);
begin
  Value := AValue;
end;

procedure TdxSpreadSheetHyperlinkData.Validate(ASpreadSheet
...
procedure TdxSpreadSheetHyperlinkDocumentReference.Assign
  (ASource: TdxSpreadSheetHyperlinkData);
begin
...
procedure TdxSpreadSheetHyperlinkDocumentReference.CheckDisplayTextAssigned
  (AView: TdxSpreadSheetTableView);
  ...
  begin
    ...
    if ASheet = nil then
      ASheet := AView;
    Result := PrepareReferenceText(dxReferenceToString(Area, ARCReference,
      OptionsMap[AView <> Sheet], ASheet.Caption));
  end;

var
  AText: string;
begin
  if (Name = nil) and Area.IsEqual(dxSpreadSheetGetRealArea(Area)) and
    ((Sheet <> nil) or (AView <> nil)) then
  begin
    AText := PrepareReferenceText(DisplayText);
    if (AText <> '') and (SameText(AText, GetDefaultDisplayText(False)) or
      SameText(AText, GetDefaultDisplayText(True))) then
...
begin
  Result := (Name <> nil) or (Sheet <> nil) and
    Area.IsEqual(dxSpreadSheetGetRealArea(Area));
end;

procedure TdxSpreadSheetHyperlinkDocumentReference.Execute
  (AView: TdxSpreadSheetTableView);
var
  R: TRect;
...
function TdxSpreadSheetHyperlinkDocumentReference.GetDisplayText
  (AView: TdxSpreadSheetTableView): string;
begin
...
function TdxSpreadSheetHyperlinkDocumentReference.GetValue
  (AView: TdxSpreadSheetTableView): string;
var
  ASpreadSheet: TdxCustomSpreadSheet;
begin
  ...
  if Name <> nil then
    ...
    Result := dxReferenceToString(Area, ASpreadSheet.OptionsView.R1C1Reference,
...
procedure TdxSpreadSheetHyperlinkDocumentReference.SaveToStream
  (ASpreadSheet: TdxCustomSpreadSheet; AWriter: TcxWriter);
begin
...
procedure TdxSpreadSheetHyperlinkDocumentReference.SetValue
  (AView: TdxSpreadSheetTableView; const AValue: string);
begin
...
procedure TdxSpreadSheetHyperlinkDocumentReference.Validate
  (ASpreadSheet: TdxCustomSpreadSheet);
begin
  if (Sheet <> nil) and (TdxSpreadSheetAccess(ASpreadSheet)
    .FSheets.IndexOf(Sheet) < 0) then
    Sheet := nil;
  if (Name <> nil) and
    (TdxSpreadSheetObjectListAccess(ASpreadSheet.DefinedNames)
    .ItemList.IndexOf(Name) < 0) then
    Name := nil;
end;

{ TdxSpreadSheetHyperlinkEMailReference }

procedure TdxSpreadSheetHyperlinkEMailReference.Assign
  (ASource: TdxSpreadSheetHyperlinkData);
begin
  inherited Assign(ASource);
...
function TdxSpreadSheetHyperlinkEMailReference.GetValue
  (AView: TdxSpreadSheetTableView): string;
begin
...
procedure TdxSpreadSheetHyperlinkEMailReference.LoadFromStream
  (ASpreadSheet: TdxCustomSpreadSheet; AReader: TcxReader);
begin
  inherited LoadFromStream(ASpreadSheet, AReader);
...
procedure TdxSpreadSheetHyperlinkEMailReference.SaveToStream
  (ASpreadSheet: TdxCustomSpreadSheet; AWriter: TcxWriter);
begin
  inherited SaveToStream(ASpreadSheet, AWriter);
...
procedure TdxSpreadSheetHyperlinkEMailReference.SetValue
  (AView: TdxSpreadSheetTableView; const AValue: string);
var
...
begin
  ...
  begin
    Subject := Copy(Value, ASubjectPosition +
      Length(dxEMailHyperlinkSubjectPrefix), Length(AValue));
  ...
  begin
    ...
    Value := Copy(AValue, AAddressPosition, ASubjectPosition -
      AAddressPosition);
  end
end;
...
begin
  ...
  try
    AParser.ParseFormula(dxSpreadSheetFormulaIncludeEqualSymbol
      (Reference.Value), FFormula);
  finally
    AParser.Free;
  end;
...
function TdxSpreadSheetHyperlinkReferenceHelper.TryConvertTokenToReference
  (AToken: TdxSpreadSheetFormulaToken): Boolean;
begin
  if AToken is TdxSpreadSheetFormulaReferenceToken then
  begin
    ClearReference;
    Reference.Sheet := TdxSpreadSheetFormulaReferenceToken(AToken)
      .View as TdxSpreadSheetCustomView;
    Reference.Area.Left := TdxSpreadSheetFormulaReferenceToken(AToken)
      .ActualColumn;
    ...
    begin
      Reference.Area.Bottom := TdxSpreadSheetFormulaAreaReferenceToken(AToken)
        .ActualRow2;
      Reference.Area.Right := TdxSpreadSheetFormulaAreaReferenceToken(AToken)
        .ActualColumn2;
    end
    else
  ...
  begin
    ...
    if (AToken <> nil) and (AToken.ChildCount = 1) and (AToken.Next = nil) and
    (AToken.Prev = nil) then
...
constructor TdxSpreadSheetHyperlinkReferenceHelperFormula.Create
  (AOwner: TdxSpreadSheetHyperlinkReferenceHelper);
begin
  FOwner := AOwner;
end;

function TdxSpreadSheetHyperlinkReferenceHelperFormula.GetController
...
begin
  ...
  begin
    ...
    try
      TdxSpreadSheetTableView(View).History.AddCommand
        (TdxSpreadSheetHistoryChangeHyperlinkCommand.Create(Self));
      BeforeDestroy;
    finally
...
begin
  ...
  begin
    ...
    try
      ...
      TdxSpreadSheetHyperlinkData(Data)
        .Assign(TdxSpreadSheetHyperlinkData(ASource.Data));
...
function TdxSpreadSheetHyperlink.Contains(const ARow, AColumn: Integer)
  : Boolean;
begin
...
begin
  ...
  begin
    ...
    else if not dxShellExecute(AView.SpreadSheet.Handle, Value) then
      raise EdxSpreadSheetHyperlinkExecuteError.CreateFmt
        (cxGetResourceString(@sdxHyperlinkExecuteError), [Value]);
...
function TdxSpreadSheetHyperlink.CreateData
  (AType: TdxSpreadSheetHyperlinkValueType): TObject;
begin
  case AType of
    hvtReference:
  ...
  if Data <> nil then
    TdxSpreadSheetHyperlinkData(Result)
      .Assign(TdxSpreadSheetHyperlinkData(Data));
  TdxSpreadSheetHyperlinkData(Result).CheckDisplayTextAssigned
    (TdxSpreadSheetTableView(View));
end;
...
begin
  Result := (Owner <> nil) and (View <> nil) and TdxSpreadSheetTableView(View)
    .History.CanAddCommand;
end;
...
begin
  ...
  begin
    TdxSpreadSheetHyperlinkDocumentReference(Data)
      .Validate(TdxSpreadSheetTableView(View).SpreadSheet);
...
procedure TdxSpreadSheetHyperlink.ReplaceData(var AData: TObject;
  var AArea: TRect);
var
  R: TRect;
begin
...
begin
  ...
  try
    ...
    if DisplayTextAssigned or (ACell.AsString = '') and (ADisplayText <> '')
    then
      ACell.SetText(ADisplayText);

    AHyperlinkTextColor := TdxSpreadSheetAccess(AView.SpreadSheet)
      .LookAndFeelPainter.DefaultHyperlinkTextColor;
    for ARow := Area.Top to Area.Bottom do
      for AColumn := Area.Left to Area.Right do
      begin
        AFont := TdxSpreadSheetFontAccess(AView.CreateCell(ARow, AColumn)
          .Style.Font);
        AFont.ChangeFont(AFont.Name, AFont.Charset, AHyperlinkTextColor,
...
begin
  ...
  try
    for ARow := Area.Top to Area.Bottom do
      for AColumn := Area.Left to Area.Right do
        TdxSpreadSheetTableView(View).CreateCell(ARow, AColumn)
          .Style.Font.Assign(TdxSpreadSheetTableView(View)
          .SpreadSheet.DefaultCellStyle.Font);
  finally
...
begin
  TdxSpreadSheetHyperlinkData(Data)
    .SetValue(TdxSpreadSheetTableView(Owner.View), AValue);
  Include(TdxSpreadSheetHyperlinkData(Data).AssignedValues, havValue);
end;

procedure TdxSpreadSheetHyperlink.SetValueType
  (AValue: TdxSpreadSheetHyperlinkValueType);
var
  AData: TObject;
begin
...
begin
  ...
  TdxSpreadSheetHyperlinkData(Data).LoadFromStream
    (TdxSpreadSheetTableView(Owner.View).SpreadSheet, AReader);
end;
...
begin
  ...
  TdxSpreadSheetHyperlinkData(Data)
    .SaveToStream(TdxSpreadSheetTableView(Owner.View).SpreadSheet, AWriter);
end;
...
begin
  ...
  try
    ...
    else if IsReference(AValue) then
...
procedure TdxSpreadSheetHyperlink.SetAssignedValues
  (AValue: TdxSpreadSheetHyperlinkAssignedValues);
begin
...
begin
  ...
  try
    ...
    DisplayTextAssigned := havDisplayText in TdxSpreadSheetHyperlinkData(Data)
      .AssignedValues;
    RelatedCellSetDisplayText;
  finally
    Changed;
...
function TdxSpreadSheetHyperlink.GetAssignedValues
  : TdxSpreadSheetHyperlinkAssignedValues;
begin
...
begin
  Result := TdxSpreadSheetHyperlinkData(Data)
    .GetDisplayText(TdxSpreadSheetTableView(Owner.View));
end;
...
begin
  Result := TdxSpreadSheetHyperlinkData(Data)
    .GetValue(TdxSpreadSheetTableView(Owner.View));
end;
...
begin
  if Data is TdxSpreadSheetHyperlinkDocumentReference then
    Result := hvtReference
  else if Data is TdxSpreadSheetHyperlinkEMailReference then
    Result := hvtEMail
  else
...
function TdxSpreadSheetHyperlinks.Add(const AArea: TRect)
  : TdxSpreadSheetHyperlink;
var
...
begin
  ...
  try
    ...
    begin
      BeginHistoryAction;
      try
        AHistory.AddCommand(TdxSpreadSheetHistoryChangeHyperlinkCommand.Create
          (Result, True));
      finally
        EndHistoryAction;
      end;
...
function TdxSpreadSheetHyperlinks.Add(const AArea: string)
  : TdxSpreadSheetHyperlink;
var
  R: TRect;
begin
...
function TdxSpreadSheetHyperlinks.Add(const ARow, AColumn: Integer)
  : TdxSpreadSheetHyperlink;
begin
...
function TdxSpreadSheetHyperlinks.FindItem(const ACellReference: string)
  : TdxSpreadSheetHyperlink;
var
  ARow, AColumn: Integer;
begin
  ...
  if not dxSpreadSheetIsValidCellReference(ARow, AColumn) then
    raise EdxSpreadSheetError.CreateFmt
      (cxGetResourceString(@sdxErrorInvalidCellReference), [ACellReference]);
  Result := FindItem(ARow, AColumn);
end;

function TdxSpreadSheetHyperlinks.FindItem(const ARow, AColumn: Integer)
  : TdxSpreadSheetHyperlink;
begin
  Result := CachedItem;
...
procedure TdxSpreadSheetHyperlinks.AssignChanges(ASource,
  ADest: TdxSpreadSheetHyperlink);
var
  ...
  try
    ...
    begin
      ...
      if TdxSpreadSheetTableView(View).Controller.FocusedContainer <> nil then
        TdxSpreadSheetTableView(View).Controller.FocusedContainer.
          Hyperlink := ADest;
    end;
    if AHistory.CanAddCommand then
    begin
...
begin
  TdxSpreadSheetTableView(View).History.BeginAction
    (TdxSpreadSheetHistoryChangeHyperlinkAction);
end;
...
function TdxSpreadSheetHyperlinks.GetItem(AIndex: Integer)
  : TdxSpreadSheetHyperlink;
begin
  Result := First;
...
class function TdxSpreadSheetHistoryChangeHyperlinkAction.
...
constructor TdxSpreadSheetHistoryChangeHyperlinkCommand.Create
  (AHyperlink: TdxSpreadSheetHyperlink; ANewItem: Boolean);
begin
  if AHyperlink.GetContainer <> nil then
    ContainerIndex := TdxSpreadSheetContainer(AHyperlink.GetContainer).Index
  else
    ContainerIndex := -1;
  Data := TdxSpreadSheetHyperlinkData
    (AHyperlink.CreateData(AHyperlink.ValueType));
  TdxSpreadSheetHyperlinkData(Data)
    .Assign(TdxSpreadSheetHyperlinkData(AHyperlink.Data));
...
class function TdxSpreadSheetHistoryChangeHyperlinkCommand.ActionClass
...
begin
  ...
  else if (ItemIndex = Count) or (ContainerIndex >= 0) then
  begin
...
function TdxSpreadSheetHistoryChangeHyperlinkCommand.GetContainer
...
function TdxSpreadSheetHistoryChangeHyperlinkCommand.GetHyperlinks
  : TdxSpreadSheetHyperlinks;
begin
