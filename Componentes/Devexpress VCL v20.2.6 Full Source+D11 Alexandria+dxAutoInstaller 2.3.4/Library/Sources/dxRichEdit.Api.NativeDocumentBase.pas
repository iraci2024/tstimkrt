...
type
  ...
  public
    constructor Create(APieceTable: TdxPieceTable;
      AServer: TdxInnerRichEditDocumentServer); virtual;
...
procedure TdxNativeSubDocumentBase.RegisterAnchor
  (APos: TdxDocumentModelPositionAnchor);
begin
  if not DocumentModel.IsDestroying then
    InternalApi.RegisterAnchor(APos);
end;

procedure TdxNativeSubDocumentBase.UnregisterAnchor
  (APos: TdxDocumentModelPositionAnchor);
begin
  if not DocumentModel.IsDestroying then
