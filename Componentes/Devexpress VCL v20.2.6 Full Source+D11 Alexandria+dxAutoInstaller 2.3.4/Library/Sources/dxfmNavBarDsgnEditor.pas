...
uses
  Dialogs, Classes, Controls, Messages, Forms, StdCtrls, ExtCtrls, Menus,
  ImgList,
...
{$ENDIF}
  ...
  private
    ...
    procedure SafeChangeLookAndFeelScheme(ALookAndFeelStyle
      : TcxLookAndFeelStyle; ANavBarID: Integer);
  ...
  protected
    ...
    function IsCategoriesSuitable(ACategories: TdxNavBarViewCategories)
      : Boolean;
    procedure FinalizeHandlers;
    procedure SynchronizeViewStyleSelection;

    function GetSchemeComboByLookAndFeelStyle(ALookAndFeelStyle
      : TcxLookAndFeelStyle): TComboBox;
    function GetLookAndFeelBySchemeCombo(ACombo: TComboBox)
      : TcxLookAndFeelStyle;
  ...
  public
    ...
    procedure ItemDeleted(const ADesigner: IDesigner;
      Item: TPersistent); override;
    procedure ItemsModified(const Designer: IDesigner); override;
    procedure SelectionChanged(const ADesigner: IDesigner;
      const ASelection: IDesignerSelections); override;

    property CurrentHandler: TdxNavBarDsgnWindowPageHandler read FCurrentHandler
      write FCurrentHandler;
    property HandlerCount: Integer read GetHandlerCount;
    property Handlers[Index: Integer]: TdxNavBarDsgnWindowPageHandler
      read GetHandler;
...
function dxNavBarViewStyleHasColorSchemes(AViewStyle: TdxNavBarPainter)
  : Boolean;
...
const
  dxNavBarPredefinedLookAndFeelScheme: array [0 .. 1, TcxLookAndFeelStyle]
    of Integer = ((dxNavBarExplorerBarView, dxNavBarExplorerBarView,
    dxNavBarUltraFlatExplorerView, dxNavBarOffice12ExplorerBarView,
    dxNavBarOffice11ExplorerBarView, dxNavBarSkinExplorerBarView),
...
begin
  if Supports(TdxCustomNavBarAccess(ANavBar).FDesignHelper,
    IdxNavBarDesignEditor, ANavBarDesignEditor) then
...
type
  TdxNavBarDsgnWindowComponentCollectionPageHandler = class
  ...
  protected
    function AddNavBarCollectionItem(AItemClass: TClass)
      : TdxNavBarComponentCollectionItem; virtual;
    function GetNavBarComponentCollection
      : TdxNavBarComponentCollection; virtual;
    function MainList: TListBox; virtual;
    function ItemClass: TComponentClass; virtual;
    procedure SetCollectionItemName(ANavBarCollectionItem
      : TdxNavBarComponentCollectionItem); virtual;
  ...
  public
    ...
    property Items[Index: Integer]: TdxNavBarComponentCollectionItem
      read GetItem;
    property ItemCount: Integer read GetItemCount;
    property NavBarCollection: TdxNavBarComponentCollection
      read GetNavBarComponentCollection;
...
begin
  if (AItem = nil) or not AItem.InheritsFrom(ItemClass) then
    exit;
...
begin
  ...
  begin
    ...
    if ANavBarCollectionItem.Owner = NavBar.Owner then
      MainList.Items.AddObject(ANavBarCollectionItem.Name,
        ANavBarCollectionItem);
  end;
end;
...
procedure TdxNavBarDsgnWindowComponentCollectionPageHandler.UpdateSelections
  (const ASelections: IDesignerSelections);
...
function TdxNavBarDsgnWindowComponentCollectionPageHandler.
  AddNavBarCollectionItem(AItemClass: TClass): TdxNavBarComponentCollectionItem;
begin
  Result := nil;
end;

function TdxNavBarDsgnWindowComponentCollectionPageHandler.
  GetNavBarComponentCollection: TdxNavBarComponentCollection;
begin
  Result := nil;
end;
...
function TdxNavBarDsgnWindowComponentCollectionPageHandler.ItemClass
  : TComponentClass;
begin
  Result := nil;
end;
...
begin
  Result := CanAdd and DsgnWindow.ClipboardComponents and
    DsgnWindow.Designer.CanPaste;
end;

function TdxNavBarDsgnWindowComponentCollectionPageHandler.CanSelectAll
  : Boolean;
begin
...
function TdxNavBarDsgnWindowComponentCollectionPageHandler.GetItemCount
  : Integer;
begin
  if MainList <> nil then
...
type
  TdxNavBarDsgnWindowCustomStylesHandler = class
    (TdxNavBarDsgnWindowComponentCollectionPageHandler)
  protected
    function AddNavBarCollectionItem(AItemClass: TClass)
      : TdxNavBarComponentCollectionItem; override;
    function MainList: TListBox; override;
    function GetNavBarComponentCollection
      : TdxNavBarComponentCollection; override;
...
function TdxNavBarDsgnWindowCustomStylesHandler.AddNavBarCollectionItem
  (AItemClass: TClass): TdxNavBarComponentCollectionItem;
begin
...
function TdxNavBarDsgnWindowCustomStylesHandler.GetNavBarComponentCollection
...
procedure TdxNavBarDsgnWindowCustomStylesHandler.SetCollectionItemName
  (ANavBarCollectionItem: TdxNavBarComponentCollectionItem);
begin
  // do nothing
end;
...
begin
  ...
  DsgnWindow.nbPreview.LookAndFeel.MasterLookAndFeel := (NavBar as TdxNavBar)
    .LookAndFeel;
...
type
  TdxNavBarDsgnWindowLookAndFeelSchemeHandler = class
    (TdxNavBarDsgnWindowPageHandler)
  private
    FViews: TStringList;
  protected
...
constructor TdxNavBarDsgnWindowLookAndFeelSchemeHandler.Create
  (ADsgnWindow: TdxfmNavBarDesignWindow);
begin
...
procedure TdxNavBarDsgnWindowLookAndFeelSchemeHandler.UpdateContent;
  ...
  begin
    ASkinsAvailable := dxNavBarViewsFactory.IsViewRegistered
...
begin
  ...
  for I := 0 to dxNavBarViewsFactory.Count - 1 do
    FViews.AddObject(dxNavBarViewsFactory.Names[I],
      TObject(dxNavBarViewsFactory.IDs[I]));
...
begin
  ...
  begin
    ...
    ACombo.ItemIndex := ACombo.Items.IndexOfObject
      (TObject(NavBar.LookAndFeelSchemes.Views[I]));
  end;
end;

type
  TdxNavBarDsgnWindowDefaultStylesHandler = class
    (TdxNavBarDsgnWindowPageHandler)
  private
...
function TdxNavBarDsgnWindowDefaultStylesHandler.GetItem(Index: Integer)
...
var
  ...
  ACount: Integer;
begin
  if MainList.Items.Count > 0 then
    exit;
  ACount := GetPropList(TypeInfo(TdxNavBarDefaultStyles), [tkClass], @PropList);
  for I := 0 to ACount - 1 do
  begin
    if TdxNavBarDefaultStyle.ClassName = dxShortStringToString
      (PropList[I].PropType^.Name) then
    begin
      V := GetPropValue(NavBar.DefaultStyles,
        dxShortStringToString(PropList[I].Name));
      // TODO !!!!
...
procedure TdxNavBarDsgnWindowDefaultStylesHandler.SetSelected(Index: Integer;
  const Value: Boolean);
begin
  if MainList <> nil then
    MainList.Selected[Index] := Value;
...
constructor TdxNavBarDsgnWindowPageHandler.Create
  (ADsgnWindow: TdxfmNavBarDesignWindow);
begin
  inherited Create;
...
procedure TdxNavBarDsgnWindowPageHandler.GetSelections(const ASelections
  : IDesignerSelections);
begin
end;
...
procedure TdxNavBarDsgnWindowPageHandler.UpdateSelections(const ASelections
  : IDesignerSelections);
begin
end;
...
constructor TdxfmNavBarDesignWindow.Create(AOwner: TComponent);
  ...
  procedure PopulateGroupItemMenu;
    ...
    function CreateAddMenuItem(AOwner: TComponent; const ACaption: string;
      AImageIndex: Integer; AClass: TClass): TMenuItem;
    begin
  ...
  begin
    pmGroupItemClasses.Items.Add(CreateAddMenuItem(pmGroupItemClasses,
      'Add Item', 9, TdxNavBarItem));
    pmGroupItemClasses.Items.Add(CreateAddMenuItem(pmGroupItemClasses,
      'Add Separator', 10, TdxNavBarSeparator));

    msiAdd.Add(CreateAddMenuItem(msiAdd, 'Add Item', 9, TdxNavBarItem));
    msiAdd.Add(CreateAddMenuItem(msiAdd, 'Add Separator', 10,
      TdxNavBarSeparator));
  end;
...
begin
  ...
  if AIndex > 0 then
    AItemNamePrefix := System.Copy(Component.ClassName,
      AIndex + Length(sdxNavBarClassesPrefix), MaxInt)
  else
...
procedure TdxfmNavBarDesignWindow.SafeChangeLookAndFeelScheme(ALookAndFeelStyle
  : TcxLookAndFeelStyle; ANavBarID: Integer);
var
...
begin
  ANeedChangeSelection := (NavBar.View = dxNavBarLookAndFeelView) and
    (TdxCustomNavBarAccess(NavBar).LookAndFeel.ActiveStyle = ALookAndFeelStyle)
    and (NavBar.ViewReal <> ANavBarID);
...
procedure TdxfmNavBarDesignWindow.Select(AComponent: TComponent;
...
begin
  ...
  begin
    ...
    Caption := Format(sdxDesignerCaption, [NavBar.Owner.Name, NavBar.Name,
      APostfix]);
  end;
end;

procedure TdxfmNavBarDesignWindow.UpdateSelections(const ASelections
  : IDesignerSelections);
begin
  if (NavBar = nil) or (csDestroying in NavBar.ComponentState) or
    (ASelections = nil) then
    exit;
  if CurrentHandler <> nil then
...
function TdxfmNavBarDesignWindow.GetCategories(AIndex: Integer)
...
function TdxfmNavBarDesignWindow.GetHandler(Index: Integer)
  : TdxNavBarDsgnWindowPageHandler;
begin
...
begin
  ...
  for I := 0 to dxNavBarViewsFactory.Count - 1 do
    if IsCategoriesSuitable(dxNavBarViewsFactory.PainterClasses[I].GetCategories)
    then
...
function TdxfmNavBarDesignWindow.IsCategoriesSuitable
  (ACategories: TdxNavBarViewCategories): Boolean;
begin
...
function TdxfmNavBarDesignWindow.GetSchemeComboByLookAndFeelStyle
  (ALookAndFeelStyle: TcxLookAndFeelStyle): TComboBox;
begin
...
function TdxfmNavBarDesignWindow.GetLookAndFeelBySchemeCombo(ACombo: TComboBox)
  : TcxLookAndFeelStyle;
begin
...
begin
  ...
  begin
    for I := Low(TcxLookAndFeelStyle) to High(TcxLookAndFeelStyle) do
      SafeChangeLookAndFeelScheme(I, dxNavBarPredefinedLookAndFeelScheme
        [AIndex - 1, I]);
    Designer.Modified;
  end;
end;
