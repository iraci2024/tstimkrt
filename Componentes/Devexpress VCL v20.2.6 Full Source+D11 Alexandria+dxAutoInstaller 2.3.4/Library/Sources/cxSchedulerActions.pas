...
uses
  SysUtils, Classes, Controls, dxActions, dxPrinting, cxScheduler,
...
uses
  cxDateUtils, cxSchedulerActionsStrs, cxSchedulerDialogs, cxSchedulerStorage,
  cxSchedulerDayView, cxSchedulerWeekView,
  cxSchedulerTimeGridView, cxSchedulerGanttView, cxSchedulerYearView,
...
begin
  Result := inherited IsEnabled and Control.EventOperations.Creating and
    Control.EventOperations.DialogEditing and
...
begin
  Result := inherited IsEnabled and (Control.CurrentView is TcxSchedulerDayView)
    and (Control.SelectedDays.Count = 1);
end;
...
begin
  ...
  if cxShowGoToDateDialog(Control, Control.DialogsLookAndFeel, ADate, AViewMode)
  then
...
begin
  inherited DoUpdateState;
  Checked := (Control.CurrentView is TcxSchedulerDayView) and
    not IsWorkWeekActive;
end;
...
begin
  ...
  begin
    ...
    Result := (ADays * Control.OptionsView.WorkDays)
      = Control.OptionsView.WorkDays;
  end;
end;
...
begin
  inherited DoUpdateState;
  Checked := (Control.CurrentView is TcxSchedulerTimeGridView) and
    not(Control.CurrentView is TcxSchedulerGanttView);
end;
...
begin
  Result := inherited IsEnabled and Supports(Control.CurrentView,
...
begin
  inherited DoUpdateState;
  Checked := Enabled and TcxSchedulerWeekView(Control.CurrentView)
    .CompressWeekEnd;
end;
...
begin
  Result := inherited IsEnabled and
    (Control.CurrentView is TcxSchedulerCustomWeekView);
end;
...
begin
  ...
  else if Control.CurrentView is TcxSchedulerTimeGridView then
...
begin
  ...
  AChecked := Enabled;
  if Control.CurrentView is TcxSchedulerDayView then
    AChecked := AChecked and TcxSchedulerDayView(Control.CurrentView)
      .WorkTimeOnly
  else if Control.CurrentView is TcxSchedulerTimeGridView then
    AChecked := AChecked and TcxSchedulerTimeGridView(Control.CurrentView)
      .WorkTimeOnly;
  Checked := AChecked;
end;
...
begin
  Result := inherited IsEnabled and
    ((Control.CurrentView is TcxSchedulerDayView) or
...
begin
  TcxSchedulerTimeGridView(Control.CurrentView).SnapEventsToTimeSlots
    := Checked;
end;
...
begin
  inherited DoUpdateState;
  Checked := Enabled and TcxSchedulerTimeGridView(Control.CurrentView)
