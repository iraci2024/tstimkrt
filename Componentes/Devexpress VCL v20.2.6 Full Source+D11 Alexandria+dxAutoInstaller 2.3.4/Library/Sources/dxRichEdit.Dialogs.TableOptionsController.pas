...
type
  ...
  protected
    ...
    class function EqualsWidthUnit(const AValue1: TdxWidthUnitInfo;
      const AValue2: TdxWidthUnit): Boolean; overload; static;
  ...
  public
    ...
    property BottomMargin: TdxNullableInteger read FBottomMargin
      write FBottomMargin;
    property CellSpacing: TdxNullableInteger read FCellSpacing
      write FCellSpacing;
    property AllowCellSpacing: TdxNullableBoolean read FAllowCellSpacing
      write FAllowCellSpacing;
    property ResizeToFitContent: TdxNullableBoolean read FResizeToFitContent
      write FResizeToFitContent;
  end;
...
type
  TdxCellMargin = record
    ...
    constructor Create(AFirstRowCellMargin: TdxMarginUnitBase;
      AIdenticalMargins: Boolean = True);
...
constructor TdxCellMargin.Create(AFirstRowCellMargin: TdxMarginUnitBase;
  AIdenticalMargins: Boolean);
begin
  FirstRowCellMargin := AFirstRowCellMargin;
  IdenticalMargins := AIdenticalMargins;
end;

procedure TdxCellMargin.CheckIdenticalMargins(ACurrentMargins
  : TdxMarginUnitBase);
begin
  IdenticalMargins := IdenticalMargins and
    TdxTableOptionsFormController.EqualsWidthUnit(FirstRowCellMargin,
    ACurrentMargins);
end;

class operator TdxCellMargin.Implicit(const A: TdxCellMargin)
  : TdxNullableInteger;
begin
...
constructor TdxTableOptionsFormControllerParameters.Create(const AControl
  : IdxRichEditControl; ATable: TdxTable);
begin
...
var
  ...
  I: Integer;
begin
  ANewCellSpacing := TdxWidthUnitInfo.Create(TdxWidthUnitType.ModelUnits,
    CellSpacing.Value div 2);
  try
...
constructor TdxTableOptionsFormController.Create(AControllerParameters
  : TdxTableOptionsFormControllerParameters);
begin
  inherited Create;
...
class function TdxTableOptionsFormController.EqualsWidthUnit
  (const AValue1: TdxWidthUnitInfo; const AValue2: TdxWidthUnit): Boolean;
begin
...
class function TdxTableOptionsFormController.EqualsWidthUnit(const AValue1,
...
procedure TdxTableOptionsFormController.ApplyMargins;
  ...
  procedure ApplyCellMargin(const AMargin: TdxNullableInteger;
    const AMarginUnit: TdxMarginUnitBase);
  var
    ANewMargin: TdxWidthUnitInfo;
  begin
    if AMargin.IsNull then
      Exit;
    ANewMargin := TdxWidthUnitInfo.Create(TdxWidthUnitType.ModelUnits,
      AMargin.Value);
    try
...
var
  ...
  ALeftCellMargin, ARightCellMargin, ATopCellMargin, ABottomCellMargin
    : TdxCellMargin;
  ACurrentLeftMargins, ACurrentRightMargins, ACurrentTopMargins,
    ACurrentBottomMargins: TdxMarginUnitBase;
  ARowsCount: Integer;
  I: Integer;
begin
  ARows := FSourceTable.Rows;
  ...
  ALeftCellMargin := TdxCellMargin.Create
    (AFirstRow.GetLeftCellMarginConsiderExceptions);
  ARightCellMargin := TdxCellMargin.Create
    (AFirstRow.GetRightCellMarginConsiderExceptions);
  ATopCellMargin := TdxCellMargin.Create
    (AFirstRow.GetTopCellMarginConsiderExceptions);
  ABottomCellMargin := TdxCellMargin.Create
    (AFirstRow.GetBottomCellMarginConsiderExceptions);
...
begin
  ...
  FSourceTable.TableProperties.ResetUse
    (TdxTablePropertiesOptions.MaskUseCellSpacing);
  ...
  begin
    ...
    if ACurrentRowProperties.GetUse
      (TdxTableRowPropertiesOptions.MaskUseCellSpacing) then
    begin
      ACurrentRowProperties.CellSpacing.CopyFrom(ADefaultCellSpacing);
      ACurrentRowProperties.ResetUse
        (TdxTableRowPropertiesOptions.MaskUseCellSpacing);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if not LeftMargin.IsNull and APropertiesException.GetUse
      (TdxTablePropertiesOptions.MaskUseLeftMargin) then
    begin
      ACellMargins.Left.CopyFrom(ADefaultCellMargins.Left);
      APropertiesException.ResetUse
        (TdxTablePropertiesOptions.MaskUseLeftMargin);
    end;
    if not RightMargin.IsNull and APropertiesException.GetUse
      (TdxTablePropertiesOptions.MaskUseRightMargin) then
    begin
      ACellMargins.Right.CopyFrom(ADefaultCellMargins.Right);
      APropertiesException.ResetUse
        (TdxTablePropertiesOptions.MaskUseRightMargin);
    end;
    if not TopMargin.IsNull and APropertiesException.GetUse
      (TdxTablePropertiesOptions.MaskUseTopMargin) then
    begin
