...
uses
  ...
  cxGroupBox, cxLookAndFeels, IniFiles, cxClasses, dxLayoutContainer,
  dxLayoutControl,
...
type
  ...
  TdxPrintDlgOption = (pdoPrintToFile, pdoAllPages, pdoCurrentPage,
    pdoSelection, pdoPageRange, pdoPrintStyles);
...
const
  ...
  pdoDefaultOptionsVisible = [pdoPrintToFile, pdoAllPages, pdoCurrentPage,
    pdoPageRange];
...
type
  ...
  published
    property ButtonsEnabled: TdxPrintDlgButtons read FButtonsEnabled
      write FButtonsEnabled default [pdbPrinterProperties, pdbNetwork,
      pdbPreview, pdbPageSetup, pdbDefineStyles, pdbStyleOptions];
    property ButtonsVisible: TdxPrintDlgButtons read FButtonsVisible
      write FButtonsVisible default [pdbPrinterProperties, pdbNetwork,
      pdbPreview, pdbPageSetup, pdbDefineStyles, pdbStyleOptions];
    ...
    property OptionsEnabled: TdxPrintDlgOptions read FOptionsEnabled
      write FOptionsEnabled default [pdoPrintToFile, pdoAllPages, pdoPageRange];
    property OptionsVisible: TdxPrintDlgOptions read FOptionsVisible
      write FOptionsVisible default [pdoPrintToFile, pdoAllPages,
      pdoCurrentPage, pdoPageRange];
    ...
    property PrintToFile: Boolean read GetPrintToFile write SetPrintToFile
      default False;
    property StyleManager: TdxPrintStyleManager read GetStyleManager
  ...
  private
    ...
    procedure DrawCollatedPages(DC: HDC; const ADrawRect: TRect;
      ACollate: Boolean);
  ...
  protected
    ...
    function DoShowStylePageSetup(AStyle: TBasedxPrintStyle;
      APageIndex: Integer; AShowPreviewBtn, AShowPrintBtn: Boolean;
      out APreviewBtnClicked, APrintBtnClicked: Boolean): Boolean; virtual;
    procedure CallOnPageSetup(ASender: TObject; var ADone: Boolean;
      APreviewBtnClicked, APrintBtnClicked: PBoolean); dynamic;
    ...
    property OnPrintDeviceChanged: TNotifyEvent read FOnPrintDeviceChanged
      write FOnPrintDeviceChanged;
    property OnPrintersChanged: TNotifyEvent read FOnPrintersChanged
      write FOnPrintersChanged;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    function Execute: Boolean;
...
uses
  Variants, WinSpool, Math, Types, cxGeometry, dxPSPopupMan, dxPSRes, dxPSEngn,
  dxPSEvnt, dxPSUtl, dxPSImgs;
...
procedure TdxPrintDialog.AssignTo(Dest: TPersistent);
  ...
  procedure XorOption(var AOptions: TPrintDialogOptions;
    AItem: TPrintDialogOption; AValue: Boolean);
  begin
    if AValue then
...
begin
  if Dest is Dialogs.TPrintDialog then
    ...
    begin
      ...
      SrcOptions := Options;
      XorOption(SrcOptions, poPrintToFile, pdoPrintToFile
        in Self.OptionsVisible);
      XorOption(SrcOptions, poDisablePrintToFile, pdoPrintToFile
        in Self.OptionsEnabled);
      XorOption(SrcOptions, poPageNums, (pdoPageRange in Self.OptionsVisible)
        and (pdoPageRange in Self.OptionsEnabled));
      XorOption(SrcOptions, poSelection, (pdoSelection in Self.OptionsVisible)
        and (pdoSelection in Self.OptionsEnabled));
      XorOption(SrcOptions, poHelp, (pdbHelp in Self.ButtonsVisible) and
        (pdbHelp in Self.ButtonsEnabled));
      Options := SrcOptions;
...
procedure TdxPrintDialog.Notification(AComponent: TComponent;
...
begin
  ...
  FSubscriber := TdxStyleListChangedSubscriber.Create
    ([TdxSMStyleListChangedEvent]);
  TdxStyleListChangedSubscriber(FSubscriber).OnStyleListChanged :=
    StyleListChanged;
...
begin
  ...
  lblNumberOfCopies.Caption := cxGetResourceString
...
function TdxfmPrintDialog.DoShowStylePageSetup(AStyle: TBasedxPrintStyle;
  APageIndex: Integer; AShowPreviewBtn, AShowPrintBtn: Boolean;
  out APreviewBtnClicked, APrintBtnClicked: Boolean): Boolean;
begin
  Result := AStyle.PageSetup(APageIndex, AShowPreviewBtn, AShowPrintBtn,
...
begin
  FDialogData.StyleManager.DefinePrintStylesDlg(APreviewBtnClicked,
    PrintBtnClicked);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      gbxPageRange.Visible := lirbtnAllPages.Visible or
        lirbtnCurrentPage.Visible or lirbtnSelection.Visible or
        lirbtnPageRanges.Visible;
      ...
      if (DialogData.StyleManager = nil) or not(pdoPrintStyles in OptionsVisible)
      then
      ...
      if libtnNetwork.Visible then
        btnNetwork.Enabled := dxPSUtl.IsNetworkPresent and
          (pdbNetwork in ButtonsEnabled);
...
begin
  ...
  begin
    Done := True;
    try
      CallOnPageSetup(Self, Done, @FPreviewBtnClicked, nil);
      // @FPrintBtnClicked);
    except
...
begin
  ...
  if FDialogData.StyleManager <> nil then
    TdxStyleListChangedSubscriber(FSubscriber)
      .StyleListChanged(FDialogData.StyleManager);
  SetActiveControl;
end;
...
begin
  ...
  if TcxCheckBox(Sender).Checked and (cbxFileName.Properties.Items.Count > 0)
  then
    cbxFileName.ItemIndex := 0;
  CheckModified;
...
begin
  if FControlsUpdating then
    Exit;
  FDialogData.PageRanges :=
    TdxPageRanges(TTagToInt(TcxRadioButton(Sender).Tag));
  CheckModified;
...
procedure TdxfmPrintDialog.FormCloseQuery(Sender: TObject;
...
var
  S: string;
begin
  S := StringReplace(TcxComboBox(Sender).Text, ' ', '',
...
begin
  ...
  begin
    try
      ...
      if not b then
        FwpPageRanges.Hint :=
          Format(cxGetResourceString(@sdxPrintDialogPageNumbersOutOfRange),
          [FDialogData.MinRange, FDialogData.MaxRange]);
    except
      b := False;
      FwpPageRanges.Hint :=
        Format(cxGetResourceString(@sdxPrintDialogInvalidPageRanges),
        [edPageRanges.Text]);
    end;
    if not b and not FwpPageRanges.State and (ActiveControl <> btnOK) and
...
procedure TdxfmPrintDialog.edPageRangesKeyPress(Sender: TObject; var KEY: Char);
  ...
  begin
    ...
    if not Result and (Text <> '') then
      Result := (AKey = cPageSeparator) or (AKey = cPageRangeSeparator) or
        (AKey = dxVKBack);
  end;
...
begin
  with lbxPrintStyles do
    FDialogData.StyleManager.CurrentStyle :=
      TBasedxPrintStyle(Items.Objects[ItemIndex]);
  btnCancel.Caption := cxGetResourceString(@sdxBtnClose);
end;
...
begin
  ...
  begin
    ...
    if DoShowStylePageSetup(Style, 0, True, True, FPreviewBtnClicked,
      FPrintBtnClicked) then
      btnCancel.Caption := cxGetResourceString(@sdxBtnClose);
    if FPreviewBtnClicked then
      ModalResult := mrOk;
  end;
...
procedure TdxfmPrintDialog.LoadFromIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  inherited LoadFromIniFile(AIniFile, ASectionName);
  FPrintStylesVisibled := AIniFile.ReadBool(ASectionName, sdxMaximized,
    FPrintStylesVisibled);
end;

procedure TdxfmPrintDialog.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
begin
  ...
  begin
    ...
    if Result then
    begin
      if (Length(FileName) > 2) and (FileName[1] = '"') and
      ...
      begin
        ...
        if not Result then
          Result := MessageQuestion
            (Format(cxGetResourceString(@sdxConfirmOverWrite), [RealFileName]))
      end
      else
        MessageWarning(Format(cxGetResourceString(@sdxInvalidFileName),
          [FileName]));
    end
    else
...
procedure TdxfmPrintDialog.DrawCollatedPages(DC: HDC; const ADrawRect: TRect;
  ACollate: Boolean);
  ...
  procedure DrawPages(const APageRect: TRect; const AOffsets: TPoint;
    ...
    begin
      ...
      TextOut(DC, R.Right - Size.cX - ScaleFactor.Apply(2),
        R.Bottom - Size.cY - ScaleFactor.Apply(1), PChar(S), Length(S));
      with R do
...
begin
  ...
  ShiftX := ((W div (3 - Byte(ACollate))) - (PageWidth + Offsets.X *
    (1 + Byte(ACollate)))) div 2;
...
begin
  with FDialogData do
  begin
    Pages := StringReplace(edPageRanges.Text, ' ', '',
      [rfReplaceAll, rfIgnoreCase]);
...
begin
  ...
  else if cbxNumberOfPages.CanFocus then
    ActiveControl := cbxNumberOfPages
  else if btnOK.CanFocus then
    ActiveControl := btnOK;
end;
...
begin
  ...
  try
    ...
    else if Assigned(FOnPageSetup) then
    begin
      btnPageSetup.Caption := cxGetResourceString(@sdxBtnPageSetup);
      btnPageSetup.Glyph := nil;
    end;
    cbxPrinters.Enabled := cbxPrinters.Properties.Items.Count > 0;
    btnPrinterProperties.Enabled := btnPrinterProperties.Enabled and
      (cbxPrinters.Properties.Items.Count > 0);
...
begin
  cbxFileName.Enabled := chbxPrintToFile.Enabled and
    lichbxPrintToFile.Visible and chbxPrintToFile.Checked;
  ...
  btnOK.Enabled := (dxPrintDevice.Printers.Count > 0) or
    chbxPrintToFile.Checked;
  btnPageSetup.Enabled := Assigned(OnPageSetup) or
    Assigned(FDialogData.StyleManager);
end;
...
begin
  try
    if WinSpool.OpenPrinter(dxPrintDevice.CurrentDevice, PrinterHandle, nil)
    then
      try
        ...
        begin
          ...
          if PBuffer <> nil then
            try
              if WinSpool.GetPrinter(PrinterHandle, 2, PBuffer, Size, @Size)
              then
              begin
                ...
                if PrinterInfo2.cJobs > 0 then
                  lStatus.Caption :=
                    ...
                    (@sdxPrintDialogPSPrintingAndWaiting), [PrinterInfo2.cJobs])
                else
                  lStatus.Caption := GetStatusString(PrinterInfo2.Status);

                if (dxPrintDevice.DeviceMode <> nil) and
                  (PrinterInfo2.pDevMode <> nil) then
    ...
    seCopies.Enabled := seCopies.Properties.MaxValue >
      seCopies.Properties.MinValue;
  except
    Application.HandleException(Self);
  end;
end;
...
begin
  with TdxPrintDeviceInfo(AControl.Properties.Items.Objects[AIndex]) do
    dxDrawPrinter(ACanvas, ARect, AControl.Properties.Items[AIndex], ilPrinters,
      Ord(IsNetwork) + 2 * Ord(IsDefault),
      not((odComboBoxEdit in AState) and AControl.ViewInfo.NativeStyle and
      IsWinVistaOrLater), ScaleFactor);
end;
