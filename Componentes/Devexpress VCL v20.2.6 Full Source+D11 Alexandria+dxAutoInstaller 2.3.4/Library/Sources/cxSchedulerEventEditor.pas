...
{$ENDIF}
  ...
  dxCore, cxDateUtils, cxSchedulerEditorFormManager, cxClasses,
  dxLayoutContainer, dxLayoutControl, dxLayoutLookAndFeels,
  dxLayoutControlAdapters, dxLayoutcxEditAdapters, dxForms;

type
  { TcxShedulerStandardEventEditorFormStyleInfo }

  TcxShedulerStandardEventEditorFormStyleInfo = class
    (TcxShedulerEventEditorFormStyleInfo)
  public
    class function CreateEditor(AEvent: TcxSchedulerControlEvent)
      : IcxSchedulerEventEditorForm; override;
  ...
  protected
    ...
    property OnIsValid: TcxSchedulerEditorIsValidEvent read FOnIsValid
      write FOnIsValid;
  public
    ...
    procedure Changed;
    function ValidateOccurrenceDate(AEvent: TcxSchedulerControlEvent;
      ANewStart, ANewFinish: TDateTime): Boolean;
    ...
    property OnSaveChanges: TNotifyEvent read FOnSaveChanges
      write FOnSaveChanges;
  end;
  ...
  public
    constructor Create(AForm: TForm; AEvent: TcxSchedulerControlEvent);
      override;
    ...
    property AllowHiddenEvents: Boolean read FAllowHiddenEvents
      write FAllowHiddenEvents;
    property AllowRecurrence: Boolean read FAllowRecurrence
      write SetAllowRecurrence;
    ...
    property IsIntersectionValid: Boolean read FIsIntersectionValid
      write SetIsIntersectionValid;
    ...
    property RecurrenceActivate: Boolean read FRecurrenceActivate
      write FRecurrenceActivate;
    ...
    property ShowTaskComplete: Boolean read FShowTaskComplete
      write FShowTaskComplete;
    property OnDelete: TcxOnDeleteEventFunc read FOnDelete write FOnDelete;
    property OnApplyEventTime: TNotifyEvent read FOnApplyEventTime
      write FOnApplyEventTime;
  end;
  ...
  TcxSchedulerEventStandardEditorFormHelper = class
    (TcxSchedulerEventEditorFormHelper);
  ...
  public
    ...
    property AllowHiddenEvents: Boolean read GetAllowHiddenEvents
      write SetAllowHiddenEvents;
    property AllowRecurrence: Boolean read GetAllowRecurrence
      write SetAllowRecurrence;
    ...
    property RecurrenceActivate: Boolean read GetRecurrenceActivate
      write SetRecurrenceActivate;
    property ShowResources: Boolean read GetShowResources
      write SetShowResources;
    property ShowTaskComplete: Boolean read GetShowTaskComplete
      write SetShowTaskComplete;
    property OnDeleteFunc: TcxOnDeleteEventFunc read GetOnDeleteFunc
      write SetOnDeleteFunc;
  end;
  ...
  TcxSchedulerEventEditorForm = class(TcxSchedulerCustomEventEditor,
    ...
    procedure cbReminderMinutesBeforeStartPropertiesValidate(Sender: TObject;
  ...
  protected
    ...
    procedure GetEditingEventTime(var AStart, AFinish: TDateTime;
      var AState: Integer); virtual;
    ...
    procedure InitializeTimeControls(const AStart, AFinish: TDateTime;
      AllDayEvent: Boolean); virtual;
    ...
    property DeleteExceptions: Boolean read GetDeleteExceptions
      write SetDeleteExceptions;
  ...
  TcxSchedulerEventEditor = TcxSchedulerEventEditorForm;
  // workaround for Delphi 2005
...
class function TcxShedulerStandardEventEditorFormStyleInfo.CreateEditor
...
procedure TcxSchedulerCustomEditorFormHelper.ExecuteAction
...
begin
  ...
  begin
    AStatus := Event.Pattern.ValidateOccurrenceTimeBounds(Event, ANewStart,
      ANewFinish);
    if AStatus <> odsValid then
    begin
      AMessage := cxGetOccurrenceDateStatusMessage(Event, AStatus);
      MessageBox(Form.Handle, PChar(AMessage), nil,
...
procedure TcxSchedulerCustomEditorFormHelper.Cancel(var AModalResult
  : TModalResult);
begin
end;

procedure TcxSchedulerCustomEditorFormHelper.Close(var AModalResult
  : TModalResult);
begin
end;

procedure TcxSchedulerCustomEditorFormHelper.Delete(var AModalResult
  : TModalResult);
begin
end;

procedure TcxSchedulerCustomEditorFormHelper.Save(var AModalResult
  : TModalResult);
begin
  FModified := False;
end;
...
procedure TcxSchedulerCustomEditorFormHelper.DoExecuteAction(var AModalResult
  : TModalResult);
begin
end;
...
function TcxSchedulerCustomEditorFormHelper.GetStorage
...
begin
  ...
  begin
    ...
    FPatternFinish := AEvent.Pattern.Finish - FBiasTime +
      AEvent.StorageTimeBias;
  ...
  FAllowShare := True;
  FNeedCheckLossOfExceptions := (AEvent.Pattern <> nil) and
    AEvent.Pattern.HasExceptions;
...
procedure TcxSchedulerEventEditorFormHelper.DoExecuteAction(var AModalResult
  : TModalResult);
begin
  inherited;
...
begin
  ...
  begin
    Event.Pattern.RecurrenceInfo.DisplayTimeBias := -Storage.TimeBias + BiasTime
      - DateTimeHelper.CurrentTimeZoneBias;
    Result := cxGetRecurrenceDescriptionStringProc
      (Event.Pattern.RecurrenceInfo, True)
  end
  else
    Result := cxGetRecurrenceDescriptionStringProc(Event.RecurrenceInfo, True);
end;

procedure TcxSchedulerEventEditorFormHelper.Initialize
  (AEventInfo: TcxSchedulerEventEditInfo;
  AEditingInfo: TcxSchedulerEditingEventInfo);
begin
  BeginUpdate;
  try
...
begin
  ...
  if Result and Modified then
    Result := MessageDlg(cxGetResourceString(@scxDeleteConfirmation), mtWarning,
      [mbYes, mbNo], 0) = mrYes
end;
...
procedure TcxSchedulerEventEditorFormHelper.DoInitialize
  (AEventInfo: TcxSchedulerEventEditInfo;
  AEditingInfo: TcxSchedulerEditingEventInfo);
begin
  ...
  AllowRecurrence := AEventInfo.RecurrenceButton and
    Assigned(cxRecurrenceEventEditorClass) and
...
begin
  inherited DoRefresh;
  RefreshRecurrenceInfo;
  NeedCheckLossOfExceptions := (Event.Pattern <> nil) and
    Event.Pattern.HasExceptions;
...
begin
  ...
  begin
    ...
    begin
      if Event.Pattern <> nil then
        Result := ChangeType(Event.Pattern, Event, etPattern, True,
          not DeleteExceptions)
      else
  ...
  else if Event.Pattern <> nil then
...
procedure TcxSchedulerEventEditorFormHelper.Cancel(var AModalResult
  : TModalResult);
begin
  if CanClose then
  begin
...
procedure TcxSchedulerEventEditorFormHelper.Close(var AModalResult
  : TModalResult);
begin
  Form.Close;
end;

procedure TcxSchedulerEventEditorFormHelper.Delete(var AModalResult
  : TModalResult);
begin
...
procedure TcxSchedulerEventEditorFormHelper.Save(var AModalResult
  : TModalResult);
var
...
procedure TcxSchedulerEventEditorFormHelper.SetIsIntersectionValid
  (const Value: Boolean);
begin
...
procedure TcxSchedulerCustomEditorForm.HelperSaveChangesHandler
  (Sender: TObject);
begin
...
function TcxSchedulerCustomEventEditor.GetHelperClass
  : TcxSchedulerEditorHelperClass;
begin
...
procedure TcxSchedulerCustomEventEditor.Initialize
  (AEventInfo: TcxSchedulerEventEditInfo;
  AEditingInfo: TcxSchedulerEditingEventInfo);
begin
  Initialize;
...
function TcxSchedulerCustomEventEditor.GetHelper
  : TcxSchedulerEventEditorFormHelper;
begin
...
begin
  ...
  for I := 0 to cbResources.Properties.Items.Count - 1 do
    if cbResources.States[I] = cbsChecked then
      Event.ShareWithResource(Storage.ResourceIDs
        [cbResources.Properties.Items[I].Tag]);
end;
...
begin
  ...
  else if meMessage.CanFocus then
    ActiveControl := meMessage
  else if deStart.CanFocus then
...
begin
  ...
  btnDelete.Enabled := not(ReadOnly or Helper.IsNewEvent) and
    Helper.AllowDelete;
end;

function TcxSchedulerEventEditorForm.CheckTimeRange(var AStart,
...
begin
  ...
  if (Event.EventType = etNone) or (Event.Source = nil) or
    ((Event.EventType = etPattern) and not Event.Source.IsRecurring) or not ForceSeries
  then
  begin
    AStart := deStart.Date;
...
var
  AStart, AFinish: TDateTime;
begin
  Result := Storage.FindAvailableTime(Event, AllowHiddenEvents, AStart,
    AFinish);
  if Result then
...
procedure TcxSchedulerEventEditorForm.GetEditingEventTime(var AStart,
  AFinish: TDateTime; var AState: Integer);
begin
...
function TcxSchedulerEventEditorForm.GetHelperClass
  : TcxSchedulerEditorHelperClass;
begin
...
procedure TcxSchedulerEventEditorForm.Initialize
  (AEventInfo: TcxSchedulerEventEditInfo;
  AEditingInfo: TcxSchedulerEditingEventInfo);
begin
...
procedure TcxSchedulerEventEditorForm.InitializeHelper
  (AEvent: TcxSchedulerControlEvent);
begin
...
procedure TcxSchedulerEventEditorForm.InitializeTimeControls(const AStart,
  AFinish: TDateTime; AllDayEvent: Boolean);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if VarIsNull(AResource.ResourceID) or VarIsEmpty(AResource.ResourceID)
      then
        Continue;
      ...
      cbResources.States[J] :=
        TcxCheckBoxState(Event.IsSharedWithResource(AResource.ResourceID));
      Inc(J);
    end;
    EndUpdate;
  end;
...
begin
  ...
  Result := not(DateTimeHelper.RoundTime(ANewStart) > DateTimeHelper.RoundTime
    (ANewFinish));
  if not Result then
  begin
  ...
  begin
    ...
    begin
      MessageDlg(Format(cxGetResourceString(@scxTaskWrongTimeBounds),
        [cxDateTimeToText(AStart), cxDateTimeToText(AFinish)]), mtWarning,
        [mbOk], 0);
      if deStart.CanFocusEx then
        ActiveControl := deStart;
    end
    else if not Helper.ValidateOccurrenceDate(Event, ANewStart, ANewFinish) then
    begin
      Result := False;
      if deStart.CanFocusEx then
        ActiveControl := deStart;
...
begin
  ...
  cbReminderMinutesBeforeStart.Text :=
    cxMinutesToTextProc(Event.ReminderMinutesBeforeStart);
...
procedure TcxSchedulerEventEditorForm.SetAllowHiddenEvents(AValue: Boolean);
begin
  Helper.AllowHiddenEvents := AValue;
end;

procedure TcxSchedulerEventEditorForm.SetDeleteExceptions(AValue: Boolean);
begin
...
var
  ACanCorrect: Boolean;
begin
  ACanCorrect := icbShowTimeAs.ItemIndex = cxOriginalEventStates
    [Event.AllDayEvent];
  Event.AllDayEvent := cbAllDayEvent.Checked;
  if ACanCorrect then
  begin
...
begin
  OnChanged(nil);
  deEnd.Date := deStart.Date + dxDateOf(Helper.Duration) -
    Ord(cbAllDayEvent.Checked);
  ...
  Helper.Duration := (dxDateOf(deEnd.Date) + dxTimeOf(teEnd.Time)) -
    (dxDateOf(deStart.Date) + dxTimeOf(teStart.Time)) +
    Ord(cbAllDayEvent.Checked);
...
procedure TcxSchedulerEventEditorForm.
  cbReminderMinutesBeforeStartPropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
var
  AMinutes: Integer;
begin
...
procedure TcxSchedulerEventEditorForm.
  cbReminderMinutesBeforeStartPropertiesPopup(Sender: TObject);
var
  AMinutes: Integer;
  AText: string;
begin
  ...
  begin
    ...
    cbReminderMinutesBeforeStart.EditModified :=
      not SameText(AText, cbReminderMinutesBeforeStart.Text);
...
begin
  ...
  if cxShowRecurrenceEditor(Event, Helper.LookAndFeel, AModified, ReadOnly,
    Helper.NeedCheckLossOfExceptions, True, Helper.Form) then
  begin
  ...
  begin
    Event.Start := AStart;
    Event.Finish := AFinish;
    DeleteExceptions := not ReadOnly and
      (DeleteExceptions or ((Event.Pattern <> nil) and
...
begin
  if not FindAvailableTime then
    MessageBox(0, PChar(cxGetResourceString(@scxNoAvailableFreeTime)), nil,
...
procedure TcxSchedulerEventEditorForm.seTaskCompleteChange(Sender: TObject);
begin
  Modified := True;
...
procedure TcxSchedulerEventEditorForm.cbxTaskStatusChange(Sender: TObject);
begin
  Modified := True;
  if Helper.IsUpdating then
    Exit;
...
begin
  cxSchedulerEditorManager.RegisterShedulerEditorForm
    (TcxShedulerStandardEventEditorFormStyleInfo);
end;

procedure Finalize;
begin
  cxSchedulerEditorManager.UnregisterShedulerEditorForm
    (TcxShedulerStandardEventEditorFormStyleInfo);
end;
