...
uses
  cxGridWizardCustomHelper, cxGridCustomView, cxGridChartView,
  cxGridDBChartView, cxGridLevel, SysUtils, cxGraphics,
...
type
  ...
  TcxGridWizardDBChartViewHelper = class(TcxGridWizardCustomChartViewHelper,
...
uses
  cxClasses, Forms, cxGridWizardCustomPage, cxGridWizardDBViewsDataSourcePage,
  cxGridWizardDBViewsSelectItemsForDisplayPage,
  cxGridWizardCommonCustomizeItemsPage,
  cxGridWizardChartViewOptionsViewSettingsPage,
  cxGridWizardUnboundViewsSelectItemsForDisplayPage,
  cxGridCommon;

const
  cxGridWizardChartViewPages: array [0 .. 2]
  ...
  cxGridWizardDBChartViewPages: array [0 .. 3]
    of TcxGridWizardCustomPageFrameClass =
...
procedure TcxGridWizardCustomChartViewHelper.ChangeItemIndex(const AOldIndex,
  ANewIndex: Integer);
begin
  Item[AOldIndex].Index := ANewIndex;
end;

procedure TcxGridWizardCustomChartViewHelper.DeleteItem(const AItemIndex
  : Integer);
begin
...
begin
  ...
  FSavedOptionsCustomizationOptionValue :=
    GridView.OptionsCustomize.OptionsCustomization;
  FSavedSeriesCustomizationOptionValue :=
    GridView.OptionsCustomize.SeriesCustomization;
...
procedure TcxGridWizardCustomChartViewHelper.CheckComponentNames
...
begin
  ...
  for I := 0 to AChartView.SeriesCount - 1 do
    AChartView.Series[I].Name :=
      CreateUniqueName(GetParentForm(AChartView.Control), AChartView,
      AChartView.Series[I], ScxGridPrefixName, '');
end;

function TcxGridWizardCustomChartViewHelper.GetItemCaption
...
function TcxGridWizardCustomChartViewHelper.GetItemVisible
  (Index: Integer): Boolean;
begin
  Result := Item[Index].Visible;
end;

procedure TcxGridWizardCustomChartViewHelper.SetItemCaption(Index: Integer;
  const AValue: string);
begin
  Item[Index].DisplayText := AValue;
end;

procedure TcxGridWizardCustomChartViewHelper.SetItemVisible(Index: Integer;
...
function TcxGridWizardCustomChartViewHelper.GetItem(Index: Integer)
  : TcxGridChartSeries;
begin
...
class function TcxGridWizardChartViewHelper.GetGridViewClass
...
function TcxGridWizardDBChartViewHelper.GetItemIndexByFieldName(const AFieldName
  : string): Integer;
var
...
class function TcxGridWizardDBChartViewHelper.GetGridViewClass
  : TcxCustomGridViewClass;
begin
...
function TcxGridWizardDBChartViewHelper.GetItemFieldName
  (Index: Integer): string;
begin
...
function TcxGridWizardDBChartViewHelper.GetItem(Index: Integer)
