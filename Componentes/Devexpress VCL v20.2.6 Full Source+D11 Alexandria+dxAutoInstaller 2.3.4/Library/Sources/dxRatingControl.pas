...
uses
  ...
  cxFilterControlUtils, cxGraphics, cxTextEdit, cxVariants, dxCoreClasses,
  dxGDIPlusClasses;
...
type
  ...
  protected
    ...
    procedure DrawIndicatorRect(ACanvas: TcxCanvas;
      AState: TdxRatingControlIndicatorState); virtual;
    function CheckHit(X, Y: Integer): Boolean;
    function GetHeight: Integer;
    function GetWidth: Integer;

    property Bounds: TRect read FBounds;
    property RatingControlViewInfo: TdxCustomRatingControlViewInfo
      read FRatingControlViewInfo;
    property Painter: TcxCustomLookAndFeelPainter read GetPainter;
  public
    constructor Create(ARatingControlViewInfo
      : TdxCustomRatingControlViewInfo); virtual;
  ...
  private
    ...
    procedure SetItem(AIndex: Integer;
      AValue: TdxCustomRatingControlIndicatorViewInfo);
  public
    property Items[Index: Integer]: TdxCustomRatingControlIndicatorViewInfo
  ...
  protected
    ...
    property CheckedGlyph: TdxSmartImage read FCheckedGlyph
      write SetCheckedGlyph;
    property FillPrecision: TdxRatingControlFillPrecision read FFillPrecision
      write FFillPrecision;
    ...
    property Indicators: TdxCustomRatingControlIndicatorViewInfos
      read FIndicators;
    ...
    property RatingLastValue: Double read FRatingLastValue
      write FRatingLastValue;
    property RequiredHeight: Integer read GetRequiredHeight;
    property RequiredWidth: Integer read GetRequiredWidth;
    property ReverseDirection: Boolean read GetReverseDirection
      write FReverseDirection;
  ...
  protected
    procedure CalculateViewInfoByProperties
  ...
  protected
    ...
    function ChangeEditValueByStep(const AEditValue: Variant; AIsInc: Boolean)
      : Variant; virtual;
  ...
  public
    ...
    property AllowHover: Boolean read FAllowHover write SetAllowHover
      default True;
    property CheckedGlyph: TdxSmartGlyph read FCheckedGlyph
      write SetCheckedGlyph;
    property FillPrecision: TdxRatingControlFillPrecision read FFillPrecision
      write SetFillPrecision default rcfpFull;
    ...
    property ItemCount: Integer read FItemCount write SetItemCount
      default dxRatingControlDefaultItemCount;
  ...
  protected
    ...
    procedure InternalSetRating(AValue: Double;
      ANeedResetEqualsValue: Boolean = True); virtual;
  ...
  public
    ...
    property ActiveProperties: TdxCustomRatingControlProperties
      read GetActiveProperties;
    property Properties: TdxCustomRatingControlProperties read GetProperties
      write SetProperties;
    property Rating: Double read GetRating write SetRating
      stored GetRatingStored;
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TdxRatingControlProperties
      read GetActiveProperties;
  published
    property Anchors;
    property AutoSize;
...
function AdjustRatingByStep(ARating: Double; AStep: Double;
  ARoundingUp: Boolean = True): Double;
var
  AStepCount: Double;
begin
  Result := ARating;
...
procedure TdxCustomRatingControlIndicatorViewInfo.Offset(DX: Integer;
  DY: Integer);
begin
  OffsetRect(FBounds, DX, DY);
end;

procedure TdxCustomRatingControlIndicatorViewInfo.CalculateBounds(ALeft,
  ATop: Integer);
begin
...
begin
  ...
  begin
    ...
    begin
      AFractionalPartSize := GetFractionalPartWidth
        (RatingControlViewInfo.Rating);
    ...
    begin
      AFractionalPartSize := GetFractionalPartHeight
        (RatingControlViewInfo.Rating);
      if not RatingControlViewInfo.ReverseDirection then
  ...
  if rcisHover in FStates then
    ...
    else if not RatingControlViewInfo.ReverseDirection then
...
begin
  ...
  begin
    ...
    begin
      AFractionalPartSize := GetFractionalPartWidth
        (RatingControlViewInfo.HoverRating);
      if not RatingControlViewInfo.ReverseDirection then
...
begin
  ...
  if CompareValue(ARating, Index + 1, DoubleResolution) >= 0 then
    FStates := [rcisChecked]
  else if (Floor(ARating) = Index) and
    (CompareValue(Round(ARating), ARating, DoubleResolution) <> 0) then
  ...
  if CompareValue(AHoverRating, Index + 1, DoubleResolution) >= 0 then
    FStates := [rcisHover]
  else if (Floor(AHoverRating) = Index) and
    (CompareValue(Round(AHoverRating), AHoverRating, DoubleResolution) <> 0)
  then
  begin
...
procedure TdxCustomRatingControlIndicatorViewInfo.DrawIndicatorRect
  (ACanvas: TcxCanvas; AState: TdxRatingControlIndicatorState);
var
  ...
  try
    ...
    begin
      Painter.DrawRatingControlScaledIndicator(ACanvas,
        cxRectCenter(Bounds, Painter.GetRatingControlScaledIndicatorSize
        (RatingControlViewInfo.ScaleFactor)), AState,
        RatingControlViewInfo.ScaleFactor);
    end
    else
      cxDrawImage(ACanvas, cxRectCenter(Bounds, dxGetImageSize(AGlyph,
        RatingControlViewInfo.ScaleFactor)), AGlyph, ifmNormal,
        Painter.GetRatingControlIndicatorColorPalette(AState),
        RatingControlViewInfo.ScaleFactor);
  finally
    ACanvas.RestoreClipRegion;
  end;
...
function TdxCustomRatingControlIndicatorViewInfo.CheckHit(X,
  Y: Integer): Boolean;
begin
...
function TdxCustomRatingControlIndicatorViewInfo.GetFractionalPartHeight
  (AValue: Double): Integer;
begin
  Result := Round(GetHeight * Frac(AValue));
end;

function TdxCustomRatingControlIndicatorViewInfo.GetFractionalPartWidth
  (AValue: Double): Integer;
begin
  Result := Round(GetWidth * Frac(AValue));
end;

function TdxCustomRatingControlIndicatorViewInfo.GetPainter
...
function TdxCustomRatingControlIndicatorViewInfos.GetItem(AIndex: Integer)
  : TdxCustomRatingControlIndicatorViewInfo;
begin
  Result := TdxCustomRatingControlIndicatorViewInfo(inherited Items[AIndex]);
end;

procedure TdxCustomRatingControlIndicatorViewInfos.SetItem(AIndex: Integer;
  AValue: TdxCustomRatingControlIndicatorViewInfo);
begin
...
procedure TdxCustomRatingControlViewInfo.Calculate(ACanvas: TcxCanvas;
  APoint: TPoint);
begin
...
begin
  ...
  if (ARatingControlViewInfo = nil) or
    (CompareValue(HoverRating, ARatingControlViewInfo.HoverRating,
    DoubleResolution) <> 0) then
  begin
...
begin
  ...
  if not IsInplace then
    ARect := cxRectInflate(ARect, -GetLeftIndent, -GetTopIndent,
      -GetRightIndent, -GetBottomIndent);
...
procedure TdxCustomRatingControlViewInfo.CalculateTextParams
  (ACanvas: TcxCanvas);
var
  ARect: TRect;
begin
...
function TdxCustomRatingControlViewInfo.GetRatingByMousePos(X,
  Y: Integer): Double;

  function CalculateRatingByIndicator(AIndicator
    : TdxCustomRatingControlIndicatorViewInfo; X, Y: Integer): Double;
  var
  ...
  begin
    ...
    else if not ReverseDirection then
  ...
  function GetIndicatorBoundsWithIndent(AIndicator
    : TdxCustomRatingControlIndicatorViewInfo): TRect;
  begin
...
procedure TdxCustomRatingControlViewInfo.DrawIndicator(ACanvas: TcxCanvas;
  AIndex: Integer);
begin
...
begin
  ...
  try
    ...
    cxBitBlt(ABufferCanvas.Handle, ACanvas.Handle, ABufferBitmap.ClientRect,
    ...
    cxBitBlt(ACanvas.Handle, ABufferCanvas.Handle, Bounds, cxNullPoint,
      SRCCOPY);
  finally
    ABufferBitmap.Free;
  end;
...
begin
  ...
  if (Painter <> nil) and (Glyph.Empty or HoverGlyph.Empty or CheckedGlyph.Empty)
  then
    Result := cxSizeMax(Result, Painter.GetRatingControlScaledIndicatorSize
      (ScaleFactor));
end;
...
begin
  if Orientation = orHorizontal then
    ...
    Result := ItemCount * IndicatorSize.cy + (ItemCount - 1) *
      GetIndicatorIndent;
end;
...
begin
  if Orientation = orHorizontal then
    Result := ItemCount * IndicatorSize.cx + (ItemCount - 1) *
      GetIndicatorIndent
  else
...
procedure TdxCustomRatingControlViewInfo.SetCheckedGlyph
  (const Value: TdxSmartImage);
begin
...
procedure TdxCustomRatingControlViewInfo.SetHoverGlyph
  (const Value: TdxSmartImage);
begin
...
procedure TdxCustomRatingControlViewData.EditValueToDrawValue(const AEditValue
...
begin
  ...
  if Properties.ShowEndEllipsis then
    ARatingControlViewInfo.DrawTextFlags :=
      ARatingControlViewInfo.DrawTextFlags or CXTO_END_ELLIPSIS
  else
    ARatingControlViewInfo.DrawTextFlags :=
      ARatingControlViewInfo.DrawTextFlags and not CXTO_END_ELLIPSIS;
  ARatingControlViewInfo.Step := Properties.Step;
  if Edit <> nil then
  begin
    ARatingControlViewInfo.Text := TdxRatingControl(Edit).Caption;
    ARatingControlViewInfo.UseRightToLeftAlignment := TdxRatingControl(Edit)
      .UseRightToLeftAlignment;
  end;
end;

function TdxCustomRatingControlViewData.InternalGetEditConstantPartSize
...
function TdxCustomRatingControlViewData.GetProperties
  : TdxCustomRatingControlProperties;
begin
...
class function TdxCustomRatingControlProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TdxRatingControl;
end;

function TdxCustomRatingControlProperties.GetDisplayText(const AEditValue
  : Variant; AFullText: Boolean = False; AIsInplace: Boolean = True): string;
var
  ADisplayValue: TcxEditValue;
begin
...
class function TdxCustomRatingControlProperties.GetStyleClass
  : TcxCustomEditStyleClass;
begin
  Result := TdxRatingControlStyle;
end;

function TdxCustomRatingControlProperties.GetSupportedOperations
  : TcxEditSupportedOperations;
begin
  Result := [esoAlwaysHotTrack, esoFiltering, esoSorting, esoEditing,
    esoTransparency];
end;
...
function TdxCustomRatingControlProperties.ChangeEditValueByStep(const AEditValue
  : Variant; AIsInc: Boolean): Variant;
var
...
function TdxCustomRatingControlProperties.CorrectRatingByRange
  (AValue: Double): Double;
begin
  Result := Math.Min(Math.Max(AValue, 0), ItemCount);
end;

procedure TdxCustomRatingControlProperties.CorrectViewInfoRating
  (AViewInfo: TdxCustomRatingControlViewInfo);
begin
  AViewInfo.Rating := GetCorrectedRating(AViewInfo.Rating);
end;

procedure TdxCustomRatingControlProperties.DoAssign
...
function TdxCustomRatingControlProperties.GetCorrectedRating
  (ARating: Double): Double;
begin
...
class function TdxCustomRatingControlProperties.GetViewDataClass
...
class function TdxCustomRatingControlProperties.GetViewInfoClass
...
procedure TdxCustomRatingControlProperties.PrepareDisplayValue(const AEditValue
...
procedure TdxCustomRatingControlProperties.SetCheckedGlyph
  (AValue: TdxSmartGlyph);
begin
  FCheckedGlyph.Assign(AValue);
end;

procedure TdxCustomRatingControlProperties.SetFillPrecision
  (AValue: TdxRatingControlFillPrecision);
begin
...
procedure TdxCustomRatingControlProperties.SetOrientation
  (AValue: TdxOrientation);
begin
...
class function TdxCustomRatingControl.GetPropertiesClass
...
procedure TdxCustomRatingControl.InternalSetRating(AValue: Double;
  ANeedResetEqualsValue: Boolean = True);
begin
  BeginUserAction;
  try
    if DoEditing then
    begin
      ...
      try
        if (CompareValue(AValue, Rating, DoubleResolution) = 0) and ANeedResetEqualsValue
        then
          AValue := 0;
        EditValue := AValue;
        ActiveProperties.Changed;
        ModifiedAfterEnter := True;
        if ActiveProperties.ImmediatePost and CanPostEditValue and ValidateEdit
        then
...
begin
  ...
  case Key of
    VK_LEFT, VK_DOWN, VK_RIGHT, VK_UP:
      begin
        AIsInc := not ActiveProperties.ReverseDirection and
          (Key in [VK_RIGHT, VK_UP]) or ActiveProperties.ReverseDirection and
          (Key in [VK_LEFT, VK_DOWN]);
...
function TdxCustomRatingControl.GetActiveProperties
  : TdxCustomRatingControlProperties;
begin
...
procedure TdxCustomRatingControl.SetProperties
  (Value: TdxCustomRatingControlProperties);
begin
...
class function TdxRatingControl.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
GetRegisteredEditProperties.Register(TdxRatingControlProperties,
  scxSEditRepositoryRatingControlItem);
FilterEditsController.Register(TdxRatingControlProperties,
  TdxFilterRatingControlHelper);

finalization

FilterEditsController.Unregister(TdxRatingControlProperties,
  TdxFilterRatingControlHelper);
