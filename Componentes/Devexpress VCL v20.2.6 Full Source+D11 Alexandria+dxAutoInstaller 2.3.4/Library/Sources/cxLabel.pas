...
type
  ...
  published
    property Alignment: TcxLabelLineAlignment read FAlignment write SetAlignment
      default cxllaCenter;
  ...
  protected
    ...
    property LabelStyleOffset: Integer read FLabelStyleOffset
      write FLabelStyleOffset;
    property LineInnerColor: TColor read FLineInnerColor write FLineInnerColor;
    property LineOrientation: TdxOrientation read FLineOrientation
      write FLineOrientation;
  ...
  protected
    ...
    function CalculateLineRect(var ATextRect: TRect; const ABounds: TRect;
      AAlignment: TcxLabelLineAlignment; AVisible: Boolean;
      ALineHeight: Integer; AOrientation: TdxOrientation): TRect; virtual;
    function CalculateTextRect(ACanvas: TcxCanvas; const R: TRect;
      const AText: string; ARealAngle: Integer): TRect; virtual;
    function CalculateTextSize(ACanvas: TcxCanvas; const R: TRect;
      AText: string; AFixedWidth: Boolean): TSize; virtual;
    procedure CalculateViewParams(ACanvas: TcxCanvas;
  ...
  public
    ...
    property LabelEffect: TcxLabelEffect read FLabelEffect write SetLabelEffect
      default cxleNormal;
    property LabelStyle: TcxLabelStyle read FLabelStyle write SetLabelStyle
      default cxlsNormal;
    property LineOptions: TcxLabelLineOptions read FLineOptions
      write SetLineOptions;
    property Orientation: TcxLabelOrientation read FOrientation
      write SetOrientation default cxoRightBottom;
    property PenWidth: Integer read FPenWidth write SetPenWidth default 1;
    property ShadowedColor: TColor read FShadowedColor write SetShadowedColor
      default clGrayText;
  ...
  public
    class function CalculatePositionOfRotatedText(ACanvas: TCanvas;
      AText: string; AShowAccelChar: Boolean; const ADrawingRect: TRect;
      const ADepthDeltaSize: TSize; AAngle: Integer;
      AAlignHorz: TcxEditHorzAlignment; AAlignVert: TcxEditVertAlignment)
      : TPoint; overload;
    class function CalculatePositionOfRotatedText(ACanvas: TCanvas;
      const AText: string; const ADrawingRect: TRect;
      AProperties: TcxCustomLabelProperties): TPoint; overload;
  end;
...
uses
  ...
  dxCoreGraphics,
  cxLookAndFeels, dxDPIAwareUtils;
...
function PrepareTextFlag(AHorzAlignments: TAlignment;
  AVertAlignments: TcxAlignmentVert; AShowEndEllipsis, AWordWrap,
  AShowAccelChar: Boolean): Longint;
const
...
begin
  Result := cxAlignmentsHorz[AHorzAlignments] or cxAlignmentsVert
    [AVertAlignments] or cxShowEndEllipsisArray[AShowEndEllipsis] or
...
procedure TcxCustomLabelViewInfo.InternalDrawBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
...
procedure TcxCustomLabelViewInfo.DrawLabel(ACanvas: TcxCanvas);
  ...
  begin
    ...
    Result := RGB(Trunc(GetRValue(BColor) + (GetRValue(EColor) -
      GetRValue(BColor)) * N / H),
      Trunc(GetGValue(BColor) + (GetGValue(EColor) - GetGValue(BColor)) * N /
      H), Trunc(GetBValue(BColor) + (GetBValue(EColor) - GetBValue(BColor))
      * N / H));
  end;
  ...
  begin
    if AAngle <> 0 then
    begin
      Result := TcxLabelHelper.CalculatePositionOfRotatedText(ACanvas, Text,
        ShowAccelChar, ADrawingRect, DepthDeltaSize, AAngle, Alignment.Horz,
        Alignment.Vert);
    end
    else
      Result := cxNullPoint;
  ...
  begin
    ...
    begin
      ...
      ExtTextOut(ACanvas.Handle, APosition.X, APosition.Y, 0, @ATextRect,
    ...
    begin
      ...
      ATextFlags := PrepareTextFlag(GetActualHorzAlignment,
        TcxAlignmentVert(Alignment.Vert), (epoShowEndEllipsis in PaintOptions),
        WordWrap, ShowAccelChar);
      if IsPaintOnGlass and not AIsInPath then
        dxDrawTextOnGlass(ACanvas, Text, ATextRect,
          cxFlagsToDTFlags(ATextFlags), True)
      else
  ...
  begin
    ...
    begin
      if LabelEffect = cxleFun then
        ACanvas.Font.Color := GetColor(BackgroundColor, ShadowedColor, I,
          AIterationCount);
  ...
  procedure DrawConventionalizedText(ACanvas: TcxCanvas; ARect: TRect;
    ...
    begin
      ...
      begin
        ...
        DrawActualText(ACanvas, ARect, Point(APosition.X + X, APosition.Y + Y),
          AIsTextRotated, True);
      ...
      cxlsRaised:
        begin
          ...
          begin
            ...
            DrawActualText(ACanvas, ARect, cxPointOffset(APosition,
              ScaleFactor.Apply(2)), AIsTextRotated);
          end;
          ACanvas.Font.Color := TextColor;
  ...
  procedure DrawLabelLine(ACanvas: TcxCanvas;
    const ALineRect, ATextRect: TRect);
  var
    ARect: TRect;
  begin
    ...
    begin
      ...
      try
        ...
        Painter.DrawLabelLine(ACanvas, ALineRect, LineOuterColor,
          LineInnerColor, LineOrientation = orVertical);
      finally
  ...
  procedure InternalDrawLabel(ADrawCanvas: TcxCanvas;
    const ABackgroundRect, ATextRect, ALineRect: TRect);
  var
    AIsTrueType: Boolean;
    ARealAngle: Integer;
    AStartPos: TPoint;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        DrawLabelEffect(ADrawCanvas, ATextRect, AStartPos, AStep,
          ARealAngle <> 0);
...
var
  ...
  ATextRect: TRect;
begin
  if not cxRectIsEmpty(ClientRect) and ACanvas.RectVisible(ClientRect) then
  begin
    ...
    if NativeStyle and not(Transparent or IsInplace or HasEffects) then
      GetThemeBackgroundContentRect(OpenTheme(totEdit, ScaleFactor),
        ACanvas.Handle, EP_EDITTEXT, NativeState, Bounds, AContentRect);
    if UseRightToLeftAlignment then
      ATextRect := TdxRightToLeftLayoutConverter.ConvertRect(ATextRect,
        AContentRect);
    InternalDrawLabel(ACanvas, AContentRect, ATextRect, LineRect);
  end;
end;
...
procedure TcxCustomLabelViewData.CalculateLabelViewInfoProps
...
begin
  inherited Calculate(ACanvas, ABounds, P, Button, Shift, AViewInfo,
    AIsMouseEvent);
  if ((ABounds.Bottom >= cxMaxRectSize) or (ABounds.Right >= cxMaxRectSize)) and IsInplace
  then
    Exit;
  ...
  ALabelViewInfo.DepthDeltaSize :=
    ScaleFactor.Apply(Properties.CalculateDepthDelta, Properties.ScaleFactor);
...
function TcxCustomLabelViewData.CalculateLabelStyleOffset
...
function TcxCustomLabelViewData.CalculateTextRect(ACanvas: TcxCanvas;
  const R: TRect; const AText: string; ARealAngle: Integer): TRect;
var
  ATextSize: TSize;
begin
  if IsInplace then
    Result := cxRectContent(R, EditContentParams.Offsets)
  else
    Result := R;

  if IsInplace and (ARealAngle = 0) and not IsLineVisible(ARealAngle) then
    Exit;
...
function TcxCustomLabelViewData.CalculateTextSize(ACanvas: TcxCanvas;
  const R: TRect; AText: string; AFixedWidth: Boolean): TSize;
var
  ADepthSize: TSize;
...
begin
  ...
  ATextFlags := PrepareTextFlag(taLeftJustify, vaTop, ShowEndEllipsis,
    Properties.WordWrap and (ARealAngle = 0) and
    not(IsInplace and (R.Right <= 0)), Properties.ShowAccelChar and
    (ARealAngle = 0));
  if not AFixedWidth then
    ATextFlags := ATextFlags or cxDontBreakChars;
  cxDrawText(ACanvas.Handle, AText, ARect, DT_CALCRECT or
    cxFlagsToDTFlags(ATextFlags));
  ...
  ADepthSize := ScaleFactor.Apply(Properties.CalculateDepthDelta,
...
procedure TcxCustomLabelViewData.CalculateViewParams(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomLabelViewInfo);
const
  LineOrientationMap: array [Boolean] of TdxOrientation = (orHorizontal,
    orVertical);
var
  ARealAngle: Integer;
  ARect: TRect;
begin
  ...
  try
    ...
    ARect := CalculateTextRect(ACanvas, AViewInfo.ClientRect, AViewInfo.Text,
      ARealAngle);
    AViewInfo.LineOrientation := LineOrientationMap[ARealAngle <> 0];
    AViewInfo.LineRect := CalculateLineRect(ARect, AViewInfo.ClientRect,
      LineOptions.Alignment, IsLineVisible(ARealAngle), GetLineHeight,
      AViewInfo.LineOrientation);
...
procedure TcxCustomLabelViewData.EditValueToDrawValue(const AEditValue
...
function TcxCustomLabelViewData.GetEditContentSize(ACanvas: TcxCanvas;
  ...
  function CalculateLabelMinSize(ACanvas: TcxCanvas; const AText: string;
    const ATextRect: TRect; ARealAngle: Integer;
    ADepthSize, AContentSizeCorrection: TSize; ALabelStyleOffset: Integer;
    AIsInplace, AWordWrap, AShowAccelChar: Boolean): TSize;
  var
    ARect: TRect;
  begin
    ...
    begin
      ...
      else if Style.BorderStyle <> ebsNone then
...
begin
  ...
  try
    ...
    Result := CalculateLabelMinSize(ACanvas, GetEditDisplayText,
      Rect(0, 0, Result.cx, Result.cy), ARealAngle,
      ScaleFactor.Apply(Properties.CalculateDepthDelta, Properties.ScaleFactor),
...
begin
  ...
  Result := DrawTextFlagsTocxTextOutFlags
    ...
    TcxAlignmentVert(Ord(Properties.Alignment.Vert)), ShowEndEllipsis,
...
begin
  Result := Properties.ShowEndEllipsis and
    ((Edit = nil) or not TcxCustomLabel(Edit).IsAutoWidth);
end;
...
function TcxCustomLabelProperties.GetSupportedOperations
...
begin
  if LineOptions.Visible and ((Angle = 0) or (Angle = 90)) then
    ...
    Result := [esoAutoHeight, esoFiltering, esoShowingCaption, esoSorting,
      esoTransparency, esoAutoWidth];
end;

class function TcxCustomLabelProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxCustomLabelViewInfo;
end;

function TcxCustomLabelProperties.IsEditValueValid(var EditValue: TcxEditValue;
...
procedure TcxCustomLabelProperties.DoAssign(AProperties
...
class function TcxCustomLabelProperties.GetViewDataClass
...
function TcxCustomLabelProperties.GetDisplayFormatOptions
  : TcxEditDisplayFormatOptions;
begin
  Result := [];
end;

function TcxCustomLabelProperties.GetEditValueSource(AEditFocused: Boolean)
...
class function TcxLabelHelper.CalculatePositionOfRotatedText(ACanvas: TCanvas;
  AText: string; AShowAccelChar: Boolean; const ADrawingRect: TRect;
  const ADepthDeltaSize: TSize; AAngle: Integer;
  AAlignHorz: TcxEditHorzAlignment; AAlignVert: TcxEditVertAlignment): TPoint;
var
  ACos: Extended;
  AHeight: Extended;
  AOffsetX: Extended;
...
begin
  ...
  ACos := Cos(ARadian);
  ASin := Sin(ARadian);
  AWidth := Abs(ATextSize.cx * ACos) + Abs(ATextSize.cy * ASin) +
    ADepthDeltaSize.cx;
  AHeight := Abs(ATextSize.cx * ASin) + Abs(ATextSize.cy * ACos) +
    ADepthDeltaSize.cy;
  ...
  case AAlignHorz of
    ...
    Result.X := Round((ADrawingRect.Left + ADrawingRect.Right -
      (ATextSize.cx * ACos + ATextSize.cy * ASin)) / 2);
  end;
  ...
  case AAlignVert of
    ...
    Result.Y := Round((ADrawingRect.Top + ADrawingRect.Bottom +
      (ATextSize.cx * ASin - ATextSize.cy * ACos)) / 2);
  end;
end;

class function TcxLabelHelper.CalculatePositionOfRotatedText(ACanvas: TCanvas;
  const AText: string; const ADrawingRect: TRect;
  AProperties: TcxCustomLabelProperties): TPoint;
begin
  Result := CalculatePositionOfRotatedText(ACanvas, AText,
    AProperties.ShowAccelChar, ADrawingRect, cxNullSize, AProperties.Angle,
    AProperties.Alignment.Horz, AProperties.Alignment.Vert);
end;
...
procedure TcxCustomLabel.Notification(AComponent: TComponent;
...
GetRegisteredEditProperties.Register(TcxLabelProperties,
