...
type
  ...
  public
    ...
    function Format(ADateTime: TDateTime; APatternLength: Integer): string;
      virtual; abstract;
  ...
  protected
    ...
    function InternalFormat(const ADateTime: TDateTime;
      const AFormatString: string): string; override;
    function FormatCore: string; virtual;
    function FormatByDefault(const AValue: TDateTime): string; override;
    function ProcessNextChar(AIndex: Integer; AResultString: TStringBuilder)
      : Integer; virtual;
    function TryCreateItem(AFormattingChar: Char;
      out AResult: TdxDateTimeFormattingItem): Boolean;
    function ProcessAsFormattingItem(AIndex: Integer;
      AFormattingItem: TdxDateTimeFormattingItem; AResultString: TStringBuilder)
      : Integer; virtual;
    function ProcessAsAMPMKeyword(AResultString: TStringBuilder)
      : Integer; virtual;
    function ProcessAsEmbedText(AIndex: Integer; AResultString: TStringBuilder)
      : Integer; virtual;
    function ProcessAsSingleCharacter(AIndex: Integer;
      AResultString: TStringBuilder): Integer; virtual;
    function IsKeyword(const AKeyword: string; AIndex: Integer)
      : Boolean; virtual;
  ...
  public
    ...
    property UseCurrentCultureDateTimeFormat: Boolean
      ...
      write FUseCurrentCultureDateTimeFormat;
    property FormatInfo: TFormatSettings read GetFormatInfo;
  end;
...
type
  ...
  strict private
    class var FPatternsLength: TArray<Integer>;
    class constructor Initialize;
  protected
    function GetPatternsLength: TArray<Integer>; override;
    function FormatInteger(AValue: Integer; APatternLength: Integer)
      : string; virtual;
  end;
  ...
  protected
    ...
    function GetAbbreviatedName(ADateTime: TDateTime): string; virtual;
      abstract;
    function GetFullName(ADateTime: TDateTime): string; virtual; abstract;
  public
    function Format(ADateTime: TDateTime; APatternLength: Integer)
      : string; override;
  end;
...
function TdxDateTimeFormattingItem.GetAvailablePatternLength(APatternLength
  : Integer): Integer;
var
  ACount, I: Integer;
...
function TdxDateTimeFieldFormatter.InternalFormat(const ADateTime: TDateTime;
...
function TdxDateTimeFieldFormatter.FormatByDefault(const AValue
  : TDateTime): string;
var
...
function TdxDateTimeFieldFormatter.ProcessNextChar(AIndex: Integer;
  AResultString: TStringBuilder): Integer;
var
  ACh: Char;
...
function TdxDateTimeFieldFormatter.ProcessAsAMPMKeyword(AResultString
  : TStringBuilder): Integer;
var
  AResult: string;
begin
...
function TdxDateTimeFieldFormatter.ProcessAsEmbedText(AIndex: Integer;
AResultString: TStringBuilder): Integer;
var
...
begin
  ...
  if AStartTextIndex >= Length(FFormatString) then
    Exit(1);
  ATextLength := GetCharacterSequenceLength(FFormatString[AIndex],
    AStartTextIndex, function(const Left, Right: Char): Boolean
    begin
      Result := Left <> Right;
    end);
...
function TdxDateTimeFieldFormatter.ProcessAsSingleCharacter(AIndex: Integer;
AResultString: TStringBuilder): Integer;
begin
...
function TdxDateTimeFieldFormatter.IsKeyword(const AKeyword: string;
...
function TdxDateTimeFieldFormatter.GetCharacterSequenceLength(ACh: Char;
AIndex: Integer; const APredicate: TEqualityComparison<Char>): Integer;
var
  ALength, ANextCharIndex: Integer;
begin
  ALength := Length(FFormatString);
  ANextCharIndex := AIndex + 1;
  while (ANextCharIndex <= ALength) and
    APredicate(ACh, FFormatString[ANextCharIndex]) do
    Inc(ANextCharIndex);
...
function TdxNumericFormattingItem.FormatInteger(AValue: Integer;
...
function TdxCombinedFormattingItem.Format(ADateTime: TDateTime;
APatternLength: Integer): string;
begin
...
APatternLength: Integer): string;
begin
  Result := FormatInteger(Calendar.FromDateTime(ADateTime).Hours,
    APatternLength);
end;
...
APatternLength: Integer): string;
begin
  Result := FormatInteger(Calendar.FromDateTime(ADateTime).Minutes,
    APatternLength);
end;
...
APatternLength: Integer): string;
begin
  Result := FormatInteger(Calendar.FromDateTime(ADateTime).Seconds,
    APatternLength);
end;
...
function TdxDayFormattingItem.GetDayOfWeek(ADateTime: TDateTime)
  : TdxDayOfWeekVCL;
begin
