...
type
  ...
  public
    property ConvertWithStyles: Boolean read FConvertWithStyles
      write FConvertWithStyles;
    property Designer_: IDesigner read FDesigner write FDesigner;
    property NameOfNewStyleRepository: string read FNameOfNewStyleRepository
      write FNameOfNewStyleRepository;
    property StyleRepository: TcxStyleRepository read GetStyleRepository
      write FStyleRepository;
  end;
  ...
  public
    ...
    procedure RegisterConverter(const AName: string;
      const AClass: TcxCustomConverterWithStylesClass);
    procedure UnregisterConverter(const AClass
      : TcxCustomConverterWithStylesClass);
    property Count: Integer read GetCount;
    property Items[AIndex: Integer]: TcxConverterFactoryItem
...
begin
  ...
  if AStyleRepository = nil then
    raise EcxConverterError.Create
      (cxGetResourceString(@scxConverterCantCreateStyleRepository));
...
begin
  raise EdxException.CreateFmt('Access class %s through Instance only',
    [ClassName]);
end;
...
function TcxConverterFactory.FindConverter(ASource: TObject)
  : TcxCustomConverterWithStylesClass;
var
  I: Integer;
begin
  ...
  begin
    ...
    if ClassInheritsFrom(ASource.ClassType, Result.GetSourceClassName) then
      Exit;
  end;
  Result := nil;
end;

class function TcxConverterFactory.Instance(AConverterGroupName: string)
  : TcxConverterFactory;
var
  I: Integer;
begin
...
procedure TcxConverterFactory.RegisterConverter(const AName: string;
  const AClass: TcxCustomConverterWithStylesClass);
var
...
procedure TcxConverterFactory.UnregisterConverter(const AClass
  : TcxCustomConverterWithStylesClass);
var
  AIndex: Integer;
begin
...
function TcxConverterFactory.IndexOf
  (AClass: TcxCustomConverterWithStylesClass): Integer;
var
  I: Integer;
begin
  Result := -1;
