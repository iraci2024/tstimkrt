...
uses
  Types, Classes, SysUtils, Windows, Graphics, Generics.Defaults,
  Generics.Collections,
  Controls,

  dxCoreClasses, cxGeometry, cxGraphics, dxCoreGraphics, dxGDIPlusAPI,
  dxGDIPlusClasses,
...
type
  ...
  public
    ...
    class function CalcTextSizeInt(G: TdxGraphics; const S: string;
      AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer)
      : TSize; overload;
    class function CalcTextSizeInt(G: TdxGraphics; const S: string;
      AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer;
      AMaxHeight: Integer): TSize; overload;
    class function CalcTextSizeInt(G: TdxGraphics; const S: string;
      AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer;
      AMaxHeight: Integer; out AIsCropped: Boolean): TSize; overload;
    class function CalcTextSize(G: TdxGraphics; const S: string;
      AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer)
      : TdxSizeF; overload; virtual;
    class function CalcTextSize(G: TdxGraphics; const S: string;
      AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer;
      AMaxHeight: Integer): TdxSizeF; overload; virtual;
    class function CalcTextSize(G: TdxGraphics; const S: string;
      AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer;
      AMaxHeight: Integer; out AIsCropped: Boolean): TdxSizeF;
      overload; virtual;
    procedure DrawLines(APen: TdxGPPen; const APoints: TArray<TdxPointF>);
    procedure DrawVString(const AText: string; AFont: TdxGPFont;
      AForeBrush: TdxGPCustomBrush; const ARect: TRect;
      AStrFormat: TdxGPStringFormat; AAngle: Integer);
    procedure FillPolygon(ABrush: TdxGPBrush;
      const APoints: TArray<TdxPointF>); overload;
    procedure FillPolygon(ABrush: TdxGPBrush; const APoints: TArray<TdxPointF>;
      AFillMode: FillMode); overload;

    procedure ExcludeClip(const R: TRect);
    procedure SetClip(const ARegion: TdxGPRegion;
    ...
    property Transform: TdxGPMatrix read GetWorldTransform
  ...
  public
    constructor Create(const ABounds: TRect; AColor1, AColor2: TdxAlphaColor;
      ALinearGradientMode: TdxGpLinearGradientMode); reintroduce;
  ...
  public
    constructor Create(AGraphics: TdxGraphics; const AViewPort: TSize;
      ADpi: Integer);
  ...
  private
    ...
    procedure Restore;
  public
    constructor Create(AGraphics: TdxGraphics;
  ...
  public
    constructor Create(AGraphics: TdxGraphics; const ANewOrigin: TPoint;
      AZoomFactor: Single; AMode: TMode = TMode.Replace);
    destructor Destroy; override;

    procedure SetHDCOrigin(const ANewOrigin: TPoint; AZoomFactor: Single;
      AMode: TMode); virtual;
  end;
...
function CreateImageFromResource(AInstance: HINST; const AResourceName: string)
  : TdxSmartImage;
...
class function TdxGraphics.CalcTextSizeInt(G: TdxGraphics; const S: string;
  AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer): TSize;
var
  AIsCropped: Boolean;
begin
  Result := CalcTextSizeInt(G, S, AFont, AStrFormat, AMaxWidth, MaxInt,
    AIsCropped);
end;
...
var
  AIsCropped: Boolean;
begin
  Result := CalcTextSizeInt(G, S, AFont, AStrFormat, AMaxWidth, AMaxHeight,
    AIsCropped);
end;

class function TdxGraphics.CalcTextSizeInt(G: TdxGraphics; const S: string;
  AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer;
  AMaxHeight: Integer; out AIsCropped: Boolean): TSize;
var
  ASize: TdxSizeF;
begin
  ASize := CalcTextSize(G, S, AFont, AStrFormat, AMaxWidth, AMaxHeight,
    AIsCropped);
  Result := TSize.Round(ASize);
end;

class function TdxGraphics.CalcTextSize(G: TdxGraphics; const S: string;
  AFont: TdxGPFont; AStrFormat: TdxGPStringFormat; AMaxWidth: Integer)
  : TdxSizeF;
var
...
procedure TdxGraphics.DrawLines(APen: TdxGPPen;
...
procedure TdxGraphics.DrawVString(const AText: string; AFont: TdxGPFont;
  AForeBrush: TdxGPCustomBrush; const ARect: TRect;
  AStrFormat: TdxGPStringFormat; AAngle: Integer);
var
  AMatrix: TdxGPMatrix;
...
procedure TdxGraphics.FillPolygon(ABrush: TdxGPBrush;
  const APoints: TArray<TdxPointF>);
begin
...
procedure TdxGraphics.FillPolygon(ABrush: TdxGPBrush;
  const APoints: TArray<TdxPointF>; AFillMode: FillMode);
begin
  if (Length(APoints) = 0) or (ABrush = nil) then
    raise EdxGdipException.Create(InvalidParameter);
  GdipCheck(GdipFillPolygon(Handle, ABrush.Handle, @APoints[0], Length(APoints),
    AFillMode));
end;

procedure TdxGraphics.ExcludeClip(const R: TRect);
begin
  GdipCheck(GdipSetClipRectI(Handle, R.Left, R.Top, R.Width, R.Height,
    CombineModeExclude));
end;

procedure TdxGraphics.SetClip(const ARegion: TdxGPRegion;
...
begin
  GdipCheck(GdipSetClipRect(Handle, R.Left, R.Top, R.Width, R.Height,
    CombineModeReplace));
  if AAdjustHdc then
...
begin
  ...
  AClipRgn := CreateRectRgn(APointPair[0].X - AViewPortOrigin.X,
    APointPair[0].Y - AViewPortOrigin.Y, APointPair[1].X - AViewPortOrigin.X,
    APointPair[1].Y - AViewPortOrigin.Y);
...
constructor TdxHdcDpiModifier.Create(AGraphics: TdxGraphics;
  const AViewPort: TSize; ADpi: Integer);
begin
  inherited Create;
  FGraphics := AGraphics;
  FViewPort := AViewPort;
...
begin
  ...
  try
    ...
    SetWindowExtEx(ADC, Trunc(FViewPort.cx * Dpi / AGraphicsDpiX),
      Trunc(FViewPort.cy * Dpi / AGraphicsDpiY), @FOldWindowExt);
...
begin
  ADC := FGraphics.GetHdc;
  try
    SetViewportExtEx(ADC, FOldViewportExt.cx, FOldViewportExt.cy,
      @FOldViewportExt);
...
constructor TdxGraphicsToLayoutUnitsModifier.Create(AGraphics: TdxGraphics;
  AUnitConverter: TdxDocumentLayoutUnitConverter);
begin
...
begin
  ...
  FHdcDpiModifier := TdxHdcDpiModifier.Create(FGraphics,
    TSize.Create(4096, 4096), Round(FUnitConverter.Dpi));
end;
...
constructor TdxHdcOriginModifier.Create(AGraphics: TdxGraphics;
  const ANewOrigin: TPoint; AZoomFactor: Single; AMode: TMode = TMode.Replace);
begin
  FGraphics := AGraphics;
...
procedure TdxHdcOriginModifier.SetHDCOrigin(const ANewOrigin: TPoint;
  AZoomFactor: Single; AMode: TMode);
var
  P: TPoint;
  ANewX, ANewY: Integer;
  ADC: HDC;
...
constructor TdxHdcZoomModifier.Create(AGraphics: TdxGraphics;
  AZoomFactor: Single);
begin
  FGraphics := AGraphics;
  ZoomHDC(AZoomFactor);
...
begin
  ...
  try
    ...
    SetWindowExtEx(ADC, Round(FOldWindowExtent.cx / AZoomFactor),
      Round(FOldWindowExtent.cy / AZoomFactor), @S);
  finally
    FGraphics.ReleaseHDC(ADC);
  end;
end;
...
constructor TdxLinearGradientBrush.Create(const ABounds: TRect;
  AColor1, AColor2: TdxAlphaColor;
  ALinearGradientMode: TdxGpLinearGradientMode);
begin
  inherited Create;
...
begin
  R := TargetRect;
  GdipCheck(GdipCreateLineBrushFromRect(@R, FColor1, FColor2,
    FLinearGradientMode, WrapModeTile, AHandle));
end;
...
const
  Map: array [TdxGPPenStyle] of TdxGpDashStyle = (DashStyleSolid, DashStyleDash,
    DashStyleDot, DashStyleDashDot, DashStyleDashDotDot);
begin
