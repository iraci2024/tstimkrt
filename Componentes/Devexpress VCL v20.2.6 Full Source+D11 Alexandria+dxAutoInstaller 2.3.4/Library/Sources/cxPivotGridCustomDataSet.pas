...
type
  ...
  protected
    ...
    function GetDefaultFieldType(APivotGridField: TcxPivotGridField)
      : TFieldType;
    function GetFieldType(APivotGridField: TcxPivotGridField)
      : TFieldType; virtual;

    procedure DoAssignFieldProperties(AField: TField;
      APivotGridField: TcxPivotGridField); virtual;
    ...
    property SynchronizeData: Boolean read FSynchronizeData
      write SetSynchronizeData default False;
    property OnCreateField: TcxPivotGridDataSetFieldEvent read FOnCreateField
      write FOnCreateField;
...
begin
  ...
  begin
    ...
    try
      ...
      if (PivotGrid <> nil) and TcxPivotGridDataBuilderAccess(DataBuilder).CanGroup
      then
      begin
        ReadOnly := False;
        try
...
function TcxPivotGridCustomDataSet.IsDBFieldAssigned(APivotGridField
  : TcxPivotGridField): Boolean;
begin
  Result := (APivotGridField is TcxDBPivotGridField) and
    (TcxDBPivotGridField(APivotGridField).DataBinding.DBField <> nil);
end;

function TcxPivotGridCustomDataSet.GetDBField(APivotGridField
  : TcxPivotGridField): TField;
begin
...
procedure TcxPivotGridCustomDataSet.DataChanged(Sender: TcxCustomPivotGrid);
begin
  // do nothing
end;

procedure TcxPivotGridCustomDataSet.LayoutChanged(Sender: TcxCustomPivotGrid);
begin
  // do nothing
end;

procedure TcxPivotGridCustomDataSet.PivotRemoved(Sender: TcxCustomPivotGrid);
begin
  PivotGrid := nil;
end;
...
function TcxPivotGridCustomDataSet.GetFieldType(APivotGridField
  : TcxPivotGridField): TFieldType;
begin
  if ((APivotGridField.Area <> faData) and (APivotGridField.Properties <> nil)
    and (APivotGridField.Properties.GetEditValueSource(False) = evsText)) or
    APivotGridField.TopValueShowOthers then
    Result := ftString
  else
    case APivotGridField.GroupInterval of
      giDate:
        Result := ftDate;
      giDateDay .. giDayAge, giNumeric:
        Result := ftInteger;
      giAlphabetical:
        Result := ftString;
    else
...
procedure TcxPivotGridCustomDataSet.DoAssignFieldProperties(AField: TField;
...
function TcxPivotGridCustomDataSet.DoCreateField(APivotGridField
  : TcxPivotGridField): TField;
...
function TcxPivotGridCustomDataSet.GetDefaultFieldType(APivotGridField
  : TcxPivotGridField): TFieldType;
begin
  if IsDBFieldAssigned(APivotGridField) then
  begin
    ...
    Result := VarTypeToDataType(TcxPivotGridFieldAccess(APivotGridField)
      .GetDataType);
  if (APivotGridField.Area = faData) and
    (APivotGridField.SummaryType in [stMin, stMax]) and
    APivotGridField.DataBinding.ValueTypeClass.IsString then
  begin
    Result := ftString;
    Exit;
  end;
  if (Result in [ftDate, ftDateTime]) and (APivotGridField.Area = faData) then
    if APivotGridField.SummaryType = stCount then
      Result := ftInteger
    else if not(APivotGridField.SummaryType in [stMin, stMax]) then
      Result := ftFloat;
end;
...
procedure TcxPivotGridCustomDataSet.SetPivotGrid(Value: TcxCustomPivotGrid);
begin
