...
type
  TdxResizeMarker = (Left, TopLeft, Top, TopRight, Right, BottomRight, Bottom,
    BottomLeft);
  ...
  public type
    TAddItemsBoundsProc = reference to procedure(AController
      : TdxGuideController);
{$REGION 'internal types'}
  ...
  type
    ...
    strict private
      class var FComparer: IComparer<TGuide>;
...
{$ENDREGION}
  strict private
    ...
    FTolerance: Integer;
  strict protected
    procedure CheckBoundsWidths(AOriginalX, ADelta: Integer; var X: Integer;
      AMarker: TdxResizeMarker; const ABounds: TRect);
    procedure CheckBoundsHeights(AOriginalY, ADelta: Integer; var Y: Integer;
      AMarker: TdxResizeMarker; const ABounds: TRect);
    procedure CheckGuide(const ABounds: TRect; AKind: TGuideKind;
      var ADelta: Integer);
    function GetBoundsValue(const ABounds: TRect; AKind: TGuideKind): Integer;
    function GetDirectionDelta(const ABounds: TRect;
      const AKinds: array of TGuideKind): Integer;
    function GetGuideBounds(const ABounds: TRect; ATolerance: Integer;
      AKind: TGuideKind; out AGuideBounds: TRect; var AKinds: TGuideKinds;
      var AValues: TGuideValues; AResize: Boolean): Boolean;
    function GetResizeTolerance(AMarker: TdxResizeMarker; AKind: TGuideKind;
      AExactMatch: Boolean): Integer;
    function FindNearestGuideValue(const ABounds: TRect; AKind: TGuideKind;
      out AValue: Integer): Boolean;
    function FindNearestSnappedX(var X: Integer; AMarker: TdxResizeMarker;
      const ABounds: TRect): Boolean;
    function FindNearestSnappedY(var Y: Integer; AMarker: TdxResizeMarker;
      const ABounds: TRect): Boolean;
  ...
  protected
    ...
    function GetGuides(const ABounds: TRect; AMarker: TdxResizeMarker)
      : TArray<TRect>; overload;

    property Tolerance: Integer read FTolerance;
  public
    constructor Create(AAddItemsBoundsProc: TAddItemsBoundsProc;
      ACapacity, ATolerance, AOverlap: Integer);
    destructor Destroy; override;
    procedure AddItemBounds(const ABounds: TRect);
    procedure GetResizePoints(const ABounds: TRect; AMarker: TdxResizeMarker;
      var ALeftTop, ABottomRight: TPoint);
    procedure GetSnappedResizePoints(const ABounds: TRect;
      AMarker: TdxResizeMarker; var ASnappedLeftTop,
      ASnappedBottomRight: TPoint);
    procedure GetMoveDeltas(const ABounds: TRect;
      out ADeltaX, ADeltaY: Integer);
  ...
  strict private
    ...
    function InitializeGuides(const ABounds: TRect;
      const AGuideBounds: TArray<TRect>): Boolean;
  public
    constructor Create(AControl: TControl);
    procedure Reset;
    procedure UpdateByResizing(AController: TdxGuideController;
      const ABounds: TRect; AMarker: TdxResizeMarker);
    procedure UpdateByMoving(AController: TdxGuideController;
      const ABounds: TRect);
...
function TdxGuideController.TGuideList.Find(AValue,
  ATolerance: Integer): TGuide;
var
...
begin
  ...
  else if ADistance <= ATolerance then
    Exit(Items[L]);
end;
...
constructor TdxGuideController.Create(AAddItemsBoundsProc: TAddItemsBoundsProc;
ACapacity, ATolerance, AOverlap: Integer);
var
  I: TGuideKind;
begin
...
begin
  ...
  FGuides[TGuideKind.VerticalCenter].Update(ABounds.CenterPoint.Y, ABounds.Left,
    ABounds.Right);
  FGuides[TGuideKind.Bottom].Update(ABounds.Bottom, ABounds.Left,
    ABounds.Right);

  FGuides[TGuideKind.Left].Update(ABounds.Left, ABounds.Top, ABounds.Bottom);
  FGuides[TGuideKind.HorizontalCenter].Update(ABounds.CenterPoint.X,
    ABounds.Top, ABounds.Bottom);
  FGuides[TGuideKind.Right].Update(ABounds.Right, ABounds.Top, ABounds.Bottom);
  FBounds.Add(ABounds);
end;

function TdxGuideController.GetBoundsValue(const ABounds: TRect;
AKind: TGuideKind): Integer;
begin
  case AKind of
    TGuideKind.Left:
      Result := ABounds.Left;
    TGuideKind.HorizontalCenter:
      Result := ABounds.CenterPoint.X;
    TGuideKind.Right:
      Result := ABounds.Right;
    TGuideKind.Top:
      Result := ABounds.Top;
    TGuideKind.VerticalCenter:
      Result := ABounds.CenterPoint.Y;
  else
    Result := ABounds.Bottom;
  end;
end;

function TdxGuideController.GetDirectionDelta(const ABounds: TRect;
const AKinds: array of TGuideKind): Integer;
var
  I: TGuideKind;
begin
...
function TdxGuideController.GetGuideBounds(const ABounds: TRect;
ATolerance: Integer; AKind: TGuideKind; out AGuideBounds: TRect;
var AKinds: TGuideKinds; var AValues: TGuideValues; AResize: Boolean): Boolean;
var
  AGuide: TGuide;
begin
  ...
  begin
    ...
    begin
      if (AKind = TGuideKind.VerticalCenter) and
        not([TGuideKind.Top, TGuideKind.Bottom] * AKinds = [TGuideKind.Top,
        TGuideKind.Bottom]) then
        Exit(False);
      if (AKind = TGuideKind.HorizontalCenter) and
        not([TGuideKind.Left, TGuideKind.Right] * AKinds = [TGuideKind.Left,
        TGuideKind.Right]) then
        Exit(False);
    end;

    if AKind in [TGuideKind.Top, TGuideKind.VerticalCenter, TGuideKind.Bottom]
    then
      AGuideBounds.Init(Min(ABounds.Left, AGuide.Min) - FOverlap, AGuide.Value,
        Max(ABounds.Right, AGuide.Max) + FOverlap, AGuide.Value)
    else
      AGuideBounds.Init(AGuide.Value, Min(ABounds.Top, AGuide.Min) - FOverlap,
        AGuide.Value, Max(ABounds.Bottom, AGuide.Max) + FOverlap);
...
function TdxGuideController.GetGuides(const ABounds: TRect;
AMarker: TdxResizeMarker): TArray<TRect>;
var
  I: TGuideKind;
  ACount: Integer;
  AGuideBounds: TRect;
...
begin
  ...
  SetLength(Result, 6);
  for I := Low(TGuideKind) to High(TGuideKind) do
    if GetGuideBounds(ABounds, GetResizeTolerance(AMarker, I, True), I,
      AGuideBounds, AKinds, AValues, False) then
    begin
...
procedure TdxGuideController.CheckGuide(const ABounds: TRect; AKind: TGuideKind;
var ADelta: Integer);
var
  AValue, ABoundValue: Integer;
begin
  ABoundValue := GetBoundsValue(ABounds, AKind);
  if FindNearestGuideValue(ABounds, AKind, AValue) and
    (Abs(ABoundValue - AValue) < Abs(ADelta)) then
    ADelta := ABoundValue - AValue;
end;

procedure TdxGuideController.GetMoveDeltas(const ABounds: TRect;
out ADeltaX, ADeltaY: Integer);
begin
  ADeltaX := GetDirectionDelta(ABounds, [TGuideKind.Left, TGuideKind.Right,
    TGuideKind.HorizontalCenter]);
  ADeltaY := GetDirectionDelta(ABounds, [TGuideKind.Top, TGuideKind.Bottom,
    TGuideKind.VerticalCenter]);
end;
...
procedure TdxGuideController.GetResizePoints(const ABounds: TRect;
AMarker: TdxResizeMarker; var ALeftTop, ABottomRight: TPoint);
var
  X, Y: Integer;
begin
  X := ABounds.Left;
...
procedure TdxGuideController.GetSnappedResizePoints(const ABounds: TRect;
AMarker: TdxResizeMarker; var ASnappedLeftTop, ASnappedBottomRight: TPoint);
var
  X, Y: Integer;
begin
  X := ABounds.Left;
  ...
  if FindNearestSnappedY(Y, AMarker, ABounds) then
  begin
    if Abs(ABounds.Bottom - Y) < Abs(ABounds.Bottom - ASnappedBottomRight.Y)
    then
      ASnappedBottomRight.Y := Y;
  end;
end;

procedure TdxGuideController.CheckBoundsHeights(AOriginalY, ADelta: Integer;
var Y: Integer; AMarker: TdxResizeMarker; const ABounds: TRect);
var
...
begin
  case AMarker of
    ...
    TdxResizeMarker.BottomLeft, TdxResizeMarker.Bottom,
...
procedure TdxGuideController.CheckBoundsWidths(AOriginalX, ADelta: Integer;
var X: Integer; AMarker: TdxResizeMarker; const ABounds: TRect);
var
...
begin
  case AMarker of
    ...
    TdxResizeMarker.TopRight, TdxResizeMarker.Right,
      TdxResizeMarker.BottomRight:
      begin
...
function TdxGuideController.FindNearestGuideValue(const ABounds: TRect;
AKind: TGuideKind; out AValue: Integer): Boolean;
var
  AGuide: TGuide;
begin
...
function TdxGuideController.FindNearestSnappedX(var X: Integer;
AMarker: TdxResizeMarker; const ABounds: TRect): Boolean;
var
  AGuide: TGuide;
...
function TdxGuideController.FindNearestSnappedY(var Y: Integer;
AMarker: TdxResizeMarker; const ABounds: TRect): Boolean;
var
  AGuide: TGuide;
...
function TdxGuideController.GetResizeTolerance(AMarker: TdxResizeMarker;
AKind: TGuideKind; AExactMatch: Boolean): Integer;
var
  ATolerance: Integer;
begin
  if AExactMatch then
...
begin
  Result := AKind in [TGuideKind.Top, TGuideKind.Bottom,
    TGuideKind.VerticalCenter];
end;

function TdxGuideController.IsVertical(AKind: TGuideKind): Boolean;
begin
  Result := AKind in [TGuideKind.Left, TGuideKind.Right,
    TGuideKind.HorizontalCenter];
end;

procedure TdxGuideController.PopulateGuides(AAddItemsBoundsProc
  : TAddItemsBoundsProc);
var
  I: TGuideKind;
begin
...
function TdxGuidesViewData.InitializeGuides(const ABounds: TRect;
const AGuideBounds: TArray<TRect>): Boolean;
var
  I: Integer;
begin
...
procedure TdxGuidesViewData.UpdateByMoving(AController: TdxGuideController;
const ABounds: TRect);
begin
...
procedure TdxGuidesViewData.UpdateByResizing(AController: TdxGuideController;
const ABounds: TRect; AMarker: TdxResizeMarker);
begin
  FController := AController;
