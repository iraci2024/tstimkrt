...
type
  ...
  protected
    function FindTargetDockControlInControl(AControl: TdxCustomDockControl)
  ...
  public
    ...
    procedure PrepareSelectionRegion(ARegion: TcxRegion;
      AControl: TdxCustomDockControl; const ARect: TRect); override;
  ...
  protected
    function GetTabInfo(AControl: TdxCustomDockControl;
      out ATabsAreaRect, ATabRect: TRect): TcxTabPosition; virtual; abstract;
  public
    procedure PrepareSelectionRegion(ARegion: TcxRegion;
      AControl: TdxCustomDockControl; const ARect: TRect); override;
  end;
  ...
  protected
    ...
    function GetTabInfo(AControl: TdxCustomDockControl;
      out ATabsAreaRect, ATabRect: TRect): TcxTabPosition; override;
  public
...
var
  R: TRect;
begin
  Result := Rect(0, 0, Min(AControl.OriginalWidth,
    Owner.Width - 2 * SelectionFrameWidth), Owner.Height);
...
var
  AZoneOffset: Integer;
begin
  if (Kind = zkDocking) and (doUseCaptionAreaToClientDocking
    in TdxCustomDockControlAccess(Owner).ControllerOptions) and
  ...
  begin
    ...
    if TdxCustomDockControlAccess(Owner).HasBorder then
      Inc(AZoneOffset, TdxCustomDockControlAccess(Owner)
        .Painter.GetBorderWidths.Left);
  end
  else
    AZoneOffset := 0;
...
function TdxLeftZone.CanConstrainedResize(NewWidth, NewHeight: Integer)
  : Boolean;
begin
  Result := ((Owner.Constraints.MinWidth <= 0) or
...
class function TdxLeftZone.ValidateDockZone(AOwner,
...
class function TdxLeftZone.ValidateResizeZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
begin
  NewWidth := Owner.Width - (StartPoint.X - EndPoint.X);
  NewHeight := Owner.Height;
  Result := (NewWidth > 0) and
    (Owner.Left + NewWidth < Owner.Parent.ClientWidth) and
    DoCanResize(NewWidth, NewHeight);
end;

function TdxRightZone.GetDockingSelection
  (AControl: TdxCustomDockControl): TRect;
var
  R: TRect;
begin
  if Owner.Width - AControl.OriginalWidth - 2 * SelectionFrameWidth > 0 then
    Result := Rect(Owner.Width - AControl.OriginalWidth, 0, Owner.Width,
      Owner.Height)
  else
...
class function TdxRightZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
var
  I: Integer;
  AZone: TdxZone;
begin
  ...
  if Result then
    ...
    begin
      AZone := AControl.DockZones[I];
      if (AZone.DockType = dtRight) and (AZone.Owner.Height = AOwner.Height)
      then
      begin
        Result := False;
        Break;
...
class function TdxRightZone.ValidateResizeZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
function TdxRightZone.CanConstrainedResize(NewWidth,
...
begin
  Result := ((Owner.Constraints.MinHeight <= 0) or
    ...
    (NewHeight < Owner.Constraints.MaxHeight));
end;
...
class function TdxTopZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
var
  I: Integer;
  AZone: TdxZone;
...
class function TdxTopZone.ValidateResizeZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
begin
  NewHeight := Owner.Height - (StartPoint.Y - EndPoint.Y);
  NewWidth := Owner.Width;
  Result := (NewHeight > 0) and
    (Owner.Top + NewHeight < Owner.Parent.ClientHeight) and
    DoCanResize(NewWidth, NewHeight);
end;

function TdxBottomZone.GetDockingSelection
  (AControl: TdxCustomDockControl): TRect;
var
  R: TRect;
begin
  if Owner.Height - AControl.OriginalHeight - 2 * SelectionFrameWidth > 0 then
    Result := Rect(0, Owner.Height - AControl.OriginalHeight, Owner.Width,
      Owner.Height)
  else
...
function TdxBottomZone.CanConstrainedResize(NewWidth,
  NewHeight: Integer): Boolean;
begin
  Result := ((Owner.Constraints.MinHeight <= 0) or
    (NewHeight > Owner.Constraints.MinHeight)) and
...
class function TdxBottomZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
var
  I: Integer;
  AZone: TdxZone;
...
class function TdxBottomZone.ValidateResizeZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
class function TdxClientZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
function TdxInvisibleClientZone.CanDock(AControl: TdxCustomDockControl)
  : Boolean;
var
  ATarget: TdxCustomDockControl;
begin
  ATarget := FindTargetDockControl;
  Result := (ATarget <> nil) and (ATarget.CanDockHost(AControl, dtClient) or
    ATarget.CanDockHost(AControl, dtRight) or ATarget.CanDockHost(AControl,
    dtBottom));
end;
...
begin
  ...
  begin
    ...
    else if ATarget.CanDockHost(AControl, dtRight) then
      AControl.DockTo(ATarget, dtRight, -1)
    else if ATarget.CanDockHost(AControl, dtBottom) then
      AControl.DockTo(ATarget, dtBottom, -1);
  end
end;

function TdxInvisibleClientZone.FindTargetDockControlInControl
  (AControl: TdxCustomDockControl): TdxCustomDockControl;
var
  I: Integer;
begin
  ...
  begin
    ...
    begin
      if AControl.Children[I].DockType <> dtClient then
        Continue;
      if (Result = nil) and (AControl.Children[I].CanDock or
        (AControl.Children[I].ChildCount = 0)) then
        Result := AControl.Children[I]
      else if (Result <> nil) and AControl.Children[I].CanDock then
        Result := AControl.Children[I];
    end;
...
class function TdxInvisibleClientZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
  Result := dtClient in AOwner.AllowDockClients;
...
procedure TdxFloatZone.PrepareSelectionRegion(ARegion: TcxRegion;
...
class function TdxFloatZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
begin
  ...
  NewHeight := Owner.Height;
  Result := (NewWidth > 0) and
    ((Owner.AutoHideHostSite.AutoSize and
    (NewWidth < Owner.AutoHideContainer.Parent.ClientWidth -
    Owner.AutoHideHostSite.Width)) or (not Owner.AutoHideHostSite.AutoSize and
    (NewWidth < Owner.AutoHideHostSite.ClientWidth))) and
    DoCanResize(NewWidth, NewHeight);
end;
...
begin
  ...
  Result := (NewHeight > 0) and
    ((Owner.AutoHideHostSite.AutoSize and
    (NewHeight < Owner.AutoHideContainer.Parent.ClientHeight -
    Owner.AutoHideHostSite.Height)) or (not Owner.AutoHideHostSite.AutoSize and
    (NewHeight < Owner.AutoHideHostSite.ClientHeight))) and
    DoCanResize(NewWidth, NewHeight);
end;
...
function TdxAutoSizeClientZone.GetDockingSelection
  (AControl: TdxCustomDockControl): TRect;
var
  R: TRect;
...
function TdxInvisibleAutoSizeClientZone.GetDockingSelection
  (AControl: TdxCustomDockControl): TRect;
var
  R: TRect;
  AWidth, AHeight: Integer;
...
procedure TdxCustomTabContainerZone.PrepareSelectionRegion(ARegion: TcxRegion;
  AControl: TdxCustomDockControl; const ARect: TRect);
var
...
function TdxTabContainerZone.GetTabInfo(AControl: TdxCustomDockControl;
  out ATabsAreaRect, ATabRect: TRect): TcxTabPosition;
const
  MinWidth = 10;
  ...
  begin
    Result := TdxTabContainerDockSiteAccess(TabContainer).Painter.GetImageWidth
      + TdxTabContainerDockSiteAccess(TabContainer)
      .Canvas.TextWidth(AControl.Caption);
  end;
  ...
  function GetTabRect(AControl: TdxCustomDockControl;
  ...
  begin
    ...
    if (DockIndex < 0) and (AControl = AContainerAccess.GetLastValidChild) then
      Result := AContainerAccess.TabsRects
        [AContainerAccess.GetLastValidChildIndex]
    else if (DockIndex >= 0) and (DockIndex < AContainerAccess.TabRectCount)
    then
    ...
    begin
      ...
      begin
        ...
        else if AContainerAccess.TabsProperties.Rotate then
...
class function TdxTabContainerZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
  Result := (AOwner = AControl) and
    (((AOwner is TdxTabContainerDockSite) and
    ((AOwner as TdxTabContainerDockSite).ActiveChild <> nil)) or
    ((AOwner.TabContainer <> nil) and (AOwner.TabContainer.ActiveChild
    <> nil)));
end;
...
begin
  Result := TdxCustomDockControlAccess(TabContainer)
    .IsCaptionPoint(TabContainer.ScreenToClient(pt));
end;
...
begin
  Result := TdxTabContainerTabZoneClass(ClassType).Create(TabContainer,
    FDockIndex);
end;
...
begin
  Result := TdxTabContainerDockSiteAccess(TabContainer)
    .TabsController.GetTabIndexAtPoint(TabContainer.ScreenToClient(pt))
    = DockIndex;
end;

class function TdxTabContainerTabZone.ValidateDockZone(AOwner,
  AControl: TdxCustomDockControl): Boolean;
begin
...
begin
  ...
  if dxDockingController.DockingDockControl <> nil then
    if not TabContainer.IsValidChild(dxDockingController.DockingDockControl)
    then
      if inherited IsZonePoint(pt) then
      begin
        GetTabInfo(dxDockingController.DockingDockControl, ATabsRect, ATabRect);
        Result := PtInRect(TdxTabContainerDockSiteAccess(TabContainer)
          .WindowRectToClient(ATabRect), TabContainer.ScreenToClient(pt));
      end;
end;
...
begin
  ...
  if Owner.SideContainer <> nil then
    Result := Result and TdxSideContainerDockSiteAccess(Owner.SideContainer)
      .CanChildResize(Owner.SideContainerItem, -(StartPoint.X - EndPoint.X));
end;

procedure TdxHorizContainerZone.DoResize(StartPoint, EndPoint: TPoint);
begin
  if Owner.SideContainer <> nil then
    TdxSideContainerDockSiteAccess(Owner.SideContainer)
      .DoChildResize(Owner.SideContainerItem, -(StartPoint.X - EndPoint.X));
end;
...
begin
  ...
  if Owner.SideContainer <> nil then
    Result := Result and TdxSideContainerDockSiteAccess(Owner.SideContainer)
      .CanChildResize(Owner.SideContainerItem, -(StartPoint.Y - EndPoint.Y));
end;
...
begin
  if Owner.SideContainer <> nil then
    TdxSideContainerDockSiteAccess(Owner.SideContainer)
      .DoChildResize(Owner.SideContainerItem, -(StartPoint.Y - EndPoint.Y));
end;
...
function TdxDockPanelClientZone.GetTabInfo(AControl: TdxCustomDockControl;
  out ATabsAreaRect, ATabRect: TRect): TcxTabPosition;
var
  AProperties: TdxTabContainerDockSiteProperties;
  ATabSize: TSize;
begin
  AProperties := Owner.Controller.DefaultTabContainerSiteProperties
    (Owner.ParentForm);
  ...
  if TdxDockPanelAccess(Owner).ShowSingleTab or
    (Result = TdxDockPanelAccess(Owner).TabsProperties.TabPosition) then
  begin
  ...
  begin
    ...
    case Result of
      ...
      tpBottom:
        begin
          ATabsAreaRect := cxRectSetBottom(ATabsAreaRect, ATabsAreaRect.Bottom,
            ATabSize.cy);
          ATabRect := cxRectSetWidth(ATabsAreaRect, ATabSize.cx);
        end;
      ...
      tpRight:
        begin
          ATabsAreaRect := cxRectSetRight(ATabsAreaRect, ATabsAreaRect.Right,
            ATabSize.cy);
          ATabRect := cxRectSetHeight(ATabsAreaRect, ATabSize.cx);
        end;
    end;
...
begin
  Result := TdxCustomDockControlAccess(Owner)
    .IsCaptionPoint(Owner.ScreenToClient(pt));
end;
