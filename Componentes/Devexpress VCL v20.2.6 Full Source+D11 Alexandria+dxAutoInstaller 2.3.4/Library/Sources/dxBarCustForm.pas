...
uses
  ...
  Buttons, cxClasses, dxBar, ActnList, ToolWin, ImgList, Contnrs,
  dxBarCustomCustomizationForm, dxSkinsCore, cxButtons,
  cxControls, cxContainer, cxEdit, cxLabel, cxCheckBox, cxGroupBox, cxGraphics,
  cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxListBox, cxPC, cxCheckListBox, cxLookAndFeels, cxLookAndFeelPainters,
  dxLayoutContainer, dxLayoutControl,
  dxLayoutControlAdapters, dxLayoutLookAndFeels, dxLayoutcxEditAdapters,
  cxImageList, cxCustomListBox;
...
uses
  ...
  dxBarAddGroupItemsEd, TypInfo, dxUxTheme,
  dxThemeManager, dxThemeConsts, dxOffice11, dxSkinInfo, Math, cxGeometry,
...
type
  ...
  private
    ...
    class function GetPopupMenuLinkSelectedElement(AIsDropDown: Boolean)
      : TdxSkinElement;
    class function GetSideStripElement: TdxSkinElement;
    class function GetSkinElementColor(AElement: TdxSkinElement;
      ATextColor: Boolean): TColor;
  ...
  protected
    class procedure DrawSkinElement(ACanvas: TCanvas; const R: TRect;
      AElement: TdxSkinElement; AIsRightToLeft: Boolean = False);
  ...
  public
    ...
    class procedure DrawCheckBox(ACanvas: TCanvas; R: TRect;
      AChecked, AEnabled: Boolean); override;
    ...
    class procedure DrawEdit(ACanvas: TCanvas; ARect: TRect;
      AItem: TdxCustomBarEdit; Selected, ADrawArrowButton: Boolean); override;
...
procedure TdxBarCustomizationForm.PrepareControls;
  ...
  begin
    liBBarReset.Visible := not BarManager.Designing and
      TdxBarManagerAccess(BarManager).CanReset;
  ...
  begin
    ...
    begin
      ...
      LabelMenuAnimations.Caption :=
        cxGetResourceString(@dxSBAR_MENUANIMATIONS);
    ...
    begin
      ...
      CBMenusShowRecentItemsFirst.Caption :=
        cxGetResourceString(@dxSBAR_MENUSSHOWRECENTITEMS);
      CBShowFullMenusAfterDelay.Caption :=
        cxGetResourceString(@dxSBAR_SHOWFULLMENUSAFTERDELAY);
  ...
  procedure PrepareKeyTipsSheet;
    ...
    begin
      ...
      if AContainerHelper.Selectable then
        AParentNode := ANodes.AddChild(AParentNode,
          AContainerHelper.OwnerObject.ClassName);
  ...
  begin
    ...
    if (ARootAccessibleObject <> nil) and
      ARootAccessibleObject.AreKeyTipsSupported(AKeyTipWindowsManager) then
...
function TdxBarCustomizationForm.GetPainterClass
...
begin
  if not(csDestroying in (Application.ComponentState + BarManager.ComponentState
    + ComponentState)) then
  begin
...
procedure TdxBarCustomizationForm.CBMenusShowRecentItemsFirstClick
  (Sender: TObject);
begin
  CBShowFullMenusAfterDelay.Enabled := CBMenusShowRecentItemsFirst.Checked;
...
procedure TdxBarCustomizationForm.CBShowCommandsWithShortCutClick
  (Sender: TObject);
begin
  SynchronizeListBox(LAllCommands.InnerListBox);
end;

procedure TdxBarCustomizationForm.CBShowFullMenusAfterDelayClick
  (Sender: TObject);
begin
...
begin
  BarManager.MenuAnimations := TdxBarMenuAnimations
...
procedure TdxBarCustomizationForm.lbItemsDrawItem(AControl: TcxListBox;
...
begin
  ...
  begin
    ...
    try
      ...
      TdxBarItemAccess(AItem).DrawCustomizingImage(ABitmap.Canvas,
        ABitmap.ClientRect, AState);
...
class function TdxBarCustomizationFormSkinPainter.BrushColors(Selected: Boolean;
...
begin
  ...
  try
    InflateRect(R, -1, -1);
    GetLookAndFeelPainter.DrawScaledCheckButton(AcxCanvas, R,
      EnableMap[AEnabled], AChecked, ScaleFactor);
  finally
    AcxCanvas.Free;
  end;
end;
...
begin
  ...
  try
    ...
    GetLookAndFeelPainter.DrawScaledEditorButton(AcxCanvas, ARect, cxbkComboBtn,
      ComboBoxStateMap[ASelected], ScaleFactor);
...
class procedure TdxBarCustomizationFormSkinPainter.DrawEdit(ACanvas: TCanvas;
  ARect: TRect; AItem: TdxCustomBarEdit; Selected: Boolean;
  ADrawArrowButton: Boolean);
var
...
begin
  ...
  DrawSkinElement(ACanvas, ASideStripRect, GetSideStripElement,
    UseRightToLeftAlignment(AItem));
  if Selected then
    DrawSkinElement(ACanvas, ARect,
      GetPopupMenuLinkSelectedElement(False), False);
...
class procedure TdxBarCustomizationFormSkinPainter.
...
var
  AIsDropDown: Boolean;
begin
  AIsDropDown := (AItem is TdxBarCustomButton) and
    (TdxBarCustomButton(AItem).ButtonStyle = bsDropDown);
...
class procedure TdxBarCustomizationFormSkinPainter.DrawEditEdge
...
begin
  if Assigned(AElement) then
    AElement.RightToLeftDependentDraw(ACanvas.Handle, R, dxSystemScaleFactor,
      AIsRightToLeft);
end;
...
class function TdxBarCustomizationFormSkinPainter.FontColors
...
begin
  if AElement = nil then
    Result := clDefault
  else if ATextColor then
    Result := AElement.TextColor
  else
    Result := AElement.Color;
end;

class function TdxBarCustomizationFormSkinPainter.GetLookAndFeelPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := dxBarCustomizingForm.LookAndFeelPainter;
end;

class function TdxBarCustomizationFormSkinPainter.GetPopupMenuElement
  : TdxSkinElement;
var
...
class function TdxBarCustomizationFormSkinPainter.
  GetPopupMenuLinkDropButtonElement: TdxSkinElement;
var
  ASkinInfo: TdxSkinInfo;
begin
  if GetSkinInfo(ASkinInfo) then
...
class function TdxBarCustomizationFormSkinPainter.
  GetPopupMenuLinkSelectedElement(AIsDropDown: Boolean): TdxSkinElement;
var
  ASkinInfo: TdxSkinInfo;
begin
  if not GetSkinInfo(ASkinInfo) then
    Result := nil
  else if AIsDropDown then
    Result := ASkinInfo.PopupMenuSplitButton
  else
    Result := ASkinInfo.PopupMenuLinkSelected;
end;

class function TdxBarCustomizationFormSkinPainter.GetSideStripElement
  : TdxSkinElement;
var
  ASkinInfo: TdxSkinInfo;
begin
...
class function TdxBarCustomizationFormSkinPainter.
  GetComboBoxButtonWidth: Integer;
begin
...
begin
  ...
  cxDrawText(ACanvas.Handle, AText, ATextRect, DT_NOCLIP or DT_NOPREFIX or
