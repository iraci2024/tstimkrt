...
type
  ...
  TcxRegExprLexemCode = (relcSymbol, relcSpecial, relcInteger, relcAll, relcId,
    relcNotId, relcDigit, relcNotDigit, relcSpace, relcNotSpace, relcReference,
    relcDateSeparator, relcTimeSeparator);
  ...
  public
    function Check(var AToken: Char; ACaseInsensitive, ACanChangeCase: Boolean)
  ...
  public
    ...
    function Next(var AToken: Char; ACaseInsensitive, ACanChangeCase: Boolean)
      : TcxRegExprStates;
  ...
  public
    constructor Create(AState: TcxRegExprState;
      AQuantifier: TcxRegExprQuantifier = nil);
    destructor Destroy; override;
  ...
  private
    ...
    function CreateLexem(ALine: Integer; AChar: Integer;
      ACode: TcxRegExprLexemCode; AValue: string): TcxLexem;
    ...
    function ParseAlt(AAlt: TcxRegExprParserAlt;
      Global: Boolean = True): Boolean;
    ...
    function ScanInteger(ALine: Integer; AChar: Integer;
      var AToken: Char): Boolean;
  ...
  public
    ...
    property OnSymbolDelete: TcxSymbolDeleteEvent read FOnSymbolDelete
      write FOnSymbolDelete;
    property OnSymbolUpdate: TcxSymbolUpdateEvent read FOnSymbolUpdate
      write FOnSymbolUpdate;
  end;
...
begin
  ...
  begin
    ...
    if FChar > 0 then
      Result := Result + ', ' + cxGetResourceString(@scxRegExprChar) +
        IntToStr(FChar);
    Result := Result + ': ';
  end;
...
function TcxRegExprSymbol.Check(var AToken: Char;
  ACaseInsensitive, ACanChangeCase: Boolean): Boolean;
begin
...
function TcxRegExprSpace.Check(var AToken: Char;
  ACaseInsensitive, ACanChangeCase: Boolean): Boolean;
begin
  if (AToken = ' ') or (AToken = #0) or (AToken = #9) or (AToken = #10) or
    (AToken = #12) or (AToken = #13) then
    Result := not FInverse
  else
    Result := FInverse;
end;
...
function TcxRegExprState.Check(var AToken: Char;
...
function TcxRegExprState.Next(var AToken: Char;
  ACaseInsensitive, ACanChangeCase: Boolean): TcxRegExprStates;
var
  I: Integer;
begin
  ...
  for I := 0 to FStates.Count - 1 do
    Result.Add(FStates[I].Check(AToken, ACaseInsensitive, ACanChangeCase and
      (Result.Count = 0)));
end;
...
function TcxRegExprBlockState.Check(var AToken: Char;
  ACaseInsensitive, ACanChangeCase: Boolean): TcxRegExprStates;
begin
...
begin
  ...
  for I := 0 to Count - 1 do
    Result.Add(State[I].Next(AToken, False, ACanChangeCase and
      (Result.Count = 0)));

  if ACaseInsensitive then
  begin
    for I := 0 to Count - 1 do
      Result.Add(State[I].Next(AToken, True, ACanChangeCase and
        (Result.Count = 0)));
  end;
end;
...
constructor TcxRegExprAutomat.Create(AExpr: TcxRegExprParserAlts;
  AOwner: TcxRegExpr);
begin
  inherited Create;
...
function TcxRegExprAutomat.Next(var AToken: Char;
  ACaseInsensitive, ACanChangeCase: Boolean): Boolean;
var
...
constructor TcxRegExprParserItem.Create(AQuantifier
  : TcxRegExprQuantifier = nil);
begin
  inherited Create;
...
procedure TcxRegExprParserItem.SetQuantifier(AQuantifier: TcxRegExprQuantifier);
begin
  if AQuantifier <> nil then
  begin
    FQuantifier.Free;
...
constructor TcxRegExprParserBlockItem.Create(AQuantifier
  : TcxRegExprQuantifier = nil);
begin
  inherited Create(AQuantifier);
...
procedure TcxRegExprParserAlt.SetFinishConnection(AFinishState
  : TcxRegExprState);
var
  I: Integer;
begin
...
function TcxRegExprParserAlt.GetItem(AIndex: Integer): TcxRegExprParserItem;
begin
...
procedure TcxRegExprParserAlts.SetFinishConnections(AFinishState
  : TcxRegExprState);
var
  I: Integer;
begin
...
begin
  ...
  try
    ...
    FErrors.Add(TcxRegExprError.Create(0, 0,
      cxGetResourceString(@scxRegExprNotAssignedSourceStream)));
  ...
  begin
    FErrors.Add(TcxRegExprError.Create(0, 0,
      cxGetResourceString(@scxRegExprEmptySourceStream)));
    raise EcxRegExprError.Create(FErrors);
  end;
...
function TcxRegExpr.CreateLexem(ALine: Integer; AChar: Integer;
  ACode: TcxRegExprLexemCode; AValue: string): TcxLexem;
begin
  Result.Line := ALine;
  Result.Char := AChar;
...
begin
  Result := (AToken >= '0') and (AToken <= '9') or (AToken >= 'A') and
    (AToken <= 'F') or (AToken >= 'a') and (AToken <= 'f');
end;
...
function TcxRegExpr.ParseAlt(AAlt: TcxRegExprParserAlt;
...
var
  ...
  function CreateParameterQuantifierBlock(AIndex, ACount: Integer)
    : TcxRegExprParserItem;
  begin
    if AIndex < (ACount - 1) then
    begin
      Result := TcxRegExprParserBlockItem.Create
        (TcxRegExprQuestionQuantifier.Create);
  ...
  begin
    ...
    ABlock := TcxRegExprParserBlockItem.Create
      (TcxRegExprSimpleQuantifier.Create);
    ABlock.Alts.AddAlt;
    for I := 0 to A - 1 do
...
begin
  ...
  begin
    if (TcxRegExprLexemCode(ALexem.Code) = relcSpecial) and
      (ALexem.Value[1] = '|') then
    begin
      FErrors.Add(TcxRegExprError.Create(ALexem.Line, ALexem.Char,
  ...
  repeat
    case TcxRegExprLexemCode(ALexem.Code) of
      relcSymbol:
        AddItem(TcxRegExprParserSimpleItem.Create(TcxRegExprSimpleState.Create
          (TcxRegExprSymbol.Create(ALexem.Value[1]))));
      relcSpecial:
        begin
...
begin
  ...
  if (TcxRegExprLexemCode(ALexem.Code) = relcSpecial) and (ALexem.Value[1] = '^')
  then
  begin
  ...
  Result := TcxRegExprParserSimpleItem.Create
    (TcxRegExprSimpleState.Create(Enumeration));
  ...
  repeat
    ...
    if (TcxRegExprLexemCode(ALexem1.Code) = relcSpecial) and
      (ALexem1.Value[1] = '-') then
    begin
      GetLexem(ALexem1);
      if ALexem.Value[1] < ALexem1.Value[1] then
        Enumeration.Add(TcxRegExprSubrange.Create(ALexem.Value[1],
          ALexem1.Value[1]))
      else
      ...
      GetLexem(ALexem);
      while (TcxRegExprLexemCode(ALexem.Code) = relcSpecial) and
        (ALexem.Value[1] = '-') do
      begin
        FErrors.Add(TcxRegExprError.Create(ALexem.Line, ALexem.Char,
          Format(cxGetResourceString(@scxRegExprIllegalSymbol), ['-'])));
  ...
  until (TcxRegExprLexemCode(ALexem.Code) = relcSpecial) and
    (ALexem.Value[1] = ']');
end;
...
function TcxRegExpr.ScanInteger(ALine, AChar: Integer;
  var AToken: Char): Boolean;
var
  AValue: string;
begin
...
function InternalIsTextValid(const AText, AMask: string;
  AIsFull: Boolean): Boolean;
var
  ARegExpr: TcxRegExpr;
  C: Char;
  I: Integer;
