...
begin
  if GridPopupMenu.HitTest.ViewInfo is TcxGridFooterCellViewInfo then
    Result := TcxGridFooterCellViewInfo(GridPopupMenu.HitTest.ViewInfo)
      .Container
  else if GridPopupMenu.HitTest.ViewInfo is TcxGridFooterViewInfo then
...
begin
  ...
  begin
    ...
    ASummaryItem := ASummaryItems.GetDataItem(HitColumn.Index, spFooter,
      True, Kind);
    if ASummaryItem <> nil then
      Params.Add(ASummaryItem);
  end;
end;
...
begin
  ...
  else if MultipleSummaries then
    Result := SummaryItems.GetDataItem(HitColumn.Index, spFooter, True,
      Kind) <> nil
  else
  begin
...
begin
  ...
  Result := not HitColumn.GetProperties.InheritsFrom
...
begin
  for ASummaryKind := Low(ASummaryKind) to High(ASummaryKind) do
    if ASummaryKind <> skNone then
      TcxGridFooterSummaryOperation(AddItem(TcxGridFooterSummaryOperation)).Kind
        := ASummaryKind;
