...
{$IFNDEF NONDB}
  DB, FMTBcd, SqlTimSt,
{$ENDIF}
  Windows, Classes, SysUtils, Math, Graphics, Variants, Generics.Defaults,
  Generics.Collections, ImgList,
  dxCore, dxGdiPlusClasses, cxGraphics, cxClasses, cxGeometry, cxExportStrs,
  dxSpreadSheet, cxEdit,
...
type
  ...
  TcxEnumExportTypes = procedure(const AExportType: Integer;
  ...
  IcxExportProvider = interface
    ...
    procedure SetCellValue(const ACol, ARow: Integer; const AValue: Variant;
      ...
      ADisplayFormatType: TcxValueDisplayFormatType = vdftAuto);
  ...
  TcxExport = class
  protected
    class function GetExportClassByType(AExportType: Integer)
      : TcxExportProviderClass; virtual;
  public
    class function Provider(AExportType: Integer; const AFileName: string)
      : TcxCustomExportProvider;
    class procedure SupportExportTypes(EnumSupportTypes: TcxEnumExportTypes);
    class procedure SupportTypes(EnumFunc: TcxEnumTypes);
    class function RegisterProviderClass(AProviderClass
      : TcxExportProviderClass): Boolean;
  end;
  ...
  public
    ...
    procedure GetPosition(AValue1, AValue2: Integer;
      out AIndex1, AIndex2: Integer);
    procedure GetPositionEx(AValue1, AValue2, AFirstIndex, ALastIndex: Integer;
      out AIndex1, AIndex2: Integer);
  ...
  protected
    ...
    property Dictionary: TDictionary<Integer, Integer> read FDictionary
      write FDictionary;
...
function cxGetDisplayFormat(AEditProperties: TcxCustomEditProperties;
  AField: TField = nil): string;
function cxGetDisplayFormatType(AEditProperties: TcxCustomEditProperties;
  AField: TField = nil): TcxValueDisplayFormatType;
...
uses
  Types, cxFormats, dxCoreGraphics, cxExportProviders, cxTextEdit, cxTimeEdit,
  cxCalendar, StrUtils,
...
begin
  ...
  begin
    Result := cxTimeEditFormats[TcxTimeEditProperties(AEditProperties)
      .TimeFormat, TcxTimeEditProperties(AEditProperties).Use24HourFormat, 0];
  end;
...
function cxGetDisplayFormatType(AEditProperties: TcxCustomEditProperties;
  AField: TField = nil): TcxValueDisplayFormatType;
begin
  if (AEditProperties is TcxTimeEditProperties) or
    (AEditProperties is TcxDateEditProperties) or (AField is TDateTimeField) or
    (AField is TSQLTimeStampField) then
    Result := vdftDateTime
  else if IsRTFSupported(AEditProperties) then
    Result := vdftRTF
  else
    Result := vdftAuto;
end;
...
function IsRTFSupported(AProperties: TcxCustomEditProperties): Boolean;
begin
  Result := IsRTFSupported and ((AProperties is TcxRichEditProperties) or
...
begin
  Result := (AGraphicClass <> nil) and (AGraphicClass.InheritsFrom(TBitmap) or
...
begin
  ...
  for I := 0 to Length(ABits) - 1 do
    ABits[I].rgbReserved := Transparency
      [PtInRect(ADrawRect, Point(I mod W, I div W))];

  Result := TdxPNGImage.Create;
  Result.LoadFromBits(cxRectWidth(AClientRect), cxRectHeight(AClientRect),
    ABits, True);
end;

{ TcxExport }

class function TcxExport.Provider(AExportType: Integer; const AFileName: string)
  : TcxCustomExportProvider;
begin
  Result := GetExportClassByType(AExportType).Create(AFileName);
end;

class procedure TcxExport.SupportExportTypes(EnumSupportTypes
  : TcxEnumExportTypes);
var
  I: Integer;
begin
...
class function TcxExport.RegisterProviderClass(AProviderClass
  : TcxExportProviderClass): Boolean;
var
  I: Integer;
begin
  ...
  begin
    if (AProviderClass.ExportType = RegisteredClasses[I].ExportType) or
      (AProviderClass = RegisteredClasses[I]) then
      Exit;
  end;
...
class function TcxExport.GetExportClassByType(AExportType: Integer)
  : TcxExportProviderClass;
var
  I: Integer;
begin
  for I := 0 to Length(RegisteredClasses) - 1 do
  ...
  raise EcxExportData.CreateFmt(cxGetResourceString(@scxUnsupportedExport),
    [AExportType]);
end;
...
function TcxExportScale.IndexOfEx(AValue, AFirstIndex,
...
procedure TcxExportScale.GetPosition(AValue1, AValue2: Integer;
  out AIndex1, AIndex2: Integer);
begin
...
procedure TcxExportScale.GetPositionEx(AValue1, AValue2, AFirstIndex,
  ALastIndex: Integer; out AIndex1, AIndex2: Integer);
begin
