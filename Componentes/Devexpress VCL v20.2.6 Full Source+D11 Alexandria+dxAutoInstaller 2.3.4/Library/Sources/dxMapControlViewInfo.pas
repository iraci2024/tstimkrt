...
{$ENDIF}
  ...
  cxGraphics, dxGdiPlusClasses, cxLookAndFeelPainters, dxScreenTip,
  dxCustomHint,
...
type
  ...
  protected
    function GeoPointToText(const AGeoPoint: TdxMapControlGeoPoint)
      : string; virtual;
  ...
  protected
    property TrackBarViewInfo: TdxMapControlZoomTrackBarViewInfo
      read GetTrackBarViewInfo;
  end;

  TdxMapControlZoomTrackBarButtonViewInfo = class
  ...
  TdxMapControlZoomTrackBarZoomInButtonViewInfo = class
    (TdxMapControlZoomTrackBarButtonViewInfo)
  protected
    function GetHitTestIndex: Integer; override;
  end;

  TdxMapControlZoomTrackBarZoomOutButtonViewInfo = class
    (TdxMapControlZoomTrackBarButtonViewInfo)
  protected
    function GetHitTestIndex: Integer; override;
  ...
  TdxMapControlZoomTrackBarThumbViewInfo = class
    (TdxMapControlZoomTrackBarPartViewInfo)
  private
    FDrawBounds: TRect;
  protected
  ...
  TdxMapControlZoomTrackBarScaleViewInfo = class
    (TdxMapControlZoomTrackBarPartViewInfo)
  private
    FDrawBounds: TRect;
  protected
  ...
  private
    ...
    FZoomTrackBarZoomInButtonViewInfo
      : TdxMapControlZoomTrackBarZoomInButtonViewInfo;
    FZoomTrackBarZoomOutButtonViewInfo
  ...
  public
    ...
    property ZoomTrackBarScaleViewInfo: TdxMapControlZoomTrackBarScaleViewInfo
      read FZoomTrackBarScaleViewInfo;
    property ZoomTrackBarThumbViewInfo: TdxMapControlZoomTrackBarThumbViewInfo
      read FZoomTrackBarThumbViewInfo;
    property ZoomTrackBarZoomInButtonViewInfo
      : TdxMapControlZoomTrackBarZoomInButtonViewInfo
      read FZoomTrackBarZoomInButtonViewInfo;
    property ZoomTrackBarZoomOutButtonViewInfo
      : TdxMapControlZoomTrackBarZoomOutButtonViewInfo
      read FZoomTrackBarZoomOutButtonViewInfo;
  end;
  ...
  public
    ...
    property CoordinatesViewInfo: TdxMapControlCoordinatesViewInfo
      read FCoordinatesViewInfo;
    property ScaleViewInfo: TdxMapControlScaleViewInfo read FScaleViewInfo;
    property ScrollButtonsViewInfo: TdxMapControlScrollButtonsViewInfo
      read FScrollButtonsViewInfo;
    property ZoomTrackBarViewInfo: TdxMapControlZoomTrackBarViewInfo
      read FZoomTrackBarViewInfo;
  end;
  ...
  public
    ...
    property NavigationPanelViewInfo: TdxMapControlNavigationPanelViewInfo
      read FNavigationPanelViewInfo;
    property Painter: TdxMapControlPainter read FPainter;
  end;
  ...
  public
    ...
    property HitAtNavigationPanel: Boolean index mchtNavigationPanel
      read GetBitState;
    property HitAtScrollButtons: Boolean index mchtScrollButtons
      read GetBitState;
    property HitAtZoomTrackBar: Boolean index mchtZoomTrackBar read GetBitState;
    property HitAtZoomTrackBarThumb: Boolean index mchtZoomTrackBarThumb
      read GetBitState;
    property HitObject: TdxMapControlElementViewInfo read FHitObject
      write FHitObject;
  ...
  protected
    ...
    property HotElement: TdxMapControlElementViewInfo read FHotElement
      write SetHotElement;
    property PressedElement: TdxMapControlElementViewInfo read FPressedElement
  ...
  private
    FMapControl: TcxControl;
    function GetElementColorByState(AState: TdxMapControlElementState)
      : TdxAlphaColor;
    function GetLookAndFeelPainter: TcxCustomLookAndFeelPainter;
    function GetScaleFactor: TdxScaleFactor;
    function GetViewInfo: TdxMapControlViewInfo;
  protected
    procedure DrawCoordinatesInfo(ACanvas: TcxCanvas;
      AViewInfo: TdxMapControlCoordinatesViewInfo); virtual;
...
type
  TdxMapControlGeoCoordinatePatternType = (cptCardinalPoint, cptDegree,
    cptMinute, cptSecond, cptPrecisionDegree, cptPrecisionMinute,
    cptPrecisionSecond);
...
const
  ...
  SMinutePattern = 'M';
  SSecondPattern = 'S';
  dxLongitudeStr: array [Boolean] of TcxResourceStringID = (@sdxMapControlWest,
    @sdxMapControlEast);
  dxLatitudeStr: array [Boolean] of TcxResourceStringID = (@sdxMapControlSouth,
    @sdxMapControlNorth);
...
constructor TdxMapControlUIElementViewInfo.Create
...
var
  ...
  ANorthYeastTextSize, ANorthWestTextSize, ASouthWestTextSize,
    ASouthYeastTextSize: TSize;
  AMaxSizeGeoPoint: TdxMapControlGeoPoint;
begin
  if (FXPattern = '') and (FYPattern = '') then
  begin
    ANorthTextSize := cxTextSize(Font,
      (cxGetResourceString(dxLatitudeStr[True])));
    AWestTextSize := cxTextSize(Font,
      (cxGetResourceString(dxLongitudeStr[False])));
    ASouthTextSize := cxTextSize(Font,
      (cxGetResourceString(dxLatitudeStr[False])));
    AEastTextSize := cxTextSize(Font,
      (cxGetResourceString(dxLongitudeStr[True])));
    ASizeOfDirectionText.cx := Max(AEastTextSize.cx, AWestTextSize.cx) +
      Max(ANorthTextSize.cx, ASouthTextSize.cx);
    ASizeOfDirectionText.cy := Max(Max(AEastTextSize.cy, AWestTextSize.cy),
      Max(ANorthTextSize.cy, ASouthTextSize.cy));
    Result := cxTextSize(Font, '00.0' + SDegreeSymbol + '    000.0' +
      SDegreeSymbol);
  ...
  begin
    ...
    Result.cx := Max(Max(Max(ANorthYeastTextSize.cx, ANorthWestTextSize.cx),
      ASouthWestTextSize.cx), ASouthYeastTextSize.cx);
    Result.cy := Max(Max(Max(ANorthYeastTextSize.cy, ANorthWestTextSize.cy),
      ASouthWestTextSize.cy), ASouthYeastTextSize.cy);
  end;
end;
...
begin
  ...
  APattern := GetMapControl(ViewInfo.MapControl)
    .NavigationPanel.XCoordinateDisplayMask;
  if APattern <> '' then
    FXPattern := ParsePattern(APattern)
  else
    FXPattern := '';
  APattern := GetMapControl(ViewInfo.MapControl)
    .NavigationPanel.YCoordinateDisplayMask;
  if APattern <> '' then
    FYPattern := ParsePattern(APattern)
  else
    FYPattern := '';
  AGeoPoint := GetGeoPointFromPos(GetMapControl(ViewInfo.MapControl)
    .ScreenToClient(GetMouseCursorPos));
  FText := GeoPointToText(AGeoPoint);
end;

procedure TdxMapControlCoordinatesViewInfo.MouseMove(Shift: TShiftState;
  X, Y: Integer);
var
  AGeoPoint: TdxMapControlGeoPoint;
begin
...
function TdxMapControlCoordinatesViewInfo.GeoPointToText(const AGeoPoint
  : TdxMapControlGeoPoint): string;
...
begin
  ...
  begin
    ...
    AXCoordinateText := Format(FXPattern, [ADirectionName, Trunc(ADegree),
      Trunc(AMinutes), Trunc(ASeconds), ADegree, AMinutes, ASeconds]);
  end
  else
    AXCoordinateText := Format('%.1f' + SDegreeSymbol + '%s',
      [ADegree, ADirectionName]);
  ...
  begin
    AMinutes := GetMinutes(ADegree);
    ASeconds := GetSeconds(ADegree);
    AYCoordinateText := Format(FYPattern, [ADirectionName, Trunc(ADegree),
      Trunc(AMinutes), Trunc(ASeconds), ADegree, AMinutes, ASeconds]);
  end
  else
    AYCoordinateText := Format('%.1f' + SDegreeSymbol + '%s',
      [ADegree, ADirectionName]);
...
begin
  Result := GetMapControl(ViewInfo.MapControl)
    .NavigationPanel.Style.CoordinateFont;
end;

function TdxMapControlCoordinatesViewInfo.GetGeoPointFromPos
  (const APoint: TPoint): TdxMapControlGeoPoint;
begin
  Result := GetMapControl(ViewInfo.MapControl).ScreenPointToGeoPoint
    (dxPointDouble(APoint));
end;

function TdxMapControlCoordinatesViewInfo.ParsePattern(const APattern
  : string): string;

  function GetArgumentNumberByType
    (AType: TdxMapControlGeoCoordinatePatternType): Integer;
  begin
    Result := Ord(AType);
  end;

  function HasSubPattern(const APattern: string; AStartSearchPos: Integer;
    out ASubPatternPos: Integer): Boolean;
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if AEndPos <> 0 then
      begin
        ASubPattern := Trim(Copy(APattern, AStartPos + 1,
          AEndPos - AStartPos - 1));
        if SameText(ASubPattern, SCardinalPointPattern) then
          Result := Result + '%' +
            IntToStr(GetArgumentNumberByType(cptCardinalPoint)) + ':s'
        else if CharInSet(ASubPattern[1], [SDegreePattern, SMinutePattern,
          SSecondPattern]) then
        begin
          ...
          begin
            case ASubPattern[1] of
              SDegreePattern:
                APatternType := cptDegree;
              SMinutePattern:
                APatternType := cptMinute
            else // SSecondPattern
              APatternType := cptSecond;
            end;
            Result := Result + '%' +
              IntToStr(GetArgumentNumberByType(APatternType)) + ':d';
          end
          else if (Length(ASubPattern) = 3) and (ASubPattern[2] = ':') and
            CharInSet(ASubPattern[3], ['1' .. '9']) then
          begin
            case ASubPattern[1] of
              SDegreePattern:
                APatternType := cptPrecisionDegree;
              SMinutePattern:
                APatternType := cptPrecisionMinute
            else // SSecondPattern
              APatternType := cptPrecisionSecond;
            end;
            Result := Result + '%' +
              IntToStr(GetArgumentNumberByType(APatternType)) + ':.' +
              ASubPattern[3] + 'f';
          end;
        end;
      end
    ...
    if AEndPos < Length(APattern) then
      Result := Result + Copy(APattern, AEndPos + 1,
        Length(APattern) - AEndPos);
  end;
end;
...
constructor TdxMapControlScrollButtonsViewInfo.Create
  (AViewInfo: TdxMapControlViewInfo);
begin
  inherited;
...
procedure TdxMapControlScrollButtonsViewInfo.MouseDown(Button: TMouseButton;
...
procedure TdxMapControlScrollButtonsViewInfo.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
  if IsCapture then
    UpdateScaleDelta(X, Y);
...
procedure TdxMapControlScrollButtonsViewInfo.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  StopScrollTimer;
end;
...
begin
  FViewInfo.Invalidate(cxRectInflate(Bounds, ScaleFactor.Apply(1),
    ScaleFactor.Apply(1)));
end;
...
procedure TdxMapControlScrollButtonsViewInfo.ScrollTimerHandler
  (Sender: TObject);
begin
  FScrollTimer.Enabled := False;
  ScrollMap;
...
begin
  ...
  begin
    ...
    FKilometersTextBounds.Bottom := FKilometersTextBounds.Top +
      FKilometersTextSize.cy;
    FKilometersTextBounds.Right := FKilometersTextBounds.Left +
      FKilometersLineWidth;
...
begin
  ...
  CalculateScaleLineLength(GetMapControl(ViewInfo.MapControl)
    .GetKilometersScale(GetMapControl(ViewInfo.MapControl).ActualCenterPoint,
    GetMaxScaleLineWidth));
  if IsKilometersVisible then
  begin
  ...
  Result.cx := Max(Max(FMilesTextSize.cx, FKilometersTextSize.cx),
    GetMaxScaleLineWidth);
...
procedure TdxMapControlScaleViewInfo.CalculateScaleLineLength
  (AKilometersScale: Double);
const
  ScaleValuesCount = 23;
  KilometersScaleRanges: array [0 .. ScaleValuesCount - 1] of Double = (10000.0,
    5000.0, 2500.0, 1000.0, 500.0, 250.0, 200.0, 100.0, 50.0, 25.0, 20.0, 10.0,
    5.0, 2.5, 1.0, 0.5, 0.25, 0.2, 0.1, 0.05, 0.025, 0.010, 0.005);
  MilesScaleRanges: array [0 .. ScaleValuesCount - 1] of Double = (10000.0,
    5000.0, 2500.0, 1000.0, 500.0, 250.0, 200.0, 100.0, 50.0, 25.0, 20.0, 10.0,
    5.0, 2.5, 1.0, 0.5, 0.25, 0.2, 0.1, 0.05, 0.025, 0.010, 0.005);
  FootsScaleRanges: array [0 .. 13] of Double = (5000.0, 2500.0, 1000.0, 500.0,
    250.0, 200.0, 100.0, 50.0, 25.0, 20.0, 10.0, 5.0, 2.0, 1.0);
...
begin
  ...
  begin
    ...
    begin
      FKilometersLineWidth := Round(AScaleRange / AKilometersScale *
        GetMaxScaleLineWidth);
  ...
  if AMileScale >= 1 then
    ...
    begin
      ...
      begin
        FMilesLineWidth :=
          Round(AScaleRange / AMileScale * GetMaxScaleLineWidth);
  ...
  begin
    ...
    begin
      ...
      begin
        FMilesLineWidth := Round(AScaleRange / AFootsScale *
          GetMaxScaleLineWidth);
...
function TdxMapControlZoomTrackBarPartViewInfo.GetTrackBarViewInfo
...
function TdxMapControlZoomTrackBarZoomOutButtonViewInfo.GetHitTestIndex
  : Integer;
begin
...
procedure TdxMapControlZoomTrackBarThumbViewInfo.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
  if IsCapture then
...
procedure TdxMapControlZoomTrackBarScaleViewInfo.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  TrackBarViewInfo.SetZoomLevelByThumbPosition(X);
end;
...
constructor TdxMapControlZoomTrackBarViewInfo.Create
  (AViewInfo: TdxMapControlViewInfo);
begin
  inherited Create(AViewInfo);
  FZoomTrackBarScaleViewInfo := TdxMapControlZoomTrackBarScaleViewInfo.Create
    (AViewInfo);
  FZoomTrackBarThumbViewInfo := TdxMapControlZoomTrackBarThumbViewInfo.Create
    (AViewInfo);
  FZoomTrackBarZoomInButtonViewInfo :=
    TdxMapControlZoomTrackBarZoomInButtonViewInfo.Create(AViewInfo);
  FZoomTrackBarZoomOutButtonViewInfo :=
    TdxMapControlZoomTrackBarZoomOutButtonViewInfo.Create(AViewInfo);
end;
...
begin
  ...
  AElementBounds.Right := AElementBounds.Left +
    FZoomTrackBarZoomOutButtonViewInfo.Size.cx;
  FZoomTrackBarZoomOutButtonViewInfo.Bounds := AElementBounds;

  AElementBounds := Bounds;
  AElementBounds.Left := AElementBounds.Right -
    FZoomTrackBarZoomInButtonViewInfo.Size.cx;
  FZoomTrackBarZoomInButtonViewInfo.Bounds := AElementBounds;

  AElementBounds := Bounds;
  AElementBounds.Left := FZoomTrackBarZoomOutButtonViewInfo.Bounds.Right +
    GetZoomButtonOffset;
  AElementBounds.Right := FZoomTrackBarZoomInButtonViewInfo.Bounds.Left -
    GetZoomButtonOffset;
  FZoomTrackBarScaleViewInfo.Bounds := AElementBounds;

  FPixelsPerScaleInterval := FZoomTrackBarScaleViewInfo.Size.cx div
    GetMapControl(ViewInfo.MapControl).GetMaxZoomLevel;
  AElementBounds := Bounds;
  AElementBounds := cxRectCenterVertically(AElementBounds,
    FZoomTrackBarThumbViewInfo.Size.cy);
  AElementBounds.Left := FZoomTrackBarScaleViewInfo.Bounds.Left +
    Round(FPixelsPerScaleInterval * (GetMapControl(ViewInfo.MapControl)
    .ZoomLevel - 1)) + FPixelsPerScaleInterval div 2 -
    FZoomTrackBarThumbViewInfo.Size.cx div 2;
  AElementBounds.Right := AElementBounds.Left +
    FZoomTrackBarThumbViewInfo.Size.cx;
...
begin
  AZoomTrackBarLength := FZoomTrackBarZoomInButtonViewInfo.Size.cx * 2 +
    FZoomTrackBarScaleViewInfo.Size.cx + GetZoomButtonOffset * 2;
  Result := cxSize(AZoomTrackBarLength,
    FZoomTrackBarZoomInButtonViewInfo.Size.cy);
end;
...
procedure TdxMapControlZoomTrackBarViewInfo.SetZoomLevelByThumbPosition
  (APosition: Integer);
var
  ANewZoomLevel: Integer;
begin
  ANewZoomLevel := Round((APosition - ZoomTrackBarScaleViewInfo.Bounds.Left) /
    PixelsPerScaleInterval);
...
begin
  ...
  FScrollButtonsViewInfo := TdxMapControlScrollButtonsViewInfo.Create
    (AViewInfo);
...
begin
  ...
  if ViewInfo.IsCoordinatesVisible then
    ...
    begin
      ...
      AElementBounds.Left := AElementBounds.Right -
        FCoordinatesViewInfo.Size.cx;
      FCoordinatesViewInfo.Bounds := cxRectCenterVertically(AElementBounds,
        FCoordinatesViewInfo.Size.cy);
  ...
  if ViewInfo.IsZoomTrackBarVisible then
    ...
    begin
      AElementBounds := AAvailableBounds;
      AElementBounds.Right := AElementBounds.Left +
        FZoomTrackBarViewInfo.Size.cx;
      FZoomTrackBarViewInfo.Bounds := cxRectCenterVertically(AElementBounds,
        FZoomTrackBarViewInfo.Size.cy);
  ...
  if ViewInfo.IsScrollButtonsVisible then
    ...
    begin
      AElementBounds := cxRectCenterVertically(AAvailableBounds,
        FScrollButtonsViewInfo.Size.cy);
      if ViewInfo.IsElementVisible(FZoomTrackBarViewInfo) then
      begin
        AElementBounds.Left := FZoomTrackBarViewInfo.Bounds.Left;
        AElementBounds.Right := AElementBounds.Left +
          FScrollButtonsViewInfo.Size.cx;
        FZoomTrackBarViewInfo.Bounds :=
          cxRectOffsetHorz(FZoomTrackBarViewInfo.Bounds,
  ...
  if ViewInfo.IsScaleVisible then
    ...
    begin
      ...
      begin
        ...
        FCoordinatesViewInfo.Bounds :=
          cxRectOffsetHorz(FCoordinatesViewInfo.Bounds, -FScaleViewInfo.Size.cx
          - AOffset);
...
procedure TdxMapControlViewInfo.CalculateHitTest
  (AHitTest: TdxMapControlHitTest);
var
  I: Integer;
begin
  for I := FVisibleElements.Count - 1 downto 0 do
    if (FVisibleElements[I] as TdxMapControlElementViewInfo).GetHitTest(AHitTest)
    then
      Break;
end;
...
function TdxMapControlViewInfo.IsElementVisible
  (AElement: TdxMapControlElementViewInfo): Boolean;
var
  I: Integer;
begin
  ...
  if not Result then
    for I := 0 to FVisibleElements.Count - 1 do
    begin
      Result := (FVisibleElements[I] as TdxMapControlElementViewInfo)
...
begin
  Result := GetMapControl(FMapControl).NavigationPanel.ShowKilometersScale and
    IsAnyLayerVisible;
end;

function TdxMapControlViewInfo.IsMilesScaleVisible: Boolean;
begin
  Result := GetMapControl(FMapControl).NavigationPanel.ShowMilesScale and
    IsAnyLayerVisible;
end;
...
begin
  AElementBounds := Bounds;
  AElementBounds.Top := AElementBounds.Bottom -
    FNavigationPanelViewInfo.Size.cy;
...
constructor TdxMapControlHintHelper.Create(AController
...
procedure TdxMapControlController.ElementDestroying
...
procedure TdxMapControlController.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
begin
  ...
  begin
    ...
    if (FHintElement <> nil) and ((FHintElement.GetHint <> '') or
      (FHintElement.GetScreenTip <> nil)) then
    begin
      if AWasVisible then
...
procedure TdxMapControlController.DoMouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TdxMapControlController.DoMouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
  if GetMapControl(FMapControl).DragAndDropState <> ddsNone then
    Exit;
  ...
  else if HotElement <> nil then
    HotElement.MouseMove(Shift, X, Y);
  CheckHint;
end;

procedure TdxMapControlController.DoMouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
procedure TdxMapControlController.CheckHotElement(AShift: TShiftState;
  const APoint: TPoint);
begin
...
procedure TdxMapControlController.SetHotElement
  (AValue: TdxMapControlElementViewInfo);
begin
  if FHotElement <> AValue then
  begin
    if (FHotElement <> nil) and (FHotElement.State <> mcesSelected) then
...
procedure TdxMapControlController.SetPressedElement
  (AValue: TdxMapControlElementViewInfo);
begin
  if FPressedElement <> AValue then
  begin
    if (FPressedElement <> nil) and (FPressedElement.State <> mcesSelected) then
...
begin
  ...
  if (FHintElement <> nil) and ((FHintElement.GetHint <> '') or
    (FHintElement.GetScreenTip <> nil)) then
    FHintHelper.ShowHint(FHintElement.Bounds, FHintElement.Bounds,
      FHintElement.GetHint, False, FHintElement);
end;
...
procedure TdxMapControlPainter.DrawCoordinatesInfo(ACanvas: TcxCanvas;
  AViewInfo: TdxMapControlCoordinatesViewInfo);
var
...
begin
  ...
  if AStyle.CoordinateTextColor = clDefault then
    ACanvas.Font.Color := dxAlphaColorToColor
      (GetElementColorByState(mcesNormal))
  else
    ACanvas.Font.Color := AStyle.CoordinateTextColor;
  ACanvas.DrawText(AViewInfo.Text, AViewInfo.Bounds, DT_CENTER);
end;

procedure TdxMapControlPainter.DrawNavigationPanel(ACanvas: TcxCanvas;
  AViewInfo: TdxMapControlNavigationPanelViewInfo);
var
...
procedure TdxMapControlPainter.DrawScale(ACanvas: TcxCanvas;
  AViewInfo: TdxMapControlScaleViewInfo);
var
...
begin
  ...
  if AViewInfo.IsKilometersVisible then
    ACanvas.DrawText(AViewInfo.MetersText, AViewInfo.KilometersTextBounds,
      DT_CENTER or DT_VCENTER);
  if AViewInfo.IsMilesVisible then
    ACanvas.DrawText(AViewInfo.MilesText, AViewInfo.MilesTextBounds,
      DT_CENTER or DT_VCENTER);
  ...
  AStartGraduationMark.Right := AStartGraduationMark.Left +
    AGraduationMarkThickness;
  ACanvas.FillRect(AStartGraduationMark, AColor);

  if AViewInfo.IsKilometersVisible then
  begin
    AKilometersGraduationMark := cxRectOffsetHorz(AStartGraduationMark,
      AViewInfo.KilometersLineWidth - AGraduationMarkThickness);
...
procedure TdxMapControlPainter.DrawScrollButtons(ACanvas: TcxCanvas;
  AViewInfo: TdxMapControlScrollButtonsViewInfo);
var
...
begin
  ...
  begin
    AArrowBounds := cxRectCenter(AViewInfo.Bounds,
      ViewInfo.ScaleFactor.Apply(20), ViewInfo.ScaleFactor.Apply(20));
    AArrowBounds := cxRectOffsetHorz(AArrowBounds, 1);
    AColor1 := dxAlphaColorToColor(AColor);
    DrawArrow(ACanvas, cxRectOffsetVert(AArrowBounds, -AArrowDistance),
      adUp, AColor1);
    DrawArrow(ACanvas, cxRectOffsetVert(AArrowBounds, AArrowDistance),
      adDown, AColor1);
    DrawArrow(ACanvas, cxRectOffsetHorz(AArrowBounds, -AArrowDistance),
      adLeft, AColor1);
    DrawArrow(ACanvas, cxRectOffsetHorz(AArrowBounds, AArrowDistance),
      adRight, AColor1);
  end;
...
procedure TdxMapControlPainter.DrawSelectedRegion(ACanvas: TcxCanvas;
  AViewInfo: TdxMapControlSelectedRegionViewInfo);
var
  AGpGrahpics: TdxGPGraphics;
begin
  AGpGrahpics := dxGpBeginPaint(ACanvas.Handle, AViewInfo.Bounds);
  try
    AGpGrahpics.Rectangle(AViewInfo.Bounds,
      LookAndFeelPainter.MapControlSelectedRegionBorderColor,
...
begin
  with TdxGPCanvas.Create(ACanvas.Handle) do
    try
      SmoothingMode := smAntiAlias;
      Ellipse(AViewInfo.DrawBounds,
        GetElementColorByState(AViewInfo.State), 0, 2);
    finally
      Free;
    end;
end;
...
var
  ...
  AColor: TdxAlphaColor;
begin
  ABounds := cxRectCenter(AViewInfo.Bounds, ScaleFactor.Apply(13),
    ScaleFactor.Apply(3));
  AColor := GetElementColorByState(AViewInfo.State);
  with TdxGPCanvas.Create(ACanvas.Handle) do
    try
      ...
      begin
        Rectangle(ABounds, AColor, AColor, ScaleFactor.Apply(1));
        ABounds := cxRectCenter(AViewInfo.Bounds, ScaleFactor.Apply(3),
...
function TdxMapControlPainter.GetElementColorByState
  (AState: TdxMapControlElementState): TdxAlphaColor;
var
  AStyle: TdxMapControlNavigationPanelStyle;
begin
  AStyle := GetMapControl(ViewInfo.MapControl).NavigationPanel.Style;
...
function TdxMapControlPainter.GetLookAndFeelPainter
