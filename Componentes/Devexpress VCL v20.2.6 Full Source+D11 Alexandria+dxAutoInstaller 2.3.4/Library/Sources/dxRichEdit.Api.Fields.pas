...
type
  ...
  public
    constructor Create(const ADocument: TdxNativeSubDocument; AField: TdxField);
    procedure Update;
...
begin
  ...
  AStart := TdxDocumentModelPosition.FromRunStart(APieceTable,
    FField.FirstRunIndex);
  AEnd := TdxDocumentModelPosition.FromRunStart(APieceTable,
    FField.LastRunIndex + 1);
...
begin
  if not FIsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedFieldError));
end;
