...
type
  ...
  protected
    ...
    procedure DoDraw(AGraphics: GpGraphics; const R: TRect;
      AAlphaValue: Byte); virtual;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Draw(DC: HDC; const R: TRect; AAlphaValue: Byte = 255;
      AColoration: TColor = clDefault; AIsRightToLeft: Boolean = False);
    procedure DrawColored(DC: HDC; const R: TRect; AColoration: TColor;
      AAlphaValue: Byte = 255; AIsRightToLeft: Boolean = False);
    procedure DrawEx(AGraphics: GpGraphics; const R: TRect;
      AAlphaValue: Byte = 255; AColoration: TColor = clDefault);
    ...
    procedure LoadFromBitmap(ABitmap: GpBitmap;
      const ARect, AFixedPartSize: TRect);
    procedure LoadFromFile(const AFileName: string;
      const AFixedPartSize: TRect); overload;
    procedure LoadFromFile(const AFileName: string;
      const ARect, AFixedPartSize: TRect); overload;
    procedure Scale(M, D: Integer);

    property ID: Integer read FID write FID;
    property InterpolationMode: Integer read FInterpolationMode
      write SetInterpolationMode default InterpolationModeDefault;
    property MinSize: TSize read FMinSize write FMinSize;
    property Name: string read FName write FName;
    property Size: TSize read FSize;
    property StretchMode: TdxSkinRectStretchMode read FStretchMode
      write SetStretchMode;
  ...
  public
    ...
    function AddPart1x3(ABitmap: GpBitmap; const R: TRect;
      ATop, ABottom, AID: Integer; const AName: string = '';
      AInterpolationMode: Integer = InterpolationModeDefault): Integer;
    function AddPart3x3(ABitmap: GpBitmap; const R, AFixedSize: TRect;
      AID: Integer; const AName: string = '';
      AInterpolationMode: Integer = InterpolationModeDefault): Integer;
...
uses
  Types, cxGeometry, dxOffice11, cxControls, dxBar, cxDWMApi, ActiveX,
  dxCoreGraphics;

function dxAlphaBlend(ADestDC: HDC; const ADestRect: TRect; ASrcDC: HDC;
  const ASrcRect: TRect; AlphaValue: Byte = 255): Boolean;
begin
  if IsWin9X then
  begin
    Result := True;
    ...
    Result := SystemAlphaBlend(ADestDC, ASrcDC, ADestRect, ASrcRect,
      AlphaValue);
end;
...
begin
  dxSkinsCalculatePartsBounds(Bounds(0, 0, AWidth, AHeight), FFixedPartSize,
    FPartsBounds);
end;
...
procedure TdxSkinnedRect.DoDraw(AGraphics: GpGraphics; const R: TRect;
  AAlphaValue: Byte);
begin
  dxGpDrawImage(AGraphics, R, cxRect(0, 0, FCalculatedWidth, FCalculatedHeight),
    FCacheBitmap, AAlphaValue);
end;
...
procedure TdxSkinnedRect.DrawColored(DC: HDC; const R: TRect;
AColoration: TColor; AAlphaValue: Byte = 255; AIsRightToLeft: Boolean = False);
begin
  Draw(DC, R, AAlphaValue, AColoration, AIsRightToLeft);
end;

procedure TdxSkinnedRect.DrawEx(AGraphics: GpGraphics; const R: TRect;
AAlphaValue: Byte; AColoration: TColor);
begin
...
begin
  ...
  GdipCheck(GdipDrawImageRectRectI(G, ABitmap, 0, 0, ARect.Right - ARect.Left,
    ARect.Bottom - ARect.Top, ARect.Left, ARect.Top, ARect.Right - ARect.Left,
    ARect.Bottom - ARect.Top, guPixel, nil, nil, nil));
...
begin
  if cxColorIsValid(FCalculatedColoration) then
    ...
    begin
      ...
      if GdipBitmapLockBits(FCacheBitmap, @ARect, ImageLockModeWrite, AFormat,
        @AData) = Ok then
        try
          dxChangeColor(AData.Scan0, AData.Width * AData.Height,
            FCalculatedColoration);
        finally
...
begin
  ...
  dxSkinsCalculatePartsBounds(cxRect(FSize), FFixedPartSize,
    FSourcePartsBounds);
end;
  ...
  try
    ...
    begin
      if StretchMode = srsmStretch then
        dxGpDrawImage(AGraphics, FPartsBounds[APart],
          FSourcePartsBounds[APart], FGPImage)
      else
        dxGpTilePartEx(AGraphics, FPartsBounds[APart],
          FSourcePartsBounds[APart], FGPImage);
    end;
  finally
...
begin
  Result := -1;
  if ASkinnedRect = nil then
    Exit;
  for I := 0 to Count - 1 do
    with Parts[I] do
      if (ASkinnedRect.ID <> -1) and (ASkinnedRect.ID = ID) then
        raise EdxException.CreateFmt('ERROR: Duplicate part''s ID = %d',
          [ASkinnedRect.ID])
      else if (ASkinnedRect.Name <> '') and
        (AnsiSameText(ASkinnedRect.Name, Name)) then
        raise EdxException.CreateFmt('ERROR: Duplicate part''s name = "%s"',
          [ASkinnedRect.Name]);
  Result := inherited Add(ASkinnedRect);
end;

function TdxCustomBarSkin.AddPart1x1(ABitmap: GpBitmap; const R: TRect;
AID: Integer; const AName: string = '';
AInterpolationMode: Integer = InterpolationModeDefault): Integer;
var
  P: TdxSkinnedRect;
...
function TdxCustomBarSkin.AddPart1x3(ABitmap: GpBitmap; const R: TRect;
ATop, ABottom, AID: Integer; const AName: string = '';
AInterpolationMode: Integer = InterpolationModeDefault): Integer;
var
  P: TdxSkinnedRect;
...
function TdxCustomBarSkin.AddPart3x3(ABitmap: GpBitmap;
const R, AFixedSize: TRect; AID: Integer; const AName: string = '';
AInterpolationMode: Integer = InterpolationModeDefault): Integer;
var
  P: TdxSkinnedRect;
begin
