...
{$ENDIF}
  ...
  dxSpellCheckerRules, dxSpellCheckerAlgorithms, dxSpellCheckerAdapters,
  dxSpellCheckerCore, dxCoreClasses;
...
type
  ...
  public
    constructor Create(const AWord: string;
      const APositionStart, APositionFinish: IdxSpellCheckerPosition); virtual;
  ...
  public
    function AddWord(const AWord: string;
      const APositionStart, APositionFinish: IdxSpellCheckerPosition)
      : TdxSpellCheckerSpelledWord;
    procedure DropCheckedFlag;
    function FindWord(const AWord: string; AUncheckedOnly: Boolean;
      out AItem: TdxSpellCheckerSpelledWord): Boolean;
    procedure RemoveItem(AItem: TdxSpellCheckerSpelledWord); virtual;
    procedure RemoveUncheckedItems;

    property Items[Index: Integer]: TdxSpellCheckerSpelledWord
  ...
  public
    constructor Create(const AWord: string;
      const APositionStart, APositionFinish: IdxSpellCheckerPosition); override;
  ...
  protected
    function GetSpellCheckerWordClass: TdxSpellCheckerSpelledWordClass;
      override;
  public
    function AddWord(const AWord: string;
      const APositionStart, APositionFinish: IdxSpellCheckerPosition)
    ...
    function ItemAtIndex(const AIndex: IdxSpellCheckerPosition;
      out AWord: TdxSpellCheckerMisspelledWord): Boolean;
    function ItemAtPos(const APoint: TPoint;
      out AWord: TdxSpellCheckerMisspelledWord): Boolean;

    property Items[Index: Integer]: TdxSpellCheckerMisspelledWord
      read GetItem; default;
  end;
  ...
  public
    ...
    property LineStyle: TdxSpellCheckerUnderlineStyle read FLineStyle
      write FLineStyle;
  end;
  ...
  public
    ...
    property RectsToDraw: TdxSpellCheckerCheckAsYouTypeViewInfoList
      read FRectsToDraw;
    property RectsToErase: TdxSpellCheckerCheckAsYouTypeViewInfoList
      read FRectsToErase;
  ...
  protected
    ...
    procedure UpdateByDictionary(ADictionary
      : TdxCustomSpellCheckerDictionary); override;
    ...
    procedure AddMisspelledWord(const AWord: string;
      AIsCurrentWord: Boolean); overload;
    function GetNextWord(out AWord: string): Boolean; override;
    function InternalCheckWord(var AWord: string)
      : TdxSpellCheckerSpellingCode; override;
  ...
  public
    ...
    property MisspelledWords: TdxSpellCheckerMisspelledWordList
      read FMisspelledWords;
  ...
  public
    ...
    property MisspelledItem: TdxSpellCheckerMisspelledWord read FMisspelledItem
      write SetMisspelledItem;
...
uses
  Math, cxRichEdit, cxRichEditUtils, dxOffice11, cxTextEdit, ComCtrls, RichEdit,
  Dialogs, ComObj, cxGraphics,
  dxSpellCheckerStrs, cxContainer, dxSpellCheckerDialogs, dxCore,
  dxSpellCheckerUtils, dxDrawRichTextUtils;
...
function TdxSpellCheckerSpelledWordList.AddWord(const AWord: string;
  ...
  : TdxSpellCheckerSpelledWord;
begin
  Result := GetSpellCheckerWordClass.Create(AWord, APositionStart,
    APositionFinish);
  Add(Result);
end;
...
function TdxSpellCheckerSpelledWordList.GetItem(AIndex: Integer)
  : TdxSpellCheckerSpelledWord;
begin
  Result := TdxSpellCheckerSpelledWord(inherited Items[AIndex]);
end;

function TdxSpellCheckerSpelledWordList.GetSpellCheckerWordClass
  : TdxSpellCheckerSpelledWordClass;
begin
...
procedure TdxSpellCheckerSpelledWordList.RemoveItem
...
procedure TdxSpellCheckerMisspelledWord.Assign
  (AWord: TdxSpellCheckerSpelledWord);
...
function TdxSpellCheckerMisspelledWordList.AddWord(const AWord: string;
  ...
  : TdxSpellCheckerMisspelledWord;
begin
  Result := TdxSpellCheckerMisspelledWord(inherited AddWord(AWord,
    APositionStart, APositionFinish));
end;
...
begin
  Result := inherited FindWord(AWord, AUncheckedOnly,
    TdxSpellCheckerSpelledWord(AItem));
end;

function TdxSpellCheckerMisspelledWordList.ItemAtIndex
  (const AIndex: IdxSpellCheckerPosition;
  out AWord: TdxSpellCheckerMisspelledWord): Boolean;
var
  I: Integer;
begin
  ...
  begin
    AWord := Items[I];
    if (AIndex.Compare(AWord.PositionStart) >= 0) and
      (AIndex.Compare(AWord.PositionFinish) < 0) then
      Exit(True);
  end;
end;

function TdxSpellCheckerMisspelledWordList.ItemAtPos(const APoint: TPoint;
  out AWord: TdxSpellCheckerMisspelledWord): Boolean;
var
  I: Integer;
begin
...
function TdxSpellCheckerMisspelledWordList.GetSpellCheckerWordClass
  : TdxSpellCheckerSpelledWordClass;
begin
  Result := TdxSpellCheckerMisspelledWord;
end;

function TdxSpellCheckerMisspelledWordList.GetItem(AIndex: Integer)
  : TdxSpellCheckerMisspelledWord;
begin
...
constructor TdxSpellCheckerCheckAsYouTypeMode.Create
  (AOwner: TdxCustomSpellChecker; AAdapter: IdxSpellCheckerAdapter);
begin
  ...
  FManager := TdxSpellCheckerCheckAsYouTypeManager
    (TdxCustomSpellCheckerAccess(AOwner).CheckAsYouTypeManager);
  ...
  FDelayTimer := cxCreateTimer(DelayTimerEvent,
    dxSpellCheckerCheckAsYouTypeTimeDelay div 2, False);
  ...
  try
    ...
    begin
      ...
      AStartSpellingThread :=
        ...
        (ASelectionFinish.Subtract(ASelectionStart)
        .Compare(SelectionFinish.Subtract(SelectionStart)) <> 0)
    end;
  finally
...
procedure TdxSpellCheckerCheckAsYouTypeMode.SpellingDone(ASucceeded: Boolean);
  ...
  procedure ValidateMisspelledWordsCache(const ASpellingPosition
...
begin
  Result := (Manager.ManualCheckCount = 0) and Manager.IsSpellCheckerReady and
    not Adapter.ReadOnly and Adapter.Edit.HandleAllocated;
end;
...
procedure TdxSpellCheckerCheckAsYouTypeMode.AddMisspelledWord
  (const AWord: string);

  function IsCurrentWord: Boolean;
  begin
    Result := (CursorPosition.Compare(FMisspellingStart) >= 0) and
      (CursorPosition.Compare(FMisspellingFinish) <= 0);
  end;
...
procedure TdxSpellCheckerCheckAsYouTypeMode.AddMisspelledWord
  (const AWord: string; AIsCurrentWord: Boolean);
var
  AItem: TdxSpellCheckerMisspelledWord;
...
begin
  ...
  try
    ...
    begin
      AItem := MisspelledWords.AddWord(AWord, FMisspellingStart,
        FMisspellingFinish);
      if AIsCurrentWord and (TypingPosition <> nil) and
        (CursorPosition.Compare(TypingPosition) = 0) then
      begin
        if MisspelledWords.ItemAtIndex(FMisspellingStart, ATempItem) and
          (ATempItem <> AItem) then
    ...
    AItem.Checked := True;
    AItem.ShowSquiggle := AItem.ShowSquiggle or not AIsCurrentWord or
      (SelectionFinish.Compare(SelectionStart) > 0);
  ...
  try
    Adapter.CalculateBounds(ATempItem.PositionStart, ATempItem.PositionFinish,
      ATempItem.AddRect);
    Lock;
    try
...
function TdxSpellCheckerCheckAsYouTypeMode.GetNextWord
...
function TdxSpellCheckerCheckAsYouTypeMode.InternalCheckWord(var AWord: string)
...
function TdxSpellCheckerCheckAsYouTypeMode.IsValidWord
  (const AWord: string): Boolean;
var
  AItem: TdxSpellCheckerSpelledWord;
begin
  Lock;
  try
    Result := not WideSameStr(AWord, PrevWord) and
      CorrectWords.FindWord(AWord, False, AItem);
    if Result then
      AItem.Checked := True
    else
    begin
...
function TdxSpellCheckerCheckAsYouTypeMode.IsWordVisible: Boolean;
  ...
  begin
    Result := (APosition.Compare(VisibleAreaStart) >= 0) and
      (APosition.Compare(VisibleAreaFinish) <= 0);
  end;

begin
  Lock;
  try
    Result := IsCharVisible(FMisspellingStart) or
      IsCharVisible(FMisspellingFinish);
  finally
    Unlock;
  end;
end;
...
function TdxSpellCheckerCheckAsYouTypeMode.GetAdapter
...
procedure TdxSpellCheckerCheckAsYouTypeMode.SyncHelperWndProc
  (var Message: TMessage);
begin
  try
    if Message.Msg <> WM_USER then
      Message.Result := DefWindowProc(FSyncHelper, Message.Msg, Message.wParam,
        Message.lParam)
    else
...
begin
  if GetTickCount - DWORD(DelayTimer.Tag) >= dxSpellCheckerCheckAsYouTypeTimeDelay
  then
  begin
    DelayTimer.Enabled := False;
    StartSpellingThread;
...
constructor TdxSpellCheckerCheckAsYouTypeManager.Create(ASpellChecker
...
function TdxSpellCheckerCheckAsYouTypeManager.CanPopup(const P: TPoint)
  : Boolean;
...
begin
  Result := Active and (CheckMode <> nil) and (Options.PopupMenuItems <> []) and
    ...
    IsQueryPopupAtMisspelledWord;
end;

function TdxSpellCheckerCheckAsYouTypeManager.CreateAdapter
  (AControl: TWinControl; out AAdapter: IdxSpellCheckerAdapter): Boolean;
begin
  Result := inherited CreateAdapter(AControl, AAdapter) and
    Supports(AAdapter, IdxSpellCheckerCheckAsYouTypeAdapter);
end;
...
procedure TdxSpellCheckerCheckAsYouTypeManager.CreateItems(APopup: TPopupMenu;
  AOwner: TComponent);

  procedure AddItem(var APosition: Integer; ATag: Integer;
    const ACaption: string; ASubMenuItem: TMenuItem = nil);
  var
    AItem: TMenuItem;
  begin
  ...
  procedure GenerateAutoCorrectItems(var APosition: Integer;
  ...
  begin
    ...
    AddItem(I, Ord(scmiAutoCorrect),
      cxGetResourceString(@sdxSpellCheckerAutoCorrectOptionsFormCaption),
      ASubMenuItem);
  end;
  ...
  var
    ASuggestionList: TdxSpellCheckerSuggestionList;
  begin
    ASuggestionList := SpellChecker.GetSuggestions
      (TdxSpellCheckerMisspelledWord(MisspelledItem).Word);
    try
      APosition := 0;
      if scmiSuggestions in Options.PopupMenuItems then
      begin
        while (APosition < ASuggestionList.Count) and
          (APosition < Options.SuggestionCount) do
          AddItem(APosition, Ord(High(TdxSpellCheckerPopupMenuItem)) + 1 +
            APosition, ASuggestionList[APosition].Word);
      end;
      if APosition > 0 then
        AddItem(APosition, 0, cLineCaption);
      if scmiIgnore in Options.PopupMenuItems then
        AddItem(APosition, Ord(scmiIgnore),
          cxGetResourceString(@sdxSpellCheckerIgnoreButton));
      if scmiIgnoreAll in Options.PopupMenuItems then
        AddItem(APosition, Ord(scmiIgnoreAll),
          cxGetResourceString(@sdxSpellCheckerIgnoreAllButton));
      if (scmiAddToDictionary in Options.PopupMenuItems) and SpellChecker.HasEnabledUserDictionary
      then
        AddItem(APosition, Ord(scmiAddToDictionary),
          cxGetResourceString(@sdxSpellCheckerAddButton));
      if SpellChecker.AutoCorrectOptions.Active and
        (scmiAutoCorrect in Options.PopupMenuItems) then
  ...
  begin
    ...
    if scmiDelete in Options.PopupMenuItems then
      AddItem(APosition, Ord(scmiDelete),
        cxGetResourceString(@sdxSpellCheckerDeleteButton));
  end;
...
begin
  ...
  begin
    ...
    begin
      AddItem(I, Ord(scmiSpelling),
        cxGetResourceString(@sdxSpellCheckerSpellingFormCaption));
      AddItem(I, 0, cLineCaption);
    end;
  end;
end;
...
function TdxSpellCheckerCheckAsYouTypeManager.GetCheckModeClass
  : TdxSpellCheckerCustomCheckModeClass;
begin
  Result := TdxSpellCheckerCheckAsYouTypeMode;
end;

function TdxSpellCheckerCheckAsYouTypeManager.GetAdapter
  : IdxSpellCheckerCheckAsYouTypeAdapter;
begin
  Result := inherited Adapter as IdxSpellCheckerCheckAsYouTypeAdapter;
end;

function TdxSpellCheckerCheckAsYouTypeManager.GetCheckMode
  : TdxSpellCheckerCheckAsYouTypeMode;
begin
...
procedure TdxSpellCheckerCheckAsYouTypeManager.MenuItemClickHandler
  (ASender: TObject);
var
  ACommand: Integer;
  AIndex: Integer;
begin
  ...
  begin
    ...
    case ACommand of
      Ord(scmiAutoCorrect):
        dxShowAutoCorrectOptionsDialog(SpellChecker,
          SpellChecker.DialogLookAndFeel);
      Ord(scmiIgnore):
        CheckMode.Ignore;
      Ord(scmiIgnoreAll):
        CheckMode.IgnoreAll;
      ...
      if (ACommand > Ord(High(TdxSpellCheckerPopupMenuItem))) or (ACommand < -1)
      then
      begin
        if ACommand < -1 then
          AIndex := -ACommand - 2
...
procedure TdxSpellCheckerCheckAsYouTypeManager.SuggestionClickHandler
  (AIndex: Integer; AIsAutoCorrect: Boolean);
var
  ASuggestionList: TdxSpellCheckerSuggestionList;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        AItem := SpellChecker.AutoCorrectOptions.Replacements.
          FindReplacement(AWord);
        if AItem = nil then
          SpellChecker.AutoCorrectOptions.Replacements.Add(AWord,
            ASuggestionList[AIndex].Word)
        else
        begin
          if MessageDlg
            (Format(cxGetResourceString
            (@sdxSpellCheckerAutoCorrectReplacementExistMessageFormat), [AWord]
...
procedure TdxSpellCheckerCheckAsYouTypeManager.DrawMisspellings
...
procedure TdxSpellCheckerCheckAsYouTypeManager.LayoutChanged
  (AControl: TWinControl);
begin
  ValidateAdapter;
...
function TdxSpellCheckerCheckAsYouTypeManager.QueryPopup(APopup: TComponent;
...
begin
  ...
  begin
    ...
    if (AControl <> nil) and Supports(AControl, IcxCompoundControl, AContainer)
    then
      CheckStart(AContainer.ActiveControl);
  end;
end;

procedure TdxSpellCheckerCheckAsYouTypeManager.Refresh
...
procedure TdxSpellCheckerCheckAsYouTypeManager.SelectionChanged
  (AControl: TWinControl);
begin
  ValidateAdapter;
  if Edit = AControl then
...
procedure TdxSpellCheckerCheckAsYouTypeManager.TextChanged
  (AControl: TWinControl);
begin
  ValidateAdapter;
  if Edit = AControl then
...
procedure TdxSpellCheckerPainter.Draw(DC: HDC; const R: TRect;
  ALineStyleNeeded: Boolean);
begin
