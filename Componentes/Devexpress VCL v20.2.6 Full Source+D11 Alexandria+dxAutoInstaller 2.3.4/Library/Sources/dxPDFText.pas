...
uses
  SysUtils, Types, Generics.Defaults, Generics.Collections, cxGeometry,
  dxGDIPlusClasses, dxPDFBase, dxPDFTypes,
...
type
  ...
  public
    ...
    class function Create(AWordIndex: Integer; AOffset: Integer)
      : TdxPDFPageTextPosition; static;
  ...
  public
    ...
    class function Create(APageIndex, AWordIndex, AOffset: Integer)
      : TdxPDFTextPosition; overload; static;
    class function Create(APageIndex: Integer;
      const APosition: TdxPDFPageTextPosition): TdxPDFTextPosition;
      overload; static;
  ...
  public
    ...
    class function Create(APageIndex: Integer): TdxPDFPageTextRange;
      overload; static;
    class function Create(APageIndex: Integer;
      const AStartPosition, AEndPosition: TdxPDFPageTextPosition)
      : TdxPDFPageTextRange; overload; static;
    class function Create(APageIndex, AStartWordIndex, AStartOffset,
      AEndWordIndex, AEndOffset: Integer): TdxPDFPageTextRange;
      overload; static;
  ...
  public
    class function Create(const AStart, AEnd: TdxPDFTextPosition)
      : TdxPDFTextRange; overload; static;
    class function Create(AStartPageIndex, AStartWordIndex, AStartOffset,
      AEndPageIndex, AEndWordIndex, AEndOffset: Integer): TdxPDFTextRange;
      overload; static;
  end;
  ...
  public
    ...
    class function Create(const ABounds: TdxPDFOrientedRect;
      const AText: string; AFontSize: Single): TdxPDFTextCharacter; static;
    procedure Initialize(const ABounds: TdxPDFOrientedRect; const AText: string;
      AFontSize: Single);
  end;
  ...
  public
    ...
    class function Create(const AName: string; AWeight: Integer;
      AItalic: Boolean; APitchAndFamily: Byte; AUseEmbeddedEncoding: Boolean;
      ARegistrator: TObject; AIsType3Font: Boolean)
      : TdxPDFFontRegistrationData; static;
  end;
  ...
  public
    class function Create(const AData: TBytes;
      APlainTextLength, ACipherTextLength, ANullSegmentLength: Integer)
      : TdxPDFType1FontFileData; static;
    class function IsPfbData(const AData: TBytes): Boolean; static;
    class function Parse(ADictionary: TdxPDFDictionary)
      : TdxPDFType1FontFileData; static;
  ...
  TdxBiDiDirectedSequenceCollection = class(TObjectList<TdxBiDiSequence>)
  // for internal use
  strict private
  ...
  public
    ...
    constructor Create(AFont: TObject; AFirstChar: Integer;
      AWidths: TDoubleDynArray; AMissingWidth: Single);
  ...
  public
    ...
    property AbsoluteHorizontalScaling: Double
      read GetAbsoluteHorizontalScaling;
    property CharacterSpacing: Double read FCharacterSpacing
      write FCharacterSpacing;
    property HorizontalScaling: Double read FHorizontalScaling
      write FHorizontalScaling;
    ...
    property RenderingMode: TdxPDFTextRenderingMode read FRenderingMode
      write FRenderingMode;
    property Rise: Double read FRise write FRise;
    property TextLineMatrix: TdxPDFTransformationMatrix read FTextLineMatrix
      write FTextLineMatrix;
    property TextSpaceMatrix: TdxPDFTransformationMatrix
      read GetTextSpaceMatrix;
    property TextMatrix: TdxPDFTransformationMatrix read FTextMatrix
      write FTextMatrix;
  ...
  public
    ...
    class function RotatePoint(APoint: TdxPointF; AAngle: Single): TdxPointF;
      static; inline;
    class function GetOrientedDistance(AFirst: TdxPointF; ASecond: TdxPointF;
    ...
    class procedure AddRange(const ARange: TdxPDFPageTextRange;
      var ARanges: TdxPDFPageTextRanges); static;
  end;
...
class function TdxPDFPageTextPosition.Create(AWordIndex, AOffset: Integer)
  : TdxPDFPageTextPosition;
begin
...
function TdxPDFPageTextPosition.IsSame(const AValue
  : TdxPDFPageTextPosition): Boolean;
begin
...
class function TdxPDFTextPosition.Create(APageIndex, AWordIndex,
  AOffset: Integer): TdxPDFTextPosition;
begin
  Result.PageIndex := APageIndex;
  Result.Position := TdxPDFPageTextPosition.Create(AWordIndex, AOffset);
end;

class function TdxPDFTextPosition.Create(APageIndex: Integer;
  const APosition: TdxPDFPageTextPosition): TdxPDFTextPosition;
begin
...
class function TdxPDFPageTextRange.Create(APageIndex, AStartWordIndex,
  AStartOffset, AEndWordIndex, AEndOffset: Integer): TdxPDFPageTextRange;
begin
  Result := Create(APageIndex, TdxPDFPageTextPosition.Create(AStartWordIndex,
    AStartOffset), TdxPDFPageTextPosition.Create(AEndWordIndex, AEndOffset));
end;
...
begin
  Result := (V1.PageIndex = V2.PageIndex) and (V1.WholePage = V2.WholePage) and
    V1.StartPosition.IsSame(V2.StartPosition) and
    V1.EndPosition.IsSame(V2.EndPosition);
end;
...
class function TdxPDFTextRangeHelper.Create(const AStart,
  AEnd: TdxPDFTextPosition): TdxPDFTextRange;
begin
  Result.StartPosition := AStart;
  Result.EndPosition := AEnd;
end;

class function TdxPDFTextRangeHelper.Create(AStartPageIndex, AStartWordIndex,
  AStartOffset, AEndPageIndex, AEndWordIndex, AEndOffset: Integer)
  : TdxPDFTextRange;
begin
  Result := Create(TdxPDFTextPosition.Create(AStartPageIndex, AStartWordIndex,
    AStartOffset), TdxPDFTextPosition.Create(AEndPageIndex, AEndWordIndex,
    AEndOffset));
end;

{ TdxPDFTextCharacter }

class function TdxPDFTextCharacter.Create(const ABounds: TdxPDFOrientedRect;
  const AText: string; AFontSize: Single): TdxPDFTextCharacter;
begin
  Result.Initialize(ABounds, AText, AFontSize);
end;

procedure TdxPDFTextCharacter.Initialize(const ABounds: TdxPDFOrientedRect;
  const AText: string; AFontSize: Single);
begin
  Bounds := ABounds;
  Text := AText;
  FontSize := AFontSize;
...
class function TdxPDFFontRegistrationData.Create(const AName: string;
  AWeight: Integer; AItalic: Boolean; APitchAndFamily: Byte;
  AUseEmbeddedEncoding: Boolean; ARegistrator: TObject; AIsType3Font: Boolean)
  : TdxPDFFontRegistrationData;
begin
  Result.Name := AName;
...
function TdxBiDiCharacterClasses.GetCharacterClass(ACharacter: Char)
  : TdxBiDiCharacterClass;
begin
...
constructor TdxBiDiDirectedSequenceCollection.Create
  (ADirection: TdxBiDiTextDirection);
begin
  inherited Create;
...
begin
  ...
  FCurrentDirectedSequence := TdxBiDiDirectedSequenceCollection.Create
    (btdUnknown);
...
procedure TdxBiDiStringBuilder.FinishSequence(ANewSequenceCharacterClass
  : TdxBiDiCharacterClass);
begin
    ...
    case FCurrentBiDiSequence.CharacterClass of
      bccRTL:
        begin
          ...
          else if FCurrentDirectedSequence.Direction <> btdRightToLeft then
          begin
      ...
      bccLTR:
        begin
          ...
          else if FCurrentDirectedSequence.Direction <> btdLeftToRight then
          begin
            FCurrentLine.Add(FCurrentDirectedSequence);
            FreeAndNil(FCurrentDirectedSequence);
            FCurrentDirectedSequence := TdxBiDiDirectedSequenceCollection.Create
...
class function TdxPDFType1FontFileData.Create(const AData: TBytes;
  APlainTextLength, ACipherTextLength, ANullSegmentLength: Integer)
  : TdxPDFType1FontFileData;
...
var
  ...
  AIndex, ANewPlainTextLength, ANewCipherTextLength,
    ANewNullSegmentLength: Integer;
  APatchedData: TBytes;
  ADataString: string;
  C: Char;
begin
  ...
  Result.FData := AData;
  if Length(Result.FData) < APlainTextLength + ACipherTextLength + ANullSegmentLength
  then
    TdxPDFUtils.Abort;
  if IsPfbData(Result.FData) then
  begin
    ANewPlainTextLength := AData[2] + (AData[3] shl 8) + (AData[4] shl 16) +
      (AData[5] shl 24);
    APosition := ANewPlainTextLength + 8;
    ANewCipherTextLength := AData[APosition] + (AData[APosition + 1] shl 8) +
      (AData[APosition + 2] shl 16) + (AData[APosition + 3] shl 24);
    Inc(APosition, ANewCipherTextLength + 6);

    ATextLength := ANewPlainTextLength + ANewCipherTextLength;
    ANewNullSegmentLength := Min(AData[APosition] + (AData[APosition + 1] shl 8)
      + (AData[APosition + 2] shl 16) + (AData[APosition + 3] shl 24),
      Length(AData) - APosition - 4);

    SetLength(APatchedData, ATextLength + ANewNullSegmentLength);
    cxCopyData(@Result.FData[0], @APatchedData[0], 6, 0, ANewPlainTextLength);
    cxCopyData(@Result.FData[0], @APatchedData[0], ANewPlainTextLength + 12,
      ANewPlainTextLength, ANewCipherTextLength);
    cxCopyData(@Result.FData[0], @APatchedData[0], ATextLength + 18,
      ATextLength, ANewNullSegmentLength);
...
class function TdxPDFType1FontFileData.Parse(ADictionary: TdxPDFDictionary)
  : TdxPDFType1FontFileData;
var
...
begin
  ...
  begin
    ...
    if not TdxPDFUtils.IsIntegerValid(ALength1) or
      not TdxPDFUtils.IsIntegerValid(ALength2) or not TdxPDFUtils.IsIntegerValid
      (ALength3) then
      TdxPDFUtils.Abort;
    Result := TdxPDFType1FontFileData.Create(AFontFileStream.UncompressedData,
      ALength1, ALength2, ALength3);
  end
  else
    Result.Reset;
end;
...
constructor TdxPDFFontData.Create(AFont: TObject; AFirstChar: Integer;
  AWidths: TDoubleDynArray; AMissingWidth: Single);
begin
  FFont := AFont;
...
begin
  ...
  begin
    ...
    Exit(TdxPDFFontData.Create(ASourceFont, ASimpleFont.FirstChar,
      ASimpleFont.Widths, AMissingWidth));
  end;
  ...
  Result := TdxPDFFontData.Create(ASourceFont, 0, AType0Font.Widths,
    ADefaultWidth);
end;
...
begin
  Result := TdxPDFTransformationMatrix.Create(FTextMatrix);
  Result.Multiply(TdxPDFTransformationMatrix.Create(AbsoluteFontSize *
    AbsoluteHorizontalScaling, 0, 0, AbsoluteFontSize, 0, FRise));
end;
...
begin
  FTextSpaceMatrix := TdxPDFTransformationMatrix.Create
    (FontSize * HorizontalScaling / 100, 0, 0, FontSize, 0, Rise);
...
const
  ...
  HANGUL_FINISH = #$D7A3;
begin
  Result := (AChar >= CJK_UNIFIED_IDOGRAPHS_START) and
    (AChar <= CJK_UNIFIED_IDOGRAPHS_FINISH) or (AChar >= HIRAGANA_START) and
    (AChar <= HIRAGANA_FINISH) or (AChar >= KATAKANA_START) and
    (AChar <= KATAKANA_FINISH) or (AChar >= HANGUL_START) and
    (AChar <= HANGUL_FINISH);
end;
...
class function TdxPDFBaseTextUtils.HasFlag(AFlags,
  AFontStyle: TdxGPFontStyle): Boolean;
begin
...
class procedure TdxPDFBaseTextUtils.AddRange(const ARange: TdxPDFPageTextRange;
  var ARanges: TdxPDFPageTextRanges);
var
  L: Integer;
begin
  L := Length(ARanges);
...
class function TdxPDFBaseTextUtils.RotatePoint(APoint: TdxPointF;
  AAngle: Single): TdxPointF;
begin
  Result := TdxPDFUtils.RotatePoint(APoint, AAngle);
end;

class function TdxPDFBaseTextUtils.GetOrientedDistance(AFirst: TdxPointF;
  ASecond: TdxPointF; AAngle: Single): Single;
begin
  Result := TdxPDFUtils.OrientedDistance(AFirst, ASecond, AAngle);
end;

class function TdxPDFBaseTextUtils.EndsWith(const S, AEndsWith: string)
  : Boolean;
begin
  Result := TdxPDFUtils.EndsWith(S, AEndsWith);
end;

class function TdxPDFBaseTextUtils.StartsWith(const S,
  AStartsWith: string): Boolean;
begin
