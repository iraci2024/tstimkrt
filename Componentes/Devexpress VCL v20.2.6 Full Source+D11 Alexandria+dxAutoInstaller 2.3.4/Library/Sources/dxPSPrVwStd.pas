...
uses
  ...
  ComCtrls,
  StdCtrls, ExtCtrls, ToolWin, Menus, ImgList, IniFiles, dxCore, dxPSCore,
  dxExtCtrls, dxPrevw,
  dxPSPrvw, cxControls, cxContainer, cxEdit, cxTextEdit, cxMaskEdit, cxSpinEdit,
  cxListBox, cxGraphics, cxDropDownEdit,
  cxLookAndFeels, cxLookAndFeelPainters, dxPSForm, cxGroupBox, dxThemeManager,
  cxImageList, dxForms;
...
type
  TdxfmStdPreview = class(TdxForm, IdxPSPreviewWindowDialog)
    ...
    procedure cbxPredefinedZoomKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  ...
  protected
    ...
    property ExplorerContextCommandMenuItemCount: Integer
      read GetExplorerContextCommandMenuItemCount;
    property ExplorerContextCommandMenuItems[Index: Integer]: TMenuItem
      read GetExplorerContextCommandMenuItem;
    property ExplorerContextCommandPopupMenuItemCount: Integer
      read GetExplorerContextCommandPopupMenuItemCount;
    property ExplorerContextCommandPopupMenuItems[Index: Integer]: TMenuItem
      read GetExplorerContextCommandPopupMenuItem;
    property ExplorerContextCommandToolButtonCount: Integer
      read GetExplorerContextCommandToolButtonCount;
    property ExplorerContextCommandToolButtons[Index: Integer]: TToolButton
...
uses
  Registry, CommCtrl, Math, Themes, UxTheme, cxClasses, dxCustomPreview,
  dxPSGlbl, dxPSEngn, dxPgsDlg, dxPSRes,
...
const
  ...
  ZoomItemCount = 13;
  ZoomItemIndexes: array [0 .. ZoomItemCount - 1] of Integer = (-1, -1, -1, 5,
    -1, -1, -1, -1, 6, 7, 8, 9, 11);
...
begin
  Command := TCustomdxPSExplorerContextCommand
    (TTagToObj(TMenuItem(Sender).Tag));
...
begin
  ...
  miExplorerCreateNewFolder.Caption :=
  ...
  miExplorerProperties.Caption := cxGetResourceString
  ...
  miViewLargeTBtns.Caption := cxGetResourceString
    (@sdxMenuViewLargeToolBarButtons);
  miViewFlatTBtns.Caption := cxGetResourceString
    (@sdxMenuViewFlatToolBarButtons);
  ...
  miZoomWidenToSourceWidth.Caption :=
  ...
  pmiReportShrinkToPageWidth.Caption :=
    cxGetResourceString(@sdxMenuFormatShrinkToPage);
  ...
  pmiZoomWidenToSourceWidth.Caption :=
    cxGetResourceString(@sdxMenuZoomWidenToSourceWidth);
  ...
  pmiExplorerCreateFolder.Caption :=
    cxGetResourceString(@sdxMenuExplorerCreateFolder);
  ...
  pmiExplorerProperties.Caption :=
    cxGetResourceString(@sdxMenuExplorerProperties);
  ...
  { toolbar hints }
  SetHint(tbReportDesigner, cxGetResourceString(@sdxHintFileDesign),
    miFileDesign);
  ...
  SetHint(tbPageSetup, cxGetResourceString(@sdxHintFilePageSetup),
    miFilePageSetup);
  ...
  SetHint(tbViewExplorer, cxGetResourceString(@sdxHintViewExplorer),
    miViewExplorer);
  SetHint(tbViewThumbnails, cxGetResourceString(@sdxHintViewThumbnails),
    miViewThumbnails);

  SetHint(cbxPredefinedZoom, cxGetResourceString(@sdxHintViewZoom));
  SetHint(tbPercent100, cxGetResourceString(@sdxHintZoomPercent100),
    miZoomPercent100);
  SetHint(tbPageWidth, cxGetResourceString(@sdxHintZoomPageWidth),
    miZoomPageWidth);
  SetHint(tbOnePage, cxGetResourceString(@sdxHintZoomWholePage),
    miZoomWholePage);
  SetHint(tbTwoPage, cxGetResourceString(@sdxHintZoomTwoPages), miZoomTwoPages);
  SetHint(tbFourPage, cxGetResourceString(@sdxHintZoomFourPages),
    miZoomFourPages);
  SetHint(tbMultiplePages, cxGetResourceString(@sdxHintZoomMultiplyPages));
  SetHint(tbWidenToSourceWidth,
    cxGetResourceString(@sdxHintZoomWidenToSourceWidth),
    miZoomWidenToSourceWidth);

  SetHint(tbGotoFirstPage, cxGetResourceString(@sdxHintGotoPageFirst),
    pmiGoToFirstPage);
  SetHint(tbGotoPrevPage, cxGetResourceString(@sdxHintGotoPagePrev),
    pmiGoToPrevPage);
  SetHint(tbGotoNextPage, cxGetResourceString(@sdxHintGotoPageNext),
    pmiGoToNextPage);
  SetHint(tbGotoLastPage, cxGetResourceString(@sdxHintGotoPageLast),
    pmiGoToLastPage);
...
function TdxfmStdPreview.CreateLargeImages(ASource: TCustomImageList)
  : TImageList;

  procedure DrawItem(ABitmap: TBitmap; AnImageIndex: Integer;
    const ASourceBounds, ADestBounds: TRect);
  var
    B: TBitmap;
  begin
    B := TBitmap.Create;
...
function TdxfmStdPreview.GetExplorerContextCommandMenuItem(Index: Integer)
  : TMenuItem;
begin
...
function TdxfmStdPreview.GetExplorerContextCommandPopupMenuItem(Index: Integer)
  : TMenuItem;
begin
...
function TdxfmStdPreview.GetExplorerContextCommandToolButton(Index: Integer)
  : TToolButton;
begin
...
begin
  ...
  if ToolBarImageList = nil then
    ...
    cbxPredefinedZoom.Properties.ItemHeight := ScaleFactor.Apply(2) +
      dxGetImageSize(ToolBarImageList, ScaleFactor).cy;
end;
...
const
  LookAndFeelKindMap: array [Boolean] of TcxLookAndFeelKind = (lfStandard,
    lfUltraFlat);
begin
...
begin
  if not Locked then
    Preview.DoShowPageMargins
      (not(pvoPageMargins in Preview.Options.VisibleOptions));
end;
...
procedure TdxfmStdPreview.LoadProperties(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxfmStdPreview.SaveProperties(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
...
procedure TdxfmStdPreview.DoShowExplorerPopup(Sender: TObject;
...
begin
  ...
  pmiFilePrintStyles.Visible := Preview.CanPrintStyle and
    Preview.IsVisible(pvoPageSetup);
...
begin
  if not Locked then
    Preview.Options.ThumbnailsOptions.Visible :=
      not Preview.Options.ThumbnailsOptions.Visible;
end;
...
begin
  ...
  pmiExplorerCreateFolder.Enabled := Preview.IsExplorerAvailable and
    Preview.ExplorerTree.CanCreateFolder;
  pmiExplorerDelete.Enabled := Preview.IsExplorerAvailable and
    Preview.ExplorerTree.CanDeleteSelection;
  pmiExplorerRename.Enabled := Preview.IsExplorerAvailable and
    Preview.ExplorerTree.CanRenameSelectedItem;
  pmiExplorerProperties.Enabled := Preview.IsExplorerAvailable and
...
begin
  ...
  begin
    ...
    cxDrawImage(ACanvas, cxRectInflate(AImageRect, -ScaleFactor.Apply(2)), nil,
      ToolBarImageList, ZoomItemIndexes[AIndex], True, nil, ScaleFactor);
    ATextRect.Left := AImageRect.Right + ScaleFactor.Apply(5);
  end;
  ACanvas.Brush.Style := bsClear;
  ACanvas.DrawTexT(AControl.Properties.Items[AIndex], ATextRect,
    cxAlignVCenter or cxAlignLeft);
end;
...
procedure TdxfmStdPreview.PreviewAddExplorerCommand(Sender: TObject;
  ACommand: TCustomdxPSExplorerContextCommand);

  function IsCommandSeparator(ACommand
    : TCustomdxPSExplorerContextCommand): Boolean;
  begin
    Result := ACommand is TdxPSExplorerContextCommandSeparator;
  end;

  function AddExplorerContextCommandMenuItem(AParent: TMenuItem;
    ACommand: TCustomdxPSExplorerContextCommand): TMenuItem;
  begin
    ...
    Result.ImageIndex := ilToolBarSmall.AddMasked(ACommand.Bitmap,
      ACommand.Bitmap.TransparentColor);
...
begin
  ...
  if not IsCommandSeparator(ACommand) and
    (FExplorerContextCommandPopupMenuItems.IndexOf(MenuItem) = -1) then
    FExplorerContextCommandPopupMenuItems.Add(MenuItem);

  MenuItem := AddExplorerContextCommandMenuItem(miExplorer, ACommand);
  if not IsCommandSeparator(ACommand) and
    (FExplorerContextCommandMenuItems.IndexOf(MenuItem) = -1) then
...
begin
  ...
  if Preview.IsExplorerAvailable then
    TControlAccess(Preview.ExplorerTree.Control).OnMouseUp :=
      DoShowExplorerPopup;
...
procedure TdxfmStdPreview.PreviewLoadProperties(Sender: TObject;
  AIniFile: TCustomIniFile; const ASectionName: string);
begin
...
begin
  ...
  begin
    ...
    try
      ...
      tbPageBackground.Enabled := Preview.IsEnabled(peoPageBackground) and
      ...
      tbGotoFirstPage.Enabled := HasPages and
        (TdxPreviewAccess(Preview.Preview).SelPageIndex <> 0);
      tbGotoPrevPage.Enabled := HasPages and
        (TdxPreviewAccess(Preview.Preview).SelPageIndex <> 0);
      tbGotoNextPage.Enabled := HasPages and
        (TdxPreviewAccess(Preview.Preview).SelPageIndex <>
        Preview.PageCount - 1);
      tbGotoLastPage.Enabled := HasPages and
        (TdxPreviewAccess(Preview.Preview).SelPageIndex <>
        Preview.PageCount - 1);
      ...
      miExplorerCreateNewFolder.Enabled := Preview.IsExplorerAvailable and
        Preview.ExplorerTree.CanCreateFolder;
      miExplorerDelete.Enabled := Preview.IsExplorerAvailable and
        Preview.ExplorerTree.CanDeleteSelection;
      if miExplorerDelete.Enabled then
        miExplorerDelete.ShortCut := ShortCut(VK_DELETE, [])
      else
        miExplorerDelete.ShortCut := 0;
      miExplorerRename.Enabled := Preview.IsExplorerAvailable and
        Preview.ExplorerTree.CanRenameSelectedItem;
      miExplorerProperties.Enabled := Preview.IsExplorerAvailable and
        Preview.ExplorerTree.CanShowPropertySheetsForSelectedItem;
      ...
      miViewPageHeaders.Enabled := HasPages and
        not(Preview.IsBuilding or Preview.IsPrinting);
      miViewPageFooters.Enabled := HasPages and
        not(Preview.IsBuilding or Preview.IsPrinting);
      if ReportLink <> nil then
      begin
      ...
      begin
        ...
        tbPrintDialog.Visible := Preview.IsVisible(pvoPrint) and
          Preview.IsVisible(pvoPrintDialog);
        tbPageSetup.Visible := Preview.IsVisible(pvoPageSetup);
        tbSeparator2.Visible := Preview.IsVisible(pvoPrint) or
          Preview.IsVisible(pvoPageSetup);
      ...
      tbPrint.Hint := cxGetResourceString(@sdxHintFilePrint) +
      ...
      miFilePrintStyles.Visible := miFilePageSetup.Visible and
        Preview.CanPrintStyle;
      miFormatPageBackground.Visible := tbPageBackground.Visible;
      miLine3.Visible := (miFilePrint.Visible or miFilePageSetup.Visible or
        miFilePrintStyles.Visible);
      miLine13.Visible := miFormatPageBackground.Visible;

      miFormatAutoText.Visible := (ReportLink <> nil) and
        (ReportLink.StyleManager <> nil) and
        (rlcHeaderFooter in ReportLink.Capabilities);
      miFormatPageNumbering.Visible := (ReportLink <> nil) and
        (rlcHeaderFooter in ReportLink.Capabilities);
      miFormatDateTime.Visible := (ReportLink <> nil) and
        (rlcHeaderFooter in ReportLink.Capabilities);
...
procedure TdxfmStdPreview.FormCloseQuery(Sender: TObject;
...
class function TdxPSStandardPreviewDialogStyleInfo.CreatePreviewWindow
