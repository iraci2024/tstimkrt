...
uses
  Types, SyncObjs, Classes, Messages, Generics.Defaults, Generics.Collections,
  dxCoreClasses,
...
type
  ...
  public
    procedure UpdateUI(ASender: TObject; const AMethod: TdxAction); overload;
      virtual; abstract;
    procedure UpdateUI(const AUpdate: TdxOwnedProcedure); overload;
      virtual; abstract;
  end;
  ...
  public
    procedure UpdateUI(ASender: TObject; const AMethod: TdxAction);
      overload; override;
...
procedure TdxBeginInvokeBackgroundThreadUIUpdater.UpdateUI(ASender: TObject;
  const AMethod: TdxAction);
begin
  TdxUIThreadSyncService.EnqueueInvokeInUIThread(ASender, AMethod);
end;

procedure TdxBeginInvokeBackgroundThreadUIUpdater.UpdateUI
  (const AUpdate: TdxOwnedProcedure);
begin
  TdxUIThreadSyncService.EnqueueInvokeInUIThread(AUpdate.Owner,
    AUpdate.ProcedureRef);
end;
...
procedure TdxDeferredBackgroundThreadUIUpdater.UpdateUI(ASender: TObject;
  const AMethod: TdxAction);
var
