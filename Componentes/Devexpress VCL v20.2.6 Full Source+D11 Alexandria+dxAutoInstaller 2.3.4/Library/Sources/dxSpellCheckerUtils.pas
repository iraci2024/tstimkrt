...
type
  TdxCapitalizationType = (ctLower, ctCapitalized, ctUpper,
  ...
  public
    ...
    property Items[AIndex: Integer]: TdxSpellCheckerCodePage
...
function CreateAlphabetFromAnsiString(ACodePage: Cardinal;
  const AnsiAlphabet: AnsiString; ACaseSensitive: Boolean): string;
function CreateDefaultAlphabet(ACodePage: Cardinal;
  ACaseSensitive: Boolean = False): string;
...
function GetWordCapitalizationType(const S: string)
  : TdxCapitalizationType; overload;
function GetWordCapitalizationType(S: PWideChar; ALength: Integer;
  ALangID: LCID): TdxCapitalizationType; overload;
...
function WideCharInSet(C: WideChar; const ACharSet: TdxAnsiCharSet)
  : Boolean; inline;
...
function StreamToAnsiString(AStream: TStream; APos: Integer; ASize: Integer = 0)
  : AnsiString;
function StreamToString(AStream: TStream; ACodePage: Cardinal; APos: Integer;
  ASize: Integer = 0): string;

function dxGetCodePageName(ACodePage: Cardinal): string;
procedure dxPopulateCodePages(AList: TStringList;
  AOnlyInstalled: Boolean = True);
...
const
  DefaultAnsiAlphabet
    : AnsiString = '&''-ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz' +
  ...
  TdxHunspellCodePages: array [0 .. 17] of TdxHunspellCodePage =
    ((Name: 'UTF-8'; CodePage: 65001; Description: 'UTF-8'), (Name: 'ISO8859-1';
    CodePage: 28591; Description: 'ISO 8859-1 Latin I'), (Name: 'ISO8859-2';
    CodePage: 28592; Description: 'ISO 8859-2 Central Europe'),
    ...
    (Name: 'ISO8859-8'; CodePage: 28598;
    Description: 'ISO 8859-8 Hebrew: Visual Ordering'), (Name: 'ISO8859-9';
    CodePage: 28599; Description: 'ISO 8859-9 Latin 5'), (Name: 'ISO8859-10';
    CodePage: 28600; Description: 'ISO 8859-10 Latin 6'), (Name: 'ISO8859-13';
    CodePage: 28603; Description: 'ISO 8859-13 Latin 7'), (Name: 'ISO8859-14';
    CodePage: 28604; Description: 'ISO 8859-14 Latin 8'), (Name: 'ISO8859-15';
    CodePage: 28605; Description: 'ISO 8859-15 Latin 9'), (Name: 'KOI8-R';
    CodePage: 20866; Description: 'Russian - KOI8'), (Name: 'KOI8-U';
    CodePage: 21866; Description: 'Ukrainian - KOI8-U'),
    (Name: 'microsoft-cp1251'; CodePage: 1251; Description: 'ANSI - Cyrillic'),
    (Name: 'ISCII-DEVANAGARI'; CodePage: 57002;
...
function GetCPInfoEx(CodePage: Cardinal; dwFlags: DWORD;
  var AInfoEx: TcxCPInfoEx): BOOL; stdcall;
...
begin
  ...
  for I := Low(TdxHunspellCodePages) to High(TdxHunspellCodePages) do
    if dxStrComp(PAnsiChar(TdxHunspellCodePages[I].Name), PAnsiChar(AName)) = 0
    then
    begin
...
begin
  Result := CP_ACP;
  for I := Low(TdxHunspellCodePages) to High(TdxHunspellCodePages) do
    if StrComp(PWideChar(dxAnsiStringToString(TdxHunspellCodePages[I].Name)),
      AName) = 0 then
    begin
      Result := TdxHunspellCodePages[I].CodePage;
      Break;
...
function GetWordCapitalizationType(S: PWideChar; ALength: Integer;
  ALangID: LCID): TdxCapitalizationType;
const
...
begin
  ...
  begin
    ...
    begin
      if (ACharTypes[I] and C1_ALPHA > 0) and not IsSpecialAlpha((S + I)^,
        ALangID) then
      begin
        Inc(AlphaCount);
    ...
    begin
      if (AlphaCount = AUpperCount) and
        not((AlphaCount = 1) and (Result = ctCapitalized)) then
        Result := ctUpper
...
function IsUrl(const S: string): Boolean;
const
  Urls: array [0 .. 8] of string = ('http://', 'https://', 'ftp://', 'www.',
    'file://', '\\', 'gopher://', 'nntp://', 'news://');
var
  ALower: string;
  I: Integer;
begin
...
begin
  LCMapStringW(ALangID, LCMAP_LOWERCASE + LCMAP_LINGUISTIC_CASING, @Ch, 1,
    @Result, 1);
end;

function WideUpCase(Ch: WideChar; ALangID: LCID): WideChar;
begin
  LCMapStringW(ALangID, LCMAP_UPPERCASE + LCMAP_LINGUISTIC_CASING, @Ch, 1,
    @Result, 1);
end;
...
function StreamToString(AStream: TStream; ACodePage: Cardinal; APos: Integer;
  ASize: Integer = 0): string;
begin
  Result := dxAnsiStringToString(StreamToAnsiString(AStream, APos, ASize),
    ACodePage);
end;
...
function TdxSpellCheckerStrings.AddRaw(AData: Pointer; ALength: Cardinal)
  : PWideChar;
var
  ASize: Cardinal;
begin
...
function TdxSpellCheckerCodePages.CallbackProc(ACodePage: PWideChar)
  : Cardinal; stdcall;
begin
...
function TdxSpellCheckerCodePages.GetItem(AIndex: Integer)
  : TdxSpellCheckerCodePage;
begin
