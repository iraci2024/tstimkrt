...
type
  ...
  public
    ...
    class procedure ThrowArgumentException(const AName: string;
      AValue: Integer); overload; static;
    class procedure ThrowArgumentException(const AName: string;
      const AValue: string); overload; static;
    class procedure ThrowArgumentException(const AName: string;
      AValue: TObject); overload; static;
    class procedure ThrowArgumentException(const AName: string;
      const AValue: TSize); overload; static;
    class procedure ThrowUnsupportedFormatException; static;
    class procedure ThrowInvalidOperationException(const AName: string);
      overload; static;
  end;
  ...
  TdxRichEditInvalidFormatExceptionEvent = procedure(Sender: TObject;
    E: TdxRichEditInvalidFormatExceptionEventArgs) of object;
  TdxRichEditInvalidFormatExceptionEventHandler =
    TdxMulticastMethod<TdxRichEditInvalidFormatExceptionEvent>;
  ...
  TdxRichEditUnhandledExceptionEvent = procedure(Sender: TObject;
    E: TdxRichEditUnhandledExceptionEventArgs) of object;
  TdxRichEditUnhandledExceptionEventHandler =
    TdxMulticastMethod<TdxRichEditUnhandledExceptionEvent>;
...
class procedure TdxRichEditExceptions.ThrowArgumentException
  (const AName: string; AValue: Integer);
begin
  ThrowArgumentException(AName, IntToStr(AValue));
end;

class procedure TdxRichEditExceptions.ThrowArgumentException(const AName,
  AValue: string);
begin
  raise EdxRichEditException.CreateFmt
    (cxGetResourceString(@sdxRichEditExceptionIsNotValid), [AValue, AName]);
end;

class procedure TdxRichEditExceptions.ThrowArgumentException
  (const AName: string; AValue: TObject);
var
  AValueStr: string;
begin
  if AValue <> nil then
...
class procedure TdxRichEditExceptions.ThrowArgumentException
  (const AName: string; const AValue: TSize);
begin
  ThrowArgumentException(AName, Format('(cx: %d, cy: %d)',
    [AValue.cx, AValue.cy]));
end;

class procedure TdxRichEditExceptions.ThrowInternalException;
begin
  raise EdxRichEditException.Create
    (cxGetResourceString(@sdxRichEditExceptionThrowInternalException));
end;

class procedure TdxRichEditExceptions.ThrowInvalidOperationException
  (const AName: string);
begin
...
begin
  raise EdxRichEditException.Create
    (cxGetResourceString(@sdxRichEditExceptionUnsupportedFormatException));
end;
