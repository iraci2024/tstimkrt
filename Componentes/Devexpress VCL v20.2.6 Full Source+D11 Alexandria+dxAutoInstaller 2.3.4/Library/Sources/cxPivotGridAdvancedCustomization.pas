...
uses
  Windows, Messages, Graphics, Classes, Controls, ExtCtrls, ActnList, Menus,
  StdCtrls, ImgList,
  cxCustomPivotGrid, cxPivotGridCustomization, cxGraphics, dxCoreGraphics,
  cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxContainer, cxEdit, cxGroupBox, cxButtons, cxCheckBox,
  cxLabel,
...
type
  ...
  TcxPivotGridCustomizationFormLayoutChooserClass = class of
    TcxPivotGridCustomizationFormLayoutChooser;
  ...
  public
    constructor Create(AFieldChooser
      : TcxPivotGridAdvancedCustomizationForm); virtual;
    destructor Destroy; override;

    procedure Popup(X, Y: Integer);

    property FieldChooser: TcxPivotGridAdvancedCustomizationForm
      read FFieldChooser;
  ...
  TcxPivotGridCustomizationFormStandardLayoutChooser = class
    (TcxPivotGridCustomizationFormLayoutChooser)
  private
  ...
  TcxPivotGridCustomizationFormLayout = (cflBottomPanelOnly1by4,
    cflBottomPanelOnly2by2, cflStackedDefault, cflStackedSideBySide,
    cflTopPanelOnly);
  TcxPivotGridCustomizationFormLayouts = set of
    TcxPivotGridCustomizationFormLayout;
  ...
  private
    ...
    procedure AlignChildren(AGroup: TcxGroupBox;
      AColumnCount, ARowCount: Integer);
    procedure CalculateTabOrders;
    function CreateFieldListBox(AParent: TWinControl;
      AFieldsType: TcxPivotGridCustomizationFormFieldListType)
      : TcxFieldListListBox;
    procedure ListBoxDblClick(Sender: TObject);
    procedure SetAllowedLayouts(const AValue
      : TcxPivotGridCustomizationFormLayouts);
  ...
  protected
    ...
    property LayoutChooser: TcxPivotGridCustomizationFormLayoutChooser
      read FLayoutChooser;
  ...
  public
    ...
    property AllowedLayouts: TcxPivotGridCustomizationFormLayouts
      read FAllowedLayouts write SetAllowedLayouts;
...
var
  cxPivotGridCustomizationFormLayoutChooserClass
    : TcxPivotGridCustomizationFormLayoutChooserClass;
...
uses
  Types, Contnrs, cxGeometry, Math, SysUtils, Forms, cxListBox, cxClasses,
  cxPivotGridStrs, dxCore;
...
procedure TcxPivotGridCustomizationFormStandardLayoutChooser.CreateItem
  (Action: TAction);
var
  AItem: TMenuItem;
begin
...
function TcxPivotGridCustomizationFormStandardLayoutChooser.GetPopupMenuClass
  : TComponentClass;
begin
  Result := TPopupMenu;
end;

procedure TcxPivotGridCustomizationFormStandardLayoutChooser.
  InitializePopupMenu;
begin
  inherited;
...
begin
  cxAdvancedDrawPopupMenuItem(TMenuItem(Sender), ACanvas, ARect, State,
    PopupMenu.Images);
end;

function TcxPivotGridCustomizationFormStandardLayoutChooser.GetPopupMenu
  : TPopupMenu;
begin
...
begin
  for I := Low(TcxPivotGridCustomizationFormFieldListType)
    to High(TcxPivotGridCustomizationFormFieldListType) do
    if GetFieldListByType(I) <> nil then
      (GetFieldListByType(I) as TcxFieldListListBox).UpdateSelection;
end;

procedure TcxPivotGridAdvancedCustomizationForm.DXMScaleChanged
...
function TcxPivotGridAdvancedCustomizationForm.HasDeferredLayoutChanges
  : Boolean;
begin
...
begin
  ...
  lbTopMain.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormFieldsCaption);
  lbFilterArea.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormFilterAreaCaption);
  lbColumnArea.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormColumnAreaCaption);
  lbRowArea.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormRowAreaCaption);
  lbDataArea.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormDataAreaCaption);

  acBottomPanelOnly1by4.Caption :=
    cxGetResourceString(@scxAdvancedCustomizationFormBottomPanelOnly1by4);
  acBottomPanelOnly2by2.Caption :=
    cxGetResourceString(@scxAdvancedCustomizationFormBottomPanelOnly2by2);
  acStackedDefault.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormStackedDefault);
  acStackedSideBySide.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormStackedSideBySide);
  acTopPanelOnly.Caption := cxGetResourceString
    (@scxAdvancedCustomizationFormTopPanelOnly);
end;
...
begin
  Result := 2 * BorderWidth + 2 * ScaleFactor.Apply(cxControlsIndent) + Width -
    ClientWidth;
end;

function TcxPivotGridAdvancedCustomizationForm.GetVertContentIndents: Integer;
begin
  Result := 2 * BorderWidth + 2 * ScaleFactor.Apply(cxControlsIndent) + Height -
    ClientHeight;
end;

procedure TcxPivotGridAdvancedCustomizationForm.
  BottomPanelOnly1by4FormCalculator;
begin
  gbMainVisibleFields.Constraints.MinHeight :=
    gbFilterArea.Constraints.MinHeight + gbColumnArea.Constraints.MinHeight +
    gbRowArea.Constraints.MinHeight + gbDataArea.Constraints.MinHeight +
    gbMainBottom.Constraints.MinHeight;
  gbMainVisibleFields.Constraints.MinWidth := gbMainBottom.Constraints.MinWidth;

  Constraints.MinHeight := gbMainVisibleFields.Constraints.MinHeight +
    gbTop.Constraints.MinHeight + GetVertContentIndents;
...
begin
  gbMainVisibleFields.Constraints.MinHeight :=
    gbFilterArea.Constraints.MinHeight + gbColumnArea.Constraints.MinHeight +
    gbMainBottom.Height + gbTop.Height;
  gbMainVisibleFields.Constraints.MinWidth := gbMainBottom.Constraints.MinWidth;

  Constraints.MinWidth := gbMainVisibleFields.Constraints.MinWidth +
    GetHorzContentIndents;
  Constraints.MinHeight := gbMainVisibleFields.Constraints.MinHeight +
    ScaleFactor.Apply(cxSplitterSize) + AvailableFieldList.Constraints.MinHeight
    + gbTop.Constraints.MinHeight + GetVertContentIndents;

  lbTopMain.Visible := True;
  gbMainVisibleFields.Visible := True;
  gbMainVisibleFields.Align := alBottom;
  gbMainVisibleFields.Height :=
    MulDiv(gbMainVisibleFields.Parent.ClientHeight, 3, 4);
...
begin
  ...
  gbMainVisibleFields.Constraints.MinWidth := gbMainBottom.Constraints.MinWidth
    + ScaleFactor.Apply(cxControlsIndent);

  Constraints.MinHeight := gbMainVisibleFields.Constraints.MinHeight +
    gbTop.Constraints.MinHeight + GetVertContentIndents;
  Constraints.MinWidth := gbMainVisibleFields.Constraints.MinWidth +
    ScaleFactor.Apply(cxSplitterSize) + AvailableFieldList.Constraints.MinWidth
    + GetHorzContentIndents;
...
begin
  ...
  Constraints.MinHeight := AvailableFieldList.Constraints.MinHeight +
    gbTop.Constraints.MinHeight + GetVertContentIndents;
  Constraints.MinWidth := gbTop.Constraints.MinWidth + GetHorzContentIndents;
  lbMain.Caption := FMainCaption;
end;

procedure TcxPivotGridAdvancedCustomizationForm.SetIsLayoutChanged
  (AValue: Boolean);
begin
...
procedure TcxPivotGridAdvancedCustomizationForm.AlignChildren
  (AGroup: TcxGroupBox; AColumnCount, ARowCount: Integer);

  procedure AlignImagesAndCaption(AImage: TcxImage; ALabel: TLabel;
    AParent: TcxGroupBox);
  begin
    ...
    begin
      ...
      ALabel.Width := AImage.Left - ScaleFactor.Apply(4) -
        TdxVisualRefinements.Padding.Left;
    end
    else
    begin
      AImage.Left := 0;
...
begin
  R := AGroup.ClientRect;
  R.Right := (cxRectWidth(R) - cxControlsIndent * (AColumnCount - 1))
    div AColumnCount;
...
function TcxPivotGridAdvancedCustomizationForm.CreateFieldListBox
  (AParent: TWinControl;
...
begin
  ...
  begin
    ...
    R.Top := AParent.Controls[0].BoundsRect.Bottom +
      ScaleFactor.Apply(cxControlsIndent);
    Result.BoundsRect := R;
  end
  else
  ...
  Result.ItemHeight := FieldItemHeight;
...
procedure TcxPivotGridAdvancedCustomizationForm.ListBoxDblClick
  (Sender: TObject);
var
...
begin
  ...
  begin
    ...
    UpdateHitTest;
    if (AListBox.SelectedField <> nil) and not PivotGrid.HitTest.HitAtFilter
    then
      ChangeFieldSorting(AListBox.SelectedField);
  end;
end;

procedure TcxPivotGridAdvancedCustomizationForm.SetAllowedLayouts
  (const AValue: TcxPivotGridCustomizationFormLayouts);
begin
...
procedure TcxPivotGridAdvancedCustomizationForm.SetLayout
  (AValue: TcxPivotGridCustomizationFormLayout);
begin
...
const
  RowCountMap: array [TcxPivotGridCustomizationFormLayout] of Integer = (4,
    2, 2, 4, 1);
  ColumnCountMap: array [TcxPivotGridCustomizationFormLayout] of Integer = (1,
    2, 2, 1, 1);
begin
  AlignChildren(TcxGroupBox(Sender), ColumnCountMap[Layout],
    RowCountMap[Layout]);
end;
