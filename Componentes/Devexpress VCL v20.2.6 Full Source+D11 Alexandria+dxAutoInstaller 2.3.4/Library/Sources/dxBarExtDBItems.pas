...
{$ENDIF}
  Types, Windows, Messages, SysUtils, Classes, Graphics, Controls, Db, StdCtrls,
  Forms,
...
{$ENDIF}
  ...
  protected
    ...
    function CheckKeyForDropDownWindow(Key: Word; Shift: TShiftState): Boolean;
      override;
  ...
  published
    ...
    property ImmediateDropDown: Boolean read FImmediateDropDown
      write FImmediateDropDown default False;
    property KeyField: string read FKeyFieldName write SetKeyFieldName;
    property ListField: string read FListFieldName write SetListFieldName;
    property ListFieldIndex: Integer read FListFieldIndex
    ...
    property OnKeyValueChange: TNotifyEvent read FOnKeyValueChange
      write FOnKeyValueChange;
  end;
...
begin
  if FBarLookupCombo <> nil then
    FBarLookupCombo.UpdateListFields;
end;

procedure TdxBarLookupLink.DataSetChanged;
begin
  if FBarLookupCombo <> nil then
    FBarLookupCombo.ListLinkDataChanged;
end;
...
procedure TdxBarLookupCombo.Notification(AComponent: TComponent;
...
begin
  if GetCapture = FPopupList.Handle then
    ReleaseCapture;
...
begin
  ...
  begin
    ...
    if not FInFindSelection and not VarIsNull(FCurKeyValue) and FListLink.Active
    then
      FListLink.DataSet.Locate(FKeyFieldName, FCurKeyValue, []);
  end;

  with CurItemLink.ItemRect do
    W := Right - Left - TdxBarLookupComboControl(CurItemLink.Control)
      .GetCaptionAreaWidth;
...
procedure TdxBarLookupCombo.DoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  FLocateEdit.SetBounds(D, D, FForm.ClientWidth - (D + D + W + D),
    FLocateEdit.Height);
  with FLocateList do
  begin
    Left := D;
...
function TdxBarLookupCombo.CheckKeyForDropDownWindow(Key: Word;
  Shift: TShiftState): Boolean;
begin
  // if {(FCombo <> nil) and }(Key = VK_RETURN) then
  if Key in [VK_RETURN, VK_TAB] then
    FSetValue := True;
  Result := Key in [VK_UP, VK_LEFT, VK_DOWN, VK_RIGHT, VK_PRIOR, VK_NEXT,
    VK_HOME, VK_END];
end;
...
begin
  if FListField <> nil then
    try
      ...
      begin
        SendMessage(GetEditHandle, EM_GETSEL, WPARAM(@AStartPos),
          LPARAM(@AEndPos));
        ...
        else if FFindSelection then
        begin
          FFindStr := Copy(FFindStr, 0, Length(FFindStr) - 1);
          SendMessage(GetEditHandle, EM_SETSEL, Length(FFindStr),
            Length(EditText));
        end;
      end
      ...
      begin
        ...
        try
          ...
          lFind := False;
          try
            lFind := FListLink.DataSet.Locate(FListField.FieldName, FFindStr,
...
begin
  ...
  begin
    ...
    if FCombo.FListActive and FCombo.FSetValue then
      if Value then
        FCombo.LocateKey
      else if (Text <> '') and (FCombo.FKeyField <> nil) then
...
procedure TdxBarPopupLookupControl.WMGetMinMaxInfo
...
begin
  if IsPopup then
    Painter.SysPanelCalcSize(Message.CalcSize_Params^.rgrc[0], FGripRect,
      ...
      TdxBarManagerAccess(FCombo.BarManager).ScaleFactor);
  inherited;
end;
...
procedure TdxBarPopupLookupControl.WMNCLButtonDown
...
procedure TdxBarPopupLookupControl.WMWindowPosChanging
...
procedure TdxBarPopupLookupControl.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if Operation = opRemove then
...
procedure TdxBarPopupLookupControl.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if (Button = mbLeft) and Assigned(ListLink.DataSet)
  { and ListLink.DataSet.CanModify } then
...
procedure TdxBarPopupLookupControl.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if FTracking then
  begin
    ...
    begin
      ...
      if Y >= ListLink.RecordCount then
        Exit;
      with FCombo do
        try
          if FKeyField <> nil then
...
begin
  ...
  begin
    ...
    if (FListFieldIndex >= 0) and (FListFieldIndex < FListFields.Count) then
      FListField := FListFields[FListFieldIndex]
    else if (FListFields.Count > 0) then
      FListField := FListFields[0];
    FListActive := FListField <> nil;
  end;
end;

procedure TdxBarPopupLookupControl.UpdateSizeGripCorner
  (ADropDownPosition: TPoint);
var
  AEditRect, ADropDownRect: TRect;
begin
  if IsPopup and (FCombo.CurItemLink <> nil) and
    (FCombo.CurItemLink.Control <> nil) then
  begin
    AEditRect := FCombo.CurItemLink.ItemRect;
    AEditRect.TopLeft := FCombo.CurItemLink.Control.Parent.ClientToScreen
      (AEditRect.TopLeft);
    AEditRect.BottomRight := FCombo.CurItemLink.Control.Parent.ClientToScreen
...
var
  Delta: Integer;
begin
  if not FCombo.FListActive then
    Exit;
  if Y < 0 then
    Y := 0;
  if Y >= ClientHeight then
...
begin
  ...
  if Value > 100 then
    Value := 100;
  Height := Value * GetTextHeight + 2 + Byte(IsPopup and FCombo.AllowResizing) *
    dxDropDownNCHeight;
end;
