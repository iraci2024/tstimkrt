...
uses
  ...
  dxLayoutControlAdapters, ActnList, StdCtrls,
  cxButtons, cxContainer, cxEdit, cxListBox, dxLayoutControl, cxClasses,
  cxGeometry, cxGridWizardCustomPage,
  cxGridWizardStrs, cxGridWizardCustomHelper, dxLayoutLookAndFeels,
  cxGridWizardUnboundViewsEditItemDialog;

type
  { TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame }

  TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame = class
    ...
    procedure lbUsedItemsDragOver(Sender, Source: TObject; X, Y: Integer;
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  ApplyLocalization;
begin
...
function TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  GetAddDialogCaption: string;
begin
  if IsLayoutView then
    Result := cxGetResourceString
      (@scxgsUnboundsSelectItemsForDisplayPageInputQueryCaptionAdd)
  else if IsTableView then
    Result := cxGetResourceString
...
function TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  GetEditDialogCaption: string;
begin
  if IsLayoutView then
    Result := cxGetResourceString
      (@scxgsUnboundsSelectItemsForDisplayPageInputQueryCaptionEdit)
  else if IsTableView then
    Result := cxGetResourceString
...
begin
  ...
  acMoveDown.Enabled := (lbUsedItems.SelCount > 0) and not lbUsedItems.Selected
    [lbUsedItems.Count - 1];
  acMoveUp.Enabled := (lbUsedItems.SelCount > 0) and
    not lbUsedItems.Selected[0];
end;

procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  PopulateProperties(var AStrings: TStringList);
var
  I: Integer;
  ADesc: string;
begin
  ...
  try
    ...
    begin
      ...
      if ADesc <> '' then
        AStrings.AddObject(ADesc,
          TObject(GetRegisteredEditProperties.Items[I]));
    end;
  finally
    AStrings.EndUpdate;
  end;
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.SwapItems
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.acAddExecute
  (Sender: TObject);
const
  FormatStr = '%s %d';
var
...
begin
  ACaption := Format(FormatStr, [Helper.GetDefaultItemCaption,
    Helper.ItemsCount + 1]);
  AProperties := TStringList.Create;
  try
    PopulateProperties(AProperties);
    APropertyIndex := 0;
    if cxgwExecuteEditItemDialog(Self, GetAddDialogCaption, AProperties,
      ACaption, APropertyIndex) then
    begin
      Helper.AddItem('', ACaption,
        TcxCustomEditPropertiesClass(AProperties.Objects[APropertyIndex]));
      LoadSettings;
    end;
  finally
    AProperties.Free;
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  acDeleteAllExecute(Sender: TObject);
var
  I: Integer;
begin
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  acDeleteSelectedExecute(Sender: TObject);
var
  I: Integer;
begin
  for I := lbUsedItems.Count - 1 downto 0 do
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.acEditExecute
  (Sender: TObject);
var
  ANewCaption: string;
...
begin
  ...
  try
    PopulateProperties(AProperties);
    ANewPropertyIndex := AProperties.IndexOfObject
      (TObject(Helper.ItemProperties[lbUsedItems.ItemIndex]));
    if cxgwExecuteEditItemDialog(Self, GetEditDialogCaption, AProperties,
      ANewCaption, ANewPropertyIndex) then
    begin
      ...
      if ANewPropertyIndex > 0 then
        Helper.ItemProperties[lbUsedItems.ItemIndex] :=
          TcxCustomEditPropertiesClass(GetRegisteredEditProperties.Items
          [ANewPropertyIndex])
      else
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  acMoveUpExecute(Sender: TObject);
var
  I: Integer;
begin
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  lbUsedItemsClick(Sender: TObject);
begin
  CheckItemsCount;
end;

procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  lbUsedItemsDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState;
  var Accept: Boolean);
begin
  Accept := True;
end;
...
procedure TcxGridWizardUnboundViewsSelectItemsForDisplayPageFrame.
  lbUsedItemsEndDrag(Sender, Target: TObject; X, Y: Integer);
var
    ...
    try
      ...
      for I := 0 to ACount - 1 do
        ...
        begin
          ...
          lbUsedItems.Items.AddObject(lbUsedItems.Items[I],
            lbUsedItems.Items.Objects[I]);
        end;
