...
type
  ...
  protected
    procedure CopyContentCore(ACopyManager
      : TdxSimpleDocumentModelCopyManager); virtual;
    procedure CopyCore(ATargetModel: TdxSimpleDocumentModel;
      ATargetRun: TdxTextRunBase); virtual;
  public
    ...
    function CreateRun(AParagraph: TdxSimpleParagraph;
      AStartIndex, ALength: Integer): TdxTextRun; virtual;
  ...
  protected
    ...
    procedure SetRowProcessingFlags(const AValue
      : TdxRowProcessingFlags); override;
  end;
...
begin
  ...
  Result := (ANextRun is TdxTextRun) and not TdxTextRun(ANextRun).IsFieldRun and
    (Paragraph = ANextRun.Paragraph) and
    (StartIndex + Length = ANextRun.StartIndex) and
...
function TdxTextRun.Copy(ACopyManager: TdxCustomDocumentModelCopyManager)
  : TdxTextRunBase;
var
...
function TdxTextRun.GetText(ABuffer: TdxChunkedStringBuilder;
...
function TdxTextRun.CreateRun(AParagraph: TdxSimpleParagraph;
  AStartIndex, ALength: Integer): TdxTextRun;
begin
  Result := TdxTextRun.Create(AParagraph, AStartIndex, ALength);
end;

procedure TdxTextRun.Measure(ABoxInfo: TdxBoxInfo;
...
begin
  ...
  begin
    ...
    Result := (not ARunOptions.UseDoubleFontSize or
      (ARunInfo.DoubleFontSize = AFormatting.DoubleFontSize)) and
      (not ARunOptions.UseFontBold or (ARunInfo.FontBold = AFormatting.FontBold)
      ) and (not ARunOptions.UseFontItalic or
      (ARunInfo.FontItalic = AFormatting.FontItalic)) and
      (not ARunOptions.UseFontStrikeoutType or
      (ARunInfo.FontStrikeoutType = AFormatting.FontStrikeoutType)) and
      (not ARunOptions.UseFontUnderlineType or
      (ARunInfo.FontUnderlineType = AFormatting.FontUnderlineType)) and
      (not ARunOptions.UseAllCaps or (ARunInfo.AllCaps = AFormatting.AllCaps))
      and (not ARunOptions.UseHidden or (ARunInfo.Hidden = AFormatting.Hidden))
      and (not ARunOptions.UseStrikeoutWordsOnly or
      (ARunInfo.StrikeoutWordsOnly = AFormatting.StrikeoutWordsOnly)) and
      (not ARunOptions.UseUnderlineWordsOnly or
      (ARunInfo.UnderlineWordsOnly = AFormatting.UnderlineWordsOnly)) and
      (not ARunOptions.UseForeColor or
      (ARunInfo.ForeColor = AFormatting.ForeColor)) and
      (not ARunOptions.UseBackColor or
      (ARunInfo.BackColor = AFormatting.BackColor)) and
      (not ARunOptions.UseUnderlineColor or
      (ARunInfo.UnderlineColor = AFormatting.UnderlineColor)) and
      (not ARunOptions.UseStrikeoutColor or
      (ARunInfo.StrikeoutColor = AFormatting.StrikeoutColor)) and
      (not ARunOptions.UseScript or (ARunInfo.Script = AFormatting.Script)) and
      (not ARunOptions.UseNoProof or (ARunInfo.NoProof = AFormatting.NoProof))
      and (not ARunOptions.UseFontName or
      (ARunInfo.FontName = AFormatting.FontName));
  end;
end;

procedure TdxTextRun.CopyContentCore(ACopyManager
  : TdxSimpleDocumentModelCopyManager);
var
  ALogPosition: TdxDocumentLogPosition;
begin
  ALogPosition := ACopyManager.TargetPosition.LogPosition;
  if ACopyManager.TargetPieceTable <> ACopyManager.TargetModel.MainPieceTable
  then
    ACopyManager.TargetPieceTable.InsertText(ALogPosition,
      StringReplace(GetTextFast(ACopyManager.SourcePieceTable.TextBuffer),
      TdxCharacters.PageBreak, TdxCharacters.LineBreak, [rfReplaceAll]))
  else
    ACopyManager.TargetPieceTable.InsertText(ALogPosition,
      GetTextFast(ACopyManager.SourcePieceTable.TextBuffer));
end;

procedure TdxTextRun.CopyCore(ATargetModel: TdxSimpleDocumentModel;
...
function TdxTextRun.TryAdjustEndPositionToFit(ABoxInfo: TdxBoxInfo;
...
begin
  AText := PieceTable.GetTextFromSingleRun(ABoxInfo.StartPos, ABoxInfo.EndPos);
  AFontInfo := DocumentModel.FontCache[FontCacheIndex];
  Result := AMeasurer.TryAdjustEndPositionToFit(ABoxInfo, AText, AFontInfo,
    AMaxWidth);
end;
...
procedure TdxSpecialTextRun.SetRowProcessingFlags(const AValue
...
function TdxSeparatorTextRun.GetTextFast
  (ABuffer: TdxChunkedStringBuilder): string;
begin
  Result := '';
end;

function TdxSeparatorTextRun.GetRawTextFast
  (ABuffer: TdxChunkedStringBuilder): string;
begin
  Result := SeparatorText;
end;

function TdxSeparatorTextRun.GetPlainText(ABuffer: TdxChunkedStringBuilder;
  AFrom, ATo: Integer): string;
begin
...
procedure TdxSeparatorTextRun.Measure(ABoxInfo: TdxBoxInfo;
  const AMeasurer: IdxObjectMeasurer);
begin
...
function TdxSeparatorTextRun.Copy(ACopyManager
...
begin
  ...
  ATargetPosition := ACopyManager.TargetPosition;
  APieceTable.InsertSeparatorTextRunCore(ATargetPosition.ParagraphIndex,
...
function TdxSeparatorTextRun.TryAdjustEndPositionToFit(ABoxInfo: TdxBoxInfo;
  AMaxWidth: Integer; const AMeasurer: IdxObjectMeasurer): Boolean;
begin
  Result := False;
end;
