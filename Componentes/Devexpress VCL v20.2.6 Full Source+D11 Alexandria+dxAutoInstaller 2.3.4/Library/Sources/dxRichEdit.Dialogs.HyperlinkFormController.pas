...
type
  ...
  TdxHyperlinkUriType = (None, Url, Anchor);
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      AHyperlinkInfo: TdxHyperlinkInfo; ARunInfo: TdxRunInfo);
    ...
    property InitialTextToDisplay: string read FInitialTextToDisplay
      write FInitialTextToDisplay;
  ...
  protected
    ...
    function CanUseNavigateUri(const ANavigateUri: string;
      const AAnchor: string): Boolean;
...
constructor TdxHyperlinkFormControllerParameters.Create(const AControl
  : IdxRichEditControl; AHyperlinkInfo: TdxHyperlinkInfo; ARunInfo: TdxRunInfo);
begin
...
function TdxHyperlinkFormControllerParameters.GetTextSource
...
constructor TdxHyperlinkFormController.Create(AControllerParameters
  : TdxHyperlinkFormControllerParameters);
begin
  inherited Create;
...
begin
  ...
  if FRunInfo.NormalizedEnd^ > FRunInfo.NormalizedStart^ then
    if CanChangeDisplayText then
      TextToDisplay := Trim(GetText(FRunInfo.NormalizedStart^,
        FRunInfo.NormalizedEnd^))
    else
      TextToDisplay := '';
...
function TdxHyperlinkFormController.CanUseNavigateUri(const ANavigateUri
  : string; const AAnchor: string): Boolean;
begin
  Result := (ANavigateUri <> '') or (AAnchor = '') or
    (not BookmarkExists(AAnchor));
end;

function TdxHyperlinkFormController.BookmarkExists(const AName: string)
  : Boolean;
var
...
begin
  Result := (FRunInfo.Start.ParagraphIndex = FRunInfo.&End.ParagraphIndex) and
    not FControl.InnerControl.DocumentModel.Selection.PieceTable.
    HasInlinePicture(FRunInfo);
end;

function TdxHyperlinkFormController.GetText(const AStart,
  AEnd: TdxDocumentModelPosition): string;
var
  AFilter: TdxVisibleTextFilterBase;
begin
  AFilter := TdxPieceTable(AStart.PieceTable).VisibleTextFilter;
  Result := TdxPieceTable(AStart.PieceTable).GetFilteredPlainText(AStart, AEnd,
    AFilter.IsRunVisible);
end;
...
begin
  ...
  HyperlinkInfo.NavigateUri := TdxHyperlinkUriHelper.EnsureUriIsValid
    (ANavigateUri);
end;
...
begin
  if (TextToDisplay <> '') or (TextSource = TdxTextToDisplaySource.ExistingText)
  then
    Exit;
  if NavigateUri <> '' then
