...
uses
  Generics.Defaults, Generics.Collections, Windows, SysUtils, Classes, Graphics,
  cxClasses, dxCore, cxGraphics,
...
type
  ...
  protected
    ...
    procedure CalculateMetric(DC: HDC;
      const AMetric: TOutlineTextmetricA); virtual;
  ...
  public
    ...
    property ConversionTable: TdxPSPDFCIDFontConversionTable
      read FConversionTable;
  ...
  public
    ...
    function GetFontForSymbol(const ASymbol: WideChar;
      ACurrentFont: TdxPSPDFCustomFont): TdxPSPDFCustomFont;
    function GetFontInfoForSymbol(const ASymbol: WideChar;
      out AInfo: TdxPSPDFFontGlyphsInfo): Boolean;
    //
    property CachedInfo[Index: Integer]: TdxPSPDFFontGlyphsInfo
      read GetCachedInfo;
...
const
  ...
  sdxThaiEncoding =
    '128/Euro 133/ellipsis 145/quoteleft/quoteright/quotedblleft' +
  ...
  sdxTurkishEncoding =
    '128/Euro 130/quotesinglbase/florin/quotedblbase/ellipsis' +
...
constructor TdxPSPDFTrueTypeFont.Create(AOwner: TdxPSPDFFontList;
  AEmbed: Boolean; AFont: TFont);
begin
...
function TdxPSPDFTrueTypeFont.ConvertToAnsiString(const AText: string)
  : AnsiString;
var
  I: Integer;
begin
...
function TdxPSPDFTrueTypeFont.EncodeText(const S: string): AnsiString;
  ...
  function CheckEscapeSequences(const AText: AnsiString): AnsiString;
    ...
    procedure EscapeChar(var ADestination: PAnsiChar; AChar: AnsiChar;
      var ACount: Integer);
    begin
      ADestination^ := '\';
      Inc(ADestination);
...
begin
  ...
  AWriter.WriteString(sdxPDFFirstChar + sdxPDFSpace +
    IntToStr(Descriptor.FirstChar));
  AWriter.WriteString(sdxPDFLastChar + sdxPDFSpace +
    IntToStr(Descriptor.LastChar));
...
begin
  ...
  AWriter.WriteString(sdxPDFEncoding + sdxPDFSpace + AWriter.MakeLinkToObject
    (Encoding));
  AWriter.WriteString(sdxPDFFontDescriptor + sdxPDFSpace +
    AWriter.MakeLinkToObject(Descriptor));
...
procedure TdxPSPDFTrueTypeFontEncoding.WriteEncodingTable
...
const
  BaseEncodingMap: array [Boolean] of string = (sdxPDFEncodingWinAnsi,
    sdxPDFEncodingMacRoman);
begin
  inherited WriteHeader(AWriter);
  AWriter.WriteString(sdxPDFBaseEncoding + BaseEncodingMap
...
procedure TdxPSPDFFontDescriptor.CalculateMetric(DC: HDC;
  const AMetric: TOutlineTextmetricA);
var
...
function TdxPSPDFFontDescriptor.CalculateFontFlags(const APanose
  : TPanose): Integer;
begin
...
begin
  ...
  AWriter.WriteString(sdxPDFMissingWidth + sdxPDFSpace +
    IntToStr(FAveCharWidth));
  ...
  if Assigned(FontFile) then
    AWriter.WriteString(sdxPDFFontFile + sdxPDFSpace + AWriter.MakeLinkToObject
      (FontFile));
end;
...
constructor TdxPSPDFCIDFont.Create(AOwner: TdxPSPDFFontList; AEmbed: Boolean;
  AFont: TFont);
begin
...
begin
  ...
  AWriter.WriteString(sdxPDFDescendantFonts + ' [' + AWriter.MakeLinkToObject
    (FontInfo) + ']');
  AWriter.WriteString(sdxPDFToUnicode + sdxPDFSpace + AWriter.MakeLinkToObject
    (ConversionTable));
end;
...
begin
  ...
  AWriter.WriteString(sdxPDFBaseFont + sdxPDFSpace + '/' +
    Owner.EncodeFontName);
  AWriter.WriteString(sdxPDFFontDescriptor + sdxPDFSpace +
    AWriter.MakeLinkToObject(Descriptor));
...
function TdxPSPDFCIDFontConversionTable.GetContentStreamType
  : TdxPSPDFStreamType;
begin
  Result := pstText;
end;
...
procedure TdxPSPDFCIDFontConversionTable.WriteContentStream
  (AWriter: TdxPSPDFWriter);
begin
...
procedure TdxPSPDFCIDFontConversionTable.WriteConversionTable
  (AWriter: TdxPSPDFWriter);
var
...
begin
  FontDataNeeded;
  inherited WriteHeader(AWriter);
  AWriter.WriteString(sdxPDFLength + '1' + sdxPDFSpace +
    IntToStr(FontData.Size));
end;
...
procedure TdxPSPDFFontGlyphsInfo.Analyze(AFont: TFont);
  ...
  procedure SetBand(var ABandIndex: Integer;
...
function TdxPSPDFFontsManager.GetFontForSymbol(const ASymbol: WideChar;
  ACurrentFont: TdxPSPDFCustomFont): TdxPSPDFCustomFont;
var
...
function TdxPSPDFFontsManager.GetFontInfoForSymbol(const ASymbol: WideChar;
  out AInfo: TdxPSPDFFontGlyphsInfo): Boolean;

  function FindFontInfo(const ASymbol: WideChar;
    out AInfo: TdxPSPDFFontGlyphsInfo): Boolean;
  var
    I: Integer;
  begin
...
begin
  ...
  begin
    ...
    try
      ...
      FAllFontsProcessed := dxEnumFontFamilies(DC, ALogFont, @EnumFontsProc,
        TRUETYPE_FONTTYPE, 0);
      Result := FindFontInfo(ASymbol, AInfo);
    finally
      ReleaseDC(0, DC);
...
function TdxPSPDFFontsManager.GetCachedInfo(Index: Integer)
  : TdxPSPDFFontGlyphsInfo;
begin
