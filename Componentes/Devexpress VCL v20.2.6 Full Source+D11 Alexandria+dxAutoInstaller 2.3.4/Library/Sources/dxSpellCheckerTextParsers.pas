...
type
  ...
  strict private
    ...
    function InternalGetNextWord(var AStart, ALength, APos: Integer;
      var ADelimiters: string): string;
  protected
    function ContainsOnlyDelimiters(const AText: string): Boolean;
    function DoGetNextWord(var AStart, ALength: Integer;
      var ADelimiters: string): Boolean;
  ...
  public
    ...
    function GetNextWord(var AStart, AFinish: IdxSpellCheckerPosition)
      : Boolean; override;
...
function TdxSpellCheckerTextController.GetNextWord(var AStart,
...
function TdxSpellCheckerTextController.GetSentenceFinishPosition
...
function TdxSpellCheckerTextController.GetSentenceStartPosition
  (const APos: IdxSpellCheckerPosition): IdxSpellCheckerPosition;
var
  AStartPos: Integer;
begin
  ...
  while (AStartPos > 1) and not IsSentenceDelimiters(Text[AStartPos]) do
    Dec(AStartPos);
  while (AStartPos > 1) and (IsSentenceDelimiters(Text[AStartPos]) or
    (Text[AStartPos] = ' ')) do
    Inc(AStartPos);
  Result := EncodePosition(AStartPos);
end;

function TdxSpellCheckerTextController.GetWordStartPosition
  (const APos: IdxSpellCheckerPosition): IdxSpellCheckerPosition;
begin
  Result := EncodePosition(GetWordNextPosition(DecodePosition(APos), -1));
end;

procedure TdxSpellCheckerTextController.UpdateMeaningfulCharacters
  (const AValue: string);
begin
  FMeaningfulCharacters := AValue;
end;

function TdxSpellCheckerTextController.ContainsOnlyDelimiters
...
function TdxSpellCheckerTextController.DoGetNextWord(var AStart,
  ALength: Integer; var ADelimiters: string): Boolean;
var
  APos: Integer;
  AWord: string;
begin
  ...
  repeat
    ...
    if APos >= FSpellingAreaFinish then
      Break;
    if ((Length(ADelimiters) = 0) or TdxCharacters.IsWhiteSpace
      (ADelimiters[Length(ADelimiters)])) and (IsUrl(AWord) or IsMail(AWord))
    then
      Break;
...
function TdxSpellCheckerTextController.IsMeaningfulCharacters
  (AChar: WideChar): Boolean;
begin
  Result := WideCharPos(AChar, FMeaningfulCharacters) > 0;
end;

function TdxSpellCheckerTextController.IsSentenceDelimiters
...
function TdxSpellCheckerTextController.GetWordFinishPosition
  (const APos: IdxSpellCheckerPosition): IdxSpellCheckerPosition;
begin
  Result := EncodePosition(GetWordNextPosition(DecodePosition(APos), 1));
end;

function TdxSpellCheckerTextController.GetWordNextPosition(APosition,
  ADirection: Integer): Integer;
begin
  Result := APosition;
  while InRange(Result + ADirection, FSpellingAreaStart, FSpellingAreaFinish)
    and not TdxCharacters.IsWhiteSpace(Text[Result]) do
...
procedure TdxSpellCheckerTextController.SkipFirstDelimiters(var AStart: Integer;
  var ADelimiters: string);
begin
  if (AStart <= FSpellingAreaFinish) and TdxCharacters.IsWhiteSpace
    (FText[AStart]) then
  begin
    while (AStart < FSpellingAreaFinish) and TdxCharacters.IsWhiteSpace
      (FText[AStart]) do
    begin
    ...
    while (AStart <= FSpellingAreaFinish) and
      not(WideIsAlphaNumeric(FText[AStart]) or
      IsMeaningfulCharacters(FText[AStart])) do
    begin
      ADelimiters := ADelimiters + FText[AStart];
      Inc(AStart);
    end;
...
function TdxSpellCheckerTextController.InternalGetNextWord(var AStart, ALength,
  APos: Integer; var ADelimiters: string): string;
begin
  ALength := 0;
  while (APos <= FSpellingAreaFinish) and not TdxCharacters.IsWhiteSpace
    (FText[APos]) do
    Inc(APos);
  ALength := APos - AStart;
  if ALength = 0 then
  ...
  begin
    while (AStart <= FSpellingAreaFinish) and
      (IsDelimiter(FText[AStart]) and
      ((ALength = 1) or (((AStart < APos) and
      not(WideIsAlphaNumeric(FText[AStart + 1]) or IsMeaningfulCharacters(FText
      [AStart + 1]))) or not IsMeaningfulCharacters(FText[AStart])))) do
    begin
      ADelimiters := ADelimiters + FText[AStart];
      Inc(AStart);
      Dec(ALength);
    end;
    while (APos >= AStart) and (IsDelimiter(FText[APos]) and
      not IsMeaningfulCharacters(FText[APos])) do
      Dec(APos);
    ...
    begin
      if ContainsOnlyDelimiters(Result) then
        Exit;
      APos := AStart;
      while (APos <= AStart + ALength) and
        not(IsDelimiter(FText[APos]) and not IsMeaningfulCharacters
        (FText[APos])) do
        Inc(APos);
      ALength := APos - AStart;
      Result := Copy(FText, AStart, ALength);
