...
type
  ...
  protected
    function GetPredefinedOptions(AFormat: TdxRichEditDocumentFormat)
      : IdxExporterOptions; override;
    procedure PreprocessContentBeforeExport
      (AFormat: TdxRichEditDocumentFormat); override;
...
function TdxDocumentExportHelper.GetPredefinedOptions
  (AFormat: TdxRichEditDocumentFormat): IdxExporterOptions;
begin
  Result := DocumentModel.DocumentExportOptions.GetOptions(AFormat);
end;

procedure TdxDocumentExportHelper.PreprocessContentBeforeExport
...
begin
  ...
  if not ADocumentSaveOptions.CanSaveToCurrentFileName or
    (ADocumentSaveOptions.CurrentFileName = '') then
    Result := ADocumentSaveOptions.DefaultFileName
  else
    Result := ADocumentSaveOptions.CurrentFileName;
end;

function TdxDocumentExportHelper.GetCurrentDocumentFormat
...
procedure TdxDocumentExportHelper.ApplyEncoding(const AOptions
  : IdxExporterOptions; AEncoding: TEncoding);
begin
