...
{$ENDIF}
  ...
  protected
    ...
    procedure SetCaption(const Value: string); {$IFDEF DELPHIXE3} override;
    {$ENDIF}
    procedure SetControl(Value: TControl); virtual;
    procedure SetHint(const Value: string); {$IFDEF DELPHIXE3} override;
    {$ENDIF}
...
{$ENDIF}
  ...
  published
    property AssignedValues: TdxBasicActionAssignedValues read FAssignedValues
...
begin
  ...
  Filer.DefineProperty('Caption', ReadCaption, WriteCaption, IsCaptionStored and
    (Caption = ''));
  Filer.DefineProperty('Hint', ReadHint, WriteHint, IsHintStored and
    (Hint = ''));
end;
...
procedure TdxBasicAction.Notification(AComponent: TComponent;
...
procedure TdxBasicAction.SetAssignedValues(const Value
  : TdxBasicActionAssignedValues);
begin
