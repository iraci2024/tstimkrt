...
type
  ...
  TcxGridDBDataController = class(TcxDBDataController,
    IcxCustomGridDataController, IcxGridDataController, IcxEditorFieldLink)
  private
  ...
  protected
    ...
    procedure PrepareFieldForSorting(AField: TcxCustomDataField;
      AMode: TcxDataControllerComparisonMode); override;
...
uses
  ...
  cxStorage, cxGridCommon, cxGridLevel, Controls, Forms, cxDBLookupComboBox,
  dxDPIAwareUtils, cxGeometry,
  cxDataControllerSpreadSheetExpressionProvider, RTLConsts;
...
function TcxGridDBDataFilterCriteriaItem.
  IsItemLinkSupportsMultiThreading: Boolean;
var
...
function TcxGridDBDataFilterCriteriaItem.GetDataController
...
function TcxGridDBDefaultValuesProvider.IsDisplayFormatDefined
...
procedure TcxGridDBDataController.AssignData(ADataController
...
function TcxGridDBDataController.GetItemDataBindingClass
  : TcxGridItemDataBindingClass;
begin
  Result := TcxGridItemDBDataBinding;
end;

function TcxGridDBDataController.GetItemDefaultValuesProviderClass
  : TcxCustomEditDefaultValuesProviderClass;
begin
...
function TcxGridDBDataController.CompareByField(ARecordIndex1,
...
function TcxGridDBDataController.CreateExpressionProvider
...
function TcxGridDBDataController.GetFilterCriteriaClass
  : TcxDataFilterCriteriaClass;
begin
  Result := TcxGridDBDataFilterCriteria;
end;

function TcxGridDBDataController.GetFilterDisplayText(ARecordIndex,
...
function TcxGridDBDataController.GetSortingBySummaryEngineClass
  : TcxSortingBySummaryEngineClass;
begin
  Result := GridView.ViewData.GetSortingBySummaryEngineClass;
end;

function TcxGridDBDataController.GetSummaryGroupItemLinkClass
...
procedure TcxGridDBDataController.PrepareFieldForSorting
  (AField: TcxCustomDataField; AMode: TcxDataControllerComparisonMode);
begin
  inherited PrepareFieldForSorting(AField, AMode);
  AField.Flags := Byte(GridView.ViewData.NeedsCustomDataComparison
    (AField, AMode));
end;
...
function TcxGridDBDataController.CreateDetailLinkObject
...
procedure TcxGridDBDataController.FocusControl(AItemIndex: Integer;
  var Done: Boolean);
begin
  inherited;
...
function TcxGridDBDataController.GetDetailDataControllerByLinkObject
  (ALinkObject: TObject): TcxCustomDataController;
begin
  Result := TcxCustomGridView(ALinkObject).DataController;
end;

function TcxGridDBDataController.GetDisplayText(ARecordIndex,
  AItemIndex: Integer): string;
begin
...
function TcxGridDBDataController.GetFilterDataValue(ARecordIndex: Integer;
...
function TcxGridDBDataController.GetFilterItemFieldCaption
...
function TcxGridDBDataController.GetItemValueSource(AItemIndex: Integer)
...
procedure TcxGridDBDataController.CreateAllItems(AMissingItemsOnly
  : Boolean = False);
var
  I: Integer;
begin
      ...
      with DataSet do
        for I := 0 to FieldCount - 1 do
          if not AMissingItemsOnly or
            (GetItemByFieldName(Fields[I].FieldName) = nil) then
...
function TcxGridDBDataController.CreateItemByField(AField: TField)
  : TcxCustomGridTableItem;
begin
...
function TcxGridDBDataController.GetItemByFieldName(const AFieldName: string)
  : TcxCustomGridTableItem;
begin
...
begin
  inherited;
  TcxGridDBDefaultValuesProvider(DefaultValuesProvider).DataSource :=
...
function TcxGridItemDBDataBinding.DefaultWidth(ATakeHeaderIntoAccount
...
begin
  ...
  begin
    ...
    Result := AField.DisplayWidth * ACanvas.TextWidth('0') + 2 *
      dxGetScaleFactor(GridView).Apply(cxTextOffset);
  end
  else
