...
uses
  ...
  dxLayoutControlAdapters, dxLayoutContainer, StdCtrls, cxRadioGroup, cxLabel,
  cxCheckBox, ExtCtrls, dxLayoutControl,
  cxGridWizardCustomPage, cxGridWizardStrs, cxGridCustomTableView, cxGroupBox,
  dxLayoutLookAndFeels;

type
  { TcxGridWizardCommonOptionsFilteringSortingPageFrame }

  TcxGridWizardCommonOptionsFilteringSortingPageFrame = class
...
type
  TcxCustomGridTableOptionsCustomizeAccess = class
...
begin
  chbFilterBox.Caption := cxGetResourceString
    (@scxgwFilteringSortingPageFilterBoxVisible);
  chbFilterBox.Hint := cxGetResourceString
  ...
  rbShowNonEmpty.Caption := cxGetResourceString
    (@scxgwFilteringSortingPageFilterBoxVisibleNonEmpty);
  rbShowNonEmpty.Hint := cxGetResourceString
    (@scxgwFilteringSortingPageFilterBoxVisibleNonEmptyHint);

  chbFilterButton.Caption := cxGetResourceString
    (@scxgwUIElementsPageFilterButton);
  chbFilterButton.Hint := cxGetResourceString
    (@scxgwUIElementsPageFilterButtonHint);

  rbShowButtonAlways.Caption := cxGetResourceString
    (@scxgwFilteringSortingPageShowColumnFilterButtonAlways);
  rbShowButtonAlways.Hint := cxGetResourceString
    (@scxgwFilteringSortingPageShowColumnFilterButtonAlwaysHint);

  rbShowButtonWhenSelected.Caption :=
    cxGetResourceString
    (@scxgwFilteringSortingPageShowColumnFilterButtonWhenSelected);
  rbShowButtonWhenSelected.Hint :=
    cxGetResourceString
...
begin
  ...
  TcxCustomGridTableOptionsCustomizeAccess(AView.OptionsCustomize).ItemFiltering
    := chbFilterButton.Checked;
  if rbShowButtonAlways.Checked then
    TcxCustomGridTableOptionsViewAccess(AView.OptionsView).ShowItemFilterButtons
      := sfbAlways
  else
    TcxCustomGridTableOptionsViewAccess(AView.OptionsView).ShowItemFilterButtons
      := sfbWhenSelected;
  ...
  else if rbShowAlways.Checked then
...
begin
  ...
  AView := TcxCustomGridTableView(Helper.GridView);
  chbFilterButton.Checked := TcxCustomGridTableOptionsCustomizeAccess
    (AView.OptionsCustomize).ItemFiltering;
  rbShowButtonAlways.Checked := TcxCustomGridTableOptionsViewAccess
    (AView.OptionsView).ShowItemFilterButtons = sfbAlways;
  rbShowButtonWhenSelected.Checked := TcxCustomGridTableOptionsViewAccess
    (AView.OptionsView).ShowItemFilterButtons = sfbWhenSelected;
...
function TcxGridWizardCommonOptionsFilteringSortingPageFrame.
...
procedure TcxGridWizardCommonOptionsFilteringSortingPageFrame.
  UpdateControlsState;
begin
...
procedure TcxGridWizardCommonOptionsFilteringSortingPageFrame.RefreshPreviewGrid
  (Sender: TObject);
begin
