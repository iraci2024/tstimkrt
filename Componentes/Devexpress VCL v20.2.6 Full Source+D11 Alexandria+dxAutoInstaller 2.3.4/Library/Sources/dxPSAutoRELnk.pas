...
type
  TdxPSdxCustomAutoRichEditProducer = class
...
begin
  ...
  if not IsDesigning and (TCustomAutoRichEditAccess(Control).Text <> '') then
    Result := dxPSContainerLnk.dxPSMakeFriendlyNameFromStrings
      (TCustomAutoRichEditAccess(Control).Lines);
  if Result = '' then
...
begin
  Result := GetRichEditAsGraphic(Control.Handle,
    TCustomAutoRichEditAccess(Control).Color, GetMargins, TMetafile,
    Control.Width, -1, GetMaxHeight, DC);
end;

procedure TdxPSdxCustomAutoRichEditProducer.InitializeItem
