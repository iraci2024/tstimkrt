...
{$ENDIF}
  ...
  dxSpreadSheetGraphics, dxGDIPlusClasses, dxSpreadSheetHyperlinks,
  dxCoreClasses;
...
type
  ...
  TdxSpreadSheetCustomClipboardData = class abstract(TInterfacedObject,
    IdxSpreadSheetClipboardData)
  protected
  ...
  TdxSpreadSheetCustomClipboardFormatClass = class of
    TdxSpreadSheetCustomClipboardFormat;
  ...
  public
    ...
    class procedure SaveToClipboard
      (AData: IdxSpreadSheetClipboardData); virtual;
    class procedure SaveToStream(AStream: TStream;
      AData: IdxSpreadSheetClipboardData); virtual;
  end;

  TdxSpreadSheetClipboardFormatEnumProc = reference to procedure
    (AFormat: TdxSpreadSheetCustomClipboardFormatClass);
  ...
  protected
    class procedure CopyDataToClipboard(AView: TdxSpreadSheetTableView;
      AMode: TdxSpreadSheetClipboardCopyMode);
    class function IsPasteSpecial(AOptions
      : TdxSpreadSheetClipboardPasteOptions): Boolean;
    class function PreparePastePoints(ASelection
      : TdxSpreadSheetTableViewPasteSelection; const AClipboardArea: TRect;
      AClipboardCopyMode: TdxSpreadSheetClipboardCopyMode): TList<TPoint>;
    class function ReadDataFromClipboard
      (AOptions: TdxSpreadSheetClipboardPasteOptions;
      out AData: IdxSpreadSheetClipboardData): Boolean;
  public
    class procedure EnumFormats
      (AProc: TdxSpreadSheetClipboardFormatEnumProc); virtual;
    class procedure EnumFormatsForPaste
    ...
    class function CanPaste(AOptions
      : TdxSpreadSheetClipboardPasteOptions): Boolean;
    ...
    class procedure PasteData(AView: TdxSpreadSheetTableView;
      AData: IdxSpreadSheetClipboardData; const AClipboardArea: TRect;
      AClipboardCopyMode: TdxSpreadSheetClipboardCopyMode;
      const AViewGUID: string; AOptions: TdxSpreadSheetClipboardPasteOptions);
...
uses
  Math, Dialogs, Controls, Variants, StrUtils, dxSpreadSheetUtils, cxGeometry,
  dxSpreadSheetClipboardFormats,
...
function TdxSpreadSheetCustomClipboardData.GetMode
...
class function TdxSpreadSheetCustomClipboardFormat.Build(const AArea: TRect;
...
class function TdxSpreadSheetCustomClipboardFormat.CanLoadFromClipboard
  : Boolean;
begin
...
class function TdxSpreadSheetCustomClipboardFormat.
  IsPasteOptionsSupported: Boolean;
begin
  Result := False;
end;

class function TdxSpreadSheetCustomClipboardFormat.LoadFromClipboard
  : IdxSpreadSheetClipboardData;
var
  AHandle: THandle;
  AMemory: Pointer;
...
class function TdxSpreadSheetCustomClipboardFormat.LoadFromStream
  (AStream: TStream): IdxSpreadSheetClipboardData;
begin
  Result := nil; // for CBuilder
end;

class procedure TdxSpreadSheetCustomClipboardFormat.SaveToClipboard
...
begin
  ...
  try
    ...
    begin
      AHandle := GlobalAlloc(GMEM_MOVEABLE or GMEM_DDESHARE,
        AMemoryStream.Size);
      AMemory := GlobalLock(AHandle);
      try
...
class procedure TdxSpreadSheetCustomClipboardFormat.SaveToStream
...
procedure TdxSpreadSheetTableViewPasteSelection.Restore
...
procedure TdxSpreadSheetTableViewPasteSelection.Store
  (AView: TdxSpreadSheetTableView; const AMinSize: TSize);
var
  I: Integer;
  AArea: TRect;
begin
...
class procedure TdxSpreadSheetTableViewClipboardHelper.EnumFormats
  (AProc: TdxSpreadSheetClipboardFormatEnumProc);
begin
...
class procedure TdxSpreadSheetTableViewClipboardHelper.EnumFormatsForPaste
  (AProc: TdxSpreadSheetClipboardFormatEnumProc; AIsPasteSpecial: Boolean);
begin
  EnumFormats(procedure(AFormat: TdxSpreadSheetCustomClipboardFormatClass)
    begin
...
class procedure TdxSpreadSheetTableViewClipboardHelper.Copy
  (AView: TdxSpreadSheetTableView);
begin
  CopyDataToClipboard(AView, ccmCopy);
end;

class procedure TdxSpreadSheetTableViewClipboardHelper.Cut
  (AView: TdxSpreadSheetTableView);
begin
...
class function TdxSpreadSheetTableViewClipboardHelper.CanPaste
...
class procedure TdxSpreadSheetTableViewClipboardHelper.Paste
  (AView: TdxSpreadSheetTableView; const AClipboardArea: TRect;
...
begin
  if ReadDataFromClipboard(AOptions, AData) then
    PasteData(AView, AData, AClipboardArea, AClipboardCopyMode, AViewGUID,
      AOptions);
end;
...
begin
  ...
  AArea := cxRect(0, 0, dxSpreadSheetAreaWidth(AClipboardArea) - 1,
    dxSpreadSheetAreaHeight(AClipboardArea) - 1);
  ASelection := TdxSpreadSheetTableViewPasteSelection.Create;
  try
    ASelection.Store(AView, dxSpreadSheetAreaSize(AClipboardArea));
    APastePoints := PreparePastePoints(ASelection, AClipboardArea,
      AClipboardCopyMode);
    try
      ...
      begin
        ...
        AIsCannotChangePartOfArray := not TdxSpreadSheetTableViewAccess(AView)
          .CanModifyDataInArrayFormulaArea(ALocalArea, cmmClear);
  ...
  begin
    ASourceView := TdxSpreadSheetTableViewAccess
      (TdxCustomSpreadSheetAccess(AView.SpreadSheet).GetSheetByGUID(AViewGUID));
    if ASourceView <> nil then
    begin
      if ASourceView = AView then
        ASourceView.ClearCells(AClipboardArea, ALocalArea,
          dxSpreadSheetTableViewClearCellsOptionsAll)
      else
        ASourceView.ClearCells(AClipboardArea,
          dxSpreadSheetTableViewClearCellsOptionsAll);
    end;
...
class procedure TdxSpreadSheetTableViewClipboardHelper.GetDataInfoFromClipboard
  (var AViewGUID: string; var ACopyMode: TdxSpreadSheetClipboardCopyMode;
var AArea: TRect);
var
...
begin
  ...
  EnumFormats(procedure(AFormat: TdxSpreadSheetCustomClipboardFormatClass)
    begin
      ...
      begin
        if AFormat.InheritsFrom(TdxSpreadSheetNativeClipboardFormat) and
          AFormat.CanLoadFromClipboard then
          ANativeFormat := TdxSpreadSheetNativeClipboardFormatClass(AFormat);
      end;
    end);

  if (ANativeFormat = nil) or not ANativeFormat.GetInfoFromClipboard(AViewGUID,
    ACopyMode, AArea) then
  begin
    AArea := cxInvalidRect;
    ACopyMode := ccmNone;
    AViewGUID := '';
  ...
  try
    ...
    EnumFormats(procedure(AFormat: TdxSpreadSheetCustomClipboardFormatClass)
      var
        AData: IdxSpreadSheetClipboardData;
      begin
        AData := AFormat.Build(AView.Selection.Area,
          AView.Selection.FocusedContainer, AMode, AView);
        if AData <> nil then
