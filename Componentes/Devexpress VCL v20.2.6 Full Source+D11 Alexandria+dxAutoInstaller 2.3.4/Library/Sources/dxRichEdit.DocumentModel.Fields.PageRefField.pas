...
type
  ...
  public
    ...
    function FindBookmarkByName(ASourcePieceTable: TdxPieceTable;
      const ABookmarkName: string): TdxBookmark;
  ...
  TdxBookmarkResolutionService = class(TInterfacedObject,
    IdxBookmarkResolutionService)
  strict private
...
procedure TdxPageRefField.Initialize(APieceTable: TdxCustomPieceTable;
...
function TdxPageRefField.GetAllowedUpdateFieldTypes(const AOptions
  : TdxFieldUpdateOnLoadOptions): TdxUpdateFieldOperationTypes;
begin
  Result := [TdxUpdateFieldOperationType.Normal];
end;

procedure TdxPageRefField.BeforeCalculateFields(ASourcePieceTable
...
begin
  FTargetPageNumber := -1;
  ADocumentLayoutService :=
    ATable.DocumentModel.GetService<IdxDocumentLayoutService>;
  ...
  if ABookmark = nil then
    Exit;
  ALayoutPosition := TdxDocumentLayoutPosition
    (ADocumentLayout.CreateLayoutPosition(ABookmark.PieceTable,
    ABookmark.NormalizedStart, -1));
  try
...
function TdxPageRefField.FindBookmarkByName(ASourcePieceTable: TdxPieceTable;
  const ABookmarkName: string): TdxBookmark;
var
  AResolutionService: IdxBookmarkResolutionService;
begin
  AResolutionService := ASourcePieceTable.DocumentModel.
    GetService<IdxBookmarkResolutionService>;
...
function TdxPageRefField.GetCalculatedValueCore(ASourcePieceTable
...
function TdxBookmarkResolutionService.FindBookmarkByName(const AName: string)
  : TdxBookmark;
begin
