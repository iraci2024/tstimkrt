...
{$ENDIF}
  ...
  cxClasses, cxImageListEditor, ActnList, Dialogs, ExtDlgs, cxImageList,
  dxGDIPlusClasses;

type
  TcxImageListEditorFormInternalState = (eisSelectingTransparentColor);
  TcxImageListEditorFormInternalStates = set of
    TcxImageListEditorFormInternalState;
...
uses
  ...
  dxIconLibraryEditorHelpers, dxSmartImage;
...
begin
  ...
  begin
    ...
    lblManifestWarning.Caption :=
      'These images may be distorted if used in standard Windows UI controls with XPManifest enabled.';
...
begin
  ...
  if cbBackgroundFill.ItemIndex <> 0 then
    cbBackgroundFill.Text :=
      ColorToString(StringToColor(cbBackgroundFill.Text));
end;
...
procedure TcxImageListEditorForm.lvImagesDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := lvImages.GetItemAt(X, Y) <> nil;
end;

procedure TcxImageListEditorForm.lvImagesEndDrag(Sender, Target: TObject;
  X, Y: Integer);
var
...
procedure TcxImageListEditorForm.lvImagesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
{$ELSE}
    ASizeAsText := Format('%dx%d', [FImageListEditor.ImageWidth,
      FImageListEditor.ImageHeight]);
{$ENDIF}
    while True do
    begin
{$IFDEF DELPHIXE2}
...
procedure TcxImageListEditorForm.DrawFocusedItem(ACanvas: TCanvas;
  ARect: TRect);
begin
...
begin
  ...
  for I := 1 to FImageListEditor.ImageWidth - 1 do
    cxFillHalfToneRect(pbPreview.Canvas,
      cxRectSetLeft(R, MulDiv(I, pbPreview.Width - 1,
      FImageListEditor.ImageWidth), 1), $808080, $C0C0C0);
  for I := 1 to FImageListEditor.ImageHeight - 1 do
    cxFillHalfToneRect(pbPreview.Canvas,
      cxRectSetTop(R, MulDiv(I, pbPreview.Height - 1,
      FImageListEditor.ImageHeight), 1), $808080, $C0C0C0);
end;
...
function TcxImageListEditorForm.ExtractImagesSize(const ASizeAsText
  : string): TSize;
var
  APosition: Integer;
begin
  APosition := Pos('x', ASizeAsText);
  if (APosition = 0) or not TryStrToInt(Trim(Copy(ASizeAsText, 1, APosition - 1)
    ), Result.cx) or (Result.cx <= 0) then
    raise EInvalidArgument.Create(sMsgErrorInvalidImageWidth);
  if (APosition = 0) or not TryStrToInt
    (Trim(Copy(ASizeAsText, APosition + 1, MaxInt)), Result.cy) or
    (Result.cy <= 0) then
...
procedure TcxImageListEditorForm.AddImagesFromDXGallery
...
begin
  ...
  begin
    ...
    try
      if AImageCollectionEditor.Execute(AFiles,
        cxSize(FImageListEditor.ImageWidth, FImageListEditor.ImageHeight)) then
...
begin
  FImageListEditor.ImportImages(FImportList.Objects[TMenuItem(Sender).Tag]
    as TCustomImageList);
end;
...
begin
  ...
  cbGridlines.Enabled := (FImageListEditor.ImageWidth <= 48) or
    (FImageListEditor.ImageWidth <= 48);
...
begin
  if not cxRectIsEqual(dxvEditorFormPosition, cxNullRect) then
    SetBounds(dxvEditorFormPosition.Left, dxvEditorFormPosition.Top,
      dxvEditorFormPosition.Right, dxvEditorFormPosition.Bottom);
end;
...
var
  ...
  ASizeIndex: Integer;
begin
  AImagesSizeDisplayText := Format('%dx%d', [FImageListEditor.ImageWidth,
    FImageListEditor.ImageHeight]);
...
procedure TcxImageListEditorForm.UpdateTransparentColorIndicator
  (AColor: TColor);
begin
...
begin
  actExportAsSVG.Visible := (FImageListEditor.GetSelectionCount = 1) and
    Supports(FocusedImageInfo.Image, IdxVectorImage);
end;
...
procedure TcxImageListEditorForm.actReplaceFromDXGalleryExecute
  (Sender: TObject);
begin
