...
const
  ...
  cxFilterCriteriaVersion3 = 3;
  // for internal use only. //#Store ItemLink Name.
  cxFilterCriteriaVersion4 = 4;
  // for internal use only. //#Store ItemLink ID and Expression.
  cxFilterCriteriaVersion5 = 5;
  // for internal use only. //#Store ItemLink Name and Expression.
  cxFilterCriteriaLastVersion = cxFilterCriteriaVersion5;
...
type
  ...
  TcxFilterOperatorKind = (foEqual, foNotEqual, foLess, foLessEqual, foGreater,
    foGreaterEqual, foLike, foNotLike, foBetween, foNotBetween, foInList,
    foNotInList, foYesterday, foToday, foTomorrow, foLast7Days, foLastWeek,
    foLast14Days, foLastTwoWeeks, foLast30Days, foLastMonth, foLastYear,
    foInPast, foThisWeek, foThisMonth, foThisYear, foNext7Days, foNextWeek,
    foNext14Days, foNextTwoWeeks, foNext30Days, foNextMonth, foNextYear,
    foInFuture, foContains, foNotContains, foBeginsWith, foEndsWith);
  ...
  protected
    ...
    function PrepareExpressionValue(const AValue: Variant;
      var DisplayValue: string): Boolean; virtual;
  ...
  protected
    ...
    function GetEscapeWildard: Char; virtual;
      deprecated 'use GetEscapeWildcard instead';
  ...
  protected
    ...
    procedure PrepareDisplayValue(var DisplayValue: string); override;
    function PrepareExpressionValue(const AValue: Variant;
      var DisplayValue: string): Boolean; override;
  public
    function CompareValues(const AValue1, AValue2: Variant): Boolean; override;
    function DisplayText: string; override;
    function FilterText: string; override;
  ...
  public
    constructor Create(AOwner: TcxFilterCriteriaItemList;
      ABoolOperatorKind: TcxFilterBoolOperatorKind); virtual;
    destructor Destroy; override;

    function AddExpressionItem(AItemLink: TObject;
      AOperatorKind: TcxFilterOperatorKind; const AExpression: string;
      const ADisplayValue: string): TcxFilterCriteriaItem;
    function AddItem(AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AValue: Variant; const ADisplayValue: string)
      : TcxFilterCriteriaItem;
    function AddItemList(ABoolOperatorKind: TcxFilterBoolOperatorKind)
      : TcxFilterCriteriaItemList;
    procedure Clear;
    function IsEmpty: Boolean; override;
    property BoolOperatorKind: TcxFilterBoolOperatorKind read FBoolOperatorKind
      write SetBoolOperatorKind default fboAnd;
    property Count: Integer read GetCount;
    property Criteria: TcxFilterCriteria read GetCriteria;
    property Items[Index: Integer]: TcxCustomFilterCriteriaItem
  ...
  protected
    ...
    function GetComparedValue(AData: TObject; out AHasError: Boolean)
      : Variant; virtual;
    ...
    function GetFilterOperatorClass(AKind: TcxFilterOperatorKind)
      : TcxFilterOperatorClass; overload;
    function GetItemLink: TObject; virtual;
    procedure SetItemLink(Value: TObject); virtual;
    function GetIsItemList: Boolean; override;
    function IsOperatorNull(AOperatorKind: TcxFilterOperatorKind)
      : Boolean; overload;
    procedure Prepare;
    ...
    procedure WriteExpression(AStream: TStream;
      const AExpression: string); virtual;
  public
    ...
    property ItemLink: TObject read GetItemLink; property
    Operator: TcxFilterOperator read FOperator;
    property OperatorKind: TcxFilterOperatorKind read FOperatorKind
      write SetOperatorKind;
  ...
  TcxUserFilteringEvent = procedure(Sender: TObject; const AValue: Variant;
    const ADisplayText: string) of object;
  TcxUserFilteringExEvent = procedure(Sender: TObject;
    AFilterList: TcxFilterCriteriaItemList; const AValue: Variant;
    const ADisplayText: string) of object;
  TcxGetFilterDisplayTextEvent = procedure(Sender: TObject;
    const AValue: Variant; var ADisplayText: string) of object;
  TcxGetFilterValuesEvent = procedure(Sender: TObject;
    AValueList: TcxFilterValueList) of object;
  ...
  protected
    ...
    property PreparedValue: Variant read FPreparedValue;
  public
    constructor Create(AOwner: TcxFilterValueList;
      AKind: TcxFilterValueItemKind; const AValue: Variant;
      const ADisplayText: string);
    function ComparePreparedValue(const APreparedValue: Variant;
  ...
  protected
    procedure Add(AKind: TcxFilterValueItemKind; const AValue: Variant;
    ...
    function CreateItem(AKind: TcxFilterValueItemKind; const AValue: Variant;
      const ADisplayText: string): TcxFilterValueItem;
    function FindValueIndex(const APreparedValue: Variant;
      const APreparedDisplayText: string; out AIndex: Integer): Boolean;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Add(AKind: TcxFilterValueItemKind; const AValue: Variant;
      const ADisplayText: string; ANoSorting: Boolean); overload; virtual;
    procedure Clear; virtual;
    procedure Delete(AIndex: Integer);
    function Find(const AValue: Variant; const ADisplayText: string;
      var AIndex: Integer): Boolean; virtual;
    function FindItemByKind(AKind: TcxFilterValueItemKind): Integer; overload;
    function FindItemByKind(AKind: TcxFilterValueItemKind;
    ...
    property SortByDisplayText: Boolean read FSortByDisplayText
      write FSortByDisplayText;
  ...
  protected
    ...
    procedure FormatFilterTextValue(AItem: TcxFilterCriteriaItem;
      const AValue: Variant; var ADisplayValue: string); virtual;
    ...
    function GetItemExpression(AItem: TcxFilterCriteriaItem;
      AIsCaption: Boolean): string; virtual;
    function GetItemExpressionFieldName(AItem: TcxFilterCriteriaItem;
      AIsCaption: Boolean): string; virtual;
    function GetItemExpressionOperator(AItem: TcxFilterCriteriaItem;
      AIsCaption: Boolean): string; virtual;
    function GetItemExpressionValue(AItem: TcxFilterCriteriaItem;
      AIsCaption: Boolean): string; virtual;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent; AIgnoreItemNames: Boolean = False);
      reintroduce; virtual;
    procedure AssignEvents(Source: TPersistent); virtual;
    procedure AssignItems(ASource: TcxFilterCriteria;
      AIgnoreItemNames: Boolean = False); virtual;
    function AddExpressionItem(AParent: TcxFilterCriteriaItemList;
      AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AExpression: string; const ADisplayValue: string)
    ...
    procedure Changed; virtual;
    procedure EndUpdate;
    function EqualItems(AFilterCriteria: TcxFilterCriteria;
    ...
    property StoreItemLinkNames: Boolean read GetStoreItemLinkNames
      write SetStoreItemLinkNames;
    property SupportedLike: Boolean read FSupportedLike write FSupportedLike
      default True;
    property TranslateBetween: Boolean read FTranslateBetween
      write FTranslateBetween default False;
    property TranslateIn: Boolean read FTranslateIn write FTranslateIn
      default False;
    property TranslateLike: Boolean read FTranslateLike write FTranslateLike
      default False;
  published
    property MaxValueListCount: Integer read FMaxValueListCount
      write FMaxValueListCount default 0;
    property Options: TcxFilterCriteriaOptions read GetOptions write SetOptions
      default [];
    property PercentWildcard: Char read FPercentWildcard
      write SetPercentWildcard default '%';
    property UnderscoreWildcard: Char read FUnderscoreWildcard
      write SetUnderscoreWildcard default '_';
    property OnChanged: TNotifyEvent read FOnChanged write FOnChanged;
  end;

function ExtractFilterDisplayValue(const AValues: string;
...
function IsOperatorSupportedForExpression(AOperator: TcxFilterOperatorKind)
...
const
  cxFilterNullDate = 0;
  // it is safe because we do not use such past dates for the property values
...
begin
  I := Pos;
  while (I <= Length(AValues)) and (AValues[I] <> ';') do
    Inc(I);
  Result := Trim(Copy(AValues, Pos, I - Pos));
  if (I <= Length(AValues)) and (AValues[I] = ';') then
    Inc(I);
  Pos := I;
end;
...
begin
  Result := (AVersion = cxFilterCriteriaVersion3) or
    (AVersion >= cxFilterCriteriaVersion5);
end;
...
function TcxFilterOperator.GetFilterExpressionText(const AValue
  : Variant): string;
begin
  Result := GetFilterExpressionValue(AValue);
end;

function TcxFilterOperator.GetFilterExpressionValue(const AValue
  : Variant): string;
var
  AVarType: Integer;
begin
...
function TcxFilterOperator.PrepareExpressionValue(const AValue: Variant;
...
function TcxFilterEqualOperator.CompareValues(const AValue1,
...
function TcxFilterEqualOperator.GetFilterExpressionText
...
begin
  ...
  APos := 1;
  Result := '(' + ExtractFieldName(AFieldNames, APos) + ' = ' +
    GetFilterExpressionValue(AValue[0]) + ')';
  for I := VarArrayLowBound(AValue, 1) + 1 to VarArrayHighBound(AValue, 1) do
    Result := Result + ' AND ' + '(' + ExtractFieldName(AFieldNames, APos) +
      ' = ' + GetFilterExpressionValue(AValue[I]) + ')';
end;

{ TcxFilterNotEqualOperator }

function TcxFilterNotEqualOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
...
function TcxFilterNotEqualOperator.GetFilterExpressionText
  (const AValue: Variant): string;
begin
...
function TcxFilterLessOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := VarCompare(AValue1, AValue2) < 0;
end;
...
function TcxFilterLessEqualOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := VarCompare(AValue1, AValue2) <= 0;
end;
...
function TcxFilterGreaterOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := VarCompare(AValue1, AValue2) > 0;
end;
...
function TcxFilterGreaterEqualOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := VarCompare(AValue1, AValue2) >= 0;
end;
...
function TcxFilterCustomLikeOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := LikeStr(PrepareString(AValue1), PrepareFilterString(AValue2),
    GetPercentWildcard, GetUnderscoreWildcard, GetEscapeWildcard);
end;
...
function TcxFilterCustomLikeOperator.GetFilterExpressionText
  (const AValue: Variant): string;
begin
  Result := CriteriaItem.GetFieldName + ' ' + GetExpressionOperatorText + ' ' +
    GetFilterExpressionValue(PreparePatternString(AValue));
end;
...
function TcxFilterCustomLikeOperator.PrepareEscapedString
...
function TcxFilterCustomLikeOperator.PrepareFilterString
...
function TcxFilterCustomLikeOperator.PreparePatternString
  (AValue: Variant): string;
begin
  Result := PrepareString(AValue);
end;
...
function TcxFilterNotLikeOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := not inherited CompareValues(AValue1, AValue2);
end;
...
function TcxFilterNotLikeOperator.GetFilterExpressionText
  (const AValue: Variant): string;
begin
  Result := ' NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
begin
  Result := CriteriaItem.Criteria.PercentWildcard +
    inherited PrepareFilterString(AValue) +
    CriteriaItem.Criteria.PercentWildcard
end;

function TcxFilterContainsOperator.PreparePatternString
  (AValue: Variant): string;
begin
...
function TcxFilterNotContainsOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := not inherited CompareValues(AValue1, AValue2);
end;
...
function TcxFilterNotContainsOperator.GetFilterExpressionText
  (const AValue: Variant): string;
begin
...
function TcxFilterBeginsWithOperator.PrepareFilterString
  (AValue: Variant): string;
begin
  Result := inherited PrepareFilterString(AValue) +
    CriteriaItem.Criteria.PercentWildcard;
end;

function TcxFilterBeginsWithOperator.PreparePatternString
  (AValue: Variant): string;
begin
  Result := inherited PreparePatternString(AValue);
  if CriteriaItem.Criteria.FilterTextUsed then
...
function TcxFilterEndsWithOperator.PreparePatternString
  (AValue: Variant): string;
begin
  Result := inherited PreparePatternString(AValue);
  if CriteriaItem.Criteria.FilterTextUsed then
...
function TcxFilterBetweenOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Lock;
  try
    Result := (VarCompare(AValue2[0], AValue1) <= 0) and
      (VarCompare(AValue1, AValue2[1]) <= 0);
  finally
    Unlock;
  end;
end;
...
function TcxFilterBetweenOperator.GetFilterExpressionText
  (const AValue: Variant): string;
var
  AFieldName: string;
begin
  ...
  Result := '(' + AFieldName + ' >= ' + GetFilterExpressionValue(AValue[0]) +
    ') AND (' + AFieldName + ' <= ' + GetFilterExpressionValue(AValue[1]) + ')';
end;

procedure TcxFilterBetweenOperator.PrepareDisplayValue(var DisplayValue
  : string);
var
  S: string;
begin
...
begin
  ...
  if Result then
    DisplayValue := GetFilterExpressionValue(AValue[0]) + ' AND ' +
      GetFilterExpressionValue(AValue[1]);
end;

{ TcxFilterNotBetweenOperator }

function TcxFilterNotBetweenOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := not inherited CompareValues(AValue1, AValue2);
end;
...
function TcxFilterNotBetweenOperator.GetFilterExpressionText
  (const AValue: Variant): string;
begin
  Result := 'NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
function TcxFilterInListOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
var
  I: Integer;
begin
...
function TcxFilterInListOperator.GetFilterExpressionText
  (const AValue: Variant): string;
var
  AFieldName: string;
  I: Integer;
begin
  AFieldName := CriteriaItem.GetFieldName;
  Result := '(' + AFieldName + ' = ' + GetFilterExpressionValue
    (AValue[0]) + ')';
  for I := VarArrayLowBound(AValue, 1) + 1 to VarArrayHighBound(AValue, 1) do
    Result := Result + ' OR ' + '(' + AFieldName + ' = ' +
      GetFilterExpressionValue(AValue[I]) + ')';
end;
...
function TcxFilterNotInListOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := not inherited CompareValues(AValue1, AValue2);
end;
...
function TcxFilterNotInListOperator.GetFilterExpressionText
  (const AValue: Variant): string;
begin
  Result := 'NOT (' + inherited GetFilterExpressionText(AValue) + ')';
end;
...
function TcxFilterNullOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
...
function TcxFilterNotNullOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  Result := not inherited CompareValues(AValue1, AValue2);
end;
...
function TcxFilterDateOperator.CompareValues(const AValue1,
  AValue2: Variant): Boolean;
begin
  if CriteriaItem.ValueIsNull(AValue1) then
    Result := False
  else
    Result := ((Date1 = cxFilterNullDate) or (Date1 <= AValue1)) and
...
function TcxFilterDateOperator.GetFilterExpressionText
  (const AValue: Variant): string;
var
  AFieldName: string;
begin
  ...
  begin
    if Result <> '' then
      Result := Result + ' AND ';
    Result := Result + '(' + AFieldName + ' < ' + GetFilterExpressionValue
      (Date2) + ')';
  end;
end;
...
constructor TcxCustomFilterCriteriaItem.Create
...
function TcxFilterCriteriaItemList.AddExpressionItem(AItemLink: TObject;
  AOperatorKind: TcxFilterOperatorKind; const AExpression: string;
  const ADisplayValue: string): TcxFilterCriteriaItem;
begin
...
function TcxFilterCriteriaItemList.AddItem(AItemLink: TObject;
  AOperatorKind: TcxFilterOperatorKind; const AValue: Variant;
  const ADisplayValue: string): TcxFilterCriteriaItem;
begin
  Result := Criteria.GetItemClass.Create(Self, AItemLink, AOperatorKind, AValue,
    ADisplayValue);
  FItems.Add(Result);
  Changed;
end;

function TcxFilterCriteriaItemList.AddItemList(ABoolOperatorKind
  : TcxFilterBoolOperatorKind): TcxFilterCriteriaItemList;
begin
...
procedure TcxFilterCriteriaItemList.RemoveItem
...
function TcxFilterCriteriaItemList.ReadItem(AStream: TStream)
  : TcxCustomFilterCriteriaItem;
var
  AIsItemList: Boolean;
begin
...
procedure TcxFilterCriteriaItemList.CheckFilterResult(var AResult: Boolean;
  out ADone: Boolean);
begin
...
function TcxFilterCriteriaItemList.GetItem(Index: Integer)
  : TcxCustomFilterCriteriaItem;
begin
...
procedure TcxFilterCriteriaItemList.SetBoolOperatorKind
  (Value: TcxFilterBoolOperatorKind);
begin
...
function TcxFilterCriteriaItem.GetComparedValue(AData: TObject;
...
begin
  ...
  function TcxFilterCriteriaItem.GetFilterExpressionValue
  ...
  function TcxFilterCriteriaItem.GetFilterOperatorClass
    (AKind: TcxFilterOperatorKind): TcxFilterOperatorClass;
  const
    AOperatorClasses: array [TcxFilterOperatorKind] of TcxFilterOperatorClass =
      (TcxFilterEqualOperator, TcxFilterNotEqualOperator, TcxFilterLessOperator,
      TcxFilterLessEqualOperator, TcxFilterGreaterOperator,
      TcxFilterGreaterEqualOperator, TcxFilterLikeOperator,
      TcxFilterNotLikeOperator, TcxFilterBetweenOperator,
      TcxFilterNotBetweenOperator, TcxFilterInListOperator,
      TcxFilterNotInListOperator, TcxFilterYesterdayOperator,
      TcxFilterTodayOperator, TcxFilterTomorrowOperator,
      TcxFilterLast7DaysOperator, TcxFilterLastWeekOperator,
      TcxFilterLast14DaysOperator, TcxFilterLastTwoWeeksOperator,
      TcxFilterLast30DaysOperator, TcxFilterLastMonthOperator,
      TcxFilterLastYearOperator, TcxFilterInPastOperator,
      TcxFilterThisWeekOperator, TcxFilterThisMonthOperator,
      TcxFilterThisYearOperator, TcxFilterNext7DaysOperator,
      TcxFilterNextWeekOperator, TcxFilterNext14DaysOperator,
      TcxFilterNextTwoWeeksOperator, TcxFilterNext30DaysOperator,
      TcxFilterNextMonthOperator, TcxFilterNextYearOperator,
      TcxFilterInFutureOperator, TcxFilterContainsOperator,
      TcxFilterNotContainsOperator, TcxFilterBeginsWithOperator,
      TcxFilterEndsWithOperator);
    ANullOperatorClasses: array [Boolean] of TcxFilterOperatorClass =
      (TcxFilterNullOperator, TcxFilterNotNullOperator);
    AExpressionOperatorClasses: array [Boolean] of TcxFilterOperatorClass =
      (TcxFilterEqualOperator, TcxFilterNotEqualOperator);
  begin
    if IsOperatorNull(AKind) then
      Result := ANullOperatorClasses[AKind in [foNotEqual, foNotLike,
        foNotContains]]
    else if (Expression <> '') and not IsOperatorSupportedForExpression(AKind)
    then
      Result := AExpressionOperatorClasses
        [OperatorKind in [foNotBetween, foNotInList]]
    else
  ...
  function TcxFilterCriteriaItem.IsOperatorNull(AOperatorKind
    : TcxFilterOperatorKind): Boolean;
  begin
    Result := (AOperatorKind in [foEqual, foNotEqual, foLike, foNotLike,
      foContains .. foEndsWith]) and ValueIsNull(Value) and (Expression = '');
  end;
  ...
  function TcxFilterCriteriaItem.ReadExpression(AStream: TStream;
  ...
  begin
    ...
    begin
      ...
      if VersionContainsItemLinkNames(Criteria.SavedVersion) then
        if Criteria.IsUnicode then
          ...
          WriteAnsiStringProc(AStream,
            dxStringToAnsiString(Criteria.GetNameByItemLink(ItemLink)));
      WriteVariantProc(AStream, Value);
      if VersionContainsExpressions(Criteria.SavedVersion) then
        WriteExpression(AStream, Expression);
    end;

    procedure TcxFilterCriteriaItem.WriteExpression(AStream: TStream;
    ...
    procedure TcxFilterCriteriaItem.SetOperatorKind
    ...
    begin
      try
        ...
        if (fcoIgnoreNull in Criteria.Options) and not IsOperatorNull and
          ValueIsNull(AValue) then
          Result := False
        else
        begin
          AComparedValue := GetComparedValue(AData, AHasError);
          Result := not AHasError and Operator.CompareValues(AValue,
            AComparedValue);
        end;
      except
        on EVariantError do
    ...
    begin
      if Criteria.IsExpressionsSupported and not AnsiSameText(AValue, Expression)
      then
      begin
        FExpression := AValue;
        ExpressionChanged;
    ...
    function TcxFilterValueItem.ComparePreparedValue(const APreparedValue
      : Variant; const APreparedDisplayText: string): Integer;
    begin
    ...
    constructor TcxFilterMRUValueItem.Create(const AValue: Variant;
      const ADisplayText: string);
    begin
      inherited Create;
      Value := AValue;
    ...
    function TcxFilterMRUValueItems.GetItem(Index: Integer)
      : TcxFilterMRUValueItem;
    begin
      Result := TcxFilterMRUValueItem(inherited Items[Index]);
    end;

    procedure TcxFilterMRUValueItems.Add(const AValue: Variant;
      const ADisplayText: string);
    begin
    ...
    function TcxFilterValueList.Find(const AValue: Variant;
      const ADisplayText: string; var AIndex: Integer): Boolean;
    var
    ...
    function TcxFilterValueList.FindItemByKind
      (AKind: TcxFilterValueItemKind): Integer;
    begin
    ...
    function TcxFilterValueList.GetIndexByCriteriaItem(ACriteriaItem
      : TcxFilterCriteriaItem): Integer;
    begin
      if ACriteriaItem = nil then
        Result := FindItemByKind(fviAll)
      else if ACriteriaItem.ValueIsNull(ACriteriaItem.Value) and
      ...
      else if not((ACriteriaItem.OperatorKind = foEqual) and
    ...
    function TcxFilterValueList.FindValueIndex(const APreparedValue: Variant;
      const APreparedDisplayText: string; out AIndex: Integer): Boolean;
    var
    ...
    procedure TcxFilterValueList.Add(AKind: TcxFilterValueItemKind;
      ...
      procedure InsertItem(AIndex: Integer; AKind: TcxFilterValueItemKind;
    ...
    begin
      ...
      else if AKind <> fviValue then
        AIndex := FStartValueIndex
      else if ANoSorting then
      begin
        AIndex := Count;
        FIsUnsortedValues := True;
      end
      else if ((MaxCount = 0) or (Count < MaxCount)) and
        Find(AValue, ADisplayText, AIndex) and AUniqueOnly then
        AIndex := -1;
    ...
    function TcxFilterValueList.CompareItem(AIndex: Integer;
      const APreparedValue: Variant;
      const APreparedDisplayText: string): Integer;
    begin
      Result := Items[AIndex].ComparePreparedValue(APreparedValue,
        APreparedDisplayText);
    end;
    ...
    begin
      ...
      begin
        BeginUpdate;
        try
          CompareByDisplayValues := TcxFilterCriteria(Source)
            .CompareByDisplayValues;
    ...
    var
      ...
      ASourceVersion: Byte;
    begin
      if EqualItems(ASource, AIgnoreItemNames) then
        Exit;
      BeginUpdate;
      try
        Clear;
    ...
    function TcxFilterCriteria.AddExpressionItem
      (AParent: TcxFilterCriteriaItemList; AItemLink: TObject;
      AOperatorKind: TcxFilterOperatorKind; const AExpression: string;
      const ADisplayValue: string): TcxFilterCriteriaItem;
    begin
      if AParent = nil then
        AParent := Root;
      Result := AParent.AddExpressionItem(AItemLink, AOperatorKind, AExpression,
        ADisplayValue);
    end;

    function TcxFilterCriteria.AddItem(AParent: TcxFilterCriteriaItemList;
      AItemLink: TObject; AOperatorKind: TcxFilterOperatorKind;
      const AValue: Variant; const ADisplayValue: string)
      : TcxFilterCriteriaItem;
    begin
      if AParent = nil then
        AParent := Root;
      Result := AParent.AddItem(AItemLink, AOperatorKind, AValue,
        ADisplayValue);
    end;
    ...
    begin
      if AIgnoreItemNames then
        ...
        AMaxVersion := cxFilterCriteriaVersion5;
    ...
    function TcxFilterCriteria.FindItemByItemLink(AItemLink: TObject)
      : TcxFilterCriteriaItem;
      ...
      begin
        ...
        begin
          ...
          else if TcxFilterCriteriaItem(AItem).FItemLink = AItemLink then
          // TODO: !!!ItemLink
          begin
    ...
    begin
      case AVersionID of
        - 1:
          Result := cxFilterCriteriaVersion2;
        -2:
          Result := cxFilterCriteriaVersion3;
        -3:
          Result := cxFilterCriteriaVersion4;
        -4:
          Result := cxFilterCriteriaVersion5;
      else
    ...
    begin
      case AVersion of
        cxFilterCriteriaVersion2:
          Result := -1;
        cxFilterCriteriaVersion3:
          Result := -2;
        cxFilterCriteriaVersion4:
          Result := -3;
        cxFilterCriteriaVersion5:
          Result := -4;
      else
        Result := 0; // undefined
      end;
    ...
    procedure TcxFilterCriteria.FormatFilterTextValue
    ...
    function TcxFilterCriteria.GetFilterExpression(AIsCaption: Boolean): string;
      ...
      begin
        ...
        begin
          if ABranch[I].IsItemList then
            Result := Result + '(' + FilterSubString
              (TcxFilterCriteriaItemList(ABranch[I])) + ')'
          else
            Result := Result + '(' + GetItemExpression
              (TcxFilterCriteriaItem(ABranch[I]), AIsCaption) + ')';
          if I < ABranch.Count - 1 then
            Result := Result + ' ' + GetOperatorText(AIsCaption,
              ABranch.BoolOperatorKind) + ' ';
        end;
    ...
    function TcxFilterCriteria.GetItemExpressionFieldName
      (AItem: TcxFilterCriteriaItem; AIsCaption: Boolean): string;
    begin
      if AIsCaption then
    ...
    function TcxFilterCriteria.GetItemExpressionOperator
      (AItem: TcxFilterCriteriaItem; AIsCaption: Boolean): string;
    begin
      if AIsCaption and (AItem.Operator.IsDescription or
        (fcoShowOperatorDescription in Options)) then
        Result := AItem.Operator.DisplayText
      else
        Result := AItem.Operator.FilterText;
    end;

    function TcxFilterCriteria.GetItemExpressionValue
      (AItem: TcxFilterCriteriaItem; AIsCaption: Boolean): string;
    begin
    ...
    begin
      Result := FSavingToStream or (Root.BoolOperatorKind <> fboAnd) or
        not Root.IsEmpty;
    end;
    ...
    function TcxFilterCriteria.PrepareDisplayText(const ADisplayText
      : string): string;
    begin
    ...
    begin
      ...
      else if VarIsStr(AValue) and (fcoCaseInsensitive in Options) then
      ...
      begin
        ...
        begin
          Lock;
          try
            for I := VarArrayLowBound(Result, 1) to VarArrayHighBound
              (Result, 1) do
    ...
    function TcxFilterCriteria.SupportsMultiThreading: Boolean;
      ...
      procedure CheckBranch(ABranch: TcxFilterCriteriaItemList;
        var ASupports: Boolean);
      var
        I: Integer;
      begin
        for I := 0 to ABranch.Count - 1 do
        begin
          ...
          if ABranch[I].IsItemList then
            ...
            ASupports := TcxFilterCriteriaItem(ABranch[I])
    ...
    begin
      if StoreItemLinkNames <> Value then
        if Value then
          Version := cxFilterCriteriaVersion5
        else
