...
type
  ...
  protected
    ...
    function CalculateViewport(AZoomLevel: Double;
      ACenterPoint: TdxMapControlGeoPoint; AViewportInPixels: TSize)
      : TdxRectDouble;
    ...
    function GetMapSizeInPixels(AZoomLevel: Double): TdxSizeDouble;
      virtual; abstract;
  ...
  public
    ...
    function Move(ACenterPoint: TdxMapControlGeoPoint; AOffset: TPoint)
      : TdxMapControlGeoPoint; virtual; // for internal use
    function MoveAndZoom(ACenterPoint: TdxMapControlGeoPoint;
      AAnchorPoint: TdxPointDouble; ACurrentZoomLevel, ANewZoomLevel: Double)
      : TdxMapControlGeoPoint; virtual; // for internal use
    procedure UpdateViewport; virtual; // for internal use

    function GetKilometersScale(const ACenterPoint: TdxMapControlGeoPoint;
      AScreenDistance: Double): Double; // for internal use
    function GeoPointToMapUnit(const APoint: TdxMapControlGeoPoint;
      AShouldNormalize: Boolean = True): TdxPointDouble;
    function GeoPointToScreenPoint(const APoint: TdxMapControlGeoPoint)
      : TdxPointDouble;
    function GeoToKilometersSize(const AAnchorPoint: TdxMapControlGeoPoint;
      const ASize: TdxSizeDouble): TdxSizeDouble; // for internal use
    function KilometersToGeoSize(const AAnchorPoint: TdxMapControlGeoPoint;
      const ASize: TdxSizeDouble): TdxSizeDouble; // for internal use
    function MapUnitToScreenPoint(const APoint: TdxPointDouble): TdxPointDouble;
    function ScreenPointToGeoPoint(const APoint: TdxPointDouble)
  ...
  public
    ...
    property Items[Index: Integer]: TdxMapLayer read GetItem
...
begin
  Result := Visible and not(GetParentComponent as TdxCustomMapControl)
    .IsDesigning and (GetMapProjection <> nil);
end;

function TdxMapLayer.Move(ACenterPoint: TdxMapControlGeoPoint; AOffset: TPoint)
  : TdxMapControlGeoPoint;
var
  AMapPoint: TdxPointDouble;
begin
  AMapPoint := MapUnitToScreenPoint(GeoPointToMapUnit(ACenterPoint, False));
  Result := ScreenPointToGeoPoint(dxPointDouble(AMapPoint.X + AOffset.X,
    AMapPoint.Y + AOffset.Y));
end;

function TdxMapLayer.MoveAndZoom(ACenterPoint: TdxMapControlGeoPoint;
  AAnchorPoint: TdxPointDouble; ACurrentZoomLevel, ANewZoomLevel: Double)
  : TdxMapControlGeoPoint;
var
...
begin
  ...
  ACenter := GetMapUnitConverter.GeoPointToMapUnit(ACenterPoint,
    GetMapProjection);
  AAnchor := GetMapUnitConverter.ScreenPointToMapUnit(AAnchorPoint, ViewPort,
    ViewportInPixels);
  ANewCenter := dxPointDouble(AAnchor.X + (ACenter.X - AAnchor.X) /
    AFactorWidth, AAnchor.Y + (ACenter.Y - AAnchor.Y) / AFactorHeight);
...
function TdxMapLayer.GetKilometersScale(const ACenterPoint
...
begin
  ...
  AScaleGeoPoint := ScreenPointToGeoPoint
    (dxPointDouble(AScreenCenterPoint.X - AHemisphereMultiplier *
    AScreenDistance, AScreenCenterPoint.Y));
  Result := GeoToKilometersSize(ACenterPoint,
    dxSizeDouble(Abs(AScaleGeoPoint.Longitude - ACenterPoint.Longitude),
    0.0)).Width;
end;

function TdxMapLayer.GeoPointToMapUnit(const APoint: TdxMapControlGeoPoint;
  AShouldNormalize: Boolean = True): TdxPointDouble;
begin
  Result := GetMapUnitConverter.GeoPointToMapUnit(APoint, GetMapProjection,
    AShouldNormalize);
end;

function TdxMapLayer.GeoPointToScreenPoint(const APoint: TdxMapControlGeoPoint)
  : TdxPointDouble;
begin
  Result := MapUnitToScreenPoint(GeoPointToMapUnit(APoint));
end;

function TdxMapLayer.GeoToKilometersSize(const AAnchorPoint
  : TdxMapControlGeoPoint; const ASize: TdxSizeDouble): TdxSizeDouble;
begin
  Result := GetMapProjection.GeoToKilometersSize(AAnchorPoint, ASize);
end;

function TdxMapLayer.KilometersToGeoSize(const AAnchorPoint
  : TdxMapControlGeoPoint; const ASize: TdxSizeDouble): TdxSizeDouble;
begin
  Result := GetMapProjection.KilometersToGeoSize(AAnchorPoint, ASize);
end;

function TdxMapLayer.MapUnitToScreenPoint(const APoint: TdxPointDouble)
  : TdxPointDouble;
begin
  Result := dxPointDoubleOffset(dxPointDoubleScale(APoint, FRenderScale),
    FRenderOffset, False);
end;

function TdxMapLayer.ScreenPointToGeoPoint(const APoint: TdxPointDouble)
  : TdxMapControlGeoPoint;
begin
  Result := GetMapUnitConverter.MapUnitToGeoPoint(ScreenPointToMapUnit(APoint),
    GetMapProjection);
end;

function TdxMapLayer.ScreenPointToMapUnit(const APoint: TdxPointDouble)
  : TdxPointDouble;
begin
  Result := GetMapUnitConverter.ScreenPointToMapUnit(APoint, ViewPort,
    ViewportInPixels);
end;

function TdxMapLayer.GetCollectionFromParent(AParent: TComponent)
...
function TdxMapLayer.CalculateViewport(AZoomLevel: Double;
  ACenterPoint: TdxMapControlGeoPoint; AViewportInPixels: TSize): TdxRectDouble;
var
...
begin
  ...
  ACenter := GetMapUnitConverter.GeoPointToMapUnit(ACenterPoint,
    GetMapProjection);
  Result := dxRectDouble(ACenter.X - AWidth * 0.5, ACenter.Y - AHeight * 0.5,
    AWidth, AHeight);
end;
...
begin
  inherited;
  if (Collection <> nil) and dxGetCurrentScaleFactor(Collection.ParentComponent,
    ANumerator, ADenominator) then
    ChangeScale(ANumerator, ADenominator);
end;

procedure TdxMapLayer.SetViewPort(const Value: TdxRectDouble);
begin
...
function TdxMapLayersViewInfo.GetItems(Index: Integer): TdxMapLayerViewInfo;
begin
...
procedure TdxMapLayers.SetItemName(AItem: TcxComponentCollectionItem;
