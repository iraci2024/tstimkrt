...
type
  ...
  strict private
    ...
    class procedure ParagraphAlignmentKeyword(AImporter: TdxCustomHtmlImporter;
  ...
  protected
    procedure ApplyTagProperties; override;
    procedure ResetParagraphFormattingProperties(AParagraphFormatting
      : TdxParagraphFormattingBase);
  public
    constructor Create(AImporter: TdxCustomHtmlImporter; AFontSize: Integer;
      AOutlineLevel: Integer); reintroduce; virtual;
  end;
  ...
  protected
    procedure ApplyTagProperties; override;
  public
    procedure AppendContent(const AText: string; AUseRawText: Boolean);
      override;
    function DecodeStringContent(const AContentText: string;
      out ADecodeContentText: string): Boolean; override;
  end;
...
begin
  ...
  FDefaultSpacing := AImporter.UnitConverter.PointsToModelUnits
    (DefaultSpacingInPoints);
  InitializeAlignment;
end;
...
class function TdxParagraphTagBase.AddAttributes
...
class procedure TdxParagraphTagBase.ParagraphAlignmentKeyword
...
function TdxParagraphTagBase.GetAttributeTable
  : TdxAttributeKeywordTranslatorTable;
begin
...
begin
  ...
  if Importer.Position.DefaultAlignment.UseAlignment then
    FAlignment.AlignmentValue := Importer.Position.DefaultAlignment.
      AlignmentValue;
end;
...
begin
  ...
  begin
    if AOptions.UseLeftIndent then
      ...
      ADelta := 0;
    Importer.Position.ParagraphFormatting.LeftIndent := Importer.TagsStack
...
constructor TdxHeadingTag.Create(AImporter: TdxCustomHtmlImporter;
  AFontSize: Integer; AOutlineLevel: Integer);
begin
  inherited Create(AImporter);
  FFontSize := AFontSize;
...
begin
  ...
  ACharacterFormatting.DoubleFontSize := Importer.HtmlFontSize.GetDoubleFontSize
    (FFontSize);
...
procedure TdxHeadingTag.ResetParagraphFormattingProperties(AParagraphFormatting
  : TdxParagraphFormattingBase);
begin
...
begin
  AParagraphProperties := Importer.PieceTable.Paragraphs[AParagraphIndex]
    .ParagraphProperties;
...
procedure TdxOfficeParagraphTag.AppendContent(const AText: string;
  AUseRawText: Boolean);
begin
...
function TdxOfficeParagraphTag.DecodeStringContent(const AContentText: string;
...
begin
  ...
  Importer.Position.ParagraphFormatting.CopyFrom
    (DocumentModel.DefaultParagraphProperties.Info);
...
procedure TdxPreformattedTag.BeforeDeleteTagFromStack(AIndexOfDeletedTag
  : Integer);
begin
...
function TdxSpanTag.DecodeStringContent(const AContentText: string;
  out ADecodeContentText: string): Boolean;
var
  AText, AStyleValue: string;
begin
  if FStyleAttributeProperties.HasRawProperties and
    FStyleAttributeProperties.RawProperties.TryGetValue('mso-spacerun',
    AStyleValue) then
  begin
    AText := TdxHtmlImporterAccess(Importer).ApplyEncoding(AContentText);
    if SameText(AStyleValue, 'yes') then
      ADecodeContentText := SysUtils.StringReplace(SysUtils.StringReplace(AText,
        '&nbsp;', ' ', [rfReplaceAll, rfIgnoreCase]),
        TdxCharacters.NonBreakingSpace, ' ', [rfReplaceAll])
    else
      ADecodeContentText := TdxHtmlImporterAccess(Importer)
        .ReplaceSpecialSymbols(SysUtils.StringReplace(AText, ' ',
        TdxCharacters.NonBreakingSpace, [rfReplaceAll, rfIgnoreCase]));
    Result := True;
  end
  else
    Result := False;
