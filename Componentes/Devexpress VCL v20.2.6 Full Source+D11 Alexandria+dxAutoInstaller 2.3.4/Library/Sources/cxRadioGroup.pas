...
{$ENDIF}
  ...
  dxCore, dxUxTheme, cxClasses, cxContainer, cxControls, cxDataStorage,
  cxGraphics, cxLookAndFeels,
  cxEdit, cxTextEdit, cxGroupBox, cxDropDownEdit, cxFilterControlUtils,
  dxFading, cxGeometry, dxCoreClasses;
...
type
  ...
  TcxRadioButton = class(TRadioButton, IdxFadingObject, IcxMouseTrackingCaller,
    IcxLookAndFeelContainer, IdxSkinSupport, IdxScaleFactor)
  private
    FAutoSize: Boolean;
    FCanvas: TcxCanvas;
    FChecked: Boolean;
...
{$ENDIF}
    function CanResize(var NewWidth: Integer; var NewHeight: Integer)
      : Boolean; override;
    procedure CheckStartFading(APrevState,
      AState: TcxRadioButtonState); virtual;
    ...
    procedure DrawRadioButton(ACanvas: TcxCanvas;
      AState: TcxRadioButtonState); virtual;
  ...
  published
    ...
    property AutoSize: Boolean read FAutoSize write SetRadioButtonAutoSize
      default False;
  ...
  public
    function GetGlyphRect(const AGlyphSize: TSize; AAlignment: TLeftRight;
      AIsPaintCopy: Boolean): TRect; override;
  end;
  ...
  protected
    procedure DrawButtonCaption(ACanvas: TcxCanvas;
      ...
      const AGlyphRect: TRect); override;
    ...
    function IsButtonGlyphTransparent(AButtonViewInfo
      : TcxGroupBoxButtonViewInfo): Boolean; override;
  public
    ItemIndex: Integer;
  ...
  published
    property Caption;
    property Value: TcxEditValue read FValue write SetValue
  ...
  public
    ...
    property Items[Index: Integer]: TcxRadioGroupItem read GetItem
  ...
  public
    ...
    property ActiveProperties: TcxCustomRadioGroupProperties
...
uses
  Math, ActnList, dxThemeConsts, dxThemeManager, cxVariants,
...
procedure TcxRadioButton.GetFadingImages(out AFadeOutImage,
...
begin
  Result := (LookAndFeel.SkinPainter <> nil) or dxFader.Contains(Self) or
    IsWinSevenOrLater;
end;

procedure TcxRadioButton.Notification(AComponent: TComponent;
  ...
  begin
    if Parent <> nil then
      ...
      begin
        ...
        begin
          if not(ASibling is TcxRadioButton) or
            (TcxRadioButton(ASibling).GroupIndex = GroupIndex) then
          begin
            if Assigned(TRadioButton(ASibling).Action) and
              (TRadioButton(ASibling).Action is TCustomAction) then
            begin
...
function TcxRadioButton.CanResize(var NewWidth: Integer;
...
procedure TcxRadioButton.CheckStartFading(APrevState,
  AState: TcxRadioButtonState);
begin
  if APrevState <> AState then
  begin
    if (APrevState = rbsNormal) and (AState = rbsHot) then
      dxFader.FadeIn(Self)
    else if (APrevState = rbsHot) and (AState = rbsNormal) then
...
procedure TcxRadioButton.DoContextPopup(MousePos: TPoint; var Handled: Boolean);
var
  P: TPoint;
begin
...
function GetTextRectCorrection(AIsInplace, ANativeStyle, AWordWrap: Boolean;
  AAlignment: TLeftRight): TRect;
const
  AInplaceTextRectCorrection: array [Boolean] of TRect = ((Left: 5; Top: 0;
    Right: 1; Bottom: 0), (Left: 3; Top: 0; Right: 0; Bottom: 0));
  AStandardTextRectCorrection: array [Boolean, TLeftRight]
    of TRect = (((Left: 2; Top: - 1; Right: 1; Bottom: 0), (Left: 5; Top: - 1;
    Right: 0; Bottom: 0)), ((Left: 2; Top: - 1; Right: 6; Bottom: 0), (Left: 5;
    Top: - 1; Right: 2; Bottom: 0)));
  ANativeStyleTextRectCorrection: array [Boolean, TLeftRight]
...
procedure CorrectRadioButtonTextRect(var R: TRect;
  AIsInplace, ANativeStyle, AWordWrap, AUseRightToLeftAlignment: Boolean;
  AAlignment: TLeftRight; AScaleFactor: TdxScaleFactor);
var
  AOffsets: TRect;
begin
  AOffsets := GetTextRectCorrection(AIsInplace, ANativeStyle, AWordWrap,
    AAlignment);
...
begin
  CorrectRadioButtonTextRect(R, IsInplace, ANativeStyle, WordWrap,
    UseRightToLeftAlignment, Alignment, ScaleFactor);
end;
...
procedure TcxRadioButton.Draw(ACanvas: TcxCanvas;
  ADrawOnlyFocusedState: Boolean);
begin
...
begin
  ...
  begin
    ...
    try
      ...
      if Text <> '' then
      begin
        AOffsets := ScaleFactor.Apply(GetTextRectCorrection(IsInplace,
          IsNativeStyle, WordWrap, Alignment));
        ARect := cxRectSetWidth(cxNullRect,
          Max(AWidth - cxRectWidth(ButtonRect) - AButtonOffset - AOffsets.Left -
          AOffsets.Right, 1));
        cxDrawText(Canvas.Handle, Text, ARect, GetTextFlags);
        if not(Align in [alTop, alBottom]) then
          AWidth := cxRectWidth(ARect) + cxRectWidth(ButtonRect) + AButtonOffset
            + AOffsets.Left + AOffsets.Right + 2;
        if not(Align in [alLeft, alRight]) then
          AHeight := Max(cxRectHeight(ARect), cxRectHeight(ButtonRect)) +
            ScaleFactor.Apply(4);
      end
      else
      begin
...
begin
  if HandleAllocated then
    UpdateState(cxmbNone, KeyboardStateToShiftState,
...
begin
  if not Enabled then
    ...
    State := rbsNormal
  else if GetCaptureControl = Self then // VCL only
    if PtInRect(GetControlRect(Self), P) then
      State := rbsPressed
    else
      State := rbsHot
  else if PtInRect(GetControlRect(Self), P) then
  begin
...
procedure TcxRadioButton.DrawRadioButton(ACanvas: TcxCanvas;
  AState: TcxRadioButtonState);
  ...
  begin
    if LookAndFeel.SkinPainter <> nil then
      Result := clNone
    else if IsTransparentBackground then
      Result := clDefault
    else
      Result := Color;
  end;
...
const
  ButtonStateMap: array [TcxRadioButtonState] of TcxButtonState = (cxbsDisabled,
    cxbsHot, cxbsNormal, cxbsPressed);
var
...
begin
  Result := GetRadioButtonRect(LookAndFeel.GetAvailablePainter(totButton)
    .ScaledRadioButtonSize(ScaleFactor), IsNativeStyle);
end;
...
function TcxRadioGroupButtonViewInfo.GetGlyphRect(const AGlyphSize: TSize;
...
begin
  ...
  CorrectRadioButtonTextRect(R, IsInplace, NativeStyle, False,
    UseRightToLeftAlignment, taRightJustify, ScaleFactor);
  ACanvas.Brush.Style := bsClear;
...
procedure TcxCustomRadioGroupViewInfo.DrawButtonGlyph(ACanvas: TcxCanvas;
  ...
  begin
    if UseSkins and not IsInplace then
      Result := clNone
    else if IsBackgroundTransparent then
      Result := clDefault
    else
...
const
  LookAndFeelKindMap: array [TcxEditButtonStyle] of TcxLookAndFeelKind =
    ...
    lfOffice11);
  ButtonStateMap: array [TcxEditButtonState] of TcxButtonState = (cxbsDisabled,
    cxbsNormal, cxbsPressed, cxbsHot);
var
  APainter: TcxCustomLookAndFeelPainter;
begin
  ...
  try
    ...
    if UseSkins then
      ...
      APainter := GetPainterClass(AButtonViewInfo.Data.NativeStyle,
        LookAndFeelKindMap[AButtonViewInfo.Data.Style]);

    APainter.DrawScaledRadioButton(ACanvas, AGlyphRect.Left, AGlyphRect.Top,
      ButtonStateMap[AButtonViewInfo.Data.State],
      ItemIndex = AButtonViewInfo.ButtonIndex, False, GetBackgroundColor,
      ScaleFactor, IsDesigning);
  finally
    ACanvas.RestoreClipRegion;
  end;
end;

function TcxCustomRadioGroupViewInfo.GetButtonViewInfoClass
  : TcxEditButtonViewInfoClass;
begin
  Result := TcxRadioGroupButtonViewInfo;
end;

function TcxCustomRadioGroupViewInfo.IsButtonGlyphTransparent(AButtonViewInfo
  : TcxGroupBoxButtonViewInfo): Boolean;
begin
  Result := IsBackgroundTransparent or AButtonViewInfo.Data.NativeStyle and
    IsThemeBackgroundPartiallyTransparent(ThemeHandle, BP_RADIOBUTTON,
...
begin
  ...
  TcxCustomRadioGroupViewInfo(AViewInfo).GlyphSize :=
    Style.LookAndFeel.GetAvailablePainter(totButton).ScaledRadioButtonSize
    (ScaleFactor);
...
const
  ButtonStateMap: array [Boolean, TcxEditButtonState] of Integer =
    ((RBS_UNCHECKEDDISABLED, RBS_UNCHECKEDNORMAL, RBS_UNCHECKEDPRESSED,
    RBS_UNCHECKEDHOT), (RBS_CHECKEDDISABLED, RBS_CHECKEDNORMAL,
    RBS_CHECKEDPRESSED, RBS_CHECKEDHOT));
begin
  ...
  AButtonViewInfo.Data.NativeState := ButtonStateMap
    [AButtonViewInfo.ButtonIndex = TcxCustomRadioGroupViewInfo
    (AButtonViewInfo.EditViewInfo).ItemIndex, AButtonViewInfo.Data.State];
end;

procedure TcxCustomRadioGroupViewData.EditValueToDrawValue(const AEditValue
...
function TcxCustomRadioGroupViewData.GetButtonNativeTheme(AButtonViewInfo
...
begin
  ...
  AMetrics.ButtonSize := Style.LookAndFeel.GetAvailablePainter(totButton)
    .ScaledRadioButtonSize(ScaleFactor);
  AMetrics.ColumnWidthCorrection :=
    ScaleFactor.Apply(AColumnWidthCorrectionA[ANativeStyle]);
  ...
  AMetrics.AutoHeightWidthCorrection :=
    ScaleFactor.Apply(Integer(IsInplace) - 6);
end;

function TcxCustomRadioGroupViewData.GetProperties
  : TcxCustomRadioGroupProperties;
begin
...
function TcxCustomRadioGroupProperties.CompareDisplayValues(const AEditValue1,
  AEditValue2: TcxEditValue): Boolean;
begin
  Result := GetRadioGroupItemIndex(AEditValue1) = GetRadioGroupItemIndex
    (AEditValue2);
end;

class function TcxCustomRadioGroupProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxRadioGroup;
end;

function TcxCustomRadioGroupProperties.GetDisplayText(const AEditValue
...
function TcxCustomRadioGroupProperties.GetRadioGroupItemIndex(const AEditValue
...
function TcxCustomRadioGroupProperties.GetSpecialFeatures
  : TcxEditSpecialFeatures;
begin
  Result := inherited GetSpecialFeatures + [esfMultiRow];
end;

function TcxCustomRadioGroupProperties.GetSupportedOperations
  : TcxEditSupportedOperations;
begin
  Result := inherited GetSupportedOperations + [esoHorzAlignment,
    esoSortingByDisplayText];
end;

class function TcxCustomRadioGroupProperties.GetViewInfoClass
...
procedure TcxCustomRadioGroupProperties.DoAssign(AProperties
...
class function TcxCustomRadioGroupProperties.GetViewDataClass
...
function TcxCustomRadioGroupProperties.GetButtonValue(AIndex: Integer)
  : TcxEditValue;
var
...
procedure TcxCustomRadioGroupProperties.SetDefaultValue
...
procedure TcxCustomRadioGroupButton.CorrectTextRect(var R: TRect;
  ANativeStyle: Boolean);
begin
...
begin
  ...
  begin
    ...
    if not Checked and not IsInplace and not FFocusingByMouse and DoEditing and
      not ClicksDisabled then
      Checked := True;
  end;
end;
...
begin
  ...
  if not(csDestroying in ComponentState) and
    (Message.FocusedWnd <> RadioGroup.Handle) then
...
class function TcxCustomRadioGroup.GetPropertiesClass
...
procedure TcxCustomRadioGroup.PrepareEditValue(const ADisplayValue
...
begin
  ...
  ControlStyle := ControlStyle + [csParentBackground, csCaptureMouse,
...
begin
  ...
  if Length(ViewInfo.ButtonsInfo) > 0 then
    for I := 0 to InternalButtons.Count - 1 do
    begin
      Buttons[I].LookAndFeel.Kind := AButtonLookAndFeelKinds
        [ViewInfo.ButtonsInfo[0].Data.Style];
...
begin
  ...
  try
    BeginUserAction;
    try
      PrepareEditValue(InternalButtons.IndexOf(AButton), AEditValue,
        InternalFocused);
      SetInternalValues(AEditValue, True, True);
    finally
      EndUserAction;
    end;
    if Focused and ActiveProperties.ImmediatePost and CanPostEditValue and InternalValidateEdit
    then
...
function TcxCustomRadioGroup.GetButton(Index: Integer)
  : TcxCustomRadioGroupButton;
begin
...
procedure TcxCustomRadioGroup.SetItemIndex(Value: Integer);
  ...
  begin
    if Value < -1 then
      Value := -1
    else if Value >= InternalButtons.Count then
...
procedure TcxCustomRadioGroup.SetProperties
  (Value: TcxCustomRadioGroupProperties);
begin
...
class procedure TcxFilterRadioGroupHelper.GetFilterValue(AEdit: TcxCustomEdit;
...
class procedure TcxFilterRadioGroupHelper.SetFilterValue(AEdit: TcxCustomEdit;
...
GetRegisteredEditProperties.Register(TcxRadioGroupProperties,
  scxSEditRepositoryRadioGroupItem);
FilterEditsController.Register(TcxRadioGroupProperties,
  TcxFilterRadioGroupHelper);

finalization

FilterEditsController.Unregister(TcxRadioGroupProperties,
  TcxFilterRadioGroupHelper);
