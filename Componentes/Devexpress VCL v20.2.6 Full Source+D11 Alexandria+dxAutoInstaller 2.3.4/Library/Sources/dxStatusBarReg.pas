...
type
  ...
  TdxStatusBarTextPanelImageIndexProperty = class(TIntegerProperty,
...
begin
  ShowCollectionEditor(Designer, Component, (Component as TdxStatusBar).Panels,
    'Panels');
end;
...
begin
  if HasSubProperties then
    Result := GetRegisteredStatusBarPanelStyles.GetDescriptionByClass
      (TdxStatusBarPanelStyle(GetOrdValue).ClassType)
  else
    Result := '';
end;
...
begin
  ...
  begin
    for I := 0 to PropCount - 1 do
      TdxStatusBarPanel(GetComponent(I)).PanelStyleClass :=
        FPanelStyleStyleClass;
    Modified;
  end;
end;

function TdxStatusBarPanelStyleProperty.CanAcceptPanelStyle
  (AClass: TClass): Boolean;
begin
  Result := TdxStatusBarPanel(GetComponent(0))
    .StatusBarControl.CanAcceptPanelStyle(TdxStatusBarPanelStyleClass(AClass));
end;
...
begin
  ...
  try
    EditorForm.Caption := TdxStatusBarStateIndicatorPanelStyle(GetComponent(0))
      .StatusBarControl.Name + '.IndicatorPanelStyle';
    EditorForm.Indicators.Assign(TdxStatusBarStateIndicatorPanelStyle
      (GetComponent(0)).Indicators);
...
function TdxStatusBarTextPanelImageIndexProperty.GetAttributes
  : TPropertyAttributes;
begin
...
procedure TdxStatusBarTextPanelImageIndexProperty.ListDrawValue
...
procedure TdxStatusBarTextPanelImageIndexProperty.ListMeasureHeight
...
procedure TdxStatusBarTextPanelImageIndexProperty.ListMeasureWidth
...
begin
  ...
  if GetComponent(0) is TdxStatusBarTextPanelStyle then
    Result := TImageList(TdxStatusBarTextPanelStyle(GetComponent(0))
      .StatusBarControl.Images);
end;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TdxStatusBarPanelStyle), TdxStatusBarPanel,
    'PanelStyle', TdxStatusBarPanelStyleProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TdxStatusBarPanel,
    'PanelStyleEvents', TdxStatusBarPanelStyleEventsProperty);
  RegisterPropertyEditor(TypeInfo(string), TdxStatusBarPanel,
    'PanelStyleClassName', nil);
  RegisterPropertyEditor(TypeInfo(TdxStatusBarContainerControl),
    TdxStatusBarContainerPanelStyle, 'Container', nil);
  RegisterPropertyEditor(TypeInfo(TdxStatusBarStateIndicators),
    TdxStatusBarPanelStyle, '', TdxStatusBarStateIndicatorsProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxStatusBarTextPanelStyle,
    'ImageIndex', TdxStatusBarTextPanelImageIndexProperty);
