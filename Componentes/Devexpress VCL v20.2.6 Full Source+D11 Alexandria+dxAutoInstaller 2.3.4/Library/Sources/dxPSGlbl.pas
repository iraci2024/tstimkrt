...
uses
  Classes, Windows, Messages, SysUtils, Graphics, Menus, Dialogs, ExtDlgs,
  IniFiles,
...
type
  ...
  TdxDriveType = (dtUnknown, dtNoRootDir, dtRemovable, dtFixed, dtRemote,
    dtCDROM, dtRamDisk);
  TdxPageOrder = (poOverThenDown, poDownThenOver);
  TdxPageNumberFormat = (pnfNumeral, pnfChars, pnfUpperChars, pnfRoman,
    pnfUpperRoman);
...
{$EXTERNALSYM HDF_SORTUP}
  ...
  dxFormatTextAlignXMask = dxFormatTextAlignXLeft or dxFormatTextAlignXCenter or
    dxFormatTextAlignXRight or dxFormatTextAlignXJustify or
    dxFormatTextAlignXDistribute;
  { Vertical Alignment }
  ...
  dxFormatTextAlignYMask = dxFormatTextAlignYTop or dxFormatTextAlignYCenter or
  ...
  dxFormatCheckButtonEdgeStyleMask = dxFormatCheckEdgeStyle3D or
    dxFormatCheckEdgeStyleSoft3D or dxFormatCheckEdgeStyleBoldFlat or
    dxFormatCheckEdgeStyleUltraFlat or dxFormatCheckEdgeStyleSingle;
  ...
  dxFormatImageLayoutMask = dxFormatImageLayoutTopLeft or
    dxFormatImageLayoutTopCenter or dxFormatImageLayoutTopRight or
    dxFormatImageLayoutCenterLeft or dxFormatImageLayoutCenterCenter or
    dxFormatImageLayoutCenterRight or dxFormatImageLayoutBottomLeft or
    dxFormatImageLayoutBottomCenter or dxFormatImageLayoutBottomRight;
  { Graphic DrawMode }
  ...
  dxFormatGraphicDrawModeMask = dxFormatGraphicDrawModeCenter or
    dxFormatGraphicDrawModeStretch or dxFormatGraphicDrawModeStretchProportional
    or dxFormatGraphicDrawModeCenterAndStretchProportional;
  { Graphic Buffering }
  ...
  dxFormatGraphicBufferingMask = dxFormatGraphicBufferingNone or
    dxFormatGraphicBufferingAlways;
  { Expand Button }
  ...
  dxFormatTreeLineModeMask = dxFormatTreeLineModeVertical or
    dxFormatTreeLineModeCross or dxFormatTreeLineModeTopRightCorner or
    dxFormatTreeLineModeBottomRightCorner;
  ...
  dxFormatExButtonAlignHorzMask = dxFormatExButtonAlignHorzCenter or
    dxFormatExButtonAlignHorzRight;
  ...
  dxFormatExButtonAlignVertMask = dxFormatExButtonAlignVertCenter or
    dxFormatExButtonAlignVertBottom;
  { Shapes }
  ...
  sdxPSRegPathRunTimeFormLayouts =
    '\Developer Express\PrintingSystem\FormLayouts'; // Don't localize

  dxDefaultMinPrintableArea: Integer = 50000; // In thousandths of mm
  dxDefaultInitialMargins: TRect = (Left: 12700; Top: 12700; Right: 12700;
    Bottom: 12700); // In thousandths of mm
  dxDefaultPhysicalPaperOffsets: TPoint = (X: 6350; Y: 6350);
...
function dxPSCheckVersion(const AVersion: TdxPSVersion;
  AMajor, AMinor: Integer): Boolean;
...
function StringToPSVersion(AVersion: string; const ASeparator: string = '.')
  : TdxPSVersion;
...
function GetPSRegRoot: string;
const
  Paths: array [Boolean] of string = (sdxPSRegPathRunTime,
    sdxPSRegPathDesignTime);
begin
...
function dxPSCheckVersion(const AVersion: TdxPSVersion;
  AMajor, AMinor: Integer): Boolean;
begin
  Result := (AVersion.Major > AMajor) or (AVersion.Major = AMajor) and
    (AVersion.Minor >= AMinor);
end;
    ...
    try
      ...
      try
        ...
        for I := 0 to AStrings.Count - 1 do
          CustomColors.Add(AStrings[I] + '=' + AIniFile.ReadString
            (sdxPSRegPathCustomColors, AStrings[I], ''));
      finally
        AStrings.Free;
      end;
    finally
  ...
  if dxPSStoringManager.BeginStoring(AIniFile) then
    try
      ...
      for I := 0 to CustomColors.Count - 1 do
        AIniFile.WriteString(sdxPSRegPathCustomColors, CustomColors.Names[I],
