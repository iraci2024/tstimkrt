...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Classes, Controls, Graphics, Messages, cxContainer,
  cxControls,
  cxDropDownEdit, cxEdit, cxEditConsts, cxGraphics, cxLookAndFeels, dxBar,
  cxGeometry, dxActions;
...
type
  ...
  protected
    ...
    class procedure InitializeAddedItem(AItemLink: TdxBarItemLink;
      AAddedItemName: string); override;
  end;
  ...
  private
    ...
    procedure CustomizingDoDrawEditButtonBackground
      (Sender: TcxEditButtonViewInfo; ACanvas: TcxCanvas; const ARect: TRect;
      var AHandled: Boolean);
    procedure CustomizingDoGetEditDefaultButtonWidth
      (Sender: TcxCustomEditViewData; AIndex: Integer;
      var ADefaultWidth: Integer);
    ...
    function IsBarCompatibleEdit(AEditProperties
      : TcxCustomEditProperties = nil): Boolean;
...
{$HINTS ON}
  ...
  published
    ...
    property BarStyleDropDownButton: Boolean read FBarStyleDropDownButton
      write SetBarStyleDropDownButton default True;
  end;

  { TdxBarEditItemActionLink }

  TdxBarEditItemActionLink = class(TdxBarItemActionLink, IUnknown,
  ...
  private
    ...
    procedure DoAfterKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure DoCanStartButtonFading(Sender: TcxCustomEditViewInfo;
      var ACanStart: Boolean);
    ...
    procedure DoDrawEditButton(Sender: TcxEditButtonViewInfo;
      ACanvas: TcxCanvas; var AHandled: Boolean);
    ...
    procedure DoEditClosePopup(Sender: TcxControl;
      AReason: TcxEditCloseUpReason);
    procedure DoEditPropertiesChange(Sender: TObject);
    procedure DoGetEditButtonContentColor(Sender: TcxEditButtonViewInfo;
      var AColor: TColor);
    procedure DoGetEditButtonState(Sender: TcxEditButtonViewInfo;
    ...
    procedure DoPrepareButtonFadingImageEvent(ASender: TcxEditButtonViewInfo;
      AState: TcxEditButtonState; out AImage: TcxBitmap32;
      var AHandled: Boolean);
    ...
    procedure DrawEditBackground(ACanvas: TcxCanvas; ARect: TRect;
      AColor: TColor);
    ...
    function GetEditButtonState(AButtonViewInfo: TcxEditButtonViewInfo)
      : Integer;
  ...
  protected
    ...
    procedure GetHintParams(out AHintText: string;
      out AShortCut: string); override;
    ...
    procedure CalculateEditViewInfo(const ABounds: TRect; P: TPoint;
      AIsMouseEvent: Boolean); overload;
    procedure CalculateEditViewInfo(const ABounds: TRect; P: TPoint;
      AIsMouseEvent: Boolean; AEditViewData: TcxCustomEditViewData;
      AEditViewInfo: TcxCustomEditViewInfo); overload;
...
uses
  Variants, Forms, StdCtrls, SysUtils, cxBarEditItemValueEditor, cxClasses,
  cxEditPaintUtils,
  cxEditRepositoryItems, cxEditUtils, cxLookAndFeelPainters, cxTextEdit,
  dxSpellCheckerCore, dxForms,
  cxVariants, dxBarCustomCustomizationForm, dxOffice11, dxBarStrs,
  dxBarSkinConsts, dxCore, dxFading, Math,
...
function InternalGetEditStyle(AEditProperties: TcxCustomEditProperties;
  ABarManager: TdxBarManager; APainter: TdxBarPainter; AFont: TFont;
  AColor, ATextColor: TColor; ADrawSelected: Boolean): TcxEditStyle;
  ...
  begin
    ...
    if APainter <> nil then
      ...
      TdxBarManagerAccess(ABarManager).GetRealLookAndFeel
        (FEditStyle.LookAndFeel);
...
procedure TEditorParentForm.ScaleForPPI(ATargetDPI: Integer;
...
procedure TEditorParentForm.Notification(AComponent: TComponent;
...
begin
  if FPrevActiveForm <> nil then
    if (FPrevActiveControl <> nil) and FPrevActiveControl.HandleAllocated and
      IsWindowVisible(FPrevActiveControl.Handle) then
    begin
      if FPrevActiveControl is TButtonControl then
      begin
        APrevClicksDisabled := TButtonControlAccess(FPrevActiveControl)
          .ClicksDisabled;
        try
          TButtonControlAccess(FPrevActiveControl).ClicksDisabled := True;
          FPrevActiveForm.SetFocusedControl(FPrevActiveControl);
        finally
          TButtonControlAccess(FPrevActiveControl).ClicksDisabled :=
            APrevClicksDisabled;
        end;
      end
      else
...
begin
  if FPrevActiveForm <> nil then
    Message.Result := FPrevActiveForm.Perform(Message.Msg, Message.WParam,
      Message.LParam)
  else
    inherited;
end;
...
begin
  ...
  else if TcxCustomBarEditItem(Item).Glyph.Empty then
    Result := psCaption
  else
  ...
  for I := 0 to GetRegisteredEditProperties.Count - 1 do
    if GetRegisteredEditProperties.Descriptions[I] <> '' then
      BarDesignController.AddInternalItem(AItemLinks, TdxBarButton,
        GetRegisteredEditProperties.Descriptions[I], OnButtonClick);
end;

class function TcxItemsEditorEx.GetAddedItemClass(const AAddedItemName: string)
  : TdxBarItemClass;
begin
...
class procedure TcxItemsEditorEx.InitializeAddedItem(AItemLink: TdxBarItemLink;
  AAddedItemName: string);
var
  APropertiesClass: TcxCustomEditPropertiesClass;
begin
  APropertiesClass := TcxCustomEditPropertiesClass
    (GetRegisteredEditProperties.FindByDescription(AAddedItemName));
...
begin
  ...
  try
    ...
    AEditViewData.OnGetDefaultButtonWidth :=
      CustomizingDoGetEditDefaultButtonWidth;
    AEditViewData.EditContentParams.ExternalBorderBounds := ARect;
    AEditViewInfo := TcxCustomEditViewInfo
      (AEditProperties.GetViewInfoClass.Create);
    AEditViewInfo.Data := Integer(ASelected);
    AEditViewInfo.OnDrawButtonBackground :=
      CustomizingDoDrawEditButtonBackground;
    ATempCanvas := TcxCanvas.Create(ACanvas);
    AEditViewData.UseRightToLeftAlignment := SysLocale.MiddleEast and
      (BarManager.BiDiMode = bdRightToLeft);
    AEditViewData.UseRightToLeftReading :=
      (ACanvas.TextFlags and ETO_RTLREADING) <> 0;
    AEditViewData.UseRightToLeftScrollBar := SysLocale.MiddleEast and
      (BarManager.BiDiMode in [bdRightToLeft, bdRightToLeftNoAlign]);
    AEditViewData.EditValueToDrawValue(Null, AEditViewInfo);
    AEditViewData.Calculate(ATempCanvas, ARect, Point(-1, -1), cxmbNone, [],
      AEditViewInfo, False);
...
function TcxCustomBarEditItem.GetControlClass(AIsVertical: Boolean)
...
procedure TcxCustomBarEditItem.InitProperties(AProperties
...
procedure TcxCustomBarEditItem.CreateProperties(APropertiesClass
...
function TcxCustomBarEditItem.IsBarCompatibleEdit(AEditProperties
  : TcxCustomEditProperties = nil): Boolean;
var
...
begin
  if not(GetProperties.CanCompareEditValue and
    (VarType(AValue) = VarType(FEditValue)) and VarEqualsExact(AValue,
    FEditValue)) then
  begin
    FEditValue := AValue;
...
procedure TcxCustomBarEditItem.SetRepositoryItemValue
...
function TdxBarEditItemActionLink.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
...
procedure TcxBarEditItemControl.ActivateEdit(AByMouse: Boolean;
  AKey: Char = #0);
var
  P: TPoint;
...
{$ELSE}
  if Parent.IsOnGlass then
    Edit.ActiveControl.ControlState := Edit.ActiveControl.ControlState +
      [csGlassPaint];
{$ENDIF}
...
procedure TcxBarEditItemControl.DoPaint(ARect: TRect;
...
procedure TcxBarEditItemControl.DrawTextField;
  ...
  begin
    Result := Painter.EditButtonAllowCompositeFrame and
      Item.UseBarPaintingStyle;
  end;

begin
  if not Focused or HasEditButtonCompositeFrame then
  begin
    CalculateEditViewInfo(GetBoundsRect,
      Parent.ScreenToClient(GetMouseCursorPos), False);
    EditViewInfo.Data := Integer(FDrawParams.PaintType);
    Canvas.SaveState;
    try
      EditViewInfo.BackgroundColor :=
        cxGetActualColor(Painter.EditControlGetBackgroundColor(DrawParams),
        EditViewInfo.BackgroundColor);
      EditViewInfo.TextColor :=
        cxGetActualColor(Painter.EditControlGetTextColor(DrawParams),
        EditViewInfo.TextColor);
...
begin
  Result := inherited DrawSelected and
    (NeedFocusOnClick or IsSelected and not FSelectedByMouse);
end;
...
function TcxBarEditItemControl.GetCurrentCursor(const AMousePos
  : TPoint): TCursor;
begin
  if Enabled then
    Result := TcxCustomEditViewInfoAccess(EditViewInfo)
      .GetCurrentCursor(AMousePos)
  else
...
procedure TcxBarEditItemControl.GetHintParams(out AHintText: string;
  out AShortCut: string);
begin
  AHintText := GetShortHint(TcxCustomEditViewInfoAccess(EditViewInfo)
    .GetHintText(TcxCustomEditViewInfoAccess(EditViewInfo)
    .GetPart(Parent.ScreenToClient(GetMouseCursorPos))));
  AShortCut := '';
  if AHintText = '' then
...
function TcxBarEditItemControl.GetViewData(out AIsViewDataCreated: Boolean)
  : TcxCustomEditViewData;
begin
...
begin
  if NeedEditShowCaption then
    if (GetPaintType = ptMenu) and Painter.SubMenuControlHasBand and
      not(cpIcon in GetViewStructure) then
...
begin
  if NeedEditShowCaption and
    (esoAutoWidth in GetProperties.GetSupportedOperations) then
...
begin
  if Edit <> nil then
    ...
    begin
      ...
      begin
        Item.FPrevOnEditValueChanged :=
          Edit.InternalProperties.OnEditValueChanged;
        Edit.InternalProperties.OnEditValueChanged := Item.DoEditValueChanged;
      end;
      if not TcxCustomEditAccess(Edit).IsEditValidated and Edit.ModifiedAfterEnter
      then
        FEditValueBeforeHiding := TcxCustomEditAccess(Edit)
          .InternalGetEditingValue
      else
      ...
      try
        if EditorParentForm.HandleAllocated then
          EditorParentForm.FocusControl(nil);
        // must be before Parent:=nil because DoEnter in WinControl Activate
        Edit.Parent := nil; // must be before DefocusControl
        if IsDropDownEdit then
        begin
          PopupWindow.ActiveControl := nil;
          TcxCustomPopupWindowAccess(PopupWindow).FPrevActiveForm :=
            EditorParentForm.FPrevActiveForm;
          TcxCustomPopupWindowAccess(PopupWindow).FPrevActiveControl :=
            EditorParentForm.FPrevActiveControl;
        end;
      ...
      EditorParentForm.DefocusControl(Edit, True);
      // must be before ClearEditEvents;
      ClearEditEvents;
  ...
  begin
    ...
    begin
      if Item.Properties <> nil then
        Edit.InternalProperties.OnEditValueChanged :=
          Item.Properties.OnEditValueChanged;
      if Assigned(AEditProperties.OnEditValueChanged) then
        Edit.InternalProperties.OnEditValueChanged :=
          Properties.OnEditValueChanged;
    end;
...
begin
  ...
  FEdit := EditList.GetEdit(TcxCustomEditPropertiesClass
    (AProperties.ClassType));
...
begin
  Result := inherited IsChildWindow(AWnd) or (Edit <> nil) and
    (Edit.IsChildWindow(AWnd));
end;
...
begin
  Result := not Item.IsEditClickable or TCustomdxBarControlAccess(Parent)
    .IsPopup or not Properties.ImmediatePost;
end;
...
begin
  ...
  begin
    if EditViewInfo.ButtonsInfo[I].FadingHelper.Active then
      EditViewInfo.ButtonsInfo[I].FadingHelper.CheckStartFading(cxbsHot,
        cxbsNormal);
  end;
end;
...
procedure TcxBarEditItemControl.CalculateEditViewInfo(const ABounds: TRect;
  P: TPoint; AIsMouseEvent: Boolean);
var
...
procedure TcxBarEditItemControl.CalculateEditViewInfo(const ABounds: TRect;
  P: TPoint; AIsMouseEvent: Boolean; AEditViewData: TcxCustomEditViewData;
  AEditViewInfo: TcxCustomEditViewInfo);

  function CanHotTrack: Boolean;
  begin
    Result := not(Item.IsDesigning or Parent.IsCustomizing) and
      IsWindowEnabled and Painter.EditButtonAllowHotTrack(FDrawParams);
  end;
...
begin
  ...
  Canvas.SaveState;
  try
    AEditViewData.Calculate(Canvas, ABounds, P, cxmbNone, [], AEditViewInfo,
      AIsMouseEvent);
  finally
    Canvas.RestoreState;
  end;
...
function TcxBarEditItemControl.CreateEditViewData(AFull: Boolean = True)
  : TcxCustomEditViewData;
begin
...
procedure TcxBarEditItemControl.InternalDrawComboBoxDropDownButton
...
procedure TcxBarEditItemControl.Invalidate(const R: TRect;
...
procedure TcxBarEditItemControl.DoAfterKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TcxBarEditItemControl.DoCanStartButtonFading
  (Sender: TcxCustomEditViewInfo; var ACanStart: Boolean);
begin
  ACanStart := ACanStart and not(csDestroying in Parent.ComponentState) and
    Painter.IsFadingAvailable;
end;

procedure TcxBarEditItemControl.DoCustomDrawPopupBorder
  (AViewInfo: TcxContainerViewInfo; ACanvas: TcxCanvas; const R: TRect;
  var AHandled: Boolean; out ABorderWidth: Integer);
begin
  AHandled := True;
  ABorderWidth := Painter.GetPopupWindowBorderWidth;
  if (ACanvas <> nil) then
    Painter.DropDownListBoxDrawBorder(ACanvas.Handle,
      AViewInfo.BackgroundColor, R);
end;
...
begin
  ...
  begin
    ...
    BeforeDrawBackground(Sender.Edit, Edit, ACanvas.Handle, ABounds,
      APrevWindowOrg);
    try
...
procedure TcxBarEditItemControl.DoDrawEditButton(Sender: TcxEditButtonViewInfo;
  ACanvas: TcxCanvas; var AHandled: Boolean);
var
...
begin
  ...
  begin
    ...
    if Painter.EditButtonAllowCompositeFrame and not AIsParentCanvas then
      Painter.ComboControlDrawArrowButton(DrawParams,
        FParts[ccpDropButton], False);
  end;
end;
...
begin
  ...
  begin
    ...
    BeforeDrawBackground(Edit, Parent, ACanvas.Handle, ABackgroundRect,
      APrevWindowOrg);
    try
      AOriginalCanvas := FDrawParams.Canvas;
      try
...
begin
  ...
  begin
    ...
    AOffset := BeforeDrawBackground(Edit, Parent, ACanvas.Handle,
      ABackgroundRect, APrevWindowOrg);
    try
      AOriginalCanvas := FDrawParams.Canvas;
      try
        FDrawParams.Canvas := ACanvas;
        Painter.EditButtonDrawBorder(FDrawParams, GetEditButtonState(Sender),
          ABackgroundRect, AContentRect);
      finally
        FDrawParams.Canvas := AOriginalCanvas
      end;
    finally
...
procedure TcxBarEditItemControl.DoEditPaint(Sender: TcxCustomEditViewInfo;
  ACanvas: TcxCanvas);
begin
  CalcDrawParams;
end;
...
procedure TcxBarEditItemControl.DoGetEditButtonContentColor
  (Sender: TcxEditButtonViewInfo; var AColor: TColor);
begin
...
begin
  ...
  begin
    if Focused or DrawSelected or (HotPartIndex = ccpDropButton) and CanSelect
    then
      AState := ebsSelected;
  end;
end;
...
procedure TcxBarEditItemControl.DoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TcxBarEditItemControl.DoKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TcxBarEditItemControl.DoPrepareButtonFadingImageEvent
  ...
  begin
    ...
    else if AHotPartIndex = ccpDropButton then
...
function TcxBarEditItemControl.GetEditButtonState(AButtonViewInfo
  : TcxEditButtonViewInfo): Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FLastEditPart := TcxCustomEditViewInfoAccess(EditViewInfo)
        .GetPart(APoint);
    end;
  end
  else
...
function TcxBarEditItemControl.GetDefaultEditButtonWidth
...
begin
  ...
  try
    AEditViewData := CreateEditViewData(not AIsMinSize or
      Item.CaptionIsEditValue);
    try
      CalculateEditViewInfo(cxRect(0, 0, cxMaxRectSize, cxMaxRectSize),
        cxInvalidPoint, False, AEditViewData, AEditViewInfo);
      AConstantPartSize := AEditViewData.GetEditConstantPartSize(Canvas,
        cxDefaultEditSizeProperties, Result, AEditViewInfo);

      if Item.CaptionIsEditValue then
        Result := cxSizeMax(Result, AEditViewData.GetEditContentSize(Canvas,
          Caption, cxDefaultEditSizeProperties));
...
function TcxBarEditItemControl.GetEditStyle(AFull: Boolean = True)
  : TcxEditStyle;
var
...
begin
  ...
  Result := InternalGetEditStyle(Properties, BarManager, APainter, EditFont,
    ABackgroundColor, ATextColor, DrawSelected);
end;

function TcxBarEditItemControl.GetEditViewInfo: TcxCustomEditViewInfo;
begin
  if (FEditViewInfo <> nil) and
    (FEditViewInfo.ClassType <> Properties.GetViewInfoClass) then
    FreeAndNil(FEditViewInfo);
...
begin
  Result := cxGetScaledEditProperties(FInternalProperties, Item.GetProperties,
    ScaleFactor);
end;
...
begin
  Result := Item.CaptionIsEditValue or inherited GetShowCaption and
    not GetShowCaption;
end;
