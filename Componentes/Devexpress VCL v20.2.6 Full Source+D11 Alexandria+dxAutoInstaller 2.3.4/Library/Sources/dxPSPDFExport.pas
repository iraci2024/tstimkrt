...
{$ENDIF}
  ...
  dxPrnPg, dxPSPDFExportCore, dxPSReportRenderCanvas, dxPSFillPatterns,
  dxPSGlbl, dxPrnDlg;
...
type
  ...
  public
    ...
    //
    property Options: TdxPSPDFReportExportOptions read FOptions
      write SetOptions;
  ...
  public
    ...
    procedure Combine(const R: TcxRegion; AOperation: TcxRegionOperation;
      const XForm: TXForm);
  ...
  strict private
    ...
    function CalculateTextOffset(const ARect: TdxRectF;
    ...
    procedure InternalDrawText(const ABounds: TRect; const AText: string;
      AFont: TFont; ATextColor: TColor; AFormat: TcxTextOutFormat;
      AMaxLineCount: Integer; AVertical: Boolean;
      ADirection: TcxVerticalTextOutDirection; ALineSpacing: Single = 1);
  ...
  protected
    procedure DoFillEdge(ARegion: TcxRegionHandle;
      ...
      AEdgePattern: TClass); override;
    procedure DrawPictureStretch(APicture: TGraphic; const R: TRect;
      ANumerator, ADenominator: Integer); override;
  ...
  public
    ...
    // Clipping
    function CombineClipRgn(ARegion: TcxRegionHandle;
      AOperation: TcxRegionOperation): Boolean;
    ...
    procedure DrawTextLine(const AText: string;
      const ATextRect, AClipRect: TRect; ATextWidth: Integer;
      ATextColor: TColor = clDefault; ADirection: TcxRotationAngle = ra0;
      ABreakExtraSpacing: Integer = 0; ANoClip: Boolean = False); overload;
    procedure DrawTextLine(const AText: string; const ATextOffset: TdxPointF;
      const AClipRect: TRect; ATextWidth: Integer; ATextColor: TColor;
      AAngle: Single; ABreakExtraSpacing: Integer; ANoClip: Boolean); overload;
    ...
    procedure Polygon(const APoints: array of TPoint; AColor: TColor;
      ABackgroundColor: TColor; ALineWidth: Integer;
      AFillMode: Integer = ALTERNATE); override;
    //
    property ClipRegion: TdxPSPDFCanvasClipRegion read FClipRegion
      write SetClipRegion;
...
function dxPSExportToPDFStream(AStream: TStream; AReportLink: TBasedxReportLink;
  AShowSettingsDialog: Boolean = True;
  ASettings: TdxPSPDFReportExportOptions = nil): Boolean;
...
uses
  Types, Math, Dialogs, ShellApi, dxCore, dxPSPDFExportDialog,
  dxPSPDFMetaFileParser,
  dxPSEdgePatterns, dxPSPDFStrings, dxCoreGraphics, cxControls,
  dxGDIPlusClasses, cxCustomCanvas;
...
type
  ...
  public
    constructor Create(APDFCanvas: TdxPSPDFReportRenderCanvas;
      ATextFormat: Cardinal); virtual;
    function CalculateBreakExtractSpacing(const AWidth: Integer;
      ARow: PcxTextRow): Integer; virtual;
    function CalculateTextOutParams(const ABounds: TRect;
      AMaxLineCount: Integer; ALineSpacing: Single; const AText: string)
      : TRect; virtual;
    ...
    function CalculateTextWidth(const AText: PChar; ATextLength: Integer)
      : Single; overload;
  ...
  public
    ...
    function CalculateTextRowOutRect(ATextRow: PcxTextRow; var R: TRect;
      out ABreakExtraSpacing: Integer): TRect; override;
  end;
  ...
  protected
    ...
    procedure InitializePixels(APattern: TdxPSEdgePatternClass;
      AIsVertical: Boolean);
    //
...
begin
  with TdxGPMatrix.CreateEx(X.eM11, X.eM12, X.eM21, X.eM22, X.eDx, X.eDy) do
    try
      Invert;
      GetElements(Result.eM11, Result.eM12, Result.eM21, Result.eM22,
        Result.eDx, Result.eDy);
    finally
      Free;
    end;
end;
...
function InternalExportToPDF(AStream: TStream; AReportLink: TBasedxReportLink;
  ...
  out AOpenDocumentAfterExport: Boolean;
  const AStreamFileName: string = ''): Boolean;

  function DoBeforeExportToPDF(AOptions: TdxPSPDFReportExportOptions): Boolean;
  begin
    Result := TBasedxReportLinkAccess(AReportLink).DoBeforeExportToPDF
      (AStreamFileName, AOptions);
  end;
...
begin
  ...
  try
    Result := AReportLink.CanExportToPDF and SetupOptions(AProvider.Options) and
      DoBeforeExportToPDF(AProvider.Options);
    if Result then
    begin
      AProvider.Build;
...
function dxPDFCalcTextRowExtents(AHandle: TCanvasHandle; AText: PWideChar;
  ATextLength: Integer; AExpandTabs: Boolean;
  PDFCanvas: TdxPSPDFReportRenderCanvas): TSize;
begin
  ...
  begin
    ...
    try
      ...
      ASaveDialog.InitialDir :=
        ExtractFilePath(AReportLink.PDFExportOptions.DefaultFileName);
      ASaveDialog.FileName := CheckFileNameLength
        (ExtractFileName(AReportLink.PDFExportOptions.DefaultFileName));
      ASaveDialog.Options := ASaveDialog.Options + [ofOverwritePrompt];
      ASaveDialog.DefaultExt := sdxPDFExt;
      Result := ASaveDialog.Execute and
        dxPSExportToPDFFile(ChangeFileExt(ASaveDialog.FileName, sdxPDFExt),
...
begin
  ...
  try
    ...
    try
      Result := InternalExportToPDF(AStream, AReportLink, AShowSettingsDialog,
        ASettings, AOpenDocumentAfterExport, AFileName);
    finally
      AStream.Free;
    end;
  finally
...
begin
  Result := InternalExportToPDF(AStream, AReportLink, AShowSettingsDialog,
    ASettings, AOpenDocumentAfterExport);
end;

function CreateTextExportHelper(ACanvas: TdxPSPDFReportRenderCanvas;
  AFormat: TcxTextOutFormat; AVertical: Boolean)
  : TdxPSPDFExportCustomTextHelper;
begin
  if AVertical then
...
function TdxPSPDFEdgePattern.GetContentData: string;
  ...
  begin
    ...
    for I := 0 to Length(FLines) - 1 do
      Result := Result + dxPDFEncodeBounds(FLines[I], PatternHeight) +
        sdxPDFSpace;
  end;
...
procedure TdxPSPDFEdgePattern.InitializePixels(APattern: TdxPSEdgePatternClass;
  AIsVertical: Boolean);

  function CompactLine(var APrev, ACurrent: TRect): Boolean;
  begin
    Result := True;
    if (APrev.Top = ACurrent.Top) and (APrev.Bottom = ACurrent.Bottom) and
      (APrev.Right = ACurrent.Left) then
      APrev.Right := ACurrent.Right
    else if (APrev.Left = ACurrent.Left) and (APrev.Right = ACurrent.Right) and
      (APrev.Bottom = ACurrent.Top) then
  ...
  begin
    for J := 0 to ASize.cy - 1 do
      ...
      begin
        ...
        if (AIndex = 0) or not CompactLine(FLines[AIndex - 1], FLines[AIndex])
        then
          Inc(AIndex);
      end;
  end;
...
begin
  ...
  FTextParams := PDFCanvas.CalculateTextParams(ATextFormat or
    CXTO_CALCROWCOUNT);
end;
...
function TdxPSPDFExportCustomTextHelper.CalculateTextOutParams
  (const ABounds: TRect; AMaxLineCount: Integer; ALineSpacing: Single;
...
function TdxPSPDFExportCustomTextHelper.CalculateTextWidth
  (const AText: string): Single;
begin
  Result := CalculateTextWidth(PChar(AText), Length(AText));
end;

function TdxPSPDFExportCustomTextHelper.CalculateTextWidth(const AText: PChar;
  ATextLength: Integer): Single;
begin
  Result := PDFCanvas.PageContent.TextWidth(AText, ATextLength);
end;

procedure TdxPSPDFExportCustomTextHelper.CalculateEndEllipsis(DC: HDC;
  const R: TRect);
var
  ACharNumber: Integer;
  AHeight: Integer;
...
begin
  ...
  begin
    FTextParams.EndEllipsisWidth := Round(CalculateTextWidth(EndEllipsis));
    Dec(AWidth, TextParams.EndEllipsisWidth);
    if AWidth > 0 then
    begin
      if not GetTextExtentExPointW(DC, ARow.Text, ARow.TextLength, AWidth,
...
procedure TdxPSPDFExportCustomTextHelper.CorrectTextRowsExtends(const S: TSize;
  ALineSpacing: Single);
var
  AExtraSpace: Integer;
  AReminder: Single;
...
begin
  ...
  begin
    ARow := TextRow[I];
    ARow^.TextExtents.cx := Min(ARow^.TextExtents.cx,
      Round(CalculateTextWidth(TextRowText[I])));
  end;
...
function TdxPSPDFExportHorizontalTextHelper.CalculateTextRect
...
function TdxPSPDFExportVerticalTextHelper.CalculateTextRect
  (const ABounds: TRect): TRect;
var
  AHeight: Integer;
begin
...
function TdxPSPDFExportVerticalTextHelper.CalculateTextOutParams
  ...
  const AText: string): TRect;
begin
  Result := inherited CalculateTextOutParams(cxRectRotate(ABounds),
    AMaxLineCount, ALineSpacing, AText);
end;
...
procedure TdxPSPDFReportExportProvider.ExportPage(APageIndex: Integer);
  ...
  begin
    ...
    Result.X := MulDiv(Result.X, APageInfo.RenderInfo.PointsPerInch,
      APageInfo.RenderInfo.UnitsPerInch);
    Result.Y := MulDiv(Result.Y, APageInfo.RenderInfo.PointsPerInch,
      APageInfo.RenderInfo.UnitsPerInch);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      TBasedxReportLinkAccess(ReportLink).PaintPageEx(ACanvas, Page.PageBounds,
        APageIndex, APageIndex, 100);
    finally
      ACanvas.Free;
    end;
  end;
...
procedure TdxPSPDFReportExportProvider.SetOptions
...
procedure TdxPSPDFCanvasClipRegion.Combine(const R: TcxRegion;
  AOperation: TcxRegionOperation; const XForm: TXForm);
begin
...
procedure TdxPSPDFCanvasClipRegion.Intersect(const R: TRect;
  const XForm: TXForm);
begin
  GetActualRegion(XForm).Combine(R, roIntersect);
end;

function TdxPSPDFCanvasClipRegion.GetActualRegion(const XForm: TXForm)
  : TdxPSPDFCanvasRegion;
var
  ASize: TSize;
begin
...
constructor TdxPSPDFReportRenderCanvas.Create
  (AProvider: TdxPSPDFReportExportProvider);
begin
  inherited Create;
  FProvider := AProvider;
...
function TdxPSPDFReportRenderCanvas.CalculateLineThickness(AUnitsPerPixel,
...
procedure TdxPSPDFReportRenderCanvas.SetCanvasExts(const APageSize: TPoint;
  AMappingMode: TdxPSReportRenderCanvasMappingMode;
  AScaleNumerator, AScaleDenominator: Integer; const AViewPort: TRect);
var
  AZoom: Double;
begin
  inherited SetCanvasExts(APageSize, AMappingMode, AScaleNumerator,
    AScaleDenominator, AViewPort);
...
procedure TdxPSPDFReportRenderCanvas.DrawPictureStretch(APicture: TGraphic;
...
procedure TdxPSPDFReportRenderCanvas.InternalDrawText(const ABounds: TRect;
  ...
  begin
    if not AVertical then
      Result := ra0
    else if ADirection = vtdTopToBottom then
      Result := raMinus90
    else
      Result := raPlus90;
  end;
...
begin
  ...
  try
    ...
    try
      R := ATextExportHelper.CalculateTextOutParams(ABounds, AMaxLineCount,
        ALineSpacing, AText);
      for I := 0 to ATextExportHelper.TextRowCount - 1 do
      begin
        ATextRow := ATextExportHelper.TextRow[I];
        ATextRowRect := ATextExportHelper.CalculateTextRowOutRect(ATextRow, R,
          ABreakExtraSpacing);
        if not IsRectEmpty(ATextRowRect) then
        begin
          DrawTextLine(ATextExportHelper.TextRowText[I], ATextRowRect, ABounds,
            ATextRow.TextExtents.cx, ATextColor, GetRotationAngle,
            ABreakExtraSpacing, ATextExportHelper.TextParams.NoClip);
        end;
      end;
    finally
...
procedure TdxPSPDFReportRenderCanvas.DrawTextLine(const AText: string;
  const ATextRect, AClipRect: TRect; ATextWidth: Integer;
  ATextColor: TColor = clDefault; ADirection: TcxRotationAngle = ra0;
  ABreakExtraSpacing: Integer = 0; ANoClip: Boolean = False);
begin
  if IsRectVisible(ATextRect) then
  begin
    DrawTextLine(AText, CalculateTextOffset(dxRectF(ATextRect), ADirection),
      AClipRect, ATextWidth, ATextColor, cxRotationAngleToAngle[ADirection],
      ABreakExtraSpacing, ANoClip);
  end;
end;

procedure TdxPSPDFReportRenderCanvas.DrawTextLine(const AText: string;
  const ATextOffset: TdxPointF; const AClipRect: TRect; ATextWidth: Integer;
  ATextColor: TColor; AAngle: Single; ABreakExtraSpacing: Integer;
  ANoClip: Boolean);
  ...
  begin
    ...
    Result := Min(0, (ATextWidth - PageContent.TextWidth(AText)) /
      Length(AText));
  ...
  begin
    ...
    try
      ...
      PageContent.DrawText(AText, ATextOffset, AAngle + Font.Orientation / 10,
        CalculateCharsSpacing, ABreakExtraSpacing);
    finally
      UnpreparePageContent;
    end;
  end;
...
begin
  ...
  begin
    PreparePageContent;
    try
      PageContent.FillRectByGraphic(R, MulDiv(APictureWidth, ADenominator,
        ANumerator), MulDiv(APictureHeight, ADenominator, ANumerator),
        APicture);
    finally
      UnpreparePageContent
    end;
...
begin
  ...
  else if APatternBrush.Style <> bsSolid then
...
function TdxPSPDFReportRenderCanvas.GetSupportsTransparentImagesDrawing
  : Boolean;
begin
  Result := False;
end;
...
procedure TdxPSPDFReportRenderCanvas.DrawEllipseFrame(const R: TRect;
...
procedure TdxPSPDFReportRenderCanvas.DrawExpandButton(R: TRect;
  ...
  procedure DrawExpandButtonBorders(const R: TRect;
    ...
    begin
      ...
      begin
        GetBorderColors(AEdgeStyle, ABorderColor, False, ARightBottomColor,
          ATopLeftColor);
        PageContent.DrawFrame(R, ALineThickness, ATopLeftColor,
          ARightBottomColor, cxBordersAll);
      end;
    end;
  end;
  ...
  begin
    ...
    PageContent.FillRect(cxRectCenter(R, cxRectWidth(R), ALineThickness),
      ABorderColor);
    if not AExpanded then
      PageContent.FillRect(cxRectCenter(R, ALineThickness, cxRectHeight(R)),
        ABorderColor);
  end;
  ...
  begin
    ...
    try
      ...
      if AFillInterior then
        PageContent.FillRect(cxRectInflate(R, -ALineThickness, -ALineThickness),
          ABackgroundColor);
...
begin
  ...
  begin
    PreparePageContent;
    try
      PageContent.DrawFrame(R, ABorderWidth, ATopLeftColor, ARightBottomColor,
        ABorders);
    finally
      UnpreparePageContent;
    end;
...
procedure TdxPSPDFReportRenderCanvas.DrawGraphic(AHelper: TdxDrawGraphicHelper;
  const R: TRect);
var
  ARect: TRect;
begin
  ...
  begin
    if AHelper.Graphic is TMetafile then
      ...
      AHelper.Export(procedure(AGraphic: TGraphic; AMask: TcxRegion)
        begin
          PreparePageContent;
          try
...
begin
  ...
  begin
    PreparePageContent;
    try
      PageContent.DrawRoundFrame(R, AEllipseWidth, AEllipseHeight, AColor,
        AThickness);
    finally
      UnpreparePageContent;
    end;
...
procedure TdxPSPDFReportRenderCanvas.ExtTextOut(const ABounds: TRect;
AFont: TFont; const AText: string; AGlyphs: PWord; AGlyphCount: Integer;
AGlyphWidths: PInteger);
begin
  SelectFonT(AFont, clDefault);
  DrawTextLine(AText, ABounds, ABounds, cxRectWidth(ABounds), clDefault,
    ra0, 0, True);
end;

procedure TdxPSPDFReportRenderCanvas.DrawText(var R: TRect; const AText: string;
AFont: TFont; AFormat: DWORD; ATextColor: TColor = clDefault;
AMaxLineCount: Integer = 0; ALineSpacing: Single = 1.0);
begin
  InternalDrawText(R, AText, AFont, ATextColor, AFormat, AMaxLineCount, False,
    vtdTopToBottom, ALineSpacing);
end;
...
AEdgePattern: TClass);
  ...
  begin
    Result := TdxPSPDFEdgePattern.Create(PageContent.PatternList, AEdgeColor,
...
var
  ARect: TRect;
begin
  if AEdgePattern.InheritsFrom(TdxPSEdgePattern) and
    (GetRgnBox(ARegion, ARect) <> NULLREGION) then
  begin
    SaveClipRgn;
    try
...
procedure TdxPSPDFReportRenderCanvas.FillRectEx(const R: TRect; AColor: TColor;
APattern: TdxPSFillPatternClass; APatternBrush: TBrush);
begin
  ...
  begin
    ...
    try
      ...
      begin
        PreparePageContent;
        try
          InternalFillRect(ARect, ABackColor, AForeColor, APattern,
            APatternBrush);
        finally
          UnpreparePageContent;
        end;
...
procedure TdxPSPDFReportRenderCanvas.Pie(const R: TRect;
...
procedure TdxPSPDFReportRenderCanvas.Polyline(const APoints: array of TPoint;
...
begin
  ...
  begin
    PreparePageContent;
    try
      PageContent.Polygon(APoints, ALineWidth, AColor, ABackgroundColor,
        AFillMode);
    finally
      UnpreparePageContent;
    end;
...
procedure TdxPSPDFReportRenderCanvas.RotatedTextOut(const ABounds: TRect;
...
begin
  ...
  InternalDrawText(ABounds, AText, AFont, clDefault, AFormat, 0, True,
    ADirection);
end;
...
ARightIndent: Integer = 0; ATextColor: TColor = clDefault;
ALineSpacing: Single = 1.0): Integer;
var
  ABreakExtraSpacing: Integer;
  ATextExportHelper: TdxPSPDFExportCustomTextHelper;
  ATextRowRect: TRect;
...
begin
  ...
  try
    ...
    try
      R1 := ATextExportHelper.CalculateTextOutParams(R, AMaxLineCount,
        ALineSpacing, AText);
      for I := 0 to ATextExportHelper.TextRowCount - 1 do
      begin
...
function TdxPSPDFReportRenderCanvas.CalculateTextOffset(const ARect: TdxRectF;
AAngle: TcxRotationAngle): TdxPointF;

  function GetPivotPoint(const ARect: TdxRectF; AAngle: TcxRotationAngle = ra0)
    : TdxPointF;
  begin
    case AAngle of
      ra180:
...
begin
  ...
  try
    AMatrix.Rotate(cxRotationAngleToAngle[AAngle],
      GetPivotPoint(ARect, AAngle));
...
function TdxPSPDFReportRenderCanvas.CombineClipRgn(ARegion: TcxRegionHandle;
AOperation: TcxRegionOperation): Boolean;
var
...
function TdxPSPDFReportRenderCanvas.IntersectClipRgnEx
  (ARegion: TcxRegionHandle): Boolean;
begin
  Result := CombineClipRgn(ARegion, roIntersect);
end;

function TdxPSPDFReportRenderCanvas.BeginCustomDraw(const AClipBounds: TRect;
...
begin
  ...
  SetWindowOrgEx(FCustomDrawMetaFileCanvas.Handle, FCustomDrawBounds.Left,
    FCustomDrawBounds.Top, nil);
...
procedure TdxPSPDFReportRenderCanvas.PreparePageContent;
  ...
  begin
    Result := (ClipRegion.Count > 1) or
      not TXForm.IsIdentity(ClipRegion.Last.XForm);
  end;
  ...
  begin
    ...
    for AIndex := ClipRegion.Count - 1 downto 0 do
      ATransform := TXForm.Combine(ATransform,
        InvertXForm(ConvertMatrix(ClipRegion[AIndex].XForm)));
...
begin
  ...
  PageContent.ModifyWorldTransform(TXForm.CreateTranslateMatrix(-WindowOrg.X,
    WindowOrg.Y));
...
procedure TdxPSPDFReportRenderCanvas.SelectFonT(AFont: TFont;
ATextColor: TColor);
  ...
  begin
    ...
    begin
      ...
      try
        ...
        Result := -MulDiv(AFont.Size, cxTextHeight(ATempFont),
          cxTextHeight(AFont));
      finally
        ATempFont.Free;
      end;
...
procedure TdxPSPDFReportRenderCanvas.SetClipRegion
  (const Value: TdxPSPDFCanvasClipRegion);
begin
