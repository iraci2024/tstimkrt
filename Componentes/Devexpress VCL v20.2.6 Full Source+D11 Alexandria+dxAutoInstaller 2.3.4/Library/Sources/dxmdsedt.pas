...
uses
  DesignIntf, Windows, Classes, Controls, Forms, StdCtrls, dxmdaset, ExtCtrls,
  Menus, Graphics, dxCore;
...
function ShowMemDataFieldEditor(AData: TdxMemData;
  AFormDesigner: IDesigner): Boolean;
...
procedure TMemDataSetDesigner.DataEvent(Event: TDataEvent; Info: TdxNativeInt);
var
  I, J: Integer;
begin
  if AForm <> nil then
...
procedure TfrmdxMemDataEditor.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  if (Data <> nil) and Not(csDestroying in Data.ComponentState) and
    (Data.Designer <> nil) and not TMemDataSetDesigner(Data.Designer).FDestroying
  then
  begin
...
procedure TfrmdxMemDataEditor.ListBoxDragOver(Sender, Source: TObject;
...
procedure TfrmdxMemDataEditor.ListBoxEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
  if OldDragIndex <> -1 then
  begin
...
procedure TfrmdxMemDataEditor.ListBoxDragDrop(Sender, Source: TObject;
  X, Y: Integer);
var
  I, Index: Integer;
  List: TList;
...
procedure TfrmdxMemDataEditor.CreateControls;
  ...
  function CreateMenuItem(ACaption: String; AShortCut: TShortCut;
    ANotifyEvent: TNotifyEvent): TMenuItem;
  begin
