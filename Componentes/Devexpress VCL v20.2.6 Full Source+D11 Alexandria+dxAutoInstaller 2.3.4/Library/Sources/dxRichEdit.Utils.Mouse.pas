...
type
  ...
  public
    ...
    Horizontal: Boolean;
    constructor Create(AButtons: TdxMouseButtons; AShift: TShiftState;
      const AMousePos: TPoint); overload;
    constructor Create(AShift: TShiftState; const AMousePos: TPoint;
      ADelta: Integer; AHorizontal: Boolean = False); overload;
  end;
  ...
  TdxDragEventArgsHelper = record helper for TdxDragEventArgs
    class function CreateFormOle(const dataObj: IDataObject;
      grfKeyState: Integer; pt: TPoint; var dwEffect: Integer)
      : TdxDragEventArgs; static;
  end;
  ...
  TdxQueryContinueDragEventArgs = record
    ...
    constructor Create(AAction: TdxDragAction; AEscapePressed: Boolean;
      AKeyStates: TShiftState); overload;
  ...
  TdxGiveFeedbackEventArgs = record
    ...
    constructor Create(AEffect: TdxDragDropEffects;
      AUseDefaultCursors: Boolean); overload;
  ...
  public
    procedure SwitchStateCore(const ANewState: IdxCustomMouseState;
      const AMousePosition: TPoint); virtual; abstract;
    ...
    function DoContextPopup(var Args: TdxMouseEventArgs): Boolean;
      virtual; abstract;
...
function dxDragDropEffectsToOleDragDropEffects(const Value: TdxDragDropEffects)
  : LongWord;
function dxOleDragDropEffectsToDragDropEffects(const Value: LongWord)
  : TdxDragDropEffects;
...
constructor TdxMouseEventArgs.Create(AButtons: TdxMouseButtons;
  AShift: TShiftState; const AMousePos: TPoint; ADelta: Integer;
  AHorizontal: Boolean);
begin
  Buttons := AButtons;
  Shift := AShift;
...
begin
  ...
  else if [ssLeft, ssRight] * KeyStates = [] then
