...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Classes, Windows, Graphics, Controls, StdCtrls, Generics.Defaults,
  Generics.Collections,
  cxGeometry,
  dxCore, dxPSCore, dxPSGlbl, dxPrnPg, dxPSRes, dxBase, dxPSFillPatterns,
  dxPSReportRenderCanvas, dxCustomPreview, dxPreVw, dxPSPrVw,
...
uses
  Forms, SysUtils, Math,
  cxClasses, dxPSUtl, dxPSImgs, cxGraphics, dxTypeHelpers, cxDrawTextUtils,
  dxPrnDev, dxRichEdit.Commands.Strs,
...
constructor TdxRichEditControlReportLinkPageInfo.Create(APage: TdxPage;
  ADPI: Integer);
begin
  ...
  Margins := Rect(APage.ClientBounds.Left - APage.Bounds.Left,
    APage.ClientBounds.Top - APage.Bounds.Top, APage.Bounds.Right -
    APage.ClientBounds.Right, APage.Bounds.Bottom - APage.ClientBounds.Bottom);
...
procedure TdxRichEditControlReportLinkPageInfo.ApplyTo
  ...
  begin
    Result := Rect(ConvertValue(R.Left), ConvertValue(R.Top),
      ConvertValue(R.Right), ConvertValue(R.Bottom));
  end;
...
begin
  ...
  try
    APrinterPage.PageSize.Point := Point(ConvertValue(Width),
      ConvertValue(Height));
...
constructor TcxRichEditControlReportLinkPageRenderInfo.Create
  (ARenderInfo: TdxPSReportRenderInfo; APageIndex: Integer);
begin
  inherited Create(ARenderInfo, APageIndex);
  TdxRichEditControlReportLink(ReportLink).Pages[APageIndex]
    .ApplyTo(PrinterPage);
end;
...
function TcxRichEditControlReportLinkRenderInfo.CalculatePageContentHeight
  (APageIndex: Integer): Integer;
begin
  Result := ReportCells.Cells[APageIndex].Height;
end;

function TcxRichEditControlReportLinkRenderInfo.CalculatePageContentWidth
  (APageIndex: Integer): Integer;
begin
...
function TcxRichEditControlReportLinkRenderInfo.GetPageRenderInfoClass
...
function TcxRichEditControlReportLinkRenderInfo.GetReportLink
  : TdxRichEditControlReportLink;
begin
  Result := TdxRichEditControlReportLink(inherited ReportLink);
end;

function TcxRichEditControlReportLinkRenderInfo.
  CalculateActualScaleFactor: Integer;
begin
  Result := 100;
end;

function TcxRichEditControlReportLinkRenderInfo.IsDrawPageFootnotesOnPage
  (APageIndex: Integer): Boolean;
begin
  Result := False;
end;

function TcxRichEditControlReportLinkRenderInfo.IsDrawPageTitleOnPage
  (APageIndex: Integer): Boolean;
begin
  Result := False;
end;
...
var
  ABounds: TRect;
begin
  FPages.Add(TdxRichEditControlReportLinkPageInfo.Create(APage,
    Printable.DocumentDPI));
...
function TdxRichEditControlReportLink.GetCapabilities
  : TdxReportLinkCapabilities;
begin
  Result := inherited GetCapabilities - [rlcPageSetup, rlcHeaderFooter,
    rlcTitle, rlcFootnotes];
end;

class function TdxRichEditControlReportLink.GetCellsBounds
...
procedure TdxRichEditControlReportLink.ConvertDelimiters(ADelimiters: TList;
  ANumerator, ADenominator: Integer);
var
  I: Integer;
begin
  for I := 0 to ADelimiters.Count - 1 do
    ADelimiters[I] := Pointer(MulDiv(Integer(ADelimiters[I]), ANumerator,
      ADenominator));
end;

function TdxRichEditControlReportLink.GetRenderInfoClass
...
function TdxRichEditControlReportLink.GetCriticalSize
  (AReportCells: TdxReportCells): Integer;
begin
  Result := Max(cxRectWidth(AReportCells.BoundsRect),
    cxRectHeight(AReportCells.BoundsRect));
end;

procedure TdxRichEditControlReportLink.ConstructReport
...
begin
  Result := (Control <> nil) and
    Control.Options.Printing.EnablePageBackgroundOnPrint
end;

procedure TdxRichEditControlReportLink.MakeDelimiters
...
procedure TdxRichEditControlReportLink.AfterCreateDocument
  (AReportCells: TdxReportCells);
begin
...
procedure TdxRichEditControlReportLink.BeforeCreateDocument
  (AReportCells: TdxReportCells);
begin
  if Control <> nil then
...
function TdxRichEditControlReportLink.GetMeasurementUnits
