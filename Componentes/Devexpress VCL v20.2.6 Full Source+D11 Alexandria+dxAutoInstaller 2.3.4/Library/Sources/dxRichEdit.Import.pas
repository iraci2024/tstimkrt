...
uses
  Types, Classes, SysUtils, dxCoreClasses, Generics.Defaults,
...
type
  ...
  TdxImportRangePermissionInfos =
    TdxNamedObjectDictionary<TdxImportRangePermissionInfo>;
  ...
  public
    ...
    property ParagraphMarkCharacterFormatting: TdxCharacterFormattingBase
      read FParagraphMarkCharacterFormatting;
    property ParagraphFormatting: TdxParagraphFormattingBase
      read FParagraphFormatting;
    property ParagraphFrameFormatting: TdxParagraphFrameFormattingBase
      read FParagraphFrameFormatting;
    property ParagraphTabs: TdxTabFormattingInfo read FParagraphTabs;
    property ParagraphMarkCharacterStyleIndex: Integer
      read FParagraphMarkCharacterStyleIndex
      write FParagraphMarkCharacterStyleIndex;
    property ParagraphStyleIndex: Integer read FParagraphStyleIndex
      write FParagraphStyleIndex;
    property TableCellStyleIndex: Integer read FTableCellStyleIndex
      write FTableCellStyleIndex;
  end;
  ...
  public
    ...
    property CodeStartIndex: TdxRunIndex read FCodeStartIndex
      write FCodeStartIndex;
    ...
    property ResultEndIndex: TdxRunIndex read FResultEndIndex
      write FResultEndIndex;
  end;
  ...
  strict private
    class var FPatternMultipliers
      : TdxEnumeratedDictionary<TdxShadingPattern, Integer>;
    class constructor Initialize;
    class destructor Finalize;
  strict private
    class function CreatePatternMultiplierDictionary
      : TdxEnumeratedDictionary<TdxShadingPattern, Integer>; static;
  public
    class function GetActualBackColor(AFill: TdxAlphaColor;
      APatternColor: TdxAlphaColor; APattern: TdxShadingPattern)
...
function TdxImportBookmarkInfoCore.Validate(APieceTable: TdxPieceTable)
  : Boolean;
var
  ATemp, AMax: Integer;
begin
...
function TdxImportRangePermissionInfo.Validate(APieceTable
  : TdxPieceTable): Boolean;
begin
...
begin
  Result := (Name <> '') and (APieceTable.Bookmarks.FindByName(Name) = nil) and
    inherited Validate(APieceTable);
end;

{ TdxImportPieceTableInfoBase }

constructor TdxImportPieceTableInfoBase.Create(APieceTable
  : TdxSimplePieceTable);
begin
  inherited Create;
...
begin
  ...
  FParagraphMarkCharacterFormatting :=
    APieceTable.DocumentModel.CreateEmptyCharacterFormatting;
  FParagraphFormatting := APieceTable.DocumentModel.
    CreateEmptyParagraphFormatting;
  FParagraphFrameFormatting := TdxParagraphFrameFormattingBase.Create
    (APieceTable, APieceTable.DocumentModel,
    TdxDocumentModel(APieceTable.DocumentModel)
    .Cache.ParagraphFrameFormattingCache.EmptyParagraphFrameFormattingInfo,
...
class function TdxShadingHelper.CreatePatternMultiplierDictionary
  : TdxEnumeratedDictionary<TdxShadingPattern, Integer>;
begin
...
class function TdxShadingHelper.GetActualBackColor(AFill: TdxAlphaColor;
  APatternColor: TdxAlphaColor; APattern: TdxShadingPattern): TdxAlphaColor;
var
  AMultiplier, AIntensity: Integer;
begin
  if (APattern = TdxShadingPattern.Clear) or (APattern = TdxShadingPattern.&Nil)
  then
    Exit(AFill);
  if (TdxAlphaColors.IsTransparentOrEmpty(AFill) or
    (AFill = TdxAlphaColors.White)) and TdxAlphaColors.IsTransparentOrEmpty
    (APatternColor) then
  begin
