...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, SysUtils, Classes, Graphics, Generics.Collections,
  Generics.Defaults,
...
type
  ...
  public
    ...
    function CreateItem(AStyleClass: TcxCustomStyleClass)
      : TcxCustomStyle; virtual;
    function CreateItemEx(AStyleClass: TcxCustomStyleClass; AOwner: TComponent)
      : TcxCustomStyle; virtual;
    function CreateStyleSheet(AStyleSheetClass: TcxCustomStyleSheetClass)
      : TcxCustomStyleSheet; virtual;
    function CreateStyleSheetEx(AStyleSheetClass: TcxCustomStyleSheetClass;
      AOwner: TComponent): TcxCustomStyleSheet; virtual;
  ...
  private
    ...
    FOwnerStyleSheet: TcxCustomStyleSheet;
    // style sheet to which styles object is aggregated
    function GetCount: Integer;
  ...
  protected
    ...
    property StyleSheet: TcxCustomStyleSheet read FStyleSheet
      write SetStyleSheet;
  public
    ...
    property Values[Index: Integer]: TcxCustomStyle read GetValue
      write SetValue;
  end;
  ...
  published
    property AssignedValues: TcxStyleValues read FAssignedValues
  ...
  protected
    ...
    procedure InternalGetViewParams(Index: Integer; AData: TObject;
  ...
  public
    ...
    procedure GetViewParams(Index: Integer; AData: TObject; AStyle: TcxStyle;
...
function CombineParamsWithStyle(AStyle: TcxStyle; AHasValues: TcxStyleValues;
  var AParams: TcxViewParams): TcxStyleValues;
function UseStyle(AStyle: TcxStyle; AStyleValue: TcxStyleValue): Boolean;
procedure CreateStyleSheetStyles(ADestStyleSheet, ASourceStyleSheet
  : TcxCustomStyleSheet; AStyleGetName: TcxStyleGetName = nil);
...
procedure SaveStyleSheetsToIniFile(const AIniFileName: string;
  const AList: TList);
...
begin
  ...
  if (AStyle = nil) or ([svBitmap .. svTextColor] = AHasValues) or
    (csDestroying in AStyle.ComponentState) then
    Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if StyleRepository <> nil then
        AStyle := StyleRepository.CreateItemEx(AStyleClass,
...
function TcxStyleRepository.CreateItem(AStyleClass: TcxCustomStyleClass)
  : TcxCustomStyle;
begin
...
function TcxStyleRepository.CreateStyleSheet(AStyleSheetClass
  : TcxCustomStyleSheetClass): TcxCustomStyleSheet;
begin
  Result := CreateStyleSheetEx(AStyleSheetClass, Self);
end;

function TcxStyleRepository.CreateStyleSheetEx(AStyleSheetClass
  : TcxCustomStyleSheetClass; AOwner: TComponent): TcxCustomStyleSheet;
begin
...
function TcxStyleRepository.StyleSheetIndexOf(AStyleSheet
  : TcxCustomStyleSheet): Integer;
begin
...
function TcxCustomStyles.IsValidStyleSheet(AStyleSheet
  : TcxCustomStyleSheet): Boolean;
var
  AStyles: TcxCustomStyles;
begin
  Result := (AStyleSheet = nil) or ((AStyleSheet <> OwnerStyleSheet) and
    (AStyleSheet.GetStylesClass = TcxCustomStylesClass(ClassType)));
  if Result and (AStyleSheet <> nil) and (OwnerStyleSheet <> nil) then
  // check for a cycle
  begin
...
function TcxCustomStyles.Find(AIndex: Integer; var AItemIndex: Integer)
  : Boolean;
var
  L, H, I, C: Integer;
begin
...
begin
  ...
  if (Result = nil) and (StyleSheet <> nil) and
    ((GetOwner = nil) or not(GetOwner is TComponent) or
    // don't persist a stylesheet's style
...
begin
  ...
  begin
    ...
    if (AStylesRootOwner <> AValueOwner) and (AStylesRootOwner is TComponent)
      and (AValueOwner is TComponent) and AStylesRootOwner.InheritsFrom
      (AValueOwner.ClassType) then
      Value := TComponent(AStylesRootOwner).FindComponent(Value.Name)
        as TcxCustomStyle;
  end;
...
procedure TcxStyles.InternalGetViewParams(Index: Integer; AData: TObject;
  ...
  begin
    ...
    begin
      ...
      else if UseStyle(AAssignedStyle, svBitmap) then
  ...
  begin
    if UseStyle(AStyle, svColor) then
      Result := AStyle.Color
    else if UseStyle(AAssignedStyle, svColor) then
  ...
  begin
    ...
    else if UseStyle(AAssignedStyle, svFont) then
  ...
  begin
    ...
    else if UseStyle(AAssignedStyle, svTextColor) then
...
begin
  ...
  begin
    if FScaledFonts = nil then
      FScaledFonts := TObjectDictionary<TcxCustomStyle, TFont>.Create
        ([doOwnsValues]);
...
begin
  if ADestStyleSheet.GetStylesClass <> ASourceStyleSheet.GetStylesClass then
    Exit;
  ACount := GetPropList(ADestStyleSheet.GetStyles.ClassInfo, [tkClass],
    @APropList);
  for I := 0 to ACount - 1 do
    if GetTypeData(APropList[I].PropType^).ClassType = TcxStyle then
    begin
      ADestStyle := TcxStyle(GetObjectProp(ADestStyleSheet.GetStyles,
        dxShortStringToString(APropList[I].Name)));
      ASourceStyle := TcxStyle(GetObjectProp(ASourceStyleSheet.GetStyles,
        dxShortStringToString(APropList[I].Name)));
      if ASourceStyle <> nil then
      begin
        if ADestStyle <> nil then
          ADestStyle.Assign(ASourceStyle)
        else if (ADestStyleSheet.StyleRepository <> nil) and
          (ASourceStyle.AssignedValues <> []) then
        begin
          ADestStyle := TcxStyle(ADestStyleSheet.StyleRepository.CreateItemEx
            (TcxStyle, ADestStyleSheet.Owner));
...
begin
  ...
  for I := 0 to (Length(S) + AStringValueMaxLength - 1)
    div AStringValueMaxLength - 1 do
...
var
  ...
  procedure SaveStyleToIni(AIniFile: TCustomIniFile;
    const AStyleSheetCaption, AStyleName: string; AStyle: TcxStyle);
  var
    Value: string;
  begin
    ...
    if (svBitmap in AStyle.AssignedValues) and IsGlyphAssigned(AStyle.Bitmap)
    then
      SaveBitmapToIniFile(AIniFile, AStyle.Bitmap, AStyleSheetCaption,
        AStyleName + BitmapCaption);
  end;

  procedure SaveStyleSheetToIni(AStyleSheet: TcxCustomStyleSheet;
  ...
  var
    ...
    AStyle: TcxStyle;
  begin
    ACount := GetPropList(AStyleSheet.GetStyles.ClassInfo, [tkClass],
      @APropList);
    for I := 0 to ACount - 1 do
      if GetTypeData(APropList[I].PropType^).ClassType = TcxStyle then
      begin
        ...
        if AStyle <> nil then
          SaveStyleToIni(AIniFile, AStyleSheet.Caption,
            dxShortStringToString(APropList[I].Name), AStyle);
      end;
  end;
...
procedure LoadStyleSheetsFromIniFile(const AIniFileName: string;
  ...
  procedure LoadStyleFromIni(AIniFile: TCustomIniFile;
    AStyleSheet: TcxCustomStyleSheet; AStyleName: string;
    var AStyleNameIndex: Integer);
  var
  ...
  begin
    ...
    APropInfo := GetPropInfo(PTypeInfo(AStyleSheet.GetStyles.ClassInfo),
      AStyleName);
    if (APropInfo <> nil) and
      (GetTypeData(APropInfo.PropType^).ClassType = TcxStyle) then
    begin
      ...
      begin
        if AOwner <> nil then
          ...
          AStyle := TcxStyle(AStyleRepository.CreateItem(TcxStyle));
        if Assigned(AStyleGetName) then
  ...
  procedure LoadStyleSheetFromIni(const AStyleSheetName: string;
    AIniFile: TCustomIniFile);
  var
    ANames: TStringList;
    AStyleSheet: TcxCustomStyleSheet;
  ...
  begin
    ...
    try
      ...
      begin
        if AOwner <> nil then
          AStyleSheet := AStyleRepository.CreateStyleSheetEx
            (AStyleSheetClass, AOwner)
        else
          AStyleSheet := AStyleRepository.CreateStyleSheet(AStyleSheetClass);
