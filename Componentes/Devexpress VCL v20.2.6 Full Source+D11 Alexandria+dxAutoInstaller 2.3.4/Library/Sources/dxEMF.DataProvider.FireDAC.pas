...
{$IFDEF DELPHIXE5}
  FireDAC.Comp.Client, FireDAC.Stan.Param, FireDAC.Stan.Def, FireDAC.Stan.Intf,
  FireDAC.Phys.Intf, FireDAC.Comp.Script,
{$ELSE}
...
{$WARN UNSUPPORTED_CONSTRUCT OFF}
  ...
  strict private[HPPGEN('TFDQuery* FCommandExecutor;')]
  ...
  TdxFireDACConnectionVendor = class(TdxCustomConnectionVendor,
    IdxDBTransaction)
  strict private
  const
    SchemaInfo: array [TdxSchemaInfo] of TFDPhysMetaInfoKind = (mkTables,
      mkTables, mkProcs, mkTableFields, mkIndexes, mkIndexFields,
      mkPrimaryKeyFields, mkForeignKeys, mkForeignKeyFields);
  strict private
    class var FDriverNameAliases: TDictionary<string, TdxDBEngine>;
    class destructor Destroy;
    class procedure PopulateDriverNameAliases;
    class function GetDriverNameAliases
      : TDictionary<string, TdxDBEngine>; static;
    class function GetDriverNameAlias(const ADriverID: string)
      : TdxDBEngine; static;
    class function GetType(AType: TFDDataType): TdxDBColumnType; static;
  strict private[HPPGEN('TFDConnection* __fastcall GetConnection(void);')]
  ...
  protected
    ...
    class procedure AssignParameterValue(AParameter: TFDParam;
  ...
  public
    ...
    class property DriverNameAliases: TDictionary<string, TdxDBEngine>
      read GetDriverNameAliases;
  ...
  protected
    ...
    property ConnectionVendor: TdxFireDACConnectionVendor
      read GetConnectionVendor;
  published
...
constructor TdxFireDACCommand.Create(AConnection: TFDConnection;
  AQuery: TdxQuery);
begin
...
procedure TdxFireDACCommand.PrepareParameters(AConvertParameter
  : TdxConvertParameter);
var
  I: Integer;
  AParameter: TFDParam;
begin
  ...
  begin
    ...
    TdxFireDACConnectionVendor.AssignParameterValue(AParameter,
      AConvertParameter(Query.Parameters[I].Value));
  end;
end;
...
class procedure TdxFireDACConnectionVendor.AssignParameterValue
  (AParameter: TFDParam; const AValue: TValue);
begin
  case AValue.Kind of
    tkInteger:
...
{$ENDIF}
    ...
    tkFloat:
      if (AValue.TypeInfo = TypeInfo(TDateTime)) or
        (AValue.TypeInfo = TypeInfo(TDate)) or
    ...
    tkArray, tkDynArray:
      begin
        ...
        AParameter.SetBlobRawData(AValue.GetArrayLength,
          PPointer(AValue.GetReferenceToRawData)^);
      end;
  else
    AParameter.Value := AValue.ToVariant;
  end;
...
class function TdxFireDACConnectionVendor.GetDriverNameAliases
  : TDictionary<string, TdxDBEngine>;
begin
...
class function TdxFireDACConnectionVendor.GetDriverNameAlias(const ADriverID
  : string): TdxDBEngine;
begin
...
procedure TdxFireDACConnectionVendor.GetColumns(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
  AFieldName: TField;
...
begin
  ...
  try
    ...
    begin
      ...
      AColumn := TdxDBColumn.Create(AFieldName.AsString, False,
        AFieldTypeName.AsString, ASize, AType);
...
function TdxFireDACConnectionVendor.SelectData(AOwner: TComponent;
  AQuery: TdxQuery): TDataSet;
var
  AResult: TFDQuery;
begin
...
function TdxFireDACConnectionVendor.CreateCommand(AQuery: TdxQuery)
  : TdxCustomDBCommand;
begin
  Result := TdxFireDACCommand.Create(Connection, AQuery);
end;

function TdxFireDACConnectionVendor.CreateConnection(ASQLConnectionProvider
    ...
    try
      ...
      try
        AConnectionStrings.Values
          [ASQLConnectionProvider.GetConnectionParameter(GetConnectionType,
...
function TdxFireDACConnectionVendor.CreateDataSet(AQuery: TdxQuery;
...
procedure TdxFireDACConnectionVendor.GetForeignKeys(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
...
begin
  ...
  try
    ...
    begin
      ...
      ADataSetFields := GetSchema(TdxSchemaInfo.ForeignKeyFields, '',
        ASchemaName, ATableName, AForeignKeyName);
      try
        AForeignKey := TdxDBForeignKey.Create
          (ADataSet.FieldByName('PKEY_TABLE_NAME').AsString);
        ...
        begin
          ...
          AForeignKey.PrimaryKeyTableKeyColumns.Add
            (ADataSet.FieldByName('PKEY_COLUMN_NAME').AsString);
          ADataSetFields.Next;
        end;
...
procedure TdxFireDACConnectionVendor.GetIndexes(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
...
begin
  ...
  try
    ...
    try
      ...
      while not ADataSet.Eof do
      begin
        AIndexKind := TFDPhysIndexKind(ADataSet.FieldByName('INDEX_TYPE')
          .AsInteger);
        if AIndexKind = TFDPhysIndexKind.ikPrimaryKey then
          Continue;
        AIndex := TdxDBIndex.Create(AField.AsString, nil,
          AIndexKind = TFDPhysIndexKind.ikUnique);
    ...
    begin
      ADataSet := GetSchema(TdxSchemaInfo.IndexFields, '', ASchemaName,
        ATableName, AIndexes[I]);
      try
...
procedure TdxFireDACConnectionVendor.GetPrimaryKey(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
...
function TdxFireDACConnectionVendor.GetSchema(ASchemaInfo: TdxSchemaInfo;
...
begin
  ...
  case ASchemaInfo of
    TdxSchemaInfo.IndexFields, TdxSchemaInfo.PrimaryKeyFields,
      TdxSchemaInfo.ForeignKeyFields:
      begin
...
procedure TdxFireDACConnectionVendor.GetTableSchema(ATable: TdxDBTable;
...
class function TdxFireDACConnectionVendor.GetType(AType: TFDDataType)
  : TdxDBColumnType;
begin
  case AType of
    dtBoolean:
    ...
    dtAnsiString, dtWideString, dtByteString, dtMemo, dtWideMemo, dtXML,
      dtHMemo, dtWideHMemo:
...
function TdxFireDACConnectionVendor.ExecuteScalar
  (AConnection: TCustomConnection; const ACommandText: string): Variant;
begin
  Result := (AConnection as TFDConnection).ExecSQLScalar(ACommandText);
end;

function TdxFireDACConnectionVendor.Execute(const ACommandText: string)
  : Integer;
begin
...
function TdxEMFFireDACDataProvider.GetConnectionVendor
  : TdxFireDACConnectionVendor;
begin
...
function TdxEMFFireDACDataProvider.CreateConnectionVendor
...
begin
  ...
  ALoader := TdxFireDACLoader.Create(ASession,
