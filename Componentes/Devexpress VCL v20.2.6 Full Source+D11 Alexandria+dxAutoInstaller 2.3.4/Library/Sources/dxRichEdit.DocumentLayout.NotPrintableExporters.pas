...
type
  ...
  protected
    procedure DrawBorder(ABorder: TdxTableBorderViewInfoBase;
      const ACellBounds: TRect); virtual;
    function CreateDashPattern: TArray<Single>;
    function GetPoints(ABorder: TdxTableBorderViewInfoBase; X: Single;
      Y: Single; AHeight: Single; AWidth: Single): TArray<TdxPointF>;
  public
  ...
  public type
    TdxGetPointsDelegate = reference to function(const X, Y, AHeight: Single)
      : TArray<TdxPointF>;
  strict private
    ...
    FExporter: TdxNotPrintableGraphicsBoxExporter;
  protected
    procedure DrawBookmarkBox(AColor: TdxAlphaColor;
      const APoints: TArray<TdxPointF>);
    function GetBookmarkStartPoints(const X, Y, AHeight: Single)
      : TArray<TdxPointF>;
    function GetBookmarkEndPoints(const X, Y, AHeight: Single)
      : TArray<TdxPointF>;
    function GetBookmarkPoints(const X, Y, AHeight, AWidth: Single)
      : TArray<TdxPointF>;

    property Exporter: TdxNotPrintableGraphicsBoxExporter read FExporter;
  public
    constructor Create(AExporter: TdxNotPrintableGraphicsBoxExporter);
    procedure ExportBookmarkStartBox(ABookmarkBox
      : TdxVisitableDocumentIntervalBox);
    procedure ExportBookmarkEndBox(ABookmarkBox
      : TdxVisitableDocumentIntervalBox);
    procedure ExportBookmarkBoxCore(ABookmarkBox
      : TdxVisitableDocumentIntervalBox;
  ...
  protected
    ...
    procedure ExportParagraphFrames(AColumn: TdxColumn;
      const APredicate: TdxFunc<TdxParagraphFrameBox, Boolean>); override;
    ...
    procedure ExportBookmarkBoxesCore
      (ABoxes: TdxVisitableDocumentIntervalBoxCollection); virtual;
    procedure ExportCustomMarkBoxesCore
      (ABoxes: TdxCustomMarkBoxCollection); virtual;
    procedure ExportHiddenTextBoxes; virtual;
    procedure ExportSpecialTextBoxes; virtual;
    procedure ExportHiddenTextBox(ABox: TdxHiddenTextUnderlineBox;
      APen: TdxGPPen);
    procedure DrawHiddenTextBox(ALeft: Integer; ARight: Integer;
      ABottom: Integer; APen: TdxGPPen);
    procedure DrawHorizontalDotLine(ALeft: Integer; ARight: Integer; Y: Integer;
      APen: TdxGPPen);
    procedure DrawVerticalDotLine(ATop: Integer; ABottom: Integer; X: Integer;
      APen: TdxGPPen);
    ...
    procedure ExportSpecialTextBox(ABox: TdxSpecialTextBox; APen: TdxGPPen);
      overload; virtual;
  ...
  public
    ...
    procedure ExportBookmarkStartBox(ABookmarkBox
      : TdxVisitableDocumentIntervalBox); override;
    procedure ExportBookmarkEndBox(ABookmarkBox
      : TdxVisitableDocumentIntervalBox); override;
    procedure ExportCommentStartBox(ACommentBox
      : TdxVisitableDocumentIntervalBox); override;
    procedure ExportCommentEndBox(ACommentBox
      : TdxVisitableDocumentIntervalBox); override;
    ...
    function IsTableBorderInvisible
      (ABorder: TdxTableBorderViewInfoBase): Boolean;
  end;

  { TdxWinFormsNotPrintableGraphicsBoxExporter }

  TdxWinFormsNotPrintableGraphicsBoxExporter = class
    (TdxNotPrintableGraphicsBoxExporter)
  strict private
...
constructor TdxTableNotPrintableBorderExporter.Create
  (AExporter: TdxNotPrintableGraphicsBoxExporter);
begin
  inherited Create;
  PenWidth := 1;
...
procedure TdxTableNotPrintableBorderExporter.DrawBorder
...
begin
  ...
  APen := APainter.GetPen(TdxAlphaColors.FromArgb(255, 30, 144, 255),
    FExporter.PixelsToDrawingUnits(PenWidth));
  try
    APen.DashPattern := CreateDashPattern;
    ADrawingBounds := FExporter.GetActualBounds(ACellBounds);
    APoints := GetPoints(ABorder, ADrawingBounds.X, ADrawingBounds.Y,
      ADrawingBounds.Height, ADrawingBounds.Width);
...
begin
  Result := TArray<Single>.Create(FExporter.PixelsToDrawingUnits(3),
    FExporter.PixelsToDrawingUnits(1));
end;

function TdxTableNotPrintableBorderExporter.GetPoints
  (ABorder: TdxTableBorderViewInfoBase; X: Single; Y: Single; AHeight: Single;
  AWidth: Single): TArray<TdxPointF>;
begin
  if ABorder.BorderType and TdxBorderTypes.Left <> 0 then
    Result := TArray<TdxPointF>.Create(TdxPointF.Create(X, Y),
      TdxPointF.Create(X, Y + AHeight))
  else if ABorder.BorderType and TdxBorderTypes.Right <> 0 then
    Result := TArray<TdxPointF>.Create(TdxPointF.Create(X + AWidth, Y),
      TdxPointF.Create(X + AWidth, Y + AHeight))
  else if ABorder.BorderType and TdxBorderTypes.Top <> 0 then
    Result := TArray<TdxPointF>.Create(TdxPointF.Create(X, Y),
      TdxPointF.Create(X + AWidth, Y))
  else if ABorder.BorderType and TdxBorderTypes.Bottom <> 0 then
    Result := TArray<TdxPointF>.Create(TdxPointF.Create(X, Y + AHeight),
      TdxPointF.Create(X + AWidth, Y + AHeight))
  else
    Result := nil;
end;
...
constructor TdxBookmarkBoxExporter.Create
  (AExporter: TdxNotPrintableGraphicsBoxExporter);
begin
  inherited Create;
  BookmarkBoxWidth := 4;
...
procedure TdxBookmarkBoxExporter.ExportBookmarkStartBox
  (ABookmarkBox: TdxVisitableDocumentIntervalBox);
begin
  ExportBookmarkBoxCore(ABookmarkBox, GetBookmarkStartPoints);
end;

procedure TdxBookmarkBoxExporter.ExportBookmarkEndBox
  (ABookmarkBox: TdxVisitableDocumentIntervalBox);
begin
  ExportBookmarkBoxCore(ABookmarkBox, GetBookmarkEndPoints);
end;

procedure TdxBookmarkBoxExporter.ExportBookmarkBoxCore
  (ABookmarkBox: TdxVisitableDocumentIntervalBox;
  const AGetPoints: TdxGetPointsDelegate);
var
...
begin
  ...
  ABounds := TRect.Create(AHorizontalPosition, ATop,
    AHorizontalPosition, ABottom);
  ADrawingBounds := FExporter.GetActualBounds(ABounds);
  DrawBookmarkBox(ABookmarkBox.Color, AGetPoints(ADrawingBounds.X,
    ADrawingBounds.Y, ADrawingBounds.Height));
end;

procedure TdxBookmarkBoxExporter.DrawBookmarkBox(AColor: TdxAlphaColor;
  const APoints: TArray<TdxPointF>);
var
  APainter: TdxPainter;
  APen: TdxGPPen;
...
function TdxBookmarkBoxExporter.GetBookmarkStartPoints(const X, Y,
  AHeight: Single): TArray<TdxPointF>;
begin
  Result := GetBookmarkPoints(X, Y, AHeight,
    FExporter.PixelsToDrawingUnits(BookmarkBoxWidth));
end;

function TdxBookmarkBoxExporter.GetBookmarkEndPoints(const X, Y,
  AHeight: Single): TArray<TdxPointF>;
begin
  Result := GetBookmarkPoints(X, Y, AHeight,
    FExporter.PixelsToDrawingUnits(-BookmarkBoxWidth));
...
function TdxBookmarkBoxExporter.GetBookmarkPoints(const X, Y, AHeight,
  AWidth: Single): TArray<TdxPointF>;
var
...
begin
  ...
  Result := TArray<TdxPointF>.Create(TdxPointF.Create(ARight, ATop),
    TdxPointF.Create(ALeft, ATop), TdxPointF.Create(ALeft, ABottom),
    TdxPointF.Create(ARight, ABottom));
end;

{ TdxNotPrintableGraphicsBoxExporter }

constructor TdxNotPrintableGraphicsBoxExporter.Create(ADocumentModel
  : TdxDocumentModel; APainter: TdxPainter; AView: TdxRichEditView;
...
function TdxNotPrintableGraphicsBoxExporter.GetUnitConverter
  : TdxDocumentLayoutUnitConverter;
begin
  Result := DocumentModel.LayoutUnitConverter;
end;

function TdxNotPrintableGraphicsBoxExporter.CreateBookmarkBoxExporter
  : TdxBookmarkBoxExporter;
begin
  Result := TdxBookmarkBoxExporter.Create(Self);
end;

function TdxNotPrintableGraphicsBoxExporter.CreateTableBorderExporter
  : TdxTableNotPrintableBorderExporter;
begin
  Result := TdxTableNotPrintableBorderExporter.Create(Self);
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportPageArea
  (APageArea: TdxPageArea);
begin
  APageArea.Columns.ExportTo(Self);
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportTablesBackground
  (AColumn: TdxColumn);
begin
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportParagraphFrames
  (AColumn: TdxColumn;
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportBookmarkBoxesCore
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportCustomMarkBoxesCore
  (ABoxes: TdxCustomMarkBoxCollection);
var
  ACount, I: Integer;
begin
  ACount := ABoxes.Count;
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportHiddenTextBox
  (ABox: TdxHiddenTextUnderlineBox; APen: TdxGPPen);
var
  ABottom: Integer;
  ABounds, ADrawBounds: TRect;
begin
  ABottom := CurrentRow.Bounds.Top + CurrentRow.BaseLineOffset +
    ABox.BottomOffset;
  ABounds := TRect.Create(ABox.Start, ABottom, ABox.&End, ABottom);
  ADrawBounds := GetActualBounds(ABounds);
  DrawHiddenTextBox(ADrawBounds.Left, ADrawBounds.Right,
    ADrawBounds.Bottom, APen);
end;

procedure TdxNotPrintableGraphicsBoxExporter.DrawHiddenTextBox(ALeft: Integer;
  ARight: Integer; ABottom: Integer; APen: TdxGPPen);
begin
  DrawHorizontalDotLine(ALeft, ARight, ABottom, APen);
end;

procedure TdxNotPrintableGraphicsBoxExporter.DrawHorizontalDotLine
  (ALeft: Integer; ARight: Integer; Y: Integer; APen: TdxGPPen);
var
  I: Single;
begin
  I := ALeft;
...
procedure TdxNotPrintableGraphicsBoxExporter.DrawVerticalDotLine(ATop: Integer;
  ABottom: Integer; X: Integer; APen: TdxGPPen);
var
  I: Single;
begin
  I := ATop;
...
function TdxNotPrintableGraphicsBoxExporter.GetActualCustomMarkBounds
  (const ABounds: TRect): TRect;
begin
  Result := GetActualBounds(ABounds);
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportCustomMarkBoxCore
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportSpecialTextBox
  (ABox: TdxSpecialTextBox; APen: TdxGPPen);
var
  ABounds: TRect;
begin
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportBookmarkStartBox
  (ABookmarkBox: TdxVisitableDocumentIntervalBox);
begin
  FBookmarkExporter.ExportBookmarkStartBox(ABookmarkBox);
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportBookmarkEndBox
  (ABookmarkBox: TdxVisitableDocumentIntervalBox);
begin
  FBookmarkExporter.ExportBookmarkEndBox(ABookmarkBox);
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportCommentStartBox
  (ACommentBox: TdxVisitableDocumentIntervalBox);
begin
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportCommentEndBox
  (ACommentBox: TdxVisitableDocumentIntervalBox);
begin
end;

procedure TdxNotPrintableGraphicsBoxExporter.ExportCustomMarkBox
  (ABox: TdxCustomMarkBox);
begin
  ExportCustomMarkBoxCore(ABox);
end;

function TdxNotPrintableGraphicsBoxExporter.ExportRotatedContent
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportFloatingObjectTextBox
...
procedure TdxNotPrintableGraphicsBoxExporter.ExportTableBorder
  (ABorder: TdxTableBorderViewInfoBase; const ACellBounds: TRect);
var
  AShouldDrawGridLinesForAllTable: Boolean;
begin
  AShouldDrawGridLinesForAllTable :=
    (DocumentModel.TableOptions.GridLines = TdxRichEditTableGridLinesVisibility.
    Visible) and IsTableBorderInvisible(ABorder);
  if AShouldDrawGridLinesForAllTable then
    FTableBorderExporter.DrawBorder(ABorder, ACellBounds);
end;

function TdxNotPrintableGraphicsBoxExporter.IsTableBorderInvisible
  (ABorder: TdxTableBorderViewInfoBase): Boolean;
var
  ABorderLineStyle: TdxBorderLineStyle;
begin
  ABorderLineStyle := ABorder.Border.Style;
  Result := (ABorderLineStyle = TdxBorderLineStyle.None) or
    (ABorderLineStyle = TdxBorderLineStyle.Nil) or
    (ABorderLineStyle = TdxBorderLineStyle.Disabled);
end;

procedure TdxNotPrintableGraphicsBoxExporter.SetClipBounds(const AClipBounds
  : TdxRectF);
var
...
begin
  ...
  AWidth := UnitConverter.LayoutUnitsToPixelsF(AClipBounds.Width,
    DocumentModel.DpiX);
  AHeight := UnitConverter.LayoutUnitsToPixelsF(AClipBounds.Height,
    DocumentModel.DpiY);
  AScaleFactor := FView.ScaleFactor;
  Painter.ClipBounds := TdxRectF.CreateSize(X * AScaleFactor, Y * AScaleFactor,
    AWidth * AScaleFactor, AHeight * AScaleFactor);
end;
...
begin
  ...
  AWidth := UnitConverter.PixelsToLayoutUnitsF(AClipBounds.Width,
    DocumentModel.DpiX);
  AHeight := UnitConverter.PixelsToLayoutUnitsF(AClipBounds.Height,
    DocumentModel.DpiY);
  Result := TdxRectF.CreateSize(X / AScaleFactor, Y / AScaleFactor,
    AWidth / AScaleFactor, AHeight / AScaleFactor);
end;

{ TdxWinFormsNotPrintableGraphicsBoxExporter }

constructor TdxWinFormsNotPrintableGraphicsBoxExporter.Create
...
procedure TdxWinFormsNotPrintableGraphicsBoxExporter.ExportPage
...
function TdxWinFormsNotPrintableGraphicsBoxExporter.GetActualBounds
  (const ABounds: TRect): TRect;
var
  AScaleFactor: Single;
  ARect: TRect;
...
begin
  ...
  ARect := TRect.CreateSize(Trunc(ABounds.X * AScaleFactor),
    Trunc(ABounds.Y * AScaleFactor), Trunc(ABounds.Width * AScaleFactor),
    Trunc(ABounds.Height * AScaleFactor));
  ADpi := Painter.DpiY;
  Result := UnitConverter.LayoutUnitsToPixels(ARect, ADpi, ADpi);
end;

function TdxWinFormsNotPrintableGraphicsBoxExporter.GetActualCustomMarkBounds
  (const ABounds: TRect): TRect;
var
  AResult: TRect;
  ADpi: Integer;
...
function TdxWinFormsNotPrintableGraphicsBoxExporter.PixelsToDrawingUnits
