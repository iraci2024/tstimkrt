...
type
  ...
  public
    ...
    function GetFirstPosition(APieceTable: TdxCustomPieceTable)
      : TdxDocumentModelPosition; virtual; abstract;
  ...
  strict private
    ...
    FBounds: TRect;
  public
    constructor Create(AFirstBoxIndex, ALastBoxIndex: Integer;
      const ABounds: TRect);
  ...
  TdxRowBoxRangeCollection = class sealed(TdxObjectList<TdxRowBoxRange>);
    ...
    TdxBoxInfo = class // for internal use
      strict private FIteratorResult: TdxParagraphIteratorResult;
  ...
  public
    ...
    property IteratorResult: TdxParagraphIteratorResult read FIteratorResult
      write FIteratorResult;
...
begin
  Result := APieceTable.DocumentModel.FontCache
    [GetRun(APieceTable).FontCacheIndex];
end;
...
constructor TdxHighlightArea.Create(AColor: TdxAlphaColor;
  const ABounds: TRect);
begin
  FBounds := ABounds;
  FColor := AColor;
...
constructor TdxRowBoxRange.Create(AFirstBoxIndex, ALastBoxIndex: Integer;
  const ABounds: TRect);
begin
  inherited Create;
...
constructor TdxBoxLogPositionComparable.Create(APieceTable: TdxCustomPieceTable;
  ALogPosition: TdxDocumentLogPosition);
begin
...
function TdxBoxLogPositionComparable.BoxStartAndLogPositionCompare
...
function TdxBoxLogPositionComparable.BoxAndLogPositionCompare
  (const Value: TdxBoxBase): Integer;
var
...
constructor TdxBoxAndFormatterPositionComparable.Create(const AFormatterPosition
  : TdxFormatterPosition);
begin
  FFormatterPosition := AFormatterPosition;
end;

function TdxBoxAndFormatterPositionComparable.CompareTo
  (const Value: TdxBoxBase): Integer;
var
...
begin
  ...
  if FFormatterPosition < AFirstPos then
    Result := 1
  else if FFormatterPosition > AFirstPos then
  begin
