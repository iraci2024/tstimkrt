...
uses
  ...
  dxCore, dxCoreClasses, cxClasses, cxControls, cxCustomData, cxDataStorage,
...
type
  ...
  protected
    ...
    function GetAdapter(AItemHandle: TcxDataItemHandle;
      out Adapter: TcxSchedulerFieldAdapter): Boolean;
  ...
  public
    constructor CreateWithItem(AConnection: TcxSchedulerGridConnection;
  ...
  TcxSchedulerInitializeGridItemEvent = procedure
    (Sender: TcxSchedulerGridConnection; AField: TcxCustomSchedulerStorageField;
    AItem: TcxCustomGridTableItem; Adapter: TcxSchedulerFieldAdapter) of object;
  TcxSchedulerGetFieldAdapterEvent = procedure
    (Sender: TcxSchedulerGridConnection; AField: TcxCustomSchedulerStorageField;
    var AdapterClass: TcxSchedulerFieldAdapterClass) of object;
  TcxSchedulerConnectionEditEventEvent = procedure
    (Sender: TcxSchedulerGridConnection; AEvent: TcxSchedulerControlEvent;
    var AModified: Boolean; var AHandled: Boolean) of object;
  TcxSchedulerConnectionGridViewDblClickEvent = procedure(Sender: TObject;
    var AHandled: Boolean) of object;
  TcxSchedulerConnectionNewEventEvent = procedure
    (Sender: TcxSchedulerGridConnection; var AEvent: TcxSchedulerControlEvent;
    var AHandled: Boolean) of object;
  TcxSchedulerConnectionInitEditDialogEvent = procedure
    (Sender: TcxSchedulerGridConnection;
    var AEditInfo: TcxSchedulerEventEditInfo) of object;

  TcxSchedulerGridConnection = class(TcxCustomComponent, IUnknown,
  ...
  protected
    ...
    function AdapterByItem(AItem: TcxCustomGridTableItem)
      : TcxSchedulerFieldAdapter;
    ...
    function AddAdapterForCustomField(AField: TcxCustomSchedulerStorageField)
      : TcxSchedulerFieldAdapter;
    ...
    procedure DoInitializeItem(AField: TcxCustomSchedulerStorageField;
      ...
      AAdapter: TcxSchedulerFieldAdapter); virtual;
  ...
  published
    property GridPopupMenu: TcxSchedulerGridBuildInPopupMenu read FGridPopupMenu
      write SetGridPopupMenu;
    property GridPopupMenuEvents: TNotifyEvent read FGridPopupMenuEvents
      write FGridPopupMenuEvents;
    ...
    property OnEditEvent: TcxSchedulerConnectionEditEventEvent read FOnEditEvent
      write FOnEditEvent;
    property OnGetFieldAdapter: TcxSchedulerGetFieldAdapterEvent
      read FOnGetFieldAdapter write FOnGetFieldAdapter;
    property OnGridViewDblClick: TcxSchedulerConnectionGridViewDblClickEvent
      read FOnGridViewDblClick write FOnGridViewDblClick;
    property OnInitializeItem: TcxSchedulerInitializeGridItemEvent
      read FOnInitializeItem write FOnInitializeItem;
    property OnInitEditDialog: TcxSchedulerConnectionInitEditDialogEvent
      read FOnInitEditDialog write FOnInitEditDialog;
    property OnNewEvent: TcxSchedulerConnectionNewEventEvent read FOnNewEvent
      write FOnNewEvent;
  end;
  ...
  TcxSchedulerGridBuildInPopupMenuItem = (bpmiNewEvent, bpmiNewAllDayEvent,
    bpmiNewReccuringEvent, bpmiOpen, bpmiEditSeries, bpmiShowTimeAs, bpmiLabel,
    bpmiDelete);
  TcxSchedulerGridBuildInPopupMenuItems = set of
    TcxSchedulerGridBuildInPopupMenuItem;

  TcxSchedulerGridBuildInPopupMenuPopupEvent = procedure
    (Sender: TcxSchedulerGridBuildInPopupMenu; ABuiltInMenu: TPopupMenu;
    var AHandled: Boolean) of object;
  TcxSchedulerGridBuildInPopupMenuClickEvent = procedure
    (Sender: TcxSchedulerGridBuildInPopupMenu;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property Items: TcxSchedulerGridBuildInPopupMenuItems read FItems
      write FItems default [bpmiNewEvent, bpmiNewAllDayEvent,
      bpmiNewReccuringEvent, bpmiOpen, bpmiEditSeries, bpmiShowTimeAs,
      bpmiLabel, bpmiDelete];
    ...
    property OnClick: TcxSchedulerGridBuildInPopupMenuClickEvent read FOnClick
      write FOnClick;
    property OnPopup: TcxSchedulerGridBuildInPopupMenuPopupEvent read FOnPopup
      write FOnPopup;
  end;
  ...
  private
    ...
    procedure UnpackCommand(ACommand: Integer;
      out AItem: TcxSchedulerGridBuildInPopupMenuItem;
      out ASubItemIndex: Integer);
  protected
    ...
    property BuildInPopupMenu: TcxSchedulerGridBuildInPopupMenu
      read GetBuildInPopupMenu;
...
type
  ...
  public
    constructor Create(AOwner: TcxSchedulerGridConnection);
...
procedure TcxSchedulerStorageDataSource.DeleteRecord(ARecordHandle
  : TcxDataRecordHandle);
var
  AEvent: TcxSchedulerEvent;
begin
...
function TcxSchedulerStorageDataSource.GetRecordHandle(ARecordIndex: Integer)
  : TcxDataRecordHandle;
begin
...
function TcxSchedulerStorageDataSource.InsertRecord(ARecordHandle
  : TcxDataRecordHandle): TcxDataRecordHandle;
begin
  Result := AddNewEvent;
end;

procedure TcxSchedulerStorageDataSource.SetValue(ARecordHandle
  : TcxDataRecordHandle; AItemHandle: TcxDataItemHandle; const AValue: Variant);
var
  Adapter: TcxSchedulerFieldAdapter;
begin
...
constructor TcxSchedulerFieldAdapter.Create(AConnection
  : TcxSchedulerGridConnection; ALink: TCollectionItem);
begin
  inherited Create;
...
begin
  if (FItem <> nil) and
    ([csDestroying, csLoading] * (FItem.ComponentState +
    Connection.ComponentState) = []) then
    FreeAndNil(FItem);
end;
...
function TcxSchedulerFieldAdapter.GetEvent(ARecordIndex: Integer)
  : TcxSchedulerEvent;
begin
  Result := Storage.Events[ARecordIndex];
end;

function TcxSchedulerFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxTextEditProperties;
end;

function TcxSchedulerFieldAdapter.GetPropertiesForEditClass
  : TcxCustomEditPropertiesClass;
begin
  Result := nil;
end;
...
begin
  if csDesigning in Connection.ComponentState then
    Exit;
  ...
  if (FEditProperties <> nil) and not Assigned(FItem.OnGetPropertiesForEdit)
  then
    FItem.OnGetPropertiesForEdit := DoGetPropertiesForEdit;
end;

procedure TcxSchedulerFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
...
procedure TcxSchedulerNullAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
end;
...
function TcxSchedulerIntegerFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
function TcxSchedulerTextFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
function TcxSchedulerCheckFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
function TcxSchedulerDateTimeFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
function TcxSchedulerEventStartFieldAdapter.GetValue
...
function TcxSchedulerEventFinishFieldAdapter.GetValue
  (AEvent: TcxSchedulerEvent): Variant;
begin
...
procedure TcxSchedulerImageComboFieldAdapter.AddItems(const AItemsValues
  : array of Variant);
var
  AIndex: Integer;
begin
...
function TcxSchedulerImageComboFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxImageComboBoxProperties;
end;

function TcxSchedulerImageComboFieldAdapter.GetProperties
...
function TcxSchedulerIDFieldAdapter.GetValue(AEvent: TcxSchedulerEvent)
  : Variant;
begin
  if Field = nil then
    Result := AEvent.ID
...
procedure TcxSchedulerIDFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
end;
...
function TcxSchedulerGroupIDFieldAdapter.GetValue
  (AEvent: TcxSchedulerEvent): Variant;
begin
...
begin
  ...
  try
    ...
    AddItems([cxGetResourceString(@scxNone), Integer(etNone), -1, // MaxInt,
      ...
      cxGetResourceString(@scxOccurrence), Integer(etOccurrence), -1,
      // MaxInt - 1,
      cxGetResourceString(@scxException), Integer(etException), -1,
      // MaxInt - 2,
...
procedure TcxSchedulerAllDayFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
...
procedure TcxSchedulerEnabledFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
...
procedure TcxSchedulerReminderFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
...
function TcxSchedulerLabelColorFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
procedure TcxSchedulerLabelColorFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
...
function TcxSchedulerRecurringFieldAdapter.GetValue
  (AEvent: TcxSchedulerEvent): Variant;
begin
...
begin
  AddItems([cxGetResourceString(@scxFree), tlsFree, 0,
...
function TcxSchedulerResourceIDFieldAdapter.GetValueTypeClass
  : TcxValueTypeClass;
begin
  Result := TcxStringValueType;
end;
...
function TcxSchedulerCompleteFieldAdapter.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxProgressBarProperties;
end;

function TcxSchedulerCompleteFieldAdapter.GetPropertiesForEditClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxSpinEditProperties;
end;

function TcxSchedulerCompleteFieldAdapter.GetValue
  (AEvent: TcxSchedulerEvent): Variant;
begin
  Result := inherited GetValue(AEvent);
...
procedure TcxSchedulerCompleteFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
...
procedure TcxSchedulerStatusFieldAdapter.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
  if VarIsNull(AValue) then
...
constructor TcxSchedulerFieldAdapters.Create
  (AOwner: TcxSchedulerGridConnection);
begin
...
function TcxSchedulerFieldAdapterItemLink.GetConnection
  : TcxSchedulerGridConnection;
begin
...
begin
  if IsLocked then
    ...
    FLink := Connection.AddAdapterEx(Self,
      TcxSchedulerFieldAdapterClass(FindClass(AValue)));
end;
...
procedure TcxSchedulerFieldAdapterItemLink.SetItem
  (AValue: TcxCustomGridTableItem);
begin
  if IsLocked then
  begin
    FItemIndex := -1;
...
function TcxSchedulerGridConnection.AddNewEvent(ARecurrence: Boolean = False;
  AnAllDayEvent: Boolean = False): TcxSchedulerEvent;
var
...
function TcxSchedulerGridConnection.AdapterByItem(AItem: TcxCustomGridTableItem)
  : TcxSchedulerFieldAdapter;
var
  I: Integer;
begin
...
function TcxSchedulerGridConnection.AddAdapter
  (AField: TcxCustomSchedulerStorageField;
  AdapterClass: TcxSchedulerFieldAdapterClass; ACaption: Pointer;
  AVisible, ACanEdit: Boolean): TcxSchedulerFieldAdapter;
begin
...
function TcxSchedulerGridConnection.CreateDataSource()
  : TcxSchedulerStorageDataSource;
begin
...
function TcxSchedulerGridConnection.DoGridViewDblClick(Sender: TObject)
  : Boolean;
begin
  Result := False;
...
procedure TcxSchedulerGridConnection.DoEditEventUsingDialog
  (AEvent: TcxSchedulerEvent; ARecurrence: Boolean = False;
  AReadOnly: Boolean = False; AForcePatternEditing: Boolean = False);
var
...
begin
  ...
  try
    ...
    AInfo.ForcePatternEditing := AInfo.ForcePatternEditing or
...
procedure TcxSchedulerGridConnection.EditingGridHandler
...
begin
  ...
  begin
    AEvent := TcxSchedulerEvent
      (DataController.CustomDataSource.GetRecordHandleByIndex
      (Sender.Controller.FocusedRecordIndex));
    AAllow := not Storage.EditingEventInfoList.IsEventEditing(AEvent,
      AEvent.RecurrenceIndex, AEvent.EventType);
  end;
end;
...
function TcxSchedulerGridConnection.GetEventEditInfo
  (AEvent: TcxSchedulerControlEvent; ARecurrence: Boolean = False)
...
begin
  if (GridView.DataController.RecordCount = 0) or
    (GridView.Controller.FocusedRecordIndex < 0) then
    Result := nil
  else
...
begin
  Result := (AField = nil) or TStorageFieldAccess(AField).GetIsActive;
end;

procedure TcxSchedulerGridConnection.Notification(AComponent: TComponent;
...
procedure TcxSchedulerGridConnection.OpenEventEditDialog(AForcePatternEditing
  : Boolean = False; AReadOnly: Boolean = False);
begin
...
var
  I: Integer;
begin
  if (Storage = nil) or (GridView = nil) or (Count <> 0) then
    Exit;
  if IsLoading then
    Exit;
  ...
  try
    try
      ...
      begin
        ...
        AddAdapter(FParentIDField, TcxSchedulerIDFieldAdapter,
          @scxParentIDField, False, False);
        AddAdapter(FGroupIDField, TcxSchedulerGroupIDFieldAdapter,
          @scxGroupIDField, False, False);

        AddAdapter(FStateField, TcxSchedulerStateFieldAdapter, @scxStateField,
          True, True);
        AddAdapter(FCaptionField, TcxSchedulerTextFieldAdapter,
          @scxCaptionField, True, True);
        AddAdapter(FLocationField, TcxSchedulerTextFieldAdapter,
          @scxLocationField, True, True);
        AddAdapter(FActualFinishField, TcxSchedulerDateTimeFieldAdapter,
          @scxActualFinishField, False, False);
        AddAdapter(FActualStartField, TcxSchedulerDateTimeFieldAdapter,
          @scxActualStartField, False, False);
        AddAdapter(FFinishField, TcxSchedulerEventFinishFieldAdapter,
          @scxFinishField, True, True);
        AddAdapter(FStartField, TcxSchedulerEventStartFieldAdapter,
          @scxStartField, True, True);
        AddAdapter(FMessageField, TcxSchedulerTextFieldAdapter,
          @scxMessageField, False, True);
        AddAdapter(FEventTypeField, TcxSchedulerTypeFieldAdapter,
          @scxEventTypeField, False, False);

        AddAdapter(FOptionsField, TcxSchedulerReminderFieldAdapter,
          @scxReminderField, True, True);
        AddAdapter(FOptionsField, TcxSchedulerAllDayFieldAdapter,
          @scxAllDayEventField, True, True);
        AddAdapter(FOptionsField, TcxSchedulerEnabledFieldAdapter,
          @scxEnabledField, True, True);

        AddAdapter(FLabelColorField, TcxSchedulerLabelColorFieldAdapter,
          @scxLabelField, True, True);

        AddAdapter(FRecurrenceInfoField, TcxSchedulerRecurringFieldAdapter,
          @scxRecurrenceField, True, False);
        AddAdapter(FRecurrenceIndexField, TcxSchedulerIntegerFieldAdapter,
          @scxRecurrenceIndexField, False, False);
        AddAdapter(FReminderDateField, TcxSchedulerDateTimeFieldAdapter,
          @scxReminderDateField, False, False);
        AddAdapter(FReminderMinutesBeforeStartField,
          TcxSchedulerIntegerFieldAdapter, @scxReminderMinutesBeforeStartField,
          False, False);

        AddAdapter(FResourceIDField, TcxSchedulerResourceIDFieldAdapter,
          @scxResourceField, False, False);

        AddAdapter(FTaskCompleteField, TcxSchedulerCompleteFieldAdapter,
          @scxTaskCompleteField, False, True);
        AddAdapter(FTaskLinksField, TcxSchedulerLinksFieldAdapter,
          @scxTaskLinksField, False, False);
        AddAdapter(FTaskIndexField, TcxSchedulerIntegerFieldAdapter,
          @scxTaskIndexField, False, False);
        AddAdapter(FTaskStatusField, TcxSchedulerStatusFieldAdapter,
          @scxTaskStatusField, False, True);
...
function TcxSchedulerGridConnection.GetAdapter(AIndex: Integer)
  : TcxSchedulerFieldAdapter;
begin
...
function TcxSchedulerGridConnection.GetEvent(ARowIndex: Integer)
  : TcxSchedulerEvent;
begin
...
begin
  if not AValue and IsLoading then
    Exit;
  ...
  if IsLoading and not AValue then
    Exit;
  if not Active then
    FAdapters.Clear
  else
...
procedure TcxSchedulerGridConnection.SetGridView
  (AValue: TcxCustomGridTableView);
begin
  if AValue = GridView then
    Exit;
  Active := False;
  if GridView <> nil then
  begin
...
procedure TcxSchedulerGridConnection.SetStorage
...
constructor TcxSchedulerGridBuildInPopupMenu.Create(ASchedulerGridConnection
  : TcxSchedulerGridConnection);
begin
  FOwner := ASchedulerGridConnection;
  FUseBuiltInPopupMenu := True;
  FItems := [bpmiNewEvent, bpmiNewAllDayEvent, bpmiNewReccuringEvent, bpmiOpen,
    bpmiEditSeries, bpmiShowTimeAs, bpmiLabel, bpmiDelete];
end;
...
begin
  ...
  begin
    ...
    UseBuiltInPopupMenu := TcxSchedulerGridBuildInPopupMenu(Source)
      .UseBuiltInPopupMenu;
...
function TcxSchedulerGridBuildInPopupMenuController.Popup(X,
  Y: Integer): Boolean;
var
  AGrid: TcxGrid;
begin
  Result := False;
  SetEvent;
  if not IsPopupMenuPossible(FOwner.GridView, AGrid) { or (FEvent = nil) } then
    Exit;
  CreateInternalMenu;
  Result := DoOnPopup;
  if not Result then
...
begin
  if bpmiNewEvent in Items then
    CreateSubItem(FInternalMenu.Items, cxGetResourceString(@scxpmNewEvent),
      Ord(bpmiNewEvent), 2);
  ...
  if (bpmiNewReccuringEvent in Items) and (Storage.IsRecurrenceAvailable) then
    CreateSubItem(FInternalMenu.Items,
      cxGetResourceString(@scxpmNewRecurringEvent),
      Ord(bpmiNewReccuringEvent), 3);
...
function TcxSchedulerGridBuildInPopupMenuController.CreateSubItem
  (AOwner: TMenuItem; const ACaption: string; ACommand: Integer = -1;
  AImageIndex: Integer = -1; AEnabled: Boolean = True;
...
function TcxSchedulerGridBuildInPopupMenuController.DoOnClick
...
function TcxSchedulerGridBuildInPopupMenuController.IsValidCommand
  (ACommand: Integer): Boolean;
begin
...
procedure TcxSchedulerGridBuildInPopupMenuController.OnItemClickHandler
  (Sender: TObject);
var
  ACommand: Integer;
begin
  ...
  begin
    ...
    if IsValidCommand(ACommand) and not DoOnClick(ACommand) then
      case ACommand of
        Ord(bpmiNewEvent):
          NewEvent;
        Ord(bpmiNewAllDayEvent):
          NewAllDayEvent;
        Ord(bpmiNewReccuringEvent):
          NewRecurrence;
        Ord(bpmiDelete):
          DeleteEvent;
        Ord(bpmiOpen):
...
begin
  if not(bpmiShowTimeAs in Items) or not Storage.IsStateAvailable then
    Exit;
...
function TcxSchedulerGridBuildInPopupMenuController.GetBuildInPopupMenu
  : TcxSchedulerGridBuildInPopupMenu;
begin
...
function TcxSchedulerGridBuildInPopupMenuController.GetItems
  : TcxSchedulerGridBuildInPopupMenuItems;
begin
  Result := BuildInPopupMenu.Items;
end;

function TcxSchedulerGridBuildInPopupMenuController.GetStorage
  : TcxCustomSchedulerStorage;
begin
  Result := FOwner.Storage;
end;

procedure TcxSchedulerGridBuildInPopupMenuController.GridMenuPopup
...
begin
  ...
  begin
    with FOwner.GridView do
      Result := GetStorage.Events
        [DataController.GetRowInfo(Controller.SelectedRecords[I].Index)
        .RecordIndex].ReadOnly;
    Inc(I);
  end;
end;
...
procedure TcxSchedulerGridBuildInPopupMenuController.SetEventLabelColor
  (AColor: Integer);
begin
  if FEvent = nil then
    Exit;
...
procedure TcxSchedulerGridBuildInPopupMenuController.SetEventState
  (AState: Integer);
begin
  if FEvent = nil then
    Exit;
...
procedure TcxSchedulerGridBuildInPopupMenuController.UnpackCommand
  (ACommand: Integer; out AItem: TcxSchedulerGridBuildInPopupMenuItem;
  out ASubItemIndex: Integer);
begin
  AItem := bpmiOpen;
  ASubItemIndex := -1;
