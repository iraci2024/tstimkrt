...
uses
  ...
  dxCore, cxClasses, dxGDIPlusClasses, cxLookAndFeels, dxBar,
  dxBuiltInPopupMenu, cxGraphics;
...
type
  ...
  public
    ...
    function CreateMenuItem(AOwner: TComponent; const ACaption: string;
      ...
      AItemType: TdxBarBuiltInMenuItemType = bmitDefault;
      AChecked: Boolean = False; AImageIndex: Integer = -1;
      AWithSeparator: Boolean = False; AGlyph: TdxSmartGlyph = nil;
      AClickEvent: TNotifyEvent = nil; AShortCut: TShortCut = 0): TComponent;
...
function TdxBarBuiltInMenuHelper.CreateMenuItem(AOwner: TComponent;
  const ACaption: string; ACommand: TcxTag; AEnabled: Boolean = True;
  AItemType: TdxBarBuiltInMenuItemType = bmitDefault; AChecked: Boolean = False;
  AImageIndex: Integer = -1; AWithSeparator: Boolean = False;
  AGlyph: TdxSmartGlyph = nil; AClickEvent: TNotifyEvent = nil;
  AShortCut: TShortCut = 0): TComponent;
const
  ItemClassMap: array [Boolean] of TdxBarItemClass = (TdxBarButton,
    TdxBarSubItem);
var
  ALinksOwner: IdxBarLinksOwner;
  ALink: TdxBarItemLink;
begin
  if Supports(AOwner, IdxBarLinksOwner, ALinksOwner) and
    (ALinksOwner.GetItemLinks <> nil) then
  begin
    ALink := BarDesignController.AddInternalItem(ALinksOwner.GetItemLinks,
      ItemClassMap[AItemType = bmitSubItem], nil, FItems);
...
procedure TdxBarBuiltInMenuHelper.Notification(AComponent: TComponent;
...
function TdxBarBuiltInPopupMenuAdapter.Add(const ACaption: string;
...
begin
  ...
  if AParentItem <> nil then
    Result := dxBarPopupHelper.CreateMenuItem(AParentItem, ACaption, ATag,
      AEnabled, bmitDefault, False, AImageIndex, FSeparatorRequired, nil,
      AClickEvent)
  else
    Result := dxBarPopupHelper.CreateMenuItem(dxBarPopupHelper.PopupMenu,
      ACaption, ATag, AEnabled, bmitDefault, False, AImageIndex,
      FSeparatorRequired, nil, AClickEvent);
  FSeparatorRequired := False;
end;

function TdxBarBuiltInPopupMenuAdapter.AddSubMenu(const ACaption: string;
  AClickEvent: TNotifyEvent; ATag: TcxTag = 0; AImageIndex: TcxImageIndex = -1;
  AEnabled: Boolean = True; AShortCut: TShortCut = 0;
  AParentItem: TComponent = nil): TComponent;
begin
  ...
  if AParentItem <> nil then
    Result := dxBarPopupHelper.CreateMenuItem(AParentItem, ACaption, ATag,
      AEnabled, bmitSubItem, False, AImageIndex, FSeparatorRequired, nil,
      AClickEvent)
  else
    Result := dxBarPopupHelper.CreateMenuItem(dxBarPopupHelper.PopupMenu,
      ACaption, ATag, AEnabled, bmitSubItem, False, AImageIndex,
      FSeparatorRequired, nil, AClickEvent);
  FSeparatorRequired := False;
end;
...
procedure TdxBarBuiltInPopupMenuAdapter.SetAction(AItem: TComponent;
  AValue: TBasicAction);
begin
  (AItem as TdxBarItem).Action := AValue;
end;

procedure TdxBarBuiltInPopupMenuAdapter.SetChecked(AItem: TComponent;
  AValue: Boolean);
var
  AButton: TdxBarButton;
begin
...
procedure TdxBarBuiltInPopupMenuAdapter.SetDefault(AItem: TComponent;
  AValue: Boolean);
begin
  // do nothing
end;

procedure TdxBarBuiltInPopupMenuAdapter.SetGlyph(AItem: TComponent;
  AGlyph: TGraphic);
begin
...
procedure TdxBarBuiltInPopupMenuAdapter.SetImages(AItem: TComponent;
