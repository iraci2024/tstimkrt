...
type
  ...
  public
    ...
    function IsValid: Boolean; {$IFDEF DELPHIXE}inline; {$ENDIF}
  end;
  ...
  public
    ...
    case Integer of
      ...
      1:
        (TopLeft, BottomRight: TdxMapControlGeoPoint);
      2:
        (NorthLatitude, WestLongitude, SouthLatitude, EastLongitude: Double);
  end;
...
const
  dxMapControlInvalidTileIndex: TdxMapControlTileIndex = (X: - 1; Y: - 1;
    Level: - 1);
...
{$HPPEMIT    'static const Extended dxMapControlInvalidLongitude = NaN;'}
  dxMapControlInvalidGeoPoint: TdxMapControlGeoPoint =
    (Latitude: dxMapControlInvalidLatitude;
    Longitude: dxMapControlInvalidLongitude);
  dxMapControlInvalidGeoRect: TdxMapControlGeoRect = (NorthLatitude
    : dxMapControlInvalidLatitude; WestLongitude: dxMapControlInvalidLongitude;
    SouthLatitude: dxMapControlInvalidLatitude;
    EastLongitude: dxMapControlInvalidLongitude);

function dxMapControlTileRangeIsEqual(const ARange1,
  ARange2: TdxMapControlTileRange): Boolean; inline;
function dxMapControlTileIndex(X, Y, ALevel: Integer)
  : TdxMapControlTileIndex; inline;
function dxMapControlTileIndexIsEqual(const AIndex1,
  AIndex2: TdxMapControlTileIndex): Boolean; inline;
function dxMapControlTileRange(const AMin, AMax: TdxMapControlTileIndex)
  : TdxMapControlTileRange; inline;
function dxMapControlGeoPoint(ALatitude, ALongitude: Double)
  : TdxMapControlGeoPoint; inline;
function dxMapControlGeoPointIsEqual(const AGeoPoint1,
  AGeoPoint2: TdxMapControlGeoPoint): Boolean; inline;
function dxMapControlGeoRect(ANorthLatitude, AWestLongitude, ASouthLatitude,
  AEastLongitude: Double): TdxMapControlGeoRect; inline;

implementation

function dxMapControlTileRangeIsEqual(const ARange1,
  ARange2: TdxMapControlTileRange): Boolean;
begin
...
function dxMapControlTileIndexIsEqual(const AIndex1,
  AIndex2: TdxMapControlTileIndex): Boolean;
begin
  Result := (AIndex1.X = AIndex2.X) and (AIndex1.Y = AIndex2.Y) and
    (AIndex1.Level = AIndex2.Level);
end;

function dxMapControlTileRange(const AMin, AMax: TdxMapControlTileIndex)
  : TdxMapControlTileRange;
begin
  Result.Max := AMax;
  Result.Min := AMin;
end;
...
function dxMapControlGeoPointIsEqual(const AGeoPoint1,
  AGeoPoint2: TdxMapControlGeoPoint): Boolean;
begin
...
function dxMapControlGeoRect(ANorthLatitude, AWestLongitude, ASouthLatitude,
  AEastLongitude: Double): TdxMapControlGeoRect;
begin
...
function TdxMapControlGeoPoint.IsEqual(const AGeoPoint
  : TdxMapControlGeoPoint): Boolean;
begin
...
begin
  Result := not IsNan(Latitude) and not IsNan(Longitude) and
    InRange(Latitude, -90, 90) and InRange(Longitude, -180, 180);
end;
...
function TdxMapControlGeoRect.IsEqual(const AGeoRect
  : TdxMapControlGeoRect): Boolean;
begin
...
procedure TdxMapControlGeoLocation.SetGeoPoint(const Value
  : TdxMapControlGeoPoint);
begin
...
procedure TdxMapControlGeoRectPersistent.SetGeoRect
