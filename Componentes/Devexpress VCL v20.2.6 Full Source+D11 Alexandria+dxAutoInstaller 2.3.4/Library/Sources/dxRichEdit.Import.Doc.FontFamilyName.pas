...
class function TdxDocFontFamilyName.FromStream(AReader: TBinaryReader)
  : TdxDocFontFamilyName;
begin
...
begin
  ...
  begin
    ...
    ABuffer := AReader.ReadBytes(FTotalLength - BaseLength -
      (FAlternateFontNameStartIndex * 2) - DelimiterSize);
    AAlternateFontName := TdxEncoding.Unicode.GetString(ABuffer, 0,
      Length(ABuffer));
  end;
  FFontName := TdxStringHelper.PrepareFontNameForDoc(AFontName);
  FAlternateFontName := TdxStringHelper.PrepareFontNameForDoc
    (AAlternateFontName);
  AReader.ReadSmallInt;
end;
