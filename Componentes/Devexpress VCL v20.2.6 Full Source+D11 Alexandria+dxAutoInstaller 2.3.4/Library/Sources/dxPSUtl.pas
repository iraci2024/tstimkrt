...
function ExcludeClipRect(DC: HDC; ALeft, ATop, ARight, ABottom: Integer)
  : HRGN; overload;
function IntersectClipRect(DC: HDC; const R: TRect; AlwaysClip: Boolean = False)
  : HRGN; overload;
function IntersectClipRect(DC: HDC; ALeft, ATop, ARight, ABottom: Integer;
  AlwaysClip: Boolean = False): HRGN; overload;
...
function dxAreFontsEqual(const AFont1Name: string; AFont1Color: TColor;
  AFont1Pitch: TFontPitch; AFont1Style: TFontStyles; AFont1Height: Integer;
  AFont2: TFont): Boolean; overload;
...
procedure ActivateComboBoxControl(AForm: TCustomForm;
  AComboBoxControl: TWinControl);
procedure CheckDialogFormHelpContext(AForm: TCustomForm;
  AHelpBtnHolder: TdxLayoutItem);
...
function CreateDoubleArrowBitmap(AUpDownGlyph: TdxUpDownGlyph;
  AWidth: Integer = 16; AHeight: Integer = 16; AFontSize: Integer = 8): TBitmap;
function CreateGlyphBitmap(AGlyphIndex: Integer; AWidth: Integer = 16;
  AHeight: Integer = 16; AFontSize: Integer = 12): TBitmap;
procedure DrawBlendedText(ACanvas: TCanvas; const R: TRect; const AText: string;
  AFont: TFont);
procedure DrawGlyph(DC: HDC; const R: TRect; AGlyph: Byte;
  ACenter: Boolean = False);
...
function DecodePageIndexes(const Source: string;
  out AOutput: TdxPSPageIndexes): Boolean;
...
function HasPropertyEx(AClass: TClass; const AName: string;
  ATypeKinds: TTypeKinds): Boolean; overload;
function HasPropertyEx(AnObject: TObject; const AName: string;
  ATypeKinds: TTypeKinds): Boolean; overload;
...
procedure SetProperty(AnObject: TObject; const AName: string;
  const AValue: Variant);

function dxGetStoringSectionName(AComponent: TComponent): string;
procedure dxDrawComboBoxItem(ACanvas: TCanvas; const R: TRect;
  const AText: string; AnImageList: TCustomImageList; AnImageIndex: Integer;
  AState: TOwnerDrawState);

procedure dxLoadStrings(AIniFile: TCustomIniFile; const ASectionName: string;
  AStrings: TStrings);
procedure dxSaveStrings(AIniFile: TCustomIniFile; const ASectionName: string;
  AStrings: TStrings);
procedure dxLoadListViewColumns(AIniFile: TCustomIniFile; ASectionName: string;
  AListView: TListView);
procedure dxSaveListViewColumns(AIniFile: TCustomIniFile; ASectionName: string;
  AListView: TListView);
...
procedure Control_DoContextPopup(AControl: TControl; const Pt: TPoint;
...
procedure Control_UpdateBoundsRect(AControl: TControl; const R: TRect);
  overload;
procedure Control_UpdateBoundsRect(AControl: TControl;
  ALeft, ARight, AWidth, AHeight: Integer); overload;
...
procedure dxLoadBitmapFromResource(ABitmap: TBitmap;
  const AResName: string); overload;
procedure dxLoadBitmapFromResource(ASmartImage: TdxSmartImage;
  const AResName: string); overload;
procedure dxLoadIconFromResource(AIcon: TIcon; const AResName: string);
procedure dxLoadIconFromResourceEx(AImage: TImage;
  const AResName: string); overload;
procedure dxLoadIconFromResourceEx(AImage: TcxImage;
  const AResName: string); overload;
procedure dxLoadImageListFromResources(AImageList: TcxImageList;
  const AResourceName: string; AInstance: HINST = 0);
...
{$IFNDEF DELPHI15}
  FileCtrl,
{$ENDIF}
  dxPSRes, dxPSImgs, cxShellControls, dxDPIAwareUtils, cxEdit;

const
  CharCount = 26;
  Chars: array [Boolean] of string = (('abcdefghijklmnopqrstuvwxyz'),
...
begin
  ...
  for I := 1 to Length(InvalidFileNameChars) do
    Result := StringReplace(Result, InvalidFileNameChars[I], '_',
...
function IntersectClipRect(DC: HDC; const R: TRect;
  AlwaysClip: Boolean = False): HRGN;
begin
  with R do
...
begin
  Result := ((ABrush1 = nil) and (ABrush2 = nil)) or
    ((ABrush1 <> nil) and (ABrush2 <> nil) and (ABrush1.Color = ABrush2.Color)
    and (ABrush1.Style = ABrush2.Style));
end;
...
begin
  if (AFont1 = nil) or (AFont2 = nil) then
    ...
    Result := (AFont1.Orientation = AFont2.Orientation) and
      (AFont1.Color = AFont2.Color) and (AFont1.Name = AFont2.Name) and
      (AFont1.Pitch = AFont2.Pitch) and (AFont1.Style = AFont2.Style) and
...
begin
  Result := (AFont2 <> nil) and (AFont1Color = AFont2.Color) and
    (AFont1Height = Abs(AFont2.Height)) and (AFont1Pitch = AFont2.Pitch) and
    (AFont1Style = AFont2.Style) and (AFont1Name = AFont2.Name);
end;

function dxAreGraphicsEqual(AGraphic1, AGraphic2: TGraphic): Boolean;
begin
  Result := (AGraphic1 = AGraphic2) or
    ((AGraphic1 <> nil) and TGraphicAccess(AGraphic1).Equals(AGraphic2));
end;

function dxArePensEqual(APen1, APen2: TPen): Boolean;
begin
  Result := ((APen1 = nil) and (APen2 = nil)) or
    ((APen1 <> nil) and (APen2 <> nil) and (APen1.Color = APen2.Color) and
    (APen1.Mode = APen2.Mode) and (APen1.Style = APen2.Style) and
...
begin
  ...
  Result := Format('%d %s %s ', [AFont.Size, cxGetResourceString(@sdxPt),
    AFont.Name]);
  if AFont.Style <> [] then
  begin
    Result := Result + ' [';
...
function EnumFontsProc(var AnEnumLogFont: TEnumLogFont;
  var ATextMetric: TNewTextMetric; AFontType: Integer; AData: LPARAM)
  : Integer; stdcall;
begin
  with AnEnumLogFont.elfLogFont do
  begin
    Result := Integer(not(StrIComp(PChar(PSearchBuffer(AData)^.FontName),
      PChar(@lfFaceName[0])) = 0));
    if Result = 0 then
      PSearchBuffer(AData)^.IsTrueType := AFontType and
        TRUETYPE_FONTTYPE = TRUETYPE_FONTTYPE;
  end;
end;
...
begin
  ...
  if not Result then
  begin
    Result := NonTrueTypeFonts.IndexOf(AFont.Name) = -1;
    // not found in non TT Fonts
    if Result then
    begin
...
function CreateArrowBitmap(AUpDownGlyph: TdxUpDownGlyph; AWidth: Integer = 16;
  AHeight: Integer = 16; AFontSize: Integer = 12): TBitmap;
const
  GlyphIndexes: array [TdxUpDownGlyph] of Integer = (GLYPH_UPARROW,
    GLYPH_DOWNARROW);
begin
  Result := CreateGlyphBitmap(GlyphIndexes[AUpDownGlyph], AWidth, AHeight,
    AFontSize);
end;
...
var
  B1, B2: TBitmap;
begin
  B1 := CreateGlyphBitmap(GlyphIndexes[AUpDownGlyph], AWidth, AHeight,
    AFontSize);
  try
    B1.Transparent := True;
    B2 := CreateGlyphBitmap(GlyphIndexes[AUpDownGlyph], AWidth, AHeight,
      AFontSize);
    try
      B2.Transparent := True;
      Result := TBitmap.Create;
...
var
  ...
  procedure CreateTextOutClipPath(const R: TRect; const AText: string;
    AFont: TFont);
  var
    F: HFONT;
    C: TColor;
...
function GetDesktopWorkArea: TRect;
  ...
  function IntersectionArea(const ABounds: TRect; APoint: TPoint;
    var Area: TRect): Integer;
  var
    R: TRect;
  begin
...
begin
  ...
  begin
    ...
    if IntersectionArea(Result, Result.TopLeft, R) > IntersectionArea(Result,
      Result.BottomRight, R1) then
      Result := R
    else
      Result := R1;
  end
...
var
  Handle: THandle;
  Proc: function(ShortPathName, LongPathName: PChar; cchBuffer: Integer)
    : Integer; stdcall;
...
begin
  ...
  begin
    ...
    if (@Proc <> nil) and (Proc(PChar(Source), Buffer, SizeOf(Buffer)) <> 0)
    then
      Result := Buffer
    else
      Result := Source;
...
begin
  ...
  try
    if GetVolumeInformation(PChar(APath), @Buffer, SizeOf(Buffer), nil, L,
      Flags, nil, 0) then
      Result := Buffer
    else
      Result := '';
...
function BFFCallBack(Wnd: HWND; uMsg: UINT; LPARAM, lpData: LPARAM)
  : Integer; stdcall;
var
  ...
  begin
    ...
    try
      ...
      BrowseInfo.ulFlags := BIF_RETURNONLYFSDIRS or
        EditBoxFlags[IsComCtrlVersion471] or NewDialogStyleFlags
        [IsComCtrlVersion500] or ValidateFlags[IsComCtrlVersion471];
    ...
    begin
      ...
      try
        ...
        begin
          ...
          else if ProcessRange(S, SubRange) then
          begin
            L := Length(AOutput);
...
const
  Max = 13;
  RomanNumbers: array [1 .. Max] of Integer = (1, 4, 5, 9, 10, 40, 50, 90, 100,
    400, 500, 900, 1000);
...
const
  ...
  RomanStrings: array [Boolean, TdxNumberOrder, TdxRomanNumber]
    of string = ((('i', 'ii', 'iii', 'iv', 'v', 'vi', 'vii', 'viii', 'ix'),
...
begin
  ...
  if AText <> '' then
    ...
    begin
      ...
      while (Number > 0) and (Pos(RomanStrings[AnUpperCase, Order, Number],
        AText) <> 1) do
        Dec(Number);
      if Number > 0 then
      begin
...
begin
  if Length(AText) > 0 then
    Result := CharCount * (Length(AText) - 1) +
      Pos(AText[1], Chars[AnUpperCase])
  else
    Result := 0;
end;
  ...
  begin
    ...
    try
      ...
      while (I < PropCount) and
        (CompareText(dxShortStringToString(PropList^[I].Name), AName) <> 0) do
        Inc(I);
      Result := I < PropCount;
    finally
...
begin
  ...
  with R do
  begin
    X := Left + 1 + Ord(IsImageAssigned(AnImageList, AnImageIndex)) *
      (AnImageList.Width + 2);
...
begin
  ...
  AIniFile.EraseSection(ASectionName);
  for I := 0 to AListView.Columns.Count - 1 do
    AIniFile.WriteInteger(ASectionName, Format(ColumnPattern, [I]),
      AListView.Columns[I].Width);
end;
...
procedure dxLoadListViewColumnsFromRegistry(AListView: TListView;
  const APath: string);
begin
end;

procedure dxSaveListViewColumnsToRegistry(AListView: TListView;
  const APath: string);
begin
end;
...
const
  ...
  procedure DrawCheckBox(ACanvas: TCanvas; const R: TRect;
...
begin
  ABitmap := TcxBitmap.CreateSize(AnImageList.Width,
    AnImageList.Height, pf24bit);
  try
    ...
    InflateRect(ACheckRect, -(ABitmap.Width - ASize.cX) div 2,
      -(ABitmap.Height - ASize.cY) div 2);
...
begin
  ...
  if Assigned(AControl) and not TcxControlAccess(AControl).LookAndFeel.NativeStyle
  then
...
procedure Control_UpdateBoundsRect(AControl: TControl;
  ALeft, ARight, AWidth, AHeight: Integer);
begin
  Control_UpdateBoundsRect(AControl, MakeBounds(ALeft, ARight, AWidth,
    AHeight));
end;
...
procedure dxLoadImageListFromResources(AImageList: TcxImageList;
  const AResourceName: string; AInstance: HINST);
var
  ABitmap: TcxBitmap;
begin
  if AInstance = 0 then
