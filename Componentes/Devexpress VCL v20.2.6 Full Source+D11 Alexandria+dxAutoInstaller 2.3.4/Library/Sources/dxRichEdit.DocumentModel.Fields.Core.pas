...
type
  ...
  TdxUpdateFieldOperationType = (Load, Copy, Normal, PasteFromIE,
    CreateModelForExport);
  TdxUpdateFieldOperationTypes = set of TdxUpdateFieldOperationType;

  TdxMailMergeDataMode = (None, ViewMergedData, FinalMerging);

  TdxFieldResultOption = (MergeFormat, CharFormat, MailMergeField,
    KeepOldResult, HyperlinkField, DoNotApplyFieldCodeFormatting,
    SuppressUpdateInnerCodeFields, SuppressMergeUseFirstParagraphStyle);
  ...
  strict private
    ...
    FEnd: TdxRunIndex;
  public
    constructor Create(AStart: TdxRunIndex = -1; AEnd: TdxRunIndex = -1);
      reintroduce;
  ...
  public
    ...
    constructor Create(const ARawValue: TValue;
      AOptions: TdxFieldResultOptions); overload;
    destructor Destroy; override;

    function AddOptions(AOptions: TdxFieldResultOptions)
      : TdxCalculatedFieldValue;
    ...
    property ConvertedToString: Boolean read FConvertedToString
      write FConvertedToString;
  ...
  public
    ...
    function GetAllowedUpdateFieldTypes(const AOptions
      : TdxFieldUpdateOnLoadOptions): TdxUpdateFieldOperationTypes; virtual;
  end;
  ...
  public
    constructor Create(APieceTable: TdxCustomPieceTable; AIndex: Integer = -1);
      reintroduce;
    destructor Destroy; override;
    ...
    procedure CopyFrom(Source: TdxCloneable); override;
    function GetResultLength(APieceTable: TdxCustomPieceTable)
      : Integer; virtual;
    ...
    property DisableUpdate: Boolean read FDisableUpdate
      write InternalSetDisableUpdate;
    ...
    property PreparedCalculatedField: TdxCustomCalculatedField
      read FPreparedCalculatedField write FPreparedCalculatedField;
  ...
  public
    ...
    property Items[Index: Integer]: TdxField read GetItem
...
constructor TdxFieldRunInterval.Create(AStart: TdxRunIndex = -1;
  AEnd: TdxRunIndex = -1);
begin
  inherited Create;
  FStart := AStart;
...
procedure TdxFieldRunInterval.SetInterval(AStart: TdxRunIndex;
  AEnd: TdxRunIndex);
begin
  if AStart < 0 then
...
begin
  ...
  try
    for ARunIndex := Start to &End do
      ABuilder.Append(TdxSimplePieceTable(APieceTable)
        .GetRunPlainText(ARunIndex));
...
constructor TdxCalculatedFieldValue.Create(const ARawValue: TValue;
  AOptions: TdxFieldResultOptions);
begin
  Create(ARawValue);
  AddOptions(AOptions);
end;
...
function TdxCalculatedFieldValue.AddOptions(AOptions: TdxFieldResultOptions)
  : TdxCalculatedFieldValue;
begin
...
begin
  Result := not FRawValue.IsEmpty and FRawValue.IsObject and
    FRawValue.IsType<TdxCustomDocumentModel>;
end;

procedure TdxCalculatedFieldValue.SetValueConvertedToString
...
function TdxCustomCalculatedField.GetAllowedUpdateFieldTypes(const AOptions
...
constructor TdxField.Create(APieceTable: TdxCustomPieceTable;
  AIndex: Integer = -1);
begin
  inherited Create;
...
begin
  Result := (FirstRunIndex < AField.FirstRunIndex) and
    (AField.LastRunIndex < LastRunIndex);
end;
...
function TdxField.CloneToNewPieceTable(APieceTable: TdxCustomPieceTable)
  : TdxField;
begin
  if Self = nil then
    Exit(nil);
...
function TdxFieldCollectionBase.LookupParentFieldIndex(AIndex: Integer)
  : Integer;
begin
  while True do
  begin
...
procedure TdxFieldCollectionBase.CopyTo(var AArray: TArray<TdxField>;
  AArrayIndex: Integer);
var
  I: Integer;
begin
