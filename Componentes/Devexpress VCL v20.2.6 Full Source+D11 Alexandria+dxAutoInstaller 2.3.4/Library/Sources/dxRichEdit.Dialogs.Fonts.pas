...
{$ENDIF}
  ...
  Controls, Forms, Dialogs, dxCore, dxRichEdit.Dialogs.CustomDialog, cxGraphics,
  cxControls, cxLookAndFeels,
  cxClasses, cxLookAndFeelPainters, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, cxContainer, cxEdit,
  cxDropDownEdit, dxCoreClasses, cxTextEdit, cxMaskEdit, cxFontNameComboBox,
  dxLayoutContainer, StdCtrls, dxLayoutControl,
  cxLabel, dxColorEdit, dxLayoutLookAndFeels, cxCheckBox, cxButtons,
  dxCoreGraphics, cxListBox,
...
type
  ...
  public
    ...
    property FontUnderlineStyleEdit: TdxUnderlinePainterHelper
      read GetUnderlineStyleHelper;
  end;
...
begin
  ...
  AItem.Width := cxTextWidth(AColorComboBox.Style.GetVisibleFont, AItem.Caption)
    + 8 * cxTextSpace;
  AColorComboBox.Properties.OnButtonClick := cbColorComboBoxButtonClick;
end;

procedure TdxRichEditFontDialogForm.cbColorComboBoxButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TdxRichEditFontDialogForm.cmbFontSizeKeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
  ...
  if Assigned(ARichEditControl) then
    FUnderlineStyleHelper.UnderlineRepository :=
      ARichEditControl.DocumentModel.UnderlineRepository;
...
function TdxRichEditFontDialogForm.GetFontEffectsEdit
...
begin
  AIsValidValue := TdxRichEditFontSizeHelper.TryGetHalfSizeValue
    (cmbFontSize.EditingValue, AValue);
...
function TdxRichEditFontDialogForm.GetFontUnderlineType
  : TdxNullableUnderlineType;
begin
  Result := FUnderlineStyleHelper.UnderlineType;
end;

function TdxRichEditFontDialogForm.GetOnFontSizeValidating
  : TcxEditValidateEvent;
begin
  Result := cmbFontSize.Properties.OnValidate;
end;

function TdxRichEditFontDialogForm.GetOnFontStyleValidating
  : TcxEditValidateEvent;
begin
  Result := cmbFontStyle.Properties.OnValidate;
end;

function TdxRichEditFontDialogForm.GetOnSomeChildControlEditValueChanged
  : TNotifyEvent;
begin
  Result := FFontStyleEditHelper.OnChange;
end;

function TdxRichEditFontDialogForm.GetUnderlineStyleHelper
  : TdxUnderlinePainterHelper;
begin
  Result := FUnderlineStyleHelper;
end;

procedure TdxRichEditFontDialogForm.SetFontForeColor
  (const Value: TdxNullableColor);
begin
...
procedure TdxRichEditFontDialogForm.SetFontUnderlineColor
  (const Value: TdxNullableColor);
begin
  if Value.IsNull then
...
procedure TdxRichEditFontDialogForm.SetFontUnderlineType
  (const Value: TdxNullableUnderlineType);
begin
  FUnderlineStyleHelper.UnderlineType := Value;
end;

procedure TdxRichEditFontDialogForm.SetOnFontSizeValidating
  (const Value: TcxEditValidateEvent);
begin
  cmbFontSize.Properties.OnValidate := Value;
end;

procedure TdxRichEditFontDialogForm.SetOnFontStyleValidating
  (const Value: TcxEditValidateEvent);
begin
  cmbFontStyle.Properties.OnValidate := Value;
end;

procedure TdxRichEditFontDialogForm.SetOnSomeChildControlEditValueChanged
...
begin
  ...
  lciFontName.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditFontDialogFontName);
  lciFontStyle.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditFontDialogFontStyle);
  lciFontSize.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditFontDialogFontSize);
  lciFontColor.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditFontDialogFontColor);
  lciUnderlineStyle.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditFontDialogUnderlineStyle);
  lciUnderlineColor.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditFontDialogUnderlineColor);
  cbStrikethrough.Caption := cxGetResourceString
    (@sdxRichEditFontDialogStrikeout);
  cbDoubleStrikethrough.Caption :=
    cxGetResourceString(@sdxRichEditFontDialogDoubleStrikeout);
  cbUnderlineWordsOnly.Caption := cxGetResourceString
    (@sdxRichEditFontDialogUnderlineWordsOnly);
  cbSuperscript.Caption := cxGetResourceString
...
function TdxRichEditFontDialogForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxFontFormController.Create
    (AControllerParameters as TdxFontFormControllerParameters);
end;
...
begin
  ...
  if (FRichEditFontHelper.FontUnderlineType = Controller.FontUnderlineType) and
    (Controller.FontUnderlineType <> TdxUnderlineType.None) then
    Controller.UnderlineColor := FRichEditFontHelper.FontUnderlineColor;
  Controller.Hidden := FRichEditFontHelper.Hidden;
  Controller.SetFontUnderline(FRichEditFontHelper.FontUnderlineType,
    FRichEditFontHelper.UnderlineWordsOnly);
  UpdateForm;
end;

procedure TdxRichEditFontDialogForm.FormCloseQuery(Sender: TObject;
...
begin
  ...
  FFontStyleEditHelper := TdxFontStyleEditHelper.Create(cmbFontStyle,
    lbFontStyle);
  FFontEffectsHelper := TdxRichEditFontEffectsHelper.Create(cbAllCaps,
    cbDoubleStrikethrough, cbHidden, cbStrikethrough, cbSubscript,
    cbSuperscript, cbUnderlineWordsOnly);
  ...
  FStyle := TdxCharacterStyle.Create(srePreview.DocumentModel, nil,
    'DXCustomStyle');
  AIndex := srePreview.DocumentModel.CharacterStyles.Add(FStyle);
  srePreview.DocumentModel.ActivePieceTable.Runs[0].CharacterStyleIndex
    := AIndex;
...
begin
  ...
  if Assigned(ARichEditControl) then
  begin
    ARestrictions := ARichEditControl.DocumentModel.DocumentCapabilities.
      CharacterFormattingDetailed;
  ...
  lciUnderlineColor.Enabled := AUnderlineColorAllowed and
    not FUnderlineStyleHelper.UnderlineType.IsNull and
...
begin
  if lbFontName.ItemIndex < 0 then
    liFontNameWarning.Caption := cxGetResourceString
      (@sdxRichEditFontDialogFontNotInstalled)
  else
  begin
    AFontInfo := lbFontName.ItemObject as TdxTrueTypeFontInfo;
    AFontStyle := TdxFontStyleEditHelper.ToSupportedFontStyle
      (Controller.FontBold, Controller.FontItalic);
    if FontStyleEdit.FontStyles.HasValue and
      not AFontInfo.StylesInfo.NativeStyles[AFontStyle] then
      liFontNameWarning.Caption :=
        cxGetResourceString(@sdxRichEditFontDialogFontStyleImitated)
    else
      liFontNameWarning.Caption :=
        cxGetResourceString(@sdxRichEditFontDialogPrintNotes);
  end;
end;
...
begin
  ...
  try
    ...
    ATable := APieceTable.InsertTable(0, 1, 1,
      TdxTableAutoFitBehaviorType.AutoFitToWindow, MinInt, MinInt,
      False, False);
    ARow := ATable.Rows[0];
    AHeight := ARow.Properties.Height;
    AHeight.&Type := TdxHeightUnitType.Minimum;
    AHeight.Value := srePreview.DocumentModel.UnitConverter.PixelsToModelUnits
      (srePreview.Height, Screen.PixelsPerInch);
    ...
    begin
      ...
      if not Controller.FontStrikeoutType.IsNull then
        FStyle.CharacterProperties.FontStrikeoutType :=
          Controller.FontStrikeoutType.Value;
      ...
      if not Controller.DoubleFontSize.IsNull then
        FStyle.CharacterProperties.DoubleFontSize :=
          Controller.DoubleFontSize.Value;
      ...
      if not Controller.UnderlineColor.IsNull then
        FStyle.CharacterProperties.UnderlineColor :=
          Controller.UnderlineColor.Value;
      if not Controller.FontUnderlineType.IsNull then
        FStyle.CharacterProperties.FontUnderlineType :=
          Controller.FontUnderlineType.Value;
      if not Controller.UnderlineWordsOnly.IsNull then
        FStyle.CharacterProperties.UnderlineWordsOnly :=
          Controller.UnderlineWordsOnly.Value;
