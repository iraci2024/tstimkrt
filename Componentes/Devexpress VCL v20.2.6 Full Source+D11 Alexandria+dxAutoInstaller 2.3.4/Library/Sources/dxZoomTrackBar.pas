...
uses
  ...
  dxCoreClasses, cxEdit, cxContainer, cxGeometry, cxTrackBar, cxExtEditConsts,
  cxGraphics;
...
type
  ...
  public
    ...
    property FirstRange: TdxZoomTrackBarRangeOptions read FFirstRange
      write SetFirstRange;
    property Max default 500;
    property Min default 10;
    property SecondRange: TdxZoomTrackBarRangeOptions read FSecondRange
      write SetSecondRange;
  ...
  public
    ...
    property ActiveProperties: TdxCustomZoomTrackBarProperties
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TdxZoomTrackBarProperties
      read GetActiveProperties;
  published
    property Anchors;
    property BiDiMode;
...
class function TdxCustomZoomTrackBarProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TdxCustomZoomTrackBarViewData;
end;

class function TdxCustomZoomTrackBarProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TdxCustomZoomTrackBarViewInfo;
end;

procedure TdxCustomZoomTrackBarProperties.DoAssign
...
procedure TdxCustomZoomTrackBarProperties.SetFirstRange
  (Value: TdxZoomTrackBarRangeOptions);
begin
  FFirstRange.Assign(Value);
end;

procedure TdxCustomZoomTrackBarProperties.SetSecondRange
  (Value: TdxZoomTrackBarRangeOptions);
begin
  FSecondRange.Assign(Value);
end;

procedure TdxCustomZoomTrackBarProperties.RangePropertiesChanged
  (Sender: TObject);
begin
  Changed;
end;
...
procedure TdxCustomZoomTrackBarViewData.CalculateTrackBarViewInfoProperties
  (AViewInfo: TcxCustomEditViewInfo);
begin
  TdxCustomZoomTrackBarViewInfo(AViewInfo).FrequencyLeft :=
    Properties.FirstRange.FFrequency;
  TdxCustomZoomTrackBarViewInfo(AViewInfo).FrequencyRight :=
    Properties.SecondRange.FFrequency;
  inherited CalculateTrackBarViewInfoProperties(AViewInfo);
end;

procedure TdxCustomZoomTrackBarViewData.CalculatePixelsPerPositionStep
...
begin
  ...
  APixelsPerPositionFirstRange :=
    (cxRectWidth(ATrackRect) - AViewInfo.ThumbSize) /
    (2 * GetMinCenterPositionDiff);
  APixelsPerPositionSecondRange :=
    (cxRectWidth(ATrackRect) - AViewInfo.ThumbSize) /
    (2 * GetCenterPositionMaxDiff);
  TdxCustomZoomTrackBarViewInfo(AViewInfo).PixelsPerPositionFirstRange :=
    APixelsPerPositionFirstRange;
  TdxCustomZoomTrackBarViewInfo(AViewInfo).PixelsPerPositionSecondRange :=
    APixelsPerPositionSecondRange;
end;
...
function TdxCustomZoomTrackBarViewData.GetProperties
  : TdxCustomZoomTrackBarProperties;
begin
...
begin
  Result := ThumbSize div 2 + Round(PixelsPerPositionFirstRange *
    (Math.Min(APosition, CenterPosition) - Min)) +
    Round(PixelsPerPositionSecondRange * Math.Max(0,
    APosition - CenterPosition));
end;
...
function TdxCustomZoomTrackBarViewInfo.GetCalculatedPosition(APosition: Integer;
...
begin
  ...
  if APosition < CenterPosition then
    if (APosition > CenterPosition - AFirstStep) and AIsInc then
      ...
      AStep := AFirstStep
  else if APosition > CenterPosition then
  ...
  if ANeedRound and (Result <> CenterPosition) then
    if AIsInc then
      Result := Floor((Result - CenterPosition) / AStep) * AStep +
        CenterPosition
    else
      Result := Ceil((Result - CenterPosition) / AStep) * AStep +
        CenterPosition;
end;

function TdxCustomZoomTrackBarViewInfo.GetPositionAfterJump(X,
  Y: Integer): Integer;

  function IsTopLeftRange: Boolean;
  begin
    Result := GetCurrentMousePos(X, Y) < TrackRect.Left +
      cxRectWidth(TrackRect) div 2;
  end;
...
begin
  ...
  if ReverseDirection then
    if IsTopLeftRange then
      Result := AEditProperties.Max -
        Trunc(GetMouseOffset / PixelsPerPositionSecondRange)
    else
      Result := CenterPosition -
        Trunc(GetMouseOffset / PixelsPerPositionFirstRange)
  else if IsTopLeftRange then
    Result := Trunc(GetMouseOffset / PixelsPerPositionFirstRange) +
      AEditProperties.Min
  else
    Result := Trunc(GetMouseOffset / PixelsPerPositionSecondRange) +
      CenterPosition
end;

{ TcxZoomTrackBarController }

function TdxZoomTrackBarController.GetPositionAfterSliding(X,
  Y: Integer): Integer;

  function NeedJumpToCenter(AMouseDelta, AInitialThumbOffCentering
    : Integer): Boolean;
  begin
    Result := (Abs(AMouseDelta - AInitialThumbOffCentering) <
      OffsetForJumpToCenter) and
      (FCanThumbJumpToCenter or (Abs(AInitialThumbOffCentering) >
      OffsetForJumpToCenter));
  end;
...
begin
  ...
  begin
    AInitialThumbOffCentering := Trunc((CenterPosition - StoredPosition) *
      AViewInfo.PixelsPerPositionFirstRange);
    if AViewInfo.GetMouseDelta(X, Y, GetMouseDownPos) <= AInitialThumbOffCentering
    then
      Result := StoredPosition + Round(AViewInfo.GetMouseDelta(X, Y,
        GetMouseDownPos) / AViewInfo.PixelsPerPositionFirstRange)
    else
    begin
      AMouseDeltaRemainder := AViewInfo.GetMouseDelta(X, Y, GetMouseDownPos) -
        AInitialThumbOffCentering;
      Result := StoredPosition + Round(AInitialThumbOffCentering /
        AViewInfo.PixelsPerPositionFirstRange + AMouseDeltaRemainder /
        AViewInfo.PixelsPerPositionSecondRange);
    end;
  end
  else
  begin
    ...
    then
      Result := StoredPosition + Round(AViewInfo.GetMouseDelta(X, Y,
        GetMouseDownPos) / AViewInfo.PixelsPerPositionSecondRange)
    else
    begin
      AMouseDeltaRemainder := AViewInfo.GetMouseDelta(X, Y, GetMouseDownPos) -
        AInitialThumbOffCentering;
      Result := StoredPosition +
        Round(AMouseDeltaRemainder / AViewInfo.PixelsPerPositionFirstRange +
        AInitialThumbOffCentering / AViewInfo.PixelsPerPositionSecondRange);
    end;
  end;
  if Abs(AViewInfo.GetMouseDelta(X, Y, GetMouseDownPos)) > OffsetForJumpToCenterRenewal
  then
    FCanThumbJumpToCenter := True;
  if NeedJumpToCenter(AViewInfo.GetMouseDelta(X, Y, GetMouseDownPos),
    AInitialThumbOffCentering) then
    Result := CenterPosition;
end;

procedure TdxZoomTrackBarController.MouseDown(Button: TMouseButton;
...
class function TdxCustomZoomTrackBar.GetPropertiesClass
...
function TdxCustomZoomTrackBar.GetActiveProperties
  : TdxCustomZoomTrackBarProperties;
begin
...
procedure TdxCustomZoomTrackBar.SetProperties
  (Value: TdxCustomZoomTrackBarProperties);
begin
