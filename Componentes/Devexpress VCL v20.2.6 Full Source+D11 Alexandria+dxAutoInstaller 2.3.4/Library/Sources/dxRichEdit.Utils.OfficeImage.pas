...
uses
  Types, Classes, SysUtils, Windows, SyncObjs, Graphics, Controls, ActiveX,
  Generics.Defaults, Generics.Collections,
  Contnrs,
  dxCore, cxGraphics, dxCoreClasses, dxCoreGraphics, dxGDIPlusClasses,
  dxGDIPlusApi, cxGeometry, dxSmartImage,
...
type
  ...
  TdxImageSizeMode = (Normal, StretchImage, AutoSize, CenterImage, ZoomImage,
    Squeeze, Tile);

  TdxOfficeImageFormat = (None, Bmp, Emf, Exif, Gif, Icon, Jpeg, MemoryBmp, Png,
    Tiff, Wmf);

  TdxMapMode = (Text = 1, LowMetric, HighMetric, LowEnglish, HighEnglish, Twips,
    Isotropic, Anisotropic);

  TdxOfficePixelFormat = (DontCare = 0, Undefined = 0, Max = 15,
    Indexed = 65536, Gdi = 131072, Format16bppRgb555 = 135173,
    Format16bppRgb565 = 135174, Format24bppRgb = 137224,
    Format32bppRgb = 139273, Format1bppIndexed = 196865,
    Format4bppIndexed = 197634, Format8bppIndexed = 198659, Alpha = 262144,
    Format16bppArgb1555 = 397319, PAlpha = 524288, Format32bppPArgb = 925707,
    Extended = 1048576, Format16bppGrayScale = 1052676,
    Format48bppRgb = 1060876, Format64bppPArgb = 1851406, Canonical = 2097152,
    Format32bppArgb = 2498570, Format64bppArgb = 3424269);
  ...
  TdxNativeImageChangedEvent = procedure(Sender: TObject;
    E: TdxNativeImageChangedEventArgs) of object;
  TdxNativeImageChangedEventHandler =
    TdxMulticastMethod<TdxNativeImageChangedEvent>;
...
{$REGION 'ContentTypeTable'}
    ContentTypeTable: array [TdxOfficeImageFormat] of string = ('',
      'image/bitmap', 'application/x-msmetafile', '', 'image/gif',
      'image/x-icon', 'image/jpeg', 'image/bitmap', 'image/png', 'image/tiff',
      'application/x-msmetafile');
{$ENDREGION}
{$REGION 'ExtensionTable'}
    ExtensionTable: array [TdxOfficeImageFormat] of string = ('', 'bmp', 'emf',
      '', 'gif', 'ico', 'jpg', 'bmp', 'png', 'tif', 'wmf');
{$ENDREGION}
  private
  ...
  strict protected
    ...
    class function GetImageStreamDataFormat(AStream: TStream)
      : TdxImageDataFormat;
  ...
  protected
    ...
    class function GetColorPalette(AImage: GPImage)
      : TArray<TdxAlphaColor>; static;

    function UnitsToTwips(const ASizeInUnits: TSize): TSize; virtual;
    function UnitsToHundredthsOfMillimeter(const ASizeInUnits: TSize)
      : TSize; virtual;
  ...
  public
    ...
    function CalculateImageSizeInModelUnits(const AUnitConverter
      : IdxDocumentModelUnitConverter): TSize; virtual;
    function CanGetImageBytes(AImageFormat: TdxOfficeImageFormat)
      : Boolean; virtual;
    procedure EnsureLoadComplete(ATimeOut: Cardinal = $FFFFFFFF); virtual;
    function GetImageBytesStream(AImageFormat: TdxOfficeImageFormat)
      : TBytesStream; virtual;
    function GetImageBytesStreamSafe(AImageFormat: TdxOfficeImageFormat)
      : TBytesStream;
    function GetImageBytesSafe(AImageFormat: TdxOfficeImageFormat)
      : TArray<Byte>;
    function IsExportSupported(ARawFormat: TdxOfficeImageFormat)
      : Boolean; virtual;
    ...
    class function GetContentType(AImageFormat: TdxOfficeImageFormat): string;
      static; inline;
    class function GetExtension(AImageFormat: TdxOfficeImageFormat): string;
      static; inline;

    property ClientRectInImageUnits: TdxRectF read GetClientRectInImageUnits;
    property DesiredSizeAfterLoad: TSize read GetDesiredSizeAfterLoad
      write SetDesiredSizeAfterLoad;
    property IsLoaded: Boolean read GetIsLoaded;
    property RawFormat: TdxOfficeImageFormat read GetRawFormat;
    property ShouldSetDesiredSizeAfterLoad: Boolean
      read GetShouldSetDesiredSizeAfterLoad
      write SetShouldSetDesiredSizeAfterLoad;
    ...
    property SizeInHundredthsOfMillimeter: TSize
      read GetSizeInHundredthsOfMillimeter;
    property SuppressStore: Boolean read GetSuppressStore
      write SetSuppressStore;
    property SuppressStorePlaceholder: Boolean read FSuppressStorePlaceholder
      write FSuppressStorePlaceholder;
  ...
  TdxEmfToWmfBitsFlags = (EmfToWmfBitsFlagsDefault = $00000000,
  ...
  private const
    MetafileSizeInHundredthsOfMillimeterInvalidValue: TSize = (cx: - 1;
      cy: - 1);
  strict private
  ...
  strict protected
    constructor Create(const AStream: IStream; AReferenceDC: HDC;
      AType: TdxGpEmfType; AFrameRect: TdxGpRect;
      AFrameUnit: TdxGpMetafileFrameUnit; ADescription: PChar);
      reintroduce; overload;
    constructor Create(AStream: TStream; const ASize: TSize);
      reintroduce; overload;
  ...
  public
    ...
    constructor CreateSize(AWidth, AHeight: Integer; AColor: TdxAlphaColor = 0);
      overload; override;
    ...
    function CalculateImageSizeInModelUnits(const AUnitConverter
      : IdxDocumentModelUnitConverter): TSize; override;
    property MetafileSizeInHundredthsOfMillimeter: TSize
      read GetMetafileSizeInHundredthsOfMillimeter
  ...
  strict protected
    function CreateClone(AOwner: TdxImageCache; AImage: TdxOfficeImage)
      : TdxOfficeImageReference; virtual;
  ...
  public
    ...
    property NativeImageChanged: TdxNativeImageChangedEventHandler
      read FOnNativeImageChanged;
  ...
  private
    function EnumEnhMetafileCallback(AHdc: HDC; AHandleTable: PHandleTable;
      AMetafileRecord: PMetaRecord; AObjectCount: Integer; AClientData: THandle)
      : Integer; stdcall;
    function EnumMetafileCallback(AHdc: HDC; AHandleTable: PHandleTable;
      AMetafileRecord: PMetaRecord; AObjectCount: Integer; AClientData: THandle)
      : Integer; stdcall;
  public
    function Calculate(AMetafile: TdxOfficeMetafile;
      const ABytes: TArray<Byte>): TSize;
    function CalculateWmfPhysicalDimension(AHmf: HMETAFILE;
      AMetafile: TdxOfficeMetafile): TSize;
    function CalculateEmfPhysicalDimension(AHEmf: HMETAFILE;
      AMetafile: TdxOfficeMetafile): TSize;
    function ConvertLogicalUnitsToHundredthsOfMillimeter
      (const ASize: TSize): TSize;
  end;
  ...
  public
    class function CreateMetafile(AStream: TMemoryStream; AMapMode: TdxMapMode;
      APictureWidth, APictureHeight: Integer): TdxOfficeImage;
  ...
  strict private
    class function CreateBmpFileStreamForDib(ADibStream: TStream;
      ADibHeight, ABytesInLine: Integer): TBytesStream; static;
  public
    class function CreateDib(AStream: TStream;
      AWidth, AHeight, ABytesInLine: Integer): TdxOfficeImage; static;
  end;
  ...
  public
    class procedure WriteBITMAPFILEHEADER(AWriter: TBinaryWriter;
      AFileSize, ABitsOffset: Integer); static;
    class procedure WriteBITMAPINFOHEADER(AWriter: TBinaryWriter;
      AWidth, AHeight, AColorPlanesCount, ABitsPerPixel, ABytesInLine: Integer;
      const APalette: TArray<TdxAlphaColor>); static;
    class procedure WritePalette(AWriter: TBinaryWriter;
      const APalette: TArray<TdxAlphaColor>); static;
  end;
  ...
  public
    ...
    class function ImageFromFile(const AFilename: string)
      : TdxOfficeImage; static;
    class function GetStream(const AFilename: string): TStream; static;
    class function GetMemoryStream(AStream: TStream; ALength: Integer)
      : TMemoryStream; static;
  ...
  public
    class function CalculateImageRectCore(AClientRect: TRect; AImageSize: TSize;
      ASizeMode: TdxImageSizeMode): TdxRectF; static;
  end;
  ...
  strict private
    ...
    function CreatePlaceHolder(APixelTargetWidth, APixelTargetHeight: Integer)
      : TdxOfficeImage;
  strict protected
    function CreateClone(AOwner: TdxImageCache; AImage: TdxOfficeImage)
      : TdxOfficeImageReference; override;
  ...
  protected
    ...
    function GetTargetPlaceHolder(AOwner: TdxImageCache;
      APixelTargetWidth, APixelTargetHeight: Integer): TdxOfficeImage;
    procedure OnEndLoadingTask;
    procedure Update(AImage: TdxOfficeImage);

    property EndLoadingTaskEvent: TSimpleEvent read FEndLoadingTaskEvent;
  public
    constructor Create(AOwner: TdxImageCache;
      const AServiceProvider: IdxServiceProvider; AUriList: TArray<string>;
      APixelTargetWidth, APixelTargetHeight: Integer;
      AAsyncImageLoading: Boolean); reintroduce; overload;
    constructor Create(AImage: TdxUriOfficeImage;
      APixelTargetWidth, APixelTargetHeight: Integer); reintroduce; overload;
    destructor Destroy; override;
...
type
  ...
  TEmfPlusRecordType = (EmfMin = 1, EmfHeader = 1, EmfPolyBezier = 2,
    EmfPolygon = 3, EmfPolyline = 4, EmfPolyBezierTo = 5, EmfPolyLineTo = 6,
    EmfPolyPolyline = 7, EmfPolyPolygon = 8, EmfSetWindowExtEx = 9,
    EmfSetWindowOrgEx = 10, EmfSetViewportExtEx = 11, EmfSetViewportOrgEx = 12,
    EmfSetBrushOrgEx = 13, EmfEof = 14, EmfSetPixelV = 15,
    EmfSetMapperFlags = 16, EmfSetMapMode = 17, EmfSetBkMode = 18,
    EmfSetPolyFillMode = 19, EmfSetROP2 = 20, EmfSetStretchBltMode = 21,
    EmfSetTextAlign = 22, EmfSetColorAdjustment = 23, EmfSetTextColor = 24,
    EmfSetBkColor = 25, EmfOffsetClipRgn = 26, EmfMoveToEx = 27,
    EmfSetMetaRgn = 28, EmfExcludeClipRect = 29, EmfIntersectClipRect = 30,
    EmfScaleViewportExtEx = 31, EmfScaleWindowExtEx = 32, EmfSaveDC = 33,
    EmfRestoreDC = 34, EmfSetWorldTransform = 35, EmfModifyWorldTransform = 36,
    EmfSelectObject = 37, EmfCreatePen = 38, EmfCreateBrushIndirect = 39,
    EmfDeleteObject = 40, EmfAngleArc = 41, EmfEllipse = 42, EmfRectangle = 43,
    EmfRoundRect = 44, EmfRoundArc = 45, EmfChord = 46, EmfPie = 47,
    EmfSelectPalette = 48, EmfCreatePalette = 49, EmfSetPaletteEntries = 50,
    EmfResizePalette = 51, EmfRealizePalette = 52, EmfExtFloodFill = 53,
    EmfLineTo = 54, EmfArcTo = 55, EmfPolyDraw = 56, EmfSetArcDirection = 57,
    EmfSetMiterLimit = 58, EmfBeginPath = 59, EmfEndPath = 60,
    EmfCloseFigure = 61, EmfFillPath = 62, EmfStrokeAndFillPath = 63,
    EmfStrokePath = 64, EmfFlattenPath = 65, EmfWidenPath = 66,
    EmfSelectClipPath = 67, EmfAbortPath = 68, EmfReserved069 = 69,
    EmfGdiComment = 70, EmfFillRgn = 71, EmfFrameRgn = 72, EmfInvertRgn = 73,
    EmfPaintRgn = 74, EmfExtSelectClipRgn = 75, EmfBitBlt = 76,
    EmfStretchBlt = 77, EmfMaskBlt = 78, EmfPlgBlt = 79,
    EmfSetDIBitsToDevice = 80, EmfStretchDIBits = 81,
    EmfExtCreateFontIndirect = 82, EmfExtTextOutA = 83, EmfExtTextOutW = 84,
    EmfPolyBezier16 = 85, EmfPolygon16 = 86, EmfPolyline16 = 87,
    EmfPolyBezierTo16 = 88, EmfPolylineTo16 = 89, EmfPolyPolyline16 = 90,
    EmfPolyPolygon16 = 91, EmfPolyDraw16 = 92, EmfCreateMonoBrush = 93,
    EmfCreateDibPatternBrushPt = 94, EmfExtCreatePen = 95, EmfPolyTextOutA = 96,
    EmfPolyTextOutW = 97, EmfSetIcmMode = 98, EmfCreateColorSpace = 99,
    EmfSetColorSpace = 100, EmfDeleteColorSpace = 101, EmfGlsRecord = 102,
    EmfGlsBoundedRecord = 103, EmfPixelFormat = 104, EmfDrawEscape = 105,
    EmfExtEscape = 106, EmfStartDoc = 107, EmfSmallTextOut = 108,
    EmfForceUfiMapping = 109, EmfNamedEscpae = 110,
    EmfColorCorrectPalette = 111, EmfSetIcmProfileA = 112,
    EmfSetIcmProfileW = 113, EmfAlphaBlend = 114, EmfSetLayout = 115,
    EmfTransparentBlt = 116, EmfReserved117 = 117, EmfGradientFill = 118,
    EmfSetLinkedUfis = 119, EmfSetTextJustification = 120,
    EmfColorMatchToTargetW = 121, EmfMax = 122, EmfCreateColorSpaceW = 122,
    Invalid = 16384, EmfPlusRecordBase = 16384, Min = 16385, Header = 16385,
    EndOfFile = 16386, Comment = 16387, GetDC = 16388, MultiFormatStart = 16389,
    MultiFormatSection = 16390, MultiFormatEnd = 16391, &Object = 16392,
    Clear = 16393, FillRects = 16394, DrawRects = 16395, FillPolygon = 16396,
    DrawLines = 16397, FillEllipse = 16398, DrawEllipse = 16399,
    FillPie = 16400, DrawPie = 16401, DrawArc = 16402, FillRegion = 16403,
    FillPath = 16404, DrawPath = 16405, FillClosedCurve = 16406,
    DrawClosedCurve = 16407, DrawCurve = 16408, DrawBeziers = 16409,
    DrawImage = 16410, DrawImagePoints = 16411, DrawString = 16412,
    SetRenderingOrigin = 16413, SetAntiAliasMode = 16414,
    SetTextRenderingHint = 16415, SetTextContrast = 16416,
    SetInterpolationMode = 16417, SetPixelOffsetMode = 16418,
    SetCompositingMode = 16419, SetCompositingQuality = 16420, Save = 16421,
    Restore = 16422, BeginContainer = 16423, BeginContainerNoParams = 16424,
    EndContainer = 16425, SetWorldTransform = 16426,
    ResetWorldTransform = 16427, MultiplyWorldTransform = 16428,
    TranslateWorldTransform = 16429, ScaleWorldTransform = 16430,
    RotateWorldTransform = 16431, SetPageTransform = 16432, ResetClip = 16433,
    SetClipRect = 16434, SetClipPath = 16435, SetClipRegion = 16436,
    OffsetClip = 16437, Max = 16438, DrawDriverString = 16438, Total = 16439,
    WmfRecordBase = 65536, WmfSaveDC = 65566, WmfRealizePalette = 65589,
    WmfSetPalEntries = 65591, WmfCreatePalette = 65783, WmfSetBkMode = 65794,
    WmfSetMapMode = 65795, WmfSetROP2 = 65796, WmfSetRelAbs = 65797,
    WmfSetPolyFillMode = 65798, WmfSetStretchBltMode = 65799,
    WmfSetTextCharExtra = 65800, WmfRestoreDC = 65831, WmfInvertRegion = 65834,
    WmfPaintRegion = 65835, WmfSelectClipRegion = 65836,
    WmfSelectObject = 65837, WmfSetTextAlign = 65838, WmfResizePalette = 65849,
    WmfDibCreatePatternBrush = 65858, WmfSetLayout = 65865,
    WmfDeleteObject = 66032, WmfCreatePatternBrush = 66041,
    WmfSetBkColor = 66049, WmfSetTextColor = 66057,
    WmfSetTextJustification = 66058, WmfSetWindowOrg = 66059,
    WmfSetWindowExt = 66060, WmfSetViewportOrg = 66061,
    WmfSetViewportExt = 66062, WmfOffsetWindowOrg = 66063,
    WmfOffsetViewportOrg = 66065, WmfLineTo = 66067, WmfMoveTo = 66068,
    WmfOffsetCilpRgn = 66080, WmfFillRegion = 66088, WmfSetMapperFlags = 66097,
    WmfSelectPalette = 66100, WmfCreatePenIndirect = 66298,
    WmfCreateFontIndirect = 66299, WmfCreateBrushIndirect = 66300,
    WmfPolygon = 66340, WmfPolyline = 66341, WmfScaleWindowExt = 66576,
    WmfScaleViewportExt = 66578, WmfExcludeClipRect = 66581,
    WmfIntersectClipRect = 66582, WmfEllipse = 66584, WmfFloodFill = 66585,
    WmfRectangle = 66587, WmfSetPixel = 66591, WmfFrameRegion = 66601,
    WmfAnimatePalette = 66614, WmfTextOut = 66849, WmfPolyPolygon = 66872,
    WmfExtFloodFill = 66888, WmfRoundRect = 67100, WmfPatBlt = 67101,
    WmfEscape = 67110, WmfCreateRegion = 67327, WmfArc = 67607, WmfPie = 67610,
    WmfChord = 67632, WmfBitBlt = 67874, WmfDibBitBlt = 67904,
    WmfExtTextOut = 68146, WmfStretchBlt = 68387, WmfDibStretchBlt = 68417,
    WmfSetDibToDev = 68915, WmfStretchDib = 69443);

  { TdxStreamHelper }

  TdxStreamHelper = class sealed
  public
    class procedure WriteTo(AInputStream: TStream;
      AOutputStream: TStream); static;
  end;
  ...
  strict private
    ...
    class function PreparePalette(APixelFormat: TdxOfficePixelFormat)
      : TArray<TdxAlphaColor>; static;
  public
    class function GetPalette(ABitsPerPixel: Integer)
      : TArray<TdxAlphaColor>; static;
  end;
  ...
  public
    constructor Create(const AUriService: IdxUriStreamService;
      const AUpdater: IdxUriOfficeImageUpdater; const AUri: TArray<string>;
      AEndTaskEvent: TSimpleEvent);
    destructor Destroy; override;

    function Run(const ACancelStatus: TdxTaskCancelCallback)
...
constructor TdxUriOfficeImageLoadingTask.Create(const AUriService
  : IdxUriStreamService; const AUpdater: IdxUriOfficeImageUpdater;
  const AUri: TArray<string>; AEndTaskEvent: TSimpleEvent);
begin
  inherited Create;
...
function TdxUriOfficeImageLoadingTask.Run(const ACancelStatus
...
procedure TdxUriOfficeImageLoadingTask.OnComplete
  (AStatus: TdxTaskCompletedStatus);
begin
...
class function TdxOfficeImage.CreateImage(const AStream: TStream)
  : TdxOfficeImage;
var
...
function TdxOfficeImage.CalculateImageSizeInModelUnits(const AUnitConverter
  : IdxDocumentModelUnitConverter): TSize;
begin
  Result := EnsureNonZeroSize(AUnitConverter.PixelsToModelUnits
    (SizeInOriginalUnits, HorizontalResolution, VerticalResolution));
end;

function TdxOfficeImage.CanGetImageBytes(AImageFormat
  : TdxOfficeImageFormat): Boolean;
begin
  Result := True;
end;
...
function TdxOfficeImage.UnitsToHundredthsOfMillimeter(const ASizeInUnits
  : TSize): TSize;
begin
  Result := EnsureNonZeroSize(PixelsToHundredthsOfMillimeter(ASizeInUnits,
    HorizontalResolution, VerticalResolution));
end;

function TdxOfficeImage.UnitsToTwips(const ASizeInUnits: TSize): TSize;
begin
  Result := EnsureNonZeroSize(PixelsToTwips(ASizeInUnits, HorizontalResolution,
    VerticalResolution));
end;
...
var
  ...
  ALength: Integer;
begin
  AMetafile := TdxOfficeMetafile.CreateSize(SizeInPixels.Width + 1,
    SizeInPixels.Height + 1);
  try
    ...
    AEmf := AMetafile.GetHenhmetafile;
    try
      ALength := GdipEmfToWmfBits(AEmf, 0, nil, MM_ANISOTROPIC,
        Ord(TdxEmfToWmfBitsFlags.EmfToWmfBitsFlagsDefault));
      if ALength = 0 then
        Exit;
      SetLength(Result, ALength);
      GdipEmfToWmfBits(AEmf, ALength, @Result[0], MM_ANISOTROPIC,
        Ord(TdxEmfToWmfBitsFlags.EmfToWmfBitsFlagsDefault));
    finally
      TdxMetafileHelper.DeleteMetafileHandle(AEmf);
    end;
...
class function TdxOfficeImage.GetBitmapImageBytesStream
  (ANativeImage: TdxOfficeImage; AImageFormat: TdxOfficeImageFormat)
  : TBytesStream;
const
...
class function TdxOfficeImage.GetColorPalette(AImage: GPImage)
  : TArray<TdxAlphaColor>;
var
...
class function TdxOfficeImage.GetImageStreamDataFormat(AStream: TStream)
  : TdxImageDataFormat;
var
...
begin
  ...
  begin
    CreateHandleFromBits(ABitmap.Width, ABitmap.Height, AColors, afIgnored);
  end;
end;
...
begin
  if (Handle = nil) or Empty or
    (GdipGetImageHorizontalResolution(Handle, Result) <> Ok) then
    Result := 96;
end;

function TdxOfficeImage.GetImageBytesStream(AImageFormat: TdxOfficeImageFormat)
  : TBytesStream;
begin
...
function TdxOfficeImage.GetImageBytesStreamSafe(AImageFormat
  : TdxOfficeImageFormat): TBytesStream;
begin
  try
...
function TdxOfficeImage.GetImageBytesSafe(AImageFormat: TdxOfficeImageFormat)
  : TArray<Byte>;
var
...
var
  AResult: TdxGPSizeF;
begin
  if (Handle = nil) or Empty or (GdipGetImageDimension(Handle, AResult.Width,
    AResult.Height) <> Ok) then
...
const
  ResultMap: array [TdxImageDataFormat] of TdxOfficeImageFormat =
    (TdxOfficeImageFormat.None, TdxOfficeImageFormat.Bmp,
    TdxOfficeImageFormat.Jpeg, TdxOfficeImageFormat.Png,
    TdxOfficeImageFormat.Tiff, TdxOfficeImageFormat.Gif,
    TdxOfficeImageFormat.Emf, TdxOfficeImageFormat.Exif,
    TdxOfficeImageFormat.Icon, TdxOfficeImageFormat.MemoryBmp,
...
begin
  Result := cxSize(Trunc(PhysicalDimension.Width),
    Trunc(PhysicalDimension.Height));
end;
...
begin
  if (Handle = nil) or Empty or (GdipGetImageVerticalResolution(Handle, Result)
    <> Ok) then
    Result := 96;
end;
...
class function TdxOfficeImage.GetContentType(AImageFormat
  : TdxOfficeImageFormat): string;
begin
  Result := ContentTypeTable[AImageFormat];
end;

class function TdxOfficeImage.GetExtension(AImageFormat
  : TdxOfficeImageFormat): string;
begin
  Result := ExtensionTable[AImageFormat];
end;

function TdxOfficeImage.IsExportSupported(ARawFormat
  : TdxOfficeImageFormat): Boolean;
begin
  Result := True;
end;
...
begin
  inherited Create;
  FMetafileSizeInHundredthsOfMillimeter :=
    MetafileSizeInHundredthsOfMillimeterInvalidValue;
end;
...
begin
  Create;
  if AStream = nil then
    GdipCheck(GdipRecordMetafileI(AReferenceDC, AType, @AFrameRect, AFrameUnit,
      ADescription, AImageHandle))
  else
    GdipCheck(GdipRecordMetafileStreamI(AStream, AReferenceDC, AType,
      @AFrameRect, AFrameUnit, ADescription, AImageHandle));
  ...
  try
    ...
    try
      ...
      try
        ...
        try
          Create(AStreamAdapter, AReferenceDC, EmfTypeEmfPlusDual, AGPRect,
            MetafileFrameUnitPixel, nil);
        finally
...
constructor TdxOfficeMetafile.CreateSize(AWidth, AHeight: Integer;
  AColor: TdxAlphaColor = 0);
begin
...
function TdxOfficeMetafile.CalculateImageSizeInModelUnits(const AUnitConverter
  : IdxDocumentModelUnitConverter): TSize;
begin
  Result := EnsureNonZeroSize(AUnitConverter.HundredthsOfMillimeterToModelUnits
    (SizeInOriginalUnits));
end;
...
function TdxOfficeMetafile.UnitsToHundredthsOfMillimeter(const ASizeInUnits
  : TSize): TSize;
begin
  Result := ASizeInUnits;
end;
...
begin
  if FMetafileSizeInHundredthsOfMillimeter.IsEqual
    (MetafileSizeInHundredthsOfMillimeterInvalidValue) then
  begin
    HandleNeeded;
    if HandleAllocated then
      FMetafileSizeInHundredthsOfMillimeter :=
        EnsureNonZeroSize(TSize.Round(PhysicalDimension));
  end;
  Result := FMetafileSizeInHundredthsOfMillimeter;
end;

procedure TdxOfficeMetafile.SetMetafileSizeInHundredthsOfMillimeter
  (const Value: TSize);
begin
...
function TdxOfficeImageReference.Clone(AOwner: TdxImageCache)
  : TdxOfficeImageReference;
var
  AImage: TdxOfficeImage;
  ACrc32: Integer;
...
function TdxOfficeImageReference.CreateClone(AOwner: TdxImageCache;
  AImage: TdxOfficeImage): TdxOfficeImageReference;
begin
...
procedure TdxOfficeImageReference.RaiseNativeImageChanged
  (const ADesiredImageSize: TSize);
var
...
function TdxOfficeImageReference.CanGetImageBytes(AImageFormat
  : TdxOfficeImageFormat): Boolean;
begin
...
function TdxOfficeImageReference.GetImageBytesStream(AImageFormat
  : TdxOfficeImageFormat): TBytesStream;
begin
...
function TdxOfficeImageReference.UnitsToHundredthsOfMillimeter
...
class function TdxPaletteHelper.GetPalette(ABitsPerPixel: Integer)
  : TArray<TdxAlphaColor>;
begin
  ...
  if ABitsPerPixel = 8 then
    Exit(FPalette256)
  else if ABitsPerPixel = 4 then
    Exit(FPalette16)
  else if ABitsPerPixel = 1 then
    Exit(FPalette2)
  else
    Exit(FPalette0);
end;
...
class function TdxPaletteHelper.PreparePalette(APixelFormat
  : TdxOfficePixelFormat): TArray<TdxAlphaColor>;
var
  ABmp: GPImage;
begin
...
class function TdxMetafileHelper.TryCreateMetafileCore(AHandle: THandle)
  : TMetafile;
begin
...
function TdxMetafilePhysicalDimensionCalculator.Calculate
  (AMetafile: TdxOfficeMetafile; const ABytes: TArray<Byte>): TSize;
var
  AHmf, AHEmf: THandle;
  AMfp: METAFILEPICT;
begin
...
begin
  ...
  AEmfRecordType := TEmfPlusRecordType(Ord(TEmfPlusRecordType.WmfRecordBase) +
    ARecordType);
  ...
  else if AEmfRecordType = TEmfPlusRecordType.WmfSetMapMode then
  begin
    Inc(APointer);
...
function TdxMetafilePhysicalDimensionCalculator.EnumEnhMetafileCallback
  (AHdc: HDC; AHandleTable: PHandleTable; AMetafileRecord: PMetaRecord;
  AObjectCount: Integer; AClientData: THandle): Integer;
var
...
begin
  ...
  else if ARecordType = TEmfPlusRecordType.WmfSetMapMode then
  begin
    Inc(APointer, 2);
    FMapMode := TdxMapMode(APointer^);
  end;
...
function InternalEnumMetafileCallback(AHdc: HDC; AHandleTable: PHandleTable;
  AMetafileRecord: PMetaRecord; AObjectCount: Integer; AClientData: THandle)
  : Integer; stdcall;
begin
  Result := FEnumMetafileCallbackCalculator.EnumMetafileCallback(AHdc,
    AHandleTable, AMetafileRecord, AObjectCount, AClientData);
end;

function TdxMetafilePhysicalDimensionCalculator.CalculateWmfPhysicalDimension
  (AHmf: HMETAFILE; AMetafile: TdxOfficeMetafile): TSize;
begin
...
function InternalEnumEnhMetafileCallback(AHdc: HDC; AHandleTable: PHandleTable;
  AMetafileRecord: PMetaRecord; AObjectCount: Integer; AClientData: THandle)
  : Integer; stdcall;
begin
  Result := FEnumEnhMetafileCallbackCalculator.EnumEnhMetafileCallback(AHdc,
    AHandleTable, AMetafileRecord, AObjectCount, AClientData);
end;

function TdxMetafilePhysicalDimensionCalculator.CalculateEmfPhysicalDimension
  (AHEmf: HMETAFILE; AMetafile: TdxOfficeMetafile): TSize;
var
  ARect: TRect;
begin
...
function TdxMetafilePhysicalDimensionCalculator.
  ConvertLogicalUnitsToHundredthsOfMillimeter(const ASize: TSize): TSize;
begin
  Result.Width := Integer(Round(2540.0 * Abs(ASize.Width) /
    TdxMetafileHelper.MetafileResolution));
  Result.Height :=
    Integer(Round(2540.0 * Abs(ASize.Height) /
    TdxMetafileHelper.MetafileResolution));
end;
...
class function TdxDibHelper.CreateBmpFileStreamForDib(ADibStream: TStream;
  ADibHeight, ABytesInLine: Integer): TBytesStream;
var
...
begin
  ...
  try
    ...
    AFileSize := TdxBitmapFileHelper.SizeOfBITMAPFILEHEADER +
      Integer(ADibStream.Size);
    AOffset := TdxBitmapFileHelper.SizeOfBITMAPFILEHEADER +
      ABitmapCoreHeader.bcSize;
...
class function TdxDibHelper.CreateDib(AStream: TStream;
  AWidth, AHeight, ABytesInLine: Integer): TdxOfficeImage;
var
...
class function TdxBitmapHelper.CreateBitmap(AStream: TStream;
  AWidth, AHeight, AColorPlanesCount, ABitsPerPixel, ABytesInLine: Integer)
  : TdxOfficeImage;
var
...
begin
  ...
  try
    ...
    try
      ...
      AHeaderSize := TdxBitmapFileHelper.SizeOfBITMAPFILEHEADER +
        TdxBitmapFileHelper.SizeOfBITMAPINFOHEADER;
      APaletteSize := Length(APalette) * 4;
      AFileSize := AHeaderSize + APaletteSize + Integer(AStream.Size);
      AOffset := AHeaderSize + APaletteSize;

      TdxBitmapFileHelper.WriteBITMAPFILEHEADER(AWriter, AFileSize, AOffset);
      TdxBitmapFileHelper.WriteBITMAPINFOHEADER(AWriter, AWidth, AHeight,
        AColorPlanesCount, ABitsPerPixel, ABytesInLine, APalette);
...
class procedure TdxBitmapFileHelper.WriteBITMAPFILEHEADER
  (AWriter: TBinaryWriter; AFileSize, ABitsOffset: Integer);
begin
...
class procedure TdxBitmapFileHelper.WriteBITMAPINFOHEADER
  (AWriter: TBinaryWriter; AWidth, AHeight, AColorPlanesCount, ABitsPerPixel,
...
class procedure TdxBitmapFileHelper.WritePalette(AWriter: TBinaryWriter;
  const APalette: TArray<TdxAlphaColor>);
var
  ACount, I: Integer;
  C: TdxAlphaColor;
  ARgb: TRGBQuad;
...
class procedure TdxStreamHelper.WriteTo(AInputStream: TStream;
...
class function TdxImageLoaderHelper.ImageFromStream(AStream: TStream)
  : TdxOfficeImage;
var
...
class function TdxImageLoaderHelper.ImageFromFile(const AFilename: string)
  : TdxOfficeImage;
var
  AStream: TStream;
begin
...
class function TdxImageLoaderHelper.GetMemoryStream(AStream: TStream;
  ALength: Integer): TMemoryStream;
var
  ACount: Integer;
  ABuffer: TArray<Byte>;
begin
...
begin
  ...
  case ASizeMode of
    TdxImageSizeMode.AutoSize, TdxImageSizeMode.Normal:
    ...
    TdxImageSizeMode.CenterImage:
      begin
        X := (Result.Left + Result.Right) / 2 -
          (AClientRect.Left + AClientRect.Right) / 2;
        Y := (Result.Top + Result.Bottom) / 2 -
          (AClientRect.Top + AClientRect.Bottom) / 2;
        Result.Offset(X, Y);
      end;
    ...
    TdxImageSizeMode.Squeeze:
      begin
        if (Result.Width > AClientRect.Width) or
          (Result.Height > AClientRect.Height) then
          Result := Result.ZoomInto(dxRectF(AClientRect));
      end;
  end;
end;
...
begin
  ...
  FUriList := AUriList;
  Uri := FUriList[0];
  AThreadPoolService := AServiceProvider.GetService(IdxThreadPoolService)
    as IdxThreadPoolService;
  if AThreadPoolService = nil then
    AAsyncImageLoading := False;
  AService := AServiceProvider.GetService(IdxUriStreamService)
    as IdxUriStreamService;
  ...
  begin
    ...
    begin
      ...
      AThreadPoolService.AddTask(TdxUriOfficeImageLoadingTask.Create(AService,
        FUpdater, FUriList, FEndLoadingTaskEvent));
    end
    else
    begin
      AStream := nil;
...
constructor TdxUriOfficeImage.Create(AImage: TdxUriOfficeImage;
  APixelTargetWidth, APixelTargetHeight: Integer);
begin
  if AImage.IsLoaded then
    inherited Create(AImage.Owner, AImage.Image)
  else
    inherited Create(AImage.Owner, GetTargetPlaceHolder(AImage.Owner,
      APixelTargetWidth, APixelTargetHeight));
  FPixelTargetHeight := APixelTargetHeight;
  FPixelTargetWidth := APixelTargetWidth;
...
function TdxUriOfficeImage.CreateClone(AOwner: TdxImageCache;
  AImage: TdxOfficeImage): TdxOfficeImageReference;
var
...
function TdxUriOfficeImage.CreatePlaceHolder(APixelTargetWidth,
  APixelTargetHeight: Integer): TdxOfficeImage;
var
  ASize: TSize;
  ACanvas: TdxGPCanvas;
begin
  ASize.cx := IfThen(APixelTargetWidth > 0, APixelTargetWidth,
    PlaceHolder.Width);
  ASize.cy := IfThen(APixelTargetHeight > 0, APixelTargetHeight,
    PlaceHolder.Height);
  ...
  begin
    ...
    try
      ...
      ACanvas.Rectangle(Result.ClientRect, TdxAlphaColors.Black,
...
begin
  Result := TSize.Create(PixelsToTwips(FPixelTargetWidth,
    TdxDocumentModelDpi.DpiX), PixelsToTwips(FPixelTargetHeight,
    TdxDocumentModelDpi.DpiY));
end;

function TdxUriOfficeImage.GetTargetPlaceHolder(AOwner: TdxImageCache;
  APixelTargetWidth, APixelTargetHeight: Integer): TdxOfficeImage;
var
  AHasImage: Boolean;
  ACrc32: Cardinal;
