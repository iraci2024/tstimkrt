...
type
  TdxTokenKind = (Eof, OpEQ, OpNEQ, OpLOW, OpLOWEQ, OpHI, OpHIEQ, OpPLUS,
    OpMINUS, OpMUL, OpDIV, OpPOW, OpenParenthesis, CloseParenthesis, Simple,
    DocPropertyInfoCommon, DocPropertyCategory, DocProperty,
    DocumentInformation, Eq, DateAndTimeFormattingSwitchBegin,
    GeneralFormattingSwitchBegin, NumericFormattingSwitchBegin,
    CommonStringFormatSwitchBegin, Text, QuotedText, FieldSwitchCharacter,
    Constant, Percent, SeparatorChar, FunctionName, Template, Invalid = $FF);
  ...
  private
    ...
    procedure SetNext(const Value: IdxToken);
  public
    constructor Create(AKind: TdxTokenKind; APosition: Integer = 0;
...
constructor TdxToken.Create(AKind: TdxTokenKind; APosition: Integer = 0;
  const Value: string = ''; ALength: Integer = 1);
begin
  FKind := Ord(AKind);
  FPosition := APosition;
