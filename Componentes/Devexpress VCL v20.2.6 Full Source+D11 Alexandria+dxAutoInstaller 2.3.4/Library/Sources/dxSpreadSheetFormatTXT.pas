...
uses
  ...
  dxSpreadSheetTextFileFormatCore;
...
type
  ...
  TdxSpreadSheetTXTTableViewWriter = class
...
class function TdxSpreadSheetTXTFormat.CanReadFromStream
...
class function TdxSpreadSheetTXTFormat.GetReader
  : TdxSpreadSheetCustomReaderClass;
begin
  Result := nil;
end;

class function TdxSpreadSheetTXTFormat.GetWriter
...
begin
  ...
  View.ForEachCell(View.Dimensions, procedure(ACell: TdxSpreadSheetCell)
    var
      ACellWidth: Integer;
      AMergedCell: TdxSpreadSheetMergedCell;
      I: Integer;
    begin
      AMergedCell := View.MergedCells.FindCell(ACell.RowIndex,
        ACell.ColumnIndex);
      if AMergedCell <> nil then
      begin
        if AMergedCell.ActiveCell = ACell then
        begin
          ACellWidth := Length(ACell.DisplayText) div dxSpreadSheetAreaWidth
            (AMergedCell.Area);
        ...
        FColumnWidths[ACell.ColumnIndex] :=
          Max(FColumnWidths[ACell.ColumnIndex], Length(ACell.DisplayText));
...
procedure TdxSpreadSheetTXTTableViewWriter.WriteCell(ARowIndex,
...
begin
  ...
  if FSettings.Separator = '' then
    StreamWriter.Write('  ' + DupeString(' ', FColumnWidths[AColumnIndex] -
      GetCellDataLength(ACell)));
end;
