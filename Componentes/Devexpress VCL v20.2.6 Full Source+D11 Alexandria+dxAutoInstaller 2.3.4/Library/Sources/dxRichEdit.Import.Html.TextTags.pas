...
type
  ...
  protected
    ...
    class procedure FontSizeKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure FontColorKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure FontFaceKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    procedure ApplyTagProperties; override;
...
begin
  TdxHtmlImporter(Importer).Position.CharacterFormatting.FontUnderlineType :=
...
begin
  TdxHtmlImporter(Importer).Position.CharacterFormatting.DoubleFontSize :=
    TdxHtmlImporter(Importer).HtmlFontSize.GetLargerDoubleFontSize;
end;
...
begin
  TdxHtmlImporter(Importer).Position.CharacterFormatting.DoubleFontSize :=
    TdxHtmlImporter(Importer).HtmlFontSize.GetSmallerDoubleFontSize;
end;
...
class procedure TdxFontTag.FontSizeKeyword(AImporter: TdxCustomHtmlImporter;
  ...
  try
    ...
    begin
      ...
      for I := 0 to ACount - 1 do
        AFontTag.DoubleFontSize := TdxHtmlImporter(AImporter)
          .HtmlFontSize.GetLargerDoubleFontSize;
    end
    else if TdxStringHelper.StartsWithChar(AValue, '-', False) then
    begin
      ACount := StrToInt(TdxStringHelper.Substring(AValue, 1));
      for I := 0 to ACount - 1 do
        AFontTag.DoubleFontSize := TdxHtmlImporter(AImporter)
          .HtmlFontSize.GetSmallerDoubleFontSize;
    end
    else
      AFontTag.DoubleFontSize := TdxHtmlImporter(AImporter)
        .HtmlFontSize.GetDoubleFontSize(StrToInt(AValue));
  except

  end;
end;

class procedure TdxFontTag.FontColorKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
var
  AFontTag: TdxFontTag;
begin
...
class procedure TdxFontTag.FontFaceKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
var
  AFontTag: TdxFontTag;
...
begin
  TdxHtmlImporter(Importer).Position.CharacterFormatting.Script :=
...
begin
  TdxHtmlImporter(Importer).Position.CharacterFormatting.FontStrikeoutType :=
...
begin
  TdxHtmlImporter(Importer).Position.DefaultAlignment.AlignmentValue :=
    TdxParagraphAlignment.Center;
  TdxHtmlImporter(Importer).Position.ParagraphFormatting.Alignment :=
    TdxParagraphAlignment.Center;
end;
...
begin
  ...
  TdxHtmlImporter(Importer).Position.CharacterFormatting.FontName :=
    'Courier New';
end;
...
begin
  TdxHtmlImporter(Importer).Position.AdditionalIndent :=
    TdxHtmlImporter(Importer).Position.AdditionalIndent + FIndent;
end;
...
begin
  Importer.PieceTable.InsertText(TdxHtmlImporter(Importer).Position,
    '"', False);
end;
