...
{$IFDEF DELPHI16}
  UITypes,
{$ENDIF}
  Windows, Types, SysUtils, Classes, Graphics, Generics.Defaults,
  Generics.Collections, dxCoreClasses, dxCoreGraphics,
  dxGDIPlusClasses, dxGDIPlusAPI, cxGraphics, cxGeometry, dxPDFCore, dxPDFTypes,
...
type
  ...
  IdxPDFShadingColorConverter = interface
    ...
    function Convert(const AComponents: TDoubleDynArray)
  ...
  strict private
    ...
    function DoCreateBitmap(const AMatrix: TdxPDFTransformationMatrix;
      const ASize: TdxSizeF; AUseTransparency: Boolean): TcxBitmap32;
    function CreatePainter: Boolean;
    procedure DoDraw(const AInfo: TdxPDFShadingInfo);
    procedure Init(AInterpreter: IdxPDFCommandInterpreter;
      AShading: TdxPDFCustomShading);
  strict protected
    function GetBitmap(AInterpreter: IdxPDFCommandInterpreter;
      APattern: TdxPDFShadingPattern): TcxBitmap32; overload;
    function GetBitmap(AInterpreter: IdxPDFCommandInterpreter;
      AShading: TdxPDFCustomShading): TcxBitmap32; overload;
    procedure DrawShading(AGraphics: TdxGPCanvas;
      AInterpreter: IdxPDFCommandInterpreter; AShading: TdxPDFCustomShading);
  public
    ...
    class procedure Draw(AGraphics: TdxGPCanvas;
      const AInterpreter: IdxPDFCommandInterpreter;
...
type
  ...
  protected
    ...
    property ColorConverter: IdxPDFShadingColorConverter read FColorConverter
      write FColorConverter;
    property CoordinateConverter: IdxPDFShadingCoordinateConverter
      read FCoordinateConverter;
  public
  ...
  strict private
    ...
    procedure DrawOuterSpaceStart(AGraphics: TdxGPCanvas;
      AShading: TdxPDFRadialShading);
    procedure DrawOuterSpaceEnd(AGraphics: TdxGPCanvas;
      AShading: TdxPDFRadialShading);
    procedure DrawPath(AGraphics: TdxGPCanvas; APath: TdxGPPath;
      const P: TdxPointF; ABrush: TdxGPBrush);
  ...
  TdxPDFBitmapShadingCoordsConverter = class(TInterfacedObject,
    IdxPDFShadingCoordinateConverter)
  strict private
    FShading: TdxPDFCustomShading;
    FSize: TSize;
  ...
  TdxPDFUserSpaceShadingCoordsConverter = class(TInterfacedObject,
    IdxPDFShadingCoordinateConverter)
  strict private
    FInterpreter: IdxPDFCommandInterpreter;
  ...
  protected
    function Convert(const APoint: TdxPointF): TdxPointF;
  public
    constructor Create(AInterpreter: IdxPDFCommandInterpreter;
      const AMatrix: TdxPDFTransformationMatrix);
  end;

  { TdxPDFShadingColorConverter }

  TdxPDFShadingColorConverter = class(TInterfacedObject,
    IdxPDFShadingColorConverter)
  strict private
...
constructor TdxPDFBitmapShadingCoordsConverter.Create
  (AShading: TdxPDFCustomShading; const ASize: TSize);
begin
  inherited Create;
  FShading := AShading;
  FSize := ASize;
...
function TdxPDFBitmapShadingCoordsConverter.Convert(const APoint: TdxPointF)
  : TdxPointF;
var
...
begin
  ...
  begin
    ...
    Result.X := (APoint.X - ABoundingBoxLeft) * FSize.cx /
      (FShading.BoundingBox.Right - ABoundingBoxLeft);
    Result.Y := FSize.cy - (APoint.Y - ABoundingBoxBottom) * FSize.cy /
      (FShading.BoundingBox.Top - ABoundingBoxBottom);
  end;
end;

{ TdxPDFUserSpaceShadingCoordsConverter }

constructor TdxPDFUserSpaceShadingCoordsConverter.Create
...
function TdxPDFUserSpaceShadingCoordsConverter.Convert(const APoint: TdxPointF)
  : TdxPointF;
begin
...
constructor TdxPDFShadingColorConverter.Create(AShadingPainter
  : TdxPDFCustomShadingPainter);
begin
  inherited Create;
...
function TdxPDFShadingColorConverter.Convert(const AComponents: TDoubleDynArray)
  : TdxAlphaColor;
begin
  Result := FShadingPainter.ConvertColor(AComponents);
end;

function TdxPDFShadingColorConverter.Convert(const AComponent: Double)
  : TdxAlphaColor;
var
...
class function TdxPDFShadingPainter.CreateBitmap(const AInterpreter
  : IdxPDFCommandInterpreter; APattern: TdxPDFShadingPattern): TcxBitmap32;
var
...
class function TdxPDFShadingPainter.CreateBitmap(const AInterpreter
  : IdxPDFCommandInterpreter; AShading: TdxPDFCustomShading): TcxBitmap32;
var
  APainter: TdxPDFShadingPainter;
begin
  APainter := TdxPDFShadingPainter.Create;
...
class procedure TdxPDFShadingPainter.Draw(AGraphics: TdxGPCanvas;
  const AInterpreter: IdxPDFCommandInterpreter; AShading: TdxPDFCustomShading);
var
  APainter: TdxPDFShadingPainter;
begin
  APainter := TdxPDFShadingPainter.Create;
...
function TdxPDFShadingPainter.GetBitmap(AInterpreter: IdxPDFCommandInterpreter;
  APattern: TdxPDFShadingPattern): TcxBitmap32;
var
  AMatrix: TdxPDFTransformationMatrix;
begin
  Init(AInterpreter, APattern.Shading);
  AMatrix := TdxPDFTransformationMatrix.Multiply(APattern.Matrix,
    AInterpreter.TransformMatrix);
  Result := DoCreateBitmap(AMatrix, dxSizeF(AInterpreter.ActualSize.cx,
    AInterpreter.ActualSize.cy), False);
end;
...
begin
  ...
  ASize := AInterpreter.TransformSize(AInterpreter.TransformMatrix,
    AShading.BoundingBox);
...
procedure TdxPDFShadingPainter.DrawShading(AGraphics: TdxGPCanvas;
  AInterpreter: IdxPDFCommandInterpreter; AShading: TdxPDFCustomShading);
var
...
begin
  ...
  begin
    AInfo.Size := dxSizeF(FInterpreter.ActualSize.cx,
      FInterpreter.ActualSize.cy);
...
function TdxPDFShadingPainter.DoCreateBitmap(const AMatrix
  : TdxPDFTransformationMatrix; const ASize: TdxSizeF;
...
function TdxPDFCustomShadingPainter.ConvertColor(const AComponents
  : TDoubleDynArray): TdxAlphaColor;
begin
  Result := TdxPDFUtils.ConvertToAlphaColor
    (FShading.TransformFunction(AComponents), 255);
end;
...
begin
  ...
  begin
    ...
    else if FShading.BackgroundColor.IsNull then
      AColor := TdxAlphaColors.Transparent
    else
...
begin
  ...
  if AInfo.TransformMatrix.IsNull then
    FCoordinateConverter := TdxPDFBitmapShadingCoordsConverter.Create
      (FShading, ASize)
  else
    FCoordinateConverter := TdxPDFUserSpaceShadingCoordsConverter.Create
      (AInfo.Interpreter, AInfo.TransformMatrix);
  FColorConverter := TdxPDFShadingColorConverter.Create(Self);
end;

{ TdxPDFGradientShadingPainter }

procedure TdxPDFGradientShadingPainter.Calculate(const AInfo
  : TdxPDFShadingInfo);
begin
...
procedure TdxPDFAxialShadingPainter.CalculateTextureSize(const ABoundsSize
  : TdxSizeF);
var
...
begin
  ...
  if (FRotateAngle >= 0.0) and (FRotateAngle < Pi / 2.0) or (FRotateAngle >= Pi)
    and (FRotateAngle < Pi + Pi / 2.0) then
  begin
  ...
  FBitmapSize := cxSize(Ceil(Abs(AAdjacentTop) + Abs(AOppositeBottom)),
    Round(Abs(AAdjacentBottom) + Abs(AOppositeTop)));
end;
...
function TdxPDFAxialShadingPainter.CreateRange(V1, V2, V3, V4: Single)
  : TdxPDFRange;
var
...
begin
  ...
  if V3 > AMaxValue then
    AMaxValue := V3
  else if V3 < AMinValue then
    AMinValue := V3;
  if V4 > AMaxValue then
    AMaxValue := V4
  else if V4 < AMinValue then
    AMinValue := V4;
  Result := TdxPDFRange.Create(AMinValue, AMaxValue);
end;

function TdxPDFAxialShadingPainter.CreatePen(const ATextureData: TBytes)
  : TdxGPPen;
var
  ABitmap: GpBitmap;
  ABrush: TdxGPBrush;
...
begin
  ...
  try
    ...
    try
      GdipCheck(GdipCreateBitmapFromScan0(FBrushBitmapWidth, 1, AStride, 137224,
        @ATextureData[0], ABitmap));
...
begin
  ...
  FLocation := dxPointF(FBounds.Left + FBounds.Width / 2,
    FBounds.Top + FBounds.Height / 2);
end;

procedure TdxPDFAxialShadingPainter.CalculatePenWidth(const ABoundsSize
  : TdxSizeF);
var
  A, B, C: Single;
  ARange: TdxPDFRange;
begin
  ...
  ARange := CreateRange(Abs(C), Abs(A * ABoundsSize.cx + C),
    Abs(B * ABoundsSize.cy + C),
    Abs(A * ABoundsSize.cx + B * ABoundsSize.cy + C));
  FPenWidth := ARange.Max / Sqrt(A * A + B * B) * 2;
end;

procedure TdxPDFAxialShadingPainter.CalculateTextureBrushWidth;
begin
  FBrushBitmapWidth := TdxPDFUtils.ConvertToInt
    (Sqrt(FBounds.Width * FBounds.Width + FBounds.Height * FBounds.Height)) + 1;
end;
...
begin
  ...
  try
    AInfo.Graphics.Line(FBounds.Left, FBounds.Bottom, FBounds.Right,
      FBounds.Top, APen);
  finally
    APen.Free;
  end;
...
begin
  ...
  ACircleStartLeft := CoordinateConverter.Convert
    (dxPointF(ABounds.Left - AShading.Radius.X, ABounds.Top));
  ACircleEndLeft := CoordinateConverter.Convert
    (dxPointF(ABounds.Right - AShading.Radius.Y, ABounds.Bottom));

  FRadius := cxPoint(Trunc(cxPointDistanceF(ACircleStartLeft, FBounds.TopLeft)),
    Trunc(cxPointDistanceF(ACircleEndLeft, FBounds.BottomRight)));
end;

procedure TdxPDFRadialShadingPainter.CalculateRotateAngle;
begin
  FRotateAngle := ArcTan2(FBounds.BottomRight.Y - FBounds.TopLeft.Y,
    FBounds.BottomRight.X - FBounds.TopLeft.X);
end;

procedure TdxPDFRadialShadingPainter.Calculate(const AInfo: TdxPDFShadingInfo);
begin
  inherited Calculate(AInfo);
  FMaxShadingSide := Max(AInfo.Interpreter.ActualSize.cx,
    AInfo.Interpreter.ActualSize.cy);
  CalculateLength;
end;
...
begin
  ...
  if ARadius = 0 then
  begin
    Result.AddArc(-FRadius.X, -FRadius.X, ADiameter, ADiameter, cdxDegree90,
      cdxDegree180);
    Result.AddArc(-ADistance - FRadius.X, -FRadius.X, ADiameter, ADiameter,
      -cdxDegree90, -cdxDegree180);
  end
  else
  begin
    ...
    begin
      ...
      begin
        Result.AddArc(-FRadius.X, -FRadius.X, ADiameter, ADiameter,
          cdxDegree180 - AAngle, AAngle * 2);
        Result.AddLine(-ADistance, 0, -ADistance, 0);
      end;
    end
    else if TdxPDFUtils.IsDoubleValid(AAngle) then
    begin
      ASweepAngle := cdxDegree360 - AAngle * 2;
      Result.AddArc(-FRadius.X, -FRadius.X, ADiameter, ADiameter, AAngle,
        ASweepAngle);
      ADiameter := FMaxShadingSide * 4;
      Result.AddArc(-ADistance - FMaxShadingSide * 2, -FMaxShadingSide * 2,
        ADiameter, ADiameter, -AAngle, -ASweepAngle);
    end;
  end;
  Result.FigureFinish;
end;
...
function TdxPDFRadialShadingPainter.CreateOuterSpaceEndPath: TdxGPPath;
  ...
  begin
    ...
    if ADistance = 0 then
      Exit(FMaxShadingSide * 2);
    if ADistance > 0 then
      Result := ((FMaxShadingSide * 2 - FRadius.Y) * GetCircleDistance /
        ADistance)
    else
...
begin
  ...
  if ARadius = 0 then
  begin
    Result.AddArc(-FRadius.Y, -FRadius.Y, ADiameter, ADiameter, cdxDegree90,
      cdxDegree180);
    Result.AddArc(ADistance - FRadius.Y, -FRadius.Y, ADiameter, ADiameter,
      -cdxDegree90, -cdxDegree180);
  end
  else
  begin
    ...
    if ARadius < 0 then
    begin
      Result.AddArc(-FRadius.Y, -FRadius.Y, ADiameter, ADiameter, AAngle,
        cdxDegree360 - AAngle * 2);
      Result.AddLine(ADistance, 0, ADistance, 0);
    end
    else
    begin
      ...
      Result.AddArc(-FRadius.Y, -FRadius.Y, ADiameter, ADiameter, AStartAngle,
        ASweepAngle);
      ARadius := FMaxShadingSide * 2;
      ADiameter := ARadius * 2;
      Result.AddArc(ADistance - ARadius, -ARadius, ADiameter, ADiameter,
...
var
  D: TdxPointDouble;
begin
  D.X := Max(Abs((FBounds.Right - FRadius.Y) - (FBounds.Left - FRadius.X)),
    Abs((FBounds.Right + FRadius.Y) - (FBounds.Left + FRadius.X)));
  D.Y := Max(Abs((FBounds.Bottom - FRadius.Y) - (FBounds.Top - FRadius.X)),
    Abs((FBounds.Bottom + FRadius.Y) - (FBounds.Top + FRadius.X)));
  FLength := Trunc(Sqrt((D.X * D.X + D.Y * D.Y)) * 1.5);
end;

procedure TdxPDFRadialShadingPainter.DrawPath(AGraphics: TdxGPCanvas;
  APath: TdxGPPath; const P: TdxPointF; ABrush: TdxGPBrush);
var
  APen: TdxGPPen;
begin
  AGraphics.SaveWorldTransform;
...
procedure TdxPDFRadialShadingPainter.DrawSpace(AGraphics: TdxGPCanvas;
  AShading: TdxPDFRadialShading);
var
  I: Integer;
  R: TdxRectF;
...
begin
  ...
  ARadius := FRadius.Y - FRadius.X;
  AComponentOffset := (AShading.Domain.Max - AShading.Domain.Min) /
    (FLength - 1);
  APen := TdxGPPen.Create;
  try
    ...
    begin
      ...
      R.TopLeft := dxPointF(FBounds.Left + AFactor * ASize.X - ARadius,
        FBounds.Top + AFactor * ASize.Y - ARadius);
      R.BottomRight := cxPointOffset(R.TopLeft,
        dxPointF(ARadius * 2, ARadius * 2));
      APen.Brush.Color := ColorConverter.Convert(AShading.Domain.Min + I *
        AComponentOffset);
...
procedure TdxPDFRadialShadingPainter.DrawOuterSpaceStart(AGraphics: TdxGPCanvas;
  AShading: TdxPDFRadialShading);
var
  APath: TdxGPPath;
  ABrush: TdxGPBrush;
...
procedure TdxPDFRadialShadingPainter.DrawOuterSpaceEnd(AGraphics: TdxGPCanvas;
  AShading: TdxPDFRadialShading);
var
  APath: TdxGPPath;
  ABrush: TdxGPBrush;
...
procedure TdxPDFGradientShading.ReadProperties(ADictionary
...
procedure TdxPDFGradientShading.Write(AHelper: TdxPDFWriterHelper;
...
function TdxPDFGradientShading.CreateDomain(AArray: TdxPDFArray;
  AIndex: Integer): TdxPDFRanges;
begin
  SetLength(Result, 1);
...
function TdxPDFGradientShading.IsValidCoordinateArray
  (AArray: TdxPDFArray): Boolean;
begin
...
function TdxPDFGradientShading.WriteCoords(AHelper: TdxPDFWriterHelper)
  : TdxPDFArray;
begin
...
function TdxPDFGradientShading.WriteDomain(AHelper: TdxPDFWriterHelper)
  : TdxPDFArray;
begin
  Result := AHelper.CreateArray;
  Result.Add(Domain);
...
function TdxPDFGradientShading.WriteExtend(AHelper: TdxPDFWriterHelper)
  : TdxPDFArray;
begin
  Result := AHelper.CreateArray;
  Result.Add(FExtendX);
...
begin
  ...
  FRadius := dxPointDouble(Max(TdxPDFUtils.ConvertToDouble(AArray[2]), 0),
    Max(TdxPDFUtils.ConvertToDouble(AArray[5]), 0));
end;
