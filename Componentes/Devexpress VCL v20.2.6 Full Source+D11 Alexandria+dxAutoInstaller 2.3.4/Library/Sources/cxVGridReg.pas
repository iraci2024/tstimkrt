...
uses
  ...
  cxVGridConverter, cxImportDialog, dxCore, dxCoreReg, cxLibraryReg,
  cxExportVGLink,
...
type
  ...
  TcxCustomEditorRowPropertiesVisibleForExpressionEditorPropertyEditor = class
...
begin
  if (GetComponent(0) is TcxCustomEditorRow) then
    Result := TcxCustomEditorRowProperties(TcxCustomRowAccess(GetComponent(0))
      .FProperties).EditProperties
  else
    Result := nil;
end;
...
begin
  if (GetComponent(0) is TcxCollectionItemEditorRowProperties) then
    Result := TcxCollectionItemEditorRowProperties(GetComponent(0))
      .EditProperties
  else
    Result := nil;
end;
...
function TcxVerticalGridComponentEditor.InternalGetVerb
...
begin
  case AIndex of
    0:
      Result := 'Edit...';
    1:
      Result := 'Layout editor...';
    2:
      Result := 'Import...';
    3:
      Result := 'Restore default values';
    4:
      begin
...
begin
  case AIndex of
    0:
      ShowVerticalGridEditor(Designer, VerticalGrid);
    1:
      ShowVerticalGridLayoutEditor(VerticalGrid, Format('Layout editor - %s',
        [VerticalGrid.Name]));
    2:
      if Component is TcxDBVerticalGrid then
    ...
    3:
      VerticalGrid.RestoreDefaults;
    4:
      VerticalGrid.ConditionalFormatting.ShowRulesManagerDialog;
  end;
end;
...
procedure TcxDBVerticalGridComponentEditor.PrepareLinkableSubItem
  (const AItem: TDesignMenuItem);
begin
  AItem.Checked := (Component as TcxDBVerticalGrid)
    .DataController.DataSource = Designer.GetComponent(AItem.Caption);
end;

{ TcxRTTIInspectorComponentEditor }

function TcxRTTIInspectorComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  case AIndex of
    0:
...
begin
  case AIndex of
    0:
      ShowImportDialog(Designer, Inspector, cxRTTIVGGroupConverterName, False);
    1:
      Inspector.RestoreDefaults;
  end;
end;
...
function TcxDBVerticalGridItemDataBindingFieldNameProperty.GetDataSource
  : TDataSource;
begin
  Result := TcxDBVerticalGridItemDataBinding(GetComponent(0))
...
procedure TcxOIPropertyEditor.GetValues(Proc: TGetStrProc);
  ...
  begin
    Result := (AComponent.Name <> '') and IsValidInspectedObject(AComponent,
      GetComponent(0) as TcxCustomRTTIInspector);
  end;
...
procedure TcxOIPropertyEditor.SetValue(const Value: string);
  ...
  begin
    if CompareText(Value, Designer.GetRoot.Name) = 0 then
      ...
      Result := Designer.GetRoot.FindComponent(Value);
    if Result = nil then
...
function TcxCustomEditorRowPropertiesVisibleForExpressionEditorPropertyEditor.
  GetDefaultValue: Boolean;
begin
  Result := TcxCustomEditorRowPropertiesAccess(GetComponent(0))
...
procedure TcxVerticalGridSelectionEditor.RequiresUnits(Proc: TGetStrProc);
  ...
  begin
    ...
    else if AProperties is TcxDBMultiEditorRowProperties then
    ...
    else if AProperties is TcxMultiEditorRowProperties then
...
begin
  ...
  try
    Designer.GetComponentNames
      (GetTypeData(PTypeInfo(TcxCustomVerticalGrid.ClassInfo)), AddComponent);
...
begin
  ...
  RegisterComponents(dxCoreLibraryProductPage,
    [TcxVerticalGrid, TcxVirtualVerticalGrid, TcxDBVerticalGrid,
    TcxRTTIInspector]);
  // rows
  ...
  RegisterPropertyEditor(TypeInfo(string), TcxCustomEditorRowProperties,
  ...
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxCustomRowProperties,
    'ImageIndex', TcxCustomRowPropertiesImageIndexProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent),
    TcxCollectionItemEditorRowProperties, 'EditPropertiesEvents',
    TcxCollectionItemEditPropertiesEventsProperty);
  ...
  RegisterComponentEditor(TcxVirtualVerticalGrid,
    TcxVerticalGridComponentEditor);
  RegisterPropertyEditor(TypeInfo(string), TcxDBVerticalGridItemDataBinding,
    'FieldName', TcxDBVerticalGridItemDataBindingFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxDBVerticalGridItemDataBinding,
    'FieldName', TcxDBVerticalGridItemDataBindingFieldNameProperty);
  // DB
  ...
  RegisterPropertyEditor(TypeInfo(string), TcxDBVerticalGridItemDataBinding,
    'FieldName', TcxDBVerticalGridItemDataBindingFieldNameProperty);
  // RTTI
  RegisterComponentEditor(TcxCustomRTTIInspector,
    TcxRTTIInspectorComponentEditor);
  RegisterPropertyEditor(TypeInfo(TPersistent), TcxCustomRTTIInspector,
    'InspectedObject', TcxOIPropertyEditor);
  RegisterSelectionEditor(TcxCustomVerticalGrid,
    TcxVerticalGridSelectionEditor);
