...
uses
  ...
  Controls, Forms, Dialogs, Generics.Defaults, Generics.Collections, cxGraphics,
  cxRadioGroup,
  dxCore, cxControls, cxLookAndFeels, cxButtonEdit, cxLookAndFeelPainters,
  dxLayoutLookAndFeels,
  cxClasses, dxLayoutContainer, dxLayoutControl, cxContainer, cxEdit,
  dxLayoutcxEditAdapters,
  cxTextEdit, cxMaskEdit, cxDropDownEdit, dxLayoutControlAdapters, cxLabel,
  cxButtons,
...
type
  TdxRichEditHyperlinkDialogForm = class(TdxRichEditCustomDialogForm)
    ...
    procedure edtEditAddressPropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
  strict private
    class var FTargetFrames: TStrings;
  ...
  protected
    ...
    property LinkToSelectedIndex: Integer read GetLinkToSelectedIndex
      write SetLinkToSelectedIndex;
...
function TdxRichEditHyperlinkDialogForm.GetController
...
procedure TdxRichEditHyperlinkDialogForm.BookmarkPropertiesChange
  (Sender: TObject);
begin
  ApplyBookmarkName;
...
begin
  if rbLinkToWebPage.Checked then
    Result := 0
  else if rbLinkToDocument.Checked then
    Result := 1
  else
    Result := -1;
end;

procedure TdxRichEditHyperlinkDialogForm.SetLinkToSelectedIndex
  (const Value: Integer);
begin
...
begin
  ...
  rbLinkToWebPage.Caption := cxGetResourceString
    (@sdxRichEditHyperlinkDialogLinkToWebPage);
  rbLinkToDocument.Caption := cxGetResourceString
  ...
  liAddress.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditHyperlinkDialogAddress);
  liText.CaptionOptions.Text := cxGetResourceString
    (@sdxRichEditHyperlinkDialogText);
  liTooltip.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditHyperlinkDialogTooltip);
  liTarget.CaptionOptions.Text := cxGetResourceString
    (@sdxRichEditHyperlinkDialogTarget);
  lilLinkTo.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditHyperlinkDialogLinkTo);
  lilBookmark.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditHyperlinkDialogBookmark);
end;

function TdxRichEditHyperlinkDialogForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxHyperlinkFormController.Create
    (AControllerParameters as TdxHyperlinkFormControllerParameters);
end;
...
procedure TdxRichEditHyperlinkDialogForm.ChangePanelsVisibility(AIsAddressActive
  : Boolean);
begin
...
procedure TdxRichEditHyperlinkDialogForm.TargetFramePropertiesChange
  (Sender: TObject);
var
  AIndex: Integer;
begin
...
class function TdxRichEditHyperlinkDialogForm.CreateTargetFrameDescriptions
  : TStrings;
begin
  Result := TStringList.Create;
  Result.Add(cxGetResourceString
...
procedure TdxRichEditHyperlinkDialogForm.EditAddressPropertiesChange
  (Sender: TObject);
begin
  ApplyNavigationUri;
  UpdateOKButtonEnabling;
...
procedure TdxRichEditHyperlinkDialogForm.edtEditAddressPropertiesButtonClick
  (Sender: TObject; AButtonIndex: Integer);
var
  AFileDialog: TOpenDialog;
  ADescription: string;
begin
  AFileDialog := TOpenDialog.Create(Self);
  try
    ADescription := cxGetResourceString
      (@sdxRichEditFileFilterDescription_AllFiles);
...
procedure TdxRichEditHyperlinkDialogForm.TooltipPropertiesChange
  (Sender: TObject);
begin
...
var
  ...
  I: Integer;
begin
  cmblBookmark.Properties.Items.Add
    (cxGetResourceString(@sdxRichEditHyperlinkSelectedBookmarkNone));
...
procedure TdxRichEditHyperlinkDialogForm.LinkToSelectedIndexChanged
  (Sender: TObject);
begin
