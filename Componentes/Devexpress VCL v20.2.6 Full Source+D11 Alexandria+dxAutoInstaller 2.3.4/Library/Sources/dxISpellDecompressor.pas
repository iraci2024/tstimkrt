...
type
  ...
  public
    procedure AddWordforms(const AWord: string;
  ...
  public
    constructor Create(AKey: WideChar; ACanCombine: Boolean;
      ALangID: Cardinal); virtual;
    destructor Destroy; override;
    procedure AddRule(ARuleDefinition: string);
    procedure AddWordforms(const AWord: string;
      ADictionary: TdxSpellCheckerStrings); virtual;
  ...
  public
    function FindForKey(AKey: WideChar; AIsPrefix: Boolean)
      : TdxCustomAffixElement;
  ...
  protected
    ...
    procedure GetLanguageInfo(AStream: TStream; out ACodePage: Cardinal;
      out AAlphabet: string); virtual; abstract;
    procedure ExtractWordInfo(const AWordDefinition: string;
      out AWord, AKeys: string);
  ...
  public
    ...
    procedure PopulateWordForms(const AWordDefinition: string;
  ...
  protected
    ...
    procedure UpdateByDecompressor(ADecompressor
      : TdxCustomAffixDecompressor); virtual;
  public
  ...
  protected
    ...
    procedure ExtractKeyInfo(const ADefinition: string; out AKey: WideChar;
      out ACanCombine: Boolean); virtual;
    procedure GetLanguageInfo(AStream: TStream; out ACodePage: Cardinal;
      out AAlphabet: string); override;
  ...
  protected
    ...
    function FindPos(const ASubStr, AStr: string;
  ...
  protected
    ...
    function CreateSuggestionBuilder: TdxSpellCheckerSuggestionBuilder;
      override;
    procedure UpdateByDecompressor(ADecompressor
      : TdxCustomAffixDecompressor); override;
...
function GetFirstWord(var S: AnsiString; ARemove: Boolean = False)
  : AnsiString; overload;
var
  L, R: Integer;
begin
  L := 1;
  while (L <= Length(S)) and (S[L] in [#0 .. ' ']) do
    Inc(L);
  R := L + 1;
  while (R <= Length(S)) and not(S[R] in [#0 .. ' ']) do
    Inc(R);
  Result := Copy(S, L, R - L);
  if ARemove and (Length(Result) > 0) then
  begin
    while (R <= Length(S)) and (S[R] in [#0 .. ' ']) do
      Inc(R);
    Delete(S, 1, R - 1);
  end;
end;

function GetFirstWord(var S: string; ARemove: Boolean = False): string;
  overload;
var
  L, R: Integer;
begin
  L := 1;
  while (L <= Length(S)) and TdxCharacters.IsWhiteSpace(S[L]) do
    Inc(L);
  R := L + 1;
  while (R <= Length(S)) and not TdxCharacters.IsWhiteSpace(S[R]) do
    Inc(R);
  Result := Copy(S, L, R - L);
  if ARemove and (Length(Result) > 0) then
  begin
    while (R <= Length(S)) and TdxCharacters.IsWhiteSpace(S[R]) do
      Inc(R);
    Delete(S, 1, R - 1);
  end;
end;
...
function TdxRuleValidator.IsValid(const AWord: string;
...
procedure TdxAffixRuleList.AddWordforms(const AWord: string;
  ADictionary: TdxSpellCheckerStrings; ALangID: Cardinal);
var
  ALower: string;
  ARule: TdxAffixRule;
...
begin
  ...
  LCMapStringW(ALangID, LCMAP_LOWERCASE, Pointer(AWord), ALen,
    Pointer(ALower), ALen);
  for I := 0 to Count - 1 do
  begin
    ARule := Items[I];
...
procedure TdxCustomAffixElement.AddWordforms(const AWord: string;
...
function TdxAffixList.FindForKey(AKey: WideChar; AIsPrefix: Boolean)
  : TdxCustomAffixElement;
var
  I: Integer;
begin
...
constructor TdxCustomAffixDecompressor.Create(ADictionary
  : TdxAffixCompressionDictionary);
begin
  inherited Create;
...
procedure TdxCustomAffixDecompressor.ExtractWordInfo(const AWordDefinition
  : string; out AWord, AKeys: string);
var
  APos: Integer;
begin
  AWord := '';
...
procedure TdxCustomAffixDecompressor.PopulateWordForms(const AWordDefinition
  : string; ADictionary: TdxSpellCheckerWordList);
var
  I: Integer;
begin
...
procedure TdxAffixCompressionDictionary.LoadGrammar(ADecompressor
  : TdxCustomAffixDecompressor);
begin
...
begin
  case AState of
    aesPrefix:
      Result := TdxISpellPrefix.Create(AKey, ACanCombine, Dictionary.Language);
    aesSuffix:
      Result := TdxISpellSuffix.Create(AKey, ACanCombine, Dictionary.Language);
  else
    Result := nil;
  end;
  if Result <> nil then
...
procedure TdxISpellDecompressor.ExtractKeyInfo(const ADefinition: string;
...
procedure TdxISpellDecompressor.GetLanguageInfo(AStream: TStream;
  out ACodePage: Cardinal; out AAlphabet: string);
begin
...
const
  UnsupportedKeyWords: array [0 .. 6] of string = ('allaffixes',
    'altstringtype', 'altstringchar', 'boundarychars', 'defstringtype',
    'stringchar', 'wordchars');
var
  I: Integer;
begin
  Result := False;
...
begin
  case AState of
    aesPrefix:
      Result := TdxHunspellPrefix.Create(AKey, ACanCombine,
        Dictionary.Language);
    aesSuffix:
      Result := TdxHunspellSuffix.Create(AKey, ACanCombine,
        Dictionary.Language);
  else
    Result := nil;
  end;
...
begin
  ...
  try
    ...
    begin
      if (ACodePage <> CP_ACP) and (AnsiAlphabet <> '') then
        Break;
...
procedure TdxHunspellDecompressor.ProcessAffix(S: string;
...
function TdxOpenOfficeSuggestionBuilder.FindPos(const ASubStr, AStr: string;
  var AStartPos: Integer): Boolean;
var
...
constructor TdxOpenOfficeDictionary.Create(ASpellChecker
...
function TdxOpenOfficeDictionary.CreateSuggestionBuilder
  : TdxSpellCheckerSuggestionBuilder;
begin
  Result := TdxOpenOfficeSuggestionBuilder.Create(Self);
end;

procedure TdxOpenOfficeDictionary.UpdateByDecompressor(ADecompressor
  : TdxCustomAffixDecompressor);
begin
...
GetRegisteredDictionaryTypes.Register(TdxISpellDictionary,
  cxGetResourceString(@sdxSpellCheckerISpellDictionary));
GetRegisteredDictionaryTypes.Register(TdxOpenOfficeDictionary,
  cxGetResourceString(@sdxSpellCheckerOpenOfficeDictionary));
