...
type
  ...
  private
    ...
    FIsDefault: Boolean;
  public
    class function Create(APosition: Integer; AAlignment: TdxTabAlignmentType;
      ALeader: TdxTabLeaderType; ADeleted, AIsDefault: Boolean): TdxTabInfo;
      overload; static;

    class function Create(APosition: Integer): TdxTabInfo; overload; static;
    class function Create(APosition: Integer; AAlignment: TdxTabAlignmentType)
      : TdxTabInfo; overload; static;
    class function Create(APosition: Integer; ALeader: TdxTabLeaderType)
      : TdxTabInfo; overload; static;

    class function CreateDefault(APosition: Integer): TdxTabInfo;
      overload; static;

    class operator Equal(const A, B: TdxTabInfo): Boolean;
    class operator NotEqual(const A, B: TdxTabInfo): Boolean;

    function GetLayoutPosition(AUnitConverter
      : TdxDocumentModelUnitToLayoutUnitConverter): Integer;
  ...
  public
    ...
    class function Merge(AMaster: TdxTabFormattingInfo;
      ASlave: TdxTabFormattingInfo): TdxTabFormattingInfo; static;
    class procedure MergeCore(AMaster, ASlave,
      AResult: TdxTabFormattingInfo); static;
    class function NormalizeTabs(ATabs: TdxTabFormattingInfo;
      AParentTabs: TdxTabFormattingInfo): TdxTabFormattingInfo; static;
    procedure OnChanged; virtual;

    property List: TList<TdxTabInfo> read FTabInfos;
    property Items[Index: Integer]: TdxTabInfo read GetItem
  ...
  TdxTabFormattingInfoWithoutSort = class(TdxTabFormattingInfo,
    IdxBatchUpdatable, IdxBatchUpdateHandler)
  private
  ...
  protected
    function CreateDefaultItem(const AUnitConverter
      : IdxDocumentModelUnitConverter): TdxTabFormattingInfo; override;
  end;
  ...
  public
    ...
    function GetCache(const ADocumentModel: TdxCustomDocumentModel)
      : TdxUniqueItemsCache<TdxTabFormattingInfo>; override;
...
class function TdxTabInfo.Create(APosition: Integer;
  AAlignment: TdxTabAlignmentType): TdxTabInfo;
begin
  Result := Create(APosition, AAlignment, DefaultLeader, False, False);
end;

class function TdxTabInfo.Create(APosition: Integer; ALeader: TdxTabLeaderType)
  : TdxTabInfo;
begin
...
class function TdxTabInfo.Create(APosition: Integer;
  AAlignment: TdxTabAlignmentType; ALeader: TdxTabLeaderType;
...
begin
  Result := (A.Position = B.Position) and (A.Alignment = B.Alignment) and
    (A.Leader = B.Leader) and (A.Deleted = B.Deleted);
end;
...
function TdxTabInfo.GetLayoutPosition(AUnitConverter
  : TdxDocumentModelUnitToLayoutUnitConverter): Integer;
begin
...
function TdxTabFormattingInfo.FindNextTab(APos: Integer)
...
function TdxTabFormattingInfo.FindTabItemIndexByPosition
...
class function TdxTabFormattingInfo.Merge(AMaster, ASlave: TdxTabFormattingInfo)
  : TdxTabFormattingInfo;
var
...
class procedure TdxTabFormattingInfo.MergeCore(AMaster, ASlave,
...
class function TdxTabFormattingInfo.NormalizeTabs(ATabs: TdxTabFormattingInfo;
  AParentTabs: TdxTabFormattingInfo): TdxTabFormattingInfo;
var
...
begin
  ...
  try
    ...
    begin
      ...
      else if ATab.Position < AParentTab.Position then
      begin
        ATabInfos.Add(ATab);
        Inc(ATabIndex);
      end
      ...
      begin
        if not AParentTab.Deleted then
          ATabInfos.Add(TdxTabInfo.Create(AParentTab.Position,
            AParentTab.Alignment, AParentTab.Leader, True, False));
        Inc(AParentTabIndex);
      end;
    end;
...
function TdxTabFormattingInfoWithoutSort.GetBatchUpdateHelper
...
function TdxTabFormattingInfoCache.CreateDefaultItem(const AUnitConverter
  : IdxDocumentModelUnitConverter): TdxTabFormattingInfo;
begin
...
constructor TdxTabProperties.Create(const AOwner
...
function TdxTabProperties.GetCache(const ADocumentModel: TdxCustomDocumentModel)
  : TdxUniqueItemsCache<TdxTabFormattingInfo>;
begin
  Result := TdxSimpleDocumentCache(ADocumentModel.Cache).TabFormattingInfoCache;
end;

procedure TdxTabProperties.SetTabCore(AIndex: Integer;
  const AValue: TdxTabInfo);
var
...
function TdxTabProperties.GetBatchUpdateChangeActions
...
function TdxTabProperties.GetObtainAffectedRangeListener
