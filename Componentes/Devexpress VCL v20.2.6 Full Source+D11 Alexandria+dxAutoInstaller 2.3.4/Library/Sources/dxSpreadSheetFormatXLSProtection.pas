...
uses
  cxVariants, SysUtils, Classes, dxCrypto, dxCryptoAPI, dxCoreClasses,
  cxClasses, dxCore, dxHash,
  dxSpreadSheetCore, dxSpreadSheetFormatXLS, dxSpreadSheetStrs,
...
type
  ...
  public
    ...
    procedure Decrypt(AData: PByte; APosition, ACount: Integer);
      virtual; abstract;
    procedure Encrypt(AData: PByte; APosition, ACount: Integer);
      virtual; abstract;
  ...
  protected
    ...
    function CreatePasswordHash(const APassword: UnicodeString;
      const ASalt: TBytes): TBytes; virtual; abstract;
    procedure DecryptSkip(APosition, ACount: Integer); virtual;
    procedure InitializeKey(const APasswordHash: TBytes; ABlockIndex: Integer);
      virtual; abstract;
  public
    ...
    procedure DecryptBlock(ABlock: PByte; ABlockSize: Integer); overload;
      virtual; abstract;
  ...
  protected
    ...
    function CreatePasswordHash(const APassword: UnicodeString;
      const ASalt: TBytes): TBytes; override;
    procedure Initialize(const APassword: string); override;
    procedure InitializeKey(const APasswordHash: TBytes;
      ABlockIndex: Integer); override;
  public
  ...
  protected
    ...
    function CreateHash: TdxHashAlgorithm; override;
    function CreatePasswordHash(const APassword: string; const ASalt: TBytes)
      : TBytes; override;
    procedure Initialize(const APassword: string); override;
    procedure InitializeKey(const APasswordHash: TBytes;
      ABlockIndex: Integer); override;
  public
    procedure DecryptBlock(ABlock: PByte; ABlockSize: Integer); override;
    procedure Encrypt(AData: PByte; APosition: Integer;
...
begin
  if not AValue then
    raise EdxSpreadSheetReaderError.CreateFmt
...
procedure TdxXLSCustomEncryptor.Decrypt(AData: PByte;
...
procedure TdxXLSStandardEncryptor.DecryptBlock(ABlock: PByte;
  ABlockSize: Integer);
begin
  dxRC4Crypt(FRC4Key, PByteArray(ABlock), PByteArray(ABlock), ABlockSize);
end;

procedure TdxXLSStandardEncryptor.Encrypt(AData: PByte;
  APosition, ACount: Integer);
begin
...
function TdxXLSStandardEncryptor.CreatePasswordHash(const APassword
  : UnicodeString; const ASalt: TBytes): TBytes;
const
...
begin
  ...
  AHash := CreateHash;
  try
    AHash.Add(@APassword[1], Min(Length(APassword), MaxPasswordLength) *
      SizeOf(WideChar));
...
procedure TdxXLSStandardEncryptor.InitializeKey(const APasswordHash: TBytes;
  ABlockIndex: Integer);
var
  AHash: TdxHashAlgorithm;
begin
...
function TdxXLSStrongEncryptor.CreatePasswordHash(const APassword: string;
...
procedure TdxXLSStrongEncryptor.DecryptBlock(ABlock: PByte;
  ABlockSize: Integer);
var
  ABuffer: TBytes;
begin
...
procedure TdxXLSStrongEncryptor.Encrypt(AData: PByte;
  APosition, ACount: Integer);
begin
  Check(False);
end;
...
procedure TdxXLSStrongEncryptor.InitializeKey(const APasswordHash: TBytes;
  ABlockIndex: Integer);
var
  AHash: TdxHashAlgorithm;
begin
  ...
  try
    ...
    AHash.Add(@ABlockIndex, SizeOf(ABlockIndex));
    FCipherKey := TdxCryptoKey.Create(FProvider, FCipherAlgorithm,
      TdxByteArray.Resize(AHash.GetHash, FKeySize div 8, 0));
  finally
    AHash.Free;
  end;
end;
...
begin
  ...
  try
    ...
    FProvider := TdxCryptoProvider.Create(PChar(AHelper.CSPName),
      AHelper.CipherProvider);
...
class function TdxXLSEncryptors.CreateEncryptor(AVersion: Word)
  : TdxXLSAbstractEncryptor;
begin
  case AVersion of
    1:
