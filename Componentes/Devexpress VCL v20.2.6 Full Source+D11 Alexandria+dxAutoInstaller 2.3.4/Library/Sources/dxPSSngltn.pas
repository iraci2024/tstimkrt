...
type
  ...
  private
    class function GetInstance(AccessCode: Integer)
      : TBasedxPSSingleton; virtual;
...
const
  rsdxSingletonAccessOnlyThroughInstance =
...
type
  ...
  public
    ...
    property ItemClasses[Index: Integer]: TBasedxPSSingletonClass
      read GetItemClass;
...
function TdxPSSingletonFactory.IndexOf(ASingletonClass
  : TBasedxPSSingletonClass): Integer;
begin
  for Result := 0 to Count - 1 do
    if ItemClasses[Result] = ASingletonClass then
      Exit;
  Result := -1;
end;
...
begin
  while Count <> 0 do
    Items[Count - 1].ReleaseInstance;
  inherited FinalizeInstance;
end;

procedure TdxPSSingletonFactory.RegisterSingleton
  (ASingleton: TBasedxPSSingleton);
begin
  if FItems = nil then
    FItems := TList.Create;
...
procedure TdxPSSingletonFactory.UnregisterSingleton
  (ASingleton: TBasedxPSSingleton);
begin
...
class function TdxPSSingletonFactory.GetInstance(AccessCode: Integer)
  : TBasedxPSSingleton;
const
...
function TdxPSSingletonFactory.GetItemClass(Index: Integer)
  : TBasedxPSSingletonClass;
begin
...
begin
  if GetInstance(SingletonAccess) = Self then
    GetInstance(SingletonRelease);
end;
...
class function TBasedxPSSingleton.GetInstance(AccessCode: Integer)
  : TBasedxPSSingleton;
var
  Index: Integer;
begin
