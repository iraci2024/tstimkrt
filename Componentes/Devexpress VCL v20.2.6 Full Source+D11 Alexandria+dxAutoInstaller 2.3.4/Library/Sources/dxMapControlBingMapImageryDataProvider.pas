...
uses
  ...
  dxMapControlImageTileProvider, dxBingMapRESTService, dxBingMapRESTServiceStrs,
...
type
  ...
  private
    ...
    function GetQuadKey(AZoomLevel, ATilePositionX, ATilePositionY
      : Integer): string;
  protected
    function TilePrefix: string; override;
  public
    constructor Create(ABingMapProvider
...
function TdxMapControlBingMapImageryDataProvider.GetMapSizeInPixels
  (AZoomLevel: Double): TdxSizeDouble;
var
  AActualTileSize: TdxSizeDouble;
begin
  AActualTileSize := dxSizeDouble(ScaleFactor.ApplyF(dxBingMapDefaultTileSize),
    ScaleFactor.ApplyF(dxBingMapDefaultTileSize));
  if AZoomLevel < 1.0 then
    Result := dxSizeDouble(AZoomLevel * AActualTileSize.Width * 2,
      AZoomLevel * AActualTileSize.Height * 2)
  else
    Result := dxSizeDouble(Power(2.0, AZoomLevel) * AActualTileSize.Width,
      Power(2.0, AZoomLevel) * AActualTileSize.Height);
end;
...
function TdxMapControlBingMapImageryDataProvider.CreateTileSource
...
function TdxMapControlBingMapImageryDataProvider.
  GetDefaultMaxParallelConnectionCount: Integer;
begin
...
procedure TdxMapControlBingMapImageryDataProvider.SetBingKey
  (const Value: string);
begin
...
procedure TdxMapControlBingMapImageryDataProvider.SetConnectionProtocol
...
procedure TdxMapControlBingMapImageryDataProvider.SetKind
  (const Value: TdxBingMapKind);
begin
  if FKind <> Value then
  begin
...
constructor TdxMapControlBingMapTileSource.Create(ABingMapProvider
  : TdxMapControlBingMapImageryDataProvider;
  AInfo: TdxMapControlBingImageryServiceInfo);
begin
  inherited Create(ABingMapProvider, AInfo.ImageWidth, AInfo.ImageHeight,
    AInfo.TileWidth, AInfo.TileHeight);
  FBingMapProvider := ABingMapProvider;
  FServiceInfo := AInfo;
end;

function TdxMapControlBingMapTileSource.GetTileByZoomLevel(AZoomLevel,
  ATilePositionX, ATilePositionY: Integer): string;
begin
  if FBingMapProvider.IsDataValid then
  begin
    Result := FServiceInfo.ImageUrlTemplate;
    Result := StringReplace(Result, '{culture}', dxBingMapGetCulture,
      [rfIgnoreCase]);
    Result := StringReplace(Result, '{subdomain}',
      FServiceInfo.Subdomain[GetSubdomainIndex(FServiceInfo.SubdomainCount)],
      [rfIgnoreCase]);
    Result := StringReplace(Result, '{quadkey}',
      GetQuadKey(AZoomLevel, ATilePositionX, ATilePositionY), [rfIgnoreCase]);
  end
  else
    Result := '';
end;
...
function TdxMapControlBingMapTileSource.GetQuadKey(AZoomLevel, ATilePositionX,
  ATilePositionY: Integer): string;
var
  I: Integer;
  ADigitCount: Integer;
  AMask: Integer;
...
dxMapControlImageryDataProviders.Register
  (TdxMapControlBingMapImageryDataProvider, 'BingMapImageryDataProvider');
