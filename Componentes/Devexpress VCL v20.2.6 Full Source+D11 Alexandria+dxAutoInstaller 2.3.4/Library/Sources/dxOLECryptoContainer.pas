...
uses
  Windows, Types, Classes, Generics.Defaults, Generics.Collections, SysUtils,
  ActiveX, ComObj,
...
type
  ...
  protected
    procedure Decrypt(const ASourceStream, ATargetStream: TStream);
      virtual; abstract;
    procedure Encrypt(const ASourceStream, ATargetStream: TStream);
      virtual; abstract;
    // Loading
    function CheckPassword(const APassword: string): Boolean; virtual; abstract;
    procedure DecryptPackage(const ASourceStream, ATargetStream
      : TStream); virtual;
    procedure LoadInfo(AStream: TStream); virtual; abstract;
    // Saving
    procedure EncryptPackage(const ASourceStream, ATargetStream
      : TStream); virtual;
  ...
  TdxOLECryptoContainerEncryptorCryptoAPI = class abstract
    (TdxOLECryptoContainerEncryptor)
  protected
    ...
    function CalculateHash(const AProvider: IdxCryptoProvider;
      const ABytes: TBytes): TBytes; overload;
    function CalculateHash(const AProvider: IdxCryptoProvider;
      const ABytes: TBytes; AHashAlgorithm: Integer): TBytes; overload;
    //
  ...
  TdxOLECryptoContainerEncryptorAgile = class
    (TdxOLECryptoContainerEncryptorCryptoAPI)
  strict private
  type
  ...
  const
{$REGION 'Agile Blocks'}
    EncryptedDataIntegrityHMACValueBlockKey: TAgileBlock = ($A0, $67, $7F, $02,
      $B2, $2C, $84, $33);
    EncryptedDataIntegritySaltBlockKey: TAgileBlock = ($5F, $B2, $AD, $01, $0C,
      $B9, $E1, $F6);
    EncryptedKeyValueBlockKey: TAgileBlock = ($14, $6E, $0B, $E7, $AB, $AC,
      $D0, $D6);
    EncryptedVerifierHashInputBlockKey: TAgileBlock = ($FE, $A7, $D2, $76, $3B,
      $4B, $9E, $79);
    EncryptedVerifierHashValueBlockKey: TAgileBlock = ($D7, $AA, $0F, $6D, $30,
      $61, $34, $4E);
{$ENDREGION}
...
{$ENDREGION}
  strict private
    ...
    function DecryptSegment(const ABlock, AInitialVector: TBytes;
      ABlockSize: Integer; AKey: IdxCryptoKey): TBytes;
    function GenerateKey(AProvider: IdxCryptoProvider;
      const APasswordHash: TBytes; const ABlockKey: PAgileBlock): TBytes;
    function HashInput(AProvider: IdxCryptoProvider;
      const APasswordHash, AInput: TBytes; ABlock: TAgileBlock)
      : TBytes; overload;
    function HashInput(AProvider: IdxCryptoProvider;
      const APasswordHash: TBytes; const AInput: AnsiString;
      ABlock: TAgileBlock): TBytes; overload;
    procedure LoadKeyData(ANode: TdxXMLNode; var ASalt: TBytes;
      var ABlockSize, AKeySize, ACipherAlgorithm, ACipherProvider,
      AChainingMode, AHashAlgorithm: Integer);
  protected
  ...
  TdxOLECryptoContainerEncryptorStandard = class
    (TdxOLECryptoContainerEncryptorCryptoAPI)
  strict private
  const
  ...
  protected
    ...
    procedure SaveInfo(AStream: TStream); override;
    //
    property EncryptedVerifier: TBytes read FEncryptedVerifier
      write FEncryptedVerifier;
    property EncryptedVerifierHash: TBytes read FEncryptedVerifierHash
      write FEncryptedVerifierHash;
    property EncryptedVerifierHashSize: Integer
      read GetEncryptedVerifierHashSize;
    property Flags: Cardinal read FFlags write FFlags;
    property Salt: TBytes read FSalt write FSalt;
    property VerifierHashSize: Integer read FVerifierHashSize
      write FVerifierHashSize;
    //
  ...
  public
    class function FindByID(ID: Integer;
      out AClass: TdxOLECryptoContainerEncryptorClass): Boolean;
  ...
  protected const
    DataSpaceDefinition =
      #6'DataSpaces/DataSpaceInfo/StrongEncryptionDataSpace';
    DataSpaceMap = #6'DataSpaces/DataSpaceMap';
    DataSpaceVersion = #6'DataSpaces/Version';
    DataTransformInfo =
      #6'DataSpaces/TransformInfo/StrongEncryptionTransform/'#6'Primary';
    EncryptionInfo = 'EncryptionInfo';
    PackageName = 'EncryptedPackage';
  protected
    class procedure ReplaceSizeMark(AStream: TStream;
      const ASizeMarkPosition: Int64);
  ...
  TdxOLECryptoContainerGetPasswordFunc = reference to function
  ...
  public
    ...
    class function Decrypt(ASourceStream: TStream; out ATargetStream: TStream;
      AGetPasswordFunc: TdxOLECryptoContainerGetPasswordFunc;
      AGetPasswordAttemptCount: Integer = MaxWord): Boolean; overload;
    class function Decrypt(ASourceStream: TStream; out ATargetStream: TStream;
      var APassword: string;
      AGetPasswordFunc: TdxOLECryptoContainerGetPasswordFunc;
      AGetPasswordAttemptCount: Integer = MaxWord): Boolean; overload;
    class function Decrypt(ASourceStream: TStream; APassword: string;
      out ATargetStream: TStream): Boolean; overload;

    class procedure Encrypt(ASourceStream, ATargetStream: TStream;
      AEncryptor: TdxOLECryptoContainerEncryptor); overload;
    class procedure Encrypt(ASourceStream, ATargetStream: TStream;
      const APassword: string;
      AEncryptorClass: TdxOLECryptoContainerEncryptorClass); overload;
  end;
...
class function TdxOLECryptoContainerEncryptors.FindByID(ID: Integer;
  out AClass: TdxOLECryptoContainerEncryptorClass): Boolean;
var
  I: Integer;
begin
...
class function TdxOLECryptoContainerEncryptors.GetByID(ID: Integer)
  : TdxOLECryptoContainerEncryptorClass;
begin
  if not FindByID(ID, Result) then
    raise EdxOLECryptoContainerError.CreateFmt
      (cxGetResourceString(@sdxOleCryptoContainerUnsupportedEncryptor), [ID]);
end;

class procedure TdxOLECryptoContainerEncryptors.Register
  (AClass: TdxOLECryptoContainerEncryptorClass);
begin
  if FEncryptors = nil then
...
class procedure TdxOLECryptoContainerEncryptors.Unregister
  (AClass: TdxOLECryptoContainerEncryptorClass);
begin
  if (FEncryptors <> nil) and (FEncryptors.Remove(AClass) >= 0) and
    (FEncryptors.Count = 0) then
...
procedure TdxOLECryptoContainerEncryptor.DecryptPackage(const ASourceStream,
...
procedure TdxOLECryptoContainerEncryptor.EncryptPackage(const ASourceStream,
  ATargetStream: TStream);
begin
...
function TdxOLECryptoContainerEncryptorCryptoAPI.CalculateHash(const AProvider
  : IdxCryptoProvider; const ABytes: TBytes): TBytes;
begin
  Result := CalculateHash(AProvider, ABytes, HashAlgorithm);
end;

function TdxOLECryptoContainerEncryptorCryptoAPI.CalculateHash(const AProvider
  : IdxCryptoProvider; const ABytes: TBytes; AHashAlgorithm: Integer): TBytes;
var
...
procedure TdxOLECryptoContainerEncryptorAgile.Decrypt(const ASourceStream,
  ATargetStream: TStream);
const
  BlockSize = 4096;
var
  ...
  repeat
    ...
    begin
      ADecryptedBlock := DecryptSegment(ABlock,
        CalculateHash(FCryptoKey.GetProvider,
        TdxByteArray.Concatenate(FKeyDataSalt, ABlockIndex),
        FKeyDataHashAlgorithm), FKeyDataBlockSize, FCryptoKey);
...
procedure TdxOLECryptoContainerEncryptorAgile.Encrypt(const ASourceStream,
  ATargetStream: TStream);
begin
  Check(False);
end;

function TdxOLECryptoContainerEncryptorAgile.CheckPassword(const APassword
  : string): Boolean;
var
  AHash: TdxCryptoHashAlgorithm;
...
begin
  ...
  try
    TdxPasswordHashCodeCalculator.CalculatePasswordHash(APassword, FSalt,
      FSpinCount, AHash, True);
  ...
  AVerifierHashCalculated := CalculateHash(AProvider,
    HashInput(AProvider, APasswordHash, FEncryptedVerifierHashInput,
    EncryptedVerifierHashInputBlockKey), FHashAlgorithm);
  AVerifierHashOriginal := TdxByteArray.Resize
    (HashInput(AProvider, APasswordHash, FEncryptedVerifierHashValue,
    EncryptedVerifierHashValueBlockKey), FHashSize);

  Result := TdxByteArray.Compare(AVerifierHashCalculated,
    AVerifierHashOriginal);
  if Result then
  begin
    FCryptoKey := TdxCryptoKey.Create(TdxCryptoProvider.Create(nil,
      FKeyDataCipherProvider), FKeyDataCipherAlgorithm,
      HashInput(AProvider, APasswordHash, FEncryptedKey,
...
begin
  ...
  try
    ...
    LoadKeyData(ANode, FKeyDataSalt, FKeyDataBlockSize, ATempValue,
      FKeyDataCipherAlgorithm, FKeyDataCipherProvider,
      FKeyDataCipherChainingMode, FKeyDataHashAlgorithm);

    Check(ADoc.FindChild([XMLEncryption, XMLKeyEncryptors, XMLKeyEncryptor,
      XMLEncryptedKey], ANode));
    LoadKeyData(ANode, FSalt, FBlockSize, FKeySize, FCipherAlgorithm,
      FCipherProvider, FCipherChainingMode, FHashAlgorithm);
    FEncryptedVerifierHashInput := ANode.Attributes.GetValue
      (XMLEncryptedVerifierHashInput);
    FEncryptedVerifierHashValue := ANode.Attributes.GetValue
      (XMLEncryptedVerifierHashValue);
...
procedure TdxOLECryptoContainerEncryptorAgile.Initialize
  (const APassword: string);
begin
  Check(False);
end;
...
begin
  if not AValue then
    raise EdxOLECryptoContainerError.Create
      (cxGetResourceString(@sdxOleCryptoContainerInternalError));
end;

function TdxOLECryptoContainerEncryptorAgile.DecryptSegment(const ABlock,
  AInitialVector: TBytes; ABlockSize: Integer; AKey: IdxCryptoKey): TBytes;
...
begin
  ...
  Result := TdxCipher.Decrypt(AKey, TdxByteArray.Resize(ABlock,
    AdjustSize(Length(ABlock), ABlockSize)));
end;

function TdxOLECryptoContainerEncryptorAgile.GenerateKey
  (AProvider: IdxCryptoProvider; const APasswordHash: TBytes;
  const ABlockKey: PAgileBlock): TBytes;
var
  AHash: TdxCryptoHashAlgorithm;
begin
  AHash := TdxCryptoHashAlgorithm.Create(FHashAlgorithm, AProvider);
...
function TdxOLECryptoContainerEncryptorAgile.HashInput
  (AProvider: IdxCryptoProvider; const APasswordHash, AInput: TBytes;
  ABlock: TAgileBlock): TBytes;
var
  ASecretKey: IdxCryptoKey;
begin
  ASecretKey := TdxCryptoKey.Create(AProvider, FCipherAlgorithm,
    GenerateKey(AProvider, APasswordHash, @ABlock));
...
function TdxOLECryptoContainerEncryptorAgile.HashInput
  (AProvider: IdxCryptoProvider; const APasswordHash: TBytes;
  const AInput: AnsiString; ABlock: TAgileBlock): TBytes;
begin
  Result := HashInput(AProvider, APasswordHash,
    TdxBase64.FromBase64String(dxAnsiStringToString(AInput)), ABlock);
end;

procedure TdxOLECryptoContainerEncryptorAgile.LoadKeyData(ANode: TdxXMLNode;
  var ASalt: TBytes; var ABlockSize, AKeySize, ACipherAlgorithm,
  ACipherProvider, AChainingMode, AHashAlgorithm: Integer);

  function StringToChainingMode(const S: AnsiString): Integer;
  const
    ModeIDs: array [0 .. 1] of Integer = (CRYPT_MODE_CBC, CRYPT_MODE_CFB);
    Names: array [0 .. 1] of AnsiString = ('ChainingModeCBC',
      'ChainingModeCFB');
  var
    I: Integer;
  begin
...
begin
  ...
  ASalt := TdxBase64.FromBase64String
    (ANode.Attributes.GetValueAsString(XMLSaltValue));
  AChainingMode := StringToChainingMode
    (ANode.Attributes.GetValue(XMLCipherChaining, ''));
  ABlockSize := ANode.Attributes.GetValueAsInteger(XMLBlockSize);
  AKeySize := ANode.Attributes.GetValueAsInteger(XMLKeyBits);

  Check(TdxCryptoAlgorithms.GetInfo(ANode.Attributes.GetValueAsString
    (XMLCipherAlgorithm, 'AES'), AKeySize, ACipherAlgorithm, ACipherProvider));

  Check(TdxCryptoAlgorithms.GetInfo(ANode.Attributes.GetValueAsString
    (XMLHashAlgorithm, 'SHA1'), ANode.Attributes.GetValueAsInteger(XMLHashSize)
    * 8, AHashAlgorithm, AHashProvider));
end;
...
procedure TdxOLECryptoContainerEncryptorStandard.CreateCipherKey
  (const APassword: string);

  function GetActualCSPName: string;
  begin
    if dxSameText(CSPName, CryptoProviderEnhancedRSA_AES_XP) and IsWinVistaOrLater
    then
...
var
  ...
  AKey: TBytes;
begin
  AHash := TdxCryptoHashAlgorithm.Create(HashAlgorithm,
    TdxCryptoProvider.Create(PChar(GetActualCSPName), CipherProvider));
  try
    TdxPasswordHashCodeCalculator.CalculatePasswordHash(APassword, FSalt, 50000,
      AHash, True);
    AKey := AHash.GetHash;
    AHash.Reset;
    AHash.Add(TdxByteArray.Concatenate(AKey, 0));

    FCipherKey := TdxCryptoKey.Create(AHash.Provider, CipherAlgorithm,
      AHash.Handle);
...
procedure TdxOLECryptoContainerEncryptorStandard.Decrypt(const ASourceStream,
  ATargetStream: TStream);
begin
  TdxCipher.Decrypt(CipherKey, ASourceStream, ATargetStream);
end;

procedure TdxOLECryptoContainerEncryptorStandard.Encrypt(const ASourceStream,
  ATargetStream: TStream);
begin
  TdxCipher.Encrypt(CipherKey, ASourceStream, ATargetStream);
end;

function TdxOLECryptoContainerEncryptorStandard.CheckPassword(const APassword
  : string): Boolean;
var
  AVerifierHashCalculated: TBytes;
  AVerifierHashOriginal: TBytes;
begin
  ...
  Result := (Length(AVerifierHashCalculated) <= Length(AVerifierHashOriginal))
    and CompareMem(@AVerifierHashOriginal[0], @AVerifierHashCalculated[0],
    Length(AVerifierHashCalculated));
end;
...
procedure TdxOLECryptoContainerEncryptorStandard.Initialize
  (const APassword: string);
var
  AVerifier: TBytes;
...
begin
  ...
  FEncryptedVerifierHash := TdxCipher.Encrypt(CipherKey,
    TdxByteArray.Resize(AVerifierHash, 32));
...
begin
  ...
  WriteCardinalProc(AStream, 8 * SizeOf(Integer) + SizeOf(WideChar) *
    (Length(CSPName) + 1));
...
function TdxOLECryptoContainerEncryptorStandard.
  GetEncryptedVerifierHashSize: Integer;
begin
...
class procedure TdxOLECryptoContainerDataSpaces.WriteDataSpaceDefinition
  (AStream: TStream);
begin
...
class procedure TdxOLECryptoContainerDataSpaces.WriteDataSpaceMap
  (AStream: TStream);
var
  AMark: Int64;
begin
...
class procedure TdxOLECryptoContainerDataSpaces.WriteDataTransformInfo
  (AStream: TStream);
var
  AMark: Int64;
begin
...
class procedure TdxOLECryptoContainerDataSpaces.ReplaceSizeMark
  (AStream: TStream; const ASizeMarkPosition: Int64);
var
  ASize: Integer;
begin
...
class function TdxOLECryptoContainerDataSpaces.WriteSizeMark
...
class procedure TdxOLECryptoContainerDataSpaces.WriteUnicodeLPP4
  (AStream: TStream; const S: UnicodeString);
var
  ABytes: TBytes;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      try
        Result := (ADocument.StreamByName
          (TdxOLECryptoContainerDataSpaces.EncryptionInfo) <> nil) and
          (ADocument.StreamByName(TdxOLECryptoContainerDataSpaces.
          PackageName) <> nil);
      finally
        ADocument.Free;
      end;
...
class function TdxOLECryptoContainer.Decrypt(ASourceStream: TStream;
  out ATargetStream: TStream;
  AGetPasswordFunc: TdxOLECryptoContainerGetPasswordFunc;
  AGetPasswordAttemptCount: Integer = MaxWord): Boolean;
var
  APassword: string;
begin
  Result := Decrypt(ASourceStream, ATargetStream, APassword, AGetPasswordFunc,
    AGetPasswordAttemptCount);
end;

class function TdxOLECryptoContainer.Decrypt(ASourceStream: TStream;
  out ATargetStream: TStream; var APassword: string;
  AGetPasswordFunc: TdxOLECryptoContainerGetPasswordFunc;
  AGetPasswordAttemptCount: Integer = MaxWord): Boolean;
var
  ADocument: TdxOLEDocument;
...
begin
  ...
  begin
    ...
    try
      AStreamInfo := ADocument.StreamByName
        (TdxOLECryptoContainerDataSpaces.EncryptionInfo);
      AStreamPackage := ADocument.StreamByName
        (TdxOLECryptoContainerDataSpaces.PackageName);
      if (AStreamInfo <> nil) and (AStreamPackage <> nil) then
      begin
        AEncryptor := TdxOLECryptoContainerEncryptors.GetByID
          (ReadCardinalFunc(AStreamInfo)).Create;
        try
          ...
          try
            ...
            begin
              ...
              if not(Assigned(AGetPasswordFunc) and AGetPasswordFunc(APassword))
              then
                Abort;
...
class function TdxOLECryptoContainer.Decrypt(ASourceStream: TStream;
  APassword: string; out ATargetStream: TStream): Boolean;
begin
  Result := Decrypt(ASourceStream, ATargetStream, APassword, nil, 0);
end;

class procedure TdxOLECryptoContainer.Encrypt(ASourceStream,
  ATargetStream: TStream; const APassword: string;
  AEncryptorClass: TdxOLECryptoContainerEncryptorClass);
var
  AEncryptor: TdxOLECryptoContainerEncryptor;
begin
...
class procedure TdxOLECryptoContainer.Encrypt(ASourceStream,
  ATargetStream: TStream; AEncryptor: TdxOLECryptoContainerEncryptor);
  ...
  begin
    ...
    try
      ...
      for I := 0 to AParts.Count - 2 do
        AParentDir := ADocument.CreateDirEntry(AParts[I], ET_STORAGE,
          AParentDir, True);
...
begin
  ...
  try
    ...
    AStream := ADocument.CreateStream(ARootDir,
      TdxOLECryptoContainerDataSpaces.EncryptionInfo);
    WriteCardinalProc(AStream, AEncryptor.ID);
    AEncryptor.SaveInfo(AStream);

    ASourceStream.Position := 0;
    AEncryptor.EncryptPackage(ASourceStream, ADocument.CreateStream(ARootDir,
      TdxOLECryptoContainerDataSpaces.PackageName));

    TdxOLECryptoContainerDataSpaces.WriteVersion(CreateStreamInDir(ADocument,
      ARootDir, TdxOLECryptoContainerDataSpaces.DataSpaceVersion));
    TdxOLECryptoContainerDataSpaces.WriteDataSpaceMap
      (CreateStreamInDir(ADocument, ARootDir,
      TdxOLECryptoContainerDataSpaces.DataSpaceMap));
    TdxOLECryptoContainerDataSpaces.WriteDataSpaceDefinition
      (CreateStreamInDir(ADocument, ARootDir,
      TdxOLECryptoContainerDataSpaces.DataSpaceDefinition));
    TdxOLECryptoContainerDataSpaces.WriteDataTransformInfo
      (CreateStreamInDir(ADocument, ARootDir,
      TdxOLECryptoContainerDataSpaces.DataTransformInfo));
    ADocument.Commit;
  finally
    ADocument.Free;
  end;
