...
type
  ...
  TdxNumberingListApplyScope = (RestartNumbering, ModifyCurrentList,
    ContinuePreviousList, ToSelectedText);

  TdxMultiLevelNumberingListApplyScope = (WholeList, WholeDocument,
    ToSelectedText);

  { TdxBaseNumberingListFormControllerParameters }

  TdxBaseNumberingListFormControllerParameters = class
  ...
  TdxSimpleNumberingListFormControllerParameters = class
    (TdxBaseNumberingListFormControllerParameters)
  private
  ...
  public
    ...
    property EditedLevelIndex: Integer read FEditedLevelIndex
      write FEditedLevelIndex;
  ...
  TdxMultiLevelNumberingListFormController = class
    (TdxSimpleNumberingListController)
  private
  ...
  protected
    ...
    procedure ModifyNumberingListLevel(ATargetList: TdxNumberingList;
      ALevelIndex: Integer);
    function CalculateListInsertionMode: TdxInsertListMode;
    procedure RemoveNumberingList;
    procedure UpdateParagraphsDown(ANumberingListIndex: TdxNumberingListIndex;
      ANewListIndex: TdxNumberingListIndex);
  public
    constructor Create(const AControllerParameters
      : TdxNumberingListFormControllerParameters);
    destructor Destroy; override;
    ...
    property SelectedParagraphsListIndex: TdxNumberingListIndex
      read FSelectedParagraphsListIndex write FSelectedParagraphsListIndex;
    property NewAbstractList: TdxAbstractNumberingList read FNewAbstractList
      write FNewAbstractList;
    property NewListIndex: TdxNumberingListIndex read FNewListIndex
      write FNewListIndex;
    property Paragraphs: TdxParagraphList read FParagraphs;
    property DocumentModel: TdxDocumentModel read GetDocumentModel;
    property ApplyScope: TdxNumberingListApplyScope read FApplyScope
      write FApplyScope;
...
constructor TdxBaseNumberingListFormControllerParameters.Create
  (const AControl: IdxRichEditControl; ALevels: TdxListLevelCollection;
  ALevelIndex: Integer);
begin
...
constructor TdxNumberingListFormControllerParameters.Create(const AControl
  : IdxRichEditControl; const AParagraphs: TdxParagraphList);
begin
  inherited Create(AControl);
...
constructor TdxSimpleNumberingListFormControllerParameters.Create
  (const AControl: IdxRichEditControl; ALevels: TdxListLevelCollection;
  ALevelIndex, AStartNumber: Integer);
begin
...
procedure TdxBulletedListFormController.AssignControllerIndentValues(AAlign,
  AIndent: Variant);
var
  AAlignValue: Integer;
...
begin
  ...
  else if AProbableFirstLineIndent < 0 then
  begin
...
constructor TdxBulletedListFormController.Create(const AControllerParameters
  : TdxBaseNumberingListFormControllerParameters);
begin
  inherited Create;
...
function TdxBulletedListFormController.CreateEditedLevels
...
function TdxBulletedListFormController.GetCharacterProperties
...
function TdxBulletedListFormController.GetFirstLineIndentType
...
procedure TdxBulletedListFormController.SetFirstLineIndent
  (const Value: Integer);
begin
  EditedLevel.FirstLineIndent := Value;
end;

procedure TdxBulletedListFormController.SetFirstLineIndentType
...
constructor TdxSimpleNumberingListController.Create(const AControllerParameters
  : TdxSimpleNumberingListFormControllerParameters);
begin
...
procedure TdxSimpleNumberingListController.SetAlignment
  (const Value: TdxListNumberAlignment);
begin
  EditedLevel.ListLevelProperties.Alignment := Value;
end;

procedure TdxSimpleNumberingListController.SetFormat
  (const Value: TdxNumberingFormat);
begin
...
procedure TdxMultiLevelNumberingListFormController.SetSeparator
  (const Value: Char);
begin
...
constructor TdxNumberingListFormController.Create(const AControllerParameters
  : TdxNumberingListFormControllerParameters);
begin
  inherited Create;
...
function TdxNumberingListFormController.GetSelectedParagraphsListIndex
...
function TdxNumberingListFormController.GetSelectedAbstractNumberingList
...
function TdxNumberingListFormController.CreateAbstractNumberingListCopy
  (ATargetModel: TdxDocumentModel; ASourceList: TdxAbstractNumberingList)
  : TdxAbstractNumberingList;
begin
...
var
  ...
  ANewNumberingListIndex, AFirstSelectedParagraphListIndex
    : TdxNumberingListIndex;
...
begin
  ...
  ACalculator := DocumentModel.CommandsCreationStrategy.
    CreateNumberingListIndexCalculator(DocumentModel, NewListType);
  try
    ...
    ANewNumberingList.Levels[LevelIndex].ListLevelProperties.Start :=
      NewAbstractList.Levels[LevelIndex].ListLevelProperties.Start;
    UpdateParagraphsDown(AFirstSelectedParagraphListIndex,
      ANewNumberingListIndex);
  finally
    ACalculator.Free;
  end;
end;
...
function TdxNumberingListFormController.
...
function TdxNumberingListFormController.GetFirstNumberingListIndex
  : TdxNumberingListIndex;
var
  AParagraph: TdxParagraph;
  I: Integer;
...
procedure TdxNumberingListFormController.UpdateParagraphsDown
...
begin
  ...
  begin
    ...
    if not AParagraph.IsInList or
      (AParagraph.NumberingListIndex <> ANumberingListIndex) then
      Continue;
    if (NewListType = TdxNumberingType.Simple) and
      (AParagraph.GetOwnListLevelIndex <> LevelIndex) then
      Continue;
...
procedure TdxNumberingListFormController.ModifyParagraph
...
begin
  ...
  AParagraph.PieceTable.AddParagraphToList(AParagraph.Index, AListIndex,
    ALevelIndex);
end;

procedure TdxNumberingListFormController.ModifyNumberingListLevel
  (ATargetList: TdxNumberingList; ALevelIndex: Integer);
var
...
begin
  ACharacterFormattingInfo := NewAbstractList.Levels[ALevelIndex]
    .CharacterProperties.Info.Info;
  ACharacterFormattingOptions := NewAbstractList.Levels[ALevelIndex]
    .CharacterProperties.Info.Options;
  ACharacterMergedProperties := TdxMergedCharacterProperties.Create
    (ACharacterFormattingInfo, ACharacterFormattingOptions);
  try
    ATargetList.Levels[ALevelIndex].CharacterProperties.CopyFrom
      (ACharacterMergedProperties);
  finally
    ACharacterMergedProperties.Free;
  end;
  AParagraphFormattingInfo := NewAbstractList.Levels[ALevelIndex]
    .ParagraphProperties.Info.Info;
  AParagraphFormattingOptions := NewAbstractList.Levels[ALevelIndex]
    .ParagraphProperties.Info.Options;
  AParagraphMergedProperties := TdxMergedParagraphProperties.Create
    (AParagraphFormattingInfo, AParagraphFormattingOptions);
  try
    ATargetList.Levels[ALevelIndex].ParagraphProperties.CopyFrom
      (AParagraphMergedProperties);
  finally
    AParagraphMergedProperties.Free;
  end;
  ATargetList.Levels[ALevelIndex].ListLevelProperties.Alignment :=
    NewAbstractList.Levels[ALevelIndex].ListLevelProperties.Alignment;
  ATargetList.Levels[ALevelIndex].ListLevelProperties.Format :=
    NewAbstractList.Levels[ALevelIndex].ListLevelProperties.Format;
end;
...
begin
  ...
  begin
    if DocumentModel <> NewAbstractList.DocumentModel then
      ANewAbstractList := CreateAbstractNumberingListCopy(DocumentModel,
        NewAbstractList)
    else
      ANewAbstractList := NewAbstractList;
    ACommand := TdxInsertListFormCommand.Create(FControl, ANewAbstractList,
      LevelIndex, AMode);
  end;
  try
    ACommand.Execute;
...
function TdxNumberingListFormController.CalculateListInsertionMode
  : TdxInsertListMode;
var
...
begin
  ...
  begin
    if TdxNumberingListHelper.GetListType(NewAbstractList) = TdxNumberingType.MultiLevel
    then
  ...
  begin
    ...
    if (ASelectedListType = ANewListType) or
      (ANewListType = TdxNumberingType.Bullet) then
      Result := TdxInsertListMode.KeepLevelIndex
    else if ANewListType = TdxNumberingType.Simple then
      Result := TdxInsertListMode.ChangeLevelIndex
    else
      Result := TdxInsertListMode.KeepLevelIndex;
  end;
