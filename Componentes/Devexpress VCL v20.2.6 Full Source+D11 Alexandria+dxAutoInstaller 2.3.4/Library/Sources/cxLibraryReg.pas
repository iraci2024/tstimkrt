...
uses
  ...
  cxPropertiesStore, cxPropertiesStoreEditor, dxGDIPlusAPI, dxGDIPlusClasses,
  cxContainer, cxLookAndFeels, cxClasses,
  cxStyles, cxControls, cxLookAndFeelPainters, dxScreenTip, dxForms,
  cxDesignWindows, cxStyleRepositoryEditor,
  cxImageListEditor, dxCoreReg, dxGallery, cxScrollBar,
  dxIconLibraryEditorHelpers, dxCalloutPopup, dxDirectX.D2D.Classes;
...
type
  ...
  strict private
    ...
    procedure CheckDesigningComponents(ADesigner: IDesigner;
      AOriginalImage: TObject);
...
procedure ShowEventMethod(ADesigner: IDesigner; AInstance: TObject;
  const AEventName, AMethodName: string;
...
uses
  ...
  cxComponentCollectionEditor, dxScreenTipRepositoryEditor, dxGalleryDesigner,
  ExtDlgs,
  Variants, dxDPIAwareUtils, dxSmartImage, dxPictureEditor;
...
function dxSkinListCanUseSkin(const ASkinName: string): Boolean;
begin
  Result := not Assigned(FdxSkinsListFilterProc) or
    FdxSkinsListFilterProc(ASkinName);
end;
...
function TcxLookAndFeelControllerEditor.InternalGetVerb
...
function TcxLookAndFeelControllerEditor.GetLookAndFeelController
...
function TcxStyleRepositoryComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  if AIndex = 0 then
    Result := 'Edit...'
...
procedure TcxStyleRepositoryComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  if AIndex = 0 then
...
function TcxStyleRepositoryComponentEditor.GetStyleRepository
  : TcxStyleRepository;
begin
...
begin
  if TcxStyles(GetComponent(0)).IsValidStyleSheet
    (TcxCustomStyleSheet(Designer.GetComponent(S))) then
    FStrProc(S);
end;
...
function TcxPropertiesStoreComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  case AIndex of
    0:
...
procedure TcxPropertiesStoreComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
...
begin
  case AIndex of
    0:
      ShowPropertiesStoreEditor(PropertiesStore, Designer.Root);
    2:
      DsgnStoreTo;
    3:
      DsgnRestoreFrom;
...
function TcxPropertiesStoreComponentEditor.GetPropertiesStore
  : TcxPropertiesStore;
begin
...
begin
  for I := 0 to PropCount - 1 do
    if not TcxContainer(GetComponent(I)).IsStylePropertyPublished(Prop.GetName,
      GetName <> 'Style') then
      Exit;
  FProc(Prop);
end;
...
procedure TcxContainerStyleAssignedValuesProperty.GetProperties
  (Proc: TGetPropProc);
var
...
procedure TcxContainerStyleAssignedValuesProperty.PopulateVisiblePropertiesList
  ...
  begin
    ...
    try
      ...
      begin
        if not AStyle.GetStyleValue(dxShortStringToString(APPropList[I].Name),
          AStyleValue) then
          Continue;
        if (AStyle.Container = nil) and
          ((AStyle.State = csNormal) or AStyle.IsExtendedStylePropertyPublished
          (dxShortStringToString(APPropList[I].Name))) or
          (AStyle.Container <> nil) and
          AStyle.Container.IsStylePropertyPublished
          (dxShortStringToString(APPropList[I].Name), AStyle.State <> csNormal)
        then
        begin
          AStyleValueIndex := AStyleValueList.IndexOf
            (dxShortStringToString(APPropList[I].Name));
          if AStyleValueIndex = -1 then
            AStyleValueList.AddObject(dxShortStringToString(APPropList[I].Name),
              TObject(1))
          else
  ...
  begin
    ...
    case Index of
      0:
        Result := '-';
      1:
        Result := GetProductName + ' ' + GetProductVersion;
      2:
...
begin
  if SameText(Prop.GetName, 'SkinName') and
    not(AreSkinsAvailable and AreSkinsSupported) then
    Exit;
...
begin
  Result := dxIsDirect2DSupported(GetComponent(0));
end;
...
procedure TcxLookAndFeelAssignedValuesProperty.GetProperties
  (Proc: TGetPropProc);
begin
  FProc := Proc;
  inherited GetProperties(GetPropProc);
...
begin
  for I := 0 to PropCount - 1 do
    if SameText(Prop.GetName, 'lfvSkinName') and
      ...
      (cxLookAndFeelPaintersManager.GetPainter(lfsSkin) = nil)) then
      Exit;
  FProc(Prop);
end;
...
begin
  if Supports(dxIconLibraryIntf, IdxImageCollectionEditor,
    AImageCollectionEditor) then
  begin
...
begin
  ShowFormEditorClass(Designer, Component, ImageCollection.Items, 'Items',
    TfrmComponentCollectionEditor);
end;

function TcxImageCollectionComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  case AIndex of
    0:
    ...
    1:
      Result := 'Add Images...';
    2:
      Result := 'Add Images from Icon Library...';
  else
    Result := '';
  end;
end;
...
procedure TcxImageCollectionComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  case AIndex of
    0:
    ...
    1:
      AddImages;
    2:
      AddImagesFromIconLibrary;
  end;
end;

procedure TcxImageCollectionComponentEditor.AddImagesFromFiles
  (AFiles: TStrings);
var
  I: Integer;
begin
...
function TcxImageCollectionComponentEditor.GetImageCollection
  : TcxImageCollection;
begin
...
function TdxScreenTipRepositoryComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  Result := 'Edit...';
end;
...
procedure TdxScreenTipRepositoryComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  ShowFormEditorClass(Designer, Component, Repository.Items, 'Items',
    TfrmScreenTipRepositoryEditor);
end;

function TdxScreenTipRepositoryComponentEditor.GetRepository
...
type
  TcxStyleProperty = class(TComponentProperty, ICustomPropertyDrawing,
...
procedure TcxStyleProperty.ListMeasureWidth(const Value: string;
...
procedure TcxStyleProperty.ListMeasureHeight(const Value: string;
...
var
  ...
  begin
    Result := cxTextHeight(ACanvas.Handle) + 2 *
      (StylePreviewOffset + StylePreviewBorderSize + StyleTextOffset);
  end;
...
begin
  ...
  if AComponent = nil then
    Inc(AHeight)
  else if AComponent is TcxStyle then
  begin
...
var
  ...
  begin
    Result := ARect;
    Inc(Result.Left, StylePreviewAreaWidth + StylePreviewAreaOffset +
      StyleTextOffset);
    Result.Top := (Result.Top + Result.Bottom - ACanvas.TextHeight
      (Value)) div 2;
  end;
...
(* procedure DrawStyleAssignedValuesInfo;
  ...
  var
    ...
    begin
      Result := (ATextR.Top + ATextR.Bottom - ACanvas.TextHeight
        (PreviewText)) div 2;
    end;

  begin
    if AStyle.AssignedValues * [svFont, svTextColor] = [] then
      Exit;
    if svFont in AStyle.AssignedValues then
      ACanvas.Font := AStyle.Font;
    ...
    ACanvas.TextRect(ATextR, ATextR.Left + StyleTextOffset, GetTextTopBound,
      PreviewText);
...
begin
  ...
  Designer.GetComponentNames(GetTypeData(TcxStyleRepository.ClassInfo),
    GetStyleRepositoryNameProc);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Proc(sCreateNewStyleInRepositoryBegin + S +
        sCreateNewStyleInRepositoryEnd);
    end;
  finally
...
var
  ...
  begin
    AStyle := AStyleRepository.CreateItemEx(TcxStyle, AStyleRepository.Owner)
      as TcxStyle;
...
begin
  ...
  begin
    ...
    AStyleRepository.Name := Designer.UniqueName
      (Copy(TcxStyleRepository.ClassName, 2,
      Length(TcxStyleRepository.ClassName) - 1));
    CreateAndAssignNewStyle(AStyleRepository);
  end
  else if Copy(Value, 1, Length(sCreateNewStyleInRepositoryBegin)) = sCreateNewStyleInRepositoryBegin
  then
  begin
    AName := Copy(Value, Length(sCreateNewStyleInRepositoryBegin) + 1,
      Length(Value) - (Length(sCreateNewStyleInRepositoryBegin) +
      Length(sCreateNewStyleInRepositoryEnd)));
...
function TdxPictureEditor.Execute(ADesigner: IDesigner;
  AOriginalImage: TObject): Boolean;
var
  AAdvancedPictureDialog: IdxAdvancedPictureEditor;
begin
  CheckDesigningComponents(ADesigner, AOriginalImage);
  if Supports(dxIconLibraryIntf, IdxAdvancedPictureEditor,
    AAdvancedPictureDialog) then
    Result := AAdvancedPictureDialog.Execute(Picture, GraphicClass, FImportList,
      FPreferSmartImage)
  else
    Result := dxExecutePictureEditor(Picture, GraphicClass, FImportList,
      FPreferSmartImage);
end;
...
procedure TdxPictureEditor.CheckDesigningComponent(const AObjectName: string);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if ACanAdd then
          FImportList.AddObject(AObjectName + '.' +
            GetPropName(APropInfo), AObject);
      end;
    finally
      FreeMem(APropList);
    end;
  ...
  begin
    Result := (AComponent is TcxCustomComponent) or
      (AComponent is TcxCustomControl);
  end;
...
procedure TdxPictureEditor.CheckDesigningComponents(ADesigner: IDesigner;
  AOriginalImage: TObject);
begin
  FPreferSmartImage := False;
  FTempOriginalImage := AOriginalImage;
  FTempDesigner := ADesigner;
  FTempDesigner.GetComponentNames(GetTypeData(TComponent.ClassInfo),
    CheckDesigningComponent);
  FTempDesigner := nil;
...
begin
  if Supports(GetComponent(0), IdxMultiPartGlyphSupport, AMultiPartGlyphSupport)
  then
  begin
    AMultiPartGlyphEditor := TdxMultiPartGlyphEditor.Create;
    try
      if AMultiPartGlyphEditor.Edit(TdxSmartGlyph(GetImage),
        AMultiPartGlyphSupport) then
        Designer.Modified;
    finally
...
begin
  Result := Format('%d dpi (%d %%)',
    [AValue, MulDiv(100, AValue, dxDefaultDPI)]);
end;
...
{$IFEND}
  ...
  RegisterSelectionEditor(TcxLookAndFeelController,
    TdxSkinsBaseSelectionEditor);
  ...
  RegisterComponents(dxLibraryProductPage, [TcxPropertiesStore, TcxImageList,
    TcxImageCollection]);
  RegisterComponents(dxLibraryProductPage, [TcxLookAndFeelController,
    TcxStyleRepository]);
  ...
  RegisterComponentEditor(TcxLookAndFeelController,
    TcxLookAndFeelControllerEditor);
  RegisterComponentEditor(TcxStyleRepository,
    TcxStyleRepositoryComponentEditor);
  RegisterComponentEditor(TcxImageList, TcxImageListComponentEditor);
  RegisterComponentEditor(TcxPropertiesStore,
    TcxPropertiesStoreComponentEditor);
  RegisterComponentEditor(TcxImageCollection,
    TcxImageCollectionComponentEditor);
  RegisterComponentEditor(TdxScreenTipRepository,
    TdxScreenTipRepositoryComponentEditor);
  ...
  RegisterPropertyEditor(TypeInfo(Integer), TcxImageList, 'SourceDPI',
    TdxDPIPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TdxSmartGlyph, 'SourceDPI',
    TdxDPIPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TcxCustomStyleSheet), TcxStyles, 'StyleSheet',
    TcxStyleSheetComponentProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxSmartImage), nil, '',
    TdxSmartImageGraphicProperty);
  RegisterPropertyEditor(TypeInfo(TdxSmartGlyph), nil, '',
    TdxSmartGlyphGraphicProperty);
  RegisterPropertyEditor(TypeInfo(TcxStyleController), TcxStyleController,
    'FakeStyleController', nil);
  RegisterPropertyEditor(TypeInfo(TcxStyleController), TcxContainer,
    'FakeStyleController', nil);
  RegisterPropertyEditor(TypeInfo(TcxContainerStyle), TcxContainer, '',
    TcxContainerStyleProperty);
  RegisterPropertyEditor(TypeInfo(TcxContainerStyle), TcxStyleController, '',
    TcxStyleControllerStyleProperty);
  RegisterPropertyEditor(TypeInfo(TComponent), TcxControl, 'PopupMenu',
    TcxControlPopupMenuProperty);
  RegisterPropertyEditor(TypeInfo(TcxStyle), nil, '', TcxStyleProperty);
  RegisterPropertyEditor(TypeInfo(TcxLookAndFeel), nil, '',
    TcxLookAndFeelProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxScreenTipCollection), nil, '',
    TdxScreenTipCollectionProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxCustomScreenTipBand, 'Glyph',
    TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(Cardinal), TcxLocalizer, 'Locale',
    TcxLocalizerLocaleProperty);
  RegisterPropertyEditor(TypeInfo(TFileName), TcxLocalizer, 'FileName',
    TcxLocalizerFilenameProperty);
end;
