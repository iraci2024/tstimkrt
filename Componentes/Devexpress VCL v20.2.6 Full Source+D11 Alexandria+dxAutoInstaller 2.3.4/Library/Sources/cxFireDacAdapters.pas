...
function TcxFireDacProviderDetailFilterAdapter.IsCurrentQuery
  (ADataSet: TDataSet; const AParamNames: string;
  const AParamValues: Variant): Boolean;
var
...
begin
  ...
  begin
    ASQLParams :=
    {$IFDEF DELPHI19}TFDQuery{$ELSE}TADQuery{$ENDIF}(ADataSet).Params;
...
cxDetailFilterControllers.RegisterAdapter
  ({$IFDEF DELPHI19}TFDQuery{$ELSE}TADQuery{$ENDIF},
  TcxFireDacProviderDetailFilterAdapter);
cxFilterOperatorAdapters.RegisterAdapter
  ({$IFDEF DELPHI19}TFDQuery{$ELSE}TADQuery{$ENDIF},
  TcxFireDacFilterOperatorAdapter);

finalization

cxDetailFilterControllers.UnregisterAdapter
  ({$IFDEF DELPHI19}TFDQuery{$ELSE}TADQuery{$ENDIF},
  TcxFireDacProviderDetailFilterAdapter);
cxFilterOperatorAdapters.UnregisterAdapter
  ({$IFDEF DELPHI19}TFDQuery{$ELSE}TADQuery{$ENDIF},
  TcxFireDacFilterOperatorAdapter);
