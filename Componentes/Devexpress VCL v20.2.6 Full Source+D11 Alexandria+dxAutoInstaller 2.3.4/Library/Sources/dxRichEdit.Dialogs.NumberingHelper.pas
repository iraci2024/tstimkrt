...
uses
  Types, SysUtils, Classes, Generics.Defaults, Generics.Collections, DB,
  Graphics, Controls,
  dxContainerListBox,
{$IFNDEF DELPHI22}
  DBPlatform,
...
type
  ...
  IdxRichEditNumberingListForm = interface
    ...
    function CreateNumberingListFormCore(AListLevels: TdxListLevelCollection;
      AFirstParagraphStartIndex: Integer; AMultiLevels: Boolean)
    ...
    property ContinuePreviousList: Boolean read GetContinuePreviousList
      write SetContinuePreviousList;
    property RestartNumbering: Boolean read GetRestartNumbering
      write SetRestartNumbering;
  end;
  ...
  IdxRichEditBulletedListForm = interface
    ...
    procedure InitializeComponents(const ASymbolsProperties
      : array of TdxSymbolProperties);
  ...
  private
    ...
    procedure AddMultiLevelNumberingListsToParagraph
      (APieceTable: TdxPieceTable);
    procedure AddNumberingListsToParagraph(APieceTable: TdxPieceTable);
    procedure AddNumberingListLevelToParagraph(APieceTable: TdxPieceTable;
      AParagraph: TdxParagraph; ANumberingListIndex: TdxNumberingListIndex;
      ALevelIndex: Integer);
    procedure AdjustLevel(const ALevel: IdxListLevel; ALevelIndex: Integer);
    function CalculateItemHeight: Integer;
    function CalculateLevelIndentAdjustment(const ALevel: IdxListLevel)
      : Integer;
    function ContainsListWithSameProperties(AAbstractList
      : TdxAbstractNumberingList): Boolean;
    function CreateSimpleControl: TdxSimpleRichEditControl;
    function CreateSimpleControlNoneText: TdxSimpleRichEditControl;
    function CreateNumberingListCopy(ATargetDocumentModel: TdxDocumentModel;
      ASourceList: TdxAbstractNumberingList): TdxNumberingList;
    procedure CreatePreviewContent(APieceTable: TdxPieceTable;
    ...
    procedure InitializeControlCore(ANumberingType: TdxNumberingType;
      ADocumentModel: TdxDocumentModel);
    procedure PrepareListPreviewContent(ADocumentModel: TdxDocumentModel;
  ...
  public
    ...
    procedure InitializeControl(AListBox: TdxCustomContainerListBox;
      ANumberingType: TdxNumberingType; ALevelIndex: Integer);
    property SelectedAbstractList: TdxAbstractNumberingList
      read GetSelectedAbstractList write SetSelectedAbstractList;
  ...
  public
    ...
    procedure ApplyChanges(ANewList: TdxAbstractNumberingList;
      AApplyScope: TdxNumberingListApplyScope; ANewListType: TdxNumberingType;
      ANewListIndex: TdxNumberingListIndex); overload;
    ...
    procedure ShowNumberingListFormCore(ASource: TdxAbstractNumberingList;
  ...
  public const
    SymbolsProperties: array [0 .. 5] of TdxSymbolProperties =
      ((UnicodeChar: #183; FontName: 'Symbol'), (UnicodeChar: #252;
      FontName: 'Wingdings'), (UnicodeChar: #216; FontName: 'Wingdings'),
      (UnicodeChar: #118; FontName: 'Wingdings'), (UnicodeChar: #111;
      FontName: 'Courier New'), (UnicodeChar: #167; FontName: 'Wingdings'));
  private
  ...
  public
    class function GetActiveSymbolIndex(const AActiveProperties
      : TdxSymbolProperties): Integer;
  ...
  public const
    FieldNames: array [0 .. 8] of string = ('F0', 'F1', 'F2', 'F3', 'F4', 'F5',
      'F6', 'F7', 'F8');
  ...
  protected
    ...
    function GetRecord(Buffer: PByte; GetMode: TGetMode; DoCheck: Boolean)
      : TGetResult; override;
...
function Format(const AFormat: string; const Args: array of string)
  : string; overload;
...
procedure TdxNumberingListBoxHelper.AddMultiLevelNumberingListsToParagraph
  (APieceTable: TdxPieceTable);
var
  ADocumentModel: TdxDocumentModel;
...
begin
  ...
  begin
    ...
    AddNumberingListLevelToParagraph(APieceTable, AParagraphs[ALevelIndex],
      ANumberingListIndex, ALevelIndex);
  end;
  APieceTable.AddNumberingListToParagraph(AParagraphs[ACount],
    ANumberingListIndex, 0);
end;
...
procedure TdxNumberingListBoxHelper.AddNumberingListLevelToParagraph
  (APieceTable: TdxPieceTable; AParagraph: TdxParagraph;
...
begin
  ADocumentModel := APieceTable.DocumentModel;
  ALevel := ADocumentModel.NumberingLists[ANumberingListIndex].Levels
    [ALevelIndex];
  ...
  try
    ...
    APieceTable.AddNumberingListToParagraph(AParagraph, ANumberingListIndex,
      ALevelIndex);
  finally
    ATabs.Free;
  end;
...
procedure TdxNumberingListBoxHelper.AddNumberingListsToParagraph
  (APieceTable: TdxPieceTable);
var
  ADocumentModel: TdxDocumentModel;
  ANumberingListIndex: TdxNumberingListIndex;
...
begin
  ...
  for I := TdxParagraphIndex(0) to ACount - 1 do
    AddNumberingListLevelToParagraph(APieceTable, AParagraphs[I],
      ANumberingListIndex, FLevelIndex);
end;

procedure TdxNumberingListBoxHelper.AdjustLevel(const ALevel: IdxListLevel;
  ALevelIndex: Integer);
var
  ALevelOffset: Integer;
  AAdjustment: Integer;
begin
  ALevelOffset := ALevel.DocumentModel.UnitConverter.PixelsToModelUnits
    (20, DpiX);
  AAdjustment := CalculateLevelIndentAdjustment(ALevel);
  ALevel.ParagraphProperties.LeftIndent := ALevel.ParagraphProperties.LeftIndent
    - AAdjustment;
  ALevel.ParagraphProperties.LeftIndent := ALevel.ParagraphProperties.LeftIndent
    + ALevelIndex * ALevelOffset;
...
function TdxNumberingListBoxHelper.CalculateLevelIndentAdjustment
  (const ALevel: IdxListLevel): Integer;
begin
  case ALevel.ParagraphProperties.FirstLineIndentType of
    TdxParagraphFirstLineIndent.Hanging:
      Result := ALevel.ParagraphProperties.LeftIndent -
        ALevel.ParagraphProperties.FirstLineIndent;
...
function TdxNumberingListBoxHelper.ContainsListWithSameProperties
  (AAbstractList: TdxAbstractNumberingList): Boolean;
var
  I: Integer;
...
begin
  ...
  begin
    AExistingList := (ListBox.ControlItems[I] as TdxSimpleRichEditControl)
      .AbstractList;
...
function TdxNumberingListBoxHelper.CreateNumberingListCopy(ATargetDocumentModel
  : TdxDocumentModel; ASourceList: TdxAbstractNumberingList): TdxNumberingList;
var
  AAbstractNumberingList: TdxAbstractNumberingList;
begin
  AAbstractNumberingList := TdxAbstractNumberingList.Create
    (ATargetDocumentModel);
  AAbstractNumberingList.CopyFrom(ASourceList);
  ATargetDocumentModel.AddAbstractNumberingListUsingHistory
    (AAbstractNumberingList);
  Result := TdxNumberingList.Create(ATargetDocumentModel, 0);
  ATargetDocumentModel.AddNumberingListUsingHistory(Result);
end;

procedure TdxNumberingListBoxHelper.CreatePreviewContent
  (APieceTable: TdxPieceTable; AParagraphCount: Integer);
const
  ListParagraphContent = TdxCharacters.TabMark + TdxCharacters.LineBreak +
...
begin
  ...
  begin
    ...
    APieceTable.InsertPlainText(TdxDocumentLogPosition(0),
      ListParagraphContent);
  end;
end;

function TdxNumberingListBoxHelper.CreateSimpleControl
  : TdxSimpleRichEditControl;
var
  ABackColor: TdxAlphaColor;
begin
  Result := TdxSimpleRichEditControl.Create(FListBox);
  Result.SetBounds(0, 0, FListBox.ColumnWidth - 2 * FListBox.ItemPadding,
    FListBox.ItemHeight - 2 * FListBox.ItemPadding);
  Result.Font.Size := 10;
...
function TdxNumberingListBoxHelper.CreateSimpleControlNoneText
  : TdxSimpleRichEditControl;
var
  ACache: TdxFontCache;
  APieceTable: TdxPieceTable;
...
begin
  ...
  APaddingTop := Result.DocumentModel.UnitConverter.PixelsToModelUnits
    (APaddingTop);
...
procedure TdxNumberingListBoxHelper.FormatPreviewContent
  (APieceTable: TdxPieceTable);
var
...
begin
  ...
  ALength := APieceTable.DocumentEndLogPosition -
    APieceTable.DocumentStartLogPosition;
  AUnderlineModifier := TdxPieceTableModifiers.
    CreateRunFontUnderlineTypeModifier(TdxUnderlineType.ThickSingle);
  try
    APieceTable.ApplyCharacterFormatting(AFrom, ALength, AUnderlineModifier);
    AUnderlineColorModifier :=
      TdxPieceTableModifiers.CreateRunUnderlineColorModifier
      (TdxAlphaColors.BtnShadow);
    try
      APieceTable.ApplyCharacterFormatting(AFrom, ALength,
        AUnderlineColorModifier);
    finally
...
function TdxNumberingListBoxHelper.GetSelectedAbstractList
  : TdxAbstractNumberingList;
begin
  if FListBox.SelectedIndex < FListBox.ControlItems.Count then
    Result := TdxSimpleRichEditControl
      (FListBox.ControlItems[FListBox.SelectedIndex]).AbstractList
  else
    Result := nil;
end;

function TdxNumberingListBoxHelper.GetSelectedListIndex
  (AValue: TdxAbstractNumberingList): Integer;
begin
  ...
  for Result := 0 to FListBox.ControlItems.Count - 1 do
    if TdxSimpleRichEditControl(FListBox.ControlItems[Result])
      .AbstractList.IsEqual(AValue) then
      Exit;
  Assert(False);
  Result := -1;
end;
...
procedure TdxNumberingListBoxHelper.InitializeControl
  (AListBox: TdxCustomContainerListBox; ANumberingType: TdxNumberingType;
  ALevelIndex: Integer);
begin
  ...
  AddNoneItem;
  InitializeControlCore(ANumberingType, TdxCustomRichEditControl(Control)
    .DocumentModel);
  InitializeControlCore(ANumberingType, TdxCustomRichEditControl(Control)
    .InnerControl.DocumentModelTemplate);
  FListBox.Refresh;
end;

procedure TdxNumberingListBoxHelper.InitializeControlCore(ANumberingType
  : TdxNumberingType; ADocumentModel: TdxDocumentModel);
var
...
procedure TdxNumberingListBoxHelper.PrepareListPreviewContent
  (ADocumentModel: TdxDocumentModel; APreviewParagraphCount: Integer;
...
begin
  ...
  try
    ...
    if TdxNumberingListHelper.GetListType(ASourceList) = TdxNumberingType.MultiLevel
    then
...
procedure TdxNumberingListBoxHelper.SetSelectedAbstractList
  (const Value: TdxAbstractNumberingList);
begin
...
procedure TdxNumberingListFormHelper.ApplyChanges
  (ANewList: TdxAbstractNumberingList; AApplyScope: TdxNumberingListApplyScope;
  ANewListType: TdxNumberingType; ANewListIndex: TdxNumberingListIndex);
begin
...
procedure TdxNumberingListFormHelper.ApplyChanges
  (ANewList: TdxAbstractNumberingList);
begin
  ApplyChanges(ANewList, TdxNumberingListApplyScope.ToSelectedText,
    TdxNumberingType.Bullet, NumberingListIndexListIndexNotSet);
end;
...
begin
  if ((Owner.IsNumberedTabSelected or Owner.IsOutlineNumberedTabSelected) and
    not DoSelectedParagraphsHaveDifferentNumberingLists and
    AreThereParagraphsInList) then
  begin
    ...
    AIndexOfParagraphInListBeforeCurrent :=
      GetIndexOfParagraphInListBeforeCurrent;
    AIsNumbListOfParagraphDifferent := IsParagraphInDifferentListFromCurrent
      (AIndexOfParagraphInListBeforeCurrent);

    if Owner.RestartNumbering then
      AApplyScope := TdxNumberingListApplyScope.RestartNumbering
    else if Owner.ContinuePreviousList then
      if AIsNumbListOfParagraphDifferent then
        AApplyScope := TdxNumberingListApplyScope.ContinuePreviousList
      else if not IsSelectedParagraphsBulletedLists then
    ...
    if AIndexOfParagraphInListBeforeCurrent >= ParagraphIndexMinValue then
      ANewListIndex := AParagraphs[AIndexOfParagraphInListBeforeCurrent]
        .NumberingListIndex
    else
      ANewListIndex := NumberingListIndexListIndexNotSet;
    ApplyChanges(Owner.ActiveGallery.SelectedAbstractList, AApplyScope,
      ANewListType, ANewListIndex);
  end
  else
...
constructor TdxNumberingListFormHelper.Create(const AOwner
...
function TdxNumberingListFormHelper.
  DoSelectedParagraphsHaveDifferentNumberingLists: Boolean;
var
  I: Integer;
  AFirstSelectedParagraphListIndex: TdxNumberingListIndex;
  AParagraph: TdxParagraph;
begin
  AFirstSelectedParagraphListIndex :=
    Controller.Paragraphs.First.NumberingListIndex;
  Result := False;
...
function TdxNumberingListFormHelper.
  GetIndexOfParagraphInListBeforeCurrent: Integer;
const
...
function TdxNumberingListFormHelper.IsParagraphInDifferentListFromCurrent
...
begin
  ...
  AFirstSelectedParagraph := GetFirstParagraphInList;
  AParagraphs := AFirstSelectedParagraph.PieceTable.Paragraphs;
  AAreNumberingListsIndexesDifferent := AParagraphs[AIndex].NumberingListIndex
    <> AFirstSelectedParagraph.NumberingListIndex;
...
begin
  ...
  Result := (AAbstractNumberingList <> nil) and
    (AAbstractNumberingList <> Controller.NoneList) and
    (TdxNumberingListHelper.GetListType(AAbstractNumberingList)
    = TdxNumberingType.Bullet);
end;

procedure TdxNumberingListFormHelper.ShowNumberingListFormCore
  (ASource: TdxAbstractNumberingList; AListLevels: TdxListLevelCollection;
  AMultiLevels: Boolean);
var
...
begin
  ...
  if AAreThereParagraphsInList then
    AFirstSelectedParagraphStartIndexes :=
      AFirstSelectedParagraph.PieceTable.GetRangeListCounters
      (AFirstSelectedParagraph)
  else
    AFirstSelectedParagraphStartIndexes := TIntegerDynArray.Create(1);
  AIndexOfParagraphInListBeforeCurrent :=
    GetIndexOfParagraphInListBeforeCurrent;
  AIsPrevParagraphInDifferentList := IsParagraphInDifferentListFromCurrent
    (AIndexOfParagraphInListBeforeCurrent);

  ASelectedParagraphsHaveDifferentNumberingLists :=
  ...
  if (FOwner.RestartNumbering or ASelectedParagraphsHaveDifferentNumberingLists
    or not AAreThereParagraphsInList) then
    AFirstParagraphStartIndex := 1
  else if AIsPrevParagraphInDifferentList then
  begin
    AParagraph := AFirstSelectedParagraph.PieceTable.Paragraphs
      [AIndexOfParagraphInListBeforeCurrent];
    APrevParagraphInDifferentListStartIndexes :=
      AFirstSelectedParagraph.PieceTable.GetRangeListCounters(AParagraph);
    if Length(APrevParagraphInDifferentListStartIndexes)
      = Length(AFirstSelectedParagraphStartIndexes) then
      AFirstParagraphStartIndex := APrevParagraphInDifferentListStartIndexes
        [Controller.LevelIndex] + 1;
  end;

  AForm := FOwner.CreateNumberingListFormCore(AListLevels,
    AFirstParagraphStartIndex, AMultiLevels);
  try
    ...
    begin
      ...
      if AMultiLevels then
        ...
        ANewListType := TdxNumberingType.Simple;
      if (not ASelectedParagraphsHaveDifferentNumberingLists and
        AAreThereParagraphsInList) then
      begin
        AIsListLevelStartIndexCorrect := AFirstSelectedParagraphStartIndexes
          [Controller.LevelIndex] = ASource.Levels[Controller.LevelIndex]
          .ListLevelProperties.Start;
        if FOwner.ContinuePreviousList and AIsPrevParagraphInDifferentList then
          AIsListLevelStartIndexCorrect :=
            AFirstParagraphStartIndex = ASource.Levels[Controller.LevelIndex]
            .ListLevelProperties.Start;
      ...
      if AIndexOfParagraphInListBeforeCurrent >= ParagraphIndexMinValue then
        ANewNumbListIndex := AFirstSelectedParagraph.PieceTable.Paragraphs
          [AIndexOfParagraphInListBeforeCurrent].NumberingListIndex
      else
...
constructor TdxBulletedListFormHelper.Create(const AOwner
  : IdxRichEditBulletedListForm);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
begin
  ...
  AActiveProperties := TdxSymbolProperties.Create(Controller.DisplayFormat[1],
    AFontName);
  ASymbolIndex := TdxBulletedListFormHelper.GetActiveSymbolIndex
    (AActiveProperties);
...
class function TdxBulletedListFormHelper.GetActiveSymbolIndex
  (const AActiveProperties: TdxSymbolProperties): Integer;
begin
...
function TdxDisplayFormatHelper.AppendFormatFromField(const AStringBuilder
  : TStringBuilder; AStartIndex: TdxRunIndex): TdxRunIndex;
var
  S: string;
  AIndex: TdxRunIndex;
  APieceTable: TdxPieceTable;
...
constructor TdxDisplayFormatHelper.Create(const AControl
  : TdxSimpleRichEditControl; const ALevels: TdxListLevelCollection;
  ALevelIndex: Integer);
begin
  inherited Create;
  FControl := AControl;
...
begin
  ...
  AInsertPosition := FControl.DocumentModel.MainPieceTable.
    DocumentEndLogPosition;
  FControl.DocumentModel.MainPieceTable.InsertText(AInsertPosition, AFieldCode);
  FControl.DocumentModel.MainPieceTable.CreateField(AInsertPosition,
    Length(AFieldCode));
...
begin
  if FStringBuilder.Length > 0 then
    FControl.DocumentModel.MainPieceTable.InsertText
      (FControl.DocumentModel.MainPieceTable.DocumentEndLogPosition,
...
begin
  ...
  try
    ...
    begin
      ...
      else if (ARun is TdxTextRun) then
      begin
        S := TdxTextRunBaseAccess(ARun).GetTextFast(ATextBuffer);
...
class function TdxDisplayFormatHelper.IndexOf(const AArray: array of string;
  const AValue: string): Integer;
var
  I: Integer;
begin
...
begin
  APrepareFormat := Format(AFormat, ['{0}', '{1}', '{2}', '{3}', '{4}', '{5}',
    '{6}', '{7}', '{8}']);
...
begin
  ...
  for I := 0 to ACount - 1 do
  begin
    AUpdateResult := AUpdater.UpdateField(AFields[I],
      TdxMailMergeDataMode.ViewMergedData);
    AUpdateResult.Free;
  end;
end;
...
function TdxNumberingListDataSet.GetFieldData(Field: TField;
  {$IFDEF DELPHI18}var {$ENDIF} Buffer: TValueBuffer): Boolean;
var
  AValue: string;
{$IFNDEF DELPHIXE3}
...
function TdxNumberingListDataSet.GetRecord(Buffer: PByte; GetMode: TGetMode;
...
constructor TdxNumberingListDataSet.TdxNumberingListTextSource.Create
  (const AOwner: TdxDisplayFormatHelper);
begin
  FOwner := AOwner;
end;

function TdxNumberingListDataSet.TdxNumberingListTextSource.FormatLevelText
  (ALevelIndex: Integer): string;
var
...
function TdxNumberingListDataSet.TdxNumberingListTextSource.GetLevels
  : TdxListLevelCollection;
begin
  Result := FOwner.Levels;
end;

function TdxNumberingListDataSet.TdxNumberingListTextSource.GetListCounters
  : TIntegerDynArray;
var
  I: Integer;
  ACount: Integer;
