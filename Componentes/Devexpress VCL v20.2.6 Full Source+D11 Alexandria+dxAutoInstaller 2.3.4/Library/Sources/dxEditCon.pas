...
uses
  ...
  cxMaskEdit, cxDropDownEdit, cxMemo, dxLayoutcxEditAdapters,
  dxLayoutControlAdapters, dxLayoutContainer, cxClasses,
...
function ConnectEditor(AChart: TdxFlowChart;
  ...
  begin
    ...
    try
      ...
      for AType := Low(AType) to High(AType) do
        AComboItems.Add
          (cxGetResourceString(dxFlowChartArrowStyleNamesMap[AType]));
    finally
...
begin
  ...
  lgTarget.Caption := cxGetResourceString
  ...
  liSourceColor.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorColor);
  liDestColor.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorColor);
  liSArrowSize.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorArrowSize);
  liDArrowSize.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorArrowSize);
  liArrowStyle.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorArrowStyle);
  liArrowStyle2.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorArrowStyle);
  liSLinkedPoint.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorLinkedPoint);
  liDLinkedPoint.Caption := cxGetResourceString
    (@sdxFlowChartConnectionEditorLinkedPoint);
...
begin
  ...
  liSLinkedPoint.Enabled := (AConnection.ObjectSource <> nil) and
    (AConnection.ObjectSource.GetLinkedPointCount > 0);
  if liSLinkedPoint.Enabled then
  begin
    seSLinkedPoint.Properties.MaxValue :=
      AConnection.ObjectSource.GetLinkedPointCount;
    seSLinkedPoint.Properties.MinValue := 1;
    seSLinkedPoint.Value := AConnection.PointSource + 1;
  end;
  liDLinkedPoint.Enabled := (AConnection.ObjectDest <> nil) and
    (AConnection.ObjectDest.GetLinkedPointCount > 0);
  if liDLinkedPoint.Enabled then
  begin
    seDLinkedPoint.Properties.MaxValue :=
      AConnection.ObjectDest.GetLinkedPointCount;
...
begin
  ...
  if IsModified(seSLinkedPoint) then
    AConnection.SetObjectSource(AConnection.ObjectSource,
      seSLinkedPoint.Value - 1);
