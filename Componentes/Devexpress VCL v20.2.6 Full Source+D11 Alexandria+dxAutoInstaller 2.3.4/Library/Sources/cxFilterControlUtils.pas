...
uses
  Variants, Classes, Controls, Forms, SysUtils, dxCore, cxClasses,
  cxDataStorage,
  cxEdit, cxFilter, cxLookAndFeels;

type
  TcxFilterControlOperator = (fcoNone, fcoEqual, fcoNotEqual, fcoLess,
    fcoLessEqual, fcoGreater, fcoGreaterEqual, fcoLike, fcoNotLike, fcoContains,
    fcoNotContains, fcoBeginsWith, fcoEndsWith, fcoBlanks, fcoNonBlanks,
    fcoBetween, fcoNotBetween, fcoInList, fcoNotInList, fcoYesterday, fcoToday,
  ...
  TcxFilterDataType = (fdtText, fdtNumeric, fdtDate, fdtTime, fdtLookup,
    fdtCheck);
  ...
  protected
    class procedure ClearPropertiesEvents(AProperties
  ...
  public
    ...
    class procedure InitializeProperties(AProperties, AEditProperties
      : TcxCustomEditProperties; AHasButtons: Boolean); virtual;
  ...
  public
    function FindHelper(APropertiesClass: TClass)
      : TcxCustomFilterEditHelperClass;
    procedure Unregister(AItemClass, ARegisteredClass: TClass); override;
  end;

function GetFilterOperatorKind(AOperator: TcxFilterControlOperator)
  : TcxFilterOperatorKind;
function GetFilterControlOperator(AOperatorKind: TcxFilterOperatorKind;
  AIsNull: Boolean): TcxFilterControlOperator;
...
function GetFilterControlOperatorText(AOperator
  : TcxFilterControlOperator): string;
...
const
  OperatorMap: array [TcxFilterControlOperator] of TcxFilterOperatorKind =
    (foEqual, foEqual, foNotEqual, foLess, foLessEqual, foGreater,
    foGreaterEqual, foLike, foNotLike, foContains, foNotContains, foBeginsWith,
    foEndsWith, foEqual, foNotEqual, // blank - non blank
    foBetween, foNotBetween, foInList, foNotInList, foYesterday, foToday,
    foTomorrow, foLast7Days, foLastWeek, foLast14Days, foLastTwoWeeks,
    foLast30Days, foLastMonth, foLastYear, foInPast, foThisWeek, foThisMonth,
    foThisYear, foNext7Days, foNextWeek, foNext14Days, foNextTwoWeeks,
...
const
  OperatorKindMap: array [TcxFilterOperatorKind] of TcxFilterControlOperator =
    ...
    fcoNextYear, fcoInFuture, fcoContains, fcoNotContains, fcoBeginsWith,
    fcoEndsWith);
...
begin
  ...
  begin
    for I := 0 to Screen.FormCount - 1 do
      if (Screen.Forms[I] <> AActiveForm) and
        (Screen.Forms[I].FormStyle = fsStayOnTop) then
      begin
        AActiveForm := nil;
        break;
      end;
...
begin
  ...
  if VarIsStr(AValue) and (AValue = '') and not AValueTypeClass.IsString then
    AValue := Null;
  if VarIsNull(AValue) or (AOperator in [fcoLike, fcoNotLike, fcoContains,
...
class function TcxCustomFilterEditHelper.GetFilterDataType(AValueTypeClass
...
begin
  ...
  if IsDateTimeValueTypeClass(AValueTypeClass) then
    Result := fdtDate
  else if not AValueTypeClass.IsString then
    Result := fdtNumeric;
end;

class function TcxCustomFilterEditHelper.GetFilterEdit(AEditProperties
  : TcxCustomEditProperties; AInplaceEditList: TcxInplaceEditList = nil)
  : TcxCustomEdit;
begin
  if AInplaceEditList = nil then
    Result := GetFilterEditClass.Create(nil)
  else
    Result := AInplaceEditList.GetEdit
      (TcxCustomEditPropertiesClass(GetFilterEditClass.GetPropertiesClass));
...
class procedure TcxCustomFilterEditHelper.InitializeProperties(AProperties,
...
class function TcxCustomFilterEditHelper.IsValueValid(AValueTypeClass
  : TcxValueTypeClass; var AValue: Variant): Boolean;
begin
...
function TcxFilterEditsController.FindHelper(APropertiesClass: TClass)
  : TcxCustomFilterEditHelperClass;
begin
