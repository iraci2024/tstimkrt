...
type
  ...
  TdxMailMergeFieldType = (Null, DbColumn);

  TdxMailMergeDestination = (EMail, Fax, NewDocument, Printer);

  TdxMailMergeDataType = (Native, Odbc, Query, Spreadsheet, TextFile);

  TdxMailMergeSourceType = (AddressBook, Database, Document1, Document2,
    EMailProgram, Legacy, Master, Native, Text);
  ...
  IdxMailMergeOptions = interface(IdxDataSourceContainerOptions)
    ...
    property MergeRecords: TdxMergeRecords read GetMergeRecords
      write SetMergeRecords;
  end;

  { TdxMergeFieldName }

  TdxMergeFieldName = class(TcxIUnknownObject, IdxRichEditMergeFieldName,
    IdxComparable<IdxRichEditMergeFieldName>)
  strict private
    FName: string;
    FDisplayName: string;
  ...
  public
    ...
    constructor Create(const AName: string;
      const ADisplayName: string); overload;
  ...
  public
    ...
    property DynamicAddress: Boolean read FDynamicAddress
      write SetDynamicAddress;
    property FieldType: TdxMailMergeFieldType read FFieldType
      write SetFieldType;
    property MappedName: string read FMappedName write SetMappedName;
    property MergeFieldNameLanguageId: Integer read FMergeFieldNameLanguageId
      write SetMergeFieldNameLanguageId;
  ...
  public
    ...
    function FindMapDataByColumnName(const AColumnName: string)
      : TdxFieldMapData; virtual;
    function FindMapDataByMapName(const AMappedFieldName: string)
      : TdxFieldMapData; virtual;

    property ColumnDelimiter: Char read FColumnDelimiter
      write SetColumnDelimiter;
    property DataSourceType: TdxMailMergeSourceType read FDataSourceType
      write SetDataSourceType;
    property FirstRowHeader: Boolean read FFirstRowHeader
      write SetFirstRowHeader;
    property FieldsMapData: TdxNotificationCollection<TdxFieldMapData>
      read FFieldsMapData;
    property TableName: string read FTableName write SetTableName;
    property UdlConnectionString: string read FUdlConnectionString
      write SetUdlConnectionString;

    property ColumnDelimiterChanged: TdxEventHandler
      read FColumnDelimiterChanged;
    ...
    property UdlConnectionStringChanged: TdxEventHandler
      read FUdlConnectionStringChanged;
  ...
  public
    ...
    property DataSourceObjectProperties: TdxDataSourceObjectProperties
      read FDataSourceObjectProperties;
    property DataType: TdxMailMergeDataType read FDataType write SetDataType;
    property Destination: TdxMailMergeDestination read FDestination
      write SetDestination;
    property EMailAddressColumnName: string read FEMailAddressColumnName
      write SetEMailAddressColumnName;
    property LeaveBlankLines: Boolean read FLeaveBlankLines
      write SetLeaveBlankLines;
    property Query: string read FQuery write SetQuery;
    property ViewMergedData: Boolean read FViewMergedData
      write SetViewMergedData;

    property EMailAddressColumnNameChanged: TdxEventHandler
      read FEMailAddressColumnNameChanged;
    property ConnectionStringChanged: TdxEventHandler
      read FConnectionStringChanged;
    ...
    property LeaveBlankLinesChanged: TdxEventHandler
      read FLeaveBlankLinesChanged;
  ...
  TdxMailMergeOptions = class(TcxIUnknownObject, IdxMailMergeOptions,
    IdxDataSourceContainerOptions)
  strict private
...
function TdxMergeFieldName.CompareTo(const AOther
  : IdxRichEditMergeFieldName): Integer;
begin
...
function TdxDataSourceObjectProperties.FindMapDataByColumnName(const AColumnName
  : string): TdxFieldMapData;
begin
  if not FMapColumnName.TryGetValue(AColumnName, Result) then
    Result := nil;
end;

function TdxDataSourceObjectProperties.FindMapDataByMapName
  (const AMappedFieldName: string): TdxFieldMapData;
begin
  FMapFieldMappedName.TryGetValue(AMappedFieldName, Result);
end;

procedure TdxDataSourceObjectProperties.InternalFieldsMapDataChanged
  (Sender: TObject);
var
  ACount: Integer;
  I: Integer;
...
procedure TdxDataSourceObjectProperties.SetDataSourceType
  (const Value: TdxMailMergeSourceType);
begin
...
procedure TdxDataSourceObjectProperties.SetUdlConnectionString
  (const Value: string);
begin
...
procedure TdxMailMergeProperties.SetDestination(const Value
...
procedure TdxMailMergeOptions.SetHeaderFooterLinkToPrevious
  (const Value: Boolean);
begin
