...
uses
  Windows, Forms, SysUtils, Graphics, Types, Generics.Defaults,
  Generics.Collections, dxCore,
  dxCoreClasses, dxCoreGraphics, cxGeometry, cxGraphics, dxGDIPlusAPI,
  dxGDIPlusClasses,
...
type
  ...
  private
    ...
    FPixelStep: Single;
  protected
    function CreatePattern(const APattern: array of Single; ADpiX: Single)
      : TArray<Single>;
  ...
  TdxRichEditPatternLinePainterParameters = class
    (TdxPatternLinePainterParameters)
  private
  ...
  public
    constructor Create(const APainter: IdxPatternLinePaintingSupport;
    ...
    procedure DrawWaveUnderline(const ABounds: TdxRectF; AColor: TdxAlphaColor;
      APenWidth: Single); overload;
    procedure DrawWaveUnderline(const ABounds: TdxRectF; APen: TdxGPPen;
      AStep: Single); overload;
    procedure DrawWaveUnderline(const ABounds: TdxRectF;
      APen: TdxGPPen); overload;
    function MakeFixedWidthPattern(const APattern: TArray<Single>;
      AThickness: Single): TArray<Single>;
    function MakeBoundsAtLeast2PixelsHigh(const ABounds: TdxRectF)
      : TdxRectF; virtual;
  ...
  TdxPatternLinePainterParametersTable = TObjectDictionary<TClass,
    TdxPatternLinePainterParameters>;

  { TdxRichEditPatternLinePainter }

  TdxRichEditPatternLinePainter = class(TdxPatternLinePainter,
    IdxCharacterLinePainter, IdxUnderlinePainter, IdxStrikeoutPainter)
  strict private
  ...
  protected
    ...
    procedure InitializeParameters(AParameters
      : TdxPatternLinePainterParameters); virtual; abstract;
    function GetPixelGraphics: TdxGraphics; override;
    function GetParametersTable: TdxPatternLinePainterParametersTable;
      virtual; abstract;
  public
    constructor Create(const APainter: IdxPatternLinePaintingSupport;
      AUnitConverter: TdxDocumentLayoutUnitConverter);
    ...
    property ParametersTable: TdxPatternLinePainterParametersTable
      read GetParametersTable;
  end;
  ...
  strict private
    class var FParametersTable: TdxPatternLinePainterParametersTable;
    class constructor Initialize;
    class destructor Finalize;
  protected
    function GetParametersTable: TdxPatternLinePainterParametersTable; override;
    procedure InitializeParameters(AParameters
      : TdxPatternLinePainterParameters); override;
  end;
...
function TdxPatternLinePainterParameters.CreatePattern(const APattern
  : array of Single; ADpiX: Single): TArray<Single>;
var
  I: Integer;
begin
...
constructor TdxRichEditPatternLinePainterParameters.Create(AUnitConverter
...
function TdxRichEditPatternLinePainterParameters.PixelsToUnits(AValue,
  ADpi: Single): Single;
begin
...
constructor TdxRichEditPatternLinePainter.Create(const APainter
  : IdxPatternLinePaintingSupport;
  AUnitConverter: TdxDocumentLayoutUnitConverter);
begin
...
function TdxRichEditPatternLinePainter.CreateParameters
  : TdxPatternLinePainterParameters;
begin
...
procedure TdxRichEditPatternLinePainter.DrawStrikeout
...
procedure TdxRichEditPatternLinePainter.DrawUnderline
...
function TdxRichEditPatternLinePainter.GetParameters
  : TdxPatternLinePainterParameters;
begin
  Result := FParameters;
end;
...
begin
  if FParameters is TdxRichEditPatternLinePainterParameters then
    Result := TdxRichEditPatternLinePainterParameters(FParameters)
      .ColumnBreakPattern
  else
...
begin
  ...
  ATopBounds.InitSize(ADrawBounds.Left, ADrawBounds.Top, ADrawBounds.Width,
    AThickness);
  ATopBounds := MakeBoundsAtLeast2PixelsHigh(ATopBounds);
  ABottomBounds.InitSize(ADrawBounds.Left, ATopBounds.Bottom, ADrawBounds.Width,
    AThickness);
...
constructor TdxPatternLinePainter.Create(const APainter
  : IdxPatternLinePaintingSupport;
  AUnitConverter: TdxDocumentLayoutUnitConverter);
begin
  inherited Create;
...
procedure TdxPatternLinePainter.DrawDoubleSolidLine(const ABounds: TdxRectF;
  AColor: TdxAlphaColor);
var
...
begin
  ...
  ATopBounds.InitSize(ADrawBounds.Left, ADrawBounds.Top + AHalfThickness,
    ADrawBounds.Width, AThickness);
  ABottomBounds.InitSize(ADrawBounds.Left, ADrawBounds.Bottom - AHalfThickness,
    ADrawBounds.Width, AThickness);

  ADistance := RoundToPixels(ABottomBounds.Top - ATopBounds.Bottom,
    UnitConverter.Dpi);
  if ADistance <= PixelsToUnits(1, UnitConverter.Dpi) then
    ADistance := PixelsToUnits(1, UnitConverter.Dpi);
  ABottomBounds := cxRectSetTop(ABottomBounds, ATopBounds.Bottom +
    Trunc(ADistance));
...
procedure TdxPatternLinePainter.DrawPatternLine(const ABounds: TdxRectF;
  AColor: TdxAlphaColor; const APattern: TArray<Single>);
var
  APen: TdxGPColorPen;
...
procedure TdxPatternLinePainter.DrawLine(APen: TdxGPPen;
  const ABounds: TdxRectF);
begin
  Painter.DrawLine(APen, ABounds.Left, ABounds.Top, ABounds.Right, ABounds.Top);
end;

procedure TdxPatternLinePainter.DrawSolidLine(const ABounds: TdxRectF;
  AColor: TdxAlphaColor);
var
  APen: TdxGPPen;
begin
  APen := FPainter.GetPen(AColor, Min(cxRectWidth(ABounds),
...
procedure TdxPatternLinePainter.DrawWaveUnderline(const ABounds: TdxRectF;
  APen: TdxGPPen; AStep: Single);
var
  X, Y, ASum: Single;
...
procedure TdxPatternLinePainter.DrawWaveUnderline(const ABounds: TdxRectF;
  AColor: TdxAlphaColor; APenWidth: Single);
var
  APen: TdxGPPen;
begin
...
function TdxPatternLinePainter.MakeBoundsAtLeast2PixelsHigh
...
function TdxPatternLinePainter.MakeFixedWidthPattern(const APattern
  : TArray<Single>; AThickness: Single): TArray<Single>;
var
  ACount: Integer;
  I: Integer;
begin
...
begin
  FParametersTable := TdxPatternLinePainterParametersTable.Create
    ([doOwnsValues]);
end;
...
function TdxRichEditHorizontalPatternLinePainter.GetParametersTable
  : TdxPatternLinePainterParametersTable;
begin
  Result := FParametersTable;
end;

procedure TdxRichEditHorizontalPatternLinePainter.InitializeParameters
  (AParameters: TdxPatternLinePainterParameters);
begin
...
procedure TdxRichEditVerticalPatternLinePainter.DrawLine(APen: TdxGPPen;
  const ABounds: TdxRectF);
begin
  Painter.DrawLine(APen, ABounds.Left, ABounds.Top, ABounds.Left,
    ABounds.Bottom);
end;

function TdxRichEditVerticalPatternLinePainter.GetParametersTable
  : TdxPatternLinePainterParametersTable;
begin
  Result := FParametersTable;
end;

procedure TdxRichEditVerticalPatternLinePainter.InitializeParameters
  (AParameters: TdxPatternLinePainterParameters);
begin
  AParameters.Initialize(Screen.PixelsPerInch);
end;

function TdxRichEditVerticalPatternLinePainter.RotateBounds
  (const ABounds: TdxRectF): TdxRectF;
begin
  Result := cxRectFBounds(ABounds.Top, ABounds.Left, ABounds.Height,
    ABounds.Width);
end;

function TdxRichEditVerticalPatternLinePainter.RotatePoint
  (const APointF: TdxPointF): TdxPointF;
begin
  Result.X := APointF.Y;
  Result.Y := APointF.X;
end;
