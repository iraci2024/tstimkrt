...
type
  ...
  strict protected
    ...
    procedure SetUseDefaultFontSize(ACharacterStyleIndex: Integer;
      AParagraphStyleIndex: Integer); virtual;
    ...
    property PictureBulletInformation: TdxDocPictureBulletInformation
      read FPictureBulletInformation write FPictureBulletInformation;
  public
    constructor Create(AOutput: TdxMemoryStream;
      AFontNameIndex: Integer); overload;
    constructor Create(AOutput: TdxMemoryStream;
      AProperties: TdxCharacterProperties; AFontNameIndex: Integer); overload;
    constructor Create(AOutput: TdxMemoryStream;
      AParagraphStyleInfo: TdxDocParagraphStyleInfo); overload;
    destructor Destroy; override;
    procedure CreateInlinePicturePropertiesModifiers(ACharacterStyleIndex
      : Integer; ADataStreamOffset: Integer);
    procedure CreateCharacterPropertiesModifiers; overload;
    procedure CreateCharacterPropertiesModifiers(ACharacterStyleIndex,
      AParagraphStyleIndex: Integer; ASpecialSymbol: Boolean); overload;
  ...
  protected
    procedure SetUseDefaultFontSize(ACharacterStyleIndex, AParagraphStyleIndex
      : Integer); override;
  public
    constructor Create(AOutput: TdxMemoryStream; AListLevel: TdxListLevel;
...
constructor TdxDocCharacterPropertiesActions.Create(AOutput: TdxMemoryStream;
  AFontNameIndex: Integer);
begin
...
constructor TdxDocCharacterPropertiesActions.Create(AOutput: TdxMemoryStream;
  AProperties: TdxCharacterProperties; AFontNameIndex: Integer);
begin
...
procedure TdxDocCharacterPropertiesActions.
  CreateInlinePicturePropertiesModifiers(ACharacterStyleIndex: Integer;
  ADataStreamOffset: Integer);
begin
  CreateCharacterPropertiesModifiers(ACharacterStyleIndex,
    TdxDocStyleIndexes.DefaultParagraphStyleIndex, True);
  PictureOffsetAction(ADataStreamOffset);
end;

procedure TdxDocCharacterPropertiesActions.CreateCharacterPropertiesModifiers;
begin
  CreateCharacterPropertiesModifiers
    (TdxDocStyleIndexes.DefaultCharacterStyleIndex,
    TdxDocStyleIndexes.DefaultParagraphStyleIndex, False);
end;

procedure TdxDocCharacterPropertiesActions.CreateCharacterPropertiesModifiers
  (ACharacterStyleIndex, AParagraphStyleIndex: Integer;
  ASpecialSymbol: Boolean);
begin
...
procedure TdxDocCharacterPropertiesActions.SetUseDefaultFontSize
  (ACharacterStyleIndex: Integer; AParagraphStyleIndex: Integer);
begin
  if ACharacterStyleIndex <> TdxDocStyleIndexes.DefaultCharacterStyleIndex then
    WriteCharacterStyleIndex(ACharacterStyleIndex)
  else
    FUseDefaultFontSize := AParagraphStyleIndex = TdxDocStyleIndexes.
      DefaultParagraphStyleIndex;
end;
...
begin
  if not CharacterFormattingOptions.UseBackColor or
    TdxAlphaColors.IsTransparentOrEmpty(CharacterFormattingInfo.BackColor) then
    Exit;
  ACommand := TdxDocCommandBackColor.Create;
  try
    ACommand.ShadingDescriptor.BackgroundColor :=
      CharacterFormattingInfo.BackColor;
    ACommand.Write(FWriter);
  finally
    ACommand.Free;
  end;
...
function TdxDocCharacterPropertiesActions.GetActualDoubleFontSize
  (AUseFontSize: Boolean): Integer;
var
...
begin
  ...
  ADefaultStyleCharacterProperties := DocumentModel.ParagraphStyles.DefaultItem.
    CharacterProperties;
...
begin
  ...
  try
    ...
    try
      if CharacterFormattingInfo.FontStrikeoutType = TdxStrikeoutType.Single
      then
      begin
      ...
      else if CharacterFormattingInfo.FontStrikeoutType = TdxStrikeoutType.Double
      then
      begin
...
procedure TdxDocCharacterPropertiesActions.WriteCharacterStyleIndex
...
begin
  if FPictureBulletInformation = nil then
    Exit;
  ACharacterPositionCommand :=
    TdxDocCommandPictureBulletCharacterPosition.Create;
  try
    ACharacterPositionCommand.Value :=
      FPictureBulletInformation.PictureCharacterPosition;
  ...
  try
    ABulletProperties.DefaultPicture :=
      FPictureBulletInformation.DefaultPicture;
    ABulletProperties.PictureBullet := FPictureBulletInformation.PictureBullet;
    ABulletProperties.SuppressBulletResize :=
      FPictureBulletInformation.SuppressBulletResize;
...
procedure TdxDocCharacterPropertiesActions.PictureOffsetAction(ADataStreamOffset
  : Integer);
var
...
constructor TdxDocListCharacterPropertiesActions.Create
  (AOutput: TdxMemoryStream; AListLevel: TdxListLevel; AFontNameIndex: Integer);
begin
  ...
  PictureBulletInformation.SuppressBulletResize :=
    AListLevel.ListLevelProperties.SuppressBulletResize;
end;

procedure TdxDocListCharacterPropertiesActions.SetUseDefaultFontSize
  (ACharacterStyleIndex, AParagraphStyleIndex: Integer);
begin
end;
