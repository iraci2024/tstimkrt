...
{$IFDEF DELPHI16}
  UITypes,
{$ENDIF}
  Types, Windows, Forms, Classes, cxClasses, SysUtils, Graphics, DateUtils,
  Math, RTLConsts,
  Generics.Collections, Generics.Defaults,
  StdCtrls, dxCore, cxGraphics, cxDateUtils, Controls, cxControls,
  cxSchedulerCustomControls,
  dxCoreClasses, cxLookAndFeelPainters, cxGeometry, cxStyles, dxRangeControl,
  cxDrawTextUtils,
...
type
  ...
  TcxSchedulerCustomWeekViewController = class
  ...
  public
    property Navigation: TcxSchedulerCustomWeekViewNavigation
      read GetNavigation;
  end;
  ...
  private
    ...
    function GetScrollPos(const ADatePos: TDateTime;
      ACheckBounds: Boolean = True): Integer;
  ...
  protected
    ...
    property Controller: TcxSchedulerCustomWeekViewController
      read GetController;
    property CompressWeekEnd: Boolean read FCompressWeekEnd
      write SetCompressWeekEnd default True;
    property DaysLayout: TcxSchedulerWeekViewDaysLayout read FDaysLayout
      write SetDaysLayout default wdlTwoColumns;
    property HideWeekEnd: Boolean read GetHideWeekEnd write SetHideWeekEnd
      default False;
  ...
  protected
    ...
    function GetWeekStart(AWeekIndex: Integer; ConsiderShowWeekEnd: Boolean)
      : TDateTime;
  ...
  public
    function ScrollResourcesEx(AGoForward: Boolean;
      var AResource: TcxSchedulerStorageResourceItem): Boolean; override;
    property ViewInfo: TcxSchedulerWeekViewViewInfo read GetViewInfo;
  end;
  ...
  TcxSchedulerWeekViewEventCellCustomViewInfo = class
  ...
  TcxSchedulerWeekViewEventCellViewInfo = class
  ...
  TcxSchedulerWeekViewEventCellModernViewInfo = class
    (TcxSchedulerWeekViewEventCellCustomViewInfo)
  private
  ...
  public
    constructor Create(AViewData: TcxSchedulerEventViewData;
      ...
      AUseRightToLeftAlignment: Boolean); override;
  end;
  ...
  protected
    ...
    function AddDayCellEventPlace(ABuilder: TcxSchedulerEventLayoutBuilder;
      AEvent: TcxSchedulerControlEvent; var AIndex: Integer;
      AStartDay: TcxSchedulerContentCellViewInfo): Boolean;
    procedure AddMoreEventsButton(AColumnIndex: Integer;
      const AContainerBounds: TRect; const ADateTime: TDateTime;
      AIsTop: Boolean; AEvent: TcxSchedulerEvent);
    function AddWeekDayHeader(AWeekDay: Integer; ALeft, ARight: Integer)
    ...
    procedure CalculateDaysLayoutForResource(const ABounds: TRect;
      AResourceIndex: Integer); virtual;
    procedure CalculateEventsOrder;
    procedure CalculateEventsViewInfo
      (ABuilder: TcxSchedulerEventLayoutBuilder); virtual;
    procedure CalculateGroupByDateForResource(const ABounds: TRect;
      AResourceIndex: Integer); virtual;
    ...
    procedure CalculateWeekDisplayRectsHorz(const AStartDate: TDateTime;
      AResourceID: Integer;
      { const } ABounds: TRect; AIsTopWeek, AIsLastWeek: Boolean); virtual;
    procedure CalculateWeekDisplayRectsVert(const AStartDate: TDateTime;
      AResourceID: Integer; const ABounds: TRect; AddBorders: TcxBorders = []
      ); virtual;
    procedure CalculateWeekDisplayRectsVertEx(const AStartDate: TDateTime;
      AResourceID: Integer; const ABounds: TRect; AddBorders: TcxBorders = []);
    function CanEventVisible(AEvent: TcxSchedulerControlEvent): Boolean;
    function CanVisibleNextLine(ADay: TcxSchedulerContentCellViewInfo;
      ALine: Integer = -1): Boolean;
    procedure CheckDaysTimeEvent(AEventInfo: TcxSchedulerEventCellViewInfo;
    ...
    procedure GetEventInfo(APlace: TcxSchedulerEventPlace;
    ...
    function GetDayCellContentHeight(ADay: TcxSchedulerContentCellViewInfo)
      : Integer; virtual;
    ...
    function IsClassicIntradayEventCell(AEvent: TcxSchedulerControlEvent)
      : Boolean; inline;
    function IsSingleLine(ADay1,
      ADay2: TcxSchedulerContentCellViewInfo): Boolean;
    function IsTransparentEventCell(AEvent: TcxSchedulerControlEvent)
      : Boolean; inline;
    ...
    property DayCells[ADay: Integer]: TcxSchedulerContentCellViewInfo
      read GetDayCell;
  ...
  TcxSchedulerWeeksViewEventCellViewInfo = class
    (TcxSchedulerWeekViewEventCellViewInfo)
  protected
  ...
  TcxSchedulerWeeksViewEventCellModernViewInfo = class
    (TcxSchedulerWeekViewEventCellModernViewInfo)
  protected
    function CanAutoHideStandardImages: Boolean; override;
  end;
  ...
  protected
    ...
    procedure CalculateDaysLayoutForResource(const ABounds: TRect;
      AResourceIndex: Integer); override;
    procedure CalculateGroupByDateForResource(const ABounds: TRect;
      AResourceIndex: Integer); override;
...
const
  WeekViewDayBorders: TcxSchedulerDaysBorders = ([bTop, bRight], [bTop, bRight],
    [bTop, bRight], [bTop], [bTop], [bTop], [bTop]);
...
constructor TcxSchedulerWeekViewEventPlaceInfo.Create
...
function TcxSchedulerCustomWeekViewController.CreateDragEventHelper
  : TcxDragEventHelper;
begin
  Result := TcxCustomWeekViewDragEventHelper.Create(Scheduler);
end;

function TcxSchedulerCustomWeekViewController.CreateNavigation
  : TcxSchedulerViewNavigation;
begin
  Result := TcxSchedulerCustomWeekView(View).CreateNavigation;
end;

function TcxSchedulerCustomWeekViewController.CreateResizeEventHelper
...
begin
  ...
  Result := Scheduler.OptionsView.ShowHints and
    (((HitTest.ContentCell <> nil) and
    (TcxSchedulerContentCellViewInfoAccess(HitTest.ContentCell).FDisplayText <>
    '') and (HitTest.HitAtTime) and (PtInRect(R, P))) or
    (HitTest.HitAtDayHeader and HitTest.HitAtTime));
end;

function TcxSchedulerCustomWeekViewController.GetNavigation
  : TcxSchedulerCustomWeekViewNavigation;
begin
  Result := TcxSchedulerCustomWeekViewNavigation(inherited Navigation);
end;

function TcxSchedulerCustomWeekViewController.GetView
  : TcxSchedulerCustomWeekView;
begin
...
function TcxSchedulerCustomWeekView.CheckDate(const ADate: TDateTime): Integer;
begin
...
function TcxSchedulerCustomWeekView.CreateController
  : TcxSchedulerSubControlController;
begin
  Result := TcxSchedulerCustomWeekViewController.Create(Self);
end;

function TcxSchedulerCustomWeekView.CreateNavigation
  : TcxSchedulerViewNavigation;
begin
  Result := TcxSchedulerCustomWeekViewNavigation.Create(Self);
end;

function TcxSchedulerCustomWeekView.CreatePainter
...
const
  ScrollKind: array [TScrollBarKind] of TScrollBarKind = (sbVertical,
    sbHorizontal);
begin
  Result := ScrollKind[TcxSchedulerWeekViewViewInfo(ViewInfo)
...
begin
  Result := cxDateToStr(GetFirstVisibleDate) + ' - ' +
    cxDateToStr(GetLastVisibleDate);
end;
...
begin
  ...
  AEvent.AllDayEvent := AllDay or (AFinish - AStart > 1) or
    (AInplaceEditing and not ARecurrence);
...
begin
  ...
  if (AScrollBarKind <> GetDateScrollBarKind) or (AScrollCode = scEndScroll)
  then
    Exit;
    ...
    scPageUp, scPageDown:
      begin
        ...
        with Controller.Navigation do
          ANewSelDate := ANewSelDate + DayInc[AScrollCode <> scPageUp] *
            Weeks.Count * 7;
      end;
    scTop:
...
function TcxSchedulerCustomWeekView.GetController
  : TcxSchedulerCustomWeekViewController;
begin
...
begin
  Result := (FHideWeekEnd and not FForceShowWeekEnd) or
    ViewInfo.Adapter.DontPrintWeekEnd;
end;

procedure TcxSchedulerCustomWeekView.GetRangeControlScales
...
function TcxSchedulerCustomWeekView.GetScrollPos(const ADatePos: TDateTime;
  ACheckBounds: Boolean = True): Integer;
begin
...
procedure TcxSchedulerCustomWeekView.SetCompressWeekEnd(const AValue: Boolean);
begin
...
procedure TcxSchedulerCustomWeekViewNavigation.KeyDown(var AKey: Word;
  AShift: TShiftState);
const
...
var
  APos: Integer;
begin
  DecodeDateWeek(FCurrentAnchor + 1 - Byte(View.StartOfWeek), FYear,
    FWeekOfYear, FDayOfWeek);
  Dec(FDayOfWeek);
  FWeekIndex := Max(0, Weeks.IndexOf(Trunc(EncodeDateWeek(FYear,
    FWeekOfYear, 1))));
  ...
  if (ssShift in FShift) and ((FCurrentResource <> SelResource) or
    (dxDateOf(FCurrentAnchor) < FirstVisibleDate) or
    (dxDateOf(FCurrentAnchor) > LastVisibleDate)) then
    Exit
  else if dxDateOf(FCurrentAnchor) < FirstVisibleDate then
  begin
    View.UnlockEventLayoutCalculating;
    View.Scroll(DateScrollBarKind, scLineUp, APos);
    SetSelAnchor(FCurrentAnchor, [], FCurrentResource);
  end
  else if dxDateOf(FCurrentAnchor) > LastVisibleDate then
  begin
    View.UnlockEventLayoutCalculating;
...
procedure TcxSchedulerCustomWeekViewNavigation.ValidateSelection(var ASelStart,
...
begin
  ...
  if View.SelectedDays.Count = 0 then
    ASelFinish := Date
  else if (dxDateOf(ASelFinish) < FirstVisibleDate) or
    (dxDateOf(ASelStart) > LastVisibleDate) then
  begin
...
function TcxSchedulerCustomWeekViewNavigation.GetWeekFinish(AWeekIndex: Integer)
  : TDateTime;
var
  I, K: Integer;
begin
  Result := 0;
  if (AWeekIndex < 0) or (AWeekIndex >= Weeks.Count) then
    Exit;
...
begin
  if View.HideWeekEnd and (ViewInfo.GroupByDate or
    (View.DaysLayout <> wdlTwoColumns) or
...
begin
  ...
  begin
    ...
    else if FWeekIndex = Weeks.Count then
    begin
...
function TcxSchedulerCustomWeekViewNavigation.IsResourceNavigation
  (AGotoDown: Boolean): Boolean;
begin
  Result := (ViewInfo.GroupingKind <> gkNone);
  if Result then
    Result := ScrollResourcesEx(AGotoDown, FCurrentResource);
  if Result then
    FCurrentAnchor := FCurrentAnchor - DayInc[AGotoDown] *
      (View.WeekCount - 1) * 7;
end;
...
function TcxSchedulerCustomWeekViewNavigation.GetView
  : TcxSchedulerCustomWeekView;
begin
  Result := TcxSchedulerCustomWeekView(inherited View);
end;
...
begin
  Result := (View is TcxSchedulerWeekView) and not ViewInfo.GroupByDate and
    (View.DaysLayout = wdlTwoColumns);
end;
...
procedure TcxSchedulerWeekView.GetRangeControlTotalRange(out AMin,
  AMax: TDateTime);
var
...
const
  DateInc: array [Boolean, 0 .. 6] of Integer = ((-4, -4, -4, -3, -3, -3, -4),
    (3, 3, 3, 4, 4, 4, 3));
begin
  ...
  begin
    ...
    else if IsLastWeek and AGotoNext then
    begin
      if not ScrollResourcesEx(AGotoNext, FCurrentResource) then
        FCurrentAnchor := FCurrentAnchor + 7;
    end
  ...
  else if GroupingKind <> gkByDate then
  begin
    ...
    begin
      if (IsLastWeek and AGotoNext and (FDayOfWeek in [3 .. 6])) or
        (IsFirstWeek and not AGotoNext and (FDayOfWeek in [0 .. 2])) then
      begin
...
function TcxSchedulerWeekViewNavigation.GetViewInfo
  : TcxSchedulerWeekViewViewInfo;
begin
  Result := TcxSchedulerWeekViewViewInfo(inherited ViewInfo);
end;
...
procedure TcxSchedulerWeeksView.GetRangeControlTotalRange(out AMin,
  AMax: TDateTime);
var
...
begin
  ...
  begin
    ...
    if (AGotoNext and IsLastDayOfWeek(FDayOfWeek) and not IsLastResource) or
      (not AGotoNext and IsFirstDayOfWeek(FDayOfWeek) and not IsFirstResource)
    then
    begin
...
begin
  ...
  begin
    if not AGotoDown and (FWeekIndex <= 0) then
      FCurrentAnchor := FCurrentAnchor - 7
    else if AGotoDown and (FWeekIndex >= Weeks.Count - 1) then
      FCurrentAnchor := FCurrentAnchor + 7
    else
      inherited GotoNextCellVert(AGotoDown);
  end
...
function TcxSchedulerWeeksViewNavigation.IsScrollResource
  (AGotoNext: Boolean): Boolean;
begin
...
function TcxSchedulerWeeksViewNavigation.GetViewInfo
  : TcxSchedulerWeeksViewViewInfo;
begin
  Result := TcxSchedulerWeeksViewViewInfo(inherited ViewInfo);
end;

function TcxSchedulerWeeksViewNavigation.IsFirstDayOfWeek
  (ADayIndex: Integer): Boolean;
var
  ADIndex: Integer;
begin
  if not View.HideWeekEnd then
...
function TcxSchedulerWeeksViewNavigation.IsLastDayOfWeek
  (ADayIndex: Integer): Boolean;
var
  ADIndex: Integer;
begin
...
procedure TcxSchedulerWeekViewEventCellCustomViewInfo.
  CalculateEventTimeVisibility;
begin
  CalculateNonDetailEventTimeVisibility;
  if not IsHeaderEvent then
    ViewData.ShowStartTime := True
  else if ViewData.ShowTimeAsClock and not Event.AllDayEvent then
  begin
    ViewData.ShowStartTime := ViewData.ShowStartTime and
      (dxTimeOf(Event.Start) <> 0);
    ViewData.ShowFinishTime := ViewData.ShowFinishTime and
      (dxTimeOf(Event.Finish) <> 0);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if not cxRectIsEmpty(FStartRect) then
...
procedure TcxSchedulerWeekViewEventCellCustomViewInfo.InitHitTest
  (AHitTest: TcxSchedulerCustomResourceViewHitTest);
begin
  inherited InitHitTest(AHitTest);
  if IsHeaderEvent then
    InitHitTestForHorzEvent(AHitTest)
  else if (ShowStartTime and (AHitTest.HitX <= FStartRect.Right)) or
...
begin
  if (dxDateOf(EventStart) <> dxDateOf(EventFinish)) and (Event.Duration < 1)
  then
  begin
...
begin
  Result := CXTO_LEFT or CXTO_CENTER_VERTICALLY or CXTO_WORDBREAK or
    CXTO_EDITCONTROL;
end;

{ TcxSchedulerWeekViewEventCellModernViewInfo }

constructor TcxSchedulerWeekViewEventCellModernViewInfo.Create
  (AViewData: TcxSchedulerEventViewData; ACanResize: Boolean;
  AScaleFactor: TdxScaleFactor; AUseRightToLeftAlignment: Boolean);
begin
  inherited Create(AViewData, ACanResize, AScaleFactor,
...
procedure TcxSchedulerWeekViewEventCellModernViewInfo.
...
function TcxSchedulerWeekViewEventCellModernViewInfo.
...
begin
  Result := AHorizontalAlignment[UseRightToLeftAlignment] or
    CXTO_CENTER_VERTICALLY or CXTO_WORDBREAK or CXTO_EDITCONTROL or
    CXTO_EXCELSTYLE or CXTO_SINGLELINE or CXTO_END_ELLIPSIS;
end;

function TcxSchedulerWeekViewEventCellModernViewInfo.GetCaptionFontStyle
  : TFontStyles;
begin
...
begin
  ...
  Result.Left := Max(Result.Left, TimeLineRect.Right +
...
function TcxSchedulerWeekViewEventCellModernViewInfo.GetViewStyle
...
begin
  Result := (ContentFinish - 1 = ViewData.ViewContentFinish) and
    (EventFinish > ContentFinish) and
    (EventFinish > ViewData.ViewContentFinish);
end;
...
procedure TcxSchedulerWeekViewViewInfo.ScrollVertical(AScrollCode: TScrollCode;
...
begin
  ...
  Result.DontPrint := PrintExactlyOneMonth and
    (MonthOf(AStart) <> PrintedMonth);
  ...
  TcxSchedulerContentCellViewInfoAccess(Result).FFirstEventTop := ARect.Top +
    GetDayCellHeaderHeight;
end;
...
begin
  AOffset := TcxSchedulerContentCellViewInfoAccess(DayCells[APlace.ColStart])
    .FFirstEventTop;
  AEvent := TcxSchedulerControlEvent(APlace.Event);
  ABounds.Top := 0;
  ABounds.Bottom := FContentLineHeight;
  GetEventInfo(APlace, ABounds.Left, ABounds.Right, AStart, AFinish,
    AVisibleRect, AResource);
  OffsetRect(ABounds, 0, AOffset + APlace.LineStart *
    (FContentLineHeight + AIndent[ViewStyle = svsModern]));
  if ABounds.Bottom <= AVisibleRect.Bottom then
  begin
    AViewData := CreateEventViewData(AEvent, ABounds, AStart, AFinish,
      AResource);
    AViewData.VisibleRect := AVisibleRect;
    AViewData.ViewContentStart := TcxSchedulerContentCellViewInfoAccess
      (DayCells[0]).DateTime;
    AViewData.ViewContentFinish := TcxSchedulerContentCellViewInfoAccess
      (DayCells[DayCellCount - 1]).DateTime;
    ACell := AddEventCell(AViewData);
    ACell.Transparent := IsTransparentEventCell(AEvent);
    if IsClassicIntradayEventCell(AEvent) then
      ACell.Borders := [];
    TcxSchedulerEventCellViewInfoAccess(ACell).FContainerInfo :=
      TcxSchedulerWeekViewEventPlaceInfo.Create(APlace);
    ...
    begin
      ...
      begin
        AddMoreEventsButton(I, DayCells[I].Bounds, DayCells[I].TimeStart,
          True, AEvent);
...
begin
  ...
  begin
    while (AIndex < (DayCellCount - 1)) and IsSingleLine(DayCells[AIndex],
      AStartDay) do
      Inc(AIndex);
    Exit;
  end;
...
procedure TcxSchedulerWeekViewViewInfo.AddMoreEventsButton
  ...
  begin
    for I := 0 to FButtons.Count - 1 do
      ...
      begin
        ...
        if (AColumnIndex = Result.ColumnIndex) and (ADateTime = Result.DateTime)
        then
          Exit;
      end;
    Result := nil;
...
function TcxSchedulerWeekViewViewInfo.AddWeekDayHeader(AWeekDay: Integer;
  ALeft, ARight: Integer): TcxSchedulerWeekDayHeaderCellViewInfo;
begin
  CreateCellInstance(WeekDayHeaderClass, cxRect(ALeft, FDayHeaderOffsetTop,
    ARight, FDayHeaderOffsetTop + FDayHeaderHeight),
    StylesAdapter.GetDayHeaderParams(View.FirstVisibleDate + AWeekDay),
...
function TcxSchedulerWeekViewViewInfo.WeekDayHeaderClass
    ...
    begin
      ...
      begin
        ...
        if AEvent = nil then
          Continue;
        AList.Add(AEvent);
        while (I < Events.AbsoluteCount) and
          ((Events.AbsoluteItems[I] = nil) or
          AEvent.IsDayEvent(Integer(Trunc(Events.AbsoluteItems[I].Start)))) do
          Inc(I);
      end;
    end;
  finally
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not AddDayCellEventPlace(ABuilder, AEvent, J, DayCells[J]) and
          (ResourceCount = 0) then
          Break;
      end
      else
        Inc(J);
...
procedure TcxSchedulerWeekViewViewInfo.CalculateGroupByDateForResource
  (const ABounds: TRect; AResourceIndex: Integer);
begin
  CalculateWeekDisplayRectsHorz(StartDates[0], AResourceIndex, ABounds,
    True, True);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AHeader := AddWeekDayHeader(AWeekDay, ACell.Bounds.Left,
        ACell.Bounds.Right);
  ...
  begin
    ...
    begin
      ...
      if (ViewStyle = svsClassic) or TcxSchedulerContentCellViewInfoAccess
        (ACell).IsDrawBySkin then
      begin
        OffsetRect(R, 0, 1);
        ...
        if (View.DaysLayout <> wdlOneColumn) and
          (not HasSeparator or ((I mod 7) <= 2)) then
          Dec(R.Right);
      end
      else if (View.DaysLayout <> wdlOneColumn) and (R.Right < Bounds.Right)
      then
        Dec(R.Right);
      with AddDayHeader(ACell.TimeStart, R, AIndex,
        ADisplayMode[ViewStyle = svsModern]) do
      begin
...
begin
  ...
  begin
    J := I + FRowColsStart;
    ARects[J] := cxRect(ABounds.Left + MulDiv(W, I, FRowCols), ABounds.Top - 1,
      ABounds.Left + MulDiv(W, I + 1, FRowCols), ABounds.Bottom);
  ...
  begin
    ...
    begin
      Inc(K);
      ACellBorders := ABorders[(K >= (FRowCols - 1)) and
        (HasSeparator or (AResourceID = (ResourceCount - 1)))];
      if AIsTopWeek and (K <> FRowCols) then
        Exclude(ACellBorders, bTop);
      if AIsLastWeek and not HasSeparator and (K <> FRowCols - 1) and
        (AResourceID <> ResourceCount - 1) then
        Include(ACellBorders, bBottom);
      J := K + FRowColsStart;
      AddContentCell(ARects[J], AStartDate + I, AStartDate + I + 1, AResourceID)
        .Borders := ACellBorders;
    end;
  end;
end;

procedure TcxSchedulerWeekViewViewInfo.CalculateWeekDisplayRectsVert
  (const AStartDate: TDateTime; AResourceID: Integer; const ABounds: TRect;
  AddBorders: TcxBorders = []);
var
  R: TRect;
...
begin
  ...
  begin
    CalculateWeekDisplayRectsVertEx(AStartDate, AResourceID, ABounds,
      AddBorders);
    Exit;
  end;
  ...
  begin
    J := RowColsStart + I;
    AddContentCell(ARects[J], AStartDate + J, AStartDate + J + 1, AResourceID)
      .Borders := WeekViewDayBorders[J] + AddBorders;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Inc(K);
      J := RowColsStart + I;
      AddContentCell(ARects[RowColsStart + K], AStartDate + J,
        AStartDate + J + 1, AResourceID).Borders := [bTop] + AddBorders;
    end;
  end;
end;
...
begin
  ...
  Result := (ALine <> MaxInt) and
    ((GetDayCellContentHeight(ADay)) >= (ALine + 1) * FContentLineHeight);
end;

procedure TcxSchedulerWeekViewViewInfo.CheckDaysTimeEvent
  (AEventInfo: TcxSchedulerEventCellViewInfo; AColStart, AColFinish: Integer);
var
  I: Integer;
  ADay: TcxSchedulerContentCellViewInfo;
begin
  if (AEventInfo.Event.State <> 0) and (AEventInfo.Event.IsAllDayOrLonger) then
    ...
    begin
      ...
      if IsCellEvent(AEventInfo.Event, ADay) and
        ((ADay.TimeEvent = nil) or (ADay.TimeEvent.Event.State <
        AEventInfo.Event.State)) then
...
var
  I: Integer;
const
  ANeighbors: array [Boolean] of TcxNeighbors = ([], [nTop]);
  ADayHeader: array [Boolean] of TcxNeighbors = ([], [nLeft]);
  ABorders: array [Boolean] of TcxBorders = ([], [bBottom]);
begin
...
function TcxSchedulerWeekViewViewInfo.ContentCellClass
...
procedure TcxSchedulerWeekViewViewInfo.CustomProcessPreviouslyCalculatedEvent
...
function TcxSchedulerWeekViewViewInfo.EventCellClass
...
begin
  Result := View.CompressWeekEnd or
    (not GroupByDate and (View.DaysLayout = wdlTwoColumns));
end;

procedure TcxSchedulerWeekViewViewInfo.GetEventInfo
  (APlace: TcxSchedulerEventPlace; var ALeft, ARight: Integer;
  var AStart, AFinish: TDateTime; var AVisibleRect: TRect;
  var AResource: TcxSchedulerResourceViewInfo);
var
  AColStart, AColFinish: Integer;
begin
  ...
  begin
    ...
    if (SchedulerStyle = svsModern) and (APlace.Event.Finish <= AFinish) then
      ARight := Bounds.Right - 2 * TcxSchedulerPainterHelper.ShadowSize
        (ScaleFactor)
    else
      ARight := Bounds.Right - cxTextOffset - 1;
...
begin
  ...
  begin
    ...
    if GroupByDate and ResourceHeaders.RotateCaptions then
      Result := ScaleFactor.Apply(cxSize(ResourceImages.Height,
        ResourceImages.Width));
  end;
end;
...
function TcxSchedulerWeekViewViewInfo.GetContentParams(const ATime: TDateTime;
...
begin
  if AResource = nil then
    ...
    Result := StylesAdapter.GetContentParams(ATime, True,
...
begin
  Result := cxRectHeight(PainterHelper.ExcludeBorders(ADay.Bounds, ADay.Borders)
    ) - GetDayCellHeaderHeight - cxTextOffset;
end;
...
function TcxSchedulerWeekViewViewInfo.IsCellEvent
  ...
  ACell: TcxSchedulerContentCellViewInfo): Boolean;
begin
  Result := (ACell <> nil) and AEvent.IsDayEvent
    (Integer(Trunc(ACell.TimeStart)));
  if Result and (GroupingKind <> gkNone) then
    Result := AEvent.IsResourceEvent(ACell.Resource.ResourceItem,
      ShowEventsWithoutResource);
  if Result and PrintExactlyOneMonth then
    Result := MonthOf(ACell.TimeStart) = PrintedMonth;
end;

function TcxSchedulerWeekViewViewInfo.IsClassicIntradayEventCell
  (AEvent: TcxSchedulerControlEvent): Boolean;
begin
  Result := (ViewStyle = svsClassic) and not AEvent.IsAllDayOrLonger;
end;

function TcxSchedulerWeekViewViewInfo.IsSingleLine(ADay1,
  ADay2: TcxSchedulerContentCellViewInfo): Boolean;
begin
...
function TcxSchedulerWeekViewViewInfo.IsTransparentEventCell
  (AEvent: TcxSchedulerControlEvent): Boolean;
begin
  Result := IsClassicIntradayEventCell(AEvent) and
    not(AEvent.Selected or Styles.IsEventStyleAssigned(AEvent));
end;
...
procedure TcxSchedulerWeekViewViewInfo.ProcessGroupByDate;
  ...
  function IsSameMonth(ACell1,
    ACell2: TcxSchedulerMonthDayContentCellViewInfo): Boolean;
  begin
...
begin
  ...
  H := cxRectHeight(ABounds) - DayHeaderHeight - SeparatorWidth *
    (ResourceCount - 1);
  AddResourceHeader(-1, cxRectSetSize(Bounds, ResourceHeaderWidth,
    DayHeaderHeight));
  for I := 0 to ResourceCount - 1 do
  begin
    ABounds.Top := Bounds.Top + DayHeaderHeight + MulDiv(H, I, ResourceCount) +
      SeparatorWidth * I + 1;
    ...
    Dec(ABounds.Top);
    ACell := AddResourceHeader(I, cxRectSetWidth(ABounds, Bounds.Left,
      ResourceHeaderWidth));
...
begin
  ...
  begin
    ...
    ACell := AddResourceHeader(I, cxRectSetTop(ABounds, Bounds.Top,
...
function TcxSchedulerWeekViewViewInfo.GetDayCell(ADay: Integer)
  : TcxSchedulerContentCellViewInfo;
begin
...
function TcxSchedulerWeeksViewEventCellViewInfo.
  CanAutoHideStandardImages: Boolean;
begin
  Result := True;
end;
...
function TcxSchedulerWeeksViewEventCellModernViewInfo.
  CanAutoHideStandardImages: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  begin
    R := cxRect(ABounds.Left, ATop + MulDiv(H, I, Weeks.Count), ABounds.Right,
      ATop + MulDiv(H, I + 1, Weeks.Count));
    if (I > 0) or not GroupByDate then
      Inc(R.Top);
    CalculateWeekDisplayRectsHorz(StartDates[I], AResourceIndex, R, I = 0,
      I = Weeks.Count - 1);
  end;
  APrevMonth := -1;
  ...
  begin
    ...
    ACell.SetContentState(APrevMonth = MonthOf(ACell.TimeStart),
      IsTimeSelected(ACell.TimeStart, ACell.Resource), FContentFontHeight,
      FSelectionParams);
...
begin
  ...
  begin
    ...
    if IsWorkDay(AWeekDay, Scheduler) or not HideWeekEnd then
    begin
      if ((J + FRowColsStartDateOffset) < 6) or HideWeekEnd or not CompressWeekEnd
      then
        AddWeekDayHeader(AWeekDay, MulDiv(W, J, FRowCols) + FHeaderOffsetLeft,
...
begin
  ...
  ProcessCheckBorders(ResourceHeaderCells, False);
  if not GroupByDate or not(bLeft in LookAndFeelPainter.HeaderBorders([nLeft]))
  then
  begin
...
function TcxSchedulerWeeksViewViewInfo.ContentCellClass
  : TcxSchedulerContentCellViewInfoClass;
const
...
function TcxSchedulerWeeksViewViewInfo.DayHeaderClass
  : TcxSchedulerDayHeaderCellViewInfoClass;
begin
  Result := WeekDayHeaderClass;
end;

function TcxSchedulerWeeksViewViewInfo.EventCellClass
  : TcxSchedulerEventCellViewInfoClass;
const
  AClass: array [Boolean] of TcxSchedulerEventCellViewInfoClass =
    (TcxSchedulerWeeksViewEventCellViewInfo,
...
function TcxSchedulerWeeksViewViewInfo.GetContentParams(const ATime: TDateTime;
  AResource: TcxSchedulerResourceViewInfo): TcxViewParams;
begin
  if AResource = nil then
    Result := StylesAdapter.GetContentParams(ATime,
      not Odd(MonthOf(ATime)), nil)
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      if IsWorkDay(AWeekDay, Scheduler) or not HideWeekEnd then
      begin
        if ((K + FRowColsStartDateOffset) < 6) or HideWeekEnd or not CompressWeekEnd
        then
          AddWeekDayHeader(AWeekDay, MulDiv(W, K, FRowCols) + ACell.Bounds.Left,
            MulDiv(W, K + 1, FRowCols) + ACell.Bounds.Left).Resource :=
            ACell.Resource;
        Inc(K);
      end;
      Inc(I);
