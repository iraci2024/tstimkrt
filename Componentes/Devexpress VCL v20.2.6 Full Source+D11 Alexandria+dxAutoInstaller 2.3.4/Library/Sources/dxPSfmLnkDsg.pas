...
uses
  ...
  Buttons, ImgList, DesignIntf, DesignWindows, dxCore, cxGraphics, cxControls,
  cxContainer, cxLookAndFeels,
  cxLookAndFeelPainters, cxButtons, cxEdit, cxGroupBox, cxListBox, dxPSCore,
  dxPSfmLnkAdd, dxPSDsgProxies, dxPSGlbl,
  dxLayoutContainer, dxLayoutControlAdapters, dxLayoutLookAndFeels, cxClasses,
  dxLayoutControl, cxImageList, cxCustomListBox;
...
type
  ...
  private
    ...
    procedure InternalAddLink(ALinkClass: TdxReportLinkClass;
      AComponent: TComponent; const AName, ACaption, ACreator,
      ADescription: string; AShowDesigner: Boolean);
  ...
  public
    ...
    property Controller: TCustomdxComponentPrinter read FController
      write SetController;
    property ControllerDesigner: TAbstractdxReportLinkDesigner
      read GetControllerDesigner;
...
uses
  DesignConst, ComponentDesigner, Registry, TypInfo, CommCtrl, ColnEdit,
  dxPSRes, dxPrnPg, dxBkgnd, dxPSPrVwDsg,
  dxPSPopupMan, dxPSfmLnkAddE, dxPSUtl, dxPSContainerLnk, dxPSCompsProvider,
  dxPSEngn, dxPSImgs, dxCoreGraphics;
...
begin
  S := AComponentPrinter.Name + AComponentPrinter.GetNewLinkName
    (TBasedxReportLink(AComponent));
  ...
  begin
    ...
    begin
      for J := 0 to AComponentPrinter.Owner.ComponentCount - 1 do
        if CompareText(AComponentPrinter.Owner.Components[J].Name, Result) = 0
        then
        begin
          NameExists := True;
          Break;
...
procedure TdxPSDesignTimedxComponentsProvider.GetComponents(AComponentPrinter
...
begin
  ...
  try
    ...
    ActiveDesigner.GetComponentNames(GetTypeData(PTypeInfo(TComponent.ClassInfo)
      ), EnumComponentsProc);
  finally
...
procedure TdxPSDesignTimedxComponentsProvider.AppendComponent
  (AComponent: TComponent; const APathName: string);
var
  ComponentItem: TdxComponentItem;
begin
  if (not(gcoExcludeOutOfActiveForm in FOptions) or (Pos('.', APathName) = 0))
    and (((FReportLink = nil) and dxPSIsSupportedCompClass(AComponent)) or
    ((FReportLink <> nil) and FReportLink.Supports(AComponent) and
    (FReportLink.Component <> AComponent))) then
  begin
    ComponentItem := TdxComponentItem.Create(AComponent, APathName,
      AComponent.ClassName);
    FComponents.AddObject(ComponentItem.Caption, ComponentItem);
  end;
end;

procedure TdxPSDesignTimedxComponentsProvider.EnumComponentsProc
  (const S: string);
var
...
procedure TdxPSDesignTimedxComponentsProvider.EnumReportLinksProc
  (const S: string);
var
  Component: TComponent;
begin
  ...
  begin
    Component := ActiveDesigner.GetComponent(S);
    if (Component is TBasedxReportLink) and
      (TBasedxReportLink(Component).Component <> nil) then
...
begin
  if ActiveDesigner <> nil then
    ActiveDesigner.GetComponentNames(GetTypeData(TComponent.ClassInfo),
      EnumReportLinksProc);
end;

function TdxPSDesignTimedxComponentsProvider.IsComponentAccessible
  (AComponent: TComponent): Boolean;
var
  Inaccessible: Boolean;
begin
  Inaccessible := ((AComponent = nil) or (AComponent is TBasedxReportLink) or
    (AComponent is TCustomdxComponentPrinter)) or
    ((FExistingComponents <> nil) and (FExistingComponents.IndexOf(AComponent)
    <> -1)) or ((gcoHideCustomContainers in FOptions) and
    dxPSContainerLnk.dxPSIsComponentContainer(AComponent));
  Result := not Inaccessible;
end;

{ TdxReportLinkDesigner }

constructor TdxReportLinkDesigner.Create(AComponentPrinter
  : TCustomdxComponentPrinter; AFormDesigner: TFormDesigner);
begin
...
begin
  ...
  AData.Initialize;
  try
    AData.Options := [Low(TdxAddReportLinkDlgDataOption)
      .. High(TdxAddReportLinkDlgDataOption)];
    ...
    if dxShowAddComponentsDlg(AData) then
      ...
      begin
        ...
        if (AData.ReportLinkName = '') or (AData.Components.Count <> 1) then
          AData.ReportLinkName := UniqueName(nil);
        InternalAddLink(TdxReportLinkClass(AData.ComponentsLinkClasses[I]),
          AComponent, AData.ReportLinkName, AData.ReportLinkCaption,
          AData.ReportLinkCreator, AData.ReportLinkDescription,
          AData.DesignBtnPressed);
      end;
  finally
    AData.Finalize;
  end;
...
begin
  ...
  InternalAddLink(TdxCompositionReportLink, nil, UniqueName(nil), '',
    dxGetUserName, cxGetResourceString(@sdxComposition), False);
end;
...
begin
  ...
  begin
    if Data.ReportLinkName = '' then
      Data.ReportLinkName := UniqueName(nil);
    InternalAddLink(Data.ReportLinkClass, nil, Data.ReportLinkName,
...
begin
  ...
  if CurrentLink is TdxCompositionReportLink then
    ShowCollectionEditor(Designer, CurrentLink,
      TdxCompositionReportLink(CurrentLink).Items, 'Items')
  else if Controller.DesignReport(CurrentLink) then
    Designer.Modified;
end;
...
begin
  for I := 0 to LinkCount - 1 do
    if (Selected[I] or (SelectedCount = 0)) and Links[I].DataProviderPresent
    then
...
begin
  ...
  else if CanPrintPreview then
    PrintPreviewClick(Sender)
  else if CanPageSetup then
...
procedure TdxfmReportLinkDesignWindow.SelectionChanged(const ADesigner
...
function TdxfmReportLinkDesignWindow.GetControllerDesigner
...
begin
  ...
  else if Link.DataProviderPresent then
    Result := sdxLinked
  else
...
begin
  Result := dxPSDsgProxies.GetBaseRegistryKey + '\' +
    sdxPSRegPathRunTimeFormLayouts + '\' + DropT(ClassName);
end;
...
procedure TdxfmReportLinkDesignWindow.SetController
...
procedure TdxfmReportLinkDesignWindow.SetSelected(Index: Integer;
  Value: Boolean);
begin
...
begin
  Result := (SelectedCount = 1) and
    not(CurrentLink is TdxCompositionReportLink);
end;
...
begin
  Result := (SelectedCount = 1) and (CurrentLink <> nil) and
    (rlcPageSetup in CurrentLink.Capabilities);
end;

function TdxfmReportLinkDesignWindow.CanPrint: Boolean;
begin
  Result := (SelectedCount = 1) and (CurrentLink <> nil) and
    CurrentLink.DataProviderPresent;
end;
    ...
    try
      ...
      try
        ...
        begin
          ...
          if not(csAncestor in Item.ComponentState) and
            (Item is TBasedxReportLink) then
            Item.Free;
        end;
        if ItemIndex < 0 then
          ItemIndex := 0;
...
begin
  Result.X := 300;
  Result.Y := btnPrint.Top + btnPrint.Height + GetSystemMetrics(SM_CYCAPTION) +
    2 * GetSystemMetrics(SM_CXFRAME) + 4;
end;

procedure TdxfmReportLinkDesignWindow.GetSelections(const ASelections
...
procedure TdxfmReportLinkDesignWindow.InternalAddLink
  (ALinkClass: TdxReportLinkClass; AComponent: TComponent;
  const AName, ACaption, ACreator, ADescription: string;
...
procedure TdxfmReportLinkDesignWindow.PrepareAddExistingItem
  (AMenuItem: TMenuItem);
begin
...
procedure TdxfmReportLinkDesignWindow.Select(AItem: TPersistent;
...
begin
  if (Controller <> nil) and (Controller.Owner <> nil) then
    NewCaption := Format(sdxLinkDesigner, [Controller.Owner.Name, '.',
      Controller.Name]);
...
procedure TdxfmReportLinkDesignWindow.UpdateSelections(const ASelections
  : TdxDesignSelectionList);
...
var
  I: Integer;
begin
  if (ASelections = nil) or (Controller = nil) or
    (csDestroying in Controller.ComponentState) or (ControllerDesigner = nil) or
    (TdxReportLinkDesigner(ControllerDesigner).FUpdateCount <> 0) then
    Exit;
  for I := 0 to LinkCount - 1 do
...
procedure TdxfmReportLinkDesignWindow.WMGetMinMaxInfo
...
procedure TdxfmReportLinkDesignWindow.lbxLinksDrawItem(AControl: TcxListBox;
...
procedure TdxfmReportLinkDesignWindow.lbxLinksMeasureItem(AControl: TcxListBox;
  AIndex: Integer; var Height: Integer);
begin
