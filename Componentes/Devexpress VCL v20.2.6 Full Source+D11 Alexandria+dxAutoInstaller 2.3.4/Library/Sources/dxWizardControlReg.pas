...
type
  ...
  TdxWizardControlPageComponentEditor = class
    (TdxWizardControlCustomComponentEditor)
  protected
    function GetWizardControl: TdxCustomWizardControl; override;
  end;
  ...
  TdxWizardControlCustomButtonCollectionItemImageIndexProperty = class
    (TImageIndexProperty)
  public
    function GetImages: TCustomImageList; override;
  end;
...
type
  TdxWizardControlCustomButtonCollectionAccess = class
...
procedure TdxWizardControlCustomComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  case AIndex of
    0:
...
function TdxWizardControlCustomComponentEditor.InternalGetVerb
...
begin
  case AIndex of
    ...
    2:
      Result := sdxPrevPage;
    3:
      Result := sdxDeletePage;
    4:
      Result := '-';
    5:
      Result := sdxPageCollectionEditor;
  else
...
procedure TdxWizardControlCustomComponentEditor.PrepareItem(Index: Integer;
  const AItem: IMenuItem);
begin
  inherited PrepareItem(Index, AItem);
  case Index of
    1:
      AItem.Enabled := WizardControl.CanGoToNextPage;
    2:
      AItem.Enabled := WizardControl.CanGoToPrevPage;
    3:
      AItem.Enabled := (WizardControl.ActivePage <> nil) and
...
function TdxWizardControlComponentEditor.GetWizardControl
...
function TdxWizardControlPageComponentEditor.GetWizardControl
  : TdxCustomWizardControl;
begin
...
procedure TdxWizardControlPageDesignWindow.DoRemoveFreeNotification
  (AList: TList);
var
  I: Integer;
begin
...
begin
  for I := 0 to AList.Count - 1 do
  begin
    if Supports(TObject(AList[I]), IdxWizardControlSelectableItem,
      ASelectableItem) then
      ASelectableItem.SelectionChanged;
  end;
end;

procedure TdxWizardControlPageDesignWindow.Notification(AComponent: TComponent;
...
procedure TdxWizardControlPageDesignWindow.ItemDeleted(const ADesigner
  : IDesigner; Item: TPersistent);
begin
  FCurrentSelection.Remove(Item);
end;

procedure TdxWizardControlPageDesignWindow.SelectionsChanged(const ASelection
  : IDesignerSelections);
var
...
begin
  Result := TdxWizardControlButtons(TdxWizardControlCustomButton(GetComponent(0)
    ).Buttons).Images;
end;

{ TdxWizardControlCustomButtonCollectionItemImageIndexProperty }

function TdxWizardControlCustomButtonCollectionItemImageIndexProperty.GetImages
  : TCustomImageList;
begin
  Result := TdxWizardControlCustomButtonsAccess
    (TdxWizardControlCustomButtonCollectionAccess
    (TdxWizardControlCustomButtonCollectionItem(GetComponent(0)).Collection)
    .CustomButtons).OwnerButtons.Images;
end;
...
begin
  ...
  RegisterComponentEditor(TdxWizardControlPage,
    TdxWizardControlPageComponentEditor);
  ...
  RegisterPropertyEditor(TypeInfo(TcxImageIndex),
    TdxWizardControlCustomButtonCollectionItem, 'ImageIndex',
    TdxWizardControlCustomButtonCollectionItemImageIndexProperty);
...
begin
  ...
  TdxWizardControlPageAccess(Sender).FDesignHelper :=
