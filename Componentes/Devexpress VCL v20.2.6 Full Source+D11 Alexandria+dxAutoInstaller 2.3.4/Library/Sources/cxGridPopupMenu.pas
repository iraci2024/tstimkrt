...
uses
  Windows, Messages, Types, SysUtils, Classes, Controls, cxGrid,
  cxGridCustomView,
...
type
  ...
  protected
    function DoOnPopup(ASenderMenu: TComponent; AHitTest: TcxCustomGridHitTest;
    ...
    function GetPopupMenuInfo(const AMenu: TComponent;
      const AView: TcxCustomGridView; SearchDefault: Boolean = True)
      : TcxPopupMenuInfo; override;
    property GridOperationHelper: TcxGridOperationHelper
      read FGridOperationHelper;
  public
    ...
    function FindPopupMenuInfo(const AView: TcxCustomGridView;
      ...
      : TcxPopupMenuInfo; override;
    function GetBuiltInPopupMenuByClass(const AMenuClass: TcxPopupMenuClass)
      : TComponent;

    procedure RegisterPopupMenu(const AMenu: TComponent;
      AHitTypes: TcxGridViewHitTypes; ADoPopupProc: TcxGridOnPopupProc;
      const AView: TcxCustomGridView); overload; virtual;
    procedure RegisterPopupMenu(const AMenu: TComponent;
      AHitTypes: TcxGridViewHitTypes; ADoPopupProc: TcxGridOnPopupProc;
      AViewClass: TcxCustomGridViewClass); overload; virtual;
    ...
    property BuiltInPopupMenus: TcxGridDefaultPopupMenu
      read FGridDefaultPopupMenu;
  ...
  published
    ...
    property PopupMenus: TcxPopupMenuInfos read GetPopupMenus
      write SetPopupMenus;
    property UseBuiltInPopupMenus: Boolean read FUseBuiltInPopupMenus
      write SetUseBuiltInPopupMenus default True;
    property OnMenuItemClick: TcxGridPopupMenuItemClickProc read FOnClick
      write FOnClick;
    property OnPopup: TcxGridBeforePopupProc read FOnPopup write FOnPopup;
    property AlwaysFireOnPopup: Boolean read FAlwaysFireOnPopup
      write FAlwaysFireOnPopup default False;
  end;
...
uses
  Forms, cxGridLevel, Dialogs, cxGridStdPopupMenu, Menus, cxGridCustomTableView,
  cxControls;
...
function TcxGridContextPopupNotification.NotificationKinds
  : TcxGridNotificationKinds;
begin
...
function TcxGridPopupMenu.GetBuiltInPopupMenuByClass(const AMenuClass
  : TcxPopupMenuClass): TComponent;
var
  I: Integer;
begin
...
procedure TcxGridPopupMenu.Notification(AComponent: TComponent;
...
begin
  if Assigned(AView) then
    AdjustMenuInfo(GetPopupMenuInfo(AMenu, AView), AMenu, AHitTypes,
      ADoPopupProc, nil, AView);
end;
...
begin
  FGridDefaultPopupMenu.RegisterPopupMenu(AMenu, AHitTypes, ADoPopupProc,
    AViewClass);
end;
...
function TcxGridPopupMenu.DoOnPopup(ASenderMenu: TComponent;
  AHitTest: TcxCustomGridHitTest; X, Y: Integer): Boolean;
begin
  Result := True;
...
function TcxGridPopupMenu.DoPopup(AAPPSKeyPressed: Boolean): Boolean;
  ...
  function DoAutoPopup(AMenuInfo: TcxPopupMenuInfo;
    AHitTest: TcxCustomGridHitTest; APoint: TPoint): Boolean;
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (FocusedItem <> nil) and (FocusedItem.FocusedCellViewInfo <> nil)
        then
        begin
        ...
        HitTest := TcxCustomGridCellViewInfoAccess(AViewInfo)
          .GetHitTestClass.Instance(APoint);
      ...
      begin
        ...
        TcxCustomGridViewInfoAccess(FHitGridView.Site.ViewInfo)
          .InitHitTest(FHitTest);
      end;
...
begin
  ...
  begin
    ...
    AHitCode := FHitTest.HitTestCode;
    if (FHitGridView = nil) or not PtInRect(FHitGridView.Site.ClientRect,
...
begin
  ...
  if Result and DoOnPopup(APopupMenu, AHitTest, APoint.X, APoint.Y) then
    TcxGridOnPopupProc(AIDoPopupIntf.GetPopupHandler)(APopupMenu, AHitTest,
      APoint.X, APoint.Y);
end;
