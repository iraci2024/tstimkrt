...
type
  ...
  protected
    ...
    procedure CreateHyperlink(AHyperlinkInfo: TdxHyperlinkInfo;
      ASource: TdxTextToDisplaySource; ARunInfo: TdxRunInfo;
      const AText: string);
    procedure ChangeHyperlink(AHyperlinkInfo: TdxHyperlinkInfo;
      ASource: TdxTextToDisplaySource; ARunInfo: TdxRunInfo;
      const AText: string);
  ...
  protected
    ...
    function SplitTable(const AStart, AEnd: TdxDocumentLogPosition)
      : Boolean; virtual;
  public
    constructor Create(const AControl: IdxRichEditControl;
      AHyperlinkInfo: TdxHyperlinkInfo; ALogPosition: TdxDocumentLogPosition;
      ALength: Integer); reintroduce; overload;
  ...
  protected
    ...
    property CreateHyperlinkCommand: TdxCreateHyperlinkCommand
      read GetCreateHyperlinkCommand;
  public
    class function GetDescription: string; override;
    class function GetMenuCaption: string; override;

    property HyperlinkInfo: TdxHyperlinkInfo read GetHyperlinkInfo
      write SetHyperlinkInfo;
    property Text: string read GetText write SetText;
  end;

  { TdxHyperlinkFieldCommandBase }

  TdxHyperlinkFieldCommandBase = class abstract
...
function TdxShowHyperlinkFormCommand.GetExecutionMode
  : TdxMultiCommandExecutionMode;
begin
  Result := TdxMultiCommandExecutionMode.ExecuteAllAvailable;
end;

function TdxShowHyperlinkFormCommand.GetUpdateUIStateMode
...
class function TdxCreateHyperlinkContextMenuItemCommand.Id
...
function TdxSelectTextForHyperlinkCommand.
...
procedure TdxSelectTextForHyperlinkCommand.ChangeSelectionStart
...
begin
  if DocumentModel.Selection.Length > 0 then
    Exit(False);
  AField := ActivePieceTable.FindFieldByRunIndex
    (DocumentModel.Selection.Interval.Start.RunIndex);
  if (AField <> nil) and ActivePieceTable.IsHyperlinkField(AField) then
    Exit(False);
  Result := inherited PerformChangeSelection;
end;

function TdxSelectTextForHyperlinkCommand.CanChangePosition
  (const APos: TdxDocumentModelPosition): Boolean;
begin
  Result := True;
end;

function TdxSelectTextForHyperlinkCommand.ChangePosition
...
function TdxShowHyperlinkFormCoreCommand.GetTreatStartPositionAsCurrent
  : Boolean;
begin
  Result := False;
end;
...
function TdxShowHyperlinkFormCoreCommand.
  GetUpdateCaretPositionBeforeChangeSelectionDetailsLevel
  : TdxDocumentLayoutDetailsLevel;
begin
  Result := TdxDocumentLayoutDetailsLevel.None;
...
begin
  ...
  try
    RichEditControl.ShowHyperlinkForm(AHyperlinkInfo, ARunInfo, ATitle,
      CreateHyperlink);
  finally
    AHyperlinkInfo.Free;
    ARunInfo.Free;
...
var
  ...
  APos: TdxDocumentModelPosition;
begin
  AHyperlinkInfo := ActivePieceTable.HyperlinkInfos
    [FHyperlinkField.Index].Clone;
  try
...
function TdxShowHyperlinkFormCoreCommand.GetSelectedHyperlinkFields
  : TdxFieldList;
var
...
procedure TdxShowHyperlinkFormCoreCommand.CreateHyperlink(AHyperlinkInfo
...
begin
  ...
  begin
    ...
    ACreateCommand := TdxCreateHyperlinkCommand.Create(RichEditControl,
      AHyperlinkInfo, AStart, ALength);
    try
...
procedure TdxShowHyperlinkFormCoreCommand.ChangeHyperlink(AHyperlinkInfo
  : TdxHyperlinkInfo; ASource: TdxTextToDisplaySource; ARunInfo: TdxRunInfo;
  const AText: string);
begin
...
procedure TdxShowHyperlinkFormCoreCommand.UpdateUIStateCore
...
begin
  ...
  try
    ACanShowHyperlinkForm := (AHyperlinks.Count = 0) or
      ((AHyperlinks.Count = 1) and (not AHyperlinks[0].HideByParent) or
      (AHyperlinks[0].Parent = nil));
    AState.Enabled :=
      ((AState.Enabled and not DocumentModel.Selection.IsMultiSelection) and
      IsTableCorrectlySelected) and ACanShowHyperlinkForm;
  finally
    AHyperlinks.Free;
  end;
end;

procedure TdxShowHyperlinkFormCoreCommand.ApplyCommandRestriction
  (const AState: IdxCommandUIState);
begin
  ApplyCommandRestrictionOnEditableControl(AState,
    Options.DocumentCapabilities.Hyperlinks);
...
begin
  ASelection := DocumentModel.Selection;
  AStartCell := ActivePieceTable.FindParagraph
    (ASelection.NormalizedStart).GetCell;
  AEndCell := ActivePieceTable.FindParagraph
  ...
  if AEndCell = nil then
    ...
    AIsNotSelectedCellMark := ActivePieceTable.Paragraphs
      [AEndCell.EndParagraphIndex].EndLogPosition + 1 <>
      ASelection.NormalizedEnd;
  ...
  if (((AStartCell <> nil) and (AEndCell <> nil)) and
    (AStartCell.Table.NestedLevel = AEndCell.Table.NestedLevel + 1)) and AIsNotSelectedCellMark
  then
    Exit(True);
...
function TdxShowHyperlinkFormCoreCommand.CanChangePosition
  (const APos: TdxDocumentModelPosition): Boolean;
begin
  Result := True;
end;

function TdxShowHyperlinkFormCoreCommand.GetFieldEndPosition(AField: TdxField)
  : TdxDocumentLogPosition;
begin
  Result := TdxDocumentModelPosition.FromRunEnd(ActivePieceTable,
    AField.Result.&End).LogPosition;
end;

function TdxShowHyperlinkFormCoreCommand.ChangePosition
...
constructor TdxCreateHyperlinkCommand.Create(const AControl: IdxRichEditControl;
  AHyperlinkInfo: TdxHyperlinkInfo; ALogPosition: TdxDocumentLogPosition;
  ALength: Integer);
begin
...
begin
  ...
  try
    ...
    if AIsSplit then
      Inc(FLength);
    ActivePieceTable.CreateHyperlink(LogPosition, Length, HyperlinkInfo,
      GetForceVisible);
...
begin
  ...
  ActivePieceTable.SplitTable(AStartCell.Table.Index,
    AStartCell.Row.IndexInTable, GetForceVisible);
  Result := True;
end;
...
procedure TdxInsertHyperlinkCommand.ForceExecuteCore(const AState
  : IdxCommandUIState);
begin
...
function TdxInsertHyperlinkCommand.GetCreateHyperlinkCommand
  : TdxCreateHyperlinkCommand;
begin
...
function TdxInsertHyperlinkCommand.GetExecutionMode
  : TdxMultiCommandExecutionMode;
begin
  Result := TdxMultiCommandExecutionMode.ExecuteAllAvailable;
end;
...
function TdxInsertHyperlinkCommand.GetUpdateUIStateMode
  : TdxMultiCommandUpdateUIStateMode;
begin
  Result := TdxMultiCommandUpdateUIStateMode.EnableIfAllAvailable;
end;
...
begin
  AHyperlinks := GetSelectedHyperlinkFields;
  try
    Result := (AHyperlinks.Count = 1) and
      ((not AHyperlinks[0].HideByParent) or (AHyperlinks[0].Parent = nil)) and
...
procedure TdxHyperlinkFieldCommandBase.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
...
procedure TdxModifyHyperlinkCommandBase.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  ApplyCommandRestrictionOnEditableControl(AState,
...
procedure TdxFollowHyperlinkCommand.NavigateToExternalUri(AHyperlinkInfo
  : TdxHyperlinkInfo);
var
  AUri: string;
begin
  ...
  if TdxHyperlinkUriHelper.IsRelativePath(AUri) then
    AUri := TdxHyperlinkUriHelper.ConvertRelativePathToAbsolute
...
procedure TdxFollowHyperlinkCommand.ChangeActivePieceTable
  (ABookmark: TdxBookmark);
var
  ...
  begin
    ...
    begin
      ACommand := TdxChangeActivePieceTableCommand.Create(RichEditControl,
        APieceTable, nil, APos.Page.PageIndex);
      try
        ACommand.Execute;
      finally
        ACommand.Free;
...
procedure TdxFollowHyperlinkCommand.MakeHeaderFooterActive
  (APieceTable: TdxPieceTable);
var
...
begin
  ...
  begin
    AHeaderCommand := TdxMakeNearestHeaderActiveCommand.Create(RichEditControl,
      ASectionHeader);
    try
  ...
  begin
    ...
    begin
      AFooterCommand := TdxMakeNearestFooterActiveCommand.Create
        (RichEditControl, ASectionFooter);
      try
...
function TdxFollowHyperlinkCommand.FindBookmarkByName(const AName: string)
  : TdxBookmark;
var
