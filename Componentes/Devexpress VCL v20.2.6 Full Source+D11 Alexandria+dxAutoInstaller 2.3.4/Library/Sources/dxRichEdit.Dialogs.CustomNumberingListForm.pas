...
uses
  ...
  Forms, Dialogs,
  dxCore, dxCoreClasses, dxRichEdit.Dialogs.CustomDialog, cxGraphics,
  cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, dxLayoutContainer, dxLayoutControl, Menus,
...
type
  ...
  public
    ...
    procedure ShowFontForm(const AControl: IdxRichEditControl;
...
procedure TdxRichEditCustomNumberingListForm.ApplyCharacterProperties
...
begin
  ...
  try
    TdxFontPropertiesModifierAccess(AModified).ApplyCharacterProperties
      (TdxCharacterProperties(AData));
  finally
    AModified.Free;
  end;
end;
...
begin
  ...
  btnFont.Caption := cxGetResourceString
...
begin
  Result := TdxBulletedListFormController.Create
    (AControllerParameters as TdxBaseNumberingListFormControllerParameters);
end;

procedure TdxRichEditCustomNumberingListForm.edtAlignedIndentPropertiesChange
  (Sender: TObject);
begin
  Controller.AssignControllerIndentValues(GetValueFromEditor(edtAligned),
    GetValueFromEditor(edtIndent));
end;

procedure TdxRichEditCustomNumberingListForm.FormCloseQuery(Sender: TObject;
...
function TdxRichEditCustomNumberingListForm.GetController
  : TdxBulletedListFormController;
begin
  Result := TdxBulletedListFormController(inherited Controller);
end;

function TdxRichEditCustomNumberingListForm.GetUnitConverter
...
begin
  InitializeMeasurementUnitEdit(edtAligned, ToMeasurementType(UnitType),
    ...
    InchesToUIUnit(-4.5), InchesToUIUnit(4.5)));
...
function TdxRichEditCustomNumberingListForm.IsValidEdit
...
procedure TdxRichEditCustomNumberingListForm.ShowFontForm(const AControl
  : IdxRichEditControl; ASourceProperties: TdxCharacterProperties);
var
  AEditedProperties: TdxMergedCharacterProperties;
begin
  AEditedProperties := TdxMergedCharacterProperties.Create
    (ASourceProperties.Info.Info, ASourceProperties.Info.Options);
  try
    AControl.ShowFontForm(AEditedProperties, ApplyCharacterProperties,
      ASourceProperties);
  finally
...
function TdxRichEditCustomNumberingListForm.UpdateFirstLineIndent
  (ANewValue: Integer): Integer;
begin
