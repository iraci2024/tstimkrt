...
{$ENDIF}
  ...
  cxDropDownEdit, cxCheckListBox, cxListBox,
  cxTextEdit, cxLookAndFeelPainters, cxControls, cxGraphics, cxImageComboBox,
  cxColorComboBox, dxCore, dxCoreClasses,
  cxGeometry, dxSpreadSheetGraphics, dxSpreadSheetUtils, dxSpreadSheetTypes,
  dxSpreadSheetNumberFormat,
  dxSpreadSheetClasses, dxSpreadSheetCoreStyles, dxSpreadSheetStyles,
...
type
  ...
  TdxSpreadSheetCellStyleEditDialogBorder = (fcdbLeft, fcdbTop, fcdbRight,
    fcdbBottom, fcdbInsideVertical, fcdbInsideHorizontal);
  TdxSpreadSheetCellStyleEditDialogBorders = set of
    TdxSpreadSheetCellStyleEditDialogBorder;
  ...
  strict private
    FBiDiMode: TBiDiMode;
    FBordersInfo: array [TdxSpreadSheetCellStyleEditDialogBorder]
      of TdxSpreadSheetCellStyleEditDialogBorderInfo;
    FBounds: TRect;
    FPreviewText: string;
    ...
    function GetBorderInfo(AIndex: TdxSpreadSheetCellStyleEditDialogBorder)
      : TdxSpreadSheetCellStyleEditDialogBorderInfo;
  ...
  protected
    ...
    procedure Changed;
    procedure DrawBorder(ACanvas: TcxCanvas;
  ...
  public
    ...
    function HitTest(const P: TPoint;
      out ABorder: TdxSpreadSheetCellStyleEditDialogBorder): Boolean;
    //
    property BiDiMode: TBiDiMode read FBiDiMode write SetBiDiMode;
    property BordersInfo[AIndex: TdxSpreadSheetCellStyleEditDialogBorder]
      : TdxSpreadSheetCellStyleEditDialogBorderInfo read GetBorderInfo;
    property Bounds: TRect read FBounds write SetBounds;
    property HasHorzInsideBorder: Boolean read GetHasHorzInsideBorder
      write SetHasHorzInsideBorder;
    property HasVertInsideBorder: Boolean read GetHasVertInsideBorder
      write SetHasVertInsideBorder;
  ...
  TdxSpreadSheetCellStyleEditDialogColorComboBoxCache = class
    (TDictionary<TcxColorComboBox, TColor>)
  public
  ...
  TdxSpreadSheetCellStyleEditDialogListBoxIndexCache = class
    (TDictionary<TcxListBox, Integer>)
  public
  ...
  strict private
  const
    FontStyles: array [0 .. 3] of TFontStyles = ([], [fsItalic], [fsBold],
      [fsBold, fsItalic]);
  strict private
  ...
  public
    constructor Create(AController
    ...
    function DisplayFormatCodeToFormatCode(AListBox: TcxListBox)
      : string; overload;
    function DisplayFormatCodeToFormatCode(const AFormatCode: string;
      AFormatCodeId: Integer = -1): string; overload;
    function FormatCodeToDisplayFormatCode(const ADataFormat
      : TdxSpreadSheetCellDataFormat): string; overload;
    function FormatCodeToDisplayFormatCode(const AHandle
      : TdxSpreadSheetFormatHandle): string; overload;

    function GetActualBorderType(ASide: TcxBorder; ARow, AColumn: Integer;
      const AArea: TRect): TdxSpreadSheetCellStyleEditDialogBorder;
    ...
    procedure PopulateFillPatterns(AImageList: TcxImageList;
      AComboBox: TcxImageComboBox);
    ...
    procedure PopulateNumberFormatTemplates(AListBox: TcxListBox;
      ACategory: TdxSpreadSheetNumberFormatCategory);
    procedure PopulateTextAlignHorz(AComboBox: TcxComboBox);
    procedure PopulateTextAlignVert(AComboBox: TcxComboBox);
    //
    property ColorComboCache
      : TdxSpreadSheetCellStyleEditDialogColorComboBoxCache
      read FColorComboCache;
    property ListBoxCache: TdxSpreadSheetCellStyleEditDialogListBoxIndexCache
      read FListBoxCache;
    property PredefinedFormats: TdxSpreadSheetPredefinedFormats
      read GetPredefinedFormats;
  end;
  ...
  public
    class function BuildCodeForCurrency(ADecimalPlaces: Integer;
      const ASymbol: string; ALocale: Integer): string;
    class function BuildCodeForNumber(ADecimalPlaces: Integer;
      AUseThousandSeparator: Boolean): string;
    ...
    class function IsCurrencyCode(ACode: string; out ADecimalPlaces: Integer;
      out ASymbol: string; out ALocale: Integer): Boolean;
    ...
    class function IsPercentageCode(const ACode: string;
      out ADecimalPlaces: Integer): Boolean;
    class function IsScientificCode(const ACode: string;
      out ADecimalPlaces: Integer): Boolean;
  end;
...
uses
  Math, SysUtils, dxHashUtils, cxClasses, dxSpreadSheetCoreStrs,
  dxSpreadSheetCoreDialogsStrs, dxTypeHelpers,
  cxDrawTextUtils, cxLookAndFeels, cxVariants, Variants, StrUtils, cxFormats,
  cxRegExpr;
...
constructor TdxSpreadSheetCellStyleEditDialogBorderInfo.Create
  (AOwner: TdxSpreadSheetCellStyleEditDialogBordersPreview);
begin
  inherited Create;
  FOwner := AOwner;
  FColor := clDefault;
...
procedure TdxSpreadSheetCellStyleEditDialogBorderInfo.SetColor
  (const AValue: TColor);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogBorderInfo.SetStyle
  (const AValue: TdxSpreadSheetCellBorderStyle);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogBorderInfo.SetUndefined
...
procedure TdxSpreadSheetCellStyleEditDialogBorderInfo.SetVisible
  (const AValue: Boolean);
begin
...
begin
  ...
  for AIndex := Low(AIndex) to High(AIndex) do
    FBordersInfo[AIndex] := TdxSpreadSheetCellStyleEditDialogBorderInfo.
      Create(Self);
end;
...
begin
  ...
  begin
    BordersInfo[fcdbLeft].Bounds :=
      cxRectSetWidth(R, BordersInfo[fcdbLeft].Size);
    BordersInfo[fcdbRight].Bounds := cxRectSetRight(R, R.Right,
      BordersInfo[fcdbRight].Size);
  end
  else
  begin
    BordersInfo[fcdbLeft].Bounds := cxRectSetRight(R, R.Right,
      BordersInfo[fcdbLeft].Size);
    BordersInfo[fcdbRight].Bounds :=
      cxRectSetWidth(R, BordersInfo[fcdbRight].Size);
  end;
  BordersInfo[fcdbBottom].Bounds := cxRectSetBottom(R, R.Bottom,
    BordersInfo[fcdbBottom].Size);
  BordersInfo[fcdbTop].Bounds := cxRectSetHeight(R, BordersInfo[fcdbTop].Size);
  BordersInfo[fcdbInsideHorizontal].Bounds :=
    cxRectCenterVertically(R, BordersInfo[fcdbInsideHorizontal].Size);
  BordersInfo[fcdbInsideVertical].Bounds := cxRectCenterHorizontally(R,
    BordersInfo[fcdbInsideVertical].Size);
  CalculateTextBounds(R);
end;

procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.Draw
  (ACanvas: TcxCanvas);
var
  AIndex: TdxSpreadSheetCellStyleEditDialogBorder;
  I: Integer;
begin
  ...
  ACanvas.FrameRect(Bounds,
    cxGetActualColor(Painter.GetContainerBorderColor(False), clWindowFrame));
  for AIndex := Low(AIndex) to High(AIndex) do
...
begin
  for AIndex := Low(AIndex) to High(AIndex) do
    if BordersInfo[AIndex].Visible and
      PtInRect(BordersInfo[AIndex].HitTestArea, P) then
    begin
      ABorder := AIndex;
      Exit(True);
    end;
...
procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.CalculateTextBounds
...
procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.DrawBorder
  (ACanvas: TcxCanvas; ABorder: TdxSpreadSheetCellStyleEditDialogBorder;
  AInfo: TdxSpreadSheetCellStyleEditDialogBorderInfo);
begin
  ...
  begin
    ...
    else if AInfo.Style <> sscbsDefault then
      dxSpreadSheetDrawBorder(ACanvas, AInfo.Bounds,
        cxGetActualColor(AInfo.Color, clBlack), BackgroundColor, AInfo.Style,
        ABorder in [fcdbTop, fcdbBottom, fcdbInsideHorizontal]);
  end;
end;

procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.DrawPreviewText
  (ACanvas: TcxCanvas; R: TRect);
begin
  if ACanvas.RectVisible(R) then
    cxTextOut(ACanvas.Handle, PreviewText, R, CXTO_CENTER_HORIZONTALLY or
      CXTO_CENTER_VERTICALLY, nil, 0, 0, 0, Painter.DefaultContentTextColor);
end;

function TdxSpreadSheetCellStyleEditDialogBordersPreview.
  GetBackgroundColor: TColor;
begin
  Result := cxGetActualColor(Painter.DefaultEditorBackgroundColor(False),
    clWindow);
end;
...
function TdxSpreadSheetCellStyleEditDialogBordersPreview.
  GetHasHorzInsideBorder: Boolean;
begin
  Result := BordersInfo[fcdbInsideHorizontal].Visible;
end;

function TdxSpreadSheetCellStyleEditDialogBordersPreview.
  GetHasVertInsideBorder: Boolean;
begin
  Result := BordersInfo[fcdbInsideVertical].Visible;
end;

function TdxSpreadSheetCellStyleEditDialogBordersPreview.GetPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := RootLookAndFeel.Painter;
end;

procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.SetBiDiMode
  (const Value: TBiDiMode);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.SetBounds
  (const AValue: TRect);
begin
...
procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.SetHasHorzInsideBorder
  (const AValue: Boolean);
begin
  BordersInfo[fcdbInsideHorizontal].Visible := AValue;
end;

procedure TdxSpreadSheetCellStyleEditDialogBordersPreview.SetHasVertInsideBorder
  (const AValue: Boolean);
begin
...
function TdxSpreadSheetCellStyleEditDialogColorComboBoxCache.GetValue
  (AColorComboBox: TcxColorComboBox): TColor;
begin
...
function TdxSpreadSheetCellStyleEditDialogListBoxIndexCache.GetValue
  (AListBox: TcxListBox): Integer;
begin
...
begin
  ...
  FColorComboCache :=
    TdxSpreadSheetCellStyleEditDialogColorComboBoxCache.Create;
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.DrawBorderStylePattern
  (AControl: TcxListBox; ACanvas: TcxCanvas; ABorderColor: TColor; R: TRect;
...
begin
  ...
  begin
    ...
    dxSpreadSheetDrawBorder(ACanvas, R, ABorderColor, ABackgroundColor,
      AStyle, True);
  end
  else
    cxTextOut(ACanvas.Handle, cxGetResourceString(@sdxFormatCellsDialogNone), R,
...
function TdxSpreadSheetCellStyleEditDialogHelper.DisplayFormatCodeToFormatCode
  (AListBox: TcxListBox): string;
begin
  if AListBox.ItemIndex >= 0 then
    Result := DisplayFormatCodeToFormatCode(AListBox.Items[AListBox.ItemIndex],
      Integer(AListBox.ItemObject))
  else
    Result := '';
end;
...
begin
  ...
  begin
    ...
    if (AHandle <> nil) and AHandle.IsDependedFromRegionalSettings and
      (Result[1] = '*') then
      Delete(Result, 1, 1);
  end;
end;
...
begin
  Result := FormatCodeToDisplayFormatCode
    (TdxSpreadSheetCellDataFormatAccess(ADataFormat).Handle);
end;
...
function TdxSpreadSheetCellStyleEditDialogHelper.GetActualBorderType
  (ASide: TcxBorder; ARow, AColumn: Integer; const AArea: TRect)
  : TdxSpreadSheetCellStyleEditDialogBorder;
begin
  case ASide of
    bTop:
...
function TdxSpreadSheetCellStyleEditDialogHelper.GetActualColor
  (AColorComboBox: TcxColorComboBox): TColor;
begin
...
function TdxSpreadSheetCellStyleEditDialogHelper.GetFontStyle
  (AListBoxIndex: Integer): TFontStyles;
begin
  Result := FontStyles[Max(0, AListBoxIndex)];
end;

function TdxSpreadSheetCellStyleEditDialogHelper.GetFontStyleName
  (AStyle: TFontStyles): string;
const
  NamesMap: array [0 .. 3] of Pointer = (@sdxFontStyleRegular,
    @sdxFontStyleItalic, @sdxFontStyleBold, @sdxFontStyleBoldItalic);
var
  I: Integer;
begin
  Result := '';
...
function TdxSpreadSheetCellStyleEditDialogHelper.GetListBoxValue
  (AListBox: TcxListBox): string;
begin
  if AListBox.ItemIndex >= 0 then
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.NavigateOnListbox(AKey: Word;
  AListBox: TcxListBox);
begin
  case AKey of
    VK_UP:
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.SyncSearchEdit
  (AListBox: TcxListBox; AEdit: TcxCustomTextEdit);
begin
  if AListBox.ItemIndex >= 0 then
    AEdit.EditValue := AListBox.Items[AListBox.ItemIndex];
end;

procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateFillPatterns
  (AImageList: TcxImageList; AComboBox: TcxImageComboBox);
var
  ABitmap: TcxBitmap;
  ...
  try
    ...
    try
      ...
      begin
        ...
        dxSpreadSheetDrawBackground(cxPaintCanvas, ABitmap.ClientRect, clWhite,
          clBlack, AFillStyle);
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateFontSizes
  (AListBox: TcxListBox);
var
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateFontStyles
  (AListBox: TcxListBox);
var
  ASavedItemIndex: Integer;
  I: Integer;
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateFontUnderlineStyles
...
begin
  ...
  try
    ...
    try
      AComboBox.Properties.Items.Clear;
      AComboBox.Properties.Items.Add
        (cxGetResourceString(@sdxFormatCellsDialogUnderlineNone));
      AComboBox.Properties.Items.Add
        (cxGetResourceString(@sdxFormatCellsDialogUnderlineSingle));
    finally
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateLocale
  (AComboBox: TcxComboBox);
var
  I: Integer;
begin
  ...
  try
    ...
    for I := 0 to Languages.Count - 1 do
      AComboBox.Properties.Items.AddObject(Languages.Name[I],
        TObject(Languages.LocaleID[I]));
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateNumberFormatCategories
  (AListBox: TcxListBox);
var
...
begin
  ...
  try
    ...
    try
      AListBox.Items.Clear;
      for AIndex := Low(AIndex) to High(AIndex) do
        AListBox.Items.AddObject
          (cxGetResourceString(dxSpreadSheetNumberFormatCategoryNames[AIndex]),
          TObject(AIndex));
    finally
      AListBox.ItemIndex := ASavedItemIndex;
    end;
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateNumberFormatTemplates
  ...
  begin
    ...
    if AHandle <> nil then
      AListBox.AddItem(FormatCodeToDisplayFormatCode(AHandle),
        TObject(AHandle.FormatCodeID));
  end;
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateTextAlignHorz
  (AComboBox: TcxComboBox);
var
  ASavedItemIndex: Integer;
    ...
    try
      ...
      for H := Low(H) to High(H) do
        AComboBox.Properties.Items.AddObject
          (cxGetResourceString(dxSpreadSheetDataHorzAlignNames[H]), TObject(H));
    finally
      AComboBox.ItemIndex := ASavedItemIndex;
    end;
  finally
...
procedure TdxSpreadSheetCellStyleEditDialogHelper.PopulateTextAlignVert
  (AComboBox: TcxComboBox);
var
  ASavedItemIndex: Integer;
  ...
  try
    ...
    try
      ...
      for V := Low(V) to High(V) do
        AComboBox.Properties.Items.AddObject
          (cxGetResourceString(dxSpreadSheetDataVertAlignNames[V]), TObject(V));
    finally
      AComboBox.ItemIndex := ASavedItemIndex;
    end;
  finally
...
function TdxSpreadSheetCellStyleEditDialogHelper.GetPredefinedFormats
...
class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  BuildCodeForCurrency(ADecimalPlaces: Integer; const ASymbol: string;
  ALocale: Integer): string;
begin
  Result := '[$' + ASymbol + '-' + IntToHex(ALocale, 0) + ']' +
    BuildCodeForNumber(ADecimalPlaces, True);
end;

class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  BuildCodeForNumber(ADecimalPlaces: Integer;
  AUseThousandSeparator: Boolean): string;
begin
  Result := IfThen(AUseThousandSeparator, '#,##') + '0' +
    IfThen(ADecimalPlaces > 0, '.') + DupeString('0', ADecimalPlaces);
end;

class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  BuildCodeForPercentage(ADecimalPlaces: Integer): string;
begin
  Result := BuildCodeForNumber(ADecimalPlaces, False) + '%';
end;

class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  BuildCodeForScientific(ADecimalPlaces: Integer): string;
begin
...
class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  IsCurrencyCode(ACode: string; out ADecimalPlaces: Integer;
  out ASymbol: string; out ALocale: Integer): Boolean;
var
  APos1: Integer;
  APos2: Integer;
...
begin
  ...
  begin
    ...
    begin
      ASymbol := '$';
      ALocale := dxMakeLCID(dxMakeLangID(LANG_ENGLISH, SUBLANG_ENGLISH_US),
        SORT_DEFAULT);
    ...
    begin
      ...
      begin
        ...
        ALocale := StrToIntDef('$' + Copy(ACode, APos1 + 1,
          APos2 - APos1 - 1), -1);
        Delete(ACode, 1, APos2);
        Result := (ALocale >= 0) and
          IsNumberCode(ACode, ADecimalPlaces, AUnused);
      end;
    end;
  end;
end;

class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.IsGeneralCode
...
class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.IsNumberCode
  (const ACode: string; out ADecimalPlaces: Integer;
  out AUseThousandSeparator: Boolean): Boolean;
var
  AIndex: Integer;
begin
...
class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  IsPercentageCode(const ACode: string; out ADecimalPlaces: Integer): Boolean;
var
  AIndex: Integer;
begin
...
class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.
  IsScientificCode(const ACode: string; out ADecimalPlaces: Integer): Boolean;
var
  AIndex: Integer;
begin
...
class function TdxSpreadSheetCellStyleEditDialogNumberFormatHelper.CheckCode
  (const ACode, ARegExpr: string): Boolean;
var
  AChar: Char;
  ARegExp: TcxRegExpr;
  I: Integer;
