...
type
  ...
  public
    function Clone(APieceTable: TdxCustomPieceTable)
      : IdxVisibleTextFilter; override;
  end;
...
function TdxEmptyTextFilter.Clone(APieceTable: TdxCustomPieceTable)
...
function TdxVisibleTextFilter.Clone(APieceTable: TdxCustomPieceTable)
  : IdxVisibleTextFilter;
begin
  if Self = nil then
    Exit(nil);
...
function TdxVisibleTextFilter.IsRunVisible(const ARunIndex
  : TdxRunIndex): Boolean;
begin
...
function TdxVisibleTextFilter.IsRunVisibleCore(const ARunIndex: TdxRunIndex)
  : TdxRunVisibility;
var
...
begin
  ...
  if Result <> TdxRunVisibility.Visible then
    Exit;
  AShouldDisplayHiddenText :=
    PieceTable.DocumentModel.FormattingMarkVisibilityOptions.HiddenText =
    TdxRichEditFormattingMarkVisibility.Visible;
...
function TdxVisibleOnlyTextFilter.Clone(APieceTable: TdxCustomPieceTable)
  : IdxVisibleTextFilter;
begin
  if Self = nil then
    Exit(nil);
...
function TdxVisibleOnlyTextFilter.IsRunVisible(const ARunIndex
  : TdxRunIndex): Boolean;
begin
  Result := (IsRunVisibleCore(ARunIndex) <> TdxRunVisibility.Hidden) or
    (ARunIndex = PieceTable.Runs.Count - 1);
...
function TdxVisibleOnlyTextFilter.IsRunVisibleCore(const ARunIndex: TdxRunIndex)
  : TdxRunVisibility;
begin
  Result := inherited IsRunVisibleCore(ARunIndex);
  if Result <> TdxRunVisibility.Visible then
