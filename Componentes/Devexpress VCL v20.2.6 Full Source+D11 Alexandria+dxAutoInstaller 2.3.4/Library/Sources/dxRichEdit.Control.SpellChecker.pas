...
type
  ...
  TdxSpellCheckerRichEditControlTextController = class
  ...
  TdxSpellCheckerRichEditControlAdapter = class(TInterfacedObject,
  ...
  protected
    function CreatePositionFromLogPosition(ALogPosition: Integer)
    ...
    procedure SetSelection(const AStart,
      AFinish: IdxSpellCheckerPosition); virtual;
    procedure UpdateController(AController
      : IdxSpellCheckTextController); virtual;
  public
...
function TdxSpellCheckerRichEditControlTextController.GetWordIterator
...
begin
  ...
  begin
    FreeAndNil(FWordIterator);
    FWordIterator := TdxSpellCheckerWordIterator.Create
...
function TdxSpellCheckerRichEditControlTextController.GetSentenceFinishPosition
...
function TdxSpellCheckerRichEditControlTextController.GetSentenceStartPosition
  (const APos: IdxSpellCheckerPosition): IdxSpellCheckerPosition;
begin
...
constructor TdxSpellCheckerRichEditControlAdapter.Create
  (AEdit: TdxCustomRichEditControl);
begin
  inherited Create;
  FEdit := AEdit;
  FController := TdxSpellCheckerRichEditControlTextController.Create
    (InnerControl);
end;
...
class function TdxSpellCheckerRichEditControlAdapter.CreateAdapter
  (AObject: TObject): IdxSpellCheckerAdapter;
begin
  Result := Create(AObject as TdxCustomRichEditControl);
end;

function TdxSpellCheckerRichEditControlAdapter.CreatePositionFromLogPosition
  (ALogPosition: Integer): IdxSpellCheckerPosition;
var
...
function TdxSpellCheckerRichEditControlAdapter.CreateController
...
procedure TdxSpellCheckerRichEditControlAdapter.GetSelection(out AStart,
  AFinish: IdxSpellCheckerPosition);
begin
  AFinish := CreatePositionFromLogPosition
    (Max(0, Min(PieceTable.DocumentEndLogPosition, Selection.&End)));
  AStart := CreatePositionFromLogPosition
    (Max(0, Min(PieceTable.DocumentEndLogPosition, Selection.Start)));
end;

procedure TdxSpellCheckerRichEditControlAdapter.GetSpellingBounds(out AStart,
  AFinish: IdxSpellCheckerPosition);
begin
  AFinish := TdxDocumentPosition.Create(TdxDocumentModelPosition.FromDocumentEnd
    (PieceTable));
  AStart := TdxDocumentPosition.Create
    (TdxDocumentModelPosition.Create(PieceTable));
end;
...
procedure TdxSpellCheckerRichEditControlAdapter.Replace(var AStart,
...
begin
  ...
  ASpellingStartLogPosition := TdxDocumentPosition(ASpellingStart)
    .Position.LogPosition;
  ASpellingFinishLogPosition := TdxDocumentPosition(ASpellingFinish)
    .Position.LogPosition;
  ADelta := Length(AWord) - (TdxDocumentPosition(AFinish).Position.LogPosition -
    AStartLogPosition);
  ...
  begin
    ...
    ASpellingFinish := CreatePositionFromLogPosition
      (ASpellingFinishLogPosition);
    AFinish := CreatePositionFromLogPosition(AStartLogPosition + Length(AWord));
    FController.Reset;
  end;
end;

procedure TdxSpellCheckerRichEditControlAdapter.SetHideSelection
  (AValue: Boolean);
begin
  // do nothing
end;

procedure TdxSpellCheckerRichEditControlAdapter.SetSelection(const AStart,
  AFinish: IdxSpellCheckerPosition);
var
...
procedure TdxSpellCheckerRichEditControlAdapter.UpdateController
  (AController: IdxSpellCheckTextController);
begin
  // do nothing
end;

function TdxSpellCheckerRichEditControlAdapter.GetDocumentModel
  : TdxDocumentModel;
begin
  Result := Edit.DocumentModel;
end;

function TdxSpellCheckerRichEditControlAdapter.GetInnerControl
...
TdxSpellCheckerAdapters.Register(TdxCustomRichEditControl,
  TdxSpellCheckerRichEditControlAdapter.CreateAdapter);
