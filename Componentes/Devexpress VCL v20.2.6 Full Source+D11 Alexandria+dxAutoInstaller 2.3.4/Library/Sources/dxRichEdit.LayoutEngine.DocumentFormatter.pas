...
type
  ...
  TdxDocumentFormatterStateType = (BeginParagraphFormatting,
    ...
    ContinueParagraphFormatting, EndParagraphFormatting,
    EndHiddenParagraphFormatting, ContinueFromParagraph, Final);
  ...
  private
    ...
    FOnResetSecondaryFormattingForPage
  ...
  public
    ...
    procedure RaisePageFormattingComplete(APage: TdxPage;
      ADocumentFormattingComplete: Boolean);
    procedure RaisePageCountChanged;
    procedure RaiseResetSecondaryFormattingForPage(APage: TdxPage;
      APageIndex: Integer);

    property PageCountChanged: TdxEventHandler read FOnPageCountChanged;
    property ResetSecondaryFormattingForPage
      : TdxResetSecondaryFormattingForPageEventHandler
      read FOnResetSecondaryFormattingForPage;

    procedure Reset(AKeepFloatingObjects: Boolean);
    function ResetFrom(var AFrom: TdxDocumentModelPosition;
      AKeepFloatingObjects: Boolean): TdxDocumentModelPosition;
    procedure ResetFromTheStartOfRowAtCurrentPage
    ...
    procedure RestartFormattingFromTheStartOfRowAtCurrentPage
      ...
      const AFrom: TdxDocumentModelPosition);
    function EnsurePositionVisible(var APos: TdxDocumentModelPosition)
      : TdxDocumentModelPosition;
    function EnsureParagraphVisible(var APos: TdxDocumentModelPosition)
      : TdxDocumentModelPosition;
    function EnsureRunVisible(var APos: TdxDocumentModelPosition)
      : TdxDocumentModelPosition;
    function PositionSectionBreakAfterParagraphBreak(AParagraph: TdxParagraph;
      ARunIndex: TdxRunIndex): Boolean;
    function CreateFormatterPosition(const AFrom: TdxDocumentModelPosition;
    ...
    function ClearInvalidatedContent(const AFrom: TdxDocumentModelPosition;
      ARoundToParagraphBoundary, AKeepFloatingObjects: Boolean)
    ...
    function CleanupEmptyBoxesForCurrentPage(const AFrom
      : TdxDocumentModelPosition): TdxDocumentModelPosition;
    function CleanupEmptyBoxes(const AFrom: TdxDocumentModelPosition)
      : TdxDocumentModelPosition;
    procedure SetPageCount(APageCount: Integer;
      APrimaryFormattingFinished: Boolean);
    ...
    property ColumnController: TdxColumnController read FColumnController
      write SetColumnController;
  ...
  protected
    ...
    procedure Init;
    function CreateParagraphBoxFormatter(AController
      : TdxDocumentFormattingController): TdxParagraphBoxFormatter; virtual;
  ...
  public
    ...
    property ParagraphFormatter: TdxParagraphBoxFormatter
      read FParagraphFormatter;
    property ParagraphIndex: TdxParagraphIndex read FParagraphIndex
      write FParagraphIndex;
...
type
  ...
  public
    ...
    property NextParagraphIndex: TdxParagraphIndex read FNextParagraphIndex
      write FNextParagraphIndex;
  end;
  ...
  public
    ...
    property BeginFromParagraphStart: Boolean read GetBeginFromParagraphStart
      write SetBeginFromParagraphStart;
  end;
  ...
  public
    ...
    property StartModelPosition: TdxDocumentModelPosition
      read FStartModelPosition write FStartModelPosition;
  end;
...
constructor TdxDocumentFormatter.Create(AController
  : TdxDocumentFormattingController);
begin
  inherited Create;
...
function TdxDocumentFormatter.CreateParagraphBoxFormatter
  (AController: TdxDocumentFormattingController): TdxParagraphBoxFormatter;
begin
  Result := TdxParagraphBoxFormatter.Create(PieceTable, Measurer,
    AController.RowsController);
end;

procedure TdxDocumentFormatter.CreateStates;
const
  StateClasses: array [TdxDocumentFormatterStateType]
    of TdxDocumentFormatterStateClass = (TdxBeginParagraphFormatting,
    TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow,
    TdxContinueParagraphFormatting, TdxEndParagraphFormatting,
    TdxEndHiddenParagraphFormatting, TdxContinueFromParagraph,
    TdxDocumentFormattingFinished);
var
  I: TdxDocumentFormatterStateType;
begin
  for I := Low(TdxDocumentFormatterStateType)
    to High(TdxDocumentFormatterStateType) do
...
procedure TdxDocumentFormatter.ChangeStateContinueFromParagraph(AParagraphIndex
  : TdxParagraphIndex);
begin
...
procedure TdxDocumentFormatter.ChangeState(AStateType
...
procedure TdxDocumentFormatter.ResetFromTheStartOfRowAtCurrentPage
  (const AFrom: TdxDocumentModelPosition;
  AKeepFloatingObjects, AForceRestart: Boolean);
var
  ANewState: TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow;
begin
  FController.ResetFromTheStartOfRowAtCurrentPage(AFrom, AKeepFloatingObjects,
    AForceRestart);
  Restart(AFrom);
  ChangeState(TdxDocumentFormatterStateType.
    BeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow);
  ANewState :=
    (TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow(State));
  ANewState.BeginFromParagraphStart :=
    (AFrom.LogPosition = PieceTable.Paragraphs[AFrom.ParagraphIndex]
    .LogPosition);
...
begin
  FIsHeaderFooter := Formatter.PieceTable <>
    Formatter.DocumentModel.MainPieceTable;
end;
...
procedure TdxBeginParagraphFormatting.SetBeginFromParagraphStart
  (Value: Boolean);
begin
end;
...
begin
  ...
  ACurrentParagraph := Formatter.PieceTable.Paragraphs
  ...
  begin
    ...
    Formatter.ChangeState
      (TdxDocumentFormatterStateType.EndHiddenParagraphFormatting);
  ...
  begin
    AParagraphFormatter.BeginParagraphFormatting(CreateIterator,
      BeginFromParagraphStart, True);
    AState := TdxStateParagraphFrame.Create(AParagraphFormatter,
      AParagraphFormatter.State);
    try
    ...
    begin
      ...
      AItems[AItems.Count - 1].LastParagraphIndex :=
        ARowsController.FrameParagraphIndex;
      ARowsController.FrameParagraphIndex := -1;
    end;

    if AStateResult = TdxStateContinueFormatResult.RestartDueFloatingObject then
    begin
      Formatter.ResetFromTheStartOfRowAtCurrentPage
        (ARowsController.RestartModelPosition, True, False);
      Exit(TdxFormattingProcess.Continue);
    end;

    if Formatter.ParagraphIndex < Formatter.PieceTable.Paragraphs.Last.Index
    then
  ...
  if AParagraphFramesLayout.ContainsParagraph(ACurrentParagraph) and
    (ARowsController.FrameParagraphIndex = -1) then
  begin
    if Formatter.ParagraphIndex < Formatter.PieceTable.Paragraphs.Last.Index
    then
      Formatter.ParagraphIndex := Formatter.ParagraphIndex + 1
      ...
      Formatter.ChangeState
        (TdxDocumentFormatterStateType.ContinueParagraphFormatting);
    Exit(TdxFormattingProcess.Continue);
  end;

  AIterator := CreateIterator;
  AParagraphFormatter.BeginParagraphFormatting(AIterator,
    BeginFromParagraphStart, False);
  Formatter.ChangeState
    (TdxDocumentFormatterStateType.ContinueParagraphFormatting);
  Result := TdxFormattingProcess.Continue;
end;
...
begin
  ...
  Result := TdxParagraphBoxIterator.Create
    (APieceTable.Paragraphs[Formatter.ParagraphIndex], APieceTable,
    AVisibleTextFilter);
end;
...
begin
  ...
  begin
    ...
    ACharacterIterator := TdxParagraphCharacterIterator.Create(AParagraph,
      APieceTable, AVisibleTextFilter);
    try
      if (ACharacterIterator.RunIndex <= AParagraph.LastRunIndex) then
      begin
        APreFormatter := TdxParagraphCharacterFormatter.Create(APieceTable,
          Formatter.Measurer);
        try
...
function TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow.
  GetBeginFromParagraphStart: Boolean;
begin
  Result := FBeginFromParagraphStart;
end;

procedure TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow.
  SetBeginFromParagraphStart(Value: Boolean);
begin
  FBeginFromParagraphStart := Value;
end;

function TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow.
  GetType: TdxDocumentFormatterStateType;
begin
  Result := TdxDocumentFormatterStateType.
    BeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow;
end;

function TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow.
  EnsureParagraphBoxes: Boolean;
begin
  Result := True;
end;

function TdxBeginParagraphFormattingFromTheMiddleOfParagraphAndStartOfRow.
  CreateIterator: TdxParagraphBoxIterator;
var
  ABoxIndex: Integer;
  AParagraph: TdxParagraph;
  APieceTable: TdxPieceTable;
...
begin
  ...
  if not AIterator.VisibleTextFilter.IsRunVisible(StartModelPosition.RunIndex)
  then
  begin
    ARunIndex := AIterator.VisibleTextFilter.GetNextVisibleRunIndex
      (StartModelPosition.RunIndex);
  ...
  ABoxIndex := AParagraph.BoxCollection.InnerCollection.BinarySearchBoxIndex
    (AFormatterPosition);
  Assert(ABoxIndex >= 0);
...
begin
  ...
  if AFormattingProcessResult.FormattingProcess = TdxFormattingProcess.Finish
  then
  begin
    Formatter.ChangeState(TdxDocumentFormatterStateType.EndParagraphFormatting);
    Result := TdxFormattingProcess.Continue;
  end
  else if AFormattingProcessResult.FormattingProcess = TdxFormattingProcess.ContinueFromParagraph
  then
  begin
    Formatter.ChangeStateContinueFromParagraph
      (AFormattingProcessResult.ParagraphIndex);
    Result := TdxFormattingProcess.Finish;
  end
  else if AFormattingProcessResult.FormattingProcess = TdxFormattingProcess.RestartFromTheStartOfRow
  then
  begin
    Formatter.ResetFromTheStartOfRowAtCurrentPage
      (AFormattingProcessResult.RestartPosition, True,
...
begin
  ...
  begin
    ...
    ALastPosition.RunIndex := AParagraphs[Formatter.ParagraphIndex]
      .LastRunIndex + 1;
...
var
  ...
  ACount: TdxParagraphIndex;
  AParagraphs: TdxParagraphCollection;
  ACurrentParagraphFrameProperties, ANextParagraphFrameProperties
...
begin
  ...
  while (AParagraphIndex < ACount) and not AVisibleTextFilter.IsRunVisible
    (AParagraphs[AParagraphIndex].LastRunIndex) do
    Inc(AParagraphIndex);
  while (AParagraphIndex + 1 < ACount) and IsParagraphInInvisibleCell
    (AParagraphs[AParagraphIndex + 1]) do
    Inc(AParagraphIndex);
  Formatter.ParagraphIndex := AParagraphIndex;

  ACurrentParagraphFrameProperties :=
    Formatter.ParagraphFormatter.GetActualParagraphFrameProperties
    (Formatter.PieceTable.Paragraphs[Formatter.ParagraphIndex]);
  try
    if Formatter.ParagraphIndex + 1 < ACount then
    begin
      ANextParagraphFrameProperties := Formatter.PieceTable.Paragraphs
        [Formatter.ParagraphIndex + 1].GetMergedFrameProperties;
      try
        if ACurrentParagraphFrameProperties <> nil then
        begin
          if (ANextParagraphFrameProperties <> nil) and
            ACurrentParagraphFrameProperties.CanMerge
            (ANextParagraphFrameProperties) then
          begin
            Formatter.ChangeState
              (TdxDocumentFormatterStateType.BeginParagraphFormatting);
            Exit(TdxFormattingProcess.Finish);
          ...
          begin
            if Formatter.PieceTable.Paragraphs[Formatter.ParagraphIndex].IsInCell
            then
            begin
              Formatter.ParagraphFormatter.RowsController.TablesController.
    ...
    if ((Formatter.ParagraphIndex + 1) >= ACount) or
      (ACurrentParagraphFrameProperties <> nil) then
      Formatter.ChangeState(TdxDocumentFormatterStateType.Final)
    else
      Formatter.ChangeState
        (TdxDocumentFormatterStateType.BeginParagraphFormatting);
...
function TdxEndParagraphFormatting.IsParagraphInInvisibleCell
...
constructor TdxDocumentFormattingController.Create(ADocumentLayout
  : TdxDocumentLayout; APieceTable: TdxPieceTable;
  AFloatingObjectsLayout: TdxFloatingObjectsLayout;
  AParagraphFramesLayout: TdxParagraphFramesLayout);
begin
  inherited Create;
  ...
  PageController.FloatingObjectsLayoutChanged.Add
    (OnFloatingObjectsLayoutChanged);
  PageController.ParagraphFramesLayoutChanged.Add
    (OnParagraphFramesLayoutChanged);
...
function TdxDocumentFormattingController.CleanupEmptyBoxes
...
function TdxDocumentFormattingController.ClearInvalidatedContent
  (const AFrom: TdxDocumentModelPosition; ARoundToParagraphBoundary,
  AKeepFloatingObjects: Boolean): TdxClearInvalidatedContentResult;
var
...
begin
  ...
  AClearContentResult := PageController.ClearInvalidatedContent(APos,
    AFrom.ParagraphIndex, AKeepFloatingObjects, AEmptyCurrentRow);
  ...
  RowsController.ClearInvalidatedContent
    (DocumentLayout.Pages.Last.Areas.Last.Columns.Last, APos,
    AFrom.ParagraphIndex);
  Result := AClearContentResult;
end;

function TdxDocumentFormattingController.
  ClearInvalidatedContentFromTheStartOfRowAtCurrentPage
  (const AFrom: TdxDocumentModelPosition; ARoundToParagraphBoundary,
  AKeepFloatingObjects: Boolean): TdxClearInvalidatedContentResult;
var
  APos: TdxFormatterPosition;
...
begin
  ...
  AEmptyCurrentRow := RowsController.CurrentRow.Boxes.Count = 0;
  AClearContentResult :=
    PageController.ClearInvalidatedContentFromTheStartOfRowAtCurrentPage(APos,
    AFrom.ParagraphIndex, AKeepFloatingObjects, RowsController.TablesController,
    AEmptyCurrentRow, ALeaveTable);
  if AClearContentResult = TdxClearInvalidatedContentResult.NoRestart then
    Exit(AClearContentResult);
  if AClearContentResult = TdxClearInvalidatedContentResult.ClearOnlyTableCellRows
  then
  begin
    RowsController.ClearInvalidatedContentFromTheStartOfRowAtCurrentPage
      (DocumentLayout.Pages.Last.Areas.Last.Columns.Last, APos,
      AFrom.ParagraphIndex);
  ...
  begin
    if ALeaveTable then
      RowsController.CurrentColumn :=
        RowsController.CurrentColumn.TopLevelColumn;
    PageAreaController.ClearInvalidatedContent(APos);
    ColumnController.ClearInvalidatedContent(APos);
    RowsController.ClearInvalidatedContent
      (DocumentLayout.Pages.Last.Areas.Last.Columns.Last, APos,
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ANextParagraph.IsEmpty and
          (APos.PieceTable.Runs[ANextParagraph.FirstRunIndex] is TdxSectionRun)
        then
        begin
          Dec(AParagraphIndex);
          Continue;
  ...
  while (AParagraphIndex > ACellStartParagraphIndex) and
    not PieceTable.VisibleTextFilter.IsRunVisible
    (AParagraphs[AParagraphIndex - 1].LastRunIndex) do
  begin
    if (AInitialCell <> nil) or
      not TdxParagraph(AParagraphs[AParagraphIndex - 1]).IsInCell then
      Dec(AParagraphIndex)
    else
      Break;
  end;
...
function TdxDocumentFormattingController.EnsureRunVisible
  (var APos: TdxDocumentModelPosition): TdxDocumentModelPosition;
var
  ARunIndex: TdxRunIndex;
...
procedure TdxDocumentFormattingController.BeginNextSectionFormatting
  (ASectionIndex: TdxSectionIndex);
begin
  PageController.BeginNextSectionFormatting(ASectionIndex);
  PageAreaController.BeginSectionFormatting(PageController.CurrentSection,
    ColumnController.TopLevelColumnsCount);
...
function TdxDocumentFormattingController.CreateFormatterPosition
  (const AFrom: TdxDocumentModelPosition; ARoundToParagraphBoundary: Boolean)
...
function TdxDocumentFormattingController.CreatePageAreaController
...
begin
  ...
  RowsController.OnPageFormattingComplete
    (FPageController.CurrentSection, APage);
...
procedure TdxDocumentFormattingController.OnParagraphFramesLayoutChanged
  (ASender: TObject; E: TdxEventArgs);
begin
  RowsController.OnParagraphFramesLayoutChanged;
end;

procedure TdxDocumentFormattingController.OnBeginNextSectionFormatting
  (ASender: TObject; E: TdxEventArgs);
begin
  Assert(E is TdxBeginNextSectionFormattingEventArgs);
  BeginNextSectionFormatting(TdxBeginNextSectionFormattingEventArgs(E)
    .SectionIndex);
end;

procedure TdxDocumentFormattingController.OnPageCountChanged(ASender: TObject;
  E: TdxEventArgs);
begin
  RaisePageCountChanged;
end;

procedure TdxDocumentFormattingController.OnPageFormattingComplete
...
begin
  ...
  RowsController.OnPageFormattingComplete
    (FPageController.CurrentSection, E.Page);
  E.Page.PrimaryFormattingComplete := True;
  AOldPageCount := DocumentModel.ExtendedDocumentProperties.Pages;
  ANewPageCount := Math.Max(AOldPageCount, PageController.PageCount +
    E.Page.NumSkippedPages);
  if ANewPageCount <> AOldPageCount then
    SetPageCount(ANewPageCount, False);
  RaisePageFormattingComplete(E.Page, False);
end;

procedure TdxDocumentFormattingController.OnPageFormattingStarted
  (ASender: TObject; E: TdxPageFormattingCompleteEventArgs);
begin
  RaisePageFormattingStarted(E.Page);
end;

procedure TdxDocumentFormattingController.OnParagraphFormattingComplete
  (AParagraphIndex: TdxParagraphIndex);
begin
end;

procedure TdxDocumentFormattingController.OnFloatingObjectsLayoutChanged
  (ASender: TObject; E: TdxEventArgs);
begin
  RowsController.OnFloatingObjectsLayoutChanged;
end;

function TdxDocumentFormattingController.PositionSectionBreakAfterParagraphBreak
  (AParagraph: TdxParagraph; ARunIndex: TdxRunIndex): Boolean;
begin
  Result := (ARunIndex > 0) and (AParagraph.Length = 1) and
    (AParagraph.PieceTable.Runs[ARunIndex] is TdxSectionRun);
end;
...
procedure TdxDocumentFormattingController.RaisePageFormattingComplete
  (APage: TdxPage; ADocumentFormattingComplete: Boolean);
var
  AArgs: TdxPageFormattingCompleteEventArgs;
begin
  if FOnPageFormattingComplete.Empty then
    Exit;
  AArgs := TdxPageFormattingCompleteEventArgs.Create(APage,
    ADocumentFormattingComplete);
  try
...
procedure TdxDocumentFormattingController.RaisePageFormattingStarted
  (APage: TdxPage);
var
  AArgs: TdxPageFormattingCompleteEventArgs;
begin
...
procedure TdxDocumentFormattingController.RaiseResetSecondaryFormattingForPage
  (APage: TdxPage; APageIndex: Integer);
var
...
function TdxDocumentFormattingController.ResetFrom
  (var AFrom: TdxDocumentModelPosition; AKeepFloatingObjects: Boolean)
  : TdxDocumentModelPosition;
var
  AClearContentResult: TdxClearInvalidatedContentResult;
...
begin
  ...
  AClearContentResult := ClearInvalidatedContent(AFrom, True,
  ...
  if AClearContentResult = TdxClearInvalidatedContentResult.RestartFromTheStartOfSection
  then
...
procedure TdxDocumentFormattingController.ResetFromTheStartOfRowAtCurrentPage
  (const AFrom: TdxDocumentModelPosition;
  AKeepFloatingObjects, AForceRestart: Boolean);
var
  ALastPage: TdxPage;
  ...
  AClearContentResult: TdxClearInvalidatedContentResult;
begin
  AClearContentResult := ClearInvalidatedContentFromTheStartOfRowAtCurrentPage
    (AFrom, False, AKeepFloatingObjects);
  if (AClearContentResult = TdxClearInvalidatedContentResult.NoRestart) and not AForceRestart
  then
    Exit;
  ...
  ALastPage.SecondaryFormattingComplete := False;
  ALastPage.CheckSpellingComplete := False;
  if AResult.LogPosition >= ALastPage.GetFirstPosition(PieceTable).LogPosition
  then
...
procedure TdxDocumentFormattingController.
  RestartFormattingFromTheMiddleOfSection;
var
  AIndex: Integer;
  ALastPage: TdxPage;
...
begin
  ...
  ASection := TdxDocumentModel(DocumentLayout.DocumentModel)
    .Sections[PageController.CurrentSectionIndex];
  PageController.RestartFormattingFromTheMiddleOfSection(ASection);
  PageAreaController.RestartFormattingFromTheMiddleOfSection
    (PageController.CurrentSection, ALastPage.Areas.Count - 1);
  AIndex := ALastPageArea.Columns.IndexOf(ALastColumn);
  ColumnController.RestartFormattingFromTheMiddleOfSection
    (PageController.CurrentSection, AIndex);
  if AIndex < ColumnController.ColumnsBounds.Count then
    ALastColumn.Bounds := ColumnController.ColumnsBounds[AIndex];
  RowsController.RestartFormattingFromTheMiddleOfSection
    (PageController.CurrentSection, ALastColumn);
end;

procedure TdxDocumentFormattingController.
  RestartFormattingFromTheStartOfRowAtCurrentPage(AClearContentResult
  : TdxClearInvalidatedContentResult; const AFrom: TdxDocumentModelPosition);
var
  ALastPage: TdxPage;
  ASection: TdxSection;
  ALastColumn: TdxColumn;
...
begin
  ...
  RowsController.RestartFormattingFromTheStartOfRowAtCurrentPage(ASection,
    ALastColumn);
end;
...
begin
  ...
  ALastColumn := ALastPageArea.Columns.Last;
  PageController.RestartFormattingFromTheStartOfSection;
  PageAreaController.RestartFormattingFromTheStartOfSection
    (PageController.CurrentSection, ALastPage.Areas.Count - 1);
  AIndex := ALastPageArea.Columns.IndexOf(ALastColumn);
  ColumnController.RestartFormattingFromTheStartOfSection
    (PageController.CurrentSection, AIndex);
  if AIndex < ColumnController.ColumnsBounds.Count then
    ALastColumn.Bounds := ColumnController.ColumnsBounds[AIndex];
  RowsController.RestartFormattingFromTheStartOfSection
    (PageController.CurrentSection, ALastColumn);
end;

procedure TdxDocumentFormattingController.SetColumnController
  (const Value: TdxColumnController);
begin
...
procedure TdxDocumentFormattingController.SetPageCount(APageCount: Integer;
  APrimaryFormattingFinished: Boolean);
var
  APages: TdxPageCollection;
  APage: TdxPage;
...
begin
  ...
  if not APrimaryFormattingFinished and
    (FPageCountSinceLastResetSecondaryFormatting < 100) then
  begin
