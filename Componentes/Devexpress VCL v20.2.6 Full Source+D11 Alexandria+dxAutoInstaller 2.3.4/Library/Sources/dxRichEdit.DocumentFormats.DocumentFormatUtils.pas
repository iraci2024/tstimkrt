...
type
  ...
  TdxVerticalMergeCellsCollection = class
    (TDictionary<TdxTableCell, TdxVerticalMergeCellProperties>)
  public
    function GetRowSpan(ACell: TdxTableCell): Integer;
    function GetMergedCellProperties(ACell: TdxTableCell)
      : TdxVerticalMergeCellProperties;
  end;
  ...
  public
    ...
    function CalculateMergeCellProperties(ACurrentCell: TdxTableCell)
      : TdxVerticalMergeCellProperties;
  ...
  public
    class function DeleteIllegalXmlCharacters(const AValue: string)
      : string; static;
  ...
  public
    ...
    function AddCellToSpanCollectionCore(AColumnIndex, ARowSpan,
      AColumnSpan: Integer): Integer; virtual;

    property CellsRowSpanCollection: TdxCellsRowSpanCollection
      read FCellsRowSpanCollection;
  ...
  protected
    ...
    function CreateTableInfo(ANewTable: TdxTable): TdxImportedTableInfo;
      virtual; abstract;
    function GetRowBeforeLastRow(ATable: TdxTable): TdxTableRow;
    function CreateCoveredInVerticalMergeCell(AColumnIndex: Integer)
      : Integer; virtual;
    function CreateCoveredCellWithEmptyParagraph(ALastRow: TdxTableRow)
      : TdxTableCell; virtual; abstract;
    procedure CloseUnclosedLastRowLastCell(ATable: TdxTable;
      ALastParagraphIndex: TdxParagraphIndex); virtual;
  ...
  public
    ...
    procedure InitializeTableCell(ACell: TdxTableCell;
      AStartParagraphIndex, AEndParagraphIndex: TdxParagraphIndex); virtual;
    function CreateCoveredInVerticalMergeCells(ANextColumnIndex
      : Integer): Integer;
    function FindPatternCellWithMergeRestartInPreviousRows(ARow: TdxTableRow;
      AColumnIndex: Integer): TdxTableCell;
    function FindStartParagraphIndexForCell(AInputPositionParagraphIndex
      : TdxParagraphIndex): TdxParagraphIndex; virtual;
    procedure UpdateFirstRowSpanCollection(ARowSpan,
      AColumnSpan: Integer); virtual;
    procedure UpdateFirstRowSpanCollectionCore(ARowSpan: Integer;
      AColumnSpan: Integer);
    function ProcessUnCoveredCell(ARowSpan, AColumnSpan,
      AColumnIndex: Integer): Integer;
    function ProcessLastCellInRow(ARowSpan, AColumnSpan, ALocalColumnIndex
      : Integer): Integer;
  ...
  public
    constructor Create(APieceTable: TdxPieceTable);
    procedure ProcessFieldBegin(AFieldInfo: TdxImportFieldInfo;
      APosition: TdxInputPosition);
    procedure ProcessFieldSeparator(AFieldInfo: TdxImportFieldInfo;
      APosition: TdxInputPosition);
    function ProcessFieldEnd(AFieldInfo: TdxImportFieldInfo;
      APosition: TdxInputPosition): TdxField;
    procedure InsertHyperlinkInstruction(AInfo: TdxHyperlinkInfo;
      APosition: TdxInputPosition);
...
function TdxVerticalMergeCellsCollection.GetRowSpan
...
function TdxVerticalMergeCellsCollection.GetMergedCellProperties
  (ACell: TdxTableCell): TdxVerticalMergeCellProperties;
begin
  if ContainsKey(ACell) then
    Exit(Self[ACell]);
...
begin
  AWidthsCalculator := TdxRtfTableWidthsCalculator.Create
    (DocumentModel.ToDocumentLayoutUnitConverter, AParentWidth);
  try
    ACalculator := TdxTableGridCalculator.Create(AWidthsCalculator, MaxInt);
    try
...
var
  ...
  AMargins: Integer;
begin
  APos := TdxDocumentModelPosition.FromParagraphStart(FTable.PieceTable,
    FTable.FirstRow.FirstCell.StartParagraphIndex);
...
function TdxTableInfo.GetMergedCellProperties(ACell: TdxTableCell)
  : TdxVerticalMergeCellProperties;
begin
    ...
    begin
      ...
      case ACurrentCell.VerticalMerging of
        ...
        TdxMergingState.Continue:
          Result.Add(ACurrentCell,
            TdxVerticalMergeCellProperties.Create(0, nil));
      end;
    end;
  end;
end;

function TdxTableInfo.CalculateMergeCellProperties(ACurrentCell: TdxTableCell)
  : TdxVerticalMergeCellProperties;
var
...
begin
  AColumnIndex := TdxTableCellVerticalBorderCalculator.GetStartColumnIndex
    (ACurrentCell, False);
  AVerticalMergedCells := TdxTableCellVerticalBorderCalculator.
    GetVerticalSpanCells(ACurrentCell, AColumnIndex, False);
  try
    AVerticalMergeCellsCount := AVerticalMergedCells.Count;
    AActualBottomCellBorder := AVerticalMergedCells
      [AVerticalMergeCellsCount - 1].GetActualBottomCellBorder;
  finally
    AVerticalMergedCells.Free;
  end;
  Result := TdxVerticalMergeCellProperties.Create(AVerticalMergeCellsCount,
    AActualBottomCellBorder);
end;
...
class function TdxXmlTextHelper.DeleteIllegalXmlCharacters
...
procedure TdxImportedTableInfo.AddCellToSpanCollection(ARowSpan,
  AColumnSpan: Integer);
begin
  AddCellToSpanCollectionCore(FColumnIndex, ARowSpan, AColumnSpan);
end;

procedure TdxImportedTableInfo.ExpandSpanCollection(ARowSpan,
  AColumnSpan: Integer);
var
...
function TdxImportedTableInfo.AddCellToSpanCollectionCore(AColumnIndex,
  ARowSpan, AColumnSpan: Integer): Integer;
var
  I: Integer;
begin
  CellsRowSpanCollection.Add(ARowSpan);
...
begin
  ...
  while (AColumnIndex < ACellsInPreviousRowCount) and
    (TableInfo.CellsRowSpanCollection[AColumnIndex] > 1) do
...
procedure TdxTablesImportHelper.InitializeTableCell(ACell: TdxTableCell;
  AStartParagraphIndex, AEndParagraphIndex: TdxParagraphIndex);
var
  AInvalid: TdxParagraphIndex;
begin
  AInvalid := -1;
  Assert((AStartParagraphIndex <> AInvalid) and
    (AEndParagraphIndex <> AInvalid));
  if ((ACell <> nil) and (AStartParagraphIndex <> AInvalid)) and
    (AEndParagraphIndex <> AInvalid) then
    PieceTable.TableCellsManager.InitializeTableCell(ACell,
      AStartParagraphIndex, AEndParagraphIndex);
end;

function TdxTablesImportHelper.CreateCoveredInVerticalMergeCells
  (ANextColumnIndex: Integer): Integer;
begin
  TablesCapabilityCheck;

  Result := ANextColumnIndex;
  while (Result < TableInfo.CellsRowSpanCollection.Count) and
    (TableInfo.CellsRowSpanCollection[Result] > 1) do
    Result := CreateCoveredInVerticalMergeCell(Result);
end;

function TdxTablesImportHelper.FindPatternCellWithMergeRestartInPreviousRows
  (ARow: TdxTableRow; AColumnIndex: Integer): TdxTableCell;
var
...
begin
  ...
  if AFoundCell.VerticalMerging <> TdxMergingState.Restart then
    AFoundCell := FindPatternCellWithMergeRestartInPreviousRows(ARow.Previous,
      AColumnIndex);
  Result := AFoundCell;
end;

function TdxTablesImportHelper.GetRowBeforeLastRow(ATable: TdxTable)
  : TdxTableRow;
var
  ARows: TdxTableRowCollection;
  ACount: Integer;
...
function TdxTablesImportHelper.CreateCoveredInVerticalMergeCell
  (AColumnIndex: Integer): Integer;
var
...
begin
  ...
  APatternCell := FindPatternCellWithMergeRestartInPreviousRows
    (GetRowBeforeLastRow(Table), AColumnIndex);
  ...
  TableInfo.CellsRowSpanCollection[AColumnIndex] :=
    TableInfo.CellsRowSpanCollection[AColumnIndex] - 1;
  ANextColumnIndex := AColumnIndex + 1;
  while (ANextColumnIndex < TableInfo.CellsRowSpanCollection.Count) and
    (TableInfo.CellsRowSpanCollection[ANextColumnIndex] = 0) do
  begin
  ...
  begin
    ...
    begin
      AHorizontalCoveredCell := CreateCoveredCellWithEmptyParagraph
        (Table.LastRow);
      AHorizontalCoveredCell.ColumnSpan := ANextColumnIndex -
        AExpectedNextColumnIndex;
    end;
  end;
  Result := ANextColumnIndex;
end;

procedure TdxTablesImportHelper.UpdateFirstRowSpanCollection(ARowSpan: Integer;
  AColumnSpan: Integer);
begin
  TablesCapabilityCheck;
...
procedure TdxTablesImportHelper.UpdateFirstRowSpanCollectionCore(ARowSpan,
  AColumnSpan: Integer);
var
  AColumnIndex, ANextColumnIndex, ACellCollectionCount, I,
    AIndexOfLastCell: Integer;
begin
  AColumnIndex := TableInfo.ColumnIndex;
  ANextColumnIndex := AColumnIndex + 1;
  ...
  begin
    if AColumnSpan > 1 then
      TableInfo.ColumnIndex := ProcessLastCellInRow(ARowSpan, AColumnSpan,
        AColumnIndex)
    else
      TableInfo.ColumnIndex := ANextColumnIndex;
  end
  else if TableInfo.CellsRowSpanCollection[ANextColumnIndex] > 1 then
  begin

    AIndexOfLastCell := Table.GetCellColumnIndexConsiderRowGrid
      (Table.LastRow.LastCell) + Table.LastRow.LastCell.ColumnSpan;
    if AIndexOfLastCell >= ANextColumnIndex then
    begin
      Table.LastRow.LastCell.ColumnSpan := 1;
    end;
    TableInfo.ColumnIndex := CreateCoveredInVerticalMergeCells
      (ANextColumnIndex);
  end
  else
  begin
    TableInfo.ColumnIndex := ProcessUnCoveredCell(ARowSpan, AColumnSpan,
      AColumnIndex);
    TableInfo.ColumnIndex := CreateCoveredInVerticalMergeCells
...
function TdxTablesImportHelper.ProcessUnCoveredCell(ARowSpan, AColumnSpan,
  AColumnIndex: Integer): Integer;
var
...
function TdxTablesImportHelper.ProcessLastCellInRow(ARowSpan, AColumnSpan,
  ALocalColumnIndex: Integer): Integer;
var
  I: Integer;
begin
  I := AColumnSpan;
  while (I > 1) and
    (ALocalColumnIndex < TableInfo.CellsRowSpanCollection.Count) do
  begin
    TableInfo.CellsRowSpanCollection[ALocalColumnIndex] := 0;
    Inc(ALocalColumnIndex);
    Dec(I);
...
procedure TdxTablesImportHelper.AddCellToSpanCollection(ARowSpan,
  AColumnSpan: Integer);
begin
  TablesCapabilityCheck;
...
procedure TdxTablesImportHelper.ExpandSpanCollection(ARowSpan,
  AColumnSpan: Integer);
begin
  TableInfo.ExpandSpanCollection(ARowSpan, AColumnSpan);
end;

function TdxTablesImportHelper.FindStartParagraphIndexForCell
  (AInputPositionParagraphIndex: TdxParagraphIndex): TdxParagraphIndex;
var
  ATable: TdxTable;
...
begin
  ...
  begin
    ...
    ALastCellParagraphIndex := ATable.LastRow.Cells
      [ATable.LastRow.Cells.Count - 2].EndParagraphIndex;
...
procedure TdxTablesImportHelper.CloseUnclosedLastRowLastCell(ATable: TdxTable;
...
begin
  ...
  begin
    ...
    begin
      ...
      InitializeTableCell(ALastCell, ALastCell.StartParagraphIndex,
        ALastCell.EndParagraphIndex);
    end;
  end;
end;

function TdxTablesImportHelper.FindLastValidCellInTable(ATable: TdxTable)
  : TdxTableCell;
var
  ARows: TdxTableRowCollection;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ACell.StartParagraphIndex = AInvalidParagraphIndex) or
        (ACell.EndParagraphIndex = AInvalidParagraphIndex) then
...
var
  ARowCount, AExpectedMaxColumnIndex, I, AMaxColumnIndex,
    ANewEndColumnListIndex, ANewEndColumnIndex: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ANewEndColumnListIndex := AExistingPositions.BinarySearch
          (AMaxColumnIndex);
        ...
        if ANewEndColumnIndex <> AMaxColumnIndex then
          ARow.LastCell.ColumnSpan := ARow.LastCell.ColumnSpan +
            ANewEndColumnIndex - AMaxColumnIndex;
        if ANewEndColumnIndex <> AExpectedMaxColumnIndex then
          ARow.GridAfter := ARow.GridAfter + AExpectedMaxColumnIndex -
            ANewEndColumnIndex;
      end;
    end;
    Table.Normalize;
...
procedure TdxImportFieldHelper.ProcessFieldBegin(AFieldInfo: TdxImportFieldInfo;
  APosition: TdxInputPosition);
begin
  AFieldInfo.CodeStartIndex := PieceTable.InsertFieldCodeStartRunCore
    (APosition);
end;

procedure TdxImportFieldHelper.ProcessFieldSeparator
  (AFieldInfo: TdxImportFieldInfo; APosition: TdxInputPosition);
begin
  AFieldInfo.CodeEndIndex := PieceTable.InsertFieldCodeEndRunCore(APosition);
end;

function TdxImportFieldHelper.ProcessFieldEnd(AFieldInfo: TdxImportFieldInfo;
  APosition: TdxInputPosition): TdxField;
var
  AField: TdxField;
...
begin
  ...
  AFieldInfo.ResultEndIndex := PieceTable.InsertFieldResultEndRunCore
    (APosition);
  ...
  AField.Result.SetInterval(AFieldInfo.CodeEndIndex + 1,
    AFieldInfo.ResultEndIndex);
...
procedure TdxImportFieldHelper.InsertHyperlinkInstruction
  (AInfo: TdxHyperlinkInfo; APosition: TdxInputPosition);
var
