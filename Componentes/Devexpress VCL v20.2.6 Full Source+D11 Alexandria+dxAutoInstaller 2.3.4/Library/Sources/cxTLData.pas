...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Classes, SysUtils, Windows, Messages, Variants, dxCore, cxVariants,
  cxControls,
...
type
  ...
  protected
    ...
    property LoadingInProcess: Boolean read FLoadingInProcess
      write FLoadingInProcess;
  ...
  public
    function CompareNodesByColumns(ANode1, ANode2: TcxTreeListNode;
      AColumns: TList): Integer; override;
    function GetNodeDisplayText(ANode: TcxTreeListNode; AIndex: Integer)
      : Variant; override;
  ...
  protected
    ...
    function GetImageIndex(ANode: TcxTreeListNode;
      AIndexType: TcxTreeListImageIndexType): Integer; virtual;
    ...
    function IsChildrenExist(ANode: TcxTreeListNode): Boolean; virtual;
      abstract;
  ...
  private
    ...
    function GetTreeList: TcxCustomVirtualTreeList;
  protected
    function AddChild(AParent: TcxTreeListNode;
    ...
    function FindParentNode(const AParentHandle: TcxDataRecordHandle;
      APrev: TcxVirtualTreeListNode; var ANode: TcxVirtualTreeListNode)
      : Boolean;
    function IsChildrenExist(ANode: TcxTreeListNode): Boolean; override;
    procedure LoadAllRecords; virtual;
    procedure LoadLevel(AParent: TcxTreeListNode;
      AParentHandle: TcxDataRecordHandle); virtual;
    procedure LoadValues(ANode: TcxTreeListNode);
    procedure LoadVirtualLevel(ANode: TcxTreeListNode;
  ...
  published
    property CheckHasChildren: Boolean read FCheckHasChildren
      write FCheckHasChildren default True;
    property SmartLoad: Boolean read FSmartLoad write SetSmartLoad
      default False;
  end;
  ...
  protected
    ...
    property DataController: TcxDataTreeListDataController
      read GetDataController;
    property DataChanges: TcxDataTreeListChanges read FDataChanges
      write FDataChanges;
    ...
    property OptionsData: TcxVirtualTreeListOptionsData read GetOptionsData
  ...
  protected
    ...
    procedure DoSetValue(ANode: TcxTreeListNode; AIndex: Integer;
    ...
    property DataController: TcxDataTreeListDataController
      read GetDataController;
    property UpdatingNode: TcxTreeListNode read FUpdatingNode
      write FUpdatingNode;
    property OnGetChildCount: TcxVirtualTreeListGetChildCountEvent
      read FOnGetChildCount write SetOnGetChildCount;
    property OnGetNodeValue: TcxVirtualTreeListNodeValueEvent
      read FOnGetNodeValue write SetOnGetNodeValue;
    property OnSetNodeValue: TcxVirtualTreeListNodeValueEvent
      read FOnSetNodeValue write SetOnSetNodeValue;
  public
    destructor Destroy; override;
    function HandleFromNode(ANode: TcxTreeListNode)
      : TcxDataRecordHandle; virtual;
    function NodeFromHandle(ARecordHandle: TcxDataRecordHandle)
      : TcxTreeListNode; virtual;

    property CustomDataSource: TcxTreeListCustomDataSource read GetDataSource
      write SetDataSource;
  end;
...
function cxCompareNodesByRecordHandle(ANode1,
  ANode2: TcxVirtualTreeListNode): Integer;
begin
...
procedure TcxTreeListCustomDataSource.DeleteChildItems(AParentHandle
  : TcxDataRecordHandle);
var
  I: Integer;
...
function TcxTreeListCustomDataSource.GetChildCount(AParentHandle
...
procedure TcxTreeListCustomDataSource.NodeMoveTo(ARecordHandle,
...
function TcxTreeListCustomDataSource.GetRecordHandle(ARecordIndex: Integer)
  : TcxDataRecordHandle;
begin
...
begin
  if (CustomDataSource = nil) or (ANode = Root) then
    ...
    with CustomDataSource do
      Result := GetDisplayText(TcxVirtualTreeListNode(ANode).RecordHandle,
        GetItemHandle(AIndex));
end;
...
begin
  if IsDataLoaded or LoadingInProcess or TreeList.IsTreeListLocked or
    not TreeList.IsDataSettingsValid then
    Exit;
  Load(nil);
end;
...
function TcxDataTreeListDataController.GetCustomDataSource
  : TcxTreeListCustomDataSource;
begin
  Result := FDataSource;
end;
...
var
  ALoaded: Boolean;
begin
  if IsDataLoading or (TreeList.LoadingLockCount > 0) or not TreeList.IsDataSettingsValid
  then
    Exit;
...
procedure TcxDataTreeListDataController.NodesMoveTo
  (AttachNode: TcxVirtualTreeListNode; AttachMode: TcxTreeListNodeAttachMode;
  ANodes: TList; IsCopy: Boolean);
var
  I: Integer;
begin
...
function TcxVirtualTreeListDataController.CompareNodesByColumns(ANode1,
...
function TcxVirtualTreeListDataController.GetNodeValue(ANode: TcxTreeListNode;
...
procedure TcxVirtualTreeListDataController.SetNodeValue(ANode: TcxTreeListNode;
...
begin
  ...
  if not TreeList.IsCompletelyVirtual and (FLoadingRefCount = 0) and
    (CustomDataSource <> nil) then
    CustomDataSource.SetValue(TcxVirtualTreeListNode(ANode).RecordHandle,
      CustomDataSource.GetItemHandle(AIndex), V);
  IsValueChanged := True;
end;

procedure TcxVirtualTreeListDataController.CheckLoaded(ANode: TcxTreeListNode);
begin
  if not(nsValuesAssigned in TcxVirtualTreeListNode(ANode).State) and
    not TreeList.IsDestroying then
    TcxVirtualTreeListLoader(Loader).LoadValues(ANode);
end;

function TcxVirtualTreeListDataController.CreateLoader
  : TcxTreeListStructureLoader;
begin
...
constructor TcxTreeListStructureLoader.Create
  (AOwner: TcxDataTreeListDataController);
begin
...
begin
  if ANode = Root then
    Exit;
  for AIndex := Low(TcxTreeListImageIndexType)
    to High(TcxTreeListImageIndexType) do
    TcxVirtualTreeListNode(ANode).FImageIndexes[AIndex] :=
      GetImageIndex(ANode, AIndex);
...
function TcxTreeListStructureLoader.GetImageIndex(ANode: TcxTreeListNode;
...
procedure TcxTreeListStructureLoader.InternalMove(ANode,
...
begin
  ...
  try
    ...
    if not IsLoadAllRecords and (ALoadingNode <> nil) and
      (ALoadingNode.Count > 0) then
...
function TcxVirtualTreeListLoader.ExtractNode(ARecordHandle
  : TcxDataRecordHandle; var ANode: TcxVirtualTreeListNode): Boolean;
var
...
function TcxVirtualTreeListLoader.FindParentNode(const AParentHandle
  : TcxDataRecordHandle; APrev: TcxVirtualTreeListNode;
  var ANode: TcxVirtualTreeListNode): Boolean;
var
  L, H, I, C: TdxNativeInt;
begin
...
function TcxVirtualTreeListLoader.IsChildrenExist
  (ANode: TcxTreeListNode): Boolean;
begin
  Result := DataSource.GetChildCount(TcxVirtualTreeListNode(ANode)
    .RecordHandle) <> 0;
end;
    ...
    try
      ...
      begin
        ...
        if FindParentNode(AHandle, ANode, ANode) then
          InternalMove(TcxVirtualTreeListNode(AOrder.List[I]), ANode,
            tlamAddChild)
        else
          InternalMove(TcxVirtualTreeListNode(AOrder.List[I]), Root,
            tlamAddChild);
      end;
...
procedure TcxVirtualTreeListLoader.LoadLevel(AParent: TcxTreeListNode;
...
var
  I: Integer;
begin
  if nsValuesAssigned in TcxVirtualTreeListNode(ANode).State then
    Exit;
  ...
  try
    if TreeList.IsCompletelyVirtual then
      ...
      for I := 0 to TreeList.ColumnCount - 1 do
        DataController.SetNodeValue(ANode, I,
          DataSource.GetValue(TcxVirtualTreeListNode(ANode).RecordHandle,
          DataSource.GetItemHandle(I)));
  finally
    InterlockedDecrement(DataController.FLoadingRefCount);
  end;
  DataController.IsValueChanged := not ANode.IsEditing;
...
procedure TcxVirtualTreeListLoader.LoadVirtualLevel(ANode: TcxTreeListNode;
  ARecursive: Boolean);
var
  ACount, I: Integer;
  ...
  begin
    ...
    if ARecursive then
      ...
      AChildNode.HasChildren := (AChildNode.Count <> 0) or
        (not TreeList.OptionsData.CheckHasChildren or
        (TreeList.DoGetChildCount(AChildNode) > 0));
    AChildNode := AChildNode.GetNextSibling;
  end;
end;

procedure TcxVirtualTreeListLoader.RestoreNodeState(ANode: TcxTreeListNode;
  AData: Pointer);
begin
  TcxVirtualTreeListNode(ANode).RestoreStateAfterRefresh;
end;

procedure TcxVirtualTreeListLoader.SaveNodeState(ANode: TcxTreeListNode;
  AData: Pointer);
begin
  TcxVirtualTreeListNode(ANode).SaveStateBeforeRefresh;
end;

function TcxVirtualTreeListLoader.GetDataController
  : TcxDataTreeListDataController;
begin
...
procedure TcxCustomDataTreeList.AddDataChanges
  (AChanges: TcxDataTreeListChanges);
begin
...
begin
  if DataController.IsLoading or (FLockDataChanges > 0) or (DataChanges = [])
  then
    Exit;
...
begin
  if DataController.IsDataLoading or (LoadingLockCount > 0) then
    Exit;
...
begin
  if (DataController.EditingNode <> nil) and DataController.EditingNode.Inserting
  then
...
function TcxCustomVirtualTreeList.HandleFromNode(ANode: TcxTreeListNode)
  : TcxDataRecordHandle;
begin
  if ANode = Root then
...
function TcxCustomVirtualTreeList.NodeFromHandle(ARecordHandle
  : TcxDataRecordHandle): TcxTreeListNode;
var
  I: Integer;
begin
  Result := nil;
  ...
  for I := 0 to AbsoluteCount - 1 do
    if TcxVirtualTreeListNode(AbsoluteItems[I]).RecordHandle = ARecordHandle
    then
    begin
...
begin
  Result := inherited CanUseMultiThreadedSorting and (CustomDataSource <> nil)
    and CustomDataSource.IsMultiThreadingSupported;
end;
...
function TcxCustomVirtualTreeList.DoGetChildCount
...
var
  ...
  AValue: Variant;
begin
  if not Assigned(FOnGetNodeValue) then
    Exit;
...
begin
  if OptionsData.SmartLoad and (AttachMode = tlamAddChild) and
    (AttachNode <> nil) and AttachNode.HasChildren and (AttachNode.Count = 0)
  then
  ...
  try
    ...
    if not IsCompletelyVirtual then
      DataController.NodesMoveTo(TcxVirtualTreeListNode(AttachNode), AttachMode,
        ANodes, IsCopy);
  finally
    EndUpdate;
  end;
...
procedure TcxCustomVirtualTreeList.DoSetValue(ANode: TcxTreeListNode;
...
function TcxCustomVirtualTreeList.GetDataControllerClass
...
function TcxCustomVirtualTreeList.InsertNode(ARelative: TcxTreeListNode;
...
function TcxCustomVirtualTreeList.GetDataController
  : TcxDataTreeListDataController;
begin
  Result := TcxDataTreeListDataController(inherited DataController);
end;
