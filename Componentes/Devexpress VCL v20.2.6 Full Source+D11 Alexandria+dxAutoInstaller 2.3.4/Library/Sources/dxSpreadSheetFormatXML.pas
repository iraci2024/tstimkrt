...
{$ENDIF}
  ...
  dxSpreadSheetTextFileFormatCore, dxSpreadSheetFormatHTML,
...
type
  ...
  TdxSpreadSheetXMLFormatCellStyleWriter = class
  ...
  TdxSpreadSheetXMLFormatTableViewWriter = class
    (TdxSpreadSheetHTMLFormatTableViewWriter)
  protected
    procedure WriteCell(ARowIndex: Integer; AColumnIndex: Integer;
      ACell: TdxSpreadSheetCell); override;
    procedure WriteContainer(AContainer: TdxSpreadSheetContainer;
      AContainerBounds: TRect); override;
...
const
  sdxXMLStyleTemplate = '<?xml version="1.0" encoding="UTF-8"?>' + #13#10 +
    '<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">'
    + #13#10 + '   <xsl:template match="/">' + #13#10 +
    '     <xsl:apply-templates select="CACHE" />' + #13#10 +
    '   </xsl:template>' + #13#10 +

    '   <xsl:template match="CACHE">' + #13#10 + '     <html>' + #13#10 +
    '        <head>' + #13#10 +
    ...
    '        </body>' + #13#10 + '     </html>' + #13#10 + '   </xsl:template>'
    + #13#10 +

    '   <xsl:template match="TITLE">' + #13#10 + '     <title>' + #13#10 +
    '       <xsl:value-of select="." />' + #13#10 + '     </title>' + #13#10 +
    '   </xsl:template>' + #13#10 +

    '   <xsl:template match="STYLES">' + #13#10 + '     <style type="text/css">'
    + #13#10 + '       table td { overflow: hidden; padding: 0px;}' + #13#10 +
    '       <xsl:apply-templates select="STYLE" />' + #13#10 + '     </style>' +
    #13#10 + '   </xsl:template>' + #13#10 +

    '   <xsl:template match="STYLE">' + #13#10 +
    '     .Style<xsl:value-of select="@Id" />' + #13#10 + '     { ' + #13#10 +
    ...
    '       font-weight: bold;' + #13#10 + '     </xsl:if>' + #13#10 +
    '     <xsl:if test="@Italic=''True''">' + #13#10 +
    '       font-style: italic;' + #13#10 + '     </xsl:if>' + #13#10 +
    '     <xsl:if test="@Underline=''True''">' + #13#10 +
    '       text-decoration: underline;' + #13#10 + '     </xsl:if>' + #13#10 +
    '     <xsl:if test="@StrikeOut=''True''">' + #13#10 +
    '       text-decoration: line-through;' + #13#10 + '     </xsl:if>' + #13#10
    ...
    + #13#10 + '       <xsl:apply-templates select="LINE" />' + #13#10 +
    '     </table>' + #13#10 + '   </xsl:template>' + #13#10 +

    '   <xsl:template match="LINE">' + #13#10 + '     <tr>' + #13#10 +
    '       <xsl:attribute name="height"><xsl:value-of select="@Height" /></xsl:attribute>'
    + #13#10 + '       <xsl:apply-templates select="CELL" />' + #13#10 +
    '     </tr>' + #13#10 + '   </xsl:template>' + #13#10 +

    '   <xsl:template match="CELL">' + #13#10 + '     <td>' + #13#10 +
    ...
    + #13#10 + '       <xsl:choose>' + #13#10 +
    '         <xsl:when test="LINES">' + #13#10 +
    '           <xsl:apply-templates select="LINES" />' + #13#10 +
    '         </xsl:when>' + #13#10 + '         <xsl:when test="IMAGE">' +
    #13#10 + '           <xsl:apply-templates select="IMAGE" />' + #13#10 +
    '         </xsl:when>' + #13#10 + '         <xsl:otherwise>' + #13#10 +
    '           <xsl:value-of select="." />' + #13#10 +
    '         </xsl:otherwise>' + #13#10 + '       </xsl:choose>' + #13#10 +
    '     </td>' + #13#10 + '   </xsl:template>' + #13#10 +

    '   <xsl:template match="IMAGE">' + #13#10 + '     <img>' + #13#10 +
    '       <xsl:attribute name="src"><xsl:value-of select="@Src" /></xsl:attribute>'
    + #13#10 + '       <xsl:value-of select="." />' + #13#10 + '     </img>' +
    #13#10 + '   </xsl:template>' + #13#10 + '</xsl:stylesheet>';

function GetXMLColor(AColor: TColor): string;
begin
  AColor := ColorToRGB(AColor);
  Result := Format('rgb(%d,%d,%d)', [GetRValue(AColor), GetGValue(AColor),
    GetBValue(AColor)]);
end;

function cxStrUnicodeNeeded(const AText: string;
  ACheckNormal: Boolean = False): Boolean;
const
  Normal = ['0' .. '9', ':', ';', '*', '+', ',', '-', '.', '/', '!', ' ',
    'A' .. 'Z', 'a' .. 'z', '_', '(', ')'];
var
...
begin
  ...
  for I := 1 to Length(AText) do
    if (Ord(AText[I]) > $7F) or
      (ACheckNormal and not dxCharInSet(AText[I], Normal)) then
    begin
      Result := True;
      Break;
    end
...
class function TdxSpreadSheetXMLFormat.CanReadFromStream
...
class function TdxSpreadSheetXMLFormat.GetReader
  : TdxSpreadSheetCustomReaderClass;
begin
  Result := nil;
end;

class function TdxSpreadSheetXMLFormat.GetWriter
...
function TdxSpreadSheetXMLFormatWriter.RegisterStyle
  (AStyle: TdxSpreadSheetCellStyle): string;
begin
  if not StyleMap.TryGetValue(AStyle.Handle, Result) then
  begin
    Result := IntToStr(StyleMap.Count);
    ExecuteSubTask(TdxSpreadSheetXMLFormatCellStyleWriter.Create(Result,
      StylesWriter, Self, AStyle.Handle));
...
begin
  ...
  begin
    ...
    StreamWriter.WriteLine('<?xml-stylesheet type="text/xsl" href="%s"?>',
      [AStyleSheetFileName]);
  end;

  StreamWriter.WriteLine('<CACHE>');
  StreamWriter.WriteLine('<TITLE>%S</TITLE>',
    [SpreadSheet.ActiveSheetAsTable.Caption]);
...
procedure TdxSpreadSheetXMLFormatWriter.WriteStyleSheet(const AFileName
  : UnicodeString);
var
  AStream: TFileStream;
...
constructor TdxSpreadSheetXMLFormatCellStyleWriter.Create(const AStyleName
  : string; AStreamWriter: TStreamWriter; AOwner: TdxSpreadSheetCustomFiler;
...
const
  SideName: array [TcxBorder] of string = ('BORDER_LEFT', 'BORDER_UP',
...
begin
  ...
  begin
    ABorderColor := cxGetActualColor(Style.Borders.BorderColor[ASide],
      ADefaultBorderColor);
    StreamWriter.WriteLine('<%s IsDefault="False" Color="%s" Width="%d"/>',
      [SideName[ASide], GetXMLColor(ABorderColor),
      IfThen(ABorderColor <> clNone, dxSpreadSheetBorderStyleThickness
      [Style.Borders.BorderStyle[ASide]])]);
  end;
end;
...
begin
  ...
  begin
    ...
    StreamWriter.Write(' BrushBkColor="' +
      GetXMLColor(cxGetActualColor(Style.Brush.BackgroundColor,
      ContentStyle.Color)) + '"');
    StreamWriter.Write(' BrushFgColor="' +
      GetXMLColor(cxGetActualColor(Style.Brush.ForegroundColor,
...
const
  FontStyleNameMap: array [TFontStyle] of string = ('Bold', 'Italic',
    'Underline', 'StrikeOut');
var
  AStyle: TFontStyle;
  ATextColor: TColor;
begin
  ...
  for AStyle := Low(TFontStyle) to High(TFontStyle) do
    StreamWriter.Write(' %s="%s"', [FontStyleNameMap[AStyle],
      BoolToStr(AStyle in Style.Font.Style, True)]);
...
procedure TdxSpreadSheetXMLFormatTableViewWriter.WriteCell(ARowIndex,
...
begin
  ...
  if (AMergedCell <> nil) and ((AMergedCell.Area.Left <> AColumnIndex) or
    (AMergedCell.Area.Top <> ARowIndex)) then
    Exit;

  ACellStyle := CalculateActualCellStyle(ARowIndex, AColumnIndex, ACell);
  try
    StreamWriter.Write('<CELL StyleClass="%s" Width="%d"',
      [Owner.RegisterStyle(ACellStyle),
      TdxSpreadSheetTableColumnsAccess(View.Columns)
      .GetItemSize(AColumnIndex)]);
  finally
    ACellStyle.Free;
  end;
  ...
  begin
    ...
    StreamWriter.WriteLine('<IMAGE Src="%s">%s</IMAGE>',
      [AFileName, ACellDisplayText]);
  end
  else
...
procedure TdxSpreadSheetXMLFormatTableViewWriter.WriteContainer
  (AContainer: TdxSpreadSheetContainer; AContainerBounds: TRect);
begin
  // do nothing, only inscribed containers are supported
end;

procedure TdxSpreadSheetXMLFormatTableViewWriter.WriteRowFooter
  (ARowIndex: Integer; ARow: TdxSpreadSheetTableRow);
begin
  StreamWriter.WriteLine('</LINE>');
end;

procedure TdxSpreadSheetXMLFormatTableViewWriter.WriteRowHeader
  (ARowIndex: Integer; ARow: TdxSpreadSheetTableRow);
begin
...
begin
  StreamWriter.WriteLine('<LINES ColCount="%d" RowCount="%d">',
