...
procedure RegisterPDFViewerPrintingActions;
  ...
  function FindCategory(AComponent: TdxUIGeneratorComponentInfo;
    const ATabName, ABArName: string;
  ...
  begin
    ...
    begin
      Result := (AComponent.Categories[I].TabName = ATabName) and
        (AComponent.Categories[I].BarName = ABArName);
      if Result then
      begin
...
begin
  ...
  begin
    if not FindCategory(AComponent, sdxPDFViewerTabPDFViewer,
      sdxPDFViewerBarPDFViewerFile, ACategory) then
      ACategory := AComponent.Add(sdxPDFViewerTabPDFViewer,
        sdxPDFViewerBarPDFViewerFile, 'Print\Print_16x16.png', 0);
    ACategory.Add(TdxPDFViewerShowPrintForm);
    RegisterActions(TdxUIGeneratorHelper.GenerateCategoryName(ACategory),
      [TdxPDFViewerShowPrintForm], nil);
  end;
end;
...
begin
  ...
  HideProperty(TypeInfo(THelpContext), TdxPDFViewerReportLink,
    'DesignerHelpContext');
  HideProperty(TypeInfo(TdxPSPDFReportExportOptions), TdxPDFViewerReportLink,
    'PDFExportOptions');
