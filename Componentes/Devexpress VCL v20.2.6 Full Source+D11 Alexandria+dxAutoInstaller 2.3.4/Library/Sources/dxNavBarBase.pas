...
type
  ...
  TdxNavBarChangeEvent = procedure(Sender: TObject; AType: TdxNavBarChangeType)
    of object; // for internal use

  { TdxNavBarComponentCollection }

  TdxNavBarComponentCollection = class;
  TdxNavBarComponentCollectionItemClass = class of
  ...
  // for internal use
    ...
    function GetNextObjectForNavigation(ADirection: TcxDirection)
  ...
  TdxNavBarCustomAccessibilityHelper = class(TcxAccessibilityHelper,
    IdxNavBarAccessibilityHelper) // for internal use
  strict private
  ...
  private
    ...
    FLocalId: Integer;
    FTag: Integer;
    function InternalGetChild(AIndex: Integer)
  ...
  protected
    ...
    function Focused(out AIsChildFocused: Boolean;
      out AFocusedChildIndex: Integer): Boolean; override;
    function GetChild(AIndex: Integer): TcxAccessibilityHelper; override;
    function GetChildCount: Integer; override;
    function GetHitTest(AScreenX: Integer; AScreenY: Integer;
      out AChildIndex: Integer): TcxAccessibleObjectHitTest; override;
    ...
    function InternalGetNextObjectForNavigation(ADirection: TcxDirection)
    ...
    function IsScrollable(out AInvisiblePartHeight, AChildMinTopScreenBound
  ...
  public
    constructor Create(AOwnerObject: TObject; AOwnerObjectControl: TWinControl);
      reintroduce; virtual;
    destructor Destroy; override;
    function GetScreenBounds(AChildID: TcxAccessibleSimpleChildElementID)
      : TRect; override;
    property Children[AIndex: Integer]: TdxNavBarCustomAccessibilityHelper
      read InternalGetChild;
    property Parent: TdxNavBarCustomAccessibilityHelper read InternalGetParent;
  end;

  TdxNavBarCustomAccessibilityHelperClass = class of
    TdxNavBarCustomAccessibilityHelper;

procedure NavBarAccessibleObjectOwnerObjectDestroyed(var AAccessibleObject
  : IdxNavBarAccessibilityHelper);
function NavBarGetAccessibilityHelper(AObject: TObject)
  : IdxNavBarAccessibilityHelper;
function NavBarGetFocusableAccessibleObjectAtPos(ARootAccessibleObject
  : IdxNavBarAccessibilityHelper; const P: TPoint)
  : IdxNavBarAccessibilityHelper;
...
function NavBarGetFocusableAccessibleObjectAtPos(ARootAccessibleObject
  ...
  function InternalGetFocusableAccessibleObjectAtPos(ARootAccessibleObject
    : TdxNavBarCustomAccessibilityHelper; const P: TPoint)
    : IdxNavBarAccessibilityHelper;
  var
    ...
    if not AHelper.IsContainer then
      ...
      begin
        ...
        if AChildHelper.Visible and
          PtInRect(AChildHelper.GetScreenBounds(cxAccessibleObjectSelfID), P)
        then
        begin
...
procedure RemoveRedundantFocusableAccessibleObjects
  ...
  var ACurrentObjectScreenBounds: TRect; ADirection: TcxDirection); forward;
...
procedure ChooseOptimalFocusableAccessibleObject(AFocusableAccessibleObjectList
  : TList; const ACurrentObjectScreenBounds: TRect; ADirection: TcxDirection;
  ADistance: Integer);
var
  ...
  begin
    ...
    if ADistance = 0 then
      with ACurrentObjectScreenBounds do
        if ((ADirection in [dirUp, dirDown]) and
          (AObjectScreenBounds.Left = Left) and
          (AObjectScreenBounds.Right = Right)) or
          ((ADirection in [dirLeft, dirRight]) and
          (AObjectScreenBounds.Top = Top) and
          (AObjectScreenBounds.Bottom = Bottom)) then
        begin
          AOptimalObject := AObject;
          Break;
        end;
    ABound := Abs(IfThen(ADirection in [dirUp, dirDown],
      ACurrentObjectScreenBounds.Left - AObjectScreenBounds.Left,
...
function GetNearestFocusableAccessibleObject(AFocusableAccessibleObjectList
  : TList; ACurrentObject: TdxNavBarCustomAccessibilityHelper;
  var ACurrentObjectScreenBounds: TRect; ADirection: TcxDirection)
  : TdxNavBarCustomAccessibilityHelper; overload;

  function GetDistance(const ACurrentObjectScreenBounds, AObjectScreenBounds
    : TRect; ADirection: TcxDirection): Integer;
  begin
    Result := 0;
    case ADirection of
      dirLeft:
...
begin
  ...
  begin
    ...
    AObjectScreenBounds := AObject.GetScreenBounds(cxAccessibleObjectSelfID);
    OffsetRect(AObjectScreenBounds, 0, AObject.Tag);
    if GetDistance(ACurrentObjectScreenBounds, AObjectScreenBounds, ADirection)
      > AMinDistance then
  ...
  if AFocusableAccessibleObjectList.Count = 0 then
    Result := nil
  else if TdxNavBarCustomAccessibilityHelper(AFocusableAccessibleObjectList[0]).IsContainer
  then
  begin
    ReplaceContainerWithItsFocusableChildren(AFocusableAccessibleObjectList, 0,
      ACurrentObject, ACurrentObjectScreenBounds);
    ...
    Result := TdxNavBarCustomAccessibilityHelper
      (AFocusableAccessibleObjectList[0]);
end;

function GetNearestFocusableAccessibleObject(ARootAccessibleObject,
  ACurrentObject: TdxNavBarCustomAccessibilityHelper;
  ACurrentObjectScreenBounds: TRect; ADirection: TcxDirection)
  : TdxNavBarCustomAccessibilityHelper; overload;
var
  AFocusableAccessibleObjectList: TList;
begin
...
begin
  ...
  begin
    ...
    else if AObject.IsContainer and AObject.IsChild(ACurrentObject) then
...
var
  ...
  AContainerBottomBound, AContainerInvisiblePartHeight,
    AContainerChildMinTopScreenBound, AContainerChildTopBoundOffset, I,
    J: Integer;
...
begin
  ...
  AContainerScreenBounds := AContainer.GetScreenBounds
  ...
  if (ACurrentObject <> nil) and AContainer.IsScrollable
    (AContainerInvisiblePartHeight, AContainerChildMinTopScreenBound) then
  begin
    for I := 0 to AFocusableAccessibleObjectList.Count - 1 do
    begin
      AObject := TdxNavBarCustomAccessibilityHelper
        (AFocusableAccessibleObjectList[I]);
      if AObject.GetScreenBounds(cxAccessibleObjectSelfID).Top + AObject.Tag >= AContainerBottomBound
      then
  ...
  begin
    ...
    AObjectScreenBounds := AObject.GetScreenBounds(cxAccessibleObjectSelfID);
    if (ACurrentObject = nil) and
      ((AObjectScreenBounds.Top < AContainerScreenBounds.Top) or
  ...
  if (ACurrentObject <> nil) and AContainer.IsChild(ACurrentObject) then
    OffsetRect(ACurrentObjectScreenBounds, 0, AContainerChildTopBoundOffset -
      AContainer.Tag);
end;
...
function TdxNavBarComponentCollectionItem.InternalGetCollection
  : TdxNavBarComponentCollection;
begin
  Result := TdxNavBarComponentCollection(inherited Collection);
end;

procedure TdxNavBarComponentCollectionItem.InternalSetCollection
...
function TdxNavBarComponentCollection.ItemByName(const AName: string)
  : TdxNavBarComponentCollectionItem;
var
  I: Integer;
begin
...
begin
  ...
  FLocalId := TdxCustomNavBarAccess(OwnerObjectControl)
    .FAccessibleObjects.Count + 1;
...
begin
  ...
  TdxCustomNavBarAccess(OwnerObjectControl)
    .AccessibleObjectOwnerObjectDestroyedNotification(Self);
end;

procedure TdxNavBarCustomAccessibilityHelper.DoDefaultAction
...
function TdxNavBarCustomAccessibilityHelper.Focused(out AIsChildFocused
  : Boolean; out AFocusedChildIndex: Integer): Boolean;
begin
  Result := IsFocused;
  if Result then
    AIsChildFocused := False
  else
    Result := inherited Focused(AIsChildFocused, AFocusedChildIndex) or
      IsFocused;
end;

function TdxNavBarCustomAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
  Result := (FAttachedChildList[AIndex] as IdxNavBarAccessibilityHelper)
    .GetHelper;
end;
...
function TdxNavBarCustomAccessibilityHelper.GetHitTest(AScreenX: Integer;
  AScreenY: Integer; out AChildIndex: Integer): TcxAccessibleObjectHitTest;
var
  I: Integer;
begin
  Result := inherited GetHitTest(AScreenX, AScreenY, AChildIndex);
  for I := 0 to GetChildCount - 1 do
    if cxRectPtIn(GetChild(I).GetScreenBounds(cxAccessibleObjectSelfID),
      Point(AScreenX, AScreenY)) then
    begin
      AChildIndex := I;
      Result := aohtChild;
    end;
...
function TdxNavBarCustomAccessibilityHelper.CanFocus(AFocusingByDefault
  : Boolean): Boolean;
...
begin
  Result := not(csDestroying in OwnerObjectControl.ComponentState) and
    (States[cxAccessibleObjectSelfID] and (cxSTATE_SYSTEM_INVISIBLE or
    cxSTATE_SYSTEM_UNAVAILABLE) = 0) and
    (not IsContainer and (not AFocusingByDefault or CanBeFocusedByDefault) or
    IsContainer and HasFocusableChild);
end;
...
begin
  ...
  begin
    ...
    NotifyWinEvent(EVENT_OBJECT_FOCUS, OwnerObjectControl.Handle, FLocalId,
      CHILDID_SELF);
  end
  else
    SetIsPressed(False);
...
function TdxNavBarCustomAccessibilityHelper.GetNavBarHelper
  : TdxNavBarCustomAccessibilityHelper;
begin
  Result := Self;
end;
...
function TdxNavBarCustomAccessibilityHelper.IsFocused: Boolean;
  ...
  begin
    if TdxCustomNavBarAccess(OwnerObjectControl).FocusedAccessibleObject <> nil
    then
      Result := TdxCustomNavBarAccess(OwnerObjectControl)
        .FocusedAccessibleObject.GetNavBarHelper
    else
      Result := nil;
  end;
...
begin
  ...
  Result := OwnerObjectControl.Focused and (AFocusedAccessibleObject <> nil) and
    ((AFocusedAccessibleObject.GetHelper = Self) or
    (AFocusedAccessibleObject.AssociatedObject = Self));
end;
...
begin
  ...
  AGroupsAreaClipBounds := TdxNavBarViewInfoAccess
    (TdxCustomNavBar(OwnerObjectControl).ViewInfo).GroupsArea;
  AGroupsAreaClipBounds := cxRectOffset(AGroupsAreaClipBounds,
    OwnerObjectControl.ClientOrigin);
  IntersectRect(AClipBounds, AClipBounds, AGroupsAreaClipBounds);
  Result := not cxRectContain(AClipBounds,
...
function TdxNavBarCustomAccessibilityHelper.GetAssociatedObject
  : TdxNavBarCustomAccessibilityHelper;
begin
  Result := nil;
end;
...
begin
  ...
  if Parent = nil then
    OffsetRect(ACurrentObjectScreenBounds, 0,
      -cxRectHeight(ACurrentObjectScreenBounds));
...
function TdxNavBarCustomAccessibilityHelper.InternalGetChild(AIndex: Integer)
  : TdxNavBarCustomAccessibilityHelper;
begin
  Result := GetChild(AIndex) as TdxNavBarCustomAccessibilityHelper;
end;

function TdxNavBarCustomAccessibilityHelper.InternalGetParent
  : TdxNavBarCustomAccessibilityHelper;
begin
