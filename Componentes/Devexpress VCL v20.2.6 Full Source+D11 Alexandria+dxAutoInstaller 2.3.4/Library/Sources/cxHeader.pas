...
type
  ...
  TcxSectionTrackEvent = procedure(HeaderControl: TcxCustomHeader;
    ...
    of object;
  TcxSectionDragEvent = procedure(Sender: TObject;
    FromSection, ToSection: TcxHeaderSection; var AllowDrag: Boolean) of object;
  ...
  published
    ...
    property ImageIndex: TcxImageIndex read FImageIndex write SetImageIndex
      default -1;
    property MaxWidth: TcxNaturalNumber read FMaxWidth write SetMaxWidth
      default 1000;
    property MinWidth: TcxNaturalNumber read FMinWidth write SetMinWidth
      default 30;
    property ParentBiDiMode: Boolean read FParentBiDiMode
      write SetParentBiDiMode default True;
    property SortOrder: TcxHeaderSortOrder read FSortOrder write SetSortOrder
      default soNone;
  ...
  public
    ...
    property Items[Index: Integer]: TcxHeaderSection read GetItem
  ...
  private
    ...
    function GetSectionIndexAtPos(X, Y: Integer;
      AIncludeNonSectionPart: Boolean = False): Integer;
    ...
    procedure HDMGetItemInfo(var Message: TDXMHeaderItemInfo);
    ...
    property HotSectionIndex: Integer read FHotSectionIndex
    ...
    property SelectedSectionIndex: Integer read FSelectedSectionIndex
      write SetSelectedSectionIndex;
    property SizedSection: TcxHeaderSection read GetSizedSection;
    property SizedSectionIndex: Integer read FSizedSectionIndex
      write SetSizedSectionIndex;
    property UnderMouseSection: TcxHeaderSection read GetUnderMouseSection;
    property UnderMouseSectionIndex: Integer read FUnderMouseSectionIndex
      write SetUnderMouseSectionIndex;
  protected
    ...
    function DoSectionDrag(FromSection, ToSection: TcxHeaderSection)
      : Boolean; virtual;
    ...
    function GetSectionRectBySectionWidths(AHeaderWidth: Integer;
      const ASectionWidths: TcxHeaderSectionWidths; AIndex: Integer)
      : TRect; virtual;
    ...
    procedure DoSectionDragEvent(FromSection, ToSection: TcxHeaderSection;
      var AllowDrag: Boolean); dynamic;
  ...
  public
    ...
    property AllowMultiSort: Boolean read FAllowMultiSort
      write SetAllowMultiSort default True;
...
procedure DrawScaledHeaderSection(AHeaderHandle: HWND; AIndex: Integer;
  ACanvas: TcxCanvas; ALookAndFeel: TcxLookAndFeel;
...
begin
  DrawScaledHeaderSection(AHeaderHandle, AIndex, ACanvas, ALookAndFeel,
    dxSystemScaleFactor, AImages);
end;
...
const
  ...
  procedure DrawSortingMark(const ARect: TRect; ASortOrder: TcxHeaderSortOrder;
    AUserRightToLeftAlignment: Boolean);
  var
  ...
  begin
    ...
    begin
      ...
      ASortingMarkRect.Left := ASortingMarkRect.Right -
        ALookAndFeel.Painter.ScaledSortingMarkSize(AScaleFactor).X;
      if AUserRightToLeftAlignment then
        ASortingMarkRect := TdxRightToLeftLayoutConverter.ConvertRect
          (ASortingMarkRect, ARect);
      ALookAndFeel.Painter.DrawScaledSortingMark(ACanvas, ASortingMarkRect,
        ASortOrder = soAscending, AScaleFactor);
      ACanvas.Pen.Color := ASavedPenColor;
    end;
  end;

  function GetTextRect(const ANormalContentRect: TRect;
    ASortOrder: TcxHeaderSortOrder): TRect;
  begin
    ...
    if ASortOrder <> soNone then
      Dec(Result.Right, AScaleFactor.Apply(SortingMarkOffset) +
        ALookAndFeel.Painter.ScaledSortingMarkSize(AScaleFactor).X);
  end;

  function GetImageRect(var ATextRect: TRect;
    var AHeaderItemInfo: THeaderItemInfo): TRect;
  var
    AImageSize: TSize;
    ATextWidth: Integer;
  ...
  begin
    ...
    case AHeaderItemInfo.SectionAlignment of
      ...
      taRightJustify:
        AImageRectLeft := ATextRect.Right -
          (ATextWidth + AImageSize.cx + AImageTextOffset);
    else { taCenter: }
      AImageRectLeft := ATextRect.Left + (cxRectWidth(ATextRect) - AImageSize.cx
        - AImageTextOffset - ATextWidth) div 2;
    end;
    Result := cxRectSetLeft(cxRectSetWidth(ATextRect, AImageSize.cx),
      AImageRectLeft);
...
begin
  ...
  AHeaderRect := Rect(0, 0, cxRectWidth(AHeaderRect),
    cxRectHeight(AHeaderRect));
  ...
  cxSendStructMessage(AHeaderHandle, DXM_GETHEADERITEMINFO, AIndex,
    AHeaderItemInfo);
  ...
  else if ALookAndFeel.Painter.LookAndFeelStyle = lfsUltraFlat then
    if AHeaderItemInfo.UseRightToLeftAlignment then
      Dec(AHeaderItemInfo.Rect.Left)
    else
      Inc(AHeaderItemInfo.Rect.Right);

  ANormalContentRect := ALookAndFeel.Painter.HeaderControlSectionContentBounds
    (AHeaderItemInfo.Rect, cxbsNormal);
  ...
  if AHeaderItemInfo.UseRightToLeftAlignment then
  begin
    AImageRect := TdxRightToLeftLayoutConverter.ConvertRect(AImageRect,
      AHeaderItemInfo.Rect);
  ...
  try
    ...
    ALookAndFeel.Painter.DrawScaledHeaderControlSection(ACanvas,
      AHeaderItemInfo.Rect, ATextRect, ANeighbors, cxBordersAll,
      AHeaderItemInfo.State, ATextAlignment, vaCenter, False, True,
      AHeaderItemInfo.Text, ACanvas.Font, ACanvas.Font.Color,
      ACanvas.Brush.Color, AScaleFactor);

    ACanvas.IntersectClipRect(ANormalContentRect);
    if IsImageAssigned(AImages, AHeaderItemInfo.ImageIndex) then
      cxDrawImage(ACanvas, AImageRect, nil, AImages, AHeaderItemInfo.ImageIndex,
        ifmNormal, idmNormal, False, nil, AScaleFactor);

    DrawSortingMark(AHeaderItemInfo.Rect, AHeaderItemInfo.SortOrder,
      AHeaderItemInfo.UseRightToLeftAlignment);
  finally
...
begin
  ...
  Result := Max(TdxTextMeasurer.TextLineHeight(Canvas.Font),
    dxGetImageSize(Images, ScaleFactor).cy) + 2 *
    (LookAndFeel.Painter.HeaderControlSectionBorderSize(cxbsNormal) +
    ScaleFactor.Apply(cxHeaderSectionTextTopBottomIndent));
end;
...
begin
  FLineDC := GetDCEx(Parent.Handle, 0, DCX_CACHE or DCX_CLIPSIBLINGS or
...
procedure TcxCustomHeader.ChangeScaleEx(M: Integer; D: Integer;
  isDpiChange: Boolean);
begin
  inherited;
...
begin
  if IsSectionIndex(SectionIndex) and Assigned(FOnDrawSection) then
    ...
    DrawScaledHeaderSection(Handle, SectionIndex, Canvas, LookAndFeel,
      ScaleFactor, Images);
end;
...
procedure TcxCustomHeader.HDMGetItemInfo(var Message: TDXMHeaderItemInfo);
  ...
  begin
    if not Enabled then
      Result := cxbsDisabled
    else if IsSectionIndex(AIndex) then
...
procedure TcxCustomHeader.DoSectionDragEvent(FromSection,
  ...
  begin
    ...
    if UseRightToLeftAlignment then
      Result := TdxRightToLeftLayoutConverter.ConvertRect(Result,
        cxRectBounds(0, 0, AHeaderWidth, Height));
  end;
end;
  ...
  case FState of
    hsSized:
      begin
        ...
        else if ACalcNewWidth > SizedSection.MaxWidth then
    ...
    hsPressed:
      begin
        ...
        begin
          ...
          ChangeSectionSortOrder(ASection, GetNextSortOrder(ASection.SortOrder,
            ssCtrl in AState), [ssCtrl, ssShift] * AState = []);
        end;
      end;
  end;
  FState := hsNone;
...
function TcxCustomHeader.GetSectionIndexAtPos(X, Y: Integer;
  AIncludeNonSectionPart: Boolean): Integer;
var
  I: Integer;
  ACount: Integer;
begin
  ...
  for I := 0 to ACount do
    if PtInRect(GetSectionRect(I), Point(X, Y)) and
...
procedure TcxCustomHeader.SetSectionState(AIndex: Integer;
...
begin
  ...
  begin
    ...
    if (AUnderMouseSectionIndex < FSections.Count) and
      (UseRightToLeftAlignment and (APoint.X <= ASectionRect.Left + 7) and
      (APoint.X >= ASectionRect.Left) or not UseRightToLeftAlignment and
      (APoint.X >= ASectionRect.Right - 7) and (APoint.X <= ASectionRect.Right))
    then
      ASizedSectionIndex := AUnderMouseSectionIndex
    else if (AUnderMouseSectionIndex > 0) and
      (UseRightToLeftAlignment and (APoint.X <= ASectionRect.Right) and
      (APoint.X >= ASectionRect.Right - 7) or not UseRightToLeftAlignment and
      (APoint.X >= ASectionRect.Left) and (APoint.X <= ASectionRect.Left + 7))
    then
      ASizedSectionIndex := AUnderMouseSectionIndex - 1;
    if (ASizedSectionIndex > -1) and not Sections[ASizedSectionIndex].AllowResize
    then
      ASizedSectionIndex := -1;
  end;
...
function TcxCustomHeader.GetSectionContentBounds(ASectionBounds: TRect;
  AState: TcxButtonState): TRect;
begin
  Result := LookAndFeelPainter.HeaderControlSectionContentBounds
...
begin
  if UseRightToLeftAlignment then
    ...
    Result := Ord(LookAndFeelPainter.LookAndFeelStyle in [lfsStandard, lfsFlat,
      lfsNative, lfsSkin]);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      TcxHeaderSections(Collection)
        .FHeaderControl.DoSectionEndResizeEvent(Self);
    end;
  end;
end;
...
function TcxHeaderSections.GetSortedSection(AIndexFrom: Integer)
  : TcxHeaderSection;
var
  I: Integer;
begin
