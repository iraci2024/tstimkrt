...
uses
  ...
  cxGraphics, cxLookAndFeels, cxGroupBox, cxClasses, dxLayoutContainer,
  dxLayoutControl,
...
function dxShowAddItemsToCompositionDlg(var AData
  : TdxAddItemsToCompositionDlgData): Boolean;
...
begin
  Result := False;
  if (AData.Composition = nil) or (AData.Items = nil) then
    Exit;
...
procedure TdxfmCompositionAddItems.LoadFromIniFile(AIniFile: TCustomIniFile;
...
begin
  ...
  FSavedHideIncludedItems := AIniFile.ReadBool(ASectionName,
    sdxHideIncludedItems, FSavedHideIncludedItems);
end;

procedure TdxfmCompositionAddItems.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASectionName: string);
begin
  inherited SaveToIniFile(AIniFile, ASectionName);
  dxSaveListViewColumns(AIniFile, ASectionName, lvItems.InnerListView);
  AIniFile.WriteBool(ASectionName, sdxHideIncludedItems,
    chbxHideIncludedItems.Checked);
end;
...
begin
  ...
  chbxHideIncludedItems.Caption :=
    cxGetResourceString(@sdxHideAlreadyIncludedItems);
end;
    ...
    try
      ...
      try
        ...
        if (Composition <> nil) and (Composition.ComponentPrinter <> nil) then
          Composition.ComponentPrinter.GetItems(Composition, Strings,
            chbxHideIncludedItems.Checked);
