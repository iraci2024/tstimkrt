...
uses
  Types, SysUtils, Classes, Windows, Controls, Forms, cxGeometry, cxGraphics,
  dxGDIPlusClasses,
...
type
  ...
  TcxGridLayoutViewCarouselModeRecordPainter = class
  ...
  protected
    ...
    property ViewInfo: TcxGridLayoutViewCarouselModeRecordViewInfo
      read GetViewInfo;
  end;

  { TcxGridLayoutViewCarouselModeRecordsPainter }

  TcxGridLayoutViewCarouselModeRecordsPainter = class
  ...
  protected
    ...
    property ViewInfo: TcxGridLayoutViewCarouselModeRecordsViewInfo
      read GetViewInfo;
  end;

  { TcxGridLayoutViewCarouselModeRecordViewInfoCacheItem }

  TcxGridLayoutViewCarouselModeRecordViewInfoCacheItem = class
  ...
  protected
    ...
    property CacheItem: TcxGridLayoutViewCarouselModeRecordViewInfoCacheItem
      read GetCacheItem;
    property CalculatedBounds: TRect read FCalculatedBounds
      write FCalculatedBounds;
    property CachedBitmap: TdxGPImage read GetCachedBitmap;
    property CachedSelected: Boolean read GetCachedSelected
      write SetCachedSelected;
  ...
  public
    ...
    property RecordsViewInfo: TcxGridLayoutViewCarouselModeRecordsViewInfo
  ...
  private
    ...
    function GetControllerHelper
      : TcxGridLayoutViewControllerCarouselHelper; inline;
    ...
    function PointToTouchPointData(const APoint: TPoint; ASignZ: Integer)
      : TcxTouchPointData;
  ...
  protected
    ...
    procedure CalculateVisibleRecordsBound(out AFirstRecordIndex,
      ALastRecordIndex: Integer); override;
    ...
    function GetRecordViewInfoByRecordIndex(ARecordIndex: Integer)
      : TcxGridLayoutViewCarouselModeRecordViewInfo;
    ...
    function CalculateTouchScrollingAngle(const APrevPoint,
      APoint: TcxTouchPointData): Double;
    procedure EndTouchScrolling;
    procedure Recalculate;

    function CreateGPCanvas(ACanvas: TcxCanvas): TdxGPCanvas;

    property Calculator: TcxGridLayoutViewInfoCarouselModeCalculator
      read GetCalculator;
    property Controller: TcxGridLayoutViewController read GetController;
    property ControllerHelper: TcxGridLayoutViewControllerCarouselHelper
      read GetControllerHelper;
    property IsRadiusCalculating: Boolean read FIsRadiusCalculating;
    property Items[Index: Integer]: TcxGridLayoutViewCarouselModeRecordViewInfo
  ...
  private
    function CalculateAlpha(AViewInfo
      : TcxGridLayoutViewCarouselModeRecordViewInfo): Integer;
    function CalculateAngle(AViewInfo
      : TcxGridLayoutViewCarouselModeRecordViewInfo): Double;
    function CalculateRecordBounds(const AContentBounds: TRect;
      AViewInfo: TcxGridLayoutViewCarouselModeRecordViewInfo): TRect;
    function CalculateScale(AViewInfo
      : TcxGridLayoutViewCarouselModeRecordViewInfo): Double;
    function GetOptions: TcxGridLayoutViewCarouselMode;
    function GetInterpolatedValue(AViewInfo
      : TcxGridLayoutViewCarouselModeRecordViewInfo;
  ...
  public
    constructor Create(AControllerHelper
      : TcxGridLayoutViewControllerCarouselHelper; Angle: Double;
  ...
  protected
    function GetPageRecordCount(AVisibleRecordCount: Integer): Integer;
      override;
    ...
    procedure StartAnimation(AEffect: TdxAnimationTransitionEffect;
      Angle: Double; ATime: Integer);
    procedure StartCarouselAnimation(AItemCountDelta: Integer);
    procedure StartKinematicAnimation;
    procedure StopAnimation;

    property AnimationTransition: TcxGridCarouselModeAnimationTransition
    ...
    property IsInternalTopRecordIndexChanging: Boolean
      read FIsInternalTopRecordIndexChanging;
    property PrevStartRecordIndex: Integer read FPrevStartRecordIndex
      write FPrevStartRecordIndex;
    property RecordsViewInfo: TcxGridLayoutViewCarouselModeRecordsViewInfo
      read GetRecordsViewInfo;
  ...
  public
    constructor Create(AController: TcxGridCustomLayoutViewController);
      override;
    destructor Destroy; override;
...
const
  ...
  cxInvalidTouchPointData: TcxTouchPointData = (X: - 1; Y: - 1; Z: - 1;
    ASignZ: 0);
...
function GetInterpolatedLineValue(ABeginValue, AEndValue,
  Angle: Double): Double;
begin
  Result := ABeginValue + TransformAngle(Angle) * (AEndValue - ABeginValue);
end;

function GetInterpolatedCubicValue(ABeginValue, AEndValue,
  Angle: Double): Double;
begin
...
function GetInternalInterpolatedValue(ABeginValue, AEndValue, ABoundAngle,
  Angle: Double; AUseCubicInterpolatedFunction: Boolean): Double;
var
...
begin
  ...
  begin
    ...
    if AUseCubicInterpolatedFunction then
      Result := GetInterpolatedCubicValue(ABeginValue, AEndValue,
        ACalculatedAngle)
    else
      Result := GetInterpolatedLineValue(ABeginValue, AEndValue,
        ACalculatedAngle);
  end;
end;
...
begin
  Angle1 := RadToDeg(TcxGridLayoutViewCarouselModeRecordViewInfo(AItem1).Angle -
    dxHalfOfPi);
  Angle2 := RadToDeg(TcxGridLayoutViewCarouselModeRecordViewInfo(AItem2).Angle -
    dxHalfOfPi);
...
begin
  Result := Abs(TcxGridLayoutViewCarouselModeRecordViewInfo(AItem2)
    .CarouselIndex) - Abs(TcxGridLayoutViewCarouselModeRecordViewInfo(AItem1)
    .CarouselIndex);
end;
...
function TcxGridLayoutViewCarouselModeRecordPainter.CanDrawExpandButton
  : Boolean;
begin
  Result := not IsPaintBitmap;
end;

function TcxGridLayoutViewCarouselModeRecordPainter.GetViewInfo
  : TcxGridLayoutViewCarouselModeRecordViewInfo;
begin
...
function TcxGridLayoutViewCarouselModeRecordsPainter.GetViewInfo
  : TcxGridLayoutViewCarouselModeRecordsViewInfo;
begin
...
constructor TcxGridLayoutViewCarouselModeRecordViewInfoCacheItem.Create
...
procedure TcxGridLayoutViewCarouselModeRecordViewInfoCacheItem.UnassignValues
  (AKeepMaster: Boolean);
begin
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.HasPoint
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.GetEditViewDataBounds
  (AItem: TcxGridLayoutViewItemViewInfo): TRect;
begin
  if not AItem.Editing or (IsTopRecord and (RecordsViewInfo.OffsetAngle = 0))
  then
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.GetPainterClass
  : TcxCustomGridCellPainterClass;
begin
  Result := TcxGridLayoutViewCarouselModeRecordPainter;
end;

function TcxGridLayoutViewCarouselModeRecordViewInfo.GetPixelScrollSize
  : Integer;
begin
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.
  CanGetChildrenHitTest: Boolean;
begin
  Result := IsTopRecord and (RecordsViewInfo.OffsetAngle = 0);
end;

function TcxGridLayoutViewCarouselModeRecordViewInfo.
  CanShowGroupScrollBars: Boolean;
begin
  Result := inherited CanShowGroupScrollBars and IsTopRecord and
...
var
  ABitmap: TcxBitmap;
begin
  if not IsVisibleForPainting or
    (not CachedBitmap.Empty and (Selected = CachedSelected) and
    cxRectIsEqual(CachedBitmap.ClientRect, cxRectOffset(Bounds, Bounds.TopLeft,
    False))) then
    Exit;
  ...
  try
    ...
    try
      ABitmap.cxCanvas.UseRightToLeftAlignment :=
        Canvas.UseRightToLeftAlignment;
...
begin
  ...
  try
    ACanvas.Draw(CachedBitmap, CalculatedBounds,
      CachedBitmap.ClientRect, Alpha);
  finally
    ACanvas.Free;
  end;
end;

function TcxGridLayoutViewCarouselModeRecordViewInfo.HasItemFocusRect
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.GetCacheItem
  : TcxGridLayoutViewCarouselModeRecordViewInfoCacheItem;
begin
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.GetCachedBitmap
  : TdxGPImage;
begin
...
function TcxGridLayoutViewCarouselModeRecordViewInfo.GetRecordsViewInfo
  : TcxGridLayoutViewCarouselModeRecordsViewInfo;
begin
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.CanOffset(AItemCountDelta
  : Integer): Boolean;
begin
  Result := True;
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetHitTest
...
begin
  ...
  try
    ...
    begin
      Result := TcxGridLayoutViewCarouselModeRecordViewInfo(AList[I])
        .GetHitTest(P);
      if Result <> nil then
        Break;
    end;
...
procedure TcxGridLayoutViewCarouselModeRecordsViewInfo.OffsetRecords
  (AItemCountDelta, APixelScrollRecordOffsetDelta: Integer);
begin
  if (AItemCountDelta <> 0) and not ControllerHelper.IsInternalTopRecordIndexChanging
  then
  begin
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.AllowPan
...
procedure TcxGridLayoutViewCarouselModeRecordsViewInfo.
  CalculateVisibleRecordsBound(out AFirstRecordIndex, ALastRecordIndex
  : Integer);
var
  ACount: Integer;
...
begin
  ...
  ALastRecordIndex := Min(ViewData.RecordCount - 1, ATopRecordIndex + ACount - 1
    - (ACount - 1) div 2);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetCalculatorClass
  : TcxGridCustomLayoutRecordsViewInfoBasedCalculatorClass;
begin
  Result := TcxGridLayoutViewInfoCarouselModeCalculator;
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.
  GetCarouselViewInfos: TList;
var
  I: Integer;
begin
  Result := TList.Create;
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetItemViewInfoClass
...
begin
  ...
  Dec(Result.Y, TcxGridLayoutViewControllerAccess(Controller)
    .ContentScrollBarPosition);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetPainterClass
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.
  GetRecordViewInfoByRecordIndex(ARecordIndex: Integer)
  : TcxGridLayoutViewCarouselModeRecordViewInfo;
var
...
begin
  ...
  begin
    Result := TcxGridLayoutViewCarouselModeRecordViewInfo
      (CreateRecordViewInfo(ARecord));
    AddRecordViewInfo(Result);
  end;
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.
  GetScrollableAreaBoundsForEdit: TRect;
var
  I: Integer;
begin
...
procedure TcxGridLayoutViewCarouselModeRecordsViewInfo.InternalSetOffsetAngle
  (const Value: Double);
begin
  FOffsetAngle := Value;
end;
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.IsRecordVisible
  (ARecord: TcxGridLayoutViewRecord): Boolean;
var
  AMax, AMin: Integer;
begin
  CalculateVisibleRecordsBound(AMin, AMax);
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.CalculateAnimationInterval
  (Angle: Double): Cardinal;
begin
  Result := Options.AnimationInterval;
  if Abs(Angle) < Options.DeltaAngle then
    Result := Trunc(Result * Abs(Angle) / Options.DeltaAngle)
  else if Abs(Angle) > Options.DeltaAngle then
    Result := Result + Trunc((Abs(Angle) / Options.DeltaAngle - 1) *
      Options.AnimationInterval / 4);
...
begin
  ...
  try
    ...
    if Sin(ARollAngle) = 0 then
      FRadius := AWidth
    else if Cos(ARollAngle) = 0 then
      FRadius := AHeight
    else
      FRadius := Min(Abs(AWidth / Cos(ARollAngle)),
        Abs(AHeight / Sin(ARollAngle)));
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.
  CalculateTouchScrollingAngle(const APrevPoint,
  APoint: TcxTouchPointData): Double;
...
var
  ...
  ASign: Integer;
begin
  ASum := APrevPoint.X * APoint.X + APrevPoint.Y * APoint.Y + APrevPoint.Z
    * APoint.Z;
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.CreateGPCanvas
  (ACanvas: TcxCanvas): TdxGPCanvas;
begin
...
procedure TcxGridLayoutViewCarouselModeRecordsViewInfo.DoRightToLeftConversion
  (const ABounds: TRect);
begin
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.
  GetAdjustedContentSize: TSize;
begin
  Result := GetRecordSize;
  Result.cx := Trunc(cxRectWidth(ContentBounds) - Result.cx -
    Options.BackgroundRecordEndScale / 100 * Result.cx);
  Result.cy := Trunc(cxRectHeight(ContentBounds) - Result.cy -
    Options.BackgroundRecordEndScale / 100 * Result.cy);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.
  GetRotatedContentSize: TSize;
var
  ASize: TSize;
  ARollAngle: Extended;
begin
...
function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetCalculator
  : TcxGridLayoutViewInfoCarouselModeCalculator;
begin
  Result := TcxGridLayoutViewInfoCarouselModeCalculator(inherited Calculator);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetController
  : TcxGridLayoutViewController;
begin
  Result := TcxGridLayoutViewController(inherited Controller);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetControllerHelper
  : TcxGridLayoutViewControllerCarouselHelper;
begin
  Result := TcxGridLayoutViewControllerCarouselHelper
    (inherited ControllerHelper);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetItem(Index: Integer)
  : TcxGridLayoutViewCarouselModeRecordViewInfo;
begin
  Result := TcxGridLayoutViewCarouselModeRecordViewInfo(inherited Items[Index]);
end;

function TcxGridLayoutViewCarouselModeRecordsViewInfo.GetOptions
  ...
  begin
    ...
    try
      ...
      begin
        ...
        R := cxRectInflate(AViewInfo.CalculatedBounds, Mouse.DragThreshold,
          Mouse.DragThreshold);
...
begin
  ...
  begin
    ...
    begin
      AIsForegroundRecordTracking := GetTrackingCarouselIndex
        (ATrackingCarouselIndex) and
        (Abs(ATrackingCarouselIndex) <= Options.RecordCount div 4);
      if (Cos(PitchAngle) > 0) <>
        ((AIsForegroundRecordTracking and (Cos(PitchAngle) < 0)) or
        (not AIsForegroundRecordTracking and (Ay > 0))) then
        Az := -Az;
    end
    else
      Az := ASignZ * Az;
...
begin
  ...
  begin
    ...
    if ACanCheckTopRecordIndex and (Abs(Value - Options.DeltaAngle * ADelta) >
      Options.DeltaAngle / 2) then
      Inc(ADelta, Sign(Value));
    if (ControllerHelper.InternalTopRecordIndex - ADelta >= 0) and
      (ControllerHelper.InternalTopRecordIndex - ADelta < ViewData.RecordCount)
    then
    begin
      ...
      begin
        ...
        begin
          ...
          ControllerHelper.InternalTopRecordIndex :=
            ControllerHelper.InternalTopRecordIndex - ADelta;
        end;
...
procedure TcxGridLayoutViewInfoCarouselModeCalculator.DoCalculate
...
begin
  ...
  begin
    ...
    begin
      ...
      ARecordViewInfo.CalculatedBounds := CalculateRecordBounds(AContentBounds,
        ARecordViewInfo);
      ARecordViewInfo.CalculationPosition :=
        ARecordViewInfo.CalculatedBounds.TopLeft;
    end;
  end;
...
function TcxGridLayoutViewInfoCarouselModeCalculator.CalculateAlpha
  (AViewInfo: TcxGridLayoutViewCarouselModeRecordViewInfo): Integer;
begin
  Result := Trunc(GetInterpolatedValue(AViewInfo, 255,
    Options.BackgroundRecordAlphaLevel, False));
end;

function TcxGridLayoutViewInfoCarouselModeCalculator.CalculateAngle
  (AViewInfo: TcxGridLayoutViewCarouselModeRecordViewInfo): Double;
begin
  Result := dxHalfOfPi + AViewInfo.CarouselIndex * Options.DeltaAngle +
    RecordsViewInfo.OffsetAngle;
end;

function TcxGridLayoutViewInfoCarouselModeCalculator.CalculateRecordBounds
  (const AContentBounds: TRect;
  AViewInfo: TcxGridLayoutViewCarouselModeRecordViewInfo): TRect;
var
  AScale: Double;
...
begin
  ...
  xAxis := dxPointF(Cos(RecordsViewInfo.RollAngle),
    Sin(RecordsViewInfo.RollAngle));
...
function TcxGridLayoutViewInfoCarouselModeCalculator.CalculateScale
  (AViewInfo: TcxGridLayoutViewCarouselModeRecordViewInfo): Double;
begin
  Result := GetInterpolatedValue(AViewInfo, 1, 1, True)
end;

function TcxGridLayoutViewInfoCarouselModeCalculator.GetOptions
  : TcxGridLayoutViewCarouselMode;
begin
...
begin
  ...
  if (Abs(Angle - dxHalfOfPi) < ABoundAngle) and
    RecordsViewInfo.ControllerHelper.IsForegroundRecord(AViewInfo.GridRecord.
    Index) then
  begin
    P1 := dxPointF(dxHalfOfPi - ABoundAngle,
      GetInternalInterpolatedValue(GetScale(Options.BackgroundRecordStartScale)
      * ABeginValue, GetScale(Options.BackgroundRecordEndScale) * AEndValue,
      ABoundAngle, dxHalfOfPi - ABoundAngle, not AUseRecordScale));
    P2 := dxPointF(dxHalfOfPi, ABeginValue);
    Result := P1.Y + (Angle - P1.X) * (P2.Y - P1.Y) / (P2.X - P1.X);
  end
  else
    Result := GetInternalInterpolatedValue
      (GetScale(Options.BackgroundRecordStartScale) * ABeginValue,
      GetScale(Options.BackgroundRecordEndScale) * AEndValue, ABoundAngle,
      Angle, not AUseRecordScale);
end;

function TcxGridLayoutViewInfoCarouselModeCalculator.GetRecordsViewInfo
  : TcxGridLayoutViewCarouselModeRecordsViewInfo;
begin
  Result := TcxGridLayoutViewCarouselModeRecordsViewInfo
    (inherited RecordsViewInfo);
...
constructor TcxGridCarouselModeAnimationTransition.Create(AControllerHelper
  : TcxGridLayoutViewControllerCarouselHelper; Angle: Double; ATime: Cardinal;
  AEffect: TdxAnimationTransitionEffect);
begin
...
function TcxGridCarouselModeAnimationTransition.PositionToAngle
...
function TcxGridCarouselModeAnimationTransition.GetRecordsViewInfo
  : TcxGridLayoutViewCarouselModeRecordsViewInfo;
begin
...
constructor TcxGridLayoutViewControllerCarouselHelper.Create
...
procedure TcxGridLayoutViewControllerCarouselHelper.MouseDown
...
procedure TcxGridLayoutViewControllerCarouselHelper.MouseMove
...
procedure TcxGridLayoutViewControllerCarouselHelper.MouseUp
  (Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
function TcxGridLayoutViewControllerCarouselHelper.GetPageRecordCount
  (AVisibleRecordCount: Integer): Integer;
begin
  Result := Trunc(RecordsViewInfo.Options.RecordCount / 2 + 0.5);
end;

function TcxGridLayoutViewControllerCarouselHelper.IsForegroundRecord
  (AIndex: Integer): Boolean;
begin
  Result := (AIndex = InternalTopRecordIndex) or
    (AIndex = PrevStartRecordIndex);
end;
...
begin
  if RecordsViewInfo.OffsetAngle = 0 then
    ...
    FPrevStartRecordIndex := InternalTopRecordIndex -
      Sign(RecordsViewInfo.OffsetAngle);
end;

procedure TcxGridLayoutViewControllerCarouselHelper.BeginTouchScrolling
  (Shift: TShiftState; const P: TPoint);
begin
  if IsTouchScrolling(Shift) and (Length(FKineticDataPoints) = 0) then
  begin
...
function TcxGridLayoutViewControllerCarouselHelper.IsTouchScrolling
...
procedure TcxGridLayoutViewControllerCarouselHelper.TouchScrolling
  (const P: TPoint);
begin
...
procedure TcxGridLayoutViewControllerCarouselHelper.StartAnimation
  (AEffect: TdxAnimationTransitionEffect; Angle: Double; ATime: Integer);
begin
  StopAnimation;
  FAnimationTransition := TcxGridCarouselModeAnimationTransition.Create(Self,
    Angle, ATime, AEffect);
  FAnimationTransition.Resume;
end;

procedure TcxGridLayoutViewControllerCarouselHelper.StartCarouselAnimation
...
begin
  if AItemCountDelta = 0 then
    Exit;
  Angle := RecordsViewInfo.OffsetAngle + AItemCountDelta *
    RecordsViewInfo.Options.DeltaAngle;
...
begin
  ...
  ADelta := Min(ADelta, RecordsViewInfo.ViewData.RecordCount - 1 -
    InternalTopRecordIndex);

  Angle := -ADelta * RecordsViewInfo.Options.DeltaAngle -
    RecordsViewInfo.OffsetAngle;
...
procedure TcxGridLayoutViewControllerCarouselHelper.AddKineticDataPoint
  (Angle: Double);
begin
  if Length(FKineticDataPoints) >= 10 then
    Move(FKineticDataPoints[1], FKineticDataPoints[0],
      Length(FKineticDataPoints) - 1)
  else
...
function TcxGridLayoutViewControllerCarouselHelper.
  GetInternalTopRecordIndex: Integer;
begin
  Result := TcxGridLayoutViewControllerAccess(Controller)
    .InternalTopRecordIndex;
end;

function TcxGridLayoutViewControllerCarouselHelper.GetRecordsViewInfo
  : TcxGridLayoutViewCarouselModeRecordsViewInfo;
begin
  Result := TcxGridLayoutViewCarouselModeRecordsViewInfo
    (ViewInfo.RecordsViewInfo);
end;

procedure TcxGridLayoutViewControllerCarouselHelper.SetInternalTopRecordIndex
  (Value: Integer);
begin
  FIsInternalTopRecordIndexChanging := True;
  try
    TcxGridLayoutViewControllerAccess(Controller)
...
procedure TcxGridLayoutViewControllerCarouselHelper.SetTouchPoint
  (const Value: TcxTouchPointData);
var
  Angle: Double;
begin
