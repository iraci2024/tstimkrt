...
type
  ...
  TdxCustomControlHelper = class(TcxIUnknownObject, IdxBatchUpdatable,
  ...
  protected
    ...
    procedure EditControlKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
constructor TdxCustomListBoxHelper.Create(AEditControl: TcxTextEdit;
...
procedure TdxCustomListBoxHelper.EditControlKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);

  function ItemsInPage: Integer;
  begin
    Result := IfThen(FListControl.ItemHeight > 0,
      FListControl.Height div FListControl.ItemHeight - 1, 1);
  end;
  ...
  case Key of
    vkPrior:
      FListControl.ItemIndex := IfThen(FListControl.ItemIndex >= 0,
        Max(FListControl.ItemIndex - ItemsInPage, 0),
        Min(FNearestIndex, FListControl.Count - 1));
    vkNext:
      FListControl.ItemIndex := IfThen(FListControl.ItemIndex >= 0,
        Min(FListControl.ItemIndex + ItemsInPage, FListControl.Count - 1),
        Min(FNearestIndex, FListControl.Count - 1));
    vkUp:
      FListControl.ItemIndex := IfThen(FListControl.ItemIndex >= 0,
        Max(FListControl.ItemIndex - 1, 0),
        Min(FNearestIndex, FListControl.Count - 1));
    vkDown:
      FListControl.ItemIndex := IfThen(FListControl.ItemIndex >= 0,
        Min(FListControl.ItemIndex + 1, FListControl.Count - 1),
        Min(FNearestIndex, FListControl.Count - 1));
  else
    Exit;
  end;
