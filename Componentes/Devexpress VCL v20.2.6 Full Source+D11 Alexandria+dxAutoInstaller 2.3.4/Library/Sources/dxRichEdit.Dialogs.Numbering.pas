...
uses
  ...
  cxButtons, dxLayoutContainer, cxGroupBox, cxRadioGroup, dxLayoutControl,
  cxListBox, cxClasses, dxContainerListBox,
...
type
  TdxRichEditNumberingListDialogForm = class(TdxRichEditCustomDialogForm,
    ...
    procedure lcgTabControlTabChanging(Sender: TObject; ANewTabIndex: Integer;
      var Allow: Boolean);
  ...
  protected
    ...
    function ChangeSelectedTabPage(ACurrentSelectedTabPage: TdxLayoutGroup)
      : TdxLayoutGroup;
    function CreateController(AControllerParameters
      : TdxFormControllerParameters): TdxFormController; override;
    function GetNumberingListType(AList: TdxAbstractNumberingList)
      : TdxNumberingType;
  ...
  public
    ...
    procedure ShowBulletedListForm(ASource: TdxAbstractNumberingList;
      ALevels: TdxListLevelCollection);
    procedure ShowSimpleNumberingListForm(ASource: TdxAbstractNumberingList;
      ALevels: TdxListLevelCollection);
    procedure ShowMultiLevelNumberingListForm(ASource: TdxAbstractNumberingList;
      ALevels: TdxListLevelCollection);
...
function TdxRichEditNumberingListDialogForm.GetActiveGallery
  : TdxNumberingListBoxHelper;
begin
  Result := TdxNumberingListBoxHelper
    (lcgTabControl.Items[lcgTabControl.ItemIndex].Data);
end;
...
function TdxRichEditNumberingListDialogForm.GetController
  : TdxNumberingListFormController;
begin
  Result := TdxNumberingListFormController(inherited Controller);
end;

function TdxRichEditNumberingListDialogForm.GetInitialTabPage
  (AList: TdxAbstractNumberingList): TdxLayoutGroup;
begin
...
function TdxRichEditNumberingListDialogForm.GetNumberingListType
  (AList: TdxAbstractNumberingList): TdxNumberingType;
begin
...
procedure TdxRichEditNumberingListDialogForm.InitializeTabs;
  ...
  begin
    ANumberingOptions := Control.InnerControl.Options.
    ...
    FBulletedListBoxHelper := TdxNumberingListBoxHelper.Create(Control,
      Controller);
    FBulletedListBoxHelper.InitializeControl(lbBulleted,
      TdxNumberingType.Bullet, Controller.LevelIndex);
    lcgBulleted.Data := FBulletedListBoxHelper;
    FNumberingListBoxHelper := TdxNumberingListBoxHelper.Create(Control,
      Controller);
    FNumberingListBoxHelper.InitializeControl(lbNumbered,
      TdxNumberingType.Simple, Controller.LevelIndex);
    lcgNumbered.Data := FNumberingListBoxHelper;
    FOutlineNumberedListBoxHelper := TdxNumberingListBoxHelper.Create(Control,
      Controller);
    FOutlineNumberedListBoxHelper.InitializeControl(lbOutlineNumbered,
      TdxNumberingType.MultiLevel, Controller.LevelIndex);
...
function TdxRichEditNumberingListDialogForm.
  IsOutlineNumberedTabSelected: Boolean;
begin
  Result := lcgTabControl.ItemIndex = lcgOutlineNumbered.Tag;
end;

procedure TdxRichEditNumberingListDialogForm.lcgTabControlTabChanged
  (Sender: TObject);
var
...
begin
  AIndexOfParagraphInListBeforeCurrent :=
    FNumberingListFormHelper.GetIndexOfParagraphInListBeforeCurrent();
  AIsParagraphValid := AIndexOfParagraphInListBeforeCurrent >=
    FNumberingListFormHelper.ParagraphIndexMinValue;
  ...
  SubscribeGalleryEvents(TdxNumberingListBoxHelper(lcgTabControl.Items
    [lcgTabControl.ItemIndex].Data));
end;

procedure TdxRichEditNumberingListDialogForm.lcgTabControlTabChanging
  (Sender: TObject; ANewTabIndex: Integer; var Allow: Boolean);
var
  AGallery, APrevGallery: TdxNumberingListBoxHelper;
  AMaxIndex: Integer;
begin
  AGallery := TdxNumberingListBoxHelper(lcgTabControl.Items[ANewTabIndex].Data);
  APrevGallery := TdxNumberingListBoxHelper
    (lcgTabControl.Items[lcgTabControl.ItemIndex].Data);
...
procedure TdxRichEditNumberingListDialogForm.OnActiveGalleryMouseDoubleClick
  (Sender: TObject);
begin
...
procedure TdxRichEditNumberingListDialogForm.OnSelectedIndexChanged
  (Sender: TObject);
begin
  btnCustomize.Enabled := TdxCustomContainerListBox(Sender).SelectedIndex > 0;
end;

procedure TdxRichEditNumberingListDialogForm.SetContinuePreviousList
  (const Value: Boolean);
begin
  rbContinuePreviousList.Checked := Value;
end;

procedure TdxRichEditNumberingListDialogForm.SetRestartNumbering
  (const Value: Boolean);
begin
  rbRestartNumbering.Checked := Value;
end;

procedure TdxRichEditNumberingListDialogForm.ShowBulletedListForm
  (ASource: TdxAbstractNumberingList; ALevels: TdxListLevelCollection);
var
  AForm: TdxRichEditBulletedListDialogForm;
  AControllerParameters: TdxBulletedListFormControllerParameters;
begin
  AControllerParameters := TdxBulletedListFormControllerParameters.Create
    (Control, ALevels, Controller.LevelIndex);
  try
...
procedure TdxRichEditNumberingListDialogForm.ShowMultiLevelNumberingListForm
  (ASource: TdxAbstractNumberingList; ALevels: TdxListLevelCollection);
begin
  FNumberingListFormHelper.ShowNumberingListFormCore(ASource, ALevels, True);
end;

procedure TdxRichEditNumberingListDialogForm.ShowSimpleNumberingListForm
  (ASource: TdxAbstractNumberingList; ALevels: TdxListLevelCollection);
begin
  FNumberingListFormHelper.ShowNumberingListFormCore(ASource, ALevels, False);
end;

procedure TdxRichEditNumberingListDialogForm.SubscribeGalleryEvents
  (AGallery: TdxNumberingListBoxHelper);
begin
...
procedure TdxRichEditNumberingListDialogForm.UnSubscribeGalleryEvents
  (AGallery: TdxNumberingListBoxHelper);
begin
...
begin
  ...
  rbRestartNumbering.Caption := cxGetResourceString
    (@sdxRichEditNumberingListDialogRestartNumbering);
  rbContinuePreviousList.Caption :=
    cxGetResourceString(@sdxRichEditNumberingListDialogContinuePreviousList);
  btnCustomize.Caption := cxGetResourceString
  ...
  lcgBulleted.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditNumberingListDialogBulleted);
  lcgNumbered.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditNumberingListDialogNumbered);
  lcgOutlineNumbered.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditNumberingListDialogOutlineNumbered);
end;
...
begin
  Result := TdxNumberingListFormController.Create
    (AControllerParameters as TdxNumberingListFormControllerParameters);
end;

function TdxRichEditNumberingListDialogForm.CreateNumberingListFormCore
  (AListLevels: TdxListLevelCollection; AFirstParagraphStartIndex: Integer;
...
begin
  ...
  AParameters := TdxSimpleNumberingListFormControllerParameters.Create(Control,
    AListLevels, Controller.LevelIndex, AFirstParagraphStartIndex);
  try
...
begin
  AIndexOfParagraphInListBeforeCurrent :=
    FNumberingListFormHelper.GetIndexOfParagraphInListBeforeCurrent();
  AAreNumberingListsDifferent :=
    FNumberingListFormHelper.IsParagraphInDifferentListFromCurrent
    (AIndexOfParagraphInListBeforeCurrent);
...
begin
  ...
  try
    AClone := Controller.CreateAbstractNumberingListCopy(ATempDocumentModel,
      ActiveGallery.SelectedAbstractList);
    AClone.SetId(AClone.GenerateNewId);
    if IsBulletedTabSelected then
      ShowBulletedListForm(AClone, AClone.Levels)
    else if IsNumberedTabSelected then
...
begin
  ...
  else if ACurrentSelectedTabPage = lcgNumbered then
  begin
  ...
  else if ACurrentSelectedTabPage = lcgOutlineNumbered then
  begin
    if lcgBulleted.Enabled then
      Result := lcgBulleted
    else
