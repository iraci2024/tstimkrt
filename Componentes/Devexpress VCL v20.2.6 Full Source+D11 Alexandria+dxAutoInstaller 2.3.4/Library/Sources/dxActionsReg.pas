...
uses
  Types, SysUtils, Classes, Graphics, ActnList, dxActions, cxGraphics,
  cxControls, dxUIGenerator,
{$IFDEF DELPHIXE3}
...
{$ENDIF}
  ...
  public
    ...
    class function CreateImageList(ActionList: TContainedActionList)
    ...
    class procedure RegisterImageLink(const ActionList: TContainedActionList;
      const ImageLink: TdxIDEActionsImageLink); override;
...
class procedure TdxIDEActions.CopyImageIfAvailable(const NewAction
  : TContainedAction; const ActionList: TContainedActionList);
var
  ...
  begin
    ...
    if AAction.FImage = nil then
      AAction.ImageIndex := TdxUIGeneratorHelper.AddImage(AImages,
        AAction.FDefaultImageNameInIconLibrary,
        TdxUIGeneratorAdapter.DefaultImageSet)
    else if (AAction.FImage is TcxBitmap32) and (AImages is TcxImageList) then
      try
        AAction.ImageIndex := TcxImageList(AImages)
          .Add(TBitmap(AAction.FImage), nil)
      finally
...
class function TdxIDEActions.CreateImageList(ActionList: TContainedActionList)
  : TCustomImageList;
begin
...
class procedure TdxIDEActions.RegisterImageLink(const ActionList
  : TContainedActionList; const ImageLink: TdxIDEActionsImageLink);
begin
  OldIDEActionsClass.RegisterImageLink(ActionList, ImageLink);
end;

class procedure TdxIDEActions.UnregisterImageLink(const ActionList
  : TContainedActionList; const ImageLink: TdxIDEActionsImageLink);
begin
...
begin
  ...
  if (TdxIDEActions.OldIDEActionsClass <> nil) and
    (TdxIDEActions.OldIDEActionsClass <> TdxIDEActions) then
  begin
