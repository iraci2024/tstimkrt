...
type
  TdxPSInsertMacrosEvent = procedure(Sender: TObject;
    const AMacrosValue: string) of object;
  ...
  protected
    ...
    function CreateButton(AWidth, ATag: Integer; AClickEvent: TNotifyEvent)
      : TcxButton;
    procedure CreateAutoTextMenuButton(AMenuButtonWidth: Integer;
      AAutoHFTextEntries: TStrings);
    //
    ...
    property MoreFunctionsPopupMenuImages: TcxImageList
      read FMoreFunctionsPopupMenuImages;
  ...
  public
    ...
    //
    property ButtonsImageList: TcxImageList read FButtonsImageList
      write FButtonsImageList;
    //
    property OnAutoTextClick: TNotifyEvent read FOnAutoTextClick
      write FOnAutoTextClick;
    property OnInsertMacros: TdxPSInsertMacrosEvent read FOnInsertMacros
      write FOnInsertMacros;
  end;
...
type
  ...
  TdxPSHFImagePopupSelectEvent = procedure(Sender: TObject; AItemIndex: Integer)
    of object;
  ...
  public
    ...
    property OnSelect: TdxPSHFImagePopupSelectEvent read FOnSelect
      write FOnSelect;
  end;
...
function TdxPSHFImagePopup.CreateButton(const ACaption: string; ATag: Integer)
  : TcxButton;
begin
...
procedure TdxPSHFImagePopup.DoDrawItem(AControl: TcxListBox; ACanvas: TcxCanvas;
  AIndex: Integer; ARect: TRect; AState: TOwnerDrawState);
var
  AImage: TGraphic;
begin
  ...
  begin
    ...
    ARect := cxRectSetSize(ARect, ScaleFactor.Apply(AImage.Width),
      ScaleFactor.Apply(AImage.Height));
...
procedure TdxPSHFImagePopup.DoMeasureItem(AControl: TcxListBox; AIndex: Integer;
  var Height: Integer);
begin
  if Assigned(ImageCollection) then
    Height := ScaleFactor.Apply(ImageCollection.Items[AIndex]
      .Picture.Height) + 4;
end;
...
procedure TdxPSHFImagePopup.SetupPopupWindow;
  ...
  begin
    ...
    if PopupWindow.ViewInfo.SizeGripCorner in [coTopRight, coBottomRight] then
      Result := cxRectSetRight(Result, PopupWindow.Width -
...
begin
  ...
  FCancelButton.Top := (R.Bottom + R.Top - FCancelButton.Height) div 2 -
    ScaleFactor.Apply(1);
...
procedure TdxPSHFToolBarHelper.Build(AFunctionList, AAutoTextEntries: TStrings;
  ...
  begin
    AButton := CreateButton(ScaleFactor.Apply(ToolBarButtonWidth),
      MakeTTag(AIndex), DoButtonClick);
  ...
  begin
    ...
    AButton := CreateButton(ScaleFactor.Apply(ToolBarMoreButtonWidth),
      MakeTTag(MoreFunctionsPopupMenu), DoShowPopup);
  ...
  begin
    ...
    AIndex := 0;
    ARequiredWidth := ScaleFactor.Apply(ToolBarMoreButtonWidth) +
      ScaleFactor.Apply(ToolBarButtonWidth) + ScaleFactor.Apply
      (ToolBarButtonOffset);
    ...
    begin
      ...
      begin
        if (ARestSpace < ScaleFactor.Apply(ToolBarButtonWidth)) or
          (AIndex < FunctionsList.Count - 1) then
          Break;
      end;
...
begin
  AFont := TFont.Create;
  try
    Result := cxTextWidth(AFont, AutoTextButtonCaption) +
      ScaleFactor.Apply(DropDownGlyphSize) + 4 * ScaleFactor.Apply(cxTextSpace);
  finally
    AFont.Free;
  end;
end;
...
begin
  if FButtons.Count > 0 then
    Result := TcxButton(FButtons.Last).BoundsRect.Right +
      ScaleFactor.Apply(ToolBarButtonOffset)
  else
    Result := 0;
end;
...
procedure TdxPSHFToolBarHelper.CreateAutoTextMenuButton(AMenuButtonWidth
  : Integer; AAutoHFTextEntries: TStrings);
var
...
function TdxPSHFToolBarHelper.CreateButton(AWidth, ATag: Integer;
  AClickEvent: TNotifyEvent): TcxButton;
var
  AOffset: Integer;
begin
...
begin
  if Assigned(OnAutoTextClick) then
    OnAutoTextClick(Sender);
end;
...
procedure TdxPSHFToolBarHelper.DoImageItemClick(Sender: TObject;
  AItemIndex: Integer);
begin
  DoInsertMacros(Format('[%s=%d]',
    [cxGetResourceString(@sdxHFFunctionNameImage), AItemIndex]));
end;
...
begin
  if Assigned(OnInsertMacros) then
    OnInsertMacros(Self, AValue);
end;
...
begin
  ...
  begin
    with GetMouseCursorPos do
      dxPSPopupMenuController.ShowPopup(X, Y, AButton,
        TPopupMenu(TTagToObj(AButton.Tag)));
  end;
end;
...
var
  ABitmap: TcxBitmap;
begin
  ABitmap := TcxBitmap.CreateSize(ScaleFactor.Apply(12),
    ScaleFactor.Apply(12), pf24bit);
  try
    ABitmap.cxCanvas.FillRect(ABitmap.ClientRect, clFuchsia);
    cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat)
      .DrawArrow(ABitmap.cxCanvas, ABitmap.ClientRect, adDown, clWindowText);
