...
const
  ...
  SmallFontStylesMap: array [Boolean, Boolean] of TFontStyles =
    (([], [fsItalic]), ([fsBold], [fsBold, fsItalic]));
...
type
  ...
  TdxRichEditFormattingMarkVisibility = (Auto, Visible, Hidden);
  ...
  TdxHeightUnitType = (Minimum = TdxHeightUnitTypeValues.Minimum,
    Auto = TdxHeightUnitTypeValues.Auto, Exact = TdxHeightUnitTypeValues.Exact);
  ...
  TdxWidthUnitType = (&Nil = TdxWidthUnitTypeValues.&Nil,
  ...
  TdxPageGenerationStrategyType = (RunningHeight, FirstPageOffset);

  TdxDocumentLayoutResetType = (None, SecondaryLayout, AllPrimaryLayout,
    PrimaryLayoutFormPosition);

  TdxSectionPageChangeType = (None, Width, Height, Landscape, PaperKind,
    BatchUpdate);

  TdxSectionMarginsChangeType = (None, Left, Right, Top, Bottom, Gutter,
    GutterAlignment, HeaderOffset, FooterOffset, BatchUpdate);

  TdxSectionGeneralSettingsChangeType = (None, OnlyAllowEditingOfFormFields,
    DifferentFirstPage, FirstPagePaperSource, OtherPagePaperSource,
    TextDirection, VerticalTextAlignment, StartType, BatchUpdate);

  TdxCompleteFormattingResult = (Success, OrphanedFloatingObjects);

  TdxSectionPageNumberingChangeType = (None, ChapterSeparator,
    ChapterHeaderStyle, NumberingFormat, StartingPageNumber, BatchUpdate);

  TdxSectionLineNumberingChangeType = (None, Distance, StartingLineNumber, Step,
    NumberingRestartType, BatchUpdate);

  TdxParagraphNumerationCopyOptions = (CopyAlways, CopyIfWholeSelected);

  TdxDocumentModelChangeType = (None, InsertParagraph, InsertText,
    InsertInlinePicture, InsertInlineCustomObject, InsertFloatingObjectAnchor,
    InsertSection, ModifySection, CreateEmptyDocument, LoadNewDocument,
    DeleteContent, JoinRun, SplitRun, ToggleFieldCodes,
    ToggleFieldLocked, Fields);
  ...
  TdxFootNotePosition = (BottomOfPage, BelowText, EndOfDocument, EndOfSection);

  TdxSectionFootNoteChangeType = (None, Position, NumberingFormat,
    NumberingRestartType, StartingNumber, CustomMark, BatchUpdate);

  TdxSectionColumnsChangeType = (None, EqualWidthColumns, DrawVerticalSeparator,
    Space, ColumnCount, BatchUpdate);

  TdxCornerViewInfoType = (Normal, OuterHorizontalStart, OuterVerticalStart,
    OuterHorizontalEnd, OuterVerticalEnd, InnerTopLeft, InnerTopMiddle,
    InnerTopRight, InnerLeftMiddle, InnerRightMiddle, InnerBottomLeft,
    InnerBottomMiddle, InnerBottomRight, InnerNormal);

  TdxRowProcessingFlag = (ProcessCharacterLines, ProcessHiddenText,
    ProcessTextHighlight, ProcessSpecialTextBoxes, ProcessLayoutDependentText,
    FirstParagraphRow, LastParagraphRow, ContainsFootNotes, ContainsEndNotes,
    LastInvisibleEmptyCellRowAfterNestedTable);

  TdxRowProcessingFlags = set of TdxRowProcessingFlag;

  TdxTextBoxPropertiesChangeType = (None, LeftMargin, RightMargin, TopMargin,
    BottomMargin, ResizeShapeToFitText, WrapText, VerticalAlignment, Upright,
    BatchUpdate);

  TdxTableGridIntervalType = (PercentBased, ModelUnit, NotSet);

  TdxTableChangeType = (None, TableStyle, BatchUpdate);

  TdxCanFitCurrentRowToColumnResult = (RowFitted, PlainRowNotFitted,
    FirstCellRowNotFitted, TextAreasRecreated, RestartDueFloatingObject);

  TdxSectionPropertiesApplyType = (WholeDocument, CurrentSection,
    SelectedSections, ThisPointForward);
  TdxSectionPropertiesApplyTypes = set of TdxSectionPropertiesApplyType;

  TdxPageSetupFormInitialTabPage = (Margins = 0, Paper = 1, Layout = 2);
  ...
  TdxBorderLineStyle = (&Nil = TdxBorderLineStyleValues.&Nil,
    ...
    Inset = TdxBorderLineStyleValues.Inset, Apples, ArchedScallops,
    BabyPacifier, BabyRattle, Balloons3Colors, BalloonsHotAir, BasicBlackDashes,
    BasicBlackDots, BasicBlackSquares, BasicThinLines, BasicWhiteDashes,
    BasicWhiteDots, BasicWhiteSquares, BasicWideInline, BasicWideMidline,
    BasicWideOutline, Bats, Birds, BirdsFlight, Cabins, CakeSlice, CandyCorn,
    CelticKnotwork, CertificateBanner, ChainLink, ChampagneBottle,
    CheckedBarBlack, CheckedBarColor, Checkered, ChristmasTree, CirclesLines,
    CirclesRectangles, ClassicalWave, Clocks, Compass, Confetti, ConfettiGrays,
    ConfettiOutline, ConfettiStreamers, ConfettiWhite, CornerTriangles,
    CouponCutoutDashes, CouponCutoutDots, CrazyMaze, CreaturesButterfly,
    CreaturesFish, CreaturesInsects, CreaturesLadyBug, CrossStitch, Cup,
    DecoArch, DecoArchColor, DecoBlocks, DiamondsGray, DoubleD, DoubleDiamonds,
    Earth1, Earth2, EclipsingSquares1, EclipsingSquares2, EggsBlack, Fans, Film,
    Firecrackers, FlowersBlockPrint, FlowersDaisies, FlowersModern1,
    FlowersModern2, FlowersPansy, FlowersRedRose, FlowersRoses, FlowersTeacup,
    FlowersTiny, Gems, GingerbreadMan, Gradient, Handmade1, Handmade2,
    HeartBalloon, HeartGray, Hearts, HeebieJeebies, Holly, HouseFunky, Hypnotic,
    IceCreamCones, LightBulb, Lightning1, Lightning2, MapleLeaf, MapleMuffins,
    MapPins, Marquee, MarqueeToothed, Moons, Mosaic, MusicNotes, Northwest,
    Ovals, Packages, PalmsBlack, PalmsColor, PaperClips, Papyrus, PartyFavor,
    PartyGlass, Pencils, People, PeopleHats, PeopleWaving, Poinsettias,
    PostageStamp, Pumpkin1, PushPinNote1, PushPinNote2, Pyramids, PyramidsAbove,
    Quadrants, Rings, Safari, Sawtooth, SawtoothGray, ScaredCat, Seattle,
    ShadowedSquares, SharksTeeth, ShorebirdTracks, Skyrocket, SnowflakeFancy,
    Snowflakes, Sombrero, Southwest, Stars, Stars3d, StarsBlack, StarsShadowed,
    StarsTop, Sun, Swirligig, TornPaper, TornPaperBlack, Trees, TriangleParty,
    Triangles, Tribal1, Tribal2, Tribal3, Tribal4, Tribal5, Tribal6,
    TwistedLines1, TwistedLines2, Vine, Waveline, WeavingAngles, WeavingBraid,
    WeavingRibbon, WeavingStrips, WhiteFlowers, Woodwork, XIllusions,
    ZanyTriangles, ZigZag, ZigZagStitch, Disabled);
  ...
  TdxProgressIndicationState = (Unknown, Allowed, Forbidden);
...
begin
  if AMethodName <> '' then
    raise ENotImplemented.Create('Method ' + AMethodName +
      ' is not implemented')
  else
...
constructor TdxBeginNextSectionFormattingEventArgs.Create
...
class operator TdxReferencedObjectContainer<T>.Implicit
  (const A: TdxReferencedObjectContainer<T>): T;
begin
...
procedure TdxReferencedObjectList<T>.Notify(Ptr: Pointer;
...
begin
  ...
  if not Result or IsEmpty then
    Exit;
  Result := (Kind = Value.Kind) and (TypeInfo = Value.TypeInfo) and
    (TypeData = Value.TypeData);
end;

function TValueHelper.IsDateTime: Boolean;
begin
  Result := (Kind = tkFloat) and ((TypeInfo.Name = 'TDateTime') or
    (TypeInfo.Name = 'TDate') or (TypeInfo.Name = 'TTime'));
end;
