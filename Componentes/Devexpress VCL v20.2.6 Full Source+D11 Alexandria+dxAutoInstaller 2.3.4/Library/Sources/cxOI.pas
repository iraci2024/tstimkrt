...
{$ENDIF}
  ...
  TypInfo, RTLConsts,
  dxCore, dxCoreClasses, dxMessages, cxGraphics, cxClasses, cxControls,
  cxLookAndFeels,
...
type
  ...
  TcxPropertyAttribute = (ipaValueList, ipaSubProperties, ipaDialog,
    ipaMultiSelect, ipaAutoUpdate, ipaSortList, ipaReadOnly, ipaRevertable);
  ...
  protected
    procedure AdjustInnerEditProperties(AProperties
  ...
  public
    ...
    procedure GetProperties(AOwner: TComponent;
      Proc: TcxGetPropEditProc); virtual;
  ...
  public
    ...
    property EditingController: TcxRTTIInspectorEditingController
  ...
  protected
    ...
    function GetDefaultShowEditButtons
      : TcxEditingControlEditShowButtons; override;
  public
    constructor Create(AOwner: TPersistent); override;
    procedure Assign(Source: TPersistent); override;
    property Inspector: TcxCustomRTTIInspector read GetInspector;
  published
    property ShowReadOnlyProperties: Boolean read FShowReadOnlyProperties
      write SetShowReadOnlyProperties default False;
  ...
  TcxFilterPropertyEvent = procedure(Sender: TObject;
  ...
  private
    ...
    procedure CNPropertyChanged(var AMessage: TMsg);
      message DXM_VG_PROPERTYCHANGED;
  ...
  public
    ...
    property OptionsView: TcxRTTIInspectorOptionsView read GetOptionsView
      write SetOptionsView;
    property InspectedObject: TPersistent read FInspectedObject
      write SetInspectedObject;
    property OnFilterProperty: TcxFilterPropertyEvent read FOnFilterProperty
      write FOnFilterProperty;
    property OnFilterPropertyEx: TcxFilterPropertyEvent read FOnFilterPropertyEx
      write FOnFilterPropertyEx;
    property OnPropertyChanged: TNotifyEvent read FOnPropertyChanged
...
procedure cxRegisterPropertyEditor(APropertyType: PTypeInfo;
  AComponentClass: TClass; const APropertyName: string;
  AEditorClass: TcxPropertyEditorClass);

type
  TcxPropertyMapperFunc = function(Obj: TPersistent; PropInfo: PPropInfo)
...
function cxGetPropertiesClassByEditor(APropertyEditor: TcxPropertyEditor)
  : TcxCustomEditPropertiesClass;
procedure cxRegisterEditPropertiesClass(AEditorClass: TcxPropertyEditorClass;
  AEditPropertiesClass: TcxCustomEditPropertiesClass);
function HasProperty(AClass: TClass; const APropertyName: string): Boolean;
function IsUniquePropertyRelativeTo(AClass, ARelativeClass: TClass;
  const APropertyName: string): Boolean;
function IsUniquePropertyRelativeParent(AClass: TClass;
  const APropertyName: string): Boolean;

function IsValidInspectedObject(AObject: TPersistent;
  AInspector: TcxCustomRTTIInspector): Boolean;

procedure cxDotNetInspectObject(AObject: TPersistent;
  AInspector: TcxRTTIInspector);
...
uses
  ...
  cxFormats, cxContainer, cxCustomData, cxGeometry, cxLookAndFeelPainters,
  cxDateUtils,
  cxTextEdit, cxDropDownEdit, cxCalendar, cxSpinEdit, cxTimeEdit, cxButtonEdit,
  cxColorComboBox, cxOIStringsEd, cxOIPictureEd, cxOICollectionEd,
  cxVGridConsts, dxGDIPlusClasses;
...
const
  dxModalResults: array [mrNone .. mrYesToAll] of string = ('mrNone', 'mrOk',
    'mrCancel', 'mrAbort', 'mrRetry', 'mrIgnore', 'mrYes', 'mrNo',
{$IFDEF DELPHI16}
    'mrClose', 'mrHelp', 'mrTryAgain', 'mrContinue',
{$ENDIF}
...
const
  cxPropClassMap: array [TTypeKind] of TcxPropertyEditorClass = (nil,
    // tkUnknown
...
procedure TcxPropertyComboBoxLookupData.Initialize(AVisualControlsParent
  : TWinControl);
begin
...
class function TcxPropertyComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxPropertyComboBox;
end;

class function TcxPropertyComboBoxProperties.GetLookupDataClass
...
class function TcxPropertyComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      I := 0;
      while (I < PropCount) and
        (CompareText(dxShortStringToString(PropList^[I].Name),
        APropertyName) <> 0) do
        Inc(I);
      Result := I < PropCount;
    finally
...
function IsUniquePropertyRelativeParent(AClass: TClass;
  const APropertyName: string): Boolean;
begin
  Result := IsUniquePropertyRelativeTo(AClass, AClass.ClassParent,
    APropertyName);
end;
...
begin
  ...
  if Result and (AObject is TcxCustomRTTIInspector) then
    Result := IsValidInspectedObject(TcxCustomRTTIInspector(AObject)
      .InspectedObject, AInspector);
end;
...
procedure TcxPropertyEditor.GetProperties(AOwner: TComponent;
...
function TcxPropertyEditor.IsDefaultValue: Boolean;
  ...
  function FindChildComponent(Component: TComponent; const Name: string)
    : TComponent;
  var
    I: Integer;
  begin
        ...
        begin
          ...
          begin
            if not AncestorIsValid(FAncestor, FRoot, FRootAncestor) or
              ...
              TCollection(GetOrdProp(FAncestor, FirstPropInfo)), FRoot,
              FRootAncestor) then
              Result := False;
          end;
        end;
...
procedure TcxPropertyEditor.SetPropEntry(Index: Integer; AInstance: TPersistent;
  APropInfo: PPropInfo);
begin
  with FPropList^[Index] do
  begin
      ...
      try
        ...
        try
          ...
          if csInline in Component.ComponentState then
            FRoot := Component;
          TComponentHack(Component)
            .GetChildren(WriteComponentSimulation, FRoot);
        finally
          FAncestorList.Free;
        end;
      finally
...
procedure TcxSetProperty.GetProperties(AOwner: TComponent;
  Proc: TcxGetPropEditProc);
var
  I: Integer;
begin
...
procedure TcxClassProperty.GetProperties(AOwner: TComponent;
  Proc: TcxGetPropEditProc);
var
...
begin
  ...
  try
    ...
    cxGetComponentProperties(AOwner, FInspector, Components,
      tkProperties, Proc);
  finally
    Components.Free;
  end;
...
begin
  Result := (AComponent.Name <> '') and
    (AComponent is GetTypeData(GetPropType)^.ClassType);
end;

function TcxComponentProperty.GetFullName(AComponent: TComponent): string;
begin
  Result := AComponent.Name;
  while (AComponent.Name <> '') and (AComponent.Owner <> nil) and
    (AComponent.Owner <> Inspector.Owner) do
  begin
...
begin
  ...
  if GraphicClass <> nil then
    if GraphicClass.InheritsFrom(TBitmap) then
      Result := CF_BITMAP
    else if GraphicClass.InheritsFrom(TMetafile) then
...
const
  cxShortCuts: array [0 .. 82] of TShortCut = (scNone, Byte('A') or scCtrl,
    ...
    Byte('Z') or scCtrl, VK_F1, VK_F2, VK_F3, VK_F4, VK_F5, VK_F6, VK_F7, VK_F8,
    VK_F9, VK_F10, VK_F11, VK_F12, VK_F1 or scCtrl, VK_F2 or scCtrl,
    VK_F3 or scCtrl, VK_F4 or scCtrl, VK_F5 or scCtrl, VK_F6 or scCtrl,
    VK_F7 or scCtrl, VK_F8 or scCtrl, VK_F9 or scCtrl, VK_F10 or scCtrl,
    VK_F11 or scCtrl, VK_F12 or scCtrl, VK_F1 or scShift, VK_F2 or scShift,
    VK_F3 or scShift, VK_F4 or scShift, VK_F5 or scShift, VK_F6 or scShift,
    VK_F7 or scShift, VK_F8 or scShift, VK_F9 or scShift, VK_F10 or scShift,
    VK_F11 or scShift, VK_F12 or scShift, VK_F1 or scShift or scCtrl,
    VK_F2 or scShift or scCtrl, VK_F3 or scShift or scCtrl, VK_F4 or scShift or
    scCtrl, VK_F5 or scShift or scCtrl, VK_F6 or scShift or scCtrl,
    VK_F7 or scShift or scCtrl, VK_F8 or scShift or scCtrl, VK_F9 or scShift or
    scCtrl, VK_F10 or scShift or scCtrl, VK_F11 or scShift or scCtrl,
    VK_F12 or scShift or scCtrl, VK_INSERT, VK_INSERT or scShift,
    VK_INSERT or scCtrl, VK_DELETE, VK_DELETE or scShift, VK_DELETE or scCtrl,
    VK_BACK or scAlt, VK_BACK or scShift or scAlt);
...
{$IFDEF DELPHI16}
    if DT = NullDate then
      Result := StringReplace(DateTimeToStr(MinDateTime), '1', '0',
        [rfReplaceAll])
    else
{$ENDIF}
...
begin
  ...
  case NewType of
    ...
    varNull:
      V := NULL;
    -1:
      raise EdxException.Create('UnknownType'); // todo resource
  else
    try
      VarCast(V, V, NewType);
    except
...
begin
  ...
  if Value = '' then
    VarClear(V)
  else if (CompareText(Value, cxSNull) = 0) then
    V := NULL
...
type
  ...
  public
    constructor Create(AInstance: TPersistent; AFilter: TTypeKinds;
      ASorted: Boolean);
    destructor Destroy; override;
...
constructor TcxPropInfoList.Create(AInstance: TPersistent; AFilter: TTypeKinds;
  ASorted: Boolean);
begin
  ...
  for I := 0 to FCount - 1 do
    with FList^[I]^ do
      if (PropType^ = P^.PropType^) and
        (CompareText(dxShortStringToString(Name), dxShortStringToString(P^.Name)
        ) = 0) then
      begin
        Result := True;
        Exit;
...
function cxGetEditorClass(APropInfo: PPropInfo; Obj: TPersistent)
  : TcxPropertyEditorClass;
var
  PropType: PTypeInfo;
  ...
  if FPropertyClasses <> nil then
    ...
    begin
      ...
      // compatible class type
        ...
        (GetTypeData(P.PropertyType)^.ClassType)) then
        if ((P.ComponentClass = nil) or (Obj.InheritsFrom(P.ComponentClass)))
          and ((P.PropertyName = '') or SameText(GetPropName(APropInfo),
          P.PropertyName)) then
          ...
          // P's proptype is more specific than C's proptype
            ...
            ^.ClassType.InheritsFrom(GetTypeData(C.PropertyType)^.ClassType))))
...
var
  ...
  Obj: TPersistent;
begin
  if (AComponents = nil) or (AComponents.Count = 0) then
    Exit;
  ...
  try
        ...
        try
          ...
          with PropInfo^ do
            if (GetProc = nil) or
              ...
              ((CompCount > 1) and not(ipaMultiSelect in Editor.GetAttributes))
    ...
    try
        ...
        try
          ...
          begin
            if (AComponents[J].ClassType <> CompType) and
              (cxGetEditorClass(TcxPropInfoList(PropLists[J])[I], AComponents[J]
              ) <> EdClass) then
            begin
              AddEditor := False;
              Break;
            end;
            Editor.SetPropEntry(J, AComponents[J],
              TcxPropInfoList(PropLists[J])[I]);
          end;
        except
          Editor.Free;
          raise;
      ...
      for I := 0 to PropLists.Count - 1 do
        TcxPropInfoList(PropLists[I]).Free;
      PropLists.Free;
    end;
  finally
    AInspector.EndLevel;
...
begin
  ...
  begin
    ...
    if EditingItem = nil then
      Exit;
    AEditViewInfo := Controller.GetFocusedCellViewInfo(EditingItem);
    if AEditViewInfo <> nil then
      Edit.InternalProperties.Update(TcxEditCellViewInfoAccess(AEditViewInfo)
        .Properties);
  end;
  AItem := EditingItem;
...
function TcxRTTIInspectorEditingController.GetController
  : TcxRTTIInspectorController;
begin
...
begin
  with Inspector do
    if (PropertyEditor <> nil) and
      not(ipaRevertable in PropertyEditor.GetAttributes) then
...
begin
  if FFocusChanging or Inspector.LockRefresh then
    Exit;
  FFocusChanging := True;
  try
...
var
  ...
  AIndex: Integer;
begin
  if VerticalGrid.IsDesigning or Assigned(Value) and
    not TcxCustomRowAccess(Value).CanFocus then
    Exit;
  if FocusedRow = Value then
    Exit;
...
function TcxRTTIInspectorController.GetEditingController
  : TcxRTTIInspectorEditingController;
begin
  Result := TcxRTTIInspectorEditingController(inherited EditingController);
end;

function TcxRTTIInspectorController.GetRowAbsoluteIndex
...
function TcxRTTIInspectorOptionsView.GetDefaultShowEditButtons
...
begin
  if (Index = vgs_Content) and (AContentInfo <> nil) and
    AContentInfo.EditorRowProperties.EditProperties.ReadOnly then
    ...
    case Index of
      ...
      vgs_Header:
        ...
        begin
          ...
          else if IsRootParentComponent then
...
procedure TcxCustomRTTIInspector.Notification(AComponent: TComponent;
...
function TcxCustomRTTIInspector.InternalMouseWheel(Shift: TShiftState;
...
function TcxCustomRTTIInspector.IsMouseWheelHandleNeeded(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
...
function TcxCustomRTTIInspector.FilterProperty(const APropertyName
  : string): Boolean;
begin
  Result := True;
  if Assigned(FOnFilterProperty) then
    FOnFilterProperty(Self, APropertyName, Result);
end;

function TcxCustomRTTIInspector.FilterPropertyEx(const AFullPropertyName
  : string): Boolean;
begin
  Result := True;
  if Assigned(FOnFilterPropertyEx) then
    FOnFilterPropertyEx(Self, AFullPropertyName, Result);
end;

function TcxCustomRTTIInspector.FindRowByPropertyName(const APropertyName
  : string): TcxPropertyRow;
var
  I: Integer;
begin
...
function TcxCustomRTTIInspector.FindRowByPropertyPath(APropertyPath: string;
  AExactMatch: Boolean): TcxPropertyRow;
var
  I, L: Integer;
  ARow: TcxCustomRow;
begin
  ...
  begin
    ARow := Rows[I];
    if (APropertyPath = GetRowPropertyPath(ARow)) and (ARow is TcxPropertyRow)
    then
    begin
  ...
  repeat
    ...
    begin
      ARow := Rows[I];
      if (AnsiPos(APropertyPath, GetRowPropertyPath(ARow)) = 1) and
        (ARow is TcxPropertyRow) then
      begin
        Result := TcxPropertyRow(ARow);
        Exit;
      end;
...
procedure TcxCustomRTTIInspector.FocusRowByPropertyName
...
function TcxCustomRTTIInspector.GetEditPropertiesClass(APropertyEditor
...
begin
  ...
  if FilterProperty(APropertyEditor.GetName) and
    FilterPropertyEx(AFullPropertyName) then
    Result := GetEditPropertiesClassCore(APropertyEditor)
  else
    Result := nil;
end;

function TcxCustomRTTIInspector.GetEditPropertiesClassCore(APropertyEditor
  : TcxPropertyEditor): TcxCustomEditPropertiesClass;
begin
...
begin
  ...
  TcxCustomEditPropertiesAccess(AProperties).ReadOnly :=
    ([ipaReadOnly] * APropertyEditor.GetAttributes <> []);
  ...
  TcxCustomEditPropertiesAccess(AProperties).OnEditValueChanged :=
    EditValueChanged;

  if AProperties is TcxCustomTextEditProperties then
    TcxCustomTextEditProperties(AProperties).MaxLength :=
      APropertyEditor.GetEditLimit;
  ...
  begin
    ...
    TcxComboBoxProperties(AProperties).Revertable := ipaRevertable
      in APropertyEditor.GetAttributes;
  end;

  if AProperties is TcxColorComboBoxProperties then
  begin
    TcxColorComboBoxPropertiesAccess(AProperties).ColorDialogShowFull := True;
    TcxColorComboBoxPropertiesAccess(AProperties).ColorDialogType :=
      cxcdtAdvanced;
    TcxColorComboBoxPropertiesAccess(AProperties).ColorValueFormat :=
      cxcvHexadecimal;
    TcxColorComboBoxPropertiesAccess(AProperties).DropDownListStyle :=
      lsEditList;
    TcxColorComboBoxPropertiesAccess(AProperties).MaxMRUColors := 0;
    TcxColorComboBoxPropertiesAccess(AProperties).ColorBoxWidth :=
      ScaleFactor.Apply(16);
    TcxColorComboBoxPropertiesAccess(AProperties).PrepareDelphiColorList
      (False, False);
  end;
...
begin
  if (PropertyEditor <> nil) and (ipaAutoUpdate in PropertyEditor.GetAttributes)
  then
  begin
...
begin
  if not((PropertyEditor <> nil) and
    (ipaAutoUpdate in PropertyEditor.GetAttributes)) then
    TrySetValue(TcxCustomEdit(Sender), False);
  Controller.CheckPostData;
end;

function TcxCustomRTTIInspector.GetControllerClass
  : TcxCustomControlControllerClass;
begin
  Result := TcxRTTIInspectorController;
end;

function TcxCustomRTTIInspector.GetControlStylesClass
  : TcxCustomControlStylesClass;
begin
  Result := TcxRTTIInspectorStyles;
end;

function TcxCustomRTTIInspector.GetEditingControllerClass
  : TcxEditingControllerClass;
begin
  Result := TcxRTTIInspectorEditingController;
end;

function TcxCustomRTTIInspector.GetOptionsBehaviorClass
...
begin
  Result := (PropertyEditor <> nil) and
    ((ipaDialog in PropertyEditor.GetAttributes) or
    (ipaValueList in PropertyEditor.GetAttributes));
end;
...
procedure TcxCustomRTTIInspector.CreatePropertyRows(AOldInspectedObject
  : TPersistent);
begin
  BeginUpdate;
  try
...
procedure TcxCustomRTTIInspector.GetComponentsProperties(const AInstances
  : array of TPersistent);
...
begin
  ...
  try
    ...
    if AOwner = nil then
      AOwner := Self;
    cxGetComponentProperties(AOwner, Self, ComponentList, tkProperties,
      CreateRows);
    FullCollapse;
    FParentRow := nil;
    FCurrentRow := nil;
...
begin
  TcxComboBoxProperties(TcxPropertyRow(FCurrentRow).Properties.EditProperties)
    .Items.Add(S);
end;
...
procedure TcxCustomRTTIInspector.RowButtonClick(Sender: TObject;
...
procedure TcxCustomRTTIInspector.SetOptionsView(const Value
  : TcxRTTIInspectorOptionsView);
begin
...
function TcxCustomRTTIInspector.TrySetValue(AEdit: TcxCustomEdit;
  AUseText: Boolean): Boolean;
var
  V: Variant;
begin
  Result := False;
  if FSettingValue or (PropertyEditor = nil) then
    Exit;
  FSettingValue := True;
  try
      ...
      try
        ...
        begin
          if (AObject is TComponent) and (TComponent(AObject).Name <> '') then
            Properties.Caption := TComponent(AObject).Name + ': ' +
              AObject.ClassName
          else
...
function TcxInspectedObjectPropertyEditor.IsValidComponent
...
cxRegisterPropertyEditor(TypeInfo(TFontCharset), nil, '',
  TcxFontCharsetProperty);
...
cxRegisterPropertyEditor(TypeInfo(string), TMediaPlayer, 'FileName',
  TcxMPFilenameProperty);
...
cxRegisterPropertyEditor(TypeInfo(TComponentName), nil, '',
  TcxComponentNameProperty);
cxRegisterPropertyEditor(TypeInfo(TImeName), nil, '', TcxImeNameProperty);
cxRegisterPropertyEditor(TypeInfo(TModalResult), nil, '',
  TcxModalResultProperty);
...
cxRegisterPropertyEditor(TypeInfo(TdxSmartGlyph), nil, '',
  TdxSmartGlyphProperty);

cxRegisterPropertyEditor(TypeInfo(TPersistent), TcxCustomRTTIInspector,
  'InspectedObject', TcxInspectedObjectPropertyEditor);
...
cxRegisterEditPropertiesClass(TcxComponentProperty,
  TcxPropertyComboBoxProperties);
...
cxRegisterEditPropertiesClass(TcxFontCharsetProperty,
  TcxPropertyComboBoxProperties);
cxRegisterEditPropertiesClass(TcxFontNameProperty,
  TcxPropertyComboBoxProperties);
...
cxRegisterEditPropertiesClass(TcxModalResultProperty,
  TcxPropertyComboBoxProperties);
...
cxRegisterEditPropertiesClass(TcxShortCutProperty,
  TcxPropertyComboBoxProperties);
...
//
cxRegisterPropertyEditor(TypeInfo(string), TcxCustomEditorRowProperties,
  'EditPropertiesClassName', nil);
cxRegisterPropertyEditor(TypeInfo(TcxCustomEditProperties),
  TcxCustomEditorRowProperties, 'EditProperties', TcxEditPropertiesProperty);

cxRegisterPropertyEditor(TypeInfo(TdxSkinName), nil, 'SkinName',
  TcxSkinNameProperty);
