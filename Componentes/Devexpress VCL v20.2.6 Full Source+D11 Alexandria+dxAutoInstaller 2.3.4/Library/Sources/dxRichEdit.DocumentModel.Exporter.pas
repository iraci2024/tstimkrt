...
type
  ...
  strict private
    ...
    FId: Integer;
  public
    constructor Create(ANote: TdxPieceTable; ANumber: Integer;
      const ANumberText: string);
  ...
  private
    FCalculators: TObjectDictionary<TdxAbstractNumberingList,
      TdxNumberingListCountersCalculator>;
  public
    procedure BeginCalculateCounters;
    procedure EndCalculateCounters;
    function CalculateNextCounters(ACurrentParagraph: TdxParagraph)
      : TIntegerDynArray;
  end;
  ...
  TdxDocumentModelExporter = class abstract(TdxCustomDocumentModelExporter,
    IdxSimpleDocumentModelExporter, IdxDocumentModelExporter)
  strict private
    ...
    FVisitableDocumentIntervalsIteratorStack
      : TdxObjectStack<TdxVisitableDocumentIntervalBoundaryIterator>;
    function GetDocumentModel: TdxDocumentModel;
    function GetVisitableDocumentIntervalsIterator
      : TdxVisitableDocumentIntervalBoundaryIterator;
    function GetPieceTableNumberingListCounters
      : TdxPieceTableNumberingListCountersManager;
  protected
    procedure ExportCell(ACell: TdxTableCell;
      ATableInfo: TdxTableInfo); virtual;
    ...
    procedure ExportEvenPageFooter(ASectionFooter: TdxSectionFooter;
      ALinkedToPrevious: Boolean); virtual;
    ...
    procedure ExportImageReference(ARun: TdxFloatingObjectAnchorRun);
      overload; virtual;
    ...
    function ExportParagraphFiltered(AParagraph: TdxParagraph)
      : TdxParagraphIndex;
    ...
    function CreateVisitableDocumentIntervalBoundaryIterator
      : TdxVisitableDocumentIntervalBasedObjectBoundaryIterator; virtual;
    ...
    function CalculateSectionParagraphCount(ASection: TdxSection)
      : Integer; virtual;
    function CalculatePieceTableParagraphCount(AHeaderFooter
      : TdxSectionHeaderFooterBase): Integer; virtual;
    ...
    function CalculateFootNoteNumberCore(AProperties: TdxSectionFootNote;
      AValue: Integer): Integer; virtual;
    function CreateFootNoteExportInfo(ANote: TdxFootNoteRun)
      : TdxFootNoteExportInfo; overload; virtual;
    function FindFootNoteExportInfoByNote(AList: TList<TdxFootNoteExportInfo>;
      APieceTable: TdxPieceTable; out AInfo: TdxFootNoteExportInfo)
      : Boolean; virtual;
    function CreateFootNoteExportInfo(ANote: TdxEndNoteRun)
      : TdxFootNoteExportInfo; overload; virtual;

    procedure TryToExportBookmarks(ARunIndex: TdxRunIndex;
      ARunOffset: Integer); virtual;

    procedure InvalidateParagraphsBoxes;
    function PrepareModelForExport(ADocumentModel: TdxCustomDocumentModel)
      : TdxCustomDocumentModel;
    procedure PerformExportPieceTable(APieceTable: TdxPieceTable;
      APieceTableExporter: TdxExportPieceTableDelegate);
    procedure SplitRuns;
    ...
    property FootNoteExportInfos: TList<TdxFootNoteExportInfo>
      read FFootNoteExportInfos;
    property EndNoteExportInfos: TList<TdxFootNoteExportInfo>
      read FEndNoteExportInfos;
    property ExportedParagraphCount: Integer read FExportedParagraphCount
      write FExportedParagraphCount;
    property FieldLevel: Integer read FFieldLevel;
    property PieceTableNumberingListCounters
      : TdxPieceTableNumberingListCountersManager
      read GetPieceTableNumberingListCounters;
    property VisitableDocumentIntervalsIterator
      : TdxVisitableDocumentIntervalBoundaryIterator
      read GetVisitableDocumentIntervalsIterator;
    property VisitableDocumentIntervalsIteratorStack
      : TdxObjectStack<TdxVisitableDocumentIntervalBoundaryIterator>
      read FVisitableDocumentIntervalsIteratorStack;
  public
...
constructor TdxFootNoteExportInfo.Create(ANote: TdxPieceTable; ANumber: Integer;
  const ANumberText: string);
begin
  FId := 0;
  FNote := ANote;
...
constructor TdxDocumentModelExporter.Create(ADocumentModel
...
begin
  ...
  FVisitableDocumentIntervalsIteratorStack :=
    TdxObjectStack<TdxVisitableDocumentIntervalBoundaryIterator>.Create(False);
...
begin
  ...
  else if ARun.InheritsFrom(TdxFieldCodeEndRun) then
    ExportFieldCodeEndRun(TdxFieldCodeEndRun(ARun))
  else if ARun.InheritsFrom(TdxFieldResultEndRun) then
    ExportFieldResultEndRun(TdxFieldResultEndRun(ARun))
  else if ARun.InheritsFrom(TdxFootNoteRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportFootNoteRun(TdxFootNoteRun(ARun));
  end
  else if ARun.InheritsFrom(TdxEndNoteRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportEndNoteRun(TdxEndNoteRun(ARun));
  end
  else if ARun.InheritsFrom(TdxTextRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportTextRun(TdxTextRun(ARun));
  end
  else if ARun.InheritsFrom(TdxSectionRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportSectionRun(TdxSectionRun(ARun))
  end
  else if ARun.InheritsFrom(TdxParagraphRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportParagraphRun(TdxParagraphRun(ARun))
  end
  else if ARun.InheritsFrom(TdxFloatingObjectAnchorRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportFloatingObjectAnchorRun(TdxFloatingObjectAnchorRun(ARun))
  end
  else if ARun.InheritsFrom(TdxInlinePictureRun) then
  begin
    if ShouldExportInlinePicture(TdxInlinePictureRun(ARun)) then
      ExportInlinePictureRun(TdxInlinePictureRun(ARun))
  end
  else if ARun.InheritsFrom(TdxSeparatorTextRun) then
  begin
    if ShouldExportRun(ARun) then
      ExportSeparatorTextRun(TdxSeparatorTextRun(ARun));
  end
  else if ARun.InheritsFrom(TdxInlineObjectRun) then
  begin
    if ShouldExportRun(ARun) then
...
begin
  ProgressIndication.&Begin(cxGetResourceString(@sdxRichEditMsg_Saving), 0,
    CalculateTotalParagraphCount, 0);
  try
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        AParentTableInfo := TdxTableInfo.Create
          (AParagraphTable.GetParentTable(ATableInfo.NestedLevel + 1));
        try
    ...
    begin
      ...
      ASkipParagraph := AllowSkipParagraphInCell and AAfterNestedTable and
        (AParagraphCell.EndParagraphIndex = I) and AParagraph.IsEmpty;
...
procedure TdxDocumentModelExporter.ExportFloatingObjectAnchorRun
...
procedure TdxDocumentModelExporter.ExportImageReference
  (ARun: TdxFloatingObjectAnchorRun);
begin
end;
...
function TdxDocumentModelExporter.ExportParagraph(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
begin
...
procedure TdxDocumentModelExporter.ExportInlinePictureRun
  (ARun: TdxInlinePictureRun);
begin
  // do nothing
end;

function TdxDocumentModelExporter.GetNumberingListText
  (AParagraph: TdxParagraph): string;
begin
  Result := AParagraph.GetNumberingListText
    (FPieceTableNumberingListCounters.CalculateNextCounters(AParagraph));
end;
...
function TdxDocumentModelExporter.ExportRootTable(ATable: TdxTable)
  : TdxParagraphIndex;
var
  ATableInfo: TdxTableInfo;
  ARootTable: TdxTable;
...
begin
  ...
  if ASection.InnerFirstPageFooter <> nil then
    ExportFirstPageFooter(ASection.InnerFirstPageFooter,
      ASection.Footers.IsLinkedToPrevious(TdxHeaderFooterType.First));
  if ASection.InnerOddPageFooter <> nil then
    ExportOddPageFooter(ASection.InnerOddPageFooter,
      ASection.Footers.IsLinkedToPrevious(TdxHeaderFooterType.Odd));
  if ASection.InnerEvenPageFooter <> nil then
    ExportEvenPageFooter(ASection.InnerEvenPageFooter,
      ASection.Footers.IsLinkedToPrevious(TdxHeaderFooterType.Even));
end;

procedure TdxDocumentModelExporter.ExportFirstPageHeader(ASectionHeader
  : TdxSectionHeader; ALinkedToPrevious: Boolean);
begin
  PerformExportPieceTable(TdxPieceTable(ASectionHeader.PieceTable),
    ExportPieceTable);
end;

procedure TdxDocumentModelExporter.ExportOddPageHeader(ASectionHeader
  : TdxSectionHeader; ALinkedToPrevious: Boolean);
begin
  PerformExportPieceTable(TdxPieceTable(ASectionHeader.PieceTable),
    ExportPieceTable);
...
procedure TdxDocumentModelExporter.ExportEvenPageHeader(ASectionHeader
  : TdxSectionHeader; ALinkedToPrevious: Boolean);
begin
  PerformExportPieceTable(TdxPieceTable(ASectionHeader.PieceTable),
    ExportPieceTable);
...
procedure TdxDocumentModelExporter.ExportFirstPageFooter(ASectionFooter
  : TdxSectionFooter; ALinkedToPrevious: Boolean);
begin
  PerformExportPieceTable(TdxPieceTable(ASectionFooter.PieceTable),
    ExportPieceTable);
end;

procedure TdxDocumentModelExporter.ExportOddPageFooter(ASectionFooter
  : TdxSectionFooter; ALinkedToPrevious: Boolean);
begin
  PerformExportPieceTable(TdxPieceTable(ASectionFooter.PieceTable),
    ExportPieceTable);
...
procedure TdxDocumentModelExporter.ExportEvenPageFooter(ASectionFooter
  : TdxSectionFooter; ALinkedToPrevious: Boolean);
begin
  PerformExportPieceTable(TdxPieceTable(ASectionFooter.PieceTable),
    ExportPieceTable);
...
procedure TdxDocumentModelExporter.ExportSectionFiltered(const ASection
  : TdxSection);
begin
  ...
  begin
    ExportedParagraphCount := ExportedParagraphCount +
      CalculateSectionParagraphCount(ASection);
    ProgressIndication.SetProgress(ExportedParagraphCount);
  end;
end;
...
function TdxDocumentModelExporter.ExportTable(ATableInfo: TdxTableInfo)
  : TdxParagraphIndex;
var
  ARowsCount: Integer;
  I: Integer;
...
function TdxDocumentModelExporter.
  CreateVisitableDocumentIntervalBoundaryIterator
  : TdxVisitableDocumentIntervalBasedObjectBoundaryIterator;
begin
  Result := TdxVisitableDocumentIntervalBasedObjectBoundaryIterator.Create
    (PieceTable);
end;
...
procedure TdxDocumentModelExporter.ExportRangePermissionStart(ARangePermission
  : TdxRangePermission);
begin
end;

procedure TdxDocumentModelExporter.ExportRangePermissionEnd(ARangePermission
  : TdxRangePermission);
begin
end;

function TdxDocumentModelExporter.GetPieceTableNumberingListCounters
  : TdxPieceTableNumberingListCountersManager;
begin
  if FPieceTableNumberingListCounters = nil then
    FPieceTableNumberingListCounters :=
      TdxPieceTableNumberingListCountersManager.Create;
...
function TdxDocumentModelExporter.GetVisitableDocumentIntervalsIterator
  : TdxVisitableDocumentIntervalBoundaryIterator;
begin
...
var
  ...
  AOriginalPieceTableNumberingListCounters
    : TdxPieceTableNumberingListCountersManager;
      ...
      try
        ...
        begin
          for I := ATransaction.Items.Count -
            1 downto ATransactionItemCountBeforeExecute do
          begin
...
function TdxDocumentModelExporter.PrepareModelForExport(ADocumentModel
  : TdxCustomDocumentModel): TdxCustomDocumentModel;
begin
...
procedure TdxDocumentModelExporter.
  PushVisitableDocumentIntervalBoundaryIterator;
begin
  FVisitableDocumentIntervalsIteratorStack.Push
    (CreateVisitableDocumentIntervalBoundaryIterator);
end;
...
function TdxDocumentModelExporter.ShouldExportRun(ARun: TdxTextRunBase)
  : Boolean;
begin
...
begin
  ...
  begin
    ...
    if (ABoundary.Position.RunIndex <> ARunIndex) or
      (ABoundary.Position.RunOffset <> ARunOffset) then
      Break;
...
function TdxDocumentModelExporter.
  ShouldCalculateFootNoteAndEndNoteNumbers: Boolean;
begin
  Result := False;
end;
...
procedure TdxDocumentModelExporter.SplitRunByModelPosition
  (const APos: TdxDocumentModelPosition);
begin
  if (APos.RunOffset <> 0) and
    (APos.LogPosition <= PieceTable.DocumentEndLogPosition) then
...
function TdxDocumentModelExporter.CalculateSectionParagraphCount
  (ASection: TdxSection): Integer;
begin
...
function TdxDocumentModelExporter.CalculatePieceTableParagraphCount
  (AHeaderFooter: TdxSectionHeaderFooterBase): Integer;
begin
...
procedure TdxDocumentModelExporter.CalculateFootNoteNumber
  (ASection: TdxSection);
begin
  FFootNoteNumber := CalculateFootNoteNumberCore(ASection.FootNote,
    FFootNoteNumber);
end;

procedure TdxDocumentModelExporter.CalculateEndNoteNumber(ASection: TdxSection);
begin
  FEndNoteNumber := CalculateFootNoteNumberCore(ASection.EndNote,
    FFootNoteNumber);
end;

function TdxDocumentModelExporter.CalculateFootNoteNumberCore
  (AProperties: TdxSectionFootNote; AValue: Integer): Integer;
begin
...
function TdxDocumentModelExporter.CreateFootNoteExportInfo
  (ANote: TdxFootNoteRun): TdxFootNoteExportInfo;
begin
  Result := TdxFootNoteExportInfo.Create(TdxPieceTable(ANote.Note.PieceTable),
    FFootNoteNumber, FCurrentSection.FootNote.FormatCounterValue
    (FFootNoteNumber));
end;

function TdxDocumentModelExporter.FindFootNoteExportInfoByNote
  (AList: TList<TdxFootNoteExportInfo>; APieceTable: TdxPieceTable;
  out AInfo: TdxFootNoteExportInfo): Boolean;
var
  ACount, I: Integer;
begin
  ACount := AList.Count;
...
function TdxDocumentModelExporter.CreateFootNoteExportInfo(ANote: TdxEndNoteRun)
  : TdxFootNoteExportInfo;
begin
  Result := TdxFootNoteExportInfo.Create(TdxPieceTable(ANote.Note.PieceTable),
    FEndNoteNumber, FCurrentSection.EndNote.FormatCounterValue(FEndNoteNumber));
end;
...
begin
  ...
  begin
    ...
    begin
      ACalculator := TdxNumberingListCountersCalculator.Create
