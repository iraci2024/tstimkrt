...
type
  TdxBingMapRouteDataManeuverType = (rdmtArriveFinish, rdmtArriveIntermediate,
    rdmtBearLeft, rdmtBearLeftThenBearLeft, rdmtBearLeftThenBearRight,
    rdmtBearLeftThenTurnLeft, rdmtBearLeftThenTurnRight, rdmtBearRight,
    rdmtBearRightThenBearLeft, rdmtBearRightThenBearRight,
    rdmtBearRightThenTurnLeft, rdmtBearRightThenTurnRight, rdmtBearThenKeep,
    rdmtBearThenMerge, rdmtContinue, rdmtDepartIntermediateStop,
    rdmtDepartIntermediateStopReturning, rdmtDepartStart, rdmtEnterRoundabout,
    rdmtExitRoundabout, rdmtEnterThenExitRoundabout, rdmtKeepLeft,
    rdmtKeepOnRampLeft, rdmtKeepOnRampRight, rdmtKeepOnRampStraight,
    rdmtKeepRight, rdmtKeepStraight, rdmtKeepToStayLeft, rdmtKeepToStayRight,
    rdmtKeepToStayStraight, rdmtMerge, rdmtNone, rdmtRampThenHighwayLeft,
    rdmtRampThenHighwayRight, rdmtRampThenHighwayStraight, rdmtRoadNameChange,
    rdmtTake, rdmtTakeRampLeft, rdmtTakeRampRight, rdmtTakeRampStraight,
    rdmtTakeTransit, rdmtTransfer, rdmtTransitArrive, rdmtTransitDepart,
    rdmtTurnBack, rdmtTurnLeft, rdmtTurnLeftThenBearLeft,
    rdmtTurnLeftThenBearRight, rdmtTurnLeftThenTurnLeft,
    rdmtTurnLeftThenTurnRight, rdmtTurnRight, rdmtTurnRightThenBearLeft,
    rdmtTurnRightThenBearRight, rdmtTurnRightThenTurnLeft,
    rdmtTurnRightThenTurnRight, rdmtTurnThenMerge, rdmtTurnToStayLeft,
    rdmtTurnToStayRight, rdmtUnknown, rdmtUTurn, rdmtWait, rdmtWalk);
  ...
  public
    ...
    property ItineraryItems: TObjectList<TdxBingMapItineraryItem>
      read FItineraryItems;
  ...
  TdxBingMapRouteDataServiceResponse = class
  ...
  public
    constructor Create(const AGeoPoint: TdxMapControlGeoPoint);
      overload; virtual;
    constructor Create(const ALandmark: string); overload; virtual;
    function AsString: string; virtual; // for internal use
    property PointType: TdxBingMapRouteWaypointType read FPointType
      write FPointType;
  end;
  ...
  public
    ...
    function Add(const AGeoPoint: TdxMapControlGeoPoint; // for internal use
      APointType: TdxBingMapRouteWaypointType = bmwtWayPoint)
      : TdxBingMapRouteWaypoint; overload; // for internal use
    function Add(const ALandmark: string;
      APointType: TdxBingMapRouteWaypointType = bmwtWayPoint)
      : TdxBingMapRouteWaypoint; overload; // for internal use
    procedure Clear; // for internal use
    property Count: Integer read GetCount;
    property Items[Index: Integer]: TdxBingMapRouteWaypoint
  ...
  public
    ...
    property DistanceUnit: TdxBingMapDistanceUnit read FDistanceUnit
      write FDistanceUnit;
    property RoutePathOutput: Boolean read FRoutePathOutput
      write FRoutePathOutput;
  end;

  TdxBingMapTravelMode = (bmtmDriving, bmtmWalking, bmtmTransit);
  TdxBingMapExcludeRoadType = (bmetNone, bmetAvoid, bmetMinimize);
  TdxBingMapOptimizeRouteType = (bmorTime, bmorDistance, bmorTimeWithTraffic,
    bmorTimeAvoidClosure);
  TdxBingMapTransitTimeType = (bmttArrival, bmttDeparture, bmttLastAvailable);

  TdxBingMapRESTServiceRouteParams = class
    (TdxBingMapRESTServiceCustomRouteParams) // for internal use
  private
    FDateTime: TDateTime;
  ...
  public
    ...
    property ExcludeHighways: TdxBingMapExcludeRoadType read FExcludeHighways
      write FExcludeHighways;
    property ExcludeTolls: TdxBingMapExcludeRoadType read FExcludeTolls
      write FExcludeTolls;
    property MaxSolutions: Integer read FMaxSolutions write FMaxSolutions;
    property Optimize: TdxBingMapOptimizeRouteType read FOptimize
      write FOptimize;
    property TimeType: TdxBingMapTransitTimeType read FTimeType write FTimeType;
    property TravelMode: TdxBingMapTravelMode read FTravelMode
      write FTravelMode;
  end;

  TdxBingMapRESTServiceRoutesFromMajorRoadsParams = class
    (TdxBingMapRESTServiceCustomRouteParams) // for internal use
  private
    FDestination: string;
  ...
  TdxBingMapRoutesFromMajorRoadsDataService = class
    (TdxBingMapCustomRouteDataService) // for internal use
  protected
    function CreateParams: TdxBingMapRESTServiceParams; override;
    function GetAddress: string; override;
  end;
...
const
  dxBingMapRouteDataManeuverType: array [Low(TdxBingMapRouteDataManeuverType)
    .. High(TdxBingMapRouteDataManeuverType)] of string = ('ArriveFinish',
    'ArriveIntermediate', 'BearLeft', 'BearLeftThenBearLeft',
    'BearLeftThenBearRight', 'BearLeftThenTurnLeft', 'BearLeftThenTurnRight',
    'BearRight', 'BearRightThenBearLeft', 'BearRightThenBearRight',
    'BearRightThenTurnLeft', 'BearRightThenTurnRight', 'BearThenKeep',
    'BearThenMerge', 'Continue', 'DepartIntermediateStop',
    'DepartIntermediateStopReturning', 'DepartStart', 'EnterRoundabout',
    'ExitRoundabout', 'EnterThenExitRoundabout', 'KeepLeft', 'KeepOnRampLeft',
    'KeepOnRampRight', 'KeepOnRampStraight', 'KeepRight', 'KeepStraight',
    'KeepToStayLeft', 'KeepToStayRight', 'KeepToStayStraight', 'Merge', 'None',
    'RampThenHighwayLeft', 'RampThenHighwayRight', 'RampThenHighwayStraight',
    'RoadNameChange', 'Take', 'TakeRampLeft', 'TakeRampRight',
    'TakeRampStraight', 'TakeTransit', 'Transfer', 'TransitArrive',
    'TransitDepart', 'TurnBack', 'TurnLeft', 'TurnLeftThenBearLeft',
    'TurnLeftThenBearRight', 'TurnLeftThenTurnLeft', 'TurnLeftThenTurnRight',
    'TurnRight', 'TurnRightThenBearLeft', 'TurnRightThenBearRight',
    'TurnRightThenTurnLeft', 'TurnRightThenTurnRight', 'TurnThenMerge',
    'TurnToStayLeft', 'TurnToStayRight', 'Unknown', 'UTurn', 'Wait', 'Walk');

function GetManeuverTypeByName(const AManeuver: string)
...
constructor TdxBingMapRouteWaypoint.Create(const AGeoPoint
...
procedure TdxBingMapRouteWaypoints.AddWaypoint
...
function TdxBingMapRouteWaypoints.GetItems(Index: Integer)
  : TdxBingMapRouteWaypoint;
begin
...
begin
  ...
  begin
    ...
    else if SameText(ANodeName, 'RouteLeg') then
      ParseRouteLegNode(ANode[I])
    else if SameText(ANodeName, 'DistanceUnit') then
      FDistanceUnit := ANode[I].TextAsString
    else if SameText(ANodeName, 'DurationUnit') then
      FDurationUnit := ANode[I].TextAsString
    else if SameText(ANodeName, 'TravelDistance') then
      FTravelDistance := dxStrToFloat(ANode[I].TextAsString)
    else if SameText(ANodeName, 'TravelDuration') then
      FTravelDuration := dxStrToFloat(ANode[I].TextAsString)
    else if SameText(ANodeName, 'TravelDurationTraffic') then
      FTravelDurationTraffic := dxStrToFloat(ANode[I].TextAsString)
    else if SameText(ANodeName, 'BoundingBox') then
...
begin
  ...
  begin
    ...
    for I := 0 to ALineNode.Count - 1 do
      ...
      begin
        TdxBingMapDataXmlParser.ParsePointNode(ALineNode[I],
          FPath[APointIndex]);
        Inc(APointIndex);
      end;
...
begin
  ...
  begin
    ...
    if SameText(ANodeName, 'TravelMode') then
      ...
      FTravelDuration := dxStrToFloat(ANode[I].TextAsString)
    else if SameText(ANodeName, 'ManeuverPoint') then
      TdxBingMapDataXmlParser.ParsePointNode(ANode[I], FManeuverPoint)
    else if SameText(ANodeName, 'Instruction') then
      ParseInstruction(ANode[I])
    else if SameText(ANodeName, 'CompassDirection') then
      FCompassDirection := ANode[I].TextAsString
    else if SameText(ANodeName, 'ChildItineraryItems') then
      ParseChildItineraryItems(ANode[I])
    else if SameText(ANodeName, 'SideOfStreet') then
      FSideOfStreet := ANode[I].TextAsString
    else if SameText(ANodeName, 'Detail') then
      ParseDetail(ANode[I]);
  end;
end;
...
begin
  ...
  begin
    ...
    else if SameText(ANodeName, 'EndPathIndex') then
      FEndPathIndex := StrToInt(ANode[I].TextAsString)
    else if SameText(ANodeName, 'LocationCode') then
      FLocationCode := ANode[I].TextAsString
    else if SameText(ANodeName, 'ManeuverType') then
      FManeuverType := GetManeuverTypeByName(ANode[I].TextAsString)
    else if SameText(ANodeName, 'Name') then
      FNames.Add(ANode[I].TextAsString)
    else if SameText(ANodeName, 'RoadType') then
      FRoadType := ANode[I].TextAsString
    else if SameText(ANodeName, 'StartPathIndex') then
...
begin
  Result := Format(dxBingRouteDistanceUnitParam,
    [dxBingRouteDistanceUnitValues[Ord(FDistanceUnit)]]);
end;

function TdxBingMapRESTServiceCustomRouteParams.GetRoutePathOutputParam: string;
begin
  Result := Format(dxBingRoutePathOutput,
    [dxBingRoutePathOutputValues[IfThen(FRoutePathOutput, 1)]])
end;
...
begin
  ...
  begin
    ...
    FWayPointsParam := FWayPointsParam + IfThen(I > 0, dxUrlParamsDevider) +
      Format(IfThen(AWayPoint.PointType = bmwtWayPoint, dxBingWayPointParam,
      dxBingViaWayPointParam), [I, AWayPoint.AsString]);
  end;
end;
...
const
  ...
  AFExcludeTolls: array [TdxBingMapExcludeRoadType] of string = ('', 'tolls',
    'minimizeTolls');
var
...
begin
  ...
  Result := AAvoidHighways + IfThen((AAvoidHighways <> '') and
    (AAvoidTolls <> ''), ',') + AAvoidTolls;
  if Result <> '' then
...
begin
  Result := Format(dxBingRouteMaxSolutionsParam,
    [EnsureRange(FMaxSolutions, 1, 3)]);
end;

function TdxBingMapRESTServiceRouteParams.GetOptimizeParam: string;
begin
  Result := Format(dxBingRouteOptimizeParam,
    [dxBingRouteOptimizationType[Ord(FOptimize)]]);
end;

function TdxBingMapRESTServiceRouteParams.GetTimeTypeParam: string;
begin
  Result := Format(dxBingRouteTimeTypeParam,
    [dxBingRouteTimeType[Ord(FTimeType)]]);
end;
...
function TdxBingMapRESTServiceRoutesFromMajorRoadsParams.
  GetDestinationParam: string;
begin
  Result := Format(dxBingRouteDestinationParam, [FDestination]);
end;

function TdxBingMapRESTServiceRoutesFromMajorRoadsParams.
  GetRouteExcludeParam: string;
begin
...
function TdxBingMapCustomRouteDataService.CreateResponse
...
function TdxBingMapRoutesFromMajorRoadsDataService.CreateParams
