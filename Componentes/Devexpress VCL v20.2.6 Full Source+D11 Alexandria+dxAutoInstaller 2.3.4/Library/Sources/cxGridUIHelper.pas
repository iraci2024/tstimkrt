...
type
  ...
  private
    ...
    FIsPerform: Boolean;
  public
    constructor Create(AView: TcxCustomGridView;
      AIsPerform: Boolean = True); virtual;
    property View: TcxCustomGridView read FView;
    property IsPerform: Boolean read FIsPerform;
  end;

  TcxShowingGridOperationHelperParameters = class
  ...
  TcxGridOperationHelperProc = procedure(const AParameter
    : TcxCustomGridOperationHelperParameters) of object;

  TcxCustomGridViewOperationHelper = class
  private
    FList: TList;
  protected
    procedure RegisterOperation(AOperationIndex: Integer;
      const AProc: TcxGridOperationHelperProc);
    procedure RegisterOperations; virtual;
    function GetShowProperty(const AParameter
      : TcxCustomGridOperationHelperParameters): Boolean;
    procedure SetShowProperty(const AParameter
      : TcxCustomGridOperationHelperParameters; AShow: Boolean);
    ...
    procedure DoInsert(const AParameter
      : TcxCustomGridOperationHelperParameters);
    procedure DoDelete(const AParameter
      : TcxCustomGridOperationHelperParameters);
  public
    ...
    function IsOperationEnabled(AView: TcxCustomGridView;
      AOperationIndex: Integer): Boolean; virtual;
    procedure PerformOperation(AOperationIndex: Integer;
      const AParameter: TcxCustomGridOperationHelperParameters); virtual;
  end;

  TcxCustomGridViewOperationHelperClass = class of
  ...
  public
    ...
    property IsOperationAccessible[AOperationIndex: Integer]: Boolean
      read GetIsOperationAccessible;
    property IsOperationEnabled[AOperationIndex: Integer]: Boolean
      read GetIsOperationEnabled;
    property IsOperationShowing[AOperationIndex: Integer]: Boolean
      read GetIsOperationShowing;
  public
    procedure DoFirst;
    procedure DoLast;
    procedure DoNext;
    ...
    property OnUpdateOperations: TNotifyEvent read FOnUpdateOperations
      write FOnUpdateOperations;
    property onFocusedRecordChanged: TNotifyEvent read FOnFocusedRecordChanged
      write FOnFocusedRecordChanged;
    property OnCustomizationFormVisibleChanged: TNotifyEvent
      read FOnCustomizationFormVisibleChanged
...
procedure RegisterGridViewOperationHelper(AGridViewOperationHelperClass
  : TcxCustomGridViewOperationHelperClass);
procedure UnregisterGridViewOperationHelper(AGridViewOperationHelperClass
  : TcxCustomGridViewOperationHelperClass);
function GetGridViewOperationHelperByGridViewClass(const AGridViewClass
  : TcxCustomGridViewClass): TcxCustomGridViewOperationHelper;
...
procedure TcxGridNotifications.Notify(AKind: TcxGridNotificationKind;
...
begin
  Result := [gnkFocusedViewChanged, gnkFocusedRecordChanged,
    gnkRecordCountChanged, gnkCustomization];
end;
...
class function TcxCustomGridViewOperationHelper.GetViewClass
  : TcxCustomGridViewClass;
begin
  Result := TcxCustomGridView;
end;

procedure TcxCustomGridViewOperationHelper.RegisterOperation(AOperationIndex
  : Integer; const AProc: TcxGridOperationHelperProc);
var
...
function TcxCustomGridViewOperationHelper.IsOperationAccessible(AOperationIndex
  : Integer): Boolean;
var
  I: Integer;
begin
  Result := False;
  for I := 0 to FList.Count - 1 do
    if PcxCustomGridViewOperationHelperRec(FList[I]).Index = AOperationIndex
    then
    begin
      Result := True;
      break;
...
function TcxCustomGridViewOperationHelper.IsOperationEnabled
...
procedure TcxCustomGridViewOperationHelper.PerformOperation(AOperationIndex
  : Integer; const AParameter: TcxCustomGridOperationHelperParameters);
var
  I: Integer;
begin
  for I := 0 to FList.Count - 1 do
...
function TcxCustomGridViewOperationHelper.GetShowProperty(const AParameter
  : TcxCustomGridOperationHelperParameters): Boolean;
begin
...
procedure TcxCustomGridViewOperationHelper.SetShowProperty(const AParameter
  : TcxCustomGridOperationHelperParameters; AShow: Boolean);
begin
  if AParameter is TcxShowingGridOperationHelperParameters then
...
procedure TcxCustomGridViewOperationHelper.DoFirst(const AParameter
  : TcxCustomGridOperationHelperParameters);
begin
  AParameter.View.DataController.GotoFirst;
end;

procedure TcxCustomGridViewOperationHelper.DoLast(const AParameter
  : TcxCustomGridOperationHelperParameters);
begin
  AParameter.View.DataController.GotoLast;
end;

procedure TcxCustomGridViewOperationHelper.DoNext(const AParameter
  : TcxCustomGridOperationHelperParameters);
begin
  AParameter.View.DataController.GotoNext;
end;

procedure TcxCustomGridViewOperationHelper.DoPrev(const AParameter
  : TcxCustomGridOperationHelperParameters);
begin
  AParameter.View.DataController.GotoPrev;
end;

procedure TcxCustomGridViewOperationHelper.DoInsert(const AParameter
  : TcxCustomGridOperationHelperParameters);
begin
  AParameter.View.DataController.Insert;
end;

procedure TcxCustomGridViewOperationHelper.DoDelete(const AParameter
  : TcxCustomGridOperationHelperParameters);
begin
  if AParameter.View.DataController.GetSelectedCount > 0 then
    AParameter.View.DataController.DeleteSelection
  else
    AParameter.View.DataController.DeleteFocused;
end;
...
begin
  ...
  TcxGridNotifications(FFocusedViewChangedNotification)
    .FGridOperationHelper := Self;
end;
...
procedure TcxCustomGridOperationHelper.PerformOperation(AOperationIndex
  : Integer; const AParameters: TcxCustomGridOperationHelperParameters);
begin
  if IsOperationEnabled[AOperationIndex] then
    GetGridViewOperationHelperByGridViewClass(FocusedViewClass)
      .PerformOperation(AOperationIndex, AParameters);
end;

function TcxCustomGridOperationHelper.GetIsOperationAccessible(AOperationIndex
  : Integer): Boolean;
var
...
begin
  if FocusedView <> nil then
  begin
    AGridViewHelper := GetGridViewOperationHelperByGridViewClass
      (FocusedViewClass);
    Result := (AGridViewHelper <> nil) and AGridViewHelper.IsOperationAccessible
      (AOperationIndex);
  end
  else
    Result := False;
...
function TcxCustomGridOperationHelper.GetIsOperationEnabled(AOperationIndex
  : Integer): Boolean;
begin
  Result := IsOperationAccessible[AOperationIndex] and
    GetGridViewOperationHelperByGridViewClass(FocusedViewClass)
    .IsOperationEnabled(FocusedView, AOperationIndex);
end;

procedure TcxCustomGridOperationHelper.DoFocusedViewChanged;
begin
  if not(csDestroying in ComponentState) and not(csDesigning in ComponentState)
    and (Grid <> nil) and not(csDestroying in Grid.ComponentState) then
    DoUpdateOperations;
end;
...
procedure TcxCustomGridOperationHelper.PerformStandardOperation(AOperationIndex
  : Integer);
var
...
procedure TcxCustomGridOperationHelper.PerformShowingOperation(AOperationIndex
...
function TcxCustomGridOperationHelper.GetIsOperationShowing(AOperationIndex
  : Integer): Boolean;
var
  AParameters: TcxShowingGridOperationHelperParameters;
begin
  AParameters := TcxShowingGridOperationHelperParameters.Create
    (FocusedView, False);
  try
    PerformOperation(AOperationIndex, AParameters);
...
begin
  for I := 0 to FGridViewOperationHelperList.Count - 1 do
    if TcxCustomGridViewOperationHelper(FGridViewOperationHelperList[I])
      .ClassType = AGridViewOperationHelperClass then
    begin
