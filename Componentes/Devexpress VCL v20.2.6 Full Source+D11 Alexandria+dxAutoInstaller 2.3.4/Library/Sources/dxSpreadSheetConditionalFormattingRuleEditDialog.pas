...
{$ENDIF}
  ...
  dxCore, cxGraphics, dxForms, cxControls, cxLookAndFeels,
  ...
  cxCustomData, cxStyles, cxTL, cxInplaceContainer, cxMaskEdit, cxDropDownEdit,
  cxTextEdit, cxEdit,
  cxRadioGroup, cxCheckBox, cxSpinEdit, cxColorComboBox, cxImageComboBox,
...
type
  ...
  TfrmSpreadSheetConditionalFormattingRuleEditDialogClass = class of
    TfrmSpreadSheetConditionalFormattingRuleEditDialog;
  ...
  TfrmSpreadSheetConditionalFormattingRuleEditDialog = class(TdxForm)
    ...
    procedure tlIconSetColumnStopValueGetEditingProperties
    ...
    procedure tlIconSetColumnGetDisplayText(Sender: TcxTreeListColumn;
    ...
    procedure tlIconSetEdited(Sender: TcxCustomTreeList;
      AColumn: TcxTreeListColumn);
    procedure tlIconSetEditing(Sender: TcxCustomTreeList;
      AColumn: TcxTreeListColumn; var Allow: Boolean);
    procedure tlRuleTypeFocusedNodeChanged(Sender: TcxCustomTreeList;
  ...
  strict private
    ...
    function GetEditMask(AValueType
  ...
  protected
    ...
    //
    property Helper: TdxSpreadSheetConditionalFormattingRuleEditDialogHelper
      read FHelper;
    property HelperIconSet
      : TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper
      read FHelperIconSet;
    property HelperScale
...
var
  dxSpreadSheetConditionalFormattingRuleEditDialogClass
    : TfrmSpreadSheetConditionalFormattingRuleEditDialogClass =
    TfrmSpreadSheetConditionalFormattingRuleEditDialog;
...
type
  ...
  TdxSpreadSheetConditionalFormattingCellStyleEditDialogSingleStyleController =
    class(TdxSpreadSheetCellStyleEditDialogSingleStyleController)
  public
    constructor Create(AConditionalFormatting
      : TdxSpreadSheetCustomConditionalFormatting;
      AStyle: TdxSpreadSheetCellStyle); reintroduce;
  end;
...
begin
  ...
  Assert(Result);
  Result := Result and ShowConditionalFormattingRuleEditDialog
    (AOwnerIntf, ARule);
end;
...
begin
  ...
  begin
    ADialog := dxSpreadSheetConditionalFormattingRuleEditDialogClass.
      Create(AOwner);
    try
...
constructor
  TdxSpreadSheetConditionalFormattingCellStyleEditDialogSingleStyleController.
  Create(AConditionalFormatting: TdxSpreadSheetCustomConditionalFormatting;
  AStyle: TdxSpreadSheetCellStyle);
begin
  inherited Create(AConditionalFormatting.CellStyles, AStyle);
  if not TdxSpreadSheetCustomConditionalFormattingAccess(AConditionalFormatting)
    .IsStyleBorderSupported then
    Exclude(FCapabilities, csecBorder);
  if not TdxSpreadSheetCustomConditionalFormattingAccess(AConditionalFormatting)
    .IsValueFormattingSupported then
    Exclude(FCapabilities, csecNumber);
end;

{ TdxSpreadSheetConditionalFormattingRuleEditDialogForm }

constructor TfrmSpreadSheetConditionalFormattingRuleEditDialog.Create
...
begin
  ...
  FHelper := TdxSpreadSheetConditionalFormattingRuleEditDialogHelper.Create
    (AConditionalFormatting);
  FHelperIconSet :=
    TdxSpreadSheetConditionalFormattingRuleEditDialogIconSetHelper.Create
    (tlIconSet);
  EditRepositoryFloatMask.Properties.EditMask := '\-?\d+(\' +
    dxFormatSettings.DecimalSeparator + '\d{0,})?([eE][-+]?\d+)?';
  ApplyLocalizations;

  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    PopulateAboveOrBelowAverageOperators
    (cbbAverageComparisonOperator.Properties.Items);
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    PopulateCellIsOperators(cbbOperator.Properties.Items);
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.PopulateRuleTypes
    (tlRuleType, Helper);
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    PopulateTopBottomDirections(cbbTopBottomValues.Properties.Items);
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    PopulateScaleStopTypes(cbbScaleMinStopValueType);
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    PopulateScaleStopTypes(cbbScaleMidStopValueType);
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    PopulateScaleStopTypes(cbbScaleMaxStopValueType);

  HelperScale.BindEditors(cbbScaleMinStopValueType, meScaleMinStopValue,
    ccbColorScaleMinStopColor);
  HelperScale.BindEditors(cbbScaleMidStopValueType, meScaleMidStopValue,
    ccbColorScaleMidStopColor);
  HelperScale.BindEditors(cbbScaleMaxStopValueType, meScaleMaxStopValue,
    ccbColorScaleMaxStopColor);

  HelperIconSet.PopulateIconSets(icbIconStyle);
  HelperIconSet.PopulateComparisonOperators
    (tlIconSetColumnComparisonOperation.Properties as TcxComboBoxProperties);
  HelperIconSet.PopulateStopValueTypes
    (tlIconSetColumnStopValueType.Properties as TcxComboBoxProperties);
  HelperIconSet.PopulateIcons
    (tlIconSetColumnIcon.Properties as TcxImageComboBoxProperties);
  icbIconStyle.Properties.Images := ConditionalFormattingIconSet.PresetPreviews;

  teExpression1.Properties.EditMask :=
    TdxSpreadSheetCustomConditionalFormattingAccess
    (Helper.ConditionalFormatting).GetFormulaEditMask;
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.SelectRuleType
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.Load
  (ARule: TdxSpreadSheetCustomConditionalFormattingRule);
var
  ANode: TcxTreeListNode;
begin
  if TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.FindRuleType
    (tlRuleType, ARule.ClassType, ANode) then
  begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  meScaleStopValueChanged(Sender: TObject);
var
  AEdit: TcxMaskEdit;
begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.Save
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.DoRuleTypeSelected
  (ANode: TcxTreeListNode);
begin
  StoreRuleInfo;
  Helper.SelectedRuleInfo :=
    TdxSpreadSheetConditionalFormattingRuleEditDialogRuleInfo(ANode.Data);
  RestoreRuleInfo
end;
...
function TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  CanShowFormatDialog: Boolean;
var
  AStyle: TdxSpreadSheetCellStyle;
begin
  Result := (Helper.SelectedRuleInfo <> nil) and
    (Helper.SelectedRuleInfo.GetStyle(AStyle) or
    (Helper.SelectedRuleInfo.RuleType = cfrtDataBar));
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  ShowFormatCellDialog(AStyle: TdxSpreadSheetCellStyle);
var
...
begin
  ...
  try
    ADialog.Initialize
      (TdxSpreadSheetConditionalFormattingCellStyleEditDialogSingleStyleController.
      Create(Helper.ConditionalFormatting, AStyle));
    ADialog.Load;
    ...
    if Helper.SelectedRuleInfo <> nil then
      case Helper.SelectedRuleInfo.RuleType of
        cfrtTwoColorScale, cfrtThreeColorScale, cfrtDataBar:
          RestoreRuleScaleInfo
            (TdxSpreadSheetConditionalFormattingRuleCustomColorScale
            (Helper.SelectedRulePreview));
        cfrtCellIs:
          RestoreRuleCellIsInfo(TdxSpreadSheetConditionalFormattingRuleCellIs
            (Helper.SelectedRulePreview));
        cfrtExpression:
          RestoreRuleExpressionInfo
            (TdxSpreadSheetConditionalFormattingRuleExpression
            (Helper.SelectedRulePreview));
        cfrtAboveOrBelowAverage:
          RestoreRuleAboveOrBelowAverage
            (TdxSpreadSheetConditionalFormattingRuleAboveOrBelowAverage
            (Helper.SelectedRulePreview));
        cfrtTopBottomValues:
          RestoreRuleTopBottomInfo
            (TdxSpreadSheetConditionalFormattingRuleTopBottomValues
            (Helper.SelectedRulePreview));
        cfrtIconSet:
          RestoreRuleIconSet(TdxSpreadSheetConditionalFormattingRuleIconSet
            (Helper.SelectedRulePreview));
...
begin
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    LoadAboveOrBelowAverageRuleInfo(cbbAverageComparisonOperator, ARule);
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  RestoreRuleCellIsInfo(ARule: TdxSpreadSheetConditionalFormattingRuleCellIs);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.RestoreRuleIconSet
  (ARule: TdxSpreadSheetConditionalFormattingRuleIconSet);
begin
  icbIconStyle.ItemIndex := HelperIconSet.GetIconSetIndex
    (icbIconStyle.Properties.Items, ARule.PresetName);
...
begin
  ...
  lciColorScaleMinStopColor.Visible := HelperScale.Stops[0]
    is TdxSpreadSheetConditionalFormattingRuleColorScaleStop;
  lciColorScaleMidStopColor.Visible := HelperScale.Stops[1]
    is TdxSpreadSheetConditionalFormattingRuleColorScaleStop;
  lciColorScaleMaxStopColor.Visible := HelperScale.Stops[2]
    is TdxSpreadSheetConditionalFormattingRuleColorScaleStop;

  lciShowBarOnly.Visible :=
    ARule is TdxSpreadSheetConditionalFormattingRuleDataBar;
  if lciShowBarOnly.Visible then
    cbShowBarOnly.Checked := not TdxSpreadSheetConditionalFormattingRuleDataBar
      (Helper.SelectedRulePreview).ShowValue;
...
begin
  if Helper.SelectedRuleInfo <> nil then
    case Helper.SelectedRuleInfo.RuleType of
      cfrtCellIs:
        StoreRuleCellIsInfo(TdxSpreadSheetConditionalFormattingRuleCellIs
          (Helper.SelectedRulePreview));
      cfrtExpression:
        StoreRuleExpressionInfo
          (TdxSpreadSheetConditionalFormattingRuleExpression
          (Helper.SelectedRulePreview));
      cfrtAboveOrBelowAverage:
        StoreRuleAboveOrBelowAverage
          (TdxSpreadSheetConditionalFormattingRuleAboveOrBelowAverage
          (Helper.SelectedRulePreview));
      cfrtTopBottomValues:
        StoreRuleTopBottomInfo
          (TdxSpreadSheetConditionalFormattingRuleTopBottomValues
          (Helper.SelectedRulePreview));
    end;
end;
...
begin
  TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
    SaveAboveOrBelowAverageRuleInfo(cbbAverageComparisonOperator, ARule);
end;
...
begin
  ...
  ARule.Direction :=
    ...
    (cbbTopBottomValues.ItemIndex);
  ARule.Value := seTopBottomValuesRank.Value;
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.btnFormatClick
  (Sender: TObject);
var
...
begin
  ...
  begin
    ...
    try
      ShowConditionalFormattingDataBarRuleStyleEditDialog(FOwner,
        ADataBar.Style);
    finally
      ADataBar.EndUpdate;
    end;
  end
  else if Helper.SelectedRuleInfo.GetStyle(AStyle) then
    ShowFormatCellDialog(AStyle);

  pbPreview.Invalidate;
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.btnOkClick
  (Sender: TObject);
...
begin
  try
    ...
    if Helper.SelectedRuleInfo <> nil then
      case Helper.SelectedRuleInfo.RuleType of
        cfrtIconSet:
          Check(HelperIconSet.AreActualValuesInValidOrder,
            @sdxConditionalFormattingRuleEditDialogErrorIncorrectStopsOrder);
        cfrtDataBar, cfrtTwoColorScale, cfrtThreeColorScale:
          Check(HelperScale.AreActualValuesInValidOrder,
            @sdxConditionalFormattingRuleEditDialogErrorIncorrectStopsOrder);
        cfrtExpression:
          Check(Helper.AreExpressionsValid
            (TdxSpreadSheetConditionalFormattingRuleExpression
            (Helper.SelectedRulePreview)),
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  cbbScaleStopValueTypePropertiesChange(Sender: TObject);
var
  AEdit: TcxComboBox;
  AValueType: TdxSpreadSheetConditionalFormattingRuleCustomScaleStopValueType;
begin
  ...
  begin
    AValueType := TdxSpreadSheetConditionalFormattingRuleEditDialogUIHelper.
      GetScaleStopValueType(AEdit);
    HelperScale.Stops[AEdit.Tag].ValueType := AValueType;
    HelperScale.EditorValue[AEdit.Tag].RepositoryItem :=
      GetEditMask(AValueType);
    HelperScale.EditorValue[AEdit.Tag].Enabled := AValueType <> cssvtLimitValue;
    HelperScale.EditorValue[AEdit.Tag].EditValue := HelperScale.Stops
      [AEdit.Tag].Value;
  end;
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  cbReverseIconOrderChanged(Sender: TObject);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  cbShowBarOnlyChanged(Sender: TObject);
begin
  if not FIsLoading then
    TdxSpreadSheetConditionalFormattingRuleDataBar(Helper.SelectedRulePreview)
      .ShowValue := not cbShowBarOnly.Checked;
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  cbShowIconOnlyChanged(Sender: TObject);
begin
  if not FIsLoading then
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  ccbColorScaleStopColorPropertiesChange(Sender: TObject);
var
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  ccbOperatorPropertiesChange(Sender: TObject);
begin
  lciExpression2.Visible :=
    TdxSpreadSheetConditionalFormattingRuleCellIsComparisonOperator
    (cbbOperator.ItemObject) in [cicoBetween, cicoNotBetween];
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.pbPreviewPaint
  (Sender: TObject);
begin
  if Helper.SelectedRuleInfo <> nil then
  begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.icbIconStyleChanged
  (Sender: TObject);
begin
  if not FIsLoading then
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  tlIconSetColumnComparisonOperationChanged(Sender: TObject);
begin
  HelperIconSet.SetComparisonOperator(tlIconSet.FocusedNode,
    tlIconSetColumnComparisonOperation);
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  tlIconSetColumnGetDisplayText(Sender: TcxTreeListColumn;
  ANode: TcxTreeListNode; var Value: string);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  tlIconSetColumnIconChanged(Sender: TObject);
begin
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  tlIconSetColumnStopValueGetEditingProperties(Sender: TcxTreeListColumn;
  ANode: TcxTreeListNode; var EditProperties: TcxCustomEditProperties);
var
  AItem: TcxEditRepositoryItem;
begin
  AItem := GetEditMask(HelperIconSet.GetStopType(ANode,
    tlIconSetColumnStopValueType));
  if AItem <> nil then
    EditProperties := AItem.Properties;
end;

procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  tlIconSetColumnStopValueTypeChanged(Sender: TObject);
begin
  HelperIconSet.SetStopValueType(tlIconSet.FocusedNode,
    HelperIconSet.GetStopType(tlIconSet.FocusedNode,
    tlIconSetColumnStopValueType));
end;
...
begin
  if not FIsLoading and (AColumn = tlIconSetColumnStopValue) then
    HelperIconSet.SetStopValue(Sender.FocusedNode,
      Sender.FocusedNode.Values[AColumn.ItemIndex]);
end;
...
procedure TfrmSpreadSheetConditionalFormattingRuleEditDialog.
  tlRuleTypeFocusedNodeChanged(Sender: TcxCustomTreeList;
  APrevFocusedNode, AFocusedNode: TcxTreeListNode);
begin
