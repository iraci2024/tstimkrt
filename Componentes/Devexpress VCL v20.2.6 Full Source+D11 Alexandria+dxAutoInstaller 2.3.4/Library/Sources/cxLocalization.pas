...
type
  ...
  public
    ...
    function FindTranslation(const AResStringName: string;
  ...
  private
    ...
    procedure TranslateResString(const AResStringName: string;
      AResString: Pointer);
  protected
    function GetItem(Index: Integer): TcxLanguage;
    function GetOwner: TPersistent; override;
    function GetLocalizedString(const AResStringName: string;
      out AValue: string): Boolean;
    ...
    property ActiveLanguage: Integer read FActiveLanguage
      write SetActiveLanguage;
    property OnLanguagesChanged: TNotifyEvent read FOnLanguagesChanged
      write FOnLanguagesChanged;
  public
    constructor Create(AOwner: TPersistent);
    destructor Destroy; override;
    function Add: TcxLanguage;
    function GetLanguageByName(AName: string): Integer;
    procedure SetCustomResStringOriginalValue(const AResStringName,
      AValue: string);

    property CustomResStrings: TStrings read FCustomResStrings;
    property Items[Index: Integer]: TcxLanguage read GetItem
  ...
  protected
    procedure Clear;
    function DoCustomTranslate(const AResStringName: string;
      out ALocalizedValue: string): Boolean;
  ...
  public
    ...
    property LanguageIndex: Integer read GetLanguageIndex
  ...
  published
    ...
    property Locale: Cardinal read GetLocale write SetLocale
      stored IsLocaleStored;
    property StorageType: TcxLocalizerStorageType read FStorageType
      write SetStorageType default lstIni;
    property OnTranslate: TcxLocalizerTranslateEvent read FOnTranslate
      write FOnTranslate;
  end;
  ...
  public
    ...
    procedure SaveToStream(AStream: TStream; ALanguages: TcxLanguages;
      AIsAnsiSaveFormat: Boolean);
  end;
...
function TcxLanguage.FindTranslation(const AResStringName: string;
  out AValue: string): Boolean;
var
  AIndex: Integer;
begin
...
procedure TcxLanguages.SetCustomResStringOriginalValue(const AResStringName,
  AValue: string);
begin
  if AResStringName <> '' then
...
function TcxLanguages.GetLocalizedString(const AResStringName: string;
  out AValue: string): Boolean;
begin
  AValue := '';
  Result := False;
...
procedure TcxLanguages.TranslateResString(const AResStringName: string;
  AResString: Pointer);
var
...
var
  AStream: TStream;
begin
  AStream := TResourceStream.Create(AHandle, 'CXLOCALIZATION',
    PChar('CXLOCALIZATION'));
  try
...
procedure TcxLocalizer.SaveToFile(const AFileName: TFileName;
  AIsAnsiSaveFormat: Boolean);
var
  AFileStream: TFileStream;
begin
...
procedure TcxLocalizer.SaveToStream(AStream: TStream;
  AIsAnsiSaveFormat: Boolean);
begin
...
function TcxLocalizer.DoCustomTranslate(const AResStringName: string;
  out ALocalizedValue: string): Boolean;
begin
  Result := False;
  if Assigned(FOnTranslate) then
  begin
    ALocalizedValue := dxResourceStringsRepository.GetOriginalValue
      (AResStringName);
...
procedure TcxLocalizerStorage.LoadFromStream(AStream: TStream;
  ALanguages: TcxLanguages);
  ...
  procedure ReadSectionValues(AIniFile: TCustomIniFile;
    const ASectionName: string; AList: TStrings);
  var
    I: Integer;
    AName, AValue: string;
  begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        ReadSectionValues(AMemIniFile, ASectionName,
          ALanguages.CustomResStrings);
    end;
  finally
    FreeAndNil(AList);
...
procedure TcxLocalizerStorage.SaveToStream(AStream: TStream;
  ALanguages: TcxLanguages; AIsAnsiSaveFormat: Boolean);

  procedure WriteSection(AIniFile: TCustomIniFile; const ASectionName: string;
    AList: TStrings);
  var
    I: Integer;
    AName, AValue: string;
...
begin
  ...
  try
    ...
    begin
      WriteSection(AMemIniFile, IntToStr(ALanguages[I].LocaleID),
        ALanguages[I].Dictionary);
    ...
    WriteSection(AMemIniFile, ACustomResourceStrings,
      ALanguages.CustomResStrings);
    AMemIniFile.GetStrings(AList);
    if AIsAnsiSaveFormat then
      AEncoding := nil
...
function TcxLocalizerStorage.GetEncodingFromLocaleID(ALocaleID: Integer)
  : TEncoding;
var
  ACodePage: Integer;
  ABuffer: array [0 .. 6] of Char;
begin
  GetLocaleInfo(ALocaleID, LOCALE_IDEFAULTANSICODEPAGE, ABuffer,
    SizeOf(ABuffer));
