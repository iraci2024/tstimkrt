...
type
  ...
  TdxWizardControlFormHelper = class(TcxIUnknownObject,
  ...
  protected
    ...
    function HitAtExtendedCaptionArea(const AScreenPoint: TPoint): Boolean;
      overload; virtual;
    function HitAtExtendedCaptionArea(const AScreenPoint: TSmallPoint)
      : Boolean; overload;
  ...
  public
    ...
    property GlassFrameTopExtend: Integer read FGlassFrameTopExtend
      write SetGlassFrameTopExtend;
    ...
    property WizardControlTitleViewInfo: TdxWizardControlCustomTitleViewInfo
      read GetWizardControlTitleViewInfo;
  end;
...
type
  ...
  public
    class function GetController(AControl: TdxCustomWizardControl)
      : TdxWizardControlController;
    class function GetViewInfo(AControl: TdxCustomWizardControl)
      : TdxWizardControlViewInfo;
    class function IsResizeAnimationActive
      (AControl: TdxCustomWizardControl): Boolean;
    class procedure RecalculateLayout(AControl: TdxCustomWizardControl);
    class procedure SetFormHelper(AControl: TdxCustomWizardControl;
      AHelper: IdxWizardControlFormHelper);
  end;
...
class function TdxWizardControlHelper.IsResizeAnimationActive
  (AControl: TdxCustomWizardControl): Boolean;
begin
  Result := (AControl <> nil) and TdxCustomWizardControlAccess(AControl)
    .IsResizingAnimationActive;
end;

class procedure TdxWizardControlHelper.RecalculateLayout
...
begin
  Result := not IsDesigning and IsCompositionEnabled and (WizardControl <> nil)
    and WizardControl.OptionsViewStyleAero.EnableTitleAero and
    CheckWizardControlAlignment(WizardControl);
end;

function TdxWizardControlFormHelper.HitAtExtendedCaptionArea(const AScreenPoint
  : TPoint): Boolean;
var
...
procedure TdxWizardControlFormHelper.Notification(AComponent: TComponent;
...
begin
  SendMessage(Form.Handle, DXM_SKINS_SETISSKINNED,
    Ord(IsGlassFrameExtended), 0);
end;
...
function TdxWizardControlFormHelper.ProcessChildHitTest
  (var Message: TWMNCHitTest): Boolean;
begin
...
procedure TdxWizardControlFormHelper.ProcessWindowPosChanging
...
begin
  ...
  if Assigned(DwmExtendFrameIntoClientArea) and
...
function TdxWizardControlFormHelper.GetWizardControlTitleViewInfo
...
procedure TdxWizardControlFormHelper.SetWizardControl
...
procedure TdxWizardControlForm.AlignControls(AControl: TControl;
  var ARect: TRect);
begin
...
procedure TdxWizardControlForm.Notification(AComponent: TComponent;
  AOperation: TOperation);
begin
...
begin
  ...
  begin
    if (Helper.WizardControl = nil) and
      (Message.Control is TdxCustomWizardControl) then
      Helper.WizardControl := TdxCustomWizardControl(Message.Control);
  end
  else if Message.Control = Helper.WizardControl then
...
var
  ASaveIndex: Integer;
begin
  if (Helper <> nil) and Helper.IsGlassFrameExtended and
    not cxIsDrawToMemory(Message) then
  begin
  ...
  if Helper <> nil then
    case Message.Msg of
      ...
      WM_WINDOWPOSCHANGING:
        Helper.ProcessWindowPosChanging(TWMWindowPosChanging(Message)
          .WindowPos);
    end;
end;
