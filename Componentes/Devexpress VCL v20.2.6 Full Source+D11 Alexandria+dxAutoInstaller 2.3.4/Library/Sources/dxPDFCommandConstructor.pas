...
uses
  Types, Classes, Generics.Defaults, Generics.Collections, SysUtils,
  dxCoreClasses, cxGeometry, dxPDFBase, dxPDFTypes,
...
type
  ...
  TdxPDFStringFormatFlags = (sffMeasureTrailingSpaces = 1, sffNoWrap = 2,
    sffLineLimit = 4, sffNoClip = 8); // for internal use
  TdxPDFStringTrimming = (stNone, stCharacter, stWord, stEllipsisCharacter,
    stEllipsisWord); // for internal use
  ...
  public
    ...
    class function Create(AFormatFlags: TdxPDFStringFormatFlags)
      : TdxPDFStringFormat; overload; static;
    class function Create(AFormat: TdxPDFStringFormat): TdxPDFStringFormat;
      overload; static;
    class function CreateGenericDefault: TdxPDFStringFormat; overload; static;
    class function CreateGenericTypographic: TdxPDFStringFormat;
      overload; static;
    function Clone: TdxPDFStringFormat;

    property Alignment: TdxPDFStringAlignment read FAlignment write FAlignment;
    property DirectionRightToLeft: Boolean read FDirectionRightToLeft
      write FDirectionRightToLeft;
    property FormatFlags: TdxPDFStringFormatFlags read FFormatFlags
      write FFormatFlags;
    property HotkeyPrefix: TdxPDFHotkeyPrefix read FHotkeyPrefix
      write FHotkeyPrefix;
    property LeadingMarginFactor: Double read FLeadingMarginFactor
      write FLeadingMarginFactor;
    property LineAlignment: TdxPDFStringAlignment read FLineAlignment
      write FLineAlignment;
    property TabStopInterval: Double read FTabStopInterval
      write FTabStopInterval;
    property TrailingMarginFactor: Double read FTrailingMarginFactor
      write FTrailingMarginFactor;
  ...
  strict private
    ...
    procedure DrawXObject(AXObjectNumber: Integer;
      const AMatrix: TdxPDFTransformationMatrix);
    procedure GeneratePathCommands(APath: TdxPDFGraphicsPath);
    procedure SetCurrentTransformationMatrix(const AValue
  ...
  public
    ...
    procedure IntersectClip(APath: TdxPDFGraphicsPath;
      ANonZero: Boolean = True); overload;
    procedure IntersectClip(APaths: TList<TdxPDFGraphicsPath>;
      ANonZero: Boolean); overload;
    ...
    procedure ModifyTransformationMatrix(const AMatrix
      : TdxPDFTransformationMatrix);
    ...
    procedure SetColorForStrokingOperations(const AColor: TdxPDFColor);
      overload;
    procedure SetColorForStrokingOperations(const AColor
      : TdxPDFARGBColor); overload;
    procedure SetGraphicsStateParameters(AParameters
    ...
    property CurrentTransformationMatrix: TdxPDFTransformationMatrix
      read FCurrentTransformationMatrix write SetCurrentTransformationMatrix;
  end;

  { TdxPDFFormCommandConstructor }

  TdxPDFFormCommandConstructor = class(TdxPDFCommandConstructor)
  // for internal use
  strict private
  ...
  TdxPDFAnnotationAppearanceBuilder<T: TdxPDFCustomAnnotation> = class
    (TInterfacedObject, IdxPDFAnnotationAppearanceBuilder) // for internal use
  strict private
    FAnnotation: T;
  protected
    procedure Rebuild(AConstructor: TdxPDFFormCommandConstructor);
      virtual; abstract;
  ...
  TdxPDFMarkupAnnotationAppearanceBuilder<T: TdxPDFMarkupAnnotation> = class
    (TdxPDFAnnotationAppearanceBuilder<TdxPDFMarkupAnnotation>)
  // for internal use
  protected
  ...
  TdxPDFWidgetAnnotationAppearanceBuilder<T: TdxPDFInteractiveFormField> = class
    (TdxPDFAnnotationAppearanceBuilder<TdxPDFWidgetAnnotation>)
  // for internal use
  strict private
    FBorderWidth: Double;
    FFormField: T;
    procedure DrawStyledBorder(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle;
      ABorderStyle: TdxPDFAnnotationBorderStyle);
  protected
    procedure Rebuild(AConstructor: TdxPDFFormCommandConstructor); override;

    function GetBackgroundColor: TdxPDFColor; virtual;
    procedure DrawBeveledBorder(AConstructor: TdxPDFFormCommandConstructor);
      virtual; abstract;
    procedure DrawContent(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle); virtual; abstract;
    procedure DrawInsetBorder(AConstructor: TdxPDFFormCommandConstructor);
      virtual; abstract;
    procedure DrawSolidBorder(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle); virtual; abstract;
    procedure DrawUnderlineBorder(AConstructor: TdxPDFFormCommandConstructor);
      virtual; abstract;
    procedure FillBackground(AConstructor: TdxPDFFormCommandConstructor);
      virtual; abstract;

    procedure DrawBorder(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle);
    procedure DrawRectangularBeveledBorder(AConstructor
      : TdxPDFFormCommandConstructor);
    procedure DrawRectangularBorderBottomRightStroke(AConstructor
      : TdxPDFFormCommandConstructor);
    procedure DrawRectangularBorderStroke(AConstructor
      : TdxPDFFormCommandConstructor);
    procedure DrawRectangularBorderUpperLeftStroke(AConstructor
      : TdxPDFFormCommandConstructor);
    procedure DrawRectangularInsetBorder(AConstructor
      : TdxPDFFormCommandConstructor);
    procedure DrawRectangularUnderlineBorder(AConstructor
      : TdxPDFFormCommandConstructor);
    procedure DrawTextCombs(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxRectF; AMaxLen: Integer);
    procedure FillBackgroundEllipse(AConstructor: TdxPDFFormCommandConstructor;
      const ARect: TdxRectF; const AColor: TdxPDFColor);
    procedure FillBackgroundRectangle(AConstructor
      : TdxPDFFormCommandConstructor; const ARect: TdxPDFRectangle;
  ...
  strict private
    ...
    FFontSize: Single;
  protected
    procedure FillBackground(AConstructor
      : TdxPDFFormCommandConstructor); override;
    procedure Rebuild(AConstructor: TdxPDFFormCommandConstructor); override;

    function CalculateCenteredLineYOffset(const AClipRect
      : TdxPDFRectangle): Double;
    function GetTextWidth(const AText: string; AFontSize: Double): Double;
    procedure DrawTextBoxText(AConstructor: TdxPDFFormCommandConstructor;
    ...
    procedure StartDrawTextBox(AConstructor: TdxPDFFormCommandConstructor;
  ...
  public
    constructor Create(AWidget: TdxPDFWidgetAnnotation; AFormField: T;
      AState: TdxPDFDocumentState); reintroduce;
  end;

  { TdxPDFTextFieldAppearanceBuilder<T> }

  TdxPDFTextFieldAppearanceBuilder<T: TdxPDFInteractiveFormField> = class
    (TdxPDFTextBasedFormFieldAppearanceBuilder<T>) // for internal use
  protected
  ...
  TdxPDFTextFormFieldAppearanceBuilder = class
    (TdxPDFTextFieldAppearanceBuilder<TdxPDFTextFormField>) // for internal use
  strict private
    procedure DrawTextField(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle; const AText: string);
  protected
    function GetMultiline: Boolean; override;
    procedure DrawBeveledBorder(AConstructor
      : TdxPDFFormCommandConstructor); override;
    procedure DrawContent(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle); override;
    procedure DrawInsetBorder(AConstructor
      : TdxPDFFormCommandConstructor); override;
    procedure DrawSolidBorder(AConstructor: TdxPDFFormCommandConstructor;
      const AContentRect: TdxPDFRectangle); override;
    procedure DrawUnderlineBorder(AConstructor
      : TdxPDFFormCommandConstructor); override;
  end;

const
  dxPDFFieldTextAlignmentMap: array [TdxPDFTextJustification]
    of TdxPDFStringAlignment = (saNear, saCenter, saFar); // for internal use

implementation

uses
  Math, dxTypeHelpers, dxStringHelper, dxCore, dxPDFCommand, dxPDFUtils,
  dxFontFile, dxPDFText, dxPDFCharacterMapping;
...
type
  ...
  strict private
    ...
    FEndPoint: TdxPointF;
  public
    class function Create(const ABeginPoint, AEndPoint: TdxPointF)
      : TdxPDFStringLine; static;
  ...
  public
    constructor Create(const AFontInfo: TdxPDFFontInfo;
      const AFormat: TdxPDFStringFormat); overload;
    constructor Create(const AFontInfo: TdxPDFFontInfo;
      const AFormat: TdxPDFStringFormat;
  ...
  public
    constructor Create(const AFontInfo: TdxPDFFontInfo;
      ...
      ASpaceGlyphRun: TdxPDFGlyphRun; ALineSpacing: TdxPDFCustomSpacing);
  ...
  public
    ...
    function GetFirstLineVerticalPosition(ALineCount: Integer): Double;
      virtual; abstract;
    function GetHorizontalPosition(ALine: TdxPDFGlyphRun): Double;
      virtual; abstract;
  ...
  strict private
    FConstructor: TdxPDFCommandConstructor;
  protected
    function CreateMeasurer(const AFontInfo: TdxPDFFontInfo;
      const AFormat: TdxPDFStringFormat): TdxPDFStringMeasurer; virtual;
    function GetLineSpacing(const AMetrics: TdxFontFileFontMetrics)
      : TdxPDFCustomSpacing; virtual;
    procedure BeginTextDrawing; virtual;
    procedure EndTextDrawing; virtual;
  public
    constructor Create(AConstructor: TdxPDFCommandConstructor);

    procedure DrawLines(ALines: TdxPDFGlyphRunList;
      const AFontInfo: TdxPDFFontInfo; const ALayoutRect: TdxPDFRectangle;
      const AFormat: TdxPDFStringFormat;
      AShowWithGlyphOffsets: Boolean); overload;
    procedure DrawLines(ALines: TdxPDFGlyphRunList;
      const AFontInfo: TdxPDFFontInfo; const ALocation: TdxPointF;
      const AFormat: TdxPDFStringFormat;
      AShowWithGlyphOffsets: Boolean); overload;
    procedure DrawLines(ALines: TdxPDFGlyphRunList;
      const AFontInfo: TdxPDFFontInfo; AStrategy: TdxPDFStringPaintingStrategy;
      AShowWithGlyphOffsets: Boolean); overload;
  end;
  ...
  protected
    function CreateMeasurer(const AFontInfo: TdxPDFFontInfo;
      const AFormat: TdxPDFStringFormat): TdxPDFStringMeasurer; override;
    function GetLineSpacing(const AMetrics: TdxFontFileFontMetrics)
      : TdxPDFCustomSpacing; override;
  ...
  public
    constructor Create(AFormatter: TdxPDFLineFormatter;
      AEllipsis: TdxPDFGlyphRun);

    class function CreateAlgorithm(ATrimming: TdxPDFStringTrimming;
      AFormatter: TdxPDFLineFormatter; AEllipsis: TdxPDFGlyphRun)
      : TdxPDFLineTrimmingAlgorithm; static;
  ...
  TdxPDFLineTrimmingEllipsisCharAlgorithm = class
    (TdxPDFLineTrimmingCharAlgorithm)
  protected
  ...
  TdxPDFLineTrimmingEllipsisWordAlgorithm = class
    (TdxPDFLineTrimmingWordAlgorithm)
  protected
    function GetUseEllipsis: Boolean; override;
  end;
  ...
  public
    class function Create(const AText: string; AGlyphs: TdxPDFGlyphRun;
      AEndsWithSoftHyphen: Boolean): TdxPDFFormatterWord; static;
  ...
  strict private
    ...
    class function CanBreak(APrevious: Char; ANext: Char): Boolean;
      overload; static;
  ...
  protected
    function GetWordGlyphs(AActualWord: TdxPDFGlyphRun)
      : TdxPDFGlyphList; virtual;
  ...
  public
    constructor Create(ALayoutWidth: Double; ALayoutLineCount: Integer;
      const AFormat: TdxPDFStringFormat; AEllipsis: TdxPDFGlyphRun;
      AFontData: TdxPDFEditableFontData; AEmTabStopInterval: Double);
    destructor Destroy; override;
  ...
  protected
    function GetWordGlyphs(AActualWord: TdxPDFGlyphRun)
      : TdxPDFGlyphList; override;
  public
    constructor Create(ALayoutWidth: Double;
      ASpaceGlyphIndex, ALayoutLineCount: Integer;
      const AFormat: TdxPDFStringFormat; AEllipsis: TdxPDFGlyphRun;
      AFontData: TdxPDFEditableFontData; AEmTabStopInterval: Double);
  ...
  strict private
    ...
    FSpaceGlyphIndex: Integer;
    class function ProcessHotkeyPrefixes(const AText: string;
      APrefix: TdxPDFHotkeyPrefix): string; static;
  protected
  ...
  public
    ...
    function FormatString(const AStr: string; const APoint: TdxPointF;
      const AFormat: TdxPDFStringFormat; AUseKerning: Boolean;
      AEmTabStopInterval: Double): TdxPDFGlyphRunList; overload;
    function FormatString(const AStr: string;
      const ALayoutRect: TdxPDFRectangle; const AFormat: TdxPDFStringFormat;
      AUseKerning: Boolean; AEmTabStopInterval: Double)
      : TdxPDFGlyphRunList; overload;
    function FormatString(const AText: TStringDynArray;
      AFormatter: TdxPDFLineFormatter; ALimitedLines: Boolean;
      ALineCount: Integer; const AFormat: TdxPDFStringFormat;
      AUseKerning: Boolean): TdxPDFGlyphRunList; overload;
  ...
  public
    constructor Create(AFontData: TdxPDFEditableFontData;
      AEmTabStopInterval: Double; ARightToLeft: Boolean;
      AShouldUseKerning: Boolean);
    function FormatString(const ALine: string): TdxPDFGlyphRun;
  end;

function HasFlag(ASourceFlags, AFlags: TdxPDFStringFormatFlags)
  : Boolean; overload;
begin
...
class function TdxPDFStringLine.Create(const ABeginPoint, AEndPoint: TdxPointF)
  : TdxPDFStringLine;
begin
...
begin
  inherited Create;
  FActualSpacing := AFont.Metrics.Height / 1000;
end;
...
class function TdxPDFStringFormat.Create(AFormat: TdxPDFStringFormat)
  : TdxPDFStringFormat;
begin
...
begin
  Result := TdxPDFStringFormat.Create
    (TdxPDFStringFormatFlags(Integer(sffLineLimit) or Integer(sffNoClip)));
  Result.FTrimming := stNone;
  Result.FLeadingMarginFactor := 0;
  Result.FTrailingMarginFactor := 0;
end;

class function TdxPDFStringFormat.Create(AFormatFlags: TdxPDFStringFormatFlags)
  : TdxPDFStringFormat;
begin
  Result.FLeadingMarginFactor := 1.0 / 6.0;
  Result.FTrailingMarginFactor := 1.0 / 6.0;
...
constructor TdxPDFStringMeasurer.Create(const AFontInfo: TdxPDFFontInfo;
  const AFormat: TdxPDFStringFormat);
begin
  Create(AFontInfo, AFormat,
    TdxPDFLineSpacing.Create((AFontInfo.FontData as TdxPDFEditableFontData)
    .Metrics));
end;

constructor TdxPDFStringMeasurer.Create(const AFontInfo: TdxPDFFontInfo;
  const AFormat: TdxPDFStringFormat; ALineSpacing: TdxPDFCustomSpacing);
begin
  inherited Create;
...
constructor TdxPDFWidgetStringMeasurer.Create(const AFontInfo: TdxPDFFontInfo;
  const AFormat: TdxPDFStringFormat;
  ATextState: TdxPDFInteractiveFormFieldTextState;
  ASpaceGlyphRun: TdxPDFGlyphRun; ALineSpacing: TdxPDFCustomSpacing);
begin
...
constructor TdxPDFStringPaintingStrategy.Create(AConstructor
  : TdxPDFCommandConstructor; AMeasurer: TdxPDFStringMeasurer);
begin
  inherited Create;
...
procedure TdxPDFStringPaintingStrategy.ShowText(ALine: TdxPDFGlyphRun;
  AUseKerning: Boolean);
var
...
constructor TdxPDFStringPaintingInsideRectStrategy.Create
  (AConstructor: TdxPDFCommandConstructor; AMeasurer: TdxPDFStringMeasurer;
  const ARect: TdxPDFRectangle);
begin
  inherited Create(AConstructor, AMeasurer);
  FRect := ARect;
  FClip := not(Integer(AMeasurer.Format.FormatFlags) and
    Integer(sffNoClip) <> 0)
end;

function TdxPDFStringPaintingInsideRectStrategy.GetFirstLineVerticalPosition
  (ALineCount: Integer): Double;
begin
  case Measurer.Format.LineAlignment of
    saCenter:
      Result := FRect.Top -
        (FRect.Height - Measurer.MeasureStringHeight(ALineCount)) / 2;
    saFar:
      Result := FRect.Top -
        (FRect.Height - Measurer.MeasureStringHeight(ALineCount));
  else
    Result := FRect.Top;
  end;
end;

function TdxPDFStringPaintingInsideRectStrategy.GetHorizontalPosition
  (ALine: TdxPDFGlyphRun): Double;
begin
  case Measurer.Format.Alignment of
    saCenter:
      Result := FRect.Left + (FRect.Width - Measurer.MeasureWidth(ALine)) / 2;
    saFar:
      Result := FRect.Right - Measurer.MeasureWidth(ALine) -
        Measurer.TrailingOffset;
  else
...
constructor TdxPDFStringPaintingAtPointStrategy.Create
  (AConstructor: TdxPDFCommandConstructor; AMeasurer: TdxPDFStringMeasurer;
  const ALocation: TdxPointF);
begin
  inherited Create(AConstructor, AMeasurer);
  FLocation := ALocation;
end;

function TdxPDFStringPaintingAtPointStrategy.GetFirstLineVerticalPosition
  (ALineCount: Integer): Double;
begin
  case Measurer.Format.LineAlignment of
    saCenter:
...
function TdxPDFStringPaintingAtPointStrategy.GetHorizontalPosition
  (ALine: TdxPDFGlyphRun): Double;
begin
  case Measurer.Format.Alignment of
    saCenter:
    ...
    saFar:
      Result := FLocation.X - Measurer.MeasureWidth(ALine) -
        Measurer.TrailingOffset;
  else
...
procedure TdxPDFStringPainter.DrawLines(ALines: TdxPDFGlyphRunList;
  const AFontInfo: TdxPDFFontInfo; const ALayoutRect: TdxPDFRectangle;
  const AFormat: TdxPDFStringFormat; AShowWithGlyphOffsets: Boolean);
var
  AMeasurer: TdxPDFStringMeasurer;
...
begin
  ...
  try
    AStrategy := TdxPDFStringPaintingInsideRectStrategy.Create(FConstructor,
      AMeasurer, ALayoutRect);
    try
...
begin
  AMeasurer := CreateMeasurer(AFontInfo, AFormat);
  try
    AStrategy := TdxPDFStringPaintingAtPointStrategy.Create(FConstructor,
      AMeasurer, ALocation);
    try
      DrawLines(ALines, AFontInfo, AStrategy, AShowWithGlyphOffsets);
    finally
...
procedure TdxPDFStringPainter.DrawLines(ALines: TdxPDFGlyphRunList;
  const AFontInfo: TdxPDFFontInfo; AStrategy: TdxPDFStringPaintingStrategy;
  AShowWithGlyphOffsets: Boolean);
var
  ABaselineY, AUnderlineY, AStrikeoutY: Double;
  ACount, I: Integer;
  AFontData: TdxPDFEditableFontData;
  AFontSize, ALineSpacing, AAscent, AFirstLineY, APreviousXOffset, X, Y,
    ADescent, ARight: Double;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if AOblique then
          FConstructor.SetObliqueTextMatrix(X, Y)
        else
          FConstructor.StartTextLineWithOffsets(X - APreviousXOffset, Y);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              AStringLines.Add(TdxPDFStringLine.Create(TdxPointF.Create(X,
                AUnderlineY), TdxPointF.Create(ARight, AUnderlineY)));
            end;
            if AFontData.Strikeout then
            begin
              AStrikeoutY := ABaselineY + AAscent / 2 - ADescent;
              AStringLines.Add(TdxPDFStringLine.Create(TdxPointF.Create(X,
                AStrikeoutY), TdxPointF.Create(ARight, AStrikeoutY)));
            end;
          end;
        end;
...
function TdxPDFStringPainter.GetLineSpacing(const AMetrics
  : TdxFontFileFontMetrics): TdxPDFCustomSpacing;
begin
...
constructor TdxPDFWidgetStringPainter.Create(AConstructor
  : TdxPDFCommandConstructor; ASpaceGlyphRun: TdxPDFGlyphRun;
...
procedure TdxPDFWidgetStringPainter.UpdateSpacing
  (AState: TdxPDFInteractiveFormFieldTextState);
begin
  FState := AState;
end;

function TdxPDFWidgetStringPainter.CreateMeasurer(const AFontInfo
  : TdxPDFFontInfo; const AFormat: TdxPDFStringFormat): TdxPDFStringMeasurer;
begin
  Result := TdxPDFWidgetStringMeasurer.Create(AFontInfo, AFormat, FState,
    FSpaceGlyphRun, FLineSpacing);
end;

function TdxPDFWidgetStringPainter.GetLineSpacing(const AMetrics
  : TdxFontFileFontMetrics): TdxPDFCustomSpacing;
begin
  Result := FLineSpacing;
end;
...
begin
  Create(AFontInfo, TdxPDFLineSpacing.Create
    ((AFontInfo.FontData as TdxPDFEditableFontData).Metrics));
end;
...
class function TdxPDFStringFormatter.ProcessHotkeyPrefixes(const AText: string;
  APrefix: TdxPDFHotkeyPrefix): string;
var
...
function TdxPDFStringFormatter.FormatString(const AStr: string;
  const APoint: TdxPointF; const AFormat: TdxPDFStringFormat;
  AUseKerning: Boolean; AEmTabStopInterval: Double): TdxPDFGlyphRunList;
var
  ALine: string;
...
begin
  ...
  AFormatter := TdxPDFTabbedStringFormatter.Create(FFontData,
    AEmTabStopInterval, AFormat.DirectionRightToLeft, AUseKerning);
  try
    for ALine in TdxPDFUtils.Split(ProcessHotkeyPrefixes(AStr,
      AFormat.HotkeyPrefix), #10#13) do
...
function TdxPDFStringFormatter.FormatString(const AStr: string;
  ...
  procedure CalculateLineCount(out ALayoutWidth: Double;
    out ALineCount: Integer; out ALimitedLines: Boolean);
  var
  ...
  begin
    ...
    ALineSpacing := FLineSpacing.GetValue(FFontSize);
    AFirstLineHeight := FFontData.Metrics.GetAscent(FFontSize) +
      FFontData.Metrics.GetDescent(FFontSize);
    ALayoutHeight := ALayoutRect.Height;
    if ALayoutHeight < AFirstLineHeight then
      ALineCount := IfThen(not ALimitedLines and
        (ALayoutHeight >= ALineSpacing * FLineAppearanceFactor), 1, 0)
    else
    begin
      APartialLineCount := (ALayoutHeight - AFirstLineHeight) / ALineSpacing;
      ALineCount := Floor(APartialLineCount);
      if not ALimitedLines and
        ((APartialLineCount - ALineCount) >= FLineAppearanceFactor) then
        Inc(ALineCount);
      Inc(ALineCount);
    end;
    ALimitedLines := ALimitedLines or (AFormat.Trimming <> stNone);
    ALayoutWidth :=
      Max(0, (ALayoutRect.Width - (AFormat.TrailingMarginFactor +
      AFormat.LeadingMarginFactor) * FFontSize) * 1000 / FFontSize);
  end;
...
begin
  ...
  if TdxPDFTextUtils.HasRTLMarker(AStr) then
    AFormatter := TdxPDFRTLLineFormatter.Create(ALayoutWidth, FSpaceGlyphIndex,
      ALineCount, AFormat, FEllipsis, FFontData, AEmTabStopInterval)
  else
    AFormatter := TdxPDFLineFormatter.Create(ALayoutWidth, ALineCount, AFormat,
      FEllipsis, FFontData, AEmTabStopInterval);
  try
    AText := TdxPDFUtils.Split(ProcessHotkeyPrefixes(AStr,
      AFormat.HotkeyPrefix), #10#13);
    ALines := FormatString(AText, AFormatter, ALimitedLines, ALineCount,
      AFormat, AUseKerning or ShouldApplyKerning(Length(AText)));
    Result := TdxPDFGlyphRunList.Create;
...
function TdxPDFStringFormatter.FormatString(const AText: TStringDynArray;
  AFormatter: TdxPDFLineFormatter; ALimitedLines: Boolean; ALineCount: Integer;
  const AFormat: TdxPDFStringFormat; AUseKerning: Boolean): TdxPDFGlyphRunList;
var
...
begin
  ...
  if AFormat.DirectionRightToLeft then
    AFlags := TdxPDFGlyphMappingFlags(Integer(AFlags) or
      Integer(mfDirectionRightToLeft));
  ...
  begin
    ...
    begin
      AFormatter.Clear;
      Exit(FormatString(AText, AFormatter, ALimitedLines, ALineCount,
        AFormat, True));
    end;
...
function TdxPDFStringFormatter.ShouldApplyKerning(ALinesCount: Integer)
  : Boolean;
begin
  Result := False;
end;
...
constructor TdxPDFTabbedStringFormatter.Create
  (AFontData: TdxPDFEditableFontData; AEmTabStopInterval: Double;
...
procedure TdxPDFTabbedStringFormatter.AppendGlyphRun(const ATabPiece: string;
  ATabCount: Integer);
var
  AGlyph: TdxPDFGlyph;
...
begin
  ...
  begin
    ...
    ATabOffset := (Round(AGlyphRunWidth / FEmTabStop) + ATabCount) * FEmTabStop
      - AGlyphRunWidth;
    APieceGlyphRun.Glyphs[0] := TdxPDFGlyph.Create(AGlyph.Index, AGlyph.Width,
      -ATabOffset);
  end;
  if FGlyphRun = nil then
...
function TdxPDFTabbedStringFormatter.MapString(const ALine: string)
  : TdxPDFGlyphRun;
var
  AFlags: TdxPDFGlyphMappingFlags;
begin
...
function TdxPDFTabbedStringFormatter.FormatString(const ALine: string)
  : TdxPDFGlyphRun;
var
  ATabbedPiece: string;
...
procedure TdxPDFCommandConstructor.AppendRectangle
  (const ARect: TdxPDFRectangle);
begin
...
procedure TdxPDFCommandConstructor.DrawImage(AXObjectNumber: Integer;
  const ARect: TdxRectF);
begin
  DrawXObject(AXObjectNumber, TdxPDFTransformationMatrix.Create(ARect.Width, 0,
    0, Abs(ARect.Height), ARect.Left, ARect.Bottom));
end;
...
procedure TdxPDFCommandConstructor.IntersectClip(APath: TdxPDFGraphicsPath;
  ANonZero: Boolean = True);
begin
  GeneratePathCommands(APath);
  WriteIntersectClipCommand(ANonZero);
end;

procedure TdxPDFCommandConstructor.IntersectClip
  (APaths: TList<TdxPDFGraphicsPath>; ANonZero: Boolean);
var
...
procedure TdxPDFCommandConstructor.FillPath(APaths: TList<TdxPDFGraphicsPath>;
  ANonZero: Boolean);
var
  APath: TdxPDFGraphicsPath;
begin
...
procedure TdxPDFCommandConstructor.FillPolygon(const APoints: TdxPDFPoints;
  ANonZero: Boolean);
begin
...
procedure TdxPDFCommandConstructor.ModifyTransformationMatrix
  (const AMatrix: TdxPDFTransformationMatrix);
begin
  CurrentTransformationMatrix := TdxPDFTransformationMatrix.Multiply(AMatrix,
...
procedure TdxPDFCommandConstructor.SetColorForNonStrokingOperations
  (const AColor: TdxPDFColor);
var
  AComponent: Double;
...
begin
  if not AColor.IsNull then
    ...
    begin
      WriteOperationData
        (Pattern + TdxPDFSetColorSpaceForNonStrokingOperationsCommand.GetName);
      ...
      begin
        WriteNameDelimiter;
        WriteString(FResources.AddPattern
          (AColor.Pattern as TdxPDFCustomPattern));
      end;
      WriteCommand(TdxPDFSetColorAdvancedForNonStrokingOperationsCommand);
    end;
end;

procedure TdxPDFCommandConstructor.SetColorForStrokingOperations
  (const AColor: TdxPDFColor);
var
  AComponent: Double;
  AComponents: TDoubleDynArray;
begin
  if not AColor.IsNull then
    ...
    begin
      WriteOperationData
        (Pattern + TdxPDFSetColorSpaceForStrokingOperationsCommand.GetName);
      for AComponent in AColor.Components do
        WriteDouble(AComponent);
      WriteSpace;
      if AColor.Pattern <> nil then
...
procedure TdxPDFCommandConstructor.SetGraphicsStateParameters
...
procedure TdxPDFCommandConstructor.SetLineJoinStyle
  (AValue: TdxPDFLineJoinStyle);
begin
  WriteSpace;
...
procedure TdxPDFCommandConstructor.SetTextFont(AFont: TObject;
  AFontSize: Single);
begin
  SetTextFont(FResources.AddFont(AFont as TdxPDFCustomFont), AFontSize);
end;

procedure TdxPDFCommandConstructor.SetTextFont(const AFontName: string;
  AFontSize: Single);
begin
  WriteNameDelimiter;
...
procedure TdxPDFCommandConstructor.SetTextRenderingMode
  (AValue: TdxPDFTextRenderingMode);
begin
  WriteSpace;
  WriteString(IntToStr(Integer(AValue)));
...
procedure TdxPDFCommandConstructor.ShowText(const AText: TBytes;
  AGlyphOffsets: TDoubleDynArray);
...
procedure TdxPDFCommandConstructor.AppendBezierCurve(const P1, P2,
  P3: TdxPointF);
begin
  WritePoint(P1);
  WritePoint(P2);
...
procedure TdxPDFCommandConstructor.AppendEllipse(const ARect: TdxRectF);
  ...
  begin
    AppendBezierCurve(dxPointF(P11, P12), dxPointF(P21, P22),
      dxPointF(p31, P32));
  end;

var
  ABottom, ATop, ACenterY, AVerticalOffset, ABottomControlPoint,
    ATopControlPoint: Double;
  ALeft, ARight, ACenterX, AHorizontalOffset, ALeftControlPoint,
    ARightControlPoint: Double;
begin
  ALeft := ARect.Left;
  ARight := ARect.Right;
  ...
  AppendCurveCommand(ARight, ATopControlPoint, ARightControlPoint, ATop,
    ACenterX, ATop);
  AppendCurveCommand(ALeftControlPoint, ATop, ALeft, ATopControlPoint, ALeft,
    ACenterY);
  AppendCurveCommand(ALeft, ABottomControlPoint, ALeftControlPoint, ABottom,
    ACenterX, ABottom);
  AppendCurveCommand(ARightControlPoint, ABottom, ARight, ABottomControlPoint,
    ARight, ACenterY);
end;
...
procedure TdxPDFCommandConstructor.DrawXObject(AXObjectNumber: Integer;
  const AMatrix: TdxPDFTransformationMatrix);
begin
  SaveGraphicsState;
...
procedure TdxPDFCommandConstructor.GeneratePathCommands
  (APath: TdxPDFGraphicsPath);
var
...
procedure TdxPDFCommandConstructor.SetCurrentTransformationMatrix
  (const AValue: TdxPDFTransformationMatrix);
begin
...
procedure TdxPDFCommandConstructor.WriteCommand(const ACommandClass
...
begin
  ...
  AHalfDimension := TdxPDFUtils.Min(ABoundingBox.Width,
    Abs(ABoundingBox.Height)) / 2;
  ACenter.X := (ABoundingBox.Left + ABoundingBox.Right) / 2;
  ACenter.Y := (ABoundingBox.Bottom + ABoundingBox.Top) / 2;
  FContentSquare := TdxRectF.CreateSize(ACenter.X - AHalfDimension,
    ACenter.Y - AHalfDimension, ACenter.X + AHalfDimension,
    ACenter.Y + AHalfDimension);
end;

function TdxPDFFormCommandConstructor.GetBoundingBox: TdxPDFRectangle;
begin
  Result := FForm.BBox;
end;
...
procedure TdxPDFAnnotationAppearanceBuilder<T>.RebuildAppearance
  (AForm: TdxPDFForm);
var
...
procedure TdxPDFMarkupAnnotationAppearanceBuilder<T>.Rebuild
  (AConstructor: TdxPDFFormCommandConstructor);
var
  AParameters: TdxPDFGraphicsStateParameters;
begin
...
constructor TdxPDFWidgetAnnotationAppearanceBuilder<T>.Create
  (AWidget: TdxPDFWidgetAnnotation; AFormField: T);
begin
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.Rebuild
  (AConstructor: TdxPDFFormCommandConstructor);
var
...
function TdxPDFWidgetAnnotationAppearanceBuilder<T>.GetBackgroundColor
  : TdxPDFColor;
begin
  Result := Annotation.BackgroundColor;
end;

procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.DrawBorder
  (AConstructor: TdxPDFFormCommandConstructor;
  const AContentRect: TdxPDFRectangle);
var
...
begin
  ...
  if (AAppearanceCharacteristics <> nil) and
    not AAppearanceCharacteristics.BorderColor.IsNull then
  begin
    AConstructor.SetColorForStrokingOperations
      (AAppearanceCharacteristics.BorderColor);
    if AWidget.BorderStyle = nil then
    begin
      if AWidget.Border <> nil then
      begin
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.
  DrawRectangularBeveledBorder(AConstructor: TdxPDFFormCommandConstructor);
var
  AComponents: TDoubleDynArray;
begin
  SetLength(AComponents, 1);
  AComponents[0] := 1.0;
  AConstructor.SetColorForNonStrokingOperations
    (TdxPDFColor.Create(AComponents));
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.DrawRectangularBorderStroke
  (AConstructor: TdxPDFFormCommandConstructor);
var
...
begin
  ...
  R := TdxPDFRectangle.Create(TdxPDFUtils.Min(AHalfBorderWidth, W),
    TdxPDFUtils.Min(AHalfBorderWidth, H), TdxPDFUtils.Max(AHalfBorderWidth, W),
    TdxPDFUtils.Max(AHalfBorderWidth, H));
...
begin
  ...
  APoints[3] := TdxPointF.Create(AAnnotationWidth - ADoubleBorderWidth,
    ABottom);
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.DrawRectangularInsetBorder
  (AConstructor: TdxPDFFormCommandConstructor);
var
  AComponents: TDoubleDynArray;
begin
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.
  DrawRectangularUnderlineBorder(AConstructor: TdxPDFFormCommandConstructor);
var
...
begin
  ...
  APoints[1] := TdxPointF.Create(AConstructor.BoundingBox.Width,
    AHalfBorderWidth);
  AConstructor.DrawLines(APoints);
end;

procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.DrawTextCombs
  (AConstructor: TdxPDFFormCommandConstructor; const AContentRect: TdxRectF;
  AMaxLen: Integer);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      AConstructor.DrawLine(TdxPointF.Create(X, AContentRect.Top),
        TdxPointF.Create(X, AContentRect.Bottom));
    end;
  end;
end;

procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.FillBackgroundRectangle
  (AConstructor: TdxPDFFormCommandConstructor; const ARect: TdxPDFRectangle;
  const AColor: TdxPDFColor);
begin
  if not AColor.IsNull then
  begin
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.FillBackgroundEllipse
  (AConstructor: TdxPDFFormCommandConstructor; const ARect: TdxRectF;
  const AColor: TdxPDFColor);
begin
  if not AColor.IsNull then
  begin
...
procedure TdxPDFWidgetAnnotationAppearanceBuilder<T>.DrawStyledBorder
  (AConstructor: TdxPDFFormCommandConstructor;
  const AContentRect: TdxPDFRectangle;
  ABorderStyle: TdxPDFAnnotationBorderStyle);
begin
  ...
  else if ABorderStyle.StyleName = 'B' then
  begin
    DrawSolidBorder(AConstructor, AContentRect);
    DrawBeveledBorder(AConstructor);
  end
  else if ABorderStyle.StyleName = 'I' then
  begin
    DrawSolidBorder(AConstructor, AContentRect);
    DrawInsetBorder(AConstructor);
  end
  else if ABorderStyle.StyleName = 'U' then
...
constructor TdxPDFTextBasedFormFieldAppearanceBuilder<T>.Create
  (AWidget: TdxPDFWidgetAnnotation; AFormField: T; AState: TdxPDFDocumentState);
var
...
procedure TdxPDFTextBasedFormFieldAppearanceBuilder<T>.FillBackground
  (AConstructor: TdxPDFFormCommandConstructor);
begin
...
procedure TdxPDFTextBasedFormFieldAppearanceBuilder<T>.Rebuild
  (AConstructor: TdxPDFFormCommandConstructor);
begin
...
function TdxPDFTextBasedFormFieldAppearanceBuilder<T>.
  CalculateCenteredLineYOffset(const AClipRect: TdxPDFRectangle): Double;
begin
  Result := AClipRect.Top + (AClipRect.Height - FFontData.Metrics.GetLineSpacing
    (FFontSize)) / 2 + FFontData.Metrics.GetDescent(FFontSize);
end;

function TdxPDFTextBasedFormFieldAppearanceBuilder<T>.GetTextWidth
  (const AText: string; AFontSize: Double): Double;
begin
  Result := FFontData.GetTextWidth(AText, AFontSize, FormField.TextState);
end;

procedure TdxPDFTextBasedFormFieldAppearanceBuilder<T>.DrawTextBoxText
  (AConstructor: TdxPDFFormCommandConstructor; const AOffset: TdxPointF;
  const AText: string);
var
  ARun: TdxPDFGlyphRun;
begin
...
procedure TdxPDFTextBasedFormFieldAppearanceBuilder<T>.EndDrawTextBox
  (AConstructor: TdxPDFFormCommandConstructor);
begin
  AConstructor.EndText;
end;

procedure TdxPDFTextBasedFormFieldAppearanceBuilder<T>.RemoveFontFromStorage
  (AFont: TdxPDFCustomFont);
begin
  FFontDataStorage.Delete(AFont);
end;

procedure TdxPDFTextBasedFormFieldAppearanceBuilder<T>.StartDrawTextBox
  (AConstructor: TdxPDFFormCommandConstructor; const AForeColor: TdxPDFColor);
begin
...
function TdxPDFTextFieldAppearanceBuilder<T>.CreateStringFormat
  : TdxPDFStringFormat;
begin
  ...
  begin
    Result.FormatFlags := TdxPDFStringFormatFlags(Integer(Result.FormatFlags) or
      Integer(sffNoWrap));
...
begin
  Result := (Integer(FormField.Flags) and
    Integer(TdxPDFInteractiveFormFieldFlags.ffMultiline)) <> 0;
end;

procedure TdxPDFTextFormFieldAppearanceBuilder.DrawBeveledBorder
  (AConstructor: TdxPDFFormCommandConstructor);
begin
  // do nothing
end;

procedure TdxPDFTextFormFieldAppearanceBuilder.DrawContent
  (AConstructor: TdxPDFFormCommandConstructor;
  const AContentRect: TdxPDFRectangle);
var
  ACh: string;
...
begin
  ...
  begin
    AMaxLen := IfThen(TdxPDFUtils.IsIntegerValid(FormField.MaxLen),
      FormField.MaxLen, 0);
    if FormField.HasFlag(ffCombo) and (AMaxLen <> 0) then
    begin
...
{$ENDIF}
      ...
      case FormField.TextJustification of
        TdxPDFTextJustification.tjCentered:
          AInitialPosition := AInitialPosition + AStep *
            (AMaxLen / 2 - ALength / 2);
        TdxPDFTextJustification.tjRightJustified:
          AInitialPosition := AInitialPosition + AStep * (AMaxLen - ALength);
      end;
      DrawTextBoxText(AConstructor, dxPointF(AInitialPosition,
        CalculateCenteredLineYOffset(AContentRect)), ACh);
      for I := 1 to ALength - 1 do
      begin
        ACh := AChars[I];
        ACharWidth := GetTextWidth(ACh, FontSize);
        DrawTextBoxText(AConstructor,
          dxPointF(AStep + (APreviousCharWidth - ACharWidth) / 2, 0), ACh);
...
procedure TdxPDFTextFormFieldAppearanceBuilder.DrawInsetBorder
  (AConstructor: TdxPDFFormCommandConstructor);
begin
  // do nothing
end;

procedure TdxPDFTextFormFieldAppearanceBuilder.DrawSolidBorder
  (AConstructor: TdxPDFFormCommandConstructor;
  const AContentRect: TdxPDFRectangle);
begin
  // do nothing
...
procedure TdxPDFTextFormFieldAppearanceBuilder.DrawUnderlineBorder
  (AConstructor: TdxPDFFormCommandConstructor);
begin
  // do nothing
end;

procedure TdxPDFTextFormFieldAppearanceBuilder.DrawTextField
  (AConstructor: TdxPDFFormCommandConstructor;
  const AContentRect: TdxPDFRectangle; const AText: string);

  procedure FormatAndDrawString(APainter: TdxPDFWidgetStringPainter;
    ALineSpacing: TdxPDFCustomSpacing; const ARect: TdxPDFRectangle;
    const AText: string);
  var
    AFontInfo: TdxPDFFontInfo;
  ...
  begin
    Result := Integer(Annotation.InteractiveFormField.Flags) and
      Integer(ffMultiline) <> 0;
  end;

  function GetActualFont(AFontCommand: TdxPDFSetTextFontCommand)
    : TdxPDFCustomFont;
  begin
...
begin
  ...
  AFontCommand := nil;
  if (FormField.TextState.FontCommand <> nil) and
    (FormField.TextState.FontCommand is TdxPDFSetTextFontCommand) then
    AFontCommand := TdxPDFSetTextFontCommand(FormField.TextState.FontCommand);
  ALineSpacing := TdxPDFMultilineWidgetLineSpacing.Create
    (GetActualFont(AFontCommand));
  try
    if IsMultiline and (ALineSpacing.GetValue(FontSize) < AContentRect.Height)
    then
      ATextRect := TdxPDFRectangle.Create(ATextRect.Left,
        AContentRect.Top - ALineSpacing.GetValue(FontSize) +
        FontData.Metrics.GetAscent(FontSize), ATextRect.Right,
        AContentRect.Bottom)
    else
    begin
      ALineSpacing.Free;
    ...
    APainter := TdxPDFWidgetStringPainter.Create(AConstructor,
      FontData.ProcessString(' ', mfNone), ALineSpacing);
    try
...
constructor TdxPDFLineTrimmingAlgorithm.Create(AFormatter: TdxPDFLineFormatter;
  AEllipsis: TdxPDFGlyphRun);
begin
  inherited Create;
...
class function TdxPDFLineTrimmingAlgorithm.CreateAlgorithm
  (ATrimming: TdxPDFStringTrimming; AFormatter: TdxPDFLineFormatter;
  AEllipsis: TdxPDFGlyphRun): TdxPDFLineTrimmingAlgorithm;
begin
  case ATrimming of
    stCharacter:
    ...
    stEllipsisCharacter:
      Result := TdxPDFLineTrimmingEllipsisCharAlgorithm.Create(AFormatter,
        AEllipsis);
    stEllipsisWord:
      Result := TdxPDFLineTrimmingEllipsisWordAlgorithm.Create(AFormatter,
        AEllipsis);
  else
    Result := nil;
  end;
...
begin
  ...
  begin
    ...
    if FFormatter.CurrentLineWidth + FEllipsis.Width <= FFormatter.LayoutWidth
    then
...
function TdxPDFLineTrimmingCharAlgorithm.ProcessWord
  (AWord: TdxPDFGlyphRun): Boolean;
var
  AGlyph: TdxPDFGlyph;
...
begin
  ...
  begin
    ...
    else if Formatter.CurrentLineWidth + AGlyph.Width > Formatter.LayoutWidth
    then
      Exit(True);
    Formatter.AddGlyph(AGlyph);
  end;
...
function TdxPDFLineTrimmingWordAlgorithm.ProcessWord
  (AWord: TdxPDFGlyphRun): Boolean;
var
...
begin
  ...
  begin
    ...
    else if Formatter.CurrentLineWidth + AWord.Width > Formatter.LayoutWidth
    then
      Exit(True);
...
class function TdxPDFFormatterWord.Create(const AText: string;
  AGlyphs: TdxPDFGlyphRun; AEndsWithSoftHyphen: Boolean): TdxPDFFormatterWord;
begin
  Result.FText := AText;
...
constructor TdxPDFLineFormatter.Create(ALayoutWidth: Double;
  ALayoutLineCount: Integer; const AFormat: TdxPDFStringFormat;
  AEllipsis: TdxPDFGlyphRun; AFontData: TdxPDFEditableFontData;
...
begin
  ...
  FTrimmingAlgorithm := TdxPDFLineTrimmingAlgorithm.CreateAlgorithm
    (AFormat.Trimming, Self, AEllipsis);
...
class function TdxPDFLineFormatter.CanBreak(APrevious: Char;
  ANext: Char): Boolean;
begin
  Result := not(IsBeginWordSymbol(APrevious) or IsEndWordSymbol(ANext));
  if Result then
    Result := CanBreak(APrevious) or CanBreak(ANext) or IsBeginWordSymbol(ANext)
      or IsEndWordSymbol(APrevious);
end;
...
procedure TdxPDFLineFormatter.FormatLine(const ALine: string;
  AFlags: TdxPDFGlyphMappingFlags);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      AWord := MapString(TdxStringHelper.Substring(ALine,
        ALastSubstringEndIndex, I - ALastSubstringEndIndex + 1));
      try
  ...
  ALastWord := TdxStringHelper.TrimEnd(TdxStringHelper.Substring(ALine,
    ALastSubstringEndIndex, ALength - ALastSubstringEndIndex), [' ']);
...
function TdxPDFLineFormatter.GetWordGlyphs(AActualWord: TdxPDFGlyphRun)
  : TdxPDFGlyphList;
begin
...
begin
  ...
  Result := TdxPDFFormatterWord.Create(S, FFontData.ProcessString(AStr, FFlags),
    AEndsWithSoftHyphen);
end;

function TdxPDFLineFormatter.AppendWord(AActualWord
  : TdxPDFFormatterWord): Boolean;
var
  AGlyph: TdxPDFGlyph;
begin
  Result := False;
  ...
  if (FTrimmingAlgorithm <> nil) and
    (FNoWrap or (FLines.Count = FLayoutLineCount - 1)) then
    Result := FTrimmingAlgorithm.ProcessWord(AActualWord.Glyphs)
  else if FNoWrap then
    AddWord(AActualWord.Glyphs)
  else if IfThen(AActualWord.EndsWithSoftHyphen,
    AActualWord.Width + HyphenRun.Width, AActualWord.Width) + CurrentLineWidth
    <= FLayoutWidth then
    AddWord(AActualWord.Glyphs)
  else if not IsCurrentLineEmpty then
  begin
    FinishLine(False);
    ...
    for AGlyph in GetWordGlyphs(AActualWord.Glyphs) do
      if CurrentLineWidth + AGlyph.Width <= FLayoutWidth then
        AddGlyph(AGlyph)
      else if (AGlyph.Width > FLayoutWidth) and IsCurrentLineEmpty then
      begin
        AddGlyph(AGlyph);
        FinishLine(False);
      end
...
procedure TdxPDFLineFormatter.ApplyTabStops(AWord: TdxPDFFormatterWord;
  ATabCount: Integer);
var
  AGlyph: TdxPDFGlyph;
begin
  if (FEmTabStopInterval <> 0) and (ATabCount <> 0) and
    (AWord.Glyphs.Glyphs.Count <> 0) then
  begin
    AGlyph := AWord.Glyphs.Glyphs[0];
    AWord.Glyphs.Glyphs[0] := TdxPDFGlyph.Create(AGlyph.Index, AGlyph.Width,
      FCurrentLine.Width - (Ceil(FCurrentLine.Width / FEmTabStopInterval) +
      ATabCount - 1) * FEmTabStopInterval);
  end;
end;

{ TdxPDFRTLLineFormatter }

constructor TdxPDFRTLLineFormatter.Create(ALayoutWidth: Double;
  ASpaceGlyphIndex, ALayoutLineCount: Integer;
  const AFormat: TdxPDFStringFormat; AEllipsis: TdxPDFGlyphRun;
  AFontData: TdxPDFEditableFontData; AEmTabStopInterval: Double);
begin
  inherited Create(ALayoutWidth, ALayoutLineCount, AFormat, AEllipsis,
    AFontData, AEmTabStopInterval);
...
function TdxPDFRTLLineFormatter.GetWordGlyphs(AActualWord: TdxPDFGlyphRun)
  : TdxPDFGlyphList;
var
  I: Integer;
begin
