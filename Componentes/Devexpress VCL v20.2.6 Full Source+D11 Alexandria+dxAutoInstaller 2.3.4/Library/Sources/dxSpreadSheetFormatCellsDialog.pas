...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Classes, Controls, Graphics, dxCoreClasses,
...
type
  ...
  TdxSpreadSheetFormatCellsDialogController = class
  ...
  public
    ...
    procedure EnumCellStyles(AProcRef: TdxSpreadSheetCellStyleEnumProcRef;
      AEnumDefaultStyles: Boolean); override;
    function FormatFocusedCellValue(const AFormatCode: string)
      : TdxSpreadSheetNumberFormatResult; override;
  ...
  strict private
    class procedure SetBorderAroundLine(ACellStyle: TdxSpreadSheetCellStyle;
      ARow, AColumn: Integer; const AArea: TRect);
    class procedure SetBorderNone(ACellStyle: TdxSpreadSheetCellStyle;
      ARow, AColumn: Integer; const AArea: TRect);
  protected
    class procedure SaveCellStyles(ASheet: TdxSpreadSheetTableView;
      AProcRef: TdxSpreadSheetCellStyleEnumProcRef);
  public
    class procedure SaveBorder(ASheet: TdxSpreadSheetTableView;
      AIsBorder: Boolean);
    class procedure SaveFontStyle(ASheet: TdxSpreadSheetTableView;
      AFontStyle: TFontStyle);
    class procedure SaveNumberFormat(ASheet: TdxSpreadSheetTableView;
      AFormat: Integer);
  end;

var
  dxSpreadSheetCellStyleEditDialogClass
    : TdxSpreadSheetCellStyleEditDialogFormClass =
    TdxSpreadSheetCellStyleEditDialogForm;

procedure ShowFormatCellsDialog(ASheet: TdxSpreadSheetTableView;
  const AActivePage: Integer = -1);

implementation

uses
  Math, Forms, Variants, SysUtils, dxTypeHelpers, cxGeometry, cxGraphics,
  dxHashUtils, dxSpreadSheetCoreStrs;
...
begin
  ADialog := dxSpreadSheetCellStyleEditDialogClass.Create(ASheet.SpreadSheet,
    AActivePage);
  try
    ADialog.Initialize(TdxSpreadSheetFormatCellsDialogController.Create
      (ASheet));
    ADialog.Load;
...
constructor TdxSpreadSheetFormatCellsDialogController.Create
...
function TdxSpreadSheetFormatCellsDialogController.FormatFocusedCellValue
  (const AFormatCode: string): TdxSpreadSheetNumberFormatResult;
var
  ACell: TdxSpreadSheetCell;
  ANumberFormatter: TdxSpreadSheetNumberFormat;
begin
  ANumberFormatter := TdxSpreadSheetNumberFormat.Create(AFormatCode,
    PredefinedFormats.GetIDByFormatCode(AFormatCode));
  try
    ACell := Sheet.Cells[Sheet.Selection.FocusedRow,
      Sheet.Selection.FocusedColumn];
    if ACell <> nil then
      ANumberFormatter.Format(ACell.AsVariant, ACell.DataType,
        Sheet.SpreadSheet.FormulaController.FormatSettings, Result)
    else
      ANumberFormatter.Format(Null, cdtBlank,
        Sheet.SpreadSheet.FormulaController.FormatSettings, Result);
  finally
...
function TdxSpreadSheetFormatCellsDialogController.GetMergeAreasInfo
  : TcxCheckBoxState;
var
...
begin
  ...
  if AHasMerged and AHasUnmerged then
    Result := cbsGrayed
  else if AHasMerged then
    Result := cbsChecked
  else
    Result := cbsUnchecked;
end;

function TdxSpreadSheetFormatCellsDialogController.
  HasMultipleCellsAreaAtHorz: Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TdxSpreadSheetFormatCellsDialogController.
  HasMultipleCellsAreaAtVert: Boolean;
var
  I: Integer;
begin
  Result := False;
...
begin
  inherited;
  TdxSpreadSheetTableViewAccess(Sheet).History.BeginAction
...
function TdxSpreadSheetFormatCellsDialogController.GetCellStyles
...
begin
  ...
  try
    ...
    try
      TdxSpreadSheetHistoryAccess(TdxSpreadSheetTableViewAccess(ASheet).History)
        .BeginAction(TdxSpreadSheetHistoryFormatCellAction);
      try
...
class procedure TdxSpreadSheetCellStylesHelper.SaveBorder
  (ASheet: TdxSpreadSheetTableView; AIsBorder: Boolean);
begin
  if AIsBorder then
...
class procedure TdxSpreadSheetCellStylesHelper.SaveFontStyle
  (ASheet: TdxSpreadSheetTableView; AFontStyle: TFontStyle);
var
...
class procedure TdxSpreadSheetCellStylesHelper.SaveNumberFormat
  (ASheet: TdxSpreadSheetTableView; AFormat: Integer);
begin
  SaveCellStyles(ASheet,
    procedure(ACellStyle: TdxSpreadSheetCellStyle; ARow, AColumn: Integer;
      const AArea: TRect)
