...
type
  ...
  TdxNativeHyperlinkCollection = class(TdxIUnknownList<IdxRichEditHyperlink>,
    IdxRichEditHyperlinkCollection, IdxRichEditReadOnlyHyperlinkCollection)
  strict private
    FDocument: TdxNativeSubDocument;
  ...
  TdxNativeBookmarkCollection = class(TdxIUnknownList<IdxRichEditBookmark>,
    IdxRichEditBookmarkCollection, IdxRichEditReadOnlyBookmarkCollection)
  strict private
    FDocument: TdxNativeSubDocument;
    ...
    procedure OnBookmarkInserted(ASender: TObject;
      E: TdxDocumentIntervalEventArgs);
    procedure OnBookmarkRemoved(ASender: TObject;
      E: TdxDocumentIntervalEventArgs);
...
begin
  if not IsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedHyperlinkError));
end;
...
procedure TdxNativeHyperlink.UpdateHyperlinkInfo(const Action
  : TUpdateHyperlinkInfoProc);
var
...
function TdxNativeHyperlinkCollection.Get(const ARange
  : IdxRichEditDocumentRange): IdxRichEditReadOnlyHyperlinkCollection;
var
...
begin
  ...
  begin
    AHyperlink := TdxNativeHyperlink.Create(FDocument, FDocument.PieceTable,
      FDocument.PieceTable.Fields[I]);
...
procedure TdxNativeHyperlinkCollection.RemoveHyperlink(const AHyperlink
  : IdxRichEditHyperlink);
begin
  if FDocument <> nil then
  begin
...
begin
  if not IsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedBookmarkError));
end;
...
function TdxNativeBookmarkCollection.FindByName(const AName: string)
  : IdxRichEditBookmark;
var
...
procedure TdxNativeBookmarkCollection.RegisterBookmark(const ABookmark
  : TdxDocumentInterval);
begin
  Add(TdxNativeBookmark.Create(FDocument, TdxBookmark(ABookmark)));
end;

function TdxNativeBookmarkCollection.CreateBookmark(const ARange
  : IdxRichEditDocumentRange; const AName: string): IdxRichEditBookmark;
begin
  Result := CreateBookmark(ARange.Start, ARange.Length, AName);
end;

function TdxNativeBookmarkCollection.CreateBookmark(const AStart
  : IdxRichEditDocumentPosition; ALength: Integer; const AName: string)
  : IdxRichEditBookmark;
begin
  FDocument.CheckValid;
  FDocument.CheckDocumentPosition(AStart);
  if FDocument.DocumentModel.ActivePieceTable.Bookmarks.FindByName(AName) <> nil
  then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionDuplicateBookmark));
  PieceTable.CreateBookmark(FDocument.NormalizeLogPosition(AStart.LogPosition),
    ALength, Trim(AName));
  Result := Self[FLastInsertedBookmarkIndex];
end;

function TdxNativeBookmarkCollection.Get(const ARange: IdxRichEditDocumentRange)
  : IdxRichEditReadOnlyBookmarkCollection;
var
...
function TdxNativeBookmarkCollection.GetItem(const Name: string)
  : IdxRichEditBookmark;
begin
...
procedure TdxNativeBookmarkCollection.RemoveBookmark(const ABookmark
  : IdxRichEditBookmark);
var
  I: Integer;
begin
...
procedure TdxNativeBookmarkCollection.SelectBookmark(const ABookmark
  : IdxRichEditBookmark);
var
...
begin
  ...
  if PieceTable <> FDocument.DocumentModel.ActivePieceTable then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionSelectBookmarkError));
  ARange := ABookmark.Range;
  FDocument.CheckDocumentRange(ARange);
  FDocument.SetSelectionCore(ARange.Start.LogPosition,
    ARange.&End.LogPosition, False);
end;
