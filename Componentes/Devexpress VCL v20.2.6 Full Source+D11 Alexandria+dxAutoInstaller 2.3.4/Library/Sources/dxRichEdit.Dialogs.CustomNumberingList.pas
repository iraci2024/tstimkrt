...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Windows, Messages, SysUtils, Variants, Classes, RTLConsts, Graphics, Controls,
  Forms, Dialogs, Menus,
  Generics.Defaults, Generics.Collections, dxCore,
  dxRichEdit.Dialogs.CustomDialog, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, dxLayoutControlAdapters, dxLayoutContainer, StdCtrls,
  cxButtons, dxLayoutControl,
  dxLayoutcxEditAdapters, cxContainer, cxEdit, cxLabel,
  dxLayoutLookAndFeels, cxClasses, cxTextEdit, cxMaskEdit, cxDropDownEdit,
...
type
  TdxRichEditCustomSimpleNumberingListForm = class
  ...
  private
    ...
    procedure SetNumberingAlignment(const Value
      : TdxListNumberAlignment); inline;
  ...
  protected
    ...
    function GetOrdinalNumberConverter(AFormat: TdxNumberingFormat)
      : TdxOrdinalBasedNumberConverter; virtual;
    procedure InitializeForm; override;
    procedure NumberFormatKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    property DisplayFormat: TdxNumberingFormat read GetDisplayFormat
      write SetDisplayFormat;
    property NumberingAlignment: TdxListNumberAlignment
      read GetNumberingAlignment write SetNumberingAlignment;
  public
...
function TdxRichEditCustomSimpleNumberingListForm.GetController
  : TdxSimpleNumberingListController;
begin
  Result := TdxSimpleNumberingListController(inherited Controller);
end;

function TdxRichEditCustomSimpleNumberingListForm.GetDisplayFormat
  : TdxNumberingFormat;
var
  AValue: Integer;
begin
...
function TdxRichEditCustomSimpleNumberingListForm.GetDisplayFormatString
  (AFormat: TdxNumberingFormat): string;
var
...
begin
  ...
  try
    Result := Format('%s,%s,%s...', [AConverter.ConvertNumber(1),
      AConverter.ConvertNumber(2), AConverter.ConvertNumber(3)]);
  finally
    AConverter.Free;
  end;
end;

function TdxRichEditCustomSimpleNumberingListForm.GetNumberingAlignment
...
procedure TdxRichEditCustomSimpleNumberingListForm.SetNumberingAlignment
  (const Value: TdxListNumberAlignment);
begin
  UpdateSelectedIndex(cmbNumberingAlignment, Ord(Value));
end;

procedure TdxRichEditCustomSimpleNumberingListForm.SetDisplayFormat
...
begin
  Populate(cmbNumberingAlignment, procedure(ACombobox: TcxCustomComboBox)
    ...
    begin
      for AAlignment := Low(dxListNumberAlignmentNames)
        to High(dxListNumberAlignmentNames) do
        AddItemValue(ACombobox.Properties.Items,
          cxGetResourceString(dxListNumberAlignmentNames[AAlignment]),
          Ord(AAlignment));
    end);
end;
  ...
  Populate(cmbDisplayFormat, procedure(ACombobox: TcxCustomComboBox)
    ...
    begin
      ...
      begin
        AFormat := AValues[I];
        AddItemValue(ACombobox.Properties.Items,
          GetDisplayFormatString(AFormat), Ord(AFormat));
      end;
    end);
end;

procedure TdxRichEditCustomSimpleNumberingListForm.UpdateMinAndMaxValueStart
...
begin
  ...
  lblNumberStyle.Caption := cxGetResourceString
    (@sdxRichEditCustomNumberingListNumberStyle);
  lblNumberPosition.Caption := cxGetResourceString
    (@sdxRichEditCustomNumberingListNumberPosition);
  lblTextPosition.Caption := cxGetResourceString
    (@sdxRichEditCustomNumberingListTextPosition);
  lciStartAt.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditCustomNumberingListStartAt);
  lciAlignedAt.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditCustomNumberingListAlignedAt);
  lciIndentAt.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditCustomNumberingListIndentAt);

  PopulateAlignment;
  PopulateDisplayFormatItems;
end;

procedure TdxRichEditCustomSimpleNumberingListForm.AlignmentValueChanged
  (Sender: TObject);
begin
  Controller.Alignment := NumberingAlignment;
end;

procedure TdxRichEditCustomSimpleNumberingListForm.ChangeFocus
...
procedure TdxRichEditCustomSimpleNumberingListForm.DisplayFormatChanged
  (Sender: TObject);
begin
  Controller.DisplayFormat := FDisplayFormatHelper.GetDisplayFormatString;
end;

procedure TdxRichEditCustomSimpleNumberingListForm.FormatValueChanged
  (Sender: TObject);
var
  AConverter: TdxOrdinalBasedNumberConverter;
  AMinValue, AMaxValue: Int64;
...
begin
  Result := TdxOrdinalBasedNumberConverter.CreateConverter(AFormat,
    TdxLanguageId.English);
end;
...
begin
  ...
  FIsCloseForm := True;
  FDisplayFormatHelper := TdxDisplayFormatHelper.Create(edtNumberFormat,
    Controller.EditedLevels, Controller.EditedLevelIndex);
  ...
  edtNumberFormat.Options.Behavior.ShowPopupMenu :=
    TdxDocumentCapability.Disabled;
  edtNumberFormat.Views.Simple.Padding.Value := Rect(3, 1, 0, 0);
end;

procedure TdxRichEditCustomSimpleNumberingListForm.NumberFormatKeyDown
...
procedure TdxRichEditCustomSimpleNumberingListForm.StartTextChanged
  (Sender: TObject);
begin
  FIsCloseForm := edStart.ValidateEdit(False);
end;

procedure TdxRichEditCustomSimpleNumberingListForm.StartValueChanged
  (Sender: TObject);
begin
