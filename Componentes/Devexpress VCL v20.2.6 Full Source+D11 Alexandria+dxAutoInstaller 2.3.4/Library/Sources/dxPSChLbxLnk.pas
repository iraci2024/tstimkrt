...
uses
  ...
  Types, dxCore, dxPSCore, dxPSGrLnks, dxPSGlbl, dxPSBaseGridLnk, cxPC,
  cxControls, cxGraphics, cxTextEdit, cxMaskEdit,
  ...
  dxLayoutControlAdapters, dxLayoutLookAndFeels, cxGeometry,
...
type
  ...
  TdxCheckListBoxPaintOption = (chlbxpoBorder, chlbxpoHorzLines,
    chlbxpoFlatCheckMarks);
  ...
  published
    ...
    property OddColor;
    property OddFont;
    property Options: TdxCheckListBoxPaintOptions read GetOptions
      write SetOptions default [chlbxpoBorder .. chlbxpoFlatCheckMarks];
    property Multiline;
...
const
  ...
  dxCheckListBoxStrings: array [0 .. DesignerStringCount - 1]
    of string = ('Sample Text Row #1', 'Sample Text Row #2',
    'Sample Text Row #3', 'Sample Text Row #4', 'Sample Text Row #5',
    'Sample Text Row #6');
...
class procedure TdxPSCheckGridCellDataMap.InitializeCellData(ACol,
...
begin
  ...
  begin
    CheckPos := ccpLeft;
    Checked := TdxCheckListBoxReportLink(AReportLink).CheckListBox.State[ARow] >
      cbUnchecked;
    Enabled := TdxCheckListBoxReportLink(AReportLink).CheckListBox.State[ARow]
      < cbGrayed;
    Enabled := Enabled and TdxCheckListBoxReportLink(AReportLink)
      .CheckListBox.ItemEnabled[ARow];
...
function TdxCheckListBoxReportLink.GetDataItemClass(ACol: Integer;
...
procedure TdxCheckListBoxReportLink.SetOptions
  (Value: TdxCheckListBoxPaintOptions);
begin
...
function TdxChlbxReportLinkDesignWindow.GetReportLink
  : TdxCheckListBoxReportLink;
begin
  Result := inherited ReportLink as TdxCheckListBoxReportLink;
end;

procedure TdxChlbxReportLinkDesignWindow.CMDialogChar
...
procedure TdxChlbxReportLinkDesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
const
  CheckBorderMap: array [Boolean] of UINT = (0, DFCS_FLAT);
  ControlStateMap: array [TCheckBoxState] of UINT = (DFCS_BUTTONCHECK,
...
begin
  ...
  for I := 0 to DesignerStringCount do
    if (((I = 0) or (I = DesignerStringCount)) and ReportLink.IsDrawBorder) or
      ((I > 0) and (I < DesignerStringCount) and ReportLink.IsDrawHorzLines)
    then
    begin
  ...
  begin
    R := Rect(R2.Left, R2.Top, R2.Left + 1,
      R2.Top + dY * DesignerStringCount + 1);
    PatBlt(DC, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top, PATCOPY);
    R := Rect(R2.Right - 1, R2.Top, R2.Right,
      R2.Top + dY * DesignerStringCount + 1);
    PatBlt(DC, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top, PATCOPY);
  end;
  ...
  begin
    R := Rect(R2.Left + 1, R2.Top + I * dY + 1, R2.Right - 1,
      R2.Top + (I + 1) * dY + Byte(not ReportLink.IsDrawHorzLines and
      (I < DesignerStringCount - 1)));
    ...
    R := Bounds(R2.Left + ScaleFactor.Apply(2), R2.Top + I * dY + 1 +
      (dY - ScaleFactor.Apply(CheckHeight)) div 2,
      ScaleFactor.Apply(CheckWidth), ScaleFactor.Apply(CheckHeight));
    uState := DFCS_TRANSPARENT or ControlStateMap[CheckStateMap[I]] or
      CheckBorderMap[ReportLink.IsFlatCheckMarks];
...
procedure TdxChlbxReportLinkDesignWindow.chbxAutoWidthClick(Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
dxPSRegisterReportLink(TdxCheckListBoxReportLink, TCheckListBox,
  TdxChlbxReportLinkDesignWindow);

finalization

dxPSUnregisterReportLink(TdxCheckListBoxReportLink, TCheckListBox,
  TdxChlbxReportLinkDesignWindow);
