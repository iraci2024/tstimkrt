...
{$ENDIF}
  ...
  dxLayoutLookAndFeels, cxClasses, dxLayoutContainer,
  dxLayoutControl, dxLayoutcxEditAdapters, cxContainer, cxEdit, cxTextEdit,
  cxMaskEdit, cxSpinEdit, cxCheckBox,
...
begin
  ...
  cbShowPageNumbers.Caption := cxGetResourceString
    (@sdxRichEditTableOfContentsShowPageNumbers);
  cbRightAlignPageNumbers.Caption :=
    cxGetResourceString(@sdxRichEditTableOfContentsRightAlignPageNumbers);
  cbUseHyperlinks.Caption := cxGetResourceString
    (@sdxRichEditTableOfContentsUseHyperlinks);
  liPrintPreview.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditTableOfContentsPrintPreview);
  liEditShowLevels.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditTableOfContentsEditShowLevels);
...
function TdxRichEditTableOfContentsForm.GetController
...
procedure TdxRichEditTableOfContentsForm.cbRightAlignPageNumbersPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxRichEditTableOfContentsForm.cbShowPageNumbersPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxRichEditTableOfContentsForm.cbUseHyperlinksPropertiesChange
  (Sender: TObject);
begin
...
function TdxRichEditTableOfContentsForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxTableOfContentsFormController.Create
    (AControllerParameters as TdxTOCFormControllerParameters);
end;
...
begin
  ...
  AListParagraphContent := cxGetResourceString
    (@sdxRichEditTableOfContentsListParagraphContent);
  ATabPosition := ADocumentModel.UnitConverter.PixelsToModelUnits
    (PreviewRichEditControl.Width - 50, DpiX);
  ...
  begin
    ...
    AContent := Format('%s%d' + TdxCharacters.TabMark,
      [AListParagraphContent, I + 1]);
    if I > 0 then
  ...
  begin
    ...
    begin
      ...
      try
        ATabInfo := TdxTabInfo.Create(ATabPosition, TdxTabAlignmentType.Right,
          TdxTabLeaderType.Dots, False, False);
        ATabs.Add(ATabInfo);
  ...
  AStartOfDocumentCommand := TdxStartOfDocumentCommand.Create
    (PreviewRichEditControl);
  try
...
begin
  ...
  PreviewRichEditControl.Options.VerticalScrollbar.Visibility :=
    TdxRichEditScrollbarVisibility.Auto;
  PreviewRichEditControl.Options.Behavior.Drag :=
    TdxDocumentCapability.Disabled;
  PreviewRichEditControl.Options.Behavior.Drop :=
    TdxDocumentCapability.Disabled;
  PreviewRichEditControl.Options.Behavior.ShowPopupMenu :=
    TdxDocumentCapability.Disabled;
  PreviewRichEditControl.EnableSelection := False;
  FillPreviewControl;
end;

procedure TdxRichEditTableOfContentsForm.spEditShowLevelsPropertiesChange
  (Sender: TObject);
begin
