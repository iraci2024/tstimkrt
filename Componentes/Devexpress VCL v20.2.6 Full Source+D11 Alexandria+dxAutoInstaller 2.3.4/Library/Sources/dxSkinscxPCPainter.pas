...
uses
  ...
  dxCore, dxCoreGraphics, cxGraphics, cxGeometry, cxControls, cxLookAndFeels,
  dxSkinsCore, cxLookAndFeelPainters,
...
type
  ...
  strict private
    ...
    procedure InternalCorrectCustomTabRect(APosition: TcxTabPosition;
      var ARect: TRect);
    procedure InternalCorrectMainTabRect(APosition: TcxTabPosition;
      var ARect: TRect);
  protected
    ...
    property NeedDrawTabBitmapBackground: Boolean
      read GetNeedDrawTabBitmapBackground;
  public
...
class function TcxPCSkinPainter.HasLookAndFeel(ALookAndFeel
  : TcxLookAndFeel): Boolean;
begin
  Result := ALookAndFeel.SkinPainter <> nil;
end;

function TcxPCSkinPainter.CalculateTabNormalWidth(ATabViewInfo
...
procedure TcxPCSkinPainter.InternalCorrectCustomTabRect
  (APosition: TcxTabPosition; var ARect: TRect);
begin
  case APosition of
    tpTop:
...
begin
  case APosition of
    ...
    tpBottom:
      ARect := Rect(-Indents[siHorzGrow],
        -Indents[siSelectedDownGrowBottomRight], Indents[siHorzGrow],
        Indents[siVertGrow]);
    tpLeft:
...
function TcxPCSkinPainter.GetTabClipRgn(ACanvas: TcxCanvas;
...
begin
  ...
  Result := TcxRegion.Create(cxRectTransform(ATabRect,
    GetTabCorrection(ATabVisibleIndex)));
end;
...
begin
  AFrameContent := FrameContent;
  if AFrameContent <> nil then
    AFrameContent.Draw(DC, cxRectOffset(GetPageFrameRect, -ATab.Left,
      -ATab.Top))
  else
    inherited;
end;

function TcxPCSkinPainter.DoGetButtonWidth
...
function TcxPCSkinPainter.GetNativeButtonsDistance(AButton1,
...
function TcxPCSkinPainter.GetTabContentWOffset(ATabVisibleIndex: Integer)
  : TcxPCWOffset;
begin
  if Header <> nil then
  begin
...
function TcxPCSkinPainter.GetHeaderButtonGlyphPalette
...
function TcxPCSkinPainter.GetTabButtonColorPalette
  (AState: TcxPCNavigatorButtonState): IdxColorPalette;
begin
...
function TcxPCSkinPainter.GetTabNormalContentOffset(ATabVisibleIndex
  : Integer): Integer;
begin
  if Header <> nil then
...
var
  ...
  begin
    ...
    NormalTabsRectCorrection :=
      RotateRect(Rect(0, Indents[siVertGrow] + ATabsContainerOffset.Top, 0, 0),
    ...
    begin
      ...
      begin
        ...
        Result.NormalRowWidth := Result.NormalTabsRect.Right -
          Result.NormalTabsRect.Left;
      end
      else
      begin
        if MultiLine or (NavigatorButtonCount = 0) then
        ...
        Result.NormalRowWidth := Result.NormalTabsRect.Bottom -
          Result.NormalTabsRect.Top;
      end;
    end;
  end;
...
function TcxPCSkinPainter.GetTabTextNormalWidth(ATabVisibleIndex
  : Integer): Integer;
begin
...
procedure TcxPCSkinPainter.DrawParentBackground(ACanvas: TcxCanvas;
  const R: TRect);
...
procedure TcxPCSkinPainter.DrawTabText(ACanvas: TcxCanvas; const ARect: TRect;
  const AText: string; AEnabled: Boolean; AColor: TColor;
...
begin
  ...
  begin
    ...
    AElement.Draw(ACanvas.Handle, ARect, ScaleFactor, AImageIndex,
      ButtonStates[AState]);
  end;
end;

procedure TcxPCSkinPainter.PaintHeaderButton(ACanvas: TcxCanvas;
...
begin
  if Header <> nil then
    Header.Draw(DC, ABounds, 0,
      GetTabState(ViewInfo.TabsViewInfo[ATabVisibleIndex]))
  else
...
begin
  Result := { (Indents[siDownGrow] > 1) and } (Header <> nil) and
    Header.IsAlphaUsed;
end;
...
function TcxPCSkinPainter.GetTabState(ATabViewInfo: TcxTabViewInfo)
  : TdxSkinElementState;
begin
  if not ATabViewInfo.Enabled then
    Result := esDisabled
    ...
    Result := esPressed
  else if ATabViewInfo.IsHotTrack or ATabViewInfo.IsHighlighted then
    Result := esHot
  else
    Result := esNormal;
end;
