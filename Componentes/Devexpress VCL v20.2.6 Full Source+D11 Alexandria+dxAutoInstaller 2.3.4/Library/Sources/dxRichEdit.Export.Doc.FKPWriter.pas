...
type
  ...
  protected
    function GetFilePositionByCharacterPosition(ACharacterPosition
      : Integer): Integer;
  ...
  public
    constructor Create(ATextStartPosition: Integer;
      APropertiesStream: TdxMemoryStream; ADataStreamWriter: TBinaryWriter);
    destructor Destroy; override;
    procedure WriteTextRun(ACharacterPosition: Integer;
      const APropertyModifiers: TBytes); overload;
    procedure WriteTextRun(ACharacterPosition: Integer); overload;
    procedure WriteTextRunBase(ACharacterPosition: Integer;
      const AGrpprl: TBytes);
    procedure WriteParagraph(ACharacterPosition: Integer;
      AParagraphStyleIndex: Integer); overload;
    procedure WriteParagraph(ACharacterPosition: Integer;
      AParagraphStyleIndex: Integer; const APropertyModifiers: TBytes);
      overload;
    procedure WriteParagraphBase(ACharacterPosition: Integer;
      AParagraphStyleIndex: Integer; const AGrpprl: TBytes);
    procedure WriteParagarphGrpprlAndPosition(AFilePosition: Integer;
      AParagraphStyleIndex: Integer; const AGrpprl: TBytes);
    procedure CreateHugeParagraphFKP(AFilePosition: Integer;
      AStyleIndex: Integer; const AGrpprl: TBytes);
    procedure WriteTableProperties(ACharacterPosition: Integer;
      AStyleIndex: Integer; const AGrpprl: TBytes);
...
constructor TdxFKPWriter.Create(ATextStartPosition: Integer;
  APropertiesStream: TdxMemoryStream; ADataStreamWriter: TBinaryWriter);
begin
...
procedure TdxFKPWriter.WriteTextRun(ACharacterPosition: Integer;
  const APropertyModifiers: TBytes);
begin
...
procedure TdxFKPWriter.WriteTextRunBase(ACharacterPosition: Integer;
  const AGrpprl: TBytes);
var
...
procedure TdxFKPWriter.WriteParagraph(ACharacterPosition: Integer;
...
procedure TdxFKPWriter.WriteParagraph(ACharacterPosition: Integer;
  AParagraphStyleIndex: Integer; const APropertyModifiers: TBytes);
begin
  WriteParagraphBase(ACharacterPosition, AParagraphStyleIndex,
    APropertyModifiers);
end;

procedure TdxFKPWriter.WriteParagraphBase(ACharacterPosition: Integer;
  AParagraphStyleIndex: Integer; const AGrpprl: TBytes);
var
  AFilePosition: Integer;
begin
...
procedure TdxFKPWriter.WriteParagarphGrpprlAndPosition(AFilePosition: Integer;
  AParagraphStyleIndex: Integer; const AGrpprl: TBytes);
begin
  if FCurrentParagraphFKP.TryToAddGrpprlAndPosition(AFilePosition,
    AParagraphStyleIndex, AGrpprl) then
    Exit;
...
procedure TdxFKPWriter.CreateHugeParagraphFKP(AFilePosition: Integer;
  AStyleIndex: Integer; const AGrpprl: TBytes);
var
  AOpcode: SmallInt;
  AOperand, AExtendedPropertyModifiersData: TBytes;
begin
  AOpcode := TdxDocCommandFactory.GetOpcodeByType
    (TdxDocCommandReadExtendedPropertyModifiers);
  AOperand := TdxByteArrayHelper.From<SmallInt>
    (SmallInt(DataStreamWriter.BaseStream.Position));
  AExtendedPropertyModifiersData :=
    TdxDocCommandHelper.CreateSinglePropertyModifier(AOpcode, AOperand);
  if not FCurrentParagraphFKP.TryToAddGrpprlAndPosition(AFilePosition,
    AStyleIndex, AExtendedPropertyModifiersData) then
    TdxRichEditExceptions.ThrowInternalException;
  DataStreamWriter.Write(SmallInt(Length(AGrpprl)));
  DataStreamWriter.Write(AGrpprl);
end;

procedure TdxFKPWriter.WriteTableProperties(ACharacterPosition: Integer;
  AStyleIndex: Integer; const AGrpprl: TBytes);
var
  AFilePosition: Integer;
  AOpcode: SmallInt;
...
begin
  ...
  AOperand := TdxByteArrayHelper.From<Integer>
    (Integer(DataStreamWriter.BaseStream.Position));
  ATablePropertyModifiers := TdxDocCommandHelper.CreateSinglePropertyModifier
    (AOpcode, AOperand);
  DataStreamWriter.Write(SmallInt(Length(AGrpprl)));
  DataStreamWriter.Write(AGrpprl);
  WriteParagarphGrpprlAndPosition(AFilePosition, AStyleIndex,
    ATablePropertyModifiers);
end;
...
begin
  ALastFilePosition := GetFilePositionByCharacterPosition
...
function TdxFKPWriter.GetFilePositionByCharacterPosition(ACharacterPosition
  : Integer): Integer;
begin
