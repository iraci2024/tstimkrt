...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Types, Variants, Windows, Classes, Controls, Dialogs, Forms, Graphics,
  Messages,
  Printers, SysUtils,
  dxCore, dxCoreClasses, cxClasses, cxContainer, cxControls, cxGraphics, cxEdit,
  cxImageComboBox, dxCoreGraphics,
  cxGeometry,
...
type
  ...
  protected
    class procedure BorderTablePainter(ABorderInfo: TdxBorderInfo;
      APainter: TdxPainter; AUnitConverter: TdxDocumentModelUnitConverter;
      AScaleFactor: Integer; const AProc: TProc<TdxTableBorderPainter>); static;
    class function GetTableBorderLine(ABorderLineStyle: TdxBorderLineStyle)
      : TdxUnderline; static;
  public
    class procedure DrawBorderLineItem(ABorderInfo: TdxBorderInfo;
      ACanvas: TcxCanvas; R: TRect;
      AUnitConverter: TdxDocumentModelUnitConverter; AIsSelected: Boolean);
      overload; static;
    class procedure DrawBorderLineItem(ABorderInfo: TdxBorderInfo;
      ACanvas: TcxCanvas; const R: TRect; ABackgroundColor: TdxAlphaColor;
      AForegroundColor: TdxAlphaColor;
      AUnitConverter: TdxDocumentModelUnitConverter); overload; static;
    class procedure PopulateBorderLineWeight(AProperties
      : TdxCustomBorderLineWeightComboBoxProperties;
      ABorderStyle: TdxBorderLineStyle; ABorderColor: TdxAlphaColor;
  ...
  private
    ...
    procedure SetProperties(const Value
      : TdxCustomBorderLineWeightComboBoxProperties);
    function GetActiveProperties: TdxCustomBorderLineWeightComboBoxProperties;
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;

    property ActiveProperties: TdxCustomBorderLineWeightComboBoxProperties
      read GetActiveProperties;
    property Properties: TdxCustomBorderLineWeightComboBoxProperties
      read GetProperties write SetProperties;
  end;

  { TdxBorderLineWeightComboBoxProperties }

  TdxBorderLineWeightComboBoxProperties = class
    (TdxCustomBorderLineWeightComboBoxProperties);
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TdxBorderLineWeightComboBoxProperties
      read GetActiveProperties;
  published
    property Anchors;
    property AutoSize;
    ...
    property Properties: TdxBorderLineWeightComboBoxProperties
      read GetProperties write SetProperties;
    property ShowHint;
    property Style;
...
class procedure TdxBorderLineWeightPainter.BorderTablePainter
  (ABorderInfo: TdxBorderInfo; APainter: TdxPainter;
  AUnitConverter: TdxDocumentModelUnitConverter; AScaleFactor: Integer;
  const AProc: TProc<TdxTableBorderPainter>);
var
...
begin
  ...
  try
    ACompoundArray := ABorderCalculator.GetDrawingCompoundArray
      (ABorderInfo.Style);
    ...
    if not Assigned(ACompoundArray) then
      Exit;
    AThickness := Max(1, AUnitConverter.ModelUnitsToPixels(ABorderInfo.Width *
      AScaleFactor, TdxCustomDocumentModel.Dpi));
    AWidth := ABorderCalculator.GetActualWidth(ABorderInfo.Style, AThickness);
    ALayoutUnitConverter := TdxDocumentLayoutUnitPixelsConverter.Create
      (TdxCustomDocumentModel.Dpi);
    AHorizontalLinePainter := TdxRichEditHorizontalPatternLinePainter.Create
      (APainter, ALayoutUnitConverter);
    AVerticalLinePainter := TdxRichEditVerticalPatternLinePainter.Create
      (APainter, ALayoutUnitConverter);
    APainterWrapper := TdxGraphicsPainterWrapper.Create(APainter,
      AHorizontalLinePainter, AVerticalLinePainter);
    try
      if Length(ACompoundArray) = 4 then
        ABorderPainter := TdxDoubleBorderPainter.Create(APainterWrapper,
          ACompoundArray, AWidth)
      else if Length(ACompoundArray) = 6 then
        ABorderPainter := TdxTripleBorderPainter.Create(APainterWrapper,
          ACompoundArray, AWidth)
      else
        ABorderPainter := TdxSingleBorderPainter.Create(APainterWrapper, AWidth,
          GetTableBorderLine(ABorderInfo.Style));
      try
...
class function TdxBorderLineWeightPainter.GetTableBorderLine(ABorderLineStyle
  : TdxBorderLineStyle): TdxUnderline;
begin
  if ABorderLineStyle = TdxBorderLineStyle.Single then
    Result := nil
  else
    Result := TdxBorderInfoRepository.GetUnderlineByBorderLineStyle
      (ABorderLineStyle);
end;

class procedure TdxBorderLineWeightPainter.DrawBorderLineItem
  (ABorderInfo: TdxBorderInfo; ACanvas: TcxCanvas; const R: TRect;
...
var
  ...
  APrevOrigin: TPoint;
begin
  ABkColor := SetBkColor(ACanvas.Handle,
    ColorToRGB(TdxAlphaColors.ToColor(ABackgroundColor)));
  AFgColor := SetTextColor(ACanvas.Handle,
    ColorToRGB(TdxAlphaColors.ToColor(AForegroundColor)));
...
class procedure TdxBorderLineWeightPainter.DrawBorderLineItem
  (ABorderInfo: TdxBorderInfo; ACanvas: TcxCanvas; R: TRect;
  AUnitConverter: TdxDocumentModelUnitConverter; AIsSelected: Boolean);
var
...
begin
  ...
  if ABorderInfo.Style in [TdxBorderLineStyle.DoubleWave,
    TdxBorderLineStyle.Wave] then
  begin
    AUnderline := TdxBorderInfoRepository.GetUnderlineByBorderLineStyle
      (ABorderInfo.Style);
    AThickness := Max(1, AUnitConverter.ModelUnitsToPixels(ABorderInfo.Width,
      TdxCustomDocumentModel.Dpi));
    TdxUnderlinePainterHelper.DrawUnderlineItemCore(AUnderline, ACanvas, R,
      AColor, AThickness, AThickness);
    Exit;
  end;
    ...
    try
      ...
      BorderTablePainter(ABorderInfo, APainter, AUnitConverter, AScaleFactor,
        ...
        begin
          ...
          ABorderPainter.DrawHorizontalBorder(AColor, TdxPointF.Create(R.Left,
            -ABorderPainter.Width / 2.0), R.Width);
        end);
    finally
      APainter.Free;
    end;
...
class procedure TdxBorderLineWeightPainter.PopulateBorderLineWeight
  (AProperties: TdxCustomBorderLineWeightComboBoxProperties;
ABorderStyle: TdxBorderLineStyle; ABorderColor: TdxAlphaColor;
AUnitConverter: TdxDocumentModelUnitConverter;
ABordersLineWeights: array of Single);
var
  AWeight: Single;
  I, AValue: Integer;
  ABitmap: TcxBitmap;
    ...
    try
      ...
      try
        ...
        begin
          ...
          DrawBorderLineItem(ABorderInfo, cxPaintCanvas, ABitmap.ClientRect,
            clWhite, clBlack, AUnitConverter);
          cxPaintCanvas.EndPaint;
          AItem := AItems.Add;
          AItem.ImageIndex := AImageList.Add(ABitmap, nil);
          AItem.Value := AValue;
          AItem.Description := TdxStringHelper.Format('%g %s',
            [ABordersLineWeights[I], AMeasurement]);
        end;
      finally
        AItems.EndUpdate;
      end;
...
constructor TdxCustomBorderLineWeightComboBoxProperties.Create
  (AOwner: TPersistent);
begin
...
procedure TdxCustomBorderLineWeightComboBoxProperties.DoAssign
...
class function TdxCustomBorderLineWeightComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TdxCustomBorderLineWeightComboBoxProperties;
end;

function TdxCustomBorderLineWeightComboBox.GetActiveProperties
  : TdxCustomBorderLineWeightComboBoxProperties;
begin
  Result := TdxCustomBorderLineWeightComboBoxProperties
...
function TdxCustomBorderLineWeightComboBox.GetProperties
  : TdxCustomBorderLineWeightComboBoxProperties;
begin
  Result := TdxCustomBorderLineWeightComboBoxProperties(inherited Properties);
end;

procedure TdxCustomBorderLineWeightComboBox.SetProperties
  (const Value: TdxCustomBorderLineWeightComboBoxProperties);
begin
...
function TdxBorderLineWeightComboBox.GetActiveProperties
  : TdxBorderLineWeightComboBoxProperties;
begin
  Result := TdxBorderLineWeightComboBoxProperties(InternalGetActiveProperties);
end;

function TdxBorderLineWeightComboBox.GetProperties
  : TdxBorderLineWeightComboBoxProperties;
begin
  Result := TdxBorderLineWeightComboBoxProperties(inherited Properties);
end;

class function TdxBorderLineWeightComboBox.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TdxBorderLineWeightComboBoxProperties;
end;

procedure TdxBorderLineWeightComboBox.SetProperties
  (Value: TdxBorderLineWeightComboBoxProperties);
begin
  Properties.Assign(Value);
end;
