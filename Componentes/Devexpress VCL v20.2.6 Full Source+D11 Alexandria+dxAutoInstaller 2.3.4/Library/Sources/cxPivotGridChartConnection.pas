...
uses
  Windows, SysUtils, Messages, Classes, cxClasses, cxCustomPivotGrid,
  cxGridChartView, Variants, Forms,
  cxControls, cxVariants;
...
type
  ...
  public
    property Items[Index: Integer]: TcxPivotGridChartCellInfo
  ...
  strict private
    ...
    function GetDataItemDisplayText(ADataItem
      : TcxPivotGridViewDataItem): string;
    function GetRowDataItem(Index: Integer): TcxPivotGridViewDataItem;
  public
    constructor Create; virtual;
    destructor Destroy; override;
    procedure AddCell(const AValue: Variant;
    ...
    property RowDataItem[Index: Integer]: TcxPivotGridViewDataItem
      read GetRowDataItem;
  ...
  TcxPivotGridChartGetDataCellsEvent = procedure
  ...
  strict private
    ...
    procedure SetSourceForCategories
      (AValue: TcxPivotGridChartViewSourceForCategories);
  protected
    procedure DoChanged; virtual;
    function DoGetDataCells(ACol, ARow: Integer;
      AViewData: TcxPivotGridViewData): Boolean; virtual;
    ...
    function IsTotal(ACol, ARow: Integer;
  ...
  published
    property GridChartView: TcxGridChartView read FGridChartView
      write SetGridChartView;
    property PivotGrid: TcxCustomPivotGrid read FPivotGrid write SetPivotGrid;
    property SourceData: TcxPivotGridChartViewSourceData read FSourceData
      write SetSourceData default sdAllShown;
    property SourceForCategories: TcxPivotGridChartViewSourceForCategories
      read FSourceForCategories write SetSourceForCategories default sfcRows;
    property OnGetDataCells: TcxPivotGridChartGetDataCellsEvent
      read FOnGetDataCells write FOnGetDataCells;
    property OnGetSeriesDisplayText: TcxPivotGridChartGetSeriesDisplayTextEvent
      read FOnGetSeriesDisplayText write FOnGetSeriesDisplayText;
  end;
...
type
  ...
  strict private
    class var FInstance: TcxPivotGridChartConnectionController;
  strict private
    FList: TList;
  protected
...
function TcxPivotGridChartSeriesHelper.CreateUniqueName(AOwnerComponent,
  AComponent: TComponent; const APrefixName: string;
...
procedure TcxPivotGridChartSeriesHelper.InitializeComponentList
  (AOwner: TComponent);
var
  I: Integer;
begin
...
function TcxPivotGridChartCellInfosList.GetItem(AIndex: Integer)
  : TcxPivotGridChartCellInfo;
begin
...
function TcxPivotGridChartConnection.DoGetDataCells(ACol, ARow: Integer;
  AViewData: TcxPivotGridViewData): Boolean;
begin
  Result := ((SourceData = sdAllShown) and not IsTotal(ACol, ARow, AViewData))
    or ((SourceData = sdSelected) and AViewData.IsCellSelected(ARow, ACol));
...
procedure TcxPivotGridChartConnection.PopulateStorage(const AViewData
  : TcxPivotGridViewData; const AStorage: TcxPivotGridChartConnectionStorage);
var
  ...
  begin
    ...
    for J := 0 to AViewData.RowCount - 1 do
      ...
      begin
        ...
        if GetDataFieldValue(AColumn, ARow, AViewData, AViewData.Cells[J, I],
          AValue) then
        begin
...
procedure TcxPivotGridChartConnection.PopulateChartView(AGridChartView
  : TcxGridChartView; AStorage: TcxPivotGridChartConnectionStorage);
var
  I: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      AGridChartView.ViewData.Values[ACell.RowIndex, ACell.ColumnIndex] :=
        ACell.Value;
    end;
  finally
...
begin
  if (APivotGrid <> nil) and (AChartView <> nil) and
    ([csDesigning, csLoading] * (ComponentState + APivotGrid.ComponentState +
    AChartView.ComponentState) = []) then
  begin
...
procedure TcxPivotGridChartConnection.InitializeChart
  (AStorage: TcxPivotGridChartConnectionStorage;
  AGridChartView: TcxGridChartView);
var
  ...
  try
    ...
    try
      ...
      begin
        ...
        ASeries.Name := AHelper.CreateUniqueName(AGridChartView, ASeries,
          GetValidName(AStorage.Rows.Strings[I]), AIndex);
        ...
        else if ARowDataItem.IsGrandTotal and
          (ARowDataItem.Level < PivotGrid.ViewData.DataBuilder.DataFields.Count)
        then
          ASeries.ValueCaptionFormat :=
            PivotGrid.ViewData.DataBuilder.DataFields[ARowDataItem.Level]
            .DisplayFormat;
      end;
    finally
      AHelper.Free;
...
function TcxPivotGridChartConnection.IsTotal(ACol, ARow: Integer;
  ...
  begin
    Result := (AGroupItem <> nil) and not AGroupItem.IsCollapsed and
      (AGroupItem.ItemCount > 0);
  end;

begin
  Result := AViewData.Columns[ACol].IsTotalItem or AViewData.Rows[ARow]
    .IsTotalItem or TcxPivotGridOptionsViewAccess(PivotGrid.OptionsView)
    .IsCompactLayout and IsGroupTotal(AViewData.Rows[ARow].GroupItem);
end;
...
procedure TcxPivotGridChartConnection.SelectionChanged
...
function TcxPivotGridChartConnection.GetDataFieldFromViewData
  (ADataBuilder: TcxPivotGridDataBuilder; AItem: TcxPivotGridViewDataItem)
  : TcxPivotGridField;
var
  I: Integer;
begin
...
function TcxPivotGridChartConnection.GetSummaryType(ACol,
  ARow: TcxPivotGridViewDataItem;
  var ASummaryType: TcxPivotGridSummaryType): Boolean;
var
...
function TcxPivotGridChartConnection.GetDataFieldValue(ACol,
  ARow: TcxPivotGridViewDataItem; AViewData: TcxPivotGridViewData;
...
begin
  ...
  begin
    ...
    if GetSummaryType(ACol, ARow, ASummaryType) or
      (ADataField.SummaryVariation = svNone) then
...
procedure TcxPivotGridChartConnection.SetGridChartView
  (AValue: TcxGridChartView);
begin
...
procedure TcxPivotGridChartConnectionStorage.AddCell(const AValue: Variant;
  ARow, AColumn: TcxPivotGridViewDataItem);
begin
  if ARow.Tag = 0 then
    ARow.Tag := Rows.AddObject(GetDataItemDisplayText(ARow), ARow) + 1;
  if AColumn.Tag = 0 then
    AColumn.Tag := Columns.AddObject(GetDataItemDisplayText(AColumn),
      AColumn) + 1;
  Cells.Add(TcxPivotGridChartCellInfo.Create(AValue, AColumn.Tag - 1,
    ARow.Tag - 1));
end;
...
begin
  ...
  if (ADataItem.Level > 0) and (ADataItem.Parent <> nil) and
    (ADataItem.Parent.Value <> '') then
    Result := GetDataItemDisplayText(ADataItem.Parent) + ' - ' + Result;
end;

function TcxPivotGridChartConnectionStorage.GetRowDataItem(Index: Integer)
...
class procedure TcxPivotGridChartConnectionController.PostRefresh
  (AOwner: TcxPivotGridChartConnection);
begin
  if FInstance <> nil then
    PostMessage(FInstance.Handle, WM_USER, 0, LPARAM(AOwner));
end;

class procedure TcxPivotGridChartConnectionController.Register
  (AOwner: TcxPivotGridChartConnection);
begin
  if FInstance = nil then
...
class procedure TcxPivotGridChartConnectionController.Unregister
  (AOwner: TcxPivotGridChartConnection);
begin
  if FInstance <> nil then
  begin
