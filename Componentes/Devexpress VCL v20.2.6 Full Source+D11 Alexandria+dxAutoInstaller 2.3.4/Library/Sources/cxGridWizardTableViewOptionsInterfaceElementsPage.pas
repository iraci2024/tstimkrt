...
uses
  ...
  dxLayoutContainer, cxGroupBox, ExtCtrls, cxTextEdit, cxMaskEdit, cxClasses,
  cxDropDownEdit, cxCheckBox, Math,
  dxLayoutControl, cxGridWizardCustomPage, cxGridWizardStrs, cxGridTableView,
  dxLayoutLookAndFeels, cxLabel,
  cxGridWizardCustomHelper, cxGridInplaceEditForm, cxCheckComboBox,
  cxGridCustomTableView, cxNavigator;

type
  { TcxGridWizardTableViewOptionsInterfaceElementsPageFrame }

  TcxGridWizardTableViewOptionsInterfaceElementsPageFrame = class
...
procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  ApplyLocalization;
begin
  lbInterfaceElements.Caption := cxGetResourceString
    (@scxgwUIElementsPageGroupInterfaceElements);
  ...
  chbGroupFooter.Caption := cxGetResourceString
    (@scxgwUIElementsPageGroupFooter);
  chbGroupFooter.Hint := cxGetResourceString
  ...
  chbFilterRow.Caption := cxGetResourceString
    (@scxgwUIElementsPageFilterRowVisible);
  chbFilterRow.Hint := cxGetResourceString
  ...
  chbBandsQuickCustomization.Caption :=
    cxGetResourceString(@scxgwUIElementsPageBandsQuickCustomization);
  chbBandsQuickCustomization.Hint :=
    cxGetResourceString(@scxgwUIElementsPageBandsQuickCustomizationHint);

  chbColumnsQuickCustomization.Caption :=
    cxGetResourceString(@scxgwUIElementsPageColumnsQuickCustomization);
  chbColumnsQuickCustomization.Hint :=
    cxGetResourceString(@scxgwUIElementsPageColumnsQuickCustomizationHint);
  ...
  lciNavigatorButtons.Caption := cxGetResourceString
    (@scxgwUIElementsPageNavigatorButtons);
  chcbNavigatorButtons.Hint := cxGetResourceString
  ...
  chbInplaceEditForm.Caption := cxGetResourceString
    (@scxgwUIElementsPageInplaceEditForm);
  chbInplaceEditForm.Hint := cxGetResourceString
    (@scxgwUIElementsPageInplaceEditFormHint);

  chbHideCurrentRow.Caption := cxGetResourceString
    (@scxgwUIElementsPageHideCurrentRow);
  chbHideCurrentRow.Hint := cxGetResourceString
...
begin
  ...
  AView.OptionsCustomize.ColumnsQuickCustomization :=
    chbColumnsQuickCustomization.Checked;
  if Supports(Helper, IcxGridWizardBandedTableView, ABandedTableViewInterface)
  then
    ABandedTableViewInterface.SetBandsQuickCustomization
  ...
  else if cbGroupFooterMode.ItemIndex = 0 then
...
begin
  ...
  lciBandsQuickCustomization.Visible :=
    Supports(Helper, IcxGridWizardBandedTableView, ABandedTableViewInterface);
  ...
  if lciBandsQuickCustomization.Visible then
    chbBandsQuickCustomization.Checked :=
      ABandedTableViewInterface.GetBandsQuickCustomization;
  chbFilterRow.Checked := AView.FilterRow.Visible;
  chbIndicator.Checked := AView.OptionsView.Indicator or
    chbColumnsQuickCustomization.Checked or chbBandsQuickCustomization.Checked;
  ...
  cbGroupFooterMode.ItemIndex :=
    Ord(AView.OptionsView.GroupFooters = gfAlwaysVisible);
...
function TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
...
begin
  ...
  try
    ...
    cbGroupFooterMode.Properties.Items.Add
      (cxGetResourceString
      (@scxgwUIElementsPageGroupFooterModeVisibleWhenExpanded));
    cbGroupFooterMode.Properties.Items.Add
      (cxGetResourceString(@scxgwUIElementsPageGroupFooterModeAlwaysVisible));
  finally
...
procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  PopulateNavigatorButtons(AButtons: TcxGridViewNavigatorButtons);
const
  CheckBoxStateMap: array [Boolean] of TcxCheckBoxState = (cbsUnchecked,
    cbsChecked);
var
  I: Integer;
begin
  ...
  try
    ...
    begin
      chcbNavigatorButtons.Properties.Items.Add.Description :=
        scxGridWizardNavigatorButtonsNames[I];
      chcbNavigatorButtons.States[I] := CheckBoxStateMap
        [AButtons.Buttons[I].Visible];
    end;
  finally
...
procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  SaveNavigatorButtons(AButtons: TcxGridViewNavigatorButtons);
const
...
begin
  for I := 0 to NavigatorButtonCount - 1 do
    AButtons.Buttons[I].Visible := CheckBoxStateMap
      [chcbNavigatorButtons.States[I]];
end;
...
procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  RefreshPreviewGrid(Sender: TObject);
begin
  RefreshPreviewGridContent;
end;

procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  chbGroupFooterClick(Sender: TObject);
begin
  UpdateControlsState;
...
procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  chbIndicatorClick(Sender: TObject);
begin
  UpdateControlsState;
...
procedure TcxGridWizardTableViewOptionsInterfaceElementsPageFrame.
  chbPreviewClick(Sender: TObject);
var
...
begin
  ...
  begin
    if ATableView.Preview.Column <> nil then
      cbPreviewColumn.ItemIndex := cbPreviewColumn.Properties.Items.IndexOf
        (ATableView.Preview.Column.Caption)
    else
