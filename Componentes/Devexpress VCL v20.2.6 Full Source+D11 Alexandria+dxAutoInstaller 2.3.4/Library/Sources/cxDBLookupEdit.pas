...
type
  ...
  protected
    ...
    procedure DBLookupGridCheckColumnByFieldName(const AFieldName: string);
      virtual; // if a column does not exist, then create it with zero index
    ...
    function FindByText(AItemIndex: Integer; const AText: string;
      APartialCompare: Boolean): Integer; override;
    ...
    property IsUseLookupList: Boolean read GetIsUseLookupList
      write SetIsUseLookupList;
  public
    ...
    property DataController: TcxDBDataController
      read GetDBLookupGridDataController;
    property KeyFieldNames: string read GetKeyFieldNames write SetKeyFieldNames;
    property ListField: TField read GetListField;
    property ListFieldNames: string read GetListFieldNames
      write SetListFieldNames stored False;
    property ListFieldIndex: Integer read GetListFieldIndex
      write SetListFieldIndex default 0;
  end;
...
function IsLinkedToDataSet(ADataSource: TDataSource { list };
  ADataSet: TDataSet { data binding } ): Boolean;
...
begin
  ...
  begin
    if ADataSet = AListDataSet then
      Exit;
    if (ADataSet.DataSetField <> nil) and
      (ADataSet.DataSetField.DataSet = AListDataSet) then
      Exit;
...
procedure TcxCustomDBLookupEditLookupData.DoSetCurrentKey
...
function TcxCustomDBLookupEditLookupData.GetProperties
  : TcxCustomDBLookupEditProperties;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      FLookupField := TcxCustomDBLookupEditProperties(Source).FLookupField;
      CaseSensitiveSearch := TcxCustomDBLookupEditProperties(Source)
        .CaseSensitiveSearch;
...
class function TcxCustomDBLookupEditProperties.GetContainerClass
  : TcxContainerClass;
begin
...
begin
  ...
  begin
    ADefaultValuesProvider := TcxCustomEditDefaultValuesProvider
      (IDefaultValuesProvider.GetInstance);
    if ADefaultValuesProvider is TcxCustomDBEditDefaultValuesProvider then
      Result := TcxCustomDBEditDefaultValuesProvider
        (ADefaultValuesProvider).Field;
  end;
end;

function TcxCustomDBLookupEditProperties.GetEditValueSource
...
begin
  Result := GetDataField;
  if (Result <> nil) and
    (not Result.Lookup or (csDestroying in Result.ComponentState)) then
    Result := nil;
end;
...
procedure TcxCustomDBLookupEditProperties.PrepareDisplayValue(const AEditValue
  : TcxEditValue; var DisplayValue: TcxEditValue; AEditFocused: Boolean);
begin
  if CanDisplayArbitraryEditValue and (DropDownListStyle <> lsEditList) and not AEditFocused
  then
...
procedure TcxCustomDBLookupEditProperties.DBLookupGridCheckColumnByFieldName
  (const AFieldName: string);
begin
end;

procedure TcxCustomDBLookupEditProperties.DBLookupGridCreateColumnsByFieldNames
...
function TcxCustomDBLookupEditProperties.GetDBLookupGridColumnField
  (AIndex: Integer): TField;
begin
  Result := nil;
end;

function TcxCustomDBLookupEditProperties.GetDBLookupGridColumnFieldName
  (AIndex: Integer): string;
begin
  Result := '';
end;

function TcxCustomDBLookupEditProperties.GetDBLookupGridColumnIndexByFieldName
  (const AFieldName: string): Integer;
begin
  Result := -1;
end;

function TcxCustomDBLookupEditProperties.GetDBLookupGridDataController
  : TcxDBDataController;
begin
  Result := nil;
end;
...
procedure TcxCustomDBLookupEditProperties.CheckLookup;
  ...
  begin
    AField := GetDataField;
    if Assigned(AField) and Assigned(DataController) and
...
procedure TcxCustomDBLookupEditProperties.LookupSourceFreeNotification
  (Sender: TComponent);
begin
  CheckLookup;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FLookupSourceFreeNotificator.OnFreeNotification :=
        LookupSourceFreeNotification;
      ...
      begin
        ...
        try
          ...
          if GetDBLookupGridColumnIndexByFieldName
            (FLookupField.LookupResultField) = -1 then
    ...
    begin
      FreeAndNil(FLookupSourceFreeNotificator);
      FLookupSource := nil;
      if (DataController <> nil) and
        (DataController.DataSource = FCachedLookupSource) then
...
function TcxCustomDBLookupEditProperties.FindByText(AItemIndex: Integer;
  ...
  begin
    ...
    if (AText = '') and not(DataController.GetItemField(AItemIndex)
      is TStringField) then
      Result := Null;
  end;
...
function TcxCustomDBLookupEditProperties.GetDisplayLookupText
      ...
      begin
        ...
        try
          if (ADataSet <> nil) and ADataSet.Active and (ListField <> nil) and
            CanCallDataSetLocate(DataController.DataSet, KeyFieldNames, AKey)
            and ADataSet.Locate(KeyFieldNames, AKey, []) then
    ...
    begin
      ...
      begin
        ...
        if (AProperties <> nil) and
          not(AProperties is TcxCustomLookupEditProperties) and
          not IsFieldFormatted(DataController.GetItemField(AItemIndex), False)
        then
          Result := AProperties.GetDisplayText
...
function TcxCustomDBLookupEditProperties.GetKeyByRecordIndex
...
class function TcxCustomDBLookupEditProperties.GetLookupDataClass
...
function TcxCustomDBLookupEditProperties.GetRecordIndexByKey
...
procedure TcxCustomDBLookupEditProperties.SetListFieldNames
  (const Value: string);
var
  AChanged: Boolean;
begin
...
class function TcxCustomDBLookupEdit.GetPropertiesClass
...
function TcxCustomDBLookupEdit.GetClearValue: TcxEditValue;
  ...
  begin
    ...
    try
      ActiveProperties.FLookupSource.DataSet.GetFieldList(AFieldList,
        ActiveProperties.KeyFieldNames);
...
function TcxCustomDBLookupEdit.ItemIndexToLookupKey(AItemIndex: Integer)
  : TcxEditValue;
begin
  Result := ActiveProperties.GetKeyByRecordIndex(AItemIndex);
end;

function TcxCustomDBLookupEdit.LookupKeyToEditValue(const AKey: TcxEditValue)
  : TcxEditValue;
begin
  Result := AKey;
end;

function TcxCustomDBLookupEdit.LookupKeyToItemIndex
...
procedure TcxCustomDBLookupEdit.PrepareDisplayValue(const AEditValue
  : TcxEditValue; var DisplayValue: TcxEditValue; AEditFocused: Boolean);
begin
  if (ActiveProperties.DropDownListStyle <> lsEditList) and not Focused and
    ...
    ActiveProperties.PrepareDisplayValue(AEditValue, DisplayValue,
      AEditFocused);
end;
...
procedure TcxCustomDBLookupEdit.SetProperties
  (Value: TcxCustomDBLookupEditProperties);
begin
