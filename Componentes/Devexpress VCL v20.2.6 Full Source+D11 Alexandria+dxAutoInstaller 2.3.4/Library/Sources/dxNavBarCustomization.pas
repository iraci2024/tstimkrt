...
uses
  Types, Windows, SysUtils, Classes, Controls, ActnList, Menus, StdCtrls,
  Messages,
...
type
  ...
  private
    ...
    procedure InternalAddGroup(ATargetGroup: TdxNavBarGroup = nil;
      APosition: Integer = -1);
    procedure InternalAddItem(AItemClass: TdxNavBarItemClass);
    procedure TreeViewWndProcHandler(ATreeView: TTreeView;
  ...
  protected
    ...
    function IsGroupCaptionTarget(const APoint: TPoint;
      var AGroup: TdxNavBarGroup; var ARect: TRect): Boolean; override;
  ...
  public
    constructor Create(ANavBar: TdxCustomNavBar;
      ACustomizationForm: TdxNavBarCustomizationForm);
  end;
  ...
  public
    constructor Create(ACustomizationForm: TdxNavBarCustomizationForm;
      AControl: TControl); reintroduce; virtual;
  end;
...
uses
  Forms, Graphics, cxClasses, dxComCtrlsUtils, dxNavBarGroupItems,
  dxNavBarConsts;
...
function TdxNavBarCustomizationForm.CanProcessDropItem(Target: TObject;
  X, Y: Integer): Boolean;
begin
  Result := True;
end;

procedure TdxNavBarCustomizationForm.DoProcessDropItem(Target: TObject;
  X, Y: Integer);
begin
  inherited;
  BeginUpdate;
  try
    ...
    begin
      if dxNavBarDragObject.SourceLink <> nil then
        dxNavBarDragObject.SourceLink.Group.RemoveLink
          (dxNavBarDragObject.SourceLink.Index);
    end
    else
...
function TdxNavBarCustomizationForm.IsDropTargetControl
  (AControl: TWinControl): Boolean;
begin
  Result := (AControl = tvGroups.InnerTreeView);
end;

procedure TdxNavBarCustomizationForm.CalculateDropInfo(var ADropTargetInfo
    ...
    try
      ...
      if Result and (TdxNavBarAccess(NavBar).FDesignHelper <> nil) then
        ...
        begin
          ...
          Result := not(AObject is TComponent) or TdxNavBarAccess(NavBar)
            .FDesignHelper.CanDeleteComponent(NavBar, TComponent(AObject));
          if not Result then
            Break;
        end;
    finally
...
procedure TdxNavBarCustomizationForm.FreeSelectedItemsData
...
function TdxNavBarCustomizationForm.GetItemText
  (AItem: TdxNavBarCustomItem): string;
begin
  if NavBar.IsDesigning then
    Result := AItem.Name
  else
...
procedure TdxNavBarCustomizationForm.MoveGroupsOrLinks(ADelta: Integer);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        APos.Position := APosition;
        if InRange(APosition, 0, ALink.Group.ChildCount - 1) and
          (AList.IndexOf(APos) = -1) then
        begin
  ...
  procedure MoveSiblings(ANodes: TTreeNodes; ADelta: Integer;
...
procedure TdxNavBarCustomizationForm.MoveItems(ADelta: Integer);
  ...
  procedure MoveItem(ANode: TTreeNode; ADelta: Integer; AList: TList<Integer>;
    var AChanged: Boolean);
  var
    AIndex: Integer;
    AItem: TdxNavBarItem;
  begin
    ...
    begin
      ...
      if InRange(AIndex, 0, NavBar.Items.Count - 1) and
        (AList.IndexOf(AIndex) = -1) then
      begin
        AItem.Index := AIndex;
        AChanged := True;
      end;
...
procedure TdxNavBarCustomizationForm.PopulateGroups;
  ...
  procedure AddGroup(AGroup: TdxNavBarGroup; AParentNode: TTreeNode;
    AExpandedList: TList);
  var
    I: Integer;
    ANode: TTreeNode;
...
procedure TdxNavBarCustomizationForm.SynchronizeSelection
  (ATreeView: TcxTreeView);
var
  AList: TList;
  I: Integer;
...
begin
  if not FIsRefreshing and not FIsSelectionChanging and
    (TdxNavBarAccess(NavBar).FDesignHelper <> nil) then
  begin
    FIsSelectionChanging := True;
    try
      AList := TList.Create;
...
procedure TdxNavBarCustomizationForm.SetItemText(AItem: TdxNavBarCustomItem;
  const AText: string);
begin
  if NavBar.IsDesigning then
    AItem.Name := AText
...
procedure TdxNavBarCustomizationForm.SetNodeImageIndex(ANode: TTreeNode);
var
  AItem: TObject;
begin
  AItem := TObject(ANode.Data);
  if AItem is TdxNavBarGroup then
    ANode.ImageIndex := 0
  else if AItem is TdxNavBarItemLink then
    ANode.ImageIndex := 3
  else if AItem is TdxNavBarItem then
...
begin
  ...
  dxTestCheck(cxTreeViewFindNodeByData(tvGroups.InnerTreeView, ANewGroup,
    ANode), 'InternalAddGroup: not find new group node');
  ANode.Selected := True;
  NavBar.DesignerModified;
end;

procedure TdxNavBarCustomizationForm.InternalAddItem
  (AItemClass: TdxNavBarItemClass);
var
...
procedure TdxNavBarCustomizationForm.TreeViewWndProcHandler
  (ATreeView: TTreeView; var Message: TMessage; var ADone: Boolean);
begin
...
begin
  ...
  Result := TdxNavBarAccess(NavBar).FDesignHelper.UniqueName(NavBar,
    NavBar.Name + AItemNamePrefix);
end;
...
begin
  ...
  FDragObject := TdxNavBarCustomizationDragControlObject.Create(Self,
    ATreeView);
  ...
  begin
    ...
    if ASelected is TdxNavBarItem then
      dxNavBarDragObject := TdxNavBarDragObject.Create(NavBar, DragObject, nil,
        nil, TdxNavBarItem(ASelected))
    else if ASelected is TdxNavBarGroup then
...
procedure TdxNavBarCustomizationForm.tvEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
...
procedure TdxNavBarCustomizationForm.tvDragOver(Sender, Source: TObject;
...
begin
  ...
  begin
    if Sender = tvItems then
      Accept := (dxNavBarDragObject.SourceGroup = nil) and
        (dxNavBarDragObject.SourceItem = nil)
    else
    begin
      UpdateTargets(X, Y);
  ...
  if (Sender = tvGroups) and (State = dsDragLeave) and
    (WindowFromPoint(GetMouseCursorPos) <> (Sender as TcxCustomTreeView)
    .InnerTreeView.Handle) then
    UpdateTargets(-1, -1);
end;
...
procedure TdxNavBarCustomizationForm.tvEdited(Sender: TObject; Node: TTreeNode;
  var S: String);
begin
...
begin
  acAddChildGroup.Enabled := (tvGroups.Selected <> nil) and
    (TObject(tvGroups.Selected.Data) is TdxNavBarGroup) and
    NavBar.OptionsBehavior.Common.AllowChildGroups;
  acDeleteGroupOrLink.Enabled := CanDeleteItems(tvGroups);
  acDeleteItem.Enabled := CanDeleteItems(tvItems);
  acRename.Enabled := tvGroups.Focused and (tvGroups.Selected <> nil) and
    (TObject(tvGroups.Selected.Data) is TdxNavBarGroup) or tvItems.Focused and
  ...
  acMoveGroupOrLinkUp.Enabled := tvGroups.Focused and
    (tvGroups.Selected <> nil);
  acMoveGroupOrLinkDown.Enabled := tvGroups.Focused and
    (tvGroups.Selected <> nil);
end;
...
procedure TdxNavBarCustomizationForm.acDeleteGroupOrLinkExecute
  (Sender: TObject);
begin
...
procedure TdxNavBarCustomizationForm.acAddChildGroupExecute(Sender: TObject);
var
    ...
    case Msg.CharCode of
      VK_DELETE:
        begin
          ...
          else if tvItems.Focused and not tvItems.IsEditing then
...
procedure TdxNavBarCustomizationDropInfoCalculator.
  CheckIfGroupFreeSpaceAreaTarget(const APoint: TPoint;
  var ADropTargetInfo: TdxNavBarDropTargetInfo);
begin
  // do nothing
end;
...
constructor TdxNavBarCustomizationDragControlObject.Create(ACustomizationForm
  : TdxNavBarCustomizationForm; AControl: TControl);
begin
