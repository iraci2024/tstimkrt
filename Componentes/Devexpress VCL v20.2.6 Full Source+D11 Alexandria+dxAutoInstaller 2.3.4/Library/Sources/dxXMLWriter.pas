...
type
  ...
  TdxXmlNewLineHandling = (Replace, Entitize, None);

  TdxXmlOutputMethod = (Xml, Html, Text, AutoDetect);

  TdxXmlStandalone = (Omit, Yes, No);

  TdxXmlNamespaceHandling = (Default, OmitDuplicates);

  TdxXmlWriteState = (Start, Prolog, Element, Attribute, Content,
    Closed, Error);
  ...
  public
    ...
    property OmitXmlDeclaration: Boolean read FOmitXmlDeclaration
      write FOmitXmlDeclaration;
    property NewLineHandling: TdxXmlNewLineHandling read FNewLineHandling
      write FNewLineHandling;
    ...
    property NewLineOnAttributes: Boolean read FNewLineOnAttributes
      write FNewLineOnAttributes;
    ...
    property NamespaceHandling: TdxXmlNamespaceHandling read FNamespaceHandling
      write FNamespaceHandling;
    property WriteEndDocumentOnClose: Boolean read FWriteEndDocumentOnClose
      write FWriteEndDocumentOnClose;
    property OutputMethod: TdxXmlOutputMethod read FOutputMethod
      write FOutputMethod;
    property MergeCDataSections: Boolean read FMergeCDataSections
      write FMergeCDataSections;
    property Standalone: TdxXmlStandalone read FStandalone write FStandalone;
    property AutoXmlDeclaration: Boolean read FAutoXmlDeclaration
      write FAutoXmlDeclaration;
    property EncodeInvalidXmlCharAsUCS2: Boolean
      read FEncodeInvalidXmlCharAsUCS2 write FEncodeInvalidXmlCharAsUCS2;
  end;
  ...
  protected
    ...
    procedure Write(const ABuffer: TCharArray; AIndex, ALength: Integer);
      overload; virtual;

    property AttrEndPos: Integer read FAttrEndPos write FAttrEndPos;
    property ContentPosition: Integer read FContentPosition
      write FContentPosition;
    property Stream: TStream read FStream;
  public
    class function Create(AStream: TStream;
      ASettings: TdxXmlWriterSettings = nil): TdxXmlWriter; overload; static;
    class function Create(ASb: TStringBuilder;
      ASettings: TdxXmlWriterSettings = nil): TdxXmlWriter; overload; static;
    ...
    procedure WriteStartElement(APrefix: string; const ALocalName: string;
      ANs: string); overload; virtual;
    ...
    procedure WriteAttributeBoolean(const ALocalName: string;
      AValue: Boolean); overload;
    procedure WriteAttributeFloat(const ALocalName: string;
      AValue: Single); overload;
    procedure WriteAttributeInteger(const ALocalName: string;
      AValue: Integer); overload;
    procedure WriteAttributeString(const ALocalName, ANs,
      AValue: string); overload;
    procedure WriteAttributeString(const ALocalName, AValue: string); overload;
    procedure WriteAttributeString(const APrefix, ALocalName, ANs,
      AValue: string); overload;
    procedure WriteAttributeString(const APrefix, ALocalName, ANs: string;
      const AValue: AnsiString); overload;
    procedure WriteAttributeString(const ALocalName: string;
      const AValue: AnsiString); overload;
    procedure WriteStartAttribute(const ALocalName, ANs: string); overload;
    procedure WriteStartAttribute(APrefix: string; ALocalName: string;
      ANs: string); overload; virtual; abstract;
    ...
    procedure WriteProcessingInstruction(const AName, AText: string);
      virtual; abstract;
    ...
    procedure WriteSurrogateCharEntity(ALowChar: Char; AHighChar: Char);
      virtual; abstract;
    procedure WriteChars(const ABuffer: TCharArray; AIndex: Integer;
      ACount: Integer); virtual; abstract;
    procedure WriteRaw(const ABuffer: TCharArray; AIndex: Integer;
      ACount: Integer); overload; virtual; abstract;
    procedure WriteRaw(const AData: string); overload; virtual; abstract;
    procedure WriteBase64(const ABuffer: TBytes; AIndex: Integer;
      ACount: Integer); virtual; abstract;
    procedure WriteBinHex(const ABuffer: TBytes; AIndex: Integer;
      ACount: Integer); virtual;
    procedure Close; virtual;
    procedure Flush; virtual;
    function LookupPrefix(const ANs: string): string; virtual; abstract;
    procedure WriteQualifiedName(const ALocalName, ANs: string);
      overload; virtual;
    ...
    procedure WriteElementString(const ALocalName, ANs,
      AValue: string); overload;
    procedure WriteElementString(const APrefix, ALocalName, ANs,
      AValue: string); overload;
    procedure WriteElementString(const ALocalName: string;
      const AValue: AnsiString); overload;
...
resourcestring
  ...
  SXmlIndentCharsNotWhitespace =
    'XmlWriterSettings.%s can contain only valid XML white space characters when '
  ...
  SXmlInvalidNameCharsDetail =
    'Invalid name character in "%s". The %d character, hexadecimal value %s, cannot be included in a name.';
  SXmlInvalidCharsInIndent =
    'WriterSettings.%s can contain only valid XML text content characters when XmlWriterSettings.CheckCharacters is true. %s';
  SXmlInvalidHighSurrogateChar =
    'Invalid high surrogate character (%s). A high surrogate character must have a value from range (0xD800 - 0xDBFF).';
  SXmlInvalidOperation =
    'Operation is not valid due to the current state of the object.';
  SXmlInvalidSurrogateMissingLowChar =
  ...
  SXmlRedefinePrefix =
    'The prefix "%s" cannot be redefined from "%s" to "%s" within the same start element tag.';
  SXmlUndefNamespace = 'The "%s" namespace is not defined.';
  SXmlWrongToken =
    'Token ord=%d in state ord=%d would result in an invalid XML document.';
  SXmlXmlnsPrefix = 'Prefix "xmlns" is reserved for use by XML.';
  SXmlXmlPrefix =
    'Prefix "xml" is reserved for use by XML and can be mapped only to namespace name "http://www.w3.org/XML/1998/namespace&quot"';
...
type
  ...
  strict private
    ...
    procedure WriteChars(const AChars: TCharArray; AIndex: Integer;
  ...
  strict protected
    ...
    procedure SetNamespaceResolver(const AValue
      : IdxXmlNamespaceResolver); virtual;
  ...
  public
    ...
    procedure WriteSurrogateCharEntity(ALowChar: Char;
      AHighChar: Char); override;
    ...
    procedure WriteXmlDeclaration(AStandalone: TdxXmlStandalone);
      overload; virtual;
    ...
    procedure WriteEndElement(const APrefix, ALocalName, ANs: string);
      overload; virtual;
    procedure WriteFullEndElement(const APrefix, ALocalName, ANs: string);
      overload; virtual;
    procedure WriteQualifiedName(const APrefix, ALocalName, ANs: string);
      overload; virtual;
    procedure WriteNamespaceDeclaration(const APrefix, ANs: string);
      virtual; abstract;
    ...
    property NamespaceResolver: IdxXmlNamespaceResolver
      read GetNamespaceResolver write SetNamespaceResolver;
    property SupportsNamespaceDeclarationInChunks: Boolean
      read GetSupportsNamespaceDeclarationInChunks;
  end;
  ...
  protected
    ...
    class function EncodeSurrogate(ASrc: PChar; ASrcEnd: PChar; ADst: PChar)
      : PChar; static;
    ...
    procedure ValidateContentChars(const AChars: string;
      const APropertyName: string; AAllowOnlyWhitespace: Boolean);
  public
    constructor CreateEx(AStream: TStream;
      ASettings: TdxXmlWriterSettings); overload;
    destructor Destroy; override;
    ...
    procedure WriteFullEndElement(const APrefix, ALocalName,
      ANs: string); override;
    procedure WriteStartAttribute(APrefix: string; ALocalName: string;
      ANs: string); override;
    procedure WriteEndAttribute; override;
    procedure WriteNamespaceDeclaration(const APrefix, ANamespaceName
      : string); override;
    ...
    procedure WriteChars(const ABuffer: TCharArray; AIndex: Integer;
      ACount: Integer); override;
    procedure WriteRaw(const ABuffer: TCharArray; AIndex: Integer;
      ACount: Integer); overload; override;
  ...
  protected
    ...
    procedure Write(const ABuffer: TCharArray; AIndex, ALength: Integer);
      overload; override;
  public
    constructor CreateEx(ASb: TStringBuilder;
      ASettings: TdxXmlWriterSettings); overload;
  end;
  ...
  type
    ...
    TToken = (StartDocument, EndDocument, PI, Comment, Dtd, StartElement,
      EndElement, StartAttribute, EndAttribute, Text, CData, AtomicValue,
      Base64, RawData, Whitespace);

    TSpecialAttribute = (No, DefaultXmlns, PrefixedXmlns, XmlSpace, XmlLang);

    TNamespaceKind = (Written, NeedToWrite, Implied, Special);
    ...
    public
      procedure &Set(const APrefix, ANamespaceUri: string;
    ...
    public
      procedure &Set(const APrefix, ALocalName, ANamespaceUri: string);
      function IsDuplicate(const APrefix, ALocalName, ANamespaceUri
        : string): Boolean;
    ...
    strict private
    type
      TItemType = (EntityRef, CharEntity, SurrogateCharEntity, Whitespace,
        &String, StringChars, Raw, RawChars, ValueString);
      ...
      TBufferChunk = record
        ...
        Index: Integer;
        Count: Integer;
        constructor Create(const ABuffer: TCharArray; AIndex: Integer;
          ACount: Integer);
      end;
    ...
    strict private
      ...
      function GetNamespacesInScope(AScope: TdxXmlNamespaceScope)
  ...
  protected
    ...
    function GetXmlSpace: TdxXmlSpace; override;
    function GetXmlLang: string; override;
    procedure ThrowInvalidStateTransition(AToken: TToken;
      ACurrentState: TState);
  public
    constructor Create(AWriter: TdxXmlRawWriter;
      ASettings: TdxXmlWriterSettings);
    destructor Destroy; override;
    procedure WriteStartDocument; override;
    procedure WriteStartDocument(AStandalone: Boolean); override;
    procedure WriteEndDocument; override;
    ...
    procedure WriteStartAttribute(APrefix: string; ALocalName: string;
      ANamespaceName: string); override;
    procedure WriteEndAttribute; override;
    procedure WriteCData(const AText: string); override;
    procedure WriteComment(const AText: string); override;
    procedure WriteProcessingInstruction(const AName, AText: string); override;
    ...
    class function DupAttrException(const APrefix, ALocalName: string)
      : EdxXmlException; static;
    ...
    class function InvalidCharsException(const AName: string;
      ABadCharIndex: Integer): EdxXmlException; static;
...
class function TdxXmlWriter.Create(AStream: TStream;
  ASettings: TdxXmlWriterSettings = nil): TdxXmlWriter;
begin
  if ASettings = nil then
    ASettings := TdxXmlWriterSettings.Create;
  Result := TdxXmlWellFormedWriter.Create
    (TdxXmlEncodedRawTextWriter.CreateEx(AStream, ASettings), ASettings);
end;

class function TdxXmlWriter.Create(ASb: TStringBuilder;
  ASettings: TdxXmlWriterSettings = nil): TdxXmlWriter;
begin
  if ASettings = nil then
    ASettings := TdxXmlWriterSettings.Create;
  Result := TdxXmlWellFormedWriter.Create(TdxXmlStringWriter.CreateEx(ASb,
    ASettings), ASettings);
end;
...
procedure TdxXmlWriter.WriteAttributeBoolean(const ALocalName: string;
  AValue: Boolean);
begin
  WriteAttributeString(ALocalName, TdxXMLHelper.EncodeBoolean(AValue));
end;

procedure TdxXmlWriter.WriteAttributeFloat(const ALocalName: string;
  AValue: Single);
begin
  WriteAttributeString(ALocalName, dxFloatToStr(AValue));
end;

procedure TdxXmlWriter.WriteAttributeInteger(const ALocalName: string;
  AValue: Integer);
begin
  WriteAttributeString(ALocalName, IntToStr(AValue));
end;

procedure TdxXmlWriter.WriteAttributeString(const ALocalName, ANs,
  AValue: string);
begin
...
procedure TdxXmlWriter.WriteAttributeString(const APrefix, ALocalName, ANs,
  AValue: string);
begin
...
procedure TdxXmlWriter.WriteAttributeString(const APrefix, ALocalName,
  ANs: string; const AValue: AnsiString);
begin
  WriteStartAttribute(APrefix, ALocalName, ANs);
  WriteString(AValue);
  WriteEndAttribute;
...
procedure TdxXmlWriter.WriteAttributeString(const ALocalName: string;
...
procedure TdxXmlWriter.WriteStartElement(APrefix: string;
  const ALocalName: string; ANs: string);
begin
  if APrefix <> '' then
...
procedure TdxXmlWriter.WriteBinHex(const ABuffer: TBytes; AIndex: Integer;
  ACount: Integer);
const
  CharsChunkSize = 128;
var
...
procedure TdxXmlWriter.WriteElementString(const ALocalName, ANs,
  AValue: string);
begin
  WriteStartElement(ALocalName, ANs);
  if AValue <> '' then
...
procedure TdxXmlWriter.WriteElementString(const APrefix, ALocalName, ANs,
  AValue: string);
begin
...
procedure TdxXmlWriter.WriteElementString(const ALocalName: string;
  const AValue: AnsiString);
begin
  WriteStartElement(ALocalName);
  if AValue <> '' then
...
procedure TdxXmlWriter.Write(const ABuffer: TCharArray;
...
procedure TdxXmlRawWriterBase64Encoder.WriteChars(const AChars: TCharArray;
...
procedure TdxXmlRawWriter.WriteEndElement(const APrefix, ALocalName,
  ANs: string);
begin
...
procedure TdxXmlRawWriter.WriteBase64(const ABuffer: TBytes; AIndex: Integer;
  ACount: Integer);
begin
...
procedure TdxXmlRawWriter.WriteSurrogateCharEntity(ALowChar: Char;
  AHighChar: Char);
begin
...
procedure TdxXmlRawWriter.WriteChars(const ABuffer: TCharArray; AIndex: Integer;
  ACount: Integer);
begin
  WriteString(TdxStringHelper.Create(ABuffer, AIndex, ACount));
end;

procedure TdxXmlRawWriter.WriteRaw(const ABuffer: TCharArray; AIndex: Integer;
  ACount: Integer);
begin
  WriteString(TdxStringHelper.Create(ABuffer, AIndex, ACount));
end;
...
procedure TdxXmlRawWriter.SetNamespaceResolver(const AValue
  : IdxXmlNamespaceResolver);
begin
  FResolver := AValue;
end;
...
procedure TdxXmlRawWriter.OnRootElement(AConformanceLevel
  : TdxXmlConformanceLevel);
begin
end;

procedure TdxXmlRawWriter.WriteFullEndElement(const APrefix, ALocalName,
  ANs: string);
begin
  WriteEndElement(APrefix, ALocalName, ANs);
end;

procedure TdxXmlRawWriter.WriteQualifiedName(const APrefix, ALocalName,
  ANs: string);
begin
  if APrefix <> '' then
  begin
...
constructor TdxXmlEncodedRawTextWriter.CreateEx(AStream: TStream;
  ASettings: TdxXmlWriterSettings);
begin
  ...
  if FCheckCharacters and (FNewLineHandling = TdxXmlNewLineHandling.Replace)
  then
...
procedure TdxXmlEncodedRawTextWriter.WriteXmlDeclaration
  (AStandalone: TdxXmlStandalone);
begin
...
procedure TdxXmlEncodedRawTextWriter.WriteXmlDeclaration(const AXmldecl
  : string);
begin
  if not FOmitXmlDeclaration and not FAutoXmlDeclaration then
...
procedure TdxXmlEncodedRawTextWriter.WriteStartElement(APrefix: string;
  const ALocalName: string; ANs: string);
begin
  Assert(ALocalName <> '');
...
procedure TdxXmlEncodedRawTextWriter.WriteEndElement(const APrefix, ALocalName,
  ANs: string);
begin
  Assert(ALocalName <> '');
...
procedure TdxXmlEncodedRawTextWriter.WriteFullEndElement(const APrefix,
  ALocalName, ANs: string);
begin
  Assert(ALocalName <> '');
...
procedure TdxXmlEncodedRawTextWriter.WriteStartAttribute(APrefix: string;
  ALocalName: string; ANs: string);
begin
  Assert(ALocalName <> '');
...
procedure TdxXmlEncodedRawTextWriter.WriteNamespaceDeclaration(const APrefix,
...
function TdxXmlEncodedRawTextWriter.
  GetSupportsNamespaceDeclarationInChunks: Boolean;
begin
  Result := True;
end;

procedure TdxXmlEncodedRawTextWriter.WriteStartNamespaceDeclaration
...
procedure TdxXmlEncodedRawTextWriter.WriteProcessingInstruction(const AName,
  AText: string);
begin
  Assert(AName <> '');
...
begin
  ...
  if FCheckCharacters and not TdxXmlCharType.IsCharData(C) then
    raise EdxXmlArgumentException.CreateFmt(SXmlInvalidCharacter,
      [C, CharToHex(C)]);
...
procedure TdxXmlEncodedRawTextWriter.WriteSurrogateCharEntity(ALowChar: Char;
  AHighChar: Char);
var
...
procedure TdxXmlEncodedRawTextWriter.WriteChars(const ABuffer: TCharArray;
  AIndex: Integer; ACount: Integer);
var
  AStart: PChar;
begin
...
procedure TdxXmlEncodedRawTextWriter.WriteRaw(const ABuffer: TCharArray;
  AIndex: Integer; ACount: Integer);
var
  AStart: PChar;
begin
...
procedure TdxXmlEncodedRawTextWriter.WriteAttributeTextBlock(ASrc: PChar;
  ASrcEnd: PChar);
var
...
begin
  ...
  begin
    ...
    begin
      C := ASrc^;
      if (TdxXmlCharType.charProperties[C] and TdxXmlCharType.AttrValue) = 0
      then
        Break;
      ADst^ := C;
      Inc(ADst);
...
procedure TdxXmlEncodedRawTextWriter.WriteElementTextBlock(ASrc: PChar;
  ASrcEnd: PChar);
var
...
procedure TdxXmlEncodedRawTextWriter.WriteRawWithCharChecking(APSrcBegin: PChar;
  APSrcEnd: PChar);
var
...
begin
  ...
  begin
    ...
    begin
      C := ASrc^;
      if not((TdxXmlCharType.charProperties[C] and TdxXmlCharType.Text) <> 0)
      then
        Break;
      ADst^ := C;
      Inc(ADst);
...
procedure TdxXmlEncodedRawTextWriter.WriteCommentOrPi(const AText: string;
  AStopChar: Char);
var
...
begin
  ...
  begin
    ...
    begin
      C := ASrc^;
      if not(((TdxXmlCharType.charProperties[C] and TdxXmlCharType.AttrValue) <>
        0) and (C <> ']')) then
        Break;
      ADst^ := C;
      Inc(ADst);
...
class function TdxXmlEncodedRawTextWriter.EncodeSurrogate(ASrc: PChar;
  ASrcEnd: PChar; ADst: PChar): PChar;
var
  ACh, ALowChar: Char;
begin
  ACh := ASrc^;
  ...
  raise EdxXmlArgumentException.CreateFmt(SXmlInvalidHighSurrogateChar,
    [CharToHex(ACh)]);
end;

function TdxXmlEncodedRawTextWriter.InvalidXmlChar(ACh: Char; ADst: PChar;
  AEntitize: Boolean): PChar;
begin
...
procedure TdxXmlEncodedRawTextWriter.EncodeChar(var ASrc: PChar;
  APSrcEnd: PChar; var ADst: PChar);
var
  C: Char;
begin
  C := ASrc^;
...
class function TdxXmlEncodedRawTextWriter.CarriageReturnEntity
  (ADst: PChar): PChar;
begin
  ADst[0] := '&';
  ADst[1] := '#';
...
class function TdxXmlEncodedRawTextWriter.CharEntity(ADst: PChar;
  ACh: Char): PChar;
begin
  ADst[0] := '&';
  ADst[1] := '#';
...
class function TdxXmlEncodedRawTextWriter.UCS2Entity(ADst: PChar;
  ACh: Char): PChar;
begin
  ADst[0] := '_';
  ADst[1] := 'x';
...
procedure TdxXmlEncodedRawTextWriter.ValidateContentChars(const AChars: string;
  const APropertyName: string; AAllowOnlyWhitespace: Boolean);
var
  I: Integer;
begin
  ...
  begin
    if not TdxXmlCharType.IsOnlyWhitespace(AChars) then
      raise EdxXmlArgumentException.CreateFmt(SXmlIndentCharsNotWhitespace,
        [APropertyName]);
  end
  else
  begin
      ...
      begin
        case AChars[I] of
          #13, #10, #9: { do nothing }
            ;
          '<', '&', ']':
            raise EdxXmlArgumentException.CreateFmt(SXmlInvalidCharsInIndent,
              [APropertyName, Format(SXmlInvalidCharacter,
              [AChars[I], CharToHex(AChars[I])])]);
        else
          begin
            ...
            begin
              ...
              raise EdxXmlArgumentException.CreateFmt(SXmlInvalidCharsInIndent,
                [APropertyName, SXmlInvalidSurrogateMissingLowChar]);
            end
            else if TdxXmlCharType.IsLowSurrogate(AChars[I]) then
              raise EdxXmlArgumentException.CreateFmt(SXmlInvalidCharsInIndent,
                [APropertyName, Format(SXmlInvalidHighSurrogateChar,
                [CharToHex(AChars[I])])]);
          end;
        end;
      end;
...
constructor TdxXmlStringWriter.CreateEx(ASb: TStringBuilder;
  ASettings: TdxXmlWriterSettings);
begin
  FSb := ASb;
...
procedure TdxXmlStringWriter.Write(const ABuffer: TCharArray;
  AIndex, ALength: Integer);
begin
...
procedure TdxXmlWellFormedWriter.TElementScope.&Set(const APrefix, ALocalName,
  ANamespaceUri: string; APrevNSTop: Integer);
begin
...
procedure TdxXmlWellFormedWriter.TElementScope.WriteEndElement
  (ARawWriter: TdxXmlRawWriter);
begin
  ARawWriter.WriteEndElement(FPrefix, FLocalName, FNamespaceUri);
end;

procedure TdxXmlWellFormedWriter.TElementScope.WriteFullEndElement
  (ARawWriter: TdxXmlRawWriter);
begin
...
procedure TdxXmlWellFormedWriter.TNamespace.&Set(const APrefix,
  ANamespaceUri: string; AKind: TNamespaceKind);
begin
  FPrefix := APrefix;
  FNamespaceUri := ANamespaceUri;
  FKind := AKind;
...
procedure TdxXmlWellFormedWriter.TNamespace.WriteDecl(AWriter: TdxXmlWriter;
  ARawWriter: TdxXmlRawWriter);
begin
...
procedure TdxXmlWellFormedWriter.TAttrName.&Set(const APrefix, ALocalName,
...
function TdxXmlWellFormedWriter.TAttrName.IsDuplicate(const APrefix, ALocalName,
  ANamespaceUri: string): Boolean;
begin
  Result := (FLocalName = ALocalName) and
    ((FPrefix = APrefix) or (FNamespaceUri = ANamespaceUri));
end;

{ TdxXmlWellFormedWriter.TAttributeValueCache.TItem }

procedure TdxXmlWellFormedWriter.TAttributeValueCache.TItem.&Set
  (AType: TItemType; const AData: TValue);
begin
  FType := AType;
  FData := AData;
end;
...
constructor TdxXmlWellFormedWriter.TAttributeValueCache.TBufferChunk.Create
  (const ABuffer: TCharArray; AIndex: Integer; ACount: Integer);
begin
  Buffer := ABuffer;
  Index := AIndex;
  Count := ACount;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteEntityRef
  (const AName: string);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteCharEntity
  (ACh: Char);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteSurrogateCharEntity
  (ALowChar: Char; AHighChar: Char);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteWhitespace
  (const AWs: string);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteString
  (const AText: string);
begin
  if FSingleStringValue <> '' then
    StartComplexValue
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteChars
  (const ABuffer: TCharArray; AIndex: Integer; ACount: Integer);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
  FStringValue.Append(ABuffer, AIndex, ACount);
  AddItem(TItemType.StringChars,
    TValue.From<TBufferChunk>(TBufferChunk.Create(ABuffer, AIndex, ACount)));
end;

procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteRaw
  (const ABuffer: TCharArray; AIndex: Integer; ACount: Integer);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
  FStringValue.Append(ABuffer, AIndex, ACount);
  AddItem(TItemType.RawChars,
    TValue.From<TBufferChunk>(TBufferChunk.Create(ABuffer, AIndex, ACount)));
end;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.WriteValue
  (const AValue: string);
begin
  if FSingleStringValue <> '' then
    StartComplexValue;
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.Replay
    ...
    case AItem.&Type of
      ...
      TItemType.StringChars:
        begin
          ...
          AWriter.WriteChars(ABufChunk.Buffer, ABufChunk.Index,
            ABufChunk.Count);
        end;
      TItemType.Raw:
...
begin
  ...
  begin
    ...
    case AItem.&Type of
      TItemType.Whitespace:
        Inc(FFirstItem);
      TItemType.String, TItemType.Raw, TItemType.ValueString:
        begin
          AItem.Data := SysUtils.TrimLeft(AItem.Data.AsString);
          if AItem.Data.AsString = '' then
            Inc(FFirstItem);
        end;
      TItemType.StringChars, TItemType.RawChars:
        begin
          ABufChunk := AItem.Data.AsType<TBufferChunk>;
          AEndIndex := ABufChunk.Index + ABufChunk.Count;
          while (ABufChunk.Index < AEndIndex) and
            TdxXmlCharType.IsWhiteSpace(ABufChunk.Buffer[ABufChunk.Index]) do
          begin
            Inc(ABufChunk.Index);
            Dec(ABufChunk.Count);
          end;
  ...
  begin
    ...
    case AItem.&Type of
      ...
      TItemType.StringChars, TItemType.RawChars:
        begin
          ABufChunk := AItem.Data.AsType<TBufferChunk>;
          while (ABufChunk.Count > 0) and TdxXmlCharType.IsWhiteSpace
            (ABufChunk.Buffer[ABufChunk.Index + ABufChunk.Count - 1]) do
            Dec(ABufChunk.Count);
          AItem.Data := TValue.From<TBufferChunk>(ABufChunk);
...
procedure TdxXmlWellFormedWriter.TAttributeValueCache.AddItem(AType: TItemType;
  AData: TValue);
var
...
begin
  ...
  if FItems = nil then
    SetLength(FItems, 4)
  else if Length(FItems) = ANewItemIndex then
...
constructor TdxXmlWellFormedWriter.TNamespaceResolverProxy.Create
  (AWfWriter: TdxXmlWellFormedWriter);
begin
  FWfWriter := AWfWriter;
end;

function TdxXmlWellFormedWriter.TNamespaceResolverProxy.GetNamespacesInScope
  (AScope: TdxXmlNamespaceScope): TdxStringsDictionary;
begin
  raise EdxXmlException.Create(SXmlNotImplemented);
end;

function TdxXmlWellFormedWriter.TNamespaceResolverProxy.LookupNamespace
  (const APrefix: string): string;
begin
  Result := FWfWriter.LookupNamespace(APrefix);
end;

function TdxXmlWellFormedWriter.TNamespaceResolverProxy.LookupPrefix
  (const ANamespaceName: string): string;
begin
...
constructor TdxXmlWellFormedWriter.Create(AWriter: TdxXmlRawWriter;
  ASettings: TdxXmlWriterSettings);
var
  ADefaultNs: string;
begin
  ...
  FCheckCharacters := ASettings.CheckCharacters;
  FOmitDuplNamespaces := ASettings.NamespaceHandling = TdxXmlNamespaceHandling.
    OmitDuplicates;
...
begin
  FStateToWriteState := TArray<TdxXmlWriteState>.Create(TdxXmlWriteState.Start,
    TdxXmlWriteState.Prolog, TdxXmlWriteState.Prolog, TdxXmlWriteState.Element,
    TdxXmlWriteState.Content, TdxXmlWriteState.Content,
    TdxXmlWriteState.Attribute, TdxXmlWriteState.Content,
    TdxXmlWriteState.Attribute, TdxXmlWriteState.Attribute,
    TdxXmlWriteState.Content, TdxXmlWriteState.Attribute,
    TdxXmlWriteState.Attribute, TdxXmlWriteState.Attribute,
    TdxXmlWriteState.Attribute, TdxXmlWriteState.Closed,
    TdxXmlWriteState.Error);
  FStateTableDocument := TArray<TState>.Create(
    { Token.StartDocument } TStates.Document, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.PostB64Cont, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.EndDocument } TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.PostB64Cont, TStates.Error,
    TStates.EndDocument, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.PI } TStates.StartDoc, TStates.TopLevel, TStates.Document,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.EndAttrSCont,
    TStates.EndAttrSCont, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error,
    { Token.Comment } TStates.StartDoc, TStates.TopLevel, TStates.Document,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.EndAttrSCont,
    TStates.EndAttrSCont, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error,
    { Token.Dtd } TStates.StartDoc, TStates.TopLevel, TStates.Document,
    TStates.Error, TStates.Error, TStates.PostB64Cont, TStates.PostB64Attr,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.StartElement } TStates.StartDocEle, TStates.Element,
    TStates.Element, TStates.StartContentEle, TStates.Element,
    TStates.PostB64Cont, TStates.PostB64Attr, TStates.Error,
    TStates.EndAttrSEle, TStates.EndAttrSEle, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.EndElement } TStates.Error, TStates.Error, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Error, TStates.EndAttrEEle,
    TStates.EndAttrEEle, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error,
    { Token.StartAttribute } TStates.Error, TStates.Error, TStates.Error,
    TStates.Attribute, TStates.Error, TStates.PostB64Cont, TStates.PostB64Attr,
    TStates.Error, TStates.EndAttrSAttr, TStates.EndAttrSAttr, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.EndAttribute } TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.PostB64Cont, TStates.PostB64Attr,
    TStates.Error, TStates.Element, TStates.Element, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.Text } TStates.Error, TStates.Error, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Error, TStates.Attribute, TStates.SpecialAttr,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error,
    { Token.CData } TStates.Error, TStates.Error, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Error, TStates.EndAttrSCont,
    TStates.EndAttrSCont, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error,
    { Token.AtomicValue } TStates.Error, TStates.Error, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Error, TStates.Attribute, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error,
    { Token.Base64 } TStates.Error, TStates.Error, TStates.Error,
    TStates.StartContentB64, TStates.B64Content, TStates.B64Content,
    TStates.B64Attribute, TStates.Error, TStates.B64Attribute, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error,
    { Token.RawData } TStates.StartDoc, TStates.Error, TStates.Document,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.Attribute,
    TStates.SpecialAttr, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error,
    { Token.Whitespace } TStates.StartDoc, TStates.TopLevel, TStates.Document,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.Attribute,
    TStates.SpecialAttr, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error);
  FStateTableAuto := TArray<TState>.Create(
    ...
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    { Token.EndDocument }
    { Token.PI } TStates.TopLevel, TStates.TopLevel, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.EndAttrSCont,
    TStates.EndAttrSCont, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, { Token.PI }
    { Token.Comment } TStates.TopLevel, TStates.TopLevel, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.EndAttrSCont,
    TStates.EndAttrSCont, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, { Token.Comment }
    { Token.Dtd } TStates.StartDoc, TStates.TopLevel, TStates.Error,
    TStates.Error, TStates.Error, TStates.PostB64Cont, TStates.PostB64Attr,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, TStates.Error, { Token.Dtd }
    { Token.StartElement } TStates.StartFragEle, TStates.Element, TStates.Error,
    TStates.StartContentEle, TStates.Element, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Element, TStates.EndAttrSEle,
    TStates.EndAttrSEle, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, { Token.StartElement }
    { Token.EndElement } TStates.Error, TStates.Error, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Error, TStates.EndAttrEEle,
    TStates.EndAttrEEle, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, { Token.EndElement }
    { Token.StartAttribute } TStates.RootLevelAttr, TStates.Error,
    TStates.Error, TStates.Attribute, TStates.Error, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Error, TStates.EndAttrSAttr,
    TStates.EndAttrSAttr, TStates.Error, TStates.StartRootLevelAttr,
    TStates.StartRootLevelAttr, TStates.PostB64RootAttr, TStates.RootLevelAttr,
    TStates.Error, { Token.StartAttribute }
    { Token.EndAttribute } TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.PostB64Cont, TStates.PostB64Attr,
    TStates.Error, TStates.Element, TStates.Element, TStates.Error,
    TStates.AfterRootLevelAttr, TStates.AfterRootLevelAttr,
    TStates.PostB64RootAttr, TStates.Error, TStates.Error,
    { Token.EndAttribute }
    { Token.Text } TStates.StartFragCont, TStates.StartFragCont, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Content, TStates.Attribute,
    TStates.SpecialAttr, TStates.Error, TStates.RootLevelAttr,
    TStates.RootLevelSpecAttr, TStates.PostB64RootAttr, TStates.Error,
    TStates.Error, { Token.Text }
    { Token.CData } TStates.StartFragCont, TStates.StartFragCont, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Content, TStates.EndAttrSCont,
    TStates.EndAttrSCont, TStates.Error, TStates.Error, TStates.Error,
    TStates.Error, TStates.Error, TStates.Error, { Token.CData }
    { Token.AtomicValue } TStates.StartFragCont, TStates.StartFragCont,
    TStates.Error, TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Content, TStates.Attribute, TStates.Error,
    TStates.Error, TStates.RootLevelAttr, TStates.Error,
    TStates.PostB64RootAttr, TStates.Error, TStates.Error, { Token.AtomicValue }
    { Token.Base64 } TStates.StartFragB64, TStates.StartFragB64, TStates.Error,
    TStates.StartContentB64, TStates.B64Content, TStates.B64Content,
    TStates.B64Attribute, TStates.B64Content, TStates.B64Attribute,
    TStates.Error, TStates.Error, TStates.RootLevelB64Attr, TStates.Error,
    TStates.RootLevelB64Attr, TStates.Error, TStates.Error, { Token.Base64 }
    { Token.RawData } TStates.StartFragCont, TStates.TopLevel, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.Content, TStates.Attribute,
    TStates.SpecialAttr, TStates.Error, TStates.RootLevelAttr,
    TStates.RootLevelSpecAttr, TStates.PostB64RootAttr,
    TStates.AfterRootLevelAttr, TStates.Error, { Token.RawData }
    { Token.Whitespace } TStates.TopLevel, TStates.TopLevel, TStates.Error,
    TStates.StartContent, TStates.Content, TStates.PostB64Cont,
    TStates.PostB64Attr, TStates.AfterRootEle, TStates.Attribute,
    TStates.SpecialAttr, TStates.Error, TStates.RootLevelAttr,
    TStates.RootLevelSpecAttr, TStates.PostB64RootAttr,
    TStates.AfterRootLevelAttr, TStates.Error { Token.Whitespace }
    );
end;
...
procedure TdxXmlWellFormedWriter.WriteStartElement(APrefix: string;
  const ALocalName: string; ANs: string);
var
  ATop: Integer;
begin
...
procedure TdxXmlWellFormedWriter.WriteStartAttribute(APrefix: string;
  ALocalName: string; ANamespaceName: string);
label
  SkipPushAndWrite;
var
  ADefinedNs: string;
begin
  try
    ...
    begin
      ...
      begin
        if not((ALocalName = 'xmlns') and
          (ANamespaceName = TdxXmlReservedNs.NsXmlNs)) then
    ...
    begin
      ...
      begin
        ...
        begin
          if (ANamespaceName <> '') and
            (ANamespaceName <> TdxXmlReservedNs.NsXml) then
      ...
      case FSpecAttr of
        ...
        TSpecialAttribute.PrefixedXmlns:
          begin
            ...
            if (AValue = TdxXmlReservedNs.NsXmlNs) or
              ((AValue = TdxXmlReservedNs.NsXml) and (FCurDeclPrefix <> 'xml'))
            then
              raise EdxXmlArgumentException.Create
                (SXmlCanNotBindToReservedNamespace);
        ...
        TSpecialAttribute.XmlSpace:
          begin
            ...
            else if AValue = 'preserve' then
              FElemScopeStack[FElemTop].XmlSpace := TdxXmlSpace.Preserve
            else
              raise EdxXmlArgumentException.CreateFmt(SXmlInvalidXmlSpace,
                [AValue]);
...
procedure TdxXmlWellFormedWriter.WriteProcessingInstruction(const AName,
  AText: string);
begin
  try
    if AName = '' then
...
procedure TdxXmlWellFormedWriter.WriteSurrogateCharEntity(ALowChar: Char;
  AHighChar: Char);
begin
  try
{$IFNDEF DELPHIXE4}
...
procedure TdxXmlWellFormedWriter.WriteChars(const ABuffer: TCharArray;
  AIndex: Integer; ACount: Integer);
begin
  try
    if ABuffer = nil then
...
procedure TdxXmlWellFormedWriter.WriteRaw(const ABuffer: TCharArray;
  AIndex: Integer; ACount: Integer);
begin
  try
    if ABuffer = nil then
...
procedure TdxXmlWellFormedWriter.WriteBase64(const ABuffer: TBytes;
  AIndex: Integer; ACount: Integer);
begin
  try
    if ABuffer = nil then
...
procedure TdxXmlWellFormedWriter.ThrowInvalidStateTransition(AToken: TToken;
  ACurrentState: TState);
var
  AWrongTokenMessage: string;
begin
  AWrongTokenMessage := Format(SXmlWrongToken,
    [Ord(AToken), Ord(ACurrentState)]);
...
procedure TdxXmlWellFormedWriter.WriteQualifiedName(const ALocalName,
  ANs: string);
var
  APrefix: string;
begin
...
procedure TdxXmlWellFormedWriter.WriteBinHex(const ABuffer: TBytes;
  AIndex: Integer; ACount: Integer);
begin
...
begin
  Result := FCurrentState in [TStates.B64Content, TStates.B64Attribute,
    TStates.RootLevelB64Attr];
end;

procedure TdxXmlWellFormedWriter.SetSpecialAttribute
  (ASpecial: TSpecialAttribute);
begin
  FSpecAttr := ASpecial;
  if TStates.Attribute = FCurrentState then
    FCurrentState := TStates.SpecialAttr
  else if TStates.RootLevelAttr = FCurrentState then
...
procedure TdxXmlWellFormedWriter.WriteStartDocumentImpl
  (AStandalone: TdxXmlStandalone);
begin
  try
    ...
    else if FConformanceLevel = TdxXmlConformanceLevel.Fragment then
      raise EdxXmlInvalidOperationException.Create
        (SXmlCannotStartDocumentOnFragment);
...
procedure TdxXmlWellFormedWriter.PushNamespaceImplicit(const APrefix,
  ANs: string);
var
  AKind: TNamespaceKind;
...
begin
  ...
  begin
    ...
    begin
      if FNsStack[AExistingNsIndex].NamespaceUri <> ANs then
        raise EdxXmlException.CreateFmt(SXmlRedefinePrefix,
          [APrefix, FNsStack[AExistingNsIndex].NamespaceUri, ANs]);
      Exit;
    end
    else
    begin
  ...
  begin
    if ((ANs = TdxXmlReservedNs.NsXml) and (APrefix <> 'xml')) or
      ((ANs = TdxXmlReservedNs.NsXmlNs) and (APrefix <> 'xmlns')) then
      raise EdxXmlArgumentException.CreateFmt(SXmlNamespaceDeclXmlXmlns,
        [APrefix]);
    if FPredefinedNamespaces <> nil then
    begin
...
function TdxXmlWellFormedWriter.PushNamespaceExplicit(const APrefix,
  ANs: string): Boolean;
var
  AWriteItOut: Boolean;
  AExistingNsIndex: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FOmitDuplNamespaces and (AExistingNsKind <> TNamespaceKind.NeedToWrite)
      then
        AWriteItOut := False;
    ...
    begin
      if FOmitDuplNamespaces and (FNsStack[AExistingNsIndex].NamespaceUri = ANs)
      then
        AWriteItOut := False;
    end;
  end
...
procedure TdxXmlWellFormedWriter.AddNamespace(const APrefix, ANs: string;
  AKind: TNamespaceKind);
var
  ATop, I: Integer;
begin
  ...
  else if FNsTop = MaxNamespacesWalkCount then
  begin
    FNsHashTable.Free;
...
procedure TdxXmlWellFormedWriter.AddToNamespaceHashtable(ANamespaceIndex
  : Integer);
var
  APrefix: string;
  AExistingNsIndex: Integer;
...
function TdxXmlWellFormedWriter.LookupNamespaceIndex(const APrefix
  : string): Integer;
var
  AIndex, I: Integer;
begin
...
procedure TdxXmlWellFormedWriter.PopNamespaces(AIndexFrom: Integer;
  AIndexTo: Integer);
var
  I: Integer;
begin
...
class function TdxXmlWellFormedWriter.DupAttrException(const APrefix,
  ALocalName: string): EdxXmlException;
var
  ASb: TStringBuilder;
begin
...
begin
  ...
  begin
    if (FCurrentState = TStates.Closed) or (FCurrentState = TStates.Error) then
      ...
      raise EdxXmlInvalidOperationException.CreateFmt(SXmlWrongToken,
        [Ord(AToken), Ord(FCurrentState)]);
  end;
Advance:
...
function TdxXmlWellFormedWriter.LookupLocalNamespace(const APrefix
  : string): string;
var
  I: Integer;
begin
...
begin
  ...
  if (TdxXmlCharType.charProperties[AStart^] and TdxXmlCharType.NCStartNameSC) = 0
  then
...
class function TdxXmlWellFormedWriter.InvalidCharsException(const AName: string;
  ABadCharIndex: Integer): EdxXmlException;
begin
  Result := EdxXmlException.CreateFmt(SXmlInvalidNameCharsDetail,
    [AName, ABadCharIndex, CharToHex(AName[ABadCharIndex + 1])]);
end;
...
procedure TdxXmlWellFormedWriter.AddAttribute(const APrefix, ALocalName,
  ANamespaceName: string);
var
