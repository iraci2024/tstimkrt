...
uses
  Types, SysUtils, Classes, Controls, Graphics, Generics.Defaults,
  Generics.Collections, dxCoreGraphics, dxGDIPlusClasses,
...
type
  ...
  TdxVCLResizeTableRowMouseHandlerStateStrategy = class
  ...
  TdxVCLResizeTableVirtualColumnMouseHandlerStateStrategy = class
  ...
  public
    ...
    procedure DrawFeedbackShape(AGraphics: TdxGraphics;
      const AShapeBounds: TRect; const AContentBounds: TRect;
      AFillColor: TdxAlphaColor; AOutlineColor: TdxAlphaColor;
    ...
    property ShadowDisplayMode: TdxResizingShadowDisplayMode
      read FShadowDisplayMode write FShadowDisplayMode;
  ...
  TdxWinFormsDragFloatingObjectManuallyMouseHandlerStateStrategy = class
  ...
  protected
    procedure ShowVisualFeedbackCore(const ABounds: TRect;
      APageViewInfo: TdxPageViewInfo; AImage: TdxOfficeImage); override;
    ...
    function CreateFeedbackImage(AOriginalImage: TdxOfficeImage)
      : TdxOfficeImage; override;
    function CreateSemitransparentFeedbackImage(ABitmap: TdxSmartImage;
      AAlpha: Single): TdxOfficeImage; virtual;
    procedure BeginVisualFeedback; override;
    procedure EndVisualFeedback; override;
  public
  ...
  TdxVCLRectangularObjectResizeMouseHandlerStateStrategy = class
...
function TdxVCLResizeTableRowMouseHandlerStateStrategy.GetControl
...
procedure TdxVCLResizeTableVirtualColumnMouseHandlerStateStrategy.
  BeginVisualFeedback;
begin
  State.DrawReversibleLineCore;
end;
...
procedure TdxVCLResizeTableVirtualColumnMouseHandlerStateStrategy.
  DrawReversibleLineCore(X: Integer);
begin
...
function TdxVCLResizeTableVirtualColumnMouseHandlerStateStrategy.GetControl
  : TdxRichEditControlBase;
begin
  Result := TdxRichEditControlBase(Controller.Control.Control);
end;
...
constructor TdxShadowedFloatingObjectPainter.Create(AUnitConverter
...
begin
  ...
  AContentBounds.Width := AContentBounds.Width + FBounds.Width -
    FInitialShapeBounds.Width;
  AContentBounds.Height := AContentBounds.Height + FBounds.Height -
    FInitialShapeBounds.Height;
  DrawFeedbackShape(AGraphics, FBounds, AContentBounds, FillColor, OutlineColor,
    FUnitConverter);

  if (FImage <> nil) and ShouldDrawImage then
    DrawImage(AGraphics, AContentBounds);

  AGraphics.Rectangle(FBounds, TdxAlphaColors.FromArgb($80, 0, 0, 0),
    TdxAlphaColors.Empty, 1);
end;
...
procedure TdxShadowedFloatingObjectPainter.DrawImage(AGraphics: TdxGraphics;
  const ABounds: TRect);
var
...
begin
  ...
  try
    AAttributes.SetColorMatrix(@AMatrix, ColorMatrixFlagsDefault,
      ColorAdjustTypeBitmap);

    AImgActualSize := FImage.Size;
    AImgRect := cxRect(TdxImageTool.CalculateImageRectCore(ABounds,
      AImgActualSize, SizeMode));
...
procedure TdxShadowedFloatingObjectPainter.DrawFeedbackShape
  (AGraphics: TdxGraphics; const AShapeBounds: TRect;
  const AContentBounds: TRect; AFillColor: TdxAlphaColor;
  AOutlineColor: TdxAlphaColor; AUnitConverter: TdxDocumentLayoutUnitConverter);
var
  ABounds: TRect;
begin
  ...
  begin
    ABounds.InitSize(AShapeBounds.X, AShapeBounds.Y,
      AContentBounds.X - AShapeBounds.X, AShapeBounds.Height);
    AGraphics.FillRectangle(ABounds, AOutlineColor);
    ABounds.InitSize(AContentBounds.X, AShapeBounds.Y, AContentBounds.Width,
      AContentBounds.Y - AShapeBounds.Y);
    AGraphics.FillRectangle(ABounds, AOutlineColor);
    ABounds.InitSize(AContentBounds.Right, AShapeBounds.Y,
      AShapeBounds.Right - AContentBounds.Right, AShapeBounds.Height);
    AGraphics.FillRectangle(ABounds, AOutlineColor);
    ABounds.InitSize(AContentBounds.X, AContentBounds.Bottom,
...
function TdxWinFormsDragFloatingObjectManuallyMouseHandlerStateStrategy.
  GetWinControl: TdxRichEditControlBase;
begin
  Result := TdxRichEditControlBase(Control);
end;

procedure TdxWinFormsDragFloatingObjectManuallyMouseHandlerStateStrategy.
  ShowVisualFeedbackCore(const ABounds: TRect; APageViewInfo: TdxPageViewInfo;
...
procedure TdxWinFormsDragFloatingObjectManuallyMouseHandlerStateStrategy.
  HideVisualFeedbackCore(const ABounds: TRect; APageViewInfo: TdxPageViewInfo);
begin
  WinControl.Invalidate;
end;

function TdxWinFormsDragFloatingObjectManuallyMouseHandlerStateStrategy.
  CreateFeedbackImage(AOriginalImage: TdxOfficeImage): TdxOfficeImage;
var
  AUnitConverter: TdxDocumentLayoutUnitConverter;
  ASize: TSize;
  ABmp: TdxOfficeImage;
...
function TdxWinFormsDragFloatingObjectManuallyMouseHandlerStateStrategy.
  CreateSemitransparentFeedbackImage(ABitmap: TdxSmartImage; AAlpha: Single)
  : TdxOfficeImage;
var
  ACanvas: TdxGPCanvas;
  AMatrix: TdxGPColorMatrix;
...
begin
  FPainter := TdxShadowedFloatingObjectPainter.Create
  ...
  FPainter.FillColor := TdxAlphaColors.FromArgb
    (Round(Alpha * TdxAlphaColors.Alpha(AFillColor)), AFillColor);
  FPainter.OutlineColor := TdxAlphaColors.FromArgb
    (Round(Alpha * TdxAlphaColors.Alpha(AOutlineColor)), AOutlineColor);
end;
...
function TdxVCLRectangularObjectResizeMouseHandlerStateStrategy.GetWinControl
  : TdxRichEditControlBase;
begin
  Result := TdxRichEditControlBase(Control);
end;

procedure TdxVCLRectangularObjectResizeMouseHandlerStateStrategy.
  BeginVisualFeedback;
begin
...
function TdxVCLRectangularObjectResizeMouseHandlerStateStrategy.
  TryFloatingObjectVisualFeedback: Boolean;
var
...
begin
  ...
  FPainter := TdxShadowedFloatingObjectPainter.Create
    (State.DocumentModel.LayoutUnitConverter);
  ...
  FPainter.FillColor := TdxAlphaColors.FromArgb
    (Byte(Trunc(Alpha * dxGetAlpha(AFillColor))), AFillColor);
  FPainter.OutlineColor := TdxAlphaColors.FromArgb
    (Byte(Trunc(Alpha * dxGetAlpha(AOutlineColor))), AOutlineColor);
...
function TdxVCLRectangularObjectResizeMouseHandlerStateStrategy.
  TryInlinePictureVisualFeedback: Boolean;
var
...
procedure TdxVCLRectangularObjectResizeMouseHandlerStateStrategy.
  DrawReversibleFrameCore;
begin
  WinControl.Painter.DeferredDrawReversibleFrame(ObjectActualBounds,
    PageViewInfo);
  WinControl.Refresh;
end;
