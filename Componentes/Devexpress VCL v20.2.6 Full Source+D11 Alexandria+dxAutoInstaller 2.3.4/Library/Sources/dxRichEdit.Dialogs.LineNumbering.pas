...
uses
  ...
  Dialogs, dxCore, cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters, dxLayoutContainer, dxLayoutControl,
  Menus, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, cxContainer, cxEdit, StdCtrls, cxRadioGroup, cxLabel,
...
type
  TdxRichEditLineNumberingDialogForm = class(TdxRichEditCustomDialogForm)
    ...
    procedure edtStartAtPropertiesValidate(Sender: TObject;
  ...
  protected
    ...
    property LineNumberingRestart: TdxLineNumberingRestart
      read GetLineNumberingRestart write SetLineNumberingRestart;
  public
...
begin
  ...
  cbAddLineNumbering.Caption := cxGetResourceString
    (@sdxRichEditLineNumberingDialogAddLineNumbering);
  lblNumbering.Caption := cxGetResourceString
    (@sdxRichEditLineNumberingDialogNumbering);
  rbNumberingRestartEachPage.Caption :=
    cxGetResourceString
    (@sdxRichEditLineNumberingDialogNumberingRestartEachPage);
  rbNumberingRestartEachSection.Caption :=
    cxGetResourceString
    (@sdxRichEditLineNumberingDialogNumberingRestartEachSection);
  rbNumberingRestartContinuous.Caption :=
    cxGetResourceString
    (@sdxRichEditLineNumberingDialogNumberingRestartContinuous);
  lciStartAt.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditLineNumberingDialogStartAt);
  lciCountBy.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditLineNumberingDialogCountBy);
  lciFromText.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditLineNumberingDialogFromText);
end;

procedure TdxRichEditLineNumberingDialogForm.cbAddLineNumberingClick
  (Sender: TObject);
begin
...
begin
  Result := TdxLineNumberingFormController.Create
    (AControllerParameters as TdxLineNumberingFormControllerParameters);
end;

procedure TdxRichEditLineNumberingDialogForm.edtStartAtPropertiesValidate
...
procedure TdxRichEditLineNumberingDialogForm.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
  if ModalResult <> mrOk then
    Exit;
  CanClose := DoValidate(edtStartAt) and DoValidate(edtCountBy) and
    edtFromText.ValidateEdit(True);
  if CanClose then
  begin
    CommitValuesToController;
...
begin
  InitializeMeasurementUnitEdit(edtFromText, ToMeasurementType(UnitType),
    ...
    ModelUnitsToUIUnit(22 * 1440)));
end;

procedure TdxRichEditLineNumberingDialogForm.SetLineNumberingRestart
  (const Value: TdxLineNumberingRestart);
begin
  case Value of
