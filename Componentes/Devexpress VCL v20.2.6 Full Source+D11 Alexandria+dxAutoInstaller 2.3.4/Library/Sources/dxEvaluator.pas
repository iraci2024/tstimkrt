...
const
  ...
  sErrorFunctionAlreadyRegistered =
    'The "%s" function with %d arguments already registered';
  ...
  sErrorTooSmallArguments =
    'Syntax error: function "%s" has too small arguments';
...
type
  ...
  protected
    ...
    function Contains(const W: WideChar; L: PChar; C: Integer)
      : LongBool; inline;
    function FetchToken(var P: PChar; var C: Integer;
      var AToken: TdxParserToken): Boolean; virtual;
  ...
  public
    ...
    function GetToken(out AToken: TdxParserToken; const ADelimiters: string)
      : Boolean; overload;
    function GetToken(out AToken: TdxParserToken;
      const ADelimiters, AQuotes, ASpaces: string): Boolean; overload;
    function MoveToNextSymbol: Boolean; inline;
    // Buffer
    property Scan: PChar read FScan;
    property ScanCount: Integer read FScanCount;
    // Parser Options
    property QuotedTextAsSingleToken: Boolean read FQuotedTextAsSingleToken
      write FQuotedTextAsSingleToken;
    property QuotedTextAsSingleTokenUnquot: Boolean
      read FQuotedTextAsSingleTokenUnquot write FQuotedTextAsSingleTokenUnquot;
  ...
  TdxExpressionEvalProc = function(AParams: TdxExpressionElements)
    : Variant of object;
  ...
  public
    constructor Create(const AName: string; AParamsCount: Integer;
      ADependsFromParametersOnly: Boolean; AProc: TdxExpressionEvalProc;
      ACategory: Byte);
  ...
  public
    ...
    destructor Destroy; override;
    //
    function Find(const AName: PChar; ANameLength, AParamsCount: Integer;
      out AFunction: TdxExpressionEvalFunction): Boolean; overload; virtual;
    function Find(const AName: PChar; ANameLength: Integer;
      out AFunction: TdxExpressionEvalFunction): Boolean; overload; virtual;
    function Find(const AName: string; AParamsCount: Integer;
      out AFunction: TdxExpressionEvalFunction): Boolean; overload; inline;
    function Find(const AName: string; out AFunction: TdxExpressionEvalFunction)
  ...
  public
    constructor Create(const AName: string; AProc: TdxExpressionEvalProc;
      APriority, AParamsCount: Integer;
      AAssociativity: TdxExpressionEvalOperatorAssociativity = eoaLeftToRight);
    //
    property Associativity: TdxExpressionEvalOperatorAssociativity
      read FAssociativity;
  ...
  protected
    ...
    procedure AddFromStack(AStack: TdxExpressionFastStack<TdxExpressionElement>;
      ACount: Integer);
    procedure Clear;
    procedure Optimize;
  public
  ...
  public
    ...
    procedure RegisterFunction(const AName: string;
      AProc: TdxExpressionEvalProc; ACategory: Byte = CategoryGeneral);
      overload;
    procedure RegisterFunction(const AName: string;
      AProc: TdxExpressionEvalProc; AParamsCount: Integer;
      ADependsFromParametersOnly: Boolean;
      ACategory: Byte = CategoryGeneral); overload;
  ...
  TdxExpressionCompilerSolidTokenType = (ecsttNone, ecsttOperand,
    ecsttOperator);
  ...
  strict private
    ...
    procedure ParseParametersList(AFunctionElement
      : TdxExpressionElementFunction);
  protected
    ...
    //
    property OperatorStack: TdxExpressionFastStack<TdxExpressionEvalOperator>
      read FOperatorStack;
    property OutputBuffer: TdxExpressionFastStack<TdxExpressionElement>
      read FOutputBuffer;
  ...
  protected
    function FetchToken(var P: PChar; var C: Integer;
      var AToken: TdxParserToken): Boolean; override;
    //
...
function dxCompareStrings(const S1, S2: string; AIgnoreCase: Boolean = True)
  : Integer; overload;
function dxCompareStrings(P1, P2: PChar; L1, L2: Integer;
  AIgnoreCase: Boolean = True): Integer; overload;
...
begin
  Result := CompareStringW(LOCALE_USER_DEFAULT, CaseMap[AIgnoreCase], P1, L1,
    P2, L2) - CSTR_EQUAL;
end;
...
function dxCompareStrings(const S1, S2: string; AIgnoreCase: Boolean = True)
  : Integer; overload;
begin
  Result := dxCompareStrings(PChar(S1), PChar(S2), Length(S1), Length(S2),
    AIgnoreCase);
end;
...
begin
  ...
  begin
    AElement.Optimize;
    if AElement.IsConstant and
      (AElement.ClassType <> TdxExpressionElementConstant) then
    begin
...
function TdxParser.GetToken(out AToken: TdxParserToken;
  const ADelimiters: string): Boolean;
var
  TB: PChar;
  TL: Integer;
begin
...
function TdxParser.FetchToken(var P: PChar; var C: Integer;
  var AToken: TdxParserToken): Boolean;

  procedure ExtractIdent(var AToken: TdxParserToken; ATokenType: Integer;
    ADelimiters: PChar; ADelimitersCount: Integer);
  begin
    AToken.Data := P;
    ...
    AToken.DataLength := (NativeUInt(P) - NativeUInt(AToken.Data))
      div SizeOf(WideChar);
  end;

  procedure SetToken(var AToken: TdxParserToken; var P: PChar; var C: Integer;
    ATokenType, ATokenLength: Integer); inline;
  begin
    AToken.Data := P;
...
begin
  Result := SkipQuotes and (AToken.TokenType = dxTokenQuot) or SkipSpaces and
    (AToken.TokenType = dxTokenSpace) or SkipDelimiters and
    (AToken.TokenType = dxTokenDelimiter);
end;

{ TdxParserToken }

function TdxParserToken.Compare(const S: string;
...
function TdxExpressionEvalFunctionList.Find(const AName: PChar;
  ANameLength: Integer; out AFunction: TdxExpressionEvalFunction): Boolean;
var
  I: Integer;
begin
  ...
  begin
    if Remap.TryGetValue(UpperCase(dxMakeString(AName, ANameLength)), AFunction)
    then
      Exit(True);
  end;
...
function TdxExpressionEvalFunctionList.Find(const AName: PChar;
  ANameLength, AParamsCount: Integer;
  out AFunction: TdxExpressionEvalFunction): Boolean;
var
...
begin
  ...
  begin
    AItem := Items[I];
    if (AItem.ParamsCount = AParamsCount) and Compare(AItem.Name, AName,
      ANameLength) then
    begin
      AFunction := AItem;
      Exit(True);
...
function TdxExpressionEvalFunctionList.Compare(const S: string; B: PChar;
  L: Integer): Boolean;
begin
...
constructor TdxExpressionEvalOperator.Create(const AName: string;
  AProc: TdxExpressionEvalProc; APriority, AParamsCount: Integer;
  AAssociativity: TdxExpressionEvalOperatorAssociativity);
begin
...
constructor TdxExpressionEvalVariable.Create(const AName: string;
  const AValue: PVariant);
begin
  inherited Create(AName, 0, False, EvalProc, 0);
  FValue := AValue;
end;

function TdxExpressionEvalVariable.EvalProc
  (AParams: TdxExpressionElements): Variant;
begin
  Result := FValue^;
end;
...
function TdxExpressionElements.ToString(AExpression
  : TdxCustomExpression): string;
var
  I: Integer;
begin
...
function TdxExpressionElementConstant.ToString(AExpression
  : TdxCustomExpression): string;
begin
  Result := FValue;
end;
...
constructor TdxExpressionElementFunction.Create
  (AEvalFunc: TdxExpressionEvalFunction);
begin
  FEvalFunc := AEvalFunc;
...
function TdxExpressionElementFunction.ToString(AExpression
  : TdxCustomExpression): string;
begin
  Result := Name;
...
function TdxExpressionElementOperator.ToString(AExpression
  : TdxCustomExpression): string;
...
begin
  if Params.Count = 2 then
    Result := ParamToString(Params[0]) + ' ' + Name + ' ' +
...
procedure TdxCustomExpression.RegisterFunction(const AName: string;
  AProc: TdxExpressionEvalProc; ACategory: Byte = 0);
begin
...
procedure TdxCustomExpression.RegisterFunction(const AName: string;
  AProc: TdxExpressionEvalProc; AParamsCount: Integer;
  ADependsFromParametersOnly: Boolean; ACategory: Byte = 0);
var
  AFunction: TdxExpressionEvalFunction;
begin
  if KnownFunctions.Find(AName, AParamsCount, AFunction) then
    raise EdxExpression.CreateFmt(sErrorFunctionAlreadyRegistered,
      [AName, AParamsCount]);
  KnownFunctions.Add(TdxExpressionEvalFunction.Create(AName, AParamsCount,
    ADependsFromParametersOnly, AProc, ACategory));
end;

procedure TdxCustomExpression.RegisterFunctionRemap(const AOldName,
  ANewName: string);
var
  AFunction: TdxExpressionEvalFunction;
begin
...
procedure TdxCustomExpression.RegisterOperator(const AName: string;
  AProc: TdxExpressionEvalProc; AParamsCount, APriority: Integer);
const
  Map: array [Boolean] of TdxExpressionEvalOperatorAssociativity =
    (eoaRightToLeft, eoaLeftToRight);
begin
  if (AParamsCount < 1) or (AParamsCount > 2) then
    raise EdxExpression.Create(sErrorOperatorArguments);
  KnownOperators.Add(TdxExpressionEvalOperator.Create(AName, AProc, APriority,
    AParamsCount, Map[AParamsCount = 2]));
end;

procedure TdxCustomExpression.RegisterVariable(const AName: string;
  const AValue: PVariant);
var
  AFunction: TdxExpressionEvalFunction;
begin
...
function TdxCustomExpression.CreateKnownFunctions
  : TdxExpressionEvalFunctionList;
begin
  Result := TdxExpressionEvalFunctionList.Create
end;

function TdxCustomExpression.CreateKnownOperators
  : TdxExpressionEvalFunctionList;
begin
  Result := TdxExpressionEvalFunctionList.Create
end;

function TdxCustomExpression.InternalCompile(const S: string)
  : TdxExpressionElement;
var
...
begin
  ...
  begin
    ...
    raise EdxExpressionCompiler.CreateFmt(AMessage + #13#10 + sErrorCursorInfo,
      [Token.ToString, AScanArea]);
  end
  else
...
begin
  AFunctionElement := ClassFunction.Create
    (TdxExpressionEvalFunction(Token.Context));
  try
    if (ScanCount > 0) and (Scan^ = '(') then
      ParseParametersList(AFunctionElement);
    if AFunctionElement.EvalFunc.ParamsCount >= 0 then
    begin
      if AFunctionElement.EvalFunc.ParamsCount <> AFunctionElement.Params.Count
      then
      begin
        // try to find overload version of this function with other number of arguments
        if Owner.KnownFunctions.Find(AFunctionElement.Name,
          AFunctionElement.Params.Count, AFunction) then
...
begin
  ...
  repeat
    ...
    begin
      if (AOperator.Associativity = eoaRightToLeft) and
        (AOperator.Priority < AOperatorPeek.Priority) or
        (AOperator.Associativity = eoaLeftToRight) and
        (AOperator.Priority <= AOperatorPeek.Priority) then
        OutputOperator(OperatorStack.Pop)
      else
        Break;
    end;
...
procedure TdxCustomExpressionCompiler.OutputOperator
  (AOperator: TdxExpressionEvalOperator);
var
...
procedure TdxCustomExpressionCompiler.ParseParametersList(AFunctionElement
  : TdxExpressionElementFunction);

  function ExtractParameter(AScanStart, AScanFinish: PChar)
    : TdxExpressionElement;
  var
    ALength: Integer;
  begin
    ALength := (NativeUInt(AScanFinish) - NativeUInt(AScanStart))
      div SizeOf(WideChar);
    if ALength > 0 then
    ...
    if (AToken.TokenType = dxTokenDelimiter) and (AToken.DataLength = 1) then
      case AToken.Data^ of
        ...
        ')':
          ...
          begin
            if (AParameterCursor <> Scan - AToken.DataLength) or
              (AFunctionElement.Params.Count > 0) then
              AFunctionElement.Params.Add(ExtractParameter(AParameterCursor,
                Scan - AToken.DataLength));
            Break;
          end
          else
            Dec(ABracketLevel);
...
function TdxMathExpression.OperatorDivide
  (AParams: TdxExpressionElements): Variant;
begin
  Result := AParams[0].Evaluate / AParams[1].Evaluate;
end;

function TdxMathExpression.OperatorDivideInt
  (AParams: TdxExpressionElements): Variant;
begin
  Result := AParams[0].Evaluate div AParams[1].Evaluate;
end;

function TdxMathExpression.OperatorEqual
  (AParams: TdxExpressionElements): Variant;
begin
  try
...
function TdxMathExpression.OperatorGreater
  (AParams: TdxExpressionElements): Variant;
begin
  Result := Ord(AParams[0].Evaluate > AParams[1].Evaluate);
end;

function TdxMathExpression.OperatorGreaterOrEqual
  (AParams: TdxExpressionElements): Variant;
begin
  Result := Ord(AParams[0].Evaluate >= AParams[1].Evaluate);
end;

function TdxMathExpression.OperatorLower
  (AParams: TdxExpressionElements): Variant;
begin
  Result := Ord(AParams[0].Evaluate < AParams[1].Evaluate);
end;

function TdxMathExpression.OperatorLowerOrEqual
  (AParams: TdxExpressionElements): Variant;
begin
  Result := Ord(AParams[0].Evaluate <= AParams[1].Evaluate);
end;

function TdxMathExpression.OperatorMinus
  (AParams: TdxExpressionElements): Variant;
begin
...
function TdxMathExpression.OperatorMultiply
  (AParams: TdxExpressionElements): Variant;
begin
  Result := AParams[0].Evaluate * AParams[1].Evaluate;
end;
...
function TdxMathExpression.OperatorNotEqual
  (AParams: TdxExpressionElements): Variant;
begin
  Result := Ord(AParams[0].Evaluate <> AParams[1].Evaluate);
end;
...
function TdxMathExpression.OperatorPlus(AParams: TdxExpressionElements)
  : Variant;
begin
  Result := AParams[0].Evaluate + AParams[1].Evaluate;
end;

function TdxMathExpression.OperatorPower
  (AParams: TdxExpressionElements): Variant;
begin
  Result := Power(AParams[0].Evaluate, AParams[1].Evaluate);
end;
...
function TdxMathExpressionCompiler.FetchToken(var P: PChar; var C: Integer;
  var AToken: TdxParserToken): Boolean;
var
...
begin
  ...
  begin
    if Owner.KnownOperators.Find(T.Data, T.DataLength,
      1 + Ord(PrevSolidToken = ecsttOperand), AEvalFunction) then
    begin
  ...
  begin
    ...
    if Result then
      case AToken.TokenType of
        dxTokenDelimiter:
          if (AToken.DataLength = 1) and (AToken.Data^ = '[') then
          // Is it variable?
          begin
            K := StrScan(P, C, ']');
            if K <> nil then
            begin
              ...
              AToken.DataLength := (NativeUInt(K) - NativeUInt(P))
                div SizeOf(Char);
              AToken.TokenType := dxExprTokenFunction;
              if Owner.KnownFunctions.Find(AToken.Data, AToken.DataLength,
                AEvalFunction) then
        ...
        dxTokenIdent:
          ...
          else if Owner.KnownFunctions.Find(AToken.Data, AToken.DataLength,
            AEvalFunction) then
          begin
            AToken.TokenType := dxExprTokenFunction;
            AToken.Context := AEvalFunction;
          end
          else if Owner.KnownOperators.Find(AToken.Data, AToken.DataLength,
            AEvalFunction) then
          begin
            AToken.TokenType := dxExprTokenOperator;
            AToken.Context := AEvalFunction;
...
function TdxMathExpressionCompiler.IsNumeric(const AToken
  : TdxParserToken): Boolean;
var
  C: PChar;
  I: Integer;
begin
