...
uses
  SysUtils, Types, Generics.Defaults, Generics.Collections, dxCore, dxPDFBase,
  dxPDFTypes, dxPDFParser;

type
  TdxPDFGlyphMappingFlags = (mfNone = 0, mfUseKerning = 1,
    mfDirectionRightToLeft = 2);
  ...
  protected
    ...
    function FindCode(const ACode: TBytes; APosition: Integer)
      : TdxPDFCharacterMappingSearchResult; virtual; abstract;
    procedure AddCode(const ACode: TBytes; APosition: Integer;
  ...
  strict protected
    ...
    function FindCode(const ACode: TBytes; APosition: Integer)
      : TdxPDFCharacterMappingSearchResult; override;
  public
  ...
  strict protected
    ...
    property Nodes: TObjectDictionary<Byte, TdxPDFCharacterMappingTreeNode>
      read FNodes;
  public
  ...
  protected
    Data: TBytes;
  public
    constructor Create(ACMapTable: TList<TdxPDFCMapRecord>;
      AMaxByteCount: Integer);
    destructor Destroy; override;

    class function CreateCharacterMappingData
      (ACharMap: TdxPDFIntegerStringDictionary;
      const AFontName: string): TBytes;

    function GetStringData(const AData: TBytes;
  ...
  strict private
    ...
    procedure ReadChars(ACMapTable: TObjectList<TdxPDFCMapRecord>;
      ACount: Integer);
    procedure ReadRange(ACMapTable: TObjectList<TdxPDFCMapRecord>;
      var ACode, ALastCode: TBytes);
    procedure ReadRangeCodes(var ACode, ALastCode: TBytes);
    procedure ReadRanges(ACMapTable: TObjectList<TdxPDFCMapRecord>;
      ACount: Integer);
  strict protected
    function CanContinueTokenReading: Boolean; override;
    function ReadHexadecimalString(AIsHexStrSeparatedByWhiteSpaces
      : Boolean = False): TdxPDFString; override;
  ...
  public
    class function Parse(ARepository: TdxPDFCustomRepository;
      const AData: TBytes): TdxPDFCharacterMapping;
  ...
  public
    ...
    property HasAddedCharacters: Boolean read FHasAddedCharacters
      write FHasAddedCharacters;
  ...
  public
    class function Create(AIndex: Integer; AWidth, AOffset: Double)
      : TdxPDFGlyph; static;
  ...
  public
    ...
    procedure Prepend(AGlyphRun: TdxPDFGlyphRun;
      ASpaceGlyphIndex: Integer); overload;
  ...
  strict private
    ...
    FMapping: TdxPDFIntegerStringDictionary;
  public
    class function Create(AGlyphRun: TdxPDFGlyphRun): TdxPDFGlyphMappingResult;
      overload; static;
  ...
  public
    class function Create(const AGlyphIndices, AGlyphPositions,
      AOrder: TIntegerDynArray; ADPI, AFontSize: Double)
      : TdxPDFGlyphPlacementInfo; static;
...
uses
  Classes, Variants, Math, dxStringHelper, dxPDFUtils, dxPDFFont,
  dxPDFFontUtils, dxFontFile;

function dxPDFCharacterMappingSearchResult(AValue: string; ACodeLength: Integer)
  : TdxPDFCharacterMappingSearchResult;
begin
...
procedure TdxPDFCharacterMappingTreeNode.AddCode(const ACode: TBytes;
  APosition: Integer; const AValue: string);
begin
  // do nothing
end;
...
function TdxPDFCharacterMappingTreeLeaf.FindCode(const ACode: TBytes;
  APosition: Integer): TdxPDFCharacterMappingSearchResult;
begin
...
begin
  inherited Create;
  FNodes := TObjectDictionary<Byte, TdxPDFCharacterMappingTreeNode>.Create
    ([doOwnsValues]);
end;
...
function TdxPDFCharacterMappingTreeBranch.FindCode(const ACode: TBytes;
  APosition: Integer): TdxPDFCharacterMappingSearchResult;
var
  ANode: TdxPDFCharacterMappingTreeNode;
  ASearchResult: TdxPDFCharacterMappingSearchResult;
begin
  if (Length(ACode) - APosition > 0) and Nodes.TryGetValue(ACode[APosition],
    ANode) then
  begin
    ASearchResult := ANode.FindCode(ACode, APosition + 1);
    Exit(dxPDFCharacterMappingSearchResult(ASearchResult.Value,
      ASearchResult.CodeLength + 1));
  end;
  Result := dxPDFCharacterMappingSearchResult('', 0);
end;

procedure TdxPDFCharacterMappingTreeBranch.AddCode(const ACode: TBytes;
  APosition: Integer; const AValue: string);
var
  AFirstByte: Byte;
...
constructor TdxPDFCharacterMapping.Create(ACMapTable: TList<TdxPDFCMapRecord>;
  AMaxByteCount: Integer);
begin
  inherited Create;
...
class function TdxPDFCharacterMapping.CreateCharacterMappingData
  (ACharMap: TdxPDFIntegerStringDictionary; const AFontName: string): TBytes;
var
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              S := '<' + IntToHex(APair.Key, 4) + '> <' + IntToHex(ABytes[1], 2)
                + IntToHex(ABytes[0], 2) + '>';
              AWriter.WriteLine(S);
            end;
...
begin
  ...
  try
    ...
    begin
      ...
      else if dxWideIsNumeric(AFindResult.Value[1]) and
        (Length(AFindResult.Value) > 1) then
...
function TdxPDFCharacterMapping.FindCode(const ACode: TBytes;
  APosition: Integer): TdxPDFCharacterMappingSearchResult;
var
  ACodeLength: Integer;
  ANewCode: TBytes;
begin
  ...
  begin
    ...
    cxCopyData(@ACode[0], @ANewCode[0], APosition, 1,
      ACodeLength * SizeOf(Byte));
...
procedure TdxPDFCharacterMapping.PopulateTree
  (ACMapTable: TList<TdxPDFCMapRecord>);
var
  I: Integer;
begin
...
class function TdxPDFCMapStreamParser.Parse(ARepository: TdxPDFCustomRepository;
  const AData: TBytes): TdxPDFCharacterMapping;
var
...
function TdxPDFCMapStreamParser.Read(const AData: TBytes)
  : TdxPDFCharacterMapping;
begin
  Data := AData;
  Result := DoRead;
...
begin
  Result := inherited CanContinueTokenReading and
    (Current <> TdxPDFDefinedSymbols.StartObject);
end;

function TdxPDFCMapStreamParser.ReadHexadecimalString
  (AIsHexStrSeparatedByWhiteSpaces: Boolean = False): TdxPDFString;
var
  AData: TBytes;
  ASymbol: Byte;
begin
...
function TdxPDFCMapStreamParser.ReadCompositeObject
...
procedure TdxPDFCMapStreamParser.ReadChars(ACMapTable
  : TObjectList<TdxPDFCMapRecord>; ACount: Integer);
var
  I: Integer;
  AValue: TdxPDFBase;
    ...
    begin
      case AValue.ObjectType of
        otString:
          ...
          begin
            AUnicodeCharacters := TdxPDFUtils.StrToByteArray
              (TdxPDFString(AValue).Value);
            ACMapTable.Add(TdxPDFCMapRecord.Create(ACode,
              ToUnicode(AUnicodeCharacters)));
          end;
        otInteger:
          ACMapTable.Add(TdxPDFCMapRecord.Create(ACode,
            IntToStr(TdxPDFInteger(AValue).Value)));
      end;
...
procedure TdxPDFCMapStreamParser.ReadRange(ACMapTable
  : TObjectList<TdxPDFCMapRecord>; var ACode, ALastCode: TBytes);
var
  I, AValue: Integer;
  ARange: TdxPDFBase;
  AUnicode: string;
  ...
  begin
    case ARange.ObjectType of
        ...
        begin
          ...
          repeat
            if I >= TdxPDFArray(ARange).Count then
              TdxPDFUtils.Abort;
            AUnicodeRangeStart := TdxPDFUtils.StrToByteArray
              ((TdxPDFArray(ARange)[I] as TdxPDFString).Value);
            Inc(I);
            ACMapTable.Add(TdxPDFCMapRecord.Create(ACode,
              ToUnicode(AUnicodeRangeStart)));
      ...
      otString:
        begin
          AUnicode :=
            ToUnicode(TdxPDFUtils.StrToByteArray(TdxPDFString(ARange).Value));
...
procedure TdxPDFCMapStreamParser.ReadRangeCodes(var ACode, ALastCode: TBytes);
  ...
  procedure CalculateRangeCode(ACodeLength, ALastCodeLength, ALengthDifference
    : Integer; var ACode: TBytes);
  var
    ATempCode: TBytes;
  begin
    SetLength(ATempCode, ALastCodeLength);
    cxCopyData(@ACode[0], @ATempCode[0], 0, -ALengthDifference,
      ACodeLength * SizeOf(Byte));
    SetLength(ACode, 0);
    ACode := ATempCode;
  end;
...
procedure TdxPDFCMapStreamParser.ReadRanges(ACMapTable
  : TObjectList<TdxPDFCMapRecord>; ACount: Integer);
var
  I: Integer;
...
procedure TdxPDFCMapStreamParser.PopulateCMapTable
          ...
          otString:
            begin
              ...
              if AToken.Value = 'endcmap' then
                Break
              else if (AToken.Value = 'beginbfchar') or
                (AToken.Value = 'begincidchar') then
                ReadChars(ACMapTable, ARecordCount)
              else if (AToken.Value = 'beginbfrange') or
                (AToken.Value = 'begincidrange') then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        TdxPDFUtils.AddChar(Char(ABytes[AIndex] shl 8 + ABytes[AIndex + 1]
          ), AArray);
        Inc(AIndex);
      end;
      Inc(AIndex);
...
function TdxPDFCMapStreamParser.CheckCode(const ALastCode: TBytes;
  var ACode: TBytes): Boolean;
var
...
procedure TdxPDFCharacterCache.RegisterString
  (AToUnicode: TdxPDFIntegerStringDictionary);
var
  ACode: Integer;
  APair: TPair<Integer, string>;
begin
...
class function TdxPDFGlyph.Create(AIndex: Integer; AWidth: Double;
  AOffset: Double): TdxPDFGlyph;
begin
...
procedure TdxPDFGlyphRun.Prepend(AGlyphRun: TdxPDFGlyphRun;
  ASpaceGlyphIndex: Integer);
var
  J, I: Integer;
  AGlyph: TdxPDFGlyph;
begin
...
class function TdxPDFGlyphPlacementInfo.Create(const AGlyphIndices,
  AGlyphPositions, AOrder: TIntegerDynArray; ADPI, AFontSize: Double)
  : TdxPDFGlyphPlacementInfo;
begin
