...
type
  ...
  public
    ...
    function FindFolderByFullQualifiedDirName(const AFullQualifiedDirName
      : string): TdxPSFileBasedExplorerFolder;
    function FindItemByFullQualifiedFileName(const AFullQualifiedName: string)
      : TdxPSFileBasedExplorerItem;

    property DirName: string read GetDirName;
    property Folders[Index: Integer]: TdxPSFileBasedExplorerFolder
  ...
  protected
    ...
    property IsIOOutwardlyControlled: Boolean read FIsIOOutwardlyControlled
      write FIsIOOutwardlyControlled;
  public
    function CannotRenameMessageText(const AOldName, ANewName: string)
      : string; override;
    function CanMoveTo(AParent: TCustomdxPSExplorerItem): Boolean; override;
  ...
  TdxPSFileBasedExplorerContextCommandClass = class of
    TdxPSFileBasedExplorerContextCommand;

  TdxPSFileBasedExplorerContextCommand = class
    (TCustomdxPSExplorerContextCommand)
  public
    function Explorer: TdxPSFileBasedExplorer; reintroduce; overload;
  end;

  TdxPSFileBasedExplorerSetAsRootContextCommand = class
    (TdxPSFileBasedExplorerContextCommand)
  public
  ...
  TdxPSFileBasedExplorerChangeRootContextCommand = class
    (TdxPSFileBasedExplorerContextCommand)
  public
    constructor Create(AnExplorer: TCustomdxPSExplorer); override;
    function Enabled: Boolean; override;
  ...
  TdxPSFileBasedExplorerGoToUpOneLevelContextCommand = class
    (TdxPSFileBasedExplorerContextCommand)
  public
    constructor Create(AnExplorer: TCustomdxPSExplorer); override;
    function Enabled: Boolean; override;
  ...
  TdxPSFileBasedExplorerLoadErrorEvent = procedure
    (Sender: TdxPSFileBasedExplorer; const AName: string) of object;
  ...
  protected
    ...
    procedure BuildCommandSet(ABuilder
      : IdxPSExplorerContextCommandBuilder); override;
    // IdxPSExplorerContextCommands2
    procedure FinalizeCommand(ACommand
      : TCustomdxPSExplorerContextCommand); override;
    ...
    function CreateDataStream(const AFileName: string;
      AMode: TdxPSStreamMode): TStream;
    function CreateItemDataStream(AnItem: TdxPSExplorerItem;
      AMode: TdxPSStreamMode): TStream; override;
    ...
    procedure ProcessIOError(AnUnconditionalRaiseException
  ...
  public
    ...
    function CreateNewFolder(AParent: TdxPSExplorerFolder)
      : TdxPSExplorerFolder; override;
    function CreateNewItem(AParent: TdxPSExplorerFolder;
      AReportLink: TBasedxReportLink): TdxPSExplorerItem; override;

    function FindCustomItemByUniqueID(const AnUniqueID: TBytes)
      : TCustomdxPSExplorerItem; override;
    function FindFolderByFullQualifiedDirName(const AFullQualifiedDirName
      : string): TdxPSFileBasedExplorerFolder;
    function FindItemByFullQualifiedFileName(const AFullQualifiedName: string)
      : TdxPSFileBasedExplorerItem;
    ...
    procedure LoadItemData(const AFullQualifiedFileName: string;
    ...
    property ActiveFolder: TdxPSFileBasedExplorerFolder read GetActiveFolder
  ...
  published
    property Options: TdxPSFileBasedExplorerOptions read FOptions
      write SetOptions default [eoShowIOErrors];
    property RootPath: string read FRootPath write SetRootPath;
    property OnLoadError: TdxPSFileBasedExplorerLoadErrorEvent read FOnLoadError
      write FOnLoadError;
  end;
...
begin
  ...
  begin
    ...
    pFrom := PChar(AName + #0);
    wFunc := FO_DELETE;
    fFlags := FOF_ALLOWUNDO or FOF_NOERRORUI or FOF_SILENT or
      ConfirmationMap[AConfirmation];
  end;
...
begin
  ...
  begin
    ...
    pFrom := PChar(APath + #0);
    wFunc := FO_DELETE;
    fFlags := FOF_ALLOWUNDO or FOF_NOCONFIRMATION or FOF_NOERRORUI or
      FOF_SILENT;
  end;
...
begin
  Result := (Trim(AName) <> '') and not HasAnyChar(AName,
    dxPSUtl.InvalidFileNameChars);
end;

{ TdxPSFileBasedExplorerFolder }

function TdxPSFileBasedExplorerFolder.CannotRenameMessageText(const AOldName,
...
function TdxPSFileBasedExplorerFolder.CanMoveTo
...
begin
  Result := inherited CanMoveTo(AParent) and (AParent <> nil) and
    ...
    CanRenameTo(TdxPSFileBasedExplorerFolder(AParent).FullQualifiedDirName +
    '\' + Name));
end;
...
begin
  Result := inherited CanRenameTo(AName) and
    ValidateFileName(ExtractFileName(AName));
end;
...
function TdxPSFileBasedExplorerFolder.GetUniqueID(out AnUniqueID
  : TBytes): Integer;
begin
...
function TdxPSFileBasedExplorerFolder.ItemByName(const AName: string)
  : TdxPSExplorerItem;
begin
  Result := inherited ItemByName(Explorer.AcquireExtension(AName));
end;

function TdxPSFileBasedExplorerFolder.FindFolderByFullQualifiedDirName
  (const AFullQualifiedDirName: string): TdxPSFileBasedExplorerFolder;

  function InternalFindFolder(AFolder: TdxPSFileBasedExplorerFolder;
    ADirName: string): TdxPSFileBasedExplorerFolder;
  var
    P: Integer;
    FolderName: string;
  begin
...
begin
  ...
  begin
    DirName := Copy(S, Length(FullQualifiedDirName) + 1 + 1 { PathDelimiter } ,
      Length(S));
    if DirName = '' then
      Result := Self
    else
...
function TdxPSFileBasedExplorerFolder.FindItemByFullQualifiedFileName
  (const AFullQualifiedName: string): TdxPSFileBasedExplorerItem;
var
  S: string;
...
function TdxPSFileBasedExplorerFolder.DoMove
    ...
    begin
      ...
      try
        if Explorer.IOStatus <> mrYesToAll then
          ...
          begin
            Explorer.IOStatus := MessageDlg(Item.OverwriteMessageText(AParent),
              mtWarning, Buttons, 0);
...
begin
  ...
  begin
    ...
    DestName := TdxPSFileBasedExplorerFolder(AParent).FullQualifiedDirName + '\'
      + DirName;
    if DirectoryExists(DestName) then
    begin
      Populate;
      NewParent := TdxPSFileBasedExplorerFolder(AParent)
        .FindFolderByFullQualifiedDirName(DestName);
      NewParent.Populate;

      if Explorer.IOStatus <> mrYesToAll then
      begin
        Explorer.IOStatus := MessageDlg(OverwriteMessageText(AParent),
          mtWarning, Buttons, 0);
...
function TdxPSFileBasedExplorerFolder.GetFolder(Index: Integer)
  : TdxPSFileBasedExplorerFolder;
begin
...
function TdxPSFileBasedExplorerFolder.GetItem(Index: Integer)
  : TdxPSFileBasedExplorerItem;
begin
...
begin
  if IsVolume and not(DriveType in [dxPSGlbl.dtUnknown, dxPSGlbl.dtNoRootDir])
  then
...
function TdxPSFileBasedExplorerItem.AcquireExtension
...
function TdxPSFileBasedExplorerItem.SuppressExtension
  (const AName: string): string;
begin
  if Explorer <> nil then
...
function TdxPSFileBasedExplorerItem.CannotRenameMessageText(const AOldName,
  ANewName: string): string;
begin
  if not ValidateFileName(ANewName) then
...
function TdxPSFileBasedExplorerItem.CanMoveTo
  (AParent: TCustomdxPSExplorerItem): Boolean;
// Name is equal '' in creation phase
begin
  Result := inherited CanMoveTo(AParent) and (AParent <> nil) and
...
function TdxPSFileBasedExplorerItem.GetUniqueID(out AnUniqueID: TBytes)
  : Integer;
begin
...
function TdxPSFileBasedExplorerItem.IsNameChanged(const ANewName
  : string): Boolean;
begin
...
begin
  ...
  if Result and FileExists(FullQualifiedFileName) then
    Result := ShellRemoveFile(FullQualifiedFileName, False) and
      not FileExists(FullQualifiedFileName);
end;
...
begin
  ...
  AFileSize := FileSize;
  if AFileSize > 0 then
    Result := Result + dxCRLF + dxPSUtl.DropAmpersand
      (cxGetResourceString(@sdxSize)) + ': ' + dxPSUtl.FormatFileSize
      (AFileSize);
end;

function TdxPSFileBasedExplorerItem.GetNewName(AReportLink
  : TBasedxReportLink): string;
begin
  Result := Explorer.AcquireExtension(inherited GetNewName(AReportLink));
end;

function TdxPSFileBasedExplorerItem.DoMove
  (AParent: TdxPSExplorerFolder): Boolean;
var
  ...
  begin
    ...
    begin
      if IsIOOutwardlyControlled then
        ...
        CancelOperation := not MessageQuestion(OverwriteMessageText(AParent)) or
          not ShellRemoveFile(DestName, False);
...
begin
  ...
  if Result and not(esLoading in Explorer.State) and
    not dxSameText(FullQualifiedFileName, FullQualifiedFilePath +
    ExtensionedName) then
  begin
    Result := RenameFile(FullQualifiedFileName, FullQualifiedFilePath +
      ExtensionedName);
    Explorer.CheckIOError(Result);
  end;
end;
...
constructor TdxPSFileBasedExplorerSetAsRootContextCommand.Create
...
constructor TdxPSFileBasedExplorerChangeRootContextCommand.Create
  (AnExplorer: TCustomdxPSExplorer);
begin
  inherited;
...
constructor TdxPSFileBasedExplorerGoToUpOneLevelContextCommand.Create
  (AnExplorer: TCustomdxPSExplorer);
begin
  inherited;
...
procedure TdxPSFileBasedExplorer.LoadFromIniFile(AIniFile: TCustomIniFile;
...
procedure TdxPSFileBasedExplorer.SaveToIniFile(AIniFile: TCustomIniFile;
  const ASection: string);
begin
...
procedure TdxPSFileBasedExplorer.PopulatePath(APath: string);
  ...
  procedure InternalPopulateFolder(AFolder: TdxPSFileBasedExplorerFolder;
    ADirName: string);
  var
    P: Integer;
    FolderName: string;
...
function TdxPSFileBasedExplorer.CreateNewFolder(AParent: TdxPSExplorerFolder)
  : TdxPSExplorerFolder;
var
...
begin
  ...
  begin
    ...
    try
      DirectoryName := TdxPSFileBasedExplorerFolder(Result)
        .FullQualifiedDirName;
...
function TdxPSFileBasedExplorer.FindCustomItemByUniqueID(const AnUniqueID
...
function TdxPSFileBasedExplorer.FindFolderByFullQualifiedDirName
  (const AFullQualifiedDirName: string): TdxPSFileBasedExplorerFolder;
begin
  Result := Root.FindFolderByFullQualifiedDirName(AFullQualifiedDirName);
end;

function TdxPSFileBasedExplorer.FindItemByFullQualifiedFileName
  (const AFullQualifiedName: string): TdxPSFileBasedExplorerItem;
begin
...
procedure TdxPSFileBasedExplorer.LoadItemData(const AFullQualifiedFileName
  : string; AReportLink: TBasedxReportLink);
var
  S: string;
  Item: TdxPSFileBasedExplorerItem;
begin
...
procedure TdxPSFileBasedExplorer.UnloadItemData
  (const AFullQualifiedFileName: string);
var
  Item: TdxPSExplorerItem;
begin
  Item := FindItemByFullQualifiedFileName
    (dxPSUtl.GetLongFileName(AFullQualifiedFileName));
  if Item <> nil then
    Item.Unload;
end;
...
begin
  Result := (ActiveFolderPath <> '') and
  // ValidateFileName(ActiveFolderPath) and
...
procedure TdxPSFileBasedExplorer.BuildCommandSet
...
begin
  ...
  ABuilder.AddExplorerContextCommand
    (AddCommand(TdxPSFileBasedExplorerChangeRootContextCommand));
  ABuilder.AddExplorerContextCommand
    (AddCommand(TdxPSFileBasedExplorerSetAsRootContextCommand));
  ABuilder.AddExplorerContextCommand
    (AddCommand(TdxPSFileBasedExplorerGoToUpOneLevelContextCommand));
end;

procedure TdxPSFileBasedExplorer.FinalizeCommand
  (ACommand: TCustomdxPSExplorerContextCommand);
begin
  inherited;
end;

procedure TdxPSFileBasedExplorer.InitializeCommand
  (ACommand: TCustomdxPSExplorerContextCommand);
begin
  inherited;
end;

class function TdxPSFileBasedExplorer.AcceptItemNameChar
...
function TdxPSFileBasedExplorer.CreateDataStream(const AFileName: string;
  AMode: TdxPSStreamMode): TStream;
const
  FileModesMap: array [TdxPSStreamMode] of Word = (fmOpenRead or
    fmShareDenyWrite, fmCreate or fmOpenWrite or fmShareDenyWrite,
    fmOpenReadWrite);
begin
...
function TdxPSFileBasedExplorer.CreateItemDataStream(AnItem: TdxPSExplorerItem;
  AMode: TdxPSStreamMode): TStream;
begin
  Result := CreateDataStream((AnItem as TdxPSFileBasedExplorerItem)
    .FullQualifiedFileName, AMode);
end;
...
procedure TdxPSFileBasedExplorer.DoLoadData(AFolder: TdxPSExplorerFolder);
  ...
  begin
    ...
    try
      ...
      try
        AReportDocument := TBasedxReportLink.ExtractReportDocument
          (AStream, True);
        try
  ...
  begin
    Result := HasExtension(AName, '.' + FileExtension) and
      DoFilterLinkClass(AName);
  end;
      ...
      try
        repeat
          if not IsSystemObject(SR) then
            Result := (SR.Attr and faDirectory <> 0) or
              IsValidReportFile(APath + SR.Name);
            ...
            try
              ...
              begin
                ...
                if eoLoadAll in Options then
                  ...
                  TdxPSExplorerFolderHelper.SetHasChildren(AFolder,
                    HasChildren(AFileName));
              end;
            except
              DoLoadError(SR.Name);
            end;
...
begin
  LoadDirectory(AFolder, TdxPSFileBasedExplorerFolder(AFolder)
    .FullQualifiedDirName);
end;
...
class function TdxPSFileBasedExplorer.GetRootFolderClass
...
begin
  if not AnIOResult then
    ProcessIOError;
end;

procedure TdxPSFileBasedExplorer.ProcessIOError(AnUnconditionalRaiseException
  : Boolean = False);
begin
...
procedure TdxPSFileBasedExplorer.SetActiveFolder
  (Value: TdxPSFileBasedExplorerFolder);
begin
  inherited ActiveFolder := Value;
end;

procedure TdxPSFileBasedExplorer.SetOptions
