...
{$ENDIF}
  ...
  Forms, Dialogs, Menus, StdCtrls, cxClasses,
  cxPC, dxSpreadSheetCore, dxSpreadSheetStrs, cxGraphics, cxControls,
  ...
  cxSpinEdit, cxCheckBox, Generics.Defaults, Generics.Collections,
  dxSpreadSheetFormatCellsDialog, ExtCtrls, cxFontNameComboBox, cxColorComboBox,
  cxCheckListBox,
  dxGalleryControl, dxColorGallery, cxImageComboBox, dxColorDialog, ImgList,
  dxSpreadSheetGraphics, dxCore,
  dxSpreadSheetClasses, dxSpreadSheetTypes, cxImage, cxButtonEdit, ComCtrls,
  cxTreeView, dxSpreadSheetHyperlinks,
...
type
  ...
  TdxSpreadSheetEditHyperlinkDialogFormClass = class of
  ...
  TdxSpreadSheetEditHyperlinkDialogForm = class(TdxForm)
    ...
    procedure edtAddressPropertiesButtonClick(Sender: TObject;
  ...
  strict private
    ...
    function GetPageLink(APage: TdxSpreadSheetHyperlinkValueType)
      : TcxCustomEdit;
  ...
  protected
    ...
    property DisplayTextChanged: Boolean read FDisplayTextChanged
      write FDisplayTextChanged;
  ...
  public
    ...
    property ActivePage: TdxSpreadSheetHyperlinkValueType read FActivePage
      write SetActivePage;
...
var
  dxSpreadSheetEditHyperlinkDialogClass
    : TdxSpreadSheetEditHyperlinkDialogFormClass =
    TdxSpreadSheetEditHyperlinkDialogForm;

procedure ShowHyperlinkEditorDialog(ASheet: TdxSpreadSheetTableView;
  AHyperlink: TdxSpreadSheetHyperlink = nil);
...
type
  ...
  public
    constructor Create(AOwner: TdxSpreadSheetEditHyperlinkDialogForm);
      reintroduce;
...
begin
  ADialogForm := dxSpreadSheetEditHyperlinkDialogClass.Create
...
constructor TdxSpreadSheetEditingHyperlink.Create
  (AOwner: TdxSpreadSheetEditHyperlinkDialogForm);
begin
  FOwner := AOwner;
  inherited Create(AOwner.Sheet.Hyperlinks);
end;

function TdxSpreadSheetEditingHyperlink.CreateData
...
begin
  ...
  try
    ...
    edtTextToDisplay.Enabled := (SelectedCell = ActiveCell) and
      ((ActiveCell = nil) or (ActiveCell.DataType in [cdtBlank, cdtString]) and
      lcEditTextToDisplay.Visible);
    ...
    case EditingHyperlink.ValueType of
      ...
      hvtEMail:
        begin
          edtSubject.Text := TdxSpreadSheetEditingHyperlink(EditingHyperlink)
            .MailSubject;
          edtEmail.Text := dxEMailHyperlinkMailtoPrefix +
            TdxSpreadSheetEditingHyperlink(EditingHyperlink).MailAddress;
      ...
      hvtReference:
        begin
          ...
          if TdxSpreadSheetEditingHyperlink(EditingHyperlink).ReferenceName = nil
          then
            edtCellRef.Text := GetReferenceText;
        end;
    end;

    if ((tvDocumentPlace.Selected = nil) or (tvDocumentPlace.Selected.Level = 0)
      ) and (GetSelectedNode <> nil) then
      GetSelectedNode.Selected := True;
  finally
    Modified := False;
    DisplayTextChanged := (edtTextToDisplay.Text <> '') and
...
begin
  TdxSpreadSheetHyperlinksAccess(Sheet.Hyperlinks)
    .AssignChanges(EditingHyperlink, Hyperlink);
end;
...
begin
  ...
  lbFileOrWebPage.Caption := cxGetResourceString
    (@sdxHyperlinkEditorFileOrWebPageLink);
  lbPlaceInDocument.Caption := cxGetResourceString
    (@sdxHyperlinkEditorPlaceInThisDocumentLink);
  lbEMail.Caption := cxGetResourceString(@sdxHyperlinkEditorEmailAddressLink);

  lcEditTextToDisplay.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorTextToDisplay);
  lcEditScreenTip.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorScreenTip);
  lcEditAddress.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorAddress);
  lcEditEMailAddress.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorEmailAddress);
  lcEditSubject.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorSubject);
  lcEditCellRef.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorCellReference);
  lcEditPlaceInDocument.CaptionOptions.Text :=
    cxGetResourceString(@sdxHyperlinkEditorSelectPlace);
end;

function TdxSpreadSheetEditHyperlinkDialogForm.CreateEditingHyperlink
  : TdxSpreadSheetHyperlink;
begin
  Result := TdxSpreadSheetEditingHyperlink.Create(Self);
end;

procedure TdxSpreadSheetEditHyperlinkDialogForm.EditValueChanged
  (Sender: TObject);
begin
  if IsUpdating then
    Exit;
  ...
  lcbtnOK.Enabled := TdxSpreadSheetEditingHyperlink(EditingHyperlink)
    .IsHyperlinkValid;
end;

procedure TdxSpreadSheetEditHyperlinkDialogForm.edtAddressPropertiesButtonClick
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.edtAddressPropertiesChange
  (Sender: TObject);
var
  APos: Integer;
begin
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.edtEmailPropertiesChange
  (Sender: TObject);
var
  APos: Integer;
begin
  ...
  begin
    APos := TcxTextEdit(Sender).SelStart;
    TcxTextEdit(Sender).Text := dxEMailHyperlinkMailtoPrefix +
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.edtScreenTipPropertiesChange
  (Sender: TObject);
begin
  if IsUpdating then
    Exit;
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.EditDisplayTextChanged
  (Sender: TObject);
begin
  if IsUpdating then
    Exit;
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.SetEditFontStyle
  (AEdit: TcxCustomEdit; AStyles: TFontStyles);
begin
  if AEdit = nil then
    Exit;
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.tvDocumentPlaceChange
...
begin
  ...
  else if ActiveContainer <> nil then
    TdxSpreadSheetEditingHyperlink(FEditingHyperlink).SetArea(cxInvalidRect)
  else
    TdxSpreadSheetEditingHyperlink(FEditingHyperlink)
      .SetArea(Sheet.Selection.Area);

  cxImage1.Style.BorderStyle := ebsNone;
  cxImage2.Style.BorderStyle := ebsNone;
  cxImage3.Style.BorderStyle := ebsNone;
  SetupControls;
  if TdxSpreadSheetEditingHyperlink(EditingHyperlink).ValueType = hvtReference
  then
    edtCellRef.Text := GetReferenceText
  else
    edtCellRef.Text := dxReferenceToString(cxNullRect,
      SpreadSheet.OptionsView.R1C1Reference);
  Load;
  Modified := False;
  btnOK.Enabled := TdxSpreadSheetEditingHyperlink(EditingHyperlink)
    .IsHyperlinkValid;
  ApplyLocalization;
end;
...
begin
  ...
  try
    ...
    FSheets := tvDocumentPlace.Items.AddChild(nil,
      cxGetResourceString(@sdxHyperlinkEditorCellReferenceNode));
    FNames := tvDocumentPlace.Items.AddChild(nil,
      cxGetResourceString(@sdxHyperlinkEditorDefinedNamesNode));

    for I := 0 to SpreadSheet.SheetCount - 1 do
    begin
      if SpreadSheet.Sheets[I] is TdxSpreadSheetTableView then
        tvDocumentPlace.Items.AddChildObject(FSheets,
          CheckCaption(SpreadSheet.Sheets[I].Caption), SpreadSheet.Sheets[I]);
    end;
...
function TdxSpreadSheetEditHyperlinkDialogForm.GetActiveCell
  : TdxSpreadSheetCell;
begin
  Result := Sheet.Cells[Area.Top, Area.Left];
end;

function TdxSpreadSheetEditHyperlinkDialogForm.GetActiveContainer
...
begin
  ...
  else if Pos(dxEMailHyperlinkMailtoPrefix, Result) = 1 then
    Delete(Result, 1, Length(dxEMailHyperlinkMailtoPrefix));
end;

function TdxSpreadSheetEditHyperlinkDialogForm.GetPageLink
  (APage: TdxSpreadSheetHyperlinkValueType): TcxCustomEdit;
begin
  case APage of
    hvtReference:
...
begin
  Result := dxReferenceToString(TdxSpreadSheetEditingHyperlink(EditingHyperlink)
    .ReferenceArea, SpreadSheet.OptionsView.R1C1Reference);
end;

function TdxSpreadSheetEditHyperlinkDialogForm.GetSelectedCell
  : TdxSpreadSheetCell;
begin
...
begin
  ...
  begin
    if TdxSpreadSheetEditingHyperlink(EditingHyperlink).ReferenceName <> nil
    then
      Result := TryGetChild(FNames,
        TdxSpreadSheetDefinedName(TdxSpreadSheetEditingHyperlink
        (EditingHyperlink).ReferenceName).Index)
    else if TdxSpreadSheetEditingHyperlink(EditingHyperlink).ReferenceSheet <> nil
    then
      Result := TryGetChild(FSheets,
        TdxSpreadSheetTableView(TdxSpreadSheetEditingHyperlink(EditingHyperlink)
        .ReferenceSheet).Index)
    else
      Result := FSheets[0];
  end;
end;

function TdxSpreadSheetEditHyperlinkDialogForm.GetSpreadSheet
  ...
  case ActivePage of
    hvtReference:
      begin
        ...
        if tvDocumentPlace.Selected <> nil then
        begin
          if TObject(tvDocumentPlace.Selected.Data) is TdxSpreadSheetDefinedName
          then
            Result := CheckExtraChars
              (TdxSpreadSheetDefinedName
              (tvDocumentPlace.Selected.Data).ToString)
          else if TObject(tvDocumentPlace.Selected.Data) is TdxSpreadSheetCustomView
          then
            Result := CheckExtraChars
              (TdxSpreadSheetTableViewAccess(tvDocumentPlace.Selected.Data)
              .Caption) + '!' + Result;
        end;
    ...
    hvtEMail:
      begin
        ...
        if Trim(edtSubject.Text) <> '' then
          Result := Result + dxEMailHyperlinkSubjectPrefix +
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.SetActivePage
  (const AValue: TdxSpreadSheetHyperlinkValueType);
begin
  lcMain.BeginUpdate;
  try
      ...
      case ActivePage of
        ...
        hvtReference:
          begin
            ...
            btnOK.Enabled := (tvDocumentPlace.Selected <> nil) and
              (tvDocumentPlace.Selected.Level = 1);
          end;
      else
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.SetHotTrackPage
  (AValue: Integer);
begin
  if AValue = HottrackPage then
    Exit;
  if (FHottrackPage >= 0) and (FHottrackPage <> Byte(FActivePage)) then
    SetEditFontStyle(GetPageLink(TdxSpreadSheetHyperlinkValueType
      (HottrackPage)), []);
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.ChangeFocus
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.NavbarItemClick
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetEditHyperlinkDialogForm.NavbarItemMouseEnter
  (Sender: TObject);
begin
  HottrackPage := TComponent(Sender).Tag;
end;

procedure TdxSpreadSheetEditHyperlinkDialogForm.NavbarItemMouseLeave
  (Sender: TObject);
var
