...
type
  ...
  public
    ...
    property WidthInHundredthsOfMillimeter: Integer
      read GetWidthInHundredthsOfMillimeter;
    property HeightInEmus: Integer read FHeightInEmus write FHeightInEmus;
    property HeightInHundredthsOfMillimeter: Integer
      read GetHeightInHundredthsOfMillimeter;
  ...
  public
    class function FromStream(AReader: TBinaryReader)
      : TdxOfficeArtRecordHeader; static;
    procedure Write(AWriter: TBinaryWriter);
  ...
  protected
    ...
    procedure LoadImageFromStream(AImageStream: TdxMemoryStream;
      const AImageCreator: IdxDocOfficeImageCreator); virtual;
  ...
  public
    ...
    constructor Create(AReader: TBinaryReader;
      AHeader: TdxOfficeArtRecordHeader;
      const AImageCreator: IdxDocOfficeImageCreator); overload; virtual;
  ...
  protected
    ...
    property HasDelayedStream: Boolean read FHasDelayedStream
      write FHasDelayedStream;
  public
    constructor Create(AReader: TBinaryReader; AEmbeddedReader: TBinaryReader;
      AHeader: TdxOfficeArtRecordHeader;
      const AImageCreator: IdxDocOfficeImageCreator); overload;
    constructor Create(AImage: TdxOfficeImageReference;
      AHasDelayedStream: Boolean); overload;
    destructor Destroy; override;
    function Write(AWriter: TBinaryWriter): TdxMD4Byte16; overload; override;
    procedure Write(AWriter: TBinaryWriter;
      AEmbeddedWriter: TBinaryWriter); overload;
  ...
  strict private
  class var
    TypeCodeTranslationTable: array [FirstBlipTypeCode .. LastBlipTypeCode]
      of TdxBlipBaseClass;
    ImageFormatTranslationTable: array [TdxOfficeImageFormat]
      of TdxBlipBaseClass;
  strict private
    class constructor Initialize;
  public
    class function ReadAllBlips(AReader: TBinaryReader; AEndPosition: Int64;
      const AImageCreator: IdxDocOfficeImageCreator): TdxList<TdxBlipBase>;
      overload; static;
    class function ReadAllBlips(AReader: TBinaryReader;
      AEmbeddedReader: TBinaryReader; AEndPosition: Int64;
      const AImageCreator: IdxDocOfficeImageCreator): TdxList<TdxBlipBase>;
      overload; static;
    ...
    class function CreateBlipFromStream(AReader: TBinaryReader;
      ...
      const AImageCreator: IdxDocOfficeImageCreator): TdxBlipBase;
      overload; static;
    class function CreateBlipFromStreamCore(AReader: TBinaryReader;
      AEmbeddedReader: TBinaryReader; AHeader: TdxOfficeArtRecordHeader;
      const AImageCreator: IdxDocOfficeImageCreator): TdxBlipBase; static;
    class function CreateBlipFromImage(AImage: TdxOfficeImageReference)
      : TdxBlipBase; static;
  end;
...
type
  ...
  protected
    ...
    property HorizontalPixelsPerMeter: Integer read FHorizontalPixelsPerMeter
      write FHorizontalPixelsPerMeter;
    property VerticalPixelsPerMeter: Integer read FVerticalPixelsPerMeter
      write FVerticalPixelsPerMeter;
    property UsedColors: Integer read FUsedColors write FUsedColors;
    property ImportantColors: Integer read FImportantColors
      write FImportantColors;
  public
    constructor Create;
  ...
  public
    class function CreateBmpFileStreamForDib(ADibStream: TStream;
      ADibHeight: Integer; ABytesInLine: Integer): TdxMemoryStream; static;
  end;
...
class function TdxDibHelper.CreateBmpFileStreamForDib(ADibStream: TStream;
  ADibHeight: Integer; ABytesInLine: Integer): TdxMemoryStream;
var
...
begin
  ...
  try
    ...
    AOffset := ADibDataSize - ABitsSize +
      TdxBitmapFileHelper.sizeofBITMAPFILEHEADER;
...
class function TdxDocMetafileHeader.FromStream(AReader: TBinaryReader)
  : TdxDocMetafileHeader;
begin
  Result := TdxDocMetafileHeader.Create;
  Result.Read(AReader);
...
procedure TdxDocMetafileReader.Read(AReader: TBinaryReader;
  const AImageCreator: IdxDocOfficeImageCreator);
var
  ABuffer: TBytes;
...
begin
  ...
  begin
    ...
    ACompressedStream := TdxMemoryStream.Create(ABuffer, 2,
      FMetafileHeader.CompressedSize - 6);
    ABuffer := nil;
    try
      AUncompressedStream := TdxMemoryStream.Create;
      try
        TdxInflateHelper.Decompress(ACompressedStream, ACompressedStream.Size,
          AUncompressedStream);
        ...
        ANativeMetafileImage := AImageCreator.CreateMetafile
          (AUncompressedStream, TdxMapMode.Anisotropic, AClipWidth,
          AClipHeight);

        if ANativeMetafileImage.Image is TdxOfficeMetafile then
          TdxOfficeMetafile(ANativeMetafileImage.Image)
            .MetafileSizeInHundredthsOfMillimeter :=
            TSize.Create(FMetafileHeader.WidthInHundredthsOfMillimeter,
            FMetafileHeader.HeightInHundredthsOfMillimeter);
...
begin
  if (Length(ABuffer) < 3) or (ABuffer[0] and $0F <> 8) or
    (ABuffer[1] and $20 <> 0) then
    TdxOfficeArtExceptions.ThrowInvalidContent;
end;

{ TdxOfficeArtRecordHeader }

class function TdxOfficeArtRecordHeader.FromStream(AReader: TBinaryReader)
  : TdxOfficeArtRecordHeader;
begin
...
constructor TdxBlipBase.Create(AReader: TBinaryReader;
  AHeader: TdxOfficeArtRecordHeader;
  const AImageCreator: IdxDocOfficeImageCreator);
begin
  Assert(AReader <> nil);
...
procedure TdxBlipBase.Read(AReader: TBinaryReader;
  AHeader: TdxOfficeArtRecordHeader;
  const AImageCreator: IdxDocOfficeImageCreator);
var
...
procedure TdxBlipBase.LoadImageFromStream(AImageStream: TdxMemoryStream;
  const AImageCreator: IdxDocOfficeImageCreator);
begin
...
begin
  Result := TdxOfficeArtRecordHeader.Size + MD4MessageSize + TagSize +
    ImageBytesLength;
end;
...
constructor TdxFileBlipStoreEntry.Create(AImage: TdxOfficeImageReference;
  AHasDelayedStream: Boolean);
begin
  FRefCount := 1;
...
constructor TdxFileBlipStoreEntry.Create(AReader: TBinaryReader;
  AEmbeddedReader: TBinaryReader; AHeader: TdxOfficeArtRecordHeader;
  const AImageCreator: IdxDocOfficeImageCreator);
begin
  FRefCount := 1;
...
procedure TdxFileBlipStoreEntry.Read(AReader: TBinaryReader;
  AHeader: TdxOfficeArtRecordHeader;
  const AImageCreator: IdxDocOfficeImageCreator);
begin
...
begin
  ...
  if EmbeddedReader <> AReader then
    EmbeddedReader.BaseStream.Seek(FEmbeddedBlipOffset,
      TSeekOrigin.soBeginning);
  AEmbeddeBlipHeader := TdxOfficeArtRecordHeader.FromStream(FEmbeddedReader);
  try
    FEmbeddedBlip := TdxBlipFactory.CreateBlipFromStream(FEmbeddedReader,
      AEmbeddeBlipHeader, AImageCreator);
  finally
...
procedure TdxFileBlipStoreEntry.Write(AWriter: TBinaryWriter;
...
class function TdxBlipFactory.ReadAllBlips(AReader: TBinaryReader;
  AEndPosition: Int64; const AImageCreator: IdxDocOfficeImageCreator)
  : TdxList<TdxBlipBase>;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      ABlip := CreateBlipFromStream(AReader, AEmbeddedReader, ABlipHeader,
        AImageCreator);
      if ABlip <> nil then
        Result.Add(ABlip);
    finally
    ...
    AReader.BaseStream.Position := Max(AExpectedBlipEnd,
      AReader.BaseStream.Position);
  end;
...
class function TdxBlipFactory.CreateBlipFromStream(AReader: TBinaryReader;
  ...
  const AImageCreator: IdxDocOfficeImageCreator): TdxBlipBase;
begin
  Result := CreateBlipFromStreamCore(AReader, AEmbeddedReader, AHeader,
    AImageCreator);
end;
...
begin
  ...
  case ATypeCode of
    TdxBlipTypeCodes.FileBlipStoreEntry:
      Result := TdxFileBlipStoreEntry.Create(AReader, AEmbeddedReader, AHeader,
        AImageCreator);
...
class function TdxBlipFactory.CreateBlipFromImage
  (AImage: TdxOfficeImageReference): TdxBlipBase;
var
  ABlipClass: TdxBlipBaseClass;
begin
...
procedure TdxBlipMetaFile.Read(AReader: TBinaryReader;
  AHeader: TdxOfficeArtRecordHeader;
  const AImageCreator: IdxDocOfficeImageCreator);
var
  AEnd: Int64;
...
begin
  ...
  try
    ...
    if (AReader.BaseStream.Position <> AEnd) and
      (AReader.BaseStream.Size >= AEnd) then
...
begin
  Result := TdxOfficeArtRecordHeader.Size + MD4MessageSize + MetafileHeaderSize
    + ImageBytesLength;
end;
...
procedure TdxBlipPict.Read(AReader: TBinaryReader;
  AHeader: TdxOfficeArtRecordHeader;
  const AImageCreator: IdxDocOfficeImageCreator);
var
  AUidOffset: Integer;
...
begin
  ...
  try
    if AMetafileHeader.Compressed then
      AReader.BaseStream.Seek(AMetafileHeader.CompressedSize,
        TSeekOrigin.soCurrent)
    else
      AReader.BaseStream.Seek(AMetafileHeader.UncompressedSize,
        TSeekOrigin.soCurrent);
  finally
    AMetafileHeader.Free;
  end;
...
begin
  ...
  try
    ...
    try
      ...
      ABMPStream := TdxDibHelper.CreateBmpFileStreamForDib(AImageStream,
        AHeader.Height, ABytesInLine);
      try
...
begin
  if (AUidInfo = TdxMessageDigestCodes.JpegRGBSingleMessage) or
    (AUidInfo = TdxMessageDigestCodes.JpegCMYKSingleMessage) then
    Exit(MD4MessageSize + TagSize);
  if (AUidInfo = TdxMessageDigestCodes.JpegRGBDoubleMessage) or
    (AUidInfo = TdxMessageDigestCodes.JpegCMYKDoubleMessage) then
    Exit((MD4MessageSize * 2) + TagSize);
