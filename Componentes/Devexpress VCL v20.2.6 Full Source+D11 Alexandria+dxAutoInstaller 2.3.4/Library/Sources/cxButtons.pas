...
uses
  ...
  dxCore, dxMessages, dxThemeManager, cxGeometry, cxGraphics, cxLookAndFeels,
  dxGDIPlusClasses,
  cxLookAndFeelPainters, cxControls, cxContainer, cxClasses, dxFading,
...
type
  ...
  TcxButtonKind = (cxbkStandard, cxbkDropDown, cxbkDropDownButton,
    cxbkCommandLink, cxbkOfficeDropDown);
  TcxButtonPaintStyle = (bpsDefault, bpsCaption, bpsGlyph);

  TcxButtonAssignedColor = (cxbcDefault, cxbcDefaultText, cxbcDisabled,
    cxbcDisabledText, cxbcHot, cxbcHotText, cxbcNormal, cxbcNormalText,
    cxbcPressed, cxbcPressedText);
  ...
  TcxButtonInternalState = (bisDown, bisHot, bisPressed, bisMenuButtonPressed,
  ...
  private
    ...
    function ButtonStateToButtonAssignedColor(AState: TcxButtonState;
  ...
  published
    property AssignedColors: TcxButtonAssignedColors read FAssignedColors
      write SetAssignedColors stored False;
    property Default: TColor index Ord(cxbcDefault)read GetColor write SetColor
      stored IsColorStored;
    property DefaultText: TColor index Ord(cxbcDefaultText)read GetColor
      write SetColor stored IsColorStored;
    property Normal: TColor index Ord(cxbcNormal)read GetColor write SetColor
      stored IsColorStored;
    property NormalText: TColor index Ord(cxbcNormalText)read GetColor
      write SetColor stored IsColorStored;
    property Hot: TColor index Ord(cxbcHot)read GetColor write SetColor
      stored IsColorStored;
    property HotText: TColor index Ord(cxbcHotText)read GetColor write SetColor
      stored IsColorStored;
    property Pressed: TColor index Ord(cxbcPressed)read GetColor write SetColor
      stored IsColorStored;
    property PressedText: TColor index Ord(cxbcPressedText)read GetColor
      write SetColor stored IsColorStored;
    property Disabled: TColor index Ord(cxbcDisabled)read GetColor
      write SetColor stored IsColorStored;
    property DisabledText: TColor index Ord(cxbcDisabledText)read GetColor
      write SetColor stored IsColorStored;
  end;
  ...
  protected
    ...
    procedure UpdateGroupValues(const AGroupIndex: Integer;
      const ASpeedButtonOptions: TcxSpeedButtonOptions);
  ...
  published
    ...
    property AllowAllUp: Boolean read FAllowAllUp write SetAllowAllUp
      default False;
    property Down: Boolean read GetDown write SetDown stored IsDownStored
      default False;
  ...
  public
    procedure DrawButton(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState; AColor: TColor; APart: TcxButtonPart); virtual;
    procedure DrawDropDownArrow(ACanvas: TcxCanvas; const R: TRect;
    ...
    // Text
    procedure DrawText(ACanvas: TcxCanvas; const AText: TCaption; R: TRect;
      ADrawTextFlags: Integer); overload; virtual;
    procedure DrawText(ACanvas: TcxCanvas; const AText: TCaption; R: TRect;
      AState: TcxButtonState; ADrawTextFlags: Integer); overload; virtual;
    function GetBiDiFlags: Integer;
    function GetDescriptionTextColor(AState: TcxButtonState;
      ADefaultColor: TColor = clWindowText): TColor; virtual;
    ...
    // SpeedButton
    procedure DrawSpeedButton(ACanvas: TcxCanvas; R: TRect;
      AState: TcxButtonState; AColor: TColor; APart: TcxButtonPart); virtual;
    //
  ...
  protected
    procedure CalculateLayout(const AContentRect: TRect;
      AMargin, ASpacing: Integer); virtual;
  public
  ...
  protected
    ...
    procedure CalculateMargins(const R: TRect;
      var AMargin, ASpacing: Integer); virtual;
  ...
  TcxButtonOfficeDropDownLayoutCalculator = class
    (TcxButtonStandardLayoutCalculator)
  strict private
    function GetScaleFactor: TdxScaleFactor;
  protected
  ...
  protected
    ...
    function CreateLayoutCalculator(AKind: TcxButtonKind)
      : TcxButtonCustomLayoutCalculator; virtual;
    ...
    property OptionsImage: TcxButtonImageOptions read GetOptionsImage;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
    property SpeedButtonOptions: TcxSpeedButtonOptions
      read GetSpeedButtonOptions;
  public
    constructor Create(AOwner: TcxCustomButton); override;
    destructor Destroy; override;
    ...
    function GetContentRect(AState: TcxButtonState = cxbsDefault)
      : TRect; virtual;
  ...
  TcxCustomButton = class(TcxBaseButton, IcxMouseTrackingCaller, IdxSkinSupport,
    IcxLookAndFeelContainer, IdxFadingObject, IdxScaleFactor,
  ...
  private
    ...
    procedure CMCloseUpPopupMenu(var Message: TMessage);
      message DXM_CLOSEUPPOPUPMENU;
    procedure CMDialogChar(var Message: TCMDialogChar); message CM_DIALOGCHAR;
    procedure CMDropDownPopupMenu(var Message: TMessage);
      message DXM_DROPDOWNPOPUPMENU;
    ...
    procedure CNCtlColorBtn(var Message: TWMCtlColorBtn);
      message CN_CTLCOLORBTN;
...
{$ENDIF}
    ...
    procedure DoGetDrawParams(AState: TcxButtonState; var AColor: TColor;
      AFont: TFont); virtual;
    function DoDropDownMenuPopup(var APopupMenu: TComponent): Boolean; virtual;
    function DoOnDropDownMenuPopup(var APopupMenu: TPopupMenu)
      : Boolean; virtual;
    function DoOnDropDownMenuPopupEx(var APopupMenu: TComponent)
      : Boolean; virtual;
    function DoShowPopupMenu(APopupMenu: TComponent; X, Y: Integer)
      : Boolean; virtual;
    ...
    // CustomDraw
    function DoCustomDraw(ACanvas: TcxCanvas;
      AViewInfo: TcxButtonViewInfo): Boolean;
    ...
    property AutoSize: Boolean read FAutoSize write SetButtonAutoSize
      default False;
    property Colors: TcxButtonColors read GetColors write SetColors
      stored IsColorsStored;
    property CommandLinkHint: string read GetCommandLinkHint
      write SetCommandLinkHint; // deprecated, use Description instead
    property Description: string read FDescription write SetDescription;
    property DropDownMenu: TComponent read FDropDownMenu write SetDropDownMenu;
    property InternalStates: TcxButtonInternalStates read FInternalStates
      write SetInternalStates;
    property IsFocused: Boolean read FIsFocused;
    property Kind: TcxButtonKind read FKind write SetKind default cxbkStandard;
    property OptionsImage: TcxButtonImageOptions read FOptionsImage
      write SetOptionsImage;
    property PaintStyle: TcxButtonPaintStyle read FPaintStyle
      write SetPaintStyle default bpsDefault;
    ...
    property UseSystemPaint: Boolean read FUseSystemPaint write FUseSystemPaint
      default False; // deprecated

    // speed button support
    property AllowAllUp: Boolean read GetAllowAllUp write SetAllowAllUp
      default False;
    property CanBeFocused: Boolean read GetCanBeFocused write SetCanBeFocused
      default True;
    ...
    property SpeedButtonOptions: TcxSpeedButtonOptions read FSpeedButtonOptions
      write SetSpeedButtonOptions;

    // CustomDrawEvents
    property OnCustomDraw: TcxButtonCustomDrawEvent read FOnCustomDraw
      write FOnCustomDraw;

    property OnDropDownMenuPopup: TcxButtonDropDownMenuPopupEvent
      read FOnDropDownMenuPopup write FOnDropDownMenuPopup;
    property OnDropDownMenuPopupEx: TcxButtonDropDownMenuPopupExEvent
      read FOnDropDownMenuPopupEx write FOnDropDownMenuPopupEx;
    property OnGetDrawParams: TcxButtonGetDrawParamsEvent read FOnGetDrawParams
      write FOnGetDrawParams;
...
function GetButtonPainter(ALookAndFeel: TcxLookAndFeel)
  ...
  begin
    for AState := Low(AState) to High(AState) do
      if not(AState in Value) then
        FColors[AState] := clDefault
      else if FColors[AState] = clDefault then
...
function TcxButtonColors.ButtonStateToButtonAssignedColor
  (AState: TcxButtonState; AIsTextColor: Boolean): TcxButtonAssignedColor;
const
  StateToAssignedColor: array [TcxButtonState, Boolean]
    of TcxButtonAssignedColor = ((cxbcDefault, cxbcDefaultText),
    (cxbcNormal, cxbcNormalText), (cxbcHot, cxbcHotText),
...
begin
  ...
  else if AButtonColor = cxbcNormal then
    Result := FColors[cxbcDefault]
  else
    Result := FColors[cxbcNormal];
end;

function TcxButtonColors.GetTextColorByState(const AState
  : TcxButtonState): TColor;
var
  AButtonColor: TcxButtonAssignedColor;
begin
  ...
  if AButtonColor in AssignedColors then
    Result := FColors[AButtonColor]
  else if AButtonColor = cxbcNormalText then
...
begin
  ANumGlyphs := 1;
  if (FGlyph <> nil) and (FGlyph.SourceHeight > 0) and
    (FGlyph.SourceWidth mod FGlyph.SourceHeight = 0) then
  begin
...
begin
  Result := IsGlyphAssigned(Glyph) or cxGraphics.IsImageAssigned(Images,
    ImageIndex);
end;
...
procedure TcxSpeedButtonOptions.InternalSetDown(AValue: Boolean);
  ...
  begin
    Result := (AValue <> Down) and
      ((not Down or FAllowAllUp or (FGroupIndex = 0) or (LockCount <> 0)) or
      Assigned(Button.Action) or (Button.SpeedButtonMode and
      Button.IsDesigning));
  end;
...
procedure TcxSpeedButtonOptions.UpdateGroupValues(const AGroupIndex: Integer;
  const ASpeedButtonOptions: TcxSpeedButtonOptions);
begin
  BeginUpdate;
  try
    ...
    if ASpeedButtonOptions.Down and Down and
      (not Assigned(Button.Action) or
      (Button.Action <> ASpeedButtonOptions.Button.Action)) then
    begin
      Down := False;
...
begin
  Result := (Button.ActionLink = nil) or not TControlActionLinkAccess
    (Button.ActionLink).IsCheckedLinked;
end;

function TcxSpeedButtonOptions.IsGroupIndexStored: Boolean;
begin
  Result := (Button.ActionLink = nil) or
    (TCustomAction(Button.Action).GroupIndex <> FGroupIndex);
end;
...
var
  AMsg: TMessage;
begin
  if (LockCount = 0) and Active and (FGroupIndex <> 0) and (Button.Parent <> nil)
  then
  begin
...
begin
  ...
  FShowFocusRect := True;
  DoubleBuffered := True;
  ControlStyle := ControlStyle + [{$IFDEF DELPHI16} csOverrideStylePaint,
  {$ENDIF} csReflector, csOpaque];
end;
...
function TcxCustomButton.DoCustomDraw(ACanvas: TcxCanvas;
  AViewInfo: TcxButtonViewInfo): Boolean;
begin
  Result := False;
...
function TcxCustomButton.DoOnDropDownMenuPopup(var APopupMenu
  : TPopupMenu): Boolean;
begin
  Result := False;
...
function TcxCustomButton.DoOnDropDownMenuPopupEx(var APopupMenu
  : TComponent): Boolean;
begin
  Result := False;
...
begin
  ...
  if SpeedButtonMode and not Assigned(Action) and CanBeFocused and
    AExecuteClick and not(bisAccelExecute in InternalStates) then
    SetDown(not Down);
...
begin
  Result := (Painter.LookAndFeelStyle in [lfsSkin, lfsNative]) and
    (IsWinVistaOrLater or (Kind <> cxbkCommandLink)) and CanHotTrack and
    not Assigned(FOnCustomDraw);
end;
...
begin
  ...
  try
    ...
    Result.cy := MulDiv(TdxTextMeasurer.TextLineHeight(ACanvas.Handle), 14, 8);
  finally
    ACanvas.Free;
  end;
end;
...
procedure TcxCustomButton.ScaleFactorChangeHandler(Sender: TObject;
...
begin
  if (Kind in [cxbkStandard, cxbkCommandLink]) or
    not PtInRect(GetMenuButtonBounds, P) then
...
procedure TcxCustomButton.CheckStartFading(APrevButtonState,
  AButtonState: TcxButtonState);
begin
  if APrevButtonState <> AButtonState then
  begin
    if (APrevButtonState in [cxbsDefault, cxbsNormal]) and
      (AButtonState = cxbsHot) then
      dxFader.FadeIn(Self)
    else if (AButtonState in [cxbsDefault, cxbsNormal]) and
      (APrevButtonState = cxbsHot) then
...
begin
  if not Enabled then
    Result := cxbsDisabled
  else if FIsPaintPressed then
    Result := cxbsPressed
  else if (bisHot in InternalStates) and CanHotTrack then
    Result := cxbsHot
  else if FIsPaintDefault then
    Result := cxbsDefault
  else
    Result := cxbsNormal;
end;
...
begin
  case Kind of
    cxbkStandard:
      Result := cxNullRect;
    cxbkDropDownButton:
      Result := cxRectSetRight(ClientRect, ClientRect.Right,
        ViewInfo.Painter.GetDropDownButtonSize);
  else
    Result := ClientRect;
  end;
end;
...
begin
  Result := (SpeedButtonOptions <> nil) and SpeedButtonOptions.CanBeFocused or
    not AllowSpeedButtonOptions;
end;
...
begin
  if SpeedButtonMode and not IsDesigning and not CanBeFocused then
    case Message.Msg of
      WM_LBUTTONDOWN:
        MouseDown(mbLeft, KeysToShiftState(Message.WParam), Message.LParamLo,
          Message.LParamHi);
      WM_LBUTTONDBLCLK:
        DblClick;
    else
...
begin
  if (Message.NotifyCode <> BN_CLICKED) or (Kind <> cxbkDropDownButton) or GetExecuteClick
  then
    inherited;
end;
...
function TcxCustomButton.DoDropDownMenuPopup(var APopupMenu
  : TComponent): Boolean;
begin
...
procedure TcxCustomButton.GetFadingImages(out AFadeOutImage,
  ...
  begin
    if AHotTracked then
      Result := cxbsHot
    else if CanBeFocused and IsFocused then
      Result := cxbsDefault
    else
      Result := cxbsNormal;
  end;
...
begin
  Result := not StandardButton and Painter.IsButtonHotTrack or
    (Kind = cxbkCommandLink) and Enabled;
end;
...
procedure TcxCustomButton.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if not CanBeFocused then
    SetCapture(Handle);
...
begin
  ...
  AExecuteClick := not CanBeFocused and (Button = mbLeft) and
    (bisHot in InternalStates) and ((bisPressed in InternalStates) or
    ((bisMenuButtonPressed in InternalStates) and (Kind in [cxbkDropDown,
    cxbkOfficeDropDown])));
...
procedure TcxCustomButton.DrawItem(const DrawItemStruct: TDrawItemStruct);
  ...
  begin
    ...
    begin
      ARect := ClientRect;
      APaintBuffer := BeginBufferedPaint(DrawItemStruct.HDC, @ARect,
        BPBF_COMPOSITED, nil, AMemDC);
    ...
    begin
      ...
      try
        ...
        try
          ...
          cxBitBlt(DrawItemStruct.HDC, ABitmap.Canvas.Handle, ClientRect,
            cxNullPoint, SRCCOPY);
        finally
...
begin
  APrevButtonState := GetButtonState;
  FIsPaintPressed := ((DrawItemStruct.itemState and ODS_SELECTED <> 0) and
    not(bisMenuButtonPressed in InternalStates)) or
    ((bisMenuVisible in InternalStates) and
    (Kind in [cxbkDropDown, cxbkOfficeDropDown])) or
...
procedure TcxCustomButton.UpdateImageInfo;
  ...
  begin
    ...
    else if (Action is TCustomAction) and
      (TCustomAction(Action).ActionList <> nil) then
...
begin
  ...
  if not IsDesigning and (WindowFromPoint(GetMouseCursorPos) <> Handle) then
    InternalStates := InternalStates - [bisHot, bisPressed,
      bisMenuButtonPressed];
end;
...
begin
  if not cxIsDestroying(Self, True) and IsMouseTracking(Self) then
  begin
    MouseLeave;
    EndMouseTracking(Self);
...
begin
  ...
  if not(csDestroying in ComponentState) and HandleAllocated and
    SpeedButtonOptions.Transparent and not CanBeFocused then
    Invalidate;
end;
...
begin
  ...
  FIsDropDownMenuClosing := (bisMenuButtonPressed in InternalStates) or
    ...
    PeekMessage(Msg, Handle, WM_LBUTTONDBLCLK, WM_LBUTTONDBLCLK, PM_NOREMOVE))
    and PtInRect(GetMenuButtonBounds, ScreenToClient(Msg.pt)));
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowPopupMenu(Self, APopupMenu, cxRectOffset(ClientRect, ClientOrigin),
        FPopupAlignment);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SpeedButtonOptions.UpdateGroupValues(GroupIndex,
          ASender.SpeedButtonOptions);
        Invalidate;
      end;
    end;
  end;
...
const
  CommandLinkStyles: array [Boolean] of Cardinal = (BS_COMMANDLINK,
    BS_DEFCOMMANDLINK);
  SplitButtonStyles: array [Boolean] of Cardinal = (BS_SPLITBUTTON,
    BS_DEFSPLITBUTTON);
begin
  ...
  begin
    case Style of
      bsCommandLink:
        Params.Style := Params.Style xor CommandLinkStyles[Default];
      bsSplitButton:
        Params.Style := Params.Style xor SplitButtonStyles[Default];
    end;
  end;
...
begin
  ...
  else if ADefault <> FIsFocused then
  begin
    FIsFocused := ADefault;
    Refresh;
  end;
...
begin
  Result := (Button.ActionLink = nil) or not TControlActionLinkAccess
    (Button.ActionLink).IsImageIndexLinked;
end;
...
begin
  CalculateLayout(ViewInfo.GetContentRect, OptionsImage.Margin,
    OptionsImage.Spacing);
end;

procedure TcxButtonCommandLinkLayoutCalculator.CalculateLayout
  (const AContentRect: TRect; AMargin, ASpacing: Integer);
  ...
  begin
    ...
    cxGetTextRect(Result, ViewInfo.Caption, ViewInfo.Font, DT_WORDBREAK or
      DT_END_ELLIPSIS or Painter.GetBiDiFlags);
  end;
...
begin
  ...
  begin
    ViewInfo.FCaptionRect := TdxRightToLeftLayoutConverter.ConvertRect
      (ViewInfo.FCaptionRect, AContentRect);
    ViewInfo.FDescriptionRect := TdxRightToLeftLayoutConverter.ConvertRect
      (ViewInfo.FDescriptionRect, AContentRect);
    ViewInfo.FGlyphPos := TdxRightToLeftLayoutConverter.ConvertPoint
      (ViewInfo.FGlyphPos, AContentRect);
...
begin
  ...
  begin
    R := ViewInfo.Bounds;
    R := cxRectSetRight(R, R.Right - cxTextOffset,
      Painter.GetDropDownArrowSize);
  end
  else
  begin
  ...
  if UseRightToLeftAlignment then
    ViewInfo.FDropDownArrowRect := TdxRightToLeftLayoutConverter.ConvertRect(R,
      ViewInfo.Bounds)
  else
...
procedure TcxButtonOfficeDropDownLayoutCalculator.CalculateTextAreaLayout
  (const R: TRect);
begin
  ...
  case Layout of
    blGlyphTop, blGlyphBottom:
      begin
        ViewInfo.FCaptionRect := cxRectCenterHorizontally(ViewInfo.CaptionRect,
          CaptionSize.cx);
        ViewInfo.FDescriptionRect := cxRectCenterHorizontally
          (ViewInfo.DescriptionRect, DescriptionSize.cx);
      end;

    blGlyphLeft:
      begin
        ViewInfo.FCaptionRect := cxRectSetWidth(ViewInfo.CaptionRect,
          CaptionSize.cx);
        ViewInfo.FDescriptionRect := cxRectSetWidth(ViewInfo.DescriptionRect,
          DescriptionSize.cx);
      end;

    blGlyphRight:
      begin
        ViewInfo.FCaptionRect := cxRectSetRight(ViewInfo.CaptionRect,
          ViewInfo.CaptionRect.Right, CaptionSize.cx);
        ViewInfo.FDescriptionRect := cxRectSetRight(ViewInfo.DescriptionRect,
          ViewInfo.DescriptionRect.Right, DescriptionSize.cx);
      end;
  end;
end;

procedure TcxButtonOfficeDropDownLayoutCalculator.CalculateTextAreaSize
  (const R: TRect);
var
  ARect: TRect;
  ATextBounds: TRect;
...
begin
  ...
  begin
    ...
    ARect := ATextBounds;
    cxGetTextRect(ARect, ViewInfo.Description, ViewInfo.DescriptionFont,
      Painter.GetBiDiFlags or DT_WORDBREAK);
...
procedure TcxButtonStandardLayoutCalculator.CalculateGlyphPosition
  (const R: TRect);
var
  P: TPoint;
begin
...
procedure TcxButtonStandardLayoutCalculator.CalculateMargins(const R: TRect;
  var AMargin, ASpacing: Integer);

  function GetMargin(const ASize1, ASize2: TSize;
    ACoefficient: Integer): Integer;
  begin
    if Layout in [blGlyphLeft, blGlyphRight] then
...
begin
  ...
  if AMargin = -1 then
  begin
    ASize := cxSize(GlyphSize.cx + TextAreaSize.cx,
      GlyphSize.cy + TextAreaSize.cy);
    if ASpacing = -1 then
    begin
  ...
  else if ASpacing = -1 then
  begin
    ASize := cxSize(cxRectWidth(R) - (AMargin + GlyphSize.cx),
      cxRectHeight(R) - (AMargin + GlyphSize.cy));
    ASpacing := GetMargin(ASize, TextAreaSize, 2);
  end;
end;

function TcxButtonStandardLayoutCalculator.CalculateTextAreaBounds
  (const R: TRect): TRect;
var
  ATextOffsets: TRect;
begin
  ...
  Result := cxRectSetSize(Result, TextAreaSize.cx -
    cxMarginsWidth(ATextOffsets), TextAreaSize.cy -
    cxMarginsHeight(ATextOffsets));

  case Layout of
    blGlyphLeft:
      Result := cxRectOffsetHorz(Result, GlyphPos.X + GlyphSize.cx + Spacing);
    blGlyphRight:
      Result := cxRectOffsetHorz(Result, GlyphPos.X - Spacing -
        TextAreaSize.cx);
    blGlyphTop:
      Result := cxRectOffsetVert(Result, GlyphPos.Y + GlyphSize.cy + Spacing);
    blGlyphBottom:
      Result := cxRectOffsetVert(Result, GlyphPos.Y - Spacing -
        TextAreaSize.cy);
  end;

  if Layout in [blGlyphLeft, blGlyphRight] then
    Result := cxRectOffsetVert(Result,
      (cxRectHeight(R) - TextAreaSize.cy + cxBtnStdVertTextOffsetCorrection)
      div 2 + R.Top)
  else
    Result := cxRectOffsetHorz(Result, (cxRectWidth(R) - TextAreaSize.cx) div 2
      + R.Left);
end;

procedure TcxButtonStandardLayoutCalculator.CalculateTextAreaLayout
  (const R: TRect);
begin
  ViewInfo.FCaptionRect := R;
end;

procedure TcxButtonStandardLayoutCalculator.CalculateTextAreaSize
  (const R: TRect);
var
  ATextOffsets: TRect;
  ATextBounds: TRect;
...
begin
  ...
  begin
    ...
    begin
      FDropDownButtonRect := cxRectSetWidth(Bounds,
        Painter.GetDropDownButtonSize);
      FButtonRect.Left := FDropDownButtonRect.Right;
      if (Painter.LookAndFeelStyle <> lfsSkin) and not SpeedButtonOptions.NeedDrawParts
      then
        Dec(FButtonRect.Left, 2);
    end
    else
    begin
      FDropDownButtonRect := cxRectSetRight(Bounds, Bounds.Right,
        Painter.GetDropDownButtonSize);
...
begin
  Result := (Button.PaintStyle in [bpsDefault, bpsGlyph]) and
    ButtonGlyph.IsImageAssigned;
end;
...
begin
  ...
  begin
    ...
    else if ALayout = blGlyphRight then
      ALayout := blGlyphLeft;
  end;
end;
...
function TcxButtonViewInfo.CreateLayoutCalculator(AKind: TcxButtonKind)
  : TcxButtonCustomLayoutCalculator;
begin
  case AKind of
    cxbkCommandLink:
...
procedure TcxButtonViewInfo.DrawGlyph(ACanvas: TcxCanvas);
  ...
  const
    States: array [TcxButtonState] of TButtonState = (bsUp, bsUp, bsUp, bsDown,
      bsDisabled);
  ...
  procedure InternalDrawGlyph(AStandardButtonState: TButtonState;
    const AImageBounds: TRect);
  var
    ABuffer: TcxBitmap32;
    AOffset: Integer;
    ASrcRect: TRect;
  begin
    ...
    ASrcRect := cxRect(cxSize(ButtonGlyph.Glyph.Width div ButtonGlyph.NumGlyphs,
      ButtonGlyph.Glyph.Height));
    ASrcRect := cxRectOffsetHorz(ASrcRect, AOffset * cxRectWidth(ASrcRect));

    if cxSizeIsEqual(AImageBounds, ASrcRect) then
      ButtonGlyph.Glyph.StretchDraw(ACanvas.Handle, AImageBounds, ASrcRect,
        MaxByte, GetColorPalette)
    else
    begin
      ABuffer := TcxBitmap32.CreateSize(AImageBounds, True);
      try
        cxBitBlt(ABuffer.Canvas.Handle, ACanvas.Handle, ABuffer.ClientRect,
          AImageBounds.TopLeft, SRCCOPY);
        ButtonGlyph.Glyph.StretchDraw(ABuffer.Canvas.Handle, ABuffer.ClientRect,
          ASrcRect, MaxByte, GetColorPalette);
        cxBitBlt(ACanvas.Handle, ABuffer.Canvas.Handle, AImageBounds,
          cxNullPoint, SRCCOPY);
      finally
        ABuffer.Free;
      end;
  ...
  procedure InternalDrawImage(AStandardButtonState: TButtonState;
    const AImageBounds: TRect);
  const
    Mode: array [Boolean] of TcxImageDrawMode = (idmNormal, idmDisabled);
  begin
    cxDrawImage(ACanvas, AImageBounds, ButtonGlyph.Glyph, ButtonGlyph.Images,
      ButtonGlyph.ImageIndex, ifmStretch,
      Mode[AStandardButtonState = bsDisabled], True, GetColorPalette,
      ScaleFactor, True);
  end;

begin
  if IsGlyphAssigned(ButtonGlyph.Glyph) and (ButtonGlyph.NumGlyphs > 1) then
    InternalDrawGlyph(GetStandardButtonState(State),
      cxRectSetOrigin(cxRect(ImageSize), GlyphPos))
  else
    InternalDrawImage(GetStandardButtonState(State),
      cxRectSetOrigin(cxRect(ImageSize), GlyphPos));
end;

function TcxButtonViewInfo.GetContentRect
  (AState: TcxButtonState = cxbsDefault): TRect;
var
  ABorderSize: Integer;
begin
...
begin
  ...
  try
    ...
    begin
      ACanvas.Font := Font;
      Painter.DrawText(ACanvas, Caption, CaptionRect, State,
        CaptionDrawTextFlags);
    end;

    if not cxRectIsEmpty(DescriptionRect) then
    begin
      ACanvas.Font := DescriptionFont;
      Painter.DrawText(ACanvas, Description, DescriptionRect, State,
        DescriptionDrawTextFlags);
    end;
  finally
...
begin
  ...
  DescriptionFont.Color := Painter.GetDescriptionTextColor(State,
    DescriptionFont.Color);
  if Kind = cxbkCommandLink then
  begin
...
begin
  case Kind of
    ...
    cxbkOfficeDropDown:
      Result := DT_VCENTER or IfThen(Layout in [blGlyphTop, blGlyphBottom],
        DT_CENTER) or IfThen(Button.WordWrap, DT_WORDBREAK);
  else
    Result := DT_CENTER or DT_VCENTER or IfThen(Button.WordWrap, DT_WORDBREAK)
      or DT_NOCLIP;
  end;
end;
...
const
  LayoutMap: array [TButtonLayout] of Integer = (DT_LEFT, DT_RIGHT, DT_CENTER,
    DT_CENTER);
begin
  Result := DT_WORDBREAK;
...
begin
  ...
  begin
    ...
    begin
      dxTestCheck((ButtonGlyph.Glyph.SourceWidth <> 0) and
        (ButtonGlyph.Glyph.SourceHeight <> 0),
        'TcxButtonGlyph.GetImageSize fails');
      Result := Size(ButtonGlyph.Glyph.SourceWidth div ButtonGlyph.NumGlyphs,
        ButtonGlyph.Glyph.SourceHeight);
      Result := AScaleFactor.Apply(Result, ButtonGlyph.Glyph.SourceDPI,
        dxDefaultDPI);
    end
    else
      Result := dxGetImageSize(ButtonGlyph.Images, AScaleFactor);
  end
  else if FButton.Kind = cxbkCommandLink then
...
begin
  Result := not(cxRectIsEmpty(DropDownArrowRect) or HasDropDownButton and
    (Painter.LookAndFeelStyle = lfsSkin));
end;

function TcxButtonViewInfo.NeedDrawFocusRect: Boolean;
begin
  Result := Button.CanFocus and Button.ShowFocusRect and Button.Focused and
    not(bisMenuVisible in Button.InternalStates);
end;
...
procedure TcxButtonPainter.DrawButton(ACanvas: TcxCanvas; const R: TRect;
  AState: TcxButtonState; AColor: TColor; APart: TcxButtonPart);
begin
  if Button.Kind = cxbkCommandLink then
    DrawCommandLinkButton(ACanvas, R, AState, AColor)
  else if Button.SpeedButtonOptions.NeedDrawParts then
    DrawSpeedButton(ACanvas, R, AState, AColor, APart)
  else
    LookAndFeelPainter.DrawScaledButton(ACanvas, R, AState, ScaleFactor,
      APart, AColor);
end;

procedure TcxButtonPainter.DrawText(ACanvas: TcxCanvas; const AText: TCaption;
  R: TRect; ADrawTextFlags: Integer);
begin
  if ViewInfo.IsDrawOnGlass then
    dxDrawTextOnGlass(ACanvas.Handle, AText, ACanvas.Font, R,
      ACanvas.Font.Color, ADrawTextFlags, 0, True)
  else
...
begin
  ...
  begin
    ...
    try
      ...
      if (AState = cxbsDisabled) and
        not(LookAndFeelStyle in [lfsSkin, lfsNative]) then
      begin
        OffsetRect(R, 1, 1);
...
procedure TcxButtonPainter.DrawCommandLinkButton(ACanvas: TcxCanvas;
  const R: TRect; AState: TcxButtonState; AColor: TColor);
begin
  LookAndFeelPainter.DrawScaledCommandLinkBackground(ACanvas, R, AState,
    ScaleFactor, AColor);
  if not ViewInfo.ButtonGlyph.IsImageAssigned then
    LookAndFeelPainter.DrawScaledCommandLinkGlyph(ACanvas, ViewInfo.GlyphPos,
      AState, ScaleFactor);
end;

procedure TcxButtonPainter.DrawDropDownArrow(ACanvas: TcxCanvas; const R: TRect;
  AState: TcxButtonState);
begin
  LookAndFeelPainter.DrawScaledScrollBarArrow(ACanvas, R, AState, adDown,
    ScaleFactor);
end;

procedure TcxButtonPainter.DrawFocusRect(ACanvas: TcxCanvas; const R: TRect);
begin
  ACanvas.DrawFocusRect(LookAndFeelPainter.ScaledButtonFocusRect(ACanvas, R,
    ScaleFactor));
end;

procedure TcxButtonPainter.DrawParentBackground(ACanvas: TcxCanvas;
  const R: TRect);
var
  ATheme: TdxTheme;
begin
  case LookAndFeelPainter.LookAndFeelStyle of
    ...
    lfsNative:
      begin
        ...
        if (ATheme <> TC_NONE) and IsThemeBackgroundPartiallyTransparent(ATheme,
          BP_PUSHBUTTON, PBS_NORMAL) then
          cxDrawThemeParentBackground(Button, ACanvas, R)
        else if Button.SpeedButtonOptions.Transparent then
          cxDrawTransparentControlBackground(Button, ACanvas, R);
      end;
...
procedure TcxButtonPainter.DrawSpeedButton(ACanvas: TcxCanvas; R: TRect;
  AState: TcxButtonState; AColor: TColor; APart: TcxButtonPart);

  function IsDrawBorder: Boolean;
  begin
    Result := (LookAndFeelStyle <> lfsSkin) and
      (not Button.SpeedButtonOptions.Flat or Button.IsDesigning or
      ((AState <> cxbsDisabled) and ((bisHot in Button.InternalStates) or
      (AState <> cxbsNormal))));
    Result := Result and not((LookAndFeelStyle = lfsNative) or
      (LookAndFeelStyle = lfsOffice11));
  end;

  function IsNotTransparent: Boolean;
  begin
    Result := not Button.SpeedButtonOptions.Transparent or
      ((AState <> cxbsDisabled) and (not(bisHot in Button.InternalStates) and
      (AState <> cxbsNormal)) or ((bisHot in Button.InternalStates) and
      LookAndFeelPainter.IsButtonHotTrack));
  end;
...
function TcxButtonPainter.GetDescriptionTextColor(AState: TcxButtonState;
  ADefaultColor: TColor): TColor;
begin
  Result := LookAndFeelPainter.ButtonDescriptionTextColor(AState,
    ADefaultColor);
end;
...
begin
  if Button.LookAndFeel.SkinPainter <> nil then
    Result := lfsSkin
  else if Button.LookAndFeel.NativeStyle then
    Result := lfsNative
  else
...
begin
  Result := cxPoint(LookAndFeelPainter.ScaledButtonTextShift(ScaleFactor),
    LookAndFeelPainter.ScaledButtonTextShift(ScaleFactor));
end;

function TcxButtonPainter.GetTextColor(AState: TcxButtonState;
  ADefaultColor: TColor): TColor;
begin
  ...
  begin
    if Button.Kind = cxbkCommandLink then
      Result := LookAndFeelPainter.DefaultCommandLinkTextColor(AState,
        ADefaultColor)
    else
