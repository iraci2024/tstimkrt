...
type
  ...
  public
    function InsertPicture(const APos: IdxRichEditDocumentPosition;
      AImage: TGraphic): IdxRichEditShape; override;
    function InsertTextBox(const APos: IdxRichEditDocumentPosition)
      : IdxRichEditShape; override;
  end;
  ...
  public
    ...
    class function TryCreate(ADocument: TdxNativeSubDocument;
      ARun: TdxTextRunBase; ARunIndex: TdxRunIndex): IdxRichEditShape; static;
    class function CreateUnsafe(ADocument: TdxNativeSubDocument;
      ARunIndex: TdxRunIndex): IdxRichEditShape; static;
    ...
    property RectangleScalableObject: IdxRectangularScalableObject
      read GetRectangleScalableObject;
  ...
  TdxNativeShapeCollection = class(TInterfacedObject,
    IdxRichEditShapeCollection, IdxRichEditReadOnlyShapeCollection)
  strict private
  type
    TProcessShapeProc = reference to function(ARun: TdxFloatingObjectAnchorRun;
...
function TdxNativeTextBoxSubDocument.InsertPicture
...
function TdxNativeTextBoxSubDocument.InsertTextBox
...
begin
  TdxRichEditExceptions.ThrowInvalidOperationException
    (cxGetResourceString(@sdxRichEditExceptionCannotInsertShapeIntoTextBox));
end;
...
begin
  ...
  FDocument := ADocument;
  FContent := AContent;
  FTextBoxContentDocument := TdxNativeTextBoxSubDocument.Create
    (TdxPieceTable(FContent.TextBox.PieceTable), FDocument.DocumentServer);
end;
...
procedure TdxNativeTextBox.SetHeightRule(const Value
...
begin
  ModelShape.OutlineWidth :=
    Integer(Round(FShape.UnitConverter.PointsToModelUnitsF(Value)));
end;

{ TdxNativeShape }

constructor TdxNativeShape.Create(ADocument: TdxNativeSubDocument;
...
begin
  inherited Create;
  FDocument := ADocument;
  AStart := TdxDocumentModelPosition.FromRunStart(ADocument.PieceTable,
    ARunIndex);
  AEnd := AStart;
...
class function TdxNativeShape.CreateUnsafe(ADocument: TdxNativeSubDocument;
  ARunIndex: TdxRunIndex): IdxRichEditShape;
begin
  Result := TdxNativeShape.Create(ADocument, ARunIndex);
end;
...
begin
  Result := TdxFloatingObjectAnchorRun(FDocument.PieceTable.Runs
    [TdxNativeDocumentPosition(FRange.Start).Position.RunIndex]);
end;
...
function TdxNativeShape.GetRectangleScalableObject
  : IdxRectangularScalableObject;
begin
  Supports(FDocument.PieceTable.Runs[TdxNativeDocumentPosition(FRange.Start)
    .Position.RunIndex], IdxRectangularScalableObject, Result);
end;
...
procedure TdxNativeShape.SetTextWrapping(const Value
...
begin
  ...
  Result := TdxPointF.Create(FDocument.ModelUnitsToUnits(APoint.X),
    FDocument.ModelUnitsToUnits(APoint.Y));
end;
...
begin
  ...
  Result := TdxSizeF.Create(FDocument.ModelUnitsToUnits(AOriginalSize.Width),
    FDocument.ModelUnitsToUnits(AOriginalSize.Height));
end;
...
begin
  ...
  Result := TdxSizeF.Create(FDocument.ModelUnitsToUnitsF(AActualSize.Width),
    FDocument.ModelUnitsToUnitsF(AActualSize.Height));
end;
...
function TdxNativeShape.GetHorizontalAlignment
  : TdxRichEditShapeHorizontalAlignment;
begin
  Result := Properties.HorizontalPositionAlignment;
end;

procedure TdxNativeShape.SetHorizontalAlignment(const Value
  : TdxRichEditShapeHorizontalAlignment);
begin
...
procedure TdxNativeShape.SetVerticalAlignment(const Value
  : TdxRichEditShapeVerticalAlignment);
begin
  Properties.VerticalPositionAlignment := Value;
end;

function TdxNativeShape.GetRelativeHorizontalPosition
  : TdxRichEditShapeRelativeHorizontalPosition;
begin
  Result := Properties.HorizontalPositionType;
end;

procedure TdxNativeShape.SetRelativeHorizontalPosition
  (const Value: TdxRichEditShapeRelativeHorizontalPosition);
begin
  Properties.HorizontalPositionType := Value;
end;

function TdxNativeShape.GetRelativeVerticalPosition
  : TdxRichEditShapeRelativeVerticalPosition;
begin
  Result := Properties.VerticalPositionType;
end;

procedure TdxNativeShape.SetRelativeVerticalPosition
  (const Value: TdxRichEditShapeRelativeVerticalPosition);
begin
...
function TdxNativeShapeCollection.Get(const ARange: IdxRichEditDocumentRange)
  : IdxRichEditReadOnlyShapeCollection;
var
...
function TdxNativeShapeCollection.CanProcessShape(const AShape
  : IdxRichEditShape): Boolean;
begin
  if AShape = nil then
    Exit(False);
...
procedure TdxNativeShapeCollection.ProcessShapes(const Action
  : TProcessShapeProc);
var
...
begin
  AResult := 0;
  ProcessShapes(
    function(ARun: TdxFloatingObjectAnchorRun; ARunIndex: TdxRunIndex): Boolean
    var
...
begin
  ...
  try
    ...
    try
      ...
      ARun := TdxFloatingObjectAnchorRun
        (PieceTable.Runs[ARunInfo.Start.RunIndex]);
...
begin
  ...
  try
    ...
    try
      ...
      AContent := TdxTextBoxFloatingObjectContent.Create(ARun,
        ATextBoxContentType);
      ARun.SetContent(AContent);
      ASize := DocumentModel.UnitConverter.TwipsToModelUnits
        (TSize.Create(3 * 1440, 2 * 1440));
      ARun.FloatingObjectProperties.ActualSize := ASize;
