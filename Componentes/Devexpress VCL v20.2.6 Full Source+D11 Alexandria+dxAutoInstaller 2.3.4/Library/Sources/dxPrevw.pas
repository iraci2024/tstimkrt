...
uses
  Classes, Controls, Windows, Graphics, Forms, SysUtils, dxCore, dxBkgnd,
  cxGraphics, cxGeometry, dxCustomPreview,
...
type
  ...
  protected
    ...
    function GetDesignHitTest(X, Y: Integer; Shift: TShiftState)
      : Boolean; override;
    //
    property PageBackgroundApplyToEntirePage: Boolean
      read FPageBackgroundApplyToEntirePage
      write SetPageBackgroundApplyToEntirePage;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;

    property PageBackground: TdxBackground read FPageBackground
      write SetPageBackground;
  end;
  ...
  published
    property OptionsBehavior: TdxPreviewOptionsBehavior read GetOptionsBehavior
      write SetOptionsBehavior default dxPreviewDefaultBehaviorOptions;
    property OptionsView: TdxPreviewOptionsView read GetOptionsView
      write SetOptionsView default dxPreviewDefaultOptionsView;
    property OptionsZoom: TdxPreviewOptionsZoom read GetOptionsZoom
      write SetOptionsZoom default dxPreviewDefaultOptionsZoom;
...
begin
  if (Mode = bmPicture) and (PictureMode = ppmCenter) and
    (Preview.ZoomFactor <> 100) then
  begin
    FBitmap.Width := MulDiv(Picture.Width, Preview.ZoomFactor, 100);
    FBitmap.Height := MulDiv(Picture.Height, Preview.ZoomFactor, 100);
    FBitmap.Canvas.StretchDraw(Rect(0, 0, FBitmap.Width,
      FBitmap.Height), Picture);
...
begin
  ...
  if (UpdateCount = 0) and IsRepaintNeeded and HasPreview and not Preview.IsUpdateLocked
  then
...
function TCustomdxPreview.CreateBackgroundRegion(APage: TdxPreviewPage)
  : TcxRegion;
begin
...
procedure TCustomdxPreview.DrawPageBackground(ACanvas: TcxCanvas;
...
function TCustomdxPreview.GetDesignHitTest(X, Y: Integer;
...
procedure TdxPreview.SetOptionsBehavior(const AValue
  : TdxPreviewOptionsBehavior);
begin
