...
uses
  ...
  Dialogs, Menus, StdCtrls, ActnList, Math,
  dxCore, cxClasses, cxControls, cxGraphics, cxStyles, cxContainer,
  cxInplaceContainer,
  dxRibbon, dxBar, cxLookAndFeels, cxLookAndFeelPainters,
  dxLayoutControl, dxLayoutControlAdapters, dxLayoutContainer,
  dxLayoutcxEditAdapters, cxEdit, cxButtons, cxTextEdit,
  cxDropDownEdit, cxCustomData, cxTL, cxMaskEdit, cxCheckBox, cxBarEditItem,
...
type
  ...
  TdxRibbonCustomizationForm = class(TdxForm)
    ...
    procedure tlCommandsBeginDragNode(Sender: TcxCustomTreeList;
      ANode: TcxTreeListNode; var Allow: Boolean);
    procedure tlCommandsCollapsing(Sender: TcxCustomTreeList;
      ANode: TcxTreeListNode; var Allow: Boolean);
    procedure tlCommandsCustomDrawDataCell(Sender: TcxCustomTreeList;
      ACanvas: TcxCanvas; AViewInfo: TcxTreeListEditCellViewInfo;
      var ADone: Boolean);
    procedure tlCommandsDeletion(Sender: TcxCustomTreeList;
      ANode: TcxTreeListNode);
    procedure tlCommandsDragOver(Sender, Source: TObject; X, Y: Integer;
    ...
    procedure tlRibbonDragOver(Sender, Source: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
    procedure tlRibbonMoveTo(Sender: TcxCustomTreeList;
      AttachNode: TcxTreeListNode; AttachMode: TcxTreeListNodeAttachMode;
      Nodes: TList; var IsCopy, Done: Boolean);
    procedure tlRibbonNodeCheckChanged(Sender: TcxCustomTreeList;
      ANode: TcxTreeListNode; AState: TcxCheckBoxState);
    procedure tlRibbonStylesGetContentStyle(Sender: TcxCustomTreeList;
  ...
  private
    ...
    function GetRibbonQATCustomizationHelper
  ...
  protected
    ...
    property RibbonCustomizationHelper: TdxRibbonCustomizationFormHelper
      read GetRibbonCustomizationHelper;
    property RibbonQATCustomizationHelper: TdxRibbonQATCustomizationFormHelper
      read GetRibbonQATCustomizationHelper;
  public
    constructor Create(ARibbon: TdxCustomRibbon); reintroduce; overload;
    constructor Create(ARibbonQAT: TdxRibbonQuickAccessToolbar);
      reintroduce; overload;
    destructor Destroy; override;
  end;

function ShowRibbonCustomizationForm(ARibbon: TdxCustomRibbon;
...
function ShowRibbonCustomizationForm(ARibbon: TdxCustomRibbon;
  ...
  begin
    if (ACustomizationFormClass = nil) or
      not ACustomizationFormClass.InheritsFrom(TdxRibbonCustomizationForm) then
      AActualCustomizationFormClass := TdxRibbonCustomizationForm
    else
      AActualCustomizationFormClass := TdxRibbonCustomizationFormClass
        (ACustomizationFormClass);

    if AMode = rcfmCustomizeRibbon then
      Result := AActualCustomizationFormClass.Create(ARibbon)
    else
      Result := AActualCustomizationFormClass.Create
        (ARibbon.QuickAccessToolbar);
  end;
...
constructor TdxRibbonCustomizationForm.Create
  (ARibbonQAT: TdxRibbonQuickAccessToolbar);
begin
  inherited Create(nil);
...
begin
  ...
  try
    ...
    if not IsRibbonCustomization then
      RibbonQATCustomizationHelper.ChangeQATPosition
        (chbShowQATBelowRibbon.Checked);
...
begin
  ...
  acAddNewContext.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionAddNewContext);
  acAddNewGroup.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionAddNewGroup);
  acAddNewTab.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionAddNewTab);
  acMoveDown.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionMoveDown);
  acMoveUp.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionMoveUp);
  acRemove.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionRemove);
  acRename.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionRename);
  acResetAllCustomizations.Caption :=
    cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionResetAllCustomizations);
  acResetSelectedTab.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionResetSelectedTab);
  acShowTab.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionShowTab);

  bbtnResetOnlySelectedTab.Caption :=
    cxGetResourceString(@sdxRibbonCustomizationFormCaptionResetOnlySelectedTab);
  ...
  btnNewElement.Caption := cxGetResourceString
  ...
  lciCommandsSource.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionCommandsSource);
  lciRibbonSource.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionRibbonSource);
  lbRibbonQAT.Caption := cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionQuickAccessToolbar);
  chbShowQATBelowRibbon.Caption :=
    cxGetResourceString
    (@sdxRibbonCustomizationFormCaptionQuickAccessToolbarShowBelowRibbon);
end;

function TdxRibbonCustomizationForm.CreateHelper
  : TdxCustomRibbonCustomizationFormHelper;
begin
  if IsRibbonCustomization then
    Result := TdxRibbonCustomizationFormHelper.Create(Ribbon, tlCommands,
      tlRibbon, cbCommands, cbRibbon)
  else
    Result := TdxRibbonQATCustomizationFormHelper.Create(Ribbon, tlCommands,
      tlRibbon, cbCommands, cbRibbon);
end;
...
begin
  Helper.Initialize;
  stNodeDisabled.TextColor := Ribbon.BarManager.LookAndFeel.Painter.
    DefaultCommandLinkTextColor(cxbsDisabled);
  if not IsRibbonCustomization then
    RibbonQATCustomizationHelper.InitializeQATPositionIndicator
      (chbShowQATBelowRibbon);
  ApplyLocalization;
  UpdateElementsVisibility;
  UpdateActionsState(False);
end;

procedure TdxRibbonCustomizationForm.UpdateActionsState(const AChanged
  : Boolean = True);
var
  ALevel: Integer;
begin
  ...
  begin
    ...
    acAdd.Enabled := Helper.CanMoveNodeTo(tlCommands.FocusedNode,
      tlRibbon.FocusedNode);
    ...
    begin
      ...
      acResetSelectedTab.Enabled :=
        RibbonCustomizationHelper.CanResetFocusedNode;
...
function TdxRibbonCustomizationForm.GetRibbonCustomizationHelper
  : TdxRibbonCustomizationFormHelper;
begin
  Result := TdxRibbonCustomizationFormHelper(Helper);
end;

function TdxRibbonCustomizationForm.GetRibbonQATCustomizationHelper
  : TdxRibbonQATCustomizationFormHelper;
begin
...
procedure TdxRibbonCustomizationForm.acResetAllCustomizationsExecute
  (Sender: TObject);
begin
...
procedure TdxRibbonCustomizationForm.acUpdateActionsStateExecute
  (Sender: TObject);
begin
  UpdateActionsState(False);
end;

procedure TdxRibbonCustomizationForm.cbCommandsPropertiesChange
  (Sender: TObject);
begin
  if Helper.CanUpdateContent then
...
procedure TdxRibbonCustomizationForm.chbShowQATBelowRibbonPropertiesChange
  (Sender: TObject);
begin
  UpdateActionsState;
end;
...
procedure TdxRibbonCustomizationForm.tlCommandsCollapsing
  (Sender: TcxCustomTreeList; ANode: TcxTreeListNode; var Allow: Boolean);
begin
  Allow := Helper.GetNodeLevel(ANode) <> dxrcfContextLevel;
end;

procedure TdxRibbonCustomizationForm.tlCommandsCustomDrawDataCell
  (Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
  AViewInfo: TcxTreeListEditCellViewInfo; var ADone: Boolean);
begin
  case Helper.GetNodeLevel(AViewInfo.Node, True) of
    dxrcfContextLevel:
      ADone := RibbonCustomizationHelper.DrawContextNode(Sender, ACanvas,
        AViewInfo, dxLayoutCxLookAndFeel.LookAndFeelPainter);
    dxrcfItemLevel:
      ADone := Helper.DrawItemNode(Sender, ACanvas, AViewInfo,
        dxLayoutCxLookAndFeel.LookAndFeelPainter, ScaleFactor);
  end;
end;

procedure TdxRibbonCustomizationForm.tlCommandsFocusedNodeChanged
  (Sender: TcxCustomTreeList; APrevFocusedNode, AFocusedNode: TcxTreeListNode);
begin
  UpdateActionsState(False);
end;

procedure TdxRibbonCustomizationForm.tlCommandsBeginDragNode
  (Sender: TcxCustomTreeList; ANode: TcxTreeListNode; var Allow: Boolean);
begin
  Allow := ((ANode.Data <> nil) or (Sender = tlRibbon)) and
    (Helper.GetNodeLevel(ANode) > dxrcfContextLevel);
  if Allow then
    Helper.InitializeMovedNode(ANode);
end;

procedure TdxRibbonCustomizationForm.tlCommandsDeletion
  (Sender: TcxCustomTreeList; ANode: TcxTreeListNode);
begin
  Helper.FreeNodeData(ANode);
end;

procedure TdxRibbonCustomizationForm.tlCommandsDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := False;
end;

procedure TdxRibbonCustomizationForm.tlCommandsEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
...
procedure TdxRibbonCustomizationForm.tlRibbonDragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := Helper.CanMoveNodeTo(Helper.MovedNode, tlRibbon.HitTest.HitNode, Y);
end;

procedure TdxRibbonCustomizationForm.tlRibbonMoveTo(Sender: TcxCustomTreeList;
  AttachNode: TcxTreeListNode; AttachMode: TcxTreeListNodeAttachMode;
  Nodes: TList; var IsCopy, Done: Boolean);
begin
  Done := True;
  if AttachNode.TreeList = tlRibbon then
    Helper.DropMovedNodeTo(tlRibbon.HitTest.HitNode,
      tlRibbon.ScreenToClient(GetMouseCursorPos).Y);
end;

procedure TdxRibbonCustomizationForm.tlRibbonNodeCheckChanged
