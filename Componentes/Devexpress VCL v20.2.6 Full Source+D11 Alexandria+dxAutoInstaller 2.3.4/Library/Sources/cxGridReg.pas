...
type
  TcxExecuteGridWizardProc = procedure(AGrid: TcxCustomGrid;
    ASelectedGridView: TcxCustomGridView = nil);
  ...
  protected
    ...
    procedure GetGridViewItemNames(AGridView: TcxCustomGridView;
      Proc: TGetStrProc); virtual;
    function InternalGetGridView(APersistent: TPersistent): TcxCustomGridView;
      virtual; abstract;
  public
  ...
  protected
    function InternalGetGridView(APersistent: TPersistent)
...
{$IFNDEF NONDB}
  ...
  cxGridDBChartView,
{$ENDIF}
  Dialogs, cxGridImportDialog, cxControls,
  ...
  cxPropEditors, cxStyles, cxViewEditor, cxBandedTableViewEditor,
  cxCardViewEditor,
  cxChartViewEditor, cxGridPredefinedStyles, cxGridStyleSheetsPreview,
  cxGridExportLink,
  ColnEdit, cxGridInplaceEditForm, cxFilter, TreeIntf,
...
const
  GridEditorVerbs: array [0 .. 3] of string = ('Editor...', 'Import...',
    'Structure Navigator', 'Wizard...');
...
type
  ...
  protected
    ...
    function ProcessViewMenuItem(var AIndex: Integer;
      AAction: TViewMenuItemAction;
    ...
    property ViewMenuProvider: TcxCustomGridViewMenuProvider
      read FViewMenuProvider;
  public
...
begin
  if ProcessViewMenuItem(AIndex, vmiaGetCaption) then
    Result := ''
  else if (AIndex >= 0) and (AIndex <= High(GridEditorVerbs)) then
...
begin
  ...
  if not ProcessViewMenuItem(AIndex, vmiaExecute) then
    case AIndex of
      0:
        ShowGridEditor(Designer, Grid);
      1:
        ShowGridImportDialog(Designer, Grid);
      2:
        Grid.StructureNavigator.Visible := not Grid.StructureNavigator.Visible;
      3:
        if FGridWizard <> nil then
  ...
  if (AIndex >= FAddOnInsertPosition) and (ViewMenuProvider <> nil) then
    ...
    begin
      if AIndex = FAddOnInsertPosition then // separator
        ...
        AMenuProviderItem := ViewMenuProvider.Items
          [AIndex - FAddOnInsertPosition - 1];
...
type
  ...
  protected
    ...
    property GridViewRepository: TcxGridViewRepository
      read GetGridViewRepository;
  end;
...
begin
  Result := TcxCustomGridViewAccess.GetStyles
    (TcxCustomGridView(GetComponent(0)));
end;
...
function TcxGridChartStackedColumnDiagramEventsProperty.GetInstance
  : TPersistent;
begin
...
begin
  Result := TcxCustomGrid(TcxGridLevel(GetComponent(0)).Control)
    .LevelTabs.Images;
end;
...
function TcxCustomGridTableItemPropertiesProperty.GetAttributes
  : TPropertyAttributes;
begin
...
procedure TcxCustomGridTableItemPropertiesProperty.SetValue
  (const Value: string);
const
...
begin
  Result := nil;
  if Supports(GetComponent(0), IcxCustomGridDataController, AIDataController)
  then
  begin
    ...
    if (AParentLevel <> nil) and (AParentLevel.GridView <> nil) and
      ...
      Result := TcxDBDataController(AParentLevel.GridView.DataController)
        .DataSource;
  end;
end;
...
function TcxGridDBChartItemDataBindingFieldNameProperty.GetDataSource
  : TDataSource;
begin
  Result := TcxGridDBChartItemDataBinding(GetComponent(0))
    .DataController.DataSource;
end;

procedure TcxGridDBChartItemDataBindingFieldNameProperty.GetValueList
  (AList: TStrings);
var
  I, J: Integer;
  AField: TField;
  ...
  begin
    ...
    begin
      ...
      for J := 0 to PropCount - 1 do
        if not(GetComponent(J) as TcxGridDBChartItemDataBinding)
          .IsValueTypeClassValid(AValueTypeClass) then
        begin
          AList.Delete(I);
          Break;
        end;
...
function TcxCustomGridColumnOptionsProperty.IsVisibleProperty
  (const APropertyName: string): Boolean;
begin
  Result := (APropertyName <> 'FilterRowOperator') or
    ((Column <> nil) and Column.GridView.FilterRow.OperatorCustomization);
end;
...
type
  ...
  protected
    function GetGridViewForComponent(AIndex: Integer): TcxGridBandedTableView;
      virtual; abstract;
...
function TcxCustomGridBandIndexProperty.GetValueForBandIndex
...
begin
  ...
  begin
    ...
    if GridView.Bands[AIndex].Caption <> '' then
      Result := Result + sIndexCaptionSeparator + GridView.Bands
        [AIndex].Caption;
  end;
end;
...
type
  ...
  protected
    function GetGridViewForComponent(AIndex: Integer)
      : TcxGridBandedTableView; override;
  public
    procedure GetValues(Proc: TGetStrProc); override;
  end;

function TcxGridBandedColumnBandIndexProperty.GetGridViewForComponent
  (AIndex: Integer): TcxGridBandedTableView;
begin
...
function TcxGridBandBandIndexProperty.GetGridViewForComponent(AIndex: Integer)
...
procedure TcxCustomGridTableItemProperty.GetGridViewItemNames
  (AGridView: TcxCustomGridView; Proc: TGetStrProc);
var
  I: Integer;
begin
...
function TcxGridIncSearchItemProperty.InternalGetGridView
...
function TcxGridPreviewColumnProperty.InternalGetGridView
  (APersistent: TPersistent): TcxCustomGridView;
begin
...
function TcxGridTableSummaryItemColumnProperty.InternalGetGridView
  (APersistent: TPersistent): TcxCustomGridView;
begin
  Result := (TcxCustomDataSummaryItem(APersistent)
    .DataController as IcxCustomGridDataController).GridView;
end;
...
function TcxGridChartItemValueTypeProperty.IsValueTypeClassValid(AValueTypeClass
...
begin
  ...
  if Result then
    for I := 0 to PropCount - 1 do
    begin
      Result := (GetComponent(I) as TcxGridChartItemDataBinding)
        .IsValueTypeClassValid(AValueTypeClass);
      if not Result then
        Break;
    end;
end;
...
type
  ...
  TcxExtLookupComboBoxPropertiesItemColumnProperty = class
    (TcxCustomGridTableItemProperty)
  protected
    function InternalGetGridView(APersistent: TPersistent)
      : TcxCustomGridView; override;
...
function TcxExtLookupComboBoxPropertiesFieldNameProperty.GetDataSource
  : TDataSource;
var
...
begin
  ...
  if (AView <> nil) and TcxExtLookupComboBoxProperties.IsViewSupported(AView)
  then
    FProc(Value);
end;
...
begin
  ...
  TcxDataControllerConditionalFormattingRulesManagerDialogProvider.
    RequiresRulesManagerDialogUnits(Designer.Root, Proc);
end;
...
procedure TcxCustomGridTableItemSelectionEditor.RequiresUnits
  (Proc: TGetStrProc);
var
  I: Integer;
...
begin
  ...
  with TfmImageComboBoxItemsEditor.Create(AProperties.Items, AProperties.Images,
    AProperties.LargeImages) do
    try
      Caption := S;
...
function TcxDataGroupSummaryItemsPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
...
type
  ...
  TcxGridColumnLayoutItemPropertyFilter = class(TSelectionEditor,
  ...
  TGridColumnVisibleForEditFormPropertyEditor = class
  ...
  TGridItemVisibleForExpressionEditorPropertyEditor = class
    (TdxDefaultBooleanPropertyEditor)
  protected
    function GetDefaultValue: Boolean; override;
  end;
  ...
  TGridColumnVisibleForRowLayoutPropertyEditor = class
    (TdxDefaultBooleanPropertyEditor)
  protected
    function GetDefaultValue: Boolean; override;
  end;
...
procedure TcxGridColumnLayoutItemPropertyFilter.FilterProperties
...
function TGridColumnCustomLayoutItemProperty.GetLayoutItem
...
begin
  Result := inherited IsLayoutItemAvailable and
    not Column.GridView.EditForm.UseDefaultLayout;
end;
...
begin
  Result := inherited IsLayoutItemAvailable and
    not Column.GridView.RowLayout.UseDefaultLayout;
end;

{ TGridColumnOptionsFilterRowOperatorProperty }

function TGridColumnOptionsFilterRowOperatorProperty.GetColumn
  : TcxCustomGridColumn;
var
  APersistent: TPersistent;
begin
...
procedure TGridColumnOptionsFilterRowOperatorProperty.GetValues
  (Proc: TGetStrProc);
var
  AOperator: TcxFilterOperatorKind;
begin
  for AOperator := Low(TcxFilterOperatorKind) to High(TcxFilterOperatorKind) do
    if (Column <> nil) and Column.GridView.Controller.
      IsFilterRowOperatorSupported(Column.Index, AOperator) then
...
function TGridFindPanelOptionsProperty.GetFindPanelOptions
...
begin
  Result := TcxCustomGridColumnAccess(GetComponent(0))
    .IsDefaultVisibleForEditForm
end;

{ TGridItemVisibleForExpressionEditorPropertyEditor }

function TGridItemVisibleForExpressionEditorPropertyEditor.
  GetDefaultValue: Boolean;
begin
  Result := TcxCustomGridTableItemAccess(GetComponent(0))
...
function TGridItemDataBindingExpressionProperty.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes;
  if PropCount = 1 then
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGrid,
    'RootLevelStylesEvents', TcxGridRootLevelStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridTableItem,
    'PropertiesEvents', TcxGridTableItemPropertiesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridTableItem,
    'StylesEvents', TcxGridTableItemStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridView,
    'DataControllerEvents', TcxGridDataControllerEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridView,
    'StylesEvents', TcxGridViewStylesEventsProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridBand, 'StylesEvents',
    TcxGridBandStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartSeries,
    'StylesEvents', TcxGridChartSeriesStylesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartHistogram,
    'ValuesEvents', TcxGridChartHistogramValuesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'CategoriesEvents', TcxGridChartCategoriesEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramAreaEvents', TcxGridChartAreaDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramBarEvents', TcxGridChartBarDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramStackedBarEvents', TcxGridChartStackedBarDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramStackedColumnEvents',
    TcxGridChartStackedColumnDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramStackedAreaEvents', TcxGridChartStackedAreaDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramColumnEvents', TcxGridChartColumnDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramLineEvents', TcxGridChartLineDiagramEventsProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxGridChartView,
    'DiagramPieEvents', TcxGridChartPieDiagramEventsProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxGridLevel, 'ImageIndex',
    TcxGridLevelImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxCustomGridTableItem,
    'PropertiesClassName', nil);
  RegisterPropertyEditor(TypeInfo(TcxCustomEditProperties),
  ...
  RegisterPropertyEditor(TypeInfo(string), TcxGridItemDataBinding, 'Expression',
    TGridItemDataBindingExpressionProperty);
  RegisterPropertyEditor(TypeInfo(TComponent), TcxCustomGridView, 'PopupMenu',
    TcxControlPopupMenuProperty);

  RegisterPropertyEditor(TypeInfo(Boolean), TcxCustomGridTableOptionsSelection,
    'HideFocusRect', nil);
  RegisterPropertyEditor(TypeInfo(TAlignment), TcxGridBand, 'Alignment', nil);

  RegisterPropertyEditor(TypeInfo(Integer), TcxGridBandedColumnPosition,
    'BandIndex', TcxGridBandedColumnBandIndexProperty);
  RegisterPropertyEditor(TypeInfo(Integer), TcxGridBandPosition, 'BandIndex',
    TcxGridBandBandIndexProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxGridChartItemDataBinding,
    'ValueType', TcxGridChartItemValueTypeProperty);
  RegisterPropertyEditor(TypeInfo(TcxGridChartDiagram), TcxGridChartView,
    'ActiveDiagram', TcxGridChartActiveDiagramProperty);

  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem),
  ...
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem),
    TcxGridTableSummaryItem, 'Column', TcxGridTableSummaryItemColumnProperty);
{$IFNDEF NONDB}
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem),
    TcxGridDBTableSummaryItem, 'Column', TcxGridTableSummaryItemColumnProperty);
{$ENDIF}
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem),
...
{$IFNDEF NONDB}
  RegisterPropertyEditor(TypeInfo(string), TcxDBDataController,
    'DetailKeyFieldNames', TFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxDBDataController, 'KeyFieldNames',
    TFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxDBDataController,
    'MasterKeyFieldNames', TMasterKeyFieldNamesProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxDBDataSummaryItem, 'FieldName',
    TcxDBDataSummaryItemFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxGridItemDBDataBinding,
    'FieldName', TcxGridItemDBDataBindingFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxGridDBChartItemDataBinding,
    'FieldName', TcxGridDBChartItemDataBindingFieldNameProperty);
{$ENDIF}
  RegisterNoIcon([TcxGridLevel, TcxGridTableView,
    {$IFNDEF NONDB}TcxGridDBTableView, {$ENDIF}
  ...
  RegisterNoIcon([TcxGridColumn, {$IFNDEF NONDB}TcxGridDBColumn, {$ENDIF}
  ...
  RegisterNoIcon([TcxGridTableViewStyleSheet, TcxGridBandedTableViewStyleSheet,
    TcxGridCardViewStyleSheet]);

  RegisterComponents(dxCoreLibraryProductPage,
    [TcxGrid, TcxGridViewRepository]);
{$IFNDEF NONDB}
  // Ext Lookup
  ...
  RegisterEditRepositoryItem(TcxEditRepositoryExtLookupComboBoxItem,
    cxSEditRepositoryExtLookupComboBoxItem);
  RegisterPropertyEditor(TypeInfo(string), TcxExtLookupComboBoxProperties,
    'KeyFieldNames', TcxExtLookupComboBoxPropertiesFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableItem),
    TcxExtLookupComboBoxProperties, 'ListFieldItem',
    TcxExtLookupComboBoxPropertiesItemColumnProperty);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridTableView),
    TcxExtLookupComboBoxProperties, 'View',
    TcxExtLookupComboBoxPropertiesViewProperty);
  RegisterPropertyEditor(TypeInfo(TShortCut), TcxExtLookupComboBoxProperties,
...
{$ENDIF}
  ...
  RegisterSelectionEditor(TcxCustomGridTableView,
    TcxCustomTableViewSelectionEditor);
  RegisterSelectionEditor(TcxGridChartView, TcxGridChartViewSelectionEditor);
{$IFNDEF NONDB}
  RegisterSelectionEditor(TcxGridDBTableView,
    TcxCustomDBTableViewSelectionEditor);
  RegisterSelectionEditor(TcxGridDBBandedTableView,
    TcxCustomDBTableViewSelectionEditor);
  RegisterSelectionEditor(TcxGridDBCardView,
    TcxCustomDBTableViewSelectionEditor);
  RegisterSelectionEditor(TcxGridDBChartView,
    TcxGridDBChartViewSelectionEditor);
{$ENDIF}
  RegisterSelectionEditor(TcxCustomGridTableItem,
    TcxCustomGridTableItemSelectionEditor);

  RegisterPropertyEditor(TypeInfo(TcxImageComboBoxItems),
    TcxImageComboBoxProperties, 'Items', TcxImageComboBoxItemsProperty);

  /// //////////////////////////////////////// GRID 7 /////////////////////////////////////////////////
  RegisterPropertyEditor(TypeInfo(TPersistent), TcxCustomGridTableView,
    'NavigatorButtons', nil);
  RegisterPropertyEditor(TypeInfo(TPersistent), TcxCustomGridTableOptionsView,
    'Navigator', nil);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomGridTableView,
    'NavigatorEvents', TcxGridViewNavigatorEventsProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TcxDataGroupSummaryItems), TcxDataSummary, '',
    TcxDataGroupSummaryItemsPropertyEditor);
  /// /////////////////////////////////////// 14.1 ////////////////////////////////////////////////////
  DesignIntf.RegisterSelectionEditor(TcxGridColumn,
    TcxGridColumnLayoutItemPropertyFilter);
  ...
  HideClassProperties(TcxGridInplaceEditFormLayoutItem,
    ['LayoutLookAndFeel', 'Name']);
  HideClassProperties(TcxGridTableRowLayoutItem, ['LayoutLookAndFeel', 'Name']);
  dxHideClassesFromStructureView([TcxGridInplaceEditFormLayoutItem,
    TcxGridInplaceEditFormGroup, TcxGridTableRowLayoutItem,
    TcxGridTableRowLayoutGroup]);

  RegisterPropertyEditor(TypeInfo(TcxGridFindPanelOptions),
    TcxCustomGridTableView, 'FindPanel', TGridFindPanelOptionsProperty);
  RegisterPropertyEditor(TypeInfo(TcxGridInplaceEditFormLayoutItem),
    TcxGridColumn, 'LayoutItem', TGridColumnLayoutItemProperty);
  RegisterPropertyEditor(TypeInfo(TcxGridTableRowLayoutItem), TcxGridColumn,
    'RowLayoutItem', TGridColumnRowLayoutItemProperty);
  RegisterPropertyEditor(TypeInfo(TdxDefaultBoolean), TcxGridColumn,
    'VisibleForEditForm', TGridColumnVisibleForEditFormPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxDefaultBoolean), TcxCustomGridTableItem,
    'VisibleForExpressionEditor',
    TGridItemVisibleForExpressionEditorPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxDefaultBoolean), TcxGridColumn,
    'VisibleForRowLayout', TGridColumnVisibleForRowLayoutPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TcxCustomGridColumnOptions), TcxGridColumn,
    'Options', TcxCustomGridColumnOptionsProperty);
  RegisterPropertyEditor(TypeInfo(TcxFilterOperatorKind),
    TcxCustomGridColumnOptions, 'FilterRowOperator',
    TGridColumnOptionsFilterRowOperatorProperty);
end;
