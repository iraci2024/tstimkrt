...
uses
  Windows, Messages, Types, Variants, SysUtils, Classes, Controls, Graphics,
  StdCtrls,
  Forms, Math, ImgList,
  dxCore, dxMessages, cxClasses, cxGeometry, cxControls, cxContainer,
  cxGraphics, cxVariants, dxGDIPlusClasses,
  cxDataUtils, cxEdit, cxListBox, cxCheckBox, cxExtEditConsts, cxExtEditUtils,
  cxScrollBar,
  cxLookAndFeels, cxLookAndFeelPainters, cxCustomListBox, dxCoreClasses;
...
type
  ...
  protected
    ...
    procedure LockChanged(ALock: Boolean;
  ...
  public
    ...
    // for internal use
    property Items[Index: Integer]: TcxCheckListBoxItem read GetItems
  ...
  protected
    ...
    procedure SynchronizeCheckStates(ANewHotCheckIndex, ANewPressedCheckIndex
      : Integer);
    ...
    property AllowGrayed: Boolean read FAllowGrayed write FAllowGrayed
      default False;
    property CheckItems: TcxCheckListBoxItems read FCheckItems
      write FCheckItems;
    ...
    property OnClickCheck: TcxClickCheckEvent read FOnClickCheck
      write FOnClickCheck;
  public
  ...
  protected
    ...
    function GetInnerCheckListBoxClass
      : TcxCustomInnerCheckListBoxClass; virtual;
    ...
    function IsValueValid(const AValue: TcxEditValue; AAllowEmpty: Boolean)
      : Boolean; virtual;
    ...
    property OnClickCheck: TcxClickCheckEvent read GetOnClickCheck
      write SetOnClickCheck;
    property OnCompare: TcxCollectionCompareEvent read GetOnCompare
      write SetOnCompare;
  ...
  public
    ...
    property Count;
    property InnerCheckListBox: TcxCustomInnerCheckListBox
      read GetInnerCheckListBox; // for internal use
...
begin
  ...
  begin
    ...
    if (Value = cbsGrayed) and
      (CheckListBox.Container.EditValueFormat = cvfInteger) then
      Value := cbsUnchecked;
...
procedure TcxCheckListBoxItems.SetItems(Index: Integer;
  const Value: TcxCheckListBoxItem);
begin
...
begin
  ...
  else if FChangedLockCount > 0 then
  begin
...
begin
  ...
  if (Container.ImageLayout = ilBeforeChecks) and
    IsImageAssigned(Container.Images, 0) then
    Inc(ACheckOffset, dxGetImageSize(Container.Images, ScaleFactor).cx +
      Metrics.ImageFrameWidth * 2);
...
procedure TcxCustomInnerCheckListBox.GetCheckMetrics(out ACheckSize: TSize;
  out ACheckBorderOffset: Integer);
begin
  ACheckSize := GetScaledEditCheckSize(Self.Canvas,
    Container.Style.LookAndFeel.NativeStyle, Glyph, GlyphCount,
    Container.Style.LookAndFeel.SkinPainter, ScaleFactor);
  ACheckBorderOffset := GetEditCheckBorderOffset
    (Container.Style.LookAndFeel.Kind, Container.FNativeStyle,
    IsGlyphAssigned(Glyph) and (GlyphCount > 0), LookAndFeel.SkinPainter);
end;
...
begin
  ...
  if IsImageAssigned(Container.Images, 0) then
    Inc(Result, dxGetImageSize(Container.Images, ScaleFactor).cx +
      Metrics.ImageFrameWidth * 2);
end;
...
begin
  ...
  Result.TextWidthCorrection :=
    ScaleFactor.Apply(cxCheckListBoxTextWidthCorrection);
end;
...
begin
  ...
  begin
    GetCheckMetrics(ACheckSize, ACheckBorderOffset);
    Result := Max(Result, ACheckSize.cy - ACheckBorderOffset * 2 +
      Metrics.CheckFrameWidth * 2);
  end;
  if IsImageAssigned(Container.Images, 0) then
    Result := Max(Result, dxGetImageSize(Container.Images, ScaleFactor).cy +
      Metrics.ImageFrameWidth * 2);
  dxAdjustToTouchableSize(Result, ScaleFactor);
end;
...
function TcxCustomInnerCheckListBox.ExecuteAction(Action: TBasicAction)
  : Boolean;
begin
...
procedure TcxCustomInnerCheckListBox.DrawCheck(R: TRect;
  ...
  begin
    ...
    if (Result = ebsFlat) and not Container.FNativeStyle and
...
begin
  ...
  DrawScaledEditCheck(Canvas, R, AState, ACheckState, Glyph, GlyphCount,
    GetCheckBorderStyle, Container.FNativeStyle, clBtnText, Color, True,
    Container.IsDesigning, False, True, LookAndFeel.SkinPainter, ScaleFactor);
end;
...
begin
  ...
  if Assigned(Container.OnEditValueChanged) then
    AEditValueChanged := not InternalVarEqualsExact(Container.FEditValue,
      ANewEditValue)
  else
...
begin
  ...
  begin
    ...
    if (ANewPressedCheckIndex <> -1) and
      ((Container.DragMode = dmAutomatic) or
      not FNewPressedCheckItemFullyVisible) and
...
procedure TcxCustomInnerCheckListBox.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TcxCustomInnerCheckListBox.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  ACheckIndex: Integer;
  ANewHotCheckIndex, ANewPressedCheckIndex: Integer;
...
begin
  if (Index < 0) or (Index >= CheckItems.Count) or not CheckItems[Index].Enabled
  then
    Exit;
...
begin
  with GetBorderExtent do
    Result := InnerCheckListBox.GetStandardItemHeight * ARowCount + Top
      + Bottom;
end;
...
begin
  with GetBorderExtent do
    Result := Left + Right;
  Inc(Result, InnerCheckListBox.GetCheckRegionWidth +
    TdxTextMeasurer.TextWidthTO(Font, Items[AIndex].Text) +
    InnerCheckListBox.Metrics.TextWidthCorrection);
end;
...
function TcxCustomCheckListBox.ItemAtPos(const APos: TPoint;
  AExisting: Boolean): Integer;
begin
  Result := InnerCheckListBox.ItemAtPos(Point(APos.X - InnerCheckListBox.Left,
    APos.Y - InnerCheckListBox.Top), AExisting);
end;
...
procedure TcxCustomCheckListBox.CopySelection(ADestination
  : TcxCustomCheckListBox);
begin
  if ItemIndex <> -1 then
...
procedure TcxCustomCheckListBox.MoveSelection(ADestination
  : TcxCustomCheckListBox);
begin
...
const
  BorderStyleMap: array [TcxLookAndFeelKind] of TcxEditCheckBoxBorderStyle =
...
function TcxCustomCheckListBox.GetRealEditValueFormat
  : TcxCheckStatesValueFormatEx;
begin
  if Assigned(OnEditValueToCheckStates) and Assigned(OnCheckStatesToEditValue)
  then
    Result := cvfCustom
  else
...
procedure TcxCustomCheckListBox.ItemsChanged(Sender: TObject;
...
procedure TcxCustomCheckListBox.DrawItem(Control: TWinControl; Index: Integer;
  ...
  begin
    ...
    if (csvTextColor in StyleDisabled.AssignedValues) or (AColor = clDefault)
    then
  ...
  begin
    ...
    begin
      InnerCheckListBox.Canvas.Font.Color :=
        LookAndFeelPainter.DefaultSelectionTextColor;
      InnerCheckListBox.Canvas.Brush.Color :=
        LookAndFeelPainter.DefaultSelectionColor;
    end
    else
    begin
      InnerCheckListBox.Canvas.Brush.Color := ViewInfo.BackgroundColor;
      InnerCheckListBox.Canvas.Font.Color := VisibleFontColor;
    end;
  ...
  begin
    if not AIsItemEnabled then
      Result := ecsDisabled
    else if InnerCheckListBox.FHotCheckIndex = Index then
      Result := ecsHot
    else if InnerCheckListBox.FPressedCheckIndex = Index then
      Result := ecsPressed
    else
      Result := ecsNormal;
  end;
  ...
  procedure DrawCheckRegion(const ACheckRegion: TRect; AIsItemEnabled: Boolean);
    ...
    begin
      ...
      Result.Top := ACheckRegion.Top + (ACheckRegion.Bottom - ACheckRegion.Top -
        ASize.cy) div 2;
    ...
    begin
      ...
      try
        ...
        cxDrawImage(ABitmap.cxCanvas.Handle, ABitmap.ClientRect,
          ABitmap.ClientRect, nil, Images, AImageIndex,
          EnabledImageDrawModeMap[AIsItemEnabled]);
        cxBitBlt(InnerCheckListBox.Canvas.Handle, ABitmap.cxCanvas.Handle, R,
          cxNullPoint, SRCCOPY);
      finally
        ABitmap.Free;
      end;
...
begin
  ...
  begin
    ...
    PrepareColors(AEnabled);
    APrevMode := SetBkMode(InnerCheckListBox.Canvas.Handle,
      Windows.TRANSPARENT);
...
procedure TcxCustomCheckListBox.DrawItemText(const AText: string;
  const ARect: TRect);
const
...
begin
  ...
  R := ARect;
  Windows.DrawTextEx(InnerCheckListBox.Canvas.Handle, PChar(AText),
    Length(AText), R, ABaseTestFlag, @ADrawTextParams);
end;
...
begin
  inherited WndProc(Message);
  if (InnerCheckListBox <> nil) and (Message.Msg = WM_COMMAND) and
...
procedure TcxCustomCheckListBox.SetEditValueFormat
...
procedure TcxCustomCheckListBox.SetImageLayout
  (Value: TcxCheckListBoxImageLayout);
begin
...
function TcxCustomCheckListBox.GetInnerCheckListBoxClass
...
procedure TcxCustomCheckListBox.InternalKeyDown(var Key: Word;
  Shift: TShiftState);
begin
...
function TcxCustomCheckListBox.IsMouseWheelHandleNeeded(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := False;
end;
