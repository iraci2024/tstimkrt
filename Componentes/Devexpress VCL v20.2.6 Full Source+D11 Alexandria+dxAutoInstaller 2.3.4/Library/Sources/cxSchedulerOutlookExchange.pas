...
type
  TcxSchedulerExchangeProgressProc = procedure(ACurrent, ACount: Integer;
    var Abort: Boolean);
  TcxAcceptEventExportFunc = function(AEvent: TcxSchedulerEvent): Boolean;
  TcxAcceptAppointmentItemImportFunc = function(const AppointmentItem
    : OleVariant): Boolean;
  TcxAppointmentItemToEventProc = procedure(const AppointmentItem: OleVariant;
    AEvent: TcxSchedulerEvent);
  TcxEventToAppointmentItemProc = procedure(AEvent: TcxSchedulerEvent;
  ...
  public
    procedure Add(AEvent: TcxSchedulerEvent;
      AEntryIDField: TcxCustomSchedulerStorageField);
  ...
  protected
    function CanAcceptAppointmentItem(const AppointmentItem
      : OleVariant): Boolean;
    function CanAcceptEvent(AEvent: TcxSchedulerEvent): Boolean;
    function CanImportAppointmentItem(const AppointmentItem: OleVariant)
      : Boolean; virtual;
    ...
    procedure CheckRecurrencePatternExceptions(const ARecurrencePattern
      : OleVariant; APatternEvent: TcxSchedulerEvent);
    procedure ExportRecurrenceChain(APatternEvent: TcxSchedulerEvent;
      const AppointmentItem: OleVariant);
    procedure ExportRecurrenceException(APatternEvent: TcxSchedulerEvent;
      const AStart: TDateTime);

    function GetIsValid: Boolean; virtual;
    function InitOleObject: Boolean;
    procedure PostCustomOccurrence(const ARecurrencePattern,
      AException: OleVariant; APatternEvent: TcxSchedulerEvent);
    procedure PostRecurrenceException(AOccurrence: TcxSchedulerEvent); virtual;
    function RoundTime(ATime: TDateTime; ASaveDate: Boolean)
      : TDateTime; virtual;
  ...
  public
    ...
    procedure ImportAppointmentItemFromOutlook(const AppointmentItem
      : OleVariant);
    ...
    property OnAcceptEvent: TcxAcceptEventExportFunc read FOnAcceptEvent
      write FOnAcceptEvent;
    property OnConvertAppointmentItemToEvent: TcxAppointmentItemToEventProc
      read FOnConvertAppointmentItemToEvent
      write FOnConvertAppointmentItemToEvent;
    property OnConvertEventToAppointmentItem: TcxEventToAppointmentItemProc
      read FOnConvertEventToAppointmentItem
      write FOnConvertEventToAppointmentItem;
    property OnProgress: TcxSchedulerExchangeProgressProc read FOnProgress
      write FOnProgress;
  end;
  ...
  protected
    procedure DeleteEventExceptions(AEvent: TcxSchedulerEvent;
      AEntryIDs: TcxEventEntryIDList);
    procedure DeleteUnsynchronizedAppointments
    ...
    procedure SynchronizeRecurrenceChainWithOutlook(APatternEvent
      : TcxSchedulerEvent; const ARecurrencePattern: OleVariant;
      AEntryIDs: TcxEventEntryIDList);
  public
    procedure SynchronizeWithOutlook(ADeleteUnsynchronizedEvents
      : Boolean = False);
    procedure SynchronizeWithStorage(ADeleteUnsynchronizedAppointments
      : Boolean = False);

    property EntryIDField: TcxCustomSchedulerStorageField read FEntryIDField
      write FEntryIDField;
  end;
...
function TcxAppointmentEntryIDList.ExtractAppointmentItem(AIndex: Integer)
  : OleVariant;
var
...
function TcxAppointmentEntryIDList.FindEntryID(const AEntryID: string;
  var Index: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
  Result := False;
  if Length(AEntryID) = 0 then
    Exit;
  L := 0;
  H := Count - 1;
  while L <= H do
...
function TcxAppointmentEntryIDList.IndexOfEntryID(const AEntryID
  : string): Integer;
begin
...
begin
  Result := CompareStr(TcxAppointmentInfo(Item1).EntryID,
    TcxAppointmentInfo(Item2).EntryID);
end;
...
constructor TcxEventInfo.Create(AEvent: TcxSchedulerEvent;
  AEntryIDField: TcxCustomSchedulerStorageField);
begin
  FEvent := AEvent;
...
function TcxEventEntryIDList.IndexOfEntryID(const AEntryID: string): Integer;
begin
  if not FindEntryID(AEntryID, Result) then
    Result := -1;
end;
...
begin
  Result := CompareStr(TcxEventInfo(Item1).EntryID,
    TcxEventInfo(Item2).EntryID);
end;
...
begin
  ...
  begin
    ...
    if CanImportAppointmentItem(AppointmentItem) and
      CanAcceptAppointmentItem(AppointmentItem) then
...
function TcxOutlookExchange.CanAcceptAppointmentItem(const AppointmentItem
  : OleVariant): Boolean;
begin
...
function TcxOutlookExchange.CanImportAppointmentItem(const AppointmentItem
  : OleVariant): Boolean;
begin
  Result := not(AppointmentItem.IsRecurring and
    not Storage.IsRecurrenceAvailable);
end;

procedure TcxOutlookExchange.ConvertAppointmentItemToEvent(const AppointmentItem
  : OleVariant; AEvent: TcxSchedulerEvent);
begin
...
procedure TcxOutlookExchange.ImportRecurrenceChain(APatternEvent
  : TcxSchedulerEvent; const ARecurrencePattern: OleVariant);
const
...
begin
  ...
  begin
    ...
    if not ADeleted and not CanAcceptAppointmentItem(AException.AppointmentItem)
    then
      Continue;
...
begin
  ...
  case AType of
    ...
    olRecursMonthNth:
      begin
        AEvent.RecurrenceInfo.DayType :=
          GetDayTypeFromMask(ARecurrencePattern.DayOfWeekMask);
        AEvent.RecurrenceInfo.Recurrence := cxreMonthly;
        AEvent.RecurrenceInfo.Periodicity := ARecurrencePattern.Interval;
...
procedure TcxOutlookExchange.PostCustomOccurrence(const ARecurrencePattern,
  AException: OleVariant; APatternEvent: TcxSchedulerEvent);
var
...
begin
  ...
  begin
    with TcxSchedulerOccurrenceCalculator.Create(APatternEvent,
      AOccurrenceStart, AOriginalDate - 1) do
      try
  ...
  begin
    ...
    begin
      ...
      if (dxDateOf(AOccurrence.Start) = dxDateOf(AOriginalDate)) and
        (AOccurrence.EventType = etException) then
      begin
  ...
  begin
    with TcxSchedulerOccurrenceCalculator.Create(APatternEvent,
      AOccurrenceStart, AOccurrenceStart) do
      try
        if GetNextOccurrence and
          (dxDateOf(OccurrenceStart) = dxDateOf(AOccurrenceStart)) then
          ExportRecurrenceException(APatternEvent, OccurrenceStart);
      finally
        Free;
      end;
  ...
  AOccurrence := Storage.CreateOccurrence(APatternEvent,
    AException.OriginalDate, etCustom);
  SetupEventInfo(AOccurrence, AException.AppointmentItem);
  UpdateEvent(AOccurrence, AException.AppointmentItem);
end;

procedure TcxOutlookExchange.PostRecurrenceException
  (AOccurrence: TcxSchedulerEvent);
begin
  AOccurrence.Post;
end;

function TcxOutlookExchange.RoundTime(ATime: TDateTime; ASaveDate: Boolean)
  : TDateTime;
begin
  if ASaveDate then
...
procedure TcxOutlookExchange.UpdateAppointmentItem(const AppointmentItem
  : OleVariant; AEvent: TcxSchedulerEvent);
begin
...
procedure TcxOutlookExchange.UpdateEventWithRecurrenceInfo
  (AEvent: TcxSchedulerEvent; const AppointmentItem: OleVariant);
begin
  AEvent.BeginEditing;
  try
...
begin
  ...
  begin
    if AEvent1.EventType = AEvent2.EventType then
      Result := 0
    else if AEvent1.EventType = etException then
      Result := -1
    else
      Result := 1;
  end
...
procedure TcxOutlookExchange.CheckRecurrencePatternExceptions
  (const ARecurrencePattern: OleVariant; APatternEvent: TcxSchedulerEvent);
var
  I: Integer;
  AEvent: TcxSchedulerEvent;
  AppointmentItem: OleVariant;
  ...
  with APatternEvent.GetRecurrenceChain do
    try
      ...
      begin
        AEvent := Items[I];
        try
          AppointmentItem := ARecurrencePattern.GetOccurrence
            (AEvent.GetOriginalDate);
          if AEvent.EventType = etException then
            AppointmentItem.Delete
          else if (AEvent.EventType = etCustom) and CanAcceptEvent(AEvent) then
          begin
            SetupAppointmentItemInfo(AppointmentItem, AEvent);
            UpdateAppointmentItem(AppointmentItem, AEvent);
          end;
...
procedure TcxOutlookExchange.ExportRecurrenceChain(APatternEvent
  : TcxSchedulerEvent; const AppointmentItem: OleVariant);
var
  ARecurrencePattern: OleVariant;
begin
...
procedure TcxOutlookExchange.ExportRecurrenceException(APatternEvent
  : TcxSchedulerEvent; const AStart: TDateTime);
var
  AOccurrence: TcxSchedulerEvent;
begin
    ...
    case Recurrence of
      cxreDaily:
        begin
          ...
          else if DayType = cxdtWeekDay then
          begin
...
function TcxOutlookExchange.GetAllDayEvent(const AppointmentItem
  : OleVariant): Boolean;
var
  ARecurrencePattern: OleVariant;
begin
  ...
  begin
    ...
    begin
      ARecurrencePattern := AppointmentItem.GetRecurrencePattern;
      if Integer(ARecurrencePattern.RecurrenceType)
        in [olRecursMonthly, olRecursMonthNth, olRecursYearly, olRecursYearNth]
      then
      begin
...
procedure TcxOutlookSynchronize.UpdateAppointmentItem(const AppointmentItem
  : OleVariant; AEvent: TcxSchedulerEvent);
var
  AEntryID: OleVariant;
begin
  ...
  try
    ...
    if not VarEqualsSoft(AEvent.GetCustomFieldValue(EntryIDField), AEntryID)
    then
    begin
    ...
    begin
      ...
      begin
        ...
        if not CanAcceptAppointmentItem(AppointmentItem) then
          Continue;
        if AOccurrence = nil then
          AOccurrence := Storage.CreateOccurrence(APatternEvent, AOriginalDate,
            etCustom);
...
begin
  ...
  begin
    ...
    begin
      ...
      UpdateEventWithRecurrenceInfo(AEvent, AppointmentItem);
      SynchronizeRecurrenceChainWithOutlook(AEvent, ARecurrencePattern,
        AEntryIDs);
    end
    else
    begin
...
begin
  ...
  for I := AEntryIDs.Count - 1 downto 0 do
    if (AEntryIDs[I].Event <> AEvent) and (AEntryIDs[I].Event.Pattern = AEvent)
    then
      AEntryIDs.Delete(I);
end;
...
procedure TcxOutlookSynchronize.PurgeDeletedOccurrences(APatternEvent
  : TcxSchedulerEvent; AEntryIDs: TcxEventEntryIDList);
var
  AEvent: TcxSchedulerEvent;
...
function TcxOutlookSynchronize.IsEqualRecurrencePattern(const AppointmentItem
  : OleVariant; AEvent: TcxSchedulerEvent): Boolean;
var
  ARecurrencePattern: OleVariant;
begin
  with AEvent do
  ...
  begin
    ...
    with AEvent.RecurrenceInfo do
    begin
      Result := (ARecurrencePattern.PatternStartDate = dxDateOf(AEvent.Start))
        and (RoundTime(ARecurrencePattern.StartTime, False)
        = RoundTime(AEvent.Start, False)) and
        (RoundTime(ARecurrencePattern.EndTime, False)
        = RoundTime(AEvent.Finish, False));
      if not Result then
        Exit;
      case Recurrence of
        cxreDaily:
        ...
        cxreWeekly:
          Result := (ARecurrencePattern.RecurrenceType = olRecursWeekly) and
            (ARecurrencePattern.DayOfWeekMask = GetMaskFromDaysOfWeek(OccurDays)
            ) and (ARecurrencePattern.Interval = Periodicity);
        cxreMonthly:
          if DayType = cxdtDay then
            ...
            Result := (ARecurrencePattern.RecurrenceType = olRecursMonthNth) and
              ...
              (ARecurrencePattern.DayOfWeekMask = GetMaskFromDayType(DayType))
              and (ARecurrencePattern.Instance = DayNumber);
        cxreYearly:
          if DayType = cxdtDay then
...
procedure TcxOutlookExchange.SetupAppointmentItemInfo(const AppointmentItem
  : OleVariant; const AEvent: TcxSchedulerEvent; ASetTime: Boolean = True);
var
