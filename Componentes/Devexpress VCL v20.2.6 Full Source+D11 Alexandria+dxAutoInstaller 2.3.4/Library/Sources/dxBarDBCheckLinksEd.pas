  ...
  begin
    ...
    begin
      if ComboBox.ItemIndex > -1 then
        SelectedDBCheckLink.Item :=
          TdxBarItem(ComboBox.Items.Objects[ComboBox.ItemIndex]);
      with SelectedDBCheckLink do
      begin
        EnableTypes := [];
        if CheckBox1.Checked then
          EnableTypes := EnableTypes + [dxdbtCanModify];
        if CheckBox2.Checked then
          EnableTypes := EnableTypes + [dxdbtNotEOF];
        if CheckBox3.Checked then
          EnableTypes := EnableTypes + [dxdbtNotBOF];
        if CheckBox4.Checked then
          EnableTypes := EnableTypes + [dxdbtHasRecords];
        if CheckBox5.Checked then
          EnableTypes := EnableTypes + [dxdbtIsModified];
        if CheckBox6.Checked then
          EnableTypes := EnableTypes + [dxdbtIsNotModified];
      end;
    end;
    SelectedDBCheckLink := TdxBarDBCheckLink
