...
type
  ...
  TcxSchedulerAggregateStorageEditingEventInfo = class
  ...
  TcxSchedulerAggregateStorageEditingEventInfoList = class
  ...
  protected
    function CreateEvent(ASource, AOriginal: TcxSchedulerEvent;
      AForcePatternEditing: Boolean): TcxSchedulerControlEvent; override;
  ...
  public
    ...
    property AggregateStorage: TcxSchedulerAggregateStorage
      read GetAggregateStorage;
  ...
  public
    ...
    function AddStorageLink(AStorage: TcxCustomSchedulerStorage)
      : TcxSchedulerStorageLink;
    function GetStorageLinkIndex(AStorage: TcxCustomSchedulerStorage): Integer;
    procedure PopulateHolidays(AList: TList);

    property AggregateStorage: TcxSchedulerAggregateStorage
      read FAggregateStorage;
    property DefaultLink: TcxSchedulerStorageLink read FDefaultLink
      write SetDefaultLink;
    property DefaultStorage: TcxCustomSchedulerStorage read GetDefaultStorage;
    property Items[AIndex: Integer]: TcxSchedulerStorageLink read GetItem
  ...
  protected
    ...
    function GetValueDefault(AField: TcxCustomSchedulerStorageField;
      const ADefValue: Variant): Variant; override;
  ...
  TcxSchedulerEventInsertingEvent = procedure
    (Sender: TcxSchedulerAggregateStorage; AEvent: TcxSchedulerEvent;
    var AStorage: TcxCustomSchedulerStorage) of object;

  TcxSchedulerAggregateStorage = class(TcxCustomSchedulerStorage,
  ...
  protected
    ...
    procedure DoEventInserting(AEvent: TcxSchedulerEvent;
      out AStorage: TcxCustomSchedulerStorage); virtual;
    function DoFilterEvent(AEvent: TcxSchedulerEvent): Boolean; override;
    procedure DoRefresh; override;
    function GetEditingEventInfoListClass
      : TcxSchedulerEditingEventInfoListClass; override;
    function GetEventClass: TcxSchedulerEventClass; override;
    function GetEventBySource(AEvent: TcxSchedulerEvent)
      : TcxSchedulerAggregateStorageEvent;
    function InternalAddEvent(AEvent: TcxSchedulerEvent)
      : TcxSchedulerAggregateStorageEvent; virtual;
    function IsDesigning: Boolean;
    function IsDestroying: Boolean;
    procedure PopulateEventsFromStorage
      (AStorage: TcxCustomSchedulerStorage); virtual;
  ...
  public
    ...
    procedure PopulateHolidayDates(AList: TcxSchedulerDateList;
      ...
      AClearList: Boolean = True); override;
  published
    ...
    property OnEventInserting: TcxSchedulerEventInsertingEvent
      read FOnEventInserting write FOnEventInserting;
...
function TcxSchedulerAggregateStorageEditingEventInfoList.IsEventEditing
...
function TcxSchedulerAggregateStorageEditingEventInfoList.CreateEvent(ASource,
  AOriginal: TcxSchedulerEvent; AForcePatternEditing: Boolean)
  : TcxSchedulerControlEvent;
begin
  Result := inherited CreateEvent(cxGetRootEvent(ASource),
    cxGetRootEvent(AOriginal), AForcePatternEditing);
end;

function TcxSchedulerAggregateStorageEditingEventInfoList.GetEventID
  (AEvent: TcxSchedulerEvent): Variant;
begin
  Result := cxGetRootEventID(AEvent);
end;

function TcxSchedulerAggregateStorageEditingEventInfoList.GetInfoClass
  : TcxSchedulerEditingEventInfoClass;
begin
  Result := TcxSchedulerAggregateStorageEditingEventInfo;
end;

function TcxSchedulerAggregateStorageEditingEventInfoList.GetItemCore
  (AIndex: Integer): TcxSchedulerEditingEventInfo;
var
  I: Integer;
  ACount: Integer;
...
function TcxSchedulerAggregateStorageEditingEventInfoList.GetRootEvent
  (AEvent: TcxSchedulerEvent): TcxSchedulerEvent;
begin
  Result := cxGetRootEvent(AEvent);
end;

function TcxSchedulerAggregateStorageEditingEventInfoList.GetStorage
  : TcxSchedulerAggregateStorage;
begin
...
function TcxSchedulerStorageLink.GetAggregateStorage
  : TcxSchedulerAggregateStorage;
begin
...
begin
  if (Links.GetStorageLinkIndex(AValue) = -1) and (AValue <> AggregateStorage)
  then
  begin
    ...
    else if Links.DefaultLink = Self then
...
constructor TcxSchedulerStorageLinks.CreateEx
...
begin
  if AStorage <> nil then
    for Result := 0 to Count - 1 do
      if Items[Result].Storage = AStorage then
        Exit;
  Result := -1;
end;
  ...
  for I := 0 to Count - 1 do
    ...
    begin
      if (Storage <> nil) then
        ...
        else if Storage.Holidays <> nil then
          AList.Add(Storage.Holidays);
    end;
  cxMakeUniqueList(AList);
end;

function TcxSchedulerStorageLinks.GetItemLinkClass
  : TcxSchedulerStorageLinkClass;
begin
...
function TcxSchedulerStorageLinks.GetItem(AIndex: Integer)
  : TcxSchedulerStorageLink;
begin
...
begin
  if ((AValue = nil) or ((AValue.Storage <> nil) and not AValue.IsDestroying))
    and (FDefaultLink <> AValue) then
  begin
    FDefaultLink := AValue;
    Changed;
  end;
...
function TcxSchedulerAggregateStorageEvent.GetValueByIndex
  (AIndex: Integer): Variant;
begin
  if IsEditing then
    Result := EditValues[AIndex]
  else if SourceEvent <> nil then
    Result := TcxSchedulerAggregateStorageEvent(SourceEvent)
      .GetValueByIndex(AIndex)
  else
...
procedure TcxSchedulerAggregateStorageEvent.SetValue
...
function TcxSchedulerAggregateStorageEvent.GetSourceStorage
...
function TcxSchedulerAggregateStorageEvent.GetStorage
  : TcxSchedulerAggregateStorage;
begin
  Result := TcxSchedulerAggregateStorage(inherited Storage);
end;

procedure TcxSchedulerAggregateStorageEvent.SetSourceEvent
...
procedure TcxSchedulerAggregateStorage.GenerateHolidayEvents(const AResourceID
  : Variant);
var
  AHolidaysList: TList;
  I: Integer;
begin
  ...
  try
    ...
    for I := 0 to AHolidaysList.Count - 1 do
      GenerateHolidayEventsBySchedulerHolidays(AResourceID,
        TcxSchedulerHolidays(AHolidaysList[I]));
  finally
    AHolidaysList.Free;
  end;
end;

function TcxSchedulerAggregateStorage.GetHolidayNamesByDate(ADate: TDate;
...
begin
  ...
  try
    Links.PopulateHolidays(AHolidaysList);
    for I := 0 to AHolidaysList.Count - 1 do
      TcxSchedulerHolidays(AHolidaysList.Items[I]).GetHolidayNamesByDate(ADate,
        ANames, AOnlyVisible);
  finally
...
begin
  Result := (Links <> nil) and (Links.Count > 0) and
    (Links.DefaultStorage <> nil) and
    (Links.DefaultStorage.IsActive or Assigned(FOnEventInserting));
  if Result then
    Result := CanRefresh;
end;

procedure TcxSchedulerAggregateStorage.PopulateHolidayDates
...
procedure TcxSchedulerAggregateStorage.DoEventInserting
  (AEvent: TcxSchedulerEvent; out AStorage: TcxCustomSchedulerStorage);
begin
  if AEvent.Pattern <> nil then
    AStorage := TcxSchedulerAggregateStorageEvent(AEvent.Pattern)
      .SourceEvent.Storage
  else if TcxSchedulerAggregateStorageEvent(AEvent).SourceEvent <> nil then
...
function TcxSchedulerAggregateStorage.DoFilterEvent
...
begin
  ...
  begin
    AStorage := TcxCustomSchedulerStorageAccess
      (TcxSchedulerAggregateStorageEvent(AEvent).SourceEvent.Storage);
    Result := AStorage.DoFilterEvent(TcxSchedulerAggregateStorageEvent(AEvent)
      .SourceEvent);
  end
  else
    Result := True;
...
function TcxSchedulerAggregateStorage.GetEditingEventInfoListClass
  : TcxSchedulerEditingEventInfoListClass;
begin
  Result := TcxSchedulerAggregateStorageEditingEventInfoList;
end;

function TcxSchedulerAggregateStorage.GetEventClass: TcxSchedulerEventClass;
begin
...
begin
  ...
  begin
    ...
    if Result.SourceEvent = AEvent then
      Exit;
  end;
  Result := nil;
end;
...
begin
  Result := nil;
  if FIsPostEvent then
    Exit;
  Result := GetEventClass.Create(Self, True)
    as TcxSchedulerAggregateStorageEvent;
...
var
  I: Integer;
begin
  if not Assigned(AStorage) or (AStorage.EventCount = 0) then
    Exit;
...
begin
  ...
  begin
    ...
    try
      ...
      if AEvent.Pattern <> nil then
      begin
        ASource.ParentID := TcxSchedulerAggregateStorageEvent(AEvent.Pattern)
          .SourceEvent.ID;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        else if AEvent.ParentID >= 0 then
        ...
        begin
          ...
          if APattern <> nil then
            ASource.ParentID := TcxSchedulerAggregateStorageEvent(APattern)
              .SourceEvent.ID;
          AEvent.EndEditing;
          ASource.Post;
...
procedure TcxSchedulerAggregateStorage.SendNotification
  (AEvent: TcxSchedulerEvent; AIsRemoved: Boolean = False;
  ACheckLockCount: Boolean = True);
begin
...
function TcxSchedulerAggregateStorage.GetCustomFields
  : TcxSchedulerStorageFields;
begin
  Result := TcxSchedulerStorageFields(inherited CustomFields);
end;

procedure TcxSchedulerAggregateStorage.SetCustomFields
