...
type
  ...
  protected
    ...
    function DoCreateField(APivotGridField: TcxPivotGridField): TField;
      override;
...
function TcxCustomPivotGridSummaryDataSet.GetFieldType(APivotGridField
...
procedure TcxCustomPivotGridSummaryDataSet.DataChanged
...
procedure TcxCustomPivotGridSummaryDataSet.LayoutChanged
  (Sender: TcxCustomPivotGrid);
begin
  if SynchronizeData then
    CreateData;
...
function TcxCustomPivotGridSummaryDataSet.DoCreateField(APivotGridField
...
var
  ...
  procedure DrillColumn(ARowItem, AColumnItem: TcxPivotGridGroupItem);
    ...
    begin
      if (VarType(AValue) = varString) and
        not(Fields[AFieldIndex] is TStringField) then
  ...
  begin
    ...
    begin
      ...
      for I := 0 to DataBuilder.DataFields.Count - 1 do
      begin
        AValue := DataBuilder.CrossCells[ARowItem, AColumnItem]
          .GetCrossCellValue(DataBuilder.DataFields[I]);
        if not VarIsNull(AValue) or
          ((DataBuilder.DataFields[I].DataBinding.ValueTypeClass =
          TcxStringValueType) or
          (DataBuilder.DataFields[I].DataBinding.ValueTypeClass =
          TcxWideStringValueType)) then
        begin
...
begin
  ...
  ADataFieldStartIndex := AColumnFieldStartIndex +
    DataBuilder.ColumnFields.Count;
