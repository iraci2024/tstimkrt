...
uses
  Types, Windows, Messages, SysUtils, Classes, Controls, ActiveX, Contnrs,
  Menus, Forms, Graphics,
  DirectShow9, {$NOINCLUDE DirectShow9}
  dxCore, dxCoreGraphics, dxGDIPlusClasses, dxMessages, cxGeometry, cxClasses,
...
type
  ...
  TdxCameraControlSettingsToolbarPage = (stpSettings, stpDevices,
    stpResolutions);
  ...
  protected
    ...
    function CreateItem(const ACaption, ADescription: string)
      : TdxCameraControlSettingsToolbarItem;
    ...
    property HotItem: TdxCameraControlSettingsToolbarItem read FHotItem
      write SetHotItem;
    property Items[Index: Integer]: TdxCameraControlSettingsToolbarItem
      read GetItems;
  public
  ...
  protected
    ...
    procedure Reinitialize;
    function RenderStream(ARenderingMode: TdxCameraRenderingMode;
      AHandle: THandle; const R: TRect; const AOutputFileName: string): Boolean;
    ...
    property MediaTypeIndex: Integer read FMediaTypeIndex
      write SetMediaTypeIndex;
    property State: TdxCameraState read FState;

    property OnDeviceLost: TNotifyEvent read FOnDeviceLost write FOnDeviceLost;
    property OnDeviceNotUsed: TNotifyEvent read FOnDeviceNotUsed
      write FOnDeviceNotUsed;
    property StateChangedHandlers: TcxEventHandlerCollection
      read FStateChangedHandlers;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  end;
  ...
  public
    ...
    function StartRecording(ADeviceIndex: Integer;
  ...
  TdxCameraControlState = (ccsInactive, ccsInitializing, ccsRunning, ccsPaused,
    ccsNoDevice, ccsDeviceIsBusy);
  ...
  protected
    ...
    property MediaTypeIndex: Integer read GetMediaTypeIndex
      write SetMediaTypeIndex;
  ...
  public
    ...
    property ResolutionIndex: Integer read GetResolutionIndex
      write SetResolutionIndex;
    property Resolutions[AIndex: Integer]: TSize read GetResolutions;
    property ShowSettingsButton: Boolean read FShowSettingsButton
      write SetShowSettingsButton;
...
procedure dxSettingsToolbarMouseHook(ACode: Integer; wParam: wParam;
...
begin
  ...
  begin
    ...
    if IsMouseDownMessage(wParam) and not PtInRect(FSettingsToolbar.BoundsRect,
      AMHS.pt) then
      FSettingsToolbar.Release;
  end;
end;

procedure dxSettingsToolbarWndProcHook(ACode: Integer; AWParam: wParam;
  ALParam: lParam; var AHookResult: LRESULT);
var
  AMsg: PCWPStruct;
begin
  AMsg := PCWPStruct(ALParam);
  if (AMsg.Message = WM_KILLFOCUS) and
    (HWND(AMsg.wParam) <> FSettingsToolbar.Handle) then
    FSettingsToolbar.Release;
end;

procedure dxShowCameraControlSettingsToolbar(ACameraControl
  : TdxCustomCameraControl);
begin
  if (FSettingsToolbar <> nil) and
    (FSettingsToolbar.FCameraControl <> ACameraControl) then
...
begin
  ...
  begin
    ...
    if (csDestroying in Result.ComponentState) or
      (Result.FDeviceIndex <> ADeviceIndex) then
      Result := nil
    else
      Break;
  end;
...
function TdxCameraManager.GetDevice(AIndex: Integer;
  out AMoniker: IMoniker): Boolean;
var
  AEnumMoniker: IEnumMoniker;
begin
  AMoniker := nil;

  FDevEnum.CreateClassEnumerator(CLSID_VideoInputDeviceCategory,
    AEnumMoniker, 0);
  Result := (AEnumMoniker <> nil) and dxMediaResult(AEnumMoniker.Reset);
  Result := Result and (dxMediaResult(AEnumMoniker.Skip(AIndex)) or
    (AIndex = 0));
...
function TdxCameraManager.StartRecording(ADeviceIndex: Integer;
  const AFileName: string): Boolean;
var
  ACamera: TdxCamera;
begin
...
begin
  ...
  begin
    ...
    repeat
      ...
      begin
        if dxMediaResult(AMoniker.BindToStorage(nil, nil, IPropertyBag,
          APropertyBag)) then
          if dxMediaResult(APropertyBag.Read('FriendlyName', ADeviceName, nil))
          then
            FDevices.Add(ADeviceName);
      end;
    until AMoniker = nil end;
  end;
  ...
  begin
    ...
    if dxMediaResult(FSampleGrabber.GetCurrentBuffer(ASize, nil)) then
      ...
      begin
        ...
        if (AInfoHeader.bmiHeader.biWidth * AInfoHeader.bmiHeader.biHeight) *
          4 = ASize then
        begin
          SetLength(AColors, ASize div 4);
          if dxMediaResult(FSampleGrabber.GetCurrentBuffer(ASize, @AColors[0]))
          then
          begin
            ABitmap.SetSize(AInfoHeader.bmiHeader.biWidth,
              AInfoHeader.bmiHeader.biHeight);
  ...
  begin
    ...
    if Connected then
      FState := csRunning
    else if (FTimer = nil) or not FTimer.Enabled or (FFailsCount < 30) then
  ...
  begin
    ...
    begin
      ...
      else if FRendered and (FFailsCount mod 30 = 0) then
      begin
        FMediaControl.Stop;
        FMediaControl.Run;
      end;
  ...
  begin
    ...
    CoCreateInstance(CLSID_CaptureGraphBuilder2, nil, CLSCTX_INPROC,
      IID_ICaptureGraphBuilder2, FBuilder);
    CoCreateInstance(CLSID_FilterGraph, nil, CLSCTX_INPROC,
      IID_IGraphBuilder, FGraph);
    Result := (FBuilder <> nil) and (FGraph <> nil) and
      dxMediaResult(FBuilder.SetFilterGraph(FGraph));
  end;
  ...
  begin
    ...
    Result := dxMediaResult(FGraph.QueryInterface(IID_IMediaControl,
      FMediaControl)) and dxMediaResult(FGraph.QueryInterface(IID_IMediaEventEx,
      FMediaEvent)) and dxMediaResult
      (FMediaEvent.SetNotifyWindow(FEventWindow.Handle, DXM_GRAPHNOTIFY, 0));
  end;
  ...
  begin
    ...
    if dxCameraManager.GetDevice(ADeviceIndex, AMoniker) then
      if dxMediaResult(AMoniker.BindToObject(nil, nil, IID_IBaseFilter, AFilter))
      then
        if dxMediaResult(AMoniker.BindToStorage(nil, nil, IPropertyBag,
          APropertyBag)) then
          if dxMediaResult(APropertyBag.Read('FriendlyName', ADeviceName, nil))
  ...
  begin
    ...
    if dxMediaResult(CoCreateInstance(CLSID_SampleGrabber, nil, CLSCTX_INPROC,
      IID_IBaseFilter, FGrabberFilter)) then
      if dxMediaResult(FGraph.AddFilter(FGrabberFilter, 'Video Grabber')) then
        if dxMediaResult(FGrabberFilter.QueryInterface(IID_ISampleGrabber,
          FSampleGrabber)) then
        begin
          dxMediaCheck(FSampleGrabber.SetBufferSamples(True),
            'SetBufferSamples fails');

          cxZeroMemory(@AMediaType, SizeOf(TAMMediaType));
          AMediaType.majortype := MEDIATYPE_Video;
          AMediaType.subtype := MEDIASUBTYPE_RGB32;
          dxMediaCheck(FSampleGrabber.SetMediaType(AMediaType),
            'SetMediaType fails');
  ...
  begin
    Result := dxMediaResult(FBuilder.FindInterface(@PIN_CATEGORY_CAPTURE,
      @MEDIATYPE_Video, FCameraFilter, IID_IAMStreamConfig, FStreamConfig));
  end;

  function TdxCamera.InitializeNullRenderer: Boolean;
  begin
    Result := False;
    if dxMediaResult(CoCreateInstance(CLSID_NullRenderer, nil, CLSCTX_INPROC,
      IID_IBaseFilter, FRenderer)) then
      Result := dxMediaResult(FGraph.AddFilter(FRenderer, 'Null Filter'));
  end;

  function TdxCamera.InitializeWindowlessVMR(AHandle: THandle;
  ...
  begin
    ...
    if FWindowlessControl = nil then
      if dxMediaResult(CoCreateInstance(CLSID_VideoMixingRenderer, nil,
        CLSCTX_INPROC, IID_IBaseFilter, FRenderer)) then
        if dxMediaResult(FGraph.AddFilter(FRenderer, 'Video Mixing Renderer'))
        then
          if dxMediaResult(FRenderer.QueryInterface(IID_IVMRFilterConfig,
            AConfig)) then
            if dxMediaResult(AConfig.SetRenderingMode(VMRMode_Windowless)) then
              FRenderer.QueryInterface(IID_IVMRWindowlessControl,
                FWindowlessControl);
  ...
  function TdxCamera.InitializeWindowedVMR(AHandle: THandle;
    const R: TRect): Boolean;
  begin
    Result := False;
    ...
    if FVideoWindow <> nil then
      if dxMediaResult(FVideoWindow.put_Owner(AHandle)) then
        if dxMediaResult(FVideoWindow.put_WindowStyle(WS_CHILD or
          WS_CLIPCHILDREN)) then
          if dxMediaResult(FVideoWindow.SetWindowPosition(R.Left, R.Top,
            cxRectWidth(R), cxRectHeight(R))) then
  ...
  begin
{$IFDEF DELPHI22}
    dxMediaCheck(FStreamConfig.SetFormat(MediaTypes[FMediaTypeIndex]),
      'FStreamConfig.SetFormat fails');
{$ELSE}
    dxMediaCheck(FStreamConfig.SetFormat
      (TAMMediaType(MediaTypes[FMediaTypeIndex]^)),
      'FStreamConfig.SetFormat fails');
{$ENDIF}
  end;
  ...
  begin
    ...
    while Result and dxMediaResult(AEnumFilters.Next(1, AFilter, nil)) do
      if AFilter <> FCameraFilter then
        Result := Result and dxMediaResult(FGraph.RemoveFilter(AFilter)) and
          dxMediaResult(AEnumFilters.Reset);
  end;
  ...
  begin
    ...
    begin
      dxMediaResult(FStreamConfig.GetStreamCaps(I, AMediaType,
        AStreamConfigCaps));
      if CompareMem(@FORMAT_VideoInfo, @TAMMediaType(AMediaType^).formattype,
        SizeOf(MEDIATYPE_Video)) and
        (TAMMediaType(AMediaType^).subtype.D1 = ACurrentSubtype) then
  ...
  begin
    if not FRecording and (MediaTypes.Count > 0) and (FMediaTypeIndex <> AValue)
      and DisconnectGraph and InitializeCaptureGrabber then
    begin
  ...
  begin
    if FStreamConfig <> nil then
      dxMediaCheck(FStreamConfig.GetFormat(AAMMediaTypePointer),
        'GetCurrentMediaType')
    else
  ...
  begin
    ...
    if FVideoWindow <> nil then
      FVideoWindow.SetWindowPosition(R.Left, R.Top, cxRectWidth(R),
        cxRectHeight(R));
  end;

  function TdxCamera.RenderStream(ARenderingMode: TdxCameraRenderingMode;
    AHandle: THandle; const R: TRect; const AOutputFileName: string): Boolean;
  var
  ...
  begin
    ...
    begin
      ...
      begin
        FBuilder.SetOutputFileName(MEDIASUBTYPE_Avi, PWideChar(AOutputFileName),
          FMux, ASink);
        FRendered := dxMediaResult(FBuilder.RenderStream(@PIN_CATEGORY_CAPTURE,
          @MEDIATYPE_Video, FCameraFilter, FGrabberFilter, FMux));
        if ARenderingMode <> rmNull then
          FBuilder.RenderStream(@PIN_CATEGORY_PREVIEW, @MEDIATYPE_Video,
            FCameraFilter, nil, FRenderer);
      end
      else if FRenderer <> nil then
        FRendered := dxMediaResult(FBuilder.RenderStream(@PIN_CATEGORY_CAPTURE,
          @MEDIATYPE_Video, FCameraFilter, FGrabberFilter, FRenderer));
      Result := FRendered;
    end;
...
{$ENDIF}
  ...
  begin
    Result := DisconnectGraph and InitializeCaptureGrabber and
      ConnectGraph(AFileName) and FileExists(AFileName);
    FRecording := Result;
    if not Result then
      StopRecording;
  end;
  ...
  procedure TdxCameraControlSettingsToolbarItem.DrawBackground
    (ACanvas: TcxCanvas);
  var
    AColor: TColor;
  begin
  ...
  begin
    ...
    cxDrawText(ACanvas.Handle, FCaption, ARect, GetHorizontalAlignFlags(DT_LEFT)
      or DT_VCENTER or DT_SINGLELINE);
    cxDrawText(ACanvas.Handle, FDescription, ARect,
      GetHorizontalAlignFlags(DT_RIGHT) or DT_VCENTER or DT_SINGLELINE);
  end;
  ...
  procedure TdxCameraControlSettingsToolbarItem.SetState
    (AState: TcxEditCheckState);
  begin
  ...
  procedure TdxCameraControlSettingsToolbar.CreateWindowHandle
  ...
  begin
    ...
    try
      if GetWindowRgn(Handle, AWindowRegion.Handle)
  ...
  begin
    ...
    SetWindowRgn(Handle, CreateRoundRectRgn(0, 0, Width + 1, Height + 1, 5,
      5), False);
  ...
  procedure TdxCameraControlSettingsToolbar.MouseMove(Shift: TShiftState;
    X, Y: Integer);
  var
    AItemIndex: Integer;
  begin
    ...
    if AItemIndex = -1 then
      HotItem := nil
    else if Items[AItemIndex] <> HotItem then
      HotItem := Items[AItemIndex];
  end;

  procedure TdxCameraControlSettingsToolbar.MouseUp(Button: TMouseButton;
  ...
  procedure TdxCameraControlSettingsToolbar.CalculateItemsBounds
    (const ARect: TRect);
  var
  ...
  begin
    ...
    Items[0].FBounds := cxRectContent
      (cxRectOffset(cxRect(cxSize(cxRectWidth(ARect), AItemHeight)), 0,
      ScaleFactor.Apply(5)), ScaleFactor.Apply(Rect(1, 0, 1, 0)));
    for I := 1 to FItems.Count - 1 do
    begin
      AColumnIndex := (I - 1) div AColumnItemCount;
      Items[I].FBounds :=
        cxRectContent(cxRectOffset(cxRect(cxSize(AItemWidth, AItemHeight)),
        AItemWidth * AColumnIndex, ScaleFactor.Apply(5) + (I - AColumnIndex *
        AColumnItemCount) * AItemHeight), ScaleFactor.Apply(Rect(1, 0, 1, 0)));
    end;
    if UseRightToLeftAlignment then
      for I := 0 to FItems.Count - 1 do
        Items[I].FBounds := TdxRightToLeftLayoutConverter.ConvertRect
          (Items[I].FBounds, ARect);
  end;
    ...
    begin
      case AItemIndex of
        0:
          Release;
        1:
          Initialize(stpDevices);
        2:
          Initialize(stpResolutions);
      end;
    end;
  ...
  begin
    case FToolbarPage of
      ...
      stpDevices:
        ClickDevicesItem;
      stpResolutions:
        ClickResolutionsItem;
    end;
  end;

  function TdxCameraControlSettingsToolbar.CreateItem(const ACaption,
    ADescription: string): TdxCameraControlSettingsToolbarItem;
  begin
  ...
  function TdxCameraControlSettingsToolbar.GetItemIndexAtPoint
  ...
  begin
    ...
    FItems.Add(CreateItem('<   ' + cxGetResourceString
  ...
  procedure TdxCameraControlSettingsToolbar.Initialize
    (AToolbarPage: TdxCameraControlSettingsToolbarPage);
  begin
    FHotItem := nil;
  ...
  begin
    ...
    begin
      AItem := CreateItem('', IntToStr(FCameraControl.Resolutions[I].cx) + 'x' +
        IntToStr(FCameraControl.Resolutions[I].cy));
  ...
  begin
    ...
    begin
      FItems.Add
        (CreateItem(cxGetResourceString(@sdxCameraControlSettingsFormDevice),
        FCameraControl.DeviceName + '   >'));
      if (FCameraControl.Camera <> nil) and FCameraControl.Camera.Connected then
      begin
        FItems.Add
          (CreateItem(cxGetResourceString
          (@sdxCameraControlSettingsFormResolution),
          IntToStr(FCameraControl.Resolutions[FCameraControl.ResolutionIndex]
          .cx) + 'x' + IntToStr(FCameraControl.Resolutions
          [FCameraControl.ResolutionIndex].cy) + '   >'));
      end
    end;
  end;
  ...
  begin
    ...
    Result := cxRectSetBottom(Result,
      FCameraControl.ClientToScreen(FCameraControl.Bounds.BottomRight).Y);
    Result := cxRectSetRight(Result,
      FCameraControl.ClientToScreen(FCameraControl.Bounds.BottomRight).X);
    ...
    begin
      ...
      ARect := TdxRightToLeftLayoutConverter.ConvertRect(ARect,
        FCameraControl.ClientRect);
  ...
  begin
    ...
    begin
      AMaxWidthCaption := 0;
      for I := 1 to FItems.Count - 1 do
        AMaxWidthCaption := Max(AMaxWidthCaption,
          cxTextWidth(Font, Items[I].FCaption));

      for I := 1 to FItems.Count - 1 do
        if Items[I].FDescription = '' then
          Result.cx := Max(Result.cx, cxTextWidth(Font, Items[I].FCaption) +
            ScaleFactor.Apply(5))
        else if Items[I].FCaption = '' then
          Result.cx := Max(Result.cx, cxTextWidth(Font, Items[I].FDescription) +
            ScaleFactor.Apply(5))
        else
          Result.cx := Max(Result.cx, cxTextWidth(Font, Items[I].FDescription) +
            AMaxWidthCaption + ScaleFactor.Apply(20));
      FColumnCount :=
        Ceil(Sqrt(FItems.Count * TdxTextMeasurer.TextLineHeight(Font) * 2 *
        Result.cx) / Result.cx);
    end;
    Result.cx := Max(Result.cx, cxTextWidth(Font, Items[0].FCaption) +
      ScaleFactor.Apply(5));
    Result := cxSize(Result.cx * FColumnCount,
      (Ceil((FItems.Count - 1) / FColumnCount) + 1) *
      TdxTextMeasurer.TextLineHeight(Font) * 2);
  end;
  ...
  procedure TdxCameraControlSettingsToolbar.DXMScaleChanged
  ...
  function TdxCameraControlSettingsToolbar.GetItems(AIndex: Integer)
    : TdxCameraControlSettingsToolbarItem;
  begin
    Result := FItems[AIndex] as TdxCameraControlSettingsToolbarItem;
  end;

  procedure TdxCameraControlSettingsToolbar.SetHotItem
    (AValue: TdxCameraControlSettingsToolbarItem);
  begin
  ...
  begin
    ...
    FSettingsButtonGlyph.LoadFromResource(HInstance,
      'DXCAMERACONTROL_SETTINGSBUTTON', 'PNG');
  ...
  const
    dxCameraStateToCameraControlState: array [TdxCameraState]
      of TdxCameraControlState = (ccsInitializing, ccsRunning, ccsDeviceIsBusy);
  var
  ...
  begin
    ...
    begin
      if FPaused then
        FState := ccsPaused
      else if FCamera = nil then
        FState := ccsNoDevice
      else
        FState := dxCameraStateToCameraControlState[Camera.State];

      if not IsDestroying and (FState <> APrevState) and
        (FSettingsToolbar <> nil) and
  ...
  begin
    ...
    else if FSettingsButtonAlpha = 255 then
    begin
      FSettingsButtonAlpha := 150;
      InvalidateRect(GetSettingsButtonRect, True);
    end;
  ...
  begin
    ...
    case State of
      ccsRunning:
        cxDrawImage(Canvas, ClientRect, FCamera.PreviewBitmap, FFitMode, nil,
          nil, True);
      ccsPaused:
        cxDrawImage(Canvas, ClientRect, CapturedBitmap, FFitMode, nil,
          nil, True);
    else
      cxDrawMultilineText(Canvas, GetDisplayText, ClientRect,
        DT_VCENTER or DT_CENTER);
    end;
    if ((FSettingsToolbar = nil) or (FSettingsToolbar.FCameraControl <> Self))
      and ShowSettingsButton then
      DrawSettingsButton;
  end;
  ...
  procedure TdxCustomCameraControl.Notification(AComponent: TComponent;
  ...
  procedure TdxCustomCameraControl.LookAndFeelChanged(Sender: TcxLookAndFeel;
  ...
  begin
    ...
    begin
      Result.cx := TVideoInfoHeader(TAMMediaType(MediaTypes[AIndex]^).pbFormat^)
        .bmiHeader.biWidth;
      Result.cy := TVideoInfoHeader(TAMMediaType(MediaTypes[AIndex]^).pbFormat^)
        .bmiHeader.biHeight;
    end
    else
      Result := cxNullSize;
  ...
  procedure TdxCustomCameraControl.CameraStateChanged(Sender: TObject;
    const AEventArgs);
  begin
    CheckState;
  end;
  ...
  begin
    FSettingsButtonGlyph.StretchDraw(Canvas.Handle, GetSettingsButtonRect,
      cxRect(FSettingsButtonGlyph.Size), FSettingsButtonAlpha);
  end;
  ...
  begin
    ...
    Result := cxRectBounds(Bounds.Right - ASize.cx - ScaleFactor.Apply(5),
