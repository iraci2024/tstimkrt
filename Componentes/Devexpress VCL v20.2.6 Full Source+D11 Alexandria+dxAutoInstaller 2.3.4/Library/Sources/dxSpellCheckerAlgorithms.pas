...
type
  ...
  protected
    ...
    function Contains(AStart: PWideChar; ALength: Integer; APatterns: PWideChar)
      : Boolean; inline;
  ...
  public
    function GetDistance(AStr1: PWideChar; ALen1: Integer; AStr2: PWideChar;
      ALen2: Integer): Integer;
...
begin
  ...
  for I := 0 to ALength - 1 do
    case (P + I)^ of
      ...
      'A':
        Result := Result or (2 shl (4 * I));
      'P':
        Result := Result or (3 shl (4 * I));
      'K':
        Result := Result or (4 shl (4 * I));
      'X':
        Result := Result or (5 shl (4 * I));
      'J':
        Result := Result or (6 shl (4 * I));
      'T':
        Result := Result or (7 shl (4 * I));
      'F':
        Result := Result or (8 shl (4 * I));
      'N':
        Result := Result or (9 shl (4 * I));
      'L':
        Result := Result or (10 shl (4 * I));
      'H':
        Result := Result or (11 shl (4 * I));
      ' ':
        Result := Result or (12 shl (4 * I));
      'M':
        Result := Result or (13 shl (4 * I));
      'R':
        Result := Result or (14 shl (4 * I));
      '0':
        Result := Result or (15 shl (4 * I));
    else
      Result := 0;
      Break;
    end;
...
begin
    ...
    case Value^ of
      ...
      'W':
        begin
          ...
          else if Contains(Current, 2, 'WH;') then
          begin
  ...
  // main loop
  while ((FPrimaryLen < MaxSoundLen) or (FAlternateLen < MaxSoundLen)) and
    (Current <= Last) do
  begin
    case Current^ of
      'B':
...
begin
  ...
  begin
    FIsSlavoGermanic := ContainsSubStr(Value, 'W') or ContainsSubStr(Value, 'K')
      or ContainsSubStr(Value, 'CZ') or ContainsSubStr(Value, 'WITZ');
...
procedure TdxDoubleMetaphone.MetaphAdd(APrimary: WideChar;
  AAlternate: WideChar = #0);
begin
  if APrimary <> #0 then
  begin
...
begin
  // various germanic
  if (CurrentIndex > 1) and not IsVowel(Current - 2) and
    Contains(Current - 1, 3, 'ACH;') and
    (((Current + 2)^ <> 'I') and (((Current + 2)^ <> 'E') or
    Contains(Current - 2, 6, 'BACHER;MACHER;'))) then
  begin
    MetaphAdd('K');
    Inc(Current);
    Exit;
  ...
  begin
    ...
    if (AtStart and (Contains(Current + 1, 5, 'HARAC;HARIS;') or
      Contains(Current + 1, 3, 'HOR;HYM;HIA;HEM;')) and not Contains(Current, 5,
      'CHORE;')) then
    begin
      MetaphAdd('K');
      Inc(Current);
  ...
  if Contains(Current, 2, 'CC;') and not((CurrentIndex = 1) and (Value^ = 'M'))
  then
  begin
    // 'bellocchio' but not 'bacchus'
    if CharInString((Current + 2)^, 'IEH') and not Contains(Current + 2, 2,
      'HU;') then
    begin
      // 'accident', 'accede' 'succeed'
      if (((CurrentIndex = 1) and ((Current - 1)^ = 'A')) or
        Contains(Current - 1, 5, 'UCCEE;UCCES;')) then
      begin
        MetaphAdd('K');
        MetaphAdd('S');
      end
  ...
  if Contains(Current + 1, 2, ' C; Q; G;') then
    Inc(Current, 2)
  else if CharInString((Value + 1)^, 'CKQ') and
    not Contains(Value + 1, 2, 'CE;CI;') then
    Inc(Current);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (CurrentIndex > 2) and ((Current - 1)^ = 'U') and
        CharInString((Current - 3)^, 'CGLRT') then
        MetaphAdd('F')
      else if (CurrentIndex > 0) and ((Current - 1)^ <> 'I') then
        MetaphAdd('K');
      Inc(Current);
    end;
    Exit;
  ...
  begin
    ...
    begin
      ...
      // not e.g. 'cagney'
      if not Contains(Current + 2, 2, 'EY;') and ((Current + 1)^ <> 'Y') and not IsSlavoGermanic
      then
      begin
        MetaphAdd('N', 'K');
        MetaphAdd(#0, 'N');
  ...
  if (AtStart and (((Current + 1)^ = 'Y') or Contains(Current + 1, 2,
  ...
  if (Contains(Current + 1, 2, 'ER;') or ((Current + 1)^ = 'Y')) and
    ...
    not((CurrentIndex > 0) and ((Current - 1)^ = 'E') or ((Current - 1)^ = 'I'))
    and not Contains(Current - 1, 3, 'RGY;OGY;') then
  begin
    MetaphAdd('K', 'J');
    Inc(Current);
    Exit;
  ...
  // italian e.g, 'biaggi'
  if CharInString((Current + 1)^, 'EIY') or
    Contains(Current - 1, 4, 'AGGI;OGGI;') then
  begin
    // obvious germanic
    if Contains(Current, 4, 'VAN ;VON ;') or Contains(Current, 3, 'SCH;') or
      Contains(Current + 1, 2, 'ET;') then
      MetaphAdd('K')
    else
...
begin
  ...
  begin
    if (AtStart and ((Current + 4)^ = ' ')) or Contains(Current, 4, 'SAN ;')
    then
      MetaphAdd('H')
    else
      MetaphAdd('J', 'H');
  ...
  if AtStart and not Contains(Current, 4, 'JOSE;') then
    ...
    if IsVowel(Current - 1) and not IsSlavoGermanic and
      (((Current + 1)^ = 'A') or ((Current + 1)^ = 'O')) then
      MetaphAdd('J', 'H')
    else if AtEnd then
      MetaphAdd('J', ' ')
    else if not CharInString((Current + 1)^, 'LTKSNMBZ') and
...
begin
  ...
  begin
    ...
    if ((Current = Last - 2) and Contains(Current - 1, 4, 'ILLO;ILLA;ALLE;')) or
      ((Contains(Last - 1, 2, 'AS;OS;') or (Last^ = 'A') or (Last^ = 'O')) and
      Contains(Current - 1, 4, 'ALLE;')) then
    begin
      MetaphAdd('L', ' ');
      Inc(Current);
      Exit;
...
begin
  ...
  if AtEnd and not IsSlavoGermanic and Contains(Current - 2, 2, 'IE;') and
    not Contains(Current - 4, 2, 'ME;MA;') then
    MetaphAdd(#0, 'R')
  else
    MetaphAdd('R');
...
begin
  ...
  if (AtStart and CharInString((Current + 1)^, 'MNLW')) or ((Current + 1)^ = 'Z')
  then
  begin
    MetaphAdd('S', 'X');
    if (Current + 1)^ = 'Z' then
...
begin
  ...
  if (AtEnd and IsVowel(Current - 1)) or Contains(Current - 1, 5,
    'EWSKI;EWSKY;OWSKI;OWSKY;') or Contains(Current, 3, 'SCH;') then
  begin
    MetaphAdd(#0, 'F');
    Exit;
  end;
...
begin
  // french e.g. breaux
  if not(AtEnd and (Contains(Current - 3, 3, 'IAU;EAU;') or
    Contains(Current - 2, 2, 'AU;OU;'))) then
  begin
    MetaphAdd('K');
    MetaphAdd('S');
  end;
...
begin
  ...
  else if Contains(Current + 1, 2, 'ZO;ZI;ZA;') or
    (IsSlavoGermanic and ((CurrentIndex > 0) and ((Current - 1)^ <> 'T'))) then
  begin
    MetaphAdd('S', 'T');
    MetaphAdd(#0, 'S');
  end
...
function TdxStringSimilarityCalculator.GetDistance(AStr1: PWideChar;
  ALen1: Integer; AStr2: PWideChar; ALen2: Integer): Integer;
var
...
begin
  ...
  begin
    ...
    begin
      AMatrix[I + 1, J + 1] :=
        Minimum(AMatrix[I, J] + SubstitutionCost(AStr1 + I, AStr2 + J),
...
function TdxStringSimilarityCalculator.GetLongestCommonSubsequenceLength(S1,
  S2: PWideChar): Integer;
type
  ...
  begin
    ...
    for J := 1 to N do
      ...
      begin
        ...
        else if AMatrix[I - 1, J].Weight >= AMatrix[I, J - 1].Weight then
        begin
...
function TdxStringSimilarityCalculator.SubstitutionCost(S1: PWideChar;
  S2: PWideChar): Integer;
begin
  if S1^ = S2^ then
    Result := 0
