...
{$ENDIF}
  ...
  cxSchedulerRecurrence, cxSchedulerHolidays, dxLayoutLookAndFeels, cxClasses,
...
type
  ...
  protected
    ...
    procedure LoadValuesIntoTimeControls(const AStart, AFinish: TDateTime;
      AllDayHoliday: Boolean); virtual;
    procedure SaveChanges; virtual;
    procedure SetReadOnly(AValue: Boolean); virtual;
    procedure UpdateHolidayValuesFromControls; virtual;
  public
    constructor CreateEx(AHoliday
      : TcxSchedulerHolidaysLocationHoliday); virtual;
  ...
  TcxSchedulerRecurrenceHolidayEditorForm = class
    (TcxSchedulerCustomRecurrenceHolidayEditor)
    btnCancel: TcxButton;
    btnOk: TcxButton;
    btnRemove: TcxButton;
  ...
  private
    ...
    procedure SetItemIndexWithoutChangeHoliday(AEdit: TcxComboBox;
      AIndex: Integer);
    procedure SetMonthlyPatternFromHoliday;
    procedure SetPatternFromHoliday;
    procedure SetTextWithoutChangeHoliday(AEdit: TcxCustomMaskEdit;
      const AText: string);
  ...
  protected
    ...
    procedure SaveRecurrenceInfo
      (AInfo: TcxSchedulerHolidayRecurrenceInfo); virtual;
  ...
  TcxSchedulerRecurrenceHolidayEditor = TcxSchedulerRecurrenceHolidayEditorForm;
  // workaround for Delphi 2005

  TcxSchedulerRecurrenceHolidayEditorClass = class of
    TcxSchedulerCustomRecurrenceHolidayEditor;
...
procedure TcxSchedulerCustomRecurrenceHolidayEditor.
  LoadHolidayValuesIntoControls;
begin
end;
...
procedure TcxSchedulerCustomRecurrenceHolidayEditor.SetReadOnly
  (AValue: Boolean);
begin
  FReadOnly := AValue;
end;
...
procedure TcxSchedulerCustomRecurrenceHolidayEditor.
  UpdateHolidayValuesFromControls;
begin
end;
...
function TcxSchedulerRecurrenceHolidayEditorForm.GetRecurrenceInfoValidStatus
...
procedure TcxSchedulerRecurrenceHolidayEditorForm.SetReadOnly(AValue: Boolean);
begin
  inherited;
...
procedure TcxSchedulerRecurrenceHolidayEditorForm.CheckViewRecurrence
...
function TcxSchedulerRecurrenceHolidayEditorForm.GetActivePattern
  : TcxRecurrence;
begin
  if rbDaily.Checked then
    Result := cxreDaily
...
function TcxSchedulerRecurrenceHolidayEditorForm.GetRecurrenceInfo
...
function TcxSchedulerRecurrenceHolidayEditorForm.GetNumber
...
begin
  SetTextWithoutChangeHoliday(meNumOfDay,
    IntToStr(DayOf(RecurrenceInfo.Start)));
...
procedure TcxSchedulerRecurrenceHolidayEditorForm.SelectDaysOfWeek
  (ADays: TDays);
...
procedure TcxSchedulerRecurrenceHolidayEditorForm.
  SetItemIndexWithoutChangeHoliday(AEdit: TcxComboBox; AIndex: Integer);
var
...
begin
  ...
  begin
    ...
    SetTextWithoutChangeHoliday(meNumMonth,
      IntToStr(RecurrenceInfo.Periodicity));
  end
  else
  begin
    rbThe.Checked := True;
    SetTextWithoutChangeHoliday(meNumMonth1,
      IntToStr(RecurrenceInfo.Periodicity));
...
begin
  ...
  case RecurrenceInfo.Recurrence of
    cxreDaily:
      SetDailyPatternFromHoliday;
    cxreWeekly:
      SetWeeklyPatternFromHoliday;
    cxreMonthly:
      SetMonthlyPatternFromHoliday;
    cxreYearly:
      SetYearlyPatternFromHoliday;
  end;
end;
...
begin
  ...
  if Holiday.IsRecurring and (Holiday.RecurrenceInfo.GetValidStatus = rvsValid)
  then
