...
{$ENDIF}
  ...
  ImgList, Menus, Forms, Dialogs, dxCore, cxDropDownEdit, cxGraphics,
  cxGeometry,
...
type
  ...
  protected
    ...
    {$IFDEF DELPHI104SYDNEY} reintroduce; {$ENDIF} virtual;
  public
  ...
  published
    ...
    property OnEnter;
    property OnExit;
    property OnGetBrushStyleName: TdxGetBrushStyleNameEvent
      read FOnGetBrushStyleName write FOnGetBrushStyleName;
    property OnKeyDown;
    property OnKeyPress;
    property OnKeyUp;
...
{$ENDIF}
    property EdgeBorders: TdxEdgeBorders read GetEdgeBorders
      write SetEdgeBorders default [ebLeft, ebTop, ebRight, ebBottom];
  ...
  published
    property AnimationSpeed: Integer read FAnimationSpeed
      write FAnimationSpeed { ms } default 10;
    property AnimationStepCount: Integer read FAnimationStepCount
      write FAnimationStepCount default 10;
  ...
  TdxPSImageScrollBoxBuiltInMenuItems = set of
    TdxPSImageScrollBoxBuiltInMenuItem;
  ...
  protected
    ...
    property IsPictureHeightExceedControlBounds: Boolean
      read GetIsPictureHeightExceedControlBounds;
    property IsPictureWidthExceedControlBounds: Boolean
      read GetIsPictureWidthExceedControlBounds;
    property PictureHeight: Integer read GetPictureHeight;
    property PictureOriginX: Integer read FPictureOriginX
      write SetPictureOriginX;
    property PictureOriginY: Integer read FPictureOriginY
      write SetPictureOriginY;
  ...
  public
    ...
    property BuiltInMenuItemsVisibility: TdxPSImageScrollBoxBuiltInMenuItems
      read FBuiltInMenuItemsVisibility write FBuiltInMenuItemsVisibility
      default [biiPreview .. biiSave];
  ...
  TdxPSValueType = (ivtDecimal, ivtLiteral, ivtCapitalLiteral, ivtRoman,
    ivtCapitalRoman);
  ...
  public
    ...
    //
    property ValueType: TdxPSValueType read FValueType write SetValueType
      default ivtDecimal;
  end;
...
uses
  Themes, UxTheme, CommCtrl, SysUtils, Registry, ExtDlgs, ClipBrd, Variants,
  Types, Math,
...
const
  ...
  PureColors: array [0 .. 19] of TColor = (clBlack, clOlive, clTeal, clGreen,
    clMoneyGreen, clLime, clNavy, clBlue, clAqua, clSkyBlue, clGray, clMedGray,
    clSilver, clMaroon, clPurple, clFuchsia, clRed, clCream, clYellow, clWhite);
  SysColors: array [0 .. 24] of TColor = (clScrollBar, clBackground,
    clActiveCaption, clInactiveCaption, clMenu, clWindow, clWindowFrame,
    ...
    clGrayText, clBtnText, clInactiveCaptionText, clBtnHighlight, cl3DDkShadow,
...
procedure TdxPSBrushStyleCombo.CalculateRects(const R: TRect;
  out ABrushRect, ATextRect: TRect);
begin
  ABrushRect := R;
  ATextRect := R;
...
procedure TdxPSBrushStyleCombo.DoDrawItem(AControl: TcxCustomComboBox;
  ...
  begin
    if AStyle = bsClear then
      Result := clWindow
    else if (ColorToRGB(BrushColor) <> ColorToRGB(clWindow)) or
      (StyleIndexes[AStyle] <= StyleIndexes[bsClear]) then
      Result := BrushColor
    else
      Result := clWindowText
  end;
...
begin
  ...
  try
    ...
    CalculateRects(cxRectInflate(ARect, -ScaleFactor.Apply(2),
      -ScaleFactor.Apply(2)), ABrushRect, ATextRect);
    ...
    begin
      ...
      ACanvas.DrawTexT(StyleNames[AIndex], ATextRect, TextFormat or
        EndEllipsisMap[EndEllipsis], Enabled);
    end;
  finally
...
procedure TdxPSBrushStyleCombo.DoGetStyleName(Index: Integer;
  var AName: string);
begin
...
begin
  ...
  try
    ...
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleSolid),
      TObject(bsSolid));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleClear),
      TObject(bsClear));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleHorizontal),
      TObject(bsHorizontal));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleVertical),
      TObject(bsVertical));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleFDiagonal),
      TObject(bsFDiagonal));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleBDiagonal),
      TObject(bsBDiagonal));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleCross),
      TObject(bsCross));
    Properties.Items.AddObject(cxGetResourceString(@sdxBrushStyleDiagCross),
      TObject(bsDiagCross));
  finally
...
begin
  Properties.ItemHeight := Max(ScaleFactor.Apply(22),
    -MulDiv(Font.Height, 12, 10));
end;
...
const
  InnerStyles: array [TdxEdgeStyle] of Integer = (0, BDR_RAISEDINNER,
    BDR_SUNKENINNER);
  OuterStyles: array [TdxEdgeStyle] of Integer = (0, BDR_RAISEDOUTER,
    BDR_SUNKENOUTER);
...
var
  ...
  R: TRect;
begin
  dY := Height div AnimationStepCount +
    Ord((Height mod AnimationStepCount) <> 0);
...
procedure TdxPSWarningPane.SetStateAndHint(NewState: Boolean;
  const AHint: string);
begin
...
procedure TdxPSWarningPane.InitializeBitmap;
  ...
  begin
    ...
    cxAlphaBlend(Bitmap, WarningSignBitmap, AImageRect,
      WarningSignBitmap.ClientRect);
...
const
  TextFormats: array [Boolean] of UINT = (DT_SINGLELINE or DT_VCENTER,
    DT_WORDBREAK);
begin
...
begin
  miPreview := CreateMenuItem(AddEndEllipsis(cxGetResourceString(@sdxPreview)),
    0, 0, PreviewClick);
  miLine1 := CreateMenuItem('-', -1, 0, nil);
  miCopy := CreateMenuItem(cxGetResourceString(@sdxCopy), 1,
    Menus.TextToShortCut('Ctrl+C'), CopyClick);
  miLine2 := CreateMenuItem('-', -1, 0, nil);
  miSave := CreateMenuItem(cxGetResourceString(@sdxSave), 2,
    Menus.TextToShortCut('Ctrl+S'), SaveClick);
...
procedure TdxPSImageScrollBox.DrawBackground(ACanvas: TcxCanvas;
  const R: TRect);
begin
  ACanvas.FillRect(R, ContentColor);
end;

procedure TdxPSImageScrollBox.DrawHintText(ACanvas: TcxCanvas;
  const ATextRect: TRect);
begin
...
begin
  if HasGraphic then
    Result := Classes.Bounds(PictureOriginX, PictureOriginY, PictureWidth,
      PictureHeight)
  else
    Result := cxNullRect;
end;
...
procedure TdxPSImageScrollBox.ScrollHorizontal(AScrollCode: TScrollCode;
...
begin
  case AScrollCode of
    ...
    scLineUp, scLineDown:
      PictureOriginX := PictureOriginX + SignsMap[AScrollCode = scLineUp] *
        HScrollBar.SmallChange;
    scPageUp, scPageDown:
      PictureOriginX := PictureOriginX + SignsMap[AScrollCode = scPageUp] *
        HScrollBar.PageSize;
  end;
end;

procedure TdxPSImageScrollBox.ScrollVertical(AScrollCode: TScrollCode;
  var AScrollPos: Integer);
const
  SignsMap: array [Boolean] of Integer = (-1, 1);
begin
  case AScrollCode of
    ...
    scLineUp, scLineDown:
      PictureOriginY := PictureOriginY + SignsMap[AScrollCode = scLineUp] *
        VScrollBar.SmallChange;
    scPageUp, scPageDown:
      PictureOriginY := PictureOriginY + SignsMap[AScrollCode = scPageUp] *
        VScrollBar.PageSize;
  end;
end;
...
procedure TdxPSValueEdit.DoValidateDisplayValue(var ADisplayValue: Variant;
  var AErrorText: TCaption; var AError: Boolean);
begin
  ADisplayValue := EditValueToDisplayValue
    (DisplayValueToEditValue(ADisplayValue));
end;
