...
type
  ...
  public
    class function DefaultFormatFunction(AOperatorType: TdxFunctionOperatorType;
      const AOperands: TArray<string>): string; static;
    class function DefaultFormatBinary(AOperatorType: TdxBinaryOperatorType;
      const ALeftOperand, ARightOperand: string): string; static;
    class function DefaultFormatUnary(AOperatorType: TdxUnaryOperatorType;
      const AOperand: string): string; static;
  end;
  ...
  public
    ...
    class function GetExpandedProperties(const AProperties: TArray<string>;
      const AExpandingAlias: string): string; static;
    class function GetColumns(const ASQLText: string): TArray<string>;
      overload; static;
  ...
  public
    class function ToValue(AField: TField; AMemberInfo: TdxMappingMemberInfo)
      : TValue; overload;
    class function ToValue(const AValue: Variant;
      AMemberInfo: TdxMappingMemberInfo): TValue; overload;
    class function ToValues(const AFields: TArray<TField>;
      AMemberInfo: TdxMappingMemberInfo): TArray<TValue>;
    class procedure SetValue(AObject: TObject; const AFields: TArray<TField>;
      AMemberInfo: TdxMappingMemberInfo); overload;
    class procedure SetValue(AObject: TObject; const AValues: TArray<Variant>;
      AMemberInfo: TdxMappingMemberInfo); overload;
  end;
...
class function TdxBaseFormatterHelper.DefaultFormatFunction
...
begin
  case AOperatorType of
    TdxFunctionOperatorType.Iif:
      begin
        ...
        if Length(AOperands) = 3 then
          Exit(Format('case when %s then %s else %s end',
        ...
        try
          ...
          repeat
            Inc(AIndex, 2);
            ASb.AppendFormat('when %s then %s ',
    ...
    TdxFunctionOperatorType.Substring:
      begin
        if Length(AOperands) < 3 then
          ...
          ALen := AOperands[2];
        Result := 'Substring(' + AOperands[0] + ', (' + AOperands[1] + ') + 1, '
          + ALen + ')';
      end;
    TdxFunctionOperatorType.IsNull, TdxFunctionOperatorType.Len,
    ...
    TdxFunctionOperatorType.Insert:
      Result := Format('Stuff(%s, %s + 1, 0, %s)', [AOperands[0], AOperands[1],
        AOperands[2]]);
    ...
    raise ENotImplemented.Create('DefaultFormatFunction for ' +
      TdxFormatterHelper.GetName(AOperatorType));
  end;
end;

class function TdxBaseFormatterHelper.DefaultFormatBinary
  (AOperatorType: TdxBinaryOperatorType;
  const ALeftOperand, ARightOperand: string): string;
const
  ASQLOperator: array [TdxBinaryOperatorType] of string = ('=', '<>', '>', '<',
    '<=', '>=', '&', '|', '^', '/', '%', '*', '+', '-');
begin
  Result := '(' + ALeftOperand + ' ' + ASQLOperator[AOperatorType] + ' ' +
    ARightOperand + ')';
end;

class function TdxBaseFormatterHelper.DefaultFormatUnary(AOperatorType
...
begin
  case AOperatorType of
    ...
    raise ENotImplemented.Create('DefaultFormatUnary('#$27 +
      TdxFormatterHelper.GetName(AOperatorType) + #$27', '#$27 + AOperand
      + #$27')');
  end;
...
class function TdxSimpleSQLParser.GetExpandedProperties(const AProperties
  : TArray<string>; const AExpandingAlias: string): string;
var
...
{$ENDIF}
      ...
      begin
{$IFDEF DELPHIXE3}
        AAlias := AField.Substring(ALastAsIndex +
          AsDelimiterString.Length).Trim;
...
{$ELSE}
        AAlias := Trim(TdxStringHelper.Substring(AField,
          ALastAsIndex + Length(AsDelimiterString)));
...
class function TdxSimpleSQLParser.GetColumns(const ASQLText: string)
  : TArray<string>;
var
...
class procedure TdxFieldValueConverter.SetValue(AObject: TObject;
  const AFields: TArray<TField>; AMemberInfo: TdxMappingMemberInfo);
var
  AStream: TStream;
begin
  ...
  begin
    ...
    begin
      ...
      try
        AMemberInfo.Serializer.LoadFromStream(AMemberInfo.GetValue(AObject)
          .AsObject, AStream);
      finally
        AStream.Free;
      end;
...
class procedure TdxFieldValueConverter.SetValue(AObject: TObject;
  const AValues: TArray<Variant>; AMemberInfo: TdxMappingMemberInfo);
var
  ADataPtr: Pointer;
  ALength: Integer;
  AStream: TStream;
...
class function TdxFieldValueConverter.ToValue(AField: TField;
  AMemberInfo: TdxMappingMemberInfo): TValue;
var
  ACurrency: Currency;
  ADateTime: TDateTime;
...
begin
  ...
  case AField.DataType of
    ...
    ftFixedWideChar, ftWideMemo:
      Result := AField.AsString;
    ftLongWord:
      Result := {$IFDEF DELPHIXE3}AField.AsLongWord{$ELSE}LongWord
        (AField.AsLargeInt){$ENDIF};
    ftShortint:
    ...
    ftTimeStampOffset:
      Result := TValue.From<TDateTime>
        (SQLTimeStampOffsetToDateTime(AField.AsSQLTimeStampOffset));
    TFieldType.ftSingle:
...
class function TdxFieldValueConverter.ToValue(const AValue: Variant;
  AMemberInfo: TdxMappingMemberInfo): TValue;
var
  S: string;
  ABytes: TBytes;
