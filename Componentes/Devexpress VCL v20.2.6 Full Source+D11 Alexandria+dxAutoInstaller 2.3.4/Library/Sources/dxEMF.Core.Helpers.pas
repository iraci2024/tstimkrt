...
type
  ...
  TdxSessionState = (GetObjectsNonReentrant, BeginTransaction,
    CommitTransactionNonReentrant, CommitChangesToDataLayer,
    RollbackTransaction, LoadingObjectsIntoNestedUow,
    ReceivingObjectsFromNestedUow, CancelEdit, CreateObjectLoadingEnforcer,
    OptimisticLockFieldsProcessing, ApplyIdentities,
    SessionObjectsToSaveOrDeleteChanging, CommitChangesToDataLayerInner,
    CrossThreadFailure);
  ...
  strict private
    class var FSuppressCrossThreadFailureDetection: Boolean;

  const
    LoadingStates = [TdxSessionState.GetObjectsNonReentrant,
      TdxSessionState.ReceivingObjectsFromNestedUow, TdxSessionState.CancelEdit,
      TdxSessionState.CreateObjectLoadingEnforcer,
      TdxSessionState.OptimisticLockFieldsProcessing,
      TdxSessionState.ApplyIdentities];
    SavingStates = [TdxSessionState.CommitTransactionNonReentrant,
      TdxSessionState.LoadingObjectsIntoNestedUow,
      TdxSessionState.OptimisticLockFieldsProcessing];
    ProhibitedForGetObjects = [TdxSessionState.BeginTransaction,
      TdxSessionState.RollbackTransaction,
      TdxSessionState.CommitChangesToDataLayer,
      TdxSessionState.GetObjectsNonReentrant, TdxSessionState.CancelEdit,
      TdxSessionState.CreateObjectLoadingEnforcer,
      TdxSessionState.OptimisticLockFieldsProcessing];
    ProhibitedForCancelEdit = ProhibitedForGetObjects +
      [TdxSessionState.LoadingObjectsIntoNestedUow];
    ProhibitedGeneric = [TdxSessionState.BeginTransaction,
      TdxSessionState.RollbackTransaction] + SavingStates + LoadingStates;
    ProhibitedSessionObjectsToSaveOrDeleteChanging =
      [TdxSessionState.CommitChangesToDataLayer,
      TdxSessionState.GetObjectsNonReentrant,
      TdxSessionState.LoadingObjectsIntoNestedUow];
  private
    class function AsString(AStates: TdxSessionStates): string;
      overload; static;
    class function AsString(AState: TdxSessionState): string; overload; static;
  public
    class procedure ThrowIfCannotEnter(ASession: TComponent;
      ANewState: TdxSessionState); static;
    class procedure Enter(ASession: TComponent;
      ANewState: TdxSessionState); static;
    class procedure Leave(ASession: TComponent;
      AState: TdxSessionState); static;
    class function IsInAnyOf(ASession: TComponent; AStates: TdxSessionStates)
      : Boolean; static;

    class property SuppressCrossThreadFailureDetection: Boolean
      read FSuppressCrossThreadFailureDetection
      write FSuppressCrossThreadFailureDetection;
  end;
  ...
  strict private
    ...
    procedure InternalInsertObject(AInsertList: TList<TdxModificationStatement>;
      AUpdateList: TList<TInsertUpdate>; AObject: TObject);
    procedure InternalUpdateObject(AUpdateList: TList<TdxModificationStatement>;
      AObject: TObject);
  protected
    ...
    property BatchWideData: TdxBatchWideDataHolderForModification
      read FBatchWideData;
  public
    constructor Create(ASession: TComponent;
...
type
  ...
  protected
    ...
    property ReferredBy: TObjectDictionary<TObject, TdxObjectSet>
      read FReferredBy;
    property References: TObjectDictionary<TObject, TdxObjectSet>
      read FReferences;
    property NonReferencedObjects: TList<TObject> read FNonReferencedObjects;
  public
    constructor Create(ASession: TdxEMFCustomSession;
      ANewObjectsToSave: TList<TObject>);
    destructor Destroy; override;

    procedure MarkObjectAsSaved(AObject: TObject);
    procedure ProcessGoodObjects;
    procedure EliminateLoop(ANextObject: TObject);
    class function DoOrder(ASession: TdxEMFCustomSession;
      ANewObjectsToSave: TList<TObject>): TArray<TObject>; static;
    class function PrepareSaveOrder(ASession: TdxEMFCustomSession)
      : TArray<TObject>; static;
  end;

  { TdxSessionStateStack }

class procedure TdxSessionStateStack.ThrowIfCannotEnter(ASession: TComponent;
  ANewState: TdxSessionState);
var
...
begin
  ...
  begin
    AMessage := Format(sdxSessionEnteringTheX0StateFromTheX1StateIsProhibit,
      [AsString(ANewState), AsString(TdxEMFCustomSessionAccess(ASession)
      .FStateStack), AsString(TdxEMFCustomSessionAccess(ASession).FStateStack *
      AProhibited), ASession.Name]);
    ...
    begin
      ...
      else if IsInAnyOf(ASession, SavingStates) then
        AMessage := AMessage +
...
class procedure TdxSessionStateStack.Enter(ASession: TComponent;
  ANewState: TdxSessionState);
begin
...
class procedure TdxSessionStateStack.Leave(ASession: TComponent;
  AState: TdxSessionState);
begin
...
class function TdxSessionStateStack.IsInAnyOf(ASession: TComponent;
  AStates: TdxSessionStates): Boolean;
begin
...
constructor TdxProcessingSave.Create(ASession: TComponent;
  ABatchWideData: TdxBatchWideDataHolderForModification);
begin
  inherited Create;
  ...
  FProcessedObjectList := TdxSaveOrderer.PrepareSaveOrder
    (TdxEMFCustomSession(ASession));
end;
...
procedure TdxProcessingSave.InternalInsertObject(AInsertList
  : TList<TdxModificationStatement>; AUpdateList: TList<TInsertUpdate>;
  AObject: TObject);
var
...
begin
  ...
  if (AOptimisticLock <> nil) and AOptimisticLock.GetValue(AObject).IsNullOrEmpty
  then
  ...
  try
    ...
    try
      for AUpdateMember in AEntityInfo.GetPropertiesListForUpdateInsert(AObject,
        False, False) do
      begin
        if (AKey.MappingField = AUpdateMember.MappingField) and
          (AKey <> AUpdateMember) then
          Continue;
        if AUpdateMember.ReferenceType <> nil then
        begin
          AReferredObject := AUpdateMember.GetValue(AObject).AsObject;
          if (AReferredObject <> nil) and ASession.IsNewObject(AReferredObject)
            and not FBatchWideData.IsObjectAlreadyInserted(AReferredObject) then
      ...
      begin
        ...
        begin
          ...
          if AKeyValue.IsNullOrEmpty or (AKeyValue.AsGUID = TGuid.Empty) then
            AKey.SetValue(AObject,
              TValue.From<TGuid>(AKey.DBGenerator.NewGuid));
        end
        else if not AKey.IsIdentity then
          raise EdxKeysAutogenerationNonSupportedTypeException.Create
            (AEntityInfo.FullName);
      end
      else
        TdxEMFCustomSessionAccess(ASession)
          .CheckDuplicateObjectInIdentityMap(AObject);
      FBatchWideData.RegisterInsertedObject(AObject);
      AQueries := TdxInsertQueryGenerator.GenerateInsert(FBatchWideData,
        AObject, AInsertMembers.ToArray);
      try
        ...
        if AUpdateMembers.Count > 0 then
          AUpdateList.Add(TInsertUpdate.Create(AObject,
            AUpdateMembers.ToArray));
      finally
        AQueries.Free;
      end;
    finally
...
procedure TdxProcessingSave.InternalUpdateObject(AUpdateList
  : TList<TdxModificationStatement>; AObject: TObject);
var
  AEntityInfo: TdxEntityInfo;
...
begin
  ASession := TdxEMFCustomSession(Session);
  TdxEMFCustomSessionAccess(ASession).CheckDuplicateObjectInIdentityMap
    (AObject);
  AEntityInfo := EntityManager.GetEntityInfo(AObject);
  AOptimisticLock := AEntityInfo.OptimisticLockField;
  APropsList := AEntityInfo.GetPropertiesListForUpdateInsert(AObject,
    True, False);
  if TdxEMFSessionOptionsAccess(ASession.Options)
    .LockingOption = TdxLockingOption.Optimistic then
  begin
  ...
  AStatement := TdxUpdateQueryGenerator.GenerateUpdate(FBatchWideData, AObject,
    APropsList.ToArray);
  try
...
begin
  ...
  try
    ...
    begin
      AStatements := TdxUpdateQueryGenerator.GenerateUpdate(FBatchWideData,
        AUpdate.TheObject, AUpdate.UpdateMembers);
      try
  ...
  begin
    ...
    try
      for I := 0 to FInsertList.Count - 1 do
        TdxEMFCustomSessionAccess(ASession).SetKeyValue(FInsertList[I],
          AResult.Identities[I].Value);
    finally
...
constructor TdxSaveOrderer.Create(ASession: TdxEMFCustomSession;
  ANewObjectsToSave: TList<TObject>);
var
...
begin
  ...
  FReferredBy := TObjectDictionary<TObject, TdxObjectSet>.Create([doOwnsValues],
    ANewObjectsToSave.Count);
  FReferences := TObjectDictionary<TObject, TdxObjectSet>.Create([doOwnsValues],
    ANewObjectsToSave.Count);
...
class function TdxSaveOrderer.DoOrder(ASession: TdxEMFCustomSession;
  ANewObjectsToSave: TList<TObject>): TArray<TObject>;
var
...
class function TdxSaveOrderer.PrepareSaveOrder(ASession: TdxEMFCustomSession)
  : TArray<TObject>;
var
