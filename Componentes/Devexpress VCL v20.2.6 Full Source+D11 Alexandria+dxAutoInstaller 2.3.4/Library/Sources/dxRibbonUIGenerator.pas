...
uses
  Classes, ActnList, Controls, dxBar, dxRibbon, dxBarUIGenerator, dxUIGenerator,
  dxRibbonColorGallery, dxRibbonGallery;
...
type
  ...
  protected
    ...
    procedure PlaceCommand(AAction: TAction;
      AItem: TdxUIGeneratorCommandInfo); override;
  public
...
class procedure TdxUIGeneratorGalleryItemInfo.Initialize(AItem: TdxBarItem;
...
begin
  ...
  InitializeGallery(AGalleryItem, ACommandInfo as TdxUIGeneratorGalleryInfo,
    AAdapter);
end;

class procedure TdxUIGeneratorGalleryItemInfo.InitializeGallery
  (AGallery: TdxRibbonGalleryItem; ACommandInfo: TdxUIGeneratorGalleryInfo;
  AAdapter: TdxUIGeneratorAdapter);

  procedure InitializeGalleryOptions(AGalleryInfo: TdxUIGeneratorGalleryInfo;
    AGallery: TdxRibbonGalleryItem);
  begin
    ...
    else if AGalleryInfo.ShowItemDescription then
      AGallery.GalleryInMenuOptions.ItemTextKind := itkCaptionAndDescription
    else
      AGallery.GalleryInMenuOptions.ItemTextKind := itkCaption;
    TdxCustomRibbonGalleryOptionsAccess(AGallery.GalleryOptions)
      .LongDescriptionDefaultRowCount := AGalleryInfo.ItemDescriptionRowCount;
    AGallery.GalleryInMenuOptions.ItemTextAlignVert :=
      AGalleryInfo.ItemTextAlignVert;
  end;

  procedure InitializeGalleryGroup(AGalleryGroupItemInfo
    : TdxUIGeneratorGalleryGroupItemInfo;
    AGalleryInfo: TdxUIGeneratorGalleryInfo; AGallery: TdxRibbonGalleryItem;
    out AGroup: TdxRibbonGalleryGroup);
  var
    I: Integer;
  begin
    AGroup := nil;
    for I := 0 to AGallery.GalleryCategories.Count - 1 do
      if AGallery.GalleryCategories.Groups[I].Caption = AGalleryGroupItemInfo.GroupCaption
      then
      begin
  ...
  procedure InitializeGalleryGroupItem(AItemInfo
    : TdxUIGeneratorGalleryGroupItemInfo; AGroup: TdxRibbonGalleryGroup);

    procedure LoadGlyphFromExternalImageList(AItem: TdxRibbonGalleryGroupItem;
      AExternalImageList: TCustomImageList; AExternalImageIndex: Integer);
    var
      ABitmap: TBitmap;
    begin
      if AExternalImageList is TcxImageList then
        TcxImageList(AExternalImageList).GetImage(AExternalImageIndex,
          AItem.Glyph)
      else
      begin
  ...
  begin
    ...
    for I := 0 to AGroup.Items.Count - 1 do
      if TdxRibbonGalleryGroupItemAccess(AGroup.Items[I])
        .FActionIndex = AItemInfo.ActionIndex then
      begin
    ...
    if ACommandInfo.ExternalImageList = nil then
      AItem.ImageIndex := TdxUIGeneratorAdapterAccess(AAdapter)
        .AddImage(AGroup.Options.Images, AItemInfo.ImageFileName)
    else
      LoadGlyphFromExternalImageList(AItem, ACommandInfo.ExternalImageList,
        AItemInfo.ExternalImageIndex);
    TdxRibbonGalleryGroupItemAccess(AItem).FActionIndex :=
      AItemInfo.ActionIndex;
  end;
...
begin
  ...
  begin
    InitializeGalleryGroup(ACommandInfo.GroupItems[I], ACommandInfo, AGallery,
      AGalleryGroup);
...
begin
  Result := inherited IsBuiltInCommand(AAction, ACommandInfo) and
    Supports(AAction, IdxActionGalleryClient) and
    (ACommandInfo is TdxUIGeneratorGalleryInfo);
end;

{ TdxUIGeneratorRibbonColorGalleryItemInfo }

class function TdxUIGeneratorRibbonColorGalleryItemInfo.GetBarItemClass
  : TdxBarItemClass;
begin
...
class function TdxUIGeneratorRibbonGalleryItemInfo.GetBarItemClass
  : TdxBarItemClass;
begin
...
class procedure TdxUIGeneratorRibbonGalleryItemInfo.Initialize
  ...
  AAdapter: TdxUIGeneratorAdapter);
begin
  InitializeGallery(AItem as TdxRibbonGalleryItem,
    ACommandInfo as TdxUIGeneratorGalleryInfo, AAdapter);
end;
...
class function TdxUIGeneratorRibbonAdapter.CreateComponent(AOwner: TComponent)
  : TComponent;
begin
...
begin
  ...
  begin
    ...
    Target.BarManager.Name := TdxUIGeneratorHelper.GenerateName
      (Target.BarManager);
  end;
...
function TdxUIGeneratorRibbonAdapter.GetBarPopupMenuClass
  : TdxBarCustomPopupMenuClass;
begin
  Result := TdxRibbonPopupMenu;
end;

procedure TdxUIGeneratorRibbonAdapter.PlaceCommand(AAction: TAction;
  AItem: TdxUIGeneratorCommandInfo);
var
...
begin
  ...
  begin
    ...
    TdxUIGeneratorHelper.LoadImage(AGroup.ToolBar.Glyph,
      AItem.Category.BarGlyphFileName, ImageSet);
