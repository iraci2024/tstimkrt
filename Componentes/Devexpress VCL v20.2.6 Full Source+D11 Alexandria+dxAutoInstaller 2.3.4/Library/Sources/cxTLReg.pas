...
uses
  Classes, SysUtils, TypInfo, Types, DesignIntf, DesignEditors, VCLEditors,
  dxCore,
  cxInplaceContainer, Forms, DB, cxDesignWindows, cxPropEditors, cxClasses,
  cxControls,
...
procedure ShowItemsDesigner(AEditor: TcxTreeListComponentEditor;
  APageIndex: Integer);
var
  ADesigner: TcxTreeListBandColumnDesigner;
begin
  ADesigner := TcxTreeListBandColumnDesigner
    (ShowFormEditorClass(AEditor.Designer, AEditor.Component,
...
begin
  ...
  begin
    ...
    else if AOperations.SupportCreateAllItems { and (not IsLinkable) } then
    begin
    ...
    if (TreeList.ColumnCount > 0) and TreeList.ConditionalFormatting.CanShowRulesManagerDialog
    then
    begin
      FItems.AddObject('-', TObject(-1));
...
procedure TcxDBTreeListComponentEditor.PrepareItem(Index: Integer;
  const AItem: TDesignMenuItem);
begin
  if InRange(Index, 0, FItems.Count - 1) and
    (FItems.Objects[Index] = TObject(-2)) then
    AItem.Enabled := False;
  inherited PrepareItem(Index, AItem);
end;

procedure TcxDBTreeListComponentEditor.PrepareLinkableSubItem
  (const AItem: TDesignMenuItem);
begin
  AItem.Checked := (Component as TcxDBTreeList)
...
function TcxDBTreeListDataControllerFieldNameProperty.GetDataSource
  : TDataSource;
begin
...
begin
  ...
  try
    Designer.GetComponentNames
      (GetTypeData(PTypeInfo(TcxTreeListColumn.ClassInfo)), AddComponent);
...
procedure TcxDesignSelectionListener.AddListener(AListener: TObject;
  AddListener: Boolean);
begin
...
procedure TcxDesignSelectionListener.ItemDeleted(const ADesigner: IDesigner;
  AItem: TPersistent);
begin
end;

procedure TcxDesignSelectionListener.ItemInserted(const ADesigner: IDesigner;
  AItem: TPersistent);
begin
end;

procedure TcxDesignSelectionListener.ItemsModified(const ADesigner: IDesigner);
begin
end;

procedure TcxDesignSelectionListener.SelectionChanged(const ADesigner
...
procedure TcxDesignSelectionListener.DesignerOpened(const ADesigner: IDesigner;
  AResurrecting: Boolean);
begin
end;

procedure TcxDesignSelectionListener.DesignerClosed(const ADesigner: IDesigner;
  AGoingDormant: Boolean);
begin
end;
...
begin
  ...
  RegisterComponents(dxCoreLibraryProductPage, [TcxTreeList, TcxDBTreeList,
    TcxVirtualTreeList]);
  RegisterClasses([TcxTreeListColumn, TcxDBTreeListColumn, TcxTreeListBands,
    TcxTreeListBand, TcxTreeListStyleSheet]);
  RegisterNoIcon([TcxTreeListColumn, TcxDBTreeListColumn,
  ...
  RegisterPropertyEditor(TypeInfo(TcxTreeListColumn),
    TcxTreeListOptionsBehavior, 'IncSearchItem', TcxTreeListColumnProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TComponent), TcxTreeListPopupMenu,
    'PopupMenu', TcxControlPopupMenuProperty);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomTreeList,
    'NavigatorEvents', TcxTreeListNavigatorEventsProperty);

  RegisterPropertyEditor(TypeInfo(string), TcxDBItemDataBinding, 'FieldName',
    TcxDBTreeListDataBindingFieldNameProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxDefaultBoolean), TcxTreeListOptionsData,
