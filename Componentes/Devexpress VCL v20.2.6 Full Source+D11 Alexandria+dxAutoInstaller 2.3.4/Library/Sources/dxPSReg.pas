...
procedure Register;
procedure dxPSRegisterReportLinkUnit(const AUnitName: string;
...
uses
  ...
  dxPSfmTtl, dxPSFileBasedXplorer, dxPSGraphicLnk, dxPSContainerLnk,
  dxPSTextLnk,
  dxBrhDlg, dxPSfmEditDesc, dxPSPrVwDsg, dxPSfmStlAdd, dxPSfmLnkDsg,
  dxPSfmLnkAdd,
...
const
  ...
  IDB_DXPSDESIGN_PAPERORIENTATION_LANDSCAPE =
    'IDB_DXPSDESIGN_PAPERORIENTATION_LANDSCAPE';
  IDB_DXPSDESIGN_PAPERORIENTATION_PORTRAIT =
    'IDB_DXPSDESIGN_PAPERORIENTATION_PORTRAIT';
  IDB_DXPSDESIGN_PAPER_ENVELOPE = 'IDB_DXPSDESIGN_PAPER_ENVELOPE';
  IDB_DXPSDESIGN_PAPER_STANDARD = 'IDB_DXPSDESIGN_PAPER_STANDARD';
  IDB_DXPSDESIGN_PRINTORDER_OVERTHENDOWN =
    'IDB_DXPSDESIGN_PRINTORDER_OVERTHENDOWN';
  IDB_DXPSDESIGN_PRINTORDER_DOWNTHENOVER =
    'IDB_DXPSDESIGN_PRINTORDER_DOWNTHENOVER';
...
type
  ...
  protected
    ...
    function IsAcceptableComponentClass(AComponentClass: TClass)
      : Boolean; virtual;
  public
    function GetAttributes: TPropertyAttributes; override;
    procedure GetValues(Proc: TGetStrProc); override;
  ...
  TdxPaperSourcePropertyEditor = class
    (TdxCustomListDrawingIntegerPropertyEditor)
  protected
    function GetImageIndex(const Value: string): Integer; override;
    function GetImageList: TImageList; override;
  ...
  public
    ...
    procedure PopulateReportLinksUnits(Proc: TGetStrProc;
      AComponentPrinter: TCustomdxComponentPrinter); overload;
    procedure PopulateReportLinkUnits(Proc: TGetStrProc;
      ALinkClass: TdxReportLinkClass); overload;
...
begin
  case AIndex of
    0:
      if ReportLink.ComponentPrinter.DesignReport(ReportLink) then
        Designer.Modified;
    1:
      begin
...
begin
  case AIndex of
    0:
      Result := sdxShowReportDesigner;
    1:
      Result := sdxMakeCurrent;
    2:
      Result := '-';
    3:
      Result := sdxRestoreOriginal;
    4:
      Result := sdxRestoreDefaults;
    5:
      Result := '-';
    6:
      Result := sdxPageBackground;
    7:
      Result := sdxClearBackground;
    8:
      Result := '-';
    9:
      Result := sdxPageSetup;
    10:
      Result := sdxPrintPreview;
    11:
      Result := sdxPrint;
  end;
end;
...
begin
  case Index of
    ...
    9:
      AItem.Enabled := rlcPageSetup in TBasedxReportLink(Component)
        .Capabilities;
    10:
...
function TdxReportLinkComponentPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
...
procedure TdxReportLinkComponentPropertyEditor.AddComponent
  (const AName: string);
begin
  if IsComponentValid(Designer.GetComponent(AName)) then
    FComponents.Add(AName);
end;

procedure TdxReportLinkComponentPropertyEditor.GetComponents;
begin
  FComponents.Clear;
  Designer.GetComponentNames(GetTypeData(PTypeInfo(TComponent.ClassInfo)),
    AddComponent);
end;

function TdxReportLinkComponentPropertyEditor.IsComponentValid
  (AComponent: TComponent): Boolean;
begin
  Result := (AComponent <> nil) and not(AComponent is TBasedxReportLink) and
    not(AComponent is TCustomdxComponentPrinter) and
    ReportLink.Supports(AComponent);
end;
...
function TdxCompositionReportLinkPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes - [paMultiSelect];
end;
...
function TdxCompositionReportLinkPropertyEditor.GetItems
  : TdxCompositionLinkItems;
begin
  Result := TdxCompositionLinkItems(TdxCompositionLinkItem(GetComponent(0))
    .Collection);
end;
...
function TdxCompositionReportLinkPropertyEditor.IsComponentValid
  (AComponent: TComponent): Boolean;
begin
  Result := (AComponent is TBasedxReportLink) and
    Items.IsLinkComposable(TBasedxReportLink(AComponent));
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if IsAcceptableComponentClass(ComponentClass) then
        Proc(ComponentClass.ClassName);
    end;
  finally
    List.Free;
  end;
...
procedure TdxPSExplorerFilterPropertyEditor.ListMeasureWidth
  (const Value: string; ACanvas: TCanvas; var AWidth: Integer);
begin
  Inc(AWidth, 1 + ImageList.Height + 2);
end;

procedure TdxPSExplorerFilterPropertyEditor.ListMeasureHeight
...
function TdxPSExplorerFilterPropertyEditor.GetImageIndex
...
function TdxPSExplorerFilterPropertyEditor.IsAcceptableComponentClass
  (AComponentClass: TClass): Boolean;
begin
  Result := (AComponentClass <> nil) and not AComponentClass.InheritsFrom
    (ExtCtrls.TPage) and not AComponentClass.InheritsFrom(TabNotBk.TTabPage);
end;

{ TdxPrintStyleImageIndexPropertyEditor }

function TdxPrintStyleImageIndexPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
...
procedure TdxPrintStyleImageIndexPropertyEditor.ListMeasureHeight
  (const Value: string; ACanvas: TCanvas; var AHeight: Integer);
var
...
procedure TdxPrintStyleImageIndexPropertyEditor.ListMeasureWidth
  (const Value: string; ACanvas: TCanvas; var AWidth: Integer);
var
  Images: TCustomImageList;
begin
...
procedure TdxPrintStyleImageIndexPropertyEditor.ListDrawValue
...
begin
  ...
  ACanvas.TextRect(R, R.Left + 1,
    R.Top + (R.Bottom - R.Top - ACanvas.TextHeight(Value)) div 2, Value);
end;

{ TdxPaperOrientationPropertyEditor }

function TdxPaperOrientationPropertyEditor.GetImageIndex
  (const Value: string): Integer;
begin
...
function TdxPrintOrderPropertyEditor.GetImageIndex(const Value: string)
  : Integer;
begin
...
function TdxPathPropertyEditor.GetExplorer(Index: Integer)
...
begin
  ...
  {$IFNDEF LIMITED_EDITION} + [paDialog]{$ENDIF};
end;
...
procedure TdxCustomContainerReportLinkComponentPropertyEditor.ListMeasureWidth
  (const Value: string; ACanvas: TCanvas; var AWidth: Integer);
var
  W: Integer;
begin
...
procedure TdxCustomContainerReportLinkComponentPropertyEditor.ListMeasureHeight
  (const Value: string; ACanvas: TCanvas; var AHeight: Integer);
begin
  AHeight := 1 + Images.Height + 1;
end;

procedure TdxCustomContainerReportLinkComponentPropertyEditor.ListDrawValue
  (const Value: string; ACanvas: TCanvas; const ARect: TRect;
  ASelected: Boolean);
var
  X, Y: Integer;
...
function TdxCustomContainerReportLinkComponentPropertyEditor.GetProject
  : IOTAProject;
var
  Module: IOTAModule;
...
begin
  ...
  if HasMultipleProjects then
    Result := nil;
end;

function TdxCustomContainerReportLinkComponentPropertyEditor.IsComponentValid
  (AComponent: TComponent): Boolean;
begin
  Result := inherited IsComponentValid(AComponent) and
    ((AComponent = nil) or (AComponent = Designer.GetRoot) or
    (AComponent.Owner = Designer.GetRoot));
end;

function TdxCustomContainerReportLinkComponentPropertyEditor.
  IsSupportedModuleType(const AModuleType: TOTAModuleType): Boolean;
begin
  Result := AModuleType = omtForm;
end;

function TdxCustomContainerReportLinkComponentPropertyEditor.GetImageIndex
  (const Value: string): Integer;
begin
  Result := dxPSDsgProxies.dxIdeImagesProvider.ImageIndexesByObject
    [Designer.GetObject(Value)];
end;

function TdxCustomContainerReportLinkComponentPropertyEditor.GetImages
  : TCustomImageList;
begin
...
begin
  ...
  with GetTypeData(EnumType)^ do
    ...
    begin
      ...
      begin
        S := GetEnumName(TypeInfo(TdxMeasurementUnits),
          Integer(GetDefaultMeasurementUnits));
...
procedure TdxMeasurementUnitsPropertyEditor.SetValue(const Value: string);
  ...
  begin
    DefaultEnumName := GetEnumName(TypeInfo(TdxMeasurementUnits),
      Integer(muDefault));
...
begin
  ...
  begin
    ...
    begin
      for I := 1 to PropCount - 1 do
        TBasedxReportLink(GetComponent(I)).PDFExportOptions.Assign
          (AReportLink.PDFExportOptions);
    end;
    Designer.Modified;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      for I := 1 to PropCount - 1 do
        TBasedxReportLink(GetComponent(I)).PrinterPage.Assign
          (ReportLink.PrinterPage);
    end;
    Designer.Modified;
  end;
end;
...
begin
  ...
  begin
    for I := 1 to PropCount - 1 do
      TBasedxReportLink(GetComponent(I)).ReportFootnotes.Assign
        (AReportLink.ReportFootnotes);
    Designer.Modified;
  end;
end;
...
begin
  ...
  begin
    for I := 1 to PropCount - 1 do
      TBasedxReportLink(GetComponent(I)).ReportTitle.Assign
        (AReportLink.ReportTitle);
    Designer.Modified;
  end;
end;
...
begin
  ...
  begin
    if PropCount > 1 then
      for I := 1 to PropCount - 1 do
        TBasedxPrintStyle(GetComponent(I)).PrinterPage.Assign
          (PrintStyle.PrinterPage);
    Designer.Modified;
  end;
end;
...
begin
  ...
  if R <> nil then
    with R do
      Result := Format('(Bottom: %d; Left: %d; Right: %d; Top: %d)',
...
procedure TdxCustomListDrawingIntegerPropertyEditor.ListMeasureWidth
  (const Value: string; ACanvas: TCanvas; var AWidth: Integer);
var
  W: Integer;
begin
...
procedure TdxCustomListDrawingIntegerPropertyEditor.ListMeasureHeight
  (const Value: string; ACanvas: TCanvas; var AHeight: Integer);
begin
  AHeight := ImageList.Height + 2;
end;

procedure TdxCustomListDrawingIntegerPropertyEditor.ListDrawValue
  (const Value: string; ACanvas: TCanvas; const ARect: TRect;
  ASelected: Boolean);
var
  R: TRect;
...
function TdxPaperSourcePropertyEditor.GetImageIndex
  (const Value: string): Integer;
begin
...
begin
  if CanCreateStyleManager then
    Proc(sdxCreateNewStyleManager);
  inherited;
end;
...
begin
  Result := (Root <> nil) and not Designer.IsSourceReadOnly and
    not(csInline in Root.ComponentState);
end;

function TdxReportLinkStyleManagerPropertyEditor.CreateNewStyleManager
...
function TdxReportDocumentDescriptionPropertyEditor.GetAttributes
  : TPropertyAttributes;
begin
  Result := inherited GetAttributes + [paDialog];
end;
...
procedure TdxComponentPrinterSelectionEditor.PopulatePreviewDialogsUnits
  (Proc: TGetStrProc);
var
  I: Integer;
begin
...
procedure HideProperty(PropertyType: PTypeInfo; ComponentClass: TClass;
...
begin
  ...
  RegisterComponents(dxPSProductPage, [TdxComponentPrinter,
    TdxPrintStyleManager, TdxPrintDialog, TdxPageSetupDialog,
    TdxPSEngineController, TdxPSFileBasedExplorer]);

  RegisterNoIcon([TBasedxReportLink, TdxCompositionReportLink,
  ...
  RegisterPropertyEditor(TypeInfo(TComponent), TdxPSPreviewWindow,
    'PreviewPopupMenu', TcxControlPopupMenuProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxPSPreviewDialogStyle), nil,
    'PreviewDialogStyle', TdxPSPreviewWindowProperty);

  RegisterPropertyEditor(TypeInfo(string), TdxPSFileBasedExplorer, 'RootPath',
    TdxPathPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TdxComponentPrinter, 'DateFormat',
    TdxDateFormatsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TdxComponentPrinter, 'TimeFormat',
    TdxTimeFormatsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TIcon), TdxPreviewOptions, 'Icon',
    TdxPreviewIconPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPageNumberFormat), TdxComponentPrinter,
    'PageNumberFormat', TdxPageNumberFormatsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(string), TdxComponentPrinter, 'RegistryPath',
    TdxRegistryPathPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TBasedxReportLink), TdxComponentPrinter,
    'ExplorerStubLink', TdxExplorerStubLinkPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TComponent), TBasedxReportLink, 'Component',
    TdxReportLinkComponentPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TComponent), TdxCustomContainerReportLink,
    'Component', TdxCustomContainerReportLinkComponentPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TBasedxReportLink, 'Index',
    TdxIndexPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Boolean), TBasedxReportLink, 'ShowDesigner',
    TdxReportLinkDesignerPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPrinterPage), TBasedxReportLink,
    'PrinterPage', TdxPrinterPagePropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPSPDFReportExportOptions),
    TBasedxReportLink, 'PDFExportOptions', TdxPDFOptionsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxReportTitle), TBasedxReportLink,
    'ReportTitle', TdxReportTitlePropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxReportFootnotes), TBasedxReportLink,
    'ReportFootnotes', TdxReportFootnotesPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TFont), TBasedxReportLink, '',
    TTTFontPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TBasedxReportLink, 'DateFormat',
    TdxDateFormatsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TBasedxReportLink, 'TimeFormat',
    TdxTimeFormatsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPageNumberFormat), TBasedxReportLink,
    'PageNumberFormat', TdxPageNumberFormatsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPrintStyleManager), TBasedxReportLink,
    'StyleManager', TdxReportLinkStyleManagerPropertyEditor);

  RegisterPropertyEditor(TypeInfo(string), TdxPSReportDocument, 'Description',
    TdxReportDocumentDescriptionPropertyEditor);

  RegisterPropertyEditor(TypeInfo(Integer), TBasedxPrintStyle, 'ImageIndex',
    TdxPrintStyleImageIndexPropertyEditor);
  RegisterPropertyEditor(TypeInfo(string), TCustomdxPSExplorer, 'FilterLink',
    TdxPSExplorerFilterPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPrinterOrientation), nil, '',
    TdxPaperOrientationPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPageOrder), nil, '',
    TdxPrintOrderPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TComponent), TdxCompositionLinkItem,
    'ReportLink', TdxCompositionReportLinkPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TStrings), TdxPrintStyleManager,
    'AutoHFTextEntries', TdxAutoHFTextEntriesPropertyEditor);

  RegisterPropertyEditor(TypeInfo(Boolean), TBasedxPrintStyle,
    'ShowPageSetupDlg', TdxShowPgsDlgPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxPrinterPage), TBasedxPrintStyle,
    'PrinterPage', TdxPrinterPagePropertyEditor2);
  RegisterPropertyEditor(TypeInfo(Integer), TBasedxPrintStyle, 'Index',
    TdxIndexPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TBitmap), TBasedxPrintStyle, 'StyleGlyph',
    TdxStyleGlyphPropertyEditor);

  RegisterPropertyEditor(TypeInfo(Integer), TdxPrinterPage, 'DMPaper',
    TdxDMPaperPropertyEditor);
  RegisterPropertyEditor(TypeInfo(Integer), TdxPrinterPage, 'PaperSource',
    TdxPaperSourcePropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxMeasurementUnits), TdxPrinterPage,
    'MeasurementUnits', TdxMeasurementUnitsPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TcxTextAlignY), TCustomdxPageObject, '',
    TdxHeaderFooterTextAlignYPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TcxTextAlignX), TdxReportTitle, 'TextAlignX',
    TdxReportTitleTextAlignXPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TcxTextAlignY), TdxReportTitle, 'TextAlignY',
    TdxReportTitleTextAlignYPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TdxBackground), nil, '',
    TdxBackgroundPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TdxPointWrapper), nil, '',
    TdxPointWrapperEditor);
  RegisterPropertyEditor(TypeInfo(TdxRectWrapper), nil, '',
    TdxRectWrapperEditor);

  RegisterPropertyEditor(TypeInfo(string), TdxPSEngineController,
    'RegistryPath', TdxRegistryPathPropertyEditor);
  RegisterPropertyEditor(TypeInfo(string), TdxPSOptionsStoring, 'RegistryPath',
    TdxRegistryPathPropertyEditor);
  RegisterPropertyEditor(TypeInfo(string), TdxPageSetupDialog, 'RegistryPath',
    TdxRegistryPathPropertyEditor);
  RegisterPropertyEditor(TypeInfo(string), TdxPrintDialog, 'RegistryPath',
    TdxRegistryPathPropertyEditor);

  RegisterPropertyEditor(TypeInfo(TBrush), nil, '', TdxBrushPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TcxTextAlignX), TdxCustomTextReportLink,
    'Alignment', TdxTextReportLinkAlignmentPropertyEditor);

  RegisterSelectionEditor(TCustomdxComponentPrinter,
    TdxComponentPrinterSelectionEditor);
  RegisterSelectionEditor(TdxPrintStyleManager,
    TdxPrintStyleManagerSelectionEditor);
  RegisterSelectionEditor(TdxPageSetupDialog,
    TdxPageSetupDialogSelectionEditor);
  ...
  HideProperty(TypeInfo(TdxReportTitleMode), TBasedxReportLink,
    'ReportTitleMode');

  HideProperty(TypeInfo(TComponent), TdxCompositionReportLink, 'Component');
  HideProperty(TypeInfo(TdxReportTitle), TdxCompositionReportLink,
    'ReportTitle');
  HideProperty(TypeInfo(TNotifyEvent), TdxCompositionReportLink,
    'OnChangeComponent');
  HideProperty(TypeInfo(TdxCustomDrawReportLinkHFEvent),
    TdxCompositionReportLink, 'OnCustomDrawPageFooter');
  HideProperty(TypeInfo(TdxCustomDrawReportLinkHFEvent),
    TdxCompositionReportLink, 'OnCustomDrawPageHeader');
  HideProperty(TypeInfo(TdxCustomDrawReportLinkTitleEvent),
    TdxCompositionReportLink, 'OnCustomDrawReportLinkTitle');
  HideProperty(TypeInfo(TdxMeasureReportLinkTitleEvent),
    TdxCompositionReportLink, 'OnMeasureReportLinkTitle');
  ...
  HideProperty(TypeInfo(TNotifyEvent), TdxPictureReportLink,
    'OnChangeComponent');
...
procedure LoadImage(AImageList: TImageList; const AResName: string;
  AMasked: Boolean);
var
  Bitmap: TBitmap;
begin
