...
uses
  ...
  dxLayoutContainer, ExtCtrls, dxLayoutControl,
  cxGridLayoutView, dxLayoutcxEditAdapters, cxContainer, cxEdit, cxTrackBar,
  cxLabel, cxTextEdit, cxMaskEdit,
  cxSpinEdit, cxCheckBox, cxClasses, cxGridWizardCustomPage, cxGridWizardStrs,
...
type
  ...
  TcxGridWizardLayoutViewOptionsCarouselModePageFrame = class
...
begin
  lbBackgroundRecordOptions.Caption :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordOptions);

  lciBackgroundRecordAlphaLevel.Caption :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordAlphaLevel);
  trbBackgroundRecordAlphaLevel.Hint :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordAlphaLevelHint);

  lciBackgroundRecordStartScale.Caption :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordStartScale);
  trbBackgroundRecordStartScale.Hint :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordStartScaleHint);

  lciBackgroundRecordEndScale.Caption :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordEndScale);
  trbBackgroundRecordEndScale.Hint :=
    cxGetResourceString(@scxgwLayoutViewCarouselBackgroundRecordEndScaleHint);

  lbAngleOptions.Caption := cxGetResourceString
    (@scxgwLayoutViewCarouselAngleOptions);

  lciRollAngle.Caption := cxGetResourceString
    (@scxgwLayoutViewCarouselRollAngle);
  trbRollAngle.Hint := cxGetResourceString
    (@scxgwLayoutViewCarouselRollAngleHint);

  lciPitchAngle.Caption := cxGetResourceString
    (@scxgwLayoutViewCarouselPitchAngle);
  trbPitchAngle.Hint := cxGetResourceString
    (@scxgwLayoutViewCarouselPitchAngleHint);
  chbAutoPitchAngle.Caption := cxGetResourceString
    (@scxgwLayoutViewCarouselAutoPitchAngle);
  chbAutoPitchAngle.Hint := cxGetResourceString
    (@scxgwLayoutViewCarouselAutoPitchAngleHint);

  lbOther.Caption := cxGetResourceString(@scxgwCommonGroupCaptionOthers);

  lciAnimationInterval.Caption := cxGetResourceString
    (@scxgwLayoutViewCarouselAnimationInterval);
  seAnimationInterval.Hint := cxGetResourceString
  ...
  lciRecordCount.Caption := cxGetResourceString
    (@scxgwLayoutViewCarouselRecordCount);
  seRecordCount.Hint := cxGetResourceString
...
begin
  Visible := TcxGridLayoutView(Helper.GridView)
...
function TcxGridWizardLayoutViewOptionsCarouselModePageFrame.
...
procedure TcxGridWizardLayoutViewOptionsCarouselModePageFrame.RefreshPreviewGrid
  (Sender: TObject);
begin
