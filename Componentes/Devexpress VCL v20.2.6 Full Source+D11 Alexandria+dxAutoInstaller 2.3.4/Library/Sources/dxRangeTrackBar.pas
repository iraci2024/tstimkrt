...
type
  ...
  TdxRangeTrackBarGetCustomHintEvent = procedure(Sender: TObject;
  ...
  protected
    ...
    procedure DoGetPositionHint(var AHintText: string; var ACanShow: Boolean;
      var AIsHintMultiLine: Boolean); override;
    procedure DrawCustomThumb(ACanvas: TcxCanvas); override;
    function DrawingThumbRectToRealThumbRect(ACanvas: TcxCanvas;
      const AThumbRect: TRect): TRect; override;
    procedure DrawThumb(ACanvas: TcxCanvas); override;
    procedure DrawLeftThumb(ACanvas: TcxCanvas; const R: TRect;
      AState: Integer);
    procedure DrawRightThumb(ACanvas: TcxCanvas; const R: TRect;
      AState: Integer);
    ...
    property RangeTrackBarProperties: TdxCustomRangeTrackBarProperties
      read GetRangeTrackBarProperties;
  ...
  protected
    procedure CalculateThumbSize(AViewInfo: TcxCustomTrackBarViewInfo);
      override;
    procedure CalculateThumbRect(ACanvas: TcxCanvas;
      AViewInfo: TcxCustomTrackBarViewInfo); override;
  ...
  protected
    ...
    procedure DoDrawRangeMinThumb(Sender: TObject; ACanvas: TcxCanvas;
      const ARect: TRect);
    procedure DoDrawRangeMaxThumb(Sender: TObject; ACanvas: TcxCanvas;
      const ARect: TRect);
    function EditValueToRange(const AEditValue: TcxEditValue): Int64; overload;
    function EditValueToRange(AEditValueFormat: TdxRangeTrackBarEditValueFormat;
      ADelimiter: Char; const AEditValue: TcxEditValue): Int64; overload;
    class function GetViewDataClass: TcxCustomEditViewDataClass; override;
    property EditValueFormat: TdxRangeTrackBarEditValueFormat
      read FEditValueFormat write SetEditValueFormat default revfInt64;
    property Delimiter: Char read FDelimiter write SetDelimiter
      stored IsDelimiterStored;
    property ShowSelection: Boolean read FShowSelection write SetShowSelection
      default True;
    property OnDrawRangeMaxThumb: TcxDrawThumbEvent read FOnDrawMaxThumb
      write FOnDrawMaxThumb;
    property OnDrawRangeMinThumb: TcxDrawThumbEvent read GetOnDrawMinThumb
      write SetOnDrawMinThumb;
  public
    constructor Create(AOwner: TPersistent); override;
    procedure CalculateEditValueByRange(AMin, AMax: Integer;
      out AValue: Variant);
    procedure CalculateRangeByEditValue(AValue: Variant;
      AEditValueFormat: TdxRangeTrackBarEditValueFormat; ADelimiter: Char;
      out AMin, AMax: Integer); overload;
    procedure CalculateRangeByEditValue(AValue: Variant;
      out AMin, AMax: Integer); overload;
    ...
    property OnGetPositionHint: TdxRangeTrackBarGetCustomHintEvent
  ...
  protected
    procedure UpdatePos(AValue: Integer;
      AThumb: TcxTrackBarThumbViewInfo); override;
  public
  ...
  public
    ...
    property ActiveProperties: TdxCustomRangeTrackBarProperties
  ...
  public
    class function GetPropertiesClass: TcxCustomEditPropertiesClass; override;
    property ActiveProperties: TdxRangeTrackBarProperties
      read GetActiveProperties;
  published
    property Anchors;
    property BiDiMode;
...
class function TdxCustomRangeTrackBar.GetPropertiesClass
...
begin
  ...
  begin
    ActiveProperties.CalculateRangeByEditValue(EditValue, FEditValueFormat,
      FDelimiter, AMin, AMax);
...
procedure TdxCustomRangeTrackBar.SetRangeValue(AValue: Int64;
  AIsUserAction: Boolean);

  procedure DoSetRangeValue(AValue: Int64);
  var
    AEditValue: TcxEditValue;
  begin
    ActiveProperties.CalculateEditValueByRange(GetMinRange(AValue),
      GetMaxRange(AValue), AEditValue);
    FEditValueFormat := ActiveProperties.EditValueFormat;
...
function TdxCustomRangeTrackBar.GetActiveProperties
  : TdxCustomRangeTrackBarProperties;
begin
...
procedure TdxCustomRangeTrackBar.SetProperties(const Value
  : TdxCustomRangeTrackBarProperties);
begin
...
procedure TdxCustomRangeTrackBarProperties.CalculateEditValueByRange(AMin,
  AMax: Integer; out AValue: Variant);
begin
...
class function TdxCustomRangeTrackBarProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TdxRangeTrackBar;
end;

function TdxCustomRangeTrackBarProperties.GetSupportedOperations
...
procedure TdxCustomRangeTrackBarProperties.DoDrawRangeMinThumb(Sender: TObject;
...
procedure TdxCustomRangeTrackBarProperties.DoDrawRangeMaxThumb(Sender: TObject;
  ACanvas: TcxCanvas; const ARect: TRect);
begin
...
function TdxCustomRangeTrackBarProperties.EditValueToRange(const AEditValue
  : TcxEditValue): Int64;
begin
  Result := EditValueToRange(FEditValueFormat, FDelimiter, AEditValue);
end;

function TdxCustomRangeTrackBarProperties.EditValueToRange(AEditValueFormat
  : TdxRangeTrackBarEditValueFormat; ADelimiter: Char;
...
begin
  ...
  if not IsVarEmpty(AEditValue) then
    ...
    begin
      ...
      else if VarIsStr(AEditValue) and TryStrToInt64(AEditValue, ARange) then
        Result := ARange;
    end
    else
    begin
...
class function TdxCustomRangeTrackBarProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TdxRangeTrackBarViewData;
end;

class function TdxCustomRangeTrackBarProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TdxCustomRangeTrackBarViewInfo;
end;

procedure TdxCustomRangeTrackBarProperties.PrepareDisplayValue(const AEditValue
...
class function TdxRangeTrackBar.GetPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
...
procedure TdxRangeTrackBar.SetProperties(const Value
  : TdxRangeTrackBarProperties);
begin
  inherited Properties := Value;
end;
...
begin
  ...
  if Source is TdxCustomRangeTrackBarViewInfo then
    RightThumb.Position := (Source as TdxCustomRangeTrackBarViewInfo)
      .RightThumb.Position;
end;
...
begin
  if Assigned(RangeTrackBarProperties.OnGetPositionHint) then
    RangeTrackBarProperties.OnGetPositionHint(Edit, GetMinThumb.Position,
      GetMaxThumb.Position, AHintText, ACanShow, AIsHintMultiLine);
end;

procedure TdxCustomRangeTrackBarViewInfo.DrawCustomThumb(ACanvas: TcxCanvas);
begin
  (TrackBarProperties as TdxCustomRangeTrackBarProperties)
    .DoDrawRangeMinThumb(Self, ACanvas, GetRealRect(GetMinThumb.FullRect));
  (TrackBarProperties as TdxCustomRangeTrackBarProperties)
    .DoDrawRangeMaxThumb(Self, ACanvas, GetRealRect(GetMaxThumb.FullRect));
end;
...
procedure TdxCustomRangeTrackBarViewInfo.DrawRightThumb(ACanvas: TcxCanvas;
  const R: TRect; AState: Integer);
begin
...
begin
  if not FRightThumb.FadingHelper.DrawImage(ACanvas.Handle,
    GetRealRect(FRightThumb.FullRect)) then
    DrawRightThumb(ACanvas, GetRealRect(FRightThumb.FullRect),
      FRightThumb.State);
...
function TdxCustomRangeTrackBarViewInfo.IsTickText(ATickValue: Integer)
  : Boolean;
begin
  Result := inherited IsTickText(ATickValue) or
    (ATickValue = GetMaxThumb.Position) or (ATickValue = GetMinThumb.Position);
end;
...
function TdxCustomRangeTrackBarViewInfo.GetRangeTrackBarProperties
  : TdxCustomRangeTrackBarProperties;
begin
...
procedure TdxRangeTrackBarLeftThumbFadingHelper.DrawThumb(ACanvas: TcxCanvas;
  const R: TRect; AState: Integer);
begin
  (ViewInfo as TdxCustomRangeTrackBarViewInfo).DrawLeftThumb(ACanvas,
    R, AState);
end;
...
function TdxRangeTrackBarRightThumbFadingHelper.GetViewInfo
  : TdxCustomRangeTrackBarViewInfo;
begin
...
function TdxRangeTrackBarLeftThumbViewInfo.CreateFadingHelper
...
function TdxRangeTrackBarRightThumbViewInfo.CreateFadingHelper
  : TcxTrackBarThumbFadingHelper;
begin
...
begin
  ...
  if (Button = mbLeft) and PtInRect(ViewInfo.GetTrackBounds, Point(X, Y)) then
  begin
...
procedure TdxRangeTrackBarController.UpdatePos(AValue: Integer;
  AThumb: TcxTrackBarThumbViewInfo);
begin
...
begin
  ...
  begin
    ASize := AViewInfo.Painter.RangeTrackBarScaledLeftThumbSize
      (AViewInfo.Orientation = tboHorizontal,
      TrackBarTicksToTicksAlign[AViewInfo.TickMarks], ScaleFactor);
    ...
    AViewInfo.ThumbSize := ARangeTrackBarViewInfo.FLeftThumbWidth +
      ARangeTrackBarViewInfo.FRightThumbWidth - 1;
    AViewInfo.ThumbLargeSize := ASize.cy;
  end
  else
  begin
    ARangeTrackBarViewInfo.FLeftThumbWidth := ThumbHalfSize(AViewInfo) + 1;
    ARangeTrackBarViewInfo.FRightThumbWidth :=
      ARangeTrackBarViewInfo.FLeftThumbWidth;
  end;
end;
...
begin
  ...
  R.Right := ARangeViewInfo.GetCoordinate
    (ARangeViewInfo.LeftThumb.Position) + 1;
  ...
  ARangeViewInfo.LeftThumb.Bounds :=
    ARangeViewInfo.DrawingThumbRectToRealThumbRect(ACanvas, R);
  ...
  R.Top := ARangeViewInfo.TrackRect.Top +
    (ARangeViewInfo.TrackSize - ARangeViewInfo.ThumbLargeSize) div 2;
  R.Bottom := R.Top + ARangeViewInfo.ThumbLargeSize;
  ARangeViewInfo.RightThumb.Bounds :=
    ARangeViewInfo.DrawingThumbRectToRealThumbRect(ACanvas, R);
...
function TdxRangeTrackBarViewData.GetProperties
  : TdxCustomRangeTrackBarProperties;
begin
  Result := inherited Properties as TdxCustomRangeTrackBarProperties;
end;
...
GetRegisteredEditProperties.Register(TdxRangeTrackBarProperties,
