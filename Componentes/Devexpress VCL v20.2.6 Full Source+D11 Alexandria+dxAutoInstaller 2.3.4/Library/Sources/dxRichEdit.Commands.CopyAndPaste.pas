...
type
  ...
  public
    ...
    property PasteSource: IdxPasteSource read GetPasteSource
      write SetPasteSource;
  end;
  ...
  TdxPasteContentConvertedToDocumentModelCommandBase = class
    (TdxPasteContentCommandBase)
  private
  ...
  protected
    ...
    function CalculateImageScale(AImage: TdxOfficeImage;
      const AColumnBounds: TRect): Integer; virtual;
    function GetCurrentColumnBounds: TRect; overload; virtual;
    class function GetCurrentColumnBounds(ACaretPosition: TdxCaretPosition)
      : TRect; overload; virtual;
    function GetImageScale(const AImageSizeInLayoutUnits: TSize;
      const AColumnBounds: TRect): Integer; virtual;

    function InsertPicture(AImage: TdxOfficeImage): Boolean; overload; virtual;
    function InsertPicture(AImage: TdxOfficeImage; const AColumnBounds: TRect)
      : Boolean; overload; virtual;
    procedure InsertPictureCore(AImage: TdxOfficeImage;
  ...
  protected
    ...
    function IsDataAvailable: Boolean; override;
    procedure ModifyModel; override;
    procedure PasteImageFromFile(const AFileName: string;
      const AColumnBounds: TRect);
  public
    class function GetDescription: string; override;
    class function GetMenuCaption: string; override;
  end;
  ...
  public
    constructor Create(const ARichEditControl: IdxRichEditControl;
      const APasteSource: IdxPasteSource); reintroduce;
    class function GetDescription: string; override;
    class function GetMenuCaption: string; override;
  ...
  public
    constructor Create(const ARichEditControl: IdxRichEditControl;
      const ADataObject: IdxDataObject); reintroduce;
    destructor Destroy; override;
  ...
  TdxPasteHtmlTextFromDragDropCommand = class
    (TdxPasteContentConvertedToDocumentModelCommandBase)
  protected
    function CreateInnerCommandCore
      : TdxPieceTablePasteContentConvertedToDocumentModelCommandBase; override;
  ...
  public
    ...
    property SuppressStoreImageSizeCollection: string
      read FSuppressStoreImageSizeCollection;
  end;
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      ACopyCommand: TdxCopyAndSaveContentCommand); reintroduce;
    class function GetDescription: string; override;
    class function GetMenuCaption: string; override;
  end;
  ...
  TdxPieceTablePasteSavedContentCommand = class
  ...
  private
    ...
    procedure SetDefaultPropertiesCopyOptions(const Value
      : TdxDefaultPropertiesCopyOptions);
    procedure SetFixLastParagraph(const Value: Boolean);
  protected
    function CreateCopySelectionManager: TdxCopySelectionManager; virtual;
    procedure UpdateUIStateCore(const AState: IdxCommandUIState); override;

    property CopySelectionManager: TdxCopySelectionManager
      read FCopySelectionManager;
    property DefaultPropertiesCopyOptions: TdxDefaultPropertiesCopyOptions
      read GetDefaultPropertiesCopyOptions
      write SetDefaultPropertiesCopyOptions;
  public
    ...
    property FixLastParagraph: Boolean read GetFixLastParagraph
      write SetFixLastParagraph;
  end;
...
constructor TdxPasteContentCommandBase.Create(const AControl
  : IdxRichEditControl);
begin
...
function TdxPasteContentConvertedToDocumentModelCommandBase.GetFormat
  : TdxRichEditDocumentFormat;
begin
  Result := InnerCommand.Format;
end;

function TdxPasteContentConvertedToDocumentModelCommandBase.GetPasteSource
  : IdxPasteSource;
begin
  Result := InnerCommand.PasteSource;
end;

function TdxPasteContentConvertedToDocumentModelCommandBase.
...
function TdxPasteRtfTextCommand.CreateInnerCommandCore
  : TdxPieceTablePasteContentConvertedToDocumentModelCommandBase;
begin
...
begin
  ...
  ApplyCommandRestrictionOnEditableControl(AState, Options.Behavior.Cut,
    AState.Enabled);
end;

function TdxCutSelectionCommand.GetUpdateUIStateMode
...
constructor TdxPasteDataObjectCoreCommand.Create(const ARichEditControl
  : IdxRichEditControl; const ADataObject: IdxDataObject);
var
...
begin
  ...
  AColumnBounds := TdxPasteImageCommand.GetCurrentColumnBounds
    (ActiveView.CaretPosition);
...
procedure TdxPasteImagesFromFilesCommand.PasteImageFromFile(const AFileName
  : string; const AColumnBounds: TRect);
var
...
begin
  ...
  ApplyCommandRestrictionOnEditableControl(AState,
    DocumentModel.DocumentCapabilities.InlinePictures, AState.Enabled);
  ApplyDocumentProtectionToSelectedCharacters(AState);
end;

{ TdxPasteLoadDocumentFromFileCommand }

function TdxPasteLoadDocumentFromFileCommand.CanLoadFile(const AFileName
  : string): Boolean;
begin
  Result := DocumentModel.AutodetectDocumentFormat(AFileName, False) <>
...
function TdxPasteLoadDocumentFromFileCommand.GetFormat
  : TdxRichEditDocumentFormat;
begin
  Result := TdxRichEditDocumentFormat.Undefined;
end;
...
function TdxPasteHtmlTextFromDragDropCommand.CreateInnerCommandCore
  : TdxPieceTablePasteContentConvertedToDocumentModelCommandBase;
begin
  Assert(False);
  Result := nil;
...
constructor TdxPasteSavedContentCommand.Create(const AControl
  : IdxRichEditControl; ACopyCommand: TdxCopyAndSaveContentCommand);
begin
  FCopyCommand := ACopyCommand;
  inherited Create(AControl);
end;

function TdxPasteSavedContentCommand.CreateInnerCommandCore
  : TdxPieceTablePasteContentConvertedToDocumentModelCommandBase;
begin
  if FCopyCommand = nil then
    Result := nil
  else
    Result := TdxPieceTablePasteSavedContentCommand.Create(ActivePieceTable,
      FCopyCommand);
end;
...
function TdxPieceTablePasteSavedContentCommand.
  GetAdditionalContentString: string;
begin
  Result := FCopyCommand.SuppressStoreImageSizeCollection;
end;

function TdxPieceTablePasteSavedContentCommand.GetContent
...
begin
  ...
  try
    ...
    try
      ...
      FRtfText := AManager.GetRtfText(ASelection.PieceTable, ASelections,
        AOptions, True, True);
      FSuppressStoreImageSizeCollection :=
        AManager.GetSuppressStoreImageSizeCollection(ASelection.PieceTable,
        ASelections);
    finally
      AOptions.Free;
    end;
...
function TdxPasteImageCommand.CalculateImageScale(AImage: TdxOfficeImage;
  const AColumnBounds: TRect): Integer;
var
  ADpiX, ADpiY: Single;
  AUnitConverter: TdxDocumentLayoutUnitConverter;
  AImageWidthInLayoutUnits, AImageHeightInLayoutUnits: Integer;
begin
  ADpiX := IfThen(AImage.RawFormat = TdxOfficeImageFormat.Emf,
    TdxMetafileHelper.MetafileResolution, AImage.HorizontalResolution);
  ADpiY := IfThen(AImage.RawFormat = TdxOfficeImageFormat.Emf,
    TdxMetafileHelper.MetafileResolution, AImage.VerticalResolution);

  AUnitConverter := DocumentModel.LayoutUnitConverter;
  AImageWidthInLayoutUnits := AUnitConverter.PixelsToLayoutUnits
    (AImage.Width, ADpiX);
  AImageHeightInLayoutUnits := AUnitConverter.PixelsToLayoutUnits
    (AImage.Height, ADpiY);

  Result := GetImageScale(cxSize(AImageWidthInLayoutUnits,
    AImageHeightInLayoutUnits), AColumnBounds);
end;
...
class function TdxPasteImageCommand.GetCurrentColumnBounds(ACaretPosition
  : TdxCaretPosition): TRect;
begin
...
function TdxPasteImageCommand.GetImageScale(const AImageSizeInLayoutUnits
  : TSize; const AColumnBounds: TRect): Integer;
var
...
begin
  Result := False;
  if (AImage = nil) or not DocumentModel.DocumentCapabilities.InlinePicturesAllowed
  then
    Exit;
...
begin
  ...
  try
    ActivePieceTable.InsertInlinePicture(DocumentModel.Selection.&End,
      AImageReference, AScaleX, AScaleY, GetForceVisible);
  finally
...
class function TdxPasteSelectionCommand.GetInsertObjectCommandClass
...
procedure TdxPasteSelectionCommand.SetFormat(const Value
  : TdxRichEditDocumentFormat);
var
  ACommand: TdxPasteSelectionCoreCommand;
begin
...
procedure TdxPasteSelectionCommand.UpdateUIState(const AState
  : IdxCommandUIState);
var
...
begin
  ...
  begin
    ApplyCommandRestrictionOnEditableControl(AState, Options.Behavior.Paste,
      ACommandState.Enabled);
    ApplyDocumentProtectionToSelectedCharacters(AState);
  end
  else
  begin
...
procedure TdxPasteSelectionCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
...
constructor TdxPasteSelectionCoreCommand.Create(const ARichEditControl
  : IdxRichEditControl; const APasteSource: IdxPasteSource);
begin
  FPasteSource := APasteSource;
...
procedure TdxPasteSelectionCoreCommand.AddCommand(ACommandClass
  : TdxPasteContentCommandBaseClass);
var
...
function TdxPasteSelectionCoreCommand.CanAddCommandFormat
  (ACommand: TdxPasteContentCommandBase): Boolean;
begin
  Result := (Format = TdxRichEditDocumentFormat.Undefined) or
    (Format = ACommand.Format);
end;

function TdxPasteSelectionCoreCommand.ContainsData(const AFormat
  : string): Boolean;
begin
...
function TdxPasteSelectionCoreCommand.GetExecutionMode
...
procedure TdxPasteSelectionCoreCommand.SetFormat(const Value
  : TdxRichEditDocumentFormat);
begin
...
function TdxPasteSelectionCoreCommand.GetUpdateUIStateMode
  : TdxMultiCommandUpdateUIStateMode;
begin
...
constructor TdxCopySelectionCommand.Create(const ARichEditControl
  : IdxRichEditControl);
begin
  inherited Create(ARichEditControl);
...
function TdxCopySelectionCommand.CreateCopySelectionManager
...
function TdxCopySelectionCommand.GetDefaultPropertiesCopyOptions
  : TdxDefaultPropertiesCopyOptions;
begin
...
procedure TdxCopySelectionCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  AState.Checked := False;
  ApplyCommandsRestriction(AState, Options.Behavior.Copy,
