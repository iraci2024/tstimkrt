...
type
  ...
  public
    ...
    property OnFreeNotification: TcxFreeNotificationEvent
  ...
  TdxListSortCompareDelegate = reference to function(Item1,
  ...
  public
    ...
    function Extract(AItem: Pointer;
      ADirection: TdxListDirection = ldFromBeginning): Pointer;
    ...
    procedure Sort(ACompare: TListSortCompare;
      AMultiThreaded: Boolean = False); overload;
    procedure SortList(const ACompare: TdxListSortCompareDelegate;
  ...
  public
    ...
    function IndexOf(AObject: TObject;
      ADirection: TdxListDirection = ldFromBeginning): Integer; inline;
...
{$ENDIF CPUX64}
  ...
  public
    constructor Create(const ADynamicInvokeEvent: TDynamicInvokeEvent;
      ATypeData: PTypeData);
    class function CreateMethodPointer(const ADynamicInvokeEvent
      : TDynamicInvokeEvent; ATypeData: PTypeData): TMethod; static;
    class function GetMethodCallingConvention(ATypeData: PTypeData): TCallConv;
      static; inline;
    class function GetMethodParametersTypeInfo(ATypeData: PTypeData): Pointer;
      static; inline;
  end;
  ...
  strict private
    ...
    class procedure InvokeMethod(const AMethod: TMethod; AParams: Pointer;
      AStackSize: Integer); static;
  ...
  TdxMulticastImplementator<T> = class(TdxMulticastImplementator,
    IdxMulticastMethod<T>)
  strict private
    FInvoke: T;
    function GetInvoke: T;
  ...
  public
    ...
    class operator GreaterThan(const A: TdxNullableValue<T>;
      const B: T): Boolean;
    class operator LessThan(const A: TdxNullableValue<T>; const B: T): Boolean;
    class operator LogicalAnd(const A, B: TdxNullableValue<T>)
      : TdxNullableValue<T>;
    class operator LogicalNot(const A: TdxNullableValue<T>)
      : TdxNullableValue<T>;
    class operator LogicalOr(const A, B: TdxNullableValue<T>)
      : TdxNullableValue<T>;
    ...
    class function IfThen(AValue: Boolean;
      const ATrue, AFalse: TdxNullableValue<T>): TdxNullableValue<T>;
      overload; static;
    class function IfThen(AValue: Boolean; const ATrue: TdxNullableValue<T>)
      : TdxNullableValue<T>; overload; static;
    class function IfThen(AValue: Boolean; const ATrue, AFalse: T)
      : TdxNullableValue<T>; overload; static;
    class function IfThen(AValue: Boolean; const ATrue: T): TdxNullableValue<T>;
      overload; static;
...
{$ENDIF}
  ...
  public
    constructor Create(const AFileName: string;
      AMode: Word = fmOpenRead or fmShareDenyWrite); overload;
    ...
    constructor Create(const AContent: TArray<Byte>;
      AOffset, ACount: Integer); overload;
    procedure LoadFromFile(const AFileName: string); reintroduce; overload;
    procedure LoadFromFile(const AFileName: string; AMode: Word);
...
procedure InsertionSort(ASortList: PdxPointerList; ALast: Integer;
  const ACompareFunc: TdxListSortCompareDelegate); overload;
var
  I, J: Integer;
  ATemp: Pointer;
begin;
...
procedure ModifiedQuickSort(ASortList: PdxPointerList; ARight: Integer;
  const ACompareFunc: TdxListSortCompareDelegate); overload;
var
  I, J: Integer;
...
procedure dxQuickSortList(AList: TList; ACompareFunc: TCompareItems;
  AMultiThreaded: Boolean = False);
begin;
  dxQuickSortList(Pointer(AList.List), AList.Count, ACompareFunc,
    AMultiThreaded);
end;
...
{$IFDEF MSWINDOWS}
    if AMultiThreaded and dxCanUseMultiThreading and
      (ACount > dxMaxUserWorkItems) then
...
function TcxOwnedInterfacedPersistent.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
    Result := 0
...
class procedure TcxFreeNotificator.Check(var AFreeNotificator
  : TcxFreeNotificator);
begin
  if (AFreeNotificator <> nil) and not AFreeNotificator.IsFreeNotifiesExists
  then
...
procedure TcxFreeNotificator.Notification(AComponent: TComponent;
...
begin
  ...
  begin
    if (Count = 0) and (TCollection(Source).Count = 0) then
      Exit;
    BeginUpdate;
    try
...
function TcxDoublyLinkedObjectList.GetLinkedObjectClass
...
procedure TcxDoublyLinkedObjectList.Remove(ALinkedObject
  : TcxDoublyLinkedObject);
begin
  Delete(ALinkedObject);
end;

procedure TcxDoublyLinkedObjectList.Delete(ALinkedObject
  : TcxDoublyLinkedObject);
begin
...
procedure TcxDoublyLinkedObjectList.Extract(ALinkedObject
  : TcxDoublyLinkedObject);
...
procedure TcxDoublyLinkedObjectList.InsertBefore(ADestination,
  ASource: TcxDoublyLinkedObject);
begin
...
procedure TcxDoublyLinkedObjectList.InsertAfter(ADestination,
  ASource: TcxDoublyLinkedObject);
begin
...
function TcxDoublyLinkedDataList.GetLinkedObjectClass
  : TcxDoublyLinkedObjectClass;
begin
  ...
  begin
    ...
    begin
      if AItem.Prev <> nil then
        ACompare := ACompareProc(TcxDoublyLinkedData(AItem.Prev).Data,
          AItem.Data)
      else
        ACompare := 0;

      if (AItem.Prev <> nil) and ((ADirection < 0) and (ACompare < 0) or
        (ADirection > 0) and (ACompare > 0) or (ACompare = 0) or
        (ADirection = 0)) then
      begin
    ...
    begin
      ...
      repeat
        ...
        repeat
          AItem := Glue(AItem);
        until AItem = nil;
        if (AFirst.FNextSorted <> nil) and (AFirst.FNextSorted.FPrevSorted = nil)
        then
...
function TcxDoublyLinkedDataIndexedList.Add(AData: Pointer; AIndex: Integer)
...
function TcxDoublyLinkedDataIndexedList.GetLinkedObjectClass
  : TcxDoublyLinkedObjectClass;
begin
...
begin
  ...
  if AIndex < Count then
    System.Move(List[AIndex], List[AIndex + AList.Count],
      (Count - AIndex) * SizeOf(Pointer));
...
procedure TdxFastList.Sort(ACompare: TCompareItems;
  AMultiThreaded: Boolean = False);
begin
  dxQuickSortList(List, Count, ACompare, AMultiThreaded);
end;

procedure TdxFastList.Sort(ACompare: TListSortCompare;
  AMultiThreaded: Boolean = False);
begin
  dxQuickSortList(List, Count, ACompare, AMultiThreaded);
end;

procedure TdxFastList.SortList(const ACompare: TdxListSortCompareDelegate;
  AMultiThreaded: Boolean = False);
begin
  if Count > 1 then
{$IFDEF MSWINDOWS}
    if AMultiThreaded and dxCanUseMultiThreading and (Count > dxMaxUserWorkItems)
    then
...
function TdxFastList.Extract(AItem: Pointer;
  ADirection: TdxListDirection = ldFromBeginning): Pointer;
var
  I: Integer;
begin
...
constructor TdxFastObjectList.Create(AOwnsObjects: Boolean = True;
  ACapacity: Integer = 0);
begin
...
begin
  ...
  if AValue > Count then
    FillChar(List[Count], (AValue - Count) * SizeOf(Pointer), 0)
  else if (AValue < Count) and OwnsObjects then
...
constructor TdxEventHandlerInstance.Create(const ADynamicInvokeEvent
  : TDynamicInvokeEvent; ATypeData: PTypeData);
{$IFNDEF CPUX64}
var
  P: PByte;
  ACurReg: Integer;
...
begin
  ...
  begin
    if TypInfo.TParamFlags(P[0]) * [pfVar, pfConst, pfAddress, pfReference,
      pfOut] <> [] then
      Size := 4
    else
  ...
  begin
    ...
    begin
      if (System.TypInfo.TParamFlags(P[0]) * [pfVar, pfConst, pfAddress,
        pfReference, pfOut] = []) and (FParamInfos^[I]^.Kind = tkFloat) then
...
class function TdxEventHandlerInstance.CreateMethodPointer
  (const ADynamicInvokeEvent: TDynamicInvokeEvent;
  ATypeData: PTypeData): TMethod;
begin
  TObject(Result.Data) := TdxEventHandlerInstance.Create(ADynamicInvokeEvent,
    ATypeData);
  Result.Code := @TdxEventHandlerInstance.RegisterStub;
end;

class function TdxEventHandlerInstance.GetMethodAdditionalInfo
  (ATypeData: PTypeData): Pointer;
var
  P: PByte;
  I: Integer;
begin
...
class function TdxEventHandlerInstance.GetMethodCallingConvention
  (ATypeData: PTypeData): TCallConv;
begin
  Result := TCallConv(PByte(GetMethodAdditionalInfo(ATypeData))^);
end;

class function TdxEventHandlerInstance.GetMethodParametersTypeInfo
  (ATypeData: PTypeData): Pointer;
var
  P: PByte;
begin
      ...
      begin
        ...
        if ATypeData^.MinValue >= 0 then
          if Cardinal(ATypeData^.MaxValue) > $FFFF then
            ...
            Result := 1
        else if (ATypeData^.MaxValue > $7FFF) or
          (ATypeData^.MinValue < -$7FFF - 1) then
          Result := 4
        else if (ATypeData^.MaxValue > $7F) or (ATypeData^.MinValue < -$7F - 1)
        then
          Result := 2
        else
          Result := 1;
      ...
      begin
        ...
        case ATypeData^.FloatType of
          ...
          ftComp, ftCurr, ftDouble:
            Result := 8;
          ftExtended:
            Result := (SizeOf(Extended) + SizeOf(Pointer) - 1) and
              not(SizeOf(Pointer) - 1);
        else
          Result := -1;
        end;
      end;
...
procedure TdxMulticastImplementator.InternalSetDispatcher(var ADispatcher;
  ATypeData: PTypeData);
var
  AMethod: TMethod absolute ADispatcher;
begin
  if Assigned(FInternalDispatcher.Code) and Assigned(FInternalDispatcher.Data)
  then
    TObject(FInternalDispatcher.Data).Free;
  TMethod(FInternalDispatcher) := TdxEventHandlerInstance.CreateMethodPointer
    (InternalInvoke, ATypeData);
    ...
    for I := Low(FHandlers) to High(FHandlers) do
      ...
      begin
        if I <> High(FHandlers) then
          System.Move(FHandlers[I + 1], FHandlers[I], (High(FHandlers) - I) *
            SizeOf(TMethod));
        SetLength(FHandlers, Length(FHandlers) - 1);
        Break;
      end;
...
class procedure TdxMulticastImplementator.InvokeMethod(const AMethod: TMethod;
  AParams: Pointer; AStackSize: Integer);
const
...
procedure TdxMulticastImplementator.InternalInvoke(AParams: PParameters;
  AStackSize: Integer);
var
  I: Integer;
begin
...
procedure TdxMulticastImplementator<T>.SetEventDispatcher(var ADispatcher: T;
  ATypeData: PTypeData);
begin
...
class operator TdxNullableValue<T>.Equal(const A,
  B: TdxNullableValue<T>): Boolean;
begin
  Result := (A.IsNull and B.IsNull) or
    (not(A.IsNull or B.IsNull) and (TComparer<T>.Default.Compare(A.Value,
    B.Value) = 0));
end;

class function TdxNullableValue<T>.IfThen(AValue: Boolean;
  const ATrue, AFalse: T): TdxNullableValue<T>;
begin
  if AValue then
...
class function TdxNullableValue<T>.IfThen(AValue: Boolean; const ATrue: T)
  : TdxNullableValue<T>;
begin
  if AValue then
    Result := TdxNullableValue<T>.Create(True, ATrue)
...
class function TdxNullableValue<T>.IfThen(AValue: Boolean;
  const ATrue: TdxNullableValue<T>): TdxNullableValue<T>;
begin
  if AValue then
    Result := ATrue
  else
...
class operator TdxNullableValue<T>.LogicalAnd(const A, B: TdxNullableValue<T>)
  : TdxNullableValue<T>;
var
...
class operator TdxNullableValue<T>.LogicalNot(const A: TdxNullableValue<T>)
  : TdxNullableValue<T>;
var
  AValue: TValue;
begin
...
class operator TdxNullableValue<T>.LogicalOr(const A, B: TdxNullableValue<T>)
  : TdxNullableValue<T>;
var
  AValueA, AValueB: TValue;
begin
...
class operator TdxNullableValue<T>.NotEqual(const A: TdxNullableValue<T>;
  const B: T): Boolean;
begin
  Result := A.IsNull or (TComparer<T>.Default.Compare(A.Value, B) <> 0);
end;

class operator TdxNullableValue<T>.Equal(const A: TdxNullableValue<T>;
  const B: T): Boolean;
begin
  if A.IsNull then
    Result := False
...
class operator TdxNullableValue<T>.GreaterThan(const A: TdxNullableValue<T>;
  const B: T): Boolean;
begin
  if A.IsNull then
    Result := False
...
class operator TdxNullableValue<T>.LessThan(const A: TdxNullableValue<T>;
  const B: T): Boolean;
begin
  if A.IsNull then
    Result := False
...
class operator TdxNullableValue<T>.NotEqual(const A,
  B: TdxNullableValue<T>): Boolean;
begin
...
function TdxNullableValue<T>.IsEqual(const AValue: TdxNullableValue<T>)
  : Boolean;
begin
  Result := (Self.IsNull and AValue.IsNull) or
    (not(Self.IsNull or AValue.IsNull) and
    (TComparer<T>.Default.Compare(Self.Value, AValue.Value) = 0));
end;
{$ENDIF}
{ TdxIStringComparer }
...
begin
  ...
  ALength := Length(L);
  Result := (ALength - Length(R) = 0) and CompareMem(PChar(L), PChar(R),
    ALength * SizeOf(Char));
end;
...
begin
  ALength := Length(ALeft);
  Result := (ALength - Length(ARight) = 0) and
    CompareMem(PChar(ALeft), PChar(ARight), ALength * SizeOf(Char));
end;
...
constructor TdxMemoryStream.Create(const AFileName: string;
  AMode: Word = fmOpenRead or fmShareDenyWrite);
begin
  inherited Create;
