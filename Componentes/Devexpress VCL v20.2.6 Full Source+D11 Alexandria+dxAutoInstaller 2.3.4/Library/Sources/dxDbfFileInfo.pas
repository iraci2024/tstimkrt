...
const
  ...
  SupportedDbfFileTypes: array [0 .. SupportedDbfFileTypesCount - 1]
    of Byte = (dbftFoxBase, dbftWithoutDBT, dbftVisualFoxPro, dbftFlagship,
    dbftWithDBT, dbftDBase4, dbftFoxProWithMemo, dbftFoxPro);
...
const
  ...
  LanguageDriver: array [0 .. LanguageDriverCount - 1] of Byte = (1, 2, 3, 4,
    100, 101, 102, 103, 104, 105, 106, 107, 150, 151, 152, 200, 201, 202, 203);
  CodePages: array [0 .. LanguageDriverCount - 1] of Integer = (cp437, cp850,
    windows_1252, macintosh, cp852, cp865, cp866, cp861, cp852, cp437, ibm737,
    cp857, x_mac_cyrillic, x_mac_ce, x_mac_greek, windows_1250, windows_1251,
    windows_1254, windows_1253);
...
begin
  ...
  try
    ...
    AStream.Seek(SizeOf(UInt16) + SizeOf(Byte) + SizeOf(Byte) + SizeOf(UInt32) +
      SizeOf(Byte) * 8 + SizeOf(Byte), soCurrent);
...
begin
  ...
  begin
    ...
    ACorrectFieldValueByType := CorrectFieldValueByType(AData,
      AField.FieldType);
...
procedure TdxDbfFileField.LoadFromStream(AStream: TStream;
  AEncoding: TEncoding);
var
  ABytes: TBytes;
begin
...
procedure TdxDbfFileRecord.AddField(const AField: TdxDbfFileField;
  const AValue: string);
begin
