...
type
  ...
  public
    constructor Create(AManager: TdxWebFileTransferManager;
      AAgent: TdxCustomAuthorizationAgent; const AProcessID, AUri: string);
      reintroduce; virtual;
  ...
  protected
    ...
    function Upload(ARequest: TdxInternet; const AHeader: string;
      const ACancelStatus: TdxTaskCancelCallback)
      : TdxTaskCompletedStatus; virtual;
    function WriteStream(ARequest: TdxInternet;
      const ACancelStatus: TdxTaskCancelCallback)
      : TdxTaskCompletedStatus; virtual;
  ...
  protected
    procedure DoDownload(const AProcessID: string;
      const APosition: Cardinal); virtual;
    procedure DoDownloaded(const AProcessID: string;
      const AStream: TStream); virtual;
...
begin
  ...
  Result := Result and (Agent = Agent) and
    (CompareText(Uri, TdxWebFileTransferTask(ATask).Uri) = 0);
end;
...
function TdxWebFileDownloadTask.DoRun(const ACancelStatus
...
procedure TdxWebFileDownloadStreamTask.WriteBuffer(const ABuffer;
ACount: Integer);
begin
...
constructor TdxWebFileUploadTask.Create(AManager: TdxWebFileTransferManager;
AAgent: TdxCustomAuthorizationAgent; const AProcessID, AUri: string;
AStream: TStream);
begin
...
function TdxWebFileUploadTask.DoRun(const ACancelStatus: TdxTaskCancelCallback)
  ...
  if TdxHttpHelper.OpenSession(UserAgent, ASession) then
    try
      if TdxHttpHelper.OpenConnection(ASession, ServerName, AConnection) then
        try
          if TdxHttpHelper.OpenRequest(AConnection, Verb, ObjectName, ARequest)
          then
            try
              DoStartUpload;
              if ACancelStatus then
...
const AHeader: string; const ACancelStatus: TdxTaskCancelCallback)
  : TdxTaskCompletedStatus;
var
...
begin
  ...
  if ACancelStatus then
    Exit(TdxTaskCompletedStatus.Cancelled);
  if not HttpAddRequestHeaders(ARequest, PChar(AHeader), Length(AHeader),
...
function TdxWebFileUploadTask.WriteStream(ARequest: TdxInternet;
...
begin
  ...
  begin
    ...
    if not InternetWriteFile(ARequest, @Stream.Bytes[Position], ASize, ACount)
    then
      Exit;
...
procedure TdxWebFileTransferManager.DoDownloading(const AProcessID: string;
const ASize: Integer);
begin
  // do nothing
end;

procedure TdxWebFileTransferManager.DoUpload(const AProcessID: string;
const APosition: Cardinal);
begin
  // do nothing
end;

procedure TdxWebFileTransferManager.DoUploaded(const AProcessID,
  AFileID: string);
begin
  // do nothing
end;

procedure TdxWebFileTransferManager.DoUploading(const AProcessID: string;
const ASize: Integer);
begin
  // do nothing
end;
...
begin
  FTaskManager.ForEach(procedure(ATask: TdxWebTask)
    begin
      if CompareText(TdxWebFileTransferTask(ATask).ProcessID, AProcessID) = 0
      then
        FTaskManager.CancelTask(ATask);
    end);
end;

procedure TdxWebFileTransferManager.RunDownloadFileTask
  (const ATask: TdxWebFileDownloadTask);
begin
  FTaskManager.RunTask(ATask);
end;

procedure TdxWebFileTransferManager.RunUploadFileTask
  (const ATask: TdxWebFileUploadTask);
begin
  FTaskManager.RunTask(ATask);
end;
