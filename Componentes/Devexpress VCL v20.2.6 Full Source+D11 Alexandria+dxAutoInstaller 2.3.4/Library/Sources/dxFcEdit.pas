...
type
  ...
  TChart = class(TdxFlowChart)
  end;

  TConnection = class(TdxFcConnection)
  end;
  ...
  public
    ...
    constructor Create(AChart: TdxFlowChart; AStepCount: Integer);
      overload; virtual;
  ...
  private
    ...
    procedure LocalizeIndexesMenu(AMenu: TPopupMenu;
...
uses
  RTLConsts, dxEditObj, dxEditCon, dxSelUnion, dxCore, dxFcStrs, Math,
...
begin
  ...
  AHasChanged := (Count = 0) or (AChartStream.Size <> Items[ItemIndex].Size) or
    not CompareMem(AChartStream.Memory, Items[ItemIndex].Memory,
    AChartStream.Size);
  if AHasChanged then
  begin
...
procedure TFChartEditor.ChartMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  hTest: TdxFcHitTest;
  R: TRect;
...
begin
  ...
  ASB := nil;
  if APopupMenu = ShapePopupMenu then
    ASB := sbShape;
  if APopupMenu = LinePopupMenu then
    ASB := sbLine;
  if APopupMenu = StylePopupMenu then
    ASB := sbStyle;
  if APopupMenu = SourceArrowPopupMenu then
    ASB := sbSourceArrow;
  if APopupMenu = DestArrowPopupMenu then
    ASB := sbDestArrow;
  if APopupMenu = SArrowSizePopupMenu then
    ASB := sbSArrowSize;
  if APopupMenu = DArrowSizePopupMenu then
    ASB := sbDArrowSize;
  if APopupMenu = SPointPopupMenu then
    ASB := sbSPoint;
  if APopupMenu = DPointPopupMenu then
    ASB := sbDPoint;
  if APopupMenu = TextPositionPopupMenu then
    ASB := sbTextPosition;
  if APopupMenu = ImagePositionPopupMenu then
    ASB := sbImagePosition;
...
procedure TFChartEditor.pBlackMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  Changing;
  try
...
function TFChartEditor.IsChildItemInUnion(AChart: TdxFlowChart;
...
function TFChartEditor.GetMainItemInUnion(AChart: TdxFlowChart;
...
function TFChartEditor.GetUnionByNumber(AChart: TdxFlowChart; Number: Integer)
  : TdxFcObject;
var
  I, Count: Integer;
begin
...
begin
  for I := 0 to ComponentCount - 1 do
    if Components[I] is TMenuItem then
      TMenuItem(Components[I]).ImageIndex :=
        GetImageIndexByMenuItem(TMenuItem(Components[I]));
end;
...
begin
  ...
  LocalizeIndexesMenu(LinePopupMenu,
    cxGetResourceString(@sdxFlowChartEditorPixels));
  LocalizeIndexesMenu(SPointPopupMenu,
    cxGetResourceString(@sdxFlowChartEditorPoint));
  LocalizeIndexesMenu(DPointPopupMenu,
    cxGetResourceString(@sdxFlowChartEditorPoint));
...
procedure TFChartEditor.LocalizeIndexesMenu(AMenu: TPopupMenu;
  const AFormatString: string);
var
  AItem: TMenuItem;
  I: Integer;
