...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Types, Windows, Messages, SysUtils, Classes, Graphics, Generics.Defaults,
  Generics.Collections, Controls,
  StdCtrls, Forms, ActiveX, dxCoreClasses, cxGeometry, cxLookAndFeels,
  cxGraphics, cxControls, cxClasses,
...
type
  ...
  protected
    ...
    procedure RenderDocument(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); virtual;
    procedure RenderDocumentDecorators(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); virtual;
    procedure DrawCaretCore(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); overload; virtual;
    procedure DrawDragCaretCore(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); virtual;
    procedure DrawCaretCore(ACaret: TdxCaret; AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); overload; virtual;
    function GetActualBounds(AGraphics: TdxGraphics; APage: TdxPageViewInfo;
      const ABounds: TRect): TRect; virtual;
    procedure DrawReversibleFrameCore(AGraphics: TdxGraphics;
      const ABounds: TRect; APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleHorizontalLineCore(AGraphics: TdxGraphics;
      Y: Integer; APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleVerticalLineCore(AGraphics: TdxGraphics; X: Integer;
  ...
  public
    ...
    procedure DeferredDraw(APage: TdxPageViewInfo;
      ADrawAtPage: TdxDrawAtPageDelegate);
    procedure DeferredDrawReversibleFrame(const ABounds: TRect;
      APage: TdxPageViewInfo);
    procedure DrawReversibleHorizontalLine(Y: Integer;
      APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleVerticalLine(X: Integer;
      APage: TdxPageViewInfo); virtual;
    procedure DrawReversibleCore(APage: TdxPageViewInfo;
      const ADraw: TdxDrawDelegate); virtual;
    procedure PerformRenderingInUnits(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter;
      const ADraw: TdxDrawDelegate); virtual;
    procedure PerformRenderingInPixels(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter;
      const ADraw: TdxDrawDelegate); virtual;
  ...
  protected
    ...
    function CreateNativeSubDocument(APieceTable: TdxPieceTable)
      : IdxRichEditSubDocument; override;
  ...
  TdxRichEditControlBase = class abstract(TdxVCLControl, IdxBatchUpdatable,
    IDropTarget, IDropSource, IdxInnerRichEditDocumentServerOwner,
    IdxInnerRichEditControlOwner, IdxRichEditControl,
  ...
  protected
    ...
    function OleDragEnter(const dataObj: IDataObject; grfKeyState: Longint;
      pt: TPoint; var dwEffect: Longint): HResult; stdcall;
    function IDropTarget.DragOver = OleDragOver;
    function OleDragOver(grfKeyState: Longint; pt: TPoint;
      var dwEffect: Longint): HResult; stdcall;
    function IDropTarget.DragLeave = OleDragLeave;
    function OleDragLeave: HResult; stdcall;
    function IDropTarget.Drop = OleDrop;
    function OleDrop(const dataObj: IDataObject; grfKeyState: Longint;
      pt: TPoint; var dwEffect: Longint): HResult; stdcall;

    function IDropSource.QueryContinueDrag = OleQueryContinueDrag;
    function OleQueryContinueDrag(fEscapePressed: BOOL; grfKeyState: Longint)
      : HResult; stdcall;
    ...
    function CreateDocumentServer(ADocumentModel: TdxDocumentModel)
      : IdxRichEditDocumentContainer; overload; virtual; abstract;
    function CreateMeasurementAndDrawingStrategy(ADocumentModel
      : TdxDocumentModel): TdxMeasurementAndDrawingStrategy; virtual;
    function GetLookAndFeel: TcxLookAndFeel;
    function CreateOptions(const ADocumentServer
      : TObject { TdxInnerRichEditDocumentServer } )
      : TObject { TdxRichEditControlOptionsBase }; virtual; abstract;
    ...
    procedure OnResizeCore(AEnsureCaretVisibleOnResize,
      AEnsureOptimalHorizontalScrollbarPosition: Boolean); overload;
    procedure OnZoomFactorChangingPlatformSpecific;
    procedure IdxInnerRichEditControlOwner.OnActiveViewBackColorChanged =
      OnBackColorChanged;
    ...
    procedure ShowColumnsSetupForm(const AProperties: TdxColumnsInfoUI;
      const ACallback: TdxShowColumnsSetupFormCallback;
    ...
    procedure ShowInsertTableForm(const AParameters: TdxCreateTableParameters;
      const ACallback: TdxShowInsertTableFormCallback;
      ACallbackData: TObject); virtual;
    ...
    procedure ShowDocumentEncryptQueryNewPasswordForm(const APassword: string;
      const ACallback: TdxPasswordFormCallback); virtual;
    procedure ShowDocumentProtectionQueryNewPasswordForm(const APassword
      : string; const ACallback: TdxPasswordFormCallback); virtual;
    procedure ShowDocumentProtectionQueryPasswordForm(const APassword: string;
      const ACallback: TdxPasswordFormCallback); virtual;
    ...
    procedure ShowHyperlinkForm(AHyperlinkInfo: TdxHyperlinkInfo;
      ...
      const ACallback: TdxShowHyperlinkFormCallback); virtual;
    procedure ShowSymbolForm(const ASymbolProperties: TdxSymbolProperties;
      const ACallback: TdxShowSymbolFormCallback;
      ACallbackData: TObject); virtual;
    ...
    procedure ShowParagraphForm(AParagraphProperties
      ...
      const ACallback: TdxShowParagraphFormCallback;
      ACallbackData: TObject); virtual;
    procedure ShowFontForm(ACharacterProperties: TdxMergedCharacterProperties;
      const ACallback: TdxShowFontFormCallback;
      ACallbackData: TObject); virtual;
    ...
    procedure ShowFloatingInlineObjectLayoutOptionsForm
      ...
      ACallbackData: TObject); virtual;
    procedure ShowTableOptionsForm(ATable: TdxTable;
      AOwner: TObject = nil); virtual;
    ...
    procedure ShowPageSetupForm(AProperties: TdxPageSetupInfo;
      ...
      AInitialTabPage: TdxPageSetupFormInitialTabPage); virtual;
    ...
    function CalcBestSize(AWidth, AHeight: Integer;
      AFixedWidth: Boolean): TSize;
    ...
    function CreateViewPainter(AView: TdxRichEditView)
      : TdxRichEditViewPainter; virtual;
    function CreateBackgroundPainter(AView: TdxRichEditView)
      : TdxRichEditViewBackgroundPainter; virtual;
    procedure RecreateBackgroundPainter(AView: TdxRichEditView);
    procedure RecreateViewPainter(AView: TdxRichEditView);

    procedure DoBeforeExport(Sender: TObject;
      Args: TdxBeforeExportEventArgs); virtual;
    procedure DoAfterExport(Sender: TObject); virtual;
    procedure DoPlainTextChanged(Sender: TObject); virtual;
    procedure DoDocumentClosing(Sender: TObject;
      var CanClose: Boolean); virtual;
    procedure DoContentChanged(Sender: TObject); virtual;
    procedure DoHyperlinkClick(Sender: TObject;
      const Args: TdxHyperlinkClickEventArgs); virtual;
    ...
    procedure DoCustomizeMergeFields(Sender: TObject;
      const Args: TdxCustomizeMergeFieldsEventArgs);
    procedure DoMailMergeFinished(Sender: TObject;
      const Args: TdxMailMergeFinishedEventArgs);
    procedure DoMailMergeRecordFinished(Sender: TObject;
      const Args: TdxMailMergeRecordFinishedEventArgs);
    procedure DoMailMergeRecordStarted(Sender: TObject;
      const Args: TdxMailMergeRecordStartedEventArgs);
    procedure DoMailMergeStarted(Sender: TObject;
      const Args: TdxMailMergeStartedEventArgs);
    procedure DoMailMergeGetTargetDocument(Sender: TObject;
    ...
    procedure PerformDeferredUIUpdates(ADeferredUpdater
      : TdxDeferredBackgroundThreadUIUpdater); virtual;

    procedure ResizeView(AEnsureCaretVisibleOnResize: Boolean);
      overload; virtual;
    procedure ResizeView(AEnsureCaretVisibleOnResize,
    ...
    function CreateScrollBarCore(AScrollBar: TdxOfficeScrollbar)
    ...
    function GetVerticalScrollbarBounds(AWidth, AOffset,
      AHorizontalScrollbarHeight: Integer): TRect; virtual;
    function PerformResize(const AInitialClientBounds: TRect;
  ...
  public
    ...
    procedure AddService(AServiceType: TdxServiceType;
      ACallback: IdxServiceCreatorCallback; APromote: Boolean); overload;
    procedure AddService(AServiceType: TdxServiceType;
      ACallback: IdxServiceCreatorCallback); overload;
    procedure AddService(AServiceType: TdxServiceType;
      AServiceInstance: IInterface; APromote: Boolean); overload;
    procedure AddService(AServiceType: TdxServiceType;
      AServiceInstance: IInterface); overload;
    procedure RemoveService(AServiceType: TdxServiceType;
      APromote: Boolean); overload;
    procedure RemoveService(AServiceType: TdxServiceType); overload;

    property DocumentModel: TdxDocumentModel read GetDocumentModel;
    property DocumentLayout: TdxRichEditDocumentLayout
      read InternalGetDocumentLayout;
    ...
    property ActiveViewType: TdxRichEditViewType read GetActiveViewType
      write SetActiveViewType stored IsActiveViewTypeStored;
    property AutoSizeMode: TdxRichEditAutoSizeMode read FAutoSizeMode
      write SetAutoSizeMode default TdxRichEditAutoSizeMode.None;
    ...
    property ShowCaretInReadOnly: Boolean read FShowCaretInReadOnly
      write SetShowCaretInReadOnly default True;

    property Views: TdxRichEditCustomViewRepository read GetViews
      write SetViews;
    ...
    property Options: TdxRichEditControlCustomOptions read GetOptions
      write SetOptions;
    property MeasurementUnit: TdxMeasurementUnit read GetMeasurementUnit
      write SetMeasurementUnit default TdxMeasurementUnit.Document;

    property Document: IdxRichEditDocument read GetDocument;

    property OnAfterExport: TNotifyEvent read FOnAfterExport
      write FOnAfterExport;
    property OnActiveViewChanged: TNotifyEvent read FOnActiveViewChanged
      write FOnActiveViewChanged;
    property OnDocumentClosing: TCloseQueryEvent read FOnDocumentClosing
      write FOnDocumentClosing;
    property OnContentChanged: TNotifyEvent read FOnContentChanged
      write FOnContentChanged;
    property OnDocumentLoaded: TNotifyEvent read FOnDocumentLoaded
      write FOnDocumentLoaded;
    property OnDocumentProtectionChanged: TNotifyEvent
      read FOnDocumentProtectionChanged write FOnDocumentProtectionChanged;
    property OnEmptyDocumentCreated: TNotifyEvent read FOnEmptyDocumentCreated
      write FOnEmptyDocumentCreated;
    property OnHyperlinkClick: TdxHyperlinkClickEvent read FOnHyperlinkClick
      write FOnHyperlinkClick;
    property OnModifiedChanged: TNotifyEvent read FOnModifiedChanged
      write FOnModifiedChanged;
    property OnPageBackgroundChanged: TNotifyEvent read FOnPageBackgroundChanged
      write FOnPageBackgroundChanged;
    property OnPlainTextChanged: TNotifyEvent read FOnPlainTextChanged
      write FOnPlainTextChanged;
    property OnReadOnlyChanged: TNotifyEvent read FOnReadOnlyChanged
      write FOnReadOnlyChanged;
    property OnSelectionChanged: TNotifyEvent read FOnSelectionChanged
      write FOnSelectionChanged;
    property OnShortCut: TdxRichEditShortCutEvent read FOnShortCut
      write FOnShortCut;
    property OnZoomChanged: TNotifyEvent read FOnZoomChanged
      write FOnZoomChanged;

    property OnCalculateDocumentVariable: TdxCalculateDocumentVariableEvent
      read FOnCalculateDocumentVariable write FOnCalculateDocumentVariable;
    property OnCustomizeMergeFields: TdxCustomizeMergeFieldsEvent
      read FOnCustomizeMergeFields write FOnCustomizeMergeFields;
    property OnMailMergeFinished: TdxMailMergeFinishedEvent
      read FOnMailMergeFinished write FOnMailMergeFinished;
    property OnMailMergeRecordFinished: TdxMailMergeRecordFinishedEvent
      read FOnMailMergeRecordFinished write FOnMailMergeRecordFinished;
    property OnMailMergeRecordStarted: TdxMailMergeRecordStartedEvent
      read FOnMailMergeRecordStarted write FOnMailMergeRecordStarted;
    property OnMailMergeStarted: TdxMailMergeStartedEvent
      read FOnMailMergeStarted write FOnMailMergeStarted;
    property OnMailMergeGetTargetDocument: TdxMailMergeGetTargetDocumentEvent
      read FOnMailMergeGetTargetDocument write FOnMailMergeGetTargetDocument;
  end;
...
type
  ...
  public
    procedure Draw(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter); override;
...
procedure TdxRichEditControlPainter.Draw(AGraphics: TdxGraphics;
  const ACustomMarkExporter: IdxCustomMarkExporter);
var
  I, ACount: Integer;
...
begin
  ...
  try
    ...
    try
      ...
      PerformRenderingInPixels(AGraphics, ACustomMarkExporter,
        RenderDocumentDecorators);
...
procedure TdxRichEditControlPainter.DrawCaret(const ADrawCaret
  : TdxDrawDelegate);
var
  AHdc: HDC;
...
procedure TdxRichEditControlPainter.RegisterDeferredDraw
  (const ADraw: TdxDrawDelegate);
begin
  FDeferredDraws.Add(ADraw);
end;

procedure TdxRichEditControlPainter.DeferredDraw(APage: TdxPageViewInfo;
  ADrawAtPage: TdxDrawAtPageDelegate);
var
  ADraw: TdxDrawDelegate;
begin
  ADraw := procedure(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter)
    begin
...
procedure TdxRichEditControlPainter.DeferredDrawReversibleFrame
  (const ABounds: TRect; APage: TdxPageViewInfo);
var
  ADraw: TdxDrawDelegate;
...
begin
  ...
  ADraw := procedure(AGraphics: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter)
    begin
      Control.ViewPainter.DrawReversibleFrameAtPage(AGraphics,
        ABoundsForDelegate, APage);
    end;
  RegisterDeferredDraw(ADraw);
end;

procedure TdxRichEditControlPainter.DrawReversibleHorizontalLine(Y: Integer;
  APage: TdxPageViewInfo);
var
  ADraw: TdxDrawDelegate;
begin
  ADraw := procedure(AGraphicsCache: TdxGraphics;
      const ACustomMarkExporter: IdxCustomMarkExporter)
    begin
...
procedure TdxRichEditControlPainter.DrawReversibleVerticalLine(X: Integer;
  APage: TdxPageViewInfo);
var
  ADraw: TdxDrawDelegate;
begin
...
procedure TdxRichEditControlPainter.DrawReversibleCore(APage: TdxPageViewInfo;
  const ADraw: TdxDrawDelegate);
var
  AHdc: THandle;
  AGraphics: TdxGraphics;
...
procedure TdxRichEditControlPainter.PerformRenderingInUnits
  (AGraphics: TdxGraphics; const ACustomMarkExporter: IdxCustomMarkExporter;
  const ADraw: TdxDrawDelegate);
var
...
begin
  ...
  AModifier := TdxGraphicsToLayoutUnitsModifier.Create(AGraphics,
    ALayoutUnitConverter);
  try
    ...
    AHdcOriginModifier := TdxHdcOriginModifier.Create(AGraphics,
      APaintOrg, 1.0);
    try
...
procedure TdxRichEditControlPainter.PerformRenderingInPixels
  (AGraphics: TdxGraphics; const ACustomMarkExporter: IdxCustomMarkExporter;
  const ADraw: TdxDrawDelegate);
begin
...
procedure TdxRichEditControlPainter.RenderDocument(AGraphics: TdxGraphics;
  const ACustomMarkExporter: IdxCustomMarkExporter);
begin
...
procedure TdxRichEditControlPainter.RenderDocumentDecorators
  (AGraphics: TdxGraphics; const ACustomMarkExporter: IdxCustomMarkExporter);
begin
...
procedure TdxRichEditControlPainter.DrawCaretCore(AGraphics: TdxGraphics;
  const ACustomMarkExporter: IdxCustomMarkExporter);
begin
  DrawCaretCore(Caret, AGraphics, ACustomMarkExporter);
end;

procedure TdxRichEditControlPainter.DrawDragCaretCore(AGraphics: TdxGraphics;
  const ACustomMarkExporter: IdxCustomMarkExporter);
begin
  DrawCaretCore(DragCaret, AGraphics, ACustomMarkExporter);
end;

procedure TdxRichEditControlPainter.DrawCaretCore(ACaret: TdxCaret;
  AGraphics: TdxGraphics; const ACustomMarkExporter: IdxCustomMarkExporter);
var
...
begin
  ...
  if not APosition.Update(TdxDocumentLayoutDetailsLevel.Character) then
    Exit;
  AViewBounds := ActiveView.CreateLogicalRectangle(APosition.PageViewInfo,
    ActiveView.Bounds);
...
function TdxRichEditControlPainter.GetActualBounds(AGraphics: TdxGraphics;
  APage: TdxPageViewInfo; const ABounds: TRect): TRect;
begin
  Result := ActiveView.CreatePhysicalRectangleFast(APage, ABounds);
  Result := ActiveView.DocumentLayout.UnitConverter.LayoutUnitsToPixels(Result,
    AGraphics.DpiX, AGraphics.DpiY);
end;

procedure TdxRichEditControlPainter.DrawReversibleFrameCore
...
begin
  ...
  if not ActiveView.CaretPosition.Update(TdxDocumentLayoutDetailsLevel.Character)
  then
    Exit;
end;

procedure TdxRichEditControlPainter.DrawReversibleHorizontalLineCore
...
procedure TdxRichEditControlPainter.DrawReversibleVerticalLineCore
...
procedure TdxEmptyRichEditControlPainter.Draw(AGraphics: TdxGraphics;
  const ACustomMarkExporter: IdxCustomMarkExporter);
begin
end;
...
function TdxRichEditCustomInnerControl.CreateKeyboardController
  : TdxCustomKeyboardController;
begin
  Result := TdxRichEditKeyboardController.Create(Self);
end;

function TdxRichEditCustomInnerControl.CreateNativeDocument
  : IdxRichEditDocument;
begin
...
function TdxRichEditControlBase.CreateMailMergeOptions
  : IdxRichEditMailMergeOptions;
begin
...
procedure TdxRichEditControlBase.MailMerge(const ADocument
  : IdxRichEditDocument);
begin
  if InnerControl <> nil then
...
procedure TdxRichEditControlBase.MailMerge(const AOptions
...
procedure TdxRichEditControlBase.MailMerge(const AFileName: string;
...
procedure TdxRichEditControlBase.MailMerge(AStream: TStream;
  AFormat: TdxRichEditDocumentFormat);
begin
  if InnerControl <> nil then
...
function TdxRichEditControlBase.CalculateActualViewBounds
...
function TdxRichEditControlBase.CalculateViewBounds(AClientBounds
  : TRect): TRect;
var
  AViewPixelBounds: TRect;
begin
  AViewPixelBounds := CalculateViewPixelBounds(AClientBounds);
  Result := DocumentModel.LayoutUnitConverter.PixelsToLayoutUnits
    (AViewPixelBounds, DpiX, DpiY);
end;

function TdxRichEditControlBase.CalculateViewPixelBounds(AClientBounds
  : TRect): TRect;
var
  AViewPadding: TRect;
begin
  ...
  Result.Inflate(-AViewPadding.Left, -AViewPadding.Top, -AViewPadding.Right,
    -AViewPadding.Bottom);
end;

procedure TdxRichEditControlBase.SetBounds(ALeft, ATop, AWidth,
  AHeight: Integer);
var
  ABestSize: TSize;
begin
  if (AutoSizeMode <> TdxRichEditAutoSizeMode.None) and
    not FIsUpdateAutoSizeMode and not FInsideResize then
  begin
...
begin
  ...
  try
    ...
    begin
      Canvas.FillRect(FSizeGripBounds,
        LookAndFeel.Painter.DefaultSizeGripAreaColor);
...
function TdxRichEditControlBase.GetLayoutCalculationMode
...
procedure TdxRichEditControlBase.AddDocumentLayoutInvalidated
  (const AHandler: TdxDocumentLayoutInvalidatedEvent);
begin
  InnerControl.DocumentLayoutInvalidated.Add(AHandler);
end;

procedure TdxRichEditControlBase.AddPageFormatted(const AHandler
  : TdxDocumentLayoutInvalidatedEvent);
begin
...
procedure TdxRichEditControlBase.RemoveDocumentLayoutInvalidated
  (const AHandler: TdxDocumentLayoutInvalidatedEvent);
begin
  InnerControl.DocumentLayoutInvalidated.Remove(AHandler);
end;

procedure TdxRichEditControlBase.RemovePageFormatted(const AHandler
  : TdxDocumentLayoutInvalidatedEvent);
begin
  InnerControl.PageFormatted.Remove(AHandler);
end;

procedure TdxRichEditControlBase.RemoveDocumentFormatted(const AHandler
  : TdxEvent);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AField <> nil) and DocumentModel.ActivePieceTable.IsHyperlinkField
        (AField) then
        Exit(True);
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ANewSize := CalcBestSize(Width, Height,
        (AutoSizeMode <> TdxRichEditAutoSizeMode.Horizontal) or
...
procedure TdxRichEditControlBase.ShowSplitTableCellsForm(const AParameters
  : TdxSplitTableCellsParameters;
...
procedure TdxRichEditControlBase.ShowNumberingListForm
  (AParagraphs: TdxParagraphList;
  const ACallback: TdxShowNumberingListFormCallback; ACallbackData: TObject);
begin
end;

procedure TdxRichEditControlBase.ShowPageSetupForm
  (AProperties: TdxPageSetupInfo; const ACallback: TdxShowPageSetupFormCallback;
  ACallbackData: TObject; AInitialTabPage: TdxPageSetupFormInitialTabPage);
begin
end;

procedure TdxRichEditControlBase.ShowParagraphForm(AParagraphProperties
  : TdxMergedParagraphProperties; const ACallback: TdxShowParagraphFormCallback;
...
procedure TdxRichEditControlBase.ShowFontForm(ACharacterProperties
  : TdxMergedCharacterProperties; const ACallback: TdxShowFontFormCallback;
  ACallbackData: TObject);
begin
end;

procedure TdxRichEditControlBase.ShowHyperlinkForm(AHyperlinkInfo
  : TdxHyperlinkInfo; ARunInfo: TdxRunInfo; const ATitle: string;
...
procedure TdxRichEditControlBase.ShowInsertTableCellsForm(const AParameters
  : TdxTableCellsParameters;
  const ACallback: TdxShowInsertDeleteTableCellsFormCallback;
  ACallbackData: TObject);
begin
...
procedure TdxRichEditControlBase.ShowInsertTableForm(const AParameters
  : TdxCreateTableParameters; const ACallback: TdxShowInsertTableFormCallback;
  ACallbackData: TObject);
begin
end;

procedure TdxRichEditControlBase.ShowLineNumberingForm
  (AProperties: TdxLineNumberingInfo;
  const ACallback: TdxShowLineNumberingFormCallback; ACallbackData: TObject);
begin
end;

procedure TdxRichEditControlBase.ShowMergeDatabaseRecordsForm
  (const AMergeRecordsParameters: TdxMergeRecordsParameters;
  const ACallback: TdxShowMergeDatabaseRecordsFormCallback);
begin
end;

procedure TdxRichEditControlBase.ShowSymbolForm(const ASymbolProperties
  : TdxSymbolProperties; const ACallback: TdxShowSymbolFormCallback;
  ACallbackData: TObject);
begin
end;

procedure TdxRichEditControlBase.ShowTableOptionsForm(ATable: TdxTable;
  AOwner: TObject);
begin
end;
...
procedure TdxRichEditControlBase.ShowTablePropertiesForm(ASelectedCells
...
procedure TdxRichEditControlBase.ShowTabsForm(ATabInfo: TdxTabFormattingInfo;
  ADefaultTabWidth: Integer; const ACallback: TdxShowTabsFormCallback;
  ACallbackData: TObject);
begin
end;
...
begin
  if not(IsDesigning or IsDestroying) and Enabled then
  begin
    Args := TdxDragEventArgs.CreateFormOle(FDragObject, grfKeyState, pt,
      dwEffect);
...
function TdxRichEditControlBase.OleDrop(const dataObj: IDataObject;
  grfKeyState: Integer; pt: TPoint; var dwEffect: Integer): HResult;
var
  Args: TdxDragEventArgs;
begin
...
function TdxRichEditControlBase.OleQueryContinueDrag(fEscapePressed: BOOL;
...
function TdxRichEditControlBase.DoDragDrop(const AData: IDataObject;
  AllowedEffects: TdxDragDropEffects): TdxDragDropEffects;
var
...
function TdxRichEditControlBase.GetBackgroundPainter
  : TdxRichEditViewBackgroundPainter;
begin
  Result := FBackgroundPainter;
end;

function TdxRichEditControlBase.CalcBestSize(AWidth, AHeight: Integer;
  AFixedWidth: Boolean): TSize;
var
  ABestSize: TSize;
begin
...
procedure TdxRichEditControlBase.ActivateViewPlatformSpecific
...
procedure TdxRichEditControlBase.ApplyChangesCorePlatformSpecific
...
function TdxRichEditControlBase.CreateDocumentContainer(ADocumentModel: TObject)
  : IdxRichEditDocumentContainer;
begin
  Result := CreateDocumentServer(TdxDocumentModel(ADocumentModel));
end;

function TdxRichEditControlBase.CreatePlatformSpecificScrollBarAdapter
  : IdxPlatformSpecificScrollBarAdapter;
begin
  Result := TdxWinFormsScrollBarAdapter.Create;
end;

procedure TdxRichEditControlBase.EnsureCaretVisible(ACheckCaretVisibility
  : Boolean = True);
begin
  if (ActiveView <> nil) then
    UpdateUIFromBackgroundThread(procedure()
      begin
...
begin
  Result := TdxWinFormsRichEditViewHorizontalScrollController.Create
    (ARichEditView);
end;

function TdxRichEditControlBase.CreateViewPainter(AView: TdxRichEditView)
  : TdxRichEditViewPainter;
begin
  Result := AView.CreatePainter;
end;

function TdxRichEditControlBase.CreateBackgroundPainter(AView: TdxRichEditView)
  : TdxRichEditViewBackgroundPainter;
begin
  Result := AView.CreateBackgroundPainter;
end;

procedure TdxRichEditControlBase.RecreateBackgroundPainter
  (AView: TdxRichEditView);
begin
  FreeAndNil(FBackgroundPainter);
...
procedure TdxRichEditControlBase.DoBeforeExport(Sender: TObject;
...
procedure TdxRichEditControlBase.DoDocumentClosing(Sender: TObject;
...
procedure TdxRichEditControlBase.DoHyperlinkClick(Sender: TObject;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FBuiltInPopupMenu = nil) or (AMenuClass <> FBuiltInPopupMenu.ClassType)
      then
      begin
...
begin
  ActiveView.ZoomFactor := EnsureRange(ActiveView.ZoomFactor + ADelta / 400,
    Options.Behavior.MinZoomFactor, Options.Behavior.MaxZoomFactor);
  AHandled := True;
end;
...
procedure TdxRichEditControlBase.DoCalculateDocumentVariable(Sender: TObject;
Args: TdxCalculateDocumentVariableEventArgs);
begin
...
procedure TdxRichEditControlBase.DoCustomizeMergeFields(Sender: TObject;
const Args: TdxCustomizeMergeFieldsEventArgs);
begin
...
procedure TdxRichEditControlBase.DoMailMergeFinished(Sender: TObject;
const Args: TdxMailMergeFinishedEventArgs);
begin
...
procedure TdxRichEditControlBase.DoMailMergeRecordFinished(Sender: TObject;
const Args: TdxMailMergeRecordFinishedEventArgs);
begin
...
procedure TdxRichEditControlBase.DoMailMergeRecordStarted(Sender: TObject;
const Args: TdxMailMergeRecordStartedEventArgs);
begin
...
procedure TdxRichEditControlBase.DoMailMergeStarted(Sender: TObject;
const Args: TdxMailMergeStartedEventArgs);
begin
...
function TdxRichEditControlBase.CreateRichEditViewVerticalScrollController
  (ARichEditView: TdxRichEditView): TdxRichEditViewVerticalScrollController;
begin
  Result := TdxWinFormsRichEditViewVerticalScrollController.Create
    (ARichEditView);
end;
...
procedure TdxRichEditControlBase.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
function TdxRichEditControlBase.GetPixelPhysicalBounds(APageViewInfo
  : TdxPageViewInfo; ALogicalBounds: TRect): TRect;
var
  APhysicalBounds: TRect;
begin
  if APageViewInfo = nil then
    Exit(cxNullRect);
  APhysicalBounds := ActiveView.CreatePhysicalRectangle(APageViewInfo,
    ALogicalBounds);
  APhysicalBounds := cxRectOffset(APhysicalBounds, ViewBounds.Left,
    ViewBounds.Top);
  Result := ActiveView.DocumentLayout.UnitConverter.LayoutUnitsToPixels
    (APhysicalBounds, DpiX, DpiY);
end;
...
function TdxRichEditControlBase.IsScrollBarsArea(const APoint: TPoint): Boolean;
begin
  Result := inherited IsScrollBarsArea(APoint);
  if not Result then
    Result := FHorizontalScrollBar.Visible and
      FHorizontalScrollBar.BoundsRect.Contains(APoint);
  if not Result then
    Result := FVerticalScrollBar.Visible and
      FVerticalScrollBar.BoundsRect.Contains(APoint);
end;
...
begin
  ...
  Result := (ACaptureControl = FVerticalScrollBar) or
    (ACaptureControl = FHorizontalScrollBar);
end;
...
function TdxRichEditControlBase.GetBackgroundThreadUIUpdater
...
function TdxRichEditControlBase.GetControlDeferredChanges
...
function TdxRichEditControlBase.InternalGetDocumentLayout
...
function TdxRichEditControlBase.GetMeasurementAndDrawingStrategy
...
function TdxRichEditControlBase.GetMouseController
  : TdxRichEditCustomMouseController;
begin
...
begin
  ...
  try
    ...
    ADeferredUpdater := BackgroundThreadUIUpdater as
      TdxDeferredBackgroundThreadUIUpdater;
    try
      InnerControl.BackgroundThreadUIUpdater :=
        TdxBeginInvokeBackgroundThreadUIUpdater.Create;
...
begin
  ...
  if InnerControl <> nil then
    InnerControl.BackgroundThreadUIUpdater :=
      TdxDeferredBackgroundThreadUIUpdater.Create;
...
begin
  Result := FControlCreated and (InnerControl <> nil) and
    InnerControl.IsUpdateLocked;
end;

procedure TdxRichEditControlBase.PerformDeferredUIUpdates(ADeferredUpdater
  : TdxDeferredBackgroundThreadUIUpdater);
var
  ACount, I: Integer;
...
function TdxRichEditControlBase.CreateNewDocument(ARaiseDocumentClosing
...
procedure TdxRichEditControlBase.LoadDocumentTemplate(AStream: TStream;
ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerControl <> nil then
...
procedure TdxRichEditControlBase.LoadDocument(AStream: TStream;
ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerControl <> nil then
    InnerControl.LoadDocument(AStream, ADocumentFormat);
...
procedure TdxRichEditControlBase.SaveDocument(AStream: TStream;
ADocumentFormat: TdxRichEditDocumentFormat);
begin
  if InnerControl <> nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      InnerControl.SaveDocument(TPath.ChangeExtension(AFileName,
        TdxExportFileFormats.GetFileExtension(AFormat)), AFormat);
    end;
  end;
end;

procedure TdxRichEditControlBase.OnOptionsChangedPlatformSpecific
...
procedure TdxRichEditControlBase.OnResizeCore(AEnsureCaretVisibleOnResize
  : Boolean);
begin
...
procedure TdxRichEditControlBase.ApplyFont(ACharacterProperties
  : TdxCharacterProperties; AFont: TFont);
begin
  case Options.Behavior.FontSource of
    TdxRichEditBaseValueSource.Control:
      TdxCharacterPropertiesFontAssignmentHelper.AssignFont
        (ACharacterProperties, Font);
    TdxRichEditBaseValueSource.Auto:
      TdxCharacterPropertiesFontAssignmentHelper.AssignFont
        (ACharacterProperties, AFont);
  end;
end;

procedure TdxRichEditControlBase.OnResizeCore(AEnsureCaretVisibleOnResize,
  AEnsureOptimalHorizontalScrollbarPosition: Boolean);
var
...
begin
  ...
  begin
    ...
    if not PerformResize(AInitialClientBounds, AEnsureCaretVisibleOnResize,
      AEnsureOptimalHorizontalScrollbarPosition) then
      Break;
...
function TdxRichEditControlBase.GetVerticalScrollbarBounds(AWidth, AOffset,
  AHorizontalScrollbarHeight: Integer): TRect;
begin
  Result.InitSize(FClientBounds.Right - AWidth, FClientBounds.Top + AOffset,
    AWidth, FClientBounds.Height - AOffset - AHorizontalScrollbarHeight);
end;
...
procedure TdxRichEditControlBase.OnVerticalScrollBarUpdateTimer
  (ASender: TObject);
begin
  if IsUpdateLocked then
    Exit;
...
procedure TdxRichEditControlBase.ResizeView(AEnsureCaretVisibleOnResize
  : Boolean);
begin
...
begin
  ...
  ActiveView.OnResize(ANormalizedViewBounds, AEnsureCaretVisibleOnResize,
    AEnsureOptimalHorizontalScrollbarPosition);
end;

function TdxRichEditControlBase.PerformResize(const AInitialClientBounds: TRect;
AEnsureCaretVisibleOnResize, AEnsureOptimalHorizontalScrollbarPosition
  : Boolean): Boolean;
var
  AVerticalScrollbarWidth, AHorizontalScrollbarHeight,
    AVerticalScrollbarHeight: Integer;
begin
  ...
  VerticalScrollBar.BoundsRect := GetVerticalScrollbarBounds
    (AVerticalScrollbarWidth, 0, AVerticalScrollbarHeight);

  HorizontalScrollBar.BoundsRect := TRect.CreateSize(FClientBounds.Left,
    FClientBounds.Bottom - AHorizontalScrollbarHeight,
    FClientBounds.Width - AVerticalScrollbarWidth, AHorizontalScrollbarHeight);
  ...
  begin
    ...
    FSizeGripBounds.Init(HorizontalScrollBar.BoundsRect.Right,
      VerticalScrollBar.BoundsRect.Bottom, FClientBounds.Right +
      AVerticalScrollbarWidth, FClientBounds.Bottom +
...
procedure TdxRichEditControlBase.RaiseDeferredEvents(AChangeActions
...
begin
  if [TdxDocumentModelChangeAction.RaiseEmptyDocumentCreated,
    TdxDocumentModelChangeAction.RaiseDocumentLoaded] * AChangeActions <> []
  then
    OnBackColorChanged;

  AInnerControl := InnerControl;
  if (AInnerControl <> nil) and not IsDestroying then
    TdxUIThreadSyncService.EnqueueInvokeInUIThread(AInnerControl, procedure
      begin
...
begin
  if IsUpdateLocked then
    ControlDeferredChanges.Redraw := True
  else if not AAfterEndUpdate then
    CustomRefresh
  else if HandleAllocated and not FIsInsideRefresh then
...
begin
  ...
  try
    ...
    try
      ...
      OnResizeCore(False);
      if not ANestedResize and ((AutoSizeMode = TdxRichEditAutoSizeMode.Both) or
        (AutoSizeMode = TdxRichEditAutoSizeMode.Vertical)) then
      begin
...
procedure TdxRichEditControlBase.MouseMove(Shift: TShiftState; X: Integer;
Y: Integer);
begin
...
function TdxRichEditControlBase.DoMouseWheel(Shift: TShiftState;
...
procedure TdxRichEditControlBase.AddKeyboardService(const AService
  : IdxKeyboardHandlerService);
begin
  KeyboardController.AddHandler(AService);
end;

procedure TdxRichEditControlBase.AddService(AServiceType: TdxServiceType;
ACallback: IdxServiceCreatorCallback; APromote: Boolean);
begin
  if InnerControl <> nil then
...
procedure TdxRichEditControlBase.AddService(AServiceType: TdxServiceType;
AServiceInstance: IInterface; APromote: Boolean);
begin
  if InnerControl <> nil then
    (InnerControl as IdxServiceContainer).AddService(AServiceType,
...
function TdxRichEditControlBase.GetKeyboardController
  : TdxCustomKeyboardController;
begin
...
begin
  ...
  FFlushPendingTextInputTimer.Interval :=
    TdxRichEditKeyboardDefaultHandler.PendingTextFlushMilliseconds;
...
procedure TdxRichEditControlBase.OnFlushPendingTextInputTimerTick
  (ASender: TObject);
begin
  if IsUpdateLocked then
    Exit;
...
procedure TdxRichEditControlBase.RemoveKeyboardService(const AService
  : IdxKeyboardHandlerService);
begin
...
procedure TdxRichEditControlBase.UpdateUIFromBackgroundThread
  (const AMethod: TdxAction);
begin
  BackgroundThreadUIUpdater.UpdateUI(Self, AMethod);
end;

function TdxRichEditControlBase.ShowWarningMessage(const AMessage: string)
  : TModalResult;
begin
  Result := MessageDlg(AMessage, mtWarning, [mbOK], 0);
end;

function TdxRichEditControlBase.ShowErrorMessage(const AMessage: string)
  : TModalResult;
begin
  Result := MessageDlg(AMessage, mtError, [mbOK], 0);
end;

procedure TdxRichEditControlBase.SetActiveViewType
  (const Value: TdxRichEditViewType);
begin
  if InnerControl <> nil then
    InnerControl.ActiveViewType := Value;
end;

procedure TdxRichEditControlBase.SetAutoSizeMode(const Value
...
procedure TdxRichEditControlBase.SetLayoutUnit(const Value
...
procedure TdxRichEditControlBase.SetMeasurementUnit
  (const Value: TdxMeasurementUnit);
begin
  if InnerControl <> nil then
    InnerControl.MeasurementUnit := Value;
end;

procedure TdxRichEditControlBase.SetViews(const Value
...
begin
  ...
  if (FHorizontalScrollBar.Height <> ANewHeight) or
    (FVerticalScrollBar.Width <> ANewWidth) then
  begin
...
procedure TdxRichEditControlBase.SetOptions(const Value
...
procedure TdxRichEditControlBase.ShowColumnsSetupForm(const AProperties
  : TdxColumnsInfoUI; const ACallback: TdxShowColumnsSetupFormCallback;
ACallbackData: TObject);
begin
end;

procedure TdxRichEditControlBase.ShowDeleteTableCellsForm(const AParameters
  : TdxTableCellsParameters;
const ACallback: TdxShowInsertDeleteTableCellsFormCallback;
ACallbackData: TObject);
begin
...
procedure TdxRichEditControlBase.ShowDocumentEncryptQueryNewPasswordForm
  (const APassword: string; const ACallback: TdxPasswordFormCallback);
begin
end;

procedure TdxRichEditControlBase.ShowDocumentProtectionQueryNewPasswordForm
  (const APassword: string; const ACallback: TdxPasswordFormCallback);
begin
end;

procedure TdxRichEditControlBase.ShowDocumentProtectionQueryPasswordForm
  (const APassword: string; const ACallback: TdxPasswordFormCallback);
begin
end;

procedure TdxRichEditControlBase.ShowEditStyleForm(AParagraphSourceStyle
  : TdxParagraphStyle; AIndex: TdxParagraphIndex;
const ACallback: TdxShowEditStyleFormCallback);
begin
end;

procedure TdxRichEditControlBase.ShowEditStyleForm(ACharacterSourceStyle
  : TdxCharacterStyle; AIndex: TdxParagraphIndex;
const ACallback: TdxShowEditStyleFormCallback);
begin
end;
...
function TdxRichEditControlBase.CreateScrollBarCore
  (AScrollBar: TdxOfficeScrollbar): IdxOfficeScrollbar;
begin
...
begin
  ...
  if FHorizontalScrollBar.Visible then
    FHorizontalScrollBar.Enabled := ActiveView.HorizontalScrollController.
      IsScrollPossible;
  if FVerticalScrollBar.Visible then
    FVerticalScrollBar.Enabled := ActiveView.VerticalScrollController.
      IsScrollPossible;
end;
...
begin
  case Options.HorizontalScrollBar.Visibility of
    ...
    begin
      AIsScrollPossible := ActiveView.HorizontalScrollController.
        IsScrollPossible;
      if AIsScrollPossible then
        Exit(True);
      ActiveView.PageViewInfoGenerator.CalculateMaxPageWidth;
      Result := ActiveView.PageViewInfoGenerator.MaxPageWidth >
        ActiveView.PageViewInfoGenerator.ViewPortBounds.Width + 1;
    end;
  end;
end;
...
procedure TdxRichEditControlBase.ScrollContentByGesture(ADeltaX,
  ADeltaY: Integer);
begin
  if ADeltaY <> 0 then
    VerticalScrollBar.Position := VerticalScrollBar.Position -
      Trunc(ADeltaY / ActiveView.ScaleFactor);
  if ADeltaX <> 0 then
    HorizontalScrollBar.Position := HorizontalScrollBar.Position -
      Trunc(ADeltaX / ActiveView.ScaleFactor);
end;
