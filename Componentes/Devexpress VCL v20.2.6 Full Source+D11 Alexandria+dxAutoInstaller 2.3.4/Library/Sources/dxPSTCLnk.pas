...
type
  ...
  TdxPSTeeChartReportLinkGetGraphicClassEvent = procedure
    (Sender: TBasedxReportLink; var AGraphicClass: TGraphicClass) of object;
  TdxPSTeeChartReportLinkCreateGraphicEvent = procedure
    (Sender: TBasedxReportLink; var AGraphic: TGraphic) of object;
  ...
  protected
    ...
    property GraphicClass: TGraphicClass read GetGraphicClassValue
      write SetGraphicClassValue;
    property GraphicClassName: string read GetGraphicClassName
      write SetGraphicClassName stored IsGraphicClassNameStored;
    property OnCreateGraphic: TdxPSTeeChartReportLinkCreateGraphicEvent
      read FOnCreateGraphic write FOnCreateGraphic;
    property OnGetGraphicClass: TdxPSTeeChartReportLinkGetGraphicClassEvent
...
begin
  ...
  if (AReportCells.Count > 0) and Aggregable and
    SysUtils.Supports(TObject(Controller), IdxReportLinkController,
    ControllerIntf) then
  begin
    ...
    begin
      TdxReportCellGraphic(Cell.DataItems[Cell.DataItemCount - 1]).DrawMode :=
        gdmStretch;
    end;
  end;
end;
...
function TCustomdxTeeChartReportLink.DoCreateGraphic(var AGraphic
  : TGraphic): Boolean;
begin
  AGraphic := nil;
  if Assigned(FOnCreateGraphic) then
    FOnCreateGraphic(Self, AGraphic);
  Result := AGraphic <> nil;
end;

function TCustomdxTeeChartReportLink.GetGraphic: TGraphic;
begin
  if not DoCreateGraphic(Result) then
    CreateGraphicStandard(Result)
end;
...
begin
  ...
  begin
    Result := Chart.Height;
    if Aggregable and SysUtils.Supports(TObject(Controller),
...
procedure TCustomdxTeeChartReportLink.CreateGraphicStandard
...
begin
  ...
  if OwnGraphicClass.InheritsFrom(TBitmap) then
    AGraphic := CheckBitmapFormat(Chart.TeeCreateBitmap(Color,
      Chart.ClientRect))
  else
{$IFDEF USEJPEGIMAGE}
...
procedure TCustomdxTeeChartReportLink.DoGetGraphicClass(var AGraphicClass
  : TGraphicClass);
begin
  if Assigned(FOnGetGraphicClass) then
    FOnGetGraphicClass(Self, AGraphicClass);
end;

function TCustomdxTeeChartReportLink.CheckBitmapFormat
  (ABitmap: TBitmap): TBitmap;
begin
...
procedure TCustomdxTeeChartReportLink.SetGraphicClassValue
  (Value: TGraphicClass);
begin
