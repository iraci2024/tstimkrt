...
uses
  Windows, SysUtils, Classes, Types, Math, RTLConsts, Graphics,
...
type
  ...
  protected
    function CreateTileImageLoadingTask(ATileSource
      : TdxMapControlTileImageSource; AUri: string)
      : TdxMapControlTileImageLoadingTask;
    procedure DestroyTileImageLoadingTask
  ...
  public
    constructor Create(ATileSource: TdxMapControlMultiScaleTileSource;
      AMaxParallelDownloads: Integer);
    ...
    property MaxParallelDownloads: Integer read FMaxParallelDownloads
      write SetMaxParallelDownloads;
  end;

  TdxMapControlTileImageSources = class(TcxDoublyLinkedObjectList)
  public
    function AddSource(const AIndex: TdxMapControlTileIndex; ATileSize: TSize)
  ...
  private
    ...
    FTileImageSources: TDictionary<TdxMapControlTileIndex,
      TdxMapControlTileImageSource>;
  protected
    procedure CheckMemoryLimit(ALimit: Int64;
      AExtraImageSources: TList<TdxMapControlTileImageSource>);
    function CreateTileImageSource(const AIndex: TdxMapControlTileIndex;
      ATileSize: TSize): TdxMapControlTileImageSource;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Clear;
    function Extract(ASource: TdxMapControlTileImageSource)
      : TdxMapControlTileImageSource;
  ...
  protected
    ...
    procedure GetTileImageSource(AIndex: TdxMapControlTileIndex;
      ABitmapTile: TdxMapControlBitmapTile);
    function GetTileLayers(ATileLevel, ATilePositionX, ATilePositionY: Integer)
      : string; virtual;
    function PeekTileImageSource(AIndex: TdxMapControlTileIndex)
      : TdxMapControlTileImageSource;
    function TilePrefix: string; virtual;

    property CacheOptions: TdxMapControlCacheOptions read GetCacheOptions;
    property LoadingQueueManager
      : TdxMapControlTileImageSourceLoadingQueueManager
      read FLoadingQueueManager;

    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
  public
    constructor Create(AOwner: TPersistent; AImageWidth: Int64;
      AImageHeight: Int64; ATileWidth: Integer; ATileHeight: Integer); virtual;
    destructor Destroy; override;
    procedure ClearCache;
    function GetTileByZoomLevel(AZoomLevel, ATilePositionX, ATilePositionY
  ...
  public
    constructor Create(ATileIndex: TdxMapControlTileIndex;
      AExpectedTileSize: TSize);
    destructor Destroy; override;
    ...
    property OnImageLoaded: TNotifyEvent read FOnImageLoaded
      write FOnImageLoaded;
  end;
  ...
  protected
    ...
    property LoadingQueueManager
      : TdxMapControlTileImageSourceLoadingQueueManager
      read FLoadingQueueManager write FLoadingQueueManager;
    property TileImageSource: TdxMapControlTileImageSource
      read FTileImageSource;
    property Uri: string read FUri;
  public
    constructor Create(ATileImageSource: TdxMapControlTileImageSource;
      const AUri: string);
    destructor Destroy; override;
    function Load: Boolean;
  end;

  TdxMapControlTileImageRemoteLoadingTask = class
    (TdxMapControlTileImageLoadingTask)
  private
    procedure DoRequest(ASender: TObject;
      ARequestMode: TdxMapControlHttpRequestMode; ACount: Int64;
      var AIsCancelled: Boolean);
  protected
    function DoLoad: Boolean; override;
    procedure RaiseImageLoaded; override;
  end;

  TdxMapControlTileImageDiskLoadingTask = class
    (TdxMapControlTileImageLoadingTask)
  protected
    function DoLoad: Boolean; override;
  end;
  ...
  public
    ...
    procedure UpdateViewPort(const AViewPortSizeInPixels: TSize;
      const AViewPortBounds: TdxRectDouble);
    ...
    property Source: TdxMapControlMultiScaleTileSource read GetSource
      write SetSource;
  end;
  ...
  public
    constructor Create(ATileFactory: TdxMapControlMultiScaleImage;
      AInflateCount: Integer);
    destructor Destroy; override;
    procedure InvalidateRect(const ARect: TRect);
    procedure Update(const ANewSizeInPixels: TSize;
      const ANewBounds: TdxRectDouble);

    property AspectRatio: Double read GetAspectRatio;
    property InflatedTiles: TSize read GetInflatedTiles;
    property IsReady: Boolean read GetIsReady;
    property MultiScaleTileImage: TdxMapControlMultiScaleImage
      read FTileFactory;
  ...
  TdxMapControlTileMaskAction = procedure(X, Y: Integer; var ADone: Boolean)
    of object;
  ...
  private
    ...
    function AddTileBitmapToRenderItems(ATileBitmap
      : TdxMapControlBitmapTile): Boolean;
    ...
    function GetIsReady: Boolean;
    function GetOverlayPieceClip(const AOffset: TdxPointDouble;
      AScale: Double): TRect;
    function GetSourceTileSize: TSize;
    function GetUpwardTileIndex(X, Y, ALevel: Integer;
      out AOffset: TdxPointDouble; out AScale: Double): TdxMapControlTileIndex;
    function LookupCache(ATile: TdxMapControlBitmapTile;
  ...
  public
    constructor Create(AViewport: TdxMapControlMultiScaleViewport;
      X, Y: Integer); virtual;
    destructor Destroy; override;
...
constructor TdxMapControlMultiScaleTile.Create
  (AViewport: TdxMapControlMultiScaleViewport; X, Y: Integer);
begin
  inherited Create;
  FViewport := AViewport;
  FX := X;
...
function TdxMapControlMultiScaleTile.IsTileLoaded
  (ATile: TdxMapControlBitmapTile): Boolean;
begin
  Result := (ATile <> nil) and ATile.IsValid;
end;

procedure TdxMapControlMultiScaleTile.TileSourceLoaded
  (ABitmapTile: TdxMapControlBitmapTile);
begin
...
procedure TdxMapControlMultiScaleTile.CalculateMaskBitmapBounds(X, Y: Integer;
  var ADone: Boolean);
begin
  FMask[X, Y].Position := cxPointOffset(FFace.Position, X * FMaskSize.cx,
    Y * FMaskSize.cy);
  FMask[X, Y].Size := FMaskSize;
  FMask[X, Y].CalculateBounds;
end;

procedure TdxMapControlMultiScaleTile.CreateMaskBitmap(X, Y: Integer;
  var ADone: Boolean);
begin
...
procedure TdxMapControlMultiScaleTile.DestroyMaskBitmap(X, Y: Integer;
  var ADone: Boolean);
begin
...
procedure TdxMapControlMultiScaleTile.UpdateMaskBitmap(X, Y: Integer;
  var ADone: Boolean);
var
...
function TdxMapControlMultiScaleTile.AddTileBitmapToRenderItems
  (ATileBitmap: TdxMapControlBitmapTile): Boolean;
begin
...
procedure TdxMapControlMultiScaleTile.EnumerateMaskBitmaps
  (AAction: TdxMapControlTileMaskAction);
var
  I, J: Integer;
  ADone: Boolean;
begin
...
function TdxMapControlMultiScaleTile.GetDownwardTileIndex(X, Y: Integer)
  : TdxMapControlTileIndex;
begin
  Result := dxMapControlTileIndex(FIndex.X * 2 + X, FIndex.Y * 2 + Y,
    FIndex.Level + 1);
end;

function TdxMapControlMultiScaleTile.GetIsReady: Boolean;
begin
  Result := dxMapControlTileIndexIsEqual(FIndex, dxMapControlInvalidTileIndex)
    or FFace.IsValid;
end;

function TdxMapControlMultiScaleTile.GetOverlayPieceClip(const AOffset
  : TdxPointDouble; AScale: Double): TRect;
begin
  Result := cxRectBounds(Ceil(AOffset.X * TileImageSize.cx),
    Ceil(AOffset.Y * TileImageSize.cy), Ceil(TileImageSize.cx / AScale),
...
begin
  ...
  AOffset := dxPointDouble((APieceX / ADistance) - AUpX,
    (APieceY / ADistance) - AUpY);
  AScale := ADistance;
...
procedure TdxMapControlMultiScaleTile.SetIndex(const Value
  : TdxMapControlTileIndex);
begin
...
begin
  ...
  begin
    ...
    if (FFace.Source = nil) or (FFace.Source.Status <> mptsReady) or
      (FFace.Source.Source = nil) then
      EnumerateMaskBitmaps(UpdateMaskBitmap);
  end;
end;

procedure TdxMapControlMultiScaleTile.UpdateRenderParams
  (AScaleFactor: TdxPointDouble);
begin
...
constructor TdxMapControlMultiScaleViewport.Create(ATileFactory
  : TdxMapControlMultiScaleImage; AInflateCount: Integer);
begin
  inherited Create;
...
begin
  ...
  begin
    AInflatedSize := cxSize(ASize.cx + FInflateCount * 2,
      ASize.cy + FInflateCount * 2);
    ...
    for Y := 0 to AInflatedSize.cy - 1 do
      for X := 0 to AInflatedSize.cx - 1 do
        if ATiles[X, Y] = nil then
          ATiles[X, Y] := MultiScaleTileImage.CreateTile(Self,
            X - FInflateCount, Y - FInflateCount);
    FTiles := ATiles;
...
procedure TdxMapControlMultiScaleViewport.ProjectRange
  (ARange: TdxMapControlTileRange);
var
  AInflatedSize: TSize;
  X, Y: Integer;
  AProjX, AProjY: Integer;
begin
  if (Length(FTiles) > 0) and not dxMapControlTileRangeIsEqual(ARange,
    FTileRange) then
  begin
    ...
    for Y := 0 to AInflatedSize.cy - 1 do
      ...
      begin
        ...
        if (AProjX >= FZoomLevelTileCount) or (AProjY >= FZoomLevelTileCount) or
          (AProjX < 0) or (AProjY < 0) then
          FTiles[X, Y].Index := dxMapControlInvalidTileIndex
        else
          FTiles[X, Y].Index := dxMapControlTileIndex(AProjX, AProjY,
            ARange.Min.Level);
      end;
    FTileRange := ARange;
  end;
...
begin
  ...
  if (FViewSize.cx = 0) or (FBounds.Width = 0) or (FTileSource = nil) or
    cxSizeIsEmpty(SourceTileSize) then
    Exit;
  ...
  FZoomLevel := Min(Max(Floor(Log2(FArrangedTiles.cx / AContentSize.Width)), 1),
    FTileSource.MaxZoomLevel);
  FZoomLevelTileCount := Round(Power(2.0, FZoomLevel));
  AMin := dxMapControlTileIndex
    (Max(Floor(AContentOrigin.X * FZoomLevelTileCount), 0),
    Max(Floor(AContentOrigin.Y * FZoomLevelTileCount), 0), FZoomLevel);

  AMax := dxMapControlTileIndex
    (Min(Ceil((AContentOrigin.X + AContentSize.Width) * FZoomLevelTileCount),
    FZoomLevelTileCount), Min(Ceil((AContentOrigin.Y + AContentSize.Height) *
    FZoomLevelTileCount), Round(FZoomLevelTileCount *
    FTileSource.TileCountRatio)), FZoomLevel);

  ProjectRange(dxMapControlTileRange(AMin, AMax));
  FScale := dxPointDouble(FViewSize.cx / (AContentSize.Width *
    SourceTileSize.cx) / FZoomLevelTileCount,
    FViewSize.cy / (AContentSize.Height * SourceTileSize.cy) /
    FZoomLevelTileCount);
  FOffset := cxPoint(FTileRange.Min.X * SourceTileSize.cx -
    Round(AContentOrigin.X * SourceTileSize.cx * FZoomLevelTileCount),
    FTileRange.Min.Y * SourceTileSize.cy - Round(AContentOrigin.Y *
    SourceTileSize.cy * FZoomLevelTileCount));
end;
...
procedure TdxMapControlMultiScaleViewport.Update(const ANewSizeInPixels: TSize;
  const ANewBounds: TdxRectDouble);
begin
...
begin
  if cxSizeIsEmpty(FArrangedTiles) then
    ...
    Result := cxSize(FArrangedTiles.cx + (FInflateCount shl 1),
      FArrangedTiles.cy + (FInflateCount shl 1));
end;
...
function TdxMapControlMultiScaleViewport.GetTiles(X, Y: Integer)
  : TdxMapControlMultiScaleTile;
begin
  Result := FTiles[X, Y];
end;

procedure TdxMapControlMultiScaleViewport.SetTileSource
  (Value: TdxMapControlMultiScaleTileSource);
begin
...
function TdxMapControlMultiScaleImage.CreateTile
  (AViewport: TdxMapControlMultiScaleViewport; X, Y: Integer)
  : TdxMapControlMultiScaleTile;
begin
  ...
  try
    for Y := 0 to Viewport.InflatedTiles.cy - 1 do
      ...
      begin
        ...
        if dxMapControlTileIndexIsEqual(ATile.Index,
          dxMapControlInvalidTileIndex) then
          Continue;
        if (X < 1) or (Y < 1) or (X > Viewport.InflatedTiles.cx - 2) or
          (Y > Viewport.InflatedTiles.cy - 2) then
          ATile.UpdateRenderParams(Viewport.Scale)
        else
          AVisibleTiles.Add(ATile);
      end;
    for I := 0 to AVisibleTiles.Count - 1 do
      TdxMapControlMultiScaleTile(AVisibleTiles[I])
        .UpdateRenderParams(Viewport.Scale);
  finally
    AVisibleTiles.Free;
  end;
end;
...
function TdxMapControlMultiScaleImage.GetSource
  : TdxMapControlMultiScaleTileSource;
begin
  Result := FViewport.TileSource;
end;

procedure TdxMapControlMultiScaleImage.SetSource(const Value
  : TdxMapControlMultiScaleTileSource);
begin
...
begin
  ...
  FBounds := cxRectBounds(Ceil(APosition.X * AScale.X),
    Ceil(APosition.Y * AScale.Y), Ceil(Size.cx * AScale.X),
...
procedure TdxMapControlBitmapTile.SetSource(const Value
  : TdxMapControlTileImageSource);
var
...
constructor TdxMapControlMultiScaleTileSource.Create(AOwner: TPersistent;
  AImageWidth: Int64; AImageHeight: Int64; ATileWidth: Integer;
  ATileHeight: Integer);
begin
  inherited Create;
  FOwner := AOwner;
  ...
  FTileCountRatio := FImageHeight /
    (FImageWidth * (FTileSize.cx / FTileSize.cy));
  FMaxZoomLevel := Round(Log2(FImageWidth / FTileSize.cx));
  FLoadingQueueManager := TdxMapControlTileImageSourceLoadingQueueManager.Create
    (Self, GetMaxParallelConnections);
  UpdateDiskCache;
end;
...
procedure TdxMapControlMultiScaleTileSource.PushToDisk
...
function TdxMapControlMultiScaleTileSource.GetCacheOptions
...
begin
  Result := (FOwner as TdxMapControlImageTileProvider)
...
begin
  ...
  begin
    if ACacheOptions.DiskFolder <> '' then
      FDiskCache := TdxMapControlDiskCache.Create(ACacheOptions.DiskFolder,
        ACacheOptions.DiskExpireTime, ACacheOptions.DiskLimit)
  end
  else if ACacheOptions.DiskFolder = '' then
    FreeAndNil(FDiskCache)
  else if not FDiskCache.LocatedAt(ACacheOptions.DiskFolder) then
  begin
    FreeAndNil(FDiskCache);
    FDiskCache := TdxMapControlDiskCache.Create(ACacheOptions.DiskFolder,
      ACacheOptions.DiskExpireTime, ACacheOptions.DiskLimit)
...
function TdxMapControlMultiScaleTileSource.GetSubdomainIndex(ASubdomainCount
  : Integer): Integer;
begin
  FCurrentSubdomainIndex := IfThen(FCurrentSubdomainIndex + 2 > ASubdomainCount,
    0, FCurrentSubdomainIndex + 1);
...
procedure TdxMapControlMultiScaleTileSource.GetTileImageSource
  ...
  begin
    ...
    if (FDiskCache <> nil) and FDiskCache.TryRetrieveFileName(AIndex,
      TilePrefix, AUri) then
      ATileLoadingTask := LoadingQueueManager.CreateTileImageLoadingTask
        (ATileImageSource, AUri);
...
begin
  ...
  begin
    ...
    begin
      ATileImageSource := FTileSourceStorage.CreateTileImageSource(AIndex,
        TileSize);
      ABitmapTile.Source := ATileImageSource;
      CheckMemoryLimit;
...
function TdxMapControlMultiScaleTileSource.GetTileLayers(ATileLevel,
...
function TdxMapControlMultiScaleTileSource.PeekTileImageSource
  (AIndex: TdxMapControlTileIndex): TdxMapControlTileImageSource;
var
  ASource: TdxMapControlTileImageSource;
begin
  FTileSourceStorage.TryGetValue(AIndex, ASource);
  if (ASource <> nil) and (ASource.Status = mptsReady) and
    (ASource.Source <> nil) then
    Result := ASource
  else
    Result := nil;
end;
...
begin
  ...
  try
    FTileSourceStorage.CheckMemoryLimit(CacheOptions.MemoryLimit shl 20,
      AExtraTileImageSources);
    for I := 0 to AExtraTileImageSources.Count - 1 do
      if not LoadingQueueManager.DelayedDestroying(AExtraTileImageSources[I])
      then
...
begin
  ...
  dxTestCheck(FBitmapTiles.Count = 0,
    'TdxMapControlTileImageSource.Destroy: FBitmapTiles.Count <> 0');
...
procedure TdxMapControlTileImageSource.AddLink(ABitmapTile
  : TdxMapControlBitmapTile);
begin
...
procedure TdxMapControlTileImageSource.SetStatusSafe
  (AStatus: TdxMapControlTileStatus);
begin
...
constructor TdxMapControTileImageLoadingThread.Create
  (AOwner: TdxMapControlTileImageSourceLoadingQueueManager);
begin
...
constructor TdxMapControlTileImageLoadingTask.Create(ATileImageSource
  : TdxMapControlTileImageSource; const AUri: string);
begin
  inherited Create;
...
procedure TdxMapControlTileImageRemoteLoadingTask.DoRequest(ASender: TObject;
  ...
  var AIsCancelled: Boolean);
begin
  AIsCancelled := not TileImageSource.IsLinked or
    TileImageSource.FNeedToBeDestoying;
end;
...
function TdxMapControlTileImageSourceLoadingQueueManager.
  CreateTileImageLoadingTask(ATileSource: TdxMapControlTileImageSource;
...
procedure TdxMapControlTileImageSourceLoadingQueueManager.
  DestroyTileImageLoadingTask(var ATask: TdxMapControlTileImageLoadingTask);
begin
  LockTaskDestroying;
  try
    FreeAndNil(ATask);
...
procedure TdxMapControlTileImageSourceLoadingQueueManager.ImageLoaded
  (ASource: TdxMapControlTileImageSource);
begin
...
procedure TdxMapControlTileImageSourceLoadingQueueManager.
  SetMaxParallelDownloads(const Value: Integer);
begin
...
function TdxMapControlTileImageSources.AddSource(const AIndex
  : TdxMapControlTileIndex; ATileSize: TSize): TdxMapControlTileImageSource;
begin
  Result := TdxMapControlTileImageSource.Create(AIndex, ATileSize);
  DoAdd(Result);
end;

procedure TdxMapControlTileImageSources.MakeLast(AImageTileSource
  : TdxMapControlTileImageSource);
begin
...
begin
  ...
  FTileImageSources := TDictionary<TdxMapControlTileIndex,
    TdxMapControlTileImageSource>.Create;
end;

function TdxMapControlTileImageSourceStorage.CreateTileImageSource
  (const AIndex: TdxMapControlTileIndex; ATileSize: TSize)
  : TdxMapControlTileImageSource;
begin
...
function TdxMapControlTileImageSourceStorage.TryGetValue
  (const AKey: TdxMapControlTileIndex;
  out AValue: TdxMapControlTileImageSource): Boolean;
begin
