...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Classes, Controls, Windows, Graphics, Generics.Defaults,
  Generics.Collections, cxGraphics, cxGeometry,
  cxClasses, cxControls, cxLookAndFeels, dxCustomPreview, dxPDFBase, dxPDFCore,
...
type
  ...
  TdxPDFDocumentViewerOnCancelRendering = procedure(Sender: TObject;
    const APageIndexes: TIntegerDynArray) of object;
  TdxPDFDocumentViewerOnGetRenderFactor = function(APageIndex: Integer)
    : Single of object;
  ...
  TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent = procedure
...
{$REGION 'internal types'}
    ...
    strict private
      ...
      FLimit: Int64;
      FQueue: TdxPDFIntegerList;
      FSize: Int64;
      function GetSize(AValue: TBitmap): Int64;
      procedure CheckSize;
    protected
      function TryGetValue(APageIndex: Integer; out ABitmap: TBitmap)
        : Boolean; overload;
      function TryGetValue(APageIndex: Integer; out ACache: TCacheItem)
        : Boolean; overload;
      function TryGetValue(APageIndex: Integer; AFactor: Single;
        out ACache: TCacheItem): Boolean; overload;
...
{$ENDREGION}
  ...
  protected
    ...
    property CacheVisiblePagesOnly: Boolean read FCacheVisiblePagesOnly
      write SetCacheVisiblePagesOnly;
  public
    constructor Create;
    destructor Destroy; override;
  end;
  ...
  TdxPDFRenderTaskInfo = record // for internal use
    Factor: Single;
    PageIndex: Integer;
    class function Create(APageIndex: Integer; AFactor: Single)
      : TdxPDFRenderTaskInfo; static;
  end;

  { TdxPDFDocumentViewerCustomRenderer }

  TdxPDFDocumentViewerCustomRendererClass = class of
    TdxPDFDocumentViewerCustomRenderer;
  ...
  strict private
    ...
    function CreatePageInfo(APageIndex: Integer)
      : TdxPDFDocumentViewerCachePageInfo;
    procedure AddPageIndex(APages: TDictionary<Integer,
      TdxPDFDocumentViewerCachePageInfo>; APageIndex: Integer);
    procedure AddPageIndexes(APages: TDictionary<Integer,
      TdxPDFDocumentViewerCachePageInfo>; AStartIndex, AEndIndex: Integer);
    procedure AddPageIndexesToRenderQueue
      (APages: TList<TdxPDFDocumentViewerCachePageInfo>; AForce: Boolean);
    procedure InitializeVisiblePageIndexes;
    procedure OnCancelRenderingHandler(Sender: TObject;
  ...
  strict protected
    ...
    function NeedProcessRenderTask(const ATask: TdxPDFRenderTaskInfo)
      : Boolean; virtual;
    ...
    procedure DoDrawPage(ACanvas: TcxCanvas; APageIndex: Integer;
    ...
    property CacheVisiblePagesOnly: Boolean read GetCacheVisiblePagesOnly
      write SetCacheVisiblePagesOnly;
  ...
  protected
    ...
    procedure DrawPage(ACanvas: TcxCanvas; APageIndex: Integer;
      const ARect: TRect); virtual;
    ...
    property OnGetRenderFactor: TdxPDFDocumentViewerOnGetRenderFactor
      read FOnGetRenderFactor write FOnGetRenderFactor;
  public
  ...
  strict private
    ...
    procedure OnCompleteHandler(APageIndex: Integer; AFactor: Single;
      ABitmap: TBitmap);
  strict protected
    FService: TObject;
  protected
  ...
  protected
    ...
    function ToDocumentPoint(APage: TdxPDFDocumentCustomViewerPage;
      const P: TdxPointF): TdxPointF;

    property DPI: TdxPointF read FDPI;
    property Viewer: TdxPDFDocumentCustomViewer read FViewer;
  public
    constructor Create(AOwner: TObject; AViewer: TdxPDFDocumentCustomViewer);
      reintroduce;

    function ToDocumentRect(APage: TdxPDFDocumentCustomViewerPage;
      const R: TdxRectF): TdxRectF;
    function ToViewerPoint(APage: TdxPDFDocumentCustomViewerPage;
      const P: TdxPointF): TdxPointF;
    function ToViewerRect(const AObject: IdxPDFInteractiveObject)
      : TRect; overload;
    function ToViewerRect(APage: TdxPDFDocumentCustomViewerPage;
      const R: TdxRectF): TdxRectF; overload;
  ...
  protected
    ...
    function GetPageSize(const AScreenFactor: TdxPointF): TPoint;
    // for internal use
    ...
    property DocumentPage: TdxPDFPage read FDocumentPage write SetDocumentPage;
    // for internal use
...
{$ENDREGION}
  ...
  protected
    ...
    function GetPageRenderFactor(APageIndex: Integer): Single; virtual;
      abstract;
    procedure DestroySubClasses; virtual;
    ...
    function TryPackDocumentContent: Boolean;
    ...
    property RenderContentDelay: Integer read FRenderContentDelay
      write SetRenderContentDelay;
    property RenderContentInBackground: Boolean read FRenderContentInBackground
      write SetRenderContentInBackground;
    ...
    property OnCustomDrawPreRenderPage
      : TdxPDFDocumentViewerOnCustomDrawPreRenderPageEvent
      read FOnCustomDrawPreRenderPage write FOnCustomDrawPreRenderPage;
  public
    destructor Destroy; override;
    procedure Clear; virtual;
  end;
  ...
  TdxPDFDocumentPageThumbnailViewer = class(TdxPDFDocumentCustomViewer,
    IdxSkinSupport, IcxFontListener)
  strict private
  type
{$REGION 'internal types'}
    ...
    protected
      ...
      property ThumbnailsPreview: TdxPDFDocumentPageThumbnailViewer
        read GetThumbnailsPreview;
    public
...
{$ENDREGION}
  strict private
    ...
    FSelectedPages: TdxPDFIntegerList;
    FSize: Integer;
  ...
  protected
    ...
    procedure DrawViewerBackground(ACanvas: TcxCanvas; const R: TRect);
      override;
    ...
    property OnThumbnailSizeChanged: TNotifyEvent read FOnThumbnailSizeChanged
      write FOnThumbnailSizeChanged;
  public
    procedure Clear; override;
  end;
...
uses
  Math, SysUtils, SyncObjs, dxCore, RTLConsts, dxCoreGraphics, dxThreading,
  dxGDIPlusClasses, dxTypeHelpers, dxGenerics,
  dxPDFTypes, dxPDFViewer, dxPDFViewerPopupMenu, dxPDFUtils;
...
type
  ...
  TdxPDFOnRenderingCanceled = procedure(APageIndex: Integer; AFactor: Single)
    of object;
  TdxPDFOnRenderingComplete = procedure(APageIndex: Integer; AFactor: Single;
    ABitmap: TBitmap) of object;

  TdxPDFDocumentViewerCachePageInfoComparer = class(TInterfacedObject,
    IComparer<TdxPDFDocumentViewerCachePageInfo>)
  strict private
    function Compare(const Left,
      Right: TdxPDFDocumentViewerCachePageInfo): Integer;
  end;
  ...
  protected
    function Render(var AImage: TdxSmartImage;
      const ACancelStatus: TdxTaskCancelCallback): Boolean; virtual; abstract;
  public
    Bitmap: TBitmap;
    Handle: THandle;
    PageIndex: Integer;
    ...
    constructor Create(AService: TdxPDFBackgroundService;
      AViewer: TdxPDFDocumentCustomViewer; APageIndex: Integer;
      AFactor: Single);
    destructor Destroy; override;
  ...
  protected
    function Render(var AImage: TdxSmartImage;
      const ACancelStatus: TdxTaskCancelCallback): Boolean; override;
  end;
  ...
  strict private
    FTasks: TList<TdxPDFRenderPageTask>;
    function CreateRenderingTask(AClass: TdxPDFRenderPageTaskClass;
      AViewer: TdxPDFDocumentCustomViewer; APageIndex: Integer;
      AFactor: Single): IdxTask;
    function Cancel(ATaskHandle: THandle; AWaitFor: Boolean = False): Boolean;
    function GetTaskCount: Integer;
    procedure AddRenderingTask(AClass: TdxPDFRenderPageTaskClass;
      AViewer: TdxPDFDocumentCustomViewer; APageIndex: Integer;
      AFactor: Single);
  protected
  ...
  public
    ...
    procedure AddRenderingByScaleFactorTask(AViewer: TdxPDFDocumentCustomViewer;
      APageIndex: Integer; AFactor: Single);
    procedure AddRenderingBySizeTask(AViewer: TdxPDFDocumentCustomViewer;
      APageIndex: Integer; AFactor: Single);
...
constructor TdxPDFRenderPageTask.Create(AService: TdxPDFBackgroundService;
  AViewer: TdxPDFDocumentCustomViewer; APageIndex: Integer; AFactor: Single);
begin
  inherited Create;
  FService := AService;
...
function TdxPDFRenderPageTask.Run(const ACancelStatus: TdxTaskCancelCallback)
  : TdxTaskCompletedStatus;
var
...
function TdxPDFRenderPageByScaleFactorTask.Render(var AImage: TdxSmartImage;
  const ACancelStatus: TdxTaskCancelCallback): Boolean;
var
  ABitmap: TcxBitmap;
...
function TdxPDFRenderPageBySizeTask.Render(var AImage: TdxSmartImage;
  const ACancelStatus: TdxTaskCancelCallback): Boolean;
begin
  Result := dxPDFDocumentExportToImage(Viewer.Document, PageIndex,
    Trunc(Factor), AImage, Viewer.RotationAngle, ACancelStatus);
end;

{ TdxPDFDocumentViewerCachePageInfoComparer }

function TdxPDFDocumentViewerCachePageInfoComparer.Compare(const Left,
  Right: TdxPDFDocumentViewerCachePageInfo): Integer;
begin
...
begin
  inherited Create;
  MaxActiveTasks := 1;
  FTasks := TList<TdxPDFRenderPageTask>.Create;
end;
...
procedure TdxPDFBackgroundService.AddRenderingByScaleFactorTask
  (AViewer: TdxPDFDocumentCustomViewer; APageIndex: Integer; AFactor: Single);
begin
  AddRenderingTask(TdxPDFRenderPageByScaleFactorTask, AViewer,
    APageIndex, AFactor);
end;

procedure TdxPDFBackgroundService.AddRenderingBySizeTask
  (AViewer: TdxPDFDocumentCustomViewer; APageIndex: Integer; AFactor: Single);
begin
...
function TdxPDFBackgroundService.CreateRenderingTask
  (AClass: TdxPDFRenderPageTaskClass; AViewer: TdxPDFDocumentCustomViewer;
  APageIndex: Integer; AFactor: Single): IdxTask;
begin
  Result := AClass.Create(Self, AViewer, APageIndex, AFactor);
end;

function TdxPDFBackgroundService.Cancel(ATaskHandle: THandle;
  AWaitFor: Boolean = False): Boolean;
begin
...
procedure TdxPDFBackgroundService.AddRenderingTask
  (AClass: TdxPDFRenderPageTaskClass; AViewer: TdxPDFDocumentCustomViewer;
  APageIndex: Integer; AFactor: Single);
var
  ATask: TdxPDFRenderPageTask;
begin
  ...
  begin
    ...
    ATask := CreateRenderingTask(AClass, AViewer, APageIndex, AFactor)
      as TdxPDFRenderPageTask;
    FTasks.Add(ATask);
    ATask.Handle := Run(ATask);
  end;
end;

procedure TdxPDFBackgroundService.Complete(const ATask: IdxTask;
    ...
    case AStatus of
      TdxTaskCompletedStatus.Success:
        if Assigned(OnRenderingComplete) then
          OnRenderingComplete(ARenderTask.PageIndex, ARenderTask.Factor,
            ARenderTask.Bitmap);
...
function TdxPDFDocumentViewerCacheStorage.CheckThumbnails(APageIndex: Integer;
  AFactor: Single): Boolean;
var
  ACache: TCacheItem;
begin
...
function TdxPDFDocumentViewerCacheStorage.Contains(APageIndex: Integer;
  AFactor: Single): Boolean;
var
  ACache: TCacheItem;
begin
...
function TdxPDFDocumentViewerCacheStorage.ContainsThumbnail(APageIndex: Integer;
  AFactor: Single): Boolean;
var
  ACache: TCacheItem;
begin
...
function TdxPDFDocumentViewerCacheStorage.CreateCache(ABitmap: TBitmap;
  AFactor: Single): TCacheItem;
var
  AClone: TBitmap;
begin
...
function TdxPDFDocumentViewerCacheStorage.GetBitmap
  (APageIndex: Integer): TBitmap;
var
  AInfo: TCacheItem;
begin
...
function TdxPDFDocumentViewerCacheStorage.GetThumbnail
  (APageIndex: Integer): TBitmap;
begin
...
procedure TdxPDFDocumentViewerCacheStorage.Add(APageIndex: Integer;
  AFactor: Single; ABitmap: TBitmap);
var
  AScale: Single;
begin
  AScale := 0;
  if FCacheInfos.TryGetValue(APageIndex, AScale) or SameValue(AFactor, AScale)
  then
    FContainer.AddOrSetValue(APageIndex, CreateCache(ABitmap, AFactor));
end;

procedure TdxPDFDocumentViewerCacheStorage.SetCacheVisiblePagesOnly
...
procedure TdxPDFDocumentViewerCacheStorage.AddThumbnail(APageIndex: Integer;
  ACache: TCacheItem);
var
  AClone: TBitmap;
begin
...
procedure TdxPDFDocumentViewerCacheStorage.Remove(APageIndex: Integer;
  AForceAddThumbnail: Boolean);
var
  ACache: TCacheItem;
begin
  if (AForceAddThumbnail or not(AForceAddThumbnail or CacheVisiblePagesOnly))
    and FContainer.TryGetValue(APageIndex, ACache) and not ACache.Bitmap.Empty
  then
    AddThumbnail(APageIndex, ACache);
  FContainer.Remove(APageIndex);
end;

procedure TdxPDFDocumentViewerCacheStorage.SetVisiblePages
  (APages: TList<TdxPDFDocumentViewerCachePageInfo>);
var
  AKeysToRemove: TIntegerDynArray;
  APageIndexes: TdxPDFIntegerList;
  APageIndex: Integer;
  APageInfo: TdxPDFDocumentViewerCachePageInfo;
begin
  FCacheInfos.Clear;
  APageIndexes := TdxPDFIntegerList.Create;
  try
...
constructor TdxPDFDocumentViewerCacheStorage.TCacheItem.Create(ABitmap: TBitmap;
  AFactor: Single);
begin
  inherited Create;
  Bitmap := ABitmap;
...
begin
  ...
  FQueue := TdxPDFIntegerList.Create;
...
function TdxPDFDocumentViewerCacheStorage.TCache.GetSize
  (AValue: TBitmap): Int64;
var
  AStream: TStream;
begin
  Result := 0;
...
function TdxPDFDocumentViewerCacheStorage.TCache.TryGetValue
  (APageIndex: Integer; out ABitmap: TBitmap): Boolean;
var
  ACache: TCacheItem;
begin
...
function TdxPDFDocumentViewerCacheStorage.TCache.TryGetValue
  (APageIndex: Integer; out ACache: TCacheItem): Boolean;
begin
  Result := FDictionary.TryGetValue(APageIndex, ACache);
end;

function TdxPDFDocumentViewerCacheStorage.TCache.TryGetValue
  (APageIndex: Integer; AFactor: Single; out ACache: TCacheItem): Boolean;
begin
  Result := FDictionary.TryGetValue(APageIndex, ACache);
  if Result then
...
class function TdxPDFRenderTaskInfo.Create(APageIndex: Integer; AFactor: Single)
  : TdxPDFRenderTaskInfo;
begin
...
constructor TdxPDFDocumentViewerCustomRenderer.Create
  (AViewer: TdxPDFDocumentCustomViewer);
begin
  inherited Create;
  FViewer := AViewer;
  CreateSubClasses;
...
procedure TdxPDFDocumentViewerCustomRenderer.DrawPage(ACanvas: TcxCanvas;
  APageIndex: Integer; const ARect: TRect);
begin
...
procedure TdxPDFDocumentViewerCustomRenderer.UpdateRenderQueue
  (AForce: Boolean = False);

  procedure CalculateAdditionalPageIndexes
    (APages: TDictionary<Integer, TdxPDFDocumentViewerCachePageInfo>;
    ...
    for I := 0 to ARows.Count - 1 do
      ...
      begin
        ...
        begin
          ...
          AddPageIndexes(APages, ARows[ARowIndex].StartIndex,
            ARows[ARowIndex].FinishIndex);
        end;
        Break;
      end;
  end;

  procedure CalculateVisiblePageIndexes
    (APages: TDictionary<Integer, TdxPDFDocumentViewerCachePageInfo>;
...
begin
  ...
  begin
    ...
    try
      ...
      if AForce or (FStartVisiblePageIndex <> AStartPageIndex) or
        (FEndVisiblePageIndex <> AEndPageIndex) then
      begin
        FStartVisiblePageIndex := AStartPageIndex;
        FEndVisiblePageIndex := AEndPageIndex;
        APageRowDelta := GetRenderingPageRowDelta;
        CalculateAdditionalPageIndexes(APages, FStartVisiblePageIndex,
          -APageRowDelta);
        CalculateAdditionalPageIndexes(APages, FEndVisiblePageIndex,
          APageRowDelta);
...
function TdxPDFDocumentViewerCustomRenderer.GetPreRenderPageScale
  (APageIndex: Integer): Single;
begin
  Result := Viewer.DocumentState.ScaleFactor.X;
end;

function TdxPDFDocumentViewerCustomRenderer.NeedProcessRenderTask
  (const ATask: TdxPDFRenderTaskInfo): Boolean;
begin
  Result := FViewer.FForceUpdate and not FViewer.IsDocumentZooming and
    not(FViewer.IsVertScrollAnimationActive or
    FViewer.IsHorzScrollAnimationActive);
end;

function TdxPDFDocumentViewerCustomRenderer.TryPackDocumentContent: Boolean;
begin
  Result := Viewer.TryPackDocumentContent;
end;

procedure TdxPDFDocumentViewerCustomRenderer.CancelRendering(const APageIndexes
  : TIntegerDynArray);
begin
  // do nothing
end;
...
procedure TdxPDFDocumentViewerCustomRenderer.DoDrawPage(ACanvas: TcxCanvas;
  APageIndex: Integer; const ARect: TRect);
...
begin
  ...
  begin
    ...
    begin
      Viewer.OnCustomDrawPreRenderPage(Viewer, ACanvas,
        GetPreRenderPageInfo(ABitmap), ADone);
      if not ADone then
        Draw(ABitmap);
    end
    else
...
procedure TdxPDFDocumentViewerCustomRenderer.Render(APageIndex: Integer;
  AFactor: Single);
var
  ABitmap: TBitmap;
  AImage: TdxSmartImage;
begin
  AImage := TdxSmartImage.Create;
  try
    if dxPDFDocumentExportToImageEx(Viewer.Document, APageIndex, AFactor,
      AImage, Viewer.RotationAngle) then
    begin
...
procedure TdxPDFDocumentViewerCustomRenderer.Add(APageIndex: Integer;
  AFactor: Single; ABitmap: TBitmap);
begin
...
procedure TdxPDFDocumentViewerCustomRenderer.SetCacheVisiblePagesOnly
  (const AValue: Boolean);
begin
  FStorage.CacheVisiblePagesOnly := AValue;
end;

function TdxPDFDocumentViewerCustomRenderer.CreatePageInfo(APageIndex: Integer)
  : TdxPDFDocumentViewerCachePageInfo;
begin
...
procedure TdxPDFDocumentViewerCustomRenderer.AddPageIndex
  (APages: TDictionary<Integer, TdxPDFDocumentViewerCachePageInfo>;
  APageIndex: Integer);
begin
...
procedure TdxPDFDocumentViewerCustomRenderer.AddPageIndexes
  (APages: TDictionary<Integer, TdxPDFDocumentViewerCachePageInfo>;
...
procedure TdxPDFDocumentViewerCustomRenderer.AddPageIndexesToRenderQueue
  (APages: TList<TdxPDFDocumentViewerCachePageInfo>; AForce: Boolean);
var
  I: Integer;
...
procedure TdxPDFDocumentViewerCustomRenderer.OnCancelRenderingHandler
  (Sender: TObject; const APageIndexes: TIntegerDynArray);
begin
...
constructor TdxPDFDocumentViewerAsyncRenderer.Create
  (AViewer: TdxPDFDocumentCustomViewer);
begin
...
begin
  Result := (FActiveTaskCount < FMaxActiveTaskCount) and
    inherited CanRenderNextPage;
end;

function TdxPDFDocumentViewerAsyncRenderer.NeedProcessRenderTask
  (const ATask: TdxPDFRenderTaskInfo): Boolean;
begin
  Result := inherited NeedProcessRenderTask(ATask) or
    not Storage.CheckThumbnails(ATask.PageIndex, ATask.Factor);
end;
...
procedure TdxPDFDocumentViewerAsyncRenderer.CancelRendering(const APageIndexes
  : TIntegerDynArray);
var
  I: Integer;
begin
...
procedure TdxPDFDocumentViewerAsyncRenderer.Render(APageIndex: Integer;
  AFactor: Single);
begin
  IncrementActiveTaskCount;
  TdxPDFBackgroundService(FService).AddRenderingByScaleFactorTask(Viewer,
    APageIndex, AFactor);
end;
...
procedure TdxPDFDocumentViewerAsyncRenderer.OnCanceledHandler
  (APageIndex: Integer; AFactor: Single);
begin
  DecrementActiveTaskCount;
end;

procedure TdxPDFDocumentViewerAsyncRenderer.OnCompleteHandler
  (APageIndex: Integer; AFactor: Single; ABitmap: TBitmap);
begin
  Add(APageIndex, AFactor, ABitmap);
...
constructor TdxPDFViewerDocumentState.Create(AOwner: TObject;
  AViewer: TdxPDFDocumentCustomViewer);
begin
...
function TdxPDFViewerDocumentState.ToDocumentRect
  (APage: TdxPDFDocumentCustomViewerPage; const R: TdxRectF): TdxRectF;
begin
  Result := APage.DocumentPage.FromUserSpace(R, DPI, ScaleFactor,
    dxRectF(APage.Bounds), Viewer.RotationAngle);
end;

function TdxPDFViewerDocumentState.ToViewerPoint
  (APage: TdxPDFDocumentCustomViewerPage; const P: TdxPointF): TdxPointF;
begin
  Result := APage.DocumentPage.ToUserSpace(P, DPI, ScaleFactor,
    dxRectF(APage.Bounds), Viewer.RotationAngle);
end;

function TdxPDFViewerDocumentState.ToViewerRect(const AObject
  : IdxPDFInteractiveObject): TRect;
var
...
function TdxPDFViewerDocumentState.ToViewerRect
  (APage: TdxPDFDocumentCustomViewerPage; const R: TdxRectF): TdxRectF;
begin
  Result := APage.DocumentPage.ToUserSpace(R, DPI, ScaleFactor,
    dxRectF(APage.Bounds), Viewer.RotationAngle);
end;

function TdxPDFViewerDocumentState.ToViewerRect
  (AHyperlink: TdxPDFHyperlink): TRect;
var
  AIntf: IdxPDFInteractiveObject;
begin
  if (AHyperlink <> nil) and Supports(AHyperlink, IdxPDFInteractiveObject, AIntf)
  then
...
function TdxPDFViewerDocumentState.CreateRenderParameters
...
function TdxPDFViewerDocumentState.ToDocumentPoint
  (APage: TdxPDFDocumentCustomViewerPage; const P: TdxPointF): TdxPointF;
begin
  Result := APage.DocumentPage.FromUserSpace(P, DPI, ScaleFactor,
    dxRectF(APage.Bounds), Viewer.RotationAngle);
end;
...
function TdxPDFViewerDocumentState.GetViewerPage(APageIndex: Integer)
  : TdxPDFDocumentCustomViewerPage;
begin
...
function TdxPDFDocumentCustomViewer.CreateRenderer
  : TdxPDFDocumentViewerCustomRenderer;
begin
...
function TdxPDFDocumentCustomViewer.GetNearestPageIndex
...
begin
  ...
  begin
    ...
    ADistance := TdxPDFUtils.DistanceToRect(P, dxRectF(R.Left, R.Top, R.Right,
      R.Bottom));
...
function TdxPDFDocumentCustomViewer.TryPackDocumentContent: Boolean;
var
  I: Integer;
begin
  if not IsDestroying then
    for I := 0 to DocumentPages.Count - 1 do
      DocumentPages.Page[I].Pack;
  Result := True;
end;
...
procedure TdxPDFDocumentCustomViewer.SetRenderContentDelay
...
procedure TdxPDFDocumentCustomViewer.SetRenderContentInBackground
  (const AValue: Boolean);
begin
...
function TdxPDFDocumentCustomViewer.OnGetRenderFactorHandler
  (APageIndex: Integer): Single;
begin
  Result := GetPageRenderFactor(APageIndex);
end;

procedure TdxPDFDocumentCustomViewer.OnInteractiveFormFieldValueChangedHandler
  (Sender: TObject);
begin
  FForceUpdate := True;
...
procedure TdxPDFDocumentCustomViewer.OnRenderContentTimerHandler
  (Sender: TObject);
begin
  StopZoomTimer;
end;
...
procedure TdxPDFDocumentCustomViewer.TDefaultRenderer.DoDrawPage
  (ACanvas: TcxCanvas; APageIndex: Integer; const ARect: TRect);
var
...
begin
  ...
  begin
    ...
    cxBitBlt(ACanvas.Handle, ACache.Canvas.Handle, APage.Bounds,
      cxNullPoint, SRCCOPY);
  end
  else
...
procedure TdxPDFDocumentCustomViewer.TDefaultRenderer.Render
  (APageIndex: Integer; AFactor: Single);
begin
  Viewer.InvalidatePage(APageIndex);
end;
...
begin
  TdxPreviewPageContentCachePoolAccess(TdxCustomPreviewAccess(Viewer)
    .PagesContentCache).Clear;
end;
...
function TdxPDFDocumentPageThumbnailViewer.CreateRenderer
  : TdxPDFDocumentViewerCustomRenderer;
begin
  Result := TRenderer.Create(Self);
end;

function TdxPDFDocumentPageThumbnailViewer.GetPageRenderFactor
  (APageIndex: Integer): Single;
begin
...
begin
  inherited CreateSubClasses;
  FSelectedPages := TdxPDFIntegerList.Create;
end;
...
begin
  ...
  try
    ...
    InternalOptionsView := [povAutoHideScrollBars,
      povDefaultDrawPageBackground];
    InternalOptionsZoom := [];
    OptionsHint := [];
    Keys := [kArrows, kChars];
  finally
...
procedure TdxPDFDocumentPageThumbnailViewer.LookAndFeelChanged
...
procedure TdxPDFDocumentPageThumbnailViewer.DoContextPopup(MousePos: TPoint;
  var Handled: Boolean);
...
procedure TdxPDFDocumentPageThumbnailViewer.DrawPageBackground
  (ACanvas: TcxCanvas; APage: TdxPreviewPage; ASelected: Boolean);
begin
  // do nothing
end;

procedure TdxPDFDocumentPageThumbnailViewer.DrawViewerBackground
  (ACanvas: TcxCanvas; const R: TRect);
begin
  LookAndFeelPainter.PDFViewerDrawPageThumbnailPreviewBackground(ACanvas, R);
end;

procedure TdxPDFDocumentPageThumbnailViewer.KeyDown(var Key: Word;
  Shift: TShiftState);
var
  I: Integer;
begin
...
procedure TdxPDFDocumentPageThumbnailViewer.MouseDown(Button: TMouseButton;
...
procedure TdxPDFDocumentPageThumbnailViewer.ProcessLeftClickByPage
  (Shift: TShiftState; X, Y: Integer);
var
  APageIndex: Integer;
...
function TdxPDFDocumentPageThumbnailViewer.IsPageSelected
...
procedure TdxPDFDocumentPageThumbnailViewer.SetDocument
  (const AValue: TdxPDFViewerDocument);
begin
  ...
  begin
    ...
    if Document <> nil then
    begin
      TdxPDFDocumentAccess(Document).State.OnRotationAngleChanged :=
        OnRotationAngleChangedHandler;
...
procedure TdxPDFDocumentPageThumbnailViewer.Changed(Sender: TObject;
  AFont: TFont);
begin
  Font.Assign(AFont);
...
procedure TdxPDFDocumentPageThumbnailViewer.RecreatePages
  (AForce: Boolean = True);
var
  I: Integer;
begin
...
procedure TdxPDFDocumentPageThumbnailViewer.OnRotationAngleChangedHandler
  (Sender: TObject);
begin
  RecreatePages;
end;
...
begin
  ...
  PageSize.MinUsefulSize := cxPoint(ThumbnailsPreview.Size,
    ThumbnailsPreview.Size);
...
procedure TdxPDFDocumentPageThumbnailViewer.TPage.CalculateLayout;
  ...
  begin
    Result.cx := Bounds.Width - ThumbnailsPreview.ScaleFactor.Apply
      (DefaultIndent * 2) - Max(ThumbnailsPreview.PageBorders.Left,
      ThumbnailsPreview.PageBorders.Top);
    Result.cy := Bounds.Height - ThumbnailsPreview.ScaleFactor.Apply
      (DefaultIndent * 2) - ThumbnailsPreview.PageCaptionAreaHeight -
      Max(ThumbnailsPreview.PageBorders.Right,
      ThumbnailsPreview.PageBorders.Bottom);
  end;
  ...
  begin
    ...
    AScale := Min(AMaximumSize.cx / Max(1, APageSize.X),
      AMaximumSize.cy / Max(1, APageSize.Y));
...
begin
  ...
  FImageBounds.Top := Bounds.Top +
    (Bounds.Height - APageSize.Y - ThumbnailsPreview.PageCaptionSize.cy) div 2;
  FImageBounds.Width := APageSize.X;
  FImageBounds.Height := APageSize.Y;

  FImageBorderBounds := cxRectInflate(FImageBounds,
    ThumbnailsPreview.PageBorders);

  ASelectionSize := GetSelectionSize;
  FSelectionBounds := cxRectInflate(FImageBounds, ASelectionSize);

  FCaptionBounds.Left := Bounds.Left +
    (Bounds.Width - ThumbnailsPreview.PageCaptionSize.cx) div 2;
  FCaptionBounds.Top := FSelectionBounds.Bottom +
    ThumbnailsPreview.PageCaptionMargin div 2;
  FCaptionBounds.Width := ThumbnailsPreview.PageCaptionSize.cx;
  FCaptionBounds.Height := ThumbnailsPreview.PageCaptionSize.cy;
end;

procedure TdxPDFDocumentPageThumbnailViewer.TPage.DrawBackground
  (ACanvas: TcxCanvas);
begin
  ThumbnailsPreview.LookAndFeel.Painter.DrawPrintPreviewPageBackground(ACanvas,
    FImageBorderBounds, FImageBounds, False, False);
end;

procedure TdxPDFDocumentPageThumbnailViewer.TPage.DrawContent
  (ACanvas: TcxCanvas);
begin
  ThumbnailsPreview.Renderer.DrawPage(ACanvas, Index, FImageBounds);
end;

procedure TdxPDFDocumentPageThumbnailViewer.TPage.DrawFocusRect
  (ACanvas: TcxCanvas);
begin
  if Selected then
...
procedure TdxPDFDocumentPageThumbnailViewer.TPage.DrawSelection
  (ACanvas: TcxCanvas);
begin
  if ThumbnailsPreview.IsPageSelected(Index) then
    dxGpFillRect(ACanvas.Handle, FSelectionBounds,
      ThumbnailsPreview.LookAndFeel.Painter.DefaultSelectionColor, 200);
end;

procedure TdxPDFDocumentPageThumbnailViewer.TPage.DrawIndex(ACanvas: TcxCanvas);
begin
  ACanvas.Font.Assign(ThumbnailsPreview.Font);
  ACanvas.Font.Color := cxGetActualColor
    (ThumbnailsPreview.LookAndFeelPainter.GetWindowContentTextColor,
    clWindowText);
  cxDrawText(ACanvas, IntToStr(Index + 1), FCaptionBounds,
...
begin
  Result := ThumbnailsPreview.ScaleFactor.Apply
    (cxRect(DefaultIndent, DefaultIndent, DefaultIndent, DefaultIndent));
end;

function TdxPDFDocumentPageThumbnailViewer.TPage.GetThumbnailsPreview
  : TdxPDFDocumentPageThumbnailViewer;
begin
...
function TdxPDFDocumentPageThumbnailViewer.TRenderer.GetPreRenderPageScale
  (APageIndex: Integer): Single;
begin
  Result := Viewer.GetPageRenderFactor(APageIndex) / Viewer.Document.PageInfo
    [APageIndex].Size.X;
end;

function TdxPDFDocumentPageThumbnailViewer.TRenderer.
  GetRenderingPageRowDelta: Integer;
begin
  Result := 0;
end;
...
procedure TdxPDFDocumentPageThumbnailViewer.TRenderer.Render
  (APageIndex: Integer; AFactor: Single);
begin
  TdxPDFBackgroundService(FService).AddRenderingBySizeTask(Viewer,
    APageIndex, AFactor);
end;
...
function TdxPDFDocumentCustomViewerPage.GetPageSizeOptionsClass
  : TdxPreviewPageSizeOptionsClass;
begin
  Result := TdxPDFViewerPageSizeOptions;
end;

function TdxPDFDocumentCustomViewerPage.GetPageSize(const AScreenFactor
  : TdxPointF): TPoint;
var
  ASize: TdxPointF;
begin
  ASize := TdxPDFViewerViewState.CalculatePageSize(DocumentPage.Size,
    TdxPDFDocumentCustomViewer(Preview).RotationAngle);
  Result.X := Round(ASize.X * AScreenFactor.X);
  Result.Y := Round(ASize.Y * AScreenFactor.Y);
end;

function TdxPDFDocumentCustomViewerPage.ToDocumentPoint(const P: TdxPointF)
  : TdxPointF;
begin
  Result := DocumentState.ToDocumentPoint(Self, P);
end;

function TdxPDFDocumentCustomViewerPage.ToDocumentRect(const R: TdxRectF)
  : TdxRectF;
begin
  Result := DocumentState.ToDocumentRect(Self, R);
end;

function TdxPDFDocumentCustomViewerPage.ToViewerPoint(const P: TdxPointF)
  : TdxPointF;
begin
  Result := DocumentState.ToViewerPoint(Self, P);
end;

function TdxPDFDocumentCustomViewerPage.ToViewerRect(const R: TdxRectF)
  : TdxRectF;
begin
  Result := DocumentState.ToViewerRect(Self, R);
end;

function TdxPDFDocumentCustomViewerPage.GetDocumentState
  : TdxPDFViewerDocumentState;
begin
  Result := TdxPDFDocumentCustomViewer(Preview).DocumentState;
end;

procedure TdxPDFDocumentCustomViewerPage.SetDocumentPage
  (const AValue: TdxPDFPage);
begin
