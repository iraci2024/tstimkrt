...
uses
  ...
  TypInfo, Windows, Classes, SysUtils, Messages, Controls, Graphics, Math,
  Forms, StdCtrls, ImgList,
  dxCore, cxGraphics, cxGeometry, cxControls, cxClasses, dxBar, dxRibbon,
  dxRibbonSkins, dxRibbonBackstageView,
  dxCoreReg, dxBarReg, cxDesignWindows, cxComponentCollectionEditor,
...
type
  ...
  TdxRibbonBackstageViewGalleryControlComponentEditor = class
  ...
  public
    ...
    procedure SelectionsChanged(const ASelection: IDesignerSelections);
      override;
  end;
...
procedure Register;
begin
  RegisterComponents(dxBarProductPage, [TdxRibbonBackstageView,
    TdxRibbonBackstageViewGalleryControl]);
  RegisterNoIcon([TdxRibbonBackstageViewGalleryGroup,
    TdxRibbonBackstageViewGalleryItem]);

  RegisterComponentEditor(TdxRibbonBackstageView,
    TdxRibbonBackstageViewComponentEditor);
  RegisterComponentEditor(TdxRibbonBackstageViewTabSheet,
    TdxRibbonBackstageViewComponentEditor);
  RegisterComponentEditor(TdxRibbonBackstageViewGalleryControl,
    TdxRibbonBackstageViewGalleryControlComponentEditor);

  RegisterPropertyEditor(TypeInfo(TcxImageIndex),
    TdxRibbonBackstageViewTabSheet, 'ImageIndex',
    TdxRibbonBackstageViewTabSheetImageIndexProperty);
  RegisterPropertyEditor(GetPropInfo(TdxRibbonBackstageViewGalleryControl,
    'Gallery').PropType^, TdxRibbonBackstageViewGalleryControl, 'Gallery',
...
begin
  ...
  TdxRibbonCustomBackstageViewAccess(Sender).FDesignHelper :=
    TcxDesignHelper.Create(TComponent(Sender));
...
function TdxRibbonBackstageViewComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  case AIndex of
    0:
      Result := cxGetResourceString(@dxSBAR_RIBBONADDTAB);
    1:
      Result := cxGetResourceString(@dxSBAR_RIBBONDELETETAB);
  else
    Result := '';
  end;
end;
...
procedure TdxRibbonBackstageViewComponentEditor.InternalExecuteVerb
  (AIndex: Integer);
begin
  case AIndex of
    0:
...
procedure TdxRibbonBackstageViewComponentEditor.PrepareItem(Index: Integer;
...
function TdxRibbonBackstageViewComponentEditor.GetBackstageView
...
procedure TdxRibbonBackstageViewGalleryControlComponentEditor.
  InternalExecuteVerb(AIndex: Integer);
begin
  Edit;
end;

function TdxRibbonBackstageViewGalleryControlComponentEditor.InternalGetVerb
  (AIndex: Integer): string;
begin
  Result := 'Edit...';
end;

function TdxRibbonBackstageViewGalleryControlComponentEditor.
...
function TdxRibbonBackstageViewTabSheetImageIndexProperty.GetImages
  : TCustomImageList;
begin
...
procedure TdxRibbonBackstageViewDesignWindow.DoAddFreeNotification
  (AList: TList);
var
  I: Integer;
begin
...
procedure TdxRibbonBackstageViewDesignWindow.DoRemoveFreeNotification
  (AList: TList);
var
  I: Integer;
begin
...
begin
  for I := 0 to AList.Count - 1 do
  begin
    if Supports(TObject(AList[I]), IdxRibbonBackstageViewSelectableItem,
      ASelectableItem) then
...
procedure TdxRibbonBackstageViewDesignWindow.ItemDeleted(const ADesigner
  : IDesigner; Item: TPersistent);
begin
  FCurrentSelection.Remove(Item);
end;

procedure TdxRibbonBackstageViewDesignWindow.SelectionsChanged(const ASelection
  : IDesignerSelections);
var
