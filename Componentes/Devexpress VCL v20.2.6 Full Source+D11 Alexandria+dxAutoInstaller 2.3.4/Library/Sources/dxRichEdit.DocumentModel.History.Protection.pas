...
type
  ...
  TdxInsertRangePermissionHistoryItem = class
  ...
  TdxRemoveRangePermissionHistoryItem = class
    (TdxInsertDocumentIntervalHistoryItem)
  protected
    procedure RedoCore; override;
    procedure UndoCore; override;
  ...
  public
    ...
    property DeletedRangePermission: TdxRangePermission
      read FDeletedRangePermission write FDeletedRangePermission;
  end;
...
begin
  ...
  ARunInfo := PieceTable.ApplyDocumentPermissionCore(AStart, AEnd,
    IndexToInsert);
  ...
  PieceTable.ApplyChangesCore([TdxDocumentModelChangeAction.RaiseContentChanged,
...
begin
  AStart := Min(Position, PieceTable.DocumentEndLogPosition);
  AEnd := Min(Position + Length, PieceTable.DocumentEndLogPosition);
  ARunInfo := PieceTable.RemoveDocumentPermissionCore(AStart, AEnd,
    IndexToInsert);
  if ARunInfo = nil then
    Exit;
  try
...
begin
  ...
  begin
    ...
    if (APermission.Properties.Index = FDeletedRangePermission.Properties.Index)
      and (APermission.NormalizedStart = FDeletedRangePermission.
      NormalizedStart) and
  ...
  PieceTable.ApplyChangesCore([TdxDocumentModelChangeAction.RaiseContentChanged,
    ...
    FDeletedRangePermission.Interval.NormalizedStart.RunIndex,
    FDeletedRangePermission.Interval.NormalizedEnd.RunIndex);
end;
