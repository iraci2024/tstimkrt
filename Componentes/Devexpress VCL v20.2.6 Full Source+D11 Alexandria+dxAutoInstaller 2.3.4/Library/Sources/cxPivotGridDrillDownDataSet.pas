...
uses
  Classes, SysUtils, DB, cxClasses, dxmdaset, cxCustomPivotGrid,
...
type
  ...
  protected
    ...
    procedure DoAssignFieldProperties(AField: TField;
      APivotGridField: TcxPivotGridField); override;
...
begin
  ...
  begin
    ...
    for J := 0 to FDrillDownDataSource.FieldCount - 1 do
      Fields[J + 1 { RecID } ].Value := FDrillDownDataSource.Values
        [TcxDataRecordHandle(I), TcxDataItemHandle(J)];
    Post;
  end;
end;

procedure TcxCustomPivotGridDrillDownDataSet.DoAssignFieldProperties
  (AField: TField; APivotGridField: TcxPivotGridField);
begin
  inherited DoAssignFieldProperties(AField, APivotGridField);
  if IsDBFieldAssigned(APivotGridField) and
    (APivotGridField.GroupInterval = giDefault) and
    (APivotGridField.Properties = nil) then
...
begin
  FDrillDownDataSource := TcxPivotGridCrossCellDataSource
    (PivotGrid.CreateDrillDownDataSource);
  try
