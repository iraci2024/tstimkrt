...
uses
  ...
  cxLookAndFeelPainters, Menus, dxLayoutControlAdapters, StdCtrls, cxRadioGroup,
  dxLayoutContainer, cxButtons,
...
type
  TdxRichEditCustomInsertDeleteTableCellsDialogForm = class
  ...
  protected
    ...
    property Controller: TdxInsertDeleteTableCellsFormController
      read GetController;
  public
    property CellOperation: TdxTableCellOperation read GetCellOperation
      write SetCellOperation;
  end;
...
procedure TdxRichEditCustomInsertDeleteTableCellsDialogForm.btnOkClick
  (Sender: TObject);
begin
...
procedure TdxRichEditCustomInsertDeleteTableCellsDialogForm.
...
procedure TdxRichEditCustomInsertDeleteTableCellsDialogForm.CellOperationChanged
  (Sender: TObject);
begin
  Controller.CellOperation := CellOperation;
end;

function TdxRichEditCustomInsertDeleteTableCellsDialogForm.GetCellOperation
  : TdxTableCellOperation;
begin
  if rbCellOperationShiftLeft.Checked then
    Result := TdxTableCellOperation.ShiftToTheHorizontally
  else if rbCellOperationShiftUp.Checked then
    Result := TdxTableCellOperation.ShiftToTheVertically
  else if rbCellOperationDeleteRow.Checked then
    Result := TdxTableCellOperation.RowOperation
  else if rbCellOperationDeleteColumn.Checked then
    Result := TdxTableCellOperation.ColumnOperation
  else
    Result := TdxTableCellOperation.ShiftToTheHorizontally;
end;

function TdxRichEditCustomInsertDeleteTableCellsDialogForm.GetController
...
procedure TdxRichEditCustomInsertDeleteTableCellsDialogForm.
  CellOperationDblClick(Sender: TObject);
begin
  Controller.ApplyChanges;
  ModalResult := mrOk;
end;

procedure TdxRichEditCustomInsertDeleteTableCellsDialogForm.SetCellOperation
  (const Value: TdxTableCellOperation);
begin
  case Value of
