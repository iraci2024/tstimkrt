...
type
  ...
  TdxPSGetComponentOption = (gcoExcludeExisting, gcoExcludeOutOfActiveForm,
  ...
  public
    ...
    procedure GetComponents(AComponentPrinter: TdxComponentPrinter;
      ...
      AnOptions: TdxPSGetComponentOptions); virtual; abstract;
  ...
  public
    ...
    property ActiveProvider: TAbstractdxPSComponentsProvider
      read GetActiveProvider;
    property Count: Integer read GetCount;
    property Providers[Index: Integer]: TAbstractdxPSComponentsProvider
      read GetProvider; default;
  end;

function dxPSCreateComponentItem(AComponent: TComponent;
  const ACaption, ADescription: string): TdxComponentItem;
...
constructor TdxComponentItem.Create(AComponent: TComponent;
  const ACaption, ADescription: string);
begin
  inherited Create;
...
class function TdxPSComponentProvidersFactory.Instance
  : TdxPSComponentProvidersFactory;
begin
  Result := inherited Instance as TdxPSComponentProvidersFactory;
end;

procedure TdxPSComponentProvidersFactory.GetComponents(AComponentPrinter
  : TdxComponentPrinter; AReportLink: TBasedxReportLink; AComponents: TStrings;
  AnOptions: TdxPSGetComponentOptions);
begin
  if ActiveProvider <> nil then
    ActiveProvider.GetComponents(AComponentPrinter, AReportLink, AComponents,
      AnOptions);
end;

function TdxPSComponentProvidersFactory.IndexOf(AProviderClass
  : TdxPSComponentsProviderClass): Integer;
begin
...
procedure TdxPSComponentProvidersFactory.RegisterProvider(AProviderClass
  : TdxPSComponentsProviderClass);
begin
  if IndexOf(AProviderClass) = -1 then
    FItems.Add(AProviderClass.Create);
end;

procedure TdxPSComponentProvidersFactory.UnregisterProvider(AProviderClass
  : TdxPSComponentsProviderClass);
var
  Index: Integer;
begin
...
function TdxPSComponentProvidersFactory.GetActiveProvider
...
function TdxPSComponentProvidersFactory.GetProvider(Index: Integer)
  : TAbstractdxPSComponentsProvider;
begin
