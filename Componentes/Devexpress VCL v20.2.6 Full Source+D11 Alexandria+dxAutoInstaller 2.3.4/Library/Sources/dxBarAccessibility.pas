...
uses
  Types, Windows, Forms, Classes, SysUtils, dxCore, cxAccessibility, cxClasses,
  dxBar;
...
type
  ...
  protected
    ...
    procedure DoClickItem(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper); virtual;
    ...
    procedure InitializeItemKeyTipPosition(AItemLinkHelper
      ...
      var AKeyTipInfo: TdxBarKeyTipInfo); virtual;
    function GetItemScreenBounds(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper): TRect;
    function GetNextAccessibleObject(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper;
      ADirection: TcxAccessibilityNavigationDirection)
      : IdxBarAccessibilityHelper; overload; virtual;
  ...
  TdxBarControlAccessibilityHelper = class
    (TCustomdxBarControlAccessibilityHelper)
  private
  ...
  protected
    ...
    procedure GetCaptionButtonKeyTipPosition(ACaptionButton
      ...
      out AVertAlign: TcxAlignmentVert); virtual;
    function GetNextAccessibleObject(AItemLinkHelper
      : TdxBarItemLinkAccessibilityHelper;
      ADirection: TcxAccessibilityNavigationDirection)
  ...
  TdxBarSubMenuControlAccessibilityHelper = class
    (TCustomdxBarControlAccessibilityHelper)
  private
  ...
  TdxBarItemControlAccessibilityHelper = class
    (TdxBarItemLinkAccessibilityHelper)
  private
  ...
  TdxBarButtonLikeControlAccessibilityHelper = class
    (TdxBarItemControlAccessibilityHelper)
  protected
    procedure DoClick; override;
    procedure DoDropDown; override;
  ...
  TdxBarButtonControlAccessibilityHelper = class
    (TdxBarButtonLikeControlAccessibilityHelper)
  private
  ...
  TdxBarSubItemControlAccessibilityHelper = class
    (TdxBarButtonLikeControlAccessibilityHelper)
  protected
    function IsDropDownControl: Boolean; override;
    function ShowDropDownWindow: Boolean; override;
  ...
  TdxBarWinControlAccessibilityHelper = class
    (TdxBarItemControlAccessibilityHelper)
  protected
    procedure KeyTipHandler(Sender: TObject); override;
    function NeedsVisibilityForKeyTipHandling: Boolean; override;
...
function TdxDockControlAccessibilityHelper.GetChild(AIndex: Integer)
...
begin
  ...
  Result := DockControl.Rows[I].Cols[AIndex]
...
begin
  ...
  for I := 0 to DockControl.RowCount - 1 do
    for J := 0 to DockControl.Rows[I].ColCount - 1 do
      if DockControl.Rows[I].Cols[J].BarControl.IAccessibilityHelper.GetHelper = AChild
      then
      begin
        Result := AIndex;
        Break;
...
function TCustomdxBarControlAccessibilityHelper.GetBarControl
  : TCustomdxBarControl;
begin
...
function TCustomdxBarControlAccessibilityHelper.CanNavigateToChildren
  (AKey: Word): Boolean;
var
...
begin
  ...
  Result := not((ABarControlInstance.SelectedControl is TdxBarWinControl) and
    TdxBarWinControlAccess(ABarControlInstance.SelectedControl)
    .NeedFocusOnClick and TdxBarWinControl
    (ABarControlInstance.SelectedControl).Focused);
end;

procedure TCustomdxBarControlAccessibilityHelper.DoClickItem(AItemLinkHelper
  : TdxBarItemLinkAccessibilityHelper);
begin
  //
end;

procedure TCustomdxBarControlAccessibilityHelper.DoDropDownItem(AItemLinkHelper
  : TdxBarItemLinkAccessibilityHelper);
begin
  //
end;

function TCustomdxBarControlAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
  Result := BarControl.ViewInfo.ItemControlViewInfos[AIndex]
    .Control.IAccessibilityHelper.GetHelper;
end;
...
function TCustomdxBarControlAccessibilityHelper.Expand
  : TCustomdxBarControlAccessibilityHelper;
begin
...
function TCustomdxBarControlAccessibilityHelper.GetItemScreenBounds
  (AItemLinkHelper: TdxBarItemLinkAccessibilityHelper): TRect;
begin
...
begin
  ...
  try
    GetChildrenForNavigation(AItemLinkHelper, Self,
      GetItemScreenBounds(AItemLinkHelper), ADirection, True, AObjects);
...
begin
  if TdxBarManagerAccess(BarControl.BarManager).FocusedBarControl = BarControl
  then
...
function TCustomdxBarControlAccessibilityHelper.LogicalNavigationGetNextChild
  (AChildIndex: Integer; AShift: TShiftState): TdxBarAccessibilityHelper;
const
  NavigationDirection: array [Boolean, Boolean]
    of TcxAccessibilityNavigationDirection = ((andRight, andLeft),
    (andUp, andDown));
var
...
begin
  ...
  begin
    ANextBarControl := TdxBarControlAccess(BarControl)
      .GetNextBarControl(not(ssShift in AShift));
    ...
    Result := TdxBarAccessibilityHelperAccess(Childs[AChildIndex])
      .GetNextAccessibleObject(NavigationDirection[BarControl.IsRealVertical,
      ssShift in AShift]).GetBarHelper;
end;

{ TdxBarControlAccessibilityHelper }

// IdxBarAccessibilityHelper
function TdxBarControlAccessibilityHelper.GetDefaultAccessibleObject
  : IdxBarAccessibilityHelper;
begin
  Result := Self;
end;

function TdxBarControlAccessibilityHelper.HandleNavigationKey
...
procedure TdxBarControlAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
...
function TdxBarControlAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
...
begin
  if (BarControl.Bar <> nil) and
    (AChild = BarControl.Bar.CaptionButtons.IAccessibilityHelper.GetHelper) then
    Result := inherited GetChildCount
  else
...
begin
  ...
  begin
    if ARelativeDirection = andLeft then
      AItemLink := TdxBarItemLinksAccess(BarControl.ItemLinks)
        .Prev(AItemLinkHelper.ItemLink)
    else
      AItemLink := TdxBarItemLinksAccess(BarControl.ItemLinks)
        .Next(AItemLinkHelper.ItemLink);
...
function TdxBarControlMarkAccessibilityHelper.HandleNavigationKey
  (var AKey: Word): Boolean;
begin
  Result := inherited HandleNavigationKey(AKey);
  if not Result then
...
procedure TdxBarControlMarkAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
  inherited Unselect(ANextSelectedObject);
  TdxBarControlAccess(BarControl).InvalidateMark;
...
procedure TdxBarCaptionButtonsAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
  inherited Unselect(ANextSelectedObject);
...
function TdxBarCaptionButtonsAccessibilityHelper.GetChild(AIndex: Integer)
  : TcxAccessibilityHelper;
begin
  Result := CaptionButtons[CaptionButtons.Count - 1 - AIndex]
...
function TdxBarCaptionButtonsAccessibilityHelper.GetParent
  : TcxAccessibilityHelper;
begin
...
begin
  ...
  if (Result and cxSTATE_SYSTEM_INVISIBLE = 0) and
    (IsRectEmpty(CaptionButtons.Rect) or TCustomdxBarControlAccessibilityHelper
    (Parent).IsCollapsed) then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;
...
function TdxBarCaptionButtonsAccessibilityHelper.GetCaptionButtons
...
begin
  ...
  case AKey of
    VK_ESCAPE:
      begin
        Unselect(nil);
        TdxBarControlAccess(CaptionButton.Collection.Bar.Control)
          .HideAllByEscape;
      end;
    VK_RETURN, VK_SPACE:
      begin
...
procedure TdxBarCaptionButtonAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
  inherited Unselect(ANextSelectedObject);
  TdxBarCaptionButtonsAccessibilityHelper(Parent).UpdateCaptionButtons;
...
function TdxBarCaptionButtonAccessibilityHelper.GetParent
  : TcxAccessibilityHelper;
begin
  Result := TdxBarCaptionButtons(CaptionButton.Collection)
    .IAccessibilityHelper.GetHelper;
end;
...
begin
  Result := Parent.States[cxAccessibleObjectSelfID];
  if (Result and cxSTATE_SYSTEM_INVISIBLE = 0) and
    IsRectEmpty(CaptionButton.Rect) then
    Result := Result or cxSTATE_SYSTEM_INVISIBLE;
end;
...
procedure TdxBarCaptionButtonAccessibilityHelper.GetKeyTipInfo(out AKeyTipInfo
  : TdxBarKeyTipInfo);
var
  ABasePoint: TPoint;
begin
  ...
  begin
    ...
    TdxBarControlAccessibilityHelper(Parent.Parent)
      .GetCaptionButtonKeyTipPosition(CaptionButton, ABasePoint.Y,
      AKeyTipInfo.VertAlign);
...
begin
  TdxBarControlAccess(CaptionButton.Collection.Bar.Control)
    .CaptionButtonClick(CaptionButton.Index);
end;

function TdxBarCaptionButtonAccessibilityHelper.GetCaptionButton
  : TdxBarCaptionButton;
begin
...
begin
  ...
  begin
    ...
    else if ((AKey = VK_DOWN) or (AKey = VK_TAB) and (AShift = [])) and
      (ABarControl.SelectedControl <> nil) then
    begin
...
begin
  ...
  if ABarControl.UseRightToLeftAlignment then
    AKeyTipInfo.BasePoint.X := ABarControl.ClientOrigin.X +
      ABarControl.BarRect.Left - ABarControl.BandSize div 2
  else
    AKeyTipInfo.BasePoint.X := ABarControl.ClientOrigin.X +
      ABarControl.BarRect.Right + ABarControl.BandSize div 2;
...
function TdxBarSubMenuControlAccessibilityHelper.GetParentForKeyTip
  : TdxBarAccessibilityHelper;
begin
...
procedure TdxBarSubMenuControlAccessibilityHelper.HandleHorzNavigationKey
  (ALeftKey: Boolean);
const
  ANavigationDirectionMap: array [Boolean]
    of TcxAccessibilityNavigationDirection = (andRight, andLeft);
var
  ABarControl: TdxBarSubMenuControlAccess;
...
begin
  ...
  begin
    ...
    if (ANextObject <> nil) and
      (ANextObject.GetBarHelper <> ASelectedObject.GetBarHelper) then
    begin
  ...
  begin
    ...
    if (AParentBarControl <> nil) and (AParentBarControl.SelectedControl <> nil)
    then
      if ABarControl.ParentBar is TdxBarSubMenuControl then
        TdxBarItemControlAccess(AParentBarControl.SelectedControl)
          .ControlInactivate(True)
      else
      begin
        AParentBarControl := TCustomdxBarControlAccess
          (GetParentBarForBar(ABarControl));
        if AParentBarControl <> nil then
          ...
          else if not(bboMouseCantUnselectNavigationItem
            in AParentBarControl.BehaviorOptions) then
            BarNavigationController.HandleKey(VK_LEFT, [],
              AParentBarControl.IAccessibilityHelper,
              AParentBarControl.SelectedControl.IAccessibilityHelper);
      end;
  end
  else
  begin
    AParentBarControl := TCustomdxBarControlAccess
      (GetParentBarForBar(ABarControl));
    if (AParentBarControl <> nil) and not(bboMouseCantUnselectNavigationItem
      in AParentBarControl.BehaviorOptions) then
      BarNavigationController.HandleKey(VK_RIGHT, [],
        AParentBarControl.IAccessibilityHelper,
        AParentBarControl.SelectedControl.IAccessibilityHelper);
  end;
end;
...
begin
  ...
  if ANextSelectedObject <> nil then
    BarNavigationController.ChangeSelectedObject(AFocusItemControl,
      ANextSelectedObject, APrevSelectedObject);
end;
...
function TdxBarSubMenuControlAccessibilityHelper.GetBarControl
...
function TdxBarItemAccessibilityHelper.GetDefaultKeyTip: string;
  ...
  begin
    Result := (AShortCut >= 16449 { Ctrl+A } ) and
      (AShortCut <= 16474 { Ctrl+Z } )
  end;
...
begin
  Result := Supports(Item, IdxBarLinksOwner, AItemLinksOwner) and
    (AItemLinksOwner.GetItemLinks <> nil);
end;
...
function TdxBarItemLinkAccessibilityHelper.GetNextAccessibleObject
  (ADirection: TcxAccessibilityNavigationDirection): IdxBarAccessibilityHelper;
begin
  Result := TCustomdxBarControlAccessibilityHelper(Parent)
    .GetNextAccessibleObject(Self, ADirection);
end;
...
procedure TdxBarItemLinkAccessibilityHelper.GetKeyTipInfo(out AKeyTipInfo
  : TdxBarKeyTipInfo);
begin
  inherited;

  (Parent as TCustomdxBarControlAccessibilityHelper)
    .InitializeItemKeyTipPosition(Self, AKeyTipInfo);
  AKeyTipInfo.Enabled := Item.Enabled;
end;

function TdxBarItemLinkAccessibilityHelper.GetParent: TcxAccessibilityHelper;
begin
  if (ItemLink.BarControl <> nil) and
    (ItemLink.BarControl.IAccessibilityHelper <> nil) then
...
begin
  if ItemControl.Parent.IsActive then
    TCustomdxBarControlAccess(ItemControl.Parent)
      .SetKeySelectedItem(ItemControl)
  else
    ItemControl.Parent.BarGetFocus(ItemControl);

  if ItemControl.HasWindow and
    (ASetFocus or not TdxBarItemControlAccess(ItemControl)
    .AllowSelectWithoutFocusing) then
    TdxBarItemControlAccess(ItemControl).Click(False);
end;

procedure TdxBarItemControlAccessibilityHelper.Unselect(ANextSelectedObject
  : IdxBarAccessibilityHelper);
begin
  inherited Unselect(ANextSelectedObject);
...
begin
  Result := cxSTATE_SYSTEM_NORMAL;
  AHandle := GetOwnerObjectWindow;
  if (AHandle = 0) or not IsWindowVisible(AHandle) or
...
procedure TdxBarItemControlAccessibilityHelper.GetKeyTipInfo(out AKeyTipInfo
  : TdxBarKeyTipInfo);
begin
...
function TdxBarItemControlAccessibilityHelper.
  NeedsVisibilityForKeyTipHandling: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  begin
    ...
    TdxBarItemControlAccessibilityHelper
      (AExpandedBarControlObject.BarControl.ViewInfo.ItemControlViewInfos
      [AIndex].Control.IAccessibilityHelper.GetHelper).KeyTipHandler(Sender);
  end
  else
...
function TdxBarButtonLikeControlAccessibilityHelper.
  NeedsVisibilityForKeyTipHandling: Boolean;
begin
...
begin
  Result := ButtonControl.ButtonItem.ButtonStyle
    in [bsDropDown, bsCheckedDropDown];
end;
...
function TdxBarButtonControlAccessibilityHelper.GetButtonControl
  : TdxBarButtonControl;
begin
...
function TdxBarWinControlAccessibilityHelper.
  NeedsVisibilityForKeyTipHandling: Boolean;
begin
  Result := True;
end;
