...
uses
  Types, Variants, Windows, Classes, Controls, DB, Graphics, Messages, SysUtils,
  Forms,
...
type
  ...
  protected
    ...
    procedure DBLookupGridCheckColumnByFieldName(const AFieldName
      : string); override;
    procedure DBLookupGridCreateColumnsByFieldNames(const AFieldNames
      : string); override;
    ...
    function GetDBLookupGridColumnIndexByFieldName(const AFieldName: string)
      : Integer; override;
  ...
  published
    ...
    property AutoSearchOnPopup: Boolean read FAutoSearchOnPopup
      write FAutoSearchOnPopup default True;
    property AutoSelect;
    ...
    property FocusPopup: Boolean read FFocusPopup write FFocusPopup
      default False;
    ...
    property ListFieldItem: TcxCustomGridTableItem read GetListFieldItem
      write SetListFieldItem;
    property MaxLength;
    property OEMConvert;
...
procedure TcxExtLookupGrid.ViewKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  begin
    BeginUpdate;
    try
      AutoSearchOnPopup := TcxExtLookupComboBoxProperties(Source)
        .AutoSearchOnPopup;
...
class function TcxExtLookupComboBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TcxExtLookupComboBox;
end;

class function TcxExtLookupComboBoxProperties.IsViewSupported
  (Value: TcxCustomGridTableView): Boolean;
begin
...
procedure TcxExtLookupComboBoxProperties.LinkView
  (AView: TcxCustomGridTableView);
begin
  TcxCustomGridTableViewAccess(AView).HighlightIncrementalFilteringText :=
    ifoHighlightSearchText in IncrementalFilteringOptions;
  CheckListFieldItem;
...
procedure TcxExtLookupComboBoxProperties.UnlinkView
  (AView: TcxCustomGridTableView);
begin
  TcxCustomGridTableViewAccess(AView).HighlightIncrementalFilteringText
    := False;
...
function TcxExtLookupComboBoxProperties.GetLookupGridDataController
...
function TcxExtLookupComboBoxProperties.
...
function TcxExtLookupComboBoxProperties.GetLookupGridNearestPopupHeight
...
function TcxExtLookupComboBoxProperties.GetLookupGridPopupHeight
...
function TcxExtLookupComboBoxProperties.IsLookupGridMouseOverList
...
begin
  ...
  begin
    ...
    begin
      TcxGridTableView(View).OptionsCustomize.ColumnFiltering :=
        FPrevColumnFiltering;
      TcxGridTableView(View).OptionsCustomize.ColumnsQuickCustomization :=
        FPrevColumnsQuickCustomization;
    end;
...
procedure TcxExtLookupComboBoxProperties.LookupGridDroppedDown
...
procedure TcxExtLookupComboBoxProperties.LookupGridInitEvents(AOnClick,
...
begin
  if View = nil then
    Exit;
  // save options
  FPrevPullFocusing := TcxCustomGridTableOptionsBehaviorAccess
    (View.OptionsBehavior).PullFocusing;
  TcxCustomGridTableOptionsBehaviorAccess(View.OptionsBehavior)
    .PullFocusing := True;
  ...
  if View is TcxGridTableView then
  begin
    FPrevColumnFiltering := TcxGridTableView(View)
      .OptionsCustomize.ColumnFiltering;
    FPrevColumnsQuickCustomization := TcxGridTableView(View)
...
procedure TcxExtLookupComboBoxProperties.LookupGridInitLookAndFeel
...
procedure TcxExtLookupComboBoxProperties.DBLookupGridCheckColumnByFieldName
  (const AFieldName: string);
begin
  if (View <> nil) and (DataController <> nil) then
  begin
    if (AFieldName <> '') and (DataController.GetItemByFieldName(AFieldName)
      = nil) then
...
procedure TcxExtLookupComboBoxProperties.DBLookupGridCreateColumnsByFieldNames
    ...
    try
      ...
      try
        for I := 0 to AFieldNamesList.Count - 1 do
          DataController.ChangeFieldName(View.CreateItem.Index,
            AFieldNamesList[I]);
      finally
        View.EndUpdate;
      end;
...
function TcxExtLookupComboBoxProperties.GetDBLookupGridColumnField
...
function TcxExtLookupComboBoxProperties.GetDBLookupGridColumnFieldName
...
function TcxExtLookupComboBoxProperties.GetDBLookupGridColumnIndexByFieldName
...
begin
  ...
  begin
    AItem := TcxCustomGridTableItem
      (DataController.GetItemByFieldName(AFieldName));
    if AItem <> nil then
      Result := AItem.Index;
  end;
end;

function TcxExtLookupComboBoxProperties.GetDBLookupGridDataController
  : TcxDBDataController;
begin
...
function TcxExtLookupComboBoxProperties.GetListFieldItem
...
procedure TcxExtLookupComboBoxProperties.SetIncrementalFilteringOptions
  (Value: TcxTextEditIncrementalFilteringOptions);
begin
  inherited SetIncrementalFilteringOptions(Value);
  if View <> nil then
    TcxCustomGridTableViewAccess(View).HighlightIncrementalFilteringText :=
      ifoHighlightSearchText in Value;
end;

procedure TcxExtLookupComboBoxProperties.SetListFieldItem
...
begin
  if (Value <> nil) and not IsViewSupported(Value) then
    Exit;
  if FView <> Value then
  begin
    if FView <> nil then
...
class function TcxCustomExtLookupComboBox.GetPropertiesClass
...
function TcxCustomExtLookupComboBox.GetActiveProperties
  : TcxExtLookupComboBoxProperties;
begin
  Result := TcxExtLookupComboBoxProperties(InternalGetActiveProperties);
end;

function TcxCustomExtLookupComboBox.GetProperties
  : TcxExtLookupComboBoxProperties;
begin
...
procedure TcxCustomExtLookupComboBox.SetProperties
  (Value: TcxExtLookupComboBoxProperties);
begin
...
class function TcxDBExtLookupComboBox.GetDataBindingClass
...
procedure TcxDBExtLookupComboBox.SetDataBinding
  (Value: TcxDBTextEditDataBinding);
begin
...
class function TcxEditRepositoryExtLookupComboBoxItem.GetEditPropertiesClass
  : TcxCustomEditPropertiesClass;
begin
  Result := TcxExtLookupComboBoxProperties;
end;

function TcxEditRepositoryExtLookupComboBoxItem.GetProperties
  : TcxExtLookupComboBoxProperties;
begin
  Result := inherited Properties as TcxExtLookupComboBoxProperties;
end;

procedure TcxEditRepositoryExtLookupComboBoxItem.SetProperties
  (Value: TcxExtLookupComboBoxProperties);
begin
