...
function TdxSpellCheckerDictionaryTypeProperty.GetAttributes
  : TPropertyAttributes;
begin
...
begin
  ...
  begin
    if GetComponent(I) is TdxSpellCheckerDictionaryItem then
      TdxSpellCheckerDictionaryItem(GetComponent(I)).DictionaryTypeClass :=
        ADictionaryTypeClass;
  end;
...
constructor TdxSpellCheckerCodePageProperty.Create(const ADesigner: IDesigner;
  APropCount: Integer);
begin
...
begin
  if AIndex = 0 then
    ShowCollectionEditor(Designer, Component, (Component as TdxSpellChecker)
...
begin
  ...
  RegisterComponentEditor(TdxCustomSpellChecker,
    TdxSpellCheckerComponentEditor);
  RegisterSelectionEditor(TdxCustomSpellChecker,
    TdxSpellCheckerSelectionEditor);
  ...
  RegisterPropertyEditor(TypeInfo(TdxCustomSpellCheckerDictionary),
    TdxSpellCheckerDictionaryItem, 'DictionaryType',
    TdxSpellCheckerDictionaryTypeProperty);
  RegisterPropertyEditor(TypeInfo(TComponent),
    TdxSpellCheckerCheckAsYouTypeOptions, 'PopupMenu',
    TcxControlPopupMenuProperty);

  // dictionary property editors
  // TdxCustomSpellCheckerDictionary
  RegisterPropertyEditor(TypeInfo(DWORD), TdxCustomSpellCheckerDictionary,
    'Language', TdxSpellCheckerLanguageProperty);
  RegisterPropertyEditor(TypeInfo(Cardinal), TdxCustomSpellCheckerDictionary,
    'CodePage', TdxSpellCheckerCodePageProperty);
  // TdxAffixCompressionDictionary
  RegisterPropertyEditor(TypeInfo(TFileName), TdxAffixCompressionDictionary,
    'DictionaryPath', TcxFileNameProperty);
  RegisterPropertyEditor(TypeInfo(TFileName), TdxAffixCompressionDictionary,
    'GrammarPath', TcxFileNameProperty);
  // TdxUserSpellCheckerDictionary
  RegisterPropertyEditor(TypeInfo(TFileName), TdxUserSpellCheckerDictionary,
    'DictionaryPath', TcxFileNameProperty);
  // TdxHunspellDictionary
  RegisterPropertyEditor(TypeInfo(TFileName), TdxHunspellDictionary,
    'DictionaryPath', TcxFileNameProperty);
  RegisterPropertyEditor(TypeInfo(TFileName), TdxHunspellDictionary,
    'GrammarPath', TcxFileNameProperty);
end;
