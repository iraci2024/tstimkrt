...
type
  ...
  TdxLockUpdateEvent = procedure(Sender: TdxBaseObject;
    ALockState: TdxLockState) of object;
  ...
  protected
    ...
    property OnLockUpdate: TdxLockUpdateEvent read FOnLockUpdate
      write FOnLockUpdate;
  public
  ...
  public
    ...
    procedure Register(AnIndex: Integer; AClass: TPersistentClass);
      overload; virtual;
    procedure Unregister(AClass: TPersistentClass); virtual;

    property Items[Index: Integer]: TPersistentClass read GetItem
  ...
  protected
    ...
    property PairClasses[AClass: TClass]: TdxCustomClassMapItemClass
      read GetPairClass;
  end;
...
function TdxClassList.Add(AClass: TClass;
  ACheckExistence: Boolean = True): Integer;
begin
...
procedure TdxClassList.Insert(Index: Integer; AClass: TClass;
  ACheckExistence: Boolean = True);
begin
...
function TdxPersistentClassList.Extract(AClass: TPersistentClass)
  : TPersistentClass;
begin
...
procedure TdxPersistentClassList.Register(AnIndex: Integer;
...
procedure TdxPersistentClassList.SetItem(Index: Integer;
...
begin
  for Result := 0 to Count - 1 do
    if Items[Result].ClassType = AClass then
      Exit;
  Result := -1;
end;
...
function TdxCustomClassMaps.GetPairClass(AClass: TClass)
  : TdxCustomClassMapItemClass;
var
  ...
  if AClass <> nil then
    ...
    begin
      ...
      if AClass.InheritsFrom(Result.PairClass) and
        ((Candidate = nil) or Result.PairClass.InheritsFrom(Candidate.PairClass))
      then
        Candidate := Result;
