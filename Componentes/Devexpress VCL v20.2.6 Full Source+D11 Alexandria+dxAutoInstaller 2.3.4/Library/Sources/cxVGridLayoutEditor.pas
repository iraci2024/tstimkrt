...
uses
  ...
  dxCore, cxButtons, cxVGrid, cxLookAndFeelPainters, cxGraphics, cxLookAndFeels,
  cxControls, dxLayoutControlAdapters,
...
begin
  ...
  with AForm do
    try
      ...
      FVerticalGrid := TcxCustomVerticalGridClass(AVerticalGrid.ClassType)
        .Create(nil);
      try
    ...
    try
      ...
      begin
        ...
        if ASourceRow.Parent = nil then
          ...
          ADestRow.Parent := ADestRows.FindRowByID
            (TcxCustomRowAccess(ASourceRow.Parent).ID);
...
begin
  ...
  else if VerticalGrid is TcxVirtualVerticalGrid then
