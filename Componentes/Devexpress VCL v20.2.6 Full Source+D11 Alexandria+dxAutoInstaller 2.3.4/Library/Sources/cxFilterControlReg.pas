...
begin
  ...
  try
    Designer.GetComponentNames
      (GetTypeData(PTypeInfo(TcxDBFilterControl.ClassInfo)), AddComponent);
...
begin
  for I := 0 to GetRegisteredEditProperties.Count - 1 do
    if IsSupportFiltering(TcxCustomEditPropertiesClass
      (GetRegisteredEditProperties[I])) then
...
begin
  Result := TcxDBFilterControl(TcxFilterItem(GetComponent(0)).FilterControl)
    .DataSource;
end;
...
begin
  ...
  RegisterComponentEditor(TcxCustomFilterControl,
  ...
  RegisterPropertyEditor(TypeInfo(string), TcxFilterItem, 'FieldName',
    TcxFilterControlItemFieldNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxFilterItem,
  ...
  RegisterSelectionEditor(TcxDBFilterControl,
    TcxDBFilterControlSelectionEditor);
end;
