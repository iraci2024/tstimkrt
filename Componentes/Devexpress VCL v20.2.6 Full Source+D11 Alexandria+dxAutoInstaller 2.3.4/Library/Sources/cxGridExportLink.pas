...
{$ENDIF}
  ...
  cxClasses, cxGraphics, cxStyles, cxLookAndFeels, cxLookAndFeelPainters,
  cxCustomData, cxDataStorage, cxDataUtils,
  cxDBData, dxLayoutContainer, cxVariants, cxExport, cxEdit, cxImage,
  cxCalendar, cxCurrencyEdit, cxSpinEdit, cxCalc,
  cxTimeEdit, cxMaskEdit, cxGrid, cxGridCustomView, cxGridCustomTableView,
  cxGridTableView, cxGridBandedTableView,
  cxGridCardView, cxGridLevel, cxGridRows, cxGridStrs, cxGridDBDataDefinitions,
  cxGridCommon, cxGridChartView, dxCoreClasses,
  cxGridLayoutView, cxGridWinExplorerView, Contnrs, Generics.Defaults,
  Generics.Collections, cxContainer, dxSparkline,
  dxSpreadSheetConditionalFormatting, cxDataControllerConditionalFormatting,
  dxSpreadSheetTypes,
...
type
  ...
  public
    ...
    procedure Commit(AProgressHelper: TcxCustomProgressCalculationHelper;
      AHandler: TObject);
  ...
  IcxGridExportProviderAdapter = interface
    ...
    function SetCellGraphic(const AGlobalArea: TRect; AStyleIndex: Integer;
      AGraphic: TGraphic; AFitMode: TcxImageFitMode = ifmStretch): TObject;
    procedure SetCellGraphicAsSharedHandle(const AGlobalArea: TRect;
      AStyleIndex: Integer; AHandle: TObject;
      AFitMode: TcxImageFitMode = ifmStretch);
    procedure SetCellStyle(const AArea: TRect;
      const AStyleIndex: Integer); overload;
    procedure SetCellStyle(const AGlobalCol, AGlobalRow,
      AStyleIndex: Integer); overload;
    procedure SetCellValue(const AGlobalCol, AGlobalRow: Integer;
      const AValue: Variant; const AValueDisplayFormat: string;
      AValueDisplayFormatType: TcxValueDisplayFormatType);
    procedure SetCellValueAsFormula(const ACol, ARow: Integer;
      const AValue: string; ADisplayText: string = ''; AFormatCode: string = '';
      AListSeparator: Char = ',');
    ...
    procedure RealBoundsToLogicalBoundsEx(const ABounds, ASearchArea: TRect;
      out ALogicalBounds: TRect);
    procedure RealRangeToLogicalRangeHorz(X1, X2: Integer;
      out ALogicalX1, ALogicalX2: Integer);
    procedure RealRangeToLogicalRangeVert(Y1, Y2: Integer;
      out ALogicalY1, ALogicalY2: Integer);
  end;

  { TcxGridExportProviderAdapter }

  TcxGridExportProviderAdapter = class(TInterfacedObject,
    IcxGridExportProviderAdapter)
  strict private
    FBoundingRect: TRect;
  ...
  public
    ...
    constructor CreateParented(AParentAdapter: IcxGridExportProviderAdapter;
      const AClientOrigin: TPoint);
    ...
    procedure SetCellValue(const AGlobalCol, AGlobalRow: Integer;
      const AValue: Variant; const AValueDisplayFormat: string;
      AValueDisplayFormatType: TcxValueDisplayFormatType);
    procedure SetCellValueAsFormula(const AGlobalCol, AGlobalRow: Integer;
      const AValue: string; ADisplayText: string = ''; AFormatCode: string = '';
      AListSeparator: Char = ',');
    procedure SetDefaultStyle(const AStyle: TcxCacheCellStyle);
  ...
  type
    ...
    TCanExpandRecordFunc = function(ARecord: TcxCustomGridRecord)
      : Boolean of object;
  strict private
    ...
    procedure DoExpandRecords(AGridView: TcxCustomGridTableView;
  ...
  protected
    ...
    function CalculateViewViewInfo(AGridView: TcxCustomGridView; ABounds: TRect)
      : TcxCustomGridViewInfo; virtual;
    ...
    function CanUseNativeFormatProperties(AProperties
      : TcxCustomEditProperties): Boolean;
    ...
    procedure FillRealArea(const ABounds: TRect; AStyleIndex: Integer;
      ABorderColor: TColor = clDefault; ABorders: TcxBorders = cxBordersAll);
    function GetContentParams(ARecord: TcxCustomGridRecord;
    ...
    function GetViewItemValue(ARecord: TcxCustomGridRecord;
      AItem: TcxCustomGridTableItem): Variant;
    function GetViewItemValueEx(ARecord: TcxCustomGridRecord;
      AItem: TcxCustomGridTableItem; out AValueDisplayFormat: string;
      out AValueDisplayFormatType: TcxValueDisplayFormatType): Variant; virtual;
    function IsCurrencyItem(AItem: TcxCustomGridTableItem): Boolean;
    function IsCurrencyProperties(AProperties: TcxCustomEditProperties)
      : Boolean;
    function IsEmpty: Boolean;
    function IsNativeFormatProperties(AProperties: TcxCustomEditProperties;
      AItem: TcxCustomGridTableItem): Boolean; virtual;
    function NeedProcessRecord(ARecord: TcxCustomGridRecord): Boolean; virtual;
    procedure RealBoundsToLogicalBounds(const ABounds: TRect;
      out ALogicalBounds: TRect);
    procedure RealBoundsToLogicalBoundsEx(const ABounds, ASearchArea: TRect;
      out ALogicalBounds: TRect);
    function RegisterContentParams(ARecord: TcxCustomGridRecord;
      AItem: TcxCustomGridTableItem; out AParams: TcxViewParams): Integer;
    function RegisterSolidStyle(AStyleIndex: Integer;
      AData: TObject = nil): Integer;
    function RegisterSolidStyleEx(AColor: TColor): Integer;
    procedure RegisterStyles; virtual;
    function RegisterViewParams(const AViewParams: TcxViewParams;
      const AAlignment: TAlignment = taLeftJustify): Integer;

    procedure SetRealCellStyle(const ARealBounds, ASearchArea: TRect;
      AStyleIndex: Integer);
    procedure SetRealCellStyleAndFormula(const ARealBounds, ASearchArea: TRect;
      AStyleIndex: Integer; const AFormula: string; ADisplayText: string = '';
    ...
    procedure SetRealCellStyleAndValue(const ARealBounds, ASearchArea: TRect;
      ...
      const AValueDisplayFormat: string = '');
    procedure SetRealCellStyleAndValueEx(const ARealBounds, ASearchArea: TRect;
      AStyleIndex: Integer; ARecord: TcxCustomGridRecord;
      AItem: TcxCustomGridTableItem);
    function TrySetCellValueAsGraphic(const ABounds, AArea: TRect;
      AStyleIndex: Integer; ARecord: TcxCustomGridRecord;
    ...
    function TextWidthEx(const AViewParams: TcxViewParams;
    ...
    procedure ViewParamsToExportStyle(const AViewParams: TcxViewParams;
      ...
      const AAlignment: TAlignment = taLeftJustify; ABorders: TcxBorders = [];
    ...
    property DefaultStyle: TcxCacheCellStyle read FDefaultStyle
      write FDefaultStyle;
    property DefaultStyleIndex: Integer read FDefaultStyleIndex
      write FDefaultStyleIndex;
  ...
  public
    constructor Create(const AFileName: string; AExportType: Integer;
      AGridView: TcxCustomGridView; AGrid: TcxCustomGrid;
      AViewInfo: TcxCustomGridViewInfo; AHandler: TObject = nil);
      overload; virtual;
    constructor Create(const AFileName: string; AGridView: TcxCustomGridView;
      AGrid: TcxCustomGrid; AViewInfo: TcxCustomGridViewInfo;
      AAdapter: IcxGridExportProviderAdapter; AHandler: TObject = nil);
      overload; virtual;
    constructor CreateFrom(AMasterExport: TcxGridCustomExport;
      AGridView: TcxCustomGridView; AViewInfo: TcxCustomGridViewInfo;
      const AClientOrigin: TPoint); virtual;
    destructor Destroy; override;
  ...
  public
    ...
    property DetailExport: TcxGridCustomExport read FDetailExport
      write FDetailExport;
    property DetailViewViewInfo: TcxCustomGridViewInfo read FDetailViewViewInfo
      write FDetailViewViewInfo;
    property Hidden: Boolean read FHidden write FHidden;
    property IsBackground: Boolean read FIsBackground write FIsBackground;
    property ItemBorderColor: TColor read FItemBorderColor
      write FItemBorderColor;
  ...
  protected
    function AddVisualDataItem(const AItemBounds: TRect; AStyle: Integer;
      ARecord: TcxCustomGridRecord; AGridItem: TcxCustomGridTableItem)
      : TcxExportVisualItem;
    function AddVisualItem(AItemClass: TcxExportVisualItemClass;
      const ABounds: TRect): TcxExportVisualItem; virtual;
    function AddVisualItemEx(const AItemBounds: TRect;
      const ADisplayText: string; const AViewParams: TcxViewParams;
      AAlignment: TAlignment; ABorders: TcxBorders;
      ABorderColor: TColor = clDefault; AIsBackground: Boolean = False;
      AWordWrap: Boolean = True): TcxExportVisualItem; overload;
    function AddVisualItemEx(const AItemBounds: TRect;
      const ADisplayText: string; AStyle: Integer;
    ...
    procedure ExportVisualItem(AItem: TcxExportVisualItem;
      const ASearchArea: TRect); virtual;
    procedure ExportDataVisualItem(AItem: TcxExportVisualItem;
      const ASearchArea: TRect); virtual;
  ...
  public
    ...
    property VisualItems[Index: Integer]: TcxExportVisualItem
      read GetVisualItem;
  ...
  protected
    ...
    procedure AddDataRow(var ATop, ALeft: Integer;
      ARow: TcxGridDataRow); virtual;
    procedure AddFooterCell(ABounds: TRect; AColumn: TcxGridColumn;
      AFooterItem: TcxDataSummaryItem; AValue: Variant;
      const AParams: TcxViewParams); virtual;
    procedure AddFooterCells(var ATop, ALeft: Integer); virtual;
    procedure AddFooterCellsByColumn(ABounds: TRect; AColumn: TcxGridColumn;
      APatternIndex: Integer; out ACount: Integer); virtual;
    procedure AddGroupFooterCells(var ATop, ALeft: Integer;
      ARow: TcxCustomGridRow); virtual;
    procedure AddGroupFooterCellsByColumn(ABounds: TRect;
      ARow: TcxCustomGridRow; AColumn: TcxGridColumn; APatternIndex: Integer;
      out ACount: Integer); virtual;
    procedure AddGroupRow(var ATop, ALeft: Integer;
      ARow: TcxGridGroupRow); virtual;
    function AddIndents(ATop, ARowHeight: Integer; ARow: TcxCustomGridRecord;
      AHasButton: Boolean; ALevel: Integer = -1): Integer; virtual;
    procedure AddMasterDataRow(var ATop, ALeft: Integer;
      ARow: TcxGridMasterDataRow); virtual;
    procedure AddRowFooter(var ATop, ALeft: Integer; ARow: TcxCustomGridRecord;
      ALevel: Integer); virtual;
    procedure AddRowFooters(var ATop, ALeft: Integer;
      ARow: TcxCustomGridRecord); virtual;
    procedure AddRowPreview(var ATop, ALeft: Integer;
      ARow: TcxCustomGridRecord); virtual;
    function AddPattern(const ABounds: TRect; AData: TObject;
      AOffset: Integer = 0): TcxExportVisualItem;
    function AddVisualItem(AItemClass: TcxExportVisualItemClass;
      const ABounds: TRect): TcxExportVisualItem; override;
    function CalculateFooterCellCount(AItems: TcxDataSummaryItems)
      : Integer; virtual;
    ...
    procedure CreateRecordFromPattern(var ATop, ALeft: Integer;
      ARecord: TcxCustomGridRecord);
    function DoMergeCell(AMasterItem, ASlaveItem: TcxExportVisualItem): Boolean;
    procedure ExportDataVisualItem(AItem: TcxExportVisualItem;
      const ASearchArea: TRect); override;
    ...
    function GetFooterItemBounds(AIndex, ALineIndex: Integer;
      const AOrigin: TRect; AIsRowFooter: Boolean = False): TRect;
    ...
    function GetGroupRowColumnIntersection(const ARowBounds: TRect;
      AColumn: TcxCustomGridTableItem): TRect;
    function GetIsSummaryUnderColumns(ARow: TcxGridGroupRow): Boolean; virtual;
    function GetPatternParams(ARecord: TcxCustomGridRecord;
      AItem: TcxExportVisualItem): TcxViewParams; virtual;
    ...
    function HasMasterRowSelectedChildren(AMasterRow
      : TcxGridMasterDataRow): Boolean;
    function HasMasterViewSelectedChildren(AView: TcxGridTableView): Boolean;
    procedure Initialize; override;
    function IsColumnContainsFooterItem(AItem: TcxDataSummaryItem;
    ...
    procedure ProcessGroupSummaryItem(ARow: TcxGridGroupRow; AValues: PVariant;
      AIndex: Integer; const ABounds: TRect;
      const ARowViewParams: TcxViewParams);
    ...
    property GroupSummaryItems[Index: Integer]: TcxExportGroupSummaryItem
      read GetGroupSummaryItem;
  ...
  public
    ...
    property IsSummaryUnderColumns[ARow: TcxGridGroupRow]: Boolean
      read GetIsSummaryUnderColumns;
  ...
  protected
    procedure CreateBandHeaders(var ATop: Integer;
      AForRootBands: Boolean); virtual;
    ...
    function GetPatternParams(ARecord: TcxCustomGridRecord;
      AItem: TcxExportVisualItem): TcxViewParams; override;
    function ProduceColumnsContainer(AContainer: TcxGridColumnContainerViewInfo;
      ATop, ALeft: Integer): Integer;
  public
  ...
  protected
    ...
    function AddRow(ALayerIndex: Integer; ACardViewInfo: TcxGridCardViewInfo;
  ...
  public
    ...
    destructor Destroy; override;
    procedure AddToScales(AAdapter: IcxGridExportProviderAdapter);
    procedure CalculateLayersCaptionWidth(AWidths: TcxExportIntList;
      AFistRowInLayerOnly: Boolean);
    function GetRowCaptionWidth(ALayerIndex, ARowIndex: Integer;
      AMaxWidth: Integer = 0): Integer;
    procedure SetLayersCaptionWidth(AWidths: TcxExportIntList;
      AFistRowInLayerOnly: Boolean);
    ...
    property Rows[ALayerIndex, ARowIndex: Integer]: TcxExportCardRow
      read GetRow;
    property SeparatorWidth: Integer read GetSeparatorWidth;
  end;
  ...
  protected
    ...
    function AddExportCard(AColumnPosition, ARowPosition: Integer;
      ACard: TcxGridCard): TcxExportCard;
    procedure AdjustRowCaptionWidth;
    procedure CalculateVisibleInfo;
    function CreateCardLayoutBuilder: TcxExportCardLayoutBuilder;
    procedure DoCreateExportCells; override;
    procedure DoExportCells; override;
    procedure ExportCardRow(ACard: TcxExportCard; ARow: TcxExportCardRow;
      const ACardLogicalBounds: TRect);
    procedure Finalize; override;
  ...
  public
    ...
    property CategoryIndent: Integer read GetCategoryIndent;
    property CategorySeparatorStyleIndex: Integer
      read FCategorySeparatorStyleIndex;
    property CategorySeparatorWidth: Integer read GetCategorySeparatorWidth;
    property ColumnCardCount: Integer read FColumnCardCount
      write FColumnCardCount;
    ...
    property LayoutDirection: TcxGridCardViewLayoutDirection
      read GetLayoutDirection;
  ...
  protected
    procedure AddRectangle(const ABounds: TRect;
    ...
    procedure ExportEditItem(const ABounds: TRect;
      AInfo: TcxGridLayoutViewItemViewInfo);
  ...
  private
    ...
    function GetRecord(ARecordIndex: Integer)
      : TcxGridWinExplorerViewCustomRecord;
  protected
    procedure AddExportDataCell(AViewInfo
      : TcxGridWinExplorerViewCustomCellViewInfo);
    procedure AddExportDataRecord(AViewInfo
      : TcxGridWinExplorerViewRecordViewInfo);
    procedure AddExportGroupRecord(AViewInfo
      : TcxGridWinExplorerViewGroupRecordViewInfo);
    procedure AddExportRecord(ARecord: TcxGridWinExplorerViewCustomRecord;
      ARecordViewInfo: TcxCustomGridRecordViewInfo);
    procedure DoCreateExportCells; override;
  public
    property GridView: TcxGridWinExplorerView read GetGridView;
    property Records[ARecordIndex: Integer]: TcxGridWinExplorerViewCustomRecord
      read GetRecord;
  end;

  { TcxGridChartViewExport }

  TcxGridChartViewExportEnumCellsProc = reference to procedure(AColumn,
  ...
  protected
    FData: TDictionary<TdxSpreadSheetCustomConditionalFormattingRule,
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure CellAdded(ASourceRow, ASourceColumn: Integer;
      const ATargetArea: TRect);
    procedure Commit(AExportProvider: IcxExportProvider);
    procedure Initialize(AGridView: TcxCustomGridView;
      AProvider: TcxDataControllerConditionalFormattingProvider);
  end;

  { TcxGridDataConditionalFormattingReferencesFormatter }

  TcxGridDataConditionalFormattingReferencesFormatter = class
  ...
  protected
    ...
    property ConditionalFormattingRules: TcxGridDataConditionalFormattingRules
      read FConditionalFormattingRules;
  ...
  protected
    ...
    function CreateExportCell(const ABounds: TRect;
      AClass: TcxExportCustomItemClass): TcxExportCustomItem; overload;
    function CreateExportCell(const ABounds: TRect; const ADisplayText: string;
      AClass: TcxExportTextItemClass): TcxExportTextItem; overload;
    procedure DoCreateExportCells; override;
    procedure DoExportCells; override;
  ...
  protected
    procedure AddFrom(ARanges: TcxGridDataExportRanges; AIndex: Integer;
      AOffset: Integer);
    function CreateRange(AStart: Integer): TcxGridDataExportRange;
    procedure FinishRange(AFinish: Integer); overload;
    procedure FinishRange(ARange: TcxGridDataExportRange;
      AFinish: Integer); overload;
  ...
  public
    ...
    property Items[Index: Integer]: TcxGridDataExportRange read GetItem
  ...
  private
    ...
    function GetItem(AIndex: Integer)
      : TcxGridDataExportFooterSubTotalFormulaRanges; inline;
    procedure SetItem(AIndex: Integer;
      AObject: TcxGridDataExportFooterSubTotalFormulaRanges); inline;
  protected
    function AddRanges(AParentRow: TcxCustomGridRecord)
      : TcxGridDataExportFooterSubTotalFormulaRanges;
    function Build(AParentRow: TcxCustomGridRecord; AColumnIndex: Integer;
      AKind: TcxSummaryKind): string;
    function FindRanges(AParentRow: TcxCustomGridRecord)
      : TcxGridDataExportFooterSubTotalFormulaRanges;
    procedure FinishRange(AFinish: Integer);
    function GetSubTotalDescription(AKind: TcxSummaryKind): string;
    function HasActiveRange: Boolean;
    function IsParentForRanges(AParent: TcxCustomGridRecord;
  ...
  public
    ...
    property Items[Index: Integer]: TcxGridDataExportFooterSubTotalFormulaRanges
  ...
  strict private
    function GetItem(AIndex: Integer)
      : TcxGridDataExportOutlineGroupRange; inline;
    procedure SetItem(AIndex: Integer;
      AObject: TcxGridDataExportOutlineGroupRange); inline;
  protected
    ...
    procedure StartLevelRange(AIndex: Integer; AStart: Integer;
      ALevel: Integer); overload;
  public
    function First: TcxGridDataExportOutlineGroupRange; inline;
    function Last: TcxGridDataExportOutlineGroupRange; inline;

    property Items[Index: Integer]: TcxGridDataExportOutlineGroupRange
      read GetItem write SetItem; default;
  end;
  ...
  public
    destructor Destroy; override;
    procedure DestroyDetailExport;
    procedure UpdateBoundByDetailBounds(ALeftOffset,
      ATopOffset: Integer); virtual;

    property DetailExport: TcxGridViewDataCustomExport read GetDetailExport
      write SetDetailExport;
  end;
  ...
  protected
    ...
    procedure CreateExportCell(const ABounds: TRect; const ADisplayText: string;
      ARecord: TcxCustomGridRecord; AData: TObject = nil); reintroduce;
      overload;
    function CreateExportCell(const ABounds: TRect; const ADisplayText: string;
      ARecord: TcxCustomGridRecord; AData: TObject;
      AClass: TcxExportGridViewDataItemClass): TcxExportGridViewDataItem;
      reintroduce; overload; virtual;
    procedure CreateExportContent(var ALeft, ATop: Integer); virtual;
    procedure CreateExportDataRecord(var ALeft, ATop: Integer;
      ARecord: TcxCustomGridRecord); virtual;
    procedure CreateExportGroupRecord(var ALeft, ATop: Integer;
      ARecord: TcxCustomGridRecord); virtual;
    procedure CreateExportRecord(var ALeft, ATop: Integer;
      ARecord: TcxCustomGridRecord); virtual;
    procedure CreateExportRecords(var ALeft, ATop: Integer); virtual;
    procedure CreateExportHeader(var ALeft, ATop: Integer); virtual;
    procedure CreateIndents(AStartLeft, AFinishLeft, AStartTop,
      AFinishTop: Integer); virtual;
    procedure DoCreateExportCells; override;
    ...
    property OutlineGroupRanges: TcxGridDataExportOutlineGroupRanges
      read FOutlineGroupRanges;
  public
    constructor Create(const AFileName: string; AGridView: TcxCustomGridView;
      AGrid: TcxCustomGrid; AViewInfo: TcxCustomGridViewInfo;
      AAdapter: IcxGridExportProviderAdapter; AHandler: TObject = nil);
  ...
  private
    FFooterSubTotalFormulaBuilder
      : TcxGridDataExportFooterSubTotalFormulaBuilder;

    function GetGridView: TcxGridTableView;
  protected
    procedure AddDetailOutlineGroupRanges(ADetailExport: TcxGridViewDataExport;
      ATop: Integer); virtual;
    procedure AddOutlineGroups; override;
    function CalculateBounds(ALeftOffset, ATopOffset: Integer): TRect; override;
    procedure CreateExportContent(var ALeft, ATop: Integer); override;
    procedure CreateExportDataRecord(var ALeft: Integer; var ATop: Integer;
      ARecord: TcxCustomGridRecord); override;
    procedure CreateExportFooterByColumn(ALeft, ATop: Integer;
      AColumn: TcxGridColumn); virtual;
    procedure CreateExportFooters(var ALeft, ATop: Integer); virtual;
    procedure CreateExportFormulaCell(const ABounds: TRect;
      const AFormula, ADisplayText: string; AData: TObject); virtual;
    procedure CreateExportGroupFooterByColumn(ALeft, ATop: Integer;
      ARow: TcxCustomGridRecord; AColumn: TcxGridColumn); virtual;
    procedure CreateExportGroupRecord(var ALeft, ATop: Integer;
      ARecord: TcxCustomGridRecord); override;
    procedure CreateExportMasterDataRow(var ALeft, ATop: Integer;
      ARecord: TcxGridMasterDataRow); virtual;
    procedure CreateExportRecord(var ALeft, ATop: Integer;
      ARecord: TcxCustomGridRecord); override;
    procedure CreateExportRowFooter(var ALeft, ATop: Integer;
      ADataLevel, ALevel: Integer; ARow: TcxCustomGridRecord); virtual;
    procedure CreateExportRowFooters(var ALeft, ATop: Integer;
      ARow: TcxCustomGridRecord); virtual;
    procedure ExportCell(ACell: TcxExportCustomItem; ARect: TRect); override;
    ...
    function GetGroupDisplayText(ARecord: TcxCustomGridRecord): string;
      override;
    function GetGroupFooterLineCount(ARow: TcxCustomGridRecord)
      : Integer; virtual;
    function HasFooter: Boolean; virtual;
    function HasHeader: Boolean; override;

    property FooterSubTotalFormulaBuilder
      : TcxGridDataExportFooterSubTotalFormulaBuilder
      read FFooterSubTotalFormulaBuilder;
  public
    constructor Create(const AFileName: string; AGridView: TcxCustomGridView;
      AGrid: TcxCustomGrid; AViewInfo: TcxCustomGridViewInfo;
      AAdapter: IcxGridExportProviderAdapter; AHandler: TObject = nil);
  ...
  protected
    function CalculateBounds(ALeftOffset, ATopOffset: Integer): TRect; override;
    procedure CreateExportCategories(ASeries: TcxGridChartSeries;
      AHasLeftIndent, AHasTopIndent: Boolean); virtual;
...
procedure ExportGridToHTML(const AFileName: string; AGrid: TcxGrid;
  ...
  const AFileExt: string = 'html'; AHandler: TObject = nil);
procedure ExportGridToXML(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const AFileExt: string = 'xml'; AHandler: TObject = nil);

procedure ExportGridToCSV(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const ASeparator: Char = ','; const AFileExt: string = 'csv';
  AHandler: TObject = nil; AEncoding: TEncoding = nil);

procedure ExportGridToText(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const AFileExt: string = 'txt'; AHandler: TObject = nil;
  AEncoding: TEncoding = nil); overload;
procedure ExportGridToText(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean; ASaveAll: Boolean; const ASeparator, ABeginString,
  AEndString: string; const AFileExt: string = 'txt'; AHandler: TObject = nil;
  AEncoding: TEncoding = nil); overload;
...
procedure ExportGridToXLSX(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  AUseNativeFormat: Boolean = True; const AFileExt: string = 'xlsx';
  AHandler: TObject = nil);
...
procedure ExportGridDataToHTML(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const AFileExt: string = 'html'; AHandler: TObject = nil);
procedure ExportGridDataToXML(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const AFileExt: string = 'xml'; AHandler: TObject = nil);

procedure ExportGridDataToCSV(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const ASeparator: Char = ','; const AFileExt: string = 'csv';
  AHandler: TObject = nil; AEncoding: TEncoding = nil);

procedure ExportGridDataToText(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  const AFileExt: string = 'txt'; AHandler: TObject = nil;
  AEncoding: TEncoding = nil); overload;
procedure ExportGridDataToText(const AFileName: string; AGrid: TcxGrid;
...
procedure ExportGridDataToExcel(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  AUseNativeFormat: Boolean = True; const AFileExt: string = 'xls';
  AHandler: TObject = nil);
procedure ExportGridDataToXLSX(const AFileName: string; AGrid: TcxGrid;
  AExpand: Boolean = True; ASaveAll: Boolean = True;
  AUseNativeFormat: Boolean = True; const AFileExt: string = 'xlsx';
  AHandler: TObject = nil);

procedure ExportGridDataToFile(AFileName: string; AExportType: Integer;
  AGrid: TcxGrid; AExpand, ASaveAll, AUseNativeFormat: Boolean;
  const ASeparator, ABeginString, AEndString: string; const AFileExt: string;
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
...
uses
  Math, RTLConsts, Dialogs, cxGridCustomLayoutView, cxGridViewLayoutContainer,
  cxImageComboBox,
  cxTextEdit, cxLookupEdit, dxSpreadSheetUtils, dxDPIAwareUtils,
  dxSpreadSheetCoreStrs, dxFormattedLabel,
  dxFormattedTextConverterRTF, dxCoreGraphics, cxExportProviders;

const
  AlignmentToExportAlignment: array [TAlignment] of TcxAlignText = (catLeft,
    catRight, catCenter);

  GridLinesToBorders: array [TcxGridLines] of TcxBorders = (cxBordersAll, [],
    [bRight, bLeft], [bTop, bBottom]);
...
type
  ...
  public
    ...
    class procedure InitStartCapacity(AList: TcxObjectList;
      AItemCount, ARecordCount: Integer; AForDetailView: Boolean); static;
  end;

class procedure TcxGridViewExportVisualItemListHelper.CheckCapacity
  (AList: TcxObjectList);
begin
...
class procedure TcxGridViewExportVisualItemListHelper.InitStartCapacity
  (AList: TcxObjectList; AItemCount, ARecordCount: Integer;
  AForDetailView: Boolean);
var
...
function cxCompareGroupSummaryItems(AItem1,
  AItem2: TcxExportGroupSummaryItem): Integer;
begin
  if AItem1.Column = AItem2.Column then
    Result := AItem1.Index - AItem2.Index
  else if AItem1.Column = nil then
    Result := -1
  else if AItem2.Column = nil then
    Result := 1
  else
    Result := AItem1.Column.VisibleIndex - AItem2.Column.VisibleIndex;
end;

function GetExportClassByGridView(AGridView: TcxCustomGridView)
  : TcxGridCustomExportClass;
begin
...
function GetExportDataClassByGridView(AGridView: TcxCustomGridView)
  : TcxGridCustomExportClass;
begin
...
procedure CalculateCardRowWidths(var AWidths: array of Integer;
...
procedure DoExportGridToFile(AFileName: string; AExportType: Integer;
  AGrid: TcxGrid; AExpand, ASaveAll, AUseNativeFormat: Boolean;
  const ASeparator, ABeginString, AEndString: string; const AFileExt: string;
  ...
  begin
    ...
    Result := AExportClass.Create(AFileName, AExportType, AView, AGrid, nil,
      AHandler);
...
begin
  ExportGridToFile(AFileName, cxExportToHtml, AGrid, AExpand, ASaveAll, False,
    '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridToXML(const AFileName: string; AGrid: TcxGrid;
  AExpand, ASaveAll: Boolean; const AFileExt: string; AHandler: TObject);
begin
  ExportGridToFile(AFileName, cxExportToXml, AGrid, AExpand, ASaveAll, False,
    '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridToCSV(const AFileName: string; AGrid: TcxGrid;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  ExportGridToFile(AFileName, cxExportToCSV, AGrid, AExpand, ASaveAll, False,
    ASeparator, '', '', AFileExt, AHandler, AEncoding);
end;
...
procedure ExportGridToText(const AFileName: string; AGrid: TcxGrid;
  ...
  AEncoding: TEncoding = nil); overload;
begin
  ExportGridToText(AFileName, AGrid, AExpand, ASaveAll, '', '', '', AFileExt,
    AHandler, AEncoding);
end;
...
procedure ExportGridToExcel(const AFileName: string; AGrid: TcxGrid;
  AExpand, ASaveAll, AUseNativeFormat: Boolean; const AFileExt: string;
  AHandler: TObject);
begin
  ExportGridToFile(AFileName, cxExportToExcel, AGrid, AExpand, ASaveAll,
    AUseNativeFormat, '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridToXLSX(const AFileName: string; AGrid: TcxGrid;
  ...
  AHandler: TObject);
begin
  ExportGridToFile(AFileName, cxExportToXLSX, AGrid, AExpand, ASaveAll,
    AUseNativeFormat, '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridDataToHTML(const AFileName: string; AGrid: TcxGrid;
  ...
  const AFileExt: string = 'html'; AHandler: TObject = nil);
begin
  ExportGridDataToFile(AFileName, cxExportToHtml, AGrid, AExpand, ASaveAll,
    True, '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridToFile(AFileName: string; AExportType: Integer;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  DoExportGridToFile(AFileName, AExportType, AGrid, AExpand, ASaveAll,
    AUseNativeFormat, ASeparator, ABeginString, AEndString, AFileExt, False,
    AHandler, AEncoding);
end;
...
procedure ExportGridDataToXML(const AFileName: string; AGrid: TcxGrid;
  ...
  const AFileExt: string = 'xml'; AHandler: TObject = nil);
begin
  ExportGridDataToFile(AFileName, cxExportToXml, AGrid, AExpand, ASaveAll, True,
    '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridDataToCSV(const AFileName: string; AGrid: TcxGrid;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  ExportGridDataToFile(AFileName, cxExportToCSV, AGrid, AExpand, ASaveAll, True,
    ASeparator, '', '', AFileExt, AHandler, AEncoding);
end;
...
begin
  ExportGridDataToText(AFileName, AGrid, AExpand, ASaveAll, '', '', '',
    AFileExt, AHandler, AEncoding);
end;
...
procedure ExportGridDataToText(const AFileName: string; AGrid: TcxGrid;
  ...
  AEncoding: TEncoding = nil);
begin
  ExportGridDataToFile(AFileName, cxExportToText, AGrid, AExpand, ASaveAll,
    True, ASeparator, ABeginString, AEndString, AFileExt, AHandler, AEncoding);
end;
...
procedure ExportGridDataToExcel(const AFileName: string; AGrid: TcxGrid;
  ...
  AHandler: TObject = nil);
begin
  ExportGridDataToFile(AFileName, cxExportToExcel, AGrid, AExpand, ASaveAll,
    AUseNativeFormat, '', '', '', AFileExt, AHandler);
end;
...
procedure ExportGridDataToXLSX(const AFileName: string; AGrid: TcxGrid;
  ...
  AHandler: TObject = nil);
begin
  ExportGridDataToFile(AFileName, cxExportToXLSX, AGrid, AExpand, ASaveAll,
    AUseNativeFormat, '', '', '', AFileExt, AHandler);
end;
...
begin
  DoExportGridToFile(AFileName, AExportType, AGrid, AExpand, ASaveAll,
    AUseNativeFormat, ASeparator, ABeginString, AEndString, AFileExt, True,
    AHandler, AEncoding);
end;
...
type
  ...
  public
    class function CalculateFooterLineCount
      (AItems: TcxDataSummaryItems): Integer;
    class function HasGroupFooter(AGridView: TcxGridTableView;
      ARow: TcxCustomGridRecord; var ALevel: Integer): Boolean;
    class function IsSameFooterItems(AItem1,
...
procedure TcxGridExportProviderCache.Commit(AProgressHelper
  : TcxCustomProgressCalculationHelper; AHandler: TObject);
begin
  Rows.Clear;
  Columns.Clear;
...
procedure TcxGridExportProviderCache.PrepareToExport
  (ADataOnly: Boolean = False);
var
  I: Integer;
begin
  Columns.Arrange;
...
constructor TcxGridExportProviderAdapter.Create
  (ACache: TcxGridExportProviderCache);
begin
  inherited Create;
...
constructor TcxGridExportProviderAdapter.CreateParented(AParentAdapter
  : IcxGridExportProviderAdapter; const AClientOrigin: TPoint);
begin
  inherited Create;
...
function TcxGridExportProviderAdapter.GetFormatCode(ACol,
  ARow: Integer): string;
begin
...
function TcxGridExportProviderAdapter.GetStyle(AStyleIndex: Integer)
  : TcxCacheCellStyle;
begin
  if FParentAdapter <> nil then
...
function TcxGridExportProviderAdapter.RegisterStyle(const AStyle
  : TcxCacheCellStyle): Integer;
begin
  if FParentAdapter <> nil then
...
function TcxGridExportProviderAdapter.SetCellGraphic(const AGlobalArea: TRect;
  AStyleIndex: Integer; AGraphic: TGraphic; AFitMode: TcxImageFitMode): TObject;
begin
  if FParentAdapter <> nil then
    Result := FParentAdapter.SetCellGraphic(AGlobalArea, AStyleIndex,
      AGraphic, AFitMode)
  else
    Result := FProviderCache.Provider.SetCellGraphic(AGlobalArea, AStyleIndex,
      AGraphic, AFitMode);
end;

procedure TcxGridExportProviderAdapter.SetCellGraphicAsSharedHandle
  (const AGlobalArea: TRect; AStyleIndex: Integer; AHandle: TObject;
  AFitMode: TcxImageFitMode);
begin
  if FParentAdapter <> nil then
    FParentAdapter.SetCellGraphicAsSharedHandle(AGlobalArea, AStyleIndex,
      AHandle, AFitMode)
  else
    FProviderCache.Provider.SetCellGraphicAsSharedHandle(AGlobalArea,
      AStyleIndex, AHandle, AFitMode);
end;

procedure TcxGridExportProviderAdapter.SetCellStyle(const AGlobalCol,
  AGlobalRow, AStyleIndex: Integer);
begin
  if FParentAdapter <> nil then
...
procedure TcxGridExportProviderAdapter.SetCellStyle(const AArea: TRect;
  const AStyleIndex: Integer);
begin
  if FParentAdapter <> nil then
...
procedure TcxGridExportProviderAdapter.SetCellValue(const AGlobalCol,
  AGlobalRow: Integer; const AValue: Variant; const AValueDisplayFormat: string;
  AValueDisplayFormatType: TcxValueDisplayFormatType);
begin
  if FParentAdapter <> nil then
    FParentAdapter.SetCellValue(AGlobalCol, AGlobalRow, AValue,
      AValueDisplayFormat, AValueDisplayFormatType)
  else
    FProviderCache.Provider.SetCellValue(AGlobalCol, AGlobalRow, AValue,
      AValueDisplayFormat, AValueDisplayFormatType);
end;

procedure TcxGridExportProviderAdapter.SetCellValueAsFormula(const AGlobalCol,
  AGlobalRow: Integer; const AValue: string; ADisplayText: string = '';
  AFormatCode: string = ''; AListSeparator: Char = ',');
begin
  if FParentAdapter <> nil then
    FParentAdapter.SetCellValueAsFormula(AGlobalCol, AGlobalRow, AValue,
      ADisplayText, AFormatCode, AListSeparator)
  else
    FProviderCache.Provider.SetCellValueAsFormula(AGlobalCol, AGlobalRow,
      AValue, ADisplayText, AFormatCode, AListSeparator);
end;

procedure TcxGridExportProviderAdapter.SetDefaultStyle
...
procedure TcxGridExportProviderAdapter.AddOutlineGroup(AStart,
  AFinish: Integer);
begin
  if FParentAdapter <> nil then
...
procedure TcxGridExportProviderAdapter.RealBoundsToLogicalBounds
  (const ABounds: TRect; out ALogicalBounds: TRect);
begin
  RealRangeToLogicalRangeHorz(ABounds.Left, ABounds.Right, ALogicalBounds.Left,
    ALogicalBounds.Right);
  RealRangeToLogicalRangeVert(ABounds.Top, ABounds.Bottom, ALogicalBounds.Top,
    ALogicalBounds.Bottom);
end;

procedure TcxGridExportProviderAdapter.RealBoundsToLogicalBoundsEx
  (const ABounds, ASearchArea: TRect; out ALogicalBounds: TRect);
begin
  if FParentAdapter <> nil then
    FParentAdapter.RealBoundsToLogicalBoundsEx
      (cxRectOffset(ABounds, FClientOrigin), ASearchArea, ALogicalBounds)
  else
  begin
    FProviderCache.Columns.GetPositionEx(ABounds.Left, ABounds.Right,
      ASearchArea.Left, ASearchArea.Right, ALogicalBounds.Left,
      ALogicalBounds.Right);
    FProviderCache.Rows.GetPositionEx(ABounds.Top, ABounds.Bottom,
      ASearchArea.Top, ASearchArea.Bottom, ALogicalBounds.Top,
      ALogicalBounds.Bottom);
  end;
end;

procedure TcxGridExportProviderAdapter.RealRangeToLogicalRangeHorz(X1,
  X2: Integer; out ALogicalX1, ALogicalX2: Integer);
begin
  if FParentAdapter <> nil then
    FParentAdapter.RealRangeToLogicalRangeHorz(X1 + FClientOrigin.X,
      X2 + FClientOrigin.X, ALogicalX1, ALogicalX2)
  else
    FProviderCache.Columns.GetPosition(X1, X2, ALogicalX1, ALogicalX2);
end;

procedure TcxGridExportProviderAdapter.RealRangeToLogicalRangeVert(Y1,
  Y2: Integer; out ALogicalY1, ALogicalY2: Integer);
begin
  if FParentAdapter <> nil then
    FParentAdapter.RealRangeToLogicalRangeVert(Y1 + FClientOrigin.Y,
      Y2 + FClientOrigin.Y, ALogicalY1, ALogicalY2)
  else
...
constructor TcxGridCustomExport.Create(const AFileName: string;
  AExportType: Integer; AGridView: TcxCustomGridView; AGrid: TcxCustomGrid;
  AViewInfo: TcxCustomGridViewInfo; AHandler: TObject = nil);
var
  AExportProvider: IcxExportProvider;
begin
  AExportProvider := TcxExport.Provider(AExportType, AFileName)
    as IcxExportProvider;
  FProviderCache := TcxGridExportProviderCache.Create(AExportProvider);
  FProviderCacheOwner := True;
  Create(AFileName, AGridView, AGrid, AViewInfo,
    TcxGridExportProviderAdapter.Create(FProviderCache), AHandler);
end;

constructor TcxGridCustomExport.Create(const AFileName: string;
  AGridView: TcxCustomGridView; AGrid: TcxCustomGrid;
  AViewInfo: TcxCustomGridViewInfo; AAdapter: IcxGridExportProviderAdapter;
  AHandler: TObject = nil);
begin
  inherited Create;
  FAdapter := AAdapter;
  ...
  FProgressHelper := TcxProgressCalculationHelper.Create(3, AGrid,
    ProgressHandler);
end;
...
begin
  Create(AMasterExport.FileName, AGridView, AMasterExport.Grid, AViewInfo,
    TcxGridExportProviderAdapter.CreateParented(AMasterExport.Adapter,
    AClientOrigin));
...
begin
  if ProviderCache <> nil then
    if Supports(ProviderCache.Provider, IcxExportWithSeparators,
      AExportWithSeparators) then
    begin
...
begin
  if ProviderCache <> nil then
  begin
    if Supports(ProviderCache.Provider, IcxExportProviderEncoding, AProvider)
    then
...
function TcxGridCustomExport.CheckNativeValue(AProperties
  : TcxCustomEditProperties; AItem: TcxCustomGridTableItem;
...
begin
  ...
  try
    if VarType(AValue) = VarFmtBcd then
      Result := dxPrepareBcdForExport(AValue, IsCurrencyItem(AItem) and
        IsCurrencyProperties(AProperties))
    else
...
begin
  ...
  begin
    TcxDBDataController(DataController).DataModeController.GridMode :=
      FSaveGridModeFlag;
...
function TcxGridCustomExport.CanUseNativeFormatProperties
  (AProperties: TcxCustomEditProperties): Boolean;
begin
  Result := not((AProperties is TcxCustomLookupEditProperties) or
    (AProperties is TdxSparklineProperties) or
...
begin
  AGridView := Safe<TcxCustomGridTableView>.Cast(GridView);
  if AGridView = nil then
    Exit;
  Grid.BeginUpdate;
  try
    if (Expand or DataOnly or RequiredExpandGroups(AGridView)) and
      not AGridView.IsDetail then
  ...
  for AIndex := 0 to RecordCount - 1 do
    RecordsList[AIndex] := AGridView.ViewData.Records
      [Integer(RecordsList[AIndex])];
end;

procedure TcxGridCustomExport.FillArea(const ABounds: TRect;
  AStyleIndex: Integer; ABorderColor: TColor = clDefault;
...
procedure TcxGridCustomExport.FillRealArea(const ABounds: TRect;
  AStyleIndex: Integer; ABorderColor: TColor = clDefault;
  ABorders: TcxBorders = cxBordersAll);
var
...
function TcxGridCustomExport.GetViewItemValue(ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem): Variant;
var
  AValueDisplayFormat: string;
  AValueDisplayFormatType: TcxValueDisplayFormatType;
begin
  Result := GetViewItemValueEx(ARecord, AItem, AValueDisplayFormat,
    AValueDisplayFormatType);
end;

function TcxGridCustomExport.GetViewItemValueEx(ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; out AValueDisplayFormat: string;
  out AValueDisplayFormatType: TcxValueDisplayFormatType): Variant;
var
...
begin
  ...
  begin
    ...
    AValueDisplayFormatType := cxGetDisplayFormatType(AProperties,
      GetField(AItem));
  end
  else
  begin
...
function TcxGridCustomExport.IsCurrencyItem
  (AItem: TcxCustomGridTableItem): Boolean;
var
  AField: TField;
begin
  ...
  begin
    AField := TcxGridDBDataController(GridView.DataController)
      .GetItemField(AItem.Index);
...
function TcxGridCustomExport.IsNativeFormatProperties
  (AProperties: TcxCustomEditProperties; AItem: TcxCustomGridTableItem)
  : Boolean;
begin
  Result := DataOnly and CanUseNativeFormatProperties(AProperties) or
    (AProperties is TcxDateEditProperties) or
    (AProperties is TcxSpinEditProperties) or
    (AProperties is TcxTimeEditProperties) or IsCurrencyProperties(AProperties);
  Result := (Result and UseNativeFormat) or
    (IsRTFSupported(AProperties) and Adapter.IsRTFSupported);
end;

function TcxGridCustomExport.NeedProcessRecord
  (ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := SaveAll or DataOnly or
    (ARecord.GridView.Controller.SelectedRecordCount = 0) or ARecord.Selected;
end;

procedure TcxGridCustomExport.RealBoundsToLogicalBounds(const ABounds: TRect;
  out ALogicalBounds: TRect);
begin
  Adapter.RealBoundsToLogicalBounds(ABounds, ALogicalBounds);
end;

procedure TcxGridCustomExport.RealBoundsToLogicalBoundsEx(const ABounds,
  ASearchArea: TRect; out ALogicalBounds: TRect);
begin
...
function TcxGridCustomExport.RegisterSolidStyle(AStyleIndex: Integer;
  AData: TObject = nil): Integer;
var
...
begin
  ...
  if GridView is TcxCustomGridTableView then
    TcxCustomGridTableView(GridView).Styles.GetViewParams(AStyleIndex, AData,
      nil, AParams);
...
function TcxGridCustomExport.RegisterViewParams(const AViewParams
  : TcxViewParams; const AAlignment: TAlignment = taLeftJustify): Integer;
var
  AStyle: TcxCacheCellStyle;
begin
...
procedure TcxGridCustomExport.SetRealCellStyle(const ARealBounds,
  ASearchArea: TRect; AStyleIndex: Integer);
begin
  SetRealCellStyleAndValue(ARealBounds, ASearchArea, AStyleIndex, Null);
end;

procedure TcxGridCustomExport.SetRealCellStyleAndFormula(const ARealBounds,
  ASearchArea: TRect; AStyleIndex: Integer; const AFormula: string;
  ADisplayText: string = ''; AFormatCode: string = '';
  AListSeparator: Char = ',');
var
  R: TRect;
begin
  RealBoundsToLogicalBoundsEx(ARealBounds, ASearchArea, R);
  Adapter.SetCellValueAsFormula(R.Left, R.Top, AFormula, ADisplayText,
    AFormatCode, AListSeparator);
  Adapter.SetCellStyle(R, AStyleIndex);
end;

procedure TcxGridCustomExport.SetRealCellStyleAndValue(const ARealBounds,
  ASearchArea: TRect; AStyleIndex: Integer; const AValue: Variant;
  const AValueDisplayFormat: string = '');
var
  R: TRect;
begin
...
procedure TcxGridCustomExport.SetRealCellStyleAndValueEx(const ARealBounds,
  ASearchArea: TRect; AStyleIndex: Integer; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem);
var
  AValue: Variant;
  AValueDisplayFormat: string;
...
begin
  ...
  if AItem <> nil then
  begin
    if not(Adapter.IsGraphicsSupported and TrySetCellValueAsGraphic(ARealBounds,
      R, AStyleIndex, ARecord, AItem)) then
    begin
      AValue := GetViewItemValueEx(ARecord, AItem, AValueDisplayFormat,
        AValueDisplayFormatType);
      Adapter.SetCellValue(R.Left, R.Top, AValue, AValueDisplayFormat,
        AValueDisplayFormatType);
    end;
  end;
  Adapter.SetCellStyle(R, AStyleIndex);
...
function TcxGridCustomExport.TrySetCellValueAsGraphic(const ABounds,
  AArea: TRect; AStyleIndex: Integer; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem): Boolean;
  ...
  begin
    ...
    try
      LoadPicture(APicture, AProperties.GetGraphicClass(AItem, ARecord.Index),
        ARecord.Values[AItem.Index]);
      ...
      begin
        ...
        begin
          ...
          if AProperties.GetRealStretch(cxSize(APicture.Width, APicture.Height),
            cxRectSize(AClientRect)) then
          begin
            if AProperties.Proportional then
              R := cxRectProportionalStretch(AClientRect, APicture.Width,
                APicture.Height)
            else
              R := AClientRect;
          end;
        ...
        Adapter.SetCellGraphic(AArea, DefaultStyleIndex,
          GraphicToPNGImage(AClientRect, R, APicture.Graphic), AFitMode);
      end;
    finally
      APicture.Free;
    end;
  ...
  function ProcessSparkLinesAsImage(AProperties
    : TdxSparklineProperties): Boolean;
  var
    R: TRect;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        try
          AViewInfo := AProperties.GetViewInfoClass.Create as
            TdxSparkLineViewInfo;
          try
            AViewData.EditValueToDrawValue(ARecord.Values[AItem.Index],
              AViewInfo);
            AViewData.Calculate(AGraphic.cxCanvas, R, cxNullPoint, cxmbNone, [],
              AViewInfo, False);
        ...
        Adapter.SetCellGraphic(AArea, DefaultStyleIndex,
          GraphicToPNGImage(R, R, AGraphic));
      finally
        AGraphic.Free;
      end;
    end;
  ...
  function ProcessCellValueAsImageComboBox(AProperties
    : TcxImageComboBoxProperties): Boolean;
  var
    ABitmap: TcxBitmap;
  ...
  begin
    ...
    begin
      Result := True;
      AProperties.GetImageComboBoxDisplayValue(ARecord.Values[AItem.Index],
        AText, AImageIndex);
      ...
      begin
        if DataOnly then
          ...
          AFitMode := ifmNormal;
        if ProviderCache.Provider.TryGetSharedImageHandle(AProperties.Images,
          AImageIndex, AHandle) then
          Adapter.SetCellGraphicAsSharedHandle(AArea, DefaultStyleIndex,
            AHandle, AFitMode)
        else
        begin
          ABitmap := TcxBitmap.CreateSize(AProperties.Images.Width,
            AProperties.Images.Height, pf24bit);
          try
            ABitmap.cxCanvas.FillRect(ABitmap.ClientRect,
              Adapter.GetStyle(AStyleIndex).BrushBkColor);
            ...
            AHandle := Adapter.SetCellGraphic(AArea, DefaultStyleIndex,
              GraphicToPNGImage(ARect, cxRectCenter(ARect, ABitmap.Width,
              ABitmap.Height), ABitmap), AFitMode);
            ProviderCache.Provider.AddSharedImageHandle(AProperties.Images,
              AImageIndex, AHandle);
          finally
            ABitmap.Free;
          end;
...
begin
  ...
  begin
    ...
    else if AItem.GetProperties is TcxCustomImageComboBoxProperties then
      Result := ProcessCellValueAsImageComboBox
        (TcxImageComboBoxProperties(AItem.GetProperties))
    else if AItem.GetProperties is TdxSparklineProperties then
      Result := ProcessSparkLinesAsImage
        (TdxSparklineProperties(AItem.GetProperties))
    else
      Result := False;
  end;
end;
...
function TcxGridCustomExport.TextHeightEx(const AViewParams
  : TcxViewParams): Integer;
begin
  Result := TextHeight(AViewParams.Font);
end;

function TcxGridCustomExport.TextWidth(AFont: TFont;
  const AText: string): Integer;
begin
  Result := cxTextWidth(AFont, AText) + cxTextOffset * 2;
end;

function TcxGridCustomExport.TextWidthEx(const AViewParams: TcxViewParams;
  const AText: string): Integer;
begin
  Result := TextWidth(AViewParams.Font, AText);
end;

procedure TcxGridCustomExport.ViewParamsToExportStyle(const AViewParams
  : TcxViewParams; var AExportStyle: TcxCacheCellStyle;
  const AAlignment: TAlignment = taLeftJustify; ABorders: TcxBorders = [];
  ABorderColor: TColor = clDefault);
var
  I: Integer;
...
begin
  ...
  for I := 0 to 3 do
  begin
    AExportStyle.Borders[I].IsDefault := (ABorderColor = clDefault) or
      not(TcxBorder(I) in ABorders);
    AExportStyle.Borders[I].Width := BorderWidths
      [not AExportStyle.Borders[I].IsDefault];
    if not AExportStyle.Borders[I].IsDefault then
      AExportStyle.Borders[I].Color := ColorToRgb(ABorderColor);
  end;
end;

function TcxGridCustomExport.CanExpandDataRecord
  (ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := ARecord.IsData and (Expand or DataOnly);
end;

function TcxGridCustomExport.CanExpandGroupRecord
  (ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := not ARecord.IsData and
    (Expand or DataOnly or RequiredExpandGroup(ARecord))
end;
...
begin
  ...
  AMasterDataRow := Safe<TcxGridMasterDataRow>.Cast(ARecord);
  if AMasterDataRow <> nil then
  begin
    AActiveDetailView := Safe<TcxCustomGridTableView>.Cast
      (AMasterDataRow.ActiveDetailGridView);
    if AActiveDetailView <> nil then
...
begin
  ...
  if AGridView.ViewData.Records[AGridView.ViewData.RecordCount - 1].Expandable
  then
    DoExpandRecords(AGridView, ExpandDataRecord, CanExpandDataRecord);
end;

function TcxGridCustomExport.RequiredExpandGroup
  (ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := RequiredExpandGroups(ARecord.GridView) and ARecord.Expandable and
    not ARecord.Expanded and ARecord.GridView.DataController.
    GroupContainsSelectedRows(ARecord.Index)
end;

function TcxGridCustomExport.RequiredExpandGroups
  (AGridView: TcxCustomGridTableView): Boolean;
begin
  Result := not SaveAll and (AGridView.Controller.SelectedRecordCount > 0) and
    DataController.MultiSelectionSyncGroupWithChildren;
end;
...
begin
  Result := ViewInfo.LookAndFeelPainter.ScaledExpandButtonSize
    (dxGetScaleFactor(Grid));
end;
...
function TcxGridCustomExport.GetRecord(ARecordIndex: Integer)
  : TcxCustomGridRecord;
begin
...
procedure TcxGridCustomExport.ProgressHandler(Sender: TObject;
  Percents: Integer);
var
...
function TcxGridCustomTableViewExport.AddVisualDataItem(const AItemBounds
  : TRect; AStyle: Integer; ARecord: TcxCustomGridRecord;
...
function TcxGridCustomTableViewExport.AddVisualItem
  (AItemClass: TcxExportVisualItemClass; const ABounds: TRect)
  : TcxExportVisualItem;
begin
...
function TcxGridCustomTableViewExport.AddVisualItemEx(const AItemBounds: TRect;
  const ADisplayText: string; const AViewParams: TcxViewParams;
  AAlignment: TAlignment; ABorders: TcxBorders;
  ABorderColor: TColor = clDefault; AIsBackground: Boolean = False;
  AWordWrap: Boolean = True): TcxExportVisualItem;
var
  ASide: TcxBorder;
  AStyle: TcxCacheCellStyle;
begin
  ...
  Result := AddVisualItemEx(AItemBounds, ADisplayText,
    Adapter.RegisterStyle(AStyle), AIsBackground);
end;

function TcxGridCustomTableViewExport.AddVisualItemEx(const AItemBounds: TRect;
  const ADisplayText: string; AStyle: Integer; AIsBackground: Boolean = False)
  : TcxExportVisualItem;
begin
  Result := AddVisualItem(TcxExportVisualItem, AItemBounds);
...
function TcxGridCustomTableViewExport.CanExportVisualItem
  (AItem: TcxExportVisualItem): Boolean;
begin
...
procedure TcxGridCustomTableViewExport.ExportBackgroundVisualItem
  (AItem: TcxExportVisualItem);
begin
  FillRealArea(AItem.Bounds, AItem.Style, AItem.ItemBorderColor,
    AItem.ItemBorders);
end;

procedure TcxGridCustomTableViewExport.ExportVisualItem
  (AItem: TcxExportVisualItem; const ASearchArea: TRect);
begin
  if AItem.Hidden then
    Exit;
  if AItem.IsBackground then
    ExportBackgroundVisualItem(AItem)
  else if IsDataVisualItem(AItem) then
    ExportDataVisualItem(AItem, ASearchArea)
  else
    SetRealCellStyleAndValue(AItem.Bounds, ASearchArea, AItem.Style,
      AItem.DisplayText);
end;

procedure TcxGridCustomTableViewExport.ExportDataVisualItem
  (AItem: TcxExportVisualItem; const ASearchArea: TRect);
var
  ARecord: TcxCustomGridRecord;
...
begin
  ...
  SetRealCellStyleAndValueEx(AItem.Bounds, ASearchArea, AItem.Style, ARecord,
    AGridItem);
end;

function TcxGridCustomTableViewExport.IsDataVisualItem
  (AItem: TcxExportVisualItem): Boolean;
begin
...
function TcxGridCustomTableViewExport.GetVisualItem(AIndex: Integer)
  : TcxExportVisualItem;
begin
...
begin
  Bounds.Right := Min(Bounds.Right, Bounds.Left + cxTextWidth(ViewParams.Font,
    Text) + cxTextOffset * 3);
end;

{ TcxGridTableViewExport }

procedure TcxGridTableViewExport.AddDataRow(var ATop, ALeft: Integer;
  ARow: TcxGridDataRow);
var
  AHeight: Integer;
...
begin
  ...
  AViewInfo := ARecordViewInfoClass.Create(ViewInfo.RecordsViewInfo, ARow)
    as TcxGridDataRowViewInfo;
  try
    TcxCustomGridRecordViewInfoAccess(AViewInfo)
    ...
    try
      ...
      if (PreviewPlace = ppTop) and AViewInfo.HasPreview and
        (AViewInfo.PreviewViewInfo.Height > 0) then
        AddRowPreview(ATop, ALeft, ARow);

      CreateRecordFromPattern(ATop, ALeft, ARow);
      Inc(ATop, RecordHeight);
      if (PreviewPlace = ppBottom) and AViewInfo.HasPreview and
        (AViewInfo.PreviewViewInfo.Height > 0) then
        AddRowPreview(ATop, ALeft, ARow);
    finally
      TcxCustomGridRecordViewInfoAccess(AViewInfo)
...
procedure TcxGridTableViewExport.AddFooterCell(ABounds: TRect;
  AColumn: TcxGridColumn; AFooterItem: TcxDataSummaryItem; AValue: Variant;
...
begin
  ...
  AFooterCell := AddVisualItemEx(ABounds, ADisplayText, AParams,
    AColumn.FooterAlignmentHorz, cxBordersAll, FooterCellBorderColor);
  ...
  begin
    ...
    if UseNativeFormat and (AColumn <> nil) and (AFooterItem.Format = '') then
      AFooterCell.Value := CheckNativeValue(AColumn.GetProperties,
        AColumn, AValue);
  end;
end;
...
begin
  ...
  FooterHeight := TdxTextMeasurer.TextLineHeight(AParams.Font) + 2 *
    cxTextOffset;
  ...
  begin
    ...
    begin
      AddFooterCellsByColumn(R, TcxGridColumn(Patterns[APatternIndex].Data),
        APatternIndex, ACount);
      Dec(ACellCount, ACount);
    end;
  end;
end;

procedure TcxGridTableViewExport.AddFooterCellsByColumn(ABounds: TRect;
  AColumn: TcxGridColumn; APatternIndex: Integer; out ACount: Integer);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    AddFooterCell(GetFooterItemBounds(APatternIndex, ACount, ABounds), AColumn,
      AItem, DataController.Summary.FooterSummaryValues[I], AParams);
    Inc(ACount);
...
procedure TcxGridTableViewExport.AddGroupFooterCells(var ATop, ALeft: Integer;
...
begin
  ...
  FooterHeight := TdxTextMeasurer.TextLineHeight(AParams.Font) + 2 *
    cxTextOffset;
  ABottom := ATop + GetGroupFooterRowCount(ARow) *
    (FooterHeight + cxGridFooterCellIndent);
  ...
  begin
    ...
    if Patterns[APatternIndex].IsColumn then
    begin
      AddGroupFooterCellsByColumn(R, ARow,
        TcxGridColumn(Patterns[APatternIndex].Data), APatternIndex, ACount);
      Dec(ACellCount, ACount);
    end;
  end;
end;

procedure TcxGridTableViewExport.AddGroupFooterCellsByColumn(ABounds: TRect;
  ARow: TcxCustomGridRow; AColumn: TcxGridColumn; APatternIndex: Integer;
  out ACount: Integer);
var
...
begin
  ACount := 0;
  for ALevelGroupedItemIndex :=
    0 to DataController.Groups.GetLevelGroupedItemCount(ARow.Level) - 1 do
  begin
    if not DataController.Summary.GetGroupSummaryInfo(ARow.Index, AItems,
      AValues, ALevelGroupedItemIndex) then
      Continue;
    ...
    begin
      ...
      if not IsColumnContainsFooterItem(AItem, AColumn) then
        Continue;
      GridView.Styles.GetFooterCellParams(ARow, AColumn, ARow.Level,
        AItem, AParams);
      AddFooterCell(GetFooterItemBounds(APatternIndex, ACount, ABounds, True),
        AColumn, AItem, AValues^[ASummaryItemIndex], AParams);
      Inc(ACount);
...
procedure TcxGridTableViewExport.AddGroupRow(var ATop, ALeft: Integer;
...
begin
  ...
  if IsSummaryUnderColumns[ARow] and ARow.GetGroupSummaryInfo(AItems, AValues)
  then
  begin
  ...
  begin
    ...
    for I := 0 to ARow.GroupedColumnCount - 1 do
      TcxCustomGridTableItemAccess.DoGetDisplayText
        (ARow.GroupedColumns[I], ARow, S);
    AddVisualItemEx(R, S, AViewParams, taLeftJustify, GridLines,
      GridLineColor, False);
  end;
  Inc(ATop, AHeight);
end;
...
function TcxGridTableViewExport.AddIndents(ATop, ARowHeight: Integer;
  ARow: TcxCustomGridRecord; AHasButton: Boolean; ALevel: Integer = -1)
  : Integer;
var
  R: TRect;
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      ViewParamsToExportStyle(AViewParams, AStyle, taLeftJustify,
        AGridLines - [bTop], GridLineColor);
    end;
...
procedure TcxGridTableViewExport.AddMasterDataRow(var ATop, ALeft: Integer;
...
begin
  ...
  begin
    ABounds := Rect((ARow.Level + 1) * ViewInfo.LevelIndent, ATop, RecordWidth,
      ATop + DefaultRowHeight);
    AVisualItem := AddVisualDataItem(ABounds, DefaultStyleIndex, ARow, nil);
    AVisualItem.DetailViewViewInfo :=
      CalculateViewViewInfo(ARow.ActiveDetailGridView, ViewInfo.Bounds);
    AVisualItem.DetailExport := GetExportClassByGridView
      (ARow.ActiveDetailGridView).CreateFrom(Self, ARow.ActiveDetailGridView,
      AVisualItem.DetailViewViewInfo, ABounds.TopLeft);
    AVisualItem.DetailExport.Initialize;
    AVisualItem.DetailExport.CreateExportCells;

    ADetailHeight := cxRectHeight
      (AVisualItem.DetailExport.Adapter.GetBoundingRect);
...
procedure TcxGridTableViewExport.AddRowFooter(var ATop, ALeft: Integer;
  ARow: TcxCustomGridRecord; ALevel: Integer);
begin
  ALeft := 0;
...
procedure TcxGridTableViewExport.AddRowFooters(var ATop, ALeft: Integer;
...
procedure TcxGridTableViewExport.AddRowPreview(var ATop, ALeft: Integer;
  ARow: TcxCustomGridRecord);
var
...
function TcxGridTableViewExport.AddPattern(const ABounds: TRect; AData: TObject;
  AOffset: Integer = 0): TcxExportVisualItem;
begin
...
function TcxGridTableViewExport.CalculateFooterCellCount
  (AItems: TcxDataSummaryItems): Integer;
var
  I: Integer;
begin
...
function TcxGridTableViewExport.CanShowMultiSummaries
  (AIsRowFooter: Boolean = False): Boolean;
begin
...
function TcxGridTableViewExport.CanExportVisualItem
  (AItem: TcxExportVisualItem): Boolean;
begin
  Result := (AItem.DetailExport <> nil) or inherited CanExportVisualItem(AItem);
end;

procedure TcxGridTableViewExport.CreateRecordFromPattern(var ATop,
  ALeft: Integer; ARecord: TcxCustomGridRecord);
var
  R: TRect;
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      AddVisualDataItem(R, Adapter.RegisterStyle(AStyle), ARecord,
        TcxGridColumn(APattern.Data));
    end
    else
      AddVisualItemEx(R, '', GetPatternParams(ARecord, APattern), taLeftJustify,
        [], clDefault, True);
  end;
end;

function TcxGridTableViewExport.DoMergeCell(AMasterItem,
  ASlaveItem: TcxExportVisualItem): Boolean;
var
  AColumn: TcxGridColumn;
...
begin
  ...
  begin
    ...
    AMasterProperties := AColumn.GetProperties
      (TcxGridDataRow(AMasterItem.Data));
    ...
    Result := AColumn.DoCompareValuesForCellMerging
      (TcxGridDataRow(AMasterItem.Data), AMasterProperties, AMasterValue,
      TcxGridDataRow(ASlaveItem.Data), ASlaveProperties, ASlaveValue);
  end;
  if Result then
  begin
...
procedure TcxGridTableViewExport.ExportDataVisualItem
  (AItem: TcxExportVisualItem; const ASearchArea: TRect);
var
...
begin
  ...
  begin
    ...
    if Assigned(ASummaryItem.OnGetText) then
      ...
      SetRealCellStyleAndValue(AItem.Bounds, ASearchArea, AItem.Style,
        AItem.Value, ASummaryItem.Format);
  end
  else
    inherited ExportDataVisualItem(AItem, ASearchArea);
end;

procedure TcxGridTableViewExport.ExportDetailVisualItem
  (AItem: TcxExportVisualItem);
begin
  try
...
procedure TcxGridTableViewExport.ExportVisualItem(AItem: TcxExportVisualItem;
  const ASearchArea: TRect);
begin
...
function TcxGridTableViewExport.GetColumnOffset(AColumn: TcxGridColumn)
  : Integer;
begin
  Result := LeftPos - IndicatorWidth;
end;

function TcxGridTableViewExport.GetExpandButtonParams(ABorders: TcxBorders)
  : TcxCacheCellStyle;
var
  ABorder: TcxBorder;
begin
...
function TcxGridTableViewExport.GetFooterItemBounds(AIndex, ALineIndex: Integer;
  const AOrigin: TRect; AIsRowFooter: Boolean = False): TRect;
var
  H, LCount: Integer;
begin
  ...
  begin
    ...
    Result.Top := AOrigin.Top + (Bounds.Top div DefaultRowHeight +
      ALineIndex) * H;
    LCount := 1;
...
begin
  if CanShowMultiSummaries then
    Result := TcxGridExportHelper.CalculateFooterLineCount
      (DataController.Summary.FooterSummaryItems)
  else
    Result := RecordRowCount;
end;

function TcxGridTableViewExport.GetGroupFooterCellCount
  (ARow: TcxCustomGridRow): Integer;
var
  I: Integer;
  AItems: TcxDataSummaryItems;
  AValues: PVariant;
begin
  Result := 0;
  for I := 0 to DataController.Groups.GetLevelGroupedItemCount
    (ARow.Level) - 1 do
    if DataController.Summary.GetGroupSummaryInfo(ARow.Index, AItems, AValues, I)
    then
      Inc(Result, CalculateFooterCellCount(AItems));
end;

function TcxGridTableViewExport.GetGroupFooterRowCount
  (ARow: TcxCustomGridRow): Integer;
  ...
  begin
    ...
    for I := 0 to DataController.Groups.GetLevelGroupedItemCount
      (ARow.Level) - 1 do
      if DataController.Summary.GetGroupSummaryInfo(ARow.Index, AItems,
        AValues, I) then
...
function TcxGridTableViewExport.GetGroupRowColumnIntersection(const ARowBounds
  : TRect; AColumn: TcxCustomGridTableItem): TRect;
var
  I: Integer;
begin
  Result := ARowBounds;
...
function TcxGridTableViewExport.GetIsSummaryUnderColumns
  (ARow: TcxGridGroupRow): Boolean;
begin
  Result := (OptionsView.GroupSummaryLayout <> gslStandard) and
    (ARow.GroupedColumnCount = 1);
end;

function TcxGridTableViewExport.GetPatternParams(ARecord: TcxCustomGridRecord;
  AItem: TcxExportVisualItem): TcxViewParams;
begin
...
function TcxGridTableViewExport.GetPreviewHeight
  (ARow: TcxCustomGridRecord): Integer;
var
  R: TRect;
  ...
  begin
    ...
    begin
      ...
      with TcxScreenCanvas.Create do
        try
          TextExtent(VarToStr(GetViewItemValue(ARow, AColumn)), R,
            cxWordBreak and not cxSingleLine);
        finally
          Free;
        end;
...
function TcxGridTableViewExport.HasDetailViewSelectedRecords
  (AView: TcxCustomGridView): Boolean;
begin
  Result := (AView is TcxCustomGridTableView) and
    ((TcxCustomGridTableView(AView).Controller.SelectedRecordCount > 0) or
    AView.IsMaster and HasMasterViewSelectedChildren(TcxGridTableView(AView)));
end;

function TcxGridTableViewExport.HasMasterRowSelectedChildren
  (AMasterRow: TcxGridMasterDataRow): Boolean;
begin
...
function TcxGridTableViewExport.HasMasterViewSelectedChildren
  (AView: TcxGridTableView): Boolean;
var
  I: Integer;
  ARow: TcxCustomGridRow;
begin
  ...
  begin
    ...
    if ARow.IsData and HasMasterRowSelectedChildren(TcxGridMasterDataRow(ARow))
    then
    begin
      Result := True;
      Break;
...
function TcxGridTableViewExport.IsColumnContainsFooterItem
  (AItem: TcxDataSummaryItem; AColumn: TcxGridColumn): Boolean;
begin
  Result := (AItem.ItemLink = AColumn) and (AItem.Position = spFooter);
end;

function TcxGridTableViewExport.IsDataVisualItem
  (AItem: TcxExportVisualItem): Boolean;
begin
  Result := inherited IsDataVisualItem(AItem) or
    (AItem.Data2 is TcxDataSummaryItem);
end;
...
function TcxGridTableViewExport.NeedProcessRecord
  (ARecord: TcxCustomGridRecord): Boolean;
begin
  Result := inherited NeedProcessRecord(ARecord) or
    ARecord.GridView.IsMaster and ARecord.IsData and
...
procedure TcxGridTableViewExport.ProcessGroupSummaryItem(ARow: TcxGridGroupRow;
  AValues: PVariant; AIndex: Integer; const ABounds: TRect;
  const ARowViewParams: TcxViewParams);
var
  ADisplayText: string;
...
begin
  ...
  if (ASummaryItem.Column <> nil) and not ASummaryItem.Column.ActuallyVisible
  then
    Exit;
  ...
  if AItem.Column <> nil then
  begin
    Styles.GetGroupSummaryCellContentParams(ARow, ASummaryItem,
      AItem.ViewParams);
...
procedure TcxGridTableViewExport.ProcessGroupSummaryItems(ARow: TcxGridGroupRow;
  ABounds: TRect);
var
  I: Integer;
  AItem: TcxExportGroupSummaryItem;
...
begin
  ...
  begin
    ...
    begin
      AItem.Text := AItem.Text + AItems.Separator + ' ' + GroupSummaryItems
        [I + 1].Text;
      AItem.Value := Null;
  ...
  begin
    GroupSummaryItems[0].Text := ARow.DisplayCaption + ' ' + AItems.BeginText +
      GroupSummaryItems[0].Text + AItems.EndText;
  ...
  begin
    ...
    begin
      for I := 0 to ARow.GroupedColumnCount - 1 do
        TcxCustomGridTableItemAccess.DoGetDisplayText(ARow.GroupedColumns[I],
          ARow, AItem.Text);
  ...
  begin
    ...
    begin
      if (I = 0) and (Column = nil) and
        (Bounds.Left >= GroupSummaryItems[I + 1].Bounds.Left) then
...
procedure TcxGridTableViewExport.ProduceHeadersContainer(var ATop,
  ALeft: Integer; AViewInfo: TcxGridColumnContainerViewInfo);
var
  AText: string;
...
begin
  ...
  begin
    ...
    AItem.Recalculate;
    R := cxRectOffset(AItem.RealBounds, GetColumnOffset(AItem.Column),
      AOffsetY);
    ...
    AVisualItem := AddVisualItemEx(AVisibleRect, AText, AItem.Params,
      AItem.AlignmentHorz, cxBordersAll, clBtnShadow);
...
begin
  for I := 0 to PatternsList.Count - 1 do
    ...
    begin
      ...
      ABounds.Bottom := (Bounds.Bottom div ARowHeight) *
        (AHeight div RecordRowCount);
      Bounds := ABounds;
    end;
...
function TcxGridTableViewExport.GetGroupSummaryItem(AIndex: Integer)
  : TcxExportGroupSummaryItem;
begin
...
function TcxGridTableViewExport.GetPattern(AIndex: Integer)
  : TcxExportVisualItem;
begin
...
procedure TcxGridBandedTableViewExport.CreateBandHeaders(var ATop: Integer;
  AForRootBands: Boolean);

  function IsFirstInGroup(ABand: TcxGridBand): Boolean;
  begin
    if ABand.IsRoot then
      Result := (ABand.VisibleRootIndex = 0) or
        (ABand.Bands.VisibleRootItems[ABand.VisibleRootIndex - 1].FixedKind <>
        ABand.FixedKind)
    else
      Result := (ABand = ABand.ParentBand.VisibleChildBands[0]) and
        IsFirstInGroup(ABand.ParentBand);
  end;
...
begin
  ...
  begin
    ...
    if (AForRootBands and (ABandViewInfo.Band.ParentBand <> nil)) or
      (not AForRootBands and (ABandViewInfo.Band.ParentBand = nil)) then
      Continue;
    if not AForRootBands then
      AOffsetX := ABandViewInfo.Bounds.Left + GetParentBandOffset
        (ABandViewInfo.Band.ParentBand);
    ...
    if (ABandHeader.Band.FixedKind = fkRight) and
      IsFirstInGroup(ABandHeader.Band) then
...
begin
  Result := Integer(GetPatternByBand(TcxGridBandedColumn(AColumn)
    .Position.Band).Data2);
end;
...
function TcxGridBandedTableViewExport.GetIsSummaryUnderColumns
  (ARow: TcxGridGroupRow): Boolean;
begin
  Result := inherited GetIsSummaryUnderColumns(ARow) and
    (GridView.Bands.Layout = blNonFixed) and
    (GridView.Bands.VisibleRowCount = 1);
end;

function TcxGridBandedTableViewExport.GetParentBandOffset
...
function TcxGridBandedTableViewExport.GetPatternByBand(ABand: TcxGridBand)
  : TcxExportVisualItem;
var
  I: Integer;
begin
...
begin
  ...
  begin
    ...
    try
      TcxGridBand(AItem.Data).Styles.GetViewParams(bsContent, ACellPos,
        nil, Result);
    finally
      ACellPos.Free;
    end;
...
begin
  Result := 0;
  if not AContainer.Visible then
    Exit;
  Dec(ATop, AContainer.Bounds.Top);
  Dec(ALeft, AContainer.Bounds.Left);
  R := AContainer.Bounds;
  AddVisualItemEx(cxRectOffset(AContainer.Bounds, ALeft, ATop), AContainer.Text,
    AContainer.Params, AContainer.AlignmentHorz, [], clDefault, True);
  Result := 0;
  ...
  begin
    ...
    AVisualItem := AddVisualItemEx(R, AItem.Text, AItem.Params,
      AItem.AlignmentHorz, cxBordersAll, clBtnShadow);
...
function TcxGridBandedTableViewExport.GetOptionsView
...
begin
  ...
  if HasIndent then
    AAdapter.AddRangeHorz(Bounds.Left + CategoryIndent,
      Bounds.Left + CategoryIndent);
  if HasSeparator then
    AAdapter.AddRangeVert(Bounds.Top + SeparatorWidth,
      Bounds.Top + SeparatorWidth);
  if ShowCaption then
    AAdapter.AddRangeHorz(Bounds.Left + CaptionWidth,
      Bounds.Left + CaptionWidth);
...
constructor TcxExportCard.Create(AOwner: TcxGridCardViewExport;
  ACard: TcxGridCard);
begin
  FCard := ACard;
...
procedure TcxExportCard.CalculateLayersCaptionWidth(AWidths: TcxExportIntList;
  AFistRowInLayerOnly: Boolean);
var
  I, J: Integer;
begin
...
function TcxExportCard.GetRowCaptionWidth(ALayerIndex, ARowIndex: Integer;
  AMaxWidth: Integer = 0): Integer;
var
...
begin
  ...
  if ARow.ShowCaption and ARow.ShowData then
    Inc(Result, Owner.TextWidthEx(ARow.CaptionStyle, ARow.VisibleCaption) +
      cxTextOffset * 2);
  Result := Max(Result, AMaxWidth);
end;

procedure TcxExportCard.SetLayersCaptionWidth(AWidths: TcxExportIntList;
  AFistRowInLayerOnly: Boolean);
var
...
procedure TcxExportCard.SetRowCaptionWidth(ALayerIndex, ARowIndex,
  AWidth: Integer);
begin
...
function TcxExportCard.AddRow(ALayerIndex: Integer;
  ACardViewInfo: TcxGridCardViewInfo; ARow: TcxGridCardViewRow)
  : TcxExportCardRow;
var
...
begin
  ...
  if ARow.Options.ShowCaption then
    Result.CaptionStyleIndex := Owner.RegisterViewParams(Result.CaptionStyle,
      ARow.CaptionAlignmentHorz);
  Result.DataStyleIndex := Owner.RegisterViewParams(Result.DataStyle,
    Result.DataAlignment);
  if Result.HasIndent then
    Result.IndentStyleIndex := Owner.RegisterViewParams(Result.IndentStyle);
  Result.Bounds := cxRectBounds(Bounds.Left, 0, ARow.Position.Width,
  ...
  if ACellViewInfo <> nil then
    Result.Bounds := cxRectSetHeight(Result.Bounds,
      ACellViewInfo.ContentHeight);
...
begin
  ...
  CalculateCardRowWidths(AWidths, (Bounds.Right - Bounds.Left) -
    BorderWidth * 2);
...
begin
  if Owner.OptionsView.CategorySeparatorWidth = 0 then
    Exit;
  for ALayerIndex := 1 to LayerCount - 1 do
  begin
...
begin
  Result := not Owner.IsSimpleLayout and (LayerCount > 1) and
    (SeparatorWidth > 0);
end;
...
begin
  if Owner.IsHorizontalRows then
  begin
    Result := cxRectSetTop(Bounds, Rows[AIndex, 0].Bounds.Bottom,
      SeparatorWidth);
    InflateRect(Result, -BorderWidth, 0);
  end
  else
  begin
    Result := cxRectSetLeft(Bounds, Rows[AIndex, 0].Bounds.Right,
      SeparatorWidth);
    InflateRect(Result, 0, -BorderWidth);
  end;
end;

function TcxExportCard.GetRow(ALayerIndex, ARowIndex: Integer)
  : TcxExportCardRow;
begin
...
procedure TcxExportCardLayoutBuilder.BuildLayout(ACard: TcxGridCard;
  AExportCard: TcxExportCard);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      ExportCard.FBounds.Bottom := Max(ExportCard.FBounds.Bottom,
        ATop + ExportCard.BorderWidth);
    end;
  end;
end;

function TcxExportCardLayoutBuilder.GetLayerIndex
...
begin
  ...
  ACardsViewInfo := TcxGridCardsViewInfoAccess
    (ACardViewViewInfo.RecordsViewInfo);
  ACardViewInfo := TcxGridCardViewInfo
    (ACardsViewInfo.GetRecordViewInfo(ExportCard.Card.Index, ANewlyCreated));
  try
...
function TcxGridCardViewExport.AddExportCard(AColumnPosition,
  ARowPosition: Integer; ACard: TcxGridCard): TcxExportCard;
begin
  ...
  with GridView.ViewInfo do
    try
      if LayoutDirection = ldVertical then
        ColumnCardCount :=
          Min(CardCount, Max(1, (Bounds.Right - Bounds.Left) div CardWidth))
      else
        RowCardCount := Min(CardCount,
          Max(1, (Bounds.Bottom - Bounds.Top) div CardHeight));
    except
      on EDivByZero do;
    end;
  if LayoutDirection = ldHorizontal then
...
function TcxGridCardViewExport.CreateCardLayoutBuilder
  : TcxExportCardLayoutBuilder;
begin
...
begin
  ...
  try
    ...
    begin
      ARowHeight := Max(ARowHeight, cxRectHeight(AddExportCard(AColumnPosition,
        ARowPosition, Cards[AIndex]).Bounds));
      if LayoutDirection = ldHorizontal then
      begin
    ...
    Adapter.AddBounds
      (cxRectBounds(cxPointOffset(Adapter.GetBoundingRect.BottomRight,
      CardIndent, CardIndent), 0, 0));
  finally
    ProgressHelper.EndStage;
  end;
end;
...
begin
  RealBoundsToLogicalBoundsEx(ARow.Bounds, ACardLogicalBounds,
    ARowLogicalBounds);
  if ARow.HasIndent then
    SetRealCellStyle(ARow.IndentBounds, ARowLogicalBounds,
      ARow.IndentStyleIndex);
  if ARow.HasSeparator then
    SetRealCellStyle(ARow.SeparatorBounds, ARowLogicalBounds,
      CategorySeparatorStyleIndex);
  if ARow.ShowCaption then
    SetRealCellStyleAndValue(ARow.CaptionBounds, ARowLogicalBounds,
      ARow.CaptionStyleIndex, ARow.VisibleCaption);
  if ARow.Row.Options.ShowData then
    SetRealCellStyleAndValueEx(ARow.DataBounds, ARowLogicalBounds,
      ARow.DataStyleIndex, ACard.Card, ARow.Row);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      RealBoundsToLogicalBounds(cxRectInflate(ACard.Bounds, -CardBorderWidth,
        -CardBorderWidth), ALogicalBounds);
      ...
      begin
        for ARowIndex := 0 to ACard.RowCount[ALayerIndex] - 1 do
          ExportCardRow(ACard, ACard.Rows[ALayerIndex, ARowIndex],
            ALogicalBounds);
        if ACard.HasSeparators and (ALayerIndex < (ACard.LayerCount - 1)) then
          SetRealCellStyle(ACard.LayerSeparators[ALayerIndex], ALogicalBounds,
            LayerSeparatorStyleIndex);
      end;
      ProgressHelper.NextTask;
    end;
...
begin
  ...
  if LayoutDirection = ldHorizontal then
  begin
    Adapter.RealRangeToLogicalRangeHorz(FCardSeparators[AIndex * 2],
  ...
  begin
    Adapter.RealRangeToLogicalRangeVert(FCardSeparators[AIndex * 2],
      FCardSeparators[AIndex * 2 + 1], AIndex1, AIndex2);
...
function TcxGridCardViewExport.GetLayoutDirection
...
var
  AItem: TcxExportVisualItem;
begin
  AItem := AddVisualItemEx(ABounds, '', AViewParams, taLeftJustify, [],
    clDefault, True);
...
begin
  inherited DoCreateExportCells;
  try
    if GridView.OptionsView.ViewMode in [lvvmSingleRecord, lvvmSingleRow,
      lvvmSingleColumn] then
      AColumnCardCount := 1
    else
  ...
  try
    ...
    begin
      FCard := Cards[ACard];
      FCardViewInfo := TcxGridLayoutViewRecordViewInfo.Create
        (GridView.ViewInfo.RecordsViewInfo, FCard);
      try
        FCardViewInfo.Calculate(-10000, FCardBounds.Top, RecordWidth,
          RecordHeight);
        FCardBounds := cxRectSetSize(FCardBounds, FCardViewInfo.Width,
          FCardViewInfo.Height);
        ...
        FCardBounds.Left := FCardBounds.Right +
          GridView.OptionsView.RecordIndent;
        if (ACard + 1) mod AColumnCardCount = 0 then
        begin
          OffsetRect(FCardBounds, 0, ACardsRowHeight +
            GridView.OptionsView.RecordIndent);
          FCardBounds.Left := 0;
          ACardsRowHeight := 0;
        end;
...
procedure TcxGridLayoutViewExport.ExportEditItem(const ABounds: TRect;
  AInfo: TcxGridLayoutViewItemViewInfo);
var
  AViewParams: TcxViewParams;
begin
  AddVisualDataItem(ABounds, Adapter.RegisterStyle(GetContentParams(Card,
    AInfo.Item, AViewParams)), Card, AInfo.Item);
end;
...
begin
  ...
  begin
    ...
    AItem := AddVisualItemEx(R, '', AParams, taLeftJustify, [],
      clDefault, True);
  ...
  if AInfo is TcxGridLayoutItemViewInfo then
    ExportEditItem(GetRelativeBounds(TcxGridLayoutItemViewInfo(AInfo)
      .ControlViewInfo.Bounds), TcxGridLayoutItemViewInfo(AInfo)
      .GridItemViewInfo);
...
procedure TcxGridLayoutViewExport.GetItemBordersInfo(const AItemInfo
  : TdxCustomLayoutItemViewInfo; AItem: TcxExportVisualItem);
begin
  if not TdxLayoutItemViewInfoAccess(AItemInfo).HasBorder and
    (AItemInfo.ParentViewInfo <> nil) then
  begin
    AItem.ItemBorders := [];
...
function TcxGridLayoutViewExport.GetParams
  (AItemInfo: TdxCustomLayoutItemViewInfo): TcxViewParams;
begin
  Result := Background;
  Result.Color := AItemInfo.Color;
end;

function TcxGridLayoutViewExport.GetRelativeBounds(const AItemBounds
  : TRect): TRect;
begin
  Result := cxRectOffset(cxRectOffset(AItemBounds, CardViewInfo.Bounds.TopLeft,
    False), CardBounds.TopLeft);
end;

function TcxGridLayoutViewExport.HasCaption
  (AInfo: TdxCustomLayoutItemViewInfo): Boolean;
begin
  Result := not(TdxLayoutItemViewInfoAccess(AInfo)
    .Item.CaptionOptions is TdxLayoutNonLabeledItemCaptionOptions)
end;
...
function TcxGridLayoutViewExport.GetCard(AIndex: Integer)
...
function TcxGridWinExplorerViewExport.GetRecord(ARecordIndex: Integer)
  : TcxGridWinExplorerViewCustomRecord;
begin
  Result := TcxGridWinExplorerViewCustomRecord(inherited Records[ARecordIndex]);
end;

procedure TcxGridWinExplorerViewExport.AddExportDataCell
  (AViewInfo: TcxGridWinExplorerViewCustomCellViewInfo);
var
  AParams: TcxViewParams;
  AStyle: TcxCacheCellStyle;
  AStyleIndex: Integer;
begin
  ...
  AddVisualDataItem(AViewInfo.Bounds, AStyleIndex, AViewInfo.GridRecord,
    AViewInfo.Item);
end;

procedure TcxGridWinExplorerViewExport.AddExportDataRecord
  (AViewInfo: TcxGridWinExplorerViewRecordViewInfo);
begin
...
procedure TcxGridWinExplorerViewExport.AddExportGroupRecord
  (AViewInfo: TcxGridWinExplorerViewGroupRecordViewInfo);
var
  AViewParams: TcxViewParams;
begin
  GridView.Styles.GetGroupParams(AViewInfo.GridRecord, nil, AViewParams);
  AddVisualItemEx(AViewInfo.Bounds, AViewInfo.Text, AViewParams, taLeftJustify,
    cxBordersAll);
end;

procedure TcxGridWinExplorerViewExport.AddExportRecord
...
begin
  if ARecord.IsData then
    ...
    AddExportGroupRecord(TcxGridWinExplorerViewGroupRecordViewInfo
      (ARecordViewInfo));
end;
...
begin
  ...
  try
    ...
    begin
      ...
      ARecordViewInfo := ARecordAccess.GetViewInfoClass.Create
        (ARecordsViewInfo, ARecord);
      try
        if (I > 0) and
          ((APos.X + ARecordViewInfo.Width > AStartPos.X + AContentWidth) or
...
begin
  ...
  try
    ...
    EnumCells(procedure(AColumn, ARow: Integer; const AValue: Variant)
      begin
        AColumnWidths[AColumn] := Max(AColumnWidths[AColumn],
          cxScreenCanvas.TextWidth(AValue));
      end);
  finally
...
procedure TcxGridChartViewExport.EnumCells
  (AProc: TcxGridChartViewExportEnumCellsProc);
var
...
constructor TcxGridViewDataAbstractExport.Create(const AFileName: string;
AGridView: TcxCustomGridView; AGrid: TcxCustomGrid;
AViewInfo: TcxCustomGridViewInfo; AAdapter: IcxGridExportProviderAdapter;
AHandler: TObject);
begin
...
begin
  ...
  if Supports(GridView, IcxDataControllerConditionalFormattingProviderOwner,
    AIntf) then
    FConditionalFormattingRules.Initialize(GridView,
      AIntf.GetConditionalFormattingProvider);
end;

{ TcxGridViewDataCustomExport }

function TcxGridViewDataCustomExport.CalculateBounds(ALeftOffset,
  ATopOffset: Integer): TRect;
begin
  Result := cxEmptyRect;
end;

function TcxGridViewDataCustomExport.CreateExportCell(const ABounds: TRect;
AClass: TcxExportCustomItemClass): TcxExportCustomItem;
begin
...
procedure TcxGridViewDataCustomExport.ExportCell(ACell: TcxExportCustomItem;
ARect: TRect);
var
  ADisplayText: string;
begin
  ...
  SetRealCellStyleAndValue(ACell.Bounds, ARect, GetStyleIndex(ACell),
    ADisplayText);
end;

function TcxGridViewDataCustomExport.GetItemBounds(ALeftPos,
  ATopPos: Integer): TRect;
begin
  Result := Rect(ALeftPos, ATopPos, ALeftPos + 1, ATopPos + 1);
end;

function TcxGridViewDataCustomExport.GetStyleIndex
  (ACell: TcxExportCustomItem): Integer;
begin
...
procedure TcxGridDataExportRanges.AddFrom(ARanges: TcxGridDataExportRanges;
...
function TcxGridDataExportRanges.CreateRange(AStart: Integer)
  : TcxGridDataExportRange;
begin
...
procedure TcxGridDataExportRanges.FinishRange(ARange: TcxGridDataExportRange;
AFinish: Integer);
begin
...
function TcxGridDataExportRanges.GetItem(AIndex: Integer)
  : TcxGridDataExportRange;
begin
  Result := TcxGridDataExportRange(inherited Items[AIndex]);
end;

procedure TcxGridDataExportRanges.SetItem(AIndex: Integer;
ARange: TcxGridDataExportRange);
begin
...
constructor TcxGridDataExportFooterSubTotalFormulaRanges.Create
  (AParentRow: TcxCustomGridRecord);
begin
  inherited Create;
  FParent := AParentRow;
end;

function TcxGridDataExportFooterSubTotalFormulaRanges.IsParent
  (AParentRow: TcxCustomGridRecord): Boolean;
var
...
constructor TcxGridDataExportFooterSubTotalFormulaBuilder.Create
  (AOwnsObjects: Boolean; ACapacity: Integer);
begin
  inherited Create(AOwnsObjects, ACapacity);
  FListSeparator := ',';
end;

function TcxGridDataExportFooterSubTotalFormulaBuilder.First
  : TcxGridDataExportFooterSubTotalFormulaRanges;
begin
  Result := TcxGridDataExportFooterSubTotalFormulaRanges(inherited First);
end;

function TcxGridDataExportFooterSubTotalFormulaBuilder.Last
  : TcxGridDataExportFooterSubTotalFormulaRanges;
begin
  Result := TcxGridDataExportFooterSubTotalFormulaRanges(inherited Last);
end;

function TcxGridDataExportFooterSubTotalFormulaBuilder.AddRanges
  (AParentRow: TcxCustomGridRecord)
  : TcxGridDataExportFooterSubTotalFormulaRanges;
begin
...
function TcxGridDataExportFooterSubTotalFormulaBuilder.Build
  (AParentRow: TcxCustomGridRecord; AColumnIndex: Integer;
AKind: TcxSummaryKind): string;
const
  ASubTotalKinds: array [TcxSummaryKind] of Char = ('0', '9', '5', '4',
    '3', '1');
var
  I, J: Integer;
  AArea: TRect;
begin
  ...
  for I := 0 to Count - 1 do
    ...
    begin
      ...
      begin
        AArea := Rect(AColumnIndex, Items[I].Items[J].Start, AColumnIndex,
          Items[I].Items[J].Finish);
        Result := Result + ListSeparator + dxReferenceToString(AArea.Top,
          AArea.Left) + ':' + dxReferenceToString(AArea.Bottom, AArea.Right);
      end;
    end;
  Result := Format('=SUBTOTAL(%s%s)', [ASubTotalKinds[AKind], Result]);
end;

function TcxGridDataExportFooterSubTotalFormulaBuilder.FindRanges
  (AParentRow: TcxCustomGridRecord)
  : TcxGridDataExportFooterSubTotalFormulaRanges;
var
  I: Integer;
...
procedure TcxGridDataExportFooterSubTotalFormulaBuilder.FinishRange
  (AFinish: Integer);
begin
  Last.FinishRange(AFinish);
end;

function TcxGridDataExportFooterSubTotalFormulaBuilder.GetSubTotalDescription
  (AKind: TcxSummaryKind): string;
const
  ASubTotalDescriptions: array [TcxSummaryKind] of string = ('', '"SUM="',
    '"MIN="', '"MAX="', '', '"AVG="');
begin
...
function TcxGridDataExportFooterSubTotalFormulaBuilder.IsParentForRanges
...
procedure TcxGridDataExportFooterSubTotalFormulaBuilder.StartRange
  (AParentRow: TcxCustomGridRecord; AStart: Integer);
var
...
function TcxGridDataExportFooterSubTotalFormulaBuilder.GetItem(AIndex: Integer)
  : TcxGridDataExportFooterSubTotalFormulaRanges;
begin
  Result := TcxGridDataExportFooterSubTotalFormulaRanges
    (inherited Items[AIndex]);
end;

procedure TcxGridDataExportFooterSubTotalFormulaBuilder.SetItem(AIndex: Integer;
AObject: TcxGridDataExportFooterSubTotalFormulaRanges);
begin
...
procedure TcxGridDataExportOutlineGroupRange.Assign
  (Source: TcxGridDataExportRange);
begin
...
function TcxGridDataExportOutlineGroupRanges.First
  : TcxGridDataExportOutlineGroupRange;
begin
  Result := TcxGridDataExportOutlineGroupRange(inherited First);
end;

function TcxGridDataExportOutlineGroupRanges.Last
  : TcxGridDataExportOutlineGroupRange;
begin
  Result := TcxGridDataExportOutlineGroupRange(inherited Last);
end;

procedure TcxGridDataExportOutlineGroupRanges.FinishLevelRange(AFinish: Integer;
ALevel: Integer);
var
  I: Integer;
...
function TcxGridDataExportOutlineGroupRanges.GetRangeClass
  : TcxGridDataExportRangeClass;
begin
  Result := TcxGridDataExportOutlineGroupRange;
end;

procedure TcxGridDataExportOutlineGroupRanges.StartLevelRange(AIndex: Integer;
AStart: Integer; ALevel: Integer);
begin
  StartRange(AIndex, AStart);
  Items[AIndex].Level := ALevel;
end;

procedure TcxGridDataExportOutlineGroupRanges.StartLevelRange(AStart: Integer;
ALevel: Integer);
begin
  StartRange(AStart);
  Last.Level := ALevel;
...
function TcxGridDataExportOutlineGroupRanges.GetItem(AIndex: Integer)
  : TcxGridDataExportOutlineGroupRange;
begin
  Result := TcxGridDataExportOutlineGroupRange(inherited Items[AIndex]);
end;

procedure TcxGridDataExportOutlineGroupRanges.SetItem(AIndex: Integer;
AObject: TcxGridDataExportOutlineGroupRange);
begin
  inherited Items[AIndex] := AObject;
end;
...
begin
  inherited Create;
  FData := TObjectDictionary<TdxSpreadSheetCustomConditionalFormattingRule,
    TdxSpreadSheetAreaList>.Create([doOwnsValues]);
end;
...
procedure TcxGridDataConditionalFormattingRules.CellAdded(ASourceRow,
  ASourceColumn: Integer; const ATargetArea: TRect);
var
...
begin
  ...
  begin
    if ARule.Areas.Contains(ASourceRow, ASourceColumn) then
      FData.Items[ARule].Add(cxRectBounds(ATargetArea.Left,
        ATargetArea.Top, 0, 0));
  end;
end;

procedure TcxGridDataConditionalFormattingRules.Commit(AExportProvider
  : IcxExportProvider);
var
...
begin
  ...
  AFormatter := TcxGridDataConditionalFormattingReferencesFormatter.
    Create(Self);
  try
    APrevFormatter := TConditionalFormattingProviderAccess(FProvider)
      .FReferencesFormatter;
    try
      TConditionalFormattingProviderAccess(FProvider).FReferencesFormatter :=
        AFormatter;
      for ARule in FData.Keys do
      begin
...
procedure TcxGridDataConditionalFormattingRules.OptimizeAreas
  (AAreas: TdxSpreadSheetAreaList);

  function TryCombineAreas(const ASourceArea: TRect;
  var ATargetArea: TRect): Boolean;
  begin
    Result := ((ASourceArea.Top = ATargetArea.Top) and
      (ASourceArea.Bottom = ATargetArea.Bottom) and
      ((ASourceArea.Left = ATargetArea.Right + 1) or
      (ASourceArea.Right + 1 = ATargetArea.Left))) or
      ((ASourceArea.Left = ATargetArea.Left) and
      (ASourceArea.Right = ATargetArea.Right) and
      ((ASourceArea.Top = ATargetArea.Bottom + 1) or
      (ASourceArea.Bottom + 1 = ATargetArea.Top)));
...
constructor TcxGridDataConditionalFormattingReferencesFormatter.Create
  (AOwner: TcxGridDataConditionalFormattingRules);
begin
  FOwner := AOwner;
...
function TcxGridDataConditionalFormattingReferencesFormatter.ToString
  (const Area: TRect): string;
var
  AArea: TRect;
begin
  AArea := Area;
...
begin
  ...
  if AColumnIndex >= 0 then
    Result := Format('$%s1',
      [TdxSpreadSheetColumnHelper.NameByIndex(AColumnIndex + Anchors.X)])
  else
    Result := serRefError;
end;

function TcxGridDataConditionalFormattingReferencesFormatter.
  AbsoluteColumnIndexToVisibleColumnIndex(AIndex: Integer): Integer;
begin
  if FGridView = nil then
    Result := AIndex
  else if InRange(AIndex, 0, FGridView.ItemCount - 1) then
...
function TcxExportGridViewDetailDataItem.GetDetailExport
  : TcxGridViewDataCustomExport;
begin
  Result := TcxGridViewDataCustomExport(Data);
end;

procedure TcxExportGridViewDetailDataItem.SetDetailExport
  (AValue: TcxGridViewDataCustomExport);
begin
  Data := AValue;
end;

procedure TcxExportGridViewDetailDataItem.UpdateBoundByDetailBounds(ALeftOffset,
  ATopOffset: Integer);
begin
...
constructor TcxGridViewDataExport.Create(const AFileName: string;
AGridView: TcxCustomGridView; AGrid: TcxCustomGrid;
AViewInfo: TcxCustomGridViewInfo; AAdapter: IcxGridExportProviderAdapter;
AHandler: TObject = nil);
begin
...
function TcxGridViewDataExport.CalculateBounds(ALeftOffset,
  ATopOffset: Integer): TRect;
begin
  Result := cxRectOffset(inherited CalculateBounds(ALeftOffset, ATopOffset),
    ALeftOffset, ATopOffset);
  if HasHeader then
    Inc(Result.Bottom);
...
begin
  CreateExportCell(ABounds, ADisplayText, ARecord, AData,
    TcxExportGridViewDataItem);
end;

function TcxGridViewDataExport.CreateExportCell(const ABounds: TRect;
const ADisplayText: string; ARecord: TcxCustomGridRecord; AData: TObject;
AClass: TcxExportGridViewDataItemClass): TcxExportGridViewDataItem;
begin
  Result := TcxExportGridViewDataItem(inherited CreateExportCell(ABounds,
    ADisplayText, AClass));
...
procedure TcxGridViewDataExport.CreateExportDataRecord(var ALeft, ATop: Integer;
ARecord: TcxCustomGridRecord);
var
  AItem: TcxCustomGridTableItem;
  I: Integer;
begin
  ...
  begin
    ...
    CreateExportCell(GetItemBounds(ALeft + I, ATop),
      VarToStr(ARecord.Values[AItem.Index]), ARecord, AItem);
  end;
  Inc(ATop);
end;

procedure TcxGridViewDataExport.CreateExportGroupRecord(var ALeft,
  ATop: Integer; ARecord: TcxCustomGridRecord);
begin
  ...
  CreateExportCell(GetItemBounds(ARecord.Level, ATop),
    GetGroupDisplayText(ARecord), ARecord);
  Inc(ATop);
end;
...
begin
  ...
  begin
    ...
    CreateExportCell(GetItemBounds(ALeft + I, ATop), AText, nil,
      GridView.VisibleItems[I]);
  end;
  Inc(ATop);
end;

procedure TcxGridViewDataExport.CreateIndents(AStartLeft, AFinishLeft,
  AStartTop, AFinishTop: Integer);
var
  I, J: Integer;
begin
...
procedure TcxGridViewDataExport.ExportCell(ACell: TcxExportCustomItem;
ARect: TRect);
var
...
begin
  ...
  begin
    ...
    ConditionalFormattingRules.CellAdded(TcxExportGridViewDataItem(ACell)
      .GridRecord.RecordIndex, ATableItem.Index, ACell.Bounds);
    SetRealCellStyleAndValueEx(ACell.Bounds, ARect, GetStyleIndex(ACell),
      TcxExportGridViewDataItem(ACell).GridRecord, ATableItem);
  end
  else
    inherited ExportCell(ACell, ARect);
end;

function TcxGridViewDataExport.GetGroupDisplayText
  (ARecord: TcxCustomGridRecord): string;
begin
  Result := ARecord.DisplayTexts[-1];
end;

function TcxGridViewDataExport.GetStyleIndex
  (ACell: TcxExportCustomItem): Integer;
  ...
  begin
    ...
    begin
      Result := (TcxExportGridViewDataItem(ACell).GridRecord = nil) and
        (TcxExportGridViewDataItem(ACell).Data <> nil) or
        (TcxExportGridViewDataItem(ACell).GridRecord <> nil) and
        not TcxExportGridViewDataItem(ACell).GridRecord.IsData;
    end
    else if ACell is TcxExportDataItem then
...
procedure TcxGridTableViewDataExport.AddDetailOutlineGroupRanges
  (ADetailExport: TcxGridViewDataExport; ATop: Integer);
begin
...
function TcxGridTableViewDataExport.CalculateBounds(ALeftOffset,
  ATopOffset: Integer): TRect;
var
  I, J: Integer;
  ARealLevel: Integer;
  ...
  begin
    ...
    begin
      ...
      if TcxGridExportHelper.HasGroupFooter(GridView, ARecord, ARealLevel) then
        if DataController.Summary.GetGroupSummaryInfo(ARecord.Index, AItems,
          AValues) then
          Inc(Result.Bottom,
            TcxGridExportHelper.CalculateFooterLineCount(AItems));
    end;
  end;
  if HasFooter then
    Inc(Result.Bottom, TcxGridExportHelper.CalculateFooterLineCount
      (DataController.Summary.FooterSummaryItems));
end;

constructor TcxGridTableViewDataExport.Create(const AFileName: string;
AGridView: TcxCustomGridView; AGrid: TcxCustomGrid;
AViewInfo: TcxCustomGridViewInfo; AAdapter: IcxGridExportProviderAdapter;
AHandler: TObject);
begin
  inherited Create(AFileName, AGridView, AGrid, AViewInfo, AAdapter, AHandler);
  FFooterSubTotalFormulaBuilder :=
    TcxGridDataExportFooterSubTotalFormulaBuilder.Create(True, 1024);
end;

procedure TcxGridTableViewDataExport.CreateExportContent(var ALeft,
  ATop: Integer);
begin
...
procedure TcxGridTableViewDataExport.CreateExportDataRecord(var ALeft: Integer;
var ATop: Integer; ARecord: TcxCustomGridRecord);
var
  AParent: TcxCustomGridRecord;
begin
  ...
  if FooterSubTotalFormulaBuilder.HasActiveRange and
    (ARecord.IsLast or not GridView.ViewData.Records[ARecord.Index + 1].IsData)
  then
    FooterSubTotalFormulaBuilder.FinishRange(Offset.Y + ATop - 1);
end;

procedure TcxGridTableViewDataExport.CreateExportFooterByColumn(ALeft,
  ATop: Integer; AColumn: TcxGridColumn);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    if (AItem.Position <> spFooter) or (AItem.ItemLink <> AColumn) then
      Continue;
    AFormula := FooterSubTotalFormulaBuilder.Build(nil,
      Offset.X + ALeft + AColumn.VisibleIndex, AItem.Kind);
    ADisplayText := AItem.FormatValue(DataController.Summary.FooterSummaryValues
      [I], True);
    CreateExportFormulaCell(GetItemBounds(ALeft + AColumn.VisibleIndex, ATop),
      AFormula, ADisplayText, AItem);
    Inc(ATop);
  end;
end;

procedure TcxGridTableViewDataExport.CreateExportFooters(var ALeft,
  ATop: Integer);
var
...
begin
  AItem := TcxExportGridViewFormulaDataItem(CreateExportCell(ABounds,
    ADisplayText, TcxExportGridViewFormulaDataItem));
  AItem.Formula := AFormula;
  AItem.Data := AData;
end;

procedure TcxGridTableViewDataExport.CreateExportGroupFooterByColumn(ALeft,
  ATop: Integer; ARow: TcxCustomGridRecord; AColumn: TcxGridColumn);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AItem.Position <> spFooter) or (AItem.ItemLink <> AColumn) then
        Continue;
      AFormula := FooterSubTotalFormulaBuilder.Build(ARow,
        Offset.Y + ALeft + AColumn.VisibleIndex, AItem.Kind);
...
procedure TcxGridTableViewDataExport.CreateExportGroupRecord(var ALeft,
  ATop: Integer; ARecord: TcxCustomGridRecord);
begin
...
procedure TcxGridTableViewDataExport.CreateExportMasterDataRow(var ALeft,
  ATop: Integer; ARecord: TcxGridMasterDataRow);
var
...
begin
  ...
  begin
    ...
    try
      ...
      AItem := TcxExportGridViewDetailDataItem(CreateExportCell(cxEmptyRect, '',
        ARecord, GetExportDataClassByGridView(ADetailView).CreateFrom(Self,
        ADetailView, ADetailView.ViewInfo, Point(ALeftIndent, ATop)),
        TcxExportGridViewDetailDataItem));
      ...
      if AItem.DetailExport is TcxGridViewDataExport then
        AddDetailOutlineGroupRanges
          (TcxGridViewDataExport(AItem.DetailExport), ATop);
...
procedure TcxGridTableViewDataExport.CreateExportRecord(var ALeft,
  ATop: Integer; ARecord: TcxCustomGridRecord);
begin
...
procedure TcxGridTableViewDataExport.CreateExportRowFooter(var ALeft,
  ATop: Integer; ADataLevel, ALevel: Integer; ARow: TcxCustomGridRecord);
var
  I, ALineCount: Integer;
begin
  while ARow.Level > ADataLevel do
    ARow := ARow.ParentRecord;
  for I := 0 to GridView.VisibleColumnCount - 1 do
    CreateExportGroupFooterByColumn(ALeft, ATop, ARow,
      GridView.VisibleColumns[I]);
  ALineCount := GetGroupFooterLineCount(ARow);
  CreateIndents(0, ALeft - 1, ATop, ATop + ALineCount - 1);
  Inc(ATop, ALineCount);
end;

procedure TcxGridTableViewDataExport.CreateExportRowFooters(var ALeft,
  ATop: Integer; ARow: TcxCustomGridRecord);
var
  ALevel, ARealLevel: Integer;
begin
  for ALevel := 0 to ARow.Level do
...
procedure TcxGridTableViewDataExport.ExportCell(ACell: TcxExportCustomItem;
ARect: TRect);
begin
  if ACell is TcxExportGridViewDetailDataItem then
    ExportDetailCell(TcxExportGridViewDetailDataItem(ACell))
  else if (ACell is TcxExportDataItem) and
    (TcxExportDataItem(ACell).Data is TcxDataSummaryItem) then
    ExportFooterCell(TcxExportDataItem(ACell), ARect)
  else
    inherited ExportCell(ACell, ARect);
end;

procedure TcxGridTableViewDataExport.ExportDetailCell
  (ACell: TcxExportGridViewDetailDataItem);
begin
  try
...
procedure TcxGridTableViewDataExport.ExportFooterCell(ACell: TcxExportDataItem;
ARect: TRect);
var
  AFormula: string;
begin
  ...
  SetRealCellStyleAndFormula(ACell.Bounds, ARect, GetStyleIndex(ACell),
    AFormula, ACell.DisplayText, GetFooterFormatCode(ACell),
    FooterSubTotalFormulaBuilder.ListSeparator);
end;

function TcxGridTableViewDataExport.GetFooterFormatCode
  (ACell: TcxExportDataItem): string;
var
  ACol, ARow: Integer;
  ASummaryItem: TcxDataSummaryItem;
begin
  ...
  if not(ASummaryItem.Kind in [skNone, skCount]) then
    Result := FooterSubTotalFormulaBuilder.GetSubTotalDescription
      (ASummaryItem.Kind) + Adapter.GetFormatCode(ACol, ARow)
  else
    Result := '';
end;
...
function TcxGridTableViewDataExport.GetGroupDisplayText
  (ARecord: TcxCustomGridRecord): string;
begin
  Result := TcxGridGroupRow(ARecord).DisplayText;
end;

function TcxGridTableViewDataExport.GetGroupFooterLineCount
  (ARow: TcxCustomGridRecord): Integer;
var
  I: Integer;
  AItems: TcxDataSummaryItems;
...
begin
  Result := GridView.OptionsView.Footer and
    (DataController.Summary.FooterSummaryItems.Count > 0);
end;
...
function TcxGridWinExplorerViewDataExport.GetGroupDisplayText
  (ARecord: TcxCustomGridRecord): string;
begin
...
function TcxGridChartViewDataExport.CalculateBounds(ALeftOffset,
  ATopOffset: Integer): TRect;
begin
  Result := inherited CalculateBounds(ALeftOffset, ATopOffset);
...
begin
  ...
  begin
    ...
    if HasCategory then
      Inc(ALeft);
    CreateExportCell(GetItemBounds(ALeft, 0),
      GridView.VisibleSeries[I].GetDisplayText, TcxExportTextItem);
  end;
end;
...
begin
  TcxGridViewExportVisualItemListHelper.InitStartCapacity(VisualItemList,
    GridView.ViewData.VisibleCategoryCount, GridView.VisibleSeriesCount,
    GridView.IsDetail);
end;
...
class function TcxGridExportHelper.CalculateFooterLineCount
  (AItems: TcxDataSummaryItems): Integer;
var
  I, J, ACount: Integer;
begin
...
class function TcxGridExportHelper.HasGroupFooter(AGridView: TcxGridTableView;
ARow: TcxCustomGridRecord; var ALevel: Integer): Boolean;
var
...
begin
  ...
  if ARow is TcxGridGroupRow then
    Result := Result or (AGroupFootersMode = gfAlwaysVisible) and (ALevel = -1)
      and not ARow.Expanded and AGridView.Controller.CanShowGroupFooter
      (ARow.Level);
  if Result then
    ALevel := ARow.Level - ALevel - 1;
end;

class function TcxGridExportHelper.IsSameFooterItems(AItem1,
  AItem2: TcxDataSummaryItem): Boolean;
begin
