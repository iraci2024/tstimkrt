...
uses
  ...
  dxPSForm, dxPgsDlg, cxLookAndFeels, cxEdit, dxLayoutContainer,
...
begin
  ...
  if Style = nil then
    Exit;
  try
    Result := Style.PageSetup(0, True, True, FPreviewBtnClicked,
      FPrintBtnClicked);
  ...
  if PreviewBtnClicked or PrintBtnClicked then
    ModalResult := mrOK
  else if lbxPrintStyles.ItemIndex <> -1 then
...
begin
  ...
  begin
    if lbxPrintStyles.ItemIndex <> -1 then
      SavedStyle := TBasedxPrintStyle(lbxPrintStyles.Items.Objects
        [lbxPrintStyles.ItemIndex])
    else
...
var
  ...
  S: string;
begin
  Style := TBasedxPrintStyle(lbxPrintStyles.Items.Objects
    [lbxPrintStyles.ItemIndex]);

  if not Style.BuiltIn then
  begin
    S := Format(cxGetResourceString(@sdxDefinePrintStylesWarningDelete),
      [Style.StyleCaption]);
    if MessageWarning(S) then
      Style.Free;
  end
  else
...
begin
  ...
  begin
    ...
    if ItemIndex > -1 then
      miReset.Caption := FBtnDelCaptions
        [TBasedxPrintStyle(Items.Objects[ItemIndex]).BuiltIn];
...
begin
  ...
  begin
    ...
    btnReset.Caption := FBtnDelCaptions[(PrintStyle <> nil) and
      PrintStyle.BuiltIn];
...
procedure TdxfmDefinePrintStyles.lbxPrintStylesDrawItem(AControl: TcxListBox;
