...
uses
  dxCore, cxCustomData, cxGridCustomView, cxGridTableView, cxGridMenuOperations,
  Classes;
...
type
  ...
  TcxGridGroupRowSortByGroupValuesOperation = class
  ...
  TcxGridGroupRowSortBySummaryCaptionOperation = class
    (TcxGridGroupRowPopupMenuOperation)
  public
    constructor Create; override;
  end;

  TcxGridGroupRowSortBySummaryOperation = class
    (TcxGridGroupRowPopupMenuOperation)
  private
...
begin
  if (GridPopupMenu.HitTest is TcxGridRecordHitTest) and
    (TcxGridRecordHitTest(GridPopupMenu.HitTest).GridRecord is TcxGridGroupRow)
  then
    Result := TcxGridGroupRow(TcxGridRecordHitTest(GridPopupMenu.HitTest)
      .GridRecord)
  else
    Result := nil;
end;
...
procedure TcxGridGroupRowSortBySummaryOperation.SetSummaryItem
...
procedure TcxGridGroupRowPopupMenuOperations.SetGroupRow
  (Value: TcxGridGroupRow);
begin
  FGroupRow := Value;
  RecreateItems;
end;
...
begin
  ...
  for I := 0 to ASummaryItems.Count - 1 do
    if not Supports(ASummaryItems[I], IcxGridSummaryItem, AGridSummaryItem) or
      ...
      TcxGridGroupRowSortBySummaryOperation
        (AddItem(TcxGridGroupRowSortBySummaryOperation)).SummaryItem :=
        ASummaryItems[I];
end;

function TcxGridGroupRowPopupMenuOperations.CanProcess
