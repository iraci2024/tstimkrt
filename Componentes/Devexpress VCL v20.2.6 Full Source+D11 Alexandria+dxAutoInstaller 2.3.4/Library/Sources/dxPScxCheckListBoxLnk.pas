...
uses
  ...
  Dialogs, Menus, dxCore, dxPSCore, cxPC,
  Types, dxPSForm, dxExtCtrls, dxPSBaseGridLnk, dxPSGrLnks, cxCheckListBox,
  cxLookAndFeelPainters, cxControls, cxContainer,
  cxEdit, cxCheckBox, cxLabel, cxTextEdit, cxButtons, cxGraphics, cxMaskEdit,
  cxDropDownEdit, cxColorComboBox,
  cxLookAndFeels, cxImageComboBox, dxPSReportLinkDesignWindow,
  dxLayoutControlAdapters, dxLayoutLookAndFeels,
  dxLayoutContainer, cxClasses, dxLayoutControl, dxLayoutcxEditAdapters,
  cxImage, cxGeometry;

type
  TcxCheckListBoxReportLinkOption = (chlbxoBorder, chlbxoHorzLines,
    chlbxoFlatCheckMarks);
  ...
  protected
    ...
    property cxCustomCheckListBox: TcxCustomCheckListBox
      read GetcxCustomCheckListBox;
  public
    procedure Assign(Source: TPersistent); override;
    property Options: TcxCheckListBoxReportLinkOptions read FOptions
      write SetOptions default [chlbxoBorder .. chlbxoFlatCheckMarks];
  end;
...
const
  cxDefaultCheckListBoxOptions = [chlbxoBorder, chlbxoHorzLines,
    chlbxoFlatCheckMarks];

  cxCheckListBoxStrings: array [0 .. 4] of string = ('Sample Text Row #1',
    'Sample Text Row #2', 'Sample Text Row #3', 'Sample Text Row #4',
    'Sample Text Row #5');
...
uses
  SysUtils, Forms, dxPSRes, dxPSGlbl, dxPSUtl, dxPrnDev, dxPScxEditorLnks,
  dxPSImgs,
...
begin
  ...
  if TcxCustomCheckListBoxAccess(cxCustomCheckListBox).LookAndFeel.Kind = lfUltraFlat
  then
...
begin
  if cxCustomCheckListBox <> nil then
    Result := cxContainer_GetInnerControl(cxCustomCheckListBox)
      as TCustomListBox
  else
    Result := nil;
end;

procedure TcxCustomCheckListBoxReportLink.AssignData(ACol, ARow: Integer;
...
begin
  ...
  begin
    CheckPos := ccpLeft;
    Checked := TcxCustomCheckListBoxAccess(cxCustomCheckListBox).Items[ARow]
      .State > cbsUnchecked;
    Enabled := (TcxCustomCheckListBoxAccess(cxCustomCheckListBox).Items[ARow]
      .State < cbsGrayed) and TcxCustomCheckListBoxAccess(cxCustomCheckListBox)
      .Items[ARow].Enabled;
...
function TcxCustomCheckListBoxReportLink.GetCellFontIndex(ACol,
  ARow: Integer): Integer;
begin
  Result := inherited GetCellFontIndex(ACol, ARow);
end;

function TcxCustomCheckListBoxReportLink.GetDataItemClass(ACol: Integer;
  ARow: Integer = 0): TdxReportCellDataClass;
begin
  Result := TdxReportCellCheckImage;
end;

function TcxCustomCheckListBoxReportLink.GetcxCustomCheckListBox
  : TcxCustomCheckListBox;
begin
  Result := TcxCustomCheckListBox(Component);
end;

procedure TcxCustomCheckListBoxReportLink.SetOptions
  (Value: TcxCheckListBoxReportLinkOptions);
begin
...
procedure TcxfmCheckListBoxDesignWindow.CreateControls;
  ...
  function CreatePreviewPane(AHost: TWinControl; AnOnPaint: TNotifyEvent)
    : TdxPSPaintPanel;
  begin
...
procedure TcxfmCheckListBoxDesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
const
  RowCount = 5;
type
  ...
  procedure CalculateViewInfos(out AViewInfos: TdxCheckRowViewInfos);
    ...
    procedure CalculateViewInfo(AnIndex: Integer;
      var AViewInfo: TdxCheckRowViewInfo);
    const
      States: array [0 .. RowCount - 1] of TCheckBoxState = (cbUnchecked,
        cbChecked, cbGrayed, cbUnchecked, cbChecked);
    var
      V: Integer;
    begin
      with AViewInfo do
      begin
        ...
        Font := TFont.Create;
        dxAssignFont(Font, ReportLink.GetCellFont(0, AnIndex), ScaleFactor,
        ...
        InflateRect(CheckRect, 0,
          -(CheckRect.Bottom - CheckRect.Top -
          ScaleFactor.Apply(dxPSGlbl.CheckHeight)) div 2);
        Inc(CheckRect.Left, ScaleFactor.Apply(2));
        CheckRect.Right := CheckRect.Left + ScaleFactor.Apply
          (dxPSGlbl.CheckWidth);
  ...
  procedure DrawCheckRows(const AViewInfos: TdxCheckRowViewInfos);
    ...
    procedure DrawCheckRow(const AViewInfo: TdxCheckRowViewInfo);
      ...
      procedure DrawCheck;
      const
        CheckStateMap: array [TCheckBoxState] of UINT = (DFCS_BUTTONCHECK,
      ...
      begin
        ...
        DrawFrameControl(ACanvas.Handle, AViewInfo.CheckRect,
          DFC_BUTTON, uState);
      end;
...
function TcxfmCheckListBoxDesignWindow.GetReportLink
  : TcxCustomCheckListBoxReportLink;
begin
  Result := inherited ReportLink as TcxCustomCheckListBoxReportLink;
end;

procedure TcxfmCheckListBoxDesignWindow.CMDialogChar
...
dxPSRegisterReportLink(TcxCheckListBoxReportLink, TcxCheckListBox,
  TcxfmCheckListBoxDesignWindow);

finalization

dxPSUnregisterReportLink(TcxCheckListBoxReportLink, TcxCheckListBox,
  TcxfmCheckListBoxDesignWindow);
