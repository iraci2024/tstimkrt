...
type
  ...
  strict private
    ...
    class function CreateAllignmentsTable
      : TdxNamedOrdinalDictionary<TdxHtmlImageAlignment>; static;
    class function AddAttributes: TdxAttributeKeywordTranslatorTable; static;
    class procedure ImageAlignmentKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure AlternativeTextKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure BorderWidthKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure ImageHeightKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure HorizontalIndentKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure IsImageOnServerKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure PreImageURLKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
  protected
    class procedure ImageURLKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure VerticalIndentKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure ImageWidthKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class function CalculateValueInfo(AImporter: TdxCustomHtmlImporter;
      const AValue: string): TdxValueInfo; static;
    ...
    function CalculateWidthUnitInfo(const AInfo: TdxValueInfo)
      : TdxWidthUnitInfo;
    procedure OpenTagProcessCore; override;
    function GetActualAlignment: TdxHtmlImageAlignment;
    procedure AppendPictureFloatingObjectContent
      (AImage: TdxOfficeImageReference; AAlignment: TdxHtmlImageAlignment;
...
class function TdxImageTag.CreateAllignmentsTable
  : TdxNamedOrdinalDictionary<TdxHtmlImageAlignment>;
begin
...
class procedure TdxImageTag.ImageAlignmentKeyword
...
class procedure TdxImageTag.AlternativeTextKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
begin
end;

class procedure TdxImageTag.BorderWidthKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
begin
end;

class procedure TdxImageTag.ImageHeightKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
var
  AImageTag: TdxImageTag;
begin
...
class procedure TdxImageTag.HorizontalIndentKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
begin
end;

class procedure TdxImageTag.IsImageOnServerKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
begin
end;

class procedure TdxImageTag.PreImageURLKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
begin
end;

class procedure TdxImageTag.ImageURLKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
var
  AImageTag: TdxImageTag;
begin
  ...
  begin
    try
      AImageTag.Uri := ATag.GetAbsoluteUri(TdxHtmlImporter(AImporter)
        .AbsoluteBaseUri, TdxUri.UnescapeDataString(AValue));
    except
    end;
  end;
end;

class procedure TdxImageTag.VerticalIndentKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
begin
end;

class procedure TdxImageTag.ImageWidthKeyword(AImporter: TdxCustomHtmlImporter;
  const AValue: string; ATag: TdxTagBase);
var
  AImageTag: TdxImageTag;
begin
...
class function TdxImageTag.CalculateValueInfo(AImporter: TdxCustomHtmlImporter;
  const AValue: string): TdxValueInfo;
var
...
function TdxImageTag.CalculateWidthUnitInfo(const AInfo: TdxValueInfo)
  : TdxWidthUnitInfo;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      Exit(TdxTagBaseHelper.GetRelativeWidth(Importer.UnitConverter,
        AInfo.&Unit, AInfo.Value, AFontSize, ARootDoubleFontSize));
    end;
    Result := TdxWidthUnitInfo.Create;
    Result.Value :=
      Max(0, Round(Importer.UnitConverter.PointsToModelUnitsF(AInfo.Value)));
...
var
  AWidth, AHeight: TdxWidthUnitInfo;
  AWidthInModelUnits, AHeightInModelUnits, AWidthInPixels,
    AHeightInPixels: Integer;
...
begin
  ...
  AWidthInPixels := Importer.UnitConverter.ModelUnitsToPixels
    (AWidthInModelUnits, DocumentModel.Dpi);
  AHeightInPixels := Importer.UnitConverter.ModelUnitsToPixels
    (AHeightInModelUnits, DocumentModel.Dpi);
  if Importer.DocumentModel.DocumentCapabilities.InlinePicturesAllowed then
  begin
    AImage := Importer.CreateUriBasedRichEditImage(Uri, AWidthInPixels,
      AHeightInPixels);
    if AImage <> nil then
      try
        AScaleX := 100.0;
        ...
        begin
          if (AImage.Image.Width > 0) and (AWidth.&Type <> TdxWidthUnitType.&Nil)
          then
            AScaleX := Importer.UnitConverter.ModelUnitsToTwips
              (AWidthInModelUnits) / AImage.Image.SizeInTwips.cx * 100;

          if (AImage.Image.Height > 0) and
            (AHeight.&Type <> TdxWidthUnitType.&Nil) then
            AScaleY := Importer.UnitConverter.ModelUnitsToTwips
              (AHeightInModelUnits) / AImage.Image.SizeInTwips.cy * 100;

          if (AHeight.&Type = TdxWidthUnitType.&Nil) and (AScaleX <> 100.0) then
          begin
            AScaleY := AScaleX;
            AHeightInModelUnits := Importer.UnitConverter.TwipsToModelUnits
              (AImage.SizeInTwips.cy);
            AHeightInModelUnits := Trunc(AHeightInModelUnits * AScaleY / 100.0);
          end
          else if (AWidth.&Type = TdxWidthUnitType.&Nil) and (AScaleY <> 100.0)
          then
          begin
            AScaleX := AScaleY;
            AWidthInModelUnits := Importer.UnitConverter.TwipsToModelUnits
              (AImage.SizeInTwips.cx);
            AWidthInModelUnits := Trunc(AWidthInModelUnits * AScaleX / 100.0);
          end;
        end;

        ADesiredSizeInModelUnits := TSize.Create(AWidthInModelUnits,
          AHeightInModelUnits);

        AAlignment := GetActualAlignment;
        if (Importer.Options.IgnoreFloatProperty) or
          ((AAlignment <> TdxHtmlImageAlignment.Left) and
          (AAlignment <> TdxHtmlImageAlignment.Right)) then
          Importer.AppendInlineImage(AImage, AScaleX, AScaleY,
            ADesiredSizeInModelUnits)
        else
          AppendPictureFloatingObjectContent(AImage, AAlignment,
            ADesiredSizeInModelUnits);
      finally
        AImage.Free;
      end;
    ...
    Importer.AppendText(' ');
  Importer.Position.CopyFrom(Importer.TagsStack[Importer.TagsStack.Count - 1]
    .OldPosition);
end;
...
procedure TdxImageTag.AppendPictureFloatingObjectContent
  (AImage: TdxOfficeImageReference; AAlignment: TdxHtmlImageAlignment;
  const ADesiredSizeInModelUnits: TSize);
var
...
begin
  ...
  ARun.FloatingObjectProperties.VerticalPositionType :=
    TdxFloatingObjectVerticalPositionType.Line;
  if AAlignment = TdxHtmlImageAlignment.Left then
  begin
    ARun.FloatingObjectProperties.TextWrapSide :=
      TdxFloatingObjectTextWrapSide.Right;
    ARun.FloatingObjectProperties.HorizontalPositionAlignment :=
      TdxFloatingObjectHorizontalPositionAlignment.Left;
    ARun.FloatingObjectProperties.HorizontalPositionType :=
  ...
  AOriginalSize := AImage.CalculateImageSizeInModelUnits
    (DocumentModel.UnitConverter);
  AContent.SetOriginalSize(AOriginalSize);

  if (ADesiredSizeInModelUnits.Width = 0) and
    (ADesiredSizeInModelUnits.Height = 0) then
