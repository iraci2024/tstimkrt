...
procedure dxSkinsPopulateSkinResources(AModule: HMODULE;
  AResNames, ASkinNames: TStringList);
function dxSkinsLoadedUserSkinHasMissingElements: Boolean;
function dxSkinsUserSkinGetLoadedSkinName(ADisplayName
  : Boolean = False): string;
function dxSkinsUserSkinLoadFromFile(const AFileName: string;
  const ASkinName: string = ''): Boolean;
function dxSkinsUserSkinLoadFromFileByIndex(const AFileName: string;
  ASkinIndex: Integer = 0): Boolean;
function dxSkinsUserSkinLoadFromResource(AInstance: THandle;
  const AResourceName: string; const ASkinName: string = ''): Boolean;
function dxSkinsUserSkinLoadFromStream(AStream: TStream;
  const ASkinName: string = ''): Boolean;
function dxSkinsUserSkinLoadFromStreamByIndex(AStream: TStream;
  ASkinIndex: Integer = 0): Boolean;
function dxSkinsUserSkinPopulateSkinNames(AStream: TStream; AList: TStrings;
  ADisplayNames: Boolean = False): Boolean; overload;
...
function EnumResNameProc(HMODULE: HMODULE; lpszType: LPCTSTR; lpszName: LPTSTR;
  AData: TdxListPair): Boolean; stdcall;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Result := Result and (AData.ResNames.IndexOf(lpszName) = -1) and
...
begin
  ...
  try
    ...
    Windows.EnumResourceNames(AModule, PChar(sdxResourceType), @EnumResNameProc,
      LPARAM(AData));
    ASkinNames.Sort;
  finally
    AData.Free;
...
begin
  Result := cxLookAndFeelPaintersManager.GetPainter(sdxSkinsUserSkinName,
    APainter) and APainter.GetPainterData(ASkinInfo);
  if Result then
...
function dxSkinsUserSkinPopulateSkinNames(AStream: TStream; AList: TStrings;
  ADisplayNames: Boolean = False): Boolean;
var
  AReader: TdxSkinBinaryReader;
  I: Integer;
begin
...
function TdxSkinsUserSkinPainter.GetSkinInfoClass
