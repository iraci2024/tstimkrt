...
type
  ...
  public
    ...
    property AdditionalStyles: TList<TdxStyleOutlineLevelInfo>
      read FAdditionalStyles write SetAdditionalStyles;
    property BookmarkName: TdxNullableString read FBookmarkName
      write FBookmarkName;
    property CreateHyperlinks: Boolean read FCreateHyperlinks
      write FCreateHyperlinks;
    property Delimiter: TdxNullableString read FDelimiter write FDelimiter;
    property EntriesGroupId: TdxNullableString read FEntriesGroupId
      write FEntriesGroupId;
    property EntriesLevels: TdxIntegerList read FEntriesLevels
      write SetEntriesLevels;
    property HeaderLevels: TdxIntegerList read FHeaderLevels
      write SetHeaderLevels;
    property NoPageNumberLevels: TdxIntegerList read FNoPageNumberLevels
      write SetNoPageNumberLevels;
    property PageNumberSeparator: TdxNullableString read FPageNumberSeparator
      write FPageNumberSeparator;
    property PrefixedSequenceId: TdxNullableString read FPrefixedSequenceId
      write FPrefixedSequenceId;
    property SequenceId: TdxNullableString read FSequenceId write FSequenceId;
    property UseParagraphOutlineLevel: Boolean read FUseParagraphOutlineLevel
      write FUseParagraphOutlineLevel;
  end;
  ...
  public const
    FieldType = 'TOC';
  strict private
    class var FSwitchesWithArgument: TdxStringBooleanDictionary;
  ...
  strict private
    ...
    function GetFieldSection(APieceTable: TdxPieceTable;
      ADocumentField: TdxField): TdxSection;
    function GetLevelsRange(const AValue: TdxNullableString;
      const ADefaultValue: string): TdxIntegerList;
    function GetLevelsRangeCore(const AValue: TdxNullableString)
      : TdxIntegerList;
    function GetAdditionalStylesInfo(const AValue: TdxNullableString)
      : TList<TdxStyleOutlineLevelInfo>;
  protected
  ...
  public
    ...
    procedure Initialize(APieceTable: TdxCustomPieceTable;
      AInstructions: TdxInstructionCollection); override;
    procedure BeforeCalculateFields(ASourcePieceTable: TdxCustomPieceTable;
      ADocumentField: TdxField); override;
...
type
  ...
  protected
    ...
    procedure InitializeField(AField: TdxCalculatedFieldBase;
      AScanner: TdxFieldScanner);
    procedure ProcessTocEntryField(AEntry: TdxTocEntryField);
    procedure ProcessSequenceField(ASequence: TdxSequenceField;
      ADocumentField: TdxField);
    function GetSequenceFieldResultText(ASequence: TdxSequenceField;
      ADocumentField: TdxField): string;
    ...
    procedure AppendTocEntry(const AText: string; ABookmark: TdxBookmark;
      ALevel: Integer);
    procedure ApplyTabsToParagraph(ATocParagraph: TdxParagraph);
    function ApplyPageNumber(const AText: string;
      ATarget: TdxParagraph): string;
    function ObtainTocBookmarkForHeaderOrOutline(AParagraph: TdxParagraph)
      : TdxBookmark;
    function ObtainTocBookmarkForTocEntry(AEntry: TdxTocEntryField;
      ATocEntryFieldCodeFirstRunIndex: TdxRunIndex): TdxBookmark;
    function GenerateTocBookmarkName(APieceTable: TdxPieceTable)
      : string; virtual;
    function LookupEntireParagraphBookmark(AParagraph: TdxParagraph;
      AActualParagraphPosition: TdxDocumentLogPosition): TdxBookmark;
    function LookupZeroLengthBookmarkAtLogPosition(APos: TdxDocumentLogPosition)
      : TdxBookmark;
    function LookupExactBookmark(APos: TdxDocumentLogPosition; ALen: Integer)
      : TdxBookmark;
    procedure ApplyTocStyleToParagraph(ALevel: Integer;
    ...
    function CalculateActualParagraphPosition(AParagraph: TdxParagraph)
    ...
    function IsParagraphInsideField(AParagraph: TdxParagraph;
...
constructor TdxStyleOutlineLevelInfo.Create(const AStyleName: string;
...
procedure TdxTocField.Initialize(APieceTable: TdxCustomPieceTable;
...
begin
  ...
  else if AInstructions.GetBool('n') then
    FOptions.NoPageNumberLevels := GetLevelsRange('1-9', '1-9');

  FOptions.AdditionalStyles := GetAdditionalStylesInfo
    (AInstructions.GetString('t'));
...
procedure TdxTocField.BeforeCalculateFields(ASourcePieceTable
...
begin
  ...
  ARightTabPosition := ASection.Page.Width - ASection.Margins.Left -
    ASection.Margins.Right;

  AGenerator := TdxTocGenerator.Create(APieceTable.DocumentModel, FTargetModel,
    FOptions, ARightTabPosition);
  try
    AGenerator.Export;
    FDocumentLayoutService :=
      APieceTable.DocumentModel.GetService<IdxDocumentLayoutService>;
    if FDocumentLayoutService = nil then
      Exit;
    FTargetModel.ReplaceService<IdxBookmarkResolutionService>
      (TdxBookmarkResolutionService.Create(APieceTable));
    FTargetModel.ReplaceService<IdxDocumentLayoutService>
      (FDocumentLayoutService);
    FTargetModel.BeginUpdate;
    AUpdater := FTargetModel.MainPieceTable.FieldUpdater;
    FFieldsToUpdate := AUpdater.GetFieldsToUpdate
      (FTargetModel.MainPieceTable.Fields, nil);
    AUpdater.PrepareFieldsCore(FFieldsToUpdate,
...
function TdxTocField.GetFieldSection(APieceTable: TdxPieceTable;
  ADocumentField: TdxField): TdxSection;
var
...
begin
  ...
  begin
    APosition := TdxDocumentModelPosition.FromRunStart(APieceTable,
      ADocumentField.FirstRunIndex);
...
function TdxTocField.GetCalculatedValueCore(ASourcePieceTable
  : TdxCustomPieceTable; AMailMergeDataMode: TdxMailMergeDataMode;
  ADocumentField: TdxField): TdxCalculatedFieldValue;
var
  AServices: IdxServiceContainer;
begin
  try
    if (FDocumentLayoutService = nil) or (FTargetModel = nil) then
      Exit(TdxCalculatedFieldValue.Create(TValue.From<TdxDocumentModel>
        (FTargetModel)));
    try
      FTargetModel.MainPieceTable.FieldUpdater.UpdateFieldsCore(FFieldsToUpdate,
        TdxUpdateFieldOperationType.Normal);
    finally
    ...
    Result := TdxCalculatedFieldValue.Create
      (TValue.From<TdxDocumentModel>(FTargetModel));
  finally
...
function TdxTocField.GetLevelsRangeCore(const AValue: TdxNullableString)
  : TdxIntegerList;
var
...
function TdxTocField.GetAdditionalStylesInfo(const AValue: TdxNullableString)
  : TList<TdxStyleOutlineLevelInfo>;
var
  AParts: TArray<string>;
...
begin
  ...
  begin
    ...
    if TryStrToInt(AOutlineLevelString, AOutlineLevel) and (AOutlineLevel >= 0)
      and (AOutlineLevel <= 9) then
      Result.Add(TdxStyleOutlineLevelInfo.Create(AStyleName, AOutlineLevel));
  end;
end;

function TdxTocField.CanUseSwitchWithoutArgument(const AFieldSpecificSwitch
  : string): Boolean;
begin
...
constructor TdxTocGenerator.Create(ADocumentModel, ATargetDocumentModel
  : TdxDocumentModel; AOptions: TdxTocGeneratorOptions;
  ARightTabPosition: Integer);
begin
  ...
  try
    ...
    try
      ...
      if TargetDocumentModel.IsEmpty then
        TargetDocumentModel.MainPieceTable.InsertText(0,
          cxGetResourceString(@sdxRichEditExceptionNoTocEntriesFound));
    finally
...
begin
  ...
  FEnd := TdxDocumentModelPosition.FromParagraphEnd(PieceTable,
    PieceTable.Paragraphs.Count - 1);
end;
...
procedure TdxTocGenerator.TryToExportBookmarks(ARunIndex: TdxRunIndex;
  ARunOffset: Integer);
begin
end;
...
begin
  ...
  if (FAnalyzeFields and (FCurrentRunIndex >= FStart.RunIndex)) and
    (FCurrentRunIndex <= FEnd.RunIndex) then
  begin
...
begin
  ...
  try
    AScanner := TdxFieldScanner.Create(AIterator,
      PieceTable.DocumentModel.MaxFieldSwitchLength,
      PieceTable.DocumentModel.EnableFieldNames,
      PieceTable.SupportFieldCommonStringFormat);
    try
...
procedure TdxTocGenerator.ProcessCalculatedField(AField: TdxCalculatedFieldBase;
  AScanner: TdxFieldScanner; ADocumentField: TdxField);
begin
  ...
  else if AField is TdxSequenceField then
  begin
    InitializeField(AField, AScanner);
...
procedure TdxTocGenerator.InitializeField(AField: TdxCalculatedFieldBase;
  AScanner: TdxFieldScanner);
var
  AInstructions: TdxInstructionCollection;
begin
  AInstructions := TdxFieldCalculatorService.ParseInstructions
    (AScanner, AField);
...
procedure TdxTocGenerator.ProcessSequenceField(ASequence: TdxSequenceField;
  ADocumentField: TdxField);
var
  AText: string;
begin
...
function TdxTocGenerator.GetSequenceFieldResultText(ASequence: TdxSequenceField;
  ADocumentField: TdxField): string;
var
...
function TdxTocGenerator.ExportParagraph(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
var
  AOutlineLevel: Integer;
begin
  ...
  if Options.HeaderLevels.Contains(AOutlineLevel) and
    CanIncludeParagraphIntoToc(AParagraph) then
  begin
...
begin
  ...
  begin
    ...
    AListLevel := AParagraph.DocumentModel.NumberingLists
      [AParagraph.GetNumberingListIndex].Levels[AListLevelIndex]
      as IdxParagraphProperties;
...
begin
  ...
  begin
    ABookmark := ObtainTocBookmarkForHeaderOrOutline
      (TdxParagraph(ARun.Paragraph));
    AppendTocEntry(AText, ABookmark, FCurrentParagraphOutlineLevel);
  end;
end;

procedure TdxTocGenerator.AppendTocEntry(const AText: string;
  ABookmark: TdxBookmark; ALevel: Integer);
var
  ATocParagraph: TdxParagraph;
...
begin
  ...
  begin
    ...
    TargetPieceTable.CreateHyperlink(ATextStartPosition, Length(S),
      AHyperlinkInfo);
  end;
  ...
  try
    ...
    try
      if FNumberingListIndent >= 0 then
        AOwnTabs.Add(TdxTabInfo.Create(FNumberingListIndent +
          ATocParagraph.LeftIndent, TdxTabAlignmentType.Left));
...
function TdxTocGenerator.ApplyPageNumber(const AText: string;
  ATarget: TdxParagraph): string;
var
  ASeparator: string;
...
begin
  ...
  begin
    ...
    try
      AOwnTabs.Add(TdxTabInfo.Create(FRightTabPosition,
        TdxTabAlignmentType.Right, TdxTabLeaderType.Dots, False, False));
...
function TdxTocGenerator.ObtainTocBookmarkForHeaderOrOutline
  (AParagraph: TdxParagraph): TdxBookmark;
var
...
begin
  ...
  ABookmark := LookupEntireParagraphBookmark(AParagraph,
    AActualParagraphPosition);
  if ABookmark <> nil then
    Exit(ABookmark);
  APieceTable := AParagraph.PieceTable;
  AName := GenerateTocBookmarkName(APieceTable);
  APieceTable.CreateBookmark(AActualParagraphPosition, AParagraph.LogPosition +
    AParagraph.Length - AActualParagraphPosition - 1, AName);
  Result := APieceTable.Bookmarks.FindByName(AName);
end;

function TdxTocGenerator.ObtainTocBookmarkForTocEntry(AEntry: TdxTocEntryField;
  ATocEntryFieldCodeFirstRunIndex: TdxRunIndex): TdxBookmark;
var
...
function TdxTocGenerator.GenerateTocBookmarkName(APieceTable
  : TdxPieceTable): string;
var
  AName: string;
begin
  Randomize;
...
function TdxTocGenerator.LookupEntireParagraphBookmark(AParagraph: TdxParagraph;
  AActualParagraphPosition: TdxDocumentLogPosition): TdxBookmark;
var
  APos: TdxDocumentLogPosition;
...
begin
  ...
  begin
    ...
    if (ABookmark.NormalizedStart = APos) and
      (ABookmark.Start + ABookmark.Length = AParagraph.LogPosition +
      AParagraph.Length - 1) then
      Exit(ABookmark);
  end;
  Result := nil;
end;

function TdxTocGenerator.LookupZeroLengthBookmarkAtLogPosition
  (APos: TdxDocumentLogPosition): TdxBookmark;
begin
  Result := LookupExactBookmark(APos, 0);
end;

function TdxTocGenerator.LookupExactBookmark(APos: TdxDocumentLogPosition;
  ALen: Integer): TdxBookmark;
var
  ABookmarks: TdxBookmarkCollection;
  ACount, I: Integer;
  ABookmark: TdxBookmark;
...
procedure TdxTocGenerator.ApplyTocStyleToParagraph(ALevel: Integer;
  ATocParagraph: TdxParagraph);
var
  AStyleIndex: Integer;
begin
...
function TdxTocGenerator.CalculateTocParagraphStyleIndex
...
function TdxTocGenerator.GetActualOutlineLevel
  (AParagraph: TdxParagraph): Integer;
var
  AStyleOutlineLevel: Integer;
begin
  AStyleOutlineLevel := GetOutlineLevelByStyleName
    (AParagraph.ParagraphStyle.StyleName);
  if AStyleOutlineLevel >= 0 then
    Exit(AStyleOutlineLevel);

  if Options.UseParagraphOutlineLevel then
  begin
    AStyleOutlineLevel := AParagraph.ParagraphStyle.ParagraphProperties.
      OutlineLevel;
    if AStyleOutlineLevel > 0 then
      Exit(AStyleOutlineLevel);
...
function TdxTocGenerator.GetOutlineLevelByStyleName(const AStyleName
  : string): Integer;
var
...
function TdxTocGenerator.CalculateActualParagraphPosition
...
begin
  ...
  try
    ...
    try
      ...
      while IsWhiteSpace(ACharacterIterator.CurrentChar) and
        not ACharacterIterator.IsEnd do
      begin
        Inc(Result);
...
begin
  Result := {$IFDEF DELPHIXE4}ACh.IsWhiteSpace or
  {$ELSE}TCharacter.IsWhiteSpace(ACh) or {$ENDIF}
    CharInSet(ACh, [TdxCharacters.NonBreakingSpace, TdxCharacters.PageBreak,
    TdxCharacters.ColumnBreak, TdxCharacters.LineBreak]);
end;

function TdxTocGenerator.CanIncludeParagraphIntoToc
...
function TdxTocGenerator.IsParagraphInsideField(AParagraph: TdxParagraph;
  AField: TdxField): Boolean;
begin
