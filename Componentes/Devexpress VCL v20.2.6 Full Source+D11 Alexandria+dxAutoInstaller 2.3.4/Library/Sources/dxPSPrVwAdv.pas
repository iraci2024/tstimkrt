...
uses
  ...
  cxGeometry, cxLookAndFeels, cxLookAndFeelPainters, IniFiles, cxImageList,
  cxImageComboBox,
...
class function TdxBarPSPopupMenuBuilder.CanShowPopup(const APopupMenu
  : TPopupMenu): Boolean;
begin
...
procedure TdxBarPSPopupMenuBuilder.InvokePopup(X, Y: Integer;
...
class function TdxBarPSPageSetupMenuBuilder.ExtractPrintStyleFromObj
...
procedure TdxBarPSAutoHFTextMenuBuilder.BuildAutoHFTextEntriesMenu
...
class function TdxBarPSAutoHFTextMenuBuilder.ExtractAutoHFTextEntryIndexFromObj
  (Obj: TObject): Integer;
begin
...
begin
  ...
  try
    ...
    dxLoadImageListFromResources(ilToolBar,
...
begin
  ...
  Preview.DoShowMultiplySelectPagesDlg(ilToolBar, PageSelectorImageIndex,
    R.TopLeft, YShift);
end;
...
begin
  ...
  if Supports(TObject(Preview.Explorer), IdxPSExplorerContextCommands2,
    CommandSet2) then
  begin
...
procedure TdxfmPreviewWdxBar.LoadStrings;
  ...
  procedure LocalizeImageComboBoxItem(const AItem: TcxImageComboBoxItem;
    AResId: Pointer);
  begin
...
begin
  ...
  bbExplorerProperties.Caption := cxGetResourceString
    (@sdxMenuExplorerProperties);
  ...
  bbViewHFSwitchHeaderFooter.Caption :=
    cxGetResourceString(@sdxMenuViewHFSwitchHeaderFooter);
  ...
  bbFormatHeaderAndFooter.Caption :=
    cxGetResourceString(@sdxMenuFormatHeaderAndFooter);
  ...
  bbFormatPageBackground.Caption :=
    cxGetResourceString(@sdxMenuFormatPageBackground);
  ...
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties
    (cbxPredefinedZoom.Properties).Items[8], @sdxPageWidth);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties
    (cbxPredefinedZoom.Properties).Items[9], @sdxWholePage);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties
    (cbxPredefinedZoom.Properties).Items[10], @sdxTwoPages);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties
    (cbxPredefinedZoom.Properties).Items[11], @sdxFourPages);
  LocalizeImageComboBoxItem(TcxImageComboBoxProperties
    (cbxPredefinedZoom.Properties).Items[12], @sdxWidenToSourceWidth);
  ...
  bsiShortcutThumbnails.Caption :=
    cxGetResourceString(@sdxMenuShortcutThumbnails);
  ...
  bbViewHFSwitchHeaderFooter.Hint :=
    cxGetResourceString(@sdxHintViewHFSwitchHeaderFooter);
  ...
  bbFormatPageBackground.Hint := cxGetResourceString
...
procedure TdxfmPreviewWdxBar.SetBarItemVisibility(Item: TdxBarItem;
  Value: Boolean);
begin
...
begin
  ...
  AProperties.DefaultDescription :=
    AddPercentageChar(IntToStr(Preview.ZoomFactor));
...
function TdxfmPreviewWdxBar.GetExplorerContextCommand(Index: Integer)
  : TCustomdxPSExplorerContextCommand;
begin
...
begin
  Result := dxBarManager.BarByCaption
    (DropAmpersand(cxGetResourceString(@sdxShortCutMenusBar)));
end;
...
begin
  ...
  else if bbViewHFSwitchHeaderFooter.Down then
    Preview.State := prsEditFooters
  else
    Preview.State := prsEditHeaders;
end;
...
procedure TdxfmPreviewWdxBar.LoadBarManager(AIniFile: TCustomIniFile;
  const ASectionName: string);
const
...
procedure TdxfmPreviewWdxBar.SaveBarManager(AIniFile: TCustomIniFile;
  const ASectionName: string);
const
  StorageType: array [Boolean] of TdxBarStoringKind = (skIni, skReg);
begin
...
procedure TdxfmPreviewWdxBar.PreviewAddExplorerCommand(Sender: TObject;
  ACommand: TCustomdxPSExplorerContextCommand);
...
begin
  ...
  begin
    ...
    BeginGroup(dxBarManager.BarByCaption(cxGetResourceString(@sdxExplorerBar))
      .ItemLinks);
  ...
  begin
    ...
    begin
      ...
      Category := dxBarManager.Categories.IndexOf
        (DropAmpersand(cxGetResourceString(@sdxMenuExplorer)));
      OnClick := ExplorerContextCommandClick;
    end;

    AddBarControl(bbExplorer.ItemLinks, BarButton);
    AddBarControl(dxBarManager.BarByCaption(cxGetResourceString(@sdxExplorerBar)
      ).ItemLinks, BarButton);
...
begin
  if ReportLink <> nil then
    TcxSpinEditProperties(seActivePage.Properties).MaxValue :=
      ReportLink.PageCount;
  SetPredefinedZoomValue(Preview.ZoomFactor);
...
procedure TdxfmPreviewWdxBar.PreviewLoadProperties(Sender: TObject;
  AIniFile: TCustomIniFile; const ASectionName: string);
begin
  LoadBarManager(AIniFile, ASectionName);
end;

procedure TdxfmPreviewWdxBar.PreviewSaveProperties(Sender: TObject;
  AIniFile: TCustomIniFile; const ASectionName: string);
begin
...
begin
  ...
  try
    ...
    bbExplorerCreateNewFolder.Enabled := Preview.IsExplorerAvailable and
      Preview.ExplorerTree.CanCreateFolder;
    bbExplorerDelete.Enabled := Preview.IsExplorerAvailable and
      Preview.ExplorerTree.CanDeleteSelection;
    bbExplorerRename.Enabled := Preview.IsExplorerAvailable and
      Preview.ExplorerTree.CanRenameSelectedItem;
    bbExplorerProperties.Enabled := Preview.IsExplorerAvailable and
      Preview.ExplorerTree.CanShowPropertySheetsForSelectedItem;

    bbFormatPageBackground.Enabled := Preview.IsEnabled(peoPageBackground) and
      not Preview.IsPrinting;
    bbFormatShrinkToPageWidth.Enabled := PagesExists and not Preview.IsPrinting;
    bbFormatFootnotes.Enabled := (ReportLink <> nil) and
    ...
    bbGoToFirstPage.Enabled := PagesExists and
      (TdxPreviewAccess(Preview.Preview).SelPageIndex <> 0);
    bbGoToPrevPage.Enabled := PagesExists and
      (TdxPreviewAccess(Preview.Preview).SelPageIndex <> 0);
    bbGoToNextPage.Enabled := PagesExists and
      (TdxPreviewAccess(Preview.Preview).SelPageIndex <> Preview.PageCount - 1);
    bbGoToLastPage.Enabled := PagesExists and
      (TdxPreviewAccess(Preview.Preview).SelPageIndex <> Preview.PageCount - 1);
    ...
    SetBarItemVisibility(bbExplorerCreateNewFolder,
      Preview.IsExplorerAvailable);
    ...
    SetBarItemVisibility(bbFormatPageBackground,
    ...
    ABoolValue := (ReportLink <> nil) and (ReportLink.StyleManager <> nil) and
    ...
    begin
      ...
      begin
        TcxSpinEditProperties(seActivePage.Properties).MaxValue :=
          ReportLink.PageCount;
        seActivePage.EditValue := TdxPreviewAccess(Preview.Preview)
          .SelPageIndex + 1;
      end
    end
    else
    ...
    { Edit }
    Index := dxBarManager.Categories.IndexOf
      (DropAmpersand(cxGetResourceString(@sdxMenuEdit)));
    if Index <> -1 then
      dxBarManager.CategoryVisible[Index] := False;

    { Help }
    Index := dxBarManager.Categories.IndexOf
      (DropAmpersand(cxGetResourceString(@sdxMenuHelp)));
    if Index <> -1 then
      dxBarManager.CategoryVisible[Index] := (bbHelpTopics.Visible = ivAlways)
        or (bbHelpAbout.Visible = ivAlways);

    { Shortcut Menus }
    Index := dxBarManager.Categories.IndexOf
      (DropAmpersand(cxGetResourceString(@sdxMenuShortCutMenus)));
    if Index <> -1 then
      dxBarManager.CategoryVisible[Index] := False;
...
procedure TdxfmPreviewWdxBar.PreviewCanShowMarginHint(Sender: TObject;
  var AAllow: Boolean);
begin
...
procedure TdxfmPreviewWdxBar.FormCloseQuery(Sender: TObject;
...
class function TdxPSAdvancedPreviewDialogStyleInfo.CreatePreviewWindow
