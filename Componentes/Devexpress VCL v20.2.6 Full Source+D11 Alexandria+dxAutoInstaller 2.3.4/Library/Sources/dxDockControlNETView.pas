...
function TdxDockControlNETPainter.GetCaptionFontColor
...
begin
  ...
  begin
    ...
    ACanvas.FillRect(Rect(Right - ABorderWidths.Right, Top,
      Left + Right, Bottom));
...
procedure TdxDockControlNETPainter.DrawCaption(ACanvas: TcxCanvas; ARect: TRect;
  IsActive: Boolean);
begin
  ACanvas.Brush.Style := bsSolid;
  if IsActive then
  ...
  begin
    ...
    ACanvas.ExcludeClipRect(cxRect(ARect.Right - 1, ARect.Top, ARect.Right,
      ARect.Top + 1));
    ACanvas.ExcludeClipRect(cxRect(ARect.Right - 1, ARect.Bottom - 1,
      ARect.Right, ARect.Bottom));
    ACanvas.ExcludeClipRect(cxRect(ARect.Left, ARect.Bottom - 1, ARect.Left + 1,
      ARect.Bottom));
    ACanvas.Rectangle(ARect);
  end;
end;

procedure TdxDockControlNETPainter.DrawCaptionButton(ACanvas: TcxCanvas;
...
procedure TdxDockControlNETPainter.DrawCaptionCloseButton(ACanvas: TcxCanvas;
  ARect: TRect; AIsActive: Boolean; AState: TcxButtonState);
begin
  ...
  ACanvas.Line(ARect.Left + 1, ARect.Top + 1, ARect.Right - 1,
    ARect.Bottom - 1);
...
begin
  ...
  if IsSwitched then
  begin
    ACanvas.Rectangle(ARect.Left + 4, ARect.Top, ARect.Right + 1,
      ARect.Bottom - 3);
    ACanvas.Rectangle(ARect.Left + 1, ARect.Top + 3, ARect.Right - 2,
      ARect.Bottom);
  end
  else
    ACanvas.Rectangle(ARect.Left + 1, ARect.Top + 1, ARect.Right - 1,
      ARect.Bottom - 1);
end;

procedure TdxDockControlNETPainter.DrawCaptionText(ACanvas: TcxCanvas;
...
procedure TdxDockControlNETPainter.DrawHideBar(ACanvas: TcxCanvas; ARect: TRect;
...
begin
  ...
  DrawColorEdge(ACanvas, AButton.Bounds, GetHideBarButtonColor, etFlat,
    Edges[APosition]);
end;
...
begin
  cxLookAndFeelPaintersManager.GetPainter(lfsUltraFlat).DrawScaledArrow(ACanvas,
...
class function TdxDockControlNETPainter.HasLookAndFeelStyle
