...
uses
  Types, SysUtils, cxDBData, dxLayoutContainer, dxLayoutLookAndFeels,
  cxGridCustomView, cxGridCustomLayoutView,
...
type
  ...
  private
    ...
    function GetItemBounds(ARecordViewInfo: TcxGridLayoutViewRecordViewInfo;
      AViewInfo: TdxCustomLayoutItemViewInfo): TRect;
    function GetLayoutLookAndFeel: TcxGridLayoutLookAndFeel;
    procedure GetViewInfoHandler(AItem: TdxCustomLayoutItem;
      var AViewInfo: TdxCustomLayoutItemViewInfo);
  protected
    ...
    function GetReportItemClass(ALayoutItem: TdxCustomLayoutItem)
      : TdxCustomReportLayoutItemClass;

    function GetGroupViewParams(ARecord: TcxCustomGridRecord)
      : TdxReportItemViewParams;
    function GetItemCaptionViewParams(ARecord: TcxCustomGridRecord)
      : TdxReportItemViewParams;
    function GetRecordCaptionViewParams(ARecord: TcxCustomGridRecord)
      : TdxReportItemViewParams;
    function GetRecordViewParams(ARecord: TcxCustomGridRecord)
      : TdxReportItemViewParams;
...
begin
  inherited;
  FGroupLookAndFeel := TdxPSReportLayoutGroupStandardLookAndFeel.Create
    (ReportCells);
end;
...
begin
  ...
  Result.BoundsRect := cxRectOffset(ViewInfo.ControlViewInfo.Bounds,
...
procedure TdxGridLayoutViewFormatter.DoInitializeRecord
...
function TdxGridLayoutViewFormatter.GetItemViewParams
...
function TdxGridLayoutViewFormatter.GetRecordClass
...
procedure TdxGridLayoutViewFormatter.CreateCaptionCell
  (ARecord: TdxReportLayoutRecord);
begin
  if FCalculatingRecordViewInfo.CaptionViewInfo <> nil then
    DoCreateCaptionCell(ARecord);
end;

procedure TdxGridLayoutViewFormatter.DoCreateCaptionCell
  (ARecord: TdxReportLayoutRecord);
var
  R: TRect;
...
begin
  ...
  with ACaption do
  begin
    R := cxRectOffset(FCalculatingRecordViewInfo.CaptionViewInfo.Bounds,
      FCalculatingRecordViewInfo.Bounds.TopLeft, False);
    Text := FCalculatingRecordViewInfo.CaptionViewInfo.Text;
    BoundsRect := R;
  end;
end;

procedure TdxGridLayoutViewFormatter.InitializeRecordCells
  (ARecord: TdxReportLayoutRecord);
  ...
  begin
    ...
    begin
      ...
      AReportItem.BoundsRect := GetItemBounds(FCalculatingRecordViewInfo,
        AViewInfo);
      ...
      try
        if AViewInfo is TdxLayoutGroupViewInfo then
          ...
          SetViewParams(AReportItem,
            GetItemCaptionViewParams(ARecord.LayoutRecord));
        TdxCustomReportLayoutItemAccess(AReportItem)
          .Initialize(Builder.ReportLink);
        ...
        begin
          ...
          if TdxCustomLayoutItemViewInfoAccess(AViewInfo).HasBorder then
            AReportItem.CellSides := TdxPSReportGroupLookAndFeelAccess
              (AReportItem.LookAndFeel).GetBorderSides(AReportItem)
          else
            AReportItem.CellSides := [];
          for I := 0 to (AViewInfo as TdxLayoutGroupViewInfo)
            .ItemViewInfoCount - 1 do
            InitializeCell((AViewInfo as TdxLayoutGroupViewInfo)
              .ItemViewInfos[I]);
        end;
      finally
...
begin
  FCalculatingRecordViewInfo := TcxGridLayoutViewRecordViewInfo.Create
    (ARecord.LayoutRecord.GridView.ViewInfo.RecordsViewInfo,
    ARecord.LayoutRecord);
  try
...
function TdxGridLayoutViewFormatter.GetReportItemClass
  (ALayoutItem: TdxCustomLayoutItem): TdxCustomReportLayoutItemClass;
begin
  if ALayoutItem is TdxLayoutGroup then
    Result := TdxReportLayoutGroup
  else if ALayoutItem is TcxGridLayoutItem then
...
function TdxGridLayoutViewFormatter.GetGroupViewParams
...
begin
  ...
  begin
    TcxStylesAccess(ReportLink.Styles).GetDefaultViewParams(vspsGridGroup,
      nil, AParams);
  ...
  begin
    ...
    if Result.NativeParams.Color = clDefault then
      Result.NativeParams.Color := TdxLayoutLookAndFeelGroupOptionsAccess
        (LayoutLookAndFeel.GroupOptions).GetDefaultColor;
    Result.Transparent := False;
  end;
  Result.FontStyle := [];
end;

function TdxGridLayoutViewFormatter.GetItemCaptionViewParams
  (ARecord: TcxCustomGridRecord): TdxReportItemViewParams;
begin
  if ReportLink.OptionsFormatting.UseNativeStyles then
...
function TdxGridLayoutViewFormatter.GetRecordCaptionViewParams
  (ARecord: TcxCustomGridRecord): TdxReportItemViewParams;
begin
  if ReportLink.OptionsFormatting.UseNativeStyles then
...
function TdxGridLayoutViewFormatter.GetRecordViewParams
  (ARecord: TcxCustomGridRecord): TdxReportItemViewParams;
begin
...
procedure TdxGridLayoutViewFormatter.GetContentParams
...
procedure TdxGridLayoutViewFormatter.GetGroupParams
  (ARecord: TcxCustomGridRecord; AItem: TcxCustomGridTableItem;
  out AParams: TcxViewParams);
begin
...
procedure TdxGridLayoutViewFormatter.GetItemParams(ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; out AParams: TcxViewParams);
begin
  AParams := GetItemCaptionViewParams(ARecord).NativeParams;
end;

procedure TdxGridLayoutViewFormatter.GetRecordCaptionParams
  (ARecord: TcxCustomGridRecord; out AParams: TcxViewParams);
begin
  AParams := GetRecordCaptionViewParams(ARecord).NativeParams;
end;

function TdxGridLayoutViewFormatter.GetItemBounds(ARecordViewInfo
  : TcxGridLayoutViewRecordViewInfo;
  AViewInfo: TdxCustomLayoutItemViewInfo): TRect;
begin
  Result := cxRectOffset(AViewInfo.Bounds,
    ARecordViewInfo.Bounds.TopLeft, False);
end;

function TdxGridLayoutViewFormatter.GetLayoutLookAndFeel
  : TcxGridLayoutLookAndFeel;
begin
  Result := TcxGridLayoutViewAccess(Builder.GridView).LayoutLookAndFeel;
end;

procedure TdxGridLayoutViewFormatter.GetViewInfoHandler
  (AItem: TdxCustomLayoutItem; var AViewInfo: TdxCustomLayoutItemViewInfo);
begin
...
class function TdxGridLayoutViewBuilder.FormatterClass
...
function TdxGridLayoutViewBuilder.GetRecord(Index: Integer)
  : TdxReportLayoutRecord;
begin
...
function TdxGridDBLayoutViewAdapter.DBDataModeController
