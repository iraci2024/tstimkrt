...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Classes, Windows, Graphics, Controls, StdCtrls, ImgList, dxCore,
  dxPSCore, dxPSGlbl,
...
type
  ...
  private
    function GetMapClass(ADataItem: TAbstractdxReportCellData)
      : TdxPSCustomGridCellDataMapClass;
  public
    class function Instance: TdxPSGridCellDataMaps; reintroduce; overload;
    procedure InitializeCellData(ACol, ARow: Integer;
      ADataItem: TAbstractdxReportCellData;
      AReportLink: TAbstractdxGridReportLink);
    property MapClasses[ADataItem: TAbstractdxReportCellData]
      : TdxPSCustomGridCellDataMapClass read GetMapClass; default;
  end;
  ...
  TdxCustomGridReportLinkEvent = procedure(Sender: TAbstractdxGridReportLink)
    of object;

  TdxCustomGridReportLinkInitializeItemEvent = procedure
    (Sender: TAbstractdxGridReportLink; ACol, ARow: Integer;
    ADataItem: TAbstractdxReportCellData) of object;
  ...
  protected
    ...
    function IsSupportedCustomDraw(AItem: TAbstractdxReportCellData)
      : Boolean; override;
    ...
    procedure AssignData(ACol, ARow: Integer;
      ADataItem: TAbstractdxReportCellData); virtual;
    function CreateDataItem(AParent: TdxReportCell; ACol, ARow: Integer;
      const ABounds: TRect): TAbstractdxReportCellData; virtual;
    ...
    function GetRowParent(AReportCells: TdxReportCells; ARow: Integer)
    ...
    procedure GetCellColRow(AItem: TdxReportVisualItem;
      var ACol, ARow: Integer);
    function GetCellContentBkColor(ACol, ARow: Integer): TColor; virtual;
    function GetCellContentPattern(ACol, ARow: Integer)
      : TdxPSFillPatternClass; virtual;
    function GetCellEdge3DSoft(AItem: TAbstractdxReportCellData;
      ACol, ARow: Integer): Boolean; virtual;
    function GetCellEdge3DStyle(AItem: TAbstractdxReportCellData;
      ACol, ARow: Integer): TdxCellEdgeStyle; virtual;
    ...
    function GetCellImageMakeSpaceForEmpty(ACol, ARow: Integer)
      : Boolean; virtual;
    ...
    function GetMinRowHeight(ACanvas: TdxPSReportRenderCustomCanvas;
      AFont: TFont): Integer; virtual;
    function GetMultiline: Boolean; virtual;

    procedure GetSelectedRange(ABeginCol, AEndCol, ABeginRow,
      AEndRow: PInteger); overload; virtual;
    procedure GetSelectedRange(out ABeginCol, AEndCol, ABeginRow,
      AEndRow: Integer); overload; virtual;
    ...
    function GetSourceCellEdge3DStyle(ACol, ARow: Integer)
      : TdxCellEdgeStyle; virtual;
    function GetSourceCellEdgeMode(ACol, ARow: Integer)
      : TdxCellEdgeMode; virtual;
    ...
    property FixedTransparent: Boolean read FFixedTransparent
      write SetFixedTransparent default False;
    ...
    property IncludeFixed: Boolean read FIncludeFixed write SetIncludeFixed
      default True;
    property InternalRowHeights[Index: Integer]: Integer
      read GetInternalRowHeight;
    property Multiline: Boolean read FMultiline write SetMultiline
      default False;
    property OddColor: TColor read GetOddColor write SetOddColor
      stored IsFontStored default clWhite;
    ...
    property ReportRowsByRow[ARow: Integer]: TdxReportCell
      read GetReportRowByRow;
  ...
  public
    ...
    procedure AddPageBreak(ARows: PIntegerArray;
      AStartIndex, ACount: Integer); overload;
    procedure AddPageBreak(ARows: TList); overload;
    // List of Integers -> Row Indexes
    procedure AddPageBreak(ARows: TList; AStartIndex, ACount: Integer);
      overload;
    procedure AddPageBreak(const ARows: array of Integer); overload;
    procedure AddPageBreak(const ARows: array of Integer;
      AStartIndex, ACount: Integer); overload;
    procedure AddPageBreak(const ARows: TBoundArray); overload;
    procedure AddPageBreak(const ARows: TBoundArray;
      AStartIndex, ACount: Integer); overload;
    // Horizontal Page Breaks
    procedure AddHorizontalPageBreak(AColumn: Integer); overload; virtual;
    procedure AddHorizontalPageBreak(AColumns: PIntegerArray;
      ACount: Integer); overload;
    procedure AddHorizontalPageBreak(AColumns: PIntegerArray;
      AStartIndex, ACount: Integer); overload;
    procedure AddHorizontalPageBreak(AColumns: TList); overload;
    // List of Integers -> Column Indexes
    procedure AddHorizontalPageBreak(AColumns: TList;
      AStartIndex, ACount: Integer); overload;
    procedure AddHorizontalPageBreak(const AColumns: array of Integer);
      overload;
    procedure AddHorizontalPageBreak(const AColumns: array of Integer;
      AStartIndex, ACount: Integer); overload;
    procedure AddHorizontalPageBreak(const AColumns: TBoundArray); overload;
    procedure AddHorizontalPageBreak(const AColumns: TBoundArray;
      AStartIndex, ACount: Integer); overload;
    ...
    property UseCustomPageBreaks: Boolean read FUseCustomPageBreaks
      write SetUseCustomPageBreaks default False;

    property OnGetCustomPageBreaks: TdxCustomGridReportLinkEvent
      read FOnGetCustomPageBreaks write FOnGetCustomPageBreaks;
    property OnInitializeItem: TdxCustomGridReportLinkInitializeItemEvent
...
procedure dxPSInitalizeDrawModeCombo(AComboBox: TcxImageComboBox;
  const AModes: TdxGridDrawModes);
procedure dxPSSyncDrawModeComboItemIndex(AComboBox: TcxImageComboBox;
  AMode: TdxGridDrawMode);

procedure dxPSDrawGridPreview(ACanvas: TCanvas; R: TRect;
  AReportLink: TAbstractdxGridReportLink; AShowFixedColumn: Boolean = True;
  AShowFixedRow: Boolean = True; AScaleFactor: TdxScaleFactor = nil);
...
const
  DrawModeNamesMap: array [TdxGridDrawMode] of Pointer = (@sdxDrawModeStrict,
    @sdxDrawModeOddEven, @sdxDrawModeChess, @sdxDrawModeBorrow);
...
begin
  if AComboBox.ItemIndex < 0 then
    ...
    Result := TdxGridDrawMode(AComboBox.Properties.Items
      [AComboBox.ItemIndex].Tag);
end;
...
const
  sdxGridStrings: array [0 .. 4, 0 .. 6] of string =
    (('7', '12', '27', '11', '11', '16', '84'), ('8', '6', '17', '12', '11',
    '16', '70'), ('23', '32', '21', '15', '10', '26', '127'),
    ('22', '12', '12', '32', '32', '12', '122'), ('60', '62', '77', '70', '64',
    '70', '403'));
begin
  ...
  begin
    if ARow = 0 then
      if ACol = 0 then
        ...
        case ACol of
          1:
            Result := cxGetResourceString(@sdxEast);
          2:
            Result := cxGetResourceString(@sdxWest);
          3:
            Result := cxGetResourceString(@sdxSouth);
          4:
            Result := cxGetResourceString(@sdxNorth);
        else
      ...
      case ARow of
        1:
          Result := cxGetResourceString(@sdxJanuaryShort);
        2:
          Result := cxGetResourceString(@sdxFebruaryShort);
        3:
          Result := cxGetResourceString(@sdxMarchShort);
        4:
          Result := cxGetResourceString(@sdxAprilShort);
        5:
          Result := cxGetResourceString(@sdxMayShort);
        6:
          Result := cxGetResourceString(@sdxJuneShort);
      else
        Result := cxGetResourceString(@sdxTotal);
      end;
  end
...
const
  ...
  procedure FrameRect(DC: HDC; var R: TRect; ABrush: HBRUSH;
    ASides: TdxCellSides);
  var
    R2: TRect;
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if ARow < FixedRowCount - Byte(AReportLink.IsDrawHorzLines) then
          Exclude(Result, csBottom);
      end;
      ...
      begin
        ...
        else if ACol < FixedColumnCount - Byte(AReportLink.IsDrawVertLines) then
          Exclude(Result, csRight);
      end;
    end
    else
...
const
  RightBottomInnerBorderColorMap: array [Boolean] of TColor = (clBtnShadow,
    clBtnFace);
var
  ABorderBrush: HBRUSH;
  ABrush: HBRUSH;
      ...
      begin
        ...
        begin
          ...
          cxPaintCanvas.DrawComplexFrame(R2, clBtnHighlight,
            RightBottomInnerBorderColorMap[AReportLink.Soft3D],
            TcxBorders(ASides));
          cxPaintCanvas.EndPaint;
          R2 := cxRectInflate(R2, -AScaleFactor.Apply(1));
        end;

        AIsTransparentCell := AFixedCell and AReportLink.FixedTransparent or
          not AFixedCell and AReportLink.Transparent;
        ...
        APreviewFont.Height := AScaleFactor.Apply(APreviewFont.Height,
          AReportLink.ScaleFactor);
        SetTextColor(ACanvas.Handle, ColorToRGB(APreviewFont.Color));
        SelectObject(ACanvas.Handle, APreviewFont.Handle);
        S := GetDesignerString(I + Ord(not AShowFixedColumn),
          J + Ord(not AShowFixedRow));
        ExtTextOut(ACanvas.Handle, R2.Left + AScaleFactor.Apply(2),
          R2.Top + AScaleFactor.Apply(2), ETO_CLIPPED, @R2, PChar(S),
          Length(S), nil);
      end;
...
procedure TdxPSGridCellDataMaps.InitializeCellData(ACol, ARow: Integer;
...
function TdxPSGridCellDataMaps.GetMapClass(ADataItem: TAbstractdxReportCellData)
  : TdxPSCustomGridCellDataMapClass;
begin
...
class procedure TdxPSCustomGridCellDataMap.InitializeCellData(ACol,
...
begin
  ...
  ADataItem.Edge3DStyle := AReportLink.GetCellEdge3DStyle(ADataItem,
    ACol, ARow);
...
class procedure TdxPSImageGridCellDataMap.InitializeCellData(ACol,
  ARow: Integer; ADataItem: TAbstractdxReportCellData;
  AReportLink: TAbstractdxGridReportLink);
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      MakeSpaceForEmptyImage := AReportLink.GetCellImageMakeSpaceForEmpty
        (ACol, ARow);
    end;
...
procedure TAbstractdxGridReportLink.AddHorizontalPageBreak(const AColumns
  : array of Integer);
begin
...
procedure TAbstractdxGridReportLink.AddHorizontalPageBreak(const AColumns
  : array of Integer; AStartIndex, ACount: Integer);
var
  I: Integer;
begin
  for I := AStartIndex to AStartIndex + ACount - 1 do
...
procedure TAbstractdxGridReportLink.AddHorizontalPageBreak(const AColumns
  : TBoundArray; AStartIndex, ACount: Integer);
var
  I: Integer;
begin
  for I := AStartIndex to AStartIndex + ACount - 1 do
...
procedure TAbstractdxGridReportLink.AddPageBreak(ARows: PIntegerArray;
  ACount: Integer);
begin
...
procedure TAbstractdxGridReportLink.ConstructReport(AReportCells
  : TdxReportCells);
begin
...
procedure TAbstractdxGridReportLink.ConstructReportContent
...
function TAbstractdxGridReportLink.GetRebuildOnPageParamsChange
...
function TAbstractdxGridReportLink.IsSupportedCustomDraw
...
begin
  if Assigned(FOnInitializeItem) then
    FOnInitializeItem(Self, ACol, ARow, ADataItem);
end;

procedure TAbstractdxGridReportLink.AssignData(ACol, ARow: Integer;
  ADataItem: TAbstractdxReportCellData);
begin
  dxPSGridCellDataMaps[ADataItem].InitializeCellData(ACol, ARow,
    ADataItem, Self);
end;
...
procedure TAbstractdxGridReportLink.CalculateRowHeights
  (AReportCells: TdxReportCells);
  ...
  begin
    ...
    Result := Renderer.CalcTextRect(ScreenCanvas, GetCellText(AActualColIndex,
      ARow), R, GetCellMultiline(AActualColIndex, ARow),
      GetCellFont(AActualColIndex, ARow));
  end;
...
begin
  Result := not IsAggregated or SysUtils.Supports(Controller,
    IdxReportLinkController);
end;

function TAbstractdxGridReportLink.CanCalculateRowAutoHeight
...
function TAbstractdxGridReportLink.GetDataItemClass(ACol: Integer;
  ARow: Integer = 0): TdxReportCellDataClass;
begin
  Result := TdxReportCellString;
end;

function TAbstractdxGridReportLink.GetRowClass(ARow: Integer)
  : TdxReportCellClass;
begin
  Result := TdxReportCell;
end;

function TAbstractdxGridReportLink.GetRowParent(AReportCells: TdxReportCells;
  ARow: Integer): TdxReportCell;
begin
  if HeadersOnEveryPage and IsHeaderRow(ARow) then
    Result := AReportCells.HeaderCells
  else if FootersOnEveryPage and IsFooterRow(ARow) then
...
function TAbstractdxGridReportLink.GetCellEdge3DSoft
  (AItem: TAbstractdxReportCellData; ACol, ARow: Integer): Boolean;
begin
...
function TAbstractdxGridReportLink.GetCellEdge3DStyle
  (AItem: TAbstractdxReportCellData; ACol, ARow: Integer): TdxCellEdgeStyle;
begin
  if DrawMode = gdmBorrowSource then
...
function TAbstractdxGridReportLink.GetCellEdgeMode
  (AItem: TAbstractdxReportCellData; ACol, ARow: Integer): TdxCellEdgeMode;
begin
  if DrawMode = gdmBorrowSource then
    Result := GetSourceCellEdgeMode(ACol, ARow)
  else if IsFixedCell(ACol, ARow) and Effects3D then
    Result := cem3DEffects
  else
    Result := cemPattern;
end;
...
function TAbstractdxGridReportLink.GetInternalRowHeight(Index: Integer)
  : Integer;
begin
...
procedure TAbstractdxGridReportLink.GetSelectedRange(out ABeginCol, AEndCol,
  ABeginRow, AEndRow: Integer);
begin
...
function TAbstractdxGridReportLink.GetSourceCellContentBkColor(ACol,
  ARow: Integer): TColor;
begin
  Result := GetSourceCellColor(ACol, ARow);
end;

function TAbstractdxGridReportLink.GetSourceCellColor(ACol,
  ARow: Integer): TColor;
begin
  if IsFixedCell(ACol, ARow) then
    Result := FixedColor
...
function TAbstractdxGridReportLink.GetSourceCellContentPattern(ACol,
  ARow: Integer): TdxPSFillPatternClass;
begin
  Result := TdxPSSolidFillPattern;
end;

function TAbstractdxGridReportLink.GetSourceCellEdge3DSoft(ACol,
  ARow: Integer): Boolean;
begin
  Result := False;
end;

function TAbstractdxGridReportLink.GetSourceCellEdge3DStyle(ACol, ARow: Integer)
  : TdxCellEdgeStyle;
begin
  if IsFixedCell(ACol, ARow) then
    Result := cesRaised
...
function TAbstractdxGridReportLink.GetSourceCellEdgeMode(ACol, ARow: Integer)
  : TdxCellEdgeMode;
begin
  Result := cemPattern;
end;

function TAbstractdxGridReportLink.GetSourceCellFont(ACol,
  ARow: Integer): TFont;
begin
  if IsFixedCell(ACol, ARow) then
    Result := FixedFont
...
function TAbstractdxGridReportLink.GetSourceCellFontIndex(ACol,
...
function TAbstractdxGridReportLink.GetSourceCellMultiline(ACol,
  ARow: Integer): Boolean;
begin
  Result := False;
end;

function TAbstractdxGridReportLink.GetSourceCellTextAlignX(ACol, ARow: Integer)
  : TcxTextAlignX;
begin
  Result := taLeft;
end;

function TAbstractdxGridReportLink.GetSourceCellTextAlignY(ACol, ARow: Integer)
  : TcxTextAlignY;
begin
  Result := taCenterY;
end;

function TAbstractdxGridReportLink.GetSourceCellTransparent(ACol,
  ARow: Integer): Boolean;
begin
  Result := True;
end;
...
begin
  ...
  Result := ((ACol >= BeginCol) and (ACol <= EndCol)) or
    (IncludeFixed and IsFixedCol(ACol));
end;
...
begin
  GetSelectedRange(BeginCol, EndCol, BeginRow, EndRow);
  Result := ((ARow >= BeginRow) and (ARow <= EndRow)) or
    (IncludeFixed and IsFixedRow(ARow));
end;
...
function TAbstractdxGridReportLink.IsProcessedCell(ACol, ARow: Integer)
  : Boolean;
begin
  Result := True;
end;
...
begin
  ...
  Result := ((ACol >= BeginCol) and (ACol <= EndCol) and (ARow >= BeginRow) and
    (ARow <= EndRow));
...
begin
  if ACell.CellCount > 0 then
    ACell.BoundsRect := Rect(0, 0, ACell.LastCell.BoundsRect.Right,
      ACell.LastCell.BoundsRect.Bottom);
end;
...
procedure TAbstractdxGridReportLink.AddVerticalHardDelimiter
  (ADelimiter: TdxReportCell);
begin
  AddVerticalHardDelimiter(ADelimiter.AbsoluteRect.Top);
end;

procedure TAbstractdxGridReportLink.AddHorizontalHardDelimiter
  (ADelimiter: Integer);
begin
...
procedure TAbstractdxGridReportLink.AfterConstruct(AReportCells
  : TdxReportCells);
begin
  FreeAndNil(FScreenCanvas);
end;

procedure TAbstractdxGridReportLink.BeforeConstruct(AReportCells
  : TdxReportCells);
begin
  FScreenCanvas := TdxPSReportRenderScreenCanvas.Create;
end;

procedure TAbstractdxGridReportLink.PrepareConstruct(AReportCells
  : TdxReportCells);
begin
  FRowWidth := -1;
...
procedure TAbstractdxGridReportLink.UnprepareConstruct
  (AReportCells: TdxReportCells);
begin
end;
...
begin
  case DrawMode of
    ...
    gdmChess:
      if IsFixedCell(ACol, ARow) then
        Result := FixedColor
      else if not Odd((ACol - FixedColCount) + (ARow - FixedRowCount)) then
        Result := Color
      else
        Result := EvenColor;
...
function TAbstractdxGridReportLink.GetCellContentBkColor(ACol,
  ARow: Integer): TColor;
begin
  if DrawMode = gdmBorrowSource then
...
function TAbstractdxGridReportLink.GetCellContentPattern(ACol, ARow: Integer)
  : TdxPSFillPatternClass;
begin
  if DrawMode = gdmBorrowSource then
...
function TAbstractdxGridReportLink.GetCellTransparent(ACol,
  ARow: Integer): Boolean;
begin
  if DrawMode = gdmBorrowSource then
    Result := GetSourceCellTransparent(ACol, ARow)
  else if IsFixedCell(ACol, ARow) then
...
function TAbstractdxGridReportLink.GetCellFontIndex(ACol,
  ARow: Integer): Integer;
begin
  case DrawMode of
    gdmStrict:
...
function TAbstractdxGridReportLink.GetCellHasImage(ACol, ARow: Integer)
  : Boolean;
begin
  Result := False;
end;
...
function TAbstractdxGridReportLink.GetCellImageIndex(ACol,
  ARow: Integer): Integer;
begin
  Result := -1;
end;

function TAbstractdxGridReportLink.GetCellImageLayout(ACol, ARow: Integer)
  : TdxImageLayout;
begin
  Result := ilImageCenterLeft;
end;

function TAbstractdxGridReportLink.GetCellImageList(ACol, ARow: Integer)
  : TCustomImageList;
begin
  Result := nil;
end;

function TAbstractdxGridReportLink.GetCellImageMakeSpaceForEmpty(ACol,
  ARow: Integer): Boolean;
begin
  Result := True;
end;

function TAbstractdxGridReportLink.GetCellImageTransparent(ACol,
  ARow: Integer): Boolean;
begin
  Result := True;
end;

function TAbstractdxGridReportLink.GetCellMultiline(ACol,
  ARow: Integer): Boolean;
begin
  if DrawMode = gdmBorrowSource then
...
function TAbstractdxGridReportLink.GetCellSides(ACol, ARow: Integer)
  : TdxCellSides;
var
  ABeginCol, AEndCol, ABeginRow, AEndRow: Integer;
begin
  ...
  begin
    if ACol = ActualFirstCol then
      Exclude(Result, csLeft);
    if ACol = ActualLastCol then
      Exclude(Result, csRight);
    if ARow = ActualFirstRow then
      Exclude(Result, csTop);
  ...
  begin
    ...
    begin
      ...
      else if ARow < FixedRowCount - Byte(IsDrawHorzLines) then
        Exclude(Result, csBottom);
    end;
    ...
    begin
      ...
      else if ACol < FixedColCount - Byte(IsDrawVertLines) then
        Exclude(Result, csRight);
    end;
  end
  else
  begin
    ...
    begin
      ...
      if ARow < AEndRow then
        if (ARow > ABeginRow) or (HasSelection and IncludeFixed and
          (FixedRowCount > 0)) then
          Result := Result - [csTop, csBottom]
        else
          Exclude(Result, csBottom)
      else if (ARow > ABeginRow) or (HasSelection and IncludeFixed and
        (FixedRowCount > 0)) then
        Exclude(Result, csTop);
    end;
    ...
    begin
      ...
      if ACol < AEndCol then
        if (ACol > ABeginCol) or (HasSelection and IncludeFixed and
          (FixedColCount > 0)) then
          Result := Result - [csLeft, csRight]
        else
          Exclude(Result, csRight)
      else if (ACol > ABeginCol) or (HasSelection and IncludeFixed and
        (FixedColCount > 0)) then
        Exclude(Result, csLeft)
    end;
  end;
...
function TAbstractdxGridReportLink.GetCellTextAlignX(ACol, ARow: Integer)
  : TcxTextAlignX;
begin
  if DrawMode = gdmBorrowSource then
...
function TAbstractdxGridReportLink.GetCellTextAlignY(ACol, ARow: Integer)
  : TcxTextAlignY;
begin
  if DrawMode = gdmBorrowSource then
...
function TAbstractdxGridReportLink.GetColSortOrder(ACol: Integer)
  : TdxCellSortOrder;
begin
  Result := csoNone;
end;
...
begin
  if IsAggregated and SysUtils.Supports(TObject(Controller),
...
function TAbstractdxGridReportLink.GetReportRowByRow(Row: Integer)
  : TdxReportCell;
var
  I: Integer;
begin
...
procedure TAbstractdxGridReportLink.SetColumnWidth(Index: Integer;
  Value: Integer);
begin
...
procedure TAbstractdxGridReportLink.SetRowHeight(Index: Integer;
  Value: Integer);
begin
