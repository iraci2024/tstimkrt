...
type
  ...
  public
    ...
    function TryToGetValueDocumentModel(const AValue: TValue)
...
procedure TdxDocVariableField.Initialize(APieceTable: TdxCustomPieceTable;
...
function TdxDocVariableField.GetAllowedUpdateFieldTypes(const AOptions
  : TdxFieldUpdateOnLoadOptions): TdxUpdateFieldOperationTypes;
begin
  Result := [TdxUpdateFieldOperationType.Copy,
...
function TdxDocVariableField.GetCalculatedValueCore(ASourcePieceTable
...
begin
  AValue := ATable.DocumentModel.Variables.GetVariableValue(FVariableName,
    FArguments);
  ...
  ACommand := TdxPieceTableInsertContentConvertedToDocumentModelCommand.Create
    (ATargetModel.MainPieceTable, AValueDocumentModel, 0, False);
  try
  ...
  Result := TdxCalculatedFieldValue.Create
    ...
    TdxFieldResultOption.SuppressMergeUseFirstParagraphStyle]);
end;

function TdxDocVariableField.TryToGetValueDocumentModel(const AValue: TValue)
  : TdxCustomDocumentModel;
var
...
begin
  Result := nil;
  AServer := TdxInternalRichEditDocumentServer.
    TryConvertInternalRichEditDocumentServer(AValue);
  if AServer <> nil then
