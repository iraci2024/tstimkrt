...
uses
  Types, Windows, Messages, Controls, Generics.Defaults, Generics.Collections,
  ComCtrls,
  cxClasses, dxCoreClasses, cxTextEdit, StdCtrls, dxSpellCheckerCore,
  cxRichEdit, cxRichEditUtils,
...
type
  ...
  TdxSpellCheckerCheckAsYouTypeAddUnderlineRectCallback = reference to procedure
    (const R: TRect);
  ...
  IdxSpellCheckerCheckAsYouTypeAdapter = interface(IdxSpellCheckerAdapter)
    ...
    procedure GetVisibleTextBounds(out AStartIndex,
      AEndIndex: IdxSpellCheckerPosition);
  ...
  TdxSpellCheckerCustomEditAdapter = class(TInterfacedObject,
  ...
  public
    ...
    procedure GetSpellingBounds(out ASpellingStart, ASpellingEnd: Integer);
      overload; virtual;
    procedure GetSpellingBounds(out AStart,
      AFinish: IdxSpellCheckerPosition); overload;
  ...
  TdxSpellCheckerCxTextEditAdapter = class(TdxSpellCheckerCustomEditAdapter,
    IdxSpellCheckerAutoCorrectAdapter, IdxSpellCheckerCheckAsYouTypeAdapter)
  strict private
    FLineHeight: Integer;
  ...
  protected
    ...
    procedure GetVisibleTextBounds(out AStartIndex, AEndIndex: Integer);
      overload; virtual;
    ...
    procedure GetVisibleTextBounds(out AStartIndex,
      AEndIndex: IdxSpellCheckerPosition); overload;
  ...
  protected
    ...
    function InternalGetLineHeightByFont(ALineIndex: Integer): Integer;
      override;
...
uses
  dxCore, SysUtils, cxContainer, cxEdit, Math, cxGraphics, cxGeometry,
  dxDrawRichTextUtils, cxMemo,
...
procedure TdxSpellCheckerCustomEditAdapter.GetSpellingBounds(out AStart,
...
procedure TdxSpellCheckerCustomEditAdapter.GetSelection(out AStart,
  AFinish: IdxSpellCheckerPosition);
begin
...
procedure TdxSpellCheckerCustomEditAdapter.GetSpellingBounds(out ASpellingStart,
  ASpellingEnd: Integer);
begin
  ASpellingStart := 0;
  if EditorHandleAllocated then
...
function TdxSpellCheckerCustomEditAdapter.CreateController
...
begin
  if Supports(GetInnerControlContainer(FEdit), IdxSpellCheckerControl, AEdit)
    and AUpdateValue and not EditorHandleAllocated then
    AEdit.SetValue(FText);
end;

procedure TdxSpellCheckerCustomEditAdapter.Replace(var AStart,
  AFinish: IdxSpellCheckerPosition; const AWord: string;
...
begin
  ...
  begin
    if AStart.Compare(ASpellingStart) < 0 then
      ASpellingStart := ASpellingStart.Add
        (TdxSpellCheckerIntegerPosition.Create(ADelta));
    if AStart.Compare(ASpellingFinish) < 0 then
      ASpellingFinish := ASpellingFinish.Add
        (TdxSpellCheckerIntegerPosition.Create(ADelta));
  end;
end;

procedure TdxSpellCheckerCustomEditAdapter.SetSelection(const AStart,
  AFinish: IdxSpellCheckerPosition);
begin
...
procedure TdxSpellCheckerCustomEditAdapter.UpdateController
  (AController: IdxSpellCheckTextController);
var
...
class function TdxSpellCheckerCustomEditAdapter.CreateAdapter(AObject: TObject)
  : IdxSpellCheckerAdapter;
begin
...
begin
  if Supports(GetInnerControlContainer(Edit), IdxSpellCheckerControl, AIntf)
  then
...
procedure TdxSpellCheckerCxTextEditAdapter.CalculateBounds(AStartIndex,
...
function TdxSpellCheckerCxTextEditAdapter.GetCharIndex
  (const APoint: TPoint): Integer;
begin
  Result := LoWord(SendMessage(EditorHandle, EM_CHARFROMPOS, 0,
    MakeLParam(APoint.X, APoint.Y)));
  if Result = MAXWORD then
    Result := -1;
end;

function TdxSpellCheckerCxTextEditAdapter.GetCharPosition
...
function TdxSpellCheckerCxTextEditAdapter.GetFirstCharIndexInLine
...
function TdxSpellCheckerCxTextEditAdapter.GetLineIndex
  (ACharIndex: Integer): Integer;
begin
  Result := SendMessage(EditorHandle, EM_LINEFROMCHAR, ACharIndex, 0);
end;

function TdxSpellCheckerCxTextEditAdapter.GetLineLength
  (ALineIndex: Integer): Integer;
begin
  Result := SendMessage(EditorHandle, EM_LINELENGTH,
    GetFirstCharIndexInLine(ALineIndex), 0);
end;
...
function TdxSpellCheckerCxTextEditAdapter.GetUnderlineRect(APosition,
  ALength: Integer): TRect;
...
procedure TdxSpellCheckerCxTextEditAdapter.GetVisibleTextBounds(out AStartIndex,
  AEndIndex: Integer);
var
  R: TRect;
begin
...
function TdxSpellCheckerCxTextEditAdapter.InternalGetLineHeightByFont
  (ALineIndex: Integer): Integer;
begin
  Result := cxTextExtent(SendMessage(EditorHandle, WM_GETFONT, 0, 0),
    dxMeasurePattern).cy;
end;

function TdxSpellCheckerCxTextEditAdapter.GetCursorPosition
  : IdxSpellCheckerPosition;
begin
...
procedure TdxSpellCheckerCxTextEditAdapter.ApplyChanges
  (const AInfo: TdxSpellCheckerAutoCorrectWordInfo);
begin
  dxSpellCheckerAutoCorrectWord := AInfo.ToWordRange;
  PostMessageW(GetInnerControlContainer(Edit).Handle, DXM_SPELL_AUTOCORRECT, 0,
    LPARAM(@dxSpellCheckerAutoCorrectWord));
end;

procedure TdxSpellCheckerCxTextEditAdapter.CalculateBounds(const AStart,
  AFinish: IdxSpellCheckerPosition;
  ACallback: TdxSpellCheckerCheckAsYouTypeAddUnderlineRectCallback);
begin
...
function TdxSpellCheckerCxRichEditAdapter.GetCharIndex
  (const APoint: TPoint): Integer;
begin
  Result := SendMessage(EditorHandle, EM_CHARFROMPOS, 0, LPARAM(@APoint));
end;

function TdxSpellCheckerCxRichEditAdapter.GetCharPosition
  (ACharIndex: Integer): TPoint;
begin
...
function TdxSpellCheckerCxRichEditAdapter.GetUnderlineRect(APosition,
  ALength: Integer): TRect;

  function GetLineHeight(ACharIndex, ALineIndex: Integer;
    const ACharPosition: TPoint): Integer;
  begin
    if IsMultiLine then
    begin
  ...
  function GetTextWidth(ACharIndex, ALineIndex: Integer; ALength: Integer;
    const ACharPosition: TPoint): Integer;
  begin
...
begin
  ...
  Result.TopLeft := GetCharPosition(APosition);
  Result.Bottom := Result.Top + GetLineHeight(APosition, ALineIndex,
    Result.TopLeft);
  Result.Right := Result.Left + GetTextWidth(APosition, ALineIndex, ALength,
    Result.TopLeft);
end;

function TdxSpellCheckerCxRichEditAdapter.InternalGetLineHeightByFont
  (ALineIndex: Integer): Integer;
var
  ACharIndex: Integer;
...
begin
  ...
  try
    ...
    cxDrawRichEdit(DC, ARect, EditorHandle, ACharIndex,
      ACharIndex + GetLineLength(ALineIndex), True, Result);
    if Boolean(SendMessage(EditorHandle, EM_GETZOOM, WPARAM(@ANumerator),
      LPARAM(@ADenominator))) then
    begin
...
begin
  ...
  else if FEdit is TcxCustomRichEdit then
    Result := True
  else
    Result := False;
end;
...
var
  AIntf: IdxSpellCheckerControl;
begin
  FText := Copy(FText, 1, FSelStart) + AValue +
    Copy(FText, FSelStart + FSelLength + 1, Length(FText) - FSelStart -
    FSelLength);
  if EditorHandleAllocated then
  begin
    if Supports(GetInnerControlContainer(Edit), IdxSpellCheckerControl, AIntf)
...
TdxSpellCheckerAdapters.Register(TCustomEdit,
  TdxSpellCheckerEditAdapter.CreateAdapter);
TdxSpellCheckerAdapters.Register(TCustomRichEdit,
  TdxSpellCheckerRichEditAdapter.CreateAdapter);
TdxSpellCheckerAdapters.Register(TcxCustomTextEdit,
  TdxSpellCheckerCxTextEditAdapter.CreateAdapter);
TdxSpellCheckerAdapters.Register(TcxCustomInnerMemo,
  TdxSpellCheckerCxTextEditAdapter.CreateAdapter);
TdxSpellCheckerAdapters.Register(TcxCustomInnerTextEdit,
  TdxSpellCheckerCxTextEditAdapter.CreateAdapter);
TdxSpellCheckerAdapters.Register(TcxCustomRichEdit,
  TdxSpellCheckerCxRichEditAdapter.CreateAdapter);
TdxSpellCheckerAdapters.Register(TcxRichInnerEdit,
  TdxSpellCheckerCxRichEditAdapter.CreateAdapter);
