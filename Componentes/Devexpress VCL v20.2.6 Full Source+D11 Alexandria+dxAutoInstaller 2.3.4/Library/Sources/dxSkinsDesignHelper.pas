...
uses
  ...
  Dialogs, ToolsApi, CheckLst, ExtCtrls,
  Menus, IniFiles, Registry, StdCtrls, dxCoreClasses, cxLookAndFeels,
  cxLookAndFeelPainters, cxClasses, cxLibraryReg;
...
type
  ...
  public
    constructor Create(AOwner: TdxSkinsUnitStateList;
      const ASkinUnitName, AName: string);
  ...
  public
    ...
    property HasDisabledItems: Boolean index susDisabled
      read GetHasItemsWithState;
    property HasEnabledItems: Boolean index susEnabled
      read GetHasItemsWithState;
    property HasUndefinedItems: Boolean index susUndefined
      read GetHasItemsWithState;
  ...
  TdxSkinsProjectSettings = class(TcxIUnknownObject,
  ...
  protected
    ...
    procedure InternalLoadSettings(AConfig: TCustomIniFile;
      const ASection: string);
    procedure InternalSaveSettings(AConfig: TCustomIniFile;
      const ASection: string);
  ...
  public
    ...
    property ShowNotification: Boolean read FShowNotification
      write SetShowNotification;
  ...
  protected
    ...
    function FindMenuItemByName(AParent: TMenuItem; const AName: string)
      : TMenuItem;
  public
    constructor Create; virtual;
    destructor Destroy; override;
  ...
  TdxSkinsDesignHelper = class(TcxIUnknownObject, IOTAModuleNotifier,
  ...
  public
    ...
    property ActiveProject: IOTAProject read FActiveProject
      write SetActiveProject;
...
const
  ...
  sdxSkinsDefaultRegistryPath =
    'Software\Developer Express\ExpressSkins\OptionsEditor';

  sdxSkinCheckListBoxHint = 'Select skins in this list to make them available' +
    #13#10 + 'within the project. Selecting skins automatically adds' + #13#10 +
  ...
  sdxSkinNotifyBoxHint =
    'If enabled, this Editor is automatically invoked when ' + #13#10 +
    'the installed ExpressSkins Library contains new skins. Otherwise, the ' +
    #13#10 + 'Editor is not shown, and new skins are automatically enabled within the '
    + #13#10 +
    'project, if all skins are enabled in a .SKINCFG file. If one or more skins '
    + #13#10 +
    'are disabled in this file, new skins will be automatically disabled in your projects.';

  sdxSkinDefaultHint =
    'Enable this option to store skin options to the registry, ' + #13#10 +
    'and make them default. These options will be applied, by default, to all new projects.';

  BoolToUnitState: array [Boolean] of TdxSkinsUnitsState = (susDisabled,
    susEnabled);
...
type
  ...
  protected
    ...
    property AllowBoldSelection: Boolean read FAllowBoldSelection
      write FAllowBoldSelection;
  end;
...
function TdxSkinsUnitStateList.AddItem(const AUnitName, AName: string)
  : TdxSkinsUnitStateListItem;
begin
...
function TdxSkinsUnitStateList.GetHasItemsWithState
  (AState: TdxSkinsUnitsState): Boolean;
var
  I: Integer;
begin
  Result := False;
...
function TdxSkinsUnitStateList.GetItem(AIndex: Integer)
  : TdxSkinsUnitStateListItem;
begin
...
procedure TdxSkinsUnitStateList.LoadSettings(AIniFile: TCustomIniFile;
  const ASection: string);
var
  I: Integer;
begin
...
procedure TdxSkinsUnitStateList.SaveSettings(AIniFile: TCustomIniFile;
  const ASection: string);
var
  I: Integer;
begin
...
constructor TdxSkinsUnitStateListItem.Create(AOwner: TdxSkinsUnitStateList;
  const ASkinUnitName, AName: string);
begin
  FName := AName;
  FOwner := AOwner;
...
procedure TdxSkinsUnitStateListItem.LoadFromIni(AIniFile: TCustomIniFile;
  const ASection: string);
begin
...
procedure TdxSkinsUnitStateListItem.SaveToIni(AIniFile: TCustomIniFile;
  const ASection: string);
begin
...
procedure TdxSkinsCheckListBox.DrawItem(Index: Integer; Rect: TRect;
...
begin
  ...
  Result := Result or (dxSkinsProjectSettings.Enabled <>
    cbSkinsAutoFilling.Checked);
  Result := Result or (dxSkinsProjectSettings.ShowNotification <>
...
begin
  ...
  else if not FIsDefaultSettings or HasChanges then
...
function TdxSkinsProjectOptionsMenuExpert.CalcMenuItemPosition
  (AParent: TMenuItem): Integer;
var
  AItem: TMenuItem;
begin
...
function TdxSkinsProjectOptionsMenuExpert.CreateMenuItem(AParent: TMenuItem)
  : TMenuItem;
var
  ABitmap: TBitmap;
begin
  ...
  begin
    ...
    begin
      ...
      Result.Bitmap.LoadFromResourceName(HInstance,
        sdxSkinsMenuItemGlyphResName);
...
function TdxSkinsProjectOptionsMenuExpert.FindMenuItemByName(AParent: TMenuItem;
...
procedure TdxSkinsDesignHelper.BeforeCompile(const Project: IOTAProject;
  var Cancel: Boolean);
begin
end;

procedure TdxSkinsDesignHelper.FileNotification
...
function TdxSkinsDesignHelper.RegisterModuleNotifier
  (AModule: IOTAModule): Integer;
begin
  if AModule = nil then
    Result := -1
  else
...
procedure TdxSkinsDesignHelper.UnregisterModuleNotifier(AModule: IOTAModule;
  ID: Integer);
begin
...
function TdxSkinsProjectSettings.CalcIsDefaultSettings: Boolean;
  ...
  function CheckValue(AReg: TRegistryIniFile; const AName: string;
    ATestValue: Boolean): Boolean;
  begin
    Result := AReg.ValueExists(sdxSkinsDefaultRegistryPath, AName) and
      (AReg.ReadBool(sdxSkinsDefaultRegistryPath, AName, ATestValue)
      = ATestValue);
  end;
...
const
  DefaultStatesMap: array [Boolean] of TdxSkinsUnitsState = (susEnabled,
    susDisabled);
var
...
procedure TdxSkinsProjectSettings.InternalLoadSettings(AConfig: TCustomIniFile;
  const ASection: string);
begin
  UnitStateList.Reset;
  UnitStateList.LoadSettings(AConfig, ASection);
  Enabled := AConfig.ReadBool(ASection, sdxSkinsProjectState, True);
  ShowNotification := AConfig.ReadBool(ASection,
    sdxSkinsProjectShowNotifications, True);
end;

procedure TdxSkinsProjectSettings.InternalSaveSettings(AConfig: TCustomIniFile;
  const ASection: string);
begin
  AConfig.WriteBool(ASection, sdxSkinsProjectShowNotifications,
    ShowNotification);
...
begin
  ...
  begin
    ...
    try
      ...
      FIsDefaultSettings := AConfig.ReadBool(sdxSkinsCfgSection,
        sdxSkinsDefault, False);
      if not(FIsDefaultSettings and AHasDefaultSettings) then
      begin
        InternalLoadSettings(AConfig, sdxSkinsCfgSection);
        ARestoreDefaultsNeeded := FIsDefaultSettings and
          not AHasDefaultSettings;
      end;
...
procedure TdxSkinsProjectSettings.PainterChanged
...
begin
  UnitStateList.Clear;
  dxSkinsEnumSkins(procedure(const ASkinName, AUnitName: string)
    begin
