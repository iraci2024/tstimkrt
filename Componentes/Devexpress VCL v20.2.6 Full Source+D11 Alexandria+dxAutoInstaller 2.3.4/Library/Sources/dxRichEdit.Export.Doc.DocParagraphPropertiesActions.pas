...
type
  ...
  public
    ...
    constructor Create(AOutput: TdxMemoryStream;
      AParagraph: TdxParagraph); overload;
    constructor Create(AOutput: TdxMemoryStream;
      AProperties: TdxParagraphProperties;
      ATabs: TdxTabFormattingInfo); overload;
    constructor Create(AOutput: TdxMemoryStream;
      AListLevel: TdxListLevel); overload;
    constructor Create(AOutput: TdxMemoryStream;
    ...
    function CalcLineSpacing(AType: TdxParagraphLineSpacing): SmallInt;
      overload;
  end;
...
constructor TdxDocParagraphPropertiesActions.Create(AOutput: TdxMemoryStream;
  AProperties: TdxParagraphProperties; ATabs: TdxTabFormattingInfo);
begin
  Create(AOutput);
...
procedure TdxDocParagraphPropertiesActions.CreateTableParagraphPropertyModifiers
  (ATableDepth: Integer);
begin
...
begin
  ...
  if (ParagraphFormattingInfo.FirstLineIndentType = TdxParagraphFirstLineIndent.
    Indented) then
...
begin
  if not ParagraphFormattingOptions.UseBackColor or
    (ParagraphFormattingInfo.BackColor = TdxAlphaColors.Transparent) then
    Exit;
  ACommand := TdxDocCommandParagraphShading.Create;
  try
    ACommand.ShadingDescriptor.BackgroundColor :=
      ParagraphFormattingInfo.BackColor;
    ACommand.Write(FWriter);
  finally
    ACommand.Free;
  end;
...
begin
  ...
  try
    AFrameWrapTypeCommand.WrapType :=
      TdxDocWrapTypeCalculator.MapToDocWrapTypeStyle
      (FrameProperties.TextWrapType);
  ...
  try
    ...
    if FrameProperties.HorizontalRule = TdxParagraphFrameHorizontalRule.AtLeast
    then
  ...
  try
    AHorizontalPositionCommand.Value := UnitConverter.ModelUnitsToTwips
      (FrameProperties.X);
  ...
  try
    AVerticalPositionCommand.Value := UnitConverter.ModelUnitsToTwips
      (FrameProperties.Y);
  ...
  try
    AFramePositionCommand.HorizontalAnchor :=
      AFramePositionCommand.HorizontalTypeToDocHorizontalType
      (FrameProperties.HorizontalPositionType);
    AFramePositionCommand.VerticalAnchor :=
      AFramePositionCommand.VerticalTypeToDocVerticalType
...
begin
  ...
  try
    ACommand.CurrentBorder.ConvertFromBorderInfo
      (ParagraphFormattingInfo.LeftBorder, FUnitConverter);
    ACommand.Write(FWriter);
  finally
    ACommand.Free;
  end;
...
begin
  ...
  try
    ACommand.CurrentBorder.ConvertFromBorderInfo
      (ParagraphFormattingInfo.RightBorder, FUnitConverter);
    ACommand.Write(FWriter);
  finally
    ACommand.Free;
  end;
...
begin
  ...
  try
    ACommand.CurrentBorder.ConvertFromBorderInfo
      (ParagraphFormattingInfo.TopBorder, FUnitConverter);
    ACommand.Write(FWriter);
  finally
    ACommand.Free;
  end;
...
begin
  ...
  try
    ACommand.CurrentBorder.ConvertFromBorderInfo
      (ParagraphFormattingInfo.BottomBorder, FUnitConverter);
    ACommand.Write(FWriter);
  finally
    ACommand.Free;
  end;
...
function TdxDocParagraphPropertiesActions.CalcLineSpacingTypeCode
  (AType: TdxParagraphLineSpacing): SmallInt;
begin
  if (AType = TdxParagraphLineSpacing.AtLeast) or
    (AType = TdxParagraphLineSpacing.Exactly) then
    Result := 0
  else
    Result := 1;
end;

function TdxDocParagraphPropertiesActions.CalcLineSpacing
  (AType: TdxParagraphLineSpacing): SmallInt;
var
