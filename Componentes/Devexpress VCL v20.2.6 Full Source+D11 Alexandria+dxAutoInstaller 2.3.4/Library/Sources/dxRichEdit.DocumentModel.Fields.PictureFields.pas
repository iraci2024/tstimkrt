...
type
  ...
  public
    ...
    property SuppressStoreGraphicsDataWithDocument: Boolean
      read FSuppressStoreGraphicsDataWithDocument;
  end;
...
function TdxIncludePictureField.GetSwitchesWithArguments
...
procedure TdxIncludePictureField.Initialize(APieceTable: TdxCustomPieceTable;
...
function TdxIncludePictureField.GetAllowedUpdateFieldTypes(const AOptions
...
begin
  ...
  try
    ...
    try
      ...
      if (ACurrentFileName <> '') and TPath.HasValidPathChars(FDocumentName,
        True) then
      begin
        AUriList.Add(TPath.Combine(TPath.GetDirectoryName(ACurrentFileName),
          FDocumentName));
        AUriList.Add(TPath.Combine(TPath.GetDirectoryName(ACurrentFileName),
          ADocumentName));
      end;
    ...
    AImage := TdxUriOfficeImage.Create(ADocumentModel.ImageCache,
      ADocumentModel, AUriList.ToArray, 0, 0, True);
    try
      AImage.SuppressStorePlaceholder := True;
      ATargetModel := ADocumentModel.GetFieldResultModel;
      InsertInlinePictureInTargetModel(AImage, ATargetModel,
        FSuppressStoreGraphicsDataWithDocument and
...
procedure TdxIncludePictureField.InsertInlinePictureInTargetModel
  (AImage: TdxOfficeImageReference; ATargetModel: TdxSimpleDocumentModel;
  ASuppressStore: Boolean);
begin
...
function TdxShapeField.GetCalculatedValueCore(ASourcePieceTable
  : TdxCustomPieceTable; AMailMergeDataMode: TdxMailMergeDataMode;
  ADocumentField: TdxField): TdxCalculatedFieldValue;
begin
...
function TdxShapeField.GetAllowedUpdateFieldTypes(const AOptions
  : TdxFieldUpdateOnLoadOptions): TdxUpdateFieldOperationTypes;
begin
