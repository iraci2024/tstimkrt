...
uses
  ...
  StdCtrls, dxCore, dxCoreClasses,
  cxControls, cxClasses, cxGraphics, dxTouch, dxGDIPlusClasses, dxGDIPlusApi,
  dxAnimation, cxLookAndFeels,
  cxGeometry, cxScrollBar, dxScrollAnimation, Generics.Defaults,
...
type
  ...
  TdxImageSliderTransitionEffect = (isteNone, isteSlide, isteFade,
    isteSegmentedFade, isteRandomSegmentedFade);

  TdxImageSliderScrollMode = (issmButtons, issmScrollBar);

  TdxImageSliderGetThumbnailEvent = procedure(Sender: TdxCustomImageSlider;
    AFrameIndex: Integer; var AThumbnail: TGraphic) of object;
  ...
  published
    property Indent: Integer read FIndent write SetIndent
      default dxImageSliderThumbnailIndent;
    ...
    property ThumbnailCaptionAlignment: TAlignment
      read FThumbnailCaptionAlignment write SetThumbnailCaptionAlignment
      default taCenter;
  ...
  TdxCustomImageSlider = class(TcxControl, IcxImageCollectionListener,
    IcxMouseTrackingCaller, IdxSkinSupport)
  strict private
  type
{$REGION 'private types'}
    TViewInfoItem = class;

    TDrawItemHandler = procedure(ACanvas: TcxCanvas; AViewInfo: TViewInfoItem)
      of object;
    ...
    TPreviewTouchScrollUIHelper = class(TcxIUnknownObject,
...
{$ENDREGION}
  private
    ...
    function AddViewInfo(const ABounds: TRect; AObjectID: Integer;
      ADrawItem: TDrawItemHandler; AClickHandler: TNotifyEvent)
      : TViewInfoItem; overload;
    function AddViewInfo(const ABounds, AVisibleBounds: TRect;
      AObjectID: Integer; ADrawItem: TDrawItemHandler;
      AClickHandler: TNotifyEvent): TViewInfoItem; overload;
    ...
    function GetPartBounds(const ABounds: TRect; APosition: TcxPosition;
      ASize: Integer): TRect; inline;
    ...
    property HotTrackItem: TViewInfoItem read GetHotTrackItem
  ...
  protected
    ...
    procedure ScrollPreview(Sender: TObject; ScrollCode: TScrollCode;
    ...
    procedure DrawPreviewPage(ACanvas: TcxCanvas;
      AViewInfo: TViewInfoItem); virtual;
    procedure DrawSplitter(ACanvas: TcxCanvas;
      AViewInfo: TViewInfoItem); virtual;
    ...
    procedure Animate(AStartIndex, AFinishIndex: Integer;
      AAnimationMode: TdxDrawAnimationMode;
      ATransition: TdxAnimationTransitionEffect); overload;
    procedure AnimationHandler(Sender: TdxAnimationTransition;
      var APosition: Integer; var AFinished: Boolean);
    procedure DrawFrame(ACanvas: TcxCanvas; const ABounds: TRect;
      AFrameIndex: Integer; AFitMode: TcxImageFitMode); virtual;
    ...
    property AnimationTime: Integer read FAnimationTime write SetAnimationTime
      default dxImageSliderDefaultAnimationTime;
    property Count: Integer read GetCount;
    property ImageFitMode: TcxImageFitMode read FImageFitMode
      write SetImageFitMode;
    property Images: TcxImageCollection read FImages write SetImages;
    property ItemIndex: Integer read FItemIndex write SetItemIndex;
    property MultiFrameImage: TdxSmartGlyph read FMultiFrameImage
      write SetMultiFrameImage;
    property ScrollBarKind: TScrollBarKind read FScrollBarKind
      write SetScrollBarKind default sbHorizontal;
    property ScrollMode: TdxImageSliderScrollMode read FScrollMode
      write SetScrollMode default issmButtons;
    property Thumbnails[Index: Integer]: TdxSmartImage read GetThumbnail;
    property PreviewScrollPosition: Integer read FPreviewScrollPosition
      write SetPreviewScrollPosition;
    property PreviewScrollBar: TdxScrollBarWrapper read FPreviewScrollBar;
    property PreviewOptions: TdxImageSliderPreviewOptions read FPreviewOptions
      write SetPreviewOptions;
    property TransitionEffect: TdxImageSliderTransitionEffect
      read FTransitionEffect write FTransitionEffect;

    property OnChange: TNotifyEvent read FOnChange write FOnChange;
    property OnGetThumbnail: TdxImageSliderGetThumbnailEvent
      read FOnGetThumbnail write FOnGetThumbnail;
  public
...
procedure TdxImageSliderResizePreviewDragAndDropObject.DragAndDrop
  (const P: TPoint; var Accepted: Boolean);
begin
  Accepted := IsHorzSizing and InRange(P.X, GetDragDropArea.Left,
    GetDragDropArea.Right) or not IsHorzSizing and
    InRange(P.X, GetDragDropArea.Left, GetDragDropArea.Right);
  FDragImage.BoundsRect := GetDragImageBounds;
  inherited;
end;

procedure TdxImageSliderResizePreviewDragAndDropObject.EndDragAndDrop
  (Accepted: Boolean);
var
  R: TRect;
begin
  ...
  begin
    ...
    if IsHorzSizing then
      Options.Size := Options.ActuallySize +
        (R.Left - ImageSlider.GetPreviewSplitterBounds.Left) *
        Sign[ImageSlider.GetPreviewPosition = posLeft]
    else
      Options.Size := Options.ActuallySize +
        (R.Top - ImageSlider.GetPreviewSplitterBounds.Top) *
        Sign[ImageSlider.GetPreviewPosition = posTop];
  end;
...
function TdxImageSliderResizePreviewDragAndDropObject.GetDragAndDropCursor
...
begin
  ...
  Result := cxRectOffset(ImageSlider.GetPreviewSplitterBounds,
  ...
  begin
    ...
    else if Result.Bottom > R.Bottom then
      Result := cxRectSetBottom(Result, R.Bottom);
  end;
end;

function TdxImageSliderResizePreviewDragAndDropObject.GetImageSlider
  : TdxCustomImageSlider;
begin
  Result := TdxCustomImageSlider(Control);
end;

function TdxImageSliderResizePreviewDragAndDropObject.GetImmediateStart
  : Boolean;
begin
  Result := True;
end;

function TdxImageSliderResizePreviewDragAndDropObject.GetOptions
  : TdxImageSliderPreviewOptions;
begin
...
begin
  ...
  FThumbnailSize := TcxSize.Create(Self, dxImageSliderThumbnailDefaultWidth,
    dxImageSliderThumbnailDefaultHeight);
...
begin
  Result := IndentsWidth + TdxTextMeasurer.TextLineHeight(ImageSlider.Font) +
    ImageSlider.ScaleFactor.Apply(cxTextOffset);
end;

function TdxImageSliderPreviewOptions.IndentsWidth: Integer;
begin
  Result := (Indent + dxImageSliderThumbnailFrameWidth +
    dxImageSliderThumbnailSelectionFrameWidth) * 2;
end;

function TdxImageSliderPreviewOptions.MinHeight: Integer;
begin
  Result := IndentsHeight + ImageSlider.ScaleFactor.Apply
    (dxImageSliderThumbnailMinSize);
end;

function TdxImageSliderPreviewOptions.MinWidth: Integer;
begin
  Result := IndentsWidth + ImageSlider.ScaleFactor.Apply
    (dxImageSliderThumbnailMinSize);
end;
...
procedure TdxImageSliderPreviewOptions.SetThumbnailCaptionAlignment
  (AValue: TAlignment);
begin
...
begin
  ASize := cxSize
    (Max(ImageSlider.ScaleFactor.Apply(dxImageSliderThumbnailMinSize),
    ThumbnailSize.Width),
    Max(ImageSlider.ScaleFactor.Apply(dxImageSliderThumbnailMinSize),
...
const
  DrawAnimationModeMap: array [TdxImageSliderTransitionEffect]
    of TdxDrawAnimationMode = (amFade, amFade, amFade, amSegmentedFade,
...
begin
  if (TransitionEffect <> isteNone) and IsValidItemIndex(AStartIndex) and
    IsValidItemIndex(AFinishIndex) then
  begin
    if TransitionEffect <> isteSlide then
      ADrawAnimationMode := DrawAnimationModeMap[TransitionEffect]
    else
      ADrawAnimationMode := AnimationScrollMode[IsVerticalScrollMode,
    ...
    Animate(AStartIndex, AFinishIndex, ADrawAnimationMode,
      TransitionMap[ADrawAnimationMode]);
  end
  else
    Invalidate;
end;
...
procedure TdxCustomImageSlider.Animate(AStartIndex, AFinishIndex: Integer;
  AAnimationMode: TdxDrawAnimationMode;
  ATransition: TdxAnimationTransitionEffect);
var
  ARect: TRect;
...
begin
  ...
  cxCopyImage(AStartBitmap, AFinishBitmap, cxNullPoint, cxNullPoint,
    AStartBitmap.ClientRect);
  DrawFrame(AStartBitmap.cxCanvas, AStartBitmap.ClientRect, AStartIndex,
    ImageFitMode);
  DrawFrame(AFinishBitmap.cxCanvas, AFinishBitmap.ClientRect, AFinishIndex,
    ImageFitMode);
  FAnimation := TdxImageAnimationTransition.Create(AStartBitmap, AFinishBitmap,
    AnimationTime, AAnimationMode, ATransition, -1, True);
  FAnimation.OnAnimate := AnimationHandler;
  FAnimation.Resume;
end;

procedure TdxCustomImageSlider.AnimationHandler(Sender: TdxAnimationTransition;
  var APosition: Integer; var AFinished: Boolean);
begin
  if AFinished then
    FAnimation := nil;
...
procedure TdxCustomImageSlider.DrawFrame(ACanvas: TcxCanvas;
  const ABounds: TRect; AFrameIndex: Integer; AFitMode: TcxImageFitMode);
var
...
begin
  ...
  begin
    ...
    try
      ...
      cxDrawImage(ACanvas, ABounds, MultiFrameImage, nil, -1, AFitMode,
        idmNormal, True, nil, ScaleFactor, cxSmoothDrawingNeeded(ABounds,
        MultiFrameImage, ScaleFactor));
    ...
    cxDrawPicture(ACanvas, ABounds, Images.Items[AFrameIndex].Picture, AFitMode,
      ScaleFactor);
end;
...
procedure TdxCustomImageSlider.DrawButton(ACanvas: TcxCanvas;
  AViewInfo: TViewInfoItem);
const
  AStates: array [Boolean] of TcxButtonState = (cxbsHot, cxbsPressed);
  ADirection: array [0 .. 3] of TcxArrowDirection = (adLeft, adRight,
    adUp, adDown);
var
  AState: TcxButtonState;
  AButtonDrawDirection: TcxArrowDirection;
begin
  if not IsSliderButtonsVisible or ((AViewInfo.ObjectID in [0, 2]) and
    not CanGoToPrevImage) or ((AViewInfo.ObjectID in [1, 3]) and
    not CanGoToNextImage) then
    Exit;
  AState := cxbsNormal;
  if HotTrackItem = AViewInfo then
    AState := AStates[MousePressed];
  AButtonDrawDirection := ADirection[AViewInfo.ObjectID];
  LookAndFeelPainter.DrawScaledSliderButton(Canvas, AViewInfo.Bounds,
    AButtonDrawDirection, AState, ScaleFactor)
end;

procedure TdxCustomImageSlider.DrawPreviewPage(ACanvas: TcxCanvas;
  AViewInfo: TViewInfoItem);
var
  R: TRect;
begin
  R := cxRectSetBottom(AViewInfo.Bounds, AViewInfo.Bounds.Bottom,
    TdxTextMeasurer.TextLineHeight(Font) + ScaleFactor.Apply(cxTextOffset));
  ACanvas.Brush.Style := bsClear;
  ACanvas.Font := Font;
  if AViewInfo.ObjectID = ItemIndex then
    ACanvas.Font.Color :=
      LookAndFeelPainter.DefaultSchedulerViewSelectedTextColor
  else
    ACanvas.Font.Color := LookAndFeelPainter.PrintPreviewBackgroundTextColor;

  ACanvas.DrawTexT(IntToStr(AViewInfo.ObjectID),
    cxRectInflate(R, -(dxImageSliderThumbnailFrameWidth +
    dxImageSliderThumbnailSelectionFrameWidth), 0),
    PreviewOptions.ThumbnailCaptionAlignment, vaTop, False, False);
  ...
  ACanvas.FrameRect(R, LookAndFeelPainter.DefaultContentTextColor,
    dxImageSliderThumbnailFrameWidth);
  R := cxRectInflate(R, -dxImageSliderThumbnailFrameWidth);
  ACanvas.StretchDraw(R, Thumbnails[AViewInfo.ObjectID]);
end;

procedure TdxCustomImageSlider.DrawSplitter(ACanvas: TcxCanvas;
  AViewInfo: TViewInfoItem);
begin
  EraseBackground(ACanvas, AViewInfo.Bounds);
  LookAndFeelPainter.DrawScaledSplitter(ACanvas, AViewInfo.Bounds,
    AViewInfo = HotTrackItem, (AViewInfo = HotTrackItem) and MousePressed,
    GetPreviewPosition in [posTop, posBottom], ScaleFactor);
  ACanvas.ExcludeClipRect(AViewInfo.Bounds);
end;

procedure TdxCustomImageSlider.DrawOpaqueBackground(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
...
function TdxCustomImageSlider.GetGestureClient(const APoint: TPoint)
  : IdxGestureClient;
begin
...
function TdxCustomImageSlider.GetTouchScrollUIOwner(const APoint: TPoint)
  : IdxTouchScrollUIOwner;
begin
  Result := nil;
  if not PtInRect(GetPreviewSplitterBounds, APoint) then
    ...
    else if PtInRect(ClientBounds, APoint) then
      Result := Self;
end;
...
begin
  ...
  PreviewScrollBar.SetScrollParams(0, AMax, PreviewScrollPosition,
    APageSize, False);
...
begin
  ...
  begin
    if IsVerticalScrollMode and (ADeltaY > 0) or not IsVerticalScrollMode and
      (ADeltaX > 0) then
      GoToPrevImage
    else
      GoToNextImage;
...
function TdxCustomImageSlider.GetMainScrollBarsClass
...
const
  AContentPosition: array [Boolean, TcxPosition] of TcxPosition =
    ((posNone, posLeft, posLeft, posTop, posTop), (posNone, posRight, posRight,
    posTop, posTop));
  ASplitterPosition: array [TcxPosition] of TcxPosition = (posNone, posLeft,
    posRight, posTop, posBottom);
begin
  if not IsPreviewVisible then
    Exit;
  FSplitter := AddViewInfo(GetPreviewSplitterBounds, -1, DrawSplitter,
    SplitterClick);
  APreviewBounds := GetPartBounds(GetPreviewBounds,
    AContentPosition[UseRightToLeftScrollBar, GetPreviewPosition],
    GetPreviewSize - GetPreviewSplitterSize - IfThen(not IsPopupScrollBars,
    GetPreviewScrollBarSize));
  if UseRightToLeftScrollBar then
  begin
    if GetPreviewPosition = posLeft then
      OffsetRect(APreviewBounds, -GetPreviewSplitterSize, 0);
  end
  else if GetPreviewPosition = posRight then
    OffsetRect(APreviewBounds, GetPreviewSplitterSize, 0)
  else if GetPreviewPosition = posBottom then
    OffsetRect(APreviewBounds, 0, GetPreviewSplitterSize);

  AOffset := PreviewOptions.IndentsWidth div 2;
  APageBounds := cxRectSetOrigin(cxRect(PreviewOptions.ThumbnailSize.Size),
    cxRectOffset(APreviewBounds, AOffset, AOffset).TopLeft);
  Inc(APageBounds.Bottom, TdxTextMeasurer.TextLineHeight(Font) +
  ...
  begin
    AOffset := Max(0, cxRectHeight(APreviewBounds) - PreviewOptions.Indent -
      (PreviewOptions.ThumbnailSize.Height + PreviewOptions.IndentsHeight -
      PreviewOptions.Indent) * PreviewRowCount);
    OffsetRect(APageBounds, -PreviewScrollPosition, AOffset div 2)
  end
  else
  begin
    AOffset := Max(0, cxRectWidth(APreviewBounds) - PreviewOptions.Indent -
      (PreviewOptions.ThumbnailSize.Width + PreviewOptions.IndentsWidth -
      PreviewOptions.Indent) * PreviewRowCount);
...
procedure TdxCustomImageSlider.ChangeScaleEx(M, D: Integer;
  isDpiChange: Boolean);
begin
  inherited;
...
function TdxCustomImageSlider.GetDragAndDropObjectClass
  : TcxDragAndDropObjectClass;
begin
  Result := TdxImageSliderResizePreviewDragAndDropObject;
end;

function TdxCustomImageSlider.GetMouseWheelScrollingKind
...
begin
  ...
  Result := GetPartBounds(cxRectInflate(Bounds, -BorderSize),
    GetPreviewPosition, GetPreviewSize);
end;
...
const
  AScrollBarPosition: array [TcxPosition] of TcxPosition = (posNone, posRight,
    posRight, posBottom, posBottom);
var
  R: TRect;
begin
  R := GetPreviewBounds;
  ...
  Result := GetPartBounds(R, AScrollBarPosition[GetPreviewPosition],
    GetPreviewScrollBarSize);
  if UseRightToLeftScrollBar then
...
const
  ASplitterPosition: array [TcxPosition] of TcxPosition = (posNone, posRight,
    posLeft, posBottom, posTop);
begin
  Result := GetPartBounds(GetPreviewBounds,
    ASplitterPosition[GetPreviewPosition], GetPreviewSplitterSize);
end;
...
var
  APos: Integer;
begin
  Result := IsPreviewVisible and PtInRect(GetPreviewBounds,
    ScreenToClient(MousePos)) and (MouseWheelScrollingKind <> mwskNone);
  if Result then
  begin
    ...
    DoSetPreviewScrollPosition(APos - Math.Sign(WheelDelta) *
      dxImageSliderPreviewScrollStep);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      AButtonRect := cxRectBounds(R.Left + AOffset.cx,
        (R.Top + R.Bottom - AButtonSize.cy) div 2, AButtonSize.cx,
        AButtonSize.cy);
      if UseRightToLeftAlignment then
        AButtonRect := TdxRightToLeftLayoutConverter.ConvertRect
          (AButtonRect, R);
      FButtonPrev := AddViewInfo(AButtonRect, 0, DrawButton,
        NavigationButtonClick);
      AButtonRect := cxRectBounds(R.Right - AButtonSize.cx - AOffset.cx,
        (R.Top + R.Bottom - AButtonSize.cy) div 2, AButtonSize.cx,
        AButtonSize.cy);
      if UseRightToLeftAlignment then
        AButtonRect := TdxRightToLeftLayoutConverter.ConvertRect
          (AButtonRect, R);
      FButtonNext := AddViewInfo(AButtonRect, 1, DrawButton,
        NavigationButtonClick);
    end;
  end;
...
procedure TdxCustomImageSlider.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
function TdxCustomImageSlider.NeedPanningFeedback
...
procedure TdxCustomImageSlider.Scroll(AScrollBarKind: TScrollBarKind;
...
procedure TdxCustomImageSlider.ScrollPreview(Sender: TObject;
...
begin
  ...
  case ScrollCode of
    scLineUp, scLineDown:
      DoSetPreviewScrollPosition(APos + Sign[ScrollCode = scLineDown] *
        dxImageSliderPreviewScrollStep);
    scPageUp, scPageDown:
      DoSetPreviewScrollPosition(APos + Sign[ScrollCode = scPageDown] *
        APageSize);
    scTrack, scPosition:
...
begin
  ...
  try
    ...
    if ACustomThumbnail <> nil then
      ...
      DrawFrame(ABitmap.cxCanvas, ABitmap.ClientRect, AIndex,
...
procedure TdxCustomImageSlider.MouseDown(Button: TMouseButton;
...
procedure TdxCustomImageSlider.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  AItem: TViewInfoItem;
begin
...
function TdxCustomImageSlider.AddViewInfo(const ABounds: TRect;
  AObjectID: Integer; ADrawItem: TDrawItemHandler; AClickHandler: TNotifyEvent)
  : TViewInfoItem;
begin
  Result := AddViewInfo(ABounds, ABounds, AObjectID, ADrawItem, AClickHandler);
end;

function TdxCustomImageSlider.AddViewInfo(const ABounds, AVisibleBounds: TRect;
  AObjectID: Integer; ADrawItem: TDrawItemHandler; AClickHandler: TNotifyEvent)
  : TViewInfoItem;
begin
...
function TdxCustomImageSlider.GetPartBounds(const ABounds: TRect;
  APosition: TcxPosition; ASize: Integer): TRect;
begin
...
begin
  ...
  if GetPreviewPosition in [posLeft, posRight] then
    Result := Result div (PreviewOptions.ThumbnailSize.Width +
      PreviewOptions.IndentsWidth - PreviewOptions.Indent)
  else
    Result := Result div (PreviewOptions.ThumbnailSize.Height +
      PreviewOptions.IndentsHeight - PreviewOptions.Indent);
  Result := Max(1, Result);
end;
...
procedure TdxCustomImageSlider.GetPreviewScrollBarInfo(var APageSize,
  AMax: Integer);
var
  R: TRect;
  AIndent: Integer;
begin
  ...
  AIndent := PreviewOptions.Indent + dxImageSliderThumbnailFrameWidth +
    dxImageSliderThumbnailSelectionFrameWidth;
  if GetPreviewPosition in [posLeft, posRight] then
  begin
    APageSize := cxRectHeight(R);
    AMax := PreviewOptions.ThumbnailSize.Height + TdxTextMeasurer.TextLineHeight
      (Font) + ScaleFactor.Apply(cxTextOffset);
  end
  else
  begin
  ...
  AMax := Max((AMax + AIndent) * Ceil(Count / PreviewRowCount) +
    PreviewOptions.Indent, APageSize);
end;

function TdxCustomImageSlider.GetPreviewSize: Integer;
begin
  Result := PreviewOptions.ActuallySize + IfThen(not IsPopupScrollBars,
    GetPreviewScrollBarSize) + GetPreviewSplitterSize;
end;
...
begin
  ...
  begin
    ASize := LookAndFeelPainter.GetScaledSplitterSize(GetPreviewPosition
      in [posTop, posBottom], ScaleFactor);
    if GetPreviewPosition in [posTop, posBottom] then
      Result := Max(ASize.cy,
        ScaleFactor.Apply(dxImageSliderPreviewSplitterDefaultSize))
    else
      Result := Max(ASize.cx,
        ScaleFactor.Apply(dxImageSliderPreviewSplitterDefaultSize));
  end;
...
procedure TdxCustomImageSlider.SetPreviewOptions
  (AValue: TdxImageSliderPreviewOptions);
begin
...
begin
  ...
  if R.Top < R1.Top then
    DoSetPreviewScrollPosition(PreviewScrollPosition -
      (Abs(R.Top) + PreviewOptions.Indent))
  else if R.Bottom > R1.Bottom then
    DoSetPreviewScrollPosition(PreviewScrollPosition + (R.Bottom - R1.Bottom) +
      PreviewOptions.Indent)
  else if FScrollAnimationHelper.IsAnimationActive then
    FScrollAnimationHelper.StopAnimation(False);
...
begin
  ...
  begin
    ...
    try
      ...
      if ACanvas.RectVisible(cxRectInflate(Bounds, dxImageSliderThumbnailIndent))
      then
...
constructor TdxCustomImageSlider.TPreviewTouchScrollUIHelper.Create
...
function TdxCustomImageSlider.TPreviewTouchScrollUIHelper.GetOwnerControl
  : TcxControl;
begin
...
function TdxCustomImageSlider.TPreviewTouchScrollUIHelper.GetManager
...
constructor TdxCustomImageSlider.TPreviewGestureHelper.Create
  (AOwner: TdxCustomImageSlider);
begin
  inherited Create;
  FSlider := AOwner;
...
function TdxCustomImageSlider.TPreviewGestureHelper.AllowGesture
  (AGestureId: Integer): Boolean;
begin
  Result := FSlider.AllowGesture(AGestureId);
end;

function TdxCustomImageSlider.TPreviewGestureHelper.AllowPan
  (AScrollKind: TScrollBarKind): Boolean;
begin
  Result := False;
  if FSlider.GetPreviewPosition in [posLeft, posRight] then
    Result := AScrollKind = sbVertical
  else if FSlider.GetPreviewPosition in [posTop, posBottom] then
    Result := AScrollKind = sbHorizontal;
end;

procedure TdxCustomImageSlider.TPreviewGestureHelper.BeginGestureScroll
  (APos: TPoint);
begin
end;
...
procedure TdxCustomImageSlider.TPreviewGestureHelper.GestureScroll(ADeltaX,
  ADeltaY: Integer);
var
  ANewPos: Integer;
  ADelta: Integer;
...
function TdxCustomImageSlider.TPreviewGestureHelper.IsPanArea
  (const APoint: TPoint): Boolean;
begin
  Result := PtInRect(FSlider.GetPreviewBounds, APoint);
end;

function TdxCustomImageSlider.TPreviewGestureHelper.NeedPanningFeedback
  (AScrollKind: TScrollBarKind): Boolean;
begin
...
function TdxCustomImageSlider.TPreviewScrollAnimationHelper.
