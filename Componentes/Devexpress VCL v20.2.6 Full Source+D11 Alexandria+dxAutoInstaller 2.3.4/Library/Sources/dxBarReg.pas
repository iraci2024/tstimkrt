...
uses
  ...
  dxCoreReg, cxLibraryReg, dxCore, cxPropEditors, cxDesignWindows, cxControls,
  dxBar,
...
uses
  ...
  dxBarStrs, dxBarCustomCustomizationForm, dxBarCustForm, dxBarPopupMenuEd,
  dxBarConverter, dxBarUIGenerator;
...
type
  ...
  public
    ...
    property OnSelectionChanged: TcxNotifyProcedure read FOnSelectionChanged
...
function TdxBarDesignHelper.GetSelectionStatus(AComponent: TPersistent)
  : TdxBarSelectionStatus;
begin
  if FdxBarDesignWindow.IsComponentSelected(AComponent) then
    Result := ssActiveSelected
  else if IsObjectSelected(AComponent) then
    Result := ssInactiveSelected
  else
    Result := ssUnselected;
end;

procedure TdxBarDesignHelper.SelectComponent(AComponent: TPersistent;
  ASelectionOperation: TdxBarSelectionOperation = soExclusive);
begin
  case ASelectionOperation of
    soAdd:
      SelectObject(AComponent, False);
    soExclude:
      UnselectObject(AComponent);
    soExclusive:
      SelectObject(AComponent, True, False);
  end;
end;
...
function TdxBarDesignWindow.IsComponentSelected
  (AComponent: TPersistent): Boolean;
begin
  Result := FCurrentSelectionList.IndexOf(TComponent(AComponent)) <> -1;
end;

procedure TdxBarDesignWindow.Notification(AComponent: TComponent;
...
procedure TdxBarDesignWindow.ItemDeleted(const ADesigner: IDesigner;
  Item: TPersistent);
begin
  FCurrentSelectionList.Remove(Item);
end;

procedure TdxBarDesignWindow.SelectionsChanged(const ASelection
...
procedure TdxBarManagerEditor.PrepareItem(Index: Integer;
...
begin
  case AIndex of
    ...
    1:
      BarManager.AddToolBar;
    2:
      BarManager.AddToolBar(True)
  end;
end;
...
begin
  case AIndex of
    0:
      Result := cxGetResourceString(@dxSBAR_CUSTOMIZINGFORM);
    1:
      Result := 'Add Toolbar';
    2:
      Result := 'Add MainMenu';
...
begin
  dxBarConvertPopupMenu(AObject as TPopupMenu,
    TdxBarCustomPopupMenu(Component));
  AObject.Free;
end;
...
function TdxBarItemLinkImageIndexProperty.GetImages: TCustomImageList;
  ...
  begin
    ...
    if (Result = nil) and (ALinks.Owner is TCustomdxBarSubItem) and
      (TCustomdxBarSubItem(ALinks.Owner).LinkCount > 0) then
      Result := GetCurrentImages(TCustomdxBarSubItem(ALinks.Owner)
        .Links[0].Owner);
  end;
...
function TdxBarScreenTipRepositoryEditor.InternalGetVerb
...
function TdxBarScreenTipRepositoryEditor.GetRepository
...
begin
  ...
  RegisterComponents(dxBarProductPage, [TdxBarManager, TdxBarPopupMenu,
    TdxBarDockControl, TdxBarScreenTipRepository]);
  RegisterNoIcon([TdxBarGroup, TdxBarButton, TdxBarLargeButton, TdxBarEdit,
    TCustomdxBarCombo, TdxBarCombo, TdxBarSeparator, TdxBarSubItem,
    TdxBarListItem, TdxBarContainerItem, TdxBar, TdxBarScreenTip]);
  ...
  RegisterComponentEditor(TdxBarScreenTipRepository,
    TdxBarScreenTipRepositoryEditor);
  RegisterComponentEditor(TdxBarDockControl, TdxBarComponentEditor);

  RegisterPropertyEditor(TypeInfo(TShortCut), TdxBarCustomButton, 'ShortCut',
    TShortCutProperty);
  ...
  RegisterPropertyEditor(TypeInfo(TdxBarItemLinks), TdxBar, 'ItemLinks',
    TdxBarItemLinksPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxBarItemLinks), TdxBarCustomPopupComponent,
    'ItemLinks', TdxBarItemLinksPropertyEditor);
  RegisterPropertyEditor(TypeInfo(TdxBarItemLinks), TCustomdxBarSubItem,
    'ItemLinks', TdxBarItemLinksPropertyEditor);
  ...
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxBarItem, 'Glyph',
    TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxBarItem, 'LargeGlyph',
    TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxBarItemLink, 'UserGlyph',
    TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TdxBarLargeButton, 'HotGlyph',
    TcxBitmapProperty);
...
begin
  ...
  TdxBarManagerAccess(ASender).FdxBarDesignHelper :=
    TdxBarDesignHelper.Create(TComponent(ASender));
end;

procedure UnregisterBarManager(ASender: TObject);
begin
  if (FdxBarDesignWindow <> nil) and
    ((dxBarManagerList = nil) or (dxBarManagerList.Count = 0)) then
