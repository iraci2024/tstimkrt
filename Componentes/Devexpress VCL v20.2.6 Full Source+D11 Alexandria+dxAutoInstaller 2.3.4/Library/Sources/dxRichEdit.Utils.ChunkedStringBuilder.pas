...
type
  ...
  protected type
    ...
    public
      ...
      procedure Clear;
      function ToString(AStartIndex, ALength: Integer): string;
    ...
    private
      function GetItem(AIndex: Integer): TChunk; inline;
    public
      property Items[AIndex: Integer]: TChunk read GetItem; default;
    end;
  ...
  public
    ...
    function Append(const AValue: TCharArray; AStartIndex, ACount: Integer)
      : TdxChunkedStringBuilder; overload;
    function Append(const AValue: string; AStartIndex, ACount: Integer)
      : TdxChunkedStringBuilder; overload;
    ...
    function AppendLine(const AValue: string): TdxChunkedStringBuilder;
      overload;
    ...
    function ToString: string; reintroduce; overload;
    procedure Clear;
    procedure AppendExistingBuffersUnsafe(AStringBuilder
      : TdxChunkedStringBuilder);
  ...
  public
    ...
    procedure WriteLine(const Format: string; Args: array of const);
      overload; override;
    procedure WriteLine(const Value: TCharArray; Index, Count: Integer);
      overload; override;
...
procedure TdxChunkedStringBuilder.TChunk.Append(AChars: Pointer;
  ACount: Integer);
begin
...
begin
  Assert((AIndex >= 0) and (AIndex < FLength),
    Format('Bad Index parameter: GetChar(Index = %d) where Self.Length = %d',
    [AIndex, FLength]));
  Result := FBuffer[AIndex];
end;

procedure TdxChunkedStringBuilder.TChunk.SetChar(AIndex: Integer;
  const AValue: Char);
begin
  Assert((AIndex >= 0) and (AIndex < FLength),
    Format('Bad Index parameter: SetChar(Index = %d) where Self.Length = %d',
    [AIndex, FLength]));
  FBuffer[AIndex] := AValue;
end;

function TdxChunkedStringBuilder.TChunk.ToString(AStartIndex,
  ALength: Integer): string;
begin
  Assert((AStartIndex >= 0) and (AStartIndex < FLength),
    Format('Bad StartIndex parameter: ToString(StartIndex = %d) where Self.Length = %d',
    [AStartIndex, FLength]));
  Assert(AStartIndex + ALength <= FLength,
    Format('Bad Length parameter: ToString(StartIndex = %d, Length = %d) where Self.Length = %d',
    [AStartIndex, ALength, FLength]));
...
function TdxChunkedStringBuilder.TChunks.GetItem(AIndex: Integer): TChunk;
begin
{$IFDEF DEBUG}
...
function TdxChunkedStringBuilder.Append(const AValue: string)
  : TdxChunkedStringBuilder;
begin
  if AValue = '' then
    Exit(Self);
...
function TdxChunkedStringBuilder.Append(const AValue: TCharArray;
  AStartIndex, ACount: Integer): TdxChunkedStringBuilder;
var
  AChunk: TChunk;
...
begin
  ...
  if ASpace >= ACount then
    AChunk.Append(@AValue[AStartIndex +
      {$IFDEF DELPHIXE3} Low(string){$ELSE}1{$ENDIF}], ACount)
  else
  begin
    AIndex := AStartIndex;
    if ASpace > 0 then
    begin
      AChunk.Append(@AValue[AStartIndex +
        {$IFDEF DELPHIXE3} Low(string){$ELSE}1{$ENDIF}], ASpace);
      Inc(AIndex, ASpace);
    end;
    AEndIndex := AStartIndex + ACount;
    while AIndex < AEndIndex do
    begin
      ALength := Min(AEndIndex - AIndex, MaxChunkSize);
      AChunk := TChunk.Create(MaxChunkSize);
      AChunk.Append(@AValue[AIndex +
        {$IFDEF DELPHIXE3} Low(string){$ELSE}1{$ENDIF}], ALength);
      FChunks.Add(AChunk);
      Inc(AIndex, ALength);
    end;
  end;
...
function TdxChunkedStringBuilder.Append(AValue: Double)
  : TdxChunkedStringBuilder;
begin
  Result := Append(FloatToStr(AValue));
end;

function TdxChunkedStringBuilder.Append(AValue: Integer)
  : TdxChunkedStringBuilder;
begin
  Result := Append(IntToStr(AValue));
end;
...
function TdxChunkedStringBuilder.Append(AValue: Single)
  : TdxChunkedStringBuilder;
begin
  Result := Append(FloatToStr(AValue));
end;
...
function TdxChunkedStringBuilder.AppendLine(const AValue: string)
  : TdxChunkedStringBuilder;
begin
  Append(AValue);
  Result := Append(sLineBreak);
...
function TdxChunkedStringBuilder.ToString(AStartIndex: Integer;
  ALength: Integer): string;
var
  AFirstBufferIndex, AFirstBufferOffset, ALastBufferIndex, I,
    AEndIndex: Integer;
  AStringBuilder: TStringBuilder;
begin
  ...
  try
    AStringBuilder.Append(FChunks[AFirstBufferIndex]
      .ToString(AFirstBufferOffset, MaxChunkSize - AFirstBufferOffset));
    for I := AFirstBufferIndex + 1 to ALastBufferIndex - 1 do
      AStringBuilder.Append(FChunks[I].ToString);

    AEndIndex := AStartIndex + ALength;
    AStringBuilder.Append(FChunks[ALastBufferIndex].ToString(0,
      AEndIndex - ALastBufferIndex * MaxChunkSize));
...
procedure TdxChunkedStringBuilder.AppendExistingBuffersUnsafe
  (AStringBuilder: TdxChunkedStringBuilder);
begin
  if (AStringBuilder = nil) or (AStringBuilder.Length <= 0) then
    Exit;

  if MaxChunkSize <> AStringBuilder.MaxChunkSize then
    raise EArgumentException.CreateFmt('stringBuilder.MaxBufferSize = %d',
      [AStringBuilder.MaxChunkSize]);
...
constructor TdxChunkedStringBuilderWriter.Create(AStringBuilder
  : TdxChunkedStringBuilder);
begin
...
function TdxChunkedStringBuilderWriter.GetStringBuilder
  : TdxChunkedStringBuilder;
begin
...
procedure TdxChunkedStringBuilderWriter.Write(const ABuffer: TArray<Char>;
...
procedure TdxChunkedStringBuilderWriter.WriteLine(const Format: string;
  Args: array of const);
begin
  if FIsOpen then
  begin
