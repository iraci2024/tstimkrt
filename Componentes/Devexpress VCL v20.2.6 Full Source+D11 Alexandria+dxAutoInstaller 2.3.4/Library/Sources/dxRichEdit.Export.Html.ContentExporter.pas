...
type
  ...
  TdxWebImageAlign = (NotSet, Left, Right, Baseline, Top, Middle, Bottom,
  ...
  public
    ...
    property CausesValidation: Boolean read GetCausesValidation
      write SetCausesValidation;
    property HRef: string read GetHRef write SetHRef;
    property Name: string read GetName write SetName;
    property Target: string read GetTarget write SetTarget;
    property Title: string read GetTitle write SetTitle;
    property ValidationGroup: string read GetValidationGroup
      write SetValidationGroup;
  end;
  ...
  public
    constructor Create(AHtmlTextWriterTag
      : TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span;
      ADataControl: TdxHtmlGenericControl = nil); reintroduce;
    destructor Destroy; override;
  end;
  ...
  public
    constructor Create(AHtmlTextWriterTag: TdxHtmlTextWriterTag;
      ...
      ACurrentLevelIndex: Integer); reintroduce;

    property AbstractListIndex: TdxAbstractNumberingListIndex
      read FAbstractListIndex;
  ...
  public
    ...
    property DescriptionUrl: string read GetDescriptionUrl
      write SetDescriptionUrl;
    property GenerateEmptyAlternateText: Boolean
      read GetGenerateEmptyAlternateText write SetGenerateEmptyAlternateText;
    property ImageAlign: TdxWebImageAlign read GetImageAlign
      write SetImageAlign;
  ...
  public
    ...
    procedure ApplyImageProperties(AImageControl: TdxWebImageControl;
      AImage: TdxOfficeImage; const AActualSize: TSize;
      const AImageRepository: IdxOfficeImageRepository;
      ADisposeConvertedImagesImmediately: Boolean); overload;
    procedure ApplyImageProperties(AImageControl: TdxWebImageControl;
      AImage: TdxOfficeImage; const AActualSize: TSize;
      const AImageRepository: IdxOfficeImageRepository;
      ADisposeConvertedImagesImmediately, ACorrectWidth: Boolean); overload;
    procedure ApplyImageProperties(AImageControl: TdxWebImageControl;
      AImage: TdxOfficeImage; const AActualSize: TSize;
      const AImageRepository: IdxOfficeImageRepository;
      ADisposeConvertedImagesImmediately, ACorrectWidth, AAlwaysWriteImageSize
      : Boolean); overload;
    procedure ApplyImageProperties(AImageControl: TdxWebImageControl;
      AImage: TdxOfficeImage; const AActualSize: TSize;
      const AImageRepository: IdxOfficeImageRepository;
      ADisposeConvertedImagesImmediately, ACorrectWidth, AAlwaysWriteImageSize,
      AKeepImageSize: Boolean); overload;
    function GetHtmlImage(AImage: TdxOfficeImage; const AActualSize: TSize;
      ACorrectWidth: Boolean): TdxOfficeImageReference;
    function CreateBitmapFromSourceImage(AImage: TdxOfficeImage;
      AWidth, AHeight: Integer; ACorrectWidth: Boolean)
      : TdxOfficeImageReference;
    procedure CalculateDefaultImageResolution;

    property HorizontalResolution: Single read FHorizontalResolution
      write FHorizontalResolution;
    property VerticalResolution: Single read FVerticalResolution
      write FVerticalResolution;
  end;
  ...
  public
    constructor Create(AFontCacheIndex: Integer;
      AForeColor, ABackColor: TdxAlphaColor; AUnderline, AStrikeout: Boolean;
      AScript: TdxCharacterFormattingScript; AAllCaps: Boolean;
      ADoubleFontSize: Integer);
  ...
  TdxHtmlVisitableDocumentIntervalBasedObjectBoundaryIterator = class
  ...
  public
    function CreateReplacementInfo(const AText: string;
      AReplaceTable: TdxCharStringDictionary; const ATabMarker: string)
      : TdxReplacementInfo; overload;
    function PerformReplacements(const AText: string;
      AReplaceTable: TdxCharStringDictionary; const ATabMarker: string)
      : string; overload;
  end;
  ...
  const
{$REGION 'AlignHT'}
    AlignHT: array [TdxParagraphAlignment] of string = ('left', 'right',
      'center', 'justify');
{$ENDREGION}
  strict private
    FUnitConverter: TdxDocumentModelUnitConverter;
  ...
  protected
    function ShouldWriteNumberingListTextIndent
      (AParagraph: TdxParagraph): Boolean;
    function CalculateParagraphLeftIndent(AParagraph: TdxParagraph;
  ...
  public
    constructor Create(AUnitConverter: TdxDocumentModelUnitConverter;
      AStyleHelper: TdxHtmlStyleHelper);

    function GetHtmlLineHeight(ASpacingType: TdxParagraphLineSpacing;
      ASpacing, AFontSizeInPoints: Single): string;
    ...
    function GetBulletType(const AFontName: string;
      const ADisplayFormat: string): string;
    function IsNotChangedDisplayFormat(const ADisplayFormat: string): Boolean;
    function GetHtmlParagraphStyle(AParagraph: TdxParagraph;
      AListControlNestingLevel, AParentLevelOffset: Integer): string; overload;
    procedure GetHtmlParagraphStyle(AParagraph: TdxParagraph;
      AListControlNestingLevel, AParentLevelOffset: Integer;
      AStyle: TdxCssStyleCollection); overload;
    procedure GetHtmlParagraphStyleCore(AParagraph: TdxParagraph;
      AListControlNestingLevel, AParentLevelOffset: Integer;
      AStyle: TdxCssStyleCollection);
    function GetHtmlParagraphInListStyle(AParagraph: TdxParagraph;
      const AListLevel: IdxListLevel; AListControlNestingLevel,
      AParentLevelOffset: Integer): string; overload;
    procedure GetHtmlParagraphInListStyle(AParagraph: TdxParagraph;
      const AListLevel: IdxListLevel; AListControlNestingLevel,
      AParentLevelOffset: Integer; AStyle: TdxCssStyleCollection;
      ADefaultCharacterPropertiesExportToCss: Boolean); overload;
    procedure GetHtmlParagraphInListStyle(AParagraph: TdxParagraph;
      const AListLevel: IdxListLevel; AListControlNestingLevel,
      AParentLevelOffset: Integer; AStyle: TdxCssStyleCollection); overload;
    function GetLineSpacingValue(ASpacingType: TdxParagraphLineSpacing;
      ASpacing: Single; AFontSizeInPoints: Single): string;
    function GetLeftPaddingValue(ALeftIndent: Integer;
      AFirstLineIndentType: TdxParagraphFirstLineIndent;
      AFirstLineIndent: Integer): Integer;
    function GetTextIndentValue(AFirstLineIndentType
      : TdxParagraphFirstLineIndent; AFirstLineIndent: Integer): string;
  ...
  public
    ...
    function CreateMsoTabStopAttributeValue(ATabCount: Integer;
      ALeader: TdxTabLeaderType): string;
    ...
    function GetFillCharacterWidth(AFontInfo: TdxFontInfo;
      ALeader: TdxTabLeaderType): Integer;
  ...
  TdxHtmlStyleHelper = class
  strict private
    class var FSpecialCharactersReplaceTable: TdxCharStringDictionary;
    class constructor Initialize;
    class destructor Finalize;
    class function CreateSpecialCharacterReplaceTable
  ...
  public
    ...
    function PreprocessHtmlContentText(const AText: string;
      const ATabMarker: string): string;
    function CreateCssStyle(AAttributes: TStringList; ASeparator: Char)
      : string; overload;
    function CreateCssStyle(AAttributes: TdxCssStyleCollection;
      ASeparator: Char): string; overload;
  end;
  ...
  protected
    ...
    function CreateHtmlExportHelper(const AScriptContainer: IdxScriptContainer;
      const AImageRepository: IdxOfficeImageRepository;
      AOptions: TdxHtmlDocumentExporterOptions): TdxHtmlExportHelper; virtual;
    function GetAllowSkipParagraphInCell: Boolean; override;
    function CreateVisitableDocumentIntervalBoundaryIterator
      : TdxVisitableDocumentIntervalBasedObjectBoundaryIterator; override;
    ...
    procedure AddControlToChild(AParent: TdxWebControlBase;
      AControl: TdxWebControlBase); virtual;
    procedure ExportCore(ATextProperties: TdxHtmlExporterTextProperties;
    ...
    function ExportParagraphInList(AParagraph: TdxParagraph)
      : TdxParagraphIndex; virtual;
    procedure ExportParagraphNumeration(AParagraph: TdxParagraph;
      const ACounters: TIntegerDynArray);
    function ExportParagraphInListCore(AParagraph: TdxParagraph;
      AParagraphControl: TdxWebControlBase; const ACounters: TIntegerDynArray)
      : TdxParagraphIndex; virtual;
    function ExportParagraphCore(AParagraph: TdxParagraph;
      AParagraphControl: TdxWebControlBase): TdxParagraphIndex;
    function ExportParagraphContent(AParagraph: TdxParagraph)
      : TdxParagraphIndex;
    ...
    function CreateHtmlExporterTextProperties(AFontCacheIndex: Integer;
      AForeColor, ABackColor: TdxAlphaColor; AUnderline, AStrikeout: Boolean;
      AScript: TdxCharacterFormattingScript; AAllCaps: Boolean;
      ADoubleFontSize: Integer): TdxHtmlExporterTextProperties; virtual;
    procedure StartHyperlinkExport(AHyperlinkInfo: TdxHyperlinkInfo);
      overload; virtual;
    procedure StartHyperlinkExport(AHyperlinkInfo: TdxHyperlinkInfo;
      ATextProperties: TdxHtmlExporterTextProperties); overload; virtual;
    procedure StartHyperlinkExportCore(AControl: TdxWebControlBase;
      AHyperlinkInfo: TdxHyperlinkInfo);
    function GetHyperlinkInfo(ARun: TdxTextRun): TdxHyperlinkInfo;
    function CreateHyperlinkControl(AHyperlinkInfo: TdxHyperlinkInfo)
      : TdxHtmlAnchor; overload; virtual;
    function CreateHyperlinkControl(AHyperlinkInfo: TdxHyperlinkInfo;
      ATextProperties: TdxHtmlExporterTextProperties): TdxHtmlAnchor;
      overload; virtual;
    function CreateHyperlinkControlCore(AHyperlinkInfo: TdxHyperlinkInfo)
      : TdxHtmlAnchor;
    procedure ExportFieldResultEndRun(ARun: TdxFieldResultEndRun); override;
    procedure FinishHyperlinkExport; virtual;
    function CreateParagraphControl(AParagraph: TdxParagraph)
      : TdxWebControlBase;
    function CreateParagraphInListControl(AParagraph: TdxParagraph;
      const ACounters: TIntegerDynArray): TdxWebControlBase;
    ...
    procedure EnsureNumberingListControl(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList;
      const ACounters: TIntegerDynArray); virtual;
    function IsRootFirstParagraphInListHasNonZeroLevel
      (AParagraph: TdxParagraph): Boolean;
    procedure CreateSeveralParentNumberingListToMakeNewNumberingListNested
      (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList;
      const ACounters: TIntegerDynArray);
    procedure CloseListsWithSmallerLevelIndex(AListLevelIndex: Integer);
    procedure CreateNumberingListControl(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList; ACounter: Integer); virtual;
    function CreateNumberingListControlCore(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList; ACounter: Integer): TdxWebControlBase;
    procedure ExportTextRun(ARun: TdxTextRun); override;
    function ProcessRunText(ARun: TdxTextRun; const AText: string)
      : string; virtual;
    procedure ExportTextRunCore(ARun: TdxTextRun; const AText: string); virtual;
    procedure ExportTextCore(AFontCacheIndex: Integer; const AText: string;
      AForeColor, ABackColor: TdxAlphaColor; AUnderlineType: TdxUnderlineType;
      AStrikeoutType: TdxStrikeoutType; AScript: TdxCharacterFormattingScript;
    ...
    function ShouldExportInlinePicture(ARun: TdxInlinePictureRun)
      : Boolean; override;
    ...
    procedure ExportFloatingObjectFrame(ARun: TdxFloatingObjectAnchorRun;
      AControl: TdxHtmlGenericControl); virtual;
    procedure ExportFloatingObjectContent
      (ARun: TdxFloatingObjectAnchorRun); virtual;
    procedure ExportFloatingObjectTextBoxContent
      (AContent: TdxTextBoxFloatingObjectContent;
      ARun: TdxFloatingObjectAnchorRun); virtual;
    procedure ExportFloatingObjectPictureContent
      (AContent: TdxPictureFloatingObjectContent;
      ARun: TdxFloatingObjectAnchorRun); virtual;
    function CreateImageControl(AContent: TdxPictureFloatingObjectContent;
      ARun: TdxFloatingObjectAnchorRun): TdxWebControlBase; virtual;
  ...
  public
    ...
    function CreateListLevelControl(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList; ACounter: Integer): TdxWebControlBase;
  ...
  strict private
    ...
    FStyleHelper: TdxHtmlStyleHelper;
  protected
    function GetHtmlTableCellStyle(ACell: TdxTableCell;
      AActualBottomBorderCell: TdxBorderBase): string; overload;
    ...
    function GetHtmlVerticalAlignment(AVAlignment
      : TdxVerticalAlignment): string;
    function GetHtmlTableAlignment(ATableAlignment: TdxTableRowAlignment)
      : string; virtual;
  public
    constructor Create(AUnitConverter: TdxDocumentModelUnitConverter;
      AStyleHelper: TdxHtmlStyleHelper);
    function GetHtmlTableStyle(ATable: TdxTable): string; overload;
    procedure GetHtmlTableStyle(ATable: TdxTable;
      AStyle: TdxCssStyleCollection); overload;
    ...
    procedure GetHtmlTableCellStyle(ACell: TdxTableCell;
      AStyle: TdxCssStyleCollection;
      AActualBottomBorderCell: TdxBorderBase); overload;
  ...
  protected
    ...
    procedure GenerateSpaces(ACount: Integer;
      AForceUseNonBreakingSpace: Boolean);
    function CalculateParagraphTag(AParagraph: TdxParagraph;
      AIgnoreOutlineLevel: Boolean): TdxHtmlTextWriterTag; virtual;

    property DisposeConvertedImagesImmediately: Boolean
      read GetDisposeConvertedImagesImmediately;
  ...
  public
    ...
    class function CreateDefaultCharacterProperties(ADocumentModel
      : TdxDocumentModel): TdxCharacterFormattingInfo; static;
    class procedure RemoveDefaultProperties(ADocumentModel: TdxDocumentModel;
      ...
      overload; static;
    class procedure TryRemoveKey(ACssStyleCollection: TdxCssStyleCollection;
      const AKey: string); static;

    function ConvertWidthUnitToPixels(AWidthUnit: TdxWidthUnit): string;
    function CreateImageControl(AImage: TdxOfficeImage;
      const AActualSize: TSize; ACssFloat: TdxHtmlCssFloat)
      : TdxWebControlBase; overload;
    function CreateImageControl(ARun: TdxInlinePictureRun)
      : TdxWebControlBase; overload;
    function CreateImageControlInternal(AImage: TdxOfficeImage;
      const AActualSize: TSize; ACssFloat: TdxHtmlCssFloat): TdxWebImageControl;
    function CreateListLevelControl(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList; AListControlNestingLevel,
      AParentLevelOffset, ACounter: Integer): TdxWebControlBase; virtual;
    function CreateNumberingListControl(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList; ACounter: Integer): TdxWebControlBase;
    function CreateParagraphControl(AParagraph: TdxParagraph;
      AIgnoreOutlineLevel: Boolean; AListControlNestingLevel, AParentLevelOffset
      : Integer): TdxWebControlBase; virtual;
    function CreateTableCellControl(ACell: TdxTableCell;
      AMergedCellProperties: TdxVerticalMergeCellProperties): TdxWebControlBase;
    ...
    function CreateTextControl(const AText: string;
      ATextProperties: TdxHtmlExporterTextProperties): TdxWebControlBase;
    function GetMostNestedCounter(const ACounters: TIntegerDynArray): Integer;
    function ShouldAddNumberingListStartAttribute(AParagraph: TdxParagraph;
      ANumberingList: TdxNumberingList): Boolean;
    procedure AppendText(AParent: TdxWebControlBase; const AText: string);
    procedure RemoveDefaultProperties(ATextProperties
      : TdxHtmlExporterTextProperties; AFontInfo: TdxFontInfo;
      ACssStyleCollection: TdxCssStyleCollection); overload;
    procedure SetTextProperties(AControl: TdxHtmlControl;
...
type
  ...
  TdxVisitableDocumentIntervalBoundaryProxy = class
...
begin
  Result := FormatFloat('0.##', Value,
    TdxCultureInfo.InvariantCulture.FormatSettings);
end;
...
function TdxHtmlContainerControl.CreateControlCollection
...
procedure TdxHtmlContainerControl.AddAttributesToRender
...
constructor TdxHtmlGenericControl.Create(AHtmlTextWriterTag
  : TdxHtmlTextWriterTag = TdxHtmlTextWriterTag.Span;
...
constructor TdxNumberingListWebControl.Create(AHtmlTextWriterTag
  : TdxHtmlTextWriterTag; AAbstractListIndex: TdxAbstractNumberingListIndex;
  ACurrentLevelIndex: Integer);
begin
  inherited Create(AHtmlTextWriterTag);
...
procedure TdxWebImageControl.SetGenerateEmptyAlternateText
...
begin
  if (AValue < TdxWebImageAlign.NotSet) or (AValue > TdxWebImageAlign.TextTop)
  then
...
begin
  ...
  begin
    if TdxStringHelper.StartsWith(AImageUrl, 'file://') then
      AWriter.AddAttribute('src', TdxNullableString.Create(True,
        AImageUrl), False)
    else
  ...
  if Length(AImageUrl) > 0 then
    AWriter.AddAttribute(TdxHtmlTextWriterAttribute.Longdesc,
      inherited ResolveClientUrl(AImageUrl));
...
procedure TdxHtmlLiteralControl.InitRecursive(ANamingContainer
  : TdxWebControlBase);
begin
...
function TdxStyleWebControl.IsClientScriptBlockRegistered
  (const AKey: string): Boolean;
begin
  Result := True;
end;

procedure TdxStyleWebControl.RegisterClientScriptBlock(const AKey,
  AScript: string);
begin
...
procedure TdxStyleWebControl.RegisterCommonCssStyle(const AStyle,
  ATagName: string);
begin
...
begin
  Result := 'cs' + TdxStringHelper.TrimStart(IntToHex(dxElfHash(AStyle),
    8), ['0']);
end;

{ TdxOfficeHtmlImageHelper }

constructor TdxOfficeHtmlImageHelper.Create(const ADocumentModel
  : TdxDocumentModel; AUnitConverter: TdxDocumentModelUnitConverter;
  AOptions: TdxHtmlDocumentExporterOptions);
begin
  inherited Create;
...
function TdxOfficeHtmlImageHelper.ShouldConvertImage
  (AImage: TdxOfficeImage): Boolean;
begin
  case AImage.RawFormat of
    TdxOfficeImageFormat.Wmf, TdxOfficeImageFormat.Emf,
      TdxOfficeImageFormat.Exif, TdxOfficeImageFormat.Icon,
      TdxOfficeImageFormat.MemoryBmp, TdxOfficeImageFormat.None:
      Result := True;
  else
    Result := False;
  end;
...
procedure TdxOfficeHtmlImageHelper.ApplyImageProperties(AImageControl
  : TdxWebImageControl; AImage: TdxOfficeImage; const AActualSize: TSize;
  const AImageRepository: IdxOfficeImageRepository;
  ADisposeConvertedImagesImmediately: Boolean);
begin
  ApplyImageProperties(AImageControl, AImage, AActualSize, AImageRepository,
    ADisposeConvertedImagesImmediately, True);
end;

procedure TdxOfficeHtmlImageHelper.ApplyImageProperties(AImageControl
  : TdxWebImageControl; AImage: TdxOfficeImage; const AActualSize: TSize;
  const AImageRepository: IdxOfficeImageRepository;
  ADisposeConvertedImagesImmediately, ACorrectWidth: Boolean);
begin
  ApplyImageProperties(AImageControl, AImage, AActualSize, AImageRepository,
    ADisposeConvertedImagesImmediately, ACorrectWidth, False);
end;
...
begin
  ApplyImageProperties(AImageControl, AImage, AActualSize, AImageRepository,
    ADisposeConvertedImagesImmediately, ACorrectWidth,
    AAlwaysWriteImageSize, False);
end;
...
procedure TdxOfficeHtmlImageHelper.ApplyImageProperties(AImageControl
  ...
  begin
    ...
    begin
      ...
      ASource.Resize(FUnitConverter.ModelUnitsToPixels(AActualSize.Width,
        HorizontalResolution), FUnitConverter.ModelUnitsToPixels
        (AActualSize.Height, HorizontalResolution));
...
begin
  ...
  begin
    AUriImageWithoutSize := (AUriOfficeImage <> nil) and
      ((AUriOfficeImage.PixelTargetWidth = 0) and
      (AUriOfficeImage.PixelTargetHeight = 0));
  ...
  begin
    AImageControl.Attributes.Add('width',
      IntToStr(FUnitConverter.ModelUnitsToPixels(AActualSize.Width,
      HorizontalResolution)));
    AImageControl.Attributes.Add('height',
      IntToStr(FUnitConverter.ModelUnitsToPixels(AActualSize.Height,
      VerticalResolution)));
  end
  else
  begin
...
function TdxOfficeHtmlImageHelper.GetHtmlImage(AImage: TdxOfficeImage;
  const AActualSize: TSize; ACorrectWidth: Boolean): TdxOfficeImageReference;
var
  AFinalWidth, AFinalHeight: Integer;
begin
  AFinalWidth := FUnitConverter.ModelUnitsToPixels(AActualSize.cx,
    HorizontalResolution);
  AFinalHeight := FUnitConverter.ModelUnitsToPixels(AActualSize.cy,
    VerticalResolution);
  Result := CreateBitmapFromSourceImage(AImage, AFinalWidth, AFinalHeight,
    ACorrectWidth);
  Result.Uri := AImage.Uri;
end;

function TdxOfficeHtmlImageHelper.CreateBitmapFromSourceImage
  (AImage: TdxOfficeImage; AWidth, AHeight: Integer; ACorrectWidth: Boolean)
  : TdxOfficeImageReference;
var
...
begin
  ...
  ABitmap := TdxOfficeImage.CreateSize
    (Max(1, IfThen(ACorrectWidth, AWidth - AGdiPlusIssueWidthCorrectionValue,
    AWidth)), Max(1, AHeight));
  try
    ACanvas := ABitmap.CreateCanvas;
    try
      ACanvas.Clear(TdxAlphaColors.Transparent);
      ACanvas.Draw(AImage, TRect.Create(0, 0, AWidth, AHeight),
        AImage.ClientRect);
    finally
      ACanvas.Free;
    end;
...
constructor TdxHtmlExporterTextProperties.Create(AFontCacheIndex: Integer;
  AForeColor, ABackColor: TdxAlphaColor; AUnderline, AStrikeout: Boolean;
  AScript: TdxCharacterFormattingScript; AAllCaps: Boolean;
  ADoubleFontSize: Integer);
begin
...
begin
  ...
  if AOther = nil then
    Exit(False);
  Result := (FontCacheIndex = AOther.FontCacheIndex) and
    (ForeColor = AOther.ForeColor) and (BackColor = AOther.BackColor) and
    (Underline = AOther.Underline) and (Strikeout = AOther.Strikeout) and
    (AllCaps = AOther.AllCaps) and (DoubleFontSize = AOther.DoubleFontSize);
end;

function TdxHtmlExporterTextProperties.GetHashCode: Integer;
begin
  Result := FontCacheIndex xor Integer(ForeColor) xor Integer(BackColor)
    xor Ord(Script) xor DoubleFontSize;
  if AllCaps then
...
constructor TdxVisitableDocumentIntervalBoundaryProxy.Create
...
begin
  ...
  FModelPosition := TdxPositionConverter.ToDocumentModelPosition
    (FBoundary.VisitableInterval.PieceTable, FPosition);
end;
...
function TdxVisitableDocumentIntervalBoundaryProxy.GetPosition
  : PdxDocumentModelPosition;
begin
  FModelPosition := TdxPositionConverter.ToDocumentModelPosition
    (FBoundary.VisitableInterval.PieceTable, FPosition);
...
function TdxVisitableDocumentIntervalBoundaryProxy.GetOrder
  : TdxBookmarkBoundaryOrder;
begin
  Result := FBoundary.Order;
end;

procedure TdxVisitableDocumentIntervalBoundaryProxy.Export(const AExporter
  : IdxDocumentModelExporter);
begin
  FBoundary.Export(AExporter);
end;

function TdxVisitableDocumentIntervalBoundaryProxy.CreateBox
  : TdxVisitableDocumentIntervalBox;
begin
  Result := FBoundary.CreateBox;
end;

procedure TdxVisitableDocumentIntervalBoundaryProxy.ChangePosition
...
procedure TdxHtmlVisitableDocumentIntervalBasedObjectBoundaryIterator.
  PopulateBookmarksCore(ABookmarks: TdxBookmarkList);
var
  ...
  ACount, I: Integer;
  APrevEndBoundary, AStartBoundary, AEndBoundary
    : TdxVisitableDocumentIntervalBoundaryProxy;
  ...
  try
    ...
    try
      ...
      begin
        ...
        AStartBoundary := TdxVisitableDocumentIntervalBoundaryProxy.Create
          (AStartBoundaryFactory.Boundary);
        AStartBoundary.IntervalIndex := I;
        if (APrevEndBoundary <> nil) and
          (AStartBoundary.Position^ < APrevEndBoundary.Position^) then
          APrevEndBoundary.ChangePosition(AStartBoundary.Position.LogPosition);
        AHyperlink := PieceTable.GetHyperlinkField
          (AStartBoundary.Position.RunIndex);
        if AHyperlink <> nil then
          AStartBoundary.ChangePosition
            (TdxDocumentModelPosition.FromRunEnd(PieceTable,
            AHyperlink.LastRunIndex).LogPosition);
        Boundaries.Add(AStartBoundary);

        AInterval.Visit(AEndBoundaryFactory);
        AEndBoundary := TdxVisitableDocumentIntervalBoundaryProxy.Create
          (AEndBoundaryFactory.Boundary);
        ...
        else if AEndBoundary.Position^ > AStartBoundary.Position^ then
        begin
          AHyperlink := PieceTable.GetHyperlinkField
            (AEndBoundary.Position.RunIndex);
          if AHyperlink <> nil then
            AEndBoundary.ChangePosition
              (TdxDocumentModelPosition.FromRunStart(PieceTable,
              AHyperlink.FirstRunIndex).LogPosition);
        end;
...
function TdxHtmlFastCharacterMultiReplacement.CreateReplacementInfo
  (const AText: string; AReplaceTable: TdxCharStringDictionary;
  const ATabMarker: string): TdxReplacementInfo;
var
  I: Integer;
  AReplaceWith: string;
begin
...
function TdxHtmlFastCharacterMultiReplacement.PerformReplacements
  (const AText: string; AReplaceTable: TdxCharStringDictionary;
  const ATabMarker: string): string;
var
...
constructor TdxHtmlParagraphStyleHelper.Create(AUnitConverter
  : TdxDocumentModelUnitConverter; AStyleHelper: TdxHtmlStyleHelper);
begin
  inherited Create;
  FUnitConverter := AUnitConverter;
...
function TdxHtmlParagraphStyleHelper.GetHtmlLineHeight
  (ASpacingType: TdxParagraphLineSpacing;
  ASpacing, AFontSizeInPoints: Single): string;
var
  AValue: Single;
begin
  case ASpacingType of
    ...
    TdxParagraphLineSpacing.Exactly:
      Result := HtmlStyleHelper.GetHtmlSizeInPoints
        (UnitConverter.ModelUnitsToPointsF(ASpacing));
...
function TdxHtmlParagraphStyleHelper.GetHtmlAlignment
  (AAlign: TdxParagraphAlignment): string;
begin
  Result := AlignHT[AAlign];
end;

function TdxHtmlParagraphStyleHelper.GetHtmlListLevelType(const AListLevel
  : IdxListLevel): string;
var
...
begin
  ...
  if IsNotChangedDisplayFormat(ADisplayFormat) and
    (AFormat <> TdxNumberingFormat.Bullet) then
    Exit('disc');
  Result := '';
  case AFormat of
    TdxNumberingFormat.Bullet:
      Result := GetBulletType(AListLevel.CharacterProperties.FontName,
        ADisplayFormat);
...
begin
  if AFontName = 'Courier New' then
    Exit('circle')
  else if AFontName = 'Wingdings' then
    Exit('square')
  else
  begin
...
function TdxHtmlParagraphStyleHelper.IsNotChangedDisplayFormat
  (const ADisplayFormat: string): Boolean;
begin
  Result := Format(ADisplayFormat, ['1', '1', '1', '1', '1', '1', '1', '1', '1']
    ) = ADisplayFormat;
end;

function TdxHtmlParagraphStyleHelper.GetHtmlParagraphStyle
  (AParagraph: TdxParagraph; AListControlNestingLevel, AParentLevelOffset
  : Integer): string;
var
  AStyle: TdxCssStyleCollection;
begin
  ...
  try
    GetHtmlParagraphStyle(AParagraph, AListControlNestingLevel,
      AParentLevelOffset, AStyle);
...
procedure TdxHtmlParagraphStyleHelper.GetHtmlParagraphStyle
  (AParagraph: TdxParagraph; AListControlNestingLevel, AParentLevelOffset
  : Integer; AStyle: TdxCssStyleCollection);
begin
  GetHtmlParagraphStyleCore(AParagraph, AListControlNestingLevel,
    AParentLevelOffset, AStyle);
end;

procedure TdxHtmlParagraphStyleHelper.GetHtmlParagraphStyleCore
  (AParagraph: TdxParagraph; AListControlNestingLevel, AParentLevelOffset
  : Integer; AStyle: TdxCssStyleCollection);
var
  ALeftIndent: Integer;
...
begin
  ...
  if not AParagraph.IsInList or ShouldWriteNumberingListTextIndent(AParagraph)
  then
    AStyle.Add('text-indent', GetTextIndentValue(AParagraph.FirstLineIndentType,
      AParagraph.FirstLineIndent));

  ALeftIndent := CalculateParagraphLeftIndent(AParagraph,
    AListControlNestingLevel, AParentLevelOffset);
  if AParagraph.ContextualSpacing then
    ASpacingBefore := 0
  else
    ASpacingBefore := UnitConverter.ModelUnitsToPointsFRound
      (AParagraph.SpacingBefore);
  if AParagraph.ContextualSpacing then
    ASpacingAfter := 0
  else
    ASpacingAfter := UnitConverter.ModelUnitsToPointsFRound
      (AParagraph.SpacingAfter);
  AStyle.Add('margin', HtmlStyleHelper.GetHtmlSizeInPoints(ASpacingBefore) + ' '
    + HtmlStyleHelper.GetHtmlSizeInPoints(UnitConverter.ModelUnitsToPointsFRound
    (AParagraph.RightIndent)) + ' ' + HtmlStyleHelper.GetHtmlSizeInPoints
    (ASpacingAfter) + ' ' + HtmlStyleHelper.GetHtmlSizeInPoints
    (UnitConverter.ModelUnitsToPointsFRound(GetLeftPaddingValue(ALeftIndent,
    AParagraph.FirstLineIndentType, AParagraph.FirstLineIndent))));

  if AParagraph.LineSpacingType <> TdxParagraphLineSpacing.Single then
  begin
    AProperties := AParagraph.GetMergedCharacterProperties;
    try
      AStyle.Add('line-height', GetLineSpacingValue(AParagraph.LineSpacingType,
        AParagraph.LineSpacing, AProperties.Info.DoubleFontSize / 2));
    finally
      AProperties.Free;
    end;
  end;
...
function TdxHtmlParagraphStyleHelper.ShouldWriteNumberingListTextIndent
  (AParagraph: TdxParagraph): Boolean;
var
  AFirstLineIndentType: TdxParagraphFirstLineIndent;
begin
  AFirstLineIndentType := AParagraph.FirstLineIndentType;
  if (AFirstLineIndentType = TdxParagraphFirstLineIndent.Hanging) or
...
function TdxHtmlParagraphStyleHelper.CalculateParagraphLeftIndent
  (AParagraph: TdxParagraph; AListControlNestingLevel, AParentLevelOffset
  : Integer): Integer;
begin
  ...
  begin
    if AListControlNestingLevel = 1 then
      Result := Max(0, AParagraph.LeftIndent -
        UnitConverter.DocumentsToModelUnits(150))
    else
...
function TdxHtmlParagraphStyleHelper.GetHtmlParagraphInListStyle
  (AParagraph: TdxParagraph; const AListLevel: IdxListLevel;
...
begin
  AStyle := TdxCssStyleCollection.Create;
  try
    GetHtmlParagraphInListStyle(AParagraph, AListLevel,
      AListControlNestingLevel, AParentLevelOffset, AStyle);
    Result := HtmlStyleHelper.CreateCssStyle(AStyle, ';');
  finally
    AStyle.Free;
  end;
...
procedure TdxHtmlParagraphStyleHelper.GetHtmlParagraphInListStyle
  (AParagraph: TdxParagraph; const AListLevel: IdxListLevel;
  AListControlNestingLevel: Integer; AParentLevelOffset: Integer;
  AStyle: TdxCssStyleCollection; ADefaultCharacterPropertiesExportToCss
  : Boolean);
var
...
begin
  ...
  try
    ...
    if AFontName = 'Symbol' then
      AFontName := 'Arial';
    TdxHtmlStyleRender.GetHtmlStyle(AFontName, AInfo.DoubleFontSize / 2,
      TdxGraphicUnit.guPoint, AInfo.FontBold, AInfo.FontItalic, False, False,
      AForeColor, ABackColor, AStyle);
    if not ADefaultCharacterPropertiesExportToCss then
      TdxHtmlExportHelper.RemoveDefaultProperties(AParagraph.DocumentModel,
        AInfo, AInfo.FontName, AInfo.FontBold, AInfo.FontItalic,
...
procedure TdxHtmlParagraphStyleHelper.GetHtmlParagraphInListStyle
  ...
  AStyle: TdxCssStyleCollection);
begin
  GetHtmlParagraphInListStyle(AParagraph, AListLevel, AListControlNestingLevel,
    AParentLevelOffset, AStyle, True);
end;

function TdxHtmlParagraphStyleHelper.GetLineSpacingValue
  (ASpacingType: TdxParagraphLineSpacing; ASpacing: Single;
...
function TdxHtmlParagraphStyleHelper.GetTextIndentValue(AFirstLineIndentType
  : TdxParagraphFirstLineIndent; AFirstLineIndent: Integer): string;
begin
  case AFirstLineIndentType of
    TdxParagraphFirstLineIndent.Hanging:
      Result := HtmlStyleHelper.GetHtmlSizeInPoints
        (UnitConverter.ModelUnitsToPointsFRound(-AFirstLineIndent));
...
constructor TdxHtmlParagraphTabsHelper.Create(AUnitConverter
  : TdxDocumentModelUnitConverter; AStyleHelper: TdxHtmlStyleHelper);
begin
  inherited Create;
  FUnitConverter := AUnitConverter;
...
function TdxHtmlParagraphTabsHelper.CreateTabPlaceholder(AWidth: Integer;
  ASpaceWidth: Integer; AFillCharacterWidth: Integer;
  AFillCharacter: Char): string;
var
...
function TdxHtmlParagraphTabsHelper.CreateTabStops
  (AParagraph: TdxParagraph): string;
var
...
function TdxHtmlParagraphTabsHelper.CreateTabStopAttribute
  (const ATab: TdxTabInfo): string;
var
...
begin
  ...
  try
    ...
    AAttributes.Add(HtmlStyleHelper.GetHtmlSizeInPoints
      (UnitConverter.ModelUnitsToPointsF(ATab.Position)));
...
function TdxHtmlParagraphTabsHelper.CreateMsoTabStopAttributeValue
  (ATabCount: Integer; ALeader: TdxTabLeaderType): string;
var
  AAttributes: TStringList;
begin
  AAttributes := TStringList.Create;
...
function TdxHtmlParagraphTabsHelper.GetHtmlTabInfoAlign
  (AAlign: TdxTabAlignmentType): string;
begin
  case AAlign of
...
function TdxHtmlParagraphTabsHelper.GetHtmlTabInfoLeader
  (ALeader: TdxTabLeaderType): string;
begin
  case ALeader of
...
function TdxHtmlParagraphTabsHelper.GetFillCharacter
  (ALeader: TdxTabLeaderType): Char;
begin
  case ALeader of
    TdxTabLeaderType.Dots:
...
function TdxHtmlParagraphTabsHelper.GetFillCharacterWidth
  (AFontInfo: TdxFontInfo; ALeader: TdxTabLeaderType): Integer;
begin
  case ALeader of
    TdxTabLeaderType.Dots:
...
begin
  ...
  FReplacement := TdxHtmlFastCharacterMultiReplacement.Create
    (FPreProcessHtmlContentTextStringBuilder);
  FCreateCssStyleStringCollectionStringBuilder :=
    TdxChunkedStringBuilder.Create;
...
class function TdxHtmlStyleHelper.CreateSpecialCharacterReplaceTable
...
function TdxHtmlStyleHelper.PreprocessHtmlContentText(const AText: string;
  const ATabMarker: string): string;
begin
  Result := FReplacement.PerformReplacements(AText,
    FSpecialCharactersReplaceTable, ATabMarker);
end;

function TdxHtmlStyleHelper.CreateCssStyle(AAttributes: TStringList;
...
function TdxHtmlStyleHelper.CreateCssStyle(AAttributes: TdxCssStyleCollection;
  ASeparator: Char): string;
var
...
constructor TdxHtmlContentExporter.Create(ADocumentModel: TdxDocumentModel;
  const AScriptContainer: IdxScriptContainer;
  const AImageRepository: IdxOfficeImageRepository;
  AOptions: TdxHtmlDocumentExporterOptions);
begin
  ...
  FExportHelper := CreateHtmlExportHelper(AScriptContainer, AImageRepository,
    AOptions);
end;
...
function TdxHtmlContentExporter.CreateHtmlExportHelper(const AScriptContainer
  : IdxScriptContainer; const AImageRepository: IdxOfficeImageRepository;
  AOptions: TdxHtmlDocumentExporterOptions): TdxHtmlExportHelper;
begin
  Result := TdxHtmlExportHelper.Create(DocumentModel, AScriptContainer,
    AImageRepository, AOptions);
end;
...
function TdxHtmlContentExporter.CreateVisitableDocumentIntervalBoundaryIterator
  : TdxVisitableDocumentIntervalBasedObjectBoundaryIterator;
begin
  Result := TdxHtmlVisitableDocumentIntervalBasedObjectBoundaryIterator.Create
    (PieceTable);
end;
...
procedure TdxHtmlContentExporter.ExportFootEndNotes
  (ANotes: TList<TdxFootNoteExportInfo>);
var
  ACount, I: Integer;
begin
  ACount := ANotes.Count;
...
procedure TdxHtmlContentExporter.AddControlToChild(AParent: TdxWebControlBase;
  AControl: TdxWebControlBase);
begin
  AParent.Controls.Add(AControl);
end;

procedure TdxHtmlContentExporter.ExportCore(ATextProperties
  : TdxHtmlExporterTextProperties; const AText: string);
var
...
begin
  ...
  AForceExportCharacterProperties := (APeek <> nil) and
    (APeek.TagKey = TdxHtmlTextWriterTag.Li);
  ...
  begin
    ...
    if AParentControlsCount > 0 then
      ...
      APrevControl := nil;
    if (APrevControl <> nil) and (FLastCreatedTextControl = APrevControl) and
      ATextProperties.Equals(FLastExportedTextProperties) then
      FExportHelper.AppendText(APrevControl, AText)
    else
    begin
      ATextControl := TdxHtmlControl(FExportHelper.CreateTextControl(AText,
        ATextProperties));
      AddControlToChild(CurrentParent, ATextControl);
      if not(CurrentParent is TdxHtmlAnchor) or
        not ATextProperties.Equals(FLastExportedHyperLinkTextProperties) then
        FExportHelper.SetTextProperties(ATextControl, ATextProperties,
          AForceExportCharacterProperties);
  ...
  begin
    ...
    try
      ...
      FExportHelper.SetTextProperties(AStyle, ATextProperties,
        AForceExportCharacterProperties);
...
procedure TdxHtmlContentExporter.ExportFirstPageHeader(ASectionHeader
  : TdxSectionHeader; ALinkedToPrevious: Boolean);
begin
end;

procedure TdxHtmlContentExporter.ExportOddPageHeader(ASectionHeader
  : TdxSectionHeader; ALinkedToPrevious: Boolean);
begin
end;

procedure TdxHtmlContentExporter.ExportEvenPageHeader(ASectionHeader
  : TdxSectionHeader; ALinkedToPrevious: Boolean);
begin
end;

procedure TdxHtmlContentExporter.ExportFirstPageFooter(ASectionFooter
  : TdxSectionFooter; ALinkedToPrevious: Boolean);
begin
end;

procedure TdxHtmlContentExporter.ExportOddPageFooter(ASectionFooter
  : TdxSectionFooter; ALinkedToPrevious: Boolean);
begin
end;

procedure TdxHtmlContentExporter.ExportEvenPageFooter(ASectionFooter
  : TdxSectionFooter; ALinkedToPrevious: Boolean);
begin
end;

function TdxHtmlContentExporter.ExportTable(ATableInfo: TdxTableInfo)
  : TdxParagraphIndex;
var
  AFirstParagraph: TdxParagraph;
  ATable: TdxWebControlBase;
begin
  AFirstParagraph := TdxParagraph(ATableInfo.Table.PieceTable.Paragraphs
    [ATableInfo.Table.Rows[0].Cells[0].StartParagraphIndex]);
  if (not AFirstParagraph.IsInList) or
    (not AFirstParagraph.ShouldExportNumbering) then
...
procedure TdxHtmlContentExporter.ExportRow(ARow: TdxTableRow;
...
procedure TdxHtmlContentExporter.ExportCell(ACell: TdxTableCell;
  ATableInfo: TdxTableInfo);
var
...
begin
  ...
  if ARowSpan = 0 then
    Exit;
  ATableCell := FExportHelper.CreateTableCellControl(ACell,
    AMergedCellProperties);
  try
...
function TdxHtmlContentExporter.ExportParagraph(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
var
...
function TdxHtmlContentExporter.ExportParagraphInList(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
var
  ACounters: TIntegerDynArray;
  AParagraphControl, AParagraphInListControl: TdxWebControlBase;
begin
  ACounters := PieceTableNumberingListCounters.CalculateNextCounters
    (AParagraph);
  if FOptions.HtmlNumberingListExportFormat = TdxHtmlNumberingListExportFormat.PlainTextFormat
  then
  begin
    AParagraphControl := CreateParagraphControl(AParagraph);
    FCurrentListLeftIndent := AParagraph.LeftIndent;
  ...
  AParagraphInListControl := CreateParagraphInListControl(AParagraph,
    ACounters);
  Result := ExportParagraphCore(AParagraph, AParagraphInListControl);
end;

procedure TdxHtmlContentExporter.ExportParagraphNumeration
  (AParagraph: TdxParagraph; const ACounters: TIntegerDynArray);
var
  ALevel: IdxListLevel;
...
begin
  ...
  AText := AParagraph.GetNumberingListText(ACounters) +
    ALevel.ListLevelProperties.Separator;
  AProperties := AParagraph.GetNumerationCharacterProperties;
  try
    AInfo := AProperties.Info;

    ExportTextCore(AFontCacheIndex, AText, AInfo.ForeColor, AInfo.BackColor,
      AInfo.FontUnderlineType, AInfo.FontStrikeoutType, AInfo.Script,
      AInfo.AllCaps, AInfo.DoubleFontSize);
  finally
    AProperties.Free;
  end;
end;

function TdxHtmlContentExporter.ExportParagraphInListCore
  (AParagraph: TdxParagraph; AParagraphControl: TdxWebControlBase;
...
function TdxHtmlContentExporter.ExportParagraphCore(AParagraph: TdxParagraph;
  AParagraphControl: TdxWebControlBase): TdxParagraphIndex;
begin
  AddControlToChild(CurrentParent, AParagraphControl);
  PushControl(AParagraphControl);
...
function TdxHtmlContentExporter.ExportParagraphContent(AParagraph: TdxParagraph)
  : TdxParagraphIndex;
var
...
function TdxHtmlContentExporter.IsEmptyParagraph
  (AParagraph: TdxParagraph): Boolean;
begin
...
procedure TdxHtmlContentExporter.ExportFieldCodeStartRun
...
procedure TdxHtmlContentExporter.ExportFieldCodeEndRun
...
begin
  ...
  StartHyperlinkExport(AHyperlinkInfo,
    CreateHtmlExporterTextProperties(ARun.FontCacheIndex, AForeColor,
    ABackColor, AUnderline, AStrikeout, ARun.Script, ARun.AllCaps,
    ARun.DoubleFontSize));
end;

function TdxHtmlContentExporter.CreateHtmlExporterTextProperties(AFontCacheIndex
  : Integer; AForeColor, ABackColor: TdxAlphaColor;
  AUnderline, AStrikeout: Boolean; AScript: TdxCharacterFormattingScript;
  AAllCaps: Boolean; ADoubleFontSize: Integer): TdxHtmlExporterTextProperties;
begin
  Result := TdxHtmlExporterTextProperties.Create(AFontCacheIndex, AForeColor,
    ABackColor, AUnderline, AStrikeout, AScript, AAllCaps, ADoubleFontSize);
  FObjectsToDelete.Add(Result);
end;

procedure TdxHtmlContentExporter.StartHyperlinkExport(AHyperlinkInfo
  : TdxHyperlinkInfo);
var
  AControl: TdxWebControlBase;
begin
...
procedure TdxHtmlContentExporter.StartHyperlinkExport(AHyperlinkInfo
  : TdxHyperlinkInfo; ATextProperties: TdxHtmlExporterTextProperties);
var
  AControl: TdxWebControlBase;
begin
...
procedure TdxHtmlContentExporter.StartHyperlinkExportCore
  (AControl: TdxWebControlBase; AHyperlinkInfo: TdxHyperlinkInfo);
var
...
function TdxHtmlContentExporter.GetHyperlinkInfo(ARun: TdxTextRun)
  : TdxHyperlinkInfo;
var
...
begin
  ...
  if not PieceTable.HyperlinkInfos.TryGetHyperlinkInfo(AField.Index, Result)
  then
    Result := nil;
end;

function TdxHtmlContentExporter.CreateHyperlinkControl(AHyperlinkInfo
  : TdxHyperlinkInfo): TdxHtmlAnchor;
begin
...
function TdxHtmlContentExporter.CreateHyperlinkControlCore(AHyperlinkInfo
  : TdxHyperlinkInfo): TdxHtmlAnchor;
begin
  ...
  begin
    if AHyperlinkInfo.Anchor = '' then
      Result.HRef := TdxHyperlinkUriHelper.ConvertToUrl
        (AHyperlinkInfo.NavigateUri)
    else
      Result.HRef := TdxHyperlinkUriHelper.ConvertToUrl
        (AHyperlinkInfo.NavigateUri + '#' + AHyperlinkInfo.Anchor);
  end
  else if AHyperlinkInfo.Anchor <> '' then
...
procedure TdxHtmlContentExporter.ExportFieldResultEndRun
...
function TdxHtmlContentExporter.CreateParagraphControl(AParagraph: TdxParagraph)
  : TdxWebControlBase;
begin
  Result := FExportHelper.CreateParagraphControl(AParagraph,
    FOptions.IgnoreParagraphOutlineLevel, GetListLevelNestingLevel,
    FParentListLeftIndent);
end;

function TdxHtmlContentExporter.CreateParagraphInListControl
  (AParagraph: TdxParagraph; const ACounters: TIntegerDynArray)
  : TdxWebControlBase;
var
  ANumberingList: TdxNumberingList;
begin
  ...
  Result := CreateListLevelControl(AParagraph, ANumberingList,
    FExportHelper.GetMostNestedCounter(ACounters));
end;
...
begin
  ...
  if FListLeftIndents.Count > 0 then
    FParentListLeftIndent := FListLeftIndents.Peek +
      DocumentModel.UnitConverter.DocumentsToModelUnits(150)
  else
...
begin
  ...
  FParentListLeftIndent := FCurrentListLeftIndent +
    DocumentModel.UnitConverter.DocumentsToModelUnits(150);
end;
...
procedure TdxHtmlContentExporter.EnsureNumberingListControl
  (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList;
  const ACounters: TIntegerDynArray);
var
...
begin
  ANumberingListWebControl := Safe<TdxNumberingListWebControl>.Cast
    (CurrentParent);
  if ANumberingListWebControl = nil then
  begin
    if IsRootFirstParagraphInListHasNonZeroLevel(AParagraph) then
      CreateSeveralParentNumberingListToMakeNewNumberingListNested(AParagraph,
        ANumberingList, ACounters);
    CreateNumberingListControl(AParagraph, ANumberingList,
      FExportHelper.GetMostNestedCounter(ACounters));
    Exit;
  end;
  AListLevelIndex := AParagraph.GetListLevelIndex;
  if ANumberingListWebControl.AbstractListIndex <> ANumberingList.AbstractNumberingListIndex
  then
  begin
...
function TdxHtmlContentExporter.IsRootFirstParagraphInListHasNonZeroLevel
  (AParagraph: TdxParagraph): Boolean;
begin
  Result := (Safe<TdxNumberingListWebControl>.Cast(CurrentParent) = nil) and
    (AParagraph.GetListLevelIndex <> 0);
end;

procedure TdxHtmlContentExporter.
  CreateSeveralParentNumberingListToMakeNewNumberingListNested
  (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList;
  const ACounters: TIntegerDynArray);
var
  ACount, I: Integer;
begin
  ...
  for I := 0 to ACount - 1 do
    CreateNumberingListControl(AParagraph, ANumberingList,
      ACounters[Min(I, Length(ACounters) - 1)]);
end;

procedure TdxHtmlContentExporter.CloseListsWithSmallerLevelIndex(AListLevelIndex
  : Integer);
var
  ANumberingListWebControl: TdxNumberingListWebControl;
begin
...
procedure TdxHtmlContentExporter.CreateNumberingListControl
  (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList;
  ACounter: Integer);
var
...
begin
  ...
  begin
    ALastItem := Safe<TdxHtmlGenericControl>.Cast
      (AParent.Controls[AParent.Controls.Count - 1]);
    if (ALastItem <> nil) and (ALastItem.TagKey = TdxHtmlTextWriterTag.Li) then
      PushControl(ALastItem);
  end;
  ANumberingListControl := CreateNumberingListControlCore(AParagraph,
    ANumberingList, ACounter);
  AddControlToChild(CurrentParent, ANumberingListControl);
  PushControl(ANumberingListControl);
  IncreaseListControlNestingLevel;
end;

function TdxHtmlContentExporter.CreateNumberingListControlCore
  (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList;
  ACounter: Integer): TdxWebControlBase;
begin
  Result := FExportHelper.CreateNumberingListControl(AParagraph, ANumberingList,
    ACounter);
end;

function TdxHtmlContentExporter.CreateListLevelControl(AParagraph: TdxParagraph;
  ANumberingList: TdxNumberingList; ACounter: Integer): TdxWebControlBase;
begin
  FCurrentListLeftIndent := AParagraph.LeftIndent;
  Result := FExportHelper.CreateListLevelControl(AParagraph, ANumberingList,
    GetListLevelNestingLevel, FParentListLeftIndent, ACounter);
end;
...
begin
  ...
  if ABackColor = TdxAlphaColors.Empty then
    ABackColor := TdxAlphaColors.Transparent;
  ExportCore(CreateHtmlExporterTextProperties(ARun.FontCacheIndex, AForeColor,
    ABackColor, False, False, TdxCharacterFormattingScript.Normal, ARun.AllCaps,
    ARun.DoubleFontSize), '<o:p>&nbsp;</o:p>');
end;
...
function TdxHtmlContentExporter.ProcessRunText(ARun: TdxTextRun;
  const AText: string): string;
begin
  Result := ExportHelper.HtmlStyleHelper.PreprocessHtmlContentText(AText,
    FOptions.TabMarker);
end;

procedure TdxHtmlContentExporter.ExportTextRunCore(ARun: TdxTextRun;
  const AText: string);
begin
  ExportTextCore(ARun.FontCacheIndex, AText, ARun.ForeColor, ARun.BackColor,
    ARun.FontUnderlineType, ARun.FontStrikeoutType, ARun.Script, ARun.AllCaps,
    ARun.DoubleFontSize);
end;

procedure TdxHtmlContentExporter.ExportTextCore(AFontCacheIndex: Integer;
  const AText: string; AForeColor, ABackColor: TdxAlphaColor;
  AUnderlineType: TdxUnderlineType; AStrikeoutType: TdxStrikeoutType;
  AScript: TdxCharacterFormattingScript; AAllCaps: Boolean;
  ADoubleFontSize: Integer);
var
  AUnderline, AStrikeout: Boolean;
begin
  ...
  ExportCore(CreateHtmlExporterTextProperties(AFontCacheIndex, AForeColor,
    ABackColor, AUnderline, AStrikeout, AScript, AAllCaps,
    ADoubleFontSize), AText);
end;

procedure TdxHtmlContentExporter.ExportInlinePictureRun
...
begin
  ...
  ATextProperties := CreateHtmlExporterTextProperties(ARun.FontCacheIndex,
    AForeColor, ABackColor, AUnderline, AStrikeout, ARun.Script, ARun.AllCaps,
    ARun.DoubleFontSize);
  AParentControls := CurrentParent.Controls;
  AParentControlsCount := AParentControls.Count;
  if AParentControlsCount > 0 then
  ...
  if (APrevControl <> nil) and (FLastCreatedTextControl = APrevControl) and
    ATextProperties.Equals(FLastExportedTextProperties) then
  begin
    AWebImageControl := FExportHelper.CreateImageControlInternal
      (ARun.Image.Image, ARun.ActualSize, TdxHtmlCssFloat.NotSet);
    if AWebImageControl.ImageUrl <> '' then
      AddControlToChild(APrevControl, AWebImageControl);
  end
  else if (AreCharacterPropertiesDefault(ARun)) or
    ((CurrentParent is TdxHtmlAnchor) and
    ATextProperties.Equals(FLastExportedHyperLinkTextProperties)) then
  begin
  ...
  begin
    ATextControl := TdxHtmlControl(FExportHelper.CreateTextControl('',
      ATextProperties));
    AddControlToChild(CurrentParent, ATextControl);
    AWebImageControl := FExportHelper.CreateImageControlInternal
      (ARun.Image.Image, ARun.ActualSize, TdxHtmlCssFloat.NotSet);
...
function TdxHtmlContentExporter.AreCharacterPropertiesDefault
  (ARun: TdxInlinePictureRun): Boolean;
var
  ACharacterProperties: TdxCharacterFormattingInfo;
begin
  ACharacterProperties := TdxHtmlExportHelper.CreateDefaultCharacterProperties
    (DocumentModel);
  try
...
function TdxHtmlContentExporter.ShouldExportInlinePicture
  (ARun: TdxInlinePictureRun): Boolean;
begin
...
begin
  ...
  begin
    ...
    ExportFootNoteRunReference(AInfo, ARun,
      FOptions.ActualFootNoteNumberStringFormat,
      FOptions.ActualFootNoteNamePrefix);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    ExportFootNoteRunReference(AInfo, ARun,
      FOptions.ActualEndNoteNumberStringFormat,
      FOptions.ActualEndNoteNamePrefix);
  end
  else
  begin
...
procedure TdxHtmlContentExporter.ExportFootNoteRunReference
  (const AInfo: TdxFootNoteExportInfo; ARun: TdxFootNoteRunBase;
  const AFormat, APrefix: string);
var
...
procedure TdxHtmlContentExporter.ExportFloatingObjectAnchorRun
...
procedure TdxHtmlContentExporter.ExportFloatingObjectFrame
  (ARun: TdxFloatingObjectAnchorRun; AControl: TdxHtmlGenericControl);
var
  AUnitConverter: TdxDocumentModelUnitConverter;
  AStyle: TdxCssStyleCollection;
  AShape: TdxShape;
...
begin
  ...
  AShape := ARun.Shape;
  if (AShape.OutlineWidth > 0) and not TdxAlphaColors.IsTransparentOrEmpty
    (AShape.OutlineColor) then
  begin
    AStyle.Add(TdxHtmlTextWriterStyle.BorderWidth,
      TdxStringHelper.Format(TdxCultureInfo.InvariantCulture.FormatSettings,
      '%gpt', [AUnitConverter.ModelUnitsToPointsF(AShape.OutlineWidth)]));
    AStyle.Add(TdxHtmlTextWriterStyle.BorderStyle, 'solid');
    AStyle.Add(TdxHtmlTextWriterStyle.BorderColor,
      TdxAlphaColors.ToHtml(AShape.OutlineColor));
  end;
  if not TdxAlphaColors.IsTransparentOrEmpty(AShape.FillColor) then
    AStyle.Add(TdxHtmlTextWriterStyle.BackgroundColor,
      TdxAlphaColors.ToHtml(AShape.FillColor));

  AFloatingObjectProperties := ARun.FloatingObjectProperties;
  if (AFloatingObjectProperties.LeftDistance > 0) or
    (AFloatingObjectProperties.RightDistance > 0) or
    (AFloatingObjectProperties.TopDistance > 0) or
    (AFloatingObjectProperties.BottomDistance > 0) then
  begin
    AStyle.Add(TdxHtmlTextWriterStyle.Margin,
      TdxStringHelper.Format(TdxCultureInfo.InvariantCulture.FormatSettings,
      '%gpt %gpt %gpt %gpt',
      [AUnitConverter.ModelUnitsToPointsF(AFloatingObjectProperties.
      TopDistance), AUnitConverter.ModelUnitsToPointsF
      (AFloatingObjectProperties.RightDistance),
      AUnitConverter.ModelUnitsToPointsF(AFloatingObjectProperties.
      BottomDistance), AUnitConverter.ModelUnitsToPointsF
  ...
  begin
    ...
    if (AProperties.LeftMargin > 0) or (AProperties.RightMargin > 0) or
      (AProperties.TopMargin > 0) or (AProperties.BottomMargin > 0) then
      AStyle.Add(TdxHtmlTextWriterStyle.Padding,
        TdxStringHelper.Format(TdxCultureInfo.InvariantCulture.FormatSettings,
        '%gpt %gpt %gpt %gpt',
    ...
    Dec(AWidth, 2 * AShape.OutlineWidth + AProperties.LeftMargin +
      AProperties.RightMargin);
    Dec(AHeight, AShape.OutlineWidth + AProperties.TopMargin +
      AProperties.BottomMargin);
  end;
  AStyle.Add(TdxHtmlTextWriterStyle.Width,
    TdxStringHelper.Format(TdxCultureInfo.InvariantCulture.FormatSettings,
    '%gpt', [AUnitConverter.ModelUnitsToPointsF(AWidth)]));
  AStyle.Add(TdxHtmlTextWriterStyle.Height,
    TdxStringHelper.Format(TdxCultureInfo.InvariantCulture.FormatSettings,
    '%gpt', [AUnitConverter.ModelUnitsToPointsF(AHeight)]));
  ...
  begin
    ...
    else if ACssFloat = TdxHtmlCssFloat.Right then
      AControl.Style.Add('float', 'right');
  end;
end;

procedure TdxHtmlContentExporter.ExportFloatingObjectContent
  (ARun: TdxFloatingObjectAnchorRun);
var
  ATextBoxContent: TdxTextBoxFloatingObjectContent;
...
procedure TdxHtmlContentExporter.ExportFloatingObjectTextBoxContent
  (AContent: TdxTextBoxFloatingObjectContent; ARun: TdxFloatingObjectAnchorRun);
begin
  PerformExportPieceTable(TdxPieceTable(AContent.TextBox.PieceTable),
    ExportPieceTable);
end;

procedure TdxHtmlContentExporter.ExportFloatingObjectPictureContent
  (AContent: TdxPictureFloatingObjectContent; ARun: TdxFloatingObjectAnchorRun);
begin
  AddControlToChild(CurrentParent, CreateImageControl(AContent, ARun));
end;

function TdxHtmlContentExporter.CreateImageControl
  (AContent: TdxPictureFloatingObjectContent; ARun: TdxFloatingObjectAnchorRun)
  : TdxWebControlBase;
begin
  Result := FExportHelper.CreateImageControl(AContent.Image.Image,
    ARun.FloatingObjectProperties.ActualSize, TdxHtmlCssFloat.NotSet);
end;

function TdxHtmlContentExporter.GetCssFloat(ARun: TdxFloatingObjectAnchorRun)
  : TdxHtmlCssFloat;
var
  AProperties: TdxFloatingObjectProperties;
begin
  AProperties := ARun.FloatingObjectProperties;
  if (AProperties.TextWrapType <> TdxFloatingObjectTextWrapType.Square) and
    (AProperties.TextWrapType = TdxFloatingObjectTextWrapType.Tight) then
    Exit(TdxHtmlCssFloat.NotSet);
  if AProperties.HorizontalPositionAlignment =
    TdxFloatingObjectHorizontalPositionAlignment.Left then
    Exit(TdxHtmlCssFloat.Left)
  else if AProperties.HorizontalPositionAlignment =
    TdxFloatingObjectHorizontalPositionAlignment.Right then
...
constructor TdxHtmlTableStyleHelper.Create(AUnitConverter
  : TdxDocumentModelUnitConverter; AStyleHelper: TdxHtmlStyleHelper);
begin
  inherited Create;
  FUnitConverter := AUnitConverter;
...
procedure TdxHtmlTableStyleHelper.GetHtmlTableStyle(ATable: TdxTable;
  AStyle: TdxCssStyleCollection);
var
  ATableIndent: Single;
begin
  ...
  if ATableIndent <> 0 then
    AStyle.Add('margin-left', TdxHtmlStyleHelper.GetHtmlSizeInPoints
      (ATableIndent));

  if ATable.TableProperties.Info.UseBackgroundColor then
    AStyle.Add('background-color',
      TdxHtmlConvert.ToHtml(ATable.BackgroundColor));
end;
...
begin
  ...
  AWidth := Max(1.0, SimpleRoundTo(UnitConverter.ModelUnitsToPointsF
    (ABorder.Width), -1));
...
begin
  Result := ABorder.Style in [TdxBorderLineStyle.None, TdxBorderLineStyle.Nil,
    TdxBorderLineStyle.Disabled];
end;

function TdxHtmlTableStyleHelper.GetHtmlBorderStyle(ABorderStyle
  : TdxBorderLineStyle): string;
begin
...
function TdxHtmlTableStyleHelper.GetHtmlTableCellStyle(ACell: TdxTableCell;
  AActualBottomBorderCell: TdxBorderBase): string;
var
  AStyle: TdxCssStyleCollection;
begin
  AStyle := TdxCssStyleCollection.Create;
...
procedure TdxHtmlTableStyleHelper.GetHtmlTableCellStyle(ACell: TdxTableCell;
  AStyle: TdxCssStyleCollection; AActualBottomBorderCell: TdxBorderBase);
var
...
begin
  ...
  AStyle.Add('padding', HtmlStyleHelper.GetHtmlSizeInPoints
    (UnitConverter.ModelUnitsToPointsF(ACell.GetActualTopMargin.Value)) + ' ' +
    HtmlStyleHelper.GetHtmlSizeInPoints(UnitConverter.ModelUnitsToPointsF
    (ACell.GetActualRightMargin.Value)) + ' ' +
    HtmlStyleHelper.GetHtmlSizeInPoints(UnitConverter.ModelUnitsToPointsF
    (ACell.GetActualBottomMargin.Value)) + ' ' +
    HtmlStyleHelper.GetHtmlSizeInPoints(UnitConverter.ModelUnitsToPointsF
    (ACell.GetActualLeftMargin.Value)));

  if ACell.Properties.Info.UseBackgroundColor then
    AStyle.Add('background-color',
      TdxHtmlConvert.ToHtml(ACell.BackgroundColor));
...
function TdxHtmlTableStyleHelper.ConvertWidthUnitToPixels
  (AWidthUnit: TdxWidthUnit): string;
begin
  if AWidthUnit.&Type = TdxWidthUnitType.ModelUnits then
    Result := IntToStr(UnitConverter.ModelUnitsToPixels(AWidthUnit.Value,
      UnitConverter.ScreenDpi))
  else if AWidthUnit.&Type = TdxWidthUnitType.FiftiethsOfPercent then
    Result := HtmlStyleHelper.GetHtmlSizeInPercents(AWidthUnit.Value div 50)
  else
    Result := '0';
end;

function TdxHtmlTableStyleHelper.ConvertWidthUnitToPoints
  (AWidthUnit: TdxWidthUnit): string;
begin
  if AWidthUnit.&Type = TdxWidthUnitType.ModelUnits then
    Result := HtmlStyleHelper.GetHtmlSizeInPoints
      (UnitConverter.ModelUnitsToPointsF(AWidthUnit.Value))
  else if AWidthUnit.&Type = TdxWidthUnitType.FiftiethsOfPercent then
    Result := HtmlStyleHelper.GetHtmlSizeInPercents(AWidthUnit.Value div 50)
  else
    Result := '';
...
function TdxHtmlTableStyleHelper.ConvertWidthUnitToPointsF
  (AWidthUnit: TdxWidthUnit): Single;
begin
  if AWidthUnit.&Type = TdxWidthUnitType.ModelUnits then
...
function TdxHtmlTableStyleHelper.GetHtmlVerticalAlignment
  (AVAlignment: TdxVerticalAlignment): string;
begin
  case AVAlignment of
...
function TdxHtmlTableStyleHelper.GetHtmlTableAlignment(ATableAlignment
  : TdxTableRowAlignment): string;
begin
...
constructor TdxHtmlExportHelper.Create(ADocumentModel: TdxDocumentModel;
  const AScriptContainer: IdxScriptContainer;
  const AImageRepository: IdxOfficeImageRepository;
  AOptions: TdxHtmlDocumentExporterOptions);
begin
  ...
  FTableStyleHelper := TdxHtmlTableStyleHelper.Create
    (ADocumentModel.UnitConverter, FHtmlStyleHelper);
  FParagraphStyleHelper := TdxHtmlParagraphStyleHelper.Create
    (ADocumentModel.UnitConverter, FHtmlStyleHelper);
  FParagraphTabsHelper := TdxHtmlParagraphTabsHelper.Create
    (ADocumentModel.UnitConverter, FHtmlStyleHelper);
  FImageHelper := TdxOfficeHtmlImageHelper.Create(ADocumentModel,
    ADocumentModel.UnitConverter, AOptions);
  if FOptions.OverrideImageResolution <> TdxHtmlDocumentExporterOptions.DefaultOverrideImageResolution
  then
  begin
...
begin
  Result := FOptions.CssPropertiesExportType =
    TdxCssPropertiesExportType.Inline;
end;
...
function TdxHtmlExportHelper.CreateTextControl(const AText: string;
  ATextProperties: TdxHtmlExporterTextProperties): TdxWebControlBase;
var
  AControl: TdxHtmlGenericControl;
begin
...
procedure TdxHtmlExportHelper.SetTextProperties(AControl: TdxHtmlControl;
  ATextProperties: TdxHtmlExporterTextProperties;
  AForceExportCharacterProperties: Boolean);
var
...
begin
  ...
  begin
    TdxHtmlStyleRender.GetHtmlStyle(AFontInfo.Name,
      ATextProperties.DoubleFontSize / 2, TdxGraphicUnit.guPoint,
      AFontInfo.Bold, AFontInfo.Italic, ATextProperties.Strikeout,
      ATextProperties.Underline, ATextProperties.ForeColor,
      ATextProperties.BackColor, AUseFontSizeInPixels, AControl.Style);
    if not FOptions.DefaultCharacterPropertiesExportToCss and not AForceExportCharacterProperties
    then
    ...
    if (AIsHtmlAnchor and not ATextProperties.Underline) and
      not ATextProperties.Strikeout then
...
procedure TdxHtmlExportHelper.RemoveDefaultProperties(ATextProperties
  : TdxHtmlExporterTextProperties; AFontInfo: TdxFontInfo;
  ACssStyleCollection: TdxCssStyleCollection);
var
  ADefaultCharacterProperties: TdxCharacterFormattingInfo;
begin
  ADefaultCharacterProperties := CreateDefaultCharacterProperties
    (FDocumentModel);
  try
    RemoveDefaultProperties(FDocumentModel, ADefaultCharacterProperties,
      AFontInfo.Name, AFontInfo.Bold, AFontInfo.Italic,
      ATextProperties.DoubleFontSize, ATextProperties.BackColor,
      ATextProperties.ForeColor, ACssStyleCollection);
  finally
    ADefaultCharacterProperties.Free;
  end;
end;

class function TdxHtmlExportHelper.CreateDefaultCharacterProperties
  (ADocumentModel: TdxDocumentModel): TdxCharacterFormattingInfo;
var
...
begin
  ...
  begin
    AMergedProperties := TdxMergedCharacterProperties.Create
      (ADocumentModel.CharacterStyles[0].CharacterProperties);
    try
...
class procedure TdxHtmlExportHelper.TryRemoveKey(ACssStyleCollection
  : TdxCssStyleCollection; const AKey: string);
begin
  if ACssStyleCollection[AKey] <> '' then
    ACssStyleCollection.Remove(AKey);
end;

class procedure TdxHtmlExportHelper.RemoveDefaultProperties(ADocumentModel
  : TdxDocumentModel; ACharacterProperties: TdxCharacterFormattingInfo;
  const AFontName: string; AFontBold, AFontItalic: Boolean;
  ADoubleFontSize: Integer; ABackColor, AForeColor: TdxAlphaColor;
  AStyle: TdxCssStyleCollection);
begin
...
procedure TdxHtmlExportHelper.AppendText(AParent: TdxWebControlBase;
  const AText: string);
var
  ALiteralControl: TdxHtmlLiteralControl;
begin
  ALiteralControl := TdxHtmlLiteralControl.Create
    (ConvertWhiteSpacesToOfficeSpace(AText));
  FObjectsToDelete.Add(ALiteralControl);

  AParent.Controls.Add(ALiteralControl);
end;

function TdxHtmlExportHelper.ConvertWhiteSpacesToOfficeSpace(const ARawText
  : string): string;
var
...
begin
  ...
  begin
    if ASpaceIndex > ACurrentIndex then
      FText.Append(TdxStringHelper.Substring(ARawText, ACurrentIndex,
        (ASpaceIndex - ACurrentIndex)));
...
procedure TdxHtmlExportHelper.GenerateSpaces(ACount: Integer;
  AForceUseNonBreakingSpace: Boolean);
begin
...
function TdxHtmlExportHelper.CreateTableControl(ATable: TdxTable)
  : TdxWebControlBase;
var
...
begin
  ...
  ATableControl.Attributes.Add('cellspacing',
    ConvertWidthUnitToPixels(ATable.CellSpacing));
  ATableControl.Attributes.Add('cellpadding', '0');

  if ATable.PreferredWidth.Value <> 0 then
    ATableControl.Attributes.Add('width',
      ConvertWidthUnitToPixels(ATable.PreferredWidth));
  ...
  if ATable.TableAlignment <> TdxTableRowAlignment.Left then
    ATableControl.Attributes.Add('align',
      FTableStyleHelper.GetHtmlTableAlignment(ATable.TableAlignment));
...
function TdxHtmlExportHelper.CreateTableRowControl(ARow: TdxTableRow)
  : TdxWebControlBase;
var
...
begin
  ...
  begin
    ARowHeight := HtmlStyleHelper.GetHtmlSizeInPoints
      (UnitConverter.ModelUnitsToPointsF(ARow.Height.Value));
    ATableRowControl.Style.Add('height', ARowHeight);
  end;
  Result := ATableRowControl;
end;

function TdxHtmlExportHelper.CreateTableCellControl(ACell: TdxTableCell;
  AMergedCellProperties: TdxVerticalMergeCellProperties): TdxWebControlBase;
var
...
begin
  ...
  if ACell.VerticalAlignment <> TdxVerticalAlignment.Center then
    ATableCellControl.Attributes.Add('valign',
      FTableStyleHelper.GetHtmlVerticalAlignment(ACell.VerticalAlignment));

  if ACell.PreferredWidth.Value <> 0 then
    ATableCellControl.Attributes.Add('width',
      ConvertWidthUnitToPixels(ACell.PreferredWidth));

  if ACell.NoWrap then
    ATableCellControl.Attributes.Add('nowrap', '');

  if IsExportInlineStyle then
    FTableStyleHelper.GetHtmlTableCellStyle(ACell, ATableCellControl.Style,
      AMergedCellProperties.ActualBottomBorder)
  else
  begin
    AStyle := FTableStyleHelper.GetHtmlTableCellStyle(ACell,
      AMergedCellProperties.ActualBottomBorder);
...
function TdxHtmlExportHelper.ConvertWidthUnitToPixels
  (AWidthUnit: TdxWidthUnit): string;
begin
  Result := FTableStyleHelper.ConvertWidthUnitToPixels(AWidthUnit);
end;

function TdxHtmlExportHelper.CreateParagraphControl(AParagraph: TdxParagraph;
  AIgnoreOutlineLevel: Boolean; AListControlNestingLevel, AParentLevelOffset
  : Integer): TdxWebControlBase;
var
...
begin
  ...
  AParagraphControl := TdxHtmlGenericControl.Create
    (CalculateParagraphTag(AParagraph, AIgnoreOutlineLevel));
  ...
  if IsExportInlineStyle then
    FParagraphStyleHelper.GetHtmlParagraphStyle(AParagraph,
      AListControlNestingLevel, AParentLevelOffset, AParagraphControl.Style)
  else
  begin
    AStyle := FParagraphStyleHelper.GetHtmlParagraphStyle(AParagraph,
      AListControlNestingLevel, AParentLevelOffset);
...
function TdxHtmlExportHelper.CalculateParagraphTag(AParagraph: TdxParagraph;
  AIgnoreOutlineLevel: Boolean): TdxHtmlTextWriterTag;
begin
...
function TdxHtmlExportHelper.CreateNumberingListControl
  (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList;
  ACounter: Integer): TdxWebControlBase;
var
...
begin
  Assert(AParagraph <> nil, 'paragraph');
  if TdxNumberingListHelper.GetListType(ANumberingList.AbstractNumberingList)
    = TdxNumberingType.Bullet then
    AHtmlTextWriterTag := TdxHtmlTextWriterTag.Ul
  else
    AHtmlTextWriterTag := TdxHtmlTextWriterTag.Ol;

  AControl := TdxNumberingListWebControl.Create(AHtmlTextWriterTag,
    ANumberingList.AbstractNumberingListIndex, AParagraph.GetListLevelIndex);
  FObjectsToDelete.Add(AControl);
...
function TdxHtmlExportHelper.ShouldAddNumberingListStartAttribute
  (AParagraph: TdxParagraph; ANumberingList: TdxNumberingList): Boolean;
var
...
begin
  ...
  if AIsFirstParagraph then
    Exit(AParagraph.IsInList and (ANumberingList.AbstractNumberingList.Levels
      [ACurParagraphLevel].ListLevelProperties.Start <> 1));
  ...
  ADifferNumberingLists := APreviousParagraph.GetNumberingListIndex <>
    AParagraph.GetNumberingListIndex;
  if APreviousParagraph.IsInList and (APrevParagraphLevel > ACurParagraphLevel)
    and ADifferNumberingLists then
    Exit(True);
  Result := (APrevParagraphLevel = ACurParagraphLevel) and ADifferNumberingLists
end;

function TdxHtmlExportHelper.CreateListLevelControl(AParagraph: TdxParagraph;
  ANumberingList: TdxNumberingList; AListControlNestingLevel,
  AParentLevelOffset, ACounter: Integer): TdxWebControlBase;
var
...
begin
  ...
  if IsExportInlineStyle then
    FParagraphStyleHelper.GetHtmlParagraphInListStyle(AParagraph, AListLevel,
      AListControlNestingLevel, AParentLevelOffset, ALevelControl.Style,
      FOptions.DefaultCharacterPropertiesExportToCss)
  else
  begin
    AStyle := FParagraphStyleHelper.GetHtmlParagraphInListStyle(AParagraph,
      AListLevel, AListControlNestingLevel, AParentLevelOffset);
    ALevelControl.CssClass := FScriptContainer.RegisterCssClass(AStyle);
  end;

  if (AListLevel.ListLevelProperties.Start <> 1) and
    (AListLevel.ListLevelProperties.Start = ACounter) then
    ALevelControl.Attributes.Add('value',
      IntToStr(AListLevel.ListLevelProperties.Start));
  Result := ALevelControl;
end;

function TdxHtmlExportHelper.GetMostNestedCounter(const ACounters
  : TIntegerDynArray): Integer;
begin
  Result := ACounters[Length(ACounters) - 1];
end;

function TdxHtmlExportHelper.CreateImageControl(AImage: TdxOfficeImage;
  const AActualSize: TSize; ACssFloat: TdxHtmlCssFloat): TdxWebControlBase;
var
...
function TdxHtmlExportHelper.CreateImageControlInternal(AImage: TdxOfficeImage;
  const AActualSize: TSize; ACssFloat: TdxHtmlCssFloat): TdxWebImageControl;
var
  AImageControl: TdxWebImageControl;
begin
  ...
  FImageHelper.ApplyImageProperties(AImageControl, AImage, AActualSize,
    FImageRepository, DisposeConvertedImagesImmediately, True,
    FOptions.ExportImageSize = TdxExportImageSize.Always,
    FOptions.KeepExternalImageSize);
...
function TdxHtmlExportHelper.CreateImageControl(ARun: TdxInlinePictureRun)
  : TdxWebControlBase;
begin
  Result := CreateImageControl(ARun.Image.Image, ARun.ActualSize,
