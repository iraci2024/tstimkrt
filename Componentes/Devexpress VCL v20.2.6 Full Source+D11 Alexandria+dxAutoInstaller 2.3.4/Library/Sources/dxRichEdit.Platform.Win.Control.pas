...
type
  ...
  TdxVCLControl = class(TcxCustomControl, IcxLookAndFeelContainer,
    IdxSkinSupport, IdxLocalizerListener, IdxSystemInfoListener,
    IdxGestureClient, IdxGestureOwner, IdxZoomClient)
  private
...
uses
  ...
  cxLibraryStrs, UxTheme, DWMApi, cxDWMApi, cxFormats, dxThemeManager,
  dxTypeHelpers,
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AMask <> 0) and (Perform(CM_WANTSPECIALKEY, CharCode, 0) = 0) and
...
begin
  ...
  begin
    ...
    begin
      if Perform(CM_CHILDKEY, CharCode, LPARAM(Self)) <> 0 then
        Exit;
      if GetParentForm(Self).Perform(CM_DIALOGKEY, CharCode, KeyData) <> 0 then
        Exit;
    end;
    Result := 0;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (IsDesigning and GetDesignHitTest(AMousePos.X, AMousePos.Y, [ssLeft])
        or not IsDesigning and (DragMode = dmAutomatic)) then
      begin
...
procedure TdxVCLControl.DoGesture(const EventInfo: TGestureEventInfo;
  var Handled: Boolean);
begin
  inherited;
...
function TdxVCLControl.GetDefaultInteractiveGestureOptions
...
function TdxVCLControl.IsTouchPropertyStored(AProperty: TTouchProperty)
  : Boolean;
begin
  case AProperty of
...
function TdxVCLControl.DoShowPopupMenu(AMenu: TComponent;
  X, Y: Integer): Boolean;
begin
...
function TdxVCLControl.InternalMouseWheel(Shift: TShiftState;
...
begin
  ...
  begin
    ...
    repeat
      ...
      if Assigned(AControlPopupMenu) and (AControlPopupMenu.WindowHandle <> 0)
        and AControlPopupMenu.IsShortCut(Message) then
        Exit;
      if (AControl is TdxVCLControl) and
        IsPopupMenuShortCut(TdxVCLControl(AControl).PopupMenu, Message) then
        Exit;
      AControl := AControl.Parent;
    until AControl = nil;
    AParentForm := GetParentForm(Self);
    if (AParentForm <> nil) and AParentForm.IsShortCut(Message) then
      Exit;
  end;
  with Message do
    if SendAppMessage(CM_APPKEYDOWN, CharCode, KeyData) <> 0 then
      Exit;
  Result := False;
end;
...
procedure TdxVCLControl.Notification(AComponent: TComponent;
...
begin
  ...
  begin
    ...
    begin
      ...
      cxBitBlt(ActiveCanvas.Handle, AIntf.GetImage.cxCanvas.Handle, ClientRect,
        P, SRCCOPY);
    end;
  end
  else
...
begin
  cxProcessControlScrollingOnMiddleButton(Self, False, True, ScrollContent,
    FIsScrollingContent);
end;
...
function TdxVCLControl.GetDesignHitTest(X, Y: Integer;
...
function TdxVCLControl.GetScrollBarClass(AKind: TScrollBarKind)
...
class procedure TdxVCLControl.InvalidateControl(AControl: TWinControl;
  ANeedInvalidateSelf, ANeedInvalidateChildren: Boolean);
...
procedure TdxVCLControl.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  ALink: TcxObjectLink;
...
function TdxVCLControl.NeedPanningFeedback(AScrollKind: TScrollBarKind)
  : Boolean;
begin
  Result := True;
end;
...
procedure TdxVCLControl.LookAndFeelChanged(Sender: TcxLookAndFeel;
  AChangedValues: TcxLookAndFeelValues);
begin
end;
...
begin
  FInternalScaleFactor.Assign(ScaleFactor.Numerator * 96,
    ScaleFactor.Denominator * Trunc(TdxGraphicsDpi.Pixel));
...
procedure TdxVCLControl.InvalidateRect(const R: TRect;
...
procedure TdxVCLControl.InvalidateRgn(ARegion: TcxRegion;
  EraseBackground: Boolean);
begin
