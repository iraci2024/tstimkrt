...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Classes, Graphics, ImgList, Menus, dxCore, cxControls,
  cxGraphics, dxBuiltInPopupMenu,
...
type
  ...
  protected
    ...
    function AddMenuItem(ACaption: Pointer; ACommandID: Word;
      const AImageResName: string = ''; AEnabled: Boolean = True;
      AShortCut: TShortCut = 0; AParent: TComponent = nil): TComponent;
      overload;
  ...
  TdxPDFViewerNavigationPaneAttachmentPopupMenu = class
    (TdxPDFViewerAttachmentPopupMenu)
  protected
  ...
  TdxPDFViewerBookmarksPageOptionsPopupMenu = class
    (TdxPDFViewerBookmarkPopupMenu)
  strict private
  const
...
uses
  SysUtils, dxGDIPlusClasses, dxCustomPreview, dxPDFDocument,
  dxPDFViewerDocumentPropertiesDialog, dxPDFViewerActionsStrs,
...
function TdxPDFViewerCustomPopupMenu.AddImage(const AResourceName
  : string): Integer;
var
...
function TdxPDFViewerCustomPopupMenu.AddMenuItem(ACaption: Pointer;
  ACommandID: Word; const AImageResName: string = ''; AEnabled: Boolean = True;
  AShortCut: TShortCut = 0; AParent: TComponent = nil): TComponent;
begin
  Result := AddMenuItem(ACaption, ACommandID, 0, AImageResName, AEnabled,
    AShortCut, AParent);
end;

function TdxPDFViewerCustomPopupMenu.AddMenuItem(ACaption: Pointer;
  ACommandID, AParam: Word; const AImageResName: string = '';
  AEnabled: Boolean = True; AShortCut: TShortCut = 0; AParent: TComponent = nil)
  : TComponent;
begin
  Result := Adapter.Add(cxGetResourceString(ACaption), MenuItemClick,
    MakeLong(ACommandID, AParam), AddImage(AImageResName), AEnabled,
    AShortCut, AParent);
end;
...
begin
  AddMenuItem(GetCopyMenuCaption, ccCopy, 'DXPDFVIEWER_POPUPMENU_GLYPH_COPY',
    TdxPDFViewerAccess(Viewer).CanExtractContent);
end;
...
begin
  AddMenuItem(GetOpenFileCaption, OpenFileID,
    'DXPDFVIEWER_POPUPMENU_GLYPH_OPENFILE');
  AddMenuItem(GetSaveFileCaption, SaveFileID,
    'DXPDFVIEWER_POPUPMENU_GLYPH_SAVEFILE');
end;
...
function TdxPDFViewerNavigationPaneAttachmentPopupMenu.GetAttachment
  : TdxPDFFileAttachment;
begin
  Result := TdxPDFViewerAttachmentsAccess(TdxPDFViewerAccess(Viewer)
...
begin
  AddSeparator;
  AddMenuItem(@sdxPDFViewerPopupMenuDocumentProperties,
    ShowDocumentPropertiesCommandID);
end;
...
procedure TdxPDFViewerPagePopupMenu.PopulatePrintMenuItems;
  ...
  begin
    Result := TdxPDFViewerAccess(Viewer).CanChangeVisibility and
      TdxPDFViewerAccess(Viewer).CanShowFindPanel;
  end;
...
begin
  ...
  begin
    AddSeparator;
    AddMenuItem(@sdxPDFViewerPopupMenuPrint, PrintCommandID,
      'DXPDFVIEWER_POPUPMENU_GLYPH_PRINT', TdxPDFViewerAccess(Viewer).CanPrint);
  end
  else if not NeedShowFindPanelMenuItem then
    AddSeparator;
  if NeedShowFindPanelMenuItem then
  begin
    AItem := AddMenuItem(@sdxPDFViewerActionFindCaption, FindCommandID,
      'DXPDFVIEWER_POPUPMENU_GLYPH_FIND');
...
begin
  AddSeparator;
  AddMenuItem(@sdxPDFViewerActionRotateClockwiseCaption,
    RotateClockwiseCommandID, 'DXPDFVIEWER_POPUPMENU_GLYPH_ROTATECLOCKWISE');
  AddMenuItem(@sdxPDFViewerActionRotateCounterclockwiseCaption,
    RotateCounterclockwiseCommandID,
...
begin
  ...
  begin
    AddSeparator;
    AddMenuItem(@sdxPDFViewerActionSelectAllCaption, SelectAllTextCommandID,
      'DXPDFVIEWER_POPUPMENU_GLYPH_SELECTALL');
  end;
end;
...
var
  AItem: TComponent;
begin
  AItem := AddMenuItem(@sdxPDFViewerActionSelectToolCaption,
    SelectToolCommandID, 'DXPDFVIEWER_POPUPMENU_GLYPH_SELECTTOOL');
...
begin
  ...
  begin
    ...
    if Viewer.CanGoToPrevView then
      AddMenuItem(@sdxPDFViewerActionGoToPrevViewCaption, PreviousViewCommandID,
        'DXPDFVIEWER_POPUPMENU_GLYPH_PREVIOUSVIEW');
    if Viewer.CanGoToNextView then
      AddMenuItem(@sdxPDFViewerActionGoToNextViewCaption, NextViewCommandID,
        'DXPDFVIEWER_POPUPMENU_GLYPH_NEXTVIEW');
  end;
end;
...
begin
  AddSeparator;
  AddMenuItem(@sdxPDFViewerActionZoomInCaption, ZoomInCommandID,
    'DXPDFVIEWER_POPUPMENU_GLYPH_ZOOMIN', TdxPDFViewerAccess(Viewer).CanZoomIn);
  AddMenuItem(@sdxPDFViewerActionZoomOutCaption, ZoomOutCommandID,
    'DXPDFVIEWER_POPUPMENU_GLYPH_ZOOMOUT', TdxPDFViewerAccess(Viewer)
    .CanZoomOut);
end;
...
begin
  AddSeparator;
  AItem := AddMenuItem(@sdxPDFViewerActionZoomActualSizeCaption,
    ActualSizeCommandID, 'DXPDFVIEWER_POPUPMENU_GLYPH_ACTUALSIZE');
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer).ZoomFactor = 100);
  AItem := AddMenuItem(@sdxPDFViewerActionZoomToPageLevelCaption,
    ZoomToPageLevelCommandID);
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer).ZoomMode = pzmPages);
  AItem := AddMenuItem(@sdxPDFViewerActionZoomFitWidthCaption,
    FitWidthCommandID);
...
begin
  ...
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer)
    .OptionsFindPanel.CaseSensitive);
  AItem := AddMenuItem(GetWholeWordsMenuCaption, WholeWordsCommandID);
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer)
    .OptionsFindPanel.WholeWords);
end;

procedure TdxPDFViewerFindPanelOptionsPopupMenu.ProcessClick(ACommand,
  AParam: Word);
var
...
begin
  ...
  case ACommand of
    CaseSensitiveCommandID:
      AViewer.OptionsFindPanel.CaseSensitive :=
        not AViewer.OptionsFindPanel.CaseSensitive;
    WholeWordsCommandID:
      AViewer.OptionsFindPanel.WholeWords :=
        not AViewer.OptionsFindPanel.WholeWords;
  end;
end;

function TdxPDFViewerFindPanelOptionsPopupMenu.
  GetCaseSensitiveMenuCaption: Pointer;
begin
  Result := @sdxPDFViewerFindPanelPopupMenuCaseSensitive;
end;

function TdxPDFViewerFindPanelOptionsPopupMenu.GetWholeWordsMenuCaption
  : Pointer;
begin
...
begin
  ...
  AddMenuItem(GetGoToBookmarkCaption, GoToBookmarkCommandID, '',
    ABookmarks.IsBookmarkSelected);
...
begin
  ABookmarks := TdxPDFViewerBookmarksAccess(Bookmarks);
  AddMenuItem(GetExpandCurrentBookmarkCaption, ExpandCurrentBookmarkCommandID,
    '', ABookmarks.CanExpandCurrentBookmark);

  AddSeparator;
  AddMenuItem(GetCollapseTopLevelBookmarksCaption,
    CollapseTopLevelBookmarksCommandID, '', not ABookmarks.Empty);

  AddSeparator;
  AItem := AddMenuItem(GetHideAfterUseCaption, HideAfterUseCommandID);
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer)
    .OptionsNavigationPane.Bookmarks.HideAfterUse);

  AddSeparator;
  AParent := Adapter.AddSubMenu(cxGetResourceString(GetTextSizeCaption), nil);
  AItem := AddMenuItem(GetSmallTextSizeCaption, SetSmallTextSizeCommandID, '',
    True, 0, AParent);
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer)
    .OptionsNavigationPane.Bookmarks.TextSize = btsSmall);
  AItem := AddMenuItem(GetMediumTextSizeCaption, SetMediumTextSizeCommandID, '',
    True, 0, AParent);
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer)
    .OptionsNavigationPane.Bookmarks.TextSize = btsMedium);
  AItem := AddMenuItem(GetLargeTextSizeCaption, SetLargeTextSizeCommandID, '',
    True, 0, AParent);
  Adapter.SetChecked(AItem, TdxPDFViewerAccess(Viewer)
    .OptionsNavigationPane.Bookmarks.TextSize = btsLarge);

  AddSeparator;
  inherited PopulateMenuItems;
end;

procedure TdxPDFViewerBookmarksPageOptionsPopupMenu.ProcessClick(ACommand,
  AParam: Word);
begin
  case ACommand of
    ...
    HideAfterUseCommandID:
      TdxPDFViewerAccess(Viewer).OptionsNavigationPane.Bookmarks.HideAfterUse :=
        not TdxPDFViewerAccess(Viewer).OptionsNavigationPane.Bookmarks.
        HideAfterUse;
    SetSmallTextSizeCommandID:
      TdxPDFViewerAccess(Viewer).OptionsNavigationPane.Bookmarks.TextSize
        := btsSmall;
...
function TdxPDFViewerBookmarksPageOptionsPopupMenu.
  GetCollapseTopLevelBookmarksCaption: Pointer;
begin
...
function TdxPDFViewerBookmarksPageOptionsPopupMenu.GetBookmarks
...
begin
  ...
  AddMenuItem(GetEnlargePageThumbnailsCaption, EnlargePageThumbnailsID, '',
    AThumbnails.CanEnlargePageThumbnails);
  AddMenuItem(GetReducePageThumbnailsCaption, ReducePageThumbnailsID, '',
    AThumbnails.CanReducePageThumbnails);
end;
...
function TdxPDFViewerThumbnailsPopupMenu.
  GetEnlargePageThumbnailsCaption: Pointer;
begin
...
function TdxPDFViewerThumbnailsPopupMenu.GetReducePageThumbnailsCaption
  : Pointer;
begin
