...
uses
  Classes, SysUtils, Menus, TypInfo, Windows, VCLEditors, Graphics, Forms,
  Controls,
  DesignIntf, DesignEditors, DesignConst, DesignMenus, dxCore, Registry,
...
type
  ...
  TdxDefaultBooleanPropertyEditor = class(TEnumProperty, ICustomPropertyDrawing,
  ...
  protected
    ...
    procedure PaintCheckbox(ACanvas: TCanvas; const ARect: TRect;
      ASelected: Boolean);
  public
...
constructor TdxComponentEditor.Create(AComponent: TComponent;
  ADesigner: IDesigner);
begin
...
begin
  ...
  begin
    ...
    case Index of
      4:
        dxShellExecute('http://' + dxCompanyURL, SW_SHOWMAXIMIZED);
    end;
  end;
end;

procedure TdxComponentEditor.PrepareItem(Index: Integer;
...
begin
  ...
  begin
    for I := 0 to FLinkableObjects.Count - 1 do
      PrepareLinkableSubItem(AItem.AddItem(FLinkableObjects[I], 0, False, True,
        LinkToClick));
  end;
end;
...
procedure TdxComponentEditor.PrepareLinkableSubItem
  (const AItem: TDesignMenuItem);
begin
  // do nothing
end;
...
begin
  ...
  Designer.GetComponentNames(GetTypeData(GetLinkToTypeClass.ClassInfo),
    CheckObjectsForLinkTo);
end;

procedure RegisterSplashItem;
begin
  SplashScreenServices.AddPluginBitmap(Format('DevExpress VCL %s',
    [dxGetBuildNumberAsString]), TdxRegisterInfo.GetSplashBitmap,
    TdxRegisterInfo.GetIsUnRegistered, TdxRegisterInfo.GetLicenseStatus);
end;

procedure DrawCheckbox(ACanvas: TCanvas; const ARect: TRect;
  AState: TCheckBoxState; AEnabled: Boolean = True);
const
  ...
  UnThemedStyles: array [TCheckBoxState] of array [Boolean] of Cardinal =
    ...
    (DFCS_BUTTON3STATE or DFCS_INACTIVE, DFCS_CHECKED or
    DFCS_INACTIVE { DFCS_BUTTON3STATE } ));
begin
{$IFDEF DELPHI16}
...
{$ENDIF}
    ...
    DrawFrameControl(ACanvas.Handle, ARect, DFC_BUTTON,
      UnThemedStyles[AState][AEnabled]);
end;

function TdxDefaultBooleanPropertyEditor.GetCheckBoxBounds
  (const ARect: TRect): TRect;
begin
  Result := Rect(ARect.Right + 2, ARect.Top, ARect.Right + ARect.Bottom -
    ARect.Top + 2, ARect.Bottom);
end;

function TdxDefaultBooleanPropertyEditor.GetDisplayText
  (Value: TdxDefaultBoolean): string;
begin
  case Value of
    bFalse:
...
begin
  ...
  else if SameText(Value, GetDisplayText(bFalse)) then
    SetOrdValue(Ord(bFalse))
  else
    SetOrdValue(Ord(bDefault));
end;

procedure TdxDefaultBooleanPropertyEditor.PropDrawName(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
begin
  DefaultPropertyDrawName(Self, ACanvas, ARect);
end;

procedure TdxDefaultBooleanPropertyEditor.PropDrawValue(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
begin
  PaintCheckbox(ACanvas, ARect, ASelected);
end;

function TdxDefaultBooleanPropertyEditor.PropDrawNameRect
  (const ARect: TRect): TRect;
begin
  Result := ARect;
end;

function TdxDefaultBooleanPropertyEditor.PropDrawValueRect
  (const ARect: TRect): TRect;
begin
...
procedure TdxDefaultBooleanPropertyEditor.MouseDown(Button: TMouseButton;
...
procedure TdxDefaultBooleanPropertyEditor.MouseMove(Shift: TShiftState;
...
procedure TdxDefaultBooleanPropertyEditor.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer; InNameRect: Boolean; const ItemRect: TRect;
  var Handled: Boolean);
const
  NextValue: array [TdxDefaultBoolean] of TdxDefaultBoolean = (bDefault,
    bFalse, bTrue);
begin
  Handled := False;
...
begin
  ...
  if ASelected then
    ExcludeClipRect(ACanvas.Handle, ARect.Left, ARect.Top, ARect.Right + 2,
      ARect.Bottom);
end;
  ...
  try
    ...
    begin
      ...
      if Supports(BorlandIDEServices, IOTAServices, AServices) and
        ...
        ARegistry.OpenKeyReadOnly(AServices.GetBaseRegistryKey +
        '\Help\HtmlHelp1Files') then
      begin
        ...
        try
          ANTAServices.AddActionMenu('HelpThirdPartyMenuItem', nil,
            FDXRootHelpMenuItem, False, True);
        except
          ...
          try
            ...
            begin
              ...
              begin
                ...
                AHandler := TdxIDEMenuHelpItemClickHandler.Create(AMenuItem,
                  AProductHelpPath);
                FHandlers.Add(AHandler);
                ANTAServices.AddActionMenu('DeveloperExpressVCLHelp', nil,
                  AMenuItem, True, True);
              end;
            end;
          finally
...
begin
  ...
  begin
    ...
    FAboutInfoIndex := AServices.AddPluginInfo(ATitle, ATitle + dxCRLF + dxCRLF
      + 'Copyright © 1998-' + FormatDateTime('YYYY', Now) + ' ' + dxCompanyName
      + dxCRLF + 'All rights reserved.', TdxRegisterInfo.GetSplashBitmap,
      TdxRegisterInfo.GetIsUnRegistered, TdxRegisterInfo.GetLicenseStatus);
  end;
end;
