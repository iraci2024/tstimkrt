...
uses
  Types, Windows, Messages, SysUtils, Variants, Classes, SyncObjs,
  dxCoreClasses, Contnrs, ExtCtrls,
...
{$ENDREGION}
  ...
  public
    ...
    procedure QueueWorkItem(ASender: TObject; AObjectProc: TWorkItemObjectProc;
      AContext: Pointer); overload;
    procedure QueueWorkItem(ASender: TObject; AObjectProc: TWorkItemObjectProc;
      AContext: Pointer; AFlags: ULONG); overload;
    procedure QueueWorkItem(AProc: TWorkItemProc; AContext: Pointer); overload;
    procedure QueueWorkItem(AProc: TWorkItemProc; AContext: Pointer;
      AFlags: ULONG); overload;
...
{$ENDREGION}
  ...
  public
    ...
    procedure Sort(AList: PdxPointerList; ACount: Integer;
      const ACompareFunc: TCompareItems); overload;
    procedure Sort(AList: TdxFastList;
      const ACompareFunc: TCompareItems); overload;
    procedure Sort(AList: PdxPointerList; ACount: Integer;
      const ACompareFunc: TListSortCompare); overload;
    procedure Sort(AList: TdxFastList;
      const ACompareFunc: TListSortCompare); overload;
    procedure SortList(AList: PdxPointerList; ACount: Integer;
      const ACompareDelegate: TdxListSortCompareDelegate); overload;
    procedure SortList(AList: TdxFastList;
      const ACompareDelegate: TdxListSortCompareDelegate); overload;
  end;
...
{$REGION 'internal types'}
  protected type
    ...
    TIterateChunkWorkItemArray = array [0 .. dxMaxUserWorkItems - 1]
      of TIterateChunkWorkItem;
{$ENDREGION}
  private
    FCount: Integer;
    FStartIndex: Integer;
    ...
    FReverse: Boolean;
  protected
    function GetChunkContext(AChunkIndex: Integer; AContext: Pointer)
      : Pointer; virtual;
    function Initialize(AStartIndex, AEndIndex: Integer;
      AReverse: Boolean): Boolean;
  ...
  TdxIterateItemFunc = function(AContext: Pointer; AIndex: Integer)
    : Boolean of object;
  ...
  protected
    ...
    function ProcessChunks(AContext: Pointer;
      const AItemFunc: TdxIterateItemFunc): Integer;
  public
    function IterateItems(AStartIndex, AEndIndex: Integer; AContext: Pointer;
      const AItemFunc: TdxIterateItemFunc; AReverse: Boolean): Integer;
  end;

  { TdxMultithreadedClassMethodIterator }

  TdxIterateItemObjectFunc = function(AContext: Pointer; AIndex: Integer)
    : Boolean of object;
  ...
  public
    function IterateItems(AStartIndex, AEndIndex: Integer; AContext: Pointer;
      const AItemObjectFunc: TdxIterateItemObjectFunc;
      AReverse: Boolean): Integer;
  end;

  { TdxMultithreadedListIterator }

  TdxFastListItemFunc = function(AList: TdxFastList; AIndex: Integer)
    : Boolean of object;
  TdxListItemFunc = function(AList: TList; AIndex: Integer): Boolean of object;
  TdxStringsListItemFunc = function(AStrings: TStrings; AIndex: Integer)
    : Boolean of object;

  TdxMultithreadedListIterator = class(TdxMultithreadedClassMethodIterator)
  protected
    function DoIterate(ACount: Integer; AContext: Pointer;
      const AItemObjectFunc: TdxIterateItemObjectFunc;
      AReverse: Boolean): Integer;
  public
    function Iterate(AContext: Pointer; ACount: Integer;
      const AItemFunc: TdxIterateItemObjectFunc; AReverse: Boolean = False)
      : Integer; overload;
    function Iterate(AList: TdxFastList; const AItemFunc: TdxFastListItemFunc;
      AReverse: Boolean = False): Integer; overload;
    function Iterate(AList: TList; const AItemFunc: TdxListItemFunc;
      AReverse: Boolean = False): Integer; overload;
    function Iterate(AStrings: TStrings;
      const AItemFunc: TdxStringsListItemFunc; AReverse: Boolean = False)
      : Integer; overload;
  end;
...
{$ENDIF}
  ...
  public
    ...
    function Run(const AProc: TdxTaskProc; ACompleteEvent: TThreadMethod;
      ACompleteEventCallMode: TdxThreadMethodCallMode): THandle; overload;
    function Run(ATask: TdxTask): THandle; overload;
    function Run(ATask: TdxTask; ACompleteEvent: TThreadMethod;
      ACompleteEventCallMode: TdxThreadMethodCallMode;
      ATaskAutoFree: Boolean = True): THandle; overload;
    function RunInCurrentThread(ATask: TdxTask; ATaskAutoFree: Boolean = True)
      : THandle; overload;
    function RunInCurrentThread(const ATask: IdxTask): THandle; overload;

    function Cancel(ATaskHandle: THandle; AWaitFor: Boolean = False)
      : Boolean; overload;
    function Cancel(ATaskHandle: THandle; AWaitTimeOut: Cardinal)
      : TWaitResult; overload;
    function IsCurrentTaskCanceled: Boolean;

    function WaitFor(ATaskHandle: THandle): Boolean; overload;
    function WaitFor(ATaskHandle: THandle; AWaitTimeOut: Cardinal)
      : TWaitResult; overload;

    property MaxActiveTasks: Integer read FMaxActiveTasks
      write SetMaxActiveTasks;
{$IFDEF DELPHI101BERLIN}
    property UseCpuUsageMonitor: Boolean read GetUseCpuUsageMonitor
      write SetUseCpuUsageMonitor;
{$ENDIF}
  end;
{$ENDREGION}
...
{$HINTS ON}
  ...
  public
    class procedure EnqueueInvokeInUIThread(AOwner: TObject;
      const AProcedure: TProc); static;
...
procedure dxCallThreadMethod(AMethod: TThreadMethod;
  AMode: TdxThreadMethodCallMode);
procedure dxCallThreadProcedure(AProcedure: TThreadProcedure;
  AMode: TdxThreadMethodCallMode);
...
var
  FQueueUserWorkItemProc: function(func: TThreadStartRoutine; Context: Pointer;
...
begin
  ...
  begin
    ...
    while ATimeOut > 0 do
    begin
      case MsgWaitForMultipleObjects(2, AHandles, False,
        Min(WaitTime, ATimeOut), QS_SENDMESSAGE) of
        WAIT_FAILED:
          Exit(wrError);
        WAIT_OBJECT_0:
          Exit(wrSignaled);
...
function InterlockedCompareExchange(var Target: Integer; Exchange: Integer;
  Comparand: Integer): Integer;
asm
  XCHG    EAX,ECX
...
procedure TdxThreadPoolHelper.Initialize(ANumThreads: Integer;
  ACreateEvent: Boolean = False);
begin
...
procedure TdxThreadPoolHelper.QueueWorkItem(ASender: TObject;
  AObjectProc: TWorkItemObjectProc; AContext: Pointer; AFlags: ULONG);
begin
  ...
  begin
    ...
    if not FQueueUserWorkItemProc(ThreadFunction, @FWorkItems[FTaskIndex],
      AFlags) then
      RaiseLastOSError;
    INC(FTaskIndex);
  end;
...
procedure TdxThreadPoolHelper.QueueWorkItem(ASender: TObject;
  AObjectProc: TWorkItemObjectProc; AContext: Pointer);
begin
  QueueWorkItem(ASender, AObjectProc, AContext, WT_EXECUTEDEFAULT
    // WT_EXECUTELONGFUNCTION
    );
end;

procedure TdxThreadPoolHelper.QueueWorkItem(AProc: TWorkItemProc;
  AContext: Pointer);
begin
  QueueWorkItem(AProc, AContext, WT_EXECUTEDEFAULT
    // WT_EXECUTELONGFUNCTION
    );
end;

procedure TdxThreadPoolHelper.QueueWorkItem(AProc: TWorkItemProc;
  AContext: Pointer; AFlags: ULONG);
begin
...
begin
  ...
  FException := Exception.CreateHelp(APatternException.Message,
    APatternException.HelpContext);
end;
...
procedure TdxMultithreadedSort.Sort(AList: PdxPointerList; ACount: Integer;
  const ACompareFunc: TCompareItems);
begin
  if Lock then
    try
...
procedure TdxMultithreadedSort.SortList(AList: TdxFastList;
  const ACompareDelegate: TdxListSortCompareDelegate);
begin
...
procedure Merge(AList, ATemp: PdxPointerList;
  ALowBound, ADivider, AHiBound: Integer;
  const ACompareFunc: TCompareItems); overload;
var
...
procedure TdxMultithreadedSort.ParallelMerge(Sender: TObject;
  AContext: Pointer);
var
...
begin
  with AMergeWorkItem^ do
    if Assigned(FCompareFunc) then
      ...
      MergeWithDelegate(List, Temp, StartIndex, MiddleIndex, EndIndex,
        FCompareDelegate)
end;
...
begin
  with ASortWorkItem^ do
    if Assigned(FCompareFunc) then
      dxInternalQuickSortList(@List[StartIndex], (EndIndex - StartIndex) + 1,
        FCompareFunc)
    else if Assigned(FCompareClassFunc) then
      dxInternalQuickSortList(@List[StartIndex], (EndIndex - StartIndex) + 1,
        FCompareClassFunc)
    else
      dxInternalQuickSortList2(@List[StartIndex], (EndIndex - StartIndex) + 1,
        FCompareDelegate)
end;
...
function TdxCustomMultithreadedIterator.Initialize(AStartIndex,
  AEndIndex: Integer; AReverse: Boolean): Boolean;
begin
  FFinishedIndex := -1;
...
function TdxMultithreadedIterator.ProcessChunks(AContext: Pointer;
  const AItemFunc: TdxIterateItemFunc): Integer;
var
  I, AStartIndex, AEndIndex, AChunkSize, ARemainder: Integer;
begin
...
procedure TdxMultithreadedIterator.ProcessChunk(AContext: TObject;
  AWorkItem: Pointer);
var
    ...
    if Reverse then
      ...
      begin
        if FFinishedIndex >= 0 then
          Break
        else if FItemFunc(Context, I) then
        begin
...
function TdxMultithreadedIterator.IterateItems(AStartIndex, AEndIndex: Integer;
  AContext: Pointer; const AItemFunc: TdxIterateItemFunc;
  AReverse: Boolean): Integer;
begin
...
procedure TdxMultithreadedClassMethodIterator.ProcessChunk(AContext: TObject;
  AWorkItem: Pointer);
var
  AChunkWorkItem: PIterateChunkWorkItem absolute AWorkItem;
  I: Integer;
  ...
  with AChunkWorkItem^ do
    if Reverse then
      ...
      begin
        if FFinishedIndex >= 0 then
          Break
        else if FItemObjectFunc(Context, I) then
        begin
          InterlockedExchange(FFinishedIndex, I);
          Break;
        end;
...
function TdxMultithreadedClassMethodIterator.IterateItems(AStartIndex,
  AEndIndex: Integer; AContext: Pointer;
...
function TdxMultithreadedListIterator.Iterate(AContext: Pointer;
  ACount: Integer; const AItemFunc: TdxIterateItemObjectFunc;
...
begin
  Result := DoIterate(AList.Count, Pointer(AList),
    TdxIterateItemObjectFunc(AItemFunc), AReverse);
end;
...
begin
  Result := DoIterate(AStrings.Count, Pointer(AStrings),
    TdxIterateItemFunc(AItemFunc), AReverse);
end;

function TdxMultithreadedListIterator.DoIterate(ACount: Integer;
  AContext: Pointer; const AItemObjectFunc: TdxIterateItemObjectFunc;
  AReverse: Boolean): Integer;
var
  I: Integer;
begin
...
function TdxTaskDispatcher.Run(const AProc: TdxTaskProc;
  ...
  ACompleteEventCallMode: TdxThreadMethodCallMode): THandle;
begin
  Result := Run(TProcWrapper.Create(AProc), ACompleteEvent,
    ACompleteEventCallMode);
end;
...
function TdxTaskDispatcher.Run(ATask: TdxTask; ACompleteEvent: TThreadMethod;
  ACompleteEventCallMode: TdxThreadMethodCallMode;
  ATaskAutoFree: Boolean = True): THandle;
begin
...
function TdxTaskDispatcher.RunInCurrentThread(ATask: TdxTask;
  ATaskAutoFree: Boolean = True): THandle;
begin
  Result := 0;
  try
...
function TdxTaskDispatcher.Cancel(ATaskHandle: THandle;
  AWaitFor: Boolean = False): Boolean;
begin
  Result := Cancel(ATaskHandle, IfThen(AWaitFor, INFINITE)) <> wrError;
end;

function TdxTaskDispatcher.Cancel(ATaskHandle: THandle; AWaitTimeOut: Cardinal)
  : TWaitResult;
var
  AIndex: Integer;
...
function TdxTaskDispatcher.WaitFor(ATaskHandle: THandle; AWaitTimeOut: Cardinal)
  : TWaitResult;
var
  AIndex: Integer;
  AWaitEvent: IdxTaskEvent;
...
begin
  ...
  try
    ...
    begin
      if not FQueueUserWorkItemProc(@ThreadProc, ATask, WT_EXECUTELONGFUNCTION)
      then
        RaiseLastOSError;
    end
    else
...
class procedure TdxUIThreadSyncService.EnqueueInvokeInUIThread(AOwner: TObject;
  const AProcedure: TProc);
var
  AQueueItem: TEnqueuedProcedure;
begin
  if IsShutdown then
...
begin
  FQueueUserWorkItemProc := GetProcAddress(GetModuleHandle(kernel32),
    'QueueUserWorkItem');
