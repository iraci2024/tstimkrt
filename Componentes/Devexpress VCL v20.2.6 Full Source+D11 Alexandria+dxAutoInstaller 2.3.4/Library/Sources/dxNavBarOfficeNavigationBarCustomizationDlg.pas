...
uses
  ...
  Dialogs, Menus, Math, StdCtrls,
  Generics.Defaults, Generics.Collections, ActnList, ExtCtrls,
  cxGraphics, cxLookAndFeels, cxClasses, cxLookAndFeelPainters, cxButtons,
  cxControls, cxContainer, cxEdit, cxLabel,
  cxListBox, dxCore, cxTextEdit, cxMaskEdit, cxSpinEdit,
  dxNavBarOfficeNavigationBar, cxCheckBox, dxLayoutContainer,
...
constructor TfrmOfficeNavigationBarCustomizationDlg.Create(ANavigationBar
...
function TfrmOfficeNavigationBarCustomizationDlg.GetMaxVisibleItemCount
  : Integer;
begin
...
procedure TfrmOfficeNavigationBarCustomizationDlg.SetOrderList
...
procedure TfrmOfficeNavigationBarCustomizationDlg.btnMoveUpClick
  (Sender: TObject);
begin
  Move(dirUp);
end;

procedure TfrmOfficeNavigationBarCustomizationDlg.ActionList1Update
  (Action: TBasicAction; var Handled: Boolean);
begin
  actMoveUp.Enabled := lbNavigationItems.ItemIndex > 0;
  actMoveDown.Enabled := InRange(lbNavigationItems.ItemIndex, 0,
    lbNavigationItems.Count - 2);
end;

procedure TfrmOfficeNavigationBarCustomizationDlg.btnMoveDownClick
  (Sender: TObject);
begin
  Move(dirDown);
end;

procedure TfrmOfficeNavigationBarCustomizationDlg.btnResetClick
  (Sender: TObject);
var
  ...
  begin
    ...
    begin
      ...
      if FNavigationBar.OptionsView.MaxVisibleItemCount > 0 then
        edtMaxVisibleItems.EditValue :=
          Min(FNavigationBar.OptionsView.MaxVisibleItemCount, AItemCount)
      else
...
begin
  ...
  try
    ...
    liMaxVisibleItems.CaptionOptions.Text :=
      cxGetResourceString(@sdxOfficeNavigationBarMaxVisibleItems);
    chbCompactNavigation.Caption :=
      cxGetResourceString(@sdxOfficeNavigationBarCompactNavigation);
    dxLayoutLabeledItem1.CaptionOptions.Text :=
      cxGetResourceString(@sdxOfficeNavigationBarDisplayInThisOrder);
...
procedure TfrmOfficeNavigationBarCustomizationDlg.Move
