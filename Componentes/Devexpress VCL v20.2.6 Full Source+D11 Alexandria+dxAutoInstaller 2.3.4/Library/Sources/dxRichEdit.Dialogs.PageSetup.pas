...
{$ENDIF}
  ...
  Dialogs, Menus, StdCtrls, Generics.Defaults, Generics.Collections,
  dxCore, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
  dxLayoutContainer, dxLayoutControl,
  dxLayoutControlAdapters, cxButtons, dxLayoutcxEditAdapters, cxContainer,
  cxEdit, cxTextEdit, cxMaskEdit,
  cxSpinEdit, dxMeasurementUnitEdit, cxLabel, cxRadioGroup, cxDropDownEdit,
  cxCheckBox, dxLayoutLookAndFeels, cxClasses,
...
function TdxRichEditPageSetupDialogForm.GetController
  : TdxPageSetupFormController;
begin
  Result := TdxPageSetupFormController(inherited Controller);
end;

function TdxRichEditPageSetupDialogForm.GetUnitConverter
  : TdxDocumentModelUnitConverter;
begin
  Result := Controller.ValueUnitConverter;
end;

procedure TdxRichEditPageSetupDialogForm.AddApplyToComboItem(AItems: TStrings;
  AApplyType: TdxSectionPropertiesApplyType);
begin
  if AApplyType in Controller.AvailableApplyType then
    AddItemValue(AItems, cxGetResourceString(dxSectionPropertiesApplyToNames
      [AApplyType]), Ord(AApplyType));
end;
...
begin
  ...
  lblOrientation.Caption := cxGetResourceString
    (@sdxRichEditPageSetupDialogOrientation);
  rbPortrait.Caption := cxGetResourceString
    (@sdxRichEditPageSetupDialogPortrait);
  rbLandscape.Caption := cxGetResourceString
    (@sdxRichEditPageSetupDialogLandscape);
  lblPaperSize.Caption := cxGetResourceString
    (@sdxRichEditPageSetupDialogPaperSize);
  lblSection.Caption := cxGetResourceString(@sdxRichEditPageSetupDialogSection);
  lblHeadersAndFooters.Caption := cxGetResourceString
    (@sdxRichEditPageSetupDialogHeadersAndFooters);
  cbDifferentOddAndEvenPage.Caption :=
    cxGetResourceString(@sdxRichEditPageSetupDialogDifferentOddAndEvenPage);
  cbDifferentFirstPage.Caption := cxGetResourceString
    (@sdxRichEditPageSetupDialogDifferentFirstPage);
  lcgPageMargins.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogPageMargins);
  lcgPagePaper.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogPagePaper);
  lciMarginLeft.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogMarginLeft);
  lciMarginTop.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogMarginTop);
  lciMarginBottom.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogMarginBottom);
  lciMarginRight.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogMarginRight);
  lciMarginsApplyTo.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogApplyTo);
  lciPaperWidth.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogPaperWidth);
  lciPaperHeight.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogPaperHeight);
  lciPaperApplyTo.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogApplyTo);
  lcgPageLayout.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogPageLayout);
  lciSectionStart.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogSectionStart);
  lciLayoutApplyTo.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditPageSetupDialogApplyTo);
end;
...
procedure TdxRichEditPageSetupDialogForm.cmbApplyToPropertiesEditValueChanged
  (Sender: TObject);
begin
  SyncApplyToCombos(TcxComboBox(Sender).ItemIndex);
end;

procedure TdxRichEditPageSetupDialogForm.cmbPaperSizePropertiesChange
  (Sender: TObject);
var
  AValue: Integer;
...
begin
  ...
  else if Controller.DifferentFirstPage.HasValue then
    Controller.DifferentFirstPage := False;
  if cbDifferentOddAndEvenPage.Checked then
    Controller.DifferentOddAndEvenPages := True
  else if Controller.DifferentOddAndEvenPages.HasValue then
    Controller.DifferentOddAndEvenPages := False;
end;

function TdxRichEditPageSetupDialogForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxPageSetupFormController.Create
    (AControllerParameters as TdxPageSetupFormControllerParameters);
end;

procedure TdxRichEditPageSetupDialogForm.
  edtMarginBottomPropertiesEditValueChanged(Sender: TObject);
var
  AValue: Variant;
begin
...
procedure TdxRichEditPageSetupDialogForm.edtMarginLeftPropertiesEditValueChanged
  (Sender: TObject);
var
  AValue: Variant;
begin
...
procedure TdxRichEditPageSetupDialogForm.
  edtMarginRightPropertiesEditValueChanged(Sender: TObject);
var
  AValue: Variant;
begin
...
procedure TdxRichEditPageSetupDialogForm.edtMarginTopPropertiesEditValueChanged
  (Sender: TObject);
var
  AValue: Variant;
begin
...
procedure TdxRichEditPageSetupDialogForm.
  edtPaperHeightPropertiesEditValueChanged(Sender: TObject);
var
  AValue: Variant;
begin
...
procedure TdxRichEditPageSetupDialogForm.edtPaperWidthPropertiesEditValueChanged
  (Sender: TObject);
var
  AValue: Variant;
begin
...
begin
  ...
  InitializeMeasurementUnitEdit(edtMarginLeft, ToMeasurementType(UnitType),
    ...
    ModelUnitsToUIUnit(TdxPageSetupFormDefaults.
    MaxLeftAndRightMarginByDefault)));
  ...
  InitializeMeasurementUnitEdit(edtMarginTop, ToMeasurementType(UnitType),
    ...
    ModelUnitsToUIUnit(TdxPageSetupFormDefaults.
    MaxTopAndBottomMarginByDefault)));
  ...
  InitializeMeasurementUnitEdit(edtPaperWidth, ToMeasurementType(UnitType),
    TdxMeasurementUnitEditHelper.Create(UnitTypeDescription, 0.1, 2,
    ModelUnitsToUIUnit(TdxPageSetupFormDefaults.
    MinPaperWidthAndHeightByDefault),
    ModelUnitsToUIUnit(TdxPageSetupFormDefaults.
    MaxPaperWidthAndHeightByDefault)));
  ...
  cbDifferentFirstPage.Checked := Controller.DifferentFirstPage.HasValue and
    Controller.DifferentFirstPage.Value;
  cbDifferentOddAndEvenPage.Checked :=
    Controller.DifferentOddAndEvenPages.HasValue and
    Controller.DifferentOddAndEvenPages.Value;
...
procedure TdxRichEditPageSetupDialogForm.PopulateApplyToComboBox
...
begin
  Populate(cmbPaperSize, procedure(AComboBox: TcxCustomComboBox)
    ...
    begin
      ...
      ADisplayName := cxGetResourceString(dxPaperKindNames
        [TdxPaperKind.Custom]);
...
begin
  Populate(cmbSectionStart, procedure(AComboBox: TcxCustomComboBox)
    begin
      AddItemValue(AComboBox.Properties.Items,
        cxGetResourceString(@sdxRichEditPageSetupSectionStartContinuous),
        Ord(TdxSectionStartType.Continuous));
      AddItemValue(AComboBox.Properties.Items,
        cxGetResourceString(@sdxRichEditPageSetupSectionStartColumn),
        Ord(TdxSectionStartType.Column));
      AddItemValue(AComboBox.Properties.Items,
        cxGetResourceString(@sdxRichEditPageSetupSectionStartNextPage),
        Ord(TdxSectionStartType.NextPage));
      AddItemValue(AComboBox.Properties.Items,
        cxGetResourceString(@sdxRichEditPageSetupSectionStartEvenPage),
        Ord(TdxSectionStartType.EvenPage));
      AddItemValue(AComboBox.Properties.Items,
        cxGetResourceString(@sdxRichEditPageSetupSectionStartOddPage),
...
begin
  ...
  edtPaperHeight.Properties.OnChange :=
    edtPaperHeightPropertiesEditValueChanged;
  edtMarginLeft.Properties.OnChange := edtMarginLeftPropertiesEditValueChanged;
  edtMarginRight.Properties.OnChange :=
    edtMarginRightPropertiesEditValueChanged;
  edtMarginTop.Properties.OnChange := edtMarginTopPropertiesEditValueChanged;
  edtMarginBottom.Properties.OnChange :=
    edtMarginBottomPropertiesEditValueChanged;
...
procedure TdxRichEditPageSetupDialogForm.SyncApplyToCombos(ASelectedIndex
  : Integer);
begin
...
procedure TdxRichEditPageSetupDialogForm.SyncApplyToCombosCore
...
begin
  rbPortrait.Checked := Controller.Landscape.HasValue and
    not Controller.Landscape.Value;
  rbLandscape.Checked := Controller.Landscape.HasValue and
    Controller.Landscape.Value;
...
procedure TdxRichEditPageSetupDialogForm.FormCloseQuery(Sender: TObject;
...
begin
  ...
  if not(edtMarginLeft.ValidateEdit(True) and edtMarginRight.ValidateEdit(True)
    and edtMarginTop.ValidateEdit(True) and edtMarginBottom.ValidateEdit(True)
    and edtPaperWidth.ValidateEdit(True) and edtPaperHeight.ValidateEdit(True))
  then
    CanClose := False
  else if not Controller.IsTopBottomMarginsValid then
  begin
  ...
  else if not Controller.IsLeftRightMarginsValid then
  begin
    AText := cxGetResourceString
