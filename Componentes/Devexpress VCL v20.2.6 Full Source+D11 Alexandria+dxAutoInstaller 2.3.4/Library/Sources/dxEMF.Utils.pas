...
uses
  ...
  TypInfo, RTTI, SyncObjs, StrUtils, FmtBcd,
...
type
  ...
  public
    ...
    class function Concat(const AOperands: array of string;
      const ADelimiter: string = ','): string; overload; static;
    class function Concat(AOperands: TList<string>; AFunc: TFunc;
      const ADelimiter: string = ','): string; overload; static;
    class function CommaText(AOperands: TList<string>;
      const ADelimiter: string = ','): string; overload; static;
...
{$IFEND}
  public
    class function ConvertArray(ASourceTypeInfo, ADestinationTypeInfo
      : PTypeInfo; ASourceBuffer: Pointer): TValue; static;
    class function FromNullable<T>(const AValue: TdxNullableValue<T>)
      : TValue; static;
...
(*$HPPEMIT '} }' *)
  ...
  public const
    SQLOrderMap: array [0 .. 15] of Word = (3, 2, 1, 0, 5, 4, 7, 6, 8, 9, 10,
      11, 12, 13, 14, 15);
  strict private
    FCurrentGuid: TGUID;
  public
  ...
  public
    ...
    class function GetTerminalInSize(ASize: Integer;
      AParametersPerObject: Integer): Integer; overload; static;
...
{$IFNDEF DELPHIXE3}
  ...
  public
    ...
    function WriteData(const Buffer: Extended; Count: Longint)
      : Longint; overload;
  end;
{$ENDIF}
  { TdxObjectSet }
  ...
  public
    ...
    class function Select<TTo>(const AArray: TArray<T>; AFunc: TFunc<T, TTo>)
      : TArray<TTo>; static;
...
function TryGetUnderlyingRttiType(ATypeInfo: PTypeInfo;
  out AUnderlyingType: TRttiType): Boolean;
function TryGetUnderlyingType(ATypeInfo: PTypeInfo;
  out AUnderlyingTypeInfo: PTypeInfo): Boolean;
procedure GetUnderlyingValue(const AValue: TValue;
  out AUnderlyingValue: TValue);
function TryGetUnderlyingValue(const AValue: TValue;
  out AUnderlyingValue: TValue): Boolean;
procedure SetUnderlyingValue(var AValue: TValue;
  const AUnderlyingValue: TValue);
function TrySetUnderlyingValue(var AValue: TValue;
  const AUnderlyingValue: TValue): Boolean;
...
for TGUID public class function Create(const B: TArray<Byte>): TGUID;
  overload;
static;
...
begin
  if Length(B) <> SizeOf(Result) then
    raise EArgumentException.CreateFmt('SInvalidGuidArray', [SizeOf(Result)]);
...
function UuidCreateSequential(out AGuid: TGUID): HResult; stdcall;
  external 'rpcrt4.dll' name 'UuidCreateSequential';
...
begin
  Result := ATypeKind in [TTypeKind.tkRecord{$IFDEF DELPHI103},
    TTypeKind.tkMRecord{$ENDIF}];
end;
...
begin
  Result := (ATypeInfo <> nil) and IsRecord(ATypeInfo.Kind) and
    (Pos(PrefixString, GetTypeName(ATypeInfo)) = 1);
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            AValueField.SetValue(AInstance, TValue.FromOrdinal(AHandle,
              AUnderlyingValue.AsInt64));
            Exit;
          end;
        end;
...
{$IFDEF DELPHIXE}
  if ALeftValue.TypeInfo = System.TypeInfo(TdxNullableInteger) then
    Result := PdxNullableInteger(ALeftValue.GetReferenceToRawData)
      ^ = PdxNullableInteger(ARightValue.GetReferenceToRawData)^
  else if ALeftValue.TypeInfo = System.TypeInfo(PdxNullableString) then
    Result := PdxNullableString(ALeftValue.GetReferenceToRawData)
      ^ = PdxNullableString(ARightValue.GetReferenceToRawData)^
  else if ALeftValue.TypeInfo = System.TypeInfo(PdxNullableBoolean) then
    Result := PdxNullableBoolean(ALeftValue.GetReferenceToRawData)
      ^ = PdxNullableBoolean(ARightValue.GetReferenceToRawData)^
  else if ALeftValue.TypeInfo = System.TypeInfo(PdxNullableSingle) then
    Result := PdxNullableSingle(ALeftValue.GetReferenceToRawData)
      ^ = PdxNullableSingle(ARightValue.GetReferenceToRawData)^
  else if ALeftValue.TypeInfo = System.TypeInfo(PdxNullableDouble) then
    Result := PdxNullableDouble(ALeftValue.GetReferenceToRawData)
      ^ = PdxNullableDouble(ARightValue.GetReferenceToRawData)^
  else
  begin
...
function GetValueHash(const AGuid: TGUID): Integer; overload;
begin
  Result := AGuid.D1 xor ((AGuid.D2 shl 16) or AGuid.D3)
    xor ((AGuid.D4[2] shl 24) or AGuid.D4[7]);
end;
...
{$IFDEF DELPHIXE8}
              Result := THashBobJenkins.GetHashValue(AExtended,
                SizeOf(AExtended), 0);
{$ELSE}
...
{$IFEND}
    ...
    tkRecord{$IFDEF DELPHI103}, tkMRecord{$ENDIF}:
      begin
        ...
        else if AData.TypeInfo = System.TypeInfo(TBCD) then
          Result := GetValueHash(PBCD(AData.GetReferenceToRawData)^)
        else if IsNullableType(AData.TypeInfo) then
        begin
...
class function TdxFormatterHelper.Concat(const AOperands: array of string;
...
class function TdxFormatterHelper.Concat(AOperands: TList<string>; AFunc: TFunc;
  const ADelimiter: string): string;
var
  AArg: string;
  ASb: TStringBuilder;
...
class function TdxFormatterHelper.CommaText(AOperands: TList<string>;
  const ADelimiter: string): string;
var
  AArg: string;
  ASb: TStringBuilder;
...
class function TdxFormatterHelper.ExtractFromBrackets
  (const AValue: string): string;
var
  APos: Integer;
begin
  APos := {$IFDEF DELPHIXE3}AValue.IndexOf('(') +
    1{$ELSE}TdxStringHelper.IndexOf(AValue, '(') + 1{$ENDIF};
  if APos > 0 then
{$IFDEF DELPHIXE3}
    Result := AValue.Substring(APos, AValue.IndexOf(')') - APos)
{$ELSE}
    Result := TdxStringHelper.Substring(AValue, APos,
      TdxStringHelper.IndexOf(AValue, ')') - APos)
{$ENDIF}
  else
    Result := '';
end;
...
begin
  if IsNullOrEmpty and AValue.IsNullOrEmpty then
    Exit(True)
  else if IsNullOrEmpty or AValue.IsNullOrEmpty then
    Exit(False);
  ...
  else if IsEnumeration and AValue.IsEnumeration then
  begin
    Result := AsOrdinal = AValue.AsOrdinal
  end
  else if IsString and AValue.IsString then
    Result := AsString = AValue.AsString
  else if IsClass and AValue.IsClass then
    Result := AsClass = AValue.AsClass
  else if IsObject and AValue.IsObject then
    Result := AsObject = AValue.AsObject
  else if IsRecord and AValue.IsRecord then
  begin
    ...
    begin
      if TypeInfo = System.TypeInfo(TGUID) then
        Result := IsEqualGUID(PGUID(GetReferenceToRawData)^,
          PGUID(AValue.GetReferenceToRawData)^)
      else if TypeInfo = System.TypeInfo(TTimeSpan) then
        Result := PTimeSpan(GetReferenceToRawData)
          ^ = PTimeSpan(AValue.GetReferenceToRawData)^
      else if IsNullableType(TypeInfo) then
...
class function TValueHelper.FromNullable<T>(const AValue
  : TdxNullableValue<T>): TValue;
begin
    ...
    case ATypeInfo.Kind of
      ...
      tkFloat:
        case ATypeInfo.TypeData.FloatType of
          ...
          TFloatType.ftDouble, TFloatType.ftExtended, TFloatType.ftComp:
            begin
              if (TypeInfo = System.TypeInfo(TDateTime)) or
                (TypeInfo = System.TypeInfo(TDate)) or
                (TypeInfo = System.TypeInfo(TTime)) then
      ...
      tkRecord{$IFDEF DELPHI103}, tkMRecord{$ENDIF}:
        begin
          ...
          else if ATypeInfo = System.TypeInfo(TBCD) then
...
begin
  Result := (TypeInfo = System.TypeInfo(TDateTime)) or
    (TypeInfo = System.TypeInfo(TDate)) or (TypeInfo = System.TypeInfo(TTime));
end;
...
begin
  Result := IsEmpty or ((Kind = tkVariant) and (FindVarData(AsVariant)^.VType
    in [varNull, varEmpty]));
end;
...
class function TValueHelper.ConvertArray(ASourceTypeInfo, ADestinationTypeInfo
  : PTypeInfo; ASourceBuffer: Pointer): TValue;
var
  AArray: Pointer;
  ALength: Longint;
  ASrcElSize, ADstElSize: Integer;
begin
  if (ASourceTypeInfo.Kind <> tkDynArray) or
    (ADestinationTypeInfo.Kind <> tkDynArray) or
    IsManaged(GetDynArrayElType(ASourceTypeInfo)) or
    IsManaged(GetDynArrayElType(ADestinationTypeInfo)) then
    NotImplemented;
  AArray := nil;
...
class function TdxEMFDefault.GetTerminalInSize(ASize: Integer;
  AParametersPerObject: Integer): Integer;
begin
  if AParametersPerObject <= 1 then
    Result := GetTerminalInSize(ASize)
  else
    Result := GetTerminalInSize
      (Math.Min(Math.Max(GetTerminalInSize(MaxInt) div AParametersPerObject,
      1), ASize));
end;
...
function TStreamHelper.Write(const Buffer: TBytes;
...
function TStreamHelper.WriteData(const Buffer: Extended;
...
class operator TdxSmartValue.Equal(const A: TdxSmartValue;
  const B: TValue): Boolean;
begin
...
class operator TdxSmartValue.Implicit(AObject: TInterfacedObject)
  : TdxSmartValue;
var
...
class operator TdxSmartValue.NotEqual(A: TObject;
  const B: TdxSmartValue): Boolean;
begin
  Result := not(A = B);
end;

class operator TdxSmartValue.NotEqual(const A: TValue;
  const B: TdxSmartValue): Boolean;
begin
  Result := not(A = B);
end;

class operator TdxSmartValue.NotEqual(const A: TdxSmartValue;
  const B: TValue): Boolean;
begin
  Result := not(A = B);
end;
...
class operator TdxSmartValue.Implicit(const AInterface: IInterface)
  : TdxSmartValue;
begin
...
class operator TdxSmartValue.Equal(const A: TValue;
  const B: TdxSmartValue): Boolean;
begin
...
procedure TdxMRUCache<K, V>.UpdateElement(AIndex: Integer; const AKey: K;
  AValue: V);
var
  AOldValue: V;
begin
...
constructor TdxObjectMRUCache<K, V>.Create(ACount: Integer;
  AOwnsValues: Boolean = True);
begin
...
class function TdxArray<T>.Select<TTo>(const AArray: TArray<T>;
  AFunc: TFunc<T, TTo>): TArray<TTo>;
var
  I: Integer;
begin
