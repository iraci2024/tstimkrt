...
uses
  SysUtils, Classes, Controls, Windows, Graphics, Messages, Contnrs, Menus,
  Generics.Defaults, Generics.Collections,
  Types, dxCore, cxGeometry, cxGraphics, dxGDIPlusClasses, cxClasses,
  cxControls, cxLookAndFeels, dxGaugeCustomScale,
...
type
  ...
  private
    ...
    procedure ScaleChangeHandler(Sender: TObject;
  ...
  public
    ...
    function AddScale(AScaleClass: TdxGaugeCustomScaleClass)
      : TdxGaugeCustomScale;
  ...
  private
    ...
    function GetDesignDragOperation(const P: TPoint; AHitObject: TComponent)
      : TdxGaugeScaleDragOperation;
    function GetDesignMarkerIndex(const AMarkers: TRects;
      const P: TPoint): Integer;
    function GetHitObject(const P: TPoint; ACheckScaleSelectorRect: Boolean)
      : TComponent;
    function IsHitObject(const P: TPoint; AComponent: TComponent;
  ...
  private
    ...
    procedure PopulateAnchoredScales(AAnchoredScales: TList;
      AScale: TdxGaugeCustomScale);
    // Design Drag Object
    function GetDragObjectSizeDelta(ADragObject: TdxGaugeCustomScale;
      const AMousePosDelta: TPoint): TdxPointF;
    procedure MoveDragObject(ADragObject: TdxGaugeCustomScale;
      const AMousePosDelta: TPoint);
    procedure MoveDragObjects(const AMousePosDelta: TPoint);
    procedure SetDragObjectSize(ADragObject: TdxGaugeCustomScale;
      const AMousePosDelta: TPoint);
    procedure SetDragObjectWidth(ADragObject: TdxGaugeCustomScale;
      ADelta: Single; AApplyMinSize: Boolean);
    procedure SetDragObjectHeight(ADragObject: TdxGaugeCustomScale;
      ADelta: Single; AApplyMinSize: Boolean);
    // ZOrder
    ...
    procedure DoChangeZOrder(AScale: TdxGaugeCustomScale;
      AType: TdxGaugeScaleZOrderChangeType);
    ...
    function CreatePopupMenuItem(const ACaption: string; ATag: TcxTag;
  ...
  protected
    ...
    procedure LinkDragObject(AScale, AAnchorScale
      : TdxGaugeCustomScale); overload;
    ...
    procedure ChangeZOrder(AScale: TdxGaugeCustomScale;
      AType: TdxGaugeScaleZOrderChangeType); overload;
    procedure ChangeZOrder(AScales: TList;
      AType: TdxGaugeScaleZOrderChangeType); overload;
    ...
    property Control: TdxCustomGaugeControl read FControl;
    property DesignDragOperation: TdxGaugeScaleDragOperation
      read FDesignDragOperation;
  ...
  private
    ...
    procedure DrawGaugeLayer(ABitmap: TcxBitmap32;
      const AScaleLayerIndexes: array of Integer);
    procedure DrawLayer(AGPGraphics: TdxGPGraphics;
      const AScaleLayerIndexes: array of Integer);
    procedure DrawScales(AGPGraphics: TdxGPGraphics;
      ADrawMode: TdxGaugeDrawMode);
  ...
  protected
    procedure DrawScale(AGPGraphics: TdxGPGraphics; const R: TRect;
      AScaleIndex: Integer);
    ...
    procedure Paint(ACanvas: TcxCanvas; ADrawMode: TdxGaugeDrawMode;
      ADrawBackground: Boolean); overload;
  public
    constructor Create(AControl: TdxCustomGaugeControl);
    destructor Destroy; override;
  end;
...
uses
  Math, Forms, cxLibraryConsts, dxCoreGraphics, dxGDIPlusAPI,
  dxGaugeQuantitativeScale, dxGaugeUtils;

type
  TdxCustomGaugeControlAccess = class(TdxCustomGaugeControl);
  TdxGaugeControlCustomSelectionHelperAccess = class
  ...
  private
    ...
    function CreateDragImage(ADragScale: TdxGaugeCustomScaleAccess)
      : TdxGaugeControlDragImage;
...
procedure TdxCustomGaugeControl.GetChildren(AProc: TGetChildProc;
  ARoot: TComponent);
var
  I: Integer;
begin
...
function TdxCustomGaugeControl.AddScale(AScaleClass: TdxGaugeCustomScaleClass)
  : TdxGaugeCustomScale;
begin
...
function TdxCustomGaugeControl.GetDesignHitTest(X, Y: Integer;
...
function TdxCustomGaugeControl.GetDragAndDropObjectClass
...
begin
  Result := inherited GetIsDesigning and (FController <> nil) and
    FController.IsSelectionHelperAvailable;
end;
...
procedure TdxCustomGaugeControl.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
procedure TdxCustomGaugeControl.ChangeScaleEx(M, D: Integer;
  isDpiChange: Boolean);
begin
  inherited;
...
begin
  Result := (Controller.HitTest.HitObject <> nil) and
    Supports(Controller.HitTest.HitObject, IdxGaugeSelectableElement) and
    (Controller.HitTest.HitObject is TdxGaugeCustomScale);
  if Result then
    if Controller.DesignDragOperation = sdoMove then
      TdxGaugeControlDragAndDropObject(DragAndDropObject).Init(P)
    else
      TdxGaugeControlDragAndDropObject(DragAndDropObject)
        .Init(GetMouseCursorPos)
end;
...
procedure TdxCustomGaugeControl.DrawPreview(ACanvas: TcxCanvas;
  ADrawBackground: Boolean = True);
var
...
procedure TdxCustomGaugeControl.SetScales(const AValue
...
procedure TdxCustomGaugeControl.ScaleChangeHandler(Sender: TObject;
  AItem: TcxComponentCollectionItem;
  AAction: TcxComponentCollectionNotification);
begin
  Changed;
...
begin
  Result := PtInRect(TdxControlsDesignSelectorHelper.CalculateBounds
    (Control.ClientBounds, Control.ScaleFactor), P);
  if not Result then
  begin
    Calculate(P);
    Result := (HitObject <> nil) and
      PtInRect(TdxGaugeCustomScaleAccess(HitObject).GetSelectorRect, P);
  end;
end;
...
function TdxGaugeControlHitTest.GetDesignDragOperation(const P: TPoint;
  AHitObject: TComponent): TdxGaugeScaleDragOperation;
var
  AIndex: Integer;
  AMarkers: TRects;
...
begin
  Result := sdoMove;
  if (AHitObject <> nil) and (Control.Controller.Selections.Count = 1) and
    (Control.Controller.Selections.IndexOf(AHitObject) <> -1) then
    if Supports(AHitObject, IdxGaugeSelectableElement, AElement) and
      not FControl.IsShiftPressed then
    begin
...
function TdxGaugeControlHitTest.GetDesignMarkerIndex(const AMarkers: TRects;
  const P: TPoint): Integer;
var
  I: Integer;
begin
...
function TdxGaugeControlHitTest.GetHitObject(const P: TPoint;
  ACheckScaleSelectorRect: Boolean): TComponent;
var
  I, J: Integer;
  AFlag: Boolean;
  AScale: TdxGaugeCustomScale;
begin
  Result := nil;
  if not ACheckScaleSelectorRect and (Control.Controller.Selections.Count = 1)
    and IsHitObject(P, Control.Controller.Selections[0], False) and
    (GetDesignDragOperation(P, Control.Controller.Selections[0]) <> sdoMove)
  then
    ...
    for I := Control.Controller.ZOrders.Count - 1 downto 0 do
      ...
      begin
        ...
        begin
          ...
          begin
            Result := TdxGaugeCustomCaptionAccess
              (TdxGaugeCustomScaleAccess(AScale).Captions[J]);
        ...
        if not AFlag and (AScale is TdxGaugeQuantitativeScale) then
          for J := 0 to TdxGaugeQuantitativeScaleAccess(AScale)
            .Ranges.Count - 1 do
          begin
            Result := TdxGaugeCustomRangeAccess
              (TdxGaugeQuantitativeScaleAccess(AScale).Ranges[J]);
            if IsHitObject(P, Result, ACheckScaleSelectorRect) then
            begin
              AFlag := True;
              Break;
...
function TdxGaugeControlHitTest.IsHitObject(const P: TPoint;
  AComponent: TComponent; ACheckScaleSelectorRect: Boolean): Boolean;
...
begin
  ...
  begin
    if ACheckScaleSelectorRect then
      ...
      R := cxRectInflate(cxRectF(AElement.GetSelectionRect),
        dxGaugeSelectionMarkerSize / 2, dxGaugeSelectionMarkerSize / 2);
...
constructor TdxCustomGaugeControlController.Create
  (AControl: TdxCustomGaugeControl);
begin
  inherited Create;
  FControl := AControl;
...
function TdxCustomGaugeControlController.CanAnchorScale(AScaleIndex,
  AAnchorScaleIndex: Integer): Boolean;

  function GetParentIndex(AIndex: Integer): Integer;
  begin
    if AIndex > -1 then
      Result := TdxGaugeCustomScaleAccess(Control.Scales[AIndex])
        .AnchorScaleIndex
    else
      Result := -1;
  end;
  ...
  begin
    Result := (AParentIndex = -1) or (AIndex <> AParentIndex) and
      (AParentIndex <> AScaleIndex) and CheckIndexes(AParentIndex,
      GetParentIndex(AParentIndex));
  end;

  function CheckAnchorScaleIndex: Boolean;
  begin
    Result := (AAnchorScaleIndex >= -1) and
      (AAnchorScaleIndex < Control.Scales.Count);
  end;

begin
  Result := CheckAnchorScaleIndex and
    ((((AAnchorScaleIndex = -1) and (AScaleIndex > -1)) or (AScaleIndex > -1)
    and (AScaleIndex <> AAnchorScaleIndex) and CheckIndexes(AAnchorScaleIndex,
    GetParentIndex(AAnchorScaleIndex))));
end;

function TdxCustomGaugeControlController.CreateAnchoredScaleList
  (AAnchorScaleList: TList): TList;
var
  I: Integer;
begin
  Result := TList.Create;
...
function TdxCustomGaugeControlController.GetDesignCursor
  (const P: TPoint): TCursor;
begin
  HitTest.Calculate(P);
...
begin
  ...
  begin
    Result := ((TObject(Selections[I]) is TdxGaugeCustomRange) or
      (TObject(Selections[I]) is TdxGaugeCustomCaption));
    if Result then
      Break;
  end;
end;

function TdxCustomGaugeControlController.GetDesignHitTest(const P: TPoint;
...
begin
  ...
  begin
    TdxGaugeControlCustomSelectionHelperAccess(FSelectionHelper)
...
function TdxCustomGaugeControlController.GetDragAndDropObjectClass
  : TcxDragAndDropObjectClass;
begin
  Result := TdxGaugeControlDragAndDropObject;
end;

procedure TdxCustomGaugeControlController.AddCaption
  (AScale: TdxGaugeCustomScale);
var
...
procedure TdxCustomGaugeControlController.CopyDragObject
  (const AMousePosDelta: TPoint);
var
  I: Integer;
...
begin
  ...
  try
    ...
    begin
      ...
      AScale := Control.AddScale
        (TdxGaugeCustomScaleClass(ABaseScale.ClassType));
...
procedure TdxCustomGaugeControlController.GetScalesByDependences(AList: TList);
  ...
  procedure AddChild(AList: TList; AStack: TStack;
    AParent: TdxGaugeCustomScale);
  var
    I: Integer;
  begin
    for I := 0 to FControl.Scales.Count - 1 do
      if TdxGaugeCustomScaleAccess(FControl.Scales[I]).AnchorScale = AParent
      then
      begin
...
begin
  if HitTest.HitObject is TdxGaugeCustomScale then
    LinkDragObject(TdxGaugeCustomScaleAccess(Selections[0]),
      TdxGaugeCustomScale(HitTest.HitObject));
end;

procedure TdxCustomGaugeControlController.LinkDragObject(AScale,
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        AScaleAccess.CenterPositionFactorX :=
          Max(Min(AScaleAccess.CenterPositionFactorX, 1), 0);
        AScaleAccess.CenterPositionFactorY :=
          Max(Min(AScaleAccess.CenterPositionFactorY, 1), 0);
      end
      else
      begin
        AScaleAccess.CenterPositionX :=
          Max(Min(AScaleAccess.CenterPositionX, Control.ClientBounds.Right),
          Control.ClientBounds.Left);
        AScaleAccess.CenterPositionY :=
          Max(Min(AScaleAccess.CenterPositionY, Control.ClientBounds.Bottom),
...
procedure TdxCustomGaugeControlController.Select(AScale: TComponent;
  AShift: TShiftState);
begin
  if ssLeft in AShift then
    TdxGaugeControlCustomSelectionHelperAccess(FSelectionHelper)
      .Select(AScale, Control.IsShiftPressed);
end;
...
procedure TdxCustomGaugeControlController.SetDragOperation
  (AOperation: TdxGaugeScaleDragOperation);
begin
...
procedure TdxCustomGaugeControlController.AddZOrder
  (AScale: TdxGaugeCustomScale);
begin
...
procedure TdxCustomGaugeControlController.ChangeZOrder
  (AScale: TdxGaugeCustomScale; AType: TdxGaugeScaleZOrderChangeType);
begin
  DoChangeZOrder(AScale, AType);
end;

procedure TdxCustomGaugeControlController.ChangeZOrder(AScales: TList;
  AType: TdxGaugeScaleZOrderChangeType);
var
  I: Integer;
begin
...
procedure TdxCustomGaugeControlController.ChangeSelectionsZOrders
  (AType: TdxGaugeScaleZOrderChangeType);
begin
...
begin
  Result := dxCompareValues(TdxGaugeCustomScaleAccess(AItem1).StoredZOrder,
    TdxGaugeCustomScaleAccess(AItem2).StoredZOrder);
end;
...
procedure TdxCustomGaugeControlController.RemoveZOrder
  (AScale: TdxGaugeCustomScale);
begin
  if CheckComponentState then
  begin
...
begin
  Result := TdxGaugeControlCustomSelectionHelperAccess(FSelectionHelper)
    .Selections;
end;

procedure TdxCustomGaugeControlController.PopulateAnchoredScales(AAnchoredScales
  : TList; AScale: TdxGaugeCustomScale);
  ...
  begin
    ...
    begin
      AScaleAccess := TdxGaugeCustomScaleAccess(TdxCustomGaugeControl(Control)
        .Scales[I]);
...
function TdxCustomGaugeControlController.GetDragObjectSizeDelta
  (ADragObject: TdxGaugeCustomScale; const AMousePosDelta: TPoint): TdxPointF;
var
...
procedure TdxCustomGaugeControlController.MoveDragObject
  (ADragObject: TdxGaugeCustomScale; const AMousePosDelta: TPoint);

  procedure SetCenterPositionFactor(ADragObjectAccess
    : TdxGaugeCustomScaleAccess);
    ...
    begin
      ...
      Result := cxPointF(AMousePosDelta.X / AAnchorBounds.Width,
        AMousePosDelta.Y / AAnchorBounds.Height);
    end;

  begin
    ADragObjectAccess.CenterPositionFactorX :=
      ADragObjectAccess.CenterPositionFactorX + GetFactorDelta.X;
    ADragObjectAccess.CenterPositionFactorY :=
      ADragObjectAccess.CenterPositionFactorY + GetFactorDelta.Y;
  end;

  procedure SetCenterPosition(ADragObjectAccess: TdxGaugeCustomScaleAccess);
  begin
    ADragObjectAccess.CenterPositionX := ADragObjectAccess.CenterPositionX +
      AMousePosDelta.X;
    ADragObjectAccess.CenterPositionY := ADragObjectAccess.CenterPositionY +
      AMousePosDelta.Y;
  end;
...
procedure TdxCustomGaugeControlController.MoveDragObjects
  (const AMousePosDelta: TPoint);
var
  I: Integer;
  AScaleAccess: TdxGaugeCustomScaleAccess;
    ...
    try
      for I := 0 to Selections.Count - 1 do
        ...
        begin
          ...
          if (AScaleAccess.AnchorScaleIndex = -1) or
            (Selections.IndexOf(AScaleAccess.AnchorScale) = -1) then
...
procedure TdxCustomGaugeControlController.SetDragObjectSize
  (ADragObject: TdxGaugeCustomScale; const AMousePosDelta: TPoint);
var
...
procedure TdxCustomGaugeControlController.SetDragObjectWidth
  (ADragObject: TdxGaugeCustomScale; ADelta: Single; AApplyMinSize: Boolean);
var
...
begin
  ...
  if ADragObjectAccess.Height = 0 then
    ...
    ADragObjectAccess.Height :=
      Max(ADragObjectAccess.Height + Round(ADelta), 2);
end;

procedure TdxCustomGaugeControlController.BringBackward
  (AScale: TdxGaugeCustomScale);
begin
  SetZOrder(AScale, TdxGaugeCustomScaleAccess(AScale).ZOrder - 1);
end;

procedure TdxCustomGaugeControlController.BringForward
  (AScale: TdxGaugeCustomScale);
begin
  SetZOrder(AScale, TdxGaugeCustomScaleAccess(AScale).ZOrder + 1);
end;

procedure TdxCustomGaugeControlController.BringToFront
  (AScale: TdxGaugeCustomScale);
begin
  SetZOrder(AScale, Control.Scales.Count - 1);
end;

procedure TdxCustomGaugeControlController.DoChangeZOrder
  (AScale: TdxGaugeCustomScale; AType: TdxGaugeScaleZOrderChangeType);
begin
  case AType of
    octBringToFront:
...
procedure TdxCustomGaugeControlController.SendToBack
  (AScale: TdxGaugeCustomScale);
begin
  SetZOrder(AScale, 0);
end;

procedure TdxCustomGaugeControlController.SetZOrder(AScale: TdxGaugeCustomScale;
...
procedure TdxCustomGaugeControlController.UpdateAnchorReferences
  (AScale: TdxGaugeCustomScale);
var
  I: Integer;
begin
  ...
  begin
    ...
    for I := 0 to Control.Scales.Count - 1 do
      if TdxGaugeCustomScaleAccess(Control.Scales[I]).AnchorScale = AScale then
        TdxGaugeCustomScaleAccess(Control.Scales[I]).AnchorScale :=
          TdxGaugeCustomScaleAccess(AScale).AnchorScale;
    Control.Scales.EndUpdate(False);
  end;
end;

function TdxCustomGaugeControlController.CheckDesignPopupMenu
  (AShift: TShiftState): Boolean;
begin
...
function TdxCustomGaugeControlController.CreatePopupMenuItem(const ACaption
  : string; ATag: TcxTag; AOnClick: TNotifyEvent): TMenuItem;
begin
...
procedure TdxCustomGaugeControlController.AddDeleteScaleMenuItem
  (APopupMenu: TPopupMenu);
begin
  APopupMenu.Items.Add(CreatePopupMenuItem(cLineCaption, -1, nil));
  APopupMenu.Items.Add(CreatePopupMenuItem('Delete', 1002,
    MenuItemOnClickHandler));
end;

procedure TdxCustomGaugeControlController.AddRestoreScaleParametersMenuItem
  (APopupMenu: TPopupMenu);
const
...
begin
  ...
  begin
    ...
    APopupMenu.Items.Add
      (CreatePopupMenuItem(sdxGaugeScaleRestoreStyleParameters, 1010,
      MenuItemOnClickHandler));
  end;
end;

procedure TdxCustomGaugeControlController.AddScalesMenuItems
  (APopupMenu: TPopupMenu);
var
  I: Integer;
...
begin
  ...
  try
    ...
    if CanScaleElementEditors then
      APopupMenu.Items.Add(CreatePopupMenuItem('Add Caption', 4001,
        MenuItemOnClickHandler));
    for I := 0 to AScaleClasses.Count - 1 do
    begin
      AScaleName := TdxGaugeCustomScaleAccessClass(AScaleClasses[I])
        .GetScaleName;
      if TdxGaugeCustomScaleAccessClass(AScaleClasses[I]).GetScaleType = stContainerScale
      then
        APopupMenu.Items.Add(CreatePopupMenuItem('Add ' + AScaleName, 2001 + I,
          MenuItemOnClickHandler));
    end;
    for I := 0 to AScaleClasses.Count - 1 do
      if TdxGaugeCustomScaleAccessClass(AScaleClasses[I]).GetScaleType <> stContainerScale
      ...
      begin
        ...
        AMenuItem.Add(CreatePopupMenuItem(AScaleName, 2001 + I,
          MenuItemOnClickHandler));
      end;
  finally
    AScaleClasses.Free;
  ...
  if CanShowRangeEditor then
    APopupMenu.Items.Add(CreatePopupMenuItem('Add Range', 4002,
      MenuItemOnClickHandler));
end;

procedure TdxCustomGaugeControlController.AddZOrdersMenuItems
  (APopupMenu: TPopupMenu);
const
  dxGaugeScaleZOrderChangeTypeName: array [TdxGaugeScaleZOrderChangeType]
    of string = ('Bring to Front', 'Bring Forward', 'Send Backward',
    'Send to Back');
var
  I: TdxGaugeScaleZOrderChangeType;
begin
  APopupMenu.Items.Add(NewLine);
  for I := Low(TdxGaugeScaleZOrderChangeType)
    to High(TdxGaugeScaleZOrderChangeType) do
    APopupMenu.Items.Add(CreatePopupMenuItem(dxGaugeScaleZOrderChangeTypeName
      [I], 101 + Integer(I), MenuItemOnClickHandler));
end;

procedure TdxCustomGaugeControlController.MenuItemOnClickHandler
  (ASender: TObject);
  ...
  function GetZOrderChangeType(AMenuItemTag: TcxTag)
    : TdxGaugeScaleZOrderChangeType;
  begin
...
begin
  ...
  begin
    ...
    else if InRange(AItem.Tag, 1000, 2000) then
      case AItem.Tag of
        1002:
          DeleteSelection;
        1010:
    ...
    else if InRange(AItem.Tag, 2001, 3000) then
      AddScale(AItem.Tag - 2001)
    else if InRange(AItem.Tag, 4000, 5000) then
      case AItem.Tag of
        4001:
...
procedure TdxCustomGaugeControlController.PopupDesignMenu
  (const APopupPoint: TPoint);
begin
...
begin
  Result := (Selections.Count = 1) and
    not(TObject(Selections[0]) is TdxGaugeContainerScale);
end;

function TdxCustomGaugeControlController.CanShowRangeEditor: Boolean;
begin
  Result := (TObject(Selections[0]) is TdxGaugeQuantitativeScale) and
    not(Selections.Count > 1);
end;

procedure TdxCustomGaugeControlController.ShowDesignPopupMenu
  (AShift: TShiftState);
begin
...
function TdxCustomGaugeControlController.CanCalculate(const ABounds
  : TRect): Boolean;
begin
...
function TdxCustomGaugeControlController.GetSelectionContentRects: TRects;
  ...
  begin
    for I := 0 to AScales.Count - 1 do
      if Supports(TComponent(AScales[I]), IdxGaugeSelectableElement, AElement)
      then
...
function TdxCustomGaugeControlController.GetSelectionRects
  (AList: TList): TRects;
var
  I: Integer;
  AElement: IdxGaugeSelectableElement;
...
function TdxCustomGaugeControlController.GetScaleBounds
  (AScale: TdxGaugeCustomScale): TdxRectF;
var
...
begin
  ...
  if AAnchorScaleIndex = -1 then
    ...
    Result := TdxGaugeCustomScaleAccess
      (FControl.Scales[AAnchorScaleIndex]).Bounds;
end;
...
begin
  ...
  try
    ...
    Control.BeginUpdate;
    AScale := Control.AddScale
      (TdxGaugeCustomScaleClass(AList[AScaleClassIndex]));
    TdxGaugeCustomScaleAccess(AScale).AnchorScale :=
      HitTest.HitObject as TdxGaugeCustomScale;
    Control.EndUpdate;
    Control.Modified;
  finally
    AList.Free;
...
var
  AScales: TList;
begin
  if FControl.HandleAllocated and CheckComponentState and
    CanCalculate(FControl.ClientBounds) then
  begin
    AScales := TList.Create;
    try
...
procedure TdxCustomGaugeControlController.DoCalculateScales(AScaleList: TList);
  ...
  procedure PopulateChangedLayers(AScaleChangedLayers
...
function TdxGaugeControlDragAndDropObject.GetDragAndDropCursor
...
function TdxGaugeControlDragAndDropObject.ProcessKeyDown(AKey: Word;
...
function TdxGaugeControlDragAndDropObject.ProcessKeyUp(AKey: Word;
  AShiftState: TShiftState): Boolean;
begin
...
procedure TdxGaugeControlDragAndDropObject.DragAndDrop(const P: TPoint;
...
function TdxGaugeControlDragAndDropObject.CreateDragImage
  (ADragScale: TdxGaugeCustomScaleAccess): TdxGaugeControlDragImage;
begin
  Result := TdxGaugeControlDragImage.Create;
  Result.SetBounds(0, 0, GetScaleSize(ADragScale).cx,
    GetScaleSize(ADragScale).cy);
  Result.OffsetPoint := cxPointOffset(ADragScale.GetSelectionRect.TopLeft,
    FStartDragPoint, False);
end;
...
function TdxGaugeControlDragAndDropObject.GetScaleSize
  (ADragScale: TdxGaugeCustomScale): TSize;
begin
...
procedure TdxGaugeControlDragAndDropObject.DrawDragImage(ACanvas: TcxCanvas;
  const R: TRect; AScale: TdxGaugeCustomScale);
var
...
begin
  ...
  try
    ...
    AGPGraphics.TranslateWorldTransform(-AScaleAccess.GetSelectionRect.Left,
      -AScaleAccess.GetSelectionRect.Top);
...
procedure TdxGaugeControlDragAndDropObject.PopulateDragImages
  (AScaleList: TList);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawDragImage(ADragImage.Canvas, ADragImage.ClientRect,
        TdxGaugeCustomScaleAccess(AScaleList[I]));
      FDragImages.Add(ADragImage);
    end;
  end;
end;

procedure TdxGaugeControlDragAndDropObject.ShowDragImages
...
begin
  if FDragImages <> nil then
    ...
    begin
      TdxGaugeControlDragImage(FDragImages[I])
        .MoveTo(cxPointOffset(ACursorPos,
        TdxGaugeControlDragImage(FDragImages[I]).OffsetPoint));
      TdxGaugeControlDragImage(FDragImages[I]).Visible := True;
    end;
end;

function TdxGaugeControlDragAndDropObject.CanChangeHeight
  (ADragObject: TdxGaugeCustomScale; AOperation: TdxGaugeScaleDragOperation;
...
begin
  ...
  if Result and ((AOperation = sdoResizeBottom) and
    (GetClientCursorPos.Y >= ACenter.Y) or (AOperation = sdoResizeTop) and
    (GetClientCursorPos.Y <= ACenter.Y)) then
    ADelta := GetSizeDelta.Y
  else
    ADelta := MaxInt;
end;

function TdxGaugeControlDragAndDropObject.CanChangeWidth
  (ADragObject: TdxGaugeCustomScale; AOperation: TdxGaugeScaleDragOperation;
  out ADelta: Integer): Boolean;
var
  ACenter: TdxPointF;
...
begin
  ...
  if Result and ((GetClientCursorPos.X >= ACenter.X) and
    (AOperation = sdoResizeRight) or (GetClientCursorPos.X <= ACenter.X) and
    (AOperation = sdoResizeLeft)) then
...
procedure TdxGaugeControlDragAndDropObject.MoveDragObjects
  (const AMousePosDelta: TPoint);
begin
  GetControl.Controller.MoveDragObjects(AMousePosDelta);
end;

procedure TdxGaugeControlDragAndDropObject.ResizeDragObject
  (const ACursorPos: TPoint);
var
...
begin
  ...
  begin
    ADragObject := TdxGaugeCustomScaleAccess
      (GetControl.Controller.Selections[0]);
    ACanResizeX := True;
    ACanResizeY := True;
    case GetControl.Controller.DesignDragOperation of
      sdoResizeTop, sdoResizeBottom:
        ACanResizeX := CanChangeHeight(ADragObject,
          GetControl.Controller.DesignDragOperation, ADelta.Y);
      sdoResizeLeft, sdoResizeRight:
        ACanResizeX := CanChangeWidth(ADragObject,
          GetControl.Controller.DesignDragOperation, ADelta.X);
      sdoResizeTopLeft:
        begin
      ...
      sdoResizeBottomLeft:
        begin
          ACanResizeX := CanChangeWidth(ADragObject, sdoResizeLeft, ADelta.X);
          ACanResizeY := CanChangeHeight(ADragObject, sdoResizeBottom,
            ADelta.Y);
        end;
      sdoResizeBottomRight:
        begin
...
procedure TdxGaugeControlDragAndDropObject.UpdateCursor
  (AShiftState: TShiftState);
begin
  FIsCopying := (ssCtrl in AShiftState) and
    (GetControl.Controller.DesignDragOperation = sdoMove) and
...
constructor TdxCustomGaugeControlPainter.Create
  (AControl: TdxCustomGaugeControl);
begin
  inherited Create;
  FControl := AControl;

  FStaticElementsLayer := TcxBitmap32.CreateSize(FControl.Width,
    FControl.Height, True);
  SetLength(FScaleStaticLayerIndexes, 1);
  FScaleStaticLayerIndexes[0] := 0;

  FDynamicElementsLayer := TcxBitmap32.CreateSize(FControl.Width,
    FControl.Height, True);
...
procedure TdxCustomGaugeControlPainter.DrawScale(AGPGraphics: TdxGPGraphics;
  const R: TRect; AScaleIndex: Integer);
var
  I: Integer;
  AScaleAccess: TdxGaugeCustomScaleAccess;
begin
...
procedure TdxCustomGaugeControlPainter.Paint(ACanvas: TcxCanvas;
  ADrawMode: TdxGaugeDrawMode);
begin
...
procedure TdxCustomGaugeControlPainter.Paint(ACanvas: TcxCanvas;
  ADrawMode: TdxGaugeDrawMode; ADrawBackground: Boolean);
var
  AGPGraphics: TdxGPGraphics;
begin
...
begin
  if FControl.IsDrawingPreview and FControl.Transparent then
    ...
    FControl.LookAndFeelPainter.DrawGaugeControlBackground(ACanvas,
      FControl.Bounds, FControl.Transparent, FControl.Color);
end;
...
procedure TdxCustomGaugeControlPainter.DrawContentRects
  (AGPGraphics: TdxGPGraphics);
var
  ARects: TRects;
begin
...
begin
  cxDrawDesignRect(ACanvas, TdxControlsDesignSelectorHelper.CalculateBounds
    (FControl.ClientBounds, FControl.ScaleFactor),
    FControl.Controller.IsControlSelected);
end;

procedure TdxCustomGaugeControlPainter.DrawGaugeLayer(ABitmap: TcxBitmap32;
  const AScaleLayerIndexes: array of Integer);
var
  AGPGraphics: TdxGPGraphics;
begin
  AGPGraphics := dxGpBeginPaint(ABitmap.Canvas.Handle,
...
procedure TdxCustomGaugeControlPainter.DrawLayer(AGPGraphics: TdxGPGraphics;
  const AScaleLayerIndexes: array of Integer);
var
...
begin
  for ALayerIndex := Low(AScaleLayerIndexes) to High(AScaleLayerIndexes) do
    for AScaleIndex := 0 to FControl.Controller.ZOrders.Count - 1 do
      TdxGaugeCustomScaleAccess(FControl.Controller.ZOrders[AScaleIndex])
        .DrawLayer(AGPGraphics, AScaleLayerIndexes[ALayerIndex]);
end;

procedure TdxCustomGaugeControlPainter.DrawScales(AGPGraphics: TdxGPGraphics;
  ADrawMode: TdxGaugeDrawMode);
...
begin
  ...
  begin
    ...
    begin
      ...
      cxDrawDesignRect(ACanvas, ACaptionAccess.GetSelectorRect,
        IsSelected(ACaptionAccess));
    end;
    if AScale is TdxGaugeQuantitativeScale then
      ...
      begin
        ARangeAccess := TdxGaugeCustomRangeAccess
          (TdxGaugeQuantitativeScaleAccess(AScale).Ranges[J]);
        cxDrawDesignRect(ACanvas, ARangeAccess.GetSelectorRect,
          IsSelected(ARangeAccess));
      end;
  end;
end;

procedure TdxCustomGaugeControlPainter.DrawSelections
  (AGPGraphics: TdxGPGraphics);
  ...
  begin
    ...
    for I := 0 to FControl.Controller.Selections.Count - 1 do
      if Supports(TComponent(FControl.Controller.Selections[I]),
        IdxGaugeSelectableElement, AElement) then
      begin
  ...
  var
    ...
    AScales: TList;
  begin
    AScales := FControl.Controller.CreateAnchoredScaleList
      (FControl.Controller.Selections);
    try
    ...
    ARects := FControl.Controller.GetSelectionRects
      (FControl.Controller.Selections);
...
procedure TdxCustomGaugeControlPainter.PrepareBitmaps;
  ...
  begin
    FControl.Controller.ChangedLayers := FControl.Controller.ChangedLayers -
      [AChangedLayer];
  end;
