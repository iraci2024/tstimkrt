...
uses
  Windows, Messages, Types, Classes, Forms,
  Controls {must be after Forms for D11} , Graphics, SysUtils,
...
type
  ...
  TcxHintAnimationStyle = (cxhaSlideFromLeft, cxhaSlideFromRight,
    cxhaSlideDownward, cxhaSlideUpward, cxhaSlideFromCenter, cxhaHide,
    cxhaActivate, cxhaFadeIn, cxhaAuto, cxhaNone);
  ...
  public
    ...
    property AnimationStyle: TcxHintAnimationStyle read FAnimationStyle
      write FAnimationStyle;
    ...
    property NeedEraseBackground: Boolean read FNeedEraseBackground
      write FNeedEraseBackground;
  end;
  ...
  protected
    procedure ControllerChangedNotification(AStyleController
      : TcxCustomHintStyleController); virtual;
    procedure ControllerFreeNotification(AHintStyleController
      : TcxCustomHintStyleController); virtual;
  ...
  TcxHintStyleChangedEvent = procedure(Sender: TObject;
    AStyle: TcxCustomHintStyle) of object;
  ...
  TcxCustomHintStyleController = class(TcxCustomComponent,
  ...
  private
    ...
    procedure DoShowHintEx(var AHintStr, AHintCaption: string;
  ...
  protected
    ...
    property HintStyleClass: TcxHintStyleClass read GetHintStyleClass
      write SetHintStyleClass;
    property HintStyleClassName: string read GetHintStyleClassName
      write SetHintStyleClassName;
    ...
    property OnShowHintEx: TcxShowHintExEvent read FOnShowHintEx
      write FOnShowHintEx;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    property UseHintControlLookAndFeel: Boolean read FUseHintControlLookAndFeel
      write FUseHintControlLookAndFeel default False;
  end;
  ...
  public
    constructor Create(const AHint, AShortCut: string;
  ...
  TcxCustomHintHelper = class(TcxIUnknownObject, IcxMouseTrackingCaller,
  ...
  public
    ...
    procedure ShowHint(const AHintAreaBounds, ATextRect: TRect;
      const AText: string; AIsHintMultiLine: Boolean; AHintObject: TObject;
      AFont: TFont = nil);
...
procedure cxActivateHint(AHintWindow: THintWindow; const ARect: TRect;
  const AHint: string);
procedure cxActivateHintData(AHintWindow: THintWindow; const ARect: TRect;
...
function cxProcessControlHintInfo(out AHintInfo: THintInfo;
  ...
  var AHintWinRect: TRect; AData: Pointer = nil;
  AHintHidePause: Integer = 0): Boolean;
...
function cxProcessControlHintInfo(out AHintInfo: THintInfo;
  ...
  begin
    if AHintClass = nil then
      AHintClass := HintWindowClass;
...
begin
  ...
  begin
    ...
    else if (AControl is TWinControl) and
      (TWinControl(AControl).ParentWindow <> 0) then
      Windows.ClientToScreen(TWinControl(AControl).ParentWindow, AParentOrigin);
    OffsetRect(AHintInfo.CursorRect, AParentOrigin.X - AClientOrigin.X,
      AParentOrigin.Y - AClientOrigin.Y);
  end;
  ...
  begin
    ...
    if Result and (AHintInfo.HintStr <> '') and
      (IsHintStyleControllerExists or not IsEqual(AHintInfo, APrevHintInfo))
    then
    begin
      ...
      with AHintInfo do
      begin
        AHintWinRect := AHintWindow.CalcHintRect(HintMaxWidth, HintStr,
          HintData);
...
type
  ...
  protected
    ...
    procedure RegisterHintStyleController(AController
      : TcxCustomHintStyleController);
    procedure UnRegisterHintStyleController(AController
      : TcxCustomHintStyleController);
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
...
procedure TcxHintController.RegisterHintStyleController
...
begin
  if (AController <> nil) and not(csDesigning in AController.ComponentState)
  then
  begin
    if FControllerList.Count = 0 then
...
procedure TcxHintController.UnRegisterHintStyleController
  (AController: TcxCustomHintStyleController);
...
begin
  ...
  begin
    ...
    if (FControllerList.Remove(AController) > -1) and AIsHintWindowClassNotChanged
    then
...
begin
  if FControllerList.Count > 0 then
    Result := TcxCustomHintStyleController(FControllerList.Last)
      .GetHintWindowClass
  else
...
function TcxHintController.FindHintStyleController
  : TcxCustomHintStyleController;

  function FindHintControllerOnParent(AParent: TWinControl)
    : TcxCustomHintStyleController;
  begin
    Result := TcxCustomHintStyleController(cxFindComponent(AParent,
      TcxCustomHintStyleController));
  end;
  ...
  begin
    ...
    begin
      ...
      if (Result = nil) and Supports(AHintedControl, IdxManagedObject,
        AManagedObject) then
      begin
...
procedure TcxHintController.ShowHintHandler(var HintStr: string;
...
procedure TcxHintController.DoApplicationShowHint(var HintStr: string;
  var CanShow: Boolean; var HintInfo: THintInfo);
begin
...
procedure TcxBaseHintWindow.CheckExecuteAnimation(const AHint: string);
  ...
  const
    AAnimationStyleMap: array [TcxHintAnimationStyle] of Integer =
      (AW_HOR_POSITIVE, AW_HOR_NEGATIVE, AW_VER_POSITIVE, AW_VER_NEGATIVE,
      AW_CENTER, AW_HIDE, AW_ACTIVATE, AW_BLEND, AW_ACTIVATE, AW_ACTIVATE);
  begin
    ...
    if not IsWinXPOrLater then
      DisableRegion;
    AnimateWindowProc(Handle, AnimationDelay,
      AAnimationStyleMap[AAnimationStyle] or AW_SLIDE);
  end;

var
  AAnimationStyle: TcxHintAnimationStyle;
begin
  FHasAnimation := (GetTickCount - FLastActive > 250) and (Length(AHint) < 100)
    and Assigned(AnimateWindowProc);
  ...
  if not HasWindowRegion then
    DisableRegion
  else if not FHasAnimation and IsWinXPOrLater then
    EnableRegion;
end;
...
begin
  ...
  if HasWindowRegion and FHasAnimation then
    cxRedrawWindow(Handle, RDW_INVALIDATE or RDW_FRAME or RDW_ERASE or
      RDW_UPDATENOW)
  else
    Invalidate;
end;
...
procedure TcxBaseHintWindow.WMWindowPosChanging(var Message
  : TWMWindowPosChanging);
var
  AWindowPos: PWindowPos;
begin
  AWindowPos := Message.WindowPos;
  if not FStandardHint and not FActivating and
    ((AWindowPos.flags and SWP_SHOWWINDOW) <> 0) then
...
function TcxCustomHintWindow.GetPainter: TcxCustomLookAndFeelPainter;
  ...
  function GetLookAndFeelContainer(out ALookAndFeelContainer
    : IcxLookAndFeelContainer): Boolean;
  var
    AManagedObject: IdxManagedObject;
    AHintedControl: TControl;
  begin
    AHintedControl := cxGetHintedControl;
    Result := Supports(AHintedControl, IcxLookAndFeelContainer,
      ALookAndFeelContainer) or Supports(AHintedControl, IdxManagedObject,
      AManagedObject) and Supports(AManagedObject.GetManager,
      IcxLookAndFeelContainer, ALookAndFeelContainer);
  end;
...
procedure TcxCustomHintStyle.ControllerChangedNotification(AStyleController
  : TcxCustomHintStyleController);
begin
  // do nothing
end;

procedure TcxCustomHintStyle.ControllerFreeNotification(AHintStyleController
  : TcxCustomHintStyleController);
begin
  // do nothing
end;

procedure TcxCustomHintStyle.DoShowHint(var AHintStr: string;
  var ACanShow: Boolean; var AHintInfo: THintInfo);
begin
  // do nothing
end;
...
procedure TcxCustomHintStyleController.ShowHint(X, Y: Integer;
  ACaption, AHint: string; AMaxWidth: Integer = 0);
begin
...
procedure TcxCustomHintStyleController.AddListener
  (AListener: TcxCustomHintStyle);
begin
  if (AListener <> nil) and (FListeners.IndexOf(AListener) < 0) then
    FListeners.Add(AListener);
end;

procedure TcxCustomHintStyleController.RemoveListener
  (AListener: TcxCustomHintStyle);
begin
  if not(csDestroying in ComponentState) and (AListener <> nil) and
...
procedure TcxCustomHintStyleController.DoHintStyleChanged
  (AStyle: TcxCustomHintStyle);
begin
...
function TcxCustomHintStyleController.GetHintWindowClass
  : TcxCustomHintWindowClass;
begin
  Result := HintStyle.GetHintWindowClass;
end;

procedure TcxCustomHintStyleController.DoShowHintEx(var AHintStr,
  AHintCaption: string; var ACanShow: Boolean; var AHintInfo: THintInfo);
begin
...
function TcxCustomHintStyleController.GetLookAndFeelPainter
...
procedure TcxCustomHintStyleController.SetHintStyleClassName
  (const Value: string);
begin
  HintStyleClass := TcxHintStyleClass
...
constructor TdxHintViewInfoHelper.Create(const AHint, AShortCut: string;
  const ACursorPos: TPoint);
begin
  inherited Create;
  if AHint <> '' then
...
begin
  R := GetTextRect;
  ATextFlags := DT_NOCLIP or DT_NOPREFIX or DT_WORDBREAK or
    AAlignment[UseRightToLeftAlignment];
...
begin
  ...
  begin
    ...
    ACanvas.Font.Height := ScaleFactor.Apply(ACanvas.Font.Height,
      dxSystemScaleFactor);
  end
  else
...
begin
  ...
  R := Rect(0, 0, AWidth, 0);
  cxDrawText(ACanvas.Handle, FText, R, DT_CALCRECT or DT_LEFT or DT_NOPREFIX or
    DT_WORDBREAK);
...
begin
  Result := (FHintableObject = nil) or
    (Supports(FHintableObject, IcxHintableObject, AIntf) and
    AIntf.UseHintHidePause);
end;
...
function TcxCustomHintHelper.GetHintWindowRect(const AHintAreaBounds,
  ATextRect: TRect; const AText: string; AIsHintMultiLine: Boolean): TRect;
...
begin
  ...
  if SysLocale.MiddleEast and (HintWindow.BiDiMode = bdRightToLeft) then
    AOffset := GetHintWinControl.ClientToScreen
      (Point(ATextRect.Right - Result.Right, ATextRect.Top))
  else
...
begin
  ...
  begin
    ...
    begin
      if Assigned(FHintHideTimer) and not cxPointIsEqual(FLastHintMousePos,
        GetMouseCursorPos) then
        RestartHintHideTimer
      else if UseHintHidePause then
        Exit;
    end
    else if UseHintHidePause then
    begin
  ...
  else if FLastHintableObject <> nil then
    CancelHint;

  if (FHintWindow = nil) or (FHintWindow.ClassType <> GetHintWindowClass) then
    RecreateHintWindow
  else if IsWindowVisible(FHintWindow.Handle) then
  begin
  ...
  AHintWindowRect := GetHintWindowRect(AHintAreaBounds, ATextRect, S,
    AIsHintMultiLine);
