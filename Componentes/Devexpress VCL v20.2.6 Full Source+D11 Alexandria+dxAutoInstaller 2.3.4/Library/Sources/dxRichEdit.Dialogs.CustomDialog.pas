...
{$IFDEF DELPHIXE2}
  System.UITypes,
{$ENDIF}
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Generics.Defaults,
  Generics.Collections,
  Controls, Forms, Dialogs, dxForms,
  cxClasses, cxDropDownEdit, cxRadioGroup, cxTextEdit, dxCoreClasses,
  cxSpinEdit, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
...
type
  ...
  private
    ...
    FMeasurementEditors: TDictionary<TdxMeasurementUnitEdit,
      TdxMeasurementUnitEditHelper>;
    FMeasurementTypes: TObjectDictionary<TdxMeasurementUnitEditHelper,
      TdxMeasurementType>;
  ...
  protected
    ...
    function CreateController(AControllerParameters
      : TdxFormControllerParameters): TdxFormController; virtual; abstract;
    function GetUnitConverter: TdxDocumentModelUnitConverter; virtual;
    class function GetUnitTypeDescription
      (AUnitType: TdxMeasurementUnit): string;
    procedure Populate(ACombobox: TcxCustomComboBox;
      AProc: TProc<TcxCustomComboBox>); overload;
    procedure Populate(ARadioGroup: TcxRadioGroup;
      AProc: TProc<TcxRadioGroup>); overload;
    procedure InitializeForm; dynamic;
    procedure SetLookAndFeel; virtual;
    procedure SetController(AController: TdxFormController);

    procedure AddItemValue(AItems: TStrings; const AText: string;
      const AValue: Integer);
    function GetItemValueContainer(ACombobox: TcxCustomComboBox)
      : TdxItemValueContainer; inline;
    function TryGetItemValue(ACombobox: TcxCustomComboBox;
      out AValue: Integer): Boolean;
    procedure UpdateSelectedIndex(ACombobox: TcxCustomComboBox;
      const AValue: Integer);

    function DoValidate(AEditor: TcxSpinEdit; AMinValue: Integer = 0;
      AMaxValue: Integer = 0): Boolean;

    class function IsMetric: Boolean; static;
    class function ListSeparator: Char; inline;

    procedure InitializeMeasurementUnitEdit(AEditor: TdxMeasurementUnitEdit;
    ...
    function GetMeasurementUnitEditHelper(AEditor: TdxMeasurementUnitEdit)
      : TdxMeasurementUnitEditHelper;
    function GetValueFromEditor(AEditor: TdxMeasurementUnitEdit;
      ACorrectRange: Boolean = True): Variant;
    function TryGetValueFromEditor(AEditor: TdxMeasurementUnitEdit;
      out AValue: Integer; ACorrectRange: Boolean = True): Boolean; overload;
    function InchesToUIUnit(AValue: Single): Single;
    function ModelUnitsToUIUnit(AValue: Single): Single;
    procedure SetValueToEditor(AEditor: TdxMeasurementUnitEdit;
      const AValue: Variant); overload;
    procedure SetValueToEditor(AEditor: TdxMeasurementUnitEdit;
      const AValue: TdxNullableInteger); overload;
    class function ToMeasurementType(AUnitType: TdxMeasurementUnit)
      : TdxMeasurementType;
    procedure MeasurementUnitEditIncrementValueHandler(Sender: TObject;
      AButton: TcxSpinEditButton; var AValue: Variant; var AHandled: Boolean);
    procedure MeasurementUnitEditValidateHandler(Sender: TObject;
  ...
  public
    ...
    procedure Initialize(AControllerParameters
      : TdxFormControllerParameters); virtual;
...
procedure TdxRichEditCustomDialogForm.AddItemValue(AItems: TStrings;
  const AText: string; const AValue: Integer);
var
...
begin
  ...
  FMeasurementEditors := TDictionary<TdxMeasurementUnitEdit,
    TdxMeasurementUnitEditHelper>.Create;
  FMeasurementTypes := TObjectDictionary<TdxMeasurementUnitEditHelper,
    TdxMeasurementType>.Create([doOwnsKeys]);
end;
...
function TdxRichEditCustomDialogForm.DoValidate(AEditor: TcxSpinEdit;
  AMinValue, AMaxValue: Integer): Boolean;
var
  ANewValue: Integer;
begin
  ...
  begin
    AEditor.SetFocus;
    MessageDlg(cxGetResourceString(@sdxRichEditInvalidNumber), mtWarning,
      [mbOK], 0);
    Exit(False);
  end;
  if AMinValue = 0 then
  ...
  if (AMaxValue <> 0) and ((ANewValue < AMinValue) or (ANewValue > AMaxValue))
  then
  begin
    AEditor.SetFocus;
    MessageDlg(Format(cxGetResourceString(@sdxRichEditInvalidSize),
      [AMinValue, AMaxValue]), mtWarning, [mbOK], 0);
    Result := False;
  end
  else
    Result := True;
...
function TdxRichEditCustomDialogForm.GetItemValueContainer
  (ACombobox: TcxCustomComboBox): TdxItemValueContainer;
begin
...
function TdxRichEditCustomDialogForm.GetUnitConverter
  : TdxDocumentModelUnitConverter;
begin
  Result := Control.DocumentModel.UnitConverter;
end;

class function TdxRichEditCustomDialogForm.GetUnitTypeDescription
  (AUnitType: TdxMeasurementUnit): string;
begin
  case AUnitType of
...
function TdxRichEditCustomDialogForm.GetValueFromEditor
  (AEditor: TdxMeasurementUnitEdit; ACorrectRange: Boolean = True): Variant;
var
  AValue: Variant;
...
var
  AUIUnit: TdxUIUnit;
begin
  AUIUnit := UnitConverter.ToUIUnitF(UnitConverter.InchesToModelUnitsF(AValue),
    UnitType);
  Result := AUIUnit.Value;
end;

procedure TdxRichEditCustomDialogForm.Initialize(AControllerParameters
...
procedure TdxRichEditCustomDialogForm.Populate(ACombobox: TcxCustomComboBox;
  AProc: TProc<TcxCustomComboBox>);
var
  ASavedIndex: Integer;
begin
...
procedure TdxRichEditCustomDialogForm.InitializeMeasurementUnitEdit
  (AEditor: TdxMeasurementUnitEdit; AType: TdxMeasurementType;
  AHelper: TdxMeasurementUnitEditHelper);
begin
  FMeasurementTypes.AddOrSetValue(AHelper, AType);
  FMeasurementEditors.AddOrSetValue(AEditor, AHelper);
  AEditor.ActiveProperties.OnIncrementValue :=
    MeasurementUnitEditIncrementValueHandler;
...
procedure TdxRichEditCustomDialogForm.MeasurementUnitEditIncrementValueHandler
...
begin
  ...
  AHandled := GetMeasurementUnitEditHelper(AEditor)
    .IncrementValue(AButton, AValue);
end;

procedure TdxRichEditCustomDialogForm.MeasurementUnitEditValidateHandler
...
begin
  ...
  begin
    ...
    Error := (CompareValue(AFloatValue, AMeasurementUnitEditHelper.MinValue) <
      0) or (CompareValue(AFloatValue,
      AMeasurementUnitEditHelper.MaxValue) > 0);
  end;
  if Error then
  begin
    AMinStr := AMeasurementUnitEditHelper.GetTextFromValue
      (AMeasurementUnitEditHelper.MinValue);
    AMaxStr := AMeasurementUnitEditHelper.GetTextFromValue
      (AMeasurementUnitEditHelper.MaxValue);
    ErrorText :=
...
procedure TdxRichEditCustomDialogForm.Populate(ARadioGroup: TcxRadioGroup;
  AProc: TProc<TcxRadioGroup>);
begin
...
procedure TdxRichEditCustomDialogForm.RemoveMeasurementUnitEdit
  (AEditor: TdxMeasurementUnitEdit);
var
  AHelper: TdxMeasurementUnitEditHelper;
begin
...
procedure TdxRichEditCustomDialogForm.SetValueToEditor
  (AEditor: TdxMeasurementUnitEdit; const AValue: Variant);
var
  AText: string;
  AMeasurementType: TdxMeasurementType;
  AHelper: TdxMeasurementUnitEditHelper;
  ...
  begin
    ...
    begin
      ...
      case AMeasurementType of
        ...
        TdxMeasurementType.mtPoint:
          AText := AHelper.GetTextFromValue
            (UnitConverter.ModelUnitsToPointsF(AValue));
        TdxMeasurementType.mtInch:
          AText := AHelper.GetTextFromValue
            (UnitConverter.ModelUnitsToInchesF(AValue));
        TdxMeasurementType.mtMillimeter:
          AText := AHelper.GetTextFromValue
            (UnitConverter.ModelUnitsToMillimetersF(AValue));
        TdxMeasurementType.mtCentimeter:
          AText := AHelper.GetTextFromValue
            (UnitConverter.ModelUnitsToCentimetersF(AValue));
      end;
    end;
  end;
  AEditor.Text := AText;
...
procedure TdxRichEditCustomDialogForm.SetController
  (AController: TdxFormController);
begin
  FreeAndNil(FController);
...
begin
  if Owner is TdxVCLControl then
    ...
    AOwnerLookAndFeel := TdxCustomLayoutLookAndFeelAccess
      (TdxRichEditCustomDialogForm(Owner).dxLayoutControl1.LookAndFeel)
      .LookAndFeel
  else
    Exit;
  for I := 0 to dxLayoutLookAndFeelList1.Count - 1 do
  begin
    ACustomLookAndFeel := Safe<TdxLayoutCxLookAndFeel>.Cast
      (dxLayoutLookAndFeelList1.Items[I]);
...
procedure TdxRichEditCustomDialogForm.SetValueToEditor
  (AEditor: TdxMeasurementUnitEdit; const AValue: TdxNullableInteger);
var
  AVariant: Variant;
begin
...
class function TdxRichEditCustomDialogForm.ToMeasurementType
  (AUnitType: TdxMeasurementUnit): TdxMeasurementType;
begin
  case AUnitType of
    TdxMeasurementUnit.Inch:
...
function TdxRichEditCustomDialogForm.TryGetItemValue
  (ACombobox: TcxCustomComboBox; out AValue: Integer): Boolean;
var
  AItemValueContainer: TdxItemValueContainer;
begin
...
function TdxRichEditCustomDialogForm.TryGetValueFromEditor
  (AEditor: TdxMeasurementUnitEdit; out AValue: Integer;
...
procedure TdxRichEditCustomDialogForm.UpdateSelectedIndex
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AItemValueContainer <> nil) and (AItemValueContainer.Value = AValue)
      then
      begin
