...
uses
  ...
  Types, Dialogs, cxComponentCollectionEditor, Menus, ActnList, ImgList,
  ComCtrls,
...
const
  BandTypeAsString: array [0 .. 3] of string = ('Header', 'Description',
    'Footer', 'Standard footer');
  ShowOptionsButtonText: array [Boolean] of string = ('Show options',
    'Hide options');
...
begin
  if (ScreenTip = nil) and ((Sender as TComponent).Tag < 3) then
    Exit;
...
begin
  GetBand(FPopupTrackPaintBox.Tag).GlyphFixedWidth :=
    not GetBand(FPopupTrackPaintBox.Tag).GlyphFixedWidth;
  FPopupTrackPaintBox.Invalidate;
  DataChanged;
end;
...
function TfrmScreenTipRepositoryEditor.EditAction
  (Action: DesignIntf.TEditAction): Boolean;
var
  AEdit: TCustomEdit;
begin
...
begin
  if ScreenTip = nil then
    Exit;
  if TryStrToInt(edtWidth.Text, ANewValue) and
    InRange(ANewValue, 0, UpDown1.Max) and (ScreenTip.Width <> ANewValue) then
  begin
...
procedure TfrmScreenTipRepositoryEditor.SelectionsChanged(const ASelection
...
function TfrmScreenTipRepositoryEditor.GetBand(AIndex: Integer)
  : TdxCustomScreenTipBand;
begin
  case AIndex of
    0:
      Result := ScreenTip.Header;
    1:
      Result := ScreenTip.Description;
    2:
      Result := ScreenTip.Footer;
...
procedure TfrmScreenTipRepositoryEditor.RefreshHint;
  ...
  begin
    ...
    begin
      ...
      Result := TdxScreenTipLikeHintViewInfo.Create(APainter, AHintText, '',
        cxNullPoint);
    end;
  end;
...
begin
  ...
  try
    ...
    begin
      FViewInfo := CreateViewInfo('<Hint provided by hinted control>',
        ScreenTip, RootLookAndFeel.Painter);
...
procedure TfrmScreenTipRepositoryEditor.btnDescriptionRichTextEditClick
  (Sender: TObject);
var
...
begin
  case TdxScreenTipBandType(AFontInfoViewer.Tag) of
    stbHeader:
      AFont := Repository.HeaderFont;
    stbDescription:
      AFont := Repository.DescriptionFont;
  else { stbFooter }
...
function TdxScreenTipCollectionProperty.GetEditorClass
