...
{$ENDIF}
  ...
  dxSpreadSheetCore, dxHashUtils,
  dxSpreadSheet, dxSpreadSheetTypes, dxSpreadSheetGraphics, dxPSCore, dxPSForm,
  dxPSBaseGridLnk, dxPSExcelEdgePatterns,
  dxPSExcelFillPatterns, dxPSEdgePatterns, dxPSFillPatterns, cxDrawTextUtils,
  cxPC, dxPSGlbl, cxControls, cxContainer,
  cxEdit, cxLabel, cxCheckBox, Menus, cxLookAndFeelPainters, cxTextEdit,
  cxButtons, cxGraphics, cxDropDownEdit, dxCore,
  cxMaskEdit, cxColorComboBox, dxPSReportRenderCanvas, cxLookAndFeels,
  dxSpreadSheetUtils, cxClasses,
  dxPSReportLinkDesignWindow, dxLayoutControlAdapters, dxLayoutLookAndFeels,
  dxLayoutContainer, dxLayoutControl,
  dxLayoutcxEditAdapters, dxSpreadSheetStyles, cxImage, dxPScxCommon,
...
type
  ...
  TdxPSSpreadSheetReportLinkOptionView = (ssovRowAndColumnHeadings,
    ssovGridLines, ssovSuppressSourceFormats);
  TdxPSSpreadSheetReportLinkOptionsView = set of
  ...
  published
    ...
    property OptionsView: TdxPSSpreadSheetReportLinkOptionsView
      read FOptionsView write SetOptionsView default [ssovRowAndColumnHeadings];
  ...
  public
    ...
    property ReportLink: TdxSpreadSheetCustomReportLink read GetReportLink
      write SetReportLink;
  end;
...
uses
  SysUtils, Forms, Math,
  dxPSRes, dxPSUtl, dxExtCtrls, dxPrnDev, dxPSImgs, cxGeometry, RTLConsts,
  dxCoreClasses, dxSpreadSheetClasses,
  dxSpreadSheetContainers, dxSpreadSheetConditionalFormattingIconSet,
...
function TdxSpreadSheetCustomReportLink.CanPrintContainer
  (AContainer: TdxSpreadSheetContainer): Boolean;
begin
  Result := inherited CanPrintContainer(AContainer) and
    not(AContainer is TdxSpreadSheetCommentContainer);
end;
...
procedure TdxSpreadSheetCustomReportLink.SetOptionsView
  (Value: TdxPSSpreadSheetReportLinkOptionsView);
begin
...
begin
  inherited;
  chbxShowRowAndColumnHeadings.Checked := ssovRowAndColumnHeadings
    in ReportLink.OptionsView;
  ...
  chbxSuppressSourceFormats.Checked := ssovSuppressSourceFormats
    in ReportLink.OptionsView;
...
begin
  ...
  chbxShowRowAndColumnHeadings.Caption :=
    cxGetResourceString(@sdxShowRowAndColumnHeadings);
  ...
  chbxSuppressSourceFormats.Caption :=
    cxGetResourceString(@sdxSuppressSourceFormats);

  stTransparent.Caption := ' ' + cxGetResourceString(@sdxTransparent) + ' ';
  lblColor.Caption := cxGetResourceString(@sdxColor);
  stFixedTransparent.Caption := ' ' + cxGetResourceString
    (@sdxFixedTransparent) + ' ';
  ...
  chbxFixedRowsOnEveryPage.Caption :=
    cxGetResourceString(@sdxRepeatHeaderRowAtTop);

  lblSelection.Caption := cxGetResourceString(@sdxSelection);
  chbxOnlySelected.Caption := cxGetResourceString(@sdxOnlySelected);
end;

procedure TfmdxSpreadSheet2DesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
begin
  inherited;
  dxPSBaseGridLnk.dxPSDrawGridPreview(ACanvas, R, ReportLink,
    ReportLink.IsShowRowAndColumnHeadings,
    ReportLink.IsShowRowAndColumnHeadings, ScaleFactor);
end;
...
function TfmdxSpreadSheet2DesignWindow.GetReportLink
  : TdxSpreadSheetCustomReportLink;
begin
  Result := inherited ReportLink as TdxSpreadSheetCustomReportLink;
end;

procedure TfmdxSpreadSheet2DesignWindow.SetReportLink
  (Value: TdxSpreadSheetCustomReportLink);
begin
...
procedure TfmdxSpreadSheet2DesignWindow.CMDialogChar
...
procedure TfmdxSpreadSheet2DesignWindow.chbxFixedRowsOnEveryPageClick
  (Sender: TObject);
begin
...
begin
  ...
  case TcxColorComboBox(Sender).Tag of
    0:
      ReportLink.Color := Color;
    1:
      ReportLink.FixedColor := Color;
    2:
      ReportLink.GridLineColor := Color;
  end;
  Modified := True;
  UpdatePreview;
end;
...
procedure TfmdxSpreadSheet2DesignWindow.chbxRowAutoHeightClick(Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
begin
  ...
  begin
    case TcxCheckBox(Sender).Tag of
      0:
        AOption := ssovRowAndColumnHeadings;
      1:
        AOption := ssovGridLines;
      2:
        AOption := ssovSuppressSourceFormats;
    else
      Exit;
    end;
...
procedure TfmdxSpreadSheet2DesignWindow.chbxFixedTransparentClick
  (Sender: TObject);
begin
  if not LockControlsUpdate then
  begin
    case TcxCheckBox(Sender).Tag of
      ...
      1:
        ReportLink.FixedTransparent := TcxCheckBox(Sender).Checked;
    end;
    Modified := True;
    UpdatePreview;
  end;
...
procedure TfmdxSpreadSheet2DesignWindow.stFixedTransparentClick
  (Sender: TObject);
begin
...
dxPSRegisterReportLink(TdxSpreadSheetDocumentBasedReportLink,
  TdxCustomSpreadSheet, nil);
dxPSRegisterReportLink(TdxSpreadSheetReportLnk, TdxCustomSpreadSheet,
  TfmdxSpreadSheet2DesignWindow);

finalization

dxPSUnregisterReportLink(TdxSpreadSheetDocumentBasedReportLink,
  TdxCustomSpreadSheet, nil);
dxPSUnregisterReportLink(TdxSpreadSheetReportLnk, TdxCustomSpreadSheet,
  TfmdxSpreadSheet2DesignWindow);
