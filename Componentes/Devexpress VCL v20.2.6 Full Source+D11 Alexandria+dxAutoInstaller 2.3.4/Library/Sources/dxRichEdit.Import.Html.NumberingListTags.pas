...
type
  ...
  protected
    class function SetBulletNumberingFormat(const AValue: string;
      ...
      : TdxHtmlListLevelProperties; static;
    class function SetNumberigFormat(const AValue: string;
      AListLevelProperties: TdxHtmlListLevelProperties)
      : TdxHtmlListLevelProperties; static;
    function GetListLevelProperties: TdxHtmlListLevelProperties;
      virtual; abstract;
  ...
  strict private
    ...
    class procedure TypeAttributeKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure StartAttributeKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
  protected
    ...
    property OldListLevelProperties: TdxHtmlListLevelProperties
      read FOldListLevelProperties;
  public
  ...
  strict private
    ...
    class procedure TypeAttributeKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
    class procedure ValueAttributeKeyword(AImporter: TdxCustomHtmlImporter;
      const AValue: string; ATag: TdxTagBase); static;
  protected
    function GetAttributeTable: TdxAttributeKeywordTranslatorTable; override;
    function GetListLevelProperties: TdxHtmlListLevelProperties; override;
    ...
    procedure ReplaceSimpleNumberingToBulleted(const ALevel
      : IdxListLevel); virtual;
    procedure CorrectLevelDisplayFormatString(const ALevel
      : IdxListLevel); virtual;
...
class function TdxNumberingListTagBase.SetBulletNumberingFormat
  (const AValue: string; AListLevelProperties: TdxHtmlListLevelProperties)
...
begin
  ...
  else if AValueName = 'circle' then
  begin
    AListLevelProperties.Format := TdxNumberingFormat.Bullet;
    AListLevelProperties.BulletFontName := 'Courier New';
  end
  else if AValueName = 'square' then
  begin
    AListLevelProperties.Format := TdxNumberingFormat.Bullet;
...
begin
  ...
  begin
    AProperties := ListLevelProperties.MergeWith
      (Importer.Position.ListLevelProperties);
    try
...
begin
  if Importer.DocumentModel.ActivePieceTable.Paragraphs
    [Importer.Position.ParagraphIndex].IsInList then
...
begin
  ...
  if TdxDocumentFormatsHelper.ShouldInsertMultiLevelNumbering(DocumentModel)
  then
    Importer.Position.LevelIndex := Importer.Position.LevelIndex + 1
  else
    Importer.Position.LevelIndex := 0;

  ALevel.CopyFrom(DocumentModel.AbstractNumberingLists.Last.Levels
    [Importer.Position.LevelIndex] as TdxListLevel);
  ALevel := Importer.Position.ListLevelProperties.
    ApplyPropertiesToListLevel(ALevel);
  DocumentModel.AbstractNumberingLists.Last.Levels[Importer.Position.LevelIndex]
    := ALevel;
end;

procedure TdxNumberingListTagBase.CreateNewBulletAbstractNumberingList;
begin
  TdxDefaultNumberingListHelper.InsertDefaultBulletNumberingList(DocumentModel,
...
begin
  TdxDefaultNumberingListHelper.InsertDefaultSimpleNumberingList(DocumentModel,
    DocumentModel.UnitConverter,
    DocumentModel.DocumentProperties.DefaultTabWidth);
end;
...
begin
  ...
  begin
    ...
    if (ATag.Name = TdxHtmlTagNameID.NumberingList) or
      (ATag.Name = TdxHtmlTagNameID.BulletList) then
      Exit;
...
class function TdxNumberingListTag.AddAttributes
...
class procedure TdxNumberingListTag.TypeAttributeKeyword
...
class procedure TdxNumberingListTag.StartAttributeKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
var
  ANumberingListTag: TdxNumberingListTag;
begin
...
function TdxNumberingListTag.GetAttributeTable
  : TdxAttributeKeywordTranslatorTable;
begin
...
procedure TdxNumberingListTag.BeforeDeleteTagFromStack(AIndexOfDeletedTag
  : Integer);
var
  ATag: TdxNumberingListTag;
begin
  ATag := Safe<TdxNumberingListTag>.Cast
    (Importer.TagsStack[AIndexOfDeletedTag].Tag);
  if ATag <> nil then
...
class function TdxBulletListTag.AddAttributes
  : TdxAttributeKeywordTranslatorTable;
var
...
class procedure TdxBulletListTag.TypeAttributeKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
var
...
begin
  if TdxDocumentFormatsHelper.NeedReplaceBulletedLevelsToDecimal(DocumentModel)
  then
  begin
    FListLevelProperties.BulletFontName :=
      DocumentModel.DefaultCharacterProperties.FontName;
    FListLevelProperties.Format := TdxNumberingFormat.Decimal;
  end
  else if TdxDocumentFormatsHelper.ShouldInsertBulletedNumbering(DocumentModel)
  then
  begin
...
class procedure TdxLevelTag.TypeAttributeKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
var
  ALevelTag: TdxLevelTag;
begin
...
class procedure TdxLevelTag.ValueAttributeKeyword
  (AImporter: TdxCustomHtmlImporter; const AValue: string; ATag: TdxTagBase);
var
  ALevelTag: TdxLevelTag;
begin
...
begin
  if ((Importer.Position.LevelIndex >= 0) and
    (Tag.ElementType = TdxHtmlElementType.OpenTag)) and
    (Importer.Position.ListLevelProperties.Format = TdxNumberingFormat.Bullet)
  then
  begin
    Importer.Position.ListLevelProperties.UseBulletFontName := False;
    SetBulletNumberingFormat
      (BulletTypes[Importer.Position.LevelIndex mod Length(BulletTypes)],
      FListLevelProperties);
  end;
...
begin
  ApplyAutoSpacing;
  AProperties := Importer.Position.ListLevelProperties.MergeWith
    (FListLevelProperties);
  try
    Importer.Position.ListLevelProperties.CopyFrom(AProperties);
  finally
...
begin
  ...
  then
  begin
    AOldLeftIndentFromParentTags := Importer.Position.ParagraphFormatting.
      LeftIndent;
...
begin
  if Importer.Position.ListLevelProperties.Format = TdxNumberingFormat.Bullet
  then
    CreateNewBulletAbstractNumberingList
  else
    CreateNewSimpleAbstractNumberingList;
...
begin
  if (not DocumentModel.DocumentCapabilities.ParagraphsAllowed) or
    ...
    Exit;
  AParagraph := DocumentModel.MainPieceTable.Paragraphs
    [Importer.Position.ParagraphIndex];
  if Importer.Position.LevelIndex < 0 then
    Importer.AppendText(DocumentModel.MainPieceTable,
      TdxCharacters.Bullet + '  ')
  else
  begin
    AAbstractLastLevel := DocumentModel.AbstractNumberingLists.Last.Levels
      [Importer.Position.LevelIndex];

    ALevel := TdxListLevel.Create(DocumentModel);
    try
      ALevel.ListLevelProperties.CopyFrom
        (AAbstractLastLevel.ListLevelProperties);
      ALevel.CharacterProperties.BeginInit;
      ALevel.CharacterProperties.CopyFrom
        (AAbstractLastLevel.CharacterProperties);
      ALevel.CharacterProperties.EndInit;

      if Importer.Position.ListLevelProperties.UseFormat then
        ALevel.ListLevelProperties.Format :=
          Importer.Position.ListLevelProperties.Format;

      if Importer.Position.ListLevelProperties.UseStart then
        ALevel.ListLevelProperties.Start :=
          Importer.Position.ListLevelProperties.Start;

      if not ALevel.ListLevelProperties.Info.Equals
        (AAbstractLastLevel.ListLevelProperties.Info) then
      begin
        if IsPreviousCreatedListUsedBefore then
          CreateNewAbstractNumberingList;
        SetDisplayFormatString(ALevel);
        DocumentModel.AbstractNumberingLists.Last.Levels
          [Importer.Position.LevelIndex] := ALevel;
        ALevel := nil;
      end
      else
      ...
      try
        ...
        DocumentModel.MainPieceTable.AddNumberingListToParagraph(AParagraph,
...
begin
  Result := Importer.UsedNumberingLists.Contains
    (DocumentModel.NumberingLists.Count - 1);
end;
...
procedure TdxLevelTag.ReplaceSimpleNumberingToBulleted
...
procedure TdxLevelTag.CorrectLevelDisplayFormatString
  (const ALevel: IdxListLevel);
begin
  if Importer.Position.ListLevelProperties.Format <> TdxNumberingFormat.Bullet
  then
    ALevel.ListLevelProperties.DisplayFormatString :=
      Format('%%%d:s.', [Importer.Position.LevelIndex])
  else
...
begin
  ALevel.CharacterProperties.FontName :=
    Importer.Position.ListLevelProperties.BulletFontName;
  SetDotCharacterCore(ALevel);
end;
...
begin
  ...
  else if UpperCase(ALevel.CharacterProperties.FontName) = 'WINGDINGS' then
