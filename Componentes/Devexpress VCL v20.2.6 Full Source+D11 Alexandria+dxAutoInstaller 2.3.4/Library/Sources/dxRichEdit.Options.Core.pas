...
type
  TdxRichEditOptionsAction = (AllCaps, AllowAlternateStyleNames, AllowCopy,
    AllowCreateNew, AllowCut, AllowDrag, AllowDrop, AllowNameResolution,
    AllowNonLinkedListDefinitions, AllowOfficeScrolling, AllowOpen, AllowPaste,
    AllowPrinting, AllowSave, AllowSaveAs, AllowShowPopupMenu,
    AllowTablesToExtendIntoMargins, AllowTouch, AllowZooming,
    AlternateImageFolder, AsyncImageLoading, AuthenticationEMail,
    AuthenticationGroup, AuthenticationPassword, AuthenticationUserName,
    AutoDetectDocumentCulture, AutoDetectEncoding, BackColor,
    BackColorExportMode, BookmarkColor, Bookmarks, BookmarkVisibility, Bulleted,
    CharacterFormatting, CharacterStyle, CssPropertiesExportType,
    CurrentFileName, CurrentFormat, DataSource, Date,
    DefaultCharacterPropertiesExportToCss, DefaultFileName, DefaultFormat,
    DefaultTableCellMargin, DefaultTableCellSpacing, DetectUrls,
    DuplicateObjectAsMetafile, EmbedImages, EnablePageBackgroundOnPrint,
    Encoding, EndNoteNamePrefix, EndNoteNumberStringFormat, EndNotes,
    EndNoteSeparator, ExportBulletsAndNumbering, ExportFinalParagraphMark,
    ExportHiddenText, ExportImageSize, ExportRootTag, ExportToClipboard,
    FallbackFormat, FieldCodeEndMarker, FieldCodeStartMarker,
    FieldResultEndMarker, Fields, FloatingObjects, FontBold, FontItalic,
    FontName, FontSize, FontSource, FontStrikeout, FontUnderline, FontUnit,
    FootNoteNamePrefix, FootNoteNumberStringFormat, FootNotes,
    FootNoteSeparator, ForeColor, ForeColorSource, GridLines, HeadersFooters,
    Hidden, HiddenText, HighlightColor, HighlightMode,
    HtmlNumberingListExportFormat, Hyperlinks, IgnoreDeletedText,
    IgnoreFloatProperty, IgnoreInsertedText, IgnoreMetaCharset, IgnoreNoProof,
    IgnoreParagraphOutlineLevel, IgnoreParseErrors, InlinePictures,
    InsertOptions, KeepBookmarksForRemovedRanges, KeepExternalImageSize,
    KeepLastParagraph, KeepPermissionsForRemovedRanges,
    LimitBookmarkNameTo40Chars, LimitFontNameTo31Chars,
    LimitStyleNameTo253Chars, ListExportFormat, MaintainDocumentSectionSettings,
    MatchHorizontalTableIndentsToTextEdge, MaxZoomFactor,
    MergeParagraphsContent, MergeUseFirstParagraphStyle, MinZoomFactor,
    ModifierKeys, MultiLevel, OverrideImageResolution, OvertypeAllowed,
    PageBreakInsertMode, ParagraphFormatting, ParagraphFrames, ParagraphMark,
    Paragraphs, ParagraphStyle, ParagraphTabs, PasteLineBreakSubstitution,
    PasteSingleCellAsText, RangePermissionsBracketsColor, RangePermissionsColor,
    RangePermissionsHighlightBracketsColor, RangePermissionsHighlightColor,
    RangePermissionsVisibility, RegExResultMaxGuaranteedLength,
    ReplaceSpaceWithNonBreakingSpaceInsidePre, Script, Sections, Separator,
    ShowHiddenText, ShowLeftIndent, ShowRightIndent, ShowTabs, ShowToolTip,
    Simple, Space, StrikeoutColor, StrikeoutWordsOnly, TabCharacter,
    TableCellStyle, Tables, TableStyle, TabMarker, TabSymbol,
    ThrowExceptionOnInvalidFormatSwitch, Time, UnderlineColor,
    UnderlineTocHyperlinks, UnderlineWordsOnly, Undo,
    UpdateDocVariablesBeforeCopy, UpdateDocVariablesBeforePrint,
    UpdateFieldsOnPaste, UriExportType, UseCurrentCultureForDateTimeFormatting,
    UseFontSubstitution, UseHtml5, ViewMergedData, Visibility,
    WrapContentInGroup);
  ...
  TdxRichEditNotificationOptionsChangedEvent = procedure(Sender: TObject;
    Args: TdxRichEditNotificationOptionsChangedArgs) of object;
  TdxRichEditNotificationOptionsChangedEventHandler =
    TdxMulticastMethod<TdxRichEditNotificationOptionsChangedEvent>;
  ...
  protected
    ...
    procedure DoInnerOptionsChanged(Sender: TObject;
      E: TdxRichEditNotificationOptionsChangedArgs); virtual;
    procedure SubscribeInnerOptions; virtual;

    procedure DoChanged(Action: TdxRichEditNotificationOptionsChangedArgs.
      TActions); overload; virtual;
  ...
  public
    ...
    property Changed: TdxRichEditNotificationOptionsChangedEventHandler
      read FOnChanged;
  end;
  ...
  published
    property Paragraphs: TdxDocumentCapability read FParagraphs
      write SetParagraphs default TdxDocumentCapability.Default;
    property Undo: TdxDocumentCapability read FUndo write SetUndo
...
constructor TdxRichEditNotificationOptionsChangedArgs.Create
...
begin
  if Source = nil then
    Reset
  else if Source is TdxRichEditNotificationOptions then
  begin
  end;
end;
...
procedure TdxRichEditNotificationOptions.DoInnerOptionsChanged(Sender: TObject;
...
procedure TdxRichEditNotificationOptions.DoChanged
  (Action: TdxRichEditNotificationOptionsChangedArgs.TActions);
var
...
procedure TdxRichEditNotificationOptions.DoChanged
  (Action: TdxRichEditOptionsAction);
begin
  DoChanged([Action]);
end;
...
function TdxCustomDocumentCapabilitiesOptions.IsAllowed
  (const Value: TdxDocumentCapability): Boolean;
begin
  Result := Value in [TdxDocumentCapability.Default,
    TdxDocumentCapability.Enabled];
end;
...
function TdxCustomDocumentCapabilitiesOptions.
  CharacterFormattingAllowed: Boolean;
begin
  Result := False;
end;
...
function TdxCustomDocumentCapabilitiesOptions.
  ParagraphFormattingAllowed: Boolean;
begin
  Result := False;
end;
...
procedure TdxCustomDocumentCapabilitiesOptions.SetParagraphs
...
procedure TdxCustomDocumentCapabilitiesOptions.SetUndo
  (const Value: TdxDocumentCapability);
begin
  if FUndo <> Value then
  begin
