...
type
  ...
  public
    ...
    function Add(X, Y, AWidth, AHeight: Integer; ABrush: TBrush)
      : Integer; overload;
    function Add(X, Y, AWidth, AHeight: Integer; AColor: TColor)
      : Integer; overload;
    procedure RightToLeftConversion(const AClientBounds: TRect);
  ...
  public
    constructor Create;
    function Add(const ABounds: TRect;
  ...
  public
    constructor Create;
    function Add(const ASize: TSize; AIsCategory, AUnderline: Boolean;
      const AViewParams: TcxViewParams): Integer;
...
begin
  if (Index < 0) or (Index >= FCount) then
    raise EdxException.CreateFmt('Error %s: Invalid index %d',
      [ClassName, Index]);
...
begin
  for I := 0 to Count - 1 do
    Rects[I] := TdxRightToLeftLayoutConverter.ConvertRect(Rects[I],
      AClientBounds);
end;
...
function TLineInfoList.Add(X, Y, AWidth, AHeight: Integer;
  ABrush: TBrush): Integer;
begin
...
begin
  for I := 0 to Count - 1 do
    Items[I]^.Rect := TdxRightToLeftLayoutConverter.ConvertRect(Items[I]^.Rect,
      AClientBounds);
end;
...
function TIndentRectInfoList.Add(const ASize: TSize;
  AIsCategory, AUnderline: Boolean; const AViewParams: TcxViewParams): Integer;
begin
  CheckCapacity;
  Result := FCount;
  Inc(FCount);
...
var
  ...
  I, J: Integer;
const
  APattern: array [0 .. 7] of Word = ($00AA, $0055, $00AA, $0055, $00AA, $0055,
    $00AA, $0055);
begin
