...
function dxChooseMultiplePages(AImageList: TCustomImageList;
  AImageIndex: Integer; AOrigin: TPoint; AYShift: Integer;
...
uses
  ...
  dxPSEngn, dxPSRes, dxPSUtl, cxControls, cxGraphics, cxLookAndFeelPainters,
  cxLookAndFeels, dxForms, dxDPIAwareUtils;
...
type
  ...
  protected
    ...
    procedure Initialize(const AOrigin: TPoint;
      AYShift, AColCount, ARowCount: Integer);
    ...
    property SelectedColCount: Integer read FSelectedColCount
      write SetSelectedColCount;
    property SelectedRowCount: Integer read FSelectedRowCount
      write SetSelectedRowCount;
  public
...
begin
  ...
  try
    ...
    Result := (AForm.FResult = mrOK) and (AForm.SelectedColCount > 0) and
      (AForm.SelectedRowCount > 0);
    if Result then
    begin
...
procedure TfmPageChooser.DrawBottomPart(ACanvas: TcxCanvas; const R: TRect);
  ...
  begin
    if (SelectedRowCount > 0) and (SelectedColCount > 0) then
      Result := Format('%d x %d %s', [SelectedRowCount, SelectedColCount,
        cxGetResourceString(@sdxPages)])
    else
...
begin
  ...
  begin
    ...
    cxDrawImage(ACanvas, R, nil, FCellImages, FImageIndex, True, nil,
      ScaleFactor);
  end;
end;
...
procedure TfmPageChooser.Initialize(const AOrigin: TPoint;
  AYShift, AColCount, ARowCount: Integer);
begin
  ScaleForPPI(dxGetTargetDPI(Self));

  if (AOrigin.Y + AColCount * CellWidth + 4 { non client } + 2 { frame } >
    FDesktop.Bottom) then
  begin
    if (AOrigin.X + AColCount * CellWidth + 4 { non client } + 2 { frame } >
      FDesktop.Right) then
...
procedure TfmPageChooser.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  inherited;
...
begin
  Result := MulDiv(-Font.Height, ScaleFactor.TargetDPI, 72) +
    ScaleFactor.Apply(6);
end;
...
begin
  if FCellImages <> nil then
    Result := MulDiv(dxGetImageSize(FCellImages, ScaleFactor).cy, 3, 2) +
      ScaleFactor.Apply(2)
  else
...
begin
  if FCellImages <> nil then
    Result := MulDiv(dxGetImageSize(FCellImages, ScaleFactor).cx, 3, 2) +
      ScaleFactor.Apply(2)
  else
    Result := ScaleFactor.Apply(CellSize);
end;
...
begin
  ...
  if FIsTextAtBottom then
    ...
    ARowCount := Ceil((Height - Y - 2 { frame } - 4 { non client } ) /
      CellHeight);

  if (FGrowDirection in [gdTopRight, gdBottomRight]) and
    (Left + (AColCount * CellWidth + 2 { frame } ) + 4 { non client } >
    FDesktop.Right) then
    Dec(AColCount);
  if (FGrowDirection in [gdTopLeft, gdTopRight]) and
    (Top + (ARowCount * CellHeight + BottomHeight + 2 { frame } ) +
    4 { non client } > FDesktop.Bottom) then
    Dec(ARowCount);
...
procedure TfmPageChooser.ProcessSelect(AColCount, ARowCount: Integer;
  GrowFlag: Boolean);
begin
  if GrowFlag then
  begin
...
begin
  InflateRect(Message.CalcSize_Params^.rgrc[0], -LookAndFeelPainter.BorderSize,
