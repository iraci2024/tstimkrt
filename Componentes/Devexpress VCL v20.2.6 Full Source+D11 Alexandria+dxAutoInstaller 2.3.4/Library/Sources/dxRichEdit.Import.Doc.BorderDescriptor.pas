...
type
  ...
  TdxDocBorderLineStyle = (None = $00, Single = $01, Double = $03,
    ThinSingle = $05, Dotted = $06, Dashed = $07, DotDash = $08,
    DotDotDash = $09, Triple = $0A, ThinThickSmallGap = $0B,
    ThickThinSmallGap = $0C, ThinThickThinSmallGap = $0D,
    ThinThickMediumGap = $0E, ThickThinMediumGap = $0F,
    ThinThickThinMediumGap = $10, ThinThickLargeGap = $11,
    ThickThinLargeGap = $12, ThinThickThinLargeGap = $13, Wave = $14,
    DoubleWave = $15);
  ...
  public
    class function MapToDocBorderLineStyle(ALineStyleType: TdxBorderLineStyle)
      : TdxDocBorderLineStyle; static;
    class function MapToBorderLineStyle(ALineStyleType: TdxDocBorderLineStyle)
      : TdxBorderLineStyle; static;
  end;
...
class function TdxBorderDescriptor97.FromStream(AReader: TBinaryReader)
  : TdxBorderDescriptor97;
begin
...
class function TdxBorderDescriptor97.FromByteArray(const AData: TBytes;
  AStartIndex: Integer): TdxBorderDescriptor97;
begin
  Result := TdxBorderDescriptor97.Create;
...
procedure TdxBorderDescriptor97.ConvertFromBorderInfo(AInfo: TdxBorderInfo;
  AUnitConverter: TdxDocumentModelUnitConverter);
begin
...
procedure TdxBorderDescriptor97.ApplyProperties(ADestination: TdxBorderBase;
  AUnitConverter: TdxDocumentModelUnitConverter);
begin
...
class function TdxBorderDescriptor.FromStream(AReader: TBinaryReader)
  : TdxBorderDescriptor;
begin
...
class function TdxBorderDescriptor.FromByteArray(const AData: TBytes;
  AStartIndex: Integer): TdxBorderDescriptor;
begin
  Result := TdxBorderDescriptor.Create;
  Result.Read(AData, AStartIndex);
end;
...
begin
  ...
  FColorReference.Free;
  FColorReference := TdxDocColorReference.FromByteArray
    (AReader.ReadBytes(TdxDocColorReference.ColorReferenceSize), 0);
...
begin
  ...
  if (PCardinal(@AData[AStartIndex])^ = EmptyBorder) or
    (PCardinal(@AData[AStartIndex + 4])^ = EmptyBorder) then
    Exit;
...
procedure TdxBorderDescriptor.ConvertFromBorderInfo(AInfo: TdxBorderInfo;
  AUnitConverter: TdxDocumentModelUnitConverter);
begin
  Color := AInfo.Color;
  FFrame := AInfo.Frame;
...
procedure TdxBorderDescriptor.ApplyProperties(ADestination: TdxBorderBase;
  AUnitConverter: TdxDocumentModelUnitConverter);
begin
...
class function TdxDocBorderCalculator.MapToDocBorderLineStyle
  (ALineStyleType: TdxBorderLineStyle): TdxDocBorderLineStyle;
begin
...
class function TdxDocBorderCalculator.MapToBorderLineStyle(ALineStyleType
  : TdxDocBorderLineStyle): TdxBorderLineStyle;
begin
  case ALineStyleType of
