...
uses
  ...
  dxGDIPlusClasses, dxBar, dxStatusBar,
  dxRibbonSkins, dxRibbon, dxRibbonFormCaptionHelper, cxGeometry,
  dxCoreGraphics, dxFluentDesignFormInterfaces;
...
type
  ...
  public
    ...
    function GetColorScheme(ABarControl: TCustomdxBarControl;
      out AColorScheme: TdxCustomRibbonSkin): Boolean; overload;
    function GetColorScheme(ABarItemControl: TdxBarItemControl;
      out AColorScheme: TdxCustomRibbonSkin): Boolean; overload;
  ...
  TdxRibbonStatusBarBarControlViewInfo = class
  ...
  TdxRibbonStatusBarBarControlDesignHelper = class
  ...
  public
    constructor Create(AOwner: TdxStatusBarToolbarPanelStyle);
      reintroduce; virtual;
  ...
  public
    ...
    class procedure DrawSizeGrip(AStatusBar: TdxCustomStatusBar;
      ACanvas: TcxCanvas; R: TRect); override;
    class function GetPanelBevel(APanel: TdxStatusBarPanel)
      : TdxStatusBarPanelBevel; override;
  ...
  protected
    ...
    function CreatePanelViewInfo(APanel: TdxStatusBarPanel)
      : TdxStatusBarPanelViewInfo; override;
  ...
  TdxRibbonStatusBar = class(TdxCustomStatusBar, IdxRibbonFormNonClientPart,
...
uses
  dxThemeConsts, dxUxTheme, dxOffice11, ActnList, StdActns, Math,
  dxCustomFluentDesignForm, dxBarStrs, cxDWMApi, dxBarSkinConsts;
...
begin
  ...
  begin
    ...
    Dec(ABounds.Top, GetToolbarContentOffsets(ABarControl.Bar, dsTop,
      TCustomdxBarControlAccess(ABarControl).ScaleFactor, False).Top);
    ABounds.Bottom := ABounds.Top + AStatusBar.Height;

    AColorScheme.DrawStatusBarPanel(ACanvas.Handle,
      dxMapWindowRect(AStatusBar.Handle, ABarControl.Handle,
      AStatusBar.ClientBounds), ABounds, GetIsPanelLowered(ABarControl));
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      AStatusBar := GetStatusBar(ABarControl);
      AColorScheme.DrawStatusBarPanel(DC, dxMapWindowRect(AStatusBar.Handle,
        ABarControl.Handle, AStatusBar.ClientBounds, False),
...
function TdxRibbonStatusBarBarControlPainter.GetButtonBorderHeight
...
var
  AColorScheme: TdxCustomRibbonSkin;
begin
  if not IsInSubMenu(ABarItemControl) and GetColorScheme(ABarItemControl,
    AColorScheme) then
  begin
    ...
    inherited GetDisabledTextColors(ABarItemControl, ASelected, AFlat,
      AColor1, AColor2);
end;

function TdxRibbonStatusBarBarControlPainter.GetEnabledTextColor(ABarItemControl
  : TdxBarItemControl; ASelected, AFlat: Boolean): TColor;
const
  States: array [Boolean, Boolean] of Integer = ((DXBAR_NORMAL, DXBAR_HOT),
    (DXBAR_CHECKED, DXBAR_HOTCHECK));
var
  AColorScheme: TdxCustomRibbonSkin;
begin
  if not IsInSubMenu(ABarItemControl) and GetColorScheme(ABarItemControl,
  ...
  begin
    Result := AColorScheme.GetPartColor(rspStatusBarText,
      States[(ABarItemControl is TdxBarButtonControl) and
      TdxBarButtonControlAccess(ABarItemControl).Down, ASelected]);
  end
  else
    Result := inherited GetEnabledTextColor(ABarItemControl, ASelected, AFlat);
end;

function TdxRibbonStatusBarBarControlPainter.GetIsPanelLowered
  (ABarControl: TCustomdxBarControl): Boolean;
begin
  Result := TdxRibbonStatusBarDockControl(TdxBarControl(ABarControl)
    .DockControl).Panel.Bevel <> dxpbRaised;
end;
...
function TdxRibbonStatusBarBarControlPainter.IsInSubMenu(ABarItemControl
  : TdxBarItemControl): Boolean;
begin
  Result := (ABarItemControl.Parent <> nil) and
    (ABarItemControl.Parent.Kind = bkSubMenu);
end;
...
function TdxRibbonStatusBarBarControlPainter.MarkSizeX
...
function TdxRibbonStatusBarBarControlPainter.GetColorScheme
  ...
  out AColorScheme: TdxCustomRibbonSkin): Boolean;
begin
  if ABarControl is TdxBarSubMenuControl then
    Result := GetColorScheme(TdxBarSubMenuControl(ABarControl)
      .ParentItemControl, AColorScheme)
  else
  begin
    Result := TdxBarControl(ABarControl)
      .DockControl is TdxRibbonStatusBarDockControl;
    if Result then
      AColorScheme := TdxRibbonStatusBarDockControl(TdxBarControl(ABarControl)
        .DockControl).Ribbon.ColorScheme;
  end;
end;

function TdxRibbonStatusBarBarControlPainter.GetColorScheme(ABarItemControl
  : TdxBarItemControl; out AColorScheme: TdxCustomRibbonSkin): Boolean;
begin
  Result := (ABarItemControl <> nil) and GetColorScheme(ABarItemControl.Parent,
    AColorScheme);
end;
...
begin
  Result := (TdxBarControl(ABarControl)
    .DockControl as TdxRibbonStatusBarDockControl).StatusBar;
end;

{ TdxDefaultRibbonStatusBarBarControlPainter }

function TdxDefaultRibbonStatusBarBarControlPainter.GetButtonBorderHeight
  (AScaleFactor: TdxScaleFactor): Integer;
begin
  Result := 4;
  if IsCompositionEnabled then
...
function TdxDefaultRibbonStatusBarBarControlPainter.MarkSizeX
  (ABarControl: TdxBarControl): Integer;
begin
  Result := 0;
end;
...
begin
  ...
  if (Ribbon = nil) or not((csDesigning in ComponentState) or
    Bar.AllowQuickCustomizing) then
...
begin
  Result := IdxBarItemControlViewInfo(AItemControl.ViewInfo)
    .GetViewLevelForButtonGroup;
end;
...
begin
  if (Ribbon <> nil) and IsValid then
    Result := Painter.GetButtonHeight(BarManager.ImageOptions.GlyphSize,
...
function TdxRibbonStatusBarBarControl.GetViewInfoClass
  : TCustomdxBarControlViewInfoClass;
begin
  Result := TdxRibbonStatusBarBarControlViewInfo;
end;

procedure TdxRibbonStatusBarBarControl.InitCustomizationPopup
...
begin
  ...
  begin
    ...
    ASubItemButton := BarDesignController.AddInternalItem(AItemLinks,
      TdxRibbonQuickAccessPopupSubItemButton, BarManager, nil)
      .Item as TdxRibbonQuickAccessPopupSubItemButton;
...
class function TdxRibbonStatusBarBarControlDesignHelper.GetForbiddenActions
  : TdxBarCustomizationActions;
begin
  Result := inherited GetForbiddenActions - [caChangeViewLevels,
    caChangeButtonGroup];
end;

{ TdxRibbonStatusBarDockControl }

constructor TdxRibbonStatusBarDockControl.Create
  (AOwner: TdxStatusBarToolbarPanelStyle);
begin
...
procedure TdxRibbonStatusBarDockControl.FillBackground(DC: HDC;
...
begin
  ...
  if ColorScheme = nil then
    ...
    ColorScheme.DrawStatusBarPanel(DC, dxMapWindowRect(StatusBar.Handle, Handle,
      StatusBar.ClientBounds), R, Panel.Bevel <> dxpbRaised);
end;
...
function TdxRibbonStatusBarDockControl.GetDockedBarControlClass
  : TdxBarControlClass;
begin
...
begin
  ...
  begin
    if FPainter = nil then
      FPainter := TdxRibbonStatusBarBarControlPainter.Create
        (TdxNativeUInt(Ribbon));
  end
  else
    FreePainter;
...
begin
  ...
  if (ABarControl <> nil) and ABarControl.CanShowPopupMenuOnMouseClick(False)
  then
...
function TdxRibbonStatusBarDockControl.GetBarControl
  : TdxRibbonStatusBarBarControl;
begin
...
procedure TdxStatusBarToolbarPanelStyle.Assign(Source: TPersistent);
  ...
  begin
    Result := (Owner.Collection <> nil) and
      (csUpdating in StatusBarControl.ComponentState);
  end;
...
begin
  ...
  try
    ...
    begin
      if (TdxStatusBarToolbarPanelStyle(Source).Toolbar <> nil) and IsInheritanceUpdating
      then
        Toolbar := FindBarManager.BarByComponentName
          (TdxStatusBarToolbarPanelStyle(Source).Toolbar.Name)
      else
...
procedure TdxStatusBarToolbarPanelStyle.DoRightToLeftConversion
  (const ABounds: TRect);
begin
  inherited DoRightToLeftConversion(ABounds);
  DockControl.BoundsRect := TdxRightToLeftLayoutConverter.ConvertRect
...
begin
  Result := 50;
  if Owner.Fixed and (Toolbar <> nil) and
    (Toolbar.Control is TdxRibbonStatusBarBarControl) then
  begin
...
begin
  DockControl.Visible := Owner.Visible and not TdxCustomStatusBarAccess
    (Owner.StatusBarControl).SimplePanelStyle.Active;
end;
...
begin
  ...
  if Result = nil then
    raise EdxException.Create
      (cxGetResourceString(@dxSBAR_CANTFINDBARMANAGERFORSTATUSBAR));
end;

procedure TdxStatusBarToolbarPanelStyle.UpdateByRibbon
...
begin
  ...
  begin
    ...
    begin
      Result := ABarControl.GetSize(Panel.Width).cy +
        cxMarginsHeight(ABarControl.Painter.GetToolbarContentOffsets
        (ABarControl.Bar, dsNone, ABarControl.ScaleFactor, False));
    end;
  end;
end;

function TdxStatusBarToolbarPanelViewInfo.GetPanelStyle
  : TdxStatusBarToolbarPanelStyle;
begin
...
class function TdxRibbonStatusBarPainter.ContentExtents
  (AStatusBar: TdxCustomStatusBar): TRect;
begin
  Result := cxNullRect;
end;

class function TdxRibbonStatusBarPainter.GripAreaSize
  (AStatusBar: TdxCustomStatusBar): TSize;
begin
  Result := TdxCustomStatusBarAccess(AStatusBar)
    .ScaleFactor.Apply(cxSize(GRIP_AREA_SIZE));
end;

class function TdxRibbonStatusBarPainter.SeparatorSize
...
class function TdxRibbonStatusBarPainter.SizeGripMargins
  (AStatusBar: TdxCustomStatusBar): TRect;
begin
  Result := Rect(1, 1, 1, 1);
end;
...
begin
  ...
  if ARibbonStatusBar.IsRibbonValid then
    ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarPanel(ACanvas.Handle,
      ARibbonStatusBar.ClientBounds, R, ABevel <> dxpbRaised)
  else
...
begin
  ...
  begin
    ...
    try
      ...
      ARibbonStatusBar.Ribbon.ColorScheme.UseRightToLeftAlignment :=
        ARibbonStatusBar.UseRightToLeftAlignment;
      ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarGripBackground
        (ACanvas.Handle, AGripRect);
      Dec(AGripRect.Bottom, ARibbonStatusBar.ScaleFactor.Apply(3));
      ARibbonStatusBar.Ribbon.ColorScheme.DrawStatusBarSizeGrip(ACanvas.Handle,
        AGripRect);
    finally
...
begin
  ...
  Result := (F <> nil) and not((F.WindowState = wsMaximized) and
    (fsCreating in F.FormState));
end;

function TdxRibbonStatusBarViewInfo.CreatePanelViewInfo
  (APanel: TdxStatusBarPanel): TdxStatusBarPanelViewInfo;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ADockControl := TdxStatusBarToolbarPanelStyle(Panels[I].PanelStyle)
          .DockControl;
        ...
        TdxStatusBarToolbarPanelStyle(Panels[I].PanelStyle)
          .UpdateDockControl(R);
      end;
    end;
  end;
...
function TdxRibbonStatusBar.CanAcceptPanelStyle
...
procedure TdxRibbonStatusBar.AdjustTextColor(var AColor: TColor;
  Active: Boolean);
begin
  if IsRibbonValid then
...
procedure TdxRibbonStatusBar.ColorSchemeChanged(Sender: TObject;
  const AEventArgs);
begin
  PaintStyleChanged;
end;
...
begin
  ...
  begin
    if AComponent = Ribbon then
      Ribbon := nil
    else if AComponent is TdxBar then
...
begin
  ...
  begin
    ...
    begin
      if ScreenToClient(SmallPointToPoint(Message.Pos)).Y > ClientBounds.Bottom
        - GetSystemMetrics(SM_CYFRAME) then
...
GetRegisteredStatusBarPanelStyles.Register(TdxStatusBarToolbarPanelStyle,
  cxSToolbarPanelStyle);
BarDesignController.RegisterBarControlDesignHelper(TdxRibbonStatusBarBarControl,
  TdxRibbonStatusBarBarControlDesignHelper);
