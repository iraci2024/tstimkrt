...
uses
  Types, Variants, Windows, Messages, SysUtils, Classes, Graphics, Controls,
  Forms,
...
procedure RegisterPredefinedStyleSheets(APredefinedStyleSheetsClass
  : TcxPredefinedStyleSheetsClass);
procedure UnregisterPredefinedStyleSheets(APredefinedStyleSheetsClass
  : TcxPredefinedStyleSheetsClass);
procedure GetPredefinedStyleSheetClasses(AList: TList);

procedure ShowLoadStyleSheetsFromIniFile(const AIniFileName: string;
  AStyleRepository: TcxStyleRepository; AOwner: TComponent;
  const AStyleSheetList: TList; AStyleGetName: TcxStyleGetName);

procedure ShowLoadStyleSheetsFromPreDefineStyles(AStyleRepository
  : TcxStyleRepository; AOwner: TComponent; const AStyleSheetList: TList;
...
begin
  ...
  if FPredefinedStyleSheetsList.IndexOf(TObject(APredefinedStyleSheetsClass)
    ) = -1 then
...
procedure TcxPredefinedStyleSheets.GetStyleSheetsByClass(AStyleSheetClass
  : TcxCustomStyleSheetClass; const AList: TList);
var
  I: Integer;
begin
  for I := 0 to FList.Count - 1 do
    if TcxCustomStyleSheetClass(TcxCustomStyleSheet(FList[I]).ClassType) = AStyleSheetClass
    then
      AList.Add(FList[I]);
end;

procedure TcxPredefinedStyleSheets.AddStyleSheet(AStyleSheet
  : TcxCustomStyleSheet);
begin
...
constructor TcxPredefinedStyleSheetsList.Create(AStyleSheetClassComboBox
  : TComboBox; AStyleSheetsListBox: TListBox; ALoadButton: TButton);
var
  I: Integer;
begin
  inherited Create;
  ...
  if FPredefinedStyleSheetsList <> nil then
    for I := 0 to FPredefinedStyleSheetsList.Count - 1 do
      FList.Add(TcxPredefinedStyleSheetsClass(FPredefinedStyleSheetsList
        [I]).Create);
end;
...
function TcxPredefinedStyleSheetsList.CurrentStyleSheetClass
...
begin
  ...
  begin
    ...
    try
      for I := 0 to FList.Count - 1 do
        TcxPredefinedStyleSheets(FList[I]).GetStyleSheetsByClass
          (CurrentStyleSheetClass, AList);
  ...
  begin
    ...
    if AForm.pnlPreviewClient.Width < FPreview.GetSize.X then
      AForm.Width := AForm.Width + FPreview.GetSize.X -
        AForm.pnlPreviewClient.Width;
    if AForm.pnlPreviewClient.Height < FPreview.GetSize.Y then
      AForm.Height := AForm.Height + FPreview.GetSize.Y -
        AForm.pnlPreviewClient.Height;
  end
  else
...
procedure TcxPredefinedStyleSheetsList.StyleSheetClassComboBoxClick
  (Sender: TObject);
begin
  FillListBox;
end;
  ...
  try
    ...
    begin
      ...
      LoadStyleSheetsFromIniFile(AIniFileName, AStyleRepository,
        AForm.CurrentStyleSheetClass, AStrings, AOwner, AStyleSheetList,
        AStyleGetName);
    end;
  finally
    AStrings.Free;
...
begin
  AStyleSheet := AStyleRepository.CreateStyleSheetEx
    (TcxCustomStyleSheetClass(ASource.ClassType), AOwner);
...
begin
  ...
  try
    ...
    begin
      for I := 0 to AForm.lbStyleSheets.Items.Count - 1 do
        if AForm.lbStyleSheets.Selected[I] then
          CreateStyleSheetByPredefine(AStyleRepository, AOwner, AStyleSheetList,
            AStyleGetName,
...
begin
  ...
  try
    ...
    begin
      ...
      cbStyleSheetClasses.Items.AddObject(StyleSheetClass.ClassName,
        TObject(StyleSheetClass));
    end;
