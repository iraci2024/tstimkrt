...
type
  ...
  public
    ...
    procedure Init(const ANumericListSortedInAscendingOrder,
      APopulateNumericListWhenAccumulateResult, AErrorIfNonReferenceToken,
      AIgnoreHiddenRows: Boolean; const AForSubTotal: Boolean = False);
  end;
  ...
  strict private
    ...
    function ConvertNullToDefValue(const ADefValue: Variant;
      var AOutputValue: Variant; ACanConvertStrToNumeric: Boolean): Boolean;
    procedure Initialize(const AInitialResultValue: Variant);
  public
    constructor Create(const AFormatSettings: TdxSpreadSheetFormatSettings;
      const AProcessingInfo: TdxSpreadSheetEnumValuesProcessingInfo); overload;
    constructor Create(const AFormatSettings: TdxSpreadSheetFormatSettings;
      const AInitialResultValue: Variant;
      const AProcessingInfo: TdxSpreadSheetEnumValuesProcessingInfo); overload;
    destructor Destroy; override;
    function AddToNumericList(const AValue: Variant;
      const AValueIsChecked: Boolean): Boolean;
    function CheckValueOnLogical(const AValue: Variant;
      ACanConvertStrToNumber: Boolean; var AOutputValue: Variant): Boolean;
    function CheckValueOnNumeric(const AValue: Variant;
      ACanConvertStrToNumber: Boolean;
      AWithoutSimilarNumericByReference: Boolean;
    ...
    function InsertIntoNumericList(const AIndex: Integer;
      const ANumber: Double): Boolean;
    function IsFinalResult: Boolean;
    function PopulateNumericList(Sender: TdxSpreadSheetFormulaResult;
    ...
    property ProcessingInfo: TdxSpreadSheetEnumValuesProcessingInfo
      read FProcessingInfo;
  ...
  public
    constructor Create(ARange: TdxSpreadSheetFormulaToken;
  ...
  public
    ...
    constructor Create;
    destructor Destroy; override;
    procedure AddAdditionalCondition(AConditionRange
      : TdxSpreadSheetFormulaToken; const AConditionValue: Variant;
...
procedure dxSpreadSheetCalculateForEachParam
  ...
  ACalculateProc: TdxSpreadSheetForEachCallBack);
function dxSpreadSheetExtractedArgumentsAreNotNull
  (Sender: TdxSpreadSheetFormulaResult;
  const AParams: TdxSpreadSheetFormulaToken;
...
uses
  Math, SysUtils, dxSpreadSheetUtils, dxCore, cxDateUtils,
...
begin
  ...
  else if dxIsLogical(AValue) then
...
function dxSpreadSheetExtractedArgumentsAreNotNull
  ...
  var AArgument1, AArgument2: Variant): Boolean;
begin
  Result := not(Sender.ParameterIsNull(AArgument1, AParams) or
    (Sender.ParameterIsNull(AArgument2, AParams.Next)));
  if not Result then
...
function TdxDescendingComparerDoubleF.Compare(const Left,
  Right: Double): Integer;
begin
...
procedure TdxSpreadSheetEnumValuesProcessingInfo.Init
  (const ANumericListSortedInAscendingOrder,
  APopulateNumericListWhenAccumulateResult, AErrorIfNonReferenceToken,
  AIgnoreHiddenRows: Boolean; const AForSubTotal: Boolean = False);
begin
  NumericListSortedInAscendingOrder := ANumericListSortedInAscendingOrder;
  PopulateNumericListWhenAccumulateResult :=
    APopulateNumericListWhenAccumulateResult;
...
constructor TdxSpreadSheetEnumValues.Create(const AFormatSettings
...
constructor TdxSpreadSheetEnumValues.Create(const AFormatSettings
  : TdxSpreadSheetFormatSettings; const AInitialResultValue: Variant;
  const AProcessingInfo: TdxSpreadSheetEnumValuesProcessingInfo);
begin
  inherited Create;
...
function TdxSpreadSheetEnumValues.AddToNumericList(const AValue: Variant;
  const AValueIsChecked: Boolean): Boolean;
begin
...
function TdxSpreadSheetEnumValues.ConvertNullToDefValue(const ADefValue
  : Variant; var AOutputValue: Variant;
...
function TdxSpreadSheetEnumValues.CheckValueOnLogical(const AValue: Variant;
  ACanConvertStrToNumber: Boolean; var AOutputValue: Variant): Boolean;
var
  ABoolean: Boolean;
  ANumeric: Variant;
begin
  ...
  else if dxIsText(AValue) and not ACanConvertStrToNumber then
  begin
    Inc(FStringCount);
    Result := True;
  end
  else if dxIsNumericOrDateTime(AValue) then
  begin
    Inc(FNumericCount);
    Result := True;
  ...
  else if dxIsText(AValue) then
  begin
    Result := (AValue <> '') and ACanConvertStrToNumber and
      dxTryStrToBool(AValue, ABoolean);
    if Result then
    begin
      Inc(FNumericCount);
...
function TdxSpreadSheetEnumValues.CheckValueOnNumeric(const AValue: Variant;
  ACanConvertStrToNumber: Boolean; AWithoutSimilarNumericByReference: Boolean;
  var AOutputValue: Variant): Boolean;
...
begin
  ...
  else if not ACanConvertStrToNumber and AWithoutSimilarNumericByReference then
  begin
  ...
  begin
    ...
    if dxIsText(AValue) and (AValue <> '') and ACanConvertStrToNumber and
      (dxTryStrToOrdinal(AValue, ANumeric, FormatSettings) or
      dxConvertToXLSDate(AValue, ADate)) then
    begin
      Inc(FNumericCount);
...
function TdxSpreadSheetEnumValues.DeleteFromNumericList
...
procedure TdxSpreadSheetEnumValues.Initialize(const AInitialResultValue
  : Variant);
begin
  FIndex := 0;
  FCount := 0;
...
procedure TdxSpreadSheetEnumValues.IncrementPassedCellCount(const AIncrement
  : Integer);
begin
...
function TdxSpreadSheetEnumValues.InsertIntoNumericList(const AIndex: Integer;
  const ANumber: Double): Boolean;
begin
  Result := (AIndex >= 0) and (AIndex < NumericList.Count - 1);
  if Result then
...
function dxAddToNumericList(const AValue: Variant;
  ACanConvertStrToNumber: Boolean;
...
begin
  ...
  if Result then
    if AData.CheckValueOnNumeric(AValue, ACanConvertStrToNumber, True, ANumeric)
    then
...
function TdxSpreadSheetEnumValues.PopulateNumericList
  (Sender: TdxSpreadSheetFormulaResult;
  const AParams: TdxSpreadSheetFormulaToken;
  const AProcessAllParams: Boolean = False): TdxSpreadSheetFormulaErrorCode;
var
...
procedure TdxSpreadSheetEnumValues.SetErrorCode
...
constructor TdxSpreadSheetAdditionalCondition.Create
  (ARange: TdxSpreadSheetFormulaToken; const AValue: Variant;
  const AOperation: TdxSpreadSheetFormulaOperation);
begin
  inherited Create;
  FRange := ARange;
...
procedure TdxSpreadSheetEnumValuesWithCondition.AddAdditionalCondition
  (AConditionRange: TdxSpreadSheetFormulaToken; const AConditionValue: Variant;
  const AOperation: TdxSpreadSheetFormulaOperation);
begin
  AdditionalConditions.Add(TdxSpreadSheetAdditionalCondition.Create
    (AConditionRange, AConditionValue, AOperation));
end;
