...
type
  ...
  private
    class function CheckPointOutside(const P: TPoint; const R: TRect)
      : TcxBorders;
    class function PartLengthQSpline(P: PdxFCPointArray; I, ACount: Integer;
      ACenter: PPoint): Integer;
    class function PartLengthRect(P: PdxFCPointArray; I, ACount: Integer;
      ACenter: PPoint; AHorizontal: Boolean): Integer;
    class function PartLengthStraight(P: PdxFCPointArray; I, ACount: Integer;
      ACenter: PPoint): Integer;
  ...
  public
    ...
    class function IsPointOnLine(ALineType: TLineType; APoints: PdxFCPointArray;
      ACount, ASigma: Integer; const P: TPoint): Boolean;
    class function IsRectOnLine(ALineType: TLineType; APoints: PdxFCPointArray;
      ACount: Integer; const R: TRect): Boolean;
    class function LineCenter(ALineType: TLineType; APoints: PdxFCPointArray;
      ACount: Integer): TPoint;
    class function PartDistance(const PS, PE, Src: TPoint): Integer;
    class function PartLength(ALineType: TLineType; APoints: PdxFCPointArray;
      I, ACount: Integer; ACenter: PPoint): Integer;
    class procedure QSpline(APoints: PdxFCPointArray; ACount: Integer;
      out ASplinePoints: TPoints);
  end;
...
function PtOnLine(LType: TLineType; var Points;
  Count, Delta, X, Y: Integer): Boolean;
function QDistance(X, Y: Integer; const P: TPoint): Integer;
function RectOnLine(LType: TLineType; var Points; Count: Integer;
...
procedure CalculateRectLine(var APoints; ACount: Integer; AVertical: Boolean;
  out P: TPoints);
var
  ASrc: PPoint;
  J: Integer;
...
function PtOnLine(LType: TLineType; var Points;
  Count, Delta, X, Y: Integer): Boolean;
begin
  Result := TdxFcLinesHelper.IsPointOnLine(LType, @Points, Count, Delta,
    Point(X, Y));
end;
...
class function TdxFcLinesHelper.CheckPointOutside(const P: TPoint;
  const R: TRect): TcxBorders;
begin
  Result := [];
  if P.X < R.Left then
...
class function TdxFcLinesHelper.LineCenter(ALineType: TLineType;
  APoints: PdxFCPointArray; ACount: Integer): TPoint;

  function PartialLength(AOldLength, AIndex: Integer;
    var ACenter: TPoint): Integer;
  var
    ANewLength: Integer;
  begin
    ...
    if (Result + AOldLength >= ANewLength) and
      (ANewLength + Result >= AOldLength) then
    begin
...
class function TdxFcLinesHelper.IsLineInRect(P1, P2: TPoint;
  const R: TRect): Boolean;
...
begin
  ...
  repeat
    ...
    Result := (B1 = []) or (B2 = []) or (B1 + B2 = [bLeft, bRight]) or
      (B1 + B2 = [bTop, bBottom]);
    if Result then
      Exit;

    if bLeft in B1 then
      RecalculateY(P1, R.Left)
    else if bTop in B1 then
      RecalculateX(P1, R.Top)
    else if bRight in B1 then
      RecalculateY(P1, R.Right)
    else if bBottom in B1 then
      RecalculateX(P1, R.Bottom)
    else
      Break;
  until False;
end;
      ...
      case ALineType of
        ...
        liQSpline:
          begin
            ...
            QSplineCalcPart(P0, P1, P2, False, ASplinePoints,
              ASplinePointsCount);
            Result := IsPointOnLine(liStraight, @ASplinePoints[0],
              ASplinePointsCount, ASigma, P);
    ...
    case ALineType of
      ...
      liQSpline:
        begin
          ...
          Result := IsRectOnLine(liStraight, @ASplinePoints[0],
            ASplinePointsCount, R);
          if LastIndex >= ASplinePointsCount then
            Inc(ASaveLastIndex);
          LastIndex := ASaveLastIndex;
        end;
...
class function TdxFcLinesHelper.PartDistance(const PS, PE, Src: TPoint)
  : Integer;
begin
...
begin
  case ALineType of
    ...
    liRectH, liRectV:
      Result := PartLengthRect(APoints, I, ACount, ACenter,
        ALineType = liRectH);
  else
    Result := 0;
  end;
...
class function TdxFcLinesHelper.PartLengthQSpline(P: PdxFCPointArray;
  I, ACount: Integer; ACenter: PPoint): Integer;
...
class function TdxFcLinesHelper.PartLengthStraight(P: PdxFCPointArray;
  I, ACount: Integer; ACenter: PPoint): Integer;
var
  X, Y: Integer;
begin
...
class procedure TdxFcLinesHelper.QSpline(APoints: PdxFCPointArray;
  ACount: Integer; out ASplinePoints: TPoints);
var
  I, AIndex: Integer;
  P0, P1, P2: TPoint;
begin
...
class procedure TdxFcLinesHelper.QSplineCalcPart(var P0, P1, P2: TPoint;
  AIsLast: Boolean; var ASplinePoints: TPoints;
  var ASplinePointsCount: Integer);
  ...
  function Initialize(var P0, P1, P2, DPx2, Px2: TPoint;
    out AStep: Integer): Boolean;
  var
    ALength, D: Integer;
  begin
...
function TdxFcPoints.IsPointOnLine(ALineType: TLineType;
  const P: TPoint): Boolean;
begin
  Result := PtOnLine(ALineType, FList^, Count, Screen.Width shr 8, P.X, P.Y);
end;

function TdxFcPoints.IsRectOnLine(ALineType: TLineType; const R: TRect)
  : Boolean;
begin
