...
uses
  ...
  Dialogs, cxStyles, cxCustomData, cxFilter,
  cxGraphics, cxData, cxDataStorage, cxEdit, cxImageComboBox, cxLookAndFeels,
  cxGridCustomTableView, cxGridTableView,
  cxControls, cxGridCustomView, cxClasses, cxGridLevel, cxGrid, Menus, StdCtrls,
  ExtCtrls, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxContainer, cxGroupBox, cxLookAndFeelPainters, cxButtons,
  cxCalendar, ImgList, cxNavigator,
...
type
  TfmImageComboBoxItemsEditor = class(TdxForm)
    ...
    procedure tvImageComboBoxItemsKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure tvImageComboBoxItemsDataControllerRecordChanged(ADataController
      : TcxCustomDataController; ARecordIndex, AItemIndex: Integer);
...
var
  cxImageComboVarTypeNames: array [varEmpty .. varInt64] of string = (
    'Unassigned', // varEmpty
    'Null', // varNull
...
begin
  TcxAccessImageComboBoxProperties(clnImage.Properties)
    .ShowImageIndexInsteadDescription := True;
...
begin
  ...
  for I := 0 to tvImageComboBoxItems.ViewData.RecordCount - 1 do
    InitImageComboBoxItem(FImageComboBoxItems.Add as TcxImageComboBoxItem,
...
procedure TfmImageComboBoxItemsEditor.FillGridViewRecords(AImageComboBoxItems
...
var
  I: Integer;
begin
  tvImageComboBoxItems.DataController.RecordCount := AImageComboBoxItems.Count;
  if tvImageComboBoxItems.DataController.RecordCount > 0 then
  begin
    tvImageComboBoxItems.BeginUpdate;
    try
...
procedure TfmImageComboBoxItemsEditor.FocusAndSelectRecord
  (ARecordIndex: Integer);
begin
  tvImageComboBoxItems.DataController.ClearSelection;
  tvImageComboBoxItems.DataController.FocusedRecordIndex := ARecordIndex;
  tvImageComboBoxItems.Controller.FocusedRecord.Selected := True;
end;
...
function TfmImageComboBoxItemsEditor.GetVarTypeByName
  (AVarTypeName: string): Word;
var
  I: Integer;
begin
  Result := varError;
...
begin
  ...
  try
    ...
    if ARecordIndex < TcxImageComboBoxProperties(clnImage.Properties).Items.Count
    then
      tvImageComboBoxItems.DataController.Values[ARecordIndex, clnImage.Index]
        := ARecordIndex
    else
      tvImageComboBoxItems.DataController.Values[ARecordIndex,
        clnImage.Index] := -1;
  finally
    tvImageComboBoxItems.EndUpdate;
  end;
...
procedure TfmImageComboBoxItemsEditor.PopulateImages(AImages,
  ALargeImages: TCustomImageList);
var
  I: Integer;
...
procedure TfmImageComboBoxItemsEditor.PopulateValueTypeNames;
  ...
  procedure AddValueTypeToDropDownMenu(const AValueTypeName: string;
    AValue: Integer);
  var
    AMenuItem: TMenuItem;
  begin
  ...
  procedure AddValueTypeToValueTypeColumn(const AValueTypeName: string;
    AValue: Integer);
  var
    AComboBoxProperties: TcxComboBoxProperties;
  begin
    AComboBoxProperties := TcxComboBoxProperties(clnValueType.Properties);
...
procedure TfmImageComboBoxItemsEditor.SetItemValueType(ARecordIndex: Integer;
  AValueType: Word);
var
  AValue: Variant;
begin
  ...
  try
    ...
    AValue := tvImageComboBoxItems.DataController.Values[ARecordIndex,
      clnValue.Index];
    ValueToValueType(AValue, AValueType);
    tvImageComboBoxItems.DataController.Values[ARecordIndex, clnValue.Index]
      := AValue;
  finally
    tvImageComboBoxItems.EndUpdate;
  end;
...
begin
  ...
  try
    for I := 0 to tvImageComboBoxItems.Controller.SelectedRecordCount - 1 do
      SetItemValueType(tvImageComboBoxItems.Controller.SelectedRecords[I]
        .RecordIndex, AValueType);
  finally
    tvImageComboBoxItems.EndUpdate;
  end;
end;

function TfmImageComboBoxItemsEditor.ValueToValueType(var AValue: Variant;
  AValueType: Word): Boolean;
var
  AStringValue: string;
begin
  Result := True;
  case AValueType of
    ...
    if Result then
      try
        ...
        Result := (AValueType in [varDate, varCurrency, varDouble, varBoolean])
          or (VarToStr(AValue) = AStringValue);
      except
        Result := False;
        VarClear(AValue);
        VarCast(AValue, AValue, AValueType);
...
procedure TfmImageComboBoxItemsEditor.
  tvImageComboBoxItemsDataControllerRecordChanged(ADataController
  : TcxCustomDataController; ARecordIndex, AItemIndex: Integer);

  function GetFocusedItemValueType: Word;
  begin
    Result := GetVarTypeByName(ADataController.GetValue(ARecordIndex,
      clnValueType.Index));
  end;
  ...
  begin
    ...
    try
      ...
      if VarIsSoftNull(AValue) then
        ADataController.SetValue(ARecordIndex, clnValueType.Index,
          GetVarTypeName(varEmpty))
      else if VarEquals(AValue, cxImageComboNullValue) then
        ADataController.SetValue(ARecordIndex, clnValueType.Index,
          GetVarTypeName(varNull))
      else if not ValueToValueType(AValue, GetFocusedItemValueType) then
        ADataController.SetValue(ARecordIndex, clnValueType.Index,
          cxImageComboStringVarTypeName)
      else
        ADataController.SetValue(ARecordIndex, clnValue.Index,
          VarToStr(AValue));
    finally
      ADataController.OnRecordChanged :=
        tvImageComboBoxItemsDataControllerRecordChanged;
    end;
  end;
end;
