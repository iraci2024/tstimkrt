...
type
  ...
  protected
    function DoDrawCaptionButton(ACanvas: TcxCanvas; AGlyphIndex: Integer;
    ...
    function GetCaptionRect(const ARect: TRect; AIsVertical: Boolean)
      : TRect; override;
    function GetElementState(AActive: Boolean; AState: TcxButtonState)
      : TdxSkinElementState;
  ...
  public
    ...
    procedure DrawHideBar(ACanvas: TcxCanvas; ARect: TRect;
      APosition: TdxAutoHidePosition); override;
  ...
  protected
    ...
    procedure PrepareTabBitmapBackground(ABitmap: TcxBitmap; const ARect: TRect;
      ATabViewInfo: TcxTabViewInfo); override;
...
begin
  if cxColorIsValid(ADefaultTextColor) and (ADefaultTextColor <> clWindowText)
  then
    Result := ADefaultTextColor
  else if cxColorIsValid(ASkinTextColor) then
...
procedure TdxDockControlSkinTabPainter.DoDrawTabButton(ACanvas: TcxCanvas;
...
procedure TdxDockControlSkinTabPainter.DoDrawTabCloseButton(ACanvas: TcxCanvas;
  const ARect: TRect; AState: TcxPCNavigatorButtonState);
begin
...
procedure TdxDockControlSkinTabPainter.DrawHeaderLine(ACanvas: TcxCanvas;
  const R: TRect);
var
  ABitmap: TcxBitmap32;
begin
...
procedure TdxDockControlSkinTabPainter.DrawParentBackground(ACanvas: TcxCanvas;
  const R: TRect);
var
  AInfo: TdxSkinInfo;
begin
  if GetSkinPainterData(AInfo) and (AInfo.DockControlTabHeaderBackground <> nil)
  then
  begin
    ...
    try
      ...
      AInfo.DockControlTabHeaderBackground.Draw(ACanvas.Handle,
        ViewInfo.TabsAreaRect, ScaleFactor);
    finally
...
procedure TdxDockControlSkinTabPainter.FillFreeSpaceArea(ACanvas: TcxCanvas;
  const ARect: TRect);
begin
...
function TdxDockControlSkinTabPainter.GetIndentByIndex
  (AType: TcxPCSkinIndents): Integer;
var
  AInfo: TdxSkinInfo;
begin
  Result := 0;
...
begin
  Result := cxRect(HeaderLineSize.cy, HeaderLineSize.cy, HeaderLineSize.cy,
    HeaderLineSize.cy);
end;
...
function TdxDockControlSkinTabPainter.GetTextColor(ATabVisibleIndex
  : Integer): TColor;
var
  AColor: TdxSkinColor;
  AInfo: TdxSkinInfo;
begin
  ...
  if GetSkinPainterData(AInfo) then
  begin
    AColor := AInfo.DockControlTabTextColor
      [ViewInfo.TabsViewInfo[ATabVisibleIndex].IsMainTab];
    if AColor <> nil then
      Result := AColor.Value;
  end;
end;
...
constructor TdxDockControlSkinPainter.Create(ADockControl
...
procedure TdxDockControlSkinPainter.DrawBorder(ACanvas: TcxCanvas;
  ARect: TRect);
var
...
procedure TdxDockControlSkinPainter.DrawCaption(ACanvas: TcxCanvas;
...
begin
  ...
  begin
    ...
    try
      ...
      AElement.Draw(ACanvas.Handle, cxRectInflate(ARect, 0, GetBorderWidths.Top,
        0, 0), ScaleFactor, 0, States[IsActive]);
    finally
      ACanvas.RestoreClipRegion;
    end;
  end;
...
procedure TdxDockControlSkinPainter.DrawCaptionButton(ACanvas: TcxCanvas;
  ARect: TRect; AIsActive: Boolean; AState: TcxButtonState);
begin
  if not DoDrawCaptionButton(ACanvas, -1, ARect, GetElementState(AIsActive,
    AState)) then
    inherited DrawCaptionButton(ACanvas, ARect, AIsActive, AState);
end;

procedure TdxDockControlSkinPainter.DrawCaptionCloseButton(ACanvas: TcxCanvas;
  ARect: TRect; AIsActive: Boolean; AState: TcxButtonState);
begin
  if not DoDrawCaptionButton(ACanvas, 2, ARect, GetElementState(AIsActive,
    AState)) then
...
begin
  if not DoDrawCaptionButton(ACanvas, GlyphsMap[IsSwitched], ARect,
    GetElementState(IsActive, AState)) then
    inherited DrawCaptionHideButton(ACanvas, ARect, IsActive,
      IsSwitched, AState);
end;

procedure TdxDockControlSkinPainter.DrawCaptionMaximizeButton
  (ACanvas: TcxCanvas; ARect: TRect; IsActive, IsSwitched: Boolean;
...
begin
  if not DoDrawCaptionButton(ACanvas, GlyphsMap[IsSwitched], ARect,
    GetElementState(IsActive, AState)) then
    inherited DrawCaptionMaximizeButton(ACanvas, ARect, IsActive,
      IsSwitched, AState);
end;

procedure TdxDockControlSkinPainter.DrawCaptionSeparator(ACanvas: TcxCanvas;
  ARect: TRect);
var
  ASkinInfo: TdxSkinInfo;
begin
...
procedure TdxDockControlSkinPainter.DrawCaptionText(ACanvas: TcxCanvas;
  ARect: TRect; IsActive: Boolean);
var
  R: TRect;
begin
  ...
  begin
    ...
    begin
      cxDrawImage(ACanvas, R, nil, DockControl.Images, DockControl.ImageIndex,
        True, GetCaptionColorPalette, ScaleFactor);
  ...
  cxDrawText(ACanvas.Handle, DockControl.Caption, ARect, DT_SINGLELINE or
    DT_LEFT or DT_VCENTER or DT_END_ELLIPSIS);
end;

procedure TdxDockControlSkinPainter.DrawClient(ACanvas: TcxCanvas;
  ARect: TRect);
var
  AElement: TdxSkinElement;
  ASkinInfo: TdxSkinInfo;
...
procedure TdxDockControlSkinPainter.DrawDockSiteClient(ACanvas: TcxCanvas;
  ARect: TRect);
var
  AColor: TdxSkinColor;
  ASkinInfo: TdxSkinInfo;
...
begin
  ...
  begin
    ...
    if AButton.MinSize.Width > 0 then
      Result.cx := Max(Result.cx, ScaleFactor.Apply(AButton.MinSize.Width) -
        cxMarginsWidth(GetCaptionButtonBorderWidths));
    if AButton.MinSize.Height > 0 then
      Result.cy := Max(Result.cy, ScaleFactor.Apply(AButton.MinSize.Height) -
        cxMarginsHeight(GetCaptionButtonBorderWidths));
  end
  else
...
begin
  ...
  begin
    ...
    Result := ScaleFactor.Apply(AElement.Size.cy) -
      cxMarginsHeight(GetBorderWidths);
    Inc(Result, Integer(Odd(Result + ACaptionButtonSize.cy)));
    Result := Max(Result, ACaptionButtonSize.cy) +
      cxMarginsHeight(GetCaptionContentOffsets(False));
  end
  else
...
procedure TdxDockControlSkinPainter.DrawHideBar(ACanvas: TcxCanvas;
...
procedure TdxDockControlSkinPainter.DrawHideBarButtonBackground
  (ACanvas: TcxCanvas; AButton: TdxDockSiteHideBarButton;
...
procedure TdxDockControlSkinPainter.DrawHideBarButtonText(ACanvas: TcxCanvas;
  ...
  procedure DrawButtonText(ACanvas: TcxCanvas; R: TRect;
    AControl: TdxCustomDockControl);
  begin
    ACanvas.Brush.Style := bsClear;
    ACanvas.Font := GetHideBarButtonFont;
    ACanvas.Font.Color := GetHideBarButtonFontColorEx
      (IsWorkingControl(AControl));
    cxDrawText(ACanvas.Handle, AControl.Caption, R, DT_LEFT or DT_SINGLELINE or
...
begin
  if GetSkinPainterData(ASkinInfo) and
    (ASkinInfo.DockControlHideBarButtons <> nil) then
  begin
    ABitmap := CreateHideBarButtonBuffer(AButtonSection.Bounds);
    try
      RotateBitmap(ABitmap, False);
      cxBitBlt(ABitmap.Canvas.Handle, ACanvas.Handle, ABitmap.ClientRect,
        AButtonSection.Bounds.TopLeft, SRCCOPY);
      RotateBitmap(ABitmap, True);
      ASkinInfo.DockControlHideBarButtons.Draw(ABitmap.Canvas.Handle,
        ABitmap.ClientRect, ScaleFactor,
        Integer(IsWorkingControl(AButtonSection.DockControl)));
      RotateBitmap(ABitmap, False);
      cxBitBlt(ACanvas.Handle, ABitmap.Canvas.Handle, AButtonSection.Bounds,
        cxNullPoint, SRCCOPY);
    finally
      ABitmap.Free;
    end;
...
procedure TdxDockControlSkinPainter.DrawHideBarButtonSectionSeparator
...
begin
  ...
  if AElement = nil then
    ...
    AElement.Draw(ACanvas.Handle, ARect, ScaleFactor, ImageIndexesMap[AArrow],
...
begin
  Result := False;
  if GetSkinPainterData(ASkinInfo) and (ASkinInfo.DockControlCaption <> nil)
  then
  begin
    if ASkinInfo.DockControlCaption.GetPropertyByName(sdxFontBold, AProperty)
    then
      Result := (AProperty is TdxSkinBooleanProperty) and
...
begin
  if AControl.AutoHideHostSite.WorkingControl is TdxTabContainerDockSite then
    Result := AControl = TdxTabContainerDockSite
      (AControl.AutoHideHostSite.WorkingControl).ActiveChild
  else
    Result := AControl = AControl.AutoHideHostSite.WorkingControl;
end;

class function TdxDockControlSkinPainter.GetTabsPainter
  (ATabsStyle: TcxPCStyleID): TcxPCPainterClass;
begin
...
class function TdxDockControlSkinPainter.HasLookAndFeelStyle
  (AStyle: TcxLookAndFeelStyle): Boolean;
begin
  Result := AStyle = lfsSkin;
end;

function TdxDockControlSkinPainter.GetCaptionFontColor
...
begin
  ...
  begin
    ...
    else if ASkinInfo.DockControlCaption <> nil then
...
function TdxDockControlSkinPainter.GetCaptionColorPalette
...
begin
  if GetSkinPainterData(ASkinInfo) and (ASkinInfo.DockControlCaption <> nil)
  then
    Result := ASkinInfo.DockControlCaption.GetGlyphColorPalette
      (ButtonStateToSkinState[AState])
  else
    Result := nil;
end;

function TdxDockControlSkinPainter.GetCaptionContentOffsets
  (AIsVertical: Boolean): TRect;
var
  ASkinInfo: TdxSkinInfo;
begin
  ...
  begin
    Result := ScaleFactor.Apply
      (ASkinInfo.DockControlCaption.ContentOffset.Rect);
...
function TdxDockControlSkinPainter.GetElementState(AActive: Boolean;
  AState: TcxButtonState): TdxSkinElementState;
begin
...
function TdxDockControlSkinPainter.GetHideBarButtonColorPalette
  : IdxColorPalette;
var
  ASkinInfo: TdxSkinInfo;
begin
  if GetSkinPainterData(ASkinInfo) and (ASkinInfo.DockControlHideBar <> nil)
  then
...
function TdxDockControlSkinPainter.GetHideBarButtonFontColorEx
...
begin
  ...
  begin
    ...
    if AGlyphIndex >= 0 then
      AGlyph.Draw(ACanvas.Handle, cxRectContent(ARect,
        AElement.ContentOffset.Rect), ScaleFactor, AGlyphIndex, AState);
  end;
end;
