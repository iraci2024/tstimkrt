...
begin
  FProc := Proc;
  Designer.GetComponentNames(GetTypeData(TcxCustomBarEditItem.ClassInfo),
    GetBarEditItem);
end;
...
begin
  ...
  begin
    ...
    if (ATempProperties.ClassType <> AProperties.ClassType) or
      (esfBlobEditValue in ATempProperties.GetSpecialFeatures <> ABlobEditValue)
    then
    begin
      Result := False;
      Break;
...
begin
  Result := CanShowDialog and
    (esfBlobEditValue in TcxCustomBarEditItem(GetComponent(0))
    .GetProperties.GetSpecialFeatures);
end;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(string), TcxCustomBarEditItem,
    'PropertiesClassName', nil);
  RegisterPropertyEditor(TypeInfo(TNotifyEvent), TcxCustomBarEditItem,
    'PropertiesEvents', TcxBarEditItemPropertiesEventsProperty);
