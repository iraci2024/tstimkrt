...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Variants, Windows, Classes, ComCtrls, Controls, Dialogs, ExtCtrls, Forms,
  Graphics,
...
type
  TcxHintStyleEditor = class(TForm)
    ...
    procedure cxClbHintFontStylesClickCheck(Sender: TObject; AIndex: Integer;
  ...
  with AHintStyleEditor do
    try
      ...
      if AHintStyleController.Owner <> nil then
        ...
        AOwnerCaption := '';
      Caption := Format('%s%s - Hint Editor',
        [AOwnerCaption, AHintStyleController.Name]);
...
begin
  ...
  cxClbHintCapFontStyles.EditValue :=
    StyleToEditValue(HintStyle.CaptionFont.Style);
  ...
  case Ord(HintStyle.IconSize) of
    0:
      cxRbHIDef.Checked := True;
    1:
      cxRbHILarge.Checked := True;
    2:
      cxRbHISmall.Checked := True;
  end;
  cxLbHfSize.ItemIndex := cxLbHfSize.Items.IndexOf
    (IntToStr(HintStyle.Font.Size));
  cxLbHcfSize.ItemIndex := cxLbHcfSize.Items.IndexOf
    (IntToStr(HintStyle.CaptionFont.Size));
End;

procedure TcxHintStyleEditor.cxSeHintHidePausePropertiesChange(Sender: TObject);
begin
  cxHsc.HintHidePause := cxSeHintHidePause.Value;
  RefreshHint;
end;

procedure TcxHintStyleEditor.cxSeHintPausePropertiesChange(Sender: TObject);
begin
...
procedure TcxHintStyleEditor.cxFnHinCapPropertiesChange(Sender: TObject);
begin
...
begin
  ...
  if fFormLoaded then
  begin
    fHintHorz := pnlPreview.ClientOrigin.X +
      (pnlPreview.Width div 2 - cxHsc.GetHintWidth(SHintText) div 2);
    fHintVert := pnlPreview.ClientOrigin.Y +
      (pnlPreview.Height div 2 - cxHsc.GetHintHeight(SHintText) div 2);
    RefreshHint;
  end;
end;
...
procedure TcxHintStyleEditor.cxClbHintFontStylesClickCheck(Sender: TObject;
  ...
  const
    AFontStylesByIndex: array [0 .. 3] of TFontStyle = (fsBold, fsItalic,
