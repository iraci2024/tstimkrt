...
uses
  Types, Windows, Classes, ImgList, Controls, SysUtils, Graphics, Messages,
  dxPSGlbl,
...
type
  ...
  public
    ...
    procedure Abort;
    function BeginDoc: Integer;
    { returns the print job identifier for the document }
    procedure EndDoc;
    procedure NewPage;
    ...
    property IsDeviceModePersistent: Boolean read FIsDeviceModePersistent
      write FIsDeviceModePersistent;
    ...
    property Orientation: TdxPrinterOrientation read GetOrientation
    ...
    property OnPrinterChange: TNotifyEvent read FOnPrinterChange
      write FOnPrinterChange;
...
procedure dxDrawPrinter(ACanvas: TcxCanvas; ARect: TRect; const AText: string;
  AImageList: TCustomImageList; AImageIndex: Integer;
  ADrawBackground: Boolean = True; AScaleFactor: TdxScaleFactor = nil);
...
const
  ...
  DuplexModeMap: array [DMDUP_SIMPLEX .. DMDUP_HORIZONTAL] of TdxDuplexMode =
    (dmSimplex, dmHorizontal, dmVertical);
  OrientationMap: array [DMORIENT_PORTRAIT .. DMORIENT_LANDSCAPE]
    of TdxPrinterOrientation = (poPortrait, poLandscape);
  PrinterOrientationMap: array [TdxPrinterOrientation] of Integer =
    (DMORIENT_PORTRAIT, DMORIENT_LANDSCAPE, DMORIENT_PORTRAIT);
...
function DocumentProperties(HWND: HWND; hPrinter: THandle; pDeviceName: PChar;
  ...
  external winspl name 'DocumentPropertiesW';
      ...
      try
        ...
        try
          with dxPrintDevice do
            Result := IDOK = DocumentProperties(AParentWnd, FDeviceHandle,
              CurrentDevice, PNewDevMode, DeviceMode, DM_IN_PROMPT or
...
begin
  Result := True;
  try
    Result := (dxPrintDevice.Printers.Count > 0) and (dxPrintDevice.Handle <> 0)
      and (dxPrintDevice.DeviceMode <> nil);
  except
    Result := False;
...
begin
  ...
  cxDrawImage(ACanvas, cxRectOffset(cxRectSetSize(ARect, AImageList.Width,
    AImageList.Height), AOffset, AOffset), nil, AImageList, AImageIndex, True,
    nil, AScaleFactor);
...
begin
  Result := (StrIComp(Device, ADevice) = 0) and
    ((Port = nil) or (APort = nil) or (StrIComp(Port, APort) = 0));
end;
  ...
  try
    ...
    try
      ...
      if FHDeviceMode <> 0 then
        AHDeviceMode := CopyDeviceMode(FHDeviceMode);
      try
        ClosePrintDevice;
        FreeAndNilPrinters;
        GetPrinters;
        ...
        begin
          ...
          else if Printers.Count > 0 then
            GetPrinterIndex;
        end;
      finally
        if AHDeviceMode <> 0 then
          GlobalFree(AHDeviceMode);
      end;
    finally
      StrDispose(APort);
    end;
...
begin
  ...
  begin
    for Result := 0 to Papers.Count - 1 do
    begin
      if dxPSUtl.ArePointsEqual(TdxPaperInfo(Papers.Objects[Result]).Size, ASize)
      then
        Exit;
    end;
...
begin
  Result := 1 = WinSpool.DeviceCapabilities(CurrentDevice, CurrentPort,
    DC_COLORDEVICE, nil, nil);
end;
  ...
  if FBins = nil then
    try
      ...
      begin
        ...
        BinCount := DeviceCapabilities(CurrentDevice, CurrentPort, DC_BINS,
          nil, nil);
        if BinCount > 0 then
        begin
          BinValues := AllocMem(BinSize * BinCount * SizeOf(Char));
          try
            if DeviceCapabilities(CurrentDevice, CurrentPort, DC_BINS,
              PChar(BinValues), nil) <> -1 then
            begin
              BinNames := AllocMem(BinNameSize * BinCount * SizeOf(Char));
              try
                if DeviceCapabilities(CurrentDevice, CurrentPort, DC_BINNAMES,
                  PChar(BinNames), nil) <> -1 then
                begin
      ...
      begin
        FBins := TStringList.Create;
        FBins.AddObject(cxGetResourceString(@sdxDefaultTray),
          TObject(DMBIN_USER));
      end;
    except
      FreeAndNil(FBins);
...
begin
  if FCanvas = nil then
    FCanvas := TdxPrintDeviceCanvas.Create(Self);
  Result := FCanvas;
end;

function TdxPrintDevice.GetCollate: Boolean;
const
  CollationMap: array [DMCOLLATE_FALSE .. DMCOLLATE_TRUE] of Boolean =
    (False, True);
begin
  Result := False;
  if Printers.Count > 0 then
...
begin
  ...
  begin
    GetPrinterIndex;
    if (FDeviceMode <> nil) and (FDeviceMode^.dmOrientation = DMORIENT_LANDSCAPE)
    then
      Result := poLandscape;
  end;
end;
  ...
  if FPapers = nil then
    try
      ...
      begin
        ...
        PaperCount := DeviceCapabilities(CurrentDevice, CurrentPort,
          DC_PAPERNAMES, nil, nil);
        if PaperCount > 0 then
        begin
          PaperNames := AllocMem(PaperNameSize * PaperCount * SizeOf(Char));
          try
            if DeviceCapabilities(CurrentDevice, CurrentPort, DC_PAPERNAMES,
              PChar(PaperNames), nil) <> -1 then
            begin
              PaperValues := AllocMem(PaperValueSize * PaperCount);
              try
                if DeviceCapabilities(CurrentDevice, CurrentPort, DC_PAPERS,
                  PChar(PaperValues), nil) <> -1 then
                begin
                  PaperSizes := AllocMem(PaperSizeSize * PaperCount);
                  try
                    if DeviceCapabilities(CurrentDevice, CurrentPort,
                      DC_PAPERSIZE, PChar(PaperSizes), nil) <> -1 then
                    begin
                      ...
                      if Pos(cxGetResourceString(@sdxCustom),
                        FPapers[PaperCount - 1]) = 0 then
                      begin
                        Paper := TdxPaperInfo.Create(Self);
                        with Paper do
                        begin
      ...
      begin
        FPapers := TStringList.Create;
        Paper := TdxPaperInfo.Create(Self);
        Paper.FSize := Point(2100, 2970); { A4 }
        // TODO: use default locale paper
...
const
  PhysicalOffsets: array [0 .. 1] of Integer = (PHYSICALOFFSETX,
    PHYSICALOFFSETY);
begin
  if Printers.Count > 0 then
  begin
...
function TdxPrintDevice.GetPrinters: TStrings;
  ...
  begin
    ...
    begin
      PrintDeviceInfo := TdxPrintDeviceInfo.Create('', pPrinterName, '',
        dxIsDefaultPrinter(pPrinterName), IsNetworkDevice(Attributes));
  ...
  begin
    ...
    begin
      ...
      while PortName^ <> #0 do
      begin
        PrintDeviceTitle := Format(cxGetResourceString(@sdxDeviceOnPort),
          [pPrinterName, PortName]);
        PrintDeviceInfo := TdxPrintDeviceInfo.Create(nil, pPrinterName,
          PortName, dxIsDefaultPrinter(pPrinterName),
          IsNetworkDevice(Attributes));
...
const
  Flags: array [Boolean] of DWORD = (PRINTER_ENUM_LOCAL,
    PRINTER_ENUM_CONNECTIONS or PRINTER_ENUM_LOCAL);
...
begin
  if (FDeviceMode <> nil) and (Bins <> nil) and (Value > -1) and
    (Value < Bins.Count) then
  begin
...
const
  DuplexModeMap: array [TdxDuplexMode] of UINT = (DMDUP_SIMPLEX, DMDUP_VERTICAL,
    DMDUP_HORIZONTAL);
begin
  // CheckPrinting(False);
  if Printers.Count > 0 then
...
begin
  if (FDeviceMode <> nil) and (Papers <> nil) and (Value > -1) and
    (Value < Papers.Count) then
  begin
    ...
    if Value = Papers.Count - 1 then
      FDeviceMode^.dmFields := FDeviceMode^.dmFields or DM_PAPERWIDTH or
        DM_PAPERLENGTH;
    FDeviceModeChanged := True;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if PaperSize.X < MinSize.X then
        MinSize.X := PaperSize.X;
      if PaperSize.X > MaxSize.X then
        MaxSize.X := PaperSize.X;
      if PaperSize.Y < MinSize.Y then
        MinSize.Y := PaperSize.Y;
      if PaperSize.Y > MaxSize.Y then
        MaxSize.Y := PaperSize.Y;
    end;
...
procedure TdxPrintDevice.InternalSelectPaperBySize(var AWidth,
  AHeight: Integer);
var
  DeviceMode: TDeviceMode;
begin
  if AWidth > MaxExtentX then
    AWidth := MaxExtentX;
  if AWidth < MinExtentX then
    AWidth := MinExtentX;
  if AHeight > MaxExtentY then
    AHeight := MaxExtentY;
  if AHeight < MinExtentY then
    AHeight := MinExtentY;
...
const
  dmFields: UINT = DM_ORIENTATION or DM_PAPERSIZE or DM_PAPERLENGTH or
    DM_PAPERWIDTH or DM_SCALE or DM_COPIES or DM_DEFAULTSOURCE or
    DM_PRINTQUALITY or DM_COLOR or DM_DUPLEX or DM_YRESOLUTION or DM_TTOPTION or
    DM_COLLATE or DM_FORMNAME or DM_LOGPIXELS or DM_BITSPERPEL or
...
begin
  ...
  try
    ...
    begin
      MemSize := DocumentProperties(0, FDeviceHandle, FCurrentDevice,
        nil, nil, 0);
      if MemSize <= 0 then
      begin
        ClosePrintDevice;
      ...
      if IDOK <> DocumentProperties(0, FDeviceHandle, FCurrentDevice,
        FDeviceMode, nil, DM_OUT_BUFFER) then
      begin
        ClosePrintDevice;
        Exit;
      end;
      ...
      FMaxCopies := DeviceCapabilities(FCurrentDevice, FCurrentPort, DC_COPIES,
        nil, nil);
      FMaxExtents := DeviceCapabilities(FCurrentDevice, FCurrentPort,
        DC_MAXEXTENT, nil, nil);
      FMinExtents := DeviceCapabilities(FCurrentDevice, FCurrentPort,
        DC_MINEXTENT, nil, nil);
      FixMinMaxExtents;
...
begin
  ...
  begin
    ...
    if Assigned(ACreateHandleFunc) and (PrinterIndex >= 0) and
      (PrinterIndex < PrinterCount) then
...
procedure TdxPrintDevice.WndProc(var Message: TMessage);
  ...
  var
    Msg: TMsg;
  begin
    Result := PeekMessage(Msg, FWindowHandle, DXM_PS_PRINTERLISTCHANGED,
      DXM_PS_PRINTERLISTCHANGED, RemoveMap[ARemove]);
  end;
