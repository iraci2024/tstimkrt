...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Classes, Graphics, Controls, StdCtrls, ExtCtrls, ComCtrls,
  dxCore,
  dxPSCore, dxPrnPg, cxDrawTextUtils, dxPSGlbl, dxExtCtrls, ImgList, cxPC,
  cxControls,
  cxContainer, cxEdit, cxLabel, cxTextEdit, cxMaskEdit, cxSpinEdit, cxGraphics,
  cxDropDownEdit, Menus, cxLookAndFeelPainters, cxButtons,
...
type
  ...
  TdxTextReportLinkCustomDrawRowEvent = procedure
    (Sender: TdxCustomTextReportLink; ACanvas: TCanvas;
    AnItem: TdxReportCellString; var ADone: Boolean) of object;

  TdxTextReportLinkInitializeRowEvent = procedure
    (Sender: TdxCustomTextReportLink; AnItem: TdxReportCellString) of object;
  ...
  protected
    ...
    function CreateItem(AParent: TdxReportCell; const ATextRow: TcxTextRow)
    ...
    procedure InitializeItem(AItem: TdxReportCellString;
    ...
    procedure DoCustomDrawRow(ACanvas: TCanvas; AnItem: TdxReportCellString;
  ...
  public
    ...
    property Alignment: TcxTextAlignX read FAlignment write SetAlignment
      default taLeft;
    property Color;
    ...
    property TextRowSpacing: Double read FTextRowSpacing write SetTextRowSpacing
      stored IsTextRowSpacingStored;
    property Transparent;
    property UseHorzDelimiters;

    property OnCustomDrawRow: TdxTextReportLinkCustomDrawRowEvent
      read FOnCustomDrawRow write SetOnCustomDrawRow;
    property OnInitializeRow: TdxTextReportLinkInitializeRowEvent
      read FOnInitializeRow write FOnInitializeRow;
  end;
...
function TdxCustomTextReportLink.GetRebuildOnPageParamsChange
...
function TdxCustomTextReportLink.IsSupportedCustomDraw
...
function TdxCustomTextReportLink.CreateHost(AReportCells: TdxReportCells)
  : TdxReportCell;
begin
...
procedure TdxCustomTextReportLink.DoConstructReport(AReportCells
  : TdxReportCells);
var
  Host: TdxReportCell;
  R: TRect;
...
procedure TdxCustomTextReportLink.PrepareConstruct(AReportCells
  : TdxReportCells);
begin
...
procedure TdxCustomTextReportLink.UnprepareConstruct(AReportCells
  : TdxReportCells);
begin
...
begin
  Result := CXTO_WORDBREAK or CXTO_EXPANDTABS or cxDrawTextUtils.cxMakeFormat
    (Alignment, taTop);
end;
...
begin
  if Assigned(FOnInitializeRow) then
    FOnInitializeRow(Self, AnItem);
end;
...
function TdxCustomTextReportLink.GetDesignWindow
...
procedure TdxCustomTextReportLink.SetOnCustomDrawRow
  (Value: TdxTextReportLinkCustomDrawRowEvent);
begin
...
begin
  ...
  begin
    ...
    try
      ...
      Items.AddObject(cxGetResourceString(@sdxTextAlignCenter),
        TObject(taCenterX));
      Items.AddObject(cxGetResourceString(@sdxTextAlignRight),
        TObject(taRight));
      Items.AddObject(cxGetResourceString(@sdxTextAlignJustified),
        TObject(taDistributeX));
    finally
      Items.EndUpdate;
    end;
  end;
...
var
  ...
  R: TRect;
begin
  Format := CXTO_PATTERNEDTEXT + CXTO_WORDBREAK + CXTO_CHARBREAK +
    cxMakeFormat(ReportLink.Alignment, taTop);
  with TPaintBox(Sender) do
  begin
    dxAssignFont(Canvas.Font, ReportLink.Font, Self.ScaleFactor,
...
begin
  ...
  cxDrawImage(ACanvas, AImageRect, nil, ilAlignments, AIndex, True, nil,
    ScaleFactor);
...
dxPSRegisterReportLink(TdxMemoReportLink, TMemo,
  TdxfmTextReportLinkDesignWindow);

finalization

dxPSUnregisterReportLink(TdxMemoReportLink, TMemo,
  TdxfmTextReportLinkDesignWindow);
dxPSUnregisterReportLink(TdxTextReportLink, nil,
  TdxfmTextReportLinkDesignWindow);
