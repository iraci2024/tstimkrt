...
uses
  ...
  cxLookAndFeelPainters, dxLayoutLookAndFeels, cxClasses,
  dxLayoutContainer, dxLayoutControl, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters, cxContainer, cxEdit,
  cxLabel, cxMaskEdit, cxSpinEdit, cxListBox, cxTextEdit, StdCtrls, cxButtons,
...
type
  TdxRichEditTabsDialogForm = class(TdxRichEditCustomDialogForm)
    ...
    procedure teTabStopPositionPropertiesValidate(Sender: TObject;
  ...
  private
    ...
    function FindTabStopWithClosePosition(ATabFormattingInfo
      : TdxTabFormattingInfo; ANewTabStopPosition: Integer)
      : TdxNullableValue<TdxTabInfo>;
    ...
    procedure RemoveTabStop(AItemIndex: Integer;
  ...
  protected
    ...
    function GetStringRepresentationsOfUnit(AUnitInTwips
      : TdxNullableInteger): string;
    ...
    property IsSetAndClearButtonDisabled: Boolean
      read GetIsSetAndClearButtonDisabled;
    property TabAlignmentType: TdxTabAlignmentType read GetTabAlignmentType
      write SetTabAlignmentType;
    property TabLeaderType: TdxTabLeaderType read GetTabLeaderType
      write SetTabLeaderType;
    property TabsFormOwner: IdxFormOwner read FTabsFormOwner;
    property TabFormattingInfo: TdxTabFormattingInfo read GetTabFormattingInfo
      write SetTabFormattingInfo;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property Controller: TdxTabsFormController read GetController;
    property TabStopPosition: TdxNullableInteger read GetTabStopPosition
      write SetTabStopPosition;
...
begin
  ...
  ACloseTabInfo := FindTabStopWithClosePosition(Controller.TabFormattingInfo,
    ATabStopPosition.Value);
  ...
  AItem := TdxTabInfo.Create(ATabStopPosition.Value, AAlignment, ALeader,
    False, False);
  Controller.TabFormattingInfo.Add(AItem);
  UpdateForm;
end;

procedure TdxRichEditTabsDialogForm.FormCloseQuery(Sender: TObject;
...
procedure TdxRichEditTabsDialogForm.seDefaultTabStopsPropertiesChange
  (Sender: TObject);
var
  AValue: Variant;
begin
...
procedure TdxRichEditTabsDialogForm.teTabStopPositionPropertiesChange
  (Sender: TObject);
begin
  UpdateSetAndClearButtons;
end;

procedure TdxRichEditTabsDialogForm.teTabStopPositionPropertiesValidate
...
begin
  ...
  else if FTabStopPositionMeasurementUnit.CorrectRange(AValue) <> AValue then
  begin
    AMinValue := PositionToText(MinTabStopPositionByDefault);
    AMaxValue := PositionToText(MaxTabStopPositionByDefault);
    ErrorText :=
      Format(cxGetResourceString(@sdxRichEditExceptionInvalidValueRange),
      [AMinValue, AMaxValue]);
    Error := True;
  end;
end;

function TdxRichEditTabsDialogForm.FindTabStopWithClosePosition
  (ATabFormattingInfo: TdxTabFormattingInfo; ANewTabStopPosition: Integer)
  : TdxNullableValue<TdxTabInfo>;
var
  I, ACount: Integer;
  ADelta: Integer;
...
begin
  ...
  else if rbRight.Checked then
    Result := TdxTabAlignmentType.Right
  else if rbDecimal.Checked then
...
begin
  ...
  else if rbDots.Checked then
    Result := TdxTabLeaderType.Dots
  else if rbMiddleDots.Checked then
    Result := TdxTabLeaderType.MiddleDots
  else if rbHyphens.Checked then
    Result := TdxTabLeaderType.Hyphens
  else if rbUnderline.Checked then
    Result := TdxTabLeaderType.Underline
  else if rbThickLine.Checked then
    Result := TdxTabLeaderType.ThickLine
  else if rbEqualSign.Checked then
...
procedure TdxRichEditTabsDialogForm.RemoveTabStop(AItemIndex: Integer;
  ATabFormattingInfo: TdxTabFormattingInfo);
var
  ATabInfo: TdxTabInfo;
  APosition: Integer;
...
procedure TdxRichEditTabsDialogForm.SetTabAlignmentType
  (const Value: TdxTabAlignmentType);
begin
  case Value of
...
procedure TdxRichEditTabsDialogForm.SetTabFormattingInfo
  (const Value: TdxTabFormattingInfo);
begin
  FTabFormattingInfo.Clear;
  FTabFormattingInfo.AddRange(Value);
end;

procedure TdxRichEditTabsDialogForm.SetTabStopPosition
...
begin
  ...
  begin
    ...
    ATabStopPosition := TabStopPosition;
    if not ATabStopPosition.IsNull and (ATabStopPosition = Value) and
      (ANewValueStr = AEditValueStr) then
      Exit;
    teTabStopPosition.EditValue := ANewValueStr;
  end;
end;

procedure TdxRichEditTabsDialogForm.SetTabLeaderType
  (const Value: TdxTabLeaderType);
begin
  case Value of
...
begin
  ...
  lblTabStopsToBeCleared.Caption :=
    cxGetResourceString(@sdxRichEditTabsDialogTabStopsToBeCleared);
  ...
  btnClearAll.Caption := cxGetResourceString
  ...
  lciTabStopPosition.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditTabsDialogTabStopPosition);
  lciDefaultTabStops.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditTabsDialogDefaultTabStops);
end;

function TdxRichEditTabsDialogForm.CreateController(AControllerParameters
  : TdxFormControllerParameters): TdxFormController;
begin
  Result := TdxTabsFormController.Create
    (AControllerParameters as TdxTabsFormControllerParameters);
end;

function TdxRichEditTabsDialogForm.GetEditedTabInfo
  : TdxNullableValue<TdxTabInfo>;
var
  AIndex: Integer;
begin
...
function TdxRichEditTabsDialogForm.GetStringRepresentationsOfUnit
  (AUnitInTwips: TdxNullableInteger): string;
begin
...
begin
  ...
  InitializeMeasurementUnitEdit(seDefaultTabStops, ToMeasurementType(UnitType),
    ...
    ModelUnitsToUIUnit(MinDefaultTabStopPosition),
    ModelUnitsToUIUnit(MaxTabStopPositionByDefault)));
  FTabStopPositionMeasurementUnit := TdxMeasurementUnitEditHelper.Create
    (UnitTypeDescription, 0.1, 2,
    ModelUnitsToUIUnit(MinTabStopPositionByDefault),
    ModelUnitsToUIUnit(MaxTabStopPositionByDefault));
...
procedure TdxRichEditTabsDialogForm.UpdateTabInfoAlignmentAndLeader
  (const ATabInfo: TdxTabInfo);
var
  ATabFormattingInfo: TdxTabFormattingInfo;
...
begin
  ...
  ANewTabInfo := TdxTabInfo.Create(ATabInfo.Position, AAlignment, ALeader,
    False, False);
...
begin
  ...
  try
    ...
    begin
      ...
      lbTabStopPosition.Items.Add
        (GetStringRepresentationsOfUnit(APositionInTwips));
    end;
    ATabStopPositionString := GetStringRepresentationsOfUnit(TabStopPosition);
    ATabStopPositionString := IfThen(ATabStopPositionString = '',
      lbTabStopPosition.Items[0], ATabStopPositionString);
    teTabStopPosition.EditValue := ATabStopPositionString;
    lbTabStopPosition.ItemIndex := lbTabStopPosition.Items.IndexOf
      (ATabStopPositionString);
  finally
