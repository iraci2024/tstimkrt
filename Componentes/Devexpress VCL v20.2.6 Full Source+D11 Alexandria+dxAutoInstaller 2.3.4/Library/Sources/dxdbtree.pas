...
{$ENDIF}
  ...
  Dialogs,
  Variants, ComCtrls, CommCtrl, DB, dxCore, dxMessages, cxGraphics, cxControls,
  dxtree;
...
type
  ...
  public
    constructor Create(AOwner: TdxDBTreeNodes; Value: Variant);
  ...
  public
    ...
    property Items[Index: Integer]: TdxDBTreeNode
      read GetDBNodeFromIndex; default;
    property MaxKeyFieldValue: Variant read GetMaxKeyFieldValue;
  end;

  TAddNewDBTreeNodeEvent = procedure(Sender: TObject;
    var DBTreeNode: TdxDBTreeNode) of Object;
  TDBTreeRefreshNodeEvent = procedure(Sender: TObject;
    DBTreeNode: TdxDBTreeNode) of Object;
  TCreateNewKeyValue = procedure(Sender: TObject; var NewKeyValue: Variant)
    of Object;
  TSetDisplayItemText = procedure(Sender: TObject; var DisplayText: string)
    of Object;
  ...
  TdxDBTreeViewAction = (trDBInsert, trDBDelete, trDBChangeText,
    trDBChangeParent, trDBMove);
  TdxDBTreeViewActionEvent = procedure(Sender: TObject;
    DBNode1, DBNode2: TdxDBTreeNode; Action: TdxDBTreeViewAction;
    var Automatic: Boolean) of Object;
  ...
  private
    ...
    function AddNode(AParentNode: TdxDBTreeNode; const AText: string)
      : TdxDBTreeNode;
  ...
  protected
    ...
    function DoDBAction(DBNode1, DBNode2: TdxDBTreeNode;
      Action: TdxDBTreeViewAction): Boolean; dynamic;
    ...
    procedure EndUpdate;
    function LockedLocate(const AFieldName: string; AValue: Variant;
  ...
  published
    ...
    property DisplayField: string read FDisplayFieldName
    ...
    property Ctl3D;
    property Options: TdxDBTreeViewOptions read FOptions write SetOptions
      default [];
    property SortType;
    ...
    property Visible;
    property OnAddNewItem: TAddNewDBTreeNodeEvent read FAddNewItem
      write FAddNewItem;
    property OnCreateNewKeyValue: TCreateNewKeyValue read FCreateNewKeyValue
      write FCreateNewKeyValue;
    property OnSetDisplayItemText: TSetDisplayItemText
      read FOnSetDisplayItemText write FOnSetDisplayItemText;
    property OnInsertNodeError: TNotifyEvent read FOnInsertNodeError
      write FOnInsertNodeError;
    property OnDBAction: TdxDBTreeViewActionEvent read FOnDBAction
      write FOnDBAction;
    property OnRefreshNode: TDBTreeRefreshNodeEvent read FOnRefreshNode
      write FOnRefreshNode;
    property OnClick;
    property OnEnter;
    property OnExit;
    property OnDragDrop;
...
begin
  if (DataSet <> nil) and ((Filter <> DataSet.Filter) or
    (Filtered <> DataSet.Filtered)) then
  begin
    Filter := DataSet.Filter;
    Filtered := DataSet.Filtered;
    FDBTreeView.DataLinkActiveChanged;
...
var
  AKeyValue: Variant;
begin
  if not DBTreeNodes.Destroying and
    not(csDestroying in DBTreeNodes.FDBTreeView.ComponentState) then
  begin
    AKeyValue := FKeyFieldValue;
    with DBTreeNodes.FDBTreeView do
      if (FKeyField <> nil) and (FParentField <> nil) and not IsUpdateLocked and
        not CNNotifyFlag and not CopyTreeNodeStructFlag and HandleAllocated then
      begin
...
begin
  with DBTreeNodes.FDBTreeView do
  begin
    if FKeyFieldValue <> FKeyField.Value then
      Exit;
...
begin
  ...
  try
    ...
    try
      ...
      try
        HasChildren := DBTreeNodes.FDBTreeView.LockedLocate
          (DBTreeNodes.FDBTreeView.FParentFieldName, KeyFieldValue, []);
        if HasChildren then
          while not ADataSet.EOF and
            VarEquals(DBTreeNodes.FDBTreeView.FParentField.Value,
            KeyFieldValue) do
          begin
            if DBTreeNodes.GetDBTreeNode
              (DBTreeNodes.FDBTreeView.FKeyField.Value) = nil then
...
function TdxDBTreeNodes.FindNearest(Value: Variant; var Index: Integer)
  : Boolean;
var
  Min, Max: LongInt;
begin
  Index := -1;
  Result := False;
  if (FList.Count = 0) or VarIsNull(Value) or
    VarFirstMoreEx(TdxDBTreeNode(FList.List[0]).FKeyFieldValue, Value) then
    Exit;
  ...
  if VarEquals(Value, TdxDBTreeNode(FList.List[Index]).KeyFieldValue) then
    Result := True
  else if (Index < FList.Count - 1) and
    VarFirstMore(Value, TdxDBTreeNode(FList.List[Index]).FKeyFieldValue) then
    Inc(Index);
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        try
          ANode.HasChildren := FDBTreeView.LockedLocate
...
begin
  ...
  try
    ...
    begin
      if (Result.Parent <> AParentNode) and (Result <> AParentNode) and
...
function TdxDBTreeView.DoDBAction(DBNode1, DBNode2: TdxDBTreeNode;
  Action: TdxDBTreeViewAction): Boolean;
begin
  Result := True;
...
begin
  ...
  else if (trSmartRecordLoad in Options) and not IsUpdateLocked then
  begin
    BeginUpdate;
    try
      ...
      begin
        ...
        try
          ...
          try
            ...
            try
              while (ANode = nil) do
                if LockedLocate(FKeyFieldName, FParentField.Value, []) and
                  not(VarEquals(FKeyField.Value, FParentField.Value)) then
                begin
                  New(PValue);
...
begin
  ...
  try
    ...
    begin
      ...
      if not VarIsNull(V) and (VarType(V) <> varEmpty) and
        not VarEquals(FKeyField.Value, V) and
        (dsInsert <> FKeyField.DataSet.State) and
        not((FDataLink.DataSet.EOF) and (FDataLink.DataSet.BOF)) then
      begin
        ASelectedNode := nil;
  ...
  begin
    ...
    try
      if (not ReadOnly) and (FListField.DataSet <> nil) and (Selected <> nil)
        and ((FListField.DataSet.State = dsEdit) or
        (FListField.DataSet.State = dsInsert)) and
        (FListField.Text <> Selected.Text) then
        FListField.Text := Selected.Text
      else if Selected.Text <> FListField.Text then
        ...
        try
          ...
          begin
            ...
            if (csDesigning in ComponentState) and
              (FDataLink.DataSet.RecordCount > 999) then
              I := MessageDlg(dxDBTreeViewSmartLoadS, mtConfirmation,
                [mbYes, mbNo], 0);
            if I = mrNo then
            begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if not(VarEquals(FKeyField.Value,
                  FDBTreeNodes[I].FKeyFieldValue)) then
                  LockedLocate(FKeyFieldName,
                    FDBTreeNodes[I].FKeyFieldValue, []);
                FDBTreeNodes.RefreshRecord;
                FDataLink.DataSet.Next;
              end;
            end
            ...
            if LockedLocate(FParentFieldName, FRootValue, []) then
              while (not FDataLink.DataSet.EOF) and
                VarEquals(FParentField.Value, FRootValue) do
              begin
                FDBTreeNodes.RefreshRecord;
                FDataLink.DataSet.Next;
              end;
...
begin
  ...
  if (FKeyField <> nil) and not VarIsNull(FRootValue) then
    case FKeyField.DataType of
      ftSmallint:
        VarCast(FRootValue, FRootValue, varSmallint);
      ftInteger, ftWord, ftAutoInc:
        VarCast(FRootValue, FRootValue, varInteger);
      ftFloat, ftCurrency:
...
begin
  if IsUpdateLocked or not HandleAllocated or
...
function TdxDBTreeView.AddNode(AParentNode: TdxDBTreeNode; const AText: string)
  : TdxDBTreeNode;
begin
  if AParentNode = nil then
...
var
  ...
  ALocate: Boolean;
begin
  if (FKeyField = nil) or (FParentField = nil) or IsUpdateLocked or
    CNNotifyFlag or CopyTreeNodeStructFlag or not HandleAllocated then
    Exit;
...
function TdxDBTreeView.LockedLocate(const AFieldName: string; AValue: Variant;
  AOptions: TLocateOptions): Boolean;
begin
  BeginUpdate;
  try
...
{$WARNINGS ON}
  ...
  begin
    ...
    begin
      AStream := FDataLink.DataSet.CreateBlobStream
        (FDataLink.DataSet.FindField(AFieldStruct.FieldName), bmWrite);
      try
...
{$WARNINGS OFF}
      FDataLink.DataSet.FindField(AFieldStruct.FieldName)
        .SetData(AFieldStruct.Buffer);
{$WARNINGS ON}
  end;
...
var
  ...
  AField: TField;
begin
  if not DoDBAction(TdxDBTreeNode(ListS[0]), TdxDBTreeNode(ListD[0]), trDBMove)
  then
    Exit;
  ...
  try
    ...
    try
      ...
      if (trSmartRecordCopy in FOptions) and (ListS.Count > 0) and
        (ASourceDBTreeView <> nil) then
      begin
        ...
        begin
          ...
          if (CompareText(AField.FieldName, FKeyFieldName) <> 0) and
            (CompareText(AField.FieldName, FParentFieldName) <> 0) and
            (FDataLink.DataSet.FindField(AField.FieldName) <> nil) then
      ...
      begin
        ...
        if (FDisplayFields.Count > 0) and (ASourceDBTreeView = Self) and
          (ASourceNode <> nil) then
        begin
        ...
        begin
          ...
          try
            ...
            begin
              ASourceDBTreeView.LockedLocate(ASourceDBTreeView.FKeyFieldName,
                ASourceNode.FKeyFieldValue, []);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SendMessage(TreeView_GetEditControl(Handle), WM_SETTEXT, 0,
          LPARAM(PChar(TreeNode.Text)));
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        AOldNode := DBTreeNodes.GetTreeNode(TdxDBTreeNode(ANode)
          .FParentFieldValue);
...
var
  ANode: TTreeNode;
begin
  if not IsUpdateLocked and not CopyTreeNodeStructFlag and not IsEditing and
    (PTVITEM(Message.LPARAM)^.mask and TVIF_TEXT = 1) and
    (PTVITEM(Message.LPARAM)^.hItem <> nil) and (not ReadOnly) and
    (FListField <> nil) and (FDisplayFields.Count = 0) and
    (FDataLink.DataSet.CanModify) and not FListField.ReadOnly then
  begin
    FDataLink.DataSet.DisableControls;
    try
      SendMessage(Handle, WM_SETREDRAW, WPARAM(False), 0);
      try
        ...
        BeginUpdate;
        try
          if (FListField.Text <> Selected.Text) and
            DoDBAction(TdxDBTreeNode(Selected), nil, trDBChangeText) then
          begin
            FDataLink.DataSet.Edit;
            FListField.Text := Selected.Text;
            FDataLink.DataSet.Post;
...
begin
  if not Focused and (Selected <> nil) and (Selected.IsVisible) and not IsEditing
  then
    FrameSelectedItem;
end;
    ...
    case Msg of
      WM_CHAR:
        if not(FDataLink.DataSet.CanModify) or FListField.ReadOnly then
          Exit
        else if not FListField.IsValidChar(Char(WPARAM)) then
        begin
          MessageBeep(0);
          Exit;
        end;
