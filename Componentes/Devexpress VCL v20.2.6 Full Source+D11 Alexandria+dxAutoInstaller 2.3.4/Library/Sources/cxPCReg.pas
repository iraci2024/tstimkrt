...
uses
  ...
  Classes, Forms, SysUtils, TypInfo, ImgList, dxCore, cxPC, cxPCConsts,
  cxPCPaintersFactory,
...
const
  ...
  cxPageControlComponentEditorVerbA: array [0 .. 3] of string = ('New Page',
    'Next Page', 'Previous Page', 'Delete Page');
...
begin
  raise EdxException.Create('TcxPCStyleProperty.' + SourceMethodName +
    ': ' + Msg);
end;
...
begin
  Result := (GetComponent(0) as TcxPCButton)
    .Collection.Owner as TcxPCCustomButtons;
end;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TcxTabSheet), TcxPageControlProperties,
    'ActivePage', TcxPageControlActivePageProperty);
  RegisterPropertyEditor(TypeInfo(TBitmap), TcxCustomTabControlProperties,
    'DragImage', TcxBitmapProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxPCButton,
    'HeaderImageIndex', TcxPCHeaderImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TcxPCButton, 'TabImageIndex',
    TcxPCTabImageIndexProperty);

  RegisterSelectionEditor(TcxCustomTabControl, TcxTabControlSelectionEditor);

  HideClassProperties(TcxTabControl, ['HideTabs', 'HotTrack', 'ImageBorder',
    'Images', 'MultiLine', 'MultiSelect', 'NavigatorPosition', 'Options',
    'OwnerDraw', 'RaggedRight', 'Rotate', 'MaxRotatedTabWidth',
    'ScrollOpposite', 'ShowFrame', 'Style', 'TabHeight', 'TabIndex',
    'TabPosition', 'Tabs', 'TabSlants', 'TabWidth', 'OnNew']);

  HideClassProperties(TcxPageControl, ['ActivePage', 'HideTabs', 'HotTrack',
    'ImageBorder', 'Images', 'MultiLine', 'NavigatorPosition', 'Options',
    'OwnerDraw', 'RaggedRight', 'Rotate', 'MaxRotatedTabWidth',
    'ScrollOpposite', 'ShowFrame', 'Style', 'TabHeight', 'TabPosition',
