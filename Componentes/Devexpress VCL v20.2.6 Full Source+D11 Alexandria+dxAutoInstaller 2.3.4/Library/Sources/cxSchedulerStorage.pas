...
uses
  Variants, DateUtils, Classes, SysUtils, Windows, Forms, Math, Graphics,
  Contnrs,
  Types, ExtCtrls, Controls, ImgList, RTLConsts, Generics.Defaults,
  Generics.Collections,
  dxCore, dxGenerics, dxCoreClasses, cxClasses, cxCustomData, cxGraphics,
  dxThreading,
  cxDataStorage, cxDateUtils, cxDataUtils, cxVariants, cxStorage,
  cxLookAndFeels,
  cxFormats, cxSchedulerUtils, cxSchedulerHolidays, cxDataConsts,
  cxSchedulerRecurrence, dxForms;
...
type
  ...
  TcxSchedulerStorageResourceItemClass = class of
    TcxSchedulerStorageResourceItem;

  TcxSchedulerForEachEventHandlerProc = procedure(AEvent: TcxSchedulerEvent;
  ...
  TcxOnDeleteEventFunc = function(AEvent: TcxSchedulerControlEvent)
    : Boolean of object;
  ...
  TcxGetRecurrenceDescriptionStringProc = function(ARecurrenceInfo
  ...
  TcxDueTimeInfoToTextProc = function(const AInfo
    : TcxSchedulerReminderDueTimeInfo): string;
  ...
  public
    function Add(AEvent: TcxSchedulerEvent): TcxDoublyLinkedEventListData;
      reintroduce;
    procedure FreeEvents; virtual;
    function GetLinkedObjectClass: TcxDoublyLinkedObjectClass; override;
    procedure ForEach(AProc: TcxSchedulerForEachEventHandlerProc;
      AData: Pointer; AGoForward: Boolean = True);
  ...
  protected
    procedure ForEachEvent(AProc: TcxSchedulerForEachEventHandlerProc;
      AData: Pointer; AGoForward: Boolean);
    procedure InitializeEventFromRecordIndex(AEvent: TcxSchedulerEvent;
      AData: Pointer);
    procedure InitializeEventHandles;
    procedure InitializeRecordIndexes;
    procedure InitializeRecordIndexFromEvent(AEvent: TcxSchedulerEvent;
      AData: Pointer);
  ...
  public
    procedure ChangeValueTypeClass(AItemIndex: Integer;
      AValueTypeClass: TcxValueTypeClass); override;
  ...
  private
    ...
    procedure SetValue(AEvent: TcxSchedulerEvent;
      const AValue: Variant); inline;
  ...
  protected
    ...
    property DataController: TcxSchedulerStorageDataController
    ...
    property Values[AEvent: TcxSchedulerEvent]: Variant read GetValue
      write SetValue;
  ...
  private
    ...
    function GetStorage: TcxCustomSchedulerStorage;
  protected
    function FindFieldByName(const AName: string)
      : TcxCustomSchedulerStorageField; virtual;
  ...
  public
    ...
    property Items[Index: Integer]: TcxSchedulerStorageField read GetItem
      write SetItem;
    property Storage;
  end;
  ...
  protected
    ...
    function GetCalculatorClass: TcxSchedulerRecurrenceCalculatorClass;
      override;
    function GetData: TcxSchedulerCustomRecurrenceInfoData; override;
    function GetOccurrences(AList: TcxSchedulerFilteredEventList;
      const AStart, AFinish: TDateTime): Boolean;
    function GetValue(var AValue: AnsiString): Boolean; override;
    procedure SetDataItem(AOffset: Pointer; ASize: Integer;
      const AValue); override;
  ...
  TcxSchedulerEventRecurrenceInfoClass = class of
    TcxSchedulerEventRecurrenceInfo;

  { TcxSchedulerEventItemLink }

  TcxSchedulerEventRelation = (trFinishToStart, trStartToStart,
  ...
  protected
    ...
    function CheckLinked(const ID: Variant; ARecurrenceIndex: Integer)
      : Boolean; overload;
  ...
  public
    ...
    property Relation: TcxSchedulerEventRelation read FRelation
      write SetRelation;
  end;
  ...
  protected
    ...
    procedure Update(Item: TCollectionItem); override;
  public
    function Add(AEvent: TcxSchedulerEvent;
      ARelation: TcxSchedulerEventRelation = trStartToFinish)
      : TcxSchedulerEventItemLink; overload;
    ...
    property ItemLinks[Index: Integer]: TcxSchedulerEventItemLink
      read GetItemLink write SetItemLink; default;
  end;
  ...
  TcxSchedulerEventTaskStatus = (tsNotStarted, tsInProgress, tsComplete,
    tsWaiting, tsDeferred);
  ...
  private
    ...
    procedure InternalSetTaskComplete(const AValue: Integer;
      AUpdateTaskStatus: Boolean = True);
    procedure InternalSetTaskStatus(AValue: TcxSchedulerEventTaskStatus;
      AUpdateTaskComplete: Boolean = True);
    ...
    procedure SetReminderResourcesData(const AValue
  ...
  protected
    ...
    function GetOccurrenceByIndex(AIndex: Integer;
      var AOccurrence: TcxSchedulerEvent): Boolean;
    function GetOwner: TPersistent; override;
    ...
    function GetValueDefault(AField: TcxCustomSchedulerStorageField;
      const ADefValue: Variant): Variant; virtual;
    ...
    procedure SetValue(AField: TcxCustomSchedulerStorageField;
    ...
    property ReminderResourcesData: TcxSchedulerReminderResourcesData
      read GetReminderResourcesData write SetReminderResourcesData;
    property ValueDefs[AFieldIndex: Integer]: TcxValueDef read GetValueDef;
  public
    constructor Create(AStorage: TcxCustomSchedulerStorage;
      ADisableEditing: Boolean = False); overload; virtual;
    constructor Create(AStorage: TcxCustomSchedulerStorage;
      ARecordHandle: Pointer); overload; virtual;
    constructor Create; overload;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    function AllowLink(AEvent: TcxSchedulerEvent): Boolean; virtual;
    procedure AssignAttributes(ASource: TcxSchedulerEvent;
      AUseSourceTime: Boolean = True); virtual;
    ...
    function CreateConflictsInfo(AExceptEventsWithoutResources: Boolean)
    ...
    function GetTaskLinkOwnerRelation(ATaskLinkOwner: TcxSchedulerEvent;
      var ARelation: TcxSchedulerEventRelation): Boolean; virtual;
    procedure GetValidTaskTimeRange(var AStart, AFinish: TDateTime);
    procedure GetValidTaskTimeRangeByRelation(const ARelation
      : TcxSchedulerEventRelation; AEvent: TcxSchedulerEvent;
      var AStart, AFinish: TDateTime); virtual;
    ...
    function ValidateOccurrenceTimeBounds(AOccurrence: TcxSchedulerEvent;
      const ANewStart, ANewFinish: TDateTime): TcxOccurrenceDateStatus;
    // resource sharing
    procedure ShareWithResource(AResourceItem
      : TcxSchedulerStorageResourceItem); overload;
    procedure ShareWithResource(AResourceID: Variant); overload;
    function IsSharedWithResource(AResourceItem
      : TcxSchedulerStorageResourceItem): Boolean; overload;
    function IsSharedWithResource(AResourceID: Variant): Boolean; overload;
    procedure UnshareWithResource(AResourceItem
      : TcxSchedulerStorageResourceItem); overload;
    ...
    function GetCustomFieldValue(ACustomField
      : TcxCustomSchedulerStorageField): Variant;
    procedure SetCustomFieldValueByIndex(AIndex: Integer;
      const AValue: Variant);
    procedure SetCustomFieldValueByName(const AName: string;
      const AValue: Variant);
    procedure SetCustomFieldValue(ACustomField: TcxCustomSchedulerStorageField;
      const AValue: Variant);
    ...
    property EditingRecordHandle: Pointer read FEditingRecordHandle
      write FEditingRecordHandle;
    property EditValues[Index: Integer]: Variant read GetEditValue
      write SetEditValue;
    ...
    property ParentGroup: TcxSchedulerEvent read FParentGroup
      write SetParentGroup;
    property ReadOnly: Boolean read GetReadOnly;
    property RecordHandle: Pointer read FRecordHandle write FRecordHandle;
    property RecurrenceIndex: Integer read GetRecurrenceIndex
      write SetRecurrenceIndex;
    property RecurrenceInfo: TcxSchedulerEventRecurrenceInfo
      read FRecurrenceInfo write SetRecurrenceInfo;
    ...
    property ReminderMinutesBeforeStart: Integer
      read GetReminderMinutesBeforeStart write SetReminderMinutesBeforeStart;
    ...
    property TaskStatus: TcxSchedulerEventTaskStatus read GetTaskStatus
      write SetTaskStatus;
  ...
  protected
    ...
    function CanResize(AResource: TcxSchedulerStorageResourceItem)
      : Boolean; overload;
    procedure CheckLinksOnChangeEventType(ANewEventType: TcxEventType);
      override;
    ...
    function IsParentGroupForEvent(AEvent: TcxSchedulerEvent): Boolean;
      override;
    ...
    property SelectionAdapter: IcxSchedulerSelectionAdapter
      read FSelectionAdapter write FSelectionAdapter;
    property VisibleIndex: Integer read FVisibleIndex write FVisibleIndex;
  public
    constructor Create(AStorage: TcxCustomSchedulerStorage;
      ADisableEditing: Boolean = False); reintroduce; overload;
    constructor Create(ASource: TcxSchedulerEvent); reintroduce; overload;
    procedure InitFromSource(ASource: TcxSchedulerEvent);
    constructor Create(ASource: TcxSchedulerEvent;
      const AStart, AFinish: TDateTime); reintroduce; overload;
    constructor Create(ASource, AOriginal: TcxSchedulerEvent);
    ...
    function GetTaskLinkOwnerRelation(ATaskLinkOwner: TcxSchedulerEvent;
      var ARelation: TcxSchedulerEventRelation): Boolean; override;
  ...
  TcxSchedulerGetResourceImageIndexEvent = procedure(Sender: TObject;
    AResource: TcxSchedulerStorageResourceItem; var AImageIndex: TcxImageIndex)
    of object;
  ...
  protected
    ...
    property Storage: TcxCustomSchedulerStorage read GetStorage;
  public
    constructor Create(AOwner: TcxSchedulerStorageResources;
      AItemClass: TcxSchedulerStorageResourceItemClass); virtual;
    destructor Destroy; override;
    function Add: TcxSchedulerStorageResourceItem;
    property Items[Index: Integer]: TcxSchedulerStorageResourceItem read GetItem
      write SetItem; default;
    property VisibleResourceCount: Integer read GetVisibleResourceCount;
    property VisibleResources[Index: Integer]: TcxSchedulerStorageResourceItem
  ...
  published
    ...
    property ImageIndex: TcxImageIndex read FImageIndex write SetImageIndex
      default -1;
    property Name: string read FName write SetName;
    property Parent: TcxSchedulerStorageResourceItem read FParent
      write SetParent;
  ...
  private
    ...
    FItemsVisibility
      : TdxVariantToObjectDictionary<TcxSchedulerStorageResourceItem>;
    FLockCount: Integer;
  ...
  protected
    function DecodePropertyName(const AName: string;
    ...
    function DoGetResourceImageIndex(AItem: TcxSchedulerStorageResourceItem)
      : TcxImageIndex; virtual;
    function DoGetResourceName(AItem: TcxSchedulerStorageResourceItem)
      : string; virtual;
    procedure DoResourceVisibleChanged(AItem: TcxSchedulerStorageResourceItem);
    function GetOwner: TPersistent; override;
    function GetResourceItemByID(const AResource: Variant)
      : TcxSchedulerStorageResourceItem;
    ...
    //
    property IsNullResourceVisible: Boolean read FIsNullResourceVisible
      write FIsNullResourceVisible;
  public
    ...
    function GetResourceName(AResource
      : TcxSchedulerStorageResourceItem): string;
    function GetResourceNameByID(const AResource: Variant): string;

    property AreImagesUsed: Boolean read GetAreImagesUsed;
    property ResourceItems: TcxSchedulerStorageResourceItems
      read GetResourceItems;
  ...
  published
    ...
    property OnGetResourceImageIndex: TcxSchedulerGetResourceImageIndexEvent
      read FOnGetResourceImageIndex write FOnGetResourceImageIndex;
    property OnGetResourceName: TcxSchedulerGetResourceNameEvent
      read FOnGetResourceName write FOnGetResourceName;
    property OnResourceVisibleChanged: TcxSchedulerResourceEvent
      read FOnResourceVisibleChanged write FOnResourceVisibleChanged;
  end;
  ...
  protected
    ...
    procedure ExpandRange(AEvent: TcxSchedulerEvent; var AIndex: Integer;
      var ALastPosition: TDateTime);
    procedure Init(AStorage: TcxCustomSchedulerStorage;
      AExceptEventsWithoutResources: Boolean; AResourceID: Variant;
      const AStart, AFinish: TDateTime);
    function IntersectEvents(ACheckedEvent: TcxSchedulerEvent)
      : Boolean; overload;
    function IntersectEvents(AEvent1, AEvent2: TcxSchedulerEvent)
      : Boolean; overload;
    function IntersectTime(const AStart, AFinish: TDateTime): Boolean; overload;
    function IntersectTime(const AStart, AFinish, AStart1, AFinish1: TDateTime)
      : Boolean; overload;
  ...
  public
    constructor Create(AEvent: TcxSchedulerEvent;
      AExceptEventsWithoutResources: Boolean); overload; virtual;
    constructor Create(AEvent: TcxSchedulerEvent;
      AExceptEventsWithoutResources: Boolean; const AStart, AFinish: TDateTime);
      overload; virtual;
    constructor Create(AStorage: TcxCustomSchedulerStorage;
      AExceptEventsWithoutResources: Boolean; const AStart, AFinish: TDateTime;
      AResourceID: Variant; AExcludedEvent: TcxSchedulerEvent = nil);
      overload; virtual;
    destructor Destroy; override;
    ...
    property ExceptEventsWithoutResources: Boolean
      read FExceptEventsWithoutResources;
    property Event: TcxSchedulerEvent read FEvent;
  ...
  private
    FStorage: TcxCustomSchedulerStorage;
  protected
    function IsLinked(AEvent, ALink: TcxSchedulerEvent;
      ACheckGroupItems: Boolean = True): Boolean;
  ...
  TcxSchedulerEventLinksControllerClass = class of
    TcxSchedulerEventLinksController;
  ...
  public
    constructor Create(AOwner: TcxSchedulerEditingEventInfoList;
    ...
    function IsEventEditing(ASourceID, APatternID: Variant;
      ARecurrenceIndex: Integer; AEventType: TcxEventType): Boolean; virtual;
    function IsValid: Boolean;
    procedure ReplaceEvent(AEvent: TcxSchedulerControlEvent);
    function ShowEditor(out AModified: Boolean): Boolean;
    procedure Destroying;

    function CreateEvent(ASource: TcxSchedulerEvent;
      AOriginal: TcxSchedulerControlEvent; AForcePatternEditing: Boolean)
      : TcxSchedulerControlEvent;
    ...
    property RecurrenceIndex: Integer read FRecurrenceIndex
      write FRecurrenceIndex;
    property Storage: TcxCustomSchedulerStorage read FStorage;
    property OnAfterEditing: TcxOnAfterEditingProc read FOnAfterEditing
  ...
  protected
    ...
    function DoShowEditor(const AEditInfo: TcxSchedulerEventEditInfo;
      out AModified: Boolean): Boolean;
    procedure DoUpdate; virtual;
    function FindExceptionInfo(const AExceptionEvent: TcxSchedulerEvent)
      : TcxSchedulerEditingEventInfo; virtual;
    ...
    function GetItemCore(AIndex: Integer)
      : TcxSchedulerEditingEventInfo; virtual;
    function GetRootEvent(AEvent: TcxSchedulerEvent)
      : TcxSchedulerEvent; virtual;
    function GetRootStorage(AEvent: TcxSchedulerEvent)
    ...
    function CreateEvent(ASource, AOriginal: TcxSchedulerEvent;
      ...
      overload; virtual;
    function CreateEvent(const AEventInfo: TcxSchedulerEventEditInfo)
      : TcxSchedulerControlEvent; overload;
  ...
  public
    ...
    property ActiveEditingEvent: TcxSchedulerControlEvent
      read GetActiveEditingEvent;
    property Count: Integer read GetCount;
    property Items[AIndex: Integer]: TcxSchedulerEditingEventInfo
      read GetItem; default;
  end;

  TcxSchedulerEditingEventInfoListClass = class of
    TcxSchedulerEditingEventInfoList;

  { TcxCustomSchedulerStorage }

  TcxSchedulerEventIntersectEvent = procedure(Sender: TcxCustomSchedulerStorage;
    AEvent1, AEvent2: TcxSchedulerEvent; var Allow: Boolean) of object;

  TcxCustomSchedulerStorage = class(TcxCustomComponent,
  ...
  protected
    ...
    procedure CreateHolidayEvent(AHoliday: TcxSchedulerHolidaysLocationHoliday;
      AResourceID: Variant); virtual;
    ...
    function DoEventIntersect(AEvent1, AEvent2: TcxSchedulerEvent)
      : Boolean; virtual;
    ...
    procedure GenerateHolidayEventsBySchedulerHolidays(const AResourceID
      : Variant; AHolidays: TcxSchedulerHolidays); virtual;
    function GetDataControllerClass: TcxCustomDataControllerClass; virtual;
    function GetEditingEventInfoListClass
      : TcxSchedulerEditingEventInfoListClass; virtual;
    function GetEventClass: TcxSchedulerEventClass; virtual;
    function GetEventID(AEvent: TcxSchedulerEvent): Variant; virtual;
    function GetFieldValueTypeClass(AField: TcxCustomSchedulerStorageField)
      : TcxValueTypeClass; virtual;
    function GetLinksControllerClass
      : TcxSchedulerEventLinksControllerClass; virtual;
    function GetParentForm: TForm;
    function GetStoreUsingGlobalTime: Boolean; virtual;
    function GetEventActualValue(AEvent: TcxSchedulerEvent;
      AIndex: Integer): Variant;
    function GetEventValue(const AHandle: Pointer; AIndex: Integer)
      : Variant; virtual;
    function GetIsBoundMode: Boolean; virtual;
    function HasEventIntersect(AEvent1, AEvent2: TcxSchedulerEvent)
      : Boolean; virtual;
    ...
    procedure SetEventValue(AEvent: TcxSchedulerEvent; AFieldIndex: Integer;
      const AValue: Variant); virtual;
    procedure SetPostFieldValue(AEvent: TcxSchedulerEvent;
      AFieldIndex: Integer); virtual;
    procedure SetValue(const ARecordHandle: Pointer; const AFieldIndex: Integer;
      const AValue: Variant); virtual;
    procedure SetValueBeforePost(AEvent: TcxSchedulerEvent;
    ...
    property DataFields[Index: Integer]: TcxCustomSchedulerStorageField
      read GetDataField;
    property Editor: TForm read FEditor write FEditor;
    property CustomFields: TcxCustomSchedulerStorageFields read FCustomFields;
    property InternalFields: TcxCustomSchedulerStorageFields
      read FInternalFields;
    ...
    property IsSchedulerBiDiModeWasChanged: Boolean
      read FIsSchedulerBiDiModeWasChanged;
    ...
    property OnEventDeleted: TcxSchedulerNotificationEvent read FOnEventDeleted
      write FOnEventDeleted;
    property OnEventInserted: TcxSchedulerNotificationEvent
      read FOnEventInserted write FOnEventInserted;
    property OnEventIntersect: TcxSchedulerEventIntersectEvent
      read FOnEventIntersect write FOnEventIntersect;
    property OnEventModified: TcxSchedulerNotificationEvent
      read FOnEventModified write FOnEventModified;
    property OnFilterEvent: TcxSchedulerFilterEventEvent read FOnFilterEvent
      write SetOnFilterEvent;
  public
    ...
    { CreateEvent conflicts with C++ macro }
    function CreateOccurrence(APattern: TcxSchedulerEvent;
    ...
    function FindAvailableAllDay(var AStart, AFinish: TDateTime;
      AResourceID: Variant; AExceptEventsWithoutResources: Boolean;
      ADuration: TDateTime = 0): Boolean;
    function FindAvailableTime(var AStart, AFinish: TDateTime; AllDay: Boolean;
      AResourceID: Variant; AExceptEventsWithoutResources: Boolean;
      ADuration: TDateTime = 0; AExcludedEvent: TcxSchedulerEvent = nil)
      : Boolean; overload;
    function FindAvailableTime(AEvent: TcxSchedulerEvent;
      AExceptEventsWithoutResources: Boolean; var AStart, AFinish: TDateTime)
      : Boolean; overload;
    ...
    function GetEvents(AList: TcxSchedulerFilteredEventList;
      ...
      overload; virtual;
    function GetFieldByName(const AName: string)
      : TcxCustomSchedulerStorageField;
    ...
    procedure PopulateHolidayDates(AList: TcxSchedulerDateList;
      AStart, AFinish: TDate; AOnlyVisible: Boolean = True;
      AClearList: Boolean = True); virtual;
    ...
    property EditingEventInfoList: TcxSchedulerEditingEventInfoList
      read FEditingEventInfoList;
    property EventCount: Integer read GetEventCount;
    property Events[Index: Integer]: TcxSchedulerEvent read GetEvent
      write SetEvent;
    property FieldCount: Integer read GetFieldCount;
    property Fields[Index: Integer]: TcxCustomSchedulerStorageField
      read GetField;
    property Holidays: TcxSchedulerHolidays read FHolidays write SetHolidays;
    property InternalUpdate: Boolean read FInternalUpdate;
    property LinksController: TcxSchedulerEventLinksController
      read FLinksController;
    ...
    property Resources: TcxSchedulerStorageResources read FResources
      write SetResources;
    property StoringName: string read FStoringName write FStoringName;
    property TimeBias: Double read FTimeBias;
  published
    property UseActualTimeRange: Boolean read FUseActualTimeRange
      write FUseActualTimeRange default False;
    property StoreUsingGlobalTime: Boolean read GetStoreUsingGlobalTime
      write SetStoreUsingGlobalTime default False;
    property RemindersEvents: TNotifyEvent read FRemindersEvents
      write FRemindersEvents;
    property ResourcesEvents: TNotifyEvent read FResourcesEvents
      write FResourcesEvents;
  end;
  ...
  published
    property CustomFields: TcxSchedulerStorageFields read GetCustomFields
      write SetCustomFields;
    property Holidays;
    property Reminders;
    property Resources;
  ...
  TcxEventSelectionChangedEvent = procedure(AEvent: TcxSchedulerControlEvent)
    of object;

  TcxSchedulerEventSelection = class(TcxIUnknownObject,
    IcxSchedulerSelectionAdapter)
  private
  ...
  public
    ...
    property Count: Integer read GetCount;
    property Items[Index: Integer]: TcxSchedulerControlEvent
      read GetItem; default;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Assign(ASource: TcxSchedulerEventList;
      AOperator: TListAssignOp = laCopy); virtual;
  ...
  protected
    ...
    function AddOccurrence(ACalculator: TcxSchedulerOccurrenceCalculator)
      : TcxSchedulerControlEvent;
    function CanAddEvent(ASource: TcxSchedulerEvent; ARecurrenceIndex: Integer)
      : Boolean; virtual;
    function CanSmartUpdate(AStorage: TcxCustomSchedulerStorage;
      const AStart, AFinish: TDateTime; const AResourceID: Variant)
      : Boolean; virtual;
    procedure Changed; virtual;
    function CheckEvent(AEvent: TcxSchedulerEvent; const AResourceID: Variant;
      AddAll: Boolean): Boolean; virtual;
    function CheckResourceID(const AEventResourceID,
      AResourceID: Variant): Boolean;
    function CheckRecurrenceEvent(AEvent: TcxSchedulerEvent;
      const AResourceID: Variant; AddAll: Boolean): Boolean;
    function CheckSimpleEvent(AEvent: TcxSchedulerEvent;
      const AResourceID: Variant; AddAll: Boolean): Boolean;
    function CreateControlEvent(AEvent: TcxSchedulerEvent)
      : TcxSchedulerControlEvent; virtual;
    function GetTimeBias: Double; virtual;
    function GetTimeBiasDaylightSavingTime(ATime: TDateTime)
      : TDateTime; virtual;
    procedure Init(const AStart, AFinish: TDateTime;
      AStorage: TcxCustomSchedulerStorage; AClearItems: Boolean); virtual;
    function NeedAddOccurrenceForReminder(ACalculator
      : TcxSchedulerOccurrenceCalculator): Boolean;
  ...
  public
    ...
    property ExceptEventsWithoutResources: Boolean
      read FExceptEventsWithoutResources write FExceptEventsWithoutResources;
    property Finish: TDateTime read FFinish;
    property ReminderEventsOnly: Boolean read FReminderEventsOnly;
    property Start: TDateTime read FStart;
    property Storage: TcxCustomSchedulerStorage read FStorage;
    property Items[AIndex: Integer]: TcxSchedulerControlEvent
      read GetEvent; default;
    property Count;
  end;
  ...
  TcxSchedulerCachedEventList = class(TcxSchedulerFilteredEventList, IUnknown,
    IcxSchedulerSelectionAdapter, IcxSchedulerStorageListener,
  ...
  protected
    ...
    function CheckEvent(AEvent: TcxSchedulerEvent; const AResourceID: Variant;
      AddAll: Boolean): Boolean; override;
    function CheckEventVisibility(AEvent: TcxSchedulerEvent;
      AIncludeUnassigned: Boolean): Boolean;
    ...
    procedure Init(const AStart, AFinish: TDateTime;
      AStorage: TcxCustomSchedulerStorage; AClearItems: Boolean); override;
    ...
    procedure PostCloneForRecurrenceEvent(AEvent: TcxSchedulerControlEvent;
      ACopy: Boolean); virtual;
    ...
    procedure SelectionAdd(AEvent: TcxSchedulerControlEvent;
  ...
  public
    ...
    procedure BeforeEditing(AEvent: TcxSchedulerControlEvent;
      AIsInplace: Boolean);
    ...
    procedure ExtractEvents(const ADate: TDateTime;
      AList: TcxSchedulerEventList);
    procedure ExtractEventsByResource(AList: TcxSchedulerEventList;
      const AResource: TcxSchedulerStorageResourceItem;
    ...
    function HasIntersection(AEvent: TcxSchedulerControlEvent)
      : Boolean; overload;
    function HasIntersection(AList: TList): Boolean; overload;
    function HasIntersection(AList1, AList2: TcxSchedulerEventList;
      AExcludeEquals: Boolean): Boolean; overload;
    ...
    property AbsoluteItems[Index: Integer]: TcxSchedulerControlEvent
      read GetAbsoluteItem;
    property AbsoluteItemsList: TdxFastList read FAbsoluteItems;
    property AlwaysShowSelectedEvent: Boolean read FAlwaysShowSelectedEvent
      write FAlwaysShowSelectedEvent;
    ...
    property NeedForceUpdate: Boolean read FNeedForceUpdate
      write FNeedForceUpdate;
    property ShowEventsWithoutResource: Boolean
      read GetShowEventsWithoutResource write SetShowEventsWithoutResource;
  ...
  public
    ...
    function ValidateTimeBoundsForOccurrence(AEvent: TcxSchedulerEvent;
      AOldStart, AOldFinish, ANewStart, ANewFinish: TDateTime)
  ...
  private
    class procedure CalcAppointmentIntervals(AnEvent: TcxSchedulerEvent;
      AInfo: TcxSchedulerContentNavigationInfo;
    ...
    class procedure ChangeResourceNavigationIntervals(AContentNavigationInfo
      : TObjectList);
    class function IsEventSharedWithAnyResource
  ...
  public
    ...
    function Find(AReminderDate: TDateTime; const AResourceID: Variant)
  ...
  public
    function CreateEventReminders(AEvent: TcxSchedulerControlEvent)
      : TcxSchedulerEventReminders;
    function FindForEvent(AEvent: TcxSchedulerControlEvent)
      : TcxSchedulerEventReminders;
    procedure Invalidate;
  ...
  TcxSchedulerRemindersEvent = procedure(Sender: TcxSchedulerReminders)
    of object;
  TcxSchedulerGetEventDueTimeTextEvent = procedure
    ...
    of object;
  TcxSchedulerGetReminderWindowCaption = procedure
    (Sender: TcxSchedulerReminders; var ACaption: string) of object;
  TcxSchedulerSnoozeReminderEvent = procedure(Sender: TcxSchedulerReminders;
    AReminder: TcxSchedulerReminder; var ASnoozeTime: TDateTime;
  ...
  protected
    function CreateControlEvent(AEvent: TcxSchedulerEvent)
      : TcxSchedulerControlEvent; override;
    function GetTimeBias: Double; override;
    function GetTimeBiasDaylightSavingTime(ATime: TDateTime)
      : TDateTime; override;
  ...
  TcxSchedulerReminders = class(TcxInterfacedPersistent,
    IcxFormatControllerListener, IcxFormatControllerListener2, IdxSkinSupport)
  private
    FActive: Boolean;
  ...
  protected
    ...
    procedure CheckAddReminder(AEvent: TcxSchedulerControlEvent;
      const AResourceID: Variant; AReminderDate: TDateTime;
      var AEventReminders: TcxSchedulerEventReminders);
    procedure CheckAddReminders(AEvent: TcxSchedulerControlEvent;
    ...
    function DoDismissReminder(AReminder: TcxSchedulerReminder)
      : Boolean; virtual;
    ...
    function DoSnoozeReminder(AReminder: TcxSchedulerReminder;
      var ASnoozeTime: TDateTime): Boolean; virtual;
    procedure DoHideReminderWindow; virtual;
    procedure DoShowReminderWindow; virtual;

    function GetFirstReminderTimeForEvent(AEvent: TcxSchedulerEvent;
      ANow: TDateTime): TDateTime;
    function GetNextPopulateEventsCacheTime(ANow: TDateTime; AIsLocked: Boolean)
      : TDateTime; virtual;
    function GetNextRefreshRemindersTime(ANow: TDateTime; AIsLocked: Boolean)
      : TDateTime; virtual;
    function GetNow: TDateTime; virtual;
    function GetReminderClass: TcxSchedulerReminderClass; virtual;
    function GetReminderResourcesFromEvent(AEvent: TcxSchedulerControlEvent;
      AGetFromPattern: Boolean): TcxSchedulerReminderResources;
    ...
    procedure CheckRemindersForRecurringEvent(AEvent: TcxSchedulerControlEvent;
      var AIndex: Integer);
    procedure ValidateRemindersForEvent(AEventReminders
      : TcxSchedulerEventReminders; AEvent: TcxSchedulerControlEvent);

    property Events: TcxSchedulerFilteredEventList read FEvents;
    property NeedRestoreSelection: Boolean read FNeedRestoreSelection
      write FNeedRestoreSelection;
  ...
  public
    ...
    property Count: Integer read GetCount;
    property Items[Index: Integer]: TcxSchedulerReminder read GetItem; default;
    property ReminderWindow: TcxSchedulerCustomReminderForm
      read FReminderWindow;
    property Storage: TcxCustomSchedulerStorage read FStorage;
  published
    property Active: Boolean read FActive write SetActive default True;
    property DefaultMinutesBeforeStart: Integer read FDefaultMinutesBeforeStart
      write SetDefaultMinutesBeforeStart default 15;
    property DefaultReminder: Boolean read FDefaultReminder
      write FDefaultReminder default False;
    property ReminderByResource: Boolean read FReminderByResource
      write SetReminderByResource default False;
    property ReminderWindowLookAndFeel: TcxLookAndFeel
      read FReminderWindowLookAndFeel write SetReminderWindowLookAndFeel;
    property ShowResourcesInReminderWindow: Boolean
      read FShowResourcesInReminderWindow write SetShowResourcesInReminderWindow
      default True;
    property TimeZone: Integer read GetTimeZone write SetTimeZone default -1;
    property TimeZoneDaylightSaving: Boolean read FTimeZoneDaylightSaving
      write FTimeZoneDaylightSaving default False;
    property UseReminderWindow: Boolean read FUseReminderWindow
      write FUseReminderWindow default True;
    // events
    property OnAlertReminder: TcxSchedulerReminderEvent read FOnAlertReminder
      write FOnAlertReminder;
    property OnDismissReminder: TcxSchedulerReminderEvent
      read FOnDismissReminder write FOnDismissReminder;
    property OnOpenEvent: TcxSchedulerReminderOpenEvent read FOnOpenEvent
      write FOnOpenEvent;
...
function cxGetRecurrenceDescriptionString(ARecurrenceInfo
  : TcxSchedulerEventRecurrenceInfo; AFullDescription: Boolean = False): string;
function cxMinutesToDueTimeInfo(AMinutes: Integer)
  : TcxSchedulerReminderDueTimeInfo;
function cxDueTimeInfoToText(const AInfo
  : TcxSchedulerReminderDueTimeInfo): string;

function cxCompareSchedulerControlEvents(AEvent1,
  AEvent2: TcxSchedulerControlEvent): Integer;

function cxCompareSelectionKeys(AItem1,
  AItem2: TcxSchedulerControlEventID): Integer;

function cxFieldValueToVariant(const AValue: Variant): Variant;
function cxVariantToFieldValue(const AValue: Variant;
  ABLOBFormat: Boolean = False): Variant;
function cxReminderResourcesDataToFieldValue(const AValue
  : TcxSchedulerReminderResourcesData): Variant;
function cxFieldValueToReminderResourcesData(const AValue: Variant)
  : TcxSchedulerReminderResourcesData;
function cxGetOccurrenceDateStatusMessage(AEvent: TcxSchedulerEvent;
  AStatus: TcxOccurrenceDateStatus): string;

function cxTaskLinksToFieldValue(ALinks: TcxSchedulerEventLinks): Variant;
procedure cxFieldValueToTaskLinks(const AValue: Variant;
  ALinks: TcxSchedulerEventLinks);

function GetStreamVersion(const AStream: TStream; var AFieldCount: Integer;
  ACustomFieldCount: Integer = 0): Double;
...
function cxSchedulerStorageFieldsCompare(AItem1,
  AItem2: TcxCustomSchedulerStorageField): Integer;
begin
...
begin
  ...
  if Result = 0 then
  begin
    if (VarIsNull(AItem1.PatternID) and (VarIsNull(AItem2.PatternID) or
      VarEquals(AItem1.PatternID, cxSchedulerEventDefaultParentID))) or
      (VarIsNull(AItem2.PatternID) and (VarIsNull(AItem1.PatternID) or
      VarEquals(AItem2.PatternID, cxSchedulerEventDefaultParentID))) then
...
function cxGetRecurrenceDescriptionString(ARecurrenceInfo
  ...
  begin
    if APattern.AllDayEvent and (APattern.Duration = 1) then
      Exit;
    Result := ' from ' + FormatDateTime('t',
      APattern.Start - ARecurrenceInfo.DisplayTimeBias);
    if APattern.Duration < 1 then
      Result := Result + ' to ' + FormatDateTime('t',
        APattern.Finish - ARecurrenceInfo.DisplayTimeBias)
    else
      Result := Result + ' for ' + cxMinutesToTextProc
        (Round(APattern.Duration / MinuteToTime));
  end;
  ...
  begin
    ...
    if ARecurrenceInfo.Count >= 0 then
      Result := Result + ' until ' + FormatDateTime('ddddd',
        ARecurrenceInfo.GetEndDate);
  end;
...
procedure TcxDoublyLinkedEventListData.SetNext(const AValue
  : TcxDoublyLinkedEventListData);
begin
  inherited Next := AValue;
end;

procedure TcxDoublyLinkedEventListData.SetPrev(const AValue
  : TcxDoublyLinkedEventListData);
begin
...
function TcxDoublyLinkedEventList.Add(AEvent: TcxSchedulerEvent)
  : TcxDoublyLinkedEventListData;
begin
...
function TcxDoublyLinkedEventList.GetLinkedObjectClass
  : TcxDoublyLinkedObjectClass;
begin
  Result := TcxDoublyLinkedEventListData;
end;

procedure TcxDoublyLinkedEventList.ForEach
  (AProc: TcxSchedulerForEachEventHandlerProc; AData: Pointer;
...
begin
  ...
  if AField.ValueTypeClass = AValueTypeClass then
    Exit;
...
begin
  ...
  AEvent.RecordHandle := TcxDataStorageHelper.RemoveRecord(DataStorage,
    Integer(AEvent.RecordHandle));
...
procedure TcxSchedulerStorageDataController.InitializeRecordIndexFromEvent
  (AEvent: TcxSchedulerEvent; AData: Pointer);
begin
  AEvent.RecordHandle := Pointer(TcxDataStorageHelper.AppendRecord(DataStorage,
    AEvent.RecordHandle));
end;
...
var
  PSource: PAnsiChar;
begin
  if AEvent.FRecordHandle = nil then
    Exit;
  ReallocMem(AEvent.FRecordHandle, PcxSchedulerFieldValueDefData(AData)
    ^.ValueDef.ValueDefs.RecordSize);
  PSource := PAnsiChar(AEvent.FRecordHandle) + PcxSchedulerFieldValueDefData
    (AData)^.ValueDef.Offset;
  FillChar(PSource^, PcxSchedulerFieldValueDefData(AData)
    ^.ValueDef.BufferSize, 0);
end;
...
begin
  if (AEvent.FRecordHandle = nil) or
    (PcxSchedulerFieldValueDefData(AData)^.ValueDef = nil) then
    Exit;
  ...
  if AFreeRecord then
    ...
    begin
      ...
      System.Move(PSource^, PDest^,
        ARecordSize - (PSource - PAnsiChar(AEvent.FRecordHandle)));
...
function TcxSchedulerStorageDataController.GetStorage
  : TcxCustomSchedulerStorage;
begin
...
function TcxCustomSchedulerStorageField.GetDataController
  : TcxSchedulerStorageDataController;
begin
...
procedure TcxCustomSchedulerStorageField.SetValue(AEvent: TcxSchedulerEvent;
  const AValue: Variant);
begin
...
function TcxCustomSchedulerStorageFields.FindFieldByName(const AName: string)
  : TcxCustomSchedulerStorageField;
var
  I: Integer;
begin
...
function TcxSchedulerStorageFields.ItemByName(const AName: string)
  : TcxSchedulerStorageField;
begin
  Result := TcxSchedulerStorageField(FindFieldByName(AName));
end;

function TcxSchedulerStorageFields.GetItem(AIndex: Integer)
  : TcxSchedulerStorageField;
begin
  Result := TcxSchedulerStorageField(inherited Items[AIndex]);
end;

procedure TcxSchedulerStorageFields.SetItem(AIndex: Integer;
  AValue: TcxSchedulerStorageField);
begin
...
function TcxSchedulerEventRecurrenceInfo.GetValidStatus
...
begin
  ...
  case Recurrence of
    cxreDaily:
      Result := GetDailyPatternStatus;
    cxreWeekly:
      Result := GetWeeklyPatternStatus;
    cxreMonthly:
      Result := GetMonthlyPatternStatus;
  else
    Result := GetYearlyPatternStatus;
  end;
  if Result = rvsInvalidPattern then
    Exit;
  with TcxSchedulerOccurrenceCalculator.Create(Event,
    ...
    try
      ...
      begin
        if Event.RecurrenceInfo.Count = 1 then
          Exit;
        ...
        repeat
          ...
          AFinish := OccurrenceFinish;
        until not(Recurrence in [cxreDaily, cxreWeekly]) or
          (OccurrenceStart > AEdge);
      end
      else
        Result := rvsInvalidDuration;
    finally
...
function TcxSchedulerEventRecurrenceInfo.GetCalculatorClass
  : TcxSchedulerRecurrenceCalculatorClass;
begin
  Result := TcxSchedulerOccurrenceCalculator;
end;

function TcxSchedulerEventRecurrenceInfo.GetData
...
procedure TcxSchedulerEventRecurrenceInfo.SetDataItem(AOffset: Pointer;
  ASize: Integer; const AValue);
var
  S: AnsiString;
begin
...
function TcxSchedulerEventItemLink.CheckLinked(const ID: Variant;
  ARecurrenceIndex: Integer): Boolean;
begin
...
procedure TcxSchedulerEventItemLink.SetRelation
  (AValue: TcxSchedulerEventRelation);
begin
  if AValue = Relation then
    Exit;
  FRelation := AValue;
  Changed(True);
end;
...
function TcxSchedulerEventLinks.IsEventLinked
  (AEvent: TcxSchedulerEvent): Boolean;
var
  AID: Variant;
...
function TcxSchedulerEventLinks.GetItemLink(AIndex: Integer)
  : TcxSchedulerEventItemLink;
begin
  Result := TcxSchedulerEventItemLink(inherited Items[AIndex]);
end;

procedure TcxSchedulerEventLinks.SetItemLink(AIndex: Integer;
  AValue: TcxSchedulerEventItemLink);
begin
...
begin
  Result := TDateTime(Event.GetValueDefault(Event.Storage.FFinishField, 0)) -
    Event.TimeBias;
end;

function TcxSchedulerEventTimeCalculator.GetStoredStart: TDateTime;
begin
  Result := TDateTime(Event.GetValueDefault(Event.Storage.FStartField, 0)) -
    Event.TimeBias;
end;

procedure TcxSchedulerEventTimeCalculator.GetValidTaskTimeRange(var AStart,
  AFinish: TDateTime);
begin
...
function TcxSchedulerEventGroupItems.IsChild(AEvent: TcxSchedulerEvent)
  : Boolean;
begin
  Result := AEvent.ParentGroup = Event;
end;

function TcxSchedulerEventGroupItems.Find(AEvent: TcxSchedulerEvent)
  : TcxDoublyLinkedEventListData;
begin
  Result := First;
...
constructor TcxSchedulerEvent.Create(AStorage: TcxCustomSchedulerStorage;
  ADisableEditing: Boolean = False);
begin
  Create(AStorage, nil);
  if ADisableEditing then
    Exit;
  BeginEditing;
  AssignDefaultValues;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      RecurrenceInfo.DismissDate := TcxSchedulerEvent(Source)
...
procedure TcxSchedulerEvent.AssignAttributes(ASource: TcxSchedulerEvent;
  AUseSourceTime: Boolean = True);
begin
  BeginEditing;
  try
...
procedure TcxSchedulerEvent.ShareWithResource(AResourceItem
  : TcxSchedulerStorageResourceItem);
begin
...
function TcxSchedulerEvent.IsSharedWithResource(AResourceItem
  : TcxSchedulerStorageResourceItem): Boolean;
var
  AIndex: Integer;
begin
  Result := False;
  ...
  Result := FInfo.ResourceIDHashTable.TryGetValue
    (VarToStr(AResourceItem.ResourceID), AIndex);
  if VarIsArray(FInfo.ResourceID) then
    Result := Result and VarEqualsSoftEx(FInfo.ResourceID[AIndex],
      AResourceItem.ResourceID)
  else
    Result := Result and VarEqualsSoftEx(FInfo.ResourceID,
      AResourceItem.ResourceID)
end;
...
var
  ...
  begin
    ...
    begin
      Result := False;
      for I := VarArrayLowBound(AResourceID, 1) to VarArrayHighBound
        (AResourceID, 1) do
...
begin
  ...
  if not VarIsArray(AResources) then
    ...
    for I := VarArrayLowBound(AResources, 1) to VarArrayHighBound
      (AResources, 1) do
    begin
...
procedure TcxSchedulerEvent.UnshareWithResource(AResourceItem
  : TcxSchedulerStorageResourceItem);
begin
  if not IsSharedWithResource(AResourceItem) then
    Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      C := 0;
      for I := VarArrayLowBound(ResourceID, 1) to VarArrayHighBound
        (ResourceID, 1) do
...
function TcxSchedulerEvent.HasReminderForResourceID(const AResourceID
  : Variant): Boolean;
var
  I: Integer;
...
function TcxSchedulerEvent.Conflicts(AExceptEventsWithoutResources
  : Boolean): Boolean;
var
  AFinish: TDateTime;
begin
  ...
  with cxSchedulerEventConflictsInfoClass.Create(Self,
    AExceptEventsWithoutResources, Start, AFinish) do
    try
      Result := HasConflicts;
    finally
      Free;
...
function TcxSchedulerEvent.CreateConflictsInfo(AExceptEventsWithoutResources
  : Boolean): TcxSchedulerEventConflictsInfo;
begin
  Result := cxSchedulerEventConflictsInfoClass.Create(Self,
    AExceptEventsWithoutResources);
end;

function TcxSchedulerEvent.GetCustomFieldValueByIndex(AIndex: Integer): Variant;
begin
  Result := GetValueDefault(TcxCustomSchedulerStorageField
    (Storage.CustomFields.Items[AIndex]), Null);
end;
...
function TcxSchedulerEvent.GetCustomFieldValue(ACustomField
  : TcxCustomSchedulerStorageField): Variant;
begin
...
procedure TcxSchedulerEvent.SetCustomFieldValueByIndex(AIndex: Integer;
  const AValue: Variant);
begin
  SetCustomFieldValue(TcxCustomSchedulerStorageField(Storage.CustomFields.Items
    [AIndex]), AValue);
end;
...
procedure TcxSchedulerEvent.SetCustomFieldValue(ACustomField
  : TcxCustomSchedulerStorageField; const AValue: Variant);
begin
  if (ACustomField <> nil) and (ACustomField.Storage = Storage) then
...
begin
  ...
  begin
    ...
    else if AOriginalDate > ADate then
      Break
    else
      ALink := ALink.FLink;
  end;
  ACalculator := TcxSchedulerOccurrenceCalculator.Create(Self, Start,
    ADate + 1);
  try
...
function TcxSchedulerEvent.GetTaskLinkOwnerRelation(ATaskLinkOwner
  : TcxSchedulerEvent; var ARelation: TcxSchedulerEventRelation): Boolean;
var
  I: Integer;
begin
  Result := False;
  if (ATaskLinkOwner = nil) or (ATaskLinkOwner.TaskLinks = nil) then
    Exit;
...
procedure TcxSchedulerEvent.GetValidTaskTimeRange(var AStart,
  AFinish: TDateTime);
begin
  AStart := MinDateTime;
  AFinish := MaxDateTime;
...
procedure TcxSchedulerEvent.GetValidTaskTimeRangeByRelation(const ARelation
  : TcxSchedulerEventRelation; AEvent: TcxSchedulerEvent;
...
procedure TcxSchedulerEvent.DoGetValidTaskTimeRange(var AStart,
  AFinish: TDateTime);
var
  I: Integer;
  ...
  begin
    for I := 0 to TaskLinkOwners.Count - 1 do
      if GetTaskLinkOwnerRelation(TaskLinkOwners[I], ARelation) then
        GetValidTaskTimeRangeByRelation(ARelation, TaskLinkOwners[I],
          AStart, AFinish);
  end;
...
function TcxSchedulerEvent.IsResourceEvent
  ...
  AllowUnassigned: Boolean): Boolean;
begin
  Result := (AResource = nil) or VarIsNull(AResource.ResourceID) or
    (AllowUnassigned and VarIsNull(ResourceID)) or
...
var
  ...
  ALink: TcxSchedulerEvent;
begin
  if not Storage.ActualTimeRangeAvailable then
    Exit;
  if EventType = etPattern then
  begin
    ...
    if RecurrenceInfo.IsInfinity then
      ...
      with TcxSchedulerOccurrenceCalculator.Create(Self, RecurrenceInfo.Start,
        cxMaxDate) do
        try
...
function TcxSchedulerEvent.IsEventLinked(AHeaderEvent
  : TcxSchedulerEvent): Boolean;
begin
...
begin
  ...
  begin
    ...
    Result := (AStartTime <= ANewTime) and
      (AFinishTime >= (ANewTime + Duration));
  end;
end;
...
procedure TcxSchedulerEvent.CheckLinksOnChangeEventType(ANewEventType
  : TcxEventType);
begin
...
function TcxSchedulerEvent.CheckRecurrenceLink
  (AEvent: TcxSchedulerEvent): Boolean;
begin
  Result := (AEvent.EventType in [etCustom, etException]) and
    VarEquals(ID, AEvent.ParentID);
  if Result then
...
function TcxSchedulerEvent.CheckTimeRange(const AStartDate,
  AFinishDate: Integer): Boolean;
begin
  if Storage.ActualTimeRangeAvailable then
    Result := (ActualStart <= AFinishDate) and (ActualFinish >= AStartDate)
  else
    Result := (EventType = etPattern) or (Start <= AFinishDate) and
      (Finish >= AStartDate);
end;
...
function TcxSchedulerEvent.CreateRecurrenceInfo
  : TcxSchedulerEventRecurrenceInfo;
begin
  Result := TcxSchedulerEventRecurrenceInfo.Create(Self);
end;

function TcxSchedulerEvent.CreateReminderResourcesData
  : TcxSchedulerReminderResourcesData;
var
  ACount, I: Integer;
...
function TcxSchedulerEvent.GetOccurrenceByDate(const ADate: TDateTime)
  : TcxSchedulerEvent;
begin
  Result := FLink;
end;

function TcxSchedulerEvent.GetOccurrenceByIndex(AIndex: Integer;
  var AOccurrence: TcxSchedulerEvent): Boolean;
begin
  Result := False;
  if (FLink = nil) or SkipExceptions then
    Exit;
  AOccurrence := FLink;
...
begin
  ...
  if (Pattern <> nil) and (Pattern <> Self) and VarIsNull(Result) and
    Storage.CanGetValueFromPattern(AIndex) then
...
function TcxSchedulerEvent.IsParentGroupForEvent
  (AEvent: TcxSchedulerEvent): Boolean;
begin
...
begin
  cxFieldValueToTaskLinks(GetValueDefault(FStorage.FTaskLinksField, Null),
    TaskLinks);
  FIsDataValid := False;
end;
...
begin
  ...
  begin
    if Reminder and Shared then
      SetValue(Storage.FReminderResourcesData,
        cxReminderResourcesDataToFieldValue(CreateReminderResourcesData))
    else
      SetValue(Storage.FReminderResourcesData, Null);
  end;
end;

procedure TcxSchedulerEvent.SetActualTimeRange(const ActualStart,
  ActualFinish: TDateTime);
begin
  SetValue(FStorage.FActualStartField, Integer(Trunc(ActualStart + TimeBias)));
  SetValue(FStorage.FActualFinishField,
    Integer(Trunc(ActualFinish + TimeBias)));
end;

procedure TcxSchedulerEvent.SetRecurrenceInfoValue(const AValue: AnsiString);
begin
  SetValue(FStorage.FRecurrenceInfoField, AValue);
end;

procedure TcxSchedulerEvent.SetValue(AField: TcxCustomSchedulerStorageField;
  const AValue: Variant);
begin
  IsDataValid := False;
...
procedure TcxSchedulerEvent.SetValueByIndex(AIndex: Integer;
  const AValue: Variant);
var
  V: Variant;
begin
  ...
  if not IsNewEvent and (not Storage.GetIsBoundMode or Storage.IsUpdatingMode)
  then
  begin
    if (EditingRecordHandle = nil) or
      (Storage.IsUpdatingMode and (RecordHandle <> nil)) then
    begin
    ...
    else if EditingRecordHandle <> nil then
      Storage.SetValue(EditingRecordHandle, AIndex, AValue);
    if not Storage.GetIsBoundMode then
      Exit;
  end;
  if (VarType(AValue) = VarType(V)) and VarEquals(AValue, V) or Storage.FIsLoading
  then
    Exit;
  BeginEditing;
  try
...
function TcxSchedulerEvent.ValidateOccurrenceTimeBounds
  (AOccurrence: TcxSchedulerEvent; const ANewStart, ANewFinish: TDateTime)
  : TcxOccurrenceDateStatus;
begin
  Result := odsValid;
  if EventType <> etPattern then
  ...
  with TcxSchedulerOccurrenceCalculator.Create(Self, 0, cxMaxDate) do
    try
      Result := ValidateTimeBoundsForOccurrence(AOccurrence, AOccurrence.Start,
        AOccurrence.Finish, ANewStart, ANewFinish);
    finally
      Free;
    end;
end;
...
begin
  ...
  FInfo.RecurrenceInfo := dxVariantToAnsiString
    (GetValueDefault(FStorage.FRecurrenceInfoField, ''));
  ...
  FInfo.Visible := False;
  if VarIsArray(FInfo.ResourceID) then
    for I := VarArrayLowBound(FInfo.ResourceID, 1) to VarArrayHighBound
      (FInfo.ResourceID, 1) do
    begin
      FInfo.ResourceIDHashTable.AddOrSetValue(VarToStr(FInfo.ResourceID[I]), I);
      FInfo.Visible := FInfo.Visible or Storage.Resources.IsResourceVisible
        (FInfo.ResourceID[I]);
    end
  else
  begin
    FInfo.ResourceIDHashTable.AddOrSetValue(VarToStr(FInfo.ResourceID), 0);
    FInfo.Visible := VarIsNull(FInfo.ResourceID) or
      Storage.Resources.IsResourceVisible(FInfo.ResourceID);
  end;

  if (FPattern <> nil) and (FInfo.EventType <> etPattern) then
    FInfo.Options := GetValueDefault(FStorage.FOptionsField,
      FPattern.GetOptionsFlag)
  else
  ...
  begin
    FInfo.Finish := TDateTime(GetValueDefault(FStorage.FFinishField, 0))
      - TimeBias;
    FInfo.Start := TDateTime(GetValueDefault(FStorage.FStartField, 0))
      - TimeBias;
...
begin
  Result := GetValueDefault(FStorage.FParentIDField,
    Integer(cxSchedulerEventDefaultParentID));
end;
...
var
  AValue: Variant;
begin
  AValue := Storage.GetEventActualValue(Self,
    FStorage.FReminderDateField.Index);
...
begin
  Result := GetValueDefault(FStorage.FReminderMinutesBeforeStartField,
    Integer(-2));
end;

function TcxSchedulerEvent.GetReminderResourcesData
  : TcxSchedulerReminderResourcesData;
begin
  Result := cxFieldValueToReminderResourcesData
    (GetValueDefault(FStorage.FReminderResourcesData, Null));
end;
...
begin
  ...
  if VarIsArray(AResources) then
    Result := VarArrayHighBound(AResources, 1) -
      VarArrayLowBound(AResources, 1) + 1
  else
    Result := 1;
end;
...
begin
  Result := VarIsArray(ResourceID) and
    ((VarArrayHighBound(ResourceID, 1) - VarArrayLowBound(ResourceID, 1)) > 0);
end;
...
begin
  ...
  if (AStatus < 0) or
    (Integer(AStatus) > Integer(High(TcxSchedulerEventTaskStatus))) then
    Result := tsNotStarted
  else
...
procedure TcxSchedulerEvent.DoSetOptionsFlag(const AMask: Integer;
  AValue: Boolean);
begin
  if AValue then
...
procedure TcxSchedulerEvent.InternalSetTaskComplete(const AValue: Integer;
  AUpdateTaskStatus: Boolean = True);
begin
  if TaskComplete = AValue then
    Exit;
  ...
  try
    ...
    begin
      ...
      else if (AValue = 0) and not(TaskStatus in [tsWaiting, tsDeferred]) then
...
procedure TcxSchedulerEvent.SetReminderMinutesBeforeStart
...
procedure TcxSchedulerEvent.SetReminderResourcesData(const AValue
  : TcxSchedulerReminderResourcesData);
begin
  SetValue(FStorage.FReminderResourcesData,
    cxReminderResourcesDataToFieldValue(AValue));
end;
...
begin
  SetValue(FStorage.FResourceIDField, cxVariantToFieldValue(AValue,
    FStorage.FResourceIDField.IsBlob));
  ResetReminderResourcesData;
end;
...
constructor TcxSchedulerControlEvent.Create(AStorage: TcxCustomSchedulerStorage;
  ADisableEditing: Boolean = False);
begin
  inherited Create;
  FStorage := AStorage;
...
constructor TcxSchedulerControlEvent.Create(ASource,
  AOriginal: TcxSchedulerEvent);
begin
  Create(ASource);
...
procedure TcxSchedulerControlEvent.Assign(Source: TPersistent);
  ...
  begin
    ...
    else if TcxSchedulerEvent(Source).EventType = etPattern then
...
function TcxSchedulerControlEvent.GetTaskLinkOwnerRelation(ATaskLinkOwner
  : TcxSchedulerEvent; var ARelation: TcxSchedulerEventRelation): Boolean;
begin
  if Source <> nil then
...
procedure TcxSchedulerControlEvent.DoGetValidTaskTimeRange(var AStart,
  AFinish: TDateTime);
begin
  if Source <> nil then
...
function TcxSchedulerControlEvent.CanResize
  (AResource: TcxSchedulerStorageResourceItem): Boolean;
begin
  Result := (AResource = nil) or AResource.CanModify and
    (not IsGroup or not HasChildren);
end;

procedure TcxSchedulerControlEvent.CheckLinksOnChangeEventType
  (ANewEventType: TcxEventType);
begin
  // don't delete
end;
...
function TcxSchedulerControlEvent.GetValueByIndex(AIndex: Integer): Variant;
begin
  Result := TcxDataStorageHelper.GetValue(ValueDefs[AIndex], FRecordHandle);
  if (Pattern <> nil) and VarIsNull(Result) and Storage.CanGetValueFromPattern
    (AIndex) then
    Result := Pattern.GetValueByIndex(AIndex);
  if (AIndex = FStorage.FResourceIDField.Index) and
    not VarIsNull(FLockedResource) then
...
function TcxSchedulerControlEvent.IsParentGroupForEvent
  (AEvent: TcxSchedulerEvent): Boolean;
...
begin
  ...
  Result := inherited IsParentGroupForEvent(AControlEvent) or
    CheckBySources(AControlEvent);
end;
...
procedure TcxSchedulerControlEvent.SetValueByIndex(AIndex: Integer;
  const AValue: Variant);
begin
  Storage.SetValue(RecordHandle, AIndex, AValue);
  Modified;
...
begin
  ...
  for I := 0 to TaskLinks.Count - 1 do
    if (TaskLinks[I].Link <> nil) and
      (TaskLinks[I].Link.EventType <> etException) then
      Inc(Result);
end;
...
function TcxSchedulerStorageResourceItems.GetItem(AIndex: Integer)
  : TcxSchedulerStorageResourceItem;
begin
...
procedure TcxSchedulerStorageResourceItems.SetItem(AIndex: Integer;
  AValue: TcxSchedulerStorageResourceItem);
begin
  Items[AIndex].Assign(AValue);
end;

function TcxSchedulerStorageResourceItems.GetVisibleResource(AIndex: Integer)
  : TcxSchedulerStorageResourceItem;
begin
  Result := nil;
...
function TcxSchedulerStorageResourceItem.HasAsParent
  (AItem: TcxSchedulerStorageResourceItem): Boolean;
var
...
function TcxSchedulerStorageResourceItem.GetResources
  : TcxSchedulerStorageResources;
begin
...
begin
  ...
  FItemsVisibility :=
    TdxVariantToObjectDictionary<TcxSchedulerStorageResourceItem>.Create(False);
  FItems := CreateItems;
end;
...
procedure TcxSchedulerStorageResources.PopulateItemsAsTreeStructure
  (AList: TList);
...
function TcxSchedulerStorageResources.GetResourceItemByID(const AResource
...
begin
  Result := nil;
  if VarIsEmptyEx(AResource) then
    Exit;
  for I := 0 to ResourceItems.Count - 1 do
  begin
...
function TcxSchedulerStorageResources.GetResourceNameByID(const AResource
  : Variant): string;
var
  AResourceItem: TcxSchedulerStorageResourceItem;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if SameText(ASubValue, 'WorkFinish') then
        AValue := AItem.WorkFinish
      else if SameText(ASubValue, 'WorkDays') then
...
var
  ...
  function GetRestoreItem(AItem: TcxSchedulerStorageResourceItem)
    : PRestoringItem;
  var
    I: Integer;
  begin
...
begin
  ...
  begin
    ...
    FIsNullResourceVisible := FIsNullResourceVisible or
      VarIsNull(AItem.ResourceID);
...
function TcxSchedulerStorageResources.CreateItems
  : TcxSchedulerStorageResourceItems;
begin
  Result := TcxSchedulerStorageResourceItems.Create(Self,
    TcxSchedulerStorageResourceItem);
end;
...
procedure TcxSchedulerStorageResources.DoResourceVisibleChanged
  (AItem: TcxSchedulerStorageResourceItem);
begin
...
function TcxSchedulerStorageResources.GetResourceItems
  : TcxSchedulerStorageResourceItems;
begin
  Result := Items;
end;
...
function TcxSchedulerStorageResources.IsResourceVisible(const AResourceID
  : Variant): Boolean;
var
  AItem: TcxSchedulerStorageResourceItem;
begin
...
function cxCompareConflictEventsEx(AEvent1, AEvent2: TcxSchedulerEvent)
  : Integer;
begin
  if AEvent1 = AEvent2 then
    Result := 0
  else if AEvent1.Finish <= AEvent2.Start then
    Result := -1
  else if AEvent1.Start = AEvent2.Start then
    Result := 0
  else
...
begin
  inherited Create;
  FEvent := AEvent;
  Init(AEvent.Storage, AExceptEventsWithoutResources, AEvent.ResourceID,
    NullDate, NullDate);
  Calculate;
end;
...
constructor TcxSchedulerEventConflictsInfo.Create
  ...
  const AStart, AFinish: TDateTime; AResourceID: Variant;
  AExcludedEvent: TcxSchedulerEvent = nil);
begin
  inherited Create;
...
procedure TcxSchedulerEventConflictsInfo.Calculate(ACalculateFreeTime
  : Boolean = True);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      if AEvent.IsFreeState or IsSameEvent(AEvent) or
        IsSameSeries(FEvent, AEvent) then
      begin
        AEvent.Free;
        FConflictEvents.Delete(I);
      end;
    ...
    begin
      ...
      begin
        ...
        if ExceptEventsWithoutResources and AHasIntersection then
          AHasIntersection := VarIsEmptyEx(ACheckedEvent.ResourceID) or
            VarIsEmptyEx(AEvent.ResourceID) or
            (ACheckedEvent.IsSharedWithResource(AEvent.ResourceID) or
            AEvent.IsSharedWithResource(ACheckedEvent.ResourceID))
        else
          AHasIntersection := AHasIntersection and
            (ACheckedEvent.IsSharedWithResource(AEvent.ResourceID) or
            AEvent.IsSharedWithResource(ACheckedEvent.ResourceID));
        AHasIntersection := AHasIntersection and
          Storage.HasEventIntersect(FEvent, ACheckedEvent);
        if AHasIntersection then
          Break;
      end;
    end;
...
function TcxSchedulerEventConflictsInfo.ExcludeFromCalculate
  (AEvent: TcxSchedulerEvent): Boolean;
begin
...
begin
  ...
  begin
    ...
    Inc(AIndex);
    if IntersectTime(ARangeStart, ARangeFinish, ACheckedEvent.Start,
      ACheckedEvent.Finish) then
    begin
...
function TcxSchedulerEventConflictsInfo.IntersectEvents(ACheckedEvent
  : TcxSchedulerEvent): Boolean;
begin
  ...
  if Result and (FEvent <> nil) and not VarIsNull(ACheckedEvent.ResourceID) and
    not VarIsNull(ResourceID) then
    Result := ACheckedEvent.IsSharedWithResource(ResourceID) and
      not IsSameEvent(ACheckedEvent);
...
function TcxSchedulerEventConflictsInfo.IntersectEvents(AEvent1,
  AEvent2: TcxSchedulerEvent): Boolean;
begin
  Result := IntersectTime(AEvent1.UTCStart, AEvent1.UTCFinish, AEvent2.UTCStart,
    AEvent2.UTCFinish);
end;

function TcxSchedulerEventConflictsInfo.IntersectTime(const AStart,
  AFinish: TDateTime): Boolean;
begin
  Result := IntersectTime(AStart, AFinish, Start, Finish);
end;

function TcxSchedulerEventConflictsInfo.IntersectTime(const AStart, AFinish,
  AStart1, AFinish1: TDateTime): Boolean;
begin
  Result := (AStart < AFinish1) and (AStart1 < AFinish);
end;

function TcxSchedulerEventConflictsInfo.IsSameEvent(ACheckedEvent
  : TcxSchedulerEvent): Boolean;
begin
  Result := (EventID <> nil) and EventID.SameEvent(ACheckedEvent);
end;

function TcxSchedulerEventConflictsInfo.IsSameSeries(AEvent1,
  AEvent2: TcxSchedulerEvent): Boolean;
begin
  Result := AEvent1.RecordHandle = AEvent2.RecordHandle;
  if (AEvent1.RecordHandle = nil) or (AEvent2.RecordHandle = nil) then
    Exit;
  if not Result and
    (((AEvent1.Pattern = AEvent2) and (AEvent1.EventType = etOccurrence)) or
...
constructor TcxSchedulerEventLinksController.Create
...
function TcxSchedulerEventLinksController.AllowLink(AEvent,
  ALink: TcxSchedulerEvent): Boolean;
begin
  Result := (AEvent <> nil) and (ALink <> nil) and
    (AEvent.Storage = ALink.Storage) and
    ((AEvent.Pattern = nil) or (AEvent.Pattern <> ALink.Pattern)) and
    AEvent.IsSharedWithResource(ALink.ResourceID) and
    not AEvent.HasAsParentGroup(ALink) and not ALink.HasAsParentGroup(AEvent)
    and DoAllowLink(AEvent, ALink);
end;

function TcxSchedulerEventLinksController.IsLinked(AEvent,
  ALink: TcxSchedulerEvent; ACheckGroupItems: Boolean = True): Boolean;
var
  I: Integer;
...
begin
  ...
  begin
    if (ALink.TaskLinks[I].Link = nil) then
      Continue;
    Result := (AEvent = ALink.TaskLinks[I].Link) or
      IsLinked(AEvent, ALink.TaskLinks[I].Link);
    if Result then
      Break;
  end;
...
function TcxSchedulerEventLinksController.DoAllowLink(AEvent,
  ALink: TcxSchedulerEvent): Boolean;

  function InternalIsEventLinked(ACurrentEvent, ACurrentLink
    : TcxSchedulerEvent): Boolean;
  begin
...
constructor TcxSchedulerEditingEventInfo.Create
...
function TcxSchedulerEditingEventInfo.IsEventEditing(ASourceID,
  APatternID: Variant; ARecurrenceIndex: Integer;
...
procedure TcxSchedulerEditingEventInfo.ReplaceEvent
  (AEvent: TcxSchedulerControlEvent);
begin
  SetEvent(AEvent);
end;

function TcxSchedulerEditingEventInfo.ShowEditor(out AModified
  : Boolean): Boolean;
var
  AEditor: IcxSchedulerEventEditorForm;
begin
  AEditor := cxSchedulerEditorManager.CreateEditor
    (EditInfo.DialogsStyle, Event);
  try
...
function TcxSchedulerEditingEventInfo.GetEventID
  (AEvent: TcxSchedulerEvent): Variant;
begin
...
begin
  ...
  begin
    ...
    begin
      if VarEquals(PatternID, AEvent.ID) and (AEvent.EventType <> etPattern)
      then
      begin
        Result := True;
        Break;
...
procedure TcxSchedulerEditingEventInfo.SetStorage
...
begin
  if FEvent.Pattern <> nil then
    Result := cxRecurrenceInfoDataToString
      (FEvent.Pattern.RecurrenceInfo.GetData)
  else
    Result := '';
end;

procedure TcxSchedulerEditingEventInfo.SetEvent
  (AValue: TcxSchedulerControlEvent);
begin
...
procedure TcxSchedulerEditingEventInfo.SetOwner
  (AValue: TcxSchedulerEditingEventInfoList);
begin
...
constructor TcxSchedulerEditingEventInfoList.Create
  (AStorage: TcxCustomSchedulerStorage);
begin
  inherited Create;
  FStorage := AStorage;
...
procedure TcxSchedulerEditingEventInfoList.DeleteInfoByEvent
...
function TcxSchedulerEditingEventInfoList.FindInfo(const ASourceID: Variant)
  : TcxSchedulerEditingEventInfo;
var
  I: Integer;
begin
...
function TcxSchedulerEditingEventInfoList.IsEventEditing(ASourceID,
  APatternID: Variant; ARecurrenceIndex: Integer;
  AEventType: TcxEventType): Boolean;
var
  I: Integer;
begin
  ...
  if ASourceID <> Null then
    for I := 0 to Count - 1 do
    begin
      Result := Items[I].IsEventEditing(ASourceID, APatternID, ARecurrenceIndex,
        AEventType);
      if Result then
        Break;
    end;
...
function TcxSchedulerEditingEventInfoList.IsEventEditing
  (AEvent: TcxSchedulerEvent; ARecurrenceIndex: Integer;
  AEventType: TcxEventType): Boolean;
var
...
function TcxSchedulerEditingEventInfoList.IsEventEditing
  (AEvent: TcxSchedulerControlEvent; AForcePatternEditing: Boolean): Boolean;
var
...
begin
  ...
  Result := (AEvent.Source <> nil) and IsEventEditing(ASource,
    AEvent.RecurrenceIndex, AEventType);
end;

function TcxSchedulerEditingEventInfoList.ShowEditor(const AEventInfo
  : TcxSchedulerEventEditInfo; out AModified: Boolean): Boolean;
begin
  AEventInfo.ForcePatternEditing := AEventInfo.ForcePatternEditing and
    AEventInfo.Event.IsRecurring;
  Result := not IsEventEditing(AEventInfo.Event,
    AEventInfo.ForcePatternEditing);
  Result := Result and DoShowEditor(AEventInfo, AModified);
end;

procedure TcxSchedulerEditingEventInfoList.ShowEditor(const AEventInfo
  : TcxSchedulerEventEditInfo);
var
  AModified: Boolean;
begin
...
procedure TcxSchedulerEditingEventInfoList.CloseExceptionEditor
  (AEvent: TcxSchedulerEvent);
var
  AInfo: TcxSchedulerEditingEventInfo;
begin
...
function TcxSchedulerEditingEventInfoList.DoShowEditor(const AEditInfo
  : TcxSchedulerEventEditInfo; out AModified: Boolean): Boolean;
var
  AInfo: TcxSchedulerEditingEventInfo;
begin
...
function TcxSchedulerEditingEventInfoList.FindExceptionInfo
  (const AExceptionEvent: TcxSchedulerEvent): TcxSchedulerEditingEventInfo;
var
  I: Integer;
begin
  Result := nil;
...
function TcxSchedulerEditingEventInfoList.GetEventID
  (AEvent: TcxSchedulerEvent): Variant;
begin
  Result := cxGetEventID(AEvent);
end;

function TcxSchedulerEditingEventInfoList.GetInfoClass
  : TcxSchedulerEditingEventInfoClass;
begin
  Result := TcxSchedulerEditingEventInfo;
end;

function TcxSchedulerEditingEventInfoList.GetItemCore(AIndex: Integer)
  : TcxSchedulerEditingEventInfo;
begin
  Result := TcxSchedulerEditingEventInfo(inherited Items[AIndex]);
end;

function TcxSchedulerEditingEventInfoList.GetRootEvent
  (AEvent: TcxSchedulerEvent): TcxSchedulerEvent;
begin
  Result := AEvent;
end;

function TcxSchedulerEditingEventInfoList.GetRootStorage
  (AEvent: TcxSchedulerEvent): TcxCustomSchedulerStorage;
begin
  if AEvent = nil then
    Result := Storage
  else
...
function TcxSchedulerEditingEventInfoList.CreateEvent(ASource,
  AOriginal: TcxSchedulerEvent; AForcePatternEditing: Boolean)
  : TcxSchedulerControlEvent;
var
  AEvent: TcxSchedulerEvent;
begin
  ...
  else if ASource is TcxSchedulerControlEvent then
...
function TcxSchedulerEditingEventInfoList.CreateEvent(const AEventInfo
  : TcxSchedulerEventEditInfo): TcxSchedulerControlEvent;
begin
  Result := CreateEvent(AEventInfo.Event, AEventInfo.Event,
    AEventInfo.ForcePatternEditing);
end;

function TcxSchedulerEditingEventInfoList.GetActiveEditingEvent
  : TcxSchedulerControlEvent;
var
  I: Integer;
  AInfo: TcxSchedulerEditingEventInfo;
...
function TcxSchedulerEditingEventInfoList.GetItem(AIndex: Integer)
  : TcxSchedulerEditingEventInfo;
begin
...
var
  I: Integer;
begin
  if not ActualTimeRangeAvailable then
    Exit;
  BeginUpdate;
  try
...
begin
  Result := nil;
  if not(AType in [etCustom, etException]) or (APattern.EventType <> etPattern)
  then
    Exit;
  ACalculator := TcxSchedulerOccurrenceCalculator.Create(APattern,
    APattern.Start, ADate + 7);
  try
    while ACalculator.GetNextOccurrence do
    begin
      if APattern.GetOccurrenceByIndex(ACalculator.Index, AOccurrence) then
        Continue
      else if dxDateOf(ACalculator.OccurrenceStart) > dxDateOf(ADate) then
        Break
      else if dxDateOf(ACalculator.OccurrenceStart) = dxDateOf(ADate) then
      begin
        Result := CreateEvent;
...
function TcxCustomSchedulerStorage.FindAvailableAllDay(var AStart,
  AFinish: TDateTime; AResourceID: Variant;
...
begin
  ...
  try
    ...
    begin
      Result := not GetEvents(AList, I + AStartDay, I + ADuration - AStartDay,
        AResourceID);
      if not Result then
      begin
        Result := True;
...
function TcxCustomSchedulerStorage.FindAvailableTime(var AStart,
  AFinish: TDateTime; AllDay: Boolean; AResourceID: Variant;
  AExceptEventsWithoutResources: Boolean; ADuration: TDateTime = 0;
...
begin
  ...
  if AllDay then
    Result := FindAvailableAllDay(AStart, AFinish, AResourceID,
      AExceptEventsWithoutResources, ADuration)
  else
  begin
    AInfo := TcxSchedulerEventConflictsInfo.Create(Self,
      AExceptEventsWithoutResources, Trunc(AStart),
    ...
    try
      ...
      for I := 0 to AInfo.TimeRanges.Count - 1 do
        if (AInfo.TimeRanges[I].Duration >= ADuration) and
          (DateTimeHelper.RoundTime(AInfo.TimeRanges[I].Finish - ADuration) >=
          AStart) then
        begin
  ...
  begin
    ...
    begin
      ...
      Result := FindAvailableTime(AStart, AFinish, AEvent.AllDayEvent,
        AEvent.ResourceID, AExceptEventsWithoutResources,
        AEvent.Duration, AEvent);
      if Result then
      begin
    ...
    begin
      AEvent.BeginEditing;
      try
        AInfo := cxSchedulerEventConflictsInfoClass.Create(Self,
          AExceptEventsWithoutResources, Trunc(AEvent.Start),
          Trunc(AEvent.Finish) + 1, AEvent.ResourceID);
        try
          Result := False;
          for I := 1 to 24 * 6 do
          begin
...
procedure TcxCustomSchedulerStorage.GenerateHolidayEvents(const AResourceID
  : Variant);
begin
  GenerateHolidayEventsBySchedulerHolidays(AResourceID, Holidays);
end;

function TcxCustomSchedulerStorage.GetEventByID(const AID: Variant)
  : TcxSchedulerEvent;
var
  I: Integer;
begin
...
procedure TcxCustomSchedulerStorage.InternalGetEvents
  (AList: TcxSchedulerFilteredEventList; const AStart, AFinish: TDateTime;
  const AResourceID: Variant);
var
  I, J: Integer;
  AIsNull: Boolean;
  AEvent: TcxSchedulerEvent;
...
begin
  ...
  begin
    ...
    if not AList.UseTimeRange or AEvent.CheckTimeRange(AStartDate, AFinishDate)
    then
    begin
      ...
      begin
        for J := VarArrayLowBound(AResourceID, 1) to VarArrayHighBound
          (AResourceID, 1) do
          if AList.CheckEvent(AEvent, AResourceID[J], False) then
            Break;
      end;
    end
    else if AList.UseTimeRange and (AEvent.FInfo.DayStart > AFinishDate) then
      Break;
  end;
end;
...
function TcxCustomSchedulerStorage.GetFieldByName(const AName: string)
  : TcxCustomSchedulerStorageField;
begin
  Result := CustomFields.FindFieldByName(AName);
end;

function TcxCustomSchedulerStorage.GetHolidayNamesByDate(ADate: TDate;
  var ANames: string; AOnlyVisible: Boolean = True): Boolean;
begin
  Result := (Holidays <> nil) and Holidays.GetHolidayNamesByDate(ADate, ANames,
    AOnlyVisible);
end;

function TcxCustomSchedulerStorage.GetReminderEvents(ADateTime: TDateTime;
  AList: TcxSchedulerFilteredEventList): Boolean;
var
  I: Integer;
begin
  ...
  AList.Init(NullDate, ADateTime + 14, Self, True);
  // calculate for two weeks (= max AdvanceTime)
...
procedure TcxCustomSchedulerStorage.PopulateHolidayDates
  (AList: TcxSchedulerDateList; AStart, AFinish: TDate;
  AOnlyVisible: Boolean = True; AClearList: Boolean = True);
begin
  if AList <> nil then
  begin
    if AClearList then
    ...
    if Holidays <> nil then
      Holidays.PopulateHolidayDates(AList, AStart, AFinish, AOnlyVisible,
        AClearList);
  end;
end;
...
begin
  if not IsActive or (FNewEvents.Count = 0) then
    Exit;
  BeginUpdate;
  try
    while FNewEvents.Count <> 0 do
      FNewEvents.Last.Post;
  finally
    EndUpdate;
  end;
end;
...
function TcxCustomSchedulerStorage.GetEventID
  (AEvent: TcxSchedulerEvent): Variant;
begin
  Result := Null;
end;
...
begin
  Result := UseActualTimeRange and FActualStartField.IsActive and
    FActualFinishField.IsActive;
end;

function TcxCustomSchedulerStorage.AddEventRecord
  (AEvent: TcxSchedulerEvent): Boolean;
begin
  Result := False;
end;
...
procedure TcxCustomSchedulerStorage.AfterPostEditingData
  (AEvent: TcxSchedulerEvent);
begin
end;

procedure TcxCustomSchedulerStorage.BeforePostEditingData
  (AEvent: TcxSchedulerEvent);
begin
end;

procedure TcxCustomSchedulerStorage.BeforeDeleteEvent
  (AEvent: TcxSchedulerEvent);
begin
...
begin
  Result := AIndex in [FCaptionField.Index, FLabelColorField.Index,
    FLocationField.Index, FMessageField.Index, FOptionsField.Index,
    FReminderMinutesBeforeStartField.Index, FStateField.Index,
    FReminderDateField.Index, FReminderResourcesData.Index];
...
function TcxCustomSchedulerStorage.CreateFields
  : TcxCustomSchedulerStorageFields;
begin
  Result := TcxSchedulerStorageFields.Create(TcxSchedulerStorageField);
end;

procedure TcxCustomSchedulerStorage.CreateHolidayEvent
  (AHoliday: TcxSchedulerHolidaysLocationHoliday; AResourceID: Variant);
begin
  with CreateEvent do
  begin
...
function TcxCustomSchedulerStorage.CreateResources
  : TcxSchedulerStorageResources;
begin
...
begin
  ...
  FDataController := GetDataControllerClass.Create(Self)
    as TcxSchedulerStorageDataController;
...
function TcxCustomSchedulerStorage.DoFilterEvent
  (AEvent: TcxSchedulerEvent): Boolean;
begin
  Result := True;
...
procedure TcxCustomSchedulerStorage.DoUpdateStructure(ANeedUpdateInfo
  : Boolean = True);
begin
  UpdateStructure;
...
var
  I: Integer;
begin
  if (AHolidays = nil) or (AHolidays.Count = 0) then
    Exit;
  BeginUpdate;
  try
...
function TcxCustomSchedulerStorage.GetDataControllerClass
  : TcxCustomDataControllerClass;
begin
  Result := TcxSchedulerStorageDataController;
end;

function TcxCustomSchedulerStorage.GetEditingEventInfoListClass
  : TcxSchedulerEditingEventInfoListClass;
begin
...
function TcxCustomSchedulerStorage.GetLinksControllerClass
  : TcxSchedulerEventLinksControllerClass;
begin
...
function TcxCustomSchedulerStorage.GetEventValue(const AHandle: Pointer;
  AIndex: Integer): Variant;
begin
  if AHandle = nil then
    Result := Null
...
function TcxCustomSchedulerStorage.IsEventDeleting
  (AEvent: TcxSchedulerEvent): Boolean;
begin
...
procedure TcxCustomSchedulerStorage.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  if (Resources <> Nil) and (Operation = opRemove) and
    (AComponent = Resources.Images) then
...
begin
  ...
  try
    ...
    begin
      ...
      if DoEventInserted(AEvent) then
        Exit;
...
var
  AFieldIndex: Integer;
begin
  if DoEventModified(AEvent) then
    Exit;
...
begin
  if (LockCount <> 0) and ACheckLockCount then
    Exit;
  for I := Listeners.Count - 1 downto 0 do
    ...
    else if Supports(Listeners[I], IcxSchedulerStorageListener, AIntf) then
    begin
      if AIsRemoved then
        AIntf.StorageRemoved(Self)
      else if not IsUpdatingMode and not FInternalUpdate then
...
procedure TcxCustomSchedulerStorage.SetEventValue(AEvent: TcxSchedulerEvent;
  AFieldIndex: Integer; const AValue: Variant);
begin
...
begin
  ...
  if (AEvent.Pattern <> nil) and (AEvent.EventType = etCustom) and
    CanGetValueFromPattern(AFieldIndex) then
  begin
    ...
    if not VarEquals(AEventValue, APatternValue) or
      (not VarEquals(AEventValue, AStoredValue) and not VarIsNull(AStoredValue))
    then
...
begin
  if AFieldIndex < 0 then
    Exit;
  TcxDataStorageHelper.SetValue(ARecordHandle,
    Fields[AFieldIndex].ValueDef, AValue);
end;
...
begin
  ...
  for I := 0 to CustomFields.Count - 1 do
    TcxCustomSchedulerStorageField(CustomFields.Items[I]).FIndex :=
      I + InternalFields.Count;
...
function TcxCustomSchedulerStorage.GetDataField(AIndex: Integer)
  : TcxCustomSchedulerStorageField;
begin
...
function TcxCustomSchedulerStorage.GetField(AIndex: Integer)
  : TcxCustomSchedulerStorageField;
begin
  Result := TcxCustomSchedulerStorageField(FFields[AIndex]);
end;
...
begin
  Result := not(FIsLoading or (ComponentState * [csDesigning, csDestroying,
    csLoading] <> []) or IsLocked or IsStructureUpdating);
end;

function TcxCustomSchedulerStorage.GetIsUpdatingMode: Boolean;
begin
  Result := FIsLoading or ([csLoading, csDestroying] * ComponentState <> []) or
    (FStructureUpdatingCount > 0);
end;
...
function TcxCustomSchedulerStorage.GetSortedList
  : TdxSortedList<TcxSchedulerEvent>;
begin
  Result := FSortedList;
...
begin
  ...
  else if Reminders.Active then
    Reminders.StartReminder;
  LayoutChanged;
end;

procedure TcxCustomSchedulerStorage.SetEvent(AIndex: Integer;
AValue: TcxSchedulerEvent);
begin
...
begin
  ...
  AStream.WriteBuffer(cxSchedulerStreamVersion,
    SizeOf(cxSchedulerStreamVersion));
...
begin
  inherited DefineProperties(Filer);
  Filer.DefineBinaryProperty('BinaryData', LoadFromStream, SaveToStream,
    EventCount > 0);
end;
...
procedure TcxSchedulerStorage.SetCustomFields(const AValue
  : TcxSchedulerStorageFields);
begin
...
constructor TcxSchedulerControlEventID.Create
...
function TcxSchedulerControlEventID.SameEvent
  (AEvent: TcxSchedulerEvent): Boolean;
begin
  Result := VarEquals(AEvent.ID, ID) and VarEquals(AEvent.ParentID, ParentID)
    and (AEvent.RecurrenceIndex = RecurrenceIndex) and
    VarEquals(cxGetEventID(AEvent.Pattern), PatternID);
end;

function TcxSchedulerControlEventID.Equals
  (AValue: TcxSchedulerControlEventID): Boolean;
begin
  Result := (AValue = Self) or VarEquals(AValue.ID, ID) and
    VarEquals(AValue.ParentID, ParentID) and
    (AValue.RecurrenceIndex = RecurrenceIndex) and
    VarEquals(AValue.PatternID, PatternID);
end;

{ TcxSchedulerEventSelection }

constructor TcxSchedulerEventSelection.Create(ASourceEvents
  : TcxSchedulerEventList);
begin
  inherited Create;
  FKeys := TList.Create;
...
procedure TcxSchedulerEventSelection.Add(AEvent: TcxSchedulerControlEvent;
AShift: TShiftState);
var
  AIndex: Integer;
  ASelected: Boolean;
begin
  ...
  if ASelected and (AShift = [ssRight]) then
    Exit;
...
function TcxSchedulerEventSelection.CreateItem(AEvent: TcxSchedulerEvent)
  : TcxSchedulerControlEventID;
begin
...
function TcxSchedulerEventSelection.GetItem(AIndex: Integer)
  : TcxSchedulerControlEvent;
begin
  Result := TcxSchedulerControlEvent(FEvents.List[AIndex])
end;

function TcxSchedulerEventSelection.GetKey(AIndex: Integer)
  : TcxSchedulerControlEventID;
begin
...
procedure TcxSchedulerEventList.Assign(ASource: TcxSchedulerEventList;
...
begin
  if AEvent.Source <> nil then
    AEvent.FTimeBias := GetTimeBias + GetTimeBiasDaylightSavingTime
      (AEvent.Source.Start)
  else
    AEvent.FTimeBias := GetTimeBias + GetTimeBiasDaylightSavingTime
      (AEvent.Start);
...
function TcxSchedulerFilteredEventList.AddEvent(AEvent: TcxSchedulerEvent)
  : TcxSchedulerControlEvent;
begin
...
begin
  ...
  if not CanAddEvent(ACalculator.Event, ACalculator.Index) then
    Exit;
  if ReminderEventsOnly and not ACalculator.Event.Reminder then
    Exit;
  begin
    ...
    begin
      ...
      begin
        if (ACalculator.FOccurrence <> nil) and
          (dxDateOf(ACalculator.ReminderStart) < dxDateOf(FNow)) then
        begin
    ...
    else if not UseTimeRange or (ACalculator.OccurrenceStart <= Finish) and
      (ACalculator.OccurrenceFinish >= Start) then
      Result := AddEvent(ACalculator.Event);
    if Result = nil then
      Exit;
    if ACalculator.Event.RecordHandle <> nil then
      Result.ParentID := ACalculator.Event.ID;
    Result.UTCStart := ACalculator.OccurrenceStart +
      ACalculator.Event.RecurrenceInfo.DisplayTimeBias;
    Result.UTCFinish := ACalculator.OccurrenceFinish +
      ACalculator.Event.RecurrenceInfo.DisplayTimeBias;
    Result.RecurrenceIndex := ACalculator.Index;
    Result.EventType := etOccurrence;
...
function TcxSchedulerFilteredEventList.CanAddEvent(ASource: TcxSchedulerEvent;
ARecurrenceIndex: Integer): Boolean;
begin
  Result := True;
end;

function TcxSchedulerFilteredEventList.CanSmartUpdate
  (AStorage: TcxCustomSchedulerStorage; const AStart, AFinish: TDateTime;
const AResourceID: Variant): Boolean;
begin
...
function TcxSchedulerFilteredEventList.CheckEvent(AEvent: TcxSchedulerEvent;
const AResourceID: Variant; AddAll: Boolean): Boolean;
begin
  Result := False;
  if AEvent.EventType in [etNone, etCustom] then
    Result := CheckSimpleEvent(AEvent, AResourceID, AddAll)
  else if (AEvent.EventType = etPattern) or AEvent.SkipExceptions then
    Result := CheckRecurrenceEvent(AEvent, AResourceID, AddAll);
end;

function TcxSchedulerFilteredEventList.CheckResourceID(const AEventResourceID,
  AResourceID: Variant): Boolean;
begin
...
begin
  ...
  if not AEvent.FInfo.Visible then
    Exit;
  if AddAll or CheckResourceID(AEvent.ResourceID, AResourceID) or
    AEvent.IsSharedWithResource(AResourceID) then
  begin
    ...
    ACalculator := TcxSchedulerOccurrenceCalculator.Create(AEvent, AEvent.Start,
      AFinish, ReminderEventsOnly);
    try
      while ACalculator.GetNextOccurrence do
      begin
...
begin
  Result := False;
  if not Storage.DoFilterEvent(AEvent) or
    not CanAddEvent(AEvent, AEvent.RecurrenceIndex) then
    Exit;
  ...
  else if not UseTimeRange or ((AEvent.Start <= Finish) and
    (AEvent.Finish >= Start)) then
    if AddAll or (CheckResourceID(AEvent.ResourceID, AResourceID) or
      AEvent.IsSharedWithResource(AResourceID)) then
    begin
...
begin
  Result := (ACalculator.DismissDate < dxDateOf(Now)) and
...
function TcxSchedulerFilteredEventList.GetEvent(AIndex: Integer)
  : TcxSchedulerControlEvent;
begin
...
procedure TcxSchedulerCachedEventList.AfterEditing
  (AEvent: TcxSchedulerControlEvent);
begin
...
begin
  FExpandedTimeRange := CalculateNecessaryDate(FMinNecessaryDate,
    FMaxNecessaryDate);
end;

procedure TcxSchedulerCachedEventList.DeleteClone
  (AClone: TcxSchedulerControlEvent);
begin
...
procedure TcxSchedulerCachedEventList.CalculateClonesRange(var AMinDate,
  AMaxDate: TDateTime);
var
  I: Integer;
begin
...
function TcxSchedulerCachedEventList.CalculateNecessaryDate(var AMinDate,
  AMaxDate: TDateTime): Boolean;
var
  AMin, AMax: TDateTime;
begin
...
procedure TcxSchedulerCachedEventList.CalculateSelectionRange(var AMinDate,
  AMaxDate: TDateTime);
var
  I: Integer;
begin
...
function TcxSchedulerCachedEventList.CreateEvent(AInsertToList: Boolean)
  : TcxSchedulerControlEvent;
begin
...
procedure TcxSchedulerCachedEventList.ExtractEvents(const ADate: TDateTime;
AList: TcxSchedulerEventList);
var
  ADateNoTime: Integer;
begin
  ...
  TdxMultithreadedListHelper.IterateItems(FAbsoluteItems,
    ...
    begin
      if TcxSchedulerEvent(AEvent).IsDayEvent(ADateNoTime) and
        (AList.IndexOf(AEvent) = -1) then
        AList.List.List^[AItemIndex] := AEvent;
    end);
  AList.List.Pack();
end;

procedure TcxSchedulerCachedEventList.ExtractEventsByResource
  (AList: TcxSchedulerEventList;
...
begin
  ...
  TdxMultithreadedListHelper.IterateItems(FAbsoluteItems,
    procedure(AEvent: Pointer; AItemIndex: Integer)
    begin
      if TcxSchedulerEvent(AEvent).IsResourceEvent(AResource, AllowUnassigned)
      then
...
procedure TcxSchedulerCachedEventList.ExtractUsedDays
...
begin
  ...
  begin
    for I := 0 to Count - 1 do
      for J := Trunc(AbsoluteItems[I].Start)
        to Trunc(AbsoluteItems[I].Finish) do
        if AList.IndexOf(J) < 0 then
          AList.Add(J);
  end
end;

function TcxSchedulerCachedEventList.HasConflict(IsDragCopy: Boolean;
AStartDrag: Boolean): Boolean;
begin
  if AStartDrag then
    Result := (Selection.Count > 1) and HasIntersection(Selection.FEvents)
  else if (Selection.Count = 1) and (Clones.Count = 0) then
...
begin
  ...
  for I := 0 to AList.Count - 1 do
    for J := 0 to AList.Count - 1 do
    begin
      Result := (I <> J) and IsIntersect(TcxSchedulerEvent(AList[I]),
        TcxSchedulerEvent(AList[J]));
      if Result then
        Exit;
    end;
end;

function TcxSchedulerCachedEventList.HasIntersection(AList1,
  AList2: TcxSchedulerEventList; AExcludeEquals: Boolean): Boolean;
var
  I, J: Integer;
begin
  Result := False;
  for I := 0 to AList1.Count - 1 do
    ...
    begin
      if AExcludeEquals and (AList1[I] = TcxSchedulerControlEvent(AList2[J])
        .Source) then
        Continue;
...
function TcxSchedulerCachedEventList.PostEvent(AEvent: TcxSchedulerControlEvent)
  : TcxSchedulerEvent;
begin
  Result := nil;
  ...
  try
    ...
    else if AEvent.EventType in [etNone, etCustom] then
      Result := PostCloneForSimpleEvent(AEvent, False, AEvent.EventType)
    else if AEvent.EventType = etOccurrence then
...
function TcxSchedulerCachedEventList.CanAddEvent(ASource: TcxSchedulerEvent;
ARecurrenceIndex: Integer): Boolean;
  ...
  begin
    ...
    begin
      Result := (TcxSchedulerControlEvent(FItems.List[I]).Source = ASource) and
        (ARecurrenceIndex = TcxSchedulerControlEvent(FItems.List[I])
        .RecurrenceIndex);
      if Result then
        Break;
    end;
...
begin
  ...
  begin
    ...
    else if ASource.CheckTimeRange(FCheckStart, FCheckFinish) then
      Result := not FDictionary.ContainsKey(ASource);
  end;
end;

function TcxSchedulerCachedEventList.CanSmartUpdate
  (AStorage: TcxCustomSchedulerStorage; const AStart, AFinish: TDateTime;
const AResourceID: Variant): Boolean;
var
  I: Integer;
begin
  Result := not FNeedForceUpdate and UseTimeRange and
    ((AFinish > Start) and (AStart < Finish)) and
    VarEqualsExact(FResourceID, AResourceID) and
    ((Storage <> nil) and (Storage.Listeners.IndexOf(Self) >= 0));
...
function TcxSchedulerCachedEventList.CheckEvent(AEvent: TcxSchedulerEvent;
const AResourceID: Variant; AddAll: Boolean): Boolean;
begin
  Result := (AEvent.FInfo.Visible and (AddAll or CheckEventVisibility(AEvent,
    ShowEventsWithoutResource))) and inherited CheckEvent(AEvent,
    AResourceID, AddAll);
end;
...
function TcxSchedulerCachedEventList.CreateCloneList
  : TcxSchedulerFilteredEventList;
begin
  Result := TcxSchedulerFilteredEventList.Create;
end;

function TcxSchedulerCachedEventList.CreateSelection
  : TcxSchedulerEventSelection;
begin
...
begin
  ...
  if DaylightSaving then
    Result := TcxSchedulerDateTimeHelper.TimeZoneDaylightBias(ATime, FTimeZone)
      * MinuteToTime;
end;
...
begin
  ...
  if FExpandedTimeRange then
    inherited Init(Min(AStart, FMinNecessaryDate),
      Max(AFinish, FMaxNecessaryDate), AStorage, AClearItems)
  else
...
function TcxSchedulerCachedEventList.IsIntersect(AEvent1,
  AEvent2: TcxSchedulerEvent): Boolean;
begin
  Result := not(AEvent1.IsFreeState or AEvent2.IsFreeState);
  if Result then
    Result := (((AEvent1.Start < AEvent2.Finish) and
      (AEvent2.Start < AEvent1.Finish)) or ((AEvent2.Start < AEvent1.Finish) and
      (AEvent1.Start < AEvent2.Finish)));
  if Result then
    Result := (VarIsNull(AEvent2.ResourceID) or AEvent1.IsSharedWithResource
      (AEvent2.ResourceID)) or (VarIsNull(AEvent1.ResourceID) or
      AEvent2.IsSharedWithResource(AEvent1.ResourceID));
...
begin
  ...
  AProc := procedure(AItem: Pointer; AIndex: Integer)
    var
      AEvent: TcxSchedulerEvent;
    begin
      AEvent := TcxSchedulerEvent(AItem);
      if not UseTimeRange or ((AEvent.Start <= SelFinish) and
        (SelStart <= AEvent.Finish)) then
      begin
...
function TcxSchedulerCachedEventList.QueryInterface(const IID: TGUID;
out Obj): HResult;
const
...
function TcxSchedulerCachedEventList.GetAbsoluteItem(AIndex: Integer)
  : TcxSchedulerControlEvent;
begin
...
function TcxSchedulerCachedEventList.GetItem(AIndex: Integer)
  : TcxSchedulerControlEvent;
begin
  Result := TcxSchedulerControlEvent(FItems.List[AIndex])
end;
...
begin
  Result := FShowEventsWithoutResource or
    Storage.Resources.IsNullResourceVisible;
end;
...
procedure TcxSchedulerCachedEventList.SetShowEventsWithoutResource
  (const AValue: Boolean);
begin
  FNeedForceUpdate := FNeedForceUpdate or
    (AValue <> FShowEventsWithoutResource);
...
procedure TcxSchedulerOccurrenceCalculator.CalcNearestOccurrenceIntervals
  (AStart, AFinish: TDateTime; var AnIntervalBefore, AnIntervalAfter
  : TDateTime);
...
function TcxSchedulerOccurrenceCalculator.ValidateTimeBoundsForOccurrence
  (AEvent: TcxSchedulerEvent; AOldStart, AOldFinish, ANewStart,
  ANewFinish: TDateTime): TcxOccurrenceDateStatus;
var
  ...
  AOccurrenceStart, AOccurrenceFinish: TDateTime;
  AAllDayEvent: Boolean): Boolean;
  begin
    Result := False;
    ...
    else if AOccurrenceFinish < AOldStart then
      AMinDate := Max(AMinDate, AOccurrenceFinish)
    else if AOldFinish < AOccurrenceStart then
    begin
...
begin
  ...
  begin
    if (AOccurrence.EventType = etCustom) and
      (AOccurrence.RecurrenceIndex <> AOccurrenceIndex) then
    begin
      CheckOccurrence(Result, AOccurrence.Start, AOccurrence.Finish,
        AOccurrence.AllDayEvent);
  ...
  while GetNextOccurrence do
  begin
    if (Index <> AOccurrenceIndex) and not AEvent.Pattern.GetOccurrenceByIndex
      (Index, AOccurrence) then
    begin
      if CheckOccurrence(Result, OccurrenceStart, OccurrenceFinish, AAllDayEvent)
      then
      begin
        if Result = odsOccupied then
          Exit
...
begin
  Result := 0;
  InitTimes;
  while GetNextOccurrence and
    (dxDateOf(FOccurrenceStart) <= dxDateOf(AEndDate)) do
    Inc(Result);
end;
...
function TcxSchedulerOccurrenceCalculator.GetRecurrenceInfo
...
begin
  ...
  begin
    FOccurrenceStart := Event.Pattern.Start -
      Event.RecurrenceInfo.DisplayTimeBias + Event.TimeBias;
...
class procedure TcxSchedulerContentNavigationCalculator.FindNavigationIntervals
  (AStorage: TcxCustomSchedulerStorage; AContentNavigationInfo: TObjectList;
AStart, AFinish: TDateTime; AWithoutResources: Boolean;
AShowEventsWithoutResource: Boolean; AScaleUnit: TDateTime);

  procedure ProcessEventsWithoutResource(AnEventIndex: Integer);
  begin
    if AShowEventsWithoutResource and not IsEventSharedWithAnyResource(AStorage,
      AnEventIndex, False) or (AStorage.ResourceCount = 0) then
      CalcIntervals(AStorage.FEventsList[AnEventIndex],
        TcxSchedulerContentNavigationInfo(AContentNavigationInfo
        [AContentNavigationInfo.Count - 1]), AStart, AFinish, AScaleUnit);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        AResourceIndex := 0;
        AResourceInfoCount := AContentNavigationInfo.Count -
          IfThen(AShowEventsWithoutResource, 1);
        while AResourceIndex < AResourceInfoCount do
        begin
          if AStorage.FEventsList[AnEventIndex].IsSharedWithResource
            (TcxSchedulerContentNavigationInfo(AContentNavigationInfo
            [AResourceIndex]).FResourceID) then
            CalcIntervals(AStorage.FEventsList[AnEventIndex],
              TcxSchedulerContentNavigationInfo(AContentNavigationInfo
              [AResourceIndex]), AStart, AFinish, AScaleUnit);
          Inc(AResourceIndex);
        end;
...
begin
  if (AStorage = nil) or (AContentNavigationInfo = nil) then
    Exit;
  AShowEventsWithoutResource := AShowEventsWithoutResource or
    AStorage.Resources.IsNullResourceVisible;
  if AShowEventsWithoutResource or (AContentNavigationInfo.Count = 0) and
  ...
  case AnEvent.EventType of
    ...
    etOccurrence, etCustom:
      if AnEvent.Pattern <> nil then
        CalcAppointmentIntervals(AnEvent, AInfo, AStart, AFinish, AScaleUnit);
    etPattern:
      CalcPatternIntervals(AnEvent, AInfo, AStart, AFinish);
  end;
end;
...
begin
  AOccurrenceCalculator := TcxSchedulerOccurrenceCalculator.Create(AnEvent,
    dxDateOf(MinDateTime), cxMaxDate);
  try
...
class procedure TcxSchedulerContentNavigationCalculator.
  ChangeResourceNavigationIntervals(AContentNavigationInfo: TObjectList);
var
  I: Integer;
...
begin
  ...
  begin
    TcxSchedulerContentNavigationInfo(AContentNavigationInfo[I]).FIntervalBefore
      := Min(TcxSchedulerContentNavigationInfo(AContentNavigationInfo[I])
      .FIntervalBefore, ABefore);
    TcxSchedulerContentNavigationInfo(AContentNavigationInfo[I]).FIntervalAfter
      := Min(TcxSchedulerContentNavigationInfo(AContentNavigationInfo[I])
      .FIntervalAfter, AnAfter);
  end;
end;

class function TcxSchedulerContentNavigationCalculator.
  IsEventSharedWithAnyResource(AStorage: TcxCustomSchedulerStorage;
...
begin
  ...
  begin
    Result := AnEvent.IsSharedWithResource(AStorage.ResourceIDs[AResourceIndex])
      and (not ConsiderVisibility or AStorage.Resources.Items
      [AResourceIndex].Visible);
    Inc(AResourceIndex);
  end;
end;
...
begin
  Result := Reminders.ReminderWindowLookAndFeel.Painter.
    DefaultSchedulerControlColor;
end;

function TcxSchedulerCustomReminderForm.IsLocked: Boolean;
begin
  Result := (csDestroying in ComponentState) or
    (Reminders.Storage.ComponentState * [csLoading, csDestroying] <> []) or
    Reminders.NeedRestoreSelection or Reminders.IsLocked;
end;
...
constructor TcxSchedulerReminderEventID.Create
  (AEvent: TcxSchedulerControlEvent);
var
  I: Integer;
begin
...
begin
  ...
  if Result then
    for I := Low(Values) to High(Values) do
      if AEvent.Storage.Fields[I].IsUnique and not VarEquals(AEvent.Values[I],
        Values[I]) then
      begin
        Result := False;
        Break;
...
procedure TcxSchedulerEventReminders.AddReminder
...
begin
  ...
  begin
    ...
    if (AReminder.ReminderDate = AReminderDate) and
      VarEquals(AReminder.ResourceID, AResourceID) then
    begin
      Result := AReminder;
      Break;
    end;
...
procedure TcxSchedulerEventRemindersList.Remove(AEventReminders
  : TcxSchedulerEventReminders);
begin
...
function TcxSchedulerEventRemindersList.GetItem(AIndex: Integer)
  : TcxSchedulerEventReminders;
begin
...
constructor TcxSchedulerReminder.Create(AOwner: TcxSchedulerReminders;
AEvent: TcxSchedulerControlEvent);
begin
  inherited Create;
  FOwner := AOwner;
...
procedure TcxSchedulerReminder.SetData(const AResourceID: Variant;
...
begin
  if ASnoozeTime > 0 then
    ATime := DateTimeHelper.RoundTime(Owner.GetNow + Max(ASnoozeTime,
      MinuteToTime))
  else
  begin
    ATime := DateTimeHelper.RoundTime(Event.Start + ASnoozeTime);
    if ATime <= Owner.GetNow then
      Exit;
  end;
  if Owner.IsReminderByResourceAvailable and Event.Shared then
  ...
  begin
    ...
    begin
      ...
      Event.Pattern.RecurrenceInfo.DismissDate := dxDateOf(ATime) -
        MinuteToTime;
...
function TcxSchedulerReminder.GetResourcesData
  : TcxSchedulerReminderResourcesData;
begin
...
constructor TcxSchedulerReminderEventsList.Create
...
begin
  Result := TcxSchedulerDateTimeHelper.TimeZoneBiasDelta(Reminders.TimeZone) +
    Storage.TimeBias
end;

function TcxSchedulerReminderEventsList.GetTimeBiasDaylightSavingTime
  (ATime: TDateTime): TDateTime;
begin
  Result := 0;
  if Reminders.TimeZoneDaylightSaving then
    Result := TcxSchedulerDateTimeHelper.TimeZoneDaylightBias(ATime,
      Reminders.TimeZone) * MinuteToTime;
end;
...
begin
  Result := (ReminderWindow <> nil) and
    not(csDestroying in ReminderWindow.ComponentState) and
    ReminderWindow.Showing;
end;
...
procedure TcxSchedulerReminders.AddReminder(AEvent: TcxSchedulerControlEvent;
const AResourceID: Variant; AReminderDate: TDateTime;
var AEventReminders: TcxSchedulerEventReminders);
var
  AReminder: TcxSchedulerReminder;
...
begin
  if AEventReminders = nil then
    AEventReminders := FEventRemindersList.CreateEventReminders
      (AReminder.Event);
...
begin
  ...
  begin
    ...
    CheckAddReminder(AEvent, AEvent.ResourceID, AEvent.ReminderDate,
      AEventReminders);
end;

procedure TcxSchedulerReminders.CheckRemindersForEvent
  (AEvent: TcxSchedulerControlEvent);
var
  AEventReminders: TcxSchedulerEventReminders;
begin
...
function TcxSchedulerReminders.CreateReminderWindow
  : TcxSchedulerCustomReminderForm;
begin
...
procedure TcxSchedulerReminders.DoGetEventDueTimeText
  (AReminder: TcxSchedulerReminder;
const ADueTimeInfo: TcxSchedulerReminderDueTimeInfo; var AText: string);
begin
...
function TcxSchedulerReminders.DoSnoozeReminder(AReminder: TcxSchedulerReminder;
var ASnoozeTime: TDateTime): Boolean;
begin
  Result := False;
...
function TcxSchedulerReminders.GetNextPopulateEventsCacheTime(ANow: TDateTime;
AIsLocked: Boolean): TDateTime;
begin
...
function TcxSchedulerReminders.GetNextRefreshRemindersTime(ANow: TDateTime;
AIsLocked: Boolean): TDateTime;
var
  I: Integer;
  AReminderDate: TDateTime;
...
begin
  ...
  if TimeZoneDaylightSaving then
    Result := Result + DateTimeHelper.TimeZoneDaylightBias(Result, TimeZone) *
      MinuteToTime;
end;
...
function TcxSchedulerReminders.GetReminderResourcesFromEvent
  (AEvent: TcxSchedulerControlEvent; AGetFromPattern: Boolean)
  : TcxSchedulerReminderResources;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if not AFound then
      begin
        AEventStart := AEvent.Start;
        if dxDateOf(AEventStart) > dxDateOf(ANow) then
        begin
          if AEventStart - AEvent.ReminderMinutesBeforeStart * MinuteToTime < ANow
          then
        ...
        begin
          if dxDateOf(AEventStart) = dxDateOf(ANow) then
            AOccurrenceEvent := AEvent
          else if AOccurrenceEvent = nil then
            AOccurrenceEvent := AEvent;
          AFound := True;
        end;
      end;
...
procedure TcxSchedulerReminders.ValidateRemindersForEvent(AEventReminders
  : TcxSchedulerEventReminders; AEvent: TcxSchedulerControlEvent);

  procedure ValidateResourceReminder(AEventReminders
    : TcxSchedulerEventReminders; AEvent: TcxSchedulerControlEvent;
  const AResourceID: Variant; AReminderDate: TDateTime);
  var
    AReminder: TcxSchedulerReminder;
  begin
    ...
    else if IsNeedAddToReminders(AEvent, AReminderDate) then
...
begin
  ...
  begin
    ...
    for I := Low(AReminderResources) to High(AReminderResources) do
      ...
      begin
        if not AIsOccurrence or (dxDateOf(GetNow) > DismissDate) then
          ValidateResourceReminder(AEventReminders, AEvent, ResourceID,
            ReminderDate);
      end;
  end
  else
    ValidateResourceReminder(AEventReminders, AEvent, AEvent.ResourceID,
      AEvent.ReminderDate);
...
begin
  ...
  try
    ...
    else if FNeedShowWindow and HasReminders and not IsLocked then
      ShowReminderWindow;
    FNeedShowWindow := False;
  finally
...
procedure TcxSchedulerReminders.UpdateTimer(ATimer: TcxTimer;
AInterval: TDateTime);
begin
  if Active then
  begin
...
procedure TcxSchedulerReminders.SetShowResourcesInReminderWindow
  (AValue: Boolean);
begin
...
begin
  if (csDesigning in Storage.ComponentState) then
    Exit;
...
begin
  ...
  try
    if ABLOBFormat or (VarIsArray(AValue) and (VarArrayHighBound(AValue, 1) > 0))
    then
    begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (AReader.ReadInteger = (varArray or varVariant)) and
          (AReader.ReadInteger = AStream.Size) then
        begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (Result.Version = 3) and (ACount > 0) and
          (AReader.ReadInteger = AStream.Size) then
        begin
          SetLength(Result.Resources, ACount);
          for I := 0 to ACount - 1 do
...
begin
  case AStatus of
    odsOccupied:
      Result := Format(cxGetResourceString(@scxTwoOccurrencesPerDay),
        [AEvent.Caption]);
    odsIntersectRange:
      Result := Format(cxGetResourceString(@scxCannotRescheduleOccurrence),
        [AEvent.Caption]);
  else
    Result := '';
  end;
...
var
  ...
  ADataValue: Variant;
begin
  if VarIsNull(AValue) and (ALinks.Count = 0) then
    Exit;
  ALinks.BeginUpdate;
  try
    ALinks.Clear;
...
ACustomFieldCount: Integer = 0): Double;
  ...
  function TryRead(const AOriginal: AnsiString; var ABuffer;
