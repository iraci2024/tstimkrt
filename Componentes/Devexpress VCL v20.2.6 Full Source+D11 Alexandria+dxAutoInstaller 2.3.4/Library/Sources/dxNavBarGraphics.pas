...
{$ENDIF}
  ...
  cxClasses, cxControls, cxGraphics, dxNavBar, dxNavBarStyles,
...
procedure dxNavBarDrawSelectedFrame(ACanvas: TCanvas; const ARect: TRect;
...
procedure GdipFillGradientRect(DC: HDC; const ARect: TRect;
...
procedure TdxStyleSheetParser.ParseBackgroundColor(var AColor: TColor;
  AStyleName: string);
var
  AStyleText, AStyleAttributeText: string;
begin
  AStyleText := StringReplace(GetStyleText(AStyleName), ' ', '',
    [rfReplaceAll]);
  if AStyleText <> '' then
  begin
    AStyleAttributeText := GetStyleAttributeText(AStyleText,
      'BACKGROUND-COLOR:');
    ParseColor(AColor, StringReplace(AStyleAttributeText, ' ', '',
      [rfReplaceAll]));
  end
end;

procedure TdxStyleSheetParser.ParseBorderColor(var AColor: TColor;
  AStyleName: string);
var
  APos: Integer;
  AStyleText, AStyleAttributeText: string;
begin
  ...
  begin
    ...
    AStyleAttributeText := StringReplace(AStyleAttributeText, '''', '',
      [rfReplaceAll]);
...
procedure TdxStyleSheetParser.ParseFontDecoration(AFont: TFont;
  AStyleText: string);
var
  AFontDecoration: string;
begin
  AFontDecoration := UpperCase(GetStyleAttributeText(AStyleText,
    'TEXT-DECORATION:'));
...
function TdxStyleSheetParser.GetStyleAttributeText(AStyleText,
...
procedure TdxStyleSheetParser.ParseColorName(var AColor: TColor;
  AAttributeText: string);
begin
  cxColorByName(AAttributeText, AColor);
end;

procedure TdxStyleSheetParser.ParseColor(var AColor: TColor;
  AAttributeText: string);
var
  APos: Integer;
  R, G, B: Byte;
...
function TdxStyleSheetParser.MiddleString(AText: string;
  AStartPos, AEndPos: Integer): string;
begin
...
const
  Colors: array [TXPSchemeColor, 0 .. 11] of TColor =
    (($E7A27B, $D67563, $C65D21, $FF8E42, $F7DFD6, $FFFFFF, $F7D3C6, $B54900,
    $CE5D29, $A53C00, $FFCDAC, $FF8E42), ($ADD9CC, $84BDA5, $2D6656, $1D9272,
    $ECF6F6, $ECFCFF, $B8E7E0, $408C77, $67A896, $1C674B, $BEEDDD, $B8E7E0),
    ($D4C8C4, $C8B3B1, $3D3D3F, $7C7C7E, $F5F1F0, $FFFFFF, $E0D7D6, $927777,
    $C7B6B4, $734431, $FEFEF9, $E6E6E6));
var
...
begin
  ...
  begin
    ...
    dxAdvExplorerBarSpecialGroupCaptionSignColor :=
      dxAdvExplorerBarGroupCaptionSignColor; // clWhite;
    dxAdvExplorerBarSpecialGroupCaptionSignHotColor :=
      dxAdvExplorerBarGroupCaptionSignHotColor; // Colors[AColorScheme, 10];
  ...
  begin
    ...
    dxAdvExplorerBarSpecialGroupCaptionColor2 :=
      dxAdvExplorerBarSpecialGroupCaptionColor1;
    dxAdvExplorerBarGroupCaptionSignColor := dxAdvExplorerBarGroupFontColor;
    dxAdvExplorerBarGroupCaptionSignHotColor :=
      dxAdvExplorerBarGroupFontHotColor;
    dxAdvExplorerBarSpecialGroupCaptionSignColor :=
      dxAdvExplorerBarSpecialGroupCaptionColor2;
    dxAdvExplorerBarSpecialGroupCaptionSignHotColor :=
      dxAdvExplorerBarSpecialGroupCaptionColor2;
    dxAdvExplorerBarSpecialGroupFontColor := ColorToRGB(clCaptionText);
    dxAdvExplorerBarSpecialGroupFontHotColor :=
      dxAdvExplorerBarSpecialGroupFontColor;
  end;
end;
...
begin
  ...
  dxAdvExplorerBarGroupCaptionCollapseSignBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBARADVEXPLORERBARGROUPCAPTIONCOLLAPSESIGN');
  dxAdvExplorerBarGroupCaptionExpandSignBitmap := TBitmap.Create;
  dxAdvExplorerBarGroupCaptionExpandSignBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBARADVEXPLORERBARGROUPCAPTIONEXPANDSIGN');
...
const
  Office11Colors: array [TXPSchemeColor, 0 .. 8] of TColor =
    (($FADAC4, $C97A4A, $FEECDD, $F1D0BA, $F9DBC4, $E08F65, $FFFFFF, $862D00,
    $B23D00), ($E7F2F3, $98C6BE, $FFFFFF, $E7F2F3, $AEDFD2, $80B0A1, $FFFFFF,
    $466B5A, $5E8674), ($F4EEEE, $C3B0B1, $FCFBFB, $F5F0F0, $DFD0D0, $BFA8A9,
    $FFFFFF, $795B5C, $99797A));
var
...
begin
  ...
  dxOffice11CaptionExpandSignBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11EXPANDSIGN');
  dxOffice11CaptionCollapseSignBitmap := TBitmap.Create;
  dxOffice11CaptionCollapseSignBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11COLLAPSESIGN');
end;
...
begin
  ...
  dxOffice11NavPaneOverflowPanelBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11NAVPANEOVERFLOW');
  dxOffice11NavPaneOverflowPanelBitmap.Transparent := True;
  dxOffice11NavPaneSplitterBitmap := TBitmap.Create;
  dxOffice11NavPaneSplitterBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11NAVPANESPLITTER');
  dxOffice11NavPaneSplitterBitmap.Transparent := True;
  dxOffice11NavPaneArrowUpBitmap := TBitmap.Create;
  dxOffice11NavPaneArrowUpBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11NAVPANEARROWUP');
  dxOffice11NavPaneArrowUpBitmap.Transparent := True;
  dxOffice11NavPaneArrowDownBitmap := TBitmap.Create;
  dxOffice11NavPaneArrowDownBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11NAVPANEARROWDOWN');
  dxOffice11NavPaneArrowDownBitmap.Transparent := True;
  dxOffice11NavPaneDefaultLargeBitmap := TBitmap.Create;
  dxOffice11NavPaneDefaultLargeBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11NAVPANEDEFAULTLARGE');
  dxOffice11NavPaneDefaultLargeBitmap.Transparent := True;
  dxOffice11NavPaneDefaultSmallBitmap := TBitmap.Create;
  dxOffice11NavPaneDefaultSmallBitmap.Handle :=
    LoadBitmap(HInstance, 'DXNAVBAROFFICE11NAVPANEDEFAULTSMALL');
...
begin
  ...
  if FShellLibrary <> 0 then
    FSHGetShellStyleHInstancePointer := GetProcAddress(FShellLibrary,
...
begin
  ...
  begin
    ...
    DrawComplexFrame(cxRectInflate(ARect, -1, -1), ATopLeftInnerColor,
      ABottomRightInnerColor, GetBorders(ATopLeftInnerColor,
      ABottomRightInnerColor));
    Free;
  end;
end;
