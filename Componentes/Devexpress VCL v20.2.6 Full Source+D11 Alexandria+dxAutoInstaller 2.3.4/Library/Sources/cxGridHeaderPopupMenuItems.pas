...
type
  ...
  public
    class function GetParentOperationClass
      : TcxGridPopupMenuOperationClass; override;
  end;
...
uses
  cxCustomData, cxEdit, cxGridCustomTableView, cxGridTableView,
...
begin
  case Tag of
    0:
      HitColumn.GetProperties.Alignment.Horz := taLeftJustify;
    1:
      HitColumn.GetProperties.Alignment.Horz := taRightJustify;
    2:
      HitColumn.GetProperties.Alignment.Horz := taCenter;
  end;
end;
...
begin
  ...
  case Tag of
    0:
      Result := AProperties.Alignment.Horz = taLeftJustify;
    1:
      Result := AProperties.Alignment.Horz = taRightJustify;
  else
...
class function TcxGridAlign.GetParentOperationClass
...
begin
  Result := HitColumn.Visible and HitColumn.VisibleForCustomization and
    TcxCustomGridTableItemAccess.CanHide(HitColumn);
end;
...
begin
  Result := HitGridView.OptionsCustomize.ColumnExpressionEditing and
    HitColumn.Options.ExpressionEditing and
    TcxDataControllerAccess(HitGridView.DataController).IsItemSupportsExpression
    (HitColumn.Index);
end;

function TcxGridShowExpressionEditor.GetImageResourceName: string;
begin
  Result := AImageName + '13';
end;
