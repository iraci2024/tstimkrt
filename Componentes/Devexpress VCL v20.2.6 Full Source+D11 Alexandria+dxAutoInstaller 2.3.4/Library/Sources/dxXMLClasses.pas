...
type
  ...
  public
    ...
    constructor Create(const ARes: string; const AArgs: array of const;
      ...
      const ASourceUri: string = ''); overload;
    constructor Create(const ARes: string; const AArgs: array of const;
      AInnerException: Exception; ALineNumber: Integer; ALinePosition: Integer;
      const ASourceUri: string); overload;
    constructor Create(const ARes: string; const AArg: string;
      ALineNumber: Integer; ALinePosition: Integer;
      const ASourceUri: string); overload;

    class function BuildCharExceptionArgs(AInvChar: Char; ANextChar: Char)
      : TArray<string>; overload; static;
    class function BuildCharExceptionArgs(const AData: TCharArray;
      ALength: Integer; AInvCharIndex: Integer): TArray<string>;
      overload; static;
  ...
  public
    ...
    function PerformReplacements(const AText: string;
      const AReplacementInfo: TdxReplacementInfo): string; overload;
    function PerformReplacements(const AText: string;
      const AReplaceTable: TdxCharStringDictionary): string; overload;
  end;
  ...
  strict private
  const
    PublicIdBitmap: array [0 .. 7] of Word = ($2400, $0000, $FFBB, $AFFF, $FFFF,
      $87FF, $FFFE, $07FF);
    class var FCharProperties: TCharProperties;
    class constructor Initialize;
    class procedure InitRanges(const ARanges: string; Attribute: Byte); static;
  public
    class function CombineSurrogateChar(ALowChar, AHighChar: Char): Integer;
      static; inline;
  ...
  TdxXmlConformanceLevel = (Auto, Fragment, Document);

  TdxXmlNamespaceScope = (All, ExcludeXml, Local);

  IdxXmlNamespaceResolver = interface
    ['{087553CA-4239-4A8D-88E8-7150B9DACDEB}']
    function GetNamespacesInScope(scope: TdxXmlNamespaceScope)
  ...
  class var
    ...
    class function CreateVariableValueReplacementTable
      : TdxCharStringDictionary; static;
    class function CreateVariableValueNoCrLfReplacementTable
      : TdxCharStringDictionary; static;
    class function CreateVariableValueNoTabCrLfReplacementTable
      : TdxCharStringDictionary; static;
  protected
    class constructor Initialize;
{$IFDEF DELPHIXE}
...
function CreateInvalidSurrogatePairException(ALowChar, AHighChar: Char)
  : EdxXmlException;
...
function CreateInvalidSurrogatePairException(ALowChar, AHighChar: Char)
  : EdxXmlException;
begin
  Result := EdxXmlArgumentException.CreateFmt(SXmlInvalidSurrogatePair,
    [CharToHex(ALowChar), CharToHex(AHighChar)]);
end;
...
constructor TdxReplacementItem.Create(ACharIndex: Integer;
...
procedure TdxReplacementInfo.Add(ACharIndex: Integer;
  const AReplaceWith: string);
begin
...
constructor TdxFastCharacterMultiReplacement.Create(AStringBuilder
  : TStringBuilder);
begin
...
function TdxFastCharacterMultiReplacement.CreateReplacementInfo
  (const AText: string; const AReplaceTable: TdxCharStringDictionary)
  : TdxReplacementInfo;
var
  AReplaceWith: string;
  I: Integer;
...
function TdxFastCharacterMultiReplacement.PerformReplacements
  (const AText: string; const AReplacementInfo: TdxReplacementInfo): string;
var
...
begin
  ...
  FBuffer.Capacity := Max(FBuffer.Capacity,
    Length(AText) + AReplacementInfo.DeltaLength);
...
function TdxFastCharacterMultiReplacement.PerformReplacements
  (const AText: string; const AReplaceTable: TdxCharStringDictionary): string;
var
...
const
  ...
  TextRanges = #$0020#$0025#$0027#$003B#$003D#$005C#$005E#$D7FF#$E000#$FFFD;
...
class procedure TdxXmlCharType.InitRanges(const ARanges: string;
  Attribute: Byte);
var
  P: PChar;
  AChar, AEndChar: Char;
...
class function TdxXmlCharType.CombineSurrogateChar(ALowChar,
  AHighChar: Char): Integer;
begin
  Result := (Ord(ALowChar) - Ord(SurLowStart)) or
    ((Ord(AHighChar) - Ord(SurHighStart)) shl 10) + $10000;
end;

class procedure TdxXmlCharType.SplitSurrogateChar(ACombinedChar: Integer;
  out ALowChar: Char; out AHighChar: Char);
var
  V: Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      if (I + 1 > Length(S)) or not(TdxXmlCharType.IsHighSurrogate(S[I]) and
        TdxXmlCharType.IsLowSurrogate(S[I + 1])) then
        Exit(I)
      else
        Inc(I);
    end;
...
begin
  ...
  FXmlCharsNoCrLfTabReplacementTable :=
    CreateVariableValueNoTabCrLfReplacementTable;
  FPreProcessVariableValueStringBuilder := TStringBuilder.Create;
  FXmlCharsReplacement := TdxFastCharacterMultiReplacement.Create
    (FPreProcessVariableValueStringBuilder);
end;
...
class function TdxXmlBasedExporterUtils.CreateVariableValueReplacementTable
...
class function TdxXmlBasedExporterUtils.
  CreateVariableValueNoCrLfReplacementTable: TdxCharStringDictionary;
var
  I: Char;
begin
  Result := TdxCharStringDictionary.Create;
...
class function TdxXmlBasedExporterUtils.
  CreateVariableValueNoTabCrLfReplacementTable: TdxCharStringDictionary;
var
  I: Char;
begin
  Result := TdxCharStringDictionary.Create;
...
class function TdxXmlBasedExporterUtils.EncodeXmlChars
  (const AValue: string): string;
begin
  Result := FXmlCharsReplacement.PerformReplacements(AValue,
    FXmlCharsReplacementTable);
end;

class function TdxXmlBasedExporterUtils.EncodeXmlCharsNoCrLf
  (const AValue: string): string;
begin
  Result := FXmlCharsReplacement.PerformReplacements(AValue,
    FXmlCharsNoCrLfReplacementTable);
end;

class function TdxXmlBasedExporterUtils.EncodeXmlCharsXML1_0
  (const AValue: string): string;
begin
  Result := FXmlCharsReplacement.PerformReplacements(AValue,
    FXmlCharsNoCrLfTabReplacementTable);
...
constructor EdxXmlException.Create(const AMessage: string;
...
constructor EdxXmlException.Create(const ARes, AArg: string;
  ALineNumber, ALinePosition: Integer);
begin
  Create(ARes, [AArg], nil, ALineNumber, ALinePosition, '');
end;

constructor EdxXmlException.Create(const ARes: string;
  const AArgs: array of const; ALineNumber, ALinePosition: Integer;
  const ASourceUri: string);
begin
...
class function EdxXmlException.CreateMessage(const ARes: string;
...
begin
  try
    ...
    begin
      ...
      Result := Format(SXmlMessageWithErrorPosition,
        [AMessage, ALineNumber, ALinePosition]);
    end;
  except
    Result := '';
  end;
...
class function EdxXmlException.FormatUserMessage(const AMessage: string;
  ALineNumber, ALinePosition: Integer): string;
begin
  if AMessage = '' then
    Result := CreateMessage(SXmlDefaultException, [], ALineNumber,
      ALinePosition)
  else
  begin
    if (ALineNumber = 0) and (ALinePosition = 0) then
      ...
      Result := CreateMessage(SXmlUserException, [AMessage], ALineNumber,
        ALinePosition);
  end;
end;

constructor EdxXmlException.Create(const ARes: string;
  const AArgs: array of const; AInnerException: Exception;
...
class function EdxXmlException.BuildCharExceptionArgs(AInvChar, ANextChar: Char)
  : TArray<string>;
var
...
class function EdxXmlException.BuildCharExceptionArgs(const AData: TCharArray;
  ALength, AInvCharIndex: Integer): TArray<string>;
begin
