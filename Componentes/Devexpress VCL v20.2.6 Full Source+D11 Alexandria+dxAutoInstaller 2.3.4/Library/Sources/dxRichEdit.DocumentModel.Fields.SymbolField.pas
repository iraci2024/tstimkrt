...
procedure TdxSymbolField.Initialize(APieceTable: TdxCustomPieceTable;
...
begin
  ...
  begin
    if TryStrToInt(Format('$%s', [Copy(AArgument, 3, Length(AArgument))]),
      Result) then
      Exit;
  end
  else
...
function TdxSymbolField.GetCalculatedValueCore(ASourcePieceTable
...
begin
  ...
  begin
    if (CharacterCode < 0) or (CharacterCode > $FF) then
      Exit(-1);
    ABytes := TEncoding.Convert(TdxEncoding.GetEncoding(1252), TEncoding.UTF8,
      TArray<Byte>.Create(Byte(CharacterCode)));
...
function TdxSymbolField.GetAllowedUpdateFieldTypes(const AOptions
