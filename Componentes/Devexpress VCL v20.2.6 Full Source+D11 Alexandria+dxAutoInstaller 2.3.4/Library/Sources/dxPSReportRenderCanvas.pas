...
{$ENDIF}
  ...
  dxSmartImage,
  cxDrawTextUtils, dxPSFillPatterns, dxCore, ImgList, dxGDIPlusAPI,
  dxGDIPlusClasses;

type
  TdxPicturePaintMode = (ppmCenter, ppmStretch, ppmTile, ppmProportional);
  TdxCheckButtonEdgeStyle = (cbesNone, cbes3D, cbesSoft3D, cbesBoldFlat,
    cbesUltraFlat, cbesSingle);
  ...
  TdxDrawGraphicHelperExportProc = reference to procedure(AGraphic: TGraphic;
    AMask: TcxRegion);
...
{$REGION 'Private Types'}
    ...
    public
      ...
      procedure Paint(ACanvas: TCanvas; const ADestRect: TRect);
        virtual; abstract;
    ...
    strict protected
      ...
      procedure Mask(ADestDC: HDC; const ADestRect: TRect; ASrcDC: HDC;
        const ASrcRect: TRect);
      procedure DrawBitmap(ACanvas: TCanvas; ABitmap: TBitmap;
        const ADestRect: TRect);
      procedure DrawBitmapImage; virtual;
      procedure PrintBitmap(ACanvas: TCanvas; ABitmap: TBitmap;
        const ADestRect: TRect);
      procedure PrintTransparentBitmap(ACanvas: TCanvas; ABitmap: TBitmap;
        const ADestRect: TRect; AMemoryDC: HDC);
      procedure PaintTransparentBitmap(ACanvas: TCanvas; ABitmap: TBitmap;
        const ADestRect: TRect; AMemoryDC: HDC);
      procedure DrawTransparentBitmap(ACanvas: TCanvas; ABitmap: TBitmap;
        const ADestRect: TRect);
...
{$ENDREGION}
  ...
  protected
    ...
    function GetDrawerClassForSmartImage(AHandle: TdxSmartImageCustomHandle)
      : TAbstractGraphicDrawerClass; virtual;
    function CreateDrawer: TAbstractGraphicDrawer; virtual;
    procedure Initialize(ABackgroundColor, ATransparentColor: TColor;
      AIsTransparent, APrinting: Boolean); overload;
  ...
  public
    ...
    procedure Initialize(AGraphic: TGraphic;
      ABackgroundColor, ATransparentColor: TColor;
      AIsTransparent, APrinting: Boolean); overload;
  ...
  protected
    ...
    procedure DoFillEdge(ARegion: TcxRegionHandle;
      ABackColor, AEdgeColor: TColor; AIsVerticalOrientation: Boolean;
      AEdgePattern: TClass { TdxPSEdgePatternClass } ); virtual;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    function CalculateLineThickness(AUnitsPerPixel, AUnitsPerPoint: Integer)
      : Integer; virtual;
    ...
    // Custom Draw
    function BeginCustomDraw(const AClipBounds: TRect; AFont: TFont;
      AColor: TColor): TCanvas; virtual; abstract;
    procedure EndCustomDraw(var ACanvas: TCanvas); virtual; abstract;

    // Text
    function CalculateTextParams(ADrawTextFormat: Integer): TcxTextParams;
      virtual; abstract;
    function CalculateTextRect(const AText: string; var R: TRect;
      ...
      ALineSpacing: Single = 1.0): Integer; virtual; abstract;
    procedure DrawText(var R: TRect; const AText: string; AFont: TFont;
      AFormat: Cardinal; ATextColor: TColor = clDefault;
      AMaxLineCount: Integer = 0; ALineSpacing: Single = 1.0); virtual;
      abstract;
    procedure ExtTextOut(const ABounds: TRect; AFont: TFont;
      const AText: string; AGlyphs: PWord; AGlyphCount: Integer;
      AGlyphWidths: PInteger); virtual; abstract;
    ...
    procedure DrawCheckBox(var R: TRect; AChecked, AEnabled, AIsRadio: Boolean;
      AEdgeStyle: TdxCheckButtonEdgeStyle; AMarlettFont: TFont;
      ALineThickness: Integer; ABorderColor: TColor = clWindowText);
      virtual; abstract;
    procedure DrawEllipseFrame(const R: TRect; AColor: TColor;
      AThickness: Integer); virtual; abstract;
    procedure DrawExpandButton(R: TRect; AEdgeStyle: TdxCheckButtonEdgeStyle;
      AMarlettFont, ASymbolFont: TFont;
      AExpanded, AShadow, AFillInterior: Boolean;
      ABorderColor, ABackgroundColor: TColor; ALineThickness: Integer);
      virtual; abstract;
    procedure DrawFrame(const R: TRect;
      ATopLeftColor, ARightBottomColor: TColor; ABorderWidth: Integer = 1;
      ABorders: TcxBorders = cxBordersAll); virtual; abstract;
    procedure DrawGlyph(const R: TRect; AGlyphFont: TFont; AGlyphIndex: Byte;
      ACenterOnRect: Boolean; ATextColor: TColor = clWindowText);
      virtual; abstract;
    procedure DrawGraphic(AHelper: TdxDrawGraphicHelper; const R: TRect);
      virtual; abstract;
    ...
    procedure DrawRoundFrame(const R: TRect;
      AEllipseWidth, AEllipseHeight: Integer; AColor: TColor;
      AThickness: Integer); virtual; abstract;

    procedure FillEdge(const R: TRect; ABackColor, AEdgeColor: TColor;
      AIsVerticalOrientation: Boolean;
      AEdgePattern: TClass { TdxPSEdgePatternClass } ); overload;
    ...
    procedure FillRegion(ARegion: TcxRegionHandle;
      ...
      APatternBrush: TBrush); virtual; abstract;
    procedure FillRoundRect(R: TRect; AEllipseWidth, AEllipseHeight: Integer;
      ABackColor, AForeColor: TColor; APattern: TdxPSFillPatternClass;
      APatternBrush: TBrush); virtual; abstract;

    procedure Polyline(const APoints: array of TPoint; AColor: TColor;
    ...
    property SupportsTransparentImagesDrawing: Boolean
      read GetSupportsTransparentImagesDrawing;
    ...
    property WorldTransform: TXForm read GetWorldTransform
      write SetWorldTransform;
  end;
  ...
  protected
    ...
    function IsSolidFillPattern(APattern: TdxPSFillPatternClass;
      APatternBrush: TBrush): Boolean;
    ...
    function CalculatePictureRect(APicture: TGraphic; const R: TRect;
      APictureMode: TdxPicturePaintMode;
      ANumerator, ADenominator: Integer): TRect;
    function CreatePatternBitmap(ABackColor, AForeColor: TColor; const R: TRect;
      ARequiredBrushOrigin: Boolean; ABrush: TBrush): TcxBitmap;
    procedure InternalDrawFrameControl(const R: TRect;
      AType, AState: Integer); virtual;
    procedure InternalFillRectByBrushBitmap(const R: TRect;
      ABackColor, AForeColor: TColor; ARequiredBrushOrigin: Boolean;
      ABrush: TBrush);

    procedure DrawPictureStretch(APicture: TGraphic; const R: TRect;
      ANumerator, ADenominator: Integer); virtual;
    procedure DrawPictureTile(APicture: TGraphic; const R: TRect;
      APictureWidth, APictureHeight, ANumerator, ADenominator
      : Integer); virtual;
  public
    ...
    // Custom Draw
    function BeginCustomDraw(const AClipBounds: TRect; AFont: TFont;
      AColor: TColor): TCanvas; override;
    procedure EndCustomDraw(var ACanvas: TCanvas); override;

    // Text
    function CalculateTextParams(ADrawTextFormat: Integer)
      : TcxTextParams; override;
    function CalculateTextRect(const AText: string; var R: TRect;
      AFormat: TcxTextOutFormat = CXTO_DEFAULT_FORMAT; AFont: TFont = nil;
      AMaxLineCount: Integer = 0; ALeftIndent: Integer = 0;
      ARightIndent: Integer = 0; ATextColor: TColor = clDefault;
      ALineSpacing: Single = 1.0): Integer; override;
    procedure DrawText(var R: TRect; const AText: string; AFont: TFont;
      AFormat: Cardinal; ATextColor: TColor = clDefault;
      AMaxLineCount: Integer = 0; ALineSpacing: Single = 1.0); override;
    procedure ExtTextOut(const ABounds: TRect; AFont: TFont;
      const AText: string; AGlyphs: PWord; AGlyphCount: Integer;
      AGlyphWidths: PInteger); override;
    function MakeTextRows(AText: PChar; ATextLength: Integer; const R: TRect;
      const ATextParams: TcxTextParams; var ATextRows: TcxTextRows;
    ...
    procedure SetCanvasExts(const APageSize: TPoint;
      AMappingMode: TdxPSReportRenderCanvasMappingMode;
      AScaleNumerator, AScaleDenominator: Integer;
    ...
    procedure DrawCheckBox(var R: TRect; AChecked, AEnabled, AIsRadio: Boolean;
      AEdgeStyle: TdxCheckButtonEdgeStyle; AMarlettFont: TFont;
      ALineThickness: Integer; ABorderColor: TColor = clWindowText); override;
    procedure DrawEllipseFrame(const R: TRect; AColor: TColor;
      AThickness: Integer); override;
    procedure DrawExpandButton(R: TRect; AEdgeStyle: TdxCheckButtonEdgeStyle;
      AMarlettFont, ASymbolFont: TFont;
      AExpanded, AShadow, AFillInterior: Boolean;
      ABorderColor, ABackgroundColor: TColor; ALineThickness: Integer);
    ...
    procedure DrawGraphic(AHelper: TdxDrawGraphicHelper;
    ...
    procedure Polygon(const APoints: array of TPoint;
      AColor, ABackgroundColor: TColor; ALineWidth: Integer;
      AFillMode: Integer = ALTERNATE); override;
...
procedure cxBkgndDrawPicture(APicture: TGraphic; ACanvas: TCanvas;
  const R: TRect; APictureMode: TdxPicturePaintMode;
  APixelsNumerator, APixelsDenominator: Integer; AOffsetX: Integer = 0;
  AOffsetY: Integer = 0);
...
uses
  ...
  dxTypeHelpers, dxCoreGraphics, dxSVGImage;
...
begin
  ...
  try
    ARenderCanvas.DrawPicture(APicture, R, APictureMode, APixelsNumerator,
      APixelsDenominator, AOffsetX, AOffsetY);
  finally
    ARenderCanvas.Free;
  end;
end;
...
begin
  StretchDIBits(ACanvas.Handle, ABounds.Left, ABounds.Top, ABounds.Width,
    ABounds.Height, 0, 0, BitmapInfo.bmiHeader.biWidth,
    BitmapInfo.bmiHeader.biHeight, Bits, BitmapInfo^, GetDIBColor, ARop);
end;
...
begin
  ...
  FBitmapInfoMemory := GlobalAlloc(GMEM_MOVEABLE or GMEM_SHARE,
    ABitmapInfoSize);
...
constructor TdxDrawGraphicHelper.TAbstractGraphicDrawer.Create
  (AOwner: TdxDrawGraphicHelper);
begin
  FOwner := AOwner;
end;

function TdxDrawGraphicHelper.TAbstractGraphicDrawer.BeginGdiPlusPaint
  (ACanvas: TCanvas): GpGraphics;
begin
...
procedure TdxDrawGraphicHelper.TAbstractGraphicDrawer.EndGdiPlusPaint
  (ACanvas: TCanvas; AGraphics: GpGraphics);
begin
...
procedure TdxDrawGraphicHelper.TAbstractGraphicDrawer.PrepareCanvas
  (ACanvas: TCanvas);
begin
...
procedure TdxDrawGraphicHelper.TAbstractGraphicDrawer.UnprepareCanvas
  (ACanvas: TCanvas);
begin
...
procedure TdxDrawGraphicHelper.TAbstractGraphicDrawer.Export
  (AProc: TdxDrawGraphicHelperExportProc);
begin
  AProc(Graphic, nil);
end;

procedure TdxDrawGraphicHelper.TAbstractGraphicDrawer.Print(ACanvas: TCanvas;
  const ADestRect: TRect);
begin
...
function TdxDrawGraphicHelper.TAbstractGraphicDrawer.CreateGraphics
  (ACanvas: TCanvas): GpGraphics;
begin
  GdipCheck(GdipCreateFromHDC(ACanvas.Handle, Result));
  if not FSmoothlyStretchImages then
    GdipCheck(GdipSetInterpolationMode(Result,
      InterpolationModeNearestNeighbor));
  if Printing then
    GdipCheck(GdipSetPageUnit(Result, TdxGraphicUnit.guPixel));
end;

procedure TdxDrawGraphicHelper.TAbstractGraphicDrawer.DeleteGraphics
...
procedure TdxDrawGraphicHelper.TMetaFileDrawer.Paint(ACanvas: TCanvas;
  const ADestRect: TRect);
var
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.Export
  (AProc: TdxDrawGraphicHelperExportProc);
var
  AMask: TcxRegion;
begin
  ...
  begin
    AMask := TcxRegion.Create(cxCreateRegionFromBitmap(Bitmap,
      Owner.TransparentColor));
    try
      AProc(Bitmap, AMask);
    finally
      AMask.Free;
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.Paint(ACanvas: TCanvas;
  const ADestRect: TRect);
var
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.Print(ACanvas: TCanvas;
  const ADestRect: TRect);
begin
  if Transparent then
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.DrawBitmap(ACanvas: TCanvas;
  ABitmap: TBitmap; const ADestRect: TRect);
var
...
begin
  ...
  try
    ...
    StretchBlt(ACanvas.Handle, ADestRect.Left, ADestRect.Top, ADestRect.Width,
      ADestRect.Height, AMemoryDC, 0, 0, ABitmap.Width, ABitmap.Height,
      SRCCOPY);
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.DrawTransparentBitmap
  (ACanvas: TCanvas; ABitmap: TBitmap; const ADestRect: TRect);
var
  AMemoryDC: HDC;
  AOldStretchBltMode: Integer;
  AOldObject: HGDIOBJ;
begin
  AOldStretchBltMode := SetStretchBltMode(ACanvas.Handle,
    GetStretchBltMode(ABitmap));
  AMemoryDC := CreateCompatibleDC(ACanvas.Handle);
  try
...
function TdxDrawGraphicHelper.TCustomBitmapDrawer.GetStretchBltMode
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.Mask(ADestDC: HDC;
  const ADestRect: TRect; ASrcDC: HDC; const ASrcRect: TRect);
var
...
begin
  ...
  StretchBlt(ABitmapDC, 0, 0, ABitmapSize.cx, ABitmapSize.cy, ASrcDC,
    ASrcRect.Left, ASrcRect.Top, ASrcRect.Width, ASrcRect.Height, SRCCOPY);
  if (ABitmapSize.cx = ADestRect.Width) and (ABitmapSize.cy = ADestRect.Height)
  then
    BitBlt(ADestDC, ADestRect.Left, ADestRect.Top, ADestRect.Width,
      ADestRect.Height, ABitmapDC, 0, 0, SRCAND)
  else
    StretchBlt(ADestDC, ADestRect.Left, ADestRect.Top, ADestRect.Width,
      ADestRect.Height, ABitmapDC, 0, 0, ABitmapSize.cx,
      ABitmapSize.cy, SRCAND);
...
begin
  StretchBlt(ACanvas.Handle, ADestRect.Left, ADestRect.Top, ADestRect.Width,
    ADestRect.Height, AMemoryDC, 0, 0, ABitmap.Width, ABitmap.Height,
    SRCINVERT);
  Mask(ACanvas.Handle, ADestRect, AMemoryDC, cxRect(0, 0, ABitmap.Width,
    ABitmap.Height));
  StretchBlt(ACanvas.Handle, ADestRect.Left, ADestRect.Top, ADestRect.Width,
    ADestRect.Height, AMemoryDC, 0, 0, ABitmap.Width, ABitmap.Height,
    SRCINVERT);
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.PrintBitmap(ACanvas: TCanvas;
  ABitmap: TBitmap; const ADestRect: TRect);
var
...
procedure TdxDrawGraphicHelper.TCustomBitmapDrawer.ValidateBitmap(AWidth,
  AHeight: Integer);
var
...
begin
  Owner.ImageList.Draw(Bitmap.Canvas, 0, 0, Owner.ImageIndex, dsNormal,
    itImage);
end;
...
procedure TdxDrawGraphicHelper.TImageListDrawer.Paint(ACanvas: TCanvas;
  const ADestRect: TRect);
begin
...
procedure TdxDrawGraphicHelper.TIconDrawer.Paint(ACanvas: TCanvas;
  const ADestRect: TRect);
var
  AGraphics: GpGraphics;
  AImage: GPImage;
begin
  ...
  try
    GdipCheck(GdipCreateBitmapFromScan0(FSize.cx, FSize.cy,
      SizeOf(TdxAlphaColor) * FSize.cx, PixelFormat32bppARGB,
      FBits.Bits, AImage));
    try
      GdipCheck(GdipDrawImageRectI(AGraphics, AImage, ADestRect.Left,
...
procedure TdxDrawGraphicHelper.TGenericDrawer.Paint(ACanvas: TCanvas;
  const ADestRect: TRect);
var
  ASavedTransparent: Boolean;
begin
...
procedure TdxDrawGraphicHelper.TGenericDrawer.Print(ACanvas: TCanvas;
  const ADestRect: TRect);
var
  ASavedTransparent: Boolean;
begin
...
procedure TdxDrawGraphicHelper.TSmartImageDrawer.Export
  (AProc: TdxDrawGraphicHelperExportProc);
begin
  if FUseTempImage then
    inherited
...
procedure TdxDrawGraphicHelper.TSmartImageDrawer.Paint(ACanvas: TCanvas;
  const ADestRect: TRect);
begin
  ACanvas.StretchDraw(ADestRect, Graphic);
end;

procedure TdxDrawGraphicHelper.TSmartImageDrawer.Print(ACanvas: TCanvas;
  const ADestRect: TRect);
var
  AImage: GPImage;
  AGraphics: GpGraphics;
begin
  if FUseTempImage then
    GdipCheck(GdipCreateBitmapFromHBITMAP(Bitmap.Handle,
      Bitmap.Palette, AImage))
  else
    AImage := FGPImage;
  try
    AGraphics := BeginGdiPlusPaint(ACanvas);
...
begin
  ...
  begin
    ASvgImageHandle := TdxCustomSmartImageAccess(Graphic)
      .Handle as TdxSVGImageHandle;
    ASvgImageSize := cxSizeScale(ASvgImageHandle.Size, PixelsNumerator,
      PixelsDenominator);
...
begin
  if Graphic = nil then
    ...
    Result := GetDrawerClassForSmartImage
      (TdxCustomSmartImageAccess(Graphic).Handle)
...
function TdxDrawGraphicHelper.GetDrawerClassForSmartImage
  (AHandle: TdxSmartImageCustomHandle): TAbstractGraphicDrawerClass;
begin
...
procedure TdxDrawGraphicHelper.Initialize(ABackgroundColor, ATransparentColor
  : TColor; AIsTransparent, APrinting: Boolean);
begin
...
procedure TdxDrawGraphicHelper.Initialize(AGraphic: TGraphic;
  ABackgroundColor, ATransparentColor: TColor;
  AIsTransparent, APrinting: Boolean);
begin
...
procedure TdxDrawGraphicHelper.Initialize(AImageList: TCustomImageList;
  AImageIndex: Integer; ABackgroundColor, ATransparentColor: TColor;
  AIsTransparent, APrinting: Boolean);
begin
  Initialize(ABackgroundColor, ATransparentColor, AIsTransparent, APrinting);
...
begin
  ...
  else if FDrawer.ClassType <> GetDrawerClass then
  begin
    FDrawer.Free;
    FDrawer := CreateDrawer;
  end;
...
function TdxPSReportRenderCustomCanvas.CalculateLineThickness(AUnitsPerPixel,
  AUnitsPerPoint: Integer): Integer;
begin
...
function TdxPSReportRenderCustomCanvas.
  GetSupportsTransparentImagesDrawing: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  FillEdge(ARegion, ABackColor, AEdgeColor, AIsVerticalOrientation,
    AEdgePattern);
  ...
  begin
    ...
    begin
      ...
      try
        ...
        FillRegion(ARegion, ABackColor, AEdgeColor,
          TdxPSSolidFillPattern, ABrush);
      finally
        ABrush.Free;
      end;
    end
    else
      DoFillEdge(ARegion, ABackColor, AEdgeColor, AIsVerticalOrientation,
        AEdgePattern);
  end;
end;

procedure TdxPSReportRenderCustomCanvas.DoFillEdge(ARegion: TcxRegionHandle;
  ABackColor, AEdgeColor: TColor; AIsVerticalOrientation: Boolean;
  AEdgePattern: TClass { TdxPSEdgePatternClass } );
const
...
procedure TdxPSReportRenderCustomCanvas.PrepareCanvasForCustomDraw(AFont: TFont;
  AColor: TColor);
begin
...
begin
  case APictureMode of
    ppmCenter:
      Result := cxRectCenter(R, MulDiv(APicture.Width, ANumerator,
        ADenominator), MulDiv(APicture.Height, ANumerator, ADenominator));
...
function TdxPSReportRenderCanvas.CreatePatternBitmap(ABackColor,
  AForeColor: TColor; const R: TRect; ARequiredBrushOrigin: Boolean;
  ABrush: TBrush): TcxBitmap;
...
procedure TdxPSReportRenderCanvas.InternalDrawFrameControl(const R: TRect;
  AType, AState: Integer);
begin
...
begin
  ...
  begin
    ...
    try
      cxStretchBlt(ABitmap.Canvas.Handle, Canvas.Handle, ABitmap.ClientRect,
        R, SRCCOPY);
      ABitmap.Canvas.StretchDraw(ABitmap.ClientRect, APicture);
      cxStretchBlt(Canvas.Handle, ABitmap.Canvas.Handle, R,
        ABitmap.ClientRect, SRCCOPY);
    finally
      ABitmap.Free;
    end;
  end;
...
procedure TdxPSReportRenderCanvas.DrawPictureTile(APicture: TGraphic;
  const R: TRect; APictureWidth, APictureHeight, ANumerator,
...
begin
  ...
  for X := 0 to ACountX do
    ...
    begin
      ...
      OffsetRect(AImageRect, R.Left + X * APictureWidth,
        R.Top + Y * APictureHeight);
...
procedure TdxPSReportRenderCanvas.DrawCheckBox(var R: TRect;
  ...
  AMarlettFont: TFont; ALineThickness: Integer;
  ABorderColor: TColor = clWindowText);

  procedure DrawCheckBoxGlyph(AGlyphIndex: Byte; AColor: TColor = clDefault);
  begin
    DrawGlyph(cxRectInflate(R, ALineThickness, ALineThickness), AMarlettFont,
      AGlyphIndex, True, AColor);
  end;
  ...
  begin
    ...
    begin
      GetBorderColors(AEdgeStyle, ABorderColor, True, ATopLeftColor,
        ARightBottomColor);
      DrawCheckBoxGlyph(TopLeftArcInnerIndexes[AIsRadio], ATopLeftColor);
      DrawCheckBoxGlyph(BottomRightArcInnerIndexes[AIsRadio],
        ARightBottomColor);
      if AEdgeStyle in [cbes3D, cbesSoft3D, cbesBoldFlat] then
      begin
        GetBorderColors(AEdgeStyle, ABorderColor, False, ATopLeftColor,
          ARightBottomColor);
        DrawCheckBoxGlyph(TopLeftArcOuterIndexes[AIsRadio], ATopLeftColor);
        DrawCheckBoxGlyph(BottomRightArcOuterIndexes[AIsRadio],
          ARightBottomColor);
      end;
    end;
  end;
...
const
  ...
  InteriorIndexes: array [Boolean] of Integer = (CheckInteriorIndex,
    RadioInteriorIndex);
...
procedure TdxPSReportRenderCanvas.DrawEllipseFrame(const R: TRect;
  AColor: TColor; AThickness: Integer);
var
  AOuterRgn, AInnerRgn: TcxRegionHandle;
begin
  AOuterRgn := CreateEllipticRgnIndirect(R);
  AInnerRgn := CreateEllipticRgnIndirect(cxRectInflate(R, -AThickness,
    -AThickness));
...
procedure TdxPSReportRenderCanvas.DrawExpandButton(R: TRect;
  ...
  procedure DrawExpandButtonBorders(const R: TRect; AMarlettFont: TFont;
    ABorderColor: TColor; AEdgeStyle: TdxCheckButtonEdgeStyle;
  ...
  begin
    ...
    begin
      Font := AMarlettFont;
      GetBorderColors(AEdgeStyle, ABorderColor, True, ARightBottomColor,
        ATopLeftColor);
      DrawGlyph(R, AMarlettFont, CheckTopLeftArcInnerIndex, True,
        ATopLeftColor);
      DrawGlyph(R, AMarlettFont, CheckBottomRightArcInnerIndex, True,
        ARightBottomColor);
      if AEdgeStyle in [cbes3D, cbesSoft3D, cbesBoldFlat] then
      begin
        GetBorderColors(AEdgeStyle, ABorderColor, False, ARightBottomColor,
          ATopLeftColor);
        DrawGlyph(R, AMarlettFont, CheckTopLeftArcOuterIndex, True,
          ATopLeftColor);
        DrawGlyph(R, AMarlettFont, CheckBottomRightArcOuterIndex, True,
          ARightBottomColor);
      end;
    end;
  end;
  ...
  begin
    DrawGlyph(cxRectOffset(R, 0, -ALineThickness), ASymbolFont,
      CrossHireIndexes[AExpanded], True, ATextColor);
  end;
  ...
  begin
    ...
    try
      if AFillInterior then
        FillRect(cxRectInflate(R, -ALineThickness, -ALineThickness),
          ABackgroundColor);
      R := cxRectInflate(R, ALineThickness, ALineThickness);
      DrawExpandButtonBorders(R, AMarlettFont, ABorderColor, AEdgeStyle,
        ALineThickness);
      DrawExpandButtonGlyph(R, ASymbolFont, ABorderColor, AExpanded,
        ALineThickness);
    finally
      RestoreState;
    end;
...
procedure TdxPSReportRenderCanvas.DrawFrame(const R: TRect;
  ATopLeftColor, ARightBottomColor: TColor; ABorderWidth: Integer = 1;
  ABorders: TcxBorders = cxBordersAll);
begin
  Canvas.DrawComplexFrame(R, ATopLeftColor, ARightBottomColor, ABorders,
    ABorderWidth);
end;
...
const
  TextFormatMap: array [Boolean] of Integer = (CXTO_LEFT,
    CXTO_CENTER_HORIZONTALLY or CXTO_CENTER_VERTICALLY);
var
  R1: TRect;
begin
  R1 := R;
  ...
  DrawText(R1, Chr(AGlyphIndex), AGlyphFont, TextFormatMap[ACenterOnRect],
    ATextColor);
end;

procedure TdxPSReportRenderCanvas.DrawGraphic(AHelper: TdxDrawGraphicHelper;
  const R: TRect);
begin
  AHelper.Draw(Canvas.Canvas, R);
end;

procedure TdxPSReportRenderCanvas.DrawPicture(APicture: TGraphic;
  const R: TRect; APictureMode: TdxPicturePaintMode;
  ANumerator, ADenominator: Integer; AOffsetX: Integer = 0;
  AOffsetY: Integer = 0);
var
  W, H: Integer;
begin
  ...
  begin
    if APictureMode <> ppmTile then
      DrawPictureStretch(APicture, CalculatePictureRect(APicture, R,
        APictureMode, ANumerator, ADenominator), ANumerator, ADenominator)
    else
    begin
      SaveClipRgn;
      try
        ...
        DrawPictureTile(APicture, cxRectOffset(R, AOffsetX mod W,
          AOffsetY mod H), W, H, ANumerator, ADenominator);
      finally
        RestoreClipRgn;
      end;
    end;
...
begin
  ...
  try
    AEllipseHeight := MulDiv(AEllipseHeight, cxRectHeight(R) - 2 * AThickness,
      cxRectHeight(R));
    AEllipseWidth := MulDiv(AEllipseWidth, cxRectWidth(R) - 2 * AThickness,
      cxRectWidth(R));
    ARegion.Combine(TcxRegion.CreateRoundCorners(cxRectInflate(R, -AThickness,
      -AThickness), AEllipseWidth, AEllipseHeight), roSubtract);
...
procedure TdxPSReportRenderCanvas.Polyline(const APoints: array of TPoint;
  AColor: TColor; ALineWidth: Integer);
begin
  SaveState;
  try
    if AColor <> clDefault then
...
procedure TdxPSReportRenderCanvas.Polygon(const APoints: array of TPoint;
  AColor, ABackgroundColor: TColor; ALineWidth: Integer;
...
procedure TdxPSReportRenderCanvas.GetBorderColors
  ...
  AInnerBorder: Boolean; out ATopLeftBorderColor,
  ARightBottomBorderColor: TColor);
const
  Soft3DTopLeftBorders: array [Boolean] of TColor = (clBtnShadow, clBtnFace);
  Soft3DRightBottomBorders: array [Boolean] of TColor = (clBtnHighlight,
    clBtnFace);
  Custom3DTopLeftBorders: array [Boolean] of TColor = (clBtnShadow,
    cl3DDkShadow);
  Custom3DRightBottomBorders: array [Boolean] of TColor = (clBtnHighlight,
    cl3DLight);
begin
  case AEdgeStyle of
...
function TdxPSReportRenderCanvas.CalculateTextParams(ADrawTextFormat: Integer)
  : TcxTextParams;
begin
...
function TdxPSReportRenderCanvas.CalculateTextRect(const AText: string;
  ...
  ARightIndent: Integer = 0; ATextColor: TColor = clDefault;
  ALineSpacing: Single = 1): Integer;
begin
  Result := cxTextOut(Canvas.Handle, AText, R, AFormat, AFont, AMaxLineCount,
    ALeftIndent, ARightIndent, ATextColor, ALineSpacing);
end;

procedure TdxPSReportRenderCanvas.DrawText(var R: TRect; const AText: string;
  AFont: TFont; AFormat: Cardinal; ATextColor: TColor = clDefault;
  AMaxLineCount: Integer = 0; ALineSpacing: Single = 1.0);
begin
  cxTextOut(Canvas.Handle, AText, R, AFormat, AFont, AMaxLineCount, 0, 0,
    ATextColor, ALineSpacing);
end;
...
function TdxPSReportRenderCanvas.MakeTextRows(AText: PChar;
...
begin
  ...
  Result := cxMakeTextRows(Canvas.Handle, AText, ATextLength, R, ATextParams,
...
procedure TdxPSReportRenderCanvas.RotatedTextOut(const ABounds: TRect;
  const AText: string; AFont: TFont; AAlignHorz: TcxTextAlignX = taCenterX;
  AAlignVert: TcxTextAlignY = taCenterY; AWordBreak: Boolean = True;
  ADirection: TcxVerticalTextOutDirection = vtdBottomToTop);
begin
  cxRotatedTextOut(Canvas.Handle, ABounds, AText, AFont, AAlignHorz, AAlignVert,
    AWordBreak, True, True, ADirection);
end;
...
const
  MappingModeFlags: array [TdxPSReportRenderCanvasMappingMode] of Integer = (0,
    MM_TEXT, MM_LOMETRIC, MM_HIMETRIC, MM_LOENGLISH, MM_HIENGLISH, MM_TWIPS,
...
begin
  ...
  ScaleWindowExtEx(Canvas.Handle, AScaleNumerator, AScaleDenominator,
    AScaleNumerator, AScaleDenominator, nil);
  SetViewPortExtEx(Canvas.Handle, cxRectWidth(AViewPort),
    cxRectHeight(AViewPort), nil);
...
begin
  with R do
    Result := Windows.IntersectClipRect(Canvas.Handle, Left, Top,
      Right, Bottom);
end;

function TdxPSReportRenderCanvas.BeginCustomDraw(const AClipBounds: TRect;
  AFont: TFont; AColor: TColor): TCanvas;
begin
  Canvas.SaveState;
...
constructor TdxPSAdjustFontSizeHelper.Create
...
function TdxPSAdjustFontSizeHelper.CheckFontSize(ALastSuitableSize,
...
function TdxPSAdjustFontSizeHelper.GetTextHeight(const AText: string;
  AWidth: Integer; ALineSpacing: Single): Integer;
const
  AlignFlags = CXTO_TOP or CXTO_CENTER_VERTICALLY or CXTO_BOTTOM or
...
begin
  ...
  FCanvas.Font := Font;
  FCanvas.CalculateTextRect(AText, R, (FFlags and not AlignFlags) or
    CXTO_CALCRECT, nil, 0, 0, 0, clDefault, ALineSpacing);
