...
type
  ...
  TdxCombinator = (None, WhiteSpace, RightAngle, PlusSign);

  { TdxSelectorParserState }

  TdxSelectorParserState = (StartState, ReadSelectorName, ReadClassName, ReadId,
    SkipWord, AutoSpace, ReadChildElement, ReadAttribute, ReadPseudoClass);
  ...
  public
    ...
    class function TryGetGroupValue(const AMatch: TMatch;
      const AGroupName: string): string; static;
  end;
  ...
  public
    ...
    property AttributeConnector: string read FAttributeConnector
      write FAttributeConnector;
  end;
  ...
  public
    ...
    property PseudoClasses: TdxStyleClasses read FPseudoClasses
      write FPseudoClasses;
    property Id: string read FId write FId;
    property SelectorAttributes: TdxObjectList<TdxSelectorAttribute>
      read FSelectorAttributes write FSelectorAttributes;
  end;
  ...
  public
    ...
    property SimpleSelector: TdxSimpleSelector read FSimpleSelector
      write FSimpleSelector;
...
begin
  FPattern := Format('%0:s%1:s%0:s(%2:s%0:s%3:s%0:s)?',
    [SpacePattern, AttrNamePattern, EqPattern, ValuePattern]);
  FRegex := TRegex.Create(FPattern, [roCompiled, roExplicitCapture]);
end;

class function TdxAttributePattern.TryGetGroupValue(const AMatch: TMatch;
...
{$IFNDEF DELPHIXE2}
  ...
  else if AMatch.Groups.Count > 2 then
    if SameStr('attrEq', AGroupName) then
      Result := AMatch.Groups[2].Value
    else if SameStr('attrValue', AGroupName) then
