...
uses
  TypInfo, Types, SysUtils, Classes, Generics.Defaults, Generics.Collections,
  Contnrs,
...
{$ENDIF}
  ...
  public
    ...
    procedure InsertRange(Index: Integer;
      const Values: TArray<string>); overload;
  ...
  public
    ...
    function BinarySearch(const Item: T; out Index: Integer;
  ...
  TdxNotificationCollectionChangedEvent = procedure(const AItem: TObject;
    Action: TListNotification) of object;
  TdxNotificationCollectionChangedEventHandler =
    TdxMulticastMethod<TdxNotificationCollectionChangedEvent>;
  ...
  public
    procedure AddChangedHandler(AHandler
      : TdxNotificationCollectionChangedEvent);
    procedure RemoveChangedHandler(AHandler
      : TdxNotificationCollectionChangedEvent);
  end;
  ...
  TdxNotificationObjectCollection<T: class> = class
...
{$IFDEF CPUX64}
      TdxOrdinalHelper.Set2, TdxOrdinalHelper.Set3, TdxOrdinalHelper.Set4,
      TdxOrdinalHelper.Set5, TdxOrdinalHelper.Set6, TdxOrdinalHelper.Set7
{$ELSE}
      TdxOrdinalHelper.Set2, TdxOrdinalHelper.Set3
{$ENDIF}
      );
{$ENDREGION}
  public
    class procedure SetValue(ASize: Integer; AValue: NativeInt; ADest: Pointer);
      static; inline;
  end;
...
{$ENDREGION}
  strict private
    ...
    FValueSize: Integer;
  strict protected
    function NewItem(const AKey: string; AHash: Cardinal;
      const AValue: NativeInt): PItem;
    procedure InternalAdd(const AKey: string; AValue: NativeInt;
      ACheckDuplicates: Boolean);
    procedure InternalRemove(AItem: PItem);
    function InternalTryGetValue(const AKey: string;
      out AValue: NativeInt): Boolean;
    procedure RemoveValue(AValue: NativeInt); virtual;
  protected
    property Table: TTable read FTable;
  public
    constructor Create(AValueSize: Integer;
      ATableSize: Cardinal = DefaultHashTableSize);
...
{$REGION 'internal types'}
  ...
  type
    ...
    strict private
      ...
      FNext: TItem;
    public
      constructor Create(const AKey: string; AHash: Cardinal;
        const AValue: TValue);
...
{$ENDREGION}
  ...
  strict protected
    ...
    procedure InternalAdd(const AKey: string; const AValue: TValue;
      ACheckDuplicates: Boolean);
    property Table: TTable read FTable;
  public
    constructor Create;
  ...
  strict protected
    procedure Clear;
    procedure InternalAdd(AKey: Cardinal; const AValue: TValue;
      ACheckDuplicates: Boolean);
  ...
  TdxEnumeratedDictionary<TEnumeration, TValue> = class
  ...
  public
    ...
    property Items[AKey: TEnumeration]: TValue read GetItem
  ...
  TdxEnumeratedObjectDictionary<TEnumeration; TValue: class> = class
    (TdxEnumeratedDictionary<TValue>)
{$REGION 'internal types'}
  strict protected
  type
...
{$ENDREGION}
  strict private
    ...
    procedure SetItem(AKey: TEnumeration; const Value: TValue); inline;
  strict protected
    function CreateMap(ATableSize: Cardinal): TdxEnumeratedMap<TValue>;
      override;
  ...
  strict protected
    ...
    procedure Grow;
    procedure Notify(const AItem: NativeUInt;
    ...
    function BinarySearch(AItem: NativeUInt; out AIndex: Integer;
      const AComparer: IComparer<NativeUInt>): Boolean;
    function Contains(AItem: NativeUInt): Boolean; inline;
    function Extract(AItem: NativeUInt;
      ADirection: TList.TDirection = FromBeginning): NativeUInt;
    ...
    property OnNotify: TCollectionNotifyEvent<NativeUInt> read FOnNotify
      write FOnNotify;
  public
    ...
    procedure Reverse;
    procedure Sort; overload;
    procedure Sort(ACompare: TCompareItems;
      AMultiThreaded: Boolean = False); overload;
  ...
  TdxTopologicalSorterIsDependOnDelegate = reference to function(ALeft,
  ...
  public
    constructor Create(AOneBasedSuccessorIndex: Integer;
      ANext: TdxTopologicalSorterNode);
  ...
  protected
    ...
    procedure CalculateRelations(ASourceObjects: TdxList<T>;
      const AIsDependOn: TdxTopologicalSorterIsDependOnDelegate);
    function CreateVirtualNoPredecessorsItemList: Integer;
    function ProcessNodes(ALastNoPredecessorItemIndex: Integer;
      ASourceObjects: TdxList<T>): TdxList<T>;
    function RemoveRelation(ANode: TdxTopologicalSorterNode;
      var ALastNoPredecessorItemIndex: Integer): TdxTopologicalSorterNode;

    function DoSort(ASourceObjects: TdxList<T>; AComparer: IComparer<T>)
      : TdxList<T>; overload;
    function DoSort(ASourceObjects: TdxList<T>;
      const AIsDependOn: TdxTopologicalSorterIsDependOnDelegate)
      : TdxList<T>; overload;
  ...
  public
    ...
    class function Sort(ASourceObjects: TdxList<T>; AComparer: IComparer<T>)
      : TdxList<T>;
  ...
  public
    class function BinarySearch(AList: TList<T>; const AValue: T;
      out AIndex: Integer): Boolean; overload;

    class function BinarySearch(AList: TList<T>;
      const APredicate: IdxComparable<T>; AStartIndex, AEndIndex: Integer;
      out AIndex: Integer): Boolean; overload;
    class function BinarySearch(AList: TList<T>;
      const APredicate: IdxComparable<T>; out AIndex: Integer)
      : Boolean; overload;

    class function BinarySearch(AList: TList<T>;
      const APredicate: TdxComparableFunc<T>; AStartIndex, AEndIndex: Integer;
      out AIndex: Integer): Boolean; overload;
    class function BinarySearch(AList: TList<T>;
      const APredicate: TdxComparableFunc<T>; out AIndex: Integer)
      : Boolean; overload;
  end;

  TdxAlgorithms1<T: class> = class
  public
    class function BinarySearch(AList: TdxList<T>;
      const APredicate: IdxComparable<T>; AStartIndex, AEndIndex: Integer;
      out AIndex: Integer): Boolean; overload;
    class function BinarySearch(AList: TdxList<T>;
      const APredicate: IdxComparable<T>; out AIndex: Integer)
    ...
    class function BinarySearchReverseOrder(AList: TdxList<T>;
      const APredicate: IdxComparable<T>; AStartIndex, AEndIndex: Integer;
      out AIndex: Integer): Boolean; overload;
    class function BinarySearchReverseOrder(AList: TdxList<T>;
      const APredicate: IdxComparable<T>; out AIndex: Integer)
  ...
  public
    ...
    function BinarySearch(const APredicate: IdxComparable<T>): Integer;
      overload;
    function BinarySearch(const APredicate: TdxComparableFunc<T>)
      : Integer; overload;
    procedure Delete(AIndex: Integer);
  ...
  public
    constructor Create; overload;
    constructor Create(AComparerKey: IEqualityComparer<TKey>;
      AComparerValue: IEqualityComparer<TValue>;
      AOwnerships: TDictionaryOwnerships = []; ACapacity: Integer = 0);
      overload;
    constructor Create(ACapacity: Integer;
...
function TdxList<T>.BinarySearch(const Item: T; out Index: Integer;
  const AComparer: IComparer<T>): Boolean;
var
  L, H: Integer;
  mid, cmp: Integer;
...
class procedure TdxOrdinalHelper.SetValue(ASize: Integer; AValue: NativeInt;
  ADest: Pointer);
begin
  if ASize = 1 then
    PByte(ADest)^ := Byte(AValue)
  else if ASize = SizeOf(NativeInt) then
...
constructor TdxStringToOrdinalMap.Create(AValueSize: Integer;
  ATableSize: Cardinal = DefaultHashTableSize);
begin
...
begin
  InternalAdd(AKey, PNativeInt(AValue)^ and TdxOrdinalHelper.SizeMask
    [FValueSize], True);
end;

procedure TdxStringToOrdinalMap.AddOrSetValue(const AKey: string;
  AValue: Pointer);
begin
  InternalAdd(AKey, PNativeInt(AValue)^ and TdxOrdinalHelper.SizeMask
    [FValueSize], False);
...
function TdxStringToOrdinalMap.TryGetValue(const AKey: string;
...
procedure TdxStringToOrdinalMap.InternalAdd(const AKey: string;
  AValue: NativeInt; ACheckDuplicates: Boolean);
var
  AIndex: Cardinal;
  AEntry, ATemp: PItem;
  AHash: Cardinal;
...
function TdxStringToOrdinalMap.InternalTryGetValue(const AKey: string;
  out AValue: NativeInt): Boolean;
var
  AEntry: PItem;
  AHash: Cardinal;
...
function TdxStringToOrdinalMap.NewItem(const AKey: string; AHash: Cardinal;
  const AValue: NativeInt): PItem;
begin
  New(Result);
  Result.Key := AKey;
...
function TdxNamedOrdinalDictionary<T>.GetValueDef(const AKey: string;
  ADefaultValue: T): T;
begin
  if not TryGetValue(AKey, Result) then
...
function TdxNamedOrdinalDictionary<T>.TryGetValue(const AKey: string;
...
procedure TdxNamedOrdinalDictionary<T>.AddOrSetValue(const AKey: string;
  AValue: T);
begin
...
procedure TdxNamedObjectDictionary<T>.AddOrSetValue(const AKey: string;
  AValue: T);
begin
  Map.AddOrSetValue(AKey, @AValue);
end;
...
procedure TdxNamedObjectDictionary<T>.SetItem(const AKey: string;
  const Value: T);
begin
  AddOrSetValue(AKey, Value);
end;
...
constructor TdxNamedDelegateDictionary<TValue>.TItem.Create(const AKey: string;
  AHash: Cardinal; const AValue: TValue);
begin
  FKey := AKey;
  FValue := AValue;
  FHash := AHash
...
function TdxNamedDelegateDictionary<TValue>.TItem.IsEqualTo
  (AItem: TItem): Boolean;
begin
...
procedure TdxNamedDelegateDictionary<TValue>.Add(const AKey: string;
  AValue: TValue);
begin
  InternalAdd(AKey, AValue, True);
end;

procedure TdxNamedDelegateDictionary<TValue>.AddOrSetValue(const AKey: string;
  AValue: TValue);
begin
...
function TdxNamedDelegateDictionary<TValue>.ContainsKey
...
class function TdxNamedDelegateDictionary<TValue>.Hash(const AKey: string)
  : Cardinal;
begin
  Result := dxElfHash(AKey);
end;

procedure TdxNamedDelegateDictionary<TValue>.InternalAdd(const AKey: string;
  const AValue: TValue; ACheckDuplicates: Boolean);
var
  AIndex: Cardinal;
  AEntry, ATemp: TItem;
  AHash: Cardinal;
...
function TdxNamedDelegateDictionary<TValue>.TryGetValue(const AKey: string;
...
procedure TdxEnumeratedMap<TValue>.AddOrSetValue(AKey: Cardinal;
  const AValue: TValue);
begin
  InternalAdd(AKey, AValue, False);
end;
...
procedure TdxEnumeratedMap<TValue>.InternalAdd(AKey: Cardinal;
  const AValue: TValue; ACheckDuplicates: Boolean);
begin
  if AKey >= FTableSize then
  ...
  if not FKeyTable[AKey] then
    Inc(FCount)
  else if ACheckDuplicates then
    raise EListError.CreateRes(@SGenericDuplicateItem)
  else if not TEqualityComparer<TValue>.Default.Equals(FValueTable[AKey], AValue)
  then
    InternalRemove(AKey);
  FKeyTable[AKey] := True;
...
function TdxEnumeratedMap<TValue>.TryGetValue(AKey: Cardinal;
  out AValue: TValue): Boolean;
begin
  if AKey >= FTableSize then
    Exit(False);
...
constructor TdxEnumeratedDictionary<TValue>.Create(AKeySize: Cardinal;
  ATypeInfo: PTypeInfo);
var
  ATypeData: PTypeData;
begin
...
function TdxEnumeratedDictionary<TValue>.CreateMap(ATableSize: Cardinal)
  : TdxEnumeratedMap<TValue>;
begin
...
procedure TdxEnumeratedDictionary<TEnumeration, TValue>.Add(AKey: TEnumeration;
  const AValue: TValue);
begin
  Map.Add(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask[KeySize], AValue);
end;

procedure TdxEnumeratedDictionary<TEnumeration, TValue>.AddOrSetValue
  (AKey: TEnumeration; const AValue: TValue);
begin
  Map.AddOrSetValue(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask
    [KeySize], AValue);
end;

function TdxEnumeratedDictionary<TEnumeration, TValue>.ContainsKey
  (AKey: TEnumeration): Boolean;
begin
  Result := Map.ContainsKey(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask
    [KeySize]);
end;

procedure TdxEnumeratedDictionary<TEnumeration, TValue>.Remove
  (AKey: TEnumeration);
begin
  Map.Remove(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask[KeySize]);
end;

function TdxEnumeratedDictionary<TEnumeration, TValue>.TryGetValue
  (AKey: TEnumeration; out AValue: TValue): Boolean;
begin
  Result := Map.TryGetValue(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask
    [KeySize], AValue);
end;

function TdxEnumeratedDictionary<TEnumeration, TValue>.GetItem
  (AKey: TEnumeration): TValue;
begin
  if not TryGetValue(AKey, Result) then
    raise EListError.CreateRes(@SGenericItemNotFound);
end;

function TdxEnumeratedDictionary<TEnumeration, TValue>.GetKeys
  : TArray<TEnumeration>;
var
  I, AIndex: Integer;
begin
...
procedure TdxEnumeratedDictionary<TEnumeration, TValue>.SetItem
  (AKey: TEnumeration; const Value: TValue);
begin
  AddOrSetValue(AKey, Value)
end;
...
procedure TdxEnumeratedObjectDictionary<TEnumeration, TValue>.TMap.
  InternalRemove(AKey: Cardinal);
begin
  if OwnsObjects then
...
constructor TdxEnumeratedObjectDictionary<TEnumeration, TValue>.Create
...
function TdxEnumeratedObjectDictionary<TEnumeration, TValue>.CreateMap
  (ATableSize: Cardinal): TdxEnumeratedMap<TValue>;
begin
  TMap(Result) := TMap.Create(ATableSize);
end;

procedure TdxEnumeratedObjectDictionary<TEnumeration, TValue>.Add
  (AKey: TEnumeration; AValue: TValue);
begin
  Map.Add(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask[KeySize], AValue);
end;

procedure TdxEnumeratedObjectDictionary<TEnumeration, TValue>.AddOrSetValue
  (AKey: TEnumeration; AValue: TValue);
begin
  Map.AddOrSetValue(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask
    [KeySize], AValue);
...
function TdxEnumeratedObjectDictionary<TEnumeration, TValue>.ContainsKey
  (AKey: TEnumeration): Boolean;
begin
  Result := Map.ContainsKey(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask
    [KeySize]);
...
procedure TdxEnumeratedObjectDictionary<TEnumeration, TValue>.Remove
  (AKey: TEnumeration);
begin
  Map.Remove(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask[KeySize]);
end;

function TdxEnumeratedObjectDictionary<TEnumeration, TValue>.TryGetValue
  (AKey: TEnumeration; out AValue: TValue): Boolean;
var
  AValueObject: TObject;
begin
  Result := Map.TryGetValue(PCardinal(@AKey)^ and TdxOrdinalHelper.SizeMask
    [KeySize], AValueObject);
  if Result then
...
function TdxEnumeratedObjectDictionary<TEnumeration, TValue>.
  GetOwnsObjects: Boolean;
begin
  Result := Map.OwnsObjects;
end;

procedure TdxEnumeratedObjectDictionary<TEnumeration, TValue>.SetOwnsObjects
  (const Value: Boolean);
begin
  Map.OwnsObjects := Value;
end;

function TdxEnumeratedObjectDictionary<TEnumeration, TValue>.GetItem
  (AKey: TEnumeration): TValue;
begin
  if not TryGetValue(AKey, Result) then
    raise EListError.CreateRes(@SGenericItemNotFound);
...
function TdxEnumeratedObjectDictionary<TEnumeration, TValue>.GetKeys
  : TArray<TEnumeration>;
var
  I, AIndex: Integer;
begin
...
procedure TdxEnumeratedObjectDictionary<TEnumeration, TValue>.SetItem
  (AKey: TEnumeration; const Value: TValue);
begin
  AddOrSetValue(AKey, Value)
end;
...
begin
  if (AIndex < 0) or (ACount < 0) or (AIndex + ACount > Count) or
...
begin
  ...
  if ANextIndex < FCount then
    System.Move(FList[ANextIndex], FList[AIndex], (FCount - ANextIndex) *
      SizeOf(NativeUInt));
...
begin
  ...
  if AIndex < FCount then
    System.Move(FList[AIndex + 1], FList[AIndex],
      (FCount - AIndex) * SizeOf(NativeUInt));
  FList[FCount] := 0;
end;
...
procedure TdxOrdinalList.Notify(const AItem: NativeUInt;
...
begin
  ...
  if AIndex < Count then
    System.Move(FList[AIndex], FList[AIndex + 1],
      (Count - AIndex) * SizeOf(NativeUInt));
  FList[AIndex] := AItem;
  Inc(FCount);
  Notify(AItem, cnAdded);
end;
...
procedure TdxOrdinalList.Sort(ACompare: TCompareItems;
  AMultiThreaded: Boolean = False);
begin
  dxQuickSortList(@FList[0], Count, ACompare, AMultiThreaded);
end;

procedure TdxOrdinalList.Sort(ACompare: TListSortCompare;
  AMultiThreaded: Boolean = False);
begin
  dxQuickSortList(@FList[0], Count, ACompare, AMultiThreaded);
end;

function TdxOrdinalList.Extract(AItem: NativeUInt;
  ADirection: TList.TDirection = FromBeginning): NativeUInt;
var
  I: Integer;
begin
...
begin
  ...
  if (ATypeInfo = nil) or not(ATypeInfo.Kind in [tkInteger, tkChar,
    tkEnumeration, tkClass, tkWChar, tkPointer]) then
...
begin
  TdxOrdinalHelper.SetValue(ItemSize, inherited Extract(PNativeUInt(@AItem)^ and
    ValueMask, ADirection), @Result);
end;

function TdxOrdinalList<T>.ExtractByIndex(AIndex: Integer): T;
begin
  TdxOrdinalHelper.SetValue(ItemSize, inherited ExtractByIndex(AIndex),
    @Result);
end;
...
constructor TdxTopologicalSorterNode.Create(AOneBasedSuccessorIndex: Integer;
  ANext: TdxTopologicalSorterNode);
begin
  inherited Create;
...
begin
  ...
  ADependencyCalculator := TDefaultDependencyCalculator.Create(ASourceObjects,
    AInnerComparer);
...
begin
  ...
  FNodeList.Add(TdxTopologicalSorterNode.Create(AOneBasedSuccessorIndex,
    Nodes[AOneBasedPredecessorIndex]));
...
function TdxTopologicalSorter<T>.ProcessNodes(ALastNoPredecessorItemIndex
  : Integer; ASourceObjects: TdxList<T>): TdxList<T>;
var
...
class function TdxComparer<T>.Construct(const Comparison: TComparison<T>)
  : IComparer<T>;
begin
...
class function TdxAlgorithms<T>.BinarySearch(AList: TList<T>; const AValue: T;
...
begin
  ...
  begin
    AMedian := AIndex + ((AHi - AIndex) shr 1);
    ACompareResult := APredicate.CompareTo
      (T(AList{$IFDEF DELPHIXE3}.List{$ENDIF}[AMedian]));
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
class function TdxAlgorithms<T>.BinarySearch(AList: TList<T>;
  const APredicate: IdxComparable<T>; out AIndex: Integer): Boolean;
begin
  Result := TdxAlgorithms<T>.BinarySearch(AList, APredicate, 0,
    AList.Count - 1, AIndex);
end;

class function TdxAlgorithms<T>.BinarySearch(AList: TList<T>;
  const APredicate: TdxComparableFunc<T>; out AIndex: Integer): Boolean;
begin
...
begin
  ...
  begin
    AMedian := AIndex + (AHi - AIndex) shr 1;
    ACompareResult := APredicate(AList{$IFDEF DELPHIXE3}.List{$ENDIF}[AMedian]);
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
class function TdxAlgorithms1<T>.BinarySearch(AList: TdxList<T>;
  const APredicate: IdxComparable<T>; AStartIndex, AEndIndex: Integer;
  out AIndex: Integer): Boolean;
var
  AHi, AMedian: Integer;
...
begin
  ...
  begin
    AMedian := AIndex + (AHi - AIndex) shr 1;
    ACompareResult := APredicate.CompareTo
      ({$IFDEF DELPHIXE2}T(AList.List[AMedian]){$ELSE}AList[AMedian]{$ENDIF});
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
class function TdxAlgorithms1<T>.BinarySearchReverseOrder(AList: TdxList<T>;
  const APredicate: IdxComparable<T>; AStartIndex, AEndIndex: Integer;
  out AIndex: Integer): Boolean;
var
  AHi, AMedian, ACompareResult: Integer;
...
begin
  ...
  begin
    AMedian := AIndex + (AHi - AIndex) shr 1;
    ACompareResult := APredicate({$IFDEF DELPHIXE2}T(AList.List[AMedian]
      ){$ELSE}AList[AMedian]{$ENDIF});
    if ACompareResult = 0 then
    begin
      AIndex := AMedian;
      Exit(True);
...
class function TdxAlgorithms1<T>.BinarySearchReverseOrder(AList: TdxList<T>;
  const APredicate: IdxComparable<T>; out AIndex: Integer): Boolean;
begin
  Result := BinarySearchReverseOrder(AList, APredicate, 0,
    AList.Count - 1, AIndex);
end;
...
function TdxSortedList<T>.BinarySearch(const APredicate
  : IdxComparable<T>): Integer;
begin
...
constructor TdxHashSet<T>.Create(ACapacity: Integer;
...
constructor TdxMap<TKey, TValue>.Create(AComparerKey: IEqualityComparer<TKey>;
  AComparerValue: IEqualityComparer<TValue>; AOwnerships: TDictionaryOwnerships;
  ACapacity: Integer);
begin
  FValueToKey := TDictionary<TValue, TKey>.Create(ACapacity, AComparerValue);
  if AOwnerships <> [] then
    FKeyToValue := TObjectDictionary<TKey, TValue>.Create(AOwnerships,
      ACapacity, AComparerKey)
  else
    FKeyToValue := TDictionary<TKey, TValue>.Create(ACapacity, AComparerKey);
end;

constructor TdxMap<TKey, TValue>.Create(ACapacity: Integer;
...
function TdxMap<TKey, TValue>.TryGetKey(const AValue: TValue;
  out AKey: TKey): Boolean;
begin
...
function TdxMap<TKey, TValue>.TryGetValue(const AKey: TKey;
  out AValue: TValue): Boolean;
begin
...
procedure TdxClassDictionary<T>.AddOrSetValue(const AClass: TClass;
  const AValue: T);
begin
...
function TdxClassDictionary<T>.TryGetValue(AObject: TObject;
  out AValue: T): Boolean;
begin
  if AObject <> nil then
...
function TdxClassDictionary<T>.TryGetValue(AClass: TClass;
  out AValue: T): Boolean;
begin
  repeat
...
begin
  if not TryGetValue(AClass, Result) then
    raise Exception.CreateFmt('Value for the %s class was not found',
      [AClass.ClassName]);
end;
