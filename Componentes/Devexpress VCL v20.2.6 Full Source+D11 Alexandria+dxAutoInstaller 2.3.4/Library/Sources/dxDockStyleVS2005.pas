...
uses
  ...
  dxCore, cxClasses, cxControls, cxGraphics, dxDockControl, cxGeometry,
  dxSkinsCore;

type
  TdxDockingTargetSelectorZoneKind = (tszLeft, tszRight, tszTop, tszBottom,
    tszClient, tszDockSiteLeft, tszDockSiteRight, tszDockSiteTop,
    tszDockSiteBottom);
  ...
  public
    ...
    procedure DrawClientTargetSelectorBackground(ACanvas: TcxCanvas;
      const R: TRect); virtual; abstract;
    ...
    procedure DrawZone(ACanvas: TcxCanvas; const R: TRect; AActive: Boolean;
      AEnabled: Boolean; AKind: TdxDockingTargetSelectorZoneKind);
      virtual; abstract;
    function GetZoneSize(AKind: TdxDockingTargetSelectorZoneKind): TSize;
      virtual; abstract;
    //
  ...
  public
    ...
    // Zones
    procedure DrawZone(ACanvas: TcxCanvas; const R: TRect;
      AActive, AEnabled: Boolean;
      AKind: TdxDockingTargetSelectorZoneKind); override;
    function GetZoneSize(AKind: TdxDockingTargetSelectorZoneKind)
      : TSize; override;
  end;
  ...
  strict private
    ...
    function GetTargetZoneViewInfo(Index: Integer)
      : TdxDockingTargetSelectorZoneViewInfo;
  ...
  protected
    function AddZoneViewInfo(AKind: TdxDockingTargetSelectorZoneKind;
      const ABounds: TRect): TdxDockingTargetSelectorZoneViewInfo;
  ...
  public
    constructor Create(AHelper: TdxDockingControllerVS2005Helper);
      reintroduce; virtual;
    destructor Destroy; override;
    procedure CalculateHitTest(const AScreenPoint: TPoint); virtual;
    function GetTargetZoneAtPoint(const APoint: TPoint)
      : TdxDockingTargetSelectorZoneViewInfo;
    procedure Update; override;
    //
    property ActiveTargetZone: TdxDockingTargetSelectorZoneViewInfo
      read FActiveTargetZone write SetActiveTargetZone;
    property DockControl: TdxCustomDockControl read FDockControl
      write SetDockControl;
    ...
    property TargetZoneSize[AKind: TdxDockingTargetSelectorZoneKind]: TSize
      read GetTargetZoneSize;
    property TargetZoneViewInfo[Index: Integer]
      : TdxDockingTargetSelectorZoneViewInfo read GetTargetZoneViewInfo;
  ...
  strict private
    ...
    function GetTargetSelector(AType: TdxDockingType)
      : TdxDockingCustomTargetSelector;
  protected
    function CheckTargetDockControl(const APoint: TPoint;
      AControl, ATargetDockControl: TdxCustomDockControl): Boolean; virtual;
    function CreateTargetSelector(AType: TdxDockingType)
      : TdxDockingCustomTargetSelector; virtual;
    function GetDockingTargetControl(AControl: TdxCustomDockControl;
      const APoint: TPoint): TdxCustomDockControl; virtual;
    ...
    procedure SetDockingParams(AControl: TdxCustomDockControl; AZone: TdxZone;
      const APoint: TPoint);
    procedure UpdateTargetSelectors(ATargetDockControl, ATargetDockSite
  ...
  public
    ...
    procedure DockingStart(AControl: TdxCustomDockControl;
      const APoint: TPoint); override;
    //
    property TargetSelector[AType: TdxDockingType]
      : TdxDockingCustomTargetSelector read GetTargetSelector;
  end;
  ...
  protected
    ...
    procedure PlaceTargetSelector(const ATargetWindowRect: TRect);
      virtual; abstract;
  public
...
procedure TdxDockingTargetSelectorCustomPainter.Initialize
...
procedure TdxDockingTargetSelectorPainter.DrawClientTargetSelectorBackground
...
procedure TdxDockingTargetSelectorPainter.DrawZone(ACanvas: TcxCanvas;
  const R: TRect; AActive, AEnabled: Boolean;
  AKind: TdxDockingTargetSelectorZoneKind);
const
...
begin
  ...
  try
    ...
    dxGPPaintCanvas.PixelOffsetMode := PixelOffsetModeHalf;
    if AKind in [tszDockSiteLeft, tszDockSiteRight, tszDockSiteTop,
      tszDockSiteBottom] then
      FBackgroundDockSite.DrawEx(dxGPPaintCanvas, R);
    FSelectors[AKind].DrawEx(dxGPPaintCanvas, R, ScaleFactor,
      StatesMap[AEnabled, AActive]);
  finally
    dxGPPaintCanvas.EndPaint;
  end;
end;

function TdxDockingTargetSelectorPainter.
  GetClientTargetSelectorContentOffsets: TRect;
begin
  Result := ScaleFactor.Apply(cxRect(1, 1, 0, 0));
end;

function TdxDockingTargetSelectorPainter.GetZoneSize
  (AKind: TdxDockingTargetSelectorZoneKind): TSize;
begin
  Result := ScaleFactor.Apply(cxSize(FBackgroundDockSite.Texture.Width,
    FBackgroundDockSite.Texture.Height));
end;

procedure TdxDockingTargetSelectorPainter.Initialize(AScaleFactor
  : TdxScaleFactor);
begin
  ResourcesRelease;
...
function TdxDockingTargetSelectorPainter.LoadTexture(AResourceName: string)
  : TdxSkinImage;
var
  ABitmap: TBitmap;
...
const
  SelectorResNames: array [TdxDockingTargetSelectorZoneKind] of string =
    ('DXDOCKINGVS2005LEFT', 'DXDOCKINGVS2005RIGHT', 'DXDOCKINGVS2005TOP',
    'DXDOCKINGVS2005BOTTOM', 'DXDOCKINGVS2005CLIENT', 'DXDOCKINGVS2005LEFT',
    'DXDOCKINGVS2005RIGHT', 'DXDOCKINGVS2005TOP', 'DXDOCKINGVS2005BOTTOM');
var
...
constructor TdxDockingCustomTargetSelector.Create
  (AHelper: TdxDockingControllerVS2005Helper);
begin
  inherited Create;
  FHelper := AHelper;
...
function TdxDockingCustomTargetSelector.AddZoneViewInfo
  (AKind: TdxDockingTargetSelectorZoneKind; const ABounds: TRect)
  : TdxDockingTargetSelectorZoneViewInfo;
  ...
  const
    DockingTypeMap: array [TdxDockingTargetSelectorZoneKind] of TdxDockingType =
      (dtLeft, dtRight, dtTop, dtBottom, dtClient, dtLeft, dtRight, dtTop,
      dtBottom);
  begin
    Result := TdxCustomDockControlAccess(DockControl)
      .DockZones.FindZone(AOwnerControl, DockingTypeMap[AKind]);
  end;

  function CanDocking(AZone: TdxZone;
    ADockControl: TdxCustomDockControl): Boolean;
  begin
    Result := (AZone <> nil) and AZone.CanDock(ADockControl) and
      FHelper.CanDocking(ADockControl, AZone.Owner, AZone,
      cxRectCenter(ABounds));
  end;
...
procedure TdxDockingCustomTargetSelector.CalculateHitTest
...
function TdxDockingCustomTargetSelector.CreatePainter
  : TdxDockingTargetSelectorCustomPainter;
begin
...
function TdxDockingCustomTargetSelector.GetTargetZoneSize
  (AKind: TdxDockingTargetSelectorZoneKind): TSize;
begin
  if Painter <> nil then
...
function TdxDockingCustomTargetSelector.GetTargetZoneViewInfo(Index: Integer)
  : TdxDockingTargetSelectorZoneViewInfo;
begin
...
procedure TdxDockingCustomTargetSelector.SetActiveTargetZone
  (AValue: TdxDockingTargetSelectorZoneViewInfo);
begin
...
procedure TdxDockingCustomTargetSelector.SetDockControl
...
begin
  ...
  Result := cxRectInflate(Result,
    Painter.GetClientTargetSelectorContentOffsets);
end;
...
begin
  ...
  ARect := cxRectOffset(ARect,
    Painter.GetClientTargetSelectorContentOffsets.TopLeft);
...
procedure TdxDockingClientTargetSelector.CalculateHorizontalZones;
  ...
  begin
    Result := cxRectCenterVertically(cxRectSetWidth(R, AZoneSize.cx),
      AZoneSize.cy);
  end;
...
begin
  ARect := cxRectBounds(0, TargetZoneSize[tszTop].cy, 0,
    CalculateHorizontalRowMaxSize);
  ARect := cxRectOffset(ARect,
    Painter.GetClientTargetSelectorContentOffsets.TopLeft);

  AZoneViewInfo := AddZoneViewInfo(tszLeft,
    PlaceZone(ARect, TargetZoneSize[tszLeft]));
...
procedure TdxDockingClientTargetSelector.CalculateVerticalZones;
  ...
  function PlaceZone(const R: TRect; const AZoneSize: TSize): TRect;
  begin
    Result := cxRectCenterHorizontally(cxRectSetHeight(R, AZoneSize.cy),
      AZoneSize.cx);
  end;
...
var
  ...
  AZoneViewInfo: TdxDockingTargetSelectorZoneViewInfo;
begin
  ARect := cxRectBounds(TargetZoneSize[tszLeft].cx, 0,
    CalculateVerticalRowMaxSize, 0);
  ARect := cxRectOffset(ARect,
    Painter.GetClientTargetSelectorContentOffsets.TopLeft);

  AZoneViewInfo := AddZoneViewInfo(tszTop,
    PlaceZone(ARect, TargetZoneSize[tszTop]));
...
function TdxDockingLeftTargetSelector.GetZoneKind
  : TdxDockingTargetSelectorZoneKind;
begin
  Result := tszDockSiteLeft;
end;

procedure TdxDockingLeftTargetSelector.PlaceTargetSelector
  (const ATargetWindowRect: TRect);
begin
  MoveTo(Point(ATargetWindowRect.Left, cxRectCenterVertically(ATargetWindowRect,
    Height).Top));
end;

{ TdxDockingRightTargetSelector }

function TdxDockingRightTargetSelector.GetZoneKind
  : TdxDockingTargetSelectorZoneKind;
begin
  Result := tszDockSiteRight;
end;

procedure TdxDockingRightTargetSelector.PlaceTargetSelector
  (const ATargetWindowRect: TRect);
begin
  MoveTo(Point(ATargetWindowRect.Right - Width,
    cxRectCenterVertically(ATargetWindowRect, Height).Top));
end;

{ TdxDockingTopTargetSelector }

function TdxDockingTopTargetSelector.GetZoneKind
  : TdxDockingTargetSelectorZoneKind;
begin
  Result := tszDockSiteTop;
end;

procedure TdxDockingTopTargetSelector.PlaceTargetSelector
  (const ATargetWindowRect: TRect);
begin
  MoveTo(Point(cxRectCenterHorizontally(ATargetWindowRect, Width).Left,
    ATargetWindowRect.Top));
end;

{ TdxDockingBottomTargetSelector }

function TdxDockingBottomTargetSelector.GetZoneKind
  : TdxDockingTargetSelectorZoneKind;
begin
  Result := tszDockSiteBottom;
end;

procedure TdxDockingBottomTargetSelector.PlaceTargetSelector
  (const ATargetWindowRect: TRect);
begin
  MoveTo(Point(cxRectCenterHorizontally(ATargetWindowRect, Width).Left,
    ATargetWindowRect.Bottom - Height));
end;
...
const
  TargetSelectorsClasses: array [TdxDockingType]
    of TdxDockingCustomTargetSelectorClass = (TdxDockingClientTargetSelector,
    TdxDockingLeftTargetSelector, TdxDockingTopTargetSelector,
    TdxDockingRightTargetSelector, TdxDockingBottomTargetSelector);
begin
  Result := TargetSelectorsClasses[AType].Create(Self);
end;

function TdxDockingControllerVS2005Helper.CheckTargetDockControl
  (const APoint: TPoint; AControl, ATargetDockControl
  ...
  begin
    ...
    begin
      ...
      if cxRectIsEmpty(R) then
        Result := (ATargetDockControl is TdxDockSite) and
          TdxDockSite(ATargetDockControl).AutoSize
      else
        Result := ATargetDockControl = GetDockingTargetControl(AControl,
          ATargetDockControl.ClientToScreen(cxRectCenter(R)));
    end
    else
      Result := False;
  end;
...
procedure TdxDockingControllerVS2005Helper.DockingMove
  ...
  function GetTabContainerZone(ATargetDockControl: TdxCustomDockControl;
    out AZone: TdxZone): Boolean;
  begin
    if ATargetDockControl <> nil then
...
begin
  ...
  begin
    AStyles := dxSetWindowStyle(AControl.FloatForm.Handle, WS_VISIBLE,
      soSubtract);
    Result := TdxCustomDockControlAccess(AControl)
      .GetDockingTargetControlAtPos(APoint);
...
function TdxDockingControllerVS2005Helper.GetDockSite(AControl: TWinControl)
  : TdxCustomDockSite;
begin
  Result := nil;
...
function TdxDockingControllerVS2005Helper.GetTargetZone
  (const APoint: TPoint): TdxZone;
var
  ADockingType: TdxDockingType;
begin
