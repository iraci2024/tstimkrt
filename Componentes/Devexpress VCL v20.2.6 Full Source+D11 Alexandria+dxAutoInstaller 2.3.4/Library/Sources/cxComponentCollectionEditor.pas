...
uses
  Types,
  DesignIntf, DesignWindows, ComponentDesigner, DesignConst, DesignEditors,
  ColnEdit,
...
begin
  ...
  TfrmComponentCollectionEditor(ShowFormEditorClass(Designer, TComponent(Obj),
    TcxComponentCollection(GetOrdValue), GetName, GetEditorClass)).Options :=
    GetColOptions;
end;
...
function TcxComponentCollectionProperty.GetEditorClass
...
begin
  ...
  try
    ...
    if (PrevCount <> Collection.Count) then
      UpdateListbox
    else if ListView1.Items.Count > PrevListCount then
...
begin
  ...
  if not((List[0] = Component) or (List[0] = Collection) or
    (TcxComponentCollectionItem(List[0]).Collection = Collection)) then
    Exit;
...
procedure TfrmComponentCollectionEditor.SelectionsChanged(const ASelection
...
procedure TfrmComponentCollectionEditor.FormKeyDown(Sender: TObject;
...
procedure TfrmComponentCollectionEditor.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    ComponentDesigner.Environment.ModalEdit(#0, Self);
...
begin
  if (Change = ctState) and (LockCount = 0) then
    if not PeekMessage(Msg, Handle, AM_DeferUpdate, AM_DeferUpdate, PM_NOREMOVE)
    then
      PostMessage(Handle, AM_DeferUpdate, 0, 0);
end;

procedure TfrmComponentCollectionEditor.ListView1DragDrop(Sender,
...
procedure TfrmComponentCollectionEditor.ListView1DragOver(Sender,
...
procedure TfrmComponentCollectionEditor.ListView1KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if Key = VK_PROCESSKEY then
    ComponentDesigner.Environment.ModalEdit(#0, Self);
...
begin
  ...
  begin
    ...
    PostMessage(Handle, AM_DeferUpdate, TMessage(Msg).WParam,
      TMessage(Msg).LParam);
end;
...
begin
  Caption := Format('Editing %s%s%s', [Component.Name, DotSep,
...
begin
  if ListView1.SelCount = 0 then
    Exit;
  if (ListView1.SelCount = Collection.Count) and
    ((csAncestor in Component.ComponentState) = False) then
    Collection.Clear
  else
    for I := ListView1.Items.Count - 1 downto 0 do
      if ListView1.Items[I].Selected then
