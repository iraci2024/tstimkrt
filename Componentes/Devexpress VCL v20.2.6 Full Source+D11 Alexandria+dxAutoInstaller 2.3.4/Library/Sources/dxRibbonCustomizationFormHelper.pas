...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Windows, SysUtils, Classes, Types, Controls, Graphics, Dialogs, Math,
  IniFiles, dxCore, cxClasses, cxGeometry,
  cxGraphics, cxTL, dxBar, dxRibbon, cxDropDownEdit, cxLookAndFeelPainters,
  dxBarStrs, cxCheckBox;
...
type
  ...
  public
    constructor Create(AItem: TdxBarItem; AItemLink: TdxBarItemLink);
      reintroduce;
    ...
    property IsNewItemNeeded: Boolean read FIsNewItemNeeded
      write SetIsNewItemNeeded;
    property IsNewItemLinkNeeded: Boolean read GetIsNewItemLinkNeeded
      write FIsNewItemLinkNeeded;
  ...
  protected
    function AddTabInfo(ATab: TdxRibbonTab)
      : TdxRibbonCustomizationFormMergedTabInfo;

    property Items[Index: Integer]: TdxRibbonCustomizationFormMergedTabInfo
  ...
  private
    ...
    procedure CompareNodes(Sender: TcxCustomTreeList;
    ...
    function GetItemNodeCaption(AItem: TdxBarItem;
      AItemLink: TdxBarItemLink): string;
    function GetRealBounds(ANode: TcxTreeListNode): TRect;
  protected
    procedure AfterNodeMoved(ASourceNode, ANewNode: TcxTreeListNode);
      virtual; abstract;
    procedure ConvertItemNodeToNew(ANode: TcxTreeListNode); virtual;
    procedure CopyNode(ASourceNode, ATargetNode: TcxTreeListNode;
      ACopyData: Boolean = True);
    procedure DeleteItemLink(AItemLink: TdxBarItemLink);
    function GetItemData(AItemNode: TcxTreeListNode)
      : TdxRibbonCustomizationFormItemNodeData;
    function GetItemNodeVisible(AItem: TdxBarItem): Boolean; virtual;
    function GetNodeFor(AData: TPersistent; AParentNode: TcxTreeListNode)
      : TcxTreeListNode;
    function GetTargetNodeForMovingFocusedNodeDown: TcxTreeListNode;
      virtual; abstract;
    function GetTargetNodeForMovingFocusedNodeUp: TcxTreeListNode;
      virtual; abstract;
    procedure LoadItemLinksContentFormIni(AItemLinks: TdxBarItemLinks;
      ATargetNode: TcxTreeListNode; ATargetTreeList: TcxTreeList;
      ASource: TCustomIniFile);
    procedure PopulateAllItems(ASource: TdxBarManager;
      const AOnlyMissingItems: Boolean = False);
    procedure PopulateCommandsComboBoxContent; virtual; abstract;
    procedure PopulateItemNodeContent(ATargetNode: TcxTreeListNode;
      AItem: TdxBarItem; AItemLink: TdxBarItemLink;
      const ANeedBeginGroupNode: Boolean = False;
      ASource: TCustomIniFile = nil); virtual;
    ...
    procedure DoAddNewItemLink(AItemLinkNode: TcxTreeListNode;
      AItemLinks: TdxBarItemLinks);
    procedure DoDeleteMissingItemLinks(AItemLinks: TdxBarItemLinks;
      AParentNode: TcxTreeListNode);
    procedure DoDropMovedNodeTo(ATargetNode: TcxTreeListNode;
      ACoordinate: Integer); virtual;
    procedure DoMoveNode(ASourceNode, ATargetNode: TcxTreeListNode;
      const AInsertBeforeTargetNode: Boolean;
      const ATargetNodeIsParent: Boolean = False);

    function IsAboveNodeCenter(ANode: TcxTreeListNode;
      const ACoordinate: Integer): Boolean;
    function IsCenterNodePart(ANode: TcxTreeListNode;
      const ACoordinate: Integer): Boolean;
    function IsTopNodePart(ANode: TcxTreeListNode;
      const ACoordinate: Integer): Boolean;

    function IsDuplicatedIn(ASourceNode, ATargetNode: TcxTreeListNode;
      ACoordinate: Integer): Boolean; virtual;
    function IsPartOfItemsStructure(AItemNode, AStructureOwnerNode
      : TcxTreeListNode): Boolean;
    function IsRecursiveInclusion(ASourceNode, ATargetNode: TcxTreeListNode;
      ACoordinate: Integer): Boolean;
    function IsRibbonContainsItem(AItem: TdxBarItem): Boolean;
    function IsSameElement(ASourceNode, ATargetNode: TcxTreeListNode)
      : Boolean; overload;
    function IsSameElement(ANode: TcxTreeListNode; AData: TPersistent)
      : Boolean; overload;
    function IsSourceNodeValid(ASourceNode: TcxTreeListNode): Boolean; virtual;
    function IsTargetNodeValid(ASourceNode, ATargetNode: TcxTreeListNode)
      : Boolean; virtual;
  ...
  public
    constructor Create(ARibbon: TdxCustomRibbon;
      ACommandsTreeList, ARibbonTreeList: TcxTreeList;
    ...
    procedure DropMovedNodeTo(ATargetNode: TcxTreeListNode;
      const ACoordinate: Integer = -1);
    procedure FreeNodeData(ANode: TcxTreeListNode);
    function GetNodeLevel(ANode: TcxTreeListNode;
      const AAsTyped: Boolean = False): Integer; virtual; abstract;
    ...
    procedure PopulateRibbonTreeListContent(const AIsReseting: Boolean = False);
      virtual; abstract;
    procedure ReleaseMovedNode;
    procedure RemoveNode(ANode: TcxTreeListNode); virtual; abstract;
    procedure RenameNode(ANode: TcxTreeListNode;
      const ANewCaption: string); virtual;
    procedure UpdateRibbonActiveTab;

    function DrawItemNode(Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
    ...
    function CanMoveNodeTo(ASourceNode, ATargetNode: TcxTreeListNode;
      const ACoordinate: Integer = -1): Boolean; virtual; abstract;
  ...
  TdxRibbonCustomizationFormHelper = class
    (TdxCustomRibbonCustomizationFormHelper)
  strict private
    function GetContextCaption(AContext: TdxRibbonContext): string;
  protected
    function AddNewNodeToFocusedNode(const ATargetLevel: Integer;
      const ACaption: string): TcxTreeListNode;
    ...
    function CreateContextNode(AContext: TdxRibbonContext;
      ATargetTreeList: TcxTreeList; AIsReseting: Boolean = False)
      : TcxTreeListNode;
    ...
    function LoadExistingGroupNodeContentFormIni(AToolBar: TdxBar;
      AParentTabNode: TcxTreeListNode; ATargetTreeList: TcxTreeList;
      ASource: TCustomIniFile): TcxTreeListNode;
    function LoadMissingGroupNodeContentFormIni(AToolBar: TdxBar;
      AParentTabNode: TcxTreeListNode; ATargetTreeList: TcxTreeList;
      ASource: TCustomIniFile): TcxTreeListNode;
    procedure LoadTabNodeContentFormIni(ANode: TcxTreeListNode;
      ATargetTreeList: TcxTreeList; ASource: TCustomIniFile);
    procedure LoadTabsOrderFormIni(AContextNode: TcxTreeListNode;
      ASource: TCustomIniFile);
    procedure PopulateCommandsComboBoxContent; override;
    procedure PopulateCustomTabsAndGroups;
    procedure PopulateGroupNodeContent(ANode: TcxTreeListNode;
      ATargetTreeList: TcxTreeList);
    procedure PopulateRibbonComboBoxContent; override;
    procedure PopulateTabNodeContent(ANode: TcxTreeListNode;
      ATargetTreeList: TcxTreeList);
    ...
    procedure CheckAndDeleteGroupsFromRibbon(AGroups: TdxRibbonTabGroups;
    ...
    function IsParentNodeTheSame(ASourceNode, ATargetNode
      : TcxTreeListNode): Boolean;
    function IsRibbonGroupNodeWithSharedToolBar(ANode: TcxTreeListNode)
      : Boolean;
  ...
  public
    ...
    function GetNodeLevel(ANode: TcxTreeListNode;
      const AAsTyped: Boolean = False): Integer; override;
    ...
    procedure PopulateRibbonTreeListContent(const AIsReseting
      : Boolean = False); override;
    procedure RemoveNode(ANode: TcxTreeListNode); override;
    procedure RenameNode(ANode: TcxTreeListNode;
      const ANewCaption: string); override;
    ...
    function CanMoveNodeTo(ASourceNode, ATargetNode: TcxTreeListNode;
      const ACoordinate: Integer = -1): Boolean; override;
    function CanRemoveFocusedNode: Boolean; override;
    function CanResetFocusedNode: Boolean;

    function DrawContextNode(Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
      AViewInfo: TcxTreeListEditCellViewInfo;
  ...
  TdxRibbonQATCustomizationFormHelper = class
    (TdxCustomRibbonCustomizationFormHelper)
  private
...
begin
  Result := TdxBarManagerAccess(ABarManager).GetBarManagerSection
    (GetBarManagerBaseIniSection(ABarManager), skIni);
end;

function GetBarIniSection(ABarManager: TdxBarManager;
  const AIndex: Integer): string;
begin
  Result := TdxBarAccess.GetIniSection
    (GetBarManagerBaseIniSection(ABarManager), AIndex);
end;

function GetItemLinkIniSection(const ABaseSection: string;
  const AIndex: Integer): string;
begin
  Result := TdxBarItemLinkAccess.GetIniSection(ABaseSection, AIndex, skIni);
end;

function GetSubItemIniSection(ABarManager: TdxBarManager;
  ASubItem: TCustomdxBarSubItem): string;
begin
  Result := TCustomdxBarSubItemAccess(ASubItem)
    .GetIniSection(GetBarManagerBaseIniSection(ABarManager));
end;

{ TdxRibbonCustomizationFormItemNodeData }

constructor TdxRibbonCustomizationFormItemNodeData.Create(AItem: TdxBarItem;
...
function TdxRibbonCustomizationFormItemNodeData.Clone
  : TdxRibbonCustomizationFormItemNodeData;
begin
...
begin
  ...
  begin
    ...
    Result.Name := Result.BarManager.GetUniqueItemName
      (TdxBarItemClass(Result.ClassType));
    Result.Assign(Item);
    FIsNewItemNeeded := False;
  end;
end;

function TdxRibbonCustomizationFormItemNodeData.CreateEqualItemLink
  (AOwner: TdxBarItemLinks): TdxBarItemLink;
begin
...
procedure TdxRibbonCustomizationFormItemNodeData.DrawImage(ACanvas: TcxCanvas;
  var ARect: TRect; AScaleFactor: TdxScaleFactor = nil);
var
  AImageWidth: Integer;
  AImageRect: TRect;
...
begin
  ...
  begin
    ...
    if (Item is TdxRibbonQuickAccessGroupButton) and
      (TdxRibbonQuickAccessGroupButton(Item).Toolbar <> nil) then
    ...
    cxDrawImage(ACanvas, AImageRect, AGlyph, Item.BarManager.Images,
      Item.ImageIndex, ifmFit, idmNormal, True, nil, AScaleFactor);
  end;
end;
...
procedure TdxRibbonCustomizationFormItemNodeData.LoadItemLinkFromIni
  (AItemLink: TdxBarItemLink);
var
  AItemLinks: TdxBarItemLinks;
  ASection: string;
...
begin
  ...
  else if AItemLinks.Owner is TdxBar then
    ASection := GetBarIniSection(AItemLinks.BarManager,
      (AItemLinks.Owner as TdxBar).Index)
  else if AItemLinks.Owner is TCustomdxBarSubItem then
    ASection := GetSubItemIniSection(AItemLinks.BarManager,
      AItemLinks.Owner as TCustomdxBarSubItem)
  else
    ASection := '';

  ASource := TdxBarManagerAccess(AItemLinks.BarManager).ReadSavedState;
  try
    for AIndexInIni := 0 to ASource.ReadInteger(ASection,
      'ItemLinkCount', 0) - 1 do
    begin
      if SameText(Item.Name, ASource.ReadString(GetItemLinkIniSection(ASection,
        AIndexInIni), 'ItemName', '')) then
        Break;
    end;
    TdxBarItemLinkAccess(AItemLink).LoadFromIni(ASource, ASection,
      AIndexInIni, skIni);
  finally
    ASource.Free;
  end;
...
function TdxRibbonCustomizationFormItemNodeData.GetItemAsSubItem
  : TCustomdxBarSubItem;
begin
  Result := Item as TCustomdxBarSubItem;
end;

procedure TdxRibbonCustomizationFormItemNodeData.SetIsNewItemNeeded
  (AValue: Boolean);
begin
...
constructor TdxRibbonCustomizationFormMergedTabInfo.Create
  (AOwnerTab: TdxRibbonTab);
begin
  inherited Create;
...
constructor TdxRibbonCustomizationFormMergedTabsList.Create
...
function TdxRibbonCustomizationFormMergedTabsList.AddTabInfo(ATab: TdxRibbonTab)
  : TdxRibbonCustomizationFormMergedTabInfo;
var
  AIndex: Integer;
begin
...
function TdxRibbonCustomizationFormMergedTabsList.GetItem(AIndex: Integer)
  : TdxRibbonCustomizationFormMergedTabInfo;
begin
...
constructor TdxCustomRibbonCustomizationFormHelper.Create
  (ARibbon: TdxCustomRibbon; ACommandsTreeList, ARibbonTreeList: TcxTreeList;
  ACommandsComboBox, ARibbonComboBox: TcxComboBox);
begin
  inherited Create;
  FRibbon := ARibbon;
  ...
  FMergedTabsList := TdxRibbonCustomizationFormMergedTabsList.Create
    (Ribbon.Tabs);
...
procedure TdxCustomRibbonCustomizationFormHelper.AddNode
...
begin
  if (RibbonTreeList.FocusedNode = nil) and (RibbonTreeList.Count > 0) and
    (FocusedNode <> nil) then
    RibbonTreeList.FocusedNode := FocusedNode;
end;

procedure TdxCustomRibbonCustomizationFormHelper.DropMovedNodeTo
  (ATargetNode: TcxTreeListNode; const ACoordinate: Integer = -1);
begin
  if CanMoveNodeTo(MovedNode, ATargetNode, ACoordinate) then
    DoDropMovedNodeTo(ATargetNode, ACoordinate);
end;

procedure TdxCustomRibbonCustomizationFormHelper.FreeNodeData
  (ANode: TcxTreeListNode);
begin
...
procedure TdxCustomRibbonCustomizationFormHelper.InitializeMovedNode
  (ANode: TcxTreeListNode);
begin
  LockContentUpdating;
  FMovedNode := ANode;
...
procedure TdxCustomRibbonCustomizationFormHelper.RenameNode
  (ANode: TcxTreeListNode; const ANewCaption: string);
begin
...
function TdxCustomRibbonCustomizationFormHelper.DrawItemNode
...
begin
  ...
  ARect := cxRectCenterVertically(ARect,
    TdxTextMeasurer.TextLineHeight(ACanvas.Handle));
  cxDrawText(ACanvas, AViewInfo.Node.Texts[0], ARect,
    AHorzAlignment[AViewInfo.IsRightToLeftConverted]);
  Result := True;
end;

function TdxCustomRibbonCustomizationFormHelper.CanMoveFocusedNodeDown: Boolean;
begin
  Result := IsSourceNodeValid(FocusedNode) and IsTargetNodeValid(FocusedNode,
    GetTargetNodeForMovingFocusedNodeDown);
end;

function TdxCustomRibbonCustomizationFormHelper.CanMoveFocusedNodeUp: Boolean;
begin
  Result := IsSourceNodeValid(FocusedNode) and IsTargetNodeValid(FocusedNode,
    GetTargetNodeForMovingFocusedNodeUp);
end;
...
begin
  Result := (FocusedNode <> nil) and FocusedNode.Enabled and
    ((GetNodeLevel(FocusedNode) <> dxrcfContextLevel) or
    not SameText(FocusedNode.Texts[0],
    cxGetResourceString(@sdxRibbonCustomizationFormMainTabs)));
end;
...
procedure TdxCustomRibbonCustomizationFormHelper.ConvertItemNodeToNew
  (ANode: TcxTreeListNode);
begin
  ANode.Enabled := True;
...
procedure TdxCustomRibbonCustomizationFormHelper.CopyNode(ASourceNode,
  ATargetNode: TcxTreeListNode; ACopyData: Boolean = True);
var
  I: Integer;
  ...
  try
    ...
    if ACopyData then
      ...
      begin
        ...
        ATargetNodeData.IsNewItemLinkNeeded :=
          ATargetNodeData.IsNewItemLinkNeeded or
          (ASourceNode.Parent <> ATargetNode.Parent);
      end
      else
...
procedure TdxCustomRibbonCustomizationFormHelper.DeleteItemLink
  (AItemLink: TdxBarItemLink);
var
  AItem: TdxBarItem;
  I: Integer;
begin
  ...
  if (AItem.LinkCount > 1) or FSavedState.ReadBool
    (GetBarManagerBaseIniSection(Ribbon.BarManager) + 'Items', AItem.Name, False)
  then
...
function TdxCustomRibbonCustomizationFormHelper.GetItemData
  (AItemNode: TcxTreeListNode): TdxRibbonCustomizationFormItemNodeData;
begin
...
function TdxCustomRibbonCustomizationFormHelper.GetItemNodeVisible
  (AItem: TdxBarItem): Boolean;
begin
  Result := AItem.VisibleForCustomization and not(AItem is TdxBarSeparator);
end;

function TdxCustomRibbonCustomizationFormHelper.GetNodeFor(AData: TPersistent;
  AParentNode: TcxTreeListNode): TcxTreeListNode;
begin
  Result := AParentNode;
  if Result <> nil then
  begin
...
procedure TdxCustomRibbonCustomizationFormHelper.LoadItemLinksContentFormIni
  (AItemLinks: TdxBarItemLinks; ATargetNode: TcxTreeListNode;
  ATargetTreeList: TcxTreeList; ASource: TCustomIniFile);
var
...
begin
  ...
  if AItemLinks.Owner is TdxBar then
    ABaseSection := GetBarIniSection(AItemLinks.BarManager,
      GetBarIndexInIni(AItemLinks.Owner as TdxBar, ASource))
  else if AItemLinks.Owner is TCustomdxBarSubItem then
    ABaseSection := GetSubItemIniSection(AItemLinks.BarManager,
      AItemLinks.Owner as TCustomdxBarSubItem);
  for I := 0 to ASource.ReadInteger(ABaseSection, 'ItemLinkCount', 0) - 1 do
  begin
    AItemLinkSection := GetItemLinkIniSection(ABaseSection, I);
    AItem := Ribbon.BarManager.GetItemByName
      (ASource.ReadString(AItemLinkSection, 'ItemName', ''));
    if AItem <> nil then
    begin
      AItemNode := ATargetTreeList.AddChild(ATargetNode);
      AItemLink := AItemLinks.FindByItem(AItem);
      PopulateItemNodeContent(AItemNode, AItem, AItemLink,
        ASource.ReadBool(AItemLinkSection, 'BeginGroup', False), ASource);
...
begin
  Result := (TObject(ANode.Data) is TdxRibbonCustomizationFormItemNodeData) and
    (TdxRibbonCustomizationFormItemNodeData(ANode.Data)
    .Item = TdxBarItem(AData));
end;

procedure TdxCustomRibbonCustomizationFormHelper.PopulateAllItems
  (ASource: TdxBarManager; const AOnlyMissingItems: Boolean = False);

  procedure DoPopulateItem(AItem: TdxBarItem;
    const ACheckForDuplicates: Boolean = False);
  begin
    if ACheckForDuplicates and ((AItem.BarManager = ASource) or
      (CommandsTreeList.Find(AItem, nil, False, True, FindItemProc) <> nil))
    then
      Exit;
...
begin
  ...
  for I := 0 to ASource.ItemCount - 1 do
    DoPopulateItem(ASource.Items[I]);
  ...
  begin
    ...
    if ABar.MergeData.CreatedByMerging or ABar.IsMerged then
      for J := 0 to ABar.ItemLinks.Count - 1 do
        DoPopulateItem(ABar.ItemLinks[J].Item, True)
  end;
end;

procedure TdxCustomRibbonCustomizationFormHelper.PopulateItemNodeContent
  (ATargetNode: TcxTreeListNode; AItem: TdxBarItem; AItemLink: TdxBarItemLink;
  const ANeedBeginGroupNode: Boolean = False; ASource: TCustomIniFile = nil);
var
  ASubItem: TCustomdxBarSubItem;
  ASubItemLink: TdxBarItemLink;
  I: Integer;
begin
  ATargetNode.Data := TdxRibbonCustomizationFormItemNodeData.Create(AItem,
    AItemLink);
  ...
  begin
    ...
    if ASource = nil then
      ...
      begin
        ...
        PopulateItemNodeContent(ATargetNode.AddChild, ASubItemLink.Item,
          ASubItemLink, ASubItemLink.BeginGroup);
      end
    else
      LoadItemLinksContentFormIni(ASubItem.ItemLinks, ATargetNode,
        ATargetNode.TreeList as TcxTreeList, ASource);
  end;
end;

procedure TdxCustomRibbonCustomizationFormHelper.SetFocusedNode
  (ANode: TcxTreeListNode);
begin
  ANode.Focused := True;
  ANode.MakeVisible;
...
procedure TdxCustomRibbonCustomizationFormHelper.SynchronizeMatchingNodesWith
  (APatternNode: TcxTreeListNode);
var
...
begin
  ...
  try
    ...
    begin
      if (GetNodeLevel(ANode, True) = GetNodeLevel(APatternNode, True)) and
        IsSameElement(ANode, APatternNode) and (ANode <> APatternNode) and
        ((GetNodeLevel(ANode, True) <> dxrcfItemLevel) or not GetItemData(ANode)
        .IsNewItemNeeded) then
      begin
...
procedure TdxCustomRibbonCustomizationFormHelper.DoAddNewItemLink
  (AItemLinkNode: TcxTreeListNode; AItemLinks: TdxBarItemLinks);
var
  ANodeData: TdxRibbonCustomizationFormItemNodeData;
begin
  ANodeData := GetItemData(AItemLinkNode);
  if (ANodeData.IsNewItemNeeded or ANodeData.IsNewItemLinkNeeded) and
    (AItemLinks.FindByItem(ANodeData.Item) = nil) then
  begin
...
procedure TdxCustomRibbonCustomizationFormHelper.DoDeleteMissingItemLinks
  (AItemLinks: TdxBarItemLinks; AParentNode: TcxTreeListNode);
var
  AItemNode: TcxTreeListNode;
  I: Integer;
begin
  ...
  try
    ...
    begin
      ...
      else if GetItemData(AItemNode).IsSubItem then
        DoDeleteMissingItemLinks(GetItemData(AItemNode).ItemAsSubItem.ItemLinks,
          AItemNode);
    end;
  finally
...
procedure TdxCustomRibbonCustomizationFormHelper.DoDropMovedNodeTo
  (ATargetNode: TcxTreeListNode; ACoordinate: Integer);
begin
  ...
  if GetNodeLevel(ATargetNode, True) = GetNodeLevel(MovedNode, True) then
    if (GetNodeLevel(ATargetNode, True) = dxrcfItemLevel) and
      GetItemData(ATargetNode).IsSubItem then
      DoMoveNode(MovedNode, ATargetNode, IsTopNodePart(ATargetNode,
        ACoordinate), IsCenterNodePart(ATargetNode, ACoordinate))
    else
      DoMoveNode(MovedNode, ATargetNode, IsAboveNodeCenter(ATargetNode,
        ACoordinate))
  else
    DoMoveNode(MovedNode, ATargetNode, False, True);
end;

procedure TdxCustomRibbonCustomizationFormHelper.DoMoveNode(ASourceNode,
  ATargetNode: TcxTreeListNode; const AInsertBeforeTargetNode: Boolean;
  const ATargetNodeIsParent: Boolean = False);
var
...
begin
  ...
  else if (ATargetNode = nil) or AInsertBeforeTargetNode then
    ANewNode := RibbonTreeList.Insert(ATargetNode)
  else if ATargetNode.getNextSibling = nil then
...
function TdxCustomRibbonCustomizationFormHelper.IsAboveNodeCenter
  (ANode: TcxTreeListNode; const ACoordinate: Integer): Boolean;
var
  ARealBounds: TRect;
begin
  ARealBounds := GetRealBounds(ANode);
  Result := not cxRectIsInvalid(ARealBounds) and
    (ACoordinate < cxRectCenter(ARealBounds).Y);
end;

function TdxCustomRibbonCustomizationFormHelper.IsCenterNodePart
  (ANode: TcxTreeListNode; const ACoordinate: Integer): Boolean;
var
  ARealBounds: TRect;
begin
  ARealBounds := GetRealBounds(ANode);
  Result := not cxRectIsInvalid(ARealBounds) and
    (ACoordinate >= (ARealBounds.Top + cxRectHeight(ARealBounds) / 3)) and
    (ACoordinate <= (ARealBounds.Top + 2 * cxRectHeight(ARealBounds) / 3));
end;

function TdxCustomRibbonCustomizationFormHelper.IsTopNodePart
  (ANode: TcxTreeListNode; const ACoordinate: Integer): Boolean;
var
  ARealBounds: TRect;
begin
  ARealBounds := GetRealBounds(ANode);
  Result := not cxRectIsInvalid(ARealBounds) and
    (ACoordinate < (ARealBounds.Top + cxRectHeight(ARealBounds) / 3));
end;

function TdxCustomRibbonCustomizationFormHelper.IsDuplicatedIn(ASourceNode,
  ATargetNode: TcxTreeListNode; ACoordinate: Integer): Boolean;
var
  I: Integer;
begin
  ...
  begin
    if ACoordinate = -1 then
      ACoordinate := cxRectCenter(GetRealBounds(ATargetNode)).Y;
    if (GetItemData(ATargetNode) <> nil) and
      not(GetItemData(ATargetNode).IsSubItem and IsCenterNodePart(ATargetNode,
      ACoordinate)) then
...
function TdxCustomRibbonCustomizationFormHelper.IsPartOfItemsStructure
  (AItemNode, AStructureOwnerNode: TcxTreeListNode): Boolean;
var
  AStructurePartNode: TcxTreeListNode;
  I: Integer;
begin
  Result := (GetNodeLevel(AItemNode, True) = dxrcfItemLevel) and
    (GetNodeLevel(AStructureOwnerNode, True) = dxrcfItemLevel) and
    GetItemData(AStructureOwnerNode).IsSubItem and
    (AStructureOwnerNode.Count > 0);
  if Result then
...
function TdxCustomRibbonCustomizationFormHelper.IsRecursiveInclusion
  (ASourceNode, ATargetNode: TcxTreeListNode; ACoordinate: Integer): Boolean;
begin
  if ACoordinate = -1 then
    ACoordinate := cxRectCenter(GetRealBounds(ATargetNode)).Y;
  Result := (GetNodeLevel(ASourceNode, True) = dxrcfItemLevel) and
    (GetNodeLevel(ATargetNode, True) = dxrcfItemLevel) and
    ((GetNodeLevel(ATargetNode) > dxrcfItemLevel) or
    IsCenterNodePart(ATargetNode, ACoordinate));
  if Result then
  begin
    if not GetItemData(ATargetNode).IsSubItem then
      ATargetNode := ATargetNode.Parent;
    repeat
      Result := IsSameElement(ASourceNode, ATargetNode) or
        IsPartOfItemsStructure(ATargetNode, ASourceNode);
      ATargetNode := ATargetNode.Parent;
    until Result or (GetNodeLevel(ATargetNode) < dxrcfItemLevel);
  end;
end;

function TdxCustomRibbonCustomizationFormHelper.IsRibbonContainsItem
  (AItem: TdxBarItem): Boolean;
var
...
function TdxCustomRibbonCustomizationFormHelper.IsSameElement(ASourceNode,
  ATargetNode: TcxTreeListNode): Boolean;
begin
  ...
  case GetNodeLevel(ATargetNode, True) of
    ...
    dxrcfGroupLevel:
      Result := Result and (TdxRibbonTabGroup(ASourceNode.Data)
        .Toolbar = TdxRibbonTabGroup(ATargetNode.Data).Toolbar);
    dxrcfItemLevel:
      Result := Result and
        (GetItemData(ASourceNode).Item = GetItemData(ATargetNode).Item);
  end;
end;

function TdxCustomRibbonCustomizationFormHelper.IsSameElement
  (ANode: TcxTreeListNode; AData: TPersistent): Boolean;
const
  NodeClassMap: array [0 .. 3] of TClass = (TdxRibbonContext, TdxRibbonTab,
    TdxRibbonTabGroup, TdxBarItem);
begin
  Result := (ANode.Data <> nil) and (AData <> nil) and
    AData.InheritsFrom(NodeClassMap[GetNodeLevel(ANode, True)]);
  case GetNodeLevel(ANode, True) of
    dxrcfContextLevel, dxrcfTabLevel:
      Result := TPersistent(ANode.Data) = AData;
    dxrcfGroupLevel:
      Result := Result and
        (TdxRibbonTabGroup(ANode.Data).Toolbar = TdxRibbonTabGroup
        (AData).Toolbar);
    dxrcfItemLevel:
...
function TdxCustomRibbonCustomizationFormHelper.IsSourceNodeValid
  (ASourceNode: TcxTreeListNode): Boolean;
begin
  Result := ASourceNode <> nil;
end;

function TdxCustomRibbonCustomizationFormHelper.IsTargetNodeValid(ASourceNode,
  ATargetNode: TcxTreeListNode): Boolean;
begin
  Result := (ATargetNode <> nil) and ATargetNode.Visible;
end;

procedure TdxCustomRibbonCustomizationFormHelper.CompareNodes
  (Sender: TcxCustomTreeList; ANode1, ANode2: TcxTreeListNode;
...
begin
  ACompare := CompareStr(GetCaptionForComparison(ANode1),
    GetCaptionForComparison(ANode2));
end;

function TdxCustomRibbonCustomizationFormHelper.GetBarIndexInIni(ABar: TdxBar;
  ASource: TCustomIniFile): Integer;
var
  I: Integer;
begin
  Result := -1;
  for I := 0 to ASource.ReadInteger
    (GetBarManagerMainIniSection(ABar.BarManager), 'BarCount', 0) - 1 do
    if ASource.ReadString(GetBarIniSection(ABar.BarManager, I), 'Name', '')
      = ABar.Name then
    begin
      Result := I;
      Break;
  ...
  if Result = -1 then
    for I := 0 to ASource.ReadInteger
      (GetBarManagerMainIniSection(ABar.BarManager), 'BarCount', 0) - 1 do
      if ASource.ReadString(GetBarIniSection(ABar.BarManager, I), 'Caption', '')
        = ABar.Caption then
      begin
        Result := I;
        Break;
...
function TdxCustomRibbonCustomizationFormHelper.GetItemNodeCaption
  (AItem: TdxBarItem; AItemLink: TdxBarItemLink): string;
begin
...
function TdxCustomRibbonCustomizationFormHelper.GetRealBounds
  (ANode: TcxTreeListNode): TRect;
begin
...
begin
  ...
  try
    ...
    ANewContextNode.Texts[0] := cxGetResourceString
...
begin
  LockContentUpdating;
  try
    AddNewNodeToFocusedNode(dxrcfGroupLevel,
      cxGetResourceString(@sdxRibbonCustomizationFormNewGroup));
  finally
    UnlockContentUpdating;
  end;
end;
...
begin
  LockContentUpdating;
  try
    ANewTabNode := AddNewNodeToFocusedNode(dxrcfTabLevel,
      cxGetResourceString(@sdxRibbonCustomizationFormNewTab));
    AddNewGroup;
...
function TdxRibbonCustomizationFormHelper.GetNodeLevel(ANode: TcxTreeListNode;
  const AAsTyped: Boolean = False): Integer;
begin
  if ANode = nil then
    Result := -1
  else if ANode.TreeList = RibbonTreeList then
    Result := ANode.Level
  else
...
begin
  Result := CommandsComboBox.ItemIndex in [dxrcfCommandsAllTabs,
    dxrcfCommandsMainTabs, dxrcfCommandsToolTabs];
end;
...
begin
  ...
  else if ATargetNode.Count > 0 then
...
begin
  ...
  if GetNodeLevel(ATargetNode) = GetNodeLevel(ASourceNode) then
    DoMoveNode(ASourceNode, ATargetNode,
      ATargetNode = ASourceNode.GetPrevSiblingVisible)
  else if ATargetNode.Count > 0 then
...
begin
  ...
  try
    ...
    CommandsTreeList.OptionsBehavior.IncSearch := CommandsComboBox.ItemIndex
      in [dxrcfCommandsCommandsNotInTheRibbon, dxrcfCommandsAllCommands];
    case CommandsComboBox.ItemIndex of
...
procedure TdxRibbonCustomizationFormHelper.PopulateRibbonTreeListContent
...
procedure TdxRibbonCustomizationFormHelper.RenameNode(ANode: TcxTreeListNode;
...
begin
  ...
  begin
    RibbonTreeList.Root.Items[0].Visible := RibbonComboBox.ItemIndex
      in [dxrcfRibbonAllTabs, dxrcfRibbonMainTabs];
    for I := 1 to RibbonTreeList.Root.Count - 1 do
      RibbonTreeList.Root.Items[I].Visible := RibbonComboBox.ItemIndex
        in [dxrcfRibbonAllTabs, dxrcfRibbonToolTabs];
...
begin
  Result := inherited CanMoveFocusedNodeDown and
    ((RibbonComboBox.ItemIndex <> dxrcfRibbonMainTabs) or
    (GetContextNode(GetTargetNodeForMovingFocusedNodeDown)
    = RibbonTreeList.Root.Items[0]));
end;

function TdxRibbonCustomizationFormHelper.CanMoveFocusedNodeUp: Boolean;
begin
  Result := inherited CanMoveFocusedNodeUp and
    ((RibbonComboBox.ItemIndex <> dxrcfRibbonToolTabs) or
    (GetContextNode(GetTargetNodeForMovingFocusedNodeUp) <>
    RibbonTreeList.Root.Items[0]));
end;

function TdxRibbonCustomizationFormHelper.CanMoveNodeTo(ASourceNode,
  ATargetNode: TcxTreeListNode; const ACoordinate: Integer = -1): Boolean;
begin
  Result := IsSourceNodeValid(ASourceNode) and IsTargetNodeValid(ASourceNode,
    ATargetNode) and (not IsRibbonGroupNodeWithSharedToolBar(ASourceNode) or
    IsParentNodeTheSame(ASourceNode, ATargetNode)) and
...
begin
  ...
  if Result then
    case GetNodeLevel(FocusedNode, True) of
      ...
      dxrcfTabLevel:
        Result := (FocusedNode.Data = nil) or not TdxRibbonTab(FocusedNode.Data)
          .IsPredefined;
      dxrcfGroupLevel:
        Result := (FocusedNode.Data = nil) or
          not TdxRibbonTabGroup(FocusedNode.Data).IsToolBarShared;
      dxrcfItemLevel:
...
begin
  AFocusedNode := FocusedNode;
  Result := (AFocusedNode <> nil) and
    (GetNodeLevel(AFocusedNode) > dxrcfContextLevel);
  if Result then
  begin
    while GetNodeLevel(AFocusedNode) > dxrcfTabLevel do
      AFocusedNode := AFocusedNode.Parent;
    Result := (AFocusedNode.Data <> nil) and TdxRibbonTab(AFocusedNode.Data)
      .IsPredefined;
  end;
end;

function TdxRibbonCustomizationFormHelper.DrawContextNode
  (Sender: TcxCustomTreeList; ACanvas: TcxCanvas;
  AViewInfo: TcxTreeListEditCellViewInfo;
  ALookAndFeelPainter: TcxCustomLookAndFeelPainter): Boolean;
var
...
begin
  ...
  ATextRect := cxRectCenterVertically(AViewInfo.BoundsRect,
    TdxTextMeasurer.TextLineHeight(ACanvas.Handle));
...
begin
  ...
  Result.Checked := (GetNodeLevel(Result) <> dxrcfGroupLevel) or
    AParentNode.Checked;
  SetFocusedNode(Result);
end;

procedure TdxRibbonCustomizationFormHelper.AfterNodeMoved(ASourceNode,
  ANewNode: TcxTreeListNode);
var
  AConvertNodeData: Boolean;
begin
  SetFocusedNode(ANewNode);
  AConvertNodeData := (ASourceNode.TreeList = CommandsTreeList) and
    not IsGroupHiddenInTab(ASourceNode, ANewNode.Parent);
...
procedure TdxRibbonCustomizationFormHelper.ConvertGroupNodeToNew
  (ANode: TcxTreeListNode);
var
  I: Integer;
begin
  ...
  ANode.Data := nil;
  ANode.Texts[0] := ANode.Texts[0] + cxGetResourceString
    (@sdxRibbonCustomizationFormCustomElementSuffix);
  ANode.Enabled := True;
end;

procedure TdxRibbonCustomizationFormHelper.ConvertTabNodeToNew
  (ANode: TcxTreeListNode);
var
  I: Integer;
begin
...
function TdxRibbonCustomizationFormHelper.CreateContextNode
  (AContext: TdxRibbonContext; ATargetTreeList: TcxTreeList;
  AIsReseting: Boolean = False): TcxTreeListNode;
var
  ATab: TdxRibbonTab;
  I: Integer;
begin
  ...
  begin
    ATab := Ribbon.Tabs[I];
    if (ATab.Context = AContext) and ((ATargetTreeList = RibbonTreeList) and
      not AIsReseting or ATab.IsPredefined) then
...
function TdxRibbonCustomizationFormHelper.CreateTabNode(ATab: TdxRibbonTab;
  AContextNode: TcxTreeListNode; ATargetTreeList: TcxTreeList;
  AIsReseting: Boolean = False): TcxTreeListNode;
begin
...
function TdxRibbonCustomizationFormHelper.GetContextNode
  (ASourceNode: TcxTreeListNode): TcxTreeListNode;
begin
  Result := ASourceNode;
...
function TdxRibbonCustomizationFormHelper.GetItemNodeVisible
  (AItem: TdxBarItem): Boolean;
begin
  Result := inherited GetItemNodeVisible(AItem) and
    not(AItem is TdxRibbonQuickAccessGroupButton);
end;

function TdxRibbonCustomizationFormHelper.GetTargetNodeForMovingFocusedNodeDown
  : TcxTreeListNode;
var
  ALevel: Integer;
begin
  ...
  begin
    ...
    repeat
      ...
      while (Result <> nil) and (GetNodeLevel(Result) <> ALevel) and
        (GetNodeLevel(Result) <> (ALevel - 1)) do
        Result := Result.GetNext;
    until (Result = nil) or
      (CanMoveNodeTo(FocusedNode, Result, GetRealBounds(Result).Bottom) and
      ((GetNodeLevel(FocusedNode) <= dxrcfItemLevel) or
      IsParentNodeTheSame(FocusedNode, Result)));
  end;
end;

function TdxRibbonCustomizationFormHelper.GetTargetNodeForMovingFocusedNodeUp
  : TcxTreeListNode;
var
  ALevel: Integer;
begin
  ...
  begin
    ...
    repeat
      ...
      while (Result <> nil) and (GetNodeLevel(Result) <> ALevel) and
        ((GetNodeLevel(Result) <> (ALevel - 1)) or
        (Result = FocusedNode.Parent)) do
        Result := Result.GetPrev;
    until (Result = nil) or
      (CanMoveNodeTo(FocusedNode, Result, GetRealBounds(Result).Top) and
      ((GetNodeLevel(FocusedNode) <= dxrcfItemLevel) or
      IsParentNodeTheSame(FocusedNode, Result)));
  end;
end;

function TdxRibbonCustomizationFormHelper.LoadExistingGroupNodeContentFormIni
  (AToolBar: TdxBar; AParentTabNode: TcxTreeListNode;
...
begin
  ...
  Result := ATargetTreeList.AddChild(AParentTabNode,
    AGroups.FindByToolBar(AToolBar));
  Result.Enabled := IsGroupCustomizingAllowed(Result);
  LoadItemLinksContentFormIni(AToolBar.ItemLinks, Result,
    ATargetTreeList, ASource);
end;

function TdxRibbonCustomizationFormHelper.LoadMissingGroupNodeContentFormIni
  (AToolBar: TdxBar; AParentTabNode: TcxTreeListNode;
  ATargetTreeList: TcxTreeList; ASource: TCustomIniFile): TcxTreeListNode;
var
  AGroups: TdxRibbonTabGroups;
...
procedure TdxRibbonCustomizationFormHelper.LoadTabNodeContentFormIni
  (ANode: TcxTreeListNode; ATargetTreeList: TcxTreeList;
  ASource: TCustomIniFile);
var
  ATab: TdxRibbonTab;
  AToolBar: TdxBar;
...
begin
  ...
  ATabSection := TdxRibbonTabCollectionAccess(Ribbon.Tabs)
    .GetIniSection(ATab.Name, ASource);
  ...
  begin
    AGroupSection := TdxRibbonTabGroupsAccess(ATab.Groups)
      .GetIniSection(I, ASource);
    ...
    if (AToolBar <> nil) and ((ATargetTreeList = CommandsTreeList) or
      AToolBarVisible) then
    begin
      if ATab.Groups.FindByToolBar(AToolBar) <> nil then
        AGroupNode := LoadExistingGroupNodeContentFormIni(AToolBar, ANode,
          ATargetTreeList, ASource)
      else
        AGroupNode := LoadMissingGroupNodeContentFormIni(AToolBar, ANode,
          ATargetTreeList, ASource);
      AGroupNode.Texts[0] := ASource.ReadString(AGroupSection, 'Caption',
        AToolBar.Caption);
    end;
  end;
...
procedure TdxRibbonCustomizationFormHelper.LoadTabsOrderFormIni
  (AContextNode: TcxTreeListNode; ASource: TCustomIniFile);
  ...
  function GetTabNode(const ATabName: string; AOwnerNode: TcxTreeListNode)
    : TcxTreeListNode;
  var
    I: Integer;
  begin
...
begin
  ...
  begin
    ...
    if ASource.ReadString(ATabSection, 'ContextCaption', '') = AContextCaption
    then
    begin
      ATabNode := GetTabNode(ASource.ReadString(ATabSection, 'Name', ''),
        AContextNode);
...
begin
  ...
  try
    AItems.Clear;
    AItems.Add(cxGetResourceString
      (@sdxRibbonCustomizationFormCommandsNotInTheRibbon));
    ...
    AItems.Add(cxGetResourceString
      (@sdxRibbonCustomizationFormCustomTabsAndGroups));
    CommandsComboBox.ItemIndex := dxrcfCommandsAllTabs;
  finally
    AItems.EndUpdate;
    UnlockContentUpdating;
...
begin
  ...
  ACustomGroupsNode.Texts[0] := cxGetResourceString
    (@sdxRibbonCustomizationFormCustomGroups);
  for I := 0 to Ribbon.TabCount - 1 do
  begin
    ATab := Ribbon.Tabs[I];
    ...
    begin
      ...
      if (AGroup.Toolbar <> nil) and not AGroup.Toolbar.IsPredefined then
        PopulateGroupNodeContent(CommandsTreeList.AddChild(ACustomGroupsNode,
          AGroup), CommandsTreeList);
    end;
  end;
  ACustomGroupsNode.MoveTo(CommandsTreeList.Root.Items
    [CommandsTreeList.Root.Count - 1], tlamAdd);
  ACustomGroupsNode.Expand(False);
end;

procedure TdxRibbonCustomizationFormHelper.PopulateGroupNodeContent
  (ANode: TcxTreeListNode; ATargetTreeList: TcxTreeList);
var
  AItemLinks: TdxBarItemLinks;
  AGroup: TdxRibbonTabGroup;
...
begin
  ...
  for I := 0 to AItemLinks.Count - 1 do
    PopulateItemNodeContent(ATargetTreeList.AddChild(ANode), AItemLinks[I].Item,
      AItemLinks[I]);
end;
...
procedure TdxRibbonCustomizationFormHelper.PopulateTabNodeContent
  (ANode: TcxTreeListNode; ATargetTreeList: TcxTreeList);
var
  ATab: TdxRibbonTab;
  AGroup: TdxRibbonTabGroup;
...
begin
  ...
  begin
    ...
    if (AGroup.Toolbar = nil) or ((ATargetTreeList = RibbonTreeList) and
      ...
      Continue;
    PopulateGroupNodeContent(ATargetTreeList.AddChild(ANode, AGroup),
      ATargetTreeList);
  end;
  ANode.Checked := ATab.Visible;
end;

procedure TdxRibbonCustomizationFormHelper.RestoreTabNodeExpanding
  (ANode: TcxTreeListNode; AStorage: TList);
var
  AGroupNode: TcxTreeListNode;
  I: Integer;
begin
...
procedure TdxRibbonCustomizationFormHelper.StoreTabNodeExpanding
  (ANode: TcxTreeListNode; AStorage: TList);
var
  AGroupNode: TcxTreeListNode;
  I: Integer;
...
procedure TdxRibbonCustomizationFormHelper.DoDropMovedNodeTo
  (ATargetNode: TcxTreeListNode; ACoordinate: Integer);
begin
...
procedure TdxRibbonCustomizationFormHelper.CheckAndAddContextNodeToRibbon
  (ANode: TcxTreeListNode);
var
...
procedure TdxRibbonCustomizationFormHelper.CheckAndAddGroupNodeToRibbon
  (ANode: TcxTreeListNode);
var
  AGroup: TdxRibbonTabGroup;
  ATab: TdxRibbonTab;
begin
  ...
  else if ATab.Groups.IndexOf(AGroup) < 0 then
    TdxRibbonTabGroupAccess(AGroup).MoveToTab(ATab);
  AGroup.Toolbar.Visible := True;
end;

procedure TdxRibbonCustomizationFormHelper.CheckAndAddItemNodeToRibbon
  (ANode: TcxTreeListNode);
var
  AItemLinks: TdxBarItemLinks;
begin
...
procedure TdxRibbonCustomizationFormHelper.CheckAndAddTabNodeToRibbon
  (ANode: TcxTreeListNode);
var
  AContext: TdxRibbonContext;
  ATab: TdxRibbonTab;
...
procedure TdxRibbonCustomizationFormHelper.CheckAndDeleteGroupsFromRibbon
  (AGroups: TdxRibbonTabGroups; ATabNode: TcxTreeListNode);
var
  AGroupNode: TcxTreeListNode;
  AGroup: TdxRibbonTabGroup;
  I: Integer;
begin
  ...
  try
    ...
    begin
      ...
      else if AGroup.Toolbar.IsPredefined then
...
procedure TdxRibbonCustomizationFormHelper.CheckAndDeleteTabsFromRibbon
  (AContext: TdxRibbonContext);
var
  ATabs: TdxRibbonTabCollection;
...
function TdxRibbonCustomizationFormHelper.IsGroupCustomizingAllowed
  (ANode: TcxTreeListNode): Boolean;
begin
  Result := (ANode <> nil) and
    ((ANode.Data = nil) or (TdxRibbonTabGroup(ANode.Data).Toolbar = nil) or
    TdxRibbonTabGroup(ANode.Data).Toolbar.AllowCustomizing);
end;

function TdxRibbonCustomizationFormHelper.IsGroupHiddenInTab(AGroupNode,
  ATabNode: TcxTreeListNode): Boolean;
var
  AGroup: TdxRibbonTabGroup;
  I: Integer;
begin
  Result := (GetNodeLevel(AGroupNode) = dxrcfGroupLevel) and
    (GetNodeLevel(ATabNode) = dxrcfTabLevel) and (AGroupNode.Data <> nil) and
    (ATabNode.Data <> nil);
  if Result then
  begin
    AGroup := TdxRibbonTabGroup(AGroupNode.Data);
    Result := (AGroup.Toolbar <> nil) and TdxRibbonTab(ATabNode.Data)
      .Groups.ContainsToolBar(AGroup.Toolbar);
    if Result then
...
function TdxRibbonCustomizationFormHelper.IsParentNodeTheSame(ASourceNode,
  ATargetNode: TcxTreeListNode): Boolean;
begin
...
function TdxRibbonCustomizationFormHelper.IsRibbonGroupNodeWithSharedToolBar
  (ANode: TcxTreeListNode): Boolean;
begin
  Result := (ANode.TreeList = RibbonTreeList) and
    (GetNodeLevel(ANode) = dxrcfGroupLevel) and (ANode.Data <> nil) and
    TdxRibbonTabGroup(ANode.Data).IsToolBarShared;
end;

function TdxRibbonCustomizationFormHelper.IsSourceNodeValid
  (ASourceNode: TcxTreeListNode): Boolean;
begin
  Result := inherited IsSourceNodeValid(ASourceNode) and
    (GetNodeLevel(ASourceNode) <> dxrcfContextLevel) and
    ((GetNodeLevel(ASourceNode) < dxrcfItemLevel) or
    (ASourceNode.TreeList <> RibbonTreeList) or ASourceNode.Enabled) and
    not((ASourceNode.TreeList = CommandsTreeList) and
    (CommandsComboBox.ItemIndex = dxrcfCommandsCustomTabsAndGroups) and
    (ASourceNode.Data = nil));
end;

function TdxRibbonCustomizationFormHelper.IsTargetNodeValid(ASourceNode,
  ATargetNode: TcxTreeListNode): Boolean;
begin
...
function TdxRibbonCustomizationFormHelper.GetContextCaption
  (AContext: TdxRibbonContext): string;
begin
...
begin
  DoDeleteMissingItemLinks(Ribbon.QuickAccessToolbar.Toolbar.ItemLinks,
    RibbonTreeList.Root);
end;
...
procedure TdxRibbonQATCustomizationFormHelper.ChangeQATPosition
  (const AShowBelowRibbon: Boolean);
const
  QATPositionMap: array [Boolean] of TdxQuickAccessToolbarPosition =
    (qtpAboveRibbon, qtpBelowRibbon);
begin
  Ribbon.QuickAccessToolbar.Position := QATPositionMap[AShowBelowRibbon];
end;

function TdxRibbonQATCustomizationFormHelper.GetNodeLevel
  (ANode: TcxTreeListNode; const AAsTyped: Boolean = False): Integer;
begin
...
procedure TdxRibbonQATCustomizationFormHelper.InitializeQATPositionIndicator
  (AIndicator: TObject);
begin
  LockContentUpdating;
  try
    if AIndicator is TcxCheckBox then
      TcxCheckBox(AIndicator).Checked :=
...
procedure TdxRibbonQATCustomizationFormHelper.PopulateRibbonTreeListContent
  (const AIsReseting: Boolean = False);
var
  AItemLinks: TdxBarItemLinks;
  I: Integer;
begin
  ...
  try
    ...
    if AIsReseting then
      LoadItemLinksContentFormIni(Ribbon.QuickAccessToolbar.Toolbar.ItemLinks,
        RibbonTreeList.Root, RibbonTreeList, FSavedState)
    else
    begin
      AItemLinks := Ribbon.QuickAccessToolbar.Toolbar.ItemLinks;
      for I := 0 to AItemLinks.Count - 1 do
        PopulateItemNodeContent(RibbonTreeList.Add, AItemLinks[I].Item,
          AItemLinks[I], AItemLinks[I].BeginGroup);
    end;
  finally
    RibbonTreeList.EndUpdate;
    UnlockContentUpdating;
...
procedure TdxRibbonQATCustomizationFormHelper.RemoveNode
  (ANode: TcxTreeListNode);
begin
  ANode.Delete;
end;

function TdxRibbonQATCustomizationFormHelper.CanMoveNodeTo(ASourceNode,
  ATargetNode: TcxTreeListNode; const ACoordinate: Integer = -1): Boolean;
begin
  Result := IsSourceNodeValid(ASourceNode) and
    ((ATargetNode = nil) or IsTargetNodeValid(ASourceNode, ATargetNode)) and
    not IsDuplicatedIn(ASourceNode, ATargetNode, ACoordinate) and
    not IsRecursiveInclusion(ASourceNode, ATargetNode, ACoordinate);
end;
...
procedure TdxRibbonQATCustomizationFormHelper.AfterNodeMoved(ASourceNode,
  ANewNode: TcxTreeListNode);
begin
  SetFocusedNode(ANewNode);
...
function TdxRibbonQATCustomizationFormHelper.
  GetTargetNodeForMovingFocusedNodeDown: TcxTreeListNode;
begin
  Result := FocusedNode.getNextSibling;
end;

function TdxRibbonQATCustomizationFormHelper.GetTargetNodeForMovingFocusedNodeUp
  : TcxTreeListNode;
begin
  Result := FocusedNode.getPrevSibling;
end;

procedure TdxRibbonQATCustomizationFormHelper.PopulateBeginGroupNodeContent
  (ATargetNode: TcxTreeListNode);
begin
  ATargetNode.Data := TdxRibbonCustomizationFormItemNodeData.Create(nil, nil);
  ATargetNode.Texts[0] := cxGetResourceString
    (@sdxRibbonCustomizationFormBeginGroup);
  ATargetNode.Enabled := ATargetNode.Parent.Enabled;
end;

procedure TdxRibbonQATCustomizationFormHelper.PopulateContextTabs
  (AContext: TdxRibbonContext; AItems: TStrings);
var
...
begin
  ...
  for I := 0 to Ribbon.TabCount - 1 do
    if Ribbon.Tabs[I].Context = AContext then
      AItems.AddObject(AContextPrefix + Ribbon.Tabs[I].Caption + ATabSuffix,
        Ribbon.Tabs[I]);
end;

procedure TdxRibbonQATCustomizationFormHelper.PopulateItemNodeContent
  (ATargetNode: TcxTreeListNode; AItem: TdxBarItem; AItemLink: TdxBarItemLink;
  const ANeedBeginGroupNode: Boolean = False; ASource: TCustomIniFile = nil);
begin
  inherited PopulateItemNodeContent(ATargetNode, AItem, AItemLink,
    ANeedBeginGroupNode, ASource);
...
procedure TdxRibbonQATCustomizationFormHelper.PopulateTabItems
  (ATab: TdxRibbonTab);
var
  AToolBar: TdxBar;
  I, J: Integer;
  ...
  begin
    ...
    if AToolBar <> nil then
      for J := 0 to AToolBar.ItemLinks.Count - 1 do
        PopulateItemNodeContent(CommandsTreeList.Add,
          AToolBar.ItemLinks[J].Item, AToolBar.ItemLinks[J]);
  end;
end;

procedure TdxRibbonQATCustomizationFormHelper.DoReorderItemLinks
  (AOwnerNode: TcxTreeListNode);
var
  ANodeData: TdxRibbonCustomizationFormItemNodeData;
...
begin
  ...
  begin
    ...
    begin
      ANodeData.ItemLink.BeginGroup := (I > 0) and
        GetItemData(AOwnerNode.Items[I - 1]).IsBeginGroup;
      ANodeData.ItemLink.Index := I - ABeginGroupCount;
    end;
  end;
end;

function TdxRibbonQATCustomizationFormHelper.IsDuplicatedIn(ASourceNode,
  ATargetNode: TcxTreeListNode; ACoordinate: Integer): Boolean;
begin
  if ATargetNode = nil then
    ATargetNode := RibbonTreeList.Root;
  Result := not GetItemData(ASourceNode).IsBeginGroup and
    inherited IsDuplicatedIn(ASourceNode, ATargetNode, ACoordinate);
end;

function TdxRibbonQATCustomizationFormHelper.IsSourceNodeValid
  (ASourceNode: TcxTreeListNode): Boolean;
begin
  Result := inherited IsSourceNodeValid(ASourceNode) and
    ((ASourceNode.TreeList <> RibbonTreeList) or ASourceNode.Enabled);
end;

function TdxRibbonQATCustomizationFormHelper.IsTargetNodeValid(ASourceNode,
  ATargetNode: TcxTreeListNode): Boolean;
begin
  Result := inherited IsTargetNodeValid(ASourceNode, ATargetNode) and
    ATargetNode.Enabled;
...
function TdxRibbonQATCustomizationFormHelper.GetContextPrefix
  (AContext: TdxRibbonContext): string;
begin
  if AContext <> nil then
    Result := AContext.Caption + cxGetResourceString
