...
type
  ...
  public
    ...
    property TemplateString: string read GetTemplateString
      write SetTemplateString;
  end;
  ...
  TdxHFFunctionEnumProc = procedure(Sender: TdxHFFunctionLibrary;
    const AHFFunction: TdxHFCustomFunction) of object;
  ...
  private
    ...
    function GetFunctionByClass(FunctionClass: TdxHFCustomFunctionClass)
      : TdxHFCustomFunction;
    procedure SetFunction(Index: Integer; Value: TdxHFCustomFunction);
    procedure SetFunctionByClass(FunctionClass: TdxHFCustomFunctionClass;
      Value: TdxHFCustomFunction);
  public
    constructor Create; virtual;
    destructor Destroy; override;
    ...
    procedure GetFunctionsByCategory(ACategory
      : TdxHFFunctionCustomCategoryClass; AStrings: TStrings);
    ...
    property Funcs[Index: Integer]: TdxHFCustomFunction read GetFunction
      write SetFunction; default;
    property FuncsByClass[FunctionClass: TdxHFCustomFunctionClass]
      : TdxHFCustomFunction read GetFunctionByClass write SetFunctionByClass;
  end;
  ...
  public
    ...
    procedure PopulateCells(AParentCell: TdxReportCell; AHost: TdxReportCells;
      const Source: string; ATextAlignX: TcxTextAlignX;
      ATextAlignY: TcxTextAlignY; APageObject: TCustomdxPageObject;
      AFormatObject: TdxHFFunctionFormatObject); overload; virtual;
  end;
...
procedure dxGetHFFunctionsListByCategory(ACategory
  : TdxHFFunctionCustomCategoryClass; AStrings: TStrings);
procedure dxPSSplitAutoHFTextEntry(Source: string;
  var APart1, APart2, APart3: string);
...
type
  ...
  public
    ...
    property ImageCollectionItem: TcxImageCollectionItem
      read GetImageCollectionItem;
    property ImageIndex: Integer read FImageIndex write FImageIndex;
    property ImageSize: TSize read GetImageSize;
  end;

function dxFormatPageNumber(ANumber: Integer;
  AFormat: TdxPageNumberFormat): string;
begin
  case AFormat of
...
procedure TdxHFReportCellBuilder.AlignDataItems(ACell: TdxReportCell;
  AAlignment: TcxTextAlignY);
var
...
const
  ...
  TextAlignMap: array [TdxPageTitlePart] of TcxTextAlignX = (taLeft,
    taCenterX, taRight);
var
  ACell: TdxReportCell;
...
procedure TdxHFReportCellBuilder.CalculateTitlePartSize(ACell: TdxReportCell;
  ACanvas: TdxPSReportRenderCustomCanvas);
var
  ADataItem: TAbstractdxReportCellData;
...
procedure TdxHFReportCellBuilder.PlaceTitleParts
  ...
  function CalculateTitlePartPosition(const R: TRect; AWidth: Integer;
    ATitlePart: TdxPageTitlePart): TRect;
  begin
    case ATitlePart of
      tpRight:
        Result := cxRect(Max(R.Right - AWidth, R.Left), R.Top, R.Right,
          R.Bottom);
  ...
  function PlaceTitlePart(const R: TRect; ACell: TdxReportCell;
    ATitlePart: TdxPageTitlePart): TRect;
  begin
...
begin
  ...
  PlaceTitlePart(cxRect(R.Left, R.Top, R1.Left, R.Bottom),
    AHostCell[0], tpLeft);
  PlaceTitlePart(cxRect(R1.Right, R.Top, R.Right, R.Bottom),
    AHostCell[2], tpRight);
end;

procedure TdxHFReportCellBuilder.PopulateCells(AParentCell: TdxReportCell;
  AHost: TdxReportCells; ATextAlignX: TcxTextAlignX;
  ATitlePart: TdxPageTitlePart; APageObject: TCustomdxPageObject);
begin
  PopulateCells(AParentCell, AHost, APageObject.Titles[ATitlePart].Text,
    ATextAlignX, APageObject.TextAlignY[ATitlePart], APageObject,
    dxHFFormatObject);
end;
...
procedure TdxHFReportCellBuilder.PopulateCells(AParentCell: TdxReportCell;
  ...
  begin
    ...
    begin
      ATextCell := TdxReportCellTitlePartText
        (AParentCell.AddDataItem(TdxReportCellTitlePartText));
  ...
  begin
    ...
    begin
      AImageCell := TdxReportCellTitlePartImage
        (AParentCell.AddDataItem(TdxReportCellTitlePartImage));
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          AddTextCell(Copy(AExpander.Str, ATextOffset,
            AFuncInfo.Left - ATextOffset));
          AddImageCell(AFuncInfo.Index);
          ATextOffset := AFuncInfo.Right + 1;
        end
        else
          AExpander.Replace(AFuncInfo, AFunction.DoProcess(AFuncInfo.Name,
            AFormatObject));
      end;
    end;
    AddTextCell(Copy(AExpander.Str, ATextOffset, AExpander.StrLength -
      ATextOffset + 1));
  finally
    AExpander.Free;
  end;
...
class function TdxHFCustomFunction.GetCategory
...
class function TdxHFPagesFunctions.GetCategory
  : TdxHFFunctionCustomCategoryClass;
begin
...
function TdxHFPageNumberFunction.ConvertFunc(const Source: string;
  const AFormatObject: TdxHFFunctionFormatObject): string;
begin
  Result := dxFormatPageNumber(AFormatObject.StartPageIndex +
    AFormatObject.CurrentPage - 1, AFormatObject.PageNumberFormat);
end;
...
begin
  Result := dxFormatPageNumber(AFormatObject.StartPageIndex +
    AFormatObject.TotalPages - 1, AFormatObject.PageNumberFormat);
end;
...
function TdxHFPageOfPagesFunction.ConvertFunc(const Source: string;
  const AFormatObject: TdxHFFunctionFormatObject): string;
begin
  Result := dxFormatPageNumber(AFormatObject.CurrentPage +
    AFormatObject.StartPageIndex - 1, AFormatObject.PageNumberFormat) + ' ' +
    cxGetResourceString(@sdxOf) + ' ' + dxFormatPageNumber
...
class function TdxHFAuthenticationFunctions.GetCategory
  : TdxHFFunctionCustomCategoryClass;
begin
...
class function TdxHFDateTimeFunctions.GetCategory
  : TdxHFFunctionCustomCategoryClass;
begin
...
function TdxHFFunctionLibrary.Add(AFunctionClass: TdxHFCustomFunctionClass)
  : TdxHFCustomFunction;
begin
...
function TdxHFFunctionLibrary.IndexOfByName(const AFunctionName
  : string): Integer;
begin
  for Result := 0 to Count - 1 do
  begin
...
function TdxHFFunctionLibrary.IndexOfByClass(AFunctionClass
  : TdxHFCustomFunctionClass): Integer;
begin
  for Result := 0 to Count - 1 do
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      if AIndex >= 0 then
        AExpander.Replace(AFuncInfo, Funcs[AIndex].DoProcess(AFuncInfo.Name,
          AFormatObject));
    end;
...
function TdxHFFunctionLibrary.GetFunctionByClass(FunctionClass
  : TdxHFCustomFunctionClass): TdxHFCustomFunction;
var
  AIndex: Integer;
begin
...
procedure TdxHFFunctionLibrary.SetFunction(Index: Integer;
  Value: TdxHFCustomFunction);
begin
  Funcs[Index].Assign(Value);
end;

procedure TdxHFFunctionLibrary.SetFunctionByClass(FunctionClass
  : TdxHFCustomFunctionClass; Value: TdxHFCustomFunction);
var
  AIndex: Integer;
begin
  AIndex := IndexOfByClass(FunctionClass);
...
procedure TdxHFTemplateExpander.Replace(const AInfo: TdxHFFunctionPlaceInfo;
  const AValue: string; AWrapBySpaces: Boolean);
begin
  if AWrapBySpaces and (AValue <> '') then
    Replace(AInfo, ' ' + AValue + ' ', False)
  else
  begin
    FStr := Copy(FStr, 1, AInfo.Left - 1) + AValue +
      Copy(FStr, AInfo.Right + 1, MaxInt);
...
function TdxHFTemplateExpander.GetFuncInfo
  (out AInfo: TdxHFFunctionPlaceInfo): Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        AInfo.Index := StrToIntDef(Copy(AInfo.Name, AIndexPos + 1,
          AIndexLength), 0);
...
function TdxReportCellTitlePartImage.GetImageCollectionItem
  : TcxImageCollectionItem;
var
  ACollection: TcxImageCollection;
begin
  ACollection := ReportLink.RealPrinterPage.ImageCollection;
  if Assigned(ACollection) and (ImageIndex >= 0) and
    (ImageIndex < ACollection.Items.Count) then
