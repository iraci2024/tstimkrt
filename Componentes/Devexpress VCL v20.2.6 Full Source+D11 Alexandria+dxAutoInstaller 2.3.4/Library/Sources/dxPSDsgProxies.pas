...
uses
  DesignIntf, DesignEditors, ComponentDesigner, Classes, ImgList, Controls,
  ComCtrls, dxBase, dxPSSngltn;
...
procedure RestoreDesignSelection(const ADesigner: TFormDesigner;
  var AList: TdxDesignSelectionList);
procedure SaveDesignSelection(const ADesigner: TFormDesigner;
  out AList: TdxDesignSelectionList);
...
type
  ...
  private
    ...
    function GetImageIndexByComponentClassName(const ClassName: string)
      : Integer;
  ...
  protected
    ...
    function ImageIndexByComponentClass(AComponentClass
      : TComponentClass): Integer;
  ...
  public
    ...
    property ImageIndexes[ComponentClass: TComponentClass]: Integer
      read GetImageIndex; default;
    property ImageIndexesByComponentClassName[const ClassName: string]: Integer
      read GetImageIndexByComponentClassName;
    property ImageIndexesByObject[AnObject: TPersistent]: Integer
      read GetImageIndexByObject;
...
uses
  ToolsAPI, PaletteAPI, Forms, Windows, Graphics, StdCtrls, SysUtils, dxPSCore,
  dxPSUtl, cxGraphics;
...
constructor TdxIdeComponentImageItem.Create(AImageIndex: Integer;
...
begin
  inherited;
  FImages := TImageList.CreateSize(CompIconSize + 1, CompIconSize + 1);
  // plus one for frame
  FItems := TList.Create;
end;

procedure TdxIdeImagesProvider.Add(AImageIndex: Integer;
  AComponentClass: TComponentClass);
begin
...
function TdxIdeImagesProvider.ImageIndexByComponentClass(AComponentClass
  : TComponentClass): Integer;
var
  I: Integer;
...
{ TPaintItemStyle = set of (piDown, piSelected, piFramed, piGosted); }
  ...
  function GetComponentImage(APaletteItem: IdxIDEPaletteItem;
    ABitmap: TcxBitmap): Boolean;
  var
    R: TRect;
  ...
  function AddComponentToImages(ABitmap: TcxBitmap;
    AComponentClass: TComponentClass): Integer;
  var
    PaletteItem: IdxIDEPaletteItem;
  begin
    Result := -1;
    if IdeEnvironment = nil then
      Exit;
...
function TdxIdeImagesProvider.GetImageIndex(ComponentClass
  : TComponentClass): Integer;
begin
  Result := ImageIndexByComponentClass(ComponentClass);
end;

function TdxIdeImagesProvider.GetImageIndexByComponentClassName(const ClassName
  : string): Integer;
var
  AClass: TClass;
begin
...
function TdxIdeImagesProvider.GetImageIndexByObject
  (AnObject: TPersistent): Integer;
begin
