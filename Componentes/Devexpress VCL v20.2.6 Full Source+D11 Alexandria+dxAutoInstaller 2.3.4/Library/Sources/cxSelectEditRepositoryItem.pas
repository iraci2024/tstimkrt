...
uses
  Types, Windows, SysUtils, Classes, Controls, Forms, StdCtrls, cxClasses,
  cxEdit,
  ExtCtrls;
...
function GetEditRepositoryItemClass(var ASetupData
  : TcxSelectRepositoryItemSetup): TcxEditRepositoryItemClass;
  ...
  try
    ...
    if ASelectRepositoryItem.ShowModal = mrOk then
      Result := TcxEditRepositoryItemClass
        (ASelectRepositoryItem.lbItems.Items.Objects
        [ASelectRepositoryItem.lbItems.ItemIndex]);
...
begin
  if (ssDouble in Shift) and (mbLeft = Button) and
    (lbItems.ItemAtPos(Point(X, Y), True) <> -1) then
    ModalResult := mrOk;
end;

procedure TcxSelectRepositoryItem.Setup(const AData
  : TcxSelectRepositoryItemSetup);
var
  I: Integer;
begin
  with AData do
...
procedure TcxSelectRepositoryItem.SaveSetup
  (var AData: TcxSelectRepositoryItemSetup);
begin
  with AData do
  begin
    Pos := Point(Left, Top);
