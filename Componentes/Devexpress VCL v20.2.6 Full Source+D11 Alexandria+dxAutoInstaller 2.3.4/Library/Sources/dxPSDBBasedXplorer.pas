...
type
  ...
  public
    ...
    property Folders[Index: Integer]: TdxPSDBBasedExplorerFolder
  ...
  TdxPSDBBasedExplorerFieldNamesMapClass = class of
  ...
  TdxPSDBBasedExplorerLoadErrorEvent = procedure(Sender: TdxPSDBBasedExplorer;
  ...
  private
    ...
    procedure SetFoldersFieldNamesMap
      (Value: TdxPSDBBasedExplorerFoldersFieldNamesMap);
    procedure SetItems(Value: TDataSet);
    procedure SetItemsFieldNamesMap
      (Value: TdxPSDBBasedExplorerItemsFieldNamesMap);
  protected
    ...
    procedure MoveTo(AnItem: TCustomdxPSExplorerItem;
      AParent: TdxPSExplorerFolder); override;
    procedure RenameTo(AnItem: TCustomdxPSExplorerItem; AName: string);
      override;

    procedure DoRefresh; override;

    class function GetFoldersFieldNamesMapClass
      : TdxPSDBBasedExplorerFoldersFieldNamesMapClass; virtual;
    class function GetItemsFieldNamesMapClass
      : TdxPSDBBasedExplorerItemsFieldNamesMapClass; virtual;

    function AreIDsEqual(AnID1, AnID2: Integer): Boolean;
    procedure Changed; dynamic;
    function CheckDataSets: Boolean; virtual;
    function CheckItemDataSet(AnItem: TCustomdxPSExplorerItem): Boolean;
      overload; virtual;
    function CheckItemDataSet(AnItemClass: TCustomdxPSExplorerItemClass)
      : Boolean; overload; virtual;
    function CreateDataStream(AMode: TBlobStreamMode): TStream; virtual;
    function GetItemDataSet(AnItem: TCustomdxPSExplorerItem): TDataSet;
      overload; virtual;
    function GetItemDataSet(AnItemClass: TCustomdxPSExplorerItemClass)
      : TDataSet; overload; virtual;
    function GetItemFieldNamesMap(AnItem: TCustomdxPSExplorerItem)
      : TdxPSDBBasedExplorerFieldNamesMap; overload; virtual;
    function GetItemFieldNamesMap(AnItemClass: TCustomdxPSExplorerItemClass)
      : TdxPSDBBasedExplorerFieldNamesMap; overload; virtual;
    ...
    procedure LoadFolderFromCurrentRecord
      (AFolder: TdxPSDBBasedExplorerFolder); virtual;
    procedure LoadItemFromCurrentRecord
      (AnItem: TdxPSDBBasedExplorerItem); virtual;
    procedure SaveFolderToCurrentRecord
      (AFolder: TdxPSDBBasedExplorerFolder); virtual;
    procedure SaveItemToCurrentRecord(AnItem: TdxPSDBBasedExplorerItem;
      AReportLink: TBasedxReportLink); virtual;
    ...
    procedure SetFieldValue(ADataSet: TDataSet; const AName: string;
      const Value: Variant);
  ...
  published
    ...
    property FoldersFieldNamesMap: TdxPSDBBasedExplorerFoldersFieldNamesMap
      read FFoldersFieldNamesMap write SetFoldersFieldNamesMap;
    property Items: TDataSet read FItems write SetItems;
    property ItemsFieldNamesMap: TdxPSDBBasedExplorerItemsFieldNamesMap
      read FItemsFieldNamesMap write SetItemsFieldNamesMap;
    property OnLoadError: TdxPSDBBasedExplorerLoadErrorEvent read FOnLoadError
      write FOnLoadError;
  end;
...
function TdxPSDBBasedExplorerFolder.GetUniqueID(out AnUniqueID: TBytes)
  : Integer;
var
...
function TdxPSDBBasedExplorerFolder.GetFolder(Index: Integer)
  : TdxPSDBBasedExplorerFolder;
begin
  Result := inherited Folders[Index] as TdxPSDBBasedExplorerFolder;
end;

function TdxPSDBBasedExplorerFolder.GetItem(Index: Integer)
...
constructor TCustomdxPSDBBasedExplorerFieldNamesMap.Create
...
function TCustomdxPSDBBasedExplorerFieldNamesMap.GetItem
  (Index: Integer): string;
begin
  Result := FItems[Index];
end;

procedure TCustomdxPSDBBasedExplorerFieldNamesMap.SetItem(Index: Integer;
  const Value: string);
begin
...
begin
  Result := inherited GetMapCount + 3;
  { 6 ->  if include Creator, DateTimeCreated }
end;
...
function TdxPSDBBasedExplorer.CreateNewFolder(AParent: TdxPSExplorerFolder)
  : TdxPSExplorerFolder;
begin
  ...
  begin
    ...
    try
      ...
      if IsFolderIDAutoIncField then
        TdxPSDBBasedExplorerFolder(Result).FID :=
          GetFieldValue(Folders, FoldersFieldNamesMap.ID);
  ...
  begin
    ...
    try
      ...
      if IsItemIDAutoIncField then
        TdxPSDBBasedExplorerItem(Result).FID :=
          GetFieldValue(Items, ItemsFieldNamesMap.ID);
...
function TdxPSDBBasedExplorer.FindCustomItemByUniqueID(const AnUniqueID: TBytes)
...
function TdxPSDBBasedExplorer.FindFolderByID(AnID: Integer)
  : TdxPSDBBasedExplorerFolder;
var
  I: Integer;
begin
  ...
  begin
    if FolderList <> nil then
      ...
      begin
        ...
        if AreIDsEqual(Result.ID, AnID) then
          Exit;
      end;
    Result := nil;
  end
...
function TdxPSDBBasedExplorer.FindItemByID(AnID: Integer)
  : TdxPSDBBasedExplorerItem;
var
  I: Integer;
begin
...
const
  BlobStreamModesMap: array [TdxPSStreamMode] of TBlobStreamMode = (bmRead,
    bmWrite, bmReadWrite);
begin
  if IsFieldNameValid(Items, ItemsFieldNamesMap.Data) and LocateItem(AnItem)
  then
...
class function TdxPSDBBasedExplorer.GetRootFolderClass
...
procedure TdxPSDBBasedExplorer.DoLoadData(AFolder: TdxPSExplorerFolder);
  ...
  begin
    ...
    try
      ...
      begin
        try
          Folder := GetFolderClass.Create(Self, nil)
            as TdxPSDBBasedExplorerFolder;
  ...
  begin
    ...
    begin
      ...
      try
        Result := FilterLinkClass = TBasedxReportLink.ExtractComponentClass
          (Stream, False);
      finally
        Stream.Free;
      end;
  ...
  procedure BuildTree;
    ...
    begin
      ...
      begin
        ...
        if not AreIDsEqual(Folder.ParentID, TdxPSDBBasedExplorerFolder(Root).ID)
        then
        begin
          ...
          if Parent <> nil then
            if not Parent.HasAsParent(Folder) then
              ...
              raise EdxPSExplorer.CreateFmt
                (cxGetResourceString(@sdxCyclicIDReferences),
    ...
    begin
      ...
      begin
        ...
        if not AreIDsEqual(Item.ParentID, TdxPSDBBasedExplorerFolder(Root).ID)
        then
        begin
...
function TdxPSDBBasedExplorer.CanDelete
  (AnItem: TCustomdxPSExplorerItem): Boolean;
begin
  Result := inherited CanDelete(AnItem) and CheckItemDataSet(AnItem);
end;

function TdxPSDBBasedExplorer.CanMoveTo(AnItem,
  AParent: TCustomdxPSExplorerItem): Boolean;
begin
  Result := inherited CanMoveTo(AnItem, AParent) and
    ((esLoading in State) or CheckItemDataSet(AnItem));
end;

function TdxPSDBBasedExplorer.CanRenameTo(AnItem: TCustomdxPSExplorerItem;
  const AName: string): Boolean;
begin
  Result := inherited CanRenameTo(AnItem, AName) and
    ((esLoading in State) or CheckItemDataSet(AnItem));
end;
...
var
  ParentID: Integer;
begin
  if not(esLoading in State) and CanMoveTo(AnItem, AParent) and
    LocateItem(AnItem) then
...
procedure TdxPSDBBasedExplorer.RenameTo(AnItem: TCustomdxPSExplorerItem;
  AName: string);
begin
...
class function TdxPSDBBasedExplorer.GetFoldersFieldNamesMapClass
  : TdxPSDBBasedExplorerFoldersFieldNamesMapClass;
begin
  Result := TdxPSDBBasedExplorerFoldersFieldNamesMap;
end;

class function TdxPSDBBasedExplorer.GetItemsFieldNamesMapClass
  : TdxPSDBBasedExplorerItemsFieldNamesMapClass;
begin
...
begin
  Result := CheckItemDataSet(TdxPSDBBasedExplorerFolder) and
    CheckItemDataSet(TdxPSDBBasedExplorerItem);
end;

function TdxPSDBBasedExplorer.CheckItemDataSet
  (AnItem: TCustomdxPSExplorerItem): Boolean;
begin
  Result := CheckItemDataSet(TdxPSExplorerItemClass(AnItem.ClassType));
end;

function TdxPSDBBasedExplorer.CheckItemDataSet(AnItemClass
  : TCustomdxPSExplorerItemClass): Boolean;
var
  DataSet: TDataSet;
begin
...
begin
  Result := Items.CreateBlobStream
    (Items.FindField(ItemsFieldNamesMap.Data), AMode);
end;

function TdxPSDBBasedExplorer.GetItemDataSet(AnItem: TCustomdxPSExplorerItem)
  : TDataSet;
begin
  Result := GetItemDataSet(TCustomdxPSExplorerItemClass(AnItem.ClassType));
end;

function TdxPSDBBasedExplorer.GetItemDataSet(AnItemClass
  : TCustomdxPSExplorerItemClass): TDataSet;
begin
  if AnItemClass.InheritsFrom(TdxPSDBBasedExplorerFolder) then
    Result := Folders
  else if AnItemClass.InheritsFrom(TdxPSDBBasedExplorerItem) then
    Result := Items
  else
    Result := nil;
end;

function TdxPSDBBasedExplorer.GetItemFieldNamesMap
  (AnItem: TCustomdxPSExplorerItem): TdxPSDBBasedExplorerFieldNamesMap;
begin
...
procedure TdxPSDBBasedExplorer.LoadFolderFromCurrentRecord
  (AFolder: TdxPSDBBasedExplorerFolder);
begin
  AFolder.FID := GetFieldValue(Folders, FoldersFieldNamesMap.ID);
  AFolder.FParentID := VarToInt(GetFieldValue(Folders,
    FoldersFieldNamesMap.ParentID));
  AFolder.Name := VarToStr(GetFieldValue(Folders, FoldersFieldNamesMap.Name));
end;

procedure TdxPSDBBasedExplorer.LoadItemFromCurrentRecord
  (AnItem: TdxPSDBBasedExplorerItem);
begin
  AnItem.FID := GetFieldValue(Items, ItemsFieldNamesMap.ID);
  AnItem.FParentID := VarToInt(GetFieldValue(Items,
    ItemsFieldNamesMap.ParentID));
  AnItem.Name := VarToStr(GetFieldValue(Items, ItemsFieldNamesMap.Name));
end;

procedure TdxPSDBBasedExplorer.SaveFolderToCurrentRecord
  (AFolder: TdxPSDBBasedExplorerFolder);
begin
  if not IsFolderIDAutoIncField then
...
procedure TdxPSDBBasedExplorer.SaveItemToCurrentRecord
  (AnItem: TdxPSDBBasedExplorerItem; AReportLink: TBasedxReportLink);
begin
  if not IsItemIDAutoIncField then
...
begin
  if Assigned(FOnLoadError) then
    FOnLoadError(Self, ADataSet);
end;

function TdxPSDBBasedExplorer.GetFieldValue(ADataSet: TDataSet;
...
function TdxPSDBBasedExplorer.IsFieldNameValid(ADataSet: TDataSet;
  const AName: string): Boolean;
begin
  Result := (AName <> '') and (ADataSet <> nil) and
    (ADataSet.FindField(AName) <> nil);
end;

procedure TdxPSDBBasedExplorer.SetFieldValue(ADataSet: TDataSet;
  const AName: string; const Value: Variant);
begin
  if IsFieldNameValid(ADataSet, AName) then
    ADataSet[AName] := Value;
end;

function TdxPSDBBasedExplorer.LocateItem(AnItem: TCustomdxPSExplorerItem)
  : Boolean;
begin
  Result := True;
  if esLoading in State then
    Exit;

  if AnItem is TdxPSDBBasedExplorerItem then
    Result := IsFieldNameValid(Items, ItemsFieldNamesMap.ID) and
      Items.Locate(ItemsFieldNamesMap.ID,
      TdxPSDBBasedExplorerItem(AnItem).ID, [])
  else if AnItem is TdxPSDBBasedExplorerFolder then
    Result := IsFieldNameValid(Folders, FoldersFieldNamesMap.ID) and
      Folders.Locate(FoldersFieldNamesMap.ID,
      TdxPSDBBasedExplorerFolder(AnItem).ID, []);
end;

procedure TdxPSDBBasedExplorer.PrepareIterate(ADataSet: TDataSet;
  out ABookmark: TBookmark);
begin
...
procedure TdxPSDBBasedExplorer.UnprepareIterate(ADataSet: TDataSet;
...
procedure TdxPSDBBasedExplorer.SetFoldersFieldNamesMap
  (Value: TdxPSDBBasedExplorerFoldersFieldNamesMap);
begin
...
procedure TdxPSDBBasedExplorer.SetItemsFieldNamesMap
  (Value: TdxPSDBBasedExplorerItemsFieldNamesMap);
begin
