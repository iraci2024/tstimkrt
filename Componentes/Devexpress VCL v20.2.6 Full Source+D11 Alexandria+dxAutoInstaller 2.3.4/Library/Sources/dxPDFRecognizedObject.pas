...
uses
  Types, SysUtils, Graphics, Generics.Defaults, Generics.Collections,
  cxGeometry, dxPDFBase, dxPDFTypes, dxPDFText;
...
type
  TdxPDFRecognitionObject = (rmAnnotations, rmImages, rmText);
  TdxPDFRecognitionObjects = set of TdxPDFRecognitionObject;
  ...
  protected
    ...
    property DocumentImage: TdxPDFReferencedObject read FDocumentImage
      write SetDocumentImage;
  public
    constructor Create;
  ...
  public
    ...
    constructor Create(const ABounds: TdxPDFOrientedRect;
      const AText: string); overload;

    property Text: string read GetText;
  end;

  TdxPDFTextObjectList<T: TdxPDFTextObject> = class
  ...
  public
    constructor Create(const ABounds: TdxPDFOrientedRect;
      const ACharacters: TdxPDFTextCharacters; AWrapOffset: Integer;
      AWordEnded: Boolean);
    destructor Destroy; override;
  ...
  protected
    ...
    function GetPosition(const APosition: TdxPDFPosition)
      : TdxPDFTextPosition; overload;
    function GetPosition(APageIndex: Integer; const P: TdxPointF)
      : TdxPDFTextPosition; overload;
    function GetRange(APageIndex: Integer; const R: TdxRectF)
      : TdxPDFPageTextRange;

    function GetHighlight(AStartWordIndex, AStartOffset, AEndWordIndex,
      AEndOffset: Integer; ASplitBounds: Boolean; ARectIndex: Integer)
      : TdxPDFOrientedRect; overload;
    ...
    function GetHighlights(AStartWordIndex, AStartOffset, AEndWordIndex,
      AEndOffset: Integer; ASplitBounds: Boolean)
      : TdxPDFOrientedRectList; overload;
  ...
  public
    ...
    function Find(const APosition: TdxPDFPosition;
      const ATextExpansionFactor: TdxPointF; out ALine: TdxPDFTextLine)
      : Boolean;
  end;
  ...
  strict private
    ...
    FCharLocations: TdxPointsF;
    FFontData: TdxPDFFontData;
    FFontDescent: Single;
    FFontHeight: Single;
    FLocation: TdxPointF;
  public
    constructor Create(const AData: TdxPDFStringData; AFontData: TdxPDFFontData;
      AGraphicsState: TObject);
    destructor Destroy; override;

    function CalculateCharacterWidth(ACharacter: Integer;
      AHorizontalFontFactor: Single): Single;
    ...
    property CharLocations: TdxPointsF read FCharLocations;
  ...
  public
    ...
    property CurrentWordLeftBoundary: TdxPointF read FCurrentWordLeftBoundary
      write FCurrentWordLeftBoundary;
    property CurrentWordRightBoundary: TdxPointF read FCurrentWordRightBoundary
      write FCurrentWordRightBoundary;
    property PreviousCharacter: TdxPDFTextCharacter read FPreviousCharacter;
    property PreviousCharacterBlock: TdxPDFTextBlock
      read FPreviousCharacterBlock;
  ...
  strict private
    ...
    procedure AddTextLine(ATextLines: TdxPDFTextLineList;
      AWordList: TdxPDFTextWordList; APartList: TdxPDFTextWordPartList);
    procedure Clear;
  ...
  strict protected
    class procedure Append(ABuilder: TdxBiDiStringBuilder;
      const AStart, AEnd: TdxPDFPageTextPosition;
      AWordPart: TdxPDFTextWordPart); overload;
    class procedure Append(ABuilder: TdxBiDiStringBuilder;
      const AStart, AEnd: TdxPDFPageTextPosition;
      ALine: TdxPDFTextLine); overload;
    class procedure Append(ABuilder: TdxBiDiStringBuilder;
      const AStart, AEnd: TdxPDFPageTextPosition;
      ALines: TdxPDFTextLineList); overload;
    class procedure AppendWordPart(ABuilder: TdxBiDiStringBuilder;
      APart: TdxPDFTextWordPart); overload;
    class procedure AppendWordPart(ABuilder: TdxBiDiStringBuilder;
      APart: TdxPDFTextWordPart; AStartOffset: Integer); overload;
    class procedure AppendWordPart(ABuilder: TdxBiDiStringBuilder;
      APart: TdxPDFTextWordPart; AStartOffset, AEndOffset: Integer); overload;
  public
    class function ConvertToString(const AStart, AEnd: TdxPDFPageTextPosition;
      AWordPart: TdxPDFTextWordPart): string; overload;
    class function ConvertToString(const AStart, AEnd: TdxPDFPageTextPosition;
      ALine: TdxPDFTextLine): string; overload;
    class function ConvertToString(const AStart, AEnd: TdxPDFPageTextPosition;
      ALines: TdxPDFTextLineList): string; overload;
  end;

implementation

uses
  Math, Classes, dxCore, dxPDFCore, dxPDFCharacterMapping, dxPDFFont,
  dxPDFFontEncoding, dxPDFUtils;
...
function TdxPDFRecognizedObject.QueryInterface(const IID: TGUID; out Obj)
  : HRESULT; stdcall;
begin
...
begin
  if FBitmap = nil then
    FBitmap := TdxPDFDocumentImageAccess(DocumentImage as TdxPDFDocumentImage)
      .GetAsBitmap;
  Result := FBitmap;
end;
...
constructor TdxPDFTextObject.Create(const ABounds: TdxPDFOrientedRect;
  const AText: string);
begin
  inherited Create;
  FBounds := ABounds;
...
constructor TdxPDFTextWordPart.Create(const ABounds: TdxPDFOrientedRect;
  const ACharacters: TdxPDFTextCharacters; AWrapOffset: Integer;
  AWordEnded: Boolean);
begin
...
begin
  Result := (WordIndex = AWordIndex) and (WrapOffset <= AOffset) and
    (AOffset <= NextWrapOffset);
end;
...
begin
  ...
  for APart in AParts do
    FWordPartList.Add(TdxPDFTextWordPart.Create(APart.Bounds, APart.Characters,
      APart.WrapOffset, APart.WordEnded));
end;
...
begin
  ...
  begin
    ...
    Result := IfThen(GetRotatedTopLeft(Left.BoundsList[0]).X <
      GetRotatedTopLeft(Right.BoundsList[0]).X, -1, 1);
  end;
end;
...
begin
  ...
  begin
    ARange := GetLineRange;
    FText := TdxPDFRecognizedTextUtils.ConvertToString(ARange.StartPosition,
      ARange.EndPosition, Self);
  end;
...
begin
  if WordPartList.Count > 0 then
    Result := TdxPDFPageTextRange.Create(-1, WordPartList[0].WordIndex, 0,
      WordPartList.Last.WordIndex, WordPartList.Last.Characters.Count)
  else
    Result := TdxPDFPageTextRange.Invalid;
end;

function TdxPDFTextLine.GetPosition(const APosition: TdxPDFPosition)
  : TdxPDFTextPosition;
begin
  Result := GetPosition(APosition.PageIndex, APosition.Point)
end;

function TdxPDFTextLine.GetPosition(APageIndex: Integer; const P: TdxPointF)
  : TdxPDFTextPosition;
var
  J: Integer;
...
begin
  ...
  for APart in FWordPartList do
  begin
    ARotatedLeft := TdxPDFRecognizedTextUtils.RotatePoint(APart.Bounds.TopLeft,
      -AAngle).X;
    ...
    begin
      ...
      if (ANearestPart = nil) or APart.Bounds.PtInRect(P) or
        (Abs(TdxPDFRecognizedTextUtils.RotatePoint(R.TopLeft, -AAngle).X +
        R.Width - X) >= Abs(ARotatedLeft - X)) then
        ANearestPart := APart;

      for J := 0 to ANearestPart.Characters.Count - 1 do
      begin
        ABounds := ANearestPart.Characters[J].Bounds;
        if TdxPDFRecognizedTextUtils.RotatePoint(ABounds.TopLeft, -AAngle).X +
          ABounds.Width / 2 >= TdxPDFRecognizedTextUtils.RotatePoint(P,
          -AAngle).X then
          Exit(TdxPDFTextPosition.Create(APageIndex, ANearestPart.WordIndex,
            J + ANearestPart.WrapOffset));
      end;
    end;
  end;
  Result := TdxPDFTextPosition.Create(APageIndex, ANearestPart.WordIndex,
    ANearestPart.WrapOffset + ANearestPart.Characters.Count);
end;

function TdxPDFTextLine.GetRange(APageIndex: Integer; const R: TdxRectF)
  : TdxPDFPageTextRange;
var
  I: Integer;
  APart: TdxPDFTextWordPart;
...
function TdxPDFTextLine.GetHighlight(AStartWordIndex, AStartOffset,
  AEndWordIndex, AEndOffset: Integer; ASplitBounds: Boolean;
  ARectIndex: Integer): TdxPDFOrientedRect;
var
  AList: TdxPDFOrientedRectList;
begin
  AList := GetHighlights(AStartWordIndex, AStartOffset, AEndWordIndex,
    AEndOffset, ASplitBounds);
  try
...
function TdxPDFTextLine.GetHighlight(AStartWordIndex, AStartOffset: Integer;
  ASplitBounds: Boolean; ARectIndex: Integer): TdxPDFOrientedRect;
begin
  Result := GetHighlight(AStartWordIndex, AStartOffset, FWordPartList.Count - 1,
    -1, ASplitBounds, ARectIndex);
end;

function TdxPDFTextLine.GetHighlights(AStartWordIndex, AStartOffset,
  AEndWordIndex, AEndOffset: Integer; ASplitBounds: Boolean)
  : TdxPDFOrientedRectList;
var
...
begin
  ...
  ATopLeft := TdxPDFRecognizedTextUtils.RotatePoint
    (AStartCharRect.TopLeft, -AAngle);
  ALeft := IfThen(AStartOnEndOfWord, ATopLeft.X + AStartCharRect.Width,
    ATopLeft.X);
  ATop := ATopLeft.Y;
  ARight := ALeft;
  ...
  begin
    ...
    AEnd := IfThen((I = AEndWordIndex) and (AEndOffset <> -1), AEndOffset,
      FWordPartList[I].Characters.Count);

    if ASplitBounds and ((AStart = 0) or (AStart < AEnd)) then
    begin
      ACharRect := FWordPartList[I].Characters[AStart].Bounds;
      ACharTopLeft := TdxPDFRecognizedTextUtils.RotatePoint
        (ACharRect.TopLeft, -AAngle);
      if ACharTopLeft.X > ARight + 2 * ACharRect.Width then
      begin
        Result.Add(TdxPDFOrientedRect.Create
          (TdxPDFRecognizedTextUtils.RotatePoint(dxPointF(ALeft, ATop), AAngle),
    ...
    if AEnd = 0 then
      ARight := TdxPDFUtils.Max(ARight,
        TdxPDFRecognizedTextUtils.RotatePoint(FWordPartList[I].Characters[0]
        .Bounds.TopLeft, -AAngle).X)
    else
  ...
  Result.Add(TdxPDFOrientedRect.Create(TdxPDFRecognizedTextUtils.RotatePoint
    (dxPointF(ALeft, ATop), AAngle), ARight - ALeft, ATop - ABottom, AAngle));
end;

function TdxPDFTextLine.GetHighlights(AStartWordIndex, AStartOffset: Integer;
  ASplitBounds: Boolean): TdxPDFOrientedRectList;
begin
  Result := GetHighlights(AStartWordIndex, AStartOffset,
    FWordPartList.Count - 1, -1, ASplitBounds);
end;

function TdxPDFTextLine.PositionInLine(AWordIndex: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Exit((AWrapOffset <= AOffset) and
          (AOffset - AWrapOffset <= ACharCount));
      end
      else
...
procedure TdxPDFTextLine.PopulateWordPartList(AWordPartList
...
function TdxPDFTextLineList.Find(const APosition: TdxPDFPosition;
  const ATextExpansionFactor: TdxPointF; out ALine: TdxPDFTextLine): Boolean;
var
...
begin
  ...
  if APosition.PageIndex >= 0 then
    ...
    begin
      ...
      if ABounds.PtInRect(APosition.Point, ATextExpansionFactor.X,
        ATextExpansionFactor.Y) then
        for APart in ACurrentLine.WordPartList do
          if APart.Bounds.PtInRect(APosition.Point, ATextExpansionFactor.X,
            ATextExpansionFactor.Y) then
            ALine := ACurrentLine;
    end;
  Result := ALine <> nil;
...
constructor TdxPDFTextBlock.Create(const AData: TdxPDFStringData;
  AFontData: TdxPDFFontData; AGraphicsState: TObject);

  function CalculateVectorLength(const P: TdxPointF): Single;
  begin
...
var
  ...
  ARotatedLocation, ATopLeft, AVectorLength: TdxPointF;
  ...
  AMatrix, ALinearMatrix: TdxPDFTransformationMatrix;
...
begin
  ...
  ATextState := AState.TextState;
  FFontData := AFontData;
  AFont := FFontData.Font as TdxPDFCustomFont;
  AMatrix := TdxPDFTransformationMatrix.Create(1, 0, 0, 1, 0, ATextState.Rise);
  AMatrix.Multiply(ATextState.TextMatrix, moAppend);
  AMatrix.Multiply(AState.TransformMatrix, moAppend);
  FLocation := AMatrix.Transform(cxNullPoint);
  ALinearMatrix := TdxPDFTransformationMatrix.Create(AMatrix.A, AMatrix.B,
    AMatrix.C, AMatrix.D, 0, 0);
  pt := ALinearMatrix.Transform(dxPointF(1, 0));
  AVectorLength.X := CalculateVectorLength(pt);
  AAngle := Math.ArcTan2(pt.Y, pt.X);
  AVectorLength.Y := CalculateVectorLength
    (ALinearMatrix.Transform(dxPointF(0, 1)));
  AAbsoluteFontSize := AState.TextState.AbsoluteFontSize * AVectorLength.Y;
  AHorizontalScaling := AState.TextState.HorizontalScaling / 100;
  AUnscaledUnitsFactor := AHorizontalScaling * AVectorLength.X;
  AHorizontalFontFactor := AState.TextState.FontSize * AFont.WidthFactor *
    AUnscaledUnitsFactor;
  AVerticalFontFactor := AAbsoluteFontSize * AFont.HeightFactor;
  FCharacterSpacing := AState.TextState.CharacterSpacing * AUnscaledUnitsFactor;
  FFontDescent := AFontData.Metrics.Descent * AVerticalFontFactor;
  FFontHeight := AFontData.Metrics.Height * AVerticalFontFactor;
  FAngle := AAngle;
  X := FLocation.X;
  Y := FLocation.Y;
  AGlyphAdvances := AData.Advances;
  ALength := Length(AGlyphAdvances);
  ...
  begin
    AGlyphOffset := AGlyphOffset + AGlyphAdvances[AOffset] * AVectorLength.X;
    Inc(AOffset);
    FCharLocations[I] := dxPointF(X + AGlyphOffset * ACos,
      Y + AGlyphOffset * ASin);
  end;
  ACount := Length(AData.CharCodes);
  AHeight := FontHeight + FontDescent;

  FCharacters := TdxPDFTextCharacters.Create;
  for I := 0 to ACount - 1 do
  begin
    ARotatedLocation := TdxPDFRecognizedTextUtils.RotatePoint
      (FCharLocations[I], -AAngle);
    ACharacterWidth := CalculateCharacterWidth(AData.Str[I],
      AHorizontalFontFactor);
    ATopLeft := TdxPDFRecognizedTextUtils.RotatePoint
      (dxPointF(ARotatedLocation.X, ARotatedLocation.Y + AHeight), AAngle);
    ABounds := TdxPDFOrientedRect.Create(ATopLeft, ACharacterWidth,
      FontHeight, AAngle);
    FCharacters.Add(TdxPDFTextCharacter.Create(ABounds,
      GetActualCharacter(AData.CharCodes[I]), AAbsoluteFontSize));
  end;
end;
...
function TdxPDFTextBlock.CalculateCharacterWidth(ACharacter: Integer;
  AHorizontalFontFactor: Single): Single;
var
  AIndex: Integer;
begin
  AIndex := ACharacter - FFontData.FirstChar;
  if (FFontData.Widths = nil) or (AIndex < 0) or
    ((FFontData.Widths <> nil) and (AIndex > Length(FFontData.Widths) - 1)) then
...
constructor TdxPDFTextParserState.Create(ABlocks: TList<TdxPDFTextBlock>;
  APageCropBox: TdxRectF);
begin
  inherited Create;
  FActualRTL := False;
  FBlocks := ABlocks;
...
var
  AOverlapFactor, AIndexHeightRatio, ABlock1FontHeight,
    ABlock2FontHeight: Double;
  ABlock1Angle, ABlock2Angle, ARowBlockHeight, AIndexBlockHeight,
    ARowBlockAngle: Double;
  AIndexBlockAngle, AOverlapValue, AActualOverlap: Double;
  ABlock1TopLeft, ABlock2TopLeft, ARowBlockTopLeft, AIndexBlockTopLeft
    : TdxPointF;
...
begin
  ...
  if (ABlock1FontHeight / AIndexHeightRatio > ABlock2FontHeight) or
    (ABlock2FontHeight / AIndexHeightRatio > ABlock1FontHeight) then
  begin
    ...
    ARotatedRowBlockTopLeft := TdxPDFRecognizedTextUtils.RotatePoint
      (ARowBlockTopLeft, -ARowBlockAngle);
    ARotatedIndexBlockTopLeft := TdxPDFRecognizedTextUtils.RotatePoint
      (AIndexBlockTopLeft, -AIndexBlockAngle);
    AActualOverlap := TdxPDFUtils.Min(ARotatedRowBlockTopLeft.Y,
      ARotatedIndexBlockTopLeft.Y) - TdxPDFUtils.Max(ARotatedRowBlockTopLeft.Y -
      ARowBlockHeight, ARotatedIndexBlockTopLeft.Y - AIndexBlockHeight);
...
var
  ADistanceFactor, AAvgWidth, AAngle, ADistanceFromLeft, ADistanceFromRight,
    ADistance: Double;
...
begin
  ...
  AAvgWidth := ADistanceFactor *
    (ACurrentCharRectangle.Width + APreviousCharRectangle.Width) / 2;
  if FIsRTL then
  begin
    AWordLeft := FCurrentWordLeftBoundary;
    AWordRight := FCurrentWordRightBoundary;
    AAngle := ACurrentCharRectangle.Angle;
    ADistanceFromLeft := TdxPDFRecognizedTextUtils.GetOrientedDistance
      (ACurrentCharRectangle.TopRight, AWordLeft, AAngle);
    ADistanceFromRight := TdxPDFRecognizedTextUtils.GetOrientedDistance
      (AWordRight, ACurrentCharRectangle.TopLeft, AAngle);
    Result := (ADistanceFromLeft > AAvgWidth) or
      (ADistanceFromRight > AAvgWidth);
  end
  else
  begin
    APreviousCharPosition := FPreviousCharLocation;
    ADistance := TdxPDFRecognizedTextUtils.GetOrientedDistance
      (FPreviousCharLocation, FCurrentCharLocation,
    ...
    begin
      ...
      if not Result then
        Result := Abs(ADistance / FPreviousCharacterBlock.CharacterSpacing) >
          (1 + ADistanceFactor);
    end;
  end;
end;
...
var
  ALineOverlapFactor, APreviousBlockAngle, ACurrentBlockAngle,
    AVerticalDifference, ABlockFontHeight: Double;
...
begin
  ...
  AVerticalDifference :=
    Abs(TdxPDFRecognizedTextUtils.RotatePoint(APreviousBlockLocation,
    -APreviousBlockAngle).Y - TdxPDFRecognizedTextUtils.RotatePoint
    (ACurrentBlockLocation, -ACurrentBlockAngle).Y);
  ABlockFontHeight := TdxPDFUtils.Min(FCurrentCharacterBlock.FontHeight,
    FPreviousCharacterBlock.FontHeight);
  Result := (AVerticalDifference > ABlockFontHeight * (1 - ALineOverlapFactor))
    and not IsIndex;
end;
...
begin
  ...
  FIsRTL := TdxPDFRecognizedTextUtils.HasRTLMarker(ACurrentCharUnicode) or
    TdxPDFRecognizedTextUtils.HasRTLMarker(APrevCharUnicode);
  FIsCJKWord := TdxPDFRecognizedTextUtils.HasCJKMarker(ACurrentCharUnicode);
  FIsLineStart := (FCurrentCharacterBlock.Angle <>
    FPreviousCharacterBlock.Angle) or CheckVerticalPosition;
  FIsSpace := FIsLineStart or TdxPDFRecognizedTextUtils.IsWhitespace
    (ACurrentCharUnicode) or CheckHorizontalPosition;
  ...
  FIsWrap := FIsLineStart and TdxPDFRecognizedTextUtils.IsWrapSymbol
    (APreviousBlockCharacters[APreviousBlockCharacters.Count - 1].Text) and
...
begin
  ...
  begin
    ...
    if FParserState.IsSpace or FParserState.IsSeparator or
      FParserState.IsFinished or FParserState.IsCJKWord then
    begin
      APartRectangle := TdxPDFOrientedRect.Create
        (TdxPDFRecognizedTextUtils.RotatePoint(dxPointF(FWordMinX, FWordMaxY),
        FWordAngle), FWordMaxX - FWordMinX, FWordMaxY - FWordMinY, FWordAngle);
      AWordWrap := FParserState.IsWrap and
        (ALineXOffset <= APartRectangle.Left);
      AWordPart := TdxPDFTextWordPart.Create(APartRectangle, FCharacters,
        FWrapOffset, not AWordWrap and (FParserState.IsSpace or
        FParserState.IsFinished));
      if not AWordPart.IsEmpty then
      begin
        FWordPartList.Add(AWordPart);
        if (AWordWrap and (Length(AWordPart.Text) > 1)) and not FParserState.IsFinished
        then
        begin
...
begin
  ...
  if ((AUnicodeData <> #$A0) and (AUnicodeData <> ' ')) and (AUnicodeData <> #9)
  then
  begin
    ABlock := FParserState.CurrentCharacterBlock;
    ABlockAngle := ABlock.Angle;
    ACharLocation := TdxPDFRecognizedTextUtils.RotatePoint
      (FParserState.CurrentCharLocation, -ABlockAngle);
    AMinY := TdxPDFRecognizedTextUtils.RotatePoint(ABlock.Location,
      -ABlockAngle).Y + ABlock.FontDescent;
    ...
    begin
      ...
      if not TdxPDFUtils.IsRectEmpty(ACharBox) and
        TdxPDFUtils.Intersects(FParserState.PageCropBox, ACharBox) then
      begin
...
constructor TdxPDFPageTextLineBuilder.Create(AParserState
  : TdxPDFTextParserState);
begin
  inherited Create;
  FWordIndex := 1;
...
function TdxPDFPageTextLineBuilder.OverlapsWithPreviousWords
  (AWord: TdxPDFTextWord): Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TdxPDFPageTextLineBuilder.AddTextLine(ATextLines: TdxPDFTextLineList;
  AWordList: TdxPDFTextWordList; APartList: TdxPDFTextWordPartList);
var
...
class procedure TdxPDFRecognizedTextUtils.AppendWordPart
  (ABuilder: TdxBiDiStringBuilder; APart: TdxPDFTextWordPart);
begin
...
class procedure TdxPDFRecognizedTextUtils.Append(ABuilder: TdxBiDiStringBuilder;
  const AStart, AEnd: TdxPDFPageTextPosition; AWordPart: TdxPDFTextWordPart);
var
...
begin
  ...
  if AWithoutStartWord and AWithoutEndWord or AWithoutStartWord and
    (AWordIndex < AEnd.WordIndex) or AWithoutEndWord and
    (AWordIndex > AStart.WordIndex) or (AWordIndex > AStart.WordIndex) and
    (AWordIndex < AEnd.WordIndex) then
  begin
    AppendWordPart(ABuilder, AWordPart);
    if AWordPart.WordEnded then
      ABuilder.Append(' ');
  end
  else if (AWordIndex = AEnd.WordIndex) then
  begin
    if AWordIndex = AStart.WordIndex then
    begin
      if (AStart.Offset = 0) and (AEnd.Offset < 0) then
        AppendWordPart(ABuilder, AWordPart)
      else if (AStartOffset <= APartLength) and (AEndOffset >= 0) then
        AppendWordPart(ABuilder, AWordPart, AStartOffset, AEndOffset);
    end
    else if AWithoutStartWord or (AWordIndex > AStart.WordIndex) then
    begin
      if AEnd.Offset < 0 then
        AppendWordPart(ABuilder, AWordPart)
      else if AEndOffset >= 0 then
        AppendWordPart(ABuilder, AWordPart, 0, AEndOffset);
    end;
  end
  else if (AWithoutEndWord or (AWordIndex < AEnd.WordIndex)) and
    (AWordIndex = AStart.WordIndex) and (AStartOffset <= APartLength) then
  begin
...
class function TdxPDFRecognizedTextUtils.ConvertToString(const AStart,
  AEnd: TdxPDFPageTextPosition; AWordPart: TdxPDFTextWordPart): string;
var
  ABuilder: TdxBiDiStringBuilder;
begin
  ABuilder := TdxBiDiStringBuilder.Create;
...
begin
  ...
  for APart in ALine.WordPartList do
    try
      ANeedNewLine := (APart.WordIndex = AEnd.WordIndex) and
        ((AEnd.Offset = APart.Characters.Count) or (AEnd.Offset < 0));
  ...
  if not ABuilder.Empty and (ALastWordIndex >= AStart.WordIndex) and
    ((ALastWordIndex < AEnd.WordIndex) or ANeedNewLine or (AEnd.WordIndex = 0))
  then
    ABuilder.AppendLine;
end;

class procedure TdxPDFRecognizedTextUtils.Append(ABuilder: TdxBiDiStringBuilder;
  const AStart, AEnd: TdxPDFPageTextPosition; ALines: TdxPDFTextLineList);
var
  ALine: TdxPDFTextLine;
begin
...
class function TdxPDFRecognizedTextUtils.ConvertToString(const AStart,
  AEnd: TdxPDFPageTextPosition; ALine: TdxPDFTextLine): string;
var
  ABuilder: TdxBiDiStringBuilder;
  AWordPart: TdxPDFTextWordPart;
begin
...
class function TdxPDFRecognizedTextUtils.ConvertToString(const AStart,
  AEnd: TdxPDFPageTextPosition; ALines: TdxPDFTextLineList): string;
var
  ABuilder: TdxBiDiStringBuilder;
begin
  ABuilder := TdxBiDiStringBuilder.Create;
