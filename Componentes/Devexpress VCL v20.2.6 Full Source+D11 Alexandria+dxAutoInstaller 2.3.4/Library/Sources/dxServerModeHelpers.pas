...
type
  ...
  protected
    ...
    function PackSample(ASample: TdxSample; ACombination: string)
      : TdxPackSample;
    function UnpackSolution(ALlsResults: array of Double; ACombination: string)
      : TdxServerModeServerAndChannelModel;
    function Weight(ASamples: TdxFastList;
  ...
  public
    ...
    function Linear(AUnknowns: Integer; AData: TdxPackSampleArray)
      : TdxPackSample;
    function LinearLeastSquares(AArgs: TdxLinearLeastSquaresArgs)
      : TdxPackSample;
    function PackSamples(ASamples: TdxFastList; const ACombination: string)
      : TdxPackSampleArray;
    procedure RegisterSample(ATake, AScan: Integer; time: Double);
    function Resolve(ASamples: TdxFastList; const ACombination: string)
      : TdxServerModeServerAndChannelModel; overload;
  ...
  private
    ...
    FTotalCount: Integer;
  public
    constructor Create(AModel: TdxServerModeServerAndChannelModel;
      AIndex, AMinIndex, AMaxIndex, ATotalCount, ABaseCountTakeData,
      AMaxAllowedTake: Integer; AFillTimeMultiplier, AEdgeTimeMultiplier,
  ...
  public
    class function CalculateOptimalFetchResult
      (AParam: TdxServerModeOptimalFetchParam): TdxServerModeOptimalFetchResult;
    class function CalculateTakeFromFixedTimeAndScan
      (AModel: TdxServerModeServerAndChannelModel; ATargetTime: Double;
      AScan: Integer; out AValue: Integer): Boolean;
    class function CalculateTakeFromFixedTimeAndSkip
      (AModel: TdxServerModeServerAndChannelModel; ATargetTime: Double;
      ASkip: Integer; out AValue: Integer): Boolean;
    class function OptimalResultCore(AParam: TdxServerModeOptimalFetchParam)
      : TdxSolution;
  end;
  ...
  public
    constructor Create(ASkip, ATake: Integer; AIsFromEnd: Boolean;
      AParams: TdxServerModeOptimalFetchParam);
    function FromI1I2(AParams: TdxServerModeOptimalFetchParam;
      AIsFromEnd: Boolean; I1, I2: Integer): TdxSolution;
    class function FromSkipTake(AParams: TdxServerModeOptimalFetchParam;
      AIsFromEnd: Boolean; ASkip, ATake: Integer): TdxSolution;
...
const
  Combinations: array [0 .. 6] of string = ('C', 'S', 'T', 'CS', 'CT',
    'TS', 'CTS');
...
function TdxServerModeServerAndChannelModelBuilder.GetMaxObservableTake
  : Integer;
var
  I: Integer;
begin
...
function TdxServerModeServerAndChannelModelBuilder.Linear(AUnknowns: Integer;
  AData: TdxPackSampleArray): TdxPackSample;
var
...
function TdxServerModeServerAndChannelModelBuilder.LinearLeastSquares
  (AArgs: TdxLinearLeastSquaresArgs): TdxPackSample;
var
  ARow: TdxPackSample;
...
function TdxServerModeServerAndChannelModelBuilder.PackSamples
  (ASamples: TdxFastList; const ACombination: string): TdxPackSampleArray;
var
  I: Integer;
begin
...
procedure TdxServerModeServerAndChannelModelBuilder.RegisterSample(ATake,
  AScan: Integer; time: Double);
begin
...
function TdxServerModeServerAndChannelModelBuilder.Resolve
  (ASamples: TdxFastList; const ACombination: string)
  : TdxServerModeServerAndChannelModel;
var
  ASolution: TdxPackSample;
  AArgs: TdxLinearLeastSquaresArgs;
begin
  AArgs := TdxLinearLeastSquaresArgs.Create(Length(ACombination),
    PackSamples(ASamples, ACombination));
  try
...
function TdxServerModeServerAndChannelModelBuilder.Resolve
  ...
  function CalculateCombinationIndex(ABaseSequence, AValidateSequence
    : TdxFastList): Integer;
  var
    I: Integer;
    ACombination: string;
...
begin
  ...
  try
    ACombinationIndex := CalculateCombinationIndex(ABaseSequence,
      AValidateSequence);
...
function TdxServerModeServerAndChannelModelBuilder.IsInvalid
  (ASolution: TdxServerModeServerAndChannelModel): Boolean;
begin
  Result := (ASolution.ConstantPart < 0) or (ASolution.ScanCoeff < 0) or
    (ASolution.TakeCoeff < 0);
end;

function TdxServerModeServerAndChannelModelBuilder.PackSample
  (ASample: TdxSample; ACombination: string): TdxPackSample;
var
  I: Integer;
  AField: Char;
  AValue: Double;
...
function TdxServerModeServerAndChannelModelBuilder.UnpackSolution
  (ALlsResults: array of Double; ACombination: string)
  : TdxServerModeServerAndChannelModel;
var
  I: Integer;
  AField: Char;
...
begin
  ...
  begin
    ASample := ASamples[I];
    APrediction := ASolution.ConstantPart + ASolution.TakeCoeff * ASample.Take +
      ASolution.ScanCoeff * ASample.Scan;
...
constructor TdxServerModeServerAndChannelModel.Create(AConstPart, ATakeCoeff,
  AScanCoeff: Double);
begin
  inherited Create;
...
function TdxServerModeServerAndChannelModel.Clone
  : TdxServerModeServerAndChannelModel;
begin
  Result := TdxServerModeServerAndChannelModel.Create(ConstantPart, TakeCoeff,
    ScanCoeff);
end;

function TdxServerModeServerAndChannelModel.ToString: string;
begin
  Result := Format('Take*%g + Scan*%g + %g', [ConstantPart, TakeCoeff,
    ScanCoeff])
end;

{ TdxServerModeOptimalFetchParam }

constructor TdxServerModeOptimalFetchParam.Create
  (AModel: TdxServerModeServerAndChannelModel; AIndex, AMinIndex, AMaxIndex,
  ATotalCount, ABaseCountTakeData, AMaxAllowedTake: Integer;
  AFillTimeMultiplier, AEdgeTimeMultiplier, AMiddleTimeMultiplier: Double);
begin
  inherited Create;
  Assert((AIndex >= 0) and (AMinIndex >= 0) and (AMaxIndex >= 0) and
    (AIndex >= AMinIndex) and (AIndex <= AMaxIndex) and (AMinIndex <= AMaxIndex)
    and (ATotalCount > 0) and (ABaseCountTakeData >= 0) and (ATotalCount >= 0)
    and (AMiddleTimeMultiplier >= 1.0) and
    (AEdgeTimeMultiplier >= AMiddleTimeMultiplier) and
    (AFillTimeMultiplier >= AEdgeTimeMultiplier), 'Incorrect input data!');
  FModel := AModel;
  FIndex := AIndex;
  FMinIndex := AMinIndex;
  FMaxIndex := AMaxIndex;
...
function TdxServerModeOptimalFetchParam.ScanFromBottom(ALastIndexToFetch
  : Integer): Integer;
begin
  Result := TotalCount - ALastIndexToFetch;
end;

function TdxServerModeOptimalFetchParam.ScanFromTop(ALastIndexToFetch
  : Integer): Integer;
begin
  Result := ALastIndexToFetch + 1;
end;

function TdxServerModeOptimalFetchParam.SkipFromBottom(AFirstIndexToFetch
  : Integer): Integer;
begin
  Result := TotalCount - AFirstIndexToFetch - 1;
end;

function TdxServerModeOptimalFetchParam.SkipFromTop(AFirstIndexToFetch
  : Integer): Integer;
begin
...
begin
  Result := Format
    ('new ServerModeOptimalFetchParam(new ServerModeServerAndChannelModel(%g, %g, %g), %d, %d, %d, %d, %d, %d, %g, %g, %g)',
    [Model.ConstantPart, Model.TakeCoeff, Model.ScanCoeff, Index, MinIndex,
    MaxIndex, TotalCount, BaseCountTakeData, MaxAllowedTake, FillTimeMultiplier,
    EdgeTimeMultiplier, MiddleTimeMultiplier]);
end;
...
begin
  ...
  try
    Result := TdxServerModeOptimalFetchResult.Create(AInternalResult.IsFromEnd,
      AInternalResult.Skip, AInternalResult.Take);
  finally
...
begin
  ...
  try
    ...
    begin
      ADoubleResult := (ATargetTime - AModel.ScanCoeff * AScan -
        AModel.ConstantPart) / AModel.TakeCoeff;
      if IsNan(ADoubleResult) and IsInfinite(ADoubleResult) and
        (High(Integer) < ADoubleResult) then
        AValue := High(Integer)
      else
...
begin
  ...
  try
    ...
    begin
      ADoubleResult := (ATargetTime - AModel.ScanCoeff * ASkip -
        AModel.ConstantPart) / ACoeff;
      if IsNan(ADoubleResult) and IsInfinite(ADoubleResult) and
        (High(Integer) < ADoubleResult) then
        AValue := High(Integer)
      else
...
class function TdxServerModeOptimalFetchHelper.OptimalResultCore
  (AParam: TdxServerModeOptimalFetchParam): TdxSolution;
var
  ...
  I, ABaseSkip, AFillTake, ASkipFromTop, ASkipFromBottom, AMaxTake,
    AScanToMiddleFromTop, AScanToMiddleFromBottom, AScanToMiddle, ATake, ASkip,
    AScan, ALastIndex, AFirstIndex: Integer;
begin
  ABaseSkip :=
    Trunc(Math.Min(Math.Max(0, AParam.Index - AParam.BaseCountTakeData / 2),
    Math.Max(0, AParam.TotalCount - AParam.Index - 1 -
    AParam.BaseCountTakeData / 2)));
  ABaseTime := AParam.Model.ConstantPart + AParam.Model.TakeCoeff *
    AParam.BaseCountTakeData + AParam.Model.ScanCoeff *
    (ABaseSkip + AParam.BaseCountTakeData);
  ...
  begin
    ...
    begin
      ASolution := TdxSolution.FromSkipTake(AParam, False, ASkipFromTop,
        AFillTake);
    ...
    begin
      ASolution := TdxSolution.FromSkipTake(AParam, True, ASkipFromBottom,
        AFillTake);
      if ATargetFillTime >= ASolution.time then
      begin
        Result := ASolution;
  ...
  try
    ...
    if CalculateTakeFromFixedTimeAndSkip(AParam.Model, AEdgeTime, ASkip, ATake)
    then
    begin
    ...
    if CalculateTakeFromFixedTimeAndScan(AParam.Model, AEdgeTime, AScan, ATake)
    then
    begin
      if ATake > AMaxTake then
        ATake := AMaxTake;
    ...
    begin
      ...
      if (ABestSolution = nil) or (ASolution.Take > ABestSolution.Take) or
        ((ASolution.Take = ABestSolution.Take) and
        (ASolution.Scan < ABestSolution.Scan)) then
  ...
  if not IsZero(AParam.Model.TakeCoeff + AParam.Model.ScanCoeff / 2) then
    ACalculatedTake := (ATargetTime - AParam.Model.ScanCoeff * AScanToMiddle -
      AParam.Model.ConstantPart) /
      (AParam.Model.TakeCoeff + AParam.Model.ScanCoeff / 2)
  else
...
constructor TdxLinearLeastSquaresArgs.Create(AUnknowns: Integer;
  AData: TdxPackSampleArray);
begin
  inherited Create;
  FUnknowns := AUnknowns;
  FData := AData;
...
constructor TdxServerModeOptimalFetchResult.Create(AIsFromEnd: Boolean;
  ASkip, ATake: Integer);
begin
  inherited Create;
...
constructor TdxSolution.Create(ASkip, ATake: Integer; AIsFromEnd: Boolean;
  AParams: TdxServerModeOptimalFetchParam);
{$IFOPT C+}
var
  I1, I2: Integer;
...
begin
  ...
  Assert(ASkip + ATake <= AParams.TotalCount,
    Format('skip(%d) + take(%d) > TotalCount(%d)',
    [ASkip, ATake, AParams.TotalCount]));
  FSkip := ASkip;
  FTake := ATake;
  FIsFromEnd := AIsFromEnd;
  FParams := AParams;
...
function TdxSolution.FromI1I2(AParams: TdxServerModeOptimalFetchParam;
  AIsFromEnd: Boolean; I1, I2: Integer): TdxSolution;
begin
  if IsFromEnd then
    Result := FromSkipTake(AParams, AIsFromEnd, AParams.TotalCount - I1 - 1,
      I1 - I2 + 1)
  else
    Result := FromSkipTake(AParams, AIsFromEnd, I1, I2 - I1 + 1);
end;

class function TdxSolution.FromSkipTake(AParams: TdxServerModeOptimalFetchParam;
  AIsFromEnd: Boolean; ASkip, ATake: Integer): TdxSolution;
begin
...
begin
  Result := FParams.Model.ScanCoeff * Scan + FParams.Model.TakeCoeff * Take +
    FParams.Model.ConstantPart;
end;
