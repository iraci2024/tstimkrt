...
uses
  Forms, Classes, SysUtils, cxConverterFactory, cxGridConverter,
  cxGridCustomView,
  cxGridDBTableView, cxGridDBBandedTableView, cxGridBandedTableView, DB,
  Controls,
  cxCustomData, cxGraphics, cxGridCustomTableView, cxFilter, cxGrid,
  cxLookAndFeels,
  cxEdit, cxBlobEdit, cxButtonEdit, cxCalc, cxCalendar, cxCheckBox,
  cxCurrencyEdit,
  cxDropDownEdit, cxHyperLinkEdit, cxImage, cxImageComboBox, cxDBLookupComboBox,
  cxMaskEdit, cxMemo, cxMRUEdit, cxSpinEdit, cxTimeEdit, cxTextEdit, StdCtrls,
  Graphics,
...
type
  ...
  protected
    ...
    procedure AssignColumn(AColumn: TcxGridColumn; ADXColumn: TObject;
      AIndex: Integer); virtual;
    procedure AssignDXOptionsView(AList: TStringList); virtual;
    procedure AssignGrid; virtual;
    procedure AssignSummaryItem(AColumn: TcxGridColumn;
      ADXColumn: TObject); virtual;
    function CreateColumn: TcxGridColumn; virtual;
    function ConvertColumnShowEditButtons(const AStyle: string)
    ...
    procedure ImportPropertiesBlobEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesButtonEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesCalcEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesCheckBox(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesComboBox(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesCurrencyEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesDateEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesHyperLinkEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesImage(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesImageComboBox(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesLookupComboBox(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesExLookupComboBox(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesMaskEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesMemo(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesMRUEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesPopupEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesSpinEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesTextEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportPropertiesTimeEdit(ADXColumn: TObject;
      AcxColumn: TcxGridColumn);
    procedure ImportSummaryGroups; virtual;
    procedure ImportSyncMode(const AValue: Boolean); virtual;
    function TestPreviewFieldName(AIndex: Integer; const APreview: string)
      : Boolean; virtual;
  ...
  private
    ...
    function GetCXColumnByFieldName(const AFieldName: string)
...
procedure TcxRealConverterToTableView.AssignColumn(AColumn: TcxGridColumn;
...
begin
  ...
  begin
    DataBinding.FieldName := Converter.GetStringProperty(ADXColumn, 'FieldName',
      'Field' + IntToStr(AIndex));
    ...
    GroupIndex := Converter.GetIntegerProperty(ADXColumn, 'GroupIndex',
      GroupIndex);
    ...
    Options.Moving := not Converter.GetBooleanProperty(ADXColumn,
      'DisableDragging', Options.Moving);
    Options.Editing := not Converter.GetBooleanProperty(ADXColumn,
      'DisableEditor', Options.Editing);
    Options.Grouping := not Converter.GetBooleanProperty(ADXColumn,
      'DisableGrouping', Options.Grouping);
    Options.Focusing := True;
    Options.Filtering := cxGridView.DataController.Filter.Active;
    Options.HorzSizing := Converter.GetBooleanProperty(ADXColumn, 'Sizing',
      Options.HorzSizing);
    HeaderAlignmentHorz := ConvertAlignment(Converter.GetEnumProperty(ADXColumn,
      'HeaderAlignment'));
...
begin
  ...
  begin
    ...
    OptionsView.Header := Converter.GetBooleanProperty(nil, 'ShowHeader',
      OptionsView.Header);
    OptionsView.NewItemRow := Converter.GetBooleanProperty(nil,
      'ShowNewItemRow', OptionsView.NewItemRow);
    OptionsView.Footer := Converter.GetBooleanProperty(nil, 'ShowSummaryFooter',
      OptionsView.Footer);
    OptionsView.IndicatorWidth := Converter.GetIntegerProperty(nil,
      'GrIndicatorWidth', OptionsView.IndicatorWidth);
    OptionsSelection.HideSelection := Converter.GetBooleanProperty(nil,
      'HideSelection', OptionsSelection.HideSelection);
    Preview.LeftIndent := Converter.GetIntegerProperty(nil, 'IndentDesc',
      Preview.LeftIndent);
    ...
    OptionsView.GroupByBox := Converter.GetBooleanProperty(nil,
      'ShowGroupPanel', OptionsView.GroupByBox);
  end;
end;

procedure TcxRealConverterToTableView.AssignSummaryItem(AColumn: TcxGridColumn;
  ADXColumn: TObject);
var
  ASummaryFooterType: string;
  AcxSummaryFooterItem: TcxGridDBTableSummaryItem;
begin
  ASummaryFooterType := Converter.GetEnumProperty(ADXColumn,
    'SummaryFooterType', 'cstNone');
  if ASummaryFooterType <> 'cstNone' then
  begin
    AcxSummaryFooterItem := TcxGridDBTableSummaryItem
      (TcxGridDBTableView(cxGridView)
      .DataController.Summary.FooterSummaryItems.Add);
    AcxSummaryFooterItem.FieldName := Converter.GetStringProperty(ADXColumn,
      'SummaryFooterField', '');
    AcxSummaryFooterItem.Format := Converter.GetStringProperty(ADXColumn,
      'SummaryFooterFormat', AcxSummaryFooterItem.Format);
...
procedure TcxRealConverterToTableView.ImportAutoDataSetFilter
  (const AValue: Boolean);
begin
  TcxGridDBTableView(cxGridView).DataController.Filter.AutoDataSetFilter
    := AValue;
end;

procedure TcxRealConverterToTableView.ImportColumn(ADXColumn: TObject;
  AcxColumn: TcxGridColumn);
begin
...
begin
  ...
  try
    ...
    for I := 0 to AColumns.Count - 1 do
    begin
      AColumn := GetCXColumnByFieldName(Converter.GetStringProperty(AColumns[I],
        'FieldName', ''));
      AColor := Converter.GetIntegerProperty(TComponent(AColumns[I]), 'Color');
      AFont := Converter.GetClassProperty(TComponent(AColumns[I]),
        'Font') as TFont;
...
begin
  ...
  try
    ...
    begin
      ...
      if AColumn = nil then
        Continue;
      ASummaryField := Converter.GetStringProperty(TComponent(AColumns[I]),
        'SummaryField', '');
      ASummaryFormat := Converter.GetStringProperty(TComponent(AColumns[I]),
        'SummaryFormat', '');
      ASummaryType := Converter.GetEnumProperty(TComponent(AColumns[I]),
        'SummaryType');
      ASummaryGroupName := Converter.GetStringProperty(TComponent(AColumns[I]),
        'SummaryGroupName', '');
      ...
      begin
        ...
        begin
          ASummaryGroup := TcxDataSummaryGroup
            (FcxSummaryGroups.Objects[AIndex]);
      ...
      begin
        ...
        ASummaryItem := TcxGridDBTableSummaryItem
          (ASummaryGroup.SummaryItems.Add);
...
begin
  ...
  if AFilter <> nil then
  begin
    cxGridView.Filtering.DropDownWidth := Converter.GetIntegerProperty(AFilter,
      'DropDownWidth', cxGridView.Filtering.DropDownWidth);
    cxGridView.DataController.Filter.MaxValueListCount :=
      Converter.GetIntegerProperty(AFilter, 'MaxDropDownCount',
      cxGridView.DataController.Filter.MaxValueListCount);
    cxGridView.Filtering.MaxDropDownCount := Converter.GetIntegerProperty
      (AFilter, 'DropDownCount', cxGridView.Filtering.MaxDropDownCount);
    ...
    cxGridView.DataController.Filter.Active := Converter.GetBooleanProperty
      (AFilter, 'Active', cxGridView.DataController.Filter.Active);
    ImportAutoDataSetFilter(Converter.GetBooleanProperty(AFilter,
  ...
  try
    ...
    try
      ...
      begin
        ...
        OptionsBehavior.GoToNextCellOnEnter :=
          AList.Find('edgoEnterThrough', I);
    ...
    try
      ...
      begin
        ...
        OptionsBehavior.FocusFirstCellOnNewRecord :=
          AList.Find('edgoResetColumnFocus', I);
...
begin
  ...
  AGroupNodeTextColor := Converter.GetIntegerProperty(nil,
    'GroupNodeTextColor');
  ...
  AGroupPanelFontColor := Converter.GetIntegerProperty(nil,
    'GroupPanelFontColor');
  if (AGroupPanelColor <> clBtnShadow) or (AGroupPanelFontColor <> clBtnFace)
  then
  begin
    AStyle := Converter.CreateStyleItem;
  ...
  AAutoSearchTextColor := Converter.GetIntegerProperty(nil,
    'AutoSearchTextColor');
  ...
  if AGridLineColor <> clNone then
    cxGridView.OptionsView.GridLineColor := Converter.GetIntegerProperty(nil,
      'GridLineColor');
  AHighlightColor := Converter.GetIntegerProperty(nil, 'HighlightColor');
  AHighlightTextColor := Converter.GetIntegerProperty(nil,
    'HighlightTextColor');
  if (AHighlightColor <> clHighlight) or (AHighlightTextColor <> clHighlightText)
  then
  begin
    AStyle := Converter.CreateStyleItem;
  ...
  AHideSelectionColor := Converter.GetIntegerProperty(nil,
    'HideSelectionColor');
  AHideSelectionTextColor := Converter.GetIntegerProperty(nil,
    'HideSelectionTextColor');
  if (AHideSelectionColor <> clHighlight) or
    (AHideSelectionTextColor <> clHighlightText) then
  begin
    AStyle := Converter.CreateStyleItem;
...
begin
  ...
  cxGridView.Preview.MaxLineCount := Converter.GetIntegerProperty(nil,
    'PreviewLines', cxGridView.Preview.MaxLineCount);
end;
...
begin
  ...
  AConverter := TcxHyperLinkEditPropertiesConverter.Create
    (AcxColumn.Properties);
  try
    AConverter.ImportFrom(ADXColumn);
  finally
    AConverter.Free;
...
procedure TcxRealConverterToTableView.ImportPropertiesImage(ADXColumn: TObject;
  AcxColumn: TcxGridColumn);
var
  AConverter: TcxCustomPropertiesConverter;
begin
...
begin
  ...
  AConverter := TcxImageComboBoxPropertiesConverter.Create
    (AcxColumn.Properties);
  try
    AConverter.ImportFrom(ADXColumn);
    AcxColumn.Options.ShowEditButtons := ConvertColumnShowEditButtons
...
begin
  ...
  AConverter := TcxLookupComboBoxPropertiesConverter.Create
    (AcxColumn.Properties);
  try
    AConverter.ImportFrom(ADXColumn);
    AcxColumn.Options.ShowEditButtons := ConvertColumnShowEditButtons
...
procedure TcxRealConverterToTableView.ImportPropertiesMemo(ADXColumn: TObject;
  AcxColumn: TcxGridColumn);
var
  AConverter: TcxCustomPropertiesConverter;
begin
  ...
  begin
    if ASummaryGroups is TCollection then
      ...
      begin
        ...
        begin
          ABeginText := Converter.GetStringProperty(Items[I],
            'BeginSummaryText', '');
          AEndText := Converter.GetStringProperty(Items[I],
            'EndSummaryText', '');
          AIsDefaultGroup := Converter.GetBooleanProperty(Items[I],
            'DefaultGroup', True);
          if AIsDefaultGroup then
          begin
            cxGridView.DataController.Summary.DefaultGroupSummaryItems.BeginText
              := ABeginText;
            cxGridView.DataController.Summary.DefaultGroupSummaryItems.EndText
              := AEndText;
            cxGridView.DataController.Summary.DefaultGroupSummaryItems.Separator
              := ASeparator;
            AcxSummaryGroup := nil;
          end
          else
          begin
            AcxSummaryGroup := cxGridView.DataController.Summary.
              SummaryGroups.Add;
          ...
          if AcxSummaryGroup <> nil then
            FcxSummaryGroups.AddObject(Converter.GetStringProperty(Items[I],
              'Name', ''), AcxSummaryGroup);
            ...
            if ASummaryItems is TCollection then
              ...
              begin
                ...
                begin
                  if AIsDefaultGroup then
                    AcxSummaryItem := TcxGridDBTableSummaryItem
                      (cxGridView.DataController.Summary.
                      DefaultGroupSummaryItems.Add)
                  else
                    AcxSummaryItem := TcxGridDBTableSummaryItem
                      (AcxSummaryGroup.SummaryItems.Add);
                  AcxSummaryItem.Column := nil;
                  ADXColumn := GetColumn(Converter.GetStringProperty(Items[J],
                    'ColumnName', ''));
                  if ADXColumn <> nil then
                  begin
                    AFieldName := Converter.GetStringProperty(ADXColumn,
                      'FieldName', '');
                    for K := 0 to cxGridView.ColumnCount - 1 do
                      if TcxGridDBTableView(cxGridView).Columns[K]
                        .DataBinding.FieldName = AFieldName then
                      begin
                        AcxSummaryItem.Column := TcxGridDBTableView(cxGridView)
                          .Columns[K];
                        AcxSummaryItem.Position := spFooter;
                        Break;
                      end;
                  end;
                  AcxSummaryItem.FieldName := Converter.GetStringProperty
                    (Items[J], 'SummaryField', '');
                  AcxSummaryItem.Format := Converter.GetStringProperty(Items[J],
                    'SummaryFormat', AcxSummaryItem.Format);
                  AcxSummaryItem.Kind :=
                    GetSummaryKind(Converter.GetEnumProperty(Items[J],
                    'SummaryType'));
                end;
              end;
          end;
...
begin
  TcxGridDBTableView(cxGridView).DataController.DataModeController.SyncMode
    := AValue;
end;
...
begin
  Result := TcxGridDBTableView(cxGridView).Columns[AIndex]
    .DataBinding.FieldName = APreview;
end;
...
begin
  ...
  with AFont do
    Result := (Pitch = DefFontData.Pitch) and (Style = DefFontData.Style) and
      (Charset = DefFontData.Charset) and
      (Name = dxShortStringToString(DefFontData.Name)) and (Color = clBlue) and
      (Size = 8);
end;
...
begin
  ...
  try
    GetColumns(AColumns);
    for I := 0 to AColumns.Count - 1 do
      if Converter.GetStringProperty(TComponent(AColumns[I]), 'Name', '') = AName
      then
      begin
...
begin
  ...
  begin
    ...
    begin
      if Converter.Source.Components[I].GetParentComponent = Converter.Source
      then
...
function TcxRealConverterToTableView.GetCXColumnByFieldName(const AFieldName
...
begin
  ...
  for I := 0 to cxGridView.ColumnCount - 1 do
  begin
    if TcxGridDBTableView(cxGridView).Columns[I].DataBinding.FieldName = AFieldName
    then
    begin
...
function TcxRealConverterToTableView.GetSummaryKind(const AType: string)
  : TcxSummaryKind;
begin
...
begin
  ...
  begin
    Position.BandIndex := Converter.GetIntegerProperty(ADXColumn, 'BandIndex',
      Position.BandIndex);
    Position.ColIndex := Converter.GetIntegerProperty(ADXColumn, 'ColIndex',
      Position.ColIndex);
    Options.VertSizing := Converter.GetBooleanProperty(ADXColumn, 'Sizing',
      Options.VertSizing);
    Position.RowIndex := Converter.GetIntegerProperty(ADXColumn, 'RowIndex',
      Position.RowIndex);
  end;
  inherited AssignColumn(AColumn, ADXColumn, AIndex);
end;

procedure TcxRealConverterToBandedTableView.ImportAutoDataSetFilter
  (const AValue: Boolean);
begin
  TcxGridDBBandedTableView(cxGridView).DataController.Filter.AutoDataSetFilter
    := AValue;
end;
...
procedure TcxRealConverterToBandedTableView.ImportColumnSummaryGroups;
  ...
  begin
    ...
    for I := 0 to cxGridView.ColumnCount - 1 do
    begin
      if TcxGridDBBandedTableView(cxGridView).Columns[I].DataBinding.FieldName = AFieldName
      then
      begin
...
procedure TcxRealConverterToBandedTableView.AssignDXOptionsView
  (AList: TStringList);
var
  I: Integer;
begin
...
begin
  with cxGridView do
  begin
    OptionsView.BandHeaders := Converter.GetBooleanProperty(nil, 'ShowBands',
      OptionsView.BandHeaders);
    OptionsView.FixedBandSeparatorWidth := Converter.GetIntegerProperty(nil,
      'FixedBandLineWidth', OptionsView.FixedBandSeparatorWidth);
  end;
  inherited AssignGrid;
end;

procedure TcxRealConverterToBandedTableView.AssignSummaryItem
  (AColumn: TcxGridColumn; ADXColumn: TObject);
var
  ASummaryFooterType: string;
  AcxSummaryFooterItem: TcxGridDBTableSummaryItem;
begin
  ...
  begin
    AcxSummaryFooterItem := TcxGridDBTableSummaryItem
      (TcxGridDBBandedTableView(cxGridView)
      .DataController.Summary.FooterSummaryItems.Add);
    AcxSummaryFooterItem.FieldName := Converter.GetStringProperty(ADXColumn,
      'SummaryFooterField', '');
    AcxSummaryFooterItem.Format := Converter.GetStringProperty(ADXColumn,
...
begin
      ...
      begin
        ...
        begin
          ...
          if FBandHeaderWidth then
            ...
            AcxGridBand.Width := Converter.GetIntegerProperty(Items[I], 'Width',
              AcxGridBand.Width);
          AcxGridBand.Caption := Converter.GetStringProperty(Items[I],
            'Caption', AcxGridBand.Caption);
          AcxGridBand.Alignment :=
            ConvertAlignment(Converter.GetEnumProperty(Items[I], 'Alignment'));
          AcxGridBand.Visible := Converter.GetBooleanProperty(Items[I],
            'Visible', AcxGridBand.Visible);
          AcxGridBand.Options.Sizing := Converter.GetBooleanProperty(Items[I],
            'Sizing', AcxGridBand.Options.Sizing);
          AcxGridBand.Options.Moving := not Converter.GetBooleanProperty
            (Items[I], 'DisableDragging', AcxGridBand.Options.Moving);
  ...
  cxGridView.OptionsView.BandHeaders := Converter.GetBooleanProperty(nil,
    'ShowBands', cxGridView.OptionsView.BandHeaders);
end;
                ...
                begin
                  ...
                  begin
                    AFieldName := Converter.GetStringProperty(ADXColumn,
                      'FieldName', '');
                    for K := 0 to cxGridView.ColumnCount - 1 do
                      if TcxGridDBBandedTableView(cxGridView).Columns[K]
                        .DataBinding.FieldName = AFieldName then
                      begin
                        AcxSummaryItem.Column :=
                          TcxGridDBBandedTableView(cxGridView).Columns[K];
                        AcxSummaryItem.Position := spFooter;
                        Break;
                      end;
                  end;
...
procedure TcxRealConverterToBandedTableView.ImportSyncMode
  (const AValue: Boolean);
begin
  TcxGridDBBandedTableView(cxGridView).DataController.DataModeController.
    SyncMode := AValue;
end;

function TcxRealConverterToBandedTableView.TestPreviewFieldName(AIndex: Integer;
  const APreview: string): Boolean;
begin
  Result := TcxGridDBBandedTableView(cxGridView).Columns[AIndex]
    .DataBinding.FieldName = APreview;
end;
...
function TcxRealConverterToBandedTableView.GetcxGridView
...
procedure TcxDXGridLayoutDataReader.SetName(Component: TComponent;
  var Name: string);
begin
  Name := '';
end;
...
ConverterFactory(cxGridGroupConverterName).RegisterConverter
  ('DX Grid Converter', TcxDXGridConverter);
ConverterFactory(cxGridGroupConverterName).RegisterConverter
  ('DX Grid Layout Converter', TcxDXGridLayoutConverter);
