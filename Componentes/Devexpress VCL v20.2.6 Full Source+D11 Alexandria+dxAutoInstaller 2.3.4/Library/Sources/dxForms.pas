...
uses
  ...
  dxCore, dxMessages, cxGeometry, dxCoreGraphics, cxClasses, cxGraphics,
  cxLookAndFeels;
...
{$ENDIF}
  ...
  protected
    ...
    {$IFDEF DELPHI101BERLIN}override; {$ELSE}reintroduce; overload; virtual;
    {$ENDIF}
...
{$ENDIF}
  ...
  public
    ...
    procedure ScaleForPPI(ATargetDPI: Integer; AWindowRect: PRect); reintroduce;
      overload; virtual;
    procedure ScaleForPPI(ATargetDPI: Integer); overload;
    {$IFDEF DELPHI101BERLIN}override; final; {$ENDIF}
{$IFNDEF DELPHI101BERLIN}
...
{$ELSE}
    procedure ChangeScale(M, D: Integer; IsDpiChange: Boolean); reintroduce;
      overload; virtual;
{$ENDIF}
    procedure CreateParams(var Params: TCreateParams); override;
    procedure CreateWnd; override;
...
{$ENDIF}
    class procedure PopulateControls(AControl: TControl;
      ATargetList: TComponentList);
  protected
  ...
  public
    ...
    class procedure SetWindowTextWithoutRedrawing(AWndHandle: THandle;
      const AText: string);
    // Redrawing
    class function LockWindowRedrawing(AWndHandle: THandle): Cardinal;
    class procedure UnlockWindowRedrawing(AWndHandle: THandle;
      APrevStyle: Cardinal);
    // Scaling
    class function IsScaleChanging(AContainer: TWinControl): Boolean;
    class procedure RepaintVisibleWindowArea(AHandle: THandle;
      var APrevVisibleRegion: TcxRegionHandle);
    ...
    class procedure ScaleForPPI(Form: TCustomForm; TargetDPI: Integer;
      WindowRect: PRect; FromMessageHandler: Boolean);
    //
...
uses
  UxTheme, DwmApi, TypInfo, Menus, dxDPIAwareUtils, cxControls, dxHooks,
  dxCoreClasses, dxThreading;
...
{$ENDIF}
  ...
  protected
    ...
    procedure ScaleForPPI(ATargetPPI: Integer;
      ACanScaleContent: Boolean = True);
  ...
  strict private
    ...
    function GenerateName(const ABaseName: TComponentName; ATargetDPI: Integer)
      : TComponentName;
  ...
  protected
    procedure UpdateImageList(AInstance: TObject; APropInfo: PPropInfo;
      APropValue: TObject);
...
procedure dxFrameWndProcHook(ACode: Integer; wParam: wParam; lParam: lParam;
...
procedure TdxFrameController.ScaleForPPI(ATargetPPI: Integer;
  ACanScaleContent: Boolean = True);
var
{$IFNDEF DELPHI101BERLIN}
...
begin
  FMap := TObjectDictionary<TWinControl, TdxFrameController>.Create
    ([doOwnsValues]);
...
begin
  ...
  begin
    ...
    if (AControl is TFrame) and not(Supports(AControl, IdxScaleFactor) or
      FMap.ContainsKey(AControl)) then
    begin
...
class procedure TdxFrameControllerManager.FreeNotificationHandler
  (Sender: TComponent);
begin
  if FMap <> nil then
...
begin
  TdxFormHelper.ScaleForPPI(TCustomFormAccess(Self), ATargetDPI,
    AWindowRect, False);
end;
...
begin
  Result := (FRightToLeftLayout = bTrue) or (FRightToLeftLayout = bDefault) and
...
begin
  ...
  begin
    ...
    TdxFormHelper.ScaleForPPI(Self, LoWord(Message.wParam),
      PRect(Message.lParam), True);
end;
...
constructor TdxCustomFloatForm.CreateNew(AOwner: TComponent;
  Dummy: Integer = 0);
begin
  inherited;
...
begin
{$IFNDEF DELPHIXE8}
  Filer.DefineProperty('TextHeight', ReadTextHeight, WriteTextHeight,
    not IsControl);
{$ENDIF}
...
begin
  Result := not(csDesigning in ComponentState) and
    ((FRightToLeftLayout = bTrue) or (FRightToLeftLayout = bDefault) and
    UseRightToLeftAlignment);
end;
...
begin
  inherited;
  if Assigned(FEnableNonClientDpiScaling) and Scaled and dxIsProcessDPIAware
  then
...
class procedure TdxFormHelper.SetWindowTextWithoutRedrawing(AWndHandle: THandle;
  const AText: string);
var
  AStyle: Cardinal;
begin
...
class procedure TdxFormHelper.UnlockWindowRedrawing(AWndHandle: THandle;
  APrevStyle: Cardinal);
begin
  dxSetWindowStyle(AWndHandle, APrevStyle, soSet);
end;

class procedure TdxFormHelper.UpdateScaleFactorOnParentChange
  (AForm: TCustomForm);
{$IFNDEF DELPHI101BERLIN}
var
...
{$ELSE}
  ...
  begin
    if not(csLoading in AForm.ComponentState) and
      Supports(AForm, IdxScaleFactor, ASource) then
    begin
      TCustomFormAccess(AForm).PixelsPerInch :=
        MulDiv(TCustomFormAccess(AForm).PixelsPerInch,
...
class procedure TdxFormHelper.RepaintVisibleWindowArea(AHandle: THandle;
  var APrevVisibleRegion: TcxRegionHandle);
  ...
  begin
    ...
    AScreenRegion := CreateRectRgnIndirect
      (GetMonitorWorkArea(MonitorFromWindow(AHandle,
      MONITOR_DEFAULTTONEAREST)));
    CombineRgn(Result, Result, AScreenRegion, RGN_AND);
    OffsetRgn(Result, -cxGetWindowRect(AHandle).Left,
      -cxGetWindowRect(AHandle).Top);
...
begin
  ...
  try
    ...
    begin
      ...
      if CombineRgn(ARegion, APrevVisibleRegion, ARegion, RGN_DIFF) <> NULLREGION
      then
        RedrawWindow(AHandle, nil, ARegion, RDW_ERASE or RDW_INVALIDATE or
...
class procedure TdxFormHelper.ScaleComponentsForDpi(AOwner: TComponent;
  NewPPI: Integer);
var
...
class procedure TdxFormHelper.ScaleForPPI(Form: TCustomForm; TargetDPI: Integer;
  WindowRect: PRect; FromMessageHandler: Boolean);

  function IsClientSizeStored(AForm: TCustomFormAccess): Boolean;
  begin
    Result := not(AForm.AutoScroll or (AForm.HorzScrollBar.Range <> 0) or
...
begin
  ...
  if (TargetDPI <> APrevDPI) and (TargetDPI >= dxMinDPI) and
    not IsScaleChanging(Form) then
  begin
...
{$IFDEF DELPHI101BERLIN}
      if (WindowRect <> nil) and FromMessageHandler then
        AForm.Perform(WM_DPICHANGED, MakeWParam(TargetDPI, TargetDPI),
          lParam(WindowRect))
      else
...
{$ENDIF}
      ...
      else if IsClientSizeStored(AForm) then
      begin
        if AForm.WindowState <> wsMaximized then
        begin
          AForm.SetBounds(APrevBounds.Left, APrevBounds.Top,
            cxRectWidth(APrevBounds) - APrevClientRect.Right +
            MulDiv(APrevClientRect.Right, TargetDPI, APrevDPI),
            cxRectHeight(APrevBounds) - APrevClientRect.Bottom +
            MulDiv(APrevClientRect.Bottom, TargetDPI, APrevDPI));
        end
        else
...
class function TdxFormHelper.ShouldPassDpiChangeMessage
  (AForm: TCustomForm): Boolean;
begin
  Result := IsScaleChanging(AForm) and
    not((FLockedForms <> nil) and (FLockedForms.IndexOf(AForm) >= 0));
end;
...
class function TdxFormHelper.HasMaximizedEmbeddedForms
  (AList: TComponentList): Boolean;
var
...
class procedure TdxFormHelper.PopulateControls(AControl: TControl;
  ATargetList: TComponentList);
var
  I: Integer;
begin
...
class procedure TdxFormHelper.ScalingBegin(AContainer: TWinControl;
  out AState: TState);
var
  I: Integer;
begin
  ...
  AState.RedrawLocked := IsWinVistaOrLater and
    IsWindowVisible(AContainer.Handle);
  AState.LockedControls := TComponentList.Create(False);
  PopulateControls(AContainer, AState.LockedControls);
{$IFDEF DELPHI103}
  AState.RedrawLocked := AState.RedrawLocked and not HasMaximizedEmbeddedForms
    (AState.LockedControls);
{$ENDIF}
...
class procedure TdxFormHelper.ScalingEnd(AContainer: TWinControl;
  AState: TState);
var
  I: Integer;
begin
  ...
  if AState.RedrawLocked then
    cxRedrawWindow(AContainer.Handle, RDW_INVALIDATE or RDW_ALLCHILDREN or
      RDW_ERASE);
...
class procedure TdxFormImageListHelper.Execute(ATargetDPI: Integer;
  AForm: TCustomForm);
begin
...
procedure TdxFormImageListHelper.UpdateImageList(AInstance: TObject;
  APropInfo: PPropInfo; APropValue: TObject);
var
  ANewValue: TObject;
begin
...
procedure TdxFormImageListHelper.UpdateImageListProperties
  (APersistent: TPersistent);

  function EnumProperties(AObject: TObject; out AList: PPropList;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          else if APropClass.InheritsFrom(TPersistent) then
          begin
            APropValue := GetObjectProp(APersistent, APropInfo);
            if APropValue <> nil then
...
function TdxFormImageListHelper.GenerateName(const ABaseName: TComponentName;
  ATargetDPI: Integer): TComponentName;
begin
  Result := Format('%s%d', [ABaseName, ATargetDPI]);
end;

function TdxFormImageListHelper.GetBaseImageListName
  (const AName: TComponentName): TComponentName;
var
  ALength: Integer;
begin
  Result := AName;
...
function TdxFormImageListHelper.GetReplacement(AImageList: TCustomImageList)
  : TCustomImageList;

  function CheckReference(const AReference: TComponent;
    var AResult: TCustomImageList): Boolean;
  begin
  ...
  function TryFind(const ABaseName: TComponentName; ATargetDPI: Integer;
    var AResult: TCustomImageList): Boolean;
  begin
    Result := CheckReference(AImageList.Owner.FindComponent
      (GenerateName(ABaseName, ATargetDPI)), AResult);
...
begin
  ...
  begin
    if not TryFind(ABaseName, FTargetDPI, Result) then
      ...
      begin
        if (dxDefaultDPIValues[I] < FTargetDPI) and
          TryFind(ABaseName, dxDefaultDPIValues[I], Result) then
          Break;
      end;
  end;
end;
...
class procedure TdxFormRightToLeftLayoutHelper.AddRightToLeftFormExStyle
  (var Params: TCreateParams);
begin
  Params.ExStyle := (Params.ExStyle or GetRightToLeftLayoutStyles) and
    not GetBiDiModeStyles;
end;

class procedure TdxFormRightToLeftLayoutHelper.RemoveBiDiModeStyles
  (AForm: TCustomForm);
begin
...
class procedure TdxFormRightToLeftLayoutHelper.SetMenusParentBiDiMode
  (AForm: TCustomForm);
var
  I: Integer;
begin
...
class procedure TdxFormRightToLeftLayoutHelper.ResetMenusBiDiMode
  (AForm: TCustomForm);
var
  I: Integer;
begin
...
class function TdxFormRightToLeftLayoutHelper.GetRightToLeftLayoutStyles
  : Cardinal;
begin
...
@FEnableNonClientDpiScaling := GetProcAddress(GetModuleHandle(user32),
