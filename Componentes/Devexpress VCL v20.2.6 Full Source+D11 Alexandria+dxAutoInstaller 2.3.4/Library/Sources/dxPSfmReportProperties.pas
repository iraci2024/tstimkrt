...
uses
  ...
  Forms, dxPSCore, dxPSForm, dxExtCtrls,
  Menus, cxLookAndFeelPainters, cxButtons, cxControls, cxContainer, cxEdit,
  cxLabel, cxTextEdit, cxMemo, cxPC,
  cxGeometry, dxCore, cxGraphics, cxLookAndFeels, dxLayoutLookAndFeels,
...
type
  ...
  public
    ...
    property ReportCreationDateTime: TDateTime read GetReportCreationDateTime
      write SetReportCreationDateTime;
    property ReportCreator: string read GetReportCreator write SetReportCreator;
    property ReportDescription: string read GetReportDescription
      write SetReportDescription;
...
uses
  Themes, Messages, SysUtils, Dialogs, ExtDlgs, cxFormats, dxPSGlbl, dxPSUtl,
  dxPSRes, dxPcPrVW;
...
begin
  ...
  tshDescription.Caption :=
    DropColon(DropAmpersand(cxGetResourceString(@sdxDescription)));
...
procedure TdxfmPSReportProperties.SetReportCreationDateTime
...
begin
  ...
  GetDateFormat(DefaultLCID, 0, @SystemTime, PChar(DateFormat), @Buffer,
    SizeOf(Buffer));
  edCreationDate.Text := Buffer;

  TimeFormat := GetLocaleStr(DefaultLCID, LOCALE_STIMEFORMAT,
    dxFormatSettings.LongTimeFormat);
  GetTimeFormat(DefaultLCID, 0, @SystemTime, PChar(TimeFormat), @Buffer,
    SizeOf(Buffer));
...
begin
  CanClose := (ModalResult = mrCancel) or ExplorerItem.IsCurrentlyLoaded or
    not ExplorerItem.IsNameChanged(ReportCaption) or
    ExplorerItem.CanRenameTo(ReportCaption);
  if not CanClose then
  begin
    dxPSUtl.MessageError(ExplorerItem.CannotRenameMessageText
      (ExplorerItem.DisplayName, ReportCaption));
