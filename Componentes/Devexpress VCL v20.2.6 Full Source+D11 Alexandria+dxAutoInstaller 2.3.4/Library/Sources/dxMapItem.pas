...
type
  ...
  TdxMapItemStyles = array [Low(TdxMapControlElementState)
    .. High(TdxMapControlElementState)] of TdxMapItemStyle;
  ...
  public
    ...
    property Style[AState: TdxMapControlElementState]: TdxMapItemStyle
  ...
  protected
    ...
    function GetTextColor(AState: TdxMapControlElementState)
      : TdxAlphaColor; virtual;
    function GetTextGlowColor(AState: TdxMapControlElementState)
      : TdxAlphaColor; virtual;
  ...
  public
    ...
    property Values[const AName: string]: Variant read GetValues
  ...
  published
    ...
    property Visibility: TdxMapItemTitleVisibility read FVisibility
      write SetVisibility default mitvAuto;
  end;
  ...
  public
    constructor Create(AMapItemViewInfo: TdxMapItemViewInfo);
      reintroduce; virtual;
    destructor Destroy; override;
  end;
  ...
  private
    ...
    procedure StyleChanged(ASender: TObject);
    procedure SetStyle(const Index: TdxMapControlElementState;
      const Value: TdxMapItemStyle);
  ...
  protected
    ...
    property TitleOptions: TdxMapItemTitleOptions read FTitleOptions
      write SetTitleOptions;
  public
  ...
  published
    ...
    property Style: TdxMapItemStyle index mcesNormal read GetStyle
      write SetStyle;
    property StyleHot: TdxMapItemStyle index mcesHot read GetStyle
      write SetStyle;
    property StyleSelected: TdxMapItemStyle index mcesSelected read GetStyle
      write SetStyle;
  ...
  public
    ...
    property Items[Index: Integer]: TdxMapItem read GetItem
  ...
  protected
    ...
    property Location: TdxMapControlGeoLocation read FLocation
      write SetLocation;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  ...
  published
    property Location;
    property ShapeKind: TdxMapDotShapeKind read FShapeKind write SetShapeKind
      default mcskCircle;
  ...
  public
    ...
    property Items[Index: Integer]: TdxMapControlGeoPointItem read GetItem
      write SetItem; default;
    property OnChanged: TNotifyEvent read FOnChanged write FOnChanged;
    // for internal use
  end;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property GeoPoints: TdxMapControlGeoPointCollection read FGeoPoints
      write SetGeoPoints;
  end;
  ...
  public
    ...
    property Items[Index: Integer]: TdxMapPathSegment read GetItem
      write SetItem; default;
    property OnChanged: TNotifyEvent read FOnChanged write FOnChanged;
    // for internal use
  end;

  TdxMapPathCustomViewInfo<T: TdxMapCustomMultiPointItemViewInfo> = class
    (TdxMapItemViewInfo) // for internal use
  private
  ...
  TdxMapPathPolygonViewInfo = class
    (TdxMapPathCustomViewInfo<TdxMapPathSegmentPolygonViewInfo>)
  // for internal use
  private
  ...
  TdxMapPathPolylineViewInfo = class
    (TdxMapPathCustomViewInfo<TdxMapPathSegmentPolylineViewInfo>)
  // for internal use
  protected
    procedure DoPaint; override;
    function PtInElement(const APoint: TPoint): Boolean; override;
  ...
  published
    ...
    property SegmentType: TdxMapSegmentType read FSegmentType
      write SetSegmentType default mstPolygon;
    property TitleOptions;
  end;
  ...
  protected
    ...
    property ImageOrigin: TdxPointDoublePersistent read FImageOrigin
      write SetImageOrigin stored IsImageOriginStored;
  ...
  public
    class function CalculateMapBounds(AItems: TdxMapItemList;
      out ARect: TdxRectDouble): Boolean;
  end;

implementation

uses
  dxCustomMapItemLayer, dxMapControlViewInfo, dxMapControl, dxSmartImage,
  dxDPIAwareUtils;
...
procedure GeoPointsToMapPoints(ALayer: TdxMapLayer;
  AGeoPoints: TdxMapControlGeoPointCollection; var AMapPoints: TdxDoublePoints;
  var AMapBoundingBox: TdxRectDouble; AEnshureClosed: Boolean);
var
  I: Integer;
...
begin
  ...
  for I := 0 to MapPath.Segments.Count - 1 do
    ...
    begin
      AItem := MapPath.Segments[I]
        .ViewInfo as TdxMapCustomMultiPointItemViewInfo;
...
begin
  ...
  FRelativeBounds := cxRectInflate(FRelativeBounds,
    LookAndFeelPainter.MapControlMapCustomElementSelectionOffset(ScaleFactor));
end;
...
begin
  CapturePaintCanvas;
  try
    LookAndFeelPainter.DrawMapCustomElementBackground(cxPaintCanvas,
      Bounds, State);
  finally
    ReleasePaintCanvas;
  end;
...
constructor TdxMapPathSegmentPolygonViewInfo.Create(AOwner: TdxMapPathSegment);
begin
...
function TdxMapPathSegmentPolygonViewInfo.GetGeoPoints
...
begin
  ...
  try
    ...
    begin
      AStyle := Style;
      dxGPPaintCanvas.Path(APath, AStyle.BorderColor, AStyle.Color,
        ScaleFactor.Apply(AStyle.BorderWidth));
    end;
  finally
    APath.Free;
  end;
...
function TdxMapPolygonViewInfo.InternalPtInElement(const APoint
  : TPoint): Boolean;
var
  APath: TdxGpPath;
begin
...
begin
  Result := inherited PtInElement(APoint) and InternalPtInElement(APoint);
end;
...
begin
  ...
  for I := 0 to MapPath.Segments.Count - 1 do
  begin
    AViewInfo := MapPath.Segments[I]
      .ViewInfo as TdxMapPathSegmentPolygonViewInfo;
...
function TdxMapPathSegmentPolylineViewInfo.GetGeoPoints
  : TdxMapControlGeoPointCollection;
begin
  Result := Segment.GeoPoints;
end;
...
begin
  ...
  try
    ...
    FGeoPoints := TdxMapControlGeoPointCollection.Create
...
procedure TdxMapPathSegment.SetGeoPoints(const Value
  : TdxMapControlGeoPointCollection);
begin
...
begin
  Result := cxPointOffset(RelativeImageRect.TopLeft,
    LookAndFeelPainter.MapControlGetMapPushpinTextOrigin(ScaleFactor));
  Result := cxPointOffset(Result, cxRectCenter(TextInfo.ClientBounds), False);
end;

function TdxMapPushpinViewInfo.GetTextColor(AState: TdxMapControlElementState)
  : TdxAlphaColor;
begin
  Result := LookAndFeelPainter.MapControlMapPushpinTextColor;
end;

function TdxMapPushpinViewInfo.GetTextGlowColor
...
begin
  FRelativeTextRect := cxRectSetOrigin(TextInfo.ClientBounds,
    DoCalculateTextOffset);
end;
...
var
  ALocation: TPoint;
begin
  ALocation := dxPointDoubleToPoint(MapPointer.Layer.MapUnitToScreenPoint
    (FMapLocation));
...
begin
  FMapLocation := MapPointer.Layer.GeoPointToMapUnit
    (MapPointer.Location.GeoPoint);
  ...
  begin
    if IsImageAssigned(MapPointer.Image) then
      ...
      FRelativeImageRect :=
        cxRect(LookAndFeelPainter.MapControlGetMapPushpinSize(ScaleFactor));
    FRelativeImageRect := cxRectOffset(FRelativeImageRect,
      dxPointDoubleToPoint(dxPointDoubleScale(GetImageOrigin,
      dxPointDouble(cxRectWidth(FRelativeImageRect),
      cxRectHeight(FRelativeImageRect)))), False);
  end;
  ...
  if IsImageVisible then
    ...
    begin
      if CheckCachedImageCreated(dxGPPaintCanvas) then
        GdipDrawCachedBitmap(dxGPPaintCanvas.Handle, FCachedImage,
          FImageRect.Left, FImageRect.Top)
      else
...
function TdxMapPointerViewInfo.CheckCachedImageCreated
  (AGpCanvas: TdxGPCanvas): Boolean;
  ...
  begin
    ...
    try
      ...
      Result := GdipCreateCachedBitmap(AImage.Handle, AGpCanvas.Handle,
        FCachedImage) = Ok;
...
begin
  Result := IsWinSevenOrLater and not Supports(MapPointer.Image, IdxVectorImage)
    and (FIsCachedImageValid or CheckRecreated);
end;
...
constructor TdxMapItemTextViewInfo.Create(AMapItemViewInfo: TdxMapItemViewInfo);
begin
  inherited Create;
...
begin
  ...
  begin
    ...
    try
      ...
      try
        dxGPGetTextRect(AGpCanvas, FText, AFont, False, cxNullRect,
          FClientBounds);
      finally
...
procedure TdxMapItemTextViewInfo.DoPaint;
  ...
  begin
    ...
    try
      ...
      AStyle.Font.Height := FMapItemViewInfo.ScaleFactor.Apply
        (AStyle.Font.Height);
      try
        dxGPDrawGlowText(AGpImageCanvas, FText, FClientBounds, AStyle.Font,
          AStyle.TextColor, AStyle.TextGlowColor);
      finally
        AStyle.Font.Height := FMapItemViewInfo.ScaleFactor.Revert
          (AStyle.Font.Height);
      end;
    finally
      AGpImageCanvas.Free;
...
procedure TdxMapSinglePointItem.SetLocation(Value: TdxMapControlGeoLocation);
begin
...
begin
  ...
  AIsOffsetOnly := dxPointDoubleIsEqual(FCachedRenderScale,
    Item.Layer.RenderScale);
  if AIsOffsetOnly then
    for I := 0 to High(FScreenPoints) do
    begin
      FScreenPoints[I] := dxPointDoubleToPoint
        (dxPointDoubleOffset(dxPointDouble(FScreenPoints[I]),
        dxPointDoubleOffset(FCachedRenderOffset,
        Item.Layer.RenderOffset, False)));
  ...
  begin
    ...
    for I := 0 to High(FScreenPoints) do
    begin
      APoint := dxPointDoubleToPoint
        (Item.Layer.MapUnitToScreenPoint(FMapPoints[I]));
      if (ACount = 0) or not cxPointIsEqual(APoint, FScreenPoints[ACount - 1])
      then
      begin
  ...
  begin
    ...
    dxClipPolylineByRect(FScreenPoints, cxRectInflate(ViewPortBounds,
      ScaleFactor.Apply(Style.BorderWidth)), FVisiblePolylines);
  end;
end;
...
begin
  ...
  else if Length(FScreenPoints) > 0 then
  begin
    Result := TdxGpPath.Create;
...
begin
  Result := (Length(FMapPoints) > 0) and Intersect(Item.Layer.ViewPort,
    FMapBounds);
end;
...
var
  I: Integer;
  Var1: TdxMapItemStyle;
begin
  Var1 := Style;
  if FCheckIntersections then
    for I := 0 to High(FVisiblePolylines) do
      dxGPPaintCanvas.Polyline(FVisiblePolylines[I],
        Var1.BorderColor,
        ScaleFactor.Apply(Var1.
        BorderWidth))
  else if Length(FScreenPoints) > 0 then
    dxGPPaintCanvas.Polyline(FScreenPoints,
      Var1.BorderColor,
      ScaleFactor.Apply(Var1.
      BorderWidth));
end;
...
function TdxMapPolylineViewInfo.InternalIsIntersect(const ARect: TRect)
  : Boolean;
var
...
function TdxMapPolylineViewInfo.InternalPtInElement(const APoint
  : TPoint): Boolean;
var
  APath: TdxGpPath;
begin
  ...
  try
    if APath <> nil then
      Result := APath.IsPointInPathOutline(APoint,
        ScaleFactor.Apply(Style.BorderWidth));
  finally
    FreeAndNil(APath);
  end;
end;
...
begin
  AStyle := Style;
  dxGPPaintCanvas.Ellipse(DrawBounds, AStyle.BorderColor, AStyle.Color,
    ScaleFactor.Apply(AStyle.BorderWidth));
end;
...
procedure TdxMapControlGeoPointItem.SetGeoPoint(const Value
...
class function TdxMapItemsZoomHelper.CalculateMapBounds(AItems: TdxMapItemList;
  out ARect: TdxRectDouble): Boolean;
var
  I: Integer;
begin
...
procedure TdxMapCustomMultiPointItem.SetGeoPoints
...
procedure TdxMapItemTitleOptions.SetVisibility(const Value
  : TdxMapItemTitleVisibility);
begin
...
begin
  FScreenCenterPoint := dxPointDoubleToPoint
    (Item.Layer.MapUnitToScreenPoint(FMapCenterPoint));
  ABounds := cxRect(MaxInt, MaxInt, MinInt, MinInt);
  for I := 0 to High(FScreenPoints) do
  begin
  ...
  FCheckIntersections := CheckIntersectionsNeeded(Bounds);
  if FCheckIntersections then
    CalculateVisiblePolygons(FScreenPoints, cxRectInflate(ViewPortBounds,
      ScaleFactor.Apply(Style.BorderWidth)), FVisiblePolygons);
end;

function GetAreaIncrement(const APoint1, APoint2: TdxPointDouble)
  : Double; inline;
begin
...
function GetCenterIncrement(const APoint1, APoint2: TdxPointDouble)
  : TdxPointDouble; inline;
var
...
function CalculatePolygonCenter(const APoints: TdxDoublePoints; AArea: Double)
  : TdxPointDouble; inline;
var
  AInc: TdxPointDouble;
  I: Integer;
...
begin
  Result := (Length(FScreenPoints) > 0) and
    (FMapBounds.Width * Item.Layer.RenderScale.X > 1) and
...
begin
  ...
  begin
    if NeedCalculateScreenBoundsToDetermineVisibility then
      CalculateBounds
    else if not FIsCachedBoundsValid then
      CalculateCachedBounds;
...
procedure TdxMapItemViewInfo.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
begin
  ...
  else if mcsvColor in AOwnerStyle.AssignedValues then
  ...
  else if mcsvBorderWidth in AOwnerStyle.AssignedValues then
  ...
  else if mcsvBorderColor in AOwnerStyle.AssignedValues then
  ...
  else if mcsvTextGlowColor in AOwnerStyle.AssignedValues then
...
function TdxMapItemViewInfo.ParseTextPattern(const ATextPattern
  : string): string;

  function FindOpenDelimeter(const S: string; ACurrentPos: Integer;
  ...
  function FindCloseDelimeter(const S: string; ACurrentPos: Integer;
    out APos: Integer): Boolean;
  begin
    APos := PosEx('}', S, ACurrentPos);
    Result := APos <> 0;
...
begin
  ...
  begin
    AName := Trim(Copy(ATextPattern, AStartPatternPos + 1,
      AEndPatternPos - AStartPatternPos - 1));
    if Item.Attributes.Items.TryGetValue(AName, AValue) then
    begin
      Result := StringReplace(Result, Format('{%s}', [AName]), AValue,
...
procedure TdxMapItemViewInfo.CalculateTitleBounds;
  ...
  begin
    if (Item.TitleOptions.Visibility = mitvHidden) or
      ((Item.Layer as TdxCustomMapItemLayer)
      .ItemTitleOptions.Visibility = mitvHidden) then
      Result := mitvHidden
    else if (Item.TitleOptions.Visibility = mitvVisible) or
      ((Item.Layer as TdxCustomMapItemLayer)
      .ItemTitleOptions.Visibility = mitvVisible) then
      Result := mitvVisible
    else
      Result := mitvAuto;
  end;
...
begin
  if cxSizeIsEmpty(Title.Size) then
    ...
    Result := cxRectSetOrigin(Title.ClientBounds,
      cxPointOffset(FScreenCenterPoint,
...
begin
  GetMapControl(Item.Layer.GetParentComponent)
...
function TdxMapItemViewInfo.GetText: string;
  ...
  begin
    ...
    if ATextPattern = '' then
      ATextPattern := (Item.Layer as TdxCustomMapItemLayer)
        .ItemTitleOptions.Text;
    Result := ATextPattern <> '';
  end;
...
function TdxMapItemViewInfo.GetTextColor(AState: TdxMapControlElementState)
  : TdxAlphaColor;
begin
  Result := LookAndFeelPainter.MapControlMapCustomElementTextColor;
end;

function TdxMapItemViewInfo.GetTextGlowColor(AState: TdxMapControlElementState)
  : TdxAlphaColor;
begin
...
begin
  ASize := cxSize(Bounds);
  Result := (FMaxTitleSize.Width < Title.Size.cx) or
    (FMaxTitleSize.Height < Title.Size.cy);
end;

function TdxMapItemViewInfo.
  NeedCalculateScreenBoundsToDetermineVisibility: Boolean;
begin
...
begin
  Result := inherited PtInElement(APoint) and
    cxRectPtIn(ViewPortBounds, APoint);
end;
...
function TdxMapItemViewInfo.GetBorderColor(AState: TdxMapControlElementState)
  : TdxAlphaColor;
begin
  case AState of
    mcesNormal:
...
function TdxMapItemViewInfo.GetBorderWidth
  (AState: TdxMapControlElementState): Integer;
begin
  case AState of
    mcesNormal:
...
function TdxMapItemViewInfo.GetColor(AState: TdxMapControlElementState)
  : TdxAlphaColor;
begin
  case AState of
    mcesNormal:
...
begin
  ...
  begin
    ...
    AMaxFontSize := 0;
    FMaxBorderWidth := 0;
    for I := Low(TdxMapControlElementState)
      to High(TdxMapControlElementState) do
...
function TdxMapItemStylesHelper.GetStyle(AState: TdxMapControlElementState)
  : TdxMapItemStyle;
begin
  Result := FStyles[AState];
end;

procedure TdxMapItemStylesHelper.SetStyle(AState: TdxMapControlElementState;
  const Value: TdxMapItemStyle);
begin
...
begin
  ...
  cxRectIntersect(FVisibleRect, cxRectInflate(ViewPortBounds,
    ScaleFactor.Apply(Style.BorderWidth)), Bounds);
end;
...
begin
  ...
  AItemGeoSize := Item.Layer.KilometersToGeoSize(AItem.Location.GeoPoint,
    dxSizeDouble(AItem.Width, AItem.Height));
  AGeoPoint2 := dxMapControlGeoPoint(AItem.Location.Latitude -
    AItemGeoSize.Height, AItem.Location.Longitude + AItemGeoSize.Width);
  FMapPoint2 := Item.Layer.GeoPointToMapUnit(AGeoPoint2);
  FMapBounds := dxRectDouble(FMapPoint1.X, FMapPoint1.Y,
    FMapPoint2.X - FMapPoint1.X, FMapPoint2.Y - FMapPoint1.Y);
end;
...
begin
  AStyle := Style;
  dxGPPaintCanvas.Rectangle(FVisibleRect, AStyle.BorderColor, AStyle.Color,
    ScaleFactor.Apply(AStyle.BorderWidth));
end;
...
begin
  Result := (MapDot.Size >= 1) and cxRectIntersect(ViewPortBounds, Bounds);
end;
...
begin
  ...
  case MapDot.ShapeKind of
    ...
    dxGPPaintCanvas.Rectangle(DrawBounds, AStyle.BorderColor, AStyle.Color,
      ABorderWidth);
  end;
end;
...
procedure TdxMapControlGeoPointCollection.AddRange
  (AGeoPoints: TdxMapControlGeoPoints);
var
  I: Integer;
begin
  BeginUpdate;
...
function TdxMapControlGeoPointCollection.GetItem(Index: Integer)
  : TdxMapControlGeoPointItem;
begin
...
function TdxMapItem.GetStyle(AState: TdxMapControlElementState)
  : TdxMapItemStyle;
begin
  Result := FStyles[AState];
end;
...
function TdxMapItem.GetCollectionFromParent(AParent: TComponent)
...
procedure TdxMapItem.SetStyle(const Index: TdxMapControlElementState;
  const Value: TdxMapItemStyle);
begin
