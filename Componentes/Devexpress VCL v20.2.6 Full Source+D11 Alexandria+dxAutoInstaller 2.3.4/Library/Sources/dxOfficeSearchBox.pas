...
uses
  Windows, SysUtils, StrUtils, Classes, Controls, Types, Variants, Graphics,
  Math, Forms,
  cxClasses, dxCore, dxCoreClasses, cxControls, dxGDIPlusClasses, cxGeometry,
  cxGraphics, cxLookAndFeelPainters,
  cxContainer, cxEdit, cxTextEdit, dxBar, dxBarSkinConsts, dxBarAccessibility,
  dxRibbon, dxCoreGraphics;
...
type
  ...
  TdxOfficeSearchBoxCustomSearchSourceHelperClass = class of
  ...
  TdxOfficeSearchBoxDropDownMenuItemAddedEvent = procedure
    (Sender: TdxOfficeSearchBoxProperties; ASourceItem: TObject;
    ADropDownMenuItem: TdxBarItemLink) of object;
  TdxOfficeSearchBoxDropDownMenuItemAddingEvent = procedure
    (Sender: TdxOfficeSearchBoxProperties; const ASearchText: string;
    ASourceItem: TObject; var AAllow: Boolean) of object;
  TdxOfficeSearchBoxDropDownMenuPopulateEvent = procedure
    (Sender: TdxOfficeSearchBoxProperties; const ASearchText: string;
    ADropDownMenuItems: TdxBarItemLinks) of object;
  TdxOfficeSearchBoxSourceItemEnumProcRef = reference to procedure
    (AItem: TObject; out ACanContinueProcessing: Boolean);
  ...
  strict private
    ...
    function GetSearchSourceHelperClass
      : TdxOfficeSearchBoxCustomSearchSourceHelperClass;
  ...
  protected
    ...
    //
    procedure DoDropDownMenuItemAdded(ASourceItem: TObject;
      ADropDownMenuItems: TdxBarItemLinks); virtual;
    function DoDropDownMenuItemAdding(const ASearchText: string;
      ASourceItem: TObject; ADropDownMenuItems: TdxBarItemLinks)
      : Boolean; virtual;
    procedure DoDropDownMenuPopulate(const ASearchText: string;
      ADropDownMenuItems: TdxBarItemLinks); virtual;
    procedure PopulateDropDownMenu(const ASearchText: string;
      ADropDownMenuItems: TdxBarItemLinks); virtual;
  public
    ...
    function GetPaths(ASourceItem: TObject;
      const APathDelimiter: Char = dxOfficeSearchBoxDefaultPathDelimiter)
      : TStringList; virtual;
  ...
  published
    ...
    property MaxResultCount: Integer read FMaxResultCount
      write SetMaxResultCount default dxOfficeSearchBoxDefaultMaxResultCount;
    property Nullstring;
    property RecursiveSearch: TdxDefaultBoolean read FRecursiveSearch
      write SetRecursiveSearch default bDefault;
    property Ribbon: TdxCustomRibbon read GetRibbon write SetRibbon;
    property SearchSource: TComponent read GetSearchSource
      write SetSearchSource;
    property ShowResultPaths: Boolean read FShowResultPaths
      write SetShowResultPaths default False;
    property UseNullString;
    //
    property OnDropDownMenuItemAdded
      : TdxOfficeSearchBoxDropDownMenuItemAddedEvent
      read FOnDropDownMenuItemAdded write FOnDropDownMenuItemAdded;
    property OnDropDownMenuItemAdding
      : TdxOfficeSearchBoxDropDownMenuItemAddingEvent
      read FOnDropDownMenuItemAdding write FOnDropDownMenuItemAdding;
    property OnDropDownMenuPopulate: TdxOfficeSearchBoxDropDownMenuPopulateEvent
      read FOnDropDownMenuPopulate write FOnDropDownMenuPopulate;
  end;
  ...
  public
    ...
    function AddToDropDownMenu(ASourceItem: TObject;
      ADropDownMenuItems: TdxBarItemLinks): TdxBarItemLink; virtual; abstract;
    function CanAddToDropDownMenu(const ASearchText: string;
      ASourceItem: TObject; ADropDownMenuItems: TdxBarItemLinks): Boolean;
      virtual; abstract;
    procedure DoForAllSearchSourceItems(ASearchSource: TComponent;
      AProcRef: TdxOfficeSearchBoxSourceItemEnumProcRef); virtual; abstract;
    procedure PopulatePaths(ASourceItem: TObject; var APaths: TStringList);
      virtual; abstract;
  end;

  { TdxOfficeSearchBoxBarManagerSearchSourceHelper }

  TdxOfficeSearchBoxBarManagerSearchSourceHelper = class
    (TdxOfficeSearchBoxCustomSearchSourceHelper)
  protected
    function CanProcessSourceItem(ASourceItem: TObject): Boolean; virtual;
    function DoProcessItemLinks(AItemLinks: TdxBarItemLinks;
      AProcRef: TdxOfficeSearchBoxSourceItemEnumProcRef): Boolean; virtual;
    procedure GetPathsBy(AOwner: TComponent; var APaths: TStringList;
      const ABasePath: string); virtual;
    procedure GetPathsByBar(AOwnerBar: TdxBar; var APaths: TStringList;
      const ABasePath: string); virtual;
    procedure GetPathsByBarItem(AOwnerBarItem: TdxBarItem;
      var APaths: TStringList; const ABasePath: string); virtual;
    procedure GetPathsByPopupMenu(AOwnerPopupMenu: TdxBarCustomPopupMenu;
      var APaths: TStringList; const ABasePath: string;
      ABarManager: TdxBarManager); virtual;
  public
    function AddToDropDownMenu(ASourceItem: TObject;
      ADropDownMenuItems: TdxBarItemLinks): TdxBarItemLink; override;
    function CanAddToDropDownMenu(const ASearchText: string;
      ASourceItem: TObject; ADropDownMenuItems: TdxBarItemLinks)
      : Boolean; override;
    procedure DoForAllSearchSourceItems(ASearchSource: TComponent;
      AProcRef: TdxOfficeSearchBoxSourceItemEnumProcRef); override;
    procedure PopulatePaths(ASourceItem: TObject;
      var APaths: TStringList); override;
  end;

  { TdxOfficeSearchBoxRibbonSearchSourceHelper }

  TdxOfficeSearchBoxRibbonSearchSourceHelper = class
    (TdxOfficeSearchBoxBarManagerSearchSourceHelper)
  protected
    procedure GetPathsBy(AOwner: TComponent; var APaths: TStringList;
      const ABasePath: string); override;
    procedure GetPathsByBar(AOwnerBar: TdxBar; var APaths: TStringList;
      const ABasePath: string); override;
  public
    procedure DoForAllSearchSourceItems(ASearchSource: TComponent;
  ...
  protected
    ...
    //
    property InnerEdit: TdxOfficeSearchBoxInnerEdit read FInnerEdit
      write FInnerEdit;
  end;

  { TdxOfficeSearchBoxPopupMenuSubMenuControlAccessibilityHelper }

  TdxOfficeSearchBoxPopupMenuSubMenuControlAccessibilityHelper = class
  ...
  protected
    ...
    //
    property BarControl: TdxOfficeSearchBoxPopupMenuSubMenuControl
      read GetBarControl;
  end;
  ...
  protected
    ...
    property InnerEdit: TdxOfficeSearchBoxInnerEdit read FInnerEdit
      write FInnerEdit;
    property SubMenuControl: TdxOfficeSearchBoxPopupMenuSubMenuControl
      read GetSubMenuControl;
  end;
  ...
  strict private
    ...
    //
    procedure DropDownMenuCloseUpExHandler(Sender: TObject;
  ...
  protected
    ...
    //
    property ActiveProperties: TdxOfficeSearchBoxProperties
...
begin
  if Properties.Ribbon <> nil then
    Result := TdxSimpleColorPalette.Create(dxColorToAlphaColor(TextColor),
...
begin
  ...
  cxDrawImage(ACanvas, FGlyphRect, Properties.Glyph, nil, -1, Enabled,
    GetGlyphColorPalette, ScaleFactor);
end;
...
procedure TdxOfficeSearchBoxViewData.Calculate(ACanvas: TcxCanvas;
...
function TdxOfficeSearchBoxViewData.InternalGetEditConstantPartSize
...
begin
  ...
  if (AViewInfo <> nil) and (TdxOfficeSearchBoxViewInfo(AViewInfo).Painter
    <> nil) then
    MinContentSize := cxSizeMax(MinContentSize,
      TdxOfficeSearchBoxViewInfo(AViewInfo).CalculateEditConstantPartSize);
end;
...
class function TdxOfficeSearchBoxProperties.GetContainerClass
  : TcxContainerClass;
begin
  Result := TdxOfficeSearchBox;
end;

class function TdxOfficeSearchBoxProperties.GetViewInfoClass
...
function TdxOfficeSearchBoxProperties.GetSupportedOperations
...
procedure TdxOfficeSearchBoxProperties.DoAssign(AProperties
...
begin
  ...
  begin
    ...
    RecursiveSearch := TdxOfficeSearchBoxProperties(AProperties)
      .RecursiveSearch;
    Ribbon := TdxOfficeSearchBoxProperties(AProperties).Ribbon;
    SearchSource := TdxOfficeSearchBoxProperties(AProperties).SearchSource;
    ShowResultPaths := TdxOfficeSearchBoxProperties(AProperties)
      .ShowResultPaths;
    //
    OnDropDownMenuItemAdded := TdxOfficeSearchBoxProperties(AProperties)
      .OnDropDownMenuItemAdded;
    OnDropDownMenuItemAdding := TdxOfficeSearchBoxProperties(AProperties)
      .OnDropDownMenuItemAdding;
    OnDropDownMenuPopulate := TdxOfficeSearchBoxProperties(AProperties)
      .OnDropDownMenuPopulate;
  end;
end;

class function TdxOfficeSearchBoxProperties.GetViewDataClass
  : TcxCustomEditViewDataClass;
begin
  Result := TdxOfficeSearchBoxViewData;
end;

class function TdxOfficeSearchBoxProperties.IsValidSearchSource
  (AValue: TComponent): Boolean;
begin
  Result := (AValue = nil) or (AValue is TdxCustomRibbon) or
    (AValue is TdxBarManager);
end;
...
procedure TdxOfficeSearchBoxProperties.DoDropDownMenuItemAdded
  (ASourceItem: TObject; ADropDownMenuItems: TdxBarItemLinks);
var
...
begin
  ...
  if FSearchSourceHelper <> nil then
    ADropDownMenuItem := FSearchSourceHelper.AddToDropDownMenu(ASourceItem,
      ADropDownMenuItems);
  if Assigned(OnDropDownMenuItemAdded) then
    OnDropDownMenuItemAdded(Self, ASourceItem, ADropDownMenuItem);
end;

function TdxOfficeSearchBoxProperties.DoDropDownMenuItemAdding(const ASearchText
  : string; ASourceItem: TObject; ADropDownMenuItems: TdxBarItemLinks): Boolean;
begin
  Result := False;
  if FSearchSourceHelper <> nil then
    Result := FSearchSourceHelper.CanAddToDropDownMenu(ASearchText, ASourceItem,
      ADropDownMenuItems);
  if Assigned(OnDropDownMenuItemAdding) then
    OnDropDownMenuItemAdding(Self, ASearchText, ASourceItem, Result);
end;

procedure TdxOfficeSearchBoxProperties.DoDropDownMenuPopulate(const ASearchText
  : string; ADropDownMenuItems: TdxBarItemLinks);
begin
  if Assigned(OnDropDownMenuPopulate) then
    OnDropDownMenuPopulate(Self, ASearchText, ADropDownMenuItems);
end;

procedure TdxOfficeSearchBoxProperties.PopulateDropDownMenu(const ASearchText
  : string; ADropDownMenuItems: TdxBarItemLinks);
var
  ACount: Integer;
begin
    ...
    try
      ...
      if FSearchSourceHelper <> nil then
        FSearchSourceHelper.DoForAllSearchSourceItems(SearchSource,
          ...
          begin
            if DoDropDownMenuItemAdding(ASearchText, ASourceItem,
              ADropDownMenuItems) then
            begin
...
function TdxOfficeSearchBoxProperties.GetComponentForOwner(AComponentClass
  : TComponentClass): TComponent;
var
...
function TdxOfficeSearchBoxProperties.GetSearchSourceHelperClass
  : TdxOfficeSearchBoxCustomSearchSourceHelperClass;
begin
  Result := nil;
...
procedure TdxOfficeSearchBoxProperties.SetRecursiveSearch
...
constructor TdxOfficeSearchBoxCustomSearchSourceHelper.Create
...
function TdxOfficeSearchBoxCustomSearchSourceHelper.GetDefaultPath
  (ASourceItem: TObject): string;

  function GetPathLength(APaths: TStringList;
  const APathIndex: Integer): Integer;
  var
...
function TdxOfficeSearchBoxCustomSearchSourceHelper.GetPathDelimiter
  (APaths: TStringList): string;
begin
...
function TdxOfficeSearchBoxBarManagerSearchSourceHelper.AddToDropDownMenu
  (ASourceItem: TObject; ADropDownMenuItems: TdxBarItemLinks): TdxBarItemLink;
begin
...
function TdxOfficeSearchBoxBarManagerSearchSourceHelper.CanAddToDropDownMenu
  (const ASearchText: string; ASourceItem: TObject;
ADropDownMenuItems: TdxBarItemLinks): Boolean;
var
  ALink: TdxBarItemLink;
begin
  ALink := TdxBarItemLink(ASourceItem);
  Result := (ALink.Item <> nil) and
    (ADropDownMenuItems.FindByItem(ALink.Item) = nil) and
    ALink.Item.ActuallyVisible and
    AnsiContainsText(RemoveAccelChars(ALink.Caption), ASearchText) and
    ((ALink.Item is TdxBarCustomButton) or (ALink.Item is TdxBarSubItem) or
    ((ALink.Item is TdxCustomRibbonGalleryItem) and
    TdxCustomRibbonGalleryItemAccess(ALink.Item)
    .GalleryInMenuOptions.CollapsedInSubmenu));
end;

procedure TdxOfficeSearchBoxBarManagerSearchSourceHelper.
  DoForAllSearchSourceItems(ASearchSource: TComponent;
...
procedure TdxOfficeSearchBoxBarManagerSearchSourceHelper.PopulatePaths
  (ASourceItem: TObject; var APaths: TStringList);
var
  ABarItem: TdxBarItem;
  I: Integer;
begin
...
function TdxOfficeSearchBoxBarManagerSearchSourceHelper.CanProcessSourceItem
  (ASourceItem: TObject): Boolean;
var
  AItem: TdxBarItem;
...
function TdxOfficeSearchBoxBarManagerSearchSourceHelper.DoProcessItemLinks
  (AItemLinks: TdxBarItemLinks;
...
begin
  ...
  begin
    ...
    if Result and (Properties.RecursiveSearch = bTrue) and
      Supports(AItemLinks[I].Item, IdxBarLinksOwner, ALinksOwner) and
      (ALinksOwner.GetItemLinks <> nil) then
...
procedure TdxOfficeSearchBoxBarManagerSearchSourceHelper.GetPathsBy
  (AOwner: TComponent; var APaths: TStringList; const ABasePath: string);
begin
  if AOwner is TdxBarItem then
    ...
    GetPathsByPopupMenu(AOwner as TdxBarCustomPopupMenu, APaths, ABasePath,
      Properties.SearchSource as TdxBarManager)
  else if AOwner is TdxBar then
    GetPathsByBar(AOwner as TdxBar, APaths, ABasePath);
end;

procedure TdxOfficeSearchBoxBarManagerSearchSourceHelper.GetPathsByBar
  (AOwnerBar: TdxBar; var APaths: TStringList; const ABasePath: string);
begin
  APaths.Add(AOwnerBar.Caption + GetPathDelimiter(APaths) + ABasePath);
end;

procedure TdxOfficeSearchBoxBarManagerSearchSourceHelper.GetPathsByBarItem
  (AOwnerBarItem: TdxBarItem; var APaths: TStringList; const ABasePath: string);
var
  I: Integer;
begin
...
procedure TdxOfficeSearchBoxBarManagerSearchSourceHelper.GetPathsByPopupMenu
  (AOwnerPopupMenu: TdxBarCustomPopupMenu; var APaths: TStringList;
const ABasePath: string; ABarManager: TdxBarManager);
var
  I: Integer;
begin
  for I := 0 to ABarManager.ItemCount - 1 do
    if (ABarManager.Items[I] is TdxBarCustomButton) and
      (TdxBarCustomButton(ABarManager.Items[I]).DropDownMenu = AOwnerPopupMenu)
    then
...
procedure TdxOfficeSearchBoxRibbonSearchSourceHelper.DoForAllSearchSourceItems
  (ASearchSource: TComponent;
AProcRef: TdxOfficeSearchBoxSourceItemEnumProcRef);
var
    ...
    if (ATab.Context = nil) or ATab.Context.Visible then
      for J := 0 to ATab.Groups.Count - 1 do
        if ATab.Groups[J].ToolBar <> nil then
          if not DoProcessItemLinks(ATab.Groups[J].ToolBar.ItemLinks, AProcRef)
          then
            Exit;
  end;
  if (ARibbon.QuickAccessToolbar.ToolBar <> nil) and ARibbon.QuickAccessToolbar.Visible
  then
    if not DoProcessItemLinks(ARibbon.QuickAccessToolbar.ToolBar.ItemLinks,
      AProcRef) then
      Exit;
  if (ARibbon.TabAreaToolbar.ToolBar <> nil) and ARibbon.TabAreaToolbar.Visible
  then
    if not DoProcessItemLinks(ARibbon.TabAreaToolbar.ToolBar.ItemLinks, AProcRef)
    then
      Exit;
  if (ARibbon.TabAreaSearchToolbar.ToolBar <> nil) and
    ARibbon.TabAreaSearchToolbar.Visible then
    if not DoProcessItemLinks(ARibbon.TabAreaSearchToolbar.ToolBar.ItemLinks,
      AProcRef) then
      Exit;
end;

procedure TdxOfficeSearchBoxRibbonSearchSourceHelper.GetPathsBy
  (AOwner: TComponent; var APaths: TStringList; const ABasePath: string);
begin
...
procedure TdxOfficeSearchBoxRibbonSearchSourceHelper.GetPathsByBar
  (AOwnerBar: TdxBar; var APaths: TStringList; const ABasePath: string);
var
...
begin
  ...
  begin
    ...
    if AGroup <> nil then
    begin
      APath := ATab.Caption + GetPathDelimiter(APaths) + AGroup.Caption +
        GetPathDelimiter(APaths) + ABasePath;
      if ATab.Context <> nil then
        APath := ATab.Context.Caption + GetPathDelimiter(APaths) + APath;
      APaths.Add(APath);
    end;
  end;
  if (ARibbon.QuickAccessToolbar.ToolBar = AOwnerBar) and
    ARibbon.QuickAccessToolbar.Visible then
    APaths.Add(cxGetResourceString(@dxSBAR_RIBBON_QUICKACCESSTOOLBARNAME) +
      GetPathDelimiter(APaths) + ABasePath);
  if (ARibbon.TabAreaToolbar.ToolBar = AOwnerBar) and ARibbon.TabAreaToolbar.Visible
  then
    APaths.Add(cxGetResourceString(@dxSBAR_RIBBON_TABAREATOOLBARNAME) +
      GetPathDelimiter(APaths) + ABasePath);
  if (ARibbon.TabAreaSearchToolbar.ToolBar = AOwnerBar) and
    ARibbon.TabAreaSearchToolbar.Visible then
    APaths.Add(cxGetResourceString(@dxSBAR_RIBBON_TABAREASEARCHTOOLBARNAME) +
      GetPathDelimiter(APaths) + ABasePath);
end;

{ TdxOfficeSearchBoxPopupMenuSubMenuControl }

procedure TdxOfficeSearchBoxPopupMenuSubMenuControl.DoHideAll
  (AReason: TdxBarCloseUpReason);
begin
...
function TdxOfficeSearchBoxPopupMenuSubMenuControl.GetAccessibilityHelperClass
...
function TdxOfficeSearchBoxPopupMenuSubMenuControl.IsNavigationKey(AKey: Word;
AShift: TShiftState): Boolean;
begin
  Result := AKey in [VK_LEFT, VK_RIGHT, VK_UP, VK_DOWN, VK_TAB, VK_RETURN];
end;

function TdxOfficeSearchBoxPopupMenuSubMenuControl.PreProcessKey(AKey: Word;
AShift: TShiftState): Boolean;
begin
  ...
  Result := InnerEdit.InternalFocused or not IsNavigationKey(AKey, AShift) or
    inherited PreProcessKey(AKey, AShift);
end;

{ TdxOfficeSearchBoxPopupMenuSubMenuControlAccessibilityHelper }

function TdxOfficeSearchBoxPopupMenuSubMenuControlAccessibilityHelper.
  CanNavigateToChildren(AKey: Word): Boolean;
begin
  if not BarControl.InnerEdit.InternalFocused and (AKey = VK_UP) and
    (BarControl.TopItemControl <> nil) and BarControl.TopItemControl.IsSelected
  then
...
function TdxOfficeSearchBoxPopupMenuSubMenuControlAccessibilityHelper.
  GetBarControl: TdxOfficeSearchBoxPopupMenuSubMenuControl;
begin
  Result := inherited GetBarControl as
    TdxOfficeSearchBoxPopupMenuSubMenuControl;
end;
...
function TdxOfficeSearchBoxPopupMenu.GetSubMenuControl
  : TdxOfficeSearchBoxPopupMenuSubMenuControl;
begin
  Result := inherited SubMenuControl as
    TdxOfficeSearchBoxPopupMenuSubMenuControl;
end;

procedure TdxOfficeSearchBoxPopupMenu.SubMenuControlMouseEnterHandler
  (Sender: TObject);
begin
...
procedure TdxOfficeSearchBoxInnerEdit.KeyDown(var Key: Word;
Shift: TShiftState);
begin
  case Key of
    VK_DOWN, VK_RETURN:
...
begin
  ...
  begin
    ...
    DropDownMenu.Popup(Container.ClientToScreen(Container.VisibleBounds.TopLeft)
      .X, Container.ClientToScreen(Container.VisibleBounds.BottomRight).Y);
  end;
end;

procedure TdxOfficeSearchBoxInnerEdit.UpdateDropDownMenu;
begin
  if (ActiveProperties.BarManager = nil) or ActiveProperties.BarManager.IsDesigning
  then
    Exit;
...
function TdxOfficeSearchBoxInnerEdit.GetActiveProperties
  : TdxOfficeSearchBoxProperties;
begin
  ...
  begin
    ...
    begin
      if FInternalFocused then
        ...
        DropDownMenu.SubMenuControl.SetKeySelectedItem
          (DropDownMenu.SubMenuControl.TopItemControl);
    end;
  end;
end;

procedure TdxOfficeSearchBoxInnerEdit.DropDownMenuCloseUpExHandler
  (Sender: TObject; AReason: TdxBarCloseUpReason);
begin
  TdxOfficeSearchBoxPopupMenu(Sender).BarManager.MenuAnimations :=
    FStoredBarMenuAnimation;
end;
...
class function TdxOfficeSearchBox.GetPropertiesClass
...
begin
  Result := HasInnerEdit and ((InnerEdit.Control as TdxOfficeSearchBoxInnerEdit)
    .DropDownMenu <> nil) and (InnerEdit.Control as TdxOfficeSearchBoxInnerEdit)
...
begin
  ...
  begin
    ARect := cxRectCenterVertically(ViewInfo.InnerEditRect,
      InnerEdit.Control.Height);
...
procedure TdxOfficeSearchBox.SetProperties
  (AValue: TdxOfficeSearchBoxProperties);
begin
...
GetRegisteredEditProperties.Register(TdxOfficeSearchBoxProperties,
  'OfficeSearchBox');
