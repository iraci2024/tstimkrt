...
type
  ...
  protected
    ...
    property OwnerWindowWndProcObject: TcxWindowProcLinkedObject
      read FOwnerWindowWndProcObject;
  public
    ...
    //
    property ResizeOwnerWindowUsingShadow: Boolean
      read FResizeOwnerWindowUsingShadow write SetResizeOwnerWindowUsingShadow;
...
const
  ...
  begin
    Result := (ABufferSize < ATargetSize) or
      (ABufferSize - ATargetSize > ResizeDelta);
  end;

begin
  if FOutputBuffer = nil then
    FOutputBuffer := TcxBitmap32.CreateSize(AWidth, AHeight);
  if CheckNeedResize(FOutputBuffer.Width, AWidth) or
    CheckNeedResize(FOutputBuffer.Height, AHeight) then
  begin
...
begin
  ...
  FOwnerWindowWndProcObject := cxWindowProcController.Add(OwnerWindow,
    OwnerWindowWndProc);
  FOwnerWindowIsActive := OwnerWindow.HandleAllocated and
    (OwnerWindow.Handle = GetActiveWindow);
...
begin
  Result := (ResizeOwnerWindowUsingShadow and Transparent or
    (OwnerWindowIsActive or FActivating)) and
    IsWindowVisible(OwnerWindow.Handle) and CanUseShadows and
  ...
  case AMessage.Msg of
    ...
    WM_WINDOWPOSCHANGED:
      begin
        ...
        if (AWindowPos.flags and SWP_NOSIZE = 0) or
          (AWindowPos.flags and SWP_NOMOVE = 0) or
          (AWindowPos.flags and SWP_SHOWWINDOW <> 0) or
          (AWindowPos.flags and SWP_HIDEWINDOW <> 0) then
        begin
          UpdateVisibility;
          UpdateBounds;
        end;

        if (AWindowPos.flags and SWP_NOZORDER = 0) and Visible then
          UpdateZOrder;
      end;
  end;
end;
...
begin
  ...
  SetWindowPos(Handle, OwnerWindow.Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R), SWP_NOACTIVATE);
end;

procedure TdxShadowWindow.UpdateZOrder;
begin
  SetWindowPos(Handle, OwnerWindow.Handle, 0, 0, 0, 0, SWP_NOMOVE or
...
begin
  case Message.Msg of
    WM_NCLBUTTONDOWN:
      ...
      begin
        SendMessage(OwnerWindow.Handle, Message.Msg, Message.WParam,
          Message.LParam);
        Message.Result := 0;
      end
      else
...
procedure TdxShadowWindow.WMNCHitTest(var Message: TWMNCHitTest);
  ...
  function CheckVerticalSide(const R: TRect; ATopCornerCode, ABottomCornerCode,
    ASideCode: Integer): Integer;
  begin
    if Message.YPos <= R.Top then
      Result := ATopCornerCode
    else if Message.YPos >= R.Bottom then
...
begin
  ...
  if ResizeOwnerWindowUsingShadow then
  begin
    R := cxRectInflate(BoundsRect, -GetSystemMetrics(SM_CXSIZEFRAME),
      -GetSystemMetrics(SM_CYSIZEFRAME));
    if not PtInRect(R, SmallPointToPoint(Message.Pos)) then
    begin
      if Message.XPos >= R.Right then
        Message.Result := CheckVerticalSide(R, HTTOPRIGHT,
          HTBOTTOMRIGHT, HTRIGHT)
      else
...
begin
  ...
  Result.Offset(dxMapWindowPoint(OwnerWindow.Handle, Handle,
    cxNullPoint, False));
end;

procedure TdxShadowWindow.SetResizeOwnerWindowUsingShadow
