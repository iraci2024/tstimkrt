...
procedure cxExportVGToExcel(const AFileName: string;
  AVerticalGrid: TcxCustomVerticalGrid; AExpand: Boolean = True;
  AUseNativeFormat: Boolean = True; ARecordPerBand: Integer = 8;
...
procedure cxExportVGToCSV(const AFileName: string;
  AVerticalGrid: TcxCustomVerticalGrid; AExpand: Boolean = True;
  const ASeparator: Char = ','; ARecordPerBand: Integer = 8;
  const AFileExt: string = 'csv'; AHandler: TObject = nil;
  AEncoding: TEncoding = nil);

procedure cxExportVGToText(const AFileName: string;
  AVerticalGrid: TcxCustomVerticalGrid; AExpand: Boolean;
  ARecordPerBand: Integer = 8; const AFileExt: string = 'txt';
...
procedure cxExportVGToFile(AFileName: string;
  ...
  ARecordPerBand: Integer; const AFileExt: string; AHandler: TObject = nil;
  AEncoding: TEncoding = nil);
...
uses
  ...
  cxEdit, cxCalendar, cxCurrencyEdit, cxSpinEdit, cxCalc, cxTimeEdit, Classes,
  cxGraphics, cxStyles,
  cxInplaceContainer, cxDataUtils, cxExport, cxVGridViewInfo, cxVGridConsts,
  cxVGridUtils, cxTextEdit, cxDBVGrid;
...
type
  ...
  protected
    ...
    procedure Calculate(AHeaderColumnsMap: TcxColumnsMap;
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure AddRowCell(APos: Integer; ARow: TcxCustomRow;
    ...
    procedure GetColumnInfoFromLevel(ALevel: Integer;
      var AStart, AEnd: Integer);
  ...
  public
    function AddCaption(AColumn: Integer; AWidth: Integer; AStyleIndex: Integer;
  ...
  protected
    ...
    procedure AddMapRightSide(AMap: TcxColumnsMap;
    ...
    procedure GetDisplayValue(ARecordIndex: Integer;
      ARow: TcxCustomEditorRowProperties; out AValue: Variant;
      out AValueDisplayFormat: string;
      out AValueDisplayFormatType: TcxValueDisplayFormatType);
    function GetMultiEditorRowProperties(ARow: TcxCustomRow)
    ...
    procedure SetCellStyle(ACol, ARow, W: Integer;
      const AStyle: TcxCacheCellStyle); overload;
    ...
    procedure WriteRowHeader(ARowIndex: Integer; AIndents: TcxRowIndentsInfo;
  ...
  public
    ...
    property ProgressHelper: TcxCustomProgressCalculationHelper
...
const
  ...
  cxIndentStyle: TcxCacheCellStyle = (AlignText: catCenter;
...
begin
  cxSetBorder(AStyle.Borders[0], (bLeft in ABorders) and
    (AGridLines in [vglVertical, vglBoth]), AColor);
  cxSetBorder(AStyle.Borders[1], (bTop in ABorders) and
    (AGridLines in [vglHorizontal, vglBoth]), AColor);
...
begin
  ...
  repeat
    ...
    begin
      ...
      else if (not IsCategory and ANeedBottom) or ALastRow then
        ABorders := [bBottom]
      else
        ABorders := [];
    end;
...
function TcxRowCellsInfo.AddCaption(AColumn, AWidth, AStyleIndex: Integer;
...
begin
  ...
  for I := 0 to FElements.Count - 1 do
    with PElementInfo(FElements.List[I])^ do
      if Width > 0 then
        ColumnEnd := FindColumnForPos(Pos + Width);
...
constructor TcxVerticalGridMapsInfo.Create(AOwner: TcxVerticalGridExportHelper);
begin
  FOwner := AOwner;
...
begin
  ...
  begin
    ARow := Rows[I];
    APos := cxSetValue(ACalcIndent, (ARow.Level + 1) *
      ViewInfo.RowIndentWidth, 0);
    if ARow is TcxCustomEditorRow then
      AMap.AddRowCell(APos, ARow, 0, ARight - APos)
    else if ARow is TcxCustomMultiEditorRow then
      with GetMultiEditorRowProperties(ARow) do
      begin
...
var
  ...
  procedure AddCaption(AProperties: TcxCaptionRowProperties;
    ACellIndex: Integer);
  var
    AStart, AEnd: Integer;
    AStyle: TcxCacheCellStyle;
  begin
    ...
    AStyle.AlignText := AlignToCxAlign[AProperties.HeaderAlignmentHorz];
    cxCheckBorders(AStyle, [bTop, bBottom, bRight], GridLineColor, GridLines);
    ACaptions.AddCaption(AStart, AEnd - AStart, Provider.RegisterStyle(AStyle),
      AProperties.Caption);
  end;
...
procedure TcxVerticalGridMapsInfo.GetDisplayValue(ARecordIndex: Integer;
  ARow: TcxCustomEditorRowProperties; out AValue: Variant;
  out AValueDisplayFormat: string;
  out AValueDisplayFormatType: TcxValueDisplayFormatType);
...
function TcxVerticalGridMapsInfo.GetMultiEditorRowProperties(ARow: TcxCustomRow)
  : TcxMultiEditorRowProperties;
begin
  Result := TcxMultiEditorRowProperties(TcxCustomMultiEditorRowAccess(ARow)
    .FProperties);
end;
...
procedure TcxVerticalGridMapsInfo.SetCellStyle(ACol, ARow, W,
  AStyleIndex: Integer);
begin
  with Provider do
...
begin
  ...
  for J := 0 to ACount - 1 do
    ...
    begin
      ...
      Provider.SetColumnWidth(FirstValuesColumn + J *
        (ValueColumnsMap.MaxColumnIndex + 1) + I, AWidth);
    end;
end;
...
procedure TcxVerticalGridMapsInfo.WriteRecord(ACol, ARowIndex,
...
var
  ...
  begin
    ...
    AViewParams := TcxEditorRowStyles(TcxCustomRowAccess(ARow).Styles)
      .GetContentParams(AProperties, False, ARecordIndex);
    cxViewParamsToCacheStyle(AViewParams, AStyle);
    AStyle.AlignText := AlignToCxAlign
      [TcxPropertiesAccess(AProperties.DisplayEditProperties[ARecordIndex])
      .Alignment.Horz];
  ...
  for I := 0 to AIndents.Count - 1 do
    ...
    begin
      if (I = 0) and AIndents.Row.HasVisibleChildren then
        Provider.SetCellValueAsString(Column, ARowIndex,
          PlusMinus[AIndents.Row.Expanded]);
...
begin
  ...
  else if Owner.IsNativeFormat then
  begin
    GetDisplayValue(ARecordIndex, AProperties, AValue, AValueDisplayFormat,
      AValueDisplayFormatType);
    Provider.SetCellValue(ACol, ARow, AValue, AValueDisplayFormat,
      AValueDisplayFormatType);
  end
  else
    Provider.SetCellValueAsString(ACol, ARow, GetDisplayText(ARecordIndex,
      AProperties));
end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        WriteRecord(ACol + J * (ValueColumnsMap.MaxColumnIndex + 1),
          ARow, ARecord);
        Inc(ARecord);
...
constructor TcxVerticalGridExportHelper.Create(AVerticalGrid
  : TcxCustomVerticalGrid; AExportType: Integer; const AFileName: string;
  AHandler: TObject);
...
begin
  ...
  FProgressHelper := TcxProgressCalculationHelper.Create(2, AVerticalGrid,
    ProgressHandler);
...
function TcxVerticalGridExportHelper.GetMapsInfoClass
  : TcxVerticalGridMapsInfoClass;
begin
  Result := TcxVerticalGridMapsInfo;
end;

procedure TcxVerticalGridExportHelper.ProgressHandler(Sender: TObject;
  Percents: Integer);
var
...
procedure cxExportVGToFile(AFileName: string;
  AVerticalGrid: TcxCustomVerticalGrid; AExportType: Integer;
  AExpand, AUseNativeFormat: Boolean; const ASeparators: array of string;
  ARecordPerBand: Integer; const AFileExt: string; AHandler: TObject;
  AEncoding: TEncoding);
var
...
begin
  ...
  with TcxVerticalGridExportHelper.Create(AVerticalGrid, AExportType, AFileName,
    AHandler) do
    try
      FExpand := AExpand;
      if AExpand then
...
begin
  cxExportVGToFile(AFileName, AVerticalGrid, cxExportToHtml, AExpand, False, [],
    ARecordPerBand, AFileExt, AHandler);
end;
...
begin
  cxExportVGToFile(AFileName, AVerticalGrid, cxExportToExcel, AExpand,
    AUseNativeFormat, [], ARecordPerBand, AFileExt, AHandler);
end;
...
procedure cxExportVGToCSV(const AFileName: string;
  ...
  AEncoding: TEncoding = nil);
begin
  cxExportVGToFile(AFileName, AVerticalGrid, cxExportToCSV, AExpand, False,
    [ASeparator], ARecordPerBand, AFileExt, AHandler, AEncoding);
end;
...
begin
  cxExportVGToText(AFileName, AVerticalGrid, AExpand, '', '', '',
    ARecordPerBand, AFileExt, AHandler, AEncoding);
end;
...
begin
  cxExportVGToFile(AFileName, AVerticalGrid, cxExportToText, AExpand, False,
    [ASeparator, ABeginString, AEndString], ARecordPerBand, AFileExt, AHandler,
    AEncoding);
end;
