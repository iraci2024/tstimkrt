...
uses
  ...
  cxSchedulerDateNavigator, cxSchedulerDayView, cxDateUtils,
...
type
  ...
  TcxInnerDateNavigator = class(TcxCustomScheduler, IUnknown,
  ...
  TcxCustomDateNavigator = class(TcxContainer, IUnknown,
    IcxSchedulerStorageListener, IcxExternalDateNavigatorListener)
  private
    ...
    procedure IcxExternalDateNavigatorListener.StorageChanged =
      SchedulerStorageChanged;
  protected
    function CanDrag(X, Y: Integer): Boolean; override;
    ...
    property BorderStyle: TcxControlBorderStyle read GetBorderStyle
      write SetBorderStyle default cxcbsDefault;
    ...
    property FirstWeekOfYear: TcxFirstWeekOfYear read GetFirstWeekOfYear
      write SetFirstWeekOfYear default fwySystem;
    property Font: TFont read GetFont write SetFont;
    property HitTest: TcxSchedulerDateNavigatorHitTest read GetHitTest;
    property HolidayColor: TColor read GetHolidayColor write SetHolidayColor
      default clRed;
    ...
    property ShowDatesContainingEventsInBold: Boolean
      read GetShowDatesContainingEventsInBold
      write SetShowDatesContainingEventsInBold default True;
    property ShowDatesContainingHolidaysInColor: Boolean
      read GetShowDatesContainingHolidaysInColor
      write SetShowDatesContainingHolidaysInColor default False;
    property ShowWeekNumbers: Boolean read GetShowWeekNumbers
      write SetShowWeekNumbers default True;
    property StartOfWeek: TcxStartOfWeek read GetStartOfWeek
      write SetStartOfWeek default swSystem;
    property Storage: TcxCustomSchedulerStorage read FStorage write SetStorage;
    property Styles: TcxSchedulerDateNavigatorStyles read GetStyles
      write SetStyles;

    property OnPeriodChanged: TcxSchedulerPeriodChangedEvent
      read GetPeriodChanged write SetPeriodChanged;
    property OnSelectionChanged: TcxSchedulerPeriodChangedEvent
      read GetSelectionChanged write SetSelectionChanged;
    property OnShowDateHint: TcxSchedulerShowDateHintEvent read FOnShowDateHint
      write FOnShowDateHint;
  ...
  public
    ...
    property SelectionIsWeeks: Boolean read GetSelectionIsWeeks
      write SetSelectionIsWeeks;
    ...
    property UnlimitedSelection: Boolean read FUnlimitedSelection
      write SetUnlimitedSelection default False;
  end;
...
function TcxInnerDateNavigator.CreateDateNavigator
...
begin
  Result := (Container <> nil) and Container.DoMouseWheel(Shift, WheelDelta,
    MousePos);
  if not Result then
...
begin
  ...
  begin
    ...
    begin
      TcxSchedulerDragObject(Source).CalculateConflictsForDateNavigator
        (DateNavigator);
      FPrevDate := ADate;
...
begin
  ...
  begin
    ...
    else if FContainer.Scheduler <> nil then
...
begin
  ...
  if (ssShift in Shift) and (Key in [VK_LEFT .. VK_DOWN]) and
    (FSelAnchor = NullDate) then
  begin
  ...
  case Key of
    VK_LEFT, VK_RIGHT:
      begin
        ...
        if (ssShift in Shift) and TcxDateNavigatorAccess(DateNavigator).SelectPeriod
        then
          SetSelectionDays(FSelAnchor - AShift[AKey = VK_RIGHT],
            AKey = VK_RIGHT)
        else
    ...
    VK_UP, VK_DOWN:
      begin
        if (ssShift in Shift) and TcxDateNavigatorAccess(DateNavigator).SelectPeriod
        then
          SetSelectionDays(FSelAnchor - AShift[Key = VK_DOWN] * 7,
            Key = VK_DOWN)
        else
...
procedure TcxInnerDateNavigator.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TcxInnerDateNavigator.SetSelectionDays(ADate: Integer;
  ACheckEnd: Boolean);
...
begin
  ...
  begin
    if (TcxDateNavigatorAccess(DateNavigator).SelectionIsWeeks and
      (WeekOf(Max(FSelStart, ADate)) - WeekOf(Min(FSelStart, ADate)) >
      cxMaxWeekCount)) or (not TcxDateNavigatorAccess(DateNavigator)
      .SelectionIsWeeks and (Max(FSelStart, ADate) - Min(FSelStart, ADate) > 14))
    then
      Exit;
  end;
  FSelAnchor := ADate;
  ...
  if (SelectedDays[SelectedDays.Count - 1] = ADate) then
    while (ADate > TcxDateNavigatorAccess(DateNavigator).LastDate) do
      TcxDateNavigatorAccess(DateNavigator).FirstDate :=
        TcxDateNavigatorAccess(DateNavigator).FirstDate + 31;
...
begin
  ...
  begin
    DoLayoutChanged;
    Result := Rect(0, 0, GetMonthSize.CX * AColCount,
      GetMonthSize.CY * ARowCount);
  end;
...
begin
  ...
  begin
    Inc(FLockUpdate);
    try
      TcxSchedulerDateNavigator(FScheduler.DateNavigator)
        .AssignProperties(TcxSchedulerDateNavigator
        (FInnerDateNavigator.DateNavigator));
    finally
...
begin
  ...
  try
    ...
    TcxDateNavigatorAccess(InnerDateNavigator.DateNavigator)
...
function TcxCustomDateNavigator.GetCustomDrawBackground
  : TcxSchedulerDateNavigatorCustomDrawBackgroundEvent;
begin
  Result := DateNavigator.OnCustomDrawBackground;
end;

function TcxCustomDateNavigator.GetCustomDrawDayCaption
  : TcxSchedulerDateNavigatorCustomDrawDayCaptionEvent;
begin
  Result := DateNavigator.OnCustomDrawDayCaption;
end;

function TcxCustomDateNavigator.GetCustomDrawDayNumber
  : TcxSchedulerDateNavigatorCustomDrawDayNumberEvent;
begin
  Result := DateNavigator.OnCustomDrawDayNumber;
end;

function TcxCustomDateNavigator.GetCustomDrawContent
  : TcxSchedulerDateNavigatorCustomDrawContentEvent;
begin
  Result := DateNavigator.OnCustomDrawContent;
end;

function TcxCustomDateNavigator.GetCustomDrawHeader
  : TcxSchedulerDateNavigatorCustomDrawHeaderEvent;
begin
...
function TcxCustomDateNavigator.GetPeriodChanged
...
function TcxCustomDateNavigator.GetSelectionChanged
  : TcxSchedulerPeriodChangedEvent;
begin
...
procedure TcxCustomDateNavigator.SetScheduler(AValue: TcxCustomScheduler);
begin
  if AValue = FScheduler then
    Exit;
  ...
  begin
    ...
    LookAndFeel.MasterLookAndFeel := TcxInnerDateNavigator(FScheduler)
      .LookAndFeel;
    SchedulerChanged;
  end
  else
...
procedure TcxCustomDateNavigator.SetShowDatesContainingHolidaysInColor
  (AValue: Boolean);
begin
...
procedure TcxCustomDateNavigator.SetStartOfWeek(AValue: TcxStartOfWeek);
begin
  FInnerDateNavigator.OptionsView.StartOfWeek := AValue;
end;

procedure TcxCustomDateNavigator.SetStorage(AValue: TcxCustomSchedulerStorage);
begin
...
begin
  ...
  TcxDateNavigatorAccess(DateNavigator).UnlimitedSelection := AValue and
    (Scheduler = nil);
end;
