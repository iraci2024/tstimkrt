...
{$ENDIF}
  ...
  cxSchedulerRecurrence, dxLayoutControlAdapters, dxLayoutcxEditAdapters,
...
type
  ...
  TcxSchedulerRecurrenceEventEditorHelper = class
  ...
  public
    property IsInternalActivation: Boolean read FIsInternalActivation
  ...
  public
    ...
    property NeedCheckLossOfExceptions: Boolean read FNeedCheckLossOfExceptions
      write SetNeedCheckLossOfExceptions;
    property IsInternalActivation: Boolean read GetIsInternalActivation
      write SetIsInternalActivation;
  end;

  { TcxSchedulerRecurrenceEventEditor }

  TcxSchedulerRecurrenceEventEditorForm = class
    (TcxSchedulerCustomRecurrenceEventEditor)
  ...
  private
    ...
    procedure SetItemIndexWithoutChangeEvent(AEdit: TcxComboBox;
      AIndex: Integer);
    procedure SetMonthlyPatternFromEvent;
    procedure SetPatternFromEvent;
    procedure SetTextWithoutChangeEvent(AEdit: TcxCustomMaskEdit;
      const AText: string);
  ...
  protected
    ...
    procedure SaveRecurrenceInfo
      (AInfo: TcxSchedulerEventRecurrenceInfo); virtual;
    ...
    property ActivePattern: TcxRecurrence read GetActivePattern
      write SetActivePattern;
  ...
  TcxSchedulerRecurrenceEventEditor = TcxSchedulerRecurrenceEventEditorForm;
  // workaround for Delphi 2005

  TcxSchedulerRecurrenceEventEditorClass = class of
    TcxSchedulerCustomRecurrenceEventEditor;
...
procedure TcxSchedulerRecurrenceEventEditorHelper.Cancel(var AModalResult
  : TModalResult);
begin
  inherited;
...
procedure TcxSchedulerRecurrenceEventEditorHelper.Delete(var AModalResult
  : TModalResult);
begin
  inherited;
...
procedure TcxSchedulerRecurrenceEventEditorHelper.Save(var AModalResult
  : TModalResult);
begin
  if ReadOnly then
    Exit;
...
begin
  inherited;
  FNeedCheckLossOfExceptions := (Event.Pattern <> nil) and
...
function TcxSchedulerCustomRecurrenceEventEditor.
  GetIsInternalActivation: Boolean;
begin
...
procedure TcxSchedulerCustomRecurrenceEventEditor.SetIsInternalActivation
  (AValue: Boolean);
begin
...
begin
  Result := not FNeedCheckLossOfExceptions or
    (MessageDlg(cxGetResourceString(@scxConfirmLostExceptions), mtWarning,
...
function TcxSchedulerRecurrenceEventEditorForm.GetRecurrenceInfoValidStatus
...
begin
  ...
  with AInfo.Event do
  begin
    AllDayEvent := (dxTimeOf(AStart) = 0) and (dxTimeOf(AFinish) = 0) and
      (AFinish - AStart >= 1);
    Start := AStart;
    Finish := AFinish;
  end;
  ...
  case ActivePattern of
    cxreDaily:
      SaveDailyParams(AInfo);
    cxreWeekly:
      SaveWeeklyParams(AInfo);
    cxreMonthly:
      SaveMonthlyParams(AInfo);
    cxreYearly:
      SaveYearlyParams(AInfo);
  end;
  if not Locked then
    AInfo.Validate;
end;
...
begin
  ...
  SetTextWithoutChangeEvent(cbDuration,
    cxMinutesToTextProc(Round(ADuration / MinuteToTime)));
end;
...
procedure TcxSchedulerRecurrenceEventEditorForm.CheckViewRecurrence
  (AResetPattern: Boolean);
begin
...
function TcxSchedulerRecurrenceEventEditorForm.GetRecurrenceInfo
...
function TcxSchedulerRecurrenceEventEditorForm.GetNumber
...
begin
  ...
  if not(cbDayOfWeek1.Checked or cbDayOfWeek2.Checked or cbDayOfWeek3.Checked or
    cbDayOfWeek4.Checked or cbDayOfWeek5.Checked or cbDayOfWeek6.Checked or
    cbDayOfWeek7.Checked) then
  begin
...
begin
  ...
  Result := (ADay >= 1) and
    (ADay <= DaysPerMonth(2000, cbMonths.ItemIndex + 1));
  if not Result then
...
begin
  ...
  begin
    rbThe.Checked := True;
    SetTextWithoutChangeEvent(meNumMonth1,
      IntToStr(RecurrenceInfo.Periodicity));
...
begin
  ...
  case RecurrenceInfo.Recurrence of
    cxreDaily:
      SetDailyPatternFromEvent;
    cxreWeekly:
      SetWeeklyPatternFromEvent;
    cxreMonthly:
      SetMonthlyPatternFromEvent;
    cxreYearly:
      SetYearlyPatternFromEvent;
  end;
end;
...
procedure TcxSchedulerRecurrenceEventEditorForm.
  deStartPropertiesEditValueChanged(Sender: TObject);
begin
  UpdateRange;
end;
    ...
    begin
      case GetRecurrenceInfoValidStatus of
        ...
        rvsReplaceOccurrenceDate:
          if CheckLostExceptions and
            (MessageDlg(Format(cxGetResourceString(@scxReplaceOccurrenceDate),
            [meNumOfDay.Text]), mtWarning, [mbOK, mbCancel], 0) = mrOk) then
            ModalResult := mrOk;
        rvsInvalidDuration:
          MessageDlg(cxGetResourceString(@scxInvalidRecurrenceDuration),
            mtWarning, [mbOK], 0);
      else
...
procedure TcxSchedulerRecurrenceEventEditorForm.
  deEndByPropertiesEditValueChanged(Sender: TObject);
begin
  if Locked then
    Exit;
  SetEndAfter;
...
procedure TcxSchedulerRecurrenceEventEditorForm.
  meEndAfterPropertiesEditValueChanged(Sender: TObject);
begin
  if Locked then
    Exit;
  SetEndDate;
...
begin
  ...
  begin
    ...
    SetTimeWithoutChangeEvent(teEnd,
      dxTimeOf(teStart.Time + AMinutes * MinuteToTime));
  end
  else
