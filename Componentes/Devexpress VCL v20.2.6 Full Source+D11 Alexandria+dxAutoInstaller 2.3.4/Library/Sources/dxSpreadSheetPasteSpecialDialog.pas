...
{$ENDIF}
  ...
  dxLayoutControl, dxLayoutLookAndFeels, Menus,
  dxLayoutControlAdapters, StdCtrls, cxButtons, dxSpreadSheetCore,
  dxSpreadSheetClipboard, dxSpreadSheetClipboardFormats,
  dxSpreadSheetTypes, cxContainer, cxEdit, cxListBox, dxLayoutcxEditAdapters,
  cxCheckBox, cxLabel, cxRadioGroup, dxForms,
  cxCustomListBox;
...
type
  ...
  TdxSpreadSheetPasteSpecialDialogFormClass = class of
    TdxSpreadSheetPasteSpecialDialogForm;
  ...
  protected
    ...
    procedure Paste(AFormat: TdxSpreadSheetCustomClipboardFormatClass;
      APasteOptions: TdxSpreadSheetClipboardPasteOptions); overload;
    procedure PopulatePasteFormats;
    procedure UpdateControlsState;
    //
    property SelectedFormat: TdxSpreadSheetCustomClipboardFormatClass
      read GetSelectedFormat;
    property SelectedPasteOptions: TdxSpreadSheetClipboardPasteOptions
      read GetSelectedPasteOptions;
    property SpreadSheet: TdxCustomSpreadSheet read FSpreadSheet;
    property Table: TdxSpreadSheetTableView read GetTable;
  end;

var
  dxSpreadSheetPasteSpecialDialogClass
    : TdxSpreadSheetPasteSpecialDialogFormClass =
    TdxSpreadSheetPasteSpecialDialogForm;
...
begin
  ...
  liPasteFormat.Caption := cxGetResourceString
    (@sdxPasteSpecialDialogPasteFormat);
  lbPasteOptions.Caption := cxGetResourceString
    (@sdxPasteSpecialDialogPasteOptions);

  cbColumnWidths.Caption := cxGetResourceString
    (@sdxPasteSpecialDialogPasteColumnWidths);
  cbComments.Caption := cxGetResourceString
    (@sdxPasteSpecialDialogPasteComments);
  cbFormulas.Caption := cxGetResourceString
    (@sdxPasteSpecialDialogPasteFormulas);
  cbSkipBlanks.Caption := cxGetResourceString
  ...
  rbNumberFormatting.Caption := cxGetResourceString
    (@sdxPasteSpecialDialogPasteStylesNumberFormatting);
end;

procedure TdxSpreadSheetPasteSpecialDialogForm.Initialize
...
begin
  ...
  begin
    ...
    begin
      TdxSpreadSheetBinaryClipboardFormatClass(AFormat)
        .GetInfoFromClipboard(AViewGUID, ACopyMode, AClipboardArea);
      TdxSpreadSheetTableViewAccess(Table).CheckPasteSelection(AClipboardArea,
        ACopyMode);
    end
    else
    begin
    ...
    try
      ...
      try
        TdxSpreadSheetTableViewClipboardHelper.PasteData(Table,
          AFormat.LoadFromClipboard, AClipboardArea, ACopyMode, AViewGUID,
          APasteOptions);
      finally
...
function TdxSpreadSheetPasteSpecialDialogForm.GetSelectedFormat
  : TdxSpreadSheetCustomClipboardFormatClass;
begin
  Result := TdxSpreadSheetCustomClipboardFormatClass(lbFormats.ItemObject);
end;

function TdxSpreadSheetPasteSpecialDialogForm.GetSelectedPasteOptions
...
begin
  Result := (SelectedFormat <> nil) and
    (SelectedPasteOptions - [cpoSkipBlanks] <> []);
end;

procedure TdxSpreadSheetPasteSpecialDialogForm.lbFormatsDblClick
  (Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TdxSpreadSheetPasteSpecialDialogForm.PasteOptionsChanged
  (Sender: TObject);
begin
  UpdateControlsState;
end;
