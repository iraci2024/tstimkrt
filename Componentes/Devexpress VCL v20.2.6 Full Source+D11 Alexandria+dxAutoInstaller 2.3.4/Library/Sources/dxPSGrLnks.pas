...
type
  TdxGridPaintOption = (gpoBorder, gpoHorzLines, gpoVertLines,
    gpoFixedHorzLines, gpoFixedVertLines);
  ...
  public
    ...
    property Options: TdxGridPaintOptions read GetOptions write SetOptions
      default [gpoBorder, gpoHorzLines, gpoVertLines, gpoFixedHorzLines,
      gpoFixedVertLines];
    property Soft3D;
  ...
  TdxCustomDrawItemEvent = procedure(Sender: TBasedxReportLink; Index: Integer;
  ...
  public
    ...
    property OnCustomDrawItem: TdxCustomDrawItemEvent read FOnCustomDrawItem
      write FOnCustomDrawItem;
  end;

const
  dxDefaultGridPaintOptions: TdxGridPaintOptions = [gpoBorder, gpoHorzLines,
    gpoVertLines, gpoFixedHorzLines, gpoFixedVertLines];
...
function GridGetRowHeight(ACustomGrid: TCustomGrid; ARowIndex: Integer)
  : Integer;
begin
...
function ListBoxGetItemText(ACustomListBox: TCustomListBox;
...
function ListBoxMeasureItem(ACustomListBox: TCustomListBox;
...
begin
  inherited;
  Options := dxDefaultGridPaintOptions;
  { [Low(TdxDrawGridPaintOption)..High(TdxDrawGridPaintOption)] }
end;

procedure TCustomdxGridReportLink.InternalRestoreFromOriginal;

  procedure XorOption(var AOptions: TdxGridPaintOptions;
    AElement: TdxGridPaintOption; Value: Boolean);
  begin
    if Value then
...
begin
  ...
  Opt := Options;
  XorOption(Opt, gpoFixedVertLines, goFixedVertLine
    in GridGetOptions(CustomGrid));
  XorOption(Opt, gpoFixedHorzLines, goFixedHorzLine
    in GridGetOptions(CustomGrid));
...
function TCustomdxGridReportLink.GetSourceCellColor(ACol,
  ARow: Integer): TColor;
begin
...
function TCustomdxGridReportLink.GetSourceCellEdgeMode(ACol, ARow: Integer)
  : TdxCellEdgeMode;
const
...
function TCustomdxGridReportLink.GetSourceCellFontIndex(ACol,
  ARow: Integer): Integer;
begin
  Result := FSourceFontIndex;
end;

function TCustomdxGridReportLink.GetSourceCellTransparent(ACol,
  ARow: Integer): Boolean;
begin
  Result := not IsFixedCell(ACol, ARow);
end;

procedure TCustomdxGridReportLink.PrepareConstruct(AReportCells
  : TdxReportCells);
begin
  inherited;
...
function TdxCustomListBoxReportLink.IsSelectedCell(ACol, ARow: Integer)
  : Boolean;
begin
...
procedure TdxCustomListBoxReportLink.CustomDraw
...
begin
  ...
  begin
    ...
    DoCustomDrawItem(AItem.Parent.Index, ACanvas, ABoundsRect, AClientRect,
      AText, FSaveFont, AColor, ATextAlignX, ATextAlignY, ADone);
    if not ADone then
    begin
...
function TdxCustomListBoxReportLink.IsSupportedCustomDraw
  (AItem: TAbstractdxReportCellData): Boolean;
begin
  Result := inherited IsSupportedCustomDraw(AItem) and
    Assigned(FOnCustomDrawItem);
end;
...
function TdxCustomListBoxReportLink.GetCellTextAlignX(ACol, ARow: Integer)
  : TcxTextAlignX;
begin
  Result := FTextAlignX;
end;

function TdxCustomListBoxReportLink.GetCellTextAlignY(ACol, ARow: Integer)
  : TcxTextAlignY;
begin
  Result := FTextAlignY;
end;

function TdxCustomListBoxReportLink.GetCellSides(ACol, ARow: Integer)
  : TdxCellSides;
...
begin
  ...
  begin
    ...
    if IsFirstItem(ARow) then
      Exclude(Result, csTop);
    if IsLastItem(ARow) then
      Exclude(Result, csBottom);
  end;
  if not IsDrawHorzLines then
  begin
    if not IsFirstItem(ARow) then
      Exclude(Result, csTop);
    if not IsLastItem(ARow) then
      Exclude(Result, csBottom);
  end;
end;
