...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  ImgList, Types, Windows, Messages, SysUtils, Classes, Graphics, Controls,
  Forms,
...
type
  ...
  TdxImageDrawItemEvent = procedure(Sender: TObject; Index: Integer;
  ...
  protected
    ...
    property OnDrawItem: TdxImageDrawItemEvent read FOnDrawItem
      write FOnDrawItem;
  public
  ...
  public
    ...
    property Items[Index: Integer]: TdxSpinImageItem read GetItem
  ...
  private
    ...
    procedure WMNCLButtonDblClk(var Message: TWMNCLBUTTONDOWN);
      message WM_NCLBUTTONDBLCLK;
    procedure WMNCMouseDown(var Message: TWMNCLBUTTONDOWN);
...
{$ENDIF}
    ...
    property ImageHAlign: TdxHSpinImageAlign read FImageHAlign
      write SetImageHAlign;
    property ImageVAlign: TdxVSpinImageAlign read FImageVAlign
      write SetImageVAlign;
...
begin
  ...
  if (Operation = opRemove) and (FImageList <> nil) and (AComponent = FImageList)
  then
    ImageList := nil;
end;
...
begin
  if (Source is TdxCustomImageComboBox) Or (Source is TdxCustomImageListBox)
  then
  begin
    if (Source is TdxCustomImageComboBox) then
    begin
    ...
    if (Source is TdxCustomImageListBox) then
    begin
...
Var
  St: String;
begin
  if (Index < FStrings.Count) And (Index > -1) And (Value <> ImageIndexes[Index])
  then
  begin
    St := Values[Index];
...
Var
  St: String;
begin
  if (Index < FStrings.Count) And (Index > -1) And (Value <> Values[Index]) then
  begin
...
procedure TdxCustomImageListBox.InsertItem(Index: Integer; St: String;
  ImageIndex: Integer);
begin
  if (Index < 0) then
    Index := 0;
...
var
  ...
  St1, St2: string;
begin
  flag := (Index1 > -1) And (Index1 < Items.Count) And (Index2 > -1) And
    (Index2 < Items.Count);
  if (flag) then
  begin
    St1 := FStrings[Index1];
...
Var
  TempString: string;
begin
  if (CurIndex <> NewIndex) And (CurIndex > -1) And (CurIndex < Items.Count)
  then
  begin
...
begin
  ...
  begin
    ...
    AImageWidth := ((Result.Bottom - Result.Top) * FImageList.Width)
      div FImageList.Height;
...
const
  CHorizontalAlignFlags: array [TAlignment] of Integer = (DT_LEFT, DT_RIGHT,
    DT_CENTER);
  CVerticalAlignFlags: array [TVertAlignment] of Integer = (DT_TOP, DT_VCENTER,
    DT_BOTTOM);
var
  R, R1: TRect;
  AImage: TBitmap;
  ...
  ADrawFlags: Integer;
begin
  AImageFlag := (FImageList <> nil) and (ImageIndexes[Index] > -1) and
    (ImageIndexes[Index] < FImageList.Count);
  ...
  begin
    ...
    if FMultiLines then
      ...
      ADrawFlags := ADrawFlags or CVerticalAlignFlags[VertAlignment] or
        DT_SINGLELINE;
...
begin
  if (FImageList <> Nil) then
  begin
    if (Index > -1) And (FDrawEdgeIndex <> Index) then
    begin
      if (FDrawEdgeIndex > -1) And (ImageIndexes[FDrawEdgeIndex] > -1) And
        (ImageIndexes[FDrawEdgeIndex] < FImageList.Count) then
      begin
...
begin
  for Result := 0 to FStrings.Count - 1 do
    if AnsiCompareText(Text, Values[Result]) = 0 then
      Exit;
  Result := -1;
end;
...
Var
  St: String;
begin
  if (Index < FStrings.Count) And (Value <> ImageIndexes[Index]) then
  begin
    St := Values[Index];
    FStrings[Index] := IntToStr(Value) + ',' + St;
  end;
...
procedure TdxCustomImageComboBox.InsertItem(Index: Integer; St: String;
  ImageIndex: Integer);
begin
  if (Index < 0) then
    Index := 0;
...
begin
  ...
  begin
    ...
    AImageWidth := ((R.Bottom - R.Top) * FImageList.Width)
      div FImageList.Height;
    if AImageAlign = dxliLeft then
    begin
      R.Left := Rect.Left - 1;
  ...
  if (FImageList <> nil) and (ImageIndexes[Index] > -1) and
    (ImageIndexes[Index] < FImageList.Count) then
  begin
...
begin
  if Source is TdxSpinImageItem then
  begin
...
begin
  if (Value <> FAutoSize) And (FImageList <> Nil) then
  begin
    FAutoSize := Value;
    if (FAutoSize) then
      MakeAutoSize;
...
begin
  if (Value <> FDefaultImages) then
  begin
    FDefaultImages := Value;
    if Not(csLoading in ComponentState) then
    begin
      if (Value And (FImageList <> Nil) And (FImageList.Count > 0)) Or
        (Not Value And (Items.Count > 0)) then
        ItemIndex := 0
      else
        ItemIndex := -1;
      UpdateItems;
...
begin
  ...
  if (Value >= -1) And (FItemIndex <> Value) And (FImageList <> Nil) And
    ((FDefaultImages And (Value < FImageList.Count)) Or
    (Not FDefaultImages And (Value < Items.Count))) then
  begin
    FItemIndex := Value;
    Change;
    Paint;
...
begin
  if (FUpDownOrientation <> Value) then
  begin
  ...
  case AKey of
    ...
    VK_UP, VK_LEFT, VK_HOME:
      begin
        ...
        if ItemIndex = 0 then
          Exit;
        if (FDefaultImages and (FImageList.Count > 0)) or
          (not FDefaultImages and (Items.Count > 0)) then
        begin
...
begin
  Result := not(FDefaultImages and (FItemIndex < FImageList.Count - 1)) Or
    (Not FDefaultImages and (FItemIndex < Items.Count - 1));
end;

procedure TdxCustomSpinImage.SetNextItem;
begin
  if CanChange And ((FDefaultImages And (FImageList.Count > 0)) Or
    (Not FDefaultImages And (Items.Count > 0))) then
  begin
...
const
  ...
  UpLeftFlag: Array [False .. True] of Integer = (DFCS_SCROLLLEFT,
    DFCS_SCROLLUP);
  DownRightFlag: Array [False .. True] of Integer = (DFCS_SCROLLRIGHT,
    DFCS_SCROLLDOWN);
begin
  inherited;
  ...
  case AUpDownAlign of
    udaBottom:
      R.Top := R.Bottom - FUpDownWidth;
    udaLeft:
      R.Right := R.Left + FUpDownWidth;
    udaRight:
      R.Left := R.Right - FUpDownWidth;
    udaTop:
      R.Bottom := R.Top + FUpDownWidth;
  end;
  FUpButtonRect := R;
  FDownButtonRect := R;
  ...
  if (FUpDownOrientation = siVertical) then
    ...
    FUpButtonRect.Right := R.Right - (R.Right - R.Left) div 2;
  if (FUpDownOrientation = siVertical) then
    FDownButtonRect.Top := R.Bottom - (R.Bottom - R.Top) div 2
  else
    FDownButtonRect.Left := R.Left + (R.Right - R.Left) div 2 + 1;
  DrawFrameControl(DC, FUpButtonRect, DFC_SCROLL,
    UpLeftFlag[FUpDownOrientation = siVertical] or EnableFlag[FUpButtonEnabled]
    or FlatFlag[Ctl3D] or PressFlag[FUpPress and FUpButtonEnabled]);
  DrawFrameControl(DC, FDownButtonRect, DFC_SCROLL,
    DownRightFlag[FUpDownOrientation = siVertical] or
    EnableFlag[FDownButtonEnabled] or FlatFlag[Ctl3D] or
    PressFlag[FDownPress and FDownButtonEnabled]);
...
begin
  ...
  begin
    ...
    begin
      ...
      else if PtInRect(FDownButtonRect, p) then
        UpDownClick(VK_DOWN)
      else
        FScrollTimerCount := 0;
    end
    else
      Inc(FScrollTimerCount);
  end
  else
  begin
    FTimer.Enabled := False;
...
begin
  ...
  begin
    ...
    Brush.Color := Color;
    R := ClientRect;
    if (FImageList <> Nil) And (FItemIndex <> -1) then
    begin
      Image := TBitmap.Create;
      if FDefaultImages then
        FImageList.GetBitmap(ItemIndex, Image)
      else
        FImageList.GetBitmap(Items[ItemIndex].ImageIndex, Image);
      if (FStretch) then
        StretchDraw(R, Image)
      else
      begin
        ...
        begin
          ...
          case AImageHAlign of
            hsiCenter:
              Inc(FLeft, (R.Right - R.Left - FImageList.Width) div 2);
            hsiRight:
              Inc(FLeft, R.Right - R.Left - FImageList.Width);
          end;
        end;
        if (R.Bottom - R.Top > FImageList.Height) then
          case FImageVAlign of
            vsiCenter:
              Inc(FTop, (R.Bottom - R.Top - FImageList.Height) div 2);
            vsiBottom:
              Inc(FTop, R.Bottom - R.Top - FImageList.Height);
          end;
    ...
    if (GetParentForm(self) <> nil) and
      (GetParentForm(self).ActiveControl = self) and
...
begin
  Result := Enabled and not FReadOnly and (FImageList <> nil) and
    (FImageList.Count > 0);
end;

procedure TdxCustomSpinImage.Change;
begin
  if Not FDefaultImages And (FItemIndex > -1) And
    Not(csDesigning in ComponentState) then
  begin
