...
type
  ...
  protected
    ...
    procedure SetPageProperty<T>(const ASetter: TSetValueDelegate<T>;
      const Value: T);
  ...
  protected
    function CreateNonUniformColumns
      : IdxRichEditSectionColumnCollection; virtual;
    function IsEqualWidthColumns(const AColumns
      : IdxRichEditSectionColumnCollection): Boolean; virtual;
    procedure SetUniformColumns(ASpacing: Single;
      AColumnCount: Integer); virtual;
    procedure SetNonUniformColumns(const AColumns
      : IdxRichEditSectionColumnCollection); virtual;
  public
    constructor Create(ADocument: TdxNativeDocument;
  ...
  TdxNativeSectionColumnCollection = class
    (TdxIUnknownList<IdxRichEditSectionColumn>,
  ...
  TdxNativeSectionLineNumbering = class(TInterfacedObject,
  ...
  TdxNativeSectionPageNumbering = class(TInterfacedObject,
  ...
  TdxNativeSectionParagraphCollection = class(TInterfacedObject,
  ...
  public
    constructor Create(const AParagraphs: IdxRichEditParagraphCollection;
...
constructor TdxNativeSectionMargins.Create(ADocument: TdxNativeDocument;
  AMargins: TdxSectionMargins);
begin
  inherited Create;
  FDocument := ADocument;
...
constructor TdxNativeSectionPage.Create(ADocument: TdxNativeDocument;
...
procedure TdxNativeSectionPage.SetPageProperty<T>(const ASetter
  : TSetValueDelegate<T>; const Value: T);
begin
...
begin
  ...
  begin
    ...
    APaperSizeInModelUnits := FDocument.DocumentModel.UnitConverter.
      TwipsToModelUnits(APaperSizeInTwips);
...
begin
  if Columns.EqualWidthColumns then
    Result := CreateUniformColumns(Page,
      Document.ModelUnitsToUnits(Columns.Space), Count)
  else
    Result := CreateNonUniformColumns;
end;

procedure TdxNativeSectionColumns.SetColumns(const AColumns
  : IdxRichEditSectionColumnCollection);
var
  ACount: Integer;
begin
...
function TdxNativeSectionColumns.CreateUniformColumns(AColumnWidth,
  AColumnSpacing: Single; AColumnCount: Integer)
  : IdxRichEditSectionColumnCollection;
var
...
function TdxNativeSectionColumns.CreateUniformColumns
  (const APage: IdxRichEditSectionPage; AColumnSpacing: Single;
...
begin
  ...
  AColumnWidth := (APage.Width - (AColumnSpacing * (AColumnCount - 1))) /
    AColumnCount;
...
function TdxNativeSectionColumns.CreateNonUniformColumns
  : IdxRichEditSectionColumnCollection;
var
  AResult: TdxNativeSectionColumnCollection;
...
function TdxNativeSectionColumns.IsEqualWidthColumns(const AColumns
  : IdxRichEditSectionColumnCollection): Boolean;
var
  ACount, I: Integer;
...
procedure TdxNativeSectionColumns.SetUniformColumns(ASpacing: Single;
...
procedure TdxNativeSectionColumns.SetNonUniformColumns(const AColumns
  : IdxRichEditSectionColumnCollection);
var
  ACount, I: Integer;
  AInnerColumns: TdxColumnInfoCollection;
...
constructor TdxNativeSectionLineNumbering.Create(ADocument: TdxNativeDocument;
  ALineNumbering: TdxSectionLineNumbering);
begin
  inherited Create;
  FDocument := ADocument;
...
function TdxNativeSectionLineNumbering.GetRestartType
  : TdxRichEditLineNumberingRestart;
begin
  Result := LineNumbering.NumberingRestartType;
end;

procedure TdxNativeSectionLineNumbering.SetRestartType
  (const Value: TdxRichEditLineNumberingRestart);
begin
...
constructor TdxNativeSectionPageNumbering.Create(ADocument: TdxNativeDocument;
  APageNumbering: TdxSectionPageNumbering);
begin
  inherited Create;
  FDocument := ADocument;
...
procedure TdxNativeSectionPageNumbering.SetFirstPageNumber
  (const Value: Integer);
begin
  Assert(Value >= 0);
  FPageNumbering.FirstPageNumber := Value;
...
procedure TdxNativeSectionPageNumbering.SetContinueNumbering
  (const Value: Boolean);
begin
  FPageNumbering.ContinueNumbering := Value;
end;

function TdxNativeSectionPageNumbering.GetNumberingFormat
  : TdxRichEditNumberingFormat;
begin
  Result := FPageNumbering.NumberingFormat;
end;

procedure TdxNativeSectionPageNumbering.SetNumberingFormat
...
constructor TdxNativeSectionParagraphCollection.Create(const AParagraphs
  : IdxRichEditParagraphCollection; AInnerSection: TdxSection);
begin
  inherited Create;
...
begin
  Result := FInnerSection.LastParagraphIndex -
    FInnerSection.FirstParagraphIndex + 1;
end;

function TdxNativeSectionParagraphCollection.GetItem(Index: Integer)
  : IdxRichEditParagraph;
begin
  Result := FParagraphs[FInnerSection.FirstParagraphIndex + Index];
end;

function TdxNativeSectionParagraphCollection.Get(const ARange
...
begin
  ...
  begin
    ...
    if (AParagraph.Range.Start.CompareTo(ARange.Start) >= 0) and
      (AParagraph.Range.&End.CompareTo(ARange.&End) <= 0) then
...
constructor TdxNativeSection.Create(ADocument: TdxNativeDocument;
  AInnerSection: TdxSection);
begin
  inherited Create;
  FDocument := ADocument;
  FInnerSection := AInnerSection;
  FParagraphs := TdxNativeSectionParagraphCollection.Create(Document.Paragraphs,
    InnerSection);
  ...
  FColumns := TdxNativeSectionColumns.Create(Document,
    AInnerSection.Columns, FPage);
  FLineNumbering := TdxNativeSectionLineNumbering.Create(Document,
    AInnerSection.LineNumbering);
  FPageNumbering := TdxNativeSectionPageNumbering.Create(Document,
    AInnerSection.PageNumbering);
  FIsValid := True;
end;
...
procedure TdxNativeSection.SetStartType(const Value
...
function TdxNativeSection.BeginUpdateHeader(AType: TdxRichEditHeaderFooterType)
...
begin
  ...
  Result := CreateSubDocument(TdxPieceTable(AHeader.PieceTable),
    FDocument.DocumentServer);
end;

procedure TdxNativeSection.EndUpdateHeader(const ADocument
  : IdxRichEditSubDocument);
begin
  CheckValid;
...
function TdxNativeSection.HasHeader(AType: TdxRichEditHeaderFooterType)
  : Boolean;
begin
...
function TdxNativeSection.BeginUpdateFooter(AType: TdxRichEditHeaderFooterType)
  : IdxRichEditSubDocument;
var
...
begin
  ...
  Result := CreateSubDocument(TdxPieceTable(AFooter.PieceTable),
    FDocument.DocumentServer);
end;

procedure TdxNativeSection.EndUpdateFooter(const ADocument
  : IdxRichEditSubDocument);
begin
  FInnerSection.DocumentModel.EndUpdate;
end;

function TdxNativeSection.HasFooter(AType: TdxRichEditHeaderFooterType)
  : Boolean;
begin
...
function TdxNativeSection.IsHeaderLinkedToPrevious
  (AType: TdxRichEditHeaderFooterType): Boolean;
begin
  Result := IsLinkedToPrevious(AType, FInnerSection.Headers);
end;

procedure TdxNativeSection.LinkHeaderToPrevious
  (AType: TdxRichEditHeaderFooterType);
begin
...
function TdxNativeSection.IsFooterLinkedToPrevious
  (AType: TdxRichEditHeaderFooterType): Boolean;
begin
...
function TdxNativeSection.IsHeaderLinkedToNext
  (AType: TdxRichEditHeaderFooterType): Boolean;
begin
...
function TdxNativeSection.IsFooterLinkedToNext
  (AType: TdxRichEditHeaderFooterType): Boolean;
begin
...
begin
  if not FIsValid then
    TdxRichEditExceptions.ThrowInvalidOperationException
      (cxGetResourceString(@sdxRichEditExceptionUseDeletedSectionError));
end;
...
begin
  ...
  try
    ...
    Result := (AHeaderFooter <> nil) and AHeaderFooter.GetContainer
      (InnerSection).IsLinkedToPrevious(AHeaderFooter.&Type)
  finally
    FInnerSection.DocumentModel.EndUpdate;
  end;
end;
...
procedure TdxNativeSection.UnlinkFromPrevious
  (AType: TdxRichEditHeaderFooterType;
  AHeadersFooters: TdxSectionHeadersFootersBase);
var
  AHeaderFooter: TdxSectionHeaderFooterBase;
...
begin
  ...
  try
    AHeaderFooter := AHeadersFooters.GetObjectCore(AType);
    Result := (AHeaderFooter <> nil) and AHeaderFooter.GetContainer
      (InnerSection).IsLinkedToNext(AHeaderFooter.&Type);
  finally
    FInnerSection.DocumentModel.EndUpdate;
  end;
end;
