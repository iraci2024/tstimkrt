...
type
  TcxGridServerModeDefaultValuesProvider = class
  ...
  protected
    ...
    procedure InitializeDescriptor(AField: TcxCustomDataField;
      var ADescriptor: TdxServerModeDescriptor); override;
  ...
  public
    ...
    // IcxCustomGridDataController
    function GetItemByFieldName(const AFieldName: string)
  ...
  public
    ...
    function DefaultWidth(ATakeHeaderIntoAccount: Boolean = True)
      : Integer; override;
    property DataController: TcxGridServerModeDataController
...
uses
  ...
  cxStorage, cxGridCommon, cxGridLevel, Controls, Forms, RTLConsts;
...
function TcxGridServerModeDefaultValuesProvider.IsDisplayFormatDefined
...
function TcxGridServerModeDataController.GetController
  : TcxCustomGridTableController;
begin
  Result := GridView.Controller;
end;

function TcxGridServerModeDataController.GetGridViewValue
  : TcxCustomGridTableView;
begin
  Result := TcxCustomGridTableView(GetGridView);
end;

procedure TcxGridServerModeDataController.AssignData(ADataController
...
function TcxGridServerModeDataController.DoScrollPage
...
function TcxGridServerModeDataController.GetItemDataBindingClass
  : TcxGridItemDataBindingClass;
begin
  Result := TcxGridItemServerModeDataBinding;
end;

function TcxGridServerModeDataController.GetItemDefaultValuesProviderClass
  : TcxCustomEditDefaultValuesProviderClass;
begin
...
function TcxGridServerModeDataController.SetScrollBarPos
  (Value: Integer): Boolean;
begin
  Result := SupportsScrollBarParams;
end;

function TcxGridServerModeDataController.CanSelectRow
  (ARowIndex: Integer): Boolean;
begin
  Result := TcxCustomGridTableViewAccess.CanSelectRecord(GridView, ARowIndex);
end;

function TcxGridServerModeDataController.CompareByField(ARecordIndex1,
...
procedure TcxGridServerModeDataController.DoDataSetCurrentChanged
  (AIsCurrent: Boolean);
begin
  inherited;
...
procedure TcxGridServerModeDataController.DoValueTypeClassChanged
  (AItemIndex: Integer);
begin
  inherited;
...
begin
  Result := TcxCustomGridTableViewInfoAccess.GetDefaultGridModeBufferCount
    (GridView.ViewInfo);
end;

procedure TcxGridServerModeDataController.InitializeDescriptor
  (AField: TcxCustomDataField; var ADescriptor: TdxServerModeDescriptor);
const
  AConvertMap: array [TcxGridDateTimeGrouping] of TdxDateTimeGrouping =
    (dgDefault, dgByDateAndTime, dgRelativeToToday, dgByHour, dgByDate,
    dgByMonth, dgByYear);
var
  AItem: TcxCustomGridTableItem;
begin
  AItem := GetItemByFieldName(GetItemFieldName(AField.Index));
  ADescriptor.DateTimeGrouping := AConvertMap[TCustomGridTableItemAccess(AItem)
    .GetDateTimeHandlingGrouping];
end;

function TcxGridServerModeDataController.GetFilterDisplayText(ARecordIndex,
...
function TcxGridServerModeDataController.DoGetGroupRowDisplayText
...
begin
  ...
  begin
    ...
    Result := GridView.ViewData.GetDisplayTextFromValue(ARecordIndex,
      AItemIndex, AValue);
    TCustomGridTableItemAccess(GridView.Items[AItemIndex])
      .DoGetDataText(ARecordIndex, Result);
  end
  else
    Result := '';
end;
...
function TcxGridServerModeDataController.GetSortingBySummaryEngineClass
  : TcxSortingBySummaryEngineClass;
begin
  Result := GridView.ViewData.GetSortingBySummaryEngineClass;
end;

function TcxGridServerModeDataController.GetSummaryGroupItemLinkClass
...
function TcxGridServerModeDataController.GetSummaryItemClass
...
procedure TcxGridServerModeDataController.PopulateFilterValues
...
begin
  ANullExists := False;
  if (ACriteria.MaxValueListCount < 0) or (ACriteria.MaxValueListCount > 0) and
    (AList.Count >= ACriteria.MaxValueListCount) then
    Exit;
  AValueList := TcxDataFilterValueList.Create(AList.Criteria);
  try
    inherited PopulateFilterValues(AValueList, AItemIndex, ACriteria,
    ...
    begin
      if (ACriteria.MaxValueListCount > 0) and
        (AList.Count >= ACriteria.MaxValueListCount) then
        Break;
      AItem := AValueList[I];
...
function TcxGridServerModeDataController.CreateDetailLinkObject
  (ARelation: TcxCustomDataRelation; ARecordIndex: Integer): TObject;
begin
  Result := TcxGridLevelAccess.CreateLinkObject(TcxGridLevel(ARelation.Item),
    ARelation, ARecordIndex);
end;

procedure TcxGridServerModeDataController.FocusControl(AItemIndex: Integer;
  var Done: Boolean);
begin
  inherited;
...
function TcxGridServerModeDataController.GetDetailDataControllerByLinkObject
  (ALinkObject: TObject): TcxCustomDataController;
begin
  Result := TcxCustomGridView(ALinkObject).DataController;
end;

function TcxGridServerModeDataController.GetDisplayText(ARecordIndex,
  AItemIndex: Integer): string;
begin
  ...
  TcxCustomGridTableItemAccess.DoGetDataText(GridView.Items[AItemIndex],
    ARecordIndex, Result);
end;

function TcxGridServerModeDataController.GetFilterDataValue
...
function TcxGridServerModeDataController.GetFilterItemFieldCaption
  (AItem: TObject): string;
begin
  Result := TcxCustomGridTableItemAccess.GetFilterCaption
...
function TcxGridServerModeDataController.GetItemSortByDisplayText
...
function TcxGridServerModeDataController.GetItemValueSource(AItemIndex: Integer)
  : TcxDataEditValueSource;
begin
  Result := TcxCustomGridTableViewAccess.GetItemValueSource(GridView,
    AItemIndex);
end;
...
procedure TcxGridServerModeDataController.CreateAllItems(AMissingItemsOnly
  : Boolean = False);
var
  I: Integer;
  AItem: TcxCustomGridTableItem;
...
function TcxGridServerModeDataController.GetItemByFieldName(const AFieldName
  : string): TcxCustomGridTableItem;
begin
...
function TcxGridItemServerModeDataBinding.GetDataController
  : TcxGridServerModeDataController;
begin
...
function TcxGridItemServerModeDataBinding.GetDefaultValueTypeClass
  : TcxValueTypeClass;
begin
  Result := nil;
end;
...
begin
  ...
  TcxGridServerModeDefaultValuesProvider(DefaultValuesProvider).FieldName :=
    FieldName;
end;
...
function TcxGridItemServerModeDataBinding.DefaultRepositoryItem
  : TcxEditRepositoryItem;
begin
  Result := GetDefaultEditDBRepositoryItems.GetItemByField(Field);
end;

function TcxGridItemServerModeDataBinding.DefaultWidth(ATakeHeaderIntoAccount
