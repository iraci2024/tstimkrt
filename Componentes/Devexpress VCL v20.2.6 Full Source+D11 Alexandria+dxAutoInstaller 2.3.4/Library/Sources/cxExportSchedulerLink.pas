...
uses
  Types, Windows, Classes, SysUtils, Graphics, Variants, cxDataUtils, cxClasses,
  cxGraphics, cxStyles,
  cxGeometry, cxSchedulerCustomControls, Math, cxSchedulerUtils,
  cxSchedulerStorage, cxSchedulerStrs, cxExport,
  Forms, Menus, cxLookAndFeelPainters, Controls, cxControls, cxContainer,
  cxEdit, cxTextEdit, cxMaskEdit, cxCalendar,
  cxDropDownEdit, StdCtrls, cxButtons, cxLookAndFeels, cxDateUtils, cxLabel,
  ComCtrls, dxCore, dxLayoutContainer,
  dxLayoutControl, dxLayoutLookAndFeels, dxLayoutcxEditAdapters,
  dxLayoutControlAdapters, dxForms;

procedure cxExportSchedulerToHTML(const AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable: Boolean = False;
  AShowDialog: Boolean = False; const AHeader: string = 'Event %d';
  const AStart: TDateTime = NullDate; const AFinish: TDateTime = NullDate;
  const AFileExt: string = 'html'; AHandler: TObject = nil);
procedure cxExportSchedulerToXML(const AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable: Boolean = False;
  AShowDialog: Boolean = False; const AHeader: string = 'Event %d';
  const AStart: TDateTime = NullDate; const AFinish: TDateTime = NullDate;
  const AFileExt: string = 'xml'; AHandler: TObject = nil);

procedure cxExportSchedulerToExcel(const AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable: Boolean = False;
  AShowDialog: Boolean = False; const AHeader: string = 'Event %d';
  const AStart: TDateTime = NullDate; const AFinish: TDateTime = NullDate;
  const AFileExt: string = 'xls'; AHandler: TObject = nil);
procedure cxExportSchedulerToXLSX(const AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable: Boolean = False;
  AShowDialog: Boolean = False; const AHeader: string = 'Event %d';
  const AStart: TDateTime = NullDate; const AFinish: TDateTime = NullDate;
  const AFileExt: string = 'xlsx'; AHandler: TObject = nil);

procedure cxExportSchedulerToCSV(const AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable: Boolean = False;
  AShowDialog: Boolean = False; const AHeader: string = 'Event %d';
  const AStart: TDateTime = NullDate; const AFinish: TDateTime = NullDate;
...
procedure cxExportSchedulerToText(const AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable: Boolean;
  AShowDialog: Boolean = False; const AStart: TDateTime = NullDate;
  const AFinish: TDateTime = NullDate; const AFileExt: string = 'txt';
...
procedure cxExportSchedulerToFile(AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable, AShowDialog: Boolean;
  const AHeader: string; AStart, AFinish: TDateTime; AExportType: Integer;
...
type
  ...
  protected
    ...
    procedure ExportEvent(AEvent: TcxSchedulerEvent; AIndex: Integer;
      var ARow: Integer); virtual;
    procedure ExportEvents; virtual;
    function RegisterCellStyle(AColor, AFontColor: TColor;
      AFontStyle: TFontStyles; ABorders, ABoldBorders: TcxBorders;
      AAlignText: TcxAlignText = catLeft): Integer;
  public
    constructor Create(AScheduler: TcxCustomScheduler;
      const AStart, AFinish: TDateTime; AExportType: Integer;
  ...
  public
    constructor CreateEx(ALookAndFeel: TcxLookAndFeel;
      AStart, AFinish: TDateTime); virtual;
  end;
...
const
  ...
  States: array [0 .. 3] of Pointer = (@scxFree, @scxTentative, @scxBusy,
    @scxOutOfOffice);
...
const
  NecessaryFields: array [0 .. 12] of Pointer = (@secxSubject, @secxStartDate,
    @secxStartTime, @secxEndDate, @secxEndTime, @secxAllDayEvent,
    @secxReminderOnOff, @secxReminderDate, @secxReminderTime, @secxCategories,
    @secxDescription, @secxLocation, @secxShowTimeAs);
...
function ShowSetRangeDialog(ALookAndFeel: TcxLookAndFeel; var AStart: TDateTime;
  var AFinish: TDateTime): Boolean;
begin
...
procedure TcxSchedulerExportHelper.ExportEvent(AEvent: TcxSchedulerEvent;
  AIndex: Integer; var ARow: Integer);

  procedure WriteSingleRow(const ACaption: string; const AData: Variant;
    ABorders: TcxBorders);
  begin
    Provider.SetCellStyle(0, ARow, RegisterCellStyle(clDefault, clDefault,
      [fsBold], cxBordersAll, ABorders));
    Provider.SetCellStyle(1, ARow, RegisterCellStyle(clDefault, clDefault, [],
      cxBordersAll, ABorders - [bLeft]));
...
const
  ATopRowBorders: array [Boolean] of TcxBorders = ([bLeft, bTop, bRight],
    [bLeft, bRight]);
begin
  if HasHeader then
  begin
    Provider.SetCellStyle(cxRectBounds(0, ARow, 1, 2),
      RegisterCellStyle(clDefault, clDefault, [fsBold], cxBordersAll,
      [bRight, bLeft, bTop], catCenter));
    Provider.SetCellValue(0, ARow, Format(Header, [AIndex]));
    Inc(ARow);
  end;
  WriteSingleRow(cxGetResourceString(@secxSubject), AEvent.Caption,
    ATopRowBorders[HasHeader]);
  WriteSingleRow(cxGetResourceString(@secxLocation), AEvent.Location,
    [bLeft, bRight]);
  WriteSingleRow(cxGetResourceString(@secxDescription), AEvent.Message,
    [bLeft, bRight]);
  WriteSingleRow(cxGetResourceString(@secxAllDay),
    YesNoToStr(AEvent.AllDayEvent), [bLeft, bRight]);
  WriteSingleRow(cxGetResourceString(@secxStart), AEvent.Start,
    [bLeft, bRight]);
  WriteSingleRow(cxGetResourceString(@secxFinish), AEvent.Finish,
    [bLeft, bRight]);
  WriteSingleRow(cxGetResourceString(@secxState), StateToStr(AEvent.State),
    [bLeft, bBottom, bRight]);
  WriteSingleRow(cxGetResourceString(@secxReminder), BoolToStr(AEvent.Reminder),
    [bLeft, bBottom, bRight]);
  WriteSingleRow(cxGetResourceString(@secxReminderDate),
    VarToDateTime(AEvent.ReminderDate), [bLeft, bRight]);
  WriteSingleRow(cxGetResourceString(@secxCategories),
    LabelColorToStr(AEvent.LabelColor), [bLeft, bRight]);
  Provider.SetCellStyle(0, ARow, RegisterCellStyle(clDefault, clDefault,
    [], [], []));
  Provider.SetCellStyle(1, ARow, RegisterCellStyle(clDefault, clDefault,
    [], [], []));
...
procedure TcxSchedulerExportHelper.ProgressHandler(Sender: TObject;
  Percents: Integer);
var
...
begin
  CalculateLayout;
  Style := RegisterCellStyle(clDefault, clDefault, [fsBold], cxBordersAll, [],
    catCenter);
  if FNameProvider <> nil then
  begin
    FNameProvider.SetName('Calendar');
    FNameProvider.SetRangeName('Calendar',
      Rect(0, 0, Length(NecessaryFields) - 1, Events.Count));
  end;
...
procedure TcxSchedulerTableExportHelper.ExportEvent(AEvent: TcxSchedulerEvent;
  AIndex: Integer; var ARow: Integer);
var
  ACol: Integer;
...
procedure cxExportSchedulerToFile(AFileName: string;
  AScheduler: TcxCustomScheduler; AsTable, AShowDialog: Boolean;
  const AHeader: string; AStart, AFinish: TDateTime; AExportType: Integer;
  const ASeparators: array of string; const AFileExt: string; AHandler: TObject;
  AEncoding: TEncoding);
var
...
begin
  ...
  begin
    ...
    begin
      AStart := TcxDateNavigatorAccess(AScheduler.DateNavigator)
        .GetRealFirstDate;
      AFinish := TcxDateNavigatorAccess(AScheduler.DateNavigator)
        .GetRealLastDate;
    end;
  end
  else if not ShowSetRangeDialog(AScheduler.DialogsLookAndFeel, AStart, AFinish)
  then
    Exit;

  with SchedulerExportHelpers[AsTable].Create(AScheduler, AStart, AFinish,
    AExportType, AFileName, AHandler) do
    try
      FHeader := AHeader;
      if Supports(Provider, IcxExportProviderEncoding, AEncodingIntf) then
        AEncodingIntf.SetEncoding(AEncoding);
      if Supports(Provider, IcxExportWithSeparators, AIntf) and
...
begin
  cxExportSchedulerToFile(AFileName, AScheduler, AsTable, AShowDialog, AHeader,
    AStart, AFinish, cxExportToCSV, [ASeparator], AFileExt, AHandler,
    AEncoding);
end;
...
begin
  cxExportSchedulerToFile(AFileName, AScheduler, AsTable, AShowDialog, AHeader,
    AStart, AFinish, cxExportToHtml, [], AFileExt, AHandler);
end;
...
begin
  cxExportSchedulerToFile(AFileName, AScheduler, AsTable, AShowDialog, AHeader,
    AStart, AFinish, cxExportToXML, [], AFileExt, AHandler);
end;
...
begin
  cxExportSchedulerToFile(AFileName, AScheduler, AsTable, AShowDialog, AHeader,
    AStart, AFinish, cxExportToExcel, [], AFileExt, AHandler);
end;
...
begin
  cxExportSchedulerToFile(AFileName, AScheduler, AsTable, AShowDialog, AHeader,
    AStart, AFinish, cxExportToXlsx, [], AFileExt, AHandler);
end;
...
begin
  cxExportSchedulerToFile(AFileName, AScheduler, AsTable, AShowDialog, AHeader,
    AStart, AFinish, cxExportToText, [ASeparator, ABeginString, AEndString],
    AFileExt, AHandler, AEncoding);
end;

{ TfmExportRangeDialog }

constructor TfmExportRangeDialog.CreateEx(ALookAndFeel: TcxLookAndFeel;
...
begin
  ...
  Result := (AStart <> NullDate) and (AFinish <> NullDate) and
    (AStart < AFinish);
end;
