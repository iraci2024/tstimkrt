...
uses
  ...
  dxCore, cxLookAndFeelPainters, cxGraphics, cxButtons, cxControls,
  cxFilterControl, cxLookAndFeels, dxForms, cxClasses,
...
type
  ...
  public
    ...
    property OnBeforeApply: TNotifyEvent read FOnBeforeApply
      write FOnBeforeApply;
  end;

function cxInternalExecuteFilterControlDialog(AFilterControlClass
  : TcxCustomFilterControlClass; ALink: TComponent;
  ...
  AOnBeforeApply: TNotifyEvent = nil; AOnAfterApply: TNotifyEvent = nil;
...
const
  cxFilterDialogPosition: TRect = (Left: - 1; Top: - 1; // screen center
...
begin
  ...
  try
    ...
    else if ALink is TControl then
    ...
    if Supports(TObject(AForm.FFilterControl), IcxFilterControlDialog, AIntf)
    then
      try
...
begin
  ...
  OpenDialog.Title := cxGetResourceString
...
procedure TfmFilterControlDialog.SetLookAndFeel(ALookAndFeel: TcxLookAndFeel);
begin
...
begin
  ...
  Caption := Format('%s - [%s]',
    [cxGetResourceString(@cxSFilterControlDialogCaption), ATitle]);
end;
...
procedure TfmFilterControlDialog.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #27) and not FHasOpenedPopupWindow then
    Close;
end;

procedure TfmFilterControlDialog.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
