...
type
  ...
  TdxTrialDesignTimeHelper = class(TcxIUnknownObject, IOTANotifier,
    IOTAIDENotifier)
  private
...
{$ENDIF}
  sdxTrialHelperRegistryRuntimePackagesErrorMessage =
    'TrialVersionRuntimePackagesErrorMessage';
...
function TdxTrialDesignTimeHelper.CheckCanCompileProject(const Project
  : IOTAProject): Boolean;
var
...
begin
  Result := True;
  if Supports(Project.ProjectOptions, IOTAProjectOptionsConfigurations,
    AProjectConfig) then
  begin
...
function TdxTrialDesignTimeHelper.ShowWarningDialog(var AOptionValue
  : Boolean): Boolean;
var
...
procedure TdxTrialDesignTimeHelper.BeforeCompile(const Project: IOTAProject;
  var Cancel: Boolean);
begin
