...
type
  ...
  class var
    ...
    class function GetConnectionParameters
      : TDictionary<string, TdxConnectionParameterValues>; static;
  strict private
  const
  ...
  strict private
    ...
    function GetTypeFromString(const ATypeName: string; ALength: Integer)
      : TdxDBColumnType;
  protected
  ...
  public
    ...
    procedure CreateDatabase(const ADatabaseName: string;
      const AValues: array of string); override;
    procedure CreateTable(ATable: TdxDBTable); override;
    function GetSQLCreateColumnFullAttributes(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    procedure GetTableSchema(ATable: TdxDBTable; ACheckIndexes: Boolean;
      ACheckForeignKeys: Boolean); override;
    ...
    function FormatTable(const ASchemaName, ATableName, ATableAlias: string)
      : string; override;
    function FormatTable(const ASchemaName, ATableName: string)
      : string; override;
    ...
    property DatabaseObjectOwner: string read FDatabaseObjectOwner
      write FDatabaseObjectOwner;
    function SupportsNativeSkipTake: Boolean; override;
    function SupportsNativeOuterApply: Boolean; override;
    class property ConnectionParameters
      : TDictionary<string, TdxConnectionParameterValues>
      read GetConnectionParameters;
  end;
...
type
  ...
  public
    class function FormatColumn(const AColumnName: string): string;
      overload; static;
    class function FormatColumn(const AColumnName, ATableAlias: string): string;
      overload; static;
    class function FormatInsertDefaultValues(const ATableName: string)
      : string; static;
    class function FormatInsert(const ATableName, AFieldNames,
      AValuesClause: string): string; static;
    class function FormatUpdate(const ATableName, ASetClause,
      AWhereClause: string): string; static;
    class function FormatDelete(const ATableName, AWhereClause: string)
      : string; static;
    class function FormatFunction(AOperatorType: TdxFunctionOperatorType;
      const ADBEngineVersion: TdxMSSQLConnectionProvider.TVersions;
      const AOperands: TArray<string>): string; overload; static;
    class function FormatFunction(AProcessParameter: TdxProcessParameter;
      AOperatorType: TdxFunctionOperatorType;
      ADBEngineVersion: TdxMSSQLConnectionProvider.TVersions;
      const AOperands: TArray<TValue>): string; overload; static;
    class function FormatBinary(AOperatorType: TdxBinaryOperatorType;
      const ALeftOperand, ARightOperand: string): string; static;
...
class function TdxMsSQLFormatterHelper.FormatColumn(const AColumnName
  : string): string;
begin
  Result := '"' + AColumnName + '"';
end;
...
class function TdxMsSQLFormatterHelper.FormatInsertDefaultValues
  (const ATableName: string): string;
begin
  Result := 'insert into ' + ATableName + ' default values';
end;

class function TdxMsSQLFormatterHelper.FormatInsert(const ATableName,
  AFieldNames, AValuesClause: string): string;
begin
  Result := 'insert into ' + ATableName + '(' + AFieldNames + ')values(' +
    AValuesClause + ')';
end;

class function TdxMsSQLFormatterHelper.FormatUpdate(const ATableName,
  ASetClause, AWhereClause: string): string;
begin
  Result := 'update ' + ATableName + ' set ' + ASetClause + ' where ' +
    AWhereClause;
end;

class function TdxMsSQLFormatterHelper.FormatDelete(const ATableName,
  AWhereClause: string): string;
begin
  Result := 'delete from ' + ATableName + ' where ' + AWhereClause;
end;

class function TdxMsSQLFormatterHelper.FormatFunction(AOperatorType
...
begin
  case AOperatorType of
    ...
    TdxFunctionOperatorType.BigMul:
      Result := Format('(Convert(bigint, %s) * CONVERT(bigint,  %s))',
    ...
    TdxFunctionOperatorType.ArcTan2:
      Result := Format
        ('(case when (%1:s) = 0 then Sign(%0:s) * Atan(1) * 2 else Atn2(%0:s,  %1:s) end)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.ToString:
      Result := Format(IfThen(TdxMSSQLConnectionProvider.TVersion.is2005
        in ADBEngineVersion, 'Cast((%s) as nvarchar(max))',
    ...
    TdxFunctionOperatorType.GetTimeOfDay:
      Result := Format
        ('(CONVERT(BigInt,((CONVERT(BigInt,DATEPART(HOUR, %0:s))) * 36000000000) + '
        + '((CONVERT(BigInt,DATEPART(MINUTE, %0:s))) * 600000000) + ((CONVERT(BigInt,DATEPART(SECOND, %0:s))) * 10000000) + '
        + '((CONVERT(BigInt,DATEPART(MILLISECOND, %0:s))) * 10000)))',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOfTheWeek:
      Result := Format
        ('CONVERT(Int,(DATEPART(dw, %s) + (@@DATEFIRST) + 6) %% 7)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOfTheYear:
      Result := Format('DATEPART(DayOfYear, %s)', [AOperands[0]]);
    TdxFunctionOperatorType.DateOf:
      Result := Format
        ('DATEADD(HOUR, -DATEPART(HOUR, %0:s), DATEADD(MINUTE, -DATEPART(MINUTE, %0:s), '
        + 'DATEADD(SECOND, -DATEPART(SECOND, %0:s), DATEADD(MILLISECOND, -DATEPART(MILLISECOND, %0:s), %0:s))))',
        [AOperands[0]]);
    TdxFunctionOperatorType.IncTick:
      Result := Format
        ('DATEADD(ms, CONVERT(BigInt, (%1:s) / 10000) %% 86400000, DATEADD(day, (%1:s) / 864000000000, %0:s))',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.AddTimeSpan, TdxFunctionOperatorType.IncSecond:
      Result := Format(IfThen(TdxMSSQLConnectionProvider.TVersion.is2005
        ...
        + '%% 86400000, DATEADD(day, (CONVERT(decimal(38, 19),(%1:s)) * 1000) / 86400000, %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMinute:
      Result := Format(IfThen(TdxMSSQLConnectionProvider.TVersion.is2005
        ...
        + ' %% 86400000, DATEADD(day, (CONVERT(decimal(38, 19),(%1:s)) * 60000) / 86400000, %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncHour:
      Result := Format(IfThen(TdxMSSQLConnectionProvider.TVersion.is2005
        ...
        + ' %% 86400000, DATEADD(day, (CONVERT(decimal(38, 19),(%1:s)) * 3600000) / 86400000, %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncDay:
      Result := Format(IfThen(TdxMSSQLConnectionProvider.TVersion.is2005
        ...
        + ' %% 86400000, DATEADD(day, (CONVERT(decimal(38, 19),(%1:s)) * 86400000) / 86400000, %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMonth:
      Result := Format('DATEADD(MONTH, %1:s, %0:s)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncYear:
      Result := Format('DATEADD(YEAR, %1:s, %0:s)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.MillisecondsBetween:
      Result := Format('DATEDIFF(millisecond, %s, %s)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MinutesBetween:
      Result := Format('DATEDIFF(minute, %s, %s)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.SecondsBetween:
      Result := Format('DATEDIFF(second, %s, %s)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.DateDiffTicks:
      Result := Format(IfThen(TdxMSSQLConnectionProvider.TVersion.is2008
        in ADBEngineVersion, '((DATEDIFF(microsecond, %s, %s)) * 10)',
        '((DATEDIFF(millisecond, %s, %s)) * 10000)'),
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.Remove:
      case Length(AOperands) of
        ...
        3:
          Result := Format('Stuff(%s, (%s)+1, %s, '#$27#$27')',
            [AOperands[0], AOperands[1], AOperands[2]]);
      else
        Result := '';
      end;
    TdxFunctionOperatorType.CharIndex:
      case Length(AOperands) of
        2:
          Result := Format('(Charindex(%s, %s) - 1)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format('(Charindex(%s, %s, (%s) + 1) - 1)',
            [AOperands[0], AOperands[1], AOperands[2]]);
        4:
          Result := Format
            ('(Charindex(%s, SUBSTRING(%1:s, 1, (%2:s) + (%3:s)), (%2:s) + 1) - 1)',
    ...
    TdxFunctionOperatorType.PadLeft:
      case Length(AOperands) of
        2:
          Result := Format
            ('isnull(REPLICATE('#$27' '#$27', ((%1:s) - LEN(%0:s))) + (%0:s), %0:s)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('isnull(REPLICATE(%2:s, ((%1:s) - LEN(%0:s))) + (%0:s), %0:s)',
            [AOperands[0], AOperands[1], AOperands[2]]);
      else
        Result := '';
      end;
    TdxFunctionOperatorType.PadRight:
      case Length(AOperands) of
        2:
          Result := Format
            ('isnull(%0:s + REPLICATE('#$27' '#$27', ((%1:s) - LEN(%0:s))), %0:s)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('isnull(%0:s + REPLICATE(%2:s, ((%1:s) - LEN(%0:s))), %0:s)',
            [AOperands[0], AOperands[1], AOperands[2]]);
      else
        Result := '';
      end;
    ...
    TdxFunctionOperatorType.EndsWith:
      Result := Format('(Right(%s, Len(%1:s)) = (%1:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.Contains:
      Result := Format('(isnull(CharIndex(%1:s, %0:s), 0) > 0)',
        [AOperands[0], AOperands[1]]);
  else
    Result := '';
  end;
...
class function TdxMsSQLFormatterHelper.FormatFunction(AProcessParameter
    ...
    TdxFunctionOperatorType.StartsWith:
      begin
        ...
        begin
          ...
          begin
            AConstantValue := TdxConstantValue.Create
              ({$IFDEF DELPHIXE3}AOperandString.Substring(0,
              ALikeIndex){$ELSE}TdxStringHelper.Substring(AOperandString, 0,
              ALikeIndex){$ENDIF} + '%');
            Exit(Format
              ('((%0:s like %2:s) And (Left(%0:s, Len(%1:s)) = (%1:s)))',
              [AProcessParameter(Safe<TdxCriteriaOperator>.Cast(AOperands[0]
              .AsObject)), AProcessParameter(ASecondOperand),
...
class function TdxMsSQLFormatterHelper.FormatBinary(AOperatorType
...
class function TdxMsSQLFormatterHelper.FormatConstraint(const AConstraintName
  : string): string;
begin
...
constructor TdxMSSQLConnectionProvider.Create(AConnectionVendor
...
begin
  FConnectionParameters.AddOrSetValue(TdxConnectionTypes.ADO,
    TConnectionParameters.Create(['initial catalog']));
  FConnectionParameters.AddOrSetValue(TdxConnectionTypes.FireDAC,
    TConnectionParameters.Create(['Database']));
end;
...
class function TdxMSSQLConnectionProvider.GetDBTypes
...
class function TdxMSSQLConnectionProvider.GetConnectionParameters
  : TDictionary<string, TdxConnectionParameterValues>;
begin
  if FConnectionParameters = nil then
  begin
    FConnectionParameters :=
      TDictionary<string, TdxConnectionParameterValues>.Create;
    PopulateConnectionParameters;
  end;
  Result := FConnectionParameters;
end;

function TdxMSSQLConnectionProvider.CollectTablesToCreate
...
function TdxMSSQLConnectionProvider.FormatColumn(const AColumnName
  : string): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatColumn(AColumnName);
end;

procedure TdxMSSQLConnectionProvider.CreateDatabase(const ADatabaseName: string;
...
begin
  ...
  try
    AConnectionVendor := TdxCustomConnectionVendorClass
      (ConnectionVendor.ClassType).Create(AConnection);
    try
      AConnection.Open;
      AConnectionVendor.Execute(Format('create database [%s]',
        [ADatabaseName]));
    finally
...
begin
  ...
  if ASchemaName = '' then
    AQuery := TdxQuery.Create
      ('select count(*) from sys.sequences s where s.name = ''' +
      ASequenceName + '''')
  else
    AQuery := TdxQuery.Create('select count(*) from sys.sequences q ' +
      'join sys.schemas s on s.schema_id = q.schema_id'#13#10 +
      'where q.name=''' + ASequenceName + ''' and s.name = ''' +
      ASchemaName + '''');
  try
    if GetScalar(AQuery) = 0 then
      ExecuteSQLSchemaUpdate('SEQUENCE', 'Key', '',
        'CREATE SEQUENCE ' + ASequenceName + ' START WITH 1');
  finally
    AQuery.Free;
  end;
end;

function TdxMSSQLConnectionProvider.FormatBinary(AOperatorType
  : TdxBinaryOperatorType; const ALeftOperand, ARightOperand: string): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatBinary(AOperatorType, ALeftOperand,
    ARightOperand);
end;
...
function TdxMSSQLConnectionProvider.FormatConstraint(const AConstraintName
  : string): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatConstraint(AConstraintName);
end;

function TdxMSSQLConnectionProvider.FormatOwnedDBObject(const ASchemaName,
  AObjectName: string): string;
begin
  if ASchemaName <> '' then
...
function TdxMSSQLConnectionProvider.FormatSelect(const ASelectClause,
  AFromClause, AWhereClause, AOrderByClause, AGroupByClause,
  AHavingClause: string; ASkipSelectedRecords, ATopSelectedRecords
  : Integer): string;
var
  AModificatorsSQL, AExpandedWhereClause, AExpandedOrderByClause,
    AExpandedHavingClause, AExpandedGroupByClause, AFetchRowsSQL, ASelectedSQL,
    ABaseFormat, AExpandedSelectedProperties: string;
...
begin
  ...
  if ASkipSelectedRecords = 0 then
    Result := Format('select %s%s from %s%s%s%s%s',
      [AModificatorsSQL, ASelectClause, AFromClause, AExpandedWhereClause,
      AExpandedGroupByClause, AExpandedHavingClause, AExpandedOrderByClause])
  else if is2012 and (AExpandedOrderByClause <> '') then
  begin
    if ATopSelectedRecords <> 0 then
      AFetchRowsSQL := Format(#10'fetch next %d rows only',
        [ATopSelectedRecords])
    else
      AFetchRowsSQL := '';
    Result := Format('select %s from %s%s%s%s%s'#10'offset %d rows%s',
      [ASelectClause, AFromClause, AExpandedWhereClause, AExpandedGroupByClause,
      AExpandedHavingClause, AExpandedOrderByClause, ASkipSelectedRecords,
      AFetchRowsSQL]);
  end
  else
  begin
    ...
    ABaseFormat :=
      'select %8:s from(select %0:s, row_number() over(%1:s) as '#$27'rowNumber'#$27
      + ' from %4:s%5:s%6:s%7:s)resultSet where resultSet.rowNumber > %2:d';
    if ATopSelectedRecords <> 0 then
      ABaseFormat := ABaseFormat + ' and resultSet.rowNumber <= %2:d + %3:d';
    Result := Format(ABaseFormat, [ASelectedSQL, AExpandedOrderByClause,
      ASkipSelectedRecords, ATopSelectedRecords, AFromClause,
      AExpandedWhereClause, AExpandedGroupByClause, AExpandedHavingClause,
      AExpandedSelectedProperties]);
  end;
end;

function TdxMSSQLConnectionProvider.FormatTable(const ASchemaName,
  ATableName: string): string;
begin
  Result := FormatOwnedDBObject(ASchemaName, ATableName);
end;

function TdxMSSQLConnectionProvider.FormatTable(const ASchemaName, ATableName,
  ATableAlias: string): string;
begin
  Result := FormatOwnedDBObject(ASchemaName, ATableName) + ' ' + ATableAlias;
end;

function TdxMSSQLConnectionProvider.FormatInsertDefaultValues
  (ATable: TdxDBTable): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatInsertDefaultValues
    (FormatTableSafe(ATable));
end;

function TdxMSSQLConnectionProvider.FormatInsert(const ATableName, AFieldNames,
  AValuesClause: string): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatInsert(ATableName, AFieldNames,
    AValuesClause);
end;

function TdxMSSQLConnectionProvider.FormatUpdate(const ATableName, ASetClause,
  AWhereClause: string): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatUpdate(ATableName, ASetClause,
    AWhereClause);
end;

function TdxMSSQLConnectionProvider.FormatDelete(const ATableName,
  AWhereClause: string): string;
begin
  Result := TdxMsSQLFormatterHelper.FormatDelete(ATableName, AWhereClause);
end;

function TdxMSSQLConnectionProvider.FormatFunction(AOperatorType
  : TdxFunctionOperatorType; const AOperands: TArray<string>): string;
var
  AFormat: string;
begin
  AFormat := TdxMsSQLFormatterHelper.FormatFunction(AOperatorType, FVersion,
    AOperands);
  if AFormat = '' then
...
function TdxMSSQLConnectionProvider.FormatFunction(AProcessParameter
  : TdxProcessParameter; AOperatorType: TdxFunctionOperatorType;
  const AOperands: TArray<TValue>): string;
var
  AFormat: string;
begin
  AFormat := TdxMsSQLFormatterHelper.FormatFunction(AProcessParameter,
    AOperatorType, FVersion, AOperands);
  if AFormat = '' then
...
function TdxMSSQLConnectionProvider.GetConnectionParameter(const AConnectionType
  : TdxConnectionType; AParameter: TdxConnectionParameter): string;
begin
  Result := ConnectionParameters[AConnectionType][AParameter];
end;

function TdxMSSQLConnectionProvider.GetDataForTables(const ATables
  : TArray<TdxDBTable>; AFilter: TdxMSSQLConnectionProvider.TTableFilter;
  const AQueryText: string): TDataSet;
var
  ...
  try
    ...
    begin
      if Assigned(AFilter) and not AFilter(ATable) then
        Continue;
      AParameters.Add(TdxOperandValue.Create
    ...
    if AInList.Count = 0 then
      Exit(nil);
    AQuery := TdxQuery.Create(Format(AQueryText, [ANames.ToString]),
...
begin
  ...
  if ASchemaName = '' then
  begin
    AQueryParameters := TdxQueryParameterCollection.Create
      (TdxOperandValue.Create(ComposeSafeTableName(ATable.Name)));
    AQuery := TdxQuery.Create
  ...
  begin
    ...
    AQuery := TdxQuery.Create
      ('select COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, COLUMN_DEFAULT, IS_NULLABLE from INFORMATION_SCHEMA.COLUMNS '
      + 'where TABLE_NAME = :p1 and TABLE_SCHEMA = :p2', AQueryParameters,
      TArray<string>.Create('p1', 'p2'));
  end;
  try
...
begin
  ...
  begin
    ...
    AQuery := TdxQuery.Create(IfThen(is2005,
      ...
      'where t.name = :p1 and p.type = ''PK'''^M + 'order by i.key_ordinal'),
  ...
  begin
    ...
    AQuery := TdxQuery.Create(IfThen(is2005,
      ...
      'order by i.key_ordinal'), AQueryParameters,
      TArray<string>.Create('p1', 'p2'));
  end;
  try
    ADataSet := SelectData(AQuery);
  ...
  begin
    ...
    begin
      AQueryParameters := TdxQueryParameterCollection.Create
        (TdxOperandValue.Create(ComposeSafeTableName(ATable.Name)));
      AQuery := TdxQuery.Create
        ('select i.name, c.name, INDEXPROPERTY(i.id, i.name, ''IsUnique'') from sysobjects o'#13#10
        ...
        + 'order by i.name, k.keyno', AQueryParameters,
        TArray<string>.Create('p1'));
    end
    else
    begin
      AQueryParameters := TdxQueryParameterCollection.Create
        (TdxOperandValue.Create(ComposeSafeTableName(ATable.Name)));
      AQuery := TdxQuery.Create
        ('select i.name, c.name, i.is_unique from sys.objects t'#13#10 +
        ...
        + 'order by i.name, ic.key_ordinal', AQueryParameters,
        TArray<string>.Create('p1'));
    end;
  end
  else
...
begin
  ...
  begin
    ...
    AQuery := TdxQuery.Create(IfThen(is2005,
      ...
      'where tbl.name = :p1'^M + 'order by fk.name, fkdata.keyno',
      ...
      + 'where fkt.name = :p1'^M + 'order by c.name, r.constraint_column_id'),
      AQueryParameters, TArray<string>.Create('p1'));
  end
  else
  begin
    ...
    AQuery := TdxQuery.Create(IfThen(is2005,
      ...
      'order by c.name, r.constraint_column_id'), AQueryParameters,
      TArray<string>.Create('p1', 'p2'));
  end;
  try
    ADataSet := SelectData(AQuery);
    try
      ...
      try
        while not ADataSet.Eof do
        begin
          if not AForeignKeys.TryGetValue(ADataSet.Fields[0].AsString,
            AForeignKey) then
          begin
            ...
            AForeignKey := TdxDBForeignKey.Create([APrimaryKeyColumn], ARTable,
              [AForeignKeyColumn]);
            ATable.ForeignKeys.Add(AForeignKey);
            AForeignKeys.AddOrSetValue(ADataSet.Fields[0].AsString,
              AForeignKey);
          end
          else
          begin
            AForeignKey.Columns.Add(ADataSet.Fields[1].AsString);
            AForeignKey.PrimaryKeyTableKeyColumns.Add
...
var
  AQuery: TdxQuery;
begin
  AQuery := TdxQuery.Create(ASQL.SQLText + #10' select ' + IfThen(is2000,
    'SCOPE_IDENTITY()', '@@Identity'), ASQL.Parameters,
    ASQL.ParameterNames, 0, 0);
  try
...
begin
  ...
  begin
    if ConnectionVendor.ExecuteScalar
      ('select SERVERPROPERTY('#$27'edition'#$27')') = 'SQL Azure' then
...
function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForBoolean
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bit';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'tinyint';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForSByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(3,0)';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForChar
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'nchar(1)';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForDecimal
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'money';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForDouble
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'double precision';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForSingle
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'float';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'int';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForUInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(10,0)';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'smallint';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForUInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(5,0)';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bigint';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForUInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(20,0)';
end;

function TdxMSSQLConnectionProvider.GetStorageTableNames(AIncludeViews: Boolean)
  : TArray<string>;
var
  AQuery: TdxQuery;
  ATables: TDataSet;
...
begin
  ...
  try
    if is2005 then
      AQuery := TdxQuery.Create
        (Format('select t.name, p.name from sys.objects t join sys.schemas p on p.schema_id = '
        + 't.schema_id where (t.type =''u'' %s) and objectProperty(t.object_id, ''IsMSShipped'') = 0',
        [IfThen(AIncludeViews, ' or t.type =''v''', '')]))
    else
      AQuery := TdxQuery.Create
        (Format('select name from sysobjects where (type=''U'' %s) and objectProperty(id, ''IsMSShipped'') = 0',
        [IfThen(AIncludeViews, ' or t.type =''v''', '')]));
    try
...
procedure TdxMSSQLConnectionProvider.GetTableSchema(ATable: TdxDBTable;
...
function TdxMSSQLConnectionProvider.GetTypeFromString(const ATypeName: string;
  ALength: Integer): TdxDBColumnType;
begin
...
function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForString
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
...
function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForDateTime
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'datetime';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForGuid
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'uniqueidentifier';
end;

function TdxMSSQLConnectionProvider.GetSQLCreateColumnTypeForByteArray
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
...
function TdxMSSQLConnectionProvider.GetSQLCreateColumnFullAttributes
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  ...
  begin
    if AColumn.IsIdentity and not HasSequence(ATable) and
      (AColumn.ColumnType in [TdxDBColumnType.Int32, TdxDBColumnType.Int64])
    then
      Result := Result + IfThen(GetIsAzure, ' IDENTITY',
        ' IDENTITY NOT FOR REPLICATION')
    else if (AColumn.ColumnType = TdxDBColumnType.Guid) and not GetIsAzure then
...
begin
  ...
  begin
    ...
    try
      try
        ANewConnection.Open;
        AVersion := ConnectionVendor.ExecuteScalar(ANewConnection,
    ...
    AVersion := ConnectionVendor.ExecuteScalar(AConnection,
      'select @@MICROSOFTVERSION / 0x1000000');
  FVersion := [];
  if AVersion > 7 then
