...
{$ENDIF}
  ...
  cxClasses, dxSpreadSheetCore, dxSpreadSheetStrs, cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters,
  cxButtons, dxLayoutLookAndFeels, dxLayoutContainer, dxLayoutControl,
  cxContainer, cxEdit, dxLayoutControlAdapters,
  cxListBox, dxLayoutcxEditAdapters, cxLabel, cxTextEdit, cxDropDownEdit,
  cxCheckBox, dxSpreadSheetGraphics, dxCore,
  cxCheckListBox, dxSpreadSheetClasses, dxSpreadSheetTypes, cxMaskEdit,
  cxMCListBox, dxForms, dxMessages,
  cxCustomListBox;
...
type
  ...
  TdxSpreadSheetFindAndReplaceDialogMatchModes = set of
  ...
  TdxSpreadSheetFindAndReplaceDialogFormClass = class of
    TdxSpreadSheetFindAndReplaceDialogForm;

  TdxSpreadSheetFindAndReplaceDialogForm = class(TdxForm,
    IdxSpreadSheetListener, IcxLookAndFeelNotificationListener,
  ...
  strict private
    ...
    procedure CheckFirstRowAndColumn(var AFirstRow: TdxSpreadSheetTableRow;
      var AFirstColumn: TdxSpreadSheetTableColumn;
    ...
    function CreateSearchResultString(ASheet: TdxSpreadSheetTableView;
      const ARowIndex, AColumnIndex: Integer): string;
    procedure DoFindAll(ASheet: TdxSpreadSheetTableView);
    function DoFindNextCell(ASheet: TdxSpreadSheetTableView;
    ...
    function FoundNextCell(AStartSheet: TdxSpreadSheetTableView)
      : TdxSpreadSheetCell;
    function GetDefinedName(ASheet: TdxSpreadSheetTableView;
      const ARowIndex, AColumnIndex: Integer): string;
    function GetFormulaText(ACell: TdxSpreadSheetCell): string;
    function GetMinClientHeight: Integer;
    procedure GetNextRowAndColumn(var ARow: TdxSpreadSheetTableRow;
      var AColumn: TdxSpreadSheetTableColumn; AFirstRow: TdxSpreadSheetTableRow;
      AFirstColumn: TdxSpreadSheetTableColumn);
    function GetReplacementPos(const AWhatReplace, AWhereReplace: string;
      const AOffset: Integer): Integer;
    function GetSearchArea(const AStartRowIndex, AStartColumnIndex
      : Integer): TRect;
  ...
  protected
    ...
    procedure CheckReplacementPossibility(AView: TdxSpreadSheetTableView;
    ...
    property LookMode: TdxSpreadSheetFindAndReplaceDialogLookMode
      read GetLookMode;
    property MasterLookAndFeel: TcxLookAndFeel read FMasterLookAndFeel
      write SetMasterLookAndFeel;
    property MatchModes: TdxSpreadSheetFindAndReplaceDialogMatchModes
      read GetMatchModes;
    property ReplacementCount: Integer read FReplacementCount;
    property SearchMode: TdxSpreadSheetFindAndReplaceDialogSearchMode
      read GetSearchMode;
    property SearchRange: TdxSpreadSheetFindAndReplaceDialogSearchRange
      read GetSearchRange;
...
var
  dxSpreadSheetFindAndReplaceDialogClass
    : TdxSpreadSheetFindAndReplaceDialogFormClass =
    TdxSpreadSheetFindAndReplaceDialogForm;

procedure ShowFindAndReplaceDialog(ASpreadSheet: TdxCustomSpreadSheet;
  const AActivePage: Integer = 0);
...
uses
  ...
  dxSpreadSheetUtils,
  dxSpreadSheetDialogStrs, dxSpreadSheetNumberFormat, StrUtils, cxFormats,
...
type
  ...
  public
    ...
    constructor Create(ASheet: TdxSpreadSheetTableView;
...
constructor TdxFindAndReplaceDialogFoundReference.Create
  (ASheet: TdxSpreadSheetTableView; const ARowIndex, AColumnIndex: Integer);
begin
  inherited Create;
  Sheet := ASheet;
  RowIndex := ARowIndex;
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.AddSearchResult
  (ACell: TdxSpreadSheetCell);
var
  AReference: TdxFindAndReplaceDialogFoundReference;
begin
  AReference := TdxFindAndReplaceDialogFoundReference.Create(ACell.View,
    ACell.RowIndex, ACell.ColumnIndex);
  mcFindAllResults.AddItem(CreateSearchResultString(ACell.View, ACell.RowIndex,
    ACell.ColumnIndex), AReference);
...
begin
  ...
  liOptionsHeightAdjustingItem1.SizeOptions.Height :=
    lcFindSpaceItem.SizeOptions.Height;
  liOptionsHeightAdjustingItem2.SizeOptions.Height :=
    lcFindSpaceItem.SizeOptions.Height;
  liOptionsHeightAdjustingItem3.SizeOptions.Height :=
    lcFindSpaceItem.SizeOptions.Height;
end;

function TdxSpreadSheetFindAndReplaceDialogForm.CreateCellName(const ARowIndex,
...
function TdxSpreadSheetFindAndReplaceDialogForm.CreateSearchResultString
...
begin
  ...
  Result := ASheet.Caption + ADelimiter + AName + ADelimiter + ACellName +
    ADelimiter + VarToStr(AValue) + ADelimiter + AFormula + ADelimiter;
end;

function TdxSpreadSheetFindAndReplaceDialogForm.GetDefinedName
  (ASheet: TdxSpreadSheetTableView;
  const ARowIndex, AColumnIndex: Integer): string;
var
  I, P: Integer;
  ...
  begin
    ...
    begin
      ...
      if SameText(ARefSheet, ASheet.Caption) and
        SameText(Copy(AReference, P + 1, Length(AReference) - P + 1),
        CreateCellName(ARowIndex, AColumnIndex)) then
        Result := AName.Caption;
    end;
  end;
end;

function TdxSpreadSheetFindAndReplaceDialogForm.GetFormulaText
...
var
  dH, dW: Integer;
begin
  dH := Max(FPriorClientHeight - ClientHeight + lcMain.OccupiedClientHeight -
    FPriorLayoutHeight, 0);
  dW := Max(FPriorClientWidth - ClientWidth + lcMain.OccupiedClientWidth -
    FPriorLayoutWidth, 0);
...
begin
  ...
  FLessOptionsCaption := cxGetResourceString
    (@sdxFindAndReplaceDialogLessOptions);
  FMoreOptionsCaption := cxGetResourceString
    (@sdxFindAndReplaceDialogMoreOptions);

  btnReplaceAll.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogButtonReplaceAll);
  btnReplace.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogButtonReplace);
  btnFindAll.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogButtonFindAll);
  btnFindNext.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogButtonFindNext);
  btnClose.Caption := cxGetResourceString(@sdxFindAndReplaceDialogButtonClose);

  // FindAll results
  mcFindAllResults.HeaderSections[0].Text :=
    cxGetResourceString(@sdxFindAndReplaceDialogColumnSheetCaption);
  mcFindAllResults.HeaderSections[1].Text :=
    cxGetResourceString(@sdxFindAndReplaceDialogColumnNameCaption);
  mcFindAllResults.HeaderSections[2].Text :=
    cxGetResourceString(@sdxFindAndReplaceDialogColumnCellCaption);
  mcFindAllResults.HeaderSections[3].Text :=
    cxGetResourceString(@sdxFindAndReplaceDialogColumnValueCaption);
  mcFindAllResults.HeaderSections[4].Text :=
    cxGetResourceString(@sdxFindAndReplaceDialogColumnFormulaCaption);

  // Find page
  lcgTabFind.Caption := cxGetResourceString
  ...
  lciWithinRangeFind.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogWithinRange);
  cmbWithinRangeFind.Properties.Items.Clear;
  cmbWithinRangeFind.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogWithinRangeSheet));
  cmbWithinRangeFind.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogWithinRangeSpreadsheet));
  cmbWithinRangeFind.ItemIndex := 0;

  lciSearchModeFind.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogSearchMode);
  cmbSearchModeFind.Properties.Items.Clear;
  cmbSearchModeFind.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogSearchModeByRows));
  cmbSearchModeFind.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogSearchModeByColumns));
  ...
  cmbLookInFind.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogLookInFormulas));
  cmbLookInFind.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogLookInValues));
  cmbLookInFind.ItemIndex := 0;

  chkMatchCaseFind.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogMatchCase);
  chkEntireCellFind.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogMatchEntireCell);

  // Replace page
  lcgTabReplace.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogReplaceTabCaption);

  lciWhatReplace.Caption := lciWhatFind.Caption;
  lciWithReplace.Caption := cxGetResourceString
  ...
  cmbWithinRangeReplace.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogWithinRangeSheet));
  cmbWithinRangeReplace.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogWithinRangeSpreadsheet));
  cmbWithinRangeReplace.ItemIndex := 0;

  lciSearchModeReplace.Caption := cxGetResourceString
    (@sdxFindAndReplaceDialogSearchMode);
  cmbSearchModeReplace.Properties.Items.Clear;
  cmbSearchModeReplace.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogSearchModeByRows));
  cmbSearchModeReplace.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogSearchModeByColumns));
  ...
  cmbLookInReplace.Properties.Items.Add
    (cxGetResourceString(@sdxFindAndReplaceDialogLookInFormulas));
...
function TdxSpreadSheetFindAndReplaceDialogForm.CheckCondition
...
begin
  lbFoundCellsInfo.Caption :=
    Format(cxGetResourceString(@sdxFindAndReplaceDialogStatusBarCellsFound),
    [mcFindAllResults.Count]);
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.CheckFirstRowAndColumn
  (var AFirstRow: TdxSpreadSheetTableRow;
  var AFirstColumn: TdxSpreadSheetTableColumn;
  const AStartRowIndex, AStartColumnIndex: Integer);
begin
  ...
  begin
    if (AFirstRow <> nil) and (AFirstRow.Index = AStartRowIndex) then
      while (AFirstColumn <> nil) and
        (AFirstColumn.Index <= AStartColumnIndex) do
        AFirstColumn := TdxSpreadSheetTableColumn(AFirstColumn.Next);
  end
  else if (AFirstColumn <> nil) and (AFirstColumn.Index = AStartColumnIndex)
  then
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.CheckReplacementPossibility
  (AView: TdxSpreadSheetTableView; const AIsReplaceAll: Boolean);
begin
  if not(AIsReplaceAll and (SearchRange = fdsrAllSheets)) and AView.Options.Protected
  then
    raise EdxSpreadSheetError.Create
      (cxGetResourceString(@sdxErrorCannotExecuteActionOnProtectedSheet));
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.CleanSearchResults
...
begin
  ...
  try
    ...
    begin
      AReference := mcFindAllResults.Items.Objects[I]
        as TdxFindAndReplaceDialogFoundReference;
...
// IdxSpreadSheetListener
procedure TdxSpreadSheetFindAndReplaceDialogForm.DataChanged
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        st := CreateSearchResultString(AReference.Sheet, AReference.RowIndex,
          AReference.ColumnIndex);
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.MasterLookAndFeelChanged
  (Sender: TcxLookAndFeel; AChangedValues: TcxLookAndFeelValues);
begin
  // do nothing
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.MasterLookAndFeelDestroying
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.DoFindAll
  (ASheet: TdxSpreadSheetTableView);
var
...
function TdxSpreadSheetFindAndReplaceDialogForm.DoFindNextCell
  (ASheet: TdxSpreadSheetTableView; const AStartRowIndex, AStartColumnIndex
  : Integer): TdxSpreadSheetCell;
var
  AArea: TRect;
  ACell: TdxSpreadSheetCell;
...
begin
  Result := nil;
  AController := TdxTableViewControllerAccess(TdxTableViewAccess(ASheet)
    .Controller);
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.DoReplaceAll
  (ASheet: TdxSpreadSheetTableView);
var
  ARowIndex, AColumnIndex: Integer;
  ACell: TdxSpreadSheetCell;
begin
  TdxTableViewAccess(ASheet).History.BeginAction
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.DoReplacement
  (ACell: TdxSpreadSheetCell);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      AWhereReplace := Copy(AWhereReplace, 1, P - 1) + AReplaceWith +
        Copy(AWhereReplace, P2, Length(AWhereReplace) - P2 + 1);
      Inc(FReplacementCount);
      P := GetReplacementPos(AWhatReplace, AWhereReplace,
        P2 + Length(AReplaceWith));
    end;
  end;
...
var
  AReference: TdxFindAndReplaceDialogFoundReference;
begin
  AReference := mcFindAllResults.Items.Objects[AIndex]
    as TdxFindAndReplaceDialogFoundReference;
  AReference.Sheet.Active := True;
  AReference.Sheet.Selection.SetFocused(AReference.RowIndex,
    AReference.ColumnIndex, []);
end;

function TdxSpreadSheetFindAndReplaceDialogForm.FoundNextCell
  (AStartSheet: TdxSpreadSheetTableView): TdxSpreadSheetCell;
var
  ASheet: TdxSpreadSheetTableView;
begin
  ASheet := AStartSheet;
  Result := DoFindNextCell(ASheet, ASheet.Selection.FocusedRow,
    ASheet.Selection.FocusedColumn);
  if Result = nil then
  ...
  begin
    ...
    SpreadSheet.ActiveSheetAsTable.Selection.SetFocused(Result.RowIndex,
      Result.ColumnIndex, []);
  end
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.GetNextRowAndColumn
  (var ARow: TdxSpreadSheetTableRow; var AColumn: TdxSpreadSheetTableColumn;
...
function TdxSpreadSheetFindAndReplaceDialogForm.GetReplacementPos
  (const AWhatReplace, AWhereReplace: string; const AOffset: Integer): Integer;
begin
  if not(fdmmCase in MatchModes) then
    Result := PosEx(AnsiUpperCase(AWhatReplace),
      AnsiUpperCase(AWhereReplace), AOffset)
  else
    Result := PosEx(AWhatReplace, AWhereReplace, AOffset);
end;

function TdxSpreadSheetFindAndReplaceDialogForm.GetSearchArea
  (const AStartRowIndex, AStartColumnIndex: Integer): TRect;
begin
  if SearchMode = fdsmByRows then
    Result := cxRect(0, AStartRowIndex, dxSpreadSheetMaxColumnIndex,
      dxSpreadSheetMaxRowIndex)
  else
    Result := cxRect(AStartColumnIndex, 0, dxSpreadSheetMaxColumnIndex,
      dxSpreadSheetMaxRowIndex)
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.GetStartIndexesForFindAll
...
function TdxSpreadSheetFindAndReplaceDialogForm.IsRemoved
  (ASheet: TdxSpreadSheetTableView): Boolean;
begin
  Result := (ASheet = nil) or
    (TdxSpreadSheetAccess(SpreadSheet).FSheets.IndexOf(ASheet) < 0);
end;

function TdxSpreadSheetFindAndReplaceDialogForm.GetLookMode
  : TdxSpreadSheetFindAndReplaceDialogLookMode;
begin
  Result := TdxSpreadSheetFindAndReplaceDialogLookMode(cmbLookInFind.ItemIndex);
end;

function TdxSpreadSheetFindAndReplaceDialogForm.GetMatchModes
  : TdxSpreadSheetFindAndReplaceDialogMatchModes;
begin
  Result := [];
...
function TdxSpreadSheetFindAndReplaceDialogForm.GetSearchMode
  : TdxSpreadSheetFindAndReplaceDialogSearchMode;
begin
  Result := TdxSpreadSheetFindAndReplaceDialogSearchMode
    (cmbSearchModeFind.ItemIndex);
end;

function TdxSpreadSheetFindAndReplaceDialogForm.GetSearchRange
  : TdxSpreadSheetFindAndReplaceDialogSearchRange;
begin
  Result := TdxSpreadSheetFindAndReplaceDialogSearchRange
    (cmbWithinRangeFind.ItemIndex);
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.Initialize
  (ASpreadSheet: TdxCustomSpreadSheet; AActivePage: Integer);
begin
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.Notification
  (AComponent: TComponent; Operation: TOperation);
begin
  inherited Notification(AComponent, Operation);
  if (AComponent is TdxCustomSpreadSheet) and (Operation = opRemove) then
    TdxSpreadSheetFindAndReplaceDialogForm.Unregister
      (TdxCustomSpreadSheet(AComponent));
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.ScaleFactorChanged(M,
  D: Integer);
begin
  inherited;
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.SaveComboHistory
  (ACombo: TcxComboBox);
var
  AText, AItem: string;
  I: Integer;
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.SetMasterLookAndFeel
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.ShowInfoMessage
  (AMessage: string; AType: Cardinal);
begin
  MessageBox(Handle, PChar(AMessage), PChar(PChar(Application.Title)),
    AType or MB_OK);
end;
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.FormClose(Sender: TObject;
...
begin
  ...
  lbFoundCellsInfo.Visible := ClientHeight - lbFoundCellsInfo.Height >
    GetMinClientHeight;
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.btnFindAllClick
  (Sender: TObject);
var
  I: Integer;
begin
  ...
  if mcFindAllResults.Count > 0 then
    ...
    ShowInfoMessage(cxGetResourceString
      (@sdxFindAndReplaceDialogFindMatchNotFound), MB_ICONWARNING);
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.btnFindNextClick
  (Sender: TObject);
begin
  SynchronizeLookMode;
  SaveComboHistory(cmbWhatFind);
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.btnOptionsFindClick
  (Sender: TObject);
begin
  lcMain.BeginUpdate;
  try
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.btnReplaceAllClick
  (Sender: TObject);
var
  I: Integer;
begin
  ...
  try
    if SearchRange = fdsrSheet then
      ...
      for I := 0 to SpreadSheet.SheetCount - 1 do
        if not(SpreadSheet.Sheets[I] as TdxSpreadSheetTableView).Options.&Protected
        then
  ...
  if ReplacementCount > 0 then
    ShowInfoMessage
      (Format(cxGetResourceString(@sdxFindAndReplaceDialogReplacementResult),
      [ReplacementCount]), MB_ICONINFORMATION)
  else
    ShowInfoMessage(cxGetResourceString
      (@sdxFindAndReplaceDialogReplaceMatchNotFound), MB_ICONWARNING);
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.btnReplaceClick
  (Sender: TObject);
var
  ASheet: TdxSpreadSheetTableView;
  ACell: TdxSpreadSheetCell;
begin
  ...
  ACell := TdxTableViewAccess(ASheet).GetActiveCell(ASheet.Selection.FocusedRow,
    ASheet.Selection.FocusedColumn);
  if ACell = nil then
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.cmbFindWhatPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.
  cmbFindWithinRangePropertiesChange(Sender: TObject);
begin
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.
  cmbFindSearchModePropertiesChange(Sender: TObject);
begin
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.
  chkFindMatchCasePropertiesChange(Sender: TObject);
begin
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.
  chkFindEntireCellPropertiesChange(Sender: TObject);
begin
  chkEntireCellFind.Checked := TcxCheckBox(Sender).Checked;
  chkEntireCellReplace.Checked := chkEntireCellFind.Checked;
end;

procedure TdxSpreadSheetFindAndReplaceDialogForm.lcMainContentGroupTabChanged
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetFindAndReplaceDialogForm.mcFindAllResultsClick
  (Sender: TObject);
begin
...
class procedure TdxSpreadSheetFindAndReplaceDialogForm.Register
  (ASpreadSheet: TdxCustomSpreadSheet);
begin
...
class procedure TdxSpreadSheetFindAndReplaceDialogForm.Unregister
  (ASpreadSheet: TdxCustomSpreadSheet);
begin
  if FFindAndReplaceDialog <> nil then
  begin
    ...
    else if FFindAndReplaceDialog.SpreadSheet = ASpreadSheet then
    begin
      FFindAndReplaceDialog.MasterLookAndFeel := nil;
