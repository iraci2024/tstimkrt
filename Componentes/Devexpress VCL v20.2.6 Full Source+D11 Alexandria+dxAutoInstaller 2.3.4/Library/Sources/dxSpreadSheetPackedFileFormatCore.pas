...
uses
  Windows, SysUtils, Classes, dxSpreadSheetCore, dxSpreadSheetClasses,
  dxZIPUtils, dxXMLDoc, dxCustomTree,
...
type
  ...
  TdxSpreadSheetCustomPackedReaderParser = class
  ...
  protected
    procedure WriteFile(const AFileName: AnsiString; AStream: TStream;
      AFreeStream: Boolean); virtual;
    procedure WriteXML(const AFileName: AnsiString;
      ADocument: TdxXMLDocument); virtual;
  public
    constructor Create(AOwner: TdxCustomSpreadSheet; AStream: TStream);
      override;
    destructor Destroy; override;
  ...
  TdxSpreadSheetCustomPackedWriterBuilder = class
    (TdxSpreadSheetCustomFilerSubTask)
  protected
    procedure WriteFile(const AFileName: AnsiString; AStream: TStream;
      AFreeStream: Boolean = False); inline;
    procedure WriteXML(const AFileName: AnsiString;
      ADocument: TdxXMLDocument); inline;
  end;
...
constructor TdxSpreadSheetCustomPackedReader.Create
  (AOwner: TdxCustomSpreadSheet; AStream: TStream);
begin
...
function TdxSpreadSheetCustomPackedReader.FileExists(const AFileName
  : AnsiString): Boolean;
begin
  Result := PackageReader.Exists(CheckFileName(AFileName));
end;

function TdxSpreadSheetCustomPackedReader.ReadFile(const AFileName: AnsiString)
  : TMemoryStream;
begin
...
function TdxSpreadSheetCustomPackedReader.ReadXML(const AFileName: AnsiString)
  : TdxXMLDocument;
var
...
function TdxSpreadSheetCustomPackedReader.CheckFileName(const AFileName
...
function TdxSpreadSheetCustomPackedReaderParser.ReadXML(const AFileName
  : AnsiString): TdxXMLDocument;
begin
...
constructor TdxSpreadSheetCustomPackedWriter.Create
  (AOwner: TdxCustomSpreadSheet; AStream: TStream);
begin
  inherited Create(AOwner, AStream);
...
procedure TdxSpreadSheetCustomPackedWriter.WriteFile(const AFileName
  : AnsiString; AStream: TStream; AFreeStream: Boolean);
begin
  PackageWriter.AddFile(AFileName, AStream, AFreeStream);
end;

procedure TdxSpreadSheetCustomPackedWriter.WriteXML(const AFileName: AnsiString;
...
procedure TdxSpreadSheetCustomPackedWriterBuilder.WriteFile(const AFileName
  : AnsiString; AStream: TStream; AFreeStream: Boolean = False);
begin
  TdxSpreadSheetCustomPackedWriter(Owner).WriteFile(AFileName, AStream,
    AFreeStream);
end;

procedure TdxSpreadSheetCustomPackedWriterBuilder.WriteXML(const AFileName
  : AnsiString; ADocument: TdxXMLDocument);
begin
