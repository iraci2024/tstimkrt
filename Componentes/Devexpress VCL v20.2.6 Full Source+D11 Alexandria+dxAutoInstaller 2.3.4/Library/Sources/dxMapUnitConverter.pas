...
type
  ...
  public
    function ScreenPointToMapUnit(APoint: TdxPointDouble;
      AViewport: TdxRectDouble; AViewportInPixels: TSize): TdxPointDouble;
    function MapUnitToScreenPoint(AMapUnit: TdxPointDouble;
      AViewport: TdxRectDouble; AViewportInPixels: TSize): TdxPointDouble;
    function GeoPointToMapUnit(const APoint: TdxMapControlGeoPoint;
      AProjection: TdxMapControlCustomProjection;
...
function dxMapGeoPointNormalize(const APoint: TdxMapControlGeoPoint)
  : TdxMapControlGeoPoint; {$IFDEF DELPHI15} inline; {$ENDIF}
function dxMapPointNormalize(const APoint: TdxPointDouble)
  : TdxPointDouble; inline;

implementation

function NormalizeRadian(AAngleRadian: Double): Double;
{$IFDEF DELPHI15} inline; {$ENDIF}
begin
  Result := ArcTan2(Sin(AAngleRadian), Cos(AAngleRadian));
end;

function NormalizeDegree(AAngleDegree: Double): Double;
{$IFDEF DELPHI15} inline; {$ENDIF}
begin
...
begin
  Result := dxPointDouble(Max(0, Min(APoint.X, 1.0)),
    Math.Max(0, Min(APoint.Y, 1.0)));
end;

{ TdxMapUnitConverter }

function TdxMapUnitConverter.GeoPointToMapUnit(const APoint
  : TdxMapControlGeoPoint; AProjection: TdxMapControlCustomProjection;
  AShouldNormalize: Boolean = True): TdxPointDouble;
var
...
function TdxMapUnitConverter.MapUnitToGeoPoint(AUnit: TdxPointDouble;
  AProjection: TdxMapControlCustomProjection; AShouldNormalize: Boolean = True)
  : TdxMapControlGeoPoint;
var
...
function TdxMapUnitConverter.MapUnitToScreenPoint(AMapUnit: TdxPointDouble;
  AViewport: TdxRectDouble; AViewportInPixels: TSize): TdxPointDouble;
var
...
function TdxMapUnitConverter.ScreenPointToMapUnit(APoint: TdxPointDouble;
  AViewport: TdxRectDouble; AViewportInPixels: TSize): TdxPointDouble;
var
