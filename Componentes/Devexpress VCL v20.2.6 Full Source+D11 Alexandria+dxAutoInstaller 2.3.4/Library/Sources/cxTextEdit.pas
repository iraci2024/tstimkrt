...
{$ENDIF}
  ...
  dxCore, dxMessages, dxCoreClasses, cxClasses, cxContainer, cxControls,
  cxDataUtils,
  dxSpellCheckerCore, cxFormats, cxGraphics, cxVariants, cxFilterControlUtils,
  cxLookAndFeels, cxEdit, cxDrawTextUtils,
  dxUxTheme, cxLookAndFeelPainters, dxTouch, cxGeometry, dxGenerics;
...
type
  ...
  TcxTextEditViewStyle = (vsNormal, vsHideCursor, vsButtonsOnly,
    vsButtonsAutoWidth);
  TcxTextEditCustomDrawHandler = procedure(ACanvas: TcxCanvas; ARect: TRect)
    of object;
  ...
  IcxInnerTextEdit = interface(IcxCustomInnerEdit)
    ...
    property InternalUpdating: Boolean read GetInternalUpdating
      write SetInternalUpdating;
    ...
    property PasswordChar: TCaptionChar read GetPasswordChar
      write SetPasswordChar;
    property SelLength: Integer read GetSelLength write SetSelLength;
    property SelStart: Integer read GetSelStart write SetSelStart;
    property SelText: string read GetSelText write SetSelText;
    property UseLeftAlignmentOnEditing: Boolean
      read GetUseLeftAlignmentOnEditing write SetUseLeftAlignmentOnEditing;
...
{$ENDIF}
  ...
  TcxCustomEditListBox = class(TcxCustomInnerListBox, IdxGestureClient,
    IdxGestureOwner)
  strict private
  ...
  protected
    ...
    function GetItemData(Index: Integer):
    {$IFDEF DELPHI16}TListBoxItemData{$ELSE}Longint{$ENDIF}; override;
    function IsHighlightSearchText: Boolean; virtual;
    function NeedDrawFocusRect: Boolean; override;
    procedure MeasureItem(Index: Integer; var Height: Integer); override;
    procedure MouseLeave(AControl: TControl); override;
    procedure MouseMove(Shift: TShiftState; X, Y: Integer); override;
    procedure SetItemData(Index: Integer; AData:
      {$IFDEF DELPHI16}TListBoxItemData{$ELSE}Longint{$ENDIF}); override;
  ...
  public
    ...
    destructor Destroy; override;
    function CanFocus: Boolean; override;
    function GetHeight(ARowCount: Integer; AMaxHeight: Integer)
      : Integer; virtual;
  ...
  protected
    ...
    function GetContainerState(const P: TPoint; AMouseTracking: Boolean)
      : TcxContainerState; override;
  ...
  TcxEditLookupDataGoDirection = (egdBegin, egdEnd, egdNext, egdPrev, egdPageUp,
    egdPageDown);

  IcxTextEditLookupData = interface
    ['{F49C5F08-7758-4362-A360-1DF02354E708}']
    function CanResizeVisualArea(var NewSize: TSize; AMaxHeight: Integer = 0;
    ...
    property OnSelectItem: TNotifyEvent read GetOnSelectItem
      write SetOnSelectItem;
  end;
  ...
  TcxCustomTextEditLookupData = class(TcxInterfacedPersistent,
    IcxTextEditLookupData, IcxTextEditLookupDataIncrementalFilter)
  private
    FSearchText: string;
    ...
    function FindItemByText(const AText: string; AStartIndex: Integer;
      AFindFullText: Boolean): Integer;
    function GetEdit: TcxCustomTextEdit;
  ...
  protected
    ...
    function InternalLocate(var AText, ATail: string;
      ANext, ASynchronizeWithText: Boolean): Boolean; virtual;
    ...
    property ActiveProperties: TcxCustomTextEditProperties
  ...
  public
    ...
    destructor Destroy; override;
    function CanResizeVisualArea(var NewSize: TSize; AMaxHeight: Integer = 0;
      AMaxWidth: Integer = 0): Boolean; virtual;
    ...
    function GetOnSelectItem: TNotifyEvent;
    function GetVisualAreaPreferredSize(AMaxHeight: Integer;
      AWidth: Integer = 0): TSize; virtual;
    procedure Go(ADirection: TcxEditLookupDataGoDirection; ACircular: Boolean);
  ...
  protected
    procedure CalculateButtonNativePartInfo(ATheme: TdxTheme;
      AButtonViewInfo: TcxEditButtonViewInfo); override;
  ...
  protected
    function GetBackgroundPaintingStyle
      : TcxEditBackgroundPaintingStyle; override;
    ...
    function DrawPasswordRevealButton(ACanvas: TcxCanvas;
      AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect): Boolean;
  ...
  public
    ...
    function NeedShowHint(ACanvas: TcxCanvas; const P: TPoint;
      ...
      AMaxLineCount: Integer = 0): Boolean; override;
  ...
  TcxNewLookupDisplayTextEvent = procedure(Sender: TObject;
    const AText: TCaption) of object;
  TcxTextEditChars = set of AnsiChar;

  TcxTextEditIncrementalFilteringOption = (ifoHighlightSearchText,
    ifoUseContainsOperator);
  TcxTextEditIncrementalFilteringOptions = set of
  ...
  protected
    ...
    procedure DoDisplayValueToDisplayText(ANativeStyle: Boolean;
      var ADisplayValue: string);
    procedure DoPrepareDisplayValue(const AEditValue: TcxEditValue;
      var ADisplayValue: TcxEditValue; AEditFocused: Boolean); virtual;
    function FindLookupText(const AText: string): Boolean; virtual;
    function GetDefaultDisplayFormat: string; virtual;
    function GetDefaultDisplayValue(const AEditValue: TcxEditValue;
      AEditFocused: Boolean): TcxEditValue;
    function GetDefaultIncrementalFilteringOptions
    ...
    function HasDigitGrouping(AIsDisplayValueSynchronizing: Boolean)
      : Boolean; virtual;
    function InternalGetEditFormat(out AIsCurrency, AIsOnGetTextAssigned
      : Boolean; AEdit: TcxCustomTextEdit = nil): string; virtual;
    ...
    procedure SetIncrementalFilteringOptions
      (Value: TcxTextEditIncrementalFilteringOptions); virtual;
    ...
    property HideCursor: Boolean read FHideCursor write SetHideCursor
      stored False;
    property IsPasswordRevealButtonPressed: Boolean
      read FIsPasswordRevealButtonPressed;
    ...
    property UseNullString: Boolean read FUseNullString write SetUseNullString
      default False;
  public
    constructor Create(AOwner: TPersistent); override;
    destructor Destroy; override;
    ...
    procedure ValidateDisplayValue(var DisplayValue: TcxEditValue;
      var ErrorText: TCaption; var Error: Boolean;
      AEdit: TcxCustomEdit); override;
    procedure DisplayValueToDisplayText(var ADisplayValue: string); virtual;
    function IsDisplayValueValid(var ADisplayValue: TcxEditValue;
    ...
    property ImmediateUpdateText: Boolean read FImmediateUpdateText
      write FImmediateUpdateText default False;
    property IncrementalFiltering: Boolean read FIncrementalFiltering
      write FIncrementalFiltering default False;
    ...
    property LookupItemsSorted: Boolean read GetLookupItemsSorted
      write SetLookupItemsSorted default False;
    property MaxLength: Integer read GetMaxLength write SetMaxLength
      stored IsMaxLengthStored;
    property MaxValue;
    property MinValue;
    property OEMConvert: Boolean read FOEMConvert write SetOEMConvert
      default False;
    property PasswordChar: TCaptionChar read FPasswordChar write SetPasswordChar
      default #0;
    property ShowPasswordRevealButton: Boolean read FShowPasswordRevealButton
      write SetShowPasswordRevealButton default False;
    property UseDisplayFormatWhenEditing: Boolean
      read FUseDisplayFormatWhenEditing write SetUseDisplayFormatWhenEditing
      default False;
    property ViewStyle: TcxTextEditViewStyle read GetViewStyle
      write SetViewStyle default vsNormal;
    property OnNewLookupDisplayText: TcxNewLookupDisplayTextEvent
      read FOnNewLookupDisplayText write FOnNewLookupDisplayText;
  end;
  ...
  private
    ...
    FText: TCaption;
    function DoIsMouseWheelHandleNeeded(Shift: TShiftState;
    ...
    function GetTextHint: string;
    procedure SetTextHint(Value: string);
    procedure TogglePassword(AButtonVisibleIndex: Integer;
      AIsMouseDown: Boolean = False);
    procedure WMClear(var Message: TMessage); message WM_CLEAR;
    procedure WMCommand(var Message: TWMCommand); message WM_COMMAND;
    procedure WMGetText(var Message: TWMGetText); message WM_GETTEXT;
    procedure WMGetTextLength(var Message: TWMGetTextLength);
  ...
  protected
    ...
    procedure PopulateSizeProperties(var AEditSizeProperties
      : TcxEditSizeProperties); override;
    ...
    function IsLookupNavigationKey(AKey: Word; AShift: TShiftState)
      : Boolean; virtual;
    ...
    function LookupKeyToEditValue(const AKey: TcxEditValue)
    ...
    property BeepOnEnter: Boolean read FBeepOnEnter write FBeepOnEnter
      default True;
    ...
    property LookupItemsScrolling: Boolean read FLookupItemsScrolling
      write FLookupItemsScrolling;
  ...
  public
    ...
    class procedure SetFilterValue(AEdit: TcxCustomEdit;
      AEditProperties: TcxCustomEditProperties; AValue: Variant); override;
  end;

procedure CheckCharsRegister(var AText: string; ACharCase: TEditCharCase);
function CheckTextEditState(ATextEdit: IcxInnerTextEdit;
  const APrevState: TcxCustomInnerTextEditPrevState): Boolean;
procedure DrawEditText(ACanvas: TcxCanvas; AViewInfo: TcxCustomTextEditViewInfo;
  ADrawPasswordCharAsBullet: Boolean = False);
procedure DrawTextEdit(ACanvas: TcxCanvas;
  AViewInfo: TcxCustomTextEditViewInfo);
function GetTextEditContentSize(ACanvas: TcxCanvas;
  AViewData: TcxCustomEditViewData; const AText: string; ADrawTextFlags: DWORD;
  const AEditSizeProperties: TcxEditSizeProperties; ALineCount: Integer = 0;
  ACorrectWidth: Boolean = True): TSize;
procedure InternalTextOut(ACanvas: TCanvas;
  AViewInfo: TcxCustomTextEditViewInfo; AText: PcxCaptionChar; var R: TRect;
  AFormat: TcxTextOutFormat; ASelStart, ASelLength: Integer;
  ASelBackgroundColor, ASelTextColor: TColor; AMaxLineCount: Integer = 0;
...
procedure SaveTextEditState(ATextEdit: IcxInnerTextEdit; ASaveText: Boolean;
...
procedure CalculateTextEditViewInfo(ACanvas: TcxCanvas;
  AViewData: TcxCustomTextEditViewData; AViewInfo: TcxCustomTextEditViewInfo;
...
begin
  ...
  begin
    ...
    if not IsInplace and (cxRectHeight(TextRect) < AMeasureTextHeight) then
      TextRect.Bottom := Min(ClientRect.Bottom, AMeasureTextHeight +
        TextRect.Top);
  end;
end;
  ...
  if not ATextEdit.Control.HandleAllocated then
    ...
    with APrevState do
      Result := (PrevSelStart <> ATextEdit.SelStart) or
        (PrevSelLength <> ATextEdit.SelLength) or IsPrevTextSaved and
...
begin
  ...
  begin
    ...
    if ADrawPasswordCharAsBullet then
      InflateRect(R, 0, 1);
    InternalTextOut(ACanvas.Canvas, AViewInfo, PcxCaptionChar(Text), R,
      DrawTextFlags, SelStart, SelLength, SelBackgroundColor, SelTextColor,
      MaxLineCount);
  end;
end;
...
procedure DrawTextEdit(ACanvas: TcxCanvas;
  ...
  begin
    if not AViewInfo.IsEditClass and AViewInfo.Focused and
      not AViewInfo.IsInplace and not AViewInfo.HasPopupWindow then
    begin
...
begin
  ...
  begin
    if ((AViewInfo.NativeState = TC_NONE) or AViewInfo.DrawSelectionBar) and
      not AViewInfo.Transparent and (AViewInfo.EditingStyle <> esNoEdit) then
  ...
  begin
    ...
    if AViewInfo.IsDBEditPaintCopyDrawing or not AViewInfo.HasInnerEdit or
      not AViewInfo.IsEditClass then
...
function GetTextEditContentSize(ACanvas: TcxCanvas;
  ...
  begin
    ...
    if not AViewData.IsInplace and (AViewData is TcxCustomTextEditViewData) then
      with TcxCustomTextEditProperties(AViewData.Properties) do
        if (EditingStyle in [esFixedList, esNoEdit]) and
          not(not AViewData.IsInplace and TcxCustomTextEditViewData(AViewData)
          .IsComboBoxStyle and AreVisualStylesMustBeUsed
          (AViewData.Style.LookAndFeel.NativeStyle, totEdit)) then
    ...
    begin
      ...
      begin
        Result.cx := 0;
        if (epoAllowZeroHeight in AViewData.PaintOptions) and (Length(AText) = 0)
        then
          Result.cy := 0
        else
        ...
        begin
          ...
          if AFlags and CXTO_SINGLELINE <> 0 then
            AFlags := AFlags and not CXTO_SINGLELINE or CXTO_WORDBREAK or
              CXTO_EDITCONTROL;
          AFlags := AFlags and not(CXTO_CENTER_VERTICALLY or CXTO_BOTTOM)
            or CXTO_TOP;
          R := Rect(0, 0, AWidth, cxMaxRectSize);
          ATextParams := cxCalcTextParams(ACanvas.Canvas, AFlags);
          cxMakeTextRows(ACanvas.Canvas, PChar(AText), Length(AText), R,
            ATextParams, ATextRows, ARowCount,
            AEditSizeProperties.MaxLineCount);
          Result.cy := ARowCount * TdxTextMeasurer.TextLineHeight
            (ACanvas.Handle);
  ...
  begin
    ...
    if (AFlags and CXTO_SINGLELINE <> 0) and
      not((epoAutoHeight in AViewData.PaintOptions) and
    ...
    begin
      ...
      ATextParams := cxCalcTextParams(ACanvas.Canvas, AFlags);
      cxMakeTextRows(ACanvas.Canvas, PChar(AText), Length(AText), R,
        ATextParams, ATextRows, ARowCount, ARowCount);
...
procedure PrepareTextRows(ACanvas: TCanvas; var TextOutData: TcxTextOutData;
  AText: PcxCaptionChar; var R: TRect; AFormat: TcxTextOutFormat;
  ASelStart, ASelLength: Integer; ASelBackgroundColor, ASelTextColor: TColor;
  AMaxLineCount: Integer = 0; ALeftIndent: Integer = 0;
  ARightIndent: Integer = 0);
  ...
  begin
    ...
    TextOutData.TextRect := cxPrepareRect(R, TextOutData.TextParams,
      ALeftIndent, ARightIndent);
    ...
    begin
      ...
      begin
        cxPlaceTextRows(ACanvas.Handle, TextOutData.TextRect,
          TextOutData.TextParams, TextOutData.TextRows, TextOutData.RowCount);
        if (ASelStart < 0) or (ASelStart >= ATextLength) then
          ASelLength := 0
        else if (ASelLength + ASelStart) > ATextLength then
...
begin
  if not AViewInfo.TextOutData.Initialized then
    PrepareTextRows(ACanvas, AViewInfo.TextOutData, AText, R, AFormat,
      ASelStart, ASelLength, ASelBackgroundColor, ASelTextColor, AMaxLineCount,
      ALeftIndent, ARightIndent);
  ...
  with AViewInfo.TextOutData do
    cxTextRowsOutHighlight(ACanvas.Handle, TextRect, TextParams, TextRows,
      RowCount, SelStart, SelLength, SelBackgroundColor, SelTextColor,
      ForceEndEllipsis);
end;

procedure PrepareTextEditDrawTextFlags(ACanvas: TcxCanvas;
  AViewData: TcxCustomTextEditViewData; AViewInfo: TcxCustomTextEditViewInfo);
var
...
begin
  ...
  begin
    ...
    AFlags := DrawTextFlags and not CXTO_SINGLELINE or CXTO_WORDBREAK or
      CXTO_EDITCONTROL or CXTO_CALCROWCOUNT;
    ...
    if TcxCustomTextEditProperties(AViewData.Properties).Alignment.Horz = taRightJustify
    then
      Inc(R.Right);
    ATextParams := cxCalcTextParams(ACanvas.Canvas, AFlags);
    AMaxLineCount := (TextRect.Bottom - TextRect.Top + ATextParams.RowHeight -
    ...
    begin
      ...
      begin
        ...
        if (AViewData.MaxLineCount = 0) or (ARowCount <= AViewData.MaxLineCount)
        then
...
function StrToFloatEx(S: string; var AValue: Double): Boolean;
  ...
  function InternalTextToFloat(const S: string;
    var AResultValue: Extended): Boolean;
  begin
{$IFDEF DELPHI103RIO}
{$IFDEF WIN64}
    if (S = dxFormatSettings.DecimalSeparator) or
      (S = '-' + dxFormatSettings.DecimalSeparator) then
    begin
      AResultValue := 0;
      Result := True;
    end
...
function GetWindowsPasswordChar(ANativeStyle: Boolean;
  APasswordChar: TCaptionChar): TCaptionChar;
begin
  if APasswordChar <> #0 then
    Result := APasswordChar
  else if ShowPasswordCharAsBullet(ANativeStyle) then
...
begin
  ...
  begin
    dxTestCheck(Edit.FLockBoundsCount > 0,
      'TcxCustomInnerTextEditHelper.LockBounds fails');
...
procedure TcxCustomInnerTextEditHelper.SetEditValue(const Value: TcxEditValue);
  ...
  procedure InternalSetValue(const Value: TcxEditValue;
...
begin
  with Edit do
  begin
    ATextChanged := IsDesigning and not InternalCompareString(Text,
      VarToStr(Value), True);
    ANeedStoreSelection := not IsDesigning and HandleAllocated and
      Container.IsEditValidating and Container.ActiveProperties.UseLookupData
      and Container.FindSelection and InternalCompareString(Text,
      VarToStr(Value), False) and not InternalCompareString(Text,
      VarToStr(Value), True);
...
procedure TcxCustomInnerTextEditHelper.SetUseLeftAlignmentOnEditing
  (Value: Boolean);
begin
...
procedure TcxCustomInnerTextEdit.SetBounds(ALeft, ATop, AWidth,
  AHeight: Integer);
begin
...
begin
  ...
  begin
    ...
    if (not(cxIsVCLThemesEnabled and Container.IsNativeStyle) or
      ((GetBasedAlignment <> taLeftJustify) and
      Container.ActiveProperties.UseLeftAlignmentOnEditing)) and
...
begin
  ...
  begin
    ...
    Style := Style or PasswordMap
      [Container.ActiveProperties.EchoMode = eemPassword];
    ExStyle := ExStyle and (not WS_EX_CLIENTEDGE);
  end;
end;

procedure TcxCustomInnerTextEdit.CreateWindowHandle(const Params
  : TCreateParams);
var
...
function TcxCustomInnerTextEdit.DoMouseWheel(Shift: TShiftState;
...
begin
  ...
  try
    ...
    AKey := Word(Key);
    if (Container.IsInplace or Container.FIsPopupWindowJustClosed) and
      (AKey in [VK_RETURN, VK_ESCAPE]) or
      (not Container.BeepOnEnter and (AKey in [VK_RETURN, 10 { Ctrl+Enter } ]))
    then
    begin
      Key := #0;
...
begin
  ...
  GetCaretPos(P);
  if Smallint(SendMessage(Handle, EM_POSFROMCHAR, ATextLength - 1, 0) and $FFFF)
    < P.X then
    Result := ATextLength
  else
  begin
...
begin
  ...
  if Container.ActiveProperties.EchoMode = eemPassword then
    APasswordChar := GetWindowsPasswordChar(Container.LookAndFeel.NativeStyle,
      FPasswordChar)
  else
    APasswordChar := #0;
  ...
  case ARealAlignment of
    taCenter:
      R.Left := (ClientWidth - ARightMargin - ALeftMargin -
        TdxTextMeasurer.TextSizeDT(Font, S, DT_NOPREFIX).cx) div 2 +
        ALeftMargin;
    taRightJustify:
      R.Left := ClientWidth - TdxTextMeasurer.TextSizeDT(Font, S, DT_NOPREFIX)
        .cx - ARightMargin + ALeftMargin;
    taLeftJustify:
      R.Left := ALeftMargin;
  end;
  ...
  try
    ...
    try
      ACanvas.Font := Font;
      if (APasswordChar <> #0) and ShowPasswordCharAsBullet
        (Container.LookAndFeel.NativeStyle) then
...
begin
  ...
  if Container.HasPopupWindow and Container.ActiveProperties.IsPopupKey
    (Message.CharCode, AShiftState) then
    with Container.ILookupData do
      if ActiveControl is TWinControl then
      begin
        SendMessage(TWinControl(ActiveControl).Handle, WM_KEYDOWN,
...
var
  ...
  PS: TPaintStruct;
begin
  if UseLeftAlignmentOnEditing and (GetBasedAlignment <> taLeftJustify) and not Focused
  then
  begin
    DC := Message.DC;
    if DC = 0 then
...
begin
  Result := DisplayFormat and (TcxCustomTextEditProperties(Properties)
    .FDisplayFormat = '') and IsPropertiesPropertyVisible('DisplayFormat');
end;

function TcxTextEditPropertiesValues.IsEditFormatStored: Boolean;
begin
  Result := EditFormat and (TcxCustomTextEditProperties(Properties)
    .FEditFormat = '') and IsPropertiesPropertyVisible('EditFormat');
end;
...
function TcxCustomEditListBox.GetHeight(ARowCount: Integer;
  AMaxHeight: Integer): Integer;
begin
...
function TcxCustomEditListBox.DoMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
var
...
begin
  ...
  begin
    ...
    Result := CallWindowProc(DefWndProc, Handle, WM_MOUSEWHEEL, AWParam,
      ALParam) = 1;
    SetExternalScrollBarsParameters;
  end;
end;

procedure TcxCustomEditListBox.DrawItem(Index: Integer; Rect: TRect;
...
procedure TcxCustomEditListBox.DrawItemText(const AText: string;
  const ARect: TRect);
var
  ASelStart: Integer;
  R: TRect;
begin
  R := ARect;
  ASelStart := Pos(AnsiUpperCase(Edit.LookupData.SearchText),
    AnsiUpperCase(AText)) - 1;
  cxTextOut(Canvas.Canvas, AText, R, GetDrawTextFlags, ASelStart,
    Length(Edit.LookupData.SearchText), Canvas.Font, clHighlight,
    clHighlightText, 0, 0, 0, Canvas.Font.Color);
end;
...
begin
  if IsHighlightSearchText then
    Result := AAlignment[UseRightToLeftAlignment] or CXTO_CENTER_VERTICALLY or
...
begin
  Result := Edit.ActiveProperties.IncrementalFiltering and
    ...
    in Edit.ActiveProperties.IncrementalFilteringOptions);
end;
...
begin
  inherited MouseMove(Shift, X, Y);
  if (GetCaptureControl <> Self) and HotTrack and
    not cxPointIsEqual(FPrevMousePos, Point(X, Y)) then
  begin
...
procedure TcxCustomEditListBox.SetItemData(Index: Integer; AData:
  {$IFDEF DELPHI16}TListBoxItemData{$ELSE}Longint{$ENDIF});
begin
end;
...
begin
  Canvas.Font := Font;
  Result := TdxTextMeasurer.TextLineHeight(Canvas.Handle);
...
begin
  Result := (AScrollKind = sbVertical) and Container.IsScrollBarActive
    (sbVertical) and Container.VScrollBar.Enabled or
    (AScrollKind = sbHorizontal) and Container.IsScrollBarActive(sbHorizontal)
    and Container.HScrollBar.Enabled;
end;
...
var
  ...
  ADelta: Integer;
begin
  if Container.IsScrollBarActive(sbVertical) and Container.VScrollBar.Enabled
  then
  begin
    ...
    begin
      ...
      FGestureHelper.CheckOverpan(Container.VScrollBar.Kind, ANewPosition, 0,
        Container.VScrollBar.Max - Container.VScrollBar.PageSize + 1,
        ADeltaX, ADeltaY);
    end;
  end;
  if Container.IsScrollBarActive(sbHorizontal) and Container.HScrollBar.Enabled
  then
  begin
    ANewPosition := Container.HScrollBar.Position - ADeltaX;
    AScrollBarPosition := EnsureRange(ANewPosition, 0, Container.HScrollBar.Max
      - Container.HScrollBar.PageSize + 1);
    DoScroll(sbHorizontal, scPosition, AScrollBarPosition);
    SetExternalScrollBarsParameters;
    FGestureHelper.CheckOverpan(Container.HScrollBar.Kind, ANewPosition, 0,
      Container.HScrollBar.Max - Container.HScrollBar.PageSize + 1,
      ADeltaX, ADeltaY);
  end;
end;
...
function TcxCustomEditListBox.NeedPanningFeedback
  (AScrollKind: TScrollBarKind): Boolean;
begin
  Result := True;
end;

function TcxCustomEditListBox.GetGestureClient(const APoint: TPoint)
  : IdxGestureClient;
begin
  Result := Self;
end;
...
procedure TcxCustomEditListBox.DoGesture(const EventInfo: TGestureEventInfo;
  var Handled: Boolean);
begin
  FGestureHelper.DoGesture(EventInfo, Handled);
end;

procedure TcxCustomEditListBox.DoGetGestureOptions(var Gestures
  : TInteractiveGestures; var Options: TInteractiveGestureOptions);
begin
  inherited;
...
begin
  Result := (Container.Owner as TcxCustomPopupWindow)
    .OwnerControl as TcxCustomTextEdit;
end;
...
function TcxEditListBoxContainer.GetContainerState(const P: TPoint;
  AMouseTracking: Boolean): TcxContainerState;
begin
  Result := [csActive];
end;

function TcxEditListBoxContainer.IsMouseWheelHandleNeeded(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
  Result := False;
end;
...
procedure TcxEditListBoxContainer.Scroll(AScrollBarKind: TScrollBarKind;
  AScrollCode: TScrollCode; var AScrollPos: Integer);
begin
  inherited Scroll(AScrollBarKind, AScrollCode, AScrollPos);
  if Enabled then
    (InnerControl as TcxCustomEditListBox).Scroll(AScrollBarKind, AScrollCode,
      AScrollPos);
end;

procedure TcxEditListBoxContainer.WMWindowPosChanged
...
begin
  if not Edit.EditModeSetting then
    if (FCurrentKey <> -1) and IsIncrementalFiltering and
      (ActiveProperties.EditingStyle = esEdit) and not Find(Edit.Text) then
...
procedure TcxCustomTextEditLookupData.DoInitialize(AVisualControlsParent
  : TWinControl);
var
...
begin
  ...
  FListContainer.Style.AssignedValues := FListContainer.Style.AssignedValues +
    [csvFont];
...
function TcxCustomTextEditLookupData.GetDisplayText
...
function TcxCustomTextEditLookupData.GetVisualAreaPreferredSize
  (AMaxHeight: Integer; AWidth: Integer = 0): TSize;
begin
  Result := cxNullSize;
end;

procedure TcxCustomTextEditLookupData.Go(ADirection
  : TcxEditLookupDataGoDirection; ACircular: Boolean);
var
...
begin
  ...
  case ADirection of
    ...
    egdEnd:
      ANewCurrentKey := GetLookupItemIndexFromFilteredItemIndex
        (GetFilteredItemCount - 1);
    egdPrev:
      begin
        ...
        if ANewIndex < 0 then
          ...
          else if UseSearchControl and IsFilterActive then
          begin
            if ANewIndex < -1 then
              ANewIndex := GetFilteredItemCount - 1;
          end
...
procedure TcxCustomTextEditLookupData.Initialize(AVisualControlsParent
  : TWinControl);
begin
...
procedure TcxCustomTextEditLookupData.UpdateFilteredLookupItems
  (const ASearchText: string);

  function IsSuitable(const ASearchText, AItemText: string): Boolean;
  begin
    Result := (ASearchText = '') or IsLikeTypeFiltering and
      (Pos(AnsiUpperCase(ASearchText), AnsiUpperCase(AItemText)) <> 0) or
      not IsLikeTypeFiltering and
      (Pos(AnsiUpperCase(ASearchText), AnsiUpperCase(AItemText)) = 1);
  end;
...
begin
  ...
  if IsIncrementalFiltering and IsFilterActive and
    (Result or (AText = '') or ((ActiveProperties.EditingStyle = esEdit) and
    ActiveProperties.IncrementalSearch)) then
    UpdateFilteredLookupItems(AText);
end;

procedure TcxCustomTextEditLookupData.PositionVisualArea
...
function TcxCustomTextEditLookupData.FindItemByText(const AText: string;
  AStartIndex: Integer; AFindFullText: Boolean): Integer;
var
  I, L: Integer;
  S: string;
  AFound: Boolean;
...
function TcxCustomTextEditLookupData.GetLookupItemIndexFromFilteredItemIndex
...
function TcxCustomTextEditLookupData.GetFilteredItemIndex
  (Index: Integer): Integer;
begin
...
function TcxCustomTextEditLookupData.GetActiveProperties
  : TcxCustomTextEditProperties;
begin
...
begin
  if (Value < -1) or (Value >= GetItemCount) or (Value = FItemIndex) and
    (Value <> -1) then
    Exit;
...
begin
  Result := IsIncrementalFiltering and
    (ifoUseContainsOperator in ActiveProperties.IncrementalFilteringOptions);
end;
...
begin
  ...
  try
    if not VarEqualsExact(APrevCurrentKey, FCurrentKey) or
      ((AKey >= 0) and (AKey < GetItemCount) and
      not InternalCompareString(Edit.Text, GetItem(AKey), True)) then
      DoSelectItem;
  finally
    if Edit <> nil then
...
begin
  if (FList = nil) or not FList.IsVisible or ActiveProperties.ImmediateUpdateText
  then
    CurrentKey := Value
  else
    InternalSetItemIndex(Value);
...
procedure TcxCustomTextEditLookupData.SetOnCurrentKeyChanged
  (Value: TNotifyEvent);
begin
...
begin
  if Edit.EditModeSetting then
    Exit;
  AText := Edit.Text;
  if (ItemIndex >= GetItemCount) or not InternalCompareString
    (GetDisplayText(ItemIndex), AText, False) then
    InternalLocate(AText, ATail, False, True);
end;

{ TcxCustomTextEditViewData }

procedure TcxCustomTextEditViewData.Calculate(ACanvas: TcxCanvas;
...
begin
  ...
  AViewInfo.HasPopupWindow := (AViewInfo.Edit <> nil) and
    AViewInfo.Edit.HasPopupWindow;
...
procedure TcxCustomTextEditViewData.DisplayValueToDrawValue(const ADisplayValue
...
procedure TcxCustomTextEditViewData.EditValueToDrawValue(const AEditValue
  : TcxEditValue; AViewInfo: TcxCustomEditViewInfo);
begin
  if PreviewMode then
    ...
    TcxCustomTextEditViewInfo(AViewInfo).Text :=
      EditValueToDisplayText(AEditValue);
...
begin
  ...
  if not IsInplace and IsComboBoxStyle and AreVisualStylesMustBeUsed
    (Style.LookAndFeel.NativeStyle, totEdit) then
    Inc(Result.Right);
end;

function TcxCustomTextEditViewData.GetDrawTextFlags: DWORD;
const
  HorzAlignmentFlags: array [TcxEditHorzAlignment] of DWORD = (CXTO_LEFT,
    CXTO_RIGHT, CXTO_CENTER_HORIZONTALLY);
  VertAlignmentFlags: array [TcxEditVertAlignment] of DWORD = (CXTO_TOP,
    CXTO_BOTTOM, CXTO_CENTER_VERTICALLY);
var
...
begin
  ...
  else if Properties.EditingStyle in [esFixedList, esNoEdit] then
  begin
    Inc(Result.Top);
    Dec(Result.Bottom);
...
procedure TcxCustomTextEditViewData.PrepareSelection
...
function TcxCustomTextEditViewData.InternalGetEditContentSize
...
begin
  with TcxCustomTextEditViewInfo(AViewInfo) do
  begin
    if not Properties.IsMultiLine and
      (esoAutoHeight in Properties.GetSupportedOperations) and
      (epoAutoHeight in Self.PaintOptions) then
      PrepareTextEditDrawTextFlags(ACanvas, Self,
        TcxCustomTextEditViewInfo(AViewInfo))
    else
...
function TcxCustomTextEditViewInfo.NeedShowHint(ACanvas: TcxCanvas;
  ...
  out AIsMultiLine: Boolean; out ATextRect: TRect;
  AMaxLineCount: Integer = 0): Boolean;
  ...
  begin
    Result := (ATextFlags and CXTO_SINGLELINE = 0) and
      (ATextFlags and CXTO_WORDBREAK <> 0);
  end;

  function GetCalcTextFlags: Integer;
  begin
    Result := DrawTextFlags and not(CXTO_CENTER_VERTICALLY or CXTO_BOTTOM or
      CXTO_SINGLELINE) or CXTO_CALCRECT;
  end;
  ...
  begin
    ...
    begin
      ALinesHeight := TdxTextMeasurer.TextLineHeight(ACanvas.Handle) *
        AMaxLineCount;
  ...
  begin
    ...
    begin
      ...
      Result := (R.Bottom > ADrawBounds.Bottom) or
        (R.Right > ADrawBounds.Right);
    end
    else
      Result := False;
  end;
...
begin
  ...
  begin
    Result := inherited NeedShowHint(ACanvas, P, AVisibleBounds, AText,
      AIsMultiLine, ATextRect);
    if not Result then
      SetTextAreaHintParameters;
  end;
end;
...
function TcxCustomTextEditViewInfo.GetBackgroundPaintingStyle
  : TcxEditBackgroundPaintingStyle;
begin
  if not ComboBoxStyle then
    Result := bpsSolid
  else if EditingStyle in [esFixedList, esNoEdit] then
    Result := bpsComboListEdit
  else
    Result := bpsComboEdit;
end;

procedure TcxCustomTextEditViewInfo.DrawNativeButtonBackground
...
procedure TcxCustomTextEditViewInfo.DrawLookAndFeelButtonBackground
  (ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo;
  const ARect: TRect);
begin
  if not DrawPasswordRevealButton(ACanvas, AButtonViewInfo, ARect) then
...
procedure TcxCustomTextEditViewInfo.DrawUsualButtonBackground
  (ACanvas: TcxCanvas; AButtonViewInfo: TcxEditButtonViewInfo;
  const ARect: TRect; ABrushColor: TColor);
begin
  if not DrawPasswordRevealButton(ACanvas, AButtonViewInfo, ARect) then
    inherited DrawUsualButtonBackground(ACanvas, AButtonViewInfo, ARect,
      ABrushColor);
end;
...
begin
  ...
  case EditProperties.Alignment.Vert of
    ...
    Result := TextRect.Top + (TextRect.Bottom - TextRect.Top -
      ATextMetric.tmHeight) div 2 + ATextMetric.tmAscent + 1;
  end;
end;
...
function TcxCustomTextEditViewInfo.DrawPasswordRevealButton(ACanvas: TcxCanvas;
  AButtonViewInfo: TcxEditButtonViewInfo; const ARect: TRect): Boolean;
var
...
begin
  ...
  Result := (AProperties <> nil) and AProperties.ShowPasswordRevealButton and
    (AButtonViewInfo.ButtonIndex = AProperties.PasswordRevealButtonIndex);
  if Result then
  begin
    if not AButtonViewInfo.Data.BackgroundPartiallyTransparent then
      ACanvas.FillRect(AButtonViewInfo.Bounds,
        AButtonViewInfo.Data.BackgroundColor);
    AState := AButtonViewInfo.Data.State;
    if (AState = ebsPressed) and not AProperties.IsPasswordRevealButtonPressed
    then
      AState := ebsNormal;
    AButtonCenter := cxRectCenter(AButtonViewInfo.Bounds);
    AHalfSize := Min(cxRectWidth(AButtonViewInfo.Bounds),
      cxRectHeight(AButtonViewInfo.Bounds) - ScaleFactor.Apply(1) * 2) div 2;
    Painter.DrawScaledPasswordRevealButton(ACanvas,
      cxRectInflate(cxRect(AButtonCenter, AButtonCenter),
      cxRect(AHalfSize, AHalfSize, AHalfSize, AHalfSize)),
      EditBtnStateToButtonState[AState], TextColor, ScaleFactor);
  end
end;
...
begin
  ...
  Result := (AProperties <> nil) and
    (AProperties.EchoMode = TcxEditEchoMode.eemPassword) and
...
function TcxCustomTextEditProperties.CompareDisplayValues(const AEditValue1,
...
function TcxCustomTextEditProperties.GetDisplayText(const AEditValue
  : TcxEditValue; AFullText: Boolean = False;
...
begin
  ...
  try
    if IsValueFormattedByProvider then
      ...
      AText := GetDefaultDisplayValue(AEditValue, not AIsInplace and
        not IsEditValueConversionDependOnFocused);
  finally
...
function TcxCustomTextEditProperties.GetSupportedOperations
...
class function TcxCustomTextEditProperties.GetViewInfoClass
  : TcxContainerViewInfoClass;
begin
  Result := TcxCustomTextEditViewInfo;
end;

function TcxCustomTextEditProperties.IsEditValueValid(var EditValue
...
procedure TcxCustomTextEditProperties.DoPrepareDisplayValue(const AEditValue
...
begin
  ...
  begin
    if VarIsSoftNull(AEditValue) then
      ADisplayValue := ''
    else if not VarIsNumericEx(AEditValue) and not VarIsStr(AEditValue) then
      raise EConvertError.CreateFmt
        (cxGetResourceString(@cxSEditNumericValueConvertError), [])
    else
      ADisplayValue := InternalPrepareDisplayValue(AEditValue);
  end
  else if VarIsArray(AEditValue) and not(VarArrayDimCount(AEditValue) = 1) then
    ADisplayValue := ''
  else
    ADisplayValue := dxVariantToString(AEditValue);
end;

procedure TcxCustomTextEditProperties.PrepareDisplayValue(const AEditValue
  : TcxEditValue; var ADisplayValue: TcxEditValue; AEditFocused: Boolean);
begin
  if IsEditValueEmpty(AEditValue) and UseNullString and not AEditFocused and not IsDesigning
  then
...
procedure TcxCustomTextEditProperties.ValidateDisplayValue(var DisplayValue
  : TcxEditValue; var ErrorText: TCaption; var Error: Boolean;
  AEdit: TcxCustomEdit);
begin
...
function TcxCustomTextEditProperties.IsDisplayValueValid(var ADisplayValue
  : TcxEditValue; AEditFocused: Boolean): Boolean;
var
  AText: string;
begin
  AText := VarToStr(ADisplayValue);
  Result := not((AText <> '') and (EditingStyle in [esEditList, esFixedList])
    and UseLookupData and not FindLookupText(AText));
  if Result then
  begin
...
procedure TcxCustomTextEditProperties.SetMinMaxValues(AMinValue,
  AMaxValue: Double);
begin
...
procedure TcxCustomTextEditProperties.DoAssign(AProperties
...
class function TcxCustomTextEditProperties.GetAssignedValuesClass
  : TcxCustomEditPropertiesValuesClass;
begin
  Result := TcxTextEditPropertiesValues;
end;

function TcxCustomTextEditProperties.GetDisplayFormatOptions
  : TcxEditDisplayFormatOptions;
begin
  Result := [dfoSupports];
end;

function TcxCustomTextEditProperties.GetValidateErrorText
...
class function TcxCustomTextEditProperties.GetViewDataClass
...
class function TcxCustomTextEditProperties.GetLookupDataClass
...
function TcxCustomTextEditProperties.GetEditValueSource(AEditFocused: Boolean)
...
procedure TcxCustomTextEditProperties.CheckEditValueBounds(const AEditValue
...
begin
  ...
  begin
    ...
    if (IsValueBoundDefined(evbMin) and (AEditValueAsDouble < MinValue) or
      IsValueBoundDefined(evbMax) and (AEditValueAsDouble > MaxValue)) then
    begin
      AError := True;
...
procedure TcxCustomTextEditProperties.CheckDisplayValueBounds
  (const ADisplayValue: TcxEditValue; var AErrorText: TCaption;
...
procedure TcxCustomTextEditProperties.DoDisplayValueToDisplayText
  (ANativeStyle: Boolean; var ADisplayValue: string);
begin
  if EchoMode = TcxEditEchoMode.eemPassword then
    ADisplayValue := StringOfChar(GetWindowsPasswordChar(ANativeStyle,
      PasswordChar), Length(ADisplayValue));
  DisplayValueToDisplayText(ADisplayValue);
end;

function TcxCustomTextEditProperties.FindLookupText
...
function TcxCustomTextEditProperties.GetDefaultDisplayValue(const AEditValue
  : TcxEditValue; AEditFocused: Boolean): TcxEditValue;
var
  AValue: TcxEditValue;
begin
  AValue := AEditValue;
  ...
  else if VarIsDate(AEditValue) then
...
function TcxCustomTextEditProperties.GetDefaultVertAlignment
...
function TcxCustomTextEditProperties.InternalGetEditFormat(out AIsCurrency,
  AIsOnGetTextAssigned: Boolean; AEdit: TcxCustomTextEdit = nil): string;
begin
  AIsCurrency := False;
  ...
  begin
    if not((AEdit <> nil) and AEdit.IsInplace) and
      (IDefaultValuesProvider <> nil) and IDefaultValuesProvider.IsOnGetTextAssigned
    then
    begin
...
function TcxCustomTextEditProperties.IsEditValueEmpty(const AEditValue
...
function TcxCustomTextEditProperties.IsPopupKey(Key: Word;
...
begin
  if AssignedValues.MinValue xor AssignedValues.MaxValue then
    Result := (ABound = evbMin) and AssignedValues.MinValue or (ABound = evbMax)
      and AssignedValues.MaxValue
  else
...
function TcxCustomTextEditProperties.TrySetPasswordRevealButtonVisible
...
begin
  ...
  if Result then
  begin
    AButtonVisible := AValue and (IsPasswordRevealButtonPressed or
...
procedure TcxCustomTextEditProperties.SetPasswordRevealButtonVisible
  (AValue: Boolean);
begin
...
function TcxCustomTextEditProperties.NeedUseNullString
  (AEditValue: TcxEditValue): Boolean;
begin
...
begin
  Result := not IsLookupEdit and IncrementalFiltering and
    (EditingStyle in [esFixedList, esEditList]);
end;

function TcxCustomTextEditProperties.GetAssignedValues
  : TcxTextEditPropertiesValues;
begin
  Result := TcxTextEditPropertiesValues(FAssignedValues);
end;

function TcxCustomTextEditProperties.GetDefaultIncrementalFilteringOptions
...
begin
  ...
  else if (IDefaultValuesProvider = nil) or
    (IDefaultValuesProvider.DefaultDisplayFormat = '') then
    Result := GetDefaultDisplayFormat
  else
    Result := IDefaultValuesProvider.DefaultDisplayFormat;
...
procedure TcxCustomTextEditProperties.ReadIsDisplayFormatAssigned
  (Reader: TReader);
begin
...
procedure TcxCustomTextEditProperties.SetIncrementalFilteringOptions
...
procedure TcxCustomTextEditProperties.SetShowPasswordRevealButton
  (Value: Boolean);
var
...
const
  AButtonsViewStyleMap: array [TcxTextEditViewStyle]
    of TcxEditButtonsViewStyle = (bvsNormal, bvsNormal, bvsButtonsOnly,
    bvsButtonsAutoWidth);
begin
...
class function TcxCustomTextEdit.GetPropertiesClass
...
begin
  Result := (ActiveProperties.EditingStyle in [esEdit, esEditList]) and
    not PropertiesChangeLocked;
end;
...
function TcxCustomTextEdit.DoInnerControlDefaultHandler
...
begin
  Result := inherited IsChildWindow(AWnd) or
    (Assigned(TdxSpellCheckerInstance.ISpellChecker) and
    TdxSpellCheckerInstance.ISpellChecker.IsSpellCheckerDialogControl(AWnd));
end;

procedure TcxCustomTextEdit.SetScrollBarsParameters(AIsScrolling
  : Boolean = False);
begin
...
procedure TcxCustomTextEdit.SetSelection(ASelStart: Integer;
  ASelLength: Integer);
begin
  SelStart := ASelStart;
...
begin
  ...
  try
    ...
    if IsInplace then
      TdxVisualRefinements.Padding.Deflate(R, ScaleFactor,
        UseRightToLeftAlignment);
    case TcxCustomTextEditProperties(ActiveProperties).Alignment.Vert of
      taTopJustify:
        AInnerEditTop := R.Top + ContentParams.Offsets.Top;
      taBottomJustify:
        AInnerEditTop := R.Bottom - AInnerEditHeight -
          ContentParams.Offsets.Bottom;
      taVCenter:
        AInnerEditTop := R.Top + ContentParams.Offsets.Top +
          (R.Bottom - R.Top - AInnerEditHeight - ContentParams.Offsets.Top -
          ContentParams.Offsets.Bottom) div 2;
    end;
    ...
    if not IsInplace and (ViewInfo.NativeState <> TC_NONE) and
      ViewInfo.ComboBoxStyle and
      (ActiveProperties.EditingStyle in [esEdit, esEditList]) then
    begin
      Dec(R.Right);
      Dec(R.Bottom);
    end;
...
function TcxCustomTextEdit.CanKeyDownModifyEdit(Key: Word;
  Shift: TShiftState): Boolean;
begin
  Result := inherited CanKeyDownModifyEdit(Key, Shift);
  Result := Result or (ActiveProperties.EditingStyle = esNoEdit) or
    (Key = VK_DELETE) and (IsDBEdit or IsInplace or (CursorPos < Length(Text))
    and (SelLength > 0));
...
begin
  if IsDBEdit or IsInplace then
    Result := not IsControlChar(Key) or
      dxCharInSet(Key, [dxVKBack, dxVKPaste, dxVKCut])
  else
    Result := not IsControlChar(Key) or (Key = dxVKBack) and
      ((CursorPos > 0) or (SelLength > 0)) or (Key = dxVKPaste) or
      (Key = dxVKCut) and (SelLength > 0);
end;
...
procedure TcxCustomTextEdit.ContainerStyleChanged(Sender: TObject);
  ...
  begin
    Result := not IsDestroying and not FIsCreating and
      (ActiveProperties.PasswordChar = #0) and
...
procedure TcxCustomTextEdit.DoEditKeyDown(var Key: Word; Shift: TShiftState);
  ...
  var
    APrevCurrentKey: TcxEditValue;
  begin
    if ((Key <> VK_HOME) and (Key <> VK_END) or
      (ActiveProperties.EditingStyle = esFixedList)) and
      ActiveProperties.UseLookupData and GetScrollLookupDataList(escKeyboard)
      and not ILookupData.IsEmpty then
    begin
      LockChangeEvents(True);
      ...
      try
        ...
        begin
          DoClick;
          if CanPostEditValue and ActiveProperties.ImmediatePost and InternalValidateEdit
          then
...
begin
  ...
  case Key of
    ...
    begin
      ...
      if (Key <> VK_HOME) and (Key <> VK_END) and not InnerTextEdit.MultiLine
      then
      begin
        if Key <> 0 then
          DoAfterKeyDown(Key, Shift);
...
begin
  ...
  try
    if Key = #8 then
    begin
      if ActiveProperties.UseLookupData and ActiveProperties.CanIncrementalSearch
      then
      begin
        if (AEditingStyle = esEditList) and (Length(DisplayText) > 0) and not FindSelection
        then
        begin
        ...
        begin
          if ActiveProperties.IsLookupEdit and
            (ActiveProperties.IncrementalFiltering) and
            (ActiveProperties.EditingStyle = esFixedList) and
            (ifoUseContainsOperator
            in ActiveProperties.IncrementalFilteringOptions) then
            AFindText := Copy(DisplayText, SelStart + 1, SelLength)
          else
            AFindText := Copy(DisplayText, 1, Length(DisplayText) - SelLength);
          SetLength(AFindText, Length(AFindText) -
            Length(AnsiLastChar(AFindText)));
    ...
    else if IsTextChar(Key) then
    begin
      if ActiveProperties.UseLookupData then
      begin
        if ActiveProperties.CanIncrementalSearch and CanContinueIncrementalSearch
        then
        begin
          LockLookupDataTextChanged;
          AFound := False;
          ...
          else if AEditingStyle = esFixedList then
            if FindSelection then
            begin
          ...
          if not AFound then
            AFound := PopulateFromList(AFindText);
          if (AEditingStyle = esFixedList) and
            not ActiveProperties.FixedListSelection and not AFound then
          begin
            AFindText := Key;
            AFound := PopulateFromList(AFindText);
          end;
        ...
        begin
          ...
          if (AEditingStyle = esEditList) and (DisplayText <> '') or
            (AEditingStyle = esFixedList) and
            ActiveProperties.FixedListSelection and APrevFindSelection then
            FindSelection := True;
        end;
      end;
    end;
...
function TcxCustomTextEdit.DoRefreshContainer(const P: TPoint;
...
procedure TcxCustomTextEdit.DoValidateDisplayValue(var ADisplayValue
  : TcxEditValue; var AErrorText: TCaption; var AError: Boolean);
var
  ACurrentKeyText: string;
begin
  if ActiveProperties.CanValidate and (ActiveProperties.EditingStyle = esEdit)
    and ActiveProperties.UseLookupData and FindSelection then
  begin
    ACurrentKeyText := ILookupData.GetDisplayText(ILookupData.CurrentKey);
    if InternalCompareString(VarToStr(ADisplayValue), ACurrentKeyText, False)
    then
...
begin
  ...
  begin
    if (ActiveProperties.EditingStyle <> esFixedList) or
      (ActiveProperties.IncrementalFiltering) and
      (ifoUseContainsOperator in ActiveProperties.IncrementalFilteringOptions)
      and not ActiveProperties.IsLookupEdit then
  ...
  begin
    ...
    if ActiveProperties.IsLookupEdit and (ActiveProperties.IncrementalFiltering)
      and (ActiveProperties.EditingStyle = esFixedList) and
      (ifoUseContainsOperator in ActiveProperties.IncrementalFilteringOptions)
    then
    begin
...
procedure TcxCustomTextEdit.PopulateSizeProperties(var AEditSizeProperties
...
function TcxCustomTextEdit.GetInnerControlBounds(const AInnerControlsRegion
  : TRect; AInnerControl: TControl): TcxContainerInnerControlBounds;
begin
  if IsEditClass then
    Result := inherited GetInnerControlBounds(AInnerControlsRegion,
      AInnerControl)
  else
  begin
...
procedure TcxCustomTextEdit.InternalValidateDisplayValue(const ADisplayValue
  : TcxEditValue);
begin
  if (ActiveProperties.EditingStyle = esEdit) and
    ActiveProperties.UseLookupData and not ILookupData.Find(ADisplayValue) then
    DoOnNewLookupDisplayText(ADisplayValue);
  inherited InternalValidateDisplayValue(ADisplayValue);
end;

function TcxCustomTextEdit.IsLookupNavigationKey(AKey: Word;
...
begin
  Result := (ActiveProperties.EchoMode = eemNormal) and
    ActiveProperties.CanModify;
end;
...
begin
  if Supports(FLookupData, IcxTextEditLookupDataIncrementalFilter,
    AIncrementalFilterIntf) then // #Ch do not use ILookupData here! T672303
  ...
  InnerTextEdit.UseLeftAlignmentOnEditing :=
    ActiveProperties.UseLeftAlignmentOnEditing;
...
begin
  ADisplayValue := Value;
  Result := ActiveProperties.IsDisplayValueValid(ADisplayValue,
    InternalFocused);
  if Result then
    ...
    try
      ...
      begin
        ...
        try
          ...
          SendMessageW(InnerControl.Handle, EM_REPLACESEL, 1,
            LParam(PWideChar(Replacement)));
...
begin
  ...
  if AEditingStyle in [esFixedList, esNoEdit] then
    SelectAll;
  if IsUserAction and (AEditingStyle in [esEdit, esNoEdit]) and
    (ActiveProperties.MaxLength > 0) then
  begin
    ...
    ANewText := ANewText + Copy(Text, SelStart + SelLength + 1,
      Length(Text) - SelStart - SelLength);
  ...
  begin
    ...
    begin
      ...
      ANewText := Copy(ANewText, 1, SelStart) + AValue +
        Copy(ANewText, SelStart + SelLength + 1, Length(ANewText) - SelLength -
        SelStart);
    end
    else
      ANewText := AValue;
...
begin
  ...
  begin
    if FLastFirstVisibleCharIndex <> InnerTextEdit.GetFirstVisibleCharIndex then
      DoLayoutChanged
    else if (InnerTextEdit.SelStart <> FLastSelPosition) or
...
begin
  case Message.Msg of
    ...
    begin
      dxMessagesController.KillMessages(InnerControl.Handle,
        DXM_SPELL_REDRAWMISSPELLINGS);
      DoDrawMisspellings;
    end;
  end;
end;
...
procedure TcxCustomTextEdit.SpellCheckerSetSelText(const AValue: string;
  APost: Boolean = False);
begin
  FIsChangeBySpellChecker := True;
  try
...
begin
  Result := TdxTextMeasurer.TextLineHeight
    (TControlAccess(InnerTextEdit.Control).Font);
end;
...
function TcxCustomTextEdit.GetScrollLookupDataList(AScrollCause
  : TcxEditScrollCause): Boolean;
begin
  Result := False;
end;
...
function TcxCustomTextEdit.InternalMouseWheel(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
const
  AGoDirectionMap: array [Boolean] of TcxEditLookupDataGoDirection =
    (egdNext, egdPrev);
var
  APrevCurrentKey: TcxEditValue;
begin
...
function TcxCustomTextEdit.IsMouseWheelHandleNeeded(Shift: TShiftState;
  WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
...
function TcxCustomTextEdit.ItemIndexToLookupKey(AItemIndex: Integer)
  : TcxEditValue;
begin
  Result := AItemIndex;
end;
...
function TcxCustomTextEdit.LookupKeyToEditValue(const AKey: TcxEditValue)
  : TcxEditValue;
var
  AText: string;
begin
...
function TcxCustomTextEdit.LookupKeyToItemIndex
  (const AKey: TcxEditValue): Integer;
begin
  Result := AKey;
end;

function TcxCustomTextEdit.NeedsInvokeAfterKeyDown(AKey: Word;
  AShift: TShiftState): Boolean;
begin
  Result := False;
end;
...
begin
  Result := not IsInplace and not DataBinding.IDefaultValuesProvider.
    IsDataStorage and not ActiveProperties.NeedUseNullString(EditValue);
end;
...
begin
  ...
  begin
    ...
    AIsEditValueValid := ActiveProperties.IsEditValueValid(AEditValue,
      InternalFocused);
    ...
    else if AIsEditValueValid then
...
begin
  ...
  if ACompareEditValue then
    AEditValueChanged := not InternalVarEqualsExact(APrevEditValue,
      ANewEditValue)
  else
...
procedure TcxCustomTextEdit.UpdateDrawValue;
  ...
  begin
    ...
    if AEditingStyle in [esFixedList, esNoEdit] then
      with ViewInfo do
        if (AEditingStyle = esNoEdit) or not FindSelection or
          not ActiveProperties.FixedListSelection then
          SelLength := 0
        else
        begin
          if DrawSelectionBar then
...
begin
  ...
  begin
    if ModifiedAfterEnter and not IsEditValidated or NeedResetInvalidTextWhenPropertiesChanged
    then
    begin
  ...
  else if not ModifiedAfterEnter then
...
begin
  Result := GetScrollLookupDataList(escMouseWheel) and not HasPopupWindow and
    ActiveProperties.UseLookupData and not ILookupData.IsEmpty and
    HandleMouseWheel(Shift);
end;
...
begin
  AEditingStyle := ActiveProperties.EditingStyle;
  if (AEditingStyle = esFixedList) and not FindSelection or
    (AEditingStyle = esNoEdit) then
    Result := 0
  else
...
procedure TcxCustomTextEdit.TogglePassword(AButtonVisibleIndex: Integer;
  AIsMouseDown: Boolean = False);
var
  ALastPressedButton: Integer;
begin
  if Properties.ShowPasswordRevealButton and
    (Properties.PasswordRevealButtonIndex = AButtonVisibleIndex) then
  begin
...
begin
  if HasInnerEdit and not IsInplace and not(csDestroyingHandle in ControlState)
  then
...
GetRegisteredEditProperties.Register(TcxTextEditProperties,
  scxSEditRepositoryTextItem);
FilterEditsController.Register(TcxTextEditProperties, TcxFilterTextEditHelper);

finalization

FilterEditsController.Unregister(TcxTextEditProperties,
  TcxFilterTextEditHelper);
