...
type
  ...
  public
    class var Defaults: TdxTextColors;
  public const
    DefaultBoundary = 3982098;
  public
    constructor Create(ADefaultBackgroundColor: TdxAlphaColor;
      ALightTextColor: TdxAlphaColor; ADarkTextColor: TdxAlphaColor;
      ADarkBoundary: Integer);
    class function FromSkinColors(ABackgroundColor: TdxAlphaColor;
      ATextColor: TdxAlphaColor): TdxTextColors; static;
  ...
  public
    ...
    class function IsDarkColor(AColor: TdxAlphaColor; ABoundary: Integer)
      : Boolean; static;
    class function GetActualForeColor(ABackColor, AForeColor: TdxAlphaColor;
      ATextColors: TdxTextColors): TdxAlphaColor; static;
  end;
...
constructor TdxTextColors.Create(ADefaultBackgroundColor: TdxAlphaColor;
  ALightTextColor: TdxAlphaColor; ADarkTextColor: TdxAlphaColor;
  ADarkBoundary: Integer);
begin
  inherited Create;
...
class function TdxTextColors.FromSkinColors(ABackgroundColor: TdxAlphaColor;
  ATextColor: TdxAlphaColor): TdxTextColors;
var
...
begin
  ...
  if ABackgroundLuma < ATextColorLuma then
    Result := TdxTextColors.Create(ABackgroundColor, ATextColor,
      TdxAlphaColors.Black, Max(ABackgroundLuma + 1, DefaultBoundary))
  else
    Result := TdxTextColors.Create(ABackgroundColor, TdxAlphaColors.White,
      ATextColor, Min(ABackgroundLuma - 1, DefaultBoundary));
end;
...
begin
  Result := 19595 * dxGetRed(AColor) + 38470 * dxGetGreen(AColor) + 7471 *
    dxGetBlue(AColor);
end;

class function TdxAutoColorUtils.IsDarkColor(AColor: TdxAlphaColor;
  ABoundary: Integer): Boolean;
begin
  Result := CalculateLumaY(AColor) < ABoundary;
end;

class function TdxAutoColorUtils.GetActualForeColor(ABackColor,
  AForeColor: TdxAlphaColor; ATextColors: TdxTextColors): TdxAlphaColor;
begin
  if not TdxAlphaColors.IsEmpty(AForeColor) then
    Exit(AForeColor);
  if not TdxAlphaColors.IsEmpty(ABackColor) and
    (IsDarkColor(ABackColor, ATextColors.DarkBoundary)) then
