...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  RTLConsts, Windows, Generics.Defaults, Generics.Collections, Classes, Types,
  SysUtils, Math, Variants, Graphics,
  dxCore, dxOLEDocument, cxClasses, cxVariants, dxCoreClasses, dxGDIPlusClasses,
  dxHashUtils, dxCoreGraphics,
  dxSpreadSheetCore, dxSpreadSheetTypes, dxSpreadSheetClasses,
  dxSpreadSheetFormatXLSTypes, dxSpreadSheetUtils,
  dxSpreadSheetGraphics, cxGraphics, dxSpreadSheetFormulas,
  dxSpreadSheetFunctions, dxSpreadSheetFormatUtils,
  dxSpreadSheetPrinting, dxSpreadSheetConditionalFormattingRules,
  dxSpreadSheetContainers, dxSpreadSheetHyperlinks,
  dxSpreadSheetFormatXLSProtection, dxSpreadSheetProtection,
  dxSpreadSheetCoreStyles, dxSpreadSheetStrs,
  dxSpreadSheetStyles;
...
type
  ...
  protected
    procedure AddBand;
    procedure WriteRecordBlock(const ABuffer; ACount: Integer;
      ADataIsString: Boolean);
  public
    constructor Create;
  ...
  protected
    function GetFormattedStringData
      (AValue: TdxSpreadSheetFormattedSharedString): TBytes;
  public
    ...
    procedure XLS_WriteSimpleString(const AValue: string; ASize: Integer;
      AOffset: Integer = -1); overload;
    ...
    property Integers[AOffset: Integer]: Integer read GetInteger
      write SetInteger;
  ...
  TdxSpreadSheetXLSWriterProgressHelper = class
  ...
  strict private
    ...
    function WriteFormulaResult(AFormula: TdxSpreadSheetFormula;
  ...
  protected
    ...
    function CreateRecordWriter(ARecordID: Integer; AWriter: TdxXLSMethod)
      : TdxXLSRecordWriterInfo;
    procedure DoWriteRecord(ARecordInfo: TdxXLSRecordWriterInfo);
    procedure EncryptWorkbook;
    function GetColorIndex(AColor: TColor; ADefValue, ADefIndex: Integer;
      ANoneIndex: Integer = -1): Integer;
    ...
    procedure WriteName(AName: TdxSpreadSheetDefinedName;
      AIsSpecialName: Boolean = False);
    ...
    procedure WritePrintArea(const AName, AReference: string;
      AScope: TdxSpreadSheetTableView);
    ...
    procedure WriteStyle(AStyle: TdxSpreadSheetCellStyleHandle;
      ACellStyle: Boolean = True; ADataFormat: Integer = -1);
    procedure WriteStyleExtProperties(AID: Word;
    ...
    procedure WriteColumnFormatInformation(AStartColumn, AFinishColumn
      : TdxSpreadSheetTableColumn);
    procedure WriteColumns;
    procedure WriteComments;
    procedure WriteConditionalFormatting;
    procedure WriteConditionalFormattingRule
      (ARule: TdxSpreadSheetConditionalFormattingRuleExpression);
    procedure WriteConditionalFormattingRuleStyle(var AOptions: Cardinal;
      AStyle: TdxSpreadSheetCellStyle);
    procedure WriteConditionalFormattingRuleStyleBorders(var AOptions: Cardinal;
      ADefaultBorders, ABorders: TdxSpreadSheetBordersHandle);
    procedure WriteConditionalFormattingRuleStyleBrush(var AOptions: Cardinal;
      ADefaultBrush, ABrush: TdxSpreadSheetBrushHandle);
    procedure WriteConditionalFormattingRuleStyleFont(ADefaultFont,
    ...
    procedure WriteMSODrawing(AContainer: TdxSpreadSheetContainer;
      var AIndex, ASize: Integer);
    ...
    property CurrentSheetDimensions: TRect read FCurrentSheetDimensions
      write FCurrentSheetDimensions;
    property Encryptor: TdxXLSAbstractEncryptor read FEncryptor;
    property ExtSSTBuckets: TList<TdxXLSWriterSSTBucketInfo>
      read FExtSSTBuckets;
    property ProgressValue: Integer read GetProgressValue
      write SetProgressValue;
    property TotalObjectsCount: Integer read FTotalObjectsCount
      write FTotalObjectsCount;
  public
    ...
    property BoundSheets: TDictionary<TdxSpreadSheetTableView, Integer>
      read FBoundSheets;
    ...
    property CommonWriters: TObjectList<TdxXLSRecordWriterInfo>
      read FCommonWriters;
    property Containers: TDictionary<TdxSpreadSheetContainer, Integer>
      read FContainers;
    ...
    property Formats: TDictionary<TdxSpreadSheetFormatHandle, Integer>
      read FFormats;
    ...
    property Styles: TDictionary<TdxSpreadSheetCellStyleHandle, Integer>
      read FStyles;
...
uses
  cxGeometry, dxSpreadSheetFormatXLSFormulas, dxSpreadSheetFormatXLSDrawing,
  dxSpreadSheetCoreHelpers,
  dxSpreadSheetFormatXLS, dxSpreadSheetCoreFormulas,
...
type
  ...
  TdxSpreadSheetConditionalFormattingExpressionAccess = class
...
constructor TdxSpreadSheetXLSWriter.Create(AOwner: TdxCustomSpreadSheet;
  AStream: TStream);
var
...
begin
  ...
  FWorkbookData := FDocument.CreateStream(ARoot, sdxXLSWorkbookStream)
    as TMemoryStream;
  if AOwner.OptionsMetadata.HasData then
    FMetadata := FDocument.CreateStream(ARoot, sdxXLSSummaryInformationStream)
      as TMemoryStream;
  ...
  FFonts := TDictionary<TdxSpreadSheetFontHandle, Integer>.Create
    (SpreadSheet.CellStyles.Fonts.Count + 1);
...
begin
  ...
  else if SpreadSheet.OptionsProtection.Protected then
    Result := TdxXLSStandardEncryptor.Create(dxSpreadSheetDefaultPassword)
  else
    Result := nil;
end;

function TdxSpreadSheetXLSWriter.CreateProgressHelper
  : TdxSpreadSheetCustomFilerProgressHelper;
begin
...
begin
  ...
  Inc(FTotalObjectsCount, TdxSpreadSheetAccess(SpreadSheet)
    .StringTable.UniqueCount);
  ...
  begin
    ...
    while ARow <> nil do
    begin
      Inc(FTotalObjectsCount, GetRowCellCount(TdxSpreadSheetRowAccess(ARow)
        .RowCells) + 1);
...
function TdxSpreadSheetXLSWriter.CreateRecordWriter(ARecordID: Integer;
AWriter: TdxXLSMethod): TdxXLSRecordWriterInfo;
begin
...
procedure TdxSpreadSheetXLSWriter.DoWriteRecord(ARecordInfo
  : TdxXLSRecordWriterInfo);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Encryptor.Encrypt(PByte(WorkbookData.Memory) + WorkbookData.Position +
        AOffset, WorkbookData.Position + AOffset, ASize - AOffset);
    end;
...
function TdxSpreadSheetXLSWriter.GetContainerID
  (AContainer: TdxSpreadSheetContainer): Integer;
begin
  if not Containers.TryGetValue(AContainer, Result) then
    Result := -1;
end;

function TdxSpreadSheetXLSWriter.HasPicture(AContainer
  : TdxSpreadSheetContainer): Boolean;
begin
  Result := (AContainer is TdxSpreadSheetPictureContainer) and
    not TdxSpreadSheetPictureContainer(AContainer).Picture.Empty or
    (AContainer is TdxSpreadSheetShapeContainer) and
    not TdxSpreadSheetShapeContainer(AContainer).Shape.Brush.Texture.Empty;
end;
...
begin
  ...
  begin
    ...
    Move(dxExcelStandardColors[0], Palette[0], Length(dxExcelStandardColors) *
      SizeOf(TColor));
  end;
...
var
  ...
  function CanProcessContainerAsShape(AContainer
    : TdxSpreadSheetContainer): Boolean;
  begin
    Result := (AContainer.ClassType = TdxSpreadSheetCommentContainer) or
...
begin
  ...
  TdxHashTableAccess(TdxSpreadSheetAccess(SpreadSheet).SharedImages).ForEach(
    procedure(AItem: TdxDynamicListItem)
    begin
      FDrawings.Add(TdxSpreadSheetSharedImageHandle(AItem),
        FDrawings.Count + 1);
  ...
  begin
    ...
    begin
      ...
      if AContainer.Visible or
        (AContainer.ClassType = TdxSpreadSheetCommentContainer) then
      begin
        ...
        else if CanProcessContainerAsShape(AContainer) then
          AddShape(AContainer)
        else
          Continue;
...
procedure TdxSpreadSheetXLSWriter.RegisterCommonWriter(ARecordID: Integer;
AWriter: TdxXLSMethod);
begin
  CommonWriters.Add(CreateRecordWriter(ARecordID, AWriter));
end;

procedure TdxSpreadSheetXLSWriter.RegisterViewWriter(ARecordID: Integer;
AWriter: TdxXLSMethod);
begin
...
procedure TdxSpreadSheetXLSWriter.WriteTableViewSheet
  (ASheet: TdxSpreadSheetTableView);
var
  AIndex: Integer;
begin
  ...
  PInteger(@PByteArray(WorkbookData.Memory)^[BoundSheets[ASheet] + 4])^ :=
    WorkbookData.Size;
...
begin
  for ASheetIndex := 0 to SpreadSheet.SheetCount - 1 do
    if SpreadSheet.Sheets[ASheetIndex] is TdxSpreadSheetTableView then
      WriteTableViewSheet(SpreadSheet.Sheets[ASheetIndex]
...
procedure TdxSpreadSheetXLSWriter.WriteDataFormat
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        try
          Writer.WriteWord(TdxSpreadSheetCommentContainer(AContainer)
            .Cell.RowIndex);
          Writer.WriteWord(TdxSpreadSheetCommentContainer(AContainer)
            .Cell.ColumnIndex);
          Writer.WriteWord(IfThen(TdxSpreadSheetCommentContainer(AContainer)
            .Visible, $2));
          Writer.WriteWord(AContainerID);

          AAuthor := dxStringToAnsiString
            (TdxSpreadSheetCommentContainer(AContainer).Author);
...
begin
  ...
  try
    ...
    Writer.Words[2] := AItalic[fsItalic in AFont.Style] + AStrikeOut
      [fsStrikeOut in AFont.Style];
    ...
    Writer.Bytes[12] := IfThen(AFont.Charset = DEFAULT_CHARSET, 0,
      Integer(AFont.Charset));
...
procedure TdxSpreadSheetXLSWriter.WriteName(AName: TdxSpreadSheetDefinedName;
AIsSpecialName: Boolean = False);
var
...
procedure TdxSpreadSheetXLSWriter.WriteNameComment
...
procedure TdxSpreadSheetXLSWriter.WritePrintArea(const AName,
  AReference: string; AScope: TdxSpreadSheetTableView);
var
...
procedure TdxSpreadSheetXLSWriter.WritePrintAreas
  (AScope: TdxSpreadSheetTableView);
begin
  if AScope.OptionsPrint.Source.Area.Assigned then
    WritePrintArea(#6, dxReferenceToString(AScope.OptionsPrint.Source.Area.Rect,
      False, [croSheetName], AScope.Caption), AScope);
  if AScope.OptionsPrint.Source.RowsToRepeat.Assigned or
    AScope.OptionsPrint.Source.ColumnsToRepeat.Assigned then
    WritePrintArea(#7, TdxSpreadSheetPrintAreasHelper.BuildPrintTitlesReference
      (AScope), AScope);
end;
...
begin
  ...
  TdxHashTableAccess(AStrings).ForEach(procedure(AItem: TdxDynamicListItem)
    begin
...
procedure TdxSpreadSheetXLSWriter.WriteStyle
  (AStyle: TdxSpreadSheetCellStyleHandle; ACellStyle: Boolean = True;
ADataFormat: Integer = -1);
const
  AAssignedState = $0400 or $0800 or $1000 or $2000 or $4000 or $8000;
  ADefaultValues: array [0 .. 19] of Byte = ($05, $00, $00, $00, $F5, $FF, $20,
    $00, $00, $F8, $00, $00, $00, $00, $00, $00, $00, $00, $C0, $20);
...
begin
  ...
  try
    ...
    Writer.Words[4] := Writer.Words[4] or Byte(csLocked in AStyle.States) or
      Byte(csHidden in AStyle.States) shl 1;
    //
    Writer.Words[6] := Byte(AStyle.AlignHorz) or
      (Byte(csWordWrap in AStyle.States) shl 3) or
      (Byte(AStyle.AlignVert) shl 4);
    //
    Writer.Words[8] := (AStyle.AlignHorzIndent and $F) or
      (Byte(csShrinkToFit in AStyle.States) shl 4) or AAssignedState;
    //
    Writer.Words[10] := GetLineStyle(bLeft) or GetLineStyle(bRight) shl 4 or
      GetLineStyle(bTop) shl 8 or GetLineStyle(bBottom) shl 12;
    ...
    if (AStyle.Brush.BackgroundColor <> clDefault) or
      (AStyle.Brush.ForegroundColor <> clDefault) then
    begin
      Writer.Integers[14] := Writer.Integers[14] or
        FillStyleMap[AStyle.Brush.Style] shl 26 or $02000000;
      ...
      Writer.Words[18] := GetColorIndex(AForegroundColor, clDefault, $41, $41)
        or GetColorIndex(ABackgroundColor, clWindow, $40, $40) shl 7;
    end
    else
...
procedure TdxSpreadSheetXLSWriter.WriteStyleExtProperties(AID: Word;
AStyle: TdxSpreadSheetCellStyleHandle);
var
  APropCount: Integer;

  procedure CheckColorPropValueValue(AColor: TColor;
  ...
  begin
    ...
    if (AColor = clDefault) or (AColor = clNone) or (AIndex = -1) or
      (Palette[AIndex] = ColorToRgb(AColor)) then
      Exit;
    Inc(APropCount);
    ...
    with TRGBQuad(ColorToRgb(AColor)) do
      AColorData.xclrValue := dxColorToAlphaColor
        (Rgb(rgbRed, rgbGreen, rgbBlue), 255);
...
begin
  ...
  try
    ...
    //
    CheckColorPropValueValue(AStyle.Borders.BorderColor[bTop],
      xfextBorderColorTop);
    CheckColorPropValueValue(AStyle.Borders.BorderColor[bLeft],
      xfextBorderColorLeft);
    CheckColorPropValueValue(AStyle.Borders.BorderColor[bRight],
      xfextBorderColorRight);
    CheckColorPropValueValue(AStyle.Borders.BorderColor[bBottom],
...
const
  PredefinedStyles: array [0 .. 5, 0 .. 3] of Byte = (($14, $80, $03, $FF),
    ($14, $80, $06, $FF), ($13, $80, $04, $FF), ($13, $80, $07, $FF),
    ($00, $80, $00, $FF), ($12, $80, $05, $FF));
var
  I: Integer;
begin
  ...
  begin
    ...
    try
      RecordWriter.WriteBuffer(PredefinedStyles[I],
        SizeOf(PredefinedStyles[I]));
    finally
      RecordWriter.EndWrite(WorkbookData);
    end;
  end;
...
procedure TdxSpreadSheetXLSWriter.WriteCell(ARowIndex: Integer;
ACell: TdxSpreadSheetCell);
const
  RecordType: array [TdxSpreadSheetCellDataType] of Word = (brcBlank,
    brcBoolErr, brcBoolErr, brcNumber, brcNumber, brcNumber, brcNumber,
    brcLabelSST, brcFORMULA);
var
...
begin
  ...
  AController := TdxSpreadSheetCustomFormulaControllerAccess
    (SpreadSheet.FormulaController);
  if AController.IsPartOfArrayFormula(CurrentSheet, ARowIndex,
    ACell.ColumnIndex, AFormulaArea) = afpSlaveCell then
    Exit;
...
procedure TdxSpreadSheetXLSWriter.WriteCellArrayFormula
  (ACell: TdxSpreadSheetCell);
var
...
begin
  AValue := Null;
  if ACell.AsFormula.IsArrayFormula or not WriteFormulaResult(ACell.AsFormula,
    AValue) then
    Writer.WriteDateTime(0);
  Writer.WriteWord(3); // Always calculate (options)
  Writer.WriteInteger(0); // Reserved
...
procedure TdxSpreadSheetXLSWriter.WriteColumnFormatInformation(AStartColumn,
  AFinishColumn: TdxSpreadSheetTableColumn);
var
...
begin
  ...
  try
    ...
    Writer.WriteWord(MulDiv(TdxSpreadSheetItemsAccess(CurrentSheet.Columns)
      .GetRealItemSize(AStartColumn.Index), 256,
      ColumnWidthHelper.MaxDigitWidth));
    ...
    if AGroup <> nil then
      AOptions := (((AGroup.Level + 1) and $7) or
        (Byte(not AGroup.Expanded) shl 4)) shl 8 or AOptions;
...
procedure TdxSpreadSheetXLSWriter.WriteColumns;
  ...
  function IsColumnsEqual(AColumn1, AColumn2
    : TdxSpreadSheetTableColumn): Boolean;
  begin
    Result := (AColumn2 <> nil) and (AColumn1.Size = AColumn2.Size) and
      (AColumn1.Visible = AColumn2.Visible) and
      (AColumn1.Style.Handle = AColumn2.Style.Handle) and
      (AColumn1.DefaultSize = AColumn2.DefaultSize) and
      (CurrentSheet.Columns.Groups.Find(AColumn1.Index)
      = CurrentSheet.Columns.Groups.Find(AColumn2.Index));
  end;
...
begin
  ...
  begin
    ...
    while IsColumnsEqual(AFirstColumn,
      TdxSpreadSheetTableColumn(ALastColumn.Next)) do
...
begin
  ...
  begin
    ...
    if (ARule is TdxSpreadSheetConditionalFormattingRuleExpression) and
      (ARule.Areas.Count > 0) then
    begin
      ...
      WriteConditionalFormattingRule
        (TdxSpreadSheetConditionalFormattingRuleExpression(ARule));
    end;
  end;
end;

procedure TdxSpreadSheetXLSWriter.WriteConditionalFormattingRule
  (ARule: TdxSpreadSheetConditionalFormattingRuleExpression);
...
begin
  ...
  try
    ...
    begin
      Writer.WriteByte(1);
      Writer.WriteByte(OperatorMap[TdxSpreadSheetConditionalFormattingRuleCellIs
        (ARule).ComparisonOperator]);
    end
    else
    begin
      Writer.WriteByte(2);
    ...
    ASize1 := WriteFormula
      (TdxSpreadSheetConditionalFormattingRuleExpressionAccess(ARule)
      .Formulas[0]);
    ASize2 := WriteFormula
      (TdxSpreadSheetConditionalFormattingRuleExpressionAccess(ARule)
      .Formulas[1]);
...
procedure TdxSpreadSheetXLSWriter.WriteConditionalFormattingRuleStyle
  (var AOptions: Cardinal; AStyle: TdxSpreadSheetCellStyle);
var
...
begin
  ...
  begin
    ...
    WriteConditionalFormattingRuleStyleFont(ADefaultStyle.Handle.Font,
      AStyle.Handle.Font);
  end;
  ...
  begin
    ...
    WriteConditionalFormattingRuleStyleBorders(AOptions,
      ADefaultStyle.Handle.Borders, AStyle.Handle.Borders);
  end;

  if not AStyle.Handle.Brush.IsEqual(ADefaultStyle.Handle.Brush) then
  begin
    AOptions := AOptions or CFRULE_OPTION_PATTERN;
    WriteConditionalFormattingRuleStyleBrush(AOptions,
      ADefaultStyle.Handle.Brush, AStyle.Handle.Brush);
  end;
end;

procedure TdxSpreadSheetXLSWriter.WriteConditionalFormattingRuleStyleBorders
  (var AOptions: Cardinal; ADefaultBorders,
  ABorders: TdxSpreadSheetBordersHandle);

  function CheckNeedWriteBorder(ASide: TcxBorder): Boolean;
  begin
    Result := (ADefaultBorders.BorderColor[ASide] <> ABorders.BorderColor[ASide]
      ) or (ADefaultBorders.BorderStyle[ASide] <> ABorders.BorderStyle[ASide]);
  end;
...
const
  ...
  OptionFlags: array [TcxBorder] of Cardinal = (CFRULE_OPTION_BORDER_LEFT,
    CFRULE_OPTION_BORDER_TOP, CFRULE_OPTION_BORDER_RIGHT,
    CFRULE_OPTION_BORDER_BOTTOM);
var
  AFlags: Word;
  ASide: TcxBorder;
begin
  AOptions := AOptions or CFRULE_OPTION_BORDER_BLTR or
    CFRULE_OPTION_BORDER_TLBR;

  AFlags := 0;
  for ASide := Low(ASide) to High(ASide) do
  begin
    if CheckNeedWriteBorder(ASide) then
      AFlags := AFlags or (BorderLineStyleMap[ABorders.BorderStyle[ASide]]
        shl Offsets[ASide])
    else
...
begin
  ...
  if ABrush.ForegroundColor <> ADefaultBrush.ForegroundColor then
    AFlags := AFlags or (GetColorIndex(ABrush.ForegroundColor, clWindow,
      $40, $40))
  else
...
procedure TdxSpreadSheetXLSWriter.WriteConditionalFormattingRuleStyleFont
  (ADefaultFont, AFont: TdxSpreadSheetFontHandle);
var
...
begin
  ...
  begin
    AFontName[0] := Min(Length(AFont.Name), (Length(AFontName) - 2)
      div SizeOf(WideChar));
    AFontName[1] := 1;
  ...
  Writer.WriteInteger(IfThen(fsItalic in AFont.Style, $2) or
    IfThen(fsStrikeOut in AFont.Style, $8));
...
begin
  Writer.WriteWord(MulDiv(CurrentSheet.Options.DefaultColumnWidth, 256,
    ColumnWidthHelper.MaxDigitWidth));
end;
...
begin
  Writer.WriteWord(1);
  Writer.WriteWord(Round(TdxValueUnitsHelper.PixelsToPoints
    (CurrentSheet.Options.DefaultRowHeight) * 20));
end;
...
procedure TdxSpreadSheetXLSWriter.WriteGuts;
  ...
  begin
    ...
    for I := 0 to AGroups.Count - 1 do
      Result := Max(Result, TdxSpreadSheetTableItemGroupAccess(AGroups.Items[I])
        .MaxNestingLevel + 1);
  end;
...
procedure TdxSpreadSheetXLSWriter.WriteHyperlink
...
procedure TdxSpreadSheetXLSWriter.WriteHyperlinkTooltip
  (AHyperlink: TdxSpreadSheetHyperlink);
var
  S: string;
begin
...
begin
  ...
  Writer.WriteWord(CurrentSheet.FrozenRow + 1);
  Writer.WriteWord(CurrentSheet.FrozenColumn + 1);
  Writer.WriteWord(FrozenPaneFlags[CurrentSheet.FrozenColumn >= 0,
    CurrentSheet.FrozenRow >= 0]);
end;
...
begin
  ...
  if (AProtection is TdxSpreadSheetCustomProtectionInfo) and
    not(AProtection is TdxSpreadSheetStandardProtectionInfo) then
    AProtection := TdxSpreadSheetStandardProtectionInfo.Create
      (TdxSpreadSheetCustomProtectionInfo(AProtection).Password);
...
procedure TdxSpreadSheetXLSWriter.WriteMergedCells;
  ...
  begin
    Result := ((R.Left >= 0) and (R.Top >= 0)) and
      ...
      ((R.Left <= R.Right) or (R.Top <= R.Bottom)) and
      ((R.Left <= dxMaxColumn) and (R.Top < dxMaxRow));
  end;
...
procedure TdxSpreadSheetXLSWriter.WriteMSODrawingObjectInfo
  (AContainer: TdxSpreadSheetContainer);
  ...
  begin
    if AContainer.ClassType = TdxSpreadSheetTextBoxContainer then
      ...
      Result := ShapeID[(AContainer as TdxSpreadSheetShapeContainer)
        .Shape.ShapeType];
  end;
...
procedure TdxSpreadSheetXLSWriter.WriteMSODrawing
  (AContainer: TdxSpreadSheetContainer; var AIndex, ASize: Integer);
var
...
begin
  ...
  if AContainer is TdxSpreadSheetCustomTextBoxContainer then
    WriteMSOTextObject(TdxSpreadSheetCustomTextBoxContainerAccess
      (AContainer).TextBox);
end;
...
procedure TdxSpreadSheetXLSWriter.WriteMSOTextObject
  ...
  begin
    Result := (AlignHorzMap[ATextBox.AlignHorz] shl 1) or
      (AlignVertMap[ATextBox.AlignVert] shl 4);
  end;
...
var
  AText: string;
begin
  AText := TdxSpreadSheetHeaderFooterHelper.Build
    (CurrentSheet.OptionsPrint.HeaderFooter.CommonFooter);
  if AText = '' then
    RecordWriter.Cancel
  else
...
procedure TdxSpreadSheetXLSWriter.WriteRowFormatInformation
  (ARow: TdxSpreadSheetTableRow);
var
  AGroup: TdxSpreadSheetTableItemGroup;
  AOptions: Word;
  ASize: Integer;
begin
  AOptions := 320 or Byte(TdxSpreadSheetRowAccess(ARow).IsCustomSize) shl 6 or
  ...
  try
    ...
    ASize := TdxSpreadSheetItemsAccess(CurrentSheet.Rows)
      .GetRealItemSize(ARow.Index);
    ...
    if AGroup <> nil then
      AOptions := AOptions or ((AGroup.Level + 1) and $7) or
        Byte(not AGroup.Expanded) shl 4;
...
begin
  ...
  begin
    Writer.WriteWord(1);
    Writer.WriteWord(1);
    Writer.XLS_WriteSimpleRef(cxRectBounds(CurrentSheet.Selection.FocusedColumn,
      CurrentSheet.Selection.FocusedRow, 0, 0));
  end;
end;
...
begin
  ...
  Writer.WriteByte(1);
  Writer.WriteInteger(-1);
  Writer.WriteInteger(IfThen(CurrentSheet.OptionsProtection.AllowDeleteColumns,
    iprotDeleteColumns) or
    IfThen(CurrentSheet.OptionsProtection.AllowDeleteRows, iprotDeleteRows) or
    IfThen(CurrentSheet.OptionsProtection.AllowResizeColumns,
    iprotFormatColumns) or
    IfThen(CurrentSheet.OptionsProtection.AllowEditContainers, iprotObject) or
    IfThen(CurrentSheet.OptionsProtection.AllowEditHyperlinks,
    iprotInsertHyperlinks) or
    ...
    IfThen(CurrentSheet.OptionsProtection.AllowInsertColumns,
    iprotInsertColumns) or
    IfThen(CurrentSheet.OptionsProtection.AllowInsertRows, iprotInsertRows) or
    IfThen(CurrentSheet.OptionsProtection.AllowSelectLockedCells,
    iprotSelLockedCells) or
    IfThen(CurrentSheet.OptionsProtection.AllowSelectUnlockedCells,
    iprotSelUnlockedCells) or IfThen(CurrentSheet.OptionsProtection.AllowSort,
    iprotSort));
end;
...
begin
  ...
  Writer.Words[0] := Byte(CurrentSheet.Options.ActualShowFormulas) or
    ...
    Byte((CurrentSheet.FrozenColumn >= 0) or (CurrentSheet.FrozenRow >= 0))
    shl 3 or Byte(CurrentSheet.Options.ActualZeroValues) shl 4 or
    Byte(SpreadSheet.OptionsView.GridLineColor = clDefault) shl 5 or
    Byte(CurrentSheet.Options.ActualRightToLeftLayout) shl 6 or
    Byte(CurrentSheet.Columns.Groups.Count + CurrentSheet.Rows.Groups.Count > 0)
    shl 7 or Byte(CurrentSheet.Active) shl 9;
  Writer.Words[2] := CurrentSheet.TopRow;
  Writer.Words[4] := CurrentSheet.LeftColumn;
  Writer.Integers[6] := GetColorIndex
    (SpreadSheet.OptionsView.GridLineColor, 0, 0, 0);
end;
...
procedure TdxSpreadSheetXLSWriter.WriteDefaultBoolean
...
begin
  ...
  else if not VarIsStr(AValue) then
    Result := False;
  if Result and ((ACode <> ecNone) or (VarType(AValue) = varBoolean) or
    VarIsStr(AValue)) then
...
procedure TdxXLSRecordWriter.WriteRecordBlock(const ABuffer; ACount: Integer;
ADataIsString: Boolean);
var
...
begin
  ...
  while ACount > 0 do
  begin
    ABlockSize := Min(dxXLSMaxRecordSize - Byte(ADataIsString and (AOffset <> 0)
      ) - CurrentBandSize, ACount);
...
procedure TdxXLSWriter.XLS_WriteSharedString
...
begin
  ...
  begin
    AData := GetFormattedStringData(TdxSpreadSheetFormattedSharedString
      (AValue));
...
procedure TdxXLSWriter.XLS_WriteSimpleString(const AValue: string;
ASize: Integer; AOffset: Integer = -1);
var
  L: Byte;
begin
  if AOffset >= 0 then
...
function TdxXLSWriter.GetFormattedStringData
  (AValue: TdxSpreadSheetFormattedSharedString): TBytes;
var
