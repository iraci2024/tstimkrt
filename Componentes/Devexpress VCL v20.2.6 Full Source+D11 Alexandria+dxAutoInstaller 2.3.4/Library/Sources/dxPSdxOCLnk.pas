...
uses
  ...
  dxCore, dxOrgChr, dxPSCore, dxPSGraphicLnk,
  cxLabel, cxControls, cxContainer, cxEdit, cxCheckBox, cxGraphics, cxTextEdit,
  cxMaskEdit, cxDropDownEdit, Menus, Types,
...
function dxGetOrgChartAsGraphic(const AData: TdxOrgChartGetAsGraphicData)
  : TGraphic;
...
function dxGetOrgChartAsGraphic(const AData: TdxOrgChartGetAsGraphicData)
  ...
  function CreateGraphic(out ACanvas: TCanvas;
    out AIsBitmap, AIsMetaFile: Boolean): TGraphic;
  begin
...
begin
  Result := nil;
  if (OrgChart = nil) or (OrgChart.Count = 0) then
    Exit;
...
dxPSRegisterReportLink(TdxOrgChartReportLink, TdxOrgChart,
  TdxOCReportLinkDesignWindow);

finalization

dxPSUnregisterReportLink(TdxOrgChartReportLink, TdxOrgChart,
  TdxOCReportLinkDesignWindow);
