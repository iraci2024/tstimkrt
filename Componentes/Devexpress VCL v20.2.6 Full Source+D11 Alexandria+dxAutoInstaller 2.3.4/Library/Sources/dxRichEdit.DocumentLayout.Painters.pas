...
uses
  ...
  Generics.Collections, Controls, StdCtrls, Forms,
  dxCore, dxCoreClasses, dxCoreGraphics, cxGeometry, cxLookAndFeels, cxGraphics,
...
type
  ...
  IdxCustomMarkExporter = interface
    ...
    function GetCustomMarkVisualInfoCollection
      : TdxCustomMarkVisualInfoCollection;
    procedure ExportCustomMarkBox(ACustomMark: TdxCustomMark;
      const ABounds: TRect);
    property CustomMarkVisualInfoCollection: TdxCustomMarkVisualInfoCollection
      read GetCustomMarkVisualInfoCollection;
  end;
  ...
  public
    ...
    procedure DrawRectangularHotZone(const ABounds: TRect;
      AColor: TdxAlphaColor); virtual;
    procedure DrawEllipticHotZone(const ABounds: TRect;
      AColor: TdxAlphaColor); virtual;
    function CreateFillBrush(const ABounds: TRect; AColor: TdxAlphaColor)
      : TdxGPCustomBrush; virtual;
    ...
    procedure DrawLine(const AFrom: TPoint; const ATo: TPoint);
      virtual; abstract;

    property Graphics: TdxGraphics read FGraphics;
    class property HotZoneGradientColor: TdxAlphaColor
      read FHotZoneGradientColor;
    class property HotZoneRotationGradientColor: TdxAlphaColor
      read FHotZoneRotationGradientColor;
  ...
  TdxRichEditSelectionPainter = class abstract(TdxOfficeSelectionPainter,
    IdxSelectionPainter, IdxHotZoneVisitor, IdxRectangularObjectHotZoneVisitor)
  // for internal use
  public const
    SelectionColor: TdxAlphaColor = $7F5097E3;
  strict private
    class var FAnchorImage: TdxSmartImage;
  ...
  public
    ...
    procedure Draw(AViewInfo: TdxRectangularObjectSelectionLayout);
      overload; virtual;
    procedure Draw(AViewInfo: TdxFloatingObjectAnchorSelectionLayout);
      overload; virtual;
  end;
...
constructor TdxCustomMarkVisualInfo.Create(ACustomMark: TdxCustomMark;
  const ABounds: TRect);
begin
  inherited Create;
...
procedure TdxCustomMarkExporter.ExportCustomMarkBox(ACustomMark: TdxCustomMark;
  const ABounds: TRect);
begin
  FVisualInfoCollection.Add(TdxCustomMarkVisualInfo.Create(ACustomMark,
    ABounds));
end;

function TdxCustomMarkExporter.GetCustomMarkVisualInfoCollection
  : TdxCustomMarkVisualInfoCollection;
begin
...
constructor TdxOfficeSelectionPainter.Create
  (AGraphics: TdxGraphics { const ACache: IdxGraphicsCache } );
begin
  inherited Create;
...
function TdxOfficeSelectionPainter.TryPushRotationTransform
  (const ACenter: TPoint; AAngleInDegrees: Single): Boolean;
begin
...
procedure TdxOfficeSelectionPainter.PushRotationTransform(const ACenter: TPoint;
  AAngleInDegrees: Single);
var
...
procedure TdxOfficeSelectionPainter.DrawRectangularHotZone(const ABounds: TRect;
  AColor: TdxAlphaColor);
var
  R: TRect;
begin
  R := ABounds;
  FillInHighQuality(procedure(const AGraphics: TdxGraphics)
    var
...
procedure TdxOfficeSelectionPainter.DrawEllipticHotZone(const ABounds: TRect;
AColor: TdxAlphaColor);
var
  R: TRect;
begin
...
function TdxOfficeSelectionPainter.CreateFillBrush(const ABounds: TRect;
AColor: TdxAlphaColor): TdxGPCustomBrush;
var
  ABrush: TdxLinearGradientBrush;
begin
  ABrush := TdxLinearGradientBrush.Create(ABounds, $FFFFFFFF, AColor,
    LinearGradientModeVertical);
  ABrush.SetSigmaBellShape(0.5, 1.0);
  Result := ABrush;
end;

procedure TdxOfficeSelectionPainter.DrawInHighQuality
  (const ADraw: TdxAction<TdxGraphics>);
var
...
procedure TdxOfficeSelectionPainter.FillInHighQuality
  (const AFill: TdxAction<TdxGraphics>);
var
  AOldSmoothingMode: TdxGPSmoothingMode;
...
procedure TdxRichEditSelectionPainter.Draw
  (AViewInfo: TdxRowSelectionLayoutBase);
begin
  FillRectangle(AViewInfo.Bounds);
end;

procedure TdxRichEditSelectionPainter.Draw
  (AViewInfo: TdxRectangularObjectSelectionLayout);
var
  ABounds: TRect;
  ACenter: TPoint;
...
begin
  ...
  ATransformApplied := TryPushRotationTransform(ACenter,
    AView.DocumentModel.GetBoxEffectiveRotationAngleInDegrees(AViewInfo.Box));
  try
...
procedure TdxRichEditSelectionPainter.Draw
  (AViewInfo: TdxFloatingObjectAnchorSelectionLayout);
var
  AHDC: HDC;
...
begin
  ...
  AUnitConverter := TdxRichEditView(AViewInfo.View)
    .DocumentLayout.UnitConverter;
  APixelPoint := AUnitConverter.LayoutUnitsToPixels(ALocation,
    TdxDocumentModel.DpiX, TdxDocumentModel.DpiY);
  ABounds.InitSize(AUnitConverter.PixelsToLayoutUnits(APixelPoint.X,
    TdxDocumentModel.DpiX), AUnitConverter.PixelsToLayoutUnits(APixelPoint.Y,
    TdxDocumentModel.DpiY), AUnitConverter.PixelsToLayoutUnits
    (AnchorImage.Width, TdxDocumentModel.DpiX),
    AUnitConverter.PixelsToLayoutUnits(AnchorImage.Height,
    TdxDocumentModel.DpiY));
...
procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectBottomRightHotZone);
begin
  DrawEllipticHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectBottomMiddleHotZone);
begin
  DrawRectangularHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectBottomLeftHotZone);
begin
  DrawEllipticHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectMiddleRightHotZone);
begin
  DrawRectangularHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectMiddleLeftHotZone);
begin
  DrawRectangularHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectTopRightHotZone);
begin
  DrawEllipticHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectTopMiddleHotZone);
begin
  DrawRectangularHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectTopLeftHotZone);
begin
  DrawEllipticHotZone(AHotZone.Bounds, HotZoneGradientColor);
end;

procedure TdxRichEditSelectionPainter.Visit
  (AHotZone: TdxRectangularObjectRotationHotZone);
begin
