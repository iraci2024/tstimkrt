...
{$ENDIF}
  ...
  ComCtrls, ImgList, ActiveX, Contnrs,
  dxCore, dxCoreGraphics, dxMessages, cxClasses, cxGeometry, dxGDIPlusClasses,
  dxGDIPlusApi, dxSmartImage, dxDPIAwareUtils;
...
type
  ...
  public
    class procedure CopyRect(ADest: TBitmap; const ADestRect: TRect;
      ASource: TGraphic); overload;
    class procedure CopyRect(ADest: TBitmap; const ADestRect: TRect;
      ASource: TGraphic; const ASourceRect: TRect); overload;
    class procedure Dormant(AGraphic: TGraphic);
    class function GetDefaultTransparentColor(AImage: TGraphic;
      AMask: TBitmap): TColor;
  ...
  strict private
    ...
    procedure AddToInternalCollection(AImage: TGraphic; AMask: TBitmap;
...
{$ENDIF}
    ...
    procedure DoDrawEx(AIndex: Integer; ACanvas: TCanvas; const ARect: TRect;
      AStyle: Cardinal; AStretch, ASmoothResize, AEnabled: Boolean);
    ...
    class procedure GetImageInfoCore(AImages: TCustomImageList; AIndex: Integer;
      AImage, AMask: TBitmap; APalette: IdxColorPalette;
      var AIsAlphaUsed: TdxDefaultBoolean); overload;

    // for cxImageListEditor
    function AddImageInfo(AImageInfo: TcxImageInfo): Integer;
    function CanSplitImage(AImage: TGraphic): Boolean;
    procedure ConvertTo32Bit;
    procedure InternalCopyImageInfos(AImageList: TcxCustomImageList;
      AStartIndex, AEndIndex: Integer);
    procedure InternalCopyImages(AImageList: TCustomImageList;
      AStartIndex, AEndIndex: Integer);
...
{$ENDIF}
    ...
    function AddBitmap(AImage, AMask: TBitmap; AMaskColor: TColor;
      AStretch, ASmooth: Boolean): Integer;
    function AddImage(AImages: TCustomImageList; AIndex: Integer): Integer;
    function AddImageFromResource(AInstance: THandle;
      const AResourceName: string; AResType: PChar): Integer;
    procedure AddImages(AImages: TCustomImageList);
    procedure CopyImages(AImages: TCustomImageList; AStartIndex: Integer = 0;
      AEndIndex: Integer = -1);
    procedure Clear;
    procedure Insert(AIndex: Integer; AImage: TGraphic; AMask: TBitmap);
    procedure InsertIcon(AIndex: Integer; AIcon: TIcon);
    procedure InsertMasked(AIndex: Integer; AImage: TGraphic;
      AMaskColor: TColor);
    procedure Replace(AIndex: Integer; AImage: TGraphic; AMask: TBitmap);
    procedure ReplaceIcon(AIndex: Integer; AIcon: TIcon);
    procedure ReplaceMasked(AIndex: Integer; AImage: TGraphic;
      AMaskColor: TColor);

    procedure DrawOverlay(Canvas: TCanvas; X, Y: Integer; ImageIndex: Integer;
      Overlay: TOverlay; Enabled: Boolean = True); overload;
    procedure DrawOverlay(Canvas: TCanvas; X, Y: Integer; ImageIndex: Integer;
      Overlay: TOverlay; ADrawingStyle: TDrawingStyle; AImageType: TImageType;
      Enabled: Boolean = True); overload;
    function LoadImage(AInstance: THandle; const AResourceName: string;
      AMaskColor: TColor = clDefault; AWidth: Integer = 0;
      AFlags: TLoadResources = []): Boolean;
    function FileLoad(AResType: TResType; const AName: string;
    ...
    function GetInstRes(AInstance: THandle; AResType: TResType;
      const AName: string; AWidth: Integer; ALoadFlags: TLoadResources;
      AMaskColor: TColor): Boolean; overload;
    function GetInstRes(AInstance: THandle; AResType: TResType; AResID: DWORD;
      AWidth: Integer; ALoadFlags: TLoadResources; AMaskColor: TColor)
      : Boolean; overload;
    ...
    procedure GetImageInfo(AIndex: Integer; AImage, AMask: TBitmap;
    ...
    class procedure GetImageInfo(AImages: TCustomImageList; AIndex: Integer;
      AImage, AMask: TBitmap; APalette: IdxColorPalette = nil); overload;
    class procedure GetImageInfo(AHandle: HImageList; AIndex: Integer;
      AImage, AMask: TBitmap); overload;
...
function cxGetImageListStyle(ADrawingStyle: TDrawingStyle;
  AImageType: TImageType): DWORD;
...
type
  ...
  published
    property ImageClass: string read GetImageClass write SetImageClass
      stored IsImageClassStored;
  ...
  public
    ...
    function Add(AImage: TGraphic; AMask: TBitmap; AMaskColor: TColor = clNone)
      : TCollectionItem;
    function Equals(Obj: TObject): Boolean; override;
...
begin
  cxDrawBitmap(ADestBitmap.Canvas.Handle, ASrcBitmap,
    cxGetImageClientRect(ADestBitmap), cxNullPoint);
...
function cxGetImageListStyle(ADrawingStyle: TDrawingStyle;
  AImageType: TImageType): DWORD;
const
  DrawingStyles: array [TDrawingStyle] of DWORD = (ILD_FOCUS, ILD_SELECTED,
...
begin
  Result := (Obj is TcxImageInfoItem) and
    ImageInfo.Equals(TcxImageInfoItem(Obj).ImageInfo);
end;
...
function TcxImageInfoCollection.Add(AImage: TGraphic; AMask: TBitmap;
  AMaskColor: TColor = clNone): TCollectionItem;
begin
...
begin
  ...
  Result := (Obj is TcxImageInfoCollection) and
    (Count = TcxImageInfoCollection(Obj).Count);
  if Result then
...
begin
  Result := (Obj is TcxImageInfo) and (MaskColor = TcxImageInfo(Obj).MaskColor)
    and cxCompareGraphics(Mask, TcxImageInfo(Obj).Mask) and
...
begin
  if FIsAlphaUsed = bDefault then
    FIsAlphaUsed := dxBooleanToDefaultBoolean
      (TcxImageInfoHelper.IsAlphaUsed(Image));
...
class procedure TcxImageInfoHelper.CopyRect(ADest: TBitmap;
  const ADestRect: TRect; ASource: TGraphic);
begin
...
begin
  if ASource is TBitmap then
    ...
    TdxCustomSmartImage(ASource).StretchDraw(ADest.Canvas.Handle, ADestRect,
      ASourceRect);
end;
...
class function TcxImageInfoHelper.GetDefaultTransparentColor(AImage: TGraphic;
  AMask: TBitmap): TColor;
begin
  if IsGlyphAssigned(AMask) or not(AImage is TBitmap) or IsAlphaUsed(AImage)
  then
    Result := clNone
  else
...
class function TcxImageInfoHelper.GetPixel(AGraphic: TGraphic;
  X, Y: Integer): TColor;
begin
  if AGraphic is TBitmap then
...
class function TcxImageInfoHelper.GetPixelFormat(AGraphic: TGraphic)
  : TPixelFormat;
begin
  if AGraphic is TBitmap then
...
class procedure TcxImageInfoHelper.Resize(AGraphic: TGraphic;
  const ASize: TSize);
var
  ABitmap: TcxBitmap32;
begin
  ...
  begin
    ...
    try
      ...
      if AImages is TcxCustomImageList then
        InternalCopyImageInfos(TcxCustomImageList(AImages), 0,
          AImages.Count - 1)
      else
...
function TcxCustomImageList.AddMasked(AImage: TGraphic;
...
begin
  ...
  begin
    ...
    try
      ...
      try
        Result := ImageList_AddMasked(Handle, ACloneImage.Handle,
          ColorToRGB(AMaskColor));
...
function TcxCustomImageList.AddBitmap(AImage, AMask: TBitmap;
  AMaskColor: TColor; AStretch, ASmooth: Boolean): Integer;
var
...
begin
  ...
  begin
    ...
    TcxBitmap32Access(ASizedImage).Resize(Width, Height, AStretch, ASmooth,
      AMaskColor);
  end
  else
    ASizedImage := AImage;

  if (AMask <> nil) and ((AMask.Width <> Width) or (AMask.Height <> Height))
  then
  begin
...
function TcxCustomImageList.AddImage(AImages: TCustomImageList;
...
procedure TcxCustomImageList.CopyImages(AImages: TCustomImageList;
  AStartIndex, AEndIndex: Integer);
...
procedure TcxCustomImageList.Insert(AIndex: Integer; AImage: TGraphic;
  AMask: TBitmap);
var
  I, ACurIndex: Integer;
begin
...
procedure TcxCustomImageList.InsertMasked(AIndex: Integer; AImage: TGraphic;
  AMaskColor: TColor);
var
  I, ACurIndex: Integer;
begin
...
procedure TcxCustomImageList.Replace(AIndex: Integer; AImage: TGraphic;
  AMask: TBitmap);
begin
  BeginUpdate;
  try
...
procedure TcxCustomImageList.ReplaceMasked(AIndex: Integer; AImage: TGraphic;
  AMaskColor: TColor);
begin
  BeginUpdate;
  try
...
begin
  ...
  begin
    ...
    inherited DoDraw(ImageIndex, Canvas, X, Y,
      cxGetImageListStyle(ADrawingStyle, AImageType) or ILD_OVERLAYMASK and
      Index, Enabled);
  end;
end;

function TcxCustomImageList.LoadImage(AInstance: THandle;
  const AResourceName: string; AMaskColor: TColor = clDefault;
  AWidth: Integer = 0; AFlags: TLoadResources = []): Boolean;
const
  FlagMap: array [TLoadResource] of DWORD = (LR_DEFAULTCOLOR, LR_DEFAULTSIZE,
    LR_LOADFROMFILE, LR_LOADMAP3DCOLORS, LR_LOADTRANSPARENT, LR_MONOCHROME);
var
  I: TLoadResource;
  ALoadFlags: DWORD;
  AHandle: HImageList;
...
begin
  ...
  AHandle := ImageList_LoadImage(AInstance, PChar(AResourceName), AWidth,
    AllocBy, ARGBColor, IMAGE_BITMAP, ALoadFlags);
  Result := AHandle <> 0;
  if Result then
  begin
...
function TcxCustomImageList.FileLoad(AResType: TResType; const AName: string;
  AMaskColor: TColor): Boolean;
begin
  BeginUpdate;
  try
...
begin
  BeginUpdate;
  try
    Result := inherited GetResource(AResType, AName, AWidth, ALoadFlags,
      AMaskColor);
    SynchronizeImageInfo;
  finally
    EndUpdate;
...
function TcxCustomImageList.GetInstRes(AInstance: THandle; AResType: TResType;
  const AName: string; AWidth: Integer; ALoadFlags: TLoadResources;
  AMaskColor: TColor): Boolean;
begin
  BeginUpdate;
  try
    Result := inherited GetInstRes(AInstance, AResType, AName, AWidth,
      ALoadFlags, AMaskColor);
    SynchronizeImageInfo;
  finally
    EndUpdate;
  end;
...
function TcxCustomImageList.GetInstRes(AInstance: THandle; AResType: TResType;
  AResID: DWORD; AWidth: Integer; ALoadFlags: TLoadResources;
  AMaskColor: TColor): Boolean;
begin
  BeginUpdate;
  try
    Result := inherited GetInstRes(AInstance, AResType, AResID, AWidth,
      ALoadFlags, AMaskColor);
    SynchronizeImageInfo;
  finally
    EndUpdate;
...
function TcxCustomImageList.ResourceLoad(AResType: TResType;
  const AName: string; AMaskColor: TColor): Boolean;
begin
  BeginUpdate;
  try
...
procedure TcxCustomImageList.Draw(ACanvas: TCanvas; const ARect: TRect;
  AIndex: Integer; AStretch: Boolean = True; ASmoothResize: Boolean = False;
  AEnabled: Boolean = True);
begin
  DoDrawEx(AIndex, ACanvas, ARect, cxGetImageListStyle(DrawingStyle, ImageType),
    AStretch, ASmoothResize, AEnabled);
end;

procedure TcxCustomImageList.GetImageInfo(AIndex: Integer;
  AImage, AMask: TBitmap; APalette: IdxColorPalette = nil);
var
...
class procedure TcxCustomImageList.GetImageInfo(AHandle: HImageList;
  AIndex: Integer; AImage, AMask: TBitmap);

  procedure GetBitmap(ASrcHandle: HBITMAP; ADestBitmap: TBitmap;
    ACopyAll, ASmoothStretch: Boolean; const ARect: TRect);
  var
    ASrcBitmap: TBitmap;
  begin
    if ACopyAll then
    ...
    begin
      ...
      try
        ...
        cxStretchGraphic(ADestBitmap, ASrcBitmap,
          cxGetImageClientRect(ADestBitmap), ARect, ASmoothStretch);
...
begin
  ...
  if ImageList_GetImageInfo(AHandle, AIndex, AImageInfo) then
    if AImage <> nil then
      GetBitmap(AImageInfo.hbmImage, AImage, ACopyAll, AImageInfo.hbmMask = 0,
        AImageInfo.rcImage);
  if AMask <> nil then
  begin
...
class function TcxCustomImageList.IsEquals(AImages1,
  AImages2: TCustomImageList): Boolean;
var
...
begin
  if not ChangeLocked and ([csLoading, csReading, csDestroying] *
    ComponentState = []) then
  begin
...
procedure TcxCustomImageList.DoDraw(Index: Integer; Canvas: TCanvas;
  X, Y: Integer; Style: Cardinal; Enabled: Boolean = True);
begin
  DoDrawEx(Index, Canvas, cxRectBounds(X, Y, Width, Height), Style, False,
    False, Enabled);
end;

procedure TcxCustomImageList.DoDrawEx(AIndex: Integer; ACanvas: TCanvas;
  const ARect: TRect; AStyle: Cardinal; AStretch, ASmoothResize,
  AEnabled: Boolean);
var
  AGlyphRect: TRect;
  ADrawBitmap: TBitmap;
begin
  ...
  if AlphaBlending then
    cxDrawImage(ACanvas.Handle, AGlyphRect, ARect, nil, Self, AIndex,
  ...
  begin
    ...
    try
      ...
      cxDrawImage(ACanvas.Handle, AGlyphRect, ARect, ADrawBitmap, nil, 0,
        EnabledImageDrawModeMap[AEnabled], ASmoothResize);
    finally
      ADrawBitmap.Free;
    end;
    ...
    inherited DoDraw(AIndex, ACanvas, AGlyphRect.Left, AGlyphRect.Top, AStyle,
      AEnabled);
end;
...
procedure TcxCustomImageList.DefineProperties(Filer: TFiler);
  ...
  begin
    Result := (Filer.Ancestor = nil) or not(Filer.Ancestor is TCustomImageList)
      or (TCustomImageList(Filer.Ancestor).DesignInfo <> DesignInfo);
  end;

  function NeedWriteRasterImageList: Boolean;
  begin
    Result := TcxImageInfoCollection(FImages).GetVectorImageCount >
      MulDiv(Count, 1, 3);
  end;

begin
  Filer.DefineProperty('FormatVersion', ReadFormatVersion,
    WriteFormatVersion, True);
  ...
  else if NeedWriteRasterImageList then
    inherited;

  Filer.DefineProperty('DesignInfo', ReadDesignInfo, WriteDesignInfo,
    NeedWriteDesignInfo);
  Filer.DefineProperty('ImageInfo', ReadImageInfo, WriteImageInfo,
    NeedWriteImageInfo);
end;
...
procedure TcxCustomImageList.GetImageInfoCore(AIndex: Integer;
  AImage, AMask: TBitmap; APalette: IdxColorPalette;
  var AIsAlphaUsed: TdxDefaultBoolean);

  procedure GetBitmap(ADestBitmap: TBitmap; ASourceImage: TGraphic;
    ASmoothStretch: Boolean);
  begin
...
begin
  ...
  else if InRange(AIndex, 0, Count - 1) then
  begin
    AInfo := GetImageInfo(AIndex);
    if (TcxImageInfoHelper.GetPixelFormat(AInfo.Image) = pf32bit) or IsWin9X
    then
    begin
      if AImage <> nil then
      begin
...
class procedure TcxCustomImageList.GetImageInfoCore(AImages: TCustomImageList;
  AIndex: Integer; AImage, AMask: TBitmap; APalette: IdxColorPalette;
  var AIsAlphaUsed: TdxDefaultBoolean);
begin
  if AImages is TcxCustomImageList then
    TcxCustomImageList(AImages).GetImageInfoCore(AIndex, AImage, AMask,
      APalette, AIsAlphaUsed)
  else
...
begin
  Result := ((AImage.Width <> Width) or (AImage.Height <> Height)) and
    (AImage.Width mod Width + AImage.Height mod Height = 0) and
    not Supports(AImage, IdxVectorImage);
end;
...
procedure TcxCustomImageList.InternalCopyImageInfos
  (AImageList: TcxCustomImageList; AStartIndex, AEndIndex: Integer);
var
  I: Integer;
begin
...
procedure TcxCustomImageList.InternalCopyImages(AImageList: TCustomImageList;
  AStartIndex, AEndIndex: Integer);
var
  I: Integer;
  AImage, AMask: TBitmap;
...
procedure TcxCustomImageList.AddToInternalCollection(AImage: TGraphic;
  AMask: TBitmap; AMaskColor: TColor);

  procedure InternalAddToInternalCollection(AImage: TGraphic; AMask: TBitmap;
    AMaskColor: TColor);
  begin
    DormantImage(TcxImageInfoCollection(FImages).Add(AImage, AMask,
      AMaskColor).Index);
  end;
...
begin
  ...
  begin
    ...
    ADestBitmap := TcxBitmap.CreateSize(Width, Height,
      TcxImageInfoHelper.GetPixelFormat(AImage));
    ...
    try
      for ARowIndex := 0 to ARowCount - 1 do
        ...
        begin
          ...
          ASourceRect := Rect(AColIndex * Width, ARowIndex * Height,
            (AColIndex + 1) * Width, (ARowIndex + 1) * Height);
          TcxImageInfoHelper.CopyRect(ADestBitmap, ADestBitmap.ClientRect,
            AImage, ASourceRect);
          if IsGlyphAssigned(AMask) then
            ADestMask.Canvas.CopyRect(ADestMask.ClientRect, AMask.Canvas,
              ASourceRect);
...
class procedure TcxCustomImageList.CheckImageSize(AImage: TBitmap;
...
function TcxCustomImageList.GetImageCount(AImage: TGraphic;
