...
uses
  Classes, SysUtils, cxConverterFactory, cxGridConverter,
  cxGridDBBandedTableView, cxGridCustomView,
  DB, cxGraphics, cxCheckBox, Graphics, cxHyperLinkEdit, cxTextEdit, cxImage,
  cxSpinEdit,
  StdCtrls, cxImageComboBox, cxButtonEdit, cxEdit, cxDBLookupComboBox, cxGrid,
  cxLookupDBGrid,
  cxDropDownEdit, cxMRUEdit, cxMemo, cxCalendar, Controls,
  cxGridCustomTableView, cxStyles,
...
type
  ...
  private
    ...
    procedure ImportColumnCheckBox(AcxColumn: TcxGridDBBandedColumn;
      const AColumnData: string; const AReadOnly: Boolean);
    procedure ImportColumnCustomEdit(AcxColumn: TcxGridDBBandedColumn;
      const AColumnData: string);
    procedure ImportColumnImage(AcxColumn: TcxGridDBBandedColumn;
      const AColumnData: string; const AReadOnly: Boolean);
    procedure ImportColumnRichEdit(AcxColumn: TcxGridDBBandedColumn;
      const AColumnData: string);
    procedure ImportColumnText(AcxColumn: TcxGridDBBandedColumn;
      const AReadOnly: Boolean);
    procedure ImportColumnUrlLink(AcxColumn: TcxGridDBBandedColumn;
      const AReadOnly: Boolean);
    procedure ImportBands;
    procedure ImportColumns;
    procedure ImportGrid;
    procedure ImportGridStyles;
    procedure ImportWWCheckBox(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWComboBox(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWComboDlg(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWDateTimePicker(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWExpandButton(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWLookupCombo(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWLookupComboDlg(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWMonthCalendar(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWOptions;
    procedure ImportWWRadioGroup(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWSpinEdit(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    procedure ImportWWTextEdit(AcxColumn: TcxGridDBBandedColumn;
      AComponent: TComponent);
    function IsColumnDefault(const AFieldName: string): string;
    function Pass(const AData: string; AStartIndex: Integer;
...
begin
  cxGridView.DataController.DataSource := GetClassProperty(nil, 'DataSource')
    as TDataSource;
  ImportBands;
  ImportColumns;
  ImportGrid;
...
function TcxWWGridConverter.ExtractData(const AData: string;
  var AResultData: string; AStartIndex: Integer; ASeparator: Char): Integer;
begin
  Result := AStartIndex;
  AResultData := '';
...
procedure TcxWWGridConverter.ImportColumnCheckBox
  (AcxColumn: TcxGridDBBandedColumn; const AColumnData: string;
  const AReadOnly: Boolean);
var
...
procedure TcxWWGridConverter.ImportColumnCustomEdit
  (AcxColumn: TcxGridDBBandedColumn; const AColumnData: string);
var
...
procedure TcxWWGridConverter.ImportColumnRichEdit
  (AcxColumn: TcxGridDBBandedColumn; const AColumnData: string);
var
  AComponentName: string;
  AComponent: TComponent;
...
begin
  ...
  begin
    ...
    begin
      ...
      HideSelection := GetBooleanProperty(AComponent, 'HideSelection',
        HideSelection);
      MaxLength := GetIntegerProperty(AComponent, 'MaxLength', MaxLength);
      ScrollBars := ConvertScrollStyle(GetEnumProperty(AComponent,
        'ScrollBars'));
...
procedure TcxWWGridConverter.ImportColumnUrlLink
  (AcxColumn: TcxGridDBBandedColumn; const AReadOnly: Boolean);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      AcxColumn.Name := UniqueColumnName(AcxColumn,
        AcxColumn.DataBinding.FieldName);
      AcxColumn.Width := StrToInt(AWidth) * AFontWidth;
      AcxColumn.Caption := AHeader;
      AcxColumn.HeaderAlignmentHorz :=
        ConvertAlignment(GetEnumProperty(nil, 'TitleAlignment'));
...
procedure TcxWWGridConverter.ImportWWCheckBox(AcxColumn: TcxGridDBBandedColumn;
  AComponent: TComponent);
...
begin
  ...
  begin
    ...
    DisplayChecked := GetStringProperty(AComponent, 'DisplayValueChecked',
      DisplayChecked);
    DisplayUnchecked := GetStringProperty(AComponent, 'DisplayValueUnchecked',
      DisplayUnchecked);
    FullFocusRect := GetBooleanProperty(AComponent, 'ShowFocusRect',
      FullFocusRect);
    ...
    ValueUnchecked := GetStringProperty(AComponent, 'ValueUnchecked',
      ValueUnchecked);
  end;
end;

procedure TcxWWGridConverter.ImportWWComboBox(AcxColumn: TcxGridDBBandedColumn;
  AComponent: TComponent);
var
  ...
  begin
    ...
    begin
      ...
      if GetEnumProperty(AComponent, 'ButtonStyle') = 'cbsCustom' then
        ButtonGlyph.Assign(GetClassProperty(AComponent, 'ButtonGlyph')
          as TBitmap);
      ImmediateDropDown := GetBooleanProperty(AComponent, 'AutoDropDown',
        ImmediateDropDown);
      DropDownRows := GetIntegerProperty(AComponent, 'DropDownCount',
        DropDownRows);
    end;
  end;
...
procedure TcxWWGridConverter.ImportWWComboDlg(AcxColumn: TcxGridDBBandedColumn;
  AComponent: TComponent);
var
...
begin
  ...
  begin
    Buttons.Clear;
    AButton := Buttons.Add;
    AButton.Glyph.Assign(GetClassProperty(AComponent, 'ButtonGlyph')
      as TBitmap);
    ...
    if AButtonStyle = 'cbsCustom' then
      ...
      AButton.Kind := bkDown;
    AButton.Width := GetIntegerProperty(AComponent, 'ButtonWidth',
      AButton.Width);
    CharCase := GetCharCase(GetEnumProperty(AComponent, 'CharCase'));
    MaxLength := GetIntegerProperty(AComponent, 'MaxLength', MaxLength);
    ReadOnly := GetBooleanProperty(AComponent, 'ReadOnly', ReadOnly);
    AButton.Visible := GetBooleanProperty(AComponent, 'ShowButton',
      AButton.Visible);
  end;
end;

procedure TcxWWGridConverter.ImportWWDateTimePicker
  (AcxColumn: TcxGridDBBandedColumn; AComponent: TComponent);
...
begin
  ...
  begin
    ...
    if AWWCalendarAttributes <> nil then
      Alignment.Horz := GetCalendarAlignment
        (GetEnumProperty(AWWCalendarAttributes, 'Alignment'));
...
procedure TcxWWGridConverter.ImportWWExpandButton
  (AcxColumn: TcxGridDBBandedColumn; AComponent: TComponent);
begin
end;

procedure TcxWWGridConverter.ImportWWLookupCombo
  (AcxColumn: TcxGridDBBandedColumn; AComponent: TComponent);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      DropDownWidth := GetIntegerProperty(AComponent, 'DropDownWidth',
        DropDownWidth);
      ...
      if GetEnumProperty(AComponent, 'ButtonStyle') = 'cbsCustom' then
        ButtonGlyph.Assign(GetClassProperty(AComponent, 'ButtonGlyph')
          as TBitmap);
      ReadOnly := GetBooleanProperty(AComponent, 'ReadOnly', ReadOnly);
      PopupAlignment := ConvertAlignment(GetEnumProperty(AComponent,
        'DropDownAlignment'));
      ...
      try
        ...
        try
          ...
          ListOptions.CaseInsensitive := not AWWSeqSearchOptions.Find
            ('ssoCaseSensitive', I);
        except
          on EcxUnknownProperty do;
        end;
...
procedure TcxWWGridConverter.ImportWWLookupComboDlg
  (AcxColumn: TcxGridDBBandedColumn; AComponent: TComponent);
var
  AListDataSource: TDataSource;
  AListTable: TObject;
...
procedure TcxWWGridConverter.ImportWWMonthCalendar
  (AcxColumn: TcxGridDBBandedColumn; AComponent: TComponent);
var
  AWWOptions: TStringList;
  ADateButtons: TDateButtons;
...
begin
  ...
  try
    ...
    try
      ...
      with cxGridView do
      begin
        OptionsBehavior.AlwaysShowEditor :=
          AOptions.Find('dgAlwaysShowEditor', I);
        ...
        OptionsSelection.HideSelection :=
          not AOptions.Find('dgAlwaysShowSelection', I);
        ...
        OptionsBehavior.FocusCellOnCycle :=
          not AOptions.Find('dgTabExitsOnLastCol', I);
      end;
      AOptions.Clear;
...
procedure TcxWWGridConverter.ImportWWRadioGroup
  (AcxColumn: TcxGridDBBandedColumn; AComponent: TComponent);
var
  AwwItems: TStringList;
...
begin
  ...
  begin
    ...
    ShowDescriptions := GetBooleanProperty(AComponent, 'ShowText',
      ShowDescriptions);
  end;
  AcxColumn.Options.ShowEditButtons := isebAlways;
end;

procedure TcxWWGridConverter.ImportWWSpinEdit(AcxColumn: TcxGridDBBandedColumn;
  AComponent: TComponent);
begin
...
procedure TcxWWGridConverter.ImportWWTextEdit(AcxColumn: TcxGridDBBandedColumn;
  AComponent: TComponent);
begin
  AcxColumn.PropertiesClass := TcxTextEditProperties;
  with TcxTextEditProperties(AcxColumn.Properties) do
...
ConverterFactory(cxGridGroupConverterName).RegisterConverter
  ('WW InfoPower Grid Converter', TcxWWGridConverter);
