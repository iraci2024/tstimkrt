...
uses
  Windows, Classes, Graphics, ImgList, Menus, cxGridMenuOperations,
  cxGridCustomPopupMenu, cxGridCustomView,
  dxBuiltInPopupMenu;
...
type
  ...
  public
    constructor Create(AOwner: TComponent;
      AOperation: TcxGridPopupMenuOperation); reintroduce; virtual;
    //
  ...
  strict private
    ...
    procedure PopupViaAdapter(const P: TPoint;
      AAdapterClass: TdxCustomBuiltInPopupMenuAdapterClass);
  protected
    // IDoPopup
    ...
    function GetOperationsClass: TcxGridPopupMenuOperationsClass;
      virtual; abstract;
...
uses
  ...
  cxGridDataRowPopupMenuItems,
  cxGridFooterPopupMenuItems, cxGridGroupRowPopupMenuItems,
  cxGridGroupByBoxPopupMenuItems;
...
constructor TcxGridPopupMenuItem.Create(AOwner: TComponent;
...
begin
  cxAdvancedDrawPopupMenuItem(TcxGridPopupMenuItem(Sender), ACanvas,
    ARect, State);
end;
...
begin
  ...
  begin
    ...
    if AOperation.GetParentOperationClass = nil then
      ...
      AParentMenuItem := GetMenuItemByOperationClass(Items,
        AOperation.GetParentOperationClass);
...
begin
  if TdxBuiltInPopupMenuAdapterManager.IsActualAdapterStandard then
    ...
    PopupViaAdapter(Point(X, Y),
      TdxBuiltInPopupMenuAdapterManager.GetActualAdapterClass);
end;
...
procedure TcxGridStdPopupMenu.PopupViaAdapter(const P: TPoint;
  AAdapterClass: TdxCustomBuiltInPopupMenuAdapterClass);
var
...
begin
  ...
  try
    DoPopup(Self);
    TdxBuiltInPopupMenuAdapterHelper.AddMenu(AAdapter, Self,
      AdapterClickHandler);
    AAdapter.Popup(P);
  finally
    AAdapter.Free;
...
function TcxGridStdHeaderMenu.GetOperationsClass
  : TcxGridPopupMenuOperationsClass;
begin
...
function TcxGridStdGroupByBoxMenu.GetOperationsClass
  : TcxGridPopupMenuOperationsClass;
begin
...
function TcxGridStdFooterMenu.GetOperationsClass
  : TcxGridPopupMenuOperationsClass;
begin
...
function TcxGridStdGroupRowMenu.GetOperationsClass
  : TcxGridPopupMenuOperationsClass;
begin
...
function TcxGridStdDataRowMenu.GetOperationsClass
  : TcxGridPopupMenuOperationsClass;
begin
...
RegisterPopupMenuClass(TcxGridStdHeaderMenu, [gvhtColumnHeader],
  TcxGridTableView);
RegisterPopupMenuClass(TcxGridStdGroupByBoxMenu, [gvhtGroupByBox],
...
RegisterPopupMenuClass(TcxGridStdGroupRowMenu, [gvhtRecord, gvhtGroupSummary],
  TcxGridTableView);
RegisterPopupMenuClass(TcxGridStdDataRowMenu, [gvhtRecord, gvhtCell],
  TcxGridTableView);
