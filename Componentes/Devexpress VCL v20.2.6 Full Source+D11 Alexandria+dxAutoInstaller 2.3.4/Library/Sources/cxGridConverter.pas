...
uses
  ...
  dxCore, cxConverterFactory, cxGrid, cxGridCustomView, cxGridLevel,
  cxGridCustomTableView,
  cxGridDBBandedTableView, cxStyles, cxClasses, cxGridStrs,
...
type
  ...
  protected
    ...
    function UniqueColumnName(AColumn: TComponent;
  ...
  public
    ...
    property DeleteAllSublevels: Boolean read FDeleteAllSublevels
      write FDeleteAllSublevels;
    property Destination: TcxCustomGrid read GetDestination;
    property DestinationLevel: TcxGridLevel read FDestinationLevel
      write FDestinationLevel;
...
begin
  ...
  if not Result then
    MessageBox(0,
      PChar(Format(cxGetResourceString(@scxGridConverterIntermediaryMissing),
      [GetIntermediaryClassName])),
      PChar(cxGetResourceString(@scxImportErrorCaption)),
      MB_OK or MB_ICONINFORMATION);
end;

procedure TcxCustomGridConverter.DeleteComponent(AComponent: TComponent);
begin
  if Assigned(AComponent) and CanDeleteComponent(AComponent.Owner, AComponent)
  then
    AComponent.Free;
end;
...
function TcxCustomGridConverter.UniqueColumnName(AColumn: TComponent;
  const AFieldName: string): string;
begin
  // Result := 'Column' + IntToStr(NCount);
  // Inc(NCount);
  Result := CreateUniqueName(AColumn.Owner, cxGridView, AColumn,
...
begin
  ...
  FDestinationLevel.GridView.Name := GenViewName(Destination,
    FDestinationLevel.GridView);
