...
uses
  ...
  dxCoreClasses, cxGeometry, cxLookAndFeels, cxGraphics, cxControls, cxClasses,
  cxScrollBar,
...
type
  ...
  TdxWinFormsScrollBarAdapter = class(TInterfacedObject,
    IdxPlatformSpecificScrollBarAdapter)
  public
    procedure OnScroll(AAdapter: TdxScrollBarAdapter; ASender: TObject;
      E: TdxScrollEventArgs); virtual;
    procedure ApplyValuesToScrollBarCore
      (AAdapter: TdxScrollBarAdapter); virtual;
  ...
  TdxWinFormsRichEditViewVerticalScrollController = class
  ...
  TdxWinFormsRichEditViewHorizontalScrollController = class
  ...
  strict protected
    ...
    procedure ExecuteScroll(Sender: TObject; ScrollCode: TScrollCode;
...
function TdxOfficeScrollBarHelper.GetControllerClass
...
procedure TdxWinFormsScrollBarAdapter.OnScroll(AAdapter: TdxScrollBarAdapter;
  ASender: TObject; E: TdxScrollEventArgs);
var
  ADelta: Integer;
...
begin
  ...
  begin
    AArgs := TdxScrollEventArgs.Create(E.&Type, AAdapter.GetRawScrollBarValue,
      AAdapter.GetRawScrollBarValue + ADelta, E.ScrollOrientation);
    try
...
procedure TdxWinFormsScrollBarAdapter.ApplyValuesToScrollBarCore
  (AAdapter: TdxScrollBarAdapter);
begin
  ...
  try
    ...
    AAdapter.ScrollBar.LargeChange :=
      Round(AAdapter.Factor * AAdapter.LargeChange);
    AAdapter.ScrollBar.SmallChange := Min(AAdapter.ScrollBar.LargeChange,
      Round(AAdapter.Factor * AAdapter.SmallChange));
...
function TdxWinFormsScrollBarAdapter.GetRawScrollBarValue
  (AAdapter: TdxScrollBarAdapter): Integer;
begin
  Result := AAdapter.ScrollBar.Value;
end;

function TdxWinFormsScrollBarAdapter.SetRawScrollBarValue
  (AAdapter: TdxScrollBarAdapter; Value: Integer): Boolean;
begin
...
begin
  Result := TdxScrollEventArgs.Create(TdxScrollEventType.Last,
    AAdapter.ScrollBar.Maximum - AAdapter.ScrollBar.LargeChange);
end;

function TdxWinFormsScrollBarAdapter.GetPageUpRawScrollBarValue
  (AAdapter: TdxScrollBarAdapter): Integer;
begin
  Result := Max(AAdapter.ScrollBar.Minimum, AAdapter.ScrollBar.Value -
    AAdapter.ScrollBar.LargeChange);
end;

function TdxWinFormsScrollBarAdapter.GetPageDownRawScrollBarValue
  (AAdapter: TdxScrollBarAdapter): Integer;
begin
  Result := Min(AAdapter.ScrollBar.Maximum - AAdapter.ScrollBar.LargeChange + 1,
    AAdapter.ScrollBar.Value + AAdapter.ScrollBar.LargeChange);
end;
...
procedure TdxWinFormsRichEditViewVerticalScrollController.
  ApplyNewScrollValueToScrollEventArgs(E: TdxScrollEventArgs; Value: Integer);
begin
  E.NewValue := Value;
end;
...
function TdxWinFormsRichEditViewVerticalScrollController.
  UpdatePageNumberOnScroll(E: TdxScrollEventArgs): Boolean;
begin
  Result := False;
end;
...
begin
  if FLockCount = 1 then
  begin
    if (Position <> FData.Value) or (LargeChange <> FData.LargeChange) or
      (SmallChange <> FData.SmallChange) or (Max <> FData.Maximum) or
      (Min <> FData.Minimum) or (Enabled <> FData.Enabled) then
    begin
      ...
      SetScrollParams(FData.Minimum, FData.Maximum, FData.Value,
        Math.Min(FData.LargeChange, FData.Maximum - FData.Minimum)
        { PageSize } , False);
...
function ScrollCodeToScrollEventType(AScrollCode: TScrollCode)
  : TdxScrollEventType;
const
  Map: array [TScrollCode] of TdxScrollEventType =
    (TdxScrollEventType.SmallDecrement, TdxScrollEventType.SmallIncrement,
    TdxScrollEventType.LargeDecrement, TdxScrollEventType.LargeIncrement,
    TdxScrollEventType.ThumbPosition, TdxScrollEventType.ThumbTrack,
    TdxScrollEventType.First, TdxScrollEventType.Last,
    TdxScrollEventType.EndScroll);
begin
  Result := Map[AScrollCode];
end;

function ScrollBarKindToScrollOrientation(AKind: TScrollBarKind)
  : TdxScrollOrientation;
const
  Map: array [TScrollBarKind] of TdxScrollOrientation =
    (TdxScrollOrientation.HorizontalScroll,
    TdxScrollOrientation.VerticalScroll);
begin
  Result := Map[AKind];
end;

procedure TdxOfficeScrollbar.ExecuteScroll(Sender: TObject;
...
begin
  ...
  begin
    E := TdxScrollEventArgs.Create(ScrollCodeToScrollEventType(ScrollCode),
      ScrollPos, ScrollBarKindToScrollOrientation(Kind));
    try
