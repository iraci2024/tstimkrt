...
type
  ...
  protected
    ...
    procedure CheckMaxRowHeaderWidth(var Value: Integer;
      AValueMinWidth: Integer); override;
  ...
  protected
    ...
    function CanAddRowToBand(const ARowRect, ABandRect: TRect;
      ABandRowIndex: Integer): Boolean; override;
    procedure CheckMaxRowHeaderWidth(var Value: Integer;
      AValueMinWidth: Integer); override;
  ...
  protected
    ...
    procedure AddRightValueSide(ARowViewInfo: TcxCustomRowViewInfo;
      const R: TRect; ALast: Boolean); override;
    procedure CalcBandWidth; override;
    ...
    function GetLTRValueRect(AValueIndex: Integer;
      ARowViewInfo: TcxCustomRowViewInfo): TRect; override;
    function GetVisibleValueCount: Integer; override;
...
begin
  if Row.HasVisibleChildren then
    ...
    begin
      FButtonRect := cxRectCenter
        (cxRect(HeaderRect.Right - ViewInfo.ExplorerButtonSize.cx - 2,
        HeaderRect.Top, HeaderRect.Right, HeaderRect.Bottom),
        ViewInfo.ExplorerButtonSize.cx, ViewInfo.ExplorerButtonSize.cy);
      FButtonAreaBounds :=
        cxRectCenter(cxRect(HeaderRect.Right -
        ViewInfo.ExplorerButtonAreaSize.cx - 2, HeaderRect.Top,
        HeaderRect.Right, HeaderRect.Bottom),
        ViewInfo.ExplorerButtonAreaSize.cx, ViewInfo.ExplorerButtonAreaSize.cy);
    end
    else
...
begin
  ...
  begin
    ...
    if NeedLTRBounds then
      ALRTClipRect := TdxRightToLeftLayoutConverter.ConvertRect(ALRTClipRect,
        RightToLeftConversionClientBounds);
    AddRectValueLines(R, (I = ACount - 1) or (R.Right >= ALRTClipRect.Right),
      ABottomLineNeeded);
...
begin
  TcxMultiEditorRowViewInfo(Row.ViewInfo).AddSeparatorRects(True,
    SeparatorRects, SeparatorInfo.Kind, AHeaderCells, HeaderCellsRect);
end;

procedure TcxMultiEditorRowHeaderInfo.CalcSeparatorWidth(ASeparatorWidth
  : Integer);
begin
...
begin
  ...
  with Row.Properties do
    FSeparatorInfo.TextFlags := GetTextAligmentFlags(taCenter,
      SeparatorAlignmentVert);
end;
...
procedure TcxMultiEditorRowHeaderInfo.DoRightToLeftConversion
...
procedure TcxMultiEditorRowViewInfo.AddSeparatorRects(AHeader: Boolean;
  ASepRects: TcxRectList; ASeparatorKind: TSeparatorKind;
    ...
    with AValueCells[I] do
      ...
      begin
        ...
        begin
          ...
          begin
            if not ViewInfo.CalcHelper.IsBottomLineNeeded
              (VerticalGrid.NextVisibleRow(Row)) then
              Dec(R.Bottom);
...
begin
  ...
  begin
    ...
    begin
      AValueRect := TcxvgCustomViewInfoAccess(ViewInfo)
        .GetLTRValueRect(I, Self);
      ...
      begin
        ...
        try
          ...
          AddSeparatorRects(False, SeparatorRects, ASepKind, ARects,
            AValueRect);
        finally
          ARects.Free;
        end;
...
procedure TcxMultiEditorRowViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
...
begin
  ...
  begin
    ...
    for I := 0 to AValuesCount - 1 do
    begin
      AValueRect := TcxvgCustomViewInfoAccess(AViewInfo)
        .GetLTRValueRect(I, Self);
      ARects := GetCellRects(Row, AValueRect, ASepWidth);
      if ARects <> nil then
      begin
...
begin
  ...
  begin
    ...
    if Right < ClientRect.Right then
      ...
      EmptyRects.Add(cxRectBounds(Right, Top, ClientRect.Right - Right,
        AHeight));
  end;
end;
...
begin
  ...
  begin
    ...
    ViewBandWidth := GetViewBandWidth;
    if ShowHeaders and (ASaveWidth <> ViewBandWidth) and (ASaveWidth <> 0) and
      not LockDividerPos then
    begin
...
begin
  if not AutoScaleBands then
    Result := ViewHeaderWidth + DividerWidth + ViewValueWidth +
      VertLineWidth * 2
  else
  begin
...
begin
  ...
  begin
    ALeft := Left + ViewHeaderWidth + DividerWidth + AValueIndex *
      (ViewValueWidth + cxSetValue(RecordsInterval = 0, VertLineWidth,
      RecordsInterval + 2 * VertLineWidth));
    Result := cxRectBounds(ALeft + Scroller.PixelScrollRecordOffset, Top,
      ViewValueWidth, Bottom - Top);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        LinesInfo.Add(Left, Bottom - HorzLineWidth, Right - Left, HorzLineWidth,
          BandBorderColor);
      end;
    end;
...
procedure TcxMultiRecordViewInfo.AddRightValueSide(ARowViewInfo
...
function TcxMultiRecordViewInfo.GetBandSizeableRect(const ABandRect
  : TRect): TRect;
var
  ALeft: Integer;
begin
  if VerticalGrid.LayoutStyle <> lsMultiRecordView then
    ...
    begin
      ...
      begin
        ALeft := ABandRect.Left + ViewHeaderWidth + DividerWidth +
          ViewValueWidth + VertLineWidth;
        Result := cxRect(ALeft - 1, Top, ALeft + 2, Bottom);
      end
      else
      begin
        ALeft := ABandRect.Right - ViewHeaderWidth - DividerWidth -
          ViewValueWidth - VertLineWidth;
...
begin
  Result := 0;
  if cxRectIsEmpty(ClientRect) then
    Exit;
  CalcBandWidth;
