...
{$ENDIF}
  ...
  Menus, cxListBox, StdCtrls, cxButtons, cxLabel, dxLayoutControl, cxTextEdit,
  cxMaskEdit, cxSpinEdit, dxMeasurementUnitEdit,
  dxRichEdit.Dialogs.CustomDialog, dxRichEdit.Dialogs.CustomNumberingListForm,
  dxRichEdit.Dialogs.NumberingHelper,
  dxRichEdit.Dialogs.NumberingFormController, dxRichEdit.Utils.Properties,
  dxSymbolListBox, dxLayoutLookAndFeels,
  cxClasses;

type
  TdxBulletCharacterControlHelper = class;

  TdxRichEditBulletedListDialogForm = class(TdxRichEditCustomNumberingListForm,
  ...
  protected
    ...
    procedure ApplySymbolProperties(const ASymbolProperties
    ...
    property ActiveSymbolProperties: TdxSymbolProperties
      read GetActiveSymbolProperties write SetActiveSymbolProperties;
  public
  ...
  protected
    procedure CreateListBoxes(const ASimpleSymbolListBox
      : array of TdxSimpleSymbolListBox);
    procedure DoSelectedIndexChanged;
    function GetNextActiveListBoxIndex(AKeyDown: Word;
      AActiveListBoxIndex: Integer): Integer;
    procedure InitializeComponents(const ASymbolsProperties
      : array of TdxSymbolProperties);
    procedure SimpleSymbolListBoxKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure SimpleSymbolListBoxMouseDown(Sender: TObject;
  ...
  public
    ...
    property ActiveSymbolProperties: TdxSymbolProperties
      read GetActiveSymbolProperties write SetActiveSymbolProperties;
    property OnSelectedIndexChanged: TNotifyEvent read FOnSelectedIndexChanged
      write FOnSelectedIndexChanged;
    property OnMouseDoubleClick: TNotifyEvent read FOnMouseDoubleClick
      write SetOnMouseDoubleClick;
  end;
...
procedure TdxRichEditBulletedListDialogForm.FormKeyDown(Sender: TObject;
...
procedure TdxRichEditBulletedListDialogForm.
  BulletCharacterControlSelectedIndexChanged(Sender: TObject);
var
  ASymbolProperties: TdxSymbolProperties;
begin
...
function TdxRichEditBulletedListDialogForm.GetActiveSymbolProperties
  : TdxSymbolProperties;
begin
...
procedure TdxRichEditBulletedListDialogForm.SetActiveSymbolProperties
  (const Value: TdxSymbolProperties);
begin
  FBulletCharacterControlHelper.ActiveSymbolProperties := Value;
end;

procedure TdxRichEditBulletedListDialogForm.SetSelectedIndex
  (const Value: Integer);
begin
...
begin
  ...
  lblBulletCharacter.Caption := cxGetResourceString
    (@sdxRichEditBulletedListDialogBulletCharacter);
  btnCharacter.Caption := cxGetResourceString
    (@sdxRichEditBulletedListDialogButtonCharacter);
  lblBulletPosition.Caption := cxGetResourceString
  ...
  lciAlignedAt.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditBulletedListDialogAlignedAt);
  lciIndentAt.CaptionOptions.Text :=
    cxGetResourceString(@sdxRichEditBulletedListDialogIndentAt);
end;

procedure TdxRichEditBulletedListDialogForm.ApplySymbolProperties
  (const ASymbolProperties: TdxSymbolProperties; AData: TObject);
var
  AIndex: Integer;
begin
...
begin
  FBulletCharacterControlHelper.CreateListBoxes
    ([lbSimpleSymbol1, lbSimpleSymbol2, lbSimpleSymbol3, lbSimpleSymbol4,
...
begin
  ...
  FBulletCharacterControlHelper.OnSelectedIndexChanged :=
    BulletCharacterControlSelectedIndexChanged;
...
function TdxBulletCharacterControlHelper.GetActiveSymbolListBox
...
function TdxBulletCharacterControlHelper.GetActiveSymbolProperties
  : TdxSymbolProperties;
var
  AActiveListBox: TdxSimpleSymbolListBox;
begin
  AActiveListBox := GetActiveSymbolListBox();
  Result := TdxSymbolProperties.Create(AActiveListBox.UnicodeChars[0],
    AActiveListBox.FontName);
end;

function TdxBulletCharacterControlHelper.GetFocusedSymbolListBox
  : TdxSimpleSymbolListBox;
begin
  for Result in FSymbolListBoxes do
...
function TdxBulletCharacterControlHelper.GetNextActiveListBoxIndex
  (AKeyDown: Word; AActiveListBoxIndex: Integer): Integer;
begin
  ...
  else if (AKeyDown = VK_RIGHT) or (AKeyDown = VK_DOWN) then
...
constructor TdxBulletCharacterControlHelper.Create
  (AOwner: TdxRichEditBulletedListDialogForm);
begin
  inherited Create;
  FOwner := AOwner;
...
procedure TdxBulletCharacterControlHelper.SetSelectedIndex
  (const Value: Integer);
begin
...
procedure TdxBulletCharacterControlHelper.SimpleSymbolListBoxKeyDown
