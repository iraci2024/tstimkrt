...
{$REGION 'protected type'}
    ...
    TdxSetDelegate = { reference to } procedure(AValue: TdxIntegerList)
      of object;
    ...
    public
      ...
      property ChangeNotificationIds: TdxIntegerList
        read GetChangeNotificationIds write SetChangeNotificationIds;
    end;
{$ENDREGION}
  strict private
    FCanExpand: Boolean;
  ...
  protected
    ...
    function RunInserted(APosition: PdxDocumentModelPosition;
      ANewRunIndex: TdxRunIndex; ALength: Integer;
      AAccessor: TdxHistoryNotificationsAccessor;
      AHistoryNotificationId: Integer): Boolean; overload;
    procedure RunRemoved(AParagraphIndex: TdxParagraphIndex;
      ARunIndex: TdxRunIndex; ALength: Integer;
      AHistoryNotificationId: Integer); overload; override;
    function RunRemoved(APos: PdxDocumentModelPosition; ARunIndex: TdxRunIndex;
      ALength: Integer; AAccessor: TdxHistoryNotificationsAccessor;
      AHistoryNotificationId: Integer): Boolean; overload;
  public
  ...
  strict private
    function GetItem(Index: Integer): TdxBookmarkBase; inline;
  public
  ...
  public
    ...
    function GetEntireBookmarksCore(AStart: TdxDocumentLogPosition;
  ...
  public
    ...
    procedure Export(const AExporter: IdxDocumentModelExporter);
      virtual; abstract;
  ...
  TdxVisitableDocumentIntervalBoundaryCollection = class
    (TdxObjectList<TdxVisitableDocumentIntervalBoundary>)
  protected
    procedure QuickSort(const AComparer
      : IComparer<TdxVisitableDocumentIntervalBoundary>; L, R: Integer);
  public
    function GetRange(AIndex, ALength: Integer)
      : TdxList<TdxVisitableDocumentIntervalBoundary>;
    procedure Sort(AIndex, ACount: Integer;
      const AComparer
      : IComparer<TdxVisitableDocumentIntervalBoundary>); overload;
  end;

  { TdxVisitableDocumentIntervalStartBoundaryFactory }

  TdxVisitableDocumentIntervalStartBoundaryFactory = class(TcxIUnknownObject,
  ...
  TdxVisitableDocumentIntervalEndBoundaryFactory = class(TcxIUnknownObject,
    IdxDocumentIntervalVisitor)
  strict private
    FBoundary: TdxVisitableDocumentIntervalBoundary;
  protected
  ...
  type
    ...
    public
      // IComparer
      function Compare(const Left, Right: TdxVisitableDocumentIntervalBoundary)
        : Integer; override;
    end;
  protected
  ...
  private
    ...
    FVisitableDocumentIntervalBoundaries
      : TdxVisitableDocumentIntervalBoundaryCollection;
  ...
  protected
    ...
    procedure PopulateBoundariesCore(AIntervals: TObjectList);
      overload; virtual;
  public
    constructor Create(APieceTable: TdxSimplePieceTable;
      AIncludeHiddenIntervals: Boolean = True);
    destructor Destroy; override;

    function IsDone: Boolean;
    procedure MoveNext;
    procedure Reset;
    class function CreateVisitableDocumentIntervalStartBoundaryFactory
      : TdxVisitableDocumentIntervalStartBoundaryFactory; static;
    class function CreateVisitableDocumentIntervalEndBoundaryFactory
      : TdxVisitableDocumentIntervalEndBoundaryFactory; static;
    function IsVisibleInterval(AInterval: TdxVisitableDocumentInterval)
      : Boolean; virtual;

    property Boundaries: TdxVisitableDocumentIntervalBoundaryCollection
      read FVisitableDocumentIntervalBoundaries;
    property Current: TdxVisitableDocumentIntervalBoundary
      read InternalGetCurrent;
    property PieceTable: TdxSimplePieceTable read FPieceTable;
  end;

  { TdxVisitableDocumentIntervalBasedObjectBoundaryIterator }

  TdxVisitableDocumentIntervalBasedObjectBoundaryIterator = class
    (TdxVisitableDocumentIntervalBoundaryIterator)
  protected
  ...
  type
    TBoundaryComparer = class(TdxComparer<TdxVisitableDocumentIntervalBoundary>)
      function Compare(const ALeft,
        ARight: TdxVisitableDocumentIntervalBoundary): Integer; override;
    end;

    TBoundaryOrderComparer = class
      (TdxComparer<TdxVisitableDocumentIntervalBoundary>)
      function Compare(const ALeft,
        ARight: TdxVisitableDocumentIntervalBoundary): Integer; override;
    end;
  ...
  protected
    ...
    function OnRunInsertedStart(AParagraphIndex: TdxParagraphIndex;
      ANewRunIndex: TdxRunIndex; ALength, AHistoryNotificationId
      : Integer): Boolean;
    function OnRunMergedStart(AParagraphIndex: TdxParagraphIndex;
      ANewRunIndex: TdxRunIndex; ALength: Integer): Boolean;
    function OnRunInsertedEnd(AParagraphIndex: TdxParagraphIndex;
      ANewRunIndex: TdxRunIndex; ALength, AHistoryNotificationId
      : Integer): Boolean;
    function OnRunMergedEnd(AParagraphIndex: TdxParagraphIndex;
      ANewRunIndex: TdxRunIndex; ALength: Integer): Boolean;
  public
    constructor Create(APieceTable: TdxSimplePieceTable;
      const AStart, AEnd: TdxDocumentLogPosition); overload;
    constructor Create(APieceTable: TdxSimplePieceTable;
      const AStart, AEnd: TdxDocumentLogPosition;
      APropertiesIndex: Integer); overload;
    destructor Destroy; override;
    function Clone: TdxRangePermission;
    procedure Delete(AIndex: Integer); override;
    procedure Visit(const AVisitor: IdxDocumentIntervalVisitor); override;
    function Subtract(AInterval: TdxRangePermission)
      : TdxRangePermissionCollection;
    class function Union(AInterval1: TdxRangePermission;
      AInterval2: TdxRangePermission): TdxRangePermission; static;

    class property BoundaryComparer: TBoundaryComparer read FBoundaryComparer;
    class property BoundaryOrderComparer: TBoundaryOrderComparer
      read FBoundaryOrderComparer;
  ...
  strict private
    function GetItem(Index: Integer): TdxRangePermission; inline;
  public
  ...
  protected
    procedure OnRunInserted(APieceTable: TdxCustomPieceTable;
      ...
      ALength: Integer; AHistoryNotificationId: Integer); override;
  ...
  TdxOwnedRangePermissionMergedCollection = class
    (TdxRangePermissionCollectionEx)
  protected
    function CreateInnerList: TdxDocumentIntervalList; override;
  end;
  ...
  public
    function IsBoundaryAffected(ABoundaryPosition: PdxDocumentModelPosition;
      ANewRunIndex: TdxRunIndex): Boolean; virtual; abstract;
    procedure ShiftBoundary(ABoundary: TdxVisitableDocumentIntervalBoundary;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength, AHistoryNotificationId: Integer); virtual; abstract;
    procedure RemainBoundary(ABoundary: TdxVisitableDocumentIntervalBoundary;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength: Integer); virtual; abstract;
  end;
  ...
  strict private
    ...
    FPieceTable: TdxSimplePieceTable;
    FBookmarkIteratorCache: TObjectDictionary<TdxSimplePieceTable,
      TdxVisitableDocumentIntervalBoundaryIterator>;
    ...
    procedure SetBookmarkIterator(const Value
      : TdxVisitableDocumentIntervalBoundaryIterator);
  protected
    function GetBookmarkBoxCollection(ARow: TdxSimpleRow)
      : TdxVisitableDocumentIntervalBoxCollection; virtual;
    procedure ClearBookmarkBoxCollection(ARow: TdxSimpleRow); virtual;
    procedure ChangeBookmarkIterator(AOldPieceTable, ANewPieceTable
      : TdxSimplePieceTable);
    procedure EnsureBookmarkIteratorIsInitialized;
    function ShouldCalculate: Boolean; virtual;

    function CalculateBoxes(ARow: TdxSimpleRow)
      : TdxVisitableDocumentIntervalBoxCollection;
    function GetHorizontalPosition(const APosition: TdxFormatterPosition;
      ABox: TdxBox): Integer;
    function GetPosition(ABoundary: TdxVisitableDocumentIntervalBoundary)
      : TdxFormatterPosition;
    function GetPrevPosition(const APos: TdxFormatterPosition)
      : TdxFormatterPosition;
    function MeasureBoxPart(const APosition: TdxFormatterPosition; ABox: TdxBox)
      : Integer;
    function CreateBookmarkBoundaryIterator
      : TdxVisitableDocumentIntervalBoundaryIterator; virtual;
    function GetBoxColor: TdxAlphaColor; virtual;

    property BookmarkIterator: TdxVisitableDocumentIntervalBoundaryIterator
      read FBookmarkIterator write SetBookmarkIterator;
    property BookmarkIteratorCache
      : TObjectDictionary<TdxSimplePieceTable,
      TdxVisitableDocumentIntervalBoundaryIterator> read FBookmarkIteratorCache;
  public
    constructor Create(APieceTable: TdxSimplePieceTable;
  ...
  protected
    function CreateBookmarkBoundaryIterator
      : TdxVisitableDocumentIntervalBoundaryIterator; override;
...
type
  ...
  TdxRangePermissionBoundary = class abstract
    (TdxVisitableDocumentIntervalBoundary)
  protected
  ...
  TdxRangePermissionBoundaryIterator = class
    (TdxVisitableDocumentIntervalBoundaryIterator)
  protected
    procedure PopulateBoundariesCore; overload; override;
    procedure PopulateBoundariesCore(AIntervals: TObjectList);
  ...
  public
    constructor Create(APieceTable: TdxSimplePieceTable;
      const AUpdateStrategy: TdxRangePermissionBoundaryUpdateStrategy);
    destructor Destroy; override;
    procedure Update(ABoundaries
      : TdxVisitableDocumentIntervalBoundaryCollection;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength, AHistoryNotificationId: Integer);
    function GetAdjoiningBoundariesCount(ABoundaries
      : TdxVisitableDocumentIntervalBoundaryCollection;
      AIndex: Integer): Integer;
    procedure ReorderBoundaries(ABoundaries
      : TdxVisitableDocumentIntervalBoundaryCollection;
      AIndex, ACount: Integer);
    procedure UpdateAdjoiningBoundaries(ABoundaries
      : TdxList<TdxVisitableDocumentIntervalBoundary>;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength, AHistoryNotificationId: Integer);
    procedure ShiftBoundaries(ABoundaries
      : TdxList<TdxVisitableDocumentIntervalBoundary>; AIndex: Integer;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength, AHistoryNotificationId: Integer);
    function ShouldUpdateBoundary(ABoundary
      : TdxVisitableDocumentIntervalBoundary): Boolean;

    property PieceTable: TdxSimplePieceTable read FPieceTable;
    property UpdateStrategy: TdxRangePermissionBoundaryUpdateStrategy
      read FUpdateStrategy;
  end;
  ...
  public
    function IsBoundaryAffected(ABoundaryPosition: PdxDocumentModelPosition;
      ANewRunIndex: TdxRunIndex): Boolean; override;
    procedure ShiftBoundary(ABoundary: TdxVisitableDocumentIntervalBoundary;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength, AHistoryNotificationId: Integer); override;
    procedure RemainBoundary(ABoundary: TdxVisitableDocumentIntervalBoundary;
      AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
      ALength: Integer); override;
  end;
...
constructor TdxRangePermissionBoundaryUpdater.Create
...
procedure TdxRangePermissionBoundaryUpdater.Update
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        UpdateAdjoiningBoundaries(ARange, AParagraphIndex, ANewRunIndex,
    ...
    begin
      FUpdateStrategy.ShiftBoundary(ABoundaries[AIndex], AParagraphIndex,
...
procedure TdxRangePermissionBoundaryUpdater.ReorderBoundaries
  (ABoundaries: TdxVisitableDocumentIntervalBoundaryCollection;
  AIndex, ACount: Integer);
begin
...
procedure TdxRangePermissionBoundaryUpdater.UpdateAdjoiningBoundaries
  (ABoundaries: TdxList<TdxVisitableDocumentIntervalBoundary>;
  AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      ShiftBoundaries(ABoundaries, AStartIndex, AParagraphIndex, ANewRunIndex,
        ALength, AHistoryNotificationId);
      Exit;
    end;
  end;
...
procedure TdxRangePermissionBoundaryUpdater.ShiftBoundaries
  (ABoundaries: TdxList<TdxVisitableDocumentIntervalBoundary>; AIndex: Integer;
  AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
var
  ACount, I: Integer;
begin
  ...
  begin
    if I < AIndex then
      FUpdateStrategy.RemainBoundary(ABoundaries[I], AParagraphIndex,
        ANewRunIndex, ALength)
    else
      FUpdateStrategy.ShiftBoundary(ABoundaries[I], AParagraphIndex,
        ANewRunIndex, ALength, AHistoryNotificationId);
  end;
end;

function TdxRangePermissionBoundaryUpdater.ShouldUpdateBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary): Boolean;
var
...
function TdxRunInsertedUpdateStrategy.IsBoundaryAffected(ABoundaryPosition
  : PdxDocumentModelPosition; ANewRunIndex: TdxRunIndex): Boolean;
begin
  Result := (ABoundaryPosition.RunIndex = ANewRunIndex) and
    (ABoundaryPosition.RunOffset = 0);
end;

procedure TdxRunInsertedUpdateStrategy.ShiftBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary;
  AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
begin
  TdxRangePermissionBoundary(ABoundary).OnRunInserted(AParagraphIndex,
    ANewRunIndex, ALength, AHistoryNotificationId);
end;

procedure TdxRunInsertedUpdateStrategy.RemainBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary;
  AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength: Integer);
begin
...
function TdxRunMergedUpdateStrategy.IsBoundaryAffected(ABoundaryPosition
  : PdxDocumentModelPosition; ANewRunIndex: TdxRunIndex): Boolean;
begin
  Result := (ABoundaryPosition.RunIndex = ANewRunIndex + 1) and
    (ABoundaryPosition.RunOffset = 0);
end;

procedure TdxRunMergedUpdateStrategy.ShiftBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary;
  AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
begin
  TdxRangePermissionBoundary(ABoundary).OnRunMerged(AParagraphIndex,
    ANewRunIndex, ALength, AHistoryNotificationId);
end;

procedure TdxRunMergedUpdateStrategy.RemainBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary;
  AParagraphIndex: TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength: Integer);
var
...
constructor TdxBookmarkBase.TdxHistoryNotificationsAccessor.Create
  (AGetter: TdxGetDelegate; ASetter: TdxSetDelegate);
begin
  inherited Create;
  FGetter := AGetter;
  FSetter := ASetter;
...
function TdxBookmarkBase.TdxHistoryNotificationsAccessor.
  GetChangeNotificationIds: TdxIntegerList;
begin
  Result := FGetter();
end;

procedure TdxBookmarkBase.TdxHistoryNotificationsAccessor.
  SetChangeNotificationIds(AValue: TdxIntegerList);
begin
  FSetter(AValue);
end;

function TdxBookmarkBase.TdxHistoryNotificationsAccessor.ShouldChange
  (AHistoryNotificationId: Integer): Boolean;
var
  AIndex: Integer;
begin
  if (ChangeNotificationIds = nil) or
    (AHistoryNotificationId = TdxNotificationIdGenerator.EmptyId) then
    Result := False
  else
    Result := ChangeNotificationIds.BinarySearch
      (AHistoryNotificationId, AIndex);
end;

procedure TdxBookmarkBase.TdxHistoryNotificationsAccessor.
  AddChangeNotificationId(AHistoryNotificationId: Integer);
var
  AIndex: Integer;
begin
...
constructor TdxBookmarkBase.Create(APieceTable: TdxCustomPieceTable;
  AStart, AEnd: TdxDocumentLogPosition; AForceUpdateInterval: Boolean = False);
begin
...
procedure TdxBookmarkBase.RunInserted(AParagraphIndex: TdxParagraphIndex;
...
begin
  AStartAccessor := TdxHistoryNotificationsAccessor.Create
    (GetStartChangeNotificationIds, SetStartChangeNotificationIds);
  try
    AEndAccessor := TdxHistoryNotificationsAccessor.Create
      (GetEndChangeNotificationIds, SetEndChangeNotificationIds);
    try
      AStartChanged := RunInserted(@Interval.Start, ANewRunIndex, ALength,
        AStartAccessor, AHistoryNotificationId);
      AEndChanged := RunInserted(@Interval.&End, ANewRunIndex, ALength,
        AEndAccessor, AHistoryNotificationId);
    finally
      AEndAccessor.Free;
    end;
  finally
...
function TdxBookmarkBase.RunInserted(APosition: PdxDocumentModelPosition;
  ANewRunIndex: TdxRunIndex; ALength: Integer;
  AAccessor: TdxHistoryNotificationsAccessor;
  AHistoryNotificationId: Integer): Boolean;
begin
  if (ANewRunIndex < APosition^.RunIndex) or
    ((ANewRunIndex = APosition^.RunIndex) and ((APosition^.RunOffset > 0) or
...
procedure TdxBookmarkBase.RunRemoved(AParagraphIndex: TdxParagraphIndex;
...
begin
  ...
  try
    AEndAccessor := TdxHistoryNotificationsAccessor.Create
      (GetEndChangeNotificationIds, SetEndChangeNotificationIds);
    try
      AStartChanged := RunRemoved(@Interval.Start, ARunIndex, ALength,
        AStartAccessor, AHistoryNotificationId);
      AEndChanged := RunRemoved(@Interval.&End, ARunIndex, ALength,
        AEndAccessor, AHistoryNotificationId);
    finally
      AEndAccessor.Free;
    end;
...
function TdxBookmarkBase.RunRemoved(APos: PdxDocumentModelPosition;
  ARunIndex: TdxRunIndex; ALength: Integer;
  AAccessor: TdxHistoryNotificationsAccessor;
  AHistoryNotificationId: Integer): Boolean;
begin
  ...
  else if ARunIndex < APos^.RunIndex then
  begin
...
var
  C: Char;
begin
  if (System.Length(AName) > 0) and not
  {$IFDEF DELPHIXE4}AName[1].IsLetter{$ELSE}TCharacter.IsLetter(AName[1]){$ENDIF} then
    Exit(False);
  for C in AName do
    if not {$IFDEF DELPHIXE4}C.IsLetterOrDigit{$ELSE}TCharacter.IsLetterOrDigit
      (C){$ENDIF} and (C <> '_') then
      Exit(False);
  Result := True;
end;
...
function TdxBookmarkBaseCollection.GetEntireBookmarksCore
...
begin
  ...
  for I := 0 to Count - 1 do
    if (Self[I].Start >= AStart) and (Self[I].&End <= AEnd) and
      (Self[I].Start < AEnd) then
      Result.Add(Self[I]);
end;
...
procedure TdxBookmarkStartBoundary.Export(const AExporter
  : IdxDocumentModelExporter);
begin
...
procedure TdxBookmarkEndBoundary.Export(const AExporter
  : IdxDocumentModelExporter);
begin
...
function TdxVisitableDocumentIntervalBoundaryCollection.GetRange(AIndex,
  ALength: Integer): TdxList<TdxVisitableDocumentIntervalBoundary>;
var
  I: Integer;
begin
...
procedure TdxVisitableDocumentIntervalBoundaryCollection.Sort(AIndex,
  ACount: Integer;
  const AComparer: IComparer<TdxVisitableDocumentIntervalBoundary>);
var
  AOwnsItems: Boolean;
begin
  if (AIndex < 0) or ((AIndex >= Count) and (ACount > 0)) or
    (AIndex + ACount - 1 >= Count) or (ACount < 0) or (AIndex + ACount < 0) then
...
procedure TdxVisitableDocumentIntervalStartBoundaryFactory.Visit
  (AInterval: TdxVisitableDocumentInterval);
begin
...
procedure TdxVisitableDocumentIntervalStartBoundaryFactory.SetBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary);
begin
  FBoundary := ABoundary;
end;
...
procedure TdxVisitableDocumentIntervalEndBoundaryFactory.Visit
  (AInterval: TdxVisitableDocumentInterval);
begin
  if AInterval is TdxBookmark then
...
procedure TdxVisitableDocumentIntervalEndBoundaryFactory.SetBoundary
  (ABoundary: TdxVisitableDocumentIntervalBoundary);
begin
  FBoundary := ABoundary;
end;
...
function TdxVisitableDocumentIntervalBoundaryIterator.TComparer.Compare
  (const Left, Right: TdxVisitableDocumentIntervalBoundary): Integer;
begin
...
begin
  ...
  FVisitableDocumentIntervalBoundaries :=
    TdxVisitableDocumentIntervalBoundaryCollection.Create;
...
function TdxVisitableDocumentIntervalBoundaryIterator.InternalGetCurrent
  : TdxVisitableDocumentIntervalBoundary;
begin
...
class function TdxVisitableDocumentIntervalBoundaryIterator.
  CreateVisitableDocumentIntervalStartBoundaryFactory
  : TdxVisitableDocumentIntervalStartBoundaryFactory;
begin
...
class function TdxVisitableDocumentIntervalBoundaryIterator.
  CreateVisitableDocumentIntervalEndBoundaryFactory
  : TdxVisitableDocumentIntervalEndBoundaryFactory;
begin
  Result := TdxVisitableDocumentIntervalEndBoundaryFactory.Create;
end;

function TdxVisitableDocumentIntervalBoundaryIterator.IsVisibleInterval
  (AInterval: TdxVisitableDocumentInterval): Boolean;
begin
  Result := True;
end;
...
procedure TdxVisitableDocumentIntervalBoundaryIterator.PopulateBookmarksCore
...
procedure TdxVisitableDocumentIntervalBasedObjectBoundaryIterator.
  PopulateBoundariesCore;
begin
...
procedure TdxVisitableDocumentIntervalBasedObjectBoundaryIterator.
  PopulateBoundariesCore(AIntervals: TObjectList);
begin
...
function TdxRangePermission.TBoundaryComparer.Compare(const ALeft,
  ARight: TdxVisitableDocumentIntervalBoundary): Integer;
begin
...
function TdxRangePermission.TBoundaryOrderComparer.Compare(const ALeft,
  ARight: TdxVisitableDocumentIntervalBoundary): Integer;
begin
  Result := ALeft.VisitableInterval.NormalizedStart -
    ARight.VisitableInterval.NormalizedStart;
  if Result <> 0 then
    Exit(Result);
  Result := ALeft.VisitableInterval.NormalizedEnd -
    ARight.VisitableInterval.NormalizedEnd;
  if Result <> 0 then
    Exit(Result);
...
constructor TdxRangePermission.Create(APieceTable: TdxSimplePieceTable;
  const AStart, AEnd: TdxDocumentLogPosition; APropertiesIndex: Integer);
begin
...
begin
  Result := TdxRangePermission.Create(PieceTable, Start, &End,
    Properties.Index);
  Result.Group := Group;
...
function TdxRangePermission.Subtract(AInterval: TdxRangePermission)
  : TdxRangePermissionCollection;
begin
  ...
  begin
    if Start < AInterval.Start then
      Result.Add(TdxRangePermission.Create(PieceTable, Start, AInterval.Start,
        Properties.Index));
    if &End > AInterval.&End then
      Result.Add(TdxRangePermission.Create(PieceTable, AInterval.&End, &End,
        Properties.Index));
  end
  else
  begin
...
class function TdxRangePermission.Union(AInterval1: TdxRangePermission;
  AInterval2: TdxRangePermission): TdxRangePermission;
var
  AStart, AEnd: TdxDocumentLogPosition;
begin
  AStart := Min(AInterval1.Start, AInterval2.Start);
  AEnd := Max(AInterval1.&End, AInterval2.&End);
  Result := TdxRangePermission.Create(AInterval1.PieceTable, AStart, AEnd,
    AInterval1.Properties.Index);
end;

function TdxRangePermission.IsGranted(APieceTable: TdxSimplePieceTable)
  : Boolean;
begin
...
function TdxRangePermission.OnRunInsertedStart(AParagraphIndex
...
begin
  ...
  try
    AAnchor.OnRunInserted(PieceTable, AParagraphIndex, ANewRunIndex, ALength,
...
function TdxRangePermission.OnRunMergedStart(AParagraphIndex: TdxParagraphIndex;
  ANewRunIndex: TdxRunIndex; ALength: Integer): Boolean;
var
  AAnchor: TdxDocumentModelPositionAnchor;
begin
...
function TdxRangePermission.OnRunInsertedEnd(AParagraphIndex: TdxParagraphIndex;
  ANewRunIndex: TdxRunIndex; ALength, AHistoryNotificationId: Integer): Boolean;
var
  AAnchor: TdxDocumentModelPositionAnchor;
begin
...
function TdxRangePermission.OnRunMergedEnd(AParagraphIndex: TdxParagraphIndex;
  ANewRunIndex: TdxRunIndex; ALength: Integer): Boolean;
var
  AAnchor: TdxDocumentModelPositionAnchor;
begin
...
function TdxRangePermissionCollection.GetBoundaries
  : TdxVisitableDocumentIntervalBoundaryCollection;
begin
...
function TdxRangePermissionCollection.GetItem(Index: Integer)
  : TdxRangePermission;
begin
  Result := TdxRangePermission(inherited Items[Index]);
end;

procedure TdxRangePermissionCollection.OnRunInserted
  (APieceTable: TdxCustomPieceTable; AParagraphIndex: TdxParagraphIndex;
  ANewRunIndex: TdxRunIndex; ALength: Integer; AHistoryNotificationId: Integer);
var
  AUpdater: TdxRangePermissionBoundaryUpdater;
begin
  AUpdater := TdxRangePermissionBoundaryUpdater.Create(PieceTable,
    TdxRunInsertedUpdateStrategy.Create);
  try
    AUpdater.Update(Boundaries, AParagraphIndex, ANewRunIndex, ALength,
      AHistoryNotificationId);
  finally
    AUpdater.Free;
  end;
...
procedure TdxRangePermissionCollection.OnRunMerged
  (APieceTable: TdxCustomPieceTable; AParagraphIndex: TdxParagraphIndex;
...
begin
  AUpdater := TdxRangePermissionBoundaryUpdater.Create(PieceTable,
    TdxRunMergedUpdateStrategy.Create);
  try
    AUpdater.Update(Boundaries, AParagraphIndex, ARunIndex, ADeltaRunLength,
      TdxNotificationIdGenerator.EmptyId);
  finally
    AUpdater.Free;
  end;
end;

function TdxRangePermissionCollection.CreateBoundaryCollection
  : TdxVisitableDocumentIntervalBoundaryCollection;
var
  I: Integer;
...
procedure TdxRangePermissionCollection.OnDocumentIntervalInserted
  (AIndex: Integer);
begin
...
procedure TdxRangePermissionCollection.OnDocumentIntervalRemoved
  (AIndex: Integer);
begin
...
function TdxRangePermissionCollectionEx.AddCore
  (AInterval: TdxDocumentInterval): Integer;
var
...
function TdxRangePermissionCollectionEx.Remove
  (AInterval: TdxRangePermission): Boolean;
var
...
function TdxRangePermissionCollectionEx.Contains
  (AInterval: TdxRangePermission): Boolean;
var
  ACount, I: Integer;
begin
...
procedure TdxRangePermissionCollectionEx.RemoveCore
  (AToRemove: TdxRangePermissionList);
var
  ACount, I: Integer;
begin
...
procedure TdxRangePermissionCollectionEx.AddRange
  (AToAdd: TdxRangePermissionList);
var
  ACount, I: Integer;
begin
  ACount := AToAdd.Count;
...
{ TdxRangePermissionMergedCollection }
function TdxRangePermissionMergedCollection.CreateInnerList
  : TdxDocumentIntervalList;
begin
...
function TdxOwnedRangePermissionMergedCollection.CreateInnerList
  : TdxDocumentIntervalList;
begin
...
procedure TdxRangePermissionStartBoundary.Export(const AExporter
  : IdxDocumentModelExporter);
begin
  AExporter.ExportRangePermissionStart(TdxRangePermission(VisitableInterval));
end;

function TdxRangePermissionStartBoundary.CreateBox
  : TdxVisitableDocumentIntervalBox;
begin
  Result := TdxBookmarkStartBox.Create;
end;

procedure TdxRangePermissionStartBoundary.OnRunInserted(AParagraphIndex
  : TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
begin
  TdxRangePermission(VisitableInterval).OnRunInsertedStart(AParagraphIndex,
    ANewRunIndex, ALength, AHistoryNotificationId);
end;

procedure TdxRangePermissionStartBoundary.OnRunMerged(AParagraphIndex
  : TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
begin
  TdxRangePermission(VisitableInterval).OnRunMergedStart(AParagraphIndex,
    ANewRunIndex, ALength);
end;
...
procedure TdxRangePermissionEndBoundary.Export(const AExporter
  : IdxDocumentModelExporter);
begin
  AExporter.ExportRangePermissionEnd(TdxRangePermission(VisitableInterval));
end;

function TdxRangePermissionEndBoundary.CreateBox
  : TdxVisitableDocumentIntervalBox;
begin
  Result := TdxBookmarkEndBox.Create;
end;

procedure TdxRangePermissionEndBoundary.OnRunInserted(AParagraphIndex
  : TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
begin
  TdxRangePermission(VisitableInterval).OnRunInsertedEnd(AParagraphIndex,
    ANewRunIndex, ALength, AHistoryNotificationId);
end;

procedure TdxRangePermissionEndBoundary.OnRunMerged(AParagraphIndex
  : TdxParagraphIndex; ANewRunIndex: TdxRunIndex;
  ALength, AHistoryNotificationId: Integer);
begin
  TdxRangePermission(VisitableInterval).OnRunMergedEnd(AParagraphIndex,
    ANewRunIndex, ALength);
end;
...
function TdxRangePermissionColorer.GetColor(ARangePermission
  : TdxRangePermission): TdxAlphaColor;
var
  AId: string;
...
function TdxRangePermissionBoundaryIterator.IsVisibleInterval
  (AInterval: TdxVisitableDocumentInterval): Boolean;
var
  ARangePermission: TdxRangePermission;
begin
  ...
  if ARangePermission = nil then
    Exit(False);
  if not TdxDocumentModel(PieceTable.DocumentModel).ProtectionProperties.EnforceProtection
  then
    Exit(True);
  Result := ARangePermission.IsGranted(PieceTable);
end;
...
constructor TdxBookmarkBoxCalculator.Create(APieceTable: TdxSimplePieceTable;
  const AMeasurerProvider: IdxBoxMeasurerProvider);
begin
  inherited Create;
  FExportToPdf := False;
  ...
  FBookmarkIteratorCache := TObjectDictionary<TdxSimplePieceTable,
    TdxVisitableDocumentIntervalBoundaryIterator>.Create([doOwnsValues]);
end;
...
procedure TdxBookmarkBoxCalculator.SetBookmarkIterator
  (const Value: TdxVisitableDocumentIntervalBoundaryIterator);
begin
...
procedure TdxBookmarkBoxCalculator.SetPieceTable(const AValue
  : TdxSimplePieceTable);
begin
...
function TdxBookmarkBoxCalculator.GetBookmarkBoxCollection(ARow: TdxSimpleRow)
  : TdxVisitableDocumentIntervalBoxCollection;
begin
  Result := ARow.GetBookmarkBoxesCore;
end;

procedure TdxBookmarkBoxCalculator.ClearBookmarkBoxCollection
  (ARow: TdxSimpleRow);
begin
  ARow.ClearBookmarkBoxes;
end;

procedure TdxBookmarkBoxCalculator.ChangeBookmarkIterator(AOldPieceTable,
  ANewPieceTable: TdxSimplePieceTable);
var
...
begin
  if BookmarkIterator <> nil then
    ...
    begin
      if not FBookmarkIteratorCache.TryGetValue(AOldPieceTable,
        AOldBookmarkIterator) or (AOldBookmarkIterator <> BookmarkIterator) then
...
begin
  Result := (TdxDocumentModel(DocumentModel)
    .BookmarkOptions.Visibility = TdxRichEditBookmarkVisibility.Visible) or
    ExportToPdf;
end;

function TdxBookmarkBoxCalculator.CalculateBoxes(ARow: TdxSimpleRow)
  : TdxVisitableDocumentIntervalBoxCollection;
var
  ABoundary: TdxVisitableDocumentIntervalBoundary;
...
begin
  ...
  begin
    ...
    if not TdxAlgorithms1<TdxBoxBase>.BinarySearch(ABoxes,
      AComparable.CompareTo, ABoxIndex) then
    begin
    ...
    ABox.HorizontalPosition := GetHorizontalPosition(APosition,
      ABoxes[ABoxIndex]);
...
function TdxBookmarkBoxCalculator.GetHorizontalPosition(const APosition
  : TdxFormatterPosition; ABox: TdxBox): Integer;
begin
  if ABox.StartPos.RunIndex <> APosition.RunIndex then
    Exit(ABox.Bounds.Left);
  if APosition = ABox.StartPos then
    Exit(ABox.Bounds.Left)
  else if GetPrevPosition(APosition) = ABox.EndPos then
    Exit(ABox.Bounds.Right);
  Result := ABox.Bounds.Left + MeasureBoxPart(APosition, ABox);
end;

function TdxBookmarkBoxCalculator.GetPosition
  (ABoundary: TdxVisitableDocumentIntervalBoundary): TdxFormatterPosition;
begin
  Result := TdxFormatterPosition.Create(ABoundary.Position.RunIndex,
    ABoundary.Position.RunOffset, 0);
end;

function TdxBookmarkBoxCalculator.GetPrevPosition
  (const APos: TdxFormatterPosition): TdxFormatterPosition;
var
...
function TdxBookmarkBoxCalculator.MeasureBoxPart(const APosition
  : TdxFormatterPosition; ABox: TdxBox): Integer;
var
  ABoxInfo: TdxBoxInfo;
...
begin
  ...
  try
    ...
    ABoxInfo.StartPos := TdxFormatterPosition.Create(ABoxStartPos.RunIndex,
      ABoxStartPos.Offset, -1);
    APrevPosition := GetPrevPosition(APosition);
    ABoxInfo.EndPos := TdxFormatterPosition.Create(APrevPosition.RunIndex,
      APrevPosition.Offset, -1);
...
procedure TdxBookmarkBoxCalculator.ResetFrom(const APosition
...
function TdxBookmarkBoxCalculator.CreateBookmarkBoundaryIterator
  : TdxVisitableDocumentIntervalBoundaryIterator;
begin
  Result := TdxVisitableDocumentIntervalBoundaryIterator.Create(PieceTable,
    ExportToPdf);
end;

{ TdxRangePermissionBoxCalculator }

function TdxRangePermissionBoxCalculator.CreateBookmarkBoundaryIterator
  : TdxVisitableDocumentIntervalBoundaryIterator;
begin
...
begin
  Result := TdxDocumentModel(DocumentModel).RangePermissionOptions.Visibility <>
    TdxRichEditRangePermissionVisibility.Hidden;
end;
...
begin
  if TdxDocumentModel(DocumentModel).ProtectionProperties.EnforceProtection then
    Exit(TdxDocumentModel(DocumentModel)
      .RangePermissionOptions.HighlightBracketsColor);
  Result := TdxDocumentModel(DocumentModel)
    .RangePermissionOptions.BracketsColor;
end;

function TdxRangePermissionBoxCalculator.GetBookmarkBoxCollection
  (ARow: TdxSimpleRow): TdxVisitableDocumentIntervalBoxCollection;
begin
  Result := ARow.GetRangePermissionBoxesCore;
end;

procedure TdxRangePermissionBoxCalculator.ClearBookmarkBoxCollection
  (ARow: TdxSimpleRow);
begin
