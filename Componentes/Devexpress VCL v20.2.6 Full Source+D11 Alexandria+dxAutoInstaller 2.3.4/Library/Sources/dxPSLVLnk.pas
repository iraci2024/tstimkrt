...
{$ENDIF}
  ...
  dxCore, dxPSGlbl, dxPSCore, dxExtCtrls,
  dxPrnPg, dxPSBaseGridLnk, cxDrawTextUtils, cxControls, cxContainer, cxEdit,
  cxCheckBox, cxLabel, Menus, cxTextEdit,
  cxLookAndFeelPainters, cxButtons, cxPC, cxGraphics, cxMaskEdit,
  cxDropDownEdit, cxColorComboBox, cxLookAndFeels,
  cxImageComboBox, ComCtrls, CommCtrl, dxPSReportLinkDesignWindow,
...
type
  ...
  TCustomdxNativeListViewReportLink = class(TAbstractdxGridReportLink,
  ...
  protected
    ...
    procedure AssignData(ACol, ARow: Integer;
      ADataItem: TAbstractdxReportCellData); override;
    ...
    procedure AddExtraImage(AParent: TdxReportCell; ACol, ARow: Integer;
      R: TRect);
    ...
    procedure InitializeInternalItem(AnItem: TAbstractdxReportCellData;
      AnIndex: Integer); virtual;
    function PlaceInternalItem(AParent: TdxReportCell; ACol, ARow: Integer;
      R: TRect): TAbstractdxReportCellData; virtual;
    ...
    property InternalItems[Index: Integer]: TAbstractdxReportCellData
      read GetInternalItem;
    property LargeImages: TCustomImageList read GetLargeImages;
    property ListViewHandle: THandle read GetListViewHandle
      write SetListViewHandle;
  ...
  public
    ...
    property HeaderColor: TColor read GetHeaderColor write SetHeaderColor
      default clBtnFace; { dxDefaultFixedColor }
    property HeaderFont: TFont read GetHeaderFont write SetHeaderFont
      stored IsFixedFontStored;
    property HeadersOnEveryPage;
    property HeaderTransparent: Boolean read GetHeaderTransparent
      write SetHeaderTransparent default False;
    property IncludeHeaders: Boolean read GetIncludeHeaders
      write SetIncludeHeaders default True;
    property OddColor;
    property OddFont;
  ...
  TdxListViewReportLinkCustomDrawInfo = record
    ...
    case Integer of
      dxListViewItemID:
        (ListItem: TListItem; SubItem: Integer);
      dxListViewHeaderID:
        (HeaderIndex: Integer);
  end;
  ...
  protected
    ...
    procedure DoInitializeHeader(ACol, ARow: Integer;
      ADataItem: TAbstractdxReportCellData); dynamic;
    procedure DoInitializeItem(ACol, ARow: Integer;
      ADataItem: TAbstractdxReportCellData); override;
    procedure InternalRestoreFromOriginal; override;
    procedure InternalRestoreDefaults; override;
    procedure PrepareConstruct(AReportCells: TdxReportCells); override;
    ...
    procedure DoCustomDrawHeader(ACanvas: TCanvas; ARect: TRect;
      AHeaderIndex: Integer; AnItem: TdxReportCellImage;
      var ADone: Boolean); dynamic;
    procedure DoCustomDrawItem(ACanvas: TCanvas; ARect: TRect;
      AListItem: TListItem; ASubItem: Integer; AnItem: TdxReportCellImage;
      var ADone: Boolean); dynamic;
  ...
  public
    ...
    property OnCustomDrawHeader: TdxCustomDrawListViewHeaderEvent
      read FOnCustomDrawHeader write SetOnCustomDrawHeader;
    property OnCustomDrawItem: TdxCustomDrawListViewItemEvent
      read FOnCustomDrawItem write SetOnCustomDrawItem;
    property OnInitializeHeader: TdxCustomGridReportLinkInitializeItemEvent
      read FOnInitializeHeader write FOnInitializeHeader;
  end;
...
const
  dxDefaultListViewOptions = [Low(TdxListViewPaintOption)
    .. High(TdxListViewPaintOption)];
...
procedure ListViewOwnerDataFetch(AControl: TCustomListView; AnIndex: Integer;
  ARequest: TItemRequest);
begin
...
procedure TCustomdxNativeListViewReportLink.GetImageLists
  (AProc: TdxPSGetImageListProc);
begin
  inherited;
  if HasLargeImages then
    AProc(LargeImages);
  if HasSmallImages then
    AProc(SmallImages);
  if HasStateImages then
    AProc(StateImages);
end;

function TCustomdxNativeListViewReportLink.GetRebuildOnPageParamsChange
...
begin
  if IsReportStyle or (GetFlatInternalItemIndex(ACol, ARow) < LV_GetItemCount)
  then
  begin
    inherited;
...
function TCustomdxNativeListViewReportLink.CanCalculateColumnAutoWidths
  : Boolean;
begin
  Result := IsReportStyle;
end;

function TCustomdxNativeListViewReportLink.CreateDataItem
  (AParent: TdxReportCell; ACol, ARow: Integer; const ABounds: TRect)
...
function TCustomdxNativeListViewReportLink.GetActualColIndex
...
function TCustomdxNativeListViewReportLink.GetCellColor(ACol,
  ARow: Integer): TColor;
begin
  if IsReportStyle then
...
function TCustomdxNativeListViewReportLink.GetCellFontIndex(ACol,
...
function TCustomdxNativeListViewReportLink.GetCellImageIndex(ACol,
  ARow: Integer): Integer;
begin
  if IsReportStyle then
...
function TCustomdxNativeListViewReportLink.GetCellImageLayout(ACol,
  ARow: Integer): TdxImageLayout;
begin
  if IsReportStyle then
    if IsFixedRow(ARow) then
  ...
  else if IsIconStyle then
    Result := ilImageTopCenter
  else
    Result := ilImageCenterLeft;
end;

function TCustomdxNativeListViewReportLink.GetCellImageList(ACol, ARow: Integer)
  : TCustomImageList;
begin
  if IsReportStyle then
    if IsFixedRow(ARow) then
...
function TCustomdxNativeListViewReportLink.GetCellMultiline(ACol,
  ARow: Integer): Boolean;
begin
  Result := IsIconStyle or inherited GetCellMultiline(ACol, ARow)
end;

function TCustomdxNativeListViewReportLink.GetCellSides(ACol, ARow: Integer)
  : TdxCellSides;
  ...
  begin
    Result := (IsReportStyle and LV_HasColumnHeaders and (ARow < 2)) or
      (ARow = 0);
  end;
...
begin
  ...
  begin
    ...
    begin
      if IsCellMostLeft(ACol, ARow) then
        Exclude(Result, csLeft);
      if IsCellMostTop(ACol, ARow) then
        Exclude(Result, csTop);
      if IsCellMostRight(ACol, ARow) then
        Exclude(Result, csRight);
      if IsCellMostBottom(ACol, ARow) then
        Exclude(Result, csBottom);
    end;
    if not IsDrawHorzLines then
    begin
      if not IsCellMostTop(ACol, ARow) then
        Exclude(Result, csTop);
      if not IsCellMostBottom(ACol, ARow) then
        Exclude(Result, csBottom);
    end;
    if not IsDrawVertLines then
    begin
      if not IsCellMostLeft(ACol, ARow) then
        Exclude(Result, csLeft);
      if not IsCellMostRight(ACol, ARow) then
        Exclude(Result, csRight);
    end;
...
function TCustomdxNativeListViewReportLink.GetCellText(ACol,
  ARow: Integer): string;
begin
  if IsReportStyle then
    if IsFixedRow(ARow) then
...
function TCustomdxNativeListViewReportLink.GetCellTextAlignX(ACol,
...
function TCustomdxNativeListViewReportLink.GetCellTextAlignY(ACol,
...
function TCustomdxNativeListViewReportLink.GetColSortOrder(ACol: Integer)
  : TdxCellSortOrder;
begin
...
begin
  Result := Ord(IsReportStyle and ShowColumnHeaders and
    (not HasSelection or IncludeHeaders));
end;
...
function TCustomdxNativeListViewReportLink.GetSourceColWidth
  (ACol: Integer): Integer;
begin
  if IsReportStyle then
...
function TCustomdxNativeListViewReportLink.GetSourceRowHeight
  (ARow: Integer): Integer;
begin
  if IsFixedRow(ARow) then
...
function TCustomdxNativeListViewReportLink.HasColumnHeaderImage
...
begin
  Result := OnlySelected and IsReportStyle and (LV_GetSelectedCount <> 0) and
    (LV_GetSelectedIndex <> -1);
end;

function TCustomdxNativeListViewReportLink.HasSelectionInRow
  (ARow: Integer): Boolean;
begin
...
function TCustomdxNativeListViewReportLink.IsProcessedCol
  (ACol: Integer): Boolean;
begin
  Result := True;
end;

function TCustomdxNativeListViewReportLink.IsSelectedCell(ACol,
  ARow: Integer): Boolean;
begin
  Result := IsSelectedRow(ARow);
end;

function TCustomdxNativeListViewReportLink.IsSelectedRow(ARow: Integer)
  : Boolean;
begin
  if IsFixedRow(ARow) then
    Result := IncludeHeaders
  else
    Result := HasSelection and LV_GetIsItemSelected
      (ARow - Ord(ShowColumnHeaders));
end;
...
procedure TCustomdxNativeListViewReportLink.PrepareConstruct
...
procedure TCustomdxNativeListViewReportLink.UnprepareConstruct
  (AReportCells: TdxReportCells);
begin
  FColumnCount := 0;
  FRowCount := 0;
...
begin
  Result := ListView_GetExtendedListViewStyle(ListViewHandle) and
    LVS_EX_CHECKBOXES = LVS_EX_CHECKBOXES;
end;

function TCustomdxNativeListViewReportLink.LV_AreColumnHeadersClickable
  : Boolean;
begin
  Result := GetWindowLong(ListViewHandle, GWL_STYLE) and LVS_NOSORTHEADER <>
    LVS_NOSORTHEADER;
end;
...
function TCustomdxNativeListViewReportLink.LV_GetColumnHeaderImageIndex
  (ACol: Integer): Integer;
var
  Column: TLVColumn;
begin
...
function TCustomdxNativeListViewReportLink.LV_GetColumnHeaderImageLayout
  (ACol: Integer): TdxImageLayout;
const
  ImageLayoutsMap: array [Boolean] of TdxImageLayout = (ilImageCenterLeft,
    ilImageCenterRight);
var
  Column: TLVColumn;
begin
  ...
  ListView_GetColumn(ListViewHandle, ACol, Column);
  Result := ImageLayoutsMap[Column.fmt and
    LVCFMT_BITMAP_ON_RIGHT = LVCFMT_BITMAP_ON_RIGHT];
end;

function TCustomdxNativeListViewReportLink.LV_GetColumnHeaderSortOrder
  (ACol: Integer): TdxCellSortOrder;
var
  Header: HWND;
  Item: THDItem;
begin
  ...
  begin
    ...
    begin
      ...
      if Item.fmt and HDF_SORTDOWN = HDF_SORTDOWN then
        Result := csoDown
      else if Item.fmt and HDF_SORTUP = HDF_SORTUP then
        Result := csoUp
    end;
  end;
end;

function TCustomdxNativeListViewReportLink.LV_GetColumnHeaderText
  (ACol: Integer): string;
const
  BufferLength = 4096;
var
...
function TCustomdxNativeListViewReportLink.LV_GetColumnHeaderTextAlignX
  (ACol: Integer): TcxTextAlignX;
var
  Column: TLVColumn;
begin
  FillChar(Column, SizeOf(Column), 0);
  ...
  if Column.fmt and LVCFMT_CENTER = LVCFMT_CENTER then
    Result := taCenterX
  else if Column.fmt and LVCFMT_RIGHT = LVCFMT_RIGHT then
    Result := taRight
  else
    Result := taLeft;
end;

function TCustomdxNativeListViewReportLink.LV_GetColumnWidth
  (ACol: Integer): Integer;
begin
...
function TCustomdxNativeListViewReportLink.LV_GetIconArrangement
  : TIconArrangement;
const
  IconArrangements: array [Boolean] of TIconArrangement = (iaLeft, iaTop);
begin
  Result := IconArrangements[(GetWindowLong(ListViewHandle, GWL_STYLE) and
    LVS_ALIGNMASK) = 0];
end;

function TCustomdxNativeListViewReportLink.LV_GetIsItemChecked
  (AIndex: Integer): Boolean;
begin
  Result := ListView_GetCheckState(ListViewHandle, AIndex) <> 0;
end;

function TCustomdxNativeListViewReportLink.LV_GetIsItemSelected
  (AIndex: Integer): Boolean;
begin
...
function TCustomdxNativeListViewReportLink.LV_GetItemImageIndex(ACol,
  ARow: Integer): Integer;
...
function TCustomdxNativeListViewReportLink.LV_GetItemStateIndex(ACol,
  ARow: Integer): Integer;
var
  Item: TLVItem;
begin
...
function TCustomdxNativeListViewReportLink.LV_GetItemText(ACol,
  ARow: Integer): string;
const
  BufferLength = 4096;
var
...
const
  ViewStylesMap: array [LVS_ICON .. LVS_LIST] of TViewStyle = (vsIcon, vsReport,
    vsSmallIcon, vsList);
var
  ViewStyle: DWORD;
begin
...
function TCustomdxNativeListViewReportLink.LV_HasColumnHeaderImage
  (ACol: Integer): Boolean;
var
  Column: TLVColumn;
begin
...
begin
  Result := GetWindowLong(ListViewHandle, GWL_STYLE) and LVS_NOCOLUMNHEADER <>
    LVS_NOCOLUMNHEADER;
end;

function TCustomdxNativeListViewReportLink.LV_IsOwnerData: Boolean;
begin
  Result := GetWindowLong(ListViewHandle, GWL_STYLE) and
...
function TCustomdxNativeListViewReportLink.
  HasSupportForInvisibledColumnHeaders: Boolean;
begin
  Result := False;
end;

procedure TCustomdxNativeListViewReportLink.SetShowColumnHeaders
  (Value: Boolean);
begin
end;
...
begin
  Result := HasSmallImages and (SmallImages.Width <> 0) and
    (SmallImages.Height <> 0);
end;

function TCustomdxNativeListViewReportLink.CheckStateImages: Boolean;
begin
  Result := HasStateImages and (StateImages.Width <> 0) and
    (StateImages.Height <> 0);
end;
...
begin
  if FLargeImages.Count = 0 then
    CopyImages(ListView_GetImageList(ListViewHandle, LVSIL_NORMAL),
      FLargeImages);
  Result := FLargeImages;
end;
...
begin
  if FSmallImages.Count = 0 then
    CopyImages(ListView_GetImageList(ListViewHandle, LVSIL_SMALL),
      FSmallImages);
  Result := FSmallImages;
end;
...
begin
  if FStateImages.Count = 0 then
    CopyImages(ListView_GetImageList(ListViewHandle, LVSIL_STATE),
      FStateImages);
  Result := FStateImages;
end;
...
procedure TCustomdxNativeListViewReportLink.AddExtraImage
  (AParent: TdxReportCell; ACol, ARow: Integer; R: TRect);
const
  ItemClasses: array [Boolean] of TdxReportCellDataClass = (TdxReportCellImage,
    TdxReportCellCheck);
var
...
function TCustomdxNativeListViewReportLink.IsExtraImageRequired(ACol,
  ARow: Integer): Boolean;
begin
  Result := IsReportStyle and (ACol = 0) and not IsFixedRow(ARow) and
    CheckStateImages;
end;
...
function TCustomdxNativeListViewReportLink.CanHasStateImage(ACol,
  ARow: Integer): Boolean;
begin
  Result := not LV_AreCheckBoxes and
    (not IsReportStyle or ((ACol = 0) and not IsFixedRow(ARow)));
end;

function TCustomdxNativeListViewReportLink.IsItemChecked(ARow: Integer)
  : Boolean;
begin
  Result := IsReportStyle and not IsFixedRow(ARow) and
    LV_GetIsItemChecked(ARow - Ord(ShowColumnHeaders));
end;
...
procedure TCustomdxNativeListViewReportLink.CalculateInternalItemsViewInfo;
  ...
  begin
    ...
    begin
      FRowCount := (AvailableSiteHeight - FItemSpaceVert)
        div (FRowHeight + FItemSpaceVert);
    ...
    begin
      FColumnCount := (AvailableSiteWidth - FItemSpaceHorz)
        div (FColumnWidth + FItemSpaceHorz);
...
procedure TCustomdxNativeListViewReportLink.CreateExtraInternalItems
  (ATemporaryParent: TdxReportCell);
var
  I: Integer;
  R: TRect;
  LVItemCount: Integer;
...
procedure TCustomdxNativeListViewReportLink.CreateInternalItems(ATemporaryParent
  : TdxReportCell);
var
  R: TRect;
  I: Integer;
...
begin
  ...
  try
    for I := 0 to LV_GetItemCount - 1 do
      if not OnlySelected or (LV_GetSelectedCount = 0) or LV_GetIsItemSelected(I)
      then
      begin
...
function TCustomdxNativeListViewReportLink.GetFlatInternalItemIndex(ACol,
  ARow: Integer): Integer;
begin
  if (LV_GetViewStyle = vsList) or (LV_GetIconArrangement = iaLeft) then
...
procedure TCustomdxNativeListViewReportLink.InitializeInternalItem
  (AnItem: TAbstractdxReportCellData; AnIndex: Integer);
begin
end;

function TCustomdxNativeListViewReportLink.PlaceInternalItem
  (AParent: TdxReportCell; ACol, ARow: Integer; R: TRect)
  : TAbstractdxReportCellData;
var
  Index: Integer;
...
function TCustomdxNativeListViewReportLink.GetInternalItem(Index: Integer)
  : TAbstractdxReportCellData;
begin
...
procedure TCustomdxNativeListViewReportLink.SetHeaderTransparent
  (Value: Boolean);
begin
...
procedure TCustomdxNativeListViewReportLink.SetOptions
  (Value: TdxListViewPaintOptions);
begin
...
procedure TdxNativeListViewReportLink.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
  inherited;
  if (Operation = opRemove) and (AComponent is TWinControl) and
    TWinControl(Component).HandleAllocated and
    (TWinControl(Component).Handle = ListViewHandle) then
    ListViewHandle := 0;
end;

function TdxNativeListViewReportLink.GetDesignerClass
...
procedure TdxNativeListViewReportLink.SetListViewHandle(Value: THandle);
begin
...
begin
  ...
  if IsReportStyle then
    ...
    begin
      ADataItem.Parent.Data :=
        Integer(TObject(ListViewGetItems(CustomListView)
        [ARow - Ord(ShowColumnHeaders)]));
      ADataItem.Data := ACol - 1;
    end;
end;

procedure TCustomdxListViewReportLink.CustomDraw
...
begin
  ...
  with DrawInfo do
    case AttributeID of
      dxListViewHeaderID:
        DoCustomDrawHeader(ACanvas, ABoundsRect, HeaderIndex,
          TdxReportCellImage(AItem), ADone);
      dxListViewItemID:
        DoCustomDrawItem(ACanvas, ABoundsRect, ListItem, SubItem,
          TdxReportCellImage(AItem), ADone);
    end;
end;
...
procedure TCustomdxListViewReportLink.PrepareConstruct
  (AReportCells: TdxReportCells);
begin
...
function TCustomdxListViewReportLink.
  HasSupportForInvisibledColumnHeaders: Boolean;
begin
  Result := True;
end;

procedure TCustomdxListViewReportLink.InitializeInternalItem
  (AnItem: TAbstractdxReportCellData; AnIndex: Integer);
begin
  inherited;
...
procedure TCustomdxListViewReportLink.SetListViewHandle(Value: THandle);
begin
end;
...
procedure TCustomdxListViewReportLink.DoCustomDrawItem(ACanvas: TCanvas;
  ARect: TRect; AListItem: TListItem; ASubItem: Integer;
...
procedure TCustomdxListViewReportLink.GetCustomDrawInfo
...
procedure TCustomdxListViewReportLink.SetOnCustomDrawHeader
  (Value: TdxCustomDrawListViewHeaderEvent);
begin
...
procedure TCustomdxListViewReportLink.SetOnCustomDrawItem
  (Value: TdxCustomDrawListViewItemEvent);
begin
...
begin
  ...
  lichbxShowColumnHeaders.Visible :=
    ReportLink.HasSupportForInvisibledColumnHeaders;
...
begin
  ...
  stTransparentHeaders.Caption := ' ' + cxGetResourceString
    (@sdxHeadersTransparent) + ' ';
...
procedure TdxfmLVReportLinkDesignWindow.PaintPreview(ACanvas: TCanvas;
  R: TRect);
begin
  inherited;
  dxPSDrawGridPreview(ACanvas, R, ReportLink, False,
    ReportLink.ShowColumnHeaders, ScaleFactor);
end;
...
begin
  ...
  chbxIncludeFixed.Enabled := chbxOnlySelected.Enabled and
    chbxOnlySelected.Checked;
...
function TdxfmLVReportLinkDesignWindow.GetReportLink
  : TCustomdxNativeListViewReportLink;
begin
...
procedure TdxfmLVReportLinkDesignWindow.CMDialogChar
...
begin
  ...
  with TcxCheckBox(Sender) do
    if Checked then
      ReportLink.Options := ReportLink.Options +
        [TdxListViewPaintOption(TTagToInt(Tag))]
    else
      ReportLink.Options := ReportLink.Options -
        [TdxListViewPaintOption(TTagToInt(Tag))];
...
procedure TdxfmLVReportLinkDesignWindow.chbxShowColumnHeadersClick
  (Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
begin
  ...
  case TTagToInt(TcxCheckBox(Sender).Tag) of
    ...
    1:
      ReportLink.HeaderTransparent := TcxCheckBox(Sender).Checked;
  end;
  Modified := True;
  UpdatePreview;
end;
...
procedure TdxfmLVReportLinkDesignWindow.stTransparentHeadersClick
  (Sender: TObject);
begin
...
begin
  ...
  case TTagToInt(TcxColorComboBox(Sender).Tag) of
    0:
      ReportLink.Color := AColor;
    1:
      ReportLink.EvenColor := AColor;
    2:
      ReportLink.FixedColor := AColor;
    3:
...
begin
  ...
  begin
    case TTagToInt(TcxButton(Sender).Tag) of
      ...
      1:
        Font := ReportLink.EvenFont;
      2:
        Font := ReportLink.HeaderFont;
    end;
...
procedure TdxfmLVReportLinkDesignWindow.chbxOnlySelectedClick(Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
procedure TdxfmLVReportLinkDesignWindow.chbxIncludeFixedClick(Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
procedure TdxfmLVReportLinkDesignWindow.chbxUse3DEffectsClick(Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
procedure TdxfmLVReportLinkDesignWindow.chbxUseSoft3DClick(Sender: TObject);
begin
  if LockControlsUpdate then
    Exit;
...
dxPSRegisterReportLink(TdxListViewReportLink, TListView,
  TdxfmLVReportLinkDesignWindow);
{$IFDEF REGISTERSHELLCTRLS}
dxPSRegisterReportLink(TdxShellListViewReportLink, TShellListView,
  TdxfmLVReportLinkDesignWindow);
{$ENDIF}
...
{$IFDEF REGISTERSHELLCTRLS}
  dxPSUnregisterReportLink(TdxShellListViewReportLink, TShellListView,
  TdxfmLVReportLinkDesignWindow);
{$ENDIF}
dxPSUnregisterReportLink(TdxListViewReportLink, TListView,
  TdxfmLVReportLinkDesignWindow);
