...
type
  ...
  TdxFieldBasedRichEditMenuItemSimpleCommand = class abstract
  ...
  public
    constructor Create(const AControl: IdxRichEditControl; AField: TdxField);
  ...
  protected
    procedure ExecuteCore; override;
    function CreateField(AForceVisible: Boolean; ASelection: TdxSelection;
      ASelectionStart: TdxDocumentLogPosition): TdxField; virtual;
    procedure UpdateUIStateCore(const AState: IdxCommandUIState); override;
  public
    class function Id: TdxRichEditCommandId; override;
    class function GetDescription: string; override;
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      const AFieldCode: string); reintroduce; overload;
    class function GetDescription: string; override;
    class function GetMenuCaption: string; override;
  ...
  public
    constructor Create(const AControl: IdxRichEditControl;
      const AFieldArgument: string); reintroduce; overload;
    procedure ForceExecute(const AState: IdxCommandUIState); override;
...
procedure TdxShowAllFieldResultsCommand.UpdateUIStateCore
...
procedure TdxShowAllFieldCodesCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
...
constructor TdxFieldBasedRichEditMenuItemSimpleCommand.Create
  (const AControl: IdxRichEditControl; AField: TdxField);
begin
  inherited Create(AControl);
  FField := AField;
end;

procedure TdxFieldBasedRichEditMenuItemSimpleCommand.ForceExecute
  (const AState: IdxCommandUIState);
begin
...
begin
  ...
  APieceTable.FindRunStartLogPosition(AParagraph, ACaretPos.LogPosition,
    ARunIndex);
...
procedure TdxToggleFieldCodesCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      ActivePieceTable.InsertText(ASelectionStart, NewEmptyFieldText,
        AForceVisible);
...
function TdxCreateFieldCommand.CreateField(AForceVisible: Boolean;
  ASelection: TdxSelection; ASelectionStart: TdxDocumentLogPosition): TdxField;
begin
  Result := ActivePieceTable.CreateField(ASelectionStart, ASelection.Length,
    AForceVisible);
end;

procedure TdxCreateFieldCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  AState.Enabled := IsContentEditable;
...
procedure TdxUpdateFieldCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  AState.Enabled := IsContentEditable;
...
function TdxUpdateFieldsCommand.ChangeProperty(const AStart,
...
procedure TdxUpdateFieldsCommand.ModifyDocumentModelCore
  (const AState: IdxCommandUIState);
var
  ARunInfo: TdxRunInfo;
...
begin
  ARunInfo := ActivePieceTable.FindRunInfo
    (DocumentModel.Selection.NormalizedStart,
    Max(1, DocumentModel.Selection.Length));
  ...
  try
    ...
    ActivePieceTable.FieldUpdater.UpdateFields(AFieldsToUpdate,
...
function TdxUpdateFieldsCommand.FindFirstField(ARunIndex: TdxRunIndex)
  : TdxField;
var
  AIndex: Integer;
begin
...
function TdxUpdateFieldsCommand.FindLastFieldIndex
  (ARunIndex: TdxRunIndex): Integer;
begin
  if not TdxAlgorithms1<TdxField>.BinarySearch
    (ActivePieceTable.Fields.InnerList,
...
procedure TdxUpdateFieldsCommand.UpdateUIStateCore(const AState
  : IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  AState.Enabled := IsContentEditable;
...
constructor TdxInsertFieldCoreCommand.Create(const AControl: IdxRichEditControl;
const AFieldCode: string);
begin
  inherited Create(AControl);
...
begin
  ...
  AField := ActivePieceTable.CreateField(AStartPosition,
    Length(GetInsertedText), GetForceVisible);
  UpdateField(AField);
end;
...
class function TdxInsertFieldCommand.GetInsertObjectCommandClass
...
constructor TdxInsertMergeFieldCommand.Create(const AControl
  : IdxRichEditControl; const AFieldArgument: string);
begin
  inherited Create(AControl);
  FFieldName := AFieldArgument;
end;

procedure TdxInsertMergeFieldCommand.ForceExecute(const AState
  : IdxCommandUIState);
var
...
procedure TdxInsertMergeFieldCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  AState.Enabled := IsContentEditable;
...
function TdxInsertMergeFieldCommand.CreateDefaultCommandUIState
  : IdxCommandUIState;
var
...
constructor TdxInsertPageCountFieldCommand.Create(const AControl
  : IdxRichEditControl);
begin
  inherited Create(AControl);
  FieldCode := 'NUMPAGES';
...
procedure TdxInsertPageCountFieldCommand.UpdateUIState
  (const AState: IdxCommandUIState);
begin
...
constructor TdxInsertPageNumberFieldCommand.Create(const AControl
  : IdxRichEditControl);
begin
  inherited Create(AControl);
  FieldCode := 'PAGE';
...
procedure TdxInsertPageNumberFieldCommand.UpdateUIState
  (const AState: IdxCommandUIState);
begin
  inherited UpdateUIState(AState);
  ApplyCommandRestrictionOnEditableControl(AState,
...
procedure TdxShowInsertMergeFieldFormCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
  CheckExecutedAtUIThread;
  AState.Enabled := IsContentEditable and
    DocumentModel.MailMergeDataController.IsReady;
  AState.Visible := True;
  ApplyDocumentProtectionToSelectedCharacters(AState);
end;
...
procedure TdxToggleViewMergedDataCommand.UpdateUIStateCore
  (const AState: IdxCommandUIState);
begin
