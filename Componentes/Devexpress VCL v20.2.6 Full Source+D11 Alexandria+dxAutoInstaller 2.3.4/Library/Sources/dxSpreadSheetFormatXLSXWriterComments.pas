...
{$ENDIF}
  ...
  dxGDIPlusClasses, dxCoreGraphics, dxXMLDoc, dxSpreadSheetContainers,
  dxSpreadSheetFormatXLSXWriter,
  dxSpreadSheetFormatXLSXTags, dxSpreadSheetPackedFileFormatCore,
  dxSpreadSheetClasses, dxSpreadSheetCoreStyles,
  dxXMLWriter;
...
type
  ...
  TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder = class
  ...
  protected
    ...
    procedure WriteComment(AWriter: TdxXmlWriter;
      AComment: TdxSpreadSheetCommentContainer); virtual;
    procedure WriteCommentText(AWriter: TdxXmlWriter;
      AComment: TdxSpreadSheetCommentTextBox); virtual;
  ...
  TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder = class
  ...
  public
    constructor Create(AContainer: TdxSpreadSheetCommentContainer;
      ANode: TdxXMLNode; AOwner: TdxSpreadSheetXLSXWriter;
      AOwnerRels: TdxSpreadSheetXLSXWriterRels);
  ...
  TdxSpreadSheetXLSXWriterWorksheetTableViewLegacyDrawingBuilder = class
  ...
  protected
    ...
    procedure WriteShapes(ANode: TdxXMLNode;
...
uses
  dxSpreadSheetUtils, dxSpreadSheetFormatUtils, AnsiStrings, dxSpreadSheetCore,
  cxGeometry, dxColorPicker,
...
constructor TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.Create
  (AOwner: TdxSpreadSheetXLSXWriterWorksheetTableViewBuilder;
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.
  AfterConstruction;
begin
...
class function TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.
...
class function TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.
  GetContentType: AnsiString;
begin
  Result := sdxXLSXCommentsContentType;
end;

procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.ExecuteCore
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.WriteAuthors
  (AWriter: TdxXmlWriter);
var
  I: Integer;
begin
...
begin
  ...
  try
    ...
    AWriter.WriteAttributeInteger(sdxXLSXAttrAuthorId,
      Authors.IndexOf(AComment.Author));
    AWriter.WriteAttributeString(sdxXLSXAttrRef,
      AComment.Cell.GetReference(False));
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentsBuilder.
  WriteComments(AWriter: TdxXmlWriter);
var
  I: Integer;
begin
...
constructor TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  Create(AContainer: TdxSpreadSheetCommentContainer; ANode: TdxXMLNode;
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  Execute;
var
  ASubNode: TdxXMLNode;
begin
...
function TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  EncodeAnchors: string;
...
begin
  ...
  CalculateAnchor(Container.AnchorPoint1, AContainerBounds.TopLeft,
    AAnchorPoints.TopLeft, AAnchorOffsets.TopLeft);
  CalculateAnchor(Container.AnchorPoint2, AContainerBounds.BottomRight,
    AAnchorPoints.BottomRight, AAnchorOffsets.BottomRight);

  Result := Format('%d, %d, %d, %d, %d, %d, %d, %d',
    [AAnchorPoints.Left, AAnchorOffsets.Left, AAnchorPoints.Top,
    AAnchorOffsets.Top, AAnchorPoints.Right, AAnchorOffsets.Right,
    AAnchorPoints.Bottom, AAnchorOffsets.Bottom]);
end;

function TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  EncodeColor(const AColor: TdxAlphaColor): string;
begin
  Result := '#' + TdxColorHelper.AlphaColorToHexCode(AColor, False, False);
end;

function TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  EncodeOpacity(AAlpha: Byte): string;
begin
  Result := IntToStr(MulDiv(AAlpha, MaxWord, MaxByte)) + 'f';
end;

procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  WriteClientData(ANode: TdxXMLNode);
begin
  ...
  ANode.AddChild(sdxXLSXNodeXTextHAlign).TextAsString :=
    dxXLSXVMLTextAlignHorzMap[Container.TextBox.AlignHorz];
  ANode.AddChild(sdxXLSXNodeXTextVAlign).TextAsString :=
    dxXLSXVMLTextAlignVertMap[Container.TextBox.AlignVert];
  ANode.AddChild(sdxXLSXNodeXRow).TextAsString :=
    IntToStr(Container.Cell.RowIndex);
  ANode.AddChild(sdxXLSXNodeXColumn).TextAsString :=
    IntToStr(Container.Cell.ColumnIndex);
  if Container.Visible then
    ANode.AddChild(sdxXLSXNodeXVisible);
end;

procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  WriteShapeFill(AFill: TdxGPBrush);
const
  AnglesMap: array [TdxGPBrushGradientMode] of Integer = (270, 180, 225, 135);
  TypeMap: array [TdxGPBrushStyle] of string = (sdxXLSXValueSolid,
    sdxXLSXValueGradient, sdxXLSXValueTile, sdxXLSXValueSolid);
var
  AFillNode: TdxXMLNode;
begin
  AFillNode := Node.AddChild(sdxXLSXNodeVMLFill);
  AFillNode.Attributes.SetValueAsString(sdxXLSXAttrTypeLC,
    TypeMap[AFill.Style]);

  case AFill.Style of
    gpbsTexture:
      AFillNode.Attributes.SetValue(sdxXLSXAttrORelID,
        WriteImage(AFill.Texture));

    gpbsSolid:
      begin
        Node.Attributes.SetValueAsString(sdxXLSXAttrFillColor,
          EncodeColor(AFill.Color));
        if dxGetAlpha(AFill.Color) <> MaxByte then
          AFillNode.Attributes.SetValueAsString(sdxXLSXAttrOpacity,
            EncodeOpacity(dxGetAlpha(AFill.Color)));
      end;
    ...
    gpbsGradient:
      ...
      begin
        ...
        AFillNode.Attributes.SetValueAsString(sdxXLSXAttrColor2,
          EncodeColor(AFill.GradientPoints.Colors
          [AFill.GradientPoints.Count - 1]));
        AFillNode.Attributes.SetValueAsString(sdxXLSXAttrOOpacity2,
          EncodeOpacity(dxGetAlpha(AFill.GradientPoints.Colors
          [AFill.GradientPoints.Count - 1])));
        AFillNode.Attributes.SetValueAsInteger(sdxXLSXAttrAngle,
          AnglesMap[AFill.GradientMode]);
      end;
  end;
end;

procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  WriteShapeStroke(AStroke: TdxGPPen);
begin
  Node.Attributes.SetValueAsString(sdxXLSXAttrStrokeColor,
    EncodeColor(AStroke.Brush.Color));
...
begin
  ...
  try
    ...
    ABuilder.Append(Format('margin-left:%0.2fpt;',
      [TdxValueUnitsHelper.PixelsToPoints(AContainerBounds.Left)],
      dxInvariantFormatSettings));
    ABuilder.Append(Format('margin-top:%0.2fpt;',
      [TdxValueUnitsHelper.PixelsToPoints(AContainerBounds.Top)],
      dxInvariantFormatSettings));
    ABuilder.Append(Format('width:%0.2fpt;',
      [TdxValueUnitsHelper.PixelsToPoints(cxRectWidth(AContainerBounds))],
      dxInvariantFormatSettings));
    ABuilder.Append(Format('height:%0.2fpt;',
      [TdxValueUnitsHelper.PixelsToPoints(cxRectHeight(AContainerBounds))],
      dxInvariantFormatSettings));
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.
  WriteTextBox(ANode: TdxXMLNode);
begin
  if not cxRectIsEqual(dxEMUToPixels(dxXLSXDefaultTextPadding),
    Container.TextBox.ContentOffsets) then
  begin
    ANode.Attributes.SetValueAsString(sdxXLSXAttrInset,
      Format('%dmm,%dmm,%dmm,%dmm',
      [TdxValueUnitsHelper.PixelsToMillimeters(Container.TextBox.ContentOffsets.
      Left), TdxValueUnitsHelper.PixelsToMillimeters
      (Container.TextBox.ContentOffsets.Top),
      TdxValueUnitsHelper.PixelsToMillimeters(Container.TextBox.ContentOffsets.
      Right), TdxValueUnitsHelper.PixelsToMillimeters
      (Container.TextBox.ContentOffsets.Bottom)]));
  end;

  if Container.TextBox.AutoSize then
    ANode.Attributes.SetValue(sdxXLSXAttrStyle,
      sdxXLSXMSOFitShapeToText + ':t');

  ANode.AddChild('div').Attributes.SetValueAsString(sdxXLSXAttrStyle,
    'text-align:' + LowerCase(dxXLSXVMLTextAlignHorzMap
    [Container.TextBox.AlignHorz]));
end;

{ TdxSpreadSheetXLSXWriterWorksheetTableViewLegacyDrawingBuilder }

procedure TdxSpreadSheetXLSXWriterWorksheetTableViewLegacyDrawingBuilder.
  Execute;
var
  ADoc: TdxXMLDocument;
  ANode: TdxXMLNode;
...
begin
  ...
  try
    ...
    ANode.Attributes.SetValue('xmlns:o',
      'urn:schemas-microsoft-com:office:office');
    ANode.Attributes.SetValue('xmlns:x',
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewLegacyDrawingBuilder.
  WriteDefaults(ANode: TdxXMLNode);
var
  ASubNode: TdxXMLNode;
begin
...
procedure TdxSpreadSheetXLSXWriterWorksheetTableViewLegacyDrawingBuilder.
  WriteShapes(ANode: TdxXMLNode; ARels: TdxSpreadSheetXLSXWriterRels);
var
  I: Integer;
begin
  for I := 0 to Comments.Count - 1 do
    ExecuteSubTask
      (TdxSpreadSheetXLSXWriterWorksheetTableViewCommentContainerBuilder.Create
      (Comments[I], ANode.AddChild(sdxXLSXNodeVMLShape), Owner, ARels));
end;

function TdxSpreadSheetXLSXWriterWorksheetTableViewLegacyDrawingBuilder.
  GetComments: TList<TdxSpreadSheetCommentContainer>;
begin
