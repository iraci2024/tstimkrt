...
type
  ...
  TcxScheduler = class(TcxCustomScheduler, IUnknown,
    IcxSchedulerCommonViewItemsPainter, IcxSchedulerEventImages,
  ...
  protected
    ...
    function GetNextView(AView: TcxSchedulerCustomView)
      : TcxSchedulerCustomView; override;
  ...
  public
    ...
    procedure SelectDays(const AStartDay, AFinishDay: TDateTime;
      AsDays: Boolean = False); overload; virtual;
    procedure SelectDays(ASelection: TcxSchedulerDateList;
      AsDays: Boolean = False); overload; virtual;
    procedure SelectDays(const ASelection: array of TDateTime;
      AsDays: Boolean = False); overload; virtual;
    procedure SelectWorkDays(const ADate: TDateTime);
  published
    property DateNavigator: TcxSchedulerDateNavigator read GetDateNavigator
      write SetDateNavigator;
    property ViewAgenda: TcxSchedulerAgendaView read FViewAgenda
      write SetViewAgenda;
    property ViewDay: TcxSchedulerDayView read FViewDay write SetViewDay;
    property ViewGantt: TcxSchedulerGanttView read FViewGantt
      write SetViewGantt;
    property ViewTimeGrid: TcxSchedulerTimeGridView read FViewTimeGrid
      write SetViewTimeGrid;
    property ViewWeek: TcxSchedulerWeekView read FViewWeek write SetViewWeek;
    property ViewWeeks: TcxSchedulerWeeksView read FViewWeeks
      write SetViewWeeks;
    ...
    property OnInitEventImages: TcxSchedulerInitEventImages
      read FOnInitEventImages write FOnInitEventImages;
    ...
    // nested events
    property DateNavigatorEvents: TNotifyEvent read FDateNavigatorEvents
      write FDateNavigatorEvents;
    property ViewDayEvents: TNotifyEvent read FViewDayEvents
      write FViewDayEvents;
    property ViewGanttEvents: TNotifyEvent read FViewGanttEvents
      write FViewGanttEvents;
    property ViewTimeGridEvents: TNotifyEvent read FViewTimeGridEvents
      write FViewTimeGridEvents;
    property ViewYearEvents: TNotifyEvent read FViewYearEvents
      write FViewYearEvents;
  end;

implementation

uses
  Math, SysUtils, cxSchedulerRangeControlClientProperties,
  dxSkinscxSchedulerPainter;

type
  TcxSchedulerCustomViewAccess = class(TcxSchedulerCustomView);
  TcxSchedulerRangeControlClientPropertiesAccess = class
    (TcxSchedulerRangeControlClientProperties);
  TcxSchedulerAgendaViewAccess = class(TcxSchedulerAgendaView);
  TcxSchedulerAgendaViewControllerAccess = class
...
begin
  ...
  try
    if SelectedDays.IndexOf(ADate) <> -1 then
      Exit;
    ...
    else if CurrentView is TcxSchedulerTimeGridView then
      TcxSchedulerTimeGridView(CurrentView).VisibleStart := SelectedDays[0];
    UpdateView(CurrentView);
    if CurrentView is TcxSchedulerAgendaView then
      TcxSchedulerAgendaViewControllerAccess
        (TcxSchedulerAgendaViewAccess(CurrentView).Controller)
        .SelectTopInfoCell(False);
...
function TcxScheduler.GoToDate(ADate: TDateTime;
        ...
        begin
          ...
          case AViewMode of
            ...
            begin
              ...
              TcxSchedulerAgendaViewControllerAccess
                (TcxSchedulerAgendaViewAccess(ViewAgenda).Controller)
                .SelectTopInfoCell(False);
            end;
          end;
        end;
...
procedure TcxScheduler.SelectDays(ASelection: TcxSchedulerDateList;
  AsDays: Boolean = False);
var
  I: Integer;
begin
...
begin
  inherited;
  if FRangeControl <> nil then
    TcxSchedulerRangeControlClientPropertiesAccess
      (FRangeControl.ClientProperties).StorageChanged;
end;
...
begin
  ...
  Filer.DefineBinaryProperty('Splitters', ReadSplittersData,
    WriteSplittersData, True);
  Filer.DefineBinaryProperty('StoredClientBounds', ReadStoredClientBounds,
    WriteStoredClientBounds, True);
end;
...
procedure TcxScheduler.DoCustomDrawEvent(AViewInfo
  : TcxSchedulerEventCellViewInfo; var ADone: Boolean);
begin
...
begin
  ...
  begin
    if CurrentView = ViewDay then
      AValue := 0
    else if CurrentView = ViewWeek then
      AValue := 1
    else if CurrentView = ViewWeeks then
      AValue := 2
    else if CurrentView = ViewTimeGrid then
      AValue := 3
    else if CurrentView = ViewYear then
      AValue := 4
    else if CurrentView = ViewGantt then
      AValue := 5
    else if CurrentView = ViewAgenda then
      AValue := 6
  end;
end;
...
begin
  if not(cfViewValid in ControlFlags) then
    Exit;
  try
...
procedure TcxScheduler.DoRangeChanged;
  ...
  begin
    Result := CurrentView;
    AProperties := FRangeControl.ClientProperties as
      TcxSchedulerRangeControlClientProperties;
    if not(aaClient in AProperties.AutoAdjustments) then
      Exit;
    if not TcxSchedulerCustomViewAccess(CurrentView).CanDeactivateOnDateNavigatorSelectionChange
    then
      Exit;
    AMinDayOfWeek := dxDayOfWeek(AMin);
    AIsStartWeek := ((AProperties.ActualPrimaryScale.ScaleUnit = rcduDay) and
      (ViewWeek.StartOfWeek = AMinDayOfWeek) or
      (AProperties.ActualPrimaryScale.ScaleUnit = rcduWeek) and
      ((dxGetStartOfWeek = AMinDayOfWeek) or
      (ViewWeek.StartOfWeek = AMinDayOfWeek)));
    AInterval := DaysBetween(AMax, AMin);
    if ViewWeek.CanShow and (AInterval = 7) and AIsStartWeek then
      Result := ViewWeek
    else if ViewWeeks.CanShow and (AInterval >= 7) and (AInterval mod 7 = 0) and AIsStartWeek
    then
      Result := ViewWeeks
    else if ViewDay.CanShow then
      Result := ViewDay;
  end;
  ...
  procedure GetRangeControlSelectedRange(ANewView: TcxSchedulerCustomView;
    var AMin, AMax: TDateTime);
  var
    AProperties: TcxSchedulerRangeControlClientProperties;
  ...
  begin
    ...
    begin
      ...
      if AScale.GetDateInterval(AMin, AMax) >
        AProperties.ActualPrimaryScale.GetDateInterval(AMin, AMax) then
        AScale := AProperties.ActualPrimaryScale;
      (CurrentView as TcxSchedulerTimeGridView).VisibleStart :=
        AScale.RoundDate(AMin);
    ...
    else if ViewYear.Active then
    begin
      if ViewYear.Scale = 3 then
      begin
        AMin := TdxRangeControlDateTimeClientHelper.RoundDate(AMin,
          rcduQuarter);
        GoToDate(AMin);
        AMax := TdxRangeControlDateTimeClientHelper.IncDate(AMin, rcduQuarter);
      end
      else if ViewYear.Scale = 6 then
      begin
        AMin := TdxRangeControlDateTimeClientHelper.RoundDate(AMin,
          rcduQuarter);
        GoToDate(AMin);
        TcxSchedulerCustomViewAccess(CurrentView).GetRangeControlRange
          (AMin, AMax);
        AMax := TdxRangeControlDateTimeClientHelper.IncDate(AMin,
          rcduQuarter, 2);
      end
      else
      begin
    ...
    else if ViewAgenda.Active then
    begin
      if AMin < AMax then
      ...
      TcxSchedulerCustomViewAccess(CurrentView).GetRangeControlRange
        (AMin, AMax);
      AMax := TdxRangeControlDateTimeClientHelper.CeilDate
        (IncSecond(AMax), rcduDay);
    end
    else
    begin
      if ANewView = ViewWeek then
        SelectWeek(AMin)
      else if ANewView = ViewWeeks then
...
begin
  AScales :=
    (FRangeControl.ClientProperties as
    TcxSchedulerRangeControlClientProperties).Scales;
  Result := AScales.Day;
  if ViewTimeGrid.Active or ViewGantt.Active then
  begin
    AScaleList := TdxRangeControlDateTimeScaleList.Create;
    TcxSchedulerCustomViewAccess(CurrentView).GetRangeControlScales(AScales,
      AScaleList);
  ...
  else if ViewWeek.Active or ViewWeeks.Active then
    Result := AScales.Week;
end;

function TcxScheduler.GetNextView(AView: TcxSchedulerCustomView)
  : TcxSchedulerCustomView;
begin
  Result := FViewWeek;
  if (AView = FViewWeek) and FViewWeeks.CanShow then
    Result := FViewWeeks
  else if not FViewWeek.CanShow or (AView = FViewWeek) then
  begin
...
begin
  ...
  SelectDays(SelectedDays, TcxSchedulerCustomViewAccess(CurrentView)
    .CanSelectIndividualDays);
...
begin
  ...
  begin
    FIsSelectionValidated := True;
    SelectDays(ASelection, (CurrentView = ViewDay) or (CurrentView = ViewYear)
      or (CurrentView = ViewTimeGrid) or (SelectedDays.Count = 1));
  end
  else
  begin
...
begin
  ...
  try
    ...
    begin
      DecodeDateWeek(SelectedDays[I] + 1 - Byte(StartOfWeek), AYear, AWeek,
        AWeekDay);
    ...
    begin
      if ASaveWeekCount and (I = AWeekCount) then
        break;
      ADate := AWeeks[I];
      for J := 0 to 6 do
...
procedure TcxScheduler.SynchronizeRangeControl;
  ...
  begin
    ...
    else if not ViewYear.Active then
    // (ViewDay, ViewAgenda, ViewWeek, ViewWeeks)
      AMax := TdxRangeControlDateTimeClientHelper.CeilDate
        (IncSecond(AMax), rcduDay);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        TcxSchedulerCustomViewAccess(CurrentView).GetRangeControlTotalRange
          (ATotalMin, ATotalMax);
        ...
        try
          TcxSchedulerCustomViewAccess(CurrentView).GetRangeControlScales
            (AProperties.Scales, AAutoAdjustScaleList);
