...
uses
  ...
  dxCustomTileControl, dxTileControl, dxTileBar, cxLibraryReg, cxPropEditors,
  cxGraphics;

const
  dxTileBarEditorVerbs: array [0 .. 2] of string = ('A&dd Regular Item',
    'Add &Large Item', 'Add Group');
  dxTileControlEditorVerbs: array [0 .. 4] of string = ('Add Small Item',
    'Add Regular Item', 'Add Large Item', 'Add Extra-Large Item', 'Add Group');
...
type
  ...
  protected
    function CheckCandidate(ACandidate, ATileControl: TComponent)
      : Boolean; virtual;
  ...
  private
    procedure CreateTileControlItem(const ASize
  ...
  TdxTileControlDesignHelper = class(TdxTileControlCustomDesignHelper, IUnknown,
...
function TdxTileControlActionBarItemImageIndexProperty.GetActionBarItem
  : TdxTileControlActionBarItem;
begin
  Result := GetComponent(0) as TdxTileControlActionBarItem;
end;

function TdxTileControlActionBarItemImageIndexProperty.GetImages
  : TCustomImageList;
begin
...
function TdxTileControlItemCustomGlyphImageIndexProperty.GetGlyph
  : TdxTileControlItemCustomGlyph;
begin
  Result := GetComponent(0) as TdxTileControlItemCustomGlyph;
end;

function TdxTileControlItemCustomGlyphImageIndexProperty.GetImages
  : TCustomImageList;
begin
...
function TdxTileControlItemDetailProperty.CheckCandidate(ACandidate,
  ATileControl: TComponent): Boolean;
begin
  Result := (ACandidate is TWinControl) and (ATileControl <> ACandidate) and
    ((TWinControl(ACandidate).Parent is TScrollingWinControl) or
    (ACandidate is TScrollingWinControl));
end;

function TdxTileControlItemDetailProperty.GetCandidate(const AName: string)
  : TComponent;
begin
...
procedure TdxTileControlComponentEditor.CreateTileControlItem
  (const ASize: TdxTileControlItemSize = tcisRegular);
var
...
procedure TdxTileBarComponentEditor.CreateTileBarItem
  (const ASize: TdxTileBarItemSize = tbisRegular);
var
...
function TdxTileBarFocusedItemProperty.CheckCandidate(ACandidate,
  ATileControl: TComponent): Boolean;
begin
  Result := (ACandidate is TdxTileBarItem) and
    (TdxTileBarItem(ACandidate).TileBar = ATileControl) and
    TdxTileBarItem(ACandidate).ActuallyVisible and
    TdxTileBarItem(ACandidate).Enabled;
end;
...
function TdxTileBarItemDetailProperty.CheckCandidate(ACandidate,
  ATileControl: TComponent): Boolean;
begin
  Result := (ACandidate is TWinControl) and (ATileControl is TdxCustomTileBar)
    and inherited CheckCandidate(ACandidate, ATileControl) and
    (TWinControl(ACandidate) <> TdxTileBarItem(GetTileItem)
    .PopupOptions.PopupControl);
end;
...
function TdxTileBarItemPopupControlProperty.CheckCandidate(ACandidate,
  ATileControl: TComponent): Boolean;
begin
  Result := (ACandidate is TWinControl) and (ATileControl is TdxCustomTileBar)
    and inherited CheckCandidate(ACandidate, ATileControl) and
    (TWinControl(ACandidate) <> TdxTileBarItem(GetTileItem)
    .DetailOptions.DetailControl);
end;
...
procedure TdxTileControlDesignHelper.CreateItemClickHandler
  (AItem: TdxTileControlItem);
const
  MethodParams: array [0 .. 0] of TMethodParam = ((Flags: [pfAddress];
    Name: 'Sender'; TypeName: 'TdxTileControlItem'));
begin
  if AItem <> nil then
    ShowEventMethod(FDesignHelper.Designer, AItem, 'OnClick',
      AItem.Name + 'Click', MethodParams);
end;

function TdxTileControlDesignHelper.IsObjectSelected
...
function TdxTileControlDesignHelper.QueryInterface(const IID: TGUID; out Obj)
  : HResult; stdcall;
begin
...
begin
  ...
  RegisterNoIcon([TdxTileControlGroup, TdxTileControlItemFrame,
    TdxTileControlItem, TdxTileControlActionBarItem, TdxTileBarItem]);
  RegisterClasses([TdxTileControl, TdxTileControlGroup, TdxTileControlItemFrame,
    TdxTileControlItem, TdxTileControlActionBarItem, TdxTileControlItemGlyph,
    TdxTileControlItemFrameGlyph, TdxTileBarItem]);
  ...
  RegisterPropertyEditor(TypeInfo(TWinControl), TdxTileControlItemDetailOptions,
    'DetailControl', TdxTileControlItemDetailProperty);
  RegisterPropertyEditor(TypeInfo(TWinControl), TdxTileBarItemDetailOptions,
    'DetailControl', TdxTileBarItemDetailProperty);
  RegisterPropertyEditor(TypeInfo(TWinControl), TdxTileBarItemPopupOptions,
    'PopupControl', TdxTileBarItemPopupControlProperty);
  RegisterPropertyEditor(TypeInfo(TdxTileBarItem), TdxTileBar, 'FocusedItem',
    TdxTileBarFocusedItemProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxTileControlItemCustomGlyph,
    'ImageIndex', TdxTileControlItemCustomGlyphImageIndexProperty);
  RegisterPropertyEditor(TypeInfo(TcxImageIndex), TdxTileControlActionBarItem,
    'ImageIndex', TdxTileControlActionBarItemImageIndexProperty);
  // RegisterPropertyEditor(TypeInfo(Integer), TdxTileControlItem, 'GroupIndex', nil);
  RegisterPropertyEditor(TypeInfo(Integer), TdxTileControlItem,
    'IndexInGroup', nil);
  RegisterPropertyEditor(TypeInfo(TColor), TdxTileControlStyle,
    'BorderColor', nil);
  //
  RegisterPropertyEditor(TypeInfo(Integer), TdxTileBarOptionsView,
    'GroupBlockMaxColumnCount', nil);
  RegisterPropertyEditor(TypeInfo(TdxTileControlGroupLayout),
    TdxTileBarOptionsView, 'GroupLayout', nil);
  RegisterPropertyEditor(TypeInfo(Integer), TdxTileBarOptionsView,
    'GroupMaxRowCount', nil);
  RegisterPropertyEditor(TypeInfo(string), TdxTileBarItemDetailOptions,
    'Caption', nil);
  RegisterPropertyEditor(TypeInfo(Boolean), TdxTileBarItemDetailOptions,
    'ShowTab', nil);
end;
