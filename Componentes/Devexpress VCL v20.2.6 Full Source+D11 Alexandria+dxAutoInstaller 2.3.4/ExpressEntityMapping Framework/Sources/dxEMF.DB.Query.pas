...
type
  ...
  IQueryCriteriaToStringCriteriaVisitor = interface
  ...
  IQueryCriteriaToStringListVisitor = interface(IdxQueryCriteriaVisitor < TList
    < string >> )
  ...
  public
    ...
    function ConsolidateParameter(const ADeserializedParameter: IdxOperandValue)
      : IdxOperandValue;
    procedure ConsolidateIdentity(const AIdentityInsertParameter
      : IdxParameterValue);
  end;
  ...
  public
    constructor Create(const AColumnName: string; const ANodeAlias: string;
      ASortDirection: TdxSortDirection); overload;
  ...
  TdxSubQueriesFinder = class(TcxIUnknownObject, IdxCriteriaVisitor,
    IdxQueryCriteriaVisitor)
  strict private
    FResult: TdxJoinNodeCollection;
  public
    constructor Create;
    class function FindSubQueries(const ACriteria: IdxCriteriaOperator)
      : TdxJoinNodeCollection; static;
    procedure Process(const ACriteria: IdxCriteriaOperator); overload;
    procedure Process(const ACriteria
  ...
  protected
    class function GetHashCode(AObject: TObject): Integer; reintroduce;
      overload; static;
    procedure CollectJoinNodesAndCriteriaInternal(ANodes: TdxJoinNodeList;
      ACriteria: TdxCriteriaOperatorList); virtual;
  public
    constructor Create; overload;
    constructor Create(ATable: TdxDBTable; const AAlias: string;
      AType: TdxJoinType); overload;
    destructor Destroy; override;

    function GetColumn(const AName: string): TdxDBColumn;
    function Equals(AObject: TObject): Boolean; override;
    function GetHashCode: Integer; overload; override;
    procedure CollectJoinNodesAndCriteria(out ANodes: TdxJoinNodeList;
      out ACriteria: TdxCriteriaOperatorList);
  ...
  protected
    procedure Notify(const Value: TdxJoinNode;
  ...
  protected
    procedure CollectJoinNodesAndCriteriaInternal(ANodes: TdxJoinNodeList;
      ACriteria: TdxCriteriaOperatorList); override;
  public
    ...
    class function GetTableNames(const AStatements: TArray<TdxBaseStatement>)
  ...
  strict private
    ...
    function GetSortProperties: TdxQuerySortingCollection;
    {$IFDEF DELPHIXE3}inline; {$ENDIF}
    function GetGroupProperties: TdxCriteriaOperatorList;
    {$IFDEF DELPHIXE3}inline; {$ENDIF}
  protected
    procedure CollectJoinNodesAndCriteriaInternal(ANodes: TdxJoinNodeList;
      ACriteria: TdxCriteriaOperatorList); override;
  public
    ...
    property GroupCondition: IdxCriteriaOperator read FGroupCondition
      write FGroupCondition;
  end;
  ...
  public
    constructor Create(ATable: TdxDBTable; const AAlias: string);
    destructor Destroy; override;
    property AffectedRecordCount: Integer read FAffectedRecordCount
      write FAffectedRecordCount;
  ...
  public
    property IdentityParameter: IdxParameterValue read FIdentityParameter
      write FIdentityParameter;
    property IdentityColumn: string read FIdentityColumn write FIdentityColumn;
    property IdentityColumnType: TdxDBColumnType read FIdentityColumnType
      write FIdentityColumnType;
  end;
  ...
  public
    ...
    constructor Create(const ASQLText: string;
      ...
      const AParameterNames: TArray<string>); overload;
    constructor Create(const ASQLText: string;
      AParameters: TdxQueryParameterCollection;
      const AParameterNames: TArray<string>;
      ATopSelectedRecords: Integer); overload;
    constructor Create(const ASQLText: string;
      const AParameters: TArray<IdxOperandValue>;
      const AParameterNames: TArray<string>; ASkipSelectedRecords: Integer;
      ATopSelectedRecords: Integer); overload;
    constructor Create(const ASQLText: string;
      AParameters: TdxQueryParameterCollection;
      const AParameterNames: TArray<string>; ASkipSelectedRecords: Integer;
    ...
    constructor Create(const ASQLText: string;
      AParameters: TdxQueryParameterCollection;
      const AParameterNames: TArray<string>; ATopSelectedRecords: Integer;
      AConstantValues: TDictionary<Integer, IdxOperandValue>;
    ...
    constructor Create(const ASQLText: string;
      ...
      AOperandIndexes: TDictionary<Integer, Integer>); overload;
    destructor Destroy; override;
    procedure AppendParameter(const AName: string;
      const AParameter: IdxOperandValue);
    function ParameterValueByName(const AName: string): IdxOperandValue;

    property ConstantValues: TDictionary<Integer, IdxOperandValue>
      read FConstantValues;
  ...
  TdxQuerySubQueryContainer = class(TdxCriteriaOperator,
    IdxQuerySubQueryContainer)
  strict private
  ...
  public
    ...
    property AggregateProperty: IdxCriteriaOperator read FAggregateProperty
      write FAggregateProperty;
  end;
...
constructor TdxQueryParameterCollection.Create(const AValues
  : array of IdxOperandValue);
begin
  inherited Create;
  AddRange(AValues);
end;
...
class function TdxQueryParameterCollection.GetEmpty
  : TdxQueryParameterCollection;
begin
  if FEmpty = nil then
...
function TdxTaggedParameterHolder.ConsolidateParameter
  (const ADeserializedParameter: IdxOperandValue): IdxOperandValue;
var
...
procedure TdxTaggedParameterHolder.ConsolidateIdentity
  (const AIdentityInsertParameter: IdxParameterValue);
{$IFOPT C+}
var
...
{$IFOPT C+}
  ...
  Assert((AConsolidated as TdxCriteriaOperator)
    = (AIdentityInsertParameter as TdxCriteriaOperator));
{$ELSE}
...
constructor TdxSortingColumn.Create(const AProperty: IdxCriteriaOperator;
...
constructor TdxSortingColumn.Create(const AColumnName: string;
  const ANodeAlias: string; ASortDirection: TdxSortDirection);
begin
...
class function TdxSubQueriesFinder.FindSubQueries(const ACriteria
  : IdxCriteriaOperator): TdxJoinNodeCollection;
var
...
procedure TdxSubQueriesFinder.Process(const ACriteria
...
constructor TdxJoinNode.Create(ATable: TdxDBTable; const AAlias: string;
  AType: TdxJoinType);
begin
  Create;
  FType := AType;
...
begin
  ...
  if AAnother = nil then
    Exit(False);
  Result := (FAlias = AAnother.FAlias) and (FType = AAnother.FType) and
    (FTable = AAnother.FTable) and (FCondition = AAnother.FCondition) and
    FSubNodes.Equals(AAnother.FSubNodes);
end;
...
procedure TdxJoinNode.CollectJoinNodesAndCriteriaInternal
  (ANodes: TdxJoinNodeList; ACriteria: TdxCriteriaOperatorList);
var
...
procedure TdxJoinNode.CollectJoinNodesAndCriteria(out ANodes: TdxJoinNodeList;
  out ACriteria: TdxCriteriaOperatorList);
begin
...
begin
  Result := Format(#9'%s join "%s" %s on %s'#10'%s',
    [TdxFormatterHelper.GetName(FType), FTable.Name, FAlias,
    (Condition as TdxCriteriaOperator).ToString, FSubNodes.ToString]);
end;
...
procedure TdxJoinNodeCollection.Notify(const Value: TdxJoinNode;
...
begin
  ...
  if AAnother = nil then
    Exit(False);
  Result := inherited Equals(AAnother) and
    Equals(FOperands, AAnother.FOperands);
end;
...
class function TdxBaseStatement.GetTableNames(const AStatements
  : TArray<TdxBaseStatement>): TArray<string>;
var
...
procedure TdxBaseStatement.CollectJoinNodesAndCriteriaInternal
  (ANodes: TdxJoinNodeList; ACriteria: TdxCriteriaOperatorList);
var
...
begin
  ...
  begin
    ACriteria.Add(C);
    for ANextNode in TdxSubQueriesFinder.FindSubQueries
      (C as TdxCriteriaOperator) do
      ANextNode.CollectJoinNodesAndCriteriaInternal(ANodes, ACriteria);
  end;
end;
...
begin
  ...
  try
    ...
    AResult.Append(#10'  from "').Append(Table).Append('" ').Append(Alias)
      .Append(#10).Append(SubNodes.ToString);
    if Condition <> nil then
      AResult.Append(' where ')
        .Append(TdxCriteriaOperator.ToString(Condition as TdxCriteriaOperator));
    Result := AResult.ToString;
  finally
    AResult.Free;
  end;
...
procedure TdxSelectStatement.CollectJoinNodesAndCriteriaInternal
  (ANodes: TdxJoinNodeList; ACriteria: TdxCriteriaOperatorList);
var
...
constructor TdxModificationStatement.Create(ATable: TdxDBTable;
  const AAlias: string);
begin
...
begin
  Create(ASQLText, AParameters, AParameterNames, ASkipSelectedRecords,
    ATopSelectedRecords);
...
begin
  Create(ASQLText, AParameters.ToArray, AParameterNames, ASkipSelectedRecords,
    ATopSelectedRecords);
end;
...
begin
  Create(ASQLText, AParameters, AParameterNames, 0, ATopSelectedRecords,
    AConstantValues, AOperandIndexes);
end;
...
procedure TdxQuery.AppendParameter(const AName: string;
  const AParameter: IdxOperandValue);
var
  I: Integer;
begin
...
constructor TdxQuerySubQueryContainer.Create(ANode: TdxBaseStatement;
  const AAggregateProperty: IdxCriteriaOperator;
...
function TdxQuerySubQueryContainer.Accept(const AVisitor
...
function TdxQuerySubQueryContainer.Accept(const AVisitor
  : IdxCriteriaOperatorVisitor): IdxCriteriaOperator;
begin
...
function TdxQuerySubQueryContainer.GetAggregateFunctionType
...
begin
  ...
  if AAnother = nil then
    Exit(False);
  Result := (TdxCriteriaOperator.Equals(Node, AAnother.Node) and
    TdxCriteriaOperator.Equals(AggregateProperty, AAnother.AggregateProperty))
    and (AggregateFunctionType = AAnother.AggregateFunctionType);
end;
...
begin
  ...
  else if AggregateProperty <> nil then
