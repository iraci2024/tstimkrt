...
type
  ...
  protected
    class function CreateForeignKeyColumns(AEntityInfo: TdxEntityInfo;
      const AColumnName: string): TArray<TdxDBColumn>; static;
    class function GetPrimaryKeyTable(AEntityInfo: TdxEntityInfo;
      AColumns: TList<TdxDBColumn>): string; static;
    class function CreateColumn(AMemberInfo: TdxMappingMemberInfo;
      const AName: string): TArray<TdxDBColumn>; static;
    class procedure ProcessMembers(ATable: TdxDBTable;
      AEntityInfo: TdxEntityInfo); static;
    class function ProcessMemberColumns(ATable: TdxDBTable;
      AEntityInfo: TdxEntityInfo; AMemberInfo: TdxMappingMemberInfo)
      : TArray<TdxDBColumn>; static;
    class procedure ProcessPrimaryKey(ATable: TdxDBTable;
      AEntityInfo: TdxEntityInfo; AColumns: TList<TdxDBColumn>);
      overload; static;
    class procedure ProcessPrimaryKey(ATable: TdxDBTable;
      AEntityInfo: TdxEntityInfo); overload; static;
    class procedure ProcessForeignKey(ATable: TdxDBTable;
      AAssociationEntityInfo: TdxEntityInfo;
      const AColumns: TArray<TdxDBColumn>); static;
    class procedure ProcessIndexes(ATable: TdxDBTable;
      AEntityInfo: TdxEntityInfo); static;
  public
    class function ProcessEntityInfo(AEntityInfo: TdxEntityInfo)
      : TObjectList<TdxDBTable>; overload; static;
    class function ProcessClassInfo(ATable: TdxDBTable;
      AEntityInfo: TdxEntityInfo): TdxEntityInfo; static;
  end;
...
function DiscriminatorTypeAsDBType(ADiscriminatorType: TdxDiscriminatorType)
  : TdxDBColumnType;
begin
...
class function TdxDBTableHelper.ProcessEntityInfo(AEntityInfo: TdxEntityInfo)
...
begin
  AForeignKey := TdxDBForeignKey.Create(AColumns,
    AAssociationEntityInfo.ClassAttributes.TableName,
    AAssociationEntityInfo.KeyAttributes.GetColumnsNames);
  ATable.ForeignKeys.Add(AForeignKey);
end;

class procedure TdxDBTableHelper.ProcessIndexes(ATable: TdxDBTable;
...
begin
  AIsOwnTable := AEntityInfo.ClassAttributes.IsInheritance and
    (AEntityInfo.ClassAttributes.MapInheritance =
    TdxMapInheritanceType.OwnTable);
  ...
  repeat
    ...
    begin
      AIndex := TdxDBIndex.Create(AEntityInfo.ClassAttributes.Indexes[I]
        .Columns, AEntityInfo.ClassAttributes.Indexes[I].IsUnique);
    ...
    AEntityInfo := EntityManager.GetEntityInfo
      ((AEntityInfo.ClassAttributes.RttiType as TRttiInstanceType)
...
begin
  ...
  begin
    ...
    Result := CreateForeignKeyColumns(AAssociationEntityInfo,
      AMemberInfo.ColumnName);
...
class procedure TdxDBTableHelper.ProcessMembers(ATable: TdxDBTable;
  AEntityInfo: TdxEntityInfo);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      AIsParentTableMember := AMemberParentEntityInfo.ClassAttributes.
        IsParentTable;
    end;
    ...
    if not AMemberInfo.IsKey then
      ...
      begin
        if AIsParentTableMember then
          AIsNullable := True;
        if (AMemberInfo.Member.Parent <> AEntityInfo.ClassAttributes.RttiType)
          and not AIsParentTable then
          Continue;
      end;
  ...
  repeat
    ...
    begin
      ADBColumn := TdxDBColumn.Create(ADiscriminator.ColumnName, False, '',
        ADiscriminator.ColumnSize,
        DiscriminatorTypeAsDBType(ADiscriminator.DiscriminatorType));
...
begin
  ...
  if (AColumns.Count = 1) and not(AEntityInfo.ClassAttributes.IsInheritance and
    (AEntityInfo.ClassAttributes.MapInheritance = TdxMapInheritanceType.
    OwnTable)) and AEntityInfo.KeyProperty.IsIdentity then
...
begin
  ...
  begin
    ...
    begin
      NotImplemented;
      AResult := TdxArray<TdxDBColumn>.Create
        (Length(AMemberInfo.ReferenceType.KeyProperty.SubMembers));
  ...
  ADBColumn.IsNullable := AIsNullable or AMemberInfo.IsNullable or
    (ADBColumnType = TdxDBColumnType.ByteArray) or AMemberInfo.IsBLOB;
  ADBColumn.Generator := AMemberInfo.DBGenerator;
  Result[0] := ADBColumn;
end;

class function TdxDBTableHelper.CreateForeignKeyColumns
  (AEntityInfo: TdxEntityInfo; const AColumnName: string): TArray<TdxDBColumn>;
var
...
class function TdxDBTableHelper.GetPrimaryKeyTable(AEntityInfo: TdxEntityInfo;
  AColumns: TList<TdxDBColumn>): string;

  function FindMemberEntity(const AColumnName: string;
  ...
  begin
    ...
    if Result = nil then
      Exit;
    if AEntityInfo.ParentEntity.ClassAttributes.PersistentClass <>
      AEntityInfo.ClassAttributes.PersistentBaseClass then
      Result := EntityManager.GetEntityInfo
