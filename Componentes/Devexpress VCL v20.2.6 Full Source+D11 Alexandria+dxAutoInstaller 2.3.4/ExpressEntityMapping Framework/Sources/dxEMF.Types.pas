...
type
  ...
  TdxFunc<T1, T2, TResult> = reference to function(const AArg1: T1;
    const AArg2: T2): TResult;
  TdxFunc<T1, T2, T3, TResult> = reference to function(const AArg1: T1;
    const AArg2: T2; const AArg3: T3): TResult;
  TdxFunc<T1, T2, T3, T4, TResult> = reference to function(const AArg1: T1;
    const AArg2: T2; const AArg3: T3; const AArg4: T4): TResult;
...
{$HPPEMIT 'template<typename T> __interface IdxQueryProvider__1;'}
  ...
  TdxSortDirection = (Ascending, Descending);
  ...
  TdxAutoCreateOption = (None, DatabaseAndSchema, SchemaOnly,
    SchemaAlreadyExists);
  ...
  TdxFunctionOperatorType = (None, Custom, CustomNonDeterministic, Iif, IsNull,
    IsNullOrEmpty, Trim, Len, Substring, UpperCase, LowerCase, Concat, Ascii,
    Char, ToString, Replace, Reverse, Insert, CharIndex, Remove, Abs, Sqr, Cos,
    Sin, ArcTan, Exp, Log, Random, Tan, Power, Sign, Round, Ceil,
    Floor, Max, Min,

    ArcCos, ArcSin, ArcTan2, BigMul, Cosh, Log10, Sinh, Tanh, PadLeft, PadRight,
    StartsWith, EndsWith, Contains, ToInteger, ToInt64, ToSingle, ToDouble,
    ToDecimal,
    ...
    IsLastYear, IsYearToDate,

    IsSameDay,

    IsJanuary, IsFebruary, IsMarch, IsApril, IsMay, IsJune, IsJuly, IsAugust,
    IsSeptember, IsOctober, IsNovember, IsDecember,

    DateDiffTicks, MillisecondsBetween, SecondsBetween, MinutesBetween,
    HoursBetween, DaysBetween, MonthsBetween, YearsBetween,

    DateOf, MillisecondOf, SecondOf, MinuteOf, HourOf, DayOf, MonthOf, YearOf,
    DayOfTheWeek, DayOfTheYear, GetTimeOfDay, Now, UTCNow, Today,

    AddTimeSpan, IncTick, IncMillisecond, IncSecond, IncMinute, IncHour, IncDay,
    IncMonth, IncYear);

  { TdxBinaryOperatorType }

  TdxBinaryOperatorType = (Equal, NotEqual, Greater, Less, LessOrEqual,
    GreaterOrEqual, BitwiseAnd, BitwiseOr, BitwiseXor, Divide, Modulo, Multiply,
    Plus, Minus);
  ...
  TdxDBColumnType = (Unknown, Boolean, Byte, SByte, Char, Decimal, Double,
    Single, Int32, UInt32, Int16, UInt16, Int64, UInt64, &String, DateTime,
    Guid, ByteArray, TimeSpan);
  ...
  TdxAttribute = (
    ...
    // Column attributes
    Column, NonPersistent, Association, Aggregated, Size, ReadOnly, Nullable,
    Default, Key, Indexed, Unique, NoForeignKey, Blob, Delayed, Generator,
    VirtualColumn);
  ...
  TdxOptimisticLockingBehavior = (NoLocking, ConsiderOptimisticLockingField,
    LockModified, LockAll);

  { TdxGeneratorType }

  TdxGeneratorType = (None, Identity, Guid, SequentialGUID, Sequence);
  ...
  public
    ...
    constructor Create(AGeneratorType: TdxGeneratorType;
