...
type
  ...
  protected
    ...
    procedure PrepareParameters(AConvertParameter
      : TdxConvertParameter); override;
  public
...
{$REGION 'ODBC helper'}
    ...
    type
      TSQLConfigDataSource = function(hwndParent: HWND; fRequest: WORD;
        lpszDriver: LPCWSTR; lpszAttributes: LPCWSTR): BOOL; stdcall;
    ...
    public
      ...
      class property SQLConfigDataSource: TSQLConfigDataSource
        read GetSQLConfigDataSource;
    end;
{$ENDREGION}
  strict private
  const
    SchemaInfo: array [TdxSchemaInfo] of TSchemaInfo = (siTables, siViews,
      siProcedures, siColumns, siIndexes, siIndexes, siPrimaryKeys,
      siForeignKeys, siForeignKeys);
  strict private
    ...
    procedure GetColumns(ATable: TdxDBTable;
      const ASchemaName, ATableName: string);
    ...
    procedure GetForeignKeys(ATable: TdxDBTable;
      const ASchemaName, ATableName: string);
  protected
    class procedure AssignParameterVariant(const AParameter: TParameter;
      const AValue: Variant); static;
    class procedure AssignParameterValue(const AParameter: TParameter;
    ...
    function SelectData(AOwner: TComponent; AQuery: TdxQuery)
      : TDataSet; override;
  public
...
function SQLAllocConnect(EnvironmentHandle: Pointer;
  var ConnectionHandle: Pointer): SmallInt; stdcall; external odbcdll;
{$EXTERNALSYM SQLAllocConnect}
function SQLAllocEnv(var EnvironmentHandle: Pointer): SmallInt; stdcall;
  external odbcdll;
{$EXTERNALSYM SQLAllocEnv}
function SQLDataSourcesW(EnvironmentHandle: Pointer; Direction: WORD;
  ServerName: PChar; BufferLength1: SQLSMALLINT;
  var NameLength1Ptr: SQLSMALLINT; Description: PChar;
  BufferLength2: SQLSMALLINT; var NameLength2Ptr: SQLSMALLINT): SmallInt;
  stdcall; external odbcdll;
...
type
  ...
  strict private
    ...
    function GetFields(AMemberInfo: TdxMappingMemberInfo;
      var AFieldIndex: Integer): TArray<TdxADOField>;
    function ToValues(const AFields: TArray<TdxADOField>;
...
begin
  ...
  begin
    ...
    begin
      ...
      ARefObject := TdxEMFCustomSessionAccess(ASession)
        .FindBatch(AMemberInfo.ReferenceType.ClassAttributes.
        PersistentClass, AKeys);
    ...
    begin
      if AFields <> nil then
        if Assigned(AMemberInfo.OnSetter) then
          ...
          TdxFieldValueConverter.SetValue(AInstance, ToVariants(AFields),
            AMemberInfo);
    end;
  end;
end;
...
function TdxADOLoader.GetFields(AMemberInfo: TdxMappingMemberInfo;
  var AFieldIndex: Integer): TArray<TdxADOField>;
var
  I: Integer;
begin
  ...
  else if AMemberInfo.IsReference then
  begin
    SetLength(Result, AMemberInfo.ReferenceType.KeyAttributes.Count);
    for I := 0 to Length(Result) - 1 do
    begin
      Result[I].ADOField := FRecordset.Fields[AFieldIndex];
      Result[I].FieldType := AMemberInfo.ReferenceType.KeyAttributes[I]
        .GetCorrectFieldType;
      Inc(AFieldIndex);
    end;
  end
  ...
  begin
    Result := TArray<TdxADOField>.Create
      (TdxADOField.Create(FRecordset.Fields[AFieldIndex],
      AMemberInfo.GetCorrectFieldType));
    Inc(AFieldIndex);
  end;
end;
...
begin
  ...
  for I := 0 to Length(FieldsCache) - 1 do
    ...
    begin
      if FieldsCache[I].Value[J].ADOField <> nil then
        AValues[I] := TdxFieldValueConverter.ToValue
          (FieldsCache[I].Value[J].GetSafeVariant, FieldsCache[I].Key)
      else
...
begin
  ...
  for I := 0 to Length(KeyFieldsCache) - 1 do
    ...
    begin
      Result[K] := TdxFieldValueConverter.ToValue
        (KeyFieldsCache[I].Value[J].GetSafeVariant, KeyFieldsCache[I].Key);
      Inc(K);
    end;
end;

function TdxADOLoader.ToValues(const AFields: TArray<TdxADOField>;
  AMemberInfo: TdxMappingMemberInfo): TArray<TValue>;
var
  I: Integer;
begin
  ...
  for I := 0 to Length(AFields) - 1 do
    Result[I] := TdxFieldValueConverter.ToValue(AFields[I].GetSafeVariant,
      AMemberInfo);
end;

function TdxADOLoader.ToVariants(const AFields: TArray<TdxADOField>)
  : TArray<Variant>;
var
  I: Integer;
begin
...
procedure TdxADOCommand.PrepareParameters(AConvertParameter
  : TdxConvertParameter);
var
  I: Integer;
...
begin
  ...
  begin
    ...
    if AParameterValue <> nil then
      TdxADOConnectionVendor.AssignParameterValue(AParameter,
        AConvertParameter(AParameterValue.Value));
  end;
end;
...
begin
  ...
  if SQLDataSourcesW(AHenv, SQL_FETCH_FIRST, ADSN, SizeOf(ADSN), ADSNLength,
    ADescription, SizeOf(ADescription), ADescriptionLength) = SQL_SUCCESS then
    AList.Add(StrPas(ADSN) + '=' + StrPas(ADescription))
  else
    Exit;

  while SQLDataSourcesW(AHenv, SQL_FETCH_NEXT, ADSN, SizeOf(ADSN), ADSNLength,
    ADescription, SizeOf(ADescription), ADescriptionLength) = SQL_SUCCESS do
    AList.Add(StrPas(ADSN) + '=' + StrPas(ADescription))
end;

class function TdxADOConnectionVendor.TODBCHelper.GetSQLConfigDataSource
...
procedure TdxADOConnectionVendor.GetColumns(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
  AFieldSize: TField;
...
begin
  ...
  try
    ...
    begin
      ...
      AColumn := TdxDBColumn.Create(ADataSet.FieldByName('COLUMN_NAME')
        .AsString, False, '', ASize, AType);
      AColumn.IsNullable := SameText(ADataSet.FieldByName('IS_NULLABLE')
        .AsString, 'True');
      if SameText(ADataSet.FieldByName('COLUMN_HASDEFAULT').AsString, 'True')
      then
...
function TdxADOConnectionVendor.SelectData(AOwner: TComponent; AQuery: TdxQuery)
  : TDataSet;
begin
...
class procedure TdxADOConnectionVendor.AssignParameterVariant(const AParameter
  : TParameter; const AValue: Variant);
var
  AVarType: TVarType;
begin
...
class procedure TdxADOConnectionVendor.AssignParameterValue(const AParameter
  : TParameter; const AValue: TValue);
var
  ALength: Integer;
  ASource: PByte;
  AStream: TStream;
...
function TdxADOConnectionVendor.CreateCommand(AQuery: TdxQuery)
  : TdxCustomDBCommand;
begin
  Result := TdxADOCommand.Create(Connection, AQuery);
end;

function TdxADOConnectionVendor.CreateConnection(AProviderSQL
  : TdxSQLConnectionProvider; const ADatabaseName: string): TCustomConnection;
var
...
begin
  ...
  begin
    ...
    try
      ...
      try
        AConnectionStrings.Values
          [AProviderSQL.GetConnectionParameter(GetConnectionType,
          TdxConnectionParameter.Database)] := ADatabaseName;
...
function TdxADOConnectionVendor.CreateDataSet(AQuery: TdxQuery;
...
begin
  ...
  AMaxStringSize := TdxSQLConnectionProviderAccess(ProviderSQL)
    .MaximumStringSize;
  ...
  begin
    ...
    AssignParameterValue(AParameter,
      AQuery.ParameterValueByName(AParameter.Name).Value);
  end;
  Result := AResult;
end;
...
type
  ...
  function GetDBEngineType(const ADSNName: string;
    const AExpectedTypes: array of TServerType): TdxDBEngine;
  var
...
begin
  ...
  else if ContainsText(Connection.Provider, 'Microsoft.Jet.OLEDB') or
    ContainsText(Connection.Provider, 'Microsoft.ACE.OLEDB') then
  begin
    AConnectionString := GetParsedConnectionString;
    try
      AFileExtension := TPath.GetExtension
        (AConnectionString.Values['Data Source']);
      if ContainsText(AFileExtension, 'mdb') or ContainsText(AFileExtension,
        'accdb') then
  ...
  else if ContainsText(Connection.Provider, 'MSDAORA.1') or
    ContainsText(Connection.Provider, 'Oracle') then
    Result := TdxDBEngines.Oracle
  else
    Result := TdxDBEngines.Unknown;
end;

procedure TdxADOConnectionVendor.GetForeignKeys(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
...
begin
  ...
  try
    ...
    try
      while not ADataSet.EOF do
      begin
        AIndex := AForeignKeys.IndexOf(ADataSet.FieldByName('FK_NAME')
          .AsString);
        if AIndex < 0 then
        begin
          AForeignKey := TdxDBForeignKey.Create
            ([ADataSet.FieldByName('FK_COLUMN_NAME').AsString],
          ...
          AForeignKeys.AddObject(ADataSet.FieldByName('FK_NAME').AsString,
            AForeignKey);
        end
        else
        begin
          AForeignKey := TdxDBForeignKey(AForeignKeys.Objects[AIndex]);
          AForeignKey.Columns.Add(ADataSet.FieldByName('FK_COLUMN_NAME')
            .AsString);
...
procedure TdxADOConnectionVendor.GetIndexes(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
  AIndex: TdxDBIndex;
begin
  ...
  try
    ...
    while not ADataSet.EOF do
    begin
      if (AIndex = nil) or (AIndex.Name <> ADataSet.FieldByName('INDEX_NAME')
        .AsString) then
      begin
        AIndex := TdxDBIndex.Create(ADataSet.FieldByName('INDEX_NAME').AsString,
          [ADataSet.FieldByName('COLUMN_NAME').AsString],
...
procedure TdxADOConnectionVendor.GetPrimaryKey(ATable: TdxDBTable;
  const ASchemaName, ATableName: string);
var
  ADataSet: TDataSet;
...
begin
  ...
  try
    ADataSet := GetSchema(TdxSchemaInfo.PrimaryKeyFields, '', ASchemaName,
      ATableName);
    try
    ...
    begin
      ...
      try
        ...
        AADODataSet.CommandText := 'select [' + ATable.PrimaryKey.Columns[0] +
          '] from [' + ATableName + ']';
        AADODataSet.CursorType := ctOpenForwardOnly;
        AADODataSet.Open;
        if AADODataSet.Fields[0].AutoGenerateValue = TAutoRefreshFlag.arAutoInc
        then
...
function TdxADOConnectionVendor.GetSchema(ASchemaInfo: TdxSchemaInfo;
  const ACatalogName, ASchemaName, ATableName, AObjectName: string): TDataSet;
var
  AADODataSet: TADODataSet;
...
begin
  ...
  case ASchemaInfo of
    ...
    TdxSchemaInfo.ForeignKeys:
      V := VarArrayOf([Unassigned, Unassigned, Unassigned, ACatalog,
        ASchema, ATable]);
  else
...
procedure TdxADOConnectionVendor.GetTableSchema(ATable: TdxDBTable;
...
function TdxADOConnectionVendor.ExecuteScalar(AConnection: TCustomConnection;
  const ACommandText: string): Variant;
var
  AResult: _Recordset;
begin
...
function TdxEMFADODataProvider.CreateConnectionVendor
...
function TdxEMFADODataProvider.CreateLoader(ASession: TdxEMFCustomSession;
  AConnectionVendor: TdxCustomConnectionVendor; AQuery: TdxQuery;
...
begin
  ...
  ALoader := TdxADOLoader.Create(ASession,
    AConnectionVendor.CreateDataSet(AQuery));
...
begin
  ...
  begin
    if (FieldDefs[I].DataType = ftInteger) and
      (Recordset.Fields[I].Type_ = adUnsignedInt) then
