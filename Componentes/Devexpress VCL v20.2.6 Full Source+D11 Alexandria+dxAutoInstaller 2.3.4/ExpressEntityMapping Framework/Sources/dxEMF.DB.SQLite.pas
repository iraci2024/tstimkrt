...
type
  ...
  class var
    ...
    function GetDataForTables(const ATables: TArray<TdxDBTable>;
      AFilter: TdxSQLiteConnectionProvider.TTableFilter;
      const AQueryText: string): TDataSet;
    function GetTypeFromString(ATypeName: string; out ASize: Integer)
      : TdxDBColumnType;
  protected
...
function TdxSQLiteConnectionProvider.ConvertToDBParameter(const AParameterValue
  : TValue): TValue;
begin
...
procedure TdxSQLiteConnectionProvider.CreateColumn(ATable: TdxDBTable;
  AColumn: TdxDBColumn);
var
  ADataType: string;
begin
  ...
  ExecuteSQLSchemaUpdate('Column', AColumn.Name, ATable.Name,
    ...
    FormatColumnSafe(AColumn.Name), ADataType]));
end;

procedure TdxSQLiteConnectionProvider.CreateDatabase(const ADatabaseName
  : string; const AValues: array of string);
begin
  ConnectionVendor.Open;
end;

procedure TdxSQLiteConnectionProvider.CreateForeignKey(ATable: TdxDBTable;
...
procedure TdxSQLiteConnectionProvider.CreateTable(ATable: TdxDBTable);
  ...
  begin
    ...
    try
      ...
      begin
        AForeignKeys.Append(',').Append(' FOREIGN KEY(')
          .Append(TdxFormatterHelper.Concat(AForeignKey.Columns,
          FormatColumnSafe)).Append(') REFERENCES ')
          .Append(FormatTable(ComposeSafeSchemaName
          (AForeignKey.PrimaryKeyTable),
          ComposeSafeTableName(AForeignKey.PrimaryKeyTable))).Append(' (')
          .Append(TdxFormatterHelper.Concat
    ...
    begin
      ...
      try
        ...
        begin
          ...
          AFormattedColumns.Append
            (FormatColumnSafe(ATable.PrimaryKey.Columns[I]));
        end;
        ExecuteSQLSchemaUpdate('Table', ATable.Name, '',
          Format('create table %s (%s, primary key (%s)%s)',
          [FormatTableSafe(ATable), AColumns.ToString,
          AFormattedColumns.ToString, GetForeignKeyClause]));
      finally
      ...
      ExecuteSQLSchemaUpdate('Table', ATable.Name, '',
        Format('create table %s (%s%s)', [FormatTableSafe(ATable),
        AColumns.ToString, GetForeignKeyClause]));
  finally
    AColumns.Free;
  end;
end;

class function TdxSQLiteConnectionProvider.GetDBTypes
...
function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForBoolean
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bit';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'tinyint';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForSByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(3,0)';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForChar
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'nchar(1)';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForDecimal
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'money';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForDouble
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'double';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForSingle
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'float';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'int';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForUInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(10,0)';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'smallint';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForUInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(5,0)';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(20,0)';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForUInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(20,0)';
end;

function TdxSQLiteConnectionProvider.GetStorageTableNames
...
begin
  AQuery := TdxQuery.Create
    (Format('SELECT [name] FROM [sqlite_master] WHERE [type] LIKE ''table''%s and [name] not like ''SQLITE_%%''',
...
procedure TdxSQLiteConnectionProvider.GetTableSchema(ATable: TdxDBTable;
...
function TdxSQLiteConnectionProvider.GetTypeFromString(ATypeName: string;
  out ASize: Integer): TdxDBColumnType;
var
  ASizeText: string;
begin
  ASize := 0;
  ...
  if ASizeText <> '' then
    ATypeName :=
    {$IFDEF DELPHIXE3}ATypeName.Substring(0, ATypeName.IndexOf('(')){$ELSE}
      TdxStringHelper.Substring(ATypeName, TdxStringHelper.IndexOf(ATypeName,
      ('('))){$ENDIF};
  if not DBTypes.TryGetValue(ATypeName, Result) then
    Exit;
  if (Result in [TdxDBColumnType.String, TdxDBColumnType.ByteArray]) and
    (ASizeText <> '') then
...
function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForString
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
...
function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForDateTime
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'datetime';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForGuid
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char(36)';
end;

function TdxSQLiteConnectionProvider.GetSQLCreateColumnTypeForByteArray
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'blob';
end;
...
var
  ...
  AFieldName, AFieldType, AFieldNotNull, AFieldDefaultValue,
    APrimaryKey: TField;
begin
  AQuery := TdxQuery.Create('PRAGMA table_info('#$27 +
  ...
  try
    ...
    try
      ...
      begin
        ...
        AColumn := TdxDBColumn.Create(AFieldName.AsString,
          APrimaryKey.AsInteger <> 0, AFieldType.AsString, ASize, AType);
...
function TdxSQLiteConnectionProvider.GetConnectionParameter
...
class function TdxSQLiteConnectionProvider.GetConnectionParameters
  : TDictionary<string, TdxConnectionParameterValues>;
begin
...
function TdxSQLiteConnectionProvider.GetDataForTables(const ATables
  : TArray<TdxDBTable>; AFilter: TdxSQLiteConnectionProvider.TTableFilter;
  const AQueryText: string): TDataSet;
var
...
begin
  ...
  try
    ...
    AQuery := TdxQuery.Create(Format(AQueryText,
      ...
      begin
        Result := ':' + AValue;
      end, ',')]), AParameters, AInList.ToArray);
    try
...
begin
  ...
  AQuery := TdxQuery.Create('pragma foreign_key_list('#$27 +
    ComposeSafeTableName(ATable.Name) + #$27')');
  try
    try
      ADataSet := SelectData(AQuery);
...
{$IFDEF DELPHIXE3}ADataSet.Fields[2].AsString.Trim(['"']
        ){$ELSE}TdxStringHelper.Trim(ADataSet.Fields[2].AsString,
        ['"']){$ENDIF}, [AForeignKeyColumns[0]]));
      ADataSet.Next;
    end;
  finally
...
begin
  ...
  AQuery := TdxQuery.Create('pragma index_list('#$27 +
    ComposeSafeTableName(ATable.Name) + #$27')');
  try
    try
      ADataSet := SelectData(AQuery);
    ...
    begin
      AColumns.Clear;
      AQueryIndexData := TdxQuery.Create('pragma index_info('#$27 +
        AField1.AsString + #$27')');
      try
        ...
        ATable.Indexes.Add(TdxDBIndex.Create(AField1.AsString, AColumns.ToArray,
          AField2.AsInteger = 1));
      finally
...
function TdxSQLiteConnectionProvider.TryCastVariant(const AValue: Variant;
AExpectedType: TdxDBColumnType): Variant;
begin
  Result := AValue;
end;

function TdxSQLiteConnectionProvider.GetParameterName(const AParameter
  : IdxOperandValue; AIndex: Integer; var ACreateParameter: Boolean): string;
begin
  ACreateParameter := False;
  if Supports(AParameter, IdxConstantValue) and not AParameter.Value.IsNullOrEmpty
  then
    case AParameter.Value.Kind of
      ...
      tkString, tkLString, tkWString, tkUString:
        Exit(Concat(#$27, ReplaceText(AParameter.Value.AsString, #$27,
          #$27#$27), #$27));
    end;
...
function TdxSQLiteConnectionProvider.GetSQLCreateColumnFullAttributes
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  ...
  else if AColumn.IsNullable then
    Result := Result + ' NULL'
  else
    Result := Result + ' NOT NULL';
end;

function TdxSQLiteConnectionProvider.FormatTable(const ASchemaName: string;
...
function TdxSQLiteConnectionProvider.FormatUpdate(const ATableName, ASetClause,
...
function TdxSQLiteConnectionProvider.FormatColumn(const AColumnName
  : string): string;
begin
  Result := '[' + AColumnName + ']';
end;

function TdxSQLiteConnectionProvider.CollectTablesToCreate
  (const ATables: TArray<TdxDBTable>): TArray<TdxDBTable>;

  procedure ProcessTables(ADataSet: TDataSet;
  ADBTables: TDictionary<string, Boolean>; AIsView: Boolean);
  var
    AField: TField;
  begin
    if ADataSet = nil then
...
function TdxSQLiteConnectionProvider.FormatBinary(AOperatorType
...
function TdxSQLiteConnectionProvider.FormatColumn(const AColumnName: string;
const ATableAlias: string): string;
begin
  Result := ATableAlias + '.[' + AColumnName + ']';
end;

function TdxSQLiteConnectionProvider.FormatConstraint(const AConstraintName
  : string): string;
begin
  Result := '[' + AConstraintName + ']';
end;

function TdxSQLiteConnectionProvider.FormatDelete(const ATableName,
  AWhereClause: string): string;
begin
  Result := Format('delete from %s where %s', [ATableName, AWhereClause]);
end;

function TdxSQLiteConnectionProvider.FormatFunction(AOperatorType
...
begin
  case AOperatorType of
    ...
    TdxFunctionOperatorType.BigMul:
      Result := Format('CAST((%s) * (%s) as BIGINT)',
    ...
    TdxFunctionOperatorType.IncTick:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(((%1:s) / 10000000) as text) || '#$27' second'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMillisecond:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(((%1:s) / 1000) as text) || '#$27' second'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.AddTimeSpan, TdxFunctionOperatorType.IncSecond:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(%1:s as text) || '#$27' second'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMinute:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(%1:s as text) || '#$27' minute'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncHour:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(%1:s as text) || '#$27' hour'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncDay:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(%1:s as text) || '#$27' day'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMonth:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(%1:s as text) || '#$27' month'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncYear:
      Result := Format
        ('strftime('#$27'%%Y-%%m-%%d %%H:%%M:%%f'#$27', %0:s , CAST(%1:s as text) || '#$27' year'#$27')',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MillisecondOf:
      Result := Format
        ('(CAST((strftime('#$27'%%f'#$27', %0:s) * 1000) as integer) % 1000)',
        [AOperands[0]]);
    TdxFunctionOperatorType.SecondOf:
      Result := Format('CAST(strftime('#$27'%%S'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.MinuteOf:
      Result := Format('CAST(strftime('#$27'%%M'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.HourOf:
      Result := Format('CAST(strftime('#$27'%%H'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOf:
      Result := Format('CAST(strftime('#$27'%%d'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.MonthOf:
      Result := Format('CAST(strftime('#$27'%%m'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.YearOf:
      Result := Format('CAST(strftime('#$27'%%Y'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOfTheWeek:
      Result := Format('CAST(strftime('#$27'%%w'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOfTheYear:
      Result := Format('CAST(strftime('#$27'%%j'#$27', %s) as integer)',
        [AOperands[0]]);
    TdxFunctionOperatorType.GetTimeOfDay:
      Result := Format
        ('(CAST((strftime('#$27'%%H'#$27', %0:s) * 36000000000) as integer) + '
        + 'CAST((strftime('#$27'%%M'#$27', %0:s) * 600000000) as integer) + ' +
        'CAST((strftime('#$27'%%f'#$27', %0:s) * 10000000) as integer))',
        [AOperands[0]]);
    TdxFunctionOperatorType.DateOf:
      Result := Format('date(%s)', [AOperands[0]]);
    TdxFunctionOperatorType.YearsBetween:
      Result := Format
        ('(strftime('#$27'%%Y'#$27', %1:s) - strftime('#$27'%%Y'#$27', %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MonthsBetween:
      Result := Format
        ('(((strftime('#$27'%%Y'#$27', %1:s) - strftime('#$27'%%Y'#$27', %0:s)) * 12) + '
        + 'strftime('#$27'%%m'#$27', %1:s) - strftime('#$27'%%m'#$27', %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.DaysBetween:
      Result := Format('(julianday(date(%1:s)) - julianday(date(%0:s)))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.HoursBetween:
      Result := Format
        ('(((julianday(date(%1:s)) - julianday(date(%0:s))) * 24) + (strftime('#$27'%%H'#$27', %1:s) - '
    ...
    TdxFunctionOperatorType.SecondsBetween:
      Result := Format
        ('(strftime('#$27'%%s'#$27', %1:s) - strftime('#$27'%%s'#$27', %0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MillisecondsBetween:
      Result := Format
        ('((strftime('#$27'%%s'#$27', %1:s) - strftime('#$27'%%s'#$27', %0:s)) * 1000)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.DateDiffTicks:
      Result := Format
        ('((strftime('#$27'%%s'#$27', %1:s) - strftime('#$27'%%s'#$27', %0:s))) * 10000000',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.CharIndex:
      case Length(AOperands) of
        2:
          Result := Format('(CharIndex(%0:s, %1:s) - 1)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format('(CharIndex(%0:s, %1:s, (%2:s) + 1) - 1)',
            [AOperands[0], AOperands[1], AOperands[2]]);
        4:
          Result := Format
            ('(CharIndex(%0:s, Substr(%1:s, 1, (%2:s) + (%3:s)), (%2:s) + 1) - 1)',
    ...
    TdxFunctionOperatorType.Ascii, TdxFunctionOperatorType.Char,
      TdxFunctionOperatorType.Max, TdxFunctionOperatorType.Min:
      raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.IsNull:
      case Length(AOperands) of
        ...
        2:
          Result := Format('COALESCE(%0:s, %1:s)',
            [AOperands[0], AOperands[1]]);
      else
        Result := inherited FormatFunction(AOperatorType, AOperands);
      end;
    ...
    TdxFunctionOperatorType.EndsWith:
      Result := Format
        ('(Substr(%0:s, Length(%0:s) - Length(%1:s) + 1) = (%1:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.Contains:
      Result := Format('(CharIndex(%1:s, %0:s) > 0)',
        [AOperands[0], AOperands[1]]);
  else
    Result := inherited FormatFunction(AOperatorType, AOperands);
  end;
...
function TdxSQLiteConnectionProvider.FormatInsert(const ATableName, AFieldNames,
  AValuesClause: string): string;
begin
  Result := Format('insert into %s (%s) values(%s)',
    [ATableName, AFieldNames, AValuesClause]);
end;

function TdxSQLiteConnectionProvider.FormatInsertDefaultValues
...
begin
  ...
  Result := Format('insert into %s values(%s)', [FormatTableSafe(ATable),
    AValues]);
end;

function TdxSQLiteConnectionProvider.FormatSelect(const ASelectClause,
  AFromClause, AWhereClause, AOrderByClause, AGroupByClause,
  AHavingClause: string; ASkipSelectedRecords, ATopSelectedRecords
  : Integer): string;
var
  AExpandedWhereClause, AExpandedOrderByClause, AExpandedHavingClause,
    AExpandedGroupByClause, AModificatorsSQL: string;
begin
  ...
  if (ASkipSelectedRecords <> 0) or (ATopSelectedRecords <> 0) then
    AModificatorsSQL := Format(' LIMIT %d OFFSET %d ',
      [IfThen(ATopSelectedRecords = 0, MaxInt, ATopSelectedRecords),
      ASkipSelectedRecords])
  else
    AModificatorsSQL := '';
  Result := Format('select %s from %s%s%s%s%s%s', [ASelectClause, AFromClause,
    AExpandedWhereClause, AExpandedGroupByClause, AExpandedHavingClause,
    AExpandedOrderByClause, AModificatorsSQL]);
end;

function TdxSQLiteConnectionProvider.FormatFunction(AProcessParameter
...
begin
  case AOperatorType of
    TdxFunctionOperatorType.StartsWith:
      begin
        ASecondOperator := Safe<TdxCriteriaOperator>.Cast
          (AOperands[0].AsObject);
...
{$ENDIF}
          if ALikeIndex < 0 then
            Exit(Format('(%s like %s)', [AProcessParameter(ASecondOperator),
              ProcessParameter(AOperandString + '%')]))
          else if ALikeIndex > 0 then
            Exit(Format
              ('((%0:s like %2:s) And (Substr(%0:s, 1, Length(%1:s)) = (%1:s)))',
              [AProcessParameter(ASecondOperator),
              AProcessParameter(ASecondOperand),
              ProcessParameter({$IFDEF DELPHIXE3}AOperandString.Substring(0,
        ...
        Result := Format('(Substr(%0:s, 1, Length(%1:s)) = (%1:s))',
          [AProcessParameter(ASecondOperator),
          AProcessParameter(AValueOperator)]);
      end;
  else
