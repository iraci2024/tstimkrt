...
type
  ...
  public
    class operator Implicit(const A: TdxCriteriaOperatorHolder)
      : IdxCriteriaOperator;
    class operator Implicit(const A: TdxCriteriaOperatorHolder): string;
    class operator Implicit(const ACriteriaOperator: IdxCriteriaOperator)
      : TdxCriteriaOperatorHolder;
    class operator Implicit(const ACriteriaOperatorText: string)
      : TdxCriteriaOperatorHolder;
    class operator Equal(const A: TdxCriteriaOperatorHolder;
...
{$REGION 'internal types'}
    ...
    public
      ...
      property Collection: IdxEMFCollection read FCollection
        write SetCollection;
    ...
    public
      ...
      procedure GetBookmark(AValue:
        {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}); inline;
      procedure SetBookmark(AValue:
        {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}); inline;
      class function BookmarkToObject(AValue:
        {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}): TObject;
        static; inline;
      class procedure ObjectToBookmark(AValue: TObject; ABookmark:
        {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}); static; inline;
    end;
  ...
  protected type
    ...
    private
      ...
      FOffset: Integer;
    public
      constructor Create(AField: TField; AMemberInfo: TdxMappingMemberInfo;
        AOffset: Integer);
      procedure CopyFromBuffer(ARecordBuffer: TRecordBuffer;
        ABuffer: TValueBuffer);
      procedure CopyToBuffer(ARecordBuffer: TRecordBuffer;
        ABuffer: TValueBuffer);
      function HasBufferData(ARecordBuffer: TRecordBuffer): Boolean;
      function GetFieldBuffer(ARecordBuffer: TRecordBuffer)
        : TRecordBuffer; inline;
      ...
      procedure SetByEdited(ARecordBuffer: TRecordBuffer;
...
{$ENDIF}
    ...
    class function GetDataSize(AValue: Variant; AField: TField): Integer;
      overload; static;
    class function GetDataSize(AField: TField): Integer; overload;
      static; inline;
  ...
  protected
    ...
    procedure DataEvent(AEvent: TDataEvent; AInfo:
      {$IFDEF DELPHIXE2}NativeInt{$ELSE}Longint{$ENDIF}); override;
    ...
    function GetFieldValue(AActiveBuffer: TRecordBuffer; ABuffer: TValueBuffer;
...
{$ENDIF}
    ...
    procedure SetObjectMemberValue(AObject: TObject;
      AActiveBuffer: TRecordBuffer; const AFieldInfo: TFieldInfo);
    procedure SetRecNo(AValue: Integer); override;

    procedure InternalAddRecord(Buffer:
      {$IFDEF DELPHIXE3}TRecordBuffer{$ELSE}Pointer{$ENDIF};
    ...
    procedure InternalGotoBookmark(ABookmark:
      {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}); override;
    procedure InternalSetToRecord(ABuffer:
      {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}); override;
    function IsCursorOpen: Boolean; override;
    function GetCanModify: Boolean; override;
    function GetRecordCount: Integer; override;
    procedure GetBookmarkData(ABuffer:
      {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}; AData:
      {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}); override;
    function GetBookmarkFlag(ABuffer:
      {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF})
      : TBookmarkFlag; override;
    procedure SetBookmarkFlag(ABuffer: TRecordBuffer;
      AValue: TBookmarkFlag); override;
    procedure SetBookmarkData(ABuffer:
      {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}; AData:
      {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}); override;

    function GetDataSetFieldCollection(ADataSetField: TDataSetField)
      : IdxEMFCollection;
    function GetBlobData(ABuffer: TRecordBuffer; AField: TField;
      AStream: TStream): Boolean;
    procedure SetBlobData(ABuffer: TRecordBuffer; AField: TField;
      AStream: TStream);
...
{$ENDIF}
    ...
    property DataContext: TdxEMFCustomDataContext read GetDataContext
      write SetDataContext;
  ...
  public
    ...
    function CompareBookmarks(ABookmark1, ABookmark2: TBookmark)
      : Integer; override;
    function CreateBlobStream(Field: TField; Mode: TBlobStreamMode)
      : TStream; override;
    procedure FlushChanges;
    function GetFieldData(AField: TField; {$IFDEF DELPHIXE4}var
  ...
  strict private
    ...
    procedure SetSortByExpressionDefinitions(const Value
    ...
    procedure PrepareLoad(out AProperties, AGroupProperties
      : IdxCriteriaOperatorCollection; out ASorting: IdxSortByExpressions);
    procedure EndLoad(const AProperties, AGroupProperties
      : IdxCriteriaOperatorCollection; ASorting: IdxSortByExpressions;
    ...
    procedure SetGroupByExpressionDefinitions(const Value
  ...
  protected
    ...
    property GroupCriteria: IdxCriteriaOperator read GetGroupCriteria
      write SetGroupCriteria;
    property GroupCriteriaText: string read GetGroupCriteriaText
      write SetGroupCriteriaText;
    property GroupByExpressionDefinitions: TdxGroupByExpressionDefinitions
      read GetGroupByExpressionDefinitions write SetGroupByExpressionDefinitions
      stored IsGroupByExpressionDefinitionsStored;
    property FieldExpressions: TdxFieldExpressions read GetFieldExpressions
      write SetFieldExpressions stored IsFieldExpressionsStored;
    property IsProjection: Boolean read GetIsProjection;
    property Params: TParams read GetParams write SetParams;
    property SortByExpressions: IdxSortByExpressions read GetSortByExpressions
      write SetSortByExpressions;
    property SortByExpressionDefinitions: TdxSortByExpressionDefinitions
      read GetSortByExpressionDefinitions write SetSortByExpressionDefinitions
      stored IsSortByExpressionDefinitionsStored;
    property SkipSelectedRecords: Integer read GetSkipSelectedRecords
      write SetSkipSelectedRecords default 0;
    property TopSelectedRecords: Integer read GetTopSelectedRecords
      write SetTopSelectedRecords default 0;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
  end;
...
type
  ...
  TdxCustomEMFQueryDataSourceHelper = class helper for
    TdxCustomEMFQueryDataSource
  private
  ...
  protected
    ...
    property MemberInfo: TdxMappingMemberInfo read FMemberInfo;
  public
    constructor Create(AOwner: TComponent; AEntityField: TdxEntityField);
      reintroduce;
    procedure SetObject(AObject: TObject);
  end;

function ValueBufferToRecordBuffer(ABuffer: TValueBuffer)
  : TRecordBuffer; inline;
begin
{$IFDEF DELPHIXE3}
  Result := @ABuffer[0];
...
function TdxCustomEMFQueryDataSourceHelper.GetQueryProperties
...
constructor TdxEMFEntityFieldDataSet.Create(AOwner: TComponent;
  AEntityField: TdxEntityField);
begin
...
begin
  if IsNull or (TdxEMFEntityFieldDataSet(FInternalDataSet).MemberInfo = nil)
  then
    Result := Null
  else
    Result := TdxEMFEntityFieldDataSet(FInternalDataSet)
      .MemberInfo.ReferenceType.KeyProperty.GetValue(AsObject).AsVariant;
end;
...
begin
  ...
  begin
    ...
    AsObject := TdxEMFCustomDataSet(DataSet)
      .Session.Find(TdxEMFCustomDataSet(FInternalDataSet).EntityClass, AId);
  end;
end;

{ TdxCriteriaOperatorHolder }

class operator TdxCriteriaOperatorHolder.Implicit(const ACriteriaOperator
  : IdxCriteriaOperator): TdxCriteriaOperatorHolder;
begin
...
class operator TdxCriteriaOperatorHolder.Equal
  (const A: TdxCriteriaOperatorHolder;
  const ACriteriaOperator: IdxCriteriaOperator): Boolean;
begin
...
class operator TdxCriteriaOperatorHolder.Implicit
  (const A: TdxCriteriaOperatorHolder): IdxCriteriaOperator;
begin
  Result := A.FCriteriaOperator;
end;

class operator TdxCriteriaOperatorHolder.Implicit(const ACriteriaOperatorText
  : string): TdxCriteriaOperatorHolder;
begin
  Result.FCriteriaOperatorText := ACriteriaOperatorText;
  Result.FCriteriaOperator := TdxCriteriaOperator.Parse(ACriteriaOperatorText,
    Result.FOperandValue);
end;

class operator TdxCriteriaOperatorHolder.Implicit
  (const A: TdxCriteriaOperatorHolder): string;
begin
...
class function TdxEMFCustomDataSet.TRecInfo.BookmarkToObject(AValue:
  {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF}): TObject;
begin
{$IFDEF DELPHIXE3}
...
class procedure TdxEMFCustomDataSet.TRecInfo.ObjectToBookmark(AValue: TObject;
  ABookmark: {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF});
begin
{$IFDEF DELPHIXE3}
...
procedure TdxEMFCustomDataSet.TRecInfo.GetBookmark(AValue:
  {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF});
begin
  ObjectToBookmark(TheObject, AValue);
end;

procedure TdxEMFCustomDataSet.TRecInfo.SetBookmark(AValue:
  {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF});
begin
...
constructor TdxEMFCustomDataSet.TFieldInfo.Create(AField: TField;
  AMemberInfo: TdxMappingMemberInfo; AOffset: Integer);
begin
  FField := AField;
  FMemberInfo := AMemberInfo;
  FOffset := AOffset;
...
function TdxEMFCustomDataSet.TFieldInfo.GetByEdited(ARecordBuffer
  : TRecordBuffer): Boolean;
begin
  Result := (ARecordBuffer[FOffset] and 2) <> 0;
end;

function TdxEMFCustomDataSet.TFieldInfo.GetFieldBuffer(ARecordBuffer
  : TRecordBuffer): TRecordBuffer;
begin
  Result := @ARecordBuffer[Offset + 1];
end;

function TdxEMFCustomDataSet.TFieldInfo.GetData(ARecordBuffer
  : TRecordBuffer): TBytes;
begin
  SetLength(Result, FField.DataSize);
  Move(GetFieldBuffer(ARecordBuffer)^, Result[0], FField.DataSize);
end;

procedure TdxEMFCustomDataSet.TFieldInfo.CopyFromBuffer(ARecordBuffer
  : TRecordBuffer; ABuffer: TValueBuffer);
begin
  if ABuffer = nil then
  begin
...
procedure TdxEMFCustomDataSet.TFieldInfo.CopyToBuffer(ARecordBuffer
  : TRecordBuffer; ABuffer: TValueBuffer);
begin
{$IFDEF DELPHIXE3}
...
function TdxEMFCustomDataSet.TFieldInfo.HasBufferData(ARecordBuffer
  : TRecordBuffer): Boolean;
begin
  Result := ARecordBuffer[FOffset] <> 0;
end;

procedure TdxEMFCustomDataSet.TFieldInfo.SetByEdited(ARecordBuffer
  : TRecordBuffer; const AValue: Boolean);
var
  B: Byte;
begin
...
function TdxEMFCustomDataSet.TFieldInfo.IsEmpty(ARecordBuffer
  : TRecordBuffer): Boolean;
begin
...
begin
  ...
  FBlobStreams := TObjectDictionary<TFieldInfo, TMemoryStream>.Create
    ([doOwnsValues]);
...
function TdxEMFCustomDataSet.CompareBookmarks(ABookmark1,
...
function TdxEMFCustomDataSet.CreateBlobStream(Field: TField;
...
begin
  Result := ([csDesigning, csLoading] * ComponentState = [csDesigning]) and
    ((Owner = nil) or ([csDesigning, csLoading] * Owner.ComponentState =
    [csDesigning]));
end;
...
procedure TdxEMFCustomDataSet.Notification(AComponent: TComponent;
...
begin
  ...
  if AObject.ClassType <> EntityClass then
    raise Exception.CreateFmt('Class mismatch (found %s; expected %s)',
      [AObject.ClassType.ClassName, EntityClass.ClassName]);
...
procedure TdxEMFCustomDataSet.GetBookmarkData(ABuffer:
  {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}; AData:
  {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF});
begin
...
function TdxEMFCustomDataSet.GetBookmarkFlag(ABuffer:
  {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}): TBookmarkFlag;
begin
  Result := GetBufferRecInfo(Pointer(ABuffer))^.BookmarkFlag;
end;

function TdxEMFCustomDataSet.GetActiveRecordBuffer(var ARecordBuffer
  : TRecordBuffer): Boolean;
begin
  case State of
    dsBrowse:
...
function TdxEMFCustomDataSet.GetFieldValue(AActiveBuffer: TRecordBuffer;
  ABuffer: TValueBuffer; AField: TField): Boolean;
var
...
{$IFNDEF NEXTGEN}
{$IFDEF DELPHIXE4}
        AnsiStrings.StrLCopy(PAnsiChar(ABuffer), PAnsiChar(AnsiString(AStr)),
          AField.Size);
{$ELSE}
...
begin
  case AFieldType of
    ftObject:
      Result := TdxEntityField;
  else
    Result := inherited GetFieldClass(AFieldType);
  end;
end;

function TdxEMFCustomDataSet.GetFieldData(AField: TField; {$IFDEF DELPHIXE4}var
...
function TdxEMFCustomDataSet.GetFieldType(AMemberInfo: TdxMappingMemberInfo)
  : TFieldType;
var
...
begin
  ...
  if (Session <> nil) and
    (TdxEMFCustomDataProviderAccess(TdxEMFCustomSessionAccess(Session)
    .DataProvider) <> nil) then
    ASQLConnectionProvider := TdxEMFCustomDataProviderAccess
      (TdxEMFCustomSessionAccess(Session).DataProvider).SQLConnectionProvider
  else
    ASQLConnectionProvider := nil;
  if (Result = TFieldType.ftWideString) and
    (AMemberInfo.IsBlob or ((ASQLConnectionProvider <> nil) and
    (AMemberInfo.DBColumnSize > TdxSQLConnectionProviderAccess
    (ASQLConnectionProvider).MaximumStringSize))) then
    Result := TFieldType.ftWideMemo;
end;

function TdxEMFCustomDataSet.Locate(const KeyFields: string;
...
begin
  ...
  if (FCurrentRecord = -1) and (RecordCount > 0) then
    Result := 1
  else if not FIsFiltered then
    Result := FCurrentRecord + 1
  else
    Result := FFilterCurrentRecord + 1;
end;

function TdxEMFCustomDataSet.GetRecord(ABuffer: TRecordBuffer;
...
{$ENDIF}
    ...
    else if (Result = grError) and ADoCheck then
      DatabaseError(SDataSetEmpty, Self);
  end;
end;

procedure TdxEMFCustomDataSet.InternalAddRecord(Buffer:
  {$IFDEF DELPHIXE3}TRecordBuffer{$ELSE}Pointer{$ENDIF}; Append: Boolean);
begin
  if Append then
...
procedure TdxEMFCustomDataSet.InternalGotoBookmark(ABookmark:
  {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF});
var
  AIndex: Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      AFieldDef.Attributes := AFieldDef.Attributes +
        [TFieldAttribute.faReadonly];
      Continue;
    end
...
begin
  if FIsFiltered then
  begin
    if (FFilterCurrentRecord >= 0) and (FFilterCurrentRecord < FFilterList.Count)
    then
      FCurrentRecord := FFilterList[FFilterCurrentRecord]
    else
      FCurrentRecord := -1;
...
class function TdxEMFCustomDataSet.GetDataSize(AValue: Variant;
...
function TdxEMFCustomDataSet.GetDataSetFieldCollection(ADataSetField
  : TDataSetField): IdxEMFCollection;
var
...
begin
  ...
  begin
    ...
    AHasTrackingChanges := not TdxEMFCustomSessionAccess(Session)
      .TrackingChanges;
...
procedure TdxEMFCustomDataSet.InternalSetToRecord(ABuffer:
  {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF});
{$IFNDEF DELPHIXE3}
var
  ABookmark: TObject;
...
procedure TdxEMFCustomDataSet.DataEvent(AEvent: TDataEvent; AInfo:
  {$IFDEF DELPHIXE2}NativeInt{$ELSE}Longint{$ENDIF});
begin
...
procedure TdxEMFCustomDataSet.SetBookmarkData(ABuffer:
  {$IFDEF DELPHIXE4}TRecBuf{$ELSE}TRecordBuffer{$ENDIF}; AData:
  {$IFDEF DELPHIXE3}TBookmark{$ELSE}Pointer{$ENDIF});
begin
...
function TdxEMFCustomDataSet.GetBlobData(ABuffer: TRecordBuffer; AField: TField;
  AStream: TStream): Boolean;
var
  AFieldInfo: TFieldInfo;
...
begin
  ...
  begin
    APosition := AStream.Position;
    AStream.WriteBuffer(TBytes(AValue.GetReferenceToRawData^)[0],
      AValue.GetArrayLength);
    AStream.Position := APosition;
  end
  else
  begin
...
procedure TdxEMFCustomDataSet.SetBlobData(ABuffer: TRecordBuffer;
  AField: TField; AStream: TStream);
var
  AFieldInfo: TFieldInfo;
  ATmpStream: TMemoryStream;
  APosition: Integer;
...
function TdxEMFCustomDataSet.LocateRecord(const AKeyFields: string;
      ...
      begin
        ...
        begin
          ...
          if not AField.IsNull and VarIsStr(AValue) and
            (AField.DataType in [ftString, ftFixedChar, ftWideString, ftGuid,
...
function TdxEMFCustomDataSet.Lookup(const KeyFields: string;
...
procedure TdxEMFCustomDataSet.SetBookmarkFlag(ABuffer: TRecordBuffer;
...
begin
  ...
  if ADataSet.FieldInfos.TryGetValue(ADataSet.FindField(DataSetField.FieldName),
    AFieldInfo) then
  begin
...
procedure TdxEMFCustomDataSet.SetFieldData(AField: TField;
  ABuffer: TValueBuffer);
var
  ARecBuf: TRecordBuffer;
  AFieldInfo: TFieldInfo;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if Field is TLongWordField then
      begin
        if AMemberInfo.MaxValue.HasValue then
...
procedure TdxEMFCustomDataSet.SetOnFilterRecord(const Value
  : TFilterRecordEvent);
begin
...
procedure TdxEMFCustomDataSet.SetObjectMemberValue(AObject: TObject;
  AActiveBuffer: TRecordBuffer; const AFieldInfo: TFieldInfo);
var
  AData: TValue;
  AInt64: Int64;
...
begin
  if (AFieldInfo.MemberInfo = nil) or AFieldInfo.MemberInfo.IsVirtualColumn or
    (not AFieldInfo.HasBufferData(AActiveBuffer) and
    AFieldInfo.MemberInfo.IsNullable) then
    Exit;
  ...
  begin
    ...
    AData := TValue.FromOrdinal
      (AFieldInfo.MemberInfo.MemberType.Handle, AInt64);
  end
  else
    case AFieldInfo.Field.DataType of
      ftSmallint:
        AData := TValue.From
          (PSmallint(AFieldInfo.GetFieldBuffer(AActiveBuffer))^);
      ftInteger:
...
{$IFDEF DELPHIXE3}
          SetLength(ATempBuffer, SizeOf(TDateTime));
          DataConvert(AFieldInfo.Field, AFieldInfo.GetData(AActiveBuffer),
            ATempBuffer, False);
{$IFDEF DELPHIXE8}
...
{$ENDIF}
{$ELSE}
          DataConvert(AFieldInfo.Field,
            AFieldInfo.GetFieldBuffer(AActiveBuffer), @ADateTime, False);
{$ENDIF}
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if AMember <> nil then
                AFieldInfo.MemberInfo.Serializer.LoadFromStream
                  (AMember, AStream);
              Exit;
            end
            else if AFieldInfo.MemberInfo.IsType(TypeInfo(string)) or
              AFieldInfo.MemberInfo.IsDynamicArrayType then
            begin
              ...
              try
                ...
                begin
                  AEncoding := nil;
                  TEncoding.GetBufferEncoding(ABytes, AEncoding,
                    TEncoding.Unicode);
      ...
      ftWideString, ftFixedWideChar:
        begin
          AStr := WideCharToString
            (PChar(AFieldInfo.GetFieldBuffer(AActiveBuffer)));
          AData := AStr;
        end;
      ...
      ftGuid:
        begin
{$IFNDEF NEXTGEN}
          AStr := dxAnsiStringToString
            (PAnsiChar(AFieldInfo.GetFieldBuffer(AActiveBuffer)));
...
{$ENDIF}
      ...
      ftLongWord:
        AData := TValue.From
          (PLongWord(AFieldInfo.GetFieldBuffer(AActiveBuffer))^);
      ftShortint:
        AData := TValue.From
          (PShortint(AFieldInfo.GetFieldBuffer(AActiveBuffer))^);
      ftByte:
        AData := TValue.From(PByte(AFieldInfo.GetFieldBuffer(AActiveBuffer))^);
      ftObject:
        AData := TValue.From
          (PObject(AFieldInfo.GetFieldBuffer(AActiveBuffer))^);
      ftSingle:
        AData := TValue.From
          (PSingle(AFieldInfo.GetFieldBuffer(AActiveBuffer))^);
    else
      NotImplemented;
    end;
...
constructor TdxEMFCustomDataSet.TDataLinkList.Create
  (ADataSet: TdxEMFCustomDataSet);
begin
  inherited Create;
  FDataSet := ADataSet;
...
procedure TdxEMFCustomDataSet.TDataLinkList.Assign(const AEMFCollection
  : IdxEMFCollection);
begin
  FCollection := nil;
...
begin
  Result := (FInternalCollection <> nil) and
    (TdxEMFCustomCollection(FInternalCollection).Loader <> nil);
end;
...
procedure TdxEMFCustomDataSet.TDataLinkList.Insert(AIndex: Integer;
  AObject: TObject);
var
  AList: IdxList;
begin
  ...
  begin
    if Supports(FInternalCollection, IdxList, AList) and AList.Contains(AObject)
    then
      Exit;
...
procedure TdxEMFCustomDataSet.TDataLinkList.SetCollection
  (const Value: IdxEMFCollection);
begin
  FCollection := Value;
...
constructor TdxEMFDataSetBlobStream.Create(AField: TBlobField;
...
constructor TdxEMFCustomQueryDataSet.TEMFDataLink.Create
  (ADataSet: TdxEMFCustomQueryDataSet);
begin
  inherited Create;
  FDataSet := ADataSet;
end;
...
procedure TdxEMFCustomQueryDataSet.DataEvent(AEvent: TDataEvent; AInfo:
  {$IFDEF DELPHIXE2}NativeInt{$ELSE}Longint{$ENDIF});
begin
  if (AEvent = deParentScroll) and (DataSetField = nil) then
  begin
...
procedure TdxEMFCustomQueryDataSet.EndLoad(const AProperties, AGroupProperties
  : IdxCriteriaOperatorCollection; ASorting: IdxSortByExpressions;
  AInfoQuery: Boolean);
var
  AEntityInfo: TdxEntityInfo;
begin
  ...
  begin
    ...
    begin
      CheckSession;
      Data.Collection := Session.GetObjects
        (AEntityInfo.ClassAttributes.PersistentClass, Criteria,
        SortByExpressions, SkipSelectedRecords, TopSelectedRecords);
    end;
  end
  else
  begin
    CheckSession;
    FQueryData := TdxEMFCustomSessionAccess(Session).SelectData(AEntityInfo,
      AProperties, Criteria, AGroupProperties, GroupCriteria,
      { SelectDeleted } False, SkipSelectedRecords, TopSelectedRecords,
      ASorting);
    if not AInfoQuery then
      FetchQueryData;
  end;
...
begin
  ...
  Data.Collection := TdxSelectStatementCollections.Create
    (ASelectStatementResult);
  EntityInfo := TdxSelectStatementResultAccess(ASelectStatementResult)
    .EntityInfo;
  FQueryData := nil;
end;
...
function TdxEMFCustomQueryDataSet.GetGroupByExpressionDefinitions
  : TdxGroupByExpressionDefinitions;
begin
...
function TdxEMFCustomQueryDataSet.GetSortByExpressionDefinitions
  : TdxSortByExpressionDefinitions;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      CreateFieldDefs((FQueryData.DataLoader as TdxDataSetLoader)
...
begin
  Result := TdxCustomEMFQueryPropertiesAccess(EMFDataSource.QueryProperties)
    .IsFieldExpressionsStored;
end;

function TdxEMFCustomQueryDataSet.IsGroupByExpressionDefinitionsStored: Boolean;
begin
  Result := TdxCustomEMFQueryPropertiesAccess(EMFDataSource.QueryProperties)
    .IsGroupByExpressionDefinitionsStored;
end;

function TdxEMFCustomQueryDataSet.IsSortByExpressionDefinitionsStored: Boolean;
begin
  Result := TdxCustomEMFQueryPropertiesAccess(EMFDataSource.QueryProperties)
    .IsSortByExpressionDefinitionsStored;
end;
...
procedure TdxEMFCustomQueryDataSet.PrepareLoad(out AProperties, AGroupProperties
  : IdxCriteriaOperatorCollection; out ASorting: IdxSortByExpressions);
var
...
begin
  ...
  begin
    ...
    ASortBy.Add(TdxSortByExpression.Create
      (ASortByExpressionDefinition.Expression,
      ASortByExpressionDefinition.SortDirection));
  end;
  ASorting := ASortBy;
end;
...
procedure TdxEMFCustomQueryDataSet.SetCriteria(const Value
  : IdxCriteriaOperator);
begin
...
procedure TdxEMFCustomQueryDataSet.SetFieldExpressions
  (const Value: TdxFieldExpressions);
begin
  EMFDataSource.QueryProperties.FieldExpressions := Value;
  DoUpdateExpressions;
end;

procedure TdxEMFCustomQueryDataSet.SetGroupByExpressionDefinitions
  (const Value: TdxGroupByExpressionDefinitions);
begin
  GroupByExpressionDefinitions.Assign(Value);
  DoUpdateExpressions;
end;

procedure TdxEMFCustomQueryDataSet.SetGroupCriteria
...
procedure TdxEMFCustomQueryDataSet.SetSortByExpressionDefinitions
  (const Value: TdxSortByExpressionDefinitions);
begin
  SortByExpressionDefinitions.Assign(Value);
end;

procedure TdxEMFCustomQueryDataSet.SetSortByExpressions
  (const Value: IdxSortByExpressions);
begin
...
begin
  ...
  begin
    Data.Collection := Session.GetObjects
      (EntityInfo.ClassAttributes.PersistentClass, GetCriteria,
      GetSortByExpressions);
  end;
end;
    ...
    try
      ...
      begin
        Result := TdxGroupOperator.Combine(TdxGroupOperatorType.&And, Result,
          TdxBinaryOperator.Create(ADetailFieldsList[I],
...
begin
  ...
  begin
    ...
    TdxSortByExpressions(Result)
      .Add(TdxSortByExpression.Create(TdxOperandProperty.Create(AFieldName)));
  end;
end;
