...
type
  ...
  public
    ...
    procedure SaveToStream(AObject: TObject; AStream: TStream);
      virtual; abstract;
    procedure LoadFromStream(AObject: TObject; AStream: TStream);
      virtual; abstract;
  end;
  ...
  strict private
    class var FBlobSerializers
      : TDictionary<TClass, TdxCustomBlobSerializerClass>;
    class destructor Destroy;
  public
    class procedure Register(AClass: TClass;
      ABlobSerializeClass: TdxCustomBlobSerializerClass);
  ...
  public
    ...
    class function ConvertToStorageType(const AValue: TValue): TValue;
      virtual; abstract;
    class function ConvertFromStorageType(const AValue: TValue): TValue;
      virtual; abstract;
  end;
...
procedure TdxPersistentBlobSerializer.LoadFromStream(AObject: TObject;
  AStream: TStream);
var
...
procedure TdxPersistentBlobSerializer.SaveToStream(AObject: TObject;
  AStream: TStream);
var
  AStreamPersist: IStreamPersist;
begin
...
class function TdxBlobSerializerFactory.GetSerializer(AClass: TClass)
...
begin
  ...
  else if TdxStringsBlobSerializer.IsSupported(AClass) then
    Result := TdxStringsBlobSerializer
  else
    Result := nil;
end;

class procedure TdxBlobSerializerFactory.Register(AClass: TClass;
  ABlobSerializeClass: TdxCustomBlobSerializerClass);
begin
  if FBlobSerializers = nil then
    FBlobSerializers :=
      TDictionary<TClass, TdxCustomBlobSerializerClass>.Create;
...
procedure TdxStringsBlobSerializer.LoadFromStream(AObject: TObject;
  AStream: TStream);
begin
  (AObject as TStrings).LoadFromStream(AStream);
end;

procedure TdxStringsBlobSerializer.SaveToStream(AObject: TObject;
  AStream: TStream);
begin
