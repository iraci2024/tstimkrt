...
type
  ...
  public
    class function FormatBinary(AOperatorType: TdxBinaryOperatorType;
      const ALeftOperand: string; const ARightOperand: string): string; static;
    class function FormatConstraint(const AConstraintName: string)
      : string; static;
    class function FormatDelete(const ATableName: string;
      const AWhereClause: string): string; static;
    class function FormatFunction(AOperatorType: TdxFunctionOperatorType;
      AOperands: TArray<string>): string; overload; static;
    class function FormatFunction(AProcessParameter: TdxProcessParameter;
      AOperatorType: TdxFunctionOperatorType; AOperands: TArray<TValue>)
      : string; overload; static;
    class function FormatInsert(const ATableName: string; const AFields: string;
      const AValues: string): string; static;
    class function FormatInsertDefaultValues(const ATableName: string)
      : string; static;
    class function FormatOrder(const ASortProperty: string;
      ASortDirection: TdxSortDirection): string; static;
  end;
  ...
  public
    constructor Create(const AFormatter: TdxSQLConnectionProvider;
      const ASeqName, AIdentityParameterName: string;
      AIdentitiesByTag: TdxTaggedParameterHolder); reintroduce;
  end;
  ...
  protected
    ...
    function GetDataSet(const ACommandText: string;
      const AValues, AParameterNames: TArray<string>): TDataSet; overload;
    function GetDataSet(const ACommandText: string): TDataSet; overload;

    function ConvertToDBParameter(const AParameterValue: TValue)
      : TValue; override;
    procedure PrepareDelegates; virtual;
    function GetSQLCreateColumnTypeForBoolean(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForByte(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForSByte(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForChar(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForDecimal(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForDouble(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForSingle(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForInt32(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForUInt32(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForInt16(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForUInt16(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForInt64(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForUInt64(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForString(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForDateTime(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForGuid(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetSQLCreateColumnTypeForByteArray(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
  ...
  public
    constructor Create(AConnectionVendor: TdxCustomConnectionVendor;
      AAutoCreateOption: TdxAutoCreateOption); override;
  end;
  ...
  private
    ...
    function GetDataForTables(const ATables: TArray<TdxDBTable>;
      AFilter: TdxOracleConnectionProvider.TTableFilter;
      const AQueryText: string): TList<TDataSet>;
    ...
    function GetTypeFromString(const ATypeName: string; ASize: Integer;
      APrecision, AScale: Integer): TdxDBColumnType;
  protected
    function CollectTablesToCreate(const ATables: TArray<TdxDBTable>)
      : TArray<TdxDBTable>; override;
    ...
    function GetIdentity(ARoot: TdxInsertStatement;
      AIdentities: TdxTaggedParameterHolder): Int64; override;
  public
    constructor Create(AConnectionVendor: TdxCustomConnectionVendor;
      AAutoCreateOption: TdxAutoCreateOption); override;
    ...
    function GetConnectionParameter(const AConnectionType: TdxConnectionType;
      AParameter: TdxConnectionParameter): string; override;
    function GetSQLCreateColumnFullAttributes(ATable: TdxDBTable;
      AColumn: TdxDBColumn): string; override;
    function GetStorageTableNames(AIncludeViews: Boolean)
      : TArray<string>; override;
    procedure GetTableSchema(ATable: TdxDBTable;
      ACheckIndexes, ACheckForeignKeys: Boolean); override;
    function FormatColumn(const AColumnName: string): string; override;
    function FormatColumn(const AColumnName: string; const ATableAlias: string)
      : string; override;
    function FormatConstraint(const AConstraintName: string): string; override;
    function FormatFunction(AOperatorType: TdxFunctionOperatorType;
      const AOperands: TArray<string>): string; override;
    function FormatFunction(AProcessParameter: TdxProcessParameter;
      AOperatorType: TdxFunctionOperatorType; const AOperands: TArray<TValue>)
      : string; override;
    function FormatTable(const ASchemaName: string; const ATableName: string)
      : string; override;
    function FormatTable(const ASchemaName: string; const ATableName: string;
      const ATableAlias: string): string; override;
    function FormatBinary(AOperatorType: TdxBinaryOperatorType;
      const ALeftOperand, ARightOperand: string): string; override;
    function FormatSelect(const ASelectClause, AFromClause, AWhereClause,
      AOrderByClause, AGroupByClause, AHavingClause: string;
      ASkipSelectedRecords: Integer; ATopSelectedRecords: Integer)
      : string; override;
    function FormatDelete(const ATableName, AWhereClause: string)
      : string; override;
    function FormatOrder(const ASortProperty: string;
      ASortDirection: TdxSortDirection): string; override;
    function FormatInsertDefaultValues(ATable: TdxDBTable): string; override;
    function FormatInsert(const ATableName, AFieldNames, AValuesClause: string)
      : string; override;
    function FormatUpdate(const ATableName, ASetClause, AWhereClause: string)
      : string; override;
...
class function TdxOracleFormatterHelper.FormatBinary(AOperatorType
  : TdxBinaryOperatorType; const ALeftOperand: string;
  const ARightOperand: string): string;
begin
  case AOperatorType of
    ...
    TdxBinaryOperatorType.BitwiseOr:
      Result := Format('(%0:s - bitand(0:s, %1:s) + %1:s)',
        [ALeftOperand, ARightOperand]);
    TdxBinaryOperatorType.BitwiseXor:
      Result := Format('(0:s - bitand(0:s, %1:s) + %1:s - bitand(%0:s, %1:s))',
        [ALeftOperand, ARightOperand]);
    TdxBinaryOperatorType.Modulo:
      Result := Format('mod(0:s, %1:s)', [ALeftOperand, ARightOperand]);
  else
    Result := TdxBaseFormatterHelper.DefaultFormatBinary(AOperatorType,
      ALeftOperand, ARightOperand);
  end;
end;

class function TdxOracleFormatterHelper.FormatConstraint(const AConstraintName
  : string): string;
begin
  Result := Format('"%s"', [AConstraintName]);
end;

class function TdxOracleFormatterHelper.FormatDelete(const ATableName: string;
  const AWhereClause: string): string;
begin
  Result := Format('delete from %s where %s', [ATableName, AWhereClause]);
end;

class function TdxOracleFormatterHelper.FormatFunction(AOperatorType
  : TdxFunctionOperatorType; AOperands: TArray<string>): string;
begin
  case AOperatorType of
    TdxFunctionOperatorType.MillisecondOf:
      raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.SecondOf:
      Result := Format('TO_NUMBER(TO_CHAR(%s, '#$27'SS'#$27'))',
        [AOperands[0]]);
    TdxFunctionOperatorType.MinuteOf:
      Result := Format('TO_NUMBER(TO_CHAR(%s, '#$27'MI'#$27'))',
        [AOperands[0]]);
    TdxFunctionOperatorType.HourOf:
      Result := Format('TO_NUMBER(TO_CHAR(%s, '#$27'HH24'#$27'))',
        [AOperands[0]]);
    ...
    TdxFunctionOperatorType.DayOfTheWeek:
      Result := Format
        ('MOD((TO_NUMBER(TO_CHAR(%s, '#$27'D'#$27'))) - (TO_NUMBER(TO_CHAR(TO_DATE('#$27'01-01-1900'#$27','#$27'DD-MM-YYYY'#$27'), '#$27'D'#$27'))) + 8, 7)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOfTheYear:
      Result := Format('TO_NUMBER(TO_CHAR(%s, '#$27'DDD'#$27'))',
        [AOperands[0]]);
    TdxFunctionOperatorType.GetTimeOfDay:
      Result := Format('(TO_NUMBER(TO_CHAR(%s, '#$27'SSSSS'#$27')) * 10000000)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DateOf:
      Result := Format('TRUNC(%s)', [AOperands[0]]);
    TdxFunctionOperatorType.IncTick:
      Result := Format
        ('((%0:s) + NUMTODSINTERVAL(TRUNC((%1:s) / 864000000000), '#$27'DAY'#$27') + NUMTODSINTERVAL(MOD(TRUNC((%1:s) / 10000000),86400), '#$27'SECOND'#$27'))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMillisecond:
      Result := Format
        ('((%0:s) + NUMTODSINTERVAL(TRUNC((%1:s) / 1000), '#$27'SECOND'#$27'))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.AddTimeSpan, TdxFunctionOperatorType.IncSecond:
      Result := Format('((%0:s) + NUMTODSINTERVAL(%1:s, '#$27'SECOND'#$27'))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMinute:
      Result := Format
        ('((%0:s) + NUMTODSINTERVAL(TRUNC(((%1:s) * 60) / 86400), '#$27'DAY'#$27') + NUMTODSINTERVAL(MOD(TRUNC((%1:s) * 60),86400), '#$27'SECOND'#$27'))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncHour:
      Result := Format
        ('((%0:s) + NUMTODSINTERVAL(TRUNC(((%1:s) * 3600) / 86400), '#$27'DAY'#$27') + NUMTODSINTERVAL(MOD(TRUNC((%1:s) * 3600),86400), '#$27'SECOND'#$27'))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncDay:
      Result := Format
        ('((%0:s) + NUMTODSINTERVAL(TRUNC(%1:s), '#$27'DAY'#$27') + NUMTODSINTERVAL(MOD(TRUNC((%1:s) * 86400),86400), '#$27'SECOND'#$27'))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMonth:
      Result := Format('ADD_MONTHS(%0:s, %1:s)', [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncYear:
      Result := Format('ADD_MONTHS(%0:s, (%1:s) * 12)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.YearsBetween:
      Result := Format
        ('(EXTRACT(YEAR FROM (%1:s)) - EXTRACT(YEAR FROM (%0:s)))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MonthsBetween:
      Result := Format
        ('(((EXTRACT(YEAR FROM (%1:s)) - EXTRACT(YEAR FROM (%0:s))) * 12) + EXTRACT(MONTH FROM (%1:s)) - EXTRACT(MONTH FROM (%0:s)))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.DaysBetween:
      Result := Format('(TRUNC(%1:s) - TRUNC(%0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.HoursBetween:
      Result := Format
        ('((TRUNC((%1:s), '#$27'HH24'#$27') - TRUNC((%0:s), '#$27'HH24'#$27')) * 24)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MinutesBetween:
      Result := Format
        ('((TRUNC((%1:s), '#$27'MI'#$27') - TRUNC((%0:s), '#$27'MI'#$27')) * 1440)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.SecondsBetween:
      Result := Format
        ('(ROUND((TRUNC((%1:s), '#$27'MI'#$27') - TRUNC((%0:s), '#$27'MI'#$27')) * 86400) + TO_NUMBER(TO_CHAR((%1:s), '#$27'SS'#$27')) - TO_NUMBER(TO_CHAR((%0:s), '#$27'SS'#$27')))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MillisecondsBetween:
      Result := Format
        ('((ROUND((TRUNC((%1:s), '#$27'MI'#$27') - TRUNC((%0:s), '#$27'MI'#$27')) * 86400) + TO_NUMBER(TO_CHAR((%1:s), '#$27'SS'#$27')) - TO_NUMBER(TO_CHAR((%0:s), '#$27'SS'#$27'))) * 1000)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.DateDiffTicks:
      Result := Format
        ('((ROUND((TRUNC((%1:s), '#$27'MI'#$27') - TRUNC((%0:s), '#$27'MI'#$27')) * 86400) + TO_NUMBER(TO_CHAR((%1:s), '#$27'SS'#$27')) - TO_NUMBER(TO_CHAR((%0:s), '#$27'SS'#$27'))) * 10000000)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.BigMul:
      Result := Format('CAST(((%0:s) * (%1:s)) as INTEGER)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.ArcTan2:
      Result := Format
        ('(case when (%1:s) = 0 then Sign(%0:s) * Atan(1) * 2 else ATAN2(%0:s, %1:s) end)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.Max:
      Result := Format('(case when %0:s > %1:s then %0:s else %1:s end)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.Substring:
      case Length(AOperands) of
        2:
          Result := Format('SUBSTR(%0:s, (%1:s) + 1)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format('SUBSTR(%0:s, (%1:s) + 1, %2:s)',
            [AOperands[0], AOperands[1], AOperands[2]]);
      else
        Result := '';
      end;
    ...
    TdxFunctionOperatorType.CharIndex:
      case Length(AOperands) of
        2:
          Result := Format('(INSTR(%1:s, %0:s) - 1)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format('(INSTR(%1:s, %0:s, (%2:s) + 1) - 1)',
            [AOperands[0], AOperands[1], AOperands[2]]);
        4:
          Result := Format
            ('(INSTR(SUBSTR(%1:s, 1, (%2:s) + (%3:s)), %0:s, (%2:s) + 1) - 1)',
    ...
    TdxFunctionOperatorType.PadLeft:
      case Length(AOperands) of
        2:
          Result := Format
            ('(case when length(%0:s) > (%1:s) then %0:s else lpad(%0:s, %1:s) end)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('(case when length(%0:s) > (%1:s) then %0:s else lpad(%0:s, %1:s, %2:s) end)',
            [AOperands[0], AOperands[1], AOperands[2]]);
      else
        Result := '';
      end;
    TdxFunctionOperatorType.PadRight:
      case Length(AOperands) of
        2:
          Result := Format
            ('(case when length(%0:s) > %1:s then %0:s else rpad(%0:s, %1:s) end)',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('(case when length(%0:s) > %1:s then %0:s else rpad(%0:s, %1:s, %2:s) end)',
            [AOperands[0], AOperands[1], AOperands[2]]);
      else
        Result := '';
      end;
    ...
    TdxFunctionOperatorType.EndsWith:
      Result := Format
        ('(SUBSTR((%0:s), GREATEST(-LENGTH(%0:s), -LENGTH(%1:s))) = (%1:s))',
        [AOperands[0], AOperands[1]]);
...
class function TdxOracleFormatterHelper.FormatFunction(AProcessParameter
  : TdxProcessParameter; AOperatorType: TdxFunctionOperatorType;
  AOperands: TArray<TValue>): string;
var
  ...
  case AOperatorType of
    TdxFunctionOperatorType.StartsWith:
      begin
        ...
        if (ASecondOperand is TdxOperandValue) and
          TdxOperandValue(ASecondOperand).Value.IsString then
        begin
          AOperandString := TdxOperandValue(ASecondOperand).Value.AsString;
          ALikeIndex :=
          {$IFDEF DELPHIXE3}AOperandString.IndexOfAny(StopChars){$ELSE}TdxStringHelper.IndexOfAny(AOperandString, StopChars){$ENDIF};
          if ALikeIndex < 0 then
          begin
            AConstantValue := TdxConstantValue.Create(AOperandString + '%');
            Exit(Format('(%0:s like %1:s)',
              [AProcessParameter(Safe<TdxCriteriaOperator>.Cast(AOperands[0]
              .AsObject)), AProcessParameter(AConstantValue)]))
          end
          else if ALikeIndex > 0 then
            Exit(Format('((%0:s like %2:s) And (Instr(%0:s, %1:s) = 1))',
              [AProcessParameter(Safe<TdxCriteriaOperator>.Cast(AOperands[0]
              .AsObject)), AProcessParameter(Safe<TdxCriteriaOperator>.Cast
              (ASecondOperand)), AProcessParameter(TdxConstantValue.Create(
{$IFDEF DELPHIXE3}AOperandString.Substring(0,
              ALikeIndex){$ELSE}TdxStringHelper.Substring(AOperandString, 0,
              ALikeIndex){$ENDIF} + '%'))]));
        end;
        Result := Format('(Instr(%0:s, %1:s) = 1)',
          [AProcessParameter(Safe<TdxCriteriaOperator>.Cast(AOperands[0]
          .AsObject)), AProcessParameter(Safe<TdxCriteriaOperator>.Cast
          (ASecondOperand))]);
      end;
...
class function TdxOracleFormatterHelper.FormatInsert(const ATableName: string;
  const AFields: string; const AValues: string): string;
begin
  Result := Format('insert into %0:s(%1:s)values(%2:s)',
    [ATableName, AFields, AValues]);
end;

class function TdxOracleFormatterHelper.FormatInsertDefaultValues
  (const ATableName: string): string;
begin
  Result := Format('insert into %s values(default)', [ATableName]);
end;

class function TdxOracleFormatterHelper.FormatOrder(const ASortProperty: string;
  ASortDirection: TdxSortDirection): string;
const
  SortingMap: array [TdxSortDirection] of string = ('asc nulls first',
    'desc nulls last');
begin
...
constructor TdxIdentityInsertSqlGenerator.Create(const AFormatter
  : TdxSQLConnectionProvider; const ASeqName, AIdentityParameterName: string;
...
begin
  ...
  try
    ...
    ANames.Append(FFormatter.FormatColumn(FFormatter.ComposeSafeColumnName
      ((TdxInsertStatement(Root)).IdentityColumn)));
    ...
    Result := FFormatter.FormatInsert
      ...
      FFormatter.ComposeSafeTableName(Root.Table.Name)), ANames.ToString,
      AValues.ToString);
    if AHasIdentityParameter then
      Result := Format('select %s.nextval into %s from DUAL;%s;'#10,
        [FSeqName, FIdentityParameterName, Result]);
  finally
    AValues.Free;
    ANames.Free;
  end;
...
constructor TdxBaseOracleConnectionProvider.Create(AConnectionVendor
  : TdxCustomConnectionVendor; AAutoCreateOption: TdxAutoCreateOption);
begin
  inherited Create(AConnectionVendor, AAutoCreateOption);
  PrepareDelegates;
  FCaseSensitiveFieldNames := TdxConnectionVendorAccess(AConnectionVendor)
...
function TdxBaseOracleConnectionProvider.GetOperandArray
  (AValues: TArray<string>): TArray<IdxOperandValue>;
var
  I: Integer;
  AValue: string;
begin
...
function TdxBaseOracleConnectionProvider.GetDataSet(const ACommandText: string;
  const AValues, AParameterNames: TArray<string>): TDataSet;
var
...
function TdxBaseOracleConnectionProvider.ConvertToDBParameter
  (const AParameterValue: TValue): TValue;
const
...
begin
  ...
  else if AParameterValue.IsType<Boolean> then
...
function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForBoolean
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(1,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(3,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForSByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(3,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForChar
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'nchar';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForDecimal
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(20,5)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForDouble
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'double precision';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForSingle
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'float';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'int';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForUInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(10,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(5,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForUInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(5,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(20,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForUInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'number(20,0)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForString
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  if (AColumn.Size > 0) and (AColumn.Size <= MaximumStringSize) then
    Exit(Format('nvarchar2(%d)', [AColumn.Size]))
  else if AColumn.Size = 0 then
    Exit(Format('nvarchar2(%d)', [DefaultStringSize]))
  else
    Exit('nclob');
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForDateTime
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'date';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForGuid
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char(36)';
end;

function TdxBaseOracleConnectionProvider.GetSQLCreateColumnTypeForByteArray
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
...
var
  ...
  ACount: Variant;
begin
  AQuery1 := TdxQuery.Create
    ('SELECT count(*) FROM "SYS"."ALL_TABLES" WHERE "OWNER" = ''SYS'' AND "TABLE_NAME" = ''USER$''');
...
function TdxOracleConnectionProvider.CollectTablesToCreate
  (const ATables: TArray<TdxDBTable>): TArray<TdxDBTable>;
var
...
begin
  ...
  try
    ...
    begin
      AQueryString :=
        'select o.TABLE_NAME, o.OWNER from SYS.ALL_TABLES o inner join SYS.USER$ u on o."OWNER" = u."NAME"'#13#10
      ...
      try
        ...
        begin
          ...
          begin
            ...
            ADBSchemaTables.Add(Concat(ATableSchemaName, '.',
              ATableName), False);
            ADataSet.Next;
          end;
        end;
      ...
      AQueryString :=
        'select o.VIEW_NAME, o.OWNER from SYS.ALL_VIEWS o inner join SYS.USER$ u on o."OWNER" = u."NAME"'#13#10
    ...
    begin
      ...
      AIsView := False;
      if not ADBSchemaTables.TryGetValue(Concat(ATableSchemaName, '.',
        ATableName), AIsView) and not ADBTables.TryGetValue(ATableName, AIsView)
      then
        AList.Add(ATable)
      else
...
function TdxOracleConnectionProvider.GetIdentity(ARoot: TdxInsertStatement;
...
begin
  ...
  AGenerator := TdxIdentityInsertSqlGenerator.Create(Self, ASeq, '',
    AIdentities);
  try
...
constructor TdxOracleConnectionProvider.Create(AConnectionVendor
  : TdxCustomConnectionVendor; AAutoCreateOption: TdxAutoCreateOption);
begin
  inherited;
  FSysUsersAvailable := CheckSysUsers;
...
begin
  ...
  begin
    ...
    begin
      ...
      if SysUsersAvailable then
        AQuery := TdxQuery.Create
          ('select count(*) from SYS.ALL_SEQUENCES o inner join SYS.USER$ u on o."SEQUENCE_OWNER" = u."NAME"'#13#10
      ...
      AQuery := TdxQuery.Create
        (Format('create sequence %s START WITH 1 INCREMENT BY 1',
        [GetSeqName(ATable)]));
      try
        Execute(AQuery);
      finally
...
function TdxOracleConnectionProvider.GetConnectionParameter
  (const AConnectionType: TdxConnectionType;
  AParameter: TdxConnectionParameter): string;
begin
  raise Exception.Create('Error GetConnectionParameter');
end;

function TdxOracleConnectionProvider.GetSQLCreateColumnFullAttributes
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  if AColumn.IsKey and AColumn.IsIdentity and
    (AColumn.ColumnType in [TdxDBColumnType.Int32, TdxDBColumnType.Int64]) and
    IsSingleColumnPKColumn(ATable, AColumn) then
    Result := 'number PRIMARY KEY'
  else
    Result := GetSqlCreateColumnType(ATable, AColumn);
  if not AColumn.IsKey and
    (AColumn.IsNullable or (AColumn.ColumnType in [TdxDBColumnType.String,
    TdxDBColumnType.Guid])) then
    Result := Result + ' NULL'
  else
    Result := Result + ' NOT NULL';
end;

function TdxOracleConnectionProvider.GetStorageTableNames
  (AIncludeViews: Boolean): TArray<string>;
var
...
begin
  if SysUsersAvailable then
    ...
    AQueryStrings := TArray<string>.Create('select TABLE_NAME from USER_TABLES',
...
procedure TdxOracleConnectionProvider.GetTableSchema(ATable: TdxDBTable;
  ACheckIndexes, ACheckForeignKeys: Boolean);
begin
  GetColumns(ATable);
...
function TdxOracleConnectionProvider.FormatColumn(const AColumnName
  : string): string;
begin
...
function TdxOracleConnectionProvider.FormatColumn(const AColumnName: string;
  const ATableAlias: string): string;
begin
  Result := ATableAlias + '.' + FormatColumn(AColumnName);
end;

function TdxOracleConnectionProvider.FormatConstraint(const AConstraintName
  : string): string;
begin
  Result := TdxOracleFormatterHelper.FormatConstraint(AConstraintName);
end;

function TdxOracleConnectionProvider.FormatFunction(AOperatorType
  : TdxFunctionOperatorType; const AOperands: TArray<string>): string;
begin
...
function TdxOracleConnectionProvider.FormatFunction(AProcessParameter
  : TdxProcessParameter; AOperatorType: TdxFunctionOperatorType;
  const AOperands: TArray<TValue>): string;
begin
  Result := TdxOracleFormatterHelper.FormatFunction(AProcessParameter,
    AOperatorType, AOperands);
  if Result = '' then
    Result := inherited FormatFunction(AProcessParameter, AOperatorType,
      AOperands);
end;

function TdxOracleConnectionProvider.FormatTable(const ASchemaName: string;
  const ATableName: string): string;
begin
  if ASchemaName = '' then
...
function TdxOracleConnectionProvider.FormatTable(const ASchemaName: string;
  const ATableName: string; const ATableAlias: string): string;
begin
  if ASchemaName = '' then
    Result := Format('"%0:s" %1:s', [ATableName, ATableAlias])
  else
    Result := Format('"%0:s"."%1:s" %2:s', [ASchemaName, ATableName,
      ATableAlias]);
end;

function TdxOracleConnectionProvider.FormatBinary(AOperatorType
  : TdxBinaryOperatorType; const ALeftOperand, ARightOperand: string): string;
begin
  Result := TdxOracleFormatterHelper.FormatBinary(AOperatorType, ALeftOperand,
    ARightOperand);
end;

function TdxOracleConnectionProvider.FormatSelect(const ASelectClause,
  AFromClause, AWhereClause, AOrderByClause, AGroupByClause,
  AHavingClause: string; ASkipSelectedRecords: Integer;
  ATopSelectedRecords: Integer): string;
var
  AExpandedWhereSql, AExpandedOrderBySql, AExpandedHavingSql,
    AExpandedGroupBySql, ASelectedSql, ABaseFormat: string;
  AFields: TArray<string>;
  AExpandedSelectedProperties: string;
begin
  inherited FormatSelect(ASelectClause, AFromClause, AWhereClause,
    AOrderByClause, AGroupByClause, AHavingClause, ASkipSelectedRecords,
    ATopSelectedRecords);
  ...
  AExpandedSelectedProperties := TdxSimpleSQLParser.GetExpandedProperties
    (AFields, 'a');
  ...
  begin
    if ATopSelectedRecords = 0 then
      Result := Format('select %0:s from %1:s%2:s%3:s%4:s%5:s',
        [ASelectClause, AFromClause, AExpandedWhereSql, AExpandedGroupBySql,
        AExpandedHavingSql, AExpandedOrderBySql])
    else
      Result := Format
        ('select * from (select %1:s from %2:s%3:s%4:s%5:s%6:s) where RowNum <= %0:d',
        [ATopSelectedRecords, ASelectedSql, AFromClause, AExpandedWhereSql,
        AExpandedGroupBySql, AExpandedHavingSql, AExpandedOrderBySql]);
  end
  else
  begin
    ABaseFormat :=
      'select %0:s from(select %0:s, RowNum rNum from(select %1:s from %2:s%3:s%4:s%5:s%6:s)a)a where rNum > %7:d';
    if ATopSelectedRecords <> 0 then
      ABaseFormat := ABaseFormat + ' and RowNum <= %8:d';
    Result := Format(ABaseFormat, [AExpandedSelectedProperties, ASelectedSql,
      AFromClause, AExpandedWhereSql, AExpandedGroupBySql, AExpandedHavingSql,
      AExpandedOrderBySql, ASkipSelectedRecords, ATopSelectedRecords]);
  end;
end;

function TdxOracleConnectionProvider.FormatDelete(const ATableName,
  AWhereClause: string): string;
begin
  Result := TdxOracleFormatterHelper.FormatDelete(ATableName, AWhereClause);
end;

function TdxOracleConnectionProvider.FormatOrder(const ASortProperty: string;
  ASortDirection: TdxSortDirection): string;
begin
  Result := TdxOracleFormatterHelper.FormatOrder(ASortProperty, ASortDirection);
end;

function TdxOracleConnectionProvider.FormatInsertDefaultValues
  (ATable: TdxDBTable): string;
begin
  Result := TdxOracleFormatterHelper.FormatInsertDefaultValues
    (FormatTableSafe(ATable));
end;

function TdxOracleConnectionProvider.FormatInsert(const ATableName, AFieldNames,
  AValuesClause: string): string;
begin
  Result := TdxOracleFormatterHelper.FormatInsert(ATableName, AFieldNames,
    AValuesClause);
end;

function TdxOracleConnectionProvider.FormatUpdate(const ATableName, ASetClause,
  AWhereClause: string): string;
begin
  Result := Format('update %s set %s where %s', [ATableName, ASetClause,
    AWhereClause]);
end;
...
function TdxOracleConnectionProvider.GetDataForTables(const ATables
  : TArray<TdxDBTable>; AFilter: TdxOracleConnectionProvider.TTableFilter;
  const AQueryText: string): TList<TDataSet>;
var
  AInGroup: string;
  ATable: TdxDBTable;
...
begin
  ...
  if ASchemaName = '' then
    ADataSet := GetDataSet
      (Format('SELECT COLUMN_NAME, DATA_TYPE, CHAR_COL_DECL_LENGTH, DATA_PRECISION, DATA_SCALE, NULLABLE, DATA_DEFAULT from USER_TAB_COLUMNS where TABLE_NAME = ''%s''',
      [ASafeTableName]))
  else
    ADataSet := GetDataSet
      (Format('SELECT COLUMN_NAME, DATA_TYPE, CHAR_COL_DECL_LENGTH, DATA_PRECISION, DATA_SCALE, NULLABLE, DATA_DEFAULT from ALL_TAB_COLUMNS where OWNER = ''%s'' and TABLE_NAME = ''%s''',
      [ASchemaName, ASafeTableName]));
  try
    while not ADataSet.Eof do
    begin
      ...
      if AType <> TdxDBColumnType.String then
        ASize := 0;
      AColumn := TdxDBColumn.Create(ADataSet.Fields[0].AsString, False, '',
        ASize, AType);
...
const
  ColumnsMap: array [Boolean] of string = ('ALL_CONS_COLUMNS',
    'USER_CONS_COLUMNS');
var
...
begin
  ...
  try
    ...
    begin
      repeat
        ACommandText :=
          Format('select tc.POSITION, tc.COLUMN_NAME, fc.COLUMN_NAME, fc.TABLE_NAME from USER_CONSTRAINTS c'#13#10
          ...
          [ColumnsMap[FGetForeignKeysHasNoRights], ASafeTableName]);
        try
    ...
    begin
      ACommandText :=
        Format('select tc.POSITION, tc.COLUMN_NAME, fc.COLUMN_NAME, fc.TABLE_NAME from ALL_CONSTRAINTS c'#13#10
    ...
    begin
      ...
      begin
        AForeignKey := TdxDBForeignKey.Create([ADataSet.Fields[1].AsString],
          ADataSet.Fields[3].AsString, [ADataSet.Fields[2].AsString]);
...
begin
  ...
  begin
    ADataSet := GetDataSet
      (Format('select ind.INDEX_NAME, cols.COLUMN_NAME, cols.COLUMN_POSITION, ind.uniqueness'#13#10
  ...
  try
    ...
    begin
      ...
      begin
        AIndex := TdxDBIndex.Create(ADataSet.Fields[0].AsString,
          [ADataSet.Fields[1].AsString],
          ADataSet.Fields[3].AsString = 'UNIQUE');
...
begin
  ...
  if ASchemaName = '' then
    ADataSet := GetDataSet
      (Format('select tc.COLUMN_NAME from USER_CONS_COLUMNS tc'#13#10 +
      'left join USER_CONSTRAINTS c on tc.CONSTRAINT_NAME = c.CONSTRAINT_NAME'#13#10
      + 'where c.CONSTRAINT_TYPE = ''P'' and tc.TABLE_NAME = ''%s''',
      [ASafeTableName]))
  else
    ADataSet := GetDataSet
      (Format('select tc.COLUMN_NAME from ALL_CONS_COLUMNS tc'#13#10 +
  ...
  try
    ...
    ATable.PrimaryKey := TdxDBPrimaryKey.Create(ATable.Name,
      AColumns.ToArray, True);
  finally
    AColumns.Free;
    ADataSet.Free;
  end;
...
begin
  ...
  if Result = '' then
    Result := ComposeSafeConstraintName
      ('sq_' + ComposeSafeTableName(ATable.Name));
end;
...
function TdxOracleConnectionProvider.GetTypeFromString(const ATypeName: string;
  ASize: Integer; APrecision, AScale: Integer): TdxDBColumnType;
var
  ATypeNameLo: string;
begin
  ...
  else if (ATypeNameLo = 'nchar') or (ATypeNameLo = 'char') then
  begin
    if ASize > 1 then
  ...
  else if ATypeNameLo = 'money' then
    Result := TdxDBColumnType.Decimal
  else if ATypeNameLo = 'float' then
    Result := TdxDBColumnType.Double
  else if (ATypeNameLo = 'nvarchar') or (ATypeNameLo = 'varchar') or
    (ATypeNameLo = 'varchar2') or (ATypeNameLo = 'nvarchar2') then
    Result := TdxDBColumnType.String
