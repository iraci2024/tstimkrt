...
type
  ...
  public
    constructor Create(const AName: string; AIsKey: Boolean;
      const ADBTypeName: string; ASize: Integer; AType: TdxDBColumnType);
    class function GetColumnType(AType: PTypeInfo;
      ASuppressExceptionOnUnknown: Boolean = False): TdxDBColumnType; static;
  ...
  strict private
    FIsUnique: Boolean;
  public
    constructor Create(const AName: string; const AColumns: array of string;
      AIsUnique: Boolean); overload;
    constructor Create(const AColumns: array of string;
      AIsUnique: Boolean); overload;
    constructor Create(const AName: string; const AColumns: TArray<TdxDBColumn>;
      AIsUnique: Boolean); overload;
    constructor Create(const AColumns: array of TdxDBColumn;
      AIsUnique: Boolean); overload;
    constructor Create(AColumns: TEnumerable<TdxDBColumn>;
      AIsUnique: Boolean); overload;
    constructor Create(AColumns: TEnumerable<string>;
      AIsUnique: Boolean); overload;
  ...
  public
    ...
    constructor Create(const AColumns: array of string;
      const APrimaryKeyTable: string;
      const APrimaryKeyTableKeyColumns: array of string); overload;
    constructor Create(const AColumns: array of TdxDBColumn;
      const APrimaryKeyTable: string;
      const APrimaryKeyTableKeyColumns: array of string); overload;
    constructor Create(const AColumns: array of TdxDBColumn;
      const APrimaryKeyTable: string); overload;
    destructor Destroy; override;
    property PrimaryKeyTable: string read FPrimaryKeyTable;
    property PrimaryKeyTableKeyColumns: TList<string>
      read FPrimaryKeyTableKeyColumns;
  end;
  ...
  public
    ...
    function AreGadgetsEqual(AExistingKey: TdxDBTableMultiColumnGadget;
      AForeignKey: TdxDBTableMultiColumnGadget): Boolean;
...
constructor TdxDBColumn.Create(const AName: string; AIsKey: Boolean;
  const ADBTypeName: string; ASize: Integer; AType: TdxDBColumnType);
begin
  inherited Create;
  IsKey := AIsKey;
...
class function TdxDBColumn.GetColumnType(AType: PTypeInfo;
  ASuppressExceptionOnUnknown: Boolean): TdxDBColumnType;
var
  ATypeData: PTypeData;
  ...
  case AType.Kind of
    ...
    tkFloat:
      case ATypeData.FloatType of
        ...
        TFloatType.ftDouble, TFloatType.ftExtended, TFloatType.ftComp:
          begin
            if (AType = TypeInfo(TDateTime)) or (AType = TypeInfo(TDate)) or
    ...
    tkRecord{$IFDEF DELPHI103}, tkMRecord{$ENDIF}:
      begin
        ...
        else if AType = TypeInfo(TBCD) then
          Result := TdxDBColumnType.Decimal
        else if IsNullableType(AType) then
        begin
          TryGetUnderlyingType(AType, AUnderlyingTypeInfo);
          Result := GetColumnType(AUnderlyingTypeInfo,
            ASuppressExceptionOnUnknown);
        end
        else
...
constructor TdxDBTableMultiColumnGadget.Create
  (AColumns: TEnumerable<TdxDBColumn>);
begin
  Create;
  AddColumns(AColumns);
end;

constructor TdxDBTableMultiColumnGadget.Create(const AColumns
  : array of TdxDBColumn);
begin
  Create;
  AddColumns(AColumns);
end;
...
procedure TdxDBTableMultiColumnGadget.AddColumns(const AColumns
  : array of TdxDBColumn);
var
  AColumn: TdxDBColumn;
begin
...
procedure TdxDBTableMultiColumnGadget.AddColumns
  (AColumns: TEnumerable<TdxDBColumn>);
var
  AColumn: TdxDBColumn;
begin
...
constructor TdxDBIndex.Create(const AColumns: array of string;
  AIsUnique: Boolean);
begin
...
constructor TdxDBIndex.Create(const AName: string;
  const AColumns: array of string; AIsUnique: Boolean);
begin
  inherited Create(AColumns);
  SetName(AName);
  FIsUnique := AIsUnique;
...
constructor TdxDBIndex.Create(const AColumns: array of TdxDBColumn;
  AIsUnique: Boolean);
begin
  inherited Create(AColumns);
  FIsUnique := AIsUnique;
...
constructor TdxDBIndex.Create(const AName: string;
  const AColumns: TArray<TdxDBColumn>; AIsUnique: Boolean);
begin
  inherited Create(AColumns);
  SetName(AName);
  FIsUnique := AIsUnique;
...
constructor TdxDBIndex.Create(AColumns: TEnumerable<TdxDBColumn>;
  AIsUnique: Boolean);
begin
  inherited Create(AColumns);
  FIsUnique := AIsUnique;
...
constructor TdxDBIndex.Create(AColumns: TEnumerable<string>;
  AIsUnique: Boolean);
begin
  inherited Create(AColumns);
  FIsUnique := AIsUnique;
...
constructor TdxDBForeignKey.Create(const AColumns: array of string;
  const APrimaryKeyTable: string; const APrimaryKeyTableKeyColumns
  : array of string);
begin
  inherited Create(AColumns);
...
constructor TdxDBForeignKey.Create(const AColumns: array of TdxDBColumn;
  const APrimaryKeyTable: string; const APrimaryKeyTableKeyColumns
  : array of string);
begin
  inherited Create(AColumns);
...
function TdxDBTable.AreGadgetsEqual(AExistingKey: TdxDBTableMultiColumnGadget;
  AForeignKey: TdxDBTableMultiColumnGadget): Boolean;
var
  I: Integer;
begin
  if (AExistingKey.Name <> AForeignKey.Name) or
    (AExistingKey.Columns.Count <> AForeignKey.Columns.Count) then
    Exit(False);
