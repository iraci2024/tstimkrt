...
type
  ...
  strict private
    class var FConnectionParameters
  ...
  protected
    ...
    function GetTypeFromString(const ATypeName: string; out ASize: Integer)
      : TdxDBColumnType;
  ...
  public
    ...
    function FormatCreateTable(const ATableName: string; const AColumns: string)
      : string; override;
    function FormatDelete(const ATableName: string; const AWhereClause: string)
      : string; override;
    ...
    function FormatFunction(AProcessParameter: TdxProcessParameter;
      ...
      : string; override;
    function FormatInsert(const ATableName: string; const AFields: string;
      const AValues: string): string; override;
    ...
    function FormatTable(const ASchemaName, ATableName, ATableAlias: string)
      : string; override;
    function FormatUpdate(const ATableName: string; const ASets: string;
      const AWhereClause: string): string; override;
    ...
    class function FnAddDateTime(const ADatetimeOperand: string;
      const ADayPart: string; const ASecondPart: string): string; static;
    class function FormatGetInt(const AArg: string;
      AMultiplier, ADivider: Integer): string; static;
    class function FormatMod(const AArg: string; AMultiplier, ADivider: Integer)
      : string; static;
...
constructor TdxMySQLConnectionProvider.Create(AConnectionVendor
...
function TdxMySQLConnectionProvider.FormatBinary(AOperatorType
...
function TdxMySQLConnectionProvider.FormatColumn(const AColumnName
  : string): string;
begin
...
function TdxMySQLConnectionProvider.FormatConstraint(const AConstraintName
  : string): string;
begin
  Result := Format('`%s`', [AConstraintName]);
end;

function TdxMySQLConnectionProvider.FormatCreateTable(const ATableName: string;
  const AColumns: string): string;
begin
  Result := Format
    ('create table if not exists %s (%s) CHARACTER SET utf8 COLLATE utf8_unicode_ci;',
    [ATableName, AColumns]);
end;

function TdxMySQLConnectionProvider.FormatDelete(const ATableName,
  AWhereClause: string): string;
begin
  Result := 'delete from ' + ATableName + ' where ' + AWhereClause;
end;

function TdxMySQLConnectionProvider.FormatFunction(AOperatorType
...
begin
  ...
  case AOperatorType of
    TdxFunctionOperatorType.MillisecondOf, TdxFunctionOperatorType.ToDouble,
    ...
    TdxFunctionOperatorType.Insert:
      Result := Format
        ('Concat(Left(%0:s,%1:s),%2:s,Right(%0:s,Length(%0:s)-(%1:s)))',
    ...
    TdxFunctionOperatorType.Tanh:
      Result := Format
        ('( (Exp(%0:s) - Exp((%0:s * (-1) ))) / (Exp(%0:s) + Exp((%0:s * (-1) ))) )',
        [AOperands[0]]);
    ...
    TdxFunctionOperatorType.Max:
      Result := Format('if(%0:s > %1:s, %0:s, %1:s)',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.EndsWith:
      Result := Format('(Right(%0:s, Length(%1:s)) = (%1:s))',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.GetTimeOfDay:
      Result := Format
        ('((Hour(%0:s)) * 36000000000) + ((Minute(%0:s)) * 600000000) + (Second(%0:s) * 10000000)',
        [AOperands[0]]);
    TdxFunctionOperatorType.DayOfTheWeek:
      Result := Format
        ('((DayOfWeek(%0:s)- DayOfWeek('#$27'1900-01-01'#$27')  + 8) % 7)',
        [AOperands[0]]);
    ...
    TdxFunctionOperatorType.IncTick:
      Result := Format
        ('cast(Adddate(%0:s, interval (%1:s div 10000000 ) second )  as datetime)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMillisecond:
      Result := Format
        ('cast(Adddate(%0:s, interval ( %1:s div 1000) second )  as datetime)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.AddTimeSpan, TdxFunctionOperatorType.IncSecond:
      Result := FnAddDateTime(AOperands[0], FormatGetInt(AOperands[1], 1,
        86400), FormatMod(AOperands[1], 1, 86400));
    TdxFunctionOperatorType.IncMinute:
      Result := FnAddDateTime(AOperands[0], FormatGetInt(AOperands[1], 60,
        86400), FormatMod(AOperands[1], 60, 86400));
    TdxFunctionOperatorType.IncHour:
      Result := FnAddDateTime(AOperands[0], FormatGetInt(AOperands[1], 3600,
        86400), FormatMod(AOperands[1], 3600, 86400));
    TdxFunctionOperatorType.IncDay:
      Result := FnAddDateTime(AOperands[0], FormatGetInt(AOperands[1], 86400,
        86400), FormatMod(AOperands[1], 86400, 86400));
    TdxFunctionOperatorType.IncMonth:
      Result := Format('cast(Adddate(%0:s, interval %1:s month )  as datetime)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncYear:
      Result := Format('cast(Adddate(%0:s, interval %1:s year )  as datetime)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.YearsBetween:
      Result := Format('(Year(%1:s) - Year(%0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MonthsBetween:
      Result := Format
        ('(((Year(%1:s) - Year(%0:s)) * 12) + Month(%1:s) - Month(%0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.SecondsBetween:
      Result := Format('(UNIX_TIMESTAMP(%1:s) - UNIX_TIMESTAMP(%0:s))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MillisecondsBetween:
      Result := Format('((UNIX_TIMESTAMP(%1:s) - UNIX_TIMESTAMP(%0:s)) * 1000)',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.DateDiffTicks:
      Result := Format
        ('((UNIX_TIMESTAMP(%1:s) - UNIX_TIMESTAMP(%0:s))) * 10000000',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.DaysBetween:
      if IsVersion5 then
        ...
        Result := Format('(TO_DAYS(%1:s) - TO_DAYS(%0:s))',
          [AOperands[0], AOperands[1]]);

    TdxFunctionOperatorType.HoursBetween:
      Result := Format('((%2:s * 24) + Hour(%1:s) - Hour(%0:s))',
        [AOperands[0], AOperands[1],
        FormatFunction(TdxFunctionOperatorType.DaysBetween, AOperands)]);

    TdxFunctionOperatorType.MinutesBetween:
      Result := Format('((%2:s) * 60 + Minute(%1:s) - Minute(%0:s))',
        [AOperands[0], AOperands[1],
        FormatFunction(TdxFunctionOperatorType.HoursBetween, AOperands)]);
    ...
    TdxFunctionOperatorType.PadLeft:
      case Length(AOperands) of
        2:
          Result := Format
            ('If(Length(%0:s) >= %1:s, %0:s, LPad(%0:s, %1:s, '#$27' '#$27'))',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('If(Length(%0:s) >= %1:s, %0:s, LPad(%0:s, %1:s, %2:s))',
            [AOperands[0], AOperands[1], AOperands[2]]);
      end;
    ...
    TdxFunctionOperatorType.PadRight:
      case Length(AOperands) of
        2:
          Result := Format
            ('If(Length(%0:s) >= %1:s, %0:s,RPad(%0:s, %1:s, '#$27' '#$27'))',
            [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('If(Length(%0:s) >= %1:s, %0:s,RPad(%0:s, %1:s, %2:s))',
            [AOperands[0], AOperands[1], AOperands[2]]);
      end;
    ...
    TdxFunctionOperatorType.CharIndex:
      case Length(AOperands) of
        ...
        3:
          Result := Format
            ('( if( Instr(Right(%1:s,(Length(%1:s) - %2:s)),%0:s) = 0, -1 , Instr(Right(%1:s,(Length(%1:s) - %2:s)),%0:s) -1 + %2:s) )',
            [AOperands[0], AOperands[1], AOperands[2]]);
        4:
          Result := Format
            ('(if( instr(Left( Right(%1:s,(Length(%1:s) - %2:s ) ) ,{3}),%0:s) = 0, -1, instr(Left( Right(%1:s,(Length(%1:s) - %2:s ) ) ,{3}),%0:s) - 1 + %2:s))',
    ...
    TdxFunctionOperatorType.Remove:
      case Length(AOperands) of
        2:
          Result := Format('Left(%0:s,%1:s)', [AOperands[0], AOperands[1]]);
        3:
          Result := Format
            ('Concat(Left(%0:s,%1:s),Right(%0:s,Length(%0:s)-%1:s-%2:s))',
            [AOperands[0], AOperands[1], AOperands[2]]);
      end;
    ...
    TdxFunctionOperatorType.Log:
      case Length(AOperands) of
        1:
          Result := Format('Log(%0:s)', [AOperands[0]]);
        2:
    ...
    TdxFunctionOperatorType.Iif:
      begin
        ...
        if Length(AOperands) = 3 then
          Exit(Format('If(%0:s, %1:s, %2:s)', [AOperands[0], AOperands[1],
            AOperands[2]]));
        ...
        try
          ...
          repeat
            Inc(AIndex, 2);
            AStringBuilder.AppendFormat('If(%0:s, %1:s, ',
...
function TdxMySQLConnectionProvider.FormatFunction(AProcessParameter
    ...
    begin
      ...
      begin
        AConstantValue := TdxConstantValue.Create
          (TdxStringHelper.Substring(AOperandString, 0, ALikeIndex) + '%');
...
function TdxMySQLConnectionProvider.FormatInsert(const ATableName, AFields,
  AValues: string): string;
begin
  Result := 'insert into ' + ATableName + '(' + AFields + ')values(' +
    AValues + ')';
end;

function TdxMySQLConnectionProvider.FormatInsertDefaultValues
  (ATable: TdxDBTable): string;
begin
  Result := Format('insert into %s values()', [FormatTableSafe(ATable)]);
end;

function TdxMySQLConnectionProvider.FormatSelect(const ASelectedPropertiesSQL,
  AFromSQL, AWhereSQL, AOrderBySQL, AGroupBySQL, AHavingSQL: string;
...
begin
  ...
  begin
    ...
    AModificatorsSql := Format('limit %0:d, %1:d ',
      [ASkipSelectedRecords, ATopSelectedRecordsValue]);
  end;
...
function TdxMySQLConnectionProvider.FormatTable(const ASchemaName,
  ATableName: string): string;
begin
  Result := Format('`%s`', [ATableName]);
end;

function TdxMySQLConnectionProvider.FormatTable(const ASchemaName, ATableName,
  ATableAlias: string): string;
begin
  Result := Format('`%0:s` %1:s', [ATableName, ATableAlias]);
end;

function TdxMySQLConnectionProvider.FormatUpdate(const ATableName, ASets,
  AWhereClause: string): string;
begin
  Result := 'update ' + ATableName + ' set ' + ASets + ' where ' + AWhereClause;
end;

function TdxMySQLConnectionProvider.GetConnectionParameter(const AConnectionType
...
function TdxMySQLConnectionProvider.GetSQLCreateColumnFullAttributes
...
class function TdxMySQLConnectionProvider.GetConnectionParameters
  : TDictionary<string, TdxConnectionParameterValues>;
begin
...
class function TdxMySQLConnectionProvider.GetDBTypes
...
function TdxMySQLConnectionProvider.ConvertToDBParameter(const AParameterValue
  : TValue): TValue;
begin
...
function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForBoolean
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bit';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'tinyint unsigned';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForByteArray
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
...
function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForSByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'tinyint';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForChar
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForDateTime
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'datetime';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForDecimal
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'decimal(28,8)';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForDouble
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'double';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForGuid
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char(38)';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForSingle
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'real';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForString
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
var
  ASize: Integer;
begin
...
function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'int';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForUInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'int unsigned';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'smallint';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForUInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'smallint unsigned';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bigint';
end;

function TdxMySQLConnectionProvider.GetSQLCreateColumnTypeForUInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bigint unsigned';
end;

function TdxMySQLConnectionProvider.CollectTablesToCreate
...
begin
  ...
  try
    for I := 0 to Length(ATables) - 1 do
      AResult.AddOrSetValue(LowerCase(ComposeSafeTableName(ATables[I].Name)),
        ATables[I]);

    if IsVersion5 then
    begin
      AQuery := TdxQuery.Create(Format(V5Query,
        [ConnectionVendor.GetDatabaseName(Self)]));
      try
...
function TdxMySQLConnectionProvider.GetStorageTableNames(AIncludeViews: Boolean)
  : TArray<string>;
...
begin
  ...
  AQuery := TdxQuery.Create(Format(BuildFormatLine,
    [ConnectionVendor.GetDatabaseName(Self)]));
  try
    ADataSet := SelectData(AQuery);
    try
...
procedure TdxMySQLConnectionProvider.GetTableSchema(ATable: TdxDBTable;
...
class function TdxMySQLConnectionProvider.FnAddDateTime(const ADatetimeOperand
  : string; const ADayPart: string; const ASecondPart: string): string;
begin
  Result := Format
    ('cast(AddDate(AddDate(%0:s, interval %1:s day), interval %2:s second) as datetime)',
    [ADatetimeOperand, ADayPart, ASecondPart]);
end;

class function TdxMySQLConnectionProvider.FormatMod(const AArg: string;
  AMultiplier: Integer; ADivider: Integer): string;
begin
  Result := Format('(Truncate(Cast(%0:s as decimal(65,30)) * %1:s, 0) %% %2:s)',
    [AArg, AMultiplier, ADivider]);
end;

class function TdxMySQLConnectionProvider.FormatGetInt(const AArg: string;
  AMultiplier: Integer; ADivider: Integer): string;
begin
  Result := Format('(Cast(%0:s as decimal(65,30)) * %1:s Div %2:s)',
    [AArg, AMultiplier, ADivider]);
end;
...
begin
  AQuery := TdxQuery.Create(Format('show columns from `%0:s`',
  ...
  try
    ...
    try
      ...
      begin
        ...
        AColumn := TdxDBColumn.Create(GetFieldValue(ADataSet, 0), False, '',
          ASize, AType);
        AColumn.IsIdentity := ContainsText(GetFieldValue(ADataSet, 5),
          'auto_increment');
...
var
  ...
  ATempString: string;
begin
  AQuery := TdxQuery.Create(Format('show create table `%0:s`',
    [ComposeSafeTableName(ATable.Name)]));
  try
    ADataSet := SelectData(AQuery);
    try
      ...
      begin
        ...
        try
          ...
          repeat
            ...
            AColumnIndex := TdxStringHelper.IndexOf(AString,
              'FOREIGN KEY', APos);
            ...
            APrimesEndIndex := TdxStringHelper.IndexOf(AString, ')',
              APrimesIndex);
            if (APrimesIndex < 0) or (APrimesEndIndex < 0) then
              Break;

            ATempString := TdxStringHelper.Substring(AString, ARefsIndex + 12,
              APrimesIndex - 12 - ARefsIndex);
            ARefTable := TdxStringHelper.Trim(ATempString, ['`', ' ']);

            ATempString := TdxStringHelper.Substring(AString, AColumnIndex + 12,
              ARefsIndex - 12 - AColumnIndex);
            SplitToList(AColumns, TdxStringHelper.Trim(ATempString,
              ['`', ' ', '(', ')']));

            ATempString := TdxStringHelper.Substring(AString, APrimesIndex,
              APrimesEndIndex - APrimesIndex);
            SplitToList(APrimaryColumns, TdxStringHelper.Trim(ATempString,
              [' ', '(', ')']));

            ATable.AddForeignKey(TdxDBForeignKey.Create(AColumns.ToArray,
              ARefTable, APrimaryColumns.ToArray));
          until False;
        finally
...
var
  ...
  ARowValues4: string;
begin
  AQuery := TdxQuery.Create(Format('show index from `%0:s`',
    [ComposeSafeTableName(ATable.Name)]));
  try
    ADataSet := SelectData(AQuery);
    try
...
function TdxMySQLConnectionProvider.GetTypeFromString(const ATypeName: string;
  ...
  begin
    ...
    begin
      ...
      if ABracketClose >= 0 then
        ATypeWithoutBrackets := TdxStringHelper.Remove(ATypeName, ABracketOpen,
          ABracketClose - ABracketOpen + 1);
    end;
...
begin
  ...
  begin
    if not DBTypes.TryGetValue(LowerCase(RemoveBrackets(ATypeName)), Result)
    then
    begin
...
function TdxMySQLConnectionProvider.GetFieldValue(ADataSet: TDataSet;
...
begin
  if FIsVersion5 = bDefault then
    FIsVersion5 := dxBooleanToDefaultBoolean
      (not TdxStringHelper.StartsWith(GetMySqlServerVersion, '4.'));
