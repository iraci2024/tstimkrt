...
type
  ...
  protected
    function ConvertToDBParameter(const AParameterValue: TValue)
      : TValue; override;
    class function GetColumnTypeByInfo(AType, ASubType, AScale, ASize: SmallInt)
      : TdxDBColumnType;
    ...
    function GetFieldValueDef(AField: TField; const ADefValue: Variant)
      : Variant;
  ...
  public
    ...
    function FormatColumn(const AColumnName, ATableAlias: string)
      : string; override;
    ...
    function FormatTable(const ASchemaName, ATableName, ATableAlias: string)
      : string; override;
    function FormatSelect(const ASelectedPropertiesSQL, AFromSQL, AWhereSQL,
      AOrderBySQL, AGroupBySQL, AHavingSQL: string;
    ...
    function GetParameterName(const AParameter: IdxOperandValue;
      AIndex: Integer; var ACreateParameter: Boolean): string; override;
...
constructor TdxFirebirdConnectionProvider.TVersion.Create
  (const AVersion: string);
var
  AVersionParts: TArray<string>;
begin
  AVersionParts :=
  {$IFDEF DELPHIXE3}AVersion.Split(['.']){$ELSE}TdxStringHelper.Split(AVersion,
    ['.']){$ENDIF};
...
function TdxFirebirdConnectionProvider.TVersion.Support(AMajorHi, AMajorLo,
  AMinor: Byte): Boolean;
begin
  if not HasValue then
    Exit(False);
  Result := (MajorHi >= AMajorHi) and (MajorLo >= AMajorLo) and
    (Minor >= AMinor);
end;

{ TdxFirebirdConnectionProvider }

constructor TdxFirebirdConnectionProvider.Create(AConnectionVendor
...
function TdxFirebirdConnectionProvider.FormatBinary(AOperatorType
...
const
  AOperatorFormat: array [TdxBinaryOperatorType] of string = ('', '', '', '',
    '', '', 'bin_and(%s, %s)', 'bin_or(%s, %s)', 'bin_xor(%s, %s)', '',
    'mod(%s, %s)', '', '', '');
begin
  if AOperatorFormat[AOperatorType] <> '' then
    Result := Format(AOperatorFormat[AOperatorType],
      [ALeftOperand, ARightOperand])
  else
    Result := inherited FormatBinary(AOperatorType, ALeftOperand,
      ARightOperand);
end;
...
begin
  ...
  begin
    ...
    if AKey.IsIdentity and not IsSequenceExists(ASequenceName) then
      ExecuteSQLSchemaUpdate('AutoInc', 'Key', '', 'create generator ' +
        FormatTable(ASequenceName));
  end;
end;

procedure TdxFirebirdConnectionProvider.CreateIndex(ATable: TdxDBTable;
  AIndex: TdxDBIndex);
begin
...
function TdxFirebirdConnectionProvider.FormatColumn(const AColumnName
  : string): string;
begin
...
function TdxFirebirdConnectionProvider.FormatConstraint(const AConstraintName
  : string): string;
begin
  if not CaseSensitiveFieldNames then
...
function TdxFirebirdConnectionProvider.FormatFunction(AOperatorType
...
begin
  case AOperatorType of
    ...
    TdxFunctionOperatorType.Log:
      case Length(AOperands) of
        1:
          Result := Format('Ln(%s)', [AOperands[0]]);
        2:
    ...
    TdxFunctionOperatorType.Max:
      Result := Format('iif(%0:s > %1:s, %0:s, %1:s)',
    ...
    TdxFunctionOperatorType.BigMul:
      Result := Format('(cast(%s as bigint) * cast(%s as bigint))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.MillisecondOf:
      Result := Format
        ('(cast(extract(second from %0:s) - floor(extract(second from %0:s)) as numeric(4, 4)) * 1000)',
        [AOperands[0]]);
    ...
    TdxFunctionOperatorType.GetTimeOfDay:
      Result := Format
        ('floor(extract(hour from %0:s) * 36000000000 + extract(minute from %0:s) * 600000000 + extract(second from {0}) * 10000000)',
        [AOperands[0]]);
    ...
    TdxFunctionOperatorType.Ascii:
      Result := Format
        ('ascii_val(substring(cast(%s as varchar(8191)) from 1 for 1))',
        [AOperands[0]]);
    ...
    TdxFunctionOperatorType.PadLeft:
      case Length(AOperands) of
        2:
          Result := Format
            ('iif(%2:s(%0:s) > %1:s, %0:s, lpad(%0:s, %0:s, '#$27' '#$27'))',
            [AOperands[0], AOperands[1], GetFunctionStrLenName]);
        3:
          Result := Format
            ('iif(%3:s(%0:s) > %0:s, %0:s, lpad(%0:s, %1:s, %2:s))',
            [AOperands[0], AOperands[1], AOperands[2], GetFunctionStrLenName])
      else
    ...
    TdxFunctionOperatorType.PadRight:
      case Length(AOperands) of
        2:
          Result := Format
            ('iif(%2:s(%0:s) > %1:s, %0:s, rpad(%0:s, %1:s, '#$27' '#$27'))',
            [AOperands[0], AOperands[1], GetFunctionStrLenName]);
        3:
          Result := Format
            ('iif(%3:s(%0:s) > %0:s, %0:s, rpad(%0:s, %1:s, %2:s))',
            [AOperands[0], AOperands[1], AOperands[2], GetFunctionStrLenName])
      else
        raise ENotSupportedException.Create('');
      end;
    TdxFunctionOperatorType.IncTick:
      Result := Format
        ('(%s + cast(%s as double precision) / cast(864000000000 as double precision))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMillisecond:
      Result := Format
        ('(%s + cast(%s as double precision) / cast(86400000 as double precision))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.AddTimeSpan, TdxFunctionOperatorType.IncSecond:
      Result := Format
        ('(%s + cast(%s as double precision) / cast(86400 as double precision))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMinute:
      Result := Format
        ('(%s + cast(%s as double precision) / cast(1440 as double precision))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncHour:
      Result := Format
        ('(%s + cast(%s as double precision) / cast(24 as double precision))',
        [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncDay:
      Result := Format('(%s + cast(%s as double precision))',
        [AOperands[0], AOperands[1]]);
    ...
    TdxFunctionOperatorType.Substring:
      if Length(AOperands) = 3 then
        Result := Format
          ('substring((%s) from cast(%s as integer)+1 for cast(%s as integer))',
          [AOperands[0], AOperands[1], AOperands[2]])
      else
        Result := Format('substring((%s) from cast(%s as integer)+1)',
          [AOperands[0], AOperands[1]]);
    TdxFunctionOperatorType.IncMonth:
      if Version.Support(2, 1) then
        Result := Format
          ('dateadd(month, cast(%s as double precision), cast(%s as timestamp))',
          [AOperands[1], AOperands[0]])
      else
        raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.IncYear:
      if Version.Support(2, 1) then
        Result := Format
          ('dateadd(year, cast(%s as double precision), cast(%s as timestamp))',
          [AOperands[1], AOperands[0]])
      else
        raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.CharIndex:
      ...
      begin
        if Length(AOperands) >= 3 then
          Result := Format('(position(%s, %s, cast(%s as integer) + 1) - 1)',
            [AOperands[0], AOperands[1], AOperands[2]])
        else
          Result := Format('(position(%s, %s) - 1)',
            [AOperands[0], AOperands[1]]);
      end
      else
        raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.Round:
      ...
      begin
        if Length(AOperands) >= 2 then
          Result := Format
            ('round(cast(%s as double precision), cast(%s as integer))',
            [AOperands[0], AOperands[1]])
        else
          Result := Format('round(cast(%s as double precision))',
            [AOperands[0]]);
      end
      else
        raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.Remove:
      ...
      begin
        if Length(AOperands) >= 3 then
          Result := Format
            ('(left(%0:s, cast(%1:s as integer)) || right(%0:s, char_length(%0:s)-cast({1} as integer)-cast(%2:s as integer)))',
            [AOperands[0], AOperands[1], AOperands[2]])
        else
          Result := Format('(left(%s, cast(%s as integer)))',
            [AOperands[0], AOperands[1]]);
      end
      else
        raise ENotSupportedException.Create('');
    TdxFunctionOperatorType.Insert:
      if Version.Support(2, 1) then
        Result := Format
          ('(left(%0:s, cast(%1:s as integer)) || (%2:s) || right(%0:s, char_length(%0:s)-cast(%1:s as integer)))',
          [AOperands[0], AOperands[1], AOperands[2]])
      else
        raise ENotSupportedException.Create('');
    ...
    TdxFunctionOperatorType.Power:
      if Version.Support(2, 1) then
        Result := Format
          ('power(cast(%s as double precision), cast(%s as double precision))',
          [AOperands[0], AOperands[1]])
      else
        raise ENotSupportedException.Create('');
...
function TdxFirebirdConnectionProvider.FormatFunction(AProcessParameter
...
begin
  if AOperatorType in [TdxFunctionOperatorType.StartsWith,
    TdxFunctionOperatorType.EndsWith, TdxFunctionOperatorType.Contains] then
  begin
    ...
    begin
      ...
      if ANeedEscape then
        AOperandString :=
          ReplaceText(ReplaceText(ReplaceText(AOperandString, '\', '\\'), '%',
          '\%'), '_', '\_');
      case AOperatorType of
      ...
      if ANeedEscape then
        Result := Format('(%s like %s ESCAPE '#$27'\'#$27')',
          [AProcessParameter(AFirstOperand), AOperandStringFormatted])
      else
        Result := Format('(%s like %s)', [AProcessParameter(AFirstOperand),
          AOperandStringFormatted]);
    end
    else
      raise ENotSupportedException.Create('');
  end
...
function TdxFirebirdConnectionProvider.FormatTable(const ATableName
  : string): string;
begin
  Result := '"' + ATableName + '"'
end;

function TdxFirebirdConnectionProvider.FormatTable(const ASchemaName: string;
...
function TdxFirebirdConnectionProvider.FormatSelect
  (const ASelectedPropertiesSQL, AFromSQL, AWhereSQL, AOrderBySQL, AGroupBySQL,
  AHavingSQL: string; ASkipSelectedRecords: Integer;
  ATopSelectedRecords: Integer): string;
var
  ATopSql, ASkipSql, AModificatorsSql, AExpandedWhereSql, AExpandedOrderBySql,
    AExpandedHavingSql, AExpandedGroupBySql: string;
begin
  Result := inherited FormatSelect(ASelectedPropertiesSQL, AFromSQL, AWhereSQL,
    AOrderBySQL, AGroupBySQL, AHavingSQL, ASkipSelectedRecords,
    ATopSelectedRecords);
  ATopSql := '';
  ASkipSql := '';
  AExpandedWhereSql := '';
  ...
  Result := 'select ' + AModificatorsSql + ASelectedPropertiesSQL + ' from ' +
    AFromSQL + AExpandedWhereSql + AExpandedGroupBySql + AExpandedHavingSql +
    AExpandedOrderBySql;
end;

function TdxFirebirdConnectionProvider.FormatOrder(const ASortProperty: string;
...
function TdxFirebirdConnectionProvider.FormatInsert(const ATableName,
...
function TdxFirebirdConnectionProvider.FormatInsertDefaultValues
  (ATable: TdxDBTable): string;
begin
  Result := 'insert into ' + FormatTableSafe(ATable) + ' values()';
end;

function TdxFirebirdConnectionProvider.FormatUpdate(const ATableName,
...
function TdxFirebirdConnectionProvider.FormatDelete(const ATableName,
...
function TdxFirebirdConnectionProvider.GetStorageTableNames
...
begin
  ...
  AQuery := TdxQuery.Create
    ('select RDB$RELATION_NAME, RDB$VIEW_BLR from RDB$RELATIONS where RDB$SYSTEM_FLAG = 0');
  try
    ...
    try
      ...
      begin
        if not AIncludeViews and not VarIsNull(GetFieldValue(ADataSet.Fields[1]))
        then
        else
...
function TdxFirebirdConnectionProvider.GetParameterName(const AParameter
...
procedure TdxFirebirdConnectionProvider.GetTableSchema(ATable: TdxDBTable;
  ACheckIndexes: Boolean; ACheckForeignKeys: Boolean);
begin
  GetColumns(ATable);
...
function TdxFirebirdConnectionProvider.IsSequenceExists(const ASequenceName
  : string): Boolean;
var
  AQuery: TdxQuery;
begin
...
class function TdxFirebirdConnectionProvider.GetConnectionParameters
  : TDictionary<string, TdxConnectionParameterValues>;
begin
...
function TdxFirebirdConnectionProvider.ConvertToDBParameter
...
class function TdxFirebirdConnectionProvider.GetColumnTypeByInfo(AType,
  ASubType, AScale, ASize: SmallInt): TdxDBColumnType;
begin
...
class function TdxFirebirdConnectionProvider.GetDBTypes
...
begin
  AQuery := TdxQuery.Create
    ('SELECT rdb$get_context('#$27'SYSTEM'#$27', '#$27'ENGINE_VERSION'#$27') as version from rdb$database');
  try
...
begin
  AQuery := TdxQuery.Create
    ('select r.RDB$FIELD_NAME, f.RDB$FIELD_TYPE, f.RDB$FIELD_SUB_TYPE, f.RDB$FIELD_SCALE, '
    + 'RDB$CHARACTER_LENGTH, r.RDB$NULL_FLAG, r.RDB$DEFAULT_SOURCE from RDB$RELATION_FIELDS r join '
    + 'RDB$FIELDS f on r.RDB$FIELD_SOURCE  = f.RDB$FIELD_NAME where r.RDB$RELATION_NAME = '#$27
    + ComposeSafeTableName(ATable.Name) + #$27);
  try
    ADataSet := SelectData(AQuery);
    try
      ...
      begin
        ...
        AType := GetColumnTypeByInfo(GetFieldValueDef(F1, 0),
          GetFieldValueDef(F2, 0), GetFieldValueDef(F3, 0), ASize);
        if AType <> TdxDBColumnType.String then
          ASize := 0;
        AColumn := TdxDBColumn.Create(GetFieldStringValue(F0), False, '',
          ASize, AType);
...
begin
  AQuery := TdxQuery.Create
    ('select RDB$INDICES.RDB$INDEX_NAME, RDB$INDEX_SEGMENTS.RDB$FIELD_NAME, RDB$INDEX_SEGMENTS.RDB$FIELD_POSITION, RDB$INDICES.RDB$UNIQUE_FLAG fro'
    ...
    + '= '#$27 + ComposeSafeTableName(ATable.Name) +
    #$27' and RDB$INDICES.RDB$FOREIGN_KEY is NULL order by RDB$INDICES.RDB$INDEX_NAME, RDB$INDEX_SEGMENTS.RDB$FIELD_POSITION');
  try
    ADataSet := SelectData(AQuery);
    try
      AIndex := nil;
      ...
      begin
        ...
        begin
          AIndex := TdxDBIndex.Create(GetFieldStringValue(F0),
            [GetFieldStringValue(F1)], AIsUnique);
...
begin
  AQuery := TdxQuery.Create
    ('select RDB$INDEX_SEGMENTS.RDB$FIELD_NAME from RDB$RELATION_CONSTRAINTS join RDB$INDEX_SEGMENTS on RDB$INDEX_SEGMENTS.RDB$INDEX_NAME = RDB$RE'
    + 'LATION_CONSTRAINTS.RDB$INDEX_NAME where RDB$RELATION_NAME = '#$27 +
    ComposeSafeTableName(ATable.Name) +
    #$27' and RDB$CONSTRAINT_TYPE = '#$27'PRIMARY KEY'#$27' order by RDB$I' +
  ...
  try
    ...
    try
      ...
      try
        while not ADataSet.Eof do
        begin
          AColumn := ATable.GetColumn
            (VarToStr(GetFieldStringValue(ADataSet.Fields[0])));
          if AColumn <> nil then
          begin
            AColumn.IsKey := True;
...
function TdxFirebirdConnectionProvider.GetFieldStringValue
...
begin
  if AField is TAggregateField then
    Result := AField.Value
  else if AField.IsNull then
    Result := Null
  else
    Result := AField.Value;
end;

function TdxFirebirdConnectionProvider.GetFieldValueDef(AField: TField;
...
begin
  AForeignKey := nil;
  AQuery := TdxQuery.Create
    ('select p.RDB$FIELD_POSITION, p.RDB$FIELD_NAME, f.RDB$FIELD_NAME, pt.RDB$RELATION_NAME'#13#10
    + 'from RDB$INDICES i'#13#10 +
    'join RDB$INDEX_SEGMENTS f on f.RDB$INDEX_NAME = i.RDB$INDEX_NAME'#13#10 +
    ...
    'where i.RDB$RELATION_NAME = '#$27 + ComposeSafeTableName(ATable.Name) +
    #$27' and i.RDB$FOREIGN_KEY is NOT NULL'#13#10 +
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          AForeignKey := TdxDBForeignKey.Create
            ([GetFieldValue(ADataSet.Fields[2])],
            GetFieldValue(ADataSet.Fields[3]),
            [GetFieldValue(ADataSet.Fields[1])]);
          ATable.ForeignKeys.Add(AForeignKey);
        end
        else if AForeignKey <> nil then
        begin
          AForeignKey.Columns.Add
            (FormatColumn(GetFieldValue(ADataSet.Fields[2])));
...
function TdxFirebirdConnectionProvider.GetIdentity(ARoot: TdxInsertStatement;
...
begin
  AQuery := TdxQuery.Create('select GEN_ID(' +
    FormatTable(GetSeqName(ARoot.Table)) + ', 1) from RDB$DATABASE');
  try
...
function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForBoolean
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char(1)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(3,0)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForSByte
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(3,0)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForChar
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char CHARACTER SET UNICODE_FSS';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForDecimal
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'decimal(18,4)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForDouble
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'double precision';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForSingle
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'float';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'integer';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForUInt32
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(10,0)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'smallint';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForUInt16
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(5,0)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'bigint';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForUInt64
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'numeric(18,0)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForString
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  if (AColumn.Size > 0) and (AColumn.Size <= MaximumStringSize) then
    Result := 'char varying (' + IntToStr(AColumn.Size) +
      ') CHARACTER SET UNICODE_FSS'
  else if AColumn.Size = 0 then
    Result := 'char varying (' + IntToStr(DefaultStringSize) +
      ') CHARACTER SET UNICODE_FSS'
  else
    Result := 'BLOB SUB_TYPE TEXT';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForDateTime
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'timestamp';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForGuid
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'char(36)';
end;

function TdxFirebirdConnectionProvider.GetSQLCreateColumnTypeForByteArray
  (ATable: TdxDBTable; AColumn: TdxDBColumn): string;
begin
  Result := 'blob';
end;

function TdxFirebirdConnectionProvider.CollectTablesToCreate
  (const ATables: TArray<TdxDBTable>): TArray<TdxDBTable>;
const
  AQueryStr =
    'select RDB$RELATION_NAME, RDB$VIEW_BLR from RDB$RELATIONS where RDB$RELATION_NAME in (%s) and RDB$SYSTEM_FLAG = 0';
var
  I: Integer;
  AQuery: TdxQuery;
  ADataSet: TDataSet;
  ...
  try
    ...
    begin
      ...
      AParameters.Add(TdxOperandValue.Create
        (ComposeSafeTableName(ATables[I].Name)));
    end;
    AQuery := TdxQuery.Create(Format(AQueryStr, [AStrList.CommaText]),
      AParameters, AParamNames.ToStringArray);
    try
      ADataSet := SelectData(AQuery);
      try
        while not ADataSet.Eof do
        begin
          ADBTables.AddOrSetValue
            (VarToStr(GetFieldStringValue(ADataSet.Fields[0])),
            not ADataSet.Fields[1].IsNull);
          ADataSet.Next;
        end;
      finally
        ADataSet.Free;
      ...
      begin
        if not ADBTables.TryGetValue(ComposeSafeTableName(ATable.Name), AValue)
        then
          AList.Add(ATable)
        else
