...
uses
  Types, Classes, SysUtils, TypInfo, Generics.Defaults, Generics.Collections,
  SyncObjs, RTTI, DB,
  dxEMF.Types,
  dxEMF.DB.Model,
  dxEMF.Utils,
...
type
  ...
  TdxObjectCreatingEvent = procedure(Sender: TObject; AClass: TClass;
...
{$IFDEF CXTTEST}
  ...
  IdxPersistentObject = interface
    ...
    procedure DoChange(const AMemberName: string;
      const AOldValue, ANewValue: TValue); overload;
  end;
...
{$ENDIF}
  ...
  public
    ...
    class function Create<T: class>(AObject: TObject;
      const APropertyName: string = ''): IdxEMFCollection<T>; overload; static;
  ...
  public
    constructor Create(const AQueries: TArray<TdxObjectsQuery>;
      const AResult: TArray<TValue>;
      const AExpanded: TArray<TdxExpandedCriteriaHolder>;
      const ARequiresResorting: TArray<Boolean>;
      const ALoadDataQuery: TArray<TdxObjectsQuery>); overload;
    constructor Create(const AQueries: TArray<TdxObjectsQuery>;
      const AResult: TArray<TValue>;
      const AExpanded: TArray<TdxExpandedCriteriaHolder>;
      const ARequiresResorting: TArray<Boolean>); overload;
  ...
  protected type
    TCollectionProperty = (DeleteObjectOnRemove,
      // reserved for future use
      CaseSensitive, SelectDeleted, LoadingEnabled,
      // internal
      IsEntity, IsPersistentClass, IsReadOnly, IsLoading, IsLoaded);
  ...
  protected
    ...
    function GetCollectionProperties(const Index: TCollectionProperty)
      : Boolean; inline;
    procedure SetCollectionProperties(const Index: TCollectionProperty;
      const Value: Boolean); inline;
    function CreateLoader(AObjectsQuery: TdxObjectsQuery;
      APredicate: TdxPredicate = nil): TdxDataLoader; overload;
    ...
    property CollectionProperties: TCollectionProperties
      read FCollectionProperties;
    property IsReadOnly: Boolean index TCollectionProperty.IsReadOnly
      read GetCollectionProperties write SetCollectionProperties;
    property IsLoaded: Boolean index TCollectionProperty.IsLoaded
      read GetCollectionProperties write SetCollectionProperties;
    property IsLoading: Boolean index TCollectionProperty.IsLoading
      read GetCollectionProperties write SetCollectionProperties;
    property LoadingEnabled: Boolean index TCollectionProperty.LoadingEnabled
      read GetCollectionProperties write SetCollectionProperties;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Init(ASession: TdxEMFCustomSession); overload; virtual;
    procedure Init(ASession: TdxEMFCustomSession; AOwner: TObject;
      AReferencedProperty: TdxMappingMemberInfo); overload; virtual; abstract;
    ...
    property SkipReturnedObjects: Integer read FSkipReturnedObjects
      write FSkipReturnedObjects;
    property TopReturnedObjects: Integer read FTopReturnedObjects
      write FTopReturnedObjects;
  ...
  public
    ...
    function PatchCriteriaFromUserToFetch(const AFetchCriteria
      : IdxCriteriaOperator): IdxCriteriaOperator; virtual;
  ...
  public
    constructor Create(AParentCollection: TdxEMFCustomCollection;
      AOwnerObject: TObject; AReferencedProperty: TdxMappingMemberInfo);
  ...
  public
    constructor Create(const ASession: IdxSession; ADataSet: TDataSet);
      overload;
    destructor Destroy; override;
  ...
  protected type
    TProc<TC: class> = reference to procedure(const ADBFields: TArray<F>;
      AObject: TC);
    TProc = reference to procedure(const ADBFields: TArray<F>;
      AObject: TObject);
  protected
  ...
  strict private
    function GetFields(AMemberInfo: TdxMappingMemberInfo;
      var AFieldIndex: Integer): TArray<TField>;
  protected
  ...
  published
    property LoadingStrategy: TdxLoadingStrategy read FLoadingStrategy
      write SetLoadingStrategy default TdxLoadingStrategy.Lazy;
  end;
  ...
  protected
    property LockingOption: TdxLockingOption read FLockingOption
      write FLockingOption default TdxLockingOption.None;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
  published
    property Collections: TdxEMFCollectionOptions read FCollections
      write SetCollections;
  end;

  { TdxEMFCustomSession }

  TdxEMFCustomSession = class(TComponent, IdxPersistentValueExtractor,
    IdxQueryProvider, IdxDataContext, IdxSession)
  strict private
  type
    ...
    public
      constructor Create(AEntityInfo: TdxEntityInfo;
        const AKey: TValue); overload;
      constructor Create(AEntityInfo: TdxEntityInfo;
        const AKeys: array of TValue); overload;
    ...
    protected
      ...
      function CreateQueryGeneric(const AExpression: IdxExpression)
        : IdxQueryable<T>;
  ...
  strict private
    ...
    FCollectionsMarkedSaved: TDictionary<TdxReferencedCollectionHelper,
      TObject>;
    ...
    function GetQueryForFindObject(AEntityInfo: TdxEntityInfo;
      const ACriteria: IdxCriteriaOperator; ASelectDeleted: Boolean)
      : TdxObjectsQuery;
    function GetQueriesByKeys(const AGetQuery: TObjectsByKeyQuery)
      : TObjectList<TdxObjectsQuery>;
    function FilterListForDelete(const AFullListForDelete: TArray<TObject>)
      : TArray<TObject>;
    ...
    procedure ProcessingProcessNextObject(AMarkedObjectsHolder: TdxObjectSet;
      AObject: TObject);
    procedure ProcessingProcessObjectListOrCollection(AMarkedObjectsHolder
      : TdxObjectSet; AObjectOrCollection: TObject);
    ...
    function SelectDataInternal(AObjectsQuery: TdxObjectsQuery;
      const AProperties, AGroupProperties: IdxCriteriaOperatorCollection;
      const AGroupCriteria: IdxCriteriaOperator): TdxQueryData;

    class function CollectClassInfosFormObjects(const AObjects: TArray<TObject>)
      : TDictionary<TdxEntityInfo, TdxEntityInfo>; static;
    class procedure CheckFilteredAggregateDeletion(AObject: TObject;
      AMemberInfo: TdxMappingMemberInfo;
      ACollection: TdxEMFCustomCollection); static;
  protected
    FStateStack: TdxSessionStates;
    procedure AppendObject(AObject: TObject); overload;
    procedure AppendObject(const AKeys: TArray<TValue>;
      AObject: TObject); overload;
    procedure BeginCommitChangesInsideTransaction
      (AContext: TCommitChangesContext; const AFullListForDelete,
      ACompleteListForSave: TArray<TObject>);
    ...
    procedure CommitChanges(const AListForDelete, AListForSave
      : TArray<TObject>);
    ...
    procedure EndCommitChangesInsideTransaction
      (AContext: TCommitChangesContext);
    procedure SetKeyValue(AObject: TObject; const AKeyValue: TValue); overload;
    procedure SetKeyValue(AObject: TObject;
      const AKeyValue: TArray<TValue>); overload;
    ...
    function CanNullifyAssociatedMember(AAssociatedMember: TdxMappingMemberInfo;
      AObjectRemoved: TObject): Boolean;
    function GetObjectsInternal(const AQueries: TArray<TdxObjectsQuery>)
      : TArray<IdxEMFCollection>; virtual;
    ...
    function FindObjectInCache(AClass: TClass;
      const AKeys: TArray<TValue>): TObject;
    function PrepareLoadObjects(const AQueries: TArray<TdxObjectsQuery>)
      : TdxLoadObjectsContext;
    procedure ProcessingProcess(AMarkedObjectsHolder: TdxObjectSet;
      AObject: TObject);
    ...
    function GetObjectsToSave(AIncludeParent: Boolean): TArray<TObject>;
      overload; virtual;
    function GetObjectsToDelete(AIncludeParent: Boolean): TArray<TObject>;
      overload; virtual;
    function Find(AClass: TClass; const AKeys: TArray<TValue>)
      : TObject; overload;

    class function GetPFEntityInfo(AClass: TClass): TdxEntityInfo;
      overload; inline;
    class function GetPFEntityInfo(AObject: TObject): TdxEntityInfo;
      overload; static;
    function GetLoadedObjectByKey(AClass: TClass; const AKey: TValue): TObject;
      overload;
    function GetLoadedObjectByKey(AClass: TClass; const AKeys: TArray<TValue>)
      : TObject; overload;
    function GetLoadedObjectByKey<T: class>(const AKey: TValue): T; overload;
    function GetLoadedObjectByKey<T: class>(const AKeys: TArray<TValue>): T;
      overload;
    function CreateEntity(AEntityInfo: TdxEntityInfo): TObject;

    function GetObjects<T: class>(const ACriteria: IdxCriteriaOperator;
      const ASortByExpressions: IdxSortByExpressions;
      ASkipSelectedRecords: Integer; ATopSelectedRecords: Integer;
      ASelectDeleted: Boolean; AForceReload: Boolean = False)
      : IdxEMFCollection<T>; overload;
    function GetObjects(AClass: TClass; const ACriteria: IdxCriteriaOperator;
      const ASortByExpressions: IdxSortByExpressions;
      ASkipSelectedRecords: Integer; ATopSelectedRecords: Integer;
      ASelectDeleted: Boolean; AForceReload: Boolean = False)
      : IdxEMFCollection; overload;

    function SelectData(AEntityInfo: TdxEntityInfo;
    ...
    procedure RegisterClient(AClient: TObject;
      AEvent: TConnectChangeEvent = nil); virtual;
    ...
    property DeletedObjects: TObjectDictionary<TObject, Integer>
      read FDeletedObjects;

    property DataProvider: TdxEMFCustomDataProvider read FDataProvider
      write SetDataProvider;

    property OnAfterBeginTrackingChanges: TNotifyEvent
      read FOnAfterBeginTrackingChanges write FOnAfterBeginTrackingChanges;
    property OnAfterConnect: TNotifyEvent read FOnAfterConnect
      write FOnAfterConnect;
    property OnAfterDropChanges: TNotifyEvent read FOnAfterDropChanges
      write FOnAfterDropChanges;
    property OnAfterFlushChanges: TNotifyEvent read FOnAfterFlushChanges
      write FOnAfterFlushChanges;
    property OnBeforeBeginTrackingChanges: TNotifyEvent
      read FOnBeforeBeginTrackingChanges write FOnBeforeBeginTrackingChanges;
    property OnBeforeConnect: TNotifyEvent read FOnBeforeConnect
      write FOnBeforeConnect;
    property OnBeforeDropChanges: TNotifyEvent read FOnBeforeDropChanges
      write FOnBeforeDropChanges;
    property OnBeforeFlushChanges: TNotifyEvent read FOnBeforeFlushChanges
      write FOnBeforeFlushChanges;
    property OnObjectCreating: TdxObjectCreatingEvent read FOnObjectCreating
      write FOnObjectCreating;
    property OnObjectDeleted: TdxObjectNotifyEvent read FOnObjectDeleted
      write FOnObjectDeleted;
    property OnObjectDeleting: TdxObjectNotifyEvent read FOnObjectDeleting
      write FOnObjectDeleting;
    property OnObjectLoaded: TdxObjectNotifyEvent read FOnObjectLoaded
      write FOnObjectLoaded;
    property OnObjectLoading: TdxObjectNotifyEvent read FOnObjectLoading
      write FOnObjectLoading;
    property OnObjectSaved: TdxObjectNotifyEvent read FOnObjectSaved
      write FOnObjectSaved;
    property OnObjectSaving: TdxObjectNotifyEvent read FOnObjectSaving
      write FOnObjectSaving;
  public
    ...
    function Find(AClass: TClass; const ACriteria: IdxCriteriaOperator)
      : TObject; overload;
    function GetObjects<T: class>: IdxEMFCollection<T>; overload;
    function GetObjects<T: class>(APredicate: TdxPredicate<T>)
      : IdxEMFCollection<T>; overload;
    function GetObjects<T: class>(const ACriteria: IdxCriteriaOperator)
      : IdxEMFCollection<T>; overload;
    function GetObjects<T: class>(const AQuery: IdxQueryable<T>)
      : IdxEMFCollection<T>; overload;
    function GetObjects<T: class>(const ACriteria: IdxCriteriaOperator;
      const ASortByExpressions: IdxSortByExpressions;
      ASkipSelectedRecords: Integer = 0; ATopSelectedRecords: Integer = 0)
      : IdxEMFCollection<T>; overload;
  ...
  protected
    ...
    property AutoDetectDBEngine: Boolean read FAutoDetectDBEngine
      write SetAutoDetectDBEngine default True;
    property ProviderSQLClass: TdxSQLConnectionProviderClass
      read FProviderSQLClass write SetProviderSQLClass;
  public
    constructor Create(AOwner: TdxEMFCustomDataProvider);
    procedure Assign(Source: TPersistent); override;
  published
    property AutoCreate: TdxAutoCreateOption read FAutoCreate
      write SetAutoCreate default TdxAutoCreateOption.None;
  ...
  protected
    ...
    function CreateConnectionVendor: TdxCustomConnectionVendor;
      virtual; abstract;
    procedure CheckConnectAndCreate; virtual; abstract;
    function CreateProviderSQL(const ADBEngineType: TdxDBEngine)
      : TdxSQLConnectionProvider; virtual;
    procedure CreateTable(const ATableName: string;
      AEntityInfo: TdxEntityInfo); virtual;
    ...
    function ModifyData(const ADMLStatements: TArray<TdxModificationStatement>)
      : TdxModificationResult; overload;
    function ModifyData(const ADMLStatements: array of TdxModificationStatement)
      : TdxModificationResult; overload;

    function CreateLoader(ASession: TdxEMFCustomSession; AClass: TClass;
      APredicate: TdxPredicate = nil): TdxDataLoader; overload;
    function CreateLoader(ASession: TdxEMFCustomSession;
      AObjectsQuery: TdxObjectsQuery; APredicate: TdxPredicate = nil)
      : TdxDataLoader; overload;
    function CreateLoader(ASession: TdxEMFCustomSession;
      AConnectionVendor: TdxCustomConnectionVendor; AQuery: TdxQuery;
      AEntityInfo: TdxEntityInfo; APredicate: TdxPredicate = nil)
      : TdxDataLoader; overload; virtual; abstract;
    ...
    function DoCreateLoader(ASession: TdxEMFCustomSession;
      AEntityInfo: TdxEntityInfo; APredicate: TdxPredicate = nil)
      : TdxDataLoader; overload;
    function DoCreateLoader(ASession: TdxEMFCustomSession;
      AObjectsQuery: TdxObjectsQuery; APredicate: TdxPredicate = nil)
    ...
    procedure GetTableNames(AList: TStrings;
      AIncludeViews: Boolean = False); virtual;
    ...
    property EntityDBModels: TObjectDictionary<TClass, TEntityDBModel>
      read FEntityDBModels;
    property SQLConnectionProvider: TdxSQLConnectionProvider
      read FSQLConnectionProvider;
...
type
  ...
  TdxEntity = class
  public
    class procedure SetSession(ASession: TdxEMFCustomSession;
...
class procedure TdxEntity.SetSession(ASession: TdxEMFCustomSession;
  AObject: TObject);
var
...
class function TdxEMFCollections.Create<T>(AObject: TObject;
  const APropertyName: string): IdxEMFCollection<T>;
var
...
function TdxEMFCustomCollection.CreateLoader(AObjectsQuery: TdxObjectsQuery;
...
function TdxEMFCustomCollection.GetCollectionProperties
  (const Index: TCollectionProperty): Boolean;
begin
...
procedure TdxEMFCustomCollection.SetCollectionProperties
  (const Index: TCollectionProperty; const Value: Boolean);
begin
  if Value then
...
begin
  ...
  if (FLoader <> nil) and (FLoader.LoadingStrategy = TdxLoadingStrategy.Eager)
  then
    DoFetchAll;
end;
...
function TdxEMFCollectionHelper.PatchCriteriaFromUserToFetch
  (const AFetchCriteria: IdxCriteriaOperator): IdxCriteriaOperator;
begin
...
begin
  Result := GetLoadCollectionOnModifyCore or
    (not ParentCollection.LoadingEnabled);
end;
...
constructor TdxReferencedCollectionHelper.Create(AParentCollection
  : TdxEMFCustomCollection; AOwnerObject: TObject;
...
constructor TdxLoadObjectsContext.Create(const AQueries
  : TArray<TdxObjectsQuery>; const AResult: TArray<TValue>;
  const AExpanded: TArray<TdxExpandedCriteriaHolder>;
  const ARequiresResorting: TArray<Boolean>);
begin
  FQueries := AQueries;
...
constructor TdxLoadObjectsContext.Create(const AQueries
  ...
  const AExpanded: TArray<TdxExpandedCriteriaHolder>;
  const ARequiresResorting: TArray<Boolean>;
  const ALoadDataQuery: TArray<TdxObjectsQuery>);
begin
...
constructor TdxDataSetLoader.Create(const ASession: IdxSession;
  ADataSet: TDataSet);
begin
  Create(ASession);
  DataSet := ADataSet;
...
var
  AFieldDef: TFieldDef;
begin
  if (Query.ConstantValues <> nil) and Query.ConstantValues.ContainsKey(AIndex)
  then
...
procedure TdxEMFCollectionOptions.SetLoadingStrategy
  (const Value: TdxLoadingStrategy);
begin
...
procedure TdxEMFSessionOptions.SetCollections(const Value
  : TdxEMFCollectionOptions);
begin
...
constructor TdxEMFCustomSession.TObjectsByKeyQuery.Create
  (AEntityInfo: TdxEntityInfo; const AKey: TValue);
begin
...
constructor TdxEMFCustomSession.TObjectsByKeyQuery.Create
  (AEntityInfo: TdxEntityInfo; const AKeys: array of TValue);
var
  I: Integer;
begin
  FEntityInfo := AEntityInfo;
...
procedure TdxEMFCustomSession.BeginCommitChangesInsideTransaction
  (AContext: TCommitChangesContext; const AFullListForDelete,
  ACompleteListForSave: TArray<TObject>);
var
  ADeleteFirstList, ADeleteLastList, AInsertUpdateList
    : TList<TdxModificationStatement>;
  AStatements: TdxArray<TdxModificationStatement>;
  AStatement: TdxModificationStatement;
begin
  ProcessDeletedObjects(AContext, AFullListForDelete, ACompleteListForSave,
    ADeleteFirstList, ADeleteLastList);
  try
    AInsertUpdateList := AContext.ProcessingSave.Process;
    try
      AStatements := TdxArray<TdxModificationStatement>.Create
        (ADeleteFirstList.Count + AInsertUpdateList.Count +
      ...
      if (AStatements.Count <> 0) and
        (AContext.Session.Options.LockingOption = TdxLockingOption.None) then
...
begin
  Result := nil;
  TdxSessionStateStack.Enter(Self,
  ...
  try
    ...
    TdxSessionStateStack.Leave(Self,
      TdxSessionState.CommitTransactionNonReentrant);
  end;
end;
...
function TdxEMFCustomSession.CanNullifyAssociatedMember(AAssociatedMember
  : TdxMappingMemberInfo; AObjectRemoved: TObject): Boolean;
begin
  if AAssociatedMember.IsKeyOrInKey then
    Exit(False);
  if TdxReferencedCollectionHelper.DoNotSetAssociatedMemberToNullWhenRemovingDeletedObjectFromAssociationList
  then
...
procedure TdxEMFCustomSession.CheckDuplicateObjectInIdentityMap
  (AObject: TObject);
var
...
begin
  ...
  if (ACachedObj <> nil) and not(ACachedObj = AObject) then
    raise EdxDifferentObjectsWithSameKeyException.Create
      (sdxSessionDifferentObjectsWithSameKeyException);
end;

class procedure TdxEMFCustomSession.CheckFilteredAggregateDeletion
  (AObject: TObject; AMemberInfo: TdxMappingMemberInfo;
...
begin
  if (ACollection.Criteria <> nil) or (ACollection.SkipReturnedObjects > 0) or
    (ACollection.TopReturnedObjects > 0) then
  begin
    ACauses := TStringBuilder.Create;
    try
      AMessage := Format(sdxCollectionsWantNotDeleteFilteredAggregateCollection,
        [AObject.ClassName, AMemberInfo.MemberName]);
      AExceptionWouldBeReallyThrown :=
        (TdxAttribute.NoForeignKey in AMemberInfo.AssociatedMember.Attributes);
      if ACollection.Criteria <> nil then
        if AExceptionWouldBeReallyThrown then
          try
            ACauses.Append('.Criteria = '#$27 + TdxCriteriaOperator.ToString
              (ACollection.Criteria as TdxCriteriaOperator) + #$27);
          except
          end
        else
          ACauses.Append('.Criteria');
      if ACollection.SkipReturnedObjects > 0 then
        ACauses.Append('.SkipReturnedObjects = ')
          .Append(ACollection.SkipReturnedObjects);
      if ACollection.TopReturnedObjects > 0 then
        ACauses.Append('.TopReturnedObjects = ')
          .Append(ACollection.TopReturnedObjects);
...
class function TdxEMFCustomSession.CollectClassInfosFormObjects
  (const AObjects: TArray<TObject>): TDictionary<TdxEntityInfo, TdxEntityInfo>;
var
  AObject: TObject;
  AEntityInfo: TdxEntityInfo;
begin
...
procedure TdxEMFCustomSession.CommitChanges(const AListForDelete,
  AListForSave: TArray<TObject>);
var
...
begin
  ...
  try
    ...
    try
      BeginCommitChangesInsideTransaction(AContext, AReadyListForDelete,
        AListForSave);
...
class function TdxEMFCustomSession.GetPFEntityInfo(AClass: TClass)
  : TdxEntityInfo;
begin
...
class function TdxEMFCustomSession.GetPFEntityInfo(AObject: TObject)
  : TdxEntityInfo;
begin
  if AObject = nil then
    Result := nil
  ...
  begin
    ...
    if Result = nil then
      raise EdxNoEntityInfoException.CreateFmt(sdxClassIsNotEntity,
        [AObject.ClassName]);
  end;
end;

function TdxEMFCustomSession.GetProjections(AClass: TClass;
  const AProperties: IdxCriteriaOperatorCollection;
  const ACriteria: IdxCriteriaOperator;
  const AGroupProperties: IdxCriteriaOperatorCollection;
  const AGroupCriteria: IdxCriteriaOperator; ASelectDeleted: Boolean;
  ASkipSelectedRecords, ATopSelectedRecords: Integer;
  const ASortByExpressions: IdxSortByExpressions): IdxEMFCollection;
var
  AEntityInfo: TdxEntityInfo;
...
begin
  ...
  try
    ...
    AQueryData := SelectData(AEntityInfo, AProperties, ACriteria,
      AGroupProperties, AGroupCriteria, { SelectDeleted } False,
      ASkipSelectedRecords, ATopSelectedRecords, ASortByExpressions);
...
begin
  ...
  Supports(TdxLinqExpressionFactory.GetDataContext(ATypeInfo, Self),
    ATypeInfo.TypeData.Guid, Result);
end;
...
begin
  ...
  if AClass = nil then
    raise EdxNoEntityInfoException.CreateFmt(sdxExpressionNotRegistered,
...
function TdxEMFCustomSession.GetQueryForFindObject(AEntityInfo: TdxEntityInfo;
  const ACriteria: IdxCriteriaOperator; ASelectDeleted: Boolean)
  : TdxObjectsQuery;
begin
...
function TdxEMFCustomSession.GetObjectsToSave(AIncludeParent: Boolean)
  : TArray<TObject>;
var
...
function TdxEMFCustomSession.GetObjectsToDelete(AIncludeParent: Boolean)
  : TArray<TObject>;
var
  AMyObjects: TArray<TObject>;
begin
...
begin
  Result := FPoolClasses.IsNewObject(AObject) and
    not FLoadingObjects.Contains(AObject);
end;
...
procedure TdxEMFCustomSession.Notification(AComponent: TComponent;
...
function TdxEMFCustomSession.PrepareLoadObjects(const AQueries
  : TArray<TdxObjectsQuery>): TdxLoadObjectsContext;
var
...
begin
  ...
  begin
    ...
    AExpanded[I] := TdxPersistentCriterionExpander.ExpandToLogical(Self,
      ALoadEntityInfo, AQueries[I].Criteria, True);
    if AExpanded[I]
      .IsFalse { or (Session.UpdateSchema(True, ALoadClassInfo) = UpdateSchemaResult.FirstTableNotExists) }
    then
      AResult[I] := 0
    else
    begin
      ...
      begin
        ...
        begin
          ...
          AExpandedSort := TdxPersistentCriterionExpander.ExpandToValue(Self,
            ALoadEntityInfo, AOrderByExpression, True);
          ...
          begin
            ...
            ASortByExpressions.Add
              (TdxSortByExpression.Create
              (TdxExpandedCriteriaHolder.IfNeededConvertToBoolOperator
              (AExpandedSort.ExpandedCriteria),
              ASortingExpression.SortDirection));
        end;
      end;
      ASkipPhysicallySelected := AQueries[I].SkipSelectedRecords;
      ATopPhysicallySelected := AQueries[I].TopSelectedRecords;
      if ((ATopPhysicallySelected + ASkipPhysicallySelected <> 0) and
        AExpanded[I].RequiresPostProcessing) or ARequiresResorting[I] then
      begin
        ATopPhysicallySelected := 0;
        ASkipPhysicallySelected := 0;
      end;
      ALoadDataQuery.Add(TdxObjectsQuery.Create(ALoadEntityInfo,
        AExpanded[I].ExpandedCriteria, ASortByExpressions,
        ASkipPhysicallySelected, ATopPhysicallySelected,
        AQueries[I].CollectionCriteriaPatcher, AQueries[I].Force));
    end;
  end;
  if ALoadDataQuery.Count = 0 then
    Result := TdxLoadObjectsContext.Create(AQueries, AResult, nil, nil)
  else
    Result := TdxLoadObjectsContext.Create(AQueries, AResult, AExpanded,
      ARequiresResorting, ALoadDataQuery.ToArray);
end;
  ...
  try
      ...
      begin
        ...
        begin
          ...
          if ARefObject.IsNullOrEmpty then
            Continue;
          if IsNewObject(ARefObject.AsObject) or
            ((AMi.IsAggregated and not IsUnitOfWork)) then
            Save(ARefObject.AsObject);
        end;
        for AMi in AEntityInfo.CollectionProperties do
        begin
          ACollection := TdxEMFCustomCollection(AMi.GetValue(AObject)
            .AsInterface);
          if ACollection = nil then
            Continue;
          AReferencedCollectionHelper :=
            Safe<TdxReferencedCollectionHelper>.Cast(ACollection.Helper);
    ...
    begin
      ...
      begin
        ...
        if ACollection = nil then
          Continue;
        AReferencedCollectionHelper := Safe<TdxReferencedCollectionHelper>.Cast
          (ACollection.Helper);
        if AReferencedCollectionHelper <> nil then
          AReferencedCollectionHelper.Save;
      end;
...
procedure TdxEMFCustomSession.ProcessDeletedObjects
  (AContext: TCommitChangesContext; const AFullListForDelete,
  ACompleteListForSave: TArray<TObject>;
  out ABeforeInserts, AAfterUpdates: TList<TdxModificationStatement>);
var
  ADeleteBeforeClassInfos, ABadClassInfos
    : TDictionary<TdxEntityInfo, TdxEntityInfo>;
  AChanges: Boolean;
  ...
  try
    ...
    try
      for AObject in AFullListForDelete do
        if ADeleteBeforeClassInfos.ContainsKey
          (AContext.Session.GetPFEntityInfo(AObject)) then
          AObjectsDeletedFirst.Add(AObject)
        else
          AObjectsDeletedLast.Add(AObject);
      ABeforeInserts := TdxDeleteQueryGenerator.GenerateDelete
        (AObjectsDeletedFirst, Options.LockingOption, AContext.BatchWideData);
      AAfterUpdates := TdxDeleteQueryGenerator.GenerateDelete
        (AObjectsDeletedLast, Options.LockingOption, AContext.BatchWideData);
    finally
...
procedure TdxEMFCustomSession.ProcessingProcess(AMarkedObjectsHolder
  : TdxObjectSet; AObject: TObject);
begin
...
procedure TdxEMFCustomSession.ProcessingProcessNextObject(AMarkedObjectsHolder
  : TdxObjectSet; AObject: TObject);
begin
...
procedure TdxEMFCustomSession.ProcessingProcessObjectListOrCollection
  (AMarkedObjectsHolder: TdxObjectSet; AObjectOrCollection: TObject);
begin
  ProcessingProcessNextObject(AMarkedObjectsHolder, AObjectOrCollection);
end;

procedure TdxEMFCustomSession.RegisterClient(AClient: TObject;
  AEvent: TConnectChangeEvent = nil);
begin
  FClients.Add(AClient);
  FConnectEvents.Add(TMethod(AEvent).Code);
end;

procedure TdxEMFCustomSession.RegisterCollection(ACollection
  : TdxEMFCustomCollection);
begin
...
begin
  ...
  if GetObjects(TClass(AObject.ClassType),
    TdxBaseObjectQueryGenerator.BuildKeyCriteria(AObject), nil, 0, 1, True,
    True).Count = 0 then
    raise EdxCannotLoadObjectsException.Create(AObject.ClassName + '(' +
      GetKeyValue(AObject).ToString + ')');
end;
...
procedure TdxEMFCustomSession.AppendObject(const AKeys: TArray<TValue>;
  AObject: TObject);
begin
...
procedure TdxEMFCustomSession.DeleteCore(AEntityInfo: TdxEntityInfo;
  AObject: TObject);
var
  AMemberInfo: TdxMappingMemberInfo;
...
begin
  for AMemberInfo in AEntityInfo.AssociationListProperties do
    ...
    begin
      ...
      begin
        ...
        AEMFCollection := Safe<TdxEMFObjectCollection>.Cast
          (ACollection as TObject);
  ...
  begin
    ...
    begin
      ...
      begin
        AInterface := AMemberInfo.AssociatedMember.GetValue(AAssociatedObject)
          .AsInterface;
...
procedure TdxEMFCustomSession.DoCreateObject(AClass: TClass;
  out AObject: TObject);
var
  AFunc: TFunc<TObject>;
begin
...
begin
  ...
  if not IsUnitOfWork and not TrackingChanges then
    raise EdxTransactionSequenceException.Create
      (sdxSessionTranSequenceRollback);
...
procedure TdxEMFCustomSession.EndCommitChangesInsideTransaction
  (AContext: TCommitChangesContext);
begin
  AContext.ProcessingSave.ProcessResults(AContext.Result);
end;

procedure TdxEMFCustomSession.EndFlushChanges(const AObjectsToFireSaved
  : TArray<TObject>);
var
...
begin
  ...
  if AEntityInfo = nil then
    Exit(nil);
  Result := TdxLinqExpressionFactory.GetEntityMetadata
    (AEntityInfo.ClassAttributes.PersistentClass, Self);
end;
...
procedure TdxEMFCustomSession.EntityInfoNotification(ASender: TObject;
  AConnecting: Boolean);
var
  AEntityInfo: TdxEntityInfo;
  AEntityClass: TClass;
...
function TdxEMFCustomSession.ExtractPersistentValue(const ACriterionValue
  : TValue): TValue;
begin
...
function TdxEMFCustomSession.SelectData(AEntityInfo: TdxEntityInfo;
  const AProperties: IdxCriteriaOperatorCollection;
  const ACriteria: IdxCriteriaOperator;
  const AGroupProperties: IdxCriteriaOperatorCollection;
  const AGroupCriteria: IdxCriteriaOperator; ASelectDeleted: Boolean;
...
begin
  CheckConnection;
  AQuery := TdxObjectsQuery.Create(AEntityInfo, ACriteria, ASortByExpressions,
    ASkipSelectedRecords, ATopSelectedRecords,
    TdxCollectionCriteriaPatcher.Create(ASelectDeleted), True);
  try
    Result := SelectDataInternal(AQuery, AProperties, AGroupProperties,
      AGroupCriteria);
  finally
    AQuery.Free;
  end;
...
function TdxEMFCustomSession.SelectDataInternal(AObjectsQuery: TdxObjectsQuery;
  const AProperties, AGroupProperties: IdxCriteriaOperatorCollection;
  const AGroupCriteria: IdxCriteriaOperator): TdxQueryData;
var
...
begin
  ARoot := TdxClientSelectSqlGenerator.GenerateSelect(AObjectsQuery.EntityInfo,
    AObjectsQuery.Criteria, AProperties, AObjectsQuery.Sorting,
    AGroupProperties, AGroupCriteria, AObjectsQuery.CollectionCriteriaPatcher,
    AObjectsQuery.SkipSelectedRecords, AObjectsQuery.TopSelectedRecords);
  try
    ASQLGenerator := TdxSelectSQLGenerator.Create
      (DataProvider.SQLConnectionProvider);
    try
      AQuery := ASQLGenerator.GenerateSQL(ARoot);
      ADataLoader := DataProvider.CreateLoader(Self,
        DataProvider.ConnectionVendor, AQuery, AObjectsQuery.EntityInfo);
      ADataLoader.Query := AQuery;
      Result := TdxQueryData.Create(AObjectsQuery.EntityInfo,
        ARoot.Operands.ToArray, ADataLoader);
    finally
      ASQLGenerator.Free;
    end;
  finally
...
procedure TdxEMFCustomSession.TriggerObjectsSaved(const AObjects
  : TArray<TObject>);
begin
end;
...
procedure TdxEMFCustomSession.UnregisterCollection
  (ACollection: TdxEMFCustomCollection);
begin
  FLinkedCollections.Remove(ACollection);
end;
...
begin
  TdxEntityManager.RegisterEntityClient((ASender as TdxCustomEntityPool)
    .EntityInfo, Self, EntityInfoNotification);
end;

function TdxEMFCustomSession.CreateQuery(const AExpression: IdxExpression)
  : IdxQueryable;
var
...
begin
  ...
  Result := GetObjects(ALinqQuery.FromClause.FromClass, ACriteria,
    ALinqQuery.OrderByClause, ALinqQuery.SkipSelectedRecords,
    ALinqQuery.TopSelectedRecords) as IdxQueryable;
end;

function TdxEMFCustomSession.CustomCreate<T>(AFunc: TFunc<T>)
  : TdxEMFCustomSession;
begin
...
function TdxEMFCustomSession.CreateLoader(AObjectsQuery: TdxObjectsQuery;
  APredicate: TdxPredicate): TdxDataLoader;
begin
...
function TdxEMFCustomSession.FilterListForDelete(const AFullListForDelete
  : TArray<TObject>): TArray<TObject>;
var
  AObject: TObject;
  J: Integer;
begin
...
function TdxEMFCustomSession.Find(AClass: TClass;
  const ACriteria: IdxCriteriaOperator): TObject;
var
  AEntityInfo: TdxEntityInfo;
...
function TdxEMFCustomSession.FindBatch(AClass: TClass;
  const AKeys: TArray<TValue>): TObject;
begin
...
function TdxEMFCustomSession.Find(AClass: TClass;
  APredicate: TdxPredicate): TObject;
var
  ALoader: TdxDataLoader;
begin
  BeginSelect;
...
begin
  ...
  try
    CheckConnection;
    AEntityInfo := GetPFEntityInfo(AClass);
    AObjectsQueries := GetQueriesByKeys
      (TObjectsByKeyQuery.Create(AEntityInfo, AKey));
    try
...
function TdxEMFCustomSession.GetQueriesByKeys(const AGetQuery
  : TObjectsByKeyQuery): TObjectList<TdxObjectsQuery>;
var
...
begin
  ...
  AKeyProperty := TdxOperandProperty.Create(AGetQuery.EntityInfo.KeyAttributes
    [0].MemberName);
  ...
  begin
    Result.Add(TdxObjectsQuery.Create(AGetQuery.EntityInfo,
      TdxBinaryOperator.Create(AKeyProperty,
      TdxOperandValue.Create(AGetQuery.Keys[0]), TdxBinaryOperatorType.Equal),
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          Result.Add(TdxObjectsQuery.Create(AGetQuery.EntityInfo,
            TdxInOperator.Create(AKeyProperty, AIdList), nil, 0, 0,
...
function TdxEMFCustomSession.GetObjectsInternal(const AQueries
  : TArray<TdxObjectsQuery>): TArray<IdxEMFCollection>;
begin
end;
...
function TdxEMFCustomSession.GetObjects(AClass: TClass;
  const ACriteria: IdxCriteriaOperator): IdxEMFCollection;
begin
...
function TdxEMFCustomSession.GetObjects(AClass: TClass;
  const ACriteria: IdxCriteriaOperator;
  const ASortByExpressions: IdxSortByExpressions;
  ASkipSelectedRecords, ATopSelectedRecords: Integer; ASelectDeleted: Boolean;
  AForceReload: Boolean): IdxEMFCollection;
var
  AEntityInfo: TdxEntityInfo;
  AObjectsQuery: TdxObjectsQuery;
  ALoader: TdxDataLoader;
...
begin
  ...
  try
    CheckConnection;
    AEntityInfo := GetPFEntityInfo(AClass);
    AObjectsQuery := TdxObjectsQuery.Create(AEntityInfo, ACriteria,
      ASortByExpressions, ASkipSelectedRecords, ATopSelectedRecords,
...
function TdxEMFCustomSession.GetObjects<T>(const ACriteria: IdxCriteriaOperator;
  const ASortByExpressions: IdxSortByExpressions;
  ASkipSelectedRecords, ATopSelectedRecords: Integer; ASelectDeleted: Boolean;
  AForceReload: Boolean): IdxEMFCollection<T>;
var
  AEntityInfo: TdxEntityInfo;
  AObjectsQuery: TdxObjectsQuery;
  ALoader: TdxDataLoader;
...
function TdxEMFCustomSession.GetObjects<T>(APredicate: TdxPredicate<T>)
  : IdxEMFCollection<T>;
var
  ALoader: TdxDataLoader;
  AList: TdxEMFEntityCollection<T>;
...
function TdxEMFCustomSession.GetObjects<T>(const AQuery: IdxQueryable<T>)
  : IdxEMFCollection<T>;
var
  ALinqQuery: IdxLinqQueryBuilder;
  ACriteria: IdxCriteriaOperator;
...
function TdxEMFCustomSession.GetLoadedObjectByKey(AClass: TClass;
  const AKey: TValue): TObject;
begin
...
function TdxEMFCustomSession.GetLoadedObjectByKey<T>
  (const AKeys: TArray<TValue>): T;
begin
...
procedure TdxEMFDataProviderOptions.SetAutoCreate
  (const Value: TdxAutoCreateOption);
begin
...
procedure TdxEMFDataProviderOptions.SetProviderSQLClass
  (const Value: TdxSQLConnectionProviderClass);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      else if (AExistsTable.Columns.Count < ATable.Columns.Count) then
        Remove(AExistsTable)
    end;
    Add(ATable);
  end;
...
function TdxEMFCustomDataProvider.TTableList.GetTableByName(const ATableName
  : string): TdxDBTable;
var
  I: Integer;
begin
...
begin
  ...
  FEntityDBModels := TObjectDictionary<TClass, TEntityDBModel>.Create
    ([doOwnsValues]);
...
function TdxEMFCustomDataProvider.DoCreateLoader(ASession: TdxEMFCustomSession;
  AObjectsQuery: TdxObjectsQuery; APredicate: TdxPredicate): TdxDataLoader;
var
  ARoot: TdxSelectStatement;
  ASQLGenerator: TdxSelectSQLGenerator;
  AQueryData: TdxQueryData;
...
begin
  ...
  ARoot := TdxClientSelectSqlGenerator.GenerateSelect(AObjectsQuery.EntityInfo,
    AObjectsQuery.Criteria, AQueryData.Properties, AObjectsQuery.Sorting, nil,
    nil, AObjectsQuery.CollectionCriteriaPatcher,
    AObjectsQuery.SkipSelectedRecords, AObjectsQuery.TopSelectedRecords);
  try
    ...
    try
      AQuery := ASQLGenerator.GenerateSQL(ARoot);
      try
        Result := CreateLoader(ASession, ConnectionVendor, AQuery,
          AObjectsQuery.EntityInfo, APredicate);
...
function TdxEMFCustomDataProvider.DoCreateLoader(ASession: TdxEMFCustomSession;
  AEntityInfo: TdxEntityInfo; APredicate: TdxPredicate): TdxDataLoader;
var
  AObjectsQuery: TdxObjectsQuery;
begin
...
begin
  Result := (Options.AutoCreate = TdxAutoCreateOption.SchemaOnly) or
    CanCreateDatabase;
end;

function TdxEMFCustomDataProvider.GetDBSchema(ASortByName: Boolean = False)
  : TObjectList<TdxDBTable>;
var
  ATables: TStringList;
  ADBTable: TdxDBTable;
  I: Integer;
...
function TdxEMFCustomDataProvider.CreateLoader(ASession: TdxEMFCustomSession;
  AClass: TClass; APredicate: TdxPredicate = nil): TdxDataLoader;
var
  AEntityInfo: TdxEntityInfo;
begin
  CheckSQLProvider;
...
function TdxEMFCustomDataProvider.GetEntityDBModel(AClass: TClass)
  : TEntityDBModel;
begin
...
function TdxEMFCustomDataProvider.GetQueryData(AEntityInfo: TdxEntityInfo)
  : TdxQueryData;
begin
...
procedure TdxEMFCustomDataProvider.UnregisterSession
  (ASession: TdxEMFCustomSession);
begin
...
function TdxEMFCustomDataProvider.CreateProviderSQL(const ADBEngineType
  : TdxDBEngine): TdxSQLConnectionProvider;
var
  AProviderSQLClass: TdxSQLConnectionProviderClass;
begin
  if (ADBEngineType = TdxDBEngines.Unknown) or
    (ADBEngineType = TdxDBEngines.Empty) then
    Exit(nil);
  AProviderSQLClass := TdxSQLConnectionProviderFactory.GetProviderSQL
    (ADBEngineType);
  if AProviderSQLClass = nil then
    raise EdxUnknownProviderSQL.CreateFmt(sdxUnknownProviderSQL,
      [ADBEngineType]);
  Result := AProviderSQLClass.Create(ConnectionVendor, Options.AutoCreate);
end;

procedure TdxEMFCustomDataProvider.CreateTable(const ATableName: string;
  AEntityInfo: TdxEntityInfo);
var
  ADBTables: TObjectList<TdxDBTable>;
  ADBTable: TdxDBTable;
...
procedure TdxEMFCustomDataProvider.GetTableNames(AList: TStrings;
  AIncludeViews: Boolean);
begin
  AList.AddStrings(SQLConnectionProvider.GetStorageTableNames(AIncludeViews));
end;

procedure TdxEMFCustomDataProvider.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
function TdxEMFCustomDataProvider.ModifyData(const ADMLStatements
  : TArray<TdxModificationStatement>): TdxModificationResult;
begin
  CheckSQLProvider;
...
function TdxEMFCustomDataProvider.ModifyData(const ADMLStatements
  : array of TdxModificationStatement): TdxModificationResult;
begin
  CheckSQLProvider;
  Result := SQLConnectionProvider.ModifyData(ADMLStatements);
end;
...
procedure TdxEMFCustomDataProvider.ConnectionStateChange(ASender: TObject;
  AConnecting: Boolean);
var
  ASession: TdxEMFCustomSession;
begin
...
procedure TdxEMFCustomDataProvider.SetConnection(const Value
  : TCustomConnection);
begin
  ...
  begin
    ...
    TCustomConnectionAccess(FConnection).RegisterClient(Self,
...
procedure TdxEMFCustomDataProvider.SetOptions(const Value
  : TdxEMFDataProviderOptions);
begin
  FOptions.Assign(Value);
end;
...
begin
  ...
  begin
    ...
    if ((Result = TdxDBEngines.Unknown) or (Result = TdxDBEngines.Empty)) and
      Options.AutoDetectDBEngine then
      Result := TdxCustomConnectionVendorAccess(ConnectionVendor)
        .GetExpectedDBEngine;
  end;
end;
...
procedure TdxEMFCustomDataProvider.RegisterSession
  (ASession: TdxEMFCustomSession);
begin
...
function TdxEMFCustomSession.FindObjectInCache(AClass: TClass;
  const AKeys: TArray<TValue>): TObject;
begin
...
function TdxEMFCustomSession.GetObjects(const AQuery: IdxQueryable)
  : IdxEMFCollection;
var
  ALinqQuery: IdxLinqQueryBuilder;
  ACriteria: IdxCriteriaOperator;
begin
  ...
  Result := GetObjects(ALinqQuery.FromClause.FromClass, ACriteria,
    ALinqQuery.OrderByClause, ALinqQuery.SkipSelectedRecords,
    ALinqQuery.TopSelectedRecords);
end;

function TdxEMFCustomSession.GetObjects(AClass: TClass;
  const ACriteria: IdxCriteriaOperator;
  const ASortByExpressions: IdxSortByExpressions;
  ASkipSelectedRecords, ATopSelectedRecords: Integer): IdxEMFCollection;
begin
  Result := GetObjects(AClass, ACriteria, ASortByExpressions,
    ASkipSelectedRecords, ATopSelectedRecords, False, False);
end;

function TdxEMFCustomSession.GetObjects<T>(const ACriteria: IdxCriteriaOperator;
  const ASortByExpressions: IdxSortByExpressions;
  ASkipSelectedRecords, ATopSelectedRecords: Integer): IdxEMFCollection<T>;
begin
  Result := GetObjects<T>(ACriteria, ASortByExpressions, ASkipSelectedRecords,
    ATopSelectedRecords, False, False);
end;

{ TdxEMFCustomSession.TCommitChangesContext }

constructor TdxEMFCustomSession.TCommitChangesContext.Create
  (ASession: TdxEMFCustomSession;
  ABatchWideData: TdxBatchWideDataHolderForModification);
begin
  FSession := ASession;
...
constructor TdxEMFCustomSession.TQueryProvider<T>.Create
  (ASession: TdxEMFCustomSession);
begin
  inherited Create;
  FSession := ASession;
...
function TdxEMFCustomSession.TQueryProvider<T>.CreateQuery(const AExpression
  : IdxExpression): IdxQueryable;
begin
  Result := FSession.CreateQuery(AExpression);
end;

function TdxEMFCustomSession.TQueryProvider<T>.CreateQueryGeneric
  (const AExpression: IdxExpression): IdxQueryable<T>;
var
  ALinqQuery: IdxLinqQueryBuilder;
  ACriteria: IdxCriteriaOperator;
begin
  ALinqQuery := AExpression as IdxLinqQueryBuilder;
  ACriteria := ALinqQuery.WhereClause;
  Result := IdxQueryable<T>(FSession.GetObjects<T>(ACriteria,
    ALinqQuery.OrderByClause, ALinqQuery.SkipSelectedRecords,
    ALinqQuery.TopSelectedRecords));
end;
...
begin
  ...
  for I := 0 to ACount - 1 do
  begin
    AMemberInfo := EntityInfo.PersistentProperties{$IFDEF DELPHIXE3}.
      List{$ENDIF}[I];
    if AMemberInfo.IsDataMember then
      if not(AMemberInfo.
        IsAssociationList { or AMemberInfo.IsAssociationObject } ) then
      begin
        ...
        begin
          if AMemberInfo.IsReference then
            AKeyFieldsCache.Add
              (TFieldCache.Create(AMemberInfo.ReferenceType.KeyProperty.
              Member, AFields))
          else
...
function TdxDataSetLoader<T>.GetFields(AMemberInfo: TdxMappingMemberInfo;
  var AFieldIndex: Integer): TArray<TField>;
var
  I: Integer;
begin
...
begin
  ...
  for I := 0 to Length(FFieldsCache) - 1 do
    ...
    begin
      if FFieldsCache[I].Value[J] <> nil then
        AValues[I] := TdxFieldValueConverter.ToValue(FFieldsCache[I].Value[J],
          FFieldsCache[I].Key)
      else
...
begin
  ...
  for I := 0 to Length(FKeyFieldsCache) - 1 do
    ...
    begin
      Result[K] := TdxFieldValueConverter.ToValue(FKeyFieldsCache[I].Value[J],
        FKeyFieldsCache[I].Key);
      Inc(K);
    end;
end;
...
begin
  ...
  begin
    ...
    if AMemberInfo.IsReference then
    begin
      AKeys := TdxFieldValueConverter.ToValues(AFields,
        AMemberInfo.ReferenceType.KeyProperty.Member);
      ARefObject := ASession.FindBatch
        (AMemberInfo.ReferenceType.ClassAttributes.PersistentClass, AKeys);
