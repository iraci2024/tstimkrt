...
uses
  ...
  dxCoreClasses, cxClasses, cxControls, cxGeometry, cxGraphics,
  dxGdiPlusClasses,
  cxLookAndFeelPainters, dxCustomHint, dxScreenTip;

type
  TdxMapControlEnumElementStateProc = procedure
    (AState: TdxMapControlElementState) of object;
  ...
  public
    ...
    function IsElementVisible(AElement: TdxMapControlElementViewInfo)
      : Boolean; virtual;
    ...
    property Items[Index: Integer]: TdxMapControlElementViewInfo
...
function TdxMapControlElementViewInfo.GetItems(Index: Integer)
  : TdxMapControlElementViewInfo;
begin
...
var
  I: Integer;
begin
  Result := (GetHitTestIndex <> mchtNone) and
    PtInElement((AHitTest as TdxMapControlHitTest).HitPoint);
  if Result then
    SetHitTest(AHitTest);
...
procedure TdxMapControlElementViewInfo.MouseMove(Shift: TShiftState;
  X, Y: Integer);
begin
end;
...
function TdxMapControlElementViewInfo.GetForbiddenStates
  : TdxMapControlElementStates;
begin
  Result := [];
end;
...
function TdxMapControlElementViewInfo.IsElementVisible
...
function TdxMapControlElementViewInfo.IsEnoughSpace(const AAvailableBounds
  : TRect): Boolean;
begin
...
function TdxMapControlElementViewInfo.PtInElement(const APoint: TPoint)
  : Boolean;
begin
