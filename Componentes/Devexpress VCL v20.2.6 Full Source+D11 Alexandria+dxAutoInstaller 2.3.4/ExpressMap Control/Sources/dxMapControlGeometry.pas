...
function dxEdgeIsIntersected(const AEdge1, AEdge2: TdxEdge;
  out AAlpha1, AAlpha2: Double; out APoint: TdxPointDouble): Boolean; inline;
function dxIsPtInPolygon(const APoint: TdxPointDouble;
  const APolygon: TdxDoublePoints): Boolean; inline;
procedure dxClipPolygon(const ASubjectPolygon, AClipPolygon: TdxDoublePoints;
  out APolygons: TdxDoublePointsArray);
...
var
  A: Double;
begin
  A := (AEdge2.EndPoint.Y - AEdge2.StartPoint.Y) *
    (AEdge1.EndPoint.X - AEdge1.StartPoint.X) -
    (AEdge2.EndPoint.X - AEdge2.StartPoint.X) *
    (AEdge1.EndPoint.Y - AEdge1.StartPoint.Y);
  Result := A <> 0;
  if Result then
  begin
    AAlpha1 := ((AEdge2.EndPoint.X - AEdge2.StartPoint.X) *
      (AEdge1.StartPoint.Y - AEdge2.StartPoint.Y) -
      (AEdge2.EndPoint.Y - AEdge2.StartPoint.Y) *
      (AEdge1.StartPoint.X - AEdge2.StartPoint.X)) / A;
    AAlpha2 := ((AEdge1.EndPoint.X - AEdge1.StartPoint.X) *
      (AEdge1.StartPoint.Y - AEdge2.StartPoint.Y) -
      (AEdge1.EndPoint.Y - AEdge1.StartPoint.Y) *
      (AEdge1.StartPoint.X - AEdge2.StartPoint.X)) / A;
    ...
    Result := (0 <= AAlpha1) and (AAlpha1 <= 1) and (0 <= AAlpha2) and
      (AAlpha2 <= 1);
    if Result then
    begin
      APoint.X := AEdge1.StartPoint.X + AAlpha1 *
        (AEdge1.EndPoint.X - AEdge1.StartPoint.X);
      APoint.Y := AEdge1.StartPoint.Y + AAlpha1 *
        (AEdge1.EndPoint.Y - AEdge1.StartPoint.Y);
    end;
  end;
end;

procedure GetBoundingBox(const APolygon: TdxDoublePoints;
  out ALeft, ATop, ARight, ABottom: Double); inline;
var
  I: Integer;
begin
  ALeft := APolygon[0].X;
...
begin
  ...
  begin
    ...
    if (APoint.X >= ALeft) and (APoint.X <= ARight) and (APoint.Y >= ATop) and
      (APoint.Y <= ABottom) then
    begin
      AIntersectCount := 0;
      ...
      begin
        ...
        if (AEdge2.StartPoint.Y = AEdge2.EndPoint.Y) and
          ...
          (AEdge1.StartPoint.X <= Max(AEdge2.StartPoint.X, AEdge2.EndPoint.X))
          and (AEdge1.StartPoint.X >= Min(AEdge2.StartPoint.X,
          AEdge2.EndPoint.X)) then
        begin
          Result := True;
          Exit;
        end;
        ...
        begin
          ...
          else if not SameValue(AAlpha2, 0) and not SameValue(AAlpha2, 1) then
            Inc(AIntersectCount)
          else
          begin
            ...
            AEdge2.StartPoint := APolygon[I];
            AEdge2.EndPoint := APolygon[I + 1];
            if dxEdgeIsIntersected(AEdge1, AEdge2, AAlpha1, AAlpha2, APoint1)
            then
              Inc(AIntersectCount);
          end;
        end;
...
function GetUnprocessedIntersectVertex(AVertices: TdxVertices;
  out AVertex: TdxVertex): Boolean; inline;
begin
...
function OffsetDegeneratePoint(const APoint: TdxPointDouble; AEdge: TdxEdge;
  ADelta: Double = 0.1): TdxPointDouble; inline;
var
...
procedure AddPoint(var APolygon: TdxDoublePoints;
  APoint: TdxPointDouble); inline;
begin
...
procedure dxClipPolygon(const ASubjectPolygon, AClipPolygon: TdxDoublePoints;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if dxEdgeIsIntersected(ASEdge, ACEdge, AAlphaS, AAlphaC, AIntersectPoint)
        then
          if not SameValue(AAlphaS, 0) and not SameValue(AAlphaC, 0) and
            not SameValue(AAlphaS, 1) and not SameValue(AAlphaC, 1) then
          begin
            ASIntersectVertex := AddIntersectVertex(ASVertices, ASEndVertex,
              AAlphaS, AIntersectPoint);
            ACIntersectVertex := AddIntersectVertex(ACVertices, ACEndVertex,
              AAlphaC, AIntersectPoint);
          ...
          begin
            ...
            else if SameValue(AAlphaS, 1) then
            begin
            ...
            else if SameValue(AAlphaC, 0) then
            begin
            ...
            else if SameValue(AAlphaC, 1) then
            begin
  ...
  function InternalIsPtInPolygon(const APoint: TdxPointDouble;
    AVertices: TdxVertices): Boolean;
  var
  ...
  procedure MarkEntryAndExitPoints(ASVertices, ACVertices: TdxVertices;
    out AIsSubjectInClip, AIsClipInSubject: Boolean);
  var
    ASVertex, ACVertex: TdxVertex;
    AIsEnter: Boolean;
  begin
  ...
  begin
    ...
    if (APolygonCount = 0) and (AIsClipInSubject or AIsSubjectInClip) then
    begin
  ...
  begin
    ...
    try
      ...
      MarkEntryAndExitPoints(ASVertices, ACVertices, AIsSubjectInClip,
        AIsClipInSubject);
      BuildPolygons(ASVertices, ACVertices, AIsSubjectInClip, AIsClipInSubject,
        APolygons);
    finally
      ASVertices.Free;
      ACVertices.Free;
...
procedure dxClipPolyline(const ASubjectPolyline, AClipPolygon: TdxDoublePoints;
  out APolylines: TdxDoublePointsArray);
      ...
      begin
        ...
        then
          if not SameValue(AAlphaS, 0) and not SameValue(AAlphaS, 1) then
            AddIntersectVertex(ASVertices, ASEndVertex, AAlphaS,
              AIntersectPoint)
          else
          begin
            Result := False;
...
procedure FindTopIntersectPoint(const AStartPoint: TPoint;
  const AClipRect: TRect; K: Double; var AIntersectPoint: TPoint); inline;
begin
  AIntersectPoint.Y := AClipRect.Top;
  AIntersectPoint.X :=
    Round(AStartPoint.X + (AIntersectPoint.Y - AStartPoint.Y) * K);
end;

procedure FindBottomIntersectPoint(const AStartPoint: TPoint;
  const AClipRect: TRect; K: Double; var AIntersectPoint: TPoint); inline;
begin
...
procedure FindLeftIntersectPoint(const AStartPoint: TPoint;
  const AClipRect: TRect; K: Double; var AIntersectPoint: TPoint); inline;
begin
  AIntersectPoint.X := AClipRect.Left;
  AIntersectPoint.Y :=
    Round(AStartPoint.Y + (AIntersectPoint.X - AStartPoint.X) * K);
end;

procedure FindRightIntersectPoint(const AStartPoint: TPoint;
  const AClipRect: TRect; K: Double; var AIntersectPoint: TPoint); inline;
begin
...
function GetIntersectPointWhenStartPointStrictInRect(const AInnerPoint,
  AExtraPoint: TPoint; const AClipRect: TRect): TPoint; inline;
var
  A, B: Integer;
begin
  ...
  else if AExtraPoint.X > AClipRect.Right then
  begin
    ...
    else if Result.Y > AClipRect.Bottom then
      FindBottomIntersectPoint(AInnerPoint, AClipRect, A / B, Result)
  end
  else if AExtraPoint.X < AClipRect.Left then
  begin
  ...
  else if AExtraPoint.Y < AClipRect.Top then
    FindTopIntersectPoint(AInnerPoint, AClipRect, A / B, Result)
  else
    FindBottomIntersectPoint(AInnerPoint, AClipRect, A / B, Result);
end;

function IsIntersectClipRect(const APoint1, APoint2: TPoint;
  const AClipRect: TRect; out AIntersectPoint1, AIntersectPoint2: TPoint)
  : Boolean; inline;
var
  A, B: Integer;
  PL, PR: TPoint;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if AIntersectPoint1.Y >= AClipRect.Bottom then
        begin
          if PL.Y > AIntersectPoint1.Y then
          begin
      ...
      else if PL.Y < AClipRect.Top then
      begin
        FindTopIntersectPoint(PL, AClipRect, AB, AIntersectPoint1);
        Result := AIntersectPoint1.X < AClipRect.Right;
        if Result then
...
function IsIntersectClipRectWhenStartPointOnEdge(const APoint1, APoint2: TPoint;
  const AClipRect: TRect; out AIntersectPoint: TPoint): Boolean; inline;
var
  A, B: Integer;
begin
  Result := False;
  ...
  begin
    ...
    else if APoint1.Y = AClipRect.Top then
    begin
      ...
      else if AIntersectPoint.X < AClipRect.Left then
        FindLeftIntersectPoint(APoint1, AClipRect, B / A, AIntersectPoint);
    end
    else if APoint1.Y = AClipRect.Bottom then
    begin
    ...
    else if APoint1.X = AClipRect.Left then
    begin
      FindRightIntersectPoint(APoint1, AClipRect, B / A, AIntersectPoint);
      if AIntersectPoint.Y < AClipRect.Top then
        FindTopIntersectPoint(APoint1, AClipRect, A / B, AIntersectPoint)
      else if AIntersectPoint.Y > AClipRect.Bottom then
        FindBottomIntersectPoint(APoint1, AClipRect, A / B, AIntersectPoint);
    end
    else
    begin
...
var
  ...
  AIsPointInRect, AIsPointOnEdge, AIsPrevPointInRect,
    AIsPrevPointOnEdge: Boolean;
begin
  APolylineCount := 0;
  APointCount := 0;
  ...
  begin
    ...
    AIsPointInRect := (APoint.X >= AClipRect.Left) and
      (APoint.X <= AClipRect.Right) and (APoint.Y >= AClipRect.Top) and
      (APoint.Y <= AClipRect.Bottom);
    AIsPointOnEdge := AIsPointInRect and
      ((APoint.X = AClipRect.Left) or (APoint.X = AClipRect.Right) or
    ...
    begin
      ...
      begin
        ...
        if I <> 0 then
          ...
          begin
            if IsIntersectClipRectWhenStartPointOnEdge(APoint,
              ASubjectPolyline[I - 1], AClipRect, AIntersectPoint) then
            begin
              Inc(APointCount);
              SetLength(APolylines[APolylineCount - 1], APointCount);
              APolylines[APolylineCount - 1, APointCount - 1] :=
                AIntersectPoint;
            end;
          end
          else
          begin
            AIntersectPoint := GetIntersectPointWhenStartPointStrictInRect
              (APoint, ASubjectPolyline[I - 1], AClipRect);
            Inc(APointCount);
            SetLength(APolylines[APolylineCount - 1], APointCount);
    ...
    else if AIsPrevPointInRect then
      if not AIsPrevPointOnEdge then
      begin
        AIntersectPoint := GetIntersectPointWhenStartPointStrictInRect
          (ASubjectPolyline[I - 1], APoint, AClipRect);
        Inc(APointCount);
        SetLength(APolylines[APolylineCount - 1], APointCount);
        APolylines[APolylineCount - 1, APointCount - 1] := AIntersectPoint;
      end
      ...
      begin
        if IsIntersectClipRectWhenStartPointOnEdge(ASubjectPolyline[I - 1],
          APoint, AClipRect, AIntersectPoint) then
        begin
          Inc(APointCount);
          SetLength(APolylines[APolylineCount - 1], APointCount);
          APolylines[APolylineCount - 1, APointCount - 1] := AIntersectPoint;
    ...
    begin
      if (I <> 0) and IsIntersectClipRect(ASubjectPolyline[I - 1], APoint,
        AClipRect, AIntersectPoint1, AIntersectPoint2) then
      begin
        if (APointCount > 1) or (APolylineCount = 0) then
        begin
          Inc(APolylineCount);
