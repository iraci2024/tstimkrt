...
uses
  ...
  cxGraphics, cxGeometry, cxClasses, dxCoreClasses, dxGDIPlusClasses,
...
type
  ...
  TdxCustomMapItemLayerGetItemHintEvent = procedure
  ...
  protected
    ...
    procedure MapItemsChanged(Sender: TObject;
  ...
  public
    ...
    property AllowHotTrack: Boolean read FAllowHotTrack write FAllowHotTrack
      default True;
    property InitialMapSize: TcxSize read FInitialMapSize
      write SetInitialMapSize stored IsInitialMapSizeStored;
    property ItemHint: string read FItemHint write FItemHint;
    property ItemStyle: TdxMapItemStyle index mcesNormal read GetItemStyle
      write SetItemStyle;
    property ItemStyleHot: TdxMapItemStyle index mcesHot read GetItemStyle
      write SetItemStyle;
    property ItemStyleSelected: TdxMapItemStyle index mcesSelected
      read GetItemStyle write SetItemStyle;
    property MapItems: TdxMapItems read FMapItems write SetMapItems;
    property ItemTitleOptions: TdxMapItemTitleOptions read FItemTitleOptions
      write SetItemTitleOptions;
    property ProjectionClass: TdxMapControlCustomProjectionClass
      read GetProjectionClass write SetProjectionClass;
    property OnGetItemHint: TdxCustomMapItemLayerGetItemHintEvent
      read FOnGetItemHint write FOnGetItemHint;
  published
    property ProjectionClassName: string read GetProjectionClassName
      write SetProjectionClassName;
    property Projection: TdxMapControlCustomProjection read FProjection
      write SetProjection;
  end;
...
begin
  ...
  begin
    ...
    else if Items[I].State = mcesHot then
      AItemHot := Items[I]
    else
  ...
  begin
    ...
    else if ATitle.State = mcesHot then
      ATitleHot := ATitle
    else
...
begin
  ...
  FInitialMapSize := TcxSize.Create(Self, dxMapControlDefaultMapSize,
    dxMapControlDefaultMapSize);
...
function TdxCustomMapItemLayer.AddItem(AMapItemClass: TdxMapItemClass)
  : TdxMapItem;
begin
...
procedure TdxCustomMapItemLayer.GetItemsInRegion(const ARect: TRect;
  AItems: TList);
var
  I: Integer;
begin
...
function TdxCustomMapItemLayer.GetItemStyle(Index: TdxMapControlElementState)
  : TdxMapItemStyle;
begin
  Result := GetStyle(Index);
end;

function TdxCustomMapItemLayer.GetStyle(AState: TdxMapControlElementState)
  : TdxMapItemStyle;
begin
...
function TdxCustomMapItemLayer.GetMapSizeInPixels(AZoomLevel: Double)
  : TdxSizeDouble;
var
  ACoeff, ALevel: Double;
begin
  if AZoomLevel < 1.0 then
    Result := dxSizeDouble(AZoomLevel * InitialMapSize.Width,
      AZoomLevel * InitialMapSize.Height)
  else
  begin
    ALevel := Max(0.0, AZoomLevel - 1.0);
    ACoeff := Power(2.0, ALevel);
    Result := dxSizeDouble(ACoeff * InitialMapSize.Width,
      ACoeff * InitialMapSize.Height);
  end;
end;
...
function TdxCustomMapItemLayer.GetProjectionClass
  : TdxMapControlCustomProjectionClass;
begin
...
begin
  Result := not FInitialMapSize.IsEqual(cxSize(dxMapControlDefaultMapSize,
    dxMapControlDefaultMapSize));
end;
...
procedure TdxCustomMapItemLayer.SetItemStyle(const Index
  : TdxMapControlElementState; const Value: TdxMapItemStyle);
begin
...
procedure TdxCustomMapItemLayer.SetMapItems(const Value: TdxMapItems);
begin
  FMapItems.Assign(Value);
end;

procedure TdxCustomMapItemLayer.SetProjection(const Value
...
begin
  ProjectionClass := TdxMapControlCustomProjectionClass
    (dxRegisteredMapProjections.FindByClassName(Value));
end;
