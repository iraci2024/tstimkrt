...
function TdxMapControlDiskCacheMap.GetItem(const AFileName: string)
...
procedure TdxMapControlDiskCacheMap.PushItem(const AFileName: string;
  ASize: Int64);
var
  AItem: TdxMapControlCacheMapItem;
begin
...
procedure TdxMapControlDiskCacheMap.RemoveItem
  (AItem: TdxMapControlCacheMapItem);
begin
...
function TdxMapControlDiskCacheMap.GetItems(Index: Integer)
  : TdxMapControlCacheMapItem;
begin
...
constructor TdxMapControlDiskCache.Create(AFolder: string; AExpireTime: Int64;
  ADiskLimit: Integer);
begin
  inherited Create;
...
function TdxMapControlDiskCache.TryRetrieveFileName
  (AIndex: TdxMapControlTileIndex; const APrefix: string;
  out AFullFileName: string): Boolean;
var
  AFileName: string;
...
function TdxMapControlDiskCache.GetItemName(AIndex: TdxMapControlTileIndex;
  const APrefix: string): string;
begin
  Result := Format('%s_%d_%d_%d%s', [APrefix, AIndex.X, AIndex.Y, AIndex.Level,
    CacheFileExtension]);
end;
...
begin
  Result := FName + ':' + dxFloatToStr(FTime) + ':' + IntToStr(FSize);
end;
