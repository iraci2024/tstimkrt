...
uses
  Windows, SysUtils, StrUtils, Classes, Types, Generics.Defaults,
...
type
  ...
  public
    ...
    property UserLocation: TdxMapControlGeoPoint read FUserLocation
      write FUserLocation;
    property UserMapView: TdxMapControlGeoRect read FUserMapView
      write FUserMapView;
  end;
  ...
  public
    ...
    property ConnectionProtocol: TdxMapControlConnectionProtocol
      read FConnectionProtocol write FConnectionProtocol;
    property UserIp: string read GetUserIp write SetUserIp;
    property UserLocation: TdxMapControlGeoPoint read GetUserLocation
      write SetUserLocation;
    property UserMapView: TdxMapControlGeoRect read GetUserMapView
  ...
  public
    class function ParseBoundingBoxNode(ANode: TdxXmlNode;
      var ARect: TdxMapControlGeoRect): Boolean;
    class function ParsePointNode(ANode: TdxXmlNode;
...
procedure TdxBingMapRESTServiceParams.AddParamAfter(var AParams: string;
  const AAddingParam: string);
begin
  if AAddingParam <> '' then
    AParams := AParams + IfThen(AParams <> '', dxUrlParamsDevider) +
      AAddingParam;
end;
...
begin
  if AAddingParam <> '' then
    AParams := AAddingParam + IfThen(AParams <> '', dxUrlParamsDevider)
      + AParams;
end;
...
begin
  Result := Format(dxBingOutputParam,
    [dxBingOutputFormats[Ord(FOutputFormat)]]);
end;
...
begin
  ...
  else if ADoc.FindChild(dxBingResourcesXmlPath, ANode) then
...
procedure TdxBingMapRESTService.SetUserLocation(const Value
  : TdxMapControlGeoPoint);
begin
  FParams.UserLocation := Value;
end;

procedure TdxBingMapRESTService.SetUserMapView(const Value
...
class function TdxBingMapDataXmlParser.ParseBoundingBoxNode(ANode: TdxXmlNode;
  var ARect: TdxMapControlGeoRect): Boolean;
var
  ASouthLatitudeNode, AWestLongitudeNode, ANorthLatitudeNode, AEastLongitudeNode
    : TdxXmlNode;
begin
  ...
  if Result then
  begin
    ARect.SouthLatitude := dxStrToFloatDef
      (ASouthLatitudeNode.TextAsString, '.');
    ARect.WestLongitude := dxStrToFloatDef
      (AWestLongitudeNode.TextAsString, '.');
    ARect.NorthLatitude := dxStrToFloatDef
      (ANorthLatitudeNode.TextAsString, '.');
    ARect.EastLongitude := dxStrToFloatDef
      (AEastLongitudeNode.TextAsString, '.');
  end;
end;
