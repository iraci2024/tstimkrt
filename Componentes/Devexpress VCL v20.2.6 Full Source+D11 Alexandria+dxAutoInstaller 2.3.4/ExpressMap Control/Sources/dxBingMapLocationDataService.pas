...
uses
  ...
  dxMapControlTypes, dxBingMapRESTService, dxRESTService,
...
type
  ...
  public
    ...
    property IncludeNeighborhood: Boolean read FIncludeNeighborhood
      write FIncludeNeighborhood;
    // property Include: string read FInclude write FInclude;
  end;

  TdxBingMapIncludeEntityType = (bmietAddress, bmietNeighborhood,
    bmietPopulatedPlace, bmietPostcode1, bmietAdminDivision1,
    bmietAdminDivision2, bmietCountryRegion);
  TdxBingMapIncludeEntityTypes = set of TdxBingMapIncludeEntityType;

  TdxBingMapRESTServiceLocationByPointParams = class
    (TdxBingMapRESTServiceLocationParams) // for internal use
  private
  ...
  public
    ...
    property IncludeEntityTypes: TdxBingMapIncludeEntityTypes
      read FIncludeEntityTypes write FIncludeEntityTypes;
    property Point: TdxMapControlGeoPoint read FPoint write FPoint;
  end;

  TdxBingMapRESTServiceLocationByQueryParams = class
    (TdxBingMapRESTServiceLocationParams) // for internal use
  private
    FMaxResults: Integer;
    FQuery: string;
  ...
  TdxBingMapLocationByQueryDataService = class
    (TdxBingMapCustomLocationDataService) // for internal use
  protected
    function CreateParams: TdxBingMapRESTServiceParams; override;
  end;

  TdxBingMapLocationByPointDataService = class
    (TdxBingMapCustomLocationDataService) // for internal use
  protected
    function CreateParams: TdxBingMapRESTServiceParams; override;
    function GetAddress: string; override;
...
const
  AllEntityTypes = [Low(TdxBingMapIncludeEntityType)
    .. High(TdxBingMapIncludeEntityType)];
...
function TdxBingMapLocationInfo.GetConfidence(const AValue: string)
...
function TdxBingMapLocationInfo.GetMatchCode(const AValue: string)
  : TdxBingMapLocationMatchCodes;
var
  I: Integer;
begin
  Result := [];
...
function TdxBingMapAddress.GetDisplayText(const AEntityType: string): string;
  ...
  function AddToDisplayText(var ADisplayText: string;
    const AAddressPart: string): Boolean;
  begin
    Result := AAddressPart <> '';
    if Result then
      ADisplayText := ADisplayText + IfThen(ADisplayText <> '', ', ') +
        AAddressPart;
  end;
...
function TdxBingMapRESTServiceLocationByPointParams.
  GetIncludeEntityTypesParam: string;
var
  I: TdxBingMapIncludeEntityType;
  AIncludeEntityTypes: string;
begin
  AIncludeEntityTypes := '';
  for I := Low(TdxBingMapIncludeEntityType)
    to High(TdxBingMapIncludeEntityType) do
...
function TdxBingMapRESTServiceLocationParams.
  GetIncludeNeighborhoodParam: string;
const
  AIncludeNeighborhood: array [Boolean] of string = ('0', '1');
begin
  Result := Format(dxBingIncludeNeighborhoodParam,
    [AIncludeNeighborhood[FIncludeNeighborhood]]);
end;
...
function TdxBingMapCustomLocationDataService.CreateResponse
...
function TdxBingMapLocationByQueryDataService.CreateParams
...
function TdxBingMapLocationByPointDataService.CreateParams
  : TdxBingMapRESTServiceParams;
begin
