...
uses
  ...
  dxflchrt, cxGraphics, cxLookAndFeels, cxLookAndFeelPainters, cxButtons,
  cxControls, cxContainer, cxEdit, cxCheckBox,
  cxTextEdit, cxMaskEdit, cxDropDownEdit, cxMemo, dxLayoutLookAndFeels,
  cxClasses, dxLayoutContainer, dxLayoutControl,
  dxLayoutcxEditAdapters, dxLayoutControlAdapters, cxListView, dxForms,
  dxColorDialog,
...
procedure TFEditObject.gcImagesItemClick(Sender: TObject;
...
begin
  ...
  cbTextPosition.ItemIndex := Integer(AObject.VertTextPos) * 3 +
    Integer(AObject.HorzTextPos);
  cbImagePosition.ItemIndex := Integer(AObject.VertImagePos) * 3 +
    Integer(AObject.HorzImagePos);
  ...
  if AObject.ShapeType = fcsAdvanced then
    cbShapeStyle.ItemIndex := cbShapeStyle.Properties.Items.IndexOfObject
      (AObject.AdvancedShape)
  else
...
procedure TFEditObject.LoadAdvancedShapes(AItems: TStrings;
  AChart: TdxFlowChart);
var
...
procedure TFEditObject.LoadLocalizations;
  ...
  begin
    ...
    try
      AItems.Clear;
      for I := Low(dxFlowChartLayoutNamesMap)
        to High(dxFlowChartLayoutNamesMap) do
...
begin
  ...
  cbTransparent.Caption := cxGetResourceString
  ...
  Label4.CaptionOptions.Text := cxGetResourceString
    (@sdxFlowChartConnectionEditorTextFontHint);
  liLineWidth.Caption := cxGetResourceString
  ...
  Label9.Caption := cxGetResourceString
  ...
  GroupBox2.Caption := cxGetResourceString
...
begin
  ...
  begin
    AItemObject := cbShapeStyle.Properties.Items.Objects
      [cbShapeStyle.ItemIndex];
...
procedure TFEditObject.cbFontButtonClickHandler(Sender: TObject;
...
begin
  ...
  if cbTop.Checked then
    Result := Result or BF_TOP;
  if cbRight.Checked then
    Result := Result or BF_RIGHT;
  if cbBottom.Checked then
    Result := Result or BF_BOTTOM;
  if cbDiag.Checked then
    Result := Result or BF_DIAGONAL;
  if cbMiddle.Checked then
    Result := Result or BF_MIDDLE;
  if cbSoft.Checked then
    Result := Result or BF_SOFT;
  if cbAdjust.Checked then
    Result := Result or BF_ADJUST;
  if cbFlat.Checked then
    Result := Result or BF_FLAT;
  if cbMono.Checked then
    Result := Result or BF_MONO;
end;
