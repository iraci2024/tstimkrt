...
{$ENDIF}
  ...
  dxRibbon, dxBar, dxFcEdit, dxRibbonForm, ImgList, cxImageList,
  ...
  dxGDIPlusClasses, cxImage, Menus, dxCoreClasses, dxCoreGraphics,
  dxTypeHelpers,
  ...
  ExtCtrls, cxMaskEdit, cxImageComboBox, dxLayoutcxEditAdapters,
  dxLayoutControlAdapters,
  dxLayoutLookAndFeels, dxLayoutContainer, cxSpinEdit, StdCtrls, cxButtons,
  cxMemo, dxLayoutControl,
...
type
  ...
  TdxFlowChartForEachStandardShapeProc = procedure(AShapeType: TdxFcShapeType)
    of object;
  TdxFlowChartForEachSelectedItemProc = reference to procedure
    (AItem: TdxFcItem);
  TdxFlowChartForEachSelectedObjectProc = reference to procedure
    (AObject: TdxFcObject);
  TdxFlowChartForEachSelectedConnectionProc = reference to procedure
    (AConnection: TdxFcConnection);
  TdxFlowChartGroupControlsActionProc = reference to procedure
    (AControl: TWinControl);
  TdxFlowChartCompareConnectionValueFunc = reference to function
    (AConnection: TdxFcConnection): Boolean;
  TdxFlowChartCompareObjectValueFunc = reference to function
  ...
  public
    constructor Create(AShapeType: TdxFcShapeType;
      AAdvancedShape: TdxFlowChartObjectAdvancedShape = nil);
  end;
  ...
  public
    procedure DragStart(X, Y: Integer;
      AKind: TdxFlowChartDragHelper.TFlowChartDragKind); override;
  ...
  protected
    ...
    procedure AddObjectOnDragFromGallery(APoint: TPoint;
    ...
    property IsAddingConnection: Boolean read FIsAddingConnection
      write FIsAddingConnection;
    property UndoRedoManager: TUndoManager read FUndoRedoManager;
    property OnTransactChanged: TNotifyEvent read FOnTransactChanged
      write FOnTransactChanged;
  public
  ...
  TdxFlowChartDesigner = class(TdxFlowChartCustomDesigner,
    ...
    procedure gcStencilsItemClick(Sender: TObject;
      AItem: TdxGalleryControlItem);
    procedure gcMoreShapesItemClick(Sender: TObject;
      AItem: TdxGalleryControlItem);
    ...
    procedure gcMoreShapesMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: Integer);
  ...
  private
    ...
    FShapeGalleryInfo: TDictionary<TdxGalleryControlItem,
      TdxFlowChartShapeInfo>;
    FShortCuts: TDictionary<TAction, TShortCut>;
    FStencilGalleryInfo: TDictionary<TdxGalleryControlItem,
      TdxFlowChartAdvancedShapeStencil>;
    FStencilMenuInfo
      : TDictionary<TdxBarButton, TdxFlowChartAdvancedShapeStencil>;
    ...
    procedure ChartDragOverHandler(ASender, ASource: TObject; AX, AY: Integer;
      AState: TDragState; var AAccept: Boolean);
    procedure ChartKeyDownHandler(ASender: TObject; var AKey: Word;
      AShift: TShiftState);
    procedure ChartMouseDownHandler(ASender: TObject; AButton: TMouseButton;
      AShift: TShiftState; AX, AY: Integer);
    procedure ChartMouseMoveHandler(ASender: TObject; AShift: TShiftState;
      AX, AY: Integer);
    procedure ChartMouseUpHandler(ASender: TObject; AButton: TMouseButton;
      AShift: TShiftState; AX, AY: Integer);
    procedure ChartMouseWheelHandler(ASender: TObject; AShift: TShiftState;
      AWheelDelta: Integer; AMousePos: TPoint; var AHandled: Boolean);
    procedure ChartSelectedHandler(ASender: TdxCustomFlowChart;
      AItem: TdxFcItem);
    ...
    procedure CopySelectedConnectionsToBuffer(ALinkObjects
      : TDictionary<TdxFcObject, TdxFcObject>);
    procedure CopySelectedObjectsToBuffer(ALinkObjects
      : TDictionary<TdxFcObject, TdxFcObject>);
    procedure CopyToBuf;
    procedure PasteBufferConnectionsToChart(ALinkObjects
      : TDictionary<TdxFcObject, TdxFcObject>);
    procedure PasteBufferObjectsToChart(ALinkObjects
      : TDictionary<TdxFcObject, TdxFcObject>);
    ...
    procedure ForEachSelectedChartItem(const AProc
      : TdxFlowChartForEachSelectedItemProc);
    procedure ForEachSelectedConnection(const AProc
      : TdxFlowChartForEachSelectedConnectionProc);
    procedure ForEachSelectedObject(const AProc
      : TdxFlowChartForEachSelectedObjectProc);
    procedure ForEachStandardShape(const AProc
      : TdxFlowChartForEachStandardShapeProc);
  ...
  protected
    procedure AddAdvancedGalleryShapes
      (AStencil: TdxFlowChartAdvancedShapeStencil; ANeedSetChecked: Boolean);
    ...
    procedure ChangeSelectedChartItems(const AProc
      : TdxFlowChartForEachSelectedItemProc);
    procedure ChangeSelectedConnections(const AProc
      : TdxFlowChartForEachSelectedConnectionProc);
    procedure ChangeSelectedObjects(const AProc
      : TdxFlowChartForEachSelectedObjectProc);
    ...
    procedure DrawAdvancedGalleryShape(AGalleryItem: TdxGalleryControlItem;
      AShape: TdxFlowChartObjectAdvancedShape);
    procedure DrawDragObject;
    procedure DrawStandardGalleryShape(AShapeGalleryItem: TdxGalleryControlItem;
      AShapeType: TdxFcShapeType);
    procedure DrawToolsStandardShape(AMenuItem: TdxBarButton;
      AShapeType: TdxFcShapeType);
    ...
    function GetStandardGalleryShapePolygonPoints(AShapeType: TdxFcShapeType;
      ABounds: TRect): TPoints;
    function GetStandardGalleryShapeShortCut(AShapeType: TdxFcShapeType)
      : TShortCut;
    function GetStandardShapeGlyph(AShapeType: TdxFcShapeType; ASize: TSize)
      : TdxSmartGlyph;
    ...
    function NeedAddStandardGalleryShapes
      (AStencil: TdxFlowChartAdvancedShapeStencil): Boolean;
    ...
    procedure UpdateGalleryStencils(AFocusedStencil
      : TdxFlowChartAdvancedShapeStencil = nil);
    ...
    // properties panel
    function CompareConnectionValue(const ACompare
      : TdxFlowChartCompareConnectionValueFunc; AStartIndex: Integer): Boolean;
    function CompareObjectValue(const ACompare
      : TdxFlowChartCompareObjectValueFunc; AStartIndex: Integer): Boolean;
    ...
    procedure IterateGroupControls(AGroup: TdxCustomLayoutGroup;
      const AAction: TdxFlowChartGroupControlsActionProc);
    ...
    property ActiveChartBackgroundColor: TColor
      read GetActiveChartBackgroundColor;
    ...
    property ShapeGalleryInfo
      : TDictionary<TdxGalleryControlItem, TdxFlowChartShapeInfo>
      read FShapeGalleryInfo;
    property ShortCuts: TDictionary<TAction, TShortCut> read FShortCuts;
    property StencilGalleryInfo
      : TDictionary<TdxGalleryControlItem, TdxFlowChartAdvancedShapeStencil>
      read FStencilGalleryInfo;
    property StencilMenuInfo
      : TDictionary<TdxBarButton, TdxFlowChartAdvancedShapeStencil>
      read FStencilMenuInfo;
    property UseAdvancedShapesOnly: Boolean read FUseAdvancedShapesOnly
      write SetUseAdvancedShapesOnly;
  public
...
uses
  RTLConsts, Math, dxSelUnion, dxFcStrs, dxEditObj, dxEditCon, dxThreading,
  cxGeometry, dxLines, dxFlowChartArrows;
...
const
  ...
  dxNewConfirmation =
    'The document you are about to close has unsaved changes. Do you want to save the document?';

  dxStandardShapeShortCutText: array [TdxFcShapeType] of string = ('', 'Ctrl+3',
    'Ctrl+4', 'Ctrl+5', 'Ctrl+6', 'Ctrl+Shift+1', 'Ctrl+Shift+2',
...
begin
  ...
  try
    ...
    try
      ...
      cxBitBlt(ABasicGlyph.Canvas.Handle, ALineGlyph.Canvas.Handle, ALineBounds,
        Point(0, 0), SRCCOPY);
    finally
      ALineGlyph.Free;
    end;
...
type
  ...
  public
    procedure PaintShape(AGraphics: TdxGPCanvas; const ABounds: TRect;
      APenColor, ABrushColor: TdxAlphaColor);
...
procedure TdxFlowChartObjectAdvancedShapeHelper.PaintShape
  (AGraphics: TdxGPCanvas; const ABounds: TRect;
  APenColor, ABrushColor: TdxAlphaColor);
begin
...
class function TdxFcConnectionArrowHelper.GetRepository
...
begin
  ...
  begin
    ...
    if AState = [ssShift] then
      AKey := #10
    else if (AState = []) and ModifiedAfterEnter then
    begin
      DoEditValueChanged;
...
procedure TcxFlowChartDesignerMoreShapesDropDownArrowImage.WMNCHitTest
...
constructor TdxFlowChartShapeInfo.Create(AShapeType: TdxFcShapeType;
  AAdvancedShape: TdxFlowChartObjectAdvancedShape = nil);
begin
  inherited Create;
...
procedure TdxDesignerFlowChartDragHelper.DragStart(X, Y: Integer;
  AKind: TdxFlowChartDragHelper.TFlowChartDragKind);
begin
  ...
  if not(DragKind in [TFlowChartDragKind.None, TFlowChartDragKind.Selection])
  then
    Owner.BeginTransactChange;
end;
...
begin
  ...
  if not(ADragKind in [TFlowChartDragKind.None, TFlowChartDragKind.Selection])
  then
    if Cancelled then
      Owner.CancelTransactChange
    else
...
constructor TdxDesignerFlowChartKeyAction.Create(AKey: Word;
  AShift: TShiftState);
begin
  inherited Create;
  FKey := AKey;
...
function TdxDesignerFlowChartKeyAction.IsEqual(AKey: Word;
  AShift: TShiftState): Boolean;
begin
  Result := (FKey = AKey) and (FShift = AShift);
end;

function TdxDesignerFlowChartKeyAction.IsRepeated(AKey: Word;
  AShift: TShiftState): Boolean;
begin
  Result := (AKey in [VK_RIGHT, VK_LEFT, VK_DOWN, VK_UP, VK_NEXT, VK_PRIOR,
    VK_HOME, VK_END]) and ((AShift = [ssShift]) or (AShift = [ssAlt])) and
    IsEqual(AKey, AShift);
end;
...
begin
  ...
  try
    ...
    DragHelper.DragStart(APoint.X, APoint.Y,
      TdxFlowChartDragHelper.TFlowChartDragKind.Endpoint);
  finally
    EndTransactChange;
  end;
end;

procedure TdxDesignerFlowChart.AddObjectOnDragFromGallery(APoint: TPoint;
  AShapeType: TdxFcShapeType; AAdvancedShape: TdxFlowChartObjectAdvancedShape);
begin
  BeginTransactChange;
  try
    StartExternalDrag(APoint, function(): TdxFcObject
      begin
...
begin
  FIsKeyActionRepeated := (FLastKeyAction <> nil) and
    FLastKeyAction.IsRepeated(Key, Shift);
  try
...
begin
  ...
  try
    ...
    Chart.Font.Height := ScaleFactor.Apply(AChart.Font.Height,
      AChart.GetScaleFactor);
    Chart.Zoom := 100;
...
begin
  if FUpdatingPropertiesControls then
    Exit;
  AText := mText.Text;
  ForEachSelectedChartItem(procedure(AItem: TdxFcItem)
    begin
      AItem.Text := AText;
    end);
end;

procedure TdxFlowChartDesigner.AddAdvancedGalleryShapes
  (AStencil: TdxFlowChartAdvancedShapeStencil; ANeedSetChecked: Boolean);
var
  I: Integer;
  AItem: TdxGalleryControlItem;
...
begin
  ...
  begin
    ...
    if ANeedSetChecked then
      AItem.Checked := I = 0;
    ShapeGalleryInfo.Add(AItem,
      TdxFlowChartShapeInfo.Create(fcsAdvanced, AShape));
...
procedure TdxFlowChartDesigner.AddStandardGalleryShape
  (AShapeType: TdxFcShapeType);
var
  AItem: TdxGalleryControlItem;
begin
...
procedure TdxFlowChartDesigner.AddToolsStandardShape
  (AShapeType: TdxFcShapeType);
var
...
begin
  Result := True;
  AMsgRes := MessageDlg(dxNewConfirmation, mtConfirmation,
  ...
  case AMsgRes of
    mrYes:
      begin
        ...
        if FModified then
          Result := False
        else if Mode = dmChartDesigner then
          FModified := True;
      end;
    mrCancel:
      Result := False;
...
begin
  ...
  try
    ...
    begin
      ...
      AMenuButton.Down := (AStencil = Chart.Repository.BasicShapes) or
        (AStencil = Chart.Repository.BasicFlowchartShapes);
...
procedure TdxFlowChartDesigner.cbDArrowSizePropertiesEditValueChanged
  (Sender: TObject);
var
  ASize: TdxFcaSize;
begin
  ...
  ForEachSelectedConnection(procedure(AConnection: TdxFcConnection)
    begin
...
procedure TdxFlowChartDesigner.icbLineStylePropertiesEditValueChanged
  (Sender: TObject);
var
  APenStyle: TPenStyle;
begin
  ...
  ChangeSelectedChartItems(procedure(AItem: TdxFcItem)
    var
      AConnection: TdxFcConnection;
      AObject: TdxFcObject;
    begin
...
procedure TdxFlowChartDesigner.cbSArrowSizePropertiesEditValueChanged
  (Sender: TObject);
var
  ASize: TdxFcaSize;
begin
...
procedure TdxFlowChartDesigner.ChangeSelectedChartItems
  (const AProc: TdxFlowChartForEachSelectedItemProc);
begin
...
procedure TdxFlowChartDesigner.ChangeSelectedConnections
  (const AProc: TdxFlowChartForEachSelectedConnectionProc);
begin
  Chart.BeginTransactChange;
  try
...
procedure TdxFlowChartDesigner.ChangeSelectedObjects
  (const AProc: TdxFlowChartForEachSelectedObjectProc);
begin
  Chart.BeginTransactChange;
  try
...
function TdxFlowChartDesigner.CheckActiveChartTextHorzAlign
  (AAlign: TdxFcHorzPos): Boolean;
begin
...
function TdxFlowChartDesigner.CheckActiveChartTextVertAlign
  (AAlign: TdxFcVertPos): Boolean;
begin
  if FirstSelectedChartItem is TdxFcObject then
...
procedure TdxFlowChartDesigner.DrawAdvancedGalleryShape
...
begin
  ...
  try
    ...
    case AShapeType of
      ...
      Chart.Canvas.Polygon(GetStandardGalleryShapePolygonPoints
        (AShapeType, ARect));
    end;
  finally
...
procedure TdxFlowChartDesigner.DrawStandardGalleryShape(AShapeGalleryItem
  : TdxGalleryControlItem; AShapeType: TdxFcShapeType);
var
  AGlyph: TdxSmartGlyph;
begin
  AGlyph := GetStandardShapeGlyph(AShapeType,
    gcShapes.OptionsView.Item.Image.Size.Size);
  try
...
procedure TdxFlowChartDesigner.DrawToolsStandardShape(AMenuItem: TdxBarButton;
AShapeType: TdxFcShapeType);
var
  AGlyph: TdxSmartGlyph;
begin
  AGlyph := GetStandardShapeGlyph(AShapeType,
    cxSize(dxToolStandardShapesGlyphSize));
  try
...
function TdxFlowChartDesigner.GetFocusedGalleryStencil
  : TdxFlowChartAdvancedShapeStencil;
begin
  StencilGalleryInfo.TryGetValue(gcStencils.Gallery.GetCheckedItem, Result);
end;

function TdxFlowChartDesigner.GetStandardGalleryShapeCaption
  (AShapeType: TdxFcShapeType): string;
begin
  Result := cxGetResourceString(dxFlowChartShapeNamesMap[AShapeType]);
end;

function TdxFlowChartDesigner.GetStandardGalleryShapeShortCut
  (AShapeType: TdxFcShapeType): TShortCut;
begin
  Result := TextToShortCut(dxStandardShapeShortCutText[AShapeType]);
end;

function TdxFlowChartDesigner.GetStandardGalleryShapePolygonPoints
  (AShapeType: TdxFcShapeType; ABounds: TRect): TPoints;
begin
  case AShapeType of
    fcsDiamond:
      begin
...
function TdxFlowChartDesigner.GetStandardShapeGlyph(AShapeType: TdxFcShapeType;
ASize: TSize): TdxSmartGlyph;
var
  ARect: TRect;
  ACanvas: TdxGPCanvas;
begin
  ...
  try
    ...
    case AShapeType of
      fcsRectangle:
        ACanvas.Rectangle(ARect, dxDefaultShapeColor, dxDefaultShapeBkColor, 1,
          psSolid, 255, 255);
      fcsEllipse:
        ACanvas.Ellipse(ARect, dxDefaultShapeColor, dxDefaultShapeBkColor, 1,
          psSolid, 255, 255);
      fcsRoundRect:
        ACanvas.RoundRect(ARect, TdxAlphaColors.FromColor(dxDefaultShapeColor),
          TdxAlphaColors.FromColor(dxDefaultShapeBkColor), ARect.Width shr 1,
          ARect.Height shr 1);
    else
      ACanvas.Polygon(GetStandardGalleryShapePolygonPoints(AShapeType, ARect),
        dxDefaultShapeColor, dxDefaultShapeBkColor, 1, psSolid, 255, 255);
    end;
  finally
    ACanvas.Free;
  end;
...
begin
  ...
  Chart.Options := [fcoCanDelete, fcoCanDrag, fcoCanSelect, fcoMultiSelect,
    fcoDelOnClick, fcoCanRotate, fcoUseShapeParameters, fcoSnapToGuides];
...
begin
  ...
  try
    ...
    begin
      if I <> fcaNone then
        TdxFcConnectionArrow.GetRepository.DrawPreview(Ord(I), G, B.ClientRect,
          Chart.ScaleFactor.ApplyF(1.0), True);
      ...
      if I <> fcaNone then
        TdxFcConnectionArrow.GetRepository.DrawPreview(Ord(I), G, B.ClientRect,
          Chart.ScaleFactor.ApplyF(1.0), False);
...
begin
  for I := Succ(fcasCustom) to High(TdxFcaSize) do
    cbSArrowSize.Properties.Items.Add
      (cxGetResourceString(dxFlowChartConnectionArrowSizeNamesMap[I]));
  cbSArrowSize.Properties.Items.Add
    (cxGetResourceString(dxFlowChartConnectionArrowSizeNamesMap[fcasCustom]));
...
function TdxFlowChartDesigner.IsStandardGalleryShape
  (AShapeType: TdxFcShapeType): Boolean;
begin
...
procedure TdxFlowChartDesigner.MultiSelect(ResetOldSelected: Boolean;
SelectRect: TRect);
var
  I: Integer;
begin
...
function TdxFlowChartDesigner.NeedAddStandardGalleryShapes
  (AStencil: TdxFlowChartAdvancedShapeStencil): Boolean;
begin
  Result := (AStencil = Chart.Repository.BasicShapes) and
    not UseAdvancedShapesOnly;
end;
...
procedure TdxFlowChartDesigner.seAnglePropertiesEditValueChanged
  (Sender: TObject);
var
  AAngle: Integer;
begin
  ...
  ChangeSelectedObjects(procedure(AObject: TdxFcObject)
    begin
      AObject.Angle := AAngle;
    end);
end;

procedure TdxFlowChartDesigner.seLineThicknessPropertiesEditValueChanged
  (Sender: TObject);
var
  AThickness: Integer;
begin
...
procedure TdxFlowChartDesigner.seShapeHeightPropertiesEditValueChanged
  (Sender: TObject);
var
  AHeight: Integer;
begin
...
procedure TdxFlowChartDesigner.seShapeWidthPropertiesEditValueChanged
  (Sender: TObject);
var
  AWidth: Integer;
begin
...
begin
  Result := FileExists(AFileName) and dxShellExecute(Handle, AFileName,
    SW_SHOWMAXIMIZED);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      AddAdvancedGalleryShapes(AStencil,
        NeedAddStandardGalleryShapes(AStencil));
    end;
  finally
    gcShapes.EndUpdate;
  end;
...
begin
  DrawHelpedColorLine(bbBackgroundColor.Glyph, FBackgroundColorPicker.Color,
    ilSmallIcons, acBackgroundColor.ImageIndex);
end;
...
begin
  DrawHelpedColorLine(btnDestArrowColor.OptionsImage.Glyph,
    FDestArrowColorPicker.Color, ilSmallIcons,
    btnDestArrowColor.OptionsImage.ImageIndex);
end;

procedure TdxFlowChartDesigner.UpdateFontColorImage;
begin
  DrawHelpedColorLine(bbFontColor.Glyph, FFontColorPicker.Color, ilSmallIcons,
    acFontColor.ImageIndex);
end;

procedure TdxFlowChartDesigner.UpdateGalleryStencils(AFocusedStencil
  : TdxFlowChartAdvancedShapeStencil = nil);
var
  I: Integer;
  AMenuButton: TdxBarButton;
...
begin
  ...
  try
    ...
    begin
      ...
      if AMenuButton.Down and StencilMenuInfo.TryGetValue(AMenuButton, AStencil)
      then
      begin
    ...
    else if gcStencils.Gallery.GetCheckedItem = nil then
...
begin
  DrawHelpedColorLine(btnObjectBkColor.OptionsImage.Glyph,
    FObjectBkColorPicker.Color, ilSmallIcons,
...
begin
  ...
  acBringToFront.Caption := cxGetResourceString
    (@sdxFlowChartEditorEditBringToFront);
  acClearSelection.Caption := cxGetResourceString
  ...
  acSendToBack.Caption := cxGetResourceString
  ...
  acActualSize.Caption := cxGetResourceString
    (@sdxFlowChartEditorViewActualSize);
  acAntialiasing.Caption := cxGetResourceString
  ...
  acClearAllUnions.Caption := cxGetResourceString
  ...
  acRemoveFromUnion.Caption := cxGetResourceString
...
begin
  ...
  acFakeTextVertAlign.Checked :=
    not(acTextAlignTop.Enabled or acTextAlignMiddle.Enabled or
    acTextAlignBottom.Enabled);
  acFakeTextHorzAlign.Checked :=
    not(acTextAlignLeft.Enabled or acTextAlignCenter.Enabled or
    acTextAlignRight.Enabled);
...
begin
  DrawHelpedColorLine(btnSourceArrowColor.OptionsImage.Glyph,
    FSourceArrowColorPicker.Color, ilSmallIcons,
    btnSourceArrowColor.OptionsImage.ImageIndex);
end;
...
procedure TdxFlowChartDesigner.IterateGroupControls
...
function TdxFlowChartDesigner.CompareObjectValue(const ACompare
  : TdxFlowChartCompareObjectValueFunc; AStartIndex: Integer): Boolean;
var
  I: Integer;
begin
...
function TdxFlowChartDesigner.CompareConnectionValue(const ACompare
  : TdxFlowChartCompareConnectionValueFunc; AStartIndex: Integer): Boolean;
var
  I: Integer;
begin
...
procedure TdxFlowChartDesigner.UpdateCommonPropertiesControls;
  ...
  function AreAllEqual(const ACompareObjectValue
    : TdxFlowChartCompareObjectValueFunc;
  const ACompareConnectionValue
    : TdxFlowChartCompareConnectionValueFunc): Boolean;
  begin
  ...
  begin
    if AreAllEqual(function(AObject: TdxFcObject): Boolean
      begin
        Result := AObject.Text = AText;
      end, function(AConnection: TdxFcConnection): Boolean
      begin
...
var
  ...
  AThickness: Integer;
begin
  AEnabled := (Chart.SelectedConnectionCount > 0) or
    (Chart.SelectedObjectCount > 0);
  IterateGroupControls(lgCommon, procedure(AControl: TWinControl)
    var
      AEdit: TcxCustomEdit;
    begin
  ...
  IterateGroupControls(lgLine, procedure(AControl: TWinControl)
    var
      AEdit: TcxCustomEdit;
    begin
      AEdit := Safe<TcxCustomEdit>.Cast(AControl);
...
procedure TdxFlowChartDesigner.UpdateObjectsPropertiesControls;
  ...
  procedure SetSpinEdit(AValue: Integer; ASpinEdit: TcxSpinEdit;
  const ACompare: TdxFlowChartCompareObjectValueFunc);
  begin
...
begin
  ...
  IterateGroupControls(lgObjects, procedure(AControl: TWinControl)
    var
      AEdit: TcxCustomEdit;
    begin
      AControl.Enabled := AEnabled;
  ...
  SetSpinEdit(AAngle, seAngle, function(AObject: TdxFcObject): Boolean
    begin
      Result := Round(AObject.Angle) = AAngle;
    end);
  AWidth := AFirstObject.Width;
  SetSpinEdit(AWidth, seShapeWidth, function(AObject: TdxFcObject): Boolean
    begin
      Result := AObject.Width = AWidth;
    end);
  AHeight := AFirstObject.Height;
  SetSpinEdit(AHeight, seShapeHeight, function(AObject: TdxFcObject): Boolean
    begin
...
procedure TdxFlowChartDesigner.UpdateConnectionsPropertiesControls;
  ...
  function AreAllEqual(const ACompare
    : TdxFlowChartCompareConnectionValueFunc): Boolean;
  var
    I: Integer;
  begin
  ...
  procedure SetArrowSize(AArrowSize: TdxFcaSize; AComboBox: TcxComboBox;
  const ACompare: TdxFlowChartCompareConnectionValueFunc);
  begin
  ...
  procedure SetArrowType(AArrowType: TdxFcaType; AComboBox: TcxImageComboBox;
  const ACompare: TdxFlowChartCompareConnectionValueFunc);
  begin
    if CompareConnectionValue(ACompare, 1) then
  ...
  procedure SetStyle(AStyle: TdxFclStyle; AComboBox: TcxImageComboBox;
  const ACompare: TdxFlowChartCompareConnectionValueFunc);
  begin
    if CompareConnectionValue(ACompare, 1) then
...
begin
  ...
  IterateGroupControls(lgConnections, procedure(AControl: TWinControl)
    var
      AEdit: TcxCustomEdit;
    begin
      AControl.Enabled := AEnabled;
  ...
  ForEachSelectedConnection(procedure(AConnection: TdxFcConnection)
    begin
      if icbBeginArrow.ModifiedAfterEnter then
        AConnection.ArrowSource.ArrowType :=
          TdxFcaType(icbBeginArrow.EditValue);
...
begin
  DrawHelpedColorLine(bbStrokeColor.Glyph, FStrokeColorPicker.Color,
    ilSmallIcons, acStrokeColor.ImageIndex);
end;
...
begin
  if FirstSelectedChartItem = nil then
    Result := clDefault
  else if FirstSelectedChartItem is TdxFcObject then
...
procedure TdxFlowChartDesigner.ChartDragOverHandler(ASender, ASource: TObject;
AX, AY: Integer; AState: TDragState; var AAccept: Boolean);
var
  AItem: TdxGalleryControlItem;
  AShapeInfo: TdxFlowChartShapeInfo;
begin
  if (ASource is TcxDragControlObject) and
    (TcxDragControlObject(ASource).Control = gcShapes) and (AState = dsDragEnter)
  then
  begin
    CancelDrag;
    AItem := gcShapes.Gallery.GetCheckedItem;
    if ShapeGalleryInfo.TryGetValue(AItem, AShapeInfo) then
      Chart.AddObjectOnDragFromGallery(cxInvisiblePoint, AShapeInfo.ShapeType,
        AShapeInfo.AdvancedShape);
  end;
end;

procedure TdxFlowChartDesigner.ChartKeyDownHandler(ASender: TObject;
...
begin
  ...
  else if acConnector.Checked then
    Chart.AddConnectionOnDrag(FDownPoint);
end;

procedure TdxFlowChartDesigner.ChartMouseMoveHandler(ASender: TObject;
AShift: TShiftState; AX, AY: Integer);
var
  APoint: TPoint;
begin
  ...
  else if (ssLeft in AShift) and not cxPointIsEqual(APoint, FDragPoint) then
  begin
...
procedure TdxFlowChartDesigner.ChartMouseUpHandler(ASender: TObject;
AButton: TMouseButton; AShift: TShiftState; AX, AY: Integer);
begin
  if acStandardShape.Checked then
  begin
...
procedure TdxFlowChartDesigner.ChartMouseWheelHandler(ASender: TObject;
...
procedure TdxFlowChartDesigner.ChartSelectedHandler(ASender: TdxCustomFlowChart;
AItem: TdxFcItem);
begin
  UpdateRibbonItemsEnabled;
  UpdateRibbonItemsByChartItems;
...
begin
  ...
  ChangeSelectedConnections(procedure(AConnection: TdxFcConnection)
    begin
      AConnection.ArrowDest.Color := FDestArrowColorPicker.Color;
    end);
end;

procedure TdxFlowChartDesigner.MoreShapesGalleryHotTrackChangedHandler
  (ASender: TObject);
begin
...
procedure TdxFlowChartDesigner.ToolsStandardShapeItemClickHandler
  (ASender: TObject);
var
  AMenuItem: TdxBarButton;
begin
...
procedure TdxFlowChartDesigner.CopySelectedConnectionsToBuffer
...
procedure TdxFlowChartDesigner.CopySelectedObjectsToBuffer
  (ALinkObjects: TDictionary<TdxFcObject, TdxFcObject>);
var
  I: Integer;
...
procedure TdxFlowChartDesigner.PasteBufferConnectionsToChart
  (ALinkObjects: TDictionary<TdxFcObject, TdxFcObject>);
var
  I: Integer;
  AObject: TdxFcObject;
...
procedure TdxFlowChartDesigner.PasteBufferObjectsToChart
  (ALinkObjects: TDictionary<TdxFcObject, TdxFcObject>);
var
  I: Integer;
  AObject, ABufferObject: TdxFcObject;
...
function TdxFlowChartDesigner.FindUnions(FromUnion, Obj: TdxFcObject)
  : TdxFcObject;
var
  I: Integer;
  FFind: Boolean;
      ...
      repeat
        ...
        begin
          if Count = 0 then
            ...
            CountText := CountText + ', ' +
              IntToStr(GetNumberByUnion(FirstObj));
          Inc(Count);
        end;
      until FirstObj = nil;
...
procedure TdxFlowChartDesigner.ForEachSelectedChartItem
  (const AProc: TdxFlowChartForEachSelectedItemProc);
var
  I: Integer;
begin
...
procedure TdxFlowChartDesigner.ForEachSelectedConnection
  (const AProc: TdxFlowChartForEachSelectedConnectionProc);
var
  I: Integer;
begin
...
procedure TdxFlowChartDesigner.ForEachSelectedObject
  (const AProc: TdxFlowChartForEachSelectedObjectProc);
var
  I: Integer;
begin
...
procedure TdxFlowChartDesigner.ForEachStandardShape
  (const AProc: TdxFlowChartForEachStandardShapeProc);
var
  AShapeType: TdxFcShapeType;
begin
...
begin
  ...
  begin
    ...
    else if acTextAlignCenter.Checked then
...
begin
  ...
  begin
    ...
    else if acTextAlignMiddle.Checked then
...
procedure TdxFlowChartDesigner.bcbFontNamePropertiesEditValueChanged
  (Sender: TObject);
begin
...
procedure TdxFlowChartDesigner.bcbFontNamePropertiesLoadFontComplete
  (Sender: TObject);
var
...
procedure TdxFlowChartDesigner.bcbFontSizePropertiesEditValueChanged
  (Sender: TObject);
begin
  if FIsInternalUpdatingRibbonItemsByChartItems then
    Exit;
...
begin
  ...
  Chart.StartConnectionEndpointHighlight(cxInvisiblePoint.X,
    cxInvisiblePoint.Y);
end;
...
begin
  ...
  try
    ...
    if ((hTest * [htByObject, htOnObject]) <> []) and
      (Chart.SelectedObjectCount > 0) then
      ObjectEditor(Chart, FirstSelectedChartObject)
    else if ((hTest * [htOnConnection, htOnConLabel, htOnArrowSrc, htOnArrowDst]
      ) <> []) and (Chart.SelectedConnectionCount > 0) then
...
procedure TdxFlowChartDesigner.FormCloseQuery(Sender: TObject;
...
begin
  if not((ModalResult = mrCancel) and FModified) then
    Exit;
  AMsgRes := MessageDlg(dxExitConfirmation, mtConfirmation,
    [mbYes, mbNo, mbCancel], 0);
  case AMsgRes of
    mrYes:
...
begin
  ...
  FShapeGalleryInfo := TObjectDictionary<TdxGalleryControlItem,
    TdxFlowChartShapeInfo>.Create([doOwnsValues]);
  FStencilGalleryInfo := TDictionary<TdxGalleryControlItem,
    TdxFlowChartAdvancedShapeStencil>.Create;
  FStencilMenuInfo :=
    TDictionary<TdxBarButton, TdxFlowChartAdvancedShapeStencil>.Create;
  FLastObj := nil;
  ...
  TcxFontNameComboBoxProperties(bcbFontName.Properties).DropDownListStyle :=
    lsEditFixedList;
  FShortCuts := TDictionary<TAction, TShortCut>.Create;
  PopulateShortCuts;
  acActualSize.ShortCut := ShortCuts[acActualSize];
  FDragPoint := cxInvalidPoint;
  FFontColorPicker := TdxFlowChartColorPickerController.Create(ppmFontColor,
    rgiFontColor, rgiColorTheme);
  FFontColorPicker.Color := dxDefaultTextColor;
  FFontColorPicker.OnColorChanged := acFontColorExecute;
  UpdateFontColorImage;
  FBackgroundColorPicker := TdxFlowChartColorPickerController.Create
    (ppmBackgroundColor, rgiBackgroundColor, rgiBackgroundThemeColor);
  FBackgroundColorPicker.Color := dxDefaultShapeBkColor;
  FBackgroundColorPicker.OnColorChanged := acBackgroundColorExecute;
  UpdateBackgroundColorImage;
  FStrokeColorPicker := TdxFlowChartColorPickerController.Create(ppmStrokeColor,
    rgiStrokeColor, rgiStrokeColorTheme);
  FStrokeColorPicker.Color := dxDefaultShapeColor;
  FStrokeColorPicker.OnColorChanged := acStrokeColorExecute;
  UpdateStrokeColorImage;
  FLineColorPicker := TdxFlowChartColorPickerController.Create(ppmLineColor,
    rgiLineColor, rgiLineColorTheme);
  FLineColorPicker.Color := dxDefaultArrowColor;
  FLineColorPicker.OnColorChanged := LineColorChangedHandler;
  UpdateLineColorImage;
  FSourceArrowColorPicker := TdxFlowChartColorPickerController.Create
    (ppmSourceArrowColor, rgiSourceArrowColor, rgiSourceArrowColorTheme);
  FSourceArrowColorPicker.Color := dxDefaultArrowColor;
  FSourceArrowColorPicker.OnColorChanged := SourceArrowColorChangedHandler;
  UpdateSourceArrowColorImage;
  FDestArrowColorPicker := TdxFlowChartColorPickerController.Create
    (ppmDestArrowColor, rgiDestArrowColor, rgiDestArrowColorTheme);
...
procedure TdxFlowChartDesigner.gcMoreShapesItemClick(Sender: TObject;
AItem: TdxGalleryControlItem);
var
  ARect: TRect;
...
procedure TdxFlowChartDesigner.gcMoreShapesMouseMove(Sender: TObject;
Shift: TShiftState; X, Y: Integer);
begin
  FMoreShapesArrow.Invalidate;
end;

procedure TdxFlowChartDesigner.gcStencilsItemClick(Sender: TObject;
AItem: TdxGalleryControlItem);
begin
  UpdateActiveGalleryShapes;
end;
