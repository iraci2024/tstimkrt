...
type
  ...
  strict private
    ...
    FPY: Variant;
  public
    constructor Create(const ASize: TdxSizeF;
      const AParameters: array of Single); overload;
    constructor Create(const ASize: TdxSizeF); overload;
    constructor Create(const ASize: TdxSizeF;
      const AParameter: Single); overload;
    constructor Create(const ASize: TdxSizeF; const APoint: TdxPointF);
      overload;
  ...
  strict private
    class var FDefault: TUnitCollection;
...
{$REGION 'internal types'}
  protected type
    ...
    public
      constructor Create(ANode: TdxXmlNode);
      destructor Destroy; override;
      class function Evaluate(AContext: TdxEvaluatorContext;
        AOp: TdxCriteriaValue): Single; static;
      ...
      function Normalize(const ASize: TdxSizeF;
...
{$ENDREGION}
  ...
  strict protected
    procedure ParseSegment(ANode: TdxXmlNode;
      ASegmentClass: TdxShapeSegmentClass);
  ...
  public type
    TAddShapeProcedure = reference to procedure
  ...
  public
    procedure LoadFromStream(AStream: TStream;
      const AAddShape: TAddShapeProcedure);
  end;
...
uses
  ...
  dxTypeHelpers, dxFcStrs, dxflchrt;
...
type
  ...
  public
    constructor Create(AValueCount: Integer; const AExpression: string;
  ...
  public
    class function GetArrayOfDouble(AAttributes: TdxXMLNodeAttributes;
      const AArrayName: TdxXMLString; const ASeparator: string)
      : TArray<Single>; static;
    class function GetNullableColor(AAttributes: TdxXMLNodeAttributes;
      const AColorName: TdxXMLString): TdxNullableValue<TdxAlphaColor>; static;
    class function GetNullableDouble(AAttributes: TdxXMLNodeAttributes;
      const ADoubleName: TdxXMLString): TdxNullableValue<Single>; static;
    class function GetKind(AAttributes: TdxXMLNodeAttributes)
      : TGeometryKinds; static;
  end;
  ...
  public
    constructor Create(ARows: TUnitCollection; AColumns: TUnitCollection;
      const ASize: TdxSizeF; AContext: TShapeContext);
    function EvaluateDouble(AOp: TdxCriteriaValue): Single;
    function EvaluatePoint(AOp: TdxCriteriaValue): TdxPointF;
    function EvaluateRect(AOp: TdxCriteriaValue): TdxRectDouble;
    function EvaluateSize(AOp: TdxCriteriaValue): TdxSizeF;
    function GetConnectionPoint(APoint: TdxShapeXmlDescription.TConnectionPoint)
      : TdxPointF;
    function GetPoint(AXOperator: TdxCriteriaValue;
      AYOperator: TdxCriteriaValue): TdxPointF;
    function GetUnitValue(AUnit: TUnit): Single;
    function ConvertUnits(const AUnits: TArray<TUnit>; ATotalLength: Single)
      : TArray<Single>;
    function GetOffsetByFactor(AFactor: Single;
      AOrientation: TOrientationKind): Single;
  ...
  TShapeBuilder = record
    class function CreateShape(ASegments: TObjectList<TdxShapeSegment>;
      AContext: TShapeLayoutCalculator): TdxShapeViewData; static;
  end;
  ...
  strict private
    ...
    class function GetDirection(AAttributes: TdxXMLNodeAttributes)
      : TdxSweepDirectionKind; static;
  public
    destructor Destroy; override;
    procedure Load(ANode: TdxXmlNode); override;
...
constructor TdxEvaluatorContext.Create(const ASize: TdxSizeF;
  const AParameters: array of Single);
var
  I: Integer;
begin
  Create(ASize);
...
class function TdxCriteriaValue.Parse(const AExpression: string)
  : TdxCriteriaValue;
var
  ADouble: Single;
begin
  ...
  begin
    ...
    else if TdxStringHelper.StartsWith(AExpression, 'CreateSize(') then
...
constructor TMultipleOperandsValue.Create(AValueCount: Integer;
  const AExpression: string; const AProcedureName: string);
var
  AExpressions: TArray<string>;
  I, AStripLength: Integer;
begin
  AStripLength := Length(AProcedureName) + 2;
  AExpressions := TdxStringHelper.Split(Copy(AExpression, AStripLength,
    Length(AExpression) - AStripLength), TArray<string>.Create(','));
...
begin
  Result := TdxSizeF.Create(Values[0].EvaluateDouble(AContext),
...
begin
  Result := TdxPointF.Create(Values[0].EvaluateDouble(AContext),
    Values[1].EvaluateDouble(AContext));
end;
...
begin
  Result := TdxRectDouble.Create(Values[0].EvaluateDouble(AContext),
    Values[1].EvaluateDouble(AContext), Values[0].EvaluateDouble(AContext),
    Values[1].EvaluateDouble(AContext));
end;

{ TAttributeReader }

class function TAttributeReader.GetArrayOfDouble(AAttributes
  : TdxXMLNodeAttributes; const AArrayName: TdxXMLString;
  const ASeparator: string): TArray<Single>;
var
  AValues: TArray<string>;
  I, AIndex, ALength: Integer;
  AValue: Single;
begin
  AValues := TdxStringHelper.Split(AAttributes.GetValueAsString(AArrayName),
    TArray<string>.Create(ASeparator));
...
class function TAttributeReader.GetKind(AAttributes: TdxXMLNodeAttributes)
  : TGeometryKinds;
var
  AValue: TdxXMLString;
begin
...
class function TAttributeReader.GetNullableColor(AAttributes
  : TdxXMLNodeAttributes; const AColorName: TdxXMLString)
...
class function TAttributeReader.GetNullableDouble
  (AAttributes: TdxXMLNodeAttributes; const ADoubleName: TdxXMLString)
  : TdxNullableValue<Single>;
var
  AValue: string;
  ADouble: Single;
begin
...
constructor TShapeLayoutCalculator.Create(ARows: TUnitCollection;
  AColumns: TUnitCollection; const ASize: TdxSizeF; AContext: TShapeContext);
begin
  FSize := ASize;
  FContext := AContext;
  if ARows = nil then
...
function TShapeLayoutCalculator.EvaluateRect(AOp: TdxCriteriaValue)
  : TdxRectDouble;
begin
...
function TShapeLayoutCalculator.GetConnectionPoint
  (APoint: TdxShapeXmlDescription.TConnectionPoint): TdxPointF;
begin
  if APoint.Relative then
    Result := GetPoint(APoint.X, APoint.Y)
  else
    Result := TdxPointF.Create(EvaluateDouble(APoint.X),
      EvaluateDouble(APoint.Y));
end;

function TShapeLayoutCalculator.GetPoint(AXOperator: TdxCriteriaValue;
  AYOperator: TdxCriteriaValue): TdxPointF;
var
...
function TShapeLayoutCalculator.ConvertUnits(const AUnits: TArray<TUnit>;
  ATotalLength: Single): TArray<Single>;
var
...
begin
  ...
  begin
    ...
    else if AUnit.UnitType = TUnitType.Star then
...
function TShapeLayoutCalculator.GetOffsetByFactor(AFactor: Single;
  AOrientation: TOrientationKind): Single;
var
...
class function TShapeBuilder.CreateShape(ASegments
  : TObjectList<TdxShapeSegment>; AContext: TShapeLayoutCalculator)
  : TdxShapeViewData;
var
  AShape: TdxGpPath;
  ASize: TdxSizeF;
  ...
  begin
    ...
    begin
      ...
      if AArc.Size = nil then
        ASize := TdxSizeF.Create(Abs(ALastPoint.X - P.X),
          Abs(ALastPoint.Y - P.Y))
      else
        ASize := AContext.EvaluateSize(AArc.Size);
      AArcCommand := TdxSVGParserPathCommandArc.Create(ALastPoint, P,
        TdxPointF.Create(ASize.Width, ASize.Height), 0,
        AArc.Direction = TdxSweepDirectionKind.Clockwise, False);
      try
...
begin
  ...
  Properties.FillBrightness := AAttributes.GetValueAsFloat
    ('FillBrightness', 0.0);
  Properties.FillColor := TAttributeReader.GetNullableColor(AAttributes,
    'FillColor');
  Properties.StrokeColor := TAttributeReader.GetNullableColor(AAttributes,
    'StrokeColor');
  Properties.StrokeThickness := TAttributeReader.GetNullableDouble(AAttributes,
    'StrokeThickness');
  Properties.StrokeDashArray := TAttributeReader.GetArrayOfDouble(AAttributes,
    'StrokeDashArray', ' ');
...
class function TArc.GetDirection(AAttributes: TdxXMLNodeAttributes)
  : TdxSweepDirectionKind;
begin
...
class function TdxShapeXmlDescription.TConnectionPoints.Load(ANode: TdxXmlNode)
  : TConnectionPoints;
var
...
class function TdxShapeXmlDescription.TParameter.Evaluate
  (AContext: TdxEvaluatorContext; AOp: TdxCriteriaValue): Single;
begin
  Result := AOp.EvaluateDouble(AContext);
end;

function TdxShapeXmlDescription.TParameter.GetValue(const ASize: TdxSizeF;
...
class function TdxShapeXmlDescription.TParameters.Load(ANode: TdxXmlNode)
  : TParameters;
var
...
function TdxShapeXmlDescription.CreateViewData(const ASize: TdxSizeF;
...
begin
  ...
  try
    AContext := TShapeLayoutCalculator.Create(Rows, Columns, ASize,
      AShapeContext);
    try
...
function TdxShapeXmlDescription.GetDefaultSize(AAttributes
  : TdxXMLNodeAttributes): TdxSizeF;
var
...
function TdxShapeXmlDescription.GetNormalizedParameters(const ASize: TdxSizeF;
...
function TdxShapeXmlDescription.GetParameterDefaultValue(AParameterIndex
  : Integer): Single;
begin
  Result := Parameters[AParameterIndex].DefaultValue;
end;

function TdxShapeXmlDescription.GetParameterPoint(AParameterIndex: Integer;
...
function TdxShapeXmlDescription.GetParameterValue(AParameterIndex: Integer;
...
function TdxShapeXmlDescription.GetParameters(const ASize: TdxSizeF;
  const AParameters: TArray<Single>): TArray<Single>;
var
  I, ACount: Integer;
begin
...
begin
  ...
  UseBackgroundAsForeground := ANode.Attributes.GetValueAsBoolean
    ('UseBackgroundAsForeground', True);
  FRows := TUnitCollection.Parse(ANode.Attributes.GetValueAsString
    ('Rows', '*'));
  FColumns := TUnitCollection.Parse
    (ANode.Attributes.GetValueAsString('Columns', '*'));
end;
...
procedure TdxShapeXmlDescription.ParseSegment(ANode: TdxXmlNode;
  ASegmentClass: TdxShapeSegmentClass);
var
  ASegment: TdxShapeSegment;
begin
...
procedure TdxShapeXmlDescription.Validate;
  ...
  const
    SBadGeometry =
      'Each geometry must include the Start segment followed by at least one Line or Arc segment.';
  begin
    raise EdxFlowChartShapeParserError.CreateFmt
      ('The "%s" custom shape error. %s', [Caption, SBadGeometry]);
  end;

  procedure ErrorIdMissed;
  begin
    raise EdxFlowChartShapeParserError.Create
      ('The custom shape error. The "ID" attribute is missed.');
  end;
...
begin
  ...
  for I := 0 to FSegments.Count - 2 do
    if (FSegments[I].ClassType = TStart) and
      (FSegments[I + 1].ClassType = TStart) then
      ErrorBadGeometry;
end;

{ TdxTemplateLoader }

procedure TdxTemplateLoader.LoadFromStream(AStream: TStream;
  const AAddShape: TAddShapeProcedure);
var
  ADoc: TdxXMLDocument;
