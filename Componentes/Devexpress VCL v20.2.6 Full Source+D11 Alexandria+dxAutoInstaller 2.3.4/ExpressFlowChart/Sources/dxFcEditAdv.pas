...
{$ENDIF}
  ...
  ImgList, ShellAPI, dxflchrt, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxButtons, cxTextEdit,
...
type
  ...
  private
    ...
    procedure LocalizeIndexesMenu(AMenu: TdxBarPopupMenu;
  ...
  public
    ...
    function GetMainItemInUnion(AChart: TdxFlowChart; Obj: TdxFcObject)
      : TdxFcObject;
    function GetUnionByNumber(AChart: TdxFlowChart; Number: Integer)
      : TdxFcObject;
...
uses
  dxEditObj, dxEditCon, dxSelUnion, dxCore, dxFcStrs, Math, dxCoreGraphics,
  dxDPIAwareUtils;
...
procedure TFChartAdvEditor.ChartMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  hTest: TdxFcHitTest;
  R: TRect;
begin
  ...
  if (hTest = [htNowhere]) and (Button = mbLeft) and (not(ssShift in Shift))
  then
...
procedure TFChartAdvEditor.ChartMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    if (Button = mbLeft) and (Abs(DownPoint.X - X) > 5) and
      (Abs(DownPoint.Y - Y) > 5) then
    begin
      FChange := False;
      ...
      FStore := False;
      Chart.CreateObject(ALeft, ATop, Abs(P1.X - P2.X), Abs(P1.Y - P2.Y),
        TdxFcShapeType(sbShape.Tag));
      SelectLastObject;
      ChangeObjects(0);
      FChange := True;
      Obj := Chart.Objects[Chart.ObjectCount - 1];
      if Obj.Text = '' then
        Obj.Text := cxGetResourceString(@sdxFlowChartEditorObject) + ' ' +
          IntToStr(Chart.ObjectCount);
      FStore := True;
  ...
  begin
    ...
    if (Button = mbLeft) and ((Abs(DownPoint.X - X) > 5) or
      (Abs(DownPoint.Y - Y) > 5)) then
    begin
      FChange := False;
      FStore := False;
      ...
      if SObj <> nil then
        SPoint := SObj.GetLinkedPoint(DownPoint.X, DownPoint.Y)
      else
        SPoint := sbSPoint.Tag;
      if DObj <> nil then
        DPoint := DObj.GetLinkedPoint(X, Y)
      else
        DPoint := sbDPoint.Tag;
...
procedure TFChartAdvEditor.ChartMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
...
begin
  ...
  if btnCreateConnect.Down and FNewObject and
    ((Abs(UpPoint.Y - DownPoint.Y) > 5) or (Abs(UpPoint.X - DownPoint.X) > 5))
  then
  begin
    DrawDrag(DownPoint, OldPoint, 2);
      ...
      begin
        ...
        repeat
          ...
          begin
            if Count = 0 then
              CountText := CountText +
                IntToStr(GetNumberByUnion(Chart, FirstObj))
            else
              CountText := CountText + ', ' +
                IntToStr(GetNumberByUnion(Chart, FirstObj));
            Inc(Count);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if Mode in [0, 1] then
          Style := TdxFclStyle(sbStyle.Tag - 1);
        if Mode in [0, 2] then
          Color := pColor.Color;
        if Mode in [0, 3] then
          ArrowSource.ArrowType := TdxFcaType(sbSourceArrow.Tag);
        if Mode in [0, 4] then
          ArrowDest.ArrowType := TdxFcaType(sbDestArrow.Tag);
        if Mode in [0, 5] then
          ArrowSource.Width := sbSArrowSize.Tag * 5 + 5;
        if Mode in [0, 5] then
          ArrowSource.Height := sbSArrowSize.Tag * 5 + 5;
        if Mode in [0, 6] then
          ArrowDest.Width := sbDArrowSize.Tag * 5 + 5;
        if Mode in [0, 6] then
          ArrowDest.Height := sbDArrowSize.Tag * 5 + 5;
        if Mode in [0, 7] then
          SetObjectSource(ObjectSource, sbSPoint.Tag);
        if Mode in [0, 8] then
          SetObjectDest(ObjectDest, sbDPoint.Tag);
        if Mode in [0, 9] then
          Font.Assign(FConnectionFont);
...
begin
  ...
  ASB := nil;
  if APopupMenu = bpmShape then
    ASB := sbShape;
  if APopupMenu = bpmLine then
    ASB := sbLine;
  if APopupMenu = bpmStyle then
    ASB := sbStyle;
  if APopupMenu = bpmSourceArrow then
    ASB := sbSourceArrow;
  if APopupMenu = bpmDestArrow then
    ASB := sbDestArrow;
  if APopupMenu = bpmSArrowSize then
    ASB := sbSArrowSize;
  if APopupMenu = bpmDArrowSize then
    ASB := sbDArrowSize;
  if APopupMenu = bpmSPoint then
    ASB := sbSPoint;
  if APopupMenu = bpmDPoint then
    ASB := sbDPoint;
  if APopupMenu = bpmTextPosition then
    ASB := sbTextPosition;
  if APopupMenu = bpmImagePosition then
    ASB := sbImagePosition;
...
begin
  if (Chart.SelectedObjectCount = 2) and (Chart.SelectedConnectionCount = 0)
  then
  begin
    with Chart do
    begin
  ...
  else if TcxButton(Sender).Down then
  begin
    Chart.ClearSelection;
    Chart.Options := Chart.Options - [fcoMultiSelect];
  end;
...
procedure TFChartAdvEditor.ChartSelected(Sender: TdxCustomFlowChart;
  Item: TdxFcItem);
var
  En: Boolean;
  I: Integer;
begin
  ...
  if Chart.SelectedObjectCount > 0 then
    ...
    begin
      ...
      if sbTextPosition.Tag <> (Integer(VertTextPos) * 3 + Integer(HorzTextPos))
      then
      begin
        sbTextPosition.Tag := (Integer(VertTextPos) * 3 + Integer(HorzTextPos));
        SetGlyph(sbTextPosition, bpmTextPosition);
      end;
      if sbImagePosition.Tag <>
        (Integer(VertImagePos) * 3 + Integer(HorzImagePos)) then
      begin
        sbImagePosition.Tag :=
          (Integer(VertImagePos) * 3 + Integer(HorzImagePos));
  ...
  if Chart.SelectedConnectionCount > 0 then
    ...
    begin
      ...
      if sbSArrowSize.Tag < 1 then
        sbSArrowSize.Tag := 1;
      if sbDArrowSize.Tag < 1 then
        sbDArrowSize.Tag := 1;
...
procedure TFChartAdvEditor.ChartSelection(Sender: TdxCustomFlowChart;
...
begin
  ...
  if (hTest * [htOnConnection, htOnConLabel, htOnArrowSrc, htOnArrowDst]) <> []
  then
    if Chart.SelectedConnectionCount > 0 then
    begin
      FStore := False;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if Chart.SelectedConnections[I].ObjectSource = Chart.SelectedObjects[J]
        then
          SObj := TdxFcObject(List.Items[J]);
        if Chart.SelectedConnections[I].ObjectDest = Chart.SelectedObjects[J]
        then
    ...
    for I := 0 to Buf.Count - 1 do
      ...
      begin
        ...
        for J := 0 to Buf.Count - 1 do
          ...
          begin
            Inc(Index);
            if TdxFcObject(TBufferItem(Buf.Items[J]).Obj)
              = TdxFcConnection(TBufferItem(Buf.Items[I]).Obj).ObjectSource then
              SObj := TdxFcObject(List.Items[Index]);
            if TdxFcObject(TBufferItem(Buf.Items[J]).Obj)
              = TdxFcConnection(TBufferItem(Buf.Items[I]).Obj).ObjectDest then
              DObj := TdxFcObject(List.Items[Index]);
          end;
        ACon.SetObjectSource(SObj, TdxFcConnection(TBufferItem(Buf.Items[I])
          .Obj).PointSource);
        ACon.SetObjectDest(DObj, TdxFcConnection(TBufferItem(Buf.Items[I]).Obj)
          .PointDest);
        ACon.Selected := True;
      end;
  finally
...
begin
  ...
  begin
    ...
    if Mode = 1 then
      case sbShape.Tag of
        0, 1:
          Rectangle(PP1.X, PP1.Y, PP2.X, PP2.Y);
        2:
          Ellipse(PP1.X, PP1.Y, PP2.X, PP2.Y);
        3:
          RoundRect(PP1.X, PP1.Y, PP2.X, PP2.Y, (PP2.X - PP1.X) shr 1,
            (PP2.Y - PP1.Y) shr 1);
        4:
          PolyLine([Point(PP1.X + (PP2.X - PP1.X) shr 1, PP1.Y),
        ...
        5:
          PolyLine([Point(PP1.X + (PP2.X - PP1.X) shr 1, PP1.Y),
            Point(PP2.X, PP2.Y), Point(PP1.X, PP2.Y),
            Point(PP1.X + (PP2.X - PP1.X) shr 1, PP1.Y)]);
        6:
          PolyLine([Point(PP1.X, PP1.Y), Point(PP2.X, PP1.Y),
            Point(PP1.X + (PP2.X - PP1.X) shr 1, PP2.Y), Point(PP1.X, PP1.Y)]);
        7:
          PolyLine([Point(PP1.X, PP1.Y), Point(PP2.X, PP1.Y + (PP2.Y - PP1.Y)
            shr 1), Point(PP1.X, PP2.Y), Point(PP1.X, PP1.Y)]);
        8:
          PolyLine([Point(PP1.X, PP1.Y + (PP2.Y - PP1.Y) shr 1),
            Point(PP2.X, PP1.Y), Point(PP2.X, PP2.Y),
            Point(PP1.X, PP1.Y + (PP2.Y - PP1.Y) shr 1)]);
        9:
          PolyLine([Point(PP1.X + (PP2.X - PP1.X) shr 2, PP1.Y),
...
procedure TFChartAdvEditor.ChartKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  P: TPoint;
begin
  ...
  if (Key = VK_INSERT) and (ssCtrl in Shift) and iCopy.Enabled then
    iCopyClick(iCopy);
  if (Key = VK_INSERT) and (ssShift in Shift) and iPaste.Enabled then
    iPasteClick(iPaste);
  if (Key = VK_DELETE) and (ssShift in Shift) and iCut.Enabled then
    iCutClick(iCut);
  ...
  begin
    ...
    begin
      ...
      if (Abs(DownPoint.X - P.X) > 5) and (Abs(DownPoint.Y - P.Y) > 5) and
        (Key <> VK_ESCAPE) then
    ...
    begin
      P := Chart.ScreenToClient(GetMouseCursorPos);
      if ((Abs(DownPoint.X - P.X) > 5) or (Abs(DownPoint.Y - P.Y) > 5)) and
        (Key <> VK_ESCAPE) then
        ChartMouseUp(Chart, mbLeft, [], P.X, P.Y)
      else
      begin
...
procedure TFChartAdvEditor.ChartKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_MENU then
  begin
...
function TFChartAdvEditor.GetMainItemInUnion(AChart: TdxFlowChart;
  Obj: TdxFcObject): TdxFcObject;
var
  I: Integer;
begin
  Result := nil;
...
function TFChartAdvEditor.GetUnionByNumber(AChart: TdxFlowChart;
  Number: Integer): TdxFcObject;
var
  I, Count: Integer;
begin
  Count := 0;
...
begin
  ...
  if Item = miPaste then
  begin
    Result := 5;
    exit;
  end;
  if Item = miDelete then
  begin
    Result := 6;
    exit;
  end;
  if Item = miSelectAll then
  begin
    Result := 7;
    exit;
  end;
  if Item = miBringToFront then
  begin
    Result := 8;
    exit;
  end;
  if Item = miSendToBack then
  begin
    Result := 9;
    exit;
  end;
  if Item = miNewUnion then
  begin
    Result := 10;
    exit;
  end;
  if Item = miAddToUnion then
  begin
    Result := 11;
    exit;
  end;
  if Item = miRemoveFromUnion then
  begin
    Result := 12;
    exit;
  end;
  ...
  if Item = iRectangle then
  begin
    Result := 0;
    exit;
  end;
  if Item = iEllipse then
  begin
    Result := 1;
    exit;
  end;
  if Item = iRoundRect then
  begin
    Result := 2;
    exit;
  end;
  if Item = iDiamond then
  begin
    Result := 3;
    exit;
  end;
  if Item = iNorthTriangle then
  begin
    Result := 4;
    exit;
  end;
  if Item = itSouthTriangle then
  begin
    Result := 5;
    exit;
  end;
  if Item = itEastTriangle then
  begin
    Result := 6;
    exit;
  end;
  if Item = itWestTriangle then
  begin
    Result := 7;
    exit;
  end;
  if Item = itHexagon then
  begin
    Result := 8;
    exit;
  end;
  ...
  if Item = iStraight then
  begin
    Result := 19;
    exit;
  end;
  if Item = iCurved then
  begin
    Result := 20;
    exit;
  end;
  if Item = iRectHorizontal then
  begin
    Result := 21;
    exit;
  end;
  if Item = iRectVertical then
  begin
    Result := 22;
    exit;
  end;
  ...
  if Item = iSArrow then
  begin
    Result := 24;
    exit;
  end;
  if Item = iSOvalArrow then
  begin
    Result := 25;
    exit;
  end;
  if Item = iSRectArrow then
  begin
    Result := 26;
    exit;
  end;
  ...
  if Item = iDArrow then
  begin
    Result := 28;
    exit;
  end;
  if Item = iDOvalArrow then
  begin
    Result := 29;
    exit;
  end;
  if Item = iDRectArrow then
  begin
    Result := 30;
    exit;
  end;
  ...
  if Item = iTextTopLeft then
  begin
    Result := 57;
    exit;
  end;
  if Item = iTextTop then
  begin
    Result := 58;
    exit;
  end;
  if Item = iTextTopRight then
  begin
    Result := 59;
    exit;
  end;
  if Item = iTextLeft then
  begin
    Result := 60;
    exit;
  end;
  if Item = iTextCenter then
  begin
    Result := 61;
    exit;
  end;
  if Item = iTextRight then
  begin
    Result := 62;
    exit;
  end;
  if Item = iTextBottomLeft then
  begin
    Result := 63;
    exit;
  end;
  if Item = iTextBottom then
  begin
    Result := 64;
    exit;
  end;
  if Item = iTextBottomRight then
  begin
    Result := 65;
    exit;
  end;
  if Item = iImageTopLeft then
  begin
    Result := 66;
    exit;
  end;
  if Item = iImageTop then
  begin
    Result := 67;
    exit;
  end;
  if Item = iImageTopRight then
  begin
    Result := 68;
    exit;
  end;
  if Item = iImageLeft then
  begin
    Result := 69;
    exit;
  end;
  if Item = iImageCenter then
  begin
    Result := 70;
    exit;
  end;
  if Item = iImageRight then
  begin
    Result := 71;
    exit;
  end;
  if Item = iImageBottomLeft then
  begin
    Result := 72;
    exit;
  end;
  if Item = iImageBottom then
  begin
    Result := 73;
    exit;
  end;
  if Item = iImageBottomRight then
  begin
    Result := 74;
    exit;
  end;
  ...
  if Item = iDelete then
  begin
    Result := 6;
    exit;
  end;
  if Item = iSelectAll then
  begin
    Result := 7;
    exit;
  end;
  if Item = iBringToFront then
  begin
    Result := 8;
    exit;
  end;
  if Item = iSendToBack then
  begin
    Result := 9;
    exit;
  end;
  if Item = iNewUnion then
  begin
    Result := 10;
    exit;
  end;
  if Item = iAddToUnion then
  begin
    Result := 11;
    exit;
  end;
  if Item = iRemoveFromUnion then
  begin
    Result := 12;
    exit;
  end;
  if Item = miZoomIn then
  begin
    Result := 13;
    exit;
  end;
  if Item = miZoomOut then
  begin
    Result := 14;
    exit;
  end;
...
begin
  ...
  try
    ...
    Chart.Font.Height := ScaleFactor.Apply(AChart.Font.Height,
      TdxFlowChartAccess(AChart).ScaleFactor);
    Chart.Zoom := 100;
...
function TFChartAdvEditor.CanKeyEnter(Edit: TcxTextEdit; Key: Char;
  MinValue, MaxValue: Integer): Boolean;
var
  Text: string;
  Value: Integer;
...
begin
  for I := 0 to ComponentCount - 1 do
    if Components[I] is TdxBarItem then
      TdxBarItem(Components[I]).ImageIndex :=
        GetImageIndexByMenuItem(TdxBarItem(Components[I]));
end;
...
begin
  ...
  miBringToFront.Caption := cxGetResourceString
    (@sdxFlowChartEditorEditBringToFront);
  miClearSelection.Caption := cxGetResourceString
  ...
  miSendToBack.Caption := cxGetResourceString
  ...
  iBringToFront.Caption := cxGetResourceString
    (@sdxFlowChartEditorEditBringToFront);
  iClearAllUnions.Caption := cxGetResourceString
    (@sdxFlowChartEditorUnionsClearAll);
  iClearSelection.Caption := cxGetResourceString
    (@sdxFlowChartEditorEditClearSelection);
  ...
  iRemoveFromUnion.Caption := cxGetResourceString
  ...
  miActualSize.Caption := cxGetResourceString
    (@sdxFlowChartEditorViewActualSize);
  miAntialiasing.Caption := cxGetResourceString
  ...
  btnCreateConnect.Hint := cxGetResourceString
    (@sdxFlowChartEditorCreateConnectionHint);
  btnCreateObject.Hint := cxGetResourceString
  ...
  sbTextPosition.Hint := cxGetResourceString
    (@sdxFlowChartEditorObjectTextPositionHint);
  sbImagePosition.Hint := cxGetResourceString
    (@sdxFlowChartEditorObjectImagePositionHint);
  sbObjectFont.Hint := cxGetResourceString
  ...
  miRemoveFromUnion.Caption := cxGetResourceString
    (@sdxFlowChartEditorUnionsRemove);
  ...
  miDynamicMoving.Caption := cxGetResourceString
    (@sdxFlowChartEditorOptionsDynamicMoving);
  miDynamicSizing.Caption := cxGetResourceString
...
begin
  ...
  begin
    ...
    AItem.Caption := cxGetResourceString(dxFlowChartArrowStyleNamesMap
      [TdxFcaType(AItem.Tag)]);
  end;
end;

procedure TFChartAdvEditor.LocalizeIndexesMenu(AMenu: TdxBarPopupMenu;
  const AFormatString: string);
var
  AItem: TdxBarItem;
  I: Integer;
...
begin
  ...
  for I := 0 to ACount - 1 do
    AMenu.ItemLinks[I].Item.Caption :=
      cxGetResourceString(dxFlowChartLayoutNamesMap[I]);
end;
...
begin
  ...
  begin
    ...
    AItem.Caption := cxGetResourceString(dxFlowChartConnectionStyleNamesMap
      [TdxFclStyle(AItem.Tag - 1)]);
  end;
end;
