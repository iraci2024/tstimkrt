...
type
  ...
  public
    class function Manhattan(const APoint1, APoint2: TPoint): Single;
      static; inline;
    class function Euclidean(const APoint1, APoint2: TPoint): Single;
      static; inline;
  end;
...
{$REGION 'internal classes'}
    ...
    TDirectionValue = record
      ...
      class function Create(const AValue: Single; ADirection: TDirection)
        : TDirectionValue; static; inline;
    end;
    ...
    TPointData = record
      ...
      procedure Init(const APoint: TPoint; ARouteOverlap: Boolean);
        overload; inline;
    end;
    ...
    strict protected
      ...
      procedure AddInterval(ALine, AValue1, AValue2: Integer;
        const AIntervals: TPointIntervals); overload;
      function HasIntersection(ALine, AValue1, AValue2: Integer;
        const AIntervals: TPointIntervals): Boolean; overload;
    public
      constructor Create;
    ...
    strict private
      ...
      class procedure AddAdditionalLine(ALine: Integer;
        var ALines: TArray<Integer>); static; inline;
      class procedure AddAdditionalLines(ALine, AOffset: Integer;
        var ALines: TArray<Integer>); static; inline;
      class function BinarySearch(const AValues: TArray<Integer>;
        AItem: Integer; out AFoundIndex: Integer): Boolean; static;
      class procedure InsertArrayItem(AItem: Integer;
        var AValues: TArray<Integer>; AIndex: Integer); static;
      function GetParentDirection(AParent: TAStarNode; const AEnd: TPoint)
        : TParentDirection;
      procedure PrepareLines(AValue1, AValue2: Integer;
        const APattern: TArray<Integer>; var ALines: TArray<Integer>);
      procedure ProcessHorizontalDirection(AForward: Boolean;
        const APoint: TPoint; var APointData: TPointData;
        AIntervals: TPathIntervals; AMargin: Integer);
      procedure ProcessVerticalDirection(AForward: Boolean;
        const APoint: TPoint; var APointData: TPointData;
        AIntervals: TPathIntervals; AMargin: Integer);
    public
      constructor Create(const ABounds: TArray<TRect>);
      procedure Prepare(const AStart, AEnd: TPoint);
      function GetNeighbors(ANode: TAStarNode; AMargin: Integer;
        AIntervals: TPathIntervals): TNeighbors;
    ...
    strict private
      class var FComparer: IComparer<TTuple>;
    ...
    strict private
      ...
      FPenalty: Single;
      function CalculateG(ANode: TAStarNode; const APoint, ATarget: TPoint;
        const AHasIntersection: THasIntersection; AIntervals: TPathIntervals;
        ARouteOverlap: Boolean = False): Single;
      function CalculatePoint(const APoint: TPoint;
        const ASource, ATarget: TdxNullableRect): TPoint;
      function CalculateStartNode(AHasIntersection: THasIntersection;
    ...
    public
      ...
      destructor Destroy; override;
      function FindPath(const AHasIntersection: THasIntersection;
        AGrid: TDiagramGrid; AIntervals: TPathIntervals;
        const AStart, ATarget: TPoint;
...
{$ENDREGION}
  ...
  public
    constructor Create(AChart: TdxCustomFlowChart;
      AItemMargin: Integer); override;
    destructor Destroy; override;
    function RouteConnection(AConnection: TdxFcConnection;
      const AStart, AEnd: TPoint): TArray<TPoint>; override;
  end;
...
type
  ...
  public
    class function GetPointPosition(const AStartPoint: TPoint;
      const ADistanceCoef: Single; const AEndLinePoint: TPoint): TPoint; inline;
    class function IsCorner(const APrevPoint, ACurrentPoint, ANextPoint: TPoint)
      : Boolean; static; inline;
  end;

  { TRoutingHelper }

class function TRoutingHelper.GetPointPosition(const AStartPoint: TPoint;
  const ADistanceCoef: Single; const AEndLinePoint: TPoint): TPoint;
begin
  Result.X := Round(AStartPoint.X + (AEndLinePoint.X - AStartPoint.X) *
    ADistanceCoef);
  Result.Y := Round(AStartPoint.Y + (AEndLinePoint.Y - AStartPoint.Y) *
    ADistanceCoef);
end;

class function TRoutingHelper.IsCorner(const APrevPoint, ACurrentPoint,
  ANextPoint: TPoint): Boolean;
begin
  Result := not(((APrevPoint.X = ACurrentPoint.X) and
    (ACurrentPoint.X = ANextPoint.X)) or ((APrevPoint.Y = ACurrentPoint.Y) and
    (ACurrentPoint.Y = ANextPoint.Y)));
end;

{ TdxHeuristicFunctions }

class function TdxHeuristicFunctions.Manhattan(const APoint1,
  APoint2: TPoint): Single;
begin
  Result := Abs(APoint1.X - APoint2.X) + Abs(APoint1.Y - APoint2.Y);
end;

class function TdxHeuristicFunctions.Euclidean(const APoint1,
  APoint2: TPoint): Single;
begin
...
class function TdxRightAngleRoutingStrategy.TDirectionValue.Create
  (const AValue: Single; ADirection: TDirection): TDirectionValue;
begin
  Result.Value := AValue;
...
procedure TdxRightAngleRoutingStrategy.TPointData.Init(AX, AY: Integer;
  ARouteOverlap: Boolean);
begin
  Point.X := AX;
  Point.Y := AY;
...
procedure TdxRightAngleRoutingStrategy.TPointData.Init(const APoint: TPoint;
  ARouteOverlap: Boolean);
begin
  Point := APoint;
  RouteOverlap := ARouteOverlap;
...
constructor TdxRightAngleRoutingStrategy.TDiagramGrid.Create
  (const ABounds: TArray<TRect>);
var
...
class procedure TdxRightAngleRoutingStrategy.TDiagramGrid.AddAdditionalLine
  (ALine: Integer; var ALines: TArray<Integer>);
var
  AIndex: Integer;
begin
...
class procedure TdxRightAngleRoutingStrategy.TDiagramGrid.AddAdditionalLines
  (ALine, AOffset: Integer; var ALines: TArray<Integer>);
begin
  AddAdditionalLine(ALine + AOffset, ALines);
  AddAdditionalLine(ALine - AOffset, ALines);
end;

class function TdxRightAngleRoutingStrategy.TDiagramGrid.BinarySearch
  (const AValues: TArray<Integer>; AItem: Integer;
...
class procedure TdxRightAngleRoutingStrategy.TDiagramGrid.InsertArrayItem
  (AItem: Integer; var AValues: TArray<Integer>; AIndex: Integer);
var
  L: Integer;
begin
  L := Length(AValues);
  ...
  if AIndex < L then
    System.Move(AValues[AIndex], AValues[AIndex + 1],
      (L - AIndex) * SizeOf(Integer));
  AValues[AIndex] := AItem;
end;

function TdxRightAngleRoutingStrategy.TDiagramGrid.GetParentDirection
  (AParent: TAStarNode; const AEnd: TPoint): TParentDirection;
var
  AStart: TPoint;
begin
  if AParent = nil then
...
function TdxRightAngleRoutingStrategy.TDiagramGrid.GetNeighbors
  (ANode: TAStarNode; AMargin: Integer; AIntervals: TPathIntervals): TNeighbors;
var
  APoint: TPoint;
...
begin
  ...
  begin
    ProcessHorizontalDirection(False, APoint, Result.Points[Result.Count],
      AIntervals, AMargin);
    Inc(Result.Count);
  end;
  if AParentDirection <> TParentDirection.Left then
  begin
    ProcessHorizontalDirection(True, APoint, Result.Points[Result.Count],
      AIntervals, AMargin);
    Inc(Result.Count);
  end;
  if AParentDirection <> TParentDirection.Down then
  begin
    ProcessVerticalDirection(False, APoint, Result.Points[Result.Count],
      AIntervals, AMargin);
    Inc(Result.Count);
  end;
  if AParentDirection <> TParentDirection.Up then
  begin
    ProcessVerticalDirection(True, APoint, Result.Points[Result.Count],
      AIntervals, AMargin);
    Inc(Result.Count);
  end;
end;

procedure TdxRightAngleRoutingStrategy.TDiagramGrid.Prepare(const AStart,
  AEnd: TPoint);
begin
  PrepareLines(AStart.X, AEnd.X, FVerticalGridLinesPattern, FVerticalGridLines);
  PrepareLines(AStart.Y, AEnd.Y, FHorizontalGridLinesPattern,
    FHorizontalGridLines);
end;

procedure TdxRightAngleRoutingStrategy.TDiagramGrid.PrepareLines(AValue1,
  AValue2: Integer; const APattern: TArray<Integer>;
  var ALines: TArray<Integer>);
var
  AIndex: Integer;
begin
  SetLength(ALines, Length(APattern));
  Move(Pointer(@APattern[0])^, Pointer(@ALines[0])^,
    SizeOf(Integer) * Length(APattern));
...
procedure TdxRightAngleRoutingStrategy.TDiagramGrid.ProcessHorizontalDirection
  (AForward: Boolean; const APoint: TPoint; var APointData: TPointData;
  AIntervals: TPathIntervals; AMargin: Integer);
var
...
procedure TdxRightAngleRoutingStrategy.TDiagramGrid.ProcessVerticalDirection
  (AForward: Boolean; const APoint: TPoint; var APointData: TPointData;
  AIntervals: TPathIntervals; AMargin: Integer);
var
  AIndex, ALine: Integer;
...
constructor TdxRightAngleRoutingStrategy.TAStarNode.Create(const APosition
  : TPoint; AH: Single);
begin
  FPosition := APosition;
  FH := AH;
...
function TdxRightAngleRoutingStrategy.TAStarNode.GetPath(const AStart,
  AFinish: TPoint): TArray<TPoint>;
var
...
constructor TdxRightAngleRoutingStrategy.TAStarRoutingRunner.Create
  (AItemMargin: Integer);
begin
...
function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.CreateNode
  (const APosition: TPoint; AH: Single): TAStarNode;
begin
  Result := TAStarNode.Create(APosition, AH);
  FObjectsToDelete.Add(Result);
end;

function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.CalculateG
  (ANode: TAStarNode; const APoint, ATarget: TPoint;
  const AHasIntersection: THasIntersection; AIntervals: TPathIntervals;
  ARouteOverlap: Boolean = False): Single;
var
  G: Single;
begin
  ...
  if (ANode.Parent <> nil) and TRoutingHelper.IsCorner(APoint, ANode.Position,
    ANode.Parent.Position) then
    G := G + TdxHeuristicFunctions.Manhattan(APoint, ATarget) * 0.001;
  if AHasIntersection(TRoutingHelper.GetPointPosition(ANode.Position, 0.5,
    APoint)) then
    G := G + 2 * Penalty;
...
function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.CalculatePoint
  (const APoint: TPoint; const ASource, ATarget: TdxNullableRect): TPoint;
var
  ARect: TRect;
...
{$ELSE}
  ...
  ADirectionValues[0] := TDirectionValue.Create(APoint.X - ARect.Left,
    TDirection.Left);
  ADirectionValues[1] := TDirectionValue.Create(APoint.Y - ARect.Top,
    TDirection.Up);
  ADirectionValues[2] := TDirectionValue.Create(ARect.Right - APoint.X,
    TDirection.Right);
  ADirectionValues[3] := TDirectionValue.Create(ARect.Bottom - APoint.Y,
    TDirection.Down);
{$ENDIF}
  ...
  if (ATarget.HasValue and ATarget.Value.Contains(Result)) and
  {$IFDEF DELPHIXE}(ASource <> ATarget){$ELSE}not ASource.IsEqual
    (ATarget){$ENDIF} then
    Exit(APoint);
end;

function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.CalculateStartNode
  (AHasIntersection: THasIntersection; const AStart, ANext, ATarget: TPoint)
  : TAStarNode;
begin
  Result := CreateNode(ANext, TdxHeuristicFunctions.Manhattan(ANext, ATarget));
  if not AStart.IsEqual(ANext) then
  begin
    Result.Parent := CreateNode(AStart, TdxHeuristicFunctions.Manhattan(AStart,
      ATarget));
    Result.G := CalculateG(Result.Parent, ANext, ATarget,
      AHasIntersection, nil);
  end;
end;

function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.CanProcess
  (const ABegin, AEnd: TPoint; const ABeginRect,
  AEndRect: TdxNullableRect): Boolean;
var
...
begin
  ...
  else if AEndRect.HasValue then
  begin
    ATargetRoutingBounds := GetRoutingBounds(AEndRect);
...
function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.FindPath
  (const AHasIntersection: THasIntersection; AGrid: TDiagramGrid;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (ACurrent.Parent <> nil) and ACurrent.Parent.Position.IsEqual
          (ANeighborPoint) then
          Continue;

        G := CalculateG(ACurrent, ANeighborPoint, ATarget, AHasIntersection,
          AIntervals, ANeighbors.Points[I].RouteOverlap);
...
function TdxRightAngleRoutingStrategy.TAStarRoutingRunner.GetRoutingBounds
  (const AObjectBounds: TRect): TRect;
begin
  ...
  FComparer := TComparer<TTuple>.Construct(
    ...
    begin
      if Right.Key < Left.Key then
        Result := -1
      else if Right.Key > Left.Key then
        Result := 1
      else
        Result := 0;
    end);
...
procedure TdxRightAngleRoutingStrategy.TPathIntervals.AddInterval(const AStart,
  AEnd: TPoint);
begin
  if AStart.X = AEnd.X then
...
procedure TdxRightAngleRoutingStrategy.TPathIntervals.AddInterval(ALine,
  AValue1, AValue2: Integer; const AIntervals: TPointIntervals);
var
  L: TList<Integer>;
  ATemp: Integer;
begin
...
procedure TdxRightAngleRoutingStrategy.TPathIntervals.AddPath
  (const APath: TArray<TPoint>);
var
  AStart, AEnd: TPoint;
  I: Integer;
begin
...
function TdxRightAngleRoutingStrategy.TPathIntervals.HasIntersection
  (const APoint, ANext: TPoint): Boolean;
begin
...
function TdxRightAngleRoutingStrategy.TPathIntervals.HasIntersection(ALine,
  AValue1, AValue2: Integer; const AIntervals: TPointIntervals): Boolean;
var
  AList: TList<Integer>;
  I, ATemp: Integer;
begin
...
constructor TdxRightAngleRoutingStrategy.Create(AChart: TdxCustomFlowChart;
AItemMargin: Integer);
begin
...
function TdxRightAngleRoutingStrategy.HasIntersection
...
begin
  ...
  begin
    ...
    begin
      ...
      if FRoutingBounds[I].Contains(FStart) and FRoutingBounds[I].Contains(FEnd)
      then
        Continue;
      Exit(True);
    end;
...
function TdxRightAngleRoutingStrategy.GetRoutingBounds
  (AObject: TdxFcObject): TRect;
begin
  Result.InitSize(AObject.RealLeft, AObject.RealTop, AObject.RealWidth,
    AObject.RealHeight);
...
function TdxRightAngleRoutingStrategy.RouteConnection
  (AConnection: TdxFcConnection; const AStart, AEnd: TPoint): TArray<TPoint>;
var
...
begin
  ...
  try
    ...
    if AObjectDest <> nil then
      ...
      AEndRect.Reset;
    Result := ARunner.FindPath(HasIntersection, FGrid, FPathIntervals, AStart,
      AEnd, AStartRect, AEndRect);
