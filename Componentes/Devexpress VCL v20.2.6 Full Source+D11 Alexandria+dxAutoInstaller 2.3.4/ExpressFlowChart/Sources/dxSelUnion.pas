...
begin
  ...
  begin
    for I := 0 to Chart.SelectedObjectCount - 1 do
      if TdxFlowChartUnionsHelper.IsChildItemInUnion(Chart,
        Chart.SelectedObjects[I]) then
      begin
        AFirstObj := nil;
        repeat
          AFirstObj := TdxFlowChartUnionsHelper.FindUnions(Chart, AFirstObj,
            Chart.SelectedObjects[I]);
          if AFirstObj <> nil then
          begin
            if not FindUnion(lbUnions, AFirstObj) then
              lbUnions.Items.AddObject(GetUnionName(AFirstObj), AFirstObj);
          end;
        until AFirstObj = nil end;
