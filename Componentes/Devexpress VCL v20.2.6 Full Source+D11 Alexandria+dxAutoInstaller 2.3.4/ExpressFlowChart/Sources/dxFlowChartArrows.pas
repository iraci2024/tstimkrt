...
{$REGION 'internal types'}
  public type
    ...
    strict private
      ...
      function CreatePen(AColor: TdxAlphaColor; AWidth: Integer; AScale: Single)
        : TdxGPPen;
    ...
    public
      ...
      procedure Draw(AGraphics: TdxGPGraphics; AScale: Single; APenWidth: Byte;
    ...
    TPopulateSegmentsProc = reference to procedure(ASegments: TSegments;
      ASize: TSize);
{$ENDREGION}
  strict private
    ...
    FTipOffset: Single;
    function GetConnectionPoint(const APoint: TPoint; const ASize: TSize)
      : TdxPointF;
  ...
  strict protected
    ...
    procedure ExcludeConnectionTipFromDrawing(AGraphics: TdxGPGraphics;
      const APoint: TPoint; AScale: Single; APenWidth: Byte);
    function GetArrowBySize(const ASize: TSize): TSegments;
    procedure PrepareGraphics(AGraphics: TdxGPGraphics;
      const ATargetPoint, APrevPoint: TPoint; ASize: TSize;
      APenWidth, APathCount: Integer; AScale: Single);
    procedure UnprepareGraphics(AGraphics: TdxGPGraphics);

    property ConnectorKind: TConnectorKind read FConnectorKind;
    property ExcludeConnectionTipKind: TExcludeConnectorTipKind
      read FExcludeConnectionTipKind;
    property Rotatable: Boolean read FRotatable;
  public
    constructor Create(const ADefaultSize: TSize;
      AConnectorKind: TConnectorKind;
      const APopulateSegmentsProc: TPopulateSegmentsProc;
      ARotatable: Boolean = True;
      AExcludeConnectionTipKind
      : TExcludeConnectorTipKind = TExcludeConnectorTipKind.None); overload;
    constructor CreateSharpTipArrow(const ADefaultSize: TSize;
      const APopulateSegmentsProc: TPopulateSegmentsProc); overload;
    destructor Destroy; override;
    procedure Draw(AGraphics: TdxGPGraphics;
      const ATargetPoint, APrevPoint: TPoint; ASize: TSize; AScale: Single;
      APenWidth: Byte; APenColor, ABrushColor: TdxAlphaColor); virtual;
    procedure DrawPreview(AGraphics: TdxGPGraphics; const ABounds: TRect;
      AScale: Single; ABeginArrow: Boolean); virtual;
    function GetActualSize(const ASize: TSize; APenWidth: Integer;
      AScale: Single; AKeepProportion: Boolean): TSize;
  ...
  public
    ...
    procedure DrawPreview(AID: Integer; AGraphics: TdxGPGraphics;
      const ABounds: TRect; AScale: Single; ABeginArrow: Boolean);

    property Count: Integer read GetCount;
    property Items[Index: Integer]: TdxFlowChartArrowShape
...
constructor TdxFlowChartArrowShape.TSegment.Create(AClosed, AFilled,
...
procedure TdxFlowChartArrowShape.TSegment.AddCircle(X, ACenterY,
  ADiameter: Single);
var
  R: TdxRectF;
  ARadius: Single;
...
function TdxFlowChartArrowShape.TSegment.CreateBrush(AColor: TdxAlphaColor)
  : TdxGPBrush;
begin
  if Transparent or not Closed or
    not(Filled or not TdxAlphaColors.IsTransparentOrEmpty(AColor)) then
    Exit(nil);
  Result := TdxGPBrush.Create;
  Result.Color := AColor;
end;

function TdxFlowChartArrowShape.TSegment.CreatePen(AColor: TdxAlphaColor;
  AWidth: Integer; AScale: Single): TdxGPPen;
begin
...
procedure TdxFlowChartArrowShape.TSegment.Draw(AGraphics: TdxGPGraphics;
  AScale: Single; APenWidth: Byte; APenColor, ABrushColor: TdxAlphaColor);
var
  APen: TdxGPPen;
  ABrush: TdxGPBrush;
begin
...
procedure TdxFlowChartArrowShape.TSegment.ExcludeRegion
...
function TdxFlowChartArrowShape.TSegments.NewPath(AClosed, AFilled,
  ATransparent: Boolean): TSegment;
begin
...
constructor TdxFlowChartArrowShape.Create(const ADefaultSize: TSize;
  AConnectorKind: TConnectorKind;
  const APopulateSegmentsProc: TPopulateSegmentsProc;
  ARotatable: Boolean = True;
  AExcludeConnectionTipKind: TExcludeConnectorTipKind =
  TExcludeConnectorTipKind.None);
begin
...
constructor TdxFlowChartArrowShape.CreateSharpTipArrow(const ADefaultSize
  : TSize; const APopulateSegmentsProc: TPopulateSegmentsProc);
begin
  Create(ADefaultSize, TConnectorKind.Left, APopulateSegmentsProc, True,
    TExcludeConnectorTipKind.Full);
end;
...
function TdxFlowChartArrowShape.GetActualSize(const ASize: TSize;
  APenWidth: Integer; AScale: Single; AKeepProportion: Boolean): TSize;
var
  ARatio: Single;
...
function TdxFlowChartArrowShape.GetConnectionPoint(const APoint: TPoint;
  const ASize: TSize): TdxPointF;
begin
...
procedure TdxFlowChartArrowShape.Draw(AGraphics: TdxGPGraphics;
  const ATargetPoint, APrevPoint: TPoint; ASize: TSize; AScale: Single;
  APenWidth: Byte; APenColor, ABrushColor: TdxAlphaColor);
var
  AArrow: TSegments;
  AActualSize: TSize;
  I: Integer;
begin
  ...
  PrepareGraphics(AGraphics, ATargetPoint, APrevPoint, AActualSize, APenWidth,
    AArrow.Count, AScale);
  try
    ...
    if ExcludeConnectionTipKind <> TExcludeConnectorTipKind.None then
      ExcludeConnectionTipFromDrawing(AGraphics,
        TPoint.Create(0, AActualSize.cy div 2), AScale, APenWidth);
  finally
    UnprepareGraphics(AGraphics);
  end;
end;

procedure TdxFlowChartArrowShape.DrawPreview(AGraphics: TdxGPGraphics;
  const ABounds: TRect; AScale: Single; ABeginArrow: Boolean);
const
  Indent = 8;
var
...
begin
  ...
  if ABeginArrow then
  begin
    ATarget.Init(Round(ABounds.Left + Indent * AScale),
      (ABounds.Bottom + ABounds.Top) div 2);
  ...
  begin
    ATarget.Init(Round(ABounds.Right - Indent * AScale),
      (ABounds.Bottom + ABounds.Top) div 2);
  ...
  try
    ...
    Draw(AGraphics, ATarget, APrev, DefaultSize, AScale, 1,
      TdxAlphaColors.Black, TdxAlphaColors.White);
    AGraphics.Line(ATarget.X, ATarget.Y, APrev.X, APrev.Y, TdxAlphaColors.Black,
      Max(1, AScale));
  finally
...
procedure TdxFlowChartArrowShape.ExcludeConnectionTipFromDrawing
  (AGraphics: TdxGPGraphics; const APoint: TPoint; AScale: Single;
  APenWidth: Byte);
var
  R: TRect;
  W, H: Integer;
...
procedure TdxFlowChartArrowShape.PrepareGraphics(AGraphics: TdxGPGraphics;
  const ATargetPoint, APrevPoint: TPoint; ASize: TSize;
  APenWidth, APathCount: Integer; AScale: Single);
var
  AAngle: Single;
...
begin
  ...
  AAngle := GetAngleABC(TPoint.Create(ATargetPoint.X + 100, ATargetPoint.Y),
    ATargetPoint, APrevPoint);
...
procedure TdxFlowChartArrowShapeRepository.DrawPreview(AID: Integer;
  AGraphics: TdxGPGraphics; const ABounds: TRect; AScale: Single;
  ABeginArrow: Boolean);
var
...
procedure TdxFlowChartArrowShapeRepository.AddArrow(AID: Integer;
  AShape: TdxFlowChartArrowShape);
begin
  try
    FIDs.Add(AID, AShape);
...
function TdxFlowChartArrowShapeRepository.GetShapeByID(AID: Integer)
  : TdxFlowChartArrowShape;
begin
  Result := FIDs[AID];
end;
...
function TdxFlowChartArrowShapeRepository.GetItem(Index: Integer)
  : TdxFlowChartArrowShape;
begin
  Result := FItems[Index];
end;
...
begin
  AddArrow(Ord(fcaArrow), TdxFlowChartArrowShape.CreateSharpTipArrow
    (TSize.Create(10, 7),
    ...
    begin
      ...
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(ASize.cx, 0),
        TdxPointF.Create(0.0, Trunc(ASize.cy / 2)), TdxPointF.Create(ASize.cx,
        ASize.cy));
      if Odd(ASize.cy) then
        APoints[1].Y := APoints[1].Y + 1;
      APath.AddPolyline(APoints);
    end));
  AddArrow(Ord(fcaEllipse), TdxFlowChartArrowShape.Create(TSize.Create(7, 7),
    TdxFlowChartArrowShape.TConnectorKind.Center,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
    begin
  ...
  AddArrow(Ord(fcaRectangle), TdxFlowChartArrowShape.Create(TSize.Create(7, 7),
    TdxFlowChartArrowShape.TConnectorKind.Center,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
...
procedure TdxFlowChartArrowShapeRepository.CreateASMEArrow(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.CreateSharpTipArrow(TSize.Create(11, 5),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
      APoints: TArray<TdxPointF>;
    begin
      ...
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(0.0, ASize.cy div 2),
        TdxPointF.Create(ASize.cx, 0), TdxPointF.Create(ASize.cx, ASize.cy));
      APath.AddPolygon(APoints);
      APath.FigureFinish;
    end));
end;

procedure TdxFlowChartArrowShapeRepository.CreateClosedOneDash(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.Create(TSize.Create(10, 7),
    TdxFlowChartArrowShape.TConnectorKind.Left,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
    begin
...
procedure TdxFlowChartArrowShapeRepository.CreateClosedTwoDash(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.Create(TSize.Create(13, 7),
    TdxFlowChartArrowShape.TConnectorKind.Left,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
    begin
      ...
      APath.FigureFinish;
      APath := AShape.NewPath(True, False, False);
      APath.AddLine(MulDiv(ASize.cx, 3, 4), 0, MulDiv(ASize.cx, 3, 4),
        ASize.cy);
      APath.FigureFinish;
    end));
end;

procedure TdxFlowChartArrowShapeRepository.CreateClosedThreeDash(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.Create(TSize.Create(16, 7),
    TdxFlowChartArrowShape.TConnectorKind.Left,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
    begin
      ...
      APath.AddLine(ASize.cx, 0, ASize.cx, ASize.cy);
      APath := AShape.NewPath(True, False, False);
      APath.AddLine(MulDiv(ASize.cx, 3, 5), 0, MulDiv(ASize.cx, 3, 5),
        ASize.cy);
      APath.FigureFinish;
      APath := AShape.NewPath(True, False, False);
      APath.AddLine(MulDiv(ASize.cx, 4, 5), 0, MulDiv(ASize.cx, 4, 5),
        ASize.cy);
      APath.FigureFinish;
    end));
end;
...
begin
  AddArrow(AID, TdxFlowChartArrowShape.Create(TSize.Create(7, 7),
    TdxFlowChartArrowShape.TConnectorKind.Left,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
...
procedure TdxFlowChartArrowShapeRepository.CreateSharpArrow(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.CreateSharpTipArrow(TSize.Create(10, 7),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
      APoints: TArray<TdxPointF>;
    begin
      APath := AShape.NewPath(True, AFilled, False);
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(ASize.cx, 0),
        TdxPointF.Create(0.0, ASize.cy div 2), TdxPointF.Create(ASize.cx,
        ASize.cy));
      APath.AddPolyline(APoints);
      APath.FigureFinish;
    end));
...
begin
  AddArrow(AID, TdxFlowChartArrowShape.Create(TSize.Create(10, 7),
    ...
    begin
      ...
      APath.AddLine(ASize.cx, 0, ASize.cx, ASize.cy);
      APath.FigureFinish;
      APath.AddLine(MulDiv(ASize.cx, 2, 3), 0, MulDiv(ASize.cx, 2, 3),
        ASize.cy);
      APath.FigureFinish;
    end));
end;
...
begin
  AddArrow(AID, TdxFlowChartArrowShape.Create(TSize.Create(13, 7),
    ...
    begin
      ...
      APath.AddLine(ASize.cx, 0, ASize.cx, ASize.cy);
      APath.FigureFinish;
      APath.AddLine(MulDiv(ASize.cx, 2, 4), 0, MulDiv(ASize.cx, 2, 4),
        ASize.cy);
      APath.FigureFinish;
      APath.AddLine(MulDiv(ASize.cx, 3, 4), 0, MulDiv(ASize.cx, 3, 4),
        ASize.cy);
...
begin
  AddArrow(Ord(fcaDimensionLine), TdxFlowChartArrowShape.Create(TSize.Create(7,
    7), TdxFlowChartArrowShape.TConnectorKind.Left,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
    begin
      ...
      APath.FigureFinish;
    end, True, TdxFlowChartArrowShape.TExcludeConnectorTipKind.Half));
  AddArrow(Ord(fcaBackslash), TdxFlowChartArrowShape.Create(TSize.Create(13, 7),
    TdxFlowChartArrowShape.TConnectorKind.Left,
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
...
procedure TdxFlowChartArrowShapeRepository.CreateDiamond(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.CreateSharpTipArrow(TSize.Create(15, 9),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
      APoints: TArray<TdxPointF>;
    begin
      APath := AShape.NewPath(True, AFilled, False);
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(0.0, ASize.cy div 2),
        TdxPointF.Create(ASize.cx div 2, 0), TdxPointF.Create(ASize.cx,
        ASize.cy div 2), TdxPointF.Create(ASize.cx div 2, ASize.cy));
      APath.AddPolygon(APoints);
      APath.FigureFinish;
    end));
end;

procedure TdxFlowChartArrowShapeRepository.CreateClosedDiamond(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.CreateSharpTipArrow(TSize.Create(23, 9),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
      APoints: TArray<TdxPointF>;
    ...
    begin
      ...
      R := TRect.CreateSize(TSize.Create(MulDiv(ASize.cx, 3, 11),
        AEllipseHeight));
      R.Offset(0, (ASize.cy - AEllipseHeight) div 2);
      APath.AddEllipse(R);
      APath.FigureFinish;
      APath := AShape.NewPath(True, False, True);
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(ASize.cx / 10 * 3 +
        1, ASize.cy div 2), TdxPointF.Create(ASize.cx / 10 * 6 + 1, 0),
...
begin
  AddArrow(Ord(fcaClosedDoubleArrow), TdxFlowChartArrowShape.CreateSharpTipArrow
    (TSize.Create(15, 6),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
    ...
    begin
      APath := AShape.NewPath(True, False, False);
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(0.0, ASize.cy div 2),
        TdxPointF.Create(ASize.cx div 2, -1), TdxPointF.Create(ASize.cx div 2,
        ASize.cy));
      APath.AddPolygon(APoints);
      APath.FigureFinish;
      APath := AShape.NewPath(True, False, False);
      APoints := TArray<TdxPointF>.Create(TdxPointF.Create(ASize.cx div 2,
        ASize.cy div 2), TdxPointF.Create(ASize.cx, -1),
        TdxPointF.Create(ASize.cx, ASize.cy));
      APath.AddPolygon(APoints);
      APath.FigureFinish;
    end));
  AddArrow(Ord(fcaFilledDoubleArrow), TdxFlowChartArrowShape.CreateSharpTipArrow
    (TSize.Create(15, 6),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
    var
      APath: TdxFlowChartArrowShape.TSegment;
...
procedure TdxFlowChartArrowShapeRepository.CreateIndentedArrow(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.CreateSharpTipArrow(TSize.Create(10, 7),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
...
procedure TdxFlowChartArrowShapeRepository.CreateOutdentedArrow(AID: Integer;
AFilled: Boolean);
begin
  AddArrow(AID, TdxFlowChartArrowShape.CreateSharpTipArrow(TSize.Create(10, 7),
    procedure(AShape: TdxFlowChartArrowShape.TSegments; ASize: TSize)
