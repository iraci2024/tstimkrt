...
uses
  DesignEditors, DesignIntf, Windows, Classes, DB, dxmdaset, dxmdsedt, dxmdatps,
  ShellAPI, SysUtils, dxCore, dxCoreReg,
...
begin
  case AIndex of
    0:
      ShowMemDataFieldEditor(MemData, Designer);
    1:
      ShowMemDataPersistentEditor(MemData, Designer);
  end;
end;
...
begin
  with TDataSet(GetComponent(0)) do
    for I := 0 to FieldCount - 1 do
      if (Fields[I].Owner = Owner) and (Fields[I].FieldName <> '') then
...
begin
  ...
  for I := 0 to MemData.FieldCount - 1 do
    if (MemData.Fields[I].Owner = MemData.Owner) and
      (MemData.Fields[I].FieldName <> '') then
