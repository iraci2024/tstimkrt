...
uses
  ...
  cxTreeView, ExtCtrls, cxGraphics, cxLookAndFeels, cxEdit, dxLayoutContainer,
...
procedure ShowPivotGridDesigner(ADesigner: IDesigner;
...
procedure TfrmPivotGridDesigner.SelectionsChanged(const ASelection
  : TDesignerSelectionList);
begin
  if (PivotGrid = nil) or PivotGrid.IsLoading or PivotGrid.IsDestroying or FIsLocked
  then
    Exit;
...
begin
  ...
  AField.Name := CreateUniqueName(PivotGrid.Owner, PivotGrid, AField,
    APrefix, '');
  UpdateFieldList;
...
begin
  ...
  begin
    ...
    else if TcxDragControlObject(ASource).Control = tvGroups then
    begin
...
function TfrmPivotGridDesigner.GetGroup(AGroupOnly: Boolean = False)
...
procedure TfrmPivotGridDesigner.ReindexFields(AList: TList; ANewIndex: Integer);
var
  I: Integer;
begin
...
begin
  ...
  btnDelete.Enabled := (lbFields.SelCount > 0) and
    CanChangeComponentList(PivotGrid);
...
procedure TfrmPivotGridDesigner.UpdateFieldListBox(AListBox: TcxListBox;
  AUnlinkedOnly: Boolean);
var
  AField: TcxPivotGridField;
...
begin
  ...
  ListBoxSaveSelection(AListBox.InnerListBox, ASelection, AItemIndex,
    ATopIndex);
  try
    ...
    // if not AUnlinkedOnly or (pcDesigner.ActivePage = tbsGroups) then
    ListBoxRestoreSelection(AListBox.InnerListBox, ASelection, AItemIndex,
      ATopIndex);
  end;
  UpdateButtons;
end;
...
begin
  ListBoxDragOver(lbFields.InnerListBox, Sender, Source, X, Y, State, Accept,
    FPrevDragIndex);
end;
...
procedure TfrmPivotGridDesigner.lbFieldsDragDrop(Sender, Source: TObject;
  X, Y: Integer);
begin
  ListBoxDragDrop(lbFields.InnerListBox, Sender, Source, X, Y, FPrevDragIndex,
    ReindexFields);
  UpdateDesigner;
end;
...
procedure TfrmPivotGridDesigner.lbFieldsEndDrag(Sender, Target: TObject;
  X, Y: Integer);
begin
...
procedure TreeViewSaveSelection(ATreeView: TTreeView; var ASelection: TList;
...
procedure TreeViewRestoreSelection(ATreeView: TTreeView; var ASelection: TList;
  ...
  begin
    while AIndex > ATreeView.Items.Count - 1 do
      Dec(AIndex);
    Result := AIndex >= 0;
  end;
...
begin
  ...
  TreeViewSaveSelection(tvGroups.InnerTreeView, ASelection, AItemIndex,
    ATopIndex);
  try
    ...
    begin
      ...
      if AGroup <> DeletedGroup then
        ...
        begin
          ...
          ANode := tvGroups.Items.AddChildObject(AGroupNode,
            AField.Name, AField);
    ...
    ASelList.Free;
    TreeViewRestoreSelection(tvGroups.InnerTreeView, ASelection, AItemIndex,
      ATopIndex, ACurSelectedNode = nil);
  ...
  begin
    ...
    begin
      if ANode.Level = 0 then
        ...
        Result := (ANode.getNextSibling <> nil) or
          (ANode.Parent.getNextSibling <> nil);
    end;
  end;
    ...
    case TButton(Sender).Tag of
      0:
        begin
          ...
          tvGroups.Items.AddChildObject(nil, AGroup.Caption, AGroup)
            .Selected := True;
        end;
      1:
        begin
        ...
        begin
          ...
          for I := 0 to lbUnlinkedFields.Items.Count - 1 do
            ...
            begin
              ...
              if ANode <> nil then
              begin
                tvGroups.Items.AddChildObject(ANode, AField.Name, AField)
                  .Selected := True;
              end;
            end;
        end;
...
procedure TfrmPivotGridDesigner.tvGroupsCustomDrawItem(Sender: TCustomTreeView;
  ...
  begin
    ...
    begin
      ...
      if ANode.Index < (AGroup.FieldCount - 1) then
        AGroup.Insert(ANode.Index + 1, AField)
      else if AGroup.Index < (AGroup.Groups.Count - 1) then
      begin
        AGroup := AGroup.Groups[AGroup.Index + 1];
...
procedure TfrmPivotGridDesigner.tvGroupsEdited(Sender: TObject; Node: TTreeNode;
  var S: String);
begin
...
procedure TfrmPivotGridDesigner.tvGroupsKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  UpdateGroupsButtons;
  SyncGroupSelection;
