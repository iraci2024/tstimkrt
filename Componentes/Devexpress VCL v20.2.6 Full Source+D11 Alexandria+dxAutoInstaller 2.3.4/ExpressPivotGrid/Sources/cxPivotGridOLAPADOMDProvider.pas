...
uses
  Windows, Dialogs, Contnrs, Types, Classes, SysUtils, Forms, Math, Variants,
  StrUtils, DB, ADODB, ComObj,
  ActiveX, Generics.Defaults, Generics.Collections, dxCore, cxClasses,
...
{$REGION 'internal types'}
  ...
  public
    ...
    procedure Open(const ACommandText: string;
      const AActiveConnection: IDispatch);
  end;
  ...
  protected type
    ...
    public
      ...
      destructor Destroy; override;
      class function ExtractMeasureUniqueName(var AUniqueNames
...
{$ENDREGION}
  ...
  strict private
    FCatalog: ICatalog;
    function AddFieldToDataSet(ADataSet: TDataSet; const AFieldName: string;
    ...
    function DoGetMeasures(const ACube: CubeDef)
      : TcxPivotGridOLAPMemDataDataSet;
    function DoGetHierarchies(const ACube: CubeDef)
      : TcxPivotGridOLAPMemDataDataSet;
    ...
    function GetPropertyValue(const AProperties: Properties;
      const APropertyName: string; const ADefaultValue: Variant): Variant;
    function GetPropertyIntValue(const AProperties: Properties;
      const APropertyName: string; const ADefaultValue: Integer = 0): Integer;
    function GetPropertyStrValue(const AProperties: Properties;
      const APropertyName: string; const ADefaultValue: string = ''): string;
  ...
  protected
    ...
    function Execute(const ACommandText: string)
      : TcxCustomPivotGridOLAPDataSet; override;
    function Execute(const AQuery: TcxMDXQuery)
      : TcxCustomPivotGridOLAPDataSet; override;
    function GetConnected: Boolean; override;
    function GetDrillDownDataSourceClass
      : TcxCustomPivotGridOLAPDrillDownDataSourceClass; override;

    procedure PopulateCatalogs(ANames: TStrings); override;
    procedure PopulateHierarchyAllMemberNames(const ACubeName: string;
      ANames: TDictionary<string, string>); override;
    function OpenSchema(const ACubeName: string;
      ASchema: TcxPivotGridOLAPSchemaInfo): TcxPivotGridOLAPDBDataSet; override;
    ...
    function CreateCrossCellsReader(ADataSet: TcxCustomPivotGridOLAPDataSet;
      ...
      : TcxCustomPivotGridOLAPDataSetReader; override;
  ...
  TcxCustomPivotGridOLAPADOMDDataSetReader = class
  ...
  TcxPivotGridOLAPADOMDDataSetCrossCellsReader = class
    (TcxCustomPivotGridOLAPADOMDDataSetReader)
  private
  ...
  protected
    procedure Initialize(AOwnedGroup, ARowGroups, AColumnGroups
      : TcxPivotGridGroupItem; ASummaryFields: TcxPivotGridFields);
  ...
  TcxPivotGridOLAPADOMDDataSetMembersReader = class
    (TcxCustomPivotGridOLAPADOMDDataSetReader)
  strict private
    FGroupValues: TcxPivotGridOLAPGroupInfos;
    FField: TcxPivotGridField;
  protected
    procedure Initialize(AGroupValues: TcxPivotGridOLAPGroupInfos;
  ...
  TcxPivotGridOLAPADOMDDataSetGrandTotalReader = class
    (TcxCustomPivotGridOLAPADOMDDataSetReader)
  strict private
    FRowGroups: TcxPivotGridGroupItem;
    FColumnGroups: TcxPivotGridGroupItem;
    ...
    FSummaryFields: TcxPivotGridFields;
  protected
    procedure Initialize(AColumnGroups, ARowGroups: TcxPivotGridGroupItem;
      ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList);
  ...
  TcxPivotGridOLAPADOMDDataSetMainGrandTotalReader = class
    (TcxCustomPivotGridOLAPADOMDDataSetReader)
  strict private
  ...
  protected
    procedure Initialize(AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
      ADataFields: TcxPivotGridFields);
...
{$ENDREGION}
  strict private
    ...
    function GetGroupByUniqueName(AGroups: TcxPivotGridGroupItem;
...
procedure TdxADORecordset.Open(const ACommandText: string;
  const AActiveConnection: IDispatch);
begin
  FRecordset.Open(ACommandText, AActiveConnection, adOpenStatic,
    adLockReadOnly, 0);
end;
...
constructor TcxPivotGridOLAPCellCoordinate.Create(const AColumn,
  ARow: TStringDynArray; const AMeasure: string);
begin
  FColumn := AColumn;
  FRow := ARow;
  FMeasure := AMeasure;
...
constructor TcxPivotGridOLAPADOMDCellSetEnumerator.TPositionsEnumerator.Create
  (const APositions: Positions);
begin
  inherited Create;
...
class function TcxPivotGridOLAPADOMDCellSetEnumerator.TPositionsEnumerator.
  ExtractMeasureUniqueName(var AUniqueNames: TStringDynArray): string;
var
  ALength: Integer;
begin
...
function TcxPivotGridOLAPADOMDCellSetEnumerator.TPositionsEnumerator.
  Count: Integer;
begin
...
function TcxPivotGridOLAPADOMDCellSetEnumerator.TPositionsEnumerator.
  GetCurrentMemberNames: TStringDynArray;
var
  AMembers: Members;
...
constructor TcxPivotGridOLAPADOMDCellSetEnumerator.Create(const ADataSet
  : TcxPivotGridOLAPADOMDDataSet);
begin
  inherited Create;
  FDataSet := ADataSet;
...
function TcxPivotGridOLAPADOMDCellSetEnumerator.GetCoordinate
  : TcxPivotGridOLAPCellCoordinate;
var
  AMeasure: string;
...
function TcxPivotGridOLAPADOMDCellSetEnumerator.DoFirst
  (AEnumerator: TPositionsEnumerator): Boolean;
begin
...
function TcxPivotGridOLAPADOMDCellSetEnumerator.DoMoveNext
  (AEnumerator: TPositionsEnumerator): Boolean;
begin
  Result := AEnumerator <> nil;
  if Result then
...
constructor TcxPivotGridOLAPADOMDProvider.Create(ADataSource
...
function TcxPivotGridOLAPADOMDProvider.CreateQueryBuilder
  : TcxCustomOLAPDataQueryBuilder;
begin
  Result := TcxADOMDOLAPDataQueryBuilder.Create;
end;

function TcxPivotGridOLAPADOMDProvider.CreateCrossCellsReader
...
function TcxPivotGridOLAPADOMDProvider.CreateGrandTotalsReader
  (ADataSet: TcxCustomPivotGridOLAPDataSet;
  AColumnGroups, ARowGroups: TcxPivotGridGroupItem;
  ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList)
  : TcxCustomPivotGridOLAPDataSetReader;
...
begin
  ...
  AReader.Initialize(AColumnGroups, ARowGroups, ASummaryFields,
    AnExpandedMembers);
  Result := AReader;
end;

function TcxPivotGridOLAPADOMDProvider.CreateMainGrandTotalReader
  (ADataSet: TcxCustomPivotGridOLAPDataSet;
  AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
...
function TcxPivotGridOLAPADOMDProvider.CreateMembersReader
  (ADataSet: TcxCustomPivotGridOLAPDataSet;
  AGroupValues: TcxPivotGridOLAPGroupInfos; AField: TcxPivotGridField)
  : TcxCustomPivotGridOLAPDataSetReader;
var
...
type
  TcxPivotGridOLAPArrayDrillDownDataSourceAccess = class
    (TcxPivotGridOLAPArrayDrillDownDataSource);

function TcxPivotGridOLAPADOMDProvider.CreateDrillDownDataSet(const ACommandText
  : string; ACrossCell: TcxPivotGridCrossCell; ASummaryIndex: Integer): TObject;
var
  I, ARowIndex: Integer;
  ARow: Variant;
  ARecordset: TdxADORecordset;
  AResult: TcxPivotGridOLAPArrayDrillDownDataSourceAccess.TDataSet
    absolute Result;
begin
  Result := nil;
  ...
  if ARecordset = nil then
    Exit;
  try
    AResult := TcxPivotGridOLAPArrayDrillDownDataSourceAccess.TDataSet.Create
      (ARecordset.RecordCount);
...
begin
  ...
  begin
    ...
    if CreateTemporaryCube(AOriginalCubeFilePath) then
      AConnectionString := StringReplace(ConnectionString,
        AOriginalCubeFilePath, CachedFileName, [rfReplaceAll]);
  end;
...
function TcxPivotGridOLAPADOMDProvider.Execute(const ACommandText: string)
...
function TcxPivotGridOLAPADOMDProvider.Execute(const AQuery: TcxMDXQuery)
  : TcxCustomPivotGridOLAPDataSet;
begin
...
function TcxPivotGridOLAPADOMDProvider.GetDrillDownDataSourceClass
  : TcxCustomPivotGridOLAPDrillDownDataSourceClass;
begin
...
procedure TcxPivotGridOLAPADOMDProvider.PopulateHierarchyAllMemberNames
  (const ACubeName: string; ANames: TDictionary<string, string>);
  ...
  begin
    ...
    begin
      S := GetPropertyStrValue(ADimension.Hierarchies[I].Properties,
        'ALL_MEMBER');
      if S <> '' then
      begin
...
function TcxPivotGridOLAPADOMDProvider.AddFieldToDataSet(ADataSet: TDataSet;
  const AFieldName: string; AFieldType: TFieldType): TField;
begin
  ADataSet.FieldDefs.Add(AFieldName, AFieldType);
  Result := ADataSet.FieldDefs.Find(AFieldName).CreateField(ADataSet);
end;

function TcxPivotGridOLAPADOMDProvider.GetCubeDefByName(const ACubeName
  : string): CubeDef;
var
  I: Integer;
  ACubeDef: CubeDef;
...
function TcxPivotGridOLAPADOMDProvider.GetPropertyValue(const AProperties
  : Properties; const APropertyName: string;
...
function TcxPivotGridOLAPADOMDProvider.GetPropertyIntValue(const AProperties
  : Properties; const APropertyName: string;
...
function TcxPivotGridOLAPADOMDProvider.GetPropertyStrValue(const AProperties
  : Properties; const APropertyName: string;
...
function TcxPivotGridOLAPADOMDProvider.OpenRecordset(const ACommandText: string)
  : TdxADORecordset;
begin
...
function TcxPivotGridOLAPADOMDProvider.DoGetKPIs(const ACube: CubeDef)
  : TcxPivotGridOLAPMemDataDataSet;
...
const
  FieldNames: array [0 .. 8] of string = ('KPI_CAPTION', 'KPI_NAME',
    'KPI_VALUE', 'KPI_STATUS_GRAPHIC', 'KPI_TREND_GRAPHIC', 'KPI_GOAL',
...
begin
  ACommandText :=
    Format('SELECT * FROM $system.MDSCHEMA_KPIS WHERE CUBE_NAME=%s',
    [QuotedStr(ACube.Name)]);
  ARecordset := OpenRecordset(ACommandText);
  if ARecordset = nil then
    Exit(nil);
  try
    ...
    begin
      ...
      begin
        ...
        if IndexStr(AFieldName, FieldNames) >= 0 then
          ADataSet.FieldByName(AFieldName).AsString :=
            VarToStr(ARecordset.GetFieldValue(I));
      end;
      ADataSet.Post;
      ARecordset.MoveNext;
    end;
...
function TcxPivotGridOLAPADOMDProvider.DoGetLevels(const ACube: CubeDef)
  : TcxPivotGridOLAPMemDataDataSet;
var
  ADimensionUniqueName, AHierarchyUniqueName, AUniqueName, AName, ACardinality,
    ADBType, ANumber, AType: TField;

  procedure AddLevel(ADataSet: TDataSet; ADimension: Dimension;
    AHierarchy: Hierarchy; ALevel: Level);
  var
  ...
  begin
    ...
    ACardinality.AsInteger := GetPropertyIntValue(ALevelProperties,
      'LEVEL_CARDINALITY');
  ...
  procedure InitializeInnerDataSet(ADataSet: TDataSet);
  begin
    ADimensionUniqueName := AddFieldToDataSet(ADataSet, 'DIMENSION_UNIQUE_NAME',
      ftWideMemo);
    AHierarchyUniqueName := AddFieldToDataSet(ADataSet, 'HIERARCHY_UNIQUE_NAME',
      ftWideMemo);
...
function TcxPivotGridOLAPADOMDProvider.DoGetMeasures(const ACube: CubeDef)
  : TcxPivotGridOLAPMemDataDataSet;
var
  AGroupName, ACaption, AUniqueName, ADisplayFolder,
    ADefaultFormatString: TField;

  procedure AddMeasures(ADataSet: TDataSet; ADimension: Dimension;
    AMember: Member);
  var
  ...
  begin
    ...
    AGroupName.AsString := GetPropertyStrValue(AMemberProperties,
      'MEASUREGROUP_NAME');
    ADisplayFolder.AsString := GetPropertyStrValue(AMemberProperties,
      'MEASURE_DISPLAY_FOLDER');
    ADefaultFormatString.AsString := GetPropertyStrValue(AMemberProperties,
  ...
  begin
    ...
    AUniqueName := AddFieldToDataSet(ADataSet, 'MEASURE_UNIQUE_NAME',
      ftWideMemo);
    AGroupName := AddFieldToDataSet(ADataSet, 'MEASUREGROUP_NAME', ftWideMemo);
    ADisplayFolder := AddFieldToDataSet(ADataSet, 'MEASURE_DISPLAY_FOLDER',
      ftWideMemo);
    ADefaultFormatString := AddFieldToDataSet(ADataSet, 'DEFAULT_FORMAT_STRING',
      ftWideMemo);
    ADataSet.Open;
  end;
...
function TcxPivotGridOLAPADOMDProvider.DoGetHierarchies(const ACube: CubeDef)
  : TcxPivotGridOLAPMemDataDataSet;
var
  ADisplayFolder, ACaption, ADimensionUniqueName, AUniqueName, AStructure,
    ACardinality: TField;

  procedure AddHierarchy(ADataSet: TDataSet; ADimension: Dimension;
    AHierarchy: Hierarchy);
  var
  ...
  begin
    ...
    ADisplayFolder.AsString := GetPropertyStrValue(AHierarchyProperties,
      'HIERARCHY_DISPLAY_FOLDER');
    ACaption.AsString := AHierarchy.Name;
    ADimensionUniqueName.AsString := ADimension.UniqueName;
    AUniqueName.AsString := AHierarchy.UniqueName;
    AStructure.AsInteger := GetPropertyIntValue(AHierarchyProperties,
      'STRUCTURE');
    ACardinality.AsInteger := GetPropertyIntValue(AHierarchyProperties,
  ...
  procedure InitializeInnerDataSet(ADataSet: TDataSet);
  begin
    ADisplayFolder := AddFieldToDataSet(ADataSet, 'HIERARCHY_DISPLAY_FOLDER',
      ftWideMemo);
    ACaption := AddFieldToDataSet(ADataSet, 'HIERARCHY_CAPTION', ftWideMemo);
    ADimensionUniqueName := AddFieldToDataSet(ADataSet, 'DIMENSION_UNIQUE_NAME',
      ftWideMemo);
    AUniqueName := AddFieldToDataSet(ADataSet, 'HIERARCHY_UNIQUE_NAME',
      ftWideMemo);
    AStructure := AddFieldToDataSet(ADataSet, 'STRUCTURE', ftInteger);
    ACardinality := AddFieldToDataSet(ADataSet, 'HIERARCHY_CARDINALITY',
      ftInteger);
    ADataSet.Open;
  end;
...
function TcxPivotGridOLAPADOMDProvider.DoGetSets(const ACube: CubeDef)
  : TcxPivotGridOLAPMemDataDataSet;
...
constructor TcxCustomPivotGridOLAPADOMDDataSetReader.Create
  (ADataSet: TcxCustomPivotGridOLAPDataSet);
begin
...
class function TcxCustomPivotGridOLAPADOMDDataSetReader.NativeValueToValue
  (const ANativeValue: Variant): Variant;
begin
...
function TcxCustomPivotGridOLAPADOMDDataSetReader.GetDataSet
  : TcxPivotGridOLAPADOMDDataSet;
begin
...
constructor TcxPivotGridOLAPADOMDDataSetCrossCellsReader.Create
  (ADataSet: TcxCustomPivotGridOLAPDataSet);
begin
  inherited Create(ADataSet);
...
procedure TcxPivotGridOLAPADOMDDataSetCrossCellsReader.Execute;
  ...
  procedure SkipEmptyRowMembers(var AIndex: Integer;
    const ACurrentRowMembers: TStringDynArray;
  ...
  procedure SkipEmptyColumnMembers(var AIndex: Integer;
    const ACurrentColumnMembers: TStringDynArray;
  ...
  begin
    ...
    begin
      ACoordinate := AColumnMembers.GetMemberCoordinate
        (AIndex mod AColumnMembers.MemberCount);
  ...
  procedure SkipEmptyMeasures(var AIndex: Integer;
    const ACurrentMeasure: string; AMembers: TcxMDXQueryAxisMembers);
  var
    I: Integer;
  begin
    for I := 0 to AMembers.MeasureCount - 1 do
    begin
      if AMembers.Measures[AIndex mod AMembers.MeasureCount].UniqueName = ACurrentMeasure
      then
        Break;
      Inc(AIndex);
    end;
  ...
  try
    ...
    begin
      ...
      repeat
        ...
        SetCrossCellValue(ACell, AFieldIndex, ANativeValue,
          NativeValueToValue(ANativeValue));
...
procedure TcxPivotGridOLAPADOMDDataSetCrossCellsReader.Initialize(AOwnedGroup,
...
var
  ...
  begin
    ...
    begin
      AMembersUniqueName := AEnumerator.ColumnEnumerator.Current.Members[0]
        .UniqueName;
...
procedure TcxPivotGridOLAPADOMDDataSetMembersReader.Initialize
  (AGroupValues: TcxPivotGridOLAPGroupInfos; AField: TcxPivotGridField);
begin
...
var
  ...
  AEnumerator: TcxPivotGridOLAPADOMDCellSetEnumerator;
begin
  ACellMatrix := TcxPivotGridOLAPCellMatrix.Create(RowGroups, ColumnGroups,
    SummaryFields);
  try
    AEnumerator := TcxPivotGridOLAPADOMDCellSetEnumerator.Create(DataSet);
    try
...
procedure TcxPivotGridOLAPADOMDDataSetGrandTotalReader.Initialize(AColumnGroups,
...
begin
  ...
  try
    ...
    if AIsValuesInitialized then
      SetCrossCellValues(RowGroup.GetCellByCrossItem(ColumnGroup), AValues,
        ANativeValues)
    else
...
procedure TcxPivotGridOLAPADOMDDataSetMainGrandTotalReader.Initialize
  (AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
  ADataFields: TcxPivotGridFields);
begin
...
constructor TcxPivotGridOLAPCellMatrix.Create(ARowGroups,
...
function TcxPivotGridOLAPCellMatrix.TryGetCellValueInfo
  (const AKey: TcxPivotGridOLAPCellCoordinate;
  out AValue: TcxPivotGridOLAPCellMatrix.TValue): Boolean;
  ...
  begin
    ...
    else if Length(AKey.Row) > 0 then
...
function TcxPivotGridOLAPCellMatrix.FindGroupByUniqueNames
  (AParentGroup: TcxPivotGridGroupItem; const ANames: TStringDynArray)
...
function TcxPivotGridOLAPCellMatrix.GetGroupByUniqueName
  (AGroups: TcxPivotGridGroupItem; const AUniqueName: string)
  : TcxPivotGridGroupItem;
...
begin
  ...
  begin
    ...
    if (AField <> nil) and SameStr(GetTotalUniqueNameForQuery(AField),
      AUniqueName) then
      Result := AGroups;
  end;
  if (Result = nil) and (AGroups.ItemCount > 0) and
    (AGroups.Items[0].ItemCount > 0) then
  begin
    AField1 := AGroups.Items[0].Field;
    AField2 := AGroups.Items[0].Items[0].Field;
    if (AField1 <> nil) and (AField2 <> nil) and
      TcxCustomOLAPDataQueryBuilder.SameHierarchy(AField1, AField2) then
      for I := 0 to AGroups.ItemCount - 1 do
      begin
...
function TcxPivotGridOLAPCellMatrix.GetSummaryFieldIndex(const AUniqueName
  : string): Integer;
var
  I: Integer;
begin
...
constructor TcxPivotGridOLAPCellMatrix.TValue.Create
  (ACell: TcxPivotGridCrossCell; AFieldIndex: Integer);
begin
  FCell := ACell;
