...
type
  ...
  protected
    ...
    property DataController: TcxDBPivotGridDataController
  ...
  published
    ...
    property AreaIndex;
    property Caption;
    property DataBinding: TcxDBPivotGridFieldDataBinding read GetDataBinding
...
function TcxDBPivotGridDataController.GetFilterItemFieldCaption
...
function TcxDBPivotGridDataController.GetItemValueSource(AItemIndex: Integer)
...
procedure TcxDBPivotGridDataController.DoValueTypeClassChanged
  (AItemIndex: Integer);
var
  AItem: TObject;
begin
...
procedure TcxDBPivotGridDataController.UpdateControl
  (AInfo: TcxUpdateControlInfo);
begin
  if (AInfo is TcxFocusedRecordChangedInfo) or
    (AInfo is TcxFocusedRowChangedInfo) then
    Exit;
...
function TcxCustomDBPivotGrid.CanRetrieveFields(var AIsActive: Boolean)
  : Boolean;
begin
  Result := True;
...
function TcxDBPivotGridFieldDataBinding.GetDefaultValuesProviderClass
...
begin
  inherited;
  TcxCustomDBEditDefaultValuesProvider(FDefaultValuesProvider).DataSource :=
...
begin
  ...
  else if DBField is TBCDField then
    Result := TBCDField(DBField).Currency
  else if DBField is TAggregateField then
    Result := TAggregateField(Field).Currency
  else if DBField is TFMTBCDField then
    Result := TFMTBCDField(DBField).Currency
  else
    Result := inherited IsCurrencyField;
end;

function TcxDBPivotGridFieldDataBinding.GetDataController
  : TcxDBPivotGridDataController;
begin
...
procedure TcxDBPivotGridField.SetPivotGrid(AValue: TcxCustomDBPivotGrid);
begin
