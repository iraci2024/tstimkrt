...
uses
  ...
  cxGraphics, cxControls, cxClasses, cxGeometry, cxCustomPivotGrid,
  cxPivotGridOLAPDataSource, cxButtons,
...
function TcxPivotGridOLAPDataSourceProviderProperty.GetAttributes
  : TPropertyAttributes;
begin
...
procedure TcxPivotGridOLAPDataSourceProviderProperty.GetValues
  (Proc: TGetStrProc);
var
  I: Integer;
  ADesc: string;
...
procedure TcxPivotGridOLAPDataSourceProviderProperty.SetValue
  (const Value: string);
var
  I: Integer;
...
begin
  ...
  if AProviderClass = nil then
    AProviderClass := TcxCustomPivotGridOLAPProviderClass
      (cxGetPivotGridOLAPDataSourceProviders.FindByDescription(Value));
  for I := 0 to PropCount - 1 do
...
begin
  ...
  RegisterComponentEditor(TcxPivotGridOLAPDataSource,
    cxPivotGridCustomComponentEditor);
  RegisterPropertyEditor(TypeInfo(TcxCustomPivotGridOLAPProvider),
    TcxPivotGridOLAPDataSource, 'Provider',
    TcxPivotGridOLAPDataSourceProviderProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxPivotGridOLAPOLEDBProvider,
    'ConnectionString', TcxConnectionStringProperty);
  RegisterPropertyEditor(TypeInfo(string), TcxPivotGridOLAPADOMDProvider,
    'ConnectionString', TcxConnectionStringProperty);
  HideClassProperties(TcxPivotGridOLAPDataSource,
    ['ConnectionString', 'ProviderClassName']);
end;
