...
uses
  Types, Classes, SysUtils, dxCore, cxClasses, cxCustomPivotGrid,
  Generics.Defaults, Generics.Collections, Contnrs;
...
type
  ...
  public
    function GetDrillDownQueryString(const ACubeName: string;
      ...
      virtual; abstract;
    function GetKPIQueryString(const ACubeName: string;
      AColumns: TcxPivotGridFields; ARows: TcxPivotGridFields;
      AFilters: TcxPivotGridFields; AKPIType: TcxOLAPKPIType;
      const AKPIName: string): string; virtual; abstract;
    function GetKPITotalQueryString(const ACubeName: string;
      AColumns, ARows: TcxPivotGridFields; AFilters: TcxPivotGridFields;
      AKPIType: TcxOLAPKPIType; const AKPIName: string): string;
      virtual; abstract;
    function GetKPITotalsQueryString(const ACubeName: string;
      AColumns, AFilters: TcxPivotGridFields; AnExpandedMembers: TList;
      AKPIType: TcxOLAPKPIType; const AKPIName: string): string;
      virtual; abstract;
    class function GetMeasureCount(AMeasures: TcxPivotGridFields;
    ...
    function GetParentQueryString(const ACubeName: string;
      AField: TcxPivotGridOLAPField; const AMemberUniqueName: string): string;
      virtual; abstract;
    function GetCrossCellsQuery(const ACubeName: string;
      AOwnedGroup: TcxPivotGridGroupItem; AColumns, ARows, AMeasures,
      AFilters: TcxPivotGridFields): TcxMDXQuery; virtual; abstract;
    function GetTotalQueryString(const ACubeName: string;
      AFilters: TcxPivotGridFields; AMeasures: TcxPivotGridFields): string;
      virtual; abstract;
    function GetTotalsQueryString(const ACubeName: string;
      AColumns: TcxPivotGridFields; AnExpandedMembers: TList;
      AMeasures: TcxPivotGridFields; AFilters: TcxPivotGridFields): string;
      virtual; abstract;
    class function SameHierarchy(AField1, AField2: TcxPivotGridField): Boolean;
    // internal use only
  end;
  ...
  strict protected
    ...
    procedure DoGetCrossCellsQuery(const ACubeName: string;
      AOwnedGroup: TcxPivotGridGroupItem; AColumns, ARows, AMeasures,
      AFilters: TcxPivotGridFields; AColumnMembers, AMeasureMembers,
      ARowMembers: TcxMDXQueryAxisMembers);
    procedure ExcludeExpandingMembersFormFilters(AnExpandedMembers: TList;
      AFilters: TcxPivotGridFields);
    procedure ExcludeField(AFilters: TcxPivotGridFields;
      AField: TcxPivotGridOLAPField);
    function GetAxis: string;
    function GetExpandedMembers(AnExpandedMembers: TList;
      AClearMember: Boolean = False): string;
    function GetFilterString(AFilters: TcxPivotGridFields;
      AFilter: TcxPivotGridOLAPField): string;
    function GetMeasures(AMeasures: TcxPivotGridFields;
    ...
    function IsFiltered(AFilters: TcxPivotGridFields;
      AField: TcxPivotGridOLAPField): Boolean;
    function IsOLAPVersion8: Boolean;
    function MakeFilterFields(AFilters: TcxPivotGridFields): TcxPivotGridFields;
    function WriteAllMembersWithSorting(AFilters: TcxPivotGridFields;
      AField: TcxPivotGridOLAPField; IncludeCalculatedMembers: Boolean): string;
    function WriteAllMembersCore(AFilters: TcxPivotGridFields;
      AField: TcxPivotGridOLAPField; IncludeCalculatedMembers: Boolean;
      AFilter: Boolean): string;
    function WriteSortedMembers(AField: TcxPivotGridOLAPField;
      const AContent: string): string;

    // query parts
    function ColumnPartVersion8(const ACubeName: string;
      AColumns: TcxPivotGridFields; AMeasures, AFilters: TcxPivotGridFields;
      IncludeCalculatedMembers, IncludeTotals: Boolean;
      const AAdditionalString: string = ''): string;
    function ColumnPart(const ACubeName: string;
      AColumns, AMeasures, AFilters: TcxPivotGridFields;
      IncludeCalculatedMembers, IncludeTotals, IsFirstMember: Boolean;
      const AAdditionalString: string): string;
    function SourcePart(const ACubeName: string; AFilters: TcxPivotGridFields;
      ANamedSets: TList): string;
    function SourcePartWithSubSelect(const ACubeName: string;
  ...
  public
    ...
    function GetDrillDownQueryString(const ACubeName: string;
      ACrossCell: TcxPivotGridCrossCell; ASummaryIndex: Integer)
      : string; override;
    function GetKPIQueryString(const ACubeName: string;
      AColumns: TcxPivotGridFields; ARows: TcxPivotGridFields;
      AFilters: TcxPivotGridFields; AKPIType: TcxOLAPKPIType;
      const AKPIName: string): string; override;
    function GetKPITotalQueryString(const ACubeName: string;
      AColumns, ARows: TcxPivotGridFields; AFilters: TcxPivotGridFields;
      AKPIType: TcxOLAPKPIType; const AKPIName: string): string; override;
    function GetKPITotalsQueryString(const ACubeName: string;
      AColumns, AFilters: TcxPivotGridFields; AnExpandedMembers: TList;
      AKPIType: TcxOLAPKPIType; const AKPIName: string): string; override;
    function GetMembersQueryString(const ACubeName: string;
    ...
    function GetParentQueryString(const ACubeName: string;
      AField: TcxPivotGridOLAPField; const AMemberUniqueName: string)
      : string; override;
    function GetTotalQueryString(const ACubeName: string;
      AFilters: TcxPivotGridFields; AMeasures: TcxPivotGridFields)
      : string; override;
    function GetTotalsQueryString(const ACubeName: string;
      AColumns: TcxPivotGridFields; AnExpandedMembers: TList;
      AMeasures: TcxPivotGridFields; AFilters: TcxPivotGridFields)
      : string; override;
  ...
  public
    function GetCrossCellsQuery(const ACubeName: string;
      AOwnedGroup: TcxPivotGridGroupItem; AColumns, ARows, AMeasures,
      AFilters: TcxPivotGridFields): TcxMDXQuery; override;
  end;
...
{$REGION 'internal types'}
  ...
  public
    constructor Create;
    destructor Destroy; override;
    function TryGetMembers(AAxis: Integer;
      out AMembers: TcxMDXQueryAxisMembers): Boolean;

    procedure AddAxisClause(const AClause: string;
      AMembers: TcxMDXQueryAxisMembers = nil);
...
const
  ...
  KPITypeMap: array [TcxOLAPKPIType] of string = ('KPIValue', 'KPIGoal',
    'KPIStatus', 'KPITrend', 'KPIWeight');
...
function Implode(const AGlue: string; const AStrings: TStringDynArray)
  : string; overload;
var
  I: Integer;
begin
...
class function TcxCustomOLAPDataQueryBuilder.GetMeasureCount
  (AMeasures: TcxPivotGridFields;
  AFieldType: TcxPivotGridOLAPFieldType): Integer;
var
  I: Integer;
begin
...
class function TcxCustomOLAPDataQueryBuilder.SameHierarchy(AField1,
  AField2: TcxPivotGridField): Boolean;
begin
  Result := AField2.IsHierarchy and AField1.IsHierarchy and
    (AField1.Group = AField2.Group);
end;
...
function TcxOLAPDataQueryBuilder.GetDrillDownQueryString(const ACubeName
  : string; ACrossCell: TcxPivotGridCrossCell; ASummaryIndex: Integer): string;
var
...
begin
  ...
  if ACrossCell = nil then
    Exit;
  AMeasure := ACrossCell.SummaryCells[ASummaryIndex]
    .DataField as TcxPivotGridOLAPField;
...
function TcxOLAPDataQueryBuilder.GetKPIQueryString(const ACubeName: string;
  AColumns: TcxPivotGridFields; ARows: TcxPivotGridFields;
  AFilters: TcxPivotGridFields; AKPIType: TcxOLAPKPIType;
  const AKPIName: string): string;
var
...
begin
  ...
  Result := ColumnPart(ACubeName, AColumns, nil, AFilters, True, False,
    True, '');
  if ARows.Count > 0 then
    Result := Result + ', non empty ' + WriteAllMembersWithSorting(AFilters,
      TcxPivotGridOLAPField(ARows[0]), True) +
      'dimension properties member_unique_name' + GetAxis;
  Result := Result + ' from ' + SourcePart(ACubeName, nil, nil) + ' where(' +
    AKPICondition + ')';
end;

function TcxOLAPDataQueryBuilder.GetKPITotalQueryString(const ACubeName: string;
  AColumns, ARows: TcxPivotGridFields; AFilters: TcxPivotGridFields;
  AKPIType: TcxOLAPKPIType; const AKPIName: string): string;

  function GetDimension(AField: TcxPivotGridOLAPField;
    ...
    begin
      ...
      begin
        Result := Result + 'non empty {' +
          GetDimension(TcxPivotGridOLAPField(AColumns[0]), AFilterFields) + '}'
          + GetAxis;
        AColumnExists := True;
      end;
      if ARows.Count > 0 then
      begin
        if AColumnExists then
          Result := Result + ', ';
        Result := Result + 'non empty {' +
          GetDimension(TcxPivotGridOLAPField(ARows[0]), AFilterFields) +
          '} dimension properties member_unique_name' + GetAxis;
      end;
    end;
    Result := Result + ' from [' + ACubeName + ']';
...
function TcxOLAPDataQueryBuilder.GetKPITotalsQueryString(const ACubeName
  : string; AColumns, AFilters: TcxPivotGridFields; AnExpandedMembers: TList;
  AKPIType: TcxOLAPKPIType; const AKPIName: string): string;
var
  AKPICondition: string;
begin
  ...
  if AKPIName = '' then
    Exit;
  AKPICondition := KPITypeMap[AKPIType] + '("' + AKPIName + '")';
  if (AnExpandedMembers <> nil) and IsNotLastVisibleOLAPHierarchyItem
    (TcxPivotGridGroupItem(AnExpandedMembers[0])) then
...
function TcxOLAPDataQueryBuilder.GetMembersQueryString(const ACubeName: string;
  AField: TcxPivotGridOLAPField; AnExpandedMembers: TList;
  AMeasures, AFilters: TcxPivotGridFields): string;

  function IsFieldInExpandingHierarchy(AField: TcxPivotGridOLAPField;
    AnExpandedMembers: TList): Boolean;
  var
    I: Integer;
    AExpandedField: TcxPivotGridOLAPField;
  begin
    Result := False;
    if (AField <> nil) and (AnExpandedMembers <> nil) and AField.IsHierarchy
    then
      for I := 0 to AnExpandedMembers.Count - 1 do
      begin
        AExpandedField := TcxPivotGridOLAPField
          (TcxPivotGridGroupItem(AnExpandedMembers[I]).Field);
        if AExpandedField.IsHierarchy and (AExpandedField.Group = AField.Group)
        then
        begin
          Result := True;
          Break;
    ...
    case Version of
      7, 8:
        begin
          ...
          Result := Result + ' from ' + SourcePartWithSubSelect(ACubeName,
            AFilters, nil);
        end
    else
      begin
        ...
        try
          ...
          begin
            ...
            ExcludeExpandingMembersFormFilters(AnExpandedMembers,
              AFilterFields);
          end;

          Result := 'with member Measures.MemberName as ' +
            AField.HierarchyUniqueName + '.MEMBER_NAME ' +
            'member Measures.MemberUniqueName as ' + AField.HierarchyUniqueName
            + '.MEMBER_UNIQUE_NAME ';
          AColumnQuery := GetMeasures(AMeasures,
            'Measures.MemberName, Measures.MemberUniqueName');

          AMembers := '';
          if not IsFieldInExpandingHierarchy(AField, AnExpandedMembers) then
          begin
            ASortedMembers := WriteSortedMembers(AField,
              WriteAllMembersCore(AFilters, AField, True,
              AFilterFields <> nil));
            if AField.FieldType = oftSet then
              AMembers := Format('{%s - {%s.[All]}}',
                [ASortedMembers, AField.LevelUniqueName])
            else
          ...
          if AnExpandedMembers <> nil then
            if AMembers = '' then
              ...
              AMembers := Format('%s * %s',
                [AMembers, GetExpandedMembers(AnExpandedMembers)]);
          ClearAxisIndex;
          Result := Result + 'select non empty ' + AColumnQuery +
            GetAxis + ', ';
          ...
          try
            ...
            Result := Result + ' from ' + SourcePart(ACubeName, AFilterFields,
              ANamedSets);
          finally
            ANamedSets.Free;
          end;
...
function TcxOLAPDataQueryBuilder.GetParentQueryString(const ACubeName: string;
  AField: TcxPivotGridOLAPField; const AMemberUniqueName: string): string;
begin
  ClearAxisIndex;
  ...
  if AField <> nil then
  begin
    Result := 'with member Measures.ParentUniqueName as ' +
      AField.HierarchyUniqueName + '.PARENT_UNIQUE_NAME ';
    Result := Result + 'select non empty {Measures.ParentUniqueName}' +
      GetAxis + ', ';
...
begin
  ...
  if AMeasures.Count > 0 then
    Result := 'select {' + GetMeasures(AMeasures) + '}' + GetAxis + ' from ' +
      SourcePartWithSubSelect(ACubeName, AFilters, nil);
end;
...
begin
  ...
  if (AnExpandedMembers <> nil) and IsNotLastVisibleOLAPHierarchyItem
    (TcxPivotGridGroupItem(AnExpandedMembers[0])) then
    Result := ColumnPart(ACubeName, AColumns, AMeasures, AFilters, True, True,
      True, GetExpandedMembers(AnExpandedMembers))
  else
  begin
    if IsOLAPVersion8 then
      Result := ColumnPartVersion8(ACubeName, AColumns, AMeasures, AFilters,
        True, True)
    else
      Result := ColumnPart(ACubeName, AColumns, AMeasures, AFilters, True, True,
        True, '');
...
function TcxOLAPDataQueryBuilder.CreateFilterWhereClause
  (AFilters: TcxPivotGridFields): string;
var
  I: Integer;
begin
  ...
  for I := AFilters.Count - 1 downto 0 do
    if ((I = AFilters.Count - 1) or
      not IsHierarchy(TcxPivotGridOLAPField(AFilters[I]),
      TcxPivotGridOLAPField(AFilters[I + 1]))) then
      Result := Result + GetFilterString(AFilters,
        TcxPivotGridOLAPField(AFilters[I])) + Comma;
  if Result <> '' then
    TrimTrailingComma(Result);
end;

procedure TcxOLAPDataQueryBuilder.DoGetCrossCellsQuery(const ACubeName: string;
  AOwnedGroup: TcxPivotGridGroupItem; AColumns, ARows, AMeasures,
  AFilters: TcxPivotGridFields; AColumnMembers, AMeasureMembers,
  ARowMembers: TcxMDXQueryAxisMembers);

  function PopulateMeasures(AMembers: TcxMDXQueryAxisMembers;
    AMeasures: TcxPivotGridFields): string;
  var
    I: Integer;
  begin
    for I := 0 to AMeasures.Count - 1 do
  ...
  function GetParentInfo(AGroup: TcxPivotGridGroupItem)
    : TArray<TcxPivotGridGroupItem>;
  var
    ALength: Integer;
  begin
    ALength := 0;
  ...
  procedure AddParentInfo(const AParentInfo: TArray<TcxPivotGridGroupItem>;
  ...
  procedure PopulateColumnMembers(AMembers: TcxMDXQueryAxisMembers;
    AColumnAxisGroups: TcxPivotGridGroupItem;
    AColumnAxisFields: TcxPivotGridFields);
  var
    I: Integer;
    AMember: TcxMDXQueryAxisMembers.TMember;
  ...
  procedure DoPopulateRowMembers(AMembers: TcxMDXQueryAxisMembers;
    AParentMember: TcxMDXQueryAxisMembers.TMember;
...
begin
  ARowAxisGroups := TcxCustomPivotGridAccess(AOwnedGroup.PivotGrid)
    .DataBuilder.Rows;
  AColumnAxisGroups := TcxCustomPivotGridAccess(AOwnedGroup.PivotGrid)
    .DataBuilder.Columns;
...
procedure TcxOLAPDataQueryBuilder.ExcludeExpandingMembersFormFilters
  (AnExpandedMembers: TList; AFilters: TcxPivotGridFields);
var
  I: Integer;
begin
  if AnExpandedMembers <> nil then
    for I := 0 to AnExpandedMembers.Count - 1 do
      ExcludeField(AFilters,
        TcxPivotGridOLAPField(TcxPivotGridGroupItem(AnExpandedMembers
        [I]).Field));
end;

procedure TcxOLAPDataQueryBuilder.ExcludeField(AFilters: TcxPivotGridFields;
  AField: TcxPivotGridOLAPField);
var
  I: Integer;
begin
  if (AFilters <> nil) and (AField <> nil) then
    for I := AFilters.Count - 1 downto 0 do
      if TcxPivotGridOLAPField(AFilters[I]).HierarchyUniqueName = AField.HierarchyUniqueName
      then
        AFilters.Delete(I);
end;
...
function TcxOLAPDataQueryBuilder.GetExpandedMembers(AnExpandedMembers: TList;
  AClearMember: Boolean = False): string;
  ...
  begin
    ...
    if IsNotLastVisibleOLAPHierarchyItem
      (TcxPivotGridGroupItem(AnExpandedMembers[Index])) and not AClearMember
    then
      Result := WriteSortedMembers
        (TcxPivotGridOLAPField(TcxPivotGridGroupItem(AnExpandedMembers[Index])
        .Field), 'addcalculatedmembers(' + AnItemName + '.children)')
    else
      Result := AnItemName;
  end;
...
function TcxOLAPDataQueryBuilder.GetFilterString(AFilters: TcxPivotGridFields;
  AFilter: TcxPivotGridOLAPField): string;
begin
  Result := Format('{%s}', [WriteAllMembersCore(AFilters, AFilter,
    False, True)]);
end;
...
function TcxOLAPDataQueryBuilder.GetMeasures(AMeasures: TcxPivotGridFields;
  const AAdditionalString: string = ''): string;
var
  I: Integer;
begin
  ...
  if AMeasures <> nil then
    for I := 0 to AMeasures.Count - 1 do
      if Result = '' then
        ...
        Result := Format('%s, %s', [Result, TcxPivotGridOLAPField(AMeasures[I])
          .UniqueName]);
  if Result <> '' then
    Result := Format('{ %s }', [Result]);
end;

function TcxOLAPDataQueryBuilder.GetNamedSetName
  (AField: TcxPivotGridOLAPField): string;
begin
...
function TcxOLAPDataQueryBuilder.GetSortOrder
  (AField: TcxPivotGridOLAPField): string;
begin
...
function TcxOLAPDataQueryBuilder.GetTuple
...
function TcxOLAPDataQueryBuilder.IsFiltered(AFilters: TcxPivotGridFields;
  AField: TcxPivotGridOLAPField): Boolean;
var
  I: Integer;
begin
...
function TcxOLAPDataQueryBuilder.MakeFilterFields(AFilters: TcxPivotGridFields)
  : TcxPivotGridFields;
begin
  if AFilters = nil then
    Result := nil
...
function TcxOLAPDataQueryBuilder.WriteAllMembersWithSorting
  (AFilters: TcxPivotGridFields; AField: TcxPivotGridOLAPField;
  IncludeCalculatedMembers: Boolean): string;
begin
  Result := '{ ' + WriteSortedMembers(AField, WriteAllMembersCore(AFilters,
    AField, IncludeCalculatedMembers, True)) + '} ';
end;

function TcxOLAPDataQueryBuilder.WriteAllMembersCore
  (AFilters: TcxPivotGridFields; AField: TcxPivotGridOLAPField;
...
function TcxOLAPDataQueryBuilder.WriteSortedMembers
  (AField: TcxPivotGridOLAPField; const AContent: string): string;
begin
...
function TcxOLAPDataQueryBuilder.ColumnPartVersion8(const ACubeName: string;
  AColumns: TcxPivotGridFields; AMeasures, AFilters: TcxPivotGridFields;
  IncludeCalculatedMembers, IncludeTotals: Boolean;
  const AAdditionalString: string = ''): string;
begin
  Result := '';
  if ((AMeasures <> nil) and (AMeasures.Count = 0) or (AMeasures = nil) and
    (AColumns.Count = 0)) and (AAdditionalString = '') then
    Exit;

  Result := Result + 'select non empty { ' + AAdditionalString;
  if AColumns.Count > 0 then
    Result := Result + WriteAllMembersWithSorting(AFilters,
      TcxPivotGridOLAPField(AColumns[0]), IncludeCalculatedMembers) +
      CrossJoinOperator;
  if AMeasures = nil then
    TrimTrailingComma(Result)
  else
...
function TcxOLAPDataQueryBuilder.ColumnPart(const ACubeName: string;
  AColumns, AMeasures, AFilters: TcxPivotGridFields;
  IncludeCalculatedMembers, IncludeTotals, IsFirstMember: Boolean;
  const AAdditionalString: string): string;
  ...
  begin
    Result := ' member Measures.MemberUniqueName as ' +
      AField.HierarchyUniqueName + '.MEMBER_UNIQUE_NAME ';
  end;
...
begin
  ...
  begin
    if AColumns.Count > 0 then
      ...
      AMember := '';
    Result := Result + ' non empty' + GetMeasures(AMeasures, AMember) +
      GetAxis + ', ';
  end;
...
function TcxOLAPDataQueryBuilder.SourcePartWithSubSelect(const ACubeName
  : string; AFilters: TcxPivotGridFields; ANamedSets: TList): string;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        ASubSelect := ASubSelect + '{' + GetMembers(AList) + '} on ' +
          IntToStr(AxisIndex) + Comma;
        Inc(AxisIndex);
      finally
        AList.Free;
      end;
...
procedure TcxOLAPDataQueryBuilder.TrimTrailingSequence(var AStr: string;
...
function TcxOLAPDataQueryBuilder.GetSortBy
  (AField: TcxPivotGridOLAPField): string;
begin
  Result := AField.HierarchyUniqueName;
  case Version of
    ...
    case AField.SortMode of
      osmValue:
        Result := Result + 'member_value';
      osmDisplayText:
        Result := Result + 'member_caption';
      osmKey:
        Result := Result + 'member_key';
    else
...
function TcxOLEDBOLAPDataQueryBuilder.GetCrossCellsQuery(const ACubeName
  : string; AOwnedGroup: TcxPivotGridGroupItem;
  AColumns, ARows, AMeasures, AFilters: TcxPivotGridFields): TcxMDXQuery;

  procedure PopulateWithClauses(AWithClauses: TStrings;
  ...
  begin
    ...
    begin
      ...
      if (I > 0) and SameHierarchy(AField, AColumnMembers.Fields[I - 1]) then
        Continue;
      AUniqueName := '[Measures].[DX_COLUMN_' + IntToStr(AIndex) +
        '_UNIQUE_NAME]';
      S := 'SetToStr(Ascendants(' + AField.HierarchyUniqueName +
        '.currentmember))';
    ...
    begin
      ...
      if (I > 0) and SameHierarchy(AField, ARowMembers.Fields[I - 1]) then
        Continue;
      AUniqueName := '[Measures].[DX_ROW_' + IntToStr(AIndex - AStartIndex) +
        '_UNIQUE_NAME]';
      S := 'SetToStr(Ascendants(' + AField.HierarchyUniqueName +
        '.currentmember))';
      AWithClauses.Add('MEMBER ' + AUniqueName + ' AS ' + QuotedStr(S));
...
begin
  ...
  DoGetCrossCellsQuery(ACubeName, AOwnedGroup, AColumns, ARows, AMeasures,
    AFilters, AColumnMembers, AMeasureMembers, ARowMembers);
  ...
  AColumnAxis :=
    Format('non empty{ %s } dimension properties MEMBER_UNIQUE_NAME',
    [AColumnMembers.ToQuery]);
...
function TcxADOMDOLAPDataQueryBuilder.GetCrossCellsQuery(const ACubeName
  : string; AOwnedGroup: TcxPivotGridGroupItem;
  AColumns, ARows, AMeasures, AFilters: TcxPivotGridFields): TcxMDXQuery;
var
...
begin
  ...
  ARowMembers := TcxMDXQueryAxisMembers.Create;
  DoGetCrossCellsQuery(ACubeName, AOwnedGroup, AColumns, ARows, AMeasures,
    AFilters, AColumnMembers, AColumnMembers, ARowMembers);
...
function TcxADOMDOLAPDataQueryBuilder.GetTotalsQueryString(const ACubeName
  : string; AColumns: TcxPivotGridFields; AnExpandedMembers: TList;
  AMeasures, AFilters: TcxPivotGridFields): string;
var
  AQuery: TcxMDXQuery;
  ...
  begin
    ...
    if AColumns.Count > 0 then
      AQuery.AddWithClause(' member Measures.MemberUniqueName as ' +
        TcxPivotGridOLAPField(AColumns[0]).HierarchyUniqueName +
        '.MEMBER_UNIQUE_NAME ');
    ...
    begin
      ...
      if AAdditionalString = '' then
        ...
        Result := Format
          ('non empty{ %s } dimension properties member_unique_name', [Result])
    end;
  end;
...
begin
  ...
  try
    if (AnExpandedMembers <> nil) and IsNotLastVisibleOLAPHierarchyItem
      (TcxPivotGridGroupItem(AnExpandedMembers[0])) then
      AQuery.AddAxisClause(ColumnPart(ACubeName, AColumns, AMeasures, AFilters,
        True, True, GetExpandedMembers(AnExpandedMembers)))
    else
    begin
      AQuery.AddAxisClause(ColumnPart(ACubeName, AColumns, AMeasures, AFilters,
        True, True, ''));
...
function TcxMDXQueryAxisMembers.TMember.Add
  (AItem: TcxPivotGridGroupItem): Integer;
begin
...
function TcxMDXQueryAxisMembers.TMember.GetCoordinate(AFields: TFields)
  : TStringDynArray;
...
begin
  ...
  begin
    if (I > 0) and not TcxCustomOLAPDataQueryBuilder.SameHierarchy
      (AFields[I - 1], AFields[I]) then
      Inc(AIndex);
    if I < Count then
      Result[AIndex] := Items[I].UniqueName
    else if not TcxCustomOLAPDataQueryBuilder.SameHierarchy(AFields[I - 1],
      AFields[I]) then
...
function TcxMDXQueryAxisMembers.TMember.GetMember(Index: Integer)
...
function TcxMDXQueryAxisMembers.GetMemberCoordinate(AMemberIndex: Integer)
  : TStringDynArray;
begin
...
function TcxMDXQuery.TryGetMembers(AAxis: Integer;
  out AMembers: TcxMDXQueryAxisMembers): Boolean;
begin
...
procedure TcxMDXQuery.AddAxisClause(const AClause: string;
  AMembers: TcxMDXQueryAxisMembers = nil);
begin
