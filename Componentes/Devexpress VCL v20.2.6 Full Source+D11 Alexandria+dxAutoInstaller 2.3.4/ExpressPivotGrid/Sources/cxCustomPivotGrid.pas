...
{ ExpressPivotGrid }
{ }
{ Copyright (c) 2005-2021 Developer Express Inc. }
{ ALL RIGHTS RESERVED }
{ }
...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Classes, SysUtils, Windows, Messages, Forms, StdCtrls, Graphics, Controls,
  cxContainer, Types,
  DateUtils, Math, Menus, Clipbrd, ExtCtrls, Contnrs, ImgList,
  cxInplaceContainer,
{$IFNDEF NONDB}
  FMTBcd, SqlTimSt,
{$ENDIF}
  ...
  cxEdit, cxStyles, cxLookAndFeels, cxLookAndFeelPainters, cxGeometry,
  cxDrawTextUtils,
  cxVariants, cxDropDownEdit, cxButtons, cxCheckListBox, cxCheckBox, cxFormats,
  cxStorage, Variants,
  cxDateUtils, dxOffice11, cxFilterControl, cxFilter, Generics.Defaults,
  Generics.Collections,
  cxFilterControlDialog, cxNavigator, dxCustomTree, dxThreading,
...
type
  ...
  TcxPivotGridGroupInterval = (giDefault, giDate, giDateDay, giDateDayOfWeek,
    giDateDayOfYear, giDateWeekOfMonth, giDateWeekOfYear, giDateMonth,
    giDateQuarter, giDateYear, giYearAge, giMonthAge, giWeekAge, giDayAge,
    giAlphabetical, giNumeric, giCustom);
  ...
  TcxPivotGridSummaryType = (stCount, stSum, stMin, stMax, stAverage, stStdDev,
    stStdDevP, stVariance, stVarianceP, stCustom, stCountDistinct);
  ...
  public
    procedure Assign(AListA: TList; AOperator: TListAssignOp = laCopy;
      AListB: TList = nil);
  end;
  ...
  public
    ...
    function CompareWithHash(const AValue: TcxPivotGridVariantValue)
      : Integer; overload;
  ...
  protected
    procedure CreateIntersection(ARecords: TcxPivotGridRecords;
      AList: PIntArray);
  ...
  public
    ...
    procedure Paint(ACanvas: TcxCanvas;
      AHandler: TcxPivotGridCustomDrawEvent); virtual;

    property Items[Index: Integer]: TcxPivotGridCustomCellViewInfo
  ...
  public
    constructor Create(AOwner: TcxPivotGridGroupItem; AField: TcxPivotGridField;
  ...
  TcxPivotGridGroupItemValueHelperComparer = class(TInterfacedPersistent,
    IEqualityComparer<TcxPivotGridGroupItemHelper>)
  private
    FOwner: TcxCustomPivotGrid;
  public
    constructor Create(AOwner: TcxCustomPivotGrid); virtual;

    function Equals(const Left, Right: TcxPivotGridGroupItemHelper): Boolean;
      reintroduce;
    function GetHashCode(const Value: TcxPivotGridGroupItemHelper): Integer;
      reintroduce;
  ...
  protected
    ...
    function FindItemByValue(const AValue: Variant;
      var AItem: TcxPivotGridGroupItem): Boolean;
    ...
    procedure PostProcessGroup(ACrossGrandTotal
      : TcxPivotGridGroupItem); virtual;
    ...
    procedure SetFieldExpanding(AField: TcxPivotGridField;
      AExpandState: Boolean);
    ...
    function Find(AValue: TcxPivotGridGroupItemHelper;
      var AItem: TcxPivotGridGroupItem): Boolean; inline;
  ...
  public
    constructor Create(AParent: TcxPivotGridGroupItem;
      const AGroupValue: Variant; ARecordIndex: Integer;
      ADataController: TcxCustomDataController; AField: TcxPivotGridField);
      overload; virtual;
    constructor Create(AParent: TcxPivotGridGroupItem;
      ADataController: TcxCustomDataController; AField: TcxPivotGridField = nil;
      ARecordIndex: Integer = -1); overload; virtual;
    destructor Destroy; override;
    function AddChild(AClass: TcxPivotGridGroupItemClass)
      : TcxPivotGridGroupItem; overload;
    function AddChild(const AGroupValue: Variant; ARecordIndex: Integer;
      AField: TcxPivotGridField): TcxPivotGridGroupItem; overload;
    ...
    function GetCellByCrossItem(AItem: TcxPivotGridGroupItem)
      : TcxPivotGridCrossCell; virtual;
    function GetPrev: TcxPivotGridGroupItem;
    procedure InitializeValue(const ADisplayText: string;
      const AnUniqueName: WideString);
  ...
  public
    constructor Create(AParent: TcxPivotGridGroupItem;
      ...
      AField: TcxPivotGridField); override;
    destructor Destroy; override;
    procedure DeleteChildren; override;
    function GetCellByCrossItem(AItem: TcxPivotGridGroupItem)
      : TcxPivotGridCrossCell; override;
  end;
  ...
  public
    ...
    property PivotGridFields[AIndex: Integer]: TcxPivotGridField
      read GetPivotGridField;
    property RecordCount: Integer read GetRecordCount;
    property Values[ARecordHandle: TcxDataRecordHandle;
      AItemHandle: TcxDataItemHandle]: Variant read GetValue;
  end;
  ...
  protected
    ...
    procedure GetDataTypes(ADataField: TcxPivotGridField;
      var ASourceType, AFloatType: Integer); virtual;
    ...
    property PrevCrossCell: TcxPivotGridCrossCell read FPrevCrossCell
      write FPrevCrossCell;
  ...
  public
    ...
    function GetSummaryByField(AField: TcxPivotGridField;
      ASummaryType: TcxPivotGridSummaryType): Variant; overload; virtual;
    function GetSummaryValue(ASummary: TcxPivotGridCrossCellSummary;
      AType: TcxPivotGridSummaryType): Variant; virtual;
    ...
    property SummaryCells[AIndex: Integer]: TcxPivotGridCrossCellSummary
      read GetSummaryCell; default;
  end;
  ...
  protected
    procedure GetDataTypes(ADataField: TcxPivotGridField;
      var ASourceType, AFloatType: Integer); override;
  ...
  public
    function GetSummaryByField(AField: TcxPivotGridField;
      ASummaryType: TcxPivotGridSummaryType): Variant; override;
    function GetSummaryValue(ASummary: TcxPivotGridCrossCellSummary;
      AType: TcxPivotGridSummaryType): Variant; override;
  ...
  TcxPivotGridGetRecordValueProc = function(const ARecord: Pointer;
    AIsGroupValue: Boolean): Variant of object;
  ...
  protected
    ...
    procedure DoCalculateSummary(ARecords: TList;
      AGetValueProc: TcxPivotGridGetRecordValueProc = nil); virtual;
    function GetIntermediateValue(const ACell: Pointer;
      AIsGroupValue: Boolean): Variant;
    function GetRecordValue(const ARecord: Pointer;
      AIsGroupValue: Boolean): Variant;
  ...
  public
    constructor Create(AOwner: TcxPivotGridCrossCell;
      ADataField: TcxPivotGridField); virtual;
    procedure Clear; virtual;
    function GetPrevCell(APrevCell: TcxPivotGridCrossCellSummary)
      : TcxPivotGridCrossCellSummary;
    ...
    property Custom: Variant index 9 read GetSummaryByIndex
      write SetSummaryByIndex;
    ...
    property SummaryVariation: Variant read FSummaryVariation
      write FSummaryVariation;
  ...
  private
    ...
    function GetCrossCell(ARow, AColumn: TcxPivotGridGroupItem)
      : TcxPivotGridCrossCell;
  ...
  public
    ...
    function GetFieldsListByArea(Area: TcxPivotGridFieldArea)
      : TcxPivotGridFields;

    property ColumnFields: TcxPivotGridFields read FColumnFields;
    // for internal use
    property Columns: TcxPivotGridGroupItem read FColumns;
    property CrossCells[ARow, AColumn: TcxPivotGridGroupItem]
      : TcxPivotGridCrossCell read GetCrossCell;
    ...
    // for internal use
    property GroupCrossItem: TcxPivotGridGroupItem read FGroupCrossItem
      write FGroupCrossItem;
    property GroupCrossRecords: PIntArray read FGroupCrossRecords;
    property GroupCrossRecordCount: Integer read FGroupCrossRecordCount
      write FGroupCrossRecordCount;
    property GroupRecords: PIntArray read FGroupRecords;
    property OLAPDataSource: TcxPivotGridCustomOLAPDataSource
      read FOLAPDataSource;
    property PivotGrid: TcxCustomPivotGrid read FPivotGrid;
  ...
  public
    function Add(AField: TcxPivotGridField)
      : TcxPivotGridViewDataLimitValue; overload;
    procedure Calculate(AField: TcxPivotGridField; V: Variant);
    function FindByField(AField: TcxPivotGridField)
    ...
    property Items[Index: Integer]: TcxPivotGridViewDataLimitValue
      read GetItem; default;
  end;
  ...
  protected
    ...
    procedure CheckSortedByGroupValue(var ASortedByGroupValue: Boolean;
      var ASortOrder: TcxDataSortOrder); virtual;
  ...
  public
    constructor Create(AParent: TcxPivotGridViewDataItem;
      AGroupItem: TcxPivotGridGroupItem); virtual;
    destructor Destroy; override;
    function Add(AData: TcxPivotGridGroupItem): TcxPivotGridViewDataItem;
    function AddTotal(AData: TcxPivotGridGroupItem; AIndex: Integer)
      : TcxPivotGridViewDataItem;
    procedure ClearCache;
    procedure DeleteChildren;
    function GetDisplayText: string;
    function GetGroupItem(var ADataField: TcxPivotGridField)
      : TcxPivotGridGroupItem;
    function GetGroupItemByField(ADataField: TcxPivotGridField)
      : TcxPivotGridGroupItem;
    function GetSummaryType(var ASummaryType: TcxPivotGridSummaryType;
      var ATotal: TcxPivotGridCustomTotal): Boolean; virtual;
    function MeasureWidth(AField: TPersistent; AIncludeTotals: Boolean)
      : Integer;
    ...
    // todo: Internal use only!!! Need for optimization chart connection !!!
  ...
  protected
    ...
    property TemporarySelection: TRect read GetTemporarySelection
      write SetTemporarySelection;
  ...
  protected
    procedure AdjustCellIndexes(var ARow, AColumn: Integer;
      AByMouse: Boolean = False);
    function AdjustCellIndexesPoint(const P: TPoint;
      AByMouse: Boolean = False): TPoint;
    procedure Calculate;
    function CalculateDataWidth(AField: TcxPivotGridField): Integer;
    procedure CalculateDataFieldInfo(var AFieldIndex: Integer;
      var AFields: TcxPivotGridGroupItem;
    ...
    procedure HeaderCellSelect(ADataItem: TcxPivotGridViewDataItem;
      AShift: TShiftState); virtual;
    function IsGroupItemEquals(AItem1,
      AItem2: TcxPivotGridViewDataItem): Boolean;
    ...
    function ScrollColumns(AScrollCode: TScrollCode;
      var AScrollPos: Integer): Boolean;
    function ScrollRows(AScrollCode: TScrollCode;
      var AScrollPos: Integer): Boolean;
    ...
    property LimitValues: TcxPivotGridViewDataLimitValues read FLimitValues;
    property OptionsSelection: TcxPivotGridOptionsSelection
  ...
  public
    ...
    property Cells[Row, Column: Integer]: TcxPivotGridCrossCellSummary
      read GetCell;
  ...
  published
    ...
    property FieldHeaderHints: Boolean read FFieldHeaderHints
      write FFieldHeaderHints default True;
    property FocusCellOnCycle: Boolean read FFocusCellOnCycle
      write FFocusCellOnCycle default False;
    property FocusCellOnTab: Boolean read FFocusCellOnTab write FFocusCellOnTab
      default False;
    property GroupHeaderHints: Boolean read FGroupHeaderHints
      write FGroupHeaderHints default True;
    property SortBySummaryDefaultOrder: TcxDataSortOrder
      read FSortBySummaryDefaultOrder write SetSortBySummaryDefaultOrder
      default soNone;
  end;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property Filtering: Boolean index 0 read FFiltering write SetValue
      default True;
    property FilterResizable: Boolean index 6 read FFilterResizable
      write SetValue default True;
    ...
    property QuickCustomization: Boolean index 3 read FQuickCustomization
      write SetValue default True;
    property QuickPrefiltering: Boolean index 7 read FQuickPrefiltering
      write SetValue default True;
    ...
    property SortingByGroupValues: Boolean index 8 read FSortingByGroupValues
      write SetValue default True;
  end;
  ...
  TcxPivotGridOptionsView = class(TcxPivotGridCustomOptions, IUnknown,
    IcxPivotGridSizableObject)
  private
    ...
    procedure SetShowHeaderFilterButtons
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property ColumnFields: Boolean read FColumnFields write SetColumnFields
      default True;
    property ColumnGrandTotals: Boolean read FColumnGrandTotals
      write SetColumnGrandTotals default True;
    property ColumnGrandTotalText: string read FColumnGrandTotalText
      write SetColumnGrandTotalText;
    property ColumnGrandTotalWidth: Integer read FColumnGrandTotalWidth
      write SetColumnGrandTotalWidth default 0;
    property ColumnTotals: Boolean read FColumnTotals write SetColumnTotals
      default True;
    property ColumnTotalsLocation: TcxPivotGridColumnTotalsLocation
      read FColumnTotalsLocation write SetColumnTotalsLocation default ctlFar;
    property DataFields: Boolean read FDataFields write SetDataFields
      default True;
    property DropArrowColor: TColor read FDropArrowColor write FDropArrowColor
      default cxPivotGridDropArrowColor;
    property FilterDropDownMaxItemCount: Integer
      read FFilterDropDownMaxItemCount write SetFilterDropDownMaxItemCount
      default cxPivotGridDropDownMaxItemCount;
    property FilterDropDownWidth: Integer read FFilterDropDownWidth
      write SetFilterDropDownWidth default cxPivotGridDropDownWidth;
    property FilterFields: Boolean read FFilterFields write SetFilterFields
      default True;
    property FilterSeparator: Boolean read FFilterSeparator
      write SetFilterSeparator default True;
    // property GrandTotalWidth: Integer read FGrandTotalWidth write SetGrandTotalWidth default 0;
    property GrandTotalsForSingleValues: Boolean
      read FGrandTotalsForSingleValues write SetGrandTotalsForSingleValues
      default False;
    property GridLineColor: TColor read FGridLineColor write SetGridLineColor
      default clDefault;
    property GridLines: TcxPivotGridLines read FGridLines write SetGridLines
      default pglBoth;
    property HeaderFilterButtonShowMode: TcxPivotGridFilterButtonShowMode
      read FHeaderFilterButtonShowMode write SetHeaderFilterButtonShowMode
      default pgfbmButton;
    property MarkNarrowCells: Boolean read FMarkNarrowCells
      write SetMarkNarrowCells default False;
    property RowFields: Boolean read FRowFields write SetRowFields default True;
    property RowGrandTotals: Boolean read FRowGrandTotals
      write SetRowGrandTotals default True;
    property RowGrandTotalText: string read FRowGrandTotalText
      write SetRowGrandTotalText;
    property RowGrandTotalWidth: Integer read FRowGrandTotalWidth
      write SetRowGrandTotalWidth default 0;
    property RowTotals: Boolean read FRowTotals write SetRowTotals default True;
    property RowTotalsLocation: TcxPivotGridRowTotalsLocation
      read FRowTotalsLocation write SetRowTotalsLocation default rtlFar;
    property ScrollBars: TcxScrollStyle read GetScrollBars write SetScrollBars
      default ssBoth;
    property ShowHeaderFilterButtons: TcxPivotGridShowHeaderFilterButtons
      read FShowHeaderFilterButtons write SetShowHeaderFilterButtons
      default pgsfbAlways;
    property TotalsForSingleValues: Boolean read FTotalsForSingleValues
      write SetTotalsForSingleValues default False;
    property TotalsLocation: TcxPivotGridTotalsLocation read GetTotalsLocation
      write SetTotalsLocation stored False;
  end;
  ...
  published
    ...
    property CalculationBase: TcxPivotGridCalculationBaseType
      read FCalculationBase write SetCalculationBase default cbRawData;
    property SaveExpanding: Boolean read FSaveExpanding write FSaveExpanding
      default True;
    property SummaryNullIgnore: Boolean read FSummaryNullIgnore
      write SetSummaryNullIgnore default False;
    property VariationNullIgnore: Boolean read FVariationNullIgnore
      write SetVariationNullIgnore default True;
  end;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property CustomizeButton: Boolean read FCustomizeButton
      write SetCustomizeButton default True;
    ...
    property MRUItemsListCount: Integer read FMRUItemsListCount
      write SetMRUItemsListCount default 10;
    property MRUItemsListDropDownCount: Integer read FMRUItemsListDropDownCount
      write SetMRUItemsListDropDownCount default 0;
    property Position: TcxPivotGridPrefilterPosition read FPosition
      write SetPosition default pfpBottom;
    property Visible: TcxPivotGridPrefilterVisible read FVisible
      write SetVisible default pfvNonEmpty;
  end;

  { TcxPivotGridOptionsSelection }

  TcxPivotGridOptionsSelectionInclude = (osiCrossCells, osiGrandTotalCells,
    osiTotalCells);
  TcxPivotGridOptionsSelectionIncludes = set of
  ...
  private
    ...
    procedure SetIncludeCells(const AValue
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property HideFocusRect: Boolean read FHideFocusRect write SetHideFocusRect
      default False;
    property HideSelection: Boolean read FHideSelection write SetHideSelection
      default False;
    property IncludeCells: TcxPivotGridOptionsSelectionIncludes
      read FIncludeCells write SetIncludeCells
      default [osiCrossCells, osiGrandTotalCells, osiTotalCells];
  ...
  TcxPivotGridCustomizationFormFieldListType = (fltAvailable, fltColumn, fltRow,
    fltFilter, fltData);

  TcxPivotGridCustomizationFormClass = class of
    TcxPivotGridCustomCustomizationForm;
  ...
  protected
    ...
    procedure CalculateFieldHitTest(AHitTest: TcxPivotGridHitTest;
      AField: TcxPivotGridField; const ABounds: TRect);
    ...
    function GetFieldListType(AField: TcxPivotGridField;
      out AType: TcxPivotGridCustomizationFormFieldListType): Boolean;
    function GetFieldListByType(AListType
      : TcxPivotGridCustomizationFormFieldListType): TObject; virtual;
    function GetImmediateUpdate: Boolean; virtual;
    procedure Init; virtual;
    procedure LookAndFeelChanged; virtual;
    procedure PopulateFieldList(AList: TList;
      AType: TcxPivotGridCustomizationFormFieldListType);
    ...
    property IsLayoutChanged: Boolean read FIsLayoutChanged
      write SetIsLayoutChanged;
  ...
  public
    ...
    procedure FieldFilterPopup(AField: TcxPivotGridField;
      AInitPopupEvent: TNotifyEvent = nil);
  ...
  private
    ...
    // CBuilder bug! conflict with macro
    function GetVisible: Boolean;
  ...
  protected
    ...
    function GetCustomizationFormClass
      : TcxPivotGridCustomizationFormClass; virtual;
    procedure LookAndFeelChanged; virtual;
  ...
  published
    property AvailableFieldsSorted: Boolean read FAvailableFieldsSorted
      write SetAvailableFieldsSorted default False;
  ...
  TcxPivotGridOptionsDataField = class(TcxPivotGridCustomOptions, IUnknown,
    IcxPivotGridSizableObject, IcxPivotGridField)
  private
  ...
  protected
    ...
    function CanResize: Boolean;
    procedure ChangeScale(M, D: Integer); override;
    function CheckIndex(AIndex: Integer; AFields: TcxPivotGridFields;
    ...
    property ActualWidthIsDirty: Boolean read FActualWidthIsDirty
      write FActualWidthIsDirty;
    property CachedActualWidth: Integer read FCachedActualWidth
      write FCachedActualWidth;
    property DataBuilder: TcxPivotGridDataBuilder read GetDataBuilder;
  ...
  public
    ...
    procedure ApplyBestFit;
  published
    property Area: TcxPivotGridDataFieldArea read FArea write SetArea
      default dfaNone;
    ...
    property IsCaptionAssigned: Boolean read FIsCaptionAssigned
      write FIsCaptionAssigned default False;
    property Caption: string read GetCaption write SetCaption
      stored FIsCaptionAssigned;
    ...
    property MinWidth: Integer read GetMinWidth write SetMinWidth
      default cxPivotGridDefaultFieldMinWidth;
    property Width: Integer read FWidth write SetWidth
      default cxPivotGridDefaultFieldWidth;
  end;
  ...
  public
    constructor Create(APainter: TcxCustomLookAndFeelPainter;
      AScaleFactor: TdxScaleFactor; const ABounds, AVisibleRect: TRect;
  ...
  protected
    ...
    procedure CalculateMarkBounds(ASize: TPoint;
      var AMarkBounds: TRect); virtual;
    ...
    function DrawBackgroundProc(ACanvas: TcxCanvas; const ABounds: TRect)
      : Boolean; virtual;
    ...
    property SortedByGroupValue: Boolean read FSortedByGroupValue
      write FSortedByGroupValue;
  public
    constructor Create(APainter: TcxCustomLookAndFeelPainter;
      AScaleFactor: TdxScaleFactor; const ABounds, AVisibleRect: TRect;
      const AViewParams: TcxViewParams); reintroduce; overload; override;
    constructor Create(APainter: TcxCustomLookAndFeelPainter;
      AScaleFactor: TdxScaleFactor; const ABounds, AVisibleRect: TRect;
      const AViewParams: TcxViewParams; const ADisplayText: string);
      reintroduce; overload;
    function GetHitTestBounds(AHitTest: TcxPivotGridHitTest;
    ...
    property Background: TcxPivotGridHeaderBackgroundCellViewInfo
      read FBackground write FBackground; // for internal use
    property Borders;
  ...
  public
    ...
    procedure PaintTo(ACanvas: TcxCanvas; const ABounds: TRect;
      AState, AFilterState: TcxButtonState; ASortOrder: TcxDataSortOrder;
      const AHasBackground: Boolean = False;
      AHandler: TcxPivotGridCustomDrawEvent = nil);
  ...
  TcxPivotGridDataCellLimitValueTypes = set of
    TcxPivotGridDataCellLimitValueType;
  ...
  public
    procedure Initialize(ARow, AColumn: TcxPivotGridViewDataItem;
      ADataField: TcxPivotGridField); virtual;
    function MeasureWidth: Integer;
    ...
    property DataField: TcxPivotGridField read FDataField;
    property DisplayText;
    property FocusRectStyle: TcxPivotGridFocusRectStyle read FFocusRectStyle
      write FFocusRectStyle;
    property IsGrandTotal: Boolean read GetIsGrandTotal;
    property IsTotal: Boolean read GetIsTotal;
    property LimitValueTypes: TcxPivotGridDataCellLimitValueTypes
      read GetLimitValueTypes;
    property MarkNarrowCells: Boolean read FMarkNarrowCells
      write FMarkNarrowCells;
  ...
  TcxPivotGridPrefilterPartViewInfoClass = class of
  ...
  protected
    ...
    procedure SetDroppedDown(AHitTest: TcxPivotGridHitTest;
      ADroppedDown: Boolean); virtual;
    function SetHotTrack(AHitTest: TcxPivotGridHitTest): Boolean; virtual;
    function SetPressed(AHitTest: TcxPivotGridHitTest; AMouseDown: Boolean)
      : Boolean; virtual;
    ...
    property Visible: Boolean read GetVisible;
  public
    constructor Create(APartsViewInfo
      : TcxPivotGridPrefilterPartsViewInfo); virtual;
    procedure Draw(ACanvas: TcxCanvas);
  end;

  TcxPivotGridPrefilterCloseButtonViewInfo = class
    (TcxPivotGridPrefilterPartViewInfo)
  protected
  ...
  TcxPivotGridPrefilterActivateButtonViewInfo = class
    (TcxPivotGridPrefilterPartViewInfo)
  protected
    function DoCalculateHeight: Integer; override;
    function DoCalculateWidth: Integer; override;
  ...
  TcxPivotGridPrefilterCustomizeButtonViewInfo = class
    (TcxPivotGridPrefilterPartViewInfo)
  private
    function GetFont: TFont;
  ...
  TcxPivotGridPrefilterDropDownButtonViewInfo = class
    (TcxPivotGridPrefilterPartViewInfo)
  protected
    function DoCalculateHeight: Integer; override;
    function DoCalculateWidth: Integer; override;
  ...
  TcxPivotGridPrefilterCaptionViewInfo = class
    (TcxPivotGridPrefilterPartViewInfo)
  protected
  ...
  protected
    ...
    function SetPressed(AHitTest: TcxPivotGridHitTest;
      AMouseDown: Boolean): Boolean;

    property PrefilterViewInfo: TcxPivotGridPrefilterViewInfo
      read FPrefilterViewInfo;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
  public
    constructor Create(APreFilterViewInfo
      : TcxPivotGridPrefilterViewInfo); virtual;
    destructor Destroy; override;
    function AddItem(AItemClass: TcxPivotGridPrefilterPartViewInfoClass)
      : TcxPivotGridPrefilterPartViewInfo;
    ...
    property Items[Index: Integer]: TcxPivotGridPrefilterPartViewInfo
      read GetItem; default;
  end;
  ...
  protected
    ...
    function SetPressed(AHitTest: TcxPivotGridHitTest;
      AMouseDown: Boolean): Boolean;
  public
    constructor Create(APrefilter: TcxPivotGridPrefilter;
  ...
  TcxPivotGridFilterSeparatorCellViewInfo = class
    (TcxPivotGridCustomCellViewInfo)
  private
    FIndentSize: Integer;
  protected
  ...
  protected
    ...
    FRowsBounds: TRect;
    FViewParams: TcxViewParams;
    function AddDragDropAreaInfo(APos: Integer;
    ...
    function AddFieldsBackground(const ABounds: TRect;
      const ADescription: string; AHasDescription: Boolean;
      AArea: TcxPivotGridFieldArea): TcxPivotGridHeaderBackgroundCellViewInfo;
    function AddFilterSeparator(const ATop: Integer)
      : TcxPivotGridFilterSeparatorCellViewInfo;
    ...
    function GetDataFieldFromViewData(AItem: TcxPivotGridViewDataItem)
      : TcxPivotGridField;
    function GetFont(AStyleIndex: Integer): TFont;
    function GetRowColumnPos(ALevel: Integer; var ALevelField: TPersistent)
      : Integer;
    ...
    function GroupHeaderOutOfBounds(AField: TcxPivotGridField;
    ...
    function GetHeaderDisplayText(ACell: TcxPivotGridHeaderCellViewInfo;
      AItem: TcxPivotGridViewDataItem): string; virtual;

    function AddPartBackground(ABounds: TRect): TcxPivotGridCustomCellViewInfo;
    function AddColumnItem(ABounds: TRect; AItem: TcxPivotGridViewDataItem)
      : TcxPivotGridHeaderCellViewInfo;
    function AddDataCell(ARow, AColumn: TcxPivotGridViewDataItem;
    ...
    procedure InitializeFieldsPosition(AFields: TcxPivotGridFields;
      AArea: TcxPivotGridFieldArea);

    procedure InitHeaderCell(ACell: TcxPivotGridHeaderCellViewInfo;
      AItem: TcxPivotGridViewDataItem);
    function IsDataFieldVisible(AArea: TcxPivotGridDataFieldAreas = [])
      : Boolean;
    ...
    function IsGroupItemExist(AList: TList;
      AItem: TcxPivotGridViewDataItem): Boolean;
    function PrepareViewDataItems(var AList: TcxPivotList;
      AStartItem: TcxPivotGridViewDataItem; AAvailableSize: Integer;
      var AStartIndex: Integer): Integer;
    function SpaceBetween(AItem1, AItem2: TcxPivotGridViewDataItem;
      AList: TList): Integer;
  ...
  public
    ...
    property DrawExpandButtons: Boolean read FDrawExpandButtons
      write FDrawExpandButtons;
    ...
    property IsRightToLeftConverted: Boolean read FIsRightToLeftConverted;
    property OptionsDataField: TcxPivotGridOptionsDataField
      read GetOptionsDataField;
    property OptionsView: TcxPivotGridOptionsView read GetOptionsView;
    property Painter: TcxCustomLookAndFeelPainter read FPainter;
    property PivotGrid: TcxCustomPivotGrid read FPivotGrid;
    property Prefilter: TcxPivotGridPrefilter read FPrefilter;
  ...
  IcxPivotGridBaseStyles = interface
    ...
    function GetColumnHeaderParams(AColumn: TcxPivotGridViewDataItem)
      : TcxViewParams;
    function GetContentParams(ACell: TcxPivotGridDataCellViewInfo)
      : TcxViewParams;
    function GetFieldHeaderParams(AField: TcxPivotGridField): TcxViewParams;
    function GetHeaderBackgroundParams(AArea: TcxPivotGridFieldArea)
      : TcxViewParams;
  ...
  protected
    ...
    function GetContentStyleIndexByCell(ACell: TcxPivotGridDataCellViewInfo)
      : Integer; virtual;
    procedure GetDefaultHeadersAreaViewParams(out AParams
    ...
    property SuppressContentColoration: Boolean read FSuppressContentColoration
      write FSuppressContentColoration;
    property SuppressBackgroundBitmaps: Boolean read FSuppressBackgroundBitmaps
      write FSuppressBackgroundBitmaps;

    property Background: TcxStyle index gs_Background read GetValue
      write SetValue;
    property ColumnHeader: TcxStyle index gs_ColumnHeader read GetValue
      write SetValue;
    property ColumnHeaderArea: TcxStyle index gs_ColumnHeaderArea read GetValue
      write SetValue;
    property ColumnMaximumValue: TcxStyle index gs_ColumnMaximumValue
      read GetValue write SetValue;
    property ColumnMinimumValue: TcxStyle index gs_ColumnMinimumValue
      read GetValue write SetValue;
    property Content: TcxStyle index gs_Content read GetValue write SetValue;
    property DataHeaderArea: TcxStyle index gs_DataHeaderArea read GetValue
      write SetValue;
    property FieldHeader: TcxStyle index gs_FieldHeader read GetValue
      write SetValue;
    property FilterHeaderArea: TcxStyle index gs_FilterHeaderArea read GetValue
      write SetValue;
    property FilterSeparator: TcxStyle index gs_FilterSeparator read GetValue
      write SetValue;
    property HeaderBackground: TcxStyle index gs_HeaderBackground read GetValue
      write SetValue;
    property Inactive: TcxStyle index gs_Inactive read GetValue write SetValue;
    property MaximumValue: TcxStyle index gs_MaximumValue read GetValue
      write SetValue;
    property MinimumValue: TcxStyle index gs_MinimumValue read GetValue
      write SetValue;
    property Prefilter: TcxStyle index gs_Prefilter read GetValue
      write SetValue;
    property RowHeader: TcxStyle index gs_RowHeader read GetValue
      write SetValue;
    property RowHeaderArea: TcxStyle index gs_RowHeaderArea read GetValue
      write SetValue;
    property RowMaximumValue: TcxStyle index gs_RowMaximumValue read GetValue
      write SetValue;
    property RowMinimumValue: TcxStyle index gs_RowMinimumValue read GetValue
      write SetValue;
    ...
    property OnGetColumnHeaderStyle: TcxPivotGridGetHeaderItemStyleEvent
      read FOnGetColumnHeaderStyle write FOnGetColumnHeaderStyle;
    property OnGetContentStyle: TcxPivotGridGetContentStyleEvent
      read FOnGetContentStyle write FOnGetContentStyle;
    property OnGetFieldHeaderStyle: TcxPivotGridGetFieldHeaderStyleEvent
      read FOnGetFieldHeaderStyle write FOnGetFieldHeaderStyle;
    property OnGetRowHeaderStyle: TcxPivotGridGetHeaderItemStyleEvent
      read FOnGetRowHeaderStyle write FOnGetRowHeaderStyle;
  public
    ...
    function GetRowHeaderParams(ARow: TcxPivotGridViewDataItem)
      : TcxViewParams; virtual;
  ...
  published
    property Styles: TcxPivotGridStyles read GetStylesValue
  ...
  TcxPivotGridOLAPKPIGraphicType = (gtNone, gtServerDefined, gtShapes,
    gtTrafficLights, gtRoadSigns, gtGauge, gtReversedGauge, gtThermometer,
    gtReversedThermometer, gtCylinder, gtReversedCylinder, gtFaces,
    gtVarianceArrow, gtStandardArrow, gtStatusArrow, gtReversedStatusArrow);

  TcxPivotGridFormatNameType = (fnGeneralNumber, fnCurrency, fnFixed,
    fnStandard, fnPercent, fnScientific, fnYesNo, fnTrueFalse, fnOnOff,
    fnCustom);

  TcxPivotGridInitializeFieldEvent = procedure
    (Sender: TcxPivotGridCustomOLAPDataSource; AField: TcxPivotGridField)
    of object;

  TcxOLAPStructureNodeType = (ntCube, ntMeasure, ntKPI, ntDimension, ntSet,
    ntFolder, ntGroup, ntField);
  ...
  protected
    ...
    procedure ReadData(AStream: TStream; const AVersion: Cardinal = 0);
      override;
  ...
  public
    ...
    function ItemByDisplayText(const ADisplayText: string)
    ...
    property AllMemberUniqueName: string read FAllMemberUniqueName
      write FAllMemberUniqueName;
    property Cardinality: Integer read FCardinality write FCardinality;
    property FormatName: TcxPivotGridFormatNameType read FFormatName
      write FFormatName;
    property FormatString: string read FFormatString write FFormatString;
    property DBType: Integer read FDBType write FDBType;
    property DimensionUniqueName: string read FDimensionUniqueName
      write FDimensionUniqueName;
    property DisplayText: string read FDisplayText write FDisplayText;
    property HierarchyUniqueName: string read FHierarchyUniqueName
      write FHierarchyUniqueName;
    property IsKPI: Boolean read GetIsKPI;
    property KPIGraphicType: TcxPivotGridOLAPKPIGraphicType read FKPIGraphicType
      write FKPIGraphicType;
    ...
    property LevelUniqueName: string read FLevelUniqueName
      write FLevelUniqueName;
    property NodeType: TcxOLAPStructureNodeType read FNodeType
      write SetNodeType;
  ...
  TcxPivotGridCustomOLAPDataSource = class(TcxCustomComponent, IUnknown,
    IdxTreeOwner)
  private
    FActive: Boolean;
  ...
  protected
    ...
    procedure Changed; virtual;
    function CreateNewField(AStructure: TcxPivotGridOLAPStructureNode)
      : TcxPivotGridOLAPField;
    procedure CreateFieldsFromStructure; virtual;
    procedure CreateStructure; virtual;
    procedure CreateStructureRoot;
    function CreateDrillDownDataSource(ACell: TcxPivotGridCrossCell;
      FieldList: TcxObjectList; ASummaryIndex: Integer)
      : TcxPivotGridCrossCellDataSource; overload; virtual;
    function CreateDrillDownDataSource(ACells: TList; FieldList: TcxObjectList;
      ASummaryIndex: Integer): TcxPivotGridCrossCellDataSource;
      overload; virtual;
    ...
    procedure CreateRootLayout(APivotGrid: TcxCustomPivotGrid;
      ARowFields, AColumnFields, ADataFields, AFilterFields
      : TcxPivotGridFields); virtual; abstract;
    procedure DoInitializeField(AField: TcxPivotGridField); virtual;
    procedure ExpandMember(AField: TcxPivotGridField;
      AMember: TcxPivotGridGroupItem; AExpandChildren: Boolean);
    function GetFieldClass: TcxPivotGridFieldClass; virtual;
    function GetFieldForStructure(AGroup: TcxPivotGridFieldGroup;
      AStructure: TcxPivotGridOLAPStructureNode): TcxPivotGridOLAPField;
    function GetGroupForStructure(AStructure: TcxPivotGridOLAPStructureNode)
      : TcxPivotGridFieldGroup;
    function GetIsActive: Boolean; virtual;
    function GetIsTerminated: Boolean; virtual;
    function GetLinkByUniqueName(const AName: WideString)
      : TcxPivotGridOLAPField;
    procedure Initialize; virtual;
    procedure InitializeExpanding(ADataBuilder: TcxPivotGridDataBuilder;
      AField: TcxPivotGridField;
      AnExpandingItem, ACrossGroup: TcxPivotGridGroupItem;
      ASummaryFields: TcxPivotGridFields); virtual;
    procedure PopulateGroupValues(AField: TcxPivotGridOLAPField;
      AList: TcxPivotGridVariantList); virtual;
    procedure NotifyListeners;
    procedure PopulateFilteredUniqueNames(AField: TcxPivotGridOLAPField;
      AFilter: TcxPivotGridFieldFilter;
      var AUniqueValues: TStringList); virtual;
    ...
    //
    procedure TreeNotification(ASender: TdxTreeCustomNode;
      ANotification: TdxTreeNodeNotifications); virtual;
    //
    ...
    property StructureWasChanged: Boolean read FStructureWasChanged
      write FStructureWasChanged;
    property OnInitializeField: TcxPivotGridInitializeFieldEvent
      read FOnInitializeField write FOnInitializeField;
  public
  ...
  public
    procedure Initialize(ARow, AColumn: TcxPivotGridViewDataItem;
      ADataField: TcxPivotGridField); override;
    property GraphicType: TcxPivotGridOLAPKPIGraphicType read FGraphicType
      write FGraphicType;
  end;
  ...
  public
    ...
    property FieldListType: TcxPivotGridCustomizationFormFieldListType
      read FFieldListType write FFieldListType;
    property GroupItem: TcxPivotGridGroupItem read GetGroupItem;
    property HitAtButton: Boolean index htcButton read GetBitState;
    property HitAtCustomizationForm: Boolean index htcCustomizationForm
      read GetBitState;
    ...
    property HitAtFieldTreeView: Boolean index htcFieldTreeView
      read GetBitState;
    ...
    property HitAtHorzSizingEdge: Boolean index htcHorzSizingEdge
      read GetBitState;
    property HitAtPrefilter: Boolean index htcPrefilter read GetBitState;
    property HitAtPrefilterCloseButton: Boolean index htcPrefilterCloseButton
      read GetBitState;
    property HitAtPrefilterActivateButton: Boolean
      index htcPrefilterActivateButton read GetBitState;
    property HitAtPrefilterCaption: Boolean index htcPrefilterCaption
      read GetBitState;
    property HitAtPrefilterCustomizationButton: Boolean
      index htcPrefilterCustomizationButton read GetBitState;
    property HitAtPrefilterDropDownButton: Boolean
      index htcPrefilterDropDownButton read GetBitState;
  ...
  TcxPivotGridFilterPopupIncrementalFilteringHelper = class
...
{$ENDREGION}
  private
    FIncrementalFilteringHelper
      : TcxPivotGridFilterPopupIncrementalFilteringHelper;
    ...
    function IncrementalFilteringOptions
  ...
  public
    ...
    property PivotGrid: TcxCustomPivotGrid read GetPivotGrid;
    property ShowAllState: TcxCheckBoxState read FShowAllState
      write SetShowAllState;
  ...
  strict private
    ...
    procedure FieldPaintTo(AFieldViewInfo: TcxPivotGridFieldHeaderCellViewInfo;
      const ABounds: TRect);
  ...
  protected
    ...
    procedure BeginDragAndDrop; override;
    function CanRemove: Boolean; virtual;
    function CheckArea(const P: TPoint; var AInfo: TcxPivotGridDragDropAreaInfo)
      : Boolean; virtual;
    procedure CreateDragImage; virtual;
    procedure DragAndDrop(const P: TPoint; var Accepted: Boolean); override;
    procedure DragDropField(AArea: TcxPivotGridFieldArea;
      AAreaIndex: Integer); virtual;
    procedure EndDragAndDrop(Accepted: Boolean); override;
    function GetDragAndDropCursor(Accepted: Boolean): TCursor; override;
    function GetImmediateUpdate: Boolean;
    function GetFieldListByType(AListType
      : TcxPivotGridCustomizationFormFieldListType): TObject;
  ...
  public
    ...
    property DragDropAreas[Index: Integer]: TcxPivotGridDragDropAreaInfo
      read GetDragDropArea;
    property DragDropArrowColor: TColor read GetDragDropArrowColor;
    property DragField: IcxPivotGridField read FDragField;
    property FieldViewInfo: TcxPivotGridFieldHeaderCellViewInfo
      read GetFieldViewInfo;
  ...
  protected
    ...
    function CalculateFilterDropDownSize
    ...
    procedure MakeSelected(AShift: TShiftState;
      AMouseMove: Boolean = False); virtual;
    function ProcessNavigationByStep(AGoBackward: Boolean;
  ...
  public
    ...
    property HotTrackController: TcxPivotGridHotTrackController
      read FHotTrackController;
    property IsDesigning: Boolean read GetIsDesigning;
    property OptionsCustomize: TcxPivotGridOptionsCustomize
  ...
  public
    ...
    property FilterType: TcxPivotGridFilterType read FFilterType
      write SetFilterType;
  ...
  public
    ...
    property Field: TcxPivotGridField read FField;
  published
    property AlwaysExpanded: Boolean read FAlwaysExpanded
      write SetAlwaysExpanded default False;
  ...
  published
    property SummaryType: TcxPivotGridSummaryType read FSummaryType
      write SetSummaryType default stSum;
  ...
  public
    ...
    function Add(ASummaryType: TcxPivotGridSummaryType)
      : TcxPivotGridCustomTotal;

    property Field: TcxPivotGridField read FOwner;
    property PivotGrid: TcxCustomPivotGrid read GetPivotGrid;
    property Items[Index: Integer]: TcxPivotGridCustomTotal read GetItem
      write SetItem; default;
  end;
  ...
  protected
    ...
    function GetOwner: TPersistent; override;
    function GetValueByField(AField: TcxPivotGridField; var AValue: Variant)
      : Boolean;
    procedure Update(Item: TCollectionItem); override;
  public
    constructor Create(AOwner: TcxPivotGridSortBySummaryInfo);
      overload; virtual;
    function Add(AField: TcxPivotGridField; const AValue: Variant)
      : TcxPivotGridFieldSortCondition;
    function ConditionDefined(AField: TcxPivotGridField;
      const AValue: Variant): Boolean;

    property Items[Index: Integer]: TcxPivotGridFieldSortCondition read GetItem
  ...
  protected
    ...
    procedure Changed; virtual;
    function ConditionDefined(ADataField: TcxPivotGridField;
      AGroupItem: TcxPivotGridGroupItem): Boolean;
    procedure CreateConditions(ADataField: TcxPivotGridField;
      AGroupItem: TcxPivotGridGroupItem);
    function GetCrossItemForSorting(ACrossGrandTotal: TcxPivotGridGroupItem)
      : TcxPivotGridGroupItem;
    function GetOwner: TPersistent; override;
  ...
  public
    ...
    property Conditions: TcxPivotGridFieldSortConditionCollection
      read FConditions write SetConditions; // for internal use
  ...
  protected
    ...
    function GetDefaultValuesProviderClass
      : TcxCustomEditDefaultValuesProviderClass; virtual;
  ...
  public
    ...
    property Field: TcxPivotGridField read FField;
    property PivotGrid: TcxCustomPivotGrid read GetPivotGrid;
    property ValueTypeAssigned: Boolean read FValueTypeAssigned
      write FValueTypeAssigned;
  ...
  TcxPivotGridSummaryVariation = (svNone, svAbsolute, svPercent,
    svPercentOfColumn, svPercentOfRow);
  TcxPivotGridFieldDataVisibility = (dvAllCells, dvCrossAndTotalCells,
    dvGrandTotalCells);
  ...
  TcxPivotGridGetGroupValueDisplayTextEvent = procedure
  ...
  TcxPivotGridGetPropertiesEvent = procedure(Sender: TcxPivotGridField;
    ...
    var AProperties: TcxCustomEditProperties) of object;
  ...
  TcxPivotGridField = class(TComponent, IcxPivotGridSizableObject,
    IcxPivotGridField, IcxStoredObject, IdxScaleFactor,
  ...
  private
    ...
    procedure SetValueByRecordIndex(ARecordIndex: Integer;
    ...
    procedure InternalDoGetDisplayText(ACell: TcxPivotGridDataCellViewInfo;
      var AText: string);
    //
  ...
  protected
    ...
    function CreateEditStyle(AProperties: TcxCustomEditProperties)
      : TcxEditStyle;
    procedure CreateEditViewData;
    procedure DataChanged; virtual;
    ...
    procedure DragDrop(AArea: TcxPivotGridFieldArea;
      AAreaIndex: Integer); virtual;
    ...
    function IsEqual(const AStructure: TcxPivotGridOLAPStructureNode)
      : Boolean; virtual;
    procedure InitGroupValues;
    procedure InitProperties(AProperties: TcxCustomEditProperties);

    // Embedded virtual methods
    procedure DoCalculateCustomSummary
      (ACell: TcxPivotGridCrossCellSummary); virtual;
    ...
    function DoGetGroupImageIndex(AItem: TcxPivotGridViewDataItem;
      var AAlignHorz: TAlignment; var AAlignVert: TcxAlignmentVert)
      : Integer; virtual;
    function DoGetGroupValue(const AValue: Variant; ARecordIndex: Integer)
      : Variant; virtual;
    function DoGetGroupValueDisplayText(const AValue: Variant): string; virtual;
    procedure DoFilterChanged; virtual;
    function DoGetProperties(ACell: TcxPivotGridCustomCellViewInfo)
      : TcxCustomEditProperties;
    procedure DoGetTotalDisplayText(AGroupItem: TcxPivotGridGroupItem;
    ...
    function GetEditStyle(AProperties: TcxCustomEditProperties; ALocal: Boolean)
      : TcxEditStyle;
    ...
    procedure SetAreaIndexInternal(AArea: TcxPivotGridFieldArea;
      AAreaIndex: Integer);
    ...
    property GroupIntervalItem: TcxPivotGridAdvancedDataGroupIntervalItem
      read FGroupIntervalItem;
    property GroupValuesValid: Boolean read FGroupValuesValid
      write FGroupValuesValid;
    property HeaderWidth: Integer read GetHeaderWidth;
    property ScaleFactor: TdxScaleFactor read GetScaleFactor;
    property SortOrderAssigned: Boolean read FSortOrderAssigned
      write FSortOrderAssigned;
    property ViewInfo: TcxPivotGridFieldHeaderCellViewInfo read FViewInfo;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    procedure SetAreaPosition(AArea: TcxPivotGridFieldArea;
      AAreaIndex: Integer);
    ...
    property Values[ARecordIndex: Integer]: Variant read GetValueByRecordIndex
      write SetValueByRecordIndex;
    property VisibleInGroup: Boolean read GetVisibleInGroup;
    property VisibleIndex: Integer read FVisibleIndex;
  published
    property Area: TcxPivotGridFieldArea read GetArea write SetArea
      default faFilter;
    property AreaIndex: Integer read FAreaIndex write SetAreaIndex
      default cxPivotGridInvalidIndex;
    property AllowedAreas: TcxPivotGridFieldAreas read GetAllowedAreas
      write FAllowedAreas stored IsAllowedAreasStored;
    property IsCaptionAssigned: Boolean read FIsCaptionAssigned
      write FIsCaptionAssigned default False;
    property Caption: string read GetCaption write SetCaption
      stored FIsCaptionAssigned;
    property CustomTotals: TcxPivotGridCustomTotalCollection read FCustomTotals
      write SetCustomTotals stored IsCustomTotalStored;
    property DataBinding: TcxPivotGridFieldDataBinding read FDataBinding
      write SetDataBinding;
    property DataVisibility: TcxPivotGridFieldDataVisibility
      read FDataVisibility write SetDataVisibility default dvAllCells;
    property DisplayFormat: string read FDisplayFormat write SetDisplayFormat;
    ...
    property ImageAlign: TAlignment read FImageAlign write SetImageAlign
    ...
    property GroupIndex: Integer read GetGroupIndex write SetGroupIndex
      default -1;
    property GroupExpanded: Boolean read GetGroupExpanded write SetGroupExpanded
      default True;
    property GroupInterval: TcxPivotGridGroupInterval read GetGroupInterval
      write SetGroupInterval default giDefault;
    property GroupIntervalAdvanced: Integer read FGroupIntervalAdvanced
      write SetGroupIntervalAdvanced default -1;
    property GroupIntervalRange: Integer read FGroupIntervalRange
      write SetGroupIntervalRange default cxPivotGridDefaultGroupIntervalRange;
    property Hidden: Boolean read GetHidden write SetHidden default False;
    property MinWidth: Integer read GetMinWidth write SetMinWidth
      default cxPivotGridDefaultFieldMinWidth;
    property SummaryType: TcxPivotGridSummaryType read FSummaryType
      write SetSummaryType default stSum;
    property SortBySummaryInfo: TcxPivotGridSortBySummaryInfo
      read FSortBySummaryInfo write SetSortBySummaryInfo;
    ...
    property SummaryVariation: TcxPivotGridSummaryVariation
      read FSummaryVariation write SetSummaryVariation default svNone;
    property TotalsVisibility: TcxPivotGridTotalsVisibility
      read FTotalsVisibility write SetTotalsVisibility default tvAutomatic;
    property TopValueCount: Integer read FTopValueCount write SetTopValueCount
      default 0;
    property TopValueShowOthers: Boolean read FTopValueShowOthers
      write SetTopValueShowOthers default False;
    property Visible: Boolean read GetVisible write SetVisible default False;
    property Width: Integer read FWidth write SetWidth default 0;
    //
    property PropertiesEvents: TNotifyEvent read FSubClassEvents
      write FSubClassEvents;
    property OnCalculateCustomSummary: TcxPivotGridCalculateCustomSummaryEvent
      read FOnCalculateCustomSummary write FOnCalculateCustomSummary;
    property OnGetDisplayText: TcxPivotGridGetDisplayTextEvent
      read FOnGetDisplayText write FOnGetDisplayText;
    property OnGetGroupImageIndex: TcxPivotGridFieldGetGroupImageIndex
      read FOnGetGroupImageIndex write FOnGetGroupImageIndex;
    property OnGetGroupValue: TcxPivotGridGetGroupValueEvent
      read FOnGetGroupValue write FOnGetGroupValue;
    property OnGetGroupValueDisplayText
      : TcxPivotGridGetGroupValueDisplayTextEvent
      read FOnGetGroupValueDisplayText write FOnGetGroupValueDisplayText;
    property OnGetProperties: TcxPivotGridGetPropertiesEvent
      read FOnGetProperties write SetOnGetProperties;
    property OnGetTotalDisplayText: TcxPivotGridGetTotalDisplayTextEvent
  ...
  published
    property KPIGraphicType: TcxPivotGridOLAPKPIGraphicType read FKPIGraphicType
      write SetKPIGraphicType stored IsKPIGraphicTypeStored;
  end;

  { TcxPivotGridOLAPField }

  TcxPivotGridOLAPFieldSortMode = (osmValue, osmDisplayText, osmKey, osmID,
    osmNone);
  ...
  protected
    ...
    property AllMemberUniqueName: string read GetAllMemberUniqueName
      write SetAllMemberUniqueName;
  public
    destructor Destroy; override;
    procedure Assign(Source: TPersistent); override;
    ...
    class function PopulateFilteredValues(AField: TcxPivotGridField;
      IsExpanding: Boolean = False): TStrings;
    ...
    property Structure: TcxPivotGridOLAPStructureNode read FStructure
      write SetStructure; // for internal use
  published
    property Options: TcxPivotGridOLAPFieldOptions read GetOptions
      write SetOptions;
    property FieldType: TcxPivotGridOLAPFieldType read GetFieldType
      write FFieldType;
    property SortMode: TcxPivotGridOLAPFieldSortMode read FSortMode
      write SetSortMode default osmValue;
  end;
  ...
  protected
    FieldList: TList;
    function CanDropTo(AArea: TcxPivotGridFieldArea; AIndex: Integer)
      : Boolean; virtual;
    procedure DefineProperties(Filer: TFiler); override;
  ...
  public
    ...
    property PivotGrid: TcxCustomPivotGrid read FOwner;
    property Items[Index: Integer]: TcxPivotGridFieldGroup read GetItem
      write SetItem; default;
  end;
  ...
  protected
    ...
    function CreateSubItem(AOwner: TMenuItem; const ACaption: string;
      ACommand: Integer; AEnabled: Boolean = True; AChecked: Boolean = False)
      : TMenuItem;
  ...
  published
    ...
    property UseBuiltInMenu: Boolean read FUseBuiltInMenu write FUseBuiltInMenu
      default True;
  end;
  ...
  TcxPivotGridFieldHeaderPopupMenuItem = (fpmiHide, fpmiOrder, fpmiFieldList,
    fpmiSummaryType);
  TcxPivotGridFieldHeaderPopupMenuItems = set of
  ...
  public
    ...
    property OptionsDataField: TcxPivotGridOptionsDataField
      read GetOptionsDataField;
  published
    property Items: TcxPivotGridFieldHeaderPopupMenuItems read FItems
      write FItems stored IsItemsStored;
    property SummaryItems: TcxPivotGridSummaryTypes read FSummaryItems
      write FSummaryItems stored IsSummaryItemsStored;
  end;

  { TcxPivotGridGroupValueMenu }

  TcxPivotGridGroupValuePopupMenuItem = (vpmiExpandCollapse, vpmiExpandAll,
    vpmiCollapseAll);
  TcxPivotGridGroupValuePopupMenuItems = set of
  ...
  private
    ...
    procedure AddSortByValueMenuItem(AField, ADataField: TcxPivotGridField;
      const AGroupItem: TcxPivotGridGroupItem; AArea: TcxPivotGridFieldArea;
      ANeedSuffix: Boolean);
    function GetDataBuilder: TcxPivotGridDataBuilder;
    function GetHitTest: TcxPivotGridHitTest;
    function IsItemsStored: Boolean;
  protected
    ...
    property ViewDataItem: TcxPivotGridViewDataItem read FViewDataItem
      write FViewDataItem;
  public
    constructor Create(AOwner: TcxPivotGridPopupMenus); override;
    destructor Destroy; override;
    property GroupItem: TcxPivotGridGroupItem read FGroupItem;
  published
    property Items: TcxPivotGridGroupValuePopupMenuItems read FItems
      write FItems stored IsItemsStored;
  end;
  ...
  protected
    ...
    procedure DoOnPopup(ASender: TcxPivotGridCustomPopupMenu;
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
  published
    property FieldHeaderMenu: TcxPivotGridFieldHeaderMenu read FFieldHeaderMenu
      write SetFieldHeaderMenu;
    property GroupValueMenu: TcxPivotGridGroupValueMenu read FGroupValueMenu
      write SetGroupValueMenu;
    property HeaderAreaMenu: TcxPivotGridHeaderAreaMenu read FHeaderAreaMenu
      write SetHeaderAreaMenu;
    property OnClick: TcxPivotGridPopupMenuClickEvent read FOnClick
      write FOnClick;
    property OnPopup: TcxPivotGridPopupMenuPopupEvent read FOnPopup
      write FOnPopup;
  end;
  ...
  public
    ...
    property ExpandColumns: Boolean read GetExpandColumns
      write SetExpandColumns;
    property OptionsView: TcxPivotGridOptionsView read GetOptionsView;
    property PivotGrid: TcxCustomPivotGrid read FPivotGrid;
    property Styles: TcxPivotGridStyles read GetStyles;
    property ViewInfo: TcxPivotGridViewInfo read GetViewInfo;
  ...
  public
    ...
    property PivotGrid: TcxCustomPivotGrid read FPivotGrid;
    property Items[Index: Integer]: TcxPivotGridFilterMRUItem
      read GetItem; default;
    property VisibleCount: Integer read GetVisibleCount write SetVisibleCount;
    property VisibleItems[Index: Integer]: TcxPivotGridFilterMRUItem
      read GetVisibleItem;
  end;
  ...
  private
    ...
    function GetColumnHeaderLevelText(AColumn: TcxPivotGridViewDataItem;
  ...
  public
    ...
    property CollapsedRowsHeaderValue: string read FCollapsedRowsHeaderValue
      write FCollapsedRowsHeaderValue;
    property CopyAll: Boolean read FCopyAll write FCopyAll;
    property IncludeAllColumnHeaders: Boolean read FIncludeAllColumnHeaders
      write FIncludeAllColumnHeaders;
    property IncludeAllRowHeaders: Boolean read FIncludeAllRowHeaders
      write FIncludeAllRowHeaders;
  ...
  published
    ...
    property Effect;
    property Font;
    property Mode: TcxLockedStateImageShowingMode read FMode write FMode
      default lsimNever;
    property ShowText;
    property Text;
  end;
  ...
  TcxPivotGridCompareEvent = procedure(Sender: TcxCustomPivotGrid;
    AField: TcxPivotGridField; const AValue1, AValue2: Variant;
  ...
  TcxPivotGridCustomDrawPartEvent = procedure(Sender: TcxCustomPivotGrid;
    ...
    var ADone: Boolean) of object;
  TcxPivotGridFieldChangedEvent = procedure(Sender: TcxCustomPivotGrid;
    AField: TcxPivotGridField) of object;
  TcxPivotGridPrepareLockedStateImageEvent = procedure
    (Sender: TcxCustomPivotGrid; AImage: TcxBitmap32; var ADone: Boolean)
    of object;
  TcxPivotGridGetCellHintEvent = procedure(Sender: TcxCustomPivotGrid;
    const APoint: TPoint; ACell: TcxPivotGridCustomCellViewInfo;
    var ABounds, ATextBounds: TRect; var AHint: string) of object;

  TcxCustomPivotGrid = class(TcxControl, IcxFilterControl,
    IcxLockedStateFontChanged, IcxLockedStatePaint, IcxNavigator,
    IcxPivotGridDesignerIntf, IcxStoredObject, IcxStoredParent, IdxSkinSupport)
  private
    ...
    procedure SetImages(var AField: TCustomImageList;
    ...
    procedure SetOptionsLockedStateImage
      (Value: TcxPivotGridOptionsLockedStateImage);
    ...
    procedure WMRefreshCustomization(var Message: TMessage);
      message DXM_REFRESHCUSTOMIZATION;
    ...
    procedure StoreTo(AStorageType: TcxStorageType; const AStorageName: string;
      AStorageStream: TStream; AReCreate: Boolean = True);
  protected
    ...
    function CreateGroupItemValueComparer
      : TcxPivotGridGroupItemValueHelperComparer; virtual;
    ...
    procedure DoGetCellHint(APoint: TPoint;
      ACell: TcxPivotGridCustomCellViewInfo; var ABounds, ATextBounds: TRect;
    ...
    function SetFieldAreaIndex(AField: TPersistent;
    ...
    function GetItemValueSource(AItemIndex: Integer)
      : TcxDataEditValueSource; virtual;
    ...
    function StoredObject_GetProperties(AProperties: TStrings): Boolean;
      overload; virtual;
    procedure StoredObject_GetPropertyValue(const AName: string;
      var AValue: Variant); virtual;
    procedure StoredObject_SetPropertyValue(const AName: string;
      const AValue: Variant); virtual;
    // IcxStoredParent
    function StoredParent_CreateChild(const AObjectName, AClassName: string)
      : TObject; virtual;
    procedure StoredParent_DeleteChild(const AObjectName: string;
    ...
    function CreateLockedStatePaintHelper
      : TcxPivotGridLockedStatePaintHelper; virtual;
    function CreateOptionsLockedStateImage
      : TcxPivotGridOptionsLockedStateImage; virtual;
    ...
    property GroupItemValueComparer: TcxPivotGridGroupItemValueHelperComparer
      read FGroupItemValueComparer;
    ...
    property HasSummaryVariation: Boolean read FHasSummaryVariation
      write FHasSummaryVariation;
    property ImagesListener: TChangeLink read FImagesListener;
    property IsLocked: Boolean read GetIsLocked;
    property LockCount: Integer read FLockCount write FLockCount;
    property LockedStatePaintHelper: TcxPivotGridLockedStatePaintHelper
      read FLockedStatePaintHelper;
    property Painter: TcxPivotGridPainter read FPainter;
    property PrefilterMRUItems: TcxPivotGridFilterMRUItems
    ...
    property OnCompare: TcxPivotGridCompareEvent read FOnCompare
      write FOnCompare;
    ...
    property OnCustomization: TNotifyEvent read FOnCustomization
      write FOnCustomization;
    property OnFieldPosChanged: TcxPivotGridFieldChangedEvent
      read FOnFieldPosChanged write FOnFieldPosChanged;
    property OnFieldSizeChanged: TcxPivotGridFieldChangedEvent
      read FOnFieldSizeChanged write FOnFieldSizeChanged;
    property OnFilterChanged: TNotifyEvent read FOnFilterChanged
      write FOnFilterChanged;
    property OnGetCellHint: TcxPivotGridGetCellHintEvent read FOnGetCellHint
      write FOnGetCellHint;
    property OnPrefilterDialogShow: TNotifyEvent read FOnPrefilterDialogShow
      write FOnPrefilterDialogShow;
    property OnPrepareLockedStateImage: TcxPivotGridPrepareLockedStateImageEvent
  ...
  public
    ...
    procedure BeginUpdate;
    procedure CopyToClipboard(ACopyAll: Boolean = False;
      AIncludeHeaders: Boolean = False; AIncludeAllRowHeaders: Boolean = True;
      AIncludeAllColumnHeaders: Boolean = True); virtual;
    function CreateDrillDownDataSource: TcxCustomDataSource; virtual;
    ...
    procedure RestoreFromIniFile(const AStorageName: string;
      ACreateChildren: Boolean = False; ADeleteChildren: Boolean = False);
    ...
    property DataController: TcxCustomDataController read FDataController;
    property FieldCount: Integer read GetFieldCount;
    property FieldHeaderImages: TCustomImageList read FFieldHeaderImages
      write SetFieldHeaderImages;
    ...
    property Groups: TcxPivotGridFieldGroupCollection read FGroups
      write SetGroups;
    property HitTest: TcxPivotGridHitTest read FHitTest;
    property LookAndFeel;
    property LookAndFeelPainter;
    property OLAPDataSource: TcxPivotGridCustomOLAPDataSource
      read FOLAPDataSource write SetOLAPDataSource;
    property OptionsBehavior: TcxPivotGridOptionsBehavior read FOptionsBehavior
      write SetOptionsBehavior;
    property OptionsCustomize: TcxPivotGridOptionsCustomize
      read FOptionsCustomize write SetOptionsCustomize;
    property OptionsData: TcxPivotGridOptionsData read FOptionsData
      write SetOptionsData;
    property OptionsDataField: TcxPivotGridOptionsDataField
      read FOptionsDataField write SetOptionsDataField;
    property OptionsLockedStateImage: TcxPivotGridOptionsLockedStateImage
      read FOptionsLockedStateImage write SetOptionsLockedStateImage;
    property OptionsPrefilter: TcxPivotGridOptionsPrefilter
      read FOptionsPrefilter write SetOptionsPrefilter;
    property OptionsSelection: TcxPivotGridOptionsSelection
      read FOptionsSelection write SetOptionsSelection;
    property OptionsView: TcxPivotGridOptionsView read FOptionsView
      write SetOptionsView;
    property PopupMenus: TcxPivotGridPopupMenus read FPopupMenus
      write SetPopupMenus;
  ...
  TcxPivotGridGroupValueFunc<T, TResult> = reference to function
    (const AValue: T): TResult;
  ...
  private
    ...
    FID: Integer;
  protected
    function GetDateTimeValue(const AValue: Variant;
  ...
  type
    ...
    public
      ...
      function GetGroupValueDisplayText(const AValue: Variant): string;
        override;
...
{$HINTS ON}
  protected
    ...
    class function FindItemByName(const ADisplayName: string;
      var AItem: TcxPivotGridAdvancedDataGroupIntervalItem): Integer;
  public
    class function GetCount: Integer;
    class function GetItem(Index: Integer)
      : TcxPivotGridAdvancedDataGroupIntervalItem;
    class function GetItemByID(ID: Integer)
      : TcxPivotGridAdvancedDataGroupIntervalItem;
    class function GetItemByName(const ADisplayName: string)
      : TcxPivotGridAdvancedDataGroupIntervalItem;
    class function Register(const ADisplayName: string;
      AGetGroupValueFunc: TcxPivotGridGroupValueFunc<Variant, Variant>;
      AGetGroupValueDisplayTextFunc: TcxPivotGridGroupValueFunc<Variant,
      string>): Integer;
...
const
  cxPivotGridCustomizationFormClass: TcxPivotGridCustomizationFormClass =
    TcxPivotGridCustomCustomizationForm;
  cxPivotGridPopupMenuImages: TCustomImageList = nil;
  DesignerHelper: IcxPivotGridDesignerHelper = nil;
  PivotGridCrossCellSummaryClass: TcxPivotGridCrossCellSummaryClass =
    TcxPivotGridCrossCellSummary;

function GetOLAPHierarchyLevelUniqueName(AField: TcxPivotGridOLAPField): string;
function IsNotLastVisibleOLAPHierarchyItem
  (AnItem: TcxPivotGridGroupItem): Boolean;
...
uses
  ...
  cxPivotGridAdvancedCustomization, dxHash, dxHashUtils, dxDPIAwareUtils,
  dxBuiltInPopupMenu;

const
  IndentOffsetSize = 6;

  OrdinalTypeSet: set of Byte = [varSmallInt, varInteger, varBoolean, varByte,
    varWord, varShortInt, varLongWord, varInt64];

  E_NOINTERFACE = HResult($80004002);

  CustomizationCommand: array [Boolean] of Integer = (pgcmShowCustomization,
    pgcmHideCustomization);

  DataAreaToFieldArea: array [TcxPivotGridDataFieldArea]
    of TcxPivotGridFieldArea = (faFilter, faColumn, faRow);

  DimensionAllowedAreas: array [Boolean] of TcxPivotGridFieldAreas =
    ([faColumn, faRow, faFilter], [faData]);

  PivotGridDefaultStoredProperties: array [0 .. 1] of string = ('Prefilter',
    'PrefilterActive');
  FieldDefaultStoredProperties: array [0 .. 6] of string = ('SortOrder',
    'GroupExpanded', 'Visible', 'Width', 'Area', 'AreaIndex', 'Filter');

  DefaultSummaryItems: TcxPivotGridSummaryTypes = [stCount, stSum, stMin, stMax,
    stAverage];
...
begin
  Result := not((VarType(ADivider) in [varEmpty, varNull]) or
    (VarIsSoftNumeric(ADivider) and (ADivider = 0)));
  if not Result then
    AResult := Null;
end;
...
function IsNotLastVisibleOLAPHierarchyItem
  (AnItem: TcxPivotGridGroupItem): Boolean;
begin
  Result := (AnItem <> nil) and (AnItem.Field <> nil) and
    IsOLAPHierarchy(AnItem) and (AnItem.Field.Group.IndexOf(AnItem.Field) <
    AnItem.Field.Group.VisibleCount - 1);
end;

function GetPropertyIndex(const AName: string;
  ANamesMap: array of string): Integer;
var
  I: Integer;
begin
...
type
  ...
  protected
    ...
    procedure DoExecute;
  public
    constructor Create(AField, ADataField: TcxPivotGridField;
      const AGroup: TcxPivotGridGroupItem);
  end;
  ...
  protected
    ...
    procedure AddChildren(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem); virtual;
    function AddData(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem): Boolean; virtual;
    procedure AddTotals(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem); virtual;
    procedure DeleteEmptyItems;
    function IsVariationFieldsOnly: Boolean;
    procedure InitVisibleItem(AItem: TcxPivotGridViewDataItem);
    procedure ProcessItem(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem); virtual;
    procedure Produce; virtual;
    function SkipFieldProcessing(AParent: TcxPivotGridViewDataItem;
      AField: TcxPivotGridField): Boolean;
    function TotalNeeded(AVisible, ASingleVisible: Boolean;
      AItem: TcxPivotGridGroupItem): Boolean;
  public
  ...
  protected
    procedure AddChildren(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem); override;
    function AddData(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem): Boolean; override;
    function AddDataEx(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem): Boolean; virtual;
    procedure AddTotals(AItem: TcxPivotGridGroupItem; ALevel: Integer;
      AParent: TcxPivotGridViewDataItem); override;
  end;
...
constructor TcxPivotGridGroupValueMenuSortByValueCriterion.Create(AField,
  ADataField: TcxPivotGridField; const AGroup: TcxPivotGridGroupItem);
begin
  Field := AField;
...
function TcxPivotGridFieldPosition.IsSameGroup
  (AField: TcxPivotGridField): Boolean;
begin
  Result := (AField = FField) or
    ((AField.Group <> nil) and (AField.Group = FGroup));
end;
...
constructor TcxPivotGridItemsProducer.Create(ASource: TcxPivotGridGroupItem;
  ADest: TcxPivotGridViewDataItem; ADestList: TList;
  ALocation: TcxPivotGridTotalsLocation; ATotals, ASingleTotals, AGrandTotals,
  ASingleGrandTotals: Boolean);
begin
  Dest := ADest;
  DestList := ADestList;
  DestList.Clear;
...
var
  I, C: Integer;
begin
  if (ALevel < 0) and (AItem.ItemCount = 0) and not AItem.HasChildren then
    Exit;
  C := DestList.Count;
  if ALevel >= 0 then
  ...
  begin
    if not AddData(AItem, ALevel + 1, AParent) or (ALevel >= 0) and
      (C = DestList.Count) then
...
begin
  Result := (ALevel = DataLevel) or (AItem.IsCollapsed and (DataLevel <> MaxInt)
    and (DataLevel > ALevel));
  if Result then
  begin
    for I := 0 to DataFields.ItemCount - 1 do
    begin
      if SkipFieldProcessing(AParent, DataFields.Items[I].Field) then
        Continue;
...
begin
  if ((ALevel = -1) and TotalNeeded(GrandTotals, GrandTotalsForSingleValues,
    AItem)) or ((ALevel >= 0) and TotalNeeded(Totals, TotalsForSingleValues,
    AItem)) then
  begin
...
begin
  Result := (AField.SummaryVariation in [svAbsolute, svPercent]) and
    (AParent.IsGrandTotal or (AParent.Index = 0) or
    ((AParent.Index = 1) and AParent.IsTotal));
  if Result and AParent.IsGrandTotal then
    Result := (Source.ItemCount > 0) or not VariationFieldsOnly;
  Result := Result or (not AParent.IsGrandTotal and
    (AField.DataVisibility = dvGrandTotalCells)) or
    (AParent.IsGrandTotal and (AField.DataVisibility = dvCrossAndTotalCells));
end;

function TcxPivotGridItemsProducer.TotalNeeded(AVisible, ASingleVisible
  : Boolean; AItem: TcxPivotGridGroupItem): Boolean;
begin
...
procedure TcxPivotGridCompactLayoutProducer.AddChildren
  (AItem: TcxPivotGridGroupItem; ALevel: Integer;
...
function TcxPivotGridCompactLayoutProducer.AddDataEx
  (AItem: TcxPivotGridGroupItem; ALevel: Integer;
  AParent: TcxPivotGridViewDataItem): Boolean;
var
  I, J: Integer;
  ADataItem: TcxPivotGridViewDataItem;
begin
...
function CompareFieldsPosition(AInfo1,
  AInfo2: TcxPivotGridFieldPosition): Integer;
begin
...
function CompareGroupItemsBySummary(AItem1,
  AItem2: TcxPivotGridGroupItem): Integer;
begin
  if (AItem1 <> AItem2) and (AItem1.RecordIndex = cxPivotGridOthersRecordIndex)
  then
  begin
...
function CompareFilterValuesByHash(AItem1,
  AItem2: TcxPivotGridVariantValue): Integer;
begin
...
function GetItemNeighbors(AIsTop, AIsLeft, AIsBottom, AIsRight: Boolean)
  : TcxNeighbors;
begin
  Result := [];
  if not AIsTop then
    Include(Result, nTop);
  if not AIsBottom then
...
function FormatDisplayValue(const AValue: Variant; const ADisplayFormat: string;
  AIsCurrency, AIsPercent: Boolean; AIsOLAP: Boolean): string;
var
  AFormatStr: string;
begin
  ...
  if VarIsNumericEx(AValue) or (VarType(AValue) = varFMTBCD) then
  ...
  begin
    ...
    begin
      if FieldViewInfo.IsRightToLeftConverted then
        HotSpot := cxPoint(FieldViewInfo.Bounds.Right - DragImage.Width -
          CurMousePos.X, FieldViewInfo.Bounds.Top - CurMousePos.Y)
      else
        HotSpot := cxPoint(FieldViewInfo.Bounds.Left - CurMousePos.X,
          FieldViewInfo.Bounds.Top - CurMousePos.Y);
    end
    else
...
begin
  ...
  if Result and not PivotGrid.OptionsCustomize.Hiding then
    Result := cxRectPtIn(PivotGrid.Customization.Form.BoundsRect,
      GetMouseCursorPos);
end;
...
begin
  ...
  begin
    if not HitTest.HitAtCustomizationForm and
      cxRectPtIn(PivotGrid.ClientBounds, P) then
    begin
      for I := 0 to DragDropAreaCount - 1 do
      begin
        AInfo := DragDropAreas[I];
        Result := cxRectPtIn(AInfo.Bounds, P) and
          DragField.IsCompatibleWidth(AInfo);
        if Result then
          Break;
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not AField.VisibleInGroup then
        Break;
      ...
      begin
        ...
        ARgn.Combine(TcxRegion.Create(ALeft - AHorzSpace, ASize.cy div 2, ALeft,
          ASize.cy div 2 + 1), roAdd);
      ...
      begin
        ...
        ARgn.Combine(TcxRegion.Create(AWidth, ASize.cy div 2,
          AWidth + AHorzSpace, ASize.cy div 2 + 1), roAdd);
...
procedure TcxPivotGridDragAndDropObject.DragAndDrop(const P: TPoint;
...
begin
  ...
  if Accepted and not IsSameDropPlace then
    Accepted := PivotGrid.Groups.CanDropTo(DragDropInfo.Area,
      DragDropInfo.AreaIndex);
...
begin
  ...
  else if not DragDropInfo.Visible or (Accepted and not AAccepted and CanRemove)
  then
  begin
...
begin
  Result := nil;
  if not PivotGrid.Customization.Visible then
    Exit;
...
begin
  ...
  begin
    if FieldViewInfo.Group = nil then
    begin
      Result := ((DragDropInfo.AreaIndex - FieldViewInfo.AreaIndex) in [0, 1])
        or (FieldViewInfo.Field = DragDropInfo.Field);
    end
    else
...
begin
  ...
  begin
    if Arrows = nil then
      Arrows := TcxPlaceArrows.CreateArrows(OptionsView.DropArrowColor,
        clBtnText);
...
procedure TcxPivotGridDragAndDropObject.FieldPaintTo(AFieldViewInfo
  : TcxPivotGridFieldHeaderCellViewInfo; const ABounds: TRect);
begin
...
function TcxPivotGridDragAndDropObject.GetDragDropArea(AIndex: Integer)
  : TcxPivotGridDragDropAreaInfo;
begin
...
function TcxPivotGridDragAndDropObject.GetFieldViewInfo
  : TcxPivotGridFieldHeaderCellViewInfo;
begin
...
procedure TcxPivotGridFieldGroup.Insert(AIndex: Integer;
  AField: TcxPivotGridField);
begin
  PivotGrid.BeginUpdate;
  try
...
begin
  ...
  begin
    ...
    if not Result or (AField = Fields[I]) then
      Break;
...
function TcxPivotGridFieldGroup.CanDropTo(AArea: TcxPivotGridFieldArea;
...
function TcxPivotGridFieldGroup.GetNextField(AField: TcxPivotGridField)
  : TcxPivotGridField;
var
  AIndex: Integer;
begin
...
function TcxPivotGridFieldGroup.IsEqual(const AStructure
  : TcxPivotGridOLAPStructureNode): Boolean;
begin
  if UniqueName = '' then
...
function TcxPivotGridFieldGroupCollection.GetItem(AIndex: Integer)
  : TcxPivotGridFieldGroup;
begin
  Result := inherited Items[AIndex] as TcxPivotGridFieldGroup;
end;

procedure TcxPivotGridFieldGroupCollection.SetItem(AIndex: Integer;
  AValue: TcxPivotGridFieldGroup);
begin
...
begin
  ...
  begin
    ...
    begin
      AAdapter := TdxBuiltInPopupMenuAdapterManager.GetActualAdapterClass.
        Create(nil);
      try
        AAdapter.SetImages(FBuiltInMenu.Images);
        AAdapter.SetLookAndFeel(PivotGrid.LookAndFeel);
        TdxBuiltInPopupMenuAdapterHelper.AddMenu(AAdapter, FBuiltInMenu,
          AdapterClickHandler);
...
function TcxPivotGridCustomPopupMenu.CreateSeparator(AOwner: TMenuItem)
  : TMenuItem;
begin
...
function TcxPivotGridCustomPopupMenu.CreateSubItem(AOwner: TMenuItem;
  const ACaption: string; ACommand: Integer; AEnabled: Boolean;
  AChecked: Boolean): TMenuItem;
begin
...
begin
  if (ACommand = pgcmShowCustomization) or (ACommand = pgcmHideCustomization)
  then
...
function TcxPivotGridCustomPopupMenu.GetItemByCommand(ACommand: Integer)
  : TMenuItem;
...
procedure TcxPivotGridCustomPopupMenu.Notification(AComponent: TComponent;
...
constructor TcxPivotGridHotTrackController.Create
  (AOwner: TcxPivotGridController);
begin
  FOwner := AOwner;
end;
...
constructor TcxPivotGridHintHelper.Create(AController
  : TcxPivotGridHintController);
begin
...
begin
  Result := Application.Active and (HitTest.HitObject <> nil) and
    (HitTest.HitAtDataCell and PivotGrid.OptionsBehavior.CellHints) or
...
function TcxPivotGridHintController.GetHintInfo(var ABounds,
  ATextBounds: TRect): Boolean;
var
  AText: string;
begin
  ...
  begin
    ...
    if HintObject <> HitTest.HitObject then
      PivotGrid.DoGetCellHint(HitTest.HitPoint, HitTest.HitObject, ABounds,
        ATextBounds, FHintText);
    Result := (Length(FHintText) > 0) and (HintObject <> HitTest.HitObject) and
      ((AText <> FHintText) or (cxTextWidth(HitTest.HitObject.Font, FHintText) >
      cxRectWidth(ATextBounds)));
    if Result then
...
procedure TcxPivotGridController.BeforeMouseDown(Button: TMouseButton;
...
begin
  ...
  with HitTest do
  begin
    if HitAtHorzSizingEdge and (ResizeField <> nil) and ResizeField.CanResize
    then
...
begin
  case AKey of
    ...
    VK_PRIOR, VK_NEXT:
      begin
        ...
        SetSelectionInc(0, Navigation[AKey = VK_NEXT] *
          ViewData.RowsPerPage, AShift);
      end;
    VK_HOME, VK_END:
      if ssCtrl in AShift then
        SetSelection(FocusedCell.X, MaxInt * Navigation[AKey = VK_END],
          AShift - [ssCtrl])
      else
  ...
  case AKey of
    VK_UP, VK_DOWN, VK_LEFT, VK_RIGHT, VK_PRIOR, VK_NEXT, VK_HOME,
      VK_END, VK_TAB:
      begin
...
procedure TcxPivotGridController.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      AEditViewInfo := (HitTest.HitObject as TcxPivotGridEditContainerViewInfo)
        .FEditViewInfo;
      if (AEditViewInfo <> nil) and AEditViewInfo.Calculated then
        TcxCustomEditViewInfoAccess(AEditViewInfo).InplaceMouseDown(Button,
          Shift, X, Y);
    end;
    if HitTest.HitAtPrefilter and not PrefilterOpenedBeforeClick and not IsDesigning
    then
    begin
      if TcxPivotGridPrefilterViewInfo(HitTest.HitObject)
        .SetPressed(HitTest, True) then
        PivotGrid.InvalidateRect
          (TcxPivotGridPrefilterViewInfo(HitTest.HitObject).Bounds, False);
      if HitTest.HitAtPrefilterDropDownButton or
        HitTest.HitAtPrefilterCaption and ViewInfo.Prefilter.CanMRUPopupShow
      then
        FPrefilterPopup.Popup;
    end;
  end;
  PrefilterOpenedBeforeClick := False;
  if FIgnoreSelection or
    ((HitTest.HitAtDataCell and
    IsCellSelected(HitTest.HitObject as TcxPivotGridDataCellViewInfo) and
    (Button = mbRight))) then
    Exit;
...
procedure TcxPivotGridController.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  I: Integer;
...
begin
  ...
  if (DownField <> nil) and not HitTest.HitAtFilter and not FilterOpenedBeforeClick
  then
...
procedure TcxPivotGridController.FieldFilterPopup(AField: TcxPivotGridField;
  AInitPopupEvent: TNotifyEvent = nil);
var
...
begin
  if SelectionTimer.Enabled then
    Exit;
  ...
  begin
    if HitTest.HitAtDataCell then
      with TcxPivotGridDataCellViewInfo(HitTest.HitObject) do
        ViewData.AnchorCells := Rect(ColumnIndex, RowIndex,
          ColumnIndex, RowIndex)
    else if HitTest.HitAtGroupHeader and
      (HitTest.HitObject is TcxPivotGridHeaderCellViewInfo) then
    begin
      if TcxPivotGridHeaderCellViewInfo(HitTest.HitObject).Data is TcxPivotGridViewDataItem
      then
      begin
        ADataItem := TcxPivotGridHeaderCellViewInfo(HitTest.HitObject)
          .Data as TcxPivotGridViewDataItem;
        if (ADataItem.IsDataField and
          (ADataItem.Parent.GroupItem is TcxPivotGridColumnItem)) or
          (ADataItem.GroupItem is TcxPivotGridColumnItem) then
          ViewData.AnchorCells := Rect(ADataItem.GetChildLeftVisibleIndex, -1,
            ADataItem.GetChildRightVisibleIndex, -1)
        else
          ViewData.AnchorCells := Rect(-1, ADataItem.GetChildLeftVisibleIndex,
            -1, ADataItem.GetChildRightVisibleIndex);
      end;
    end
    else
...
begin
  ...
  begin
    ...
    if cxSizeIsEqual(Result, cxNullSize) then
      Result := ViewInfo.ScaleFactor.Apply
        (Size(cxPivotGridFilterPopupDefaultWidth,
        cxPivotGridFilterPopupDefaultHeight));
  end
  else
  begin
    Result.cx := Max(ViewInfo.ScaleFactor.Apply(cxPivotGridFilterPopupMinWidth),
      OptionsView.FilterDropDownWidth);
    Result.cy :=
      Max(ViewInfo.ScaleFactor.Apply(cxPivotGridFilterPopupMinHeight),
...
begin
  ...
  InflateRect(R, -cxPivotGridAutoScrollAreaWidth,
    -cxPivotGridAutoScrollAreaWidth);
  if not cxRectPtIn(R, Point(X, Y)) and SuspendSelectionTimer and HitTest.HitAtDataCell
  then
    StartSelectionTimer
  else
    StopSelectionTimer;
...
function TcxPivotGridController.CreateHintController
  : TcxPivotGridHintController;
begin
  Result := TcxPivotGridHintController.Create(Self);
end;

function TcxPivotGridController.CreateHotTrackController
  : TcxPivotGridHotTrackController;
begin
  Result := TcxPivotGridHotTrackController.Create(Self)
end;

procedure TcxPivotGridController.DoFieldHeaderClick(AField: TPersistent;
  AShift: TShiftState);
var
...
begin
  ...
  if PivotGrid.IsDesignerAvailable and (AField is TcxPivotGridField) or
    (AField = nil) then
    DesignerHelper.Select(AField, AShift);
  if Supports(AField, IcxPivotGridField, DownField) and not HitTest.HitAtFilter
  then
  begin
...
begin
  ...
  else if HitTest.HitObject is TcxPivotGridEditContainerViewInfo then
  begin
    AEditViewInfo := TcxPivotGridEditContainerViewInfo(HitTest.HitObject)
      .FEditViewInfo;
    if (AEditViewInfo <> nil) and AEditViewInfo.Calculated then
      Result := TcxCustomEditViewInfoAccess(AEditViewInfo)
        .GetCurrentCursor(HitTest.HitPoint);
  end;
end;

function TcxPivotGridController.GetDragAndDropObjectClass
  : TcxDragAndDropObjectClass;
begin
  if HitTest.HitAtDataField or HitTest.HitAtField then
    Result := TcxPivotGridDragAndDropObject
  else if HitTest.HitAtHorzSizingEdge then
...
procedure TcxPivotGridController.MakeSelected(AShift: TShiftState;
  AMouseMove: Boolean = False);
var
  AObject: TObject;
begin
  if FIgnoreSelection then
    Exit;
  if (HitTest.HitAtDataCell and
    (HitTest.HitObject is TcxPivotGridDataCellViewInfo)) then
  ...
  begin
    if HitTest.HitAtGroupHeader and
      (HitTest.HitObject is TcxPivotGridHeaderCellViewInfo) and not FIgnoreSelection
    then
    begin
    ...
    else if HitTest.HitAtDataCell and
      (HitTest.HitObject is TcxPivotGridDataCellViewInfo) then
...
begin
  ...
  else if PivotGrid.OptionsBehavior.FocusCellOnCycle then
  begin
...
begin
  ...
  begin
    if PivotGrid.IsFocused and ProcessNavigationByStep(AGoBackward, []) then
      PostMessage(GetParentForm(PivotGrid).Handle, WM_NEXTDLGCTL,
        WPARAM(AGoBackward), 0);
  end;
end;
...
begin
  ...
  if APos.X < R.Left then
    DX := -1
  else if APos.X > R.Right then
    DX := 1;
  if APos.Y < R.Top then
    DY := -1
  else if APos.Y > R.Bottom then
    DY := 1;
...
procedure TcxPivotGridController.SetSelection(ACol, ARow: Integer;
  AShift: TShiftState);
var
  R: TRect;
begin
  ViewData.AdjustCellIndexes(ARow, ACol, (ssLeft in AShift) or
    (ssRight in AShift));
  if PivotGrid.OptionsSelection.MultiSelect and
    ((ssCtrl in AShift) or (ssLeft in AShift) or (ssShift in AShift)) then
    with cxRect(Point(ACol, ARow), ViewData.AnchorCells.TopLeft) do
      R := Rect(Min(Left, Right), Min(Top, Bottom), Max(Left, Right),
        Max(Top, Bottom))
  else
...
procedure TcxPivotGridController.SetSelectionInc(const DX, DY: Integer;
  AShift: TShiftState; AByTimer: Boolean = False);
var
...
begin
  ...
  ViewData.AdjustCellIndexes(ARow, AColumn, (ssLeft in AShift) or
    (ssRight in AShift));
...
function TcxPivotGridController.GetOptionsCustomize
  : TcxPivotGridOptionsCustomize;
begin
  Result := PivotGrid.OptionsCustomize;
end;

function TcxPivotGridController.GetOptionsSelection
  : TcxPivotGridOptionsSelection;
begin
...
function TcxPivotGridOptionsDataField.CanDrop
  (Area: TcxPivotGridFieldArea): Boolean;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if AField.Visible or (AField.AreaIndex = -1) or (AField.Area <> AArea)
      then
        Continue;
...
procedure TcxPivotGridOptionsDataField.DragDrop(AArea: TcxPivotGridFieldArea;
  AIndex: Integer);
var
  ANewArea: TcxPivotGridDataFieldArea;
begin
  if not(AArea in [faRow, faColumn]) then
    Exit;
  PivotGrid.BeginUpdate;
  try
...
function TcxPivotGridOptionsDataField.GetActualAreaIndex(AInHeaderArea
  : Boolean = True): Integer;
begin
  Result := -1;
  if (Area = dfaNone) and AInHeaderArea then
    Exit;
  if Area = dfaRow then
  begin
  ...
  begin
    if AInHeaderArea and not PivotGrid.OptionsView.ColumnFields then
      ...
      Result := CheckIndex(AreaIndex, PivotGrid.DataBuilder.ColumnFields,
        faColumn);
  end;
end;
...
begin
  if not PivotGrid.OptionsView.IsCompactLayout or (Area <> dfaRow) or
    (csLoading in PivotGrid.ComponentState) then
    Exit;
  AreaIndex := DataBuilder.RowFields.Count;
end;

function TcxPivotGridOptionsDataField.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
...
begin
  ...
  Result := Max(Result, ViewInfo.ScaleFactor.Apply
...
procedure TcxPivotGridOptionsDataField.SetArea
  (AValue: TcxPivotGridDataFieldArea);
begin
...
begin
  ...
  begin
    ...
    if (AValue <> -1) and (Area <> dfaNone) then
      if not PivotGrid.SetFieldAreaIndex(Self, DataAreaToFieldArea[Area], AValue)
      then
        FAreaIndex := AValue;
...
function TcxPivotGridOptionsDataField.GetViewInfo
  : TcxPivotGridFieldHeaderCellViewInfo;
begin
  Result := ViewInfo;
end;
...
procedure TcxPivotGridFieldFilter.SetFilterType(AFilterType
...
begin
  ...
  SizingBounds.Left := CurMousePos.X - SizableObject.GetActualWidth +
    SizableObject.GetMinWidth;
  inherited BeginDragAndDrop;
end;
...
procedure TcxPivotGridResizingObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
begin
  Accepted := True;
  SizeCursorPos := P.X;
...
begin
  Result := cxRectSetWidth(SizingBounds,
    Min(Max(SizingBounds.Left, StartPos + SizeDelta + StartPosDelta),
    SizingBounds.Right) - 1, cxPivotGridSizeMarkWidth);
end;
...
begin
  SizableObject.SetSizeDelta(ASign[PivotGrid.ViewInfo.UseRightToLeftAlignment]
    * ADelta);
  PivotGrid.Modified;
end;
...
procedure TcxPivotGridDragDropAreaInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  Bounds := TdxRightToLeftLayoutConverter.ConvertRect(Bounds, AClientBounds);
  DisplayBounds := TdxRightToLeftLayoutConverter.ConvertRect(DisplayBounds,
    AClientBounds);
end;
...
begin
  ...
  ARect.Left := ARect.Left - PopupWindow.ViewInfo.GetClientExtent.Left -
    ScaleFactor.Apply(cxTextOffset) - 1;
  BoundsRect := ARect;
  InitValues;
  if PivotGrid.OptionsPrefilter.MRUItemsListDropDownCount = 0 then
    ...
    Properties.DropDownRows :=
      PivotGrid.OptionsPrefilter.MRUItemsListDropDownCount;
  Visible := True;
end;
...
begin
  inherited DoCloseUp;
  PivotGrid.Controller.PrefilterOpenedBeforeClick :=
    (PivotGrid.HitTest.HitAtPrefilterDropDownButton or
...
function TcxPivotGridMRUPrefilterPopup.SelectedMRUItem
  : TcxPivotGridFilterMRUItem;
begin
...
procedure TcxPivotGridFilterPopupListBox.DrawItemText(const AText: string;
  const ARect: TRect);
const
  AFlags: array [Boolean] of Cardinal = (CXTO_LEFT, CXTO_RIGHT or
    CXTO_RTLREADING);
var
  ASelStart: Integer;
  ASearchText: string;
...
begin
  ...
  begin
    ...
    cxTextOut(InnerCheckListBox.Canvas.Canvas, AText, R,
      AFlags[UseRightToLeftAlignment] or CXTO_CENTER_VERTICALLY or
      CXTO_SINGLELINE, ASelStart, Length(ASearchText),
      InnerCheckListBox.Canvas.Font, clHighlight, clHighlightText, 0, 0, 0,
      InnerCheckListBox.Canvas.Font.Color);
  end
  else
...
begin
  if Key = VK_SPACE then
    FPopup.Values.Items[ItemIndex].Checked := not FPopup.Values.Items
      [ItemIndex].Checked
  else
    inherited DoKeyDown(Key, Shift);
end;

function TcxPivotGridFilterPopupIncrementalFilteringHelper.GetItemCount
  : Integer;
begin
  Result := FPopup.Values.Count;
end;

function TcxPivotGridFilterPopupIncrementalFilteringHelper.GetItemIndex
  : Integer;
begin
  Result := FPopup.Values.ItemIndex;
end;

function TcxPivotGridFilterPopupIncrementalFilteringHelper.GetLookAndFeelPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := FPopup.PivotGrid.LookAndFeelPainter;
end;

function TcxPivotGridFilterPopupIncrementalFilteringHelper.GetMouseWheelHandler
  : TcxCustomInnerListBox;
begin
  Result := FPopup.Values.InnerCheckListBox;
end;

function TcxPivotGridFilterPopupIncrementalFilteringHelper.
...
begin
  ...
  SearchEdit.Margins.SetBounds(GetSearchEditOffsets.Left,
    GetSearchEditOffsets.Top, GetSearchEditOffsets.Right,
    GetSearchEditOffsets.Bottom);
  SearchEdit.AlignWithMargins := True;
end;

procedure TcxPivotGridFilterPopupIncrementalFilteringHelper.
  SearchEditValueChanged(Sender: TObject);
begin
...
function TcxPivotGridFilterPopupWindowViewInfo.GetSearchInfoPanelTextDrawFlags
  : Cardinal;
begin
  Result := CXTO_CENTER_VERTICALLY or CXTO_SINGLELINE or
    CXTO_CENTER_HORIZONTALLY;
  if UseRightToLeftAlignment then
    Result := Result or CXTO_RTLREADING;
end;

function TcxPivotGridFilterPopupWindowViewInfo.
  IsSearchInfoPanelInClientRect: Boolean;
begin
  Result := True;
end;
...
function TcxPivotGridFilterPopupWindow.GetViewInfoClass
...
begin
  ...
  ViewInfo.SearchInfoPanelHeight := ClientHeight -
    (Edit as TcxPivotGridFilterPopup).FIncrementalFilteringHelper.SearchEdit.
    Height - AClientExtent.Top - AClientExtent.Bottom;
...
class function TcxPivotGridFilterPopupProperties.GetPopupWindowClass
  : TcxCustomEditPopupWindowClass;
begin
...
class function TcxPivotGridFilterPopup.GetPropertiesClass
...
procedure TcxPivotGridFilterPopup.AddValue(const AValue
...
begin
  if Field.UseEditProperties and (Field.SummaryVariation = svNone) and
    (Field.Area <> faData) and Field.IsDefaultGroupValues then
  ...
  if AValue.FUnUsed then
    ...
    ACheckItem.State := TcxCheckBoxState
      (Ord(not Filter.HasFilter or Filter.Contains(AValue.Value)));
...
begin
  ...
  CalculateButtons;
  PopupWindow.MinSysPanelHeight :=
    Max(ScaleFactor.Apply(cxPivotGridFilterMinSysPanelHeight),
    FCancelButton.Height + (FCancelButton.Height div 3));
  ActiveProperties.PopupMinWidth :=
    Max(ScaleFactor.Apply(cxPivotGridFilterPopupMinWidth),
    ((FCancelButton.Height div 3) + FCancelButton.Width) * 2 +
    GetSystemMetrics(SM_CXVSCROLL) + FCancelButton.Height div 2);
...
begin
  ...
  UpdateButtonCaptions;
  FIncrementalFilteringHelper :=
    TcxPivotGridFilterPopupIncrementalFilteringHelper.Create(Self);
...
begin
  ...
  if (FCloseUpReason <> crCancel) and (ShowAllState <> cbsUnchecked) and FilterModified
  then
    SaveChanges;
  Filter.WindowSize := cxSize(PopupWindow.Width, PopupWindow.Height);
  if PivotGrid.HitTest.HitAtCustomizationForm then
    PivotGrid.Controller.Update;
  PivotGrid.Controller.FilterOpenedBeforeClick := (FCloseUpReason = crUnknown)
    and PivotGrid.HitTest.HitAtFilter and (PivotGrid.HitTest.Field = Field);
  Visible := False;
...
begin
  Result := FValues.GetHeight(AItemCount) +
    PopupWindow.ViewInfo.GetSysPanelHeight + ScaleFactor.Apply
    (cxPivotGridDoubleSpace);
end;
...
function TcxPivotGridFilterPopup.GetStateByCount(ACount: Integer)
  : TcxCheckBoxState;
begin
  if ACount = 0 then
    Result := cbsUnchecked
  else if ACount = FOriginalValues.Count then
    Result := cbsChecked
  else
    Result := cbsGrayed;
end;
...
procedure TcxPivotGridFilterPopup.InitValues;
  ...
  function CheckItemFiltered(const AItemCaption: string): Boolean;
  var
    APos: Integer;
  begin
    APos := Pos(AnsiUpperCase(GetSearchText), AnsiUpperCase(AItemCaption));
    Result := (APos = 1) or (APos <> 0) and
      (ifoUseContainsOperator in IncrementalFilteringOptions);
  end;
...
begin
  ...
  if (not UseRightToLeftAlignment and (PopupWindow.ViewInfo.SizeGripCorner
    in [coTopRight, coBottomRight])) or
    (UseRightToLeftAlignment and (PopupWindow.ViewInfo.SizeGripCorner
    in [coTopLeft, coBottomLeft])) then
  begin
  ...
  AOriginalBounds := cxRectBounds(PopupWindow.Width - ADelta,
    (R.Bottom + R.Top - AHeight) div 2 -
    Integer(PopupWindow.ViewInfo.SizeGripCorner in [coBottomLeft, coBottomRight]
    ), AWidth, AHeight);
  ABounds := AOriginalBounds;
  if UseRightToLeftAlignment then
  begin
    FCancelButton.Anchors := TdxRightToLeftLayoutConverter.ConvertAnchors
      (FCancelButton.Anchors);
    ABounds := TdxRightToLeftLayoutConverter.ConvertRect(ABounds,
      PopupWindow.ClientBounds);
  end;
  FCancelButton.SetBounds(ABounds.Left, ABounds.Top, AWidth, AHeight);

  if FOkButton <> nil then
  begin
    ABounds := cxRectBounds(AOriginalBounds.Left - (AWidth + (AHeight div 3)),
      AOriginalBounds.Top, AWidth, AHeight);
    if UseRightToLeftAlignment then
      ABounds := TdxRightToLeftLayoutConverter.ConvertRect(ABounds,
        PopupWindow.ClientBounds);
...
begin
  ...
  if (AShowAllCheckItem <> nil) and (GetShowAllCheckItem.State <> FShowAllState)
  then
...
function TcxPivotGridFilterPopup.IncrementalFilteringOptions
  : TcxTextEditIncrementalFilteringOptions;
begin
...
begin
  if Source is TcxPivotGridFieldOptions then
    ...
    begin
      ...
      Self.FFilteringPopupIncrementalFiltering :=
        FFilteringPopupIncrementalFiltering;
      Self.FFilteringPopupIncrementalFilteringOptions :=
        FFilteringPopupIncrementalFilteringOptions;
    end
  else
    inherited Assign(Source);
...
begin
  Result := Field.PivotGrid.OptionsCustomize.Filtering and Filtering;
end;

function TcxPivotGridFieldOptions.CanSorting: Boolean;
begin
  Result := Field.PivotGrid.OptionsCustomize.Sorting and Sorting and
    (Field.Area in [faColumn, faRow]);
end;
...
procedure TcxPivotGridFieldOptions.SetOption(var AOption: Boolean;
  ANewValue: Boolean);
begin
...
procedure TcxPivotList.Assign(AListA: TList; AOperator: TListAssignOp = laCopy;
  AListB: TList = nil);
var
  I: Integer;
  AItem: Pointer;
...
function TcxPivotGridCustomDragDropObject.GetOptionsView
  : TcxPivotGridOptionsView;
begin
  Result := PivotGrid.OptionsView;
end;

function TcxPivotGridCustomDragDropObject.GetPainter
  : TcxCustomLookAndFeelPainter;
begin
...
begin
  with TcxPivotGridFieldDataBinding(Owner) do
    Result := PivotGrid.DataController.IsDisplayFormatDefined(Field.Index,
      not AIsCurrencyValueAccepted);
end;
...
function TcxPivotGridFieldDataBinding.GetDefaultValuesProvider
...
function TcxPivotGridFieldDataBinding.GetDefaultValuesProviderClass
...
procedure TcxPivotGridFieldDataBinding.SetValueTypeClass
...
procedure TcxPivotGridCustomTotal.SetSummaryType
...
function TcxPivotGridCustomTotalCollection.Add(ASummaryType
  : TcxPivotGridSummaryType): TcxPivotGridCustomTotal;
begin
  PivotGrid.BeginUpdate;
  try
...
function TcxPivotGridCustomTotalCollection.GetItem(AIndex: Integer)
  : TcxPivotGridCustomTotal;
begin
...
procedure TcxPivotGridCustomTotalCollection.SetItem(AIndex: Integer;
  AValue: TcxPivotGridCustomTotal);
begin
...
function TcxPivotGridFieldSortCondition.IsEqual(AField: TcxPivotGridField;
  const AValue: Variant): Boolean;
begin
...
constructor TcxPivotGridFieldSortConditionCollection.Create
  (AOwner: TcxPivotGridSortBySummaryInfo);
begin
  inherited Create(TcxPivotGridFieldSortCondition);
  FOwner := AOwner;
end;

function TcxPivotGridFieldSortConditionCollection.Add(AField: TcxPivotGridField;
  const AValue: Variant): TcxPivotGridFieldSortCondition;
begin
...
function TcxPivotGridFieldSortConditionCollection.FindItem
  (AParent: TcxPivotGridGroupItem): TcxPivotGridGroupItem;
var
  AValue: Variant;
  ACount: Integer;
begin
  ...
  ACount := 0;
  repeat
    if (Result.ItemCount > 0) and
      (not GetValueByField(Result.Items[0].Field, AValue) or
...
procedure TcxPivotGridFieldSortConditionCollection.Update
  (Item: TCollectionItem);
begin
  SortBySummaryInfo.Changed;
end;

function TcxPivotGridFieldSortConditionCollection.GetItem(AIndex: Integer)
  : TcxPivotGridFieldSortCondition;
begin
  Result := TcxPivotGridFieldSortCondition(inherited Items[AIndex]);
end;

procedure TcxPivotGridFieldSortConditionCollection.SetItem(AIndex: Integer;
  AValue: TcxPivotGridFieldSortCondition);
begin
  Items[AIndex].Assign(AValue);
end;
...
begin
  ...
  begin
    ...
    Add(TcxPivotGridPrefilterViewInfo.Create(Self, APivotGridViewInfo.Painter,
      APivotGridViewInfo.ScaleFactor, AClientBounds, AClientBounds,
      APivotGridViewInfo.BaseStyles.GetPrefilterParams));
...
begin
  ...
  Result := (AVisible = pfvAlways) or
    ((AVisible = pfvNonEmpty) and not Filter.IsEmpty);
end;
...
const
  AImageIndexes: array [TcxPivotGridSummaryType] of Integer = (0, 1, 2, 3, 4,
    -1, -1, -1, -1, -1, -1);
begin
  inherited CreateItems;
  if (Field.Area = faData) and ((fpmiSummaryType in Items) and
    (SummaryItems <> [])) then
  begin
    AItem := CreateSubItem(Root, cxGetResourceString(@scxSummaryType), -1,
      Field <> nil);
    for ASummaryType in SummaryItems do
    begin
      ASubItem := CreateSubItem(AItem,
        Trim(Format(cxGetResourceString(TotalDescriptions[ASummaryType]), [' '])
        ), pgcmSummaryTypeBase + Byte(ASummaryType), True,
        Field.SummaryType = ASummaryType);
  ...
  begin
    ...
    CreateSubItem(AItem, cxGetResourceString(@scxMoveToBeginning),
      pgcmMoveToBeginning, (AFirst < AIndex) and
      PivotGrid.OptionsCustomize.Moving);
    CreateSubItem(AItem, cxGetResourceString(@scxMoveToEnd), pgcmMoveToEnd,
      (ALast > AIndex) and PivotGrid.OptionsCustomize.Moving);
    CreateSubItem(AItem, cxGetResourceString(@scxMoveToLeft), pgcmMoveToLeft,
      (AFirst < AIndex) and PivotGrid.OptionsCustomize.Moving);
    CreateSubItem(AItem, cxGetResourceString(@scxMoveToRight), pgcmMoveToRight,
      (ALast > AIndex) and PivotGrid.OptionsCustomize.Moving);
  end;
  // field list
  ...
  begin
    ...
    with PivotGrid.Customization do
    begin
      ASubItem := CreateSubItem(Root,
        cxGetResourceString(scxCustomization[Visible]),
        CustomizationCommand[Visible],
        PivotGrid.OptionsCustomize.QuickCustomization);
      if FBuiltInMenu.Images = InternalMenuImages then
      ...
      begin
        ...
        if Field <> nil then
          ...
          OptionsDataField.AreaIndex :=
            AIndexes[ACommand - pgcmMoveToBeginning];
      end;
    pgcmSummaryTypeBase .. pgcmSummaryTypeBase +
      Byte(High(TcxPivotGridSummaryType)):
      Field.SummaryType := TcxPivotGridSummaryType
        (ACommand - pgcmSummaryTypeBase);
  else
    inherited DoExecute(ACommand);
  end;
end;

procedure TcxPivotGridFieldHeaderMenu.GetIndexes(var AStart, APrev, ANext,
  AFinish: Integer);
  ...
  function SetPosValue(var APos: Integer;
    const AValue, ADefValue: Integer): Integer;
  begin
    Result := AValue;
  ...
  begin
    Inc(AIndex, AInc);
    if (AIndex > 0) and (AIndex < FieldHeaders.Count) and
      (GetCell(AIndex).Group <> nil) then
...
begin
  ...
  ACurIndex := AIndex;
  while (ACurIndex < FieldHeaders.Count) and
    (GetCell(ACurIndex).Area = FieldViewInfo.Area) do
  begin
...
function TcxPivotGridFieldHeaderMenu.GetFieldViewInfo
  : TcxPivotGridFieldHeaderCellViewInfo;
begin
  if Field <> nil then
...
function TcxPivotGridFieldHeaderMenu.GetOptionsDataField
  : TcxPivotGridOptionsDataField;
begin
...
const
  ExpandCollapse: array [Boolean] of Integer = (pgcmExpand, pgcmCollapse);
begin
  inherited CreateItems;
  ...
  begin
    ...
    begin
      CreateSubItem(Root,
        cxGetResourceString(scxExpandCollapse[GroupItem.Expanded]),
        ExpandCollapse[GroupItem.Expanded]);
      CreateSeparator(Root);
    end;
    if vpmiExpandAll in Items then
      CreateSubItem(Root, cxGetResourceString(@scxExpandAll), pgcmExpandAll)
        .ImageIndex := IfThen(FBuiltInMenu.Images = InternalMenuImages, 7, -1);

    if vpmiCollapseAll in Items then
      CreateSubItem(Root, cxGetResourceString(@scxCollapseAll), pgcmCollapseAll)
        .ImageIndex := IfThen(FBuiltInMenu.Images = InternalMenuImages, 8, -1);
  end;
  if PivotGrid.OptionsCustomize.Sorting and PivotGrid.OptionsCustomize.SortingByGroupValues
  then
    AddSortingItems;
end;
  ...
  case ACommand of
    ...
    begin
      ...
      begin
        PivotGrid.BeginUpdate;
        try
          TcxPivotGridGroupValueMenuSortByValueCriterion(SortCriteria[ACommand])
            .DoExecute;
          if not(ssShift in ShiftState) then
            RemoveSortingConditions
              (TcxPivotGridGroupValueMenuSortByValueCriterion
              (SortCriteria[ACommand]).Field);
        finally
          PivotGrid.EndUpdate;
        end;
      end
...
begin
  FViewDataItem := (HitTest.HitObject as TcxPivotGridHeaderCellViewInfo)
    .Data as TcxPivotGridViewDataItem;
...
procedure TcxPivotGridGroupValueMenu.RemoveSortingConditions
  (AExceptField: TcxPivotGridField);
var
  I: Integer;
begin
  for I := PivotGrid.SortedByGroupValueFields.Count - 1 downto 0 do
    if PivotGrid.SortedByGroupValueFields[I] <> AExceptField then
      TcxPivotGridField(PivotGrid.SortedByGroupValueFields[I])
        .SortBySummaryInfo.CancelSorting;
  if AExceptField <> nil then
    Exit;
...
begin
  ...
  if ViewDataItem.ItemCount <> 0 then
    Exit;
  SortCriteria.Clear;
  ...
  if DataBuilder.DataFields.Count = 0 then
    Exit;
  AArea := faRow;
    ...
    for I := 0 to AFields.Count - 1 do
      if ADataField = nil then
        for J := 0 to DataBuilder.DataFields.Count - 1 do
          AddSortByValueMenuItem(AFields[I], DataBuilder.DataFields[J],
            ADataGroup, AArea, True)
      else
        AddSortByValueMenuItem(AFields[I], ADataField, ADataGroup,
          AArea, False);
    if FHasSortedItems then
    begin
      CreateSeparator(Root);
      CreateSubItem(Root, cxGetResourceString(@scxRemoveAllSorting),
        pgcmRemoveAllSorting);
    end;
  end;
end;

procedure TcxPivotGridGroupValueMenu.AddSortByValueMenuItem(AField,
  ADataField: TcxPivotGridField; const AGroupItem: TcxPivotGridGroupItem;
  AArea: TcxPivotGridFieldArea; ANeedSuffix: Boolean);
var
  S: string;
  AMenuItem: TMenuItem;
begin
  if not AField.Options.CanSorting then
    Exit;
  S := AField.Caption;
  if ANeedSuffix then
  ...
  AMenuItem := CreateSubItem(Root, S, pgcmSortByGroupValue +
    SortCriteria.Count);
  SortCriteria.Add(TcxPivotGridGroupValueMenuSortByValueCriterion.Create(AField,
    ADataField, AGroupItem));
  AMenuItem.Checked := AField.SortBySummaryInfo.ConditionDefined(ADataField,
    AGroupItem);
...
procedure TcxPivotGridPopupMenus.DoOnClick(AItem: TMenuItem;
...
function TcxPivotGridPopupMenus.CreateFieldHeaderMenu
  : TcxPivotGridFieldHeaderMenu;
begin
  Result := TcxPivotGridFieldHeaderMenu.Create(Self);
end;

function TcxPivotGridPopupMenus.CreateGroupValueMenu
  : TcxPivotGridGroupValueMenu;
begin
  Result := TcxPivotGridGroupValueMenu.Create(Self);
end;

function TcxPivotGridPopupMenus.CreateHeaderAreaMenu
  : TcxPivotGridHeaderAreaMenu;
begin
...
procedure TcxPivotGridPopupMenus.DoOnPopup(ASender: TcxPivotGridCustomPopupMenu;
  var AHandled: Boolean);
begin
  AHandled := False;
...
begin
  ...
  else if HitTest.HitAtGroupHeader and (HitTest.GroupItem <> nil) and
    GroupValueMenu.Initialize then
    Result := GroupValueMenu.Popup(P.X, P.Y)
  else if HitTest.HitAtHeaderArea then
...
procedure TcxPivotGridPopupMenus.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
var
  AWidth, I: Integer;
begin
  if not VisibleInGroup or (Area = faFilter) then
    Exit;
  PivotGrid.CheckChanges;
  PivotGrid.ShowHourglassCursor;
  try
    ...
    begin
      AWidth := MinWidth;
      if (Area = faRow) or
        ((Area = faData) and (PivotGrid.OptionsDataField.Area = dfaRow)) then
      begin
        AWidth := Max(ViewInfo.MeasureWidth + ScaleFactor.Apply
          (cxPivotGridDoubleSpace), PivotGrid.ViewData.FRows.MeasureWidth(Self,
          False) + ScaleFactor.Apply(cxTextOffset))
      end
      else
        with PivotGrid.ViewData do
        begin
          AWidth := Max(AWidth, FColumns.MeasureWidth(Self, True) +
            ScaleFactor.Apply(cxTextOffset));
          if Area = faColumn then
          begin
            for I := 0 to PivotGrid.SummaryFields.Count - 1 do
              AWidth := Max(AWidth,
                CalculateDataWidth(PivotGrid.SummaryFields[I]) +
                ScaleFactor.Apply(cxTextOffset));
          end
          else
            AWidth := Max(AWidth, CalculateDataWidth(Self) +
              ScaleFactor.Apply(cxTextOffset));
        end;
      Width := AWidth;
      PivotGrid.DataChanged;
...
begin
  ...
  try
    ...
    if AGroupInterval in [giYearAge, giMonthAge, giWeekAge, giDayAge, giNumeric]
    then
    begin
...
function TcxPivotGridField.GetGroupValueDisplayText(const AGroupValue
  : Variant): string;
begin
  try
    case GroupInterval of
      ...
      giDateQuarter:
        Result := Format(cxGetResourceString(@scxQuarterFormat),
          [Integer(AGroupValue)]);
...
procedure TcxPivotGridField.SetAreaPosition(AArea: TcxPivotGridFieldArea;
  AAreaIndex: Integer);
begin
  PivotGrid.BeginUpdate;
  try
...
procedure TcxPivotGridField.AssignAreaIndex(AArea: TcxPivotGridFieldArea;
  AIndex: Integer);
begin
  FArea := AArea;
  FAreaIndex := AIndex;
...
begin
  ...
  begin
    if VarIsStr(AItem1.Value) and VarIsStr(AItem2.Value) then
      Result := PivotGrid.OptionsData.CompareAsString(AItem1.Value,
        AItem2.Value)
    else
      Result := VarCompare(AItem1.Value, AItem2.Value);
...
function TcxPivotGridField.CreateEditStyle(AProperties: TcxCustomEditProperties)
  : TcxEditStyle;
begin
...
begin
  Result := GetDefaultEditDataRepositoryItems.GetItem
...
begin
  ACell.FDisplayText := FormatDisplayValue(ACell.Value, ACell.DisplayFormat,
    DataBinding.IsCurrencyField, (ACell.Total = nil) and
    (SummaryVariation in [svPercent, svPercentOfColumn, svPercentOfRow]),
...
begin
  Result := -1;
  if (PivotGrid.GroupHeaderImages <> nil) and Assigned(FOnGetGroupImageIndex)
  then
  begin
...
function TcxPivotGridField.DoGetGroupValue(const AValue: Variant;
...
function TcxPivotGridField.DoGetProperties
  (ACell: TcxPivotGridCustomCellViewInfo): TcxCustomEditProperties;
begin
...
procedure TcxPivotGridField.DoGetTotalDisplayText
  (AGroupItem: TcxPivotGridGroupItem; var AText: string);
begin
  if Assigned(FOnGetTotalDisplayText) then
    FOnGetTotalDisplayText(Self, AGroupItem, AText);
end;

procedure TcxPivotGridField.DragDrop(AArea: TcxPivotGridFieldArea;
  AAreaIndex: Integer);
var
...
begin
  ...
  if (Area = AArea) and (AAreaIndex > ViewInfo.AreaIndex) and (AAreaIndex > 0)
    and Visible then
    Dec(AAreaIndex);
  PivotGrid.BeginUpdate;
  try
    ...
    begin
      ...
      if ((AArea <> Group.Area) or not APrevVisible) and
        (AAreaIndex > ViewInfo.AreaIndex) then
...
function TcxPivotGridField.GetCellViewInfoClass
  : TcxPivotGridDataCellViewInfoClass;
begin
...
function TcxPivotGridField.IsEqual(const AStructure
  : TcxPivotGridOLAPStructureNode): Boolean;
begin
...
var
  ...
  AIndex, I: Integer;
begin
  if FGroupValuesValid then
    Exit;
  FGroupValueList.Clear;
  if PivotGrid.OLAPDataSource <> nil then
    PivotGrid.OLAPDataSource.PopulateGroupValues(TcxPivotGridOLAPField(Self),
      FGroupValueList)
  else
  begin
    ...
    begin
      ...
      begin
        FUnUsed := FUnUsed or
          ((PivotGrid.DataBuilder.FilteredIndexes[I] <
          cxPivotGridRecordInvisible) or
          ((PivotGrid.DataBuilder.FilteredIndexes[I]
          = cxPivotGridRecordInvisible) and Filter.Contains(AValue)));
      end;
    end;
...
procedure TcxPivotGridField.InitProperties(AProperties
  : TcxCustomEditProperties);
begin
...
var
  AIndex: Integer;
begin
  Result := Options.AlwaysExpanded or
    ((ExpandingInfo.Count <> 0) and ExpandingInfo.HashedIndexOf
    (AGroup.GetUniqueValue, AIndex));
end;
...
procedure TcxPivotGridField.EditViewDataGetDisplayTextHandler
...
function TcxPivotGridField.CreateCustomTotals
  : TcxPivotGridCustomTotalCollection;
begin
...
function TcxPivotGridField.CreateSortBySummaryInfo
  : TcxPivotGridSortBySummaryInfo;
begin
...
procedure TcxPivotGridField.GroupCheckExpanding(AGroup: TcxPivotGridGroupItem);
begin
...
begin
  try
    if not PivotGrid.OptionsData.SaveExpanding then
      Exit;
...
begin
  ...
  else if PivotGrid.IsOLAPActive then
    Result := varUString
  else
    Result := varUnknown;
end;
...
begin
  Result := not Filter.HasFilter or
    Filter.Contains(GetGroupValue(ARecordIndex));
end;

procedure TcxPivotGridField.SetAreaIndexInternal(AArea: TcxPivotGridFieldArea;
  AAreaIndex: Integer);
begin
  FArea := AArea;
...
procedure TcxPivotGridField.RepositoryItemPropertiesChanged
...
function TcxPivotGridField.GetProperties(AProperties: TStrings): Boolean;
var
  I: Integer;
begin
  for I := Low(FieldDefaultStoredProperties)
    to High(FieldDefaultStoredProperties) do
...
procedure TcxPivotGridField.GetPropertyValue(const AName: string;
  var AValue: Variant);
...
begin
  case GetPropertyIndex(AName, FieldDefaultStoredProperties) of
    6:
      AValue := GetFilterValue;
  end;
  if Assigned(OnGetStoredPropertyValue) then
    OnGetStoredPropertyValue(Self, AName, AValue);
end;

procedure TcxPivotGridField.SetPropertyValue(const AName: string;
  const AValue: Variant);
...
begin
  ...
  if not SortOrderAssigned and SortedBySummary then
    if (Area in [faRow, faColumn]) and
      (PivotGrid.OptionsBehavior.SortBySummaryDefaultOrder <> soNone) then
...
begin
  Result := FArea;
  if (Result in AllowedAreas) or (AllowedAreas = []) then
    Exit;
  for Result := faColumn to faData do
    if Result in AllowedAreas then
      Break;
end;
...
begin
  ...
  if Result and (DataAreaToFieldArea[PivotGrid.OptionsDataField.Area] = Area)
  then
...
begin
  Result := (Area in [faRow, faColumn]) and
    SortBySummaryInfo.ValidateProperties;
end;

function TcxPivotGridField.GetValueByRecordIndex(ARecordIndex: Integer)
  : Variant;
begin
  with PivotGrid.DataController do
  begin
    ARecordIndex := DataControllerInfo.GetInternalRecordIndex(ARecordIndex);
    if (Area <> faData) and (Properties <> nil) and
      (Properties.GetEditValueSource(False) = evsText) then
...
begin
  ...
  AValue := Max(-1, AValue);
  if (PivotGrid <> nil) and CanModifyArea and not(csDestroying in ComponentState)
  then
  begin
...
procedure TcxPivotGridField.SetDataBinding
  (AValue: TcxPivotGridFieldDataBinding);
begin
  FDataBinding.Assign(AValue);
end;

procedure TcxPivotGridField.SetDataVisibility
  (AValue: TcxPivotGridFieldDataVisibility);
begin
...
begin
  if (Group = AValue) or (csDestroying in PivotGrid.ComponentState) then
    Exit;
  PivotGrid.BeginUpdate;
  try
    if Group <> nil then
...
procedure TcxPivotGridField.SetGroupInterval(AValue: TcxPivotGridGroupInterval);
begin
...
begin
  ...
  begin
    FGroupIntervalItem := TcxPivotGridAdvancedDataGroupIntervalRepository.
      GetItemByID(AValue);
...
begin
  ...
  begin
    with PivotGrid.FieldList do
      Exchange(ValidateIndex(Index, 0, Count - 1),
        ValidateIndex(AValue, 0, Count - 1));
...
procedure TcxPivotGridField.SetOnGetProperties
  (AValue: TcxPivotGridGetPropertiesEvent);
begin
...
procedure TcxPivotGridField.SetPropertiesClass
...
procedure TcxPivotGridField.SetValueByRecordIndex(ARecordIndex: Integer;
  const AValue: Variant);
begin
  PivotGrid.DataController.Values
    [PivotGrid.DataController.GetRowInfo(ARecordIndex).RecordIndex, Index]
    := AValue;
end;
...
procedure TcxPivotGridField.InternalDoGetDisplayText
...
begin
  Result := inherited CanSorting and
    (TcxPivotGridOLAPField(Field).SortMode <> osmNone);
end;
...
begin
  ...
  if Result or not IsHierarchy then
    Exit;
...
class function TcxPivotGridOLAPField.PopulateFilteredValues
  (AField: TcxPivotGridField; IsExpanding: Boolean = False): TStrings;
var
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          for I := AField.Group.FieldCount - 1 downto AField.Group.IndexOf
            (AField) + 1 do
          begin
            AChildField := AField.Group.Fields[I];
            AField.PivotGrid.OLAPDataSource.PopulateFilteredUniqueNames
              (TcxPivotGridOLAPField(AChildField), AChildField.Filter,
              AUniqueValues);
          end;
        end;
...
begin
  ...
  if Structure <> nil then
    case Structure.FormatName of
      ...
      fnFixed:
        Result := cxPivotGridDefaultFieldFloatFormat;
      fnPercent:
        Result := cxPivotGridDefaultFieldPercentFormat;
      fnCustom:
        Result := Structure.FormatString;
    end;
  if Result = '' then
...
function TcxPivotGridOLAPField.GetCellViewInfoClass
  : TcxPivotGridDataCellViewInfoClass;
begin
...
function TcxPivotGridOLAPField.IsEqual(const AStructure
  : TcxPivotGridOLAPStructureNode): Boolean;
var
...
begin
  ...
  if ntKPI in AType then
    AFieldType := oftKPI
  else if ntSet in AType then
    AFieldType := oftSet
  else if ntMeasure in AType then
...
procedure TcxPivotGridOLAPField.SetOptions
  (AValue: TcxPivotGridOLAPFieldOptions);
begin
  Options.Assign(AValue);
end;

procedure TcxPivotGridOLAPField.SetSortMode
  (AValue: TcxPivotGridOLAPFieldSortMode);
begin
...
procedure TcxPivotGridOLAPField.SetStructure
  (AValue: TcxPivotGridOLAPStructureNode);
begin
...
begin
  if PivotGrid.IsOLAPActive then
    Result := PivotGrid.OLAPDataSource.CreateDrillDownDataSource(Self,
      PivotGrid.FieldList, 0)
  else
    Result := TcxPivotGridCrossCellDataSource.Create(Self);
end;

function TcxPivotGridCrossCell.GetCrossCellValue
  (AField: TcxPivotGridField): Variant;
begin
...
function TcxPivotGridCrossCell.GetSummaryByField(AField: TcxPivotGridField;
  ASummaryType: TcxPivotGridSummaryType): Variant;
begin
...
begin
  ...
  else if Column.Parent = nil then
    Result := Row.CreateSummaryRecords
  else if Row.Parent = nil then
  ...
  begin
    ...
    begin
      ...
      Result.CreateIntersection(Column.CreateSummaryRecords,
        PivotGrid.DataBuilder.GroupRecords);
    end
    else
    begin
      Column.InitializeRecords;
      Result.CreateIntersection(Row.CreateSummaryRecords,
        PivotGrid.DataBuilder.GroupRecords);
    end;
  end;
end;
...
function TcxPivotGridCrossCell.CreateCrossCellsList: TList;
  ...
  function GetSubItem(AItem: TcxPivotGridGroupItem; AIndex: Integer)
    : TcxPivotGridGroupItem;
  begin
    Result := AItem;
    if AItem.Expanded and (AItem.ItemCount > 0) then
...
procedure TcxPivotGridCrossCell.GetDataTypes(ADataField: TcxPivotGridField;
  var ASourceType, AFloatType: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      else if AType.GetVarType = varDate then
        ASourceType := varDate
      else
...
begin
  ...
  if not Result then
    Result := ((Row.ItemCount + Column.ItemCount) = 0) or
      (not Row.Expanded and not Column.Expanded);
end;

function TcxPivotGridCrossCell.GetSummaryCell(AIndex: Integer)
  : TcxPivotGridCrossCellSummary;
begin
...
function TcxPivotGridOLAPCrossCell.GetSummaryByField(AField: TcxPivotGridField;
  ASummaryType: TcxPivotGridSummaryType): Variant;
begin
  Result := Null;
  PivotGridError(AField.SummaryIndex <> cxPivotGridInvalidIndex,
...
procedure TcxPivotGridOLAPCrossCell.GetDataTypes(ADataField: TcxPivotGridField;
  var ASourceType, AFloatType: Integer);
begin
  inherited GetDataTypes(ADataField, ASourceType, AFloatType);
  if not IsCalculationAvailable then
    Exit;
  ASourceType := VarType(NativeValues[ADataField.SummaryIndex]);
  AFloatType := ASourceType;
  if not(VarTypeIsCurrency(ASourceType) and
    ADataField.DataBinding.IsCurrencyField) then
  begin
...
procedure TcxPivotGridOLAPCrossCell.SetNativeValue(AIndex: Integer;
  const AValue: Variant);
begin
...
constructor TcxPivotGridCrossCellSummary.Create(AOwner: TcxPivotGridCrossCell;
  ADataField: TcxPivotGridField);
begin
  FOwner := AOwner;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        AValue := AGetValueProc(AList.List[I],
          not DataField.IsDefaultGroupValues);
    ...
    begin
      ...
      try
        case DataField.SummaryVariation of
          ...
          svPercent:
            begin
              if VarIsNull(APrevValue) or
                (VarIsSoftNumeric(APrevValue) and (APrevValue = 0)) then
                FSummaryVariation := Null
              else
              begin
                FSummaryVariation := (AValue - APrevValue) * 100 /
                  Abs(APrevValue);
      ...
      case DataField.SummaryVariation of
        svPercentOfColumn:
          if Owner.Row.Parent <> nil then
            AOwnerValue := Owner.Row.Parent.GetCellByCrossItem(Owner.Column)
              .GetSummaryByField(DataField)
          else
            AOwnerValue := AValue;
        svPercentOfRow:
          if Owner.Column.Parent <> nil then
            AOwnerValue := Owner.Column.Parent.GetCellByCrossItem(Owner.Row)
              .GetSummaryByField(DataField)
          else
            AOwnerValue := AValue;
      end;
...
procedure TcxPivotGridCrossCellSummary.DoCalculateSummary(ARecords: TList;
  AGetValueProc: TcxPivotGridGetRecordValueProc = nil);
var
  ...
  begin
    ...
    begin
      ...
      if not AIsStringField or not Owner.UseRawData then
        ASourceValue := AGetValueProc(ARecords[I],
          not DataField.IsDefaultGroupValues)
      else
      begin
      ...
      if not(SummaryNullIgnore and (VarIsNull(ASourceValue) or
        ((AIsStringField or VarIsStr(ASourceValue)) and (ASourceValue = ''))))
      then // not null or empty
        Inc(ACount);
  ...
  begin
    ...
    if DataField.IsDefaultGroupValues and not AIsStringField and
      (ACount <> ANotNumericCount) then
    begin
    ...
    if AIsStringField or (ANotNumericCount = ARecords.Count) then
      Exit;
    SetSummaryByType(stSum, ASum, IfThen(ASourceType <> varDate, ASourceType,
      AFloatType));
    if ACount = 0 then
...
begin
  if not Assigned(AGetValueProc) then
    AGetValueProc := GetRecordValue;
  Clear;
  if not Owner.IsCalculationAvailable then
    Exit;
  AIsStringField := (DataField.DataBinding.ValueTypeClass <> nil) and
    DataField.DataBinding.ValueTypeClass.IsString;
  InitializeValues;
  BaseCalculation;
  if ACount <> 0 then
  begin
...
function TcxPivotGridCrossCellSummary.GetIntermediateValue(const ACell: Pointer;
  AIsGroupValue: Boolean): Variant;
begin
  Result := TcxPivotGridCrossCell(ACell).GetSummaryByField(FDataField);
end;

function TcxPivotGridCrossCellSummary.GetRecordValue(const ARecord: Pointer;
  AIsGroupValue: Boolean): Variant;
begin
...
begin
  if (APrevCell = nil) or (APrevCell.Owner.PrevCrossCell = nil) then
    ...
    Result := APrevCell.Owner.PrevCrossCell.SummaryCells
      [DataField.SummaryIndex];
end;

procedure TcxPivotGridCrossCellSummary.SetSummaryByIndex(AIndex: Integer;
  AValue: Variant);
begin
...
function TcxPivotGridCrossCellSummary.GetDataController
...
function TcxPivotGridCustomCustomizationForm.CalculateHitTest
    ...
    begin
      ...
      if (AControl <> nil) then
        if AControl is TcxFieldListListBox then
          AHitTest.BitState[htcFieldList] := TcxFieldListListBox(AControl)
            .CalculateHitTest(AHitTest)
        else if AControl.Parent is TcxFieldListListBox then
          AHitTest.BitState[htcFieldList] :=
            TcxFieldListListBox(AControl.Parent).CalculateHitTest(AHitTest)
        else if AControl is TcxPivotGridOLAPStructureInnerTreeView then
          AHitTest.BitState[htcFieldTreeView] :=
            TcxPivotGridOLAPStructureInnerTreeView(AControl)
            .CalculateHitTest(AHitTest)
        else if AControl.Parent is TcxPivotGridOLAPStructureInnerTreeView then
          AHitTest.BitState[htcFieldTreeView] :=
            TcxPivotGridOLAPStructureInnerTreeView(AControl.Parent)
            .CalculateHitTest(AHitTest);
    end;
  end;
end;

function TcxPivotGridCustomCustomizationForm.CanDrag
  (AField: IcxPivotGridField): Boolean;
begin
...
procedure TcxPivotGridCustomCustomizationForm.CalculateFieldHitTest
  (AHitTest: TcxPivotGridHitTest; AField: TcxPivotGridField;
  const ABounds: TRect);
begin
...
begin
  ...
  begin
    ...
    else if (PivotGrid <> nil) and not PivotGrid.IsDestroying then
...
procedure TcxPivotGridCustomCustomizationForm.DoCustomDrawFieldHeader
  (ACanvas: TcxCanvas; ACell: TcxPivotGridCustomCellViewInfo;
  var ADone: Boolean);
begin
...
function TcxPivotGridCustomCustomizationForm.
  GetCustomizationFormListBackgroundColor: TColor;
begin
  Result := clWindow;
end;

function TcxPivotGridCustomCustomizationForm.GetDragDropInfo
  : TcxPivotGridDragDropAreaInfo;
begin
...
begin
  AType := fltAvailable;
  Result := not AField.Hidden and
    ((AField.Group = nil) or (AField.Group[0] = AField));
...
begin
  ...
  for I := 0 to High(UnusedItems) do
    DeleteMenu(GetSystemMenu(Handle, False), UnusedItems[I, 0],
      UnusedItems[I, 1]);
  LookAndFeelChanged;
end;
...
begin
  Result := TcxPivotGridField(AItem1).AreaIndex - TcxPivotGridField(AItem2)
    .AreaIndex;
end;

procedure TcxPivotGridCustomCustomizationForm.PopulateFieldList(AList: TList;
  AType: TcxPivotGridCustomizationFormFieldListType);
var
  I: Integer;
...
begin
  AList.Clear;
  for I := 0 to PivotGrid.FieldCount - 1 do
    if GetFieldListType(PivotGrid.Fields[I], AFieldListType) and
      (AFieldListType = AType) then
...
procedure TcxPivotGridCustomCustomizationForm.SetIsLayoutChanged
  (AValue: Boolean);
begin
...
function TcxPivotGridCustomCustomizationForm.GetPainter
  : TcxCustomLookAndFeelPainter;
begin
  Result := PivotGrid.LookAndFeelPainter;
end;
...
begin
  if IsIconic(Application.Handle) then
    Visible := False
  else if not IsControlVisible(PivotGrid) then
...
procedure TcxPivotGridCustomCustomizationForm.SetPivotGrid
...
function TcxPivotGridCustomization.CalculateHitTest
  (AHitTest: TcxPivotGridHitTest): Boolean;
begin
  Result := Visible and Form.HandleAllocated and
    Form.CalculateHitTest(AHitTest);
end;
...
begin
  ...
  begin
    Result.BottomRight := PivotGrid.ClientToScreen
      (PivotGrid.ClientRect.BottomRight);
  ...
  begin
    ...
    Result.Bottom := PivotGrid.ClientToScreen
      (PivotGrid.ClientRect.BottomRight).Y;
  end;
...
function TcxPivotGridCustomization.GetCustomizationFormClass
  : TcxPivotGridCustomizationFormClass;
const
  FormClasses: array [Boolean, TcxPivotGridCustomizationFormStyle]
    of TcxPivotGridCustomizationFormClass = ((TcxPivotGridCustomizationForm,
    TcxPivotGridAdvancedCustomizationForm), (TcxPivotGridOLAPCustomizationForm,
...
procedure TcxPivotGridCustomization.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
begin
  AVisible := Visible;
  ReleaseCustomizationForm;
  if PivotGrid.IsDestroying or PivotGrid.IsLoading then
    Exit;
  if AVisible then
  begin
...
begin
  if (FSite = AValue) or not CanAssignedSite(AValue) then
    Exit;
  if FSite <> nil then
...
function TcxPivotGridDataController.GetFilterItemFieldCaption
...
function TcxPivotGridDataController.GetItemValueSource(AItemIndex: Integer)
...
function TcxPivotGridOLAPStructureNode.GetLinkedField
  (APivotGrid: TcxCustomPivotGrid): TcxPivotGridField;
var
  I: Integer;
  AList: TList;
begin
...
function TcxPivotGridOLAPStructureNode.ItemByDisplayText(const ADisplayText
  : string): TcxPivotGridOLAPStructureNode;
begin
...
procedure TcxPivotGridOLAPStructureNode.RemoveFieldLink
  (AField: TcxPivotGridField);
begin
...
function TcxPivotGridOLAPStructureNode.GetAggregateType
  : TcxOLAPStructureNodeTypes;
var
...
procedure TcxPivotGridOLAPStructureNode.SetNodeType
  (AValue: TcxOLAPStructureNodeType);
begin
  FNodeType := AValue;
end;
...
begin
  ...
  try
    ...
    try
      ...
      if (APrevData.Size <> ANewData.Size) or not CompareMem(APrevData.Memory,
        ANewData.Memory, APrevData.Size) then
        APrevStructure.Free
      else
      begin
        FreeAndNil(FStructure);
...
function TcxPivotGridCustomOLAPDataSource.CreateNewField
  ...
  begin
    ...
    for I := 0 to PivotGrid.FieldCount - 1 do
      if PivotGrid.Fields[I].Name = AName then
      // when owner of PivotGrid is Frame
      begin
        Inc(AIndex);
...
procedure TcxPivotGridCustomOLAPDataSource.CreateFieldsFromStructure;
  ...
  procedure ProcessNode(ANode: TcxPivotGridOLAPStructureNode;
    AGroup: TcxPivotGridFieldGroup);
  begin
    if ANode.Count > 0 then
    begin
      AGroup := nil;
      if (ANode.NodeType = ntGroup) or
        ((ANode.NodeType = ntFolder) and (ntKPI in ANode.AggregateType)) then
    ...
    else if ANode.NodeType in [ntField, ntSet] then
...
function TcxPivotGridCustomOLAPDataSource.CreateDrillDownDataSource
  (ACell: TcxPivotGridCrossCell; FieldList: TcxObjectList;
  ASummaryIndex: Integer): TcxPivotGridCrossCellDataSource;
var
  AList: TList;
begin
...
function TcxPivotGridCustomOLAPDataSource.CreateDrillDownDataSource
  (ACells: TList; FieldList: TcxObjectList; ASummaryIndex: Integer)
  : TcxPivotGridCrossCellDataSource;
begin
  Result := nil;
end;
...
function TcxPivotGridCustomOLAPDataSource.CreateMembersFromGroup
  (AItem: TcxPivotGridGroupItem; AIncludeParent: Boolean = True;
  ACheckExpanding: Boolean = False): TList;
  ...
  begin
    ...
    if AIncludeSelf then
      Result.Add(AItem)
    else if ACheckExpanding and AItem.FExpanded and (AItem.ItemCount = 0) and
      (AItem.Parent <> nil) then
      Result.Add(AItem);
...
procedure TcxPivotGridCustomOLAPDataSource.ExpandMember
  (AField: TcxPivotGridField; AMember: TcxPivotGridGroupItem;
...
begin
  if (AField = nil) or (AExpandChildren and (AField.ExpandingInfo.Count = 0))
  then
    Exit;
  ...
  AList := CreateMembersFromGroup(AMember, not AExpandChildren,
    AExpandChildren);
  try
    if AList.Count = 0 then
      Exit;
    InitializeExpanding(ADataBuilder, AField, AMember, ACrossGroup,
...
function TcxPivotGridCustomOLAPDataSource.GetFieldForStructure
  (AGroup: TcxPivotGridFieldGroup; AStructure: TcxPivotGridOLAPStructureNode)
  : TcxPivotGridOLAPField;
var
  I: Integer;
begin
...
procedure TcxPivotGridCustomOLAPDataSource.InitializeExpanding
...
function TcxPivotGridCustomOLAPDataSource.CanCollapse
  (ASender: TdxTreeCustomNode): Boolean;
begin
  Result := True;
end;

function TcxPivotGridCustomOLAPDataSource.CanExpand
  (ASender: TdxTreeCustomNode): Boolean;
begin
  Result := True;
end;

procedure TcxPivotGridCustomOLAPDataSource.Collapsed
...
procedure TcxPivotGridCustomOLAPDataSource.LoadChildren
  (ASender: TdxTreeCustomNode);
begin
end;

//
procedure TcxPivotGridCustomOLAPDataSource.BeforeDelete
  (ASender: TdxTreeCustomNode);
begin
end;

procedure TcxPivotGridCustomOLAPDataSource.DeleteNode
  (ASender: TdxTreeCustomNode);
begin
end;
...
function TcxPivotGridCustomOLAPDataSource.GetNodeClass(ARelativeNode
  : TdxTreeCustomNode): TdxTreeCustomNodeClass;
begin
...
function TcxPivotGridCustomOLAPDataSource.GetListener(AIndex: Integer)
  : TcxCustomPivotGrid;
begin
...
procedure TcxPivotGridKPICellViewInfo.Initialize(ARow,
  AColumn: TcxPivotGridViewDataItem; ADataField: TcxPivotGridField);
begin
  inherited Initialize(ARow, AColumn, ADataField);
  FGraphicType := TcxPivotGridOLAPField(ADataField).Options.KPIGraphicType;
  if (FGraphicType = gtServerDefined) and
    (TcxPivotGridOLAPField(ADataField).Structure <> nil) then
...
begin
  ...
  begin
    ...
    if AImageIndex > 0 then
      cxDrawImage(Canvas, Bounds, nil, cxPivotGridKPIImageList, AImageIndex,
        True, nil, ScaleFactor);
  end;
end;
...
begin
  ...
  case TcxPivotGridOLAPField(DataField).Structure.KPIType of
    oktStatus:
      case FValue of
        - 1:
          Result := cxGetResourceString(@scxKPIStatusBad);
        0:
          Result := cxGetResourceString(@scxKPIStatusNeutral);
        1:
          Result := cxGetResourceString(@scxKPIStatusGood);
      end;
    oktTrend:
      case FValue of
        - 1:
          Result := cxGetResourceString(@scxKPITrendGoingDown);
        0:
          Result := cxGetResourceString(@scxKPITrendNoChange);
        1:
          Result := cxGetResourceString(@scxKPITrendGoingUp);
      end;
  else
...
function TcxPivotGridKPICellViewInfo.NeedShowHint(const APoint: TPoint)
  : Boolean;
begin
  if TcxPivotGridOLAPField(DataField).Structure.KPIType in [oktStatus, oktTrend]
  then
    Result := True
  else
...
procedure TcxPivotGridViewDataLimitValues.Calculate(AField: TcxPivotGridField;
  V: Variant);
var
...
function TcxPivotGridViewDataLimitValues.FindByField(AField: TcxPivotGridField)
  : TcxPivotGridViewDataLimitValue;
var
  I: Integer;
begin
...
function TcxPivotGridViewDataLimitValues.GetMaximumValue
  (AField: TcxPivotGridField): Variant;
var
  AItem: TcxPivotGridViewDataLimitValue;
begin
...
function TcxPivotGridViewDataLimitValues.GetMinimumValue
  (AField: TcxPivotGridField): Variant;
var
  AItem: TcxPivotGridViewDataLimitValue;
begin
...
function TcxPivotGridViewDataLimitValues.GetItem(Index: Integer)
  : TcxPivotGridViewDataLimitValue;
begin
...
procedure TcxPivotGridCustomOptions.SetBoolValue(var AFieldValue: Boolean;
  AValue: Boolean);
begin
...
begin
  ...
  begin
    ...
    SortBySummaryDefaultOrder := TcxPivotGridOptionsBehavior(Source)
      .SortBySummaryDefaultOrder;
  end
  else
    inherited Assign(Source);
end;

procedure TcxPivotGridOptionsBehavior.SetSortBySummaryDefaultOrder
  (AValue: TcxDataSortOrder);
begin
...
begin
  ...
  begin
    ...
    FQuickCustomization := TcxPivotGridOptionsCustomize(Source)
      .FQuickCustomization;
    FQuickPrefiltering := TcxPivotGridOptionsCustomize(Source)
      .FQuickPrefiltering;
    FSizing := TcxPivotGridOptionsCustomize(Source).FSizing;
    FSorting := TcxPivotGridOptionsCustomize(Source).FSorting;
    FSortingByGroupValues := TcxPivotGridOptionsCustomize(Source)
      .FSortingByGroupValues;
...
procedure TcxPivotGridOptionsCustomize.SetValue(AIndex: Integer;
  AValue: Boolean);
begin
  case AIndex of
    0:
...
function TcxPivotGridOptionsView.GetActualItemWidth
  (AColumnItem: Boolean): Integer;
const
...
begin
  ...
  if Result = 0 then
    Result := ScaleFactor.Apply(DefWidth[AColumnItem,
      RowTotalsLocation = rtlTree]);
  Result := Max(Result, GetMinWidth);
end;

function TcxPivotGridOptionsView.QueryInterface(const IID: TGUID;
  out Obj): HResult;
begin
  if GetInterface(IID, Obj) then
    Result := 0
...
begin
  ...
  begin
    ...
    AWidth := Max(AWidth, ARow.MeasureWidth(ARow.Field, False) +
...
begin
  ...
  if IsCompactLayout then
    Inc(Result, (PivotGrid.ViewData.MaxRowLevel - 1) *
      PivotGrid.RowLevelIndentWidth);
end;
...
var
  AWidth: Integer;
begin
  AWidth := Max(ScaleFactor.Apply(cxPivotGridDefaultFieldMinWidth),
    GetActualWidth + ADelta);
...
begin
  Result := (RowTotalsLocation = rtlTree) and
    (PivotGrid.DataBuilder.RowFields.Count > 0)
end;
...
procedure TcxPivotGridOptionsView.SetFilterDropDownMaxItemCount
  (AValue: Integer);
begin
  AValue := Max(1, AValue);
...
procedure TcxPivotGridOptionsView.SetGrandTotalsForSingleValues
  (AValue: Boolean);
begin
...
procedure TcxPivotGridOptionsView.SetTotalsLocation(const AValue
...
function TcxPivotGridOptionsData.CompareAsString(const V1, V2: Variant)
  : Integer;
begin
  if AnsiSort then
...
procedure TcxPivotGridOptionsPrefilter.SetMRUItemsListDropDownCount
  (AValue: Integer);
begin
  AValue := Max(0, AValue);
...
procedure TcxPivotGridOptionsPrefilter.SetPosition
  (AValue: TcxPivotGridPrefilterPosition);
begin
...
procedure TcxPivotGridOptionsPrefilter.SetVisible
  (AValue: TcxPivotGridPrefilterVisible);
begin
...
procedure TcxPivotGridOptionsSelection.SetIncludeCells
  (const AValue: TcxPivotGridOptionsSelectionIncludes);
begin
...
procedure TcxPivotGridRecords.CreateIntersection(ARecords: TcxPivotGridRecords;
  AList: PIntArray);
var
  I, C: Integer;
begin
...
begin
  Result := (PivotGrid.IsDesigning and (ARow = 0) and (AColumn = 0)) or
    (not PivotGrid.IsDesigning and FSelection.IsCellSelected(AColumn, ARow) and
    CanCellSelect(ARow, AColumn));
end;
...
procedure TcxPivotGridViewData.AdjustCellIndexes(var ARow, AColumn: Integer;
  AByMouse: Boolean = False);
  ...
  procedure InnerCalculateDelta(APrevRow, APrevColumn: Integer;
    var DY, DX: Integer);
  begin
...
function TcxPivotGridViewData.AdjustCellIndexesPoint(const P: TPoint;
  AByMouse: Boolean = False): TPoint;
var
  AColumn, ARow: Integer;
begin
...
function TcxPivotGridViewData.CalculateDataWidth
  (AField: TcxPivotGridField): Integer;
var
...
begin
  ...
  AFakeCell := TcxPivotGridDataCellViewInfo.Create(PivotGrid.LookAndFeelPainter,
    PivotGrid.ScaleFactor, cxSimpleRect, cxSimpleRect,
    PivotGrid.ViewInfo.FViewParams);
  try
    ARow := Rows[0];
    while ARow <> nil do
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            AFakeCell.FViewParams := PivotGrid.Styles.GetContentParams
              (AFakeCell);
      ...
      if PivotGrid.OLAPDataSource = nil then
      begin
        if not IsGroupItemEquals(APrevRow, ARow) and
          (PivotGrid.OptionsDataField.Area <> dfaRow) and
...
procedure TcxPivotGridViewData.CalculateDataFieldInfo(var AFieldIndex: Integer;
...
begin
  ...
  begin
    if DataBuilder.DataFields.Count <= 1 then
      Exit;
    if (Area = AcceptedArea) or ((Area = dfaNone) and (AcceptedArea = dfaColumn))
    then
    begin
...
procedure TcxPivotGridViewData.CalculateLimitValues;
  ...
  function GetRootParent(AItem: TcxPivotGridViewDataItem)
    : TcxPivotGridViewDataItem;
  begin
    if AItem.Parent <> nil then
  ...
  procedure InnerCalculateForItem(AItem, ARootCrossItem
    : TcxPivotGridViewDataItem);
  var
    I: Integer;
    V: Variant;
  ...
  begin
    ...
    begin
      if ARootCrossItem.Items[I].IsTotal or ARootCrossItem.Items[I].IsGrandTotal
      then
        Continue;
      ...
      begin
        ...
        if AItem.IsDataField then
          ACrossCell := ARootCrossItem.Items[I].GroupItem.GetCellByCrossItem
            (AItem.GetGroupItem(ADataField))
        else if ARootCrossItem.Items[I].IsDataField then
          ACrossCell := AItem.GroupItem.GetCellByCrossItem
            (ARootCrossItem.Items[I].GetGroupItem(ADataField))
        else
          ACrossCell := AItem.GetGroupItem(ADataField)
            .GetCellByCrossItem(ARootCrossItem.Items[I].GroupItem);
  ...
  procedure InnerCalculateLimitValues(AItem, ARootCrossItem
    : TcxPivotGridViewDataItem);
  var
    I: Integer;
  begin
    if AItem.ItemCount = 0 then
      ...
      for I := 0 to AItem.ItemCount - 1 do
      begin
        if AItem.Items[I].IsTotal or AItem.Items[I].IsGrandTotal then
          Continue;
...
var
  ...
  AIsCell: Boolean;
begin
  Result := (ARow >= 0) and (ARow < RowCount) and (AColumn >= 0) and
    (AColumn < ColumnCount);
  if not Result then
    Exit;
  AIsGrandTotal := Columns[AColumn].IsGrandTotal or Rows[ARow].IsGrandTotal;
  AIsTotal := not AIsGrandTotal and
    ((Columns[AColumn].IsTotalItem and not Columns[AColumn].IsGrandTotal) or
...
procedure TcxPivotGridViewData.HeaderCellSelect
  (ADataItem: TcxPivotGridViewDataItem; AShift: TShiftState);
var
  R: TRect;
...
function TcxPivotGridViewData.IsGroupItemEquals(AItem1,
  AItem2: TcxPivotGridViewDataItem): Boolean;
...
begin
  ...
  begin
    AProducer := TcxPivotGridItemsProducer.Create(DataBuilder.Columns, FColumns,
      FColumnsList, TcxPivotGridTotalsLocation(ColumnTotalsLocation),
      ColumnTotals, TotalsForSingleValues, ColumnGrandTotals,
      GrandTotalsForSingleValues);
  end;
  try
    CalculateDataFieldInfo(AProducer.DataLevel, AProducer.DataFields,
      dfaColumn);
    AProducer.Produce;
...
begin
  with OptionsView do
  begin
    AProducer := ProducerClasses[PivotGrid.OptionsView.IsCompactLayout]
      .Create(DataBuilder.Rows, FRows, FRowsList,
      TcxPivotGridTotalsLocation(RowTotalsLocation), RowTotals,
      TotalsForSingleValues, RowGrandTotals, GrandTotalsForSingleValues);
  end;
  try
...
procedure TcxPivotGridViewData.Scroll(AScrollCode: TScrollCode;
  AItem: TcxPivotGridViewDataItem; APage, AMax, ASize, AScrollPos: Integer;
  var APos: Integer);
var
  C: Integer;
begin
...
function TcxPivotGridViewData.ScrollColumns(AScrollCode: TScrollCode;
  var AScrollPos: Integer): Boolean;
var
  APos: Integer;
begin
...
function TcxPivotGridViewData.ScrollRows(AScrollCode: TScrollCode;
  var AScrollPos: Integer): Boolean;
var
  APos: Integer;
begin
...
function TcxPivotGridViewData.GetCell(ARow, AColumn: Integer)
  : TcxPivotGridCrossCellSummary;
var
  ADataField: TcxPivotGridField;
  ACrossCell: TcxPivotGridCrossCell;
...
function TcxPivotGridViewData.GetColumn(AIndex: Integer)
  : TcxPivotGridViewDataItem;
begin
...
function TcxPivotGridViewData.GetRow(AIndex: Integer): TcxPivotGridViewDataItem;
begin
  if not PivotGrid.ViewInfo.IsPrinting then
    PivotGrid.CheckChanges;
...
function TcxPivotGridViewData.NeedCalculateLimitValues: Boolean;
  ...
  function NeedCalculateLimitValuesForStyles
    (AStyles: TcxPivotGridCustomStyles): Boolean;
  begin
    with AStyles do
      Result := Assigned(OnGetContentStyle) or (ColumnMaximumValue <> nil) or
        (ColumnMinimumValue <> nil) or (RowMaximumValue <> nil) or
        (RowMinimumValue <> nil) or (MaximumValue <> nil) or
        (MinimumValue <> nil);
  end;
...
var
  I: Integer;
begin
  Result := not FCalculatedLimitValues and (ColumnCount > 0) and (RowCount > 0)
    and (DataBuilder.DataFields.Count > 0);
  if Result then
  begin
    Result := NeedCalculateLimitValuesForStyles(PivotGrid.Styles) or
      Assigned(PivotGrid.OnCustomDrawCell);
    if not Result then
      for I := 0 to DataBuilder.DataFields.Count - 1 do
      begin
        Result := NeedCalculateLimitValuesForStyles
...
procedure TcxCustomPivotGrid.BeginLockedStatePaint
...
procedure TcxCustomPivotGrid.CopyToClipboard(ACopyAll, AIncludeHeaders,
  AIncludeAllRowHeaders, AIncludeAllColumnHeaders: Boolean);
var
...
function TcxCustomPivotGrid.GetFieldByName(const AName: string)
  : TcxPivotGridField;
var
  I: Integer;
begin
...
procedure TcxCustomPivotGrid.StoreToIniFile(const AStorageName: string;
  AReCreate: Boolean = True);
begin
  StoreTo(stIniFile, AStorageName, nil, AReCreate);
end;

procedure TcxCustomPivotGrid.StoreToRegistry(const AStorageName: string;
  AReCreate: Boolean = True);
begin
...
procedure TcxCustomPivotGrid.BeforeMouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
function TcxCustomPivotGrid.DoShowPopupMenu(AMenu: TComponent;
  X, Y: Integer): Boolean;
begin
...
begin
  ...
  with ViewData do
  begin
    SetScrollBarInfo(sbHorizontal, 0, ColumnCount - 1, 1, ColumnsPerPage,
      ColumnIndex, True, True);
    SetScrollBarInfo(sbVertical, 0, RowCount - 1, 1, RowsPerPage, RowIndex,
      True, True)
  end;
end;
...
begin
  ...
  if not OptionsCustomize.Filtering then
    Exit;
  for I := 0 to FieldCount - 1 do
...
function TcxCustomPivotGrid.GetDesignHitTest(X, Y: Integer;
...
begin
  ...
  begin
    HitTest.HitPoint := Point(X, Y);
    Result := Controller.StartDragAndDrop(Point(X, Y)) or
      (DragAndDropState <> ddsNone) or HitTest.HitAtButton or
      HitTest.HitAtField;
    if not Result then
...
function TcxCustomPivotGrid.GetDragAndDropObjectClass
  : TcxDragAndDropObjectClass;
begin
...
function TcxCustomPivotGrid.CreateGroupItemValueComparer
  : TcxPivotGridGroupItemValueHelperComparer;
begin
...
function TcxCustomPivotGrid.CreateOptionsCustomize
  : TcxPivotGridOptionsCustomize;
begin
...
function TcxCustomPivotGrid.CreateOptionsDataField
  : TcxPivotGridOptionsDataField;
begin
  Result := TcxPivotGridOptionsDataField.Create(Self);
end;

function TcxCustomPivotGrid.CreateOptionsPrefilter
  : TcxPivotGridOptionsPrefilter;
begin
  Result := TcxPivotGridOptionsPrefilter.Create(Self);
end;

function TcxCustomPivotGrid.CreateOptionsSelection
  : TcxPivotGridOptionsSelection;
begin
...
procedure TcxCustomPivotGrid.DoGetCellHint(APoint: TPoint;
  ACell: TcxPivotGridCustomCellViewInfo; var ABounds, ATextBounds: TRect;
  var AHint: string);
begin
...
begin
  if IsLocked or ViewInfo.IsPrinting then
    Exit;
  CheckChanges;
  if Changes <> [] then
    Exit;
...
function TcxCustomPivotGrid.GetMainScrollBarsClass
...
procedure TcxCustomPivotGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TcxCustomPivotGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  HitTest.ShiftState := Shift;
  HitTest.HitPoint := Point(X, Y);
...
procedure TcxCustomPivotGrid.Notification(AComponent: TComponent;
  Operation: TOperation);
begin
...
begin
  ...
  else if AScrollBarKind = sbVertical then
  begin
    ...
    begin
      ...
      begin
        ...
        if not TcxPivotGridFieldPosition(AFields[I]).Visible then
          Inc(AIndex)
        else if TcxPivotGridFieldPosition(AFields[I]).Group <> nil then
          Dec(AIndex, TcxPivotGridFieldPosition(AFields[I])
...
function TcxCustomPivotGrid.GetProperties(Index: Integer)
  : TcxCustomEditProperties;
begin
...
function TcxCustomPivotGrid.GetItemValueSource(AItemIndex: Integer)
  : TcxDataEditValueSource;
begin
  Result := evsValue;
  if (Fields[AItemIndex].Area <> faData) and
    (Fields[AItemIndex].Properties <> nil) then
...
function TcxCustomPivotGrid.StoredObject_GetProperties
  (AProperties: TStrings): Boolean;
var
  I: Integer;
begin
  for I := Low(PivotGridDefaultStoredProperties)
    to High(PivotGridDefaultStoredProperties) do
...
procedure TcxCustomPivotGrid.StoredObject_GetPropertyValue(const AName: string;
  var AValue: Variant);
...
procedure TcxCustomPivotGrid.StoredObject_SetPropertyValue(const AName: string;
  const AValue: Variant);
...
begin
  case GetPropertyIndex(AName, PivotGridDefaultStoredProperties) of
    0:
      SetFilterValue;
    1:
      DataController.Filter.Active := AValue;
  end;
  if Assigned(OnSetStoredPropertyValue) then
    OnSetStoredPropertyValue(Self, AName, AValue);
end;
...
// IcxStoredParent
function TcxCustomPivotGrid.StoredParent_CreateChild(const AObjectName,
...
procedure TcxCustomPivotGrid.StoredParent_DeleteChild(const AObjectName: string;
  AObject: TObject);
begin
  AObject.Free;
end;
...
function TcxCustomPivotGrid.CreateLockedStatePaintHelper
  : TcxPivotGridLockedStatePaintHelper;
begin
  Result := TcxPivotGridLockedStatePaintHelper.Create(Self);
end;

function TcxCustomPivotGrid.CreateOptionsLockedStateImage
...
begin
  Result := LookAndFeelPainter.ScaledExpandButtonSize(ScaleFactor) +
    ScaleFactor.Apply(cxPivotGridDoubleSpace) * 2;
end;

procedure TcxCustomPivotGrid.SetCustomization
  (AValue: TcxPivotGridCustomization);
begin
  FCustomization.Assign(AValue);
end;

procedure TcxCustomPivotGrid.SetField(AIndex: Integer;
...
procedure TcxCustomPivotGrid.SetGroups
  (AValue: TcxPivotGridFieldGroupCollection);
begin
...
procedure TcxCustomPivotGrid.SetOptionsLockedStateImage
  (Value: TcxPivotGridOptionsLockedStateImage);
begin
  FOptionsLockedStateImage.Assign(Value);
end;

procedure TcxCustomPivotGrid.SetOptionsPrefilter
  (AValue: TcxPivotGridOptionsPrefilter);
begin
...
begin
  ...
  if IsDesigning and (DragAndDropState = ddsNone) and
    GetDesignHitTest(P.X, P.Y, [ssLeft]) then
...
function TcxPivotGridCells.CalculateHitTest
  (AHitTest: TcxPivotGridHitTest): Boolean;
var
  I: Integer;
begin
...
procedure TcxPivotGridCells.Paint(ACanvas: TcxCanvas;
...
function TcxPivotGridCells.GetItem(AIndex: Integer)
  : TcxPivotGridCustomCellViewInfo;
begin
...
function TcxPivotGridFilterMRUItem.StreamEquals(AStream: TMemoryStream)
  : Boolean;
var
...
function TcxPivotGridFilterMRUItem.FilterEquals
  (AFilter: TcxDataFilterCriteria): Boolean;
begin
...
function TcxPivotGridFilterMRUItems.GetItem(Index: Integer)
  : TcxPivotGridFilterMRUItem;
begin
...
function TcxPivotGridFilterMRUItems.GetVisibleItem(Index: Integer)
  : TcxPivotGridFilterMRUItem;
begin
...
begin
  ...
  begin
    AItem := Items[I];
    if not AItem.FilterEquals(AFilter) then
      FVisibleItems.Add(AItem);
  end;
  if PivotGrid.OptionsPrefilter.MRUItemsList and
    (VisibleCount <> APrevVisibleCount) and
...
constructor TcxPivotGridCopyToClipboardHelper.Create
  (APivotGrid: TcxCustomPivotGrid);
begin
  inherited Create;
  FPivotGrid := APivotGrid;
...
begin
  ...
  ShowHourglassCursor;
  try
    if (FCellsRect.Left <= FCellsRect.Right) and
      (FCellsRect.Top <= FCellsRect.Bottom) then
...
begin
  ...
  begin
    ...
    if (APrevRow <> nil) and not ViewData.IsGroupItemEquals(APrevRow,
      ViewData.Rows[ARowIndex]) and (FPivotGrid.OptionsDataField.Area <> dfaRow)
      and not(APrevRow.IsTotal or APrevRow.IsGrandTotal) then
      APrevRow.ClearCache;
...
begin
  ...
  FCellsRect.BottomRight :=
    Point(Min(FCellsRect.Right, ViewData.ColumnCount - 1),
...
begin
  ...
  begin
    ...
    if (ALevel = FMaxLevelColumnHeaders - 1) and AColumnItem.IsDataField and
      (ALevel = AColumn.Level) then
      Result := ''
    else
      Result := AColumnItem.GetDisplayText;
  end;
...
begin
  ...
  if FIncludeAllColumnHeaders then
    ...
    begin
      Result := Result + GetRowHeaderText
        (-1 - (FMaxLevelColumnHeaders - ALevel));
      for AColumnIndex := FCellsRect.Left to FCellsRect.Right do
        Result := Result + GetColumnHeaderLevelText
          (ViewData.Columns[AColumnIndex], ALevel) + GetSeparator(AColumnIndex);
    end
  else
  begin
    Result := GetRowHeaderText(-1);
    for AColumnIndex := FCellsRect.Left to FCellsRect.Right do
      Result := Result + ViewData.Columns[AColumnIndex].GetDisplayText +
        GetSeparator(AColumnIndex);
  end;
end;
...
function TcxPivotGridCopyToClipboardHelper.GetRowHeaderText
...
function TcxPivotGridLockedStatePaintHelper.GetOptions
...
function TcxPivotGridVariantValue.CompareWithHash(const AValue
  : TcxPivotGridVariantValue): Integer;
begin
...
function TcxPivotGridVariantValue.IsEqual(const AValue
  : TcxPivotGridVariantValue): Boolean;
begin
  Result := (AValue <> nil) and
    ((AValue = Self) or VarEquals(AValue.Value, Value));
end;
...
function TcxPivotGridVariantList.HashedIndexOf(const AValue: Variant;
...
begin
  ...
  begin
    ...
    begin
      AIndex := (L + H) shr 1;
      C := TcxPivotGridVariantValue(List[AIndex])
        .CompareWithHash(CompareHelper);
      if C < 0 then
        L := AIndex + 1
      else
      begin
...
function TcxPivotGridVariantList.InternalAddValue(const AValue: Variant;
...
function TcxPivotGridVariantList.GetItem(AIndex: Integer)
  : TcxPivotGridVariantValue;
begin
...
constructor TcxPivotGridViewDataItem.Create(AParent: TcxPivotGridViewDataItem;
  AGroupItem: TcxPivotGridGroupItem);
begin
  inherited Create;
...
function TcxPivotGridViewDataItem.Add(AData: TcxPivotGridGroupItem)
  : TcxPivotGridViewDataItem;
begin
...
function TcxPivotGridViewDataItem.AddTotal(AData: TcxPivotGridGroupItem;
...
begin
  AFakeCell := TcxPivotGridHeaderCellViewInfo.Create
    (PivotGrid.LookAndFeelPainter, PivotGrid.ScaleFactor, cxSimpleRect,
    cxSimpleRect, PivotGrid.ViewInfo.FViewParams);
  try
    AFakeCell.Data := Self;
...
function TcxPivotGridViewDataItem.GetGroupItem(var ADataField
  : TcxPivotGridField): TcxPivotGridGroupItem;
var
...
function TcxPivotGridViewDataItem.GetSummaryType(var ASummaryType
  : TcxPivotGridSummaryType; var ATotal: TcxPivotGridCustomTotal): Boolean;
begin
  if Parent <> nil then
...
function TcxPivotGridViewDataItem.MeasureWidth(AField: TPersistent;
  AIncludeTotals: Boolean): Integer;
var
...
begin
  Result := 0;
  if (Field = AField) and (PivotGrid <> nil) and (not IsTotal or AIncludeTotals)
  then
    with PivotGrid do
    begin
      AImageIndex := Field.DoGetGroupImageIndex(Self, AAlignHorz, AAlignVert);
      if AImageIndex >= 0 then
        Inc(Result, dxGetImageSize(GroupHeaderImages, ScaleFactor).cx +
          ScaleFactor.Apply(cxPivotGridDoubleSpace));
      if (ItemCount > 0) or ((GroupItem <> nil) and (GroupItem.ItemCount > 0))
      then
        Inc(Result, LookAndFeelPainter.ScaledExpandButtonSize(ScaleFactor) +
          ScaleFactor.Apply(cxPivotGridDoubleSpace) * 2);
      AParams := Styles.GetRowHeaderParams(Self);
      Inc(Result, cxTextWidth(AParams.Font, Value) +
        ScaleFactor.Apply(cxPivotGridDoubleSpace));
      Exit;
    end;

  if IsGrandTotal and ((AField = Field) or (ItemCount = 0)) then
    Inc(Result, cxTextWidth(PivotGrid.Styles.GetRowHeaderParams(Self).Font,
      Value) + ScaleFactor.Apply(cxPivotGridDoubleSpace))
  else
...
begin
  Result := not IsTotal and ((GroupItem.ItemCount > 0) or
    GroupItem.HasChildren);
end;
...
function TcxPivotGridViewDataItem.GetItem(AIndex: Integer)
  : TcxPivotGridViewDataItem;
begin
...
begin
  Result := IsTotalItem and ((GroupItem.Parent = nil) or
    ((Parent <> nil) and (Parent.IsGrandTotal)));
end;
...
begin
  ...
  begin
    Result := Result.Parent;
    if (Result <> nil) and (Result.VisibleIndex >= 0) then
      Exit;
  end;
  if Result <> nil then
  begin
    Result := Result.Parent.Items[Result.Index - 1];
    while Result.ItemCount > 0 do
      Result := Result.Last;
  end;
end;
...
begin
  ASortOrder := soNone;
  ASortedByGroupValue := (ItemCount = 0) and
    (PivotGrid.DataBuilder.DataFields.Count > 0) { and (GroupItem.Level >= 0) };
  if not ASortedByGroupValue then
    Exit;
  ADataField := nil;
  AItem := Self;
  ...
  begin
    if ADataField <> nil then
      ASortedByGroupValue := AFields[I].SortBySummaryInfo.ConditionDefined
        (ADataField, AItem.GroupItem)
    else
      for J := 0 to ADataFields.Count - 1 do
      begin
        ASortedByGroupValue := AFields[I].SortBySummaryInfo.ConditionDefined
          (ADataFields[J], AItem.GroupItem);
...
function TcxPivotGridViewDataTotalItem.GetSummaryType(var ASummaryType
  : TcxPivotGridSummaryType; var ATotal: TcxPivotGridCustomTotal): Boolean;
begin
  if Total <> nil then
  begin
...
begin
  Result := ((PivotGrid.RecordCount > 0) or (OLAPDataSource <> nil)) and
    (DataFields.Count > 0);
end;
  ...
  try
    ...
    begin
      if not FOLAPDataSource.IsTerminated then
        FOLAPDataSource.CreateRootLayout(PivotGrid, RowFields, ColumnFields,
          SummaryFields, FilteredFields);
    end
    else
    begin
      GroupByRows;
...
procedure TcxPivotGridDataBuilder.AddSummaryField(AField: TcxPivotGridField);
begin
...
begin
  for I := 0 to DataFields.Count - 1 do
    FData.ItemList.Add(TcxPivotGridDataItem.Create(FData, DataController,
      DataFields[I]));
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if FilteredIndexes[I] <= cxPivotGridRecordInvisible then
        Continue;
...
begin
  ...
  AIndexes := FilteredIndexes.List;
  FillChar(AIndexes^, PivotGrid.RecordCount * SizeOf(Pointer), Integer(-1));
end;
...
begin
  ...
  begin
    ...
    if (AField.Area = faData) and (not(AField is TcxPivotGridOLAPField) or
      AField.VisibleInGroup) then
    ...
    if AField.VisibleInGroup and (AField.FVisibleIndex <> AFields.Add(AField))
    then
    begin
  ...
  if PivotGrid.ViewInfo.IsDataFieldVisible([dfaRow, dfaColumn]) and (AIndex >= 0)
  then
  begin
    for I := 0 to Fields.Count - 1 do
    begin
      if (Fields[I].Area = DataAreaToFieldArea[PivotGrid.OptionsDataField.Area])
        and (Fields[I].AreaIndex >= AIndex) then
        Inc(Fields[I].FAreaIndex);
    end;
  end;
...
function TcxPivotGridDataBuilder.GetCrossCell(ARow,
  AColumn: TcxPivotGridGroupItem): TcxPivotGridCrossCell;
begin
...
const
  AStyleID: array [TcxPivotGridFieldArea] of Integer = (gs_ColumnHeaderArea,
    gs_RowHeaderArea, gs_FilterHeaderArea, gs_DataHeaderArea);
begin
...
function TcxPivotGridCustomStyles.GetContentStyleIndexByCell
  (ACell: TcxPivotGridDataCellViewInfo): Integer;
const
...
begin
  ...
  begin
    ...
    begin
      ...
      if (lvtMaximum in ALimitValues) and (Values[AStyleID[lvtMaximum]] <> nil)
      then
        Result := AStyleID[lvtMaximum];
      if (Result = gs_Content) and (lvtMinimum in ALimitValues) and
        (Values[AStyleID[lvtMinimum]] <> nil) then
        Result := AStyleID[lvtMinimum];
      if (Result = gs_Content) and (lvtColumnMaximum in ALimitValues) and
        (Values[AStyleID[lvtColumnMaximum]] <> nil) then
        Result := AStyleID[lvtColumnMaximum];
      if (Result = gs_Content) and (lvtColumnMinimum in ALimitValues) and
        (Values[AStyleID[lvtColumnMinimum]] <> nil) then
        Result := AStyleID[lvtColumnMinimum];
      if (Result = gs_Content) and (lvtRowMaximum in ALimitValues) and
        (Values[AStyleID[lvtRowMaximum]] <> nil) then
        Result := AStyleID[lvtRowMaximum];
      if (Result = gs_Content) and (lvtRowMinimum in ALimitValues) and
        (Values[AStyleID[lvtRowMinimum]] <> nil) then
...
begin
  ...
  case Index of
    ...
    gs_ColumnHeaderArea, gs_RowHeaderArea, gs_FilterHeaderArea,
      gs_DataHeaderArea:
    ...
    gs_Total:
      begin
        AParams.Color := dxGetDarkerColor
          (ColorToRgb(APainter.GridLikeControlContentColor), 90);
        AParams.TextColor := APainter.GridLikeControlContentTextColor;
      end;
    gs_Selected:
      begin
...
function TcxPivotGridStyles.GetRowHeaderParams(ARow: TcxPivotGridViewDataItem)
  : TcxViewParams;
begin
...
procedure TcxPivotGridFieldStyles.GetDefaultViewParams(Index: Integer;
  AData: TObject; out AParams: TcxViewParams);
begin
  PivotGrid.Styles.GetViewParams(Index, AData,
    PivotGrid.Styles.Values[Index], AParams)
end;
...
begin
  if Owner is TcxCustomPivotGrid then
    Result := TcxCustomPivotGrid(Owner)
  else if (Owner is TcxPivotGridField) then
...
procedure TcxPivotGridCustomCellViewInfo.CalculateCellBounds(const ABounds,
  AVisibleRect: TRect);
begin
  FBounds := ABounds;
...
procedure TcxPivotGridCustomCellViewInfo.CorrectBoundsForPrinting
  (ABounds: TRect);
begin
  // do nothing
end;
...
procedure TcxPivotGridCustomCellViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
...
function TcxPivotGridCustomCellViewInfo.ExcludeBorders(const ABounds: TRect;
...
function TcxPivotGridCustomCellViewInfo.NeedShowHint(const APoint
  : TPoint): Boolean;
begin
  Result := False;
end;

procedure TcxPivotGridCustomCellViewInfo.RightToLeftConversion
  (const AClientBounds: TRect);
begin
...
function TcxPivotGridCustomCellViewInfo.SetHotTrack
  (AHitTest: TcxPivotGridHitTest): Boolean;
begin
  Result := False;
end;
...
procedure TcxPivotGridEditContainerViewInfo.CalculateEditViewInfo
...
begin
  ...
  try
    ...
    begin
      AEditViewInfo.UseRightToLeftAlignment :=
        FDataField.PivotGrid.ViewInfo.UseRightToLeftAlignment;
      FEditViewData.UseRightToLeftAlignment :=
        FDataField.PivotGrid.ViewInfo.UseRightToLeftAlignment;
      FEditViewData.UseRightToLeftReading :=
        FDataField.PivotGrid.ViewInfo.UseRightToLeftReading;
      FEditViewData.UseRightToLeftScrollBar :=
        FDataField.PivotGrid.ViewInfo.UseRightToLeftScrollBar;
    end;
    FEditViewData.Data := Self;
    if OnGetDisplayTextSupported then
      FEditViewData.OnGetDisplayText :=
        FDataField.EditViewDataGetDisplayTextHandler;
    ...
    FEditViewData.Calculate(Canvas, cxRectInflate(FTextBounds, cxTextOffset),
      AMousePos, cxmbNone, [], AEditViewInfo, True);
...
begin
  ...
  FTextBounds.Deflate(TdxVisualRefinements.Padding.Left, 0,
    TdxVisualRefinements.Padding.Right, 0);
end;

function TcxPivotGridEditContainerViewInfo.CreateEditViewInfo
...
procedure TcxPivotGridEditContainerViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
...
begin
  Result := cxFlagsToDTFlags(cxAlignmentsHorz[GetRealAlignHorz] or
    cxAlignmentsVert[FAlignVert] or MultiLines[FMultiLine] or
    ShowEndEllipses[FShowEndEllipsis]);
end;
...
begin
  ...
  begin
    ...
    FUseLocalEditStyle := not FDataField.UseEditProperties or
      (FProperties.GetStyleClass <> FDataField.GetEditStyle(FProperties, False)
      .ClassType);
...
begin
  ...
  FButtonAreaRect := TdxRightToLeftLayoutConverter.ConvertRect
    (FButtonAreaRect, Bounds);
  ...
  FSortMarkBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FSortMarkBounds, Bounds);
  FNeighbors := TdxRightToLeftLayoutConverter.ConvertNeighbors(FNeighbors);
end;

function TcxPivotGridHeaderCellViewInfo.GetHitTestBounds
  (AHitTest: TcxPivotGridHitTest; const ABounds: TRect): Boolean;
var
...
begin
  ...
  begin
    FImageRect := cxRectCenter(FTextBounds, dxGetImageSize(Images,
      ScaleFactor));
    FImageRect := cxRectSetTop(FImageRect,
      Bounds.Top + ScaleFactor.Apply(cxTextOffset + Byte(bTop in Borders)));
    case ImageAlignHorz of
      taLeftJustify:
        begin
          FImageRect := cxRectSetLeft(FImageRect, TextBounds.Left);
          FTextBounds.Left := FImageRect.Right +
            ScaleFactor.Apply(cxTextOffset + 1);
        end;
      taRightJustify:
        begin
          FImageRect := cxRectSetRight(FImageRect, TextBounds.Right);
          FTextBounds.Right := FImageRect.Left -
            ScaleFactor.Apply(cxTextOffset + 1);
        end;
    end;
    case ImageAlignVert of
      vaBottom:
        FImageRect := cxRectSetBottom(FImageRect,
          Bounds.Bottom - ScaleFactor.Apply(cxTextOffset));
      vaCenter:
        FImageRect := cxRectSetTop(FImageRect,
          (Bounds.Top + Bounds.Bottom - dxGetImageSize(Images, ScaleFactor)
          .cy) div 2);
    end;
...
procedure TcxPivotGridHeaderCellViewInfo.CalculateMarkBounds(ASize: TPoint;
  var AMarkBounds: TRect);
begin
  with AMarkBounds do
  begin
...
begin
  if SortOrder <> soNone then
    CalculateMarkBounds(Painter.ScaledSortingMarkAreaSize(ScaleFactor),
      FSortMarkBounds);
end;
...
begin
  ...
  if FMainViewInfo = nil then
    FTextBounds := cxRectContent(ExcludeBorders(FBounds, Borders),
  ...
  begin
    if Properties = nil then
      ...
      FTextBounds := Bounds;
    FTextBounds := cxRectContent(ExcludeBorders(FTextBounds, Borders),
      Painter.HeaderContentOffsets(ScaleFactor));
  end;
  if HasButton then
  begin
    if FMainViewInfo <> nil then
    begin
      FButtonRect := cxRectContent(ExcludeBorders(cxRectSetHeight(Bounds,
        FMainViewInfo.HeaderHeight + 1), Borders),
        Painter.HeaderContentOffsets(ScaleFactor));
      FButtonRect := cxRectSetWidth(cxRectCenterVertically(FButtonRect,
        Painter.ScaledExpandButtonSize(ScaleFactor)),
        Painter.ScaledExpandButtonSize(ScaleFactor));
    end
    else
    begin
      FButtonRect := cxRectSetSize(cxRectContent(ExcludeBorders(Bounds,
        Borders), Painter.HeaderContentOffsets(ScaleFactor)),
        Painter.ScaledExpandButtonSize(ScaleFactor),
        Painter.ScaledExpandButtonSize(ScaleFactor));
      OffsetRect(FButtonRect, 1, 1);
    end;
    FButtonAreaRect := cxRectCenter(FButtonRect,
      Painter.ScaledExpandButtonAreaSize(ScaleFactor),
      Painter.ScaledExpandButtonAreaSize(ScaleFactor));
    CheckClipping(Bounds.Right - FButtonRect.Right <
      ScaleFactor.Apply(cxPivotGridDoubleSpace));
    FTextBounds.Left := FButtonAreaRect.Right +
      ScaleFactor.Apply(cxTextOffset) + 2;
  end;
...
begin
  ...
  begin
    if not IsRightToLeftConverted then
      ...
      R := Rect(Left - cxPivotGridSizeAreaDelta, Top,
        Left + cxPivotGridSizeAreaDelta, Bottom);
  end;
  if (FSizeField <> nil) and Visible and cxRectPtIn(R, AHitTest.HitPoint) then
  begin
    AHitTest.SetBitState(htcHorzSizingEdge,
      Supports(FSizeField, IcxPivotGridSizableObject, AHitTest.FResizeField));
    if (AHitTest.FResizeField <> nil) and not AHitTest.FResizeField.CanResize
    then
    begin
...
function TcxPivotGridHeaderCellViewInfo.DrawBackgroundProc(ACanvas: TcxCanvas;
  const ABounds: TRect): Boolean;
begin
...
begin
  ...
  Painter.DrawScaledHeader(Canvas, Bounds, TextBounds, Neighbors,
    GetRealBorders, State, RealAlignHorz, AlignVert, MultiLine, ShowEndEllipsis,
    '', Canvas.Font, TextColor, Color, ScaleFactor, DrawBackgroundProc,
    GetIsLast, IsSingle);
  DrawHeaderText;
  ...
  begin
    if SortedByGroupValue then
      Painter.DrawScaledSummaryValueSortingMark(Canvas, SortMarkBounds,
        SortOrder = soAscending, ScaleFactor)
    else if IsFieldSortedBySummary then
      Painter.DrawScaledSummarySortingMark(Canvas, SortMarkBounds,
        SortOrder = soAscending, ScaleFactor)
    else
      Painter.DrawScaledSortingMark(Canvas, SortMarkBounds,
        SortOrder = soAscending, ScaleFactor);
  end;

  if IsImageAssigned(Images, ImageIndex) then
    cxDrawImage(Canvas, ImageRect, nil, Images, ImageIndex, True, nil,
      ScaleFactor);
end;
...
function TcxPivotGridHeaderCellViewInfo.NeedShowHint(const APoint
  : TPoint): Boolean;
begin
...
begin
  Result := inherited UseEditProperties and FDataField.IsDefaultGroupValues and
...
begin
  ...
  FFilterBounds := TdxRightToLeftLayoutConverter.ConvertRect
    (FFilterBounds, Bounds);
end;

procedure TcxPivotGridFieldHeaderCellViewInfo.Initialize(ACanvas: TcxCanvas;
  AScaleFactor: TdxScaleFactor; APainter: TcxCustomLookAndFeelPainter;
  ...
  begin
    ...
    FIsFilterButtonAlwaysVisible :=
      AField.PivotGrid.OptionsView.ShowHeaderFilterButtons = pgsfbAlways;
    FIsSmartTag := AField.PivotGrid.OptionsView.HeaderFilterButtonShowMode =
      pgfbmSmartTag;
...
begin
  Result := Painter.ScaledHeaderHeight(cxTextHeight(FViewParams.Font),
    ScaleFactor);
  if SortOrder <> soNone then
    Result := Max(Result, Painter.ScaledSortingMarkAreaSize(ScaleFactor).X);
  if FFilterState = cxbsNormal then
    Result := Max(Result, Painter.ScaledFilterDropDownButtonSize(ScaleFactor).Y
      + ScaleFactor.Apply(cxPivotGridDoubleSpace));
end;
...
begin
  ...
  if HasButton then
    Inc(Result, Painter.ScaledExpandButtonSize(ScaleFactor) +
      ScaleFactor.Apply(cxPivotGridDoubleSpace));
  if IsImageAssigned(Images, ImageIndex) then
  begin
    if ImageAlignHorz <> taCenter then
      Inc(Result, dxGetImageSize(Images, ScaleFactor).cx +
        ScaleFactor.Apply(cxPivotGridDoubleSpace));
  end;
...
procedure TcxPivotGridFieldHeaderCellViewInfo.SetBounds(const ABounds,
  AClipRect: TRect);
begin
  FVisibleInfoCalculated := False;
...
procedure TcxPivotGridFieldHeaderCellViewInfo.PaintTo(ACanvas: TcxCanvas;
  const ABounds: TRect; AState, AFilterState: TcxButtonState;
  ASortOrder: TcxDataSortOrder; const AHasBackground: Boolean = False;
  AHandler: TcxPivotGridCustomDrawEvent = nil);
var
  ADone: Boolean;
...
var
  ASize: TSize;
begin
  if not IsFilterButtonVisible then
    Exit;
  if FIsSmartTag then
...
begin
  ...
  begin
    if FIsSmartTag then
      Painter.DrawScaledFilterSmartTag(Canvas, FilterBounds, GetSmartTagState,
        FFilterActive, ScaleFactor)
    else
      Painter.DrawScaledFilterDropDownButton(Canvas, FilterBounds, FilterState,
        FFilterActive, ScaleFactor);
  end;
...
begin
  ...
  begin
    ...
    if (FilterState <> cxbsDisabled) and cxRectPtIn(FilterBounds,
      AHitTest.HitPoint) then
...
function TcxPivotGridFieldHeaderCellViewInfo.GetSmartTagState
...
begin
  Result := (FilterState <> cxbsDisabled) and
    (FIsFilterButtonAlwaysVisible or (State in [cxbsHot, cxbsPressed]) or
    (FilterState in [cxbsHot, cxbsPressed]) or FIsSmartTag and FilterActive);
end;
...
procedure TcxPivotGridFieldHeaderCellViewInfo.SetFilterState
...
procedure TcxPivotGridFieldHeaderCellViewInfo.SetState
  (const Value: TcxButtonState);
begin
  if State <> Value then
  begin
...
begin
  ...
  if not IsRightToLeftConverted then
    Result := cxRect(Bounds.Right, ACenter,
      Bounds.Right + ScaleFactor.Apply(cxPivotGridHorzSpace), ACenter + 1)
  else
    Result := cxRect(Bounds.Left - ScaleFactor.Apply(cxPivotGridHorzSpace),
      ACenter, Bounds.Left, ACenter + 1);
end;

{ TcxPivotGridDataCellViewInfo }

procedure TcxPivotGridDataCellViewInfo.Initialize(ARow,
  AColumn: TcxPivotGridViewDataItem; ADataField: TcxPivotGridField);
begin
  FRow := ARow;
  FColumn := AColumn;
...
begin
  Result := cxTextWidth(Font, FDisplayText) + ScaleFactor.Apply
    (cxPivotGridDoubleSpace);
end;
...
begin
  ...
  FAlignVert := vaCenter;
  FormatDisplayValue;
  if MarkNarrowCells and (cxTextWidth(Font, DisplayText) >
    cxRectWidth(TextBounds)) then
  begin
...
begin
  ...
  else if FocusRectStyle = frsDot then
  begin
...
function TcxPivotGridDataCellViewInfo.NeedShowHint(const APoint
  : TPoint): Boolean;
begin
  Result := True;
end;
...
function TcxPivotGridDataCellViewInfo.ReplaceDigitsByPattern(const ADisplayText
  : string): string;
begin
...
function TcxPivotGridDataCellViewInfo.GetLimitValueTypes
  : TcxPivotGridDataCellLimitValueTypes;
var
...
procedure TcxPivotGridHeaderBackgroundCellViewInfo.CorrectBoundsForPrinting
  (ABounds: TRect);
var
...
begin
  ...
  begin
    ...
    try
      ...
      FPainter.DrawGroupByBox(FCanvas, FieldHeadersBounds.Rect, Bitmap <> nil,
        Color, Bitmap);
    finally
  ...
  begin
    ATextFormat := cxMakeFormat(AAlignment[IsRightToLeftConverted], taCenterY)
      or CXTO_WORDBREAK or CXTO_PREVENT_TOP_EXCEED;
    if UseRightToLeftReading then
      ATextFormat := ATextFormat or CXTO_RTLREADING;
    R := Bounds;
    cxTextOut(Canvas.Handle, FDisplayText, R, ATextFormat, nil, 0, cxTextOffset,
      cxTextOffset);
  end;
end;
...
procedure TcxPivotGridPrefilterPartViewInfo.ApplyRightToLeftConversion
  (const AClientBounds: TRect);
begin
  FBounds := TdxRightToLeftLayoutConverter.ConvertRect(FBounds, AClientBounds);
end;

procedure TcxPivotGridPrefilterPartViewInfo.Calculate(ALeftBound, ATopBound,
...
constructor TcxPivotGridPrefilterPartViewInfo.Create(APartsViewInfo
...
function TcxPivotGridPrefilterPartViewInfo.GetAlignment
  : TcxPivotGridPrefilterButtonAlignment;
begin
  Result := pfbaLeft;
end;

function TcxPivotGridPrefilterPartViewInfo.GetHitTest
  (AHitTest: TcxPivotGridHitTest): Boolean;
begin
...
procedure TcxPivotGridPrefilterPartViewInfo.SetDroppedDown
  (AHitTest: TcxPivotGridHitTest; ADroppedDown: Boolean);
begin
  ...
  if APrevState <> cxbsDisabled then
    ...
    begin
      if not(ssLeft in AHitTest.ShiftState) then
        FState := cxbsHot
      else if FState = cxbsHot then
        FState := cxbsPressed;
    end
    else if not(ssLeft in AHitTest.ShiftState) then
      FState := cxbsNormal
    else if FState = cxbsPressed then
      FState := cxbsHot;
  Result := APrevState <> FState;
end;

function TcxPivotGridPrefilterPartViewInfo.SetPressed
  (AHitTest: TcxPivotGridHitTest; AMouseDown: Boolean): Boolean;
begin
  Result := False;
...
begin
  ...
  FPainter.DrawScaledFilterActivateButton(ACanvas, ABounds, FState, GetChecked,
    ScaleFactor);
end;
...
function TcxPivotGridPrefilterCustomizeButtonViewInfo.DoCalculateHeight
  : Integer;
begin
  Result := GetBorderWidth(bTop) + GetBorderWidth(bBottom) +
    cxTextHeight(GetFont) + GetTextOffset;
end;

function TcxPivotGridPrefilterCustomizeButtonViewInfo.DoCalculateWidth: Integer;
begin
  Result := GetBorderWidth(bLeft) + GetBorderWidth(bRight) +
    cxTextWidth(GetFont, GetText) + GetTextOffset;
end;
...
function TcxPivotGridPrefilterCustomizeButtonViewInfo.GetAlignment
  : TcxPivotGridPrefilterButtonAlignment;
begin
  Result := pfbaRight;
end;

function TcxPivotGridPrefilterCustomizeButtonViewInfo.GetBorderWidth
...
procedure TcxPivotGridPrefilterDropDownButtonViewInfo.DoDraw
  (ACanvas: TcxCanvas);
begin
  FPainter.DrawScaledFilterDropDownButton(ACanvas, Bounds, State, False,
    ScaleFactor);
end;

function TcxPivotGridPrefilterDropDownButtonViewInfo.GetAlignment
  : TcxPivotGridPrefilterButtonAlignment;
begin
  Result := pfbaLeft;
end;
...
begin
  Result := cxTextHeight(FPrefilter.ViewInfo.Font) + 2 *
    (PrefilterTextOffset + ScaleFactor.Apply(cxTextOffset));
end;

function TcxPivotGridPrefilterCaptionViewInfo.CalculateWidth: Integer;
begin
  Result := cxTextWidth(FPrefilter.ViewInfo.Font,
    FPrefilter.ViewInfo.DisplayText);
end;
...
begin
  ...
  begin
    ...
    ACanvas.DrawText(DisplayText, Self.Bounds, cxAlignLeft or cxAlignVCenter);
  end;
  ACanvas.Brush.Style := bsSolid;
end;

function TcxPivotGridPrefilterCaptionViewInfo.GetAlignment
  : TcxPivotGridPrefilterButtonAlignment;
begin
  Result := pfbaLeft;
end;
...
procedure TcxPivotGridPrefilterCaptionViewInfo.SetDroppedDown
  (AHitTest: TcxPivotGridHitTest; ADroppedDown: Boolean);
begin
...
constructor TcxPivotGridPrefilterPartsViewInfo.Create(APreFilterViewInfo
  : TcxPivotGridPrefilterViewInfo);
begin
  inherited Create;
...
function TcxPivotGridPrefilterPartsViewInfo.GetItem(Index: Integer)
  : TcxPivotGridPrefilterPartViewInfo;
begin
...
procedure TcxPivotGridPrefilterPartsViewInfo.SetDroppedDown
  (AHitTest: TcxPivotGridHitTest; ADroppedDown: Boolean);
begin
...
function TcxPivotGridPrefilterPartsViewInfo.SetHotTrack
  (AHitTest: TcxPivotGridHitTest): Boolean;
var
  I: Integer;
begin
...
function TcxPivotGridPrefilterPartsViewInfo.SetPressed
  (AHitTest: TcxPivotGridHitTest; AMouseDown: Boolean): Boolean;
var
  I: Integer;
begin
...
function TcxPivotGridPrefilterPartsViewInfo.AddItem
  (AItemClass: TcxPivotGridPrefilterPartViewInfoClass)
  : TcxPivotGridPrefilterPartViewInfo;
begin
...
begin
  ...
  ARightMargin := ABounds.Right - ScaleFactor.Apply
    (PrefilterButtonsFirstOffset);
  for I := 0 to Count - 1 do
    with Items[I], ABounds do
      ...
      begin
        ...
        if Alignment = pfbaLeft then
          ALeftMargin := Items[I].Bounds.Right +
            ScaleFactor.Apply(PrefilterButtonsOffset)
        else
          ARightMargin := Items[I].Bounds.Left -
            ScaleFactor.Apply(PrefilterButtonsOffset);
      end;

  ARestWidth := ARightMargin - ALeftMargin + ScaleFactor.Apply
    (PrefilterButtonsOffset);
  if ARestWidth < 0 then
  begin
    ARestWidth := Min(cxRectWidth(FCaptionViewInfo.Bounds), -ARestWidth);
    FCaptionViewInfo.FBounds.Right := FCaptionViewInfo.FBounds.Right -
      ARestWidth;
...
function TcxPivotGridPrefilterPartsViewInfo.GetHitTest
  (AHitTest: TcxPivotGridHitTest): Boolean;
var
  I: Integer;
begin
...
constructor TcxPivotGridPrefilterViewInfo.Create
  (APrefilter: TcxPivotGridPrefilter; APainter: TcxCustomLookAndFeelPainter;
  AScaleFactor: TdxScaleFactor; const ABounds, AVisibleRect: TRect;
  const AViewParams: TcxViewParams);
begin
  inherited Create(APainter, AScaleFactor, ABounds, AVisibleRect, AViewParams);
...
procedure TcxPivotGridPrefilterViewInfo.CorrectBoundsForPrinting
  (ABounds: TRect);
var
  ACorrectedBounds: TRect;
begin
...
procedure TcxPivotGridPrefilterViewInfo.DoRightToLeftConversion
  (const AClientBounds: TRect);
begin
  inherited DoRightToLeftConversion(AClientBounds);
...
function TcxPivotGridPrefilterViewInfo.GetHitTest
  (AHitTest: TcxPivotGridHitTest): Boolean;
begin
  Result := inherited GetHitTest(AHitTest);
  if Result then
...
procedure TcxPivotGridPrefilterViewInfo.SetDroppedDown
  (AHitTest: TcxPivotGridHitTest; ADroppedDown: Boolean);
begin
  FPartsViewInfo.SetDroppedDown(AHitTest, ADroppedDown);
end;

function TcxPivotGridPrefilterViewInfo.SetHotTrack
  (AHitTest: TcxPivotGridHitTest): Boolean;
begin
  Result := FPartsViewInfo.SetHotTrack(AHitTest);
end;

function TcxPivotGridPrefilterViewInfo.SetPressed(AHitTest: TcxPivotGridHitTest;
  AMouseDown: Boolean): Boolean;
begin
...
begin
  ...
  Result.Bounds := cxRectSetXPos(ABounds, ADisplayBounds.Left,
    ADisplayBounds.Right);
...
function TcxPivotGridViewInfo.AddFieldHeader(const ABounds: TRect;
  AField: TcxPivotGridField; AArea: TcxPivotGridFieldArea; AAreaIndex: Integer)
  : TcxPivotGridFieldHeaderCellViewInfo;
begin
  if AField <> nil then
...
function TcxPivotGridViewInfo.AddFieldsBackground(const ABounds: TRect;
  const ADescription: string; AHasDescription: Boolean;
  AArea: TcxPivotGridFieldArea): TcxPivotGridHeaderBackgroundCellViewInfo;
var
  AParams: TcxViewParams;
begin
  ...
  Result := TcxPivotGridHeaderBackgroundCellViewInfo.Create(Painter,
    ScaleFactor, ABounds, FCellsBounds, AParams);
...
function TcxPivotGridViewInfo.AddFilterSeparator(const ATop: Integer)
  : TcxPivotGridFilterSeparatorCellViewInfo;
var
  ABounds: TRect;
begin
  ABounds := cxRectSetTop(FCellsBounds, ATop,
    ScaleFactor.Apply(cxPivotGridFilterSeparatorHeight));
  if PivotGrid.VScrollBarVisible then
    Inc(ABounds.Right, ScaleFactor.Apply(cxTextOffset));
  Result := TcxPivotGridFilterSeparatorCellViewInfo.Create(Painter, ScaleFactor,
    ABounds, ABounds, Styles.GetFilterSeparatorParams);
  CommonCells.Add(Result);
end;

procedure TcxPivotGridViewInfo.AfterRowCellsCalculated
  (ARow: TcxPivotGridViewDataItem);
begin
end;
...
function TcxPivotGridViewInfo.GetRowColumnPos(ALevel: Integer;
  var ALevelField: TPersistent): Integer;
begin
  ...
  begin
    ...
    begin
      ...
      if (FRowColumnPos.Count = 1) and (ALevel = 1) and
        (OptionsDataField.Area = dfaRow) then
...
begin
  ...
  begin
    ...
    else if AIndex >= OptionsDataField.GetActualAreaIndex(False) then
      Dec(AIndex);
  end;
...
function TcxPivotGridViewInfo.GroupHeaderOutOfBounds(AField: TcxPivotGridField;
  ARight: Integer): Boolean;
var
  I, AWidth: Integer;
begin
  ...
  begin
    ...
    for I := 1 to AField.Group.FieldCount - 1 do
      if not AField.Group[I].VisibleInGroup then
        ...
        Inc(AWidth, AField.Group[I].HeaderWidth +
          ScaleFactor.Apply(cxPivotGridSpace));
...
function TcxPivotGridViewInfo.AddPartBackground(ABounds: TRect)
  : TcxPivotGridCustomCellViewInfo;
begin
  Result := nil;
  if IntersectRect(ABounds, ABounds, FCellsBounds) and not IsPrinting then
  begin
    Result := TcxPivotGridCustomCellViewInfo.Create(Painter, ScaleFactor,
      ABounds, FCellsBounds, FViewParams);
    CommonCells.Add(Result);
  end;
end;

function TcxPivotGridViewInfo.AddColumnItem(ABounds: TRect;
  AItem: TcxPivotGridViewDataItem): TcxPivotGridHeaderCellViewInfo;
begin
  Result := TcxPivotGridHeaderCellViewInfo.Create(Painter, ScaleFactor, ABounds,
    ColumnsBounds, BaseStyles.GetColumnHeaderParams(AItem));
  Result.Data := AItem;
  with AItem do
  begin
    Result.Neighbors := GetItemNeighbors(AItem.Level = 0,
      ABounds.Left <= ColumnsBounds.Left, not AItem.HasChildren,
      AItem.GetNextVisible = nil);
    if not AItem.Expanded and (not AItem.HasChildren or (AItem.ItemCount <= 1))
    then
    begin
...
function TcxPivotGridViewInfo.AddDataCell(ARow,
  AColumn: TcxPivotGridViewDataItem; ALeft, ATop: Integer)
  : TcxPivotGridDataCellViewInfo;
var
  ABounds: TRect;
  AField: TcxPivotGridField;
const
  ABorders: array [TcxPivotGridLines] of TcxBorders = ([], [bBottom], [bRight],
    [bRight, bBottom]);
begin
  ...
  if AField <> nil then
    Result := AField.GetCellViewInfoClass.Create(Painter, ScaleFactor, ABounds,
      DataCellsBounds, FViewParams)
  else
    Result := TcxPivotGridDataCellViewInfo.Create(Painter, ScaleFactor, ABounds,
      DataCellsBounds, FViewParams);
...
function TcxPivotGridViewInfo.AddRowItem(ABounds: TRect;
  AItem: TcxPivotGridViewDataItem; ASizeField: TPersistent)
  : TcxPivotGridHeaderCellViewInfo;
begin
  if FNeedCorrectHeaders and (RowsBounds.Top - ABounds.Top >= -1) then
    Dec(ABounds.Top);
  Result := TcxPivotGridHeaderCellViewInfo.Create(Painter, ScaleFactor, ABounds,
    RowsBounds, BaseStyles.GetRowHeaderParams(AItem));
  Result.Data := AItem;
  with AItem do
  begin
    Result.Neighbors := GetItemNeighbors(ABounds.Top <= RowsBounds.Top,
      AItem.Level = 0, AItem.GetNextVisible = nil, not AItem.HasChildren);
...
begin
  ACell.FSelected := not IsPrinting and not PivotGrid.IsDesigning and
    ViewData.IsCellSelected(ACell.RowIndex, ACell.ColumnIndex);
  with FocusedCell do
    AStyle := TcxPivotGridFocusRectStyle((ACell.RowIndex = Y) and
      (ACell.ColumnIndex = X) and ACell.FSelected);
  ...
  ACell.FSelected := ACell.FSelected and
    (PivotGrid.Controller.Focused or
...
begin
  FFilterHeight := 0;
  FDataFieldsWidth := 0;
  AContentHeight := Max(cxTextHeight(GetFont(gs_FieldHeader)),
    Painter.ScaledExpandButtonAreaSize(PivotGrid.ScaleFactor));
  FFieldHeaderHeight := Painter.ScaledHeaderHeight
    (Max(AContentHeight, GetIconsSize.cy), PivotGrid.ScaleFactor);
  for I := 0 to PivotGrid.FieldCount - 1 do
  begin
    AField := PivotGrid.Fields[I];
    AField.ViewInfo.Initialize(Canvas, ScaleFactor, Painter,
      BaseStyles.GetFieldHeaderParams(AField));
  ...
  OptionsDataField.ViewInfo.Initialize(Canvas, ScaleFactor, Painter,
    BaseStyles.GetFieldHeaderParams(nil));
...
procedure TcxPivotGridViewInfo.CreateRows(ARows: TcxPivotList);
  ...
  begin
    ...
    begin
      if IsGroupItemExist(RowHeaders, AItem) then
        Break;
...
begin
  ...
  begin
    Inc(ATop, CreateRow(TcxPivotGridViewDataItem(ARows.List[I]), ATop,
      RowsBounds.Right));
  end;
end;
...
procedure TcxPivotGridViewInfo.CreateColumns(AColumns: TcxPivotList);
  ...
  begin
    ...
    while AItem.Level >= 0 do
    begin
      if IsGroupItemExist(ColumnHeaders, AItem) then
        Break;
      ABounds.Left := ALeft - SpaceBetween(AItem, AStartItem,
        ViewData.ColumnsList);
...
begin
  ...
  begin
    Inc(ALeft, CreateColumn(TcxPivotGridViewDataItem(AColumns.List[I]),
      ColumnsBounds.Bottom, ALeft));
  end;
end;
...
begin
  ...
  begin
    Bottom := Top + PrepareViewDataItems(FRowItems,
      ViewData.Rows[StartRowIndex], cxRectHeight(DataCellsBounds),
      ViewData.FRowIndex);
  ...
  FRowsBounds := cxRectSetLeft(FDataCellsBounds, FCellsBounds.Left,
    RowHeadersWidth);
...
begin
  ...
  begin
    ACell := AddFieldsBackground(DataCellsBounds,
      cxGetResourceString(@scxDropDataItems), (DataBuilder.DataFields.Count = 0)
      and OptionsView.ColumnFields, faData);
...
begin
  ...
  if OptionsView.IsCompactLayout then
    FRowHeadersWidth := ViewData.MaxRowLevel * PivotGrid.RowLevelIndentWidth +
      OptionsView.GetActualItemWidth(False)
  else
  ...
  with OptionsView do
  begin
    Inc(FDataCellsBounds.Top, Max((Byte(DataFields) + Byte(RowFields)) *
      (FFieldHeaderHeight + ScaleFactor.Apply(cxPivotGridSpace) * 2),
      Byte(ColumnFields) * (FFieldHeaderHeight +
      ScaleFactor.Apply(cxPivotGridSpace) * 2) + ColumnHeadersHeight));
  end;
...
begin
  if not OptionsView.FilterFields then
    Exit;
  FFilterHeight := FFieldHeaderHeight + ScaleFactor.Apply(cxPivotGridSpace) * 2;
  AFieldBounds := cxRectSetTop(FCellsBounds, FCellsBounds.Top +
    ScaleFactor.Apply(cxPivotGridSpace), FFieldHeaderHeight);
  ...
  begin
    ...
    if (ARowFieldCount > 0) and GroupHeaderOutOfBounds(AField,
      AFieldBounds.Right) then
    begin
      ...
      Inc(FFilterHeight, FFieldHeaderHeight + ScaleFactor.Apply
        (cxPivotGridSpace));
      ARowFieldCount := 1;
    end
    else
      Inc(ARowFieldCount);
    AFilterBounds := cxRectSetBottom(FCellsBounds,
      FCellsBounds.Top + FFilterHeight, FFieldHeaderHeight);
    AddFieldHeader(AFieldBounds, AField, faFilter, I);
    AFieldBounds.Left := AFieldBounds.Right + ScaleFactor.Apply
      (cxPivotGridHorzSpace);
  end;
  AFilterBounds := cxRectSetHeight(FCellsBounds, FFilterHeight);
  // filter separator cell
  if OptionsView.FilterSeparator then
  begin
    Inc(FFilterHeight, ScaleFactor.Apply(cxPivotGridDoubleSpace) +
      cxPivotGridFilterSeparatorHeight);
  ...
  if OptionsView.FilterSeparator then
    AddFilterSeparator(AFilterBounds.Bottom - ScaleFactor.Apply(cxTextOffset) -
      cxPivotGridFilterSeparatorHeight);
...
begin
  with FDataCellsBounds do
  begin
    Right := Left + PrepareViewDataItems(FColumnItems,
      ViewData.Columns[StartColumnIndex],
    ...
    begin
      ...
      AddPartBackground(Rect(Right, Top - FColumnHeadersHeight,
        Self.Bounds.Right, Bottom));
  end;
  FColumnBounds := cxRectSetBottom(FDataCellsBounds, FDataCellsBounds.Top,
    FColumnHeadersHeight);
...
begin
  ...
  begin
    ...
    FCellsBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FCellsBounds, Bounds);
    FColumnBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FColumnBounds, Bounds);
    FDataCellsBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FDataCellsBounds, Bounds);
    FRowsBounds := TdxRightToLeftLayoutConverter.ConvertRect
      (FRowsBounds, Bounds);

    for I := 0 to FDragDropAreas.Count - 1 do
      (FDragDropAreas[I] as TcxPivotGridDragDropAreaInfo)
...
begin
  ...
  begin
    ...
    begin
      if not(CommonCells[ACellIndex] is TcxPivotGridHeaderBackgroundCellViewInfo)
      then
        Continue;
      ACell := TcxPivotGridHeaderBackgroundCellViewInfo
        (CommonCells[ACellIndex]);
...
function TcxPivotGridViewInfo.GetHeaderDisplayText
  (ACell: TcxPivotGridHeaderCellViewInfo;
  AItem: TcxPivotGridViewDataItem): string;
  ...
  begin
    ...
    else if AItem.GroupItem is TcxPivotGridRowItem then
...
begin
  ...
  begin
    if ACell.Properties <> nil then
      Result := Format((AItem as TcxPivotGridViewDataTotalItem).GetDescription,
        [Result]);
...
procedure TcxPivotGridViewInfo.CalculateColumnsFields;
  ...
  begin
    ...
    begin
      ...
      else if AIndex >= OptionsDataField.GetActualAreaIndex then
        Dec(AIndex);
    end;
...
begin
  ...
  if not OptionsView.ColumnFields then
    Exit;
  AAreaBounds := ABounds;
  AFirstIndex := FieldHeaders.Count;
...
begin
  ...
  if not OptionsView.DataFields then
    Exit;
  AAreaBounds := ABounds;
  AFirstIndex := FieldHeaders.Count;
  ...
  begin
    ...
    if FDataFieldsWidth > (Bounds.Left + RowHeadersWidth) -
      ScaleFactor.Apply(cxPivotGridHorzSpace) then
    begin
      ABounds.Right := ABounds.Left + MulDiv(AField.HeaderWidth,
        RowHeadersWidth - ScaleFactor.Apply(cxPivotGridHorzSpace),
        FDataFieldsWidth);
...
begin
  ...
  if OptionsView.DataFields then
    Inc(ABounds.Top, FieldHeaderHeight + ScaleFactor.Apply
      (cxPivotGridSpace) * 2);
  ...
  begin
    ...
    begin
      if AField <> nil then
        ...
        FRowColumnPos[FRowColumnPos.Count - 1] :=
          Pointer(Bounds.Left + RowHeadersWidth - AWidth);
      ABounds.Left := Bounds.Left + ScaleFactor.Apply(cxPivotGridHorzSpace) +
        MulDiv(RowHeadersWidth - ScaleFactor.Apply(cxPivotGridHorzSpace), I,
        RowFieldsCount);
      ABounds.Right := Bounds.Left + ScaleFactor.Apply(cxPivotGridHorzSpace) +
        MulDiv(RowHeadersWidth - ScaleFactor.Apply(cxPivotGridHorzSpace), I + 1,
        RowFieldsCount) - ScaleFactor.Apply(cxPivotGridHorzSpace);
    end
    else
    begin
      Inc(ALeft, AWidth);
      ABounds.Right := ABounds.Left + AWidth -
        ScaleFactor.Apply(cxPivotGridHorzSpace);
      if I = 0 then
        Dec(ABounds.Right, ScaleFactor.Apply(cxPivotGridHorzSpace));
    end;
...
begin
  ...
  for I := AStartIndex to FieldHeaders.Count - 2 do
    if FieldHeaders[I].Bounds.Top <> FieldHeaders[I + 1].Bounds.Top then
      Inc(ARowCount);
  ARow := 0;
  ABounds := AAreaBounds;
  AIndex := 0;
  ...
  begin
    ...
    R.Left := Max(R.Left - ScaleFactor.Apply(cxPivotGridHalfSpace) - 1,
      AAreaBounds.Left);
    R1.Right := Min(R1.Right + ScaleFactor.Apply(cxPivotGridHalfSpace),
      AAreaBounds.Right);
    APos := R1.Right;
    if (AArea = faRow) and (OptionsView.RowTotalsLocation = rtlTree) then
      AIsLast := (I = FieldHeaders.Count - 1) or
        (ACell.Bounds.Top <> FieldHeaders[I + 1].Bounds.Top) or
        (TcxPivotGridFieldHeaderCellViewInfo(FieldHeaders[I + 1])
        .Field = OptionsDataField) or
        (TcxPivotGridFieldHeaderCellViewInfo(FieldHeaders[I])
        .Field = OptionsDataField)
    else
      AIsLast := (I = FieldHeaders.Count - 1) or
    ...
    AIsLastPreviousDataField := AIsLast and (AArea = faRow) and
      (OptionsView.RowTotalsLocation = rtlTree) and
      (TcxPivotGridFieldHeaderCellViewInfo(FieldHeaders[I])
      .Field = OptionsDataField);
  ...
  begin
    R := cxRectInflate(AAreaBounds, -ScaleFactor.Apply(cxPivotGridHorzSpace),
      -ScaleFactor.Apply(cxPivotGridHorzSpace) * 2);
    AddDragDropAreaInfo(R.Left, AAreaBounds, R, AArea, 0, nil);
  end
  else if (AArea <> faRow) or (OptionsView.RowTotalsLocation <> rtlTree) then
    AddDragDropAreaInfo(ABounds.Right + ScaleFactor.Apply(cxPivotGridHalfSpace),
      AAreaBounds, cxRectSetYPos(AAreaBounds, R.Top, R.Bottom), AArea,
      AIndex, nil);
end;

function TcxPivotGridViewInfo.IsGroupItemExist(AList: TList;
  AItem: TcxPivotGridViewDataItem): Boolean;
var
  I: Integer;
begin
...
function TcxPivotGridViewInfo.PrepareViewDataItems(var AList: TcxPivotList;
  AStartItem: TcxPivotGridViewDataItem; AAvailableSize: Integer;
  var AStartIndex: Integer): Integer;
var
  AItem: TcxPivotGridViewDataItem;
...
function TcxPivotGridViewInfo.SpaceBetween(AItem1,
  AItem2: TcxPivotGridViewDataItem; AList: TList): Integer;
var
  I: Integer;
begin
  Result := 0;
...
begin
  ...
  begin
    ...
    if FieldHeaderImages <> nil then
      Result := cxSizeMax(Result, dxGetImageSize(FieldHeaderImages,
        ScaleFactor))
  end;
end;
...
constructor TcxPivotGridViewDataSelection.Create
  (AViewData: TcxPivotGridViewData);
begin
  inherited Create;
  FViewData := AViewData;
...
procedure TcxPivotGridViewDataSelection.EndUpdate(ASendNotification
  : Boolean = True);
begin
  Dec(FLockCount);
...
begin
  ...
  for I := 0 to Count - 1 do
  begin
    if EqualRect(Regions[I], cxInvalidRect) then
      Continue;
    with Result do
    begin
...
function TcxPivotGridViewDataSelection.IsCellSelected(AColumn,
...
begin
  if EqualRect(ARegion, cxInvalidRect) or
    ((Count = 1) and EqualRect(ARegion, Regions[0])) then
    Exit;
  BeginUpdate;
  try
    InnerClear;
...
procedure TcxPivotGridViewDataSelection.ApplyTemporarySelection
  (AIsSet: Boolean);
begin
  FHasTemporarySelection := False;
end;

procedure TcxPivotGridViewDataSelection.ChangeSelection(const R: TRect;
  AShift: TShiftState);
begin
  ...
  begin
    ...
    begin
      BeginUpdate;
      try
        if not IsTemporarySelected and
          (not(ssCtrl in AShift) or (ssRight in AShift)) then
          InnerClear;
...
procedure TcxPivotGridViewDataSelection.InnerSetFocusedCell
  (const APoint: TPoint);
begin
  FFocusedCell := APoint;
end;
...
var
  ...
  function FindValidateCell(ADelta: Integer;
    var ARow, AColumn: Integer): Boolean;
...
begin
  ...
  begin
    ...
    AMaxIteration := Max(Max(ViewData.RowCount - 1 - AOldFocusedCell.Y,
      AOldFocusedCell.Y), Max(ViewData.ColumnCount - 1 - AOldFocusedCell.X,
      AOldFocusedCell.X));
  ...
  if not IsCellSelected(FFocusedCell.X, FFocusedCell.Y) then
    if not MultiSelect or
      ((Count = 1) and EqualRect(Classes.Rect(AOldFocusedCell, AOldFocusedCell),
      Regions[0])) then
...
begin
  Result := FHasTemporarySelection and not EqualRect(TemporarySelection,
    cxInvalidRect);
end;
...
procedure TcxPivotGridViewDataSelection.SetTemporarySelection
  (const AValue: TRect);
begin
  ...
  begin
    if FHasTemporarySelection and (Count > 0) then
      ...
      Add(AValue);
    FHasTemporarySelection := FHasTemporarySelection or
      not EqualRect(AValue, cxInvalidRect);
    DoChanged;
  end;
end;
...
function TcxPivotGridSortBySummaryInfo.ConditionDefined
  (ADataField: TcxPivotGridField; AGroupItem: TcxPivotGridGroupItem): Boolean;
begin
  ...
  begin
    if (AGroupItem.Field <> nil) and not(AGroupItem is TcxPivotGridDataItem)
    then
...
function TcxPivotGridSortBySummaryInfo.GetCrossItemForSorting(ACrossGrandTotal
  : TcxPivotGridGroupItem): TcxPivotGridGroupItem;
begin
...
procedure TcxPivotGridSortBySummaryInfo.SetField(AValue: TcxPivotGridField);
begin
...
constructor TcxPivotGridCrossCellDataSource.Create
...
function TcxPivotGridCrossCellDataSource.GetPivotGridField(AIndex: Integer)
  : TcxPivotGridField;
begin
...
function TcxPivotGridCrossCellDataSource.GetValue(ARecordHandle
...
begin
  ...
  begin
    if Provider <> nil then
      AField := PivotGridFields[TcxPivotGridDataController(DataController)
        .GetItemID(DataController.GetItem(Integer(AItemHandle)))]
    else
...
begin
  ...
  SubItems := TDictionary<TcxPivotGridGroupItemHelper, TcxPivotGridGroupItem>.
    Create(20, PivotGrid.GroupItemValueComparer);
end;
...
procedure TcxPivotGridGroupItemHelper.AddSubItem
...
function TcxPivotGridGroupItemHelper.IsEqual
  (AValue: TcxPivotGridGroupItemHelper): Boolean;
begin
...
function TcxPivotGridGroupItemHelper.Compare
  (AValue: TcxPivotGridGroupItemHelper): Integer;
begin
...
constructor TcxPivotGridGroupItemValueHelperComparer.Create
  (AOwner: TcxCustomPivotGrid);
begin
  FOwner := AOwner;
end;

function TcxPivotGridGroupItemValueHelperComparer.Equals(const Left,
  Right: TcxPivotGridGroupItemHelper): Boolean;
begin
  Result := Left.IsEqual(Right);
end;

function TcxPivotGridGroupItemValueHelperComparer.GetHashCode
  (const Value: TcxPivotGridGroupItemHelper): Integer;
begin
...
function TcxPivotGridGroupItem.AddChild(AClass: TcxPivotGridGroupItemClass)
  : TcxPivotGridGroupItem;
begin
...
function TcxPivotGridGroupItem.AddChild(const AGroupValue: Variant;
  ARecordIndex: Integer; AField: TcxPivotGridField): TcxPivotGridGroupItem;
begin
  Result := ChildItemsClass.Create(Self, AGroupValue, ARecordIndex,
    DataController, AField);
...
begin
  FExpanded := FExpanded or
    ((Field <> nil) and (Field.IsItemExpanded(Self) or
    Field.Options.AlwaysExpanded));
end;
...
begin
  ...
  begin
    ...
    begin
      for I := 0 to PivotGrid.RecordCount - 1 do
        if PivotGrid.DataBuilder.FilteredIndexes[I] = cxPivotGridRecordVisible
        then
        begin
          AList[ACount] := I;
          Inc(ACount);
    ...
    begin
      ...
      repeat
        ...
        begin
          ...
          if Integer(PivotGrid.DataBuilder.FilteredIndexes.List[ARecNo]) = cxPivotGridRecordVisible
          then
          begin
            AList[ACount] := ARecNo;
            Inc(ACount);
...
function TcxPivotGridGroupItem.GetCellByCrossItem(AItem: TcxPivotGridGroupItem)
  : TcxPivotGridCrossCell;
begin
  Result := nil;
end;
...
procedure TcxPivotGridGroupItem.InitializeValue(const ADisplayText: string;
  const AnUniqueName: WideString);
begin
  FDisplayText := ADisplayText;
...
procedure TcxPivotGridGroupItem.RemoveChildrenFrom
  (AItem: TcxPivotGridGroupItem);
var
  AIndex, I: Integer;
  ASubItem: TcxPivotGridGroupItem;
begin
  ...
  begin
    ...
    if not FindItem(ItemList, ASubItem.Value, AIndex,
      ASubItem.Field.ActuallySortOrder) then
    begin
...
procedure TcxPivotGridGroupItem.InitSummaryValue(ACrossTotal
  : TcxPivotGridGroupItem);
var
  ACrossCell: TcxPivotGridCrossCell;
begin
  ...
  if Field.SortBySummaryInfo.Field.SummaryVariation <> svNone then
    FSummaryValue := ACrossCell.GetSummaryCell
      (Field.SortBySummaryInfo.Field.SummaryIndex).SummaryVariation
  else
    FSummaryValue := ACrossCell.GetSummaryByField(Field.SortBySummaryInfo.Field,
      Field.SortBySummaryInfo.SummaryType)
end;
...
procedure TcxPivotGridGroupItem.PostProcessGroup(ACrossGrandTotal
  : TcxPivotGridGroupItem);
var
  I: Integer;
...
begin
  ...
  if Items[0].Field <> nil then
    ACrossGroupValue := Items[0].Field.SortBySummaryInfo.GetCrossItemForSorting
      (ACrossGrandTotal)
  else
...
procedure TcxPivotGridGroupItem.ProcessTopValues(ACrossTotal
  : TcxPivotGridGroupItem);
var
  I, ACount: Integer;
...
begin
  ...
  begin
    ...
    begin
      AOthers := ChildItemsClass.Create(Self, DataController, Items[0].Field,
        cxPivotGridOthersRecordIndex);
    end
    else
      FSortingValid := False;
...
function TcxPivotGridGroupItem.Find(AValue: TcxPivotGridGroupItemHelper;
  var AItem: TcxPivotGridGroupItem): Boolean;
begin
...
begin
  Result := TcxPivotGridGroupItem(AItem1)
    .FHelper.Compare(TcxPivotGridGroupItem(AItem2).FHelper);
end;
...
function TcxPivotGridGroupItem.FindItemByValue(const AValue: Variant;
  var AItem: TcxPivotGridGroupItem): Boolean;
var
  I: Integer;
begin
...
procedure TcxPivotGridGroupItem.SetFieldExpanding(AField: TcxPivotGridField;
  AExpandState: Boolean);
var
  I: Integer;
begin
...
constructor TcxPivotGridRowItem.Create(AParent: TcxPivotGridGroupItem;
  const AGroupValue: Variant; ARecordIndex: Integer;
  ADataController: TcxCustomDataController; AField: TcxPivotGridField);
begin
...
function TcxPivotGridRowItem.GetCellByCrossItem(AItem: TcxPivotGridGroupItem)
  : TcxPivotGridCrossCell;
begin
...
function TcxPivotGridColumnItem.GetCellByCrossItem(AItem: TcxPivotGridGroupItem)
  : TcxPivotGridCrossCell;
begin
...
function TcxPivotGridAdvancedDataGroupIntervalItem.GetGroupValue
  (const AValue: Variant): Variant;
begin
  Result := FGetGroupValueFunc(AValue);
end;

function TcxPivotGridAdvancedDataGroupIntervalItem.GetGroupValueDisplayText
  (const AValue: Variant): string;
begin
...
function TcxPivotGridAdvancedDataGroupIntervalItem.GetDateTimeValue
  (const AValue: Variant; var ADateTime: TDateTime): Boolean;
begin
...
function TcxPivotGridAdvancedDataGroupIntervalItem.HalfOfTheYear
  (const AValue: TDateTime): Integer;
begin
  Result := (MonthOf(AValue) - 1) div 6 + 1;
end;

function TcxPivotGridAdvancedDataGroupIntervalItem.QuarterOfTheYear
  (const AValue: TDateTime): Integer;
begin
...
begin
  AIndex := TcxPivotGridAdvancedDataGroupIntervalRepository.FindItemByName
    (GetName, APrevItem);
  if AIndex >= 0 then
  begin
...
class function TcxPivotGridAdvancedDataGroupIntervalRepository.GetCount
  : Integer;
begin
  Result := FItems.Count;
end;

class function TcxPivotGridAdvancedDataGroupIntervalRepository.GetItem
  (Index: Integer): TcxPivotGridAdvancedDataGroupIntervalItem;
begin
  Result := FItems[Index];
end;

class function TcxPivotGridAdvancedDataGroupIntervalRepository.GetItemByID
  (ID: Integer): TcxPivotGridAdvancedDataGroupIntervalItem;
var
  I: Integer;
begin
  Result := nil;
...
class function TcxPivotGridAdvancedDataGroupIntervalRepository.GetItemByName
  (const ADisplayName: string): TcxPivotGridAdvancedDataGroupIntervalItem;
begin
  if FindItemByName(ADisplayName, Result) < 0 then
    Result := nil;
end;

class function TcxPivotGridAdvancedDataGroupIntervalRepository.Register
...
class procedure TcxPivotGridAdvancedDataGroupIntervalRepository.Unregister
  (ID: Integer);
var
  I: Integer;
begin
...
class function TcxPivotGridAdvancedDataGroupIntervalRepository.
  GetNextID: Integer;
begin
  Inc(FID);
  Result := FID;
...
function TcxPivotGridAdvancedDataGroupIntervalRepository.
  TMonthYearGroupInterval.GetGroupValue(const AValue: Variant): Variant;
var
  ADate: TDateTime;
begin
...
function TcxPivotGridAdvancedDataGroupIntervalRepository.
  TMonthYearGroupInterval.GetGroupValueDisplayText(const AValue
  : Variant): string;
begin
  if VarIsSoftNumeric(AValue) then
    Result := dxFormatSettings.LongMonthNames[MonthOf(AValue)] + ', ' +
      IntToStr(YearOf(AValue))
  else
    Result := '';
end;

function TcxPivotGridAdvancedDataGroupIntervalRepository.
  TMonthYearGroupInterval.GetName: string;
begin
  Result := 'Month, Year';
end;
...
function TcxPivotGridAdvancedDataGroupIntervalRepository.
  TQuarterYearGroupInterval.GetGroupValue(const AValue: Variant): Variant;
var
  Y, M, D: Word;
  ADate: TDateTime;
begin
...
function TcxPivotGridAdvancedDataGroupIntervalRepository.
  TQuarterYearGroupInterval.GetGroupValueDisplayText(const AValue
  : Variant): string;
begin
  if VarIsSoftNumeric(AValue) then
    Result := Format(cxGetResourceString(@scxQuarterFormat),
      [QuarterOfTheYear(AValue)]) + ', ' + IntToStr(YearOf(AValue))
  else
    Result := ''
end;

function TcxPivotGridAdvancedDataGroupIntervalRepository.
  TQuarterYearGroupInterval.GetName: string;
begin
  Result := 'Quarter, Year';
end;
...
function TcxPivotGridAdvancedDataGroupIntervalRepository.THalfYearGroupInterval.
  GetGroupValue(const AValue: Variant): Variant;
var
  Y, M, D: Word;
  ADate: TDateTime;
begin
...
function TcxPivotGridAdvancedDataGroupIntervalRepository.THalfYearGroupInterval.
  GetGroupValueDisplayText(const AValue: Variant): string;
begin
  if VarIsSoftNumeric(AValue) then
    Result := Format(cxGetResourceString(@scxHalfFormat),
      [HalfOfTheYear(AValue)]) + ', ' + IntToStr(YearOf(AValue))
  else
    Result := ''
end;
...
var
  I: Integer;
const
  MenuResNames: array [0 .. 8] of string = ('CXPIVOT_POPUPMENU_COUNT',
    'CXPIVOT_POPUPMENU_SUM', 'CXPIVOT_POPUPMENU_MIN', 'CXPIVOT_POPUPMENU_MAX',
    'CXPIVOT_POPUPMENU_AVERAGE', 'CXPIVOT_POPUPMENU_FIELDLIST',
    'CXPIVOT_POPUPMENU_REFRESHDATA', 'CXPIVOT_POPUPMENU_EXPANDALL',
    'CXPIVOT_POPUPMENU_COLLAPSEALL');
begin
  ...
  for I := 0 to High(MenuResNames) do
    PivotGridError((InternalMenuImages.AddImageFromResource(HInstance,
      MenuResNames[I], 'SVG') >= 0) or True,
      Format('Invalid %d Resource Image', [I]));
end;
...
dxUnitsLoader.AddUnit(@InitializePivotGrid, @FinalizePivotGrid);

finalization

dxUnitsLoader.RemoveUnit(@FinalizePivotGrid);
