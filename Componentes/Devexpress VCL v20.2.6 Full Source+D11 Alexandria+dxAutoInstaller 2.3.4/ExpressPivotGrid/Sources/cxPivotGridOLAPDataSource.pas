...
uses
  Types, Classes, SysUtils, Windows, Forms, Contnrs, Math, DB, ActiveX, dxCore,
  cxClasses,
  cxCustomPivotGrid, cxPivotGridOLAPQueryBuilder, cxCustomData, cxPivotGridStrs,
...
{$REGION 'internal types'}
  TcxPivotGridOLAPSchemaInfo = (osiLevels, osiMeasures, osiKPIs, osiSets,
    osiHierarchies);
  ...
  protected
    ...
    function FieldByName(const AFieldName: string)
      : TcxPivotGridOLAPDBDataSetField;
    function FindField(const AFieldName: string)
      : TcxPivotGridOLAPDBDataSetField;
    ...
    property Fields[Index: Integer]: TcxPivotGridOLAPDBDataSetField
      read GetItem;
...
{$ENDREGION}
  ...
  public
    constructor Create;
    destructor Destroy; override;
    function AddItem(const AUniqueName, ADisplayText: string;
      AValueIsNull: Boolean): Integer;
  ...
  protected
    class procedure SetCrossCellFormattedValue(ACell: TcxPivotGridCrossCell;
      ASummaryCellIndex: Integer; const AFormattedValue: Variant);
    class procedure SetCrossCellValues(ACell: TcxPivotGridCrossCell;
      const AFormattedValues, ANativeValues: Variant);
    class procedure SetCrossCellValue(Cell: TcxPivotGridCrossCell;
      ASummaryCellIndex: Integer; const AFormattedValue, ANativeValue: Variant);
...
{$ENDREGION}
  ...
  TcxCustomPivotGridOLAPDrillDownDataSource = class
  ...
  TcxCustomPivotGridOLAPDrillDownDataSourceClass = class of
    TcxCustomPivotGridOLAPDrillDownDataSource;

  { TcxPivotGridOLAPDrillDownDataSource }

  TcxPivotGridOLAPDrillDownDataSource = class
    (TcxCustomPivotGridOLAPDrillDownDataSource)
  strict private
  ...
  protected
    ...
    property DataSet: TcxPivotGridOLAPDBDataSet read GetDataSet
      write SetDataSet;
  public
    destructor Destroy; override;
  end;
  ...
  TcxPivotGridOLAPArrayDrillDownDataSource = class
    (TcxCustomPivotGridOLAPDrillDownDataSource)
  protected type
    TDataSet = class
    private
  ...
  protected
    ...
    property DataSet: TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet
  ...
  protected
    ...
    function Execute(const ACommandText: string): TcxCustomPivotGridOLAPDataSet;
      overload; virtual; abstract;
    function Execute(const AQuery: TcxMDXQuery): TcxCustomPivotGridOLAPDataSet;
      overload; virtual; abstract;
    function GetConnected: Boolean; virtual; abstract;
    function GetDrillDownDataSourceClass
      : TcxCustomPivotGridOLAPDrillDownDataSourceClass; virtual; abstract;
    procedure PopulateCatalogs(ANames: TStrings); virtual; abstract;
    procedure PopulateHierarchyAllMemberNames(const ACubeName: string;
      ANames: TDictionary<string, string>); virtual; abstract;
    function OpenSchema(const ACubeName: string;
    ...
    function CreateDrillDownDataSet(const ACommandText: string;
      ACrossCell: TcxPivotGridCrossCell; ASummaryIndex: Integer): TObject;
      virtual; abstract;
    function CreateQueryBuilder: TcxCustomOLAPDataQueryBuilder;
      virtual; abstract;
    function CreateCrossCellsReader(ADataSet: TcxCustomPivotGridOLAPDataSet;
      AOwnedGroup, ARowGroups, AColumnGroups: TcxPivotGridGroupItem;
      ASummaryFields: TcxPivotGridFields): TcxCustomPivotGridOLAPDataSetReader;
      virtual; abstract;
    function CreateGrandTotalsReader(ADataSet: TcxCustomPivotGridOLAPDataSet;
      AColumnGroups, ARowGroups: TcxPivotGridGroupItem;
      ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList)
      : TcxCustomPivotGridOLAPDataSetReader; virtual; abstract;
    function CreateMainGrandTotalReader(ADataSet: TcxCustomPivotGridOLAPDataSet;
      AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
      ADataFields: TcxPivotGridFields): TcxCustomPivotGridOLAPDataSetReader;
      virtual; abstract;
    function CreateMembersReader(ADataSet: TcxCustomPivotGridOLAPDataSet;
      AGroupValues: TcxPivotGridOLAPGroupInfos; AField: TcxPivotGridField)
      : TcxCustomPivotGridOLAPDataSetReader; virtual; abstract;

    property Connected: Boolean read GetConnected write SetConnected;
    property CacheLocalCubes: Boolean read FCacheLocalCubes
      write SetCacheLocalCubes default True;
  ...
  TcxPivotGridAddOLAPStructureItemProc = function(AParent: Pointer;
    const ACaption: string; const ANameIndex, AImageIndex: Integer)
    : Pointer of object;
  ...
  strict private
    ...
    procedure ExpandedItemWithParentsToList(AnGroup: TcxPivotGridGroupItem;
      AList: TList);
    function GetActive: Boolean;
    function GetDrillDownCommandText(ACell: TcxPivotGridCrossCell;
      ASummaryIndex: Integer): string;
    function IsDataValid(ADataSet: TcxCustomPivotGridOLAPDataSet)
      : Boolean; overload;
    procedure SetActive(AValue: Boolean);
    procedure SetCube(const AValue: string);
    procedure SetProviderClass(const Value
  ...
  protected
    ...
    function CreateNode(AParent: TcxPivotGridOLAPStructureNode;
      const ACaption: string; ANodeType: TcxOLAPStructureNodeType)
      : TcxPivotGridOLAPStructureNode;
    procedure CreateRootLayout(APivotGrid: TcxCustomPivotGrid;
      ARowFields, AColumnFields, ASummaryFields, AFilterFields
      : TcxPivotGridFields); override;
    procedure CreateStructure; override;
    procedure DoPopulateGroupValues(AGroupValues: TcxPivotGridOLAPGroupInfos;
      AField: TcxPivotGridOLAPField; AnExpandedMembers: TList;
    ...
    procedure PopulateChildrenGroups(AParentGroup: TcxPivotGridGroupItem;
      AField: TcxPivotGridField; AnExpandedMembers: TList;
      AHasChildren: Boolean; ADataFields, AFilterFields: TcxPivotGridFields);
    procedure PopulateGrandTotals(AColumnGroups,
      ARowGroups: TcxPivotGridGroupItem; AColumnFields, ASummaryFields,
      AFilterFields: TcxPivotGridFields; AnExpandedMembers: TList);
    procedure PopulateGroupValues(AField: TcxPivotGridOLAPField;
      AList: TcxPivotGridVariantList); override;
    procedure PopulateCrossCells(AOwnedGroup, ARowGroups,
      AColumnGroups: TcxPivotGridGroupItem; ARowFields, AColumnFields,
      ASummaryFields, AFilterFields: TcxPivotGridFields);

    function CreateDrillDownDataSet(ACells: TList;
    ...
    function ExecuteQuery(const AQuery: string)
      : TcxCustomPivotGridOLAPDataSet; overload;
    function ExecuteQuery(AQuery: TcxMDXQuery)
      : TcxCustomPivotGridOLAPDataSet; overload;
    ...
    function GetNonExpandedFields(ADataBuilder: TcxPivotGridDataBuilder;
      AnExpandingGroup, AnGroup: TcxPivotGridGroupItem; ACrossGroupItems: TList;
      AddHierarchyField: Boolean): TcxPivotGridFields;
    procedure Initialize; override;
    procedure InitializeExpanding(ADataBuilder: TcxPivotGridDataBuilder;
      AField: TcxPivotGridField;
      AnExpandingGroup, ACrossGroup: TcxPivotGridGroupItem;
      ASummaryFields: TcxPivotGridFields); override;
    procedure PopulateMainGrandTotal(ADataFields, AFilterFields
      : TcxPivotGridFields; AColumns, ARows: TcxPivotGridGroupItem);
    function IsValueVisible(AFilter: TcxPivotGridFieldFilter;
    ...
    procedure PopulateFilteredUniqueNames(AField: TcxPivotGridOLAPField;
      ...
      var AUniqueValues: TStringList); override;
    procedure PopulateFilteredValues(AField: TcxPivotGridOLAPField;
      AFilter: TcxPivotGridFieldFilter; AValues: TStrings;
      AUniqueValues: TStringList); override;
    procedure TerminateConnection;

    property IsCubeNameAssigned: Boolean read FIsCubeNameAssigned
      write FIsCubeNameAssigned;
    property QueryBuilder: TcxCustomOLAPDataQueryBuilder read FQueryBuilder
      write FQueryBuilder;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;

    property ProviderClass: TcxCustomPivotGridOLAPProviderClass
      read FProviderClass write SetProviderClass;
  published
    property ProviderClassName: string read GetProviderClassName
      write SetProviderClassName;
    property Provider: TcxCustomPivotGridOLAPProvider read FProvider
      write SetProvider;
    property ConnectionString: string read GetConnectionString
      write SetConnectionString stored False; // obsolete
...
procedure cxPivotGridOLAPDataSourceRegisterProvider
  (AClass: TcxCustomPivotGridOLAPProviderClass; const AName: string);
begin
  cxGetPivotGridOLAPDataSourceProviders.Register(AClass, AName);
end;

procedure cxPivotGridOLAPDataSourceUnregisterProvider
  (AClass: TcxCustomPivotGridOLAPProviderClass);
begin
...
function TcxPivotGridOLAPDBDataSet.FieldByName(const AFieldName: string)
  : TcxPivotGridOLAPDBDataSetField;
begin
...
function TcxPivotGridOLAPDBDataSet.FindField(const AFieldName: string)
  : TcxPivotGridOLAPDBDataSetField;
var
  I: Integer;
begin
...
begin
  ...
  for I := 0 to InnerDataSet.FieldCount - 1 do
    OLAPFields.Add(TcxPivotGridOLAPDBDataSetField.Create
      (InnerDataSet.Fields[I]));
end;

function TcxPivotGridOLAPDBDataSet.IsValid: Boolean;
begin
  Result := InnerDataSet.Active and (InnerDataSet.RecordCount > 0) and
    (InnerDataSet.FieldCount > 0);
end;
...
function TcxPivotGridOLAPDBDataSet.GetItem(Index: Integer)
  : TcxPivotGridOLAPDBDataSetField;
begin
...
function TcxPivotGridOLAPGroupInfos.AddItem(const AUniqueName,
  ADisplayText: string; AValueIsNull: Boolean): Integer;
var
  AItem: TInfo;
begin
  AItem := TInfo.Create;
...
constructor TcxCustomPivotGridOLAPDataSetReader.Create
...
class procedure TcxCustomPivotGridOLAPDataSetReader.SetCrossCellFormattedValue
  (ACell: TcxPivotGridCrossCell; ASummaryCellIndex: Integer;
  const AFormattedValue: Variant);
var
  I: Integer;
begin
  for I := 0 to Integer(High(TcxPivotGridSummaryType)) do
    TcxPivotGridCrossCellSummaryAccess(ACell.SummaryCells[ASummaryCellIndex])
      .SetSummaryByIndex(I, AFormattedValue);
end;

class procedure TcxCustomPivotGridOLAPDataSetReader.SetCrossCellValues
  (ACell: TcxPivotGridCrossCell; const AFormattedValues, ANativeValues
  : Variant);
var
  I: Integer;
begin
...
class procedure TcxCustomPivotGridOLAPDataSetReader.SetCrossCellValue
  (Cell: TcxPivotGridCrossCell; ASummaryCellIndex: Integer;
...
begin
  ...
  if not VarIsArray(ACell.NativeValues) then
    ACell.NativeValues := VarArrayCreate([0, ACell.SummaryCellCount - 1],
      varVariant);
...
constructor TcxCustomPivotGridOLAPProvider.Create
  (ADataSource: TcxPivotGridOLAPDataSource);
begin
  inherited Create;
...
function TcxCustomPivotGridOLAPProvider.CreateTemporaryCube
  (const AOriginalCubeFilePath: string): Boolean;
var
...
function TcxCustomPivotGridOLAPProvider.ExtractCubeName(const AConnectionString
  : string): string;
const
...
begin
  ...
  AEndPos := APos;
  while (AEndPos < Length(AConnectionString)) and
    (AConnectionString[AEndPos] <> ';') do
    Inc(AEndPos);
...
procedure TcxCustomPivotGridOLAPProvider.SetCacheLocalCubes
  (const Value: Boolean);
begin
...
procedure TcxCustomPivotGridOLAPProvider.SetConnectionString
  (const Value: string);
begin
...
procedure TcxCustomPivotGridOLAPDrillDownDataSource.SetDataSet
  (const Value: TObject);
begin
  FreeAndNil(FDataSet);
  FDataSet := Value;
...
function TcxCustomPivotGridOLAPDrillDownDataSource.
  TryGetPivotGridFieldByFieldName(const ADataSetFieldName: string;
  out APivotGridField: TcxPivotGridField): Boolean;
var
...
begin
  ...
  for I := 0 to PivotGrid.FieldCount - 1 do
    if IsUniqueNamesEqual(PivotGrid.Fields[I].UniqueName, ADataSetFieldName,
      ACurrentCoincidenceCount) and
...
function TcxPivotGridOLAPDrillDownDataSource.GetDataSet
  : TcxPivotGridOLAPDBDataSet;
begin
  Result := TcxPivotGridOLAPDBDataSet(inherited DataSet);
end;

procedure TcxPivotGridOLAPDrillDownDataSource.SetDataSet
  (const Value: TcxPivotGridOLAPDBDataSet);
begin
...
begin
  ...
  if HasData and (DataSet <> nil) then
    for I := 0 to DataSet.FieldCount - 1 do
      if TryGetPivotGridFieldByFieldName(DataSet.Fields[I].FullName,
        APivotGridField) then
      begin
...
function TcxPivotGridOLAPDrillDownDataSource.GetPivotGridField(AIndex: Integer)
  : TcxPivotGridField;
begin
...
function TcxPivotGridOLAPDrillDownDataSource.GetValue(ARecordHandle
...
begin
  ...
  if HasData and (DataSet <> nil) then
    for I := 0 to DataSet.FieldCount - 1 do
      if TryGetPivotGridFieldByFieldName(DataSet.FieldName[I], APivotGridField)
      then
      begin
...
function TcxPivotGridOLAPArrayDrillDownDataSource.GetPivotGridField
...
function TcxPivotGridOLAPArrayDrillDownDataSource.GetValue
...
function TcxPivotGridOLAPArrayDrillDownDataSource.GetDataSet
  : TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet;
begin
  Result := TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet
    (inherited DataSet);
end;
...
begin
  inherited DataSet :=
    Value as TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet;
end;

{ TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet }

constructor TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet.Create
  (ARowCount: Integer);
begin
  inherited Create;
...
procedure TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet.AddField
...
function TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet.
  GetFieldCount: Integer;
begin
  Result := Length(FFields);
end;

function TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet.GetRow
...
procedure TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet.SetRow
  (Index: Integer; const Value: Variant);
begin
  FRows[Index] := Value;
end;

function TcxPivotGridOLAPArrayDrillDownDataSource.TDataSet.GetFieldName
...
var
  ...
  ACardinality, AStructure, AFolder, ACaption, ADimensionUniqueName,
    AHierarchyUniqueName: TcxPivotGridOLAPDBDataSetField;
begin
    ...
    begin
      ...
      try
        ...
        begin
          ...
          ANode.LevelUniqueName := ANode.HierarchyUniqueName + '.[' +
            GetLastNameFromPath(ANode.HierarchyUniqueName) + ']';
          ADataSet.Next;
        end;
        AddLevels(ALevels);
      finally
...
var
  ...
  ADimensionUniqueName, AUniqueName, AHierarchyUniqueName, AName, ACardinality,
    ADBType, ANumber, AType: TcxPivotGridOLAPDBDataSetField;
begin
...
procedure TcxPivotGridOLAPDataSource.AddMeasures;
  ...
  begin
    ...
    if AName = 'Currency' then
      Result := fnCurrency
    else if AName = 'Fixed' then
      Result := fnFixed
    else if AName = 'Percent' then
      Result := fnPercent
    else if AName = 'General Number' then
      Result := fnGeneralNumber
    else if AName = 'Standard' then
      Result := fnStandard
    else if AName = 'Scientific' then
      Result := fnScientific
    else if AName = 'Yes/No' then
      Result := fnYesNo
    else if AName = 'True/False' then
      Result := fnTrueFalse
    else if AName = 'On/Off' then
      Result := fnOnOff
    else if AName <> '' then
      Result := fnCustom;
  end;
...
var
  I: Integer;
  ADataSet: TcxPivotGridOLAPDBDataSet;
  AFolder, ACaption, AUniqueName, AMeasureDisplayFolder, ADefaultFormatString
    : TcxPivotGridOLAPDBDataSetField;
...
begin
  ...
  try
    if IsDataValid(ADataSet) then
    begin
      AMeasures := CreateNode(Structure, cxGetResourceString(@scxMeasures),
        ntMeasure);
...
const
  KPI_Names: array [TcxOLAPKPIType] of string = ('KPI_VALUE', 'KPI_GOAL',
    'KPI_STATUS', 'KPI_TREND', 'KPI_WEIGHT');

  function GetGraphicType(AField: TcxPivotGridOLAPDBDataSetField)
  ...
  try
    ...
    begin
      ...
      try
        ...
        AGraphicTypes.AddObject('Traffic Light - Single',
          TObject(gtTrafficLights));
        AGraphicTypes.AddObject('Traffic Light - Multiple',
          TObject(gtTrafficLights));
        ...
        AGraphicTypes.AddObject('Status Arrow - Ascending',
          TObject(gtStatusArrow));
        AGraphicTypes.AddObject('Status Arrow - Descending',
          TObject(gtReversedStatusArrow));
        for I := 0 to ADataSet.RecordCount - 1 do
        begin
...
function TcxPivotGridOLAPDataSource.CreateNode
  (AParent: TcxPivotGridOLAPStructureNode; const ACaption: string;
  ANodeType: TcxOLAPStructureNodeType): TcxPivotGridOLAPStructureNode;
begin
...
procedure TcxPivotGridOLAPDataSource.CreateRootLayout
  (APivotGrid: TcxCustomPivotGrid; ARowFields, AColumnFields, ASummaryFields,
...
begin
  ...
  if ARowFields.Count > 0 then
    PopulateChildrenGroups(ARowGroup, ARowFields[0], nil, ARowFields.Count > 1,
      ASummaryFields, AFilterFields);
  if AColumnFields.Count > 0 then
    PopulateChildrenGroups(AColumnGroup, AColumnFields[0], nil,
      AColumnFields.Count > 1, ASummaryFields, AFilterFields);
  if ASummaryFields.Count > 0 then
  begin
    if (ARowGroup.ItemCount > 0) and (AColumnGroup.ItemCount > 0) then
      PopulateCrossCells(AColumnGroup, ARowGroup, AColumnGroup, ARowFields,
        AColumnFields, ASummaryFields, AFilterFields);
    if (AColumnGroup.ItemCount > 0) then
      PopulateGrandTotals(AColumnGroup, ARowGroup, AColumnFields,
        ASummaryFields, AFilterFields, nil);
    if (ARowGroup.ItemCount > 0) then
      PopulateGrandTotals(ARowGroup, AColumnGroup, ARowFields, ASummaryFields,
        AFilterFields, nil);
  end;
  PopulateMainGrandTotal(ASummaryFields, AFilterFields, AColumnGroup,
    ARowGroup);
...
function CompareStructureNodes(ANode1,
  ANode2: TcxPivotGridOLAPStructureNode): Integer;
const
  NodeType2Compare: array [TcxOLAPStructureNodeType] of Integer = (0, 1, 2, 3,
    4, 5, 7, 6);
begin
  Result := 0;
  if ANode1 = ANode2 then
    Exit;
  Result := NodeType2Compare[ANode1.NodeType] - NodeType2Compare
    [ANode2.NodeType];
  if Result = 0 then
  begin
...
procedure TcxPivotGridOLAPDataSource.DoPopulateGroupValues
  (AGroupValues: TcxPivotGridOLAPGroupInfos; AField: TcxPivotGridOLAPField;
...
var
  ...
  ADataSet: TcxCustomPivotGridOLAPDataSet;
begin
  AQuery := QueryBuilder.GetMembersQueryString(Cube,
    TcxPivotGridOLAPField(AField), AnExpandedMembers, ADataFields,
    AFilterFields);
...
function TcxPivotGridOLAPDataSource.GetLastNameFromPath
...
procedure TcxPivotGridOLAPDataSource.PopulateChildrenGroups
  (AParentGroup: TcxPivotGridGroupItem; AField: TcxPivotGridField;
  AnExpandedMembers: TList; AHasChildren: Boolean;
  ADataFields, AFilterFields: TcxPivotGridFields);
var
  I: Integer;
...
begin
  ...
  try
    DoPopulateGroupValues(AGroupValues, TcxPivotGridOLAPField(AField),
    ...
    begin
      ...
      begin
        ...
        AGroupItem.InitializeValue(AGroupValues[I].DisplayText,
          AGroupValues[I].UniqueName);
...
procedure TcxPivotGridOLAPDataSource.PopulateGrandTotals(AColumnGroups,
  ARowGroups: TcxPivotGridGroupItem; AColumnFields, ASummaryFields,
  AFilterFields: TcxPivotGridFields; AnExpandedMembers: TList);
var
  AQuery: string;
...
begin
  if ASummaryFields.Count = 0 then
    Exit;
  AQuery := QueryBuilder.GetTotalsQueryString(Cube, AColumnFields,
    AnExpandedMembers, ASummaryFields, AFilterFields);
  ADataSet := ExecuteQuery(AQuery);
  try
    if not IsDataValid(ADataSet) then
      Exit;
    AReader := Provider.CreateGrandTotalsReader(ADataSet, AColumnGroups,
      ARowGroups, ASummaryFields, AnExpandedMembers);
    try
      AReader.Execute;
    finally
      AReader.Free;
...
procedure TcxPivotGridOLAPDataSource.PopulateGroupValues
...
begin
  ...
  try
    ...
    try
      DoPopulateGroupValues(AGroupValues, TcxPivotGridOLAPField(AField), nil,
        nil, nil);
    finally
      TcxCustomOLAPDataQueryBuilderAccess(QueryBuilder).FAxisIndex :=
        AAxisIndex;
    end;
    for I := 0 to AGroupValues.Count - 1 do
    begin
...
procedure TcxPivotGridOLAPDataSource.PopulateCrossCells(AOwnedGroup, ARowGroups,
  AColumnGroups: TcxPivotGridGroupItem; ARowFields, AColumnFields,
  ASummaryFields, AFilterFields: TcxPivotGridFields);
var
  AQuery: TcxMDXQuery;
...
begin
  if ASummaryFields.Count = 0 then
    Exit;
  AQuery := QueryBuilder.GetCrossCellsQuery(Cube, AOwnedGroup, AColumnFields,
    ARowFields, ASummaryFields, AFilterFields);
  try
    ADataSet := ExecuteQuery(AQuery);
    try
      if not IsDataValid(ADataSet) then
        Exit;
      AReader := Provider.CreateCrossCellsReader(ADataSet, AOwnedGroup,
        ARowGroups, AColumnGroups, ASummaryFields);
      try
        AReader.Execute;
      finally
        AReader.Free;
...
function TcxPivotGridOLAPDataSource.CreateDrillDownDataSet(ACells: TList;
  ASummaryIndex: Integer): TObject;
var
  AQuery: string;
begin
  ...
  begin
    AQuery := GetDrillDownCommandText(TcxPivotGridCrossCell(ACells[0]),
      ASummaryIndex);
    if AQuery <> '' then
      Result := Provider.CreateDrillDownDataSet(AQuery,
        TcxPivotGridCrossCell(ACells[0]), ASummaryIndex);
  end;
end;

function TcxPivotGridOLAPDataSource.CreateDrillDownDataSource(ACells: TList;
  AFieldList: TcxObjectList; ASummaryIndex: Integer)
  : TcxPivotGridCrossCellDataSource;
var
...
function TcxPivotGridOLAPDataSource.ExecuteQuery(const AQuery: string)
  : TcxCustomPivotGridOLAPDataSet;
begin
  if AQuery = '' then
    Exit(nil);
...
function TcxPivotGridOLAPDataSource.ExecuteQuery(AQuery: TcxMDXQuery)
  : TcxCustomPivotGridOLAPDataSet;
begin
  Result := Provider.Execute(AQuery);
end;
...
function TcxPivotGridOLAPDataSource.GetNonExpandedFields
  (ADataBuilder: TcxPivotGridDataBuilder;
  AnExpandingGroup, AnGroup: TcxPivotGridGroupItem; ACrossGroupItems: TList;
  AddHierarchyField: Boolean): TcxPivotGridFields;

  function HierarchyFieldDuplicationAvoidance(AFields: TcxPivotGridFields;
    ACrossGroupItems: TList; AnExpandingItem: TcxPivotGridGroupItem): Boolean;
  var
    I: Integer;
  begin
    Result := True;
    ...
    begin
      if (AFields[I].Group <> nil) and
        (AFields[I].Group = AnExpandingItem.Field.Group) then
        Result := False;
    end;
    ...
    if Result and (ACrossGroupItems <> nil) then
      ...
      begin
        if TcxPivotGridGroupItem(ACrossGroupItems[I])
          .Field.Group = AnExpandingItem.Field.Group then
...
begin
  ...
  begin
    ...
    begin
      AFieldList := ADataBuilder.GetFieldsListByArea
        (AnExpandingGroup.Field.Area);
      Result.Add(AFieldList.Fields
        [AFieldList.IndexOf(AnExpandingGroup.Field) + 1]);
    end
    else if AddHierarchyField and IsOLAPHierarchy(AnExpandingGroup) and
      HierarchyFieldDuplicationAvoidance(Result, ACrossGroupItems,
      AnExpandingGroup) and IsFieldExist(AnExpandingGroup.Field) then
      Result.Add(AnExpandingGroup.Field.Group.Fields
        [AnExpandingGroup.Field.GroupIndex + 1]);
  end;
  if (AnGroup <> nil) and not IsNotLastVisibleOLAPHierarchyItem(AnGroup) and
    AnGroup.HasChildren then
    Result.Add(AnGroup.Items[0].Field)
  else if AddHierarchyField and (AnGroup <> nil) and IsOLAPHierarchy(AnGroup)
    and IsFieldExist(AnGroup.Field) then
...
procedure TcxPivotGridOLAPDataSource.InitializeExpanding
...
begin
  ...
  try
    ...
    AHasChildren := (AField.Area = faColumn) and
      (AField.VisibleIndex < (ADataBuilder.ColumnFields.Count - 1)) or
      (AField.Area = faRow) and
      (AField.VisibleIndex < (ADataBuilder.RowFields.Count - 1));
    PopulateChildrenGroups(AnExpandingGroup, AField, AMembers, AHasChildren,
      ASummaryFields, ADataBuilder.FilteredFields);
    if AnExpandingGroup.ItemCount = 0 then
      Exit;

    ANonExpandedFields := GetNonExpandedFields(ADataBuilder, AnExpandingGroup,
      nil, nil, False);
    try
      AFields := GetNonExpandedFields(ADataBuilder, AnExpandingGroup,
        ACrossGroup, nil, False);
      try
        ...
        begin
          ...
          try
            ...
            PopulateCrossCells(AnExpandingGroup, ADataBuilder.Rows,
              ADataBuilder.Columns, ADataBuilder.RowFields,
              ADataBuilder.ColumnFields, ASummaryFields, AFilter);
          finally
            AFilter.Free;
          end;
        end;
        if AnExpandingGroup.Field.Area = faColumn then
          PopulateGrandTotals(AnExpandingGroup, ADataBuilder.Rows,
            ANonExpandedFields, ASummaryFields,
            ADataBuilder.FilteredFields, AMembers)
        else
          PopulateGrandTotals(AnExpandingGroup, ADataBuilder.Columns,
            ANonExpandedFields, ASummaryFields, ADataBuilder.FilteredFields,
            AMembers);
      finally
        AFields.Free;
      end;
...
procedure TcxPivotGridOLAPDataSource.PopulateMainGrandTotal(ADataFields,
  AFilterFields: TcxPivotGridFields; AColumns, ARows: TcxPivotGridGroupItem);
var
  AQuery: string;
  ADataSet: TcxCustomPivotGridOLAPDataSet;
  AReader: TcxCustomPivotGridOLAPDataSetReader;
begin
  ...
  try
    if IsDataValid(ADataSet) then
    begin
      AReader := Provider.CreateMainGrandTotalReader(ADataSet, AColumns, ARows,
        ADataFields);
      try
        AReader.Execute;
      finally
...
procedure TcxPivotGridOLAPDataSource.PopulateFilteredUniqueNames
  ...
  begin
    ...
    begin
      ADataSet := ExecuteQuery(QueryBuilder.GetParentQueryString(Cube,
        TcxPivotGridOLAPField(AField), AUniqueName));
      try
        Result := (ADataSet as TcxPivotGridOLAPDBDataSet)
          .FieldByName(OLAPParentUniqueNameColumnName).AsString;
      finally
        ADataSet.Free;
      end;
    end
  ...
  try
    ...
    begin
      ...
      if IsValueVisible(AFilter, AField.GroupValueList.Items[I].Value) and
        ((AUniqueValues = nil) or (AUniqueValues.IndexOf(AUniqueName) <> -1))
      then
...
procedure TcxPivotGridOLAPDataSource.ExpandedItemWithParentsToList
  (AnGroup: TcxPivotGridGroupItem; AList: TList);
var
...
begin
  ...
  begin
    if (APreviousGroup = nil) or (AnGroup.Field.Group = nil) or
      (APreviousGroup.Field.Group <> AnGroup.Field.Group) then
      AList.Add(AnGroup);
...
function TcxPivotGridOLAPDataSource.GetDrillDownCommandText
  (ACell: TcxPivotGridCrossCell; ASummaryIndex: Integer): string;
begin
  Result := QueryBuilder.GetDrillDownQueryString(Cube, ACell, ASummaryIndex);
end;

function TcxPivotGridOLAPDataSource.IsDataValid
  (ADataSet: TcxCustomPivotGridOLAPDataSet): Boolean;
begin
...
procedure TcxPivotGridOLAPDataSource.SetProvider(const Value
  : TcxCustomPivotGridOLAPProvider);
begin
...
procedure TcxPivotGridOLAPDataSource.SetProviderClass
  (const Value: TcxCustomPivotGridOLAPProviderClass);
var
...
begin
  if ProviderClassName <> Value then
    ProviderClass := TcxCustomPivotGridOLAPProviderClass
      (cxGetPivotGridOLAPDataSourceProviders.FindByClassName(Value));
end;
