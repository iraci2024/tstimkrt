...
uses
  ...
  cxLabel, cxControls, cxContainer, cxEdit, cxGroupBox, cxButtons,
  cxLookAndFeels,
  cxPivotGridStrs, DB, ADODB, cxPivotGridOLAPDataSource,
...
type
  ...
  private
    ...
    procedure PopulateComboBox(AComboBox: TcxComboBox;
      AProc: TcxPivotGridOLAPPopulateComboProc);
...
const
  ConnectionDesignerClass: TcxPivotGridOLAPConnectionDesignerClass =
...
function cxPivotGridOLAPCreateConnectionString(var ACube: string;
  ...
  ALookAndFeel: TcxLookAndFeel = nil): string; overload;
function cxPivotGridOLAPCreateConnectionString(var ACube: WideString;
  ALookAndFeel: TcxLookAndFeel = nil): WideString; overload;
  deprecated
  'use the cxPivotGridOLAPCreateConnectionString overloaded function instead and pass a provider class as the AProviderClass parameter';
...
function cxPivotGridOLAPCreateConnectionString(var ACube: string;
  AProviderClass: TcxCustomPivotGridOLAPProviderClass;
  ALookAndFeel: TcxLookAndFeel = nil): string;
var
...
var
  S: string;
begin
  Result := cxPivotGridOLAPCreateConnectionString(S,
    TcxPivotGridOLAPOLEDBProvider, ALookAndFeel);
  ACube := S;
end;
...
begin
  ...
  try
    TcxCustomPivotGridOLAPProviderAccess(Result).ConnectionString :=
      GetConnectionString;
...
var
  ASecurity: string;
begin
  Result := 'Provider=MSOLAP%s;Persist Security Info=False;Data Source=' +
    edtServer.Text;
...
begin
  ...
  try
    PopulateComboBox(edtDatabase, TcxCustomPivotGridOLAPProviderAccess
      (AProvider).PopulateCatalogs);
...
begin
  Result := (edtServer.Text <> '') and
    (rbCubeFile.Checked or ((edtDatabase.Text <> '') and (edtCube.Text <> '')));
end;

procedure TfrmConnectionDesigner.PopulateComboBox(AComboBox: TcxComboBox;
  AProc: TcxPivotGridOLAPPopulateComboProc);
begin
...
procedure TfrmConnectionDesigner.edtServerPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
begin
  try
    if rbAnalysisServer.Checked and (edtServer.Text <> '') and
      (FChangeLockCount = 0) then
    begin
      Inc(FChangeLockCount);
      try
        edtDatabase.Properties.Items.Clear;
