...
uses
  Windows, Contnrs, Types, Classes, SysUtils, Forms, Math, Variants, StrUtils,
  DB, ADODB,
  Generics.Defaults, Generics.Collections, dxCore, cxClasses,
...
type
  ...
  protected
    ...
    function DoOpenSchema(const ASchemaType: string;
      const ARestriction: OleVariant): TcxPivotGridOLAPOLEDBDataSet;
  ...
  TcxCustomPivotGridOLAPOLEDBDataSetReader = class
  ...
  public
    ...
    class procedure ProcessDataFields(DataSet: TcxCustomPivotGridOLAPDataSet;
      AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
      ADataFields: TcxPivotGridFields; AFieldIndex: Integer;
  ...
  TcxPivotGridOLAPOLEDBDataSetCrossCellsReader = class
    (TcxCustomPivotGridOLAPOLEDBDataSetReader)
  strict private
  ...
  TcxPivotGridOLAPOLEDBDataSetMembersReader = class
    (TcxCustomPivotGridOLAPOLEDBDataSetReader)
  strict private
  ...
  TcxPivotGridOLAP8OLEDBDataSetMembersReader = class
    (TcxPivotGridOLAPOLEDBDataSetMembersReader)
  strict private
  ...
  TcxPivotGridOLAPOLEDBDataSetGrandTotalsReader = class
    (TcxCustomPivotGridOLAPOLEDBDataSetReader)
  strict private
    FColumnGroups: TcxPivotGridGroupItem;
    FRowGroups: TcxPivotGridGroupItem;
  ...
  TcxPivotGridOLAP8OLEDBDataSetGrandTotalsReader = class
    (TcxPivotGridOLAPOLEDBDataSetGrandTotalsReader)
  protected
  ...
  TcxPivotGridOLAPOLEDBDataSetMainGrandTotalReader = class
    (TcxCustomPivotGridOLAPOLEDBDataSetReader)
  strict private
...
constructor TcxPivotGridOLAPOLEDBProvider.Create(ADataSource
...
function TcxPivotGridOLAPOLEDBProvider.CreateCrossCellsReader
...
function TcxPivotGridOLAPOLEDBProvider.CreateGrandTotalsReader
  (ADataSet: TcxCustomPivotGridOLAPDataSet;
  AColumnGroups, ARowGroups: TcxPivotGridGroupItem;
  ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList)
  : TcxCustomPivotGridOLAPDataSetReader;
...
function TcxPivotGridOLAPOLEDBProvider.CreateMainGrandTotalReader
  (ADataSet: TcxCustomPivotGridOLAPDataSet;
  AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
...
function TcxPivotGridOLAPOLEDBProvider.CreateMembersReader
  (ADataSet: TcxCustomPivotGridOLAPDataSet;
  AGroupValues: TcxPivotGridOLAPGroupInfos; AField: TcxPivotGridField)
  : TcxCustomPivotGridOLAPDataSetReader;
var
...
function TcxPivotGridOLAPOLEDBProvider.CreateQueryBuilder
...
function TcxPivotGridOLAPOLEDBProvider.GetDrillDownDataSourceClass
...
procedure TcxPivotGridOLAPOLEDBProvider.ExtractProviderVersion
...
function TcxPivotGridOLAPOLEDBProvider.CreateDataSet
  : TcxPivotGridOLAPOLEDBDataSet;
begin
  Result := TcxPivotGridOLAPOLEDBDataSet.Create;
end;

function TcxPivotGridOLAPOLEDBProvider.CreateDrillDownDataSet(const ACommandText
...
function TcxPivotGridOLAPOLEDBProvider.DoOpenSchema(const ASchemaType: string;
  const ARestriction: OleVariant): TcxPivotGridOLAPOLEDBDataSet;
begin
  Result := CreateDataSet;
  try
    Connection.OpenSchema(siProviderSpecific, ARestriction, ASchemaType,
      TcxPivotGridOLAPOLEDBDataSet(Result).InnerDataSet);
...
function TcxPivotGridOLAPOLEDBProvider.Execute(const ACommandText: string)
  : TcxCustomPivotGridOLAPDataSet;
begin
  Result := CreateDataSet;
  try
    TcxPivotGridOLAPOLEDBDataSet(Result).InnerDataSet.Recordset :=
      Connection.Execute(ACommandText);
    TcxPivotGridOLAPOLEDBDataSet(Result).InitializeFields;
  except
    FreeAndNil(Result);
...
function TcxPivotGridOLAPOLEDBProvider.Execute(const AQuery: TcxMDXQuery)
  : TcxCustomPivotGridOLAPDataSet;
begin
...
procedure TcxPivotGridOLAPOLEDBProvider.PopulateHierarchyAllMemberNames
...
begin
  if Version <= 9 then
    Exit;
  ACommandText :=
    Format('SELECT [HIERARCHY_UNIQUE_NAME], [ALL_MEMBER] FROM $system.MDSCHEMA_HIERARCHIES WHERE [CUBE_NAME]=%s AND [ALL_MEMBER]>''''',
    [QuotedStr(ACubeName)]);
...
function TcxPivotGridOLAPOLEDBProvider.OpenSchema(const ACubeName: string;
...
constructor TcxCustomPivotGridOLAPOLEDBDataSetReader.Create
  (ADataSet: TcxCustomPivotGridOLAPDataSet);
begin
...
function TcxCustomPivotGridOLAPOLEDBDataSetReader.GetDataSet
  : TcxPivotGridOLAPOLEDBDataSet;
begin
  Result := TcxPivotGridOLAPOLEDBDataSet(inherited DataSet);
end;

class procedure TcxCustomPivotGridOLAPOLEDBDataSetReader.ProcessDataFields
  (DataSet: TcxCustomPivotGridOLAPDataSet;
  AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
  ADataFields: TcxPivotGridFields; AFieldIndex: Integer;
  const AFieldDimension: string);

  function SeparateDimensionAndMeasure(AValue: string;
    out ADimension, AMeasure: string): Boolean;
  var
    APosition: Integer;
  begin
    Result := False;
...
begin
  ...
  begin
    ...
    if AFieldIndex + AProcessedFieldCount < ADataSet.FieldCount then
    // OLAP v.8 problem
      if SeparateDimensionAndMeasure
        (ADataSet.Fields[AFieldIndex + AProcessedFieldCount].DisplayName,
        ADimension, AMeasure) and
        ((AFieldDimension = '') or (AFieldDimension = ADimension)) and
        (ADataFields[ASummaryIndex].UniqueName = AMeasure) then
      begin
        AValues[ASummaryIndex] := ADataSet.Fields
          [AFieldIndex + AProcessedFieldCount].Value;
  ...
  if AIsValuesInitialized then
    SetCrossCellValues(ARowGroup.GetCellByCrossItem(AColumnGroup), AValues,
      ANativeValues)
  else
    SetCrossCellValues(ARowGroup.GetCellByCrossItem(AColumnGroup), Null, Null);
end;

class procedure TcxCustomPivotGridOLAPOLEDBDataSetReader.ProcessDataFieldsEx
  (DataSet: TcxCustomPivotGridOLAPDataSet;
  AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
  ADataFields: TcxPivotGridFields);
var
...
type
  ...
  protected type
    ...
    private
      ...
      function IsSame(const AMembers: TMembers;
    ...
    public
      ...
      function IsSameColumn(const ACoordinate: array of string;
        ALevel: Integer): Boolean;
      function IsSameRow(const ACoordinate: array of string;
        ALevel: Integer): Boolean;
  ...
  public
    ...
    procedure GetMeasureValue(AItemIndex: Integer;
      const MeasureUniqueName: string);

    property Count: Integer read GetCount;
    property Items[Index: Integer]: TcxOLEDBQueryData.TItem
      read GetItem; default;
  end;

  { TcxOLEDBQueryData.TItem }

constructor TcxOLEDBQueryData.TItem.Create(const AColumn,
  ARow: TStringDynArray);
var
  I: Integer;
begin
  inherited Create;
...
function TcxOLEDBQueryData.TItem.IsSameColumn(const ACoordinate
  : array of string; ALevel: Integer): Boolean;
begin
  Result := IsSame(FColumn, ACoordinate, ALevel);
end;

function TcxOLEDBQueryData.TItem.IsSameRow(const ACoordinate: array of string;
  ALevel: Integer): Boolean;
begin
  Result := IsSame(FRow, ACoordinate, ALevel);
end;

function TcxOLEDBQueryData.TItem.ExtractMembers(const AMembers: string)
  : TMember;
const
  Quote = '''';
  Separator = ',';
...
function TcxOLEDBQueryData.TItem.IsSame(const AMembers: TMembers;
  ...
  function NameInArray(const AUniqueName: string;
...
procedure TcxOLEDBQueryData.Initialize(ADataSet: TcxPivotGridOLAPOLEDBDataSet);
  ...
  function DoGetCoordinate(AAxisMembers: TcxMDXQueryAxisMembers;
    const AUniqueNameFormat: string): TStringDynArray;
  var
    V: Variant;
    I, AIndex: Integer;
  begin
    ...
    begin
      if (I > 0) and TcxCustomOLAPDataQueryBuilder.SameHierarchy
        (AAxisMembers.Fields[I], AAxisMembers.Fields[I - 1]) then
        Continue;
...
begin
  ...
  begin
    AUniqueName := (AMeasureMembers.Measures[I] as TcxPivotGridOLAPField)
      .UniqueName;
  ...
  begin
    AColumn := DoGetCoordinate(AColumnMembers,
      '[Measures].[DX_COLUMN_%d_UNIQUE_NAME]');
...
procedure TcxOLEDBQueryData.Normalize;
  ...
  function IndexOf(AItems: TList; const ARowCoordinate: TStringDynArray;
  ...
  begin
    ...
    for I := 0 to AItems.Count - 1 do
      if TcxOLEDBQueryData.TItem(AItems[I]).IsSameRow(ARowCoordinate, ALevel)
      then
        Exit(I);
  end;
...
procedure TcxOLEDBQueryData.GetMeasureValue(AItemIndex: Integer;
  const MeasureUniqueName: string);
var
  I, AIndex: Integer;
begin
...
var
  ...
  begin
    ...
    begin
      ...
      SetCrossCellValue(ACell, I, ANativeValue,
        NativeValueToValue(ANativeValue));
    end;
    Inc(AItemIndex);
  end;

  procedure IterateColumns(AData: TcxOLEDBQueryData; var AItemIndex: Integer;
    ARowGroup: TcxPivotGridGroupItem);
  var
    I: Integer;
  ...
  begin
    ...
    begin
      ...
      if AItem.IsSameColumn(ACoordinate, AColumnMembers.Members[I].Count) then
        IterateMeasures(AData, AItemIndex, AColumnMembers.Members[I].Last,
          ARowGroup);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      while (I < AMemberCount - 1) and (AItemIndex + 1 < AData.Count) and
        AData.Items[AItemIndex].IsSameRow(ACoordinate,
        ARowMembers.Members[I].Count) and not AData.Items[AItemIndex]
        .IsSameRow(ARowMembers.GetMemberCoordinate(I + 1),
        ARowMembers.Members[I + 1].Count) do
        Inc(AItemIndex);
    end;
  end;
...
begin
  ...
  if not AQuery.TryGetMembers(TcxOLEDBQueryData.MEASURE_AXIS, AMeasureMembers)
    or not AQuery.TryGetMembers(TcxOLEDBQueryData.COLUMN_AXIS, AColumnMembers)
    or not AQuery.TryGetMembers(TcxOLEDBQueryData.ROW_AXIS, ARowMembers) then
...
procedure TcxPivotGridOLAPOLEDBDataSetCrossCellsReader.Initialize(AOwnedGroup,
...
procedure TcxPivotGridOLAPOLEDBDataSetMembersReader.Initialize
  (AGroupValues: TcxPivotGridOLAPGroupInfos; AField: TcxPivotGridField);
begin
...
procedure TcxPivotGridOLAPOLEDBDataSetMembersReader.AddGroupItem
  (AIndex: Integer);
  ...
  begin
    Result := True;
    for I := DataSet.FieldByName(OLAPMemberUniqueNameColumnName).
      Index + 1 to DataSet.FieldCount - 1 do
    begin
...
begin
  ...
  if Pos(TcxPivotGridOLAPField(Field).HierarchyUniqueName + '.[All]',
    AUniqueName) <= 1 then
  begin
...
function TcxPivotGridOLAP8OLEDBDataSetMembersReader.ExtractMemberName
  (const AUniqueName: string): string;
const
  StartSign = '.[';
  EndSign = ']';
var
...
procedure TcxPivotGridOLAP8OLEDBDataSetMembersReader.AddGroupItem
  (AIndex: Integer);
var
  AUniqueName: string;
begin
...
procedure TcxPivotGridOLAPOLEDBDataSetGrandTotalsReader.Execute;
  ...
  function GetChildColumnGroupByUniqueName(const AUniqueName: string)
...
function TcxPivotGridOLAPOLEDBDataSetGrandTotalsReader.GetDimensionField
...
begin
  if (AnExpandedMembers <> nil) and IsNotLastVisibleOLAPHierarchyItem
    ...
    AName := TcxPivotGridOLAPField(TcxPivotGridGroupItem(AnExpandedMembers[0])
...
procedure TcxPivotGridOLAPOLEDBDataSetGrandTotalsReader.Initialize
  (AColumnGroups, ARowGroups: TcxPivotGridGroupItem;
  ASummaryFields: TcxPivotGridFields; AnExpandedMembers: TList);
begin
  FColumnGroups := AColumnGroups;
...
procedure TcxPivotGridOLAPOLEDBDataSetMainGrandTotalReader.Initialize
  (AColumnGroup, ARowGroup: TcxPivotGridGroupItem;
  ADataFields: TcxPivotGridFields);
begin
