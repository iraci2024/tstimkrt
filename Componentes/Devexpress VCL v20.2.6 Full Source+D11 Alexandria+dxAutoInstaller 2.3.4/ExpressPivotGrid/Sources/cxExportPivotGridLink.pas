...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Windows, Classes, Graphics, SysUtils, Variants, Types, DB, Math,
  cxCustomPivotGrid;
...
procedure cxExportPivotGridToCSV(const AFileName: string;
  APivotGrid: TcxCustomPivotGrid; AExpand: Boolean = True;
...
procedure cxExportPivotGridToText(const AFileName: string;
  APivotGrid: TcxCustomPivotGrid; AExpand: Boolean;
...
procedure cxExportPivotGridDataToExcel(const AFileName: string;
  APivotGrid: TcxCustomPivotGrid; AHandler: TObject = nil;
  AAutoBestFit: Boolean = True; AHighlightTotals: Boolean = True;
  ACellBorders: Boolean = True; AFieldHeaders: Boolean = False);

implementation

uses
  cxPivotGridStrs, dxCore, dxCoreClasses, cxClasses, dxSpreadSheetCore,
  dxHashUtils, dxSpreadSheetClasses, cxFormats,
  dxSpreadSheetGraphics, dxSpreadSheetTypes, cxExportProviders, cxGeometry,
  cxEdit, cxCalendar, cxCurrencyEdit,
  cxSpinEdit, cxCalc, cxTimeEdit, cxMaskEdit, cxGraphics, cxStyles, cxDataUtils,
  cxDataStorage, cxExport, cxTextEdit,
  cxVariants, dxSpreadSheetNumberFormat, dxSpreadSheetCoreStyles,
  dxSpreadSheetStyles;
...
type
  ...
  public
    ...
    property DisplayFormatType: TcxValueDisplayFormatType
      read FDisplayFormatType;
  ...
  private
    ...
    function IsCurrencyProperties(AProperties: TcxCustomEditProperties)
      : Boolean;
    function IsNativeFormatProperties(AProperties
  ...
  protected
    ...
    procedure ConvertCellToCacheCellStyle(ACell: TcxPivotGridCustomCellViewInfo;
      var AStyle: TcxCacheCellStyle);
    procedure ConvertDataCell(ACell: TcxPivotGridDataCellViewInfo;
      var AStyle: TcxCacheCellStyle);
    procedure ConvertHeaderCell(ACell: TcxPivotGridHeaderCellViewInfo;
      var AStyle: TcxCacheCellStyle);
    procedure InitializeExportCell(AExportCell: TcxPivotGridExportCell;
      ASource: TcxPivotGridCustomCellViewInfo);
  ...
  public
    constructor Create(AGroup: TcxPivotGridGroupItem;
      ADataField: TcxPivotGridField; ALevel: Integer);
    destructor Destroy; override;
    function Add(AGroup: TcxPivotGridGroupItem; ADataField: TcxPivotGridField;
      ALevel: Integer): TdxPivotGridExportGroupItem;
  ...
  protected
    ...
    //
    procedure AddContentItem(AGroup: TdxPivotGridExportGroupItem;
      var AContentItems: TdxPivotGridExportContentItem);
    function AddFieldHeader(ARow, AColumn: Integer; AField: TcxPivotGridField)
      : TdxSpreadSheetCell;
    function AddGroupValue(ARow, AColumn: Integer;
      AGroup: TdxPivotGridExportGroupItem;
      ATableItems: TdxSpreadSheetTableItems;
      var AContentItems: TdxPivotGridExportContentItem; var AIndex: Integer)
      : TdxSpreadSheetCell;
    procedure ApplyBestFit(AStartItem: TdxSpreadSheetTableItem);
    procedure CalculateGroupsFinish;
    function GetDataField(ARow, AColumn: TdxPivotGridExportContentItem)
      : TcxPivotGridField;
    function GetDataFieldIndex(AArea: TcxPivotGridDataFieldArea): Integer;
    function GetColumnWidthForColumn
      (AColumn: TdxPivotGridExportGroupItem): Integer;
    ...
    function GetSummaryType(AItem: TdxPivotGridExportContentItem;
      ADefaultType: TcxPivotGridSummaryType; var ADisplayFormat: string)
    ...
    function NeedNewLineForNextItem(AItem: TdxPivotGridExportGroupItem)
      : Boolean;
    procedure PrepareColumns;
    procedure PrepareRows;
    procedure SetContentCellStyle(ACell: TdxSpreadSheetCell;
      ADataField: TcxPivotGridField; AIsTotal: Boolean;
      const ADisplayFormat: string = '');
    procedure WriteFieldHeaders(ARow, AColumn: Integer;
      AFields: TcxPivotGridFields; AArea: TcxPivotGridDataFieldArea);
    procedure WriteGroupColumns;
    procedure WriteGroupRows;
    procedure WriteGroupsPost(ARoot: TdxPivotGridExportGroupItem;
      AItems: TdxSpreadSheetTableItems);
    procedure WriteRow(ARow: TdxPivotGridExportContentItem);
    procedure WriteRows;
  public
    constructor Create(AControl: TObject; const AFileName: string;
      AHandler: TObject); override;
    destructor Destroy; override;
    ...
    property HighlightTotals: Boolean read FHighlightTotals
      write FHighlightTotals;
...
const
  ...
  cxDefaultExportStyle: TcxCacheCellStyle = (AlignText: catCenter;
    BrushStyle: cbsSolid; FontCharset: 0; FontColor: 0; FontSize: 12;
    FontStyle: [];);

  AlignToCxAlign: array [TAlignment] of TcxAlignText = (catLeft, catRight,
    catCenter);
  AlignVertToCxAlignVert: array [TcxEditVertAlignment] of TcxAlignTextVert =
    (atvTop, atvBottom, atvCenter);

  cxUsedBorder: TcxCellBorders = (IsDefault: False; Width: 1);
  cxEmptyBorder: TcxCellBorders = (IsDefault: True; Width: 0);

function cxSetBorder(var ABorder: TcxCellBorders; NeedSet: Boolean;
...
begin
  cxSetBorder(AStyle.Borders[0], (bLeft in ABorders) and
    (AGridLines in [pglVert, pglBoth]), AColor);
  cxSetBorder(AStyle.Borders[1], (bTop in ABorders) and
    (AGridLines in [pglHorz, pglBoth]), AColor);
  cxSetBorder(AStyle.Borders[2], (bRight in ABorders) and
    (AGridLines in [pglVert, pglBoth]), AColor);
  cxSetBorder(AStyle.Borders[3], (bBottom in ABorders) and
    (AGridLines in [pglHorz, pglBoth]), AColor);
end;

procedure cxViewParamsToCacheStyle(AViewParams: TcxViewParams;
  var ACacheStyle: TcxCacheCellStyle);
begin
...
procedure TcxPivotGridExportCell.Initialize(const ABounds: TRect;
  AStyleIndex: Integer);
begin
...
function TcxPivotGridExportCellList.GetLinkedObjectClass
...
constructor TcxPivotGridMapsInfo.Create(AOwner: TcxPivotGridExportHelper;
...
begin
  ...
  AExportCell.Initialize(ExportArea.BoundsToPosition(ACell.Bounds),
...
begin
  ...
  try
    ...
    begin
      Provider.SetRange(ExportArea.AxisX.Count - 1, ExportArea.AxisY.Count -
        1, False);
      WriteColumnWidths;
      WriteRowHeights;
    end;
...
begin
  ...
  begin
    AStyle.AlignText := AlignToCxAlign[ACell.Properties.Alignment.Horz];
    AStyle.AlignTextVert := AlignVertToCxAlignVert
      [ACell.Properties.Alignment.Vert];
  end;
end;

procedure TcxPivotGridMapsInfo.InitializeExportCell
  (AExportCell: TcxPivotGridExportCell;
  ASource: TcxPivotGridCustomCellViewInfo);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      AExportCell.FNativeFormat := Owner.IsNativeFormat and
        not ADisplayTextAssigned;
      ...
      begin
        ...
        begin
          AExportCell.FValue := CheckNativeValue(AProperties,
            ADataCell.DataField, ADataCell.Value);
  ...
  else if ASource is TcxPivotGridHeaderCellViewInfo then
  begin
    AProperties := TcxPivotGridHeaderCellViewInfo(ASource).Properties;
    if IsNativeFormatProperties(AProperties) then
    begin
      AExportCell.FNativeFormat := Owner.IsNativeFormat and
        (TcxPivotGridHeaderCellAccess(ASource).FDataField <> nil) and
        not Assigned(TcxPivotGridHeaderCellAccess(ASource)
        .FDataField.OnGetGroupValueDisplayText);
      AExportCell.FValue := CheckNativeValue(AProperties,
        TcxPivotGridHeaderCellAccess(ASource).FDataField,
        TcxPivotGridHeaderCellViewInfo(ASource).Value);
      AExportCell.FDisplayFormat := cxGetDisplayFormat(AProperties);
      AExportCell.FDisplayFormatType := cxGetDisplayFormatType(AProperties);
    end
    else
...
begin
  if AExportCell.NativeFormat then
    Provider.SetCellValue(AExportCell.Bounds.Left, AExportCell.Bounds.Top,
      AExportCell.Value, AExportCell.DisplayFormat,
      AExportCell.DisplayFormatType)
  else
    Provider.SetCellValueAsString(AExportCell.Bounds.Left,
      AExportCell.Bounds.Top, AExportCell.Value);
...
function TcxPivotGridMapsInfo.CheckNativeValue(AProperties
  : TcxCustomEditProperties; APivotField: TcxPivotGridField;
  const AValue: Variant): Variant;
begin
  if VarIsSoftEmpty(AValue) then
  begin
    if TcxPropertiesAccess(AProperties).UseNullString and
      (TcxPropertiesAccess(AProperties).Nullstring <> '') then
  ...
  begin
    try
      ...
      else if (VarType(AValue) = varCurrency) and
        not IsCurrencyField(APivotField) and
...
function TcxPivotGridMapsInfo.IsCurrencyField(APivotField
  : TcxPivotGridField): Boolean;
begin
  Result := TcxPivotGridFieldDataBindingAccess(APivotField.DataBinding)
    .IsCurrencyField;
end;

function TcxPivotGridMapsInfo.IsCurrencyProperties
...
function TcxPivotGridMapsInfo.IsNativeFormatProperties
  (AProperties: TcxCustomEditProperties): Boolean;
begin
  Result := (AProperties is TcxDateEditProperties) or
...
begin
  ...
  FProgressHelper := TcxProgressCalculationHelper.Create(3, APivotGrid,
    ProgressHandler);
  TcxExport.Provider(AExportType, AFileName).GetInterface(IcxExportProvider,
    FProvider);
...
procedure TcxPivotGridExportHelper.ProgressHandler(Sender: TObject;
  Percents: Integer);
var
...
function cxSupports(const Instance: IInterface; const IID: TGUID;
  out Intf): Boolean;
begin
...
begin
  ...
  with TcxPivotGridExportHelper.Create(APivotGrid, AExportType, AFileName,
    AHandler) do
    try
      Expand := AExpand;
      ...
      if cxSupports(Provider, IcxExportWithSeparators, AIntf) and
...
procedure cxExportPivotGridToCSV(const AFileName: string;
  ...
  AHandler: TObject = nil; AEncoding: TEncoding = nil);
begin
  cxExportPivotGridToFile(AFileName, APivotGrid, cxExportToCSV, AExpand, False,
...
procedure cxExportPivotGridToText(const AFileName: string;
  ...
  AEncoding: TEncoding = nil); overload;
begin
  cxExportPivotGridToText(AFileName, APivotGrid, AExpand, '', '', '', AFileExt,
    AHandler, AEncoding);
end;
...
//
procedure ExportPivotGridDataToExcel(const AFileName: string;
  APivotGrid: TcxCustomPivotGrid; AHandler: TObject = nil;
  AAutoBestFit: Boolean = True; AHighlightTotals: Boolean = True;
  ACellBorders: Boolean = True; AFieldHeaders: Boolean = False);
var
...
begin
  AHelper := TcxPivotGridDataExportHelper.Create(APivotGrid, AFileName,
    AHandler);
  try
...
procedure TcxPivotGridExportViewInfo.AfterRowCellsCalculated
...
type
  ...
  protected
    ...
    function Add(AData: TcxPivotGridGroupItem; ALevel: Integer)
      : TdxPivotGridExportGroupItem; overload;
    function Add(AData: TcxPivotGridGroupItem; AField: TcxPivotGridField;
      ALevel: Integer): TdxPivotGridExportGroupItem; overload;
    function IsTotalNeeded(AItem: TcxPivotGridGroupItem): Boolean; inline;
    function LastLevel: Integer;
    procedure PrepareDataFields(AParent: TcxPivotGridGroupItem;
      ALevel: Integer);
    procedure PrepareLevelItems(AParent: TcxPivotGridGroupItem;
      ALevel: Integer);
    procedure PrepareTotals(AParent: TcxPivotGridGroupItem;
      ALevel: Integer); inline;
...
constructor TcxPivotGridDataExportGroupsProducer.Create
...
function TcxPivotGridDataExportGroupsProducer.MakeItems
  : TdxPivotGridExportGroupItem;
begin
  if (GroupItem.ItemCount = 1) and not OptionsView.GrandTotalsForSingleValues
  then
...
function TcxPivotGridDataExportGroupsProducer.Add(AData: TcxPivotGridGroupItem;
  ALevel: Integer): TdxPivotGridExportGroupItem;
begin
...
function TcxPivotGridDataExportGroupsProducer.Add(AData: TcxPivotGridGroupItem;
  AField: TcxPivotGridField; ALevel: Integer): TdxPivotGridExportGroupItem;
begin
  if Current = nil then
...
procedure TcxPivotGridDataExportGroupsProducer.PrepareDataFields
  (AParent: TcxPivotGridGroupItem; ALevel: Integer);
var
...
procedure TcxPivotGridDataExportGroupsProducer.PrepareLevelItems
  (AParent: TcxPivotGridGroupItem; ALevel: Integer);
var
  AIndex: Integer;
  ANeedTotal: Boolean;
...
procedure TcxPivotGridDataExportGroupsProducer.PrepareTotals
  (AParent: TcxPivotGridGroupItem; ALevel: Integer);
var
  ...
  begin
    ...
    if (DataFieldsIndex >= ALevel) and (DataFields.Count > 1) then
      for ADataIndex := 0 to DataFields.Count - 1 do
        Add(AParent, DataFields[ADataIndex], LastLevel).TotalIndex :=
          ATotalIndex;
  end;
end;

function TcxPivotGridDataExportGroupsProducer.IsTotalNeeded
...
function TcxPivotGridDataExportGroupsProducer.GetOptionsView
...
constructor TdxPivotGridExportGroupItem.Create(AGroup: TcxPivotGridGroupItem;
  ADataField: TcxPivotGridField; ALevel: Integer);
begin
  Group := AGroup;
  GroupStart := -1;
...
function TdxPivotGridExportGroupItem.Add(AGroup: TcxPivotGridGroupItem;
  ADataField: TcxPivotGridField; ALevel: Integer): TdxPivotGridExportGroupItem;
begin
...
begin
  if Group.Field.CustomTotals.Count = 0 then
    ...
    Result := cxGetResourceString
      (TotalDescriptions[Group.Field.CustomTotals[TotalIndex].SummaryType]);
...
begin
  ...
  else if IsTotal and not IsGrandTotal then
...
constructor TdxPivotGridExportContentItem.Create
  (APattern: TdxPivotGridExportContentItem);
begin
  TotalIndex := -1;
...
constructor TcxPivotGridDataExportHelper.Create(AControl: TObject;
...
procedure TcxPivotGridDataExportHelper.AddContentItem
  (AGroup: TdxPivotGridExportGroupItem;
  var AContentItems: TdxPivotGridExportContentItem);
begin
  ...
  else if FContentItem.TableItem <> AGroup.TableItem then
  begin
...
function TcxPivotGridDataExportHelper.AddFieldHeader(ARow, AColumn: Integer;
  AField: TcxPivotGridField): TdxSpreadSheetCell;
var
  AText: string;
begin
  if AField <> nil then
...
begin
  ...
  SetContentCellStyle(Result, AField, (AGroup.TotalIndex >= 0) and
    HighlightTotals);
  ...
  else if not IsTotal(AGroup) then
...
procedure TcxPivotGridDataExportHelper.ApplyBestFit
  (AStartItem: TdxSpreadSheetTableItem);
begin
...
function TcxPivotGridDataExportHelper.GetColumnWidthForColumn
  (AColumn: TdxPivotGridExportGroupItem): Integer;
begin
  Result := PivotGrid.OptionsView.ColumnGrandTotalWidth;
  if AColumn.DataField <> nil then
    Result := AColumn.DataField.Width
  else if (AColumn.Group <> nil) and (AColumn.Group.Field <> nil) then
    Result := AColumn.Group.Field.Width;
  if Result = 0 then
    Result := cxPivotGridDefaultFieldWidth;
end;

function TcxPivotGridDataExportHelper.GetColumnWidthForRow
  (ARow: TdxPivotGridExportGroupItem): Integer;
begin
  Result := PivotGrid.OptionsView.RowGrandTotalWidth;
  if ARow.DataField <> nil then
    Result := PivotGrid.OptionsDataField.Width
  else if (ARow.Group <> nil) and (ARow.Group.Field <> nil) then
    Result := ARow.Group.Field.Width;
  if Result = 0 then
    Result := cxPivotGridDefaultFieldWidth;
end;

function TcxPivotGridDataExportHelper.GetDataField(ARow,
  AColumn: TdxPivotGridExportContentItem): TcxPivotGridField;
begin
  Result := DataField;
  if ARow.DataField <> nil then
...
function TcxPivotGridDataExportHelper.GetDataFieldIndex
  (AArea: TcxPivotGridDataFieldArea): Integer;
begin
  Result := TcxPivotGridDataFieldAccess(PivotGrid.OptionsDataField)
    .GetActualAreaIndex(False);
...
function TcxPivotGridDataExportHelper.GetSummaryType
  (AItem: TdxPivotGridExportContentItem; ADefaultType: TcxPivotGridSummaryType;
  var ADisplayFormat: string): TcxPivotGridSummaryType;
begin
  Result := ADefaultType;
  if (AItem.TotalIndex >= 0) and (AItem.Field <> nil) and
    (AItem.Field.CustomTotals.Count > 0) then
  begin
...
procedure FreeAndNilLinkedList(var AList: TObject;
  ARemoveItemProc: TRemoveNextListItem); inline;
var
  AItem, ANext: TObject;
begin
  AItem := AList;
...
procedure TcxPivotGridDataExportHelper.FreeAndNilContent
  (var AContent: TdxPivotGridExportContentItem);
begin
  FreeAndNilLinkedList(TObject(AContent), function(AItem: TObject): TObject
    begin
...
procedure TcxPivotGridDataExportHelper.FreeAndNilGroup
  (var AGroup: TdxPivotGridExportGroupItem);
begin
  FreeAndNilLinkedList(TObject(AGroup), function(AItem: TObject): TObject
    begin
...
function TcxPivotGridDataExportHelper.IsItemExpanded
  (AGroup: TdxPivotGridExportGroupItem): Boolean;
begin
  Result := (AGroup.Group = nil) or AGroup.Group.Expanded;
end;

function TcxPivotGridDataExportHelper.IsLastGroup
  (AGroup: TdxPivotGridExportGroupItem): Boolean;
begin
...
function TcxPivotGridDataExportHelper.IsNewGroup
  (AGroup: TdxPivotGridExportGroupItem): Boolean;
begin
  Result := (AGroup.DataField = nil) and (AGroup.Group <> nil) and
    (AGroup.Group.ItemCount > 0);
end;

function TcxPivotGridDataExportHelper.IsTotal
  (AGroup: TdxPivotGridExportGroupItem): Boolean;
begin
  Result := AGroup.TotalIndex >= 0;
end;

function TcxPivotGridDataExportHelper.NeedNewLineForNextItem
  (AItem: TdxPivotGridExportGroupItem): Boolean;
begin
...
begin
  AProducer := TcxPivotGridDataExportGroupsProducer.Create
    (ViewData.DataBuilder.Columns);
  try
    ...
    AProducer.FarTotalsLocation :=
      PivotGrid.OptionsView.ColumnTotalsLocation = ctlFar;
    AProducer.Totals := PivotGrid.OptionsView.ColumnTotals;
    AProducer.TotalsForSingleValues :=
      PivotGrid.OptionsView.TotalsForSingleValues;
...
procedure TcxPivotGridDataExportHelper.SetContentCellStyle
  (ACell: TdxSpreadSheetCell; ADataField: TcxPivotGridField; AIsTotal: Boolean;
const ADisplayFormat: string = '');
var
  AFormat: string;
  ...
  begin
    ...
    begin
      ...
      ABrush.BackgroundColor :=
        dxGetDarkerColor(ColorToRgb(ABrush.BackgroundColor), 85);
      ABrush.Style := sscfsSolid;
      AStyle.Brush := SpreadSheet.CellStyles.Brushes.AddBrush(ABrush);
    end;
    dxChangeHandle(TdxHashTableItem(ContentStyles[AIsTotal]),
      SpreadSheet.CellStyles.AddStyle(AStyle));
  end;
...
procedure TcxPivotGridDataExportHelper.WriteFieldHeaders(ARow, AColumn: Integer;
AFields: TcxPivotGridFields; AArea: TcxPivotGridDataFieldArea);
var
...
begin
  AIndex := 0;
  AFieldIndex := 0;
  ADataFieldIndex := TcxPivotGridDataFieldAccess(PivotGrid.OptionsDataField)
    .GetActualAreaIndex;
  if (ViewData.DataBuilder.DataFields.Count <= 1) or
    (PivotGrid.OptionsDataField.Area <> AArea) then
    ADataFieldIndex := -1;
...
begin
  ...
  begin
    AColumnIndex := AIndex;
    AddGroupValue(AColumn.Level + Byte(FieldHeaders), AIndex, AColumn,
      View.Columns, ColumnsContent, AIndex);
...
begin
  ...
  begin
    ...
    AddGroupBy(AItems, AGroup.GroupStart, AGroup.GroupFinish,
      IsItemExpanded(AGroup));
  end;
end;

procedure TcxPivotGridDataExportHelper.WriteRow
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if AField.SummaryVariation <> svNone then
          AValue := ACrossCell.SummaryCells[AField.SummaryIndex]
            .SummaryVariation
        else
          AValue := ACrossCell.GetSummaryByField(AField, ASummaryType);
        try
          if not(VarIsNull(AValue) or VarIsEmpty(AValue) or
            ((AValue = Unassigned) and (AValue <> 0))) then
          begin
    ...
    SetContentCellStyle(ADestCell, AField, (ARow.IsTotal or AColumn.IsTotal) and
      HighlightTotals, ADisplayFormat);
    AColumn := AColumn.Next;
  end;
