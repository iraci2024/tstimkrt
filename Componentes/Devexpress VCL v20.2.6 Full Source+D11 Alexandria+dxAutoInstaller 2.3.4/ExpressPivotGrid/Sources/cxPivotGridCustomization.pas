...
uses
  Windows, Messages, Classes, SysUtils, Forms, Controls, StdCtrls, Graphics,
  Math,
  dxCore, cxClasses,
  cxControls, cxGraphics, dxCoreGraphics, cxDropDownEdit, cxButtons,
  cxLookAndFeels, cxListBox, cxLabel,
  cxStyles, cxLookAndFeelPainters, cxGeometry, cxCustomPivotGrid,
  cxPivotGridStrs,
...
type
  ...
  protected
    ...
    function GetAreaIndexOffset(AField: TcxPivotGridField;
      ACheckGroup: Boolean): Integer;
  ...
  public
    ...
    constructor CreateEx(AOwner: TComponent;
      AFieldsType: TcxPivotGridCustomizationFormFieldListType); virtual;
    ...
    property FieldChooser: TcxPivotGridCustomCustomizationForm
      read GetFieldChooser;
    property FieldsType: TcxPivotGridCustomizationFormFieldListType
      read FFieldsType;
  ...
  TcxPivotGridListListBoxDragAndDropObject = class
  ...
  protected
    ...
    function GetFieldListByType(AListType
      : TcxPivotGridCustomizationFormFieldListType): TObject; override;
  ...
  protected
    ...
    property Field: TcxPivotGridField read GetField;
    property FieldChooser: TcxPivotGridOLAPCustomizationForm
      read GetFieldChooser;
    property FieldHeaderHeight: Integer read GetFieldHeaderHeight;
  ...
  protected
    ...
    function GetCustomizationFormListBackgroundColor: TColor; override;
    function GetStructureDisplayText(AStructure
      : TcxPivotGridOLAPStructureNode): string;
    procedure Init; override;
...
const
  ListType2FieldArea: array [TcxPivotGridCustomizationFormFieldListType]
    of TcxPivotGridFieldArea = (faFilter, faColumn, faRow, faFilter, faData);
  FieldArea2ListType: array [TcxPivotGridFieldArea]
    of TcxPivotGridCustomizationFormFieldListType = (fltColumn, fltRow,
    fltFilter, fltData);
...
type
  ...
  public
    ...
    procedure Initialize(ACanvas: TcxCanvas; AScaleFactor: TdxScaleFactor;
      ASource: TcxPivotGridFieldHeaderCellViewInfo);
  end;
...
begin
  ...
  try
    TcxPivotGridCustomizationForm(AFieldChooser)
      .PopulateFieldList(AList, AType);
    ...
    try
      ...
      AListBox.Sorted := (AType = fltAvailable) and
        AFieldChooser.PivotGrid.Customization.AvailableFieldsSorted;
    finally
      AListBox.Initialize;
      AListBox.EndUpdate;
    end;
...
constructor TcxFieldListListBox.CreateEx(AOwner: TComponent;
...
procedure TcxFieldListListBox.DrawItem(const R: TRect;
  AItem: TdxCustomListBoxItem; AState: TcxButtonState);
const
  HeaderStateMap: array [Boolean] of TcxButtonState = (cxbsNormal, cxbsPressed);
  FilterStateMap: array [Boolean] of TcxButtonState = (cxbsNormal,
    cxbsDisabled);

  function Clone(AFieldViewInfo: TcxPivotGridFieldHeaderCellViewInfo)
    : TcxFieldListListBoxFieldViewInfo;
  begin
  ...
  function GetSortOrder(AFieldViewInfo: TcxPivotGridFieldHeaderCellViewInfo)
    : TcxDataSortOrder;
  begin
  ...
  begin
    ...
    try
      AFieldViewInfo.PaintTo(Canvas, R, AState,
        FilterStateMap[AFieldViewInfo.FilterState <> cxbsDisabled],
        GetSortOrder(AFieldViewInfo), False,
...
function TcxFieldListListBox.CalculateHitTest
...
begin
  ...
  begin
    TcxPivotGridCustomizationForm(FieldChooser).CalculateFieldHitTest(AHitTest,
      FieldByIndex(AIndex), dxMapWindowRect(Handle, PivotGrid.Handle,
      ItemRect(AIndex)));
  end
end;
...
begin
  ItemHeight := LookAndFeelPainter.ScaledHeaderHeight(cxTextHeight(Font),
    ScaleFactor);
end;
...
begin
  ItemIndex := Items.IndexOfObject(TcxPivotGridCustomizationForm(FieldChooser)
    .SelectedObject);
...
procedure TcxFieldListListBox.LookAndFeelChanged(Sender: TcxLookAndFeel;
...
procedure TcxFieldListListBox.MouseDown(Button: TMouseButton;
...
begin
  ...
  begin
    ...
    if PivotGrid.HitTest.HitAtFilter then
      FieldChooser.FieldFilterPopup(TcxPivotGridField(PivotGrid.HitTest.Field),
        FilterInitPopupHandler);
  end;
end;
...
begin
  ...
  if Result then
    TcxPivotGridCustomizationForm(FieldChooser).SetDragFieldToController
      (SelectedField);
end;

procedure TcxFieldListListBox.SynchronizeField(AField: TcxPivotGridField;
  AAreaIndex: Integer);
var
  I: Integer;
begin
  ...
  begin
    (AField as IcxPivotGridField).AssignAreaIndex
      (ListType2FieldArea[FieldsType], AAreaIndex);
    if AField.Group <> nil then
    begin
      for I := 1 to AField.Group.FieldCount - 1 do
        (AField.Group.Fields[I] as IcxPivotGridField)
          .AssignAreaIndex(ListType2FieldArea[FieldsType], AAreaIndex + I);
    end;
    AField.Visible := True;
  end;
end;
...
begin
  Color := TcxPivotGridCustomizationForm(FieldChooser)
    .GetCustomizationFormListBackgroundColor;
end;

function TcxFieldListListBox.GetAreaIndexOffset(AField: TcxPivotGridField;
  ACheckGroup: Boolean): Integer;
begin
  Result := 0;
  if AField <> nil then
...
function TcxFieldListListBox.GetDragAndDropObjectClass
...
begin
  ...
  begin
    if cxPointIsEqual(P, cxInvalidPoint) then
      AItemIndex := Items.IndexOfObject
        (TcxPivotGridCustomizationForm(FieldChooser).SelectedObject)
    else
      AItemIndex := ItemAtPos(P, False);
    if AItemIndex < 0 then
      AItemIndex := 0
    else if AItemIndex >= Count then
      AItemIndex := Count - 1;
    ItemIndex := AItemIndex;
    TcxPivotGridCustomizationForm(FieldChooser).SelectedObject := SelectedField;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AFilterPopup.BoundsRect :=
        cxRectOffset(cxRectOffset(AFilterPopup.BoundsRect, P),
        -cxRectWidth(R), 0);
    end;
  end;
end;

function TcxFieldListListBox.GetFieldChooser
...
procedure TcxFieldListListBoxFieldViewInfo.Initialize(ACanvas: TcxCanvas;
  AScaleFactor: TdxScaleFactor; ASource: TcxPivotGridFieldHeaderCellViewInfo);
begin
  SetBounds(ASource.Bounds, ASource.ClipRect);
  inherited Initialize(ACanvas, AScaleFactor, ASource.Painter,
    ASource.ViewParams);
...
function TcxPivotGridListListBoxDragAndDropObject.CheckArea(const P: TPoint;
...
procedure TcxPivotGridListListBoxDragAndDropObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
var
  APoint: TPoint;
begin
  APoint := dxMapWindowPoint(Control.Handle, PivotGrid.Handle, P);
  CurMousePos := dxMapWindowPoint(Control.Handle, PivotGrid.Handle,
    CurMousePos);
  inherited DragAndDrop(APoint, Accepted);
end;

procedure TcxPivotGridListListBoxDragAndDropObject.EndDragAndDrop
  (Accepted: Boolean);
var
  AIndex: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      with TcxPivotGridCustomizationForm((Control as TcxFieldListListBox)
        .FieldChooser) do
      begin
...
function TcxPivotGridListListBoxDragAndDropObject.GetPivotGrid
  : TcxCustomPivotGrid;
begin
...
begin
  ...
  if ImmediateUpdate or not DragDropInfo.Visible then
    Exit;
  Result := (ListType2FieldArea[TcxFieldListListBox(Control).FieldsType]
    = DragDropInfo.Area) and (Field = DragDropInfo.Field);
end;
...
procedure TcxPivotGridCustomizationForm.CMBiDiModeChanged
...
begin
  ...
  FieldList.ItemHeight := Max(Painter.ScaledHeaderHeight(cxTextHeight(Font),
    ScaleFactor), FieldItemHeight);
...
function TcxPivotGridCustomizationForm.
...
begin
  inherited Init;
  SetBounds(Left, Top, ScaleFactor.Apply(cxPivotGridCustomizationDefaultHeight),
...
begin
  ...
  Result.Parent := BottomPanel;
  BottomPanel.Height := Result.Height + Result.Margins.Bottom +
    Result.Margins.Top;
...
procedure TcxPivotGridCustomizationForm.FieldListClickHandler(Sender: TObject);
begin
  SyncArea;
end;
...
begin
  AddButton.Enabled := (FieldList.ItemIndex >= 0) and
    (TcxPivotGridFieldArea(AreaList.ItemObject)
    in FieldList.SelectedField.AllowedAreas);
...
function TcxPivotGridOLAPTreeDragAndDropObject.GetFieldChooser
  : TcxPivotGridOLAPCustomizationForm;
begin
...
begin
  Result := PivotGrid.LookAndFeelPainter.ScaledHeaderHeight
    (cxTextHeight(FieldViewInfo.Font), FieldChooser.ScaleFactor);
end;
...
function TcxPivotGridOLAPTreeDragAndDropObject.CheckArea(const P: TPoint;
  var AInfo: TcxPivotGridDragDropAreaInfo): Boolean;
begin
  Result := inherited CheckArea(P, AInfo);
  if not ImmediateUpdate then
...
procedure TcxPivotGridOLAPTreeDragAndDropObject.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
var
  APoint: TPoint;
begin
  APoint := dxMapWindowPoint(TreeView.Handle, PivotGrid.Handle, P);
  CurMousePos := dxMapWindowPoint(TreeView.Handle, PivotGrid.Handle,
    CurMousePos);
  inherited DragAndDrop(APoint, Accepted);
end;

procedure TcxPivotGridOLAPTreeDragAndDropObject.EndDragAndDrop
  (Accepted: Boolean);
var
  AIndex: Integer;
  ASource, ADest: TcxFieldListListBox;
...
begin
  if not Accepted then
    DirtyChanged
  else if (Field <> nil) and (DragDropInfo = nil) then
  begin
...
function TcxPivotGridOLAPStructureInnerTreeView.CalculateHitTest
  (AHitTest: TcxPivotGridHitTest): Boolean;
var
  P, Origin: TPoint;
  AInfo: TcxPivotGridDragDropAreaInfo;
...
procedure TcxPivotGridOLAPStructureInnerTreeView.DragAndDrop(const P: TPoint;
  var Accepted: Boolean);
begin
  DragAndDropObject.DoDragAndDrop(P, Accepted);
end;

procedure TcxPivotGridOLAPStructureInnerTreeView.EndDragAndDrop
  (Accepted: Boolean);
begin
...
procedure TcxPivotGridOLAPStructureInnerTreeView.FinishDragAndDrop
  (Accepted: Boolean);
begin
  if FFinishingDragAndDrop then
    Exit;
...
function TcxPivotGridOLAPStructureInnerTreeView.CanEdit
  (Node: TTreeNode): Boolean;
begin
  Result := False;
end;

procedure TcxPivotGridOLAPStructureInnerTreeView.MouseDown(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
  ...
  try
    ...
    begin
      ...
      if (GetParentForm(Self) <> nil) and
        (GetParentForm(Self).ActiveControl = Self) and not Focused then
...
procedure TcxPivotGridOLAPStructureInnerTreeView.MouseMove(Shift: TShiftState;
  X, Y: Integer);
var
  AAccepted: Boolean;
begin
  inherited;
  if (DragAndDropState = ddsStarting) and not IsPointInDragDetectArea
...
procedure TcxPivotGridOLAPStructureInnerTreeView.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
function TcxPivotGridOLAPStructureInnerTreeView.StartDragAndDrop
...
begin
  ...
  begin
    ...
    if (ANode = nil) or (ANode.Level <= 1) or
      not(htOnItem in GetHitTestInfoAt(P.X, P.Y)) then
      Exit;
    AStructure := TcxPivotGridOLAPStructureNode(ANode.Data);
    AField := AStructure.GetLinkedField(PivotGrid);
    Result := ((AStructure.NodeType in [ntField, ntSet]) and
      not(ntGroup in AStructure.AggregateType)) or
...
function TcxPivotGridOLAPStructureInnerTreeView.GetDragAndDropObject
  : TcxDragAndDropObject;
begin
  if FDragAndDropObject = nil then
  begin
    FDragAndDropObject := TcxPivotGridOLAPTreeDragAndDropObject.Create
      (PivotGrid);
...
function TcxPivotGridOLAPStructureInnerTreeView.GetFieldChooser
  : TcxPivotGridCustomCustomizationForm;
begin
  Result := Owner.Owner as TcxPivotGridCustomCustomizationForm;
end;

function TcxPivotGridOLAPStructureInnerTreeView.GetForm
  : TcxPivotGridOLAPCustomizationForm;
var
  AParent: TWinControl;
begin
  ...
  while (AParent <> nil) and
    not(AParent is TcxPivotGridOLAPCustomizationForm) do
    AParent := AParent.Parent;
  Result := AParent as TcxPivotGridOLAPCustomizationForm;
end;

function TcxPivotGridOLAPStructureInnerTreeView.GetPivotGrid
  : TcxCustomPivotGrid;
begin
...
begin
  ...
  begin
    ...
    if (AValue = ddsNone) and not FFinishingDragAndDrop then
      DoCancelMode;
  end;
end;

procedure TcxPivotGridOLAPStructureInnerTreeView.SetMouseCaptureObject
  (AValue: TObject);
var
...
procedure TcxPivotGridOLAPStructureInnerTreeView.CNKeyDown
...
procedure TcxPivotGridOLAPStructureInnerTreeView.WMCancelMode
...
class function TcxPivotGridOLAPTreeView.GetTreeViewClass
  : TcxCustomInnerTreeViewClass;
begin
...
begin
  ...
  Constraints.MinWidth :=
    Round(ScaleFactor.Apply(cxPivotGridCustomizationMinWidth) * 1.3);
  Constraints.MinHeight :=
    Round(ScaleFactor.Apply(cxPivotGridCustomizationMinHeight) * 2.5);
...
begin
  ...
  else if (AInfo.NodeType = ntField) or (AInfo.NodeType = ntSet) then
  begin
    AField := AInfo.GetLinkedField(PivotGrid);
    if (AField <> nil) and (((AField.Group <> nil) and AField.Group.Visible) or
      ((AField.Group = nil) and AField.Visible)) then
    begin
...
function TcxPivotGridOLAPCustomizationForm.
  GetCustomizationFormListBackgroundColor: TColor;
begin
  Result := Painter.GetCustomizationFormListBackgroundColor;
end;

function TcxPivotGridOLAPCustomizationForm.GetStructureDisplayText
  (AStructure: TcxPivotGridOLAPStructureNode): string;
var
  I: Integer;
...
var
  ...
  begin
    ...
    begin
      ADestChild := FieldList.Items.AddChildObject(ADest,
        GetStructureDisplayText(ASource), ASource);
...
function TcxPivotGridOLAPCustomizationForm.GetStructure
