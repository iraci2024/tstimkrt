...
uses
  ...
  AnsiStrings, Generics.Defaults, Generics.Collections;
...
type
  ...
  public
    ...
    constructor Create(const AName: AnsiString; ADateTime: Cardinal;
      ACRC32, APackedSize, AOriginalSize: Integer);
  ...
  public
    constructor Create(ACompressionLevel: TCompressionLevel = clDefault;
      ABufferCapacity: Cardinal = 0);
  ...
  public
    constructor Create(AStream: TStream; AAutoFreeStream: Boolean = False);
      overload; virtual;
  ...
  TdxZIPStreamReaderUnpackMethod = procedure(APackedSize: Integer;
    ATargetStream: TStream) of object;
  ...
  public
    ...
    procedure Extract(const AEntry: TdxZIPFileEntry;
      ATargetStream: TStream); overload;
    function Extract(const AFileName: AnsiString; ATargetStream: TStream)
      : Boolean; overload;
    //
    property Files;
  end;
  ...
  strict private
    procedure AddCompressedFile(const AName: AnsiString; AData: PByte;
      ACompressedSize, AUncompressedSize, ACRC32: Integer);
    procedure AddFileHeader(const AName: AnsiString;
      ACRC32, APackedSize, ASize: Integer);
    procedure WriteCentralDirectory;
  public
    constructor Create(const AFileName: string); override;
    destructor Destroy; override;
    procedure AddDirectory(const AName: AnsiString);
    procedure AddFile(const AFileName: AnsiString; AData: PByte;
      ADataSize: Integer); overload;
    procedure AddFile(const AFileName: AnsiString; AStream: TStream;
      AFreeStream: Boolean = False); overload;
  end;
  ...
  public
    class function AbsoluteFileName(const ACurrentPath, AFileName: AnsiString)
      : AnsiString; static;
    ...
    class function ExcludeRootPathDelimiter(const APath: AnsiString)
      : AnsiString; static;
    class function ExcludeTrailingPathDelimiter(const APath: AnsiString)
      : AnsiString; static;
    class function ExpandFileName(const AFileName: AnsiString)
      : AnsiString; static;
    class function ExtractFilePath(const AFileName: AnsiString)
      : AnsiString; static;
    class function IsAbsoluteFileName(const AFileName: AnsiString)
      : Boolean; static;
    class function RelativePath(const ACurrentPath, AFileName: AnsiString)
      : AnsiString; static;
  end;
  ...
  TdxInflateHelper = class
    class procedure Decompress(APackedStream: TStream; APackedSize: Integer;
      ATargetStream: TStream); static;
  end;
...
constructor TdxZIPFileEntry.Create(const AName: AnsiString; ADateTime: Cardinal;
  ACRC32, APackedSize, AOriginalSize: Integer);
begin
  Sign := FileHeader;
  VersionToExtract := 20;
...
begin
  Result := TdxZIPPathHelper.ExcludeRootPathDelimiter
    (TdxZIPPathHelper.EncodePath(AName));
end;

procedure TdxZIPFileEntry.LoadFromStream(AStream: TStream;
...
constructor TdxCompressedStream.Create(ACompressionLevel
  : TCompressionLevel = clDefault; ABufferCapacity: Cardinal = 0);
begin
  inherited Create;
  FCRC32 := $FFFFFFFF;
...
function TdxCompressedStream.Seek(const Offset: Int64;
...
constructor TdxZIPCustomStream.Create(AStream: TStream;
  AAutoFreeStream: Boolean = False);
begin
  inherited Create;
  FStream := AStream;
...
procedure TdxZIPStreamReader.Extract(const AEntry: TdxZIPFileEntry;
...
begin
  ...
  begin
    CheckRead((AEntry.CRC32 = dxCRC32(ATargetStream, ATargetPosition,
      ATargetStream.Position - ATargetPosition)),
      dxAnsiStringToString(AEntry.Name) + ' is corrupted.');
  end;
end;

function TdxZIPStreamReader.Extract(const AFileName: AnsiString;
...
function TdxZIPStreamReader.Find(AFileName: AnsiString;
  var AEntry: TdxZIPFileEntry): Boolean;
var
  I: Integer;
begin
  Result := False;
  AFileName := TdxZIPPathHelper.ExcludeRootPathDelimiter
    (TdxZIPPathHelper.ExpandFileName(AFileName));
...
begin
  ...
  begin
    ...
    try
      ...
      until (AEndOfDir.Sign = EndOfDir) or
        (Stream.Position < SizeOf(AEndOfDir));
      ...
      begin
        ...
        ADirs.Add(ADir);
        Stream.Seek(ADir.NameLength + ADir.ExtraLength + ADir.CommentLength,
          soCurrent);
      end;
      ...
      begin
        ...
        AFileEntry.IsDirectory := ADir.ExtAttributes and
...
procedure TdxZIPStreamReader.UnpackDeflate(APackedSize: Integer;
  ATargetStream: TStream);
begin
  TdxInflateHelper.Decompress(Stream, APackedSize, ATargetStream);
end;

procedure TdxZIPStreamReader.UnpackNone(APackedSize: Integer;
  ATargetStream: TStream);
begin
...
procedure TdxZIPStreamWriter.AddFile(const AFileName: AnsiString; AData: PByte;
  ADataSize: Integer);
var
  ABuffer: Pointer;
  APackedSize: Integer;
begin
  ...
  begin
    ...
    try
      AddCompressedFile(AFileName, ABuffer, APackedSize, ADataSize,
        dxCRC32(AData, ADataSize));
    finally
      FreeMem(ABuffer);
    end;
  end
...
procedure TdxZIPStreamWriter.AddFile(const AFileName: AnsiString;
  AStream: TStream; AFreeStream: Boolean = False);
var
  ABuffer: Pointer;
  ABufferSize: Integer;
begin
  ...
  begin
    ...
    AddCompressedFile(AFileName, TdxCompressedStream(AStream).FBuffer.Memory,
      TdxCompressedStream(AStream).FBuffer.Size, TdxCompressedStream(AStream)
      .UncompressedSize, TdxCompressedStream(AStream).CRC32);
  end
  else

    if AStream is TMemoryStream then
    AddFile(AFileName, TMemoryStream(AStream).Memory,
      TMemoryStream(AStream).Size)
  else
  begin
...
procedure TdxZIPStreamWriter.AddFileHeader(const AName: AnsiString;
  ACRC32, APackedSize, ASize: Integer);
var
  AEntry: TdxZIPFileEntry;
begin
  AEntry := TdxZIPFileEntry.Create(AName, DateTimeToFileDate(Now), ACRC32,
    APackedSize, ASize);
  AEntry.RelativeOffset := Stream.Position;
  AEntry.WriteToStream(Stream);
  Files.Add(AEntry);
...
class function TdxZIPPathHelper.AbsoluteFileName(const ACurrentPath,
  AFileName: AnsiString): AnsiString;
begin
...
class function TdxZIPPathHelper.ExcludeRootPathDelimiter
...
class function TdxZIPPathHelper.ExcludeTrailingPathDelimiter
  (const APath: AnsiString): AnsiString;
var
  AIndex: Integer;
begin
...
class function TdxZIPPathHelper.ExtractFilePath(const AFileName: AnsiString)
  : AnsiString;
var
  AIndex: Integer;
begin
...
class function TdxZIPPathHelper.ExpandFileName(const AFileName: AnsiString)
  : AnsiString;

  function FindNearestFromLeftPathDelimiter(const S: AnsiString;
...
begin
  ...
  repeat
    ...
    begin
      ADelimPos := FindNearestFromLeftPathDelimiter(Result,
        AParentPathMacroPos);
      Delete(Result, ADelimPos + 1, AParentPathMacroPos - ADelimPos +
        Length(sdxParentPathMacro) - 1);
    end;
  until AParentPathMacroPos <= 1;
end;

class function TdxZIPPathHelper.IsAbsoluteFileName(const AFileName
  : AnsiString): Boolean;
begin
  Result := (AFileName <> '') and (AFileName[1] = dxUnixPathDelim);
end;

class function TdxZIPPathHelper.RelativePath(const ACurrentPath,
  AFileName: AnsiString): AnsiString;
begin
  Result := EncodePath(ExtractRelativePath(DecodePath(ACurrentPath),
    DecodePath(AFileName)));
end;

{ TdxInflateHelper }

class procedure TdxInflateHelper.Decompress(APackedStream: TStream;
  APackedSize: Integer; ATargetStream: TStream);

  procedure FlushBuffer(var AZStream: TZStreamRec; AOutBuffer: Pointer;
    AOutBufferSize: Integer);
  var
    AErrorCode: Integer;
  begin
    repeat
      ...
      ATargetStream.Write(AOutBuffer^, AOutBufferSize -
        Integer(AZStream.avail_out));
...
begin
  ...
  try
    ...
    try
      ProcessBuffer(AZStream, @ZLibHeader, AOutBuffer, ZLibHeaderSize,
        BufferSize);
      repeat
        ...
        begin
          ...
          ProcessBuffer(AZStream, AInBuffer, AOutBuffer, AInBufferSize,
            BufferSize);
        end;
