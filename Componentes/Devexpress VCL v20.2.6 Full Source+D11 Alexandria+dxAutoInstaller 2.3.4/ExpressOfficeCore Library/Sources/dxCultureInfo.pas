...
type
  ...
  public
    ...
    property NumberDecimalSeparator: Char read GetNumberDecimalSeparator
      write SetNumberDecimalSeparator;
    property NumberGroupSeparator: Char read GetNumberGroupSeparator
      write SetNumberGroupSeparator;
    property NumberGroupSizes: TArray<Integer> read FNumberGroupSizes
      write FNumberGroupSizes;
  end;
  ...
  public
    constructor Create(const LCID: Integer;
      AUseUserOverride: Boolean = False); overload;
  ...
  strict private
    class var FDictionary: TDictionary<Integer, string>;
  private
...
type
  TLocaleNameToLCIDFunc = function(AName: PWideChar; AFlags: DWORD)
    : Integer; stdcall;
...
begin
  if not Assigned(FLocaleNameToLCID) then
    @FLocaleNameToLCID := GetProcAddress(GetModuleHandle(kernel32),
      'LocaleNameToLCID');
...
constructor TdxCultureInfo.Create(const LCID: Integer;
  AUseUserOverride: Boolean = False);
var
...
constructor TdxCultureInfo.Create(const AName: string;
  AUseUserOverride: Boolean = False);
...
begin
  ...
  begin
    if Win32MajorVersion >= 6 then
      ...
      ALocale := dxLanguages.LocaleID
        [dxLanguages.IndexOf(AdjustLocaleName(AName))];
  end
  else
...
begin
  Result := (A.Latin = B.Latin) and (A.Bidi = B.Bidi) and
...
begin
  Result := TdxLangInfo.Create(TdxCultureInfo.InvalidCulture,
    TdxCultureInfo.InvalidCulture, TdxCultureInfo.InvalidCulture);
end;
...
class function TdxCultureInfoHelper.CreateCultureInfo(AKey: Integer)
  : TdxCultureInfo;
var
  ANameLang: string;
begin
