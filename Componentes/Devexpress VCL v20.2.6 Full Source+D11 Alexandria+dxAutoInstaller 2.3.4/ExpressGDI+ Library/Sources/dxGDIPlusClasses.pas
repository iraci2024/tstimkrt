...
uses
  Windows, Classes, SysUtils, SyncObjs, Graphics, ActiveX, Types, Clipbrd,
  ExtCtrls,
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Generics.Collections, Generics.Defaults,
  dxCore, dxCoreGraphics, dxDPIAwareUtils, cxGeometry, dxGDIPlusAPI,
  dxCoreClasses, Contnrs, dxSmartImage;
...
type
  ...
  TdxGpCompositionMode = (cmSourceOver = CompositingModeSourceOver,
    cmSourceCopy = CompositingModeSourceCopy);

  TdxGPInterpolationMode = (imDefault = InterpolationModeDefault,
  ...
  TdxGPSmoothingMode = (smDefault, smHighSpeed, smHighQuality, smNone,
    smAntiAlias);
  ...
  public
    ...
    constructor CreateFlip(AFlipHorizontally, AFlipVertically: Boolean;
      const APivotPointX, APivotPointY: Single); overload;
    constructor CreateFlip(AFlipHorizontally, AFlipVertically: Boolean;
      const APivotPoint: TdxPointF); overload;
    ...
    procedure Multiply(AMatrix: TdxGPMatrix;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure Reset;
    procedure Rotate(AAngle: Single;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure Rotate(AAngle: Single; const APivotPoint: TdxPointF;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure Scale(const AScale: TdxPointF;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload; inline;
    procedure Scale(const AScale, ACenter: TdxPointF;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload; inline;
    procedure Scale(const AScaleX, AScaleY: Single;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure Shear(const AShear: TdxPointF;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload; inline;
    procedure Shear(const AShearX, AShearY: Single;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure Translate(const AOffset: TdxPointF;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload; inline;
    procedure Translate(const AOffsetX, AOffsetY: Single;
      AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
  ...
  strict private
    ...
    function InternalAdd(AOffset: Single; AColor: TdxAlphaColor)
      : Integer; inline;
    procedure SetCapacity(AValue: Integer);
    procedure SetColor(Index: Integer; const AValue: TdxAlphaColor);
    procedure SetOffset(Index: Integer; const AValue: Single);
  protected
    procedure CalculateParams(out AColors: PdxAlphaColor; out AOffsets: PSingle;
      out ACount: Integer);
    procedure Changed;
  ...
  TdxGPBrushGradientMode = (gpbgmHorizontal, gpbgmVertical,
    gpbgmForwardDiagonal, gpbgmBackwardDiagonal);
  ...
  public
    ...
    property GradientMode: TdxGPBrushGradientMode read FGradientMode
      write SetGradientMode;
    property GradientPoints: TdxGPBrushGradientPoints read FGradientPoints
      write SetGradientPoints;
    property Style: TdxGPBrushStyle read FStyle write SetStyle;
    property Texture: TdxGPImage read FTexture write SetTexture;
    property TextureTransform: TdxGPMatrix read FTextureTransform
  ...
  protected
    ...
    procedure SetDashArray(const AValue: array of Single;
      AScale: Single); overload;
  public
    constructor Create; overload; override;
    constructor Create(AColor: TdxAlphaColor; AWidth: Single = 1;
      APenStyle: TPenStyle = psSolid; AScale: Single = 1); overload;
    destructor Destroy; override;
    procedure Assign(ASource: TdxGPCustomGraphicObject); override;
    ...
    property DashCapStyle: TdxGPPenDashCapStyle read FDashCapStyle
      write SetDashCapStyle;
    property LineEndCapStyle: TdxGPPenLineCapStyle read FLineEndCapStyle
      write SetLineEndCapStyle;
    property LineJoin: TdxGpLineJoin read FLineJoin write SetLineJoin;
    property LineStartCapStyle: TdxGPPenLineCapStyle read FLineStartCapStyle
      write SetLineStartCapStyle;
  ...
  public
    ...
    constructor Create(APoints: TdxGpPointFDynArray; const APointTypes: TBytes);
      overload; virtual;
    destructor Destroy; override;
    ...
    procedure AddRoundRect(const R: TRect;
      ARadiusX, ARadiusY: Integer); overload;
    procedure AddRoundRect(const R: TdxRectF;
      ARadiusX, ARadiusY: Single); overload;

    procedure AddString(const S: string; AFont: TFont; AEmSize: Single;
      const ARect: TRect); overload;
    procedure AddString(const S: string; AFont: TdxGPFont;
      AFormat: TdxGPStringFormat; const ARect: TdxRectF); overload;
    procedure AddString(const S: string; AFont: TdxGPFont;
      AFormat: TdxGPStringFormat; const AOrigin: TdxPointF); overload;
    procedure AddString(const S: string; AFamily: TdxGPFontFamily;
      AStyle: Integer; AEmSize: Single; const AOrigin: TdxPointF;
      AFormat: TdxGPStringFormat); overload;
    procedure AddString(const S: string; AFamily: TdxGPFontFamily;
      AStyle: Integer; AEmSize: Single; const AOrigin: TPoint;
      AFormat: TdxGPStringFormat); overload;
    procedure AddString(const S: string; AFamily: TdxGPFontFamily;
      AStyle: Integer; AEmSize: Single; const ALayoutRect: TdxRectF;
      AFormat: TdxGPStringFormat); overload;
    procedure AddString(const S: string; AFamily: TdxGPFontFamily;
      AStyle: Integer; AEmSize: Single; const ALayoutRect: TRect;
      AFormat: TdxGPStringFormat); overload;
    //
    ...
    function IsPointInPath(const P: TdxPointF; ACanvas: TdxGPCanvas = nil)
      : Boolean; overload;
    function IsPointInPath(const P: TPoint; ACanvas: TdxGPCanvas = nil)
      : Boolean; overload;
    function IsPointInPathOutline(const P: TdxPointF; APen: TdxGPPen;
      ACanvas: TdxGPCanvas = nil): Boolean; overload;
    function IsPointInPathOutline(const P: TPoint; APen: TdxGPPen;
      ACanvas: TdxGPCanvas = nil): Boolean; overload;
    function IsPointInPathOutline(const P: TPoint; APenWidth: Integer;
      ACanvas: TdxGPCanvas = nil): Boolean; overload;
  ...
  TdxGPCombineMode = (gmReplace, gmIntersect, gmUnion, gmXor, gmExclude,
    gmComplement);
  ...
  public
    ...
    constructor CreateFromRegion(ARegion: GpRegion;
      ADummy: Integer = 0); virtual;
    destructor Destroy; override;

    procedure CombineRegionPath(const APath: TdxGPPath;
      AMode: TdxGPCombineMode);
    procedure CombineRegionRect(const ARect: TRect;
      AMode: TdxGPCombineMode); overload;
    procedure CombineRegionRect(const ARect: TdxRectF;
      AMode: TdxGPCombineMode); overload;
    procedure CombineRegionRegion(const ARegion: HRGN; AMode: TdxGPCombineMode;
      AFreeRegion: Boolean = False); overload;
    procedure CombineRegionRegion(const ARegion: TdxGPRegion;
      AMode: TdxGPCombineMode; AFreeRegion: Boolean = False); overload;
    function GetGdiRegionHandle(ACanvas: TdxGPCanvas): HRGN;
    procedure MakeEmpty;
  ...
  public
    constructor Create(AFormatAttributes: Integer = 0;
      ALanguage: LANGID = LANG_NEUTRAL); reintroduce; overload;
    ...
    procedure SetTabStops(AFirstTabOffset: Single; ACount: Integer;
      const ATabStops: TArray<Single>);
    function GetTabStopCount: Integer;
    function GetTabStops(ACount: Integer; out AFirstTabOffset: Single)
      : TArray<Single>;
    procedure SetDigitSubstitution(ALanguage: LANGID;
      ASubstitute: TdxGPStringDigitSubstitute);
    procedure SetMeasurableCharacterRanges(ACount: Integer;
      const ARanges: TArray<TdxGPCharacterRange>);

    property Alignment: TdxGpStringAlignment read GetAlignment
      write SetAlignment;
    property DigitSubstitutionLanguage: LANGID
      read GetDigitSubstitutionLanguage;
    property DigitSubstitutionMethod: TdxGPStringDigitSubstitute
      read GetDigitSubstitutionMethod;
    property FormatFlags: Integer read GetFormatFlags write SetFormatFlags;
    property Handle: GpStringFormat read FHandle;
    property HotkeyPrefix: TdxGPHotkeyPrefix read GetHotkeyPrefix
      write SetHotkeyPrefix;
    property LineAlignment: TdxGpStringAlignment read GetLineAlignment
      write SetLineAlignment;
    property MeasurableCharacterRangeCount: Integer
      read GetMeasurableCharacterRangeCount;
    ...
    // for internal use
    class property GenericTypographic: TdxGPStringFormat
      read GetGenericTypographic; // for internal use
  end;
  ...
  protected
    constructor Create(AHandle
      : GpFontFamily { TODO: ; AFontCollection: TdxGPFontCollection = nil } );
      reintroduce; overload;
    class procedure Finalize;
  public
    constructor Create(const AName
      : string { TODO: ; AFontCollection: TdxGPFontCollection = nil } );
      reintroduce; overload;
    destructor Destroy; override;
  ...
  TdxGPFontStyle = (FontStyleRegular = 0, FontStyleBold = 1,
    FontStyleItalic = 2, FontStyleUnderline = 4, FontStyleStrikeout = 8);
  ...
  strict private
    ...
    procedure Initialize(const AFamilyName: string; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit; AGdiCharSet: Byte;
      AGdiVerticalFont: Boolean); overload;
    procedure Initialize(AFamily: TdxGPFontFamily; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit; AGdiCharSet: Byte;
      AGdiVerticalFont: Boolean); overload;
  ...
  public
    constructor Create(APrototype: TdxGPFont;
      ANewStyle: TdxGPFontStyle); overload;
    constructor Create(AFamily: TdxGPFontFamily; AEmSize: Single); overload;
    constructor Create(AFamily: TdxGPFontFamily; AEmSize: Single;
      AStyle: TdxGPFontStyle); overload;
    constructor Create(AFamily: TdxGPFontFamily; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit); overload;
    constructor Create(AFamily: TdxGPFontFamily; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit;
      AGdiCharSet: Byte); overload;
    constructor Create(AFamily: TdxGPFontFamily; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit; AGdiCharSet: Byte;
      AGdiVerticalFont: Boolean); overload;
    constructor Create(AFamily: TdxGPFontFamily; AEmSize: Single;
      AUnit: TdxGraphicUnit); overload;
    constructor Create(const AFamilyName: string; AEmSize: Single); overload;
    constructor Create(const AFamilyName: string; AEmSize: Single;
      AStyle: TdxGPFontStyle); overload;
    constructor Create(const AFamilyName: string; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit); overload;
    constructor Create(const AFamilyName: string; AEmSize: Single;
    ...
    constructor Create(const AFamilyName: string; AEmSize: Single;
      AUnit: TdxGraphicUnit); overload;
    constructor Create(ANativeFont: GpFont; AGdiCharSet: Byte;
      AGdiVerticalFont: Boolean); overload;
    destructor Destroy; override;
    class function FromHFont(AHFont: THandle): TdxGPFont; static;
    class function FromLogFont(const ALogFont: TLogFont): TdxGPFont;
      overload; static;
    class function FromLogFont(const ALogFont: TLogFont; AHdc: THandle)
      : TdxGPFont; overload; static;
    class function FromHdc(AHdc: THandle): TdxGPFont; static;
    class function SafeCreate(const AFamilyName: string; AEmSize: Single;
      AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit = guPoint;
      AGdiCharSet: Byte = DEFAULT_CHARSET): TdxGPFont;
  ...
  protected
    ...
    procedure FreeHandle;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      ALayoutRect: TdxGpRectF; AStringFormat: TdxGPStringFormat;
      out ACharactersFitted, ALinesFilled: Integer): TdxSizeF; overload;
  public
    constructor Create; overload; virtual;
    ...
    //
    procedure Draw(AGraphic: TdxGPImage; const ADestRect, ASourceRect: TdxRectF;
      AAlpha: Byte = 255); overload;
    procedure Draw(AGraphic: TdxGPImage; const ADestRect, ASourceRect: TRect;
      AAlpha: Byte = 255); overload;
    procedure Draw(AGraphic: TdxGPImage; const R: TdxRectF;
      AAlpha: Byte = 255); overload;
    procedure Draw(AGraphic: TdxGPImage; const R: TRect;
      AAlpha: Byte = 255); overload;
    procedure Draw(AGraphic: TdxGPImage; const ADestRect, ASourceRect: TdxRectF;
      AAttributes: TdxGPImageAttributes); overload;
    procedure Draw(AGraphic: TdxGPImage; const R: TRect;
      AAttributes: TdxGPImageAttributes); overload;
    procedure Draw(AGraphic: TdxGPImage;
      const APoints: array of TdxPointF); overload;
    procedure Draw(AGraphic: TdxGPImage; const APoints: array of TdxPointF;
      AAttributes: TdxGPImageAttributes); overload;
    procedure DrawBitmap(ABitmap: TBitmap; const R: TRect; AAlpha: Byte = 255);
    // deprecated
    procedure DrawTile(AGraphic: TdxGPImage; const R: TRect;
      AAlpha: Byte = 255); overload;
    procedure DrawTile(AGraphic: TdxGPImage;
      const ADestRect, ASourceRect: TRect; AAlpha: Byte = 255); overload;
    ...
    procedure SetClipRegion(ARegion: TdxGPRegion;
      AMode: TdxGPCombineMode); overload;
    ...
    procedure Arc(R: TRect; AStartAngle, ASweepAngle: Single;
      APenColor: TdxAlphaColor; APenWidth: Single = 1;
    ...
    // Ellipse
    procedure Ellipse(R: TRect; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    procedure Ellipse(R: TdxRectF; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    procedure Ellipse(R: TdxRectF; APenColor, ABrushColor: TColor;
    ...
    procedure Ellipse(R: TdxRectF; APenColor, ABrushColor: TdxAlphaColor;
      APenWidth: Single = 1; APenStyle: TPenStyle = psSolid); overload;
    procedure Ellipse(R: TRect; APenColor, ABrushColor: TColor;
      APenWidth: Single; APenStyle: TPenStyle;
      APenColorAlpha, ABrushColorAlpha: Byte); overload;

    // Line
    procedure Line(X1, Y1, X2, Y2: Integer; APenColor: TColor;
      APenWidth: Single; APenStyle: TPenStyle; APenColorAlpha: Byte); overload;
    procedure Line(X1, Y1, X2, Y2: Integer; APenColor: TdxAlphaColor;
      APenWidth: Single = 1; APenStyle: TPenStyle = psSolid); overload;
    procedure Line(X1, Y1, X2, Y2: Single; APenColor: TdxAlphaColor;
      APenWidth: Single = 1; APenStyle: TPenStyle = psSolid); overload;
    procedure Line(X1, Y1, X2, Y2: Integer; APen: TdxGPPen); overload;
    procedure Line(X1, Y1, X2, Y2: Single; APen: TdxGPPen); overload;

    // Path
    procedure Path(APath: TdxGPPath; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    procedure Path(APath: TdxGPPath; APenColor, ABrushColor: TdxAlphaColor;
      APenWidth: Single = 1; APenStyle: TPenStyle = psSolid); overload;
    procedure Path(APath: TdxGPPath; APenColor, ABrushColor: TColor;
      APenWidth: Single; APenStyle: TPenStyle;
      APenColorAlpha, ABrushColorAlpha: Byte); overload;
    ...
    procedure Pie(R: TRect; AStartAngle, ASweepAngle: Single; APenColor: TColor;
      ...
      APenColorAlpha, ABrushColorAlpha: Byte); overload;
    procedure Pie(R: TRect; AStartAngle, ASweepAngle: Single;
      APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
      APenStyle: TPenStyle = psSolid); overload;
    ...
    procedure Polygon(const APoints: array of TPoint;
      APenColor, ABrushColor: TColor; APenWidth: Single; APenStyle: TPenStyle;
      APenColorAlpha, ABrushColorAlpha: Byte); overload;
    procedure Polygon(const APoints: array of TPoint;
      APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
      APenStyle: TPenStyle = psSolid); overload;
    procedure Polygon(const APoints: array of TdxPointF;
      APenColor, ABrushColor: TColor; APenWidth: Single; APenStyle: TPenStyle;
      APenColorAlpha, ABrushColorAlpha: Byte); overload;
    procedure Polygon(const APoints: array of TdxPointF;
      APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single;
      APenStyle: TdxGPPenStyle); overload;
    procedure Polygon(const APoints: array of TdxPointF; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    procedure Polygon(const APoints: array of TPoint; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    ...
    procedure Polyline(const APoints: array of TdxPointF; APenColor: TColor;
      APenWidth: Single; APenStyle: TPenStyle; APenColorAlpha: Byte); overload;
    procedure Polyline(const APoints: array of TdxPointF;
      APenColor: TdxAlphaColor; APenWidth: Single = 1;
      APenStyle: TPenStyle = psSolid); overload;
    procedure Polyline(const APoints: array of TdxPointF;
      APen: TdxGPPen); overload;
    procedure Polyline(const APoints: array of TPoint; APen: TdxGPPen);
      overload;

    // Rectangle
    procedure FillRectangle(const R: TdxRectF;
      ABrushColor: TdxAlphaColor); overload;
    procedure FillRectangle(const R: TRect;
      ABrushColor: TdxAlphaColor); overload;
    procedure FillRectangle(const R: TdxRectF;
      ABrush: TdxGPCustomBrush); overload;
    ...
    procedure Rectangle(const R: TdxRectF;
      APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
      APenStyle: TPenStyle = psSolid); overload;
    procedure Rectangle(const R: TdxRectF; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    procedure Rectangle(R: TRect; APen: TdxGPPen;
      ABrush: TdxGPCustomBrush); overload;
    procedure Rectangle(R: TRect; APenColor, ABrushColor: TdxAlphaColor;
      APenWidth: Single = 1; APenStyle: TPenStyle = psSolid); overload;
    procedure Rectangle(R: TRect; APenColor, ABrushColor: TColor;
      APenWidth: Single; APenStyle: TPenStyle;
      APenColorAlpha, ABrushColorAlpha: Byte); overload;

    // Region
    procedure Region(ARegion: TdxGPRegion;
      APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
      APenStyle: TPenStyle = psSolid);

    // RoundRect
    procedure RoundRect(R: TRect; APen: TdxGPPen; ABrush: TdxGPCustomBrush;
      ARadiusX, ARadiusY: Integer); overload;
    procedure RoundRect(R: TRect; APenColor, ABrushColor: TColor;
      ARadiusX, ARadiusY: Integer; APenWidth: Integer;
      APenColorAlpha, ABrushColorAlpha: Byte); overload;
    procedure RoundRect(R: TRect; APenColor, ABrushColor: TdxAlphaColor;
      ARadiusX, ARadiusY: Integer; APenWidth: Single = 1); overload;

    // text
    procedure DrawString(const S: string; AFont: TdxGPFont;
      ABrush: TdxGPCustomBrush; X, Y: Single); overload;
    procedure DrawString(const S: string; AFont: TdxGPFont;
      ABrush: TdxGPCustomBrush; const ARect: TdxRectF); overload;
    procedure DrawString(const S: string; AFont: TdxGPFont;
      ABrush: TdxGPCustomBrush; const APoint: TdxPointF); overload;
    procedure DrawString(const S: string; AFont: TdxGPFont;
      ABrush: TdxGPCustomBrush; X, Y: Single;
      AFormat: TdxGPStringFormat); overload;
    procedure DrawString(const S: string; AFont: TdxGPFont;
      ABrush: TdxGPCustomBrush; const APoint: TdxPointF;
      AFormat: TdxGPStringFormat); overload;
    procedure DrawString(const S: string; AFont: TdxGPFont;
      ABrush: TdxGPCustomBrush; const ARect: TdxRectF;
      AFormat: TdxGPStringFormat); overload;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      const ALayoutArea: TdxSizeF; AStringFormat: TdxGPStringFormat;
      out ACharactersFitted, ALinesFilled: Integer): TdxSizeF; overload;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      const AOrigin: TdxPointF; AStringFormat: TdxGPStringFormat)
      : TdxSizeF; overload;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      const ALayoutArea: TdxSizeF): TdxSizeF; overload;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      const ALayoutArea: TdxSizeF; AStringFormat: TdxGPStringFormat)
      : TdxSizeF; overload;
    function MeasureString(const AText: string; AFont: TdxGPFont)
      : TdxSizeF; overload;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      const AWidth: Double): TdxSizeF; overload;
    function MeasureString(const AText: string; AFont: TdxGPFont;
      const AWidth: Double; AFormat: TdxGPStringFormat): TdxSizeF; overload;
    function MeasureCharacterRanges(const AText: string; AFont: TdxGPFont;
      const ALayoutRect: TdxRectF; AStringFormat: TdxGPStringFormat)
      : TArray<TdxGPRegion>;

    // World Transform
    procedure FlipWorldTransform(AFlipHorizontally, AFlipVertically: Boolean;
      const APivotPoint: TdxPointF); overload; inline;
    procedure FlipWorldTransform(AFlipHorizontally, AFlipVertically: Boolean;
      const APivotPoint: TPoint); overload; inline;
    procedure FlipWorldTransform(AFlipHorizontally, AFlipVertically: Boolean;
      const APivotPointX, APivotPointY: Single); overload;
    function GetWorldTransform: TdxGPMatrix;
    procedure ModifyWorldTransform(AMatrix: TdxGPMatrix;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
    procedure RotateWorldTransform(AAngle: Single;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure RotateWorldTransform(AAngle: Single; const APivotPoint: TPoint;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure RotateWorldTransform(AAngle: Single; const APivotPoint: TdxPointF;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure ScaleWorldTransform(AScaleX, AScaleY: Single;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure ScaleWorldTransform(AScaleX, AScaleY: Single;
      const ACenter: TdxPointF;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend); overload;
    procedure SetWorldTransform(AMatrix: TdxGPMatrix);
    procedure TranslateWorldTransform(AOffsetX, AOffsetY: Single;
      AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);

    procedure TransformPoints(ADestSpace, ASrcSpace: TdxGPCoordinateSpace;
      var APoints: TArray<TdxPointF>); overload;
    procedure TransformPoints(ADestSpace, ASrcSpace: TdxGPCoordinateSpace;
      var APoints: TArray<TPoint>); overload;
    //
    ...
    property CompositionMode: TdxGpCompositionMode read GetCompositionMode
      write SetCompositionMode;
    property InterpolationMode: TdxGPInterpolationMode read GetInterpolationMode
    ...
    property PixelOffsetMode: TdxGpPixelOffsetMode read GetPixelOffsetMode
      write SetPixelOffsetMode;
    property SmoothingMode: TdxGPSmoothingMode read GetSmoothingMode
      write SetSmoothingMode;
    property TextRenderingHint: TdxGpTextRenderingHint read GetTextRenderingHint
      write SetTextRenderingHint;
  end;
...
{$IFDEF DELPHI22}
    function Stat(out statstg: TStatStg; grfStatFlag: DWORD): HResult;
      override; stdcall;
{$ELSE}
    function Stat(out statstg: TStatStg; StatFlag: Integer): HResult;
      override; stdcall;
{$ENDIF}
  end;
  ...
  IdxGPGraphicDraw = interface
    ...
    procedure Draw(ACanvas: TdxGPCanvas; const ADest, ASource: TdxRectF;
      AAttributes: TdxGPImageAttributes); overload;
    procedure Draw(ACanvas: TdxGPCanvas; const ADest, ASource: TRect;
      AAttributes: TdxGPImageAttributes); overload;
  end;
  ...
  TdxGPImageHandle = class(TdxSmartImageCustomHandle, IdxAnimatedImage,
    IdxGpFlipContent, IdxGPGraphicDraw, IdxImageDataFormat)
  strict private
  const
  ...
  strict private
    ...
    function GetPropertyValue(APropID: Cardinal;
      var AData: TdxGPImagePropertyData): Boolean;
  ...
  public
    ...
    procedure Draw(DC: HDC; const ADest, ASource: TdxRectF; AAlpha: Byte = 255;
      APalette: IdxColorPalette = nil); override;
    procedure Draw(DC: HDC; const ADest, ASource: TRect; AAlpha: Byte = 255;
      APalette: IdxColorPalette = nil); override;
    procedure Draw(ACanvas: TdxGPCanvas; const ADest, ASource: TdxRectF;
      AAttributes: TdxGPImageAttributes); overload;
    procedure Draw(ACanvas: TdxGPCanvas; const ADest, ASource: TRect;
  ...
  protected
    class function CheckHeader(AStream: TStream;
      const AHeader: AnsiString): Boolean;
    ...
    class function SaveCore(AStream: TStream; AHandle: TdxGPImageHandle;
      AParameters: Pointer = nil): Boolean; virtual;
  public
    class function CanSaveImage(AHandle: TdxSmartImageCustomHandle)
      : Boolean; override;
    class function Load(AStream: TStream;
      out AHandle: TdxSmartImageCustomHandle): Boolean; override;
    class function Save(AStream: TStream; AHandle: TdxSmartImageCustomHandle)
      : Boolean; override;
  end;
  ...
  public
    ...
    class function GetSize(AStream: TStream; out ASize: TSize): Boolean;
      override;
    class function Load(const ABits: TRGBColors; AAlphaFormat: TAlphaFormat;
      AWidth, AHeight: Integer; out AHandle: TdxSmartImageCustomHandle)
      : Boolean; override;
  end;
  ...
  protected
    ...
    class function ID: TdxImageDataFormat; override;
    class function SaveCore(AStream: TStream; AHandle: TdxGPImageHandle;
      AParameters: Pointer = nil): Boolean; override;
  public
    class function CanLoadStream(AStream: TStream): Boolean; override;
    class function Extensions: string; override;
    class function GetClipboardFormatId: Word; override;
    class function GetSize(AStream: TStream; out ASize: TSize)
      : Boolean; override;
  ...
  public
    ...
    procedure ClearColorKey(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearColorMatrix(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearGamma(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearNoOp(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearOutputChannel(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearOutputChannelColorProfile(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearRemapTable(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure ClearThreshold(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    function Clone: TdxGPImageAttributes;
    procedure GetAdjustedPalette(APalette: TGpColorPalette;
    ...
    procedure SetColorKey(const AColorLow, AColorHigh: TdxAlphaColor;
      const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetColorMatrices(const ANewColorMatrix,
      AGrayMatrix: TdxGpColorMatrix;
    ...
    procedure SetGamma(AGamma: Single;
      const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetNoOp(const AType
      : TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetOutputChannel(const AFlags: TdxGpColorChannelFlags;
      const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetOutputChannelColorProfile(const AColorProfileFilename: string;
      const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetRemapTable(const AMap: array of TGpColorMap;
      const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetThreshold(AThreshold: Single;
      const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
    procedure SetWrapMode(const AMode: TdxGpWrapMode); overload;
    procedure SetWrapMode(const AMode: TdxGpWrapMode; AColor: TdxAlphaColor;
      AClamp: Boolean = False); overload;
  end;
  ...
  protected
    function CreateCache(const ASize: TSize)
      : TdxSmartImageCustomHandle; override;

    property HandleAsObject: TdxSmartImageCustomHandle read GetHandleAsObject
      write SetHandleAsObject;
  public
    function Clone: TdxGPImage;
    function CreateCanvas: TdxGPCanvas;
    procedure Flip(AHorizontally, AVertically: Boolean);
    function MakeComposition(AOverlayImage: TdxGPImage; AOverlayAlpha: Byte)
      : TdxGPImage; overload;
    function MakeComposition(AOverlayImage: TdxGPImage;
      AOverlayAlpha, ASourceAlpha: Byte): TdxGPImage; overload;
    procedure StretchDraw(ACanvas: TdxGPCanvas; const ADest, ASource: TdxRectF;
      AAttributes: TdxGPImageAttributes;
      AColorPalette: IdxColorPalette = nil); overload;
...
{$ENDIF}
  ...
  TdxSmartGlyph = class(TdxSmartImage, IdxSourceDPI, IdxSourceSize)
  strict private
    FSourceDPI: Integer;
  ...
  published
    property SourceDPI: Integer read FSourceDPI write SetSourceDPI
      stored IsSourceDPIStored;
    property SourceHeight: Integer read GetSourceHeight write SetSourceHeight
      stored IsSourceHeightStored;
    property SourceWidth: Integer read GetSourceWidth write SetSourceWidth
      stored IsSourceWidthStored;
  end;
...
const
  ...
  dxGpVerticalAlignmentToLineAlignment: array [TVerticalAlignment]
    of TdxGpStringAlignment = (StringAlignmentNear, StringAlignmentFar,
    StringAlignmentCenter);
  dxGpSmoothStretchModeMap: array [Boolean] of TdxGPInterpolationMode =
    (imNearestNeighbor, imHighQualityBicubic);
  dxGpWordWrapFlagsMap: array [Boolean] of TdxGPStringFormatFlags =
    (StringFormatFlagsNoWrap, StringFormatFlagsNone);
  dxGpRtlReadingFlagsMap: array [Boolean] of TdxGPStringFormatFlags =
    (StringFormatFlagsNone, StringFormatFlagsDirectionRightToLeft);

function dxGpGetAlphaBlendAttributes(const AAlpha: Byte): TdxGPImageAttributes;
function dxGpIsDoubleBufferedNeeded(DC: HDC): Boolean;
function dxGpIsRectVisible(AGraphics: GpGraphics; const R: TdxRectF)
  : LongBool; overload;
function dxGpIsRectVisible(AGraphics: GpGraphics; const R: TRect)
  : LongBool; overload;

function dxGetNearestGradientMode(const AAngle: Double;
  out AInverseOrder: Boolean): TdxGPBrushGradientMode;

procedure dxGpDrawImage(AGraphics: GpGraphics; const ADestRect: TRect;
  const ASourceRect: TRect; AImage: GpImage; AAlpha: Byte = 255;
  AAttributes: GpImageAttributes = nil); overload;
procedure dxGpDrawImage(AGraphics: GpGraphics; const ADestRect: TdxRectF;
  ...
  AAttributes: GpImageAttributes = nil); overload;
procedure dxGpRightToLeftDependentDraw(AGPCanvas: TdxGPCanvas; const R: TRect;
...
procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string;
  ...
  ARendering: TdxGpTextRenderingHint = TextRenderingHintSystemDefault;
  ATrimming: TdxGPStringTrimming = StringTrimmingNone;
  ARtlReading: Boolean = False); overload;
procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string;
...
procedure dxGPGetTextRect(AGraphics: TdxGPCanvas; const AText: string;
  AFont: TFont; AWordWrap: Boolean; const ALayoutRect: TRect;
  out ATextRect: TRect); overload;
procedure dxGPGetTextRect(AGraphics: TdxGPCanvas; const AText: string;
  AFont: TFont; AWordWrap: Boolean; const ALayoutRect: TdxRectF;
  out ATextRect: TdxRectF); overload;
procedure dxGPGetTextRect(AGraphics: TdxGPCanvas; const AText: string;
  AGpFont: GpFont; AWordWrap: Boolean; const ALayoutRect: TdxRectF;
  out ATextRect: TdxRectF); overload;

function dxGpGetBrushHandle(AColor: TdxAlphaColor): IdxGPHandle;
procedure dxGpFillRect(DC: HDC; const R: TRect; AColor: TColor;
  AColorAlpha: Byte = 255); overload;
...
procedure dxGpTilePart(DC: HDC; const ADestRect, ASourceRect: TRect;
  AImage: GpBitmap);
...
function dxGetImageDataFormat(const AFormatId: TGUID)
  : TdxImageDataFormat; overload;
function dxGetImageDataFormat(const AImage: GpImage)
  : TdxImageDataFormat; overload;
function dxGetImageDataFormatExtension(AImageDataFormat
  : TdxImageDataFormat): string;
...
const
  ...
  PenLineStyleToGpLineCap: array [TdxGPPenLineCapStyle] of TdxGpLineCap =
    (LineCapFlat, LineCapSquare, LineCapRound);
  PenDashCapStyleToGpDashCap: array [TdxGPPenDashCapStyle] of TdxGpDashCap =
...
type
  ...
  TdxGPSolidPenCacheID = record
    ...
    constructor Create(AColor: TdxAlphaColor; AWidth: Single;
      AStyle: DashStyle);
  end;

  { TdxGPSolidPenCacheManager }

  TdxGPSolidPenCacheManager = class
    (TdxGPHandleCacheManager<TdxGPSolidPenCacheID>)
  protected
  ...
  public
    ...
    class function GetPen(AColor: TColor; AColorAlpha: Byte; AWidth: Single;
      AStyle: TPenStyle; out AHandle: IdxGPHandle): Boolean; overload;
    class function GetPen(AColor: TdxAlphaColor; AWidth: Single;
      AStyle: DashStyle; out AHandle: IdxGPHandle): Boolean; overload;
    class function GetPen(AColor: TdxAlphaColor; AWidth: Single;
      AStyle: TdxGPPenStyle; out AHandle: IdxGPHandle): Boolean; overload;
    class function GetPen(AColor: TdxAlphaColor; AWidth: Single;
      AStyle: TPenStyle; out AHandle: IdxGPHandle): Boolean; overload;
    class function GetSolidBrush(const AColor: TColor; AColorAlpha: Byte;
      out AHandle: IdxGPHandle): Boolean; overload;
    class function GetSolidBrush(const AColor: TdxAlphaColor;
      out AHandle: IdxGPHandle): Boolean; overload;
    class function GetSolidBrush(const AColor: TdxAlphaColor)
      : IdxGPHandle; overload;
  end;
...
function dxGpGetImageAttributesHandle(AAttrs: TdxGPImageAttributes)
  : GpImageAttributes;
begin
  if AAttrs <> nil then
...
function dxGetImageDataFormatExtension(AImageDataFormat
  : TdxImageDataFormat): string;
const
  Map: array [TdxImageDataFormat] of string = ('', '.bmp', '.jpg', '.png',
    '.tif', '.gif', '.emf', '', '.ico', '', '.wmf');
begin
...
procedure dxGpDrawImage(AGraphics: GpGraphics; const ADestRect: TRect;
  const ASourceRect: TRect; AImage: GpImage; AAlpha: Byte = 255;
  AAttributes: GpImageAttributes = nil);
begin
  dxGpDrawImage(AGraphics, dxRectF(ADestRect), dxRectF(ASourceRect), AImage,
    AAlpha, AAttributes);
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          GdipCheck(GdipSetClipRect(AGraphics, ADestRect.Left, ADestRect.Top,
            ADstW, ADstH, CombineModeIntersect));
      ...
      GdipCheck(GdipDrawImageRectRect(AGraphics, AImage, ADestRect.Left,
        ADestRect.Top, ADstW, ADstH, ASourceRect.Left, ASourceRect.Top, ASrcW,
        ASrcH, guPixel, AAttributes, nil, nil));
    finally
      if ARegion <> nil then
      begin
...
function dxFontStylesToGpFontStyles(AStyles: TFontStyles): Integer;
const
  Map: array [TFontStyle] of TdxGPFontStyle = (FontStyleBold, FontStyleItalic,
    FontStyleUnderline, FontStyleStrikeout);
var
  AStyle: TFontStyle;
begin
  Result := 0;
...
procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string;
  ...
  ARendering: TdxGpTextRenderingHint = TextRenderingHintSystemDefault;
  ATrimming: TdxGPStringTrimming = StringTrimmingNone;
  ARtlReading: Boolean = False);
begin
  dxGPDrawText(AGraphics, AText, cxRectF(ARect), AFont, ATextColor,
    AHorzAlignment, AVertAlignment, AWordWrap, ARendering, ATrimming,
    ARtlReading);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      AStringFormat.LineAlignment := dxGpVerticalAlignmentToLineAlignment
        [AVertAlignment];

      AGpFont := TdxGPResourceManager.GetFont(AFont);
      AGraphics.TextRenderingHint := ARendering;
      AGpRectF := MakeRect(ARect.Left, ARect.Top, cxRectWidth(ARect),
        cxRectHeight(ARect));
      GdipCheck(GdipDrawString(AGraphics.Handle, PWideChar(AText), -1,
        AGpFont.GetNativeHandle, @AGpRectF, AStringFormat.Handle,
...
begin
  ...
  try
    APath.AddString(PWideChar((AText)), AFont, -AFont.Height,
    ...
    try
      ...
      if TdxGPResourceManager.GetSolidBrush(ATextColor, ABrush) then
        GdipCheck(GdipFillPath(AGraphics.Handle, ABrush.GetNativeHandle,
          APath.Handle));
    finally
      AGraphics.TranslateWorldTransform(-ARect.Left, -ARect.Top);
    end;
...
begin
  ...
  ATextRect := cxRect(Trunc(R.Left), Trunc(R.Top), Ceil(R.Left + cxRectWidth(R)
    ), Ceil(R.Top + cxRectHeight(R)));
end;

procedure dxGPGetTextRect(AGraphics: TdxGPCanvas; const AText: string;
  AFont: TFont; AWordWrap: Boolean; const ALayoutRect: TdxRectF;
  out ATextRect: TdxRectF);
begin
  dxGPGetTextRect(AGraphics, AText, TdxGPResourceManager.GetFont(AFont)
    .GetNativeHandle, AWordWrap, ALayoutRect, ATextRect);
end;
...
begin
  ALayoutRectF := MakeRect(ALayoutRect.Left, ALayoutRect.Top,
    cxRectWidth(ALayoutRect), cxRectHeight(ALayoutRect));
  GdipCheck(GdipCreateStringFormat(Integer(dxGpWordWrapFlagsMap[AWordWrap]),
    LANG_NEUTRAL, AStringFormat));
  try
    GdipCheck(GdipMeasureString(AGraphics.Handle, PWideChar(AText),
      Length(AText), AGpFont, @ALayoutRectF, AStringFormat, @ABoundingBoxF,
      ACodePointsFitted, ALinesFilled));
    ATextRect := cxRectF(ABoundingBoxF.X, ABoundingBoxF.Y,
      ABoundingBoxF.X + ABoundingBoxF.Width,
...
begin
  ...
  begin
    dxGPPaintCanvas.BeginPaint(DC, R);
    dxGPPaintCanvas.FillRectangleByGradient(R, dxColorToAlphaColor(AColor1,
      AColor1Alpha), dxColorToAlphaColor(AColor2, AColor2Alpha), AMode);
    dxGPPaintCanvas.EndPaint;
  end;
end;
...
begin
  ...
  begin
    ...
    if AWindowHandle <> 0 then
      Result := GetWindowLong(AWindowHandle, GWL_EXSTYLE) and
        WS_EX_LAYERED <> 0;
  end;
end;
...
begin
  GdipCheck(GdipIsVisibleRect(AGraphics, R.Left, R.Top, R.Right - R.Left,
    R.Bottom - R.Top, Result));
end;

function dxGpIsRectVisible(AGraphics: GpGraphics; const R: TRect): LongBool;
begin
  GdipCheck(GdipIsVisibleRectI(AGraphics, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R), Result));
end;
...
procedure dxGpTilePartEx(AGraphics: GpGraphics;
  ...
  begin
    Result := GdipCreateTexture2I(AImage, WrapModeTile, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R), ATexture) = Ok;
  end;

  procedure ManualTilePart(ADestWidth, ADestHeight, ASourceWidth,
    ASourceHeight: Integer);
  var
    ALastCol, ALastRow, ACol, ARow: Integer;
    RDest, RSrc: TRect;
  begin
    ALastCol := ADestWidth div ASourceWidth -
      Ord(ADestWidth mod ASourceWidth = 0);
    ALastRow := ADestHeight div ASourceHeight -
      Ord(ADestHeight mod ASourceHeight = 0);
  ...
  function CanTilePartViaBrush(ADestWidth, ADestHeight, ASourceWidth,
    ASourceHeight: Integer): Boolean;
  begin
    Result := (ADestWidth > ASourceWidth) and (ADestHeight > ASourceHeight) or
      (Min(ASourceWidth, ASourceHeight) <= 256);
  end;

  function TilePartViaBrush(ADestWidth, ADestHeight, ASourceWidth,
    ASourceHeight: Integer): Boolean;
  var
    ABitmap: GpBitmap;
    ...
    ATexture: GpTexture;
  begin
    Result := CanTilePartViaBrush(ADestWidth, ADestHeight, ASourceWidth,
      ASourceHeight) and CreateTextureBrush(ASourceRect, ATexture);
    if Result then
      try
        GdipCheck(GdipCreateBitmapFromScan0(ADestWidth, ADestHeight, 0,
          PixelFormat32bppPARGB, nil, ABitmap));
        try
          GdipCheck(GdipGetImageGraphicsContext(ABitmap, ABitmapGraphics));
          GdipCheck(GdipFillRectangleI(ABitmapGraphics, ATexture, 0, 0,
            ADestWidth, ADestHeight));
          GdipCheck(GdipDrawImageRectI(AGraphics, ABitmap, ADestRect.Left,
            ADestRect.Top, ADestWidth, ADestHeight));
...
begin
  if not IsRectEmpty(ASourceRect) and dxGpIsRectVisible(AGraphics, ADestRect)
  then
  begin
    ...
    if (AAlpha <> 255) or not TilePartViaBrush(ADestWidth, ADestHeight,
      ASourceWidth, ASourceHeight) then
...
begin
  dxGPPaintCanvas.BeginPaint(DC, R);
  try
    dxGPPaintCanvas.RoundRect(R, APenColor, ABrushColor, ARadius, ARadius,
      APenWidth, APenColorAlpha, ABrushColorAlpha);
  finally
    dxGPPaintCanvas.EndPaint;
  end;
end;
...
begin
  CreateEx(XForm.eM11, XForm.eM12, XForm.eM21, XForm.eM22, XForm.eDx,
    XForm.eDy);
end;
...
constructor TdxGPMatrix.CreateFlip(AFlipHorizontally, AFlipVertically: Boolean;
  const APivotPointX, APivotPointY: Single);
const
...
procedure TdxGPMatrix.Flip(AFlipHorizontally, AFlipVertically: Boolean;
  const APivotPoint: TdxPointF);
var
  AMatrix: TdxGPMatrix;
begin
  ...
  begin
    AMatrix := TdxGPMatrix.CreateFlip(AFlipHorizontally, AFlipVertically,
      APivotPoint.X, APivotPoint.Y);
    try
...
begin
  ...
  Result := dxRectF(Min(Min(ATopLeft.X, ATopRight.X), Min(ABottomLeft.X,
    ABottomRight.X)), Min(Min(ATopLeft.Y, ATopRight.Y), Min(ABottomLeft.Y,
    ABottomRight.Y)), Max(Max(ATopLeft.X, ATopRight.X), Max(ABottomLeft.X,
    ABottomRight.X)), Max(Max(ATopLeft.Y, ATopRight.Y), Max(ABottomLeft.Y,
    ABottomRight.Y)));
end;

procedure TdxGPMatrix.Multiply(AMatrix: TdxGPMatrix;
...
procedure TdxGPMatrix.Rotate(AAngle: Single;
  AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
  GdipCheck(GdipRotateMatrix(Handle, AAngle, AMatrixOrder));
  Changed;
end;

procedure TdxGPMatrix.Rotate(AAngle: Single; const APivotPoint: TdxPointF;
  AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
const
...
procedure TdxGPMatrix.Scale(const AScale: TdxPointF;
  AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
  Scale(AScale.X, AScale.Y, AMatrixOrder);
end;

procedure TdxGPMatrix.Scale(const AScale, ACenter: TdxPointF;
  AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
  SetElements(AScale.X, 0.0, 0.0, AScale.Y, ACenter.X - AScale.X * ACenter.X,
    ACenter.Y - AScale.Y * ACenter.Y);
end;

procedure TdxGPMatrix.Scale(const AScaleX, AScaleY: Single;
  AMatrixOrder: TdxGpMatrixOrder);
begin
...
procedure TdxGPMatrix.Shear(const AShear: TdxPointF;
  AMatrixOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
  Shear(AShear.X, AShear.Y, AMatrixOrder);
end;

procedure TdxGPMatrix.Shear(const AShearX, AShearY: Single;
  AMatrixOrder: TdxGpMatrixOrder);
begin
...
procedure TdxGPMatrix.Translate(const AOffset: TdxPointF;
  AMatrixOrder: TdxGpMatrixOrder);
begin
...
begin
  ...
  for I := Low(APoints) to High(APoints) do
    APoints[I] := Point(Round(APoints[I].X * M11 + APoints[I].Y * M21 + DX),
      Round(APoints[I].X * M12 + APoints[I].Y * M22 + DY));
end;
...
begin
  GetElements(M11, M12, M21, M22, DX, DY);
  for I := Low(APoints) to High(APoints) do
    APoints[I] := dxPointF(APoints[I].X * M11 + APoints[I].Y * M21 + DX,
      APoints[I].X * M12 + APoints[I].Y * M22 + DY);
end;
...
begin
  ...
  begin
    APoints^ := Point(Round(APoints.X * M11 + APoints.Y * M21 + DX),
      Round(APoints.X * M12 + APoints.Y * M22 + DY));
    Inc(APoints);
  end;
end;
...
procedure TdxGPMatrix.Translate(const AOffsetX, AOffsetY: Single;
  AMatrixOrder: TdxGpMatrixOrder);
begin
...
function TdxGPBrushGradientPoints.Add(AOffset: Single;
  AColor: TdxAlphaColor): Integer;
begin
...
procedure TdxGPBrushGradientPoints.CalculateParams(out AColors: PdxAlphaColor;
  out AOffsets: PSingle; out ACount: Integer);
begin
  ACount := Count;
  if Offsets[0] > 0 then
  begin
...
function TdxGPBrushGradientPoints.InternalAdd(AOffset: Single;
  AColor: TdxAlphaColor): Integer;
begin
  Result := Count;
...
procedure TdxGPBrushGradientPoints.SetColor(Index: Integer;
  const AValue: TdxAlphaColor);
begin
  CheckIndex(Index);
...
procedure TdxGPBrushGradientPoints.SetOffset(Index: Integer;
  const AValue: Single);
begin
  CheckIndex(Index);
...
begin
  GdipCheck(GdipCreateHatchBrush(FStyle, FForegroundColor, FBackgroundColor,
    AHandle));
end;
...
var
  ...
  AOffsets: PSingle;
begin
  if (GradientPoints.Count = 0) or (FTargetRect.Width = 0) or
    (FTargetRect.Height = 0) then
  begin
  ...
  GdipCheck(GdipCreateLineBrushFromRect(@FTargetRect, 0, 0, Map[GradientMode],
    WrapModeTileFlipX, AHandle));
...
begin
  ...
  begin
    ApplyTextureTransform;
    GdipCheck(GdipTranslateTextureTransform(Handle, FTargetRect.X,
      FTargetRect.Y, MatrixOrderAppend));
  end;
end;
...
constructor TdxGPPen.Create(AColor: TdxAlphaColor; AWidth: Single = 1;
  APenStyle: TPenStyle = psSolid; AScale: Single = 1);
begin
  inherited Create;
  FBrush := CreateBrush;
  FBrush.Color := AColor;
...
begin
  GdipCheck(GdipSetPenDashCap197819(AHandle,
    PenDashCapStyleToGpDashCap[FDashCapStyle]));
end;

procedure TdxGPPen.DoSetLineEndCapStyle(AHandle: GpHandle);
begin
  GdipCheck(GdipSetPenEndCap(AHandle,
    PenLineStyleToGpLineCap[FLineEndCapStyle]));
end;
...
begin
  GdipCheck(GdipSetPenStartCap(AHandle,
    PenLineStyleToGpLineCap[FLineStartCapStyle]));
end;
...
constructor TdxGPPath.Create(APoints: TdxGpPointFDynArray;
  const APointTypes: TBytes);
begin
  inherited Create;
  GdipCheck(GdipCreatePath2(@APoints[0], @APointTypes[0], Length(APoints),
    FillModeAlternate, FHandle));
end;
...
procedure TdxGPPath.Flatten(AMatrix: TdxGPMatrix = nil;
  AFlatness: Single = 0.25);
begin
...
procedure TdxGPPath.AddArc(const X, Y, Width, Height, StartAngle,
  SweepAngle: Single);
begin
  GdipCheck(GdipAddPathArc(Handle, X, Y, Width, Height, StartAngle,
    SweepAngle));
end;

procedure TdxGPPath.AddBezier(const P1, P2, P3, P4: TdxPointF);
begin
  GdipCheck(GdipAddPathBezier(Handle, P1.X, P1.Y, P2.X, P2.Y, P3.X, P3.Y,
    P4.X, P4.Y));
end;

procedure TdxGPPath.AddEllipse(const R: TRect);
begin
  GdipCheck(GdipAddPathEllipseI(Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R)));
end;

procedure TdxGPPath.AddEllipse(const R: TdxRectF);
begin
  GdipCheck(GdipAddPathEllipse(Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R)));
end;
...
procedure TdxGPPath.AddPie(const X, Y, Width, Height, StartAngle,
  SweepAngle: Single);
begin
  GdipCheck(GdipAddPathPie(Handle, X, Y, Width, Height, StartAngle,
    SweepAngle));
end;
...
begin
  GdipCheck(GdipAddPathRectangleI(Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R)));
end;

procedure TdxGPPath.AddRect(const R: TdxRectF);
begin
  GdipCheck(GdipAddPathRectangle(Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R)));
end;
...
begin
  ...
  FigureStart;
  try
    if (ARadiusX > 0) and (ARadiusY > 0) and not IsZero(ARadiusX) and
      not IsZero(ARadiusY) then
    begin
      AddLine(R.Left + ARadiusX, R.Top, R.Right - 2 * ARadiusX, R.Top);
      AddArc(R.Right - 2 * ARadiusX, R.Top, 2 * ARadiusX, 2 * ARadiusY,
        270, 90);
      AddLine(R.Right, R.Top + ARadiusY, R.Right, R.Bottom - 2 * ARadiusY);
      AddArc(R.Right - 2 * ARadiusX, R.Bottom - 2 * ARadiusY, 2 * ARadiusX,
        2 * ARadiusY, 0, 90);
      AddLine(R.Right - 2 * ARadiusX, R.Bottom, R.Left + ARadiusX, R.Bottom);
      AddArc(R.Left, R.Bottom - 2 * ARadiusY, 2 * ARadiusX,
        2 * ARadiusY, 90, 90);
...
procedure TdxGPPath.AddString(const S: string; AFont: TdxGPFont;
  AFormat: TdxGPStringFormat; const ARect: TdxRectF);
var
  AGpRectF: TdxGpRectF;
...
begin
  ...
  ReleaseDC(0, ADC);
  ASize := ASize * ADpi / 72;
  GdipCheck(GdipAddPathString(Handle, PChar(S), Length(S),
    AFont.FontFamily.Handle, AFont.Style, ASize, @AGpRectF, ANativeFormat));
end;

procedure TdxGPPath.AddString(const S: string; AFont: TdxGPFont;
  AFormat: TdxGPStringFormat; const AOrigin: TdxPointF);
begin
  AddString(S, AFont, AFormat, TdxRectF.CreateSize(AOrigin.X, AOrigin.Y, 0, 0));
end;

procedure TdxGPPath.AddString(const S: string; AFont: TFont; AEmSize: Single;
  const ARect: TRect);
var
  AGpRect: TdxGpRect;
...
begin
  ...
  GdipCheck(GdipCreateFontFamilyFromName(PChar(AFont.Name), nil,
    ANativeFamily));
  try
    AGpRect := ARect;
    GdipCheck(GdipAddPathStringI(Handle, PChar(S), Length(S), ANativeFamily,
      dxFontStylesToGpFontStyles(AFont.Style), AEmSize, @AGpRect,
      ANativeFormat));
  finally
...
procedure TdxGPPath.AddString(const S: string; AFamily: TdxGPFontFamily;
  AStyle: Integer; AEmSize: Single; const AOrigin: TdxPointF;
...
begin
  ...
  if AFormat = nil then
    ...
    ANativeFormat := AFormat.Handle;
  GdipCheck(GdipAddPathString(Handle, PChar(S), Length(S), ANativeFamily,
    AStyle, AEmSize, @ARectf, ANativeFormat));
end;
...
begin
  ...
  if AFormat = nil then
    ...
    ANativeFormat := AFormat.Handle;
  GdipCheck(GdipAddPathStringI(Handle, PChar(S), Length(S), ANativeFamily,
    AStyle, AEmSize, @ARect, ANativeFormat));
end;
...
function TdxGPPath.IsPointInPath(const P: TPoint;
  ACanvas: TdxGPCanvas = nil): Boolean;
var
  AValue: LongBool;
begin
  GdipCheck(GdipIsVisiblePathPointI(Handle, P.X, P.Y,
    dxGpGetCanvasHandle(ACanvas), AValue));
  Result := AValue;
end;

function TdxGPPath.IsPointInPath(const P: TdxPointF;
  ACanvas: TdxGPCanvas = nil): Boolean;
var
  AValue: LongBool;
begin
  GdipCheck(GdipIsVisiblePathPoint(Handle, P.X, P.Y,
    dxGpGetCanvasHandle(ACanvas), AValue));
  Result := AValue;
end;

function TdxGPPath.IsPointInPathOutline(const P: TPoint; APen: TdxGPPen;
  ACanvas: TdxGPCanvas = nil): Boolean;
var
  AValue: LongBool;
begin
  GdipCheck(GdipIsOutlineVisiblePathPointI(Handle, P.X, P.Y, APen.Handle,
    dxGpGetCanvasHandle(ACanvas), AValue));
  Result := AValue;
end;

function TdxGPPath.IsPointInPathOutline(const P: TPoint; APenWidth: Integer;
  ACanvas: TdxGPCanvas = nil): Boolean;
var
  AHandle: IdxGPHandle;
  AValue: LongBool;
begin
  Result := False;
  if TdxGPResourceManager.GetPen(TdxAlphaColors.Red, APenWidth, psSolid, AHandle)
  then
  begin
    GdipCheck(GdipIsOutlineVisiblePathPointI(Handle, P.X, P.Y,
      AHandle.GetNativeHandle, dxGpGetCanvasHandle(ACanvas), AValue));
    Result := AValue;
  end;
end;

function TdxGPPath.TryGetBounds(out ARect: TRect; APen: TdxGPPen = nil)
  : GpStatus;
var
  R: TdxGpRect;
begin
...
function TdxGPPath.TryGetBoundsF(out ARect: TdxRectF; APen: TdxGPPen = nil)
  : GpStatus;
var
  R: TdxGpRectF;
begin
...
function TdxGPPath.IsPointInPathOutline(const P: TdxPointF; APen: TdxGPPen;
  ACanvas: TdxGPCanvas = nil): Boolean;
var
  AValue: LongBool;
begin
  GdipCheck(GdipIsOutlineVisiblePathPoint(Handle, P.X, P.Y, APen.Handle,
    dxGpGetCanvasHandle(ACanvas), AValue));
  Result := AValue;
end;
...
constructor TdxGPRegion.CreateFromRegion(ARegion: GpRegion;
  ADummy: Integer = 0);
begin
  inherited Create;
...
procedure TdxGPRegion.CombineRegionPath(const APath: TdxGPPath;
  AMode: TdxGPCombineMode);
begin
  GdipCheck(GdipCombineRegionPath(Handle, APath.Handle, TGpCombineMode(AMode)));
end;

procedure TdxGPRegion.CombineRegionRect(const ARect: TRect;
  AMode: TdxGPCombineMode);
var
  R: TdxGpRect;
begin
...
procedure TdxGPRegion.CombineRegionRegion(const ARegion: TdxGPRegion;
  AMode: TdxGPCombineMode; AFreeRegion: Boolean = False);
begin
  GdipCheck(GdipCombineRegionRegion(Handle, ARegion.Handle,
    TGpCombineMode(AMode)));
  if AFreeRegion then
    ARegion.Free;
end;
...
procedure TdxGPRegion.CombineRegionRegion(const ARegion: HRGN;
  AMode: TdxGPCombineMode; AFreeRegion: Boolean = False);
var
  AHandle: GpRegion;
begin
...
constructor TdxGPStringFormat.Create(AFormatAttributes: Integer = 0;
  ALanguage: LANGID = LANG_NEUTRAL);
begin
  inherited Create;
...
procedure TdxGPStringFormat.SetTabStops(AFirstTabOffset: Single;
  ACount: Integer; const ATabStops: TArray<Single>);
begin
  GdipCheck(GdipSetStringFormatTabStops(FHandle, AFirstTabOffset, ACount,
    @ATabStops[0]));
end;
...
function TdxGPStringFormat.GetTabStops(ACount: Integer;
  out AFirstTabOffset: Single): TArray<Single>;
begin
  SetLength(Result, ACount);
  GdipCheck(GdipGetStringFormatTabStops(FHandle, ACount, @AFirstTabOffset,
    @Result[0]));
end;

procedure TdxGPStringFormat.SetDigitSubstitution(ALanguage: LANGID;
  ASubstitute: TdxGPStringDigitSubstitute);
begin
  GdipCheck(GdipSetStringFormatDigitSubstitution(FHandle, ALanguage,
    ASubstitute));
end;
...
function TdxGPStringFormat.GetDigitSubstitutionMethod
...
procedure TdxGPStringFormat.SetLineAlignment(const AValue
...
procedure TdxGPStringFormat.SetMeasurableCharacterRanges(ACount: Integer;
  const ARanges: TArray<TdxGPCharacterRange>);
begin
  GdipCheck(GdipSetStringFormatMeasurableCharacterRanges(FHandle, ACount,
    @ARanges[0]));
end;
...
constructor TdxGPFontFamily.Create(const AName
  : string { TODO: ; AFontCollection: TdxGPFontCollection } );
begin
  inherited Create;
  GdipCreateFontFamilyFromName(PChar(AName),
    nil { TODO: AFontCollection.Handle } , FHandle);
end;
...
begin
  ...
  GdipCheck(GdipGetFontCollectionFamilyList(FHandle, ACount, AFamilyArray,
    AActualCount));
...
constructor TdxGPFont.Create(ANativeFont: GpFont; AGdiCharSet: Byte;
...
begin
  ...
  Initialize(FFontFamily, ASize, TdxGPFontStyle(AStyle), AUnit, AGdiCharSet,
    AGdiVerticalFont);
end;
...
begin
  inherited Create;
  Initialize(AFamilyName, AEmSize, FontStyleRegular, guPoint, DEFAULT_CHARSET,
    IsVerticalName(AFamilyName));
end;
...
begin
  inherited Create;
  Initialize(AFamilyName, AEmSize, FontStyleRegular, AUnit, DEFAULT_CHARSET,
    IsVerticalName(AFamilyName));
end;
...
begin
  inherited Create;
  Initialize(AFamilyName, AEmSize, AStyle, guPoint, DEFAULT_CHARSET,
    IsVerticalName(AFamilyName));
end;
...
begin
  inherited Create;
  Initialize(AFamilyName, AEmSize, AStyle, AUnit, DEFAULT_CHARSET,
    IsVerticalName(AFamilyName));
end;
...
begin
  inherited Create;
  Initialize(AFamily, AEmSize, FontStyleRegular, guPoint,
...
begin
  ...
  Initialize(AFamilyName, AEmSize, AStyle, AUnit, AGdiCharSet,
    AGdiVerticalFont);
end;
...
begin
  ...
  Initialize(APrototype.FontFamily, APrototype.Size, ANewStyle,
    APrototype.&Unit, DEFAULT_CHARSET, False);
end;
...
begin
  GdipCheck(GdipCreateFont(FFontFamily.Handle, FFontSize, Ord(FFontStyle),
    Ord(FFontUnit), FNativeFont));
end;

class function TdxGPFont.SafeCreate(const AFamilyName: string; AEmSize: Single;
  AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit = guPoint;
  AGdiCharSet: Byte = DEFAULT_CHARSET): TdxGPFont;
var
...
begin
  ...
  try
    ...
    Result := TdxGPFont.Create(AFontFamily, AEmSize, AStyle, AUnit,
      AGdiCharSet);
  finally
...
procedure TdxGPFont.Initialize(const AFamilyName: string; AEmSize: Single;
  AStyle: TdxGPFontStyle; AUnit: TdxGraphicUnit; AGdiCharSet: Byte;
  AGdiVerticalFont: Boolean);
begin
  FOriginalFontName := AFamilyName;
  FFontFamily := TdxGPFontFamily.Create(StripVerticalName(AFamilyName)
    { TODO: , True } );
  Initialize(FFontFamily, AEmSize, AStyle, AUnit, AGdiCharSet,
    AGdiVerticalFont);
end;
...
class function TdxGPFont.FromLogFont(const ALogFont: TLogFont; AHdc: THandle)
  : TdxGPFont;
var
  AFont: GpFont;
begin
  ...
  Result := TdxGPFont.Create(AFont, ALogFont.lfCharSet,
...
begin
  Result := ((FFontStyle shl 13) or (FFontStyle shr 19))
    xor ((Ord(FFontUnit) shl 26) or (Ord(FFontUnit) shr 6))
    xor ((Round(FFontSize) shl 7) or (Round(FFontSize) shr 25));
end;
...
begin
  ...
  begin
    Move(ALogFont.lfFaceName[0], ALogFont.lfFaceName[1],
      SizeOf(ALogFont.lfFaceName) - SizeOf(Char));
    ALogFont.lfFaceName[0] := '@';
  end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        AEmHeightInPixels := ALineSpacingInPixels * FontFamily.GetEmHeight
          (Style) / FontFamily.GetLineSpacing(Style);
...
begin
  dxTestCheck(FSavedClipRegions.Count = 0,
    ClassName + '.FSavedClipRegions <> 0');
  dxTestCheck(FSavedWorldTransforms.Count = 0,
    ClassName + '.FSavedWorldTransforms <> 0');
  FreeHandle;
...
procedure TdxGPCanvas.Draw(AGraphic: TdxGPImage; const R: TdxRectF;
  AAlpha: Byte = 255);
begin
  Draw(AGraphic, R, dxRectF(AGraphic.ClientRect), AAlpha);
end;

procedure TdxGPCanvas.Draw(AGraphic: TdxGPImage; const R: TRect;
  AAlpha: Byte = 255);
begin
...
procedure TdxGPCanvas.Draw(AGraphic: TdxGPImage;
  const APoints: array of TdxPointF);
begin
  GdipCheck(GdipDrawImagePoints(Handle, AGraphic.Handle, @APoints[0],
    Length(APoints)));
end;

procedure TdxGPCanvas.Draw(AGraphic: TdxGPImage;
  const APoints: array of TdxPointF; AAttributes: TdxGPImageAttributes);
begin
  GdipCheck(GdipDrawImagePointsRect(Handle, AGraphic.Handle, @APoints[0],
    Length(APoints), 0, 0, AGraphic.Width, AGraphic.Height, guPixel,
    AAttributes.Handle, nil, nil));
end;

procedure TdxGPCanvas.Draw(AGraphic: TdxGPImage;
  const ADestRect, ASourceRect: TdxRectF; AAlpha: Byte = 255);
begin
  AGraphic.StretchDraw(Self, ADestRect, ASourceRect,
    dxGpGetAlphaBlendAttributes(AAlpha));
end;

procedure TdxGPCanvas.Draw(AGraphic: TdxGPImage;
  const ADestRect, ASourceRect: TdxRectF; AAttributes: TdxGPImageAttributes);
begin
...
procedure TdxGPCanvas.DrawBitmap(ABitmap: TBitmap; const R: TRect;
  AAlpha: Byte = 255);
var
...
begin
  ...
  begin
    ...
    try
      ...
      AImage := TdxGPImageHandle.CreateFromBits(ABitmap.Width, ABitmap.Height,
        AData.FBits, AData.FBitsFormat);
      try
        AImage.Draw(Self, R, cxRect(AImage.Size),
          dxGpGetAlphaBlendAttributes(AAlpha));
      finally
        AImage.Free;
      end;
...
procedure TdxGPCanvas.DrawTile(AGraphic: TdxGPImage; const R: TRect;
  AAlpha: Byte);
begin
  DrawTile(AGraphic, R, AGraphic.ClientRect, AAlpha);
end;

procedure TdxGPCanvas.DrawTile(AGraphic: TdxGPImage;
  const ADestRect, ASourceRect: TRect; AAlpha: Byte);
begin
...
begin
  GdipCheck(GdipSetClipRectI(Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R), TGpCombineMode(AMode)));
end;

procedure TdxGPCanvas.SetClipRect(const R: TdxRectF; AMode: TdxGPCombineMode);
begin
  GdipCheck(GdipSetClipRect(Handle, R.Left, R.Top, cxRectWidth(R),
    cxRectHeight(R), TGpCombineMode(AMode)));
end;
...
procedure TdxGPCanvas.SetClipRegion(ARegion: TdxGPRegion;
  AMode: TdxGPCombineMode);
begin
...
procedure TdxGPCanvas.Arc(R: TRect; AStartAngle, ASweepAngle: Single;
  APenColor: TColor; APenWidth: Single; APenStyle: TPenStyle;
  APenColorAlpha: Byte);
var
  APen: IdxGPHandle;
begin
  CheckDestRect(R);
  if TdxGPResourceManager.GetPen(APenColor, APenColorAlpha, APenWidth,
    APenStyle, APen) then
    GdipCheck(GdipDrawArcI(Handle, APen.GetNativeHandle, R.Left, R.Top,
...
begin
  Curve(APoints, dxColorToAlphaColor(APenColor, APenColorAlpha), APenWidth,
    APenStyle);
end;

procedure TdxGPCanvas.Curve(const APoints: array of TPoint;
  APenColor: TdxAlphaColor; APenWidth: Single = 1;
  APenStyle: TPenStyle = psSolid);
var
  APen: IdxGPHandle;
begin
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, APen) then
    GdipCheck(GdipDrawCurveI(Handle, APen.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;

procedure TdxGPCanvas.Curve(const APoints: array of TdxPointF;
  APenColor: TColor; APenWidth: Single; APenStyle: TPenStyle;
  APenColorAlpha: Byte);
var
  APen: IdxGPHandle;
begin
  if TdxGPResourceManager.GetPen(APenColor, APenColorAlpha, APenWidth,
    APenStyle, APen) then
    GdipCheck(GdipDrawCurve(Handle, APen.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;

procedure TdxGPCanvas.Ellipse(R: TRect; APen: TdxGPPen;
...
begin
  ...
  begin
    ...
    GdipCheck(GdipFillEllipseI(Handle, ABrush.Handle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  end;

  if APen <> nil then
  begin
    APen.SetTargetRect(R);
    GdipCheck(GdipDrawEllipseI(Handle, APen.Handle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  end;
end;

procedure TdxGPCanvas.Ellipse(R: TdxRectF; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush);
begin
  if ABrush <> nil then
  begin
    ABrush.SetTargetRect(R);
    GdipCheck(GdipFillEllipse(Handle, ABrush.Handle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  end;
  ...
  begin
    APen.SetTargetRect(R);
    GdipCheck(GdipDrawEllipse(Handle, APen.Handle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  end;
end;
...
procedure TdxGPCanvas.Ellipse(R: TRect; APenColor, ABrushColor: TColor;
  APenWidth: Single; APenStyle: TPenStyle;
  APenColorAlpha, ABrushColorAlpha: Byte);
begin
  Ellipse(R, dxColorToAlphaColor(APenColor, APenColorAlpha),
    dxColorToAlphaColor(ABrushColor, ABrushColorAlpha), APenWidth, APenStyle);
end;

procedure TdxGPCanvas.Ellipse(R: TRect; APenColor, ABrushColor: TdxAlphaColor;
...
begin
  ...
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillEllipseI(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawEllipseI(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
end;

procedure TdxGPCanvas.Ellipse(R: TdxRectF;
  APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
  APenStyle: TPenStyle = psSolid);
var
  AHandle: IdxGPHandle;
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillEllipse(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawEllipse(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
end;

procedure TdxGPCanvas.Ellipse(R: TdxRectF; APenColor, ABrushColor: TColor;
  APenWidth: Single; APenStyle: TPenStyle;
  APenColorAlpha, ABrushColorAlpha: Byte);
begin
  Ellipse(R, dxColorToAlphaColor(APenColor, APenColorAlpha),
...
procedure TdxGPCanvas.Line(X1, Y1, X2, Y2: Integer; APenColor: TColor;
  APenWidth: Single; APenStyle: TPenStyle; APenColorAlpha: Byte);
begin
  Line(X1, Y1, X2, Y2, dxColorToAlphaColor(APenColor, APenColorAlpha),
    APenWidth, APenStyle);
end;

procedure TdxGPCanvas.Line(X1, Y1, X2, Y2: Integer; APenColor: TdxAlphaColor;
  APenWidth: Single; APenStyle: TPenStyle);
var
  AHandle: IdxGPHandle;
begin
...
procedure TdxGPCanvas.Line(X1, Y1, X2, Y2: Single; APenColor: TdxAlphaColor;
  APenWidth: Single; APenStyle: TPenStyle);
var
  AHandle: IdxGPHandle;
begin
...
procedure TdxGPCanvas.Path(APath: TdxGPPath; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush);
var
  R: TdxRectF;
begin
...
procedure TdxGPCanvas.Path(APath: TdxGPPath;
  APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single;
  APenStyle: TPenStyle);
var
  AHandle: IdxGPHandle;
...
procedure TdxGPCanvas.Path(APath: TdxGPPath; APenColor, ABrushColor: TColor;
  ...
  APenColorAlpha, ABrushColorAlpha: Byte);
begin
  Path(APath, dxColorToAlphaColor(APenColor, APenColorAlpha),
    dxColorToAlphaColor(ABrushColor, ABrushColorAlpha), APenWidth, APenStyle);
end;

procedure TdxGPCanvas.Pie(R: TRect; AStartAngle, ASweepAngle: Single;
  APenColor: TColor; ABrushColor: TColor; APenWidth: Single;
  APenStyle: TPenStyle; APenColorAlpha, ABrushColorAlpha: Byte);
begin
  Pie(R, AStartAngle, ASweepAngle, dxColorToAlphaColor(APenColor,
    APenColorAlpha), dxColorToAlphaColor(ABrushColor, ABrushColorAlpha),
    APenWidth, APenStyle);
end;
...
begin
  CheckDestRect(R);
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillPieI(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R), -AStartAngle, -ASweepAngle));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawPieI(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R), -AStartAngle, -ASweepAngle));
end;
...
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillPie(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R), -AStartAngle, -ASweepAngle));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawPie(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R), -AStartAngle, -ASweepAngle));
end;
...
procedure TdxGPCanvas.Polygon(const APoints: array of TPoint;
  ...
  APenColorAlpha, ABrushColorAlpha: Byte);
begin
  Polygon(APoints, dxColorToAlphaColor(APenColor, APenColorAlpha),
    dxColorToAlphaColor(ABrushColor, ABrushColorAlpha), APenWidth, APenStyle);
end;
...
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillPolygonI(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints), FillModeWinding));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawPolygonI(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;
...
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillPolygon(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints), FillModeWinding));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawPolygon(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;
...
var
  AHandle: IdxGPHandle;
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, ABrushColorAlpha, AHandle)
  then
    GdipCheck(GdipFillPolygon(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints), FillModeWinding));
  if TdxGPResourceManager.GetPen(APenColor, APenColorAlpha, APenWidth,
    APenStyle, AHandle) then
    GdipCheck(GdipDrawPolygon(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;

procedure TdxGPCanvas.Polygon(const APoints: array of TdxPointF; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush);
var
  ABox: TdxRectF;
begin
  ...
  begin
    ...
    GdipCheck(GdipFillPolygon(Handle, ABrush.Handle, @APoints[0],
      Length(APoints), FillModeWinding));
  end;
  if APen <> nil then
  begin
    APen.SetTargetRect(ABox);
    GdipCheck(GdipDrawPolygon(Handle, APen.Handle, @APoints[0],
      Length(APoints)));
  end;
end;

procedure TdxGPCanvas.Polygon(const APoints: array of TPoint; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush);
var
  ABox: TRect;
begin
  ...
  begin
    ABrush.SetTargetRect(ABox);
    GdipCheck(GdipFillPolygonI(Handle, ABrush.Handle, @APoints[0],
      Length(APoints), FillModeWinding));
  end;
  if APen <> nil then
  begin
    APen.SetTargetRect(ABox);
    GdipCheck(GdipDrawPolygonI(Handle, APen.Handle, @APoints[0],
      Length(APoints)));
  end;
end;
...
procedure TdxGPCanvas.Polyline(const APoints: array of TPoint;
  ...
  APenColorAlpha: Byte);
begin
  Polyline(APoints, dxColorToAlphaColor(APenColor, APenColorAlpha), APenWidth,
    APenStyle);
end;
...
begin
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawLinesI(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;
...
begin
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawLines(Handle, AHandle.GetNativeHandle, @APoints[0],
      Length(APoints)));
end;
...
begin
  ...
  begin
    APen.SetTargetRect(cxPointsBox(APoints));
    GdipCheck(GdipDrawLinesI(Handle, APen.Handle, @APoints[0],
      Length(APoints)));
  end;
end;

procedure TdxGPCanvas.FillRectangle(const R: TdxRectF;
  ABrushColor: TdxAlphaColor);
var
  ABrush: IdxGPHandle;
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, ABrush) then
    GdipCheck(GdipFillRectangle(Handle, ABrush.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
end;
...
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, ABrush) then
    GdipCheck(GdipFillRectangleI(Handle, ABrush.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
end;
...
begin
  ABrush.SetTargetRect(R);
  GdipCheck(GdipFillRectangle(Handle, ABrush.Handle, R.Left, R.Top,
    cxRectWidth(R), cxRectHeight(R)));
end;
...
begin
  ABrush.SetTargetRect(R);
  GdipCheck(GdipFillRectangleI(Handle, ABrush.Handle, R.Left, R.Top,
    cxRectWidth(R), cxRectHeight(R)));
end;

procedure TdxGPCanvas.FillRectangleByGradient(const R: TRect;
  AColor1, AColor2: TdxAlphaColor; AMode: TdxGPLinearGradientMode);
var
...
begin
  ...
  begin
    // Inflate: Avoid GDIPlus gradient fill bug
    ABrushRect := MakeRect(R.Left - 1, R.Top - 1, R.Right - R.Left + 2,
      R.Bottom - R.Top + 2);
    GdipCheck(GdipCreateLineBrushFromRectI(@ABrushRect, AColor1, AColor2, AMode,
      WrapModeTile, ABrush));
    GdipCheck(GdipFillRectangleI(Handle, ABrush, R.Left, R.Top,
...
procedure TdxGPCanvas.Rectangle(const R: TdxRectF;
  APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
  APenStyle: TPenStyle = psSolid);
var
  AHandle: IdxGPHandle;
begin
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillRectangle(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawRectangle(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
end;

procedure TdxGPCanvas.Rectangle(const R: TdxRectF; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush);
begin
  if ABrush <> nil then
  begin
  ...
  begin
    APen.SetTargetRect(R);
    GdipCheck(GdipDrawRectangle(Handle, APen.Handle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  end;
end;

procedure TdxGPCanvas.Rectangle(R: TRect; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush);
begin
  CheckDestRect(R);
  ...
  begin
    APen.SetTargetRect(R);
    GdipCheck(GdipDrawRectangleI(Handle, APen.Handle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  end;
end;
...
procedure TdxGPCanvas.Rectangle(R: TRect; APenColor, ABrushColor: TColor;
  ...
  APenColorAlpha, ABrushColorAlpha: Byte);
begin
  Rectangle(R, dxColorToAlphaColor(APenColor, APenColorAlpha),
    dxColorToAlphaColor(ABrushColor, ABrushColorAlpha), APenWidth, APenStyle);
end;
...
begin
  CheckDestRect(R);
  if TdxGPResourceManager.GetSolidBrush(ABrushColor, AHandle) then
    GdipCheck(GdipFillRectangleI(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
  if TdxGPResourceManager.GetPen(APenColor, APenWidth, APenStyle, AHandle) then
    GdipCheck(GdipDrawRectangleI(Handle, AHandle.GetNativeHandle, R.Left, R.Top,
      cxRectWidth(R), cxRectHeight(R)));
end;

procedure TdxGPCanvas.Region(ARegion: TdxGPRegion;
  APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
  APenStyle: TPenStyle = psSolid);
var
  ABrush: IdxGPHandle;
...
procedure TdxGPCanvas.RoundRect(R: TRect; APen: TdxGPPen;
  ABrush: TdxGPCustomBrush; ARadiusX, ARadiusY: Integer);
var
  APath: TdxGPPath;
begin
  APath := TdxGPPath.Create;
...
procedure TdxGPCanvas.RoundRect(R: TRect; APenColor, ABrushColor: TColor;
  ARadiusX, ARadiusY: Integer; APenWidth: Integer;
  APenColorAlpha, ABrushColorAlpha: Byte);
begin
  RoundRect(R, dxColorToAlphaColor(APenColor, APenColorAlpha),
    dxColorToAlphaColor(ABrushColor, ABrushColorAlpha), ARadiusX, ARadiusY,
    APenWidth);
end;

procedure TdxGPCanvas.RoundRect(R: TRect; APenColor, ABrushColor: TdxAlphaColor;
  ARadiusX, ARadiusY: Integer; APenWidth: Single);
var
  APath: TdxGPPath;
begin
  APath := TdxGPPath.Create;
...
procedure TdxGPCanvas.DrawString(const S: string; AFont: TdxGPFont;
  ABrush: TdxGPCustomBrush; X, Y: Single);
begin
...
procedure TdxGPCanvas.DrawString(const S: string; AFont: TdxGPFont;
  ABrush: TdxGPCustomBrush; X, Y: Single; AFormat: TdxGPStringFormat);
begin
...
procedure TdxGPCanvas.DrawString(const S: string; AFont: TdxGPFont;
  ...
  AFormat: TdxGPStringFormat);
begin
  DrawString(S, AFont, ABrush, cxRectFBounds(APoint.X, APoint.Y, 0, 0),
    AFormat);
end;

procedure TdxGPCanvas.DrawString(const S: string; AFont: TdxGPFont;
  ABrush: TdxGPCustomBrush; const ARect: TdxRectF; AFormat: TdxGPStringFormat);
var
  AGpRectF: TdxGpRectF;
...
begin
  ...
  AGpRectF := MakeRect(ARect.Left, ARect.Top, ARect.Right - ARect.Left,
    ARect.Bottom - ARect.Top);
  GdipCheck(GdipDrawString(Handle, PChar(S), Length(S), AFont.NativeFont,
    @AGpRectF, ANativeStringFormat, ABrush.Handle));
end;

function TdxGPCanvas.MeasureString(const AText: string; AFont: TdxGPFont;
  ALayoutRect: TdxGpRectF; AStringFormat: TdxGPStringFormat;
  out ACharactersFitted, ALinesFilled: Integer): TdxSizeF;
var
...
begin
  ...
  GdipCheck(GdipMeasureString(Handle, PChar(AText), Length(AText),
    AFont.NativeFont, @ALayoutRect, ANativeFormat, @ABoundingBox,
...
begin
  ...
  Result := MeasureString(AText, AFont, ALayoutRect, AStringFormat,
    ACharactersFitted, ALinesFilled);
end;

function TdxGPCanvas.MeasureString(const AText: string; AFont: TdxGPFont;
  const AOrigin: TdxPointF; AStringFormat: TdxGPStringFormat): TdxSizeF;
var
  ALayoutRect: TdxGpRectF;
  A, B: Integer;
begin
...
function TdxGPCanvas.MeasureString(const AText: string; AFont: TdxGPFont;
  const ALayoutArea: TdxSizeF): TdxSizeF;
begin
  Result := MeasureString(AText, AFont, ALayoutArea, nil);
end;

function TdxGPCanvas.MeasureString(const AText: string; AFont: TdxGPFont;
  const ALayoutArea: TdxSizeF; AStringFormat: TdxGPStringFormat): TdxSizeF;
var
  ALayoutRect: TdxGpRectF;
  A, B: Integer;
begin
...
function TdxGPCanvas.MeasureString(const AText: string; AFont: TdxGPFont;
  const AWidth: Double): TdxSizeF;
begin
  Result := MeasureString(AText, AFont, TdxSizeF.Create(AWidth, 999999));
end;

function TdxGPCanvas.MeasureString(const AText: string; AFont: TdxGPFont;
  const AWidth: Double; AFormat: TdxGPStringFormat): TdxSizeF;
begin
  Result := MeasureString(AText, AFont, TdxSizeF.Create(AWidth,
    999999), AFormat);
end;

function TdxGPCanvas.MeasureCharacterRanges(const AText: string;
  AFont: TdxGPFont; const ALayoutRect: TdxRectF;
...
begin
  ...
  GdipCheck(GdipGetStringFormatMeasurableCharacterRangeCount
    (ANativeFormat, ACount));
  ...
  try
    ...
    GdipCheck(GdipMeasureCharacterRanges(Handle, PChar(AText), Length(AText),
      AFont.NativeFont, @AGPLayoutRect, ANativeFormat, ACount, @AGpRegions[0]));
  finally
    AGpRegions := nil;
  end;
end;

procedure TdxGPCanvas.FlipWorldTransform(AFlipHorizontally, AFlipVertically
  : Boolean; const APivotPoint: TdxPointF);
begin
  FlipWorldTransform(AFlipHorizontally, AFlipVertically, APivotPoint.X,
    APivotPoint.Y);
end;

procedure TdxGPCanvas.FlipWorldTransform(AFlipHorizontally, AFlipVertically
  : Boolean; const APivotPoint: TPoint);
begin
  FlipWorldTransform(AFlipHorizontally, AFlipVertically, APivotPoint.X,
    APivotPoint.Y);
end;
...
procedure TdxGPCanvas.FlipWorldTransform(AFlipHorizontally, AFlipVertically
  : Boolean; const APivotPointX, APivotPointY: Single);
var
  AMatrix: TdxGPMatrix;
begin
  if AFlipHorizontally or AFlipVertically then
...
procedure TdxGPCanvas.ModifyWorldTransform(AMatrix: TdxGPMatrix;
  AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
...
procedure TdxGPCanvas.RotateWorldTransform(AAngle: Single;
  const APivotPoint: TdxPointF; AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
const
  DirectionMap: array [TdxGpMatrixOrder] of Integer = (1, -1);
begin
  TranslateWorldTransform(DirectionMap[AOrder] * APivotPoint.X,
    DirectionMap[AOrder] * APivotPoint.Y, AOrder);
  RotateWorldTransform(AAngle, AOrder);
  TranslateWorldTransform(-DirectionMap[AOrder] * APivotPoint.X,
    -DirectionMap[AOrder] * APivotPoint.Y, AOrder);
end;

procedure TdxGPCanvas.ScaleWorldTransform(AScaleX, AScaleY: Single;
  AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
...
procedure TdxGPCanvas.ScaleWorldTransform(AScaleX, AScaleY: Single;
  const ACenter: TdxPointF; AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
var
  AMatrix: TdxGPMatrix;
begin
  AMatrix := TdxGPMatrix.CreateEx(AScaleX, 0.0, 0.0, AScaleY,
    ACenter.X - AScaleX * ACenter.X, ACenter.Y - AScaleY * ACenter.Y);
  try
...
procedure TdxGPCanvas.TranslateWorldTransform(AOffsetX, AOffsetY: Single;
  AOrder: TdxGpMatrixOrder = MatrixOrderPrepend);
begin
...
procedure TdxGPCanvas.TransformPoints(ADestSpace,
  ASrcSpace: TdxGPCoordinateSpace; var APoints: TArray<TdxPointF>);
begin
  if Length(APoints) = 0 then
    raise EArgumentOutOfRangeException.Create('points');
  GdipCheck(GdipTransformPoints(Handle, ADestSpace, ASrcSpace, Pointer(APoints),
    Length(APoints)));
end;

procedure TdxGPCanvas.TransformPoints(ADestSpace,
  ASrcSpace: TdxGPCoordinateSpace; var APoints: TArray<TPoint>);
begin
  if Length(APoints) = 0 then
    raise EArgumentOutOfRangeException.Create('points');
  GdipCheck(GdipTransformPointsI(Handle, ADestSpace, ASrcSpace,
    Pointer(APoints), Length(APoints)));
end;
...
begin
  ...
  if AResult in [Ord(Low(TdxGPInterpolationMode))
    .. Ord(High(TdxGPInterpolationMode))] then
...
begin
  ...
  if AResult in [Ord(Low(TdxGPSmoothingMode)) .. Ord(High(TdxGPSmoothingMode))]
  then
...
begin
  ...
  BitBlt(FBuffer.Canvas.Handle, 0, 0, FBuffer.Width, FBuffer.Height, DC, R.Left,
    R.Top, SRCCOPY);
...
function TdxGPStreamAdapter.Stat(out statstg: TStatStg;
...
constructor TdxGPImageHandle.CreateFromBits(AWidth, AHeight: Integer;
  const ABitsRef: TRGBColors; AAlphaFormat: TAlphaFormat = afPremultiplied);
const
  PixelFormatMap: array [TAlphaFormat] of Integer = (PixelFormat32bppRGB,
    PixelFormat32bppARGB, PixelFormat32bppPARGB);
var
  AHandle: GpImage;
begin
  GdipCheck(GdipCreateBitmapFromScan0(AWidth, AHeight, AWidth * 4,
    PixelFormatMap[AAlphaFormat], @ABitsRef[0], AHandle));
  Create(AHandle);
end;
...
begin
  GdipCheck(GdipCreateBitmapFromScan0(AWidth, AHeight, AWidth * 4,
    PixelFormat32bppPARGB, nil, AHandle));
  Create(AHandle);
end;
...
procedure TdxGPImageHandle.Draw(DC: HDC; const ADest, ASource: TdxRectF;
  AAlpha: Byte; APalette: IdxColorPalette);
begin
  if Handle <> nil then
  begin
    dxGPPaintCanvas.BeginPaint(DC, cxRect(ADest, False));
    try
      Draw(dxGPPaintCanvas, ADest, ASource,
        dxGpGetAlphaBlendAttributes(AAlpha));
    finally
      dxGPPaintCanvas.EndPaint;
    end;
...
procedure TdxGPImageHandle.Draw(DC: HDC; const ADest, ASource: TRect;
  AAlpha: Byte; APalette: IdxColorPalette);
begin
  if Handle <> nil then
  begin
...
procedure TdxGPImageHandle.Draw(ACanvas: TdxGPCanvas;
  const ADest, ASource: TdxRectF; AAttributes: TdxGPImageAttributes);
var
  AActualSource: TdxRectF;
begin
  AActualSource := ASource;
  if FScaleFactor > 0 then
    AActualSource := cxRectScale(AActualSource, 1 / FScaleFactor);
  dxGpDrawImage(ACanvas.Handle, ADest, ASource, Handle, MaxByte,
    dxGpGetImageAttributesHandle(AAttributes));
end;

procedure TdxGPImageHandle.Draw(ACanvas: TdxGPCanvas;
  const ADest, ASource: TRect; AAttributes: TdxGPImageAttributes);
var
  AActualSource: TRect;
begin
  AActualSource := ASource;
  if FScaleFactor > 0 then
    AActualSource := cxRectScale(AActualSource, 1 / FScaleFactor);
  dxGpDrawImage(ACanvas.Handle, ADest, AActualSource, Handle, MaxByte,
    dxGpGetImageAttributesHandle(AAttributes));
end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        dxGPPaintCanvas.FlipWorldTransform(AHorizontally, AVertically,
          ABitmapSize.cx / 2, ABitmapSize.cy / 2);
  ...
  try
    ...
    begin
      ...
      if AInterpolationMode <> imDefault then
        GdipCheck(GdipSetInterpolationMode(AGraphics,
          Integer(AInterpolationMode)));
      GdipCheck(GdipDrawImageRectI(AGraphics, Handle, 0, 0, ASize.cx,
        ASize.cy));
...
begin
  ...
  begin
    ...
    if FScaleFactor > 0 then
      Result := cxSizeScale(Result, FScaleFactor);
  end
  else
    Result := cxNullSize;
end;
...
function TdxGPImageHandle.GetPropertyValue(APropID: Cardinal;
  var AData: TdxGPImagePropertyData): Boolean;
var
...
begin
  ASize := 0;
  SetLength(AData, 0);
  if (GdipGetPropertyItemSize(Handle, APropID, ASize) = Ok) and
    (ASize > SizeOf(TGpPropertyItem)) then
  begin
...
procedure TdxGPImageHandle.UpdateScaleFactor;
var
  AValue: Single;
begin
  FScaleFactor := 0;
  ...
  begin
    if (GdipGetImageVerticalResolution(Handle, AValue) = Ok) and (AValue > 0)
    then
      FScaleFactor := dxGetSystemDPI / AValue
    else
      FScaleFactor := 0;
  end;
end;
...
class function TdxGPImageCodec.CanSaveImage
  (AHandle: TdxSmartImageCustomHandle): Boolean;
begin
  Result := True;
end;

class function TdxGPImageCodec.Load(AStream: TStream;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
var
  AImage: GpImage;
...
class function TdxGPImageCodec.Save(AStream: TStream;
  AHandle: TdxSmartImageCustomHandle): Boolean;
var
...
begin
  ...
  begin
    ...
    begin
      ATempHandle := TdxGPImageHandle.CreateFromBits(AHandle.Width,
        AHandle.Height, AHandle.GetAsBitmapBits);
      try
...
class function TdxGPImageCodec.CheckHeader(AStream: TStream;
  const AHeader: AnsiString): Boolean;
var
...
begin
  ...
  try
    ...
    Result := (AStream.Read(ABuffer[0], ALength) = ALength) and
      CompareMem(@ABuffer[0], @AHeader[1], ALength);
  finally
...
class function TdxGPImageCodec.SaveCore(AStream: TStream;
  AHandle: TdxGPImageHandle; AParameters: Pointer = nil): Boolean;
var
  ACodec: TGUID;
  AStreamAdapter: IStream;
begin
  ACodec := GetCodecID;
  AStreamAdapter := TdxGPStreamAdapter.Create(AStream, soReference);
  Result := GdipSaveImageToStream(AHandle.Handle, AStreamAdapter, @ACodec,
    AParameters) = Ok;
  AStreamAdapter := nil;
end;
...
class function TdxGPImageCodecBMP.GetSize(AStream: TStream;
...
class function TdxGPImageCodecBMP.Load(const ABits: TRGBColors;
  AAlphaFormat: TAlphaFormat; AWidth, AHeight: Integer;
  out AHandle: TdxSmartImageCustomHandle): Boolean;
begin
  AHandle := TdxGPImageHandle.CreateFromBits(AWidth, AHeight, ABits,
    AAlphaFormat);
  Result := True;
end;
...
class function TdxGPImageCodecGIF.GetGraphicClassForRegistrationInVCL
  : TGraphicClass;
begin
  Result := TdxSmartImage;
end;

class function TdxGPImageCodecGIF.GetSize(AStream: TStream;
  out ASize: TSize): Boolean;
begin
  Result := CanLoadStream(AStream);
  if Result then
...
class function TdxGPImageCodecJPEG.GetGraphicClassForRegistrationInVCL
  : TGraphicClass;
begin
  Result := TdxSmartImage;
end;

class function TdxGPImageCodecJPEG.GetSize(AStream: TStream;
  out ASize: TSize): Boolean;
var
  ARecordID: Word;
begin
...
class function TdxGPImageCodecJPEG.SaveCore(AStream: TStream;
  AHandle: TdxGPImageHandle; AParameters: Pointer = nil): Boolean;
var
...
class function TdxGPImageCodecPNG.GetGraphicClassForRegistrationInVCL
  : TGraphicClass;
begin
  Result := TdxPNGImage;
end;

class function TdxGPImageCodecPNG.GetSize(AStream: TStream;
  out ASize: TSize): Boolean;
begin
  Result := CanLoadStream(AStream);
  if Result then
...
class function TdxGPImageCodecTIFF.GetGraphicClassForRegistrationInVCL
  : TGraphicClass;
begin
  Result := TdxSmartImage;
end;

class function TdxGPImageCodecTIFF.GetSize(AStream: TStream;
  out ASize: TSize): Boolean;
const
  HeightTag = $101;
  WidthTag = $100;
  ...
  begin
    ...
    begin
      ...
      case ATagInfo.Count * GetTokenSize(ATagInfo.FieldType) of
        2:
          ATagInfo.WordValue := GetWord(AIsBigEndian);
        4:
...
begin
  Result := CheckHeader(AStream, TIFFHeader1) or
    CheckHeader(AStream, TIFFHeader2);
end;
...
class function TdxGPImageCodecEMF.SaveCore(AStream: TStream;
  AHandle: TdxGPImageHandle; AParameters: Pointer): Boolean;
var
  ACanvas: TCanvas;
  AMetafile: TMetafile;
...
begin
  ...
  begin
    ...
    SetColorMatrix(@FColorMatrix, ColorMatrixFlagsDefault,
...
procedure TdxGPImageAttributes.ClearColorKey(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesColorKeys(FHandle, AType, False, 0, 0));
end;

procedure TdxGPImageAttributes.ClearColorMatrix(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesColorMatrix(FHandle, AType, False, nil, nil,
    ColorMatrixFlagsDefault));
end;

procedure TdxGPImageAttributes.ClearGamma(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesGamma(FHandle, AType, False, 0));
end;

procedure TdxGPImageAttributes.ClearNoOp(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
...
procedure TdxGPImageAttributes.ClearOutputChannel
  (const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesOutputChannel(FHandle, AType, False,
    ColorChannelFlagsLast));
end;

procedure TdxGPImageAttributes.ClearOutputChannelColorProfile
  (const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesOutputChannelColorProfile(FHandle, AType,
    False, nil));
end;

procedure TdxGPImageAttributes.ClearRemapTable(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesRemapTable(FHandle, AType, False, 0, nil));
end;

procedure TdxGPImageAttributes.ClearThreshold(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
...
procedure TdxGPImageAttributes.SetBrushRemapTable
  (const AMap: array of TGpColorMap);
begin
  SetRemapTable(AMap, ColorAdjustTypeBrush);
end;

procedure TdxGPImageAttributes.SetColorKey(const AColorLow,
  AColorHigh: TdxAlphaColor;
  const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesColorKeys(FHandle, AType, True, AColorLow,
    AColorHigh));
end;

procedure TdxGPImageAttributes.SetColorMatrices(const ANewColorMatrix,
  AGrayMatrix: TdxGpColorMatrix;
  const AMode: TdxGPColorMatrixFlags = ColorMatrixFlagsDefault;
  const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesColorMatrix(FHandle, AType, True,
    @ANewColorMatrix, @AGrayMatrix, AMode));
end;
...
procedure TdxGPImageAttributes.SetNoOp(const AType
  : TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesNoOp(FHandle, AType, True));
end;

procedure TdxGPImageAttributes.SetOutputChannel(const AFlags
...
procedure TdxGPImageAttributes.SetOutputChannelColorProfile
  (const AColorProfileFilename: string;
  const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesOutputChannelColorProfile(FHandle, AType,
    True, PChar(AColorProfileFilename)));
end;

procedure TdxGPImageAttributes.SetRemapTable(const AMap: array of TGpColorMap;
  const AType: TdxGpColorAdjustType = ColorAdjustTypeDefault);
begin
  GdipCheck(GdipSetImageAttributesRemapTable(FHandle, AType, True, Length(AMap),
    @AMap[0]));
end;
...
function TdxGPImage.MakeComposition(AOverlayImage: TdxGPImage;
  AOverlayAlpha: Byte): TdxGPImage;
begin
  Result := MakeComposition(AOverlayImage, AOverlayAlpha, 255);
end;

function TdxGPImage.MakeComposition(AOverlayImage: TdxGPImage;
  AOverlayAlpha, ASourceAlpha: Byte): TdxGPImage;
var
  ACanvas: TdxGPCanvas;
begin
  ...
  begin
    ...
    try
      ...
      ACanvas.Draw(AOverlayImage, Result.ClientRect, AOverlayImage.ClientRect,
        AOverlayAlpha);
    finally
      ACanvas.Free;
    end;
...
procedure TdxGPImage.StretchDraw(ACanvas: TdxGPCanvas;
  const ADest, ASource: TdxRectF; AAttributes: TdxGPImageAttributes;
...
begin
  if Supports(HandleAsObject, IdxGPGraphicDraw, AIntf) then
    ...
    StretchDraw(ACanvas, cxRect(ADest), cxRect(ASource), AAttributes,
      AColorPalette);
end;
...
begin
  ...
  GdipCheck(GdipFillRectangleI(AGraphics,
    TdxGPResourceManager.GetSolidBrush(TdxAlphaColors.Empty).GetNativeHandle, 0,
    0, Width, Height));
  GdipCheck(GdipDeleteGraphics(AGraphics));
end;
...
begin
  ...
  GdipCheck(GdipCreateBitmapFromScan0(Width, Height, Width * 4,
    PixelFormat32bppPARGB, PByte(FBits), FHandle));
end;
...
begin
  if FHandle <> nil then
  begin
    GdipCheck(GdipDisposeImage(FHandle));
    FreeMem(FBits);
    FBits := nil;
  end;
end;
...
constructor TdxGPSolidPenCacheID.Create(AColor: TdxAlphaColor; AWidth: Single;
  AStyle: DashStyle);
begin
  Color := AColor;
  Width := AWidth;
...
function TdxGPSolidPenCacheManager.CreateValue(const ID: TdxGPSolidPenCacheID)
  : IdxGPHandle;
var
  AHandle: GpPen;
begin
...
function TdxGPSolidBrushCacheManager.CreateValue(const ID: TdxAlphaColor)
  : IdxGPHandle;
var
  ABrush: GpBrush;
begin
...
begin
  Result := GetPen(dxColorToAlphaColor(AColor, AColorAlpha), AWidth,
    AStyle, AHandle);
end;
...
const
  PenStyleToDashStyle: array [psSolid .. psDashDotDot] of GpDashStyle =
    ...
    DashStyleDashDotDot);
begin
  Result := (AStyle >= psSolid) and (AStyle <= psDashDotDot) and
    GetPen(AColor, AWidth, PenStyleToDashStyle[AStyle], AHandle);
end;
...
class function TdxGPResourceManager.GetPen(AColor: TdxAlphaColor;
  AWidth: Single; AStyle: DashStyle; out AHandle: IdxGPHandle): Boolean;
begin
  Result := dxAlphaColorIsValid(AColor) and (AWidth > 0);
  if Result then
    AHandle := FSolidPens.Get(TdxGPSolidPenCacheID.Create(AColor,
      AWidth, AStyle));
end;

class function TdxGPResourceManager.GetSolidBrush(const AColor: TdxAlphaColor;
  out AHandle: IdxGPHandle): Boolean;
begin
...
class function TdxGPResourceManager.GetSolidBrush(const AColor: TColor;
  AColorAlpha: Byte; out AHandle: IdxGPHandle): Boolean;
begin
