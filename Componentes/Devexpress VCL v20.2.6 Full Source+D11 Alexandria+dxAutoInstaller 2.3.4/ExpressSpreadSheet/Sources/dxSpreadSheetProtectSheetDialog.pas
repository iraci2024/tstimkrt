...
{$ENDIF}
  ...
  Dialogs, Menus, StdCtrls, TypInfo,
  Generics.Defaults, Generics.Collections,
  dxCore, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
  cxClasses, cxContainer, cxEdit,
  cxCheckListBox, cxCheckBox, cxButtons, cxTextEdit, dxLayoutControlAdapters,
  dxLayoutcxEditAdapters,
  dxLayoutContainer, dxLayoutControl, dxLayoutLookAndFeels, dxSpreadSheetCore,
  dxSpreadSheetProtection, dxForms;

type

  { TdxSpreadSheetProtectSheetDialogForm }

  TdxSpreadSheetProtectSheetDialogFormClass = class of
    TdxSpreadSheetProtectSheetDialogForm;
  ...
  TdxSpreadSheetProtectSheetDialogForm = class(TdxForm)
    ...
    procedure clbPermissionsClickCheck(Sender: TObject; AIndex: Integer;
...
var
  dxSpreadSheetProtectSheetDialogClass
...
begin
  ADialog := dxSpreadSheetProtectSheetDialogClass.Create
...
begin
  ...
  liPermissions.Caption := cxGetResourceString
  ...
  try
    for I := 0 to clbPermissions.Items.Count - 1 do
      clbPermissions.Items[I].Text :=
        cxGetResourceString(FPermissions[I].Value);
  finally
    clbPermissions.Items.EndUpdate;
  end;
end;

procedure TdxSpreadSheetProtectSheetDialogForm.Initialize
...
begin
  FPermissions.Add(TPair<string, Pointer>.Create('AllowSelectLockedCells',
    @sdxProtectSheetDialogAllowSelectLockedCells));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowSelectUnlockedCells',
    @sdxProtectSheetDialogAllowSelectUnlockedCells));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowFormatCells',
    @sdxProtectSheetDialogAllowFormatCells));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowResizeColumns',
    @sdxProtectSheetDialogAllowResizeColumns));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowResizeRows',
    @sdxProtectSheetDialogAllowResizeRows));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowEditContainers',
    @sdxProtectSheetDialogAllowEditContainers));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowEditHyperlinks',
    @sdxProtectSheetDialogAllowEditHyperlinks));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowInsertColumns',
    @sdxProtectSheetDialogAllowInsertColumns));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowInsertRows',
    @sdxProtectSheetDialogAllowInsertRows));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowDeleteColumns',
    @sdxProtectSheetDialogAllowDeleteColumns));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowDeleteRows',
    @sdxProtectSheetDialogAllowDeleteRows));
  FPermissions.Add(TPair<string, Pointer>.Create('AllowSort',
    @sdxProtectSheetDialogAllowSort));
end;

procedure TdxSpreadSheetProtectSheetDialogForm.Load
  (AOptions: TdxSpreadSheetSheetProtectionOptions);
begin
...
begin
  ...
  try
    for I := 0 to clbPermissions.Items.Count - 1 do
      clbPermissions.Items[I].Checked :=
        GetOrdProp(FOptions, FPermissions[I].Key) <> 0;
  finally
    clbPermissions.Items.EndUpdate;
  end;
end;

procedure TdxSpreadSheetProtectSheetDialogForm.Save
  (AOptions: TdxSpreadSheetSheetProtectionOptions);
begin
  AOptions.Assign(FOptions);
  AOptions.Protected := cbProtected.Checked;
  if edPassword.Text <> '' then
    AOptions.ProtectionInfo := dxSpreadSheetDefaultProtectionProvider.Create
      (edPassword.Text)
  else
...
begin
  if (edPassword.Text = '') or TdxSpreadSheetPasswordDialogForm.
    ExecuteConfirmation(Self, Sheet.SpreadSheet, edPassword.Text) then
    ModalResult := mrOk;
end;

procedure TdxSpreadSheetProtectSheetDialogForm.cbProtectedClick
  (Sender: TObject);
begin
