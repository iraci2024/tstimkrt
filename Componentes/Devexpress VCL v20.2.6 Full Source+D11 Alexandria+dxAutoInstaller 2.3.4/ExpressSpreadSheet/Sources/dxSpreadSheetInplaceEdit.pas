...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Messages, Classes, Controls, dxCore, cxControls, cxGraphics,
  cxRichEdit, cxLookAndFeels,
  dxAutoCompleteWindow, cxImageList, cxListBox, cxLookAndFeelPainters,
  dxCoreGraphics, dxCustomHint, cxHint,
  cxDrawTextUtils;
...
type
  ...
  protected
    ...
    function GetController: IdxSpreadSheetInplaceEditController;
      virtual; abstract;
    ...
    procedure PopulateAutoCompleteSuggestions(const ASearchText: string;
      ASuggestions: TStrings); virtual;
    procedure RefreshAutoCompleteSuggestions(const ASearchText
      : string); overload;
    procedure RefreshAutoCompleteSuggestions; overload;
    procedure RefreshAutoCompleteWindow(const ASearchText: string;
      ASuggestions: TStrings); virtual;
    //
    property AutoCompleteWindow: TdxCustomAutoCompleteWindow
  ...
  TdxSpreadSheetInplaceEditAutoCompleteInnerListBox = class
  ...
  TdxSpreadSheetInplaceEditAutoCompleteWindow = class
...
uses
  StrUtils, SysUtils, Math, Graphics, cxGeometry, cxFormats, dxGDIPlusClasses,
  cxRichEditUtils,
  dxSpreadSheetCore,
...
function GetActualMasterLookAndFeel(AInplaceEdit
  : TdxSpreadSheetCustomInplaceEdit): TcxLookAndFeel;
var
...
begin
  Result := HasInnerEdit and TcxRichInnerEditAccess(InnerRich)
    .FInImeComposition;
end;
...
begin
  Result := not IsFormula and dxSpreadSheetTextService.IsRTFSupported and
    not Properties.PlainText;
end;
...
procedure TdxSpreadSheetCustomInplaceEdit.KeyDown(var Key: Word;
  Shift: TShiftState);
const
...
begin
  ...
  begin
    ...
    if dxCharInSet(Char(Key), ['1' .. '5', 'E', 'J', 'L', 'R']) or
      (ssShift in Shift) and (Char(Key) = 'A') then
      Key := 0;
  end;
...
procedure TdxSpreadSheetCustomInplaceEdit.KeyUp(var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_TAB then
    Key := 0;
...
function TdxSpreadSheetCustomInplaceEdit.ProcessReturnKey
...
begin
  ...
  begin
    ...
    begin
      if (AShiftStates = [ssCtrl]) or (AShiftStates = ArrayFormulaShiftStates)
      then
...
function TdxSpreadSheetCustomInplaceEdit.ProcessShortCutChar
...
function TdxSpreadSheetCustomInplaceEdit.ProcessTabKey
  (Shift: TShiftState): Boolean;
begin
...
function TdxSpreadSheetCustomInplaceEdit.CreateAutoCompleteWindow
  : TdxCustomAutoCompleteWindow;
begin
...
function TdxSpreadSheetCustomInplaceEdit.
  GetAutoCompleteWindowOwnerBounds: TRect;
var
  ACharPosition: Integer;
begin
  Result := ClientBounds;
  ACharPosition := Result.Left + cxRichEditGetCharPosition(InnerRich.Handle,
    TcxRichInnerEditAccess(InnerRich).RichVersion, SelStart).X;
...
begin
  ...
  begin
    ...
    while (ASelStart > 0) and not dxIsOperandSeparator(AText[ASelStart]) do
      Dec(ASelStart);
    RefreshAutoCompleteSuggestions(Copy(AText, ASelStart + 1,
      SelStart - ASelStart));
  end
  else
    HideAutoCompleteWindow;
...
procedure TdxSpreadSheetCustomInplaceEdit.PopulateAutoCompleteSuggestions
  (const ASearchText: string; ASuggestions: TStrings);
...
procedure TdxSpreadSheetCustomInplaceEdit.RefreshAutoCompleteWindow
  (const ASearchText: string; ASuggestions: TStrings);
begin
  ...
  begin
    ...
    begin
      ...
      FAutoCompleteWindow.ShowHint :=
        Controller.IsAutoCompleteSuggestionsHintsAllowed;
      UpdateKeys;
    end;
...
procedure TdxSpreadSheetCustomInplaceEdit.HandlerAutoCompleteWindowSelectItem
  (Sender: TObject);
var
...
constructor TdxSpreadSheetInplaceEditAutoCompleteWindow.Create
...
begin
  ...
  try
    for I := 0 to InnerListBox.Count - 1 do
      InnerListBox.Items[I].ImageIndex :=
        GetObjectImageIndex(InnerListBox.Items[I].Data);
  finally
    InnerListBox.Items.EndUpdate;
  end;
end;

procedure TdxSpreadSheetInplaceEditAutoCompleteWindow.Popup(AFocusedControl
  : TWinControl);
begin
  inherited;
  UpdateHintWindow;
...
function TdxSpreadSheetInplaceEditAutoCompleteWindow.CreateHintWindow
  : THintWindow;
begin
  Result := TdxSpreadSheetInplaceEditAutoCompleteHintWindow.Create(Self);
end;

function TdxSpreadSheetInplaceEditAutoCompleteWindow.CreateInnerListBox
  : TdxCustomAutoCompleteInnerListBox;
begin
  Result := TdxSpreadSheetInplaceEditAutoCompleteInnerListBox.Create(Self);
  Result.Images := TdxSpreadSheetInplaceEditAutoCompleteWindowImages.
    GetImageList;
...
function TdxSpreadSheetInplaceEditAutoCompleteWindow.GetObjectHint
  (AObject: TObject): string;
begin
  if AObject is TdxSpreadSheetFunctionInfo then
    Result := cxGetResourceString(TdxSpreadSheetFunctionInfo(AObject)
      .DescriptionPtr)
...
function TdxSpreadSheetInplaceEditAutoCompleteWindow.GetObjectImageIndex
  (AObject: TObject): TcxImageIndex;
begin
  Result := TdxSpreadSheetInplaceEditAutoCompleteWindowImages.
    GetImageIndex(AObject);
end;
...
begin
  ...
  begin
    ...
    ARect.Top := InnerListBox.ClientToScreen
      (InnerListBox.ItemRect(InnerListBox.ItemIndex).TopLeft).Y;
...
procedure TdxSpreadSheetInplaceEditAutoCompleteWindow.HandlerItemIndexChanged
  (Sender: TObject);
begin
  UpdateHintWindow;
end;
...
procedure TdxSpreadSheetCustomInplaceInnerEdit.CMChildKey
...
procedure TdxSpreadSheetCustomInplaceInnerEdit.CMColorChanged
...
procedure TdxSpreadSheetCustomInplaceInnerEdit.KeyDown(var Key: Word;
Shift: TShiftState);
begin
...
procedure TdxSpreadSheetCustomInplaceInnerEdit.KeyUp(var Key: Word;
Shift: TShiftState);
begin
...
begin
  ...
  begin
    ...
    cxDrawImage(Canvas.Handle, AImageRect, AImageRect, nil, Images,
      AItem.ImageIndex, idmNormal, False, 0, clNone, True, GetColorPalette);
  end;
end;

function TdxSpreadSheetInplaceEditAutoCompleteInnerListBox.
...
function TdxSpreadSheetInplaceEditAutoCompleteInnerListBox.GetColorPalette
  : IdxColorPalette;
begin
  if FColorPalette = nil then
  begin
    FColorPalette := TdxSimpleColorPalette.Create
      (dxMakeAlphaColor(LookAndFeelPainter.DefaultControlTextColor),
...
class function TdxSpreadSheetInplaceEditAutoCompleteWindowImages.GetImageIndex
  (AObject: TObject): Integer;
begin
  Result := Ord(AObject is TdxSpreadSheetDefinedName);
end;

class function TdxSpreadSheetInplaceEditAutoCompleteWindowImages.GetImageList
  : TcxCustomImageList;
begin
  if FImages = nil then
  begin
...
class procedure TdxSpreadSheetInplaceEditAutoCompleteWindowImages.ImportImage
...
begin
  ...
  cxTextOut(cxScreenCanvas.Handle, AHint, Result, CXTO_CALCRECT or
    GetDrawTextFlags);
...
begin
  ...
  try
    ...
    cxTextOut(cxPaintCanvas.Handle, Text, ATextRect, GetDrawTextFlags or
...
function TdxSpreadSheetInplaceEditAutoCompleteHintWindow.
...
function TdxSpreadSheetInplaceEditAutoCompleteHintWindow.GetOwnerEx
  : TdxSpreadSheetInplaceEditAutoCompleteWindow;
begin
  Result := inherited Owner as TdxSpreadSheetInplaceEditAutoCompleteWindow;
end;

function TdxSpreadSheetInplaceEditAutoCompleteHintWindow.GetPainter
