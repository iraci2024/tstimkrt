...
{$ENDIF}
  ...
  dxLayoutControl, dxLayoutControlAdapters, cxButtons, dxForms,
  dxSpreadSheetCore, dxLayoutcxEditAdapters, cxContainer,
  cxEdit, cxTextEdit, cxEditRepositoryItems, dxSpreadSheetPrinting, cxLabel,
  cxCheckBox, cxMaskEdit, cxDropDownEdit,
  ExtCtrls, cxRadioGroup, ImgList, cxImageList, cxSpinEdit, cxGroupBox,
  dxCustomPreview, dxMeasurementUnits, cxGeometry,
...
type
  ...
  TdxSpreadSheetHeaderFooterMacroPreview = class
  ...
  protected
    ...
    class function EvalFuncTime(const S: string; var AIndex: Integer;
      ALength: Integer): string; override;
  public
    class function Evaluate(const S: string;
      ASheet: TdxSpreadSheetTableView): string;
  end;

  { TdxSpreadSheetPageSetupDialogForm }

  TdxSpreadSheetPageSetupDialogFormClass = class of
    TdxSpreadSheetPageSetupDialogForm;
  ...
  protected
    ...
    procedure PopulateHeaderFooterTemplates(AStrings: TStrings);
      overload; virtual;
    ...
    procedure SetHeaderFooterText(const ATemplate: string;
      AText: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText); overload;
    procedure SetHeaderFooterText(const L, C, R: string;
    ...
    procedure UpdateTemplateSelection(ATemplate: TcxComboBox;
      AText: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText);

    procedure DrawHeaderFooterPreview(ACanvas: TCanvas; ARect: TRect;
      AText: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText;
      AIsHeader: Boolean); virtual;
    procedure DrawPageContent(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
    ...
    function AreaToString(const AArea: TdxSpreadSheetTableViewOptionsPrintRect)
      : string; overload;
    function TryStringToReference(const S: string;
      out AView: TdxSpreadSheetTableView; out AArea: TRect): Boolean;
      overload; override;
    function TryStringToReference(const S: string;
      AArea: TdxSpreadSheetTableViewOptionsPrintRect): Boolean;
    ...
    property DefaultPageMargins: TdxSpreadSheetTableViewOptionsPrintPageMargins
      read FDefaultPageMargins;
  public
...
var
  dxSpreadSheetPageSetupDialogClass: TdxSpreadSheetPageSetupDialogFormClass =
    TdxSpreadSheetPageSetupDialogForm;

procedure HidePageSetupDialog;
procedure ShowPageSetupDialog(ASheet: TdxSpreadSheetTableView;
  const AActivePage: Integer = -1);

implementation

uses
  dxSpreadSheetDialogStrs, dxCore, dxSpreadSheetUtils,
  dxSpreadSheetCoreReferences, StrUtils, Math,
  dxPrintUtils, dxTypeHelpers, cxFormats, dxPrinting,
  dxSpreadSheetPageSetupHeaderFooterDialog,
...
const
  BuildInTemplates: array [0 .. 8] of string = ('%0:sPage &P%0:s',
    '%0:sPage &P of &N%0:s', '%0:s&A%0:s', 'Confidential%0:s&D%0:sPage &P',
    '%0:s&A%0:sPage &P', '&A%0:sConfidential%0:sPage &P', '%0:sPage &P%0:s&A',
    '%1:s%0:sPage &P%0:s&D', '%0:sPrepared by %1:s &D%0:sPage &P');
...
begin
  if FDialog = nil then
  begin
    FDialog := dxSpreadSheetPageSetupDialogClass.Create
      (GetParentForm(ASheet.SpreadSheet));
...
class function TdxSpreadSheetHeaderFooterMacroPreview.Evaluate(const S: string;
  ASheet: TdxSpreadSheetTableView): string;
begin
  FSheet := ASheet;
  try
...
begin
  ...
  begin
    SpreadSheet.History.BeginAction
...
begin
  ...
  btnPrintPreview.Caption := cxGetResourceString
    (@sdxPageSetupDialogButtonPrintPreview);

  // Page
  lbOrientation.Caption := cxGetResourceString
  ...
  liAdjustTo.Caption := cxGetResourceString
    (@sdxPageSetupDialogScalingAdjustToSuffix);
  liFirstPageNumber.Caption := cxGetResourceString
    (@sdxPageSetupDialogPageFirstPageNumber);
  liPagesTall.Caption := cxGetResourceString
    (@sdxPageSetupDialogScalingFitToPageTall);
  liPagesWide.Caption := cxGetResourceString
  ...
  rbPageOrientationPortrait.Caption :=
    cxGetResourceString(@sdxPageSetupDialogPageOrientationPortrait);
  rbPageOrientationLandscape.Caption :=
    cxGetResourceString(@sdxPageSetupDialogPageOrientationLandscape);

  // Margins
  lgMargins.Caption := cxGetResourceString(@sdxPageSetupDialogMargins);
  liMarginBottom.Caption := cxGetResourceString
    (@sdxPageSetupDialogMarginBottom);
  liMarginFooter.Caption := cxGetResourceString
    (@sdxPageSetupDialogMarginFooter);
  liMarginHeader.Caption := cxGetResourceString
  ...
  lbCenterOnPage.Caption := cxGetResourceString
    (@sdxPageSetupDialogCenterOnPage);
  cbCenterHorizontally.Caption := cxGetResourceString
    (@sdxPageSetupDialogCenterHorizontally);
  cbCenterVertically.Caption := cxGetResourceString
    (@sdxPageSetupDialogCenterVertically);

  // Header/Footer
  lgHeaderFooter.Caption := cxGetResourceString
    (@sdxPageSetupDialogHeaderFooter);
  btnCustomHeaderFooter.Caption :=
    cxGetResourceString(@sdxPageSetupDialogButtonCustomHeaderFooter);
  cbAlignWithPageMargins.Caption :=
    cxGetResourceString(@sdxPageSetupDialogAlignWithMargins);
  cbScaleWithDocument.Caption := cxGetResourceString
  ...
  // Sheet
  cbBlackAndWhite.Caption := cxGetResourceString
    (@sdxPageSetupDialogBlackAndWhite);
  cbDraftQuality.Caption := cxGetResourceString
    (@sdxPageSetupDialogPrintDraftQuality);
  cbPrintGridLines.Caption := cxGetResourceString
    (@sdxPageSetupDialogPrintGridLines);
  cbPrintHeaders.Caption := cxGetResourceString
  ...
  liPrintCellErrorsMode.Caption :=
    cxGetResourceString(@sdxPageSetupDialogPrintCellErrorsMode);
  liPrintCommentsMode.Caption := cxGetResourceString
    (@sdxPageSetupDialogPrintCommentsMode);
  liRowsToRepeat.Caption := cxGetResourceString
    (@sdxPageSetupDialogPrintTitlesRowsToRepeat);
  liColumnsToRepeat.Caption := cxGetResourceString
    (@sdxPageSetupDialogPrintTitlesColumnsToRepeat);
  lbPageOrder.Caption := cxGetResourceString(@sdxPageSetupDialogPageOrder);
  rbtnDownThenOver.Caption := cxGetResourceString
    (@sdxPageSetupDialogDownThenOver);
  rbtnOverThenDown.Caption := cxGetResourceString
...
begin
  ...
  ersiMargins.Properties.DisplayFormat :=
    Format('0.0 %s', [cxGetResourceString(AUnitsName)]);
end;
...
var
  ...
  APageSize: TPoint;
begin
  TdxPrintingDefaults.GetDefaultPageInfo(AMargins, AHeader, AFooter, APageSize,
    FDefaultPageSize, AOrientation, muInches);
...
begin
  for AIndex := Low(AIndex) to High(AIndex) do
    cbPrintCommentsMode.Properties.Items.AddObject
      (cxGetResourceString(dxPrintCommentsModesNames[AIndex]), TObject(AIndex));
end;

procedure TdxSpreadSheetPageSetupDialogForm.SelectionChanged
...
procedure TdxSpreadSheetPageSetupDialogForm.SetHeaderFooterText(const ATemplate
  : string; AText: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText);
var
  AParts: TStringDynArray;
begin
  AParts := SplitString(ATemplate, SectionsDelimiter);
  if Length(AParts) >= 3 then
    SetHeaderFooterText(AParts[IfThen(IsRightToLeft, 2, 0)], AParts[1],
      AParts[IfThen(IsRightToLeft, 0, 2)], AText)
  else
    SetHeaderFooterText('', '', '', AText);
end;

procedure TdxSpreadSheetPageSetupDialogForm.SetHeaderFooterText(const L, C,
  R: string; AText: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText);
begin
...
const
  Map: array [Boolean] of TdxPreviewPaperOrientation = (ppoLandscape,
    ppoPortrait);
begin
  if FLoading then
    Exit;

  TdxCustomPreviewAccess(FPagePreview).PageSize.Orientation :=
    Map[rbPageOrientationPortrait.Checked];
  TdxCustomPreviewAccess(FPagePreview).PageSize.Size :=
    TdxPrintingDefaults.Papers[cbPaperSize.ItemIndex].Size;
  TdxCustomPreviewAccess(FPagePreview).PageSize.Header :=
    ConvertValue(seMarginHeader.Value);
  TdxCustomPreviewAccess(FPagePreview).PageSize.Footer :=
    ConvertValue(seMarginFooter.Value);
  TdxCustomPreviewAccess(FPagePreview).PageSize.Margins :=
    Rect(ConvertValue(seMarginLeft.Value), ConvertValue(seMarginTop.Value),
...
procedure TdxSpreadSheetPageSetupDialogForm.DrawHeaderFooterPreview
  ...
  begin
    Result := cxRectContent(R,
      DialogsLookAndFeel.Painter.PrintPreviewPageBordersScaledWidth
      (ScaleFactor));
  end;

  function GetSectionWidth(const S: string): Integer;
  begin
    Result := cxTextWidth(ACanvas.Font,
      TdxSpreadSheetHeaderFooterMacroPreview.Evaluate(S, ActiveSheet));
  end;
  ...
  begin
    ...
    DialogsLookAndFeel.Painter.DrawPrintPreviewPageScaledBackground(ACanvas, R,
      GetContentRect(R), False, True, ScaleFactor);
  end;

  procedure DrawSection(ACanvas: TcxCanvas; const R: TRect; const S: string;
    AAlignment: TAlignment);
  begin
    ACanvas.Font.Color := DialogsLookAndFeel.Painter.
      PrintPreviewBackgroundTextColor;
    ACanvas.DrawTexT(TdxSpreadSheetHeaderFooterMacroPreview.Evaluate(S,
      ActiveSheet), cxRectInflate(R, -ScaleFactor.Apply(cxTextOffset)),
      AAlignment, vaCenter, False, True);
  end;
...
begin
  ...
  try
    ...
    try
      ...
      ACalculator.AddItem.Width := Max(GetSectionWidth(AText.LeftSection),
        GetSectionWidth(AText.RightSection));
      ...
      DrawSection(cxPaintCanvas, ARect, IfThen(IsRightToLeft,
        AText.RightSection, AText.LeftSection), taLeftJustify);
      ARect := cxRectSetWidth(ARect, ARect.Right,
        ACalculator.Items[1].AutoWidth);
      DrawSection(cxPaintCanvas, ARect, AText.CenterSection, taCenter);
      ARect := cxRectSetWidth(ARect, ARect.Right,
        ACalculator.Items[2].AutoWidth);
      DrawSection(cxPaintCanvas, ARect, IfThen(IsRightToLeft, AText.LeftSection,
        AText.RightSection), taRightJustify);
    finally
      ACalculator.Free;
    end;
  finally
...
procedure TdxSpreadSheetPageSetupDialogForm.DrawPageContent(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; APageIndex: Integer);
begin
  dxDrawDefaultPagePreview(ACanvas, TdxCustomPreviewAccess(FPagePreview)
    .PageGetContentBounds(ARect), cbCenterHorizontally.Checked,
    cbCenterVertically.Checked, ScaleFactor);
end;

procedure TdxSpreadSheetPageSetupDialogForm.UpdateTemplateSelection
  (ATemplate: TcxComboBox;
  AText: TdxSpreadSheetTableViewOptionsPrintHeaderFooterText);
  ...
  begin
    if (L <> '') or (C <> '') or (R <> '') then
    begin
      Result := EscapeString(L) + SectionsDelimiter + EscapeString(C) +
        SectionsDelimiter + EscapeString(R);
...
begin
  ATemplateString := BuildTemplate(AText.LeftSection, AText.CenterSection,
    AText.RightSection);
...
begin
  for AIndex := pseiBlank to High(AIndex) do
    cbPrintCellErrorsMode.Properties.Items.AddObject
      (cxGetResourceString(dxPrintCellErrorsModesNames[AIndex]),
      TObject(AIndex));
end;
...
begin
  ...
  rbPageOrientationPortrait.Checked := Options.Page.Orientation <>
    oppoLandscape;
  rbPageOrientationLandscape.Checked :=
    Options.Page.Orientation = oppoLandscape;
  meFirstPageNumber.Text := IfThen(Options.Page.FirstPageNumber > 0,
    IntToStr(Options.Page.FirstPageNumber));
  cbPaperSize.ItemIndex := TdxPrintingDefaults.Papers.FindByDMPaper
    (GetActualDMPaper);
end;
...
begin
  ...
  cbPrintCellErrorsMode.ItemIndex := cbPrintCellErrorsMode.Properties.Items.
    IndexOfObject(TObject(Options.Source.ActualErrorIndication));
  cbPrintCommentsMode.ItemIndex := cbPrintCommentsMode.Properties.Items.
    IndexOfObject(TObject(Options.Source.CellComments));
  cbBlackAndWhite.Checked := Options.Printing.ActualBlackAndWhite;
  cbDraftQuality.Checked := Options.Printing.ActualDraft;
  rbtnDownThenOver.Checked := Options.Printing.ActualPageOrder =
    opppDownThenOver;
  rbtnOverThenDown.Checked := Options.Printing.ActualPageOrder =
    opppOverThenDown;
end;
...
function TdxSpreadSheetPageSetupDialogForm.AreaToString
  (const AArea: TdxSpreadSheetTableViewOptionsPrintRect): string;
begin
...
function TdxSpreadSheetPageSetupDialogForm.TryStringToReference(const S: string;
  out AView: TdxSpreadSheetTableView; out AArea: TRect): Boolean;
begin
...
function TdxSpreadSheetPageSetupDialogForm.ValidateReference
...
function TdxSpreadSheetPageSetupDialogForm.FlipTemplate
...
begin
  ...
  if Length(AParts) = 3 then
    Result := AParts[2] + SectionsDelimiter + AParts[1] + SectionsDelimiter
      + AParts[0]
  else
    Result := S;
end;

procedure TdxSpreadSheetPageSetupDialogForm.FlipTemplates
...
begin
  ...
  try
    for I := 0 to AComboBox.Properties.Items.Count - 1 do
      AComboBox.Properties.Items[I] :=
        FlipTemplate(AComboBox.Properties.Items[I]);
  finally
...
procedure TdxSpreadSheetPageSetupDialogForm.CMBiDiModeChanged
...
function TdxSpreadSheetPageSetupDialogForm.GetOptions
...
procedure TdxSpreadSheetPageSetupDialogForm.btnCustomHeaderFooterClick
  (Sender: TObject);
var
...
procedure TdxSpreadSheetPageSetupDialogForm.btnPrintPreviewClick
  (Sender: TObject);
begin
  ApplyChanges;
...
procedure TdxSpreadSheetPageSetupDialogForm.FormClose(Sender: TObject;
...
procedure TdxSpreadSheetPageSetupDialogForm.PageSettingsChanged
  (Sender: TObject);
begin
  UpdatePagePreview;
end;

procedure TdxSpreadSheetPageSetupDialogForm.cbFooterTemplatesPropertiesChange
  (Sender: TObject);
begin
  SetHeaderFooterText(cbFooterTemplates.Text, FPageFooter);
end;

procedure TdxSpreadSheetPageSetupDialogForm.cbHeaderTemplatesPropertiesChange
  (Sender: TObject);
begin
  SetHeaderFooterText(cbHeaderTemplates.Text, FPageHeader);
end;

procedure TdxSpreadSheetPageSetupDialogForm.pbFooterPreviewPaint
  (Sender: TObject);
begin
  DrawHeaderFooterPreview(pbFooterPreview.Canvas, pbFooterPreview.ClientRect,
    FPageFooter, False);
end;

procedure TdxSpreadSheetPageSetupDialogForm.pbHeaderPreviewPaint
  (Sender: TObject);
begin
  DrawHeaderFooterPreview(pbHeaderPreview.Canvas, pbHeaderPreview.ClientRect,
    FPageHeader, True);
end;
...
begin
  ...
  try
    cxDrawImage(cxPaintCanvas, TPaintBox(Sender).ClientRect, nil, ilPrintOrders,
      Ord(rbtnOverThenDown.Checked), True, nil, ScaleFactor);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;

procedure TdxSpreadSheetPageSetupDialogForm.pbxPageOrientationPaint
  (Sender: TObject);
begin
  cxPaintCanvas.BeginPaint(TPaintBox(Sender).Canvas);
  try
    cxDrawImage(cxPaintCanvas, TPaintBox(Sender).ClientRect, nil, ilLarge,
      Ord(rbPageOrientationLandscape.Checked), True, nil, ScaleFactor);
  finally
    cxPaintCanvas.EndPaint;
  end;
end;
...
procedure TdxSpreadSheetPageSetupDialogForm.rbPageOrientationPortraitClick
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetPageSetupDialogForm.rbtnOverThenDownClick
  (Sender: TObject);
begin
  pbxPageOrder.Invalidate;
end;

procedure TdxSpreadSheetPageSetupDialogForm.seAdjustToPropertiesChange
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetPageSetupDialogForm.sePagesWidePropertiesChange
  (Sender: TObject);
begin
  if TcxSpinEdit(Sender).Focused then
    rbFitTo.Checked := True;
...
begin
  lgPrintButtons.Parent := lgTabs.Items[lgTabs.ItemIndex]
