...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, SysUtils, Classes, dxSpreadSheetCore, dxSpreadSheetTypes,
  dxSpreadSheetClasses, dxCore;
...
type
  ...
  public
    ...
    class function CreateReader(ASpreadSheet: TdxCustomSpreadSheet;
      AStream: TStream): TdxSpreadSheetCustomReader; override;
  ...
  private const
    PercentsResolution = 1000;

  const
    PositiveFixedAngleResolution = 60000;
  private
  ...
  public
    ...
    class function GetRelsFileNameForFile(const AFileName: AnsiString)
      : AnsiString;
  end;
...
uses
  ...
  dxSpreadSheetFormatXLSXReader, dxSpreadSheetFormatXLSXWriter,
  dxSpreadSheetUtils, dxSpreadSheetFormatUtils,
  dxDPIAwareUtils;
...
class function TdxSpreadSheetXLSXFormat.CanReadFromStream
  ...
  try
    ...
    if not Result then
      try
        ...
        try
          AReader.IgnoreMessages :=
            [Low(TdxSpreadSheetMessageType) .. High(TdxSpreadSheetMessageType)];
...
class function TdxSpreadSheetXLSXFormat.CreateFormatSettings
  : TdxSpreadSheetFormatSettings;
begin
  Result := TdxSpreadSheetFormatSettings.Create;
end;

class function TdxSpreadSheetXLSXFormat.CreateReader(ASpreadSheet
  : TdxCustomSpreadSheet; AStream: TStream): TdxSpreadSheetCustomReader;
var
  APassword: string;
begin
...
class function TdxSpreadSheetXLSXFormat.GetReader
  : TdxSpreadSheetCustomReaderClass;
begin
  Result := TdxSpreadSheetXLSXReader;
end;

class function TdxSpreadSheetXLSXFormat.GetWriter
...
class function TdxSpreadSheetXLTXFormat.GetWriter
  : TdxSpreadSheetCustomWriterClass;
begin
...
begin
  Result := cxRect(DecodeValue(R.Left), DecodeValue(R.Top),
    DecodeValue(R.Right), DecodeValue(R.Bottom));
end;
...
begin
  Result := cxRect(EncodeValue(R.Left), EncodeValue(R.Top),
    EncodeValue(R.Right), EncodeValue(R.Bottom));
end;
...
class function TdxSpreadSheetXLSXUtils.GetRelsFileNameForFile(const AFileName
  : AnsiString): AnsiString;
begin
  Result := TdxZIPPathHelper.DecodePath(AFileName);
  Result := ExtractFilePath(Result) + '_rels' + PathDelim +
    ExtractFileName(Result) + '.rels';
