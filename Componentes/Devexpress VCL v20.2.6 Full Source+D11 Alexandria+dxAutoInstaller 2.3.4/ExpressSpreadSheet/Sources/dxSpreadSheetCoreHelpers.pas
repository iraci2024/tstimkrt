...
{$IFDEF DELPHI16}
  System.UITypes,
{$ENDIF}
  Types, Windows, Classes, Graphics, Generics.Defaults, Generics.Collections,
  SysUtils, Clipbrd, cxClasses, cxGraphics,
  dxCore, cxVariants, dxHashUtils, dxSpreadSheetCore, dxSpreadSheetClasses,
  dxSpreadSheetTypes, dxSpreadSheetStrs,
  dxSpreadSheetGraphics, dxSpreadSheetHyperlinks, dxSpreadSheetClipboardFormats,
  dxCoreClasses, dxSpreadSheetCoreStyles,
  dxSpreadSheetCoreFormulas, dxSpreadSheetStyles, Math, dxSpreadSheetUtils;

const
  NeighborBorderSide: array [TcxBorder] of TcxBorder = (bRight, bBottom,
    bLeft, bTop);
...
type
  ...
  public
    ...
    class procedure MergeBorders(ACurrRowCell, APrevRowCell,
      ANextRowCell: TdxSpreadSheetCell;
      ACellStyles: TdxSpreadSheetCellStyles); overload;
    class procedure MergeBorders(ACell, ALeftCell, ATopCell, ARightCell,
      ABottomCell: TdxSpreadSheetCell; ACellStyles: TdxSpreadSheetCellStyles;
      ATakeWeightIntoAccount: Boolean); overload;
    class procedure MergeBorders(var ACellBorders: TdxSpreadSheetBordersHandle;
      ALeftCell, ATopCell, ARightCell, ABottomCell: TdxSpreadSheetCell;
      ATakeWeightIntoAccount: Boolean); overload;
  ...
  TdxSpreadSheetTableViewCustomCellsModificationHelper = class
    (TdxSpreadSheetCustomTableViewHelper)
  strict private
    FFixedContainerAnchorTypes
      : TDictionary<TdxSpreadSheetContainer, TdxSpreadSheetContainerAnchorType>;
    FFixedContainers: TDictionary<TdxSpreadSheetContainer, TRect>;

    function GetIsHistoryAction: Boolean;
  protected
    function IsAnchorPointInArea(const AArea: TRect;
      AAnchorPoint: TdxSpreadSheetContainerAnchorPoint): Boolean;
    function IsDeletion: Boolean; virtual;

    procedure CreateIndependentConditionalFormattingAreasForMovedArea
      (const AMovingArea: TRect);
    procedure MoveConditionalFormattingAreas(const AMovingArea: TRect;
      ADeltaX, ADeltaY: Integer);

    procedure Move(var ALeftIndex, ARightIndex: Integer;
      AMovingAreaStartIndex, ADelta: Integer); inline;
    function MoveArea(var AArea: TRect; const AMovingArea: TRect;
      ADeltaX, ADeltaY: Integer): Boolean; inline;

    function NeedSaveContainersBounds(AContainer: TdxSpreadSheetContainer)
      : Boolean; virtual;
    ...
    procedure RestoreContainerPosition(AContainer: TdxSpreadSheetContainer;
      ...
      AAnchorType: TdxSpreadSheetContainerAnchorType); virtual;
    procedure RestoreContainersPosition;
  public
    constructor Create(AView: TdxSpreadSheetTableView); override;
    destructor Destroy; override;

    property FixedContainerAnchorTypes
      : TDictionary<TdxSpreadSheetContainer, TdxSpreadSheetContainerAnchorType>
      read FFixedContainerAnchorTypes;
    property FixedContainers: TDictionary<TdxSpreadSheetContainer, TRect>
      read FFixedContainers;
    property IsHistoryAction: Boolean read GetIsHistoryAction;
  end;

  { TdxSpreadSheetTableViewMoveCellsModificationHelper }

  TdxSpreadSheetTableViewMoveCellsModificationHelper = class
    (TdxSpreadSheetTableViewCustomCellsModificationHelper)
  strict private
    FSourceArea: TRect;
    FTargetArea: TRect;
  protected
    ...
    procedure RestoreContainerPosition(AContainer: TdxSpreadSheetContainer;
      const AContainerBounds: TRect;
      AAnchorType: TdxSpreadSheetContainerAnchorType); override;
  ...
  TdxSpreadSheetTableViewCellsModificationHelper = class
    (TdxSpreadSheetTableViewCustomCellsModificationHelper)
  strict private
    FArea: TRect;
    FIsHorizontalShifting: Boolean;
    FProcessedArea: TRect;
    FMergedCellsConflictResolution
  ...
  protected
    ...
    procedure CheckIsAreaEmpty(AItems: TdxSpreadSheetTableItems;
      const AArea: TRect);
    procedure CreateBlankCellsForStyledItems(AItems, AOppositeItems
      : TdxSpreadSheetTableItems; const AArea: TRect; AShiftDelta: Integer);
    function IsAreaAffected(const AArea: TRect; ADeltaX, ADeltaY: Integer;
      var ANewArea: TRect): Boolean;
    function IsAreaEmpty(AItems: TdxSpreadSheetTableItems;
    ...
    procedure MoveGroups(AGroups: TdxSpreadSheetTableItemGroup;
      AStartFromIndex, ADelta: Integer);
    procedure MovePrintAreas(const AMovingArea: TRect;
      ADeltaX, ADeltaY: Integer);
    procedure TransformAffectedObjectArea(ADeltaX, ADeltaY: Integer);
    procedure UnmergeAllAffectedCells;

    procedure ShiftCellsHorizontally(AStartFromIndex, AStartRowIndex,
      AFinishRowIndex, ADelta: Integer);
    procedure ShiftCellsVertically(AStartFromIndex, AStartColumnIndex,
      AFinishColumnIndex, ADelta: Integer);
    procedure ShiftColumns(AStartFromIndex, ADelta: Integer);
    procedure ShiftIndexes(AList: TdxDynamicItemList;
      AStartFromIndex, ADelta: Integer);
  ...
  public
    ...
    procedure ProcessCore(const AArea: TRect;
      AModification: TdxSpreadSheetCellsModification; AIsHorizontal: Boolean);
    // for history, for internal use
    //
    property IsHorizontalShifting: Boolean read FIsHorizontalShifting;
    property MergedCellsConflictResolution
      : TdxSpreadSheetTableViewMergedCellsConflictResolution
      read FMergedCellsConflictResolution write FMergedCellsConflictResolution;
    property Modification: TdxSpreadSheetCellsModification read FModification
      write FModification;
  ...
  TdxSpreadSheetTableViewDeleteCellsHelper = class
    (TdxSpreadSheetTableViewCellsModificationHelper)
  strict private
    procedure MoveAnchor(AAnchor: TdxSpreadSheetContainerAnchorPoint;
      const AArea: TRect; AMoveForward: Boolean);
  protected
    procedure CheckContainer(const AArea: TRect;
    ...
    procedure DeleteItems(AItems: TdxSpreadSheetTableItems;
  ...
  TdxSpreadSheetTableViewInsertCellsHelper = class
    (TdxSpreadSheetTableViewCellsModificationHelper)
  protected
    procedure BeforeShiftCells; override;
    procedure CalculateStyle(ANewStyle, ALeftSourceStyle, ARightSourceStyle
      : TdxSpreadSheetCellStyle; AIsFirst, AIsLast: Boolean); virtual;
    procedure CalculateStyleForNewCells(ATarget, ALeftSource,
      ARightSource: TdxSpreadSheetTableItem;
      AOppositeItems: TdxSpreadSheetTableItems;
      AStartIndex, AFinishIndex: Integer; AIsFirst, AIsLast: Boolean);
    procedure CalculateStyleForNewItems(const AArea: TRect;
      AItems, AOppositeItems: TdxSpreadSheetTableItems);
    procedure DoProcess(const AArea: TRect); override;

    function GetCellStyle(ASource: TdxSpreadSheetTableItem; ACellIndex: Integer;
      AOppositeItems: TdxSpreadSheetTableItems)
      : TdxSpreadSheetCellStyle; inline;
    function GetStyle(AItem: TdxSpreadSheetTableItem)
      : TdxSpreadSheetCellStyle; inline;
    //
    procedure EndAction; override;
    procedure StartAction; override;
  ...
  TdxSpreadSheetTableViewMergedCellStyleHelper = class
    (TdxSpreadSheetCustomTableViewHelper)
  strict private
  ...
  protected
    procedure CalculateBorder(ABorder: TcxBorder;
      AItem: TdxSpreadSheetTableItem; AStartIndex, AFinishIndex: Integer;
      out ABorderColor: TColor;
      out ABorderStyle: TdxSpreadSheetCellBorderStyle);
    procedure CalculateCellStyles;
    procedure CalculateCellStylesInCustomArea(const AArea: TRect;
      AStylePattern: TdxSpreadSheetCellStyle);
    procedure CalculateCellStylesInEntireColumnOrRow
  ...
  public
    constructor Create(AMergeCell: TdxSpreadSheetMergedCell;
      AUseActiveCellAsPattern: Boolean); reintroduce;
    class procedure Calculate(AMergeCell: TdxSpreadSheetMergedCell;
      AUseActiveCellAsPattern: Boolean = False);
  end;

  { TdxSpreadSheetTableViewSortingHelper }

  TdxSpreadSheetTableViewSortingHelper = class
    (TdxSpreadSheetCustomTableViewHelper)
  protected
    Area: TRect;
    ...
    function Initialize(AIndex1, AIndex2: Integer;
      const ASortOrders: array of TdxSortOrder;
      const AIndexes: array of Integer): Boolean;
  ...
  public
    ...
    procedure SortByColumnValues(const AArea: TRect;
      const ASortOrders: array of TdxSortOrder;
      const AColumnIndexes: array of Integer);
    procedure SortByRowValues(const AArea: TRect;
      const ASortOrders: array of TdxSortOrder;
      const ARowIndexes: array of Integer);
  end;
  ...
  type
    ...
    TCache = class
    strict private
      FCrossStyles: TObjectDictionary<TdxSpreadSheetCellStyleHandle,
        TList<TdxSpreadSheetCellStyleHandle>>;
      FStyles: TList<TdxSpreadSheetCellStyleHandle>;
    public
      constructor Create;
      destructor Destroy; override;
      function Add(ARowStyle, AColumnStyle: TdxSpreadSheetCellStyleHandle)
        : Boolean; overload;
  ...
  strict private
    ...
    procedure EnumColumnStyles(AArea: TRect;
      AProcRef: TdxSpreadSheetCellStyleEnumProcRef);
    procedure EnumDefaultCellsStyles(const R: TRect;
      AProcRef: TdxSpreadSheetCellStyleEnumProcRef);
    procedure EnumRowStyles(AArea: TRect;
      AProcRef: TdxSpreadSheetCellStyleEnumProcRef);
  public
    constructor Create(ASheet: TdxSpreadSheetTableView); virtual;
    destructor Destroy; override;
    procedure PrepareToSave(AArea: TRect);
    procedure ProcessArea(const AArea: TRect;
      AProcRef: TdxSpreadSheetCellStyleEnumProcRef);
    //
    property EnumDefaultStyles: Boolean read FEnumDefaultStyles
      write FEnumDefaultStyles;
  ...
  TdxSpreadSheetPasteCellDataFromStreamHelper = class
    (TdxSpreadSheetBinaryClipboardFormatData)
  protected
    procedure RestoreCells(AView: TdxSpreadSheetTableView;
      AStyles: TList<TdxSpreadSheetCellStyleHandle>; const AArea: TRect;
  ...
  public
    ...
    procedure Copy(const AArea: TRect; AContainer: TdxSpreadSheetContainer;
      ...
      AView: TdxSpreadSheetTableView); override;
  end;

  { TdxSpreadSheetDefinedNameHelper }

  TdxSpreadSheetDefinedNameEnumProc = reference to procedure
    (ADefinedName: TdxSpreadSheetDefinedName);

  TdxSpreadSheetDefinedNameHelper = class
  public
    class procedure Enum(AView: TdxSpreadSheetCustomView;
      AProc: TdxSpreadSheetDefinedNameEnumProc);
    class function FindByArea(ADefinedNames: TdxSpreadSheetDefinedNames;
      AView: TdxSpreadSheetCustomView; const AArea: TRect;
      out ADefinedName: TdxSpreadSheetDefinedName): Boolean;
    class function GetHeaders(AView: TdxSpreadSheetTableView;
      const AArea: TRect): TcxBorders; overload;
    class function GetHeaders(AView: TdxSpreadSheetTableView;
      const AAreas: TdxRectList): TcxBorders; overload;
    class function GetBoundingArea(ADefinedName: TdxSpreadSheetDefinedName)
      : TRect; overload;
    class function GetBoundingArea(ADefinedName: TdxSpreadSheetDefinedName;
      out AView: TdxSpreadSheetCustomView): TRect; overload;
    class function GetUsedAreas(ADefinedName: TdxSpreadSheetDefinedName;
      out AView: TdxSpreadSheetCustomView): TdxRectList;
    class function IsInternalDefinedName(ADefinedName
      : TdxSpreadSheetDefinedName): Boolean;
    class function ReplaceIllegalChars(const ACaption: string;
      ASpreadSheet: TdxCustomSpreadSheet): string;
  end;
...
const
  dxSpreadSheetTableViewClearCellsOptionsAll = [ccoValues, ccoHyperlinks,
    ccoComments, ccoStyles];

procedure dxSpreadSheetMergeBorderStyle(ASide: TcxBorder;
  AStyle1: TdxSpreadSheetCellStyleHandle;
  AStyle2: TdxSpreadSheetCellStyleHandle; var AColor: TColor;
  var AStyle: TdxSpreadSheetCellBorderStyle); overload; inline;
procedure dxSpreadSheetMergeBorderStyle(ASide: TcxBorder;
  AStyle1: TdxSpreadSheetBordersHandle; AStyle2: TdxSpreadSheetBordersHandle;
  var AColor: TColor; var AStyle: TdxSpreadSheetCellBorderStyle); overload;

procedure dxSpreadSheetStoreSelectionAndScrollPosition
  (AView: TdxSpreadSheetTableView; ASelection: TdxRectList;
  out AFocusedCell, AScrollPosition: TPoint);
procedure dxSpreadSheetRestoreSelectionAndScrollPosition
  (AView: TdxSpreadSheetTableView; ASelection: TdxRectList;
  const AFocusedCell, AScrollPosition: TPoint);

implementation

uses
  Dialogs, Controls, Variants, StrUtils, cxGeometry, cxControls, dxTypeHelpers,
  dxSpreadSheetFormulas, cxDateUtils,
  dxSpreadSheetFormatBinary, dxGDIPlusClasses, dxCoreGraphics,
  dxSpreadSheetCoreHistory, dxSpreadSheetContainers,
  dxSpreadSheetConditionalFormatting, dxSpreadSheetCoreStrs,
  dxSpreadSheetPrinting, dxStringHelper;
...
type
  ...
  TdxSpreadSheetFormulaControllerAccess = class
  ...
  public
    ...
    procedure Calculate(ARow: TdxSpreadSheetTableRow;
  ...
  TdxSpreadSheetExpressionController = class
...
begin
  ...
  AScrollPosition.X := TdxSpreadSheetTableViewAccess(AView)
    .ViewInfo.FirstScrollableColumn;
  AScrollPosition.Y := TdxSpreadSheetTableViewAccess(AView)
    .ViewInfo.FirstScrollableRow;
end;
...
begin
  ...
  TdxSpreadSheetTableViewAccess(AView).ViewInfo.FirstScrollableColumn :=
    AScrollPosition.X;
  TdxSpreadSheetTableViewAccess(AView).ViewInfo.FirstScrollableRow :=
    AScrollPosition.Y;
end;
...
procedure dxSpreadSheetMergeBorderStyle(ASide: TcxBorder;
  AStyle1: TdxSpreadSheetCellStyleHandle;
  AStyle2: TdxSpreadSheetCellStyleHandle; var AColor: TColor;
  var AStyle: TdxSpreadSheetCellBorderStyle);
begin
  dxSpreadSheetMergeBorderStyle(ASide, AStyle1.Borders, AStyle2.Borders,
    AColor, AStyle);
  if AStyle = sscbsDefault then
  begin
    if cxColorIsValid(AStyle1.Brush.BackgroundColor) or
      cxColorIsValid(AStyle2.Brush.BackgroundColor) then
      AStyle := sscbsNone;
  end;
end;
...
begin
  ...
  ADelta := dxSpreadSheetBorderStyleWeights[ABorderStyle1] -
    dxSpreadSheetBorderStyleWeights[ABorderStyle2];
  if (ADelta < 0) or (ADelta = 0) and (AStyle1.BorderColor[ASide] = clDefault)
  then
  begin
...
procedure TdxSpreadSheetFakeCellStyle.Calculate(ARow: TdxSpreadSheetTableRow;
  AColumn: TdxSpreadSheetTableColumn);
var
  ABorderColor: TColor;
...
begin
  AViewInfo := TdxSpreadSheetTableViewInfoAccess
    (TdxSpreadSheetTableViewAccess(View).ViewInfo);
  Merge(AColumn.Style, ARow.Style);
  for ASide := Low(TcxBorder) to High(TcxBorder) do
  begin
    ANeighborCellStyle := AViewInfo.GetNeighborCellStyle(ARow.Index,
      AColumn.Index, ASide, False, False);
    dxSpreadSheetMergeBorderStyle(ASide, Handle, ANeighborCellStyle,
...
class procedure TdxSpreadSheetCellStyleHelper.Calculate
...
begin
  ...
  try
    ...
    MergeBorders(ABordersHandle, AView.Cells[ACell.RowIndex,
      ACell.ColumnIndex - 1], AView.Cells[ACell.RowIndex - 1,
      ACell.ColumnIndex], AView.Cells[ACell.RowIndex, ACell.ColumnIndex + 1],
      AView.Cells[ACell.RowIndex + 1, ACell.ColumnIndex], False);
...
class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(ACurrRowCell,
  APrevRowCell, ANextRowCell: TdxSpreadSheetCell;
  ACellStyles: TdxSpreadSheetCellStyles);
var
  ABottomCell: TdxSpreadSheetCell;
  ACurrCell: TdxSpreadSheetCellAccess;
  ALeftCell: TdxSpreadSheetCell;
  ANextCell: TdxSpreadSheetCellAccess;
  APrevCell: TdxSpreadSheetCellAccess;
  ARightCell: TdxSpreadSheetCell;
  ATopCell: TdxSpreadSheetCell;
begin
  APrevCell := nil;
  ACurrCell := TdxSpreadSheetCellAccess(ACurrRowCell);
  while ACurrCell <> nil do
  begin
    ANextCell := TdxSpreadSheetCellAccess(ACurrCell.FNext);
    while (ANextRowCell <> nil) and (ANextRowCell.Index < ACurrCell.Index) do
      ANextRowCell := TdxSpreadSheetCell
        (TdxSpreadSheetCellAccess(ANextRowCell).FNext);
    while (APrevRowCell <> nil) and (APrevRowCell.Index < ACurrCell.Index) do
      APrevRowCell := TdxSpreadSheetCell
        (TdxSpreadSheetCellAccess(APrevRowCell).FNext);
    ...
    MergeBorders(ACurrCell, ALeftCell, ATopCell, ARightCell, ABottomCell,
      ACellStyles, True);
...
class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(ACell, ALeftCell,
  ATopCell, ARightCell, ABottomCell: TdxSpreadSheetCell;
  ACellStyles: TdxSpreadSheetCellStyles; ATakeWeightIntoAccount: Boolean);
var
...
begin
  ...
  begin
    ...
    try
      MergeBorders(ABorderHandle, ALeftCell, ATopCell, ARightCell, ABottomCell,
        ATakeWeightIntoAccount);
    finally
...
class procedure TdxSpreadSheetCellStyleHelper.MergeBorders(var ACellBorders
  : TdxSpreadSheetBordersHandle; ALeftCell, ATopCell, ARightCell,
  ABottomCell: TdxSpreadSheetCell; ATakeWeightIntoAccount: Boolean);

  procedure MergeBorder(var ATarget: TdxSpreadSheetBordersHandle;
    ASource: TdxSpreadSheetBordersHandle; ABorder: TcxBorder);
  const
    OppositeBordersMap: array [TcxBorder] of TcxBorder = (bRight, bBottom,
      bLeft, bTop);
  var
  ...
  begin
    ...
    begin
      if dxSpreadSheetBorderStyleWeights[ASource.BorderStyle[AOppositeBorder]] <
        dxSpreadSheetBorderStyleWeights[ATarget.BorderStyle[ABorder]] then
        Exit;
    end;
  ...
  try
    ...
    begin
      ...
      if ACurrRow <> nil then
        ACurrRowCell := TdxSpreadSheetCell
          (TdxSpreadSheetTableRowAccess(ACurrRow).RowCells.First)
      else
        ACurrRowCell := nil;

      if (APrevRow <> nil) and (APrevRow.Index + 1 = ACurrRow.Index) then
        APrevRowCell := TdxSpreadSheetCell
          (TdxSpreadSheetTableRowAccess(APrevRow).RowCells.First)
      else
        APrevRowCell := nil;

      if (ANextRow <> nil) and (ANextRow.Index - 1 = ACurrRow.Index) then
        ANextRowCell := TdxSpreadSheetCell
          (TdxSpreadSheetTableRowAccess(ANextRow).RowCells.First)
      else
        ANextRowCell := nil;
...
constructor TdxSpreadSheetCustomTableViewHelper.Create
  (AView: TdxSpreadSheetTableView);
begin
  inherited Create;
  FView := AView;
...
procedure TdxSpreadSheetCustomTableViewHelper.ClearCells(const AArea,
  AExcludedArea: TRect; AOptions: TdxSpreadSheetTableViewClearCellsOptions);
begin
  TdxSpreadSheetTableViewAccess(View).ClearCells(AArea, AExcludedArea,
    AOptions);
end;

procedure TdxSpreadSheetCustomTableViewHelper.ClearCells(const AArea: TRect;
...
function TdxSpreadSheetCustomTableViewHelper.GetColumns
...
function TdxSpreadSheetCustomTableViewHelper.GetSpreadSheet
...
constructor TdxSpreadSheetTableViewCustomCellsModificationHelper.Create
  (AView: TdxSpreadSheetTableView);
begin
  inherited Create(AView);
  FFixedContainers := TDictionary<TdxSpreadSheetContainer, TRect>.Create;
  FFixedContainerAnchorTypes :=
    TDictionary<TdxSpreadSheetContainer,
    TdxSpreadSheetContainerAnchorType>.Create;
end;
...
function TdxSpreadSheetTableViewCustomCellsModificationHelper.
  IsAnchorPointInArea(const AArea: TRect;
  AAnchorPoint: TdxSpreadSheetContainerAnchorPoint): Boolean;
begin
  Result := dxSpreadSheetContains(AArea, AAnchorPoint.Cell.RowIndex,
    AAnchorPoint.Cell.ColumnIndex);
end;
...
procedure TdxSpreadSheetTableViewCustomCellsModificationHelper.Move
  (var ALeftIndex, ARightIndex: Integer;
  AMovingAreaStartIndex, ADelta: Integer);
begin
  if ADelta <> 0 then
  begin
    Inc(ARightIndex, ADelta);
    if (ALeftIndex > AMovingAreaStartIndex) or
      (ALeftIndex = AMovingAreaStartIndex) and not IsDeletion then
      ALeftIndex := Max(ALeftIndex + ADelta, 0);
  end;
end;

function TdxSpreadSheetTableViewCustomCellsModificationHelper.MoveArea
  (var AArea: TRect; const AMovingArea: TRect;
  ADeltaX, ADeltaY: Integer): Boolean;
var
...
begin
  ...
  begin
    AMoveHorizontally := not dxSpreadSheetIsEntireRow(AArea) and
      (AArea.Top >= AMovingArea.Top) and (AArea.Bottom <= AMovingArea.Bottom);
    AMoveVertically := not dxSpreadSheetIsEntireColumn(AArea) and
      (AArea.Left >= AMovingArea.Left) and (AArea.Right <= AMovingArea.Right);
...
procedure TdxSpreadSheetTableViewCustomCellsModificationHelper.
  CreateIndependentConditionalFormattingAreasForMovedArea
  (const AMovingArea: TRect);

  procedure AddArea(ARule: TdxSpreadSheetCustomConditionalFormattingRule;
    const AArea: TRect);
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if not dxSpreadSheetContains(AMovingArea, ARuleArea) and
        dxSpreadSheetIntersects(ARuleArea, AMovingArea, AArea) then
      begin
        ...
        AddArea(ARule, Rect(ARuleArea.Left, ARuleArea.Top, AArea.Left - 1,
          ARuleArea.Bottom));
        AddArea(ARule, Rect(AArea.Right + 1, ARuleArea.Top, ARuleArea.Right,
          ARuleArea.Bottom));
        AddArea(ARule, Rect(AArea.Left, ARuleArea.Top, AArea.Right,
          AArea.Top - 1));
        AddArea(ARule, Rect(AArea.Left, AArea.Bottom + 1, AArea.Right,
          ARuleArea.Bottom));
...
procedure TdxSpreadSheetTableViewCustomCellsModificationHelper.
  MoveConditionalFormattingAreas(const AMovingArea: TRect;
  ADeltaX, ADeltaY: Integer);
var
  AArea: TRect;
  AAreaIndex: Integer;
...
function TdxSpreadSheetTableViewCustomCellsModificationHelper.
  NeedSaveContainersBounds(AContainer: TdxSpreadSheetContainer): Boolean;
begin
  Result := False;
end;
...
begin
  ...
  begin
    ...
    TdxSpreadSheetContainersAccess(View.Containers).UnregisterCommentContainer
      (AContainer);
  end;
end;

procedure TdxSpreadSheetTableViewCustomCellsModificationHelper.
  RestoreContainerPosition(AContainer: TdxSpreadSheetContainer;
  const AContainerBounds: TRect;
...
begin
  ...
  begin
    ...
    TdxSpreadSheetContainerAccess(AContainer)
      .Calculator.UpdateAnchorsAfterResize(AContainerBounds, True);
end;
...
begin
  for I := 0 to View.Containers.Count - 1 do
    TdxSpreadSheetContainersAccess(View.Containers).RegisterCommentContainer
      (View.Containers[I]);
  ...
  if not IsHistoryAction then
    ...
    begin
      ...
      try
        RestoreContainerPosition(AContainer, FixedContainers.Items[AContainer],
          FixedContainerAnchorTypes.Items[AContainer]);
      finally
...
procedure TdxSpreadSheetTableViewMoveCellsModificationHelper.Process
  (const AArea: TRect; const ATargetPoint: TPoint);
begin
  FSourceArea := AArea;
...
function TdxSpreadSheetTableViewMoveCellsModificationHelper.
  NeedSaveContainersBounds(AContainer: TdxSpreadSheetContainer): Boolean;

  function CheckAnchor(AAnchor: TdxSpreadSheetContainerAnchorPoint): Boolean;
  begin
    Result := AAnchor.FixedToCell and
      (IsAnchorPointInArea(SourceArea, AAnchor) or
      IsAnchorPointInArea(TargetArea, AAnchor));
  end;
...
begin
  ...
  case AContainer.AnchorType of
    ...
    catTwoCell:
      Result := CheckAnchor(AContainer.AnchorPoint1) or
        CheckAnchor(AContainer.AnchorPoint2);
  end;
end;
...
begin
  ...
  try
    ...
    View.SpreadSheet.FormulaController.UpdateReferences(View, SourceArea,
      TargetArea.TopLeft, urmMove);

    if IsHistoryAction then
      ClearCells(TargetArea, SourceArea, [ccoFreeCellInstances])
    else
    begin
      TdxSpreadSheetTableViewAccess(View).MergedCells.DeleteItemsInArea
        (SourceArea, False);
      TdxSpreadSheetTableViewAccess(View).MergedCells.DeleteItemsInArea
        (TargetArea, False);
      ClearCells(TargetArea, SourceArea,
        dxSpreadSheetTableViewClearCellsOptionsAll + [ccoFreeCellInstances]);
      History.AddCommand(TdxSpreadSheetHistoryMoveCellCommand.Create(SourceArea,
        TargetArea));
    end;

    CreateIndependentConditionalFormattingAreasForMovedArea(SourceArea);
    MoveConditionalFormattingAreas(SourceArea,
      TargetArea.Left - SourceArea.Left, TargetArea.Top - SourceArea.Top);

    TdxSpreadSheetTableRowsAccess(View.Rows).ForEach(
      procedure(ARow: TdxDynamicListItem)
      begin
        TdxSpreadSheetTableRowCellsAccess(TdxSpreadSheetTableRowAccess(ARow)
          .RowCells).ForEach(procedure(AItem: TdxDynamicListItem)
          begin
            TdxSpreadSheetCellAccess(AItem).Move(ARow.Index - SourceArea.Top +
              TargetArea.Top, AItem.Index - SourceArea.Left + TargetArea.Left);
          end, SourceArea.Left, SourceArea.Right,
          TargetArea.Left <= SourceArea.Left);
      end, SourceArea.Top, SourceArea.Bottom, TargetArea.Top <= SourceArea.Top);
...
procedure TdxSpreadSheetTableViewMoveCellsModificationHelper.
  RestoreContainerPosition(AContainer: TdxSpreadSheetContainer;
const AContainerBounds: TRect; AAnchorType: TdxSpreadSheetContainerAnchorType);
begin
  AContainer.AnchorType := AAnchorType;
  TdxSpreadSheetContainerAccess(AContainer).Calculator.UpdateAnchors
    (AContainerBounds);
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.ProcessCore
  (const AArea: TRect; AModification: TdxSpreadSheetCellsModification;
AIsHorizontal: Boolean);
begin
  FMovableArea := AArea;
  FModification := AModification;
  FIsHorizontalShifting := Modification in [cmShiftCellsHorizontally,
    cmShiftColumns];
  FProcessedArea := AArea;
  DoProcess(AArea);
end;

procedure TdxSpreadSheetTableViewCellsModificationHelper.Initialize
  (var AArea: TRect);
begin
  if dxSpreadSheetIsEntireRow(AArea) and (Modification = cmShiftCellsVertically)
  then
    Modification := cmShiftRows;
  if dxSpreadSheetIsEntireColumn(AArea) and
    (Modification = cmShiftCellsHorizontally) then
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.CheckIsAreaEmpty
  (AItems: TdxSpreadSheetTableItems; const AArea: TRect);
begin
  ...
  begin
    if not IsAreaEmpty(AItems, AArea) and not IsHistoryAction then
      raise EdxSpreadSheetError.Create
        (cxGetResourceString(@sdxErrorPossibleDataLoss));
  end;
end;

procedure TdxSpreadSheetTableViewCellsModificationHelper.
  CreateBlankCellsForStyledItems(AItems, AOppositeItems
  : TdxSpreadSheetTableItems; const AArea: TRect; AShiftDelta: Integer);

  procedure ApplyStyleToBlankCellsInItem(AItem: TdxSpreadSheetTableItem;
  AStyleHandle: TdxSpreadSheetCellStyleHandle);
  var
  ...
  begin
    for I := AArea.Top to AArea.Bottom do
      ...
      begin
        ...
        if IsHorizontalShifting and (AOppositeItem <> nil) and
          not AOppositeItem.Style.IsDefault then
          Continue;

        if (AStyleHandle <> nil) and (AItem.Style.Handle <> AStyleHandle) then
          AItem.CreateCell(I).StyleHandle := AStyleHandle
        else if (AStyleHandle = nil) and (AOppositeItem <> nil) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if InRange(AItem.Index + AShiftDelta, AArea.Left, AMaxItemIndex)
        then
          ApplyStyleToBlankCellsInItem
            (AItems.CreateItem(AItem.Index + AShiftDelta), AItem.Style.Handle);
      end;
      AItem := AItem.Prev;
    end;
  end;
...
begin
  ...
  if Result then
  begin
    if cxRectIsEqual(AArea, ANewArea) and
      not(IsDeletion and dxSpreadSheetIntersects(ANewArea, FArea)) then
...
function TdxSpreadSheetTableViewCellsModificationHelper.IsAreaEmpty
  (AItems: TdxSpreadSheetTableItems; const AArea: TRect): Boolean;
var
  AItem: TdxSpreadSheetTableItem;
  I: Integer;
begin
...
function TdxSpreadSheetTableViewCellsModificationHelper.NeedSaveContainersBounds
...
begin
  ...
  case AContainer.AnchorType of
    catOneCell:
      Result := not(AContainer.AnchorPoint1.FixedToCell and
        IsAnchorPointInArea(MovableArea, AContainer.AnchorPoint1));
    catTwoCell:
      begin
        R := MovableArea;
        ...
        Result := not(AContainer.AnchorPoint1.FixedToCell and
          IsAnchorPointInArea(R, AContainer.AnchorPoint1) and
          AContainer.AnchorPoint2.FixedToCell and IsAnchorPointInArea(R,
          AContainer.AnchorPoint2));
      end;
  end;
end;
  ...
  if HasAffectedMergedCells then
    case MergedCellsConflictResolution of
      mccrShowConfirmation:
        if MessageDlg(cxGetResourceString(@sdxUnmergeCellsConfirmation),
      ...
      raise EdxSpreadSheetError.Create
        (cxGetResourceString(@sdxErrorCannotMoveBecauseOfMergedCells));
    end;
end;

function TdxSpreadSheetTableViewCellsModificationHelper.
  HasAffectedMergedCells: Boolean;
var
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.MoveGroups
  ...
  function ProcessGroup(AGroup: TdxSpreadSheetTableItemGroup;
  AStartFromIndex, ADelta: Integer): Boolean;
  var
    AStartIndex, AFinishIndex: Integer;
  begin
    ...
    if Result and ((AGroup.StartIndex <> AStartIndex) or
      (AGroup.FinishIndex <> AFinishIndex)) then
    begin
      AGroup.BeginUpdate;
      try
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.MovePrintAreas
  (const AMovingArea: TRect; ADeltaX, ADeltaY: Integer);

  function AdjustPrintRect
    (ARect: TdxSpreadSheetTableViewOptionsPrintRect): Boolean;
  var
    R: TRect;
  begin
    Result := True;
  ...
  procedure AdjustPageBreaks(AList: TList<Cardinal>; const APrintArea: TRect;
  AIsHorizontal: Boolean);
  var
    I: Integer;
    R: TRect;
  begin
    ...
    begin
      ...
      if dxSpreadSheetIntersects(R, APrintArea, R) and
        MoveArea(R, AMovingArea, ADeltaX, ADeltaY) then
...
begin
  ...
  if ADeltaX <> 0 then
    AdjustPageBreaks(View.OptionsPrint.Pagination.ColumnPageBreaks,
      View.PrintArea, True);
  if ADeltaY <> 0 then
    AdjustPageBreaks(View.OptionsPrint.Pagination.RowPageBreaks,
      View.PrintArea, False);
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.
  TransformAffectedObjectArea(ADeltaX, ADeltaY: Integer);
var
  ANewArea: TRect;
...
begin
  ...
  begin
    ...
    begin
      if dxSpreadSheetIsValidArea(ANewArea) and
        not dxSpreadSheetIsSingleCellArea(ANewArea) then
      begin
        History.AddCommand(TdxSpreadSheetHistoryMergeCellsCommand.Create
          (AMergedCell, ANewArea));
        TdxSpreadSheetMergedCellAccess(AMergedCell)
          .Initialize(AMergedCell.Owner, ANewArea);
      end
      else
        AMergedCell.Free;
    end;
...
begin
  ...
  TdxSpreadSheetTableRowsAccess(Rows).ForEach(
    ...
    begin
      ShiftIndexes(TdxSpreadSheetTableRowAccess(AItem).RowCells,
        AStartFromIndex, ADelta);
    end, AStartRowIndex, AFinishRowIndex);

  View.ForEachCell(cxRect(AStartFromIndex, AStartRowIndex, MaxInt,
    AFinishRowIndex), procedure(ACell: TdxSpreadSheetCell)
    begin
  ...
  TdxSpreadSheetTableRowsAccess(Rows).ForEach(
    ...
    begin
      ...
      begin
        ...
        if InRange(ACell.ColumnIndex, AStartColumnIndex, AFinishColumnIndex)
        then
          ACell.RowIndex := ATargetRowIndex;
        ACell := ANextCell;
      end;
    end, AStartFromIndex, MaxInt, ADelta <= 0);
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.ShiftColumns
  (AStartFromIndex, ADelta: Integer);
begin
  if ADelta > 0 then
    CheckIsAreaEmpty(Columns, cxRect(dxSpreadSheetMaxColumnIndex - ADelta + 1,
      0, dxSpreadSheetMaxColumnIndex, MaxInt));
  BeforeShiftCells;
  ...
  MovePrintAreas(MovableArea, ADelta, 0);
  TransformAffectedObjectArea(ADelta, 0);
  MoveGroups(TdxSpreadSheetTableItemGroupsAccess(Columns.Groups).Root,
    AStartFromIndex, ADelta);
end;

procedure TdxSpreadSheetTableViewCellsModificationHelper.ShiftIndexes
...
procedure TdxSpreadSheetTableViewCellsModificationHelper.ShiftRows
  (AStartFromIndex, ADelta: Integer);
begin
  if ADelta > 0 then
    CheckIsAreaEmpty(Rows, cxRect(dxSpreadSheetMaxRowIndex - ADelta + 1, 0,
      dxSpreadSheetMaxRowIndex, MaxInt));
  BeforeShiftCells;
  ...
  MovePrintAreas(MovableArea, 0, ADelta);
  TransformAffectedObjectArea(0, ADelta);
  MoveGroups(TdxSpreadSheetTableItemGroupsAccess(Rows.Groups).Root,
    AStartFromIndex, ADelta);
end;

{ TdxSpreadSheetTableViewDeleteCellsHelper }

procedure TdxSpreadSheetTableViewDeleteCellsHelper.CheckContainer
  (const AArea: TRect; AContainer: TdxSpreadSheetContainer);
var
...
begin
  ...
  case AContainer.AnchorType of
    catTwoCell:
      if AContainer.AnchorPoint1.FixedToCell and
        IsAnchorPointInArea(AArea, AContainer.AnchorPoint1) and
        AContainer.AnchorPoint2.FixedToCell and
        IsAnchorPointInArea(AArea, AContainer.AnchorPoint2) then
        ANeedDelete := True
      else if AContainer.AnchorPoint1.FixedToCell and
        IsAnchorPointInArea(AArea, AContainer.AnchorPoint1) then
      begin
...
procedure TdxSpreadSheetTableViewDeleteCellsHelper.CheckContainers
  (const AArea: TRect);
var
  I: Integer;
begin
...
procedure TdxSpreadSheetTableViewDeleteCellsHelper.DeleteCellsInArea
  (const AArea: TRect);
begin
  View.ForEachCell(AArea, procedure(ACell: TdxSpreadSheetCell)
    begin
      ACell.Free;
    end, False);
end;
...
procedure TdxSpreadSheetTableViewDeleteCellsHelper.DeleteItems
  (AItems: TdxSpreadSheetTableItems; AStartIndex, AFinishIndex: Integer);
begin
  TdxDynamicItemListAccess(AItems).ForEach(procedure(AItem: TdxDynamicListItem)
    begin
      AItem.Free;
    end, AStartIndex, AFinishIndex);
end;

procedure TdxSpreadSheetTableViewDeleteCellsHelper.DoProcess
  (const AArea: TRect);
begin
  ...
  SpreadSheet.FormulaController.UpdateReferences(View, AArea,
    cxPoint(IfThen(IsHorizontalShifting, -1, AArea.Left),
    IfThen(IsHorizontalShifting, AArea.Top, -1)), urmDelete);
  ...
  try
    ...
    case Modification of
      cmShiftCellsHorizontally:
        ShiftCellsHorizontally(AArea.Left, AArea.Top, AArea.Bottom,
          -(cxRectWidth(AArea) + 1));

      cmShiftCellsVertically:
        ShiftCellsVertically(AArea.Top, AArea.Left, AArea.Right,
          -(cxRectHeight(AArea) + 1));
    ...
    History.AddCommand(TdxSpreadSheetHistoryDeleteCommand.Create(View, AArea,
      Modification, IsHorizontalShifting));

    case Modification of
      cmShiftCellsHorizontally:
        CreateBlankCellsForStyledItems(Columns, Rows, MovableArea,
          -(cxRectWidth(AArea) + 1));
      cmShiftCellsVertically:
        CreateBlankCellsForStyledItems(Rows, Columns,
          cxRect(MovableArea.Top, MovableArea.Left, MovableArea.Bottom,
          MovableArea.Right), -(cxRectHeight(AArea) + 1));
    end;
  finally
    RestoreContainersPosition;
  end;
...
procedure TdxSpreadSheetTableViewDeleteCellsHelper.MoveAnchor
  (AAnchor: TdxSpreadSheetContainerAnchorPoint; const AArea: TRect;
...
begin
  SpreadSheet.FormulaController.UpdateReferences(View, ProcessedArea,
    cxPointOffset(ProcessedArea.TopLeft, IfThen(IsHorizontalShifting,
    cxRectWidth(ProcessedArea) + 1), IfThen(not IsHorizontalShifting,
    cxRectHeight(ProcessedArea) + 1)), urmInsert);
end;

procedure TdxSpreadSheetTableViewInsertCellsHelper.CalculateStyle(ANewStyle,
  ALeftSourceStyle, ARightSourceStyle: TdxSpreadSheetCellStyle;
AIsFirst, AIsLast: Boolean);

  function HasBorder(AStyle: TdxSpreadSheetCellStyle;
  ABorder: TcxBorder): Boolean;
  begin
    Result := (AStyle <> nil) and
      (AStyle.Borders[ABorder].Style <> sscbsDefault);
  end;
...
begin
  ...
  try
    ...
    if AHasRightBorder and AIsLast then
      ANewStyle.Borders[ARightSide].Assign
        (ARightSourceStyle.Borders[ALeftSide]);
  finally
    ANewStyle.EndUpdate;
  end;
end;
...
begin
  ...
  begin
    ...
    if (ALeftSourceStyle <> nil) or (ARightSourceStyle <> nil) then
      CalculateStyle(ATarget.CreateCell(I).Style, ALeftSourceStyle,
        ARightSourceStyle, AIsFirst, AIsLast);
  end;
end;
...
begin
  ...
  begin
    CreateBlankCellsForStyledItems(AItems, AOppositeItems,
      cxRect(AArea.Left, AArea.Top, MaxInt, AArea.Bottom),
      cxRectWidth(AArea) + 1);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        CalculateStyle(AItem.Style, GetStyle(ALeftSourceItem),
          GetStyle(ARightSourceItem), AIsFirst, AIsLast);
        if (ALeftSourceItem <> nil) and not ALeftSourceItem.DefaultSize then
          AItem.Size := ALeftSourceItem.CustomSize;
      end;
      CalculateStyleForNewCells(AItem, ALeftSourceItem, ARightSourceItem,
        AOppositeItems, AArea.Top, AArea.Bottom, AIsFirst, AIsLast);
    end;
  end;
end;

procedure TdxSpreadSheetTableViewInsertCellsHelper.DoProcess
  (const AArea: TRect);
begin
  StoreContainersPosition;
  try
    case Modification of
      cmShiftColumns:
        begin
          ShiftColumns(AArea.Left, cxRectWidth(AArea) + 1);
          CalculateStyleForNewItems(cxRect(AArea.Left, 0, AArea.Right, MaxInt),
            Columns, Rows);
        end;
      ...
      cmShiftRows:
        begin
          ShiftRows(AArea.Top, cxRectHeight(AArea) + 1);
          CalculateStyleForNewItems(cxRect(AArea.Top, 0, AArea.Bottom, MaxInt),
            Rows, Columns);
        end;
      ...
      cmShiftCellsVertically:
        begin
          ShiftCellsVertically(AArea.Top, AArea.Left, AArea.Right,
            cxRectHeight(AArea) + 1);
          CalculateStyleForNewItems(cxRect(AArea.Top, AArea.Left, AArea.Bottom,
            AArea.Right), Rows, Columns);
        end;
    end;
  finally
    RestoreContainersPosition;
  end;
  History.AddCommand(TdxSpreadSheetHistoryInsertCommand.Create(View, AArea,
    Modification, IsHorizontalShifting));
end;

function TdxSpreadSheetTableViewInsertCellsHelper.GetCellStyle
  (ASource: TdxSpreadSheetTableItem; ACellIndex: Integer;
AOppositeItems: TdxSpreadSheetTableItems): TdxSpreadSheetCellStyle;
var
  ACell: TdxSpreadSheetCell;
begin
  if ASource <> nil then
  ...
  if ACell <> nil then
    Result := ACell.Style
  else if IsHorizontalShifting then
  begin
...
function TdxSpreadSheetTableViewInsertCellsHelper.GetStyle
  (AItem: TdxSpreadSheetTableItem): TdxSpreadSheetCellStyle;
begin
  if AItem <> nil then
    Result := AItem.Style
  else
...
class procedure TdxSpreadSheetTableViewMergedCellStyleHelper.Calculate
  (AMergeCell: TdxSpreadSheetMergedCell;
AUseActiveCellAsPattern: Boolean = False);
begin
...
begin
  FNonBlankCell := nil;
  View.ForEachCell(MergeCell.Area, procedure(ACell: TdxSpreadSheetCell)
    begin
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.CalculateBorder
  (ABorder: TcxBorder; AItem: TdxSpreadSheetTableItem;
AStartIndex, AFinishIndex: Integer; out ABorderColor: TColor;
out ABorderStyle: TdxSpreadSheetCellBorderStyle);

  procedure GetBorderParams(ACell: TdxSpreadSheetCell; out ABorderColor: TColor;
  out ABorderStyle: TdxSpreadSheetCellBorderStyle);
  begin
    if ACell <> nil then
    begin
  ...
  procedure ResetBorderParams(out ABorderColor: TColor;
  out ABorderStyle: TdxSpreadSheetCellBorderStyle);
  begin
...
begin
  ...
  begin
    GetBorderParams(AItem.Cells[AStartIndex], ATempBorderColor,
      ATempBorderStyle);
    if (ABorderColor <> ATempBorderColor) or (ABorderStyle <> ATempBorderStyle)
    then
    begin
      ResetBorderParams(ABorderColor, ABorderStyle);
      Break;
...
begin
  ...
  CalculateBorder(bBottom, Rows[AStylePatternArea.Bottom],
    AStylePatternArea.Left, AStylePatternArea.Right, FBorderColors[bBottom],
    FBorderStyles[bBottom]);
  ...
  CalculateBorder(bRight, Columns[AStylePatternArea.Right],
    AStylePatternArea.Top, AStylePatternArea.Bottom, FBorderColors[bRight],
    FBorderStyles[bRight]);
  ...
  begin
    if dxSpreadSheetIsEntireColumn(AArea) then
      CalculateCellStylesInEntireColumnOrRow(View.Columns, AArea.Left,
        AArea.Right, AStylePattern)
    else if dxSpreadSheetIsEntireRow(AArea) then
      CalculateCellStylesInEntireColumnOrRow(View.Rows, AArea.Top, AArea.Bottom,
        AStylePattern)
    else
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.
  CalculateCellStylesInCustomArea(const AArea: TRect;
AStylePattern: TdxSpreadSheetCellStyle);
var
  ACell: TdxSpreadSheetCell;
...
procedure TdxSpreadSheetTableViewMergedCellStyleHelper.
  CalculateCellStylesInEntireColumnOrRow(AItems: TdxSpreadSheetTableItems;
...
begin
  ...
  begin
    ...
    if not TdxSpreadSheetContainersAccess(View.Containers)
      .FindCommentContainerCore(ActiveCell.RowIndex, ActiveCell.ColumnIndex,
      AComment) then
    begin
      if TdxSpreadSheetContainersAccess(View.Containers)
        .FindCommentContainerCore(NonBlankCell.RowIndex,
        NonBlankCell.ColumnIndex, AComment) then
  ...
  TdxSpreadSheetTableViewAccess(View).ClearCells(MergeCell.Area,
    cxRectBounds(MergeCell.Area.TopLeft, 0, 0), [ccoComments]);
end;

function TdxSpreadSheetTableViewMergedCellStyleHelper.GetActiveCell
  : TdxSpreadSheetCell;
begin
  Result := MergeCell.ActiveCell;
end;

function TdxSpreadSheetTableViewMergedCellStyleHelper.GetDefaultStyle
  : TdxSpreadSheetCellStyle;
begin
  Result := SpreadSheet.DefaultCellStyle;
end;

function TdxSpreadSheetTableViewMergedCellStyleHelper.GetStylePattern
  : TdxSpreadSheetCellStyle;
begin
  if NonBlankCell <> nil then
    Result := NonBlankCell.Style
  else if ActiveCell <> nil then
...
constructor TdxSpreadSheetTableViewSortingHelper.Create
  (AView: TdxSpreadSheetTableView);
begin
  inherited Create(AView);
...
procedure TdxSpreadSheetTableViewSortingHelper.SortByColumnValues
  (const AArea: TRect; const ASortOrders: array of TdxSortOrder;
const AColumnIndexes: array of Integer);
var
  AIndex, ARow: Integer;
begin
...
procedure TdxSpreadSheetTableViewSortingHelper.SortByRowValues
  (const AArea: TRect; const ASortOrders: array of TdxSortOrder;
const ARowIndexes: array of Integer);
var
...
function TdxSpreadSheetTableViewSortingHelper.Compare(AItem1,
...
function TdxSpreadSheetTableViewSortingHelper.CompareCells(AData1,
  AData2: TdxSpreadSheetCellData): Integer;
begin
  if AData1 = AData2 then
    Result := 0
  else
...
function TdxSpreadSheetTableViewSortingHelper.CompareValues(const AValue1,
  AValue2: Variant): Integer;
begin
  Result := VarCompare(AValue1, AValue2);
end;

procedure TdxSpreadSheetTableViewSortingHelper.SwapValues(AValue1,
  AValue2: TdxSpreadSheetCellData);
begin
...
procedure TdxSpreadSheetTableViewSortingHelper.DoSort(AFromIndex, AToIndex,
  ASortIndex: Integer);
var
  AValues: TcxObjectList;
  AIndex, AIndex2: Integer;
begin
  if (AFromIndex = AToIndex) or (ASortIndex >= SortOrders.Count) then
    Exit;
  ...
  try
    ...
    begin
      AIndex2 := AIndex;
      while (AIndex2 < AToIndex) and
        (CompareCells(TdxSpreadSheetCellData(AValues[AIndex - AFromIndex]),
...
procedure TdxSpreadSheetTableViewSortingHelper.ExchangeColumnValues(AColumn1,
  AColumn2: Integer);
var
  ARow: Integer;
begin
  for ARow := Area.Top to Area.Bottom do
    TdxSpreadSheetTableViewAccess(View).ExchangeValues(ARow, AColumn1, ARow,
      AColumn2);
end;

procedure TdxSpreadSheetTableViewSortingHelper.ExchangeRowValues(ARow1,
  ARow2: Integer);
var
  AColumn: Integer;
begin
  for AColumn := Area.Left to Area.Right do
    TdxSpreadSheetTableViewAccess(View).ExchangeValues(ARow1, AColumn,
      ARow2, AColumn);
end;

function TdxSpreadSheetTableViewSortingHelper.GetCellData(ARow,
  AColumn: Integer): TdxSpreadSheetCellData;
var
  AValue: Variant;
  ACell: TdxSpreadSheetCell;
begin
...
function TdxSpreadSheetTableViewSortingHelper.Initialize(AIndex1,
  AIndex2: Integer; const ASortOrders: array of TdxSortOrder;
const AIndexes: array of Integer): Boolean;
var
  AIndex: Integer;
begin
  ...
  begin
    ...
    begin
      if AIndex > AIndex2 - AIndex1 then
        Break;
      ...
      else if Length(AIndexes) = 0 then
...
function TdxSpreadSheetTableViewSortingHelper.IsColumnEmpty
...
function TdxSpreadSheetTableViewSortingHelper.IsRowEmpty
  (AIndex: Integer): Boolean;
var
  AColumn: Integer;
  ACell: TdxSpreadSheetCell;
...
procedure TdxSpreadSheetTableViewSortingHelper.SortValues
  (const AValues: TcxObjectList; L, H: Integer);
var
  I, J: Integer;
  Middle: TObject;
begin
  repeat
    ...
    repeat
      while Compare(AValues[I], Middle) < 0 do
        Inc(I);
      while Compare(AValues[J], Middle) > 0 do
        Dec(J);
      if I <= J then
      begin
        if I <> J then
        begin
          ...
          SwapValues(TdxSpreadSheetCellData(AValues[I]),
            TdxSpreadSheetCellData(AValues[J]));
        end;
        Inc(I);
        Dec(J);
      end;
...
function TdxSpreadSheetTableViewSortingHelper.ValidateArea
  (const AArea: TRect): Boolean;
begin
...
constructor TdxSpreadSheetTableViewEnumCellStylesHelper.Create
...
procedure TdxSpreadSheetTableViewEnumCellStylesHelper.PrepareToSave
  (AArea: TRect);
var
  AColumnIndex: Integer;
...
begin
  ...
  try
    ...
    if dxSpreadSheetIsEntireColumn(ARect) and dxSpreadSheetIsEntireRow(ARect)
    then
    begin
    ...
    Sheet.ForEachCell(ARect, procedure(ACell: TdxSpreadSheetCell)
      begin
...
begin
  TdxSpreadSheetTableColumnsAccess(Sheet.Columns).ForEach(
    ...
    end, AArea.Left, AArea.Right, True);
end;
...
begin
    ...
    begin
      ...
      begin
        if ARow.Cells[AColumn.Index] <> nil then
          Inc(ACellsCount)
        else if not(ARow.Style.IsDefault and AColumn.Style.IsDefault) then
        begin
  ...
  FHasEntireRowWithDefaultStyle := FHasEntireRowWithDefaultStyle or
    (ARowCount <> R.Height + 1);
...
begin
  ...
  TdxSpreadSheetTableRowsAccess(Sheet.Rows).ForEach(
    ...
    begin
      ...
      FHasEntireRowWithDefaultStyle := FHasEntireRowWithDefaultStyle or
        ARow.Style.IsDefault;
      Inc(ACount);
    end, AArea.Top, AArea.Bottom, True);

  FHasEntireRowWithDefaultStyle := FHasEntireRowWithDefaultStyle or
    (ACount <> AArea.Height + 1);
end;
...
begin
  ...
  FCrossStyles := TObjectDictionary < TdxSpreadSheetCellStyleHandle,
    TList < TdxSpreadSheetCellStyleHandle >>.Create([doOwnsValues]);
end;
...
function TdxSpreadSheetTableViewEnumCellStylesHelper.TCache.Add
  (AStyle: TdxSpreadSheetCellStyleHandle): Boolean;
begin
...
function TdxSpreadSheetTableViewEnumCellStylesHelper.TCache.Add(ARowStyle,
  AColumnStyle: TdxSpreadSheetCellStyleHandle): Boolean;
var
...
begin
  View.ForEachCell(procedure(ACell: TdxSpreadSheetCell)
    begin
...
procedure TdxSpreadSheetTableViewPackHelper.RemoveUnusedItems
...
constructor TdxSpreadSheetPasteCellDataFromStreamHelper.Create
  (AStream: TStream);
var
  AReader: TcxReader;
  ASection: Integer;
...
AContainer: TdxSpreadSheetContainer; AMode: TdxSpreadSheetClipboardCopyMode;
AView: TdxSpreadSheetTableView);
begin
  raise EdxSpreadSheetError.Create('Unavailable');
end;

procedure TdxSpreadSheetPasteCellDataFromStreamHelper.RestoreCells
  (AView: TdxSpreadSheetTableView;
AStyles: TList<TdxSpreadSheetCellStyleHandle>; const AArea: TRect;
  ...
  try
    for ARow := AArea.Top to AArea.Bottom do
      ...
      begin
        ...
        begin
          AHyperlink := TdxSpreadSheetHyperlinkAccess
            (AView.Hyperlinks.Add(cxInvalidRect));
...
class procedure TdxSpreadSheetDefinedNameHelper.Enum
  (AView: TdxSpreadSheetCustomView; AProc: TdxSpreadSheetDefinedNameEnumProc);
var
  ADefinedName: TdxSpreadSheetDefinedName;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((ADefinedName.Scope = nil) or (ADefinedName.Scope = AView)) and
        ADefinedName.IsCellReference then
      begin
...
class function TdxSpreadSheetDefinedNameHelper.FindByArea
  (ADefinedNames: TdxSpreadSheetDefinedNames; AView: TdxSpreadSheetCustomView;
const AArea: TRect; out ADefinedName: TdxSpreadSheetDefinedName): Boolean;
var
...
begin
  ...
  begin
    ...
    if cxRectIsEqual(GetBoundingArea(ADefinedName, AScope), AArea) and
      (AScope = AView) then
    begin
      if not IsInternalDefinedName(ADefinedName) then
        Exit(True);
...
class function TdxSpreadSheetDefinedNameHelper.GetBoundingArea
  (ADefinedName: TdxSpreadSheetDefinedName): TRect;
var
...
class function TdxSpreadSheetDefinedNameHelper.GetBoundingArea
  (ADefinedName: TdxSpreadSheetDefinedName;
out AView: TdxSpreadSheetCustomView): TRect;
var
...
begin
  ...
  ADefinedName.EnumReferences(procedure(const Area: TRect; View: TObject)
    begin
...
class function TdxSpreadSheetDefinedNameHelper.GetHeaders
  (AView: TdxSpreadSheetTableView; const AArea: TRect): TcxBorders;

  function Check(ADefinedNames: TdxSpreadSheetDefinedNamesAccess;
  ARow, AColumn: Integer): Boolean;
  var
    ACell: TdxSpreadSheetCell;
  begin
    ...
    Result := (ACell <> nil) and (ACell.DataType = cdtString) and
      ...
      ADefinedNames.IsCaptionValid(ACell.AsString +
      ADefinedNames.AutoNameSuffix) or ADefinedNames.IsCaptionValid
      (ReplaceIllegalChars(ACell.AsString, ADefinedNames.SpreadSheet)));
  end;
...
begin
  Result := [];
  ADefinedNames := TdxSpreadSheetDefinedNamesAccess
    (AView.SpreadSheet.DefinedNames);
  ...
  begin
    if Check(ADefinedNames, AArea.Top, AArea.Left + Ord(AAreaWidth > 1)) then
      Include(Result, bTop)
    else if Check(ADefinedNames, AArea.Bottom, AArea.Left + Ord(AAreaWidth > 1))
    then
  ...
  begin
    ...
    else if Check(ADefinedNames, AArea.Bottom - Ord(AAreaHeight > 1),
      AArea.Right) then
    begin
...
class function TdxSpreadSheetDefinedNameHelper.GetHeaders
  (AView: TdxSpreadSheetTableView; const AAreas: TdxRectList): TcxBorders;
var
  I: Integer;
begin
...
class function TdxSpreadSheetDefinedNameHelper.GetUsedAreas
  (ADefinedName: TdxSpreadSheetDefinedName; out AView: TdxSpreadSheetCustomView)
  : TdxRectList;
var
  ARanges: TdxRectList;
  ATargetView: TdxSpreadSheetCustomView;
begin
  ...
  ADefinedName.EnumReferences(procedure(const AArea: TRect; AView: TObject)
    begin
      if ATargetView = nil then
...
class function TdxSpreadSheetDefinedNameHelper.IsInternalDefinedName
  (ADefinedName: TdxSpreadSheetDefinedName): Boolean;
begin
  Result := StartsText('_xlnm._', ADefinedName.Caption);
end;

class function TdxSpreadSheetDefinedNameHelper.ReplaceIllegalChars
  (const ACaption: string; ASpreadSheet: TdxCustomSpreadSheet): string;
var
  ABreakChars: string;
...
begin
  AFormulaParser := TdxSpreadSheetFormulaControllerAccess(Controller)
    .CreateParser as TdxSpreadSheetFormulaParser;
  try
...
function TdxSpreadSheetExpression.GetController
  : TdxSpreadSheetCustomFormulaController;
begin
  Result := FController;
end;
...
constructor TdxSpreadSheetExpressionController.Create
...
function TdxSpreadSheetExpressionController.GetFormatSettings
