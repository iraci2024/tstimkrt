...
uses
  Types, Windows, SysUtils, Generics.Defaults, Generics.Collections, cxClasses,
  dxHashUtils,
...
type
  ...
  TdxSpreadSheetAutoFillingDataType = (afdtText, afdtTextWithNumbers,
    afdtDateTime, afdtNumeric, afdtFormula);
  ...
  TdxSpreadSheetAutoFillingPatternNumericSequence = class
    (TdxSpreadSheetAutoFillingPattern)
  protected
    FIndex: Integer;
    FReversed: Boolean;
  ...
  TdxSpreadSheetAutoFillingPatternNumericInTextSequence = class
    (TdxSpreadSheetAutoFillingPatternNumericSequence)
  protected
  ...
  TdxSpreadSheetAutoFillingPatternDateTimeSequence = class
    (TdxSpreadSheetAutoFillingPattern)
  protected type
  ...
  protected
    ...
    procedure CalculateDistance(AView: TdxSpreadSheetTableView;
      const ASourceArea: TRect; ADirection: TcxDirection); virtual;
  ...
  public
    constructor Create(const AArea: TRect;
      APattern: TdxSpreadSheetAutoFillingPattern); virtual;
    destructor Destroy; override;
    procedure ApplyTo(ACell: TdxSpreadSheetCell;
      const APatternPosition: TPoint); virtual;
    //
  ...
  protected
    ...
    procedure ValidatePatterns(ARangeStartIndex, ARangeFinishInteger: Integer;
      AIsMultipleSourceArea: Boolean);

    function CreatePattern(const AArea: TRect)
      : TdxSpreadSheetAutoFillingPattern; virtual;
    function CreateRange(const AArea: TRect;
      APattern: TdxSpreadSheetAutoFillingPattern)
      : TdxSpreadSheetAutoFillingRange; virtual;

    function FindRange(const P: TPoint;
      out ARange: TdxSpreadSheetAutoFillingRange): Boolean;
    function GetCellInfo(ACell: TdxSpreadSheetCell)
      : TdxSpreadSheetAutoFillingCellInfo; virtual;
    function GetCellPlacementInPattern(ACell: TdxSpreadSheetCell)
      : TPoint; virtual;
  ...
  protected
    ...
    // Filling
    function CalculateFillingArea(const ASourceArea: TRect;
      ADirection: TcxDirection; ACount: Integer): TRect;
    function CreateRangeList: TdxSpreadSheetAutoFillingRanges; virtual;
    procedure FillCore(const ATargetArea: TRect; ADirection: TcxDirection;
      ARanges: TdxSpreadSheetAutoFillingRanges);
  public
    procedure Fill(ASourceArea: TRect; ADirection: TcxDirection;
      ACount: Integer);
  end;

  { TdxSpreadSheetTableViewFillCellsHelper }

  TdxSpreadSheetTableViewFillCellsHelper = class
    (TdxSpreadSheetCustomTableViewHelper)
  strict private
    procedure PrepareAreasForFilling(const ASourceArea: TRect; AToDown: Boolean;
      var AMasterArea, ADestArea: TRect);
  public
...
uses
  DateUtils, Math, cxDateUtils, dxCore, dxCoreClasses, cxGeometry,
  dxTypeHelpers,
  dxSpreadSheetClipboard, dxSpreadSheetClipboardFormats, dxSpreadSheetStrs,
...
function GetDigitsPlaceholder(const S: string;
  out AStart, AFinish: Integer): Boolean;

  function Check(AStartIndex, AFinishIndex: Integer;
    out AStart, AFinish: Integer): Boolean;
  var
    ADelta: Integer;
  begin
    ...
    begin
      ...
      while (AStartIndex <> AFinishIndex) and dxWideIsNumeric(S[AStartIndex]) do
        Inc(AStartIndex, ADelta);
      AFinish := AStartIndex;
      if (AStartIndex <> AFinishIndex) or not dxWideIsNumeric(S[AStartIndex])
      then
        Dec(AFinish, ADelta);
...
begin
  Result := (S <> '') and (Check(1, Length(S), AStart, AFinish) and
    ((AFinish = Length(S)) or dxWideIsSpace(S[AFinish + 1])) or Check(Length(S),
    1, AStart, AFinish));
end;

{ TdxSpreadSheetAutoFillingCellInfo }

constructor TdxSpreadSheetAutoFillingCellInfo.Create
  (ADataType: TdxSpreadSheetAutoFillingDataType);
begin
  DataType := ADataType;
end;

constructor TdxSpreadSheetAutoFillingCellInfo.Create(const S: string;
...
function TdxSpreadSheetAutoFillingCellInfo.Compare
  (const AInfo: TdxSpreadSheetAutoFillingCellInfo): Boolean;
begin
  Result := DataType = AInfo.DataType;
  if Result and (DataType = afdtTextWithNumbers) then
    Result := (PatternPrefixPart = AInfo.PatternPrefixPart) and
      (PatternSuffixPart = AInfo.PatternSuffixPart);
end;
...
constructor TdxSpreadSheetAutoFillingPatternNumericSequence.Create
  (AView: TdxSpreadSheetTableView; const ASourceArea: TRect;
  ADirection: TcxDirection);
begin
  inherited Create;
  FReversed := ADirection in [dirLeft, dirUp];

  AView.ForEachCell(ASourceArea, procedure(ACell: TdxSpreadSheetCell)
    begin
      AccumulateValue(ExtractValue(ACell));
    end, ADirection in [dirRight, dirDown]);
end;

procedure TdxSpreadSheetAutoFillingPatternNumericSequence.CalculateValue
  (ACell: TdxSpreadSheetCell);
begin
  ACell.AsFloat := GenerateValue;
end;

procedure TdxSpreadSheetAutoFillingPatternNumericSequence.AccumulateValue
  (const AValue: Double);
begin
  FSumX := FSumX + FIndex;
  FSumY := FSumY + AValue;
...
function TdxSpreadSheetAutoFillingPatternNumericSequence.ExtractValue
  (ACell: TdxSpreadSheetCell): Double;
begin
...
constructor TdxSpreadSheetAutoFillingPatternNumericInTextSequence.Create
  (AView: TdxSpreadSheetTableView; const ASourceArea: TRect;
ADirection: TcxDirection; const AInfo: TdxSpreadSheetAutoFillingCellInfo);
begin
...
procedure TdxSpreadSheetAutoFillingPatternNumericInTextSequence.CalculateValue
  (ACell: TdxSpreadSheetCell);
var
  AValue: Double;
  AValueAsInt: Integer;
begin
  ...
  begin
    ...
    ACell.AsString := Format('%s%d%s', [FPatternPrefixPart, AValueAsInt,
      FPatternSuffixPart]);
  end;
end;

function TdxSpreadSheetAutoFillingPatternNumericInTextSequence.
  CheckIsQuarterTemplate: Boolean;

  function Check(const S: string): Boolean;
  begin
    Result := (S <> '') and (SameText(cxGetResourceString(@sdxQuarter), S) or
...
function TdxSpreadSheetAutoFillingPatternNumericInTextSequence.ExtractValue
  (ACell: TdxSpreadSheetCell): Double;
var
  AValue: string;
begin
  AValue := ACell.AsString;
  AValue := Copy(AValue, FPatternPrefixPartLength + 1,
    Length(AValue) - FPatternSuffixPartLength - FPatternPrefixPartLength);
...
procedure TdxSpreadSheetAutoFillingPatternDateTimeSequence.CalculateValue
  (ACell: TdxSpreadSheetCell);
var
  AValue: Integer;
  Y, M, D: Word;
begin
  ...
  begin
    ...
    ACell.AsDateTime := EncodeDate(Y, M, D) + TimeOf(FInitialDate) + FIndex *
      FDistance.MilliSeconds / MSecsPerDay;
    Inc(FIndex);
  end;
end;
...
begin
  ...
  begin
    if dtpDate in GetDateTimeParts(AView.Cells[ASourceArea.Top,
      ASourceArea.Left]) then
      FDistance.Days := 1
    else
...
function TdxSpreadSheetAutoFillingPatternDateTimeSequence.GetDateTime
  (ACell: TdxSpreadSheetCell): TDateTime;
begin
...
function TdxSpreadSheetAutoFillingPatternDateTimeSequence.GetDateTimeParts
  (ACell: TdxSpreadSheetCell): TDateTimeParts;
...
begin
  Result := [];
  if GetFormattedValue(EncodeDate(2000, 1, 1)) <>
    GetFormattedValue(EncodeDate(2001, 2, 2)) then
    Include(Result, dtpDate);
  if GetFormattedValue(EncodeTime(1, 1, 1, 1)) <>
    GetFormattedValue(EncodeTime(2, 2, 2, 2)) then
    Include(Result, dtpTime);
end;

function TdxSpreadSheetAutoFillingPatternDateTimeSequence.GetDistance
  (const AValue1, AValue2: TDateTime): TDistance;
var
  AYear1, AMonth1, ADay1: Word;
  AYear2, AMonth2, ADay2: Word;
begin
  Result.MilliSeconds := Sign(AValue2 - AValue1) * MilliSecondsBetween
    (TimeOf(AValue2), TimeOf(AValue1));
  Result.Days := Trunc(AValue2) - Trunc(AValue1);

  DecodeDate(AValue1, AYear1, AMonth1, ADay1);
  DecodeDate(AValue2, AYear2, AMonth2, ADay2);
  if (ADay1 = ADay2) or (ADay1 = DaysInAMonth(AYear1, AMonth1)) and
    (ADay2 = DaysInAMonth(AYear2, AMonth2)) then
...
procedure TdxSpreadSheetAutoFillingPatternDateTimeSequence.TDistance.Combine
  (const ASource: TDistance);
begin
...
function TdxSpreadSheetAutoFillingPatternDateTimeSequence.TDistance.
  IsAssigned: Boolean;
begin
...
constructor TdxSpreadSheetAutoFillingRange.Create(const AArea: TRect;
APattern: TdxSpreadSheetAutoFillingPattern);
begin
  inherited Create;
  FArea := AArea;
...
procedure TdxSpreadSheetAutoFillingRange.ApplyTo(ACell: TdxSpreadSheetCell;
const APatternPosition: TPoint);
var
...
begin
  ...
  ASourceCellArea := ACell.View.MergedCells.ExpandArea(APatternPosition.X,
    APatternPosition.Y);
  if not dxSpreadSheetIsSingleCellArea(ASourceCellArea) and
    cxPointIsEqual(ASourceCellArea.TopLeft, APatternPosition) then
    ACell.View.MergedCells.Add(cxRectSetOrigin(ASourceCellArea,
      Point(ACell.ColumnIndex, ACell.RowIndex)));
...
constructor TdxSpreadSheetAutoFillingRanges.Create
...
procedure TdxSpreadSheetAutoFillingRanges.Initialize(const ASourceArea: TRect;
ADirection: TcxDirection);
begin
...
function TdxSpreadSheetAutoFillingRanges.CreatePattern(const AArea: TRect)
  : TdxSpreadSheetAutoFillingPattern;
var
...
begin
  ...
  case AInfo.DataType of
    afdtDateTime:
      Result := TdxSpreadSheetAutoFillingPatternDateTimeSequence.Create(View,
        AArea, Direction);
    afdtTextWithNumbers:
      Result := TdxSpreadSheetAutoFillingPatternNumericInTextSequence.Create
        (View, AArea, Direction, AInfo);
    afdtNumeric:
      Result := TdxSpreadSheetAutoFillingPatternNumericSequence.Create(View,
        AArea, Direction);
  end;
end;

function TdxSpreadSheetAutoFillingRanges.CreateRange(const AArea: TRect;
APattern: TdxSpreadSheetAutoFillingPattern): TdxSpreadSheetAutoFillingRange;
begin
  Result := TdxSpreadSheetAutoFillingRange.Create(AArea, APattern);
end;

function TdxSpreadSheetAutoFillingRanges.FindRange(const P: TPoint;
out ARange: TdxSpreadSheetAutoFillingRange): Boolean;
var
  I: Integer;
begin
...
procedure TdxSpreadSheetAutoFillingRanges.Add(const AFirstCellInRange,
  ALastCellInRange: TPoint);
begin
  Add(cxRect(AFirstCellInRange, ALastCellInRange));
end;

procedure TdxSpreadSheetAutoFillingRanges.ValidatePatterns(ARangeStartIndex,
  ARangeFinishInteger: Integer; AIsMultipleSourceArea: Boolean);

  function HasSimilarPatternsInArea(APattern
    : TdxSpreadSheetAutoFillingPattern): Boolean;
  var
    ARange: TdxSpreadSheetAutoFillingRange;
    I: Integer;
  begin
    ...
    begin
      ARange := FList[I];
      if (ARange.Pattern <> nil) and (ARange.Pattern <> APattern) and
        APattern.InheritsFrom(ARange.Pattern.ClassType) then
        Exit(True);
    end;
    Result := False;
  end;

  function ShouldDeletePatternOfRange
    (ARange: TdxSpreadSheetAutoFillingRange): Boolean;
  begin
    Result := (ARange.Pattern <> nil) and not ARange.Pattern.IsLoner and
      IsSingleCell(ARange);
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ARange := FList[I];
        if ShouldDeletePatternOfRange(ARange) and
          not HasSimilarPatternsInArea(ARange.Pattern) then
          AList.Add(ARange);
      end;
  ...
  else if ARangeStartIndex = ARangeFinishInteger then
  begin
...
function TdxSpreadSheetAutoFillingRanges.GetCellInfo(ACell: TdxSpreadSheetCell)
  : TdxSpreadSheetAutoFillingCellInfo;
const
...
begin
  ...
  else if (ACell.DataType = cdtDateTime) or (ACell.DataType in NumericDataTypes)
    and ACell.Style.DataFormat.IsDateTime then
    Result := TdxSpreadSheetAutoFillingCellInfo.Create(afdtDateTime)
  else if ACell.DataType in NumericDataTypes then
    Result := TdxSpreadSheetAutoFillingCellInfo.Create(afdtNumeric)
  else if (ACell.DataType = cdtString) and GetDigitsPlaceholder(ACell.AsString,
    AStart, AFinish) then
    Result := TdxSpreadSheetAutoFillingCellInfo.Create(ACell.AsString,
      AStart, AFinish)
  else
    Result := TdxSpreadSheetAutoFillingCellInfo.Create(afdtText);
end;

function TdxSpreadSheetAutoFillingRanges.GetCellPlacementInPattern
  (ACell: TdxSpreadSheetCell): TPoint;
begin
...
function TdxSpreadSheetAutoFillingRanges.IsSingleCell
  (ARange: TdxSpreadSheetAutoFillingRange): Boolean;
var
  R: TRect;
begin
  ...
  begin
    ...
    if Direction in [dirLeft, dirRight] then
      Result := Result or (R.Left = ARange.Area.Left) and
        (R.Right = ARange.Area.Right)
    else
      Result := Result or (R.Top = ARange.Area.Top) and
        (R.Bottom = ARange.Area.Bottom);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SplitAreaProcessCell(ARow, AColumn, AFirstCellInRange, ALastCellInRange,
          APrevCellInfo);
        AColumn := View.MergedCells.ExpandArea(AColumn, ARow).Right + 1;
      end;
      Add(AFirstCellInRange, ALastCellInRange);
      ValidatePatterns(AStartIndex, FList.Count - 1,
        dxSpreadSheetAreaHeight(SourceArea) > 1);
      AFirstCellInRange := cxInvalidPoint;
    end;
  end
  else
    ...
    begin
      ...
      Add(AFirstCellInRange, ALastCellInRange);
      ValidatePatterns(AStartIndex, FList.Count - 1,
        dxSpreadSheetAreaWidth(SourceArea) > 1);
      AFirstCellInRange := cxInvalidPoint;
    end;
end;

procedure TdxSpreadSheetAutoFillingRanges.SplitAreaProcessCell(ARow,
  AColumn: Integer; var AFirstCellInRange, ALastCellInRange: TPoint;
var APrevCellInfo: TdxSpreadSheetAutoFillingCellInfo);
  ...
  begin
    Result := (AInfo.DataType in [afdtText, afdtFormula]) or
      IsValid(AFirstCellInRange) and not APrevCellInfo.Compare(AInfo);
  end;
...
procedure TdxSpreadSheetTableViewAutoFillingHelper.Fill(ASourceArea: TRect;
...
begin
  ...
  if (ADirection <> dirNone) and (ACount > 0) and
    dxSpreadSheetIsValidArea(ASourceArea) then
  begin
...
function TdxSpreadSheetTableViewAutoFillingHelper.CreateRangeList
  : TdxSpreadSheetAutoFillingRanges;
begin
  Result := TdxSpreadSheetAutoFillingRanges.Create(View);
end;

procedure TdxSpreadSheetTableViewAutoFillingHelper.FillCore(const ATargetArea
  : TRect; ADirection: TcxDirection; ARanges: TdxSpreadSheetAutoFillingRanges);
var
  APosition, I: Integer;
begin
  if ADirection in [dirLeft, dirRight] then
  begin
    APosition := IfThen(ADirection = dirLeft, ATargetArea.Right,
      ATargetArea.Left);
  ...
  begin
    APosition := IfThen(ADirection = dirUp, ATargetArea.Bottom,
      ATargetArea.Top);
...
procedure TdxSpreadSheetTableViewFillCellsHelper.FillCells(const AArea: TRect;
AToDown: Boolean);
var
...
begin
  ...
  if dxSpreadSheetIsValidArea(AMasterArea) and
    dxSpreadSheetIsValidArea(ADestArea) then
  begin
    CheckProtection(ADestArea);
    ASelection := TdxRectList.Create;
    dxSpreadSheetStoreSelectionAndScrollPosition(View, ASelection, AFocusedCell,
      AScrollPos);
    try
      AData := TdxSpreadSheetBinaryClipboardFormat.Build(AMasterArea, nil,
        ccmCopy, View);

      History.BeginAction(TdxSpreadSheetHistoryFillCellsAction);
      try
        View.MergedCells.DeleteItemsInArea(ADestArea);
        TdxSpreadSheetTableViewAccess(View).ClearCells(ADestArea,
          dxSpreadSheetTableViewClearCellsOptionsAll);
        ...
        begin
          ...
          AData.Paste(View, View.Selection.Area.TopLeft,
            dxSpreadSheetDefaultPasteOptions);
        end;
      finally
        History.EndAction;
      end;
    finally
      dxSpreadSheetRestoreSelectionAndScrollPosition(View, ASelection,
...
procedure TdxSpreadSheetTableViewFillCellsHelper.PrepareAreasForFilling
  (const ASourceArea: TRect; AToDown: Boolean;
var AMasterArea, ADestArea: TRect);
begin
