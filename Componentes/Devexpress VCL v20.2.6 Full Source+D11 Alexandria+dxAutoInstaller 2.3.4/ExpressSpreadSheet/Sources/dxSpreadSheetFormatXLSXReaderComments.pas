...
type
  ...
  TdxSpreadSheetXLSXReaderCommentsParser = class
  ...
  TdxSpreadSheetXLSXReaderCommentContainerParser = class
  ...
  strict private
    ...
    function GetNodeValueAsBoolean(ANode: TdxXMLNode;
      ADefaultValueForEmptyValue, ADefaultValueForNonExistsNode: Boolean)
      : Boolean; overload;
    function GetNodeValueAsBoolean(AParentNode: TdxXMLNode;
      const ANode: TdxXMLString; ADefaultValueForEmptyValue,
      ADefaultValueForNonExistsNode: Boolean): Boolean; overload;
  ...
  protected
    ...
    procedure ReadStyles(const AStyles: string); virtual;
...
uses
  dxSpreadSheetFormatXLSXTags, dxSpreadSheetFormatUtils, Math,
  dxSpreadSheetFormatXLSXReaderDrawing,
  dxSpreadSheetCoreStyles, dxSVGCoreParsers, dxSVGCore;
...
constructor TdxSpreadSheetXLSXReaderCommentsParser.Create(const AFileName
...
var
  ANode: TdxXMLNode;
begin
  if Document.FindChild([sdxXLSXNodeComments, sdxXLSXNodeCommentList], ANode)
  then
    ANode.ForEach(ProcessComment);
end;

procedure TdxSpreadSheetXLSXReaderCommentsParser.ProcessComment
...
begin
  AContainer := TdxSpreadSheetCommentContainer
    (View.Containers.AddCommentContainer
    (View.CreateCell(ANode.Attributes.GetValueAsString(sdxXLSXAttrRef))));
  AContainer.Author :=
    ReadAuthor(ANode.Attributes.GetValueAsInteger(sdxXLSXAttrAuthorId, -1));
  AContainer.Visible := False;
  if ANode.FindChild(sdxXLSXNodeTextFull, ATextNode) then
  begin
    AString := TdxSpreadSheetXLSXReaderSharedStringParser.ParseString(Owner,
      ATextNode);
    AContainer.TextBox.Text := AString;

    if AString is TdxSpreadSheetFormattedSharedString then
    begin
      if TdxSpreadSheetFormattedSharedString(AString).Runs.Count > 0 then
        AContainer.TextBox.Font.Handle := TdxSpreadSheetFormattedSharedString
          (AString).Runs[0].FontHandle;
    end;
  end;
end;

function TdxSpreadSheetXLSXReaderCommentsParser.ReadAuthor
  (AAuthorID: Integer): string;
var
  ANode: TdxXMLNode;
begin
  Result := '';
...
constructor TdxSpreadSheetXLSXReaderCommentContainerParser.Create
  (ANode: TdxXMLNode;
  AOwnerParser: TdxSpreadSheetXLSXReaderCommentContainersParser;
  AContainer: TdxSpreadSheetCommentContainer);
begin
...
var
  ASubNode: TdxXMLNode;
begin
  ReadStyles(Node.Attributes.GetValueAsString(sdxXLSXNodeStyle));
...
function TdxSpreadSheetXLSXReaderCommentContainerParser.DecodeColor(S: string;
  ADefaultColor: TColor = clBlack): TColor;
var
  AValue: string;
begin
  if Pos(' ', S) > 0 then
...
function TdxSpreadSheetXLSXReaderCommentContainerParser.DecodeOpacity
  (const S: string): Byte;
var
  ALength: Integer;
begin
...
function TdxSpreadSheetXLSXReaderCommentContainerParser.ReadAlphaColor
  (ANode: TdxXMLNode; const AAttrName: TdxXMLString;
  const AOpacityAttrName: TdxXMLString = ''; AOpacityNode: TdxXMLNode = nil;
  const ADefaultColor: TColor = clBlack): TdxAlphaColor;
var
  AOpacity: Byte;
begin
  ...
  begin
    if AOpacityNode = nil then
      AOpacityNode := ANode;
    AOpacity := DecodeOpacity(AOpacityNode.Attributes.GetValueAsString
      (AOpacityAttrName));
  end;
  Result := dxColorToAlphaColor
    (DecodeColor(ANode.Attributes.GetValueAsString(AAttrName), ADefaultColor),
    AOpacity);
end;

function TdxSpreadSheetXLSXReaderCommentContainerParser.ReadGradientAngle
  (ANode: TdxXMLNode): Double;
var
  AFocus: Integer;
begin
  Result := dxNormalizeAngle(ANode.Attributes.GetValueAsInteger
    (sdxXLSXAttrAngle));
  AFocus := StrToIntDef(StringReplace(ANode.Attributes.GetValueAsString
    (sdxXLSXAttrFocus), '%', '', [rfReplaceAll]), 0);
...
procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadAnchorPoints
  (ANode: TdxXMLNode);

  function GetCell(const ARow, AColumn: string): TdxSpreadSheetCell;
  begin
    Result := TdxSpreadSheetTableView(Container.Parent)
      .CreateCell(StrToInt(ARow), StrToInt(AColumn));
  end;
...
procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadClientData
  (ANode: TdxXMLNode);
begin
  ReadClientDataAnchors(ANode);
  ReadClientDataTextBox(ANode);
  Container.Visible := GetNodeValueAsBoolean(ANode, sdxXLSXNodeXVisible, True,
    FVisible);
end;

procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadClientDataAnchors
  (ANode: TdxXMLNode);
  ...
  begin
    ...
    begin
      if GetNodeValueAsBoolean(ANode, sdxXLSXNodeXSizeWithCells, False, True)
      then
        Result := catTwoCell
      else
        Result := catOneCell;
...
begin
  ...
  ASavedBounds := TdxSpreadSheetContainerAccess(Container)
    .Calculator.CalculateBounds;
  Container.AnchorType := ReadAnchorType(ANode);
  TdxSpreadSheetContainerAccess(Container).Calculator.UpdateAnchors
    (ASavedBounds);
end;

procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadClientDataTextBox
  (ANode: TdxXMLNode);
begin
  Container.TextBox.AlignHorz :=
    TdxSpreadSheetXLSXHelper.StringToVMLTextAlignHorz
    (ANode.ChildValues[sdxXLSXNodeXTextHAlign]);
  Container.TextBox.AlignVert :=
    TdxSpreadSheetXLSXHelper.StringToVMLTextAlignVert
    (ANode.ChildValues[sdxXLSXNodeXTextVAlign]);
end;

procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadShapeFill
  (AFill: TdxGPBrush);
var
...
begin
  ...
  begin
    if ASubNode.Attributes.GetValue(sdxXLSXAttrTypeLC) = sdxXLSXValueGradient
    then
    begin
      AFill.Style := gpbsGradient;
      AFill.GradientPoints.Add(0, ReadAlphaColor(Node, sdxXLSXAttrFillColor,
        sdxXLSXAttrOpacity, ASubNode));
      AFill.GradientPoints.Add(1, ReadAlphaColor(ASubNode, sdxXLSXAttrColor2,
        sdxXLSXAttrOOpacity2));
      AFill.GradientMode := dxGetNearestGradientMode
        (ReadGradientAngle(ASubNode), AInvertOrder);
    ...
    begin
      if ReadEmbeddedImage(ASubNode.Attributes.GetValue(sdxXLSXAttrORelID),
        AStream) then
      begin
      ...
      if ASubNode.Attributes.GetValue(sdxXLSXAttrTypeLC) = sdxXLSXValuePattern
    then
    begin
      if ReadEmbeddedImage(ASubNode.Attributes.GetValue(sdxXLSXAttrORelID),
        AStream) then
      begin
        ...
        dxSpreadSheetInitializeBrushPattern(AFill, AFill.Texture,
          ...
          ReadAlphaColor(Node, sdxXLSXAttrFillColor, sdxXLSXAttrOpacity,
          ASubNode));
      end;
    end
    else
    begin
      AFill.Style := gpbsSolid;
      AFill.Color := ReadAlphaColor(Node, sdxXLSXAttrFillColor,
        sdxXLSXAttrOpacity, ASubNode, clInfoBk);
    end;
  end;
end;

procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadShapeStroke
  (AStroke: TdxGPPen);
var
...
begin
  ...
  if Node.FindChild(sdxXLSXNodeVMLStroke, ASubNode) then
    AStroke.Style := TdxSpreadSheetXLSXHelper.StringToVMLDashStyle
      (ASubNode.Attributes.GetValue(sdxXLSXAttrDashStyle));
end;

procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadStyles
  (const AStyles: string);
var
  AList: TStringList;
  APoint1, APoint2: TPoint;
  AValue: TdxSVGValue;
begin
  AList := SplitString(AStyles, ';');
  try
    ...
    begin
      ...
      if TdxSVGParserValue.Parse(AList.Values['margin-left'], AValue) then
        APoint1.X := Round(AValue.ToPixels);
      if TdxSVGParserValue.Parse(AList.Values['margin-top'], AValue) then
        APoint1.Y := Round(AValue.ToPixels);

      if TdxSVGParserValue.Parse(AList.Values['margin-right'], AValue) then
        APoint2.X := Round(AValue.ToPixels) - APoint1.X
      else if TdxSVGParserValue.Parse(AList.Values['width'], AValue) then
        APoint2.X := Round(AValue.ToPixels);

      if TdxSVGParserValue.Parse(AList.Values['margin-bottom'], AValue) then
        APoint2.Y := Round(AValue.ToPixels) - APoint1.Y
      else if TdxSVGParserValue.Parse(AList.Values['height'], AValue) then
        APoint2.Y := Round(AValue.ToPixels);

      Container.BeginUpdate;
      try
        Container.AnchorType := catAbsolute;
        Container.AnchorPoint1.Offset := APoint1;
        Container.AnchorPoint2.Offset := APoint2;
      finally
        Container.EndUpdate;
      end;
    end;

    FVisible := AList.Values[sdxXLSXAttrVisibility] = 'visible';
  finally
    AList.Free;
  end;
end;

procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadTextBox
  (ANode: TdxXMLNode);
begin
...
procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadTextBoxInset
  (const AInset: string);
var
...
procedure TdxSpreadSheetXLSXReaderCommentContainerParser.ReadTextBoxStyles
  (const AStyles: string);
var
...
begin
  ...
  try
    ...
    Container.TextBox.AutoSize :=
      Trim(AStyleList.Values[sdxXLSXMSOFitShapeToText]) = 't';
  finally
    AStyleList.Free;
  end;
end;
...
function TdxSpreadSheetXLSXReaderCommentContainerParser.GetNodeValueAsBoolean
  (ANode: TdxXMLNode; ADefaultValueForEmptyValue, ADefaultValueForNonExistsNode
  : Boolean): Boolean;
begin
  if ANode = nil then
...
function TdxSpreadSheetXLSXReaderCommentContainerParser.GetNodeValueAsBoolean
  (AParentNode: TdxXMLNode; const ANode: TdxXMLString;
  ADefaultValueForEmptyValue, ADefaultValueForNonExistsNode: Boolean): Boolean;
begin
  Result := GetNodeValueAsBoolean(AParentNode.FindChild(ANode),
    ADefaultValueForEmptyValue, ADefaultValueForNonExistsNode);
end;

function TdxSpreadSheetXLSXReaderCommentContainerParser.SplitString(const S,
  ASplitter: string): TStringList;
var
  I: Integer;
begin
...
constructor TdxSpreadSheetXLSXReaderCommentContainersParser.Create
  (const AFileName: AnsiString; AView: TdxSpreadSheetTableView;
  AOwner: TdxSpreadSheetXLSXReader);
begin
  inherited Create(AFileName, AOwner);
  FView := AView;
...
procedure TdxSpreadSheetXLSXReaderCommentContainersParser.ProcessShape
  (ANode: TdxXMLNode; AUserData: Pointer);
var
  ACellRef: TPoint;
  AContainer: TdxSpreadSheetCommentContainer;
...
begin
  ...
  if ANode.FindChild([sdxXLSXNodeXClientData, sdxXLSXNodeXColumn], ASubNode)
  then
  ...
  if View.Containers.FindCommentContainer(ACellRef.Y, ACellRef.X, AContainer)
  then
    ExecuteSubTask(TdxSpreadSheetXLSXReaderCommentContainerParser.Create(ANode,
      Self, AContainer));
end;

procedure TdxSpreadSheetXLSXReaderCommentContainersParser.ProcessSubNodes
  (ANode: TdxXMLNode; AUserData: Pointer);
begin
