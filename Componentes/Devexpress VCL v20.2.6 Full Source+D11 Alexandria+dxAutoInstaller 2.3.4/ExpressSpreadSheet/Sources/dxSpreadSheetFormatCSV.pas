...
uses
  Types, Variants, SysUtils, Classes, dxSpreadSheetCore, dxSpreadSheetClasses,
  dxCore, dxSpreadSheetTextFileFormatCore;
...
type
  ...
  public
    ...
    property Export: TdxSpreadSheetCSVFormatExportSettings read FExport
      write SetExport;
    property Quote: WideChar read FQuote write FQuote;
    property ValueSeparator: WideChar read FValueSeparator
      write FValueSeparator;
  end;
  ...
  public
    ...
    procedure Reset;
    //
    property QuoteStringValues: Boolean read FQuoteStringValues
      write FQuoteStringValues;
    property UseDisplayValues: Boolean read FUseDisplayValues
      write FUseDisplayValues;
  end;
  ...
  protected
    function CreateParser(const AData: string)
      : TdxSpreadSheetCSVCustomParser; virtual;
  ...
  public
    constructor Create(AView: TdxSpreadSheetTableView; AChars: PWideChar;
      ACount: Integer);
    destructor Destroy; override;
    //
    property KeepSourceNumberFormat: Boolean read FKeepSourceNumberFormat
      write FKeepSourceNumberFormat;
  ...
  TdxSpreadSheetCSVTableViewWriter = class
...
uses
  dxHashUtils, Math, dxSpreadSheetTypes, dxSpreadSheetUtils, cxClasses,
  dxSpreadSheetStrs, cxDateUtils,
  dxSpreadSheetStyles;
...
class function TdxSpreadSheetCSVFormat.CreateFormatSettings
...
class function TdxSpreadSheetCSVFormat.GetReader
  : TdxSpreadSheetCustomReaderClass;
begin
  Result := TdxSpreadSheetCSVReader;
end;

class function TdxSpreadSheetCSVFormat.GetWriter
...
procedure TdxSpreadSheetCSVFormatSettings.SetExport
  (AValue: TdxSpreadSheetCSVFormatExportSettings);
begin
...
begin
  ...
  begin
    QuoteStringValues := TdxSpreadSheetCSVFormatExportSettings(Source)
      .QuoteStringValues;
    UseDisplayValues := TdxSpreadSheetCSVFormatExportSettings(Source)
      .UseDisplayValues;
  end;
end;
...
constructor TdxSpreadSheetCSVCustomParser.Create(AChars: PWideChar;
  ACount: Integer);
begin
  inherited Create;
  FChars := AChars;
...
begin
  ...
  begin
    ...
    begin
      DoValueEnd;
      GoToNext;
      while (FCount > 0) and CharInSet(FChars^, [#9, ' ']) and
        (FChars^ <> Settings.ValueSeparator) do
        GoToNext;
      DoValueBegin;
    end
    else
...
procedure TdxSpreadSheetCSVCustomParser.DoValue(const AValue: string;
...
var
  ...
  AValue: string;
begin
  ACount := (TdxNativeUInt(FChars) - TdxNativeUInt(FValueCursor))
    div SizeOf(WideChar);
  if ACount > 0 then
  begin
    AIsQuotedValue := (FValueCursor^ = Settings.Quote) and
      ((FValueCursor + ACount - 1)^ = Settings.Quote);
    if AIsQuotedValue then
    begin
      Inc(FValueCursor);
      Dec(ACount, 2);
    end
    else if (FCount > 0) and (FChars^ = Settings.ValueSeparator) then
    begin
      while (ACount > 0) and CharInSet((FValueCursor + ACount - 1)^,
        [' ', #9]) do
        Dec(ACount);
    end;
    ...
    if FValueContainsQuote then
      AValue := StringReplace(AValue, Settings.Quote + Settings.Quote,
        Settings.Quote, [rfReplaceAll]);
...
begin
  ...
  begin
    ...
    AEncoding := nil;
    ASize := TEncoding.GetBufferEncoding(ABuffer, AEncoding{$IFDEF DELPHIXE},
      dxSpreadSheetCSVFormatSettings.Encoding{$ENDIF});
    Parse(AEncoding.GetString(ABuffer, ASize, Length(ABuffer) - ASize));
  end;
end;

function TdxSpreadSheetCSVReader.CreateParser(const AData: string)
  : TdxSpreadSheetCSVCustomParser;
begin
  Result := TdxSpreadSheetCSVReaderParser.Create(ProgressHelper,
    AddTableView(''), PWideChar(AData), Length(AData));
end;

function TdxSpreadSheetCSVReader.CreateProgressHelper
...
constructor TdxSpreadSheetCSVReaderCustomParser.Create
  (AView: TdxSpreadSheetTableView; AChars: PWideChar; ACount: Integer);
begin
...
function TdxSpreadSheetCSVReaderCustomParser.CreateFormatSettings
  : TdxSpreadSheetFormatSettings;
begin
...
procedure TdxSpreadSheetCSVReaderCustomParser.DoValue(const AValue: string;
  AIsQuotedValue: Boolean);
var
...
begin
  ...
  begin
    ...
    if not AIsQuotedValue and dxTryStrToOrdinal(AValue, AOrdinalValue,
...
constructor TdxSpreadSheetCSVReaderParser.Create(AProgressHelper
  : TdxSpreadSheetCustomFilerProgressHelper; AView: TdxSpreadSheetTableView;
  AChars: PWideChar; ACount: Integer);
begin
...
constructor TdxSpreadSheetCSVWriter.Create(AOwner: TdxCustomSpreadSheet;
  AStream: TStream);
begin
...
function TdxSpreadSheetCSVTableViewWriter.EncodeValue
...
begin
  Result := StringReplace(GetCellValue(ACell), Settings.Quote,
    Settings.Quote + Settings.Quote, [rfReplaceAll]);
  if Settings.Export.QuoteStringValues and IsStringValue(ACell) or
    HasProhibitedSymbols(Result) then
    Result := Settings.Quote + Result + Settings.Quote;
end;

procedure TdxSpreadSheetCSVTableViewWriter.WriteCell(ARowIndex,
...
function TdxSpreadSheetCSVTableViewWriter.GetSettings
  : TdxSpreadSheetCSVFormatSettings;
begin
