...
{$ENDIF}
  ...
  SysUtils, Math, Graphics, Variants,
  dxCore, dxOLEDocument, cxClasses, cxVariants, cxGraphics, dxSpreadSheetCore,
  dxCoreClasses, dxSpreadSheetTypes,
  dxSpreadSheetClasses, dxSpreadSheetFormatXLSTypes, dxSpreadSheetUtils,
  dxSpreadSheetGraphics, dxSpreadSheetFormulas,
  dxSpreadSheetFunctions, dxGDIPlusClasses, dxSpreadSheetFormatUtils,
  dxSpreadSheetPrinting, dxHashUtils, dxCoreGraphics,
  dxSpreadSheetConditionalFormattingRules, dxSpreadSheetContainers,
  dxSpreadSheetHyperlinks, dxSpreadSheetFormatXLSProtection,
...
type
  ...
  protected
    procedure CheckRange(const ASize: Integer; var AFirstSize, ASecondSize,
      APosition: Integer); inline;
  public
    ...
    function XLS_ReadSharedString(AOptions, ALength, ARunCount: Integer)
      : TdxSpreadSheetSharedString; overload;
    function XLS_ReadSimpleString(AOffset: Integer): string; inline;
    function XLS_ReadString(const AOptions, ALength: Integer): string;
      overload; inline;
...
{$ENDREGION}
  ...
  TdxSpreadSheetXLSConditionalFormattingRules = class
    (TObjectList<TdxSpreadSheetXLSConditionalFormattingRule>)
  public
    function Add(ID: Integer): TdxSpreadSheetXLSConditionalFormattingRule;
    function Get(ID: Integer;
      out ARule: TdxSpreadSheetXLSConditionalFormattingRule): Boolean;
  end;
  ...
  public
    ...
    function CreateContainer(AContainerType: Integer = -1)
      : TdxSpreadSheetContainer; virtual; abstract;
  ...
  strict private
    ...
    function CreateConditionalFormattingRule(AOptions: Word)
    ...
    function ReadConditionalFormattingRuleExpression(ARefSize, AAnchorColumn,
      AAnchorRow: Integer): string;
    procedure ReadConditionalFormattingRuleStyle
      (AStyle: TdxSpreadSheetCellStyle); overload;
    procedure ReadConditionalFormattingRuleStyle(AStyle: TdxXLSCellStyle;
      out AFontHandle: TdxSpreadSheetFontHandle); overload;
  ...
  protected
    ...
    procedure ConvertContainer(AContainer: TdxSpreadSheetContainer;
      ATargetContainerClass: TdxSpreadSheetContainerClass;
      var ATargetContainer); overload;
    procedure ConvertContainer(AContainer: TdxSpreadSheetContainer;
      AMinContainerClass, ATargetContainerClass: TdxSpreadSheetContainerClass;
      var ATargetContainer); overload;

    function CreateCell: TdxSpreadSheetCell; overload;
    function CreateCell(ARow, AColumn, AStyleIndex: Integer)
      : TdxSpreadSheetCell; overload;
    function CreateProgressHelper
      : TdxSpreadSheetCustomFilerProgressHelper; override;
    function GetColorDef(AIndex, ADefaultIndex: Integer;
      ADefaultColor: TColor): TColor;
    function GetFillStyle(AXLSFillStyle: Byte;
      var AFillStyle: TdxSpreadSheetCellFillStyle): Boolean;
    function GetFont(AIndex: Integer): TdxSpreadSheetFontHandle; inline;
    function GetFontEx(AFontHandle: TdxSpreadSheetFontHandle; AColor: TColor)
    ...
    procedure PrepareFormattedSharedString
    ...
    function TryDefineName(const AName: string;
      var ADefinedName: TdxSpreadSheetDefinedName): Boolean;
    //
  ...
  public
    ...
    function GetExternalName(AIndex: Integer;
      var AInfo: TdxSpreadSheetFunctionInfo): string; inline;
    function GetName(AIndex: Integer; var AInfo: TdxSpreadSheetFunctionInfo)
      : string; inline;
    ...
    property CommonDataReady: Boolean read FCommonDataReady
      write FCommonDataReady;
    property ConditionalFormattingRules
      : TdxSpreadSheetXLSConditionalFormattingRules
    ...
    property NamesDefinition: TObjectList<TdxXLSNameDefinition>
      read FNamesDefinition;
    property NumberFormats: TDictionary<Word, TdxSpreadSheetFormatHandle>
      read FNumberFormats;
...
uses
  ...
  dxSpreadSheetFormatXLS, dxOLECryptoContainer, dxOLECryptoContainerStrs,
  dxProtectionUtils, dxSpreadSheetStrs,
...
begin
  case AContainer.AnchorType of
    ...
    Result := not(cxPointIsNull(AContainer.AnchorPoint1.Offset) and
      cxPointIsNull(AContainer.AnchorPoint2.Offset));
  end;
end;
...
procedure TdxSpreadSheetXLSConditionalFormattingRule.ApplyRuleToAllAreas
  (ARule: TdxSpreadSheetConditionalFormattingRuleExpression);
var
  AAreaInfo: TAreaInfo;
  I: Integer;
begin
  ...
  begin
    AAreaInfo := Areas[I];
    AAreaInfo.Rule := TdxSpreadSheetConditionalFormattingRuleExpression
      (ARule.Clone(AAreaInfo.Area));
    Areas[I] := AAreaInfo;
  end;
end;
...
function TdxSpreadSheetXLSConditionalFormattingRules.Add(ID: Integer)
  : TdxSpreadSheetXLSConditionalFormattingRule;
begin
...
function TdxSpreadSheetXLSConditionalFormattingRules.Get(ID: Integer;
  out ARule: TdxSpreadSheetXLSConditionalFormattingRule): Boolean;
var
  I: Integer;
begin
...
constructor TdxSpreadSheetXLSCustomMSODrawingReader.Create
...
constructor TdxSpreadSheetXLSReader.Create(AOwner: TdxCustomSpreadSheet;
  AStream: TStream);
begin
  ...
  FConditionalFormattingRules :=
    TdxSpreadSheetXLSConditionalFormattingRules.Create;
...
function TdxSpreadSheetXLSReader.GetExternalName(AIndex: Integer;
  var AInfo: TdxSpreadSheetFunctionInfo): string;
begin
  Result := '';
  AInfo := nil;
...
function TdxSpreadSheetXLSReader.GetName(AIndex: Integer;
  var AInfo: TdxSpreadSheetFunctionInfo): string;
begin
  Result := '';
  AInfo := nil;
...
function TdxSpreadSheetXLSReader.GetSheet(AIndex: Integer)
...
procedure TdxSpreadSheetXLSReader.ConvertContainer
  (AContainer: TdxSpreadSheetContainer;
  ATargetContainerClass: TdxSpreadSheetContainerClass; var ATargetContainer);
begin
  ConvertContainer(AContainer, ATargetContainerClass, ATargetContainerClass,
    ATargetContainer);
end;
...
function TdxSpreadSheetXLSReader.CreateCell(ARow, AColumn, AStyleIndex: Integer)
  : TdxSpreadSheetCell;
begin
...
function TdxSpreadSheetXLSReader.CreateProgressHelper
  : TdxSpreadSheetCustomFilerProgressHelper;
begin
  Result := TdxSpreadSheetCustomFilerProgressHelper.Create(Self, 0);
end;

function TdxSpreadSheetXLSReader.GetColorDef(AIndex, ADefaultIndex: Integer;
  ADefaultColor: TColor): TColor;
begin
...
function TdxSpreadSheetXLSReader.GetFillStyle(AXLSFillStyle: Byte;
  var AFillStyle: TdxSpreadSheetCellFillStyle): Boolean;
var
  I: Byte;
begin
...
function TdxSpreadSheetXLSReader.GetFont(AIndex: Integer)
  : TdxSpreadSheetFontHandle;
begin
...
function TdxSpreadSheetXLSReader.GetFontEx(AFontHandle
  : TdxSpreadSheetFontHandle; AColor: TColor): TdxSpreadSheetFontHandle;
begin
  Result := AFontHandle;
...
function TdxSpreadSheetXLSReader.GetFormat(AIndex: Integer)
...
begin
  ...
  begin
    ...
    if Encryptor.CheckPassword(APassword) then
      ...
      raise EdxOLECryptoContainerError.Create
...
begin
  ...
  begin
    ...
    if ASharedString.ClassType = TdxSpreadSheetFormattedSharedString then
      PrepareFormattedSharedString(TdxSpreadSheetFormattedSharedString
        (ASharedString));
...
const
  ...
  XLSBorderStyleToSpreadSheetStyle: array [0 .. 15]
    of TdxSpreadSheetCellBorderStyle = (sscbsDefault, sscbsThin, sscbsMedium,
    sscbsDashed, sscbsDotted, sscbsThick, sscbsDouble, sscbsHair,
    sscbsMediumDashed, sscbsDashDot, sscbsMediumDashDot, sscbsDashDotDot,
    sscbsMediumDashDotDot, sscbsSlantedDashDot, sscbsDefault, sscbsDefault);
  ...
  begin
    if bLeft in AXLSStyle.Borders then
      ABorders.BorderColor[bLeft] :=
        GetColor(AXLSStyle.LeftRightBorders and $007F);
    if bRight in AXLSStyle.Borders then
      ABorders.BorderColor[bRight] :=
        GetColor((AXLSStyle.LeftRightBorders and $3F80) shr 7);
    if bTop in AXLSStyle.Borders then
      ABorders.BorderColor[bTop] :=
        GetColor(AXLSStyle.TopBottomBordersAndFill and $007F);
    if bBottom in AXLSStyle.Borders then
      ABorders.BorderColor[bBottom] :=
        GetColor((AXLSStyle.TopBottomBordersAndFill and $3F80) shr 7);
    for ABorder := Low(ABorder) to High(ABorder) do
      ...
      begin
        ABorders.BorderStyle[ABorder] := XLSBorderStyleToSpreadSheetStyle
          [(AXLSStyle.BordersStyle and BorderStyleMask[ABorder])
          shr BorderStyleOffset[ABorder]];
      end;
  end;
...
begin
  ...
  begin
    ...
    if xfextTextColor in AXLSStyle.ExtendedData.AssignedValues then
      AFontHandle := GetFontEx(AFontHandle, AXLSStyle.ExtendedData.DataValue
        [xfextTextColor]);
  end;
  ...
  AStyle.AlignVert := TdxSpreadSheetDataAlignVert
    (AXLSStyle.Alignments shr 4 and $7);
  AStyle.AlignHorzIndent := ColumnWidthHelper.SpacesNumberToPixels
    (AXLSStyle.Indents and $F);
  if AXLSStyle.Alignments and $7 >= 6 then
    AStyle.AlignHorz := ssahCenter
  else
    AStyle.AlignHorz := TdxSpreadSheetDataAlignHorz
      (AXLSStyle.Alignments and $7);

  PrepareBorders(ABorders);

  if GetFillStyle(AXLSStyle.TopBottomBordersAndFill shr 26 and $FF, AFillStyle)
  then
  begin
    ...
    if AXLSStyle.FillColors shr 7 and $7F > $40 then
      ...
      ABrush.BackgroundColor := GetColorDef(AXLSStyle.FillColors shr 7 and $7F,
        $40, clDefault);

    if ABrush.ForegroundColor = $40 then
      ABrush.ForegroundColor := clBlack
    else
      ABrush.ForegroundColor := GetColorDef(AXLSStyle.FillColors and $7F, $41,
        clDefault);
  end;
  ...
  if (ABrush.Style = sscfsSolid) and (ABrush.ForegroundColor <> clDefault) then
    ExchangeLongWords(Longword(ABrush.BackgroundColor),
      Longword(ABrush.ForegroundColor));
...
function TdxSpreadSheetXLSReader.TryDefineName(const AName: string;
  var ADefinedName: TdxSpreadSheetDefinedName): Boolean;
begin
  Result := True;
  ADefinedName := nil;
...
procedure TdxSpreadSheetXLSReader.RegisterReader(ARecordID: Word;
  AReader: TdxXLSMethod);
begin
...
procedure TdxSpreadSheetXLSReader.ReadSpecialName;
  ...
  function ReadDefinedName(const ACode: Char;
    out AName: TdxSpreadSheetDefinedName): Boolean;
  begin
    ...
    begin
      ...
      with TdxXLSFormulaReader.Create(Self) do
        try
          ReadName(AName, Reader.Words[2], @PByteArray(RecordReader.Memory)
            ^[RecordReader.Position]);
        finally
          Free;
        end;
    end;
...
begin
  ...
  begin
    ...
    AColumn.Size := MulDiv(Reader.Words[2],
      ColumnWidthHelper.MaxDigitWidth, 256);
  ...
  if AOutlineLevel > 0 then
    TdxSpreadSheetOutlineHelper.IncreaseOutlineLevelTo(CurrentSheet.Columns,
      Reader.Words[0], Reader.Words[1], AOutlineLevel - 1);
...
begin
  ...
  begin
    ConvertContainer(AContainer, TdxSpreadSheetCommentContainer,
      ACommentContainer);
    ...
    begin
      ...
      ACommentContainer.Calculator.UpdateAnchors
        (TdxSpreadSheetCommentContainer.GetDefaultPosition
        (ACommentContainer.Cell));
    end;
...
function TdxSpreadSheetXLSReader.CreateConditionalFormattingRule(AOptions: Word)
  : TdxSpreadSheetConditionalFormattingRuleExpression;
const
  ComparisonOperatorMap: array [1 .. 8]
...
begin
  ...
  case LoByte(AOptions) of
    1:
      if InRange(HiByte(AOptions), Low(ComparisonOperatorMap),
        High(ComparisonOperatorMap)) then
      begin
        Result := TdxSpreadSheetConditionalFormattingRuleCellIs.Create
          (CurrentSheet.ConditionalFormatting);
        TdxSpreadSheetConditionalFormattingRuleCellIs(Result).ComparisonOperator
          := ComparisonOperatorMap[HiByte(AOptions)];
      end;
    2:
...
begin
  ...
  begin
    RecordReader.Position := 16;
    if ConditionalFormattingRules.Get(Reader.ReadWord, ARuleInfo) and ARuleInfo.IsAssigned
    then
    begin
      Reader.ReadInteger;
      Reader.ReadWord;
...
begin
  ...
  begin
    ARule := TdxSpreadSheetConditionalFormattingRuleExpressionAccess
      (CreateConditionalFormattingRule(Reader.ReadWord));
    if ARule <> nil then
    begin
      for I := 0 to ARuleInfo.Areas.Count - 1 do
      ...
      ARule.Expression := ReadConditionalFormattingRuleExpression(AValue1Size,
        ARule.Area.Left, ARule.Area.Top);
      ARule.Expression2 := ReadConditionalFormattingRuleExpression(AValue2Size,
        ARule.Area.Left, ARule.Area.Top);
...
function TdxSpreadSheetXLSReader.ReadConditionalFormattingRuleExpression
  (ARefSize, AAnchorColumn, AAnchorRow: Integer): string;
...
begin
  ...
  if ARefSize > 0 then
  begin
    AFormula := TdxSpreadSheetConditionalFormattingExpression.Create
      (CurrentSheet.ConditionalFormatting);
    try
      if ReadFormula(AFormula) then
      begin
        TdxSpreadSheetConditionalFormattingExpressionAccess(AFormula).FAnchorRow
          := AAnchorRow;
        TdxSpreadSheetConditionalFormattingExpressionAccess(AFormula)
...
const
  BorderFlagsMap: array [TcxBorder] of Integer = (CFRULE_OPTION_BORDER_LEFT,
...
function TdxSpreadSheetXLSReader.ReadConditionalFormattingRuleStyleFont
  : TdxSpreadSheetFontHandle;
var
  AOptions: Integer;
  AOptionsEx: Integer;
...
begin
  ...
  NumberFormats.Add(Reader.Words[0],
    AddNumberFormat(Reader.XLS_ReadString(Reader.Bytes[4], Reader.Bytes[2]),
    Reader.Words[0]));
end;

procedure TdxSpreadSheetXLSReader.ReadDateSystem;
begin
  SpreadSheet.OptionsView.DateTimeSystem := TdxSpreadSheetDateTimeSystem
    (Boolean(Reader.ReadWord));
end;

procedure TdxSpreadSheetXLSReader.ReadDefaultColumnWidth;
begin
  CurrentSheet.Options.DefaultColumnWidth :=
    MulDiv(Reader.Words[0], ColumnWidthHelper.MaxDigitWidth, 256);
end;

procedure TdxSpreadSheetXLSReader.ReadDefColumnWidth;
begin
  CurrentSheet.Options.DefaultColumnWidth := Reader.Words[0] *
    (ColumnWidthHelper.MaxDigitWidth + 1);
end;

procedure TdxSpreadSheetXLSReader.ReadDefaultRowHeight;
begin
  CurrentSheet.Options.DefaultRowHeight := TdxValueUnitsHelper.PointsToPixels
    (Reader.Words[1] / 20);
end;
...
begin
  ...
  begin
    ...
    if (AInfo = nil) and (Pos(dxSpreadSheetFeatureFunctionPrefix, AName) = 1)
    then
    begin
      ANameNoPrefix := Copy(AName, Length(dxSpreadSheetFeatureFunctionPrefix) +
        1, MaxInt);
  ...
  else if (AOptions = 0) or (AOptions = 2) then
  begin
    ExternalNames.Add(AName);
    if TryDefineName(AName, ADefinedName) then
      with TdxXLSFormulaReader.Create(Self) do
        try
          ReadName(ADefinedName, ASize, @PByteArray(RecordReader.Memory)
            ^[RecordReader.Position]);
        finally
          Free;
        end;
...
procedure TdxSpreadSheetXLSReader.ReadFormulas(ATop, ALeft, ABottom, ARight,
  AOffset: Integer);
var
...
begin
  ...
  CreateCell.AsSharedString := TdxSpreadSheetSharedString
    (Strings[Reader.ReadInteger]);
end;
...
begin
  ...
  for AIndex := 0 to (RecordReader.Size - 6) div 6 - 1 do
    CreateCell(ARow, AIndex + AStartColumn, Reader.Words[2 + AIndex * 3])
      .AsFloat := Reader.XLS_ReadRKValue((AIndex + 1) * 6);
end;
...
const
  AllowedObjects: set of Byte = [oGroup .. oText, oPicture, oPolygon, oComment,
    oDrawing];
var
  AContainer: TdxSpreadSheetContainer;
  AContainerID: Word;
...
begin
  ...
  begin
    ...
    else if CurrentSheet.Containers.Count > 0 then
    begin
      CurrentSheet.Containers.Add
        (TdxSpreadSheetContainerClass(CurrentSheet.Containers.Last.ClassType),
        AContainer);
...
begin
  ...
  ConvertContainer(CurrentSheet.Containers.Last,
    TdxSpreadSheetCustomTextBoxContainer, TdxSpreadSheetTextBoxContainer,
    AContainer);
  ...
  try
    ...
    case (AAlignFlags and $E) shr 1 of
      2:
        AContainer.TextBox.AlignHorz := taCenter;
      3:
        AContainer.TextBox.AlignHorz := taRightJustify;
    else
    ...
    case (AAlignFlags and $70) shr 4 of
      2:
        AContainer.TextBox.AlignVert := taVerticalCenter;
      3:
        AContainer.TextBox.AlignVert := taAlignBottom;
    else
    ...
    else if ARunCount > 0 then
    begin
      AContainer.TextBox.Text := ReadFormattedString(ATextLength, ARunCount);
      ARun := TdxSpreadSheetFormattedSharedString
        (AContainer.TextBox.Text).Runs[0];
      if ARun.FontHandle <> nil then
        AContainer.TextBox.Font.Handle := ARun.FontHandle;
    end
    else
      AContainer.TextBox.Text := TdxSpreadSheetSharedString.CreateObject
        (Reader.XLS_ReadString(1, ATextLength));
  finally
...
begin
  ...
  begin
    if ((AOptions and $FF) in [0, 2, $10]) and TryDefineName(AName, ADefinedName)
    then
      NamesDefinition.Add(TdxXLSNameDefinition.Create(ADefinedName, ASize,
        @PByteArray(RecordReader.Memory)^[RecordReader.Position]));
    Names.AddObject(AName, ADefinedName);
  end;
end;
...
begin
  CurrentSheet.OptionsPrint.Printing.HorizontalCentered :=
    TdxDefaultBoolean(Reader.ReadWord = 1);
end;

procedure TdxSpreadSheetXLSReader.ReadPrintCenterV;
begin
  CurrentSheet.OptionsPrint.Printing.VerticalCentered :=
    TdxDefaultBoolean(Reader.ReadWord = 1);
end;

procedure TdxSpreadSheetXLSReader.ReadPrintGridLines;
begin
  CurrentSheet.OptionsPrint.Source.GridLines :=
    TdxDefaultBoolean(Reader.ReadWord = 1);
end;
...
begin
  CurrentSheet.OptionsPrint.Source.Headers :=
    TdxDefaultBoolean(Reader.ReadWord = 1);
end;
...
var
  AOptions: Word;
const
  AOrientation: array [Boolean]
    of TdxSpreadSheetTableViewOptionsPrintPageOrientation = (oppoLandscape,
    oppoPortrait);
  APageOrder: array [Boolean]
    of TdxSpreadSheetTableViewOptionsPrintPrintingPageOrder = (opppDownThenOver,
    opppOverThenDown);
begin
  AOptions := Reader.Words[5];
  CurrentSheet.OptionsPrint.Page.Paper.SizeID := Reader.Words[0];
  CurrentSheet.OptionsPrint.Page.Orientation :=
    AOrientation[AOptions and $2 = $2];
  ...
  CurrentSheet.OptionsPrint.Printing.BlackAndWhite :=
    TdxDefaultBoolean(AOptions and $8 = $8);
  CurrentSheet.OptionsPrint.Printing.Draft :=
    TdxDefaultBoolean(AOptions and $10 = $10);
  CurrentSheet.OptionsPrint.Printing.PageOrder :=
    APageOrder[AOptions and $1 = 1];
...
const
  AScaleModes: array [Boolean]
    of TdxSpreadSheetTableViewOptionsPrintPageScaleMode = (oppsmAdjustToScale,
    oppsmFitToPage);
begin
  CurrentSheet.OptionsPrint.Page.ScaleMode :=
    AScaleModes[Reader.Words[0] and $100 = $100];
  CurrentSheet.Rows.Groups.ExpandButtonPosition :=
    TdxSpreadSheetTableItemGroupExpandButtonPosition(Reader.Words[0] and
    $40 = $40);
  CurrentSheet.Columns.Groups.ExpandButtonPosition :=
    TdxSpreadSheetTableItemGroupExpandButtonPosition(Reader.Words[0] and
    $80 = $80);
end;
...
begin
  ...
  begin
    ...
    SpreadSheet.OptionsProtection.AllowChangeStructure :=
      not SpreadSheet.OptionsProtection.Protected;
  end;
end;
...
begin
  ...
  if AOutlineLevel > 0 then
    TdxSpreadSheetOutlineHelper.IncreaseOutlineLevelTo(CurrentSheet.Rows,
      ARow.Index, AOutlineLevel - 1);
...
begin
  ...
  begin
    ...
    begin
      ...
      CurrentSheet.OptionsProtection.AllowDeleteColumns :=
        iprotDeleteColumns and AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowDeleteRows := iprotDeleteRows and
        AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowResizeColumns :=
        iprotFormatColumns and AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowEditContainers := iprotObject and
        AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowEditHyperlinks :=
        iprotInsertHyperlinks and AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowResizeRows := iprotFormatRows and
        AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowFormatCells := iprotFormatCells and
        AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowInsertColumns :=
        iprotInsertColumns and AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowInsertRows := iprotInsertRows and
        AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowSelectLockedCells :=
        iprotSelLockedCells and AFlags <> 0;
      CurrentSheet.OptionsProtection.AllowSelectUnlockedCells :=
        iprotSelUnlockedCells and AFlags <> 0;
...
begin
  ReadFormulas(Reader.Words[0], Reader.Bytes[4], Reader.Words[1],
    Reader.Bytes[5], 8);
end;
...
begin
  ...
  CurrentSheet.Options.RightToLeftLayout :=
    TdxDefaultBoolean(AOptions and $40 = $40);
  HasFrozenPanes := AOptions and $8 = $8;
  if (AOptions and $20 = 0) and (AColor <> 0) and (AColor <> $40) and
    InRange(AColor, 0, Length(Palette) - 1) then
...
var
  ...
  begin
    ...
    case APropData.xclrType of
      ...
      3:
        begin
          AStyle.ExtendedData.SetThemeColorValue(APropType, APropData.xclrValue,
            APropData.nTintShade);
...
begin
  ...
  begin
    ...
    case APropType of
      xfextRGBForeColor, xfextRGBBackColor, xfextForeColor, xfextBackColor,
        xfextTextColor, xfextBorderColorTop, xfextBorderColorBottom,
        xfextBorderColorLeft, xfextBorderColorRight:
...
begin
  ...
  begin
    ...
    if (AType = brcContinue) or ((RecordID = brcMSODRAWINGGROUP) and
      (AType = RecordID)) then
...
function TdxXLSReader.XLS_ReadSharedString(AOptions, ALength,
  ARunCount: Integer): TdxSpreadSheetSharedString;
var
  ACharIndex, AFontIndex: Word;
begin
  if ALength = 0 then
    ...
    Result := TdxSpreadSheetSharedString.CreateObject
      (XLS_ReadString(AOptions, ALength))
  else
  begin
    Result := TdxSpreadSheetFormattedSharedString.CreateObject
      (XLS_ReadString(AOptions, ALength));
    ...
    begin
      ...
      AFontIndex := ReadWord;
      TdxSpreadSheetFormattedSharedString(Result).Runs.Add(ACharIndex, nil,
        AFontIndex);
      Dec(ARunCount);
    end;
  end;
...
procedure TdxXLSReader.CheckRange(const ASize: Integer;
  var AFirstSize, ASecondSize, APosition: Integer);
var
...
begin
  ...
  ASecondSize := 0;
  APosition := 0;
  if XLSRecord.Parts.Count = 1 then
    Exit;
  ...
  begin
    ...
    if AFinishPos < APosition then
      Break
    else if AStartPos <= APosition then
    begin
