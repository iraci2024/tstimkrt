...
type
  ...
  protected
    class function ApplyToColumns(const AFormula: string; const AArea: TRect;
      ASheet: TdxSpreadSheetTableView): Integer;
    class function ApplyToRows(const AFormula: string; const AArea: TRect;
      ASheet: TdxSpreadSheetTableView): Integer;
    class function IsCellEmpty(const ARowIndex, AColumnIndex: Integer;
      ASheet: TdxSpreadSheetTableView): Boolean; inline;
    class function IsNonEmptyCellWithoutCorrespondingFormula(const AFormula
      : string; const ARowIndex, AColumnIndex: Integer;
      ASheet: TdxSpreadSheetTableView): Boolean;
    class function IsNumericDataInCell(const ARowIndex, AColumnIndex: Integer;
      ASheet: TdxSpreadSheetTableView): Boolean;
  public
    class procedure ApplyToSelection(const AFormula: string;
...
uses
  Math, dxSpreadSheetTypes, dxSpreadSheetUtils, cxMemo, cxRichEdit, cxTextEdit,
...
begin
  if (AChar = dxLeftParenthesis) or (AChar = dxLeftArrayParenthesis) or
    (AChar = dxFormatSettings.ListSeparator) then
    Exit(True);
...
class procedure TdxModifyFormulasHelper.ApplyToSelection(const AFormula: string;
  ASheet: TdxSpreadSheetTableView);
  ...
  begin
    ...
    Result := (AArea.Left < AArea.Right) and
      (Result or (AArea.Top = AArea.Bottom));
  end;
...
begin
  ...
  if ACounter = 0 then
    Insert(AFormula, TdxSpreadSheetTableViewAccess(ASheet)
      .EditingController.Edit);
end;

class function TdxModifyFormulasHelper.GetBracketsBalance(const AFormula
  : string): Integer;
var
  I: Integer;
begin
...
class procedure TdxModifyFormulasHelper.Insert(const AFormula: string;
  AEdit: TcxCustomEdit; AAddParenthesis: Boolean);
var
...
begin
  ...
  begin
    ...
    begin
      System.Insert(dxLeftParenthesis + dxRightParenthesis, AText,
        ACursorPosition + 1);
...
class function TdxModifyFormulasHelper.ApplyToColumns(const AFormula: string;
  const AArea: TRect; ASheet: TdxSpreadSheetTableView): Integer;
  ...
  function BuildFormulaForColumn(AColumnIndex: Integer;
    ...
    begin
      if IsNumericDataInCell(I, AColumnIndex, ASheet) then
        Exit(dxDefaultOperations[opEQ] + AFormula + dxLeftParenthesis +
          dxReferenceToString(cxRect(AColumnIndex, AArea.Top, AColumnIndex,
          Min(AArea.Bottom, dxSpreadSheetMaxRowIndex - 1))) +
          dxRightParenthesis);
    end;
    Result := '';
  end;
  ...
  begin
    ...
    repeat
      ANext := False;
      for C := AArea.Left to Min(AArea.Right, ASheet.Columns.LastIndex) do
        if IsNonEmptyCellWithoutCorrespondingFormula(AFormula, Result, C, ASheet)
        then
        begin
          ANext := True;
          Inc(Result);
...
begin
  ...
  begin
    ...
    begin
      Inc(Result);
      ASheet.CreateCell(Min(R, dxSpreadSheetMaxRowIndex), C)
        .SetText(ATempFormula, True);
    end;
  end;
end;

class function TdxModifyFormulasHelper.ApplyToRows(const AFormula: string;
  const AArea: TRect; ASheet: TdxSpreadSheetTableView): Integer;
  ...
  begin
    ...
    begin
      if IsNumericDataInCell(ARowIndex, I, ASheet) then
        Exit(dxDefaultOperations[opEQ] + AFormula + dxLeftParenthesis +
          dxReferenceToString(cxRect(AArea.Left, ARowIndex, Min(AArea.Right,
          dxSpreadSheetMaxColumnIndex - 1), ARowIndex)) + dxRightParenthesis);
    end;
    Result := '';
  end;
  ...
  begin
    ...
    repeat
      ANext := False;
      for R := AArea.Top to Min(AArea.Bottom, ASheet.Rows.LastIndex) do
        if IsNonEmptyCellWithoutCorrespondingFormula(AFormula, R, Result, ASheet)
        then
        begin
          ANext := True;
          Inc(Result);
...
begin
  ...
  begin
    ...
    begin
      Inc(Result);
      ASheet.CreateCell(R, Min(C, dxSpreadSheetMaxColumnIndex))
        .SetText(ATempFormula, True);
    end;
  end;
end;

class function TdxModifyFormulasHelper.IsCellEmpty(const ARowIndex,
  AColumnIndex: Integer; ASheet: TdxSpreadSheetTableView): Boolean;
var
...
class function TdxModifyFormulasHelper.IsNonEmptyCellWithoutCorrespondingFormula
  (const AFormula: string; const ARowIndex, AColumnIndex: Integer;
  ASheet: TdxSpreadSheetTableView): Boolean;
var
  ACell: TdxSpreadSheetCell;
begin
  ACell := ASheet.Cells[ARowIndex, AColumnIndex];
  Result := (ACell <> nil) and (ACell.DataType <> cdtBlank) and
    not(ACell.IsFormula and StartsText(dxDefaultOperations[opEQ] + AFormula,
    ACell.AsFormula.AsText));
end;

class function TdxModifyFormulasHelper.IsNumericDataInCell(const ARowIndex,
  AColumnIndex: Integer; ASheet: TdxSpreadSheetTableView): Boolean;
var
  ACell: TdxSpreadSheetCell;
begin
  ACell := ASheet.Cells[ARowIndex, AColumnIndex];
  Result := (ACell <> nil) and (ACell.DataType in [cdtCurrency, cdtFloat,
