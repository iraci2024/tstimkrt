...
{$ENDIF}
  ...
  dxCore, cxControls, cxContainer, cxEdit, cxRichEdit, dxSpreadSheetCore,
  cxDropDownEdit, cxLookAndFeelPainters,
  cxGeometry, dxCoreClasses, dxGenerics, cxGraphics, cxLookAndFeels,
  cxButtonEdit, cxHeader, cxScrollBar, dxMessages,
...
type
  ...
  strict private
    ...
    procedure SetOptionsBehavior
      (AValue: TdxSpreadSheetFormulaBarOptionsBehavior);
    ...
    procedure DXMContainerSetFocus(var AMessage: TMessage);
  ...
  protected
    ...
    function CreateOptionsBehavior
      : TdxSpreadSheetFormulaBarOptionsBehavior; virtual;
    ...
    procedure ProcessChanges(AChanges
      : TdxSpreadSheetFormulaBarChanges); virtual;
    ...
    procedure ControllerKeyDown(var Key: Word;
      ShiftState: TShiftState); virtual;
    ...
    procedure HandlerFormulaEditKeyDown(Sender: TObject; var Key: Word;
      ShiftState: TShiftState); virtual;
    procedure HandlerFormulaEditKeyPress(Sender: TObject;
      var Key: Char); virtual;
    procedure HandlerFormulaEditKeyUp(Sender: TObject; var Key: Word;
      ShiftState: TShiftState); virtual;
    ...
    procedure DoHideEdit(AAccept: Boolean; AIsArrayFormula: Boolean = False;
      ACanChangeFocus: Boolean = True); virtual;
    procedure DoSelectNamedObject(const AText: string; AObject: TObject);
      virtual; abstract;
    ...
    property OptionsBehavior: TdxSpreadSheetFormulaBarOptionsBehavior
      read FOptionsBehavior write SetOptionsBehavior;
    property OptionsView: TdxSpreadSheetFormulaBarOptionsView read FOptionsView
      write SetOptionsView;
    property SeparatorPosition: Integer read FSeparatorPosition
      write SetSeparatorPosition default DefaultSeparatorPosition;
  ...
  published
    property FormulaAutoComplete: TdxDefaultBoolean read FFormulaAutoComplete
      write FFormulaAutoComplete default bDefault;
    property FormulaAutoCompleteShowHint: TdxDefaultBoolean
  ...
  published
    property ShowButtons: Boolean read FShowButtons write SetShowButtons
      default True;
    property ShowNameBox: Boolean read FShowNameBox write SetShowNameBox
      default True;
  end;
...
{$ENDIF}
  ...
  public
    property InplaceEdit: TdxSpreadSheetFormulaBarInplaceEdit
      read GetInplaceEdit;
  end;
  ...
  public
    ...
    property ScrollBar: TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper
      read GetScrollBar;
  ...
  TdxSpreadSheetFormulaBarInplaceEditScrollBarController = class
  ...
  public
    property ExpandButtonState: TcxButtonState read GetExpandButtonState
      write SetExpandButtonState;
    property ScrollBar: TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper
      read GetScrollBar;
    property ViewInfo: TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo
      read GetViewInfo;
  end;

  { TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter }

  TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter = class
    (TcxScrollBarPainter)
  strict private
    function GetViewInfo: TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo;
  protected
    procedure DrawExpandButton(ACanvas: TcxCanvas; const R: TRect;
      AState: TcxButtonState; AExpanded: Boolean); virtual;
  public
  ...
  TdxSpreadSheetEditingController = class
    (TdxSpreadSheetTableViewEditingController);
  TdxSpreadSheetFormulaBarExecuteInsertFunctionDialogEvent = procedure
    (Sender: TdxSpreadSheetCustomFormulaBar; var AHandled: Boolean) of object;

  TdxSpreadSheetCustomFormulaBar = class(TdxSpreadSheetAbstractFormulaBar,
    IdxSpreadSheet, IdxSpreadSheetListener, IdxSpreadSheetListener2,
    IdxSpreadSheetEditControllerListener, IdxSpreadSheetKeyboardListener,
  ...
  strict private
    ...
    FOnExecuteInsertFunctionDialog
      : TdxSpreadSheetFormulaBarExecuteInsertFunctionDialogEvent;
  ...
  protected
    ...
    procedure DoCreateDefinedName(const AArea: TRect;
      const AName, ASheetName: string); virtual;
    procedure DoExecuteInsertFunctionDialog; override;
    procedure DoHideEdit(AAccept: Boolean; AIsArrayFormula: Boolean = False;
      ACanChangeFocus: Boolean = False); override;
    procedure DoRenameContainer(AContainer: TdxSpreadSheetContainer;
      const AName: string); virtual;
    procedure DoSelectArea(const AView: TdxSpreadSheetTableView;
      const AArea: TRect); virtual;
    procedure DoSelectContainer(AContainer: TdxSpreadSheetContainer); virtual;
    procedure DoSelectNamedObject(const AText: string;
    ...
    procedure SpreadSheetKeyDown(Sender: TdxCustomSpreadSheet; var Key: Word;
      Shift: TShiftState);
    // IdxSpreadSheetSelectionListener
    ...
    property EditingController: TdxSpreadSheetEditingController
  ...
  public
    ...
    property SpreadSheet: TdxCustomSpreadSheet read FSpreadSheet
      write SetSpreadSheet;
    property Transparent default False;
    //
    property OnExecuteInsertFunctionDialog
      : TdxSpreadSheetFormulaBarExecuteInsertFunctionDialogEvent
      read FOnExecuteInsertFunctionDialog write FOnExecuteInsertFunctionDialog;
  end;
...
uses
  SysUtils, Math, dxTypeHelpers, cxEditUtils, dxSpreadSheetStrs,
  dxDPIAwareUtils, dxSpreadSheetUtils,
  dxSpreadSheetTypes, dxSpreadSheetCoreHelpers, cxTextEdit,
  dxSpreadSheetInsertFunctionDialog, dxSpreadSheetFunctions,
  dxSpreadSheetCoreStrs, dxSpreadSheetFormulasHelpers, dxSpreadSheetCoreHistory,
...
begin
  Result := inherited or NameBox.Focused or ButtonBox.Focused or
    FormulaEdit.Focused;
end;

procedure TdxSpreadSheetAbstractFormulaBar.FullRefresh;
begin
  Changed([Low(TdxSpreadSheetFormulaBarChange)
    .. High(TdxSpreadSheetFormulaBarChange)]);
end;
...
function TdxSpreadSheetAbstractFormulaBar.CreateFormulaEdit
  : TdxSpreadSheetFormulaBarInplaceEdit;
begin
...
function TdxSpreadSheetAbstractFormulaBar.CreateOptionsBehavior
  : TdxSpreadSheetFormulaBarOptionsBehavior;
begin
  Result := TdxSpreadSheetFormulaBarOptionsBehavior.Create(Self);
end;

function TdxSpreadSheetAbstractFormulaBar.CreateOptionsView
  : TdxSpreadSheetFormulaBarOptionsView;
begin
  Result := TdxSpreadSheetFormulaBarOptionsView.Create(Self);
end;

function TdxSpreadSheetAbstractFormulaBar.CreateViewInfo
  : TdxSpreadSheetFormulaBarViewInfo;
begin
...
function TdxSpreadSheetAbstractFormulaBar.CanAutoSize(var NewWidth,
  NewHeight: Integer): Boolean;
begin
  Result := True;
  if ViewInfo.LineHeight > 0 then
    NewHeight := ViewInfo.LineHeight *
      Max(1, Round(NewHeight / ViewInfo.LineHeight));
end;

function TdxSpreadSheetAbstractFormulaBar.
  CanExecuteInsertFunctionDialog: Boolean;
begin
  Result := not IsEditing or dxSpreadSheetIsFormula(FormulaEdit.Text);
end;

procedure TdxSpreadSheetAbstractFormulaBar.Changed
  (AChanges: TdxSpreadSheetFormulaBarChanges);
begin
  if not IsDestroying then
    ProcessChanges(AChanges);
end;

procedure TdxSpreadSheetAbstractFormulaBar.ChangeScaleEx(M, D: Integer;
  isDpiChange: Boolean);
begin
...
begin
  ...
  FFormulaEdit.Properties.OnSelectionChange :=
    HandlerFormulaEditSelectionChanged;
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        DeferWindowPos(AHandle, NameBox.Handle, 0, ARect.Left, ARect.Top,
          ARect.Width, ARect.Height, SWP_NOZORDER);

        ARect := ViewInfo.ButtonBoxRect;
        DeferWindowPos(AHandle, ButtonBox.Handle, 0, ARect.Left, ARect.Top,
          ARect.Width, ARect.Height, SWP_NOZORDER);

        ARect := ViewInfo.FormulaEditRect;
        DeferWindowPos(AHandle, FormulaEdit.Handle, 0, ARect.Left, ARect.Top,
          ARect.Width, ARect.Height, SWP_NOZORDER);
        FormulaEdit.LineHeight := ViewInfo.LineHeight;
      finally
...
begin
  ...
  AddButton('DXSPREADSHEET_FORMULABAR_INSERT',
    @sdxFormulaBarInsertFunctionHint);
end;

procedure TdxSpreadSheetAbstractFormulaBar.ProcessChanges
  (AChanges: TdxSpreadSheetFormulaBarChanges);
begin
...
begin
  ...
  ButtonBox.Enabled := FormulaEdit.Enabled and
    not FormulaEdit.Properties.ReadOnly;
  ...
  ButtonBox.Properties.Buttons[2].Enabled := ButtonBox.Enabled and
    CanExecuteInsertFunctionDialog;
  ButtonBox.Visible := OptionsView.ShowButtons;
end;

procedure TdxSpreadSheetAbstractFormulaBar.KeyDown(var Key: Word;
...
procedure TdxSpreadSheetAbstractFormulaBar.MouseDown(Button: TMouseButton;
...
procedure TdxSpreadSheetAbstractFormulaBar.MouseMove(Shift: TShiftState;
  X, Y: Integer);
const
...
function TdxSpreadSheetAbstractFormulaBar.StartDragAndDrop
...
procedure TdxSpreadSheetAbstractFormulaBar.UpdateDragAndDropObjectClass
  (const P: TPoint);
begin
...
procedure TdxSpreadSheetAbstractFormulaBar.ControllerKeyDown(var Key: Word;
  ShiftState: TShiftState);
begin
  // do nothing
end;
...
begin
  Result := dxDefaultBooleanToBoolean
    (OptionsBehavior.FormulaAutoComplete, True);
end;
...
begin
  Result := dxDefaultBooleanToBoolean
    (OptionsBehavior.FormulaAutoCompleteShowHint, True);
end;
...
procedure TdxSpreadSheetAbstractFormulaBar.SetOptionsBehavior
  (AValue: TdxSpreadSheetFormulaBarOptionsBehavior);
begin
  FOptionsBehavior.Assign(AValue);
end;

procedure TdxSpreadSheetAbstractFormulaBar.SetOptionsView
  (AValue: TdxSpreadSheetFormulaBarOptionsView);
begin
  FOptionsView.Assign(AValue);
end;

procedure TdxSpreadSheetAbstractFormulaBar.SetSeparatorPosition
  (AValue: Integer);
begin
  AValue := Max(AValue, 0);
...
procedure TdxSpreadSheetAbstractFormulaBar.HandlerAfterFocusChanged
  (Sender: TObject);
begin
  Changed([ssfbcState]);
end;

procedure TdxSpreadSheetAbstractFormulaBar.HandlerButtonClick(Sender: TObject;
...
procedure TdxSpreadSheetAbstractFormulaBar.HandlerFormulaEditChanged
  (Sender: TObject);
begin
  Changed([ssfbcState]);
end;

procedure TdxSpreadSheetAbstractFormulaBar.HandlerFormulaEditExit
  (Sender: TObject);
begin
  if TdxSpreadSheetTableViewEditingController.CanFinishEditingOnExit(FormulaEdit)
  then
    DoHideEdit(True, False, False);
end;

procedure TdxSpreadSheetAbstractFormulaBar.HandlerFormulaEditKeyDown
...
procedure TdxSpreadSheetAbstractFormulaBar.HandlerFormulaEditKeyPress
  (Sender: TObject; var Key: Char);
begin
  KeyPress(Key);
end;

procedure TdxSpreadSheetAbstractFormulaBar.HandlerFormulaEditKeyUp
  (Sender: TObject; var Key: Word; ShiftState: TShiftState);
begin
  KeyUp(Key, ShiftState);
end;

procedure TdxSpreadSheetAbstractFormulaBar.HandlerFormulaEditSelectionChanged
  (Sender: TObject);
begin
  // do nothing
end;
...
procedure TdxSpreadSheetAbstractFormulaBar.HandlerExpandButtonClick
  (Sender: TObject);
begin
  Expanded := not Expanded;
end;
...
procedure TdxSpreadSheetAbstractFormulaBar.DoHideEdit(AAccept, AIsArrayFormula,
  ACanChangeFocus: Boolean);
begin
  Changed([ssfbcState]);
end;
...
procedure TdxSpreadSheetAbstractFormulaBar.DXMContainerSetFocus
  (var AMessage: TMessage);
begin
  HandlerAfterFocusChanged(TObject(AMessage.LPARAM));
end;

procedure TdxSpreadSheetAbstractFormulaBar.DrawOpaqueBackground
...
constructor TdxSpreadSheetFormulaBarCustomOptions.Create
  (AOwner: TdxSpreadSheetAbstractFormulaBar);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
begin
  ...
  begin
    FormulaAutoComplete := TdxSpreadSheetFormulaBarOptionsBehavior(Source)
      .FormulaAutoComplete;
    FormulaAutoCompleteShowHint := TdxSpreadSheetFormulaBarOptionsBehavior
      (Source).FormulaAutoCompleteShowHint;
  end;
end;
...
function TdxSpreadSheetFormulaBarInplaceEdit.GetController
  : IdxSpreadSheetInplaceEditController;
begin
  Result := FBar;
end;

function TdxSpreadSheetFormulaBarInplaceEdit.GetScrollBarClass
...
function TdxSpreadSheetFormulaBarInplaceEditScrollBar.GetHelperClass
  : TcxScrollBarHelperClass;
begin
  Result := TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper;
end;

procedure TdxSpreadSheetFormulaBarInplaceEditScrollBar.SetEnabled
  (Value: Boolean);
begin
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper.
  DoExpandButtonClick;
begin
  InplaceEdit.DoExpandButtonClick;
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper.GetControllerClass
  : TcxScrollBarControllerClass;
begin
  Result := TdxSpreadSheetFormulaBarInplaceEditScrollBarController;
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper.GetInplaceEdit
  : TdxSpreadSheetFormulaBarInplaceEdit;
begin
  Result := ((Owner.GetControl as TdxSpreadSheetFormulaBarInplaceEditScrollBar)
    .OwnerControl as TdxSpreadSheetFormulaBarInplaceEdit);
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper.
  GetExpandButtonHeight: Integer;
begin
  Result := InplaceEdit.LineHeight - 2 * LookAndFeelPainter.BorderSize;
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper.GetPainterClass
  : TcxScrollBarPainterClass;
begin
  Result := TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter;
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper.GetViewInfoClass
...
constructor TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo.Create
  (AScrollBar: TcxScrollBarHelper);
begin
  inherited;
...
function TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo.
  GetOriginalBounds: TRect;
begin
  Result := inherited GetBounds;
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo.GetScrollBar
  : TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper;
begin
  Result := inherited ScrollBar as
    TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper;
end;
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarController.DoMouseLeave
  (AControl: TControl);
begin
  if ExpandButtonState <> cxbsDisabled then
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarController.DoMouseMove
  (Shift: TShiftState; X, Y: Integer);
const
...
function TdxSpreadSheetFormulaBarInplaceEditScrollBarController.
  IsMouseOverExpandButton(X, Y: Integer): Boolean;
begin
  Result := PtInRect(ViewInfo.ExpandButtonRect, cxPoint(X, Y));
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarController.
  GetExpandButtonState: TcxButtonState;
begin
  Result := ViewInfo.FExpandButtonState;
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarController.GetScrollBar
  : TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper;
begin
  Result := TdxSpreadSheetFormulaBarInplaceEditScrollBarHelper
    (inherited ScrollBar);
...
function TdxSpreadSheetFormulaBarInplaceEditScrollBarController.GetViewInfo
  : TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo;
begin
  Result := TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo
    (inherited ViewInfo);
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarController.
  SetExpandButtonState(AValue: TcxButtonState);
begin
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter.Paint
  (ACanvas: TcxCanvas);
begin
  inherited;
  DrawExpandButton(ACanvas, ViewInfo.ExpandButtonRect,
    ViewInfo.ExpandButtonState, not cxRectIsEmpty(ViewInfo.ScrollBarBounds));
end;
...
procedure TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter.DrawExpandButton
  ...
  AExpanded: Boolean);
begin
  LookAndFeelPainter.DrawSpreadSheetFormulaBarScaledExpandButton(ACanvas, R,
    AState, AExpanded, ScaleFactor);
end;

function TdxSpreadSheetFormulaBarInplaceEditScrollBarPainter.GetViewInfo
  : TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo;
begin
  Result := TdxSpreadSheetFormulaBarInplaceEditScrollBarViewInfo
...
procedure TdxSpreadSheetFormulaBarSplitterDragHandler.DragAndDrop
...
begin
  ...
  if Control.IsRightToLeft then
    ADelta := -ADelta;
  Control.SeparatorPosition :=
    Max(0, Min(FSeparatorPosition + ADelta, FSeparatorPositionMax));
  Control.Update;
end;

function TdxSpreadSheetFormulaBarSplitterDragHandler.GetControl
  : TdxSpreadSheetAbstractFormulaBar;
begin
...
constructor TdxSpreadSheetFormulaBarViewInfo.Create
  (ABar: TdxSpreadSheetAbstractFormulaBar);
begin
  inherited Create;
  FBar := ABar;
end;
...
begin
  ...
  begin
    ...
    FSeparatorRect := Place(ABounds,
      Painter.SpreadSheetFormulaBarGetScaledSeparatorSize(ScaleFactor));
  ...
  begin
    ...
    FButtonBoxRect := Place(ABounds, 2 * Painter.BorderSize + AButtons.Count *
      AButtonWidth);
  end;
...
function TdxSpreadSheetFormulaBarViewInfo.CalculateMaxSeparatorPosition
  : Integer;
begin
  Result := Bar.ClientWidth - cxRectWidth(ButtonBoxRect) -
    cxRectWidth(SeparatorRect);
end;

procedure TdxSpreadSheetFormulaBarViewInfo.Draw(ACanvas: TcxCanvas);
begin
  Painter.DrawSpreadSheetFormulaBarScaledSeparator(ACanvas, SeparatorRect,
    ScaleFactor);
end;

function TdxSpreadSheetFormulaBarViewInfo.MeasureLineHeight: Integer;

  procedure AdjustLineHeight(ACanvas: TcxCanvas; AEdit: TcxCustomEditAccess;
    var AHeight: Integer);
  var
  ...
  begin
    ...
    try
      AHeight := Max(AHeight, AViewData.GetEditSize(ACanvas, AEdit.EditValue,
        ASizeProperties, AEdit.ViewInfo).cy);
    finally
      AViewData.Free;
    end;
  end;
...
function TdxSpreadSheetFormulaBarViewInfo.GetPainter
...
procedure TdxSpreadSheetCustomFormulaBar.HandlerAfterFocusChanged
  (Sender: TObject);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.HandlerFormulaEditChanged
  (Sender: TObject);
begin
  inherited;
...
procedure TdxSpreadSheetCustomFormulaBar.HandlerFormulaEditSelectionChanged
  (Sender: TObject);
begin
  inherited;
  ...
  if FormulaEdit.Focused and (EditingController <> nil) then
  begin
    if (EditingController.Edit is TcxCustomTextEdit) and EditingController.Edit.HandleAllocated
    then
      TcxCustomTextEdit(EditingController.Edit).SelStart :=
        FormulaEdit.SelStart;
  end;
end;
...
procedure TdxSpreadSheetCustomFormulaBar.DoCreateDefinedName(const AArea: TRect;
  const AName, ASheetName: string);
begin
  SpreadSheet.DefinedNames.Add(AName, dxReferenceToString(AArea, False,
    [croSheetName], ASheetName));
end;
...
procedure TdxSpreadSheetCustomFormulaBar.DoHideEdit(AAccept, AIsArrayFormula,
  ACanChangeFocus: Boolean);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.DoRenameContainer
  (AContainer: TdxSpreadSheetContainer; const AName: string);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.DoSelectArea
  (const AView: TdxSpreadSheetTableView; const AArea: TRect);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.DoSelectContainer
  (AContainer: TdxSpreadSheetContainer);
begin
  AContainer.Focused := True;
end;

procedure TdxSpreadSheetCustomFormulaBar.DoSelectNamedObject
  (const AText: string; AObject: TObject);
var
  AArea: TRect;
  AContainer: TdxSpreadSheetContainer;
begin
  ...
  begin
    ...
    if AContainer <> nil then
      ...
      DoCreateDefinedName(ActiveSheet.Selection.Area, AText,
        ActiveSheet.Caption);
  end;
...
procedure TdxSpreadSheetCustomFormulaBar.ControllerKeyDown(var Key: Word;
ShiftState: TShiftState);
var
  AController: IdxSpreadSheetInplaceEditController;
begin
  if Supports(EditingController, IdxSpreadSheetInplaceEditController,
    AController) then
    AController.KeyDown(Key, ShiftState);
end;

procedure TdxSpreadSheetCustomFormulaBar.Notification(AComponent: TComponent;
...
function TdxSpreadSheetCustomFormulaBar.
  IsAutoCompleteSuggestionsHintsAllowed: Boolean;
begin
  Result := dxDefaultBooleanToBoolean
    (OptionsBehavior.FormulaAutoCompleteShowHint, (SpreadSheet <> nil) and
    SpreadSheet.OptionsBehavior.FormulaAutoCompleteShowHint);
end;
...
begin
  Result := inherited or (EditingController <> nil) and
...
procedure TdxSpreadSheetCustomFormulaBar.SynchronizeEditingValues(ATarget,
  ASource: TcxCustomEdit);
...
{$IFDEF DXSPREADSHEETFORMULABAR_RTF}
    Result := dxSpreadSheetTextService.IsRTFSupported and
      (ATarget is TcxCustomRichEdit) and not TcxCustomRichEdit(ATarget)
...
{$ENDIF}
  ...
  function IsFormattedValue(const AValue: TcxEditValue;
  const APlainValue: string): Boolean;
  begin
...
begin
  ...
  begin
    ASourceValue := TdxSpreadSheetTableViewEditingController.
      GetEditingText(ASource);
    if IsFormattingSupported(ATarget) and IsFormattedValue(ASource.EditValue,
      ASourceValue) then
...
begin
  ...
  begin
    ...
    FormulaEdit.EditValue := EditingController.GetValueFromCell
      (ActiveSheet.Cells[ACellPosition.Y, ACellPosition.X],
      FormulaEdit.IsFormattingSupported);
  end
  else
...
begin
  ...
  begin
    ...
    if TdxSpreadSheetDefinedNameHelper.FindByArea(SpreadSheet.DefinedNames,
      ActiveSheet, AArea, ADefinedName) then
      NameBox.ItemIndex := NameBox.Properties.Items.IndexOfObject(ADefinedName)
    else if (ActiveSheet.Controller.SelectionMode <> smNone) and
      not dxSpreadSheetIsSingleCellArea(AArea) then
    begin
...
procedure TdxSpreadSheetCustomFormulaBar.DataChanged
...
procedure TdxSpreadSheetCustomFormulaBar.ActiveSheetChanged
  (Sender: TdxCustomSpreadSheet);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.DefinedNamesChanged
  (Sender: TdxCustomSpreadSheet);
begin
  PopulateNameBox;
end;

procedure TdxSpreadSheetCustomFormulaBar.Edited
...
procedure TdxSpreadSheetCustomFormulaBar.Editing
  (Sender: TdxSpreadSheetCustomView);
begin
  Changed([ssfbcState]);
  UpdateReferencesHighlighting;
...
procedure TdxSpreadSheetCustomFormulaBar.EditingValueChanged
  (Sender: TdxSpreadSheetCustomView);
begin
...
procedure TdxSpreadSheetCustomFormulaBar.SelectionChanged
  (Sender: TdxSpreadSheetCustomView);
begin
...
begin
  ASheet := ActiveSheet;
  if ASheet <> nil then
    Result := ASheet.MergedCells.ExpandArea(ASheet.Selection.FocusedColumn,
      ASheet.Selection.FocusedRow).TopLeft
  else
    Result := cxNullPoint;
end;

function TdxSpreadSheetCustomFormulaBar.GetFocusedContainer
...
function TdxSpreadSheetCustomFormulaBar.GetEditingController
  : TdxSpreadSheetEditingController;
var
  AView: TdxSpreadSheetTableView;
begin
  AView := ActiveSheet;
  if AView <> nil then
    Result := TdxSpreadSheetEditingController
      (TdxSpreadSheetTableViewAccess(AView).EditingController)
  else
    Result := nil;
end;

procedure TdxSpreadSheetCustomFormulaBar.SetSpreadSheet
  (AValue: TdxCustomSpreadSheet);
begin
