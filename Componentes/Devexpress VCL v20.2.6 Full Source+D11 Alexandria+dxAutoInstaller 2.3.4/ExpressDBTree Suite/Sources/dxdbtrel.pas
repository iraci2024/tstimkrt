...
type
  ...
  private
    ...
    procedure AllowChangeTreeNode(Sender: TObject; Node: TTreeNode;
  ...
  published
    property CanSelectParents: Boolean read FCanSelectParents
      write FCanSelectParents;
    property Color;
    property Ctl3D;
    property DragCursor;
    property DragMode;
    ...
    property TreeViewCursor: TCursor read GetTreeViewCursor
      write SetTreeViewCursor;
    ...
    property TreeViewImages: TCustomImageList read GetTreeViewImages
      write SetTreeViewImages;
    property TreeViewIndent: Integer read GetTreeViewIndent
      write SetTreeViewIndent;
    property TreeViewPopupMenu: TPopupMenu read GetTreeViewPopupMenu
      write SetTreeViewPopupMenu;
    property TreeViewReadOnly: Boolean read GetTreeViewReadOnly
      write SetTreeViewReadOnly;
    property TreeViewShowButtons: Boolean read GetTreeViewShowButtons
      write SetTreeViewShowButtons;
    property TreeViewShowHint: Boolean read GetTreeViewShowHint
      write SetTreeViewShowHint;
    property TreeViewShowLines: Boolean read GetTreeViewShowLines
      write SetTreeViewShowLines;
    property TreeViewShowRoot: Boolean read GetTreeViewShowRoot
      write SetTreeViewShowRoot;
    property TreeViewSortType: TSortType read GetTreeViewSortType
      write SetTreeViewSortType;
    property TreeViewStateImages: TCustomImageList read GetTreeViewStateImages
      write SetTreeViewStateImages;
    property Visible;
    property OnCloseUp: TdxTreeViewCloseUp read FOnCloseUp write FOnCloseUp;
    property OnClick;
    property OnCustomDraw: TTreeViewCustomDraw read GetCustomDraw
      write SetCustomDraw;
    property OnDragDrop;
    property OnDragOver;
  ...
  public
    ...
    function GetTreeNodeByText(ParentTreeNode: TTreeNode; Text: string;
      flag: Boolean): TTreeNode;
  ...
  published
    ...
    property Options: TdxDBTreeViewOptions read GetOptions write SetOptions
      default [];
    ...
    property StateIndexField: string read FStateIndexFieldName
      write FStateIndexFieldName;
    property TextStyle;
    property OnAddNewItem: TAddNewDBTreeNodeEvent read GetAddNewItem
      write SetAddNewItem;
...
function DBTrDataSetLocate(DataSet: TDataSet; const AFieldName: string;
  AValue: Variant; AOptions: TLocateOptions): Boolean;
begin
...
begin
  with Message do
    if (Msg = WM_KEYDOWN) and ((wParam = VK_ESCAPE) or (wParam = VK_RETURN))
    then
...
begin
  ...
  FTreeView.ControlStyle := FTreeView.ControlStyle +
...
begin
  ...
  begin
    ...
    cxLookAndFeelPaintersManager.GetPainter(lfsFlat)
      .CalculateScaledPopupPanelClientRect(AClientRect, APanelRect, FGripRect,
      FCloseButtonRect, FCorner, cxSimpleRect, GetSysPanelBorders,
      dxSystemScaleFactor);
...
begin
  with Message do
    ...
    begin
      ...
      FMouseAboveCloseButton := PtInRect(FCloseButtonRect,
        SmallPointToPoint(Pos));
...
begin
  if FTreeViewEdit.FListVisible then
    cxLookAndFeelPaintersManager.GetPainter(lfsFlat).DrawScaledPopupNCPanel
      (FTreeView.Handle, FMouseAboveCloseButton, FCloseButtonIsTracking,
      FCorner, FCloseButtonRect, FGripRect, clWindowFrame, dxSystemScaleFactor);
end;
...
var
  ANode: TTreeNode;
begin
  if not cxIsDestroying(Self, True) and (VirtualTreeView <> nil) and
    (VirtualTreeView.Items.Count > 0) and (FMemStream = nil) then
  begin
...
begin
  ...
  begin
    if not FDataLink.Editing then
      FDataLink.Edit;
    if (FDataLink.DataSet.State = dsInsert) or (FDataLink.DataSet.State = dsEdit)
    then
...
begin
  Result := '';
  if (FDataLink.Field = nil) or (FKeyField = nil) or (FListField = nil) or
    (FParentField = nil) then
    Exit;
  if ListLink.Active then
  begin
    ListLink.DataSet.DisableControls;
    if FAssignField <> nil then
      b := DBTrDataSetLocate(ListLink.DataSet, FAssignFieldName,
        FDataLink.Field.Value, [])
    else
      b := DBTrDataSetLocate(ListLink.DataSet, FKeyFieldName,
        FDataLink.Field.Value, []);
    if b then
    begin
      ...
      begin
        ...
        while DBTrDataSetLocate(ListLink.DataSet, FKeyFieldName,
          FParentField.Value, []) and not VarEquals(FParentField.Value,
          FKeyField.Value) do
...
begin
  if FDataLink.Active and ListLink.Active and
    (FDataLink.DataSet.IsLinkedTo(ListSource) or
    ListLink.DataSet.IsLinkedTo(DataSource)) then
...
begin
  CreateParams(Var1);
  SetRect(R, 0, 0, 0, 0);
  AdjustWindowRectEx(R, Var1.Style,
    False, Var1.ExStyle);
  Result := R.Bottom - R.Top;
end;

procedure TCustomdxVTreeViewEdit.AllowChangeTreeNode(Sender: TObject;
...
var
  p: TPoint;
begin
  if (VirtualTreeView.Selected <> nil) and
    CanSelectTreeNode(VirtualTreeView.Selected) then
  begin
    p := GetMouseCursorPos;
    Windows.ScreenToClient(VirtualTreeView.Handle, p);
    if ((VirtualTreeView.GetHitTestInfoAt(p.X, p.Y) * [htOnItem, htOnIcon,
      htOnLabel, htOnStateIcon] <> []) and (VirtualTreeView.GetNodeAt(p.X,
      p.Y) = VirtualTreeView.Selected)) then
      CloseUp(True);
  end;
end;
...
begin
  ...
  begin
    ...
    FDropDownRows := (VirtualTreeView.Height - 7 - TreeBorderHeight)
      div GetTextHeight;
    if FDropDownRows < 2 then
      FDropDownRows := 2;

    if GetCapture = VirtualTreeView.Handle then
      ReleaseCapture;
    FListVisible := False;
    if IsWindowVisible(VirtualTreeView.Handle) then
    begin
      SetWindowPos(VirtualTreeView.Handle, 0, 0, 0, 0, 0, SWP_NOZORDER or
    ...
    if Accept and not IsReadOnly then
      DropDownOk;
...
begin
  ...
  begin
    ...
    if p.X < AWorkArea.Left then
      p.X := AWorkArea.Left;
    if p.X + AWidth > AWorkArea.Right then
      Dec(p.X, p.X + AWidth - AWorkArea.Right);
    if p.Y < AWorkArea.Top then
      p.Y := AWorkArea.Top;
    if p.Y + AHeight > AWorkArea.Bottom then
      Dec(p.Y, Height + AHeight);
    ...
    SendMessage(GetParentWinControl(TWinControl(Owner)).Handle, WM_NCACTIVATE,
      wParam(True), 0);
    Repaint;
  end;
end;
...
procedure TCustomdxVTreeViewEdit.MouseDown(Button: TMouseButton;
  ...
  begin
    ...
    begin
      ...
      if PtInRect(R, ListPos) then
        SendMessage(VirtualTreeView.Handle, WM_LBUTTONDOWN, 0,
          dxPointToLParam(ListPos))
      else
        CloseUp(False);
    end;
  end;
...
procedure TCustomdxVTreeViewEdit.MouseUp(Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  if FPressed then
  begin
...
begin
  ...
  case FAlignment of
    taRightJustify:
      X := W - Canvas.TextWidth(AText) - 3;
    taCenter:
      X := (W - Canvas.TextWidth(AText)) div 2;
  end;
...
procedure TCustomdxVTreeViewEdit.SetBounds(ALeft, ATop, AWidth,
  AHeight: Integer);
begin
...
procedure TCustomdxVTreeViewEdit.SetTreeViewStateImages
...
begin
  with Message do
    ...
    begin
      if (Msg = WM_DESTROY) and (Owner <> nil) and not cxIsDestroying(Self, True)
      then
