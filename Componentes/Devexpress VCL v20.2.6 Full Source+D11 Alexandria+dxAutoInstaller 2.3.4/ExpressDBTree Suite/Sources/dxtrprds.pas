...
begin
  if (FRootStrValue <> Value) then
  begin
    FRootStrValue := Value;
    if (FRootStrValue = '') then
      FRootValue := NULL
    else
      FRootValue := FRootStrValue;
    if Not(csLoading in ComponentState) And Active then
    begin
      Close;
      Open;
...
begin
  ...
  if (FDataLink.DataSet <> Nil) and FDataLink.DataSet.Active And
    (FDataLink.DataSet.FindField(FKeyFieldName) <> Nil) And
    (FDataLink.DataSet.FindField(FParentFieldName) <> Nil) then
...
type
  ...
  public
    constructor Create(AOwner: TDBTreePrintedNodes;
      AKeyValue, AParentValue: Variant);
    destructor Destroy; override;
...
{ TDBTreePrintedNode }
constructor TDBTreePrintedNode.Create(AOwner: TDBTreePrintedNodes;
  AKeyValue, AParentValue: Variant);
Var
  Index: Integer;
begin
...
begin
  ...
  if (Parent <> nil) and not Parent.Destroying and (Parent.ChildList <> nil)
  then
  begin
...
{ TDBTreePrintedNodes }
constructor TDBTreePrintedNodes.Create(APrintedDS: TdxDBTreePrintData;
  ASourceDS: TDataSet);
begin
...
begin
  ...
  begin
    ...
    if (KeyField = nil) Or (ParentField = nil) then
      Exit;
    ...
    begin
      ...
      while Not EOF do
      begin
        if (SortedList[FRecNumber - 1] <> nil) then
        begin
          ...
          PrintedDS.FindField(cDBTreeLevelFieldName).AsInteger :=
            TDBTreePrintedNode(SortedList[FRecNumber - 1]).Level;
          PrintedDS.FindField(cDBTreeRecNoFieldName).AsInteger :=
            TDBTreePrintedNode(SortedList[FRecNumber - 1]).RecNumber;
          if (PrintedDS.FMaxLevelCount < TDBTreePrintedNode
            (SortedList[FRecNumber - 1]).Level) then
            PrintedDS.FMaxLevelCount :=
              TDBTreePrintedNode(SortedList[FRecNumber - 1]).Level;
          PrintedDS.FindField(cDBTreeHasChildrenFieldName).AsBoolean :=
            TDBTreePrintedNode(SortedList[FRecNumber - 1]).ChildList <> Nil;
          PrintedDS.Post;
        end;
        Next;
        Inc(FRecNumber);
...
function TDBTreePrintedNodes.FindNearest(Value: Variant;
  Var Index: Integer): Boolean;
var
  Min, Max: LongInt;
begin
  Result := False;
  if (List.Count = 0) Or VarIsNull(Value) Or
    VarFirstMoreEx(TDBTreePrintedNode(List[0]).KeyValue, Value) then
  begin
    Index := -1;
    Exit;
  end;

  if VarFirstMoreEx(Value, GetMaxKeyValue) then
  begin
    Index := List.Count;
    Exit;
  end;
  ...
  repeat
    ...
    if VarEquals(Value, TDBTreePrintedNode(List[Index]).KeyValue) then
      break;
    if VarFirstMore(Value, TDBTreePrintedNode(List[Index]).KeyValue) then
      ...
      Max := Index;
  until (Min = Max);
  if Not VarEquals(Value, TDBTreePrintedNode(List[Index]).KeyValue) then
  begin
    if (Index < List.Count - 1) And
      VarFirstMore(Value, TDBTreePrintedNode(List[Index]).KeyValue) then
      Inc(Index);
  end
  else
    Result := True;
...
begin
  ...
  begin
    ...
    if (i < RootList.Count) And (OldCount = RootList.Count) And
      (TDBTreePrintedNode(RootList[i]).Parent = Nil) then
      Inc(i);
  end;

  if Not VarIsNull(PrintedDS.FRootValue) And FindNearest(PrintedDS.FRootValue,
    Index) then
  begin
  ...
  if (PrintedDS.FLevelCount > 0) then
  begin
    i := 0;
    Index := PrintedDS.FLevelCount - 1;
    while (i < List.Count) do
    begin
      if (TDBTreePrintedNode(List[i]).Level >= Index) then
      begin
        prNode := TDBTreePrintedNode(List[i]);
        if (prNode.ChildList <> Nil) then
        begin
...
procedure TDBTreePrintedNodes.SortNodes(List: TList);
  ...
  begin
    ...
    repeat
      while GetValue(Lo) < Mid do
        Inc(Lo);
      while GetValue(Hi) > Mid do
        Dec(Hi);
      if Lo <= Hi then
      begin
        List.Exchange(Lo, Hi);
