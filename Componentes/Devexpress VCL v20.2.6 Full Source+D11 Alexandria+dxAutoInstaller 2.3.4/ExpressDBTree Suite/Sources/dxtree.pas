...
type
  ...
  protected
    ...
    procedure DoDragDropTreeNode(Destination, Source: TTreeNode;
      var Accept: Boolean); virtual;
    procedure DoDragDropIsCopy(Destination, Source: TTreeNode;
      var IsCopy: Boolean); virtual;
  ...
  public
    ...
    function MoveTreeNodeStructure(Source, Destination: TTreeNode;
      IsCopy: Boolean): TTreeNode;
    procedure UpdateSizeGripCorner; virtual;
    property DragSourceTreeNode: TTreeNode read GetDragSourceTreeNode;
    property IsConfirmationAssigned: Boolean read FIsConfirmationAssigned
      write FIsConfirmationAssigned;
  ...
  published
    ...
    property OnCustomDraw: TTreeViewCustomDraw read FOnCustomDraw
      write FOnCustomDraw;
    property OnDragDropIsCopy: TDragDropIsCopy read FOnDragDropIsCopy
      write FOnDragDropIsCopy;
    ...
    property DeletingConfirmationMessage: string
      read GetDeletingConfirmationMessage write SetDeletingConfirmationMessage
      stored FIsConfirmationAssigned;
    property DragKind;
    property HotTrack;
...
resourcestring
  ...
  dxDBTreeViewSmartLoadS =
    'Set trSmartRecordLoad in the Options property to decrease the loading time. Do you want to do it now?';
...
begin
  ...
  begin
    ...
    if Mode = naAddChildFirst then
      Dummy := TTreeView(Destination.TreeView).Items.AddChildFirst
        (Destination, '')
    else
...
function TCustomdxTreeViewDragObject.GetDragCursor(Accepted: Boolean;
  X, Y: Integer): TCursor;
begin
  if not Accepted then
    Result := crNoDrop
  else if not(GetKeyState(VK_CONTROL) < 0) then
...
procedure TCustomdxTreeView.DoDragDropIsCopy(Destination, Source: TTreeNode;
  var IsCopy: Boolean);
begin
...
procedure TCustomdxTreeView.SetDeletingConfirmationMessage
  (const AValue: string);
begin
...
var
  St: string;
begin
  if (Key = VK_DELETE) and (trCanDelete in Options) and not ReadOnly and
    (Selected <> nil) and not IsEditing then
  begin
...
begin
    ...
    case DragMessage of
      ...
      dmDragMove:
        begin
          ...
          begin
            if (OldDragOverTreeNode <> nil) and (OldDragOverTreeNode = TreeNode)
            then
            begin
              if (TreeNode = TopItem) or
                ((TreeNode.GetPrevVisible <> nil) and
                (TreeNode.GetPrevVisible = TopItem)) and
                (TopItem.GetPrevVisible <> nil) then
                TopItem := TopItem.GetPrevVisible
              else if ((TreeNode.GetNextVisible <> nil) and
                not IsNodeVisible(TreeNode.GetNextVisible) or
                ((TreeNode.GetNextVisible <> nil) and
                (TreeNode.GetNextVisible.GetNextVisible <> nil) and
                not IsNodeVisible(TreeNode.GetNextVisible.GetNextVisible))) then
      ...
      dmDragDrop:
        begin
          ...
          begin
            ...
            Invalidate;
            if (DropSourceTreeNode <> nil) and
              not OldDragOverTreeNode.HasAsParent(DropSourceTreeNode) and
              (OldDragOverTreeNode <> DropSourceTreeNode) then
            begin
              DoDragDropIsCopy(DropSourceTreeNode, OldDragOverTreeNode,
                IsCopyFlag);
              if ((OldDragOverTreeNode <> DropSourceTreeNode.Parent) or
                IsCopyFlag or Assigned(FOnEndDragTreeNode)) then
              begin
                CorrectDragFlag := True;
                DoDragDropTreeNode(OldDragOverTreeNode, DropSourceTreeNode,
                  CorrectDragFlag);
              end;
            end;
          end
          else if DropSourceTreeNode <> nil then
          begin
            DoDragDropIsCopy(DropSourceTreeNode, OldDragOverTreeNode,
              IsCopyFlag);
            DoDragDropTreeNode(OldDragOverTreeNode, DropSourceTreeNode,
          ...
          begin
            TreeNode := MoveTreeNodeStructure(DropSourceTreeNode,
              OldDragOverTreeNode, IsCopyFlag);
            if TreeNode <> nil then
            begin
  ...
  if (DropSourceTreeNode <> nil) and (Message.DragMessage <> dmDragDrop) and
    (Message.DragMessage <> dmDragCancel) and
    (OldDragOverTreeNode <> DropSourceTreeNode) then
  begin
    if OldDragOverTreeNode <> nil then
    begin
      CorrectDragFlag := (not DropSourceTreeNode.HasChildren or
        not OldDragOverTreeNode.HasAsParent(DropSourceTreeNode)) and
        not Assigned(OnDragOver);
      if CorrectDragFlag then
        DoDragDropTreeNode(OldDragOverTreeNode, DropSourceTreeNode,
          CorrectDragFlag);
    end
  ...
  if (Message.DragMessage = dmDragDrop) or (Message.DragMessage = dmDragCancel)
  then
  begin
    DropSourceTreeNode := nil;
...
function TCustomdxTreeView.MoveTreeNodeStructure(Source, Destination: TTreeNode;
  IsCopy: Boolean): TTreeNode;
var
  i, ind: Integer;
  ListS, ListD: TList;
...
begin
  ...
  begin
    ...
    begin
      if i = 0 then
        case AStyle of
          naAdd:
            tr := Items.Add(Destination,
              GetListItemText(TTreeNode(ListS.List[i])));
          naAddFirst:
            tr := Items.AddFirst(Destination,
              GetListItemText(TTreeNode(ListS.List[i])));
          naAddChild:
            tr := Items.AddChild(Destination,
              GetListItemText(TTreeNode(ListS.List[i])));
          naAddChildFirst:
            tr := Items.AddChildFirst(Destination,
              GetListItemText(TTreeNode(ListS.List[i])));
          naInsert:
            tr := Items.Insert(Destination,
              GetListItemText(TTreeNode(ListS.List[i])));
        end
      else
      begin
        ind := ListS.IndexOf(TTreeNode(ListS.List[i]).Parent);
        tr := Items.AddChild(TTreeNode(ListD.List[ind]),
          GetListItemText(TTreeNode(ListS.List[i])))
      end;
      if tr <> nil then
        ListD.Add(tr);
  ...
  ListS.Free;
  ListD.Free;
  if (Source <> nil) and not TdxTreeView(Source.TreeView).ReadOnly and
    (Self <> Source.TreeView) and not IsCopy then
    Source.Free;
end;
...
begin
  ...
  if (Message.nmhdr^.code = NM_CUSTOMDRAW) and
    not(Assigned(OnAdvancedCustomDraw) or Assigned(OnAdvancedCustomDrawItem))
  then
  begin
    Result := False;
    ...
    begin
      ...
      if ((pnmlv^.nmcd.uItemState and TVGN_CARET = 0) and
        ...
        (OldDragOverTreeNode <> tr)) or not Focused then
      begin
