...
uses
  Windows, Classes, cxClasses, Forms, Types, DesignIntf, DesignEditors,
  VCLEditors, SysUtils, TypInfo,
  Generics.Collections, Generics.Defaults, dxCoreReg, cxLibraryReg,
...
{$ENDREGION}
  strict private
    ...
    class function EnumResNameProc(hModule: hModule; lpszType: LPCTSTR;
      lpszName: LPTSTR; AInfo: TPackageInfo): Boolean; stdcall; static;
    class procedure EnumPackageUnitsProc(const Name: string;
      NameType: TNameType; Flags: Byte; Info: TPackageInfo); static;
    class procedure PackagesNeeded;
    class procedure PopulatePackagesList;
    class function TryGetPackageInfo(const AFileName: string;
      out AInfo: TPackageInfo): Boolean;
  ...
  protected
    ...
    class function DoGetPainter(const ASkinName: string)
      : TcxCustomLookAndFeelPainter; static;
  public
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TdxSkinName), nil, 'SkinName',
    TdxSkinNameProperty);
  RegisterPropertyEditor(TypeInfo(string), TdxSkinController, 'SkinPaletteName',
    TdxSkinPaletteNameProperty);
...
var
  ...
  I: Integer;
begin
  if TdxSkinController.GetPainterData(AData) and
    (AData.Skin.ColorPalettes.Count > 0) then
  begin
  ...
  try
    ...
    begin
      ...
      if APainter.LookAndFeelStyle = lfsSkin then
        AValues.Add(APainter.LookAndFeelName + AValues.NameValueSeparator +
          cxGetUnitName(APainter.ClassType));
    end;
...
begin
  DoEnumSkins(procedure(const ASkinName, AUnitName: string)
    begin
      AList.Add(ASkinName);
    end);
end;

class function TdxSkinsPackageManager.DoGetPainter(const ASkinName: string)
...
class function TdxSkinsPackageManager.EnumResNameProc(hModule: hModule;
lpszType: LPCTSTR; lpszName: LPTSTR; AInfo: TPackageInfo): Boolean; stdcall;
var
...
class procedure TdxSkinsPackageManager.EnumPackageUnitsProc(const Name: string;
NameType: TNameType; Flags: Byte; Info: TPackageInfo);
begin
...
begin
  ...
  if FindFirst(APath + sdxSkinUnitPrefix + '*' + ASuffix + '.bpl', faAnyFile,
    ASearchRec) = 0 then
    try
      repeat
...
class function TdxSkinsPackageManager.TryGetPackageInfo(const AFileName: string;
out AInfo: TPackageInfo): Boolean;
var
  AFlags: Integer;
  ALibHandle: THandle;
begin
  ...
  begin
    ...
    try
      ...
      Windows.EnumResourceNames(ALibHandle, PChar(sdxResourceType),
        @EnumResNameProc, LPARAM(AInfo));
...
class function TdxSkinsPackageManager.TryLoadPackageForSkin(const ASkinName
  : string): Boolean;
var
