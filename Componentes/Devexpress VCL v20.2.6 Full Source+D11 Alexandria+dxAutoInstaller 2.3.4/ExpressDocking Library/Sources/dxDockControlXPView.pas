...
begin
  Result := Max(inherited GetCaptionAreaContentHeight,
    TdxTextMeasurer.TextLineHeight(GetFont));
end;

function TdxDockControlXPPainter.GetCaptionHeight: Integer;
begin
  if OpenTheme(totTab) <> 0 then
    Result := Max(2 * ScaleFactor.Apply(7) + GetFontSize,
      2 * ScaleFactor.Apply(2) + GetImageHeight)
  else
...
procedure TdxDockControlXPPainter.DrawCaption(ACanvas: TcxCanvas; ARect: TRect;
  IsActive: Boolean);
var
  ATheme: TdxTheme;
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        dxDrawThemeBackground(ACanvas.Handle, totExplorerBar, ScaleFactor,
          ARect, EBP_NORMALGROUPBACKGROUND, 0);
      finally
    ...
    dxDrawThemeBackground(ACanvas.Handle, totButton, ScaleFactor, ARect,
      BP_GROUPBOX, 0);
  end
  else
    inherited;
...
procedure TdxDockControlXPPainter.DrawCaptionButton(ACanvas: TcxCanvas;
  ARect: TRect; AIsActive: Boolean; AState: TcxButtonState);
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totToolBar, ScaleFactor, ARect,
    BP_PUSHBUTTON, BtnStateToXPBtnState(AState)) then
    inherited DrawCaptionButton(ACanvas, ARect, AIsActive, AState);
end;

procedure TdxDockControlXPPainter.DrawCaptionSeparator(ACanvas: TcxCanvas;
  ARect: TRect);
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totTab, ScaleFactor, ARect,
    TABP_BODY, 0) then
    inherited;
end;

procedure TdxDockControlXPPainter.DrawCaptionCloseButton(ACanvas: TcxCanvas;
  ARect: TRect; AIsActive: Boolean; AState: TcxButtonState);
const
  StateMap: array [TcxButtonState] of Integer = (CBS_NORMAL, CBS_NORMAL,
    CBS_HOT, CBS_PUSHED, CBS_DISABLED);
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totWindow, ScaleFactor, ARect,
    WP_SMALLCLOSEBUTTON, StateMap[AState]) then
...
const
  StateMap: array [TcxButtonState] of Integer = (EBHP_NORMAL, EBHP_NORMAL,
    EBHP_HOT, EBHP_PRESSED, EBHP_NORMAL);
var
  ABitmap: TcxBitmap;
...
begin
  ...
  begin
    ...
    try
      DrawThemeBackground(ATheme, ABitmap.Canvas.Handle, EBP_HEADERPIN,
        StateMap[AState], ABitmap.ClientRect);
      if not IsSwitched then
        ABitmap.Rotate(raPlus90);
      ARotatedBitmap := TcxBitmap.CreateSize(cxRectInflate(ARect, -1,
        -1), pf24bit);
      try
...
procedure TdxDockControlXPPainter.DrawCaptionMaximizeButton(ACanvas: TcxCanvas;
  ARect: TRect; IsActive, IsSwitched: Boolean; AState: TcxButtonState);
const
  MaximizeButtonStateMap: array [TcxButtonState] of Integer = (MAXBS_NORMAL,
    MAXBS_NORMAL, MAXBS_HOT, MAXBS_PUSHED, MAXBS_DISABLED);
  RestoreButtonStateMap: array [TcxButtonState] of Integer = (RBS_NORMAL,
    RBS_NORMAL, RBS_HOT, RBS_PUSHED, RBS_DISABLED);
var
  ATheme: TdxTheme;
...
procedure TdxDockControlXPPainter.DrawCaptionText(ACanvas: TcxCanvas;
...
procedure TdxDockControlXPPainter.DrawHideBar(ACanvas: TcxCanvas; ARect: TRect;
...
begin
  ...
  begin
    ...
    try
      DrawThemeBackground(ATheme, ABitmap.Canvas.Handle, TABP_TOPTABITEM,
        TTIS_NORMAL, R);
      case APosition of
        ahpTop:
      ...
      cxBitBlt(ACanvas.Handle, ABitmap.Canvas.Handle, AButton.Bounds,
        cxNullPoint, SRCCOPY);
    finally
      ABitmap.Free;
    end;
...
const
  ArrowButtonPartIdMap: array [TcxArrowDirection] of Integer = (SPNP_UP,
    SPNP_DOWN, SPNP_DOWNHORZ, SPNP_UPHORZ);
  ArrowButtonStateIdMap: array [TcxArrowDirection, TcxButtonState]
    of Integer = ((UPS_NORMAL, UPS_NORMAL, UPS_HOT, UPS_PRESSED, UPS_DISABLED),
...
begin
  if not dxDrawThemeBackground(ACanvas.Handle, totSpin, ScaleFactor, ARect,
    ArrowButtonPartIdMap[AArrow], ArrowButtonStateIdMap[AArrow, AState]) then
...
class function TdxDockControlXPPainter.HasLookAndFeelStyle
