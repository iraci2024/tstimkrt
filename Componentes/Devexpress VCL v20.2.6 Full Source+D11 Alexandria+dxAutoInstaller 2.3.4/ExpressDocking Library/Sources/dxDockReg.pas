...
procedure TdxDockDesignWindow.SelectionsChanged(const ASelection
...
begin
  ...
  begin
    if (ASelection[I] is TdxCustomDockControl) and
      (TdxCustomDockControl(ASelection[I]).ParentDockControl = nil) then
...
begin
  Result := TdxDockControlCustomCaptionButtons
    (TdxDockControlCaptionButton(GetComponent(0)).Collection.Owner).Images;
end;

procedure Register;
begin
  ForceDemandLoadState(dlDisable);
  RegisterComponents(dxDockProductPage, [TdxDockingManager, TdxDockPanel,
    TdxDockSite]);
...
begin
  if (FdxDockDesignWindow <> nil) and
    ((dxDockingController = nil) or (dxDockingController.DockControlCount = 1))
  then
