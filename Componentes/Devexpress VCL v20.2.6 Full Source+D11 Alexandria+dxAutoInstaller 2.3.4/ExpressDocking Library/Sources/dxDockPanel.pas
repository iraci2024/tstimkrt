...
uses
  ...
  dxCore, dxDockConsts, dxDockControl, cxControls, cxPC, cxGraphics,
  cxLookAndFeels;
...
type
  ...
  protected
    ...
    procedure DoUndock(const APoint: TPoint;
      ADockControl: TdxCustomDockControl); override;
  end;
  ...
  TdxDockPanel = class(TdxCustomDockControl, IcxTabControl,
  ...
  protected
    ...
    procedure AssignTabContainerSiteProperties
      (ASite: TdxTabContainerDockSite); override;
    ...
    property TabsController: TdxDockPanelTabControlController
  ...
  published
    ...
    property ShowSingleTab: Boolean read FShowSingleTab write SetShowSingleTab
      default False;
    property TabsProperties: TdxDockPanelTabControlProperties
...
function TdxDockPanelTabControlController.IsDockable
  (AControl: TdxCustomDockControl): Boolean;
begin
  Result := AControl <> nil;
end;

procedure TdxDockPanelTabControlController.DoUndock(const APoint: TPoint;
...
begin
  ...
  begin
    ...
    ADockControlAccess.RestoreDockPosition
      (ADockControlAccess.ScreenToWindow(APoint));
  end
  else
...
function TdxDockPanel.CanDockHost(AControl: TdxCustomDockControl;
...
begin
  if Active and not IsDesigning and CanFocusEx and
...
procedure TdxDockPanel.InvalidateRect(const R: TRect;
...
begin
  ...
  Filer.DefineProperty('TabPosition', TabsProperties.ReadOldTabsPosition,
    nil, False);
end;
...
begin
  ...
  if (disLoading in TdxDockingControllerAccess(Controller).FInternalState) and
    ...
    ((Height = 0) or (Width = 0) or (cxRectWidth(Result) = 0) or
    (cxRectHeight(Result) = 0)) then
...
procedure TdxDockPanel.UpdateControlDockZones(AControl: TdxCustomDockControl;
  AZoneWidth: Integer);
begin
  if not(doUseCaptionAreaToClientDocking in ControllerOptions) then
  begin
    if (TabContainer <> nil) and TdxTabContainerZone.ValidateDockZone(Self,
      AControl) then
      DockZones.Insert(0, TdxTabContainerZone.Create(TabContainer))
    else if TdxDockPanelClientZone.ValidateDockZone(Self, AControl) then
      DockZones.Insert(0, TdxDockPanelClientZone.Create(Self));
  end;
  inherited;
  if doUseCaptionAreaToClientDocking in ControllerOptions then
  begin
    if (TabContainer <> nil) and TdxTabContainerCaptionZone.ValidateDockZone
      (Self, AControl) then
      DockZones.Insert(0, TdxTabContainerCaptionZone.Create(TabContainer))
    else if TdxDockPanelCaptionClientZone.ValidateDockZone(Self, AControl) then
...
function TdxDockPanel.CanAcceptSideContainerItems
  (AContainer: TdxSideContainerDockSite): Boolean;
begin
  Result := True;
end;

procedure TdxDockPanel.AssignTabContainerSiteProperties
...
function TdxDockPanel.CanAcceptTabContainerItems
...
begin
  Result := (FloatDockSite = nil) and
    (AutoHide or ((TabContainer = nil) or ((TabContainer.ValidChildCount < 2)
    and not TabContainer.AutoHide)) and ((ParentDockControl = nil) or
    TdxCustomDockControlAccess(ParentDockControl).CanUndock(Self)));
end;
...
begin
  Result := ShowSingleTab and
    ((TabContainer = nil) or (TabContainer.ValidChildCount = 1));
end;
...
procedure TdxDockPanel.SetTabsProperties
  (AValue: TdxDockPanelTabControlProperties);
begin
  TabsProperties.Assign(AValue);
end;

procedure TdxDockPanel.HandlerDrawTab(AProperties
...
procedure TdxDockPanel.HandlerDrawTabEx(AProperties
  : TcxCustomTabControlProperties; ATab: TcxTab; AFont: TFont);
begin
