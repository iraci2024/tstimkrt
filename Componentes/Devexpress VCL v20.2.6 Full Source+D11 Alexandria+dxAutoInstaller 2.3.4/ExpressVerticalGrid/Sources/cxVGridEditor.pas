...
uses
  ...
  cxLookAndFeelPainters, cxButtons, cxDBVGrid, Menus, cxGraphics,
  cxLookAndFeels, cxControls, cxClasses,
  dxLayoutContainer, dxLayoutControl, dxLayoutControlAdapters, cxContainer,
  cxEdit, cxListBox, dxLayoutLookAndFeels;
...
begin
  ...
  if liCreateAll.Visible then
    liCreateAll.Enabled := TcxDBVerticalGrid(VerticalGrid)
      .DataController.Dataset <> nil;
end;
...
begin
  ...
  try
    ...
    for I := 0 to VerticalGrid.Rows.Count - 1 do
      lbRows.Items.AddObject(VerticalGrid.Rows[I].Name,
        VerticalGrid.Rows.Items[I]);
  finally
    ListBoxRestoreSelection(lbRows.InnerListBox, ASelection, AItemIndex,
      ATopIndex);
  end;
end;
...
procedure TcxVerticalGridEditor.SelectionsChanged(const ASelection
