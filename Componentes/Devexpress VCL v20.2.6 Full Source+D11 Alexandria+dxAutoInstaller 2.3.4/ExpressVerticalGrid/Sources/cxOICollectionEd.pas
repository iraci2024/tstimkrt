...
type
  ...
  private
    ...
    procedure AMDeferUpdate(var Msg: TMessage);
...
procedure cxShowCollectionEditor(AInspector: TcxCustomRTTIInspector;
  AComponent: TComponent; ACollection: TCollection; const PropertyName: string);
...
begin
  ...
  begin
    ...
    with Result do
      if (Inspector = AInspector) and (Component = AComponent) and
        (Collection = ACollection) and
        (CompareText(CollectionPropertyName, PropertyName) = 0) then
      begin
        Show;
        BringToFront;
        Exit;
  ...
  begin
    for I := 0 to cxCollectionEditorsList.Count - 1 do
      with TcxCollectionEditor(cxCollectionEditorsList[I]) do
        if Inspector = Self.Inspector then
          CloseEditor;
  end;
  inherited Destroy;
end;
...
begin
  ...
  cxShowCollectionEditorClass(Inspector, GetEditorClass, TComponent(Obj),
    TCollection(GetOrdValue), GetName, GetColOptions);
end;
...
var
  I, InsPos: Integer;
begin
  if (ListView1.SelCount = 0) or (ListView1.SelCount = Collection.Count) then
    Exit;

  InsPos := ListView1.Items.Count - 1;
  while not ListView1.Items[InsPos].Selected do
    Dec(InsPos);
  if InsPos < (ListView1.Items.Count - 1) then
    Inc(InsPos);
...
begin
  with TCollectionAccess(Collection) do
    if GetAttrCount < 1 then
      Result := Format('%d - %s',
        [ItemIndex, Collection.Items[ItemIndex].DisplayName])
    else
      Result := GetItemAttr(Index, ItemIndex);
end;
...
{$IFDEF RTTIMULTISELECTION}
  ...
  try
    ...
    if (List.Count = 0) or (List.Count > Collection.Count) then
      Exit;
    if not((List[0] = Component) or (List[0] = Collection) or
      (TcxCollectionEditor(List[0]).GetOwner = Collection)) then
      Exit;
...
var
  ...
  begin
    with TRegIniFile.Create(GetRegKey) do
      try
        ...
        acTextLabels.Checked := ReadBool(FCollectionClassName,
          'TextLabels', False);
  ...
  begin
    if (Collection <> nil) and
      ...
      (ListView1.Columns.Count <> TCollectionAccess(Collection).GetAttrCount))
      or ((ListView1.Columns.Count = 0) and (TCollectionAccess(Collection)
      .GetAttrCount < 1))) then
    begin
      ListView1.HandleNeeded;
      with TCollectionAccess(Collection) do
  ...
  begin
    Result := False;
    if ListItems.Count <> Items.Count then
      Exit;
...
procedure TcxCollectionEditor.ListView1Change(Sender: TObject; Item: TListItem;
  Change: TItemChange);
var
  Msg: TMsg;
begin
...
procedure TcxCollectionEditor.ListView1DragDrop(Sender, Source: TObject;
  X, Y: Integer);
var
  Item: TListItem;
  I, J, InsPos: Integer;
...
procedure TcxCollectionEditor.ListView1DragOver(Sender, Source: TObject;
  X, Y: Integer; State: TDragState; var Accept: Boolean);
var
  Item: TListItem;
begin
  Item := ListView1.GetItemAt(X, Y);
  Accept := (Item <> nil) and (Source = ListView1) and (not Item.Selected);
end;
...
procedure TcxCollectionEditor.ListView1KeyPress(Sender: TObject; var Key: Char);
begin
...
begin
  ...
  if Enabled then
    ...
    else if (Sender = acMoveUp) or (Sender = acMoveDown) then
