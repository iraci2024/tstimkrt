...
begin
  ...
  if Update then Perform(EM_SETRECTNP, 0, LPARAM(@R));
end;
...
begin
  ...
  FTextArea := R;
  Self.Perform(EM_SETRECTNP, 0, LPARAM(@R));
end;
...
procedure TbsSkinMemo2.WMNCPAINT(var Message: TMessage);
begin
end;
...
procedure TbsListBox.KeyPress(var Key: Char);
  ...
  begin
    if Length(FFilter) = 1
    then
      Idx := SendMessage(Handle, LB_FINDSTRING, ItemIndex, LPARAM((PChar(FFilter)))
    else
      Idx := SendMessage(Handle, LB_FINDSTRING, -1, LPARAM((PChar(FFilter)));
    if Idx <> LB_ERR then
    begin
      if MultiSelect then
      begin
...
begin
  ...
  if Length(Text) = 1
  then
    I := SendMessage(FListBox.ListBox.Handle, LB_FINDSTRING, ItemIndex, LPARAM(PChar(Text)))
  else
    I := SendMessage(FListBox.ListBox.Handle, LB_FINDSTRING, -1, LPARAM(PChar(Text)));
  if I >= 0
  then
    begin
      S1 := Text;
...
begin
  ...
  then
    begin
      if Length(S) = 1
      then
        I := SendMessage(FListBox.ListBox.Handle, LB_FINDSTRING, ItemIndex, LPARAM(PChar(S)))
      else
        I := SendMessage(FListBox.ListBox.Handle, LB_FINDSTRING, -1, LPARAM(PChar(S)));
    end
  else
    I := -1;
...
begin
  ...
  then
    begin
      I := SendMessage(FListBox.ListBox.Handle, LB_FINDSTRING, -1, LPARAM(PChar(FEdit.Text)));
      if I >= 0
      then
        if FAutoComplete
        then
