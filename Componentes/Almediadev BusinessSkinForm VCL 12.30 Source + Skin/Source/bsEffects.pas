...
{ TColor -> TbsColor }
begin
  TbsColorRec(Result).R := TbsColorRec(Color).B;
  TbsColorRec(Result).G := TbsColorRec(Color).G;
  TbsColorRec(Result).B := TbsColorRec(Color).R;
  TbsColorRec(Result).A := $FF;
end;

function ToRGB(Color32: Cardinal): Cardinal; inline;
{ TbsColor -> TColor }
begin
  TbsColorRec(Result).R := TbsColorRec(Color32).B;
  TbsColorRec(Result).G := TbsColorRec(Color32).G;
  TbsColorRec(Result).B := TbsColorRec(Color32).R;
  TbsColorRec(Result).A := TbsColorRec(Color32).A;
end;
...
begin
  for I := 0 to Count - 1 do
    if PbsColorRecArray(Src)[I].Color and not $FF000000 = Value then
      PbsColorRecArray(Src)[I].A := 0;
end;
...
begin
  ...
  begin
    if Value = 0 then
      FillChar(PbsColorArray(Src)[X1 + (J * W)], (X2 - X1) * SizeOf(TbsColor), 0)
    else if Value = $FFFFFFFF then
      FillChar(PbsColorArray(Src)[X1 + (J * W)], (X2 - X1) * SizeOf(TbsColor), $FF)
    else
      _FillLongword(@PbsColorArray(Src)[X1 + (J * W)], X2 - X1, Value);
  end;
end;
...
begin
  for I := 0 to Count - 1 do
    PbsColorRecArray(Src)[I].A := Alpha;
end;
...
begin
  ...
  if Y1 > Y2 then
    Y1 := Y2;
  for J := Y1 to Y2 - 1 do
    _FillAlpha(@PbsColorArray(Src)[X1 + (J * W)], X2 - X1, Alpha);
end;

function _PixelAlphaBlend(F, B: TbsColor): TbsColor; inline;
var
  FA: Byte;
  OneMinusFA: Byte;
begin
  if TbsColorRec(F).A = 0 then
  begin
    Result := B;
    Exit;
  end;

  if TbsColorRec(F).A = $FF then
  begin
    Result := F;
    Exit;
  end;
  ...
  Result := B;
  TbsColorRec(Result).R := MulDiv(TbsColorRec(F).R, FA, $FF) + MulDiv(TbsColorRec(B).R, OneMinusFA, $FF);
  TbsColorRec(Result).G := MulDiv(TbsColorRec(F).G, FA, $FF) + MulDiv(TbsColorRec(B).G, OneMinusFA, $FF);
  TbsColorRec(Result).B := MulDiv(TbsColorRec(F).B, FA, $FF) + MulDiv(TbsColorRec(B).B, OneMinusFA, $FF);
end;
...
begin
  for I := 0 to Count - 1 do
    PbsColorArray(Dst)[I] := _PixelAlphaBlend(PbsColorArray(Src)[I], PbsColorArray(Dst)[I]);
end;
