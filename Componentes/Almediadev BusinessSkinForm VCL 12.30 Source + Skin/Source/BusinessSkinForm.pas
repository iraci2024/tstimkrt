...
function TbsBusinessSkinForm.GetBoundsRect: TRect;
begin
...
begin
  if (not FSkinnableForm) or (not IsNullHeight and FIsW10 and (FSD <> nil) and not FSD.Empty and
     ...
     (FBorderLayer.FData <> nil) and FBorderLayer.FData.FullBorder) then
  begin
    if FRgn <> 0 then
    begin
      SetWindowRgn(FForm.Handle, 0, True);
  ...
  then
    begin
      ...
      if not IsNullRect(FData.ButtonsRect) then
        C.ObjectRect.Right := NewButtonsRect.Left - BSF.FSD.ButtonsOffset
      else
