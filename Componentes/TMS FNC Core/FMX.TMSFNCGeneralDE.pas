...
uses
  ...
  SysUtils
  {$IFDEF FNCLIB}
  ,FMX.TMSFNCStylesEditor
  {$ENDIF}
  ,FMX.TMSFNCStyles, FMX.TMSFNCCustomComponent, FMX.TMSFNCTypes,
  FMX.TMSFNCPersistence, FMX.TMSFNCUtils, DateUtils, FMX.Dialogs
  {$IFNDEF LCLWEBLIB}
  {$HINTS OFF}
  ...
  {$ENDIF}
  {$IFDEF VCLLIB}
  ,FMX.TMSFNCGraphics, FMX.TMSFNCGraphicsTypes, VCL.Forms
  {$ENDIF}
  {$IFDEF FMXLIB}
  ,FMX.Forms
  {$ENDIF}
...
begin
  inherited;
  Proc('FMX.TMSFNCTypes');
  Proc('FMX.TMSFNCUtils');
  Proc('FMX.TMSFNCGraphics');
  Proc('FMX.TMSFNCGraphicsTypes');
end;
{$ENDIF}
