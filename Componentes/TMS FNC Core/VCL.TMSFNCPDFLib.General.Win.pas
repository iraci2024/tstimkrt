...
uses
  Classes, Types, VCL.TMSFNCPDFCoreLibBase,
  VCL.Graphics
  {$IFDEF MSWINDOWS}
  ,Windows
  {$ENDIF}
...
function TTMSFNCGeneralPDFLibFontInitializer.CompressFontString(AValue: AnsiString): TStringStream;
var
  vDest: TStringStream;
  vSource: TStream;
...
{$IFDEF MSWINDOWS}
var
  used: TTMSFNCPDFGraphicsLibFontCharArray;
  I: Integer;
  v: Integer;
  subdt: PAnsiChar;
  subMem: cardinal;
...
{$IFDEF MSWINDOWS}
function TTMSFNCGeneralPDFLibFontInitializer.GetFontTableData(ADocHandle: HDC;
...
var
  ...
  id, gi: Integer;
  wl, nol: word;
  st: ^TTMSFNCGeneralPDFLibFontCmapArray absolute fd;
...
begin
  ...
  begin
    h := fd;
    {$IFDEF LCLLIB}
    inc({%H-}PtrUInt(fd), SizeOf(TTMSFNCGeneralPDFLibFontCmapHeader));
    {$ENDIF}
    {$IFNDEF LCLLIB}
    inc(NativeInt(fd), SizeOf(TTMSFNCGeneralPDFLibFontCmapHeader));
    {$ENDIF}
    off := 0;
    for i := 0 to h^.n - 1 do
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          {$IFNDEF LCLLIB}
          {$IF COMPILERVERSION >= 22}
          ec := pointer(NativeInt(@ft^.fmt) + SizeOf(TTMSFNCGeneralPDFLibFontCmapFormat4));
