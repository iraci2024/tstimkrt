...
uses
  Classes, FMX.TMSFNCCustomComponent, FMX.TMSFNCTypes,
  FMX.Forms, FMX.Controls, FMX.TMSFNCJSONWriter, FMX.TMSFNCPersistence
  {$IFDEF FMXLIB}
  ,FMX.Types
  {$ENDIF}
  ,TypInfo
...
type
  ...
  protected
    ...
    function GetControlsCount(AControl: TTMSFNCStateManagerControl): Integer;
...
uses
  FMX.TMSFNCGraphics, SysUtils, Math, Types, FMX.TMSFNCUtils,
  FMX.TMSFNCCustomControl, FMX.TMSFNCGraphicsTypes, FMX.Graphics
  {$IFDEF WEBLIB}
...
type
  ...
  TTMSFNCStateManagerJSONValueList = TObjectList<TTMSFNCStateManagerJSONValue>;
  ...
  TTMSFNCStateManagerRootJSONValueList = TObjectList<TTMSFNCStateManagerRootJSONValue>;
...
var
  ...
  begin
    ...
    for I := 0 to GetControlsCount(AControl) - 1 do
    begin
...
function TTMSFNCCustomStateManager.GetControlsCount(
  AControl: TTMSFNCStateManagerControl): Integer;
begin
  Result := 0;
  {$IFDEF FMXLIB}
  if AControl is TControl then
    Result := (AControl as TControl).ControlsCount
  {$ELSE}
  if AControl is TWinControl then
    Result := (AControl as TWinControl).ControlsCount
  {$ENDIF}
  else if AControl is TCustomForm then
    Result := (AControl as TCustomForm).ControlsCount;
end;
...
constructor TTMSFNCStateManagerItem.Create(Collection: TCollection);
var
  m: TTMSFNCCustomStateManager;
begin
  inherited;

  if Assigned(Collection) and Assigned((Collection as TTMSFNCStateManagerItems).FOwner) then
