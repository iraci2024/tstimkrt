...
unit FMX.TMSFNCPDFLib.General.Mac;
...
uses
  SysUtils
  {$IFDEF MACOS_OK}
  ,MacApi.CoreFoundation, MacApi.Foundation, MacApi.CoreText, MacApi.AppKit
  {$ENDIF}
  {$IFNDEF LCLLIB}
  ,zlib, Generics.Collections
  {$ENDIF}
  ;
...
begin
  ...
  begin
    Result.CapHeight := Round(CTFontGetCapHeight(FFontRef) * ft);
    Result.Ascent := Round(CTFontGetAscent(FFontRef) * ft);
    Result.Descent := Round(CTFontGetDescent(FFontRef) * ft);
    r := CTFontGetBoundingBox(FFontRef);
    Result.FontBox.Left := Round(r.origin.x * ft);
    Result.FontBox.Bottom := Round(r.origin.y * ft);
    Result.FontBox.Right := Round((r.origin.x + r.size.width) * ft);
    Result.FontBox.Top := Round((r.origin.y + r.size.height) * ft);
    Result.ItalicAngle := Round(CTFontGetSlantAngle(FFontRef)* ft);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SetLength(glyphs, Length(s));
          CTFontGetGlyphsForCharacters(AFontRef, PWideChar(s), @glyphs[0], Length(glyphs));

          for I := 1 to Length(s) do
          begin
            v := Ord(s[I]);
            g := glyphs[I - 1];
            if (v >= 32) and (g > 0) then
            begin
              FCharArray.c := FCharArray.c + 1;
              SetLength(FCharArray.v, FCharArray.c);
              SetLength(FCharWidths, FCharArray.c);
              FCharArray.v[FCharArray.c - 1] := I - 1;
              FCharWidths[FCharArray.c - 1].g := g;
            end;
          end;
