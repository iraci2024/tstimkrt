...
uses
  ...
  SysUtils
  {$IFDEF FNCLIB}
  ,LCLTMSFNCStylesEditor
  {$ENDIF}
  ,LCLTMSFNCStyles, LCLTMSFNCCustomComponent, LCLTMSFNCTypes,
  LCLTMSFNCPersistence, LCLTMSFNCUtils, DateUtils, Dialogs
  {$IFNDEF LCLWEBLIB}
  {$HINTS OFF}
  ...
  {$ENDIF}
  {$IFDEF VCLLIB}
  ,LCLTMSFNCGraphics, LCLTMSFNCGraphicsTypes, VCL.Forms
  {$ENDIF}
  {$IFDEF FMXLIB}
  ,Forms
  {$ENDIF}
...
begin
  ...
  Proc('LCLTMSFNCTypes');
  Proc('LCLTMSFNCUtils');
  Proc('LCLTMSFNCGraphics');
  Proc('LCLTMSFNCGraphicsTypes');
end;
{$ENDIF}
