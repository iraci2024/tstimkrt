...
begin
  ...
  CleanupRandom;
  // <-- RLebeau: why is this here and not in IdSSLOpenSSLHeaders.Unload()?
...
procedure TIdServerIOHandlerSSLOpenSSL.DoGetPasswordEx(var VPassword: String;
...
function TIdServerIOHandlerSSLOpenSSL.GetPassword(const AIsWrite
  : Boolean): string;
begin
...
function TIdServerIOHandlerSSLOpenSSL.GetIOHandlerSelf
...
function TIdServerIOHandlerSSLOpenSSL.MakeClientIOHandler
...
function TIdSSLIOHandlerSocketOpenSSL.Readable
...
procedure TIdSSLIOHandlerSocketOpenSSL.DoStatusInfoEx(const AsslSocket: PSSL;
  const AWhere, Aret: TIdC_INT; const AWhereStr, ARetStr: String);
begin
  if Assigned(FOnStatusInfoEx) then
  begin
...
procedure TIdSSLIOHandlerSocketOpenSSL.DoBeforeConnect
...
function TIdSSLIOHandlerSocketOpenSSL.CheckForError
...
function TIdSSLIOHandlerSocketOpenSSL.GetPassword(const AIsWrite
  : Boolean): string;
begin
  DoGetPasswordEx(Result, AIsWrite);
  if Result = '' then
...
function TIdSSLIOHandlerSocketOpenSSL.GetIOHandlerSelf
  : TIdSSLIOHandlerSocketOpenSSL;
begin
  Result := Self;
end;
...
begin
  ...
  if IsOpenSSL_SSLv2_Available then
  begin
  ...
  if IsOpenSSL_SSLv3_Available then
  begin
  ...
  if IsOpenSSL_TLSv1_0_Available then
  begin
  ...
  if IsOpenSSL_TLSv1_1_Available then
  begin
  ...
  if IsOpenSSL_TLSv1_2_Available then
  begin
  ...
  case fMethod of
    ...
    sslvSSLv23:
      case fMode of
        ...
        if Assigned(SSLv23_method) then
        begin
...
RegisterSSL('OpenSSL', 'Indy Pit Crew', { do not localize }
  ...
  TIdSSLIOHandlerSocketOpenSSL, TIdServerIOHandlerSSLOpenSSL);
