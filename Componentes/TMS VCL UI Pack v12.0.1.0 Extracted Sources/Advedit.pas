...
begin
  ...
  if (FLengthLimit > 0) and (FixedLength(Text) >= FLengthLimit) and
    (SelLength = 0) and (msg.charcode <> vk_back) and (EditType = etFloat) and (SelStart < DecimalPos) and (msg.charcode <> ord(decimalseparator)) and not AllowMin(chr(msg.CharCode)) then Exit;

  if (FLengthLimit > 0) and (Length(Text) >= FLengthLimit) and
    (SelLength = 0) and (msg.charcode <> vk_back) and (EditType <> etFloat) then Exit;
