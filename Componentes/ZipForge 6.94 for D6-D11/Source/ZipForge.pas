...
// compress file
//------------------------------------------------------------------------------
function TZFBaseArchiver.InternalCompressFile(rfs, wfs: TStream; var d: TDirItem; rfsSize: int64): boolean;
var
  ...
  begin
    randomize;
    Count1 := 0;
    while Count1 < rfsSize do
    begin
      if (rfsSize - Count1 > FBlockSize) then
        size := FBlockSize
      else
        size := rfsSize - Count1;
...
begin
  ...
  if not FProgressCancel then
    raise EZFException.Create(00034, [rfsSize, Count], self)
  else
    Result := False;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          decompressedBlockSize := strm.total_out - oldSize;
          if decompressedBlockSize < 0 then
            Inc(decompressedBlockSize, $100000000);

          if not WriteToStreamWithOnDiskFull(wfs, Buf^, decompressedBlockSize) then
              break;

          UpdateCRC32(PByte(Buf), decompressedBlockSize, fcrc32);

          Inc(cSize, decompressedBlockSize);
    ...
    try
      ...
      begin
       ...
       // end;
        ...
        // Thanks to HEIKO: Start HEIKO 05.06.2018 - if Cancel then break the decompress of all blocks
        if (FProgressCancel) then
           break;
        // End HEIKO
