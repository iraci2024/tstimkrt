{===============================================================================
  ...
  7.0.0  (15 March 2021)
    ...
    * Update drawing of TRzGroupBar control (now it looks more modern)
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzGroupBar and modified the
...
uses
  ...
  Vcl.Dialogs,
  Vcl.ImgList,
  // Must be defined after Vcl.ImgList so that the correct System.UITypes.TImageList is used
  System.UITypes,
  Vcl.ExtCtrls,
  Vcl.StdCtrls,
  Vcl.Controls,
...
const
  ...
  cDefaultScrollButtonSize = 17;
  cDefaultGroupButtonOpenCloseSize = 8;
...
begin
  ...
  FOpenCloseButtonSize := cDefaultGroupButtonOpenCloseSize;
...
var
  ...
  begin
      ...
      case FItems[ Index ].CaptionState of
        ...
        begin
          ...
          begin
            // Static Item
            Canvas.Font := FItemStaticFont;
            if not UsingSystemStyle( Self ) and UseActiveStyleFontColor( Self ) then
              Canvas.Font.Color := ActiveStyleFontColor( Self, sfButtonTextNormal );
          end;
    ...
    begin
      ...
      begin
        Canvas.Font.Color := ActiveStyleFontColor( Self, sfButtonTextDisabled );
      end;
      Canvas.Font.Style := [];
    end;
...
begin {= TRzGroup.DrawItems =}
  ...
  begin
    if UsingSystemStyle( Self ) then
    begin
      if ( GroupBar.VisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
      begin
        if FSpecial then
          ItemFontColor := GetXPThemeColor( Self, xptcSpecialGroupFont )
        else
          ItemFontColor := GetXPThemeColor( Self, xptcNormalGroupFont );
        ItemFontHotColor := BlendColors( clWhite, ItemFontColor, 50 );
      end
      else if GroupBar.GradientColorStyle <> gcsCustom then
      begin
        GetGradientGroupItemColors( GroupBar, GroupBar.GradientColorStyle, ItemFontColor,
                                    ItemFontHotColor );
      end
      else // Custom
      begin
    ...
    begin
      ...
      begin
        ItemFontColor := ActiveStyleFontColor( Self, sfButtonTextNormal );
        ItemFontHotColor := ActiveStyleSystemColor( Self, clHighlight );
      end
      else
      begin
        ItemFontColor := Self.Font.Color;
  ...
  begin
    GetGradientSelectionColors( GroupBar, GroupBar.GradientColorStyle, SelectionBorderColor,
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              C := ActiveStyleSystemColor( Self, Color );
              DivColor := ActiveStyleSystemColor( Self, FDividerColor );
...
begin
  ...
  if Opened then
  begin
    DrawModernArrow(dirUp, FOpenCloseButtonSize, Canvas,
      Rect(Left, Top, Left + FOpenCloseButtonSize, Top + FOpenCloseButtonSize), ChevronColor, FScaleFactor );
    DrawModernArrow(dirUp, FOpenCloseButtonSize, Canvas,
      Rect(Left, Top + FOpenCloseButtonSize div 2, Left + FOpenCloseButtonSize,
           Top + FOpenCloseButtonSize + FOpenCloseButtonSize div 2), ChevronColor, FScaleFactor );
  end
  else
  begin
    DrawModernArrow(dirDown, FOpenCloseButtonSize, Canvas,
      Rect(Left, Top, Left + FOpenCloseButtonSize, Top + FOpenCloseButtonSize), ChevronColor, FScaleFactor );
    DrawModernArrow(dirDown, FOpenCloseButtonSize, Canvas,
      Rect(Left, Top + FOpenCloseButtonSize div 2, Left + FOpenCloseButtonSize,
           Top + FOpenCloseButtonSize + FOpenCloseButtonSize div 2), ChevronColor, FScaleFactor );
...
begin
  ...
  if UsingSystemStyle( GroupBar ) then
  begin
    if GroupBar.GradientColorStyle <> gcsCustom then
    begin
      GetGradientCategoryGroupColors( GroupBar, GroupBar.GradientColorStyle, FSpecial,
  ...
  begin
    if FSpecial then
    begin
      CaptionBackColor := ActiveStyleSystemColor( GroupBar, clHighlight );
      CaptionFontColor := ActiveStyleFontColor( GroupBar, sfButtonTextNormal );
      CaptionFontHotColor := ActiveStyleFontColor( GroupBar, sfButtonTextHot );
      CaptionBackColorStart := ActiveStyleSystemColor( GroupBar, clHighlight );
      CaptionBackColorStop := DarkerColor( CaptionBackColorStart, 30 );
    end
    else
    begin
      CaptionBackColor := ActiveStyleColor( GroupBar, scToolBarGradientBase );
      CaptionFontColor := ActiveStyleFontColor( GroupBar, sfButtonTextNormal );
      CaptionFontHotColor := ActiveStyleFontColor( GroupBar, sfButtonTextHot );
      CaptionBackColorStart := LighterColor( ActiveStyleSystemColor( GroupBar, clBtnFace ), 30 );
      CaptionBackColorStop := LighterColor( CaptionBackColorStart, 30 );
    end;
    CaptionDividerColor := ActiveStyleSystemColor( GroupBar, clHighlight );
    CaptionButtonColor := CaptionBackColor;
    CaptionButtonBorderColor := DarkerColor( CaptionButtonColor, 30 );
    GroupColor := ActiveStyleSystemColor( GroupBar, clBtnFace );
    GroupBorderColor := ActiveStyleSystemColor( GroupBar, clBtnHighlight );
  end;
  ...
  begin
    if not UseRightToLeftAlignment then
      DrawOpenCloseButton( FOpened, Width - FOpenCloseButtonSize - ScaleValue(6), ( FCaptionHeight - Round(FOpenCloseButtonSize * 1.5)) div 2,
                           ForeColor )
    else
      DrawOpenCloseButton( FOpened, ScaleValue(6), ( FCaptionHeight - Round(FOpenCloseButtonSize * 1.5)) div 2,
                           ForeColor );
  end;
...
begin
  ...
  try
    ...
    ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebExplorerBarRoot );
    ActiveStyleServices( GroupBar ).DrawElement( Bmp.Canvas.Handle, ElementDetails, ThemeRect );
    ...
    if FSpecial then
      ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupHead )
    else
      ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupHead );
    ActiveStyleServices( GroupBar ).DrawElement( Canvas.Handle, ElementDetails, R );
  ...
  if not UseRightToLeftAlignment then
    ActiveStyleServices( GroupBar ).DrawText( Canvas.Handle, ElementDetails, Caption, R,
                                          TTextFormatFlags( dt_Left or dt_VCenter or dt_SingleLine ) )
  else
    ActiveStyleServices( GroupBar ).DrawText( Canvas.Handle, ElementDetails, Caption, R,
                                          TTextFormatFlags( dt_Right or dt_RtlReading or dt_VCenter or dt_SingleLine ) );
  ...
  begin
    ...
    begin
      if FSpecial then
        ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupCollapseNormal )
      else
        ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupCollapseNormal );
      if not ( csDesigning in ComponentState ) then
      begin
        case FCaptionState of
          csHot:
          begin
            if FSpecial then
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupCollapseHot )
            else
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupCollapseHot );
          end;

          csDown:
          begin
            if FSpecial then
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupCollapsePressed )
            else
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupCollapsePressed );
          end;
        end;
      end;
    end
    ...
    begin
      if FSpecial then
        ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupExpandNormal )
      else
        ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupExpandNormal );
      if not ( csDesigning in ComponentState ) then
      begin
        case FCaptionState of
          csHot:
          begin
            if FSpecial then
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupExpandHot )
            else
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupExpandHot );
          end;

          csDown:
          begin
            if FSpecial then
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupExpandPressed )
            else
              ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupExpandPressed );
          end;
        end;
      end;
    end;

    GetThemePartSize( ActiveStyleServices( GroupBar ).Theme[ teExplorerBar ], Canvas.Handle, ElementDetails.Part, ElementDetails.State,
                      nil, TS_TRUE, TS );
    ...
    *)
    ...
    ActiveStyleServices( GroupBar ).DrawElement( Canvas.Handle, ElementDetails, R, nil );
  end;

  // Draw Client Area
  R := Rect( 0, FCaptionHeight, Width, Height );
  if FSpecial then
    ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebSpecialGroupBackground )
  else
    ElementDetails := ActiveStyleServices( GroupBar ).GetElementDetails( tebNormalGroupBackground );
  ActiveStyleServices( GroupBar ).DrawElement( Canvas.Handle, ElementDetails, R );
...
begin
  ...
  begin
    if GroupBar.GradientColorStyle <> gcsCustom then
    begin
      GetGradientOutlookGroupColors( GroupBar, GroupBar.GradientColorStyle,
  ...
  begin
    CaptionBackStopColor := ActiveStyleColor( GroupBar, scButtonNormal );
    CaptionBackStartColor := LighterColor( CaptionBackStopColor, 30 );

    CaptionFontColor := ActiveStyleFontColor( GroupBar, sfButtonTextNormal );
    CaptionFontHotColor := ActiveStyleFontColor( GroupBar, sfButtonTextHot );
    GroupStartColor := LighterColor( ActiveStyleSystemColor( GroupBar, clBtnFace ), 20 );
    GroupStopColor:= ActiveStyleSystemColor( GroupBar, clBtnFace );
  end;
  ...
  begin
    case FCaptionState of
      csHot:
        R := DrawBorder( GroupBar, Canvas, R, fsButtonUp );

      csDown:
        R := DrawBorder( GroupBar, Canvas, R, fsStatus );

      else
        R := DrawBorder( GroupBar, Canvas, R, fsPopup );
    end;
  end
  else
  begin
...
begin
  ...
  begin
    ViewColor := DarkerColor( ActiveStyleSystemColor( GroupBar, clBtnFace ), 20 );
    CaptionFontColor := ActiveStyleFontColor( GroupBar, sfButtonTextNormal );
  end;
  CaptionDividerColor := ActiveStyleSystemColor( GroupBar, clHighlight );
...
begin
  ...
  begin
    case GroupBar.Style of
      gbsCategoryView:
      begin
        if UsingSystemStyle( GroupBar ) and ( GroupBar.VisualStyle = vsWinXP ) and ActiveStyleServices( GroupBar ).Enabled then
...
procedure TRzGroup.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FOpenedHeight := MulDiv(FOpenedHeight, M , D);
  FCaptionHeight := MulDiv(FCaptionHeight, M , D);
  FScrollButtonSize := MulDiv(FScrollButtonSize, M, D);
  FOpenCloseButtonSize := MulDiv(FOpenCloseButtonSize, M, D);
  FItemHeight := MulDiv(FItemHeight, M, D);
  FItemStaticFont.Height := MulDiv(FItemStaticFont.Height, M, D);
...
begin
  ...
  begin
    ...
    FScrollUpBtn.SetBounds( ClientWidth - FScrollButtonSize - 4, FCaptionHeight + 4, FScrollButtonSize, FScrollButtonSize);
    if Opened then
      Offset := ClientHeight - FScrollButtonSize - 4
    else
      Offset := FCaptionHeight + 4;
    FScrollDownBtn.SetBounds( ClientWidth - FScrollButtonSize - 4, Offset, FScrollButtonSize, FScrollButtonSize);
  end;
end;
...
begin
  case Style of
    ...
    begin
      if ( GroupBar.VisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
      begin
        Result := GetXPThemeColor( Self, xptcGroupBarFill );
      end
      else
        Result := clBtnFace;
    end;
...
begin
  ...
  begin
    System.Classes.RegisterClasses( [ TRzGroup ] );
    Registered := True;
  end;
...
begin
  ...
  begin
    DrawGroupBarBackground( Self, Canvas, ClientRect, FVisualStyle,
                            FGradientColorStyle, FGradientPath,
                            FGradientColorStart, FGradientColorStop );
  end
  else if not UsingSystemStyle( Self ) then
  begin
    Canvas.Brush.Color := DarkerColor( ActiveStyleSystemColor( Self, clBtnFace ), 20 );
    Canvas.FillRect( ClientRect );
  end;
...
begin
  ...
  try
    ...
    R := DrawInnerOuterBorders( Self, FCanvas, R, FBorderOuter, FBorderInner, BorderWidth, FBorderSides, BevelWidth,
