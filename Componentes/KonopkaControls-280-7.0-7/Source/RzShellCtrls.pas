{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Removed old conditional debugging statements.
  ------------------------------------------------------------------------------
  6.2    (16 Jul 2015)
...
begin
  ...
  begin
    ...
    System.AnsiStrings.StrPCopy( ( PAnsiChar( ptrDROPFILES ) + CurPos ), FileName );
...
var
  ...
  size: Integer;
begin
  size := Winapi.Windows.GlobalSize( hsrc );
  Result := Winapi.Windows.GlobalAlloc( GMEM_MOVEABLE, size );
  if ( Result <> 0 ) then
  begin
    pdest := Winapi.Windows.GlobalLock( Result );
    psrc := Winapi.Windows.GlobalLock( hsrc );
    System.Move( psrc^, pdest^, size );
    Winapi.Windows.GlobalUnlock( hsrc );
    Winapi.Windows.GlobalUnlock( Result );
  end;
...
type
  ...
  begin
    ...
    if not Winapi.Windows.CreateDirectory( PChar( aPathname + s ), nil ) then
    begin
      dw := GetLastError;
...
begin
  ...
  begin
    ...
    try
      ...
      h := Winapi.Windows.FindFirstFile( PChar( pathname ), w32fd );
  ...
  begin                                     // Finds the command Id from the string - sometimes string commands don't work, so we look up the id and use that
    ...
    try
      ...
      begin
        ...
        if System.AnsiStrings.AnsiStrIComp( ca, item ) = 0 then
        begin
...
begin
  ...
  info.fMask := SIF_ALL;
  Winapi.Windows.GetScrollInfo( ctl.Handle, code, info );
  if ( info.nMax - info.nMin - Integer( info.nPage ) ) > 0 then
    ctl.Perform( conversion[ code ], direction, 0 );
  Result := info.nPos <> Winapi.Windows.GetScrollPos( ctl.Handle, code );
end;
...
procedure SendTo_WMInitMenuPopup( var aMsg: TWMInitMenuPopup; activeContextMenu: IContextMenu_NRC; var items: TStringList );
  ...
  begin
    ...
    begin
      fh := Winapi.Windows.FindFirstFile( PChar( path + '\' + extensions[ I ] ), w32fd );
      ...
      begin
        ...
        until not Winapi.Windows.FindNextFile( fh, w32fd );
...
begin
  Winapi.Windows.GetShortPathName( PChar(longPathName), sz, Sizeof(sz) );
  Result := sz;
end;}
...
begin
  ...
  FDoActionEvent := Winapi.Windows.CreateEvent( nil, False, False, nil );
  FActionProcessedEvent := Winapi.Windows.CreateEvent( nil, False, False, nil );
  FOwnerWnd := OwnerWnd;
...
begin
  ...
  Winapi.Windows.SetEvent( FDoActionEvent );
  Winapi.Windows.WaitForSingleObject( Handle, 30000 ); // Wait for thread to exit
...
procedure TRzChangeHandlerThread.Execute;
  ...
  begin
    Winapi.Windows.FindCloseChangeNotification( Integer( FChangeHandlerList[ handlerIndex ] ) );
  ...
  begin
    for I := 0 to FChangeHandlerList.Count - 1 do
      if Integer( FChangeHandlerList[ I ] ) <> Integer( INVALID_HANDLE_VALUE ) then
      begin
        Winapi.Windows.FindCloseChangeNotification( Integer( FChangeHandlerList[ I ] ) );
  ...
  begin
    ...
    if not Winapi.Windows.FindNextChangeNotification( Integer( FChangeHandlerList[ handlerIndex ] ) ) then
...
begin
  repeat
    ...
    dw := Winapi.Windows.WaitForMultipleObjects( j, @obja, False, INFINITE );
    ...
    begin
      ...
      Winapi.Windows.SetEvent( FActionProcessedEvent );
    end
...
// If param 2 is declared LongBool as in Winapi.Windows.pas, then in Delphi 3 it takes on the value 0=False, -1=True. If you
...
begin
  ...
  begin
    ...
    Winapi.Windows.ResetEvent( FActionProcessedEvent );
    Winapi.Windows.SetEvent( FDoActionEvent );
    if Winapi.Windows.WaitForSingleObject( FActionProcessedEvent, 2000 ) = WAIT_OBJECT_0 then
    begin
...
begin
  ...
  Winapi.Windows.ResetEvent( FActionProcessedEvent );
  Winapi.Windows.SetEvent( FDoActionEvent );
  Winapi.Windows.WaitForSingleObject( FActionProcessedEvent, 2000 );
...
var
  ...
  hr: HResult;
begin
  FLastAutoScrollTick := Winapi.Windows.GetTickCount;
...
begin
  if Winapi.Windows.GetTickCount > Cardinal( FLastAutoScrollTick + RZSH_AUTOSCROLL_MINDELAY_MS ) then
  begin
    if ( pt.x < RZSH_AUTOSCROLL_THRESHOLD_X ) then
    begin
      if DoAutoScroll( Self, SB_HORZ, SB_LINEUP ) then
        FLastAutoOpenTick := Winapi.Windows.GetTickCount;
    end
  ...
  begin
    if Winapi.Windows.GetTickCount > Cardinal( FLastAutoOpenTick + RZSH_AUTOOPEN_DELAY_MS ) then
    begin
...
procedure TRzCustomShellTree.InstallChangeHandlersForAllLocalDrives;
  ...
  var
    nType: DWORD;
  begin
    nType := Winapi.Windows.GetDriveType( PChar( s ) );
    case nType of
      DRIVE_REMOTE:
...
begin
  ...
  sDrive := '?:\';
  dwDriveList := Winapi.Windows.GetLogicalDrives;
...
begin
  ...
  try
    SetErrorMode( uCode or SEM_FAILCRITICALERRORS );
    h := Winapi.Windows.FindFirstFile( PChar( aFileSpec ), w32fd );
    if ( h <> INVALID_HANDLE_VALUE ) then
    begin
      repeat
        if ( w32fd.dwFileAttributes and FILE_ATTRIBUTE_DIRECTORY ) = 0 then
          TStringList( aLookupTable ).Add( AnsiUppercase( w32fd.cFileName ) );
      until not Winapi.Windows.FindNextFile( h, w32fd );
      Winapi.Windows.FindClose( h );
      Result := True;
    end
    else
  ...
  try
    ...
    begin
      ...
      begin
        ...
        defid := Winapi.Windows.GetMenuDefaultItem( hm, 0, 0 );
