{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzComboBox and descendants and
      modified components to honor StyleElements settings when VCL Styles
      are used.
    * Surfaced StyleName property in RAD Studio 10.4+.
    * Fixed display issue in TRzImageComboBox where the font color of the
      selected item was displayed in the wrong color under certain VCL Styles.
  ------------------------------------------------------------------------------
  6.2.1  (01 Sep 2015)
...
type
  ...
  private
    ...
    { Images }
    class var FFontImageCollection: TImageCollection;
...
begin
  ...
  if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
    ItemColor := Self.Brush.Color
  else
    ItemColor := ActiveStyleColor( Self, scComboBox );

  if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
    ItemFontColor := Self.Font.Color
  else
    ItemFontColor := ActiveStyleFontColor( Self, sfComboBoxItemNormal );
...
begin
  Result := ( FFramingPreference = fpXPThemes ) and ActiveStyleServices( Self ).Enabled;
end;
...
begin
  ...
  if FFrameVisible and not UseThemes and
     ( not ( RunningAtLeast( WinVista ) and ActiveStyleServices( Self ).Enabled ) or ( Style <> csDropDownList ) ) then
  begin
    // Erase Ctl3D Border
    if ActiveStyleServices( Self ).Enabled then
    begin
  ...
  else if ActiveStyleServices( Self ).Enabled then
  begin
  ...
  if ActiveStyleServices( Self ).Enabled and ( Style <> csSimple ) and
  ...
  begin
    ...
    begin
      ...
      if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
        ...
        FCanvas.Brush.Color := ActiveStyleColor( Self, scEdit );
      FCanvas.FillRect( BtnRect );

      if ActiveStyleServices( Self ).Enabled then
        DrawDropDownArrow( Self, FCanvas, BtnRect, uiWindowsXP, False, Enabled )
      else
        DrawDropDownArrow( Self, FCanvas, BtnRect, uiWindows95, False, Enabled );
    end
    else
    begin
      // Erase Button Border
      ...
      begin
        if DroppedDown then
          ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tcDropDownButtonPressed )
        else
          ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tcDropDownButtonHot );

        ActiveStyleServices( Self ).DrawElement( FCanvas.Handle, ElementDetails, BtnRect );
      end
      else // No Themes
      begin
        ...
        DrawDropDownArrow( Self, FCanvas, TempRect, uiWindows95, DroppedDown, Enabled );
      end;
    end;
  end
  ...
  begin
    ...
    begin
      if FFrameHotStyle = fsFlat then
        ...
        DrawBorderSides( Self, FCanvas, ClientRect, FFrameHotStyle, FFrameSides )
      else
    ...
    begin
      if FFrameStyle = fsFlat then
        ...
        DrawBorderSides( Self, FCanvas, ClientRect, FFrameStyle, FFrameSides )
      else
...
begin
  ...
  PaintIt := ( FFlatButtons and not ( RunningAtLeast( WinVista ) and ActiveStyleServices( Self ).Enabled ) ) or FFrameHotTrack;
...
begin
  ...
  begin
    ...
    R.Right := R.Left + ScaleValue(24);
    if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
      Canvas.Brush.Color := Color
    else
      Canvas.Brush.Color := ActiveStyleColor( Self, scComboBox );
    Canvas.FillRect( R );
    Canvas.Handle := 0;

    Msg.DrawItemStruct^.rcItem.Left := Msg.DrawItemStruct^.rcItem.Left + ScaleValue(24);
  end;
  inherited;
end;
...
begin
  ...
  if FShowColorNames then
  begin
    Dec( R.Left, ScaleValue(24) );
    R.Right := R.Left + ScaleValue(16);
  end
  else if FShowCustomColor and ( Index = Items.Count - 1 ) then
    Dec( R.Right, ScaleValue(20) );

  Canvas.Brush.Color := GetColorFromItem( Index );
  Canvas.Pen.Color := ActiveStyleSystemColor( Self, clBtnShadow );
  ...
  begin
    ...
    begin
      Canvas.Brush.Color := ActiveStyleColor( Self, scComboBox );
      if Enabled then
        Canvas.Pen.Color := ActiveStyleFontColor( Self, sfComboBoxItemNormal )
      else
        Canvas.Pen.Color := ActiveStyleFontColor( Self, sfComboBoxItemDisabled );
    end;
  end;
  ...
  begin
    ...
    Canvas.Rectangle( Rect.Right - ScaleValue(16), Rect.Bottom - ScaleValue(7), Rect.Right - ScaleValue(14), Rect.Bottom - ScaleValue(4) );
    Canvas.Rectangle( Rect.Right - ScaleValue(12), Rect.Bottom - ScaleValue(7), Rect.Right - ScaleValue(10), Rect.Bottom - ScaleValue(4) );
    Canvas.Rectangle( Rect.Right - ScaleValue(8), Rect.Bottom - ScaleValue(7), Rect.Right - ScaleValue(6), Rect.Bottom - ScaleValue(4) );
...
class constructor TRzFontComboBox.Create;
begin
  FFontImageCollection := TImageCollection.Create(nil);
  FFontImageCollection.Add('RZCMBOBX_TRUETYPE', HInstance, 'RZCMBOBX_TRUETYPE', ['', '_20X']);
  FFontImageCollection.Add('RZCMBOBX_FIXEDPITCH', HInstance, 'RZCMBOBX_FIXEDPITCH', ['', '_20X']);
  FFontImageCollection.Add('RZCMBOBX_TRUETYPEFIXED', HInstance, 'RZCMBOBX_TRUETYPEFIXED', ['', '_20X']);
  FFontImageCollection.Add('RZCMBOBX_PRINTER', HInstance, 'RZCMBOBX_PRINTER', ['', '_20X']);
  FFontImageCollection.Add('RZCMBOBX_DEVICE', HInstance, 'RZCMBOBX_DEVICE', ['', '_20X']);
end;

class destructor TRzFontComboBox.Destroy;
begin
  FreeAndNil(FFontImageCollection);
end;
...
begin
  ...
  try
    ...
    if seClient in StyleElements then
      Canvas.Brush.Color := LStyle.GetStyleColor(scListBox)
    else
      Canvas.Brush.Color := Self.Color;

    ImageRect := System.Types.Rect(
      R.Left + (R.Width - IW) div 2,
      R.Top + (R.Height - IW) div 2,
      R.Left + (R.Width - IW) div 2 + IW,
      R.Top + (R.Height - IW) div 2 + IW);

    Canvas.FillRect( ImageRect );   { Clear area for icon}

    Canvas.Brush.Color := LColor;
    Canvas.FillRect( Rect );   { Clear area text }
    ...
    if ( ( FT and tmpf_TrueType ) = tmpf_TrueType ) and
      ...
      FFontImageCollection.Draw(Canvas, ImageRect, 'RZCMBOBX_TRUETYPEFIXED', True)
    else if ( FT and tmpf_TrueType ) = tmpf_TrueType then
      FFontImageCollection.Draw(Canvas, ImageRect, 'RZCMBOBX_TRUETYPE', True)
    else if ( FT and tmpf_Fixed_Pitch ) <> tmpf_Fixed_Pitch then
      FFontImageCollection.Draw(Canvas, ImageRect, 'RZCMBOBX_FIXEDPITCH', True)
    else if FFontDevice = fdPrinter then
      FFontImageCollection.Draw(Canvas, ImageRect, 'RZCMBOBX_PRINTER', True)
    else
      FFontImageCollection.Draw(Canvas, ImageRect, 'RZCMBOBX_DEVICE', True);
    ...
    begin
      if odSelected in State then
        ...
        if UsingSystemStyle( Self ) or not (seFont in StyleElements) then
          Canvas.Font.Color := Self.Font.Color
        else
          Canvas.Font.Color := ActiveStyleFontColor( Self, sfListItemTextNormal );
    end
    else
    begin
      if UsingSystemStyle( Self ) or not (seFont in StyleElements) then
        ...
        Canvas.Font.Color := ActiveStyleFontColor( Self, sfListItemTextDisabled );
    end;
    ...
    case TempStyle of
      ...
      begin
        ...
        begin
          if Enabled then
            Canvas.Pen.Color := ActiveStyleFontColor( Self, sfListItemTextNormal )
          else
            Canvas.Pen.Color := ActiveStyleFontColor( Self, sfListItemTextDisabled );
        end;
...
begin
  ...
  begin
    ...
    if UsingSystemStyle( Self ) then
      ...
      BrushColor := ActiveStyleColor( Self, scComboBox );
    ...
    begin
      ...
      begin
        ...
        begin
          if odSelected in State then
            Canvas.Font.Color := ActiveStyleFontColor( Self, sfComboBoxItemSelected )
          else
            Canvas.Font.Color := ActiveStyleFontColor( Self, sfComboBoxItemNormal );
        end
        else
          Canvas.Font.Color := ActiveStyleFontColor( Self, sfComboBoxItemDisabled );
      end;
