...
type
  ...
  private
    ...
    class var FInternalImageCollection: TImageCollection;
...
class constructor TRzDBStateStatus.Create;
begin
  FInternalImageCollection := TImageCollection.Create(nil);
  FInternalImageCollection.Add('RZDBSTAT_INACTIVE', HInstance, 'RZDBSTAT_INACTIVE' , ['', '_20X']);
  FInternalImageCollection.Add('RZDBSTAT_SEARCH', HInstance, 'RZDBSTAT_SEARCH' , ['', '_20X']);
  FInternalImageCollection.Add('RZDBSTAT_EDIT', HInstance, 'RZDBSTAT_EDIT' , ['', '_20X']);
  FInternalImageCollection.Add('RZDBSTAT_INSERT', HInstance, 'RZDBSTAT_INSERT' , ['', '_20X']);
  FInternalImageCollection.Add('RZDBSTAT_SETKEY', HInstance, 'RZDBSTAT_SETKEY' , ['', '_20X']);
  FInternalImageCollection.Add('RZDBSTAT_CALC', HInstance, 'RZDBSTAT_CALC' , ['', '_20X']);
  FInternalImageCollection.Add('RZDBSTAT_FILTER', HInstance, 'RZDBSTAT_FILTER' , ['', '_20X']);
end;

class destructor TRzDBStateStatus.Destroy;
begin
  FreeAndNil(FInternalImageCollection);
end;
...
begin
  ...
  FInternalImages := TVirtualImageList.Create( nil );
  FInternalImages.SetSize(15, 15);
  FInternalImages.AutoFill := True;
  FInternalImages.ImageCollection := FInternalImageCollection;

  Self.Images := FInternalImages;
...
procedure TRzDBStateStatus.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
...
begin
  ...
  if ShowGlyph then
  begin
    GlyphOffset := ScaleValue(5);
    FInternalImages.ImageCollection := FInternalImageCollection;
    ImageIndex := Ord( DatasetState );
  end
  else
  begin
    GlyphOffset := 0;
    if FInternalImages.ImageCollection <> nil then
    begin
      FInternalImages.ImageCollection := nil;
      ImageIndex := -1;
    end;
  end;
end;
