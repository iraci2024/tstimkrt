...
uses
  ...
  Vcl.ExtCtrls,
  Vcl.ImgList,
  System.UITypes, // Must occur after Vcl.ImgList
  RzCommon,
  RzPanel,
  RzButton,
  Vcl.ImageCollection,
...
const
  ...
  NavBmpNames: array[ TRzNavigatorButton] of PChar =
      ( 'RZDBNAV_FIRST', 'RZDBNAV_PRIOR', 'RZDBNAV_NEXT',
        'RZDBNAV_LAST', 'RZDBNAV_INSERT', 'RZDBNAV_DELETE',
        'RZDBNAV_EDIT', 'RZDBNAV_POST', 'RZDBNAV_CANCEL',
        'RZDBNAV_REFRESH');
...
class constructor TRzDBNavigator.Create;

  procedure InitButtonsImageCollection;
  var
    I: TRzNavigatorButton;
  begin
    FButtonsImageCollection := TImageCollection.Create(nil);
    for I := Low(TRzNavigatorButton) to High(TRzNavigatorButton) do
      FButtonsImageCollection.Add(NavBmpNames[I], HInstance,  NavBmpNames[I], ['', '_20X']);
  end;
...
class destructor TRzDBNavigator.Destroy;
begin
...
procedure TRzDBNavigator.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
...
begin
  if UsingSystemStyle( Self ) then
    FInternalImages.ImageCollection := FButtonsImageCollection
  else
...
begin
  ...
  begin
    ...
    Buttons[ NavBtn ].DisabledIndex := -1;
  end;
end;
...
begin
  ...
  if not UsingSystemStyle( Self ) and ( TRzDBNavigator( Parent ).FImageIndexes.GetImageIndex( Idx ) = -1 ) then
  begin
    if not Enabled then
    ...
    ActiveStyleServices( Self ).DrawElement(Canvas.Handle, StyleServices.GetElementDetails(Element), R, nil, FCurrentPPI);
  end;
