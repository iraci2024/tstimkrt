{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzSpinner and descendants and modified
...
begin
  ...
  if FUpRightButton <> nil then
    FUpRightButton.StyleElements := StyleElements;
  if FDownLeftButton <> nil then
    FDownLeftButton.StyleElements := StyleElements;
end;
...
procedure TRzSpinButtons.UpdateStyleElements;
begin
  inherited;
  if FUpRightButton <> nil then
    FUpRightButton.StyleElements := StyleElements;
...
procedure TRzSpinEdit.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
...
function TRzSpinEdit.GetEditRectMargin: Integer;
begin
...
procedure TRzSpinner.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FButtonWidth := MulDiv(FButtonWidth, M, D);
  inherited;
end;
...
begin
  ...
  if UsingSystemStyle( Self ) then
  begin
    if RunningAtLeast( winVista ) and FThemeAware and ActiveStyleServices( Self ).Enabled then
    begin
...
begin
  ...
  if FThemeAware and ActiveStyleServices( Self ).Enabled and UseActiveStyleColor( Self ) then
  begin
    Canvas.Brush.Color := Self.Color;
    // Eventually add DisabledColor property like TRzEdit

    if Enabled then
      ElementDetails := ActiveStyleServices( Self ).GetElementDetails( teEditTextNormal )
    else
...
//    GetThemeColor( ActiveStyleServices( Self ).Theme[ teEdit ], ElementDetails.Part,
...
begin
  UseSystemStyle := UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self );
  Canvas.Font := Self.Font;
  if not UseSystemStyle then
  begin
    if Enabled then
      Canvas.Font.Color := ActiveStyleFontColor( Self, sfEditBoxTextNormal )
    else
      Canvas.Font.Color := ActiveStyleFontColor( Self, sfEditBoxTextDisabled );
  end
  else
  if not Enabled then
...
var
  ...
  ElementDetails: TThemedElementDetails;
begin
  if FThemeAware and ActiveStyleServices( Self ).Enabled and not UseCustomImages( Area ) then
  begin
    if UsingSystemStyle( Self ) then
    begin
      ...
      begin
        ...
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( SpinElement );
        ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, Bounds, nil, CurrentPPI );
    ...
    begin
      ...
      begin
        ...
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( BtnElement );
        ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, Bounds, nil, CurrentPPI );
      end
      else // Area = saPlusButton
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        DrawSpinArrow( Self, Canvas, Bounds, uiWindows95, Direction, Down, True, Enabled, FScaleFactor );
      end;
    end
    else // Area = saPlusButton
    begin
