{===============================================================================
  ...
  ------------------------------------------------------------------------------
  6.2.4  (15 Oct 2018)
    * Moved EditRect processing into the base TRzCustomEdit class.
  ------------------------------------------------------------------------------
  6.0    (09 Oct 2011)
...
type
  ...
  private
    ...
    FAltBtnImages: TCustomImageList;
    FAltBtnImageIndex: TImageIndex;
    ...
    FOnAltBtnClick: TNotifyEvent;
    FOnButtonClick: TNotifyEvent;
    FInternalButtonImages: TVirtualImageList;
  ...
  protected
    ...
    function GetEditRectMargin: Integer; override;
    ...
    procedure SetButtonImages(Value: TCustomImageList); virtual;
    procedure SetButtonImageIndex(Value: TImageIndex); virtual;
    procedure SetAltBtnImages(Value: TCustomImageList); virtual;
    procedure SetAltBtnImageIndex(Value: TImageIndex); virtual;
  public
    constructor Create(AOwner: TComponent); override;
    ...
    property AltBtnImages: TCustomImageList
      read FAltBtnImages write SetAltBtnImages;
    property AltBtnImageIndex: TImageIndex
      read FAltBtnImageIndex write SetAltBtnImageIndex default -1;
    property ButtonImages: TCustomImageList
      read FButtonImages write SetButtonImages;
    property ButtonImageIndex: TImageIndex
      read FButtonImageIndex write SetButtonImageIndex default -1;
  ...
  published
    ...
    property AltBtnImages;
    property AltBtnImageIndex;
    ...
    property HideButtonsOnReadOnly default True;
    property Touch;
  ...
  function GetBtnEdtCollection: TImageCollection;
  function GetBtnEdtIndex( Kind: TButtonKind ): Integer;
...
var
  FButtonImageCollection: TImageCollection;
...
//  FFlat := ActiveStyleServices( Self ).Enabled;
...
begin
  ...
  begin
    ...
    begin
      Winapi.Windows.SetFocus( FFocusControl.Handle );
...
begin
  ...
  FInternalButtonImages := TVirtualImageList.Create(Self);
  FInternalButtonImages.SetSize(16, 16);
  FInternalButtonImages.AutoFill := True;
  FInternalButtonImages.ImageCollection := GetBtnEdtCollection;
  ...
  FButtons.AltBtn.Images := FInternalButtonImages;
  FButtons.AltBtn.ImageIndex :=  GetBtnEdtIndex( bkLookup );
  FButtons.Button.Visible := True;
  FButtons.Button.Images := FInternalButtonImages;
  FButtons.Button.ImageIndex :=  GetBtnEdtIndex( bkLookup );
  {&RCI}
end;

destructor TRzCustomButtonEdit.Destroy;
begin
  FreeAndNil(FInternalButtonImages);
  inherited;
end;
...
procedure TRzCustomButtonEdit.Notification( AComponent: TComponent; Operation: TOperation );
begin
  inherited;
  if ( Operation = opRemove ) and ( AComponent = FButtonImages ) then
    SetButtonImages( nil );
  if ( Operation = opRemove ) and ( AComponent = FAltBtnImages ) then
    SetAltBtnImages( nil );
end;

procedure TRzCustomButtonEdit.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FInternalButtonImages.SetSize(MulDiv(FInternalButtonImages.Width, M, D), MulDiv(FInternalButtonImages.Height, M, D));
  inherited;
end;
...
begin
  ...
  begin
    Button.Images := FInternalButtonImages;
    Button.ImageIndex :=  GetBtnEdtIndex( FButtonKind );
    if Button.Width < 17 then
      Button.Width := 17;
  end
  else
  begin
    if Button.Images = FInternalButtonImages then
    begin
      Button.Images := nil;
      Button.ImageIndex := -1;
    end;
  ...
  begin
    AltBtn.Images := FInternalButtonImages;
    AltBtn.ImageIndex := GetBtnEdtIndex( FAltBtnKind );
    if AltBtn.Width < 17 then
      AltBtn.Width := 17;
  end
  else
  begin
    if AltBtn.Images = FInternalButtonImages then
    begin
      AltBtn.Images := nil;
      AltBtn.ImageIndex := -1;
    end;
...
procedure TRzCustomButtonEdit.SetButtonImages(Value: TCustomImageList);
begin
  if FButtonImages <> Value then
  begin
    FButtonImages := Value;
    Button.Images := FButtonImages;
    Button.ImageIndex := FButtonImageIndex;
    if Button.Images <> nil then
      FButtonKind := bkCustom;
  end;
end;

procedure TRzCustomButtonEdit.SetButtonImageIndex(Value: TImageIndex);
begin
...
procedure TRzCustomButtonEdit.SetAltBtnImages(Value: TCustomImageList);
begin
  if FAltBtnImages <> Value then
  begin
    FAltBtnImages := Value;
    AltBtn.Images := FAltBtnImages;
    AltBtn.ImageIndex := FAltBtnImageIndex;
    if AltBtn.Images <> nil then
      FAltBtnKind := bkCustom;
  end;
end;

procedure TRzCustomButtonEdit.SetAltBtnImageIndex(Value: TImageIndex);
begin
  if FAltBtnImageIndex <> Value then
  begin
    FAltBtnImageIndex := Value;
    if FAltBtnKind = bkCustom then
      AltBtn.ImageIndex := FAltBtnImageIndex;
  end;
end;
...
function TRzCustomButtonEdit.GetEditRectMargin: Integer;
begin
  Result := FButtons.Width + 2;
end;
...
begin
  ...
  if not ( ReadOnlyValue and HideButtonsOnReadOnly ) then
  begin
    if not UseRightToLeftLayout then
      Dec( Result.Right, GetEditRectMargin )
    else
      Inc( Result.Left, GetEditRectMargin );
  end;
end;
...
begin
  inherited;
  if FButtons <> nil then
    FButtons.Visible := not ( ReadOnlyValue and HideButtonsOnReadOnly );
  ResizeButtons;
end;
...
begin
  ...
  for I := Low(TButtonKind) to High(TButtonKind) do
    if I <> bkCustom then
      FButtonImageCollection.Add( BtnEdtResNames[I], HInstance,  BtnEdtResNames[I], ['', '_20X']);
end;
