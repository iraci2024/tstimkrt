{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzLabel and descendants and modified
      components to honor StyleElements settings when VCL Styles are used.
    * Fixed issue where TRzLabel would not pickup the font color of the
      selected VCL Style if the label's Font.Color property was changed from
      the default value.
  ------------------------------------------------------------------------------
  6.0    (09 Oct 2011)
...
begin
  ...
  begin
    ...
    begin
      ...
      if FTextStyle = tsRaised then
      begin
        ULColor := ActiveStyleSystemColor( Self, FHighlightColor );
        LRColor := ActiveStyleSystemColor( Self, FShadowColor );
      end
      else
      begin
        ULColor := ActiveStyleSystemColor( Self, FShadowColor );
        LRColor := ActiveStyleSystemColor( Self, FHighlightColor );
      end;
    ...
    begin
      Canvas.Brush.Style := bsClear;
      Canvas.Font.Color := ActiveStyleSystemColor( Self, FShadowColor );
  ...
  begin
    if UsingSystemStyle( Self ) then
      ...
      Canvas.Font.Color := ActiveStyleFontColor( Self, sfWindowTextDisabled );
...
function TRzLabel.GetFontColor: TColor;
begin
  if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
    Result := FFontColor
  else
    Result := ActiveStyleFontColor( Self, sfTextLabelNormal );
end;


function TRzLabel.GetDisabledFontColor: TColor;
begin
  if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
    Result := clGrayText
  else
    Result := ActiveStyleFontColor( Self, sfTextLabelDisabled );
end;


procedure TRzLabel.UpdateStyleElements;
begin
  Invalidate;
  FFontColor := Font.Color;
  if not FBlinking then
