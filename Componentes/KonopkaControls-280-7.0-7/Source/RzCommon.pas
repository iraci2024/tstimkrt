{===============================================================================
  ...
  7.0.0  (15 March 2021)
    ...
    * Rewrite methods to draw elements in controls with any scale factor (arrows, checkbox, radionbutton, ...)
    * Added improvments and optimizations in code
  ------------------------------------------------------------------------------
  6.5.0  (28 Nov 2020)
    * Update core VCL Style functions (ActiveStyleServices, UsingSystemStyle,
      ActiveStyleColor, etc.) to take a TControl parameter. This is to allow the
      new ThemeServices functionality introduced in RAD Studio 10.4 (i.e. the
      Form Designer using a different VCL Style from the IDE itself). 
    * Updated all drawing related functions that depend on VCL Styles to accept
      a TControl parameter (see above item).
...
function GetXPThemeColor( Control: TControl; Element: TRzXPThemeColor ): TColor;
...
procedure GetGradientSelectionColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
                                      var FrameColor, StartColor, StopColor: TColor );

procedure GetGradientPanelColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
                                  var StartColor, StopColor: TColor );

function GetGradientPanelFrameColor( Control: TControl; ColorStyle: TRzGradientColorStyle ): TColor;

procedure GetGradientStatusBarColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
                                      var StartColor, StopColor,
                                      DividerColor: TColor );

procedure GetGradientGroupBarColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
                                     var StartColor, StopColor: TColor );

procedure GetGradientCategoryGroupColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
...
procedure GetGradientOutlookGroupColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
                                         var CaptionBackStartColor,
                                         CaptionBackStopColor, CaptionFontColor,
                                         CaptionFontHotColor,
                                         GroupStartColor, GroupStopColor: TColor );

procedure GetGradientGroupItemColors( Control: TControl; ColorStyle: TRzGradientColorStyle;
                                      var ItemFontColor, ItemFontHotColor: TColor );

{== VCL Styles Support Functions ==}

function ActiveStyleServices( Control: TControl ): TCustomStyleServices; inline;
function UsingSystemStyle( Control: TControl ): Boolean; inline;
function ActiveStyleSystemColor( Control: TControl; C: TColor ): TColor;
function ActiveStyleColor( Control: TControl; SC: TStyleColor ): TColor;
function UseActiveStyleColor( Control: TControl ): Boolean;
function ActiveStyleFontColor( Control: TControl; SF: TStyleFont ): TColor;
function UseActiveStyleFontColor( Control: TControl ): Boolean;
...
function DrawCtl3DBorder( Control: TControl; Canvas: TCanvas; Bounds: TRect; Lowered: Boolean ): TRect;

function DrawCtl3DBorderSides( Control: TControl; Canvas: TCanvas; Bounds: TRect; Lowered: Boolean; Sides: TSides ): TRect;

function DrawButtonBorder( Control: TControl; Canvas: TCanvas; Bounds: TRect; Lowered: Boolean ): TRect;

function DrawButtonBorderSides( Control: TControl; Canvas: TCanvas; Bounds: TRect; Lowered: Boolean; Sides: TSides ): TRect;
...
function DrawBorder( Control: TControl; Canvas: TCanvas; Bounds: TRect; Style: TFrameStyle ): TRect;

function DrawBorderSides( Control: TControl; Canvas: TCanvas; Bounds: TRect; Style: TFrameStyle; Sides: TSides ): TRect;
...
function DrawInnerOuterBorders( Control: TControl; Canvas: TCanvas; Bounds: TRect;
...
procedure DrawGroupBarBackground( Control: TControl; Canvas: TCanvas; Bounds: TRect;
...
procedure DrawLEDBar( Control: TControl; Canvas: TCanvas; Bounds: TRect; Orientation: TOrientation;
...
procedure DrawFrame( Control: TControl; Canvas: TCanvas; Width, Height: Integer; FrameStyle: TFrameStyle; EraseColor, FrameColor: TColor;
...
procedure DrawArrowImage(ACanvas: TCanvas; ARect: TRect; AColor: TColor; ADirection: TDirection; ASmallSize: Boolean; AScaleFactor: Single = 1);

procedure DrawDropDownArrow( Control: TControl; Canvas: TCanvas; Bounds: TRect; UIStyle: TRzUIStyle; Down: Boolean;
                             Enabled: Boolean = True; AScaleFactor: Single = 1);

procedure DrawSpinArrow( Canvas: TCanvas; Bounds: TRect; Color, DisabledColor: TColor;
                         Direction: TDirection; Down: Boolean; SmallSize: Boolean;
                         Enabled: Boolean = True; ScaleFactor: Single = 1 ); overload;

procedure DrawSpinArrow( Control: TControl; Canvas: TCanvas; Bounds: TRect; UIStyle: TRzUIStyle;
                         Direction: TDirection; Down: Boolean; SmallSize: Boolean;
                         Enabled: Boolean = True; ScaleFactor: Single = 1 ); overload;


procedure DrawCloseX( Canvas: TCanvas; Bounds: TRect; Color: TColor;
                      Down: Boolean; Enabled: Boolean = True; ScaleFactor: Single = 1 ); overload;

procedure DrawCloseX( Control: TControl; Canvas: TCanvas; Bounds: TRect; UIStyle: TRzUIStyle;
                      Down: Boolean; Enabled: Boolean = True; ScaleFactor: Single = 1 ); overload;
...
procedure DrawCheckBox( Canvas: TCanvas; Bounds: TRect; CheckState: TCheckBoxState;
                        ...
                        ReadOnlyColor: TColor; AScaleFactor: Single = 1 );
...
procedure DrawModernArrow(ADirection: TDirection; AArrowWidth: Integer; ACanvas: TCanvas;
  ARect: TRect; AColor: TColor; AScaleFactor: Single = 1);
...
type
  PrzColor = ^TrzColor;
  TrzColor = type Cardinal;
  PrzColorRec = ^TrzColorRec;
  TrzColorRec = packed record
  case Cardinal of
    0: (Color: Cardinal);
    2: (HiWord, LoWord: Word);
    3: (B, G, R, A: Byte);
  end;
  PrzColorRecArray = ^TrzColorRecArray;
  TrzColorRecArray = array [0..0] of TrzColorRec;

var
  RadioImages: array[0..2] of TWicImage = (nil, nil, nil);

procedure LoadRadioImages;
begin
  if RadioImages[0] <> nil then
    Exit;

  RadioImages[0] := TWicImage.Create;
  RadioImages[0].LoadFromResourceName(HInstance,  'RZCOMMON_RADIOBKGND');
  RadioImages[0].InterpolationMode := wipmHighQualityCubic;
  RadioImages[1] := TWicImage.Create;
  RadioImages[1].LoadFromResourceName(HInstance,  'RZCOMMON_RADIOFRAME');
  RadioImages[1].InterpolationMode := wipmHighQualityCubic;
  RadioImages[2] := TWicImage.Create;
  RadioImages[2].LoadFromResourceName(HInstance,  'RZCOMMON_RADIOCHECKMARK');
  RadioImages[2].InterpolationMode := wipmHighQualityCubic;
end;

procedure ReleaseRadioImages;
begin
  FreeAndNil(RadioImages[0]);
  FreeAndNil(RadioImages[1]);
  FreeAndNil(RadioImages[2]);
end;

procedure SetBitmapAlpha(ABitmap: TBitmap; AAlphaValue: Byte);
var
  I: Integer;
  Src: Pointer;
begin
  ...
  for I := 0 to ABitmap.Width * ABitmap.Height - 1 do
    PrzColorRecArray(Src)[I].A := AAlphaValue;
  {$RANGECHECKS ON}
end;

procedure SetBitmapColor(ABitmap: TBitmap; AColor: TColor);
var
  I: Integer;
  Src: Pointer;
  R, G, B: Byte;
begin
  ...
  for I := 0 to ABitmap.Width * ABitmap.Height - 1 do
  begin
    PrzColorRecArray(Src)[I].R := R;
    PrzColorRecArray(Src)[I].G := G;
    PrzColorRecArray(Src)[I].B := B;
  end;
  {$RANGECHECKS ON}
end;
...
begin
  if not ActiveStyleServices( Control ).Enabled then
  begin
    Result := clBtnFace;
    Exit;
  end;
  ...
  case Element of
    xptcEditBorder:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( teEditRoot );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teEdit ], ElementDetails.Part,
                     ElementDetails.State, TMT_BORDERCOLOR, C );
    end;

    xptcNormalGroupFont:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tebNormalGroupBackground );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teExplorerBar ], ElementDetails.Part,
                     ElementDetails.State, TMT_TEXTCOLOR, C );
    end;

    xptcSpecialGroupFont:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tebSpecialGroupBackground );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teExplorerBar ], ElementDetails.Part,
                     ElementDetails.State, TMT_TEXTCOLOR, C );
    end;
    ...
    xptcListGroupFill:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tebExplorerBarRoot );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teExplorerBar ], ElementDetails.Part,
    ...
    xptcListGroupFont:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tebSpecialGroupHead );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teExplorerBar ], ElementDetails.Part,
                     ElementDetails.State, TMT_FILLCOLOR, C );
    end;
    ...
    xptcGroupBoxFont:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tbGroupBoxNormal );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teButton ], ElementDetails.Part,
                     ElementDetails.State, TMT_TEXTCOLOR, C );
    end;

    xptcSpinButtonBorder:
    begin
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tsSpinRoot );
      GetThemeColor( ActiveStyleServices( Control ).Theme[ teSpin ], ElementDetails.Part,
                     ElementDetails.State, TMT_BORDERCOLOR, C );
    end;
...
var
  C: TColor;
begin
  if ( ColorStyle = gcsMSOffice ) and ActiveStyleServices( Control ).Enabled then
  begin
  ...
  begin
    FrameColor := ActiveStyleSystemColor( Control, clHighlight );
    C := BlendColors( ActiveStyleSystemColor( Control, clHighlight ), ActiveStyleSystemColor( Control, clWindow ), 77 );
...
begin
  ...
  begin
    StartColor := ActiveStyleSystemColor( Control, clWindow );
    StopColor := DarkerColor( ActiveStyleSystemColor( Control, clBtnFace ), 10 );
  end;
...
begin
  ...
  begin
    Result := LighterColor( ActiveStyleSystemColor( Control, clBtnShadow ), 30 );
  end;
end;
...
begin
  ...
  begin
    StartColor := DarkerColor( ActiveStyleSystemColor( Control, clBtnFace ), 20 );
    StopColor := ActiveStyleSystemColor( Control, clWindow );
    DividerColor := DarkerColor( ActiveStyleSystemColor( Control, clBtnFace ), 40 );
  end;
...
begin
  ...
  begin
    StartColor := BlendColors( clBlack, ActiveStyleSystemColor( Control, clBtnFace ), 30 );
    StopColor := BlendColors( clBlack, ActiveStyleSystemColor( Control, clBtnFace ), 60 );
  end;
...
begin
  ...
  GetGradientPanelColors( Control, ColorStyle, CaptionBackStartColor, CaptionBackStopColor );
...
function ActiveStyleServices( Control: TControl ): TCustomStyleServices;
begin
  Result := StyleServices( Control );
end;


function UsingSystemStyle( Control: TControl ): Boolean;
begin
  // IsSystemStyle will return False if a VCL Style is used
  Result := ActiveStyleServices( Control ).IsSystemStyle;
end;
...
begin
  Result := C;
  if not UsingSystemStyle( Control ) then
  begin
    Result := ActiveStyleServices( Control ).GetSystemColor( C );
  end;
end;
...
begin
  ...
  if not UsingSystemStyle( Control ) then
  begin
    Result := ActiveStyleServices( Control ).GetStyleColor( SC );
  end;
end;
...
begin
  ...
  if not UsingSystemStyle( Control ) then
  begin
    Result := ActiveStyleServices( Control ).GetStyleFontColor( SF );
  end;
end;
...
begin
  ...
  if not UsingSystemStyle( Control ) then
  begin
    Result := seFont in Control.StyleElements;
  end;
end;
...
function DrawCtl3DBorder( Control: TControl; Canvas: TCanvas; Bounds: TRect; Lowered: Boolean ): TRect;
begin
  Result := DrawCtl3DBorderSides( Control, Canvas, Bounds, Lowered, sdAllSides );
end;
...
begin
  Bounds := DrawBevel( Canvas, Bounds,
                       ActiveStyleSystemColor( Control, Colors[ 1, Lowered ] ),
                       ActiveStyleSystemColor( Control, Colors[ 2, Lowered ] ), 1, Sides );

  Result := DrawBevel( Canvas, Bounds,
                       ActiveStyleSystemColor( Control, Colors[ 3, Lowered ] ),
                       ActiveStyleSystemColor( Control, Colors[ 4, Lowered ] ), 1, Sides );
end;
...
function DrawButtonBorder( Control: TControl; Canvas: TCanvas; Bounds: TRect; Lowered: Boolean ): TRect;
begin
  Result := DrawButtonBorderSides( Control, Canvas, Bounds, Lowered, sdAllSides );
end;
...
const
  ...
  StyleColors: array[ 1..4, Boolean ] of TColor = ( ( clBtnHighlight, cl3DDkShadow ),
                                                    ...
                                                    ( clBtnShadow, clBtnShadow ) );
begin
  if UsingSystemStyle( Control ) then
  begin
  ...
  begin
    Bounds := DrawBevel( Canvas, Bounds,
                         ActiveStyleSystemColor( Control, StyleColors[ 1, Lowered ] ),
                         ActiveStyleSystemColor( Control, StyleColors[ 2, Lowered ] ), 1, Sides );

    Result := DrawBevel( Canvas, Bounds,
                         ActiveStyleSystemColor( Control, StyleColors[ 3, Lowered ] ),
                         ActiveStyleSystemColor( Control, StyleColors[ 4, Lowered ] ), 1, Sides );
  end;
end;
...
function DrawBorder( Control: TControl; Canvas: TCanvas; Bounds: TRect; Style: TFrameStyle ): TRect;
begin
  Result := DrawBorderSides( Control, Canvas, Bounds, Style, sdAllSides );
end;
...
var
  ...
  R: TRect;
begin
  ULColor := ActiveStyleSystemColor( Control, ULFrameColor[ Style ] );
  LRColor := ActiveStyleSystemColor( Control, LRFrameColor[ Style ] );
  ...
  begin
    if Style in [ fsFlat, fsStatus, fsPopup ] then
      ...
      Bounds := DrawCtl3DBorderSides( Control, Canvas, Bounds, Style = fsLowered, Sides )
    else if Style in [ fsButtonDown, fsButtonUp ] then
      Bounds := DrawButtonBorderSides( Control, Canvas, Bounds, Style = fsButtonDown, Sides )
    else
    begin
...
begin
  ...
  begin
    ...
    Result := DrawBorderSides( Control, Canvas, Result, BorderOuter, BorderSides );
  ...
  begin
    ...
    Result := DrawBorderSides( Control, Canvas, Result, BorderInner, BorderSides );
...
var
  ...
  StartColor, StopColor: TColor;
begin
  if UsingSystemStyle( Control ) and ( VisualStyle = vsWinXP ) and ActiveStyleServices( Control ).Enabled then
  begin
    ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tebExplorerBarRoot );
    ActiveStyleServices( Control ).DrawElement( Canvas.Handle, ElementDetails, Bounds );
  end
  else if ( VisualStyle <> vsClassic ) and FullColorSupported then
  begin
    if ColorStyle <> gcsCustom then
      GetGradientGroupBarColors( Control, ColorStyle, StartColor, StopColor )
    else
    begin
...
var
  ...
  Bmp: TBitmap;
begin
  if ThemeAware and ActiveStyleServices( Control ).Enabled then
  begin
    if Orientation = orHorizontal then
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tpChunk )
    else
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tpChunkVert );
    ...
    begin
      ...
      ActiveStyleServices( Control ).DrawElement( Canvas.Handle, ElementDetails, ThemeRect );
    end
    else
    begin
      ...
      try
        ...
        ActiveStyleServices( Control ).DrawElement( Bmp.Canvas.Handle, ElementDetails, ThemeRect );
        FlipBitmap( Bmp );
...
begin
  ...
  begin
    if EraseColor = clWindow then
      DrawBorderSides( Control, Canvas, R, FrameStyle, FrameSides )
    else
...
begin
  ...
  begin
    ...
    case UIStyle of
      ...
      uiWindowsXP:
      begin
        ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tcDropDownButtonNormal );
        TempBmp := TBitmap.Create;
        try
          R := Rect( 0, 0, 16, 20 );
          TempBmp.Width := 17;
          TempBmp.Height := 21;
          ActiveStyleServices( Control ).DrawElement( TempBmp.Canvas.Handle, ElementDetails, R );
  ...
  if UIStyle = uiWindows10 then
    DrawModernArrow(dirDown, Round(10 * AScaleFactor), Canvas, Bounds, LColor, AScaleFactor)
  else
    DrawArrowImage(Canvas, Bounds, LColor, dirDown, False, AScaleFactor);
...
begin
  if ASmallSize then
    LSize := Round(2 * AScaleFactor)
  else
    LSize := Round(3 * AScaleFactor);
...
procedure DrawSpinArrow( Canvas: TCanvas; Bounds: TRect; Color, DisabledColor: TColor;
                         Direction: TDirection; Down: Boolean; SmallSize: Boolean; Enabled: Boolean = True; ScaleFactor: Single = 1 );
var
  LColor: TColor;
 begin
   if Enabled then
    ...
    LColor := DisabledColor;
  DrawArrowImage(Canvas, Bounds, LColor, Direction, SmallSize, ScaleFactor);
end; {= DrawSpinArrow =}


procedure DrawSpinArrow( Control: TControl; Canvas: TCanvas; Bounds: TRect; UIStyle: TRzUIStyle; Direction: TDirection; Down: Boolean;
                         SmallSize: Boolean; Enabled: Boolean = True; ScaleFactor: Single = 1 );
var
  C: TColor;
  ElementDetails: TThemedElementDetails;
  R: TRect;
...
begin
  ...
  begin
    ...
    case UIStyle of
      ...
      uiWindowsVista:
      begin
        ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tsDownNormal );
        TempBmp := TBitmap.Create;
        try
          R := Rect( 0, 0, 13, 10 );
          TempBmp.Width := 13;
      ...
      uiCustomVclStyle:
      begin
        LColor := ActiveStyleFontColor( Control, sfButtonTextNormal );
      end;
    end;
  end
  else
  begin
    LColor := ActiveStyleSystemColor( Control, clBtnShadow );
  end;
  DrawArrowImage(Canvas, Bounds, LColor, Direction, SmallSize, ScaleFactor);
end; {= DrawSpinArrow =}
...
procedure DrawCloseX( Canvas: TCanvas; Bounds: TRect; Color: TColor;
                      Down: Boolean; Enabled: Boolean = True; ScaleFactor: Single = 1 );
var
  X, Y, I: Integer;
  LSize: Integer;
begin
  Canvas.Pen.Color := Color;
  LSize := Round(6 * ScaleFactor);
  X := Bounds.Left + (Bounds.Width - LSize) div 2 - Round(ScaleFactor * 3) div 2;
  Y := Bounds.Top + (Bounds.Height - LSize) div 2;
  for I := 1 to Round(ScaleFactor * 3) do
  begin
    Canvas.MoveTo(X, Y);
...
begin
  ...
  case UIStyle of
    ...
    begin
      if Enabled then
        XColor := ActiveStyleFontColor( Control, sfButtonTextNormal )
      else
        XColor := ActiveStyleFontColor( Control, sfButtonTextDisabled );
    end;
  end;

  DrawCloseX( Canvas, Bounds, XColor, Down, Enabled, ScaleFactor );
end; {= DrawCloseX =}
...
procedure DrawCheckMarkImage(Canvas: TCanvas; Bounds: TRect; Color: TColor; AScaleFactor: Double = 1);
var
  X, Y: Integer;
  Size, Size2, Size3: Integer;
  i: Integer;
begin
  Canvas.Pen.Color := Color;
  Size := Round(9 * AScaleFactor);
  Size2 := Size div 2;
  Size := Size2 * 2;
  Size3 := Round(2 * AScaleFactor);
  X := Bounds.Left + (Bounds.Width + Size3) div 2 - Round(2 * AScaleFactor) div 2  + 1;
  Y := Bounds.Top + (Bounds.Height + Size3) div 2;
  X := X - Size2;
  for I := 1 to Round(2 * AScaleFactor) do
  begin
    Canvas.MoveTo(X, Y);
    Canvas.LineTo(X + Size2 - Size3, Y + Size2 - Size3);
    Canvas.LineTo(X + Size, Y - Size3 * 2);
    Dec(Y);
  end;
end;
...
var
  I: Integer;
begin
  for I := 1 to Trunc(AScaleFactor) do
  begin
  ...
  case CheckState of
    ...
    cbChecked:
    begin
      DrawCheckMarkImage(Canvas, Bounds, MarkColor, AScaleFactor);
    end;
...
procedure DrawRadioButton( Canvas: TCanvas; Bounds: TRect; Checked: Boolean;
                           ...
                           ReadOnlyColor: TColor; AScaleFactor: Single = 1);
var
  BackColor, DotColor: TColor;

  procedure DrawRadioButtonElement(AIndex: Byte; AColor: TColor);
  var
    ISize: Integer;
    R: TRect;
    B: TBitmap;
  begin
    ISize := Round(13 * AScaleFactor);
    R := Rect(0, 0, ISize, ISize);
    B := TBitmap.Create(ISize, ISize);
    try
      B.PixelFormat := pf32bit;
      SetBitmapAlpha(B, 0);
      B.Canvas.StretchDraw(R, RadioImages[AIndex]);
      SetBitmapColor(B, ColorToRGB(AColor));
...
begin
  ...
  case DisplayState of
    ...
    bdsDown:
      BackColor := DarkerColor( FocusColor, 20 );
    bdsDisabled:
      BackColor := DisabledColor;
  end;


  DrawRadioButtonElement(0, BackColor);
  DrawRadioButtonElement(1, FrameColor);

  if Checked then
  begin
    if DisplayState = bdsDisabled then
       DotColor := clBtnShadow
    else
      DotColor := MarkColor;
    DrawRadioButtonElement(2, DotColor);
  end;
...
var
  ...
  begin
    ...
    begin
      if Form <> nil then
        LScaleFactor := Form.ScaleFactor
      else
        LScaleFactor := 1;

      if Item.RadioItem then
        DrawMenuRadioImage(Canvas, GlyphRect, FontColor, LScaleFactor)
      else
        DrawMenuCheckImage(Canvas, GlyphRect, FontColor, LScaleFactor);
    end;
  end; {= PaintCheckState =}
...
begin {= PaintMenuItem =}
  ...
  if Form <> nil then
    WInc := MulDiv(20, Form.CurrentPPI, Screen.DefaultPixelsPerInch)
  else
    WInc := 20;
  ...
  begin
    ...
    begin
      GetGradientSelectionColors( nil, FGradientColorStyle, SelBarFrameColor, SelBarStartColor, SelBarStopColor );
      GetGradientPanelColors( nil, FGradientColorStyle, IcoStartColor, IcoStopColor );
    end
    else
    begin
  ...
  begin
    SelBarFrameColor := ActiveStyleSystemColor( nil, clHighlight );
    C := BlendColors( ActiveStyleSystemColor( nil, clHighlight ), ActiveStyleSystemColor( nil, clWindow ), 77 );
    SelBarStartColor := LighterColor( C, 20 );
    SelBarStopColor := C;
    IcoStopColor := LighterColor( ActiveStyleSystemColor( nil, clBtnFace ), 30 );
    IcoStartColor := LighterColor( IcoStopColor, 30 );
    FontColor := ActiveStyleFontColor( nil, sfMenuItemTextNormal );
    DisabledFontColor := ActiveStyleFontColor( nil, sfMenuItemTextDisabled );
    BackColor := ActiveStyleSystemColor( nil, clMenu );
  ...
  begin
    if ImgList <> nil then
      GlyphRect.Left := GlyphRect.Right - Max(ImgList.Width, WInc - 4) - 8
    else
      GlyphRect.Left := GlyphRect.Right - WInc - 4;
    CaptionRect := Rect;
  ...
  begin
    if ImgList <> nil then
      GlyphRect.Right := GlyphRect.Left + Max(ImgList.Width, WInc - 4) + 8
    else
      GlyphRect.Right := GlyphRect.Left + WInc + 4;
    CaptionRect := Rect;
    if not TopLevel then
  ...
  begin
    ...
    if Form <> nil then
      Canvas.Font.Height := MulDiv(Canvas.Font.Height, Form.CurrentPPI, Screen.DefaultPixelsPerInch);
  end
  else
  begin
    Canvas.Font := Screen.MenuFont;
    if Form <> nil then
      Canvas.Font.Height := MulDiv(Canvas.Font.Height, Form.CurrentPPI, Screen.PixelsPerInch);
  end;
  ...
  begin
    ...
    begin
      ...
      else if ActiveStyleServices( nil ).Enabled then
      begin
        ElementDetails := ActiveStyleServices( nil ).GetElementDetails( tmMenuRoot );
        ActiveStyleServices( nil ).DrawElement( Canvas.Handle, ElementDetails, CaptionRect );
      end
      else
      begin
        Canvas.Brush.Color := clBtnFace;
...
begin
  ...
  begin
    ...
    if Item.ShortCut <> 0 then
      Width := Canvas.TextWidth( Item.Caption + ShortCutToText( Item.ShortCut ) ) + ImgWidth + WInc
    else
      Width := Canvas.TextWidth( Item.Caption ) + ImgWidth + WInc;
