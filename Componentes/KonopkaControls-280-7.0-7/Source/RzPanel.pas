{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzPanel and descendants and modified
      components to honor StyleElements settings when VCL Styles are used.
...
begin
  ...
  begin
    GetGradientPanelColors( Self, FGradientColorStyle, StartColor, StopColor );
  end
  else
  begin
...
begin
  ...
  begin
    FrameColor := GetGradientPanelFrameColor( Self, FGradientColorStyle );
...
begin
  ...
  begin
    ...
    begin
      Details := ActiveStyleServices( Self ).GetElementDetails( tpPanelBackground );
      C := Font.Color;  // Just calculating the rectangle, no need to worry about font color
      ActiveStyleServices( Self ).DrawText( Canvas.Handle, Details, Caption, TextRct, TTextFormatFlags( Flags ), C );
    ...
    begin
      ...
      begin
        if not ActiveStyleServices( Self ).GetElementColor( Details, ecTextColor, C ) or ( C = clNone ) then
          C := Font.Color;
        if not Enabled then
          C := ActiveStyleServices( Self ).GetSystemColor( clGrayText );
      end
      else
      begin
        C := Font.Color;
      ...
      ActiveStyleServices( Self ).DrawText( Canvas.Handle, Details, Caption, Rect, TTextFormatFlags( Flags ), C );
    end;
  end;
...
begin {= TRzCustomPanel.DrawGrid =}
  ...
  GC := ActiveStyleSystemColor( Self, FGridColor );
...
begin
  ...
  if UsingSystemStyle( Self ) or ( VisualStyle = vsClassic ) or not UseActiveStyleColor( Self ) then
  begin
  ...
  R := DrawInnerOuterBorders( Self, Canvas, R, FBorderOuter, FBorderInner, BorderWidth, FBorderSides, BevelWidth,
...
begin
  if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
    ...
    Color := ActiveStyleSystemColor( Self, clBtnFace );
end;


procedure TRzCustomPanel.UpdateStyleElements;
begin
  UpdatePanelColor;
  Invalidate;
end;
...
procedure TRzCustomGroupBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
...
var
  ...
  begin
    Result := Rect( 0, 0, 0, 0 );
    GetThemeTextExtent( ActiveStyleServices( Self ).Theme[ teButton ], DC, Details.Part, Details.State,
...
begin {= TRzCustomGroupBox.Paint =}
  ...
  begin
    ...
    if ( FVisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled and UsingSystemStyle( Self ) and
       ( FGroupStyle in [ gsStandard, gsFlat ] ) then
    begin
      GroupBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbGroupBoxNormal );
  ...
  if ( FVisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled and UseActiveStyleColor( Self ) and
     ( FGroupStyle in [ gsStandard, gsFlat ] ) then
  begin
    ...
    if Enabled then
      ...
      GroupBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbGroupBoxDisabled );

    SaveIndex := SaveDC(Canvas.Handle);
    try
      ActiveStyleServices( Self ).DrawElement( Canvas.Handle, GroupBoxDetails, R );
    finally
    ...
    begin
      ...
      if seFont in StyleElements then
        ActiveStyleServices( Self ).DrawText( Canvas.Handle, GroupBoxDetails, Caption, R, TTextFormatFlags( DrawTextAlignments[ Alignment ] ) )
      else
        DrawText(Canvas.Handle, Caption, Length(Caption), R, DrawTextAlignments[Alignment]);
      ...
      begin
        if UsingSystemStyle( Self ) then
          ...
          DrawFocusBorder( Canvas, R, ActiveStyleFontColor( Self, sfGroupBoxTextNormal ) );
      end;
    end;
  end
  else // No Themes
  begin
    ...
    begin
      ...
      begin
        Canvas.Brush.Color := ActiveStyleSystemColor( Self, Color );
        Canvas.FillRect( R );
      end;
      ...
      case FGroupStyle of
        ...
        begin
          ...
          begin
            ...
            DrawBorderSides( Self, Canvas, R, BorderOuter, sdAllSides );
        end;

        gsTopLine:
        begin
          if BorderOuter = fsNone then
            DrawBorderSides( Self, Canvas, R, fsGroove, [ sdTop ] )
          else if BorderOuter in [ fsFlat, fsFlatBold ] then
          begin
            if UsingSystemStyle( Self ) then
              C := AdjustColor( FFlatColor, FFlatColorAdjustment )
            ...
            DrawBorderSides( Self, Canvas, R, BorderOuter, [ sdTop ] );
        end;
        ...
        begin
          ...
          begin
            GetGradientStatusBarColors( Self, GradientColorStyle, StopColor,
    ...
    begin
      ...
      if UsingSystemStyle( Self ) then
      begin
        if Enabled and ( FVisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
        begin
          Canvas.Font.Color := GetXPThemeColor( Self, xptcGroupBoxFont );
        end
      ...
      begin
        ...
        if Enabled then
          Canvas.Font.Color := ActiveStyleFontColor( Self, sfPanelTextNormal )
        else
          Canvas.Font.Color := ActiveStyleFontColor( Self, sfPanelTextDisabled );
      end;
  ...
  begin
    ...
    begin
      if FShowDownVersion then
        CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedPressed )
      else if FMouseOverCheckBox then
        CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedHot )
      else
      CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedNormal );
    end
    else
    begin
      CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedDisabled );
    end;
  end
  else // Unchecked
  begin
    ...
    begin
      if FShowDownVersion then
        CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedPressed )
      else if FMouseOverCheckBox then
        CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedHot )
      else
      CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedNormal );
    end
    else
    begin
      CheckBoxDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedDisabled );
    end;
  end;

  ActiveStyleServices( Self ).DrawElement( Canvas.Handle, CheckBoxDetails, R );
end;
  ...
  begin
    ...
    begin
      if Width > Height then
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( ttbSeparatorVertNormal )
      else
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( ttbSeparatorNormal );
      R := ClientRect;
...
var
  ...
  begin
    ...
    begin
      GetGradientPanelColors( FToolbar, FToolbar.GradientColorStyle, StartColor, StopColor );
    end
    else
    begin
...
begin {= TRzToolbarPopupButton.DrawBackground =}
  ...
  case FToolbar.VisualStyle of
    vsClassic, vsWinXP:
    begin
      if ( FToolbar.VisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
      begin
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( trRebarRoot );
        ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, R );

        if Down then
          ElementDetails := ActiveStyleServices( Self ).GetElementDetails( trChevronPressed )
        else
          ElementDetails := ActiveStyleServices( Self ).GetElementDetails( trChevronNormal );
        ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, R );
      end
      else
      begin
...
begin
  ...
  if ActiveStyleServices( Self ).Enabled then
  begin
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tcDropDownButtonNormal );
    TempBmp := TBitmap.Create;
    try
      R := Rect( 0, 0, 16, 20 );
      TempBmp.Width := 17;
      TempBmp.Height := 21;
      ActiveStyleServices( Self ).DrawElement( TempBmp.Canvas.Handle, ElementDetails, R );
...
begin
  ...
  if not ( ( FToolbar.VisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled ) then
    DrawChevron( R );
...
procedure TRzToolbar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
...
begin
  ...
  begin
    ...
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( trRebarRoot );
    ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, ClientRect );
  end
  else
  begin
    inherited;
...
begin
  ...
  if ActiveStyleServices( Self ).Available then
  begin
...
begin
  if ( FVisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
  begin
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tsGripper );
    R.Left := R.Right - ScaleValue(20);
    ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, R, nil, Self.CurrentPPI);
  end
  else
  begin
    if FVisualStyle = vsGradient then
    begin
      GetGradientStatusBarColors( Self, FGradientColorStyle, StartColor,
    ...
    FSizeGripCanvas.Rectangle( X - ScaleValue(4),  Y - ScaleValue(12), X - ScaleValue(2),  Y - ScaleValue(10) );
    FSizeGripCanvas.Rectangle( X - ScaleValue(4),  Y - ScaleValue(8),  X - ScaleValue(2),  Y - ScaleValue(6) );
    FSizeGripCanvas.Rectangle( X - ScaleValue(4),  Y - ScaleValue(4),  X - ScaleValue(2),  Y - ScaleValue(2) );
    FSizeGripCanvas.Rectangle( X - ScaleValue(8),  Y - ScaleValue(8),  X - ScaleValue(6),  Y - ScaleValue(6) );
    FSizeGripCanvas.Rectangle( X - ScaleValue(8),  Y - ScaleValue(4),  X - ScaleValue(6),  Y - ScaleValue(2) );
    FSizeGripCanvas.Rectangle( X - ScaleValue(12), Y - ScaleValue(4),  X - ScaleValue(10), Y - ScaleValue(2) );
...
begin
  if ( FVisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
  begin
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tsPane );
    ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, R );
  end
  ...
  begin
    ...
    begin
      ...
      DrawBorder( Self, Canvas, R, FSimpleFrameStyle );
  end;
...
begin
  if GradientColorStyle <> gcsCustom then
  begin
    GetGradientStatusBarColors( Self, GradientColorStyle, StartColor, StopColor, C );
  end
  else
  begin
...
begin
  if ( FVisualStyle = vsWinXP ) and ActiveStyleServices( Self ).Enabled then
  begin
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tsStatusRoot );
    R := Rect( 0, 0, Width, Height );
    ActiveStyleServices( Self ).DrawElement( Canvas.Handle, ElementDetails, R );
    ...
    begin
      ...
      begin
        CR := ClientRect;
        ActiveStyleServices( Self ).DrawText( Canvas.Handle, ElementDetails, WideString( FSimpleCaption ), CR,
                                              TTextFormatFlags( dt_SingleLine or dt_NoPrefix or dt_VCenter ) );
      end;
    end;
