{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzRadioButton and TRzCheckBox and
...
type
  ...
  protected
    ...
    function IsAlphaGlyph: Boolean; virtual;
  ...
  protected
    ...
    function IsAlphaGlyph: Boolean; override;
...
function TRzCustomGlyphButton.IsAlphaGlyph: Boolean;
begin
  Result := False;
end;

procedure TRzCustomGlyphButton.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  if M <> D then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      if IsAlphaGlyph or (UseThemes and not FUseCustomGlyphs) then
      begin
    ...
    begin
      SelectGlyph( FGlyph );
      Phase1Bmp.Canvas.Brush.Color := ActiveStyleSystemColor( Self, Color );
    ...
    begin
      ...
      begin
        ...
        Phase2Bmp.Canvas.Brush.Color := ActiveStyleSystemColor( Self, clWindow );
...
begin
  ...
  try
    ...
    begin
	  //!! Verify this new addition is good. I think it is. Add comment at top.
      if FTransparent and not DoubleBuffered then
      begin
    ...
    begin
      MemImage.Canvas.Brush.Color := ActiveStyleSystemColor( Self, Color );
    ...
    begin
      ...
      begin
        DrawFocusBorder( MemImage.Canvas, R, ActiveStyleSystemColor( Self, clWindowText ) );
      end;
    end;
...
begin
  ...
  begin
    ...
    ActiveStyleServices( Self ).GetElementSize( Canvas.Handle, Details, esActual, Size, CurrentPPI );
  ...
  begin
    FGlyphWidth := ScaleValue(DefaultGlyphWidth);
    FGlyphHeight := ScaleValue(DefaultGlyphHeight);
  end;
end;


function TRzRadioButton.IsAlphaGlyph: Boolean;
begin
  Result := not FUseCustomGlyphs and not UseThemes;
end;
      ...
      begin
        ...
        begin
          if FShowDownVersion then
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonCheckedPressed )
          else if FMouseOverButton then
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonCheckedHot )
          else
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonCheckedNormal );
        end
        else
        begin
          ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonCheckedDisabled );
        end;
      end
      else // Unchecked
      begin
        ...
        begin
          if FShowDownVersion then
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonUncheckedPressed )
          else if FMouseOverButton then
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonUncheckedHot )
          else
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonUncheckedNormal );
        end
        else
        begin
          ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonUncheckedDisabled );
        end;
      end;

      ActiveStyleServices( Self ).DrawElement( Glyph.Canvas.Handle, ElementDetails, R, nil, CurrentPPI );
    end
...
begin
  ...
  begin
    if Enabled then
      Result := ActiveStyleFontColor( Self, sfRadioButtonTextNormal )
    else
      Result := ActiveStyleFontColor( Self, sfRadioButtonTextDisabled );
  end
  else
...
begin
  if not UseCustomGlyphs and UseThemes then
  begin
    Details := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedNormal );
    ActiveStyleServices( Self ).GetElementSize( Canvas.Handle, Details, esActual, Size, CurrentPPI );
    if ( FGlyphWidth <> Size.cx ) or ( FGlyphHeight <> Size.cy ) then
    begin
      FGlyphWidth := Size.cx;
  ...
  begin
    ...
    begin
      case FState of
        ...
        begin
          ...
          begin
            if FShowDownVersion then
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedPressed )
            else if FMouseOverButton then
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedHot )
            else
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedNormal );
          end
          else
          begin
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedDisabled );
          end;
        end;
        ...
        begin
          ...
          begin
            if FShowDownVersion then
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedPressed )
            else if FMouseOverButton then
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedHot )
            else
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedNormal );
          end
          else
          begin
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxCheckedDisabled );
          end;
        end;
        ...
        begin
          ...
          begin
            if FShowDownVersion then
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxMixedPressed )
            else if FMouseOverButton then
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxMixedHot )
            else
              ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxMixedNormal );
          end
          else
          begin
            ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxMixedDisabled );
          end;
        end;
      end;
    ...
    begin
      ...
      DrawCheckBox( Glyph.Canvas, R, FState, DisplayState, Focused, FHotTrackStyle,
                    ...
                    FReadOnlyColorOnFocus, FReadOnlyColor, FScaleFactor );
    end
    else // Default OS appearance
    begin
      case FState of
...
begin
  ...
  begin
    if Enabled then
      Result := ActiveStyleFontColor( Self, sfCheckBoxTextNormal )
    else
      Result := ActiveStyleFontColor( Self, sfCheckBoxTextDisabled );
  end
  else
    Result := Self.Font.Color;
end;
