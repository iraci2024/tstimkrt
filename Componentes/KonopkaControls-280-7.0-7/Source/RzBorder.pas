{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzColorPicker and modified the
      component to honor StyleElements settings when VCL Styles are used.
    * Surface StyleName property in RAD Studio 10.4+.
  ------------------------------------------------------------------------------
  6.1.10 (05 Sep 2014)
...
begin
  ...
  R := DrawInnerOuterBorders( Self, Canvas, R, FBorderOuter, FBorderInner, BorderWidth, FBorderSides, FBevelWidth,
...
begin
  ...
  if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
    ...
    Canvas.Brush.Color := ActiveStyleColor( Self, scPanel );
  ...
  if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
    ...
    Canvas.Font.Color := ActiveStyleFontColor( Self, sfStatusPanelTextNormal );
...
begin
  ...
  begin
    ...
    begin
      FThemeBorderColor := GetXPThemeColor( Self, xptcEditBorder );
      BorderOuter := fsFlat;
  ...
  begin
    BorderColor := ActiveStyleSystemColor( Self, clWindow );
    ParentColor := False;
    FThemeBorderColor := ActiveStyleSystemColor( Self, clBtnShadow );
    BorderOuter := fsFlat;
    FlatColor := FThemeBorderColor;
  end;
...
var
  ...
  begin
    ...
    begin
      R := DrawBox( Canvas, Bounds, ActiveStyleSystemColor( Self, clBtnShadow ) );
      Canvas.Brush.Color := ActiveStyleColor( Self, scButtonNormal );
    end;
  ...
  begin
    ...
    begin
      DrawFilledBox( Canvas, CR, DarkerColor( ActiveStyleSystemColor( Self, clHighlight ), 20 ),
                     ActiveStyleSystemColor( Self, clHighlight ) );
    end
    else if FHighlightColorIndex = sciNoColor  then
    begin
      DrawFilledBox( Canvas, CR, ActiveStyleSystemColor( Self, clHighlight ),
                     ActiveStyleSystemColor( Self, clWindow ) );
    end
    else
      CR := R;
  ...
  begin
    ...
    DrawFilledBox( Canvas, CellRect, ActiveStyleSystemColor( Self, clBtnShadow ), FDefaultColor );
    ExcludeClipRect( Canvas.Handle, CellRect.Left, CellRect.Top, CellRect.Right, CellRect.Bottom );

    if FSelColorIndex = sciDefault then
    begin
      DrawFilledBox( Canvas, SampleRect, DarkerColor( ActiveStyleSystemColor( Self, clHighlight ), 20 ),
                     ActiveStyleSystemColor( Self, clHighlight ) );
    ...
    begin
      ...
      DrawBox( Canvas, AreaRect, ActiveStyleSystemColor( Self, clHighlight ) );
  ...
  begin
    ...
    CellRect := SampleRect;
    InflateRect( CellRect, -3, -3 );
    DrawFilledBox( Canvas, CellRect, ActiveStyleSystemColor( Self, clBtnShadow ), FCustomColor );
    ExcludeClipRect( Canvas.Handle, CellRect.Left, CellRect.Top, CellRect.Right, CellRect.Bottom );
    ...
    begin
      DrawFilledBox( Canvas, SampleRect, ActiveStyleSystemColor( Self, clHighlight ), ActiveStyleSystemColor( Self, clWindow ) );
      ExcludeClipRect( Canvas.Handle, SampleRect.Left, SampleRect.Top, SampleRect.Right, SampleRect.Bottom );
    end;
    ...
    begin
      ...
      DrawBox( Canvas, TempRect, ActiveStyleSystemColor( Self, clHighlight ) );
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        CellRect := R;
        InflateRect( CellRect, -3, -3 );
        DrawFilledBox( Canvas, CellRect, ActiveStyleSystemColor( Self, clBtnShadow ), ColorSwatches[ CellIndex ].Color );
        ExcludeClipRect( Canvas.Handle, CellRect.Left, CellRect.Top, CellRect.Right, CellRect.Bottom );

        if FSelColorIndex = CellIndex then
        begin
          DrawFilledBox( Canvas, R, DarkerColor( ActiveStyleSystemColor( Self, clHighlight ), 20 ),
                         ActiveStyleSystemColor( Self, clHighlight ) );
          ExcludeClipRect( Canvas.Handle, R.Left, R.Top, R.Right, R.Bottom );
        end
        else if FHighlightColorIndex = CellIndex then
        begin
          DrawFilledBox( Canvas, R, ActiveStyleSystemColor( Self, clHighlight ),
                         ActiveStyleSystemColor( Self, clWindow ) );
          ExcludeClipRect( Canvas.Handle, R.Left, R.Top, R.Right, R.Bottom );
        end;
...
begin {= TRzColorPicker.Paint =}
  ...
  begin
    C := ActiveStyleColor( Self, scButtonDisabled );
    R := Rect( 0, 0, Width, Height );
    R := DrawBox( Canvas, R, C );
  end;
  ...
  begin
    ...
    begin
      CellRect := R;
      InflateRect( CellRect, -3, -3 );
      DrawFilledBox( Canvas, CellRect, ActiveStyleSystemColor( Self, clBtnShadow ),
...
var
  LCellSize: Integer;
begin
  LCellSize := ScaleValue(18);

  NewWidth := LCellSize * 8 + 2 * ClientRect.Left;
  NewHeight := LCellSize * 5 + 2 * ClientRect.Top;

  if FShowSystemColors then
    Inc( NewHeight, 4 * LCellSize + 8 );
