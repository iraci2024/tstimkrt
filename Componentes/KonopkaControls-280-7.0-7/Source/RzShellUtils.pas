...
function ShellGetSystemImageList( Size: TRzShellIconSize ): THandle;
  ...
  var
    ...
    w: Word;
  begin
    hShell32 := Winapi.Windows.LoadLibrary( 'shell32.dll' );
    if ( hShell32 = 0 ) then
      Exit;
    try
      w := 0;
      ...
      hicon := Winapi.Windows.LoadImage( hShell32, PChar( SIC_SHARING_HAND ), IMAGE_ICON, 16, 16, LR_SHARED );
      ImageList_SetOverlayImage( h16, ImageList_AddIcon( h16, hicon ), 1 );
      DestroyIcon( hicon );

      hicon := Winapi.Windows.LoadImage( hShell32, PChar( SIC_SHARING_HAND ), IMAGE_ICON, 32, 32, LR_SHARED );
      ImageList_SetOverlayImage( h32, ImageList_AddIcon( h32, hicon ), 1 );
      DestroyIcon( hicon );

      hicon := Winapi.Windows.LoadImage( hShell32, PChar( SIC_SHORTCUT ), IMAGE_ICON, 16, 16, LR_SHARED );
      ImageList_SetOverlayImage( h16, ImageList_AddIcon( h16, hicon ), 2 );
      DestroyIcon( hicon );

      hicon := Winapi.Windows.LoadImage( hShell32, PChar( SIC_SHORTCUT ), IMAGE_ICON, 32, 32, LR_SHARED );
...
initialization
  ...
  Winapi.Windows.GetVersionEx( gOsVer );
  _SHGetMalloc( g_IShm );
