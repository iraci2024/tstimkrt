...
procedure TRzCustomTreeView.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FItemHeight := MulDiv(FItemHeight, M, D);
  FItemHeightMargin := MulDiv(FItemHeightMargin, M, D);
  inherited;
  UpdateItemHeight;
end;
...
begin
  ...
  begin
    ...
    if (FItemHeight = 0) and FItemHeightChanging then
    begin
...
begin
  ...
  if not HideSelection and not ( Focused or ( FocusWnd = EditWnd ) ) and
     ( Selected <> nil ) and not UsingSystemStyle( Self ) then
  begin
...
begin
  if ( csLoading in ComponentState ) or ( not UsingSystemStyle( Self ) ) then
    Exit;
...
begin
  ...
  H := ScaleValue(Max( 16, FItemHeight ));
  W := ScaleValue(20);

  if ActiveStyleServices( Self ).Enabled and HandleAllocated then
  begin
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedNormal );
    if ActiveStyleServices( Self ).GetElementSize( Canvas.Handle, ElementDetails, esActual, Size, CurrentPPI ) then
    begin
      W := Size.cx;
      H := Size.cy;
    end;
  ...
  try
    ...
    R := Rect( 0, 0, W, H );
    CheckRect := Rect( 0, 0, ScaleValue(13), ScaleValue(13) );

    if UsingSystemStyle( Self ) then
      ChkBmp.Canvas.Brush.Color := Color
    else
      ChkBmp.Canvas.Brush.Color := ActiveStyleColor( Self, scTreeView );
    ...
    begin
      ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedNormal );
      ActiveStyleServices( Self ).DrawElement( ChkBmp.Canvas.Handle, ElementDetails, R, nil, CurrentPPI );
    ...
    begin
      ...
      try
        ...
        DrawCheckBox( ChkBmp.Canvas, CheckRect, cbUnchecked, bdsNormal, False,
                      ...
                      clLime, DisabledColor, clLime, clRed, False, False, clWindow, FScaleFactor );
