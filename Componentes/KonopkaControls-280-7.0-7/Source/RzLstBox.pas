{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzListBox and descendants and modified
...
procedure TRzCustomListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
...
begin
  ...
  begin
    ItemColor := ActiveStyleColor( Self, scListBox );

    if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
      ItemFontColor := Self.Font.Color
    else
      ItemFontColor := ActiveStyleFontColor( Self, sfListItemTextNormal );
  end;
...
begin
  ...
  begin
    if ActiveStyleServices( Self ).Enabled and FGroupColorFromTheme then
    begin
      ForeColor := GetXPThemeColor( Self, xptcListGroupFont );
      BackColor := GetXPThemeColor( Self, xptcListGroupFill );
    end
    else
    begin
  ...
  begin
    if UseActiveStyleFontColor( Self ) then
      ForeColor := ActiveStyleFontColor( Self, sfButtonTextNormal )
    else
      ForeColor := FGroupFont.Color;

    if UseActiveStyleColor( Self ) then
    begin
      BackColor := ActiveStyleSystemColor( Self, clHighlight );
      ItemColor := ActiveStyleColor( Self, scListBox );
    end
    else
    begin
...
begin
  ...
  begin
    ...
    Winapi.Windows.SetFocus( FPopupEdit.Handle );
    FPopupVisible := True;
  end;
end;
...
begin
  ...
  begin
    ...
    if FOldIndex <> -1 then
    begin
      FOldCursor := Winapi.Windows.GetCursor;
...
begin
  ...
  begin
    ...
    Winapi.Windows.SetCursor( FOldCursor );
    FMoving := False;
  end;
end;
...
class constructor TRzFontListBox.Create;
begin
...
class destructor TRzFontListBox.Destroy;
begin
...
begin
  ...
  with Msg.DrawItemStruct^ do
    rcItem.Left := rcItem.Left + ScaleValue(24);
  inherited;
end;
