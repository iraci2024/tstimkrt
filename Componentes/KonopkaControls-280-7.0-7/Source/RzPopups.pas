{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzCalendar, TRzTimePicker, and
      TRzCalculator and modified the components to honor StyleElements settings
      when VCL Styles are used.
    * Surfaced StyleName property in RAD Studio 10.4+.
    * Fixed auto sizing issue in TRzCalculator.
    * Fixed issue in TRzCalendar where control would allow navigating beyond
      1/1/0001 and 12/31/9999, which would lead to an exception.
  ------------------------------------------------------------------------------
  6.1.10 (05 Sep 2014)
...
procedure DrawButton( Control: TControl; Canvas: TCanvas; Bounds: TRect; const Caption: string;
...
begin
  ...
  begin
    if Down then
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tbPushButtonPressed )
    else
      ElementDetails := ActiveStyleServices( Control ).GetElementDetails( tbPushButtonNormal );
    ...
    begin
      ...
      try
        ...
        ActiveStyleServices( Control ).DrawElement( B.Canvas.Handle, ElementDetails, R );
  ...
  begin
    ...
    begin
      if Down then
        Bounds := DrawButtonBorder( Control, Canvas, Bounds, True )
      else
      begin
        if Flat then
          Bounds := DrawBorder( Control, Canvas, Bounds, fsFlat )
        else
          Bounds := DrawCtl3DBorder( Control, Canvas, Bounds, False );
      end;
    end
    else
    begin
...
var
  ...
  begin
    ...
    begin
      Canvas.Pen.Color := ActiveStyleFontColor( Self, sfEditBoxTextDisabled );
    end;
...
begin {= TRzCalendar.Paint =}
  ...
  begin
    if FThemeAware and ActiveStyleServices( Self ).Enabled then
    begin
      C := GetXPThemeColor( Self, xptcEditBorder );
  ...
  begin
    R := Rect( 0, 0, Width, Height );
    Details := ActiveStyleServices( Self ).GetElementDetails( teEditTextNormal );

    B := TBitmap.Create;
    try
      B.Width := Width;
      B.Height := Height;
      ActiveStyleServices( Self ).DrawElement( B.Canvas.Handle, Details, R );
      Canvas.Draw( 0, 0, B );
    finally
      B.Free;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        if FThemeAware and ActiveStyleServices( Self ).Enabled then
        begin
          GetGradientPanelColors( Self, gcsMSOffice, TempC, C );
        end
        else
        begin
      ...
      begin
        if UseActiveStyleColor( Self ) then
        begin
          C := ActiveStyleColor( Self, scGenericGradientBase );
        end
        else
        begin
          if not DrawFlat then
  ...
  begin
    ...
    begin
      ...
      DrawButton( Self, Canvas, Areas[ caTodayButton ], S,
    ...
    begin
      ...
      DrawButton( Self, Canvas, Areas[ caClearButton ], S,
                  FButtonColor, GetButtonFontColor, DrawFlat,
    ...
    begin
      ...
      if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
        ...
        Canvas.Font.Color := ActiveStyleFontColor( Self, sfEditBoxTextNormal );
    ...
    if (ceFillDays in FElements ) and
       ( ( ViewYear <> 1 ) or ( ViewMonth <> 1 ) ) and
       ( ( ViewYear <> 9999 ) or ( ViewMonth <> 12 ) ) then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          DayColor := ActiveStyleColor( Self, scEdit );
        end;
        DayFontStyle := [];
        ...
        begin
          if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
            ...
            DayFontColor := ActiveStyleFontColor( Self, sfEditBoxTextNormal );
        ...
        begin
          if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
            ...
            DayFontColor := ActiveStyleFontColor( Self, sfEditBoxTextDisabled );

          if ( Year > 0 ) and ( Year < 10000 ) then
          begin
        ...
        begin
          ...
          if DrawDate = Trunc( System.SysUtils.Date ) then
          begin
...
begin
  ...
  if FShowDays then
  begin
    Inc( NewHeight, ( FCharSize.Y + ScaleValue(2) ) * 6 + 1 );

    if ceDaysOfWeek in FElements then
      Inc( NewHeight, ( FCharSize.Y + 2 ) + 1 );

    NewWidth := ( FCharSize.X * 2 +  ScaleValue(5) ) * 7;
    ...
    if ( [ ceTodayButton, ceClearButton ] * FElements ) <> [ ] then
    begin
      Inc( NewHeight, FCharSize.Y + ScaleValue(15) + 2 );
...
begin
  ...
  Winapi.Windows.SetFocus( FYearSpinner.Handle );
...
begin
  ...
  begin
    ...
    if FDate = 0 then
      SetViewDate( System.SysUtils.Date )
    else
      SetViewDate( FDate );
...
begin
  ...
  begin
    ...
    begin
      FFirstDateInGrid := EncodeDate( 1, 1, 1 );
    end;
...
var
  ...
  begin
    ...
    try
      ...
      if FThemeAware and ActiveStyleServices( Self ).Enabled then
      begin
        Details := ActiveStyleServices( Self ).GetElementDetails( tbCheckBoxUncheckedNormal );
        if ActiveStyleServices( Self ).GetElementSize( Canvas.Handle, Details, esActual, Size, CurrentPPI ) then
        begin
          W := Size.cx;
          H := Size.cy;
        end;
      ...
      begin
        ...
        begin
          if Checked then
            Details := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonCheckedNormal )
          else
            Details := ActiveStyleServices( Self ).GetElementDetails( tbRadioButtonUncheckedNormal );
          ActiveStyleServices( Self ).DrawElement( Bmp.Canvas.Handle, Details, R, nil, CurrentPPI );
        end
        else
        begin
          if Checked then
          ...
          DrawFrameControl(Bmp.Canvas.Handle, R, dfc_Button, Flags );
        end;
      end
      else // VCL Styles
      begin
        Bmp.Canvas.Brush.Color := ActiveStyleColor( Self, scEdit );
        Bmp.Canvas.FillRect( R );
      ...
      R := System.Classes.Bounds( Bounds.Left, Bounds.Top + Offset, W, H );
...
begin {= TRzTimePicker.Paint =}
  ...
  begin
    if FCaptionSet <> '' then
      ...
      S := sRzCaptionSet;
    DrawButton( Self, Canvas, FSetRect, S, FButtonColor, GetButtonFontColor, DrawFlat,
...
begin {= TRzTimePicker.DrawClock =}
  ...
  begin
    if Focused then
      Canvas.Pen.Color := ActiveStyleColor( Self, scGenericGradientEnd )
    else
      Canvas.Pen.Color := ActiveStyleColor( Self, scWindow );

    Canvas.Brush.Color := ActiveStyleColor( Self, scWindow );
  end;
  ...
  // Draw Ticks
  // Ticks are like text, and therefore we check the seFont StyleElement
  if UsingSystemStyle( Self ) or not UseActiveStyleFontColor( Self ) then
  begin
  ...
  begin
    Canvas.Brush.Color := ActiveStyleColor( Self, scGenericGradientBase );
    Canvas.Pen.Color := ActiveStyleColor( Self, scGenericGradientBase );
    MinuteTickColor := ActiveStyleFontColor( Self, sfEditBoxTextDisabled );
  end;
  ...
  begin
    Canvas.Brush.Color := ActiveStyleFontColor( Self, sfEditBoxTextNormal );
    Canvas.Pen.Color := ActiveStyleFontColor( Self, sfEditBoxTextNormal );
  end;
...
var
  ...
  S: string;
  CR: TRect;
  RadioAreaSize: Integer;
begin
  CR := ClientRect;
  AdjustClientRect( CR );

  RadioAreaSize := ScaleValue(22);
  ...
  FAMRect := Bounds( Margin, Height - Margin - FontHeight, Canvas.TextWidth( S ) + RadioAreaSize, FontHeight );
  ...
  FPMRect := Bounds( FAMRect.Right + Margin, Height - Margin - FontHeight, Canvas.TextWidth( S ) + RadioAreaSize, FontHeight );
...
begin
  ...
  begin
    ...
    if FHowToUseMsg = '' then
      HintStr := System.SysUtils.Format( sRzHowToSelectTime, [ FRestrictMinutesBy ] )
    else
...
//    InflateRect( R, 1, 1 );
  ...
  DrawButton( Self, Canvas, R, Captions[ Area ],
...
begin
  ...
  begin
    if FThemeAware and ActiveStyleServices( Self ).Enabled then
    begin
      FrameColor := GetXPThemeColor( Self, xptcEditBorder );
      R := Rect( 0, 0, Width, Height );
  ...
  begin
    ...
    FrameColor := ActiveStyleFontColor( Self, sfEditBoxTextDisabled );
  end;
...
begin
  ...
  begin
    ElementDetails := ActiveStyleServices( Self ).GetElementDetails( teEditTextDisabled );
...
var
  ...
  begin
    ...
    Winapi.Windows.SetFocus( FPopupPanel.Handle );
