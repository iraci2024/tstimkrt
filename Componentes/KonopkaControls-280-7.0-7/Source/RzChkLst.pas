{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzCheckList and modified the
      component to honor StyleElements settings when VCL Styles are used.
    * Surfaced StyleName property in RAD Studio 10.4+.
    * Fixed issue where first item in a TRzCheckList may not be indented
      correctly when using a VCL Style with a larger check box style.
    * Fixed issue where check box background in a TRzCheckList would be
      displayed in the wrong color under certain VCL Styles.
    * Fixed issue in TRzCheckList where loading strings from a file or stream
      that contained empty strings would cause an exception.
  ------------------------------------------------------------------------------
  6.2.4  (15 Oct 2018)
    * Surfaced inherited GroupPrefix property in TRzCheckList
  ------------------------------------------------------------------------------
  6.0    (09 Oct 2011)
...
procedure TRzCheckList.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  FGlyphWidth := MulDiv( FGlyphWidth, M, D );
  FGlyphHeight := MulDiv( FGlyphHeight, M, D );
...
begin
  ...
  begin
    ...
    try
      R := System.Classes.Rect( 0, 0, FGlyphWidth, FGlyphHeight );

      if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
        FGlyph.Canvas.Brush.Color := Color
      else
        FGlyph.Canvas.Brush.Color := ActiveStyleColor( Self, scListBox );
      FGlyph.Canvas.FillRect( R );
...
var
  ...
  Size: TSize;
begin
  //!! Probably should do this GetElementSize call for Windows as well.


  if not UsingSystemStyle( Self ) and not UseCustomGlyphs then
  begin
    (*
...
procedure TRzCheckList.CMStyleChanged( var Msg: TMessage );
begin
  inherited;
  UpdateGlyphDimensions;
end;
