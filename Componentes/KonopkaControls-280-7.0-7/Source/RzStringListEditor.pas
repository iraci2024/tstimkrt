    ...
    try
      ...
      case Dialog.ShowModal of
        ...
        begin
          ...
          if Module <> nil then
          begin
            StringsFileSystem.GetTStringsProperty( Ident, Component, GetName ).DiskAge := DateTimeToFileDate( Age );
...
begin
  ...
  edtStrings.Font.Name := DelphiIni.ReadString( Section, 'FontName', 'Tahoma' );
  edtStrings.Font.Size := DelphiIni.ReadInteger( Section, 'FontSize', 8 );
  edtStrings.Font.Color := DelphiIni.ReadInteger( Section, 'FontColor', clWindowText );
  StyleBits := DelphiIni.ReadInteger( Section, 'FontStyle', fsNormal );
  edtStrings.Font.Style := [];
  if StyleBits and fsBoldMask = fsBoldMask then
    edtStrings.Font.Style := edtStrings.Font.Style + [ fsBold ];
  if StyleBits and fsItalicMask = fsItalicMask then
    edtStrings.Font.Style := edtStrings.Font.Style + [ fsItalic ];
  if StyleBits and fsUnderlineMask = fsUnderlineMask then
    edtStrings.Font.Style := edtStrings.Font.Style + [ fsUnderline ];
  if StyleBits and fsStrikeOutMask = fsStrikeOutMask then
    edtStrings.Font.Style := edtStrings.Font.Style + [ fsStrikeOut ];

  FTabSize := DelphiIni.ReadInteger( Section, 'TabSize', 8 );
  W := DelphiIni.ReadInteger( Section, 'Width', 420 );
  H := DelphiIni.ReadInteger( Section, 'Height', 320 );

  L := DelphiIni.ReadInteger( Section, 'Left', ( Monitor.Width - W ) div 2 );
  if L < Monitor.Left then
    L := 0
  else if L > ( Monitor.Width - W ) then
    L := Monitor.Width - W;

  T := DelphiIni.ReadInteger( Section, 'Top', ( Monitor.Height - H ) div 2 );
  if T < Monitor.Top - H  then
    T := 0
  else if T > ( Monitor.Height - H ) then
    T := Monitor.Height - H;
...
var
  StyleBits: Byte;
begin
  DelphiIni.WriteString( Section, 'FontName', edtStrings.Font.Name );
  DelphiIni.WriteInteger( Section, 'FontSize', edtStrings.Font.Size );
  DelphiIni.WriteInteger( Section, 'FontColor', edtStrings.Font.Color );

  StyleBits := 0;
  if fsBold in edtStrings.Font.Style then
    StyleBits := fsBoldMask;
  if fsItalic in edtStrings.Font.Style then
    StyleBits := StyleBits + fsItalicMask;
  if fsUnderline in edtStrings.Font.Style then
    StyleBits := StyleBits + fsUnderlineMask;
  if fsStrikeOut in edtStrings.Font.Style then
...
var
  ...
  P: Integer;
begin
  L := edtStrings.Perform( em_LineIndex, LineNum + 1, 0 ) - 2;
  if Integer( L ) < 0 then
  begin
    L := edtStrings.Perform( em_LineIndex, LineNum, 0 );
    P := edtStrings.Perform( em_LineLength, L, 0 );
    Result := L + P;
  end
  else
    Result := L;
...
var
  ...
  LineCount, P: Integer;
begin
  StartLine := edtStrings.Perform( em_LineFromChar, edtStrings.SelStart, 0 );
  StopLine := edtStrings.Perform( em_LineFromChar, edtStrings.SelStart + edtStrings.SelLength, 0 );

  edtStrings.SelStart := Perform( em_LineIndex, StartLine, 0 );
  P := EndOfLine( StopLine );
  edtStrings.SelLength := P - edtStrings.SelStart;

  OldSelStart := edtStrings.SelStart;
  OldSelLength := edtStrings.SelLength;
  ...
  edtStrings.SelStart := OldSelStart;                     { Re-establish text selection }
  if Indent then
    edtStrings.SelLength := OldSelLength + StopLine - StartLine
  else
    edtStrings.SelLength := OldSelLength - LineCount;
end;
