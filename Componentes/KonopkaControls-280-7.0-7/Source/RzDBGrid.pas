{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Added new UseDrawingStyle property to TRzDBGrid. When it is True (the
      ...
      LineColor. Plus, the customizations are also applied when VCL Styles
      are used.
  ------------------------------------------------------------------------------
  6.1.3  (01 May 2013)
...
procedure TRzDBGrid.SetUseDrawingStyle( Value: Boolean );
begin
...
var
  ...
  begin
    ...
    if FrameStyle = fsFlat then
      ...
      DrawBorderSides( Self, Canvas, R, FrameStyle, FrameSides );
  end;
...
var
  ...
  C, ColorLine, ColorFixed, ColorFixedBorder: TColor;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          {$ENDIF}
          begin
            if ( goFixedHotTrack in GridOptions ) then
          ...
          if not ( gdFocused in DrawState ) or
             not ( goEditing in GridOptions ) or
             not EditorMode or
          ...
          begin
            ...
            begin
              ...
              if ( gdSelected in DrawState ) and
                 ( not ( gdFocused in DrawState ) or
                   ( [ goDrawFocusSelected, goRowSelect ] * GridOptions <> [] ) ) then
              begin
                DrawCellHighlight( Where, DrawState, CurCol, CurRow );
              end
              else
              begin
                // TDBGrid.DrawCellBackground inflates the passed in TRect, which causes
                // the grid lines to ge overwritten. So we shrink the rectangle first,
                // then restore it when the method returns.
                InflateRect( Where, -1, -1 );
                DrawCellBackground( Where, Color, DrawState, CurCol, CurRow );
                InflateRect( Where, 1, 1 );
              end;
            end;
            ...
            if DefaultDrawing and not (csDesigning in ComponentState) and
               not (TStyleManager.IsCustomStyleActive and (goDrawFocusSelected in GridOptions)) and
               (gdFocused in DrawState) and
               ( [goEditing, goAlwaysShowEditor] * GridOptions <> [goEditing, goAlwaysShowEditor] )
               and not ( goRowSelect in GridOptions ) then
            begin
              TempRect := Where;
...
begin {= TRzDBGrid.Paint =}
  ...
  GridOptions := TCustomGridAccess( Self ).Options;

  Style := StyleServices;

  if ( FInternalDrawingStyle = gdsThemed ) then
  begin
    Style.GetElementColor( Style.GetElementDetails(tgCellNormal), ecBorderColor, ColorLine );
    if seClient in StyleElements then
      Style.GetElementColor( Style.GetElementDetails(tgCellNormal), ecFillColor, FInternalColor );
    Style.GetElementColor( Style.GetElementDetails(tgFixedCellNormal), ecBorderColor, ColorFixedBorder );
    Style.GetElementColor( Style.GetElementDetails(tgFixedCellNormal), ecFillColor, ColorFixed );
  end
  else
  begin
    ...
    begin
      ColorLine := $F0F0F0;
      ColorFixed := Color;
      ColorFixedBorder := GetShadowColor($F0F0F0, -45);

      if Style.Enabled then
      begin
        if Style.GetElementColor( Style.GetElementDetails(tgGradientCellNormal), ecBorderColor, C ) and (C <> clNone) then
          ColorLine := C;
        if Style.GetElementColor(Style.GetElementDetails(tgGradientCellNormal), ecFillColor, C) and (C <> clNone) then
          FInternalColor := C;
        if Style.GetElementColor(Style.GetElementDetails(tgGradientFixedCellNormal), ecBorderColor, C) and (C <> clNone) then
          ColorFixedBorder := C;
        if Style.GetElementColor(Style.GetElementDetails(tgGradientFixedCellNormal), ecFillColor, C) and (C <> clNone) then
          ColorFixed := C;
      end;
    end
    else
    begin
      ColorLine := clSilver;
      ColorFixed := FixedColor;
      ColorFixedBorder := clBlack;

      if Style.Enabled then
      begin
        if Style.GetElementColor(Style.GetElementDetails(tgClassicCellNormal), ecBorderColor, C) and (C <> clNone) then
          ColorLine := C;
        if Style.GetElementColor(Style.GetElementDetails(tgClassicCellNormal), ecFillColor, C) and (C <> clNone) then
          FInternalColor := C;
        if Style.GetElementColor(Style.GetElementDetails(tgClassicFixedCellNormal), ecBorderColor, C) and (C <> clNone) then
          ColorFixedBorder := C;
        if Style.GetElementColor(Style.GetElementDetails(tgClassicFixedCellNormal), ecFillColor, C) and (C <> clNone) then
          ColorFixed := C;
      end;
    end;
  end;
  ...
  begin
    ...
    if ColorToRGB( LineColor ) = ColorToRGB( Color ) then
      C := DarkerColor( LineColor, 50 );

    DrawLines( goFixedHorzLine in GridOptions, goFixedVertLine in GridOptions, 0, 0,
               [ 0, 0, DrawInfo.Horz.FixedBoundary, DrawInfo.Vert.FixedBoundary ],
               FFixedLineColor, FixedColor );

    DrawLines( goFixedHorzLine in GridOptions, goFixedVertLine in GridOptions, LeftCol, 0,
               [ DrawInfo.Horz.FixedBoundary, 0, DrawInfo.Horz.GridBoundary, DrawInfo.Vert.FixedBoundary],
               FFixedLineColor, FixedColor );

    DrawLines( goFixedHorzLine in GridOptions, goFixedVertLine in GridOptions, 0, TopRow,
               [ 0, DrawInfo.Vert.FixedBoundary, DrawInfo.Horz.FixedBoundary, DrawInfo.Vert.GridBoundary ],
               FFixedLineColor, FixedColor );

    DrawLines( goHorzLine in GridOptions, goVertLine in GridOptions, LeftCol, TopRow,
  ...
  FrameFlags1 := 0;
  FrameFlags2 := 0;
  if goFixedVertLine in GridOptions then
  begin
    FrameFlags1 := BF_RIGHT;
    FrameFlags2 := BF_LEFT;
  end;
  if goFixedHorzLine in GridOptions then
  begin
  ...
  DrawCells( LeftCol, TopRow, DrawInfo.Horz.FixedBoundary {- FColOffset},
             DrawInfo.Vert.FixedBoundary, DrawInfo.Horz.GridBoundary, DrawInfo.Vert.GridBoundary, FInternalColor, [] );

  if not ( csDesigning in ComponentState ) and ( goRowSelect in GridOptions ) and
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Canvas.Brush.Color := BlendColors( ActiveStyleSystemColor( Self, clHighlight ), ActiveStyleSystemColor( Self, clBtnFace ), 50 );
        end;
      end;
    end;
  end;
