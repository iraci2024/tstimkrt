{===============================================================================
  ...
  Copyright © 1995-2020 by Embarcadero Technologies, Inc.  All Rights Reserved.
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzEdit and descendants and modified
      components to honor StyleElements settings when VCL Styles are used.
    * Surfaced StyleName property in RAD Studio 10.4+.
    * TRzEditStyleHook now correctly unregistered from the style system.
  ------------------------------------------------------------------------------
  6.2.4  (15 Oct 2018)
    * Fixed issue in TRzDateTimeEdit where setting Date 1/1/0001 and then trying
      to display the popup calendar would result in an exception.
    * Moved EditRect processing from TRzCustomButtonEdit and TRzSpinEdit into
      the base TRzCustomEdit class in order to better handle font and other
      size changes. Added new virtual method GetEditRectMargin that allows each 
      descendant to customize how much space is removed from editing area to 
      accommodate for embedded buttons.
  ------------------------------------------------------------------------------
  6.2.1  (01 Sep 2015)
...
type
  ...
  protected
    ...
    function GetEditRectMargin: Integer; virtual;
...
begin
  if RunningAtLeast( winXP ) and ActiveStyleServices( Self ).Enabled and HandleAllocated then
  begin
...
function TRzCustomEdit.GetEditRectMargin: Integer;
begin
...
begin
  ...
  if ReadOnlyValue and HideButtonsOnReadOnly then
  begin
  ...
  begin
    ...
    begin
      SendMessage( Handle, em_SetMargins, ec_LeftMargin, 0 );
      SendMessage( Handle, em_SetMargins, ec_RightMargin, MakeLParam( 0, GetEditRectMargin ) );
    end
    else
    begin
      SendMessage( Handle, em_SetMargins, ec_LeftMargin, MakeLParam( GetEditRectMargin, 0 ) );
      SendMessage( Handle, em_SetMargins, ec_RightMargin, 0 );
    end;
  end;
end;
  ...
  try
    ...
    begin
      if FFrameHotTrack and ( FInControl or FOverControl ) then
        DrawFrame( Self, FCanvas, Width, Height, FFrameHotStyle, Color, FFrameHotColor, FFrameSides )
      else
        DrawFrame( Self, FCanvas, Width, Height, FFrameStyle, Color, FFrameColor, FFrameSides );
...
begin
  ...
  if FShowDropButton then
  begin
    PaintIt := FFlatButtons or FFrameHotTrack or ActiveStyleServices( Self ).Enabled;
...
procedure TRzCustomEdit.SetHideButtonsOnReadOnly( Value: Boolean );
begin
...
var
  ...
  begin
    if not Enabled then
      ElementDetails := ActiveStyleServices( Self ).GetElementDetails( tcDropDownButtonDisabled )
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DrawDropDownArrow( Self, Canvas, R, uiWindowsXP, False, Enabled, FScaleFactor );
      end
      else
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DrawDropDownArrow( Self, Canvas, R, uiWindows95, False, Enabled, FScaleFactor );
      end
      else
      begin
        ...
        DrawDropDownArrow( Self, Canvas, TempRect, uiWindows95, FButtonState = bsDown, Enabled, FScaleFactor );
      end;
...
begin
  if FMouseOverButton then
    Msg.HitTest := Winapi.Windows.HTCAPTION;  (*was HTNOWHERE*)
...
begin
  ...
  begin
    ...
    begin
      W := GetSystemMetrics( sm_CxVScroll );
      if ActiveStyleServices( Control ).Enabled then
        Dec( W );
...
var
  ...
  BtnWidth: Integer;
begin
  if ActiveStyleServices( Control ).Available then
  begin
    if MouseInControl then
      Details := ActiveStyleServices( Control ).GetElementDetails( teEditBorderNoScrollHot )
    else
      Details := ActiveStyleServices( Control ).GetElementDetails( teEditBorderNoScrollNormal );
    ...
    ActiveStyleServices( Control ).DrawElement( Canvas.Handle, Details, Rect( 0, 0, Control.Width, Control.Height ) );
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          FCanvas.Pen.Color := ActiveStyleSystemColor( Self, clBtnShadow );
          FCanvas.Brush.Color := ActiveStyleSystemColor( Self, clBtnShadow );
        end
        else
        begin
          FCanvas.Pen.Color := ActiveStyleSystemColor( Self, clBtnFace );
          FCanvas.Brush.Color := ActiveStyleSystemColor( Self, clBtnFace );
        end;
      ...
      begin
        DrawBox( FCanvas, R, ActiveStyleSystemColor( Self, clBtnShadow ) );
  ...
  try
    ...
    begin
      ...
      try
        ...
        if DT = 0 then
        begin
          Calendar.Date := System.SysUtils.Date;
        end
        else
        begin
...
begin
  Winapi.Windows.GetCaretPos( CaretPos );
