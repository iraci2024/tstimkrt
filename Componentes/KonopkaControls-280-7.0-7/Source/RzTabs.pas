{===============================================================================
  ...
  6.5.0  (28 Nov 2020)
    ...
    * Surfaced StyleElements property in TRzPageControl and TRzTabControl and
      modified the components to honor StyleElements settings when VCL Styles
      are used.
    * Surfaced StyleName property in RAD Studio 10.4+.
    * Fixed display issues in tabs that could occur with certain VCL Styles.
    * Fixed cannot focus hidden window issue when an exception is raised while
      changing tabs in the TRzPageControl.
  ------------------------------------------------------------------------------
  6.1.11 (08 Apr 2015)
  ...
  5.2    (05 Sep 2009)
    ...
    * Fixed issue in TRzPageControl and TRzTabControl where the default
...
type
  ...
  private
    ...
    FActiveTabCloseButtonAreaWidth: Integer;
    FActiveTabCloseButtonWidth: Integer;
    FActiveTabCloseButtonHeight: Integer;
  ...
  private
    ...
    function ChangeActivePage( Page: TRzTabSheet ): Boolean;
...
const
  cActiveTabCloseButtonAreaWidth = 20;
  cActiveTabCloseButtonWidth = 15;
  cActiveTabCloseButtonHeight = 13;
...
procedure DrawTabButtonBackground( Control: TControl; Canvas: TCanvas; Bounds: TRect;
...
begin
  if UsingSystemStyle( Control ) or not UseActiveStyleColor( TabControl ) then
    C := TabControl.ButtonColor
  else
    C := ActiveStyleColor( Control, scButtonNormal );
  ...
  begin
    ...
    begin
      Canvas.Pen.Color := FrameColor;
      Canvas.Brush.Color := ActiveStyleColor( Control, scButtonDisabled );
    end;
    Canvas.Rectangle( Bounds );
  end;
end;
...
var
  ...
  FC: TColor;
begin
  if UsingSystemStyle( FTabControl ) or not UseActiveStyleColor( FTabControl ) then
    FC := LighterColor( FFrameColor, 30 )
  else
    FC := ActiveStyleSystemColor( FTabControl, clBtnShadow );
  ...
  DrawTabButtonBackground( FTabControl, Canvas, R, FTabControl, FDown and ( FCurrent = sbDownLeft ),
                           FMouseOverButton and ( FOver = sbDownLeft ), FC, not Disabled );

  if UsingSystemStyle( FTabControl ) then
  begin
    if FTabControl.ButtonSymbolColor <> clNone then
      DrawSpinArrow( Canvas, R, FTabControl.ButtonSymbolColor, FTabControl.ButtonSymbolColorDisabled,
                     Direction, FDown and ( FCurrent = sbDownLeft ), False, not Disabled, FScaleFactor )
    else
      DrawSpinArrow( FTabControl, Canvas, R, uiWindows95, Direction, FDown and ( FCurrent = sbDownLeft ), False, not Disabled, FScaleFactor );
  end
  else
    DrawSpinArrow( FTabControl, Canvas, R, uiCustomVCLStyle, Direction, FDown and ( FCurrent = sbDownLeft ), False, not Disabled, FScaleFactor );
  ...
  DrawTabButtonBackground( FTabControl, Canvas, R, FTabControl, FDown and ( FCurrent = sbUpRight ),
  ...
  begin
    if FTabControl.ButtonSymbolColor <> clNone then
      DrawSpinArrow( Canvas, R, FTabControl.ButtonSymbolColor, FTabControl.ButtonSymbolColorDisabled,
                     Direction, FDown and ( FCurrent = sbUpRight ), False, not Disabled, FScaleFactor )
    else
      DrawSpinArrow( FTabControl, Canvas, R, uiWindows95, Direction, FDown and ( FCurrent = sbUpRight ), False, not Disabled, FScaleFactor );
  end
  else
    DrawSpinArrow( FTabControl, Canvas, R, uiCustomVCLStyle, Direction, FDown and ( FCurrent = sbUpRight ), False, not Disabled, FScaleFactor );
...
begin
  ...
  DrawTabButtonBackground( FTabControl, Canvas, R, FTabControl, FDown, FMouseOverButton, FC );

  if UsingSystemStyle( FTabControl ) then
  begin
    if FTabControl.ButtonSymbolColor <> clNone then
      DrawCloseX( Canvas, R, FTabControl.ButtonSymbolColor, FDown, True, FScaleFactor )
    else
      DrawCloseX( FTabControl, Canvas, R, uiWindows95, FDown, True, FScaleFactor );
  end
  else
    DrawCloseX( FTabControl, Canvas, R, uiCustomVclStyle, FDown, True, FScaleFactor );
...
begin
  ...
  UseSystemStyle := UsingSystemStyle( FTabControl ) or not UseActiveStyleColor( FTabControl );
  ...
  if FMouseOverButton then
  begin
    DrawTabButtonBackground( FTabControl, Canvas, R, FTabControl, FDown, True, FC );
    if UseSystemStyle then
      DrawCloseX ( Canvas, R, FTabControl.TabColors.HighlightBar, FDown, True, FScaleFactor )
    else
      DrawCloseX( Canvas, R, ActiveStyleFontColor( FTabControl, sfButtonTextHot ), FDown, True, FScaleFactor );
  end
  else
  begin
    if UseSystemStyle then
    begin
      if FTabControl.ButtonSymbolColor <> clNone then
        ...
        DrawCloseX( Canvas, R, FC, FDown, True, FScaleFactor );
    end
    else
      DrawCloseX( Canvas, R, ActiveStyleFontColor( FTabControl, sfButtonTextNormal ), FDown, True, FScaleFactor );
  end;
...
begin
  ...
  begin
    if FTabControl.ButtonSymbolColor <> clNone then
      ...
      DrawSpinArrow( FTabControl, Canvas, R, uiWindows95, Direction, FDown, True );
  end
  else
    DrawSpinArrow( FTabControl, Canvas, R, uiCustomVCLStyle, Direction, FDown, True );
end;
...
begin
  ...
  FActiveTabCloseButtonAreaWidth := cActiveTabCloseButtonAreaWidth;
  FActiveTabCloseButtonWidth := cActiveTabCloseButtonWidth;
  FActiveTabCloseButtonHeight := cActiveTabCloseButtonHeight;
...
procedure TRzCustomTabControl.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FActiveTabCloseButtonAreaWidth := MulDiv(FActiveTabCloseButtonAreaWidth, M, D);
  FActiveTabCloseButtonWidth := MulDiv(FActiveTabCloseButtonWidth, M, D);
  FActiveTabCloseButtonHeight := MulDiv(FActiveTabCloseButtonHeight, M, D);
  inherited;
end;
...
begin
  ...
  // TResourceManager.FreeResource in Vcl.Graphics, but don't know why?)
  ...
  begin
    ...
    begin
      BrushHandle := CreateSolidBrush( ColorToRGB( ActiveStyleSystemColor( Self, FBackgroundColor ) ) );
...
begin
  ...
  if FTextOrientation = orVertical then
    Inc( Result, ScaleValue(2) )
  else
    Inc( Result, ScaleValue(4) );
      ...
      begin
        ...
        begin
          if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
            ...
            BrushColor := ActiveStyleSystemColor( Self, clBtnFace );
        end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        BrushColor := ActiveStyleSystemColor( Self, clBtnFace );
        UnselColor := ActiveStyleColor( Self, scButtonDisabled );
        ShadowColor := DarkerColor( ActiveStyleColor( Self, scButtonDisabled ), 40 );
      end;
    end;
    ...
    begin
      ...
      begin
        UnselColor := ActiveStyleColor( Self, scButtonHot );
        ShadowColor := DarkerColor( ActiveStyleColor( Self, scButtonHot ), 20 );
      end;
    end;
...
//!! This method needs to be rewritten or removed because it messes up BiDi RTL drawing. The problem is that the DoTextOut
// method breaks up the string into segments around the && but this in turns causes problems for displaying text RTL
...
begin
  ...
  begin
    ...
    SetBkMode( ACanvas.Handle, Winapi.Windows.TRANSPARENT );
...
var
    ...
    begin
      ...
      begin
        case FTabOrientation of
          ...
          begin
            if FTabSequence = tsStandard then
              Dec( Result.Right, FActiveTabCloseButtonAreaWidth )
            else { FTabSequence = tsReverse }
              Inc( Result.Left, FActiveTabCloseButtonAreaWidth );
          end;

          toLeft:
          begin
            if FTabSequence = tsStandard then
              Inc( Result.Top, FActiveTabCloseButtonAreaWidth )
            else { FTabSquence = tsReverse }
              Dec( Result.Bottom, FActiveTabCloseButtonAreaWidth );
          end;
    ...
    case FImagePosition of
      ...
      begin
        ...
        begin
          ...
          begin
            OffsetRect( Result, 0, FActiveTabCloseButtonAreaWidth );
          end;
        end;
      ...
      begin
        ...
        begin
          OffsetRect( Result, FActiveTabCloseButtonAreaWidth, 0 );
        end;
  ...
  begin
    ...
    begin
      ...
      L := 0;
      T := 0;
      LWidth := Self.FActiveTabCloseButtonWidth;
      LHeight := Self.FActiveTabCloseButtonHeight;
      case FTabOrientation of
        toTop, toBottom:
        begin
          ...
          if LHeight > H - 2 then
            LHeight := H - 2;
          T := Result.Top + ( H div 2 ) - LHeight div 2;
        end;
        ...
        begin
          ...
          if LWidth > W - 2 then
            LWidth := W - 2;
          L := Result.Left + ( W div 2 ) - LWidth div 2;
        end;
      end;
      ...
      case FTabOrientation of
        ...
        begin
          if FTabSequence = tsStandard then
            Result := Rect( Result.Right - LWidth, T, Result.Right, T + LHeight)
          else { FTabSquence = tsReverse }
            Result := Rect( Result.Left, T, Result.Left + LWidth, T + LHeight );
        end;

        toLeft:
        begin
          if FTabSequence = tsStandard then
            Result := Rect( L, Result.Top, L + LWidth, Result.Top + LHeight )
          else { FTabSquence = tsReverse }
           Result := Rect( L, Result.Bottom - LHeight, L + LWidth, Result.Bottom )
        end;
  ...
  begin
    ...
    begin
      case TextColorElement of
        ttcSelected:
          Result := ActiveStyleFontColor( Self, sfTabTextActiveNormal );
        ttcUnselected:
          Result := ActiveStyleFontColor( Self, sfTabTextInactiveNormal );
        ttcDisabled:
          Result := ActiveStyleFontColor( Self, sfTabTextInactiveDisabled );
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if FHotTrackStyle = htsText then
            TextColor := ActiveStyleFontColor( Self, sfTabTextActiveHot )
          else
            TextColor := ActiveStyleFontColor( Self, sfTabTextActiveNormal );
        end;
      end;
...
begin
  ...
  if not UsingSystemStyle( Self ) then
    FrameColor := ActiveStyleSystemColor( Self, clBtnShadow );
  ...
  begin
    ...
    case Cmd of
      ...
      begin
        ...
        if not UsingSystemStyle( Self ) then
          ItemColor := ActiveStyleSystemColor( Self, clBtnShadow );
...
begin
  ...
  case FTabStyle of
    ...
    begin
      if FTabOrientation in [ toTop, toBottom ] then
      begin
        Result.cX := TabFaceExtent.cX + ( GetTabOffset( TabFaceExtent.cY + ScaleValue(4) ) * 2 ) +
          ScaleValue(4) + FTabHeight div 2;
        Result.cY := TabFaceExtent.cY + ScaleValue(4);
      end
      else
      begin
        Result.cX := TabFaceExtent.cX + ScaleValue(4);
        Result.cY := TabFaceExtent.cY + ( GetTabOffset( TabFaceExtent.cX + ScaleValue(4) ) * 2 ) +
          ScaleValue(4) + FTabHeight div 2;
      end;
    end;
    ...
    begin
      if FTabOrientation in [ toTop, toBottom ] then
      begin
        Result.cX := TabFaceExtent.cX + ( GetTabOffset( TabFaceExtent.cY ) * 2 ) +
          ScaleValue(4) + FTabHeight;
        Result.cY := TabFaceExtent.cY + ScaleValue(4);
      end
      else
      begin
        Result.cX := TabFaceExtent.cX + ScaleValue(4);
        Result.cY := TabFaceExtent.cY + ( GetTabOffset( TabFaceExtent.cX ) * 2 ) +
          ScaleValue(4) + FTabHeight;
      end;
    end;
    ...
    begin
      ...
      begin
        if FTextOrientation = orVertical then
        begin
          Result.cX := TabFaceExtent.cX + ScaleValue(8);
          Result.cY := TabFaceExtent.cY + FCutCornerSize + ScaleValue(5);
        end
        else
        begin
          Result.cX := TabFaceExtent.cX + ( FCutCornerSize * 2 ) + ScaleValue(6);
          Result.cY := TabFaceExtent.cY + ScaleValue(5);
        end
      end
      else                                  // toLeft, toRight
      begin
        if FTextOrientation = orVertical then
        begin
          Result.cX := TabFaceExtent.cX + ScaleValue(5);
          Result.cY := TabFaceExtent.cY + ( FCutCornerSize * 2 ) + ScaleValue(6);
        end
        else
        begin
          Result.cX := TabFaceExtent.cX + FCutCornerSize + ScaleValue(5);
          Result.cY := TabFaceExtent.cY + ScaleValue(8);
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      if ( FTextOrientation = orHorizontal ) and ( FTabOrientation in [ toTop, toBottom ] ) then
        Inc( Result.cX, FActiveTabCloseButtonAreaWidth )
      else if ( FTextOrientation = orVertical ) and ( FTabOrientation in [ toLeft, toRight ] ) then
        Inc( Result.cY, FActiveTabCloseButtonAreaWidth );
    end;
...
//!! Adjust to scroll tabs to fill remaining space
...
begin
  ...
  begin
    System.Classes.RegisterClasses( [ TRzTabSheet ] );
    Registered := True;
  end;
...
(*
function TRzPageControl.ChangeActivePage( Page: TRzTabSheet ): boolean; // Atys
var
  Form: TCustomForm;
begin
  Result := True; // Atys
  ...
  begin
    ...
    begin // Atys
      ...
      if Form.ActiveControl <> FActivePage then // Atys
        Exit(False); // Atys
   end; // Atys*)
function TRzPageControl.ChangeActivePage( Page: TRzTabSheet ): Boolean;
var
  Form: TCustomForm;
begin
  Result := True;
  ...
  begin
    ...
    begin
      Form.ActiveControl := FActivePage;
      if Form.ActiveControl <> FActivePage then
        Exit( False );
    end;
...
begin
  ...
  begin
    ...
    Allowed := ChangeActivePage( PageForTab( NewIndex ) );
  end;
end;
...
begin
  ...
  begin
    if UsingSystemStyle( Self ) or not UseActiveStyleColor( Self ) then
      ...
      ACanvas.Brush.Color := ActiveStyleSystemColor( Self, clBtnFace );
