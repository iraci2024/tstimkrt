...
begin
  ...
  R := Rect;
  if not ActiveStyleServices( Self ).Enabled then
  begin
...
begin
  ...
  begin
    ...
    begin
      DrawArrow( HeaderCanvas, Rect, ActiveStyleFontColor( Self, sfHeaderSectionTextNormal ), FHeaderSortDirection = sdAscending );
    end;
  end;
end;
    ...
    begin
      ...
      begin
        if FHeaderSectionDown then
          C := ActiveStyleFontColor( Self, sfHeaderSectionTextPressed )
        else
          C := ActiveStyleFontColor( Self, sfHeaderSectionTextHot );
      end
      else
        C := ActiveStyleFontColor( Self, sfHeaderSectionTextNormal );
...
begin {= TRzCustomListView.DefaultDrawHeader =}
  ...
  begin
    ...
    begin
      if FHeaderSectionDown then
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( thHeaderItemPressed )
      else
        ElementDetails := ActiveStyleServices( Self ).GetElementDetails( thHeaderItemHot );
    end
    else
      ElementDetails := ActiveStyleServices( Self ).GetElementDetails( thHeaderItemNormal );

    ActiveStyleServices( Self ).DrawElement( HeaderCanvas.Handle, ElementDetails, Rect );
  end
  else
  begin
    R := Rect;
    ...
    if ( FOverHeaderSection = Index ) and ( Index <> -1 ) and FHeaderSectionDown then
      DrawButtonBorder( Self, HeaderCanvas, R, True )
    else
      DrawButtonBorder( Self, HeaderCanvas, R, False );
  end;
  ...
  try
    case Msg.Msg of
      ...
      begin
        if csDesigning in ComponentState then
        begin
          Msg.Result := Winapi.Windows.HTTRANSPARENT;
          Exit;
        end;
      end;
      ...
      begin
        ...
        begin
          ...
          if ( FOverHeaderSection <> LastSection ) and ActiveStyleServices( Self ).Enabled then
          begin
        ...
        if ActiveStyleServices( Self ).Enabled and not FMouseInNonClient then
        begin
