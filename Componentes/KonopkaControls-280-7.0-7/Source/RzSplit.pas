...
begin
  ...
  begin
    System.Classes.RegisterClasses( [ TRzSplitterPane ] );
    Registered := True;
  end;
...
procedure TRzSplitter.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FSplitterWidth := MulDiv(FSplitterWidth, M, D);
...
begin
  ...
  if UsingSystemStyle( Self ) then
    ...
    Color := ActiveStyleColor( Self, scPanel );
...
begin
  ...
  begin
    HSColor := ActiveStyleColor( Self, scPanel );
    HSDotColor := ActiveStyleSystemColor( Self, clHighlight );
    HSFrameColor := ActiveStyleSystemColor( Self, clBtnShadow );
...
begin
  ...
  begin
    ...
    begin
      DrawBorder( Self, Canvas, R1, fsPopup );
      DrawBorder( Self, Canvas, R2, fsPopup );
    end
    else
    begin
      DrawBorder( Self, Canvas, R1, fsStatus );
      DrawBorder( Self, Canvas, R2, fsStatus );
    end;
  end
  else if ( VisualStyle <> vsClassic ) and ( FSplitterStyle = ssGroupBar ) then
  begin
    DrawGroupBarBackground( Self, Canvas, FBarRect, VisualStyle,
...
procedure TRzCustomSizePanel.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FSizeBarWidth := MulDiv(FSizeBarWidth, M, D);
  inherited;
end;
...
begin
  ...
  if UsingSystemStyle( Self ) then
    ...
    FillColor := ActiveStyleColor( Self, scPanel );
  ...
  begin
    DrawGroupBarBackground( Self, Canvas, BarRect, VisualStyle,
                            GradientColorStyle, FGradientPath,
                            GradientColorStart, GradientColorStop );
  end;
