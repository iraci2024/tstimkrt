...
{$IFNDEF SBRIDGE}
  ...
  EntityDAC.Common.Vio, EntityDAC.Common.VioTcp;
  {$ENDIF}
{$ELSE}
  ScCLRClasses, ScTypes, ScFunctions, ScVio, ScVioTcp,
  ScUtils, ScConsts, ScBridge, ScSSLTypes, ScSSLClient, ScSSLExtensions;
{$ENDIF}
...
type
  ...
  {$ENDIF}
    ...
    property SSLOptions: {$IFDEF SBRIDGE}TScSSLClientOptions{$ELSE}TSSLOptions{$ENDIF} read FSSLOptions;
  end;
  ...
  {$IFDEF SBRIDGE}
    ...
    procedure CreateSSLClient(ConnectionParameters: TScSecureConnectionParameters);
  ...
  {$IFDEF SBRIDGE}
    procedure AssignSession(SourceConnection: TScSecureConnection); overload;
  ...
  {$ENDIF}
    ...
    procedure BeginWriteBuffering;
    procedure EndWriteBuffering;
...
begin
  ...
  FSSLOptions := {$IFDEF SBRIDGE}TScSSLClientOptions.Create(nil){$ELSE}TSSLOptions.Create{$ENDIF};
end;
...
begin
  ...
  {$IFDEF SBRIDGE}
    if FSSLClient <> nil then
      FSSLClient.OnAsyncReceive := FOnAsyncReceive
    else
  {$ENDIF}
...
begin
  if FIsSecure then
  {$IFDEF SBRIDGE}
    Result := (FSSLClient <> nil) and FSSLClient.IsSecure
  {$ELSE}
...
begin
  ...
  {$IFDEF SBRIDGE}
    Assert(FSSLClient <> nil);
    FSSLClient.IsSecure := Value;
  {$ELSE}
    Assert(FVio <> nil);
...
{$IFDEF SBRIDGE}
procedure TScSecureConnection.AssignSession(SourceConnection: TScSecureConnection);
var
  SrcSessionTicketExtension: TTLSHelloExtension;
begin
  ...
  if SourceConnection.FSSLClient = nil then
    raise EScError.Create(seSourceConnectionNotSecure);

  if SourceConnection.FSSLClient.SessionInfo = nil then
    Exit;

  if SourceConnection.FSSLClient.SessionInfo.Protocol = spTls13 then begin
    if SourceConnection.FSSLClient.SessionInfo.NewSessionTickets.Count = 0 then
      FSSLClient.AssignSession(SourceConnection.FSSLClient.SessionInfo);
  end
  else begin
    SrcSessionTicketExtension := SourceConnection.FSSLClient.ServerHelloExtensions.Find(TTLSSessionTicketExtension);
    if (SrcSessionTicketExtension <> nil) or (Length(SourceConnection.FSSLClient.SessionInfo.SessionID) > 0) then
      FSSLClient.AssignSession(SourceConnection.FSSLClient.SessionInfo);
  end;
end;

procedure TScSecureConnection.AssignSession(SourceSessionInfo: TScSSLSessionInfo);
begin
  ...
  FSSLClient.AssignSession(SourceSessionInfo);
end;

procedure TScSecureConnection.CreateSSLClient(ConnectionParameters: TScSecureConnectionParameters);
begin
  if FSSLClient <> nil then
    raise Exception.Create(SConnectionActive);

  FSSLClient := TScSSLClient.Create(nil);
  FSSLClient.HostName := ConnectionParameters.Hostname;
  FSSLClient.Port := ConnectionParameters.Port;
  FSSLClient.IPVersion := ConnectionParameters.IPVersion;
  FSSLClient.EventsCallMode := ecDirectly;
  FSSLClient.DisableCloseSocketOnShutdownAlert := FDisableCloseSocketOnShutdownAlert;
  FSSLClient.ProxyOptions.Assign(ConnectionParameters.ProxyOptions);
  FSSLClient.AssignOptions(ConnectionParameters.SSLOptions);
end;

function TScSecureConnection.GetSessionInfo: TScSSLSessionInfo;
begin
  if FSSLClient <> nil then
    Result := FSSLClient.SessionInfo
  else
    Result := nil;
end;
{$ELSE}
...
begin
  ...
  try
    ...
    if FIsSecure then begin
    {$IFDEF SBRIDGE}
      CreateSSLClient(ConnectionParameters);
    {$ELSE}
...
begin
  ...
  try
    ...
    {$IFDEF SBRIDGE}
      CreateSSLClient(ConnectionParameters);
      TScSSLClientUtils.SetVio(FSSLClient, AVio);
    {$ELSE}
      CreateSSLIOHandler(ConnectionParameters);
    {$ENDIF}
    end
...
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then
    FSSLClient.BindAddress := BindAddress
  else
{$ENDIF}
...
{$IFDEF SBRIDGE}
  if FIsSecure then
    Result := not FIsDisconnected and (FSSLClient <> nil)
  else
{$ENDIF}
...
{$IFDEF SBRIDGE}
  if FIsSecure then
    FIsConnected := not FIsDisconnected and (FSSLClient <> nil) and FSSLClient.Connected
  else
{$ENDIF}
...
function TScSecureConnection.DirectWrite(const Buffer: TValueArr; Offset, Count: integer): integer;
begin
{$IFDEF SBRIDGE}
  if FIsSecure then begin
    Assert(FSSLClient <> nil);
    Result := FSSLClient.WriteBuffer(Buffer[Offset], Count);
  end
  else
{$ENDIF}
  begin
...
procedure TScSecureConnection.BeginWriteBuffering;
begin
  if FWriteBuffer = nil then
    SetLength(FWriteBuffer, SEND_BLOCK_SIZE);

  FIsWriteBuffering := True;
end;

procedure TScSecureConnection.EndWriteBuffering;
begin
  FIsWriteBuffering := False;
  if FWritePos > 0 then begin
    DirectWrite(TValueArr(FWriteBuffer), 0, FWritePos);
    FWritePos := 0;
  end;
end;
...
var
  WriteCount: integer;
begin
  if FIsWriteBuffering then begin
    Result := Count;

    if (FWritePos = 0) and (Count >= SEND_BLOCK_SIZE) then begin
      DirectWrite(Buffer, Offset, Count);
      Exit;
    end;

    if (FWritePos + Count) > SEND_BLOCK_SIZE then
      WriteCount := SEND_BLOCK_SIZE - FWritePos
    else
      WriteCount := Count;

    Move(Buffer[Offset], FWriteBuffer[FWritePos], WriteCount);
    Inc(FWritePos, WriteCount);
    Inc(Offset, WriteCount);
    Dec(Count, WriteCount);

    if FWritePos >= SEND_BLOCK_SIZE then begin
      DirectWrite(TValueArr(FWriteBuffer), 0, FWritePos);
      FWritePos := 0;

      if Count >= (SEND_BLOCK_SIZE - UNBUFFERED_READ_MIN_SIZE) then
        DirectWrite(Buffer, Offset, Count)
      else
      if Count > 0 then begin
        Move(Buffer[Offset], FWriteBuffer[FWritePos], Count);
        Inc(FWritePos, Count);
      end;
    end;
  end
...
begin
  Write(TValueArr(Buffer), 0, Length(Buffer));
end;
...
{$IFDEF SBRIDGE}
  if FIsSecure then begin
    Assert(FSSLClient <> nil);
    Result := FSSLClient.ReadNoWait(Buffer[Offset], Count);
  end
  else
{$ENDIF}
  begin
...
begin
  if FReadEnd > FReadStart then
    Result := True
  else
{$IFDEF SBRIDGE}
  if FIsSecure then
    Result := (FSSLClient <> nil) and FSSLClient.WaitForData(MillisecondsTimeout)
  else
{$ENDIF}
...
begin
  if FReadEnd = FReadStart then begin
    FReadStart := 0;
    FReadEnd := 0;
  end
  else
  if FReadStart >= READ_BUFFER_SIZE then begin // performance increase
    Move(FReadBuffer[FReadStart], FReadBuffer[0], FReadEnd - FReadStart);
    Dec(FReadEnd, FReadStart);
    FReadStart := 0;
  end;
end;
...
begin
  ...
  repeat
    for i := TmpPos to FReadEnd - 1 do begin
    ...
    if FindPos < 0 then begin
      if FReadEnd >= Length(FReadBuffer) then
        SetLength(FReadBuffer, Length(FReadBuffer) + READ_BUFFER_SIZE);

      ReadCount := ReadNoWait(@FReadBuffer[0], FReadEnd, Length(FReadBuffer) - FReadEnd);
      if ReadCount > 0 then
        Inc(FReadEnd, ReadCount)
      else
        RaiseLastError;
  ...
  Count := FindPos - FReadStart + 1;

  if AEncoding = nil then
    AEncoding := Encoding.Default;
  Result := AEncoding.GetString(FReadBuffer, FReadStart, Count - 2{CRLF});

  Inc(FReadStart, Count);
  DefragBuffer;
end;
...
var
  ...
  ReadCount: integer;
begin
  if (NeedCount > 0) and ((FReadEnd - FReadStart) < NeedCount) then begin
    if (FReadEnd + NeedCount) > Length(FReadBuffer) then
      SetLength(FReadBuffer, Length(FReadBuffer) + READ_BUFFER_SIZE);
...
begin
  if Count > 0 then begin
    ...
    if Buffer <> nil then
      Move(FReadBuffer[FReadStart], Buffer[Offset], Result);
    Inc(FReadStart, Result);
    DefragBuffer;
  end
  else
    Result := 0;
...
begin
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then begin
    if FSSLClient.GetLastException <> nil then
      raise CloneException(FSSLClient.GetLastException);
  end
  else
{$ENDIF}
  if FVio <> nil then begin
...
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then
    FSSLClient.SetSocketOption(OptionLevel, OptionName, OptionValue)
  else
{$ENDIF}
...
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then
    Result := FSSLClient.GetLocalPort
  else
{$ENDIF}
  if (FVio <> nil) and (FVio is TCRVioTcp) then
...
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then
    Result := FSSLClient.GetRemotePort
  else
{$ENDIF}
  if (FVio <> nil) and (FVio is TCRVioTcp) then
...
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then
    Result := FSSLClient.AfterDisconnect
  else
{$ENDIF}
  if FVio <> nil then
    Result := FVio.OnClose
...
{$IFDEF SBRIDGE}
  if FSSLClient <> nil then
    FSSLClient.AfterDisconnect := Value
  else
{$ENDIF}
  if FVio <> nil then
    FVio.OnClose := Value;
...
{$IFDEF SBRIDGE}
procedure TScSecureConnection.SetDisableCloseSocketOnShutdownAlert(Value: boolean);
begin
  if Value <> FDisableCloseSocketOnShutdownAlert then begin
    FDisableCloseSocketOnShutdownAlert := Value;
    if FSSLClient <> nil then
      FSSLClient.DisableCloseSocketOnShutdownAlert := Value;
  end;
end;
{$ENDIF}
