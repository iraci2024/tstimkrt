...
{$IFNDEF SBRIDGE}
  ...
  {$ELSE}
  EntityDAC.Common.DacClasses, EntityDAC.Common.Types, EntityDAC.Common.Functions,
  EntityDAC.Common.Base64, EntityDAC.Common.Vio, EntityDAC.Common.SecureConnection;
  {$ENDIF}
{$ELSE}
  ScCLRClasses, ScTypes, ScFunctions, ScBase64, ScVio,
  ScSecureConnection,
  ScSSLTypes, ScBridge, ScSSLClient;
{$ENDIF}
...
type
  ...
  TScHttpStatusCode = (
    ...
    scGone, scHttpVersionNotSupported, scInternalServerError, scLengthRequired, scLocked,
  ...
  {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF} = class;
  {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF} = class;
  ...
  private
    ...
    FOwner: {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF};
  protected
    procedure CheckNewValue(const Key, Value: string); override;
  public
    constructor Create(Owner: {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF});
  ...
  private
    {$IFDEF AUTOREFCOUNT}[weak]{$ENDIF}
    FOwner: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF};
  public
    constructor Create(Owner: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF});
...
{$ENDIF}
  TScHttpWebRequest = class(TComponent, ISequentialStream)
{$ELSE}
...
{$ENDIF}
  private
    ...
    FSSLOptions: {$IFDEF SBRIDGE}TScSSLClientOptions{$ELSE}TSSLOptions{$ENDIF};
  {$IFNDEF SBRIDGE}
  ...
  {$ENDIF}
    ...
    procedure SetSSLOptions(Value: {$IFDEF SBRIDGE}TScSSLClientOptions{$ELSE}TSSLOptions{$ENDIF});
  ...
  {$ENDIF}
    constructor Create(const URL: string); {$IFDEF SBRIDGE}reintroduce; overload;{$ENDIF}
    destructor Destroy; override;

  {$IFDEF SBRIDGE}
    procedure AssignSecureSession(Source: TScHttpWebRequest);
  {$ENDIF}

    procedure Abort;
    procedure Disconnect;
    function GetResponse(CancellationToken: TScCancellationToken = nil): {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF};
  ...
  published
    ...
    property SSLOptions: {$IFDEF SBRIDGE}TScSSLClientOptions{$ELSE}TSSLOptions{$ENDIF} read FSSLOptions write SetSSLOptions;
  end;

  {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF} = class(TInterfacedPersistent, ISequentialStream)
  private
    FCookies: TStringList;
    FHeaders: TScWebHeaderCollection;
    FMethod: TScRequestMethod;
  ...
  TScHttpWebResponseHelper = class
    class function GetSecureConnection(Obj: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}): TScSecureConnection;
    class procedure SetSecureConnection(Obj: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}; Value: TScSecureConnection);
  end;
...
resourcestring
  ...
  SBinaryTransferEncodingAndSendChunked = 'TransferEncoding can not be ''binary'' when SendChunked is set';
...
const
  ...
  HTTP_STATUS_CODES: array[TScHttpStatusCode] of integer = (
    ...
    411{scLengthRequired}, 423{scLocked},
    405{scMethodNotAllowed}, 301{scMoved}, 301{scMovedPermanently},
...
constructor TScWebRequestHeaderCollection.Create(Owner: {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF});
begin
  inherited Create;
...
constructor TScWebResponseHeaderCollection.Create(Owner: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF});
begin
  inherited Create;
...
{$IFDEF SBRIDGE}
constructor TScHttpWebRequest.Create(AOwner: TComponent);
begin
...
constructor {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Create(const URL: string);
begin
  inherited Create{$IFDEF SBRIDGE}(nil){$ENDIF};
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Init;
begin
  ...
  FSSLOptions := {$IFDEF SBRIDGE}TScSSLClientOptions.Create(Self){$ELSE}TSSLOptions.Create{$ENDIF};
...
destructor {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Destroy;
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.CheckInactive;
begin
  if FSecureConnection <> nil then
    raise HttpException.CreateRes(@SRequestExecuted);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.DefineProperties(Filer: TFiler);
begin
  inherited;
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.ReadHeadersText(Reader: TReader);
begin
  FHeaders.Text := Reader.ReadString();
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.WriteHeadersText(Writer: TWriter);
begin
  Writer.WriteString(FHeaders.Text);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.WriteChunk(
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.WriteChunkSize(Size: integer);
begin
  FSecureConnection.WriteLine(IntToHex(Size, 1));
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.GetNextChunkData(
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.InternalWriteData(
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.CheckRequest;
begin
  ...
  if FSendChunked then begin
    ...
    if TransferEncoding = SChunked then
      raise HttpException.Create(SSendChunkedAndTransferEncoding);
  end;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SendRequest(
...
{$IFDEF SBRIDGE}
procedure TScHttpWebRequest.AssignSecureSession(Source: TScHttpWebRequest);
begin
  if (Source.FSecureConnection = nil) or (Source.FSecureConnection.SessionInfo = nil) then
    raise HttpException.Create(SRequestNotExecuted);

  if FAssignedSessionInfo = nil then
    FAssignedSessionInfo := TScSSLSessionInfo.Create;

  FAssignedSessionInfo.Assign(Source.FSecureConnection.SessionInfo);
end;
{$ENDIF}

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.DoBeforeSendRequest;
begin
  if Assigned(FBeforeSendRequest) then
    FBeforeSendRequest(Self);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.DoAfterSendRequest;
begin
  if Assigned(FAfterSendRequest) then
    FAfterSendRequest(Self);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.DoAfterRetrieveHeaders;
begin
  if Assigned(FAfterRetrieveHeaders) then
    FAfterRetrieveHeaders(Self);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Abort;
begin
  if FSecureConnection <> nil then
    FSecureConnection.Abort;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Disconnect;
begin
  if FSecureConnection <> nil then begin
    FSecureConnection.Disconnect;
    FSecureConnection.Release;
...
{$IFNDEF FPC}
{$IFDEF VER22P}
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Read(pv: IntPtr; cb: FixedUInt; pcbRead: PFixedUInt): HResult;
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Read(pv: IntPtr; cb: Longint; pcbRead: PLongint): HResult;
{$ENDIF}
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Read(pv: Pointer; cb: DWord; pcbRead: PDWord): HRESULT;
{$ENDIF}
begin
...
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.ReadBuffer: TBytes;
var
  cnt: integer;
begin
...
{$IFNDEF FPC}
{$IFDEF VER22P}
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Write(pv: IntPtr; cb: FixedUInt; pcbWritten: PFixedUInt): HResult;
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Write(pv: IntPtr; cb: Longint; pcbWritten: PLongint): HResult;
{$ENDIF}
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.Write(pv: Pointer; cb: DWord; pcbWritten: PDWord): HRESULT;
{$ENDIF}
var
  OldLen: integer;
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.WriteBuffer(const Buffer: TValueArr; Offset, Count: integer);
var
  OldLen: integer;
begin
  if FSendChunked then
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.WriteBuffer(const Data: TBytes);
begin
  WriteBuffer(TValueArr(Data), 0, Length(Data));
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.WriteData(Stream: TStream);
var
  Count: integer;
begin
  if FSendChunked then
...
class function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.DefaultPort(const Scheme: string): string;
var
  i: integer;
begin
  Result := PortDefault;
...
function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.GetStatusCode: TScHttpStatusCode;
begin
  Result := ConvertToStatusCode(FStatusCode);
end;

function {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.GetIsSecure: boolean;
begin
  if FSecureConnection <> nil then
...
{$IFDEF SBRIDGE}
procedure TScHttpWebRequest.Notification(Component: TComponent; Operation: TOperation);
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetAccept(const Value: string);
begin
  CheckInactive;
  FAccept := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetCachePolicy(Value: TScRequestCachePolicy);
begin
  CheckInactive;
  FCachePolicy.Assign(Value);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetConnection(const Value: string);
begin
  CheckInactive;
  FConnection := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetConnectionGroupName(const Value: string);
begin
  CheckInactive;
  FConnectionGroupName := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetContentLength(const Value: Int64);
begin
  CheckInactive;
  FContentLength := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetContentType(const Value: string);
begin
  CheckInactive;
  FContentType := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetCookies(Value: TStringList);
begin
  CheckInactive;
  FCookies.Assign(Value);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetCredentials(Value: TScNetworkCredential);
begin
  CheckInactive;
  FCredentials.Assign(Value);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetDate(const Value: TDateTime);
begin
  CheckInactive;
  FDate := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetExpect(const Value: string);
begin
  CheckInactive;
  FExpect := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetFrom(const Value: string);
begin
  CheckInactive;
  FFrom := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetHeaders(Value: TScWebHeaderCollection);
begin
  CheckInactive;
  FHeaders.Assign(Value);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetHost(const Value: string);
begin
  CheckInactive;
  FHost := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetIfModifiedSince(const Value: TDateTime);
begin
  CheckInactive;
  FIfModifiedSince := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.CheckKeepAliveHeader;
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetKeepAlive(const Value: boolean);
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetMethod(const Value: TScRequestMethod);
begin
  CheckInactive;
  FMethod := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetProtocolVersion(Value: TScVersion);
begin
  CheckInactive;
  FProtocolVersion.Assign(Value);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetProxy(Value: TScWebProxy);
begin
  CheckInactive;
  FProxy.Assign(Value);
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetRange(const Value: string);
begin
  CheckInactive;
  FRange := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetReadWriteTimeout(const Value: integer);
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetReferer(const Value: string);
begin
  CheckInactive;
  FReferer := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetRequestUri(const Value: string);
begin
  CheckInactive;
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetTransferEncoding(const Value: string);
begin
  CheckInactive;

  if FSendChunked and (Value = 'binary') then
    raise ArgumentException.Create(SBinaryTransferEncodingAndSendChunked);

  FTransferEncoding := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetUpgrade(const Value: string);
begin
  CheckInactive;
  FUpgrade := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetUserAgent(const Value: string);
begin
  CheckInactive;
  FUserAgent := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetIPVersion(const Value: TIPVersion);
begin
  CheckInactive;
  FIPVersion := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetMaximumAutomaticRedirections(const Value: integer);
begin
  CheckInactive;
  FMaximumAutomaticRedirections := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetMaximumAutomaticReconnections(const Value: integer);
begin
  CheckInactive;
  FMaximumAutomaticReconnections := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetSendChunked(const Value: boolean);
begin
  CheckInactive;
  FSendChunked := Value;

  if FSendChunked and (FTransferEncoding = 'binary') then
    FTransferEncoding := SChunked;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetSendBlockSize(const Value: integer);
begin
  CheckInactive;
...
{$IFDEF SBRIDGE}
procedure TScHttpWebRequest.SetSSLOptions(Value: TScSSLClientOptions);
{$ELSE}
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetRequestStream(Value: TStream);
begin
  CheckInactive;
  FRequestStream := Value;
end;

procedure {$IFNDEF SBRIDGE}TCRHttpWebRequest{$ELSE}TScHttpWebRequest{$ENDIF}.SetAllowedStatuses(const Value: TScHttpStatusCodes);
begin
  CheckInactive;
...
constructor {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Create;
begin
  inherited;
...
destructor {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Destroy;
begin
  try
    if FSecureConnection <> nil then
      SkipAll;
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Abort;
begin
  if FSecureConnection <> nil then
    FSecureConnection.Abort;
end;
...
{$IFNDEF FPC}
{$IFDEF VER22P}
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Read(pv: IntPtr; cb: FixedUInt; pcbRead: PFixedUInt): HResult;
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Read(pv: IntPtr; cb: Longint; pcbRead: PLongint): HResult;
{$ENDIF}
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Read(pv: Pointer; cb: DWord; pcbRead: PDWord): HRESULT;
{$ENDIF}
var
  cbReadBytes: integer;
begin
...
{$IFNDEF FPC}
{$IFDEF VER22P}
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Write(pv: IntPtr; cb: FixedUInt; pcbWritten: PFixedUInt): HResult;
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Write(pv: IntPtr; cb: Longint; pcbWritten: PLongint): HResult;
{$ENDIF}
{$ELSE}
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.Write(pv: Pointer; cb: DWord; pcbWritten: PDWord): HRESULT;
{$ENDIF}
begin
  if pcbWritten <> nil then
    pcbWritten^ := 0;
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.ReadBuffer(const Buffer: TValueArr; Offset, Count: integer): integer;
begin
  Result := InternalRead(Buffer, Offset, Count);
end;

function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.ReadAsString: string;
var
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.ReadAsBytes: TBytes;
var
  Offset, Count: integer;
begin
  SetLength(Result, 0);
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.ReadToStream(Stream: TStream): integer;
var
  Count: integer;
begin
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.RetrieveHeaders(CancellationToken: TScCancellationToken = nil);
var
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetResponseHeader(const HeaderName: string): string;
var
  Idx: integer;
begin
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.WaitForData(MillisecondsTimeout: integer): boolean;
begin
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.ReadChunk(const Buffer: TValueArr; Offset, Count: integer): integer;
var
  ReadCount: integer;
begin
  Result := 0;
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.DoProgress(const Total, Current: Int64);
var
  Cancel: boolean;
begin
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.InternalRead(const Buffer: TValueArr; Offset, Count: integer): integer;
...
procedure {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.SkipAll;
begin
  if FSecureConnection = nil then
    Exit;
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetIsSecure: boolean;
begin
  if FSecureConnection <> nil then
    Result := FSecureConnection.IsSecure
  else
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetStatusCode: TScHttpStatusCode;
begin
  Result := ConvertToStatusCode(FStatusCode);
end;

function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetContentEncoding: string;
begin
  FHeaders.TryGetValue(Entity_Header_ContentEncoding, Result);
end;

function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetContentType: string;
begin
  FHeaders.TryGetValue(Entity_Header_ContentType, Result);
end;

function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetLastModified: TDateTime;
var
  Value: string;
begin
...
function {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}.GetServer: string;
begin
...
class function TScHttpWebResponseHelper.GetSecureConnection(Obj: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}): TScSecureConnection;
begin
  Result := Obj.FSecureConnection;
end;

class procedure TScHttpWebResponseHelper.SetSecureConnection(Obj: {$IFNDEF SBRIDGE}TCRHttpWebResponse{$ELSE}TScHttpWebResponse{$ENDIF}; Value: TScSecureConnection);
begin
