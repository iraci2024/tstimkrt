...
type
  TScDualListEditorForm = class(TScEditorForm)
    Panel2: TPanel;
    SrcLabel: TLabel;
    SrcList: TListBox;
    PanelButtons: TPanel;
...
procedure TScDualListEditorForm.IncludeBtnClick(Sender: TObject);
begin
  MoveSelected(SrcList, DstList, DstList.Items.Count, True);
end;

procedure TScDualListEditorForm.ExcludeBtnClick(Sender: TObject);
begin
  MoveSelected(DstList, SrcList, SrcList.Items.Count, True);
end;

procedure TScDualListEditorForm.IncAllBtnClick(Sender: TObject);
begin
  MoveAll(SrcList, DstList, DstList.Items.Count);
end;

procedure TScDualListEditorForm.ExcAllBtnClick(Sender: TObject);
begin
  MoveAll(DstList, SrcList, SrcList.Items.Count);
end;

procedure TScDualListEditorForm.MoveSelected(Source, Dest: TListBox; Index: Integer;
  SelectNext: boolean);
var
  i, FirstSel: integer;
begin
...
procedure TScDualListEditorForm.MoveAll(Source, Dest: TListBox; Index: Integer);
var
  i: integer;
begin
  for i := 0 to Dest.Items.Count - 1 do
...
procedure TScDualListEditorForm.SetButtons;
var
...
function TScDualListEditorForm.GetFirstSelection(List: TCustomListBox): Integer;
var
  i: integer;
begin
...
procedure TScDualListEditorForm.DoInit;
var
  i, j: integer;
  List: TStringList;
begin
...
procedure TScDualListEditorForm.DstListDragOver(Sender, Source: TObject; X,
...
procedure TScDualListEditorForm.DstListDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
  pos: TPoint;
...
procedure TScDualListEditorForm.SrcListDragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := (Source = DstList) and (DstList.SelCount > 0);
end;

procedure TScDualListEditorForm.SrcListDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var
  pos: TPoint;
  Index: Integer;
...
procedure TScDualListEditorForm.FormResize(Sender: TObject);
begin
  inherited;
  Panel2.Width := (ClientWidth - PanelButtons.Width * 2) div 2;
end;

function TScDualListEditorForm.GetSrcLabelCaption: string;
begin
  Result := 'Source list items';
end;

procedure TScDualListEditorForm.GetSrcListItems(Items: TStrings);
begin
  Items.Clear;
end;

function TScDualListEditorForm.GetDestLabelCaption: string;
begin
  Result := 'Destination list items';
end;

procedure TScDualListEditorForm.GetDstListItems(Items: TStrings);
begin
  Items.Clear;
end;

procedure TScDualListEditorForm.Move(List: TListBox; FromIndex, ToIndex: integer);
begin
...
procedure TScDualListEditorForm.UpBtnClick(Sender: TObject);
var
  SelIndex: integer;
begin
  SelIndex := GetFirstSelection(DstList);
...
procedure TScDualListEditorForm.DownBtnClick(Sender: TObject);
var
  SelIndex: integer;
begin
  SelIndex := GetFirstSelection(DstList);
...
procedure TScDualListEditorForm.DstListKeyDown(Sender: TObject;
...
procedure TScDualListEditorForm.ListClick(Sender: TObject);
begin
  SetButtons;
end;
