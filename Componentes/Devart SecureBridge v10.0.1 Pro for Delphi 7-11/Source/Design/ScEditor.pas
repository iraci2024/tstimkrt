...
{$IFDEF FPC}
  LResources, LCLType,
{$ENDIF}
  ScTypes, ScDesignUtils;

type
  TScEditorClass = class of TScEditorForm;

  TScEditorForm = class(TForm)
    BtnPanel: TPanel;
    btOk: TBitBtn;
    btCancel: TBitBtn;
    imCorner: TImage;
  ...
  protected
    FModified: boolean;
    FolderName: string;
    FScDesignUtilsClass: TScDesignUtilsClass;
  ...
  public
    constructor Create(Owner: TComponent; ScDesignUtilsClass: TScDesignUtilsClass); reintroduce; virtual;
  ...
  /// changes. By default ConfirmCancel is set to True.
    ...
    property ScDesignUtilsClass: TScDesignUtilsClass read FScDesignUtilsClass;
      ...
      if (PropInfo <> nil) // published property
        ...
        case PropList[i].PropType^.Kind of
          ...
          {$ENDIF}
            ...
            if (Obj <> nil) and (Obj.ClassName <> 'TMemoStrings') then begin
...
{$ENDIF}
  if Memo is TMemo then
    ...
    if Memo.ClassName = sSynMemo then
      SetOrdProp(Memo, 'ReadOnly', integer(Value));
end;
...
procedure TScMemo.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited;
...
procedure TScMemo.KeyPress(var Key: Char);
var
  StartI, EndI: integer;
  Text: string;
begin
...
constructor TScEditorForm.Create(Owner: TComponent; ScDesignUtilsClass: TScDesignUtilsClass);
begin
  inherited Create(Owner);
  FScDesignUtilsClass := ScDesignUtilsClass;
  FolderName := ClassName;
  ConfirmCancel := True;
end;

{$IFDEF USE_EXT_MENU}
procedure TScEditorForm.ExtMenuPopup(Sender: TObject);
var
  n, m: DWORD;
  Memo: TWinControl;
...
procedure TScEditorForm.ExtMenuItemClick(Sender: TObject);
begin
...
procedure TScEditorForm.AddMenu(Memo: TWinControl);
var
  Menu: TPopupMenu;
  i: integer;
...
{$IFDEF USE_SYNEDIT}
procedure TScEditorForm.ReplaceMemo(var Memo: TWinControl; DrawGutter: boolean);
type
    ...
    if GetObjectProp(NewMemo, 'Lines').ClassParent <> TStrings then
      ...
      with NewMemo do begin
        ...
        if Memo is TMemo then begin
          ...
          if MemoClass = sSynMemo then
            SetOrdProp(NewMemo, 'Color', integer(TMemo(Memo).Color));
        end;
      ...
      if MemoClass = sSynMemo then begin
        ...
        if DrawGutter then begin
          SetOrdProp(Gutter, 'Visible', integer(True));

          SetOrdProp(Gutter, 'AutoSize', integer(True));
          SetOrdProp(Gutter, 'DigitCount', 2);
          SetOrdProp(Gutter, 'LeftOffset', 0);
          SetOrdProp(Gutter, 'RightOffset', 0);
          SetOrdProp(Gutter, 'ShowLineNumbers', integer(True));
          SetOrdProp(Gutter, 'Width', 1);
        end
        else
          SetOrdProp(Gutter, 'Visible', integer(False));
...
procedure TScEditorForm.ReplaceMemos;
var
  i: integer;
...
begin
{$IFDEF USE_SYNEDIT}
  if not (FUseSynEdit or FScDesignUtilsClass.DBToolsAvailable) then
{$ENDIF}
  begin
    ProcessComponent(Self);
    for i := 0 to Length(MemoArr) - 1 do begin
      ReplaceControl(MemoArr[i], TScMemo);
    {$IFDEF USE_CODE_EDITOR}
      AddMenu(MemoArr[i]);
    {$ENDIF}
    end;
...
procedure TScEditorForm.DoInit;
begin
  Modified := False;
...
procedure TScEditorForm.DoActivate;
begin
end;

procedure TScEditorForm.DoSave;
begin
  SaveControlData;
end;

procedure TScEditorForm.DoFinish;
begin
end;

procedure TScEditorForm.SaveControlData;
begin
end;

{$IFDEF MSWINDOWS}

function TScEditorForm.SaveState: boolean;
var
  Registry: TRegistry;
begin
...
function TScEditorForm.LoadState: boolean;
var
  Registry: TRegistry;
  _left, _top: integer;
begin
...
function TScEditorForm.KeyPath: string;
begin
  Result := '\SOFTWARE\Devart\' + FScDesignUtilsClass.GetProjectName + '\Editors';
end;

{$ENDIF}

{$IFDEF DBTOOLS}
procedure TScEditorForm.ActiveChanged;
begin
  inherited;
...
function TScEditorForm.GetComponent: TComponent;
begin
  Assert(False, 'Must be overriden');
  Result := nil;
end;

procedure TScEditorForm.SetComponent(Value: TComponent);
begin
  Assert(False, 'Must be overriden');
end;

function TScEditorForm.GetLocalComponent: TComponent;
begin
  Assert(False, 'Must be overriden');
  Result := nil;
end;

procedure TScEditorForm.FormShow(Sender: TObject);
{$IFDEF USE_SYNEDIT}
var
...
{$IFDEF USE_SYNEDIT}
  ...
  if FUseSynEdit and (NewSHClass <> nil) then begin
    ...
    SetOrdProp(FSynSQLSyn, 'SQLDialect', FScDesignUtilsClass.SQLDialect);
...
procedure TScEditorForm.FormHide(Sender: TObject);
begin
  DoFinish;
  SaveState;
{$IFDEF USE_SYNEDIT}
...
procedure TScEditorForm.SaveClick(Sender: TObject);
begin
  DoSave;
...
procedure TScEditorForm.CloseClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TScEditorForm.FormCloseQuery(Sender: TObject;
...
procedure TScEditorForm.SetConfirmCancel(const Value: boolean);
begin
  FConfirmCancel := Value;
end;

procedure TScEditorForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
function TScEditorForm.GetModified: boolean;
begin
  Result := FModified;
end;

procedure TScEditorForm.SetModified(Value: boolean);
begin
  FModified := Value;
end;

procedure TScEditorForm.Resize;
begin
  inherited;
...
{$IFDEF DBTOOLS}
procedure TScEditorForm.WMActivate(var Message: TWMActivate);
var
  Control: TWinControl;
begin
