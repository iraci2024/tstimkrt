...
begin
  Result := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.UnicodeToUtf8(Dest, MaxBytes, Source, Cardinal(-1));
end;
...
begin
  Result := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.Utf8ToUnicode(Dest, MaxChars, Source, Cardinal(-1));
end;
...
begin
  ...
  if Len1 > 0 then begin
    ...
    Len2 := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.UnicodeToUtf8(PAnsiChar(Result), Len2 + 1, PWideChar(WS), Len1);
...
begin
  ...
  if Len1 > 0 then begin
    ...
    Len2 := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.Utf8ToUnicode(PWideChar(Result), Len1 + 1, PAnsiChar(S), Len1);
    Dec(Len2); // zero terminator
    if Len2 <> Len1 then
      SetLength(Result, Len2);
  end
