...
{$IFNDEF SBRIDGE}
  ...
  CLRClasses, CRTypes;
  {$ELSE}
  EntityDAC.Common.DacClasses, EntityDAC.Common.Types;
  {$ENDIF}
{$ELSE}
...
{$ENDIF}
  ...
  {$IFDEF ODBC_DRIVER}
    property WalletPath: string read FWalletPath write FWalletPath;
  {$IFDEF MSWINDOWS}
    property WalletReg: string read FWalletReg write FWalletReg;
  {$ENDIF}
  {$ELSE}
...
begin
  ...
  {$IFDEF ODBC_DRIVER}
    TSSLOptions(Dest).FWalletPath := FWalletPath;
  {$IFDEF MSWINDOWS}
    TSSLOptions(Dest).FWalletReg := FWalletReg;
  {$ENDIF}
  {$ENDIF}
...
begin
  ...
  if SslOptions <> nil then
    ...
    {$IFDEF ODBC_DRIVER}
      (SslOptions.FWalletPath = FWalletPath) and
    {$IFDEF MSWINDOWS}
      (SslOptions.FWalletReg = FWalletReg) and
    {$ENDIF}
    {$ENDIF}
