...
{$IFNDEF SBRIDGE}
  ...
  EntityDAC.Common.Vio, EntityDAC.Common.VioSocket;
  {$ENDIF}
{$ELSE}
  ScCLRClasses, ScTypes, ScVio, ScVioSocket;
{$ENDIF}
...
{$IFNDEF SBRIDGE}
  ...
  {$ELSE}
  EntityDAC.Common.Functions, EntityDAC.Common.Base64;
  {$ENDIF}
{$ELSE}
  ScFunctions, ScBase64;
{$ENDIF}

{$IFNDEF EntityDAC}
{$I CRSocketImpl.inc}
{$ELSE}
{$I EntityDAC.Common.SocketImpl.inc}
{$ENDIF}
...
begin
  AErrorDesc := '';
  Result := FEndEvent.WaitFor(AConnectionTimeoutMSec) = wrSignaled;
end;
