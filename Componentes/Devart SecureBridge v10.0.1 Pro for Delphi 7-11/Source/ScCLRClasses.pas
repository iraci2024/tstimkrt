...
{$ENDIF}
{$ELSE}
  EntityDAC.Common.Types, EntityDAC.Common.Functions;
{$ENDIF}
...
begin
  ...
  {$ELSE}
    if TEncoding.Default.CodePage = codepage then
  {$ENDIF}
      Result := theANSIEncoding
    else begin
...
{$IFDEF IS_UTF8}
  ...
  if charCount > 0 then
    Move(PAnsiChar(chars)^, Pointer(Result)^, charCount);
{$ELSE}
  UTF8 := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.AnsiToUtf8(chars);
...
{$ELSE}
  if charCount > 0 then begin
    ...
    UTF8 := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.UTF8Encode(ws);
...
var
  ...
  UTF8: UTF8String;
begin
  UTF8 := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.UTF8Encode(chars);
...
begin
  ...
  UTF8 := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.UTF8Encode(SubChars);
...
{$ELSE}
  ...
  Result := AnsiString({$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.Utf8ToAnsi(UTF8));
{$ENDIF}
end;
...
begin
  ...
  SetLength(UTF8, count);
  Move(bytes[index], PAnsiChar(UTF8)^, count);
  Result := {$IFNDEF EntityDAC}{$IFNDEF SBRIDGE}CRFunctions{$ELSE}ScFunctions{$ENDIF}{$ELSE}EntityDAC.Common.Functions{$ENDIF}.UTF8Decode(UTF8);
end;
...
begin
  ...
  {$IFDEF MSWINDOWS}
    ...
    Win32Check(LongBool(charCount));
  {$ELSE}
    Result := FEncoding.GetString(bytes, index, count);
  {$ENDIF}
  end;
end;
