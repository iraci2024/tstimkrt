...
begin
  ...
  if csd <> nil then
    try
      ...
      with SkinData.SkinManager.SkinCommonInfo do
        si := PaintSection(TempBmp, Sections[ssColHeader], Sections[ssButton], State, csd, ControlRect.TopLeft, CI.FillColor);
  ...
  {$ENDIF}
        ...
        if item.fmt and HDF_SORTDOWN = HDF_SORTDOWN then begin
          ArrowSide := asBottom;
          ArrowIndex := csd.ScrollBtns[ArrowSide].GlyphIndex;
        end
        else
        ...
        if ArrowIndex <> -2 then
          if ArrowIndex >= 0 then
            ArrowSize := MkSize(csd.ma[ArrowIndex].Width + 6, csd.ma[ArrowIndex].Height)
          else
  ...
  {$ENDIF}
        ...
        if ArrowIndex >= 0 then
          DrawSkinGlyph(TempBmp, Point(TextRc.Right + 6, (HeightOf(TextRc) - ArrowSize.cy) div 2), State, 1, csd.ma[ArrowIndex], MakeCacheInfo(TempBmp))
        else
          if ArrowIndex = -1 then begin
            ...
            gR.Bottom := gR.Top + 3;
            State := mini(State, ac_MaxPropsIndex);
            C := csd.gd[si].Props[State].FontColor.Color;
            DrawColorArrow(TempBmp, C, gR, ArrowSide, GetPPI(SkinData));
          end;

        if (State = 2) and CanClickShift(SkinData.SkinIndex, csd) then
...
{$ENDIF}
    ...
    WM_DESTROY, WM_NCDESTROY: if SkinData <> nil then begin
...
{$ENDIF}
    ...
    if Btn.iBitmap >= 0 then begin
      ImgLst := ProcessMessage(TB_GETIMAGELIST, 0, 0);
      if ImgLst <> 0 then begin
        ImageList_GetIconSize(ImgLst, cx, cy);
        ImageList_SetBkColor(ImgLst, CLR_NONE);
        if Btn.fsStyle and $0080 = $0080 then begin // BTNS_WHOLEDROPDOWN
          ...
          if ArrowIndex >= 0 then begin
            ...
            if s <> '' then begin
              ...
              ImageList_Draw(ImgLst, Btn.iBitmap, BtnBmp.Canvas.Handle, (WidthOf(R) - cx - ArrowSize.cx) div 2 + pOffset, 4 + pOffset, 0);
            end
            else begin
            ...
            ImageList_Draw(ImgLst, Btn.iBitmap, BtnBmp.Canvas.Handle, (WidthOf(R) - cx) div 2 + pOffset, 4 + pOffset, 0);
        end
        else begin

          if s = '' then
            ImageList_Draw(ImgLst, Btn.iBitmap, BtnBmp.Canvas.Handle, (WidthOf(R) - cx) div 2 + pOffset, (HeightOf(R) - cy) div 2 + pOffset, 0)
          else
            ImageList_Draw(ImgLst, Btn.iBitmap, BtnBmp.Canvas.Handle, (WidthOf(r) - cx) div 2 + pOffset, 4 + pOffset, 0);
        end;
      end
