...
var
  ScreenPPI: integer;
begin
  ScreenPPI := GetControlPPI(Form);
  if ScreenPPI <> 96 then
    CXFixedFrame := ac_GetSysMetrics(SM_CXPADDEDBORDER, ScreenPPI)
  else
    CXFixedFrame := ac_GetSysMetrics(SM_CXFIXEDFRAME, ScreenPPI);
...
{$ENDIF} // NOWNDANIMATION
        ...
        if not stdMessageHandling then begin
          stdMessageHandling := True;
  {$IFNDEF DELPHI_11}
    {$IFDEF DELPHI_XE2}
          b := Application.MainFormOnTaskbar;
          if fAnimating then begin
...
{$ENDIF}
      ...
      if FDrawClientArea and Form.Showing then begin
        ...
        if Assigned(SkinData.SkinManager) then begin
          ...
          if SkinData.SkinManager.UpdateFontName(Form) then begin
            M := MakeMessage(SM_ALPHACMD, MakeWParam(0, AC_FONTSCHANGED), LPARAM(SkinData.SkinManager), 0);
            AlphaBroadCast(Form, M);
          end;
        end;
    ...
    fsMDIForm: begin
      ...
      case Message.WParam of
        SC_MAXIMIZE: begin
          ...
          FormState := FormState and not FS_ZOOMING;
          if Assigned(SystemMenu) and SystemMenu.VisibleMax then
