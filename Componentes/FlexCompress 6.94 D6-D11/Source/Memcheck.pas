...
var
  MemCheckLogFileName: string = 's:\MemCheck.log';
...
uses
  ...
  SysUtils;
{Because of this uses, SysUtils must be finalized after MemCheck - Which is necessary anyway because SysUtils calls DoneExceptions in its finalization}
...
const
  ...
  CheckWipedBlocksOnTermination   =
    True and WipeOutMemoryOnFreeMem and not (DanglingInterfacesVerified);
...
begin
  Result :=
    ...
    (Old.TotalAllocated <> New.TotalAllocated) or (Old.TotalFree <> New.TotalFree) or
    (Old.FreeSmall <> New.FreeSmall) or (Old.FreeBig <> New.FreeBig) or
    (Old.Unused <> New.Unused) or (Old.Overhead <> New.Overhead) or
    (Old.HeapErrorCode <> New.HeapErrorCode) or (New.TotalUncommitted +
    New.TotalCommitted <> New.TotalAddrSpace) or
...
const
  TObjectVirtualMethodNames: array[1..8] of string =
...
begin
  ...
  try
    ...
    FeedBackStr := FeedBackStr + ' on a FREED instance of ' + T.VMT.ClassName +
      ' (destroyed at ' + TextualDebugInfoForAddress(cardinal(T.DestructionAdress)) +
      ' - had been created at ' + TextualDebugInfoForAddress(
      cardinal(T.CallerAddress[0])) + ')';
...
begin
  try
    ...
    raise EInterfaceFreedInstance.Create(
      'Calling an interface method on an freed Pascal instance')at InterfaceErrorCaller
  except
...
begin
  ...
  while (longint(CurrentFrame) >= longint(StackStart)) and
    (longint(CurrentFrame) < longint(StackMax)) and (Count <= StoredCallStackDepth) do
  begin
...
begin
  ...
  begin
    ...
    begin  //Neither an object nor a string, this is a MUser
      Result := OldMemoryManager.GetMem(Size +
        (SizeOf(TMemoryBlocHeader) + SizeOf(TMemoryBlockFooter)));
      if Result = nil then
        raise OutOfMemory;
...
asm
  ...
  ADD   EBX,4
  @@4:  LEA   ESI, BadInterfaceVMT
  // mettre dans ESI l'adresse du début de MyInterfaceVMT: correct ?????
...
begin
  ...
  NotDestroyedFieldsInfos[NotDestroyedFieldsCount] :=
    TFieldInfo.Create(OwnerClass, FieldPos);
  ...
  begin
      ...
      if (Block.KindOfBlock = MClass) and
        ...
        for i := 1 to (Block.AllocatedSize div 4) - 1 do
          if (PInteger(PChar(P) + i * 4)^ > SizeOf(TMemoryBlocHeader)) and
            ...
            AddFieldInfo(Pointer(PInteger(PChar(P) + i * 4)^ - SizeOf(TMemoryBlocHeader)),
              TObject(P).ClassType, i);
    ...
    begin
      try
        ...
        raise EMemoryLeak.Create('second release of block attempt, already freed')
    ...
    begin
      ...
      begin
        ...
        if (BlocksToShow[Block.KindOfBlock]) and
          ((Block.KindOfBlock <> MClass) or (TObject(PChar(Block) + SizeOf(TMemoryBlocHeader)) is
          InstancesConformingToForReporting)) then
          try
        ...
        if ShowCallStack then
          for i := 1 to StoredCallStackDepth do
            if integer(Block.CallerAddress[i]) > 0 then
              try
                raise EStackUnwinding.Create(S + ' unwinding level ' + chr(Ord('0') + i))
                at Block.CallerAddress[i]
              except
                on EStackUnwinding do ;
              end;
      ...
      begin
        try
          raise EFreedBlockDamaged.Create(
            ...
            TextualDebugInfoForAddress(cardinal(Block.CallerAddress[0])) +
            ' - destroyed at ' + TextualDebugInfoForAddress(cardinal(Block.DestructionAdress)))
...
begin
  ...
  begin
    ...
    if
{$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}
    CurrentUnitInfo.Init = @System.System then
      NewUnitsInfoOrder.Insert(0, CurrentUnitInfoCopy)
    else
    if
{$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}
    CurrentUnitInfo.Init = @SysUtils.SysUtils then
          ...
          {$ENDIF}
{
    ...
    if
{$IFNDEF DELPHI6_OR_LATER}@{$ENDIF}
    CurrentUnitInfo.Init = Pointer(PChar(@Dummy) + 356) then
  ...
  begin
    WriteProcessMemory(ProcessHandle, Pointer(PChar(@UnitsInfo^.UnitInfo^[0]) +
      i * SizeOf(PackageUnitEntry)), NewUnitsInfoOrder[i], SizeOf(PackageUnitEntry),
      BytesWritten);
...
begin
  ...
  begin
    ...
    if TUnitDebugInfos(Units[Pivot]).Addresses[Length(
      TUnitDebugInfos(Units[Pivot]).Addresses) - 1].Address < Address then
      Start := Pivot + 1
    else
    begin
      Result := Units[Pivot];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        CurrentUnit := TUnitDebugInfos.Create(ExtractFileName(
          PChar(NameTbl^[srcfile^._nName - 1])), srcln^._cPair);
        AddUnit(CurrentUnit);

        for k := 0 to pred(srcln^._cPair) do
          CurrentUnit.Addresses[k] :=
            TAddressToLine.Create(integer(PArrayOfPointer(@srcln^._Offset[0])^[k]),
...
begin
  ...
  case Block.KindOfBlock of
    MClass:
      WriteLn(F, 'Instance of ', TObject(PChar(Block) +
        SizeOf(TMemoryBlocHeader)).ClassName);
    MUser:
...
begin
  case Block.KindOfBlock of
    MClass:
      Write(F, '* Instance of ', TObject(PChar(Block) +
        SizeOf(TMemoryBlocHeader)).ClassName);
    MUser:
...
begin
  ...
  if fBlock.KindOfBlock = MClass then
    Result := Result and (TObject(PChar(fBlock) + SizeOf(TMemoryBlocHeader)).ClassName =
      TObject(PChar(Other.Block) + SizeOf(TMemoryBlocHeader)).ClassName);
  ...
  if WasFieldOfAnotherObject then
    Result := Result and (OtherObjectClassName = Other.OtherObjectClassName) and
      (OtherFieldIndex = Other.OtherFieldIndex) and (OtherIsDestroyed =
      Other.OtherIsDestroyed);
      ...
      if IdentifyObjectFields then
        for i := 0 to NotDestroyedFieldsCount - 1 do
          ...
          begin
            ...
            CurrentLeak.fOwnerClassName   :=
              TFieldInfo(NotDestroyedFieldsInfos[i]).OwnerClass.ClassName;
            CurrentLeak.fOtherFieldIndex  :=
              TFieldInfo(NotDestroyedFieldsInfos[i]).FieldIndex;
...
procedure GetBadBlocks(const B: TBlockList;
  const MaxNumberOfBlocks, MaxBlockSize: integer; var StoppedDueToMaxBlock: boolean);
var
  Block: PMemoryBlocHeader;
begin
...
begin
  ...
  begin
    ...
    begin
      for j := LastDisplayedTimeStamp + 1 to ChronogicalInfo.Item(
          i).Block.LastTimeStamp do
  ...
  begin
    ...
    for i := 0 to AllocStatsCount - 1 do
      WriteLn(OutputFile, #9, AllocatedInstances[i], #9#9,
        AllocatedObjectsClasses[i].InstanceSize, #9#9, AllocatedObjectsClasses[i].ClassName);
  ...
  if KeepMaxMemoryUsage then
    WriteLn(OutputFile, #13#10'*** Biggest memory usage was: ',
      MaxMemoryUsage, ' ***' + #13#10#13#10#13#10);
  ...
  begin
    ...
    begin
      ...
      Write(OutputFile, CallStackTextualRepresentation(
        BadBlocks.Item(i).CallerAddress, #9#9#9));
...
begin
  ...
  begin
    ...
    if U <> nil then
      Result := 'Module ' + U.Name + RoutineWhichContainsAddress(AddressInDebugInfos) +
        U.LineWhichContainsAddress(AddressInDebugInfos)
    else
