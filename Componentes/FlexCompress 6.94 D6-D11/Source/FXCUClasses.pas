...
uses
  ...
  Registry, TypInfo, FXCUSystem, FXCUSysUtils;
...
begin
  if FAdapterCodePage = 0 then
    Result := FXCUSystem.DefaultSystemCodePage
  else
...
begin
  Item1 := @FList^[Index1];
  Item2 := @FList^[Index2];
  Temp := {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(Item1^.FString);
  Integer(Item1^.FString) := {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(Item2^.FString);
  Integer(Item2^.FString) := Temp;
  Temp := {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(Item1^.FObject);
  Integer(Item1^.FObject) := {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(Item2^.FObject);
...
begin
  if {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(TWideComponentHelper(Item).FComponent) < 
     {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(Target) then
    Result := -1
  else if {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(TWideComponentHelper(Item).FComponent) > 
          {$IFNDEF WIN64}Integer{$ELSE}NativeInt{$ENDIF}(Target) then
    Result := 1
  else
    Result := 0;
