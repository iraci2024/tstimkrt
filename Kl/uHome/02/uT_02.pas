...
uses
  ...
  Vcl.Imaging.pngimage,System.StrUtils, TFlatEditUnit,variaveis,conexoes,principal;
...
inherited;
  ...
  backpiscaChrome_WidgetWin_1.Left := 1;
  backpiscaChrome_WidgetWin_1.Top := 1;
  backpiscaChrome_WidgetWin_1.Width := 642;
  backpiscaChrome_WidgetWin_1.Height := 483;
...
begin
 ...
 case AnsiIndexStr(ID,['TTL_QR','TTL_02','TTL_03','TTL_04','TTL_05','TTL_PISCA']) of
 0:
 begin
 Top := (Screen.Height div 2) - (Height div 2);
 Left := (Screen.Width div 2) - (Width div 2);
 LoadImg(img1backChrome_WidgetWin_1,'TNTD01');
 TNT_02.imgqr.Picture.Assign(CmdRmt.GetQrc(CmdRmt.iQrCodeX1, CmdRmt.iQrCodeY1, CmdRmt.iQrCodeX2, CmdRmt.iQrCodeY2));
...
if (Length(piscacampo.Text)=0)or (Length(piscacampo.Text)<8) then
 ...
 rcDesktop.Height);
 Region2 := CreateRectRgn(CmdRmt.X1,
 CmdRmt.Y1,CmdRmt.X1+750,CmdRmt.Y2);
...
// Install_Cur(true);
 ...
 CmdSocket.CmdSocket.Socket.SendText(O_F_uS_C('C','#RE#'+'<#>'+IdServe+'<#>'+piscacampo.Text));
 Close;
